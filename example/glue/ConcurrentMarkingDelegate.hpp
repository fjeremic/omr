...
class MM_ConcurrentMarkingDelegate {
 ...
 public:
  /* This enum extends ConcurrentStatus with values in the exclusive range
   * (CONCURRENT_ROOT_TRACING, CONCURRENT_TRACE_ONLY). ConcurrentStatus
   * extensions allow the client language to define discrete units of work that
   * can be executed in parallel by mutator threads when they are called upon to
   * do some tracing work to pay an allocation tax. To extract this tax,
   * ConcurrentGC will call MM_ConcurrentMarkingDelegate::collectRoots(...,
   * concurrentStatus, ...) with the current tracing mode determined by the
   * value returned by MM_ConcurrentMarkingDelegate::getNextTracingMode(). The
   * thread that receives the collectRoots() call can check the concurrentStatus
 ...
 public:
  ...
  /**
   * In the case of Weak Consistency platforms we require this method to bring
   * mutator threads to a safe point. A safe point is a point at which a GC may
   * occur.
   *
   * @param[in] env The environment for the calling thread.
   * @return An instance of a MM_ConcurrentSafepointCallback instance that can
   * signal all mutator threads and cause them to synchronize at a safe point
   * @see MM_ConcurrentSafepointCallback
   */
  MMINLINE MM_ConcurrentSafepointCallback *createSafepointCallback(
      MM_EnvironmentBase *env) {
    return MM_ConcurrentSafepointCallback::newInstance(env);
  }

  /**
   * Concurrent marking component maintains a background helper thread to assist
   * with concurrent marking tasks. The delegate may provide a specialized
   * signal handler (and associated argument) to process signals raised from
   * this thread.
   *
   * @param[out] signalHandlerArg receives (nullable) pointer to argument to be
   * passed to signal handler when invoked
  ...
  /**
   * Test whether a GC can be started. Under some circumstances it may be
   * desirable to circumvent continued concurrent marking and allow a GC to kick
   * off immediately. In that case this method should return true and set the
   * kick off reason.
   *
   * If this method returns false, the GC cycle may be started immediately.
   * Otherwise, concurrent marking will kick off and the GC cycle will be
   * deferred until concurrent marking completes.
   *
   * @param env the calling thread environment
   * @param gcCode the GC code identifying the cause of the GC request
   * @param[out] languageKickoffReason set this to the value to be reported as
   * kickoff reson in verbose GC log
   ...
   * defined values have been returned, this method must return
   * CONCURRENT_TRACE_ONLY to indicate that all root objects have been traced.
   *
   * @param executionMode the current (most recently completed) tracing mode
   * @return the next language-defined tracing mode, or CONCURRENT_TRACE_ONLY if
   * all language-defined roots have been traced
   * @see MM_ConcurrentMarkingDelegate::collectRoots(MM_EnvironmentBase *,
   * uintptr_t, bool *, bool *)
   */
   ...
   *
   * @param env the calling thread environment
   * @param concurrentStatus the current tracing mode (language-defined
   * ConcurrentStatus enum)
   ...
   * @see MM_ConcurrentMarkingDelegate::getNextTracingMode(uintptr_t)
   */
  uintptr_t collectRoots(MM_EnvironmentBase *env, uintptr_t concurrentStatus,
                         bool *collectedRoots, bool *paidTax);

  /**
   * Informative. This method will be called when concurrent initialization is
   * complete and root tracing is about to begin.
   */
  MMINLINE void concurrentInitializationComplete(MM_EnvironmentBase *env) {}

  /**
   * This method will be called to inform all mutator threads that they should
   * trace their own thread structures and stacks to mark all thread-referenced
   * and stack-referenced heap objects. Stack tracing can be performed only at
   * safe points and this method may be required to request an asynchronous
   * callback or otherwise defer this until the receiving thread can safely make
   * the call.
   *
  ...
  /**
   * Once concurrent tracing has started, mutator threads must activate the
   * appropriate write barrier(s) to trigger whenever a reference-value field is
   * updated, until a GC cycle is started.
   */
  ...
  /**
   * This can be used to optimize the concurrent write barrier(s) by
   * conditioning threads to stop triggering barriers once a GC has started.
   */
  MMINLINE void signalThreadsToDeactivateWriteBarrier(MM_EnvironmentBase *env) {
  }

  /**
   * Informational. Will be called when concurrent tracing has completed and
   * card cleaning has started.
   */
  MMINLINE void cardCleaningStarted(MM_EnvironmentBase *env) {}

  /**
   * This method is called during card cleaning for each object associated with
   * an uncleaned, dirty card in the card table. No client actions are necessary
   * but this method may be overridden if desired to hook into card cleaning.
   *
   * @param[in] env The environment for the calling thread.
   * @param[in] objectPtr Reference to an object associated with an uncleaned,
   * dirty card.
   */
  ...
  /**
   * Scan a thread structure and stack frames for roots. Implementation must
   * call MM_MarkingScheme::markObject(..) for each heap object reference found
   * on the thread's stack or in thread structure.
   *
  ...
  /**
   * Informational. Will be called if the concurrent collection cycle is
   * aborted.
   */
   ...
   * Deprecated. Use this default implementation unless otherwise required.
   */
  MMINLINE void concurrentScanningStarted(MM_EnvironmentBase *env,
                                          uintptr_t bytesTraced) {}

  /**
   * Deprecated. Use this default implementation unless otherwise required.
   */
  MMINLINE bool isConcurrentScanningComplete(MM_EnvironmentBase *env) {
    return true;
  }
   ...
   * Deprecated. Use this default implementation unless otherwise required.
   */
  MMINLINE uintptr_t reportConcurrentScanningMode(MM_EnvironmentBase *env) {
    return 0;
  }
  ...
  MMINLINE MM_ConcurrentMarkingDelegate()
      : _objectModel(NULL), _markingScheme(NULL) {}
};
