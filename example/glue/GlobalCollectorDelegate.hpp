...
class MM_GlobalCollectorDelegate {
 ...
 public:
   ...
   */
  bool initialize(MM_EnvironmentBase *env, MM_GlobalCollector *globalCollector,
   ...
   * @param env environment for calling thread
   */
  void masterThreadGarbageCollectFinished(MM_EnvironmentBase *env,
                                          bool compactedThisCycle) {}
  ...
  bool heapAddRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace,
                    UDATA size, void *lowAddress, void *highAddress) {
    return true;
  }
   ...
   */
  CompactPreventedReason checkIfCompactionShouldBePrevented(
