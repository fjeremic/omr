...
public:
   ...
   * any root object, for example, a Java Class object. This method is called to determine whether and object is
   ...
   * scavenger forwarding slot, which may have been overwritten in the original copy process, and backing out changes to
   * object flags in the forwarding slot. With the exception of the scavenger forwarding slot, all of the other bits of
   * the original copied object should be intact.
   *
   ...
   * must first determine whether or not the compressed slot adjacent to the scavenger forwarding slot may contain an
   * object reference. In that case, this implementation must:
   *
   * 1- get the preserved adjacent slot value from the forwarded header (originalForwardedHeader->getPreservedOverlap())
   * 2- expand the preserved adjacent slot value to obtain an uncompressed pointer P to the referenced object (see
   * GC_SlotObject) 3- test whether the referenced object has object alignment and is in new space or tenure space (stop
   * otherwise) 4- instantiate a new MM_ForwardedHeader instance F using the referenced object pointer 5- test whether F
   * is a reverse forwarded object (F.isReverseForwardedPointer(), stop otherwise) 6- call
   * originalForwardedHeader->restoreDestroyedOverlap(T), where T is the compressed pointer for P (see GC_SlotObject)
   *
   * @param[in] env The environment for the calling thread.
   * @param[in] forwardedObject A forwarded header containing the original and destination addresses of the object.
  ...
  /**
   * After aborted Concurrent Scavenger, handle indirect object references (off heap structures associated with object
   * with refs to Nursery). Fixup should update slots to point to the forwarded version of the object and/or remove self
   * forwarded bit in the object itself.
   */
