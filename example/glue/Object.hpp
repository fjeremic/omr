...
public:
  ...
  explicit ObjectHeader(ObjectSize sizeInBytes, ObjectFlags flags)
  {
    assign(sizeInBytes, flags);
  }

  ObjectSize sizeInBytes() const
  {
    return _value >> SIZE_SHIFT;
  }

  void sizeInBytes(ObjectSize value)
  {
    assign(value, flags());
  }
  ...
  {
    return (ObjectFlags)_value;
  }

  void flags(ObjectFlags value)
  {
    assign(sizeInBytes(), value);
  }

  void assign(ObjectSize sizeInBytes, ObjectFlags flags)
  {
    _value = (sizeInBytes << SIZE_SHIFT) | flags;
  }

  RawObjectHeader raw() const
  {
    return _value;
  }

  void raw(RawObjectHeader raw)
  {
    _value = raw;
  }
...
public:
  ...
  size_t sizeOfSlotsInBytes() const
  {
    return header.sizeInBytes() - sizeof(ObjectHeader);
  }

  size_t slotCount() const
  {
    return sizeOfSlotsInBytes() / sizeof(Slot);
  }
  ...
  const Slot* cbegin() const
  {
    return begin();
  }

  const Slot* cend() const
  {
    return end();
  }
