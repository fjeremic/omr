...
private:
  /**
   * OMR requires that the language reserve the least significant byte in the
   * first fomrobject_t slot of an object to record object flag bits used in
   * generational and compacting garbage collectors.
   *
   * This constraint may be removed in future revisions. For now,
   * _objectHeaderSlotOffset must be zero as it represents the fomrobject_t
   * offset to the object header slot containing the OMR flag bits.
   *
   * The _objectHeaderSlotFlagsShift is the right shift required to bring the
   * flags byte in the object header slot into the least significant byte. For
   * the time being this shift must be zero.
   *
   * The _objectHeaderSlotSizeShift is unique to this example (transparent to
   * OMR). It is used to extract example object size from the object header
   * slot.
   */
...
public:
   ...
   * generational GC is configured for the build
   * (--enable-OMR_GC_MODRON_SCAVENGER in configure_includes/configure_*.mk).
   * They typically involve a MM_ForwardedHeader object, and allow information
   * about the forwarded object to be obtained.
   */
