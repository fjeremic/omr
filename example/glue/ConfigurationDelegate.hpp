...
public:
   ...
   * @param writeBarrierType The write barrier type selected for the GC
   * configuration
  ...
  /**
   * For standard gc policies, an opaque (to OMR) object may be attached to each
   * heap region to maintain language-specific per region metadata.
   *
   * If the delegate allocates a region extension object in this method, it must
   * set region->_heapRegionDescriptorExtension to point to the object.
   *
   * @param env The calling thread environment
   ...
   */
  bool
  initializeHeapRegionDescriptorExtension(MM_EnvironmentBase *env,
   ...
   * region->_heapRegionDescriptorExtension, it must be deallocated in this
   * method and region->_heapRegionDescriptorExtension should be cleared (NULL).
   *
   * @param env The calling thread environment
   ...
   * may perform any additional heap-related initialization at this point.
   * @param env The calling thread environment
  ...
  /**
   * The OMR GC preallocates a pool of MM_EnvironmentBase subclass instances on
   * startup. This method is called to allow GC clients to determine the number
   * of pooled instances to preallocate.
   * @param env The calling thread environment
   * @return The number of pooled instances to preallocate, or 0 to use default
   * number
   */
  ...
  /**
   * The OMR segregated GC requires GC clients to specify a distribution of
   * segment sizes with which it will partition the heap. This method is called
   * to retrieve the size distribution metadata. It is required only if the
   * segregated heap is enabled.
   * @param env The calling thread environment
   * @return A pointer to segregated heap size distribution metadata, or NULL if
   * not using segregated heap
   */
  ...
  /**
   * Perform GC client-specific initialization for a new MM_EnvironmentBase
   * subclass instance.
  ...
  /**
   * The OMR GC preallocates a pool of collection helper threads. This method is
   * called to allow GC clients to determine the maximum number of collection
   * helper threads to run during GC cycles.
   * @param env The calling thread environment
  ...
  /**
   * This method is called to determine when to start tracking heap
   * fragmentation, which is initially inhibited to allow the heap to grow to a
   * stable operational size. Frequent transitions true->false will limit the
   * quality of heap fragmentation stats.
   *
