...
void MM_ScavengerDelegate::workerSetupForGC_clearEnvironmentLangStats(
    MM_EnvironmentBase *env) {
  /* Do nothing for now */
}

void MM_ScavengerDelegate::reportScavengeEnd(MM_EnvironmentBase *envBase,
                                             bool scavengeSuccessful) {
  /* Do nothing for now */
}

void MM_ScavengerDelegate::mergeGCStats_mergeLangStats(
    MM_EnvironmentBase *envBase) {
  /* Do nothing for now */
}

void MM_ScavengerDelegate::masterThreadGarbageCollect_scavengeComplete(
    MM_EnvironmentBase *envBase) {
  /* Do nothing for now */
}

void MM_ScavengerDelegate::masterThreadGarbageCollect_scavengeSuccess(
    MM_EnvironmentBase *envBase) {
  /* Do nothing for now */
}
...
GC_ObjectScanner *
MM_ScavengerDelegate::getObjectScanner(MM_EnvironmentStandard *env,
...
bool MM_ScavengerDelegate::hasIndirectReferentsInNewSpace(
...
bool MM_ScavengerDelegate::scavengeIndirectObjectSlots(
    MM_EnvironmentStandard *env, omrobjectptr_t objectPtr) {
...
void MM_ScavengerDelegate::backOutIndirectObjectSlots(
    MM_EnvironmentStandard *env, omrobjectptr_t objectPtr) {
  /* This method must be implemented if an object may hold any object references
   * that are live but not reachable by traversing the reference graph from the
   * root set or remembered set. In that case, this method should call
...
void MM_ScavengerDelegate::reverseForwardedObject(
  ...
  if (forwardedHeader->isForwardedPointer()) {
    ...
    originalObject->header.assign(
        (ObjectSize)objectModel->getConsumedSizeInBytesWithHeader(
            forwardedObject),
        (uint8_t)objectModel->getObjectFlags(forwardedObject));

#if defined(OMR_GC_COMPRESSED_POINTERS)
    if (env->compressObjectReferences()) {
      /* Restore destroyed overlapped slot in the original object. This slot
       * might need to be reversed as well or it may be already reversed - such
       * fixup will be completed at in a later pass.
       */
