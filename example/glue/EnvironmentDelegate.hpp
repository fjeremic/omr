...
/**
 * The GC_Environment class is opaque to OMR and may be used by the client
 * language to maintain language-specific information relating to a OMR VM
 ...
 *
 * This (example) environment delegate implements a simplistic VM access
 * framework that is not pre-emptive; that is, it relies on threads that are
 * holding non- exclusive VM access to check frequently (sub-millisecond)
 * whether any other thread is requesting exclusive VM access and release
 * non-exclusive VM access immediately in that event. Continuity of VM access
 * can be ensured by reacquiring non-exclusive VM access immediately after
 * releasing it.
 */
...
public:
   ...
   */
  static OMR_VMThread *attachVMThread(OMR_VM *omrVM, const char *threadName,
                                      uintptr_t reason);
   ...
   *
   * @param omrVM Points to OMR VM structure
   * @param omrVMThread Points to OMR_VMThread structure binding calling thread
   * to OMR VM
   * @param reason Language-defined value to pass to environment delegate
   */
  static void detachVMThread(OMR_VM *omrVM, OMR_VMThread *omrVMThread,
                             uintptr_t reason);
  ...
  /**
   * Flush any local material relating to GC here. This is called before a GC
   * cycle begins, and can be used to make local GC_Environment content
   * available to the upcoming GC as required.
   *
   * This is informational. OMR does not require any specific action to be
   * implemented.
   *
   * @see GC_Environment
   *
  ...
  /**
   * Set or clear the transient master GC status on this thread. This thread
   * obtains master status when isMasterThread is true and relinquishes it when
   * isMasterThread is false.
   *
   * This is informational. OMR does not require any specific action to be
   * implemented.
   *
   * @param isMasterThread true if thread is acquiring master status, false if
   * losing it
   */
  ...
  /**
   * Acquire shared VM access. Threads must acquire VM access before accessing
   * any OMR internal structures such as the heap. Requests for VM access will
   * be blocked if any other thread is requesting or has obtained exclusive VM
   * access until exclusive VM access is released.
   *
   * This implementation is not pre-emptive. Threads that have obtained shared
   * VM access must check frequently whether any other thread is requesting
   * exclusive VM access and release shared VM access as quickly as possible in
   * that event.
   */
   ...
   * Check whether another thread is requesting exclusive VM access. This method
   must be
   * called frequently by all threads that are holding shared VM access if the
   VM access framework
   * is not pre-emptive. If this method returns true, the calling thread should
   release shared
  ...
  /**
   * Release exclusive VM acccess. If no other thread is waiting for exclusive
   * VM access this method will notify all threads waiting for shared VM access
   * to continue and acquire shared VM access.
   */
   ...
   * involve nothing more than transferring OMR_VMThread::exclusiveCount from
   * the owning thread to the another thread that thereby assumes exclusive
   * access. Implement if this kind of collaboration is required.
   *
