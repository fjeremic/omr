...
extern "C"
{
  ...
  int omr_main_entry(int argc, char** argv, char** envp)
  {
    ...
    /* Initialize the VM */
    omr_error_t rc =
      OMR_Initialize_VM(&exampleVM._omrVM, &omrVMThread, &exampleVM, NULL);
    Assert_MM_true(OMR_ERROR_NONE == rc);

    /* Set up the vm access mutex */
    intptr_t rw_rc = omrthread_rwmutex_init(
      &exampleVM._vmAccessMutex, 0, "VM exclusive access");
    ...
    while (true) {
      ...
      omrobjectptr_t obj =
        (omrobjectptr_t)OMR_GC_AllocateObject(omrVMThread, &allocationModel);
      if (NULL != obj) {
        RootEntry rEntry = { "root1", obj };
        RootEntry* entryInTable =
          (RootEntry*)hashTableAdd(exampleVM.rootTable, &rEntry);
    ...
    MM_AllocationStats* allocationStats =
      allocationInterface->getAllocationStats();
    omrtty_printf("thread allocated %d tlh bytes, %d non-tlh bytes, from %d "
                  "allocations before NULL\n",
                  allocationStats->tlhBytesAllocated(),
                  allocationStats->nontlhBytesAllocated(),
                  allocatedCount);

    /* Force GC to print verbose system allocation stats -- should match thread
     * allocation stats from before GC */
    ...
    /* Can not assert the value of rc since the portlibrary and trace engine
     * have been shutdown */
