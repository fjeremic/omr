...
int omr_main_entry(int argc, char **argv, char **envp) {
  ...
  intptr_t rw_rc = omrthread_rwmutex_init(&exampleVM._vmAccessMutex, 0,
                                          "VM exclusive access");
  Assert_MM_true(J9THREAD_RWMUTEX_OK == rw_rc);

  /* Initialize root table */
  exampleVM.rootTable =
      hashTableNew(exampleVM._omrVM->_runtime->_portLibrary, OMR_GET_CALLSITE(),
  ...
  omrtty_printf("thread allocated %d tlh bytes, %d non-tlh bytes, from %d "
                ...
                allocationStats->nontlhBytesAllocated(), allocatedCount);
  ...
  /* Can not assert the value of rc since the portlibrary and trace engine have
   * been shutdown */
