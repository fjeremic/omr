...
private:
  ...
  FILE *_publicFile; /**< FILE handle for public header file.  close in tearDown
                        if non-NULL */
  const char *_privateFileName; /**< value (or substring) from XML parsing so no
                                   need to free */
  FILE *_privateFile; /**< FILE handle for prviate header file.  close in
                         tearDown if non-NULL */
...
private:
  ...
  void writeEventToPublicHeader(const char *name, const char *description,
                                const char *condition, const char *structName,
                                const char *reverse, pugi::xml_node event);
  void writeEventToPrivateHeader(const char *name, const char *condition,
...
public:
  HookGen()
      : _eventNum(1), _fileName(NULL), _publicFileName(NULL), _publicFile(NULL),
        _privateFileName(NULL), _privateFile(NULL), _verbose(false) {}
