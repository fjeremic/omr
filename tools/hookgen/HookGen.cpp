...
static const char*
getChildTextOrElse(pugi::xml_node node,
                   const char* name,
...
HookGen::completePrivateHeader(const char* structName)
{
  ...
  if (NULL != macroName) {
    ...
    fprintf(
      _privateFile, "\tstruct OMREventInfo4Dump infos4Dump[%d];\n", _eventNum);
 ...
 */
void
HookGen::writeEventToPublicHeader(const char* name,
                                  ...
                                  pugi::xml_node event)
{
  ...
  if (NULL != exampleBuf) {
    ...
    int length =
      sprintf(exampleBuf, (char*)example, name, description, name, structName);
    exampleBuf[length] = '\0';
  ...
  if (NULL == reverse) {
    ...
    fprintf(_publicFile,
            "#define %s (%d | J9HOOK_TAG_REVERSE_ORDER)\n",
            name,
            thisEventNum);
  }
 ...
 */
void
HookGen::writeEventToPrivateHeader(const char* name,
                                   const char* condition,
                                   const char* once,
                                   int sampling,
                                   const char* structName,
...
HookGen::writeEvent(pugi::xml_node event)
{
  ...
  writeEventToPublicHeader(
    name, description, condition, structName, reverse, event);
...
#if defined(DEBUG)
  fprintf(stderr,
          "Processing %s into public header %s and private header %s\n",
          _fileName,
          publicFileName,
          privateFileName);
#endif /* DEBUG */
