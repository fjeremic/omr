...
static const char* getChildTextOrElse(pugi::xml_node node, const char* name, const char* defaultText)
{
  pugi::xml_node child = node.child(name);
 ...
 */
void HookGen::writeEventToPublicHeader(const char* name, const char* description, const char* condition,
 ...
 */
void HookGen::writeEventToPrivateHeader(
  const char* name, const char* condition, const char* once, int sampling, const char* structName, pugi::xml_node event)
{
  if (NULL != condition) {
  ...
  if (0 == sampling) {
    fprintf(_privateFile,
      "\t\t(*J9_HOOK_INTERFACE(hookInterface))->J9HookDispatch(J9_HOOK_INTERFACE(hookInterface), %s%s, &eventData); "
      "\\\n",
      name, once == NULL ? "" : " | J9HOOK_TAG_ONCE");
  } else {
    fprintf(_privateFile,
      "\t\t(*J9_HOOK_INTERFACE(hookInterface))->J9HookDispatch(J9_HOOK_INTERFACE(hookInterface), %s%s%s%d, "
      "&eventData); \\\n",
 ...
 */
RCType HookGen::createFiles(const char* publicFileName, const char* privateFileName)
{
  RCType rc = RC_OK;
...
#if defined(DEBUG)
  fprintf(
    stderr, "Processing %s into public header %s and private header %s\n", _fileName, publicFileName, privateFileName);
#endif /* DEBUG */
...
RCType startHookGen(int argc, char* argv[])
{
  ...
  if (hookGen.isVerbose()) {
    fprintf(stderr, "Processed %s to create public header %s and private header %s\n", hookGen.getFileName(),
      hookGen.getPublicFileName(), hookGen.getPrivateFileName());
  }
