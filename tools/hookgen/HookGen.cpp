...
RCType HookGen::startPrivateHeader() {
  ...
  if (NULL != macroName) {
    ...
    fprintf(_privateFile,
            "/* This file should be included by the IMPLEMENTOR of the hook "
            "interface\n");
    fprintf(_privateFile,
...
void HookGen::writeEventToPublicHeader(
  ...
  const char *example =
      ...
      "\n\n"
      "\tstatic void\n"
      "\teventOccurred(J9HookInterface** hookInterface, UDATA eventNum, void* "
      "voidData, void* userData)\n"
      "\t{\n"
...
void HookGen::writeEventToPrivateHeader(const char *name, const char *condition,
  ...
  if (NULL == once) {
    ...
    fprintf(_privateFile,
            "\t\t/* always trigger this 'report once' event, so that it will "
            "be disabled after this point */ \\\n");
  }
