...
static const char *getChildText(pugi::xml_node node, const char *name) {
...
static const char *getChildTextOrElse(pugi::xml_node node, const char *name,
 ...
 */
char *HookGen::getHeaderGateMacro(const char *fileName) {
 ...
 */
RCType HookGen::startPublicHeader(const char *declarations) {
  RCType rc = RC_OK;
 ...
 */
RCType HookGen::completePublicHeader() {
  RCType rc = RC_OK;
  char *macroName = getHeaderGateMacro(_publicFileName);
  if (NULL != macroName) {
 ...
 */
RCType HookGen::completePrivateHeader(const char *structName) {
  RCType rc = RC_OK;
  char *macroName = getHeaderGateMacro(_privateFileName);
  if (NULL != macroName) {
 ...
 */
void HookGen::writeComment(FILE *file, char *text) {
  fprintf(file, "/* %s */\n", text);
}
 ...
 */
void HookGen::writeEventToPublicHeader(
    const char *name, const char *description, const char *condition,
    const char *structName, const char *reverse, pugi::xml_node event) {
  int thisEventNum = _eventNum++;
  const char *example = "%s\n"
                        "%s\n\n"
                        "Example usage:\n"
                        "\t(*hookable)->J9HookRegisterWithCallSite(hookable, "
                        "%s, eventOccurred, OMR_GET_CALLSITE(), NULL);"
                        "\n\n"
                        "\tstatic void\n"
                        "\teventOccurred(J9HookInterface** hookInterface, "
                        "UDATA eventNum, void* voidData, void* userData)\n"
                        "\t{\n"
  ...
  if (NULL != exampleBuf) {
    ...
    int length = sprintf(exampleBuf, (char *)example, name, description, name,
                         structName);
    exampleBuf[length] = '\0';
  ...
  for (pugi::xml_node data = event.child("data"); data;
 ...
 */
void HookGen::writeEventToPrivateHeader(const char *name, const char *condition,
                                        const char *once, int sampling,
  ...
  if (0 == sampling) {
    fprintf(_privateFile,
            "\t\t(*J9_HOOK_INTERFACE(hookInterface))->J9HookDispatch(J9_HOOK_"
            "INTERFACE(hookInterface), %s%s, &eventData); \\\n",
            name, once == NULL ? "" : " | J9HOOK_TAG_ONCE");
  } else {
    fprintf(_privateFile,
            "\t\t(*J9_HOOK_INTERFACE(hookInterface))->J9HookDispatch(J9_HOOK_"
            "INTERFACE(hookInterface), %s%s%s%d, &eventData); \\\n",
            name, once == NULL ? "" : " | J9HOOK_TAG_ONCE", " | ",
            sampling << 16);
  }
  for (pugi::xml_node data = event.child("data"); data;
    ...
    if (data.attribute("return").as_bool()) {
      fprintf(_privateFile,
              "\t\t(arg_%s) = eventData.%s; /* return argument */ \\\n",
              data.attribute("name").as_string(),
              data.attribute("name").as_string());
    }
  }
  ...
  if (NULL == once) {
    fprintf(_privateFile,
            "\t\tif (J9_EVENT_IS_HOOKED(hookInterface, %s)) { \\\n", name);
  } else {
    fprintf(_privateFile, "\t\t/* always trigger this 'report once' event, so "
                          "that it will be disabled after this point */ \\\n");
  }
 ...
 */
void HookGen::writeEvent(pugi::xml_node event) {
  ...
  writeEventToPublicHeader(name, description, condition, structName, reverse,
                           event);
 ...
 * @NOTE caller is responsible for freeing memory
 */
char *HookGen::getAbsolutePath(const char *filename) {
 ...
 */
FILE *HookGen::createFile(char *path, const char *fileName) {
 ...
 * @NOTE caller is responsible for freeing memory
 */
const char *HookGen::getActualFileName(const char *fileName) {
 ...
 */
RCType HookGen::createFiles(const char *publicFileName,
                            const char *privateFileName) {
  RCType rc = RC_OK;
...
RCType HookGen::processFile() {
  ...
  for (pugi::xml_node event = node.child("event"); event;
...
#if defined(DEBUG)
  fprintf(stderr,
          "Finished procssing %s into public header %s and private header %s\n",
          _fileName, publicFileName, privateFileName);
#endif /* DEBUG */
...
void HookGen::displayUsage() { fprintf(stderr, "hookgen [-v] fileName\n"); }

RCType HookGen::parseOptions(int argc, char *argv[]) {
...
RCType startHookGen(int argc, char *argv[]) {
  ...
  if (hookGen.isVerbose()) {
    fprintf(stderr,
            "Processed %s to create public header %s and private header %s\n",
            hookGen.getFileName(), hookGen.getPublicFileName(),
            hookGen.getPrivateFileName());
  }
