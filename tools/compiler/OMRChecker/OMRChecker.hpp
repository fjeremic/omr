...
class OMRClassCheckingVisitor
 ...
 public:
  ...
  void VerifyTypeStructure() {
    ...
    for (std::map<CXXRecordDecl *, bool>::iterator I = Types.begin(),
      ...
      if (extensible && !getAssociatedConcreteType(Type)) {
        trace(
            "xxxx Issue diagnostic because there's no associated concrete "
            "type");
        continue;
      }
  ...
  const CXXRecordDecl *getAssociatedConcreteType(const CXXRecordDecl *decl) {
    ...
    if (isOMRConcreteType(decl)) return decl;
 ...
 public:
  ...
  bool VisitCXXMemberCallExpr(CXXMemberCallExpr *call) {
    ...
    if (receiver->isImplicitCXXThis()) {
      ...
      if (isSelfCall(call)) {
        ...
        if (isAllowedSelflessCall(call)) return true;
      ...
      if (thisExpr) {
        ...
        unsigned diagID =
            diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
                                       "Use of this in an extensible class can "
  ...
  bool isAllowedSelflessCall(CXXMemberCallExpr *call) {
    ...
    if ((memberFunc = dyn_cast<MemberExpr>(call->getCallee())) &&
      ...
      trace(
          "isAllowedSelflessCall: True because it's a qualified base class "
          "member function");
      return true;
    }
  ...
  bool computeCallInformation(const CXXMemberCallExpr *call,
    ...
    if (!calleeMostDerived) {
      trace(
          "Didn't find a most derived type for the callee class. Assuming not "
          "correct self call");
      return false;
    }
    ...
    if (!callerMethod) {
      trace(
          "Didn't find a caller method... Assuming not correct self call. Call "
          "node: ");
      call->dump();
      return false;
    }
    ...
    if (!calleeMostDerived) {
      trace(
          "Didn't didn't find the caller method decl. Assuming not correct "
          "self call");
      return false;
    }
  ...
  bool callerAndCalleeHaveSameMDT(CXXMemberCallExpr *call) {
    ...
    if (computeCallInformation(call, calleeMostDerived, callerMostDerived)) {
      if (calleeMostDerived == callerMostDerived) return true;
    }
    return false;
  }
