    ...
    const CXXRecordDecl* getAssociatedConcreteType(const CXXRecordDecl* decl)
    {
        ...
        if (isExtensible(decl)) {
            ...
            while (concrete) {
                ...
                if (concrete)
                    trace("==== new concrete  " << concrete->getQualifiedNameAsString() << " for " << decl->getQualifiedNameAsString());
            }
...
static CXXThisExpr* getThisExpr(Expr* E)
{
    while (true) {
        ...
        if (ImplicitCastExpr* ICE = dyn_cast<ImplicitCastExpr>(E)) {
            if (ICE->getCastKind() == CK_NoOp || ICE->getCastKind() == CK_LValueToRValue || ICE->getCastKind() == CK_UncheckedDerivedToBase) {
                E = ICE->getSubExpr();
                continue;
            }
        }
