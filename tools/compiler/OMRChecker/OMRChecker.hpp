  ...
  void VerifyTypeStructure()
  {
    ...
    for (std::map<CXXRecordDecl*, bool>::iterator I = Types.begin(),
      ...
      if (extensible && !getAssociatedConcreteType(Type)) {
        trace(
          "xxxx Issue diagnostic because there's no associated concrete type");
        continue;
      }
...
private:
  ...
  bool inSameClassString(const CXXRecordDecl* queryType,
                         const CXXRecordDecl* derivedType,
                         int level = 0)
  {
    ...
    if ((referencedFunc =
      ...
      unsigned diagID =
        diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
                                   "Static member function must be called with "
                                   "scope qualifier. Suggested fix:");
  ...
  bool VisitCXXMemberCallExpr(CXXMemberCallExpr* call)
  {
    ...
    if (receiver->isImplicitCXXThis()) {
      ...
      if (isSelfCall(call)) {
        ...
        if (callerAndCalleeHaveSameMDT(call)) {
          ...
          unsigned diagID = diagEngine.getCustomDiagID(
            ...
            "self call is resolving outside the current extensible class. You "
            "probably didn't define self() in all extensible OMR layers.");
      ...
      if (thisExpr) {
        ...
        if (thisConcrete) {
          ...
          if (targetClass == NULL || targetClass != thisConcrete) {
            ...
            unsigned diagID = diagEngine.getCustomDiagID(
              DiagnosticsEngine::Error,
              "Static casts of this in extensible classes need to be the most "
              "derived type. The preferred way of handling this is actually a "
              "call to self()");
            DiagnosticBuilder builder =
              diagEngine.Report(cast->getExprLoc(), diagID);
            std::string staticCastHint = "self()->";
            builder.AddFixItHint(
              FixItHint::CreateReplacement(cast->getExprLoc(), staticCastHint));
          } else {
      ...
      if (thisExpr) {
        ...
        std::string staticCastHint = "self()->";
        builder.AddFixItHint(
          FixItHint::CreateReplacement(receiver->getExprLoc(), staticCastHint));
      }
    }
    return true;
  }
