 ...
 *
 * * *Concrete Class:*          The TR namespace end of an extensible class
 * string. A concrete class is the one which is consumed by most clients.
 *
 * * *Root Class:*              The least-derived class in an extensible class
 * string. Currently this is assumed to be in the OMR layer.
 *
 * * *Extensible Class String:* An inheritance path from the Concrete Class to
 *                              the OMR layer root class, and all classes on
 * that path.
 *
 * # Requirements:
 ...
 *
 * The third phase checks for implicit this or casts to the wrong types, issuing
 * diagnostics.
 *
 * # Known Weaknesses:
 *
 ...
 *
 */
class ExtensibleClassDiscoveryVisitor
    : public RecursiveASTVisitor<ExtensibleClassDiscoveryVisitor> {
public:
   ...
   *
   * Builds a most-derived type mapping that can be then analyzed for Composible
   * classes.
   */
  ...
  bool isExtensible(const CXXRecordDecl *declIn) {
    // Check canonical map.
    std::map<const CXXRecordDecl *, bool>::iterator itr =
        ExtensibleMap.find(declIn->getCanonicalDecl());
    if (itr != ExtensibleMap.end()) {
      return itr->second;
    }
...
private:
  ...
  bool isExtensibleDecl(const CXXRecordDecl *declIn) {
    ...
    while (decl) {
      for (Decl::attr_iterator A = decl->attr_begin(), E = decl->attr_end();
           A != E; ++A) {
...
/**
 * Extensible class checking visitor. Verifies that all extensible classes in a
 * hierarchy are in a single chain.
 *
 * isExtensibleClassHierarchyCorrect(const CXXRecordDecl*) is a wrapper, used to
 * find the extensible classes in the class hierarchy.
 * isExtensibleClassHierarchyCorrect(const CXXRecordDecl*, bool, int) contains
 ...
 *    then the pass number must have been propagated from an extensible class
 ...
 *    While in the extensiblePhase, iterate through extensible classes until you
 * reach a non-extensible class. After exiting the extensiblePhase, iterate
 * through the rest of the parent classes, checking if they are extensible. If
 * you find an extensible class, you've found an extensible class that is in a
 * hierarchy like so: (parent -> child)
 *
 *         ... -> [Extensible] -> [Non-extensible] -> ... -> [Non-extensible] ->
 * [Extensible] -> [Extensible] ->... { - - - - - - - - - - - - -
 * !extensiblePhase - - - - - - - - - - - } { - - - extensiblePhase - - - - }
 */
class ExtensibleClassCheckingVisitor
    : public RecursiveASTVisitor<ExtensibleClassCheckingVisitor> {
public:
  explicit ExtensibleClassCheckingVisitor(
      ASTContext *Context, ExtensibleClassDiscoveryVisitor &extensible)
      : currentPass(1), passNumber(), Extensible(extensible), Context(Context) {
  }
...
private:
  /**
   * Iterates through the class hierarchy and calls
   * isExtensibleClassHierarchyCorrect(const CXXRecordDecl*, bool, int) for all
   * extensible classes found.
   */
  bool isExtensibleClassHierarchyCorrect(const CXXRecordDecl *decl) {
    ...
    if (decl->hasDefinition()) {
      for (CXXRecordDecl::base_class_const_iterator BI = decl->bases_begin(),
        ...
        if (base_class &&
            !isExtensibleClassHierarchyCorrect(base_class->getCanonicalDecl()))
          return false;
      }
    }
    return true;
  ...
  /**
   * Starting point for extensible classes. Verifies that all extensible classes
   * are in one chain.
   */
  bool isExtensibleClassHierarchyCorrect(const CXXRecordDecl *decl,
                                         bool extensiblePhase, int pass) {
      ...
      if (Extensible.isExtensible(decl)) {
        ...
        if (pass != passNumber[decl]) {
          std::string diagnostic(
              "Extensible classes heirarchy was incorrect, multiple extensible "
              "classes deriving from non-extensible class.");
          DiagnosticsEngine &diagEngine = Context->getDiagnostics();
          unsigned diagID =
              diagEngine.getCustomDiagID(DiagnosticsEngine::Error, "%0");
    ...
    for (CXXRecordDecl::base_class_const_iterator BI = decl->bases_begin(),
                                                  BE = decl->bases_end();
         BI != BE; ++BI) {
      CXXRecordDecl *base_class =
          BI->getType()->getAsCXXRecordDecl()->getCanonicalDecl();
      if (extensiblePhase) {
        if (!isExtensibleClassHierarchyCorrect(
                base_class, Extensible.isExtensible(base_class), pass))
          return false;
      } else {
        if (!Extensible.isExtensible(decl)) {
 ...
 */
class OMRClassCheckingVisitor
    : public RecursiveASTVisitor<OMRClassCheckingVisitor> {
public:
  explicit OMRClassCheckingVisitor(ASTContext *Context,
                                   ExtensibleClassDiscoveryVisitor &extensible)
      : Extensible(extensible), Context(Context) {}
  ...
  void printRelations() {
    ...
    for (std::map<const CXXRecordDecl *, const CXXRecordDecl *>::iterator
             I = MostDerivedType.begin(),
             E = MostDerivedType.end();
         I != E; ++I) {
      llvm::errs() << "\t\t" << I->first->getQualifiedNameAsString() << " -> "
                   << I->second->getQualifiedNameAsString();
      auto *concrete = getAssociatedConcreteType(I->first);
      llvm::errs() << " => "
                   << (concrete ? concrete->getQualifiedNameAsString()
                                : "<no concrete found>");
      llvm::errs() << "\n";
    }
  }
  ...
  const CXXRecordDecl *mostDerivedType(const CXXRecordDecl *decl) {
    std::map<const CXXRecordDecl *, const CXXRecordDecl *>::iterator itr =
        MostDerivedType.find(decl->getCanonicalDecl());
    if (itr != MostDerivedType.end()) {
      return itr->second;
    }
    return NULL;
  }

  bool emptyMap() { return MostDerivedType.empty(); }
  ...
  void VerifyTypeStructure() {
    ...
    for (std::map<CXXRecordDecl *, bool>::iterator I = Types.begin(),
                                                   E = Types.end();
         I != E; ++I) {
      CXXRecordDecl *Type = I->first;
      bool extensible = I->second;
      trace(Type << " " << extensible << " "
                 << getAssociatedConcreteType(Type));
      if (extensible && !getAssociatedConcreteType(Type)) {
        trace("xxxx Issue diagnostic because there's no associated concrete "
              "type");
        continue;
      }
      if (extensible) { // Extnsible type .
        trace("xxxx Verifying " << Type->getQualifiedNameAsString()
                                << " has no non-extensible base classes.");
        for (CXXRecordDecl::base_class_iterator BI = Type->bases_begin(),
                                                BE = Type->bases_end();
             BI != BE; ++BI) {
          CXXRecordDecl *base_class = BI->getType()->getAsCXXRecordDecl();
          if (base_class &&
              ...
              && !isOMRRootType(
                     Type)) { // OMR Root type can have non-extensible parents.
            // Base is not extensible, but an extensible type reaches it, with
            // no concrete class in the middle. Issue diagnostic.
        ...
        for (CXXRecordDecl::base_class_iterator BI = Type->bases_begin(),
             ...
             BI != BE; ++BI) {
          CXXRecordDecl *base_class = BI->getType()->getAsCXXRecordDecl();
          if (base_class && isExtensible(base_class) &&
              !isOMRConcreteType(base_class)) {
            // Base is not extensible, but an extensible type reaches it, with
            // no concrete class in the middle. Issue diagnostic.
   ...
   *
   * \FIXME: Right now, we assume any extensible decl in the TR:: namespace is a
   * concrete type.
   */
  ...
  bool checkDeclForNamespace(const CXXRecordDecl *decl,
                             std::string concreteNamespace) {
  ...
  const CXXRecordDecl *getAssociatedConcreteType(const CXXRecordDecl *decl) {
    ...
    if (isExtensible(decl)) {
      ...
      trace("==isExtensible: "
            << decl->getQualifiedNameAsString() << " concrete: "
            << (concrete ? concrete->getQualifiedNameAsString()
                         : "no concrete found"));
      while (concrete) {
        if (loopCount++ > 50) {
          std::string diagnostic(
              "Found more than 50 layers of classes, likely bug.");
          DiagnosticsEngine &diagEngine = Context->getDiagnostics();
          unsigned diagID =
              diagEngine.getCustomDiagID(DiagnosticsEngine::Error, "%0");
          diagEngine.Report(decl->getLocation(), diagID) << diagnostic;
        ...
        if (concrete)
          trace("==== new concrete  " << concrete->getQualifiedNameAsString()
                                      << " for "
                                      << decl->getQualifiedNameAsString());
      }

      trace("Didn't find a concrete class for "
            << decl->getQualifiedNameAsString() << " ... issue diagnostic?")
    }
    return NULL;
  }
...
private:
   ...
   *
   *
   * \FIXME: This could be tighter integrated with getAssociatedConcreteType,
   * reducing complexity.
   */
  bool inSameClassString(const CXXRecordDecl *queryType,
                         const CXXRecordDecl *derivedType, int level = 0) {
    ...
    trace(prefix << query_name
                 << "\t[ - ]  C = " << derivedType->getQualifiedNameAsString());
    for (auto BI = derivedType->bases_begin(), BE = derivedType->bases_end();
         BI != BE; ++BI) {
      auto *base_class = BI->getType()->getAsCXXRecordDecl();
      if (base_class) {
        if (isOMRConcreteType(
                base_class)) { // Concrete parent terminates search.
          trace(prefix << "not searching "
                       << base_class->getQualifiedNameAsString()
                       << ", is concrete");
        } else {
          if (inSameClassString(queryType, base_class, level + 1)) {
            trace(prefix << query_name << "\t[ " << level
                         << " ] reached through "
                         << base_class->getQualifiedNameAsString());
            return true;
          } else {
            trace(prefix << query_name << "\t[ " << level
                         << " ] not reached reached through "
                         << base_class->getQualifiedNameAsString())
          }
        }
      }
  ...
  /**
   * For algorithmic verification, ensure that every most-derived-type actually
   * is derived from type.
   */
  void AssertMostDerivedDerivesFromTypes(CXXRecordDecl *type,
                                         CXXRecordDecl *mostDerivedType) {
    // TODO
    return;
  }
  ...
  void BuildMostDerivedTypeMap(CXXRecordDecl *decl) {
    ...
    for (CXXRecordDecl::base_class_iterator BI = decl->bases_begin(),
                                            BE = decl->bases_end();
         BI != BE; ++BI) {
      if (BI->getType()->getAsCXXRecordDecl() &&
          !isOMRConcreteType(BI->getType()->getAsCXXRecordDecl())) {
    ...
    while (!toProcess.empty()) {
      ...
      if (toUpdate != toUpdate->getCanonicalDecl())
        trace("Missed cannonicalization")

            if (MostDerivedType.find(toUpdate) !=
                    MostDerivedType.end() // Found a most derived type.
                && MostDerivedType[toUpdate] !=
                       decl) { // ... and it wasn't this decl

          // Ensure we update everyone who used to think this was most derived
          auto *oldDerivedType = MostDerivedType[toUpdate];
          for (auto I = MostDerivedType.begin(), E = MostDerivedType.end();
               I != E; ++I) {
            trace("second == old: " << (I->second == oldDerivedType));
            trace("First " << I->first->getQualifiedNameAsString()
                           << " Concrete: " << isOMRConcreteType(I->first));
            trace("Second " << I->second->getQualifiedNameAsString()
                            << " Concrete: " << isOMRConcreteType(I->second));
            if (I->second == oldDerivedType && !isOMRConcreteType(I->first)) {
              toProcess.push(I->first);
              trace("\tQueuing previous most derived for processing "
                    << I->first->getQualifiedNameAsString());
            }
          }

          MostDerivedType[toUpdate] = decl;
          trace("\t (X) Updating MDT mapping "
                << toUpdate->getQualifiedNameAsString() << " -> "
                << decl->getQualifiedNameAsString());
        }
      else {
        MostDerivedType[toUpdate] = decl;
        trace("\t (Y) Updating MDT mapping "
              << toUpdate->getQualifiedNameAsString() << " -> "
              << decl->getQualifiedNameAsString());
      }
    }
 ...
 */
class OMRThisCheckingVisitor
    : public RecursiveASTVisitor<OMRThisCheckingVisitor> {
public:
  explicit OMRThisCheckingVisitor(ASTContext *Context,
                                  OMRClassCheckingVisitor *ClassChecker)
      : Context(Context), ClassChecker(ClassChecker) {}
  ...
  bool VisitCallExpr(CallExpr *call) {
    ...
    if ((referencedFunc =
        ...
        !referencedFunc->hasQualifier()) {
      // get the static function's parent class. This cast should be guarenteed
      // to work but let's play it safe.
      ...
      unsigned diagID = diagEngine.getCustomDiagID(
          DiagnosticsEngine::Error, "Static member function must be called "
                                    "with scope qualifier. Suggested fix:");
      DiagnosticBuilder builder = diagEngine.Report(call->getExprLoc(), diagID);
      std::string qualifierHint =
          "TR::" + methodDecl->getParent()->getNameAsString() + "::";
      builder.AddFixItHint(
          FixItHint::CreateInsertion(call->getExprLoc(), qualifierHint));
    }
    return true;
  }
  ...
  bool VisitCXXMemberCallExpr(CXXMemberCallExpr *call) {
    ...
    if (receiver->isImplicitCXXThis()) {
      ...
      if (isSelfCall(call)) {
        ...
        // Need still to verify that the self() call is to the right receiver!
        // Otherwise we can pass upcasts in the case where we have an extensible
        // class hierarchy deriving from an extensible class hierarchy.
        if (callerAndCalleeHaveSameMDT(call)) {
          ...
          unsigned diagID = diagEngine.getCustomDiagID(
              ...
              "self call is resolving outside the current extensible class. "
              "You probably didn't define self() in all extensible OMR "
              "layers.");
        ...
        DiagnosticBuilder builder =
            diagEngine.Report(receiver->getExprLoc(), diagID);
        std::string staticCastHint = "self()->";
        builder.AddFixItHint(
            FixItHint::CreateInsertion(receiver->getExprLoc(), staticCastHint));
      }
    } else if (isa<CXXStaticCastExpr>(receiver)) {
      CXXStaticCastExpr *cast = dyn_cast<CXXStaticCastExpr>(receiver);
      CXXRecordDecl *targetClass = cast->getType()->getAs<PointerType>()
                                       ? cast->getType()
                                             ->getAs<PointerType>()
                                             ->getPointeeType()
                                             ->getAsCXXRecordDecl()
                                       ...
                                       : NULL;
      trace(
          "targetClass of static cast"
          << (targetClass ? targetClass->getQualifiedNameAsString() : "NULL"));
      CXXThisExpr *thisExpr = getThisExpr(cast->getSubExpr());
      if (thisExpr) {
        auto *thisConcrete =
            ClassChecker->getAssociatedConcreteType(thisExpr->getType()
                                                        ->getAs<PointerType>()
                                                        ->getPointeeType()
                                                        ->getAsCXXRecordDecl());
        if (thisConcrete) {
          trace("Associated concrete class: "
                << thisConcrete->getQualifiedNameAsString());
          if (targetClass == NULL || targetClass != thisConcrete) {
            DiagnosticsEngine &diagEngine = Context->getDiagnostics();
            unsigned diagID = diagEngine.getCustomDiagID(
                DiagnosticsEngine::Error,
                "Static casts of this in extensible classes need to be the "
                "most derived type. The preferred way of handling this is "
                "actually a call to self()");
            DiagnosticBuilder builder =
                diagEngine.Report(cast->getExprLoc(), diagID);
            std::string staticCastHint = "self()->";
            builder.AddFixItHint(FixItHint::CreateReplacement(
                cast->getExprLoc(), staticCastHint));
          } else {
            // Static cast is correct, but, we want a warning.
            DiagnosticsEngine &diagEngine = Context->getDiagnostics();
            unsigned diagID = diagEngine.getCustomDiagID(
                DiagnosticsEngine::Warning,
                "extensible class dereference prefers calls to self()");
            DiagnosticBuilder builder =
                diagEngine.Report(cast->getExprLoc(), diagID);
            std::string staticCastHint = "self()->";
            builder.AddFixItHint(FixItHint::CreateReplacement(
      ...
      if (thisExpr) {
        DiagnosticsEngine &diagEngine = Context->getDiagnostics();
        unsigned diagID = diagEngine.getCustomDiagID(
            DiagnosticsEngine::Error, "Use of this in an extensible class can "
                                      "lead to unexpected behaviour");
        DiagnosticBuilder builder =
            diagEngine.Report(receiver->getExprLoc(), diagID);
        std::string staticCastHint = "self()->";
        builder.AddFixItHint(FixItHint::CreateReplacement(
            receiver->getExprLoc(), staticCastHint));
      }
    }
    return true;
  }
  ...
  bool isAllowedSelflessCall(CXXMemberCallExpr *call) {
    /*
     * Ignore member function calls that specifically call a base class member
     * function
     */
    MemberExpr *memberFunc;
    // hasQualifier checks for a nested name specifier, e.g. the 'IBM::Foo::'
    // part of 'IBM::Foo::baz()'
    if ((memberFunc = dyn_cast<MemberExpr>(call->getCallee())) &&
        memberFunc->hasQualifier()) {
      trace("isAllowedSelflessCall: True because it's a qualified base class "
            "member function");
      return true;
    }
    ...
    const CXXRecordDecl *calleeMostDerived =
        ClassChecker->mostDerivedType(calleeClassDecl);
    const CXXMethodDecl *callerMethod = getCallerDecl(call);
    const CXXRecordDecl *callerDecl = callerMethod->getParent();

    if (calleeMostDerived->getCanonicalDecl() ==
        callerDecl->getCanonicalDecl()) {
      trace("isAllowedSelflessCall: True because caller is most derived type");
      return true;
    } else {
      trace("isAllowedSelflessCall: false");
      if (getenv("OMR_CHECK_TRACE")) {
        llvm::errs() << "isAllowedSelflessCall: calleeDecl            => "
                     << calleeDecl->getQualifiedNameAsString() << "\n";
        llvm::errs() << "isAllowedSelflessCall: calleeClassDecl       => "
                     << calleeClassDecl->getQualifiedNameAsString() << "\n";
        llvm::errs() << "isAllowedSelflessCall: calleeMostDerived     => "
                     << calleeMostDerived->getQualifiedNameAsString() << "\n";
        llvm::errs() << "isAllowedSelflessCall: callerDecl            => "
                     << (callerDecl ? callerDecl->getQualifiedNameAsString()
                                    : "NULL")
                     << "\n";
      }
    }
  ...
  bool computeCallInformation(const CXXMemberCallExpr *call,
    ...
    // This is the class that defines the callee method -- so where self() is
    // defined.
    ...
    if (!calleeMostDerived) {
      trace("Didn't find a most derived type for the callee class. Assuming "
            "not correct self call");
      return false;
    }
    ...
    const CXXMethodDecl *callerMethod = getCallerDecl(call);
    if (!callerMethod) {
      trace("Didn't find a caller method... Assuming not correct self call. "
            "Call node: ");
      call->dump();
      return false;
    }
    ...
    const CXXRecordDecl *callerDecl = callerMethod->getParent();
    if (!calleeMostDerived) {
      trace("Didn't didn't find the caller method decl. Assuming not correct "
            "self call");
      return false;
    }
    ...
    if (getenv("OMR_CHECK_TRACE")) {
      llvm::errs() << "computeCallInformation: calleeDecl            => "
                   << calleeDecl->getQualifiedNameAsString() << "\n";
      llvm::errs() << "computeCallInformation: calleeClassDecl       => "
                   << calleeClassDecl->getQualifiedNameAsString() << "\n";
      llvm::errs() << "computeCallInformation: calleeMostDerived     => "
                   << calleeMostDerived->getQualifiedNameAsString() << "\n";
      llvm::errs() << "\n";
      llvm::errs() << "computeCallInformation: callerDecl            => "
                   << (callerDecl ? callerDecl->getQualifiedNameAsString()
                                  : "NULL")
                   << "\n";
      llvm::errs() << "computeCallInformation: callerMostDerived     => "
                   << (callerMostDerived
                           ? callerMostDerived->getQualifiedNameAsString()
                           : "NULL")
                   << "\n";
    }
    return true;
  ...
  /**
   * Return true iff
   * 1) the declaration corresponding to a CallExpr can be found (not
   * type-dependend) 2) a CallExpr is a call to a static member function of an
   * extensible class.
   */
