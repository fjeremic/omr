 ...
 *
 * isExtensibleClassHierarchyCorrect(const CXXRecordDecl*) is a wrapper, used to find the extensible classes in the
 * class hierarchy. isExtensibleClassHierarchyCorrect(const CXXRecordDecl*, bool, int) contains the checking algorithm.
 *
 ...
 *    After exiting the extensiblePhase, iterate through the rest of the parent classes, checking if they are
 * extensible. If you find an extensible class, you've found an extensible class that is in a hierarchy like so: (parent
 * -> child)
 *
...
private:
  /**
   * Iterates through the class hierarchy and calls isExtensibleClassHierarchyCorrect(const CXXRecordDecl*, bool, int)
   * for all extensible classes found.
   */
    ...
    if (passNumber[decl] != 0) {
      if (Extensible.isExtensible(decl)) {
        ...
        if (pass != passNumber[decl]) {
          std::string diagnostic("Extensible classes heirarchy was incorrect, multiple extensible classes deriving "
                                 "from non-extensible class.");
      ...
      if (extensiblePhase) {
        ...
        if (!Extensible.isExtensible(decl)) {
          ...
          std::string diagnostic(
            "Extensible classes heirarchy was incorrect, multiple extensible class chains in heirarchy.");
          DiagnosticsEngine& diagEngine = Context->getDiagnostics();
          unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error, "%0");
          diagEngine.Report(decl->getLocation(), diagID) << diagnostic;
          return false;
...
public:
  ...
  void printRelations()
  {
    ...
    for (std::map<const CXXRecordDecl*, const CXXRecordDecl*>::iterator I = MostDerivedType.begin(),
    ...
    for (std::map<CXXRecordDecl*, bool>::iterator I = Types.begin(), E = Types.end(); I != E; ++I) {
      ...
      if (extensible) { // Extnsible type .
        ...
        for (CXXRecordDecl::base_class_iterator BI = Type->bases_begin(), BE = Type->bases_end(); BI != BE; ++BI) {
          CXXRecordDecl* base_class = BI->getType()->getAsCXXRecordDecl();
          if (base_class && !isExtensible(base_class) // Ensure extensible parent.
  ...
  const CXXRecordDecl* getAssociatedConcreteType(const CXXRecordDecl* decl)
  {
    ...
    if (isExtensible(decl)) {
      ...
      trace("==isExtensible: " << decl->getQualifiedNameAsString() << " concrete: "
                               << (concrete ? concrete->getQualifiedNameAsString() : "no concrete found"));
      while (concrete) {
        if (loopCount++ > 50) {
        ...
        if (concrete)
          trace("==== new concrete  " << concrete->getQualifiedNameAsString() << " for "
...
private:
      ...
      if (base_class) {
        if (isOMRConcreteType(base_class)) { // Concrete parent terminates search.
          ...
          if (inSameClassString(queryType, base_class, level + 1)) {
            trace(prefix << query_name << "\t[ " << level << " ] reached through "
                         << base_class->getQualifiedNameAsString());
            return true;
          } else {
            trace(prefix << query_name << "\t[ " << level << " ] not reached reached through "
                         << base_class->getQualifiedNameAsString())
          }
        }
      }
  ...
  void BuildMostDerivedTypeMap(CXXRecordDecl* decl)
  {
    ...
    if (MostDerivedType.find(decl) != MostDerivedType.end() && mostDerivedType(decl) == decl) {
    ...
    while (!toProcess.empty()) {
      ...
      if (toUpdate != toUpdate->getCanonicalDecl())
        ...
        { // ... and it wasn't this decl
          ...
          MostDerivedType[toUpdate] = decl;
          trace("\t (X) Updating MDT mapping " << toUpdate->getQualifiedNameAsString() << " -> "
                                               << decl->getQualifiedNameAsString());
        }
      else {
        MostDerivedType[toUpdate] = decl;
        trace("\t (Y) Updating MDT mapping " << toUpdate->getQualifiedNameAsString() << " -> "
                                             << decl->getQualifiedNameAsString());
      }
    }
  }
...
public:
  explicit OMRThisCheckingVisitor(ASTContext* Context, OMRClassCheckingVisitor* ClassChecker)
    : Context(Context)
  ...
  bool VisitCallExpr(CallExpr* call)
  {
    ...
    if ((referencedFunc = dyn_cast<DeclRefExpr>(call->getCallee()->IgnoreImplicit()))
      ...
      unsigned diagID = diagEngine.getCustomDiagID(
        DiagnosticsEngine::Error, "Static member function must be called with scope qualifier. Suggested fix:");
  ...
  bool VisitCXXMemberCallExpr(CXXMemberCallExpr* call)
  {
    ...
    if (receiver->isImplicitCXXThis()) {
      ...
      if (isSelfCall(call)) {
        ...
        if (callerAndCalleeHaveSameMDT(call)) {
          ...
          unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
            "self call is resolving outside the current extensible class. You probably didn't define self() in all "
            "extensible OMR layers.");
        ...
        DiagnosticsEngine& diagEngine = Context->getDiagnostics();
        unsigned diagID = diagEngine.getCustomDiagID(
          DiagnosticsEngine::Error, "Implicit this receivers are prohibited in extensible classes");
      ...
      CXXRecordDecl* targetClass = cast->getType()->getAs<PointerType>()
        ? cast->getType()->getAs<PointerType>()->getPointeeType()->getAsCXXRecordDecl()->getCanonicalDecl()
        : NULL;
      ...
      if (thisExpr) {
        auto* thisConcrete = ClassChecker->getAssociatedConcreteType(
          thisExpr->getType()->getAs<PointerType>()->getPointeeType()->getAsCXXRecordDecl());
        if (thisConcrete) {
          trace("Associated concrete class: " << thisConcrete->getQualifiedNameAsString());
          if (targetClass == NULL || targetClass != thisConcrete) {
            DiagnosticsEngine& diagEngine = Context->getDiagnostics();
            unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error,
              "Static casts of this in extensible classes need to be the most derived type. The preferred way of "
              "handling this is actually a call to self()");
            ...
            DiagnosticsEngine& diagEngine = Context->getDiagnostics();
            unsigned diagID = diagEngine.getCustomDiagID(
              DiagnosticsEngine::Warning, "extensible class dereference prefers calls to self()");
            DiagnosticBuilder builder = diagEngine.Report(cast->getExprLoc(), diagID);
            std::string staticCastHint = "self()->";
            builder.AddFixItHint(FixItHint::CreateReplacement(cast->getExprLoc(), staticCastHint));
          }
      ...
      if (thisExpr) {
        DiagnosticsEngine& diagEngine = Context->getDiagnostics();
        unsigned diagID = diagEngine.getCustomDiagID(
          DiagnosticsEngine::Error, "Use of this in an extensible class can lead to unexpected behaviour");
        DiagnosticBuilder builder = diagEngine.Report(receiver->getExprLoc(), diagID);
        std::string staticCastHint = "self()->";
    ...
    if (calleeMostDerived->getCanonicalDecl() == callerDecl->getCanonicalDecl()) {
      ...
      if (getenv("OMR_CHECK_TRACE")) {
        ...
        llvm::errs() << "isAllowedSelflessCall: callerDecl            => "
                     << (callerDecl ? callerDecl->getQualifiedNameAsString() : "NULL") << "\n";
      }
    }
  ...
  bool computeCallInformation(
    const CXXMemberCallExpr* call, const CXXRecordDecl*& calleeMostDerived, const CXXRecordDecl*& callerMostDerived)
  {
    ...
    if (getenv("OMR_CHECK_TRACE")) {
      ...
      llvm::errs() << "computeCallInformation: callerMostDerived     => "
                   << (callerMostDerived ? callerMostDerived->getQualifiedNameAsString() : "NULL") << "\n";
    }
    return true;
  }
...
static CXXThisExpr* getThisExpr(Expr* E)
{
  while (true) {
    ...
    if (ImplicitCastExpr* ICE = dyn_cast<ImplicitCastExpr>(E)) {
      if (ICE->getCastKind() == CK_NoOp || ICE->getCastKind() == CK_LValueToRValue
