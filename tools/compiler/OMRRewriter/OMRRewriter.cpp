...
public:
  explicit OMRThisRewritingVisitor(ASTContext* C, OMRThisCheckingVisitor* TC, OMRClassCheckingVisitor* CC, Rewriter& R)
    : ThisChecker(TC)
    , TheRewriter(R)
    , ClassChecker(CC)
  ...
  bool VisitCallExpr(CallExpr* call)
  {
    ...
    if ((referencedFunc = dyn_cast<DeclRefExpr>(call->getCallee()->IgnoreImplicit()))
      ...
      /*DiagnosticsEngine &diagEngine = Context->getDiagnostics();
      unsigned diagID = diagEngine.getCustomDiagID(DiagnosticsEngine::Error, "Static member function must be called with
      scope qualifier. Suggested fix:"); DiagnosticBuilder builder = diagEngine.Report(call->getExprLoc(), diagID);
  ...
  bool VisitCXXMemberCallExpr(CXXMemberCallExpr* call)
  {
    ...
    if (receiver->isImplicitCXXThis()) {
      ...
      if (thisExpr) {
        const CXXRecordDecl* thisConcrete = ClassChecker->getAssociatedConcreteType(
