 ...
 */
class OMRThisRewritingVisitor
    : public RecursiveASTVisitor<OMRThisRewritingVisitor> {
  ASTContext *Context;
  ...
  bool isUnique(int RawEncoding) {
    int i = 0;
    while (i < RepeatedMacros.size() && RepeatedMacros[i] != RawEncoding) {
      i++;
    }
   ...
   *        macro, so macro expansions may be silently replaced if the
   * replacement range contains a macro expansion. An example of where this
   * occurs is
   *
  ...
  void ReplaceMacroCheck(Expr *e) {
    ...
    if (startLoc.isMacroID() && endLoc.isMacroID()) {
      if (SrcMgr.isMacroBodyExpansion(startLoc)) {
        diagnostic +=
            "Expression is inside a macro body expansion; replace aborted.";
        trace(diagnostic);
        trace("-----------------");
    ...
    } else if ((!startLoc.isMacroID() && endLoc.isMacroID()) ||
               (startLoc.isMacroID() && !endLoc.isMacroID())) {
      ...
      while (itr != nullItr) {
        contains_macro =
            contains_macro || SrcMgr.isMacroBodyExpansion(itr->getLocStart());
    ...
    if (eLoc.isMacroID()) {
      if (SrcMgr.isMacroBodyExpansion(eLoc)) {
        ...
        if (isUnique(SrcMgr.getSpellingLoc(eLoc).getRawEncoding())) {
          InsertSelf(e, SrcMgr.getSpellingLoc(eLoc));
          RepeatedMacros.push_back(
              SrcMgr.getSpellingLoc(eLoc).getRawEncoding());
        } else {
  ...
  /**
   * Inserts the specified scope qualifier while undoing macro expansion if
   * necessary.
   *
   * Current behaviour is similar to that of InserMacroCheck.
   */
  void InsertScopeQualifierWithMacroCheck(Expr *e,
                                          const std::string &qualifier) {
    SourceLocation eLoc = e->getExprLoc();
    if (eLoc.isMacroID()) {
      if (SrcMgr.isMacroBodyExpansion(eLoc)) {
        std::string diagnostic =
            "Expression is inside a macro body; insertion aborted.";
        trace(diagnostic);
        trace("-----------------");
        diagnose_warning(diagnostic, eLoc);
        // InsertSelf(e, SrcMgr.getSpellingLoc(eLoc));
   ...
   */
  void InsertScopeQualifier(Expr *e, SourceLocation loc,
                            const std::string &qualifier) {
...
public:
  explicit OMRThisRewritingVisitor(ASTContext *C, OMRThisCheckingVisitor *TC,
                                   OMRClassCheckingVisitor *CC, Rewriter &R)
      : ThisChecker(TC), TheRewriter(R), ClassChecker(CC),
        SrcMgr(R.getSourceMgr()), Context(C) {}
  ...
  bool VisitCallExpr(CallExpr *call) {
    ...
    if ((referencedFunc =
      ...
      auto calleeDecl = call->getCalleeDecl(); // needed to avoid potential
                                               // segfault with dyn_cast and g++
      ...
      "Static member function must be called with scope qualifier. Suggested
      fix:"); DiagnosticBuilder builder = diagEngine.Report(call->getExprLoc(),
      diagID); std::string qualifierHint = "TR::" +
      methodDecl->getParent()->getNameAsString() + "::";
      builder.AddFixItHint(FixItHint::CreateInsertion(call->getExprLoc(),
      qualifierHint));*/
  ...
  bool VisitCXXMemberCallExpr(CXXMemberCallExpr *call) {
    ...
    if (receiver->isImplicitCXXThis()) {
      ...
      if (thisExpr) {
        const CXXRecordDecl *thisConcrete =
  ...
  void RewriteFiles() {
    for (std::map<FileID, int>::iterator files = replacements.begin();
         files != replacements.end(); files++) {
      std::string filename =
          std::string(SrcMgr.getFileEntryForID(files->first)->getName()) +
          ".OMRRewritten";
  ...
  virtual void HandleTranslationUnit(ASTContext &Context) {
    ...
    if (extVisitor.TraverseDecl(Context.getTranslationUnitDecl())) {
      ...
      if (ClassVisitor.TraverseDecl(Context.getTranslationUnitDecl())) {
        ...
        OMRThisRewritingVisitor ThisRewriter(&Context, &ThisChecker,
                                             &ClassVisitor, TheRewriter);
      ...
      trace("Returning early, extensible class discovery visitor returned "
            "false!");
    }
