...
RCType DATFileWriter::writeOutputFiles(J9TDFOptions* options, J9TDFFile* tdf) {
  RCType rc = RC_FAILED;
  ...
  const char* fileName =
      FileUtils::getTargetFileName(options, tdf->fileName, UT_FILENAME_PREFIX,
                                   tdf->header.executable, ".pdat");
  ...
  if (NULL != datFile) {
    while (NULL != tp) {
      ...
      fprintf(datFile, datLineTemplate, tdf->header.executable, id, tp->type,
              tp->overhead, tp->level, explicitChar, tp->name, exceptChar,
              entryExitChar, format);
      tp = tp->nexttp;
      id += 1;
    }
    fclose(datFile);
