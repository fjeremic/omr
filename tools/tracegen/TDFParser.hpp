...
class TDFParser {
 ...
 private:
   ...
   */
  RCType processTracePointDetail(const char *line, J9TDFTracepoint *tp,
   ...
   * @return RC_OK on success, RC_FAILED on failure
   */
  RCType processTemplate(const char *formatTemplate, char *str,
   ...
   * @param formatTemplate
   */
  void flagTdfSyntaxError(const char *message, unsigned int parmCount,
                          const char *formatTemplate, const char *fileName,
                          unsigned int lineNumber);
