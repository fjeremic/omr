...
TDFParser::parse()
{
  ...
  while (_fileReader->hasNext()) {
    ...
    if (0 == strlen(line) || StringUtils::startsWithUpperLower(line, "//") ||
      ...
      if (0 != processTracePointDetail(line,
                                       tdf->lasttp,
                                       tdf->header.executable,
                                       id++,
                                       _fileReader->_fileName,
...
RCType
TDFParser::processTracePointDetail(const char* line,
                                   J9TDFTracepoint* tp,
                                   const char* module,
                                   unsigned int id,
                                   const char* fileName,
                                   unsigned int lineNumber)
{
  ...
  while (NULL != tok) {
    if (StringUtils::startsWithUpperLower(tok, "Explicit")) {
      ...
      /* tp->isSuffix = true; */
      FileUtils::printError(
        "WARNING : obsolete keyword 'Suffix' in %s:%u\n", fileName, lineNumber);
      line = line + strlen(tok);
      while ((' ' == *line || '\t' == *line) && '\0' != *line) {
        line = line + 1;
      }
      ...
      /* tp->isPrefix = true; */
      FileUtils::printError(
        "WARNING : obsolete keyword 'Prefix' in %s:%u\n", fileName, lineNumber);
      line = line + strlen(tok);
      while ((' ' == *line || '\t' == *line) && '\0' != *line) {
        line = line + 1;
      }
      ...
      tok = strtok(NULL, " \t");
                 ...
                 tok, (UT_ASSERT_TYPE == tp->type ? "Assert=" : "Template="))) {
  ...
  if (UT_ASSERT_TYPE == tp->type) {
    ...
    rc = processTemplate(
      tp->format, tp->parameters, &tp->parmCount, fileName, lineNumber);
 ...
 */
RCType
TDFParser::processAssertTemplate(const char* formatTemplate,
                                 unsigned int* count)
{
  RCType rc = RC_OK;
 ...
 */
RCType
TDFParser::processTemplate(const char* formatTemplate,
                           char* str,
                           unsigned int* count,
                           const char* fileName,
                           unsigned int lineNumber)
              ...
              switch (*(pos + 1)) {
                ...
                case 'u':
                  if (_treatWarningAsError) {
                    eprintf("ERROR : %%l is not valid modifier. (parameter %u "
                            "in mutated format string %s). %s:%u",
                            parmCount,
                            formatTemplate,
                            fileName,
                            lineNumber);
                    ...
                    eprintf("WARNING : %%l is not valid modifier. (parameter "
                            "%u in mutated format string %s). %s:%u",
                            parmCount,
                            formatTemplate,
                            fileName,
                            lineNumber);
...
void
TDFParser::flagTdfSyntaxError(const char* message,
...
TDFParser::findTraceTypeIndex(const char* traceLine, int* result)
{
  ...
  const char* TRACE_TYPES[] = {
    "TraceEvent", "TraceException",       "TraceEntry", "TraceEntry-Exception",
