...
void TDFParser::init(FileReader *reader, bool treatWarningAsError) {
  _fileReader = reader;
  _treatWarningAsError = treatWarningAsError;
}

J9TDFFile *TDFParser::parse() {
  int type = -1;
  unsigned int id = 0;
  ...
  while (_fileReader->hasNext()) {
    ...
    if (0 == strlen(line) || StringUtils::startsWithUpperLower(line, "//") ||
      ...
      if (NULL == tdf->tracepoints) {
        tdf->tracepoints =
            (J9TDFTracepoint *)Port::omrmem_calloc(1, sizeof(J9TDFTracepoint));
        if (NULL == tdf->tracepoints) {
          eprintf("Failed to allocate memory");
          goto failed;
        }
      ...
      if (0 != processTracePointDetail(
                   line, tdf->lasttp, tdf->header.executable, id++,
                   _fileReader->_fileName, _fileReader->_lineNumber)) {
  ...
  if (NULL == tdf->header.datfilename) {
    tdf->header.datfilename =
        (char *)Port::omrmem_calloc(1, strlen(DEFAULT_DAT_FILE) + 1);
    if (NULL == tdf->header.datfilename) {
...
RCType TDFParser::processTracePointDetail(const char *line, J9TDFTracepoint *tp,
                                          const char *module, unsigned int id,
                                          const char *fileName,
  ...
  /* Save original string because strtok mangles the source string by placing \0
   * at delimiter */
  ...
  while (NULL != tok) {
    if (StringUtils::startsWithUpperLower(tok, "Explicit")) {
      ...
      tok = strtok(NULL, " \t");
    } else if (StringUtils::startsWithUpperLower(
                   tok,
                   (UT_ASSERT_TYPE == tp->type ? "Assert=" : "Template="))) {
      tp->format = getTemplate(
          line, (UT_ASSERT_TYPE == tp->type ? "Assert=" : "Template="));
      if (NULL == tp->format) {
        goto failed;
      }
      line = line +
             strlen((UT_ASSERT_TYPE == tp->type ? "Assert=" : "Template=")) +
      ...
      if (RC_OK !=
          StringUtils::getPositiveIntValue(tok, "Level=", &tp->level)) {
        goto failed;
      }
      line = line + strlen(tok);
      while ((' ' == *line || '\t' == *line) && '\0' != *line) {
      ...
      if (RC_OK !=
          StringUtils::getPositiveIntValue(tok, "Overhead=", &tp->overhead)) {
        goto failed;
      }
      line = line + strlen(tok);
      while ((' ' == *line || '\t' == *line) && '\0' != *line) {
  ...
  if (UT_ASSERT_TYPE == tp->type) {
    /* Assertion parameters are always \"\\377\\4\\377\" for "** ASSERTION
     * FAILED ** at %s:%d: %s" */
    tp->parameters = NULL;
    ...
    size_t len =
        ...
        (percentSignCount *
         (strlen(TRACE_DATA_TYPE_STRING) +
          strlen(TRACE_DATA_TYPE_PRECISION))) /* TRACE_DATA_TYPE_STRING is the
                                                 longest parameter string +
                                                 precision specifier*/
        ;
    ...
    rc = processTemplate(tp->format, tp->parameters, &tp->parmCount, fileName,
                         lineNumber);
 ...
 */
char *TDFParser::getTemplate(const char *line, const char *key) {
  char *format = NULL;
  size_t len = 0;
  /*
   * Number of quotes in the string. Must be even when the string is fully
   * parsed to ensure there is an opening and closing quote.
   */
  int count = 0;
  ...
  /*
   * There's actually only one quoted string allowed in a trace point
   * definition, the template. So bypass clever quote handling by assuming that
   * the quote at the other end of the line *must* belong to this template. Note
   * that the template string should include the quotes as it will be included
   * in #defines later on!
   */
  while (0 != *pos) {
    pos++;
  ...
  /*
   * If number of quotes is not even, then there must have been an opening quote
 ...
 */
char **TDFParser::getGroups(const char *line) {
  char *pos = NULL;
  char **values = NULL;
 ...
 */
RCType TDFParser::processAssertTemplate(const char *formatTemplate,
                                        unsigned int *count) {
  RCType rc = RC_OK;
 ...
 * str - a malloc'd buffer to hold the result
 */
RCType TDFParser::processTemplate(const char *formatTemplate, char *str,
   ...
   */
  char stateBits[] = {(char)0x80, (char)0x40, (char)0x20, (char)0x10,
  ...
  while ((NULL != (pos = strchr(pos, '%'))) && (RC_OK == rc)) {
    ...
    while ((++pos < (formatTemplate + templateLength)) &&
           (0 != (stateBits[state] & allowFromStates[0])) && (RC_OK == rc)) {
      ...
      case 'L':
      case 'z':
      case 'P': /* The P size specifier indicates an integer that can contain a
                   pointer */
          ...
          if (formatTemplate + templateLength > pos + 1) {
            ...
            case 'u':
              if (_treatWarningAsError) {
                eprintf("ERROR : %%l is not valid modifier. (parameter %u in "
                        "mutated format string %s). %s:%u",
                        parmCount, formatTemplate, fileName, lineNumber);
                return RC_FAILED;
              } else {
                eprintf("WARNING : %%l is not valid modifier. (parameter %u in "
                        "mutated format string %s). %s:%u",
                        parmCount, formatTemplate, fileName, lineNumber);
              }
            }
      ...
      case 's':
        ...
        case 'u':
          ...
          default:
            flagTdfSyntaxError("Invalid length modifier and type", parmCount,
                               formatTemplate, fileName, lineNumber);
            rc = RC_FAILED;
          }
          break;
    ...
    if ((RC_OK != rc) || (0 == (stateBits[state] & allowFromStates[8]))) {
      flagTdfSyntaxError("Invalid template specifier", parmCount,
                         formatTemplate, fileName, lineNumber);
      rc = RC_FAILED;
    } else {
      state = 1;
...
void TDFParser::flagTdfSyntaxError(const char *message, unsigned int parmCount,
   ...
   */
  eprintf(
      "%s: in parameter %u of format string at %s:%u\n\tformatString: \"%s\"",
      message, parmCount + 1, fileName, lineNumber, formatTemplate);
}

RCType TDFParser::findTraceTypeIndex(const char *traceLine, int *result) {
  RCType rc = RC_FAILED;
  const char *TRACE_TYPES[] = {"TraceEvent", "TraceException",
                               "TraceEntry", "TraceEntry-Exception",
                               "TraceExit",  "TraceExit-Exception",
                               "TraceMem",   "TraceMemException",
                               "TraceDebug", "TraceDebug-Exception",
                               "TracePerf",  "TracePerf-Exception",
                               "TraceAssert"};
