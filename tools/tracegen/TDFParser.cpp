...
J9TDFFile* TDFParser::parse()
{
  ...
  while (_fileReader->hasNext()) {
    ...
    if (0 == strlen(line) || StringUtils::startsWithUpperLower(line, "//")
      ...
      if (0
        != processTracePointDetail(
          line, tdf->lasttp, tdf->header.executable, id++, _fileReader->_fileName, _fileReader->_lineNumber)) {
...
RCType TDFParser::processTracePointDetail(const char* line, J9TDFTracepoint* tp, const char* module, unsigned int id,
  const char* fileName, unsigned int lineNumber)
{
  ...
  if (UT_ASSERT_TYPE == tp->type) {
    ...
    size_t len = 5 /* \\\"\\\"\0 when no parameters specified */
      + (percentSignCount
        * (strlen(TRACE_DATA_TYPE_STRING)
          + strlen(TRACE_DATA_TYPE_PRECISION))) /* TRACE_DATA_TYPE_STRING is the longest parameter string + precision
                                                   specifier*/
      ;
...
char* TDFParser::getTemplate(const char* line, const char* key)
{
  ...
  size_t len = 0;
  /*
   * Number of quotes in the string. Must be even when the string is fully parsed to ensure there is an opening and
   * closing quote.
   */
  int count = 0;
 ...
 */
RCType TDFParser::processAssertTemplate(const char* formatTemplate, unsigned int* count)
{
  RCType rc = RC_OK;
 ...
 * str - a malloc'd buffer to hold the result
 */
RCType TDFParser::processTemplate(
  const char* formatTemplate, char* str, unsigned int* count, const char* fileName, unsigned int lineNumber)
{
  char* pos = (char*)formatTemplate;
  RCType rc = RC_OK;
  char* modifier = NULL;
  ...
  while ((NULL != (pos = strchr(pos, '%'))) && (RC_OK == rc)) {
    ...
    state = 0;
    while (
      (++pos < (formatTemplate + templateLength)) && (0 != (stateBits[state] & allowFromStates[0])) && (RC_OK == rc)) {
          ...
          if (formatTemplate + templateLength > pos + 1) {
            ...
            case 'u':
              if (_treatWarningAsError) {
                eprintf("ERROR : %%l is not valid modifier. (parameter %u in mutated format string %s). %s:%u",
                  parmCount, formatTemplate, fileName, lineNumber);
                return RC_FAILED;
              } else {
                eprintf("WARNING : %%l is not valid modifier. (parameter %u in mutated format string %s). %s:%u",
                  parmCount, formatTemplate, fileName, lineNumber);
              }
            }
          }
...
void TDFParser::flagTdfSyntaxError(const char* message, unsigned int parmCount, const char* formatTemplate,
  const char* fileName, unsigned int lineNumber)
{
...
{
  RCType rc = RC_FAILED;
  const char* TRACE_TYPES[] = { "TraceEvent", "TraceException", "TraceEntry", "TraceEntry-Exception", "TraceExit",
    "TraceExit-Exception", "TraceMem", "TraceMemException", "TraceDebug", "TraceDebug-Exception", "TracePerf",
    "TracePerf-Exception", "TraceAssert" };
