  ...
  while ((NULL != (pos = strchr(pos, '%'))) && (RC_OK == rc)) {
    ...
    while ((++pos < (formatTemplate + templateLength)) &&
      ...
      switch (*pos) {
        ...
        case 'L':
        case 'z':
        case 'P': /* The P size specifier indicates an integer that can contain
                     a pointer */
              ...
              switch (*(pos + 1)) {
                ...
                case 'u':
                  if (_treatWarningAsError) {
                    eprintf(
                        "ERROR : %%l is not valid modifier. (parameter %u in "
                        "mutated format string %s). %s:%u",
                        parmCount, formatTemplate, fileName, lineNumber);
                    return RC_FAILED;
                  } else {
                    eprintf(
                        "WARNING : %%l is not valid modifier. (parameter %u in "
                        "mutated format string %s). %s:%u",
                        parmCount, formatTemplate, fileName, lineNumber);
                  }
              }
              ...
              switch (*modifier) {
                ...
                default:
                  flagTdfSyntaxError("Invalid length modifier and type",
                                     parmCount, formatTemplate, fileName,
                                     lineNumber);
                  rc = RC_FAILED;
              }
              break;
