...
const char*
FileUtils::getTargetFileName(J9TDFOptions* options,
...
RCType
FileUtils::visitDirectory(J9TDFOptions* options,
                          const char* dirName,
                          ...
                          VisitFileCallBack callback)
{
  ...
  while (RC_FAILED != rc) {
    ...
    snprintf(
      nextEntry, nextEntrySize, "%s" PATH_SEP "%s", dirName, resultBuffer);
