...
const char *FileUtils::getFileName(const char *path) {
...
const char *FileUtils::getFileExt(const char *fileName) {
...
const char *FileUtils::getTargetFileName(J9TDFOptions *options,
                                         const char *tdfFileName,
  ...
  filePathLen = strlen(baseName) + 1 + strlen(filePrefix) + strlen(fileName) +
                strlen(extension) + 1;
  ...
  if (baseName[strlen(baseName) - 1] == PATH_SEP[0]) {
    snprintf(filePath, filePathLen, "%s%s%s%s", baseName, filePrefix, fileName,
             extension);
  } else {
    snprintf(filePath, filePathLen, "%s" PATH_SEP "%s%s%s", baseName,
...
time_t FileUtils::getMtime(const char *filePath) {
  dirstat statbuf;
...
RCType FileUtils::visitDirectory(J9TDFOptions *options, const char *dirName,
                                 const char *fileExtFilter, void *targetObject,
  ...
  while (RC_FAILED != rc) {
    ...
    size_t nextEntrySize =
        strlen(dirName) + 1 /* PATH_SEP */ + strlen(resultBuffer) + 1;
...
void FileUtils::printError(const char *format, ...) {
  va_list args;
