...
RCType ArgParser::parseOptions(int argc, char* argv[], J9TDFOptions* options) {
  RCType rc = RC_OK;
  ...
  for (int i = 1; i < argc; i++) {
    if (StringUtils::startsWithUpperLower(argv[i], "-root") ||
      ...
      if (i >= argc) {
        FileUtils::printError(
            "TraceGen: threshold specified with no number aborting\n");
        rc = RC_FAILED;
        goto done;
      } else {
      ...
      if (i >= argc) {
        FileUtils::printError(
            "TraceGen: majorversion specified with no number aborting\n");
        rc = RC_FAILED;
        goto done;
      } else {
      ...
      if (i >= argc) {
        FileUtils::printError(
            "TraceGen: minorversion specified with no number aborting\n");
        rc = RC_FAILED;
        goto done;
      } else {
    ...
    } else if (StringUtils::startsWithUpperLower(argv[i],
                                                 "-treatWarningAsError")) {
...
RCType ArgParser::freeOptions(J9TDFOptions* options) {
