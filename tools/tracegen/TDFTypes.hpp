...
typedef struct J9TDFHeader {
  ...
  bool auxiliary;
  J9TDFHeader()
      : executable(NULL), submodules(NULL), datfilename(NULL),
        auxiliary(false) {}
} J9TDFHeader;
...
typedef struct J9TDFTracepoint {
  ...
  J9TDFTracepoint()
      : type(UT_EVENT_TYPE), overhead(0), level(0), parmCount(0), hasEnv(false),
        obsolete(false), test(false), isExplicit(false), name(NULL),
        groups(NULL), format(NULL), parameters(NULL), nexttp(NULL) {}
} J9TDFTracepoint;
...
typedef struct J9TDFOptions {
  ...
  J9TDFOptions()
      : rasMajorVersion(5), rasMinorVersion(1), threshold(1), force(false),
        generateCFiles(false), writeToCurrentDir(false), rootDirectory(NULL),
        files(NULL), debugOutput(false), verboseOutput(false),
