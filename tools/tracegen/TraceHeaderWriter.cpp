...
const char* TP_ASSERT_TEMPLATE = "#if UT_TRACE_OVERHEAD >= %u\n"
...
const char* TP_TEMPLATE = "#if UT_TRACE_OVERHEAD >= %u\n"
                          "%s" /* Place holder for option test macro (specified by "Test" option in tp spec) */
                          "#define %s(%s%s) do { /* tracepoint name: %s.%u */ \\\n"
                          "	if ((unsigned char) %s_UtActive[%u] != 0){ \\\n"
...
TraceHeaderWriter::tpTemplate(FILE* fd, unsigned int overhead, unsigned int test, const char* name, const char* module, unsigned int id, unsigned int envParam, const char* parameters, unsigned int parmCount, unsigned int auxiliary)
{
    ...
    if (auxiliary) {
        if (0 < fprintf(fd, TP_AUX_TEMPLATE, overhead, testMacro, name, envParam ? "thr" : "", envParam ? parmString : parmStringNoLeadingComma, module, id, module, envParam ? UT_ENV_PARAM : UT_NOENV_PARAM, module, id, parameters, parmString, testNop, name, envParam ? "thr" : "", envParam ? parmString : parmStringNoLeadingComma, module, id)) {
            rc = RC_OK;
        } else {
            rc = RC_FAILED;
            goto failed;
        ...
        if (0 <= fprintf(fd, TP_TEMPLATE, overhead, testMacro, name, envParam ? "thr" : "", envParam ? parmString : parmStringNoLeadingComma, module, id, module, id, module, envParam ? UT_ENV_PARAM : UT_NOENV_PARAM, module, id, module, id, parameters, parmString, testNop, name, envParam ? "thr" : "", envParam ? parmString : parmStringNoLeadingComma, module, id)) {
            rc = RC_OK;
        } else {
            rc = RC_FAILED;
            goto failed;
...
TraceHeaderWriter::tpAssert(FILE* fd, unsigned int overhead, unsigned int test, const char* name, const char* module, unsigned int id, unsigned int envParam, const char* conditionStr, unsigned int parmCount)
{
    ...
    if (0 <= fprintf(fd, TP_ASSERT_TEMPLATE, overhead, testmacro, name, envParam ? "thr" : parmStringNoLeadingComma, module, id, module, id, conditionStr, module, module, envParam ? "thr" : "(void *)NULL", module, id, module, id, conditionStr, module, id, name, conditionStr, testnop, name, envParam ? "thr" : parmStringNoLeadingComma, module, id)) {
        rc = RC_OK;
    } else {
        rc = RC_FAILED;
        goto failed;
