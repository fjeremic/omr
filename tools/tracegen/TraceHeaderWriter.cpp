...
const char* TP_ASSERT_TEMPLATE
    = "#if UT_TRACE_OVERHEAD >= %u\n"
      ...
      "				%s_UtModuleInfo.intf->Trace(%s, &%s_UtModuleInfo, (UT_SPECIAL_ASSERTION | (%uu << 8) | "
      "%s_UtActive[%u]), \"\\377\\4\\377\", __FILE__, __LINE__, UT_STR((%s))); \\\n"
      "				Trace_Unreachable(); \\\n"
      "			} else { \\\n"
    ...
    while (NULL != tp) {
        if (!tp->obsolete) {
            if (UT_ASSERT_TYPE == tp->type) {
                tpAssert(fd, tp->overhead, tp->test, tp->name, tdf->header.executable, id, tp->hasEnv, tp->format,
                    tp->parmCount);
            } else {
    ...
    const char* module, unsigned int id, unsigned int envParam, const char* conditionStr, unsigned int parmCount)
{
    ...
    if (0 <= fprintf(fd, TP_ASSERT_TEMPLATE, overhead, testmacro, name, envParam ? "thr" : parmStringNoLeadingComma,
            module, id, module, id, conditionStr, module, module, envParam ? "thr" : "(void *)NULL", module, id, module,
            id, conditionStr, module, id, name, conditionStr, testnop, name,
