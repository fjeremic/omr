...
const char *UT_H_FILE_HEADER_TEMPLATE =
    ...
    "#endif\n"
    "\n"
    "int32_t register%sWithTrace(UtInterface * utIntf, UtModuleInfo* "
    "containerName);\n"
...
const char *TP_ASSERT_TEMPLATE =
    ...
    "%s" /* Place holder for option test macro (specified by "Test" option in tp
            spec) */
    ...
    "&%s_UtModuleInfo, (UT_SPECIAL_ASSERTION | (%uu << 8) | %s_UtActive[%u]), "
    "\"\\377\\4\\377\", __FILE__, __LINE__, UT_STR((%s))); \\\n"
    "				Trace_Unreachable(); \\\n"
    "			} else { \\\n"
    "				fprintf(stderr, \"** ASSERTION FAILED ** %s.%u "
...
const char *TP_AUX_TEMPLATE =
    ...
    "		%s_UtModuleInfo.intf->Trace(%s, &%s_UtModuleInfo, ((%uu << "
    "8)), %s%s);	} while(0)\n"
    "#else\n"
    "%s" /* Place holder for option test macro (specified by "Test" option in tp
            spec) */
...
const char *TP_TEMPLATE =
    ...
    "#define %s(%s%s) do { /* tracepoint name: %s.%u */ \\\n"
    "	if ((unsigned char) %s_UtActive[%u] != 0){ \\\n"
    "		%s_UtModuleInfo.intf->Trace(%s, &%s_UtModuleInfo, ((%uu << 8) "
    "| %s_UtActive[%u]), %s%s);} \\\n"
    "	} while(0)\n"
    "#else\n"
    "%s" /* Place holder for option test macro (specified by "Test" option in tp
...
RCType TraceHeaderWriter::writeOutputFiles(J9TDFOptions *options,
  ...
  const char *fileName = FileUtils::getTargetFileName(
      options, tdf->fileName, UT_FILENAME_PREFIX, tdf->header.executable, ".h");
  ...
  while (NULL != tp) {
    if (!tp->obsolete) {
      if (UT_ASSERT_TYPE == tp->type) {
        tpAssert(fd, tp->overhead, tp->test, tp->name, tdf->header.executable,
                 id, tp->hasEnv, tp->format, tp->parmCount);
      } else {
        tpTemplate(fd, tp->overhead, tp->test, tp->name, tdf->header.executable,
                   id, tp->hasEnv, tp->parameters, tp->parmCount,
                   tdf->header.auxiliary);
      }
    }
    id++;
    tp = tp->nexttp;
 ...
 * 5 - a string containing "UT_THREAD(thr)" or "(void *)NULL" if NoEnv was
 * specified for this trace point. 6 - NULL or trace point argument string and
 * varargs parameters.
 *
 * Note: windows doesn't support doing %2$s (which is a bit of a pain)
 */
RCType TraceHeaderWriter::tpTemplate(
    FILE *fd, unsigned int overhead, unsigned int test, const char *name,
    const char *module, unsigned int id, unsigned int envParam,
    const char *parameters, unsigned int parmCount, unsigned int auxiliary) {
  RCType rc = RC_FAILED;
  ...
  char *testMacro = NULL;
  char *testNop = NULL;
  char *testMacroTemplate =
      (char *)"#define TrcEnabled_%s  (%s_UtActive[%u] != 0)\n";
  char *testNopTemplate = (char *)"#define TrcEnabled_%s  (0)\n";

  parmString =
      (char *)Port::omrmem_calloc(1, (parmCount * sizeof(char) * 5) + 1);
  if (NULL == parmString) {
  ...
  if (test) {
    /* Allow 7 digits for tracepoints + 1 for the null byte. (Millions of trace
     * points are unlikely.) */
    testMacro = (char *)Port::omrmem_calloc(
        1, (strlen(testMacroTemplate) + strlen(name) + strlen(module) + 8));
    if (NULL == testMacro) {
      eprintf("Failed to allocate memory");
      goto failed;
    }
    ...
    testNop = (char *)Port::omrmem_calloc(
        1, (strlen(testMacroTemplate) + strlen(name) + 1));
    if (NULL == testNop) {
      eprintf("Failed to allocate memory");
      goto failed;
    }
  ...
  if (auxiliary) {
    if (0 <
        fprintf(fd, TP_AUX_TEMPLATE, overhead, testMacro, name,
                envParam ? "thr" : "",
                envParam ? parmString : parmStringNoLeadingComma, module, id,
                module, envParam ? UT_ENV_PARAM : UT_NOENV_PARAM, module, id,
                parameters, parmString, testNop, name, envParam ? "thr" : "",
                envParam ? parmString : parmStringNoLeadingComma, module, id)) {
      rc = RC_OK;
    } else {
      rc = RC_FAILED;
    ...
    if (0 <=
        fprintf(fd, TP_TEMPLATE, overhead, testMacro, name,
                envParam ? "thr" : "",
                envParam ? parmString : parmStringNoLeadingComma, module, id,
                module, id, module, envParam ? UT_ENV_PARAM : UT_NOENV_PARAM,
                module, id, module, id, parameters, parmString, testNop, name,
                envParam ? "thr" : "",
                envParam ? parmString : parmStringNoLeadingComma, module, id)) {
      rc = RC_OK;
    } else {
...
RCType TraceHeaderWriter::tpAssert(FILE *fd, unsigned int overhead,
                                   unsigned int test, const char *name,
  ...
  if (0 <= fprintf(fd, TP_ASSERT_TEMPLATE, overhead, testmacro, name,
                   envParam ? "thr" : parmStringNoLeadingComma, module, id,
                   module, id, conditionStr, module, module,
                   envParam ? "thr" : "(void *)NULL", module, id, module, id,
                   conditionStr, module, id, name, conditionStr, testnop, name,
                   envParam ? "thr" : parmStringNoLeadingComma, module, id)) {
    rc = RC_OK;
  } else {
    rc = RC_FAILED;
...
RCType TraceHeaderWriter::headerTemplate(J9TDFOptions *options, FILE *fd,
  ...
  if (0 <= fprintf(fd, UT_H_FILE_HEADER_TEMPLATE, ucModule, ucModule,
                   options->threshold, moduleName, moduleName, moduleName,
                   moduleName, ucModule, moduleName, ucModule, moduleName,
                   moduleName, moduleName, moduleName, moduleName, ucModule,
                   moduleName, ucModule, moduleName)) {
    rc = RC_OK;
  } else {
    rc = RC_FAILED;
    goto failed;
...
RCType TraceHeaderWriter::footerTemplate(FILE *fd, const char *moduleName) {
  char *ucModule = NULL;
  char *pos = NULL;
  RCType rc = RC_FAILED;
  ...
  if (0 <= fprintf(fd, UT_H_FILE_FOOTER_TEMPLATE, moduleName, moduleName,
                   moduleName, moduleName, ucModule)) {
    rc = RC_OK;
  } else {
    rc = RC_FAILED;
    goto failed;
