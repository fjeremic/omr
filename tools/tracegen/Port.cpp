...
wchar_t*
Port::file_get_unicode_path(const char* utf8Path,
                            ...
                            unsigned int unicodeBufferSize)
{
  ...
  if (pathLen * sizeof(wchar_t) <= EsMaxPath) {
    ...
    memmove(fullPathNameBuffer + wcslen(J9FILE_UNC_EXTENDED_LENGTH_PREFIX),
            fullPathNameBuffer,
            fullPathNameLengthInWcharts * sizeof(wchar_t));
    wcsncpy(fullPathNameBuffer,
            J9FILE_UNC_EXTENDED_LENGTH_PREFIX,
...
wchar_t*
Port::convertFromUTF8(const char* string,
                      wchar_t* unicodeBuffer,
                      unsigned int unicodeBufferSize)
{
...
{
  int sizeNeeded = WideCharToMultiByte(OS_ENCODING_CODE_PAGE,
  ...
  *utf8Buffer = (char*)Port::omrmem_malloc(sizeNeeded);
  ...
  if (0 == WideCharToMultiByte(OS_ENCODING_CODE_PAGE,
                               OS_ENCODING_WC_FLAGS,
                               unicodeString,
                               -1,
                               *utf8Buffer,
...
Port::omrfile_realpath(const char* utf8Path)
{
  ...
  fullPathNameLengthInWcharts = GetFullPathNameW(
    unicodePath, 0 /* zero means give us back the size we need */, NULL, NULL);
  if (0 == fullPathNameLengthInWcharts) {
    pathName = NULL;
    goto cleanup;
  }
