...
#if defined(OMR_OS_WINDOWS)
RCType Port::omrfile_findfirst(const char* path, char** resultbuf, intptr_t* handle)
{
...
wchar_t* Port::file_get_unicode_path(const char* utf8Path, wchar_t* unicodeBuffer, unsigned int unicodeBufferSize)
{
  wchar_t* unicodePath = NULL;
...
wchar_t* Port::convertFromUTF8(const char* string, wchar_t* unicodeBuffer, unsigned int unicodeBufferSize)
{
  ...
  if (0
    == MultiByteToWideChar(OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, string, -1, unicodeString, (int)length + 1)) {
...
{
  int sizeNeeded
    = WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeString, -1, NULL, 0, NULL, NULL);
  if (0 == sizeNeeded) {
    goto failed;
  }
  *utf8Buffer = (char*)Port::omrmem_malloc(sizeNeeded);
  ...
  if (0
    == WideCharToMultiByte(
      OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeString, -1, *utf8Buffer, sizeNeeded, NULL, NULL)) {
    goto failed;
  }
  return RC_OK;
failed:
...
RCType Port::omrfile_stat(const char* path, unsigned int flags, struct J9FileStat* buf)
{
  DWORD result;
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE];
  wchar_t* unicodePath;
...
char* Port::omrfile_realpath(const char* utf8Path)
{
   ...
   * The only way to get the fully qualified path is using GetFullPathNameW. The MSDN docs for GetFullPathNameW state
   * that it should not be used in multi-threaded applications, but the docs are fairly clear that this is because the
   * conversion from a relative path to a fully qualified path depends on the current working directory (CWD), which is
   * a process global that may change.
   *
   * However, we can't simply limit the use of GetFullPathNameW to non-absolute paths, because all occurrences of "."
   * and "..", need to be removed. This includes the occurrences that don't simply prepend the path, in which case the
   * CWD is not relevant and where we assume the call to GetFullPathNameW is thread-safe.
