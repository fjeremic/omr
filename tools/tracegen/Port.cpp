...
#if defined(OMR_OS_WINDOWS)
RCType Port::omrfile_findfirst(const char *path, char **resultbuf,
                               intptr_t *handle) {
  ...
  unicodePath =
      file_get_unicode_path(newPath, unicodeBuffer, UNICODE_BUFFER_SIZE);
  if (NULL == unicodePath) {
    goto failed;
  }
...
RCType Port::omrfile_findclose(intptr_t findhandle) {
  RCType rc = RC_FAILED;
...
RCType Port::omrfile_findnext(intptr_t findhandle, char **resultbuf) {
  WIN32_FIND_DATAW findFileData;
...
wchar_t *Port::file_get_unicode_path(const char *utf8Path,
  ...
  /* If the file name is longer than (the system-defined) EsMaxPath characters
  ...
  if (pathLen * sizeof(wchar_t) <= EsMaxPath) {
    ...
    /*
     * We need to prepend the path with "\\?\", but this format is only
     * supported if the path is fully qualified.
     *
    ...
    fullPathNameBuffer =
        (wchar_t *)Port::omrmem_malloc(fullPathNameBufferSizeBytes);
    ...
    memmove(fullPathNameBuffer + wcslen(J9FILE_UNC_EXTENDED_LENGTH_PREFIX),
...
wchar_t *Port::convertFromUTF8(const char *string, wchar_t *unicodeBuffer,
                               unsigned int unicodeBufferSize) {
...
RCType Port::convertToUTF8(const wchar_t *unicodeString, char **utf8Buffer) {
  int sizeNeeded =
      WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                          unicodeString, -1, NULL, 0, NULL, NULL);
  if (0 == sizeNeeded) {
    goto failed;
  }
  *utf8Buffer = (char *)Port::omrmem_malloc(sizeNeeded);
  ...
  if (0 == WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS,
                               unicodeString, -1, *utf8Buffer, sizeNeeded, NULL,
                               NULL)) {
    goto failed;
  }
  return RC_OK;
...
FILE *Port::fopen(const char *path, const char *mode) {
  ...
  unicodePath =
      file_get_unicode_path(path, unicodePathBuffer, UNICODE_BUFFER_SIZE);
...
RCType Port::omrfile_stat(const char *path, unsigned int flags,
...
RCType Port::stat(const char *path, dirstat *statbuf) {
  RCType rc = RC_FAILED;
  wchar_t unicodePathBuffer[UNICODE_BUFFER_SIZE];
  wchar_t *unicodePath = NULL;

  unicodePath =
      Port::file_get_unicode_path(path, unicodePathBuffer, UNICODE_BUFFER_SIZE);
...
RCType Port::omrfile_move(const char *pathExist, const char *pathNew) {
  ...
  unicodePathExist =
      file_get_unicode_path(pathExist, unicodeBufferExist, UNICODE_BUFFER_SIZE);
  if (NULL == unicodePathExist) {
    return RC_FAILED;
  }
  unicodePathNew =
      file_get_unicode_path(pathNew, unicodeBufferNew, UNICODE_BUFFER_SIZE);
...
char *Port::omrfile_realpath(const char *utf8Path) {
  wchar_t *unicodePath = NULL;
  wchar_t *pathName = NULL;
  size_t pathLen = 0;
  wchar_t unicodeBuffer[UNICODE_BUFFER_SIZE];
  ...
  /*
   * We need to prepend the path with "\\?\", but this format is only supported
   * if the path is fully qualified.
   *
   * The only way to get the fully qualified path is using GetFullPathNameW. The
   * MSDN docs for GetFullPathNameW state that it should not be used in
   * multi-threaded applications, but the docs are fairly clear that this is
   * because the conversion from a relative path to a fully qualified path
   * depends on the current working directory (CWD), which is a process global
   * that may change.
   *
   * However, we can't simply limit the use of GetFullPathNameW to non-absolute
   * paths, because all occurrences of "." and "..", need to be removed. This
   * includes the occurrences that don't simply prepend the path, in which case
   * the CWD is not relevant and where we assume the call to GetFullPathNameW is
   * thread-safe.
   *
   */
  ...
  getFullPathNameRc = GetFullPathNameW(unicodePath, fullPathNameLengthInWcharts,
...
char *Port::omrfile_realpath(const char *path) {
  char *result = NULL;
...
int Port::strncasecmp(const char *s1, const char *s2, size_t n) {
#if defined(OMR_OS_WINDOWS)
