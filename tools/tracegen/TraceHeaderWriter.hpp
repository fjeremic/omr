...
private:
   ...
   */
  RCType tpTemplate(FILE *fd, unsigned int overhead, unsigned int test,
                    const char *name, const char *module, unsigned int id,
                    unsigned int envparam, const char *format,
                    unsigned int formatParamCount, unsigned int auxiliary);
   ...
   *  @return RC_OK on success, RC_FAILED on failure
   */
  RCType tpAssert(FILE *fd, unsigned int overhead, unsigned int test,
                  const char *name, const char *module, unsigned int id,
                  unsigned int envparam, const char *format,
                  unsigned int formatParamCount);
   ...
   * @return RC_OK on success, RC_FAILED on failure
   */
  RCType headerTemplate(J9TDFOptions *options, FILE *fd,
