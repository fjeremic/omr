...
RCType
CFileWriter::writeOutputFiles(J9TDFOptions* options,
                              J9TDFFile* tdf,
                              J9TDFGroup* groups,
  ...
  if (NULL == fd) {
    ...
    while (NULL != tp) {
      ...
      if ((UT_ASSERT_TYPE == tp->type) && (1 == tp->level)) {
        assertDefaults[i] =
          1; /* Enable default assertions prior to full trace initialisation. */
      } else {
        assertDefaults[i] = 0;
      }
      tp = tp->nexttp;
    ...
    if (RC_OK !=
        writeComponentDataOnStream(
          fd, tdf, tpCount, levels, assertDefaults, groups, groupsCount)) {
...
RCType
CFileWriter::writeComponentDataOnStream(FILE* fd,
                                        J9TDFFile* tdf,
...
CFileWriter::writeRegistrationFunctionsOnStream(FILE* fd, J9TDFFile* tdf)
{
  ...
  fprintf(fd, "int32_t\n"); /* type */
  fprintf(fd,
          "register%sWithTrace(%s%s, UtModuleInfo * containerModule)\n",
          tdf->header.executable,
          parameterType,
  ...
  fprintf(fd, "\n\tif (rc == %s) {\n", ok);
  fprintf(fd,
          "\t\tutIntf->module->TraceInit(NULL, &%s_UtModuleInfo);\n",
          tdf->header.executable);
  fprintf(fd, "\t}\n");
  ...
  if (NULL != tdf->header.submodules) {
    ...
    while (NULL != registerSubmodule) {
      ...
      fprintf(fd,
              ...
              parameterName);
      fprintf(fd,
              "\t\trc = register%sWithTrace(%s, &%s_UtModuleInfo);\n",
              registerSubmodule,
              parameterName,
              tdf->header.executable);
      fprintf(fd, "\t}\n");
  ...
  fprintf(fd, "\n\tif (rc == %s) {\n", ok);
  fprintf(fd,
          "\t\tutIntf->module->TraceTerm(NULL, &%s_UtModuleInfo);\n",
          tdf->header.executable);
  fprintf(fd, "\t}\n");
  ...
  if (NULL != tdf->header.submodules) {
    ...
    while (NULL != unregisterSubmodule) {
      fprintf(fd, "\n\tif (rc == %s) {\n", ok);
      fprintf(fd,
              "\t\textern int32_t deregister%sWithTrace(%s%s);\n\n",
              unregisterSubmodule,
              parameterType,
              parameterName);
      fprintf(fd,
              "\t\trc = deregister%sWithTrace(%s);\n",
              unregisterSubmodule,
              parameterName);
      fprintf(fd, "\t}\n");
...
RCType
CFileWriter::writeComponentDataForNonTraceEnabledBuildsOnStream(FILE* fd,
                                                                J9TDFFile* tdf)
{
  RCType rc = RC_FAILED;
...
RCType
CFileWriter::writeModuleInfo(FILE* fd,
                             J9TDFFile* tdf,
                             unsigned int ntracepoints)
{
  RCType rc = RC_FAILED;
...
RCType
CFileWriter::writeGroups(FILE* fd,
                         J9TDFFile* tdf,
                         J9TDFGroup* groups,
...
RCType
CFileWriter::writeLevels(FILE* fd,
                         J9TDFFile* tdf,
                         unsigned int ntracepoints,
                         unsigned int levels[])
{
...
RCType
CFileWriter::writeActiveArray(FILE* fd,
                              J9TDFFile* tdf,
                              unsigned int ntracepoints,
                              unsigned int earlyAssertDefaults[])
{
