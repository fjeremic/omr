...
CFileWriter::writeComponentDataForNonTraceEnabledBuildsOnStream(FILE* fd, J9TDFFile* tdf)
{
    ...
    fprintf(fd, "UtModuleInfo %s_UtModuleInfo = {(char*) \"%s\", %u, 0, 0, NULL, NULL, NULL, &%s_UtTraceVersionInfo, (char*) \"%s\", NULL, NULL, NULL, NULL, 0, 0};\n", tdf->header.executable, tdf->header.executable, (unsigned int)strlen(tdf->header.executable), tdf->header.executable, tdf->header.datfilename);
    rc = RC_OK;
    return rc;
}
...
{
    RCType rc = RC_FAILED;
    fprintf(fd, "UtModuleInfo %s_UtModuleInfo = {(char*) \"%s\", %u, %u, 0, %s_UtActive , NULL, NULL, &%s_UtTraceVersionInfo, (char*) \"%s\", %s_UtLevels, &%s_group0, NULL, NULL, 0, %s};", tdf->header.executable, tdf->header.executable, (unsigned int)strlen(tdf->header.executable), ntracepoints, tdf->header.executable, tdf->header.executable, tdf->header.datfilename, tdf->header.executable, tdf->header.executable, tdf->header.auxiliary ? "1" : "0");
    rc = RC_OK;
    return rc;
}
