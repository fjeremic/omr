...
RCType CFileWriter::writeOutputFiles(J9TDFOptions *options, J9TDFFile *tdf,
                                     J9TDFGroup *groups,
  ...
  const char *fileName = FileUtils::getTargetFileName(
      options, tdf->fileName, UT_FILENAME_PREFIX, tdf->header.executable, ".c");
  ...
  if (NULL == fd) {
    ...
    if (tpCount > 0) {
      levels =
          (unsigned int *)Port::omrmem_calloc(tpCount, sizeof(unsigned int));
      assertDefaults =
          (unsigned int *)Port::omrmem_calloc(tpCount, sizeof(unsigned int));
    } else {
      levels = (unsigned int *)Port::omrmem_calloc(1, sizeof(unsigned int));
      assertDefaults =
          (unsigned int *)Port::omrmem_calloc(1, sizeof(unsigned int));
    }
    ...
    fprintf(fd,
            "UtTraceVersionInfo %s_UtTraceVersionInfo = {UT_TRACE_VERSION};\n",
            tdf->header.executable);
    ...
    if (RC_OK != writeComponentDataOnStream(fd, tdf, tpCount, levels,
                                            assertDefaults, groups,
                                            groupsCount)) {
...
RCType CFileWriter::writeComponentDataOnStream(
    FILE *fd, J9TDFFile *tdf, unsigned int ntracepoints, unsigned int *levels,
    unsigned int *earlyAssertDefaults, J9TDFGroup *groups,
    unsigned int groupsCount) {
  RCType rc = RC_OK;
  if (0 == ntracepoints) {
...
RCType CFileWriter::writeRegistrationFunctionsOnStream(FILE *fd,
                                                       J9TDFFile *tdf) {
  ...
  fprintf(fd, "/* function to register with trace engine and configure current "
              "module */\n");
  fprintf(fd, "int32_t\n"); /* type */
  ...
  if (NULL != tdf->header.submodules) {
    ...
    while (NULL != registerSubmodule) {
      fprintf(fd, "\n\tif (rc == %s) {\n", ok);
      fprintf(fd,
              "\t\textern int32_t register%sWithTrace(%s%s, UtModuleInfo* "
              "containerModule);\n\n",
  ...
  fprintf(fd, "\n/* function to deregister with trace engine and configure "
              "current module */\n");
  fprintf(fd, "int32_t\n"); /* type */
...
RCType CFileWriter::writeComponentDataForNonTraceEnabledBuildsOnStream(
    FILE *fd, J9TDFFile *tdf) {
  RCType rc = RC_FAILED;
  fprintf(fd, "unsigned char %s_UtActive[1];\n", tdf->header.executable);
  fprintf(fd,
          "UtModuleInfo %s_UtModuleInfo = {(char*) \"%s\", %u, 0, 0, NULL, "
          "NULL, NULL, &%s_UtTraceVersionInfo, (char*) \"%s\", NULL, NULL, "
          "NULL, NULL, 0, 0};\n",
          tdf->header.executable, tdf->header.executable,
          (unsigned int)strlen(tdf->header.executable), tdf->header.executable,
          tdf->header.datfilename);
  rc = RC_OK;
  return rc;
}

RCType CFileWriter::writeModuleInfo(FILE *fd, J9TDFFile *tdf,
                                    unsigned int ntracepoints) {
  RCType rc = RC_FAILED;
  fprintf(fd,
          "UtModuleInfo %s_UtModuleInfo = {(char*) \"%s\", %u, %u, 0, "
          "%s_UtActive , NULL, NULL, &%s_UtTraceVersionInfo, (char*) \"%s\", "
          "%s_UtLevels, &%s_group0, NULL, NULL, 0, %s};",
          tdf->header.executable, tdf->header.executable,
          (unsigned int)strlen(tdf->header.executable), ntracepoints,
          tdf->header.executable, tdf->header.executable,
          tdf->header.datfilename, tdf->header.executable,
          tdf->header.executable, tdf->header.auxiliary ? "1" : "0");
  rc = RC_OK;
  return rc;
}

RCType CFileWriter::writeGroups(FILE *fd, J9TDFFile *tdf, J9TDFGroup *groups,
                                unsigned int tracegroups) {
...
RCType CFileWriter::writeLevels(FILE *fd, J9TDFFile *tdf,
                                unsigned int ntracepoints,
                                unsigned int levels[]) {
  J9TDFTracepoint *tp = tdf->tracepoints;
  unsigned int id = 0;
  ...
  fprintf(fd, "unsigned char %s_UtLevels[%u] = { ", tdf->header.executable,
          ntracepoints);
  while (NULL != tp) {
...
RCType CFileWriter::writeActiveArray(FILE *fd, J9TDFFile *tdf,
                                     unsigned int ntracepoints,
  ...
  fprintf(fd, "unsigned char %s_UtActive[%u] = { ", tdf->header.executable,
          ntracepoints);
  while (NULL != tp) {
    if (0 == (id % PRETTY_PRINT_TP_LIMIT)) {
      fprintf(fd, "\n\t");
