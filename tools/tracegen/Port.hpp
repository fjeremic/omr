...
public:
   ...
   * 					If the return value is not the same as @ref unicodeBuffer, file_get_unicode_path had to allocate memory to
   * hold the unicode path - the caller is responsible for freeing this memory by calling @ref omrmem_free on the
   * returned value.
   */
