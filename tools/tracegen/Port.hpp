...
public:
#if defined(OMR_OS_WINDOWS)
  /**
   * Uses convertFromUTF8 and if the resulting unicode path is longer than the
   * Windows defined MAX_PATH, the path is converted to an absolute path and
   * prepended with //?/
   *
   ...
   * file_get_unicode_path had to allocate memory to hold the unicode path - the
   * caller is responsible for freeing this memory by calling @ref omrmem_free
  ...
  /**
   * @internal
   * Converts the UTF8 encoded string to Unicode. Use the provided buffer if
   * possible or allocate memory as necessary. Return the new string.
   *
   * @param[in] portLibrary The port library
   * @param[in] string The UTF8 encoded string
   * @param[in] unicodeBuffer The unicode buffer to attempt to use
   * @param[in] unicodeBufferSize The number of bytes available in the unicode
   * buffer
   *
   * @return	the pointer to the Unicode string, or NULL on failure.
   *
   * If the returned buffer is not the same as @ref unicodeBuffer, the returned
   * buffer needs to be freed using @ref omrmem_free_memory
   */
  static wchar_t *convertFromUTF8(const char *string, wchar_t *unicodeBuffer,
                                  unsigned int unicodeBufferSize);
   ...
   * @param[in] utf8Buffer Address of a buffer to store the UTF8 encoded bytes
   * into. Buffer of appropriate size is allocated to hold UTF8 encoded string.
   * - utf8 string will be NUL terminated.
   * - the caller is responsible for freeing this memory by calling @ref
   * omrmem_free on the returned value.
  ...
  static RCType omrfile_findfirst(const char *path, char **resultbuf,
                                  intptr_t *handle);
  ...
  static RCType omrfile_stat(const char *path, unsigned int flags,
  ...
  /**
   * Resolve full path to the file. The caller should deallocate this buffer
   * using omrmem_free.
   ...
   * @see strncasecmp
   * @return negative value, zero or positive value if s1 is less then, equal to
   * or greater than s2.
   */
