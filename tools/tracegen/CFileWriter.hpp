...
class CFileWriter {
 ...
 private:
   ...
   */
  RCType writeActiveArray(FILE *fd, J9TDFFile *tdf, unsigned int ntracepoints,
                          unsigned int earlyAssertDefaults[]);
   ...
   * @return RC_OK on success, RC_FAILED on failure
   */
  RCType writeLevels(FILE *fd, J9TDFFile *tdf, unsigned int ntracepoints,
                     unsigned int levels[]);
   ...
   * @return RC_OK on success, RC_FAILED on failure
   */
  RCType writeGroups(FILE *fd, J9TDFFile *tdf, J9TDFGroup *groups,
   ...
   * @return RC_OK on success, RC_FAILED on failure
   */
  RCType writeComponentDataOnStream(FILE *fd, J9TDFFile *tdf,
                                    unsigned int ntracepoints,
                                    unsigned int *levels,
                                    unsigned int *earlyAssertDefaults,
                                    J9TDFGroup *groups,
                                    unsigned int groupsCount);
   ...
   * @return RC_OK on success, RC_FAILED on failure
   */
  RCType writeComponentDataForNonTraceEnabledBuildsOnStream(FILE *fd,
                                                            J9TDFFile *tdf);
 ...
 public:
  ...
  RCType writeOutputFiles(J9TDFOptions *options, J9TDFFile *tdf,
                          J9TDFGroup *groups, unsigned int groupCount);
};
