...
static RCType help(int argc, char* argv[])
{
  for (int i = 0; i < argc; i++) {
    if (0 == strcmp(argv[i], "-help") || 0 == strcmp(argv[i], "--help") || 0 == strcmp(argv[i], "-h")
      ...
      printf("%s [-threshold num] [-w2cd] [-generateCfiles] [-treatWarningAsError] [-root rootDir] [-file file.tdf] "
             "[-force]\n",
        argv[0]);
      printf("\t-threshold Ignore trace level below this threshold (default 1)\n");
      printf("\t-w2cd Write generated .C and .H files to current directory (default: generate in the same directory as "
             "the TDF file)\n");
...
J9TDFGroup* TraceGen::newGroup(const char* groupName, unsigned int* groupCount)
{
...
J9TDFGroup* TraceGen::calculateGroups(J9TDFOptions* options, J9TDFFile* tdf, unsigned int* groupCount)
{
...
RCType TraceGen::generateCallBack(void* targetObject, J9TDFOptions* options, const char* fileName)
{
