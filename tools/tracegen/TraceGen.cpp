...
static RCType help(int argc, char *argv[]) {
  for (int i = 0; i < argc; i++) {
    if (0 == strcmp(argv[i], "-help") || 0 == strcmp(argv[i], "--help") ||
      ...
      printf(
          "%s [-threshold num] [-w2cd] [-generateCfiles] "
          "[-treatWarningAsError] [-root rootDir] [-file file.tdf] [-force]\n",
          argv[0]);
      printf(
          "\t-threshold Ignore trace level below this threshold (default 1)\n");
      printf("\t-w2cd Write generated .C and .H files to current directory "
             "(default: generate in the same directory as the TDF file)\n");
      printf("\t-generateCfiles Generate C files (default false)\n");
      printf("\t-treatWarningAsError Abort parsing at the first TDF error "
             "encountered (default false)\n");
      printf("\t-root Comma-separated directories to start scanning for TDF "
             "files (default .)\n");
      printf("\t-file Comma-separated list of TDF files to process\n");
      printf("\t-force Do not check TDF file timestamp, always generate output "
             "files. (default false)\n");
      return RC_OK;
    }
  }
...
RCType startTraceGen(int argc, char *argv[]) {
  ...
  if (options.force) {
    printf(
        "tracegen -force option is enabled. All files will be regenerated.\n");
  }

  dir = options.rootDirectory;
  while (NULL != dir) {
    /* Recursively visit all directories under dirName, processing TDF files */
    if (RC_OK != FileUtils::visitDirectory(&options, dir->path, TDF_FILE_EXT,
                                           &tracegen,
                                           TraceGen::generateCallBack)) {
...
RCType TraceGen::generate(J9TDFOptions *options, const char *currentTDFFile) {
  RCType rc = RC_FAILED;
   ...
   * To avoid processing trace files multiple times keep the record of processed
   * files. To handle relative paths omrfile_realpath is used to resolve
...
J9TDFGroup *TraceGen::getDefaultGroupSet(unsigned int *groupCount) {
...
RCType TraceGen::addTpIdToGroup(J9TDFGroup *group, unsigned int id) {
  J9TDFGroupTp *newGroupTp =
      (J9TDFGroupTp *)Port::omrmem_calloc(1, sizeof(J9TDFGroupTp));
  if (NULL == newGroupTp) {
    eprintf("Failed to allocate memory");
    goto failed;
  }
...
J9TDFGroup *TraceGen::findGroupByName(J9TDFGroup *head, const char *groupName) {
...
J9TDFGroup *TraceGen::newGroup(const char *groupName,
                               unsigned int *groupCount) {
  J9TDFGroup *newGroup =
      (J9TDFGroup *)Port::omrmem_calloc(1, sizeof(J9TDFGroup));
  if (NULL == newGroup) {
    eprintf("Failed to allocate memory");
    goto failed;
  }
...
J9TDFGroup *TraceGen::calculateGroups(J9TDFOptions *options, J9TDFFile *tdf,
                                      unsigned int *groupCount) {
  const char *EVENT_TYPES[] = {
      "Event",    "Exception",       "Entry", "Entry-Exception",
      "Exit",     "Exit-Exception",  "Mem",   "MemException",
      "Debug",    "Debug-Exception", "Perf",  "Perf-Exception",
      "Assertion"};
...
RCType TraceGen::freeGroups(J9TDFGroup *head) {
...
RCType TraceGen::generateCallBack(void *targetObject, J9TDFOptions *options,
                                  const char *fileName) {
