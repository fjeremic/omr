...
static RCType help(int argc, char *argv[]) {
  for (int i = 0; i < argc; i++) {
    if (0 == strcmp(argv[i], "-help") || 0 == strcmp(argv[i], "--help") ||
      ...
      printf(
          "\t-w2cd Write generated .C and .H files to current directory "
          "(default: generate in the same directory as the TDF file)\n");
      printf("\t-generateCfiles Generate C files (default false)\n");
      printf(
          "\t-treatWarningAsError Abort parsing at the first TDF error "
          "encountered (default false)\n");
      printf(
          "\t-root Comma-separated directories to start scanning for TDF files "
          "(default .)\n");
      printf("\t-file Comma-separated list of TDF files to process\n");
      printf(
          "\t-force Do not check TDF file timestamp, always generate output "
          "files. (default false)\n");
      return RC_OK;
    }
  }
