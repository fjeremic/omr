...
help(int argc, char* argv[])
{
  for (int i = 0; i < argc; i++) {
    if (0 == strcmp(argv[i], "-help") || 0 == strcmp(argv[i], "--help") ||
      ...
      printf(
        "%s [-threshold num] [-w2cd] [-generateCfiles] [-treatWarningAsError] "
        "[-root rootDir] [-file file.tdf] [-force]\n",
        argv[0]);
      printf(
...
startTraceGen(int argc, char* argv[])
{
  ...
  while (NULL != dir) {
    ...
    if (RC_OK != FileUtils::visitDirectory(&options,
                                           dir->path,
                                           TDF_FILE_EXT,
                                           &tracegen,
...
J9TDFGroup*
TraceGen::calculateGroups(J9TDFOptions* options,
                          J9TDFFile* tdf,
...
RCType
TraceGen::generateCallBack(void* targetObject,
                           J9TDFOptions* options,
                           const char* fileName)
{
