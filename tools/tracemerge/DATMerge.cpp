...
static RCType help(int argc, char *argv[]) {
  for (int i = 0; i < argc; i++) {
    if (0 == strcmp(argv[i], "-help") || 0 == strcmp(argv[i], "--help") ||
      ...
      printf("\t-root Comma-separated directories to start scanning for pdat "
             "files (default .)\n");
      return RC_OK;
    }
  }
...
RCType startTraceMerge(int argc, char *argv[]) {
  ...
  while (NULL != dir) {
    if (0 != FileUtils::visitDirectory(&options, dir->path,
                                       TRACE_FILE_FRAGMENT_EXT, &datMerge,
                                       DATMerge::mergeCallback)) {
...
RCType DATMerge::merge(J9TDFOptions *options, const char *fromFileName) {
  RCType rc = RC_FAILED;
  ...
  if (toFile != NULL) {
    ...
    char *newToFileName =
        (char *)Port::omrmem_calloc(1, strlen(toFileName) + strlen(".new") + 1);
    ...
    while (toFilereader.hasNext()) {
      ...
      if (strlen(next) > 0) {
        ...
        if (0 != strncmp(component, next, len)) {
          /* Important - the buffer will contain formatting strings like %s, %p
           * etc if we just do fprintf(newDatFile, buffer) they will be expanded
           * and fprintf will use garbage off the stack to do it. Luckily
           * escaping the whole thing is trivial - just format a string.
           */
          fprintf(tmpFile, "%s\r\n", next);
        }
      }
    ...
    fprintf(toFile, "%u.%u\n", options->rasMajorVersion,
            options->rasMinorVersion);
  }
...
RCType DATMerge::mergeCallback(void *targetObject, J9TDFOptions *options,
