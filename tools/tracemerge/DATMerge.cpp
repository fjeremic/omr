...
RCType startTraceMerge(int argc, char* argv[])
{
  ...
  while (NULL != dir) {
    if (0
      != FileUtils::visitDirectory(&options, dir->path, TRACE_FILE_FRAGMENT_EXT, &datMerge, DATMerge::mergeCallback)) {
...
RCType DATMerge::mergeCallback(void* targetObject, J9TDFOptions* options, const char* fromFileName)
{
