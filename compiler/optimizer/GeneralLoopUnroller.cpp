...
static TR::ILOpCodes
getConstOpCode(TR::DataType type)
{
...
join(TR_ScratchList<TR::CFGEdge>* to, TR_ScratchList<TR::CFGEdge>* from)
{
  if (from == NULL)
    return to;
  if (to == NULL)
...
static TR_ScratchList<TR::CFGEdge>*
findCorrespondingCFGEdges(TR_Structure* from,
                          TR_Structure* to,
                          TR::Compilation* comp)
{
  if (from->asBlock()) {
    ...
    for (node = it.getFirst(); node; node = it.getNext())
      edges =
        join(edges, findCorrespondingCFGEdges(node->getStructure(), to, comp));
    return edges;
  }
}

static bool
isBranchAtEndOfLoop(TR_RegionStructure* loop, TR::Block* branchBlock)
{
  for (auto e = branchBlock->getSuccessors().begin();
       e != branchBlock->getSuccessors().end();
       ++e) {
...
bool
TR_LoopUnroller::nodeRefersToSymbol(TR::Node* node, TR::Symbol* sym)
{
  if (!node)
    return false;
...
TR::Symbol*
TR_LoopUnroller::findSymbolInTree(TR::Node* node)
{
  if (!node)
    return NULL;
...
static bool
checkNodeFrequency(TR_StructureSubGraphNode* subNode, TR_RegionStructure* loop)
{
...
static bool
hasConnectableEdges(TR::Compilation* comp, TR_RegionStructure* loop)
{
  ...
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    for (auto edge = subNode->getSuccessors().begin();
         edge != subNode->getSuccessors().end();
         ++edge) {
...
bool
TR_LoopUnroller::shouldConnectToNextIteration(TR_StructureSubGraphNode* subNode,
...
void
TR_LoopUnroller::unrollLoopOnce(TR_RegionStructure* loop,
                                ...
                                bool finalUnroll)
{
  ...
  memset(_nodeMapper[CURRENT_MAPPER],
         0,
  ...
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    ...
    /*GGLU*/
    {
      if (isCountedLoop())
        ...
        fixExitEdges(
          subNode->getStructure(), clonedSubNode->getStructure(), branchNode);
    }
  }
  ...
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    ...
    for (auto edge = subNode->getSuccessors().begin();
      ...
      } else if (subNode != branchNode && !clonedSucc) // Type B
      {
        ...
        addExitEdgeAndFixEverything(
          loop, *edge, clonedSubNode, NULL, NULL, context);
      } else if (succ == loop->getEntry()) // Type C
      {
        ...
        if (finalUnroll) {
          if (_unrollKind != CompleteUnroll)
            ...
            redirectBackEdgeToExitDestination(
              loop, branchNode, clonedSubNode, (subNode != branchNode));
          }
        }
...
void
TR_LoopUnroller::redirectBackEdgeToExitDestination(
  TR_RegionStructure* loop,
  TR_StructureSubGraphNode* branchNode,
...
void
TR_LoopUnroller::modifyBranchTree(TR_RegionStructure* loop,
                                  ...
                                  TR_StructureSubGraphNode* branchNode)
{
  ...
  if (pBlock->getEntry() == 0) {
    TR::Block* newBlock =
      TR::Block::createEmptyBlock(branch,
                                  comp(),
  ...
  if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
    ...
    pBlock->append(TR::TreeTop::create(
      comp(),
      TR::Node::createif(
        opCode, originalLimitClone, c2, spillBlock->getEntry())));
  ...
  if (1) /// moveEntryNode
  {
    ...
    TR::TreeTop* iterGuard = TR::TreeTop::create(
      comp(),
      TR::Node::createif(
        opCode, firstChild, dupNewLimit, spillBlock->getEntry()));
    ...
    if (lastTree->getNode()->getOpCodeValue() != TR::Goto) {
      ...
      TR::CFGEdge::createEdge(
        newSubNode,
        parent->findSubNodeInRegion(nextBlock->getNumber()),
        trMemory());
...
bool
TR_LoopUnroller::isInternalPointerLimitExceeded()
{
...
void
TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure* loop,
                                    TR_StructureSubGraphNode* branchNode)
{
  ...
  if (_spillLoopRequired) {
    ...
    if (1) {
      ...
      TR::Block* newIfBlock =
        TR::Block::createEmptyBlock(branchTreeTop->getNode(),
                                    comp(),
                                    origExitBlock->getFrequency(),
                                    origExitBlock);
      ...
      // Add L->I
      TR::CFGEdge::createEdge(
        loopNode, newIfSubNode, trMemory()); // the CFG is already correct
  ...
  if (0 && _spillLoopRequired) {
    ...
    if (!skipIt) {
      ...
      // add L->I
      TR::CFGEdge::createEdge(
        loopNode, newIfSubNode, trMemory()); // the CFG is already fine

      // Remove L->S'
      if (1) {
        for (auto edge = loopNode->getSuccessors().begin();
             edge != loopNode->getSuccessors().end();
             ++edge) {
      ...
      if (_overflowTestBlock) {
        ...
        _cfg->addEdge(TR::CFGEdge::createEdge(
          _overflowTestBlock, newEntryBlock, trMemory()));
      ...
      if (1 && _loopIterTestBlock) {
        ...
        _cfg->addEdge(TR::CFGEdge::createEdge(
          _loopIterTestBlock, newEntryBlock, trMemory()));
      }
  ...
  if (isCountedLoop() && _unrollKind != CompleteUnroll) {
    ...
    dumpOptDetails(comp(),
                   "%schanged entry node of region %d [%x] to %d\n",
                   OPT_DETAILS,
                   cursor->getNumber(),
                   cursor,
                   newNumber);
...
void
TR_LoopUnroller::removeExternalEdge(TR_RegionStructure* parent,
...
int32_t
TR_LoopUnroller::numExitEdgesTo(TR_RegionStructure* from, int32_t to)
{
  if (!from)
    return 0;
...
void
TR_LoopUnroller::prepareForArrayShadowRenaming(TR_RegionStructure* loop)
{
...
void
TR_LoopUnroller::getLoopPreheaders(TR_RegionStructure* loop,
                                   TR_ScratchList<TR::Block>* preheaders)
{
  ...
  for (auto edge = loopEntry->getPredecessors().begin();
       edge != loopEntry->getPredecessors().end();
       ++edge) {
...
void
TR_LoopUnroller::collectInternalPointers()
{
              ...
              if (canMatch || canMatchAsIP) {
                if (trace())
                  traceMsg(comp(),
                           ...
                           secondChild);
                IntrnPtr* intrnPtr =
                  (IntrnPtr*)trMemory()->allocateStackMemory(sizeof(IntrnPtr));
                intrnPtr->symRefNum =
...
void
TR_LoopUnroller::collectArrayAccesses()
{
  ...
  if (trace())
    traceMsg(
      comp(), "Looking for array accesses in loop %d\n", _loop->getNumber());
...
void
TR_LoopUnroller::examineNode(TR::Node* node, intptrj_t visitCount)
{
...
TR_LoopUnroller::IntrnPtr*
TR_LoopUnroller::findIntrnPtr(int32_t symRefNum)
{
...
bool
TR_LoopUnroller::haveIdenticalOffsets(IntrnPtr* intrnPtr1, IntrnPtr* intrnPtr2)
{
...
void
TR_LoopUnroller::examineArrayAccesses()
{
  ...
  for (ListsOfArrayAccesses* loaa = it.getFirst(); loaa; loaa = it.getNext()) {
    if (trace())
      traceMsg(comp(),
               "Examining list of array accesses with sym ref %d\n",
               loaa->symRefNum);
    ...
    for (aa1 = it1.getFirst(); aa1 && (aa2 = it1.getNext()); aa1 = aa2) {
      if (trace())
        traceMsg(comp(),
                 "\tComparing array accesses %p and %p\n",
                 aa1->aaNode,
                 aa2->aaNode);
...
void
TR_LoopUnroller::refineArrayAliasing()
{
  ...
  if (_listOfListsOfArrayAccesses.isEmpty() ||
      !performTransformation(comp(),
                             "%sRefine array aliasing in loop %d\n",
                             OPT_DETAILS1,
                             _loop->getNumber()))
    return;
...
int32_t
TR_LoopUnroller::unroll(TR_RegionStructure* loop,
                        TR_StructureSubGraphNode* branchNode)
{
...
TR::Node*
TR_LoopUnroller::createIfNodeForSpillLoop(TR::Node* ifNode)
{
...
void
TR_LoopUnroller::generateSpillLoop(TR_RegionStructure* loop,
                                   TR_StructureSubGraphNode* branchNode)
{
  // Reset the mappers.
  _iteration = 0;
  ...
  if (trace()) {
    traceMsg(comp(),
             "trees after creating the spill loop %d for loop %d:\n",
             spillNode->getNumber(),
             loop->getNumber());
...
void
TR_LoopUnroller::addEdgeForSpillLoop(TR_RegionStructure* region,
                                     ...
                                     bool notLoopBranchNode)
{
  ...
  if (toNode->getStructure() == NULL) // exit edge
    toNode =
      findNodeInHierarchy(region->getParent()->asRegion(), toNode->getNumber());
...
void
TR_LoopUnroller::addExitEdgeAndFixEverything(
...
void
TR_LoopUnroller::addEdgeAndFixEverything(TR_RegionStructure* region,
                                         TR::CFGEdge* originalEdge,
                                         TR_StructureSubGraphNode* newFromNode,
                                         TR_StructureSubGraphNode* newToNode,
...
bool
TR_LoopUnroller::cfgEdgeAlreadyExists(TR::Block* from,
                                      TR::Block* to,
                                      EdgeContext edgeContext)
{
  for (auto edge = from->getSuccessors().begin();
       edge != from->getSuccessors().end();
       ++edge) {
    TR::Block* succ = toBlock((*edge)->getTo());
...
bool
TR_LoopUnroller::edgeAlreadyExists(TR_StructureSubGraphNode* from,
                                   TR_StructureSubGraphNode* to)
{
  for (auto edge = from->getSuccessors().begin();
       edge != from->getSuccessors().end();
...
void
TR_LoopUnroller::cloneBlocksInRegion(TR_RegionStructure* region,
                                     bool isSpillLoop)
{
  TR_ScratchList<TR::Block> blocksInRegion(trMemory());
...
inline TR_Structure*
TR_LoopUnroller::cloneStructure(TR_Structure* s)
{
  if (s->asRegion())
...
void
TR_LoopUnroller::swingBlocks(TR::Block* from, TR::Block* to)
{
...
void
TR_LoopUnroller::processSwingQueue()
{
...
void
TR_LoopUnroller::processSwingBlocks(TR::Block* from, TR::Block* to)
{
...
bool
TR_LoopUnroller::isSuccessor(TR::Block* A, TR::Block* B)
{
...
void
TR_LoopUnroller::fixExitEdges(TR_Structure* s,
                              TR_Structure* clone,
                              TR_StructureSubGraphNode* branchNode)
{
  if (s->asBlock())
  ...
  for (edge = ei.getFirst(); edge; edge = ei.getNext()) {
    if (region->isExitEdge(edge)) {
      ...
      if (clonedBlock) // ie. the exit node was cloned
      {
        ...
        addExitEdgeAndFixEverything(
          clonedRegion, edge, clonedFromNode, exitNode, clonedBlock, context);
      } else {
        TR_StructureSubGraphNode* exitNode =
          findNodeInHierarchy(region->getParent()->asRegion(), toNum);
        addExitEdgeAndFixEverything(
          clonedRegion, edge, clonedFromNode, exitNode, NULL, context);
      }
    }
  }
  processSwingQueue();
...
bool
TR_LoopUnroller::isTransactionStartLoop(TR_RegionStructure* loop,
                                        TR::Compilation* comp)
{
  if (comp->getOption(TR_DisableTLE) ||
      !comp->cg()
         ->getSupportsTM()) // if TLE is not supported or on, it is not possible
    return false;
  ...
  if (loopNode && (loopNode->getPredecessors().size() == 1)) {
    ...
    if (predBlock && predBlock->isLoopInvariantBlock()) {
      ...
      for (auto e = entryNode->getPredecessors().begin();
           e != entryNode->getPredecessors().end();
           ++e) {
...
TR_LoopUnroller::TR_LoopUnroller(TR::Compilation* c,
  ...
  , _listOfListsOfArrayAccesses(c->trMemory())
{
  ...
  bool indexOnLHS =
    nodeRefersToSymbol(branch->getFirstChild(), piv->getSymRef()->getSymbol());
...
// ::unrollCountedLoop
bool
TR_LoopUnroller::unroll(TR::Compilation* comp,
                        TR_RegionStructure* loop,
                        TR_PrimaryInductionVariable* piv,
                        UnrollKind unrollKind,
                        int32_t unrollCount,
                        ...
                        TR::Optimizer* optimizer)
{
  ...
  if (peelCount != 0) {
    ...
    dumpOptDetails(comp,
                   "Cannot unroll loop %d: peeling not supported yet\n",
                   loop->getNumber());
    return false;
  }
  ...
  if (!isWellFormedLoop(loop, comp, loopInvariantBlock)) {
    // if (comp->trace(generalLoopUnroller))
    dumpOptDetails(comp,
                   "Cannot unroll loop %d: not a well formed loop\n",
                   loop->getNumber());
    return false;
  }
  ...
  if (1) {
    ...
    if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
      ...
      if ((TR::ILOpCode::isEqualCmp(op) && !unroller._branchToExit) ||
        ...
        dumpOptDetails(comp,
                       "Cannot unroll loop %d: stange controlling test\n",
                       loop->getNumber());
        return false;
      }
      ...
      if (performTransformation(
            comp,
            "%sCanonicalize branch test %p for eq/ne loop %d\n",
            OPT_DETAILS,
            branch,
            loop->getNumber())) {
    ...
    if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
      dumpOptDetails(comp,
                     "Cannot unroll loop %d: unsupported branch opcode\n",
                     loop->getNumber());
      return false;
    }
  }
  ...
  if (unroller.isInternalPointerLimitExceeded()) {
    dumpOptDetails(
      comp,
      "Cannot unroll loop %d: number of internal pointers has been exceeded\n",
      loop->getNumber());
    return false;
  }
...
void
TR_GeneralLoopUnroller::collectNonColdInnerLoops(
  TR_RegionStructure* region,
...
void
TR_GeneralLoopUnroller::gatherStatistics(TR_Structure* str,
                                         int32_t& numNodes,
                                         int32_t& numBlocks,
                                         int32_t& numBranches,
...
void
TR_GeneralLoopUnroller::countNodesAndSubscripts(TR::Node* node,
                                                int32_t& numNodes,
                                                int32_t& numSubscripts,
                                                LoopWeightProbe& lwp)
{
  ...
  // recurse children
  for (int32_t child = 0; child < node->getNumChildren(); ++child)
    countNodesAndSubscripts(
      node->getChild(child), numNodes, numSubscripts, lwp);
}

void
TR_GeneralLoopUnroller::profileNonCountedLoops(
  List<TR_RegionStructure>& innerLoops)
{
  ListIterator<TR_RegionStructure> it(&innerLoops);
  for (TR_RegionStructure* loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    TR_StructureSubGraphNode* invarNode =
      loopNode->getPredecessors().front()->getFrom()->asStructureSubGraphNode();
    ...
    entryBlock->prepend(TR::TreeTop::create(
      ...
      TR::Node::create(
        ...
        1,
        TR::Node::create(
          TR::ladd, 2, TR::Node::createLoad(firstNode, counterSymRef), one))));
    ...
    postBlock->prepend(TR::TreeTop::create(
      comp(),
      TR::Node::createWithSymRef(TR::lstore,
                                 1,
                                 1,
...
static bool
exitsLoop(TR::Compilation* comp,
          TR_RegionStructure* loop,
          TR_StructureSubGraphNode* source)
{
  for (auto edge = source->getSuccessors().begin();
       edge != source->getSuccessors().end();
       ++edge) {
...
// Returns the weight
int32_t
TR_GeneralLoopUnroller::weighNaturalLoop(
  TR_RegionStructure* loop,
  TR_LoopUnroller::UnrollKind& unrollKind,
  int32_t& unrollCount,
  int32_t& peelCount,
  int32_t& cost)
{
  ...
  if (trace()) {
    traceMsg(comp(),
             ...
             maxNumUseDefd);
    traceMsg(comp(),
             "ratio = numUseOnly/numUsed = %g(minRatio:%g) \n",
             ratio,
             minRatio);
    traceMsg(comp(),
             "numCalls = %u, numBIFs = %u, numPureFns = %u\n",
             lwp._numCalls,
             lwp._numBIFs,
             lwp._numPureFunctions);
  }
  ...
  if (_haveProfilingInfo && outerLoop) {
    ...
    if (maxCount > 0) {
      ...
      dumpOptDetails(
        ...
        outerLoop->getNumber(),
        outerLoop->getEntryBlock()->getNumber());
      ...
      dumpOptDetails(comp(),
                     "\touterloop relative frequency = %.2g\n",
  ...
  if (piv) {
    ...
    if (entryValueConst && exitBoundConst) {
      if ((increasingLoop && (entryValue > exitBound)) ||
        ...
        dumpOptDetails(
          comp(),
          "\twraparound, %s, entry value: 0x%x, exit bound: 0x%x\n",
          increasingLoop ? "loop increasing" : "loop decreasing",
          entryValue,
          exitBound);
        wrapAround = true;
      }
    ...
    if (iterCount > 0 && !wrapAround) {
      ...
      if (iterCount <= maxIterCountForCompleteUnroll &&
        ...
        bool multipleOfPreferred =
          preferredUnrollFactor > 0 && (iterCount % preferredUnrollFactor) == 0;
  ...
  } else // of if piv
  {
    ...
    if (_haveProfilingInfo) {
      ...
      if (canUnrollUnCountedLoop(
            loop, numBlocks, numNodes, entryBlockFrequency)) {
...
//
bool
TR_GeneralLoopUnroller::branchContainsInductionVariable(
  TR_RegionStructure* loop,
  TR::Node* branchNode,
  bool checkOnlyPiv)
{
