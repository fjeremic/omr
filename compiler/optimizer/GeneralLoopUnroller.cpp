...
static TR_ScratchList<TR::CFGEdge> *
findCorrespondingCFGEdges(TR_Structure *from, TR_Structure *to,
                          TR::Compilation *comp) {
  if (from->asBlock()) {
...
void TR_LoopUnroller::modifyBranchTree(TR_RegionStructure *loop,
                                       ...
                                       TR_StructureSubGraphNode *branchNode) {
  TR_ASSERT(isCountedLoop() && _unrollKind != CompleteUnroll &&
                _spillLoopRequired,
    ...
    if (1) {
      ...
      if (1) {
        ...
        if (1) {
          ...
          if (getIndexType().isAddress() || getTestChildType().isAddress()) {
            TR_ASSERT(getIndexType().isAddress() &&
                          getTestChildType().isAddress(),
                      "Test child type and IV type should both be address");
...
TR_StructureSubGraphNode *
TR_LoopUnroller::findNodeInHierarchy(TR_RegionStructure *region, int32_t num) {
  if (!region)
    return NULL;
...
TR_StructureSubGraphNode *
TR_LoopUnroller::getEntryBlockNode(TR_StructureSubGraphNode *node) {
  if (node->getStructure()->asBlock())
    return node;
  else
...
TR_Structure *
TR_LoopUnroller::cloneBlockStructure(TR_BlockStructure *blockStructure) {
...
TR_Structure *
TR_LoopUnroller::cloneRegionStructure(TR_RegionStructure *region) {
...
TR_LoopUnroller::TR_LoopUnroller(TR::Compilation *c, TR::Optimizer *optimizer,
    ...
    : _comp(c), _trMemory(c->trMemory()), _optimizer(optimizer), _loop(loop),
      _vectorSize(vectorSize), _branchNode(branchNode),
      _unrollCount(unrollCount), _peelCount(peelCount), _unrollKind(unrollKind),
      _iteration(0), _firstEntryNode(0), _piv(0), _spillLoopRequired(false),
      _branchToExit(false), _spillNode((TR_StructureSubGraphNode *)0xdeadf00d),
      _overflowTestBlock((TR::Block *)0xdeadf00d),
      _loopIterTestBlock((TR::Block *)0xdeadf00d),
      _loopInvariantBlock(invariantBlock), _loopInvariantBlockAtEnd(false),
      _swingQueue(c->trMemory()), _wasEQorNELoop(false),
      _origLoopCondition(TR::BadILOp), _newSymRefs(c->trMemory()),
      _startPosOfUnrolledBodies(NULL), _endPosOfUnrolledBodies(NULL),
...
TR_LoopUnroller::TR_LoopUnroller(TR::Compilation *c, TR::Optimizer *optimizer,
                                 ...
                                 int32_t vectorSize)
    : _comp(c), _trMemory(c->trMemory()), _optimizer(optimizer), _loop(loop),
      _unrollKind(unrollKind), _vectorSize(vectorSize),
      _unrollCount(unrollCount), _peelCount(peelCount), _piv(piv),
      _spillNode(0), _overflowTestBlock(0), _loopIterTestBlock(0),
      _loopInvariantBlock(invariantBlock), _loopInvariantBlockAtEnd(false),
      _iteration(0), _firstEntryNode(0), // goto init
      _swingQueue(c->trMemory()), _wasEQorNELoop(false),
      _origLoopCondition(TR::BadILOp), _newSymRefs(c->trMemory()),
      _startPosOfUnrolledBodies(NULL), _endPosOfUnrolledBodies(NULL),
      _listOfInternalPointers(c->trMemory()),
...
int32_t TR_GeneralLoopUnroller::weighNaturalLoop(
  ...
  if (_haveProfilingInfo && outerLoop) {
    ...
    if (maxCount > 0) {
      ...
      {
        dumpOptDetails(comp(), "\trejecting loop because its not warm enough "
