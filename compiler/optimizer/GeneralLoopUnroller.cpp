    ...
    TR_StructureSubGraphNode* branchNode)
{
    ...
    if (originalLimitNode->getOpCode().isLoadConst() && getTestIsUnsigned()) {
        ...
        if (getTestChildType().isInt32()) {
            ...
            const_overflow = (isIncreasingLoop() && originalLimitNode->getUnsignedInt() < utemp) || (!isIncreasingLoop() && originalLimitNode->getUnsignedInt() > utemp);
        } else if (getTestChildType().isInt64()) {
            uint64_t utemp = originalLimitNode->getUnsignedLongInt() - adjustment;
            const_overflow = (isIncreasingLoop() && originalLimitNode->getUnsignedLongInt() < utemp) || (!isIncreasingLoop() && originalLimitNode->getUnsignedLongInt() > utemp);
        }
    }
    ...
    if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
        ...
        if (getIndexType().isAddress()) {
            ...
            tempSym->getSymbol()->castToInternalPointerAutoSymbol()->setPinningArrayPointer(_piv->getSymRef()->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
        } else
        ...
        TR::ILOpCodes addOpCode = getIndexType().isAddress() ? (is64BitComparison ? TR::aladd : TR::aiadd) : (is64BitComparison ? TR::ladd : TR::iadd);
        tempValue = TR::Node::create(addOpCode, 2, originalLimitClone, constNode);

        if (getIndexType().isAddress()) {
            tempValue->setPinningArrayPointer(_piv->getSymRef()->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
        ...
        pBlock->append(TR::TreeTop::create(comp(), TR::Node::createif(opCode, originalLimitClone, c2, spillBlock->getEntry())));
    ...
    if (1) /// moveEntryNode
    {
        ...
        if (loop->getPrimaryInductionVariable() && loop->getPrimaryInductionVariable()->usesUnchangedValueInLoopTest()) {
        ...
        TR::TreeTop* iterGuard = TR::TreeTop::create(comp(),
...
{
    return _spillLoopRequired && isCountedLoop() && getIndexType().isAddress() && comp()->getSymRefTab()->getNumInternalPointers() >= comp()->maxInternalPointers();
}
...
void TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure* loop, TR_StructureSubGraphNode* branchNode)
{
    ...
    } else if (_unrollKind == CompleteUnroll) /*GGLU*/
    {
        ...
        if (loop->contains(destStruct, loop->getParent())) {
            ...
            TR_StructureSubGraphNode* concreteExitNode = findNodeInHierarchy(loop->getParent()->asRegion(), exitNode->getNumber());
    ...
    if (_spillLoopRequired) {
        ...
        TR_StructureSubGraphNode* exitNode = findNodeInHierarchy(parent, toStructureSubGraphNode(exitEdge->getTo())->getNumber());
        ...
        if (1) {
            ...
            if (1) {
                ...
                TR_StructureSubGraphNode* gotoSubNode = new (_cfg->structureRegion()) TR_StructureSubGraphNode(new (_cfg->structureRegion()) TR_BlockStructure(comp(), gotoBlock->getNumber(), gotoBlock));
    ...
    if (0 && _spillLoopRequired) {
        ...
        if (!skipIt) {
            ...
            if (1) {
                ...
                if (lastTree->getNode()->getOpCode().isBranch() && lastTree->getNode()->getBranchDestination() == oldSpillEntryBlock->getEntry()) {
            ...
            if (_overflowTestBlock) {
                _overflowTestBlock->getLastRealTreeTop()->getNode()->setBranchDestination(newEntryBlock->getEntry());
                TR_StructureSubGraphNode* overflowTestNode = parent->findSubNodeInRegion(_overflowTestBlock->getNumber());
            ...
            if (1 && _loopIterTestBlock) {
                TR::Block* newEntryBlock = spillLoop->getEntryBlock();
                _loopIterTestBlock->getLastRealTreeTop()->getNode()->setBranchDestination(newEntryBlock->getEntry());
    ...
    for (TR::Block* preheader = it.getFirst(); preheader; preheader = it.getNext()) {
        ...
        while (currentTree != exitTree) {
            ...
            if (node->getOpCodeValue() == TR::astore && node->getSymbol()->isAuto() && node->getSymbol()->castToAutoSymbol()->isInternalPointer() && node->getFirstChild()->getNumChildren() > 1) {
                ...
                if ((secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub()) && secondChild->getSecondChild()->getOpCode().isLoadConst()) {
                    TR::Node* firstChild = secondChild->getFirstChild();

                    if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift()) && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
                        ...
                        if (firstChild->getOpCode().isLoad()) {
                            ...
                            if (isBiv(_loop, firstChild->getSymbolReference(), biv))
                                canMatch = true;
                            else if (isBiv(_loop, node->getSymbolReference(), biv) && firstChild->getOpCode().isLoadVarDirect() && firstChild->getSymbol()->isAuto() && node->getFirstChild()->getFirstChild()->getOpCode().isLoadVarDirect() && node->getFirstChild()->getFirstChild()->getSymbol()->isAuto() && node->getFirstChild()->getFirstChild()->getSymbol()->castToAutoSymbol()->isPinningArrayPointer())
                                canMatchAsIP = true;
...
void TR_LoopUnroller::examineNode(TR::Node* node, intptrj_t visitCount)
{
    ...
    if (symbol && symbol->isArrayShadowSymbol()) {
        ...
        if (node->getFirstChild()->getOpCodeValue() == TR::aload && node->getFirstChild()->getSymbol()->isAuto() && node->getFirstChild()->getSymbol()->castToAutoSymbol()->isInternalPointer()) {
...
bool TR_LoopUnroller::haveIdenticalOffsets(IntrnPtr* intrnPtr1, IntrnPtr* intrnPtr2)
{
	...
	*/
    ...
    int64_t entryValue1 = entryValueNode1->getType().isInt64() ? entryValueNode1->getLongInt() : (int64_t)entryValueNode1->getInt();
    int64_t entryValue2 = entryValueNode2->getType().isInt64() ? entryValueNode2->getLongInt() : (int64_t)entryValueNode2->getInt();
    if (intrnPtr1->biv->getDeltaOnBackEdge() == intrnPtr2->biv->getDeltaOnBackEdge() && entryValue1 + intrnPtr1->offsetValue == entryValue2 + intrnPtr2->offsetValue)
        return true;
...
void TR_LoopUnroller::refineArrayAliasing()
{
    ...
    if (_listOfListsOfArrayAccesses.isEmpty() || !performTransformation(comp(), "%sRefine array aliasing in loop %d\n", OPT_DETAILS1, _loop->getNumber()))
        return;
    ...
    bool notLoopBranchNode)
{
    ...
    TR_ScratchList<TR::CFGEdge>* cfgEdges = findCorrespondingCFGEdges(fromNode->getStructure(), toNode->getStructure(), comp());
    ...
    for (TR::CFGEdge* edge = it.getFirst(); edge; edge = it.getNext()) {
        ...
        if ((edgeContext == BackEdgeFromLastGenerationCompleteUnroll) && (newFrom->getLastRealTreeTop()->getNode()->getOpCode().isBranch()))
            lastGenerationEndsInBranch = true;

        //BRANCH
        if (lastNode->getOpCode().isBranch() && (lastNode->getBranchDestination() == to->getEntry()) && !lastGenerationEndsInBranch) {
    ...
    for (auto edge = from->getSuccessors().begin(); edge != from->getSuccessors().end(); ++edge) {
        ...
        if (edgeContext != BackEdgeFromLastGenerationCompleteUnroll) {
            ...
            if (firstRealTree == lastRealTree && firstRealTree->getNode()->getOpCodeValue() == TR::Goto) {
                ...
                //
                if (dest->getNumber() == to->getNumber() && lastRealTree->getNode()->getLocalIndex() == CREATED_BY_GLU && succ->getEntry()->getNode()->getLocalIndex() == CREATED_BY_GLU)
                    return true;
            }
        }
    }
...
TR_Structure* TR_LoopUnroller::cloneBlockStructure(TR_BlockStructure* blockStructure)
{
    ...
    TR::Block* clonedBlock = GET_CLONE_BLOCK(block);
    TR_BlockStructure* clonedBlockStructure = new (_cfg->structureRegion()) TR_BlockStructure(comp(), clonedBlock->getNumber(), clonedBlock);
    ...
    TR_StructureSubGraphNode* branchNode)
{
    ...
    TR_StructureSubGraphNode* subNode;
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        TR_StructureSubGraphNode* clonedSubNode = findNodeInHierarchy(clonedRegion, _blockMapper[CURRENT_MAPPER][subNode->getNumber()]->getNumber());
        fixExitEdges(subNode->getStructure(), clonedSubNode->getStructure());
    }
    ...
    for (edge = ei.getFirst(); edge; edge = ei.getNext()) {
        if (region->isExitEdge(edge)) {
            ...
            TR_StructureSubGraphNode* clonedFromNode = findNodeInHierarchy(clonedRegion, _blockMapper[CURRENT_MAPPER][fromNode->getNumber()]->getNumber());
    ...
    , _listOfListsOfArrayAccesses(c->trMemory())
{
    ...
    _branchToExit = !loop->contains(branch->getBranchDestination()->getNode()->getBlock()->getStructureOf(),
        loop->getParent());
}
    ...
    int32_t unrollCount, int32_t peelCount, TR::Optimizer* optimizer)
{
    ...
    if (1) {
        ...
        if (unroller._spillLoopRequired) {
            ...
            if (!backEdgeFound) {
                ...
                if (branchBlock == loop->getEntryBlock()) {
                    if ((branchBlock->getLastRealTreeTop() == branchBlock->getFirstRealTreeTop()) || ((branchBlock->getFirstRealTreeTop()->getNode()->getOpCodeValue() == TR::asynccheck) && (branchBlock->getLastRealTreeTop() == branchBlock->getFirstRealTreeTop()->getNextTreeTop())))
                        loopCanBeUnrolled = true;
                }

                if (!loopCanBeUnrolled) {
                    if (blocksInLoop.isDoubleton() && nextBlock->getSuccessors().size() == 1) {
                        if ((nextBlock->getEntry()->getNextTreeTop() == nextBlock->getExit()) || (nextBlock->getLastRealTreeTop() == nextBlock->getFirstRealTreeTop()) || ((nextBlock->getFirstRealTreeTop()->getNode()->getOpCodeValue() == TR::asynccheck) && (nextBlock->getLastRealTreeTop() == nextBlock->getFirstRealTreeTop()->getNextTreeTop())))
                            loopCanBeUnrolled = true;
                    }
                }
        ...
        if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
            ...
            //
            if ((TR::ILOpCode::isEqualCmp(op) && !unroller._branchToExit) || (TR::ILOpCode::isNotEqualCmp(op) && unroller._branchToExit)) {
            ...
            TR::ILOpCodes opCode = branch->getFirstChild()->getType().isInt64() ? (branch->getOpCode().isUnsignedCompare() ? TR::iflucmpge : TR::iflcmpge) : (branch->getOpCode().isUnsignedCompare() ? TR::ifiucmpge : TR::ificmpge);
    ...
    LoopWeightProbe& lwp)
{
    ...
    if (node->getOpCodeValue() == TR::aiadd || node->getOpCodeValue() == TR::aiuadd || node->getOpCodeValue() == TR::aladd || node->getOpCodeValue() == TR::aluadd) {
...
void TR_GeneralLoopUnroller::profileNonCountedLoops(List<TR_RegionStructure>& innerLoops)
{
    ...
    for (TR_RegionStructure* loop = it.getFirst(); loop; loop = it.getNext()) {
        ...
        TR::SymbolReference* counterSymRef = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Int64);
        ...
        entryBlock->prepend(TR::TreeTop::create(comp(), TR::Node::create(TR::lstore, 1, TR::Node::create(TR::ladd, 2, TR::Node::createLoad(firstNode, counterSymRef), one))));
        ...
        postBlock->prepend(TR::TreeTop::create(comp(), TR::Node::createWithSymRef(TR::lstore, 1, 1, TR::Node::createLoad(firstNode, counterSymRef), counterSymRef)));
    ...
    int32_t& cost)
{
    ...
    if (_haveProfilingInfo && outerLoop) {
        ...
        if (maxCount > 0) {
            ...
            float outerLoopRelativeFrequency = outerLoopFrequency == 6 ? 1.3 + (10 * entryBlockFrequency / (float)maxCount) : entryBlockFrequency / (float)outerLoopFrequency;
    ...
    if (piv) {
        ...
        if (iterCount > 0 && !wrapAround) {
            ...
            if (iterCount <= maxIterCountForCompleteUnroll && ((iterCount * numNodes <= maxSizeForSingleBlockCompleteUnroll && numBlocks == 1) || (iterCount * numNodes <= maxSizeForCompleteUnroll))) {
                ...
                if (!multipleOfPreferred && !multipleOf2 && !multipleOf3) {
                    ...
                    if (comp()->getOption(TR_Randomize)) {
                       ...
                       traceMsg(comp(), "\nTR_Randomize Enabled||TR_GeneralLoopUnroller::weighNaturalLoop(), unrollCount:%d Original unrollCount:%d", unrollCount, originalUnrollCount);*/
                        ...
                        unrollCount = (multipleOfPreferred ? preferredUnrollFactor - 1 : multipleOf4 ? 3 : multipleOf3 ? 2 : multipleOf2 ? 1 : 0);
                    }
                    if (unrollCount > 0) {
...
bool TR_GeneralLoopUnroller::canUnrollUnCountedLoop(TR_RegionStructure* loop,
    ...
    int32_t entryBlockFrequency)
{
    ...
    if ((numBlocks <= 1 && numNodes <= 13 && ((entryBlockFrequency * 200) > (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT))) || (numBlocks <= 2 && numNodes <= 25 && ((entryBlockFrequency * 100) > (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT)))) {
        ...
        for (b = bilIt.getCurrent(); b; b = bilIt.getNext()) {
            ...
            for (TR::TreeTop* tt = entry; tt != exit; tt = tt->getNextRealTreeTop()) {
                ...
                if (node->getOpCodeValue() == TR::NULLCHK && (firstChild = node->getFirstChild()) && firstChild->getOpCode().isLoad()) {
                    ...
                    for (TR::TreeTop* tt1 = tt; tt1 != exit; tt1 = tt1->getNextRealTreeTop()) {
                        ...
                        if (n->getOpCode().isBooleanCompare() && (n->getFirstChild() == firstChild) && (n->getSecondChild()->getOpCodeValue() == TR::aconst) && (n->getSecondChild()->getAddress() == 0)) {
                            if (trace()) {
