      ...
      } else if (succ == loop->getEntry()) // Type C
      {
        ...
        if (finalUnroll) {
          if (_unrollKind != CompleteUnroll)
            ...
            addEdgeAndFixEverything(loop, *edge, clonedSubNode, loop->getEntry(), false, false, true, BackEdgeToEntry);
          else {
        ...
        if (_iteration != 1) // for all iterations except the first iteration
        {
          if (shouldConnectToNextIteration(toStructureSubGraphNode(subNode), loop)) {
            addEdgeAndFixEverything(
              loop, *edge, GET_PREV_CLONE_NODE(subNode), newEntryNode, false, false, false, BackEdgeFromPrevGeneration);
          } else {
            if (trace())
              traceMsg(comp(),
                "Redirecting cold edge from block_%d to unrolled loop header rather than going to the next iteration\n",
                subNode->getNumber());
            addEdgeAndFixEverything(
              loop, *edge, GET_PREV_CLONE_NODE(subNode), loop->getEntry(), false, false, true, BackEdgeToEntry);
          }
        }
      } else // Type D
      {
  ...
  TR_StructureSubGraphNode* fromNode, bool notLoopBranchNode)
{
  ...
  for (auto edge = branchNode->getSuccessors().begin(); (edge != branchNode->getSuccessors().end()) && exitEdge == NULL;
       ++edge) {
  ...
  addEdgeForSpillLoop(loop, exitEdge, fromNode,
    findNodeInHierarchy(loop->getParent()->asRegion(), toStructureSubGraphNode(exitEdge->getTo())->getNumber()), false,
    BackEdgeFromLastGenerationCompleteUnroll, notLoopBranchNode);
}
  ...
  TR_RegionStructure* loop, TR_StructureSubGraphNode* loopNode, TR_StructureSubGraphNode* branchNode)
{
  ...
  if (pBlock->getEntry() == 0) {
    TR::Block* newBlock
      = TR::Block::createEmptyBlock(branch, comp(), MAX_COLD_BLOCK_COUNT + 1, pBlock); // branchBlock->getFrequency());
    ...
    TR_BlockStructure* newBlockStr
      = new (_cfg->structureRegion()) TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
  ...
  if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
    ...
    if (getIndexType().isAddress() || getTestChildType().isAddress()) {
      ...
      TR::ILOpCodes o2xOp
        = TR::ILOpCode::getProperConversion(newLimitNode->getDataType(), getTestChildType().getDataType(), true);
...
void TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure* loop, TR_StructureSubGraphNode* branchNode)
{
  ...
  if (_spillLoopRequired) {
    ...
    if (1) {
      ...
      TR::Block* newIfBlock
        = TR::Block::createEmptyBlock(branchTreeTop->getNode(), comp(), origExitBlock->getFrequency(), origExitBlock);
      ...
      if (1) {
        ...
        if (1) {
          ...
          newIfTree = TR::TreeTop::create(
            comp(), TR::Node::createif(ifNode->getOpCodeValue(), c1, boundNode, ifNode->getBranchDestination()));
        }

        newIfBlock->append(newIfTree);
        _cfg->addNode(newIfBlock);
        newIfBlockStructure
          = new (_cfg->structureRegion()) TR_BlockStructure(comp(), newIfBlock->getNumber(), newIfBlock);
        ...
        if (spillLoop->contains(branchDestination->getStructureOf(), parent)) {
          TR::Node::recreate(newIfTree->getNode(), newIfTree->getNode()->getOpCode().getOpCodeForReverseBranch());
    ...
    if (1) {
      ...
      if (!e) // there do not remain an edge in L going to X
      {
        if (exitNode->getStructure()->getParent() == parent) {
          bool found
            = (std::find(loopNode->getSuccessors().begin(), loopNode->getSuccessors().end(), correspondingExitEdge)
          ...
          found
            = (std::find(exitNode->getPredecessors().begin(), exitNode->getPredecessors().end(), correspondingExitEdge)
...
void TR_LoopUnroller::removeExternalEdge(TR_RegionStructure* parent, TR_StructureSubGraphNode* from, int32_t to)
{
  ...
  if (numExitEdgesTo(from->getStructure()->asRegion(), to) == 0) {
    ...
    TR_ASSERT(found, "GLU: bad structure while removing edge");
    found = (std::find(relevantEdge->getTo()->getPredecessors().begin(), relevantEdge->getTo()->getPredecessors().end(),
               relevantEdge)
  ...
  for (TR::Block* preheader = it.getFirst(); preheader; preheader = it.getNext()) {
    ...
    while (currentTree != exitTree) {
      ...
      if (node->getOpCodeValue() == TR::astore && node->getSymbol()->isAuto()
        && node->getSymbol()->castToAutoSymbol()->isInternalPointer() && node->getFirstChild()->getNumChildren() > 1) {
          ...
          if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift())
            ...
            if (firstChild->getOpCode().isLoad()) {
              ...
              if (isBiv(_loop, firstChild->getSymbolReference(), biv))
                ...
                && node->getFirstChild()->getFirstChild()->getSymbol()->castToAutoSymbol()->isPinningArrayPointer())
                canMatchAsIP = true;
...
void TR_LoopUnroller::examineArrayAccesses()
{
  ...
  for (ListsOfArrayAccesses* loaa = it.getFirst(); loaa; loaa = it.getNext()) {
    ...
    { // and it doesn't have an internal pointer
      if (trace())
        traceMsg(
          comp(), "List of array accesses with sym ref %d does not qualify for aliasing refinement\n", loaa->symRefNum);
...
void TR_LoopUnroller::generateSpillLoop(TR_RegionStructure *loop,
   ...
   TR_BlockStructure *newBranchStructure = new (_cfg->structureRegion()) TR_BlockStructure(comp(),
newBranchBlock->getNumber(), newBranchBlock); TR_StructureSubGraphNode *newBranchNode = new (_cfg->structureRegion())
TR_StructureSubGraphNode(newBranchStructure); spillLoop->addSubNode(newBranchNode);
  ...
  UnrollKind unrollKind, int32_t unrollCount, int32_t peelCount, TR::Optimizer* optimizer)
{
  ...
  if (1) {
    ...
    if (unroller._spillLoopRequired) {
      ...
      if (!backEdgeFound) {
        ...
        if (loopCanBeUnrolled) {
          ...
          // Further the only code other from the entry block in the loop must be the branch (and an optional async
          // check)
          //
        } else {
          dumpOptDetails(
            comp, "Cannot unroll loop %d: exit condition is not in a block containing a backedge\n", loop->getNumber());
          return false;
        }
      }
    }
    ...
    if (1) {
      ...
      if (!foundExitEdge) {
        dumpOptDetails(
          comp, "Cannot unroll loop %d: eq loop with branch not an exit out of the loop\n", loop->getNumber());
        return false;
      }
    }
    ...
    if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
      ...
      //
      TR_ASSERT(
        !branch->getFirstChild()->getType().isAddress(), "Excpeting an integral compare with integral first child");
...
int32_t TR_GeneralLoopUnroller::perform()
{
  ...
  while (1) {
    ...
    if (top->_loop->getPrimaryInductionVariable())
      TR_LoopUnroller::unroll(comp(), top->_loop, top->_loop->getPrimaryInductionVariable(), top->_unrollKind,
        top->_unrollCount, top->_peelCount, optimizer());
    else
  ...
  TR::Node* node, int32_t& numNodes, int32_t& numSubscripts, LoopWeightProbe& lwp)
{
  ...
  if (node->getOpCodeValue() == TR::aiadd || node->getOpCodeValue() == TR::aiuadd || node->getOpCodeValue() == TR::aladd
...
void TR_GeneralLoopUnroller::profileNonCountedLoops(List<TR_RegionStructure>& innerLoops)
{
  ...
  for (TR_RegionStructure* loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    TR_StructureSubGraphNode* postNode = TR_LoopUnroller::findNodeInHierarchy(loop->getParent()->asRegion(), postNum);
    TR::Block* postBlock = postNode->getStructure()->getEntryBlock();
    postBlock->prepend(TR::TreeTop::create(comp(),
      TR::Node::createWithSymRef(TR::lstore, 1, 1, TR::Node::createLoad(firstNode, counterSymRef), counterSymRef)));
  ...
  int32_t& unrollCount, int32_t& peelCount, int32_t& cost)
{
      ...
      for (auto e = entryNode->getPredecessors().begin(); e != entryNode->getPredecessors().end(); ++e) {
        ...
        if (source->getStructure()->asBlock()) {
          ...
          if (!branchNode->getOpCode().isGoto() && !branchContainsInductionVariable(loop, branchNode)) {
            dumpOptDetails(
              comp(), "backedge branch [%p] is controlled using a biv, so unrolling as non-counted\n", branchNode);
            loop->setPrimaryInductionVariable(NULL);
            break;
          }
        }
  ...
  if (_haveProfilingInfo && outerLoop) {
    ...
    if (maxCount > 0) {
      ...
      float outerLoopRelativeFrequency = outerLoopFrequency == 6 ? 1.3 + (10 * entryBlockFrequency / (float)maxCount)
        ...
        if (!multipleOfPreferred && !multipleOf2 && !multipleOf3) {
          ...
          if (comp()->getOption(TR_Randomize)) {
            ...
            /*unrollCount = (multipleOf4 ? randomInt(0,3) :
                   ...
                   traceMsg(comp(), "\nTR_Randomize Enabled||TR_GeneralLoopUnroller::weighNaturalLoop(), unrollCount:%d
               Original unrollCount:%d", unrollCount, originalUnrollCount);*/
          } else {
  ...
  TR_RegionStructure* loop, int32_t numBlocks, int32_t numNodes, int32_t entryBlockFrequency)
{
  ...
  if ((numBlocks <= 1 && numNodes <= 13 && ((entryBlockFrequency * 200) > (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT)))
    || (numBlocks <= 2 && numNodes <= 25 && ((entryBlockFrequency * 100) > (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT)))) {
        ...
        if (node->getOpCodeValue() == TR::NULLCHK && (firstChild = node->getFirstChild())
          ...
          for (TR::TreeTop* tt1 = tt; tt1 != exit; tt1 = tt1->getNextRealTreeTop()) {
            ...
            if (n->getOpCode().isBooleanCompare() && (n->getFirstChild() == firstChild)
              && (n->getSecondChild()->getOpCodeValue() == TR::aconst) && (n->getSecondChild()->getAddress() == 0)) {
              if (trace()) {
