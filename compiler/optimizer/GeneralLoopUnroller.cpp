...
#define SET_CLONE_BLOCK(x, y) \
  _blockMapper[_iteration % 2][(x)->getNumber()] = (y)
...
#define GET_PREV_CLONE_BLOCK(x) \
  _blockMapper[(1 + _iteration) % 2][(x)->getNumber()]
#define GET_PREV_CLONE_NODE(x) \
  _nodeMapper[(1 + _iteration) % 2][(x)->getNumber()]
...
static TR::ILOpCodes getConstOpCode(TR::DataType type) {
  switch (type.getDataType()) {
    ...
    case TR::Int64:
      return TR::lconst;
    case TR::Address:
      return TR::Compiler->target.is64Bit() ? TR::lconst : TR::iconst;
    default:
...
static TR_ScratchList<TR::CFGEdge> *join(TR_ScratchList<TR::CFGEdge> *to,
                                         TR_ScratchList<TR::CFGEdge> *from) {
...
static TR_ScratchList<TR::CFGEdge> *findCorrespondingCFGEdges(
  ...
  if (from->asBlock()) {
    ...
    for (auto edge = block->getSuccessors().begin();
      ...
      if (to->contains(succ->getStructureOf()))
        return new (comp->trHeapMemory())
            TR_ScratchList<TR::CFGEdge>(*edge, comp->trMemory());
    }
    return NULL;
  } else {
    ...
    for (node = it.getFirst(); node; node = it.getNext())
      edges = join(edges,
                   findCorrespondingCFGEdges(node->getStructure(), to, comp));
    return edges;
  }
}

static bool isBranchAtEndOfLoop(TR_RegionStructure *loop,
                                TR::Block *branchBlock) {
  for (auto e = branchBlock->getSuccessors().begin();
       e != branchBlock->getSuccessors().end(); ++e) {
...
bool TR_LoopUnroller::nodeRefersToSymbol(TR::Node *node, TR::Symbol *sym) {
  ...
  for (int32_t child = 0; child < node->getNumChildren(); child++)
    if (nodeRefersToSymbol(node->getChild(child), sym)) return true;
  return false;
}

TR::Symbol *TR_LoopUnroller::findSymbolInTree(TR::Node *node) {
  if (!node) return NULL;
  if (node->getOpCode().hasSymbolReference()) return node->getSymbol();
  for (int32_t child = node->getNumChildren() - 1; child >= 0; child--) {
    TR::Symbol *sym;
    if ((sym = findSymbolInTree(node->getChild(child)))) return sym;
  }
  return NULL;
}

static bool checkNodeFrequency(TR_StructureSubGraphNode *subNode,
                               TR_RegionStructure *loop) {
  TR::Block *block = subNode->getStructure()->asBlock() != NULL
                         ? subNode->getStructure()->asBlock()->getBlock()
                         : NULL;
  return !block || loop->getEntryBlock()->getFrequency() < 5000 ||
         block->getFrequency() > 2000 || block->getFrequency() == -1;
}

static bool hasConnectableEdges(TR::Compilation *comp,
                                TR_RegionStructure *loop) {
  if (!TR::isJ9() || comp->getOption(TR_DisableGLUColdRedirection)) return true;
  ...
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    for (auto edge = subNode->getSuccessors().begin();
         edge != subNode->getSuccessors().end(); ++edge) {
...
bool TR_LoopUnroller::shouldConnectToNextIteration(
    TR_StructureSubGraphNode *subNode, TR_RegionStructure *loop) {
  if (!TR::isJ9() || comp()->getMethodHotness() <= warm ||
      comp()->getOption(TR_DisableGLUColdRedirection))
    return true;
  if (!hasConnectableEdges(comp(), loop)) return true;
  return _unrollKind == CompleteUnroll || isCountedLoop() ||
         checkNodeFrequency(subNode, loop);
}

void TR_LoopUnroller::unrollLoopOnce(TR_RegionStructure *loop,
  ...
  memset(_nodeMapper[CURRENT_MAPPER], 0,
         _numNodes * sizeof(TR_StructureSubGraphNode *));
  ...
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    ...
    TR_StructureSubGraphNode *clonedSubNode =
        new (_cfg->structureRegion()) TR_StructureSubGraphNode(clonedSubStruct);
    ...
    if (subNode == loop->getEntry()) newEntryNode = clonedSubNode;
  }

  // Fix all the exit edges in the subnodes
  nodeIt.reset();
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    if (subNode->getNumber() >= _numNodes) continue;
  ...
  if (_iteration == 1) _firstEntryNode = newEntryNode;
  ...
  if (isCountedLoop()) {
    // If the exit branch is in the middle of the loop and a complete unroll is
    // going to happen, it is not valid to remove the branch for the last unroll
    // (the top half of the loop maybe executed more number of times compared to
    // the bottom half)
    //
    if (finalUnroll && _unrollKind == CompleteUnroll) {
      branchAtEnd = isBranchAtEndOfLoop(
          loop, branchNode->getStructure()->asBlock()->getBlock());
    }

    if (branchAtEnd)
      clonedBranchNode->getStructure()->asBlock()->getBlock()->removeBranch(
          comp());
  }

  // Look at all the structures edges in the original loop, and create
  // appropriate edges for each edge type (described below).
  nodeIt.reset();
  for (subNode = nodeIt.getFirst(); subNode; subNode = nodeIt.getNext()) {
    if (subNode->getNumber() >= _numNodes) continue;
    ...
    for (auto edge = subNode->getSuccessors().begin();
      ...
      } else if (subNode != branchNode && !clonedSucc)  // Type B
      {
        ...
        addExitEdgeAndFixEverything(loop, *edge, clonedSubNode, NULL, NULL,
                                    context);
      } else if (succ == loop->getEntry())  // Type C
      {
        ...
        if (finalUnroll) {
          if (_unrollKind != CompleteUnroll)
            ...
            addEdgeAndFixEverything(loop, *edge, clonedSubNode,
                                    loop->getEntry(), false, false, true,
                                    BackEdgeToEntry);
          else {
            ...
            redirectBackEdgeToExitDestination(loop, branchNode, clonedSubNode,
                                              (subNode != branchNode));
          }
        }
        ...
        {
          if (shouldConnectToNextIteration(toStructureSubGraphNode(subNode),
                                           loop)) {
            addEdgeAndFixEverything(loop, *edge, GET_PREV_CLONE_NODE(subNode),
                                    newEntryNode, false, false, false,
                                    BackEdgeFromPrevGeneration);
          } else {
            if (trace())
              traceMsg(comp(),
                       "Redirecting cold edge from block_%d to unrolled loop "
                       "header rather than going to the next iteration\n",
                       subNode->getNumber());
            addEdgeAndFixEverything(loop, *edge, GET_PREV_CLONE_NODE(subNode),
                                    loop->getEntry(), false, false, true,
                                    BackEdgeToEntry);
...
void TR_LoopUnroller::redirectBackEdgeToExitDestination(
    TR_RegionStructure *loop, TR_StructureSubGraphNode *branchNode,
  ...
  for (auto edge = branchNode->getSuccessors().begin();
       (edge != branchNode->getSuccessors().end()) && exitEdge == NULL;
       ++edge) {
    if (loop->isExitEdge(*edge)) exitEdge = *edge;
  }
...
void TR_LoopUnroller::modifyBranchTree(TR_RegionStructure *loop,
                                       ...
                                       TR_StructureSubGraphNode *branchNode) {
  TR_ASSERT(
      isCountedLoop() && _unrollKind != CompleteUnroll && _spillLoopRequired,
  ...
  if (pBlock->getEntry() == 0) {
    TR::Block *newBlock =
        TR::Block::createEmptyBlock(branch, comp(), MAX_COLD_BLOCK_COUNT + 1,
    ...
    TR_BlockStructure *newBlockStr = new (_cfg->structureRegion())
        TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
    TR_StructureSubGraphNode *newNode =
        new (_cfg->structureRegion()) TR_StructureSubGraphNode(newBlockStr);
    ...
    _cfg->addEdge(
        TR::CFGEdge::createEdge(newBlock, loop->getEntryBlock(), trMemory()));
  ...
  TR::Block *spillBlock =
      _spillNode->getStructure()->asRegion()->getEntryBlock();
  ...
  int32_t adjustment =
      getLoopStride() * (1 + _unrollCount) + (isIncreasingLoop() ? -1 : 1);

  bool const_overflow = false;
  bool is64BitComparison =
      TR::DataType::getSize(getTestChildType()) == 8 ? true : false;
  ...
  if (originalLimitNode->getOpCode().isLoadConst() && getTestIsUnsigned()) {
    ...
    TR_ASSERT(!originalLimitNode->getType().isAddress(),
              "Counted loop with a constant address limit node is unexpected, "
              "test node is %p",
              getTestNode());
    ...
    if (getTestChildType().isInt32()) {
      ...
      const_overflow = (isIncreasingLoop() &&
                        originalLimitNode->getUnsignedLongInt() < utemp) ||
                       (!isIncreasingLoop() &&
                        originalLimitNode->getUnsignedLongInt() > utemp);
    }
  }
  ...
  if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
    ...
    if (getIndexType().isAddress()) {
      tempSym = comp()->getSymRefTab()->createTemporary(
          comp()->getMethodSymbol(), TR::Address, true);
      tempSym->getSymbol()
          ->castToInternalPointerAutoSymbol()
          ->setPinningArrayPointer(_piv->getSymRef()
                                       ->getSymbol()
                                       ->castToInternalPointerAutoSymbol()
                                       ->getPinningArrayPointer());
    } else
      tempSym = comp()->getSymRefTab()->createTemporary(
          comp()->getMethodSymbol(), getTestChildType().getDataType());

    TR::Node *constNode =
        TR::Node::create(branch, getConstOpCode(getTestChildType()), 0);
    ...
    //          TR::ILOpCodes addOpCode = getTestChildType().isInt64() ?
    //          TR::lsub  : TR::isub; tempValue = TR::Node::create(addOpCode,
    //          2,originalLimitClone, constNode);

    //          storeOpCode = getTestChildType().isInt64() ? TR::lstore :
    //          TR::istore;
    //          }
    //       else
    //          {
    ...
    if (getIndexType().isAddress()) {
      tempValue->setPinningArrayPointer(_piv->getSymRef()
                                            ->getSymbol()
                                            ->castToInternalPointerAutoSymbol()
                                            ->getPinningArrayPointer());
    ...
    storeToTemp =
        TR::Node::createWithSymRef(storeOpCode, 1, 1, tempValue, tempSym);
    ...
    } else if (newLimitNode->getType().isAggregate() &&
               !getTestChildType().isAggregate()) {
      TR::ILOpCodes o2xOp = TR::ILOpCode::getProperConversion(
          newLimitNode->getDataType(), getTestChildType().getDataType(), true);
      TR_ASSERT(o2xOp != TR::BadILOp,
                "could not find conv op for %s -> %s conversion\n",
    ...
    if (lastTree->getNode()->getOpCode().isBranch()) {
      ...
      TR_ASSERT(lastTree->getNode()->getOpCodeValue() == TR::Goto,
                "expecting canon loops");

      TR::Block *newBlock = TR::Block::createEmptyBlock(
          lastTree->getNode(), comp(), pBlock->getFrequency(), pBlock);
      ...
      TR_BlockStructure *newBlockStructure = new (_cfg->structureRegion())
          TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
      TR_StructureSubGraphNode *newSubNode = new (_cfg->structureRegion())
          TR_StructureSubGraphNode(newBlockStructure);
    ...
    pBlock->append(TR::TreeTop::create(
        comp(), TR::Node::createif(opCode, originalLimitClone, c2,
                                   spillBlock->getEntry())));
  ...
  if (1)  /// moveEntryNode
  {
    TR::ILOpCodes opCode = _branchToExit ? branch->getOpCodeValue()
                                         : /* use same polarity */
    ...
    if (getIndexType().isAddress() || getTestChildType().isAddress()) {
      TR_ASSERT(getIndexType().isAddress() && getTestChildType().isAddress(),
                "Test child type and IV type should both be address");
    } else if (firstChild->getType().isAggregate() &&
               !getTestChildType().isAggregate()) {
      TR::ILOpCodes o2xOp = TR::ILOpCode::getProperConversion(
          firstChild->getDataType(), getTestChildType().getDataType(), true);
      TR_ASSERT(o2xOp != TR::BadILOp,
                "could not find conv op for %s -> %s conversion\n",
    ...
    TR::TreeTop *iterGuard = TR::TreeTop::create(
        comp(), TR::Node::createif(opCode, firstChild, dupNewLimit,
                                   spillBlock->getEntry()));
    ...
    if (lastTree->getNode()->getOpCodeValue() != TR::Goto) {
      ...
      for (auto edge = pBlock->getSuccessors().begin();
           edge != pBlock->getSuccessors().end(); ++edge) {
      ...
      TR::CFGEdge::createEdge(
          newSubNode, parent->findSubNodeInRegion(nextBlock->getNumber()),
          trMemory());

      _cfg->removeEdge(edgeToRemove);
      parent->removeEdge(
          predBlock,
          parent->findSubNodeInRegion(nextBlock->getNumber())->getStructure());
      _loopIterTestBlock = newBlock;
    } else {
      // loop invariant block ends with a goto
      // create a new block - move the goto in the new block and add append an
      // if to the loop inv block
      //
      ...
      // TR::Block::split does not know how to fix the structure edges, so do
      // that ourselves
      TR::CFGEdge::createEdge(predNode, newSubNode, trMemory());
      TR::CFGEdge::createEdge(newSubNode, loopNode, trMemory());
...
bool TR_LoopUnroller::isInternalPointerLimitExceeded() {
  return _spillLoopRequired && isCountedLoop() && getIndexType().isAddress() &&
         comp()->getSymRefTab()->getNumInternalPointers() >=
             comp()->maxInternalPointers();
}

void TR_LoopUnroller::modifyOriginalLoop(TR_RegionStructure *loop,
                                         TR_StructureSubGraphNode *branchNode) {
  ...
  if (!isCountedLoop()) {
    TR_ASSERT(_unrollKind != CompleteUnroll,
              "Non induction loops cannot be unrolled completely");
    TR_ASSERT(_spillLoopRequired == false,
              "Non induction loops must be unrolled without a spill loop");
  }

  TR::CFGEdge *edge;
  // Redirect all the local predecessors of the entry to the 'firstEntryNode'
  for (auto edge = loop->getEntry()->getPredecessors().begin();
       edge != loop->getEntry()->getPredecessors().end();) {
    TR::CFGEdge *current = *(edge++);
    if (!loop->isExitEdge(current)) {
      TR_StructureSubGraphNode *predNode =
          toStructureSubGraphNode(current->getFrom());
      if (predNode->getNumber() < _numNodes) {
        if (shouldConnectToNextIteration(predNode, loop)) {
          addEdgeAndFixEverything(loop, current, predNode, _firstEntryNode,
                                  true, true, true, BackEdgeFromPrevGeneration);
          loop->removeEdge(predNode->getStructure(),
                           loop->getEntry()->getStructure());
        } else {
          if (trace())
            traceMsg(comp(),
                     "Redirecting cold edge from block_%d to unrolled loop "
                     "header rather than going to the next iteration\n",
                     predNode->getNumber());
          addEdgeAndFixEverything(loop, current, predNode, loop->getEntry(),
                                  true, false, true, BackEdgeToEntry);
        }
      }
    }
  }
  ...
  for (TR_StructureSubGraphNode *node = nodeIt.getFirst(); node;
       node = nodeIt.getNext())
  ...
  if (_unrollKind == SPMDKernel) {
    if (_spillLoopRequired) modifyBranchTree(loop, loopNode, branchNode);
  } else if (isCountedLoop() && _unrollKind != CompleteUnroll) /*GGLU*/
  {
    ...
    // IVA does not create PIV for ifacmpne and ifacmpeq, thus loops with
    // ifacmpne are not counted loop and they won't enter this if so there is no
    // need to check for ifacmpne
    TR_ASSERT(branch->getOpCodeValue() != TR::ifacmpeq &&
                  branch->getOpCodeValue() != TR::ifacmpne,
  ...
  } else if (_unrollKind == CompleteUnroll) /*GGLU*/
  {
    ...
    if (loop->contains(destStruct, loop->getParent())) {
      ...
      for (auto edge = branchNode->getSuccessors().begin();
           (edge != branchNode->getSuccessors().end()) && exitNode == NULL;
           ++edge) {
        if (loop->isExitEdge(*edge)) exitNode = (*edge)->getTo();
      }

      // Remove B->X*
      //
      removeExternalEdge(loop, branchNode, exitNode->getNumber());
      TR_StructureSubGraphNode *concreteExitNode = findNodeInHierarchy(
          loop->getParent()->asRegion(), exitNode->getNumber());
  ...
  //       {
  //       //Adjust the induction variable information to include the new
  //       information _indVar->setIncr(new (trHeapMemory())
  //       TR::VPIntConst(_loopStride * (1 + _unrollCount)));
  //       }
  //    else
  //       {
  if (_unrollKind != SPMDKernel) loop->clearInductionVariables();
  //      }
  ...
  if (_spillLoopRequired) {
    ...
    TR_StructureSubGraphNode *exitNode = findNodeInHierarchy(
        parent, toStructureSubGraphNode(exitEdge->getTo())->getNumber());

    // Look at the exit edges of S and put the representatives in the parent
    // region as needed
    //
    ...
    for (edge = it.getFirst(); edge; edge = it.getNext()) {
      ...
      TR_StructureSubGraphNode *concreteNode =
          findNodeInHierarchy(parent, destNum);
    ...
    if (1) {
      ...
      TR::Block *newIfBlock = TR::Block::createEmptyBlock(
          branchTreeTop->getNode(), comp(), origExitBlock->getFrequency(),
          origExitBlock);
      TR_BlockStructure *newIfBlockStructure;
      TR_StructureSubGraphNode *newIfSubNode;

      // Duplicate the branch tree from the spill entry block and place it
      // correctly in the trees connect this block up properly in the parent
      // region
      //
      if (1) {
        TR::Block *branchDestination = branchTreeTop->getNode()
                                           ->getBranchDestination()
                                           ->getNode()
                                           ->getBlock();
        TR::TreeTop *newIfTree = 0;
        // TR::TreeTop::create(comp(),
        // branchTreeTop->getNode()->duplicateTree());
        ...
        if (1) {
          ...
          if (getIndexType().isAddress() || getTestChildType().isAddress()) {
            TR_ASSERT(
                getIndexType().isAddress() && getTestChildType().isAddress(),
                "Test child type and IV type should both be address");
          } else if (c1->getType().isAggregate() &&
                     !getTestChildType().isAggregate()) {
            TR::ILOpCodes o2xOp = TR::ILOpCode::getProperConversion(
          ...
          if (loop->getPrimaryInductionVariable() &&
              loop->getPrimaryInductionVariable()
            ...
            if (!getTestIsUnsigned()) {
              ...
              if (is64BitComparison) {
                ...
                constNode =
                    TR::Node::lconst(boundNode, (int64_t)getLoopStride());
              } else {
          ...
          newIfTree = TR::TreeTop::create(
              comp(),
              TR::Node::createif(ifNode->getOpCodeValue(), c1, boundNode,
                                 ifNode->getBranchDestination()));
        }

        newIfBlock->append(newIfTree);
        _cfg->addNode(newIfBlock);
        newIfBlockStructure = new (_cfg->structureRegion())
            TR_BlockStructure(comp(), newIfBlock->getNumber(), newIfBlock);
        newIfSubNode = new (_cfg->structureRegion())
            TR_StructureSubGraphNode(newIfBlockStructure);
        ...
        TR::Block *gotoBlock = TR::Block::createEmptyBlock(
            gotoNode, comp(), origExitBlock->getFrequency(),
        ...
        _cfg->addEdge(TR::CFGEdge::createEdge(
            gotoBlock, spillLoop->getEntryBlock(), trMemory()));
        ...
        TR_StructureSubGraphNode *gotoSubNode =
            new (_cfg->structureRegion()) TR_StructureSubGraphNode(
                new (_cfg->structureRegion()) TR_BlockStructure(
                    comp(), gotoBlock->getNumber(), gotoBlock));
        ...
        _cfg->addEdge(
            TR::CFGEdge::createEdge(newIfBlock, origExitBlock, trMemory()));
        _cfg->addEdge(
      ...
      // Add L->I
      TR::CFGEdge::createEdge(loopNode, newIfSubNode,
                              trMemory());  // the CFG is already correct
    ...
    // Find the edge corresponding the exitEdge in the successors of the loop
    // node (L->X?)
    TR::CFGEdge *correspondingExitEdge = NULL;
    for (auto edge = loopNode->getSuccessors().begin();
         (edge != loopNode->getSuccessors().end()) && !correspondingExitEdge;
         ++edge)
      if (toStructureSubGraphNode((*edge)->getTo())->getNumber() ==
          exitNode->getNumber())
    ...
    if (1) {
      ...
      for (e = eit.getFirst(); e; e = eit.getNext()) {
        if (e->getTo()->getNumber() == exitNode->getNumber()) break;
      }
      if (!e)  // there do not remain an edge in L going to X
      {
        if (exitNode->getStructure()->getParent() == parent) {
          bool found = (std::find(loopNode->getSuccessors().begin(),
          ...
          found = (std::find(exitNode->getPredecessors().begin(),
  ...
  if (0 && _spillLoopRequired) {
    ...
    // Find the edge corresponding to exitEdge in the successors of the loop
    // node
    TR::CFGEdge *correspondingExitEdge = NULL;
    for (auto edge = loopNode->getSuccessors().begin();
         (edge != loopNode->getSuccessors().end()) && !correspondingExitEdge;
    ...
    if (!skipIt) {
      ...
      if (1) {
        TR::Block *branchDestination = branchTreeTop->getNode()
                                           ...
                                           ->getNode()
                                           ->getBlock();
        TR::TreeTop *newIfTree = TR::TreeTop::create(
            comp(), branchTreeTop->getNode()->duplicateTree());
        newIfBlock->append(newIfTree);
        ...
        gotoBlock = TR::Block::createEmptyBlock(
            gotoNode, comp(), newIfBlock->getFrequency(), newIfBlock);
        gotoBlock->append(TR::TreeTop::create(comp(), gotoNode));
        _cfg->addNode(gotoBlock);
        TR_StructureSubGraphNode *gotoSubNode =
            new (_cfg->structureRegion()) TR_StructureSubGraphNode(
      ...
      if (1) {
        ...
        TR::CFGEdgeList &list =
            _spillNode->getStructure()->asRegion()->getEntry()->getSuccessors();
      ...
      if (1) {
        ...
        TR::Block *fromBlock =
            fromSubNode->getStructure()->asBlock()->getBlock();
        TR::TreeTop *lastTree = fromBlock->getLastRealTreeTop();

        if (lastTree->getNode()->getOpCode().isBranch() &&
            lastTree->getNode()->getBranchDestination() ==
                oldSpillEntryBlock->getEntry()) {
          lastTree->getNode()->setBranchDestination(newIfBlock->getEntry());
        }

        _cfg->addEdge(
            TR::CFGEdge::createEdge(fromBlock, newIfBlock, trMemory()));
      ...
      // add L->I
      TR::CFGEdge::createEdge(loopNode, newIfSubNode,
                              trMemory());  // the CFG is already fine

      // Remove L->S'
      if (1) {
        for (auto edge = loopNode->getSuccessors().begin();
             edge != loopNode->getSuccessors().end(); ++edge) {
      ...
      _cfg->addEdge(
          TR::CFGEdge::createEdge(gotoBlock, newEntryBlock, trMemory()));

      /// Fixup the overflow test
      if (_overflowTestBlock) {
        _overflowTestBlock->getLastRealTreeTop()
            ->getNode()
            ->setBranchDestination(newEntryBlock->getEntry());
        TR_StructureSubGraphNode *overflowTestNode =
            parent->findSubNodeInRegion(_overflowTestBlock->getNumber());

        _cfg->addEdge(TR::CFGEdge::createEdge(_overflowTestBlock, newEntryBlock,
                                              trMemory()));
      ...
      if (1 && _loopIterTestBlock) {
        TR::Block *newEntryBlock = spillLoop->getEntryBlock();
        _loopIterTestBlock->getLastRealTreeTop()
            ->getNode()
            ->setBranchDestination(newEntryBlock->getEntry());
        _cfg->removeEdge(_loopIterTestBlock, oldSpillEntryBlock);
        _cfg->addEdge(TR::CFGEdge::createEdge(_loopIterTestBlock, newEntryBlock,
                                              trMemory()));
      }
  ...
  if (isCountedLoop() && _unrollKind != CompleteUnroll) {
    ...
    // Change the loop structure, and mark the first unroll body as the entry
    // node
    //
    ...
    // More work is required if there are multiple regions with the same number
    // as the loop
    //
    do {
      ...
      // Goto the parent region, and redirect all the edges going to loop's old
      // entry to the new entry.  Look at all the predecessors of the loop entry
      // node.
      //
      for (auto edge = cursorNode->getPredecessors().begin();
           edge != cursorNode->getPredecessors().end();) {
        auto next = edge;
        ++next;
        TR_StructureSubGraphNode *predNode =
            toStructureSubGraphNode((*edge)->getFrom());
    ...
    for (auto edge = loopNode->getPredecessors().begin();
         edge != loopNode->getPredecessors().end();) {
      auto next = edge;
      ++next;
      TR_StructureSubGraphNode *predNode =
          toStructureSubGraphNode((*edge)->getFrom());
...
int32_t TR_LoopUnroller::numExitEdgesTo(TR_RegionStructure *from, int32_t to) {
  ...
  TR::CFGEdge *edge;
  for (edge = ei.getFirst(); edge; edge = ei.getNext())
    if (edge->getTo()->getNumber() == to) count++;
...
void TR_LoopUnroller::prepareForArrayShadowRenaming(TR_RegionStructure *loop) {
  ...
  // Non-counted loops are being unrolled in such a way that prevents scheduling
  // across iterations, so there is no point to refine this way.
  //
  if (!_piv) return;
...
void TR_LoopUnroller::getLoopPreheaders(TR_RegionStructure *loop,
                                        TR_ScratchList<TR::Block> *preheaders) {
  TR::Block *loopEntry = loop->getEntryBlock();
  // locate the first preheader in the loop
  TR::Block *firstPreheader = NULL;
  for (auto edge = loopEntry->getPredecessors().begin();
       edge != loopEntry->getPredecessors().end(); ++edge) {
  ...
  for (TR::Block *pred =
           toBlock(preheader->getPredecessors().front()->getFrom());
       (preheader->getPredecessors().size() == 1) &&
       pred->getStructureOf()->isLoopInvariantBlock();
       preheader = pred, pred = toBlock(
                             preheader->getPredecessors().front()->getFrom())) {
    preheaders->add(pred);
  }
}

static bool isBiv(TR_RegionStructure *loop, TR::SymbolReference *symRef,
                  TR_BasicInductionVariable *&biv) {
  biv = loop->getPrimaryInductionVariable();
  if (biv && biv->getSymRef() == symRef) return true;

  ListIterator<TR_BasicInductionVariable> it(
      &loop->getBasicInductionVariables());
  for (biv = it.getFirst(); biv; biv = it.getNext()) {
    if (biv->getSymRef() == symRef) return true;
  }
...
void TR_LoopUnroller::collectInternalPointers() {
  ...
  for (TR::Block *preheader = it.getFirst(); preheader;
       preheader = it.getNext()) {
    if (trace())
      traceMsg(
          comp(),
          "Examining pre-header %d of loop %d for array aliasing refinement\n",
        ...
        if ((secondChild->getOpCode().isAdd() ||
          ...
          if ((firstChild->getOpCode().isMul() ||
            ...
            if ((firstChild->getOpCode().isAdd() ||
              ...
              else if (firstChild->getSecondChild()->getOpCodeValue() ==
                       TR::lconst)
            ...
            if (firstChild->getOpCode().isLoad()) {
              ...
              if (canMatch || canMatchAsIP) {
                if (trace())
                  traceMsg(comp(),
                           "\tFound internal pointer %p with iv %d in offset "
                           "node %p\n",
                           node, biv->getSymRef()->getReferenceNumber(),
                           secondChild);
                IntrnPtr *intrnPtr =
                    (IntrnPtr *)trMemory()->allocateStackMemory(
                        sizeof(IntrnPtr));
                intrnPtr->symRefNum =
                ...
                if (canMatch) {
                  ...
                  intrnPtr->bivNum =
                      firstChild->getSymbolReference()->getReferenceNumber();
                  intrnPtr->biv = NULL;
                }
...
void TR_LoopUnroller::collectArrayAccesses() {
  ...
  for (block = it.getFirst(); block; block = it.getNext()) {
    ...
    if (block->getNumber() < _numNodes) {
      ...
      while (currentTree != exitTree) {
        TR::Node *node = currentTree->getNode();
        if (node->getNumChildren() > 0) examineNode(node, visitCount);
        currentTree = currentTree->getNextTreeTop();
      }
    }
  }
...
void TR_LoopUnroller::examineNode(TR::Node *node, intptrj_t visitCount) {
  ...
  if (node->getOpCode().hasSymbolReference()) symbol = node->getSymbol();
  ...
  if (symbol && symbol->isArrayShadowSymbol()) {
    ...
    for (ListsOfArrayAccesses *loaa = it.getFirst(); loaa;
         loaa = it.getNext()) {
    ...
    if (!listOfAA) {
      ListsOfArrayAccesses *loaa =
          (ListsOfArrayAccesses *)trMemory()->allocateStackMemory(
              sizeof(ListsOfArrayAccesses));
    ...
    ArrayAccess *aa =
        (ArrayAccess *)trMemory()->allocateStackMemory(sizeof(ArrayAccess));
    aa->aaNode = node;
    ...
    if (trace())
      traceMsg(comp(),
               "\t\tFound array access node %p with sym ref %d and internal "
               "pointer node %p\n",
...
TR_LoopUnroller::IntrnPtr *TR_LoopUnroller::findIntrnPtr(int32_t symRefNum) {
  ...
  for (IntrnPtr *ip = it.getFirst(); ip; ip = it.getNext()) {
    if (ip->symRefNum == symRefNum) return ip;
  }
  return NULL;
}

bool TR_LoopUnroller::haveIdenticalOffsets(IntrnPtr *intrnPtr1,
                                           IntrnPtr *intrnPtr2) {
  if (!intrnPtr1->isOffsetConst || !intrnPtr2->isOffsetConst) return false;
  ...
  if (!entryValueNode1 || !entryValueNode2) return false;

  int64_t entryValue1 = entryValueNode1->getType().isInt64()
                            ? entryValueNode1->getLongInt()
                            : (int64_t)entryValueNode1->getInt();
  int64_t entryValue2 = entryValueNode2->getType().isInt64()
                            ? entryValueNode2->getLongInt()
                            : (int64_t)entryValueNode2->getInt();
  if (intrnPtr1->biv->getDeltaOnBackEdge() ==
          intrnPtr2->biv->getDeltaOnBackEdge() &&
      entryValue1 + intrnPtr1->offsetValue ==
          entryValue2 + intrnPtr2->offsetValue)
    return true;
...
void TR_LoopUnroller::examineArrayAccesses() {
  // What we are trying to achieve here is to allow the possibility to schedule
  // array accesses that belong to different unrolled iterations across these
  // iterations. To do so, we need to guarantee that the array accesses, for
  // which we will alow that, will either access the same location at any given
  // iteration, or always access different locations. a[i] and b[i] are an
  // example of the case: if (a == b), the accesses are the same in any given
  // iteration, and, thus, always different between unrolled iterations. if (a
  // != b), the accesses are simply always different. So as long as we can prove
  // that two accesses have the same index, we can refine their shadows between
  // the iterations.
  //
  // Iterate on the collected array accesses and see if they qualify for alias
  // refinement. If an array access does not qualify, remove the list where it
  // belongs, as we cannot quarantee that they all either access the same
  // location or belong to entirely different arrays, and, thus, will never
  ...
  for (ListsOfArrayAccesses *loaa = it.getFirst(); loaa; loaa = it.getNext()) {
    ...
    for (aa1 = it1.getFirst(); aa1 && (aa2 = it1.getNext()); aa1 = aa2) {
      ...
      // Do not take care of array accesses without internal pointers at this
      // point.
      //
      ...
      IntrnPtr *intrnPtr1 = findIntrnPtr(
          aa1->intrnPtrNode->getSymbolReference()->getReferenceNumber());
      IntrnPtr *intrnPtr2 = findIntrnPtr(
          aa2->intrnPtrNode->getSymbolReference()->getReferenceNumber());
      if (intrnPtr1 && intrnPtr2) {
        if (aa1->intrnPtrNode == aa2->intrnPtrNode) continue;
        if (intrnPtr1->offsetNode == intrnPtr2->offsetNode) continue;
        if (haveIdenticalOffsets(intrnPtr1, intrnPtr2)) continue;
      }
      canProve = false;
      break;
    }

    if (!canProve || !aa1->intrnPtrNode)  // if cannot prove or if there is only
                                          // one element in the list
    {  // and it doesn't have an internal pointer
      if (trace())
        traceMsg(comp(),
                 "List of array accesses with sym ref %d does not qualify for "
                 "aliasing refinement\n",
                 loaa->symRefNum);
...
int32_t TR_LoopUnroller::unroll(TR_RegionStructure *loop,
                                TR_StructureSubGraphNode *branchNode) {
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());

  // Initialize the mapping datastructures
  //
  _blockMapper[0] = (TR::Block **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(TR::Block *));
  _blockMapper[1] = (TR::Block **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(TR::Block *));
  _nodeMapper[0] = (TR_StructureSubGraphNode **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(TR::Block *));
  _nodeMapper[1] = (TR_StructureSubGraphNode **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(TR::Block *));
  ...
  if (_spillLoopRequired) generateSpillLoop(loop, branchNode);
  ...
  if (!_newSymRefs.isEmpty()) optimizer()->setAliasSetsAreValid(false);
...
TR::Node *TR_LoopUnroller::createIfNodeForSpillLoop(TR::Node *ifNode) {
  ...
  return TR::Node::createif(ifNode->getOpCodeValue(), c1,
                            ifNode->getSecondChild()->duplicateTree(),
                            ifNode->getBranchDestination());
}
...
void TR_LoopUnroller::generateSpillLoop(TR_RegionStructure *loop,
  ...
  TR_StructureSubGraphNode *spillNode =
      new (_cfg->structureRegion()) TR_StructureSubGraphNode(spillLoop);
  ...
  _spillBranchBlock =
      GET_CLONE_NODE(branchNode)->getStructure()->asBlock()->getBlock();

  if (_wasEQorNELoop) {
    TR_ASSERT(_spillBranchBlock->getLastRealTreeTop()
                  ->getNode()
                  ->getOpCode()
                  .isBranch(),
              "expecting a branch in the spill loop branch block");
    TR::Node::recreate(_spillBranchBlock->getLastRealTreeTop()->getNode(),
                       _origLoopCondition);
  }
}
...
*branchNode)
   ...
   _iteration=0;
   memset(_blockMapper[CURRENT_MAPPER], 0, _numNodes * sizeof(TR::Block *));
   memset( _nodeMapper[CURRENT_MAPPER], 0, _numNodes *
sizeof(TR_StructureSubGraphNode *));


   //clone the loop
   cloneBlocksInRegion(loop);
   TR_RegionStructure *spillLoop = cloneStructure(loop)->asRegion();
   TR_StructureSubGraphNode *spillNode = new (_cfg->structureRegion())
TR_StructureSubGraphNode(spillLoop); fixExitEdges(loop, spillLoop, branchNode);

   TR_StructureSubGraphNode *clonedBranchNode = GET_CLONE_NODE(branchNode);
   TR_BlockStructure *clonedBranchStructure =
clonedBranchNode->getStructure()->asBlock(); TR::Block *clonedBranchBlock =
clonedBranchStructure->getBlock(); TR::TreeTop *branchTT =
clonedBranchBlock->getLastRealTreeTop(); TR::Node *node = branchTT->getNode();
   ...
   TR_BlockStructure *newBranchStructure = new (_cfg->structureRegion())
TR_BlockStructure(comp(), newBranchBlock->getNumber(), newBranchBlock);
   TR_StructureSubGraphNode *newBranchNode = new (_cfg->structureRegion())
TR_StructureSubGraphNode(newBranchStructure);
...
loop->getNumber()); getDebug()->print(comp()->getOutFile(), _rootStructure, 6);
      getDebug()->print(comp()->getOutFile(), _cfg);
      comp()->dumpMethodTrees("method trees:");
      }
   ...
   for (edge = edgeIt.getFirst(); edge; edge = edgeIt.getNext())
      {
      TR_StructureSubGraphNode *successor =
toStructureSubGraphNode(edge->getTo()); if (successor->getStructure() != NULL)
         internalEdge = edge;
      }

   //Find the exit edge from B to X
   //find the original exit edge in the original loop, because the one in the
spill does not have cfg edges edgeIt.set(&branchNode->getSuccessors()); for
(edge = edgeIt.getFirst(); edge; edge = edgeIt.getNext())
      {
      TR_StructureSubGraphNode *successor =
toStructureSubGraphNode(edge->getTo()); if (successor->getStructure() == NULL)
         exitEdge = edge;
...
toStructureSubGraphNode(exitEdge->getTo())->getNumber());
   ...
   //Remove B->K
   {
      TR_StructureSubGraphNode *nodeK =
toStructureSubGraphNode(internalEdge->getTo()); TR::Block *blockK =
getEntryBlockNode(nodeK)->getStructure()->asBlock()->getBlock();
   ...
   {
      TR::Block *blockX =
getEntryBlockNode(exitNode)->getStructure()->asBlock()->getBlock();
   ...
   for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop; treeTop =
...
TR_StructureSubGraphNode *TR_LoopUnroller::findNodeInHierarchy(
    TR_RegionStructure *region, int32_t num) {
  if (!region) return NULL;
  TR_RegionStructure::Cursor it(*region);
  TR_StructureSubGraphNode *node;
  for (node = it.getFirst(); node; node = it.getNext()) {
    if (node->getNumber() == num) return node;
  }
  return findNodeInHierarchy(region->getParent()->asRegion(), num);
}

void TR_LoopUnroller::addEdgeForSpillLoop(
    TR_RegionStructure *region, TR::CFGEdge *originalEdge,
    TR_StructureSubGraphNode *newFromNode, TR_StructureSubGraphNode *newToNode,
    bool removeOriginal, EdgeContext edgeContext, bool notLoopBranchNode) {
  TR_StructureSubGraphNode *fromNode =
      toStructureSubGraphNode(originalEdge->getFrom());
  TR_StructureSubGraphNode *toNode =
      toStructureSubGraphNode(originalEdge->getTo());
  if (toNode->getStructure() == NULL)  // exit edge
    toNode = findNodeInHierarchy(region->getParent()->asRegion(),
                                 toNode->getNumber());
  ...
  for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
    ...
    if (newFromNode->getStructure()->asBlock())
      ...
      newFrom =
          getEntryBlockNode(newFromNode)->getStructure()->asBlock()->getBlock();

    if (newToNode->getStructure()->asBlock())
      newTo = newToNode->getStructure()->asBlock()->getBlock();
    else
      newTo =
          getEntryBlockNode(newToNode)->getStructure()->asBlock()->getBlock();
    ...
    if (lastNode->getOpCode().isBranch() &&
      ...
      // In case of this special contex the exit edge was already removed by
      // unrollLoopOnce
      if (edgeContext != BackEdgeFromLastGenerationCompleteUnroll) {
        newFrom->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
            comp(), to->getEntry(), newTo->getEntry());
      } else {
    ...
    // FALLS INTO
      ...
      if (newFromNextBlock != newTo)  // but the copy does not fall through
      {
        ...
        gotoNode->setLocalIndex(CREATED_BY_GLU);
        TR::Block *gotoBlock = TR::Block::createEmptyBlock(
            lastNode, comp(), newFrom->getFrequency(), newFrom);
        ...
        if (notLoopBranchNode &&
          ...
          //     -- addExitEdgeAndFixEverything will add the new exit edge
          //     eventually
          //

          if (newFromLastNode->getOpCode().isBranch()) {
            if (newFromLastNode->getBranchDestination() ==
                _loop->getEntryBlock()->getEntry()) {
              TR::TreeTop *endTree =
                  comp()->getMethodSymbol()->getLastTreeTop();
            ...
            /*
            else if (newFromLastNode->getBranchDestination() ==
            newTo->getEntry())
               {
               newFrom->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(comp(),
            newTo->getEntry(), gotoBlock->getEntry());
               }
            */
          }
        }
        ...
        TR_BlockStructure *gotoStruct = new (_cfg->structureRegion())
            TR_BlockStructure(comp(), gotoBlock->getNumber(), gotoBlock);
        TR_StructureSubGraphNode *gotoSubNode =
            new (_cfg->structureRegion()) TR_StructureSubGraphNode(gotoStruct);
    ...
    if (removeOriginal) _cfg->removeEdge(edge);
  }
}

void TR_LoopUnroller::addExitEdgeAndFixEverything(
    TR_RegionStructure *region, TR::CFGEdge *originalEdge,
    TR_StructureSubGraphNode *newFromNode, TR_StructureSubGraphNode *exitNode,
    TR::Block *newExitBlock, EdgeContext context) {
  TR_StructureSubGraphNode *fromNode =
      toStructureSubGraphNode(originalEdge->getFrom());
      ...
      if (newFromNextBlock != newTo) {
        ...
        if (context == ExitEdgeFromBranchNode) {
          ...
          if (!cfgEdgeAlreadyExists(newFrom, newTo)) {
            ...
            gotoNode->setLocalIndex(CREATED_BY_GLU);
            TR::Block *gotoBlock = TR::Block::createEmptyBlock(
                lastNode, comp(), newTo->getFrequency(), newTo);
            gotoBlock->append(gotoTree);
            gotoBlock->getEntry()->getNode()->setLocalIndex(CREATED_BY_GLU);
            _cfg->addNode(gotoBlock);
            ...
            TR_StructureSubGraphNode *gotoSubNode =
                new (_cfg->structureRegion())
                    TR_StructureSubGraphNode(gotoStruct);
            region->addSubNode(gotoSubNode);

            // create the cfg edges
            _cfg->addEdge(
                TR::CFGEdge::createEdge(newFrom, gotoBlock, trMemory()));
            _cfg->addEdge(
                TR::CFGEdge::createEdge(gotoBlock, newTo, trMemory()));
...
void TR_LoopUnroller::addEdgeAndFixEverything(
    TR_RegionStructure *region, TR::CFGEdge *originalEdge,
    TR_StructureSubGraphNode *newFromNode, TR_StructureSubGraphNode *newToNode,
    bool redirectOriginal, bool removeOriginalEdges, bool edgeToEntry,
    EdgeContext context) {
  TR_StructureSubGraphNode *fromNode =
      toStructureSubGraphNode(originalEdge->getFrom());
  TR_StructureSubGraphNode *toNode =
  ...
  if (newToNode == NULL) newToNode = GET_CLONE_NODE(toNode);
  ...
  for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
    ...
    // SWITCH
      ...
      if (!edgeAlreadyExists(newFromNode, newToNode))
        TR::CFGEdge::createEdge(newFromNode, newToNode, trMemory());
      if (!cfgEdgeAlreadyExists(newFrom, newTo)) _cfg->addEdge(newFrom, newTo);
      newFrom->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
          comp(), to->getEntry(), newTo->getEntry());
    }
    // RETURNS
    ...
    // FALLS INTO
      ...
      if (newFromNextBlock != newTo) {
        TR_ASSERT(
            context == BackEdgeFromPrevGeneration || context == BackEdgeToEntry,
    ...
    if (removeOriginalEdges) _cfg->removeEdge(edge);
  }
}

bool TR_LoopUnroller::cfgEdgeAlreadyExists(TR::Block *from, TR::Block *to,
                                           EdgeContext edgeContext) {
  for (auto edge = from->getSuccessors().begin();
       edge != from->getSuccessors().end(); ++edge) {
    TR::Block *succ = toBlock((*edge)->getTo());
    if (succ->getNumber() == to->getNumber()) return true;

    // Other possibility is that we have already inserted a goto block to get to
    // 'to'
    // Ignore it for the special case of fixing up backedges for the last
    // generation in a complete unroll
    if (edgeContext != BackEdgeFromLastGenerationCompleteUnroll) {
      ...
      if (firstRealTree == lastRealTree &&
        ...
        TR::Block *dest = firstRealTree->getNode()
                              ->getBranchDestination()
                              ->getNode()
                              ->getBlock();
        // check if the goto block was really added by GLU
        // in some cases, the controlling branch may have been replaced with a
        // goto to the loop entry, so just checking the goto node is
        // insufficient
        //
...
bool TR_LoopUnroller::edgeAlreadyExists(TR_StructureSubGraphNode *from,
  ...
  for (auto edge = from->getSuccessors().begin();
       edge != from->getSuccessors().end(); ++edge)
    if ((*edge)->getTo()->getNumber() == toNum) return true;
  return false;
}

TR_StructureSubGraphNode *TR_LoopUnroller::getEntryBlockNode(
    TR_StructureSubGraphNode *node) {
  if (node->getStructure()->asBlock())
    return node;
  else
...
void TR_LoopUnroller::cloneBlocksInRegion(TR_RegionStructure *region,
                                          bool isSpillLoop) {
  TR_ScratchList<TR::Block> blocksInRegion(trMemory());
  ...
  if (!_startPosOfUnrolledBodies) _startPosOfUnrolledBodies = endTreeTop;
...
TR_Structure *TR_LoopUnroller::cloneBlockStructure(
    TR_BlockStructure *blockStructure) {
  TR::Block *block = blockStructure->getBlock();
  TR::Block *clonedBlock = GET_CLONE_BLOCK(block);
  TR_BlockStructure *clonedBlockStructure = new (_cfg->structureRegion())
      TR_BlockStructure(comp(), clonedBlock->getNumber(), clonedBlock);
  clonedBlockStructure->setAsLoopInvariantBlock(
      blockStructure->isLoopInvariantBlock());
  clonedBlockStructure->setNestingDepth(blockStructure->getNestingDepth());
  clonedBlockStructure->setMaxNestingDepth(
      blockStructure->getMaxNestingDepth());
  return clonedBlockStructure;
}

TR_Structure *TR_LoopUnroller::cloneRegionStructure(
    TR_RegionStructure *region) {
  TR_RegionStructure *clonedRegion = new (_cfg->structureRegion())
      TR_RegionStructure(comp(), 0xDeadF00d);  // for now
...
void TR_LoopUnroller::swingBlocks(TR::Block *from, TR::Block *to) {
  ...
  SwingPair *pair =
      (SwingPair *)trMemory()->allocateStackMemory(sizeof(SwingPair));
  pair->from = from;
  pair->to = to;
...
// We want block 'from' to fall into block 'to', but they are not it the correct
// order in the tree tops.  Swing some blocks around to make it so.
void TR_LoopUnroller::processSwingBlocks(TR::Block *from, TR::Block *to) {
  ...
  if (pF == NULL ||
      isSuccessor(pF, from) == false) {  // insert F between pT and T
    ...
    if (pT)
      ...
      from->getEntry()->setPrevTreeTop(NULL);
             ...
             isSuccessor(to, nT) == false) {  // insert T between F and nF
...
bool TR_LoopUnroller::isSuccessor(TR::Block *A, TR::Block *B) {
  for (auto edge = A->getSuccessors().begin(); edge != A->getSuccessors().end();
       ++edge) {
    if (B == toBlock((*edge)->getTo())) return true;
  }
...
void TR_LoopUnroller::fixExitEdges(TR_Structure *s, TR_Structure *clone,
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    TR_StructureSubGraphNode *clonedSubNode = findNodeInHierarchy(
        clonedRegion,
        _blockMapper[CURRENT_MAPPER][subNode->getNumber()]->getNumber());
    fixExitEdges(subNode->getStructure(), clonedSubNode->getStructure());
  }
  ...
  for (edge = ei.getFirst(); edge; edge = ei.getNext()) {
    if (region->isExitEdge(edge)) {
      ...
      // We cannot use GET_CLONE_NODE, because it does not have the correct
      // bottom
      // level mappings at this stage
      TR_StructureSubGraphNode *clonedFromNode = findNodeInHierarchy(
          clonedRegion,
          _blockMapper[CURRENT_MAPPER][fromNode->getNumber()]->getNumber());
...
bool TR_LoopUnroller::isTransactionStartLoop(TR_RegionStructure *loop,
                                             TR::Compilation *comp) {
  if (comp->getOption(TR_DisableTLE) ||
      !comp->cg()->getSupportsTM())  // if TLE is not supported or on, it is not
                                     // possible
    return false;

  TR::Block *entryBlock = loop->getEntryBlock();

  for (TR::TreeTop *tt = entryBlock->getEntry(); tt != entryBlock->getExit();
       tt = tt->getNextTreeTop()) {
    if (tt->getNode()->getOpCodeValue() == TR::tstart) return true;
  }

  return false;
}

bool TR_LoopUnroller::isWellFormedLoop(TR_RegionStructure *loop,
                                       TR::Compilation *comp,
                                       TR::Block *&loopInvariantBlock) {
  if (loop->getExitEdges().isEmpty()) return false;
  ...
  if (loopNode && (loopNode->getPredecessors().size() == 1)) {
    ...
    if (predBlock && predBlock->isLoopInvariantBlock()) {
      ...
      for (auto e = entryNode->getPredecessors().begin();
           e != entryNode->getPredecessors().end(); ++e) {
        TR_StructureSubGraphNode *source =
            toStructureSubGraphNode((*e)->getFrom());
      ...
      if (backEdgesFromBlocks) {
        ...
        dumpOptDetails(
            comp,
            "loop has backedges from other regions, not a well formed loop\n");
    } else
      dumpOptDetails(
          comp, "loop has no loop-invariant block, not a well formed loop\n");
  }

  return false;
}

TR_LoopUnroller::TR_LoopUnroller(TR::Compilation *c, TR::Optimizer *optimizer,
                                 TR_RegionStructure *loop,
                                 TR_StructureSubGraphNode *branchNode,
...
TR_LoopUnroller::TR_LoopUnroller(TR::Compilation *c, TR::Optimizer *optimizer,
                                 ...
                                 UnrollKind unrollKind, int32_t unrollCount,
                                 int32_t peelCount, TR::Block *invariantBlock,
                                 int32_t vectorSize)
    : _comp(c),
      _trMemory(c->trMemory()),
      _optimizer(optimizer),
  ...
  bool indexOnLHS = nodeRefersToSymbol(branch->getFirstChild(),
                                       piv->getSymRef()->getSymbol());
  TR_ASSERT(indexOnLHS, "FIXME: loop not canonicalized");

  _branchToExit = !loop->contains(
      branch->getBranchDestination()->getNode()->getBlock()->getStructureOf(),
      loop->getParent());
}

// ::unrollCountedLoop
bool TR_LoopUnroller::unroll(TR::Compilation *comp, TR_RegionStructure *loop,
                             TR_PrimaryInductionVariable *piv,
                             UnrollKind unrollKind, int32_t unrollCount,
                             int32_t peelCount, TR::Optimizer *optimizer) {
  ...
  if (isTransactionStartLoop(loop, comp)) {
    dumpOptDetails(comp,
                   "Cannot unroll loop %d: it is a transaction start loop\n",
                   loop->getNumber());
    return false;
  }

  TR_LoopUnroller unroller(comp, optimizer, loop, piv, unrollKind, unrollCount,
                           peelCount, loopInvariantBlock);

  if (1) {
    TR::Block *branchBlock =
        unroller._branchNode->getStructure()->asBlock()->getBlock();
    TR::Node *branch = branchBlock->getLastRealTreeTop()->getNode();
    ...
    if (unroller._spillLoopRequired) {
      ...
      if (!backEdgeFound) {
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock;
          ...
          if (nextBlock != loop->getEntryBlock()) break;
        }

        bool loopCanBeUnrolled = false;
        if (branchBlock == loop->getEntryBlock()) {
          if ((branchBlock->getLastRealTreeTop() ==
               branchBlock->getFirstRealTreeTop()) ||
              ((branchBlock->getFirstRealTreeTop()
                    ->getNode()
                    ->getOpCodeValue() == TR::asynccheck) &&
        ...
        if (!loopCanBeUnrolled) {
          if (blocksInLoop.isDoubleton() &&
              nextBlock->getSuccessors().size() == 1) {
            if ((nextBlock->getEntry()->getNextTreeTop() ==
                 nextBlock->getExit()) ||
                (nextBlock->getLastRealTreeTop() ==
                 nextBlock->getFirstRealTreeTop()) ||
                ((nextBlock->getFirstRealTreeTop()
                      ->getNode()
                      ->getOpCodeValue() == TR::asynccheck) &&
                 (nextBlock->getLastRealTreeTop() ==
        ...
        if (loopCanBeUnrolled) {
          // Either the entry block has the branch condition or the only 2
          // blocks in the loop are the entry and the branch block Further the
          // only code other from the entry block in the loop must be the branch
          // (and an optional async check)
          //
        } else {
          dumpOptDetails(comp,
                         "Cannot unroll loop %d: exit condition is not in a "
                         "block containing a backedge\n",
                         loop->getNumber());
          return false;
        }
      }
    ...
    if (1) {
      ...
      if (!foundExitEdge) {
        dumpOptDetails(comp,
                       "Cannot unroll loop %d: eq loop with branch not an exit "
                       "out of the loop\n",
                       loop->getNumber());
        return false;
      }
    ...
    if (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) {
      if (piv->getDeltaOnExitEdge() != 1 && piv->getDeltaOnExitEdge() != -1) {
        dumpOptDetails(comp,
                       "Cannot unroll loop %d: eq loop with non unit inc/dec "
                       "not supported\n",
                       loop->getNumber());
        return false;
      }
      ...
      TR::ILOpCodes opCode =
          branch->getFirstChild()->getType().isInt64()
              ? (branch->getOpCode().isUnsignedCompare() ? TR::iflucmpge
                                                         : TR::iflcmpge)
              : (branch->getOpCode().isUnsignedCompare() ? TR::ifiucmpge
                                                         : TR::ificmpge);
      ...
      if (performTransformation(
              comp, "%sCanonicalize branch test %p for eq/ne loop %d\n",
  ...
  if (unrollKind == CompleteUnroll) {
    ...
    if (!loop->contains(destStruct, loop->getParent())) {
      dumpOptDetails(comp,
                     "Cannot unroll loop %d: complete unroll of a loop with a "
                     "reversed branch\n",
                     loop->getNumber());
      return false;
    }
  ...
  if (unroller.isInternalPointerLimitExceeded()) {
    dumpOptDetails(comp,
                   "Cannot unroll loop %d: number of internal pointers has "
                   "been exceeded\n",
                   loop->getNumber());
    return false;
  }
  ...
  if (!performTransformation(comp,
                             "%sUnrolling counted loop %d [unrollfactor:%d, "
                             "peelcount:%d, spill:%s completeunroll:%s]\n",
                             OPT_DETAILS, loop->getNumber(), unrollCount + 1,
                             peelCount,
...
bool TR_LoopUnroller::unroll(TR::Compilation *comp, TR_RegionStructure *loop,
  ...
  TR_StructureSubGraphNode *branchNode = loop->getExitEdges()
                                             .getListHead()
                                             ->getData()
                                             ->getFrom()
  ...
  if (!branchStr) {
    // if (comp->trace(generalLoopUnroller))
    dumpOptDetails(comp,
                   "Cannot unroll loop %d: branchnode %d is not a block\n",
                   loop->getNumber(), branchNode->getNumber());
    return false;
  }

  TR_LoopUnroller unroller(comp, optimizer, loop, branchNode, unrollCount,
                           peelCount, loopInvariantBlock, GeneralUnroll);
  ...
  if (unroller._spillLoopRequired) {
    ...
    if (!backEdgeFound) {
      ...
      if (branchBlock == loop->getEntryBlock()) {
        if ((branchBlock->getLastRealTreeTop() ==
             branchBlock->getFirstRealTreeTop()) ||
            ((branchBlock->getFirstRealTreeTop()->getNode()->getOpCodeValue() ==
              TR::asynccheck) &&
             (branchBlock->getLastRealTreeTop() ==
              branchBlock->getFirstRealTreeTop()->getNextTreeTop())))
          loopCanBeUnrolled = true;
      ...
      if (!loopCanBeUnrolled) {
        if (blocksInLoop.isDoubleton() &&
          ...
          if ((nextBlock->getEntry()->getNextTreeTop() ==
              ...
              (nextBlock->getLastRealTreeTop() ==
               nextBlock->getFirstRealTreeTop()) ||
              ((nextBlock->getFirstRealTreeTop()->getNode()->getOpCodeValue() ==
                TR::asynccheck) &&
               (nextBlock->getLastRealTreeTop() ==
                nextBlock->getFirstRealTreeTop()->getNextTreeTop())))
            loopCanBeUnrolled = true;
      ...
      if (loopCanBeUnrolled) {
        // Either the entry block has the branch condition or the only 2 blocks
        // in the loop are the entry and the branch block Further the only code
        // other from the entry block in the loop must be the branch (and an
  ...
  if (!performTransformation(
          comp,
          "%sUnrolling non-counted loop %d [unrollfactor:%d, peelcount:%d]\n",
...
TR_GeneralLoopUnroller::TR_GeneralLoopUnroller(TR::OptimizationManager *manager)
  ...
  _basicSizeThreshold =
      e ? atoi(e) : (comp()->getOption(TR_ProcessHugeMethods) ? 500 : 82);
};

int32_t TR_GeneralLoopUnroller::perform() {
  if (optimizer()->optsThatCanCreateLoopsDisabled()) return 0;
  ...
  dumpOptDetails(
      comp(),
      "Starting GLU with a budget of %d.  Total number of nodes in method %d\n",
      budget, nodeCount);
  ...
  for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    int32_t weight =
        weighNaturalLoop(loop, unrollKind, unrollCount, peelCount, cost);

    bool loopCanBeUnrolled = true;

    if (weight > 0 && unrollCount > 0 && loopCanBeUnrolled) {
      UnrollInfo *info = new (trStackMemory())
          UnrollInfo(loop, weight, cost, unrollKind, unrollCount, peelCount);
      Q.add(info);
    }
  }
  ...
  while (1) {
    ...
    for (UnrollInfo *i = uit.getFirst(); i; i = uit.getNext()) {
      if (!top || top->_weight < i->_weight) top = i;
    }
    ...
    if (top->_loop->getPrimaryInductionVariable())
      TR_LoopUnroller::unroll(
          comp(), top->_loop, top->_loop->getPrimaryInductionVariable(),
          top->_unrollKind, top->_unrollCount, top->_peelCount, optimizer());
    else
      TR_LoopUnroller::unroll(comp(), top->_loop, top->_unrollCount,
                              top->_peelCount, optimizer());

    if (trace()) traceMsg(comp(), "</unroll>\n");
  }

  return 1;
}

void TR_GeneralLoopUnroller::collectNonColdInnerLoops(
    TR_RegionStructure *region, List<TR_RegionStructure> &innerLoops) {
...
void TR_GeneralLoopUnroller::gatherStatistics(
    TR_Structure *str, int32_t &numNodes, int32_t &numBlocks,
...
void TR_GeneralLoopUnroller::countNodesAndSubscripts(TR::Node *node,
                                                     int32_t &numNodes,
                                                     int32_t &numSubscripts,
                                                     LoopWeightProbe &lwp) {
  if (!node) return;
  if (node->getVisitCount() == comp()->getVisitCount()) return;
  node->setVisitCount(comp()->getVisitCount());

  if (node->getOpCode().isLikeDef()) lwp._numKilled++;
  if (node->getOpCode().isLikeUse()) lwp._numUsed++;
  ...
  if (node->getOpCode().isCall()) {
    ...
    if (auto methodSym = node->getSymbol()->getMethodSymbol()) {
      if (methodSym->isPureFunction()) lwp._numPureFunctions++;

      // TODO: leaf routine?

      // Is it always inlined BIF?
      if (methodSym->getRecognizedMethod() != TR::unknownMethod) lwp._numBIFs++;
    }
  }

  if (node->getOpCodeValue() != TR::treetop) numNodes++;

  if (node->getOpCodeValue() == TR::aiadd ||
      node->getOpCodeValue() == TR::aiuadd ||
      node->getOpCodeValue() == TR::aladd ||
      node->getOpCodeValue() == TR::aluadd) {
  ...
  // recurse children
  for (int32_t child = 0; child < node->getNumChildren(); ++child)
    countNodesAndSubscripts(node->getChild(child), numNodes, numSubscripts,
                            lwp);
}

void TR_GeneralLoopUnroller::profileNonCountedLoops(
    List<TR_RegionStructure> &innerLoops) {
  ListIterator<TR_RegionStructure> it(&innerLoops);
  for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
    if (loop->getPrimaryInductionVariable()) continue;

    TR_StructureSubGraphNode *loopNode =
        loop->getParent()->findSubNodeInRegion(loop->getNumber());
    if (loopNode->getPredecessors().size() != 1) continue;

    if (loop->getExitEdges().getSize() != 1) continue;

    TR::Block *entryBlock = loop->getEntryBlock();
    TR::Node *firstNode = entryBlock->getFirstRealTreeTop()->getNode();
    TR_StructureSubGraphNode *invarNode = loopNode->getPredecessors()
                                              .front()
                                              ->getFrom()
                                              ->asStructureSubGraphNode();

    if (!invarNode->getStructure()->asBlock()) continue;

    TR::Block *invarBlock = invarNode->getStructure()->asBlock()->getBlock();

    if (!performTransformation(comp(),
                               "%sInserting artificial counter in loop %d in "
                               "order to profile the itercount\n",
    ...
    invarBlock->prepend(TR::TreeTop::create(
        comp(),
        TR::Node::createWithSymRef(TR::lstore, 1, 1, zero, counterSymRef)));
    entryBlock->prepend(TR::TreeTop::create(
        comp(), TR::Node::create(
                    TR::lstore, 1,
                    TR::Node::create(
                        TR::ladd, 2,
                        TR::Node::createLoad(firstNode, counterSymRef), one))));

    int32_t postNum =
        loop->getExitEdges().getListHead()->getData()->getTo()->getNumber();
    TR_StructureSubGraphNode *postNode = TR_LoopUnroller::findNodeInHierarchy(
        loop->getParent()->asRegion(), postNum);
    TR::Block *postBlock = postNode->getStructure()->getEntryBlock();
    postBlock->prepend(TR::TreeTop::create(
        comp(),
        TR::Node::createWithSymRef(
            TR::lstore, 1, 1, TR::Node::createLoad(firstNode, counterSymRef),
            counterSymRef)));
    // FIXME: complete - mark the node as needs to be profiled
  }
}

static bool exitsLoop(TR::Compilation *comp, TR_RegionStructure *loop,
                      TR_StructureSubGraphNode *source) {
  for (auto edge = source->getSuccessors().begin();
       edge != source->getSuccessors().end(); ++edge) {
    if (loop->getExitEdges().find(*edge)) return true;
  }
  return false;
}
...
// Returns the weight
int32_t TR_GeneralLoopUnroller::weighNaturalLoop(
    TR_RegionStructure *loop, TR_LoopUnroller::UnrollKind &unrollKind,
  ...
  int32_t weight = 0;
  int32_t entryBlockFrequency = _haveProfilingInfo
                                    ? entryBlock->getFrequency()
                                    : loop->getNestingDepth() * 8;
  if (entryBlockFrequency == 0) entryBlockFrequency = 1;

  dumpOptDetails(comp(), "\nAnalyzing Loop %d with entryBlock %d\n",
                 loop->getNumber(), entryBlock->getNumber());
  dumpOptDetails(comp(),
                 "\tnumNodes = %d, numBlocks = %d, entryBlockFreq = %d, "
                 "numSubscripts = %d\n",
  ...
  if (trace()) {
    dumpOptDetails(
        comp(),
        "\tnumKilled = %u, numUsed = %u, numMatConst = %u, numUnmatConst = %u",
  ...
  if (piv)
    useDefd[piv->getSymRef()->getReferenceNumber()] =
        0;  // Ignore PIV in calculation
  ...
  if (trace()) {
    traceMsg(comp(),
             "numUsed = %i, numUseOnly = %i(min:%i), numUseDefd = %i(max:%i)\n",
  ...
  if (piv) {
    ...
    // the branches are not fixed up correctly. So tentatively unroll as
    // uncounted
    //
    int32_t numBranches =
        loop->getPrimaryInductionVariable()->getNumLoopExits();
    dumpOptDetails(comp(),
                   "loop %d has primary iv and %d legitimate branches\n",
    ...
    if (loop->getPrimaryInductionVariable()) {
      ...
      for (auto e = entryNode->getPredecessors().begin();
        ...
        //
        if (source->getStructure()->asBlock()) {
          TR::Node *branchNode = source->getStructure()
                                     ->asBlock()
                                     ->getBlock()
                                     ->getLastRealTreeTop()
                                     ->getNode();
          /// traceMsg(comp(), "got branchnode = %p\n", branchNode);
          // if the backedge contains a biv (not the primary iv), then unroll
          // conservatively as non-counted
          //
          if (!branchNode->getOpCode().isGoto() &&
              !branchContainsInductionVariable(loop, branchNode)) {
            dumpOptDetails(comp(),
                           "backedge branch [%p] is controlled using a biv, so "
                           "unrolling as non-counted\n",
                           branchNode);
            loop->setPrimaryInductionVariable(NULL);
            break;
          }
  ...
  // If we have profile information available, use that to check the the inner
  // loop is relatively hotter than the outer loop.  Otherwise there is not
  // sufficient benefit in unrolling the inner loop.  When profiling information
  // is not available, we statically assume that the inner loops run more
  // frequently.  High relative freq of the inner loop is going provide a weight
  // boost.
  //
  TR_RegionStructure *outerLoop =
      (TR_RegionStructure *)loop->getContainingLoop();
  if (_haveProfilingInfo && outerLoop) {
    ...
    if (maxCount > 0) {
      ...
      dumpOptDetails(
          comp(),
          "\t outerLoop number = %d outerLoop->getEntryBlock->GetNumber = %d\n",
          outerLoop->getNumber(), outerLoop->getEntryBlock()->getNumber());
      float outerLoopRelativeFrequency =
          outerLoopFrequency == 6
      ...
      {
        dumpOptDetails(comp(),
                       "\trejecting loop because its not warm enough compared "
                       "to the outer loop\n");
        return -1;
      }
  ...
  if (piv) {
    ...
    bool entryValueConst =
        entryValueNode ? entryValueNode->getOpCode().isLoadConst() : false;
    int64_t entryValue =
        entryValueConst ? entryValueNode->get64bitIntegralValue() : 0;
    TR::Node *exitBoundNode = piv->getExitBound();
    bool exitBoundConst =
        exitBoundNode ? exitBoundNode->getOpCode().isLoadConst() : false;
    int64_t exitBound =
        exitBoundConst ? exitBoundNode->get64bitIntegralValue() : 0;
    if (entryValueConst && exitBoundConst) {
      if ((increasingLoop && (entryValue > exitBound)) ||
          (!increasingLoop && (entryValue < exitBound))) {
        dumpOptDetails(
            comp(), "\twraparound, %s, entry value: 0x%x, exit bound: 0x%x\n",
            increasingLoop ? "loop increasing" : "loop decreasing", entryValue,
            exitBound);
        wrapAround = true;
      }
    }
    ...
    if (iterCount > 0 && !wrapAround) {
      dumpOptDetails(
          comp(),
          "\twe have a primary induction variable #%i with itercount %d\n",
          piv->getSymRef()->getReferenceNumber(), iterCount);

      if (iterCount <= maxIterCountForCompleteUnroll &&
          ((iterCount * numNodes <= maxSizeForSingleBlockCompleteUnroll &&
            numBlocks == 1) ||
        ...
        bool multipleOfPreferred = preferredUnrollFactor > 0 &&
                                   (iterCount % preferredUnrollFactor) == 0;
        ...
        if (!multipleOfPreferred && !multipleOf2 && !multipleOf3) {
          ...
          if (comp()->getOption(TR_Randomize)) {
            unrollCount =
                (multipleOf4 ? 3 : multipleOf3 ? 2 : multipleOf2 ? 1 : 0);

            /*unrollCount = (multipleOf4 ? randomInt(0,3) :
                            multipleOf3 ? randomInt(0,2) :
                            multipleOf2 ? randomInt(0,1) : 0);
                   traceMsg(comp(), "\nTR_Randomize
               Enabled||TR_GeneralLoopUnroller::weighNaturalLoop(),
               unrollCount:%d Original unrollCount:%d", unrollCount,
               originalUnrollCount);*/
          } else {
            unrollCount =
                (multipleOfPreferred
                     ? preferredUnrollFactor - 1
                     : multipleOf4 ? 3 : multipleOf3 ? 2 : multipleOf2 ? 1 : 0);
          }
          if (unrollCount > 0) {
            unrollKind = TR_LoopUnroller::ExactUnroll;

            if ((unrollCount < 3) && (comp()->getMethodHotness() >= hot) &&
                (iterCount >= 64)) {
    ...
    if (iterCount <= 0 || unrollKind == TR_LoopUnroller::NoUnroll) {
      if (iterCount <= 0)
        dumpOptDetails(comp(),
                       "\twe have a primary induction variable with unknown "
                       "itercount stride=%d\n",
                       piv->getDeltaOnBackEdge());

      // Default 4x unrolling unless codegen specifies a factor > 0
      const int32_t preferredUnrollFactor =
          cg()->getPreferredLoopUnrollFactor();
      const int32_t unrollFactor =
          preferredUnrollFactor > 0 ? preferredUnrollFactor : 4;
    ...
    if (unrollKind != TR_LoopUnroller::CompleteUnroll &&
      ...
      dumpOptDetails(
          comp(),
          "\tloop unroll size threshold hit: %d using an unroll count of %d\n",
          growth, unrollCount);
      return -1;
    }

    if ((unrollKind == TR_LoopUnroller::ExactUnroll) &&
        (iterCount == (unrollCount + 1))) {
      // eliminate the back edge from an exact unroll where ic == (uc + 1)
      boostFactor = 400;
      unrollKind = TR_LoopUnroller::CompleteUnroll;
      dumpOptDetails(comp(),
                     "\tloop unroll count %d iterCount %d, making complete "
                     "unroll out of exact unroll\n",
  ...
  } else  // of if piv
  {
    ...
    if (_haveProfilingInfo) {
      // We must have enough profile information to be confident enough to
      // unroll this loop.  Current simple heuristic:
      //
      // int32_t maxCount = comp()->getFlowGraph()->getMaxFrequency();
      // if (maxCount >= 100 && entryBlockFrequency * 6 > maxCount)

      if (canUnrollUnCountedLoop(loop, numBlocks, numNodes,
                                 entryBlockFrequency)) {
        int32_t hotnessFactor = 1;
        if ((comp()->getMethodHotness() == scorching) &&
            (entryBlockFrequency >= HIGH_FREQ_LOOP_CUTOFF))
          hotnessFactor = 4;
  ...
  // Unrolling is going to amortize the cost of the async check (if the loop has
  // one)
  //
  if (!loop->getEntryBlock()->getStructureOf()->isEntryOfShortRunningLoop())
    weight += (unrollCount * 100) / numNodes;

  // Presence of any sequential memory access patterns inside the loop gives a
  // boost to the loop as well, since they could overlap.
  // TODO: Need to tweak this to only detect such pattern
  weight += 150 * numSubscripts;

  // The hottest loops in scorching should be favoured regardless of what they
  // contain
  //
  if ((comp()->getMethodHotness() == scorching) &&
      (loop->getEntryBlock()->getFrequency() >= HIGH_FREQ_LOOP_CUTOFF))
    weight += 300;
  else {
    // By unrolling, we increase the number of branches inside the loop which is
    // going to limit local optimizations and have an impact on the B2B.
    //
  ...
  TR::RecognizedMethod methodId =
      comp()->getMethodSymbol()->getRecognizedMethod();
  if (cg()->getSizeOfCombinedBuffer()) {
    ...
    if (forceUnrollCount) unrollCount = 7;
  }
...
bool TR_GeneralLoopUnroller::canUnrollUnCountedLoop(
    TR_RegionStructure *loop, int32_t numBlocks, int32_t numNodes,
    int32_t entryBlockFrequency) {
  if (comp()->getOption(TR_DisableUncountedUnrolls)) return false;
  ...
  //   [0x37047ed0]       aload #392[0x36e8c8e4]  Auto[<temp slot 8>]
  //   <flags:"0x4" (X!=0 )/> [0x375191d4]   NULLCHK on [0x37047ef8]
  //   #18[0x36a24b1c]  Method[jitThrowNullPointerException] [0x375191fc] iaload
  //   #549[0x37047c9c]+24
    ...
    for (b = bilIt.getCurrent(); b; b = bilIt.getNext()) {
      ...
      for (TR::TreeTop *tt = entry; tt != exit; tt = tt->getNextRealTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::NULLCHK &&
          ...
          // Found a NULLCHK node, lets search to see if we have a compare with
          // NIL for the NULLCHK's first child
          for (TR::TreeTop *tt1 = tt; tt1 != exit;
               tt1 = tt1->getNextRealTreeTop()) {
            ...
            if (n->getOpCode().isBooleanCompare() &&
              ...
              if (trace()) {
                traceMsg(comp(),
                         "\tLoop %d can be unrolled because of common NULLCHK "
                         "and compare to NIL\n",
                         loop->getNumber());
              }
...
bool TR_GeneralLoopUnroller::branchContainsInductionVariable(
  ...
  /// ListIterator<TR_BasicInductionVariable>
  /// it(&loop->getBasicInductionVariables());
  bool result = false;
...
bool TR_GeneralLoopUnroller::branchContainsInductionVariable(
    TR::Node *node, TR::SymbolReference *ivSymRef) {
  if (node->getOpCode().hasSymbolReference() &&
...
const char *TR_GeneralLoopUnroller::optDetailString() const throw() {
