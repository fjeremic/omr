...
static bool isBiv(TR_RegionStructure* loop,
              ...
              if (canMatch || canMatchAsIP) {
                if (trace())
                  traceMsg(comp(),
                           ...
                           secondChild);
                IntrnPtr* intrnPtr = (IntrnPtr*)trMemory()->allocateStackMemory(
                    sizeof(IntrnPtr));
                intrnPtr->symRefNum =
...
bool TR_LoopUnroller::cfgEdgeAlreadyExists(TR::Block* from,
                                           TR::Block* to,
                                           EdgeContext edgeContext) {
...
void TR_LoopUnroller::fixExitEdges(TR_Structure* s,
                                   TR_Structure* clone,
                                   TR_StructureSubGraphNode* branchNode) {
  if (s->asBlock())
    return;
...
// ::unrollCountedLoop
bool TR_LoopUnroller::unroll(TR::Compilation* comp,
                             TR_RegionStructure* loop,
                             TR_PrimaryInductionVariable* piv,
                             UnrollKind unrollKind,
                             int32_t unrollCount,
...
void TR_GeneralLoopUnroller::gatherStatistics(TR_Structure* str,
                                              int32_t& numNodes,
                                              int32_t& numBlocks,
                                              int32_t& numBranches,
