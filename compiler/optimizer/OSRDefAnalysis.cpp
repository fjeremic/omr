...
*/
    ...
    for (treeTop = comp()->getStartTree(); treeTop != NULL; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (isPotentialOSRPoint && (comp()->isOSRTransitionTarget(TR::preExecutionOSR) || comp()->requiresAnalysisOSRPoint(node))) {
...
void TR_OSRDefInfo::buildOSRDefs(TR::Node* node, void* vanalysisInfo, TR_OSRPoint* osrPoint, TR_OSRPoint* osrPoint2, TR::Node* parent, AuxiliaryData& aux)
{
    ...
    if (expandedNodeIndex != NULL_USEDEF_SYMBOL_INDEX && expandedNodeIndex != 0) {
        ...
        if (!defsForSymbol->isEmpty() && isExpandedDefIndex(expandedNodeIndex) && !sym->isRegularShadow() && !sym->isMethod()) {
            if (trace()) {
...
void TR_OSRLiveRangeAnalysis::intersectWithExistingDeadSlots(TR_OSRPoint* osrPoint, TR_BitVector* deadPPSSlots, TR_BitVector* deadAutoSlots, bool containsPendingPushes)
{
    TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
...

    {
        ...
        for (i = 0; i < numSlots; i++) {
            ...
            for (nextSymRef = symRefsIt.getCurrent(); nextSymRef; nextSymRef = symRefsIt.getNext()) {
                ...
                else if (nextSymRef->getSymbol()->isParm() && !nextSymRef->getSymbol()->castToParmSymbol()->isLiveLocalIndexUninitialized())
        ...
        while (block) {
            ...
            for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
                ...
                if (comp()->isOSRTransitionTarget(TR::postExecutionOSR) && comp()->getMethodSymbol()->isOSRRelatedNode(tt->getNode())) {
    ...
    TR::Block* block)
{
    ...
    if (node->getOpCode().isStoreDirect()) {
        ...
        if (local && !local->isLiveLocalIndexUninitialized()) {
            ...
            if ((!disallowOSRPPS3 || !_pendingPushSymRefs->get(node->getSymbolReference()->getReferenceNumber())) && ((node->getLocalIndex() == 1 || node->getOpCodeValue() == TR::loadaddr) && !liveVars->isSet(localIndex))) {
...
void TR_OSRExceptionEdgeRemoval::removeDeadStores(TR::Block* osrBlock, TR_BitVector& dead)
{
    ...
    for (TR::TreeTop* tt = osrBlock->getFirstRealTreeTop(); tt != osrBlock->getExit(); tt = tt->getNextTreeTop()) {
        TR::Node* node = tt->getNode();
        if (node->getOpCode().isStoreDirect() && node->getSymbol()->isAutoOrParm() && node->storedValueIsIrrelevant() && dead.get(node->getSymbolReference()->getReferenceNumber())) {
            if (comp()->getOption(TR_TraceOSR))
...
int32_t TR_OSRExceptionEdgeRemoval::perform()
{
    ...
    if (!disableDeadStoreCleanup) {
        ...
        for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
            ...
            while (tt && !((tt->getNode()->getOpCode().isCheck() || tt->getNode()->getOpCodeValue() == TR::treetop) && tt->getNode()->getFirstChild()->getOpCode().isCall() && tt->getNode()->getFirstChild()->getSymbolReference()->getReferenceNumber() == TR_prepareForOSR)) {
                tt = tt->getNextTreeTop();
            }
