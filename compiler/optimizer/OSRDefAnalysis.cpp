      ...
      while (cursor.hasMoreElements()) {
        ...
        if (methodSymbol->sharesStackSlot(defSymRef)) {
          ...
          comp()->getOSRCompilationData()->addSlotSharingInfo(
            point->getByteCodeInfo(), slot, symRefNum, symRefOrder, defSymRef->getSymbol()->getSize(), takesTwoSlots);
          if (trace()) {
...
bool TR_OSRLiveRangeAnalysis::shouldPerformAnalysis()
{
  if (!comp()->getOption(TR_EnableOSR)) {
    ...
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(
        comp(), "Should not perform OSRLiveRangeAnlysis -- OSR is not supported under the current configuration\n");
    return false;
  ...
  if (comp()->getOption(TR_DisableOSRLiveRangeAnalysis)) {
    if (comp()->getOption(TR_TraceOSR)) {
      if (!_sharedSymRefs->isEmpty())
        traceMsg(comp(),
          "OSRLiveRangeAnalysis is disabled but it is required for correctness. OSR transitions may not work.\n");
      else
...
int32_t TR_OSRLiveRangeAnalysis::partialAnalysis()
{
  ...
  TR_ASSERT(
    comp()->pendingPushLivenessDuringIlgen(), "Partial analysis is only possible if liveness has been solved in Ilgen");
    ...
    while (bvi.hasMoreElements()) {
      ...
      for (TR::SymbolReference* slotSymRef = listIt.getFirst(); slotSymRef;
        ...
        if (symRef == slotSymRef)
          break;
      TR_ASSERT(
        symRefOrder < list->getSize(), "symref #%d not found in list of shared slots\n", symRef->getReferenceNumber());

      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(), "  Slot:%d SymRef:%d TwoSlots:%d\n", slot, symRefNum, takesTwoSlots);

      comp()->getOSRCompilationData()->addSlotSharingInfo(
        osrPoint->getByteCodeInfo(), slot, symRefNum, symRefOrder, symRef->getSymbol()->getSize(), takesTwoSlots);
    }
  }
  ...
  TR::Node* node, TR_BitVector* livePendingPushSymRefs, TR_OSRPoint* osrPoint)
{
  ...
  if (livePendingPushSymRefs && !livePendingPushSymRefs->isEmpty()) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace())
        traceMsg(
          comp(), "pending push slot %d is live with symref %d takesTwoSlots %d\n", slot, symRefNum, takesTwoSlots);
  ...
  if (deadPPSSlots && !deadPPSSlots->isEmpty()) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      comp()->getOSRCompilationData()->addSlotSharingInfo(osrPoint->getByteCodeInfo(), slot, -1 /* symRefNum */,
        -1 /* symRefOrder */, TR::Compiler->om.sizeofReferenceAddress() /*symSize*/, false /*takesTwoSlots*/);
    }
  }
      ...
      if (slotInfo.symRefOrder == -1) {
        ...
        if (slotInfo.slot < 0 && containsPendingPushes) {
          TR_ASSERT(deadPPSSlots,
            "with slotInfo.slot < 0  and containsPendingPushes, there must be at least on pending push symbol in this "
            "method");
        ...
        if (slotInfo.slot >= 0) {
          TR_ASSERT_FATAL(
            deadAutoSlots, "with slotInfos.slot >= 0, there must be at least on auto symbol in this method");
    ...
    if (containsPendingPushes) {
      for (i = 0; i < numPendingSlots; i++) {
        ...
        for (nextSymRef = symRefsIt.getCurrent(); nextSymRef; nextSymRef = symRefsIt.getNext()) {
          if (nextSymRef->getSymbol()->isAuto()
            && !nextSymRef->getSymbol()->castToAutoSymbol()->isLiveLocalIndexUninitialized()) {
            liveLocalIndexToSymRefNumberMap[nextSymRef->getSymbol()->castToAutoSymbol()->getLiveLocalIndex()]
    ...
    while (block) {
      ...
      for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
        ...
        if (isPotentialOSRPoint && comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
          ...
          if (!_sharedSymRefs->isEmpty())
            buildOSRSlotSharingInfo(node, _liveVars, offsetOSRPoint, liveLocalIndexToSymRefNumberMap, _sharedSymRefs);
        }
        ...
        if ((isPotentialOSRPoint ||
              ...
              // that is not an OSRPoint but another treetop with the same bci is an OSRPoint. Need to call
              // buildOSRLiveRangeInfo in this case for the first evaluation point to make sure the liveness info is
              // bookkept correctly for this bci
          ...
          buildOSRLiveRangeInfo(
            node, _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap, numBits, osrMethodData, containsPendingPushes);
          if (!_sharedSymRefs->isEmpty())
            buildOSRSlotSharingInfo(node, _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap, _sharedSymRefs);
          if (comp()->getOption(TR_FullSpeedDebug))
      ...
      if (comp()->getStartTree()->getNode()->getBlock() == block && comp()->isOutermostMethod()
        ...
        if (!_sharedSymRefs->isEmpty())
          buildOSRSlotSharingInfo(
            comp()->getStartTree()->getNode(), _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap, _sharedSymRefs);
      }
  ...
  int32_t* liveLocalIndexToSymRefNumberMap, TR_BitVector* sharedSymRefs)
{
  ...
  if (!liveVars->isEmpty()) {
    ...
    if (trace())
      traceMsg(
        comp(), "Shared slots at OSR point [%p] at %d:%d\n", node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
...
int32_t TR_OSRExceptionEdgeRemoval::perform()
{
  ...
  if (comp()->getOSRMode() == TR::involuntaryOSR) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
        "Involuntary OSR enabled -- returning from OSR exception edge removal analysis since we implement involuntary "
        "OSR with exception edges.\n");
    return 0;
  }
    ...
    for (auto edge = block->getExceptionSuccessors().begin(); edge != block->getExceptionSuccessors().end();) {
      ...
      if (catchBlock->isOSRCatchBlock()
        && performTransformation(comp(),
          ...
          block->getNumber(), block, catchBlock->getNumber(), catchBlock))
      ...
      for (auto edge = block->getPredecessors().begin(); edge != block->getPredecessors().end(); ++edge) {
        ...
        if (osrBlock->isOSRCodeBlock() && addDeadStores(osrBlock, alwaysDead, !firstPass))
          ...
          for (auto exEdge = osrBlock->getExceptionPredecessors().begin();
            ...
            TR_ASSERT(
              exBlock && exBlock->isOSRInduceBlock(), "Predecessors to an OSR catch block should be OSR induce blocks");
      ...
      for (int32_t i = 0; i < prepareForOSR->getNumChildren(); ++i) {
        ...
        if (child->getOpCode().isLoadVar() && alwaysDead.get(child->getSymbolReference()->getReferenceNumber())) {
          if (performTransformation(comp(), "%s: Remove dead OSR load [%p] from OSR code block_%d\n", OPT_DETAILS,
                child, block->getNumber())) {
            TR::Node::recreate(
              prepareForOSR->getChild(i), comp()->il.opCodeForConst(prepareForOSR->getChild(i)->getDataType()));
