...
int32_t TR_OSRDefAnalysis::perform() {
  if (comp()->getOption(TR_EnableOSR)) {
    ...
    if (trace())
      traceMsg(comp(), "Options is not enabled -- returning from OSR reaching "
...
TR_OSRLiveRangeAnalysis::TR_OSRLiveRangeAnalysis(
    ...
    : TR::Optimization(manager), _liveVars(NULL), _pendingPushSymRefs(NULL),
      _sharedSymRefs(NULL), _workBitVector(NULL), _workDeadSymRefs(NULL),
      _visitedBCI(NULL) {}

bool TR_OSRLiveRangeAnalysis::shouldPerformAnalysis() {
  if (!comp()->getOption(TR_EnableOSR)) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "Should not perform OSRLiveRangeAnalysis -- OSR Option "
                       "not enabled\n");
    return false;
  } else if (comp()->isPeekingMethod()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "Should not perform OSRLiveRangeAnalysis -- Not "
                       "required because we are peeking\n");
    return false;
  } else if (!comp()->supportsInduceOSR()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "Should not perform OSRLiveRangeAnlysis -- OSR is not "
                       "supported under the current configuration\n");
    return false;
    ...
    if (comp()->getOption(TR_MimicInterpreterFrameShape)) {
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(), "No need to perform OSRLiveRangeAnlysis under mimic "
                         "interpreter frame shape\n");
      return false;
    } else if (disableOSRPointDeadslotsBookKeeping) // save some compile time
    {
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(), "Dead slots bookkeeping is disabled and therefore "
  ...
  if (comp()->getOption(TR_DisableOSRLiveRangeAnalysis)) {
    if (comp()->getOption(TR_TraceOSR)) {
      if (!_sharedSymRefs->isEmpty())
        traceMsg(comp(), "OSRLiveRangeAnalysis is disabled but it is required "
                         "for correctness. OSR transitions may not work.\n");
      else
...
int32_t TR_OSRExceptionEdgeRemoval::perform() {
  if (comp()->getOption(TR_EnableOSR)) {
    ...
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "OSR Option not enabled -- returning from OSR exception "
  ...
  if (comp()->getOSRMode() == TR::involuntaryOSR) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "Involuntary OSR enabled -- returning from OSR "
                       "exception edge removal analysis since we implement "
  ...
  if (comp()->osrInfrastructureRemoved()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(), "OSR infrastructure removed -- returning from OSR "
                       "exception edge removal analysis since we have already "
  ...
  if (!disableDeadStoreCleanup) {
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
      ...
      TR_ASSERT(tt, "OSRCodeBlocks must contain a call to prepareForOSR and no "
                    "other calls");
