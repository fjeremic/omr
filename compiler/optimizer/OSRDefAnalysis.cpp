...
TR_OSRDefInfo::TR_OSRDefInfo(TR::OptimizationManager *manager)
    : TR_UseDefInfo(manager->comp(),
                    manager->optimizer()->getMethodSymbol()->getFlowGraph(),
                    manager->optimizer(), false /* !requiresGlobals */,
                    false /* !prefersGlobals */, false /* !loadsShouldBeDefs */,
                    false /* !cannotOmitTrivialDefs */,
                    false /* !conversionRegsOnly */,
                    false /* !doCompletion */) {
  // need to call prepareUseDefInfo here not in base constructor. If called in
  // base constructor, base class instances of virtual functions are called,
  // because the derived object would not yet have been set up. ie
  // prepareUseDefInfo will call base function
  // TR_UseDefInfo::prepareAnalysisback NOT derived function
  // TR_OSRDefInfo:prepareAnalysis
  prepareUseDefInfo(false /* !requiresGlobals */, false /* !prefersGlobals */,
                    false /* !cannotOmitTrivialDefs */,
                    false /* !conversionRegsOnly */);
}
...
void TR_OSRDefInfo::performFurtherAnalysis(AuxiliaryData &aux) {
  if (!infoIsValid()) {
    // TR_ASSERT(0, "osr def analysis failed. need to undo whatever we did for
    // OSR and disable OSR but we don't. Implementation is not completed.\n");
    traceMsg(comp(),
             "compilation failed for %s because osr def analysis failed\n",
             optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
    comp()->failCompilation<TR::ILGenFailure>(
        "compilation failed because osr def analysis failed");
  }

  // Iterate through OSR reaching definitions bit vectors and save it in method
  ...
  for (intptrj_t i = 0; i < methodSymbol->getOSRPoints().size(); ++i) {
    ...
    // one reason that info can be NULL is that the block that contains that
    // i-th osr point has been deleted (e.g., because it was unreachable), and
    // therefore, we have never set _defsForOSR[i] to a non-NULL value
    if (info) {
      ...
      while (cursor.hasMoreElements()) {
        ...
        // Only interested in parameters, autos and pending pushes that can be
        // shared
        ...
        if (methodSymbol->sharesStackSlot(defSymRef)) {
          ...
          for (TR::SymbolReference *symRef = listIt.getFirst(); symRef;
            ...
            if (symRef == defSymRef) break;
          TR_ASSERT(symRefOrder < list->getSize(),
                    "symref #%d on node n%dn not found\n",
                    defSymRef->getReferenceNumber(), defNode->getGlobalIndex());
          comp()->getOSRCompilationData()->addSlotSharingInfo(
              point->getByteCodeInfo(), slot, symRefNum, symRefOrder,
              defSymRef->getSymbol()->getSize(), takesTwoSlots);
          if (trace()) {
            TR_ByteCodeInfo &bcInfo = point->getByteCodeInfo();
            traceMsg(comp(),
                     "added (callerIndex=%d, bcIndex=%d)->(slot=%d, ref#=%d) "
                     "at OSR point %d side %d def %d\n",
                     bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex(), slot,
                     symRefNum, i, j, jj);
          }
        }
      }
    }
    comp()->getOSRCompilationData()->ensureSlotSharingInfoAt(
        point->getByteCodeInfo());
  }
}

void TR_OSRDefInfo::addSharingInfo(AuxiliaryData &aux) {
  // For each slot shared by auto/parm/pps, let S be the set of symbols that are
  // mapped to that slot. we compute the union of the def bit-vectors of the
  // symbols in S and assign it to the def bit-vector of every symbol in S.

  TR_Array<List<TR::SymbolReference>> *ppsListArray =
      _methodSymbol->getPendingPushSymRefs();
  TR_BitVector *prevTwoSlotUnionDef =
      new (comp()->trMemory()->currentStackRegion()) TR_BitVector(
          getBitVectorSize(), comp()->trMemory()->currentStackRegion());
  TR_BitVector *twoSlotUnionDef =
      new (comp()->trMemory()->currentStackRegion()) TR_BitVector(
          getBitVectorSize(), comp()->trMemory()->currentStackRegion());
  TR_BitVector unionDef(getBitVectorSize(),
  ...
  for (int i = 0; ppsListArray && i < ppsListArray->size(); ++i) {
    ...
    if ((ppsList.getSize() > 1) || isTwoSlotSymRefAtThisSlot ||
      ...
      for (TR::SymbolReference *symRef = ppsIt.getFirst(); symRef;
        ...
        TR::DataType dt = symRef->getSymbol()->getDataType();
        bool takesTwoSlots = dt == TR::Int64 || dt == TR::Double;
        if (takesTwoSlots) *twoSlotUnionDef |= *defs;
      }
      ...
      for (TR::SymbolReference *symRef = ppsIt.getFirst(); symRef;
        ...
        *aux._defsForSymbol[symIndex] = unionDef;
        ...
        if (!prevTwoSlotUnionDef->isEmpty()) {
          ...
          for (TR::SymbolReference *prevSymRef = prevppsIt.getFirst();
               prevSymRef; prevSymRef = prevppsIt.getNext()) {
            TR::DataType prevdt = prevSymRef->getSymbol()->getDataType();
            bool doesPrevTakesTwoSlots =
                prevdt == TR::Int64 || prevdt == TR::Double;
  ...
  for (int i = 0; autosListArray && i < autosListArray->size(); ++i) {
    ...
    if ((autosList.getSize() > 1) || isTwoSlotSymRefAtThisSlot ||
        isTwoSlotSymRefAtPrevSlot) {
      if (!autosIt.getFirst() || (autosIt.getFirst()->getCPIndex() >=
                                  _methodSymbol->getFirstJitTempIndex()))
        continue;
      unionDef.empty();
      twoSlotUnionDef->empty();
      ...
      for (TR::SymbolReference *symRef = autosIt.getFirst(); symRef;
        ...
        *aux._defsForSymbol[symIndex] = unionDef;
        ...
        if (!prevTwoSlotUnionDef->isEmpty()) {
          ...
          for (TR::SymbolReference *prevSymRef = prevautosIt.getFirst();
               prevSymRef; prevSymRef = prevautosIt.getNext()) {
            TR::DataType prevdt = prevSymRef->getSymbol()->getDataType();
            bool doesPrevTakesTwoSlots =
                prevdt == TR::Int64 || prevdt == TR::Double;
            if (doesPrevTakesTwoSlots) {
...
void TR_OSRDefInfo::processReachingDefinition(void *vblockInfo,
                                              AuxiliaryData &aux) {
...
void TR_OSRDefInfo::buildOSRDefs(void *vblockInfo, AuxiliaryData &aux) {
  /*
     for (treeTop = comp()->getStartTree(); treeTop != NULL; treeTop =
  ...
  */
  if (trace()) traceMsg(comp(), "Just before buildOSRDefs\n");

  // Allocate the array of bit vectors that will represent live definitions at
  // OSR points
  //
  ...
  TR::Block *block;
  TR::TreeTop *treeTop;
  TR_ReachingDefinitions::ContainerType **blockInfo =
      (TR_ReachingDefinitions::ContainerType **)vblockInfo;
  ...
  if (comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr) {
    ...
    buildOSRDefs(
        ...
        blockInfo[comp()->getStartTree()->getNode()->getBlock()->getNumber()],
        NULL, nextOsrPoint, NULL, aux);
    nextOsrPoint = NULL;
  }

  for (treeTop = comp()->getStartTree(); treeTop != NULL;
       treeTop = treeTop->getNextTreeTop()) {
    TR::Node *node = treeTop->getNode();

    if (node->getOpCodeValue() == TR::BBStart) {
      block = node->getBlock();
      if (blockInfo) analysisInfo = blockInfo[block->getNumber()];
      continue;
    }
    ...
    if (isPotentialOSRPoint &&
        (comp()->isOSRTransitionTarget(TR::preExecutionOSR) ||
         comp()->requiresAnalysisOSRPoint(node))) {
      osrPoint = _methodSymbol->findOSRPoint(node->getByteCodeInfo());
      TR_ASSERT(osrPoint != NULL, "Cannot find a pre OSR point for node %p",
                node);
    }
    ...
    if (isPotentialOSRPoint &&
      ...
      // Skip to the end of the OSR region, processing all treetops along the
      // way
      TR::TreeTop *pps = treeTop->getNextTreeTop();
      TR_ByteCodeInfo bci =
          _methodSymbol->getOSRByteCodeInfo(treeTop->getNode());
      ...
      // If we require a induction point after the OSR point, store the OSR
      // point to be processed on the next call to buildOSRDefs
      bci.setByteCodeIndex(bci.getByteCodeIndex() +
                           comp()->getOSRInductionOffset(node));
      nextOsrPoint = _methodSymbol->findOSRPoint(bci);
      TR_ASSERT(nextOsrPoint != NULL,
                "Cannot find a post OSR point for node %p", node);
    }
  }
  ...
  for (int i = 0; i < numOSRPoints; ++i) {
    ...
    if (trace()) {
      ...
      TR_ByteCodeInfo &bcinfo =
          _methodSymbol->getOSRPoints()[i]->getByteCodeInfo();
...
void TR_OSRDefInfo::buildOSRDefs(TR::Node *node, void *vanalysisInfo,
                                 TR_OSRPoint *osrPoint, TR_OSRPoint *osrPoint2,
                                 TR::Node *parent, AuxiliaryData &aux) {
  vcount_t visitCount = comp()->getVisitCount();
  if (node->getVisitCount() == visitCount) return;
  node->setVisitCount(visitCount);
  TR_ReachingDefinitions::ContainerType *analysisInfo =
      (TR_ReachingDefinitions::ContainerType *)vanalysisInfo;

  // Look in the children first.
  //
  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++) {
    buildOSRDefs(node->getChild(i), analysisInfo, osrPoint, osrPoint2, node,
                 aux);
  }
  ...
  if (expandedNodeIndex != NULL_USEDEF_SYMBOL_INDEX && expandedNodeIndex != 0) {
    ...
    if (!defsForSymbol->isEmpty() && isExpandedDefIndex(expandedNodeIndex) &&
        !sym->isRegularShadow() && !sym->isMethod()) {
      if (trace()) {
...
int32_t TR_OSRDefAnalysis::perform() {
  if (comp()->getOption(TR_EnableOSR)) {
    ...
    if (!comp()->canAffordOSRControlFlow()) {
      if (trace())
        traceMsg(comp(),
                 "OSR is enabled but no longer in use for this compilation.\n");
      return 0;
    }
  } else {
    if (trace())
      traceMsg(comp(),
               "Options is not enabled -- returning from OSR reaching "
  ...
  if (!requiresAnalysis()) {
    if (trace()) {
      traceMsg(comp(),
               "%s OSR reaching definitions analysis is not required because "
               "there is no sharing\n",
               optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
    ...
    if (trace()) {
      traceMsg(comp(),
               "%s OSR reaching definition analysis is not required because we "
               "are peeking\n",
               optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
      traceMsg(comp(), "Returning...\n");
    }
    ...
    if (osrMethodData->hasSlotSharingOrDeadSlotsInfo()) {
      if (trace()) {
        traceMsg(comp(),
                 "%s OSR reaching definition analysis is not required as it "
  ...
  TR_Structure *rootStructure =
      TR_RegionAnalysis::getRegions(comp(), optimizer()->getMethodSymbol());
  ...
  if (trace()) {
    ...
    comp()->dumpMethodTrees("Before OSR reaching definitions analysis",
  ...
  // set the structure to NULL so that the inliner (which is applied very soon
  // after) doesn't need update it.
...
const char *TR_OSRDefAnalysis::optDetailString() const throw() {
  return "O^O OSR DEF ANALYSIS: ";
}

// Not needed anymore. I'll keep it commented out just in case it's needed in
// the future. Check whether the first real non-profiling tree top in the block
...
symRefTab->findOrCreateRuntimeHelper(TR_prepareForOSR, false, false, true);
   ...
   for (TR::TreeTop* t = osrCodeBlock->getFirstRealTreeTop(); t; t =
t->getNextTreeTop())
      {
      TR::Node* n = t->getNode();
      ...
      if (n->getOpCodeValue() == TR::call && n->getSymbolReference() ==
osrHelper)
         {
         ...
         {
         //TR_ASSERT(false, "first treetop in OSR code block %x is not an OSR
helper call", osrBlock);
         }
      }
   return callTree;
   }
...
TR_OSRLiveRangeAnalysis::TR_OSRLiveRangeAnalysis(
...
bool TR_OSRLiveRangeAnalysis::shouldPerformAnalysis() {
  if (!comp()->getOption(TR_EnableOSR)) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
               "Should not perform OSRLiveRangeAnalysis -- OSR Option not "
               "enabled\n");
    return false;
  } else if (comp()->isPeekingMethod()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
               "Should not perform OSRLiveRangeAnalysis -- Not required "
               "because we are peeking\n");
    return false;
  } else if (!comp()->supportsInduceOSR()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
               "Should not perform OSRLiveRangeAnlysis -- OSR is not supported "
               "under the current configuration\n");
    return false;
  } else if (comp()->getOSRMode() == TR::involuntaryOSR) {
    static const char *disableOSRPointDeadslotsBookKeeping =
        feGetEnv("TR_DisableOSRPointDeadslotsBookKeeping");
    if (comp()->getOption(TR_MimicInterpreterFrameShape)) {
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "No need to perform OSRLiveRangeAnlysis under mimic "
                 "interpreter frame shape\n");
      return false;
    } else if (disableOSRPointDeadslotsBookKeeping)  // save some compile time
    {
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "Dead slots bookkeeping is disabled and therefore "
                 "OSRLiveRangeAnlysis is not needed\n");
      return false;
    }
  }
...
int32_t TR_OSRLiveRangeAnalysis::perform() {
  ...
  if (!shouldPerformAnalysis()) return 0;
  ...
  if (comp()->getOSRMode() == TR::involuntaryOSR && autosListArray) {
    TR_BitVector *symRefs =
        new (trHeapMemory()) TR_BitVector(0, trMemory(), heapAlloc);
  ...
  for (uint32_t i = 0; autosListArray && i < autosListArray->size(); ++i) {
    ...
    for (TR::SymbolReference *symRef = autosIt.getFirst(); symRef;
      ...
      if (comp()->getOSRMode() == TR::involuntaryOSR &&
          osrMethodData->getSymRefs())
        osrMethodData->getSymRefs()->set(symRef->getReferenceNumber());
    }
  }

  TR_Array<List<TR::SymbolReference>> *pendingPushSymRefs =
      comp()->getMethodSymbol()->getPendingPushSymRefs();
  for (int32_t i = 0; pendingPushSymRefs && i < pendingPushSymRefs->size();
       ++i) {
    ...
    for (TR::SymbolReference *symRef = symRefsIt.getCurrent(); symRef;
         symRef = symRefsIt.getNext()) {
  ...
  if (comp()->getOption(TR_DisableOSRLiveRangeAnalysis)) {
    if (comp()->getOption(TR_TraceOSR)) {
      if (!_sharedSymRefs->isEmpty())
        traceMsg(comp(),
                 "OSRLiveRangeAnalysis is disabled but it is required for "
                 "correctness. OSR transitions may not work.\n");
      else
...
int32_t TR_OSRLiveRangeAnalysis::partialAnalysis() {
  ...
  TR_ASSERT(
      comp()->pendingPushLivenessDuringIlgen(),
  ...
  for (TR::TreeTop *tt = comp()->getMethodSymbol()->getLastTreeTop(); tt;
    ...
    if (isPotentialOSRPoint &&
      ...
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "Analysing post OSR point for n%dn at %d:%d offset by %d\n",
                 node->getGlobalIndex(), bci.getCallerIndex(),
                 bci.getByteCodeIndex(), comp()->getOSRInductionOffset(node));

      TR_ByteCodeInfo bcInfo = bci;
      bcInfo.setByteCodeIndex(bcInfo.getByteCodeIndex() +
                              comp()->getOSRInductionOffset(node));
      TR_OSRPoint *offsetOSRPoint =
          comp()->getMethodSymbol()->findOSRPoint(bcInfo);
      TR_ASSERT(offsetOSRPoint != NULL,
                "Cannot find a post OSR point for node %p", node);

      TR_BitVector *ppInfo =
          osrMethodData->getPendingPushLivenessInfo(bcInfo.getByteCodeIndex());
      if (comp()->getOption(TR_TraceOSR)) {
      ...
      pendingPushLiveRangeInfo(node, ppInfo, _pendingPushSymRefs,
                               offsetOSRPoint, osrMethodData);
    ...
    if (isPotentialOSRPoint &&
      ...
      TR_BitVector *ppInfo =
          osrMethodData->getPendingPushLivenessInfo(bci.getByteCodeIndex());
      if (comp()->getOption(TR_TraceOSR)) {
        traceMsg(comp(), "Existing liveness information:\n");
        if (ppInfo)
          ppInfo->print(comp());
      ...
      pendingPushLiveRangeInfo(node, ppInfo, _pendingPushSymRefs, osrPoint,
                               osrMethodData);
  ...
  if (comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr) {
    ...
    pendingPushLiveRangeInfo(comp()->getStartTree()->getNode(), NULL,
                             _pendingPushSymRefs, osrPoint, osrMethodData);
    pendingPushSlotSharingInfo(comp()->getStartTree()->getNode(), NULL,
                               _sharedSymRefs, osrPoint);
  }
...
/*
 * Allocate the BitVector of dead symbol references against the provided OSR
 * point.
 */
void TR_OSRLiveRangeAnalysis::pendingPushLiveRangeInfo(
    TR::Node *node, TR_BitVector *liveSymRefs,
    TR_BitVector *allPendingPushSymRefs, TR_OSRPoint *osrPoint,
    TR_OSRMethodData *osrMethodData) {
  int32_t byteCodeIndex = osrPoint->getByteCodeInfo().getByteCodeIndex();
  TR_BitVector *deadBV = NULL;

  // Calculate the dead symbol references, allocating a new BitVector if its
  // required
  _workBitVector->empty();
  *_workBitVector |= *allPendingPushSymRefs;
  if (liveSymRefs) *_workBitVector -= *liveSymRefs;
  ...
  if (comp()->getOption(TR_TraceOSR)) {
    traceMsg(comp(),
             "Live PP variables at OSR point %p of %p bytecode offset %d\n",
...
void TR_OSRLiveRangeAnalysis::pendingPushSlotSharingInfo(
    TR::Node *node, TR_BitVector *liveSymRefs, TR_BitVector *slotSharingSymRefs,
  ...
  if (liveSymRefs && !liveSymRefs->isEmpty()) {
    ...
    *_workBitVector &= *slotSharingSymRefs;
    ...
    while (bvi.hasMoreElements()) {
      ...
      List<TR::SymbolReference> *list = NULL;
      if (slot < 0)
        list = &comp()->getMethodSymbol()->getPendingPushSymRefs()->element(
            -slot - 1);
      else
      ...
      for (TR::SymbolReference *slotSymRef = listIt.getFirst(); slotSymRef;
        ...
        if (symRef == slotSymRef) break;
      TR_ASSERT(symRefOrder < list->getSize(),
                "symref #%d not found in list of shared slots\n",
                symRef->getReferenceNumber());
      ...
      comp()->getOSRCompilationData()->addSlotSharingInfo(
          osrPoint->getByteCodeInfo(), slot, symRefNum, symRefOrder,
          symRef->getSymbol()->getSize(), takesTwoSlots);
    }
  }

  comp()->getOSRCompilationData()->ensureSlotSharingInfoAt(
      osrPoint->getByteCodeInfo());
}

/*
 * figure out dead pending push slots for this osrPoint and bookkeep the
 * information in OSRMethodMetaData
 */
void TR_OSRLiveRangeAnalysis::buildDeadPendingPushSlotsInfo(
    TR::Node *node, TR_BitVector *livePendingPushSymRefs,
    TR_OSRPoint *osrPoint) {
  TR_ByteCodeInfo &bcInfo = osrPoint->getByteCodeInfo();
  if (trace())
  ...
  if (pendingPushSymRefs) {
    ...
    deadPPSSlots = new (trStackMemory())
        TR_BitVector(numOfPPSSlots, trMemory(), stackAlloc);
  ...
  if (livePendingPushSymRefs && !livePendingPushSymRefs->isEmpty()) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace())
        traceMsg(
            comp(),
            "pending push slot %d is live with symref %d takesTwoSlots %d\n",
            slot, symRefNum, takesTwoSlots);

      deadPPSSlots->reset(-slot - 1);
      if (takesTwoSlots) deadPPSSlots->reset(-slot);
    }
  }
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace())
        traceMsg(comp(), "ppslot %d is dead at %d:%d\n", slot,
                 bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
      // symRefNum = -1 and symRefOrder = -1 indicate those slots should be
      // zeroed in prepareForOSR
      comp()->getOSRCompilationData()->addSlotSharingInfo(
          osrPoint->getByteCodeInfo(), slot, -1 /* symRefNum */,
          -1 /* symRefOrder */,
          TR::Compiler->om.sizeofReferenceAddress() /*symSize*/,
          false /*takesTwoSlots*/);
    }
  }
...
void TR_OSRLiveRangeAnalysis::intersectWithExistingDeadSlots(
    TR_OSRPoint *osrPoint, TR_BitVector *deadPPSSlots,
    TR_BitVector *deadAutoSlots, bool containsPendingPushes) {
  TR_ByteCodeInfo &bcInfo = osrPoint->getByteCodeInfo();
  if (!_visitedBCI->isSet(bcInfo.getByteCodeIndex())) return;
  TR_OSRSlotSharingInfo *slotsInfo =
      comp()->getOSRCompilationData()->getSlotsInfo(bcInfo);
  ...
  if (slotsInfo)  // remove every dead slot that's not in the current dead
                  // symbols bit vector from slotInfos
  {
    TR_Array<TR_OSRSlotSharingInfo::TR_SlotInfo> &slotInfos =
        slotsInfo->getSlotInfos();
      ...
      if (slotInfo.symRefOrder == -1) {
        ...
        if (slotInfo.slot < 0 && containsPendingPushes) {
          TR_ASSERT(deadPPSSlots,
                    "with slotInfo.slot < 0  and containsPendingPushes, there "
                    "must be at least on pending push symbol in this method");
          int32_t ppsIndex = -slotInfo.slot - 1;
          existingDeadPPSSlots.set(ppsIndex);
          if (!deadPPSSlots->isSet(ppsIndex)) slotInfos.remove(i);
        }

        if (slotInfo.slot >= 0) {
          TR_ASSERT_FATAL(deadAutoSlots,
                          "with slotInfos.slot >= 0, there must be at least on "
                          "auto symbol in this method");
          existingDeadAutoSlots.set(slotInfo.slot);
          if (!deadAutoSlots->isSet(slotInfo.slot)) slotInfos.remove(i);
        }
      }
    }
  }

  if (deadPPSSlots) *deadPPSSlots &= existingDeadPPSSlots;
  if (deadAutoSlots) *deadAutoSlots &= existingDeadAutoSlots;
}

/*
 * figure out dead slots for this osrPoint and bookkeep the information in
 * OSRMethodMetaData
 */
void TR_OSRLiveRangeAnalysis::buildDeadSlotsInfo(
    TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
    int32_t *liveLocalIndexToSymRefNumberMap, bool containsPendingPushes) {
  TR_ByteCodeInfo &bcInfo = osrPoint->getByteCodeInfo();
  if (trace())
    traceMsg(comp(),
             "buildOSRSlotSharingInfoForDeadSlots: OSR point [%p] at %d:%d\n",
             node, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex());
  ...
  if (numOfAutoSlots > 0) {
    deadAutoSlots = new (trStackMemory())
        TR_BitVector(numOfAutoSlots, trMemory(), stackAlloc);
  ...
  if (!liveVars->isEmpty()) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (symRefNum < 0) continue;

      TR::SymbolReference *symRef = symRefTab->getSymRef(symRefNum);
      int32_t slot = symRef->getCPIndex();
      if (slot >= comp()->getMethodSymbol()->getFirstJitTempIndex()) continue;
      ...
      if (slot < 0) {
        ...
        if (takesTwoSlots) deadAutoSlots->reset(slot + 1);
      }
    }
  }

  intersectWithExistingDeadSlots(osrPoint, deadPPSSlots, deadAutoSlots,
...
int32_t TR_OSRLiveRangeAnalysis::fullAnalysis(bool includeParms,
  ...
  if (comp()->getOption(TR_TraceOSR)) {
    ...
    comp()->dumpMethodTrees("Before full OSRLiveRangeAnalysis",
                            optimizer()->getMethodSymbol());
  }

  {
    ...
    TR_Liveness liveLocals(comp(), optimizer(),
                           ...
                           ignoreOSRuses, NULL, false, includeParms);
    _liveVars = new (trStackMemory())
        TR_BitVector(liveLocals.getNumberOfBits(), trMemory(), stackAlloc);
    ...
    TR_Array<List<TR::SymbolReference>> *autoSymRefs =
        comp()->getMethodSymbol()->getAutoSymRefs();
    TR_Array<List<TR::SymbolReference>> *pendingPushSymRefs =
        comp()->getMethodSymbol()->getPendingPushSymRefs();
    ...
    int32_t numSlots = 0;
    if (autoSymRefs) numSlots = autoSymRefs->size();
    ...
    for (i = 0; i < numSlots; i++) {
      ...
      for (nextSymRef = symRefsIt.getCurrent(); nextSymRef;
        ...
        if (nextSymRef->getSymbol()->isAuto() &&
            !nextSymRef->getSymbol()
                 ->castToAutoSymbol()
                 ->isLiveLocalIndexUninitialized())
          liveLocalIndex =
              nextSymRef->getSymbol()->castToAutoSymbol()->getLiveLocalIndex();
                 ...
                 !nextSymRef->getSymbol()
                      ...
                      ->isLiveLocalIndexUninitialized())
          liveLocalIndex =
              nextSymRef->getSymbol()->castToParmSymbol()->getLiveLocalIndex();
        else {
          continue;
        }

        if (!nextSymRef->getSymbol()->holdsMonitoredObject()) {
          liveLocalIndexToSymRefNumberMap[liveLocalIndex] =
        ...
        for (nextSymRef = symRefsIt.getCurrent(); nextSymRef;
          ...
          if (nextSymRef->getSymbol()->isAuto() &&
              !nextSymRef->getSymbol()
                   ->castToAutoSymbol()
                   ->isLiveLocalIndexUninitialized()) {
            liveLocalIndexToSymRefNumberMap[nextSymRef->getSymbol()
                                                ->castToAutoSymbol()
                                                ->getLiveLocalIndex()] =
                nextSymRef->getReferenceNumber();
            nextSymRef->getSymbol()->setLocalIndex(0);
    ...
    while (block) {
      ...
      for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
        if (tt->getNode()->getOpCodeValue() == TR::BBEnd) {
          ...
          // If the block contains an OSR point, it should have an exception
          // edge to an OSR catch block
          edge = block->getExceptionSuccessors().begin();
          while (edge != block->getExceptionSuccessors().end()) {
            if (toBlock((*edge)->getTo())->isOSRCatchBlock()) break;
            ++edge;
          }
          if (edge == block->getExceptionSuccessors().end()) {
            if (comp()->getOption(TR_TraceOSR))
              traceMsg(comp(), "skipping block_%d without OSR points",
                       block->getNumber());
            break;
          }
          ...
          if (comp()->getOption(TR_TraceOSR)) {
            traceMsg(comp(),
                     "BB_End for block_%d: live vars = ", block->getNumber());
        ...
        TR_ByteCodeInfo &bci =
            comp()->getMethodSymbol()->getOSRByteCodeInfo(tt->getNode());

        // Check if this treetop is an OSR pending push store or load. If so, it
        // will shift the cursor to before the PPs. The cursor will either point
        // at the first PP or the treetop before it, depending on whether it
        // could be the OSR point for these PP.
        ...
        if (comp()->isOSRTransitionTarget(TR::postExecutionOSR) &&
          ...
          // If it is the start of this block or has a different bytecode index,
          // it cannot be the OSR point
          if (tt == firstTT ||
              osrBCI.getCallerIndex() != bci.getCallerIndex() ||
        ...
        // If the transition point is after the OSR point, build the OSR live
        // info before any PPS are processed.
        if (isPotentialOSRPoint &&
            comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
          ...
          buildOSRLiveRangeInfo(node, _liveVars, offsetOSRPoint,
                                liveLocalIndexToSymRefNumberMap, numBits,
                                osrMethodData, containsPendingPushes);
          if (!_sharedSymRefs->isEmpty())
            buildOSRSlotSharingInfo(node, _liveVars, offsetOSRPoint,
        ...
        while (offsetOSRTreeTop && offsetOSRTreeTop != tt) {
          maintainLiveness(offsetOSRTreeTop->getNode(), NULL, -1, visitCount,
                           &liveLocals, _liveVars, block);
          offsetOSRTreeTop = offsetOSRTreeTop->getPrevTreeTop();
        }
        maintainLiveness(node, NULL, -1, visitCount, &liveLocals, _liveVars,
                         block);

        // If the transition point is before the OSR point, build the OSR live
        // info after the post PPS and the OSR point are processed.
        if ((isPotentialOSRPoint ||
             // maintainLiveness only kicks in for the first evaluation point.
             // In involuntaryOSR mode, there are cases where the first
             // evaluation point of a node is under a treetop that is not an
             // OSRPoint but another treetop with the same bci is an OSRPoint.
             // Need to call buildOSRLiveRangeInfo in this case for the first
             // evaluation point to make sure the liveness info is bookkept
             // correctly for this bci
             (_visitedBCI->isSet(node->getByteCodeInfo().getByteCodeIndex()) &&
              comp()->getOSRMode() == TR::involuntaryOSR)) &&
            (comp()->isOSRTransitionTarget(TR::preExecutionOSR) ||
             comp()->requiresAnalysisOSRPoint(node))) {
          TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
          TR_ASSERT_FATAL(bcInfo.getCallerIndex() == callerIndex,
                          "Found node with invalid caller index");
          TR_OSRPoint *osrPoint =
              comp()->getMethodSymbol()->findOSRPoint(bcInfo);
          TR_ASSERT_FATAL(osrPoint != NULL,
                          "Cannot find a pre OSR point for node %p", node);

          buildOSRLiveRangeInfo(node, _liveVars, osrPoint,
                                liveLocalIndexToSymRefNumberMap, numBits,
                                osrMethodData, containsPendingPushes);
          if (!_sharedSymRefs->isEmpty())
            buildOSRSlotSharingInfo(node, _liveVars, osrPoint,
                                    liveLocalIndexToSymRefNumberMap,
                                    _sharedSymRefs);
          if (comp()->getOption(TR_FullSpeedDebug))
            buildDeadSlotsInfo(node, _liveVars, osrPoint,
                               liveLocalIndexToSymRefNumberMap,
                               containsPendingPushes);
        }
      ...
      //
      if (comp()->getStartTree()->getNode()->getBlock() == block &&
          comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr) {
        TR_ByteCodeInfo bci;
        bci.setCallerIndex(-1);
        bci.setByteCodeIndex(0);
        TR_OSRPoint *osrPoint = comp()->getMethodSymbol()->findOSRPoint(bci);
        ...
        buildOSRLiveRangeInfo(comp()->getStartTree()->getNode(), _liveVars,
                              osrPoint, liveLocalIndexToSymRefNumberMap,
                              numBits, osrMethodData, containsPendingPushes);
        if (!_sharedSymRefs->isEmpty())
          buildOSRSlotSharingInfo(comp()->getStartTree()->getNode(), _liveVars,
                                  osrPoint, liveLocalIndexToSymRefNumberMap,
                                  _sharedSymRefs);
      }
...
//
// To reduce the impact of pending push temps in postExecutionOSR, the live
// pending push symrefs can be stored or referenced in anchored loads, between
// the OSR point and the transition. In the event that the anchored loads are
// not commoned, they will be removed.
//
TR::TreeTop *TR_OSRLiveRangeAnalysis::collectPendingPush(
    TR_ByteCodeInfo bci, TR::TreeTop *tt, TR_BitVector *liveVars) {
  while (comp()->getMethodSymbol()->isOSRRelatedNode(tt->getNode(), bci)) {
    TR::Node *node = tt->getNode();
    if (node->getOpCode().isStoreDirect()) {
      TR::AutomaticSymbol *local =
          node->getSymbolReference()->getSymbol()->getAutoSymbol();
      ...
      if (node->getFirstChild()->getReferenceCount() == 1) {
        TR::AutomaticSymbol *local = node->getFirstChild()
                                         ->getSymbolReference()
                                         ->getSymbol()
                                         ->getAutoSymbol();
        int32_t localIndex = local->getLiveLocalIndex();
      ...
      TR_ASSERT(0,
                "Unexpected OSR related node %p found, should be either "
                "pending push store or anchored load",
                node);
    }
    tt = tt->getPrevTreeTop();
  }
...
void TR_OSRLiveRangeAnalysis::maintainLiveness(
    TR::Node *node, TR::Node *parent, int32_t childNum, vcount_t visitCount,
  ...
  if (node->getOpCode().isStoreDirect()) {
    ...
    if (local && !local->isLiveLocalIndexUninitialized()) {
      ...
      // This local is killed only if the live range of any loads of this symbol
      // do not overlap with this store.
      //
    ...
    if (local && !local->isLiveLocalIndexUninitialized()) {
      ...
      if (node->getLocalIndex() == node->getReferenceCount()) {
        local->setLocalIndex(local->getLocalIndex() +
    ...
    for (auto edge = block->getExceptionSuccessors().begin();
         edge != block->getExceptionSuccessors().end(); ++edge) {
      succ = toBlock((*edge)->getTo());
  ...
  //
  if (node->getLocalIndex() > 0) return;

  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i)
    maintainLiveness(node->getChild(i), node, i, visitCount, liveLocals,
                     liveVars, block);
}

/**
 *  Based on the BitVector of currently live symbol references, calculate which
 * of the shared slots should be used for a transition at this OSR point.
 */
void TR_OSRLiveRangeAnalysis::buildOSRSlotSharingInfo(
    TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
  ...
  if (!liveVars->isEmpty()) {
    ...
    while (bvi.hasMoreElements()) {
      int32_t index = bvi.getNextElement();
      int32_t symRefNum = liveLocalIndexToSymRefNumberMap[index];
      if (symRefNum < 0 || !sharedSymRefs->get(symRefNum)) continue;
...
void TR_OSRLiveRangeAnalysis::buildOSRLiveRangeInfo(
    TR::Node *node, TR_BitVector *liveVars, TR_OSRPoint *osrPoint,
    int32_t *liveLocalIndexToSymRefNumberMap, int32_t numBits,
    TR_OSRMethodData *osrMethodData, bool containsPendingPushes) {
  int32_t byteCodeIndex = osrPoint->getByteCodeInfo().getByteCodeIndex();
  ...
  *_workBitVector -= *liveVars;
  ...
  if (!_workBitVector->isEmpty()) {
    ...
    // There are cases where one bci has several OSRPoints with different
    // liveness. A symbol is only dead if it's dead at all the OSRPoints
    if (_visitedBCI->isSet(byteCodeIndex)) {
      if (!containsPendingPushes) *_workDeadSymRefs |= *_pendingPushSymRefs;
      if (deadSymRefs) *deadSymRefs &= *_workDeadSymRefs;
    } else {
  ...
  if (comp()->getOption(TR_TraceOSR)) {
    traceMsg(comp(),
             "Dead variables at OSR point %p of %p bytecode offset %d\n", node,
             osrMethodData, osrPoint->getByteCodeInfo().getByteCodeIndex());
    if (deadSymRefs)
...
const char *TR_OSRLiveRangeAnalysis::optDetailString() const throw() {
  return "O^O OSR LIVE RANGE ANALYSIS: ";
}

/**
 * Collect the dead stores in the provided block. Either initialize the provided
 * bitvector with the dead stores symrefs that were found or take the
 * intersection of the two, filtering down to symrefs that are always dead.
 */
bool TR_OSRExceptionEdgeRemoval::addDeadStores(TR::Block *osrBlock,
                                               TR_BitVector &dead,
                                               bool inited) {
  TR_ASSERT(osrBlock->isOSRCodeBlock() || osrBlock->isOSRInduceBlock(),
            "It is only safe to identify dead stores in OSRCodeBlocks and "
            "OSRInduceBlocks");

  _seenDeadStores->empty();
  for (TR::TreeTop *tt = osrBlock->getFirstRealTreeTop();
       tt != osrBlock->getExit(); tt = tt->getNextTreeTop()) {
    TR::Node *node = tt->getNode();
    if (node->getOpCode().isStoreDirect() &&
...
/**
 * If loads of a symbol along a OSR paths are zeroed, the dead stores can be
 * removed as well. Given an OSRCodeBlock or an OSRInduceBlock and a bitvector
 * of dead symbol references, this function will remove them.
 */
void TR_OSRExceptionEdgeRemoval::removeDeadStores(TR::Block *osrBlock,
                                                  TR_BitVector &dead) {
  TR_ASSERT(osrBlock->isOSRCodeBlock() || osrBlock->isOSRInduceBlock(),
            "It is only safe to remove dead stores from OSRCodeBlocks and "
            "OSRInduceBlocks");
...
int32_t TR_OSRExceptionEdgeRemoval::perform() {
  if (comp()->getOption(TR_EnableOSR)) {
    // if (comp()->getOption(TR_DisableOSRExceptionEdgeRemoval)) // add to
    // Options later
    if (false) {
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "OSR is enabled but OSR exception edge removal is not.\n");
      return 0;
    }
  } else {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
               "OSR Option not enabled -- returning from OSR exception edge "
               "removal analysis.\n");
    return 0;
  }
  ...
  if (comp()->getOSRMode() == TR::involuntaryOSR) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
               "Involuntary OSR enabled -- returning from OSR exception edge "
               "removal analysis since we implement involuntary OSR with "
               "exception edges.\n");
    return 0;
  }
  ...
  if (comp()->osrInfrastructureRemoved()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
               "OSR infrastructure removed -- returning from OSR exception "
               "edge removal analysis since we have already removed OSR "
               "infrastructure.\n");
    return 0;
  }
  ...
  if (comp()->isPeekingMethod()) {
    if (comp()->getOption(TR_TraceOSR)) {
      traceMsg(comp(),
               "%s OSR exception edge removal is not required because we are "
               "peeking\n",
               optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
      traceMsg(comp(), "Returning...\n");
    }
  ...
  for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
    ...
    TR_ASSERT(block->verifyOSRInduceBlock(comp()),
              "osr induce calls can only exist in osr induce blocks");
    ...
    for (auto edge = block->getExceptionSuccessors().begin();
      ...
      if (catchBlock->isOSRCatchBlock() &&
          performTransformation(
              comp(),
              "%s: Remove redundant exception edge from block_%d at [%p] to "
              "OSR catch block_%d at [%p]\n",
              OPT_DETAILS, block->getNumber(), block, catchBlock->getNumber(),
              catchBlock))
        cfg->removeEdge(block, catchBlock);
    }
  }

  // Under voluntary OSR, it is common for all paths to an OSR code block to
  // contain a dead store for some of the prepareForOSR loads. As this is a cold
  // path and OSR infrastructure is commonly ignored, the optimizer won't
  // eliminate these loads.
  //
  // As this is cheap to do and will reduce the overhead of analysis like UseDef
  // and CompactLocals, identify the loads that have dead stores on all paths
  // and zero them.
  //
  static const char *disableDeadStoreCleanup =
      feGetEnv("TR_DisableDeadStoreCleanup");
  if (!disableDeadStoreCleanup) {
    // A stack of blocks is used to stash blocks that will be revisited for
    // cleanup
    TR_Stack<TR::Block *> osrBlocks(trMemory(), 8, false, stackAlloc);

    TR_BitVector alwaysDead(comp()->trMemory()->currentStackRegion());
    _seenDeadStores = new (trStackMemory())
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
      ...
      while (tt && !((tt->getNode()->getOpCode().isCheck() ||
      ...
      TR_ASSERT(tt,
                "OSRCodeBlocks must contain a call to prepareForOSR and no "
                "other calls");
      ...
      for (auto edge = block->getPredecessors().begin();
        ...
        if (osrBlock->isOSRCodeBlock() &&
            addDeadStores(osrBlock, alwaysDead, !firstPass))
          ...
          for (auto exEdge = osrBlock->getExceptionPredecessors().begin();
            ...
            TR_ASSERT(exBlock && exBlock->isOSRInduceBlock(),
                      "Predecessors to an OSR catch block should be OSR induce "
                      "blocks");
      ...
      if (alwaysDead.isEmpty()) continue;
      ...
      for (int32_t i = 0; i < prepareForOSR->getNumChildren(); ++i) {
        ...
        if (child->getOpCode().isLoadVar() &&
          ...
          if (performTransformation(
                  comp(),
                  "%s: Remove dead OSR load [%p] from OSR code block_%d\n",
                  OPT_DETAILS, child, block->getNumber())) {
            TR::Node::recreate(prepareForOSR->getChild(i),
                               comp()->il.opCodeForConst(
                                   prepareForOSR->getChild(i)->getDataType()));
...
const char *TR_OSRExceptionEdgeRemoval::optDetailString() const throw() {
