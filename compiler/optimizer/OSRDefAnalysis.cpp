                  ...
                  false /* !doCompletion */)
{
  ...
  prepareUseDefInfo(false /* !requiresGlobals */,
                    false /* !prefersGlobals */,
                    false /* !cannotOmitTrivialDefs */,
                    false /* !conversionRegsOnly */);
}
...
bool
TR_OSRDefInfo::performAnalysis(AuxiliaryData& aux)
{
...
void
TR_OSRDefInfo::performFurtherAnalysis(AuxiliaryData& aux)
{
  if (!infoIsValid()) {
...
void
TR_OSRDefInfo::addSharingInfo(AuxiliaryData& aux)
{
...
void
TR_OSRDefInfo::processReachingDefinition(void* vblockInfo, AuxiliaryData& aux)
{
...
void
TR_OSRDefInfo::buildOSRDefs(void* vblockInfo, AuxiliaryData& aux)
{
  ...
  */
  ...
  for (treeTop = comp()->getStartTree(); treeTop != NULL;
    ...
    if (isPotentialOSRPoint &&
      ...
      TR_ASSERT(
        osrPoint != NULL, "Cannot find a pre OSR point for node %p", node);
    }
    ...
    if (isPotentialOSRPoint &&
      ...
      nextOsrPoint = _methodSymbol->findOSRPoint(bci);
      TR_ASSERT(
        nextOsrPoint != NULL, "Cannot find a post OSR point for node %p", node);
    }
  }
  ...
  for (int i = 0; i < numOSRPoints; ++i) {
    ...
    if (trace()) {
      ...
      traceMsg(comp(),
               "OSR defs at index %d bcIndex %d callerIndex %d\n",
               i,
               bcinfo.getByteCodeIndex(),
               bcinfo.getCallerIndex());
      info->print(comp());
...
void
TR_OSRDefInfo::buildOSRDefs(TR::Node* node,
                            void* vanalysisInfo,
                            TR_OSRPoint* osrPoint,
                            TR_OSRPoint* osrPoint2,
                            TR::Node* parent,
                            AuxiliaryData& aux)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    buildOSRDefs(
      node->getChild(i), analysisInfo, osrPoint, osrPoint2, node, aux);
  }
  ...
  if (expandedNodeIndex != NULL_USEDEF_SYMBOL_INDEX && expandedNodeIndex != 0) {
    ...
    if (!defsForSymbol->isEmpty() && isExpandedDefIndex(expandedNodeIndex) &&
      ...
      if (trace()) {
        traceMsg(comp(),
                 "defs for symbol %d with symref index %d\n",
                 symIndex,
...
TR_OSRDefAnalysis::perform()
{
  ...
  if (trace()) {
    traceMsg(comp(),
             "%s OSR reaching definition analysis is required\n",
             optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
  }
...
bool
TR_OSRDefAnalysis::requiresAnalysis()
{
  TR::ResolvedMethodSymbol* methodSymbol = optimizer()->getMethodSymbol();
...
bool
TR_OSRLiveRangeAnalysis::shouldPerformAnalysis()
{
  if (!comp()->getOption(TR_EnableOSR)) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(
        comp(),
        "Should not perform OSRLiveRangeAnalysis -- OSR Option not enabled\n");
    return false;
  } else if (comp()->isPeekingMethod()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
...
int32_t
TR_OSRLiveRangeAnalysis::perform()
{
 ...
 */
int32_t
TR_OSRLiveRangeAnalysis::partialAnalysis()
{
  ...
  for (TR::TreeTop* tt = comp()->getMethodSymbol()->getLastTreeTop(); tt;
    ...
    if (isPotentialOSRPoint &&
      ...
      TR_ASSERT(offsetOSRPoint != NULL,
                "Cannot find a post OSR point for node %p",
                node);
      ...
      pendingPushLiveRangeInfo(
        node, ppInfo, _pendingPushSymRefs, offsetOSRPoint, osrMethodData);
    ...
    if (isPotentialOSRPoint &&
      ...
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "Analysing pre OSR point for n%dn at %d:%d\n",
                 node->getGlobalIndex(),
                 bci.getCallerIndex(),
                 bci.getByteCodeIndex());
      ...
      pendingPushLiveRangeInfo(
        node, ppInfo, _pendingPushSymRefs, osrPoint, osrMethodData);
  ...
  if (comp()->isOutermostMethod() && comp()->getHCRMode() == TR::osr) {
    ...
    pendingPushLiveRangeInfo(comp()->getStartTree()->getNode(),
                             NULL,
                             _pendingPushSymRefs,
                             osrPoint,
                             osrMethodData);
    pendingPushSlotSharingInfo(
      comp()->getStartTree()->getNode(), NULL, _sharedSymRefs, osrPoint);
  }
 ...
 */
void
TR_OSRLiveRangeAnalysis::pendingPushLiveRangeInfo(
  TR::Node* node,
  TR_BitVector* liveSymRefs,
  TR_BitVector* allPendingPushSymRefs,
  TR_OSRPoint* osrPoint,
  TR_OSRMethodData* osrMethodData)
{
 ...
 */
void
TR_OSRLiveRangeAnalysis::pendingPushSlotSharingInfo(
  TR::Node* node,
  TR_BitVector* liveSymRefs,
  TR_BitVector* slotSharingSymRefs,
  TR_OSRPoint* osrPoint)
{
  if (liveSymRefs && !liveSymRefs->isEmpty()) {
    ...
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
               "Shared PP slots at OSR point [%p] at %d:%d\n",
               node,
               bcInfo.getCallerIndex(),
               bcInfo.getByteCodeIndex());
 ...
 */
void
TR_OSRLiveRangeAnalysis::buildDeadPendingPushSlotsInfo(
  TR::Node* node,
  TR_BitVector* livePendingPushSymRefs,
  TR_OSRPoint* osrPoint)
{
  TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
  if (trace())
    traceMsg(comp(),
             "buildDeadPendingPushSlotsInfo: OSR point [%p] at %d:%d\n",
             node,
             bcInfo.getCallerIndex(),
             bcInfo.getByteCodeIndex());
  ...
  if (pendingPushSymRefs) {
    ...
    deadPPSSlots =
      new (trStackMemory()) TR_BitVector(numOfPPSSlots, trMemory(), stackAlloc);
...
void
TR_OSRLiveRangeAnalysis::intersectWithExistingDeadSlots(
  TR_OSRPoint* osrPoint,
  TR_BitVector* deadPPSSlots,
  TR_BitVector* deadAutoSlots,
 ...
 */
void
TR_OSRLiveRangeAnalysis::buildDeadSlotsInfo(
  TR::Node* node,
  ...
  bool containsPendingPushes)
{
  ...
  if (!liveVars->isEmpty()) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace())
        traceMsg(comp(),
                 "slot %d is live with symref %d takesTwoSlots %d\n",
                 slot,
                 symRefNum,
                 takesTwoSlots);
  ...
  intersectWithExistingDeadSlots(
    osrPoint, deadPPSSlots, deadAutoSlots, containsPendingPushes);
...
int32_t
TR_OSRLiveRangeAnalysis::fullAnalysis(bool includeParms,
                                      bool containsPendingPushes)
{
  TR_Structure* rootStructure =
    TR_RegionAnalysis::getRegions(comp(), optimizer()->getMethodSymbol());
...

  {
    ...
    while (block) {
      ...
      for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
        if (tt->getNode()->getOpCodeValue() == TR::BBEnd) {
          ...
          if (edge == block->getExceptionSuccessors().end()) {
            if (comp()->getOption(TR_TraceOSR))
              traceMsg(comp(),
                       "skipping block_%d without OSR points",
                       block->getNumber());
            break;
          }
          ...
          if (comp()->getOption(TR_TraceOSR)) {
            traceMsg(
              comp(), "BB_End for block_%d: live vars = ", block->getNumber());
        ...
        while (offsetOSRTreeTop && offsetOSRTreeTop != tt) {
          maintainLiveness(offsetOSRTreeTop->getNode(),
                           NULL,
                           -1,
                           visitCount,
                           &liveLocals,
        ...
        maintainLiveness(
          node, NULL, -1, visitCount, &liveLocals, _liveVars, block);
      ...
      if (comp()->getStartTree()->getNode()->getBlock() == block &&
        ...
        buildOSRLiveRangeInfo(comp()->getStartTree()->getNode(),
                              _liveVars,
                              osrPoint,
                              liveLocalIndexToSymRefNumberMap,
                              numBits,
                              ...
                              containsPendingPushes);
        if (!_sharedSymRefs->isEmpty())
          buildOSRSlotSharingInfo(comp()->getStartTree()->getNode(),
                                  _liveVars,
                                  osrPoint,
                                  liveLocalIndexToSymRefNumberMap,
                                  _sharedSymRefs);
...
//
TR::TreeTop*
TR_OSRLiveRangeAnalysis::collectPendingPush(TR_ByteCodeInfo bci,
                                            TR::TreeTop* tt,
                                            TR_BitVector* liveVars)
{
  while (comp()->getMethodSymbol()->isOSRRelatedNode(tt->getNode(), bci)) {
    ...
    if (node->getOpCode().isStoreDirect()) {
      ...
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "+++ local index %d OSR PENDING PUSH STORE LIVE\n",
                 localIndex);
      ...
      if (node->getFirstChild()->getReferenceCount() == 1) {
        ...
        if (comp()->getOption(TR_TraceOSR))
          traceMsg(comp(),
                   "+++ local index %d OSR PENDING PUSH LOAD LIVE\n",
                   localIndex);
...
void
TR_OSRLiveRangeAnalysis::maintainLiveness(TR::Node* node,
                                          TR::Node* parent,
                                          int32_t childNum,
                                          vcount_t visitCount,
                                          ...
                                          TR::Block* block)
{
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i)
    maintainLiveness(
      node->getChild(i), node, i, visitCount, liveLocals, liveVars, block);
}
 ...
 */
void
TR_OSRLiveRangeAnalysis::buildOSRSlotSharingInfo(
  TR::Node* node,
  TR_BitVector* liveVars,
  TR_OSRPoint* osrPoint,
  int32_t* liveLocalIndexToSymRefNumberMap,
  TR_BitVector* sharedSymRefs)
{
  TR_ByteCodeInfo& bcInfo = osrPoint->getByteCodeInfo();
  ...
  if (!liveVars->isEmpty()) {
    ...
    if (trace())
      traceMsg(comp(),
               "Shared slots at OSR point [%p] at %d:%d\n",
               node,
               bcInfo.getCallerIndex(),
               bcInfo.getByteCodeIndex());
...
void
TR_OSRLiveRangeAnalysis::buildOSRLiveRangeInfo(
  TR::Node* node,
  TR_BitVector* liveVars,
  TR_OSRPoint* osrPoint,
  int32_t* liveLocalIndexToSymRefNumberMap,
 ...
 */
bool
TR_OSRExceptionEdgeRemoval::addDeadStores(TR::Block* osrBlock,
                                          TR_BitVector& dead,
                                          bool inited)
{
  ...
  if (comp()->getOption(TR_TraceOSR)) {
    traceMsg(
      comp(), "Identified dead stores for block_%d:\n", osrBlock->getNumber());
 ...
 */
void
TR_OSRExceptionEdgeRemoval::removeDeadStores(TR::Block* osrBlock,
                                             TR_BitVector& dead)
{
  TR_ASSERT(osrBlock->isOSRCodeBlock() || osrBlock->isOSRInduceBlock(),
  ...
  for (TR::TreeTop* tt = osrBlock->getFirstRealTreeTop();
    ...
    if (node->getOpCode().isStoreDirect() &&
      ...
      if (comp()->getOption(TR_TraceOSR))
        traceMsg(comp(),
                 "Removing dead store n%dn of symref #%d\n",
                 node->getGlobalIndex(),
                 node->getSymbolReference()->getReferenceNumber());
      TR::TransformUtil::removeTree(comp(), tt);
    }
...
int32_t
TR_OSRExceptionEdgeRemoval::perform()
{
  if (comp()->getOption(TR_EnableOSR)) {
  ...
  if (comp()->getOSRMode() == TR::involuntaryOSR) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(
        comp(),
        "Involuntary OSR enabled -- returning from OSR exception edge removal "
        "analysis since we implement involuntary OSR with exception edges.\n");
    return 0;
  }
  ...
  if (comp()->osrInfrastructureRemoved()) {
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(
        comp(),
        "OSR infrastructure removed -- returning from OSR exception edge "
        "removal analysis since we have already removed OSR infrastructure.\n");
    return 0;
  }
  ...
  if (comp()->isPeekingMethod()) {
    ...
    if (comp()->getOption(TR_TraceOSR)) {
      traceMsg(comp(),
               "%s OSR exception edge analysis can be done\n",
               optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
    }
  }
    ...
    for (auto edge = block->getExceptionSuccessors().begin();
      ...
      if (catchBlock->isOSRCatchBlock() &&
          performTransformation(
            comp(),
            "%s: Remove redundant exception edge from block_%d at [%p] to OSR "
            "catch block_%d at [%p]\n",
            OPT_DETAILS,
            block->getNumber(),
            block,
  ...
  if (!disableDeadStoreCleanup) {
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
      ...
      for (auto edge = block->getPredecessors().begin();
           edge != block->getPredecessors().end();
           ++edge) {
        ...
        if (osrBlock->isOSRCodeBlock() &&
          ...
          for (auto exEdge = osrBlock->getExceptionPredecessors().begin();
               exEdge != osrBlock->getExceptionPredecessors().end();
               ++exEdge) {
            TR::Block* exBlock = toBlock((*exEdge)->getFrom());
            TR_ASSERT(
              exBlock && exBlock->isOSRInduceBlock(),
