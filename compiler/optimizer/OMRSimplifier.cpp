...
static void
resetBlockVisitFlags(TR::Compilation* comp)
{
...
static TR::TreeTop*
findNextLegalTreeTop(TR::Compilation* comp, TR::Block* block)
{
...
static void
countNodes(CS2::ABitVector<TR::Allocator>& mark, TR::Node* n, size_t& numNodes)
{
...
static void
computeInvarianceOfAllStructures(TR::Compilation* comp, TR_Structure* s)
{
...
TR::Optimization*
OMR::Simplifier::create(TR::OptimizationManager* manager)
{
...
OMR::Simplifier::simplifyExtendedBlock(TR::TreeTop* treeTop)
{
  ...
  for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
    ...
    for (auto cursor = _performLowerTreeNodePairs.begin();
         cursor != _performLowerTreeNodePairs.end();
         ++cursor) {
      ...
      if (trace())
        traceMsg(comp(),
                 ...
                 treeNodePair.first,
                 treeNodePair.second);
      TR::Node* performLowerNode = postWalkLowerTreeSimplifier(
        treeNodePair.first, treeNodePair.second, block, (TR::Simplifier*)this);
...
OMR::Simplifier::simplify(TR::Node* node, TR::Block* block)
{
  ...
  //
  TR::Node* newNode =
    simplifierOpts[node->getOpCodeValue()](node, block, (TR::Simplifier*)this);
  if ((node != newNode) ||
...
TR::Node*
OMR::Simplifier::unaryCancelOutWithChild(TR::Node* node,
                                         TR::Node* firstChild,
                                         ...
                                         bool anchorChildren)
{
    ...
    if (disallow) {
      if (trace())
        traceMsg(comp(),
                 ...
                 node->getSize(),
                 firstChild->getSize(),
                 firstChild->getFirstChild()->getSize());
      return NULL;
    }
  }
  ...
  if (firstChild->getOpCodeValue() == opcode &&
      performTransformation(comp(),
                            ...
                            "] %s and its child [" POINTER_PRINTF_FORMAT
                            "] %s\n",
                            optDetailString(),
                            node,
                            node->getOpCode().getName(),
...
//
bool
OMR::Simplifier::conditionalToUnconditional(TR::Node*& node,
                                            TR::Block* block,
                                            int takeBranch)
{
...
void
OMR::Simplifier::anchorOrderDependentNodesInSubtree(TR::Node* node,
                                                    TR::Node* replacement,
                                                    TR::TreeTop* anchorTree)
{
  if (node == replacement)
  ...
  if (nodeIsOrderDependent(node, 0, false)) {
    ...
    anchorChildren(
      node, anchorTree, 0, node->getReferenceCount() > 1, replacement);
}

bool
OMR::Simplifier::isBoundDefinitelyGELength(TR::Node* boundChild,
                                           TR::Node* lengthChild)
{
