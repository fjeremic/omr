...
static_assert(TR::NumIlOps == (sizeof(simplifierOpts) / sizeof(simplifierOpts[0])),
...
OMR::Simplifier::simplifyExtendedBlock(TR::TreeTop* treeTop)
{
    ...
    for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (!block && _reassociate && comp()->getFlowGraph()->getStructure() != NULL // [99391] getStructureOf() only valid if structure isn't invalidated
...
OMR::Simplifier::simplify(TR::Node* node, TR::Block* block)
{
    ...
    if ((node != newNode) || (newNode && ((newNode->getOpCodeValue() != node->getOpCodeValue()) || (newNode->getNumChildren() != node->getNumChildren()))))
        requestOpt(OMR::localCSE, true, block);
...
OMR::Simplifier::unaryCancelOutWithChild(TR::Node* node, TR::Node* firstChild, TR::TreeTop* anchorTree, TR::ILOpCodes opcode, bool anchorChildren)
{
    ...
    if (firstChild->getOpCodeValue() == opcode && (node->getType().isAggregate() || firstChild->getType().isAggregate()) && (node->getSize() > firstChild->getSize() || node->getSize() != firstChild->getFirstChild()->getSize())) {
        ...
        if (node->getType().isIntegral() && nodeSize == grandChild->getSize() && nodeSize > firstChild->getSize()) {
            ...
            if (grandChild->getOpCode().isRightShift() && grandChild->getOpCode().isShiftLogical() && grandChild->getSecondChild()->getOpCode().isLoadConst() && (grandChild->getSecondChild()->get64bitIntegralValue() == truncatedBits)) {
                disallow = false;
                if (trace())
    ...
    if (firstChild->getOpCodeValue() == opcode && performTransformation(comp(), "%sRemoving node [" POINTER_PRINTF_FORMAT "] %s and its child [" POINTER_PRINTF_FORMAT "] %s\n", optDetailString(), node, node->getOpCode().getName(), firstChild, firstChild->getOpCode().getName())) {
        TR::Node* grandChild = firstChild->getFirstChild();
        grandChild->incReferenceCount();
        bool anchorChildrenNeeded = anchorChildren && (node->getNumChildren() > 1 || firstChild->getNumChildren() > 1 || node->getOpCode().hasSymbolReference() || firstChild->getOpCode().hasSymbolReference());
    ...
    if (boundOp == TR::iadd) {
        ...
        if (first == lengthChild) {
            ...
            if (second->getOpCode().isArrayLength() || secondOp == TR::bu2i || secondOp == TR::su2i ||
                ...
                (secondOp == TR::iand && second->getSecondChild()->getOpCodeValue() == TR::iconst && (second->getSecondChild()->getInt() & 80000000) == 0) ||

                (secondOp == TR::iushr && second->getSecondChild()->getOpCodeValue() == TR::iconst && (second->getSecondChild()->getInt() & 0x1f) > 0)) {
                return true;
            }
            ...
            if (first->getOpCode().isArrayLength() || firstOp == TR::bu2i || firstOp == TR::su2i ||
                ...
                (firstOp == TR::iand && first->getSecondChild()->getOpCodeValue() == TR::iconst && (first->getSecondChild()->getInt() & 80000000) == 0) ||

                (firstOp == TR::iushr && first->getSecondChild()->getOpCodeValue() == TR::iconst && (first->getSecondChild()->getInt() & 0x1f) > 0)) {
                return true;
            }
        }
        ...
        if (first == lengthChild) {
            ...
            if ((secondOp == TR::iconst && second->getInt() < 0) ||
                ...
                (secondOp == TR::ior && second->getSecondChild()->getOpCodeValue() == TR::iconst && (second->getSecondChild()->getInt() & 0x80000000) != 0)) {
                return true;
            }
        }
    }
