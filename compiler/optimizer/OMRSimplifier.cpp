...
static void resetBlockVisitFlags(TR::Compilation *comp) {
  for (TR::Block *block = comp->getStartBlock(); block != NULL;
       block = block->getNextBlock()) {
    block->setHasBeenVisited(false);
  }
}

static TR::TreeTop *findNextLegalTreeTop(TR::Compilation *comp,
                                         TR::Block *block) {
  vcount_t startVisitCount = comp->getStartTree()->getNode()->getVisitCount();
  TR::TreeTop *tt = NULL;
  for (tt = comp->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    if (tt->getNode()->getVisitCount() < startVisitCount) break;
...
static void countNodes(CS2::ABitVector<TR::Allocator> &mark, TR::Node *n,
                       size_t &numNodes) {
...
static void computeInvarianceOfAllStructures(TR::Compilation *comp,
                                             TR_Structure *s) {
...
OMR::Simplifier::Simplifier(TR::OptimizationManager *manager)
    : TR::Optimization(manager),
      ...
      _performLowerTreeNodePairs(
          getTypedAllocator<std::pair<TR::TreeTop *, TR::Node *>>(
              self()->allocator())) {
...
void OMR::Simplifier::prePerformOnBlocks() {
  ...
  if (_reassociate) {
    ...
    computeInvarianceOfAllStructures(comp(),
...
void OMR::Simplifier::postPerformOnBlocks() {
  if (trace()) comp()->dumpMethodTrees("Trees after simplification");
  ...
  //
  if (_useDefInfo && _invalidateUseDefInfo) optimizer()->setUseDefInfo(NULL);
...
int32_t OMR::Simplifier::perform() {
  ...
  while (tt) tt = simplifyExtendedBlock(tt);
...
void OMR::Simplifier::cleanupFlags(TR::Node *node) {
  ...
  if (node->isDualHigh()) node->getChild(2)->setIsAdjunct(true);
}
...
void OMR::Simplifier::setCC(TR::Node *node, OMR::TR_ConditionCodeNumber cc) {
  ...
  TR_HashId index = 0;
  //   FIXME: is this assume actually needed? It seems harmless if the node is
  //   already present TR_ASSERT(!_ccHashTab.locate(node->getGlobalIndex(),
  //   index), "node already present in cc hashtable");
...
TR::TreeTop *OMR::Simplifier::simplifyExtendedBlock(TR::TreeTop *treeTop) {
  ...
  for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
    ...
    TR_ASSERT(node->getOpCodeValue() == TR::BBStart,
              "Simplification, expected BBStart treetop");

    TR::Block *b = node->getBlock();
    if (block && !b->isExtensionOfPreviousBlock()) break;
...
#ifdef DEBUG
    if (block != b) b->setHasBeenVisited();
#endif

    if (!block && _reassociate &&
        comp()->getFlowGraph()->getStructure() !=
            NULL  // [99391] getStructureOf() only valid if structure isn't
                  // invalidated
      ...
      if (blockStructure) {
        ...
        while (parent) {
          ...
          if (region
                  ->isNaturalLoop() /* || region->containsInternalCycles() */) {
    ...
    if (trace()) traceMsg(comp(), "simplifying block_%d\n", block->getNumber());

    simplify(block);

    for (auto cursor = _performLowerTreeNodePairs.begin();
         cursor != _performLowerTreeNodePairs.end(); ++cursor) {
      auto treeNodePair = *cursor;
      if (trace())
        traceMsg(comp(),
                 "process _performLowerTreeNodePairs treetop %p node %p\n",
                 treeNodePair.first, treeNodePair.second);
      TR::Node *performLowerNode =
          postWalkLowerTreeSimplifier(treeNodePair.first, treeNodePair.second,
                                      block, (TR::Simplifier *)this);
  ...
  if (_blockRemoved) {
    ...
    if (treeTop) {
      ...
      if (b->nodeIsRemoved()) treeTop = findNextLegalTreeTop(comp(), b);
    }
  }
...
TR::TreeTop *OMR::Simplifier::simplify(TR::TreeTop *treeTop, TR::Block *block) {
  ...
  //
  if (node == NULL && (!block->getPredecessors().empty() ||
...
TR::Node *OMR::Simplifier::simplify(TR::Node *node, TR::Block *block) {
  ...
  //
  TR::Node *newNode = simplifierOpts[node->getOpCodeValue()](
      node, block, (TR::Simplifier *)this);
  if ((node != newNode) ||
      (newNode && ((newNode->getOpCodeValue() != node->getOpCodeValue()) ||
...
TR::Node *OMR::Simplifier::unaryCancelOutWithChild(TR::Node *node,
                                                   TR::Node *firstChild,
                                                   TR::TreeTop *anchorTree,
                                                   TR::ILOpCodes opcode,
                                                   bool anchorChildren) {
  if (!isLegalToUnaryCancel(node, firstChild, opcode)) return NULL;
  ...
  if (firstChild->getOpCodeValue() == opcode &&
      ...
      (node->getSize() > firstChild->getSize() ||
       node->getSize() != firstChild->getFirstChild()->getSize())) {
    // ensure a truncation side-effect of a conversion is not lost
    // o2a size=3
    //   a2o size=3 // conversion truncates in addition to type cast so cannot
    //   be removed
    //     loadaddr size=4
    // This restriction could be loosened to only disallow intermediate
    // truncations (see BCD case above) but then would require a node op that
    // would just correct for size (e.g. addrSizeMod size=3 to replace the
    // o2a/a2o pair)
    //
    // Do allow cases when all three sizes are the same and when the middle node
    // widens but the top and bottom node have the same size, e.g.
    //
    // i2o size=3
    //   o2i size=4
    //     oload size=3
    //
    // Also allow the special case where the grandchild is not really truncated
    // as the 'truncated' bytes are known to be zero (i.e. there really isn't an
    // intermediate truncation of 4->3 even though it appears that way from
    ...
    if (node->getType().isIntegral() && nodeSize == grandChild->getSize() &&
        nodeSize > firstChild->getSize()) {
      size_t truncatedBits = (nodeSize - firstChild->getSize()) * 8;
      if (grandChild->getOpCode().isRightShift() &&
          grandChild->getOpCode().isShiftLogical() &&
          grandChild->getSecondChild()->getOpCode().isLoadConst() &&
          (grandChild->getSecondChild()->get64bitIntegralValue() ==
           truncatedBits)) {
        disallow = false;
        if (trace())
          traceMsg(comp(),
                   "do allow unaryCancel of node %s (%p) and firstChild %s "
                   "(%p) as grandChild %s (%p) zeros the %d truncated bytes\n",
    ...
    if (disallow) {
      if (trace())
        traceMsg(comp(),
                 "disallow unaryCancel of node %s (%p) and firstChild %s (%p) "
                 "due to unequal sizes (nodeSize %d, firstChildSize %d, "
                 "firstChild->childSize %d)\n",
                 node->getOpCode().getName(), node,
                 firstChild->getOpCode().getName(), firstChild, node->getSize(),
  ...
  if (firstChild->getOpCodeValue() == opcode &&
      performTransformation(
          comp(),
          "%sRemoving node [" POINTER_PRINTF_FORMAT
          "] %s and its child [" POINTER_PRINTF_FORMAT "] %s\n",
          optDetailString(), node, node->getOpCode().getName(), firstChild,
          firstChild->getOpCode().getName())) {
    TR::Node *grandChild = firstChild->getFirstChild();
    grandChild->incReferenceCount();
    bool anchorChildrenNeeded =
        anchorChildren &&
        (node->getNumChildren() > 1 || firstChild->getNumChildren() > 1 ||
...
//
bool OMR::Simplifier::conditionalToUnconditional(TR::Node *&node,
                                                 TR::Block *block,
                                                 int takeBranch) {
  if (!performTransformation(comp(),
                             "%s change conditional to unconditional n%in\n",
                             optDetailString(), node->getGlobalIndex())) {
    return false;
  }

  TR::CFGEdge *removedEdge = changeConditionalToUnconditional(
      node, block, takeBranch, _curTree, optDetailString());
  bool blocksWereRemoved =
      removedEdge ? removedEdge->getTo()->nodeIsRemoved() : false;

  if (takeBranch) {
    TR_ASSERT(node->getOpCodeValue() == TR::Goto,
...
void OMR::Simplifier::anchorOrderDependentNodesInSubtree(
    TR::Node *node, TR::Node *replacement, TR::TreeTop *anchorTree) {
  if (node == replacement) return;

  if (nodeIsOrderDependent(node, 0, false)) {
    if (trace()) traceMsg(comp(), "anchor detached node %p\n", node);

    generateAnchor(node, anchorTree);
  } else
    anchorChildren(node, anchorTree, 0, node->getReferenceCount() > 1,
                   replacement);
}

bool OMR::Simplifier::isBoundDefinitelyGELength(TR::Node *boundChild,
                                                TR::Node *lengthChild) {
  ...
  if (boundOp == TR::iadd) {
    ...
    if (first == lengthChild) {
      ...
      if (second->getOpCode().isArrayLength() || secondOp == TR::bu2i ||
          secondOp == TR::su2i ||
      ...
      if (first->getOpCode().isArrayLength() || firstOp == TR::bu2i ||
          firstOp == TR::su2i ||
...
const char *OMR::Simplifier::optDetailString() const throw() {
