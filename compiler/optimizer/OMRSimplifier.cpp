...
static_assert(
  TR::NumIlOps == (sizeof(simplifierOpts) / sizeof(simplifierOpts[0])), "simplifierOpts is not the correct size");
...
TR::TreeTop* OMR::Simplifier::simplifyExtendedBlock(TR::TreeTop* treeTop)
{
  ...
  for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
    ...
    for (auto cursor = _performLowerTreeNodePairs.begin(); cursor != _performLowerTreeNodePairs.end(); ++cursor) {
      ...
      if (trace())
        traceMsg(
          comp(), "process _performLowerTreeNodePairs treetop %p node %p\n", treeNodePair.first, treeNodePair.second);
  ...
  TR::Node* node, TR::Node* firstChild, TR::TreeTop* anchorTree, TR::ILOpCodes opcode, bool anchorChildren)
{
  ...
  if (firstChild->getOpCodeValue() == opcode && (node->getType().isAggregate() || firstChild->getType().isAggregate())
    ...
    //     loadaddr size=4
    // This restriction could be loosened to only disallow intermediate truncations (see BCD case above) but then would
    // require a node op that would just correct for size (e.g. addrSizeMod size=3 to replace the o2a/a2o pair)
    //
    // Do allow cases when all three sizes are the same and when the middle node widens but the top and bottom node have
    // the same size, e.g.
    //
    // i2o size=3
    //   o2i size=4
    //     oload size=3
    //
    // Also allow the special case where the grandchild is not really truncated as the 'truncated' bytes are known to be
    // zero (i.e. there really isn't an intermediate truncation of 4->3 even though it appears that way from looking at
    // the sizes alone) o2i
    //   i2o size=3
    //     iushr
    //       x
    ...
    if (node->getType().isIntegral() && nodeSize == grandChild->getSize() && nodeSize > firstChild->getSize()) {
      ...
      if (grandChild->getOpCode().isRightShift() && grandChild->getOpCode().isShiftLogical()
        ...
        if (trace())
          traceMsg(comp(),
            "do allow unaryCancel of node %s (%p) and firstChild %s (%p) as grandChild %s (%p) zeros the %d truncated "
            "bytes\n",
