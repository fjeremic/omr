...
TR::Node*
OMR::TransformUtil::scalarizeArrayCopy(TR::Compilation* comp,
                                       TR::Node* node,
                                       TR::TreeTop* tt,
                                       bool useElementType,
                                       ...
                                       bool castToIntegral)
{
  ...
  if (trace)
    traceMsg(
      ...
      node,
      targetRef ? targetRef->getReferenceNumber() : -1,
  ...
  if (performTransformation(
        comp, "%sScalarize arraycopy 0x%p\n", OPT_DETAILS, node)) {
  ...
  bool store)
{
  ...
  size_t byteLength =
    dataType.isBCD()
      ? TR::DataType::getSizeFromBCDPrecision(dataType, byteLengthOrPrecision)
...
//
void
OMR::TransformUtil::recursivelySetNodeVisitCount(TR::Node* node,
                                                 vcount_t visitCount)
{
...
bool
OMR::TransformUtil::fieldShouldBeCompressed(TR::Node* node,
                                            TR::Compilation* comp)
{
  return false;
}
...
void
OMR::TransformUtil::transformCallNodeToPassThrough(TR::Optimization* opt,
                                                   TR::Node* node,
                                                   ...
                                                   bool markCold)
{
  ...
  elseBlock = ifBlock->split(ifTree->getNextTreeTop(),
                             cfg,
  ...
  thenBlock->append(TR::TreeTop::create(
    comp,
    TR::Node::create(
      thenTree->getNode(), TR::Goto, 0, mergeBlock->getEntry())));
