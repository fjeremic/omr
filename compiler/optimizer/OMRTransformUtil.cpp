...
TR::TransformUtil *OMR::TransformUtil::self() {
  return static_cast<TR::TransformUtil *>(this);
}

TR::Node *OMR::TransformUtil::scalarizeArrayCopy(
    TR::Compilation *comp, TR::Node *node, TR::TreeTop *tt, bool useElementType,
    bool &didTransformArrayCopyNode, TR::SymbolReference *sourceRef,
    TR::SymbolReference *targetRef, bool castToIntegral) {
  TR::CodeGenerator *cg = comp->cg();

  didTransformArrayCopyNode = false;

  if ((comp->getOptLevel() == noOpt) || !comp->getOption(TR_ScalarizeSSOps) ||
      node->getOpCodeValue() != TR::arraycopy || node->getNumChildren() != 3 ||
  ...
  if (trace)
    traceMsg(
        comp,
        "scalarizeArrayCopy: node %p got targetRef (#%d) and sourceRef (#%d)\n",
        node, targetRef ? targetRef->getReferenceNumber() : -1,
        sourceRef ? sourceRef->getReferenceNumber() : -1);

  if (targetRef == NULL || sourceRef == NULL) {
    if (trace)
      traceMsg(comp,
               "do not scalarizeArrayCopy node %p : targetRef is NULL (%s) or "
               "sourceRef is NULL (%s)\n",
...
TR::Node *OMR::TransformUtil::scalarizeAddressParameter(
    TR::Compilation *comp, TR::Node *address,
    size_t byteLengthOrPrecision,  // precision for BCD types and byteLength for
                                   // all other types
    TR::DataType dataType, TR::SymbolReference *ref, bool store) {
  TR_ASSERT(ref,
            "symRef should not be NULL in scalarizeAddressParameter for "
            "address node %p\n",
            address);
...
#ifdef J9_PROJECT_SPECIFIC
  size_t byteLength = dataType.isBCD() ? TR::DataType::getSizeFromBCDPrecision(
                                             dataType, byteLengthOrPrecision)
  ...
  if (address->getOpCodeValue() == TR::loadaddr &&
      !address->getSymbol()->isStatic() && isLengthValidForDirect &&
    ...
    TR::ILOpCodes opcode = store
                               ? comp->il.opCodeForIndirectArrayStore(dataType)
...
#endif
      if (!store && loadOrStore->getType().isIntegral() &&
          !loadOrStore->getType().isInt64()) {
...
TR::Node *OMR::TransformUtil::transformIndirectLoad(TR::Compilation *,
                                                    TR::Node *node) {
  return NULL;
}

bool OMR::TransformUtil::isNoopConversion(TR::Compilation *comp,
                                          TR::Node *node) {
  if (node->getOpCodeValue() == TR::i2a && node->getSize() == 4) return true;
  if (node->getOpCodeValue() == TR::a2i &&
      node->getFirstChild()->getSize() == 4)
    return true;
  if (node->getOpCodeValue() == TR::l2a && node->getSize() == 8) return true;
  if (node->getOpCodeValue() == TR::a2l &&
      node->getFirstChild()->getSize() == 8)
    return true;

  // address truncations
  if (node->getOpCodeValue() == TR::lu2a && node->getSize() <= 8) return true;
  if (node->getOpCodeValue() == TR::iu2a && node->getSize() <= 4) return true;
  if (node->getOpCodeValue() == TR::su2a && node->getSize() <= 2) return true;
...
//
void OMR::TransformUtil::recursivelySetNodeVisitCount(TR::Node *node,
                                                      vcount_t visitCount) {
...
bool OMR::TransformUtil::transformDirectLoad(TR::Compilation *,
                                             TR::Node *node) {
  return false;
}

bool OMR::TransformUtil::transformIndirectLoadChain(
    TR::Compilation *comp, TR::Node *node, TR::Node *baseExpression,
    TR::KnownObjectTable::Index baseKnownObject, TR::Node **removedNode) {
  return false;
}

bool OMR::TransformUtil::transformIndirectLoadChainAt(
    TR::Compilation *comp, TR::Node *node, TR::Node *baseExpression,
    uintptrj_t *baseReferenceLocation, TR::Node **removedNode) {
  return false;
}

bool OMR::TransformUtil::fieldShouldBeCompressed(TR::Node *node,
                                                 TR::Compilation *comp) {
  return false;
}
...
void OMR::TransformUtil::transformCallNodeToPassThrough(TR::Optimization *opt,
                                                        TR::Node *node,
...
void OMR::TransformUtil::createConditionalAlternatePath(
    TR::Compilation *comp, TR::TreeTop *ifTree, TR::TreeTop *thenTree,
    TR::Block *elseBlock, TR::Block *mergeBlock, TR::CFG *cfg, bool markCold) {
  cfg->setStructure(0);
  ...
  elseBlock =
      ifBlock->split(ifTree->getNextTreeTop(), cfg, false /*fixupCommoning*/,
                     true /*copyExceptionSuccessors*/);

  // Since `elseBlock` is changed above, update `mergeBlock`
  if (mergeToElseBlock) mergeBlock = elseBlock;

  TR::Block *thenBlock =
      TR::Block::createEmptyBlock(thenTree->getNode(), comp, 0, elseBlock);
  if (markCold) {
  ...
  thenBlock->append(
      TR::TreeTop::create(comp, TR::Node::create(thenTree->getNode(), TR::Goto,
                                                 0, mergeBlock->getEntry())));
  ifTree->getNode()->setBranchDestination(thenBlock->getEntry());
  cfg->addEdge(
      TR::CFGEdge::createEdge(thenBlock, mergeBlock, comp->trMemory()));
