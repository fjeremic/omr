...
TR::Node* OMR::TransformUtil::scalarizeArrayCopy(TR::Compilation* comp, TR::Node* node, TR::TreeTop* tt,
  bool useElementType, bool& didTransformArrayCopyNode, TR::SymbolReference* sourceRef, TR::SymbolReference* targetRef,
  bool castToIntegral)
{
  ...
  if (trace)
    traceMsg(comp, "scalarizeArrayCopy: node %p got targetRef (#%d) and sourceRef (#%d)\n", node,
      targetRef ? targetRef->getReferenceNumber() : -1, sourceRef ? sourceRef->getReferenceNumber() : -1);
  ...
  if (targetRef == NULL || sourceRef == NULL) {
    if (trace)
      traceMsg(comp, "do not scalarizeArrayCopy node %p : targetRef is NULL (%s) or sourceRef is NULL (%s)\n", node,
        targetRef ? "no" : "yes", sourceRef ? "no" : "yes");
    return node;
  }
  ...
  if (performTransformation(comp, "%sScalarize arraycopy 0x%p\n", OPT_DETAILS, node)) {
    TR::Node* store
      = TR::TransformUtil::scalarizeAddressParameter(comp, node->getSecondChild(), byteLen, dataType, targetRef, true);
    TR::Node* load
      = TR::TransformUtil::scalarizeAddressParameter(comp, node->getFirstChild(), byteLen, dataType, sourceRef, false);
...
#ifdef J9_PROJECT_SPECIFIC
  size_t byteLength
    = dataType.isBCD() ? TR::DataType::getSizeFromBCDPrecision(dataType, byteLengthOrPrecision) : byteLengthOrPrecision;
#else
...
TR::Node* OMR::TransformUtil::transformIndirectLoad(TR::Compilation*, TR::Node* node)
{
  return NULL;
}
