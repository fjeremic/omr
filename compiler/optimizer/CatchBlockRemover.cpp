...
int32_t TR_CatchBlockRemover::perform() {
  ...
  if (cfg == NULL) {
    if (trace()) traceMsg(comp(), "Can't do Catch Block Removal, no CFG\n");
    return 0;
  }

  if (trace()) traceMsg(comp(), "Starting Catch Block Removal\n");
...

  {
    ...
    // Go through all blocks that have exception successors and see if any of
    // them are not reached. Mark each of these edges with a visit count so they
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
      if (cfgNode->getExceptionSuccessors().empty()) continue;

      block = toBlock(cfgNode);
      uint32_t reachedExceptions = 0;
      TR::TreeTop *treeTop;
      for (treeTop = block->getEntry(); treeTop != block->getExit();
           treeTop = treeTop->getNextTreeTop()) {
        reachedExceptions |= treeTop->getNode()->exceptionsRaised();

        if (treeTop->getNode()->getOpCodeValue() ==
            TR::monexitfence)  // for live monitor metadata
          reachedExceptions |= TR::Block::CanCatchMonitorExit;
      }

      if (reachedExceptions & TR::Block::CanCatchUserThrows) continue;
      ...
      for (auto edge = block->getExceptionSuccessors().begin();
        ...
        if (!reachedExceptions &&
            performTransformation(
                comp(),
                "%sRemove redundant exception edge from block_%d at [%p] to "
                "catch block_%d at [%p]\n",
                optDetailString(), block->getNumber(), block,
    ...
    // predecessors have the visit count set. If so, the block is unreachable
    // and can be removed. If only some of the exception predecessors are
    // marked, these edges are left in place to identify the try/catch structure
    // properly.
    //
    while (thereMayBeRemovableCatchBlocks) {
      thereMayBeRemovableCatchBlocks = false;
      for (cfgNode = cfg->getFirstNode(); cfgNode;
           cfgNode = cfgNode->getNext()) {
        if (cfgNode->getExceptionPredecessors().empty()) continue;
        auto edgeIt = cfgNode->getExceptionPredecessors().begin();
        for (; edgeIt != cfgNode->getExceptionPredecessors().end(); ++edgeIt) {
          if ((*edgeIt)->getVisitCount() != visitCount) break;
        }

        if (edgeIt == cfgNode->getExceptionPredecessors().end() &&
            performTransformation(
                comp(), "%sRemove redundant catch block_%d at [%p]\n",
...
const char *TR_CatchBlockRemover::optDetailString() const throw() {
