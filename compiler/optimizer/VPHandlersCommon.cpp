...
TR::Node* constrainVcall(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (node->getSymbolReference() == finalizeSymRef) {
        ...
        if (canBeRemoved && performTransformation(vp->comp(), "%s Removing redundant call to jitCheckIfFinalize [%p]\n", OPT_DETAILS, node)) {
...
static_assert(TR::NumIlOps == (sizeof(constraintHandlers) / sizeof(constraintHandlers[0])),
