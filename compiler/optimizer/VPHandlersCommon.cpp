...
TR::Node*
constrainChildrenFirstToLast(OMR::ValuePropagation* vp, TR::Node* node)
{
  TR::Node* myParent = vp->getCurrentParent();
...
constrainVcall(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  // Look for System.arraycopy call. If the node is transformed into an
  // arraycopy re-process it.
  //
  ...
  TR::SymbolReference* finalizeSymRef =
    vp->comp()->getSymRefTab()->findOrCreateRuntimeHelper(
  ...
  if (node->getSymbolReference() == finalizeSymRef) {
    ...
    // If a class has a finalizer or is an ownableSync it won't be allocated on
    // the stack. That's ensured by virtue of (indirectly) calling bool
    // J9::ObjectModel::canAllocateInlineClass(TR_OpaqueClassBlock *block)
    // Doesn't make sense to call jitCheckIfFinalizeObject for a stack
    // allocated object, so optimize
    else if (receiver->getOpCode().hasSymbolReference() &&
             receiver->getSymbol()->isLocalObject()) {
      canBeRemoved = true;
    }
    ...
    if (canBeRemoved &&
        performTransformation(
          vp->comp(),
          "%s Removing redundant call to jitCheckIfFinalize [%p]\n",
          OPT_DETAILS,
          node)) {
      /// printf("found opportunity in %s to remove call to checkfinalize\n",
      /// vp->comp()->signature());fflush(stdout); traceMsg(vp->comp(), "found
      /// opportunity to remove call %p to checkfinalize\n", node);
