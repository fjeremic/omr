...
TR::Node *constrainChildrenFirstToLast(OMR::ValuePropagation *vp,
                                       TR::Node *node) {
  TR::Node *myParent = vp->getCurrentParent();
...
TR::Node *constrainVcall(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (vp->transformUnsafeCopyMemoryCall(node)) return node;
  ...
  if (node->getSymbolReference() == finalizeSymRef) {
    ...
    if (canBeRemoved &&
      ...
      TR::TransformUtil::transformCallNodeToPassThrough(vp, node, vp->_curTree,
                                                        receiver);
      return node;
    }
  }
