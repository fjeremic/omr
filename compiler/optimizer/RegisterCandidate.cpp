...
void TR_RegisterCandidate::addAllBlocksInStructure(
...
TR_RegisterCandidate* TR_RegisterCandidates::findOrCreate(TR::SymbolReference* symRef)
{
...
TR_RegisterCandidate* TR_RegisterCandidates::find(TR::SymbolReference* symRef)
{
  if (symRef->getSymbol()->isAutoOrParm()) {
    ...
    // two symrefs can point to same symbols so it is possible that we have an rc for symref->getSymbol but not have it
    // added to the cache
    if (rc == NULL) {
...
void TR_RegisterCandidate::addBlock(TR::Block* block, int32_t numberOfLoadsAndStores)
{
...
int32_t TR_RegisterCandidate::countNumberOfLoadsAndStoresInBlocks(List<TR::Block>* blocksInLoop)
{
...
void TR_RegisterCandidate::addLoop(TR_Structure* loop, int32_t numberOfLoadsAndStores, TR_Memory* m)
{
...
int32_t TR_RegisterCandidate::getNumberOfLoadsAndStoresInLoop(TR_Structure* loop)
{
  LoopInfo* li = find(loop);
  if (li)
...
void TR_RegisterCandidate::removeAssignedCandidateFromLoop(TR::Compilation* comp, TR_Structure* loop, int32_t regNum,
...
bool TR_RegisterCandidates::aliasesPreventAllocation(TR::Compilation* comp, TR::SymbolReference* symRef)
{
  ...
  if (!symRef->getSymbol()->isAutoOrParm() && !(TR::Compiler->target.cpu.isZ() && TR::Compiler->target.isLinux()))
    return true;
...
void TR_RegisterCandidate::setWeight(TR::Block** blocks, int32_t* blockStructureWeight, TR::Compilation* comp,
...
void TR_RegisterCandidate::recalculateWeight(TR::Block** blocks, int32_t* blockStructureWeight, TR::Compilation* comp,
  TR_Array<int32_t>& blockGPRCount, TR_Array<int32_t>& blockFPRCount, TR_Array<int32_t>& blockVRFCount,
...
// register pressue
bool TR_RegisterCandidate::prevBlockTooRegisterConstrained(TR::Compilation* comp, TR::Block* block,
  TR_Array<int32_t>& blockGPRCount, TR_Array<int32_t>& blockFPRCount, TR_Array<int32_t>& blockVRFCount)
{
...
bool TR_RegisterCandidate::rcNeeds2Regs(TR::Compilation* comp)
{
  if (getType().isAggregate()) {
    if ((TR::Compiler->target.is32Bit() && !comp->cg()->use64BitRegsOn32Bit() && getSymbol()->getSize() > 4)
      || (getSymbol()->getSize() > 8))
      return true;
    else
      return false;
...
bool TR_RegisterCandidate::hasSameGlobalRegisterNumberAs(TR::Node* node, TR::Compilation* comp)
{
...
void TR_RegisterCandidate::processLiveOnEntryBlocks(TR::Block** blocks, int32_t* blockStructureWeight,
  ...
  bool callToRemoveUnusedLoops)
{
    ...
    while (lookForBlocksToRemove) {
      ...
      if (trace)
        traceMsg(comp, "At frequency %d for %d numBlocks %d numUnreferencedBlocks %d\n", frequency,
          getSymbolReference()->getReferenceNumber(), numBlocksAtThisFrequency, numUnreferencedBlocksAtThisFrequency);
              ...
              if (!referencedInNearestLoop) {
                ...
                if (parent && !unreferencedLoops.find(parent)) {
                  ...
                  if (!comp->getOptimizer()->getResetExitsGRA())
                    comp->getOptimizer()->setResetExitsGRA(new (comp->trHeapMemory())
                  ...
                  for (TR::CFGEdge* edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
                    ...
                    int32_t fromFrequency
                      = edge->getFrom()->asStructureSubGraphNode()->getStructure()->getEntryBlock()->getFrequency();
                  ...
                  if (shouldResetThisLoop) {
                    if (trace) {
                      dumpOptDetails(comp,
                        "\n1Loop %d does not use candidate, removing the following blocks from live ranges of "
                        "candidate #%d \n",
                ...
                // frequency, getSymbolReference()->getReferenceNumber()); dumpOptDetails(comp, "loadsStores %d\n",
  ...
  while (bvi.hasMoreElements()) {
    ...
    for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
      ...
      if (((predWeight > blockWeight) && !liveOnExitFromPred && !_liveOnEntry.get(pred->getNumber()))
        ...
        bool reset = true;
        if (useProfilingFrequencies && (pred->getFrequency() > 0) && (4 * block->getFrequency() > pred->getFrequency()))
          reset = false;
  ...
  while (bvi.hasMoreElements()) {
    ...
    do {
      ...
      if (_blocks.getNumberOfLoadsAndStores(blockNumber) > 0) {
        ...
        for (auto succ = block->getSuccessors().begin(); succ != block->getSuccessors().end(); ++succ) {
          if (((*succ)->getTo() != block->getNextBlock()) && ((*succ)->getTo() != comp->getFlowGraph()->getEnd())) {
            if (!_liveOnEntry.get((*succ)->getTo()->getNumber())) {
              TR_BlockStructure* blockStructure = block->getStructureOf();
              int32_t blockWeight = 1;
              ...
              if (!exclude && blockStructure
                && (/* !dontAssignInColdBlocks(comp) || */ (!block->isCold() && !(*succ)->getTo()->asBlock()->isCold()))
                ...
                // blockWeight = (blockWeight + succWeight)/2; // maybe the store will be inserted in a split block in
                ...
                if (trace)
                  traceMsg(comp,
                    "Reduced weight of candidate %d by %d (new weight %d) because of end in live range at block_%d "
                    "succ %d\n",
                    getSymbolReference()->getReferenceNumber(), blockWeight, loadsAndStores, blockNumber,
          ...
          if ((*succ)->getTo() != block->getNextBlock()) {
            if (_liveOnEntry.get((*succ)->getTo()->getNumber())) {
              ...
              if (!exclude && blockStructure && (/* !dontAssignInColdBlocks(comp) || */ !block->isCold())
                ...
                if (trace)
                  traceMsg(comp,
                    "Reduced weight of candidate %d by %d (new weight %d) because of start of live range at block_%d "
                    "succ %d\n",
                    getSymbolReference()->getReferenceNumber(), blockWeight, loadsAndStores, blockNumber,
                    (*succ)->getTo()->getNumber());
                foundEndOfRange = true;
  ...
  bool isPPS = false; // (comp->getOption(TR_EnableOSR) && getSymbol()->isAuto() && (getSymbolReference()->getCPIndex()
      ...
      for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
        ...
        do {
          ...
          if (!_liveOnEntry.get(currBlock->getNumber()) && // not set already
            _liveOnExit.get(currBlock->getNumber()) && currBlock->isExtensionOfPreviousBlock() && blocksReferencing
...
void TR_RegisterCandidates::lookForCandidates(
...
bool TR_RegisterCandidates::prioritizeCandidate(TR_RegisterCandidate* newRC, TR_RegisterCandidate*& first)
{
...
TR_RegisterCandidate* TR_RegisterCandidates::reprioritizeCandidates(TR_RegisterCandidate* first, TR::Block** blocks,
  ...
  TR_Array<int32_t>& blockFPRCount, TR_Array<int32_t>& blockVRFCount, TR_BitVector* successorBits, bool trace)
{
  ...
  for (TR_RegisterCandidate* rc = first; rc; rc = next) {
...
#ifdef J9_PROJECT_SPECIFIC
      || rc->getDataType() == TR::DecimalFloat || rc->getDataType() == TR::DecimalDouble
    ...
    if (rc->getBlocksLiveOnEntry().intersects(*successorBits)) {
      if ((!onlyReprioritizeLongs || (rc->getType().isInt64() && TR::Compiler->target.is32Bit()))
...
static void scanPressureSimulatorCacheForConflicts(TR_RegisterCandidate* rc, TR_BitVector& blocks,
  ...
  TR::Compilation* comp, TR::Block** all_blocks)
{
  ...
  if (trace) {
    traceMsg(
      comp, "\tscanPressureSimulatorCacheForConflits: Candidate #%d\n", rc->getSymbolReference()->getReferenceNumber());
...
bool TR_RegisterCandidates::assign(
  TR::Block** cfgBlocks, int32_t numberOfBlocks, int32_t& lowestNumber, int32_t& highestNumber)
{
  ...
  for (; rc; rc = next) {
    ...
    rc->setWeight(blocks, blockStructureWeight, comp(), totalGPRCount, totalFPRCount, totalVRFCount, &referencedBlocks,
      _startOfExtendedBBForBB, _firstBlock, _isExtensionOfPreviousBlock);
    numCandidates++;
  ...
  for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next) {
    ...
    if (((catchBlockLiveLocalsExist && rc->getSymbolReference()->getSymbol()->isAuto()
           && catchBlockLiveLocals.get(rc->getSymbolReference()->getSymbol()->getAutoSymbol()->getLiveLocalIndex()))
      ...
      // TODO:GRA Decide if this fix is OK -- without it on zemulator we would have a useonlyalias set for metadata and
      // so we would set this propert
    ...
    if (true) {
      ...
      while (bvi.hasMoreElements()) {
        ...
        // b->getNumber(), maxGPRsLiveOnExit[b->getNumber()], maxFPRsLiveOnExit[b->getNumber()],  blockNum,
        // maxGPRsLiveOnExit[blockNum], maxFPRsLiveOnExit[blockNum], maxFPRsLiveOnExit[blockNum]); dumpOptDetails(comp(),
        // "For block_%d numGPRs %d numFPRs %d ext block_%d numGPRs %d numFPRs %d maxVRFs %d\n", b->getNumber(),
        // totalGPRCount[b->getNumber()], totalFPRCount[b->getNumber()],  blockNum, totalGPRCount[blockNum],
          ...
          if (totalFPRCount[blockNum] >= (maxFPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0))) {
            ...
            for (TR::CFGEdge* e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              ...
              if (hasMultiWayBranch
                || (totalFPRCountOnEntry[b->getNumber()] >= (maxFPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0)))) {
                if (trace)
                  dumpOptDetails(comp(),
                    "FPR count exceeded : resetting live on entry block_%d (because of extended block_%d)\n",
                    b->getNumber(), blockNum);
                rc->getBlocksLiveOnEntry().reset(b->getNumber());
                TR::Block* cursorBlock = problematicBlock;
                if (trace)
                  dumpOptDetails(
                    comp(), "FPR count exceeded : resetting live on exit block_%d)\n", cursorBlock->getNumber());

                rc->getBlocksLiveOnExit().reset(cursorBlock->getNumber());
                for (auto succ = cursorBlock->getSuccessors().begin(); succ != cursorBlock->getSuccessors().end();
                     ++succ) {
        ...
        if (isVector) {
          if (totalVRFCount[blockNum] >= maxVRFsLiveOnExit[blockNum]) {
            ...
            for (TR::CFGEdge* e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              ...
              if (hasMultiWayBranch || (totalVRFCountOnEntry[b->getNumber()] >= maxVRFsLiveOnExit[blockNum])) {
                if (trace)
                  dumpOptDetails(comp(),
                    "VRF count exceeded : resetting live on entry block_%d (because of extended block_%d)\n",
                    b->getNumber(), blockNum);
                rc->getBlocksLiveOnEntry().reset(b->getNumber());
                ...
                if (trace)
                  dumpOptDetails(
                    comp(), "VRF count exceeded : resetting live on exit block_%d)\n", cursorBlock->getNumber());
          ...
          if (totalGPRCount[blockNum] >= (maxGPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0))) {
            ...
            for (TR::CFGEdge* e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              TR::Block* b = e->getTo()->asBlock();
              if (hasMultiWayBranch
                || (totalGPRCountOnEntry[b->getNumber()] >= (maxGPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0)))) {
                if (trace)
                  dumpOptDetails(comp(),
                    "GPR count exceeded : resetting live on entry block_%d (because of extended block_%d)\n",
                    b->getNumber(), blockNum);
                rc->getBlocksLiveOnEntry().reset(b->getNumber());
                TR::Block* cursorBlock = problematicBlock;
                if (trace)
                  dumpOptDetails(
                    comp(), "GPR count exceeded : resetting live on exit block_%d)\n", cursorBlock->getNumber());
      ...
      for (int i = firstRegister; i <= lastRegister; ++i) {
        if (vrbVecGRA && (cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i)) {
          dumpOptDetails(comp(), "ed: alias for %d %s %d %s\t", i, cg->getDebug()->getGlobalRegisterName(i),
            cg->getOverlappedAliasForGRN(i), cg->getDebug()->getGlobalRegisterName(cg->getOverlappedAliasForGRN(i)));
        ...
        if ((cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i)
          ...
          if (trace) {
            traceMsg(comp(), "removing %s (%d) from available list since it is aliased with %s (%d)\n",
              cg->getDebug()->getGlobalRegisterName(i), i, cg->getDebug()->getGlobalRegisterName(overlappedReg),
              overlappedReg);
          }
    ...
    if ((registerNumber == -1) || (needs2Regs && (highRegisterNumber == -1))) {
      ...
      //
        ...
        const bool useRegisterPressureInfo = !comp()->getOption(TR_DisableRegisterPressureSimulation)
          ...
          && (!trace
            || performTransformation(comp(), "%s Including register pressure info for trimming live range of #%d\n",
        ...
        if (useRegisterPressureInfo) {
          ...
          scanPressureSimulatorCacheForConflicts(rc, liveOnEntryOrExit, _liveOnEntryConflicts, _liveOnExitConflicts,
            _entryExitConflicts, firstRegister, lastRegister, &spilledRegs, comp(), blocks);
        }
        ...
        for (i = firstRegister; i <= lastRegister; ++i) {
          ...
          if (needs2Regs) {
            ...
            if (trace) {
              ...
              traceMsg(
                comp(), "num clear structures %d; max clear structures %d\n", numClearStructures, maxClearStructures);
        ...
        if (conflictingRegister != -1) {
          ...
          if (trace) {
            dumpOptDetails(comp(),
              "\nDue to conflicts, removing the following blocks from live on entry ranges of candidate #%d "
              "conflicting register %d\n",
            ...
            while (bvi.hasMoreElements()) {
              ...
              if (cand) {
                cand->removeAssignedCandidateFromLoop(comp(), loop, conflictingRegister,
                  &_liveOnEntryUsage[conflictingRegister], &_liveOnExitUsage[conflictingRegister], trace);
              }
            }
          }
#endif
          ...
          rc->recalculateWeight(blocks, blockStructureWeight, comp(), totalGPRCount, totalFPRCount, totalVRFCount,
            &referencedBlocks, _startOfExtendedBBForBB);
    ...
    if (needs2Regs) {
      ...
      if (!performTransformation(comp(), "%s assign auto #%d %s to reg %d (%s)\n", OPT_DETAILS,
            ...
            rc->getSymbolReference()->getSymbol()->isMethodMetaData()
              ? rc->getSymbolReference()->getSymbol()->castToMethodMetaDataSymbol()->getName()
              : "",
            registerNumber, comp()->getDebug() ? comp()->getDebug()->getGlobalRegisterName(registerNumber) : "?"))
        continue;
    }
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (isFloat) {
        if (++numberOfFPRsLiveOnExit[blockNumber] == maxFPRsLiveOnExit[blockNumber]) {
          ...
          next = reprioritizeCandidates(next, blocks, blockStructureWeight, numberOfBlocks, b, comp(), true, false,
            &referencedBlocks, totalGPRCount, totalFPRCount, totalVRFCount,
      ...
      if (isVector) {
        if (++numberOfVRFsLiveOnExit[blockNumber] == maxVRFsLiveOnExit[blockNumber]) {
          ...
          next = reprioritizeCandidates(next, blocks, blockStructureWeight, numberOfBlocks, b, comp(), false, false,
            &referencedBlocks, totalGPRCount, totalFPRCount, totalVRFCount,
            comp()->getOptimizer()->getSuccessorBitsGRA(), trace);
        }
      } else {
          ...
          == (maxGPRsLiveOnExit[blockNumber] - 1)) // only long cands need to be reprioritized
        {
          ...
          next = reprioritizeCandidates(next, blocks, blockStructureWeight, numberOfBlocks, b, comp(), false, true,
            &referencedBlocks, totalGPRCount, totalFPRCount, totalVRFCount,
            comp()->getOptimizer()->getSuccessorBitsGRA(), trace);
        }
      }
...
void ComputeOverlaps(
  TR::Node* node, TR::Compilation* comp, TR_RegisterCandidates::Coordinates& overlaps, uint32_t& seqno)
{
...
void ComputeOverlaps(TR::Block* block, TR::Compilation* comp, TR_RegisterCandidates::Coordinates& overlaps)
{
  uint32_t seqno = 0;
  comp->incVisitCount();
...
void TR_RegisterCandidates::computeAvailableRegisters(TR_RegisterCandidate* rc, int32_t firstRegister,
  ...
  for (i = firstRegister; i <= lastRegister; ++i) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (overlapLookup == overlapTable->end()) {
        overlaps = new (comp()->trMemory()->currentStackRegion())
          Coordinates((CoordinatesComparator()), (CoordinatesAllocator(comp()->trMemory()->currentStackRegion())));
      ...
      if (rc1) {
        ...
        if (rc1Itr == overlaps->end() || rcItr == overlaps->end() || rc1Itr->second.last < rcItr->second.first) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (rc2) {
        ...
        if (rcItr == overlaps->end() || rc2Itr == overlaps->end() || rcItr->second.last < rc2Itr->second.first) {
