...
void TR_RegisterCandidate::removeAssignedCandidateFromLoop(TR::Compilation* comp, TR_Structure* loop, int32_t regNum, TR_BitVector* liveOnEntryUsage, TR_BitVector* liveOnExitUsage, bool trace)
{
    List<TR::Block> blocksInLoop(comp->trMemory());
    loop->getBlocks(&blocksInLoop);
    ListIterator<TR::Block> blocksIt(&blocksInLoop);
    ...
    TR_BitVector& isExtensionOfPreviousBlock)
{
    ...
    while (itr.hasMoreElements()) {
        ...
        if ((firstBlock && isAllBlocks() && cg->getSupportsGlRegDepOnFirstBlock()) || (!firstBlock && (((blockWeight == 0) && hasLoopExitBlock(b)) || symbolIsLive(b))))
...
void TR_RegisterCandidate::processLiveOnEntryBlocks(TR::Block** blocks, int32_t* blockStructureWeight, TR::Compilation* comp,
    TR_Array<int32_t>& blockGPRCount,
    TR_Array<int32_t>& blockFPRCount,
    TR_Array<int32_t>& blockVRFCount,
    ...
    TR_Array<TR::Block*>& startOfExtendedBBForBB, bool callToRemoveUnusedLoops)
{
            ...
            if (!referencedInPred)
                ...
                //if (toBlock(e->getFrom())->startOfExtendedBlock()->findFirstReference(sym, true))
                {
                    ...
                    while (parentPredBlock) {
                        ...
                        if (parentPredBlock == predBlock || (parentPredBlock->getNextBlock() && (parentPredBlock->getNextBlock() == block || !parentPredBlock->getNextBlock()->isExtensionOfPreviousBlock())))
                            break;
    ...
    if ((maxFrequency > 1) && removeUnusedLoops) {
        ...
        while (lookForBlocksToRemove) {
            ...
            if ((numUnreferencedBlocksAtThisFrequency > 0) && (comp->getOption(TR_EnableRangeSplittingGRA) || // move this check above ?
                ...
                while (bvi.hasMoreElements()) {
                    ...
                    if (blockWeight == frequency) {
                        ...
                        if (!referencedInNearestLoop && ((_blocks.getNumberOfLoadsAndStores(blockNumber) == 0) || (dontAssignInColdBlocks(comp) && block->isCold()) /*||
                                                                            block->isRare(comp) */
                                                            )) {
                            referencedInNearestLoop = false;
                            TR_RegionStructure* parent = blockStructure->getParent()->asRegion();
                                        ...
                                        for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
                                            if (nextBlock->getEntry()) {
                                                ...
                                                if (_liveOnEntry.get(liveBlock->getNumber())) {
                                                    if ((_blocks.getNumberOfLoadsAndStores(blockNumber) == 0) && (!dontAssignInColdBlocks(comp) || !nextBlock->isCold())) {
    ...
    while (bvi.hasMoreElements()) {
        ...
        for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
            ...
            if (((predWeight > blockWeight) && !liveOnExitFromPred && !_liveOnEntry.get(pred->getNumber())) && _blocks.getNumberOfLoadsAndStores(startOfExtendedBBForBB[pred->getNumber()]->getNumber()) == 0 && _blocks.getNumberOfLoadsAndStores(pred->getNumber()) == 0) {
                bool reset = true;
                ...
                for (auto succ = block->getSuccessors().begin(); succ != block->getSuccessors().end(); ++succ) {
                    if (((*succ)->getTo() != block->getNextBlock()) && ((*succ)->getTo() != comp->getFlowGraph()->getEnd())) {
                        if (!_liveOnEntry.get((*succ)->getTo()->getNumber())) {
                            ...
                            if (!exclude && blockStructure && (/* !dontAssignInColdBlocks(comp) || */ (!block->isCold() && !(*succ)->getTo()->asBlock()->isCold())) && !comp->getOptimizer()->getSeenBlocksGRA()->get(block->getNumber()) && ( // symbolIsLive(block) ||
    ...
    while (bvi.hasMoreElements()) {
        ...
        if (!_liveOnEntry.get(extBlockNumber) && (block != comp->getFlowGraph()->getStart()) && (_blocks.getNumberOfLoadsAndStores(extBlockNumber) == 0)) {
            //do {
            if ((_blocks.getNumberOfLoadsAndStores(extBlockNumber) == 0) && (_blocks.getNumberOfLoadsAndStores(blockNumber) == 0)) {
                bool foundEndOfRange = false;
                for (auto succ = block->getSuccessors().begin(); succ != block->getSuccessors().end(); ++succ) {
...
//
void TR_RegisterCandidate::extendLiveRangesForLiveOnExit(TR::Compilation* comp, TR::Block** blocks, TR_Array<TR::Block*>& startOfExtendedBBForBB)
{
            ...
            for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
                ...
                do {
                    ...
                    if (!_liveOnEntry.get(currBlock->getNumber()) && // not set already
                        _liveOnExit.get(currBlock->getNumber()) && currBlock->isExtensionOfPreviousBlock() && blocksReferencing && blocksReferencing->get(currBlock->getPrevBlock()->getNumber()) && !blocksReferencing->get(currBlock->getNumber())) {
                        if (trace)
...
bool TR_RegisterCandidates::prioritizeCandidate(
    ...
    bool trace)
{
    ...
    for (TR_RegisterCandidate* rc = first; rc; rc = next) {
        ...
        if (rc->getBlocksLiveOnEntry().intersects(*successorBits)) {
            if ((!onlyReprioritizeLongs || (rc->getType().isInt64() && TR::Compiler->target.is32Bit())) && ((reprioritizeFP && isFPCandidate) || (!reprioritizeFP && !isFPCandidate))) {
                if (trace) {
...
bool TR_RegisterCandidates::assign(TR::Block** cfgBlocks, int32_t numberOfBlocks, int32_t& lowestNumber, int32_t& highestNumber)
{
        ...
        if (node->getOpCode().isStoreDirect()) {
            if (node->getType().isInt64()) {
                if (node->getSymbolReference()->getSymbol()->isAutoOrParm() || (0 && node->getSymbolReference()->getSymbol()->isMethodMetaData())) {
                    if ((!node->getFirstChild()->isHighWordZero() && !node->getFirstChild()->getOpCode().isLoadConst()) || (node->getFirstChild()->getOpCode().isLoadConst() && (node->getFirstChild()->getLongInt() & (((uint64_t)0xffffffff) << 32))))
    ...
    for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next) {
        ...
        if (((catchBlockLiveLocalsExist && rc->getSymbolReference()->getSymbol()->isAuto() && catchBlockLiveLocals.get(rc->getSymbolReference()->getSymbol()->getAutoSymbol()->getLiveLocalIndex())) || ((!catchBlockLiveLocalsExist || !rc->getSymbolReference()->getSymbol()->isAuto()) && !rc->getSymbolReference()->getUseonlyAliases().isZero(comp()))) && (!comp()->penalizePredsOfOSRCatchBlocksInGRA() || ((catchBlockLiveLocalsExist && rc->getSymbolReference()->getSymbol()->isAuto() && nonOSRCatchBlockLiveLocals.get(rc->getSymbolReference()->getSymbol()->getAutoSymbol()->getLiveLocalIndex())) || ((!catchBlockLiveLocalsExist || !rc->getSymbolReference()->getSymbol()->isAuto()) && !comp()->getSymRefTab()->aliasBuilder.hasUseonlyAliasesOnlyDueToOSRCatchBlocks(rc->getSymbolReference())))))
        ...
        if (enableVectorGRA) {
            ...
            for (int i = firstRegister; i <= lastRegister; ++i) {
                ...
                if ((cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i) && (/*!availableRegisters.isSet(i) */ !availableRegisters.isSet(cg->getOverlappedAliasForGRN(i)))) {
                    ...
                    if (needs2Regs) {
                        ...
                        for (j = firstRegister; j < i; ++j)
                            if (!availableRegisters.get(j)) {
                                ...
                                if (conflictingRegister == -1 || ((numBlocksAtMaxFrequency < numberOfConflicts) || ((numBlocksAtMaxFrequency == numberOfConflicts) && (intersectionElemCount < intersection.elementCount())))) {
                        ...
                        if (conflictingRegister == -1 || maxFrequency < smallestMaxFrequency || ((maxFrequency == smallestMaxFrequency) && ((numClearStructures > maxClearStructures) || ((numClearStructures == maxClearStructures) && ((numBlocksAtMaxFrequency < numberOfConflicts) || ((numBlocksAtMaxFrequency == numberOfConflicts) && (_liveOnEntryConflicts[i].elementCount() < _liveOnEntryConflicts[conflictingRegister].elementCount()))))))) {
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnExit() != rc) {
                if ((b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnExit() != NULL) && (b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnExit() != b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnEntry()) && (rc != b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnEntry())) {
                ...
                } else if ((rc != b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnEntry()) || (b->getGlobalRegisters(comp())[registerNumber].getRegisterCandidateOnExit() == NULL)) {
            ...
            if (needs2Regs) {
                if (b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit() != rc) {
                    if ((b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit() != NULL) && (b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit() != b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnEntry()) && (rc != b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnEntry())) {
                        ...
                        TR_ASSERT(0, "Candidate is required on exit out of block\n");
                    } else if ((rc != b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnEntry()) || (b->getGlobalRegisters(comp())[highRegisterNumber].getRegisterCandidateOnExit() == NULL))
...
void TR_RegisterCandidates::computeAvailableRegisters(TR_RegisterCandidate* rc, int32_t firstRegister, int32_t lastRegister, TR::Block** blocks, TR_BitVector* availableRegisters)
{
    ...
    for (i = firstRegister; i <= lastRegister; ++i) {
        ...
        if (liveOnEntryConflicts.isEmpty() && liveOnExitConflicts.isEmpty() && exitEntryConflicts.isEmpty() && entryExitConflicts.isEmpty() && comp()->cg()->isGlobalRegisterAvailable(i, rc->getDataType())) {
