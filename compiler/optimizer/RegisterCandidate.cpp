...
void
TR::GlobalSet::collectReferencedAutoSymRefs(TR::Node* node,
                                            ...
                                            TR::NodeChecklist& visited)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i)
    collectReferencedAutoSymRefs(
      node->getChild(i), referencedAutoSymRefs, visited);
}
...
void
TR_RegisterCandidate::addAllBlocksInStructure(TR_Structure* structure,
                                              TR::Compilation* comp,
                                              ...
                                              bool recursiveCall)
{
  ...
  if (structure->asBlock() != NULL) {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext())
      addAllBlocksInStructure(
        node->getStructure(), comp, description, count, true);
  }
}
...
void
TR_RegisterCandidate::removeLoopExitBlock(TR::Block* b)
{
  _loopExitBlocks.remove(b);
}

void
TR_RegisterCandidate::addLoopExitBlock(TR::Block* b)
{
...
bool
TR_RegisterCandidate::hasLoopExitBlock(TR::Block* b)
{
  return (_loopExitBlocks.find(b) != 0);
}

int32_t
TR_RegisterCandidate::countNumberOfLoadsAndStoresInBlocks(
...
TR_RegisterCandidate::printLoopInfo(TR::Compilation* comp)
{
  for (LoopInfo* li = _loops.getFirst(); li; li = li->getNext()) {
    traceMsg(comp,
             "Candidate #%d has %d loads and stores in loop %d\n",
             getSymbolReference()->getReferenceNumber(),
...
void
TR_RegisterCandidate::addLoop(TR_Structure* loop,
...
bool
isInnerLoop(TR::Compilation* comp, TR_Structure* loop)
{
...
void
TR_RegisterCandidate::removeAssignedCandidateFromLoop(
  TR::Compilation* comp,
  TR_Structure* loop,
  int32_t regNum,
  ...
  bool trace)
{
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    if (trace)
      traceMsg(comp,
               "Removed candidate #%d from block_%d entry and exit\n",
               getSymbolReference()->getReferenceNumber(),
               blockNumber);
    ...
    for (auto pred = nextBlock->getPredecessors().begin();
         pred != nextBlock->getPredecessors().end();
         ++pred) {
...
bool
findStoreNearEndOfBlock(TR::Block* block, TR::SymbolReference* ref)
{
...
bool
findLoad(TR::Node* node, TR::SymbolReference* ref)
{
...
bool
findLoadNearStartOfBlock(TR::Block* block, TR::SymbolReference* ref)
{
...
bool
TR_RegisterCandidates::aliasesPreventAllocation(TR::Compilation* comp,
...
void
TR_RegisterCandidate::setWeight(TR::Block** blocks,
                                ...
                                TR_BitVector& isExtensionOfPreviousBlock)
{
  ...
  _originalLiveOnEntry.init(
    numberOfBlocks, comp->trMemory(), stackAlloc, growable);
  ...
  processLiveOnEntryBlocks(blocks,
                           blockStructureWeight,
                           comp,
                           blockGPRCount,
                           blockFPRCount,
                           blockVRFCount,
...
void
TR_RegisterCandidate::recalculateWeight(
  TR::Block** blocks,
  int32_t* blockStructureWeight,
  TR::Compilation* comp,
  TR_Array<int32_t>& blockGPRCount,
...
// register pressue
bool
TR_RegisterCandidate::prevBlockTooRegisterConstrained(
  TR::Compilation* comp,
  TR::Block* block,
  TR_Array<int32_t>& blockGPRCount,
  TR_Array<int32_t>& blockFPRCount,
...
bool
TR_RegisterCandidate::rcNeeds2Regs(TR::Compilation* comp)
{
...
bool
TR_RegisterCandidate::hasSameGlobalRegisterNumberAs(TR::Node* node,
                                                    TR::Compilation* comp)
{
...
void
TR_RegisterCandidate::processLiveOnEntryBlocks(
  TR::Block** blocks,
  int32_t* blockStructureWeight,
  TR::Compilation* comp,
  TR_Array<int32_t>& blockGPRCount,
  ...
  bool callToRemoveUnusedLoops)
{
  ...
  while (lookForBlocksToRemove) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (!referencedInPred) {
        if (trace)
          dumpOptDetails(comp,
                         "Not referenced in pred, re-setting block_%d \n",
                         blockNumber);
      ...
      if (_blocks.getNumberOfLoadsAndStores(blockNumber) == 0) {
        ...
        if (!liveOnExit && block->isCold()) {
          for (auto pred = block->getPredecessors().begin();
               pred != block->getPredecessors().end();
               ++pred)
              ...
              if (!referencedInNearestLoop) {
                ...
                if (shouldResetThisLoop) {
                  if (trace)
                    dumpOptDetails(
                      comp,
                      "\n2Loop does not use candidate, removing block_%d from "
  ...
  while (trace && bvi.hasMoreElements()) {
    ...
    if (trace &&
        !performTransformation(comp,
                               "%s Candidate %d live on entry at block_%d\n",
                               OPT_DETAILS,
                               getSymbolReference()->getReferenceNumber(),
                               bnum)) {
  ...
  while (bvi.hasMoreElements()) {
    ...
    for (auto e = block->getPredecessors().begin();
      ...
      //
      if (
        !comp->cg()->allowGlobalRegisterAcrossBranch(this, node)
        //             ||
      ...
      if (((predWeight > blockWeight) && !liveOnExitFromPred &&
        ...
        if (reset) {
          if (trace)
            dumpOptDetails(comp,
                           " is NOT live on entry because of pred %d ",
                           pred->getNumber());
...
//
void
TR_RegisterCandidate::extendLiveRangesForLiveOnExit(
  TR::Compilation* comp,
  TR::Block** blocks,
  TR_Array<TR::Block*>& startOfExtendedBBForBB)
{
...
void
TR_RegisterCandidates::lookForCandidates(TR::Node* node,
                                         ...
                                         bool& foundExit)
{
  ...
  for (i = 0; i < node->getNumChildren(); ++i)
    lookForCandidates(
      node->getChild(i), entrySymbol, exitSymbol, foundEntry, foundExit);
...
bool
TR_RegisterCandidates::candidatesOverlap(TR::Block* block,
                                         ...
                                         bool trace)
{
  ...
  for (TR::TreeTop* tt = block->getFirstRealTreeTop(); tt;
    ...
    // exitSymbolInTree);
    lookForCandidates(
      node, entrySymbol, exitSymbol, entrySymbolInTree, seenExitSymbol);
...
bool
TR_RegisterCandidates::prioritizeCandidate(TR_RegisterCandidate* newRC,
...
TR_RegisterCandidate*
TR_RegisterCandidates::reprioritizeCandidates(TR_RegisterCandidate* first,
                                              TR::Block** blocks,
                                              int32_t* blockStructureWeight,
  ...
  TR::Block** all_blocks)
{
  ...
  if (trace) {
    traceMsg(comp,
             "\tscanPressureSimulatorCacheForConflits: Candidate #%d\n",
             rc->getSymbolReference()->getReferenceNumber());
  ...
  while (blockIterator.hasMoreElements()) {
    ...
    spilledRegs->empty();
    comp->cg()->computeSimulatedSpilledRegs(
      spilledRegs, blockNum, rc->getSymbolReference());
  ...
  if (trace)
    traceMsg(
      comp,
      "\t Update conflicts on entry due to conflicts on predecessor exits:\n");
...
void
TR_RegisterCandidates::collectCfgProperties(TR::Block** blocks,
                                            int32_t numberOfBlocks)
{
  TR::CFG* cfg = comp()->getFlowGraph();
  ...
  _firstBlock.init(
    cfg->getNextNodeNumber(), _trMemory, stackAlloc, notGrowable);
  _firstBlock.empty();

  TR::CFGNode* start = cfg->getStart();

  for (auto e = start->getSuccessors().begin();
       e != start->getSuccessors().end();
       ++e) {
  ...
  int i;
  _isExtensionOfPreviousBlock.init(
    cfg->getNextNodeNumber(), _trMemory, stackAlloc, notGrowable);
...
static void
assign_candidate_loop_trace_increment(TR::Compilation* comp,
                                      TR_RegisterCandidate* rc,
                                      unsigned count)
{
  bool trace = comp->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
...
bool
TR_RegisterCandidates::assign(TR::Block** cfgBlocks,
                              int32_t numberOfBlocks,
                              ...
                              int32_t& highestNumber)
{
  ...
  TR_BitVector switchBlocks(comp()->getFlowGraph()->getNextNodeNumber(),
                            ...
                            stackAlloc,
                            growable);
  TR_BitVector temp(comp()->getFlowGraph()->getNextNodeNumber(),
                    trMemory(),
                    stackAlloc,
                    growable);
  TR_BitVector highWordZeroLongs(
    comp()->getSymRefCount(), trMemory(), stackAlloc, growable);

  TR_BitVector catchBlockLiveLocals(
    comp()->getSymRefCount(), trMemory(), stackAlloc, growable);
  TR_BitVector nonOSRCatchBlockLiveLocals(
    comp()->getSymRefCount(), trMemory(), stackAlloc, growable);
  ...
  TR_Array<int32_t> numberOfGPRsLiveOnExit(
    trMemory(), numberOfBlocks, true, stackAlloc);
  TR_Array<int32_t> numberOfFPRsLiveOnExit(
    trMemory(), numberOfBlocks, true, stackAlloc);
  TR_Array<int32_t> numberOfVRFsLiveOnExit(
    trMemory(), numberOfBlocks, true, stackAlloc);
  TR_Array<int32_t> maxGPRsLiveOnExit(
    trMemory(), numberOfBlocks, true, stackAlloc);
  TR_Array<int32_t> maxFPRsLiveOnExit(
    trMemory(), numberOfBlocks, true, stackAlloc);
  TR_Array<int32_t> maxVRFsLiveOnExit(
    trMemory(), numberOfBlocks, true, stackAlloc);
  ...
  TR_Array<int32_t> totalGPRCountOnEntry(
    trMemory(), numberOfBlocks, true, stackAlloc);
  TR_Array<int32_t> totalFPRCountOnEntry(
    trMemory(), numberOfBlocks, true, stackAlloc);
  TR_Array<int32_t> totalVRFCountOnEntry(
    trMemory(), numberOfBlocks, true, stackAlloc);
  ...
  for (; rc; rc = next) {
    ...
    dumpOptDetails(comp(),
                   "Weight of candidate (symRef #%d %s) is %d\n",
                   rc->getSymbolReference()->getReferenceNumber(),
  ...
  _liveOnEntryConflicts.init(
    trMemory(), numberOfGlobalRegisters, true, stackAlloc);
  _liveOnExitConflicts.init(
    trMemory(), numberOfGlobalRegisters, true, stackAlloc);
  _entryExitConflicts.init(
    trMemory(), numberOfGlobalRegisters, true, stackAlloc);
  _exitEntryConflicts.init(
    trMemory(), numberOfGlobalRegisters, true, stackAlloc);
#ifdef TRIM_ASSIGNED_CANDIDATES
  _availableBlocks.init(trMemory(), numberOfGlobalRegisters, true, stackAlloc);
#endif
  for (i = 0; i < numberOfGlobalRegisters; i++) {
    _liveOnEntryConflicts[i].init(
      numberOfBlocks, trMemory(), stackAlloc, growable);
    _liveOnExitConflicts[i].init(
      numberOfBlocks, trMemory(), stackAlloc, growable);
    _entryExitConflicts[i].init(
      numberOfBlocks, trMemory(), stackAlloc, growable);
    _exitEntryConflicts[i].init(
      numberOfBlocks, trMemory(), stackAlloc, growable);
#ifdef TRIM_ASSIGNED_CANDIDATES
  ...
  for (rc = first; rc;
    ...
    computeAvailableRegisters(
      rc, firstRegister, lastRegister, blocks, &availableRegisters);
    if (trace) {
    ...
    TR_GlobalRegisterNumber registerNumber;
    {
      ...
      registerNumber = cg->pickRegister(
    ...
    if ((registerNumber == -1) || (needs2Regs && (highRegisterNumber == -1))) {
      ...
      //
        ...
        int32_t maxClearStructures =
          -1; // largest number of clear structures of the unavailable registers
        ...
        const bool useRegisterPressureInfo =
          ...
          (!trace || performTransformation(
                       comp(),
                       "%s Including register pressure info for trimming live "
                       "range of #%d\n",
                       OPT_DETAILS,
                       rc->getSymbolReference()->getReferenceNumber()));
        if (useRegisterPressureInfo) {
          ...
          //
          TR_BitVector spilledRegs(
            cg->getNumberOfGlobalRegisters(), trMemory(), stackAlloc);
          TR_BitVector liveOnEntryOrExit(
            numberOfBlocks, trMemory(), stackAlloc);
          liveOnEntryOrExit = rc->getBlocksLiveOnEntry();
          liveOnEntryOrExit |= rc->getBlocksLiveOnExit();
          scanPressureSimulatorCacheForConflicts(rc,
                                                 liveOnEntryOrExit,
                                                 _liveOnEntryConflicts,
                                                 _liveOnExitConflicts,
                                                 _entryExitConflicts,
                                                 firstRegister,
                                                 lastRegister,
                                                 &spilledRegs,
        ...
        for (i = firstRegister; i <= lastRegister; ++i) {
          ...
          if (trace) {
            traceMsg(
              comp(), "Final live on entry conflicts with register : %d\n", i);
          ...
          if (needs2Regs) {
            ...
            if (trace) {
              ...
              traceMsg(comp(),
                       "max freq %d; smallest max freq %d\n",
                       maxFrequency,
                       smallestMaxFrequency);
              traceMsg(comp(),
        ...
        if (conflictingRegister != -1) {
          ...
          if (trace) {
            dumpOptDetails(
              comp(),
              "\nDue to conflicts, removing the following blocks from live on "
              "entry ranges of candidate #%d conflicting register %d\n",
              rc->getSymbolReference()->getReferenceNumber(),
              conflictingRegister);
            ...
            while (bvi.hasMoreElements()) {
              ...
              if (cand) {
                cand->removeAssignedCandidateFromLoop(
                  ...
                  &_liveOnExitUsage[conflictingRegister],
                  trace);
              }
            }
          }
    ...
    if (needs2Regs) {
      if (!performTransformation(
            comp(),
            "%s assign auto #%d to low reg %d and high reg %d (%s)\n",
            OPT_DETAILS,
            rc->getSymbolReference()->getReferenceNumber(),
            registerNumber,
            highRegisterNumber,
            comp()->getDebug()
              ? comp()->getDebug()->getGlobalRegisterName(highRegisterNumber)
              : "?"))
        continue;
    } else {
      if (!performTransformation(
            comp(),
            "%s assign auto #%d %s to reg %d (%s)\n",
            OPT_DETAILS,
            rc->getSymbolReference()->getReferenceNumber(),
            rc->getSymbolReference()->getSymbol()->isMethodMetaData()
              ? rc->getSymbolReference()
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace)
        traceMsg(comp(),
                 "Register %d in block_%d is candidate %d on entry\n",
                 registerNumber,
                 nextElement,
                 rc->getSymbolReference()->getReferenceNumber());
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace)
        traceMsg(comp(),
                 "Register %d in block_%d is candidate %d on exit\n",
                 registerNumber,
                 blockNumber,
                 rc->getSymbolReference()->getReferenceNumber());
        ...
        if (++numberOfFPRsLiveOnExit[blockNumber] ==
          ...
          if (!comp()->getOptimizer()->getSuccessorBitsGRA())
            comp()->getOptimizer()->setSuccessorBitsGRA(
              new (trHeapMemory())
                TR_BitVector(numberOfBlocks, trMemory(), heapAlloc, growable));
          else
          ...
          next = reprioritizeCandidates(
            ...
            totalFPRCount,
            totalVRFCount,
            comp()->getOptimizer()->getSuccessorBitsGRA(),
            trace);
        }
      }
      if (isVector) {
                ...
                uint32_t& seqno)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (itr != overlaps.end())
      ...
      overlaps.insert(
        std::make_pair(ref, TR_RegisterCandidates::coordinates(seqno, seqno)));
    }
  }
}

void
ComputeOverlaps(TR::Block* block,
                TR::Compilation* comp,
                TR_RegisterCandidates::Coordinates& overlaps)
{
  uint32_t seqno = 0;
...
void
TR_RegisterCandidates::computeAvailableRegisters(
  TR_RegisterCandidate* rc,
  int32_t firstRegister,
  int32_t lastRegister,
  TR::Block** blocks,
  TR_BitVector* availableRegisters)
{
  ...
  for (i = firstRegister; i <= lastRegister; ++i) {
    ...
    if (rcSymbol->isParm() &&
      ...
      if (lri >= 0) {
        TR_GlobalRegisterNumber parmReg =
          comp()->cg()->getLinkageGlobalRegisterNumber(lri,
                                                       rcSymbol->getDataType());
    ...
    if (trace) {
      traceMsg(comp(),
               "For candidate %d real register %d : \n",
               rc->getSymbolReference()->getReferenceNumber(),
               i);
