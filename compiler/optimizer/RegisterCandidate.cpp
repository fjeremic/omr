...
void TR::GlobalSet::collectBlocks() {
  ...
  for (TR::CFGNode *node = _comp->getFlowGraph()->getFirstNode(); node;
       node = node->getNext()) {
    ...
    for (TR::TreeTop *tt = block->getEntry(); tt && tt != block->getExit();
...
void TR::GlobalSet::collectReferencedAutoSymRefs(
    TR::Node *node, TR_BitVector &referencedAutoSymRefs,
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i)
    collectReferencedAutoSymRefs(node->getChild(i), referencedAutoSymRefs,
                                 visited);
}
...
int32_t
    TR_RegisterCandidates::_candidateTypeWeights[TR_NumRegisterCandidateTypes] =
        {
            1,  // TR_InductionVariableCandidate
            1   // 10        // TR_PRECandidate
};

TR_RegisterCandidate::TR_RegisterCandidate(TR::SymbolReference *sr,
                                           TR::Region &r)
    : _symRef(sr),
      _splitSymRef(NULL),
      _restoreSymRef(NULL),
...
TR::DataType TR_RegisterCandidate::getType() { return getSymbol()->getType(); }
TR::DataType TR_RegisterCandidate::getDataType() {
...
TR_RegisterKinds TR_RegisterCandidate::getRegisterKinds() {
...
void TR_RegisterCandidate::addAllBlocksInStructure(TR_Structure *structure,
                                                   TR::Compilation *comp,
                                                   const char *description,
                                                   vcount_t count,
                                                   bool recursiveCall) {
  if (!recursiveCall) count = comp->incVisitCount();
  ...
  if (structure->asBlock() != NULL) {
    ...
    if (description)
      traceMsg(
          ...
          description, getSymbolReference()->getReferenceNumber(),
          getSymbolReference(), block->getNumber());
  } else {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext())
      addAllBlocksInStructure(node->getStructure(), comp, description, count,
                              true);
  }
}
...
TR_RegisterCandidate *TR_RegisterCandidates::newCandidate(
    TR::SymbolReference *ref) {
  return new (_candidateRegion) TR_RegisterCandidate(ref, _candidateRegion);
}

TR_RegisterCandidate *TR_RegisterCandidates::findOrCreate(
...
TR_RegisterCandidate *TR_RegisterCandidates::find(TR::SymbolReference *symRef) {
  if (symRef->getSymbol()->isAutoOrParm()) {
    TR_RegisterCandidate *rc =
        (_candidateForSymRefs)
            ? (*_candidateForSymRefs)[GET_INDEX_FOR_CANDIDATE_FOR_SYMREF(
                  symRef)]
            : NULL;
    // two symrefs can point to same symbols so it is possible that we have an
    // rc for symref->getSymbol but not have it added to the cache
    if (rc == NULL) {
      rc = find(symRef->getSymbol());
      if (_candidateForSymRefs)
        (*_candidateForSymRefs)[GET_INDEX_FOR_CANDIDATE_FOR_SYMREF(symRef)] =
            rc;
    }
    return rc;
  } else
...
TR_RegisterCandidate *TR_RegisterCandidates::find(TR::Symbol *sym) {
  TR_RegisterCandidate *rc;
  for (rc = _candidates.getFirst(); rc; rc = rc->getNext())
    if (rc->getSymbolReference()->getSymbol() == sym) return rc;

  return NULL;
}

bool TR_RegisterCandidate::find(TR::Block *b) {
  return _blocks.find(b->getNumber());
}

void TR_RegisterCandidate::addBlock(TR::Block *block,
                                    int32_t numberOfLoadsAndStores) {
  _blocks.incNumberOfLoadsAndStores(block->getNumber(), numberOfLoadsAndStores);
}

int32_t TR_RegisterCandidate::removeBlock(TR::Block *block) {
  if (find(block)) {
    int32_t numLoadsAndStores =
        _blocks.getNumberOfLoadsAndStores(block->getNumber());
...
void TR_RegisterCandidate::addLoopExitBlock(TR::Block *b) {
  if (!_loopExitBlocks.find(b)) _loopExitBlocks.add(b);
}
...
int32_t TR_RegisterCandidate::countNumberOfLoadsAndStoresInBlocks(
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    numLoadsAndStores +=
        _blocks.getNumberOfLoadsAndStores(nextBlock->getNumber());
  }
  return numLoadsAndStores;
}

#ifdef TRIM_ASSIGNED_CANDIDATES
// The following are methods for maintaining info about number of candidate's
// loads and stores per loop (vs. per block)
TR_RegisterCandidate::LoopInfo *TR_RegisterCandidate::find(TR_Structure *l) {
  for (LoopInfo *li = _loops.getFirst(); li; li = li->getNext())
    if (li->_loop == l) return li;
  return 0;
}

void TR_RegisterCandidate::printLoopInfo(TR::Compilation *comp) {
  for (LoopInfo *li = _loops.getFirst(); li; li = li->getNext()) {
...
void TR_RegisterCandidate::addLoop(TR_Structure *loop,
                                   int32_t numberOfLoadsAndStores,
                                   TR_Memory *m) {
...
void TR_RegisterCandidate::countNumberOfLoadsAndStoresInLoops(TR_Memory *m) {
...
int32_t TR_RegisterCandidate::getNumberOfLoadsAndStoresInLoop(
    TR_Structure *loop) {
  LoopInfo *li = find(loop);
  if (li)
...
bool isInnerLoop(TR::Compilation *comp, TR_Structure *loop) {
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
       nextBlock = blocksIt.getNext()) {
    if (nextBlock->getStructureOf()->getContainingLoop() != loop) return false;
  }
  return true;
}

void TR_RegisterCandidate::removeAssignedCandidateFromLoop(
    TR::Compilation *comp, TR_Structure *loop, int32_t regNum,
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    nextBlock->getGlobalRegisters(comp)[regNum].setRegisterCandidateOnEntry(
        NULL);
    nextBlock->getGlobalRegisters(comp)[regNum].setRegisterCandidateOnExit(
        NULL);

    // handle predecessors
    for (auto pred = nextBlock->getPredecessors().begin();
         pred != nextBlock->getPredecessors().end(); ++pred) {
...
bool TR_RegisterCandidate::symbolIsLive(TR::Block *block) {
  TR_BitVector *liveLocals = block->getLiveLocals();
  if (!liveLocals) return true;  // can be a new block
...
bool findLoad(TR::Node *node, TR::SymbolReference *ref) {
  ...
  for (int i = 0; i < node->getNumChildren(); i++) {
    if (findLoad(node->getChild(i), ref)) return true;
  }
  return false;
}
...
bool TR_RegisterCandidates::aliasesPreventAllocation(
    TR::Compilation *comp, TR::SymbolReference *symRef) {
  if (!symRef->getSymbol()->isAutoOrParm() &&
      !(TR::Compiler->target.cpu.isZ() && TR::Compiler->target.isLinux()))
    return true;
  ...
  if (!use_def_aliases.IsZero()) {
    ...
    if (!use_def_aliases.IsZero()) return true;
  }

  return false;
}

void TR_RegisterCandidate::setWeight(
    TR::Block **blocks, int32_t *blockStructureWeight, TR::Compilation *comp,
    TR_Array<int32_t> &blockGPRCount, TR_Array<int32_t> &blockFPRCount,
    TR_Array<int32_t> &blockVRFCount, TR_BitVector *referencedBlocks,
    TR_Array<TR::Block *> &startOfExtendedBBForBB, TR_BitVector &firstBlocks,
  ...
  _originalLiveOnEntry.init(numberOfBlocks, comp->trMemory(), stackAlloc,
                            growable);
  ...
  while (itr.hasMoreElements()) {
    ...
    TR_ASSERT(blockNumber < cfg->getNextNodeNumber(),
              "Overflow on candidate BB numbers");
    TR::Block *b = blocks[blockNumber];
    if (!b) continue;

    TR_ASSERT(blockNumber == b->getNumber(), "blocks[x]->getNumber() != x");
    TR_ASSERT(
        (blockNumber < cfg->getNextNodeNumber()) && (blocks[blockNumber] == b),
        "blockNumber is wrong");

    int32_t blockWeight = _blocks.getNumberOfLoadsAndStores(blockNumber);
    bool firstBlock = firstBlocks.isSet(blockNumber);
    if ((firstBlock && isAllBlocks() &&
         cg->getSupportsGlRegDepOnFirstBlock()) ||
        (!firstBlock &&
         (((blockWeight == 0) && hasLoopExitBlock(b)) || symbolIsLive(b))))
    ...
    // We should not mark a candidate as being referenced in the extended block
    // if the only references to it are in the portion of the extended block
    // that is outside a loop This is important as we cannot recognize that a
    // candidate is not referenced at all in a loop otherwise. We see a 'false'
    // use in the extended basic block that started inside the loop because of
    // the portion of the extended basic block outside the loop.
    //
    if (extendedBlockFreq <= blockFreq) {
      if ((uint32_t)blockWeight >
          _blocks.getNumberOfLoadsAndStores(extendedBlock->getNumber()))
        _blocks.setNumberOfLoadsAndStores(extendedBlock->getNumber(),
                                          blockWeight);
    }
  }
  ...
  processLiveOnEntryBlocks(blocks, blockStructureWeight, comp, blockGPRCount,
                           blockFPRCount, blockVRFCount, referencedBlocks,
                           startOfExtendedBBForBB);
}

void TR_RegisterCandidate::recalculateWeight(
    TR::Block **blocks, int32_t *blockStructureWeight, TR::Compilation *comp,
    TR_Array<int32_t> &blockGPRCount, TR_Array<int32_t> &blockFPRCount,
    TR_Array<int32_t> &blockVRFCount, TR_BitVector *referencedBlocks,
...
// register pressue
bool TR_RegisterCandidate::prevBlockTooRegisterConstrained(
    TR::Compilation *comp, TR::Block *block, TR_Array<int32_t> &blockGPRCount,
    TR_Array<int32_t> &blockFPRCount, TR_Array<int32_t> &blockVRFCount) {
  ...
  if (_liveOnEntry.get(blockNum)) {
    ...
    for (auto e = block->getPredecessors().begin();
      ...
      if (pred == comp->getFlowGraph()->getStart()) continue;
      TR::Node *node = pred->getLastRealTreeTop()->getNode();
      // printf("looking at block_%d's  previous node %p\n",blockNum,node);
      int32_t maxGPRs =
          comp->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(pred);
      int32_t maxFPRs =
          comp->cg()->getMaximumNumberOfFPRsAllowedAcrossEdge(node);
      int32_t maxVRFs =
          comp->cg()->getMaximumNumberOfVRFsAllowedAcrossEdge(node);

      // dumpOptDetails(comp, " pred %d actual gprs %d max gprs %d ",
      // pred->getNumber(), gprCount, maxGPRs);
...
bool TR_RegisterCandidate::rcNeeds2Regs(TR::Compilation *comp) {
  if (getType().isAggregate()) {
    if ((TR::Compiler->target.is32Bit() && !comp->cg()->use64BitRegsOn32Bit() &&
         getSymbol()->getSize() > 4) ||
        (getSymbol()->getSize() > 8))
      return true;
    else
    ...
    return ((getType().isInt64() && TR::Compiler->target.is32Bit() &&
...
bool TR_RegisterCandidate::hasSameGlobalRegisterNumberAs(
    TR::Node *node, TR::Compilation *comp) {
  if (node->requiresRegisterPair(comp))
    return (getLowGlobalRegisterNumber() ==
            node->getLowGlobalRegisterNumber()) &&
...
void TR_RegisterCandidate::processLiveOnEntryBlocks(
    TR::Block **blocks, int32_t *blockStructureWeight, TR::Compilation *comp,
    TR_Array<int32_t> &blockGPRCount, TR_Array<int32_t> &blockFPRCount,
    TR_Array<int32_t> &blockVRFCount, TR_BitVector *referencedBlocks,
    TR_Array<TR::Block *> &startOfExtendedBBForBB,
  ...
  if (comp->cg()->areAssignableGPRsScarce()) removeUnusedLoops = true;
  ...
  // used for MetaData that are live on entry in blocks, just to mark that they
  // can escape via return but there is no real loads so we do not want such
  // live on entry blocks to contribute to weigh of the candidate TR_BitVector
  // *ignoreLiveOnEntryBlocks = new (comp->trHeapMemory())
  // TR_BitVector(comp->getFlowGraph()->getNextNodeNumber(), comp->trMemory());
  while (lookForBlocksToRemove) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace)
        dumpOptDetails(comp,
                       "Examining original live-on-entry block_%d with loads "
                       "and stores = %d\n",
                       blockNumber,
      ...
      for (auto e = block->getPredecessors().begin();
        ...
        TR_ASSERT(comp->getOptimizer()->cachedExtendedBBInfoValid(),
                  "Incorrect value in _startOfExtendedBBForBB");
        if (_liveOnEntry.get(
                startOfExtendedBBForBB[toBlock((*e)->getFrom())->getNumber()]
                    ->getNumber()))
      ...
      if (!referencedInPred)
        for (auto e = block->getPredecessors().begin();
             e != block->getPredecessors().end(); ++e)
        // if
        // (toBlock(e->getFrom())->startOfExtendedBlock()->findFirstReference(sym,
        // true))
        {
          TR::Block *predBlock = toBlock((*e)->getFrom());
          TR_ASSERT(comp->getOptimizer()->cachedExtendedBBInfoValid(),
                    "Incorrect value in _startOfExtendedBBForBB");
          TR::Block *parentPredBlock =
              startOfExtendedBBForBB[predBlock->getNumber()];
  ...
  while (lookForBlocksToRemove) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (trace)
        traceMsg(
            comp,
            "\tExamining live-on-entry block_%d with loads and stores = %d\n",
      ...
      if (_blocks.getNumberOfLoadsAndStores(blockNumber) == 0) {
        ...
        if (!liveOnExit && block->isCold()) {
          for (auto pred = block->getPredecessors().begin();
               pred != block->getPredecessors().end(); ++pred)
        ...
        if (!liveOnExit) {
          if (trace)
            traceMsg(
                comp,
                "\tNot referenced and not live on exit, re-setting block %d \n",
                blockNumber);
          _liveOnEntry.reset(blockNumber);
          lookForBlocksToRemove = true;
        }
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (trace)
      dumpOptDetails(comp,
                     "Examining live-on-entry block_%d with loads and stores = "
                     "%d to find maxFrequency\n",
                     blockNumber,
                     _blocks.getNumberOfLoadsAndStores(blockNumber));
    TR::Block *block = blocks[blockNumber];
    ...
    if (blockStructure && (!dontAssignInColdBlocks(comp) || !block->isCold())) {
      ...
      if (blockWeight > maxFrequency) maxFrequency = blockWeight;
    }
  }
  ...
  if (!getLoopsWithHoles().isEmpty())
    removeUnusedLoops =
        true;  // try to remove even if weight is not negative yet
  ...
  TR_BitVector *resetLiveOnEntry = new (comp->trHeapMemory())
      TR_BitVector(comp->getFlowGraph()->getNextNodeNumber(),
                   comp->trMemory());  // Added by Nakaike
  ...
  if ((maxFrequency > 1) && removeUnusedLoops) {
    ...
    while (lookForBlocksToRemove) {
      ...
      if (1)  // to calculate numUnreferencedBlocksAtThisFrequency for setting
              // maxFrequency later
      {
        // Maybe there is some value in figuring out the ratio of unutilized
        // blocks before we decide to discard any; experiment more with that; in
        // that case use the commented out portion below
        //
        bvi.setBitVector(_liveOnEntry);
        while (bvi.hasMoreElements()) {
          int32_t blockNumber = bvi.getNextElement();
          ...
          if (blockStructure) {
            // blockStructure->calculateFrequencyOfExecution(&blockWeight);
            TR_ASSERT(blockNumber == block->getNumber(),
                      "blocks[x]->getNumber() != x");
      ...
      if (trace)
        traceMsg(
            comp,
            "At frequency %d for %d numBlocks %d numUnreferencedBlocks %d\n",
            frequency, getSymbolReference()->getReferenceNumber(),
            numBlocksAtThisFrequency, numUnreferencedBlocksAtThisFrequency);
      ...
      if ((numUnreferencedBlocksAtThisFrequency > 0) &&
          (comp->getOption(
               TR_EnableRangeSplittingGRA) ||  // move this check above ?
          ...
          if (blockWeight == frequency) {
            ...
            if (!getLoopsWithHoles().isEmpty()) {
              TR_RegionStructure *parent =
                  blockStructure->getParent()->asRegion();
                  ...
                  if (!referencedLoops.find(parent)) {
                    ...
                    for (nextBlock = blocksIt.getCurrent(); nextBlock;
                      ...
                      if (nextBlock->getEntry()) {
                        TR_ASSERT(
                            comp->getOptimizer()->cachedExtendedBBInfoValid(),
                            "Incorrect value in _startOfExtendedBBForBB");
                        TR::Block *liveBlock =
                            startOfExtendedBBForBB[nextBlock->getNumber()];
                        if (_liveOnEntry.get(liveBlock->getNumber())) {
                          if ((_blocks.getNumberOfLoadsAndStores(blockNumber) ==
                               0) &&
              ...
              if (!referencedInNearestLoop) {
                ...
                if (parent && !unreferencedLoops.find(parent)) {
                  ...
                  if (!comp->getOptimizer()->getResetExitsGRA())
                    comp->getOptimizer()->setResetExitsGRA(
                  ...
                  int32_t loopFrequency =
                      parent->getEntryBlock()->getFrequency();
                  ...
                  for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL;
                    ...
                    if (toStructureNumber ==
                        comp->getFlowGraph()->getEnd()->getNumber()) {
                      // The exit block shouldn't be in any live ranges anyway,
                      // so putting it in the resetExits set is unnecessary at
                      // best.
                      //
                    ...
                    //   for (succ = succIt.getFirst(); succ != NULL; succ =
                    //   succIt.getNext())
                    //       {
                    //       TR_StructureSubGraphNode *succNode =
                    //       (TR_StructureSubGraphNode *) succ->getTo(); if
                    //       (succNode->getNumber() == toStructureNumber)
                    //          {
                    ...
                    //      {
                    //      toFrequency =
                    //      succStructure->getEntryBlock()->getFrequency(); if
                    //      (4*toFrequency > fromFrequency)
                    //         shouldResetExit = false;
                    ...
                    if (shouldResetExit) {
                      comp->getOptimizer()->getResetExitsGRA()->set(
                          toStructureNumber);
                  ...
                  if (shouldResetThisLoop) {
                    if (trace) {
                      dumpOptDetails(
                          comp,
                          "\n1Loop %d does not use candidate, removing the "
                          "following blocks from live ranges of candidate #%d "
                          "\n",
                          parent->getNumber(),
                          getSymbolReference()->getReferenceNumber());
                    ...
                    *resetLiveOnEntry |=
                        *comp->getOptimizer()
                             ->getResetExitsGRA();  // Added by Nakaike
                  }
                }
                ...
                if (shouldResetThisLoop) {
                  if (trace)
                    dumpOptDetails(
                        comp,
                        "\n2Loop does not use candidate, removing block_%d "
                        "from live ranges of candidate #%d \n",
                        blockNumber,
                        getSymbolReference()->getReferenceNumber());

                  unreferencedLoops.add(
                      parent);  // should not be moved where exits are removed ?
                  _liveOnEntry.reset(blockNumber);
                  _liveOnExit.reset(blockNumber);
                  ...
                  referencedLoops.add(parent);
                // dumpOptDetails(comp, "Resetting liveOnEntry at block_%d
                // frequency %d for %d\n", blockNumber, frequency,
                // getSymbolReference()->getReferenceNumber());
            ...
            if (_liveOnEntry.get(blockNumber)) currMaxFrequency = frequency;
          }
        }
      }
      ...
      if (frequency <= maxFrequency) lookForBlocksToRemove = true;
  ...
  if (callToRemoveUnusedLoops) getLoopsWithHoles().deleteAll();
  ...
  while (trace && bvi.hasMoreElements()) {
    ...
    bool ignoreBlock = (dontAssignInColdBlocks(comp) && block->isCold()) ||
                       block->isOSRInduceBlock();
    ...
    if (trace &&
        !performTransformation(
            comp, "%s Candidate %d live on entry at block_%d\n", OPT_DETAILS,
  ...
  while (bvi.hasMoreElements()) {
    ...
    bool ignoreBlock = (dontAssignInColdBlocks(comp) && block->isCold()) ||
                       block->isOSRInduceBlock();
    if (!ignoreBlock &&
        (blockWeight >= maxFrequency * freqRatio || useProfilingFrequencies)) {
      ++numberOfBlocks;
    }
    ...
    if (trace)
      dumpOptDetails(comp,
                     "Examining live-on-entry block_%d (blockWeight %d "
                     "maxFrequency %d) numberOfBlocks %d loadsAndStores %d",
                     block->getNumber(), blockWeight, maxFrequency,
                     numberOfBlocks, loadsAndStores);
    ...
    for (auto e = block->getPredecessors().begin();
      ...
      if (!comp->cg()->allowGlobalRegisterAcrossBranch(this, node)
        ...
        if (trace)
          dumpOptDetails(
              comp,
              " (too constrained), is NOT live on entry because of pred %d ",
              pred->getNumber());
        _liveOnEntry.reset(blockNumber);
        break;
      }
      ...
      for (TR::Block *succBlock = ebsi.getFirst(); succBlock;
           succBlock = ebsi.getNext()) {
        if (succBlock != block) {
          // if (trace)
          //   dumpOptDetails(comp, " (some other succ %d of pred %d) ",
          //   succBlock->getNumber(), pred->getNumber());
      ...
      if (!someOtherSuccSeen) liveOnExitFromPred = true;

      if (((predWeight > blockWeight) && !liveOnExitFromPred &&
           !_liveOnEntry.get(pred->getNumber())) &&
          _blocks.getNumberOfLoadsAndStores(
              startOfExtendedBBForBB[pred->getNumber()]->getNumber()) == 0 &&
        ...
        if (reset) {
          if (trace)
            dumpOptDetails(comp, " is NOT live on entry because of pred %d ",
                           pred->getNumber());
    ...
    if (_liveOnEntry.get(blockNumber)) {
      if (trace) dumpOptDetails(comp, "is live on entry");

      TR_ASSERT(comp->getOptimizer()->cachedExtendedBBInfoValid(),
                "Incorrect value in _startOfExtendedBBForBB");
      TR::Block *startOfExtendedBlock =
          startOfExtendedBBForBB[block->getNumber()];
      for (auto e = block->getPredecessors().begin();
           e != block->getPredecessors().end(); ++e) {
        TR::Block *pred = toBlock((*e)->getFrom());
        if (pred == comp->getFlowGraph()->getStart()) continue;
        ...
        if ((pred->getNextBlock() == block) &&
          ...
          if (lastNode->getOpCode().isBranch()) {
            if (lastNode->getBranchDestination() !=
                startOfExtendedBlock->getEntry())
              continue;
        ...
        if (!_liveOnEntry.get(pred->getNumber()) &&
            (!dontAssignInColdBlocks(comp) || !pred->isCold())) {
          //++numberOfBlocks;  // TODO: try uncomment this
          loadsAndStores +=
              _blocks.getNumberOfLoadsAndStores(pred->getNumber());
          if (trace)
            dumpOptDetails(
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (!_liveOnEntry.get(blockNumber) &&
      ...
      if (trace)
        traceMsg(comp,
                 "Increased weight of candidate %d by %d (new weight %d) "
                 "because of live on exit from block_%d\n",
                 getSymbolReference()->getReferenceNumber(), blockWeight,
                 loadsAndStores, blockNumber);
    }
  }
  ...
  while (bvi.hasMoreElements()) {
    ...
    do {
      ...
      if (_blocks.getNumberOfLoadsAndStores(blockNumber) > 0) {
          ...
          if (((*succ)->getTo() != block->getNextBlock()) &&
            ...
            if (!_liveOnEntry.get((*succ)->getTo()->getNumber())) {
              ...
              if (!exclude && blockStructure &&
                  (/* !dontAssignInColdBlocks(comp) || */ (
                      !block->isCold() &&
                      !(*succ)->getTo()->asBlock()->isCold())) &&
                  !comp->getOptimizer()->getSeenBlocksGRA()->get(
                      block->getNumber()) &&
                  (  // symbolIsLive(block) ||
                ...
                blockWeight = blockStructureWeight[blockNumber];
                int32_t succWeight =
                    blockStructureWeight[(*succ)->getTo()->getNumber()];
                if (succWeight < blockWeight) blockWeight = succWeight;

                TR_Structure *containingLoop =
                    blockStructure->getContainingLoop();
                if (containingLoop) addLoopWithHole(containingLoop);

                // blockWeight = (blockWeight + succWeight)/2; // maybe the
                // store will be inserted in a split block in between the 2
                // blocks
                ...
                if (trace)
                  traceMsg(
                      comp,
                      "Reduced weight of candidate %d by %d (new weight %d) "
                      "because of end in live range at block_%d succ %d\n",
                      getSymbolReference()->getReferenceNumber(), blockWeight,
                      loadsAndStores, blockNumber,
        ...
        if (foundEndOfRange || block == comp->getFlowGraph()->getEnd()) break;
      }
              ...
              if (!exclude && blockStructure &&
                ...
                if (trace)
                  traceMsg(
                      comp,
                      "Reduced weight of candidate %d by %d (new weight %d) "
                      "because of start of live range at block_%d succ %d\n",
                      getSymbolReference()->getReferenceNumber(), blockWeight,
                      loadsAndStores, blockNumber,
                      (*succ)->getTo()->getNumber());
                foundEndOfRange = true;
  ...
  if ((numberOfBlocks == 0) && (loadsAndStores > 0)) numberOfBlocks = 1;
  ...
  bool isPPS =
      false;  // (comp->getOption(TR_EnableOSR) && getSymbol()->isAuto() &&
              // (getSymbolReference()->getCPIndex() < 0)); // This hurts in WAS
              // performance; so it is disable until I can figure out the issue
  ...
  if (numberOfBlocks && isPPS) {
    ...
    if (trace)
      dumpOptDetails(comp,
                     "For candidate #%d weight = %d loadsAndStores = %d "
                     "numberOfBlocks = %d\n",
                     getSymbolReference()->getReferenceNumber(), _weight,
                     loadsAndStores, numberOfBlocks);
    ...
    if (!comp->allocateAtThisOptLevel() &&
      ...
      if (!useProfilingFrequencies)
        _weight = std::max(
            1, (loadsAndStores * 10000) / (numberOfBlocks * numberOfBlocks));
      else
        _weight =
            std::max(1, (loadsAndStores) / (numberOfBlocks * numberOfBlocks));
    } else {
      if (trace) traceMsg(comp, "handle negative weight\n");

      if (!callToRemoveUnusedLoops) {
        getBlocksLiveOnExit().empty();
        _liveOnEntry |= _originalLiveOnEntry;
        processLiveOnEntryBlocks(
            blocks, blockStructureWeight, comp, blockGPRCount, blockFPRCount,
...
//
// If a candidate is live on entry to an extended block, it has to be live on
// exit from all its successors. Moreover, it will be live from the time it is
// first referenced in the successor till the corressponding exit
//
void TR_RegisterCandidate::extendLiveRangesForLiveOnExit(
    TR::Compilation *comp, TR::Block **blocks,
    TR_Array<TR::Block *> &startOfExtendedBBForBB) {
  LexicalTimer t("extendLiveRangesForLiveOnExit", comp->phaseTimer());

  bool trace = comp->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
  if (trace) traceMsg(comp, "Extending live ranges due to live on exits\n");

  TR_BitVector blocksVisited(comp->trMemory()->currentStackRegion());
  TR_BitVector *blocksReferencing =
      comp->getGlobalRegisterCandidates()->getBlocksReferencingSymRef(
          getSymbolReference()->getReferenceNumber());
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (!block->isExtensionOfPreviousBlock()) {
      for (auto e = block->getPredecessors().begin();
        ...
        if (blocksVisited.get(pred->getNumber())) continue;
        ...
        //    liveOnEntry |= liveOnExit & !<available in this block> &
        //    <available in prev block>
        currBlock = lastBlock;
        do {
          for (auto e = currBlock->getSuccessors().begin();
               e != currBlock->getSuccessors().end(); ++e) {
          ...
          if (!_liveOnEntry.get(currBlock->getNumber()) &&  // not set already
              ...
              currBlock->isExtensionOfPreviousBlock() && blocksReferencing &&
              blocksReferencing->get(currBlock->getPrevBlock()->getNumber()) &&
              !blocksReferencing->get(currBlock->getNumber())) {
            if (trace)
              traceMsg(comp,
                       "\tFor candidate #%d, set live on block_%d entry due to "
                       "live on exit\n",
                       getSymbolReference()->getReferenceNumber(),
                       currBlock->getNumber());
...
void TR_RegisterCandidates::lookForCandidates(TR::Node *node,
                                              TR::Symbol *entrySymbol,
                                              TR::Symbol *exitSymbol,
                                              bool &foundEntry,
                                              bool &foundExit) {
  if (node->getVisitCount() == _compilation->getVisitCount()) return;
  ...
  for (i = 0; i < node->getNumChildren(); ++i)
    lookForCandidates(node->getChild(i), entrySymbol, exitSymbol, foundEntry,
                      foundExit);

  if (node->getOpCode().hasSymbolReference()) {
    TR::Symbol *s = node->getSymbol();
    if (s == exitSymbol) {
      if (!foundExit && !foundEntryBeforeLookingAtChildren) foundEntry = false;
...
bool TR_RegisterCandidates::candidatesOverlap(
    TR::Block *block, TR_RegisterCandidate *candidateOnEntry,
    TR_RegisterCandidate *candidateOnExit, bool trace) {
  LexicalTimer t("candidatesOverlap", comp()->phaseTimer());

  if (candidateOnEntry == NULL || candidateOnExit == NULL) return false;

  // The candidates overlap if we see a reference to the entry symbol after
  // seeing a reference to the exit symbol. Note, since a RegStore will have to
  // be inserted to switch from one register to another, if both symbol are used
  // in the same tree, they are overlapping regardless of which order they are
  // referenced in.
  //
  ...
  for (TR::TreeTop *tt = block->getFirstRealTreeTop(); tt;
    ...
    // lookForCandidates(node, entrySymbol, exitSymbol, entrySymbolInTree,
    // exitSymbolInTree);
    lookForCandidates(node, entrySymbol, exitSymbol, entrySymbolInTree,
                      seenExitSymbol);
    ...
    if (entrySymbolInTree && seenExitSymbol) {
      if (trace)
        traceMsg(
            comp(),
            "Returning true in block_%d node %p entry cand %d exit cand %d\n",
            block->getNumber(), node,
            candidateOnEntry->getSymbolReference()->getReferenceNumber(),
...
bool TR_RegisterCandidates::prioritizeCandidate(TR_RegisterCandidate *newRC,
                                                TR_RegisterCandidate *&first) {
  ...
  if (newRC->getWeight() != 0) {
    ...
    for (; rc; prev = rc, rc = rc->getNext()) {
      if (newRC->getWeight() > rc->getWeight()) break;
    }
    if (prev)
      prev->setNext(newRC);
    else
...
TR_RegisterCandidate *TR_RegisterCandidates::reprioritizeCandidates(
    TR_RegisterCandidate *first, TR::Block **blocks,
    int32_t *blockStructureWeight, int32_t numberOfBlocks, TR::Block *fullBlock,
    TR::Compilation *comp, bool reprioritizeFP, bool onlyReprioritizeLongs,
    TR_BitVector *referencedBlocks, TR_Array<int32_t> &blockGPRCount,
    TR_Array<int32_t> &blockFPRCount, TR_Array<int32_t> &blockVRFCount,
    TR_BitVector *successorBits, bool trace) {
  LexicalTimer t("reprioritizeCandidates", comp->phaseTimer());
  if (!successorBits) {
    successorBits = new (trStackMemory())
        TR_BitVector(numberOfBlocks, trMemory(), stackAlloc, growable);
    // TR_BitVector successorBits(numberOfBlocks, trMemory(), stackAlloc,
    // growable);

    TR_ExtendedBlockSuccessorIterator ebsi(fullBlock, comp->getFlowGraph());
    // TR_Array<int32_t> blockGPRCount(trMemory(),
    // numberOfBlocks,true,stackAlloc); TR_Array<int32_t>
    // blockFPRCount(trMemory(), numberOfBlocks,true,stackAlloc);
    // TR_Array<int32_t> blockVRFCount(trMemory(),
    // numberOfBlocks,true,stackAlloc);
  ...
  for (TR_RegisterCandidate *rc = first; rc; rc = next) {
    ...
    if (rc->getBlocksLiveOnEntry().intersects(*successorBits)) {
      if ((!onlyReprioritizeLongs ||
           (rc->getType().isInt64() && TR::Compiler->target.is32Bit())) &&
          ((reprioritizeFP && isFPCandidate) ||
           (!reprioritizeFP && !isFPCandidate))) {
        if (trace) {
          dumpOptDetails(comp,
                         "\nBefore repriortization, removing the following "
                         "blocks from live ranges of candidate #%d \n",
                         rc->getSymbolReference()->getReferenceNumber());
          successorBits->print(comp);
        }

        rc->getBlocksLiveOnEntry() -= *successorBits;
        rc->recalculateWeight(blocks, blockStructureWeight, comp, blockGPRCount,
                              blockFPRCount, blockVRFCount, referencedBlocks,
...
static void scanPressureSimulatorCacheForConflicts(
    TR_RegisterCandidate *rc, TR_BitVector &blocks,
    TR_Array<TR_BitVector> &liveOnEntryConflicts,
    TR_Array<TR_BitVector> &liveOnExitConflicts,
    TR_Array<TR_BitVector> &entryExitConflicts, int32_t firstRegister,
    int32_t lastRegister, TR_BitVector *spilledRegs, TR::Compilation *comp,
    TR::Block **all_blocks) {
  ...
  if (trace) {
    ...
    traceMsg(comp, "\n");
    traceMsg(comp,
             "\t   adding conflicts as reg#: { [Entry|Exit](block#) "
             "[Entry|Exit](block#) ...}\n");
  }
  ...
  while (blockIterator.hasMoreElements()) {
    ...
    spilledRegs->empty();
    comp->cg()->computeSimulatedSpilledRegs(spilledRegs, blockNum,
    ...
    if (trace) {
      ...
      while (ri.hasMoreElements()) {
        ...
        if (r < firstRegister || r > lastRegister) tmpSpilledRegs.reset(r);
      }
    ...
    while (regIterator.hasMoreElements()) {
      ...
      if (firstRegister <= regNum && regNum <= lastRegister) {
        TR_ASSERT(rc->getBlocksLiveOnEntry().get(blockNum) ||
                      rc->getBlocksLiveOnExit().get(blockNum),
        ...
        while (rc->getBlocksLiveOnEntry().get(currBlockNum)) {
          ...
          if (!curr || !rc->getBlocksLiveOnExit().get(currBlockNum)) break;
  ...
  if (trace)
    traceMsg(comp,
             "\t Update conflicts on entry due to conflicts on predecessor "
             "exits:\n");
  ...
  for (i = firstRegister; i <= lastRegister; ++i) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      for (auto e = block->getSuccessors().begin();
           e != block->getSuccessors().end(); ++e) {
        TR::Block *succ = toBlock((*e)->getTo());

        if (trace) {
          if (!liveOnEntryConflicts[i].isSet(succ->getNumber()))
            traceMsg(comp, " Entry(%d)<-Exit(%d)", succ->getNumber(),
                     block->getNumber());
        }
    ...
    while (bvi.hasMoreElements()) {
      ...
      for (auto e = block->getSuccessors().begin();
        ...
        if (trace)
          if (!liveOnEntryConflicts[i].isSet(succ->getNumber()))
            traceMsg(comp, " Entry(%d)<-EntryExit(%d)", succ->getNumber(),
                     block->getNumber());
  ...
  if (trace) {
    ...
    while (regIterator.hasMoreElements()) {
      ...
      if (firstRegister <= regNum && regNum <= lastRegister) {
        traceMsg(
            comp,
            "\tFor candidate #%d, on entry block conflicts with register %d ",
            rc->getSymbolReference()->getReferenceNumber(), regNum);
        liveOnEntryConflicts[regNum].print(comp);
        traceMsg(comp, "\n");
        traceMsg(
            comp,
            "\tFor candidate #%d, on exit block conflicts with register %d ",
            rc->getSymbolReference()->getReferenceNumber(), regNum);
...
void TR_RegisterCandidates::collectCfgProperties(TR::Block **blocks,
                                                 int32_t numberOfBlocks) {
  TR::CFG *cfg = comp()->getFlowGraph();

  // Collect firstBlock property. These are BBs that follow the procedure entry.
  _firstBlock.init(cfg->getNextNodeNumber(), _trMemory, stackAlloc,
                   notGrowable);
  _firstBlock.empty();

  TR::CFGNode *start = cfg->getStart();

  for (auto e = start->getSuccessors().begin();
       e != start->getSuccessors().end(); ++e) {
    TR::CFGNode *b = toBlock((*e)->getTo());
    _firstBlock.set(b->getNumber());
  }

  // Collect ExtensionofPreviousBlock property
  // This seems redundant as the property is a simple flag of the block;
  // however, profiles have shown that indexing blocks followed by extracting
  // information from has significant data cache misses. I'm hopeful that
  // bundling this info together into a bit vector to be referenced repeatedly
  // for each RegisterCandidate will speed up compilation of large programs.
  int i;
  _isExtensionOfPreviousBlock.init(cfg->getNextNodeNumber(), _trMemory,
...
static void assign_candidate_loop_trace_increment(TR::Compilation *comp,
                                                  TR_RegisterCandidate *rc,
                                                  unsigned count) {
  bool trace = comp->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
  if (trace) {
...
bool TR_RegisterCandidates::assign(TR::Block **cfgBlocks,
                                   int32_t numberOfBlocks,
                                   int32_t &lowestNumber,
                                   int32_t &highestNumber) {
#if (defined(__IBMCPP__) || defined(__IBMC__)) && !defined(__ibmxl__)
  // __func__ is not defined for this function on XLC compilers (Notably XLC on
  // Linux PPC and ZOS)
  ...
  bool enableVectorGRA = comp()->cg()->getSupportsVectorRegisters() &&
                         !comp()->getOption(TR_DisableVectorRegGRA);

  TR_BitVector catchBlocks(comp()->getFlowGraph()->getNextNodeNumber(),
                           trMemory(), stackAlloc, growable);
  TR_BitVector callBlocks(comp()->getFlowGraph()->getNextNodeNumber(),
                          trMemory(), stackAlloc, growable);
  TR_BitVector switchBlocks(comp()->getFlowGraph()->getNextNodeNumber(),
                            trMemory(), stackAlloc, growable);
  TR_BitVector temp(comp()->getFlowGraph()->getNextNodeNumber(), trMemory(),
                    stackAlloc, growable);
  TR_BitVector highWordZeroLongs(comp()->getSymRefCount(), trMemory(),
                                 stackAlloc, growable);

  TR_BitVector catchBlockLiveLocals(comp()->getSymRefCount(), trMemory(),
                                    stackAlloc, growable);
  TR_BitVector nonOSRCatchBlockLiveLocals(comp()->getSymRefCount(), trMemory(),
                                          stackAlloc, growable);

  TR_BitVector referencedBlocks(comp()->getFlowGraph()->getNextNodeNumber(),
                                trMemory(), stackAlloc, growable);
  highWordZeroLongs.setAll(comp()->getSymRefCount());
  int32_t *blockStructureWeight = (int32_t *)trMemory()->allocateStackMemory(
      comp()->getFlowGraph()->getNextNodeNumber() * sizeof(int32_t));
  memset(blockStructureWeight, 0,
         comp()->getFlowGraph()->getNextNodeNumber() * sizeof(int32_t));
  bool trace =
  ...
  // Visit entire method looking at top nodes recording properties such as
  // blocks with multiple targets, blocks with call, blocks that catch
  // exceptions, and high word candidates
  int32_t blockNum = -1;
  TR::Block *block = NULL;
  ...
  for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop;
    ...
    if (node->getOpCode().isTreeTop() && (node->getNumChildren() > 0)) {
      ...
      if (node->getOpCode().isCall()) isCallBlock = true;
    }
  ...
  while (bvi.hasMoreElements()) {
    ...
    TR::SymbolReference *longVar =
        comp()->getSymRefTab()->getSymRef(nextLongCandidate);
    if (longVar && longVar->getSymbol()->isAuto() &&
        longVar->getSymbol()->getType().isInt64()) {
      TR_RegisterCandidate *registerCandidate =
          comp()->getGlobalRegisterCandidates()->find(longVar);
      if (registerCandidate) registerCandidate->setHighWordZero(true);
    }
  }
  ...
  if (rc == 0) return globalFPAssignmentDone;

  TR::CodeGenerator *cg = comp()->cg();
  TR::Block **blocks = cfgBlocks;

  TR_Array<int32_t> numberOfGPRsLiveOnExit(trMemory(), numberOfBlocks, true,
                                           stackAlloc);
  TR_Array<int32_t> numberOfFPRsLiveOnExit(trMemory(), numberOfBlocks, true,
                                           stackAlloc);
  TR_Array<int32_t> numberOfVRFsLiveOnExit(trMemory(), numberOfBlocks, true,
                                           stackAlloc);
  TR_Array<int32_t> maxGPRsLiveOnExit(trMemory(), numberOfBlocks, true,
                                      stackAlloc);
  TR_Array<int32_t> maxFPRsLiveOnExit(trMemory(), numberOfBlocks, true,
                                      stackAlloc);
  TR_Array<int32_t> maxVRFsLiveOnExit(trMemory(), numberOfBlocks, true,
                                      stackAlloc);
  ...
  do {
    ...
    maxGPRsLiveOnExit[blockNumber] =
        cg->getMaximumNumberOfGPRsAllowedAcrossEdge(b);
    maxFPRsLiveOnExit[blockNumber] =
        cg->getMaximumNumberOfFPRsAllowedAcrossEdge(node);
    maxVRFsLiveOnExit[blockNumber] =
        cg->getMaximumNumberOfVRFsAllowedAcrossEdge(node);
  ...
  TR_Array<int32_t> totalGPRCountOnEntry(trMemory(), numberOfBlocks, true,
                                         stackAlloc);
  TR_Array<int32_t> totalFPRCountOnEntry(trMemory(), numberOfBlocks, true,
                                         stackAlloc);
  TR_Array<int32_t> totalVRFCountOnEntry(trMemory(), numberOfBlocks, true,
                                         stackAlloc);
  ...
  // Collect properties of the CFG that are used in candidate heuristics but are
  // invariant to the candidate and only depend on the structure of the CFG.
  // This will speed up computing the heuristics.
  ...
  for (; rc; rc = next) {
    ...
    rc->setWeight(blocks, blockStructureWeight, comp(), totalGPRCount,
                  totalFPRCount, totalVRFCount, &referencedBlocks,
                  _startOfExtendedBBForBB, _firstBlock,
                  _isExtensionOfPreviousBlock);
    numCandidates++;
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (!seenBlocks.get(blockNum)) {
        ...
        if (isFloat) {
          ...
          if (needs2Regs) ++totalFPRCount[blockNum];
        } else if (isVector) {
          ...
          // totalGPRCount[blockNum]);
          if (needs2Regs) ++totalGPRCount[blockNum];
        }
      }
    }
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (!seenBlocks.get(blockNum)) {
        ...
        if (isFloat) {
          ...
          if (needs2Regs) ++totalFPRCountOnEntry[blockNum];
        } else if (isVector) {
          ...
          // dumpOptDetails(comp(), "Block %d has %d gprs live\n", blockNum,
          // totalGPRCount[blockNum]);
          if (needs2Regs) ++totalGPRCountOnEntry[blockNum];
        }
      }
    }
  ...
  int32_t limitedGRACandidateMax =
      comp()->getOptions()->getMaxLimitedGRACandidates();
  int32_t limitedGRACandidateCount = 0;

  uint8_t maxReprioritized =
      0;  // 0 seems to give better throughput and lower CPU than 1
  ...
  _liveOnEntryConflicts.init(trMemory(), numberOfGlobalRegisters, true,
                             stackAlloc);
  _liveOnExitConflicts.init(trMemory(), numberOfGlobalRegisters, true,
                            stackAlloc);
  _entryExitConflicts.init(trMemory(), numberOfGlobalRegisters, true,
                           stackAlloc);
  _exitEntryConflicts.init(trMemory(), numberOfGlobalRegisters, true,
                           stackAlloc);
#ifdef TRIM_ASSIGNED_CANDIDATES
  _availableBlocks.init(trMemory(), numberOfGlobalRegisters, true, stackAlloc);
#endif
  for (i = 0; i < numberOfGlobalRegisters; i++) {
    _liveOnEntryConflicts[i].init(numberOfBlocks, trMemory(), stackAlloc,
                                  growable);
    _liveOnExitConflicts[i].init(numberOfBlocks, trMemory(), stackAlloc,
                                 growable);
    _entryExitConflicts[i].init(numberOfBlocks, trMemory(), stackAlloc,
                                growable);
    _exitEntryConflicts[i].init(numberOfBlocks, trMemory(), stackAlloc,
                                growable);
#ifdef TRIM_ASSIGNED_CANDIDATES
  ...
  for (rc = first; rc; rc = rc->getNext()) {
    ...
    if (trace) rc->printLoopInfo(comp());
  }
#endif
  ...
  for (rc = first; rc;
       assign_candidate_loop_trace_increment(comp(), rc, iterationCount),
      rc = next) {
    next = rc->getNext();
    ...
    if (comp()->getOption(TR_EnableGRACostBenefitModel)) {
      ...
      float numCandsThresholdFactor =
          (float)((float)numCandsThresholdPercent / (float)100);

      numAssigns++;

      if ((rc->getWeight() < weightThreshold) &&
          (((float)numAssigns) >
           (float)(numCandsThresholdFactor * (float)numCands))) {
        if (trace)
          traceMsg(comp(),
                   "Leaving candidate because the compile time cost is not "
                   "worth it\n");
    ...
    if (type.isInt64() && cg->getDisableLongGRA()) {
      if (trace)
        traceMsg(comp(),
                 "Leaving candidate because LongGRA is disabled and candidate "
                 "is 64 bit\n");
      continue;
    }

    if (dt == TR::Aggregate &&
        (1 || rc->getSymbolReference()->getSymbol()->getSize() > 8)) {
      if (trace)
        traceMsg(comp(),
                 "Leaving candidate because its an aggregate and > 64 bits\n");
      continue;
    }
    ...
    if (dt.isVector() && !comp()->cg()->hasGlobalVRF()) {
      if (trace)
        traceMsg(comp(),
                 "Leaving candidate because it has vector type but no global "
                 "vector registers provided\n");
      TR_ASSERT(!TR::Compiler->target.cpu.isZ(),
                "ed : debug : Should never get here for vector GRA on z");
      continue;
    }

    // don't put this auto into a global register if it can be accessed from a
    // catch clause
    //
    temp = rc->getBlocksLiveOnEntry();
    temp &= catchBlocks;

    if (((catchBlockLiveLocalsExist &&
          rc->getSymbolReference()->getSymbol()->isAuto() &&
          catchBlockLiveLocals.get(rc->getSymbolReference()
                                       ->getSymbol()
                                       ->getAutoSymbol()
                                       ->getLiveLocalIndex())) ||
         ((!catchBlockLiveLocalsExist ||
           !rc->getSymbolReference()->getSymbol()->isAuto()) &&
          !rc->getSymbolReference()->getUseonlyAliases().isZero(comp()))) &&
        (!comp()->penalizePredsOfOSRCatchBlocksInGRA() ||
         ((catchBlockLiveLocalsExist &&
           rc->getSymbolReference()->getSymbol()->isAuto() &&
           nonOSRCatchBlockLiveLocals.get(rc->getSymbolReference()
                                              ->getSymbol()
                                              ->getAutoSymbol()
                                              ->getLiveLocalIndex())) ||
          ((!catchBlockLiveLocalsExist ||
           ...
           !comp()
                ->getSymRefTab()
                ->aliasBuilder.hasUseonlyAliasesOnlyDueToOSRCatchBlocks(
                    rc->getSymbolReference())))))
      // TODO:GRA Decide if this fix is OK -- without it on zemulator we would
      // have a useonlyalias set for metadata and so we would set this propert
      // and then later on not get rid of the store ot metadata, and create a
      // new store to global reg
    ...
    if (isFloat) {
      if (debug("disableGlobalFPRs") || cg->getDisableFpGRA()
#ifdef J9_PROJECT_SPECIFIC
          || (dt == TR::DecimalLongDouble)
#endif
      ) {
      ...
      if (0 && comp()->cg()->spillsFPRegistersAcrossCalls()) {
        ...
        if (!temp.isEmpty()) {
          ...
          // rc->getSymbolReference()->getReferenceNumber()); printf("Discarding
          // FP candidate in %s\n", comp()->getCurrentMethod()->signature());
          continue;
        }
      }

      firstRegister = cg->getFirstGlobalFPR(),
      lastRegister = cg->getLastGlobalFPR();
    } else if (isVector) {
      firstRegister = cg->getFirstGlobalVRF(),
      lastRegister = cg->getLastGlobalVRF();
    } else {
      firstRegister = cg->getFirstGlobalGPR(),
      lastRegister = cg->getLastGlobalGPR();
    }
    ...
    if (true) {
      ...
      if (trace)
        dumpOptDetails(
            comp(), "%d: Inspecting %d %s\t(weight %d)\n\t", iterationCount,
            rc->getSymbolReference()->getReferenceNumber(),
            rc->getSymbolReference()->getSymbol()->isParm() ? "parm " : "",
            rc->getWeight());

      while (bvi.hasMoreElements()) {
        int32_t nextElement = bvi.getNextElement();
        TR_ASSERT(blocks[nextElement]->getNumber() == nextElement,
                  "blocks[x]->getNumber() is not x");
        blockNum = nextElement;

        // dumpOptDetails(comp(), "For block_%d maxGPRs %d maxFPRs %d ext
        // block_%d maxGPRs %d maxFPRs %d maxVRFs %d\n", b->getNumber(),
        // maxGPRsLiveOnExit[b->getNumber()], maxFPRsLiveOnExit[b->getNumber()],
        // blockNum, maxGPRsLiveOnExit[blockNum], maxFPRsLiveOnExit[blockNum],
        // maxFPRsLiveOnExit[blockNum]); dumpOptDetails(comp(), "For block_%d
        // numGPRs %d numFPRs %d ext block_%d numGPRs %d numFPRs %d maxVRFs
        // %d\n", b->getNumber(), totalGPRCount[b->getNumber()],
        // totalFPRCount[b->getNumber()],  blockNum, totalGPRCount[blockNum],
        // totalFPRCount[blockNum], totalVRFCount[blockNum]);

        if (isFloat) {
          if (totalFPRCount[blockNum] >=
              (maxFPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0))) {
            if (trace)
              dumpOptDetails(comp(),
                             "FPR count exceeded at some exit out of extended "
                             "block_%d: %d >= %d\n",
            ...
            TR::TreeTop *exitTree =
                blocks[blockNum]
                    ->getExit();  // Entry()->getExtendedBlockExitTreeTop();
            ...
            for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              ...
              if (hasMultiWayBranch ||
                ...
                if (trace)
                  dumpOptDetails(comp(),
                                 "FPR count exceeded : resetting live on entry "
                                 "block_%d (because of extended block_%d)\n",
                ...
                if (trace)
                  dumpOptDetails(
                      comp(),
                      "FPR count exceeded : resetting live on exit block_%d)\n",
        ...
        if (isVector) {
          if (totalVRFCount[blockNum] >= maxVRFsLiveOnExit[blockNum]) {
            if (trace)
              dumpOptDetails(comp(),
                             "VRF count exceeded at some exit out of extended "
                             "block_%d: %d >= %d\n",
                             blockNum, totalVRFCount[blockNum],
                             maxVRFsLiveOnExit[blockNum]);
            ...
            for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              TR::Block *b = e->getTo()->asBlock();
              if (hasMultiWayBranch || (totalVRFCountOnEntry[b->getNumber()] >=
                                        maxVRFsLiveOnExit[blockNum])) {
                if (trace)
                  dumpOptDetails(comp(),
                                 "VRF count exceeded : resetting live on entry "
                                 "block_%d (because of extended block_%d)\n",
                                 b->getNumber(), blockNum);
                rc->getBlocksLiveOnEntry().reset(b->getNumber());
                ...
                if (trace)
                  dumpOptDetails(
                      comp(),
                      "VRF count exceeded : resetting live on exit block_%d)\n",
                      cursorBlock->getNumber());
          ...
          if (totalGPRCount[blockNum] >=
              (maxGPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0))) {
            if (trace)
              dumpOptDetails(comp(),
                             "GPR count exceeded at some exit out of extended "
                             "block_%d: %d >= %d\n",
            ...
            for (TR::CFGEdge *e = ebsi.getFirst(); e; e = ebsi.getNext()) {
              ...
              if (hasMultiWayBranch ||
                ...
                if (trace)
                  dumpOptDetails(comp(),
                                 "GPR count exceeded : resetting live on entry "
                                 "block_%d (because of extended block_%d)\n",
                                 b->getNumber(), blockNum);
                rc->getBlocksLiveOnEntry().reset(b->getNumber());
                TR::Block *cursorBlock = problematicBlock;
                if (trace)
                  dumpOptDetails(
                      comp(),
                      "GPR count exceeded : resetting live on exit block_%d)\n",
                      cursorBlock->getNumber());
      ...
      if (!sufficientRegisters) continue;
    }
    ...
    if (trace)  // Feel this would be useful by default
    {
      dumpOptDetails(comp(),
                     ...
                     rc->getSymbolReference()->getReferenceNumber(),
                     rc->getReprioritized(), rc->getWeight());
    ...
    computeAvailableRegisters(rc, firstRegister, lastRegister, blocks,
                              &availableRegisters);
    if (trace) {
    ...
    if (enableVectorGRA) {
      for (int i = firstRegister; i <= lastRegister; ++i) {
        if (vrbVecGRA && (cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) &&
            cg->isAliasedGRN(i)) {
          dumpOptDetails(comp(), "ed: alias for %d %s %d %s\t", i,
                         cg->getDebug()->getGlobalRegisterName(i),
                         cg->getOverlappedAliasForGRN(i),
                         cg->getDebug()->getGlobalRegisterName(
                             cg->getOverlappedAliasForGRN(i)));
          dumpOptDetails(comp(),
                         "ffpr %d lfpr %d fopf %d lopr %d fvrf %d lvrf %d "
                         "fovrf %d lovrf %d\n",
                         cg->getFirstGlobalFPR(), cg->getLastGlobalFPR(),
                         cg->getFirstOverlappedGlobalFPR(),
                         cg->getLastOverlappedGlobalFPR(),
                         cg->getFirstGlobalVRF(), cg->getLastGlobalVRF(),
      ...
      for (int i = firstRegister; i <= lastRegister; ++i) {
        if (vrbVecGRA && (cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) &&
            cg->isAliasedGRN(i)) {
          dumpOptDetails(comp(), "ed: rc sym ref: %d\t",
                         rc->getSymbolReference()->getReferenceNumber());
        ...
        if ((cg->isGlobalFPR(i) || cg->isGlobalVRF(i)) && cg->isAliasedGRN(i) &&
            (/*!availableRegisters.isSet(i) */ !availableRegisters.isSet(
                cg->getOverlappedAliasForGRN(i)))) {
          TR_GlobalRegisterNumber overlappedReg =
              cg->getOverlappedAliasForGRN(i);
          if (trace) {
            traceMsg(comp(),
                     "removing %s (%d) from available list since it is aliased "
                     "with %s (%d)\n",
                     cg->getDebug()->getGlobalRegisterName(i), i,
                     cg->getDebug()->getGlobalRegisterName(overlappedReg),
                     overlappedReg);
          }
    ...
    TR_GlobalRegisterNumber registerNumber;
    {
      ...
      registerNumber = cg->pickRegister(rc, blocks, availableRegisters,
                                        otherRegisterNumber, &_candidates);
      if (needs2Regs && (registerNumber > -1)) {
        otherRegisterNumber = 1;
        availableRegisters.reset(registerNumber);
        highRegisterNumber = cg->pickRegister(
            rc, blocks, availableRegisters, otherRegisterNumber, &_candidates);
        availableRegisters.set(registerNumber);
      }
    }

    if ((registerNumber == -1) || (needs2Regs && (highRegisterNumber == -1))) {
      if (!rc->canBeReprioritized())  // only reprioritize a certain number of
                                      // times based on hotness
      {
        if (trace) traceMsg(comp(), "Can't reprioritize anymore\n");

      }
      // If there's no available registers then attempt to remove some of the
      // blocks from the candidate to avoid conflicts with other candidates
      //
      else {
        ...
        int32_t maxClearStructures = -1;  // largest number of clear structures
                                          // of the unavailable registers

        intersection.empty();
        List<TR_Structure> regStructureUses(
            trMemory());  // list of deepest loops where a register is currently
                          // live
        List<TR_Structure> candStructureUses(
            trMemory());  // list of loops where this candidate is currently
                          // live

        // loop over the blocks where this candidate is live, and add the loops
        // that contain those blocks to candStructureUses
        TR_BitVectorIterator bvi(rc->getBlocksLiveOnEntry());
        while (bvi.hasMoreElements()) {
          int32_t blockNumber = bvi.getNextElement();
          TR::Block *block = blocks[blockNumber];
        ...
        // list of block numbers to remove from the candidate's liveOnEntry
        // list.

        const bool useRegisterPressureInfo =
            !comp()->getOption(TR_DisableRegisterPressureSimulation) &&
            cg->hasRegisterPressureInfo() &&
            (!trace ||
             performTransformation(
                 comp(),
                 "%s Including register pressure info for trimming live range "
                 "of #%d\n",
        ...
        if (useRegisterPressureInfo) {
          ...
          //
          TR_BitVector spilledRegs(cg->getNumberOfGlobalRegisters(), trMemory(),
                                   stackAlloc);
          TR_BitVector liveOnEntryOrExit(numberOfBlocks, trMemory(),
                                         stackAlloc);
          liveOnEntryOrExit = rc->getBlocksLiveOnEntry();
          liveOnEntryOrExit |= rc->getBlocksLiveOnExit();
          scanPressureSimulatorCacheForConflicts(
              rc, liveOnEntryOrExit, _liveOnEntryConflicts,
              _liveOnExitConflicts, _entryExitConflicts, firstRegister,
              lastRegister, &spilledRegs, comp(), blocks);
        }

        for (i = firstRegister; i <= lastRegister; ++i) {
          if (trace) {
            traceMsg(comp(),
                     "Searching for blocks/structures with max frequency for "
                     "reg: %d\n",
                     i);
          }
          ...
          if (trace) {
            traceMsg(comp(),
                     "Final live on entry conflicts with register : %d\n", i);
          ...
          if (!needs2Regs) {
            ...
            while (bvi.hasMoreElements()) {
              ...
              TR_BlockStructure *blockStructure = block->getStructureOf();
              int32_t blockWeight = 1;
              TR_BitVector *autosInBlock = getBlocksReferencingSymRef(
                  rc->getSymbolReference()->getReferenceNumber());
              ...
              if (blockStructure) {
                ...
                if (containingLoop && isInnerLoop(comp(), containingLoop)) {
                  TR_RegisterCandidate *regCand =
                      block->getGlobalRegisters(comp())[i]
                  ...
                  if (regCand) {
                    ...
                    int32_t regCandNumLoadsAndStores =
                        regCand->getNumberOfLoadsAndStoresInLoop(
                            containingLoop);
                ...
                if (autosInBlock && autosInBlock->get(block->getNumber())) {
                  // blockWeight = blockStructureWeight[block->getNumber()];
                  blockWeight =
                      rc->_blocks.getNumberOfLoadsAndStores(block->getNumber());
                  ...
                  if (blockWeight > maxFrequency) {
                    ...
                    // collect all loops with blocks at this frequency where
                    ...
                    // collect loops with blocks at this frequency where this
                    // register is live
          ...
          // i.e. how many structures are in candStructureUses but not in
          // regStructureUses
          ...
          for (TR_Structure *s = si.getFirst(); s != NULL; s = si.getNext()) {
            if (!regStructureUses.find(s)) numClearStructures++;
          }
#ifdef TRIM_ASSIGNED_CANDIDATES
          ...
          if (needs2Regs) {
            ...
            for (j = firstRegister; j < i; ++j)
              if (!availableRegisters.get(j)) {
                // use the _liveOnEntryConflicts array element to maintain the
                // list of block numbers to remove from the candidate's
                // liveOnEntry list.
                //
                ...
                while (bvi.hasMoreElements()) {
                  ...
                  if (blockStructure) {
                    // blockWeight = blockStructureWeight[block->getNumber()];
                    blockWeight = rc->_blocks.getNumberOfLoadsAndStores(
                        block->getNumber());
                    if (blockWeight > maxFrequency) {
                      maxFrequency = blockWeight;
                      numBlocksAtMaxFrequency = 1;
                ...
                if (conflictingRegister != -1) {
                  ...
                  if (secondConflictingRegister != -1)
                    intersection &=
                        _liveOnEntryConflicts[secondConflictingRegister];
                }
            ...
            // first check for smaller maximum frequency, then try to get as
            // many clear
            //    structures as possible...finally, look at the number of
            //    conflicts
            //
            if (trace) {
              ...
              traceMsg(comp(),
                       "num clear structures %d; max clear structures %d\n",
                       numClearStructures, maxClearStructures);
              traceMsg(comp(),
                       "num blocks at max freq %d;  min num conflicts %d\n",
                       numBlocksAtMaxFrequency, numberOfConflicts);
            }
                   ...
                   ((numBlocksAtMaxFrequency < numberOfConflicts) ||
                    ((numBlocksAtMaxFrequency == numberOfConflicts) &&
                     (_liveOnEntryConflicts[i].elementCount() <
                      _liveOnEntryConflicts[conflictingRegister]
                          .elementCount()))))))) {
        ...
        if (conflictingRegister != -1) {
          ...
          if (trace) {
            dumpOptDetails(
                comp(),
                "\nDue to conflicts, removing the following blocks from live "
                "on entry ranges of candidate #%d conflicting register %d\n",
                rc->getSymbolReference()->getReferenceNumber(),
                conflictingRegister);
          ...
          TR_BitVector blocksToBeFreed(
              comp()->getFlowGraph()->getNextNodeNumber(), trMemory(),
              stackAlloc, growable);
          ...
          if (!blocksToBeFreed.isEmpty()) {
            ...
            while (bvi.hasMoreElements()) {
              ...
              TR_RegisterCandidate *cand =
                  block->getGlobalRegisters(comp())[conflictingRegister]
                      .getRegisterCandidateOnEntry();
              if (cand) {
                cand->removeAssignedCandidateFromLoop(
                    comp(), loop, conflictingRegister,
          ...
          rc->recalculateWeight(blocks, blockStructureWeight, comp(),
                                totalGPRCount, totalFPRCount, totalVRFCount,
    ...
    if (needs2Regs) {
      if (!performTransformation(
              comp(), "%s assign auto #%d to low reg %d and high reg %d (%s)\n",
              OPT_DETAILS, rc->getSymbolReference()->getReferenceNumber(),
              registerNumber, highRegisterNumber,
              comp()->getDebug() ? comp()->getDebug()->getGlobalRegisterName(
                                       highRegisterNumber)
                                 : "?"))
        continue;
    } else {
      if (!performTransformation(
              comp(), "%s assign auto #%d %s to reg %d (%s)\n", OPT_DETAILS,
              rc->getSymbolReference()->getReferenceNumber(),
              rc->getSymbolReference()->getSymbol()->isMethodMetaData()
                  ? rc->getSymbolReference()
                        ->getSymbol()
              ...
              registerNumber,
              comp()->getDebug()
                  ? comp()->getDebug()->getGlobalRegisterName(registerNumber)
                  : "?"))
        continue;
    }
    ...
    if (isFloat) globalFPAssignmentDone = true;
    ...
    // only the low word reg and only whether the low word global reg has an 8
    // bit counterpart is important
    //
    // rc->setIs8BitGlobalGPR(cg->is8BitGlobalGPR(highRegisterNumber));

    int32_t numRegs = 1;
    if (registerNumber > highestNumber) highestNumber = registerNumber;
    if (registerNumber < lowestNumber) lowestNumber = registerNumber;
    ...
    if (needs2Regs) {
      ...
      if (highRegisterNumber < lowestNumber) lowestNumber = highRegisterNumber;
      numRegs = 2;
    }
    ...
    while (bvi.hasMoreElements()) {
      ...
      blocks[nextElement]
          ->getGlobalRegisters(comp())[registerNumber]
          .setRegisterCandidateOnEntry(rc);
      if (needs2Regs)
        blocks[nextElement]
            ->getGlobalRegisters(comp())[highRegisterNumber]
            .setRegisterCandidateOnEntry(rc);
    }
    ...
    while (bvi.hasMoreElements()) {
      int32_t blockNumber = bvi.getNextElement();
      TR_ASSERT(blocks[blockNumber]->getNumber() == blockNumber,
                "blocks[x]->getNumber() is not x");
      TR::Block *b = blocks[blockNumber];
      ...
      if (b->getGlobalRegisters(comp())[registerNumber]
              .getRegisterCandidateOnExit() != rc) {
        if ((b->getGlobalRegisters(comp())[registerNumber]
                 .getRegisterCandidateOnExit() != NULL) &&
            (b->getGlobalRegisters(comp())[registerNumber]
                 .getRegisterCandidateOnExit() !=
             b->getGlobalRegisters(comp())[registerNumber]
                 .getRegisterCandidateOnEntry()) &&
            (rc != b->getGlobalRegisters(comp())[registerNumber]
                       .getRegisterCandidateOnEntry())) {
          traceMsg(comp(),
                   "Candidate %d instead of candidate %d is required in "
                   "register %d on exit out of block_%d\n",
                   b->getGlobalRegisters(comp())[registerNumber]
                       .getRegisterCandidateOnExit()
                       ->getSymbolReference()
                       ->getReferenceNumber(),
        ...
        } else if ((rc != b->getGlobalRegisters(comp())[registerNumber]
                              .getRegisterCandidateOnEntry()) ||
                   (b->getGlobalRegisters(comp())[registerNumber]
                        .getRegisterCandidateOnExit() == NULL)) {
          // traceMsg(comp(), "block_%d reg %d cand %d\n", b->getNumber(),
          // registerNumber, rc->getSymbolReference()->getReferenceNumber());
          b->getGlobalRegisters(comp())[registerNumber]
              .setRegisterCandidateOnExit(rc);
          // traceMsg(comp(), "block_%d reg %d cand %d\n", b->getNumber(),
          // registerNumber,
      ...
      if (needs2Regs) {
        if (b->getGlobalRegisters(comp())[highRegisterNumber]
                .getRegisterCandidateOnExit() != rc) {
          if ((b->getGlobalRegisters(comp())[highRegisterNumber]
                   .getRegisterCandidateOnExit() != NULL) &&
              (b->getGlobalRegisters(comp())[highRegisterNumber]
               ...
               b->getGlobalRegisters(comp())[highRegisterNumber]
                   .getRegisterCandidateOnEntry()) &&
              (rc != b->getGlobalRegisters(comp())[highRegisterNumber]
                         .getRegisterCandidateOnEntry())) {
            traceMsg(comp(),
                     "Candidate %d instead of candidate %d is required in "
                     "register %d on exit out of block_%d\n",
                     b->getGlobalRegisters(comp())[highRegisterNumber]
                         ...
                         ->getReferenceNumber(),
                     rc->getSymbolReference()->getReferenceNumber(),
                     highRegisterNumber, blockNumber);
            TR_ASSERT(0, "Candidate is required on exit out of block\n");
          } else if ((rc != b->getGlobalRegisters(comp())[highRegisterNumber]
                                .getRegisterCandidateOnEntry()) ||
                     (b->getGlobalRegisters(comp())[highRegisterNumber]
                          .getRegisterCandidateOnExit() == NULL))
            b->getGlobalRegisters(comp())[highRegisterNumber]
      ...
      if (isFloat) {
        if (++numberOfFPRsLiveOnExit[blockNumber] ==
          ...
          for (TR::Block *succBlock = ebsi.getFirst(); succBlock;
               succBlock = ebsi.getNext()) {
            if (totalFPRCountOnEntry[succBlock->getNumber()] ==
                maxFPRsLiveOnExit[blockNumber])
              comp()->getOptimizer()->getSuccessorBitsGRA()->set(
                  succBlock->getNumber());
          }

          next = reprioritizeCandidates(
              next, blocks, blockStructureWeight, numberOfBlocks, b, comp(),
              true, false, &referencedBlocks, totalGPRCount, totalFPRCount,
              totalVRFCount, comp()->getOptimizer()->getSuccessorBitsGRA(),
              trace);
        }
      }
      if (isVector) {
        if (++numberOfVRFsLiveOnExit[blockNumber] ==
            maxVRFsLiveOnExit[blockNumber]) {
          if (!comp()->getOptimizer()->getSuccessorBitsGRA())
            comp()->getOptimizer()->setSuccessorBitsGRA(
                new (trHeapMemory()) TR_BitVector(numberOfBlocks, trMemory(),
          ...
          for (TR::Block *succBlock = ebsi.getFirst(); succBlock;
               succBlock = ebsi.getNext()) {
            if (totalVRFCountOnEntry[succBlock->getNumber()] ==
                maxVRFsLiveOnExit[blockNumber])
              comp()->getOptimizer()->getSuccessorBitsGRA()->set(
                  succBlock->getNumber());
          }
          ...
          next = reprioritizeCandidates(
              next, blocks, blockStructureWeight, numberOfBlocks, b, comp(),
              false, false, &referencedBlocks, totalGPRCount, totalFPRCount,
              totalVRFCount, comp()->getOptimizer()->getSuccessorBitsGRA(),
              trace);
        }
      } else {
        numberOfGPRsLiveOnExit[blockNumber] =
            numberOfGPRsLiveOnExit[blockNumber] + numRegs;
        if (numberOfGPRsLiveOnExit[blockNumber] ==
            maxGPRsLiveOnExit[blockNumber])  // all reg cands need to be
                                             // reprioritized
        {
          // static TR_BitVector *successorBits;
          if (!comp()->getOptimizer()->getSuccessorBitsGRA())
          ...
          for (TR::Block *succBlock = ebsi.getFirst(); succBlock;
               succBlock = ebsi.getNext()) {
            if (totalGPRCountOnEntry[succBlock->getNumber()] ==
                maxGPRsLiveOnExit[blockNumber])
              comp()->getOptimizer()->getSuccessorBitsGRA()->set(
                  succBlock->getNumber());
          }
          ...
          next = reprioritizeCandidates(
              ...
              trace);
                   ...
                   (maxGPRsLiveOnExit[blockNumber] -
                    1))  // only long cands need to be reprioritized
        {
          // static TR_BitVector *successorBits;
          if (!comp()->getOptimizer()->getSuccessorBitsGRA())
            comp()->getOptimizer()->setSuccessorBitsGRA(
          ...
          for (TR::Block *succBlock = ebsi.getFirst(); succBlock;
               succBlock = ebsi.getNext()) {
            if (totalGPRCountOnEntry[succBlock->getNumber()] ==
                (maxGPRsLiveOnExit[blockNumber] - 1))
              comp()->getOptimizer()->getSuccessorBitsGRA()->set(
                  succBlock->getNumber());
          }
          ...
          next = reprioritizeCandidates(
              next, blocks, blockStructureWeight, numberOfBlocks, b, comp(),
              false, true, &referencedBlocks, totalGPRCount, totalFPRCount,
              totalVRFCount, comp()->getOptimizer()->getSuccessorBitsGRA(),
              trace);
        }
      }
    ...
    if (trace) {
      traceMsg(
          comp(),
          "After assigning candidate %d real register %d, entry usage : \n",
    ...
    if (trace) {
      traceMsg(comp(),
               "After assigning candidate %d real register %d, exit usage : \n",
               rc->getSymbolReference()->getReferenceNumber(), registerNumber);
...
void ComputeOverlaps(TR::Node *node, TR::Compilation *comp,
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (itr != overlaps.end())
      ...
      overlaps.insert(std::make_pair(
          ref, TR_RegisterCandidates::coordinates(seqno, seqno)));
    }
  }
}

void ComputeOverlaps(TR::Block *block, TR::Compilation *comp,
                     TR_RegisterCandidates::Coordinates &overlaps) {
  uint32_t seqno = 0;
  comp->incVisitCount();
...
void TR_RegisterCandidates::computeAvailableRegisters(
    TR_RegisterCandidate *rc, int32_t firstRegister, int32_t lastRegister,
    TR::Block **blocks, TR_BitVector *availableRegisters) {
  ...
  for (i = firstRegister; i <= lastRegister; ++i) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      TR_RegisterCandidate *rc1 =
          b->getGlobalRegisters(comp())[i].getRegisterCandidateOnEntry();
      if (rc1) {
        Coordinates::iterator rc1Itr =
            overlaps->find(rc1->getSymbolReference()->getReferenceNumber());
        Coordinates::iterator rcItr =
            overlaps->find(rc->getSymbolReference()->getReferenceNumber());
    ...
    while (bvi.hasMoreElements()) {
      ...
      TR_RegisterCandidate *rc2 =
          b->getGlobalRegisters(comp())[i].getRegisterCandidateOnExit();
      if (rc2) {
        Coordinates::iterator rcItr =
            overlaps->find(rc->getSymbolReference()->getReferenceNumber());
        Coordinates::iterator rc2Itr =
            overlaps->find(rc2->getSymbolReference()->getReferenceNumber());
    ...
    int32_t entryBlockNumber =
        comp()->getStartTree()->getNode()->getBlock()->getNumber();
    TR::Symbol *rcSymbol = rc->getSymbolReference()->getSymbol();
    if (rcSymbol->isParm() &&
        rc->getBlocksLiveOnEntry().get(entryBlockNumber)) {
      int8_t lri = rcSymbol->getParmSymbol()->getLinkageRegisterIndex();
      if (lri >= 0) {
        TR_GlobalRegisterNumber parmReg =
            comp()->cg()->getLinkageGlobalRegisterNumber(
                lri, rcSymbol->getDataType());
        TR_BitVector *linkageRegisters =
            comp()->cg()->getGlobalRegisters(TR_linkageSpill, TR_System);

        bvi.setBitVector(*linkageRegisters);
        while (bvi.hasMoreElements()) {
          int32_t reg = bvi.getNextElement();
          if (reg != parmReg) _liveOnEntryConflicts[reg].set(entryBlockNumber);
        }
      }
    }
    ...
    if (liveOnEntryConflicts.isEmpty() && liveOnExitConflicts.isEmpty() &&
        exitEntryConflicts.isEmpty() && entryExitConflicts.isEmpty() &&
