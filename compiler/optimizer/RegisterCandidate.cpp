...
TR::GlobalSet::GlobalSet(TR::Compilation *comp, TR::Region &region)
    : _region(region),
      ...
      _EMPTY(region), _collected(false), _comp(comp) {}
...
TR_RegisterCandidate::TR_RegisterCandidate(TR::SymbolReference *sr,
                                           TR::Region &r)
    : _symRef(sr), _splitSymRef(NULL), _restoreSymRef(NULL), _lowRegNumber(-1),
      _highRegNumber(-1), _liveOnEntry(), _liveOnExit(), _originalLiveOnEntry(),
      _reprioritized(0), _blocks(r), _loopExitBlocks(r), _stores(r),
      _loopsWithHoles(r), _mostRecentValue(NULL), _lastLoad(NULL) {}
...
TR_RegisterCandidates::TR_RegisterCandidates(TR::Compilation *comp)
    : _compilation(comp), _trMemory(comp->trMemory()),
...
TR_RegisterCandidates::newCandidate(TR::SymbolReference *ref) {
...
void TR_RegisterCandidate::processLiveOnEntryBlocks(
  ...
  if (numberOfBlocks && isPPS) {
    ...
    if (!comp->allocateAtThisOptLevel() &&
      ...
      if (!useProfilingFrequencies)
        _weight = std::max(1, (loadsAndStores * 10000) /
...
static void scanPressureSimulatorCacheForConflicts(
  ...
  if (trace) {
    ...
    blocks.print(comp);
    traceMsg(comp, "\n");
    traceMsg(comp, "\t   adding conflicts as reg#: { [Entry|Exit](block#) "
                   "[Entry|Exit](block#) ...}\n");
  }
  ...
  if (trace)
    traceMsg(comp, "\t Update conflicts on entry due to conflicts on "
                   "predecessor exits:\n");
  ...
  for (rc = first; rc;
    ...
    if (comp()->getOption(TR_EnableGRACostBenefitModel)) {
      ...
      if ((rc->getWeight() < weightThreshold) &&
        ...
        if (trace)
          traceMsg(comp(), "Leaving candidate because the compile time cost is "
                           "not worth it\n");
    ...
    if (type.isInt64() && cg->getDisableLongGRA()) {
      if (trace)
        traceMsg(comp(), "Leaving candidate because LongGRA is disabled and "
                         "candidate is 64 bit\n");
      continue;
    }
    ...
    if (dt.isVector() && !comp()->cg()->hasGlobalVRF()) {
      if (trace)
        traceMsg(comp(), "Leaving candidate because it has vector type but no "
                         "global vector registers provided\n");
