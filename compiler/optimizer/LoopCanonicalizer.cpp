...
void TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(ListAppender<TR_Structure>& whileLoopsInnerFirst,
  List<TR_Structure>& whileLoops, ListAppender<TR_Structure>& doWhileLoopsInnerFirst, List<TR_Structure>& doWhileLoops,
  TR_Structure* root, TR_StructureSubGraphNode* rootNode, TR_RegionStructure* region, vcount_t visitCount,
  TR_BitVector* pendingList, bool innerFirst)
{
  if (trace())
...
void TR_LoopTransformer::detectWhileLoops(ListAppender<TR_Structure>& whileLoopsInnerFirst,
  List<TR_Structure>& whileLoops, ListAppender<TR_Structure>& doWhileLoopsInnerFirst, List<TR_Structure>& doWhileLoops,
  TR_Structure* root, bool innerFirst)
{
  ...
  // if (debug("nonRecursiveBVA"))
  {
    ...
    detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops, region,
    ...
    if (!isDoWhileLoop && loopHeader && (numSubNodes > 1)) {
      ...
      if ((hdrBlock->hasExceptionPredecessors() == false) && (hdrBlock->hasExceptionSuccessors() == false)) {
        ...
        if (entryNode->getSuccessors().size() == 2) {
          ...
          TR::CFGEdge* succ2 = *it;
          bool succ1Internal
            = region->contains(toStructureSubGraphNode(succ1->getTo())->getStructure(), region->getParent());
          bool succ2Internal
            = region->contains(toStructureSubGraphNode(succ2->getTo())->getStructure(), region->getParent());
                  ...
                  for (exitBlock = blocksIt.getCurrent(); exitBlock; exitBlock = blocksIt.getNext()) {
                    if (exitBlock != hdrBlock) {
                      if (exitBlock->getSuccessors().size() == 2) {
                        ...
                        bool successor1Internal
                          = region->contains(toBlock(exitSucc1->getTo())->getStructureOf(), region->getParent());
                        bool successor2Internal
                          = region->contains(toBlock(exitSucc2->getTo())->getStructureOf(), region->getParent());
                        if (successor1Internal ^ successor2Internal) {
                          ...
                          if (exitBlockLoopTestTree->getNode()->getOpCode().isCompBranchOnly()
                            || exitBlockLoopTestTree->getNode()->getOpCode().isJumpWithMultipleTargets())
                            continue;
      ...
      if (!(subNode->getExceptionPredecessors().empty() &&
          ...
          */
          ...
          TR_BlockStructure* pred
            = toStructureSubGraphNode(subNode->getPredecessors().front()->getFrom())->getStructure()->asBlock();
...
bool TR_LoopTransformer::findMatchingIVInRegion(TR::TreeTop* loopTestTree, TR_RegionStructure* region)
{
  ...
  if (firstChild->getOpCode().hasSymbolReference())
    ...
    if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && (firstChild->getReferenceCount() > 1)
...
void TR_LoopCanonicalizer::canonicalizeNaturalLoop(TR_RegionStructure* whileLoop)
{
  ...
  */
  ...
  TR_BlockStructure* clonedHdrBlockStructure
    = new (_cfg->structureRegion()) TR_BlockStructure(comp(), clonedHeader->getNumber(), clonedHeader);
  if (needToSetFlag) {
  ...
  TR_StructureSubGraphNode* hdrNode
    = new (_cfg->structureRegion()) TR_StructureSubGraphNode(loopHeader->getStructureOf());
  properRegion->addSubNode(hdrNode);
  properRegion->setEntry(hdrNode);

  TR_StructureSubGraphNode* node = new (_cfg->structureRegion()) TR_StructureSubGraphNode(
    new (_cfg->structureRegion()) TR_BlockStructure(comp(), splitter2->getNumber(), splitter2));
  ...
  node = new (_cfg->structureRegion()) TR_StructureSubGraphNode(
    new (_cfg->structureRegion()) TR_BlockStructure(comp(), splitter1->getNumber(), splitter1));
  properRegion->addSubNode(node);
  TR::CFGEdge::createEdge(hdrNode, node, trMemory());
  ...
  if (reversedBranch) {
    ...
    TR_BlockStructure* extraGotoStruct
      = new (_cfg->structureRegion()) TR_BlockStructure(comp(), extraGoto->getNumber(), extraGoto);
    ...
    for (edge = ei.getCurrent(); edge; edge = ei.getNext()) {
      ...
      if (toNum == joinNumber) {
        if (fromNode->getStructure() == clonedHdrBlockStructure) {
          edge->setTo(TR_StructureSubGraphNode::create(node->getNumber(),
            whileLoop)); // new (_cfg->structureRegion()) TR_StructureSubGraphNode(node->getNumber()));
        } else
    ...
    if ((!doWhileLoop->contains(pred->getStructureOf())) && (pred != loopInvariantBlock)) {
      *sumPredFreq = *sumPredFreq + (*edge)->getFrequency();
      if (pred != _cfg->getStart()) {
        ...
        TR_ASSERT(
          isCheckOnly, "Cannot reach here if preparation work for the loopInvariantBlock has been done correctly\n");
        if (!isCheckOnly) {
          TR::TreeTop* targetEntry = targetBlock->getEntry();
          TR::TreeTop* targetExit = targetBlock->getExit();
          TR::TreeTop* prevTree = targetEntry->getPrevTreeTop();
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(TR_RegionStructure* doWhileLoop)
{
  ...
  {
    for (auto iedge = blockAtHeadOfLoop->getPredecessors().begin(); iedge != blockAtHeadOfLoop->getPredecessors().end();
         ++iedge) {
  ...
  TR_BlockStructure* invariantBlockStructure
    = new (_cfg->structureRegion()) TR_BlockStructure(comp(), loopInvariantBlock->getNumber(), loopInvariantBlock);
  ...
  if (isEntry) {
    ...
    dummyEntryBlockStructure
      = new (_cfg->structureRegion()) TR_BlockStructure(comp(), dummyEntryBlock->getNumber(), dummyEntryBlock);
  ...
  if (isEntry) {
    TR_StructureSubGraphNode* dummyEntryNode
      = new (_cfg->structureRegion()) TR_StructureSubGraphNode(dummyEntryBlockStructure);
  ...
  if (trace()) {
    traceMsg(
      comp(), "Structure after canonicalizing do while loop %p: number: %d\n", doWhileLoop, doWhileLoop->getNumber());
  ...
  if (primaryInductionVariable && bivIterator.getFirst()) {
    ...
    for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
      ...
      if ((nextIndVar->getSymRef()->getSymbol()->getDataType()
        ...
        // comp()->incVisitCount();
        bool incrementsInLockStep = incrementedInLockStep(naturalLoop, derivedInductionVar, symRefInCompare,
          nextIndVarIncr, primaryIncr, &derivedInductionVarIncrementBlocks, &primaryInductionVarIncrementBlocks);
        if (incrementsInLockStep
          && (!derivedInductionVarIncrementBlocks.isEmpty() || !primaryInductionVarIncrementBlocks.isEmpty())) {
          incrementsInLockStep = checkIfOrderOfBlocksIsKnown(naturalLoop, entryBlock, loopTestBlock,
            &derivedInductionVarIncrementBlocks, &primaryInductionVarIncrementBlocks, _primaryIncrementedFirst);
    ...
    if (alteredCode) {
      if (loopInvariantBlock->getBlock()->getLastRealTreeTop()->getNode()->getOpCodeValue() != TR::BBStart) {
        ...
        // TR_ASSERT((loopInvariantBlock->getBlock()->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto),
        // "Expect the loop pre-header to end in a goto for call to TR::Block::split to work correctly\n");
  ...
  TR::SymbolReference* derivedInductionVar)
{
  ...
  for (auto edge = _entryBlock->getSuccessors().begin(); edge != _entryBlock->getSuccessors().end();) {
    ...
    if (!structure->contains(block->getStructureOf())) {
      ...
      TR::Node* storeNode
        = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1, 1, subNode, derivedInductionVar);
  ...
  if (structure->asBlock() != NULL) {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (!incrementedInLockStep(node->getStructure(), derivedInductionVar, primaryInductionVar,
            derivedInductionVarLoopIncrement, primaryInductionVarLoopIncrement, derivedInductionVarIncrementBlocks,
    ...
    if ((node->getSymbolReference() == _symRefBeingReplaced)
      ...
      if (!loadOfPrimaryInductionVar)
        loadOfPrimaryInductionVar
          = TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(dataType), 0, _primaryInductionVariable);
      ...
      if (_primaryIncrementedFirst == 0) {
        ...
        } else if (0) // Do not need adjustment in this case since we have obtained the correctly commoned node in this
                      // case
        {
          if (_primaryInductionVarStoreInBlock) {
...
bool TR_LoopTransformer::cleanseTrees(TR::Block* block)
{
  ...
  //        treeTop = exitTreeTop->getNextTreeTop())
  {
    ...
    if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
      ...
      {
        TR_ASSERT(
          targetTree->getPrevTreeTop()->getNode()->getOpCodeValue() == TR::BBEnd, "BBStart is not preceded by a BBEnd");
      ...
      if ((prevTreeOfTarget->getNode()->getOpCode().isBranch()
        ...
        // if (isPrevTreeOfTargetReturn || ((prevTreeOfTarget->getNode()->getOpCode().getOpCodeValue() == TR::Goto) ||
        // (prevTreeOfTarget->getNode()->getBranchDestination() == targetTree)))
        {
          ...
          if (treeTop->getPrevTreeTop()) {
            ...
            if (!prevNode->getOpCode().isReturn()) {
              if (!prevNode->getOpCode().isBranch()
                || ((prevNode->getOpCodeValue() != TR::Goto) && (prevNode->getBranchDestination() != treeTop)))
...
TR::Node* TR_LoopTransformer::getCorrectNumberOfIterations(TR::Node* node, TR::Node* constNode)
{
  ...
  case TR::ificmplt: {
    ...
    subNode
      = TR::Node::create(TR::isub, 2, node->getSecondChild()->duplicateTree(), node->getFirstChild()->duplicateTree());
    break;
  }
  case TR::ificmple: {
    // if (!_isAddition)
    ...
    // node->getFirstChild()->duplicateTree()); subNode = TR::Node::create(TR::iadd, 2, subNode, TR::Node::create(node,
    // TR::iconst, 0, 1)); break;
  }
  case TR::ificmpgt: {
    if (_isAddition)
    ...
    subNode
      = TR::Node::create(TR::isub, 2, node->getFirstChild()->duplicateTree(), node->getSecondChild()->duplicateTree());
    break;
  }
  case TR::ificmpge: {
    // if (_isAddition)
    ...
    // node->getSecondChild()->duplicateTree()); subNode = TR::Node::create(TR::iadd, 2, subNode, TR::Node::create(node,
    // TR::iconst, 0, 1)); break;
  }
  default:
    return NULL;
    ...
    if (isInvertibleLoop(nextInductionVariableNumber, loopStructure)) {
      ...
      if (v) {
        ...
        if (entryVal && entryVal->asIntConst()
          ...
          // Workaround is only allow abs(incr)==1.   This is also a hard requirement for unsigned IV to not introduce
          // wraparound. For signed case, we may allow abs(incr)>1, but exit value calc is not just RhsOfStoreNode if it
          // doesn't land on it. todo
        ...
        for (auto edge = nextBlock->getSuccessors().begin(); edge != nextBlock->getSuccessors().end(); ++edge) {
          ...
          if (!blocksInRegion.find(next)) {
            ...
            if (loopTestNode->getOpCodeValue() == TR::ificmpgt)
              storeNode
                = TR::Node::createWithSymRef(TR::istore, 1, 1, rhsOfStoreNode->duplicateTree(), nextInductionSymRef);
            else {
              if (origOpCodeWasAdd)
