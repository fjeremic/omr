...
void TR_LoopCanonicalizer::canonicalizeNaturalLoop(
  ...
  */
  ...
  node = new (_cfg->structureRegion()) TR_StructureSubGraphNode(
      new (_cfg->structureRegion())
          TR_BlockStructure(comp(), splitter1->getNumber(), splitter1));
  properRegion->addSubNode(node);
  TR::CFGEdge::createEdge(hdrNode, node, trMemory());
    ...
    if ((!doWhileLoop->contains(pred->getStructureOf())) &&
      ...
      *sumPredFreq = *sumPredFreq + (*edge)->getFrequency();
      if (pred != _cfg->getStart()) {
        ...
        TR_ASSERT(isCheckOnly, "Cannot reach here if preparation work for the "
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(
  ...
  TR_BlockStructure *invariantBlockStructure = new (_cfg->structureRegion())
      TR_BlockStructure(comp(), loopInvariantBlock->getNumber(),
                        loopInvariantBlock);
...
TR::Node *
TR_LoopTransformer::getCorrectNumberOfIterations(TR::Node *node,
                                                 TR::Node *constNode) {
  TR::Node *subNode;
  ...
  case TR::ificmple: {
    ...
    // TR::Node::create(TR::iadd, 2, subNode, TR::Node::create(node, TR::iconst,
    // 0, 1)); break;
  }
  case TR::ificmpgt: {
    if (_isAddition)
...
TR::Node *
TR_LoopTransformer::updateLoadUsedInLoopIncrement(TR::Node *node,
