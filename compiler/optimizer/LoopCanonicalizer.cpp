...
int32_t TR_LoopCanonicalizer::perform() {
...

  {
    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL;
      ...
      TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),
                "Loop canonicalizer, expecting natural loop");
      if (naturalLoop->getEntryBlock()->isCold()) continue;
    ...
    //
    // Move the trees around in this method so that the blocks that are
    // currently violating the CFG constraints (only 1 branch in a block) are
    // cleansed
    //
    ...
    for (nextInvariantBlock = invariantBlocksIt.getCurrent();
         nextInvariantBlock; nextInvariantBlock = invariantBlocksIt.getNext()) {
    ...
    /*
       ...
       nextWhileLoop = whileLoopsIt.getNext())
          {
          TR_RegionStructure *naturalLoop = nextWhileLoop->asRegion();
          TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),"Loop
       canonicalizer, expecting natural loop"); if
       (naturalLoop->getEntryBlock()->isCold()) continue;
          eliminateRedundantInductionVariablesFromLoop(naturalLoop);
          }

       for (nextDoWhileLoop = doWhileLoopsIt.getFirst(); nextDoWhileLoop !=
       NULL; nextDoWhileLoop = doWhileLoopsIt.getNext())
          {
          TR_RegionStructure *naturalLoop = nextDoWhileLoop->asRegion();
          TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),"Loop
       canonicalizer, expecting natural loop"); if
  ...
  // if (trace())
  {
    if (trace()) comp()->dumpMethodTrees("Trees after canonicalization\n");
  }

  return 1;  // actual cost
}

void TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(
    ListAppender<TR_Structure> &whileLoopsInnerFirst,
    List<TR_Structure> &whileLoops,
    ListAppender<TR_Structure> &doWhileLoopsInnerFirst,
    List<TR_Structure> &doWhileLoops, TR_Structure *root,
    TR_StructureSubGraphNode *rootNode, TR_RegionStructure *region,
    vcount_t visitCount, TR_BitVector *pendingList, bool innerFirst) {
  if (trace())
    traceMsg(comp(),
             "Begin looking for canonicalizable loops in node %p numbered %d\n",
             root, root->getNumber());

  bool alreadyVisitedNode = false;
  // if (rootNode->getVisitCount() == visitCount)
  if (_nodesInCycle->get(rootNode->getNumber())) alreadyVisitedNode = true;

  // rootNode->setVisitCount(visitCount);
  _nodesInCycle->set(rootNode->getNumber());

  for (auto pred = rootNode->getPredecessors().begin();
       pred != rootNode->getPredecessors().end(); ++pred) {
    ...
    if (pendingList->get(predStructure->getNumber()) && (!alreadyVisitedNode)) {
      detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops,
                                        doWhileLoopsInnerFirst, doWhileLoops,
                                        predStructure, predNode, region,
                                        visitCount, pendingList, innerFirst);
      return;
    }
  }

  for (auto pred = rootNode->getExceptionPredecessors().begin();
       pred != rootNode->getExceptionPredecessors().end(); ++pred) {
    TR_StructureSubGraphNode *predNode =
        (TR_StructureSubGraphNode *)(*pred)->getFrom();
    TR_Structure *predStructure = predNode->getStructure();
    if (pendingList->get(predStructure->getNumber()) && (!alreadyVisitedNode)) {
  ...
  _nodesInCycle->empty();
  detectWhileLoops(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst,
                   doWhileLoops, root, innerFirst);
  pendingList->reset(root->getNumber());

  for (auto succ = rootNode->getSuccessors().begin();
       succ != rootNode->getSuccessors().end(); ++succ) {
    TR_StructureSubGraphNode *succNode =
        (TR_StructureSubGraphNode *)(*succ)->getTo();
    ...
    if ((!isExitEdge) && pendingList->get(succStructure->getNumber())) {
      ...
      detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops,
                                        doWhileLoopsInnerFirst, doWhileLoops,
                                        succStructure, succNode, region,
                                        visitCount, pendingList, innerFirst);
    }
  }

  for (auto succ = rootNode->getExceptionSuccessors().begin();
       succ != rootNode->getExceptionSuccessors().end(); ++succ) {
    TR_StructureSubGraphNode *succNode =
        (TR_StructureSubGraphNode *)(*succ)->getTo();
    TR_Structure *succStructure = succNode->getStructure();
    bool isExitEdge = false;
...
void TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(
    ...
    List<TR_Structure> &whileLoops,
    ListAppender<TR_Structure> &doWhileLoopsInnerFirst,
    List<TR_Structure> &doWhileLoops, TR_RegionStructure *region,
    vcount_t visitCount, TR_BitVector *pendingList, bool innerFirst) {
  while (!_analysisStack.isEmpty() &&
    ...
    if (!_nodesInCycle->get(rootNode->getNumber())) {
      ...
      for (auto precedingEdge = precedingEdges.getFirst(); precedingEdge;
        ...
        TR_StructureSubGraphNode *predNode =
            (TR_StructureSubGraphNode *)precedingEdge->getFrom();
        TR_Structure *predStructure = predNode->getStructure();
    ...
    for (auto succeedingEdge = succeedingEdges.getFirst(); succeedingEdge;
      ...
      TR_StructureSubGraphNode *succNode =
          (TR_StructureSubGraphNode *)succeedingEdge->getTo();
      TR_Structure *succStructure = succNode->getStructure();
      bool isExitEdge = false;
      ListIterator<TR::CFGEdge> ei(&region->getExitEdges());
      TR::CFGEdge *edge;
...
void TR_LoopTransformer::createWhileLoopsList(
    TR_ScratchList<TR_Structure> *whileLoops) {
  TR_ScratchList<TR::CFGNode> newEmptyBlocks(trMemory());
  TR_ScratchList<TR::CFGNode> newEmptyExceptionBlocks(trMemory());
  ...
  if (asLoopVersioner())
    detectWhileLoops(whileLoopsInnerFirst, *whileLoops, doWhileLoopsInnerFirst,
                     doWhileLoops, _rootStructure, false);
  else
    detectWhileLoops(whileLoopsInnerFirst, *whileLoops, doWhileLoopsInnerFirst,
                     doWhileLoops, _rootStructure, true);
 ...
 */
void TR_LoopTransformer::detectWhileLoops(
    ListAppender<TR_Structure> &whileLoopsInnerFirst,
    List<TR_Structure> &whileLoops,
    ListAppender<TR_Structure> &doWhileLoopsInnerFirst,
  ...
  TR_BitVector *pendingList =
      new (trStackMemory()) TR_BitVector(numberOfNodes, trMemory(), stackAlloc);
  ...
  // if (debug("nonRecursiveBVA"))
  {
    ...
    detectWhileLoopsInSubnodesInOrder(
        whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops,
        region, visitCount, pendingList, innerFirst);
  }
  /*
else
  detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst, whileLoops,
doWhileLoopsInnerFirst, doWhileLoops, region->getEntry()->getStructure(),
region->getEntry(), region, visitCount, pendingList, innerFirst);
  */

  TR_RegionStructure::Cursor si(*region);
  for (node = si.getCurrent(); node; node = si.getNext()) numSubNodes++;

  if (!region->isNaturalLoop()) return;
  ...
  */
  ...
  if (asLoopCanonicalizer()) {
    ...
    if ((loopHeader && loopHeader->getBlock()->getNextBlock() &&
        ...
        (loopHeader && loopHeader->getBlock()
                           ->getLastRealTreeTop()
                           ->getNode()
                           ->getOpCode()
    ...
    if (!isDoWhileLoop && loopHeader && (numSubNodes > 1)) {
      ...
      if ((hdrBlock->hasExceptionPredecessors() == false) &&
        ...
        // There must be two edges out of the header, one internal and one
        // external.
        //
        if (entryNode->getSuccessors().size() == 2) {
          ...
          bool succ1Internal = region->contains(
              toStructureSubGraphNode(succ1->getTo())->getStructure(),
              region->getParent());
          bool succ2Internal = region->contains(
              toStructureSubGraphNode(succ2->getTo())->getStructure(),
              region->getParent());
          if (succ1Internal ^ succ2Internal) {
            TR::TreeTop *firstTree = hdrBlock->getFirstRealTreeTop();
            static const bool skipTt =
                feGetEnv("TR_canonicalizerStopAtTreetop") == NULL;
            static const bool skipCRAnchor =
                feGetEnv("TR_canonicalizerStopAtCompressedRefsAnchor") == NULL;
            while ((firstTree &&
                    (firstTree->getNode()->getOpCodeValue() == TR::asynccheck ||
                     (skipTt &&
                      firstTree->getNode()->getOpCodeValue() == TR::treetop) ||
                     (skipCRAnchor && firstTree->getNode()->getOpCodeValue() ==
                                          TR::compressedRefs) ||
            ...
            if (firstTree == hdrBlock->getLastRealTreeTop() ||
              ...
              if (!exitBlocks.isSingleton()) {
                TR::TreeTop *hdrBlockLoopTestTree =
                    hdrBlock->getLastRealTreeTop();
                ...
                if (!foundInductionVariable) {
                  ...
                  TR::Block *exitBlock;
                  for (exitBlock = blocksIt.getCurrent(); exitBlock;
                       exitBlock = blocksIt.getNext()) {
                    if (exitBlock != hdrBlock) {
                      if (exitBlock->getSuccessors().size() == 2) {
                        ...
                        if (successor1Internal ^ successor2Internal) {
                          ...
                          foundInductionVariable = findMatchingIVInRegion(
                              exitBlockLoopTestTree, region);
                        }
                      }
                    }
    ...
    if ((loopHeader && loopHeader->getBlock()
                           ->getLastRealTreeTop()
                           ->getNode()
                           ->getOpCode()
                           .isJumpWithMultipleTargets()) ||
    ...
    if (parentStructure) {
      ...
      if (!(subNode->getExceptionPredecessors().empty() &&
          ...
          */
          ...
          if (subNode->getPredecessors().empty()) return;

          TR_BlockStructure *pred =
              toStructureSubGraphNode(
                  subNode->getPredecessors().front()->getFrom())
                  ->getStructure()
                  ->asBlock();
          if (!pred || !pred->isLoopInvariantBlock()) return;

          // This condition is required as we cannot create new catch blocks
          // easily; a catch block normally has some specific info (handler info
          // etc.) that are created by IL generator and if we try to create new
          // catch blocks, then there was a crash when generating exception
          // table entries.
          //
          ...
          for (nextBlock = blocksIt.getCurrent(); nextBlock;
            ...
            if (!nextBlock->getExceptionPredecessors().empty()) return;
    ...
    // bool pred1Internal =
    // region->contains(toStructureSubGraphNode(pred1->getFrom())->getStructure(),
    // region->getParent()); bool pred2Internal =
    // region->contains(toStructureSubGraphNode(pred2->getFrom())->getStructure(),
    // region->getParent()); if (!(pred1Internal ^ pred2Internal))
    //   return;
  }
...
bool TR_LoopTransformer::findMatchingIVInRegion(TR::TreeTop *loopTestTree,
  ...
  if (!firstChildSymRef) return false;

  if (region->findMatchingIV(firstChildSymRef)) return true;
  ...
  ListIterator<TR_BasicInductionVariable> it(
      &region->getBasicInductionVariables());
 ...
 * goto.
 */
void TR_LoopCanonicalizer::canonicalizeNaturalLoop(
  ...
  if (trace()) comp()->dumpMethodTrees("Trees at this stage");

  TR_StructureSubGraphNode *entryGraphNode = whileLoop->getEntry();
  TR_ASSERT(entryGraphNode->getStructure()->asBlock(),
            "Loop canonicalizer, header is not a block");
  ...
  for (auto nextSucc = loopHeader->getSuccessors().begin();
       nextSucc != loopHeader->getSuccessors().end(); ++nextSucc) {
  ...
  if (!_blocksToBeCleansed.find(loopHeader))
    ...
    TR_ASSERT(loopHeader->getLastRealTreeTop()->getNode()->getOpCodeValue() ==
                  TR::Goto,
              "Unexpected opcode instead of TR::Goto\n");
    fallThroughBlock = loopHeader->getLastRealTreeTop()
                           ->getNode()
  ...
  TR::Block *clonedHeader = TR::Block::createEmptyBlock(
      entryNode, comp(), loopHeader->getFrequency(), loopHeader);
  ...
  for (auto edge = loopHeader->getPredecessors().begin();
       edge != loopHeader->getPredecessors().end(); ++edge) {
    TR::Block *pred = toBlock((*edge)->getFrom());

    if ((*edge)->getCreatedByTailRecursionElimination()) needToSetFlag = true;
    ...
    if (!whileLoop->contains(pred->getStructureOf(), whileLoop->getParent())) {
      ...
      //
      if (pred->getExit())
        loopHeader->getEntry()->getNode()->copyByteCodeInfo(
            pred->getExit()->getNode());
      ...
      if (!(pred == _cfg->getStart())) {
        ...
        if (lastNonFenceTree->getNode()->getOpCode().isBranch()) {
          ...
          //
          if (lastNonFenceTree->getNode()->getBranchDestination() !=
              entryTree) {
            ...
            TR::TreeTop *gotoTreeTop = TR::TreeTop::create(
                comp(), TR::Node::create(entryNode, TR::Goto, 0, entryTree));
        ...
        } else if (!lastNonFenceTree->getNode()
                        ->getOpCode()
                        .isJumpWithMultipleTargets() &&
                   !lastNonFenceTree->getNode()
                        ->getOpCode()
                        .isCompBranchOnly()) {
          if (adjPred) {
      ...
      TR::CFGEdge *newEdge =
          TR::CFGEdge::createEdge(pred, clonedHeader, trMemory());
      ...
      if (pred != _cfg->getStart()) {
        ...
        if (lastNonFenceTree->getNode()->getOpCode().isBranch() ||
          ...
          // case.
          //
          if (!lastNonFenceTree->adjustBranchOrSwitchTreeTop(comp(), entryTree,
                                                             newEntryTree)) {
            TR::TreeTop *gotoTreeTop = TR::TreeTop::create(
                comp(), TR::Node::create(entryNode, TR::Goto, 0, newEntryTree));
            TR::TreeTop *treeTop = lastNonFenceTree->getNextTreeTop();
            lastNonFenceTree->join(gotoTreeTop);
            gotoTreeTop->join(treeTop);
            _blocksToBeCleansed.add(pred);
  ...
  while (!(currentTree == exitTree)) {
    ...
    for (int32_t childNum = 0;
      ...
      TR::Node *duplicateChildNode =
          duplicateExact(currentTree->getNode()->getChild(childNum), &seenNodes,
                         &duplicateNodes);
  ...
  */
  ...
  TR::Block *splitter1 = TR::Block::createEmptyBlock(
      loopHeader->getEntry()->getNode(), comp(),
  ...
  if (reversedBranch) newBlocks.add(extraGoto);
  ...
  if (loopBody == fallThroughBlock) {
    ...
    if (_startOfHeader->getNextTreeTop()) _startOfHeader = newExitTree1;
  } else {
    newExitTree1->join(newEntryTree2);
    newExitTree2->join(exitTree->getNextTreeTop());
    exitTree->join(newEntryTree1);
    if (_startOfHeader->getNextTreeTop()) _startOfHeader = newExitTree2;
  }
  ...
  if (reversedBranch) {
    ...
    //
    TR::CFGEdge *newEdge =
        TR::CFGEdge::createEdge(clonedHeader, extraGoto, trMemory());
    _cfg->addEdge(newEdge);
    predBlock = extraGoto;
  } else
  ...
  //
  TR::CFGEdge *newEdge;
  for (auto nextEdge = loopHeader->getSuccessors().begin();
       nextEdge != loopHeader->getSuccessors().end(); ++nextEdge) {
  ...
  if (newLoopHeaderFrequency > 10000) newLoopHeaderFrequency = 10000;

  if (trace()) {
    traceMsg(comp(),
             "Setting frequency of peeled iteration block_%d to %d, s1 "
             "block_%d to %d, s2 block_%d to %d\n",
             loopHeader->getNumber(), newLoopHeaderFrequency,
             splitter1->getNumber(), exitBlockFrequency, splitter2->getNumber(),
             sumPredFreq);
  }
  ...
  if (_blocksToBeCleansed.find(loopHeader)) {
    TR::TreeTop *prevNonFenceTreeInHdr =
        lastNonFenceTreeInHdr->getPrevRealTreeTop();
  ...
  if (fallThroughBlock == loopBody) {
    lastNonFenceTreeInHdr->getNode()->setBranchDestination(
        splitter1->getEntry());
  } else {
    lastNonFenceTreeInHdr->getNode()->setBranchDestination(
        splitter2->getEntry());
  ...
  for (TR::CFGEdge *nextEdge = removedEdgesIt.getFirst(); nextEdge != NULL;
       nextEdge = removedEdgesIt.getNext()) {
  ...
  TR::Block *nextBlock;
  for (nextBlock = newBlocksIt.getFirst(); nextBlock;
       nextBlock = newBlocksIt.getNext()) {
  ...
  if (trace()) {
    traceMsg(
        comp(),
        "Cloning header block : %d(%p) as block_%d(%p) for canonicalization\n",
        loopHeader->getNumber(), loopHeader, clonedHeader->getNumber(),
        clonedHeader);
  ...
  for (auto edge = entryGraphNode->getSuccessors().begin();
       edge != entryGraphNode->getSuccessors().end(); ++edge) {
  ...
  TR_RegionStructure *properRegion = new (_cfg->structureRegion())
      TR_RegionStructure(comp(), loopHeader->getNumber());
  ...
  TR_BlockStructure *clonedHdrBlockStructure = new (_cfg->structureRegion())
      TR_BlockStructure(comp(), clonedHeader->getNumber(), clonedHeader);
  if (needToSetFlag) {
    ...
    if (trace()) traceMsg(comp(), "Marked block %p\n", bodyNode->getNumber());
  }

  clonedHdrBlockStructure->setWasHeaderOfCanonicalizedLoop(true);
  whileLoop->replacePart(entryGraphNode->getStructure(),
                         clonedHdrBlockStructure);

  TR_StructureSubGraphNode *hdrNode = new (_cfg->structureRegion())
      TR_StructureSubGraphNode(loopHeader->getStructureOf());
  properRegion->addSubNode(hdrNode);
  properRegion->setEntry(hdrNode);

  TR_StructureSubGraphNode *node = new (_cfg->structureRegion())
      TR_StructureSubGraphNode(new (_cfg->structureRegion()) TR_BlockStructure(
          comp(), splitter2->getNumber(), splitter2));
  ...
  node = new (_cfg->structureRegion())
      TR_StructureSubGraphNode(new (_cfg->structureRegion()) TR_BlockStructure(
          comp(), splitter1->getNumber(), splitter1));
  properRegion->addSubNode(node);
  TR::CFGEdge::createEdge(hdrNode, node, trMemory());
  ...
  if (reversedBranch) {
    ...
    TR_BlockStructure *extraGotoStruct = new (_cfg->structureRegion())
        TR_BlockStructure(comp(), extraGoto->getNumber(), extraGoto);
    node =
        new (_cfg->structureRegion()) TR_StructureSubGraphNode(extraGotoStruct);
    properRegion->addSubNode(node);
    ...
    for (edge = ei.getCurrent(); edge; edge = ei.getNext()) {
      ...
      if (toNum == joinNumber) {
        if (fromNode->getStructure() == clonedHdrBlockStructure) {
          edge->setTo(TR_StructureSubGraphNode::create(
              node->getNumber(),
              whileLoop));  // new (_cfg->structureRegion())
                            // TR_StructureSubGraphNode(node->getNumber()));
        } else
...
bool TR_LoopCanonicalizer::isLegalToSplitEdges(TR_RegionStructure *doWhileLoop,
                                               TR::Block *blockAtHeadOfLoop) {
  // checking legality before loopInvariantBlock is created, so can set it to
  // null
  int32_t sumPredFreq;
  return modifyBranchesForSplitEdges(doWhileLoop, blockAtHeadOfLoop, NULL, NULL,
                                     false, &sumPredFreq,
                                     /* isCheckOnly = */ true);
}

/**
 * Looks at all predecessor blocks to blockAtHeadOfLoop, and if they are
 * external to the doWhileLoop, modifies any branches or direct paths on their
 * last tree top so that their new destination is the targetBlock, instead of
 * blockAtHeadOfLoop.
 *
 * @param isCheckOnly if true (not the default) just check if legal, don't
 * transform.
 */
bool TR_LoopCanonicalizer::modifyBranchesForSplitEdges(
    TR_RegionStructure *doWhileLoop, TR::Block *blockAtHeadOfLoop,
    TR::Block *loopInvariantBlock, TR::Block *targetBlock, bool addToEnd,
    int32_t *sumPredFreq, bool isCheckOnly) {
  TR_ASSERT(
      (!isCheckOnly) || (isCheckOnly && (loopInvariantBlock == NULL) &&
                         (targetBlock == NULL)),
    ...
    if ((!doWhileLoop->contains(pred->getStructureOf())) &&
      ...
      *sumPredFreq = *sumPredFreq + (*edge)->getFrequency();
      if (pred != _cfg->getStart()) {
        ...
        if (isCheckOnly) {
          if (!lastTree->isLegalToChangeBranchDestination(comp())) return false;
        } else {
          if (!lastTree->adjustBranchOrSwitchTreeTop(comp(), blockHeadTreeTop,
            ...
            if (addToEnd) {
              ...
              if ((targetBlock->getLastRealTreeTop()
                       ->getNode()
                       ->getOpCodeValue() == TR::Goto) &&
                  (targetBlock->getLastRealTreeTop()
                       ->getNode()
                       ->getBranchDestination() == blockHeadTreeTop))
                targetEntry->join(targetExit);
            }
          }
        }
      } else {
        // it is okay to reach here if checking only, as the preparation work
        // will not have been done
        TR_ASSERT(isCheckOnly,
                  "Cannot reach here if preparation work for the "
                  "loopInvariantBlock has been done correctly\n");
        if (!isCheckOnly) {
          TR::TreeTop *targetEntry = targetBlock->getEntry();
          TR::TreeTop *targetExit = targetBlock->getExit();
          TR::TreeTop *prevTree = targetEntry->getPrevTreeTop();
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(
    TR_RegionStructure *doWhileLoop) {
  ...
  TR::Block *blockAtHeadOfLoop =
      blockNode->getStructure()->asBlock()->getBlock();

  if (!isLegalToSplitEdges(doWhileLoop, blockAtHeadOfLoop)) return;
  ...
  if (blockHeadTreeTop->getPrevTreeTop() == NULL) return;

  if (!comp()->getOption(
          TR_DisableEmptyPreHeaderCheck))  // Skip adding another pre-header if
                                           // you don't need to.
  {
    for (auto iedge = blockAtHeadOfLoop->getPredecessors().begin();
      ...
      if (ipred->getStructureOf()->isLoopInvariantBlock() &&
          ipred->isEmptyBlock() &&
          !(ipred == ipred->getStructureOf()
                         ->getParent()
                         ->asRegion()
                         ->getEntryBlock())) {
        return;  // if there is an empty pre-header which is not the entry block
                 // of a region, don't add another.
      }
    }
  }

  TR::Node *predBlockExitNode = blockAtHeadOfLoop->getPredecessors()
                                    .front()
                                    ->getFrom()
                                    ->asBlock()
                                    ->getExit()
  ...
  TR::Block *loopInvariantBlock = TR::Block::createEmptyBlock(
      predBlockExitNode, comp(), blockAtHeadOfLoop->getFrequency(),
      blockAtHeadOfLoop);
  ...
  bool addToEnd;
  if (doWhileLoop->contains(
          cursorTree->getNode()->getBlock()->getStructureOf())) {
    addToEnd = true;
    endTree->join(newEntry);
    TR::TreeTop *gotoTreeTop = TR::TreeTop::create(
        comp(), TR::Node::create(blockHeadNode, TR::Goto, 0, blockHeadTreeTop));
  ...
  TR_BlockStructure *invariantBlockStructure =
      new (_cfg->structureRegion()) TR_BlockStructure(
          comp(), loopInvariantBlock->getNumber(), loopInvariantBlock);
  ...
  if (isEntry) {
    ...
    if (addToEnd) {
      ...
      TR::TreeTop *gotoTreeTop = TR::TreeTop::create(
          comp(), TR::Node::create(blockHeadNode, TR::Goto, 0, newEntry));
    ...
    dummyEntryBlockStructure = new (_cfg->structureRegion()) TR_BlockStructure(
        comp(), dummyEntryBlock->getNumber(), dummyEntryBlock);
  ...
  TR::CFGEdge *loopInvariantBlock_blockAtHeadOfLoop_Edge =
      TR::CFGEdge::createEdge(loopInvariantBlock, blockAtHeadOfLoop,
                              trMemory());
  _cfg->addEdge(loopInvariantBlock_blockAtHeadOfLoop_Edge);
  TR::CFGEdge *dummyEntryBlock_loopInvariantBlock_Edge = NULL;
  if (isEntry) {
    dummyEntryBlock_loopInvariantBlock_Edge = TR::CFGEdge::createEdge(
        dummyEntryBlock, loopInvariantBlock, trMemory());
    _cfg->addEdge(dummyEntryBlock_loopInvariantBlock_Edge);
  }

  int32_t sumPredFreq;
  modifyBranchesForSplitEdges(doWhileLoop, blockAtHeadOfLoop,
                              loopInvariantBlock, targetBlock, addToEnd,
                              &sumPredFreq);

  loopInvariantBlock->setFrequency(sumPredFreq);
  // This edge should have min(frequency of loopInvariantBlock, frequency of
  // blockAtHeadOfLoop) Since we just calculated the actual frequency of
  // loopInvariantBlock, we need to apply that to the edge if necessary
  ...
  if (isEntry) {
    TR_StructureSubGraphNode *dummyEntryNode = new (_cfg->structureRegion())
        TR_StructureSubGraphNode(dummyEntryBlockStructure);
    ...
    dummyEntryBlockStructure->renumberRecursively(dummyEntryNode->getNumber(),
                                                  doWhileNumber);
  ...
  for (auto predIt = doWhileNode->getPredecessors().begin();
       predIt != doWhileNode->getPredecessors().end();) {
    if ((*predIt)->getFrom() != invariantNode) {
      /* Store a reference to current element, is removed in the
       * removePredecessor */
      TR::CFGEdge *nextPred = *(predIt++);
      doWhileNode->removePredecessor(nextPred);
      nextPred->setTo(targetNode);
      TR_RegionStructure *pred = toStructureSubGraphNode(nextPred->getFrom())
                                     ->getStructure()
                                     ->asRegion();
      if (pred && !isEntry)
        pred->replaceExitPart(doWhileNode->getNumber(),
                              targetStructure->getNumber());
    } else
      ++predIt;
  }
  ...
  if (trace()) {
    traceMsg(comp(),
             "Structure after canonicalizing do while loop %p: number: %d\n",
             doWhileLoop, doWhileLoop->getNumber());
    if (comp()->getFlowGraph()->getStructure())
      getDebug()->print(comp()->getOutFile(),
                        comp()->getFlowGraph()->getStructure(), 6);
  }

  return;
}

void TR_LoopCanonicalizer::eliminateRedundantInductionVariablesFromLoop(
    TR_RegionStructure *naturalLoop) {
  // if (!debug("enableRednIndVarElim"))
  // return;

  static char *disableInductionVariableReplacement =
      feGetEnv("TR_disableIndVarOpt");

  if (disableInductionVariableReplacement) return;
  ...
  TR_RegionStructure::Cursor si(*parentStructure);
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    if (subNode->getNumber() == naturalLoop->getNumber()) break;
  }
  ...
  if (!loopInvariantBlock) return;

  TR::Block *entryBlock = naturalLoop->getEntryBlock();
  TR::Block *loopTestBlock = NULL;
  TR::TreeTop *loopTestTree = NULL;

  for (auto nextEdge = entryBlock->getPredecessors().begin();
       nextEdge != entryBlock->getPredecessors().end(); ++nextEdge) {
  ...
  /*
     ...
     ||
                   ((v->getLocal()->getDataType() == TR::Int64) &&
                    (v->getIncr()->getLowLong() ==
     v->getIncr()->getHighLong()))))
                 {
  ...
  TR_PrimaryInductionVariable *primaryInductionVariable =
      naturalLoop->getPrimaryInductionVariable();
  ListIterator<TR_BasicInductionVariable> bivIterator(
      &naturalLoop->getBasicInductionVariables());
  if (primaryInductionVariable && bivIterator.getFirst()) {
    // printf("Found primary induction variable in loop %d in method %s\n",
    // naturalLoop->getNumber(), comp()->signature()); fflush(stdout);
    dumpOptDetails(comp(),
                   "Found primary induction variable in loop %d in method %s\n",
    ...
    //   primaryIncr = (int64_t)
    //   primaryInductionVariable->getIncr()->getLowInt();
    // else
    ...
    for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
      ...
      if ((nextIndVar->getSymRef()->getSymbol()->getDataType() ==
        ...
        TR_ScratchList<TR::Block> primaryInductionVarIncrementBlocks(
            trMemory());
        // comp()->incVisitCount();
        bool incrementsInLockStep = incrementedInLockStep(
            naturalLoop, derivedInductionVar, symRefInCompare, nextIndVarIncr,
            primaryIncr, &derivedInductionVarIncrementBlocks,
            &primaryInductionVarIncrementBlocks);
        if (incrementsInLockStep &&
            (!derivedInductionVarIncrementBlocks.isEmpty() ||
             !primaryInductionVarIncrementBlocks.isEmpty())) {
          incrementsInLockStep = checkIfOrderOfBlocksIsKnown(
              naturalLoop, entryBlock, loopTestBlock,
              &derivedInductionVarIncrementBlocks,
              &primaryInductionVarIncrementBlocks, _primaryIncrementedFirst);
        ...
        if (incrementsInLockStep) {
          ...
          if ((!comp()->cg()->hasComplexAddressingMode()) ||
            ...
            if (performTransformation(
                    comp(), "%sRemoving redundant induction variable #%d\n",
                    OPT_DETAILS, derivedInductionVar->getReferenceNumber()))
              replaceAllInductionVariableComputations(
                  loopInvariantBlock->getBlock(), naturalLoop,
                  &newSymbolReference, symRefInCompare);
          }

          if (newSymbolReference) {
            optimizer()->setAliasSetsAreValid(false);
            placeInitializationTreeInLoopPreHeader(
                loopInvariantBlock->getBlock(),
                loopInvariantBlock->getBlock()->getEntry()->getNode(),
                newSymbolReference, symRefInCompare, derivedInductionVar);
            replaceInductionVariableComputationsInExits(
                naturalLoop,
                loopInvariantBlock->getBlock()->getEntry()->getNode(),
                newSymbolReference, symRefInCompare, derivedInductionVar);
    ...
    if (alteredCode) {
      if (loopInvariantBlock->getBlock()
              ->getLastRealTreeTop()
              ->getNode()
              ->getOpCodeValue() != TR::BBStart) {
        ...
        // == TR::Goto), "Expect the loop pre-header to end in a goto for call to
        // TR::Block::split to work correctly\n");
        TR::Block *newLoopInvariantBlock =
            loopInvariantBlock->getBlock()->split(
                loopInvariantBlock->getBlock()->getLastRealTreeTop(),
                comp()->getFlowGraph(), false);
        if (treeToMoveAfterSplit) {
          treeToMoveAfterSplit->getPrevTreeTop()->join(
              treeToMoveAfterSplit->getNextTreeTop());
...
bool TR_LoopCanonicalizer::replaceInductionVariableComputationsInExits(
    TR_Structure *structure, TR::Node *node,
    TR::SymbolReference *newSymbolReference,
    TR::SymbolReference *primaryInductionVar,
    TR::SymbolReference *derivedInductionVar) {
  bool seenInductionVariableComputation = false;
  for (auto edge = _entryBlock->getSuccessors().begin();
       edge != _entryBlock->getSuccessors().end();) {
    auto next = edge;
    ++next;
    TR::Block *block = (*edge)->getTo()->asBlock();
    if (!structure->contains(block->getStructureOf())) {
      TR::Block *newBlock = (*edge)->getFrom()->asBlock()->splitEdge(
          (*edge)->getFrom()->asBlock(), block, comp());
      TR::DataType dataType = newSymbolReference->getSymbol()->getDataType();
      TR::Node *subNode = TR::Node::create(
          (dataType == TR::Int32) ? TR::iadd : TR::ladd, 2,
          TR::Node::createWithSymRef(node,
                                     comp()->il.opCodeForDirectLoad(dataType),
                                     0, newSymbolReference),
          TR::Node::createWithSymRef(node,
                                     comp()->il.opCodeForDirectLoad(dataType),
                                     0, primaryInductionVar));
      if (_primaryIncrementedFirst != 0) {
        ...
        subNode =
            TR::Node::create(((dataType == TR::Int32) ? TR::iadd : TR::ladd), 2,
                             subNode, adjustmentConstChild);
      ...
      TR::Node *storeNode =
          TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType),
                                     1, 1, subNode, derivedInductionVar);
      TR::TreeTop *newStoreTree =
          TR::TreeTop::create(comp(), storeNode, NULL, NULL);
  ...
  for (auto edge = _loopTestBlock->getSuccessors().begin();
       edge != _loopTestBlock->getSuccessors().end();) {
    auto next = edge;
    ++next;
    TR::Block *block = (*edge)->getTo()->asBlock();
    if (!structure->contains(block->getStructureOf())) {
...
void TR_LoopCanonicalizer::findIncrements(
    TR::Node *currentNode, vcount_t visitCount,
    TR::SymbolReference *derivedInductionVar,
    TR::SymbolReference *primaryInductionVar,
    int64_t &derivedInductionVarIncrement,
  ...
  if (currentNode->getOpCode().isStore()) {
    ...
    if (currentNode->getSymbolReference() == derivedInductionVar) {
      ...
      if ((derivedInductionVarIncrement == 0) && child->getOpCode().isAdd()) {
        ...
        if (secondChild->getOpCode().isLoadConst()) {
          ...
          if (firstChild->getOpCode().isLoadVar() &&
            ...
            derivedInductionVarIncrement =
                derivedInductionVarIncrement + curIncr;
          }
        }
      ...
      if (!incrInStandardForm) unknownIncrement = true;
      ...
      if ((primaryInductionVarIncrement == 0) && child->getOpCode().isAdd()) {
        ...
        if (secondChild->getOpCode().isLoadConst()) {
          ...
          if (firstChild->getOpCode().isLoadVar() &&
            ...
            incrInStandardForm = true;
            primaryInductionVarIncrement =
                primaryInductionVarIncrement + curIncr;
          }
        }
  ...
  for (i = 0; i < currentNode->getNumChildren(); i++) {
    ...
    if (child->getOpCode().isStore())
      findIncrements(child, visitCount, derivedInductionVar,
                     primaryInductionVar, derivedInductionVarIncrement,
                     primaryInductionVarIncrement, unknownIncrement);
  }
}

bool TR_LoopCanonicalizer::incrementedInLockStep(
    TR_Structure *structure, TR::SymbolReference *derivedInductionVar,
    TR::SymbolReference *primaryInductionVar,
    int64_t derivedInductionVarLoopIncrement,
    int64_t primaryInductionVarLoopIncrement,
    TR_ScratchList<TR::Block> *derivedInductionVarIncrementBlocks,
    TR_ScratchList<TR::Block> *primaryInductionVarIncrementBlocks) {
  if (structure->asBlock() != NULL) {
    ...
    while (currentTree != exitTree) {
      ...
      findIncrements(currentNode, comp()->getVisitCount(), derivedInductionVar,
                     primaryInductionVar, derivedInductionVarIncrement,
                     primaryInductionVarIncrement, unknownIncrement);
      if (unknownIncrement) return false;
      currentTree = currentTree->getNextTreeTop();
    }

    if (primaryInductionVarIncrement == derivedInductionVarIncrement)
      return true;
    else if ((primaryInductionVarIncrement ==
              primaryInductionVarLoopIncrement) &&
             (derivedInductionVarIncrement == 0)) {
      primaryInductionVarIncrementBlocks->add(block);
      return true;
    } else if ((derivedInductionVarIncrement ==
                derivedInductionVarLoopIncrement) &&
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (!incrementedInLockStep(node->getStructure(), derivedInductionVar,
                                 primaryInductionVar,
                                 derivedInductionVarLoopIncrement,
                                 primaryInductionVarLoopIncrement,
                                 derivedInductionVarIncrementBlocks,
...
bool TR_LoopCanonicalizer::checkIfOrderOfBlocksIsKnown(
    TR_RegionStructure *naturalLoop, TR::Block *entryBlock,
    TR::Block *loopTestBlock,
    TR_ScratchList<TR::Block> *derivedInductionVarIncrementBlocks,
    TR_ScratchList<TR::Block> *primaryInductionVarIncrementBlocks,
    uint8_t &primaryFirst) {
  if (!derivedInductionVarIncrementBlocks->isSingleton()) return false;

  if (!primaryInductionVarIncrementBlocks->isSingleton()) return false;

  TR::Block *derivedBlock =
      derivedInductionVarIncrementBlocks->getListHead()->getData();
  TR::Block *primaryBlock =
      primaryInductionVarIncrementBlocks->getListHead()->getData();

  for (auto edge = entryBlock->getExceptionSuccessors().begin();
       edge != entryBlock->getExceptionSuccessors().end(); ++edge) {
    TR::Block *catchBlock = (*edge)->getTo()->asBlock();
    if (naturalLoop->contains(catchBlock->getStructureOf())) return false;
  }
  ...
  if (entryBlock == primaryBlock) {
    ...
    if (loopTestBlock == derivedBlock) {
      ...
      TR::Block *onlyPredOfLoopTestBlock =
          loopTestBlock->getPredecessors().front()->getFrom()->asBlock();
...
/**
 *  Checking to see if the induction variable to be removed participates in any
 * compress address expressions, such as [base + stride*index] on x86 hardware.
 *  If the address expression contains a mul operand the address expression
 * formed by the codegen will not have any space for a folded in variable
 * substitution.
 *
 *  @return true, when the induction variable is used only in simple addressing
 * modes false, when the induction variable is used in complex addressing modes
 */
bool TR_LoopCanonicalizer::checkComplexInductionVariableUse(
  ...
  if (structure->asBlock() != NULL) {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (!checkComplexInductionVariableUse(node->getStructure())) return false;
    }
  }
...
bool TR_LoopCanonicalizer::checkComplexInductionVariableUseNode(
    TR::Node *node, bool addrExpression) {
  ...
  if (opCode.isStoreIndirect()) {
    ...
    if (node->getOpCodeValue() == TR::imul) {
      ...
      if (node->getFirstChild()->getOpCode().hasSymbolReference() &&
        ...
        traceMsg(comp(),
                 "\tAvoiding induction variable replacement because of address "
                 "mode complexity. Sym Ref. = %p\n",
      ...
      if (node->getFirstChild()->getOpCodeValue() == TR::i2l &&
          node->getFirstChild()
              ...
              ->getOpCode()
              .hasSymbolReference() &&
          node->getFirstChild()->getFirstChild()->getSymbolReference() ==
              _symRefBeingReplaced) {
        traceMsg(comp(),
                 "\tAvoiding induction variable replacement because of address "
                 "mode complexity. Sym Ref. = %p\n",
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    if (!checkComplexInductionVariableUseNode(node->getChild(i),
                                              addrExpression))
      return false;
  }
...
bool TR_LoopCanonicalizer::examineTreeForInductionVariableUse(
    ...
    if ((node->getSymbolReference() == _symRefBeingReplaced) &&
        ...
        if (!_derivedInductionVarStoreInBlock) {
          if (!_primaryInductionVarStoreSomewhereInBlock) {
            ...
            while (curTree != exitTree) {
              ...
              if (curNode->getOpCode().isStore() &&
                  (curNode->getSymbolReference() ==
                   _primaryInductionVariable)) {
          ...
          if (_primaryInductionVarStoreSomewhereInBlock)
            loadOfPrimaryInductionVar =
                _primaryInductionVarStoreSomewhereInBlock->getFirstChild()
                    ->getFirstChild();
        }
      }
      ...
      if (!loadOfPrimaryInductionVar)
        loadOfPrimaryInductionVar = TR::Node::createWithSymRef(
            node, comp()->il.opCodeForDirectLoad(dataType), 0,
            _primaryInductionVariable);
      ...
      loadOfPrimaryInductionVar =
          TR::Node::create(((dataType == TR::Int32) ? TR::iadd : TR::ladd), 2,
                           ...
                           TR::Node::createWithSymRef(
                               node, comp()->il.opCodeForDirectLoad(dataType),
                               0, *newSymbolReference));
      ...
      if (_primaryIncrementedFirst == 0) {
        if (_derivedInductionVarStoreInBlock) {
          if (!_primaryInductionVarStoreInBlock) {
            TR::Node *constChild =
                _derivedInductionVarStoreInBlock->getFirstChild()
                    ->getSecondChild();
            ...
            TR::Node *adjustmentConstChild = TR::Node::create(
                constChild, constChild->getOpCodeValue(), 0, 0);
            if (adjustmentConstChild->getDataType() == TR::Int32)
              adjustmentConstChild->setInt((int32_t)additiveConstant);
            else
              adjustmentConstChild->setLongInt(additiveConstant);
        ...
        } else if (0)  // Do not need adjustment in this case since we have
                       // obtained the correctly commoned node in this case
        {
          if (_primaryInductionVarStoreInBlock) {
            TR::Node *constChild =
                _primaryInductionVarStoreInBlock->getFirstChild()
                    ->getSecondChild();
            int64_t additiveConstant;
            if (constChild->getOpCodeValue() == TR::lconst)
              additiveConstant = constChild->getLongInt();
        ...
        if (_primaryIncrementedFirst == 1) {
          if (((_primaryInductionIncrementBlock != _currentBlock) ||
            ...
            TR::Node *adjustmentConstChild = 0;
            if (dataType == TR::Int32)
              adjustmentConstChild = TR::Node::create(
                  node, TR::iconst, 0, (int32_t)-1 * additiveConstant);
            else {
              adjustmentConstChild = TR::Node::create(node, TR::lconst, 0, 0);
              adjustmentConstChild->setLongInt(-1 * additiveConstant);
            }
            ...
            nodeSetUp = true;
            TR::Node::recreate(node,
                               ((dataType == TR::Int32) ? TR::iadd : TR::ladd));
            node->setNumChildren(2);
            node->setAndIncChild(0, loadOfPrimaryInductionVar);
            node->setAndIncChild(1, adjustmentConstChild);
          }
          ...
          if (((_primaryInductionIncrementBlock != _currentBlock) ||
            ...
            if (dataType == TR::Int32)
              adjustmentConstChild = TR::Node::create(
                  node, TR::iconst, 0, (int32_t)additiveConstant);
            else {
              adjustmentConstChild = TR::Node::create(node, TR::lconst, 0, 0);
              adjustmentConstChild->setLongInt(additiveConstant);
            }
      ...
      // TR::Node *addNode = TR::Node::create((dataType == TR_SInt32) ? TR::iadd
      // : TR::ladd, 2,
      //                                   TR::Node::create(node,
      //                                   comp()->il.opCodeForDirectLoad(dataType),
...
void TR_LoopCanonicalizer::placeInitializationTreeInLoopPreHeader(
    TR::Block *b, TR::Node *node, TR::SymbolReference *newSymbolReference,
    TR::SymbolReference *primaryInductionVar,
    TR::SymbolReference *derivedInductionVar) {
  TR::DataType dataType = newSymbolReference->getSymbol()->getDataType();
  TR::Node *subNode = TR::Node::create(
      (dataType == TR::Int32) ? TR::isub : TR::lsub, 2,
      TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(dataType),
                                 0, derivedInductionVar),
      TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(dataType),
                                 0, primaryInductionVar));
  TR::Node *storeNode =
      TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType), 1,
                                 1, subNode, newSymbolReference);
  TR::TreeTop *newStoreTree =
      TR::TreeTop::create(comp(), storeNode, NULL, NULL);
 ...
 */
void TR_LoopCanonicalizer::rewritePostToPreIncrementTestInRegion(
    TR_RegionStructure *region) {
  TR_RegionStructure::Cursor it(*region);
  for (TR_StructureSubGraphNode *sub = it.getCurrent(); sub != NULL;
       sub = it.getNext()) {
...
void TR_LoopCanonicalizer::rewritePostToPreIncrementTestInBlock(
  ...
  if (!test->getOpCode().isIf()) return;

  if (!test->getOpCode().isCompareForOrder()) return;

  if (!test->getChild(0)->getOpCode().isInteger()) return;
  ...
  if (left->getReferenceCount() == 1) return;

  TR::Node *const updated = store->getChild(0);
  if (!updated->getOpCode().isAdd() && !updated->getOpCode().isSub()) return;

  if (!updated->cannotOverflow()) return;

  TR::Node *const updateBase = updated->getChild(0);
  if (!isLoadVarDirectOf(updateBase, symrefNum)) return;

  TR::Node *const addendNode = updated->getChild(1);
  if (!addendNode->getOpCode().isLoadConst()) return;
  ...
  const int ifOpIdx = int(ifOp.isCompareTrueIfGreater()) << 1 |
  ...
  if (addend != usableAddends[ifOpIdx]) return;
  ...
  if (left != updateBase) {
    ...
    for (TR::PostorderNodeIterator it(start, comp()); true; ++it) {
      ...
      if (eval == left || eval == updateBase) {
        if (trace()) traceMsg(comp(), " n%un", eval->getGlobalIndex());
  ...
  if (!performTransformation(
          ...
          optDetailString(), test->getGlobalIndex(), ifOp.getName(), symrefNum,
          right->getGlobalIndex(), adjustedIfOp.getName(),
          updated->getGlobalIndex(), right->getGlobalIndex()))
    return;
...
const char *TR_LoopCanonicalizer::optDetailString() const throw() {
  return "O^O LOOP CANONICALIZER: ";
}

bool TR_LoopTransformer::replaceAllInductionVariableComputations(
  ...
  if (structure->asBlock() != NULL) {
    ...
    while (currentTree != exitTree) {
      ...
      if (examineTreeForInductionVariableUse(
              loopInvariantBlock, NULL, -1, currentNode,
              comp()->getVisitCount(), newSymbolReference))
        seenInductionVariableComputation = true;
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (replaceAllInductionVariableComputations(
              loopInvariantBlock, node->getStructure(), newSymbolReference,
              inductionVarSymRef))
        seenInductionVariableComputation = true;
    }
  }
...
//
TR::Node *TR_LoopTransformer::duplicateExact(TR::Node *node,
  ...
  for (int i = 0; i < node->getNumChildren(); i++)
    newRoot->setChild(
        i, duplicateExact(node->getChild(i), seenNodes, duplicateNodes));
...
void TR_LoopTransformer::printTrees() {
  ...
  if (trace()) getDebug()->print(comp()->getOutFile(), comp()->getFlowGraph());
}

/**
 * MUST do this step if there are any blocks to be cleansed as otherwise the
 * block would violate the CFG constraint that it can contain only one branch
 * instruction. The goto inserted at the end earlier in canonicalization is
 * removed and the target block is made the fall through block.
 */
bool TR_LoopTransformer::cleanseTrees(TR::Block *block) {
  ...
  //        treeTop = exitTreeTop->getNextTreeTop())
  {
    ...
    if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
      ...
      {
        TR_ASSERT(targetTree->getPrevTreeTop()->getNode()->getOpCodeValue() ==
                      TR::BBEnd,
                  "BBStart is not preceded by a BBEnd");
        prevTreeOfTarget = targetTree->getPrevTreeTop()
                               ->getNode()
                               ->getBlock()
                               ->getLastRealTreeTop();
      }

      bool isPrevTreeOfTargetReturn =
          prevTreeOfTarget->getNode()->getOpCode().isReturn();
      if ((prevTreeOfTarget->getNode()->getOpCode().isBranch() &&
           ((prevTreeOfTarget->getNode()->getOpCodeValue() == TR::Goto) ||
            (prevTreeOfTarget->getNode()->getBranchDestination() ==
             targetTree))) ||
          isPrevTreeOfTargetReturn ||
          prevTreeOfTarget->getNode()->getOpCode().isSwitch() ||
          prevTreeOfTarget->getNode()
              ->getOpCode()
              .isJumpWithMultipleTargets() ||
          (prevTreeOfTarget->getNode()->getOpCodeValue() == TR::athrow) ||
          (prevTreeOfTarget->getNode()->getOpCode().isTreeTop() &&
           (prevTreeOfTarget->getNode()->getFirstChild()->getOpCodeValue() ==
            TR::athrow))) {
        // if (isPrevTreeOfTargetReturn ||
        // ((prevTreeOfTarget->getNode()->getOpCode().getOpCodeValue() ==
        // TR::Goto) || (prevTreeOfTarget->getNode()->getBranchDestination() ==
        // targetTree)))
        {
          ...
          if (exitTreeTop->getNextTreeTop() != targetTree) {
            ...
            while (placeToMoveTrees && placeToMoveTrees->hasSuccessor(
                                           placeToMoveTrees->getNextBlock())) {
            ...
            if (!placeToMoveTrees) placeToMoveTrees = prevBlock;
 ...
 */
bool TR_LoopTransformer::makeInvariantBlockFallThroughIfPossible(
    TR::Block *block) {
  if (_cfg == NULL) return false;
      ...
      if ((prevTreeOfTarget->getNode()->getOpCode().isBranch() &&
        ...
        // (prevTreeOfTarget->getNode()->getBranchDestination() != targetTree))
        {
          ...
          if (treeTop->getPrevTreeTop()) {
            prevTreeOfBlock = treeTop->getPrevTreeTop()
                                  ->getNode()
                                  ->getBlock()
                                  ->getLastRealTreeTop();
          ...
          //
          if ((exitTreeTop->getNextTreeTop() != targetTree) &&
              allPredsBranchExplicitly) {
            if (trace())
              traceMsg(
                  comp(),
                  "Moving invariant block_%d to fall through into loop %d\n",
                  block->getNumber(),
                  targetTree->getNode()->getBlock()->getNumber());
...
bool TR_LoopTransformer::isStoreInRequiredForm(int32_t symRefNum,
...
int32_t TR_LoopTransformer::checkLoopForPredictability(
    TR_Structure *loopStructure, TR::Block *loopInvariantBlock,
  ...
  TR::CFGNode *next;
  for (next = comp()->getFlowGraph()->getFirstNode(); next;
       next = next->getNext()) {
    int32_t i = toBlock(next)->getNumber();

    if (i == doWhileNumber) break;
  }

  int32_t result = 1;
  for (auto nextEdge = next->getPredecessors().begin();
       nextEdge != next->getPredecessors().end(); ++nextEdge) {
    TR::Block *nextBlock = toBlock((*nextEdge)->getFrom());
...
void TR_LoopTransformer::updateInfo(TR::Node *node, vcount_t visitCount,
  ...
  if (refNo != 0) mayKillAliases[refNo] = false;
  ...
  *_neverWritten -= defAliases;
  ...
  //   comp()->getDebug()->print(comp()->getOutFile(),
  //   &(uinfo.currentlyWrittenOnce)); traceMsg(comp(), "\n");
  // For most store operations
  if (refNo && node->getOpCode().isLikeDef() &&
      defAliases.PopulationCount() > 1) {
  ...
  } else if (node->getOpCode().isStore())  // For all other store operations
  {
    ...
    defAliases.Andc(uinfo.currentlyWrittenOnce,
                    tmp);  // tmp = defAliases - uinfo.currentlyWrittenOnce
    _writtenExactlyOnce.Or(tmp);  // _writtenExactlyOnce += tmp
  } else {
    TR::BitVector tmp(comp()->allocator());
    defAliases.And(uinfo.currentlyWrittenOnce,
                   tmp);  // tmp = defAliases * uinfo.currentlyWrittenOnce
  ...
  *_neverRead -= useAliases;
  ...
  if (node->getOpCode().isStore()) {
    updateStoreInfo(node->getSymbolReference()->getReferenceNumber(),
                    _currTree);
  }
...
void TR_LoopTransformer::collectSymbolsWrittenAndReadExactlyOnce(
  ...
  if (structure->asBlock() != NULL) {
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      collectSymbolsWrittenAndReadExactlyOnce(node->getStructure(), visitCount,
                                              uinfo);
    }
  }
}

TR_LoopTransformer::TR_TransformerDefUseState
TR_LoopTransformer::getSymbolDefUseStateInSubTree(
  ...
  if (opCode.isLoadVar() && opCode.hasSymbolReference()) {
    TR::RegisterMappedSymbol *sym =
        node->getSymbol()->getRegisterMappedSymbol();
    if (sym == indVarSym) {
...
TR_LoopTransformer::TR_TransformerDefUseState
TR_LoopTransformer::getSymbolDefUseStateInBlock(
    TR::Block *block, TR::RegisterMappedSymbol *indVarSym) {
  for (TR::TreeTop *tree = block->getFirstRealTreeTop();
       tree != block->getExit(); tree = tree->getNextTreeTop()) {
    if (tree->getNode()->getOpCodeValue() == TR::asynccheck) continue;
...
bool TR_LoopTransformer::isSymbolReferenceWrittenNumberOfTimesInStructure(
    TR_Structure *structure, int32_t symRefNum, int32_t *numTimesSeen,
    int32_t maxTimes) {
  if (structure->asBlock() != NULL) {
    ...
    while (currentTree != exitTree) {
...

      {
        ...
        if (sameSym) {
          (*numTimesSeen)++;
          if ((*numTimesSeen) > maxTimes) return false;
        }
        ...
        if (!defNode->isTheVirtualCallNodeForAGuardedInlinedCall() ||
          ...
          for (defAliasesCursor.SetToFirstOne(); defAliasesCursor.Valid();
               defAliasesCursor.SetToNextOne()) {
...
TR::Node *TR_LoopTransformer::getCorrectNumberOfIterations(
    TR::Node *node, TR::Node *constNode) {
  TR::Node *subNode;
  switch (node->getOpCodeValue()) {
    case TR::ificmplt: {
      if (!_isAddition) return 0;

      subNode =
          TR::Node::create(TR::isub, 2, node->getSecondChild()->duplicateTree(),
    ...
    case TR::ificmple: {
      ...
      // subNode = TR::Node::create(TR::isub, 2,
      // node->getSecondChild()->duplicateTree(),
      // node->getFirstChild()->duplicateTree()); subNode =
      // TR::Node::create(TR::iadd, 2, subNode, TR::Node::create(node,
      // TR::iconst, 0, 1)); break;
    }
    case TR::ificmpgt: {
      if (_isAddition) return 0;
      ...
      subNode =
          TR::Node::create(TR::isub, 2, node->getFirstChild()->duplicateTree(),
                           node->getSecondChild()->duplicateTree());
      break;
    }
    case TR::ificmpge: {
      ...
      // subNode = TR::Node::create(TR::isub, 2,
      // node->getFirstChild()->duplicateTree(),
      // node->getSecondChild()->duplicateTree()); subNode =
      // TR::Node::create(TR::iadd, 2, subNode, TR::Node::create(node,
      // TR::iconst, 0, 1)); break;
    }
    default:
...
TR::Node *TR_LoopTransformer::updateLoadUsedInLoopIncrement(
  ...
  if (_indirectInductionVariable && opCode.isLoadVar() &&
    ...
    if (useDefInfo->getUseDef(defs, useIndex) &&
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!defNode->getOpCode().isStore()) continue;
...
void TR_LoopTransformer::checkIfIncrementInDifferentExtendedBlock(
...
bool TR_LoopTransformer::blockIsAlwaysExecutedInLoop(
    TR::Block *currentBlock, TR_RegionStructure *loopStructure, bool *atEntry) {
  TR::Block *entryBlock = loopStructure->asRegion()->getEntryBlock();
  if ((currentBlock == _loopTestBlock) || (currentBlock == entryBlock)) {
    if (atEntry) {
  ...
  while (flag) {
    ...
    if (flag) {
      ...
      if (seenBlocks.find(cursorBlock)) return false;
    }
  }
...
const char *TR_RedundantInductionVarElimination::optDetailString() const
    throw() {
...
bool TR_LoopTransformer::detectEmptyLoop(TR_Structure *structure,
                                         int32_t *numTrees) {
  if (structure->asBlock()) {
    TR_BlockStructure *blockStructure = structure->asBlock();
    TR::Block *block = blockStructure->getBlock();

    if (*numTrees > 1) return false;
    ...
    while (currentTree != exitTree) {
      ...
      if (!opCode.isExceptionRangeFence() && !opCode.isBranch() &&
          (opCode.getOpCodeValue() != TR::asynccheck)) {
        (*numTrees)++;
        if (*numTrees > 1) return false;
      }
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (!detectEmptyLoop(node->getStructure(), numTrees)) return false;
    }
  }
...
int32_t TR_LoopInverter::detectCanonicalizedPredictableLoops(
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
       nextBlock = blocksIt.getNext())
    if (!nextBlock->getExceptionPredecessors().empty()) return 0;
  ...
  if (!loopInvariantBlock) return 0;

  TR_ScratchList<TR::Block> exitBlocks(trMemory());
  loopStructure->collectExitBlocks(&exitBlocks);

  if (!exitBlocks.isSingleton()) return 0;
  ...
  if (isPredictableLoop <= 0) return 0;
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (loopTestNode->getFirstChild()->getOpCode().hasSymbolReference()) {
      if (nextInductionVariableNumber == loopTestNode->getFirstChild()
                                             ->getSymbolReference()
                                             ->getReferenceNumber()) {
    ...
    if (!isSameInductionVariable ||
        ...
        !loopStructure->asRegion()->isExprInvariant(
    ...
    if (isInvertibleLoop(nextInductionVariableNumber, loopStructure)) {
      TR::SymbolReference *inductionSymRef =
          symRefTab->getSymRef(nextInductionVariableNumber);
      TR_InductionVariable *v =
          loopStructure->asRegion()->findMatchingIV(inductionSymRef);
      if (v) {
        ...
        if (entryVal && entryVal->asIntConst() &&
            (entryVal->getLowInt() == 0 ||
             (exitVal && exitVal->asIntConst() &&
              (llabs((long long)(exitVal->getLowInt()) -
                     (long long)(entryVal->getLowInt()))) < INT_MAX)) &&
            // We cannot safely guess exit value of IV if it doesn't land on
            // comparand(RhsOfStoreNode). Workaround is only allow abs(incr)==1.
            // This is also a hard requirement for unsigned IV to not introduce
            // wraparound. For signed case, we may allow abs(incr)>1, but exit
            // value calc is not just RhsOfStoreNode if it doesn't land on it.
            // todo
            incrVal->asIntConst() && abs(incrVal->getLowInt()) == 1) {
          entryValue = entryVal->getLowInt();

          if (exitVal && exitVal->asIntConst()) isInvertible = true;

          if ((_loopTestTree->getNode()
                   ->getSecondChild()
                   ->getOpCode()
                   .isLoadVarDirect() ||
    ...
    if (isInvertible &&
      ...
      if (_loopTestTree->getNode()
              ...
              .isLoadConst()) {
        int testValue =
            _loopTestTree->getNode()->getSecondChild()->get32bitIntegralValue();

        if (incrVal->asIntConst() && incrVal->getLowInt() == 1 &&
            testValue < INT_MAX &&
        ...
        storeNode = TR::Node::createWithSymRef(
            TR::istore, 1, 1,
            TR::Node::create(loopTestNode, TR::iconst, 0, initValue),
            nextInductionSymRef);
      } else {
        TR::Node *subNode = TR::Node::create(TR::isub, 2);
        subNode->setAndIncChild(
            0, _loopTestTree->getNode()->getSecondChild()->duplicateTree());
        subNode->setAndIncChild(
            1, TR::Node::create(loopTestNode, TR::iconst, 0, entryValue));
        storeNode = TR::Node::createWithSymRef(TR::istore, 1, 1, subNode,
                                               nextInductionSymRef);
      }
      ...
      dumpOptDetails(
          comp(), "Init node: %p, update node: %p, loopTest node: %p\n",
      ...
      */
      ...
      loopTestNode->setAndIncChild(
          1, TR::Node::create(loopTestNode, TR::iconst, 0, 0));
      ...
      for (nextBlock = blocksIt.getCurrent(); nextBlock;
        ...
        for (auto edge = nextBlock->getSuccessors().begin();
          ...
          if (!blocksInRegion.find(next)) {
            TR::Block *splitBlock =
                nextBlock->splitEdge(nextBlock, next, comp());

            if (loopTestNode->getOpCodeValue() == TR::ificmpgt)
              storeNode = TR::Node::createWithSymRef(
                  TR::istore, 1, 1, rhsOfStoreNode->duplicateTree(),
                  nextInductionSymRef);
            else {
              if (origOpCodeWasAdd)
                storeNode = TR::Node::createWithSymRef(
                    ...
                    TR::Node::create(TR::iadd, 2,
                                     rhsOfStoreNode->duplicateTree(),
                                     origStoreNode->getFirstChild()
                                         ->getSecondChild()
                                         ->duplicateTree()),
                    nextInductionSymRef);
              else
...
bool TR_LoopInverter::isInvertibleLoop(int32_t symRefNum,
                                       TR_Structure *structure) {
  if (structure->asBlock() != NULL) {
    if (comp()->getSymRefTab()->getSymRef(symRefNum)->getSymbol()->isVolatile())
      return false;
    ...
    while (currentTree != exitTree) {
      ...
      if (!checkIfSymbolIsReadInKnownTree(currentNode, symRefNum, currentTree,
                                          checklist) ||
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (!isInvertibleLoop(symRefNum, node->getStructure())) return false;
    }
  }
...
bool TR_LoopInverter::checkIfSymbolIsReadInKnownTree(
    TR::Node *node, int32_t symRefNum, TR::TreeTop *currentTree,
    TR::NodeChecklist &checklist) {
  // Do this check before the checklist.contains() check so that we don't waste
  // time traversing these two trees. NOTE that we don't want to mark the nodes
  // in these trees as visited (i.e., add them to the check list) except if the
  // tree top we start from is other than these two types of trees in this if
  // statement.
  if (currentTree == _loopTestTree || currentTree == _storeTrees[symRefNum])
    return true;

  if (checklist.contains(node)) return true;

  checklist.add(node);

  TR_UseDefAliasSetInterface defAliases =
      comp()->getSymRefTab()->getSymRef(symRefNum)->getUseDefAliases();
  ...
  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++) {
    if (!checkIfSymbolIsReadInKnownTree(node->getChild(i), symRefNum,
                                        currentTree, checklist))
      return false;
  }
...
const char *TR_LoopInverter::optDetailString() const throw() {
