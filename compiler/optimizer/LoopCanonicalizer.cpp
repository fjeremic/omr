...
void TR_LoopTransformer::detectWhileLoops(ListAppender<TR_Structure>& whileLoopsInnerFirst, List<TR_Structure>& whileLoops, ListAppender<TR_Structure>& doWhileLoopsInnerFirst, List<TR_Structure>& doWhileLoops, TR_Structure* root, bool innerFirst)
{
   ...
   */
    ...
    if (asLoopCanonicalizer()) {
        ...
        /////   return;
        /////
        if ((loopHeader && loopHeader->getBlock()->getNextBlock() && loopHeader->getBlock()->getNextBlock()->isExtensionOfPreviousBlock()) || (loopHeader && loopHeader->getBlock()->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets()) || (loopHeader && loopHeader->getBlock()->getLastRealTreeTop()->getNode()->getOpCode().isCompBranchOnly())) {
            isDoWhileLoop = true;
        }
            ...
            if ((hdrBlock->hasExceptionPredecessors() == false) && (hdrBlock->hasExceptionSuccessors() == false)) {
                ...
                if (entryNode->getSuccessors().size() == 2) {
                    ...
                    if (succ1Internal ^ succ2Internal) {
                        ...
                        while ((firstTree && (firstTree->getNode()->getOpCodeValue() == TR::asynccheck || (skipTt && firstTree->getNode()->getOpCodeValue() == TR::treetop) || (skipCRAnchor && firstTree->getNode()->getOpCodeValue() == TR::compressedRefs) || firstTree->getNode()->getOpCode().isCheck())))
                            firstTree = firstTree->getNextTreeTop();

                        if (firstTree == hdrBlock->getLastRealTreeTop() || firstTree->getNextTreeTop() == hdrBlock->getLastRealTreeTop()) {
                                        ...
                                        if (exitBlock != hdrBlock) {
                                            if (exitBlock->getSuccessors().size() == 2) {
                                                ...
                                                if (successor1Internal ^ successor2Internal) {
                                                    ...
                                                    if (exitBlockLoopTestTree->getNode()->getOpCode().isCompBranchOnly() || exitBlockLoopTestTree->getNode()->getOpCode().isJumpWithMultipleTargets())
                                                        continue;

                                                    foundInductionVariable = findMatchingIVInRegion(exitBlockLoopTestTree, region);
                                                }
                                            }
                                        }
        ...
        if ((loopHeader && loopHeader->getBlock()->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets()) || (loopHeader && loopHeader->getBlock()->getLastRealTreeTop()->getNode()->getOpCode().isCompBranchOnly()))
            return;
...
bool TR_LoopTransformer::findMatchingIVInRegion(TR::TreeTop* loopTestTree, TR_RegionStructure* region)
{
    ...
    if (firstChild->getOpCode().hasSymbolReference())
        ...
        if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && (firstChild->getReferenceCount() > 1) && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
    ...
    for (auto edge = loopHeader->getPredecessors().begin(); edge != loopHeader->getPredecessors().end(); ++edge) {
        ...
        if (!whileLoop->contains(pred->getStructureOf(), whileLoop->getParent())) {
            ...
            if (!(pred == _cfg->getStart())) {
                ...
                } else if (!lastNonFenceTree->getNode()->getOpCode().isJumpWithMultipleTargets() && !lastNonFenceTree->getNode()->getOpCode().isCompBranchOnly()) {
                    if (adjPred) {
                ...
                if (isCheckOnly) {
                    ...
                    if (!lastTree->adjustBranchOrSwitchTreeTop(comp(), blockHeadTreeTop, targetBlock->getEntry())) {
                        if (addToEnd) {
                            ...
                            if ((targetBlock->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto) && (targetBlock->getLastRealTreeTop()->getNode()->getBranchDestination() == blockHeadTreeTop))
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(TR_RegionStructure* doWhileLoop)
{
    ...
    if (!comp()->getOption(TR_DisableEmptyPreHeaderCheck)) //Skip adding another pre-header if you don't need to.
    {
        for (auto iedge = blockAtHeadOfLoop->getPredecessors().begin(); iedge != blockAtHeadOfLoop->getPredecessors().end(); ++iedge) {
            ...
            if (ipred->getStructureOf()->isLoopInvariantBlock() && ipred->isEmptyBlock() && !(ipred == ipred->getStructureOf()->getParent()->asRegion()->getEntryBlock())) {
    ...
    if (primaryInductionVariable && bivIterator.getFirst()) {
        ...
        for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
            ...
            if ((nextIndVar->getSymRef()->getSymbol()->getDataType() == primaryInductionVariable->getSymRef()->getSymbol()->getDataType()) &&
                ...
                if (incrementsInLockStep && (!derivedInductionVarIncrementBlocks.isEmpty() || !primaryInductionVarIncrementBlocks.isEmpty())) {
        ...
        if (currentNode->getSymbolReference() == derivedInductionVar) {
            ...
            if ((derivedInductionVarIncrement == 0) && child->getOpCode().isAdd()) {
                ...
                if (secondChild->getOpCode().isLoadConst()) {
                    ...
                    if (firstChild->getOpCode().isLoadVar() && (firstChild->getSymbolReference() == derivedInductionVar)) {
            ...
            if ((primaryInductionVarIncrement == 0) && child->getOpCode().isAdd()) {
                ...
                if (secondChild->getOpCode().isLoadConst()) {
                    ...
                    if (firstChild->getOpCode().isLoadVar() && (firstChild->getSymbolReference() == primaryInductionVar)) {
                        incrInStandardForm = true;
...
bool TR_LoopCanonicalizer::incrementedInLockStep(TR_Structure* structure, TR::SymbolReference* derivedInductionVar, TR::SymbolReference* primaryInductionVar, int64_t derivedInductionVarLoopIncrement, int64_t primaryInductionVarLoopIncrement, TR_ScratchList<TR::Block>* derivedInductionVarIncrementBlocks, TR_ScratchList<TR::Block>* primaryInductionVarIncrementBlocks)
{
    if (structure->asBlock() != NULL) {
        ...
        if (primaryInductionVarIncrement == derivedInductionVarIncrement)
            return true;
        else if ((primaryInductionVarIncrement == primaryInductionVarLoopIncrement) && (derivedInductionVarIncrement == 0)) {
            primaryInductionVarIncrementBlocks->add(block);
            return true;
        } else if ((derivedInductionVarIncrement == derivedInductionVarLoopIncrement) && (primaryInductionVarIncrement == 0)) {
...
bool TR_LoopCanonicalizer::checkComplexInductionVariableUseNode(TR::Node* node, bool addrExpression)
{
    ...
    if (opCode.isStoreIndirect()) {
        ...
        if (node->getOpCodeValue() == TR::imul) {
            ...
            if (node->getFirstChild()->getOpCode().hasSymbolReference() && node->getFirstChild()->getSymbolReference() == _symRefBeingReplaced) {
            ...
            if (node->getFirstChild()->getOpCodeValue() == TR::i2l && node->getFirstChild()->getFirstChild()->getOpCode().hasSymbolReference() && node->getFirstChild()->getFirstChild()->getSymbolReference() == _symRefBeingReplaced) {
                traceMsg(comp(), "\tAvoiding induction variable replacement because of address mode complexity. Sym Ref. = %p\n", _symRefBeingReplaced);
                return false;
            }
        }
...
bool TR_LoopCanonicalizer::examineTreeForInductionVariableUse(TR::Block* loopInvariantBlock, TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount, TR::SymbolReference** newSymbolReference)
{
    ...
    if (node->getOpCode().hasSymbolReference() && !node->getOpCode().isStore()) {
        if ((node->getSymbolReference() == _symRefBeingReplaced) && performTransformation(comp(), "Replacing use %p of sym ref #%d by sym ref #%d\n", node, node->getSymbolReference()->getReferenceNumber(), _primaryInductionVariable->getReferenceNumber())) {
                ...
                if (!_derivedInductionVarStoreInBlock) {
                    if (!_primaryInductionVarStoreSomewhereInBlock) {
                        ...
                        while (curTree != exitTree) {
                            ...
                            if (curNode->getOpCode().isStore() && (curNode->getSymbolReference() == _primaryInductionVariable)) {
            ...
            if (_primaryIncrementedFirst == 0) {
                ...
                if (_primaryIncrementedFirst == 1) {
                    if (((_primaryInductionIncrementBlock != _currentBlock) || _primaryInductionVarStoreInBlock) && ((_derivedInductionIncrementBlock != _currentBlock) || !_derivedInductionVarStoreInBlock) && ((_derivedInductionIncrementBlock == _loopTestBlock) || (_currentBlock != _loopTestBlock))) {
                        int64_t additiveConstant = _primaryIncr;
                        TR::Node* adjustmentConstChild = 0;
                        if (dataType == TR::Int32)
                    ...
                    if (((_primaryInductionIncrementBlock != _currentBlock) || !_primaryInductionVarStoreInBlock) && ((_derivedInductionIncrementBlock != _currentBlock) || _derivedInductionVarStoreInBlock) && ((_primaryInductionIncrementBlock == _loopTestBlock) || (_currentBlock != _loopTestBlock))) {
...
bool TR_LoopTransformer::cleanseTrees(TR::Block* block)
{
    ...
    //        treeTop = exitTreeTop->getNextTreeTop())
    {
        ...
        if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
            ...
            if ((prevTreeOfTarget->getNode()->getOpCode().isBranch() && ((prevTreeOfTarget->getNode()->getOpCodeValue() == TR::Goto) || (prevTreeOfTarget->getNode()->getBranchDestination() == targetTree))) || isPrevTreeOfTargetReturn || prevTreeOfTarget->getNode()->getOpCode().isSwitch() || prevTreeOfTarget->getNode()->getOpCode().isJumpWithMultipleTargets() || (prevTreeOfTarget->getNode()->getOpCodeValue() == TR::athrow) || (prevTreeOfTarget->getNode()->getOpCode().isTreeTop() && (prevTreeOfTarget->getNode()->getFirstChild()->getOpCodeValue() == TR::athrow))) {
...
bool TR_LoopTransformer::makeInvariantBlockFallThroughIfPossible(TR::Block* block)
{
    ...
    //        treeTop = exitTreeTop->getNextTreeTop())
    {
        ...
        if (node1->getOpCode().getOpCodeValue() == TR::Goto) {
            ...
            bool isPrevTreeOfTargetReturn = prevTreeOfTarget->getNode()->getOpCode().isReturn();
            if ((prevTreeOfTarget->getNode()->getOpCode().isBranch() && ((prevTreeOfTarget->getNode()->getOpCodeValue() == TR::Goto) || (prevTreeOfTarget->getNode()->getBranchDestination() == targetTree))) || isPrevTreeOfTargetReturn || (prevTreeOfTarget->getNode()->getOpCodeValue() == TR::athrow)) {
...
{
    TR::ILOpCode& opCode = node->getOpCode();
    if (_indirectInductionVariable && opCode.isLoadVar() && _writtenExactlyOnce.ValueAt(node->getSymbolReference()->getReferenceNumber())) {
...
TR::Node* TR_LoopTransformer::containsOnlyInductionVariableAndAdditiveConstant(TR::Node* node, int32_t inductionVariable)
{
    ...
    if (opCode.isAdd()) {
        _isAddition = true;
        if (node->getFirstChild()->getOpCode().hasSymbolReference() && (node->getFirstChild()->getSymbolReference()->getReferenceNumber() == inductionVariable)) {
...
int32_t TR_LoopInverter::detectCanonicalizedPredictableLoops(TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!nextInductionSymRef->getUseonlyAliases().isZero(comp()) || !_storeTrees[nextInductionVariableNumber] || !_loopTestTree || ((_loopTestTree->getNode()->getOpCodeValue() != TR::ificmplt) && (_loopTestTree->getNode()->getOpCodeValue() != TR::ificmple)) || !_isAddition || (exitBlocks.getListHead()->getData()->getLastRealTreeTop() != _loopTestTree) || !nextInductionSymRef->getSymbol()->getType().isInt32())
            continue;
        ...
        vcount_t visitCount = comp()->incVisitCount();
        if (!isSameInductionVariable || (_loopTestTree->getNode()->getNumChildren() <= 1) || !loopStructure->asRegion()->isExprInvariant(_loopTestTree->getNode()->getSecondChild()))
            continue;
        ...
        if (isInvertibleLoop(nextInductionVariableNumber, loopStructure)) {
            ...
            if (v) {
                ...
                if (entryVal && entryVal->asIntConst() && (entryVal->getLowInt() == 0 || (exitVal && exitVal->asIntConst() && (llabs((long long)(exitVal->getLowInt()) - (long long)(entryVal->getLowInt()))) < INT_MAX)) &&
                    ...
                    if ((_loopTestTree->getNode()->getSecondChild()->getOpCode().isLoadVarDirect() || _loopTestTree->getNode()->getSecondChild()->getOpCode().isLoadConst()))
                        isInvertible = true;
                }
            }
        }
        ...
        if (isInvertible && performTransformation(comp(), "%sInverting loop %d\n", OPT_DETAILS, loopStructure->getNumber())) {
            ...
            if (_loopTestTree->getNode()->getSecondChild()->getOpCode().isLoadConst()) {
                ...
                if (incrVal->asIntConst() && incrVal->getLowInt() == 1 && testValue < INT_MAX && loopTestNode->getOpCodeValue() == TR::ificmple) {
                    testValue += 1;
...
bool TR_LoopInverter::isInvertibleLoop(int32_t symRefNum, TR_Structure* structure)
{
    if (structure->asBlock() != NULL) {
        ...
        while (currentTree != exitTree) {
            ...
            if (!checkIfSymbolIsReadInKnownTree(currentNode, symRefNum, currentTree, checklist) || ((currentNode->getOpCodeValue() != TR::asynccheck) && currentNode->canGCandReturn()))
                return false;
