...
void
TR_LoopTransformer::detectWhileLoopsInSubnodesInOrder(
  ...
  bool innerFirst)
{
  ...
  for (auto pred = rootNode->getPredecessors().begin();
       pred != rootNode->getPredecessors().end();
       ++pred) {
    ...
    if (pendingList->get(predStructure->getNumber()) && (!alreadyVisitedNode)) {
      detectWhileLoopsInSubnodesInOrder(whileLoopsInnerFirst,
                                        whileLoops,
                                        doWhileLoopsInnerFirst,
                                        doWhileLoops,
                                        predStructure,
  ...
  for (auto pred = rootNode->getExceptionPredecessors().begin();
       pred != rootNode->getExceptionPredecessors().end();
       ++pred) {
    TR_StructureSubGraphNode* predNode =
      (TR_StructureSubGraphNode*)(*pred)->getFrom();
    TR_Structure* predStructure = predNode->getStructure();
  ...
  for (auto succ = rootNode->getSuccessors().begin();
       succ != rootNode->getSuccessors().end();
       ++succ) {
  ...
  for (auto succ = rootNode->getExceptionSuccessors().begin();
       succ != rootNode->getExceptionSuccessors().end();
       ++succ) {
    TR_StructureSubGraphNode* succNode =
      (TR_StructureSubGraphNode*)(*succ)->getTo();
    TR_Structure* succStructure = succNode->getStructure();
...
void
TR_LoopTransformer::createWhileLoopsList(
 ...
 */
void
TR_LoopTransformer::detectWhileLoops(
  ListAppender<TR_Structure>& whileLoopsInnerFirst,
  List<TR_Structure>& whileLoops,
  ListAppender<TR_Structure>& doWhileLoopsInnerFirst,
  List<TR_Structure>& doWhileLoops,
...
region->getEntry(), region, visitCount, pendingList, innerFirst);
  ...
  */
  ...
  if (isDoWhileLoop) {
    if (trace())
      traceMsg(comp(),
               "Adding structure %d(%p) as a doWhile loop\n",
               region->getNumber(),
               region);
    if (innerFirst)
    ...
    if (trace())
      traceMsg(comp(),
               "Adding structure %d(%p) as a While loop\n",
               region->getNumber(),
               region);
...
bool
TR_LoopTransformer::findMatchingIVInRegion(TR::TreeTop* loopTestTree,
                                           TR_RegionStructure* region)
{
 ...
 */
void
TR_LoopCanonicalizer::canonicalizeNaturalLoop(TR_RegionStructure* whileLoop)
{
  rewritePostToPreIncrementTestInRegion(whileLoop);

  if (!performTransformation(comp(),
                             "%sCanonicalizing natural loop %d\n",
                             OPT_DETAILS,
                             whileLoop->getNumber()))
    return;
  ...
  for (auto nextSucc = loopHeader->getSuccessors().begin();
       nextSucc != loopHeader->getSuccessors().end();
       ++nextSucc) {
  ...
  for (auto edge = loopHeader->getPredecessors().begin();
       edge != loopHeader->getPredecessors().end();
       ++edge) {
    ...
    if (!whileLoop->contains(pred->getStructureOf(), whileLoop->getParent())) {
      ...
      if (pred != _cfg->getStart()) {
        ...
        if (lastNonFenceTree->getNode()->getOpCode().isBranch() ||
          ...
          // case.
          //
          if (!lastNonFenceTree->adjustBranchOrSwitchTreeTop(
                comp(), entryTree, newEntryTree)) {
            TR::TreeTop* gotoTreeTop = TR::TreeTop::create(
  ...
  */
  ...
  TR::Block* splitter1 =
    TR::Block::createEmptyBlock(loopHeader->getEntry()->getNode(),
                                comp(),
                                sumPredFreq < 0 ? 0 : sumPredFreq,
                                loopHeader);
  TR::Block* splitter2 =
    TR::Block::createEmptyBlock(loopHeader->getEntry()->getNode(),
                                comp(),
  ...
  if (reversedBranch) {
    extraGoto = TR::Block::createEmptyBlock(loopHeader->getEntry()->getNode(),
                                            comp(),
                                            sumPredFreq < 0 ? 0 : sumPredFreq,
                                            loopHeader);
  }
  ...
  for (auto nextEdge = loopHeader->getSuccessors().begin();
       nextEdge != loopHeader->getSuccessors().end();
       ++nextEdge) {
  ...
  if (loopBody->isCold()) {
    if (trace())
      traceMsg(comp(),
               "Setting s2 block_%d cold because loop body is cold\n",
               splitter2->getNumber());
  ...
  if (joinBlock->isCold()) {
    if (trace())
      traceMsg(comp(),
               "Setting s1 block_%d cold because join block is cold\n",
               splitter1->getNumber());
    splitter2->setIsCold(true);
  }
  ...
  if (trace()) {
    traceMsg(
      ...
      clonedHeader);
    traceMsg(comp(),
             "Creating splitter blocks %d and %d\n",
             splitter1->getNumber(),
             splitter2->getNumber());
    if (reversedBranch)
      traceMsg(
        comp(), "Creating extra goto block_%d\n", extraGoto->getNumber());
  }
  ...
  for (auto edge = entryGraphNode->getSuccessors().begin();
       edge != entryGraphNode->getSuccessors().end();
       ++edge) {
  ...
  if (trace()) {
    traceMsg(comp(),
             "Structure after canonicalizing natural loop %d:\n",
...
bool
TR_LoopCanonicalizer::isLegalToSplitEdges(TR_RegionStructure* doWhileLoop,
                                          TR::Block* blockAtHeadOfLoop)
{
  // checking legality before loopInvariantBlock is created, so can set it to
  // null
  int32_t sumPredFreq;
  return modifyBranchesForSplitEdges(doWhileLoop,
                                     blockAtHeadOfLoop,
                                     NULL,
                                     NULL,
                                     false,
 ...
 */
bool
TR_LoopCanonicalizer::modifyBranchesForSplitEdges(
  ...
  bool isCheckOnly)
{
  TR_ASSERT(
    (!isCheckOnly) ||
      (isCheckOnly && (loopInvariantBlock == NULL) && (targetBlock == NULL)),
      ...
      *sumPredFreq = *sumPredFreq + (*edge)->getFrequency();
      if (pred != _cfg->getStart()) {
        ...
        if (isCheckOnly) {
          ...
          if (!lastTree->adjustBranchOrSwitchTreeTop(
                comp(), blockHeadTreeTop, targetBlock->getEntry())) {
            if (addToEnd) {
...
void
TR_LoopCanonicalizer::canonicalizeDoWhileLoop(TR_RegionStructure* doWhileLoop)
{
  ...
  if (!performTransformation(comp(),
                             "%sCanonicalizing do while loop %d\n",
                             OPT_DETAILS,
  ...
  if (!comp()->getOption(
        TR_DisableEmptyPreHeaderCheck)) // Skip adding another pre-header if you
                                        // don't need to.
  {
    for (auto iedge = blockAtHeadOfLoop->getPredecessors().begin();
         iedge != blockAtHeadOfLoop->getPredecessors().end();
         ++iedge) {
  ...
  TR::Block* loopInvariantBlock =
    TR::Block::createEmptyBlock(predBlockExitNode,
                                comp(),
  ...
  if (isEntry) {
    ...
    dummyEntryBlockStructure = new (_cfg->structureRegion())
  ...
  int32_t sumPredFreq;
  modifyBranchesForSplitEdges(doWhileLoop,
                              blockAtHeadOfLoop,
                              loopInvariantBlock,
                              targetBlock,
                              addToEnd,
  ...
  for (auto predIt = doWhileNode->getExceptionPredecessors().begin();
    ...
    TR_RegionStructure* pred =
      toStructureSubGraphNode(nextPred->getFrom())->getStructure()->asRegion();
    if (pred && !isEntry)
      pred->replaceExitPart(doWhileNode->getNumber(),
                            targetStructure->getNumber());
  }
  ...
  if (trace()) {
    ...
    if (comp()->getFlowGraph()->getStructure())
      getDebug()->print(
        comp()->getOutFile(), comp()->getFlowGraph()->getStructure(), 6);
  }
...
void
TR_LoopCanonicalizer::eliminateRedundantInductionVariablesFromLoop(
  TR_RegionStructure* naturalLoop)
{
  ...
  for (auto nextEdge = entryBlock->getPredecessors().begin();
       nextEdge != entryBlock->getPredecessors().end();
       ++nextEdge) {
    ...
    for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
      ...
      if ((nextIndVar->getSymRef()->getSymbol()->getDataType() ==
        ...
        TR_ScratchList<TR::Block> primaryInductionVarIncrementBlocks(
          trMemory());
        // comp()->incVisitCount();
        bool incrementsInLockStep =
          incrementedInLockStep(naturalLoop,
                                derivedInductionVar,
                                symRefInCompare,
                                nextIndVarIncr,
                                primaryIncr,
        ...
        if (incrementsInLockStep &&
          ...
          incrementsInLockStep =
            checkIfOrderOfBlocksIsKnown(naturalLoop,
                                        entryBlock,
                                        loopTestBlock,
                                        &derivedInductionVarIncrementBlocks,
                                        &primaryInductionVarIncrementBlocks,
        ...
        if (incrementsInLockStep) {
          ...
          if ((!comp()->cg()->hasComplexAddressingMode()) ||
            ...
            if (performTransformation(
                  comp(),
                  "%sRemoving redundant induction variable #%d\n",
                  OPT_DETAILS,
...
bool
TR_LoopCanonicalizer::replaceInductionVariableComputationsInExits(
  ...
  for (auto edge = _entryBlock->getSuccessors().begin();
    ...
    if (!structure->contains(block->getStructureOf())) {
      ...
      if (_primaryIncrementedFirst != 0) {
        ...
        subNode =
          TR::Node::create(((dataType == TR::Int32) ? TR::iadd : TR::ladd),
                           2,
                           subNode,
...
void
TR_LoopCanonicalizer::findIncrements(TR::Node* currentNode,
                                     vcount_t visitCount,
                                     TR::SymbolReference* derivedInductionVar,
                                     TR::SymbolReference* primaryInductionVar,
...
bool
TR_LoopCanonicalizer::incrementedInLockStep(
  TR_Structure* structure,
  TR::SymbolReference* derivedInductionVar,
  TR::SymbolReference* primaryInductionVar,
  ...
  TR_ScratchList<TR::Block>* primaryInductionVarIncrementBlocks)
{
  if (structure->asBlock() != NULL) {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (!incrementedInLockStep(node->getStructure(),
                                 derivedInductionVar,
                                 primaryInductionVar,
                                 derivedInductionVarLoopIncrement,
                                 primaryInductionVarLoopIncrement,
...
bool
TR_LoopCanonicalizer::checkIfOrderOfBlocksIsKnown(
  TR_RegionStructure* naturalLoop,
  TR::Block* entryBlock,
  ...
  uint8_t& primaryFirst)
{
  ...
  for (auto edge = entryBlock->getExceptionSuccessors().begin();
       edge != entryBlock->getExceptionSuccessors().end();
       ++edge) {
  ...
  for (auto edge = loopTestBlock->getExceptionSuccessors().begin();
       edge != loopTestBlock->getExceptionSuccessors().end();
       ++edge) {
    TR::Block* catchBlock = (*edge)->getTo()->asBlock();
    if (naturalLoop->contains(catchBlock->getStructureOf()))
      return false;
 ...
 */
bool
TR_LoopCanonicalizer::checkComplexInductionVariableUse(TR_Structure* structure)
{
  if (structure->asBlock() != NULL) {
    TR_BlockStructure* blockStructure = structure->asBlock();
    TR::Block* block = blockStructure->getBlock();
...
bool
TR_LoopCanonicalizer::checkComplexInductionVariableUseNode(TR::Node* node,
                                                           bool addrExpression)
{
  ...
  if (opCode.isStoreIndirect()) {
    ...
    if (node->getOpCodeValue() == TR::imul) {
      traceMsg(
        comp(), "Found imul node 0x%p used in address expression.\n", node);
      ...
      traceMsg(
        comp(), "Found lmul node 0x%p used in address expression.\n", node);
...
bool
TR_LoopCanonicalizer::examineTreeForInductionVariableUse(
  TR::Block* loopInvariantBlock,
  TR::Node* parent,
  int32_t childNum,
  TR::Node* node,
  ...
  if (node->getOpCode().hasSymbolReference() && !node->getOpCode().isStore()) {
    if ((node->getSymbolReference() == _symRefBeingReplaced) &&
        performTransformation(
          comp(),
          "Replacing use %p of sym ref #%d by sym ref #%d\n",
          node,
      ...
      loadOfPrimaryInductionVar = TR::Node::create(
        ((dataType == TR::Int32) ? TR::iadd : TR::ladd),
        2,
        loadOfPrimaryInductionVar,
        TR::Node::createWithSymRef(node,
...
void
TR_LoopCanonicalizer::placeInitializationTreeInLoopPreHeader(
  TR::Block* b,
  TR::Node* node,
  TR::SymbolReference* newSymbolReference,
  TR::SymbolReference* primaryInductionVar,
  TR::SymbolReference* derivedInductionVar)
{
  TR::DataType dataType = newSymbolReference->getSymbol()->getDataType();
  TR::Node* subNode = TR::Node::create(
    (dataType == TR::Int32) ? TR::isub : TR::lsub,
    2,
    TR::Node::createWithSymRef(
      node, comp()->il.opCodeForDirectLoad(dataType), 0, derivedInductionVar),
    TR::Node::createWithSymRef(
      node, comp()->il.opCodeForDirectLoad(dataType), 0, primaryInductionVar));
  TR::Node* storeNode =
    TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(dataType),
                               1,
                               1,
 ...
 */
void
TR_LoopCanonicalizer::rewritePostToPreIncrementTestInRegion(
  TR_RegionStructure* region)
{
...
static bool
isLoadVarDirectOf(TR::Node* const node, int32_t symrefNum)
{
...
void
TR_LoopCanonicalizer::rewritePostToPreIncrementTestInBlock(
  TR::Block* const block)
{
  ...
  const int ifOpIdx =
    int(ifOp.isCompareTrueIfGreater()) << 1 | int(ifOp.isCompareTrueIfEqual());
...
bool
TR_LoopTransformer::replaceAllInductionVariableComputations(
  TR::Block* loopInvariantBlock,
  TR_Structure* structure,
  TR::SymbolReference** newSymbolReference,
  TR::SymbolReference* inductionVarSymRef)
{
  ...
  if (structure->asBlock() != NULL) {
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (replaceAllInductionVariableComputations(loopInvariantBlock,
                                                  node->getStructure(),
                                                  newSymbolReference,
                                                  inductionVarSymRef))
        seenInductionVariableComputation = true;
...
//
TR::Node*
TR_LoopTransformer::duplicateExact(TR::Node* node,
...
void
TR_LoopTransformer::adjustTreesInBlock(TR::Block* block)
{
 ...
 */
bool
TR_LoopTransformer::cleanseTrees(TR::Block* block)
{
  if (_cfg == NULL)
    return false;
 ...
 */
bool
TR_LoopTransformer::makeInvariantBlockFallThroughIfPossible(TR::Block* block)
{
  if (_cfg == NULL)
    return false;
...
bool
TR_LoopTransformer::isStoreInRequiredForm(int32_t symRefNum,
...
int32_t
TR_LoopTransformer::getInductionSymbolReference(TR::Node* node)
{
  if (node->getOpCode().hasSymbolReference() &&
...
int32_t
TR_LoopTransformer::checkLoopForPredictability(TR_Structure* loopStructure,
                                               TR::Block* loopInvariantBlock,
                                               ...
                                               bool returnIfNotPredictable)
{
  ...
  for (auto nextEdge = next->getPredecessors().begin();
       nextEdge != next->getPredecessors().end();
       ++nextEdge) {
    TR::Block* nextBlock = toBlock((*nextEdge)->getFrom());
...
void
TR_LoopTransformer::updateInfo(TR::Node* node,
                               vcount_t visitCount,
...
void
TR_LoopTransformer::collectSymbolsWrittenAndReadExactlyOnce(
  TR_Structure* structure,
  vcount_t visitCount)
{
  updateInfo_tables uinfo(comp()->allocator());
  ...
  updateInfo_tables& uinfo)
{
  if (structure->asBlock() != NULL) {
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      collectSymbolsWrittenAndReadExactlyOnce(
        node->getStructure(), visitCount, uinfo);
    }
  }
}
...
bool
TR_LoopTransformer::isSymbolReferenceWrittenNumberOfTimesInStructure(
  TR_Structure* structure,
  int32_t symRefNum,
                                                 ...
                                                 TR::Node* constNode)
{
  ...
  switch (node->getOpCodeValue()) {
    case TR::ificmplt: {
      ...
      subNode = TR::Node::create(TR::isub,
                                 2,
...
TR::Node*
TR_LoopTransformer::containsOnlyInductionVariableAndAdditiveConstant(
  TR::Node* node,
  int32_t inductionVariable)
{
  TR::ILOpCode& opCode = node->getOpCode();
...
void
TR_LoopTransformer::checkIfIncrementInDifferentExtendedBlock(
  TR::Block* block,
  int32_t inductionVariable)
{
...
bool
TR_LoopTransformer::blockIsAlwaysExecutedInLoop(
...
int32_t
TR_RedundantInductionVarElimination::perform()
{
...
bool
TR_LoopTransformer::detectEmptyLoop(TR_Structure* structure, int32_t* numTrees)
{
...
int32_t
TR_LoopInverter::detectCanonicalizedPredictableLoops(
  TR_Structure* loopStructure,
  ...
  int32_t bitVectorSize)
{
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (trace())
      traceMsg(comp(),
               "Loop %d is %s invertable\n",
               loopStructure->getNumber(),
               isInvertible ? "" : "not");

    if (isInvertible && performTransformation(comp(),
                                              "%sInverting loop %d\n",
                                              OPT_DETAILS,
                                              loopStructure->getNumber())) {
      ...
      if (_loopTestTree->getNode()
        ...
        storeNode = TR::Node::createWithSymRef(
          TR::istore, 1, 1, subNode, nextInductionSymRef);
      }
      ...
      dumpOptDetails(comp(),
                     "Init node: %p, update node: %p, loopTest node: %p\n",
                     storeNode,
                     _storeTrees[nextInductionVariableNumber]->getNode(),
                     _loopTestTree->getNode());
        ...
        for (auto edge = nextBlock->getSuccessors().begin();
          ...
          if (!blocksInRegion.find(next)) {
            ...
            if (loopTestNode->getOpCodeValue() == TR::ificmpgt)
              storeNode =
                TR::Node::createWithSymRef(TR::istore,
                                           1,
                                           1,
...
bool
TR_LoopInverter::isInvertibleLoop(int32_t symRefNum, TR_Structure* structure)
{
  if (structure->asBlock() != NULL) {
    if (comp()->getSymRefTab()->getSymRef(symRefNum)->getSymbol()->isVolatile())
      return false;
    ...
    while (currentTree != exitTree) {
      ...
      if (!checkIfSymbolIsReadInKnownTree(
            currentNode, symRefNum, currentTree, checklist) ||
 ...
 */
bool
TR_LoopInverter::checkIfSymbolIsReadInKnownTree(TR::Node* node,
                                                int32_t symRefNum,
                                                ...
                                                TR::NodeChecklist& checklist)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    if (!checkIfSymbolIsReadInKnownTree(
          node->getChild(i), symRefNum, currentTree, checklist))
      return false;
  }
