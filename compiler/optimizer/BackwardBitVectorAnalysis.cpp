...
void TR_BackwardDFSetAnalysis<Container *>::initializeDFSetAnalysis() {
  this->initializeBasicDFSetAnalysis();

  _currentOutSetInfo = (Container **)this->trMemory()->allocateStackMemory(
      this->_numberOfNodes * sizeof(Container *));
  _originalOutSetInfo = (Container **)this->trMemory()->allocateStackMemory(
      this->_numberOfNodes * sizeof(Container *));
...
template <class Container>
bool TR_BackwardDFSetAnalysis<Container *>::canGenAndKillForStructure(
  ...
  if (naturalLoop && naturalLoop->isNaturalLoop()) {
    ...
    for (auto pred = predecessors.getFirst(); pred;
      ...
      if (!exitEdgeSeen) naturalLoop->setContainsImproperRegion(true);
    }
...
template <class Container>
void TR_BackwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForBlock(
    TR_BlockStructure *s) {
  ...
  for (auto succ = block->getSuccessors().begin();
    ...
    typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
        *pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
            Container *>::TR_ContainerNodeNumberPair(NULL,
                                                     succBlock->getNumber());
    analysisInfo->_currentRegularGenSetInfo->add(pair);
    pair = new (this->trStackMemory())
        typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair(
            NULL, succBlock->getNumber());
...
void TR_BackwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfo(
    ...
    TR_BitVector &exitNodes, bool lastIteration) {
  while (this->_analysisQueue.getListHead() &&
    ...
    if (this->_analysisInterrupted) {
      ...
      comp->failCompilation<TR::CompilationInterrupted>(
          "interrupted in backward bit vector analysis");
    }
      ...
      for (auto succ = successors.getFirst(); succ;
        ...
        if (!regionStructure->isExitEdge(succ)) {
              ...
              !succStructure->hasBeenAnalyzedBefore())  //_firstIteration)
          {
            ...
            typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
                *succInfo = this->getAnalysisInfo(succStructure);
          }
        }
        firstSucc = false;
      }
    ...
    if (traceBBVA()) {
      ...
      if (analysisInfo->_exceptionGenSetInfo) {
        traceMsg(this->comp(),
                 "\nException Gen Set Info for Region or Block : %p numbered "
                 "%d is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
      ...
      if (analysisInfo->_exceptionKillSetInfo) {
        traceMsg(this->comp(),
                 "\nException Kill Set Info for Region or Block : %p numbered "
                 "%d is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
    ...
    if (nodeStructure->getStructure()->asBlock()) isBlockStructure = true;
    ...
    for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
      ...
      TR_LinkHead<typename TR_BasicDFSetAnalysis<
          ...
          *_regionKillSetInfo = normalSucc
                                    ? regionAnalysisInfo->_regularKillSetInfo
                                    : regionAnalysisInfo->_exceptionKillSetInfo,
          *_currentKillSetInfo =
              normalSucc ? analysisInfo->_currentRegularKillSetInfo
          ...
          *_regionGenSetInfo = normalSucc
                                   ? regionAnalysisInfo->_regularGenSetInfo
                                   : regionAnalysisInfo->_exceptionGenSetInfo,
          *_currentGenSetInfo =
              normalSucc ? analysisInfo->_currentRegularGenSetInfo
                         : regionAnalysisInfo->_currentExceptionGenSetInfo;

      Container *killBitVector = analysisInfo->getContainer(
          _killSetInfo, isBlockStructure ? node->getNumber() : nodeNumber);

      Container *genBitVector = analysisInfo->getContainer(
          _genSetInfo, isBlockStructure ? node->getNumber() : nodeNumber);
      ...
      if (!seenSuccNodes.get(nodeNumber)) {
        ...
        this->_temp2->empty();
        if (killBitVector) *this->_temp2 |= *killBitVector;

        if (node == regionStructure->getEntry()) {
          typename TR_BasicDFSetAnalysis<
              Container *>::TR_ContainerNodeNumberPair *pair =
              regionAnalysisInfo->getContainerNodeNumberPair(_regionGenSetInfo,
                                                             nodeNumber);
          if (pair) {
            if (!pair->_container) {
          ...
          typename TR_BasicDFSetAnalysis<
              Container *>::TR_ContainerNodeNumberPair *pair =
              analysisInfo->getContainerNodeNumberPair(_currentGenSetInfo,
                                                       nodeNumber);
          if (!pair) {
            pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
                Container *>::TR_ContainerNodeNumberPair(NULL, nodeNumber);
          ...
          pair = analysisInfo->getContainerNodeNumberPair(_currentKillSetInfo,
                                                          nodeNumber);
          if (!pair) {
            pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
                Container *>::TR_ContainerNodeNumberPair(NULL, nodeNumber);
      ...
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
          *nodePair;
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
          *killNodePair = succInfo->_currentRegularKillSetInfo->getFirst();
      for (nodePair = succInfo->_currentRegularGenSetInfo->getFirst(); nodePair;
           nodePair = nodePair->getNext(),
          killNodePair = killNodePair->getNext()) {
        ...
        if (genBitVector) *this->_temp2 -= *genBitVector;
        if (killBitVector) *this->_temp2 |= *killBitVector;
      ...
      for (nodePair = succInfo->_currentExceptionGenSetInfo->getFirst();
           nodePair; nodePair = nodePair->getNext(),
          killNodePair = killNodePair->getNext()) {
        if (nodePair->_container)
          *this->_temp = *(nodePair->_container);
        else
    ...
    if (traceBBVA()) {
      ...
      if (analysisInfo->_currentRegularGenSetInfo) {
        traceMsg(this->comp(),
                 "\nCurrent Gen Set Info for Region or Block : %p numbered %d "
                 "is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
        for (pair = analysisInfo->_currentRegularGenSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          if (pair->_container) {
            traceMsg(this->comp(),
                     "Exit or Succ numbered %d : ", pair->_nodeNumber);
      ...
      if (analysisInfo->_currentRegularKillSetInfo) {
        traceMsg(this->comp(),
                 "\nCurrent Kill Set Info for Region or Block : %p numbered %d "
                 "is : \n",
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
        for (pair = analysisInfo->_currentRegularKillSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          if (pair->_container) {
            traceMsg(this->comp(),
                     "Exit or Succ numbered %d : ", pair->_nodeNumber);
      ...
      if (analysisInfo->_currentExceptionGenSetInfo) {
        traceMsg(this->comp(),
                 ...
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
        for (pair = analysisInfo->_currentExceptionGenSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          if (pair->_container) {
            traceMsg(this->comp(),
                     "Exit or Succ numbered %d : ", pair->_nodeNumber);
      ...
      if (analysisInfo->_currentExceptionKillSetInfo) {
        traceMsg(this->comp(),
                 ...
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
        for (pair = analysisInfo->_currentExceptionKillSetInfo->getFirst();
             pair; pair = pair->getNext()) {
          if (pair->_container) {
            traceMsg(this->comp(),
                     "Exit or Succ numbered %d : ", pair->_nodeNumber);
            pair->_container->print(this->comp());
...
template <class Container>
void TR_BackwardDFSetAnalysis<Container *>::
    initializeGenAndKillSetInfoForRegion(TR_RegionStructure *region) {
  typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *analysisInfo =
  ...
  for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
    ...
    if (!exitNodes.get(toStructureNumber)) exitNodes.set(toStructureNumber);
  }
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    if (subNode->getSuccessors().empty() &&
      ...
      if (!exitNodes.get(fromStructureNumber)) {
        ...
        typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
            *pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
                Container *>::TR_ContainerNodeNumberPair(b,
                                                         fromStructureNumber);
        analysisInfo->_regularGenSetInfo->add(pair);

        pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
            Container *>::TR_ContainerNodeNumberPair(b, fromStructureNumber);
        analysisInfo->_regularKillSetInfo->add(pair);
  ...
  while (changed) {
    ...
    si.reset();
    for (subNode = si.getCurrent(); subNode; subNode = si.getNext()) {
      if (noExitEdges || (subNode->getSuccessors().empty() &&
                          subNode->getExceptionSuccessors().empty())) {
...
bool TR_BackwardDFSetAnalysis<Container *>::analyzeRegionStructure(
  ...
  if (!analysisInfo) analysisInfo = this->getAnalysisInfo(regionStructure);
  ...
  for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
    ...
    int32_t toStructureNumber = edge->getTo()->getNumber();
    Container *outBitVector = analysisInfo->getContainer(
  ...
  while (changed) {
    ...
    for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
      TR_StructureSubGraphNode *edgeFrom =
          toStructureSubGraphNode(edge->getFrom());
      this->addToAnalysisQueue(edgeFrom, 0);
      if (analyzeNodeIfSuccessorsAnalyzed(regionStructure, pendingList,
                                          exitNodes)) {
  ...
  Container *inSetInfo =
      this->getAnalysisInfo(regionStructure->getEntry()->getStructure())
          ->_inSetInfo;
  ...
  while (this->_analysisQueue.getListHead() &&
      ...
      if (!regionStructure->isExitEdge(succ)) {
            ...
            !succStructure->hasBeenAnalyzedBefore())  // this->_firstIteration)
        {
          ...
          if (!seenNodes.get(succStructure->getNumber()))
            initializeInfo(_currentOutSetInfo[succStructure->getNumber()]);
          Container *succinfo =
              this->getAnalysisInfo(succStructure)->_inSetInfo;
    ...
    if (exitNodes.get(nodeStructure->getNumber())) {
      if (!(nodeStructure->getSuccessors().empty() &&
        ...
        typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
            *pair;
        for (pair = regionAnalysisInfo->_outSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          typename TR_BasicDFSetAnalysis<
              Container *>::TR_ContainerNodeNumberPair *otherPair;
        ...
        for (pair = analysisInfo->_outSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          this->copyFromInto(_originalOutSetInfo[pair->_nodeNumber],
                             _currentOutSetInfo[pair->_nodeNumber]);
        }
      }
    }
    ...
    if (this->supportsGenAndKillSets() &&
      ...
      if (nodeStructure->getStructure()->asRegion() &&
        ...
        TR_BitVector backEdgeNodes(
            this->comp()->trMemory()->currentStackRegion());
        ListIterator<TR::CFGEdge> exitIt(
            &nodeStructure->getStructure()->asRegion()->getExitEdges());
        TR::CFGEdge *exitEdge;
        for (exitEdge = exitIt.getCurrent(); exitEdge != NULL;
             exitEdge = exitIt.getNext()) {
          TR_StructureSubGraphNode *edgeFrom =
              toStructureSubGraphNode(exitEdge->getFrom());
          int32_t toStructureNumber = exitEdge->getTo()->getNumber();

          if (edgeFrom->hasSuccessor(
                  nodeStructure->getStructure()->asRegion()->getEntry())) {
            if (edgeFrom->hasSuccessor(exitEdge->getTo()))
              backEdgeNodes.set(toStructureNumber);
          } else if (edgeFrom->hasExceptionSuccessor(
                         nodeStructure->getStructure()
                             ->asRegion()
                             ->getEntry())) {
        ...
        Container *genBitVector = analysisInfo->getContainer(
            analysisInfo->_regularGenSetInfo, node->getNumber());
        ...
        for (nodePair = analysisInfo->_regularGenSetInfo->getFirst(); nodePair;
             nodePair = nodePair->getNext()) {
          if (!backEdgeNodes.get(nodePair->_nodeNumber)) continue;
        ...
        for (nodePair = analysisInfo->_exceptionGenSetInfo->getFirst();
             nodePair; nodePair = nodePair->getNext()) {
          if (!backEdgeNodes.get(nodePair->_nodeNumber)) continue;
    ...
    if (this->supportsGenAndKillSets() &&
      ...
      if (checkForChange) *this->_temp = *(analysisInfo->_inSetInfo);
      ...
      for (auto succ = successors.getFirst(); succ;
        ...
        bool normalSucc = (++count <= nodeStructure->getSuccessors().size());
        int32_t nodeNumber =
            isBlockStructure ? node->getNumber() : succ->getTo()->getNumber();
        Container *killBitVector = analysisInfo->getContainer(
        ...
        *this->_regularInfo = *(_currentOutSetInfo[nodeNumber]);
        ...
        if (killBitVector) *this->_regularInfo -= *killBitVector;

        if (genBitVector) *this->_regularInfo |= *genBitVector;
      ...
      if (nodeStructure->getStructure()->asRegion() &&
        ...
        for (exitEdge = exitIt.getCurrent(); exitEdge != NULL;
             exitEdge = exitIt.getNext()) {
          int32_t toStructureNumber = exitEdge->getTo()->getNumber();
          compose(_currentOutSetInfo[toStructureNumber],
                  analysisInfo->_inSetInfo);
        }
      }
    ...
    if (b || pendingList.get(node->getNumber())) inSetChanged = true;
    pendingList.reset(node->getNumber());
    this->removeHeadFromAnalysisQueue();
        ...
        if (!(nodeStructure->getSuccessors().empty() &&
          ...
          for (pair = regionAnalysisInfo->_outSetInfo->getFirst(); pair;
            ...
            for (otherPair = analysisInfo->_outSetInfo->getFirst(); otherPair;
                 otherPair = otherPair->getNext()) {
              if (pair->_nodeNumber == otherPair->_nodeNumber) {
                this->copyFromInto(pair->_container,
                                   _currentOutSetInfo[pair->_nodeNumber]);
                break;
              }
            }
    ...
    for (auto pred = predecessors.getFirst(); pred;
      ...
      if ((needToIterate || (node != regionStructure->getEntry())) &&
        ...
        this->_nodesInCycle->empty();
        this->addToAnalysisQueue(toStructureSubGraphNode(predNode),
                                 inSetChanged ? 1 : 0);
      }
    }
...
template <class Container>
bool TR_BackwardDFSetAnalysis<Container *>::analyzeBlockStructure(
  ...
  if (!analysisInfo) analysisInfo = this->getAnalysisInfo(blockStructure);
    ...
    *analysisInfo->_inSetInfo = *this->_regularInfo;
    if (!this->_blockAnalysisInfo[blockStructure->getNumber()])
      this->allocateBlockInfoContainer(
          ...
          this->_regularInfo);
    this->copyFromInto(this->_regularInfo,
                       this->_blockAnalysisInfo[blockStructure->getNumber()]);
  }
...
void TR_BackwardDFSetAnalysis<Container *>::analyzeNode(
    ...
    Container *_analysisInfo) {}

template <class Container>
void TR_BackwardDFSetAnalysis<Container *>::analyzeTreeTopsInBlockStructure(
  ...
  while (!(currentTree == entryTree)) {
    ...
    if (currentTreeHasChecks) compose(this->_regularInfo, this->_exceptionInfo);
...
template <class Container>
void TR_BackwardDFSetAnalysis<Container *>::compose(Container *, Container *) {}

template <class Container>
TR_DataFlowAnalysis::Kind TR_BackwardDFSetAnalysis<Container *>::getKind() {
