  ...
  while (this->_analysisQueue.getListHead()
    ...
    for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
      ...
      TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>*_killSetInfo
        ...
        *_currentKillSetInfo
        = normalSucc ? analysisInfo->_currentRegularKillSetInfo : regionAnalysisInfo->_currentExceptionKillSetInfo,
        ...
        *_currentGenSetInfo
        = normalSucc ? analysisInfo->_currentRegularGenSetInfo : regionAnalysisInfo->_currentExceptionGenSetInfo;
      ...
      for (nodePair = succInfo->_currentRegularGenSetInfo->getFirst(); nodePair;
        ...
        if (node == regionStructure->getEntry()) {
          ...
          typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair
            = analysisInfo->getContainerNodeNumberPair(analysisInfo->_currentRegularGenSetInfo, nodePair->_nodeNumber);
          if (!pair) {
            pair = new (this->trStackMemory())
              typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(NULL, nodePair->_nodeNumber);
            analysisInfo->_currentRegularGenSetInfo->add(pair);
          }
          if (!pair->_container) {
            pair->_container = initializeInfo(NULL);
          ...
          pair
            = analysisInfo->getContainerNodeNumberPair(analysisInfo->_currentRegularKillSetInfo, nodePair->_nodeNumber);
          if (!pair) {
            pair = new (this->trStackMemory())
              typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair(NULL, nodePair->_nodeNumber);
            analysisInfo->_currentRegularKillSetInfo->add(pair);
