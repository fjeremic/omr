...
template <class Container>
void TR_BackwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(TR_RegionStructure* regionStructure, TR_BitVector& pendingList, TR_BitVector& exitNodes, bool lastIteration)
{
    while (this->_analysisQueue.getListHead() && (this->_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
            ...
            Container* killBitVector = analysisInfo->getContainer(_killSetInfo, isBlockStructure ? node->getNumber() : nodeNumber);

            Container* genBitVector = analysisInfo->getContainer(_genSetInfo, isBlockStructure ? node->getNumber() : nodeNumber);
            ...
            if (!seenSuccNodes.get(nodeNumber)) {
                ...
                if (node == regionStructure->getEntry()) {
                    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair = regionAnalysisInfo->getContainerNodeNumberPair(_regionGenSetInfo, nodeNumber);
                    if (pair) {
                        if (!pair->_container) {
                    ...
                    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair = analysisInfo->getContainerNodeNumberPair(_currentGenSetInfo, nodeNumber);
                    if (!pair) {
...
template <class Container>
bool TR_BackwardDFSetAnalysis<Container*>::analyzeRegionStructure(TR_RegionStructure* regionStructure, bool checkForChange)
{
    ...
    if (this->supportsGenAndKillSets() && (canGenAndKillForStructure(regionStructure) && regionStructure != this->_cfg->getStructure())) {
...
template <class Container>
bool TR_BackwardDFSetAnalysis<Container*>::analyzeNodeIfSuccessorsAnalyzed(TR_RegionStructure* regionStructure, TR_BitVector& pendingList, TR_BitVector& exitNodes)
{
    ...
    while (this->_analysisQueue.getListHead() && (this->_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
            ...
            if (isBlockStructure && nodeStructure->getSuccessors().empty() && nodeStructure->getExceptionSuccessors().empty()) {
                Container* genBitVector = analysisInfo->getContainer(analysisInfo->_regularGenSetInfo, node->getNumber());
        ...
        for (auto pred = predecessors.getFirst(); pred; pred = predecessors.getNext()) {
            TR_StructureSubGraphNode* predNode = (TR_StructureSubGraphNode*)pred->getFrom();
            TR_Structure* predStructure = predNode->getStructure();
            if ((needToIterate || (node != regionStructure->getEntry())) && (pendingList.get(predStructure->getNumber()) || inSetChanged)) {
                this->_nodesInCycle->empty();
...
template <class Container>
void TR_BackwardDFSetAnalysis<Container*>::analyzeNode(TR::Node* node, vcount_t visitCount, TR_BlockStructure* blockStructure, Container* _analysisInfo)
{
}
