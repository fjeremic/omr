    ...
    TR::SparseBitVector& symRefsToCheck, bool trace, TR::SparseBitVector& visitedNodes)
{
    ...
    if (opCode.hasSymbolReference() && !opCode.isLoad()) {
        ...
        return TR_maybe;
    } else if (opCode.isArithmetic() || opCode.isConversion() || opCode.isMax() || opCode.isMin() || opCode.isArrayLength() || (opCode.isBooleanCompare() && !opCode.isBranch())) {
    ...
    bool trace)
{
    ...
    while (start != end) {
        ...
        if (start->getNode() && start->getNode()->getOpCode().isStoreDirect() && start->getNode()->getSymbol()->isAuto() && failedTargets.ValueAt(start->getNode()->getFirstChild()->getGlobalIndex())) {
            if (trace)
                traceMsg(comp, "  priv arg remat: considering partial remat with node [%p]", start->getNode());
            for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
                if ((*iter) && (*iter)->getNode()->getOpCode().hasSymbolReference() && (*iter)->getNode()->getSymbolReference()->getReferenceNumber() != start->getNode()->getSymbolReference()->getReferenceNumber() && (*iter)->getNode()->getFirstChild() == start->getNode()->getFirstChild()) {
                    if (trace)
