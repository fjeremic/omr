...
void RematSafetyInformation::add(TR::TreeTop *argStore,
                                 TR::SparseBitVector &symRefDependencies) {
...
void RematSafetyInformation::add(TR::TreeTop *argStore,
                                 TR::TreeTop *rematStore) {
  TR::SparseBitVector symrefDeps(comp->allocator());
  symrefDeps
      [rematStore->getNode()->getSymbolReference()->getReferenceNumber()] =
          true;
  ...
  for (uint32_t i = 0; i < dependentSymRefs.size(); ++i) {
    ...
    if (rematTreeTops[i]) {
      if (rematTreeTops[i] != argumentTreeTops[i])
        traceMsg(comp, "     partial remat candidate node %d",
                 rematTreeTops[i]->getNode()->getGlobalIndex());
      else
      ...
      traceMsg(comp, "    candidate is unsafe for remat - no candidates under "
                     "consideration");
    }
  }
}
...
/*
 * This function allows the remat methods to follow nodes along a path. The path
 * is described as a bit vector of blocks that may be visited, including both
 * the start and the end blocks. To ensure the path is deterministic, none of
 * the blocks along the path may be a merge point, except for the start block.
 * Due to this, the function must be provided with the start block, to avoid
 * revisiting it.
 *
 * Modifies the provided TreeTop pointer to point at the next treetop or the
 * treetop for the first valid successor if one exists.
 *
 * Returns true if it was able to find a valid treetop, false otherwise.
 */
bool RematTools::getNextTreeTop(TR::TreeTop *&treeTop,
                                TR_BitVector *blocksToVisit,
                                TR::Block *startBlock) {
  if (!blocksToVisit || !treeTop->getNode() ||
  ...
  for (auto e = block->getSuccessors().begin();
    ...
    if (blocksToVisit->get((*e)->getTo()->getNumber()) &&
        (*e)->getTo() != startBlock) {
  ...
  if (!nextBlock)
    for (auto e = block->getExceptionSuccessors().begin();
         e != block->getExceptionSuccessors().end(); ++e)
      if (blocksToVisit->get((*e)->getTo()->getNumber()) &&
          (*e)->getTo() != startBlock) {
        nextBlock = (*e)->getTo()->asBlock();
        break;
  ...
  if (nextBlock) {
    if (nextBlock->getPredecessors().size() +
            nextBlock->getExceptionPredecessors().size() !=
        1) {
 ...
 * constant expression (we won't duplicate because constant prop should sort
 * these), TR_no if remat is unsafe Side-effect - adds node to scanTarget if
 * returning true, otherwise none
 */
TR_YesNoMaybe RematTools::gatherNodesToCheck(
    TR::Compilation *comp, TR::Node *privArg, TR::Node *currentNode,
    TR::SparseBitVector &scanTargets, TR::SparseBitVector &symRefsToCheck,
  ...
  if (opCode.hasSymbolReference() && !opCode.isLoad()) {
    if (trace)
      traceMsg(comp,
               "  priv arg remat: Can't fully remat [%p] due to [%p] - "
               "non-load with a symref",
               privArg, currentNode);
    return TR_no;
    ...
    if (currentNode->getSymbolReference()->hasKnownObjectIndex() ||
      ...
      symRefsToCheck[currentNode->getSymbolReference()->getReferenceNumber()] =
          true;
      return TR_yes;
    }
  } else if (opCode.isLoadIndirect()) {
    // cannot rematerialize an array access without also creating a spine check
    // avoiding this case for now but this case can be handled in the future by
    // creating a spine check
    //
    if (comp->requiresSpineChecks() &&
        currentNode->getSymbol()->isArrayShadowSymbol()) {
      if (trace)
        traceMsg(comp,
                 "  priv arg remat: Can't fully remat [%p] due to [%p] - array "
                 "access needs spine check",
                 privArg, currentNode);
    ...
    if (gatherNodesToCheck(comp, privArg, currentNode->getFirstChild(),
                           scanTargets, symRefsToCheck, trace,
                           visitedNodes) != TR_no) {
      scanTargets[currentNode->getGlobalIndex()] = true;
      symRefsToCheck[currentNode->getSymbolReference()->getReferenceNumber()] =
          true;
    ...
    return TR_maybe;
  } else if (opCode.isArithmetic() || opCode.isConversion() || opCode.isMax() ||
             opCode.isMin() || opCode.isArrayLength() ||
             (opCode.isBooleanCompare() && !opCode.isBranch())) {
    // gather the child nodes of interest separately until we know it is worth
    // checking them
    ...
    for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
      ...
      TR_YesNoMaybe childResult = gatherNodesToCheck(
          comp, privArg, currentNode->getChild(i), childScanTargets,
          childSymRefsToCheck, trace, visitedNodes);
      if (childResult == TR_no) {
        if (trace)
          traceMsg(comp,
                   "  priv arg remat: Can't fully remat [%p] due to [%p] - "
                   "unsafe arithmetic child %d",
    ...
    if (trace)
      traceMsg(comp,
               "  guarded call remat: Can't fully remat [%p] due to [%p] - "
               "unhandled case",
               privArg, currentNode);
    return TR_no;
  }
...
void RematTools::walkNodesCalculatingRematSafety(
    TR::Compilation *comp, TR::Node *currentNode,
  ...
  for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
    ...
    walkNodesCalculatingRematSafety(comp, currentNode->getChild(i), scanTargets,
                                    enabledSymRefs, unsafeSymRefs, trace,
                                    visitedNodes);
  }
  ...
  if (opCode.isLikeDef() && opCode.hasSymbolReference()) {
    ...
    unsafeSymRefs[currentNode->getSymbolReference()->getReferenceNumber()] =
        true;
  }

  // update unsafeSymRefs - only set enabledSymRefs since the others are not
  // "live"
  unsafeSymRefs &= enabledSymRefs;

  // step 2 - if we have encountered a node we are checking for, update
  // enabledSymRefs
  if (scanTargets.ValueAt(currentNode->getGlobalIndex()) &&
      opCode.hasSymbolReference()) {
    // enable the symref we just found
    if (trace)
      traceMsg(comp, "Enabling symref #%d\n",
               currentNode->getSymbolReference()->getReferenceNumber());
    enabledSymRefs[currentNode->getSymbolReference()->getReferenceNumber()] =
        true;
  }
  if (trace) {
 ...
 */
bool RematTools::walkTreesCalculatingRematSafety(
    TR::Compilation *comp, TR::TreeTop *start, TR::TreeTop *end,
  ...
  while (start != end) {
    if (start->getNode()) {
      walkNodesCalculatingRematSafety(comp, start->getNode(), scanTargets,
                                      enabledSymRefs, unsafeSymRefs, trace,
                                      visitedNodes);
    }
    if (!getNextTreeTop(start, blocks, firstBlock)) {
      traceMsg(
          comp,
          "  remat tools: failed to follow path for remat safety at [%p]\n",
          start->getNode());
      return false;
    }
  }
...
void RematTools::walkTreesCalculatingRematSafety(
    TR::Compilation *comp, TR::TreeTop *start, TR::TreeTop *end,
    TR::SparseBitVector &scanTargets, TR::SparseBitVector &unsafeSymRefs,
    bool trace) {
  // As no path is provided, the call cannot fail so its return value is ignored
  walkTreesCalculatingRematSafety(comp, start, end, scanTargets, unsafeSymRefs,
                                  NULL, trace);
}
 ...
 * same block, a NULL pointer can be provided.
 *
 * This method is used to find alternative temps which we might be able to load
 * from if we can't full rematerialize a privatized inliner arg. If we find a
 * store of one of the failed arguments, we record the store node and add the
 * symref to the dependencies for the arg. If the symref is not killed before
 * the end of the block we do a partial rematerialization by simply loading the
 * temp. Returns true to indicate it successfully completed the analysis.
 */
bool RematTools::walkTreeTopsCalculatingRematFailureAlternatives(
    TR::Compilation *comp, TR::TreeTop *start, TR::TreeTop *end,
    TR::list<TR::TreeTop *> &failedArgs, TR::SparseBitVector &scanTargets,
    RematSafetyInformation &rematInfo, TR_BitVector *blocksToVisit,
    bool trace) {
  TR::Block *firstBlock = start->getEnclosingBlock();
  TR_BitVector *blocks = blocksToVisit;
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
  ...
  while (start != end) {
    if (trace)
      traceMsg(comp,
               "  priv arg remat: visiting [%p]: isStore %d privArg %d "
               "failedTargetMatch %d",
               start->getNode(), start->getNode()->getOpCode().isStoreDirect(),
               start->getNode()->getSymbol() &&
                   start->getNode()->getSymbol()->isAuto(),
               start->getNode()->getNumChildren() > 0
                   ? failedTargets.ValueAt(
                         start->getNode()->getFirstChild()->getGlobalIndex())
                   : -1);
    if (start->getNode() && start->getNode()->getOpCode().isStoreDirect() &&
      ...
      if (trace)
        traceMsg(comp,
                 "  priv arg remat: considering partial remat with node [%p]",
                 start->getNode());
      for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
        if ((*iter) && (*iter)->getNode()->getOpCode().hasSymbolReference() &&
            ...
            (*iter)->getNode()->getFirstChild() ==
                start->getNode()->getFirstChild()) {
          if (trace)
            traceMsg(comp,
                     "  priv arg remat: attempting partial remat with node "
                     "[%p] for [%p]",
                     start->getNode(), (*iter)->getNode());
          rematInfo.add(*iter, start);
          scanTargets[start->getNode()->getGlobalIndex()] = true;
          failedTargets[start->getNode()->getFirstChild()->getGlobalIndex()] =
              false;
          *iter = NULL;
        }
      }
    ...
    if (!getNextTreeTop(start, blocks, firstBlock)) {
      traceMsg(comp,
               "  remat tools: failed to follow path for failure alternatives "
               "at [%p]\n",
               start->getNode());
      return false;
    }
...
void RematTools::walkTreeTopsCalculatingRematFailureAlternatives(
    TR::Compilation *comp, TR::TreeTop *start, TR::TreeTop *end,
    TR::list<TR::TreeTop *> &failedArgs, TR::SparseBitVector &scanTargets,
    RematSafetyInformation &rematInfo, bool trace) {
  // As no path is provided, the call cannot fail so its return value is ignored
  walkTreeTopsCalculatingRematFailureAlternatives(
      comp, start, end, failedArgs, scanTargets, rematInfo, NULL, trace);
}
