...
void
RematSafetyInformation::add(TR::TreeTop* argStore,
...
void
RematSafetyInformation::add(TR::TreeTop* argStore, TR::TreeTop* rematStore)
{
  TR::SparseBitVector symrefDeps(comp->allocator());
  symrefDeps
    [rematStore->getNode()->getSymbolReference()->getReferenceNumber()] = true;
...
void
RematSafetyInformation::dumpInfo(TR::Compilation* comp)
{
  for (uint32_t i = 0; i < dependentSymRefs.size(); ++i) {
    traceMsg(comp,
             "  Arg Remat Safety Info for priv arg store node %d",
             argumentTreeTops[i]->getNode()->getGlobalIndex());
    if (rematTreeTops[i]) {
      if (rematTreeTops[i] != argumentTreeTops[i])
        traceMsg(comp,
                 "     partial remat candidate node %d",
 ...
 */
bool
RematTools::getNextTreeTop(TR::TreeTop*& treeTop,
                           ...
                           TR::Block* startBlock)
{
  ...
  if (!nextBlock)
    for (auto e = block->getExceptionSuccessors().begin();
         e != block->getExceptionSuccessors().end();
         ++e)
      if (blocksToVisit->get((*e)->getTo()->getNumber()) &&
          (*e)->getTo() != startBlock) {
        nextBlock = (*e)->getTo()->asBlock();
 ...
 */
TR_YesNoMaybe
RematTools::gatherNodesToCheck(TR::Compilation* comp,
                               TR::Node* privArg,
                               TR::Node* currentNode,
                               ...
                               TR::SparseBitVector& visitedNodes)
{
  ...
  if (opCode.hasSymbolReference() && !opCode.isLoad()) {
    ...
    for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
      ...
      TR_YesNoMaybe childResult = gatherNodesToCheck(comp,
                                                     privArg,
                               ...
                               bool trace)
{
  ...
  return gatherNodesToCheck(comp,
                            privArg,
                            currentNode,
                            scanTargets,
                            symRefsToCheck,
...
void
RematTools::walkNodesCalculatingRematSafety(TR::Compilation* comp,
                                            TR::Node* currentNode,
                                            TR::SparseBitVector& scanTargets,
                                            ...
                                            TR::SparseBitVector& visitedNodes)
{
  for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
    ...
    walkNodesCalculatingRematSafety(comp,
                                    currentNode->getChild(i),
                                    scanTargets,
                                    enabledSymRefs,
                                    unsafeSymRefs,
  ...
  if (trace) {
    traceMsg(
      comp, "Remat Safety Walk visiting n%dn\n", currentNode->getGlobalIndex());
  ...
  if (trace) {
    traceMsg(comp,
             "Remat Safety Walk after visiting n%dn\n",
             currentNode->getGlobalIndex());
    traceMsg(comp, "Enabled symrefs: ");
    (*comp) << enabledSymRefs;
    traceMsg(comp, "\nUnsafe symrefs: ");
 ...
 */
bool
RematTools::walkTreesCalculatingRematSafety(TR::Compilation* comp,
                                            TR::TreeTop* start,
                                            TR::TreeTop* end,
                                            TR::SparseBitVector& scanTargets,
                                            TR::SparseBitVector& unsafeSymRefs,
...
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
  if (blocks) {
    blocks = new (comp->trStackMemory()) TR_BitVector(
                                            ...
                                            bool trace)
{
  ...
  walkTreesCalculatingRematSafety(
    comp, start, end, scanTargets, unsafeSymRefs, NULL, trace);
}
