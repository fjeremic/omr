...
void RematSafetyInformation::dumpInfo(TR::Compilation* comp)
{
  for (uint32_t i = 0; i < dependentSymRefs.size(); ++i) {
    traceMsg(
      comp, "  Arg Remat Safety Info for priv arg store node %d", argumentTreeTops[i]->getNode()->getGlobalIndex());
    if (rematTreeTops[i]) {
...
TR_YesNoMaybe RematTools::gatherNodesToCheck(TR::Compilation* comp, TR::Node* privArg, TR::Node* currentNode,
  TR::SparseBitVector& scanTargets, TR::SparseBitVector& symRefsToCheck, bool trace, TR::SparseBitVector& visitedNodes)
{
  ...
  if (opCode.hasSymbolReference() && !opCode.isLoad()) {
    if (trace)
      traceMsg(
        comp, "  priv arg remat: Can't fully remat [%p] due to [%p] - non-load with a symref", privArg, currentNode);
    return TR_no;
    ...
    if (comp->requiresSpineChecks() && currentNode->getSymbol()->isArrayShadowSymbol()) {
      if (trace)
        traceMsg(comp, "  priv arg remat: Can't fully remat [%p] due to [%p] - array access needs spine check", privArg,
          currentNode);
    ...
    for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
      ...
      if (childResult == TR_no) {
        if (trace)
          traceMsg(comp, "  priv arg remat: Can't fully remat [%p] due to [%p] - unsafe arithmetic child %d", privArg,
            currentNode, i);
...
void RematTools::walkNodesCalculatingRematSafety(TR::Compilation* comp, TR::Node* currentNode,
  TR::SparseBitVector& scanTargets, TR::SparseBitVector& enabledSymRefs, TR::SparseBitVector& unsafeSymRefs, bool trace,
    ...
    if (start->getNode() && start->getNode()->getOpCode().isStoreDirect() && start->getNode()->getSymbol()->isAuto()
      ...
      for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
        if ((*iter) && (*iter)->getNode()->getOpCode().hasSymbolReference()
          ...
          if (trace)
            traceMsg(comp, "  priv arg remat: attempting partial remat with node [%p] for [%p]", start->getNode(),
              (*iter)->getNode());
