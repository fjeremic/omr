...
void
TR_RegionAnalysis::simpleIterator(TR_Stack<int32_t>& workStack,
                                  ...
                                  bool doThisCheck)
{
  ...
  while (cursor.hasMoreElements()) {
    ...
    // In the original version we would descend into another level of a
    // recursion but if we do it here, we would screw up big time since we can't
    // push the same node three times. We should be able to push the same node at
    // most twice so if we come across the same node the second time
    // (nodesInPath[next]) we could say that we are done with this particular
    // node and move on
...
void
TR_RegionAnalysis::StructInfo::initialize(TR::Compilation* comp,
                                          int32_t index,
                                          TR::Block* block)
{
  _structure = new (comp->getFlowGraph()->structureRegion())
 ...
 */
void
TR_RegionAnalysis::createLeafStructures(TR::CFG* cfg, TR::Region& region)
{
  TR::CFGNode* cfgNode;
  TR::Block* b;
  int32_t nodeNum;
  ...
  _infoTable = (StructInfo**)_workingRegion.allocate(
    (cfg->getNumberOfNodes() + 1) * sizeof(StructInfo*));
  ...
  for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
    ...
    TR::Block* next;
    int32_t index;
    for (auto p = b->getPredecessors().begin(); p != b->getPredecessors().end();
         ++p) {
      next = toBlock((*p)->getFrom());
      index = _dominators._dfNumbers[next->getNumber()];
      si._pred.set(index);
    }
    for (auto p = b->getSuccessors().begin(); p != b->getSuccessors().end();
         ++p) {
      next = toBlock((*p)->getTo());
      index = _dominators._dfNumbers[next->getNumber()];
      si._succ.set(index);
    }
    for (auto p = b->getExceptionPredecessors().begin();
         p != b->getExceptionPredecessors().end();
         ++p) {
      next = toBlock((*p)->getFrom());
      index = _dominators._dfNumbers[next->getNumber()];
      si._exceptionPred.set(index);
    }
    for (auto p = b->getExceptionSuccessors().begin();
         p != b->getExceptionSuccessors().end();
         ++p) {
      next = toBlock((*p)->getTo());
      index = _dominators._dfNumbers[next->getNumber()];
 ...
 */
TR_Structure*
TR_RegionAnalysis::getRegions(TR::Compilation* comp,
                              TR::ResolvedMethodSymbol* methSym)
{
 ...
 *
 * A proper acyclic region is a sub-graph with a single entry node and no
 * cycles. A proper cyclic region is a sub-graph with a single entry node and
 * one or more back-edges to the entry node, and no internal cycles. An improper
 * region is a sub-graph with a single entry node and which contains cycles.
 *
 * There are two passes.
 * On the first pass we go bottom up in the flow graph looking for natural
 * loops. On the second pass we go bottom up in the flow graph looking for
 * regions large enough to consider. The root region is returned.
 */
TR_Structure*
TR_RegionAnalysis::findRegions(TR::Region& memRegion)
{
  // Create work areas
 ...
 */
TR_RegionStructure*
TR_RegionAnalysis::findNaturalLoop(StructInfo& node,
                                   WorkBitVector& regionNodes,
                                   WorkBitVector& nodesInPath)
{
  regionNodes.empty();
  ...
  while (cursor.hasMoreElements()) {
    ...
    if (_dominators.dominates(node._originalBlock,
                              backEdgeNode._originalBlock)) {
      // A back-edge has been found. Add its loop nodes to the region
      //

      if (_useNew) {
        addNaturalLoopNodesIterativeVersion(backEdgeNode,
                                            regionNodes,
                                            nodesInPath,
                                            cyclesFound,
                                            node._originalBlock);
      } else {
        addNaturalLoopNodes(backEdgeNode,
                            regionNodes,
                            nodesInPath,
                            cyclesFound,
                            node._originalBlock);
  ...
  TR_RegionStructure* region = new (_structureRegion) TR_RegionStructure(
    _compilation, node._structure->getNumber() /* node._nodeIndex */);
  if (cyclesFound) {
    if (trace())
...
void
TR_RegionAnalysis::addNaturalLoopNodesIterativeVersion(
  StructInfo& node,
  WorkBitVector& regionNodes,
  WorkBitVector& nodesInPath,
  bool& cyclesFound,
  TR::Block* hdrBlock)
{
  ...
  // We will be pushing elements backwards to emulate processing in a
  // depth-first order Also, we will be pushing the same node twice in order to
  // update nodesInPath correctly
  ...
  while (!workStack.isEmpty()) {
    ...
    if (nodesInPath.get(index)) {
      // If node is in path, in region and cycles have been found then
      // processing the node again will not have any sideeffects, so we can skip
      // it completely
      ...
      workStack.push(
        index); // push again but now it will be marked in nodesInPath,
    ...
    if (trace()) {
      traceMsg(
        comp(), "addNaturalLoopNodesIterativeVersion, index = %d\n", index);
    }

    StructInfo& next = getInfo(index);
    // process the preds of next
    simpleIterator(
      workStack, next._pred, regionNodes, nodesInPath, cyclesFound, hdrBlock);
    // process the exception preds of next
    simpleIterator(workStack,
                   next._exceptionPred,
                   regionNodes,
                   nodesInPath,
                   cyclesFound,
...
void
TR_RegionAnalysis::addNaturalLoopNodes(StructInfo& node,
                                       WorkBitVector& regionNodes,
                                       WorkBitVector& nodesInPath,
                                       bool& cyclesFound,
                                       TR::Block* hdrBlock)
{
  ...
  while (cursor.hasMoreElements()) {
    ...
    if (_dominators.dominates(hdrBlock, next._originalBlock))
      addNaturalLoopNodes(
        next, regionNodes, nodesInPath, cyclesFound, hdrBlock);
  }
 ...
 * Create a proper acyclic region or an improper region if there are enough
 * nodes to justify it.
 */
TR_RegionStructure*
TR_RegionAnalysis::findRegion(StructInfo& node,
                              WorkBitVector& regionNodes,
                              WorkBitVector& nodesInPath)
{
  ...
  if (_useNew) {
    addRegionNodesIterativeVersion(
      node, regionNodes, nodesInPath, cyclesFound, node._originalBlock);
  } else {
    addRegionNodes(
      node, regionNodes, nodesInPath, cyclesFound, node._originalBlock);
  }
...
void
TR_RegionAnalysis::addRegionNodesIterativeVersion(StructInfo& node,
                                                  WorkBitVector& regionNodes,
                                                  WorkBitVector& nodesInPath,
                                                  bool& cyclesFound,
                                                  TR::Block* hdrBlock)
...
void
TR_RegionAnalysis::addRegionNodes(StructInfo& node,
                                  WorkBitVector& regionNodes,
                                  WorkBitVector& nodesInPath,
                                  bool& cyclesFound,
                                  TR::Block* hdrBlock)
 ...
 */
void
TR_RegionAnalysis::buildRegionSubGraph(TR_RegionStructure* region,
                                       ...
                                       TR::Region& memRegion)
{
  ...
  // Initialize the array of cfg nodes to be used to represent the region
  // subgraph.
  //
  ...
  while (rCursor.hasMoreElements()) {
    ...
    if (cfgNodes[fromIndex] == NULL)
      cfgNodes[fromIndex] =
        new (_structureRegion) TR_StructureSubGraphNode(fromNode._structure);
    ...
    while (cursor.hasMoreElements()) {
      ...
      if (cfgNodes[toIndex] == NULL) {
        if (regionNodes.get(toIndex))
          cfgNodes[toIndex] =
            new (_structureRegion) TR_StructureSubGraphNode(toNode._structure);
        else
          cfgNodes[toIndex] = new (_structureRegion) TR_StructureSubGraphNode(
            toNode._structure->getNumber(), _structureRegion);
      }
      to = cfgNodes[toIndex];
