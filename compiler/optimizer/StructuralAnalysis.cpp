...
void TR_RegionAnalysis::simpleIterator(TR_Stack<int32_t> &workStack,
  ...
  while (cursor.hasMoreElements()) {
    ...
    if (doThisCheck && next._succ.isEmpty() &&
      ...
      if (hdrBlock->getNumber() != 0) continue;
    }
...
void TR_RegionAnalysis::StructInfo::initialize(TR::Compilation *comp,
                                               int32_t index,
                                               TR::Block *block) {
...
TR_RegionStructure *TR_RegionAnalysis::findNaturalLoop(
  ...
  while (cursor.hasMoreElements()) {
    ...
    if (_dominators.dominates(node._originalBlock,
      ...
      if (_useNew) {
        addNaturalLoopNodesIterativeVersion(backEdgeNode, regionNodes,
                                            nodesInPath, cyclesFound,
                                            node._originalBlock);
      } else {
        addNaturalLoopNodes(backEdgeNode, regionNodes, nodesInPath, cyclesFound,
                            node._originalBlock);
      }
...
void TR_RegionAnalysis::addNaturalLoopNodesIterativeVersion(
  ...
  while (!workStack.isEmpty()) {
    ...
    if (nodesInPath.get(index)) {
      ...
      // it completely
      if (regionNodes.get(index) && cyclesFound) continue;
    ...
    simpleIterator(workStack, next._pred, regionNodes, nodesInPath, cyclesFound,
                   hdrBlock);
...
void TR_RegionAnalysis::addNaturalLoopNodes(StructInfo &node,
                                            WorkBitVector &regionNodes,
                                            WorkBitVector &nodesInPath,
                                            bool &cyclesFound,
                                            TR::Block *hdrBlock) {
  ...
  while (cursor.hasMoreElements()) {
    ...
    if (_dominators.dominates(hdrBlock, next._originalBlock))
      addNaturalLoopNodes(next, regionNodes, nodesInPath, cyclesFound,
                          hdrBlock);
  }
...
TR_RegionStructure *TR_RegionAnalysis::findRegion(StructInfo &node,
  ...
  if (_useNew) {
    addRegionNodesIterativeVersion(node, regionNodes, nodesInPath, cyclesFound,
                                   node._originalBlock);
  } else {
    addRegionNodes(node, regionNodes, nodesInPath, cyclesFound,
                   node._originalBlock);
  }
  if (!cyclesFound && (node._nodeIndex > 0)) {
    if (regionNodes.elementCount() < 100) return NULL;
  }
...
void TR_RegionAnalysis::addRegionNodesIterativeVersion(
  ...
  while (!workStack.isEmpty()) {
    ...
    simpleIterator(workStack, next._succ, regionNodes, nodesInPath, cyclesFound,
                   hdrBlock, true);
...
void TR_RegionAnalysis::addRegionNodes(StructInfo &node,
                                       WorkBitVector &regionNodes,
                                       WorkBitVector &nodesInPath,
                                       bool &cyclesFound, TR::Block *hdrBlock) {
  int32_t index = node._nodeIndex;
    ...
    while (cursor.hasMoreElements()) {
      ...
      if (cfgNodes[toIndex] == NULL) {
        if (regionNodes.get(toIndex))
          cfgNodes[toIndex] = new (_structureRegion)
              TR_StructureSubGraphNode(toNode._structure);
        else
