  ...
  StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath)
{
  ...
  TR_RegionStructure* region
    = new (_structureRegion) TR_RegionStructure(_compilation, node._structure->getNumber() /* node._nodeIndex */);
  if (cyclesFound) {
    if (trace())
    ...
    while (cursor.hasMoreElements()) {
      ...
      if (cfgNodes[toIndex] == NULL) {
        if (regionNodes.get(toIndex))
          ...
          cfgNodes[toIndex]
            = new (_structureRegion) TR_StructureSubGraphNode(toNode._structure->getNumber(), _structureRegion);
      }
      to = cfgNodes[toIndex];
