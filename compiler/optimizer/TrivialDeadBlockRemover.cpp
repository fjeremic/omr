...
TR_YesNoMaybe TR_TrivialDeadBlockRemover::evaluateTakeBranch(TR::Node* ifNode) {
  ...
  if (!first->getOpCode().isIntegerOrAddress() ||
      ifNode->getSize() > sizeof(ifNode->getConstValue()))
    return TR_maybe;
  ...
  static TR_YesNoMaybe decisionTable[][8] =
      //               <         >        <>         ==       <=        >=
      {
          {TR_maybe, TR_yes, TR_no, TR_yes, TR_no, TR_yes, TR_no,
           TR_maybe},  // less
          {TR_maybe, TR_no, TR_yes, TR_yes, TR_no, TR_no, TR_yes,
           TR_maybe},  // greater
          {TR_maybe, TR_no, TR_no, TR_no, TR_yes, TR_yes, TR_yes,
           TR_maybe}  // equal
      };

  int col = ifOp.isCompareTrueIfLess() * 1 + ifOp.isCompareTrueIfGreater() * 2 +
            ifOp.isCompareTrueIfEqual() * 4;
...
bool TR_TrivialDeadBlockRemover::foldIf(TR::Block* b) {
  ...
  if (!ifNode->getOpCode().isIf()) return false;
...
const char* TR_TrivialDeadBlockRemover::optDetailString() const throw() {
