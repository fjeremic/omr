...
TR_YesNoMaybe
TR_TrivialDeadBlockRemover::evaluateTakeBranch(TR::Node* ifNode)
{
  if (ifNode->getFirstChild() == ifNode->getSecondChild() &&
    ...
    if (trace())
      traceMsg(comp(),
               "An equality comparison %p folded to %d\n",
               ifNode,
               TR::ILOpCode::isEqualCmp(ifNode->getOpCodeValue()));
  ...
  if (trace())
    traceMsg(comp(),
             ...
             row,
             col,
             decisionTable[row][col]);

  return decisionTable[row][col];
}

bool
TR_TrivialDeadBlockRemover::foldIf(TR::Block* b)
{
...
int32_t
TR_TrivialDeadBlockRemover::perform()
{
