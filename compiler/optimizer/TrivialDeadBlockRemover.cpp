...
{
    if (ifNode->getFirstChild() == ifNode->getSecondChild() && !ifNode->getFirstChild()->getOpCode().isFloatingPoint() && (TR::ILOpCode::isEqualCmp(ifNode->getOpCodeValue()) || TR::ILOpCode::isNotEqualCmp(ifNode->getOpCodeValue()))) {
        if (trace())
