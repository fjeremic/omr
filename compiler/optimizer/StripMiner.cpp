...
bool TR_StripMiner::shouldPerform() {
  ...
  if (!comp()->generateArraylets()) {
    if (trace()) traceMsg(comp(), "Not enabled in non-rtj mode.\n");
    return false;
...
int32_t TR_StripMiner::perform() {
  if (trace()) traceMsg(comp(), "Processing method: %s\n", comp()->signature());
...
void TR_StripMiner::collectLoops(TR_Structure *str) {
  ...
  if (region->isNaturalLoop()) {
    ...
    //
    LoopInfo *li =
        (LoopInfo *)trMemory()->allocateStackMemory(sizeof(LoopInfo));
    li->_region = region;
    ...
    if (comp()->generateArraylets() && li->_arrayDataSize <= 0) {
      if (trace()) {
        if (li->_arrayDataSize < 0)
          traceMsg(comp(),
                   "\tReject loop %d ==> array accesses of more than one data "
                   "size\n",
                   region->getNumber());
        else
    ...
    if (trace()) {
      ...
      traceMsg(
          comp(), "\t\tarray data size = %d, step = %d need-offset-loop = %d\n",
          li->_arrayDataSize, piv->getDeltaOnBackEdge(), (li->_offset < 0));
    }
  } else {
    if (trace())
...
void TR_StripMiner::transformLoop(LoopInfo *li) {
  ...
  bool isInt32 =
      ltTree->getNode()
          ->getOpCode()
          .isBooleanCompare() &&  // isCompare only returns true for signed
                                  // compare -- a better name would be
                                  // isCompareArith()
    ...
    if ((li->_increasing && limit < limit + li->_stripLen) ||
      ...
      if (trace())
        traceMsg(
            ...
            li->_increasing ? "increasing" : "decreasing", loop->getNumber(),
            limit, li->_stripLen);
      testOverflow = false;
    }
  }
  ...
  if (testOverflow) {
    ...
    for (auto edge = ph->getPredecessors().begin();
         edge != ph->getPredecessors().end(); ++edge) {
  ...
  if (li->_preOffset > 0) {
    ...
    for (auto edge = lt->getSuccessors().begin();
         edge != lt->getSuccessors().end(); ++edge) {
...
TR::Block *TR_StripMiner::createStartOffsetLoop(LoopInfo *li,
                                                TR::Block *outerHeader) {
...
TR::Block *TR_StripMiner::stripMineLoop(LoopInfo *li, TR::Block *outerHeader) {
  ...
  TR::Node *zeroNode = isInt32 ? TR::Node::iconst(mainPhNode, 0)
                               : TR::Node::lconst(mainPhNode, 0);
  ...
  TR::Node *newConst =
      isInt32
          ? TR::Node::iconst(phNode, li->_piv->getDeltaOnBackEdge())
    ...
    if ((block != lt) && (block->getNumber() < _nodesInCFG)) {
      for (exitEdge = eItExit.getFirst(); exitEdge;
        ...
        if (exitEdge->getFrom()->getNumber() == block->getNumber()) {
          ...
          TR::Node *gotoNode = TR::Node::create(dest->getEntry()->getNode(),
                                                TR::Goto, 0, dest->getEntry());
...
void TR_StripMiner::redirect(TR::Block *source, TR::Block *oldDest,
                             TR::Block *newDest) {
...
//
void TR_StripMiner::examineLoop(LoopInfo *li, TR_ClonedLoopType type,
                                bool checkClone) {
  if (trace())
  ...
  for (block = bIt.getFirst(); block; block = bIt.getNext()) {
    ...
    while (currentTree != exitTree) {
      ...
      if (node->getNumChildren() > 0)
        examineNode(li, node, node, oldCounter, visitCount, type, checkClone,
                    -1);
...
void TR_StripMiner::examineNode(LoopInfo *li, TR::Node *parent, TR::Node *node,
                                ...
                                vcount_t visitCount, TR_ClonedLoopType type,
  ...
  if (checkClone && node->getSymbolReference() == oldSymRef) {
    if (node->getOpCode().isLoad()) {
      ...
      if (foundLoad) {
        if (trace())
          traceMsg(comp(),
                   "      adding node [%p] to load list parent: [%p], "
                   "childNum: %d\n",
      ...
      type == mainLoop
          ...
          : li->_residualParentsOfStores.add(
                new (trStackMemory()) TR_ParentOfChildNode(parent, -1));
    }
  }
  ...
  if (!checkClone) {
    ...
    if (symbol &&
        (symbol
             ->isArrayShadowSymbol()))  //|| symbol->isArrayletShadowSymbol()))
    {
...
void TR_StripMiner::replaceLoopPivs(LoopInfo *li, TR::ILOpCodes newOpCode,
                                    TR::Node *newConst,
  ...
  for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
...

    {
      ...
      if (loadNode == NULL) {
        ...
        loadPair = new (comp()->trStackMemory()) TR_Pair<TR::Node, TR::Node>(
            parentNode->getChild(childNum), loadNode);
    ...
    if ((parentNode->getOpCode().isRightShift() ||
      ...
      if (trace())
        traceMsg(comp(),
                 "      replacing load [%p] with new load [%p]: parent [%p], "
                 "childNum %d\n",
                 parentNode->getChild(childNum), loadNode, parentNode,
                 childNum);
      parentNode->getChild(childNum)->decReferenceCount();
...
TR::Block *TR_StripMiner::getLoopPreHeader(TR_Structure *str) {
  ...
  for (auto e = headerBlock->getPredecessors().begin();
       e != headerBlock->getPredecessors().end(); ++e) {
...
bool TR_StripMiner::findPivInSimpleForm(TR::Node *node,
                                        TR::SymbolReference *pivSym) {
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    if (findPivInSimpleForm(node->getChild(i), pivSym)) return true;
  }

  return false;
}

const char *TR_StripMiner::optDetailString() const throw() {
