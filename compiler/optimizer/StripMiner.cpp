...
void TR_StripMiner::collectLoops(TR_Structure* str)
{
    ...
    if (region->isNaturalLoop()) {
       ...
       */
        ...
        if ((li->_offset == -1) || ((li->_increasing && ((li->_offset & (li->_stripLen - 1)) != 0)) || (!li->_increasing && (li->_offset & (li->_stripLen - 1)) != li->_stripLen - 1))) {
            if (trace())
...
void TR_StripMiner::duplicateLoop(LoopInfo* li, TR_ClonedLoopType type)
{
    ...
    case offsetLoop:
        ...
        // an arraylet
        //
        if (!comp()->generateArraylets() || ((li->_offset != -1) && ((li->_increasing && ((li->_offset & (li->_stripLen - 1)) == 0)) || (!li->_increasing && (li->_offset & (li->_stripLen - 1)) == li->_stripLen - 1))))
...
void TR_StripMiner::transformLoop(LoopInfo* li)
{
    ...
    if (limitNode->getOpCode().isLoadConst()) {
        intptrj_t limit = limitNode->getType().isInt32() ? limitNode->getInt() : limitNode->getLongInt();
...
TR::Block* TR_StripMiner::createStartOffsetLoop(LoopInfo* li, TR::Block* outerHeader)
{
    ...
    TR::Node* stripLenNode = isInt32 ? TR::Node::iconst(newLtNode, li->_stripLen - 1) : TR::Node::lconst(newLtNode, (int64_t)(li->_stripLen - 1));
...
TR::Block* TR_StripMiner::stripMineLoop(LoopInfo* li, TR::Block* outerHeader)
{
    ...
    */
    ...
    TR::Node* constNode = isInt32 ? TR::Node::iconst(phNode, (int32_t)(constVal - 1)) : TR::Node::lconst(phNode, constVal - 1);
    ...
    if (TR::ILOpCode::isNotEqualCmp(ltTree->getNode()->getOpCodeValue()) || TR::ILOpCode::isEqualCmp(ltTree->getNode()->getOpCodeValue())) {
        ...
        cmpOpCode = li->_branchToExit ? ltTree->getNode()->getOpCode().getOpCodeForReverseBranch() : ltTree->getNode()->getOpCodeValue();
    ...
    TR::Node* newConst = isInt32 ? TR::Node::iconst(phNode, li->_piv->getDeltaOnBackEdge()) : TR::Node::lconst(phNode, (int64_t)li->_piv->getDeltaOnBackEdge());
        ...
        if ((block != lt) && (block->getNumber() < _nodesInCFG)) {
            for (exitEdge = eItExit.getFirst(); exitEdge; exitEdge = eItExit.getNext()) {
                if (exitEdge->getFrom()->getNumber() == block->getNumber()) {
                    ...
                    freq = (block->getFrequency() < dest->getFrequency()) ? block->getFrequency() : dest->getFrequency();
...
TR::Block* TR_StripMiner::createGotoBlock(TR::Block* source, TR::Block* dest)
{
    ...
    intptrj_t freq = (source->getFrequency() < dest->getFrequency()) ? source->getFrequency() : dest->getFrequency();
...
TR::Block* TR_StripMiner::createLoopTest(LoopInfo* li, TR_ClonedLoopType type)
{
    ...
    TR::SymbolReference* tempSymRef = comp()->getSymRefTab()->createTemporary(
        comp()->getMethodSymbol(), isInt32 ? TR::Int32 : TR::Int64);
    TR::Node* iNode = TR::Node::createLoad(phNode, li->_piv->getSymRef());
    TR::Node* offsetNode = isInt32 ? TR::Node::iconst(clonedPhNode, offset) : TR::Node::lconst(clonedPhNode, (int64_t)offset);
...
void TR_StripMiner::examineLoop(LoopInfo* li, TR_ClonedLoopType type, bool checkClone)
{
    ...
    for (block = bIt.getFirst(); block; block = bIt.getNext()) {
        ...
        while (currentTree != exitTree) {
            ...
            if (checkClone && li->_canMoveAsyncCheck && node->getOpCodeValue() == TR::asynccheck && li->_asyncTree == NULL) {
                if (trace())
    ...
    if (checkClone && node->getSymbolReference() == oldSymRef) {
        if (node->getOpCode().isLoad()) {
            ...
            if (foundLoad) {
                ...
                type == mainLoop ? li->_mainParentsOfLoads.add(new (trStackMemory()) TR_ParentOfChildNode(parent, childNum)) : li->_residualParentsOfLoads.add(new (trStackMemory()) TR_ParentOfChildNode(parent, childNum));
            }
        } else {
            if (trace())
                traceMsg(comp(), "      adding node [%p] store list parent: [%p]\n", node, parent);

            type == mainLoop ? li->_mainParentsOfStores.add(new (trStackMemory()) TR_ParentOfChildNode(parent, -1)) : li->_residualParentsOfStores.add(new (trStackMemory()) TR_ParentOfChildNode(parent, -1));
        }
    }
                ...
                if (node->getOpCodeValue() == TR::aloadi) {
                    if (node->getFirstChild()->getOpCode().isArrayRef()) {
                        ...
                        if (secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub()) {
                            if (secondChild->getFirstChild()->getOpCode().isMul() || secondChild->getFirstChild()->getOpCode().isLeftShift()) {
                                ...
                                if (firstChild->getOpCode().isRightShift() || firstChild->getOpCode().isDiv()) {
                                    ...
                                    if ((accessNode->getOpCodeValue() == TR::iload) && (accessNode->getSymbolReference() == li->_piv->getSymRef())) {
                                        foundArrayAccess = true;
                                        if (trace())
                                            traceMsg(comp(), "Node %p accesses array with no pre/post offset\n", node);
                                    } else {
                                        if (accessNode->getOpCodeValue() != TR::iload) {
                                            if ((accessNode->getOpCode().isAdd() || accessNode->getOpCode().isSub()) && accessNode->getFirstChild()->getSymbolReference() == oldSymRef && accessNode->getSecondChild()->getOpCodeValue() == TR::iconst) {
                                                foundArrayAccess = true;
                                                intptrj_t offset = accessNode->getOpCodeValue() == TR::iadd ? accessNode->getSecondChild()->getInt() : accessNode->getSecondChild()->getInt() * -1;
                                                if (li->_increasing) {
                                                    if (offset < 0)
    ...
    TR::SymbolReference* newSymRef, TR_ClonedLoopType type)
{
    ...
    for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
        ...
        bool skipNode = false;
        intptrj_t value;
        if ((parentNode->getOpCode().isRightShift() || parentNode->getOpCode().isDiv()) && parentNode->getSecondChild()->getOpCode().isLoadConst()) {
            value = (parentNode->getSecondChild()->getType().isInt32()) ? parentNode->getSecondChild()->getInt() : parentNode->getSecondChild()->getLongInt();
...
TR::Block* TR_StripMiner::getLoopTest(TR_Structure* str, TR::Block* preHeader)
{
    ...
    //
    if (loopTest) {
        if (TR::ILOpCode::isNotEqualCmp(loopTest->getLastRealTreeTop()->getNode()->getOpCodeValue()) || TR::ILOpCode::isEqualCmp(loopTest->getLastRealTreeTop()->getNode()->getOpCodeValue())) {
            if (trace())
    ...
    for (block = bIt.getFirst(); block; block = bIt.getNext()) {
        ...
        while (currentTree != exitTree) {
            ...
            if (node->getOpCode().isStore() && node->getSymbolReference() == oldCounter) {
                if (((node->getFirstChild()->getOpCode().isSub()) || (node->getFirstChild()->getOpCode().isAdd())) && node->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
