...
TR_StripMiner::shouldPerform()
{
  ...
  if (!comp()->generateArraylets()) {
    ...
    if (trace())
      traceMsg(comp(),
               "Option is not enabled -- returning from strip mining.\n");
    return false;
  }
  ...
  if (!comp()->mayHaveLoops()) {
    if (trace())
      traceMsg(comp(),
               "Method does not have loops -- returning from strip mining.\n");
    return false;
  }
...
void
TR_StripMiner::collectLoops(TR_Structure* str)
{
  ...
  if (region->isNaturalLoop()) {
    ...
    if (preHeader == NULL) {
      if (trace())
        traceMsg(
          comp(), "\tReject loop %d ==> no pre-header\n", region->getNumber());
      return;
    }
    ...
    if (loopTest == NULL) {
      if (trace())
        traceMsg(comp(),
                 "\tReject loop %d ==> no loop test block\n",
                 region->getNumber());
      return;
    }
    ...
    if (piv == NULL) {
      if (trace())
        traceMsg(comp(),
                 "\tReject loop %d ==> no primary induction variable\n",
                 region->getNumber());
      return;
    }
    ...
    // Check to see if there is only 1 back edge (the other edge is from
    // pre-header
    //
    if (region->getEntryBlock()->getPredecessors().size() != 2) {
      if (trace())
        traceMsg(comp(),
                 "\tReject loop %d ==> more than 1 back edge\n",
                 region->getNumber());
      return;
    }
    ...
    for (TR::Block* b = bIt.getFirst(); b; b = bIt.getNext()) {
      if (b->hasExceptionPredecessors()) // catch
      {
        if (trace())
          traceMsg(comp(),
                   "\tReject loop %d ==> block_%d has exception predecessors\n",
                   region->getNumber(),
                   b->getNumber());
        return;
      }
      if (b->hasExceptionSuccessors()) // try
      {
        if (trace())
          traceMsg(comp(),
                   "\tReject loop %d ==> block_%d has exception successors\n",
                   region->getNumber(),
                   b->getNumber());
        return;
      }
    ...
    // Reject if the stores are incremental. i.e. i=i+1; i=i+1... In that case
    // the deltaOnBackEdge is 2.
    // It is ok in case where if the stores are in different paths of the code.
    // Then the value is equal to the deltaOnBackEdge
    // Looking only on Main loop

    if (checkIfIncrementalIncreasesOfPIV(li)) {
      if (trace())
        traceMsg(comp(),
                 "\tReject loop %d ==> multiple store of induction variable "
                 "were found\n",
                 region->getNumber());
      return;
    }
    ...
    if (comp()->generateArraylets() && li->_arrayDataSize <= 0) {
      if (trace()) {
        if (li->_arrayDataSize < 0)
          traceMsg(
            comp(),
            "\tReject loop %d ==> array accesses of more than one data size\n",
            region->getNumber());
        else
          traceMsg(comp(),
                   "\tReject loop %d ==> no array accesses found\n",
                   region->getNumber());
      }
      return;
    }
    ...
    //
    // If entry value is known, we need to be at the start of the leaf for
    // increasing loops,
    // and for end of the leaf for decreasing loops so that no offset loop will
    // be needed.
    // If no entry value is known (-1) then we need offset loop.
    if ((li->_offset == -1) ||
        ((li->_increasing && ((li->_offset & (li->_stripLen - 1)) != 0)) ||
         (!li->_increasing &&
          (li->_offset & (li->_stripLen - 1)) != li->_stripLen - 1))) {
      if (trace())
        traceMsg(comp(),
                 "\tReject loop %d ==> needs a offsetLoop - cannot deal with "
                 "this now\n",
                 region->getNumber());
      // printf("reject %s: need offset loop
    ...
    if (trace())
      traceMsg(comp(),
               "\titerationCount = %d stripLength = %d\n",
               iterCount,
               li->_stripLen);
    if (iterCount != -1) {
      if (iterCount < li->_stripLen) {
        if (trace())
          traceMsg(comp(),
                   "\tReject loop %d ==> iteration count is less than the "
                   "strip length\n",
                   region->getNumber());
        return;
      }
    ...
    if (!li->_canMoveAsyncCheck) {
      if (trace())
        traceMsg(
          comp(), "\tReject loop %d ==> calls present\n", region->getNumber());
      return;
    }
    ...
    if (trace()) {
      traceMsg(comp(),
               "\tSuccess => adding candidate loop %d to the queue\n",
               region->getNumber());
      traceMsg(comp(),
               "\t\tpre-header = %d, loop test = %d, primary induction "
               "variable = %d\n",
               preHeader->getNumber(),
               loopTest->getNumber(),
               piv->getSymRef()->getReferenceNumber());
      traceMsg(comp(),
               "\t\tpre-offset = %d, post-offset = %d, offset = %d, strip "
               "length = %d\n",
               li->_preOffset,
               li->_postOffset,
               li->_offset,
               li->_stripLen);
      traceMsg(comp(),
               "\t\tarray data size = %d, step = %d need-offset-loop = %d\n",
               li->_arrayDataSize,
    ...
    if (trace())
      traceMsg(comp(),
               "\tReject region %d ==> not a natural loop\n",
               region->getNumber());
  }
}
...
TR_StripMiner::transformLoops()
{
  ...
  for (LoopInfo* li = it.getFirst(); li; li = it.getNext()) {
    if (li != NULL &&
      ...
      // printf("\n--Strip Mining in -- %s start %d post %d pre %d increasing
      // %d\n", comp()->signature(), li->_offset, li->_postOffset,
      // li->_preOffset, li->_increasing);
    }
  }
}

void
TR_StripMiner::duplicateLoop(LoopInfo* li, TR_ClonedLoopType type)
{
  ...
  switch (type) {
    case offsetLoop:
      ...
      if (trace())
        traceMsg(
          comp(), "\tcreating start offset loop: loop %d...\n", li->_regionNum);
    ...
    case residualLoop:
      ...
      if (trace())
        traceMsg(
          comp(), "\tcreating residual loop: loop %d...\n", li->_regionNum);
      break;
    default:
      if (trace())
        traceMsg(comp(),
                 "\tcannot recognize loop type: %d, abort duplication...\n",
                 type);
      return;
  }
  ...
  if (pEnv && (type == offsetLoop && li->_needOffsetLoop)) {
    traceMsg(comp(),
             "loop %d needs a start offset loop %d\n",
             li->_regionNum,
             li->_needOffsetLoop);
    li->_needOffsetLoop = false;
    return;
    ...
    if (block->getNumber() < _nodesInCFG) {
      ...
      /*if (trace())
         ...
         traceMsg(comp(), "Cloned block (%d): %d\n",
               block->getNumber(),
         blockMapper[block->getNumber()]->getNumber());
         }
       */
    }
  }
  ...
  for (block = bIt.getFirst(); block; block = bIt.getNext()) {
    ...
    if (trace())
      traceMsg(comp(),
               "\tprocessing edges for: block [%d] => cloned block [%d]\n",
               block->getNumber(),
    ...
    if (block == li->_loopTest && !isMainOrResidual) {
      if (trace())
        traceMsg(
          comp(),
          "\tskipping edge from non-main non-residual loop test block [%d]\n",
          block->getNumber());
      continue;
    }
...
void
TR_StripMiner::transformLoop(LoopInfo* li)
{
  TR_RegionStructure* loop = li->_region;
  ...
  bool isInt32 =
    ltTree->getNode()
      ->getOpCode()
      .isBooleanCompare() && // isCompare only returns true for signed compare
                             // -- a better name would be isCompareArith()
  ...
  if (limitNode->getOpCode().isLoadConst()) {
    intptrj_t limit = limitNode->getType().isInt32() ? limitNode->getInt()
                                                     : limitNode->getLongInt();
    if ((li->_increasing && limit < limit + li->_stripLen) ||
        (!li->_increasing && limit > limit - li->_stripLen)) {
      if (trace())
        traceMsg(
          comp(),
          "No overflow test for %s loop %d: limit = %ld, stripLength = %ld\n",
  ...
  // N > TR::getMaxSigned<TR::Int32>() - stripLength or N <
  // TR::getMinSigned<TR::Int32>() + stripLength
  TR::Block* ifOverBlock = NULL;
  if (testOverflow) {
    ifOverBlock =
      TR::Block::createEmptyBlock(phNode, comp(), ph->getFrequency(), ph);
    TR::ILOpCodes opCode;
    ...
    TR::Node* ifOverNode = TR::Node::createif(
      opCode, limitNode->duplicateTree(), overflowNode, phEntry);
    ...
    if (trace())
      traceMsg(comp(),
               "\tcreating overflow test [%p] in block [%d]\n",
               ifOverNode,
               ifOverBlock->getNumber());

    // Point the original loop pre-header's predecessor to the if over block
    if (trace())
      traceMsg(
        comp(),
        "\tfixing predecessor edges of original loop pre-header [%d] :\n",
        ph->getNumber());

    for (auto edge = ph->getPredecessors().begin();
         edge != ph->getPredecessors().end();
         ++edge) {
    ...
    if (trace())
      traceMsg(comp(),
               "\t   adding edge: overflow test block [%d] => original "
               "pre-header [%d]\n",
               ifOverBlock->getNumber(),
               ph->getNumber());
  ...
  //
  TR::Block* outerHeader = TR::Block::createEmptyBlock(
    clonedPhNode, comp(), clonedPh->getFrequency(), clonedPh);
  ...
  if (li->_needOffsetLoop) {
    ...
    if (testOverflow) {
      ...
      if (trace())
        traceMsg(
          comp(),
          "\tfixing edges: preds of pre-header [%d] -> offset loop [%d]\n",
          ph->getNumber(),
          offsetLoopPh->getNumber());
    ...
    if (testOverflow) {
      ...
      if (trace())
        traceMsg(comp(),
                 "\t   adding edge: overflow test block [%d] => outer "
                 "pre-header [%d]\n",
                 ifOverBlock->getNumber(),
      ...
      if (trace())
        traceMsg(
          comp(),
          "\tfixing edges: preds of pre-header [%d] -> outer header [%d]\n",
          ph->getNumber(),
          outerHeader->getNumber());
  ...
  if (trace())
    traceMsg(comp(),
             "<stripMining loop=%d outer header=%d>\n",
             li->_regionNum,
             outerHeader->getNumber());
  ...
  if (li->_preOffset > 0) {
    ...
    for (auto edge = lt->getSuccessors().begin();
         edge != lt->getSuccessors().end();
         ++edge) {
  ...
  /* If there is a post-offset, redirect cloned loop's exit edges to the
   * postloop pre-header */
  if (li->_postOffset > 0) {
...
TR::Block*
TR_StripMiner::createStartOffsetLoop(LoopInfo* li, TR::Block* outerHeader)
{
  ...
  TR::Block* newLtBlock = TR::Block::createEmptyBlock(
    ltNode, comp(), ltBlock->getFrequency(), ltBlock);
  ...
  TR::Node* stripLenNode =
    isInt32 ? TR::Node::iconst(newLtNode, li->_stripLen - 1)
  ...
  if (trace())
    traceMsg(comp(),
             "\t   adding edge: test block [%d] => exit [%d]\n",
             ltBlock->getNumber(),
             origExit->getNumber());
  ///_cfg->addEdge(ltBlock, origExit);
  redirect(ltBlock, li->_branchToExit ? origExit : NULL, origExit);

  for (auto e = ltBlock->getSuccessors().begin();
       e != ltBlock->getSuccessors().end();
       ++e)
  ...
  if (trace())
    traceMsg(comp(),
             "\t   adding edge: new test block [%d] => outer pre-header [%d]\n",
  ...
  if (trace())
    traceMsg(
      comp(),
      "\t created a new block [%d] to position at arraylet with test [%p]\n",
      newLtBlock->getNumber(),
      newLtNode);
  return newLtBlock;
}
...
TR::Block*
TR_StripMiner::stripMineLoop(LoopInfo* li, TR::Block* outerHeader)
{
  TR_RegionStructure* loop = li->_region;
  TR::Block* ph = li->_preHeader;
  TR::TreeTop* phEntry = ph->getEntry();
  ...
  TR::Node* constNode = isInt32
                          ? TR::Node::iconst(phNode, (int32_t)(constVal - 1))
                          : TR::Node::lconst(phNode, constVal - 1);

  TR::ILOpCodes updateOpCode = li->_increasing
                                 ? (isInt32 ? TR::iadd : TR::ladd)
                                 : (isInt32 ? TR::isub : TR::lsub);
  ...
  TR::Node* ifNode =
    TR::Node::createif(cmpOpCode, iAddNode, limitNode, mainPhEntry);
  ...
  if (trace())
    traceMsg(comp(),
             "\tcreating striplength test [%p] in outer loop block [%d]\n",
             ifNode,
             branchBlock->getNumber());
  ...
  if (trace())
    traceMsg(comp(),
             "\t   adding edge: branch [%d] => main pre-header [%d]\n",
             branchBlock->getNumber(),
             mainPh->getNumber());
  ...
  TR::Node* zeroNode =
    isInt32 ? TR::Node::iconst(mainPhNode, 0) : TR::Node::lconst(mainPhNode, 0);
  ...
  TR::Node* zeroNodeResid = isInt32 ? TR::Node::iconst(residualPhNode, 0)
                                    : TR::Node::lconst(residualPhNode, 0);
  ...
  TR::Node* newLoopLimit =
    cmpTree->getNode()->getSecondChild()->duplicateTree();
  ...
  if (trace())
    traceMsg(
      comp(),
      "\t   created a new block [%d] to store i = i + j and test if i < N\n",
  ...
  for (auto edge = lt->getSuccessors().begin();
       edge != lt->getSuccessors().end();
       ++edge) {
    TR::Block* fromMainBlock =
      _mainBlockMapper[(*edge)->getFrom()->getNumber()];
    TR::Block* fromResidualBlock =
      _residualBlockMapper[(*edge)->getFrom()->getNumber()];
    TR::Block* dest = toBlock((*edge)->getTo());
    ...
    if (clonedMainDest) {
      ...
      TR::Block* newDest =
        li->_postOffset > 0 ? _postBlockMapper[ph->getNumber()] : outerHeader;
  ...
  if (trace())
    traceMsg(
      comp(),
      "\t   changed main loop test [%p] to j < (strip length - pre - post)\n",
      mainLtTree->getNode());

  /* For both main and residual loops, replace old induction variable in the
   * inner loop */
  traceMsg(comp(),
           "\t   replacing original induction variable symRef [%d]\n",
           li->_piv->getSymRef()->getReferenceNumber());
  examineLoop(li, mainLoop, true);
  examineLoop(li, residualLoop, true);

  TR::Node* newConst =
    isInt32 ? TR::Node::iconst(phNode, li->_piv->getDeltaOnBackEdge())
  ...
  if (!li->_residualParentsOfLoads.isEmpty() ||
      !li->_residualParentsOfStores.isEmpty())
  ...
  if (trace())
    traceMsg(comp(),
             "\t   changed residual loop test [%p] to j < (N - i)\n",
  ...
  for (block = bIt.getFirst(); block; block = bIt.getNext()) {
    if ((block != lt) && (block->getNumber() < _nodesInCFG)) {
      for (exitEdge = eItExit.getFirst(); exitEdge;
           exitEdge = eItExit.getNext()) {
        if (exitEdge->getFrom()->getNumber() == block->getNumber()) {
          ...
          TR::Block* updateBeforeExitBlock = TR::Block::createEmptyBlock(
            dest->getEntry()->getNode(), comp(), freq, block);
          ...
          TR::TreeTop* iStoreTree = TR::TreeTop::create(
            comp(), TR::Node::createStore(li->_piv->getSymRef(), iAddNode));
          updateBeforeExitBlock->prepend(iStoreTree);
          TR::Node* gotoNode = TR::Node::create(
            dest->getEntry()->getNode(), TR::Goto, 0, dest->getEntry());
  ...
  if (li->_canMoveAsyncCheck) {
    if (li->_asyncTree != NULL) {
      if (trace())
        traceMsg(comp(),
                 "\t   moved asynccheck tree [%p] to block [%d]\n",
                 li->_asyncTree,
                 outerHeader->getNumber());
...
TR::Block*
TR_StripMiner::createGotoBlock(TR::Block* source, TR::Block* dest)
{
...
void
TR_StripMiner::redirect(TR::Block* source,
                        TR::Block* oldDest,
                        TR::Block* newDest)
{
  ...
  if (branchNode->getOpCode().isSwitch()) {
    for (intptrj_t i = branchNode->getCaseIndexUpperBound() - 1; i > 0; --i) {
      if (branchNode->getChild(i)
            ->getBranchDestination()
            ->getNode()
            ->getBlock() == oldDest) {
        if (trace())
          traceMsg(comp(),
                   "\t      fixing switch statement: [%d] => [%d]\n",
                   source->getNumber(),
                   newDest->getNumber());
    ...
    if (trace())
      traceMsg(comp(),
               "\t      fixing branch/goto statement: [%d] => [%d]\n",
               source->getNumber(),
               newDest->getNumber());
    branchNode->setBranchDestination(newDest->getEntry());
    found = true;
  } else if (branchNode->getOpCode().isBranch() &&
             source->getNextBlock() == newDest) {

    if (trace())
      traceMsg(comp(),
               "\t      skipping edge: [%d] => [%d], already exist\n",
               source->getNumber(),
               newDest->getNumber());
    found = true;
  }
  ...
  if (found) {
    ...
    if (trace()) {
      traceMsg(comp(),
               "\t      adding edges: source [%d] => goto [%d]; goto [%d] => "
               "new dest [%d]\n",
               source->getNumber(),
               gotoBlock->getNumber(),
               gotoBlock->getNumber(),
...
TR::Block*
TR_StripMiner::createLoopTest(LoopInfo* li, TR_ClonedLoopType type)
{
  TR::Block* ph = li->_preHeader;
  TR::TreeTop* phEntry = ph->getEntry();
  TR::Node* phNode = phEntry->getNode();
  TR::Block* lt = li->_loopTest;
  TR::TreeTop* ltTree = lt->getLastRealTreeTop();
  TR::Block** blockMapper =
    type == preLoop ? _preBlockMapper : _postBlockMapper;
  ...
  if (trace())
    traceMsg(comp(),
             "\t Adding temp = i + %s-offset in block [%d]\n",
  ...
  TR::SymbolReference* tempSymRef = comp()->getSymRefTab()->createTemporary(
    comp()->getMethodSymbol(), isInt32 ? TR::Int32 : TR::Int64);
  TR::Node* iNode = TR::Node::createLoad(phNode, li->_piv->getSymRef());
  TR::Node* offsetNode = isInt32
                           ? TR::Node::iconst(clonedPhNode, offset)
                           : TR::Node::lconst(clonedPhNode, (int64_t)offset);
  TR::ILOpCodes opCode = li->_increasing ? (isInt32 ? TR::iadd : TR::ladd)
                                         : (isInt32 ? TR::isub : TR::lsub);
  TR::Node* storeNode = TR::Node::createStore(
    tempSymRef, TR::Node::create(opCode, 2, iNode, offsetNode));
  ...
  if (trace())
    traceMsg(comp(),
             "\t created a new block [%d] to test if i < temp [%p]\n",
             ifBlock->getNumber(),
             ifTree->getNode());
...
//
void
TR_StripMiner::examineLoop(LoopInfo* li,
                           ...
                           bool checkClone)
{
  ...
  for (block = bIt.getFirst(); block; block = bIt.getNext()) {
    if (checkClone)
      block = type == mainLoop ? _mainBlockMapper[block->getNumber()]
                               : _residualBlockMapper[block->getNumber()];
    ...
    while (currentTree != exitTree) {
      ...
      if (checkClone && li->_canMoveAsyncCheck &&
          node->getOpCodeValue() == TR::asynccheck && li->_asyncTree == NULL) {
        if (trace())
          traceMsg(comp(),
                   "      found asynccheck [%p] in block [%d]\n",
                   currentTree,
                   block->getNumber());
      ...
      if (node->getNumChildren() > 0)
        examineNode(
          li, node, node, oldCounter, visitCount, type, checkClone, -1);
...
void
TR_StripMiner::examineNode(LoopInfo* li,
                           TR::Node* parent,
                           TR::Node* node,
    ...
    if (node->getOpCode().isLoad()) {
      ...
      if (foundLoad) {
        if (trace())
          traceMsg(
            comp(),
            "      adding node [%p] to load list parent: [%p], childNum: %d\n",
            node,
            parent,
            childNum);

        type == mainLoop
          ? li->_mainParentsOfLoads.add(
              new (trStackMemory()) TR_ParentOfChildNode(parent, childNum))
          : li->_residualParentsOfLoads.add(
              new (trStackMemory()) TR_ParentOfChildNode(parent, childNum));
      }
    } else {
      if (trace())
        traceMsg(comp(),
                 "      adding node [%p] store list parent: [%p]\n",
                 node,
                 parent);

      type == mainLoop
        ? li->_mainParentsOfStores.add(new (trStackMemory())
                                         TR_ParentOfChildNode(parent, -1))
        : li->_residualParentsOfStores.add(new (trStackMemory())
                                             TR_ParentOfChildNode(parent, -1));
    }
  }
           ...
           iaload <arraylet>                                iaload <arraylet>
              aiadd                                            aladd
                 ...
                 isub/iadd                                        lsub/ladd
                    imul/ishl                                        lmul/lshl
                       ishr/idiv                                        i2l <--
           optional iload/iadd/isub                                  ... iconst
                       iconst
                    iconst
         */
          ...
          if (node->getFirstChild()->getOpCode().isArrayRef()) {
            ...
            if (secondChild->getOpCode().isAdd() ||
              ...
              if (secondChild->getFirstChild()->getOpCode().isMul() ||
                ...
                TR::Node* firstChild =
                  secondChild->getFirstChild()->getFirstChild();
                ...
                if (firstChild->getOpCode().isRightShift() ||
                  ...
                  if ((accessNode->getOpCodeValue() == TR::iload) &&
                      (accessNode->getSymbolReference() ==
                       li->_piv->getSymRef())) {
                    foundArrayAccess = true;
                    if (trace())
                      traceMsg(
                        comp(),
                        "Node %p accesses array with no pre/post offset\n",
                        node);
                  } else {
                    if (accessNode->getOpCodeValue() != TR::iload) {
                      if ((accessNode->getOpCode().isAdd() ||
                           accessNode->getOpCode().isSub()) &&
                          accessNode->getFirstChild()->getSymbolReference() ==
                            oldSymRef &&
                          accessNode->getSecondChild()->getOpCodeValue() ==
                            TR::iconst) {
                        foundArrayAccess = true;
                        intptrj_t offset =
                          accessNode->getOpCodeValue() == TR::iadd
                            ? accessNode->getSecondChild()->getInt()
                            : accessNode->getSecondChild()->getInt() * -1;
                        if (li->_increasing) {
                          if (offset < 0)
                            li->_preOffset =
                              std::max(li->_preOffset, -1 * offset);
                          else if (offset > 0)
                          ...
                          if (offset > 0)
                            ...
                            li->_postOffset =
                              std::max(li->_postOffset, -1 * offset);
                        }
                        if (trace())
                          traceMsg(
                            comp(),
                            "Node %p has pre-offset: %d, post-offset: %d\n",
                            node,
                            li->_preOffset,
                            li->_postOffset);
                      } else {
                        bool foundPiv =
                          findPivInSimpleForm(accessNode, oldSymRef);
                        if (foundPiv) {
                          foundArrayAccess = true;
                          li->_preOffset = 0;
                          li->_postOffset = 0;
                    ...
                    else // if iload
                    {
                      if (trace())
                        traceMsg(
                          comp(),
                          "Found iload %p which is not piv ... skipping it\n",
                          node);
                    }
                  }
                } else {
                  if (trace())
                    traceMsg(comp(),
                             "Node %p does not have an ishr/idiv tree\n",
                             node);
                }
              }
            }
...
TR::Node*
findIndexChild(TR::Node* node, TR::SymbolReference* newSymRef)
{
...
void
TR_StripMiner::replaceLoopPivs(LoopInfo* li,
                               ...
                               TR_ClonedLoopType type)
{
  ...
  if (trace())
    traceMsg(comp(),
             "   replacing Pivs in %s loop %d...\n",
             type == mainLoop ? "main" : "residual",
             li->_regionNum);

  /* Process list of loads */
  type == mainLoop ? it.set(&li->_mainParentsOfLoads)
                   : it.set(&li->_residualParentsOfLoads);
  ...
  for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
    ...
    bool skipNode = false;
    intptrj_t value;
    if ((parentNode->getOpCode().isRightShift() ||
         parentNode->getOpCode().isDiv()) &&
        parentNode->getSecondChild()->getOpCode().isLoadConst()) {
      value = (parentNode->getSecondChild()->getType().isInt32())
                ? parentNode->getSecondChild()->getInt()
...

    {
      ...
      for (loadPair = lit.getFirst(); loadPair != NULL;
           loadPair = lit.getNext()) {
    ...
    if ((parentNode->getOpCode().isRightShift() ||
      ...
      if (trace())
        traceMsg(comp(),
                 "      replacing load [%p] with new load [%p]: parent [%p], "
                 "childNum %d\n",
                 parentNode->getChild(childNum),
                 iNode,
                 parentNode,
                 childNum);
      ...
      if (trace())
        traceMsg(comp(),
                 "    changing and node parent  [%p] with new load [%p]\n",
                 parentNode,
  ...
  type == mainLoop ? it.set(&li->_mainParentsOfStores)
                   : it.set(&li->_residualParentsOfStores);
  for (parent = it.getFirst(); parent != NULL; parent = it.getNext()) {
    ...
    for (storePair = sit.getFirst(); storePair != NULL;
         storePair = sit.getNext()) {
    ...
    if (storeNode == NULL) {
      ...
      storeNode =
        TR::Node::create(newOpCode, 2, index, newConst->duplicateTree());
      storePair = new (comp()->trStackMemory())
        TR_Pair<TR::Node, TR::Node>(node, storeNode);
    ...
    if (trace())
      traceMsg(
        comp(),
        "      replacing store [%p] with new store [%p]: new symref [%d]\n",
        node,
        storeNode,
...
TR::Block*
TR_StripMiner::getLoopPreHeader(TR_Structure* str)
{
  TR_RegionStructure* region = str->asRegion();
  TR::Block* headerBlock = region->getEntryBlock();

  TR::Block* pBlock = NULL;
  for (auto e = headerBlock->getPredecessors().begin();
       e != headerBlock->getPredecessors().end();
       ++e) {
...
TR::Block*
TR_StripMiner::getLoopTest(TR_Structure* str, TR::Block* preHeader)
{
  TR_RegionStructure* region = str->asRegion();
  TR::Block* headerBlock = region->getEntryBlock();
  ...
  //
  if (loopTest && !loopTest->getLastRealTreeTop()
                     ->getNode()
                     ->getOpCode()
                     .isBooleanCompare()) {
    if (trace())
      traceMsg(comp(),
               "loop %d: no loop test found on backedge\n",
               region->getNumber());
    loopTest = NULL;
  }
  ...
  //
  if (loopTest) {
    if (TR::ILOpCode::isNotEqualCmp(
          loopTest->getLastRealTreeTop()->getNode()->getOpCodeValue()) ||
        TR::ILOpCode::isEqualCmp(
          loopTest->getLastRealTreeTop()->getNode()->getOpCodeValue())) {
      if (trace())
        traceMsg(comp(),
                 "loop %d: found loop with eq/ne test condition\n",
                 region->getNumber());
      loopTest = NULL;
...
bool
TR_StripMiner::checkIfIncrementalIncreasesOfPIV(LoopInfo* li)
{
  if (trace())
    traceMsg(
      comp(), "   looking for stores in original loop %d...\n", li->_regionNum);
  ...
  for (block = bIt.getFirst(); block; block = bIt.getNext()) {
    ...
    while (currentTree != exitTree) {
      ...
      if (node->getOpCode().isStore() &&
          node->getSymbolReference() == oldCounter) {
        if (((node->getFirstChild()->getOpCode().isSub()) ||
          ...
          if (isInt32)
            ...
            pivIncInStore =
              node->getFirstChild()->getSecondChild()->getLongInt();
          ...
          if (pivIncInStore != li->_piv->getDeltaOnBackEdge()) {
            if (trace())
              traceMsg(comp(),
                       "\t loop %d ==> Found a store to induction variable "
                       "with increment different than deltaObBackEdge\n",
...
void
TR_StripMiner::findLeavesInList()
{
  ...
  for (li = it.getNext(); li; li = it.getNext()) {
    if (!(li->_region->contains(currentLeaf->_region,
                                li->_region->getParent()))) {
      currentLeaf = li;
      _loopInfos.add(currentLeaf);
    }
  }
...
bool
TR_StripMiner::findPivInSimpleForm(TR::Node* node, TR::SymbolReference* pivSym)
{
