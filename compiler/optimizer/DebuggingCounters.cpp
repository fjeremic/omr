...
void TR_DebuggingCounters::insertCounter(const char* name, TR::Compilation* comp, TR::TreeTop* tt, int32_t d)
{
  ...
  if (comp->getOptions()->insertDebuggingCounters()) {
    ...
    TR::Node* addNode = TR::Node::create(TR::iuadd, 2, loadNode, TR::Node::create(node, TR::iuconst, 0, 1));

    TR::TreeTop* incrementTree
      = TR::TreeTop::create(comp, TR::Node::createWithSymRef(TR::iustore, 1, 1, addNode, counterRef));
    ...
    if (counterInfo->totalCount > 0) {
      ...
      if (deviation != INT_MAX)
        fprintf(output, "Name: [%31s (%5d)] dynamic : (%5.2lf ) static : (%5.2lf ) [%lu]\n", counterInfo->counterName,
          ...
          // counterInfo->compilationCount,
          (double)((double)(counterInfo->compilationCount * 100) / ((double)compilationSum)), counterInfo->totalCount);
