...
NamedCounterInfo *TR_DebuggingCounters::getOrCreateNamedCounter(
  ...
  for (NamedCounterInfo *counterInfo = namedCounterInfos; counterInfo != NULL;
       counterInfo = counterInfo->_next) {
  ...
  NamedCounterInfo *newCounterInfo =
      (NamedCounterInfo *)jitPersistentAlloc(sizeof(NamedCounterInfo));

  // Initialize the counter info with the name, and zero counts
  newCounterInfo->counterName = (char *)jitPersistentAlloc(
      (strlen(hotnessName) + 3 + strlen(name)) * sizeof(char) + 1);
 ...
 */
void TR_DebuggingCounters::insertCounter(const char *name,
                                         TR::Compilation *comp, TR::TreeTop *tt,
                                         int32_t d) {
  ...
  if (comp->getOptions()->insertDebuggingCounters()) {
    if (!strncmp(name, "callee has too many bytecodes", 29))
      ...
      bucketSize =
          comp->getOptions()->getInlineCntrWarmCalleeTooBigBucketSize();
      ...
      bucketSize = comp->getOptions()
                       ->getInlineCntrWarmCalleeHasTooManyNodesBucketSize();
    else if (!strncmp(name, "caller has too many nodes", 25))
      bucketSize = comp->getOptions()
                       ->getInlineCntrWarmCallerHasTooManyNodesBucketSize();
  ...
  NamedCounterInfo *counterInfo =
      getOrCreateNamedCounter(comp, name, d, bucketSize);
  ...
  if (comp->getOptions()->insertDebuggingCounters()) {
    ...
    TR::SymbolReference *counterRef =
        comp->getSymRefTab()->createKnownStaticDataSymbolRef(ctrAddress,
                                                             TR::Int32);
    ...
    TR::Node *loadNode =
        TR::Node::createWithSymRef(node, TR::iuload, 0, counterRef);

    TR::Node *addNode = TR::Node::create(
        TR::iuadd, 2, loadNode, TR::Node::create(node, TR::iuconst, 0, 1));

    TR::TreeTop *incrementTree = TR::TreeTop::create(
        comp,
        TR::Node::createWithSymRef(TR::iustore, 1, 1, addNode, counterRef));
...
void TR_DebuggingCounters::report() {
  ...
  //
  for (counterInfo = namedCounterInfos; counterInfo != NULL;
       counterInfo = counterInfo->_next) {
  ...
  fprintf(output,
          "\nName: [                    counterName (delta)] dynamic : (  %% ) "
          "static : (  %% )  \n\n");
  ...
  for (counterInfo = namedCounterInfos; counterInfo != NULL;
    ...
    if (counterInfo->totalCount > 0) {
      ...
      if (deviation != INT_MAX)
        fprintf(
            output,
            "Name: [%31s (%5d)] dynamic : (%5.2lf ) static : (%5.2lf ) [%lu]\n",
            counterInfo->counterName,
            ((counterInfo->delta + 1) * counterInfo->bucketSize),
            //(uint32_t) counterInfo->totalCount,
            (double)(((double)(counterInfo->totalCount * 100) /
                      ((double)dynamicSum))),
            // counterInfo->compilationCount,
            (double)((double)(counterInfo->compilationCount * 100) /
                     ((double)compilationSum)),
            counterInfo->totalCount);
      //            fprintf(output, "DEBUG: totalCount =
      //            %x\n",counterInfo->totalCount);
      else
        fprintf(output,
                "Name: [%38s ] dynamic : (%5.2lf ) static : (%5.2lf )\n",
                counterInfo->counterName,
                //(uint32_t) counterInfo->totalCount,
                (double)(((double)(counterInfo->totalCount * 100) /
                          ((double)dynamicSum))),
...
/**
 *   Go through the int counters and move the counts into the large counters (to
 * avoid overflow)
 */
 ...
 *   tt:   the treetop to insert the counter before
 */
void TR_DebuggingCounters::insertInliningCounter(const char *name,
                                                 TR::Compilation *comp,
                                                 TR::TreeTop *callTreeTop,
                                                 int32_t size, int32_t d,
                                                 int32_t numLocals) {
  if (!comp->getOptions()->insertDebuggingCounters()) return;
  // If we're given a null instruction to insert before, we can't do anything
  if (!callTreeTop) return;

  bool siteIsNew =
      insertIfMissing(comp, name, false, callTreeTop, size, numLocals);
...
void TR_DebuggingCounters::insertInliningCounterOK(
    const char *name, TR::Compilation *comp, TR::TreeTop *origTT,
...
void TR_DebuggingCounters::inliningReportForMethod(TR::Compilation *comp) {
  ...
  if (output == NULL) output = fopen("inlinereport.txt", "wt");
  ...
  while (curSite != NULL) {
    if (!curSite->method && !curSite->opaque_method) {
      fprintf(output, "\t%d %d %d %d %d %s \t%s\n", curSite->flag, -1,
              curSite->frequency, curSite->size, curSite->numLocals,
              curSite->name, "Unknown");
      ...
      if (curSite->opaque_method)
        fprintf(output, "\t%d %d %d %d %d %s \t%s\n", curSite->flag,
                (comp->fej9()->getIProfilerCallCount(curSite->bcInfo, comp)),
                curSite->frequency, curSite->size, curSite->numLocals,
                curSite->name,
                comp->fe()->sampleSignature(curSite->opaque_method, buffer, 512,
                                            comp->trMemory()));
      else
        fprintf(output, "\t%d %d %d %d %d %s \t%s\n", curSite->flag,
                (comp->fej9()->getIProfilerCallCount(curSite->bcInfo, comp)),
                curSite->frequency, curSite->size, curSite->numLocals,
                curSite->name, curSite->method->signature(comp->trMemory()));
#endif
    }
 ...
 */
bool TR_DebuggingCounters::insertIfMissing(TR::Compilation *comp,
                                           const char *name, bool flag,
                                           TR::TreeTop *tt, int32_t size,
                                           int32_t numLocals) {
  CountedCallSite *curSite = countedCallSiteList;
  while (curSite != NULL) {
  ...
  CountedCallSite *newHead =
      (CountedCallSite *)comp->trMemory()->allocateHeapMemory(
          sizeof(CountedCallSite));
  ...
  if (!newHead->callTreeTop->getNode() ||
      newHead->callTreeTop->getNode()->getNumChildren() <= 0) {
    newHead->method = NULL;
    newHead->opaque_method = NULL;

    if (newHead->callTreeTop->getNode()) {
      TR::Node *callNode = newHead->callTreeTop->getNode();
      memcpy((void *)&newHead->bcInfo, (void *)&callNode->getByteCodeInfo(),
             sizeof(struct TR_ByteCodeInfo));
    ...
    if (callNode->getSymbolReference() && callNode->getOpCode().isCall()) {
      newHead->method =
          callNode->getSymbol()->castToMethodSymbol()->getMethod();
      newHead->opaque_method = NULL;
    } else {
      newHead->opaque_method = callNode->getOwningMethod();
      newHead->method = NULL;
