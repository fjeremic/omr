 ...
 */
NamedCounterInfo*
TR_DebuggingCounters::getOrCreateNamedCounter(TR::Compilation* comp,
                                              const char* name,
                                              int32_t d,
                                              int32_t bucketSize)
{
 ...
 */
void
TR_DebuggingCounters::insertCounter(const char* name,
                                    TR::Compilation* comp,
                                    TR::TreeTop* tt,
                                    int32_t d)
{
  ...
  if (comp->getOptions()->insertDebuggingCounters()) {
    if (!strncmp(name, "callee has too many bytecodes", 29))
      ...
      bucketSize =
        comp->getOptions()->getInlineCntrWarmCalleeHasTooManyNodesBucketSize();
    else if (!strncmp(name, "caller has too many nodes", 25))
      bucketSize =
        comp->getOptions()->getInlineCntrWarmCallerHasTooManyNodesBucketSize();
  ...
  if (comp->getOptions()->insertDebuggingCounters()) {
    ...
    TR::TreeTop* incrementTree = TR::TreeTop::create(
      comp, TR::Node::createWithSymRef(TR::iustore, 1, 1, addNode, counterRef));
...
TR_DebuggingCounters::report()
{
  ...
  fprintf(output,
          "Compilation sum %d Dynamic sum %lu \n",
          (int32_t)compilationSum,
          dynamicSum);
  fprintf(output, "\n");
 ...
 */
void
TR_DebuggingCounters::insertInliningCounter(const char* name,
                                            TR::Compilation* comp,
...
void
TR_DebuggingCounters::insertInliningCounterOK(const char* name,
                                              TR::Compilation* comp,
                                              TR::TreeTop* origTT,
                                              TR::TreeTop* callTreeTop,
                                              int32_t size,
...
void
TR_DebuggingCounters::inliningReportForMethod(TR::Compilation* comp)
{
      ...
      if (curSite->opaque_method)
        fprintf(output,
                ...
                comp->fe()->sampleSignature(
                  curSite->opaque_method, buffer, 512, comp->trMemory()));
      else
        fprintf(output,
                "\t%d %d %d %d %d %s \t%s\n",
                curSite->flag,
                ...
                curSite->numLocals,
                curSite->name,
                curSite->method->signature(comp->trMemory()));
#endif
    }
 ...
 */
bool
TR_DebuggingCounters::insertIfMissing(TR::Compilation* comp,
                                      const char* name,
                                      bool flag,
                                      TR::TreeTop* tt,
                                      int32_t size,
                                      int32_t numLocals)
{
  ...
  if (!newHead->callTreeTop->getNode() ||
    ...
    if (newHead->callTreeTop->getNode()) {
      ...
      memcpy((void*)&newHead->bcInfo,
             (void*)&callNode->getByteCodeInfo(),
