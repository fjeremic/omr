    ...
    if (cannotExtendWithCurrentFallThrough || (bestExtension && !bestExtension->isCold() && (bestExtension != block))) {
      ...
      if (lastBlockToMove) {
        ...
        if (newBlock->getPredecessors().size() == 1) {
          int32_t result
            = performChecksAndTreesMovement(newBlock, prevBlock, block, endTreeTop, visitCount, optimizer());
          if (result < 0) {
    ...
    if (prevNode->getOpCodeValue() == TR::Goto) {
      ...
      if (nextBlock && prevNode->getOpCode().isBranch() && !prevNode->isNopableInlineGuard()) {
        ...
        if (prevNode->getBranchDestination() == nextBlock->getEntry() && !nextBlock->isCold()) {
          ...
          TR_ASSERT(
            prevNode->getNumChildren() != 3, "the branch node has a GlRegDep child that we're about to invalidate");
          prevNode->reverseBranch(gotoDest);
    ...
    for (entry = sequences[bestFrom].first; entry; entry = entry->next) {
      ...
      if (entry->next && entry->next->block->getEntry()) {
        ...
        if (entry->fallThrough) {
          comp()->getFlowGraph()->setStructure(NULL);
          TR::Block* extraBlock
            = TR::Block::createEmptyBlock(node, comp(), entry->fallThrough->getFrequency(), entry->fallThrough);
          TR::TreeTop::create(
            comp(), extraBlock->getEntry(), TR::Node::create(node, TR::Goto, 0, entry->fallThrough->getEntry()));
  ...
  for (auto nextEdge = block1->getSuccessors().begin(); nextEdge != block1->getSuccessors().end(); ++nextEdge) {
    ...
    if (trace())
      traceMsg(comp(), "    Estimating hotness for BB [%d], next BB [%d], estimated hotness %d\n", block1->getNumber(),
        nextBlock->getNumber(), nextHotness);
...
bool TR_BlockManipulator::isBestChoiceForFallThrough(TR::Block* block1, TR::Block* block2)
{
  ...
  if (block1->isSuperCold()
    || ((block1->getSuccessors().size() == 1) && block1->getSuccessors().front()->getTo()->asBlock()->isSuperCold()))
    return false;
        ...
        if (oldDestination == block) {
          ...
          if (newBlock->getStructureOf())
            comp()->getFlowGraph()->addNode(
              gotoBlock, newBlock->getCommonParentStructureIfExists(oldDestination, comp()->getFlowGraph()));
          else
        ...
        if (!tt->getNode()->getOpCode().isJumpWithMultipleTargets() && canHoist) {
          ...
          */
          ...
          if (synergyExists
            ...
            TR::TreeTop* newPrevTree = tt;
            {
              ...
              while (!(currentTree == exitTreeTop)) {
                {
                  ...
                  for (int32_t childNum = 0; childNum < currentNode->getNumChildren(); childNum++) {
                    TR::Node* duplicateChildNode
                      = duplicateExact(currentNode->getChild(childNum), &seenNodes, &duplicateNodes, comp());
              ...
              if (fallThroughSuccBlock) {
                ...
                extraGotoBlock = TR::Block::createEmptyBlock(node1, comp(),
                  fallThroughTree->getNode()->getBlock()->getFrequency(), fallThroughTree->getNode()->getBlock());
                newBlocks.add(extraGotoBlock);
                TR::TreeTop* gotoTreeTop
                  = TR::TreeTop::create(comp(), TR::Node::create(node1, TR::Goto, 0, fallThroughTree));
            ...
            for (auto nextSuccEdge = block->getSuccessors().begin(); nextSuccEdge != block->getSuccessors().end();
                 ++nextSuccEdge) {
              TR::CFGNode* succBlock = (*nextSuccEdge)->getTo();
    ...
    if ((prevNode->getOpCodeValue() == TR::NULLCHK)
      ...
      while (cursorTreeTop != exitTree && cursorTreeTop->getNode()->getOpCodeValue() != TR::BBEnd) {
        ...
        replacedNullCheck = replaceNullCheckIfPossible(
          cursorNode, objectRef, prevNode, NULL, writtenSymbols, visitCount, initialVisitCount, compactionDone);
        if (!replacedNullCheck) {
          if (cursorNode->getOpCode().isStore() && cursorNode->getSymbolReference()->getSymbol()->isAutoOrParm()
  ...
  bool& compactionDone)
{
  ...
  if (cursorNode->getOpCodeValue() == TR::NULLCHK) {
      ...
      if (canBeRemoved
        ...
        if (!compactionDone) {
          ...
          TR_Pair<TR_ByteCodeInfo, TR::Node>* bcInfo
            = new (trHeapMemory()) TR_Pair<TR_ByteCodeInfo, TR::Node>(&prevNode->getByteCodeInfo(), cursorNode);
    ...
    for (i = 0; i < cursorNode->getNumChildren(); i++) {
      ...
      if (!replaceNullCheckIfPossible(
            child, objectRef, prevNode, cursorNode, writtenSymbols, visitCount, initialVisitCount, compactionDone))
        return false;
...
void TR_ArraysetStoreElimination::reduceArraysetStores(
  TR::Block* block, TR_BitVector* readAndWrittenSymbols, TR_BitVector* symbolsWrittenByArrayset, TR_BitVector* scratch)
{
  TR::TreeTop* prevTree = block->getEntry();
  TR::TreeTop* currentTree = block->getFirstRealTreeTop();
  TR::TreeTop* exitTree = block->getExit();
  ...
  if (currentNode->getOpCodeValue() == TR::arraycopy) {
    ...
    if ((arraycopyDestAddressNode == addressChild)
      ...
      *scratch &= *readAndWrittenSymbols;
      ...
      if (scratch->isEmpty()
        && performTransformation(comp(),
          "%sEliminating some stores done by arrayset node %p because stores done by a following arraycopy node %p "
          "will overwrite them\n",
  ...
  TR::Compilation* comp, TR::Node* andNode, TR::Node* lastRealNode, TR::ILOpCodes ifOpCode, TR::ILOpCodes andOpCode)
{
  ...
  return (andNode->getOpCodeValue() == ifOpCode && lastRealNode->getOpCodeValue() == ifOpCode
    ...
    && andNode->getSecondChild()->getOpCode().isLoadConst() && lastRealNode->getSecondChild()->getOpCode().isLoadConst()
    ...
    if ((lastRealNode->getOpCode().isBranch() && (lastRealNode->getOpCodeValue() != TR::Goto)
      ...
      while (seenAndNode) {
        ...
        if ((andNode->getFirstChild() == lastRealNode->getFirstChild())
          ...
          && lastRealNode->getOpCode()
               .isBooleanCompare()) // this code was allowing unsigned types in -- I made this test stronger // NOTE
                                    // that isCompare is only true for signed compares -- this is very ugly though
        {
          ...
          && andNode->getSecondChild()->getOpCode().isLoadConst()
          && lastRealNode->getSecondChild()->getOpCode().isLoadConst()
          && (lastRealNode->getSecondChild()->getType().isInt32()) && lastRealNode->getOpCode().isUnsignedCompare()) {
      ...
      if (!seenAndBefore) {
        ...
        if (cg()->branchesAreExpensive()
          && (lastRealNode->getOpCodeValue() == TR::ificmpge || lastRealNode->getOpCodeValue() == TR::ifiucmpge
          ...
          if (lastRealNode->getBranchDestination() == nextToNextBlockEntry) {
            ...
            if ((firstTree == lastTree) && (nextBlock->getPredecessors().size() == 1)) {
              ...
              if (firstNode->getOpCode().isStoreDirect()
                && ((firstNode->getFirstChild()->getOpCode().isAdd() || firstNode->getFirstChild()->getOpCode().isSub())
                ...
                if (firstNode->getFirstChild()->getFirstChild()->getOpCode().isLoadVar()
                  ...
                  && // in actual fact the ref count can be > 1 as long as this is the first reference to this load (we
                     // could check that in future if reqd)
                  (firstNode->getFirstChild()->getFirstChild()->getSymbolReference()->getReferenceNumber()
                ...
                if (areTreesInRequiredForm
                  ...
                  TR::Node* newIShrNode
                    = TR::Node::create((lastFirstChild->getType().isInt32()) ? TR::ishr : TR::lshr, 2, lastFirstChild,
                  ...
                  TR::Node* newIAndNode = TR::Node::create(
                    firstNode->getType().isInt32() ? TR::iand : TR::land, 2, secondGrandChild, newIShrNode);
...
void TR_EliminateRedundantGotos::placeAsyncCheckBefore(TR::TreeTop* tt)
{
  TR::TreeTop* actt = TR::TreeTop::create(comp(),
    TR::Node::createWithSymRef(
      tt->getNode(), TR::asynccheck, 0, getSymRefTab()->findOrCreateAsyncCheckSymbolRef(comp()->getMethodSymbol())));
...
int32_t TR_EliminateRedundantGotos::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
  ...
  for (TR::TreeTop *treeTop = startTree, *exitTreeTop; (treeTop != endTree); treeTop = exitTreeTop->getNextTreeTop()) {
    ...
    for (auto inEdge = preds.begin(); inEdge != preds.end(); ++inEdge) {
      ...
      if (!emptyBlock
        && (!tt->getNode()->getOpCode().isBranch() || tt->getNode()->getBranchDestination() != block->getEntry()))
        continue;
    ...
    if (containsTreesOtherThanGoto) {
      if (block->getEntry()->getNode()->getNumChildren() > 0 || destBlock->getEntry()->getNode()->getNumChildren() > 0)
    ...
    if (predBlock->getNextBlock() == destBlock) {
      ...
      if (last->getOpCodeValue() == TR::Goto) {
        ...
        if (last->getNumChildren() > 0) {
          TR_ASSERT_FATAL(
            last->getNumChildren() == 1, "n%un goto has %d children\n", last->getGlobalIndex(), last->getNumChildren());
...
void TR_EliminateRedundantGotos::fixPredecessorRegDeps(TR::Node* regdepsParent, TR::Block* destBlock)
{
  ...
  TR_ASSERT_FATAL(remainingDeps == newReceivingRegdeps->getNumChildren(), "n%un: bad number %d of remaining regdeps\n",
    ...
    if (tt2 && node->getBranchDestination() == tt2) {
      ...
      // if (node->getOpCodeValue() == TR::Goto)
      {
        result = performChecksAndTreesMovement(
          tt2->getNode()->getBlock(), block, block->getNextBlock(), endTree, comp()->getVisitCount(), optimizer());
      }
      // else
      //{
    ...
    if (node->getOpCodeValue() == TR::BBEnd) {
      ...
      case TR::anewarray: {
        ...
        if (numElementsInfo) {
          ...
          if (doProfiling
            && performTransformation(comp(), "O^O PROFILED NODE VERSIONING: Versioning %s %s, size %s %d %.01f%%\n",
            ...
            TR::SymbolReference* storeSymRef
              = getSymRefTab()->createTemporary(comp()->getMethodSymbol(), numElementsNode->getDataType());

            TR::TreeTop* storeTree
              = TR::TreeTop::create(comp(), TR::Node::createWithSymRef(TR::istore, 1, 1, numElementsNode, storeSymRef));
            ...
            if (originalFrequency >= 0) {
              ...
              int16_t slowFrequency
                = std::max<int16_t>(originalFrequency * (1 - combinedProbability), MAX_COLD_BLOCK_COUNT + 1);
              if (trace()) {
                traceMsg(comp(), " -> Setting fast block_%d frequency = %d; slow block_%d frequency = %d\n",
                  fastTree->getEnclosingBlock()->getNumber(), fastFrequency, slowTree->getEnclosingBlock()->getNumber(),
                  slowFrequency);
              }
    ...
    if (node->getNumChildren() == 2 && node->isInternalPointer()) {
      ...
      while (++throttle < 10) {
        if ((node->getReferenceCount() > 1 && node->isInternalPointer() && node->getNumChildren() == 2
          ...
          if (performTransformation(
                comp(), "%sRematerializing node %p(%s)\n", optDetailString(), node, node->getOpCode().getName())) {
            TR::Node* newChild
              = TR::Node::create(node->getOpCodeValue(), 2, node->getFirstChild(), node->getSecondChild());
            ...
            if ((node->getNumChildren() == 2) && node->getFirstChild()->isInternalPointer()
              && (node->getFirstChild()->getReferenceCount() > 1) && (node->getFirstChild()->getNumChildren() == 2))
              node = node->getFirstChild();
            else
              break;
          } else
            ...
            if (secondChild->getReferenceCount() > 1) {
              ...
              if (performTransformation(comp(), "%sRematerializing node %p(%s)\n", optDetailString(), secondChild,
                ...
                TR::Node* newSecondChild = TR::Node::create(
                  secondChild->getOpCodeValue(), 2, secondChild->getFirstChild(), secondChild->getSecondChild());
  ...
  for (treeTop = startTree; (treeTop != endTree); treeTop = treeTop->getNextRealTreeTop()) {
    ...
    if (node->getOpCodeValue() == TR::BBStart) {
      ...
      if (!_curBlock->isExtensionOfPreviousBlock()) {
        ...
        TR_ASSERT(
          state->_currentlyCommonedVector128Loads.isEmpty(), "Vector128 loads list should be empty and is not\n");
      }
    }
...
void TR_Rematerialization::removeNodeFromList(TR::Node* node, List<TR::Node>* nodes, List<List<TR::Node> >* parents,
  bool checkSymRefs, List<TR::Node>* loadsAlreadyVisited, List<TR::Node>* loadsAlreadyVisitedThatCannotBeRematerialized,
  ...
  TR_RematState* state, TR_RematAdjustments& adjustments)
{
  ...
  if ((((node->getFutureUseCount() & 0x7fff) == 0) && (node->getReferenceCount() > 1))
    ...
    if (node->getOpCode().isFloatingPoint()) {
      ...
      if (isRematerializable(parent, node, true) && node->isRematerializeable()) {
        if (isRematerializableLoad(node, parent))
          removeNodeFromList(
            node, &state->_currentlyCommonedVector128Loads, &state->_parentsOfCommonedVector128Loads, false);
        else
  ...
  for (i = 0; i < node->getNumChildren(); ++i) {
    ...
    if ((child->getFutureUseCount() & 0x7fff) < child->getReferenceCount()) {
      ...
      TR_RematAdjustments newAdjustments
        = { adjustments.adjustmentFromParent + childAdjustment, adjustments.fpAdjustmentFromParent + fpChildAdjustment,
  ...
  */
  ...
  if (node->getOpCode().isCall()
    && (node->getOpCode().isTreeTop()
      || (parent && ((parent->getOpCodeValue() == TR::treetop) || (parent->getOpCode().isResolveOrNullCheck()))))) {
    isCall = true;
  ...
  if (removeNodes) {
    ...
    if (node->getOpCode().isFloatingPoint())
      ...
      removeNodeFromList(node, &state->_currentlyCommonedVector128Loads, &state->_parentsOfCommonedVector128Loads, true,
...
void TR_Rematerialization::rematerializeNode(TR::TreeTop* treeTop, TR::Node* parent, TR::Node* node,
  ...
  List<List<TR::Node> >* parents, List<TR::Node>* currentlyCommonedLoads, List<List<TR::Node> >* parentsOfCommonedLoads,
    ...
    if (disableSynergy() && comp()->isOptServer() && !mergeNode->isEmptyBlock()) {
      ...
      for (auto edge = mergeNode->getPredecessors().begin(); edge != mergeNode->getPredecessors().end(); ++edge) {
        ...
        if (predBlock->getEntry() && (predBlock->getFrequency() >= splitFrequency)
          ...
          if (trace())
            traceMsg(
              comp(), "    rejecting because predecessor block %d has more than 4 treetops\n", predBlock->getNumber());
          break;
        }
      }
      if (!allSimplePreds) {
  ...
  for (int32_t i = 0; i <= array->lastIndex(); ++i) {
    ...
    for (auto edge = mergeNode->getPredecessors().begin(); edge != mergeNode->getPredecessors().end(); ++edge) {
      ...
      if (toBlock(node)->getEntry()) {
        ...
        // a GCR counting guard which could be patched, we are not allowed to reverse these either and must choose
        // someone else
        ...
        if (child->getFrequency() <= (target->getFrequency() - tollerance)) {
          if (trace())
            traceMsg(comp(), "    Reject successor block_%d: frequency is too low (%d < %d)\n", child->getNumber(),
              child->getFrequency(), (target->getFrequency() - tollerance));
          continue;
        }
    ...
    if (pruneAndPopulateBlockMapper(&bMap, synergisticDepthCalculator(&bMap, splitPred)) > 0) {
        ...
        if (force || splitPred->getSuccessors().size() == 1) {
          TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "blockSplitter.inconsistentFreqs/%s/(%s)/block_%d",
              comp()->getHotnessName(comp()->getMethodHotness()), comp()->signature(), mergeNode->getNumber()));
      ...
      for (BlockMapper* itr = bMap.getFirst(); itr; itr = itr->getNext()) {
        if (trace())
          traceMsg(
            comp(), "    setting block_%d cold and block_%d hot\n", itr->_from->getNumber(), itr->_to->getNumber());
        ...
        if (origFreq > 0) {
          ...
          for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
            ...
            else if ((*predEdge)->getFrom()->asBlock()->isCold() && (*predEdge)->getTo()->asBlock()->isCold()
              && freq > MAX_COLD_BLOCK_COUNT)
              freq = MAX_COLD_BLOCK_COUNT;
          ...
          for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
            ...
            if (!(*succEdge)->getFrom()->asBlock()->isCold() && !(*succEdge)->getTo()->asBlock()->isCold()
              && (freq <= MAX_COLD_BLOCK_COUNT))
              freq = MAX_COLD_BLOCK_COUNT + 1;
            else if ((*succEdge)->getFrom()->asBlock()->isCold() && (*succEdge)->getTo()->asBlock()->isCold()
              && freq > MAX_COLD_BLOCK_COUNT)
              freq = MAX_COLD_BLOCK_COUNT;
          ...
          if (itr->_from->getPredecessors().size() == 1) {
            ...
            if (trace())
              traceMsg(
                comp(), "single predEdge freq=%d. set it to %d instead\n", predEdge->getFrequency(), coldEdgeFreq);
          ...
          if (itr->_from->getSuccessors().size() == 1) {
            ...
            if (trace())
              traceMsg(
                comp(), "single succEdge freq=%d. set it to %d instead\n", succEdge->getFrequency(), coldEdgeFreq);
...
int32_t TR_BlockSplitter::synergisticDepthCalculator(TR_LinkHeadAndTail<BlockMapper>* bMap, TR::Block* startPoint)
{
  ...
  while (!preamble.isEmpty()) {
    ...
    for (TR::TreeTop* treeTopItr = blockItr->getEntry();
         treeTopItr && treeTopItr != blockItr->getExit()->getNextTreeTop(); treeTopItr = treeTopItr->getNextTreeTop()) {
...
bool TR_BlockSplitter::hasIVUpdate(TR::Block* block)
{
  ...
  for (OMR::TreeTop* treeTopItr = block->getEntry(); treeTopItr && treeTopItr != block->getExit()->getNextTreeTop();
       treeTopItr = treeTopItr->getNextTreeTop()) {
    if (treeTopItr->getNode()->getOpCode().isStoreDirect() && treeTopItr->getNode()->getOpCode().hasSymbolReference()
    ...
    if (lastRealNode->getOpCode().isBranch()) {
      if (lastRealNode->getOpCode().isIf()) {
        if (lastRealNode->getBranchDestination()->getNode()->getBlock()->getNumber() == candidate->getNumber()) {
          ...
          // this check handles splitting through a loop header in a region where we have not detected a valid loop
          // header
...
int32_t TR_InvariantArgumentPreexistence::perform()
{
...

  {
    ...
    // so TR_InvariantArgumentPreexistence can validate preexistence args and propagate info onto parmSymbols
    {
      ...
      if (trace())
        traceMsg(
          comp(), "PREX:    Populating parmInfo of peeked method %s %p\n", feMethod->signature(trMemory()), peekInfo);
        ...
        // make sure this path isn't executed iff getCurrentPeekingArgInfo is provided
          ...
          if (arg) {
            if (arg->classIsFixed()) {
              ...
              if (trace())
                traceMsg(
                  comp(), "PREX:      Parm %d symbol [%p] has fixed type %p\n", index, symbol, symbol->getFixedType());
            }
...
TR_InvariantArgumentPreexistence::ParmInfo* TR_InvariantArgumentPreexistence::getSuitableParmInfo(TR::Node* node)
{
  ...
  if (comp()->isPeekingMethod() && !info->classIsRefined())
    return NULL;
  if (!comp()->isPeekingMethod() && !symbol->getParmSymbol()->getIsPreexistent()
...
void TR_InvariantArgumentPreexistence::processIndirectCall(TR::Node* node, TR::TreeTop* treeTop, vcount_t visitCount)
{
  ...
  if (receiver->getSymbolReference()->hasKnownObjectIndex()) {
...

    {
      ...
      receiverInfo.setClass(
        TR::Compiler->cls.objectClass(comp(), knot->getPointer(receiver->getSymbolReference()->getKnownObjectIndex())));
    }
  ...
  if (methodSymbol->isComputed()) {
    ...
    if (refinedMethod) {
      ...
      if (!needsAssumptions || addAssumptions) {
        if (!refinedMethod->isSameMethod(resolvedMethod)) {
          if (performTransformation(comp(),
                "%sspecialize and devirtualize invoke [%p] on currently fixed or final parameter %d [%p]\n",
            ...
            isModified = true;
                     ...
                     "%sdevirtualize invoke [%p] on currently fixed or final parameter  %d [%p]\n", optDetailString(),
      ...
      if (isModified && !(receiverInfo.classIsRefined() && receiverInfo.classIsFixed()) && receiverSymbol
        && receiverSymbol->getParmSymbol()->getFixedType() == 0) {
    ...
    TR_ASSERT((fixedOrFinalInfoExists || (receiverSymbol->getParmSymbol()->getFixedType() == 0)),
      "Symbol %p is fixed ref type - but is not marked to be currently final (fixedOrFinalInfoExists=%d, fixedType=%p)",
      receiverSymbol, fixedOrFinalInfoExists, receiverSymbol->getParmSymbol()->getFixedType());
    ...
    if (!isInterface && !resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
      ...
      if (methSymbol->isInterface() || methodSymbol) {
        ...
        if (recompInfo && recompInfo->getMethodInfo()->getNumberOfInvalidations() >= 1
          ...
          if (comp()->getPersistentInfo()->getRuntimeAssumptionTable()->getAssumptionCount(
            ...
            method = chTable->findSingleInterfaceImplementer(receiverInfo.getClass(),
              node->getSymbolReference()->getCPIndex(), node->getSymbolReference()->getOwningMethod(comp()), comp());
          // if (method)
        ...
        if (method && !method->virtualMethodIsOverridden()) {
          ...
          if (addAssumptions
            && performTransformation(comp(),
              "%sspecialize and devirtualize invoke [%p] based on only a single implementation for call on parameter "
              "%d [%p]\n",
              optDetailString(), node, receiverParmOrdinal, receiverSymbol)) {
            if (newMethod || !method->isSameMethod(resolvedMethod)) {
              ...
              if (methSymbol->isInterface())
                offset = node->getSymbolReference()->getOwningMethod(comp())->getResolvedInterfaceMethodOffset(
...
void TR_InvariantArgumentPreexistence::processIndirectLoad(TR::Node* node, TR::TreeTop* treeTop, vcount_t visitCount)
{
  ...
  if (addressChild->getSymbolReference()->hasKnownObjectIndex()) {
    ...
    somethingMayHaveChanged = TR::TransformUtil::transformIndirectLoadChainAt(comp(), node, addressChild,
      (uintptrj_t*)addressChild->getSymbol()->castToStaticSymbol()->getStaticAddress(), &removedNode);
  ...
  if (somethingMayHaveChanged && nodeToNullCheck) {
    ...
    if (trace())
      traceMsg(
        comp(), "PREX:          Anchored [%p] formerly under %s [%p]\n", node, ttNode->getOpCode().getName(), ttNode);
  }
}
  ...
  for (TR::TreeTop* tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    case TR::ifacmpne:
      if (node->isProfiledGuard() && node->getFirstChild() && checkcastTree) {
        TR_ASSERT(checkcastTree->getNode()->getFirstChild()->getOpCode().hasSymbolReference(),
          "the first checkcast child [%p] should have a symref!", checkcastTree->getNode()->getFirstChild());
        TR_ASSERT(checkcastTree->getNode()->getFirstChild()->getOpCode().hasSymbolReference(),
          "the second checkcast child [%p] should have a symref!", checkcastTree->getNode()->getSecondChild());

        TR_VirtualGuard* virtualGuard = comp()->findVirtualGuardInfo(node);
        TR::Node* vftLoad = node->getFirstChild();
        if (virtualGuard && transformationIsValid && virtualGuard->getTestType() == TR_VftTest
          ...
          && vftLoad->getFirstChild()->getSymbolReference()
            == checkcastTree->getNode()->getFirstChild()->getSymbolReference()
          ...
          if (checkcastTree->getNode()->getOpCodeValue() == TR::checkcastAndNULLCHK) {
            ...
            TR::Node* nullCheckNode = TR::Node::createWithSymRef(
              TR::NULLCHK, 1, 1, passThroughNode, comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(NULL));
    ...
    case TR::ifacmpeq:
      ...
      if (checkcastTree) {
        if (node->getFirstChild() != checkcastTree->getNode()->getFirstChild()
          || !node->getSecondChild()->getOpCode().isLoadConst() || node->getSecondChild()->getAddress() != 0) {
          transformationIsValid = false;
        } else {
          ...
          traceMsg(comp(),
            "Found suitable ifacmpeq [%p] after checkcast [%p], safe to continue to next block in search of a profiled "
            "guard\n",
      ...
      break;
    default:
      if (!node->getOpCode().isStore() || !node->getOpCode().hasSymbolReference()
...
TR::Node* TR_CheckcastAndProfiledGuardCoalescer::storeObjectInATemporary(TR::TreeTop* checkcastTree)
{
  ...
  TR::SymbolReference* tempCheckcastSymRef
    = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), obj->getDataType(), false);
...
void TR_CheckcastAndProfiledGuardCoalescer::doBasicCase(TR::TreeTop* checkcastTree, TR::TreeTop* profiledGuardTree)
{
  ...
  //
  TR::Node* classConst = TR::Node::createWithSymRef(checkcastTree->getNode()->getSecondChild(), TR::loadaddr, 0,
    checkcastTree->getNode()->getSecondChild()->getSymbolReference());

  TR::TreeTop* checkCastInsertionPoint = slowPathBlock->getEntry();

  TR::Node* newCheckcastNode
    = TR::Node::createWithSymRef(TR::checkcast, 2, TR::Node::createLoad(storeNode, tempCheckcastSymRef), classConst, 0,
      comp()->getSymRefTab()->findOrCreateCheckCastSymbolRef(comp()->getMethodSymbol()));
...
int32_t TR_ColdBlockMarker::isBlockCold(TR::Block* block)
{
  ...
  // for (TR::PostorderNodeOccurrenceIterator iter(block->getFirstRealTreeTop(), this); iter != block->getExit();
  // ++iter)
  //
...
void TR_ColdBlockOutlining::reorderColdBlocks()
{
  ...
  for (currentTree = comp()->getStartTree(); currentTree != NULL; currentTree = exitTree->getNextTreeTop()) {
    ...
    // First, check if a predeccessor of this block is genAsmFlow, this block is not it's predeccessor' fall through,
    // continue if yes
...
TR::Block* TR_BlockManipulator::breakFallThrough(TR::Block* faller, TR::Block* fallee, bool isOutlineSuperColdBlock)
{
  ...
  if (lastNode->getOpCode().isBranch() || lastNode->getOpCode().isJumpWithMultipleTargets()) {
    ...
    TR::Block* gotoBlock = TR::Block::createEmptyBlock(lastNode, comp(),
      (fallee->getFrequency() < faller->getFrequency()) ? fallee->getFrequency() : faller->getFrequency(), fallee);
...
void TR_TrivialDeadTreeRemoval::preProcessTreetop(
  TR::TreeTop* treeTop, List<TR::TreeTop>& commonedTreeTopList, char* optDetails, TR::Compilation* comp)
{
...
void TR_TrivialDeadTreeRemoval::processCommonedChild(TR::Node* child, TR::TreeTop* treeTop,
  List<TR::TreeTop>& commonedTreeTopList, char* optDetails, TR::Compilation* comp)
{
  bool trace = comp->getOption(TR_TraceTrivialDeadTreeRemoval);
  if (child->getReferenceCount() > 1) {
        ...
        if (listTT->getNode()->getFirstChild() == child) {
          if (listTT != treeTop) {
            if (performTransformation(comp, "%sFound commoned reference to child %s (%p) so unlink %s (0x%p)\n",
                  optDetails, child->getOpCode().getName(), child, listTT->getNode()->getOpCode().getName(),
                  listTT->getNode())) {
              listTT->unlink(true);
            }
            ...
            if (treeTop->getNode()->getNumChildren() == 1 && treeTop->getNode()->getOpCodeValue() == TR::treetop
              ...
              if (performTransformation(comp,
                    ...
                    child->getOpCode().getName(), child, treeTop->getNode()->getOpCode().getName(), treeTop->getNode()))
                treeTop->unlink(true);
              commonedTreeTopList.removeNext(prevElement);
              return;
            }
...
int32_t TR_TrivialBlockExtension::performOnBlock(TR::Block* block)
{
  ...
  if (block->isExtensionOfPreviousBlock()) {
    ...
    if (trace())
      traceMsg(
        comp(), "BlockExtension: block_%d has %d predecessors\n", block->getNumber(), block->getPredecessors().size());
  }
