    ...
    if (prevNode->getOpCodeValue() == TR::Goto) {
      ...
      if (nextBlock && prevNode->getOpCode().isBranch() &&
        ...
        if (prevNode->getBranchDestination() == nextBlock->getEntry() &&
          ...
          cfg->removeEdge(
              F,
              block); // ideally 'block' will become unreachable, and be removed
...
int32_t
TR_BlockManipulator::countNumberOfTreesInSameExtendedBlock(TR::Block *block) {
      ...
      if (canBeRemoved &&
        ...
        if (!compactionDone) {
          ...
          TR_Pair<TR_ByteCodeInfo, TR::Node> *bcInfo = new (trHeapMemory())
              TR_Pair<TR_ByteCodeInfo, TR::Node>(&prevNode->getByteCodeInfo(),
                                                 cursorNode);
              ...
              if (firstNode->getOpCode().isStoreDirect() &&
                ...
                if (firstNode->getFirstChild()
                    ...
                    (firstNode->getFirstChild()
                         ->getFirstChild()
                         ->getReferenceCount() ==
                     1) && // in actual fact the ref count can be > 1 as long as
                           // this is the first reference to this load (we could
                           // check that in future if reqd)
                    (firstNode->getFirstChild()
                         ->getFirstChild()
                         ->getSymbolReference()
                         ->getReferenceNumber() ==
      ...
      case TR::anewarray: {
        ...
        if (numElementsInfo) {
          ...
          if (doProfiling &&
              performTransformation(
                  comp(),
                  "O^O PROFILED NODE VERSIONING: Versioning %s %s, size %s %d "
                  "%.01f%%\n",
            ...
            if (numDifferentSizes >= 2) {
              ...
              TR::TreeTop::create(
                  ...
                  TR::Node::createWithSymRef(
                      TR::istorei, 2, 2, fixedSizeNew, fixedSizeNewOriginalSize,
                      comp()
                          ->getSymRefTab()
            ...
            fixedSizeNewOriginalSize
                ->recursivelyDecReferenceCount(); // removed from fixedSizeNew

            // Store new object to a temp, and replace uses of object with loads
            // of the temp
            //
            ...
            if (originalFrequency >= 0) {
              ...
              if (trace()) {
                traceMsg(
                    comp(),
                    " -> Setting fast block_%d frequency = %d; slow block_%d "
                    "frequency = %d\n",
...
public:
  ...
  TR_RematState(TR::Compilation *comp)
      : _currentlyCommonedNodes(comp->trMemory()),
        ...
        _parents(comp->trMemory()), _currentlyCommonedLoads(comp->trMemory()),
...
#ifdef J9_PROJECT_SPECIFIC
static bool
specializeInvokeExactSymbol(TR::Node *callNode,
                            TR::KnownObjectTable::Index receiverIndex,
...
int32_t TR_CheckcastAndProfiledGuardCoalescer::perform() {
  ...
  for (TR::TreeTop *tt = comp()->getStartTree(); tt;
    ...
    case TR::ifacmpne:
      if (node->isProfiledGuard() && node->getFirstChild() && checkcastTree) {
        ...
        if (virtualGuard && transformationIsValid &&
          ...
          if (checkcastTree->getNode()->getOpCodeValue() ==
            ...
            TR::Node *passThroughNode = TR::Node::create(
                TR::PassThrough, 1, checkcastTree->getNode()->getFirstChild());
      ...
      break;
    case TR::ifacmpeq:
      // It's safe to move a checkcast across a null test on the same object if
      // we fall through when non-null
      if (checkcastTree) {
        if (node->getFirstChild() !=
          ...
          // Jump to the next BBStart, next iteration of the loop will start at
          // the tree following the BBStart
...
TR_ColdBlockMarker::TR_ColdBlockMarker(TR::OptimizationManager *manager)
    : TR_BlockManipulator(manager), _exceptionsAreRare(true),
      _notYetRunMeansCold(false), _enableFreqCBO(true) {}
