...
int32_t
TR_PeepHoleBasicBlocks::perform()
{
...
int32_t
TR_ExtendBasicBlocks::orderBlocksWithoutFrequencyInfo()
{
...
int32_t
TR_ExtendBasicBlocks::orderBlocksWithFrequencyInfo()
{
  ...
  TR_SequenceHead* sequences =
    (TR_SequenceHead*)trMemory()->allocateStackMemory(numBlocks *
                                                      sizeof(TR_SequenceHead));
  ...
  for (numSequences = numBlocks;
    ...
    for (i = 0; i < numBlocks; ++i) {
      ...
      for (auto edge = currentBlockEntry->block->getSuccessors().begin();
           edge != currentBlockEntry->block->getSuccessors().end();
           ++edge) {
    ...
    for (int32_t i = 0; i < numBlocks; ++i) {
      if (sequences[i].first != NULL) {
        ...
        for (TR_SequenceEntry* entry = sequences[i].first; entry;
          ...
          traceMsg(
            comp(), "%3d(%5d) ", block->getNumber(), block->getFrequency());
        }
        traceMsg(comp(), "\n");
      }
    }
    ...
    for (entry = sequences[bestFrom].first; entry; entry = entry->next) {
      ...
      if (entry->next && entry->next->block->getEntry()) {
        ...
        if (entry->fallThrough) {
          comp()->getFlowGraph()->setStructure(NULL);
          TR::Block* extraBlock =
            TR::Block::createEmptyBlock(node,
                                        comp(),
                                        entry->fallThrough->getFrequency(),
                                        entry->fallThrough);
          TR::TreeTop::create(
            comp(),
            extraBlock->getEntry(),
            TR::Node::create(
              node, TR::Goto, 0, entry->fallThrough->getEntry()));
...
int32_t
TR_BlockManipulator::estimatedHotness(TR::CFGEdge* edge, TR::Block* block)
{
...
TR::Block*
TR_BlockManipulator::getBestChoiceForExtension(TR::Block* block1)
{
  TR::Block* bestExtension = NULL;
  ...
  for (auto nextEdge = block1->getSuccessors().begin();
       nextEdge != block1->getSuccessors().end();
       ++nextEdge) {
...
bool
TR_BlockManipulator::isBestChoiceForFallThrough(TR::Block* block1,
                                                TR::Block* block2)
{
...
//
static bool
insideLoop(TR::Compilation* comp,
           TR::Block* block1,
           TR::Block* block2,
           TR::Block* block3)
{
...
//
int32_t
TR_BlockManipulator::performChecksAndTreesMovement(TR::Block* newBlock,
                                                   TR::Block* prevBlock,
                                                   TR::Block* block,
        ...
        if (oldDestination == block) {
          ...
          if (newBlock->getStructureOf())
            comp()->getFlowGraph()->addNode(
              gotoBlock,
              newBlock->getCommonParentStructureIfExists(
...
bool
TR_HoistBlocks::hasSynergy(TR::Block* block, TR::Node* node1)
{
...
int32_t
TR_HoistBlocks::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
...
//
static TR::Node*
duplicateExact(TR::Node* node,
               List<TR::Node>* seenNodes,
...
int32_t
TR_HoistBlocks::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
  TR::CFG* cfg = comp()->getFlowGraph();
  if (cfg == NULL)
    return 0;
          ...
          */
          ...
          if (synergyExists &&
              ...
              while (!(currentTree == exitTreeTop)) {
                {
                  ...
                  for (int32_t childNum = 0;
                       childNum < currentNode->getNumChildren();
                       childNum++) {
            ...
            for (auto nextSuccEdge = block->getSuccessors().begin();
                 nextSuccEdge != block->getSuccessors().end();
                 ++nextSuccEdge) {
              TR::CFGNode* succBlock = (*nextSuccEdge)->getTo();
              if (extraGotoBlock && (succBlock == fallThroughSuccBlock)) {
                ...
                if (block != succBlock)
                  cfg->addEdge(
                    TR::CFGEdge::createEdge(prevBlock, succBlock, trMemory()));
                else
...
static void
markNodesUsedInIndirectAccesses(TR::Node* node,
...
static void
initializeFutureUseCounts(TR::Node* node,
                          TR::Node* parent,
                          vcount_t visitCount,
                          TR::Compilation* comp,
                          int32_t* heightArray)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i) {
    initializeFutureUseCounts(
      node->getChild(i), node, visitCount, comp, heightArray);
...
int32_t
TR_CompactNullChecks::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
    process(block->getEntry(),
            block->getEntry()->getExtendedBlockExitTreeTop()->getNextTreeTop());
...
int32_t
TR_CompactNullChecks::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
  comp()->incVisitCount();
  TR::TreeTop* treeTop;
...
void
TR_CompactNullChecks::compactNullChecks(TR::Block* block,
    ...
    if ((prevNode->getOpCodeValue() == TR::NULLCHK) &&
      ...
      if (referenceChild == prevNode->getFirstChild()->getFirstChild()) {
        ...
        while (!replacedPassThroughNode) {
          ...
          replacedPassThroughNode =
            replacePassThroughIfPossible(cursorNode,
                                         referenceChild,
                                         prevNode,
                                         NULL,
                                         &isTreeTopNode,
      ...
      while (cursorTreeTop != exitTree &&
        ...
        TR::Node* cursorNode = cursorTreeTop->getNode();
        replacedNullCheck = replaceNullCheckIfPossible(cursorNode,
                                                       objectRef,
                                                       prevNode,
                                                       NULL,
                                                       writtenSymbols,
...
bool
TR_CompactNullChecks::replacePassThroughIfPossible(TR::Node* currentNode,
                                                   TR::Node* referenceChild,
                                                   TR::Node* prevNode,
                                                   TR::Node* currentParent,
                                                   bool* isTreeTopNode,
                                                   TR_BitVector* writtenSymbols,
                                                   vcount_t visitCount,
                                                   vcount_t initialVisitCount,
                                                   TR::TreeTop* currentTree)
{
  ...
  for (i = 0; i < currentNode->getNumChildren(); ++i) {
    ...
    if (replacePassThroughIfPossible(child,
                                     referenceChild,
                                     prevNode,
                                     currentNode,
                                     isTreeTopNode,
    ...
    if (!(opCode.isArrayLength() &&
      ...
      if (canCompact && performTransformation(
                          comp(),
                          "%sCompact null check %p with node %p in next tree\n",
                          optDetailString(),
                          prevNode,
                          currentNode)) {
        // if (!_isNextTree)
...
static bool
nodeCanRaiseException(TR::Node* node)
{
...
bool
TR_CompactNullChecks::replaceNullCheckIfPossible(TR::Node* cursorNode,
                                                 TR::Node* objectRef,
                                                 TR::Node* prevNode,
                                                 TR::Node* currentParent,
                                                 TR_BitVector* writtenSymbols,
                                                 ...
                                                 bool& compactionDone)
{
  ...
  if (cursorNode->getOpCodeValue() == TR::NULLCHK) {
    ...
    if (isEquivalent) {
      ...
      if (canBeRemoved && performTransformation(
                            comp(),
                            "%sCompacting checkcast [%p] and null check [%p]\n",
                            optDetailString(),
                            prevNode,
                            cursorNode)) {
    ...
    for (i = 0; i < cursorNode->getNumChildren(); i++) {
      ...
      if (!replaceNullCheckIfPossible(child,
                                      objectRef,
                                      prevNode,
                                      cursorNode,
                                      writtenSymbols,
...
static int32_t
compareValues(TR::Node* node1, TR::Node* node2)
{
...
int32_t
TR_ArraysetStoreElimination::perform()
{
  process(comp()->getStartTree(), NULL);
  return 0;
}
...
int32_t
TR_ArraysetStoreElimination::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
    process(block->getEntry(),
            block->getEntry()->getExtendedBlockExitTreeTop()->getNextTreeTop());
...
int32_t
TR_ArraysetStoreElimination::process(TR::TreeTop* startTree,
                                     TR::TreeTop* endTree)
{
  ...
  for (treeTop = startTree; (treeTop != endTree);
    ...
    TR_ASSERT(node->getOpCodeValue() == TR::BBStart,
              "Local Opts, expected BBStart treetop");
    TR::Block* block = node->getBlock();
    reduceArraysetStores(
      block, &readAndWrittenSymbols, &symbolsWrittenByArrayset, &scratch);
    exitTreeTop = block->getEntry()->getExtendedBlockExitTreeTop();
  }
...
void
TR_ArraysetStoreElimination::reduceArraysetStores(
  TR::Block* block,
  TR_BitVector* readAndWrittenSymbols,
  TR_BitVector* symbolsWrittenByArrayset,
  TR_BitVector* scratch)
{
  TR::TreeTop* prevTree = block->getEntry();
  TR::TreeTop* currentTree = block->getFirstRealTreeTop();
  TR::TreeTop* exitTree = block->getExit();
    ...
    if ((prevNode->getNumChildren() > 0) &&
      ...
      if (arraysetNode && (arraysetNode->getOpCodeValue() == TR::arrayset) &&
        ...
        while (!optimizedArraysetNode) {
          ...
          optimizedArraysetNode =
            optimizeArraysetIfPossible(cursorNode,
                                       addressChild,
                                       arraysetTree,
                                       NULL,
                                       readAndWrittenSymbols,
                                       symbolsWrittenByArrayset,
                                       scratch,
                                       initialVisitCount,
                                       currentTree);
      ...
      if ((prevNode->getNumChildren() > 0) &&
        ...
        TR::Node* cursorNode = currentTree->getNode();
        optimizeArraysetIfPossible(cursorNode,
                                   prevNode->getFirstChild()->getFirstChild(),
                                   prevTree,
                                   NULL,
                                   readAndWrittenSymbols,
                                   symbolsWrittenByArrayset,
...
bool
TR_ArraysetStoreElimination::optimizeArraysetIfPossible(
  TR::Node* currentNode,
  TR::Node* addressChild,
  TR::TreeTop* arraysetTree,
  TR::Node* currentParent,
  TR_BitVector* readAndWrittenSymbols,
  TR_BitVector* symbolsWrittenByArrayset,
  TR_BitVector* scratch,
  ...
  TR::TreeTop* currentTree)
{
    ...
    if ((arraycopyDestAddressNode == addressChild) ||
      ...
      *scratch &= *readAndWrittenSymbols;
      ...
      if (scratch->isEmpty() &&
          performTransformation(
            comp(),
            "%sEliminating some stores done by arrayset node %p because stores "
            "done by a following arraycopy node %p will overwrite them\n",
            optDetailString(),
            arraysetNode,
            currentNode)) {
        ...
        if (arraysetLenNode->getType().isInt32()) {
          ...
          subMaxNode = TR::Node::create(
            TR::imax, 2, subMaxNode, TR::Node::iconst(subMaxNode, 0));
        ...
        } else // if (arraysetLenNode->getType().isInt64())
        {
          ...
          subMaxNode = TR::Node::create(
            TR::lmax, 2, subMaxNode, TR::Node::lconst(subMaxNode, 0));
  ...
  for (i = 0; i < currentNode->getNumChildren(); ++i) {
    TR::Node* child = currentNode->getChild(i);
    if (optimizeArraysetIfPossible(child,
                                   addressChild,
                                   arraysetTree,
                                   currentNode,
                                   readAndWrittenSymbols,
...
int32_t
TR_SimplifyAnds::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
    process(block->getEntry(),
...
bool
isPowerOfTwo(TR::Compilation* comp, TR::Node* node)
{
...
static bool
isAndOfTwoFlags(TR::Compilation* comp,
                TR::Node* andNode,
                TR::Node* lastRealNode,
                TR::ILOpCodes ifOpCode,
...
int32_t
TR_SimplifyAnds::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
  comp()->incVisitCount();
  TR::TreeTop* treeTop = NULL;
  ...
  for (treeTop = startTree; (treeTop != endTree);
    ...
    if ((lastRealNode->getOpCode().isBranch() &&
      ...
      while (seenAndNode) {
        ...
        if (trace())
          traceMsg(comp(),
                   "Comparing current AND node %p with old AND node\n",
                   lastRealNode,
                   andNode);
            ...
            break;
                   ...
                   lastRealNode->getOpCode()
                     .isBooleanCompare()) // this code was allowing unsigned
                                          ...
                                          // stronger // NOTE that isCompare is
                                          // only true for signed compares --
        ...
        // (a&b && a&c) ==> ((a & b|c) == b|c)
                 ...
                 (isAndOfTwoFlags(
                    comp(), andNode, lastRealNode, TR::ificmpeq, TR::iand) ||
                  isAndOfTwoFlags(
                    comp(), andNode, lastRealNode, TR::iflcmpeq, TR::land))) {
          if (trace())
            traceMsg(
              comp(), "Found two iand nodes: %p %p\n", andNode, lastRealNode);
      ...
      if (!seenAndBefore) {
              ...
              if (firstNode->getOpCode().isStoreDirect() &&
                ...
                if (areTreesInRequiredForm &&
                  ...
                  TR::Node* newIShrNode = TR::Node::create(
                    (lastFirstChild->getType().isInt32()) ? TR::ishr : TR::lshr,
                    2,
                    lastFirstChild,
                    TR::Node::create(
                      lastRealNode,
                  ...
                  TR::Node* newIAndNode = TR::Node::create(
                    firstNode->getType().isInt32() ? TR::iand : TR::land,
                    2,
                    secondGrandChild,
                    newIShrNode);
        ...
        if (!removingConditional) {
          ...
          noSideEffectsInBetween = true;
                   ...
                   comp(),
                   "%sRemoving conditional node %p (condition has been checked "
                   "before in this extended BB)\n",
                   optDetailString(),
                   lastRealNode)) {
...
void
TR_EliminateRedundantGotos::renumberInAncestors(TR_Structure* str, int32_t num)
{
...
void
TR_EliminateRedundantGotos::renumberExitEdges(TR_RegionStructure* region,
                                              int32_t oldNum,
                                              int32_t newNum)
{
...
void
TR_EliminateRedundantGotos::placeAsyncCheckBefore(TR::TreeTop* tt)
{
  TR::TreeTop* actt = TR::TreeTop::create(
    comp(),
    TR::Node::createWithSymRef(tt->getNode(),
                               TR::asynccheck,
                               0,
...
int32_t
TR_EliminateRedundantGotos::perform()
{
  process(comp()->getStartTree(), NULL);
  return 0;
}
...
int32_t
TR_EliminateRedundantGotos::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
    process(block->getEntry(),
            block->getEntry()->getExtendedBlockExitTreeTop()->getNextTreeTop());
...
int32_t
TR_EliminateRedundantGotos::process(TR::TreeTop* startTree,
                                    TR::TreeTop* endTree)
{
  // The CFG must exist for this optimization.
  //
  ...
  for (TR::TreeTop *treeTop = startTree, *exitTreeTop; (treeTop != endTree);
    ...
    if (emptyBlock &&
        !performTransformation(comp(),
                               "%sRemoving empty block_%d with BBStart %p\n",
                               optDetailString(),
                               block->getNumber(),
    ...
    if (manuallyFixStructure) {
      ...
      redirectPredecessors(
        block, destBlock, fixablePreds, emptyBlock, asyncMessagesFlag);
...
void
TR_EliminateRedundantGotos::redirectPredecessors(TR::Block* block,
                                                 TR::Block* destBlock,
                                                 ...
                                                 bool asyncMessagesFlag)
{
  ...
  if (regdepsAreOutgoing) {
    ...
    if (exitNode->getNumChildren() > 0) {
      ...
      TR_ASSERT_FATAL(
        emptyBlock, "expected block_%d to be empty\n", block->getNumber());
  ...
  for (auto edge = preds.begin(); edge != preds.end(); ++edge) {
    ...
    if (predBlock->getLastRealTreeTop()->getNode()->getOpCode().isBranch() &&
        predBlock->getLastRealTreeTop()->getNode()->getBranchDestination() ==
          block->getEntry()) {
      predBlock->changeBranchDestination(destBlock->getEntry(),
                                         cfg,
                                         /* callerFixesRegdeps = */ true);
      predExitTree = predBlock->getLastRealTreeTop();
    } else {
      predBlock->redirectFlowToNewDestination(
        comp(), current, destBlock, false);
...
void
TR_EliminateRedundantGotos::fixPredecessorRegDeps(TR::Node* regdepsParent,
                                                  TR::Block* destBlock)
{
...
int32_t
TR_CleanseTrees::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
    process(block->getEntry(),
            block->getEntry()->getExtendedBlockExitTreeTop()->getNextTreeTop());
...
void
TR_CleanseTrees::prePerformOnBlocks()
{
  // TR_Structure *rootStructure = NULL;
  //
  // Set up the block nesting depths
...
int32_t
TR_CleanseTrees::process(TR::TreeTop* startTree, TR::TreeTop* endTreeTop)
{
  if (comp()->getProfilingMode() == JitProfiling)
    return 0;
  ...
  for (treeTop = startTree; (treeTop != endTreeTop);
    ...
    if (tt2 && node->getBranchDestination() == tt2) {
      ...
      if ((tt1->getNode()->getOpCodeValue() == TR::Goto ||
        ...
        requestOpt(
          OMR::treeSimplification, true, exitTreeTop->getNode()->getBlock());
        // cleansedTrees = true;
        if (lastNonFenceTree->getNode()->getOpCodeValue() != TR::Goto) {
...
int32_t
TR_ProfiledNodeVersioning::perform()
{
#ifdef J9_PROJECT_SPECIFIC
      ...
      switch (node->getOpCodeValue()) {
        ...
        case TR::anewarray: {
          ...
          if (numElementsInfo) {
            ...
            if (doProfiling &&
                performTransformation(comp(),
                                      "O^O PROFILED NODE VERSIONING: "
                                      "Versioning %s %s, size %s %d %.01f%%\n",
              ...
              block->createConditionalBlocksBeforeTree(tt,
                                                       compareTree,
                                                       slowTree,
                                                       fastTree,
                                                       comp()->getFlowGraph(),
            ...
            traceMsg(comp(),
                     "No profiling info for size of %s node %s\n",
                     node->getOpCode().getName(),
                     comp()->getDebug()->getName(node));
          }
        } break;
...
void
TR_Rematerialization::rematerializeAddresses(TR::Node* indirectNode,
                                             TR::TreeTop* treeTop,
                                             vcount_t visitCount)
{
      ...
      while (++throttle < 10) {
        if ((node->getReferenceCount() > 1 && node->isInternalPointer() &&
          ...
          if (performTransformation(comp(),
                                    ...
                                    node,
                                    node->getOpCode().getName())) {
            TR::Node* newChild = TR::Node::create(node->getOpCodeValue(),
                                                  2,
                                                  node->getFirstChild(),
                                                  node->getSecondChild());
            ...
            if (secondChild->getReferenceCount() > 1) {
              ...
              if (performTransformation(comp(),
                ...
                TR::Node* newSecondChild =
                  TR::Node::create(secondChild->getOpCodeValue(),
                                   2,
        ...
        if (child->getReferenceCount() > 1) {
          if (performTransformation(comp(),
            ...
            TR::Node* newChild = TR::Node::create(
              child->getOpCodeValue(), 1, child->getFirstChild());
...
int32_t
TR_Rematerialization::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
    process(block->getEntry(),
            block->getEntry()->getExtendedBlockExitTreeTop());
...
void
TR_Rematerialization::prePerformOnBlocks()
{
  _counter = 0;
  _curBlock = NULL;
}
...
TR::Node*
rematerializeNode(TR::Compilation* comp, TR::Node* node)
{
...
int32_t
TR_Rematerialization::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
  ...
  for (treeTop = startTree; (treeTop != endTree);
       treeTop = treeTop->getNextTreeTop())
    initializeFutureUseCounts(
      treeTop->getNode(), NULL, visitCount, comp(), _heightArray);
...
bool
TR_Rematerialization::isRematerializable(TR::Node* parent,
                                         TR::Node* node,
...
bool
TR_Rematerialization::isRematerializableLoad(TR::Node* node, TR::Node* parent)
{
...
static bool
storeMightKillRematerializableSym(TR::Node* node)
{
...
void
TR_Rematerialization::removeNodeFromList(
  TR::Node* node,
  List<TR::Node>* nodes,
...
void
TR_Rematerialization::addParentToList(TR::Node* node,
                                      List<TR::Node>* nodes,
                                      TR::Node* parent,
                                      List<List<TR::Node>>* parents)
{
...
void
TR_Rematerialization::findSymsUsedInIndirectAccesses(
  TR::Node* node,
  List<TR::Node>* nodes,
  List<List<TR::Node>>* parents)
{
...
bool
TR_Rematerialization::examineNode(TR::TreeTop* treeTop,
                                  TR::Node* parent,
                                  TR::Node* node,
                                  vcount_t visitCount,
                                  TR_RematState* state,
                                  TR_RematAdjustments& adjustments)
{
    ...
    if (node->getOpCode().isFloatingPoint()) {
      ...
      if (isRematerializable(parent, node, true) &&
        ...
        if (isRematerializableLoad(node, parent))
          ...
          removeNodeFromList(node,
                             &state->_currentlyCommonedFPCandidates,
                             &state->_fpParents,
                             false);
      }
      ...
      if ((isRematerializable(parent, node, true) &&
        ...
        if (isRematerializableLoad(node, parent) || _prefetchNodes.find(node))
          ...
          removeNodeFromList(node,
                             &state->_currentlyCommonedCandidates,
                             &state->_parents,
                             false);
      }
    }
  ...
  if (((((node->getFutureUseCount() & 0x7fff) > 0) &&
    ...
    if (node->getOpCode().isFloatingPoint()) {
      ...
      if (isRematerializableLoad(node, parent))
        ...
        addParentToList(
          node, &state->_currentlyCommonedCandidates, parent, &state->_parents);
    }
  }
  ...
  if (trace())
    traceMsg(comp(),
             "Parent adjust %d node %p parent %p\n",
  ...
  */
  ...
  if (trace()) {
    ...
    traceMsg(comp(),
             "candidate nodes size %d candidate loads size %d\n",
  ...
  if ((!considerRegPressure ||
    ...
    rematerializeNode(treeTop,
                      parent,
                      node,
                      visitCount,
                      &state->_currentlyCommonedNodes,
                      &state->_currentlyCommonedCandidates,
                      &state->_parents,
                      &state->_currentlyCommonedLoads,
                      &state->_parentsOfCommonedLoads,
  ...
  if ((!considerRegPressure ||
    ...
    rematerializeNode(treeTop,
                      ...
                      node,
                      visitCount,
                      &state->_currentlyCommonedFPNodes,
                      &state->_currentlyCommonedFPCandidates,
                      &state->_fpParents,
                      &state->_currentlyCommonedFPLoads,
                      &state->_parentsOfCommonedFPLoads,
  ...
  if (trace() && node->getOpCode().isVector()) {
    traceMsg(
      comp(),
      "At node %p VSR pressure is %d (child adjust %d parent adjust %d) limit "
      "is %d\n",
      node,
...
void
TR_Rematerialization::rematerializeNode(
  TR::TreeTop* treeTop,
  TR::Node* parent,
  TR::Node* node,
  vcount_t visitCount,
  List<TR::Node>* currentlyCommonedNodes,
  List<TR::Node>* currentlyCommonedCandidates,
  List<List<TR::Node>>* parents,
  ...
  bool rematSpecialNode)
{
  if (trace())
    traceMsg(
      comp(), "rematerializeNode: parent = %p node = %p\n", parent, node);
  ...
  if (isAdjunctNode)
    traceMsg(comp(),
             "Prevented adjunct node %p from being rematerialized\n",
             nodeToBeRematerialized);
  ...
  if (!isAdjunctNode &&
    ...
    if (nodeToBeRematerialized->getNumChildren() > 0) {
      ...
      if (isRematerializable(nodeToBeRematerialized, firstChild) &&
        ...
        for (int32_t j = 0; j < newFirstChild->getNumChildren(); j++) {
          ...
          parentList = getParentList(newFirstChild->getChild(j),
                                     parentsOfCommonedLoads,
    ...
    for (int32_t j = 0; j < newNode->getNumChildren(); j++) {
      ...
      List<TR::Node>* parentList = getParentList(
        newNode->getChild(j), parents, currentlyCommonedCandidates);
      if (parentList) {
      ...
      parentList = getParentList(
        newNode->getChild(j), parentsOfCommonedLoads, currentlyCommonedLoads);
      if (parentList) {
        parentList->add(newNode);
        parentList->remove(nodeToBeRematerialized);
      }
      ...
      if (parentNode /* &&
        ...
        for (int32_t j = 0; j < parentNode->getNumChildren(); j++) {
          if (parentNode->getChild(j) == nodeToBeRematerialized) {
            ...
            parentList2 = getParentList(nodeToBeRematerialized,
                                        parentsOfCommonedLoads,
                                        currentlyCommonedLoads);
            if (parentList2)
    ...
    if (!mustBeAnchored)
      ...
      parentList = getParentList(
        nodeToBeRematerialized, parentsOfCommonedLoads, currentlyCommonedLoads);
      if (parentList)
        parentList->add(treetopNode);
      ...
      optimizer()->setRequestOptimization(
        OMR::deadTreesElimination, true, _curBlock);
      nodeToBeRematerialized->setVisitCount(visitCount - 1);
    }
...
TR_RegionStructure*
TR_BlockSplitter::getParentStructure(TR::Block* block)
{
...
bool
TR_BlockSplitter::disableSynergy()
{
...
int32_t
TR_BlockSplitter::perform()
{
  if (trace() && disableSynergy())
    traceMsg(comp(),
             "Block splitting hotness = %d, method = %s\n",
  ...
  while (children->size() > 0) {
    ...
    for (auto edge = children->top()->getSuccessors().begin();
         edge != children->top()->getSuccessors().end();
         ++edge) {
    ...
    if (isLoopHeader(mergeNode)) {
      if (trace())
        traceMsg(comp(),
                 "    rejecting loop header block_%d\n",
                 mergeNode->getNumber());
      children->pop();
      continue;
    }
    if (hasIVUpdate(mergeNode)) {
      if (trace())
        traceMsg(comp(),
                 "    reject merge block_%d: IV update\n",
                 mergeNode->getNumber());
      children->pop();
      continue;
    }
    if (hasLoopAsyncCheck(mergeNode)) {
      if (trace())
        traceMsg(comp(),
                 "    reject merge block_%d: asynccheck\n",
                 mergeNode->getNumber());
      children->pop();
      continue;
    }
    ...
    if (mergeNode->getNumber() == skipBlock) {
      if (trace())
        traceMsg(comp(),
                 "     rejecting block_%d, magic skip block found\n",
                 mergeNode->getNumber());
      children->pop();
      continue;
    }
    ...
    if (disableSynergy() && comp()->isOptServer() &&
      ...
      if (trace())
        traceMsg(comp(),
                 "    opt server mode for mergeNode %d\n",
                 mergeNode->getNumber());
      ...
      for (auto edge = mergeNode->getPredecessors().begin();
           edge != mergeNode->getPredecessors().end();
           ++edge) {
        node = (*edge)->getFrom();
  ...
  heapElementQuickSort(
    (TR_Array<TR_IndexedBinaryHeapElement*>*)heap, 0, heap->lastIndex());
  if (trace()) {
  ...
  for (int32_t i = 0; i <= array->lastIndex(); ++i) {
    ...
    if (!splitPred || (splitPred->getFrequency() < splitPredFrequency) ||
      ...
      if (trace())
        traceMsg(comp(),
                 "Merge block_%d has no suitable predecessor\n",
                 mergeNode->getNumber());
      continue;
    }
    ...
    bMap.set(0, 0);
    TR::Block* to =
      new (trHeapMemory()) TR::Block(*mergeNode,
    ...
    while (depth > 1) {
      if (trace())
        traceMsg(comp(),
                 "  Processing block_%d, depth budget %d\n",
                 target->getNumber(),
                 depth);
      ...
      for (auto edge = target->getSuccessors().begin();
        ...
        if (!child->getEntry()) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: has no trees\n",
                     child->getNumber());
          continue;
        }
        ...
        if (child->getFrequency() <= (target->getFrequency() - tollerance)) {
          if (trace())
            traceMsg(
              comp(),
              "    Reject successor block_%d: frequency is too low (%d < %d)\n",
              child->getNumber(),
              child->getFrequency(),
        ...
        if (child->hasExceptionPredecessors()) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: catch block\n",
                     child->getNumber());
          continue;
        }
        ...
        if (isLoopHeader(toCheck)) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: loop header\n",
                     child->getNumber());
          continue;
        }
        ...
        if (hasIVUpdate(toCheck)) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: IV update\n",
                     child->getNumber());
          continue;
        }
        ...
        if (hasLoopAsyncCheck(toCheck)) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: asynccheck\n",
                     child->getNumber());
          continue;
        }
        ...
        if (isExitEdge(target, toCheck)) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: outside of loop\n",
                     child->getNumber());
          continue;
        }
        ...
        if (trace())
          traceMsg(
            comp(), "  Split through successor block_%d\n", child->getNumber());
    ...
    if (pruneAndPopulateBlockMapper(
      ...
      if (hotFreq < splitPred->getFrequency()) {
        ...
        if (force || splitPred->getSuccessors().size() == 1) {
          TR::DebugCounter::incStaticDebugCounter(
            ...
            TR::DebugCounter::debugCounterName(
              comp(),
              "blockSplitter.inconsistentFreqs/%s/(%s)/block_%d",
              comp()->getHotnessName(comp()->getMethodHotness()),
              comp()->signature(),
              mergeNode->getNumber()));
      ...
      if (trace()) {
        traceMsg(comp(),
                 "  New block frequencies are cold: %d hot: %d\n",
                 coldFreq,
                 hotFreq);
        traceMsg(comp(),
                 "  New edge frequencies are cold: %d hot: %d\n",
                 coldEdgeFreq,
                 hotEdgeFreq);
      }
      for (BlockMapper* itr = bMap.getFirst(); itr; itr = itr->getNext()) {
        if (trace())
          traceMsg(comp(),
                   "    setting block_%d cold and block_%d hot\n",
                   itr->_from->getNumber(),
                   itr->_to->getNumber());
        ...
        if (origFreq > 0) {
          ...
          for (auto predEdge = predecessors.begin();
            ...
            if (trace())
              traceMsg(comp(),
                       "predEdge freq=%d. set it to %d instead\n",
          ...
          for (auto succEdge = successors.begin(); succEdge != successors.end();
            ...
            if (trace())
              traceMsg(comp(),
                       "succEdge freq=%d. set it to %d instead\n",
...
void
TR_BlockSplitter::heapElementQuickSort(
  TR_Array<TR_IndexedBinaryHeapElement*>* array,
  int32_t left,
  int32_t right)
{
...
void
TR_BlockSplitter::quickSortSwap(TR_Array<TR_IndexedBinaryHeapElement*>* array,
                                int32_t left,
                                int32_t right)
{
  TR_IndexedBinaryHeapElement* temp;
...
int32_t
TR_BlockSplitter::pruneAndPopulateBlockMapper(
  TR_LinkHeadAndTail<BlockMapper>* bMap,
  int32_t depth)
{
  int32_t cloned = 0;
  if (depth) {
    if (trace()) {
      for (BlockMapper* itr = bMap->getFirst(); itr; itr = itr->getNext()) {
        traceMsg(comp(),
                 "prune bMap iterator, from 0x%p to 0x%p\n",
                 itr->_from,
                 itr->_to);
      }
    }
    ...
    for (itr = bMap->getFirst(); itr; itr = itr->getNext()) {
      ...
      if (trace())
        traceMsg(comp(),
                 "prune bMap iterator for join, from 0x%p to 0x%p\n",
                 itr->_from,
                 itr->_to);

      itr->_to =
        new (trHeapMemory()) TR::Block(*itr->_from,
                                       TR::TreeTop::create(comp(), NULL),
  ...
  if (trace())
    traceMsg(
      comp(), "  pruneAndPopulateBlockMapper returning depth of %d\n", cloned);
  return cloned;
}

int32_t
TR_BlockSplitter::synergisticDepthCalculator(
  TR_LinkHeadAndTail<BlockMapper>* bMap,
  TR::Block* startPoint)
{
  ...
  for (BlockMapper* itr = bMap->getFirst(); itr; itr = itr->getNext()) {
    for (TR::TreeTop* treeTopItr = itr->_from->getEntry();
         ...
         treeTopItr = treeTopItr->getNextTreeTop()) {
      cost += processNode(
        treeTopItr->getNode(), blockIndex, &synergyIndices, &synergies);
    }
...
int32_t
TR_BlockSplitter::processNode(TR::Node* node,
                              int32_t blockIndex,
...
double
TR_BlockSplitter::calculateBlockSplitScore(Synergy& synergy)
{
...
void
TR_BlockSplitter::dumpSynergies(TR_Array<Synergy>* toDump)
{
  if (trace()) {
    traceMsg(
      comp(),
    ...
    for (uint32_t i = 2; i < toDump->size(); ++i)
      traceMsg(comp(),
               ...
               (*toDump)[i].upwardSynergy,
               (*toDump)[i].downwardSynergy,
               (*toDump)[i].replicationCost,
               (*toDump)[i].blockFrequency);
  }
}

void
TR_BlockSplitter::dumpBlockMapper(TR_LinkHeadAndTail<BlockMapper>* bMap)
{
  if (trace()) {
    for (BlockMapper* itr = bMap->getFirst(); itr; itr = itr->getNext()) {
      if (itr == bMap->getFirst())
        traceMsg(comp(),
                 "    Splitting block_%d for %s",
                 itr->_from->getNumber(),
                 comp()->signature());
...
bool
TR_BlockSplitter::containCycle(TR::Block* blk,
                               TR_LinkHeadAndTail<BlockMapper>* bMap)
{
  for (auto edge = blk->getSuccessors().begin();
       edge != blk->getSuccessors().end();
       ++edge) {
...
bool
TR_BlockSplitter::isLoopHeader(TR::Block* block)
{
  TR::CFG* cfg = comp()->getFlowGraph();
  TR::CFGEdge* edge;
  TR::Block* next;
...
bool
TR_BlockSplitter::isExitEdge(TR::Block* mergeNode, TR::Block* successor)
{
  if (trace())
    traceMsg(comp(),
             "    considering isExit on %d and %d\n",
             mergeNode->getNumber(),
             successor->getNumber());
  ...
  if (trace())
    traceMsg(comp(),
             "    parent region is %p (%d) and isNaturalLoop is %d\n",
             parent,
             parent->getNumber(),
...
bool
TR_BlockSplitter::hasIVUpdate(TR::Block* block)
{
  ...
  if (trace())
    traceMsg(
      comp(), "   checking for IVUpdate in block_%d\n", block->getNumber());
...
bool
TR_BlockSplitter::hasLoopAsyncCheck(TR::Block* block)
{
  TR_RegionStructure* parent = getParentStructure(block);
  if (getLastRun() || comp()->getProfilingMode() == JitProfiling || !parent ||
      !parent->isNaturalLoop())
  ...
  if (trace())
    traceMsg(comp(),
             "   checking for loopAsyncCheck in block_%d\n",
             block->getNumber());
  for (OMR::TreeTop* treeTopItr = block->getEntry();
       treeTopItr && treeTopItr != block->getExit()->getNextTreeTop();
       treeTopItr = treeTopItr->getNextTreeTop()) {
...
TR::Block*
TR_BlockSplitter::splitBlock(TR::Block* pred,
                             TR_LinkHeadAndTail<BlockMapper>* bMap)
{
      ...
      if (lastRealNode->getOpCode().isIf()) {
        if (lastRealNode->getBranchDestination()
          ...
          if (bMap->getLast()->_from->getNumber() == pred->getNumber()) {
            lastRealNode =
              bMap->getLast()->_to->getExit()->getPrevRealTreeTop()->getNode();
            lastRealNode->reverseBranch(cloneEnd->getExit()->getNextTreeTop());
            if (trace())
              traceMsg(comp(),
...
void
TR_InvariantArgumentPreexistence::ParmInfo::clear()
{
...
TR_InvariantArgumentPreexistence::perform()
{
...

  {
    ...
    if (trace())
      traceMsg(comp(),
               "PREX: Starting preexistence for %s\n",
               feMethod->signature(trMemory()));

    int32_t numParms = methodSymbol->getParameterList().getSize();
    _parmInfo =
      (ParmInfo*)trMemory()->allocateStackMemory(numParms * sizeof(ParmInfo));
    ...
    if (_isOutermostMethod) {
      ...
      if (p && p->getOffset() == 0 && !feMethod->isStatic()) {
        ...
        if (trace())
          traceMsg(
            comp(), "PREX:      Receiver class %p is %.*s\n", clazz, len, sig);
      ...
      for (; p != NULL; index++, p = parms.getNext()) {
        ...
        if (*sig == 'L') {
          ...
          if (clazz && !fe()->classHasBeenExtended(clazz)) {
            ...
            if (trace())
              traceMsg(
                comp(), "PREX:      Parm %d class is currently final\n", index);
          }
          _parmInfo[index].setClass(clazz);
          if (trace())
            traceMsg(comp(),
                     "PREX:      Parm %d class %p is %.*s\n",
                     index,
                     clazz,
                     len,
                     sig);
      ...
      if (argInfo) {
        ...
        for (TR::ParameterSymbol* p = parms.getFirst(); p != NULL;
          ...
          if (arg) {
            if (trace())
              traceMsg(comp(),
                       "PREX:      Parm %d is arg %p parmInfo %p\n",
                       index,
                       arg,
                       &parmInfo);
          } else {
          ...
          if (classIsFixed || classIsPreexistent) {
            ...
            TR_ASSERT(!clazz || !clazzFromMethod || clazz == clazzFromMethod ||
                        fe()->isInstanceOf(
                          clazz, clazzFromMethod, true, true, true) == TR_yes,
          ...
          if (arg->hasKnownObjectIndex()) {
            ...
            if (trace())
              traceMsg(comp(),
                       "PREX:        Parm %d is known object obj%d\n",
                       index,
    ...
    if (trace())
      traceMsg(comp(),
               "PREX:    Setting parm symbol info for %s\n",
               feMethod->signature(trMemory()));
...
void
TR_InvariantArgumentPreexistence::processNode(TR::Node* node,
                                              TR::TreeTop* treeTop,
                                              vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
...
void
TR_InvariantArgumentPreexistence::processIndirectCall(TR::Node* node,
                                                      TR::TreeTop* treeTop,
                                                      vcount_t visitCount)
{
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (methodSymbol->isComputed()) {
    ...
    if (methodSymbol->getRecognizedMethod() ==
      ...
      specializeInvokeExactSymbol(
        node, receiverInfo.getKnownObjectIndex(), comp(), this);
    ...
    if (refinedMethod) {
      ...
      if (!needsAssumptions || addAssumptions) {
        if (!refinedMethod->isSameMethod(resolvedMethod)) {
          if (performTransformation(
                comp(),
                "%sspecialize and devirtualize invoke [%p] on currently fixed "
                "or final parameter %d [%p]\n",
                optDetailString(),
                node,
                receiverParmOrdinal,
                receiverSymbol)) {
...
void
TR_InvariantArgumentPreexistence::processIndirectLoad(TR::Node* node,
                                                      TR::TreeTop* treeTop,
                                                      vcount_t visitCount)
{
...
int32_t
TR_CheckcastAndProfiledGuardCoalescer::perform()
{
...
TR::Node*
TR_CheckcastAndProfiledGuardCoalescer::storeObjectInATemporary(
  TR::TreeTop* checkcastTree)
{
  ...
  TR::SymbolReference* tempCheckcastSymRef =
    comp()->getSymRefTab()->createTemporary(
      comp()->getMethodSymbol(), obj->getDataType(), false);
...
void
TR_CheckcastAndProfiledGuardCoalescer::doBasicCase(
  TR::TreeTop* checkcastTree,
  TR::TreeTop* profiledGuardTree)
{
  TR::Block* slowPathBlock =
    profiledGuardTree->getNode()->getBranchDestination()->getNode()->getBlock();
  ...
  TR::Node* newCheckcastNode = TR::Node::createWithSymRef(
    TR::checkcast,
    2,
    TR::Node::createLoad(storeNode, tempCheckcastSymRef),
    classConst,
    0,
  ...
  for (TR::AllBlockIterator iter(optimizer()->getMethodSymbol()->getFlowGraph(),
    ...
    if (block->isCold()) {
      ...
      if ((coldness <= MAX_COLD_BLOCK_COUNT) &&
          ((!controlled) || performTransformation(comp(),
                                                  "%s%s marked block_%d cold\n",
                                                  optDetailString(),
                                                  name(),
                                                  block->getNumber()))) {
        block->setIsCold();
...
static bool
coldBlock(TR::Block* block, TR::Compilation* comp)
{
...
TR_ColdBlockOutlining::reorderColdBlocks()
{
  ...
  if (trace())
    traceMsg(comp(),
             "Cold Block Outlining: outlined %d cold blocks so far:\n",
             numBlocksSoFar);
}
...
TR::Block*
TR_BlockManipulator::breakFallThrough(TR::Block* faller,
                                      TR::Block* fallee,
...
void
TR_TrivialDeadTreeRemoval::preProcessTreetop(
  TR::TreeTop* treeTop,
  List<TR::TreeTop>& commonedTreeTopList,
  char* optDetails,
  TR::Compilation* comp)
{
  ...
  if (ttNode->getOpCodeValue() == TR::treetop &&
    ...
    if (firstChild->getReferenceCount() == 1) {
      if (!firstChild->getOpCode().hasSymbolReference() &&
          performTransformation(
            comp,
            "%sUnlink trivial %s (%p) of %s (%p) with refCount==1\n",
            optDetails,
...
void
TR_TrivialDeadTreeRemoval::postProcessTreetop(
  TR::TreeTop* treeTop,
  List<TR::TreeTop>& commonedTreeTopList,
  char* optDetails,
  TR::Compilation* comp)
...
void
TR_TrivialDeadTreeRemoval::processCommonedChild(
  TR::Node* child,
  TR::TreeTop* treeTop,
  List<TR::TreeTop>& commonedTreeTopList,
  char* optDetails,
  ...
  if (child->getReferenceCount() > 1) {
    if (!commonedTreeTopList.isEmpty()) {
        ...
        if (listTT->getNode()->getFirstChild() == child) {
          if (listTT != treeTop) {
            ...
            if (treeTop->getNode()->getNumChildren() == 1 &&
              ...
              if (performTransformation(
                    comp,
                    "%sFound commoned reference to single child %s (%p) case 1 "
                    "so unlink %s (0x%p)\n",
                    optDetails,
                    child->getOpCode().getName(),
                    child,
      ...
      if (treeTop->getNode()->getNumChildren() == 1 &&
          ...
          treeTop->getNode()->getFirstChild() == child) {
        if (performTransformation(comp,
                                  "%sFound commoned reference to single child "
                                  "%s (%p) case 2 so unlink %s (0x%p)\n",
                                  optDetails,
                                  child->getOpCode().getName(),
                                  child,
                                  treeTop->getNode()->getOpCode().getName(),
...
TR_TrivialDeadTreeRemoval::examineNode(TR::Node* node, vcount_t visitCount)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (child->getVisitCount() != visitCount) {
      ...
      processCommonedChild(
        child, _currentTreeTop, _commonedTreeTopList, OPT_DETAILS_TDTR, comp());
    }
  }
...
void
TR_TrivialDeadTreeRemoval::transformBlock(TR::TreeTop* entryTree,
                                          TR::TreeTop* exitTree)
{
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  ...
  for (TR::TreeTop* currentTree = entryTree->getNextRealTreeTop();
    ...
    preProcessTreetop(
      currentTree, _commonedTreeTopList, OPT_DETAILS_TDTR, comp());
...
int32_t
TR_TrivialBlockExtension::performOnBlock(TR::Block* block)
{
  ...
  if (block->isExtensionOfPreviousBlock()) {
    ...
    if (pred != block->getPrevBlock()) {
      ...
      if (performTransformation(
            comp(),
            "O^O BLOCK EXTENSION: Mark block_%d as an extension of block_%d\n",
            block->getNumber(),
            pred->getNumber()))
        block->setIsExtensionOfPreviousBlock();
    }
    ...
    if (trace())
      traceMsg(comp(),
               "BlockExtension: block_%d has %d predecessors\n",
               block->getNumber(),
