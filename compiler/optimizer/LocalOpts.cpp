...
int32_t TR_ExtendBasicBlocks::orderBlocksWithoutFrequencyInfo()
{
    ...
    for (; block; prevBlock = block, block = block->getNextBlock()) {
        ...
        if (cannotExtendWithCurrentFallThrough || (bestExtension && !bestExtension->isCold() && (bestExtension != block))) {
            //
        ...
        if (prevNode->getOpCodeValue() == TR::Goto) {
        ...
        } else if ((firstNonFenceTree == lastNonFenceTree) && (lastNonFenceTree->getNode()->getOpCodeValue() == TR::Goto)) {
            ...
            if (nextBlock && prevNode->getOpCode().isBranch() && !prevNode->isNopableInlineGuard()) {
                ...
                if (prevNode->getBranchDestination() == nextBlock->getEntry() && !nextBlock->isCold()) {
                    ...
                    continue;
                } else if (prevBranchDest->getVisitCount() < visitCount && prevBranchDest->getPredecessors().size() == 1) {
...
int32_t TR_ExtendBasicBlocks::orderBlocksWithFrequencyInfo()
{
    ...
    for (block = comp()->getStartTree()->getNode()->getBlock(); block;
        ...
        } else if (!node->getOpCode().isReturn() && !node->getOpCode().isJumpWithMultipleTargets() && node->getOpCodeValue() != TR::athrow) {
            entry->fallThrough = block->getNextBlock();
        }
    }
            ...
            for (auto edge = currentBlockEntry->block->getSuccessors().begin(); edge != currentBlockEntry->block->getSuccessors().end(); ++edge) {
                ...
                if (edgeTo != i && sequences[edgeTo].first) {
                    ...
                    if ((*edge)->getTo()->getPredecessors().size() == 1) {
                        ...
                        if (block->getExit() && !(block->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets())) {
                            edgeFrequency += extendedBlockPreference;
                        }
                    }
                    if ((edgeFrequency > bestFrequency) || ((edgeFrequency == bestFrequency) && (currentBlockEntry->fallThrough == (*edge)->getTo()))) {
...
TR::Block* TR_BlockManipulator::getBestChoiceForExtension(TR::Block* block1)
{
    ...
    if (block1->getLastRealTreeTop()->getNode()->getOpCode().isBranch() && block1->getLastRealTreeTop()->getNode()->isNopableInlineGuard())
    ...
    for (auto nextEdge = block1->getSuccessors().begin(); nextEdge != block1->getSuccessors().end(); ++nextEdge) {
        ...
        if (nextHotness > hotness) {
            ...
            if ((nextBlockNestingDepth > nestingDepth) || ((nextBlockNestingDepth == nestingDepth) && (nextBlockNumTrees > numberOfRealTreesInBlock))) {
                bestExtension = nextBlock;
...
bool TR_BlockManipulator::isBestChoiceForFallThrough(TR::Block* block1, TR::Block* block2)
{
    ...
    if (block1->isSuperCold() || ((block1->getSuccessors().size() == 1) && block1->getSuccessors().front()->getTo()->asBlock()->isSuperCold()))
        return false;
    ...
    TR::TreeTop* endTreeTop, vcount_t visitCount, TR::Optimizer* optimizer)
{
    ...
    if (newBlock->getVisitCount() < visitCount) {
        ...
        for (cursor = newBlock, cursorNext = cursor->getNextBlock();
             cursorNext && (cursor->hasSuccessor(cursorNext) || insideLoop(comp(), cursor, cursorNext, block)) && (!cursorNext->isCold() || cursorNext->isExtensionOfPreviousBlock() || cursor->getLastRealTreeTop()->getNode()->isNopableInlineGuard());
      ...
      */
        ...
        if (
            endOfTreesBeingMoved && !prevNode->isNopableInlineGuard() && performTransformation(comp(), "%sswing down block_%d to maximize fall through with block_%d\n", optDetailString(), newBlock->getNumber(), prevBlock->getNumber())) {
            ...
            if ( //comp()->cg()->allowBlockToBeReordered(newBlock) &&
                lastNode->getOpCode().isBooleanCompare() && !lastNode->isNopableInlineGuard() && !nextOfNewBlock->isExtensionOfPreviousBlock() && performTransformation(comp(), "%sswing down block_%d and break original fall through to join with block_%d\n", optDetailString(), newBlock->getNumber(), prevBlock->getNumber())) {
    ...
    for (TR::TreeTop *treeTop = startTree, *exitTreeTop; (treeTop != endTree); treeTop = exitTreeTop->getNextTreeTop()) {
        ...
        if ((node1->getOpCode().isReturn() ||
#ifdef J9_PROJECT_SPECIFIC
                (node1->getOpCodeValue() == TR::athrow && (catchInfo = TR_CatchBlockProfileInfo::get(comp())) && catchInfo->getCatchCounter() >= 100) ||
#endif
                (node1->getOpCode().isBranch() && (node1->getOpCodeValue() != TR::Goto)) || ((node1->getOpCodeValue() == TR::Goto) && (!(block->getPredecessors().size() == 1))))) {
            ...
            for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end();) {
                ...
                if ((prevBlock->isCold()) || (prevBlock == cfgStart) || (prevBlock == block) || (exceptionEdgesMatter && cfg->compareExceptionSuccessors(block, prevBlock))) {
                    nextEdge = next;
                    continue;
                };
               ...
               */
                    ...
                    if (synergyExists && performTransformation(comp(), "%sHoist basic block [%d] into predecessor block [%d]\n", optDetailString(), block->getNumber(), prevBlock->getNumber())) {
                        //debugCounter++;
                        //if (debugCounter > 4)
                        // return 1;
...
static void initializeFutureUseCounts(TR::Node* node, TR::Node* parent, vcount_t visitCount, TR::Compilation* comp, int32_t* heightArray)
{
    ...
    if ((node->getOpCode().hasSymbolReference() && node->getOpCode().isIndirect()) || (node->getOpCode().isArrayLength()))
        indirectAccess = true;
...
void TR_CompactNullChecks::compactNullChecks(TR::Block* block, TR_BitVector* writtenSymbols)
{
    ...
    while (currentTree != stopTree) {
        ...
        // a null check
        //
        if ((prevNode->getOpCodeValue() == TR::NULLCHK) && (prevNode->getFirstChild()->getOpCodeValue() == TR::PassThrough)) {
            ...
            if (referenceChild == prevNode->getFirstChild()->getFirstChild()) {
                ...
                while (!replacedPassThroughNode) {
                    ...
                    if (replacedPassThroughNode && (cursorNode->getOpCodeValue() == TR::NULLCHK) && (cursorNode->getNullCheckReference() == prevNode->getNullCheckReference()))
                        TR::Node::recreate(cursorNode, TR::treetop);

                    // set flag to remove tree if treetop contains vcall and ivcall
                    if (replacedPassThroughNode && cursorNode->getOpCodeValue() == TR::treetop && cursorNode->getFirstChild() == prevNode->getFirstChild() && (prevNode->getFirstChild()->getDataType() == TR::NoType))
                        isTreeTopNode = true;
            ...
            while (cursorTreeTop != exitTree && cursorTreeTop->getNode()->getOpCodeValue() != TR::BBEnd) {
                ...
                if (!replacedNullCheck) {
                    if (cursorNode->getOpCode().isStore() && cursorNode->getSymbolReference()->getSymbol()->isAutoOrParm() && cursorNode->getSymbolReference()->getUseonlyAliases().isZero(comp())) {
                        cursorTreeTop = cursorTreeTop->getNextRealTreeTop();
                        continue;
                    }
    ...
    for (i = 0; i < currentNode->getNumChildren(); ++i) {
        ...
        if (child == referenceChild)
            ...
            if (referenceChild->getOpCode().isLoadVarDirect() && child->getOpCode().isLoadVarDirect()) {
                if (((referenceChild->getSymbol() == child->getSymbol()) && (referenceChild->getSymbolReference()->getOffset() == child->getSymbolReference()->getOffset())) && (_isNextTree || !writtenSymbols->get(child->getSymbolReference()->getReferenceNumber())))
                    childEquivalent = true;
            }
        }
        ...
        if (!(opCode.isArrayLength() && comp()->cg()->getDisableNullCheckOfArrayLength()) && childEquivalent && (opCode.isIndirect() || (opCode.isArrayLength()) || opCode.isCall() || (opCodeValue == TR::monent) || (opCodeValue == TR::monexit)) && ((!opCode.isCall() && (i == 0)) || (opCode.isCall() && !opCode.isIndirect() && (i == 0)) || (opCode.isCallIndirect() && (i == 1))) && (currentParent == NULL || !currentParent->getOpCode().isResolveCheck())) {
            bool canCompact = false;
            if (_isNextTree || (opCode.isArrayLength()) || (opCode.isLoadVar() && !writtenSymbols->get(currentNode->getSymbolReference()->getReferenceNumber())))
                canCompact = true;

            if (canCompact && performTransformation(comp(), "%sCompact null check %p with node %p in next tree\n", optDetailString(), prevNode, currentNode)) {
                //if (!_isNextTree)
                ...
                if (0 && comp()->useAnchors() && currentTree->getNode()->getOpCode().isAnchor()) {
...
static bool nodeCanRaiseException(TR::Node* node)
{
    ...
    if (node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::multianewarray)
        return false;
    ...
    if (cursorNode->getOpCodeValue() == TR::NULLCHK) {
        ...
        if (reference == objectRef)
            ...
            if (reference->getOpCode().isLoadVarDirect() && objectRef->getOpCode().isLoadVarDirect()) {
                if ((reference->getSymbol() == objectRef->getSymbol() && reference->getSymbolReference()->getOffset() == objectRef->getSymbolReference()->getOffset()) && (_isNextTree || !writtenSymbols->get(reference->getSymbolReference()->getReferenceNumber())))
                    isEquivalent = true;
            }
        }
        ...
        if (isEquivalent) {
            ...
            if (TR::comp()->getOption(TR_DisableTraps) || TR::Compiler->om.offsetOfObjectVftField() >= comp()->cg()->getNumberBytesReadInaccessible())
                canBeRemoved = false;

            if (canBeRemoved && performTransformation(comp(), "%sCompacting checkcast [%p] and null check [%p]\n", optDetailString(), prevNode, cursorNode)) {
    ...
    while (currentTree != stopTree) {
        ...
        if ((prevNode->getNumChildren() > 0) && (prevNode->getFirstChild()->getOpCodeValue() == TR::newarray)) {
            ...
            while (currentTree != exitTree) {
                TR::Node* currentNode = currentTree->getNode();
                if ((currentNode->getNumChildren() > 0) && (currentNode->getFirstChild()->getOpCodeValue() == TR::arrayset)) {
                    arraysetTree = currentTree;
                    arraysetNode = currentNode->getFirstChild();
                    break;
                }

                if (!currentNode->getOpCode().isStore() || !currentNode->getSymbol()->isAuto() || !currentNode->getSymbolReference()->getUseonlyAliases().isZero(comp()))
                    break;
            ...
            if (arraysetNode && (arraysetNode->getOpCodeValue() == TR::arrayset) && arraysetNode->getFirstChild()->getOpCode().isArrayRef() && (arraysetNode->getFirstChild()->getFirstChild() == prevNode->getFirstChild())) {
                ...
                while (!optimizedArraysetNode) {
                    ...
                    if ((cursorNode->getNumChildren() > 0) && (cursorNode->getFirstChild()->getOpCodeValue() == TR::arraycopy))
                        break;
            ...
            if ((prevNode->getNumChildren() > 0) && (prevNode->getFirstChild()->getOpCodeValue() == TR::arrayset) && prevNode->getFirstChild()->getFirstChild()->getOpCode().isArrayRef()) {
                readAndWrittenSymbols->empty();
                TR::Node* cursorNode = currentTree->getNode();
...
bool TR_ArraysetStoreElimination::optimizeArraysetIfPossible(TR::Node* currentNode, TR::Node* addressChild, TR::TreeTop* arraysetTree, TR::Node* currentParent, TR_BitVector* readAndWrittenSymbols, TR_BitVector* symbolsWrittenByArrayset, TR_BitVector* scratch, vcount_t initialVisitCount, TR::TreeTop* currentTree)
{
    ...
    if (currentNode->getOpCodeValue() == TR::arraycopy) {
        ...
        if ((arraycopyDestAddressNode == addressChild) || (arraycopyDestAddressNode->getOpCode().isArrayRef() && addressChild->getOpCode().isArrayRef() && (arraycopyDestAddressNode->getFirstChild() == addressChild->getFirstChild()) && (arraycopyDestAddressNode->getSecondChild() == addressChild->getSecondChild()))) {
    ...
    TR::ILOpCodes ifOpCode, TR::ILOpCodes andOpCode)
{
    ...
    return (andNode->getOpCodeValue() == ifOpCode && lastRealNode->getOpCodeValue() == ifOpCode && andNode->getBranchDestination() == lastRealNode->getBranchDestination() && andNode->getFirstChild()->getOpCodeValue() == andOpCode && lastRealNode->getFirstChild()->getOpCodeValue() == andOpCode && andNode->getSecondChild()->getOpCode().isLoadConst() && lastRealNode->getSecondChild()->getOpCode().isLoadConst() && andNode->getSecondChild()->get64bitIntegralValue() == 0 && lastRealNode->getSecondChild()->get64bitIntegralValue() == 0 && andNode->getFirstChild()->getFirstChild() == lastRealNode->getFirstChild()->getFirstChild() && isPowerOfTwo(comp, andNode->getFirstChild()->getSecondChild()) && isPowerOfTwo(comp, lastRealNode->getFirstChild()->getSecondChild()));
}
...
int32_t TR_SimplifyAnds::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    for (treeTop = startTree; (treeTop != endTree); treeTop = treeTop->getNextTreeTop()) {
        ...
        if (lastRealNode->exceptionsRaised() || lastRealNode->getOpCode().isStore() || lastRealNode->getOpCode().isCall())
            noSideEffectsInBetween = false;

        //TR::Node *lastRealNode = lastRealTree->getNode();

        if ((lastRealNode->getOpCode().isBranch() && (lastRealNode->getOpCodeValue() != TR::Goto) && (!lastRealNode->getOpCode().isCompBranchOnly())) && !lastRealNode->isNopableInlineGuard()) {
            ...
            while (seenAndNode) {
                ...
                if ((andNode->getFirstChild() == lastRealNode->getFirstChild()) && (andNode->getSecondChild() == lastRealNode->getSecondChild())) {
                ...
                } else if ((andNode->getFirstChild() == lastRealNode->getFirstChild()) && andNode->getSecondChild()->getOpCode().isLoadConst() && lastRealNode->getSecondChild()->getOpCode().isLoadConst() && (lastRealNode->getSecondChild()->getType().isInt32()) && lastRealNode->getOpCode().isBooleanCompare()) // this code was allowing unsigned types in -- I made this test stronger // NOTE that isCompare is only true for signed compares -- this is very ugly though
                {
                    int32_t oldValue = andNode->getSecondChild()->getInt();
                    int32_t newValue = lastRealNode->getSecondChild()->getInt();

                    if ((andOpCodeValue == TR::ificmplt && ((lastRealOpCodeValue == TR::ificmple && (oldValue >= (newValue + 1) && (newValue != TR::getMaxSigned<TR::Int32>()))) || ((lastRealOpCodeValue == TR::ificmplt) && (oldValue >= newValue)))) || (andOpCodeValue == TR::ificmple && ((lastRealOpCodeValue == TR::ificmplt && (oldValue >= (newValue - 1) && (newValue != TR::getMinSigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ificmple && (oldValue >= newValue)))) || (andOpCodeValue == TR::ificmpgt && ((lastRealOpCodeValue == TR::ificmpge && (newValue >= (oldValue + 1) && (oldValue != TR::getMaxSigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ificmpgt && (newValue >= oldValue)))) || (andOpCodeValue == TR::ificmpge && ((lastRealOpCodeValue == TR::ificmpgt && (newValue >= (oldValue - 1) && (oldValue != TR::getMinSigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ificmpge && (newValue >= oldValue))))) {
                        //printf("Old value %d New value %d in %s\n", oldValue, newValue, comp()->signature());
                        //printf("detected new chance in %s\n", comp()->signature());
                        seenAndBefore = true;
                        break;
                    } else if ((andOpCodeValue == TR::ificmplt && ((lastRealOpCodeValue == TR::ificmpgt && (oldValue >= (newValue + 1) && (newValue != TR::getMaxSigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ificmpge && (oldValue >= newValue)))) || (andOpCodeValue == TR::ificmple && ((lastRealOpCodeValue == TR::ificmpge && (oldValue >= (newValue - 1) && (newValue != TR::getMinSigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ificmpgt && (oldValue >= newValue)))) || (andOpCodeValue == TR::ificmpgt && ((lastRealOpCodeValue == TR::ificmplt && (newValue >= (oldValue + 1) && (oldValue != TR::getMaxSigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ificmple && (newValue >= oldValue)))) || (andOpCodeValue == TR::ificmpge && ((lastRealOpCodeValue == TR::ificmple && (newValue >= (oldValue - 1) && (oldValue != TR::getMinSigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ificmplt && (newValue >= oldValue))))) {
                        //printf("detected new chance in %s\n", comp()->signature());
                        seenAndBefore = true;
                        needGoto = true;
                        break;
                ...
                else if ((andNode->getFirstChild() == lastRealNode->getFirstChild()) && andNode->getSecondChild()->getOpCode().isLoadConst() && lastRealNode->getSecondChild()->getOpCode().isLoadConst() && (lastRealNode->getSecondChild()->getType().isInt32()) && lastRealNode->getOpCode().isUnsignedCompare()) {
                    uint32_t oldValue = andNode->getSecondChild()->getUnsignedInt();
                    uint32_t newValue = lastRealNode->getSecondChild()->getUnsignedInt();

                    if ((andOpCodeValue == TR::ifiucmplt && ((lastRealOpCodeValue == TR::ifiucmple && (oldValue >= (newValue + 1) && (newValue != TR::getMaxUnsigned<TR::Int32>()))) || ((lastRealOpCodeValue == TR::ifiucmplt) && (oldValue >= newValue)))) || (andOpCodeValue == TR::ifiucmple && ((lastRealOpCodeValue == TR::ifiucmplt && (oldValue >= (newValue - 1) && (newValue != TR::getMinUnsigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ifiucmple && (oldValue >= newValue)))) || (andOpCodeValue == TR::ifiucmpgt && ((lastRealOpCodeValue == TR::ifiucmpge && (newValue >= (oldValue + 1) && (oldValue != TR::getMaxUnsigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ifiucmpgt && (newValue >= oldValue)))) || (andOpCodeValue == TR::ifiucmpge && ((lastRealOpCodeValue == TR::ifiucmpgt && (newValue >= (oldValue - 1) && (oldValue != TR::getMinUnsigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ifiucmpge && (newValue >= oldValue))))) {
                        //printf("Old value %d New value %d in %s\n", oldValue, newValue, comp()->signature());
                        //printf("detected new chance in %s\n", comp()->signature());
                        seenAndBefore = true;
                        break;
                    } else if ((andOpCodeValue == TR::ifiucmplt && ((lastRealOpCodeValue == TR::ifiucmpgt && (oldValue >= (newValue + 1) && (newValue != TR::getMaxUnsigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ifiucmpge && (oldValue >= newValue)))) || (andOpCodeValue == TR::ifiucmple && ((lastRealOpCodeValue == TR::ifiucmpge && (oldValue >= (newValue - 1) && (newValue != TR::getMinUnsigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ifiucmpgt && (oldValue >= newValue)))) || (andOpCodeValue == TR::ifiucmpgt && ((lastRealOpCodeValue == TR::ifiucmplt && (newValue >= (oldValue + 1) && (oldValue != TR::getMaxUnsigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ifiucmple && (newValue >= oldValue)))) || (andOpCodeValue == TR::ifiucmpge && ((lastRealOpCodeValue == TR::ifiucmple && (newValue >= (oldValue - 1) && (oldValue != TR::getMinUnsigned<TR::Int32>()))) || (lastRealOpCodeValue == TR::ifiucmplt && (newValue >= oldValue))))) {
                        //printf("detected new chance in %s\n", comp()->signature());
                        seenAndBefore = true;
                        needGoto = true;
                        break;
                ...
                // (a&b && a&c) ==> ((a & b|c) == b|c)
                else if (lastSeenAnd && noSideEffectsInBetween && andNode->getFirstChild()->getReferenceCount() == 1 && (isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::ificmpeq, TR::iand) || isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::iflcmpeq, TR::land))) {
                    if (trace())
            ...
            if (!seenAndBefore) {
                ...
                if (cg()->branchesAreExpensive() && (lastRealNode->getOpCodeValue() == TR::ificmpge || lastRealNode->getOpCodeValue() == TR::ifiucmpge || lastRealNode->getOpCodeValue() == TR::iflcmpge) && (lastRealNode->getSecondChild()->getOpCodeValue() == TR::iconst || lastRealNode->getSecondChild()->getOpCodeValue() == TR::iuconst || lastRealNode->getSecondChild()->getOpCodeValue() == TR::lconst) && (lastRealNode->getSecondChild()->get64bitIntegralValue() == 0)) {
                    ...
                    if (lastRealNode->getBranchDestination() == nextToNextBlockEntry) {
                        ...
                        if ((firstTree == lastTree) && (nextBlock->getPredecessors().size() == 1)) {
                            ...
                            if (firstNode->getOpCode().isStoreDirect() && ((firstNode->getFirstChild()->getOpCode().isAdd() || firstNode->getFirstChild()->getOpCode().isSub()) && firstNode->getFirstChild()->getReferenceCount() == 1) && ((firstNode->getType().isInt32() || firstNode->getType().isInt64()))) {
                                bool areTreesInRequiredForm = false;

                                if (firstNode->getFirstChild()->getFirstChild()->getOpCode().isLoadVar() && (firstNode->getFirstChild()->getFirstChild()->getReferenceCount() == 1) && // in actual fact the ref count can be > 1 as long as this is the first reference to this load (we could check that in future if reqd)
                                ...
                                else if (treeBeforeLastRealNode->getOpCode().isStoreDirect() && (firstNode->getFirstChild()->getFirstChild() == treeBeforeLastRealNode->getFirstChild()) && (treeBeforeLastRealNode->getSymbolReference()->getReferenceNumber() == firstNode->getSymbolReference()->getReferenceNumber()))
                                    areTreesInRequiredForm = true;

                                if (areTreesInRequiredForm && performTransformation(comp(), "%sSimplifying branch %p and store %p into conditional\n", optDetailString(), lastRealNode, firstNode)) {
                                    if (asyncTree)
            ...
            if (prevNode && prevNode->getOpCodeValue() == lastRealNode->getOpCodeValue()) {
                ...
                if (lastRealNode->getOpCode().isSpineCheck()) {
                    ...
                    //
                    if (0 && (prevNode->getChild(2) == lastRealNode->getChild(2)) && (compareValues(prevNode->getChild(3), lastRealNode->getChild(3)) > 0) && performTransformation(comp(), "%sRemoving BNDCHK from BNDCHKwithSpineCHK lastRealNode %p, prevNode=%p\n", optDetailString(), lastRealNode, prevNode)) {
                    ...
                    if ((prevNode->getFirstChild() == lastRealNode->getFirstChild()) && (compareValues(prevNode->getSecondChild(), lastRealNode->getSecondChild()) > 0)) {
                ...
                prevBoundCheckTree = treeTop;
        } else if ((lastRealNode->getOpCodeValue() == TR::BBStart) || lastRealNode->exceptionsRaised() || (lastRealNode->getOpCode().isStore() && (lastRealNode->getSymbolReference()->getSymbol()->isShadow() || lastRealNode->getSymbolReference()->getUseonlyAliases().hasAliases() || (lastRealNode->getSymbolReference()->getSymbol()->isStatic()))))
...
{
    TR::TreeTop* actt = TR::TreeTop::create(comp(), TR::Node::createWithSymRef(tt->getNode(), TR::asynccheck, 0, getSymRefTab()->findOrCreateAsyncCheckSymbolRef(comp()->getMethodSymbol())));
...
int32_t TR_EliminateRedundantGotos::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    for (TR::TreeTop *treeTop = startTree, *exitTreeTop;
        ...
        for (auto inEdge = preds.begin(); inEdge != preds.end(); ++inEdge) {
            ...
            if (tt->getNode()->getOpCode().isJumpWithMultipleTargets() || tt->getNode()->isTheVirtualGuardForAGuardedInlinedCall())
                continue;

            if (!emptyBlock && (!tt->getNode()->getOpCode().isBranch() || tt->getNode()->getBranchDestination() != block->getEntry()))
                continue;

            if (tt->getNode()->getOpCode().isBranch() && ((tt->getNode()->getBranchDestination() != block->getEntry()) && (block->isExtensionOfPreviousBlock() || !(destBlock->getSuccessors().size() == 1) || !(destBlock->getPredecessors().size() == 1) || !destBlock->getLastRealTreeTop()->getNode()->getOpCode().isBranch())))
                continue;
        ...
        if (emptyBlock && !performTransformation(comp(), "%sRemoving empty block_%d with BBStart %p\n", optDetailString(), block->getNumber(), block->getEntry()->getNode()))
            continue;
        else if (!emptyBlock && !performTransformation(comp(), "%sEliminating goto at the end of block_%d with BBStart %p\n", optDetailString(), block->getNumber(), block->getEntry()->getNode()))
            continue;
    ...
    bool asyncMessagesFlag)
{
    ...
    for (auto edge = preds.begin(); edge != preds.end(); ++edge) {
        ...
        if (predBlock->getLastRealTreeTop()->getNode()->getOpCode().isBranch() && predBlock->getLastRealTreeTop()->getNode()->getBranchDestination() == block->getEntry()) {
...
int32_t TR_CleanseTrees::process(TR::TreeTop* startTree, TR::TreeTop* endTreeTop)
{
    ...
    for (treeTop = startTree; (treeTop != endTreeTop); treeTop = exitTreeTop->getNextTreeTop()) {
        ...
        if ((node->getOpCodeValue() != TR::Goto) && block->getNextBlock() && (block->getNextBlock()->isExtensionOfPreviousBlock() || node->isNopableInlineGuard()))
            continue;
        ...
        if (tt2 && node->getBranchDestination() == tt2) {
            ...
            if ((tt1->getNode()->getOpCodeValue() == TR::Goto || tt1->getNode()->getOpCode().isReturn() || (tt1->getNode()->getOpCode().isTreeTop() && tt1->getNode()->getNumChildren() > 0 && tt1->getNode()->getFirstChild()->getOpCodeValue() == TR::athrow)) && (/* (!rootStructure) || */ isBestChoiceForFallThrough(block, block2)) && performTransformation(comp(), "%sMoving trees contained in block that was the fall through of block_%d\n", optDetailString(), block->getNumber())) {
            ...
            if ((tt2 == comp()->getStartTree()) || block->isCold() || block2->isCold() || (!isBestChoiceForFallThrough(block, tt2->getNode()->getBlock())))
                continue;
        ...
        if (node->getOpCodeValue() == TR::BBEnd) {
            ...
            case TR::anewarray: {
                ...
                if (numElementsInfo) {
                    ...
                    if (doProfiling && performTransformation(comp(), "O^O PROFILED NODE VERSIONING: Versioning %s %s, size %s %d %.01f%%\n", node->getOpCode().getName(), getDebug()->getName(node), (numDifferentSizes == 1) ? "==" : "<=", profiledSize, 100.0 * combinedProbability)) {
...
void TR_Rematerialization::rematerializeAddresses(TR::Node* indirectNode, TR::TreeTop* treeTop, vcount_t visitCount)
{
    ...
    if (indirectNode->getOpCode().isIndirect()) {
        ...
        if (node->getNumChildren() == 2 && node->isInternalPointer()) {
            ...
            while (++throttle < 10) {
                if ((node->getReferenceCount() > 1 && node->isInternalPointer() && node->getNumChildren() == 2 && (!node->getSecondChild()->getOpCode().isLoadConst() || (cg()->isMaterialized(node->getSecondChild()) || (cg()->getSupportsConstantOffsetInAddressing() && cg()->getSupportsConstantOffsetInAddressing(node->getSecondChild()->get64bitIntegralValue()))))) || (node->getReferenceCount() == 1 && node->isInternalPointer() && node->getNumChildren() == 2 && node->getFirstChild()->getReferenceCount() > 1 && node->getFirstChild()->isInternalPointer() && node->getFirstChild()->getNumChildren() == 2 && (node->getSecondChild()->getOpCode().isLoadConst() || (node->getFirstChild()->getSecondChild()->getOpCode().isLoadConst() && cg()->getSupportsConstantOffsetInAddressing(node->getFirstChild()->getSecondChild()->get64bitIntegralValue()))))) {
                    if (node->getReferenceCount() == 1) {
                    ...
                    if (performTransformation(comp(), "%sRematerializing node %p(%s)\n", optDetailString(), node, node->getOpCode().getName())) {
                        ...
                        parentOfInternalPointer = node;
                        if ((node->getNumChildren() == 2) && node->getFirstChild()->isInternalPointer() && (node->getFirstChild()->getReferenceCount() > 1) && (node->getFirstChild()->getNumChildren() == 2))
                            node = node->getFirstChild();
                        else
                            break;
                    } else
            ...
            if (cg()->getSupportsScaledIndexAddressing() || cg()->getSupportsConstantOffsetInAddressing()) {
                ...
                comp()->getVisitCount();
                TR::Node* parent = node;
                if (cg()->getSupportsConstantOffsetInAddressing() && (secondChild->getOpCode().isSub() || secondChild->getOpCode().isAdd())
              ...
              ((secondChildOp == TR::lsub) || (secondChildOp == TR::ladd)))*/
                    ...
                    if (secondChild->getSecondChild()->getOpCode().isLoadConst() && cg()->getSupportsConstantOffsetInAddressing(secondChild->getSecondChild()->get64bitIntegralValue())) {
                ...
                if (cg()->getSupportsScaledIndexAddressing() && (((secondChildOp == TR::imul) || (secondChildOp == TR::ishl)) || ((secondChildOp == TR::lmul) || (secondChildOp == TR::lshl)))) {
                    TR::ILOpCodes childOp = secondChild->getSecondChild()->getOpCodeValue();
                    if (childOp == TR::iconst || childOp == TR::lconst) {
                        ...
                        if (childOp == TR::lconst && (secondChild->getSecondChild()->getLongIntHigh() == 0) && cg()->isAddressScaleIndexSupported(secondChild->getSecondChild()->getLongIntLow()))
                            allowUncommoning = true;
...
int32_t TR_Rematerialization::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    if ((!cg()->doRematerialization()) && !comp()->containsBigDecimalLoad() && !comp()->getSymRefTab()->findPrefetchSymbol())
        return 0;
...
void TR_Rematerialization::removeNodeFromList(TR::Node* node, List<TR::Node>* nodes, List<List<TR::Node>>* parents, bool checkSymRefs, List<TR::Node>* loadsAlreadyVisited, List<TR::Node>* loadsAlreadyVisitedThatCannotBeRematerialized, const TR::SparseBitVector& aliases)
{
    ...
    while (commonedElement) {
        ...
        if ((commonedElement->getData() == node) || (checkSymRefs && ((commonedElement->getData()->getSymbolReference() == node->getSymbolReference()) || aliases.ValueAt(commonedElement->getData()->getSymbolReference()->getReferenceNumber())))) {
            if (prevNodeElem) {
    ...
    if (checkSymRefs && loadsAlreadyVisited) {
        ...
        while (visitedLoadsElement) {
            if ((visitedLoadsElement->getData()->getSymbolReference() == node->getSymbolReference()) || aliases.ValueAt(visitedLoadsElement->getData()->getSymbolReference()->getReferenceNumber())) {
...
void TR_Rematerialization::findSymsUsedInIndirectAccesses(TR::Node* node, List<TR::Node>* nodes, List<List<TR::Node>>* parents)
{
    ...
    TR_RematAdjustments& adjustments)
{
    ...
    if ((((node->getFutureUseCount() & 0x7fff) == 0) && (node->getReferenceCount() > 1)) || (parent && (_underGlRegDeps || parent->getOpCode().isStoreReg()))) {
   ...
   */
    ...
    bool isCall = false;
    if ((node->getOpCode().hasSymbolReference() && node->getSymbolReference()->isUnresolved() && parent && parent->getOpCode().isResolveCheck()) || (node->getOpCodeValue() == TR::monent) || (node->getOpCodeValue() == TR::monexit) || node->mightHaveVolatileSymbolReference())
        isSimilarToCall = true;

    bool isCallDirect = false;
    if (node->getOpCode().isCall() && (node->getOpCode().isTreeTop() || (parent && ((parent->getOpCodeValue() == TR::treetop) || (parent->getOpCode().isResolveOrNullCheck()))))) {
        isCall = true;
    ...
    if ((NULL != regPress) || (!state->_currentlyCommonedLoads.isEmpty() && (state->_currentlyCommonedLoads.getListHead()->getData()->getOpCodeValue() == TR::lloadi) && state->_currentlyCommonedLoads.getListHead()->getData()->isBigDecimalLoad()))
    ...
    if ((NULL != regPress) || (!state->_currentlyCommonedLoads.isEmpty() && (state->_currentlyCommonedLoads.getListHead()->getData()->getOpCodeValue() == TR::aloadi) && !state->_parentsOfCommonedLoads.getListHead()->getData()->isEmpty() && (state->_parentsOfCommonedLoads.getListHead()->getData()->getListHead()->getData()->getOpCodeValue() == TR::Prefetch))) {
        considerRegPressure = false;
    }
    ...
    if ((!considerRegPressure || ((state->_currentlyCommonedNodes.getSize() + childAdjustment + adjustments.adjustmentFromParent) > (cg()->getMaximumNumbersOfAssignableGPRs() /* -1 */))) && (!state->_currentlyCommonedCandidates.isEmpty() || !state->_currentlyCommonedLoads.isEmpty())) {
        //_counter++;
    ...
    if ((!considerRegPressure || ((state->_currentlyCommonedFPNodes.getSize() + fpChildAdjustment + adjustments.fpAdjustmentFromParent) > cg()->getMaximumNumbersOfAssignableFPRs() /* -1 */)) && (!state->_currentlyCommonedFPCandidates.isEmpty() || !state->_currentlyCommonedFPLoads.isEmpty())) {
        //_counter++;
    ...
    if ((!considerRegPressure || ((state->_currentlyCommonedVector128Nodes.getSize() + vector128ChildAdjustment + adjustments.vector128AdjustmentFromParent) > cg()->getMaximumNumbersOfAssignableVRs())) && (!state->_currentlyCommonedVector128Candidates.isEmpty() || !state->_currentlyCommonedVector128Loads.isEmpty())) {
        //_counter++;
    ...
    if ((node->getFutureUseCount() & 0x7fff) > 0) {
        if (!node->getOpCode().isLoadConst()) {
            ...
            if (parent && !_underGlRegDeps && !parent->getOpCode().isStoreReg() && isRematerializable(parent, node) && node->isRematerializeable()) {
                if (isRematerializableLoad(node, parent)) {
                    if (node->getOpCode().isFloatingPoint()) {
...
void TR_Rematerialization::rematerializeNode(TR::TreeTop* treeTop, TR::Node* parent, TR::Node* node, vcount_t visitCount, List<TR::Node>* currentlyCommonedNodes, List<TR::Node>* currentlyCommonedCandidates, List<List<TR::Node>>* parents, List<TR::Node>* currentlyCommonedLoads, List<List<TR::Node>>* parentsOfCommonedLoads, List<TR::Node>* loadsAlreadyVisited, List<TR::Node>* loadsAlreadyVisitedThatCannotBeRematerialized, bool rematSpecialNode)
{
    ...
    if (!isAdjunctNode && performTransformation(comp(), "%sRematerializing node %p(%s)\n", optDetailString(), nodeToBeRematerialized, nodeToBeRematerialized->getOpCode().getName())) {
        ...
        if (nodeToBeRematerialized->getNumChildren() > 0) {
            ...
            if (isRematerializable(nodeToBeRematerialized, firstChild) && firstChild->isRematerializeable() && !firstChild->getOpCode().isLoadVarDirect() &&
        ...
        if (mustBeAnchored && !loadsAlreadyVisited->find(nodeToBeRematerialized) && !loadsAlreadyVisitedThatCannotBeRematerialized->find(nodeToBeRematerialized))
...
{
    return block->getStructureOf() && block->getStructureOf()->getParent() ? block->getStructureOf()->getParent()->asRegion() : NULL;
}
...
int32_t TR_BlockSplitter::perform()
{
    ...
    while (children->size() > 0) {
        ...
        if (
            (mergeNode->hasExceptionPredecessors() == true) || (mergeNode->getPredecessors().size() == 1) || mergeNode->getPredecessors().empty()) {
            children->pop();
            continue;
        }
        ...
        if (disableSynergy() && comp()->isOptServer() && !mergeNode->isEmptyBlock()) {
            ...
            for (auto edge = mergeNode->getPredecessors().begin(); edge != mergeNode->getPredecessors().end(); ++edge) {
                ...
                if (predBlock->getEntry() && (predBlock->getFrequency() >= splitFrequency) && predBlock->getNumberOfRealTreeTops() > 4) {
    ...
    for (int32_t i = 0; i <= array->lastIndex(); ++i) {
        ...
        for (auto edge = mergeNode->getPredecessors().begin(); edge != mergeNode->getPredecessors().end(); ++edge) {
            ...
            if (toBlock(node)->getEntry() && toBlock(node)->getExit()->getPrevRealTreeTop() && (toBlock(node)->getExit()->getPrevRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets() && toBlock(node)->getExit()->getPrevRealTreeTop()->getNode()->getOpCode().hasBranchChildren()))
                continue;
            ...
            if (toBlock(node)->getEntry()) {
                ...
                if (lastRealNode->getOpCode().isIf() && lastRealNode->isTheVirtualGuardForAGuardedInlinedCall() && lastRealNode->getBranchDestination()->getNode()->getBlock()->getNumber() == mergeNode->getNumber())
                    continue;
        ...
        if (!splitPred || (splitPred->getFrequency() < splitPredFrequency) || !splitPred_to_mergeNode_edge ||
            ...
            (splitPred_to_mergeNode_edge->getFrequency() < (splitPredFrequency / 10)) || (predFrequency <= 0) || (predEdgeFrequency <= 0)) {
            if (trace())
        ...
        while (depth > 1) {
            ...
            for (auto edge = target->getSuccessors().begin(); edge != target->getSuccessors().end(); ++edge) {
                ...
                if (lastRealNode->getOpCode().isIf() && lastRealNode->isTheVirtualGuardForAGuardedInlinedCall() && lastRealNode->getBranchDestination()->getNode()->getBlock()->getNumber() == child->getNumber()) {
                    if (trace())
        ...
        if (pruneAndPopulateBlockMapper(&bMap, synergisticDepthCalculator(&bMap, splitPred)) > 0) {
            ...
            for (BlockMapper* itr = bMap.getFirst(); itr; itr = itr->getNext()) {
                ...
                if (origFreq > 0) {
                    ...
                    for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
                        ...
                        if (!(*predEdge)->getFrom()->asBlock()->isCold() && !(*predEdge)->getTo()->asBlock()->isCold() && (freq <= MAX_COLD_BLOCK_COUNT))
                            freq = MAX_COLD_BLOCK_COUNT + 1;
                        else if ((*predEdge)->getFrom()->asBlock()->isCold() && (*predEdge)->getTo()->asBlock()->isCold() && freq > MAX_COLD_BLOCK_COUNT)
                    ...
                    for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
                        ...
                        if (!(*succEdge)->getFrom()->asBlock()->isCold() && !(*succEdge)->getTo()->asBlock()->isCold() && (freq <= MAX_COLD_BLOCK_COUNT))
                            freq = MAX_COLD_BLOCK_COUNT + 1;
                        else if ((*succEdge)->getFrom()->asBlock()->isCold() && (*succEdge)->getTo()->asBlock()->isCold() && freq > MAX_COLD_BLOCK_COUNT)
                            freq = MAX_COLD_BLOCK_COUNT;
            ...
            // a candidate
            //
            if (!(mergeNode->getPredecessors().size() == 1) && mergeNode->getFrequency() >= splitFrequency && horizontalCount < 2) {
                ++i;
                ++horizontalCount;
...
int32_t TR_BlockSplitter::synergisticDepthCalculator(TR_LinkHeadAndTail<BlockMapper>* bMap, TR::Block* startPoint)
{
    ...
    if (!startPoint->getPredecessors().empty()) {
        ...
        for (itr = startPoint->getPredecessors().front();
             toBlock(itr->getFrom())->getEntry() && (itr->getFrom()->getPredecessors().size() == 1) && (toBlock(itr->getFrom())->hasExceptionPredecessors() == false);
...
TR::Block* TR_BlockSplitter::splitBlock(TR::Block* pred, TR_LinkHeadAndTail<BlockMapper>* bMap)
{
    ...
    for (BlockMapper* itr = bMap->getFirst(); itr; itr = itr->getNext()) {
        ...
        if (
            lastRealNode->getOpCode().isBranch() && lastRealNode->getOpCode().isIf()) {
            ...
            !lastRealNode->getOpCode().isBranch() && !lastRealNode->getOpCode().isReturn() && !lastRealNode->getOpCode().isJumpWithMultipleTargets() && lastRealNode->getOpCodeValue() != TR::athrow && !(lastRealNode->getNumChildren() >= 1 && lastRealNode->getFirstChild()->getOpCodeValue() == TR::athrow)) {
            ...
            if (argInfo) {
                ...
                for (TR::ParameterSymbol* p = parms.getFirst(); p != NULL; p = parms.getNext(), index++) {
                    ...
                    if (classIsFixed || classIsPreexistent) {
                        ...
                        TR_ASSERT(!clazz || !clazzFromMethod || clazz == clazzFromMethod || fe()->isInstanceOf(clazz, clazzFromMethod, true, true, true) == TR_yes,
...
TR_InvariantArgumentPreexistence::ParmInfo* TR_InvariantArgumentPreexistence::getSuitableParmInfo(TR::Node* node)
{
    ...
    if (comp()->isPeekingMethod() && !info->classIsRefined())
        return NULL;
    if (!comp()->isPeekingMethod() && !symbol->getParmSymbol()->getIsPreexistent() && symbol->getParmSymbol()->getFixedType() == 0)
        return NULL;
...
void TR_InvariantArgumentPreexistence::processIndirectCall(TR::Node* node, TR::TreeTop* treeTop, vcount_t visitCount)
{
    ...
    if (methodSymbol->isComputed()) {
        ...
        if (refinedMethod) {
            ...
            if (!(receiverInfo.classIsRefined() && receiverInfo.classIsFixed()) && receiverSymbol && (receiverSymbol->getParmSymbol()->getFixedType() == 0))
            ...
            if (comp()->ilGenRequest().details().supportsInvalidation()) {
                ...
                if (needsAssumptions) {
                    ...
                    //
                    if ((comp()->getMethodHotness() == warm) && classInfo && classInfo->getNumPrexAssumptions() > comp()->getOptions()->getMaxNumPrexAssumptions())
            ...
            if (!needsAssumptions || addAssumptions) {
                if (!refinedMethod->isSameMethod(resolvedMethod)) {
                    if (performTransformation(comp(), "%sspecialize and devirtualize invoke [%p] on currently fixed or final parameter %d [%p]\n", optDetailString(), node, receiverParmOrdinal, receiverSymbol)) {
                        TR::SymbolReference* newSymRef = getSymRefTab()->findOrCreateMethodSymbol(symRef->getOwningMethodIndex(), -1, refinedMethod, TR::MethodSymbol::Virtual);
        ...
        if (!isInterface && !resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
            ...
            if (comp()->ilGenRequest().details().supportsInvalidation()) {
                addAssumptions = true;
                if ((comp()->getMethodHotness() == warm) && (callInfo = TR_PersistentMethodInfo::get(resolvedMethod)) && callInfo->getNumPrexAssumptions() > comp()->getOptions()->getMaxNumPrexAssumptions())
                    addAssumptions = false;
            }

            if (addAssumptions && performTransformation(comp(), "%sdevirtualizing invoke [%p] on preexistent argument %d [%p]\n", optDetailString(), node, receiverParmOrdinal, receiverSymbol)) {
                if (trace())
            ...
            if (methSymbol->isInterface() || methodSymbol) {
                ...
                if (method && !method->virtualMethodIsOverridden()) {
                    ...
                    if (addAssumptions && performTransformation(comp(), "%sspecialize and devirtualize invoke [%p] based on only a single implementation for call on parameter %d [%p]\n", optDetailString(), node, receiverParmOrdinal, receiverSymbol)) {
                        if (newMethod || !method->isSameMethod(resolvedMethod)) {
                            TR::SymbolReference* newSymRef = getSymRefTab()->findOrCreateMethodSymbol(symRef->getOwningMethodIndex(), -1, method, TR::MethodSymbol::Virtual);
                            newSymRef->copyAliasSets(symRef, getSymRefTab());
    ...
    for (TR::TreeTop* tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        case TR::ifacmpne:
            if (node->isProfiledGuard() && node->getFirstChild() && checkcastTree) {
                ...
                if (virtualGuard && transformationIsValid && virtualGuard->getTestType() == TR_VftTest && vftLoad->getOpCodeValue() == TR::aloadi && vftLoad->getSymbolReference() == comp()->getSymRefTab()->findVftSymbolRef() && !checkcastTree->getNode()->getSecondChild()->getSymbolReference()->isUnresolved() && vftLoad->getFirstChild()->getOpCode().hasSymbolReference() && !vftLoad->getFirstChild()->getSymbolReference()->isUnresolved() && vftLoad->getFirstChild()->getSymbolReference() == checkcastTree->getNode()->getFirstChild()->getSymbolReference() && performTransformation(comp(), "%s Merging checkcast [%p] and profiled guard [%p] (basic case)\n", optDetailString(), checkcastTree->getNode(), node)) {
                    // Transformation
        ...
        case TR::ifacmpeq:
            ...
            if (checkcastTree) {
                if (node->getFirstChild() != checkcastTree->getNode()->getFirstChild() || !node->getSecondChild()->getOpCode().isLoadConst() || node->getSecondChild()->getAddress() != 0) {
                    transformationIsValid = false;
                } else {
            ...
            break;
        default:
            if (!node->getOpCode().isStore() || !node->getOpCode().hasSymbolReference() || !node->getSymbolReference()->getUseonlyAliases().isZero(comp())) {
                transformationIsValid = false;
            }
...
bool TR_ColdBlockMarker::identifyColdBlocks()
{
    ...
    for (TR::AllBlockIterator iter(optimizer()->getMethodSymbol()->getFlowGraph(), comp()); iter.currentBlock(); ++iter) {
        ...
        if (block->isCold()) {
            ...
            if ((coldness <= MAX_COLD_BLOCK_COUNT) && ((!controlled) || performTransformation(comp(), "%s%s marked block_%d cold\n", optDetailString(), name(), block->getNumber()))) {
                block->setIsCold();
                block->setFrequency(coldness);
                foundColdBlocks = true;
            } else if (_enableFreqCBO && block->getFrequency() == 0 && ((!controlled) || performTransformation(comp(), "%s%s marked block_%d rare\n", optDetailString(), name(), block->getNumber()))) {
                foundColdBlocks = true;
            }
        }
    }
...
bool TR_ColdBlockMarker::hasNotYetRun(TR::Node* node)
{
    ...
    if (hasBeenRun != TR_maybe) {
        ...
        if (comp()->compileRelocatableCode() && !comp()->getOption(TR_UseSymbolValidationManager) && !comp()->getOption(TR_DisablePeekAOTResolutions))
...
static bool coldBlock(TR::Block* block, TR::Compilation* comp)
{
    ...
    retValue = (block->isCold() || (comp->getFlowGraph() && (comp->getFlowGraph()->getMaxFrequency() > (lowCFGFrequency << 2)) && block->getFrequency() <= lowCFGFrequency));
    return retValue;
}
...
void TR_ColdBlockOutlining::reorderColdBlocks()
{
    ...
    for (currentTree = comp()->getStartTree();
        ...
        for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
            ...
            if (predBlock->getExit() && predBlock->getExit()->getNextTreeTop() && predBlock->getExit()->getNextTreeTop()->getNode()->getBlock() == currentBlock && currentBlock->getEntry()->getNode()->getLabel() == NULL)
                continue;
        }
...
TR_BlockManipulator::breakFallThrough(TR::Block* faller, TR::Block* fallee, bool isOutlineSuperColdBlock)
{
    ...
    if (lastNode->getOpCode().isReturn() || lastNode->getOpCode().isGoto() || (lastNode->getOpCode().isJumpWithMultipleTargets() && lastNode->getOpCode().hasBranchChildren()) || lastNode->getOpCodeValue() == TR::athrow || lastNode->getOpCodeValue() == TR::igoto)
    ...
    if (lastNode->getOpCode().isBranch() || lastNode->getOpCode().isJumpWithMultipleTargets()) {
        ...
        if ((lastNode->getOpCode().isBranch() && lastNode->getBranchDestination() != fallee->getEntry()) || (lastNode->getOpCode().isCall() && lastNode->getOpCode().isJumpWithMultipleTargets()))
...
void TR_TrivialDeadTreeRemoval::preProcessTreetop(TR::TreeTop* treeTop, List<TR::TreeTop>& commonedTreeTopList, char* optDetails, TR::Compilation* comp)
{
    ...
    if (ttNode->getOpCodeValue() == TR::treetop && ttNode->getFirstChild()->getReferenceCount() >= 1) {
        ...
        if (firstChild->getReferenceCount() == 1) {
            if (!firstChild->getOpCode().hasSymbolReference() && performTransformation(comp, "%sUnlink trivial %s (%p) of %s (%p) with refCount==1\n", optDetails, treeTop->getNode()->getOpCode().getName(), treeTop->getNode(), firstChild->getOpCode().getName(), firstChild)) {
                if (trace)
        ...
        } else if (!firstChild->getOpCode().hasSymbolReference() || firstChild->getOpCode().isLoadAddr() || (firstChild->getOpCode().isLoad() && !firstChild->getOpCode().isStore())) {
            if (trace)
...
void TR_TrivialDeadTreeRemoval::postProcessTreetop(TR::TreeTop* treeTop, List<TR::TreeTop>& commonedTreeTopList, char* optDetails, TR::Compilation* comp)
{
    bool trace = comp->getOption(TR_TraceTrivialDeadTreeRemoval);
...
void TR_TrivialDeadTreeRemoval::processCommonedChild(TR::Node* child, TR::TreeTop* treeTop, List<TR::TreeTop>& commonedTreeTopList, char* optDetails, TR::Compilation* comp)
{
    bool trace = comp->getOption(TR_TraceTrivialDeadTreeRemoval);
    if (child->getReferenceCount() > 1) {
