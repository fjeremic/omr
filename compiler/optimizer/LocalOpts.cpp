...
int32_t TR_PeepHoleBasicBlocks::perform() {
  ...
  if (cfg == NULL || comp()->getOption(TR_DisableBasicBlockPeepHole)) return 0;

  TR_OrderBlocks orderBlocks(manager());
  cfg->setIgnoreUnreachableBlocks(true);
  int32_t rc =
      orderBlocks.lookForPeepHoleOpportunities("O^O BLOCK PEEP HOLE: ");
  cfg->setIgnoreUnreachableBlocks(false);
  if (cfg->getHasUnreachableBlocks()) cfg->removeUnreachableBlocks();
  return rc;
}

const char *TR_PeepHoleBasicBlocks::optDetailString() const throw() {
...
int32_t TR_ExtendBasicBlocks::perform() {
  ...
  if (comp()->getFlowGraph() == NULL) return 0;
  ...
  if (true) {
    ...
    if (p) return orderBlocksWithFrequencyInfo();
  }
  ...
  if (!comp()->getOption(TR_DisableNewBlockOrdering)) {
    ...
    // comp()->getFlowGraph()->setStructure(NULL); TR_OrderBlocks should reset
    // structure if needed
  ...
  if (!disableFreqCBO) comp()->getFlowGraph()->setStructure(NULL);
...
int32_t TR_ExtendBasicBlocks::orderBlocksWithoutFrequencyInfo() {
  ...
  for (; block; prevBlock = block, block = block->getNextBlock()) {
    ...
    if (block->isExtensionOfPreviousBlock()) continue;
    ...
    if (prevNode->getOpCode().isJumpWithMultipleTargets() ||
    ...
    if (!(preds.size() == 1)) cannotExtendWithCurrentFallThrough = true;
    TR::Block *bestExtension = NULL;

    if (trace())
      traceMsg(
          comp(),
          "    Current block [%d] looking for best successor to extend it...\n",
          prevBlock->getNumber());
    ...
    if (block->isCold() || (blockFreq >= 0)) {
      ...
      if (cannotExtendWithCurrentFallThrough && bestExtension) {
        if (block != bestExtension) {
          ...
          {
            // if (((bestHotness == deadCold) && (blockHotness != deadCold)) ||
            // (blockHotness > (bestHotness + 1)))
            if ((((bestFreq == (MAX_COLD_BLOCK_COUNT + 1)) &&
                  (blockFreq != (MAX_COLD_BLOCK_COUNT + 1))) ||
    ...
    if (trace())
      traceMsg(
          comp(),
          "    Current block [%d] with freq %d, best extension is BB[%d]\n",
          prevBlock->getNumber(), block->getFrequency(),
          bestExtension ? bestExtension->getNumber() : -1);

    if (cannotExtendWithCurrentFallThrough ||
        (bestExtension && !bestExtension->isCold() &&
         (bestExtension != block))) {
      //
      // The prevBlock cannot be extended to include the current block.
      // See if the prevBlock has a successor whose only predecessor is the
      // prevBlock. If so, see if we can rearrange the blocks so that the
      // prevBlock can be extended without adding any new gotos.
      //
      TR::Block *newBlock = 0;
      if (!bestExtension) {
      ...
      if (!newBlock) continue;  // there's no way to extend the prevBlock
      ...
      for (;; lastBlockToMove = next) {
        ...
        if (!next || !lastBlockToMove->hasSuccessor(next)) break;
      ...
      if (lastBlockToMove) {
        ...
        if (newBlock->getPredecessors().size() == 1) {
          ...
          if (result < 0) {
            if (cannotExtendWithCurrentFallThrough) continue;
          } else {
            if (result == 1) endTreeTop = previousTree;
            block = newBlock;
          }
        } else {
          if (cannotExtendWithCurrentFallThrough) continue;
    ...
    if (prevNode->getOpCodeValue() == TR::Goto) {
      ...
      if (nextBlock && prevNode->getOpCode().isBranch() &&
          !prevNode->isNopableInlineGuard()) {
        TR::TreeTop *gotoDest =
            lastNonFenceTree->getNode()->getBranchDestination();
        TR::Block *prevBranchDest =
            prevNode->getBranchDestination()->getNode()->getBlock();
        if (prevNode->getBranchDestination() == nextBlock->getEntry() &&
            !nextBlock->isCold()) {
          ...
          for (uintptr_t c = 0; c < prevNode->getNumChildren(); ++c) {
            TR_ASSERT(prevNode->getChild(c)->getOpCodeValue() != TR::GlRegDeps,
                      "the conditional branch node has a GlRegDep child and "
                      "we're changing control flow");
          }
          ...
          if (prevBranchDest->isCold()) continue;

          if (!performTransformation(
                  comp(),
                  "%sReverse branch in %d (possibly swinging up destintion and "
                  "removign goto block_%d)\n",
                  optDetailString(), prevBlock->getNumber(),
                  block->getNumber()))
            continue;
          ...
          // if the F and pT are the same (we have a redundant compare) - do not
          // do any transformation
          if (F == pT)
            continue;  // FIXME: maybe we should remove the redundant comparison

          for (G = T, nG = G->getNextBlock(); nG && G->hasSuccessor(nG);
          ...
          TR_ASSERT(prevNode->getNumChildren() != 3,
                    "the branch node has a GlRegDep child that we're about to "
                    "invalidate");
          prevNode->reverseBranch(gotoDest);
          ...
          if (!F->hasSuccessor(gotoDest->getNode()->getBlock())) {
            ...
            for (TR::CFGEdge *edge = ei.getFirst(); edge != NULL;
                 edge = ei.getNext()) {
              if (edge->getTo() == block) {
          ...
          cfg->removeEdge(F, block);  // ideally 'block' will become
                                      // unreachable, and be removed

          block = prevBlock;
          reattemptThisExtension = true;
          continue;
        } else if (gotoDest->getNode()->getBlock() ==
                   prevBlock->startOfExtendedBlock()) {
          if (!performTransformation(comp(),
                                     "%sRemove backedge goto block_%d by "
                                     "reversing the branch in block_%d\n",
                                     optDetailString(), block->getNumber(),
                                     prevBlock->getNumber()))
            continue;

          // goto is a backedge, reverse branch, swap destinations between
          // branch and goto
          //
  ...
  if (blocksWereExtended) optimizer()->enableAllLocalOpts();
...
int32_t TR_ExtendBasicBlocks::orderBlocksWithFrequencyInfo() {
  ...
  TR_SequenceHead *sequences =
      (TR_SequenceHead *)trMemory()->allocateStackMemory(
          numBlocks * sizeof(TR_SequenceHead));
  ...
  for (block = comp()->getStartTree()->getNode()->getBlock(); block;
    ...
    if (node->getOpCode().isBranch()) {
      if (node->getOpCode().isIf()) entry->fallThrough = block->getNextBlock();
  ...
  //
  static const char *p;
  static int32_t extendedBlockPointPreference =
      (p = feGetEnv("TR_ExtendedBlockPoints")) ? atoi(p) : 500;
  int32_t extendedBlockPreference = extendedBlockPointPreference *
                                    (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT) /
                                    10000;

  if (trace()) traceMsg(comp(), "Start block re-ordering\n");

  // Merge sequences
  //
  int32_t numSequences;
  int32_t oldNumSequences = numBlocks + 1;
  for (numSequences = numBlocks;
       numSequences > 1 && numSequences < oldNumSequences;) {
    ...
    for (i = 0; i < numBlocks; ++i) {
      if (!sequences[i].first) continue;  // This block already consumed
      ...
      for (auto edge = currentBlockEntry->block->getSuccessors().begin();
        ...
        if (edgeTo != i && sequences[edgeTo].first) {
          ...
          if ((*edge)->getTo()->getPredecessors().size() == 1) {
            ...
            if (block->getExit() && !(block->getLastRealTreeTop()
                                          ->getNode()
                                          ->getOpCode()
    ...
    if (bestFrequency >= 0) {
      ...
      numSequences--;
      if (trace()) traceMsg(comp(), "   add %d to %d\n", bestTo, bestFrom);
    }
  }
  ...
  if (trace())
    for (int32_t i = 0; i < numBlocks; ++i) {
      if (sequences[i].first != NULL) {
        ...
        for (TR_SequenceEntry *entry = sequences[i].first; entry;
             entry = entry->next) {
          TR::Block *block = entry->block;
          traceMsg(comp(), "%3d(%5d) ", block->getNumber(),
                   block->getFrequency());
        }
        traceMsg(comp(), "\n");
      }
      ...
      if (entry->next && entry->next->block->getEntry()) {
        ...
        if (entry->fallThrough) {
          ...
          TR::Block *extraBlock = TR::Block::createEmptyBlock(
              node, comp(), entry->fallThrough->getFrequency(),
              entry->fallThrough);
          TR::TreeTop::create(comp(), extraBlock->getEntry(),
                              TR::Node::create(node, TR::Goto, 0,
                                               entry->fallThrough->getEntry()));
...
const char *TR_ExtendBasicBlocks::optDetailString() const throw() {
...
int32_t TR_BlockManipulator::estimatedHotness(TR::CFGEdge *edge,
                                              TR::Block *block) {
...
TR::Block *TR_BlockManipulator::getBestChoiceForExtension(TR::Block *block1) {
  ...
  if (nextTree) fallThroughBlock = nextTree->getNode()->getBlock();
  ...
  for (auto nextEdge = block1->getSuccessors().begin();
       nextEdge != block1->getSuccessors().end(); ++nextEdge) {
    ...
    if (trace())
      traceMsg(comp(),
               "    Estimating hotness for BB [%d], next BB [%d], estimated "
               "hotness %d\n",
               block1->getNumber(), nextBlock->getNumber(), nextHotness);

    if (nextHotness > hotness) {
      bestExtension = nextBlock;
      numberOfRealTreesInBlock =
          countNumberOfTreesInSameExtendedBlock(nextBlock);
      hotness = nextHotness;
      int32_t nextBlockNestingDepth = 1;
      if (nextBlock->getStructureOf())
        nextBlock->getStructureOf()->calculateFrequencyOfExecution(
            &nextBlockNestingDepth);
      ...
      if ((nextBlockNestingDepth > nestingDepth) ||
        ...
        nestingDepth = nextBlockNestingDepth;
                 ...
                 (nextBlockNumTrees == numberOfRealTreesInBlock)) {
        if (nextBlock == fallThroughBlock) bestExtension = nextBlock;
      }
...
bool TR_BlockManipulator::isBestChoiceForFallThrough(TR::Block *block1,
                                                     TR::Block *block2) {
  ...
  bool bestSucc = false;
  bool bestPred = false;
  if (block1->getSuccessors().size() == 1) bestSucc = true;
  if (block2->getPredecessors().size() == 1) bestPred = true;

  if (bestSucc && bestPred) return true;
  ...
  if (comp()->getFlowGraph()->getStructure()) {
    ...
    while (blockStructure2) {
      if (blockStructure2->asRegion() &&
          blockStructure2->asRegion()->isNaturalLoop()) {
  ...
  int32_t numberOfRealTreesInBlock =
      countNumberOfTreesInSameExtendedBlock(block2);
  ...
  for (nextEdge = block1->getSuccessors().begin();
       nextEdge != block1->getSuccessors().end(); ++nextEdge) {
    TR::Block *nextBlock = toBlock((*nextEdge)->getTo());
    if (nextBlock == block2) continue;
  ...
  if (block1->getLastRealTreeTop()->getNode()->getOpCode().isBranch() &&
    ...
    if (block2 != fallThroughBlock) return false;
  }

  // at this point we know that block2 is a good choice as a fall through -
  // unless its better to keep it where it is because of synergy with its
  // predecessors
  ...
  if (prevBlock && prevBlock->hasSuccessor(block2)) {
    ...
    if (prevBlockHotness > hotness1) return false;
  }

  return true;
}

int32_t TR_BlockManipulator::countNumberOfTreesInSameExtendedBlock(
    TR::Block *block) {
  int32_t numTrees = block->getNumberOfRealTreeTops();
  TR::Block *prevB = block;
  TR::Block *nextB = block->getNextBlock();
  while (nextB && !nextB->isCold() && prevB->hasSuccessor(nextB) &&
...
//
// Checks if block1 and block2 are inside the same loop and block3 is not in
// that loop
//
static bool insideLoop(TR::Compilation *comp, TR::Block *block1,
                       TR::Block *block2, TR::Block *block3) {
  if (!comp->getFlowGraph()->getStructure()) return false;
  if (!block1->getStructureOf() || !block2->getStructureOf()) return false;

  TR_Structure *loop1 = block1->getStructureOf()->getContainingLoop();
  TR_Structure *loop2 = block2->getStructureOf()->getContainingLoop();
  TR_Structure *loop3 = block3->getStructureOf()
                            ? block3->getStructureOf()->getContainingLoop()
                            : NULL;

  if (loop1 && loop1 == loop2 && loop1 != loop3) return true;
...
//
int32_t TR_BlockManipulator::performChecksAndTreesMovement(
    TR::Block *newBlock, TR::Block *prevBlock, TR::Block *block,
  ...
  if (startOfTreesBeingMoved != comp()->getStartTree()) {
    if (startOfTreesBeingMoved->getPrevTreeTop()
            ->getNode()
            ->getBlock()
            ->hasSuccessor(startOfTreesBeingMoved->getNode()->getBlock()))
      return -1;
  } else
    return -1;
  ...
  if (newBlock->getVisitCount() < visitCount) {
    ...
    if (!performTransformation(
            comp(),
            "%sswing up block_%d to maximize fall through of block_%d\n",
            optDetailString(), newBlock->getNumber(), prevBlock->getNumber()))
      return -1;

    // Follow the fall through chain of newBlock uptil the last non-cold block
    // (or the method entry if there are no cold blocks)
    //
    TR::Block *cursor = NULL;
    TR::Block *cursorNext = NULL;
    for (cursor = newBlock, cursorNext = cursor->getNextBlock();
         cursorNext &&
         (cursor->hasSuccessor(cursorNext) ||
          insideLoop(comp(), cursor, cursorNext, block)) &&
    ...
    // If it was non-goto (cond. branch), just reverse condition and make it
    // point to block (old fallthrough) newBlock is the new fallthrough.
    ...
    */
    ...
    if (endOfTreesBeingMoved && !prevNode->isNopableInlineGuard() &&
        performTransformation(
            comp(),
            "%sswing down block_%d to maximize fall through with block_%d\n",
            optDetailString(), newBlock->getNumber(), prevBlock->getNumber())) {
      ...
      if (  // comp()->cg()->allowBlockToBeReordered(newBlock) &&
          lastNode->getOpCode().isBooleanCompare() &&
          !lastNode->isNopableInlineGuard() &&
          !nextOfNewBlock->isExtensionOfPreviousBlock() &&
          performTransformation(comp(),
                                "%sswing down block_%d and break original fall "
                                "through to join with block_%d\n",
                                optDetailString(), newBlock->getNumber(),
                                prevBlock->getNumber())) {
        TR::Block *prevOfNewBlock = newBlock->getPrevBlock();
        TR::Block *oldDestination =
            lastNode->getBranchDestination()->getNode()->getBlock();
        ...
        TR::Node *prevBlockLastNode =
            prevBlock->getLastRealTreeTop()->getNode();
        ...
        if (oldDestination == block) {
          ...
          TR::Block *gotoBlock = TR::Block::createEmptyBlock(
              lastNode, comp(), oldDestination->getFrequency(), oldDestination);
          gotoBlock->append(TR::TreeTop::create(comp(), gotoNode));
          if (newBlock->getStructureOf())
            comp()->getFlowGraph()->addNode(
                gotoBlock, newBlock->getCommonParentStructureIfExists(
                               oldDestination, comp()->getFlowGraph()));
          else
...
const char *TR_HoistBlocks::optDetailString() const throw() {
...
int32_t TR_HoistBlocks::performOnBlock(TR::Block *block) {
  if (block->getEntry())
    process(block->getEntry(),
            block->getEntry()->getExtendedBlockExitTreeTop()->getNextTreeTop());
  return 0;
}
...
//
static TR::Node *duplicateExact(TR::Node *node, List<TR::Node> *seenNodes,
                                List<TR::Node> *duplicateNodes,
                                TR::Compilation *comp) {
  ...
  if (visitCount == node->getVisitCount()) {
    ...
    TR::Node *nextNode;
    for (nextNode = seenNodesIt.getFirst(); nextNode != NULL;
         nextNode = seenNodesIt.getNext()) {
      if (nextNode == node) {
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i)
    newRoot->setChild(
        i, duplicateExact(node->getChild(i), seenNodes, duplicateNodes, comp));
...
int32_t TR_HoistBlocks::process(TR::TreeTop *startTree, TR::TreeTop *endTree) {
  ...
  for (TR::TreeTop *treeTop = startTree, *exitTreeTop; (treeTop != endTree);
    ...
    TR_ASSERT(node->getOpCodeValue() == TR::BBStart,
              "Local Opts, expected BBStart treetop");
    ...
    if ((node1->getOpCode().isReturn() ||
         ...
         (node1->getOpCodeValue() == TR::athrow &&
          (catchInfo = TR_CatchBlockProfileInfo::get(comp())) &&
          catchInfo->getCatchCounter() >= 100) ||
#endif
         (node1->getOpCode().isBranch() &&
          (node1->getOpCodeValue() != TR::Goto)) ||
         ((node1->getOpCodeValue() == TR::Goto) &&
          (!(block->getPredecessors().size() == 1))))) {
      bool exceptionEdgesMatter = true;
      if (tt1 == block->getFirstRealTreeTop()) exceptionEdgesMatter = false;
      ...
      while (blockStructure) {
        if (blockStructure->asRegion() &&
            blockStructure->asRegion()->isNaturalLoop() &&
      ...
      for (auto nextEdge = block->getPredecessors().begin();
           nextEdge != block->getPredecessors().end(); ++nextEdge) {
      ...
      for (auto nextEdge = block->getPredecessors().begin();
           nextEdge != block->getPredecessors().end();) {
        auto next = nextEdge;
        ++next;
        TR::CFGEdge *current = *nextEdge;
        TR::Block *prevBlock = toBlock((*nextEdge)->getFrom());

        if ((prevBlock->isCold()) || (prevBlock == cfgStart) ||
            (prevBlock == block) ||
            (exceptionEdgesMatter &&
             cfg->compareExceptionSuccessors(block, prevBlock))) {
          nextEdge = next;
          continue;
        };
        ...
        // Do not hoist this block if the predecessor is only remaining
        // predecessor as this will result in structure repair (because this
        // block would be removed from structure/CFG after this hoisting) that
        // could create improper regions. In any case if this is the last
        // predecessor, then this block should be moved will block
        // extension/trees cleansing to be adjacent to the predecessor (no need
        // for hoisting)
        //
        ...
        bool canHoist = true;
        for (nextNewBlock = newBlocksIt.getCurrent(); nextNewBlock;
             nextNewBlock = newBlocksIt.getNext()) {
        ...
        if (!tt->getNode()->getOpCode().isJumpWithMultipleTargets() &&
          ...
          else if ((block->getHotness(cfg) >= warm) &&
          (prevBlock->getHotness(cfg) >= hot))
             {
          ...
          */
          ...
          if (synergyExists &&
              performTransformation(
                  comp(),
                  "%sHoist basic block [%d] into predecessor block [%d]\n",
                  optDetailString(), block->getNumber(),
                  prevBlock->getNumber())) {
            // debugCounter++;
            // if (debugCounter > 4)
            ...
            if (prevBlock->getSuccessors().size() > 1) {
              TR::Block *splitBlock =
                  prevBlock->splitEdge(prevBlock, block, comp());
              dumpOptDetails(comp(),
                             "Aggressive hoisting by creating a new block_%d\n",
                             splitBlock->getNumber());
              newBlocks.add(splitBlock);

              for (auto nextSuccEdge = block->getExceptionSuccessors().begin();
                   nextSuccEdge != block->getExceptionSuccessors().end();
                   ++nextSuccEdge) {
              ...
              if (splitFrequency < 0) splitFrequency = block->getFrequency();
              ...
              if (splitFrequency >= 0)
                splitBlock->getPredecessors().front()->setFrequency(
                    splitFrequency);
              tt = splitBlock->getLastRealTreeTop();
              prevBlock = splitBlock;
              current = splitBlock->getSuccessors().front();
              if (splitFrequency >= 0) current->setFrequency(splitFrequency);
            }
              ...
              while (!(currentTree == exitTreeTop)) {
                {
                  ...
                  for (int32_t childNum = 0;
                       childNum < currentNode->getNumChildren(); childNum++) {
                    TR::Node *duplicateChildNode =
                        duplicateExact(currentNode->getChild(childNum),
                                       &seenNodes, &duplicateNodes, comp());
                    newCurrentNode->setChild(childNum, duplicateChildNode);
                  }

                  newCurrentTree =
                      TR::TreeTop::create(comp(), newCurrentNode, NULL, NULL);
                  newCurrentTree->join(originalNextTree);
                  newPrevTree->join(newCurrentTree);
                }

                currentTree = currentTree->getNextTreeTop();
                if (newCurrentTree) newPrevTree = newCurrentTree;
              }

              if (fallThroughSuccBlock) {
                comp()->getFlowGraph()->setStructure(NULL);
                extraGotoBlock = TR::Block::createEmptyBlock(
                    node1, comp(),
                ...
                TR::TreeTop *origNextTree =
                    prevBlock->getExit()->getNextTreeTop();
                ...
                TR_RegionStructure *parent =
                    prevBlock->getCommonParentStructureIfExists(
                        fallThroughTree->getNode()->getBlock(),
                        comp()->getFlowGraph());
            ...
            for (auto nextSuccEdge = block->getSuccessors().begin();
              ...
              if (extraGotoBlock && (succBlock == fallThroughSuccBlock)) {
                ...
                if (extraGotoFrequency >= 0) {
                  extraGotoBlock->getPredecessors().front()->setFrequency(
                      extraGotoFrequency);
                  extraGotoBlock->getSuccessors().front()->setFrequency(
                      extraGotoFrequency);
                }
              } else {
                // dumpOptDetails(comp(), "Added edge from %d to %d\n\n",
                // prevBlock->getNumber(), succBlock->getNumber());
                // We don't want to try to add another edge between prevBlock
                // and block as there already exists an edge; this will result
                // in the edge being added to the CFG but NOT to the structure
                // (which checks for duplicate edges before adding them). Then
                // removing the edge from prevBlock->block will fail after
                // structure repair as there would be one edge in the CFG left,
                // but NONE in the structure subgraph.
                //
                if (block != succBlock)
                  cfg->addEdge(TR::CFGEdge::createEdge(prevBlock, succBlock,
                                                       trMemory()));
                else
                  needToRemoveEdge = false;
              }
            }
            if (needToRemoveEdge) cfg->removeEdge(current);
          }
        }
        nextEdge = next;
      }
...
static void markNodesUsedInIndirectAccesses(TR::Node *node,
  ...
  if (examineChildren && (node->getNumChildren() > 0)) {
    ...
    if (child->getOpCode().isArrayRef()) node = child;
...
static void initializeFutureUseCounts(TR::Node *node, TR::Node *parent,
                                      vcount_t visitCount,
                                      TR::Compilation *comp,
                                      int32_t *heightArray) {
  ...
  if (visitCount == node->getVisitCount()) return;
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i) {
    initializeFutureUseCounts(node->getChild(i), node, visitCount, comp,
                              heightArray);
  ...
  if (heightArray) heightArray[node->getGlobalIndex()] = height;
}
...
int32_t TR_CompactNullChecks::process(TR::TreeTop *startTree,
                                      TR::TreeTop *endTree) {
  comp()->incVisitCount();
  TR::TreeTop *treeTop;
  ...
  //
  for (treeTop = startTree; (treeTop != endTree);
       treeTop = exitTreeTop->getNextTreeTop()) {
    // Get information about this block
    //
    TR::Node *node = treeTop->getNode();
...
void TR_CompactNullChecks::compactNullChecks(TR::Block *block,
                                             TR_BitVector *writtenSymbols) {
  ...
  while (currentTree != stopTree) {
    ...
    if (prevNode->getOpCodeValue() == TR::BBStart) {
      ...
      while (blockItr && blockItr->isExtensionOfPreviousBlock()) {
        if (blockItr->getPrevBlock()->getSuccessors().size() != 1) break;
        exitTree = blockItr->getExit();
        blockItr = blockItr->getNextBlock();
      }
    }
    if (block->isOSRInduceBlock() || block->isOSRCatchBlock() ||
    ...
    if ((prevNode->getOpCodeValue() == TR::NULLCHK) &&
      ...
      if (referenceChild == prevNode->getFirstChild()->getFirstChild()) {
        ...
        while (!replacedPassThroughNode) {
          ...
          replacedPassThroughNode = replacePassThroughIfPossible(
              cursorNode, referenceChild, prevNode, NULL, &isTreeTopNode,
              writtenSymbols, visitCount, initialVisitCount, cursorTreeTop);

          if (replacedPassThroughNode &&
              (cursorNode->getOpCodeValue() == TR::NULLCHK) &&
              (cursorNode->getNullCheckReference() ==
               prevNode->getNullCheckReference()))
      ...
      while (cursorTreeTop != exitTree &&
             cursorTreeTop->getNode()->getOpCodeValue() != TR::BBEnd) {
        TR::Node *cursorNode = cursorTreeTop->getNode();
        replacedNullCheck = replaceNullCheckIfPossible(
            cursorNode, objectRef, prevNode, NULL, writtenSymbols, visitCount,
        ...
        if (!replacedNullCheck) {
          if (cursorNode->getOpCode().isStore() &&
              ...
              cursorNode->getSymbolReference()->getUseonlyAliases().isZero(
                  comp())) {
            cursorTreeTop = cursorTreeTop->getNextRealTreeTop();
            continue;
          }
      ...
      if (replacedNullCheck) requestOpt(OMR::deadTreesElimination, true, block);
    }
    TR::TransformUtil::recursivelySetNodeVisitCount(prevNode,
                                                    initialVisitCount);
...
bool TR_CompactNullChecks::replacePassThroughIfPossible(
    TR::Node *currentNode, TR::Node *referenceChild, TR::Node *prevNode,
    TR::Node *currentParent, bool *isTreeTopNode, TR_BitVector *writtenSymbols,
    vcount_t visitCount, vcount_t initialVisitCount, TR::TreeTop *currentTree) {
  if (currentNode->getVisitCount() == visitCount) return false;

  if (currentNode->getVisitCount() == initialVisitCount) return false;

  currentNode->setVisitCount(visitCount);

  if (currentNode->isNopableInlineGuard()) return false;

  int32_t i = 0;
  for (i = 0; i < currentNode->getNumChildren(); ++i) {
    TR::Node *child = currentNode->getChild(i);
    if (replacePassThroughIfPossible(
            child, referenceChild, prevNode, currentNode, isTreeTopNode,
            writtenSymbols, visitCount, initialVisitCount, currentTree))
      return true;
    ...
    if (child == referenceChild)
      ...
      if (referenceChild->getOpCode().isLoadVarDirect() &&
          child->getOpCode().isLoadVarDirect()) {
    ...
    if (!(opCode.isArrayLength() &&
          comp()->cg()->getDisableNullCheckOfArrayLength()) &&
        childEquivalent &&
        (opCode.isIndirect() || (opCode.isArrayLength()) || opCode.isCall() ||
         (opCodeValue == TR::monent) || (opCodeValue == TR::monexit)) &&
        ((!opCode.isCall() && (i == 0)) ||
         (opCode.isCall() && !opCode.isIndirect() && (i == 0)) ||
         (opCode.isCallIndirect() && (i == 1))) &&
        (currentParent == NULL ||
         !currentParent->getOpCode().isResolveCheck())) {
      ...
      if (canCompact &&
          performTransformation(
              comp(), "%sCompact null check %p with node %p in next tree\n",
        ...
        if (child->getOpCodeValue() !=
            TR::loadaddr)  // For loadaddr, IsNonNull is always true
          child->setIsNonNull(
              false);  // it is no longer known that the reference is non null
...
bool TR_CompactNullChecks::replaceNullCheckIfPossible(
    TR::Node *cursorNode, TR::Node *objectRef, TR::Node *prevNode,
    TR::Node *currentParent, TR_BitVector *writtenSymbols, vcount_t visitCount,
    vcount_t initialVisitCount, bool &compactionDone) {
  if (cursorNode->getVisitCount() == visitCount) return true;

  if (cursorNode->getVisitCount() == initialVisitCount) return true;
  ...
  if (cursorNode->getOpCodeValue() == TR::NULLCHK) {
    ...
    if (reference == objectRef)
      ...
      if (reference->getOpCode().isLoadVarDirect() &&
          objectRef->getOpCode().isLoadVarDirect()) {
        if ((reference->getSymbol() == objectRef->getSymbol() &&
             reference->getSymbolReference()->getOffset() ==
                 objectRef->getSymbolReference()->getOffset()) &&
            (_isNextTree ||
             !writtenSymbols->get(
                 reference->getSymbolReference()->getReferenceNumber())))
          isEquivalent = true;
      }
    }
    /// dumpOptDetails(comp(), "reference [%p] objectRef [%p] isEquivalent
    /// %d\n", reference, objectRef, isEquivalent);
    if (isEquivalent) {
      bool canBeRemoved =
          true;  // comp()->cg()->canNullChkBeImplicit(cursorNode);
      if (TR::comp()->getOption(TR_DisableTraps) ||
          TR::Compiler->om.offsetOfObjectVftField() >=
              comp()->cg()->getNumberBytesReadInaccessible())
        canBeRemoved = false;

      if (canBeRemoved &&
          performTransformation(
              comp(), "%sCompacting checkcast [%p] and null check [%p]\n",
              optDetailString(), prevNode, cursorNode)) {
        ...
        if (!compactionDone) {
          ...
          compactionDone = true;
          TR_Pair<TR_ByteCodeInfo, TR::Node> *bcInfo =
              new (trHeapMemory()) TR_Pair<TR_ByteCodeInfo, TR::Node>(
                  &prevNode->getByteCodeInfo(), cursorNode);
    ...
    for (i = 0; i < cursorNode->getNumChildren(); i++) {
      ...
      if (opCode.isLikeDef() && opCode.hasSymbolReference())
        writtenSymbols->set(
            cursorNode->getSymbolReference()->getReferenceNumber());
    }
  }
  return true;
}

const char *TR_CompactNullChecks::optDetailString() const throw() {
...
TR_ArraysetStoreElimination::TR_ArraysetStoreElimination(
...
int32_t TR_ArraysetStoreElimination::process(TR::TreeTop *startTree,
                                             TR::TreeTop *endTree) {
  if (comp()->getOption(TR_DisableArraysetStoreElimination)) return 0;
  ...
  for (treeTop = startTree; (treeTop != endTree);
    ...
    TR_ASSERT(node->getOpCodeValue() == TR::BBStart,
              "Local Opts, expected BBStart treetop");
    TR::Block *block = node->getBlock();
    reduceArraysetStores(block, &readAndWrittenSymbols,
                         &symbolsWrittenByArrayset, &scratch);
    exitTreeTop = block->getEntry()->getExtendedBlockExitTreeTop();
  }
...
void TR_ArraysetStoreElimination::reduceArraysetStores(
    TR::Block *block, TR_BitVector *readAndWrittenSymbols,
    ...
    if ((prevNode->getNumChildren() > 0) &&
      ...
      while (currentTree != exitTree) {
        ...
        if (!currentNode->getOpCode().isStore() ||
            !currentNode->getSymbol()->isAuto() ||
            !currentNode->getSymbolReference()->getUseonlyAliases().isZero(
                comp()))
          break;

        TR::TransformUtil::recursivelySetNodeVisitCount(currentNode,
                                                        initialVisitCount);
      ...
      if (arraysetNode && (arraysetNode->getOpCodeValue() == TR::arrayset) &&
          ...
          (arraysetNode->getFirstChild()->getFirstChild() ==
           prevNode->getFirstChild())) {
        ...
        while (!optimizedArraysetNode) {
          ...
          optimizedArraysetNode = optimizeArraysetIfPossible(
              cursorNode, addressChild, arraysetTree, NULL,
              readAndWrittenSymbols, symbolsWrittenByArrayset, scratch,
              initialVisitCount, currentTree);

          if (currentTree == exitTree) break;
      ...
      if ((prevNode->getNumChildren() > 0) &&
        ...
        readAndWrittenSymbols->empty();
        TR::Node *cursorNode = currentTree->getNode();
        optimizeArraysetIfPossible(
            cursorNode, prevNode->getFirstChild()->getFirstChild(), prevTree,
            NULL, readAndWrittenSymbols, symbolsWrittenByArrayset, scratch,
            initialVisitCount, currentTree);
      }
    }

    prevTree = currentTree;
    if (currentTree == stopTree) break;
    currentTree = currentTree->getNextRealTreeTop();
  }
}

bool TR_ArraysetStoreElimination::optimizeArraysetIfPossible(
    TR::Node *currentNode, TR::Node *addressChild, TR::TreeTop *arraysetTree,
    TR::Node *currentParent, TR_BitVector *readAndWrittenSymbols,
    TR_BitVector *symbolsWrittenByArrayset, TR_BitVector *scratch,
  ...
  if (currentNode->getOpCodeValue() == TR::arraycopy) {
    ...
    if ((arraycopyDestAddressNode == addressChild) ||
        (arraycopyDestAddressNode->getOpCode().isArrayRef() &&
         ...
         (arraycopyDestAddressNode->getFirstChild() ==
          addressChild->getFirstChild()) &&
         (arraycopyDestAddressNode->getSecondChild() ==
          addressChild->getSecondChild()))) {
      *scratch = *symbolsWrittenByArrayset;
      *scratch &= *readAndWrittenSymbols;

      if (scratch->isEmpty() &&
          performTransformation(comp(),
                                "%sEliminating some stores done by arrayset "
                                "node %p because stores done by a following "
                                "arraycopy node %p will overwrite them\n",
        ...
        if (arraysetLenNode->getType().isInt32()) {
          subMaxNode =
              TR::Node::create(TR::isub, 2, arraysetLenNode, arraycopyLenNode);
          subMaxNode = TR::Node::create(TR::imax, 2, subMaxNode,
                                        TR::Node::iconst(subMaxNode, 0));

          newAddressChild =
              TR::Node::create(TR::aiadd, 2, addressChild, arraycopyLenNode);

        } else  // if (arraysetLenNode->getType().isInt64())
        {
          subMaxNode =
              TR::Node::create(TR::lsub, 2, arraysetLenNode, arraycopyLenNode);
          subMaxNode = TR::Node::create(TR::lmax, 2, subMaxNode,
                                        TR::Node::lconst(subMaxNode, 0));

          newAddressChild =
              TR::Node::create(TR::aladd, 2, addressChild, arraycopyLenNode);
        }
  ...
  if (currentNode->canGCandReturn())  // cannot have an uninitialized array be
                                      // held live across a GC point
    return false;
  ...
  for (i = 0; i < currentNode->getNumChildren(); ++i) {
    TR::Node *child = currentNode->getChild(i);
    if (optimizeArraysetIfPossible(child, addressChild, arraysetTree,
                                   currentNode, readAndWrittenSymbols,
                                   symbolsWrittenByArrayset, scratch,
                                   initialVisitCount, currentTree))
      return true;
  }
...
const char *TR_ArraysetStoreElimination::optDetailString() const throw() {
...
bool isPowerOfTwo(TR::Compilation *comp, TR::Node *node) {
  if (node->isPowerOfTwo()) return true;
...
static bool isAndOfTwoFlags(TR::Compilation *comp, TR::Node *andNode,
                            TR::Node *lastRealNode, TR::ILOpCodes ifOpCode,
  ...
  return (andNode->getOpCodeValue() == ifOpCode &&
          lastRealNode->getOpCodeValue() == ifOpCode &&
          andNode->getBranchDestination() ==
              lastRealNode->getBranchDestination() &&
...
int32_t TR_SimplifyAnds::process(TR::TreeTop *startTree, TR::TreeTop *endTree) {
  ...
  for (treeTop = startTree; (treeTop != endTree);
    ...
    if ((lastRealNode->getOpCode().isBranch() &&
         (lastRealNode->getOpCodeValue() != TR::Goto) &&
      ...
      while (seenAndNode) {
        ...
        if ((andNode->getFirstChild() == lastRealNode->getFirstChild()) &&
          ...
          if (andOpCodeValue == lastRealOpCodeValue) {
            ...
            seenAndBefore = true;
            break;
          } else if (andNode->getOpCode().getOpCodeForReverseBranch() ==
                     lastRealNode->getOpCodeValue()) {
            // if (andNode->getOpCode().getOpCodeForReverseBranch() !=
            // lastRealOpCodeValue) printf("detected new chance in %s\n",
            // comp()->signature());
        ...
        } else if ((andNode->getFirstChild() ==
                    lastRealNode->getFirstChild()) &&
                   andNode->getSecondChild()->getOpCode().isLoadConst() &&
                   lastRealNode->getSecondChild()->getOpCode().isLoadConst() &&
                   (lastRealNode->getSecondChild()->getType().isInt32()) &&
                   lastRealNode->getOpCode()
                       .isBooleanCompare())  // this code was allowing unsigned
                                             // types in -- I made this test
                                             // stronger // NOTE that isCompare
                                             // is only true for signed compares
        ...
        // (a&b && a&c) ==> ((a & b|c) == b|c)
                 ...
                 (isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::ificmpeq,
                                  TR::iand) ||
                  isAndOfTwoFlags(comp(), andNode, lastRealNode, TR::iflcmpeq,
                                  TR::land))) {
          if (trace())
      ...
      if (!seenAndBefore) {
        //
        // NOTE : this portion of code is really a special case peephole
        // optimization suggested by Julian wherein we replace given IL pattern
        // by another IL pattern which is expected to be more efficient from a
        // codegen point of view. Note that this pattern is seen in
        // Compressor.compress() and might result in a slowdown in compress if
        // not performed. Note this is only done on platforms where branches are
        // a real bottleneck; on some PPC architectures, we should not do this.
        //
        ...
        if (cg()->branchesAreExpensive() &&
            ...
            (lastRealNode->getSecondChild()->get64bitIntegralValue() == 0)) {
          TR::Block *nextBlock =
              block->getExit()->getNextTreeTop()->getNode()->getBlock();
          TR::TreeTop *nextToNextBlockEntry =
              nextBlock->getExit()->getNextTreeTop();
              ...
              if (firstNode->getOpCode().isStoreDirect() &&
                ...
                if (firstNode->getFirstChild()
                    ...
                    (firstNode->getFirstChild()
                         ->getFirstChild()
                         ->getReferenceCount() ==
                     1) &&  // in actual fact the ref count can be > 1 as long
                            // as this is the first reference to this load (we
                            // could check that in future if reqd)
                    (firstNode->getFirstChild()
                         ->getFirstChild()
                         ->getSymbolReference()
                         ->getReferenceNumber() ==
                  ...
                  areTreesInRequiredForm = true;
                         ...
                         (treeBeforeLastRealNode->getSymbolReference()
                              ->getReferenceNumber() ==
                          firstNode->getSymbolReference()
                              ->getReferenceNumber()))
                  areTreesInRequiredForm = true;

                if (areTreesInRequiredForm &&
                    performTransformation(comp(),
                                          "%sSimplifying branch %p and store "
                                          "%p into conditional\n",
                                          optDetailString(), lastRealNode,
                                          firstNode)) {
                  if (asyncTree)
                    asyncTree->getPrevTreeTop()->join(
                        asyncTree->getNextTreeTop());
                  removingConditional = true;
                  TR::Node *secondGrandChild =
                      firstNode->getFirstChild()->getSecondChild();

                  TR::Node *lastFirstChild = lastRealNode->getFirstChild();

                  // Shift the lastFirstChild right by (size-1) to generate 0x0
                  // or 0xFF..FF
                  TR::Node *newIShrNode = TR::Node::create(
                      (lastFirstChild->getType().isInt32()) ? TR::ishr
                                                            : TR::lshr,
                      2, lastFirstChild,
                      TR::Node::create(
                          lastRealNode, TR::iconst, 0,
                  ...
                  TR::Node *newIAndNode = TR::Node::create(
                      firstNode->getType().isInt32() ? TR::iand : TR::land, 2,
                  ...
                  if (succBlock !=
                      nextToNextBlockEntry->getNode()->getBlock()) {
        ...
        if (!removingConditional) {
          ...
          noSideEffectsInBetween = true;
                     ...
                     comp(),
                     "%sRemoving conditional node %p (condition has been "
                     "checked before in this extended BB)\n",
                     optDetailString(), lastRealNode)) {
        TR::TreeTop *previousTree =
            block->getLastRealTreeTop()->getPrevTreeTop();
        TR::CFGEdge *currentSucc = block->getSuccessors().front();
        TR::CFGNode *succBlock = currentSucc->getTo();
#ifdef J9_PROJECT_SPECIFIC
        ...
        if (newOrOpcode != TR::BadILOp) {
          ...
          TR::Node::recreate(andNode,
                             andNode->getOpCode().getOpCodeForReverseBranch());
          TR::Node *orNode = TR::Node::create(
              newOrOpcode, 2, lastRealNode->getFirstChild()->getSecondChild(),
              andNode->getFirstChild()->getSecondChild());
          andNode->getFirstChild()
              ->getSecondChild()
        ...
        if (needGoto) {
          ...
          TR::TreeTop *gotoTree =
              TR::TreeTop::create(comp(), gotoNode, NULL, NULL);
      ...
      if (prevBoundCheckTree) prevNode = prevBoundCheckTree->getNode();

      if (prevNode &&
          prevNode->getOpCodeValue() == lastRealNode->getOpCodeValue()) {
        // The first BNDCHKxxx can subsume the second if it checks against the
        // larger of the two indices.
        //
        if (lastRealNode->getOpCode().isSpineCheck()) {
          TR_ASSERT(lastRealNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK,
                    "unexpected spine check node");
          ...
          if (0 && (prevNode->getChild(2) == lastRealNode->getChild(2)) &&
              (compareValues(prevNode->getChild(3), lastRealNode->getChild(3)) >
               0) &&
              performTransformation(comp(),
                                    "%sRemoving BNDCHK from BNDCHKwithSpineCHK "
                                    "lastRealNode %p, prevNode=%p\n",
                                    optDetailString(), lastRealNode,
                                    prevNode)) {
          ...
          if ((prevNode->getFirstChild() == lastRealNode->getFirstChild()) &&
              (compareValues(prevNode->getSecondChild(),
                             lastRealNode->getSecondChild()) > 0)) {
        ...
        prevBoundCheckTree = treeTop;
               ...
               (lastRealNode->getOpCode().isStore() &&
                (lastRealNode->getSymbolReference()->getSymbol()->isShadow() ||
                 lastRealNode->getSymbolReference()
                     ->getUseonlyAliases()
                     .hasAliases() ||
                 (lastRealNode->getSymbolReference()
                      ->getSymbol()
...
const char *TR_SimplifyAnds::optDetailString() const throw() {
...
//
TR_EliminateRedundantGotos::TR_EliminateRedundantGotos(
    TR::OptimizationManager *manager)
    : TR::Optimization(manager) {}

void TR_EliminateRedundantGotos::renumberInAncestors(TR_Structure *str,
                                                     int32_t num) {
  TR_RegionStructure *parent = str->getParent()->asRegion();
  TR_ASSERT(parent, "assertion failure");

  TR_StructureSubGraphNode *nodeInParent =
      parent->findSubNodeInRegion(str->getNumber());
  if (parent->getNumber() == str->getNumber()) {
    renumberInAncestors(parent, num);
    parent->setNumber(
        num);  // don't use parent->renumber, since that recurses to children
  }
  ...
  for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
    TR_StructureSubGraphNode *predNode =
        toStructureSubGraphNode(edge->getFrom());
...
void TR_EliminateRedundantGotos::renumberExitEdges(TR_RegionStructure *region,
                                                   int32_t oldNum,
                                                   int32_t newNum) {
...
void TR_EliminateRedundantGotos::placeAsyncCheckBefore(TR::TreeTop *tt) {
  TR::TreeTop *actt = TR::TreeTop::create(
      comp(), TR::Node::createWithSymRef(
                  tt->getNode(), TR::asynccheck, 0,
...
int32_t TR_EliminateRedundantGotos::process(TR::TreeTop *startTree,
                                            TR::TreeTop *endTree) {
  // The CFG must exist for this optimization.
  //
  TR::CFG *cfg = comp()->getFlowGraph();
  ...
  if (comp()->getProfilingMode() == JitProfiling) return 0;
  ...
  for (TR::TreeTop *treeTop = startTree, *exitTreeTop; (treeTop != endTree);
    ...
    if (block->hasExceptionPredecessors()) continue;

    // It's possible that all the exception predecessor edges of an OSR catch
    // block have been removed because the exception predecessor nodes have
    // become empty and removed. Even in that case, we don't want to remove the
    // gotos at the end of such OSR blocks.
    if (block->isOSRCodeBlock()) continue;

    if (block->getEntry() && block->getEntry()->getNode()->getLabel()) continue;

    // See if the only "real" node in this block is a goto or the block is
    // empty. At the same time see if there is an async check in the block.
    //
    ...
    bool emptyBlock =
        (lastNonFenceTree->getNode()->getOpCodeValue() == TR::BBStart);
    ...
    if (block->isLoopInvariantBlock()) continue;
    ...
    for (auto inEdge = preds.begin(); inEdge != preds.end(); ++inEdge) {
      if ((*inEdge)->getFrom() == cfg->getStart()) continue;
    ...
    if (containsTreesOtherThanGoto) {
      ...
      if (!(destBlock->getPredecessors().size() == 1)) continue;
      ...
      if (!block->getExceptionSuccessors().empty()) {
        ...
        // goto blocks)
        //
        for (auto edge = block->getExceptionSuccessors().begin();
             edge != block->getExceptionSuccessors().end();)
          cfg->removeEdge(*(edge++));
      }
    }

    if (emptyBlock && !performTransformation(
                          comp(), "%sRemoving empty block_%d with BBStart %p\n",
                          optDetailString(), block->getNumber(),
                          block->getEntry()->getNode()))
      continue;
    else if (!emptyBlock &&
             !performTransformation(
                 comp(),
                 "%sEliminating goto at the end of block_%d with BBStart %p\n",
                 optDetailString(), block->getNumber(),
                 block->getEntry()->getNode()))
      continue;
    ...
    if (manuallyFixStructure) {
      ...
      // Attempt to remove the goto block will result in the collapse of
      // blockStrucutre into the parent.  1) if the blockRegion is a loop
      // region, we will end up with an improper region 2) otherwise, we will
      // end up with a bigger parent region, which although is not draconian,
      // but is still bad.
      //
      ...
      TR_StructureSubGraphNode *blockNode =
          blockRegion->findSubNodeInRegion(block->getNumber());
      TR_StructureSubGraphNode *destNode =
          blockRegion->findSubNodeInRegion(destBlock->getNumber());

      bool cannotRepairStructure = false;
      for (auto edge = blockNode->getPredecessors().begin();
           edge != blockNode->getPredecessors().end(); ++edge) {
        TR_StructureSubGraphNode *predNode =
      ...
      if (!destNode) cannotRepairStructure = true;
      ...
      if (!cannotRepairStructure) {
        ...
        // to goto destNode instead (Don't try to fix CFG edges yet, they will
        // be fixed later)
        //
        for (auto edge = blockNode->getPredecessors().begin();
             edge != blockNode->getPredecessors().end();) {
          TR_StructureSubGraphNode *predNode =
              (*(edge++))->getFrom()->asStructureSubGraphNode();
      ...
      //
      redirectPredecessors(block, destBlock, fixablePreds, emptyBlock,
                           asyncMessagesFlag);
      ...
      if (!cannotRepairStructure) cfg->setStructure(rootStructure);
    } else {
    ...
    // Place an asynccheck as the first treetop of the successor, if there was
    // one in the removed block This is necessary as placing it in the
    // predecessor may result in a seperation from its OSR guard
    //
    if (asyncMessagesFlag && comp()->getHCRMode() == TR::osr)
      placeAsyncCheckBefore(destBlock->getFirstRealTreeTop());

    if (emptyBlock && !block->isExtensionOfPreviousBlock() &&
...
void TR_EliminateRedundantGotos::redirectPredecessors(
    TR::Block *block, TR::Block *destBlock, const TR::CFGEdgeList &preds,
    bool emptyBlock, bool asyncMessagesFlag) {
  TR::CFG *cfg = comp()->getFlowGraph();
  ...
  if (regdepsAreOutgoing) {
    ...
    if (emptyBlock && entryNode->getNumChildren() > 0 &&
        destBlock->isExtensionOfPreviousBlock()) {
  ...
  if (regdepsToMove != NULL) {
    TR_ASSERT_FATAL(newRegdepParent->getNumChildren() == 0,
    ...
    if (predBlock->getNextBlock() == destBlock) {
      ...
      if (last->getOpCodeValue() == TR::Goto) {
        ...
        if (last->getNumChildren() > 0) {
          TR_ASSERT_FATAL(last->getNumChildren() == 1,
...
void TR_EliminateRedundantGotos::fixPredecessorRegDeps(TR::Node *regdepsParent,
                                                       TR::Block *destBlock) {
  ...
  if (destEntry->getNumChildren() == 0) {
    ...
    TR::DebugCounter::incStaticDebugCounter(
        ...
        TR::DebugCounter::debugCounterName(
            comp(), "redundantGotoElimination.regDeps/wiped/%s/(%s)/block_%d",
            comp()->getHotnessName(comp()->getMethodHotness()),
            comp()->signature(), destBlock->getNumber()));
  ...
  TR_ASSERT_FATAL(
      ...
      newReceivingRegdeps->getGlobalIndex(), destEntry->getGlobalIndex(),
      destBlock->getNumber());

  if (regdeps->getNumChildren() == newReceivingRegdeps->getNumChildren()) {
    TR::DebugCounter::incStaticDebugCounter(
        comp(), "redundantGotoElimination.regDeps/retained");
  } else {
    TR::DebugCounter::incStaticDebugCounter(
        comp(),
        TR::DebugCounter::debugCounterName(
            comp(), "redundantGotoElimination.regDeps/dropped/%s/(%s)/block_%d",
            comp()->getHotnessName(comp()->getMethodHotness()),
            comp()->signature(), destBlock->getNumber()));
  }
...
const char *TR_EliminateRedundantGotos::optDetailString() const throw() {
...
// textually follows block C (which textually follows block B), then we can
// remove the trees for block C and place them at the end of the method's tree
// list as it's known that no block falls through to block C. We can then
// eliminate the goto.
//
...
void TR_CleanseTrees::prePerformOnBlocks() {
  ...
  if (rootStructure) {
    TR::CFGNode *node;
    for (node = comp()->getFlowGraph()->getFirstNode(); node;
         node = node->getNext()) {
      TR::Block *block = toBlock(node);
      int32_t nestingDepth = 0;
      if (block->getStructureOf() != NULL)
...
int32_t TR_CleanseTrees::process(TR::TreeTop *startTree,
  ...
  // Process each block in treetop order
  //
  for (treeTop = startTree; (treeTop != endTreeTop);
       treeTop = exitTreeTop->getNextTreeTop()) {
    TR::Node *node = treeTop->getNode();
    TR_ASSERT(node->getOpCodeValue() == TR::BBStart,
              "Local Opts, expected BBStart treetop");
    ...
    if (block->getVisitCount() == comp()->getVisitCount()) continue;
    ...
    // if (!node->getOpCode().isBranch()) // commented out since OrderBlocks is
    // now the pass which will decide fall throughs in cases where the branch
    // ends in an 'if'
    if (node->getOpCodeValue() != TR::Goto) continue;
    ...
    if (node->getBranchDestination() == treeTop) continue;
    ...
    if (tt1) tt2 = tt1->getNode()->getBlock()->getExit()->getNextTreeTop();
    ...
    if (tt2 && node->getBranchDestination() == tt2) {
      ...
      if ((tt1->getNode()->getOpCodeValue() == TR::Goto ||
          ...
          (/* (!rootStructure) || */ isBestChoiceForFallThrough(block,
                                                                block2)) &&
          performTransformation(comp(),
                                "%sMoving trees contained in block that was "
                                "the fall through of block_%d\n",
                                optDetailString(), block->getNumber())) {
        // FIXME: the following moves the tree at the end of the method, which
        // is bad
        //
      ...
      if ((tt2 == comp()->getStartTree()) || block->isCold() ||
          block2->isCold() ||
      ...
      // if (node->getOpCodeValue() == TR::Goto)
      {
        result = performChecksAndTreesMovement(
            tt2->getNode()->getBlock(), block, block->getNextBlock(), endTree,
            comp()->getVisitCount(), optimizer());
      }
      // else
      //{
      // result = performChecksAndTreesMovement(tt2->getNode()->getBlock(),
      // block, block->getNextBlock(), endTree, comp()->getVisitCount(),
      // optimizer());
      //}
...
const char *TR_CleanseTrees::optDetailString() const throw() {
...
TR_ProfiledNodeVersioning::temporarilySetProfilingBcInfoOnNewArrayLengthChild(
  ...
  TR_ASSERT(newArray->getOpCodeValue() == TR::newarray ||
                newArray->getOpCodeValue() == TR::anewarray,
            "assertion failure");
    ...
    if (node->getOpCodeValue() == TR::BBEnd) {
      ...
      switch (node->getOpCodeValue()) {
        ...
        case TR::anewarray: {
          ...
          if (numElementsNode->getOpCode().isLoadConst()) continue;

          TR_ByteCodeInfo originalBcInfo = TR_ProfiledNodeVersioning::
              temporarilySetProfilingBcInfoOnNewArrayLengthChild(node, comp());
          TR_ValueInfo *numElementsInfo = static_cast<TR_ValueInfo *>(
              TR_ValueProfileInfoManager::getProfiledValueInfo(
                  numElementsNode, comp(), ValueInfo));
          ...
          if (numElementsInfo) {
            ...
            if (totalFrequency > 0) {
              ...
              TR_ScratchList<TR_ExtraValueInfo> valuesSortedByFrequency(
                  trMemory());
              numElementsInfo->getSortedList(comp(), &valuesSortedByFrequency);
              ListIterator<TR_ExtraValueInfo> i(&valuesSortedByFrequency);
              for (TR_ExtraValueInfo *profiledInfo = i.getFirst();
                   profiledInfo != NULL; profiledInfo = i.getNext()) {
                float probability =
                    (float)profiledInfo->_frequency / totalFrequency;
                if (trace())
                ...
                if (!doProfiling ||
                    (profiledInfo->_value <
                     (uint32_t)(profiledSize * (1.0 + probability)))) {
                  if (numDifferentSizes == 1 &&
                      !versionNewarrayForMultipleSizes)
                    break;

                  numDifferentSizes++;
                  profiledSize = std::max(profiledSize, profiledInfo->_value);
                  minProfiledSize =
                      std::min(minProfiledSize, profiledInfo->_value);
            ...
            if (doProfiling &&
                performTransformation(
                    comp(),
                    "O^O PROFILED NODE VERSIONING: Versioning %s %s, size %s "
                    "%d %.01f%%\n",
                    node->getOpCode().getName(), getDebug()->getName(node),
                    (numDifferentSizes == 1) ? "==" : "<=", profiledSize,
                    100.0 * combinedProbability)) {
              // Create basic diamond
              //
              int16_t originalFrequency = block->getFrequency();

              TR::SymbolReference *storeSymRef =
                  getSymRefTab()->createTemporary(
                      comp()->getMethodSymbol(),
                      numElementsNode->getDataType());

              TR::TreeTop *storeTree = TR::TreeTop::create(
                  comp(), TR::Node::createWithSymRef(
                              TR::istore, 1, 1, numElementsNode, storeSymRef));

              TR::Node *origNumElementsNode = numElementsNode;
              numElementsNode = TR::Node::createWithSymRef(
                  numElementsNode, TR::iload, 0, storeSymRef);
              continuationPoint->join(storeTree);
              storeTree->join(tt);
              node->setAndIncChild(0, numElementsNode);
              origNumElementsNode->recursivelyDecReferenceCount();

              TR::TreeTop *compareTree = TR::TreeTop::create(
                  comp(),
                  TR::Node::createif(
                      (numDifferentSizes == 1) ? TR::ificmpne : TR::ificmpgt,
              ...
              TR::Node *fixedSizeNewOriginalSize =
                  fixedSizeNew->getFirstChild();
              fixedSizeNew->setAndIncChild(
                  0, TR::Node::iconst(numElementsNode, profiledSize));

              TR::TreeTop *slowTree = TR::TreeTop::create(
                  comp(), TR::Node::create(TR::treetop, 1, variableSizeNew));
              TR::TreeTop *fastTree = TR::TreeTop::create(
                  comp(), TR::Node::create(TR::treetop, 1, fixedSizeNew));

              block->createConditionalBlocksBeforeTree(
                  tt, compareTree, slowTree, fastTree, comp()->getFlowGraph(),
                  false);

              if (numDifferentSizes >= 2) {
                // The array is conservatively large; must clobber its length to
                // the right value
                TR::TreeTop::create(
                    comp(), fastTree,
                    TR::Node::createWithSymRef(
                        TR::istorei, 2, 2, fixedSizeNew,
                        fixedSizeNewOriginalSize,
                        comp()
                            ->getSymRefTab()
                            ->findOrCreateContiguousArraySizeSymbolRef()));
              }

              fixedSizeNewOriginalSize
                  ->recursivelyDecReferenceCount();  // removed from
                                                     // fixedSizeNew

              // Store new object to a temp, and replace uses of object with
              // loads of the temp
              //
              TR::SymbolReference *newObjectPointer =
                  getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
                                                  TR::Address);
              TR::TreeTop::create(
                  comp(), slowTree,
              ...
              TR::Node::recreate(
                  node, comp()->il.opCodeForDirectLoad(
                            newObjectPointer->getSymbol()->getDataType()));
              ...
              if (originalFrequency >= 0) {
                int16_t fastFrequency =
                    std::max<int16_t>(originalFrequency * combinedProbability,
                                      MAX_COLD_BLOCK_COUNT + 1);
                int16_t slowFrequency = std::max<int16_t>(
                    originalFrequency * (1 - combinedProbability),
                    MAX_COLD_BLOCK_COUNT + 1);
                if (trace()) {
                  traceMsg(comp(),
                           " -> Setting fast block_%d frequency = %d; slow "
                           "block_%d frequency = %d\n",
                           fastTree->getEnclosingBlock()->getNumber(),
                           fastFrequency,
                           slowTree->getEnclosingBlock()->getNumber(),
                           slowFrequency);
                }
              ...
              traceMsg(comp(),
                       "Not versioning %s node %s because size node %s top "
                       "probability is %.0f%% < %.0f%%\n",
...
const char *TR_ProfiledNodeVersioning::optDetailString() const throw() {
...
void TR_Rematerialization::rematerializeAddresses(TR::Node *indirectNode,
                                                  TR::TreeTop *treeTop,
                                                  vcount_t visitCount) {
  if (visitCount == indirectNode->getVisitCount()) return;

  TR::Symbol *ind_sym = indirectNode->getOpCode().hasSymbolReference()
                            ? indirectNode->getSymbol()
                            : 0;
  ...
  if (indirectNode->getOpCode().isIndirect()) {
    ...
    if (node->getNumChildren() == 2 && node->isInternalPointer()) {
      ...
      while (++throttle < 10) {
        if ((node->getReferenceCount() > 1 && node->isInternalPointer() &&
            ...
            (node->getReferenceCount() == 1 && node->isInternalPointer() &&
             node->getNumChildren() == 2 &&
...
#ifdef J9_PROJECT_SPECIFIC
          if (TR::isJ9() &&
              treeTop->getNode()->getOpCodeValue() == TR::pdstorei) {
            return;
          }
#endif
          ...
          if (performTransformation(comp(), "%sRematerializing node %p(%s)\n",
                                    ...
                                    node->getOpCode().getName())) {
            TR::Node *newChild =
                TR::Node::create(node->getOpCodeValue(), 2,
      ...
      if (cg()->getSupportsScaledIndexAddressing() ||
        ...
        if (cg()->getSupportsConstantOffsetInAddressing() &&
            ...
            {
          TR::ILOpCodes childOp =
              secondChild->getSecondChild()->getOpCodeValue();
          if (secondChild->getSecondChild()->getOpCode().isLoadConst() &&
            ...
            if (secondChild->getReferenceCount() > 1) {
              ...
              // printf("Found a chance in %s\n", comp()->signature());
              if (performTransformation(comp(),
                                        "%sRematerializing node %p(%s)\n",
                                        optDetailString(), secondChild,
                                        secondChild->getOpCode().getName())) {
                TR::Node *newSecondChild =
                    TR::Node::create(secondChild->getOpCodeValue(), 2,
        ...
        if (cg()->getSupportsScaledIndexAddressing() &&
          ...
          if (childOp == TR::iconst || childOp == TR::lconst) {
            ...
            if (childOp == TR::iconst &&
                cg()->isAddressScaleIndexSupported(
                    secondChild->getSecondChild()->getInt()))
              allowUncommoning = true;

            if (childOp == TR::lconst &&
                (secondChild->getSecondChild()->getLongIntHigh() == 0) &&
                cg()->isAddressScaleIndexSupported(
                    secondChild->getSecondChild()->getLongIntLow()))
              allowUncommoning = true;
  ...
  for (int32_t i = 0; i < indirectNode->getNumChildren(); ++i) {
    ...
    if ((child->getOpCodeValue() == TR::iu2l) &&
        comp()->cg()->opCodeIsNoOp(child->getOpCode())) {
      if (treeTop->getNode()->getOpCodeValue() != TR::BBEnd) {
        ...
        if (prevNode->getOpCode().isBranch() ||
            prevNode->getOpCode().isJumpWithMultipleTargets())
          prevTree = prevTree->getPrevTreeTop();
 ...
 public:
  ...
  TR_RematState(TR::Compilation *comp)
      : _currentlyCommonedNodes(comp->trMemory()),
        ...
        _vector128LoadsAlreadyVisitedThatCannotBeRematerialized(
            comp->trMemory()) {}
...
int32_t TR_Rematerialization::process(TR::TreeTop *startTree,
                                      TR::TreeTop *endTree) {
  ...
  _heightArray =
      (int32_t *)trMemory()->allocateStackMemory(_nodeCount * sizeof(int32_t));
  memset(_heightArray, 0, _nodeCount * sizeof(int32_t));

  visitCount = comp()->incVisitCount();
  for (treeTop = startTree; (treeTop != endTree);
       treeTop = treeTop->getNextTreeTop())
    initializeFutureUseCounts(treeTop->getNode(), NULL, visitCount, comp(),
                              _heightArray);
  ...
  for (treeTop = startTree; (treeTop != endTree);
    ...
    if (node->getOpCodeValue() == TR::BBStart) {
      ...
      if (!_curBlock->isExtensionOfPreviousBlock()) {
        ...
        TR_ASSERT(state->_currentlyCommonedLoads.isEmpty(),
                  "Non FP loads list should be empty and is not\n");
        TR_ASSERT(state->_currentlyCommonedFPLoads.isEmpty(),
                  "FP loads list should be empty and is not\n");
...
bool TR_Rematerialization::isRematerializable(TR::Node *parent, TR::Node *node,
                                              bool onlyConsiderOpCode) {
  return node->isRematerializable(parent, onlyConsiderOpCode);
}

bool TR_Rematerialization::isRematerializableLoad(TR::Node *node,
                                                  TR::Node *parent) {
  ...
  if (parent && (parent->getOpCodeValue() == TR::Prefetch) &&
    ...
    if (!_prefetchNodes.find(node)) _prefetchNodes.add(node);
    return true;
  }

  if (!cg()->doRematerialization()) return false;
...
static bool storeMightKillRematerializableSym(TR::Node *node) {
  ...
  if (node->getOpCodeValue() == TR::lstorei) return true;

  if (node->getOpCode().isStoreIndirect()) return true;

  return false;
}

void TR_Rematerialization::removeNodeFromList(
    TR::Node *node, List<TR::Node> *nodes, List<List<TR::Node> > *parents,
    bool checkSymRefs, List<TR::Node> *loadsAlreadyVisited,
    List<TR::Node> *loadsAlreadyVisitedThatCannotBeRematerialized,
  ...
  while (commonedElement) {
    ...
    if ((commonedElement->getData() == node) ||
        (checkSymRefs && ((commonedElement->getData()->getSymbolReference() ==
                           node->getSymbolReference()) ||
                          aliases.ValueAt(commonedElement->getData()
                                              ->getSymbolReference()
                                              ->getReferenceNumber())))) {
      if (prevNodeElem) {
  ...
  if (checkSymRefs && loadsAlreadyVisited) {
    ListElement<TR::Node> *visitedLoadsElement =
        loadsAlreadyVisited->getListHead();
    ListElement<TR::Node> *prevVisitedLoadsElement = NULL;
    while (visitedLoadsElement) {
      if ((visitedLoadsElement->getData()->getSymbolReference() ==
           node->getSymbolReference()) ||
          aliases.ValueAt(visitedLoadsElement->getData()
                              ->getSymbolReference()
                              ->getReferenceNumber())) {
        if (prevVisitedLoadsElement)
          prevVisitedLoadsElement->setNextElement(
              visitedLoadsElement->getNextElement());
        else
          loadsAlreadyVisited->setListHead(
              visitedLoadsElement->getNextElement());
        if (!loadsAlreadyVisitedThatCannotBeRematerialized->find(
                visitedLoadsElement->getData()))
          loadsAlreadyVisitedThatCannotBeRematerialized->add(
              visitedLoadsElement->getData());
      } else
...
void TR_Rematerialization::addParentToList(TR::Node *node,
                                           List<TR::Node> *nodes,
                                           TR::Node *parent,
...
bool TR_Rematerialization::examineNode(TR::TreeTop *treeTop, TR::Node *parent,
  ...
  if ((((node->getFutureUseCount() & 0x7fff) == 0) &&
    ...
    if (node->getOpCode().isFloatingPoint()) {
      ...
      if (isRematerializable(parent, node, true) &&
        ...
        if (isRematerializableLoad(node, parent))
          removeNodeFromList(node, &state->_currentlyCommonedFPLoads,
                             &state->_parentsOfCommonedFPLoads, false);
        else
          removeNodeFromList(node, &state->_currentlyCommonedFPCandidates,
                             &state->_fpParents, false);
      }
      ...
      if (isRematerializable(parent, node, true) &&
        ...
        if (isRematerializableLoad(node, parent))
          ...
          removeNodeFromList(node,
                             &state->_currentlyCommonedVector128Candidates,
                             &state->_vector128Parents, false);
      }
    } else {
      ...
      if ((isRematerializable(parent, node, true) &&
        ...
        if (trace()) traceMsg(comp(), "Removing node %p\n", node);
        if (isRematerializableLoad(node, parent) || _prefetchNodes.find(node))
          removeNodeFromList(node, &state->_currentlyCommonedLoads,
                             &state->_parentsOfCommonedLoads, false);
        else
          removeNodeFromList(node, &state->_currentlyCommonedCandidates,
                             &state->_parents, false);
      }
    }
  }
  ...
  if (((((node->getFutureUseCount() & 0x7fff) > 0) &&
    ...
    if (node->getOpCode().isFloatingPoint()) {
      if (isRematerializableLoad(node, parent))
        addParentToList(node, &state->_currentlyCommonedFPLoads, parent,
                        &state->_parentsOfCommonedFPLoads);
      else
        addParentToList(node, &state->_currentlyCommonedFPCandidates, parent,
                        &state->_fpParents);
    } else if (node->getOpCode().isVector()) {
      if (isRematerializableLoad(node, parent))
        ...
        addParentToList(node, &state->_currentlyCommonedVector128Candidates,
                        parent, &state->_vector128Parents);
    } else {
      if (isRematerializableLoad(node, parent))
        addParentToList(node, &state->_currentlyCommonedLoads, parent,
                        &state->_parentsOfCommonedLoads);
      else
        addParentToList(node, &state->_currentlyCommonedCandidates, parent,
                        &state->_parents);
    }
  }
  ...
  if (node->getOpCodeValue() == TR::GlRegDeps) _underGlRegDeps = true;
  ...
  while (true) {
    ...
    for (i = 0; i < node->getNumChildren(); ++i) {
      ...
      // if (child->getVisitCount() != visitCount)
      // if ((child->getFutureUseCount() & 0x7fff) ==
  ...
  node->getNumChildren(), seenChildren.getSize()); TR_ASSERT(0, "Did not visit
  all children\n");
     }
  */

  if (node->getOpCodeValue() == TR::GlRegDeps) _underGlRegDeps = false;

  node->setVisitCount(visitCount);

  static char *regPress = feGetEnv("TR_IgnoreRegPressure");

  if ((regPress != NULL) && (node->getNumChildren() == 2) &&
      isRematerializableLoad(node->getFirstChild(), node) &&

      (!node || (node->getOpCodeValue() != TR::Prefetch) ||
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst() ||
      ...
      if (node->getOpCode().isFloatingPoint())
        removeNodeFromList(node->getFirstChild(),
                           &state->_currentlyCommonedFPLoads,
                           &state->_parentsOfCommonedFPLoads, true);
      else if (node->getOpCode().isVector()) {
        removeNodeFromList(node->getFirstChild(),
                           &state->_currentlyCommonedVector128Loads,
                           &state->_parentsOfCommonedVector128Loads, true);
      } else
        removeNodeFromList(node->getFirstChild(),
                           &state->_currentlyCommonedLoads,
  ...
  // if (node->getOpCode().isStoreDirect() &&
  //    node->getSymbolReference()->getSymbol()->isAutoOrParm())
  if (storeMightKillRematerializableSym(node)) removeNodes = true;

  bool isSimilarToCall = false;
  bool isCall = false;
  if ((node->getOpCode().hasSymbolReference() &&
       node->getSymbolReference()->isUnresolved() && parent &&
  ...
  if (node->getOpCode().isCall() &&
    ...
    isCall = true;
    if (node->getOpCode().isCallDirect()) isCallDirect = true;
  }
  ...
  if (removeNodes) {
    ...
    if (node->getOpCode().isFloatingPoint())
      removeNodeFromList(
          node, &state->_currentlyCommonedFPLoads,
          &state->_parentsOfCommonedFPLoads, true,
          &state->_fpLoadsAlreadyVisited,
          &state->_fpLoadsAlreadyVisitedThatCannotBeRematerialized, aliases);
    else if (node->getOpCode().isVector()) {
      removeNodeFromList(
          node, &state->_currentlyCommonedVector128Loads,
          &state->_parentsOfCommonedVector128Loads, true,
          &state->_vector128LoadsAlreadyVisited,
          &state->_vector128LoadsAlreadyVisitedThatCannotBeRematerialized,
          aliases);
    } else
      removeNodeFromList(
          node, &state->_currentlyCommonedLoads,
          &state->_parentsOfCommonedLoads, true, &state->_loadsAlreadyVisited,
          &state->_loadsAlreadyVisitedThatCannotBeRematerialized, aliases);
  }

  bool considerRegPressure = true;  // enable for other platforms later
  bool rematSpecialNode = false;

  if ((NULL != regPress) || (!state->_currentlyCommonedLoads.isEmpty() &&
                             (state->_currentlyCommonedLoads.getListHead()
                                  ->getData()
                                  ->getOpCodeValue() == TR::lloadi) &&
                             state->_currentlyCommonedLoads.getListHead()
                                 ->getData()
                                 ->isBigDecimalLoad()))
  ...
  if ((NULL == regPress) && !considerRegPressure) rematSpecialNode = true;
  ...
  if ((NULL != regPress) ||
      (!state->_currentlyCommonedLoads.isEmpty() &&
       ...
       (state->_parentsOfCommonedLoads.getListHead()
            ->getData()
            ->getListHead()
            ->getData()
            ->getOpCodeValue() == TR::Prefetch))) {
    considerRegPressure = false;
  }
  ...
  if (TR::Compiler->target.is32Bit()) {
    ...
    for (commonedNode = nodesIt.getFirst(); commonedNode;
         commonedNode = nodesIt.getNext()) {
      if (commonedNode->getType().isInt64()) numRegisters++;
      numRegisters++;
    }
  }
  ...
  if (trace()) {
    traceMsg(comp(),
             "At node %p parent %p GPR pressure is %d (child adjust %d parent "
             "adjust %d) limit is %d\n",
             node, parent,
             numRegisters + childAdjustment + adjustments.adjustmentFromParent,
             childAdjustment, adjustments.adjustmentFromParent,
  ...
  if ((!considerRegPressure ||
       ((state->_currentlyCommonedNodes.getSize() + childAdjustment +
         adjustments.adjustmentFromParent) >
        (cg()->getMaximumNumbersOfAssignableGPRs() /* -1 */))) &&
      (!state->_currentlyCommonedCandidates.isEmpty() ||
       !state->_currentlyCommonedLoads.isEmpty())) {
    //_counter++;
    // printf("Rematerializing in %s\n", comp()->signature());
    rematerializeNode(
        treeTop, parent, node, visitCount, &state->_currentlyCommonedNodes,
        &state->_currentlyCommonedCandidates, &state->_parents,
        &state->_currentlyCommonedLoads, &state->_parentsOfCommonedLoads,
        &state->_loadsAlreadyVisited,
        &state->_loadsAlreadyVisitedThatCannotBeRematerialized,
        rematSpecialNode);
  }

  if (trace() && node->getOpCode().isFloatingPoint()) {
    traceMsg(comp(),
             "At node %p FPR pressure is %d (child adjust %d parent adjust %d) "
             "limit is %d\n",
             node,
             state->_currentlyCommonedFPNodes.getSize() + fpChildAdjustment +
                 adjustments.fpAdjustmentFromParent,
             fpChildAdjustment, adjustments.fpAdjustmentFromParent,
             (cg()->getMaximumNumbersOfAssignableFPRs() - 1));
    traceMsg(comp(), "candidate nodes size %d candidate loads size %d\n",
             state->_currentlyCommonedFPCandidates.getSize(),
             state->_currentlyCommonedFPLoads.getSize());
  }
  ...
  if ((!considerRegPressure ||
       ((state->_currentlyCommonedFPNodes.getSize() + fpChildAdjustment +
         adjustments.fpAdjustmentFromParent) >
        cg()->getMaximumNumbersOfAssignableFPRs() /* -1 */)) &&
      (!state->_currentlyCommonedFPCandidates.isEmpty() ||
       !state->_currentlyCommonedFPLoads.isEmpty())) {
    //_counter++;
    // printf("Rematerializing FP node in %s\n", comp()->signature());
    rematerializeNode(
        treeTop, parent, node, visitCount, &state->_currentlyCommonedFPNodes,
        &state->_currentlyCommonedFPCandidates, &state->_fpParents,
        &state->_currentlyCommonedFPLoads, &state->_parentsOfCommonedFPLoads,
        &state->_fpLoadsAlreadyVisited,
        &state->_fpLoadsAlreadyVisitedThatCannotBeRematerialized,
        rematSpecialNode);
  }
  ...
  if (trace() && node->getOpCode().isVector()) {
    traceMsg(
        comp(),
        "At node %p VSR pressure is %d (child adjust %d parent adjust %d) "
        "limit is %d\n",
        node,
        state->_currentlyCommonedVector128Nodes.getSize() +
            vector128ChildAdjustment +
            adjustments.vector128AdjustmentFromParent,
        vector128ChildAdjustment, adjustments.vector128AdjustmentFromParent,
        vector128ChildAdjustment + adjustments.vector128AdjustmentFromParent);
    traceMsg(comp(), "candidate nodes size %d candidate loads size %d\n",
             state->_currentlyCommonedVector128Candidates.getSize(),
             state->_currentlyCommonedVector128Loads.getSize());
  }
  ...
  if ((!considerRegPressure ||
    ...
    rematerializeNode(
        treeTop, parent, node, visitCount,
        &state->_currentlyCommonedVector128Nodes,
        &state->_currentlyCommonedVector128Candidates,
        &state->_vector128Parents, &state->_currentlyCommonedVector128Loads,
        &state->_parentsOfCommonedVector128Loads,
        &state->_vector128LoadsAlreadyVisited,
        &state->_vector128LoadsAlreadyVisitedThatCannotBeRematerialized,
        rematSpecialNode);
  ...
  if ((node->getFutureUseCount() & 0x7fff) > 0) {
    if (!node->getOpCode().isLoadConst()) {
      ...
      if (parent && !_underGlRegDeps && !parent->getOpCode().isStoreReg() &&
          isRematerializable(parent, node) && node->isRematerializeable()) {
        if (isRematerializableLoad(node, parent)) {
          if (node->getOpCode().isFloatingPoint()) {
            if (!state->_fpLoadsAlreadyVisitedThatCannotBeRematerialized.find(
                    node)) {
              // traceMsg(comp(), "1Adding node %p to
              // _currentlyCommonedFPLoads\n", node);
            ...
            if (!state->_vector128LoadsAlreadyVisitedThatCannotBeRematerialized
                     .find(node)) {
              // traceMsg(comp(), "1Adding node %p to
              // _currentlyCommonedVector128Loads\n", node);
            ...
            if (!state->_loadsAlreadyVisitedThatCannotBeRematerialized.find(
                    node)) {
              // traceMsg(comp(), "1Adding node %p to
              // _currentlyCommonedLoads\n", node);
          ...
          if (node->getOpCode().isFloatingPoint()) {
            // traceMsg(comp(), "2Adding node %p to
            // _currentlyCommonedFPCandidates\n", node);
            ...
            // traceMsg(comp(), "2Adding node %p to
            // _currentlyCommonedVector128Candidates\n", node);
...
static List<TR::Node> *getParentList(TR::Node *node,
                                     List<List<TR::Node> > *parents,
                                     List<TR::Node> *nodes) {
...
void TR_Rematerialization::rematerializeNode(
    TR::TreeTop *treeTop, TR::Node *parent, TR::Node *node, vcount_t visitCount,
    List<TR::Node> *currentlyCommonedNodes,
    List<TR::Node> *currentlyCommonedCandidates, List<List<TR::Node> > *parents,
    List<TR::Node> *currentlyCommonedLoads,
    List<List<TR::Node> > *parentsOfCommonedLoads,
    List<TR::Node> *loadsAlreadyVisited,
    List<TR::Node> *loadsAlreadyVisitedThatCannotBeRematerialized,
  ...
  if (!rematSpecialNode && !currentlyCommonedCandidates->isEmpty()) {
    ...
    ListElement<TR::Node> *commonedNodeListElem =
        currentlyCommonedCandidates->getListHead();
    ListElement<List<TR::Node> > *parentListElem = parents->getListHead();
    TR::Node *commonedNode = NULL;
    ...
    while (commonedNodeListElem) {
      ...
      if (commonedNode->getOpCode().isArrayLength()) break;
    ...
    if (commonedNodeListElem) {
      ...
      ListElement<TR::Node> *nextNodeListElem =
          commonedNodeListElem->getNextElement();
      ListElement<List<TR::Node> > *nextParentListElem =
          parentListElem->getNextElement();
  ...
  if (!nodeToBeRematerialized) {
    if (!currentlyCommonedLoads->isEmpty()) {
      ...
      nodeToBeRematerialized =
          currentlyCommonedCandidates->getListHead()->getData();
      nodes = currentlyCommonedCandidates;
      parentsOfNodes = parents;
    }
  }
  ...
  if (!isAdjunctNode &&
    ...
    if (nodeToBeRematerialized->getNumChildren() > 0) {
      ...
      if (isRematerializable(nodeToBeRematerialized, firstChild) &&
          ...
          (!parent || (parent->getOpCodeValue() != TR::Prefetch) ||
           (node->getOpCodeValue() != TR::aloadi)) &&
        ...
        for (int32_t j = 0; j < newFirstChild->getNumChildren(); j++) {
          ...
          List<TR::Node> *parentList = getParentList(
              newFirstChild->getChild(j), parents, currentlyCommonedCandidates);
          if (parentList) {
          ...
          parentList =
              getParentList(newFirstChild->getChild(j), parentsOfCommonedLoads,
                            currentlyCommonedLoads);
          if (parentList) {
            parentList->add(newFirstChild);
            parentList->remove(firstChild);
          }
    ...
    for (int32_t j = 0; j < newNode->getNumChildren(); j++) {
      ...
      List<TR::Node> *parentList = getParentList(newNode->getChild(j), parents,
                                                 currentlyCommonedCandidates);
      if (parentList) {
        parentList->add(newNode);
        parentList->remove(nodeToBeRematerialized);
      }

      parentList = getParentList(newNode->getChild(j), parentsOfCommonedLoads,
                                 currentlyCommonedLoads);
      if (parentList) {
        parentList->add(newNode);
        parentList->remove(nodeToBeRematerialized);
      ...
      if (parentNode /* &&
        ...
        for (int32_t j = 0; j < parentNode->getNumChildren(); j++) {
          if (parentNode->getChild(j) == nodeToBeRematerialized) {
            if (trace()) traceMsg(comp(), "\tin parent node %p\n", parentNode);
            parentNode->setAndIncChild(j, newNode);
            nodeToBeRematerialized->recursivelyDecReferenceCount();

            List<TR::Node> *parentList2 = getParentList(
                nodeToBeRematerialized, parents, currentlyCommonedCandidates);
            if (parentList2) parentList->remove(parentNode);

            parentList2 =
                getParentList(nodeToBeRematerialized, parentsOfCommonedLoads,
                              currentlyCommonedLoads);
            if (parentList2) parentList2->remove(parentNode);
          ...
          if (parentList->isSingleton()) {
            parentNode->setAndIncChild(
                ...
                TR::Node::create(TR::PassThrough, 1,
                                 parentNode->getFirstChild()->getFirstChild()));
            optimizer()->setRequestOptimization(OMR::compactNullChecks, true,
                                                _curBlock);
    ...
    if (!mustBeAnchored)
      ...
      TR::Node *treetopNode =
          TR::Node::create(TR::treetop, 1, nodeToBeRematerialized);

      List<TR::Node> *parentList = getParentList(
          nodeToBeRematerialized, parents, currentlyCommonedCandidates);
      if (parentList) parentList->add(treetopNode);

      parentList = getParentList(nodeToBeRematerialized, parentsOfCommonedLoads,
                                 currentlyCommonedLoads);
      if (parentList) parentList->add(treetopNode);
      ...
      optimizer()->setRequestOptimization(OMR::deadTreesElimination, true,
                                          _curBlock);
      nodeToBeRematerialized->setVisitCount(visitCount - 1);
    }

    if (mustBeAnchored && !loadsAlreadyVisited->find(nodeToBeRematerialized) &&
        !loadsAlreadyVisitedThatCannotBeRematerialized->find(
            nodeToBeRematerialized))
      loadsAlreadyVisited->add(nodeToBeRematerialized);

    nodes->setListHead(nodes->getListHead()->getNextElement());
    parentsOfNodes->setListHead(
        parentsOfNodes->getListHead()->getNextElement());
    currentlyCommonedNodes->remove(nodeToBeRematerialized);
  }
}

const char *TR_Rematerialization::optDetailString() const throw() {
...
bool TR_BlockSplitter::disableSynergy() {
  static const char *disableBlockSplitterSynergyStr =
      feGetEnv("TR_DisableBlockSplitterSynergy");
...
int32_t TR_BlockSplitter::perform() {
  ...
  TR::CFG *cfg = comp()->getFlowGraph();
  if (!cfg) {
    if (trace()) traceMsg(comp(), "No cfg, aborting BlockSplitter\n");
    return 0;
  }
  ...
  if (disableSynergy() && comp()->isOptServer()) {
    if (trace())
      traceMsg(comp(),
               "Large app detected, making BlockSplitter very conservative to "
               "avoid cloning code (increases i-cache pressure)\n");
  }

  // static const char *doit = feGetEnv("TR_BlockSplitting");

  static char *maxWarmSplitterDepth = feGetEnv("TR_maxWarmSplitterDepth");
  static char *maxScorchingSplitterDepth =
      feGetEnv("TR_maxScorchingSplitterDepth");
  ...
  switch (comp()->getMethodHotness()) {
    ...
    case scorching:
      maxDepth =
          maxScorchingSplitterDepth ? atoi(maxScorchingSplitterDepth) : 9;
  ...
  // from this point on, all stack allocations will die when the function
  // returns
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  ...
  static const char *p;
  static int32_t splitPercentage =
      (p = feGetEnv("TR_SplitPercentage")) ? atoi(p) : DEFAULT_SPLIT_PERCENTAGE;
  int32_t splitFrequency = splitPercentage * totalFrequency / 100;
  static int32_t splitPredPercentage = (p = feGetEnv("TR_SplitPredPercentage"))
                                           ? atoi(p)
                                           : DEFAULT_SPLIT_PRED_PERCENTAGE;
  int32_t splitPredFrequency = splitPredPercentage * totalFrequency / 100;
  // the following is a debug facility to allow splitting to be disabled by
  // block number for testing purposes
  static int32_t skipBlock = (p = feGetEnv("TR_SkipMergeBlock")) ? atoi(p) : -1;

  if (trace())
    traceMsg(comp(),
             "  alpha=%f maxDepth=%d totalFrequency=%d splitFrequency=%d "
             "splitPredFrequency=%d\n",
             alpha, maxDepth, totalFrequency, splitFrequency,
             splitPredFrequency);
  ...
  TR_Stack<TR::CFGNode *> *children =
      new (trStackMemory()) TR_Stack<TR::CFGNode *>(trMemory());
  uint32_t index = 0;
  ...
  cfg->comp()->getStartTree()->getNode()->getBlock()->setVisitCount(
      comp()->getVisitCount());

  // post order traversal of the nodes in the graph looking for the merge points
  // we should visit the order is important because the index will be set in
  // visitation order and we need to sort by this index to obtain optimal
  // splitting

  if (trace()) traceMsg(comp(), "  Starting search for merge points\n");
  while (children->size() > 0) {
    TR::CFGEdge *unvisitedEdge = NULL;
    for (auto edge = children->top()->getSuccessors().begin();
         edge != children->top()->getSuccessors().end(); ++edge) {
    ...
    if ((mergeNode->hasExceptionPredecessors() == true) ||
    ...
    if (mergeNode->getFrequency() < splitFrequency) {
      if (trace())
        traceMsg(comp(),
                 "    rejecting block_%d, frequency is %d, threshold is %d\n",
                 mergeNode->getNumber(), mergeNode->getFrequency(),
                 splitFrequency);
      children->pop();
      continue;
    }
    ...
    if (mergeNode->getFrequency() > 0) hasNonZeroFrequencies = true;

    if (disableSynergy() && comp()->isOptServer() &&
        !mergeNode->isEmptyBlock()) {
      if (trace())
      ...
      if (mergeNode->getNumberOfRealTreeTops() > 1) {
        ...
        if (trace())
          traceMsg(
              comp(),
              "    rejecting because mergeNode %d has more than 1 treetop\n",
              mergeNode->getNumber());
        continue;
      }
      ...
      for (auto edge = mergeNode->getPredecessors().begin();
           edge != mergeNode->getPredecessors().end(); ++edge) {
        node = (*edge)->getFrom();
        ...
        if (predBlock->getEntry() &&
          ...
          if (trace())
            traceMsg(comp(),
                     "    rejecting because predecessor block %d has more than "
                     "4 treetops\n",
                     predBlock->getNumber());
          break;
        }
    ...
    if (!mergeNode->isEmptyBlock()) {
      ...
      TR_IndexedBinaryHeapElement *temp =
          new (trStackMemory()) TR_IndexedBinaryHeapElement(mergeNode, index++);
      heap->add(temp);
    }
    children->pop();
  }
  heapElementQuickSort((TR_Array<TR_IndexedBinaryHeapElement *> *)heap, 0,
                       heap->lastIndex());
  if (trace()) {
  ...
  if (!hasNonZeroFrequencies) {
    if (trace())
      traceMsg(comp(),
               "Terminating: found merge blocks with nonzero frequency\n");
    return 0;
  }

  // Process the hottest merge block iteratively until it's time to stop
  //
  TR_Array<TR_IndexedBinaryHeapElement *> *array =
      (TR_Array<TR_IndexedBinaryHeapElement *> *)heap;
  ...
  for (int32_t i = 0; i <= array->lastIndex(); ++i) {
    ...
    if (disableSynergy()) splitPred_to_mergeNode_edge = NULL;
    ...
    for (auto edge = mergeNode->getPredecessors().begin();
      ...
      if (toBlock(node)->getEntry() &&
          ...
          (toBlock(node)
           ...
           toBlock(node)
               ...
               .hasBranchChildren()))
        continue;
      if (toBlock(node)->getNumber() == mergeNode->getNumber()) continue;

      if (toBlock(node)->getEntry()) {
        TR::Node *lastRealNode =
            toBlock(node)->getExit()->getPrevRealTreeTop()->getNode();
        // check to make sure we aren't about to chose a block that is reached
        // as the target of a virtual guard that we are not allowed to reverse,
        // if this is the case we must chose someone else
        if (lastRealNode->getOpCode().isIf() &&
            lastRealNode->isTheVirtualGuardForAGuardedInlinedCall() &&
            lastRealNode->getBranchDestination()
                    ->getNode()
                    ->getBlock()
                    ->getNumber() == mergeNode->getNumber())
          continue;
        // check to make sure we aren't about to choose a block that is the
        // target of a GCR counting guard which could be patched, we are not
        // allowed to reverse these either and must choose someone else
        if (lastRealNode->getOpCode().isIf() &&
      ...
      if (!splitPred || splitPred->getFrequency() < node->getFrequency()) {
        ...
        if (trace())
          traceMsg(comp(),
                   "mergeNode(%d) splitPred(%d) blockfreq = %d edgefreq = %d "
                   "candidate\n",
                   mergeNode->getNumber(), splitPred->getNumber(),
                   splitPred->getFrequency(), (*edge)->getFrequency());
      }
    }

    // Make sure there is a candidate predecessor and that it is hot enough
    //
    if (!splitPred || (splitPred->getFrequency() < splitPredFrequency) ||
        !splitPred_to_mergeNode_edge ||
        // TODO: the divide by 10 here is because edge frequencies are much
        // lower than block frequencies because of frequency maintenance issues
        // elsewhere, it can be removed once all frequencies are properly
        // normalized
        (splitPred_to_mergeNode_edge->getFrequency() <
         (splitPredFrequency / 10)) ||
        (predFrequency <= 0) || (predEdgeFrequency <= 0)) {
      if (trace())
    ...
    if (!performTransformation(
            comp(),
            "%sSplitting merge block_%d for predecessor %d (frequency %d)\n",
            optDetailString(), mergeNode->getNumber(), splitPred->getNumber(),
            mergeNode->getFrequency()))
      continue;
    ...
    TR::Block *to = new (trHeapMemory())
        TR::Block(*mergeNode, TR::TreeTop::create(comp(), NULL),
                  TR::TreeTop::create(comp(), NULL));
    to->getEntry()->join(to->getExit());
    if (bMap.getLast()) bMap.getLast()->_to->getExit()->join(to->getEntry());
    ...
    while (depth > 1) {
      ...
      TR::Node *lastRealNode =
          target->getExit()->getPrevRealTreeTop()->getNode();
      for (auto edge = target->getSuccessors().begin();
           edge != target->getSuccessors().end(); ++edge) {
        child = toBlock((*edge)->getTo());

        // Editorial Comment, the parent check is an expensive check so leave it
        // until last we want to skip as many checks as we can, so check the
        // frequency early
        ...
        if (child->getFrequency() <= (target->getFrequency() - tollerance)) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: frequency is too low (%d "
                     "< %d)\n",
                     child->getNumber(), child->getFrequency(),
                     (target->getFrequency() - tollerance));
          continue;
        }
        ...
        if (!(child->getPredecessors().size() == 1)) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: merge block -- we'll "
                     "process this separately\n",
                     child->getNumber());
          continue;
        }

        // if we have a cloned node, structure questions need to be asked about
        // the original node so perform the mapping here
        TR::Block *toCheck = child;
        for (BlockMapper *itr = newToOldMapping.getFirst(); itr;
             itr = itr->getNext()) {
          if (itr->_to == child) {
            toCheck = itr->_from;
            break;
        ...
        if (containCycle(child, &bMap)) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: would cause cycle in "
                     "block mapper\n",
                     child->getNumber());
          continue;
        }

        // finally check to make sure we aren't about to chose a block that is
        // reached as the target of a virtual guard that we are not allowed to
        // reverse, if this is the case we must stop here
        if (lastRealNode->getOpCode().isIf() &&
            lastRealNode->isTheVirtualGuardForAGuardedInlinedCall() &&
            lastRealNode->getBranchDestination()
                    ->getNode()
                    ->getBlock()
                    ->getNumber() == child->getNumber()) {
          if (trace())
            traceMsg(comp(),
                     "    Reject successor block_%d: virtual guard branch that "
                     "can't be reversed\n",
                     child->getNumber());
          continue;
        }

        // if we reach this point, we passed all the checks so we are good to
        // split through the child we are checking
        if (trace())
          traceMsg(comp(), "  Split through successor block_%d\n",
                   child->getNumber());

        // create an empty block ready to be filled by the cloner and add it to
        // the block mapper
    ...
    if (pruneAndPopulateBlockMapper(
            &bMap, synergisticDepthCalculator(&bMap, splitPred)) > 0) {
      for (BlockMapper *itr = bMap.getFirst(); itr; itr = itr->getNext())
        newToOldMapping.append(new (trStackMemory())
                                   BlockMapper(itr->_from, itr->_to));
      // we need to setup the mergeNode with the frequency for all the childern
      int16_t hotFreq =
          splitPred->getFrequency() * mergeNode->getFrequency() / predFrequency;
      int16_t coldFreq = mergeNode->getFrequency() - hotFreq;
      if (hotFreq < splitPred->getFrequency()) {
        static const bool force =
            feGetEnv("TR_forceBlockSplitterFrequencyAdjustment") != NULL;
        if (force || splitPred->getSuccessors().size() == 1) {
          TR::DebugCounter::incStaticDebugCounter(
              comp(),
              TR::DebugCounter::debugCounterName(
                  comp(), "blockSplitter.inconsistentFreqs/%s/(%s)/block_%d",
                  comp()->getHotnessName(comp()->getMethodHotness()),
                  comp()->signature(), mergeNode->getNumber()));
          hotFreq = splitPred->getFrequency();
        }
      }
      int16_t hotEdgeFreq = splitPred_to_mergeNode_edge->getFrequency();
      int16_t coldEdgeFreq =
          (predEdgeFrequency - splitPred_to_mergeNode_edge->getFrequency()) /
          predEdgeFrequency;
      if (coldFreq < 6) coldFreq = 6;
      ...
      for (BlockMapper *itr = bMap.getFirst(); itr; itr = itr->getNext()) {
        ...
        if (trace())
          traceMsg(comp(), "block %d origFreq = %d ", itr->_from->getNumber(),
                   origFreq);

        itr->_from->setFrequency(coldFreq);

        // set frequencies for incoming/outcoming edges to/from the block
        // itr->_from
        if (origFreq > 0) {
          TR::CFGEdgeList &predecessors = itr->_from->getPredecessors();
          for (auto predEdge = predecessors.begin();
               predEdge != predecessors.end(); ++predEdge) {
          ...
          for (auto succEdge = successors.begin(); succEdge != successors.end();
               ++succEdge) {
          ...
          if (itr->_from->getPredecessors().size() == 1) {
            ...
            if (trace())
              traceMsg(comp(),
                       "single predEdge freq=%d. set it to %d instead\n",
          ...
          if (itr->_from->getSuccessors().size() == 1) {
            ...
            if (trace())
              traceMsg(comp(),
                       "single succEdge freq=%d. set it to %d instead\n",
        ...
        // possibly update frequencies for incoming/outcoming edges to/from the
        // block itr->_to
        if (itr->_to->getPredecessors().size() == 1) {
          if (trace())
            traceMsg(comp(), "predEdge freq=%d. set it to %d instead\n",
                     itr->_to->getPredecessors().front()->getFrequency(),
                     hotEdgeFreq);
        ...
        if (itr->_to->getSuccessors().size() == 1) {
          if (trace())
            traceMsg(comp(), "succEdge freq=%d. set it to %d instead\n",
                     itr->_to->getSuccessors().front()->getFrequency(),
                     hotEdgeFreq);
      ...
      if (!(mergeNode->getPredecessors().size() == 1) &&
          mergeNode->getFrequency() >= splitFrequency && horizontalCount < 2) {
        ++i;
        ++horizontalCount;
...
void TR_BlockSplitter::heapElementQuickSort(
    TR_Array<TR_IndexedBinaryHeapElement *> *array, int32_t left,
    int32_t right) {
  if (right - left > 1) {
    ...
    if (left < i) heapElementQuickSort(array, left, i - 1);
    if (right > i) heapElementQuickSort(array, i + 1, right);
...
void TR_BlockSplitter::quickSortSwap(
    TR_Array<TR_IndexedBinaryHeapElement *> *array, int32_t left,
    int32_t right) {
  TR_IndexedBinaryHeapElement *temp;
  temp = (*array)[left];
...
int32_t TR_BlockSplitter::pruneAndPopulateBlockMapper(
    TR_LinkHeadAndTail<BlockMapper> *bMap, int32_t depth) {
  int32_t cloned = 0;
  if (depth) {
    if (trace()) {
      for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
    ...
    for (itr = bMap->getFirst(); itr; itr = itr->getNext()) {
      ...
      itr->_to = new (trHeapMemory())
          TR::Block(*itr->_from, TR::TreeTop::create(comp(), NULL),
                    OMR::TreeTop::create(comp(), NULL));
      itr->_to->getEntry()->join(itr->_to->getExit());
      if (prevTreeTop) prevTreeTop->join(itr->_to->getEntry());
...
int32_t TR_BlockSplitter::synergisticDepthCalculator(
    TR_LinkHeadAndTail<BlockMapper> *bMap, TR::Block *startPoint) {
  // Step 1 - find the start of the preamble
  TR_Stack<TR::Block *> preamble = TR_Stack<TR::Block *>(trMemory());
  preamble.push(startPoint);

  if (trace()) traceMsg(comp(), "  Starting synergisticDepthCalculator\n");
  ...
  if (trace()) traceMsg(comp(), "    Find synergy in blocks being split\n");
  comp()->incVisitCount();
  ...
  for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
    for (TR::TreeTop *treeTopItr = itr->_from->getEntry();
         ...
         treeTopItr = treeTopItr->getNextTreeTop()) {
      cost += processNode(treeTopItr->getNode(), blockIndex, &synergyIndices,
                          &synergies);
    }
  ...
  for (i = 2; i < synergies.size(); ++i) {
    ...
    synergies[synergies.lastIndex() - i].downwardSynergy +=
        synergies[synergies.size() - i].downwardSynergy;
  }
  ...
  if (disableSynergy()) {
    ...
    for (i = 2; i < synergies.size(); ++i) {
      ...
      // push the merge as far down as possible which is what the below
      // calcualtions represent
      //
      // a scoreDelta >= 0 means we have an equally good solution so take it
      // a scoreDelta between 0 and -1 means the drop is due to extra nodes
      // copied so allow up to one alpha's worth of cost to push the merge point
      // further down
      ...
      if (scoreDelta >= 0) {
        ...
        newDepth = i - 1;
      } else if (scoreDelta < 0 && scoreDelta >= -1 &&
                 (alpha * (double)costDelta) < 1) {
        newDepth = i - 1;
      }
    }
  }

  if (trace()) traceMsg(comp(), "  Suggested new depth is %d\n", newDepth);
  return newDepth;
}

int32_t TR_BlockSplitter::processNode(TR::Node *node, int32_t blockIndex,
                                      TR_Array<uint32_t> *synergyIndices,
                                      TR_Array<Synergy> *synergies) {
  node->setVisitCount(comp()->getVisitCount());
  // splitting through a goto is free because the goto will disappear -
  // otherwise the node counts
  int32_t cloneSize = 1;
  ...
  if (node->getOpCode().hasSymbolReference() &&
    ...
    int32_t symbolIndex = node->getSymbolReference()->getReferenceNumber() -
                          comp()->getSymRefTab()->getNumHelperSymbols();
    if (synergies &&
        (node->getOpCode().isLoad() || node->getOpCode().isStore())) {
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i) {
    if (node->getChild(i)->getVisitCount() != comp()->getVisitCount()) {
      cloneSize +=
          processNode(node->getChild(i), blockIndex, synergyIndices, synergies);
    }
  }
  return cloneSize;
}
  ...
  if (trace()) {
    for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
      if (itr == bMap->getFirst())
        traceMsg(comp(), "    Splitting block_%d for %s",
                 itr->_from->getNumber(), comp()->signature());
      else if (itr == bMap->getFirst()->getNext())
        traceMsg(comp(), "\n      Splitting additional block(s): %d",
                 itr->_from->getNumber());
      else
...
bool TR_BlockSplitter::containCycle(TR::Block *blk,
                                    TR_LinkHeadAndTail<BlockMapper> *bMap) {
  for (auto edge = blk->getSuccessors().begin();
       edge != blk->getSuccessors().end(); ++edge) {
    TR::Block *succBlk = toBlock((*edge)->getTo());
    for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
      if (itr->_from->getNumber() == succBlk->getNumber()) return true;
    }
  }
  return false;
}
...
bool TR_BlockSplitter::isExitEdge(TR::Block *mergeNode, TR::Block *successor) {
  ...
  if (!parent || !parent->isNaturalLoop()) return false;
...
bool TR_BlockSplitter::hasIVUpdate(TR::Block *block) {
  ...
  if (trace())
    traceMsg(comp(), "   checking for IVUpdate in block_%d\n",
             block->getNumber());
  for (OMR::TreeTop *treeTopItr = block->getEntry();
       treeTopItr && treeTopItr != block->getExit()->getNextTreeTop();
       treeTopItr = treeTopItr->getNextTreeTop()) {
  ...
  if (trace()) traceMsg(comp(), "    no IVUpdate found\n");
  return false;
}
...
TR::Block *TR_BlockSplitter::splitBlock(TR::Block *pred,
                                        TR_LinkHeadAndTail<BlockMapper> *bMap) {
  ...
  if (trace()) {
    if (pred)
      traceMsg(comp(), "  join %d to %d\n", pred->getNumber(),
               cloneStart->getNumber());
    if (pred && pred->getNextBlock() && pred->getNextBlock()->getEntry())
      traceMsg(comp(), "  join %d to %d\n", cloneEnd->getNumber(),
               pred->getNextBlock()->getNumber());
  }

  // if we have a valid predecessor we just add the new code as normal
  // if the pred has a null entry we have the dummy start so we must set the new
  // start location for the program and update the trees accordingly
  //
  if (pred->getEntry()) {
    ...
    if (lastRealNode->getOpCode().isBranch()) {
      if (lastRealNode->getOpCode().isIf()) {
        if (lastRealNode->getBranchDestination()
                ->getNode()
                ->getBlock()
                ->getNumber() == candidate->getNumber()) {
          lastRealNode->reverseBranch(cloneEnd->getExit()->getNextTreeTop());
          if (trace())
            traceMsg(comp(),
                     "  Reversing branch, node %d now jumps to block_%d\n",
                     pred->getNumber(),
                     lastRealNode->getBranchDestination()
                         ->getNode()
                         ->getBlock()
                         ->getNumber());
          // this check handles splitting through a loop header in a region
          // where we have not detected a valid loop header
          if (bMap->getLast()->_from->getNumber() == pred->getNumber()) {
            lastRealNode = bMap->getLast()
                               ->_to->getExit()
                               ->getPrevRealTreeTop()
                               ->getNode();
            lastRealNode->reverseBranch(cloneEnd->getExit()->getNextTreeTop());
            if (trace())
              traceMsg(comp(),
                       "    Also reversing cloned branch, node %d now jumps to "
                       "block_%d\n",
        ...
        TR::TransformUtil::removeTree(comp(),
                                      pred->getExit()->getPrevRealTreeTop());
      }
    }
  } else {
  ...
  for (BlockMapper *itr = bMap->getFirst(); itr; itr = itr->getNext()) {
    if (itr->_to != cloneEnd) continue;

    TR::Node *lastRealNode =
        itr->_to->getExit()->getPrevRealTreeTop()->getNode();
    TR::TreeTop *nextTree = itr->_from->getExit()->getNextTreeTop();
    if (lastRealNode->getOpCode().isBranch() &&
        lastRealNode->getOpCode().isIf()) {
      // if we are at the end of the clone we have to get back to the origional
      // execution stream so fall through to a goto
      //
      ...
      TR::Block *newBlock = TR::Block::createEmptyBlock(
          lastRealNode, comp(), newFreq, nextTree->getNode()->getBlock());
      ...
      newBlock->append(TR::TreeTop::create(
          comp(), TR::Node::create(lastRealNode, TR::Goto, 0, nextTree)));
    ...
    } else if (!lastRealNode->getOpCode().isBranch() &&
               !lastRealNode->getOpCode().isReturn() &&
               !lastRealNode->getOpCode().isJumpWithMultipleTargets() &&
               lastRealNode->getOpCodeValue() != TR::athrow &&
               !(lastRealNode->getNumChildren() >= 1 &&
                 lastRealNode->getFirstChild()->getOpCodeValue() ==
                     TR::athrow)) {
      itr->_to->append(TR::TreeTop::create(
          comp(), TR::Node::create(lastRealNode, TR::Goto, 0, nextTree)));
      if (trace())
...
const char *TR_BlockSplitter::optDetailString() const throw() {
...
TR_InvariantArgumentPreexistence::TR_InvariantArgumentPreexistence(
    TR::OptimizationManager *manager)
    : TR::Optimization(manager) {
  _success = false;
}
...
int32_t TR_InvariantArgumentPreexistence::perform() {
  ...
  if (comp()->mustNotBeRecompiled()) {
    if (trace())
      traceMsg(comp(),
               "PREX: Aborting preexistence because %s mustNotBeRecompiled\n",
               feMethod->signature(trMemory()));
    return 0;
  }

  static const char *disablePREX = feGetEnv("TR_disablePREX");
  if (disablePREX || comp()->getOptions()->realTimeGC()) return 0;

  {
    ...
    _isOutermostMethod =
        ((comp()->getInlineDepth() == 0) && (!comp()->isPeekingMethod()));
    ...
    _parmInfo = (ParmInfo *)trMemory()->allocateStackMemory(numParms *
                                                            sizeof(ParmInfo));
    for (int i = 0; i < numParms; i++) _parmInfo[i].clear();

    // Now iterate over the trees, check if there is a store to any parm symbol
    // that is in the candidate list
    //
    TR::TreeTop *tt;
    int32_t numInvariantArgs = numParms;
    for (TR::TreeTopIterator iter(methodSymbol->getFirstTreeTop(), comp());
         iter != NULL && numInvariantArgs > 0; ++iter) {
    ...
    if (_isOutermostMethod) {
      if (trace())
        traceMsg(comp(),
                 "PREX:    Populating parmInfo of outermost method %s\n",
                 feMethod->signature(trMemory()));
      ...
      if (p && p->getOffset() == 0 && !feMethod->isStatic()) {
        ...
        TR_OpaqueClassBlock *clazz =
            fe()->getClassFromSignature(sig, len, feMethod);
        ...
        if (knot) {
          TR::KnownObjectTable::Index knotIndex =
              comp()->fej9()->getCompiledMethodReceiverKnownObjectIndex(comp());
    ...
    // comp()->getCurrentPeekingArgInfo() is supplied only in
    // TR_J9EstimateCodeSize::realEstimateCodeSize, so
    // TR_InvariantArgumentPreexistence can validate preexistence args and
    // propagate info onto parmSymbols
    {
      TR_PeekingArgInfo *peekInfo = comp()->getCurrentPeekingArgInfo();
      if (trace())
        traceMsg(comp(),
                 "PREX:    Populating parmInfo of peeked method %s %p\n",
      ...
      if (peekInfo) {
        ...
        int32_t index = 0;
        for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL;
             p = parms.getNext(), index++) {
          const char *sig = NULL;
          if (argInfo && argInfo[index]) {
            sig = argInfo[index];
            int32_t len = lenInfo[index];
            TR_ASSERT(((*sig == 'L') || (*sig == '[')),
                      "non address argument cannot be of fixed ref type");
      ...
      if (argInfo) {
        if (trace())
          traceMsg(comp(),
                   "PREX:    Populating parmInfo of inlined method %s from "
                   "argInfo %p\n",
        ...
        for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL;
          ...
          if (classIsFixed || classIsPreexistent) {
            ...
            const char *sig = p->getTypeSignature(len);
            TR_ASSERT(((*sig == 'L') || (*sig == '[')),
                      "non address argument cannot be fixed/preexistent");

            parmInfo.setSymbol(p);
            TR_OpaqueClassBlock *clazz = arg->getClass();
            TR_OpaqueClassBlock *clazzFromMethod =
                fe()->getClassFromSignature(sig, len, feMethod);
            TR_ASSERT(!clazz || !clazzFromMethod || clazz == clazzFromMethod ||
                          fe()->isInstanceOf(clazz, clazzFromMethod, true, true,
                                             true) == TR_yes,
                      "Type from argInfo should be more specific clazz %p "
                      "clazzFromMethod %p",
            ...
            if (classIsFixed) {
              ...
              if (trace()) {
                ...
                traceMsg(
                    comp(),
                    "PREX:        Parm %d class %p is currently final %s\n",
              ...
              if (clazz) {
                ...
                if (!fe()->classHasBeenExtended(clazz)) {
                  ...
                  if (trace())
                    traceMsg(
                        comp(),
                        "PREX:            Parm %d class is currently final\n",
                        index);
                }
                parmInfo.setClass(clazz);
              }
        ...
        if (trace())
          traceMsg(comp(),
                   "PREX:    No argInfo -- can't populate parmInfo for inlined "
                   "method %s\n",
                   feMethod->signature(trMemory()));
      }
    }
    ...
    if (numInvariantArgs == 0) {
      if (trace()) traceMsg(comp(), "PREX: No invariant arguments\n");
      return 1;
    }

    // Now that we know what args are invariant - we can go ahead and mark their
    // symbols appropriately as being preexistent or fixed
    //
    if (trace())
    ...
    for (TR::ParameterSymbol *p = parms.getFirst(); p != NULL;
         p = parms.getNext(), index++) {
      // Reset several fields on symbol, which may have been marked on a
      // previous inlining of the same method with different arguments list.
      p->setFixedType(NULL);
      ...
      if (info.getSymbol()) {
        ...
        if (_isOutermostMethod) {
          if (info.isInvariant()) {
            ...
            if (trace())
              traceMsg(comp(),
                       "PREX:      Parm %d symbol [%p] is preexistent\n", index,
                       symbol);
            if (info.hasKnownObjectIndex()) {
              ...
              if (trace())
                traceMsg(
                    comp(),
                    "PREX:      Parm %d symbol [%p] is known object obj%d\n",
        ...
        // make sure this path isn't executed iff getCurrentPeekingArgInfo is
        // provided
        else if (info.isInvariant() && !(comp()->isPeekingMethod() &&
                                         comp()->getCurrentPeekingArgInfo())) {
          // For inner methods - an arg is fixed/preexistent if
          // its invariant here and is fixed/prexistent in the outer
          // method
          //
          TR_PrexArgument *arg =
              comp()->getCurrentInlinedCallArgInfo()->get(index);
          if (arg) {
            if (arg->classIsFixed()) {
              symbol->setFixedType(arg->getClass());
              if (trace())
                traceMsg(comp(),
                         "PREX:      Parm %d symbol [%p] has fixed type %p\n",
...
static bool specializeInvokeExactSymbol(
    TR::Node *callNode, TR::KnownObjectTable::Index receiverIndex,
  ...
  TR::ResolvedMethodSymbol *owningMethod =
      callNode->getSymbolReference()->getOwningMethodSymbol(comp);
  TR_ResolvedMethod *resolvedMethod =
      comp->fej9()->createMethodHandleArchetypeSpecimen(
  ...
  if (resolvedMethod) {
    ...
    if (performTransformation(
            comp,
            "%sSubstituting more specific method symbol on %p: %s <- %s\n",
...
TR_InvariantArgumentPreexistence::ParmInfo *
TR_InvariantArgumentPreexistence::getSuitableParmInfo(TR::Node *node) {
  if (!node->getOpCode().isLoadVarDirect()) return NULL;

  TR::Symbol *symbol = node->getSymbolReference()->getSymbol();
  if (!symbol->isParm()) return NULL;

  int32_t index = symbol->getParmSymbol()->getOrdinal();
  ParmInfo *info = _parmInfo + index;
  if (!info->getSymbol()) return NULL;

  // The argument must be preexistent in order to allow us to devirtualize the
  // dispatches. Or if we are peeking - then it must be a refined type
  //
  if (comp()->isPeekingMethod() && !info->classIsRefined()) return NULL;
  if (!comp()->isPeekingMethod() &&
      !symbol->getParmSymbol()->getIsPreexistent() &&
      symbol->getParmSymbol()->getFixedType() == 0)
    return NULL;

  return info;
}

void TR_InvariantArgumentPreexistence::processNode(TR::Node *node,
                                                   TR::TreeTop *treeTop,
                                                   vcount_t visitCount) {
...
void TR_InvariantArgumentPreexistence::processIndirectCall(
    TR::Node *node, TR::TreeTop *treeTop, vcount_t visitCount) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (!node->getSymbol()->castToMethodSymbol()->firstArgumentIsReceiver()) {
    if (trace()) traceMsg(comp(), "PREX:        - First arg is not receiver\n");
    return;
  }
  ...
  if (!resolvedMethod) {
    if (methodSymbol->isInterface()) {
      ...
      if (trace()) traceMsg(comp(), "PREX:        - Unresolved\n");
      return;
    }
  }
  ...
  if (receiver->getOpCode().isLoadDirect()) {
    ...
    if (methodSymbol->isVirtual() || methodSymbol->isInterface()) {
      if (node->getSymbolReference() ==
          getSymRefTab()->findObjectNewInstanceImplSymbol()) {
        // Let's not get fancy with these guys
        // They are the java/lang/Object.newInstancePrototype special methods
        if (trace()) traceMsg(comp(), "PREX:        - newInstancePrototype\n");
        return;
      }

      if (trace())
        traceMsg(comp(),
                 "PREX:        Receiver is %p incoming Parm %d parmInfo %p\n",
                 receiver, receiverParmOrdinal, existingInfo);

      if ((receiverInfo.classIsRefined() && receiverInfo.classIsFixed()) ||
          receiverInfo.classIsCurrentlyFinal())
  ...
  if (receiver->getSymbolReference()->hasKnownObjectIndex()) {
    ...
    receiverInfo.setKnownObjectIndex(
        receiver->getSymbolReference()->getKnownObjectIndex());
  ...
  if (methodSymbol->isVirtual() && fixedOrFinalInfoExists) {
    ...
    if (clazz) {
      ...
      if (thisClazz) {
        ...
        if (isInstance == TR_yes) devirtualize = true;
      }
    }
  }
  ...
  if (methodSymbol->isComputed()) {
    ...
    if (methodSymbol->getRecognizedMethod() ==
      ...
      specializeInvokeExactSymbol(node, receiverInfo.getKnownObjectIndex(),
                                  comp(), this);

    // The method is a specialized thunk archetype, no further improvement is
    // needed. Keeping running subsequent code may result in a crash because
    // `offset` on the symref is not valid.
    if (node->getSymbol()
            ->castToMethodSymbol()
            ->getMethod()
            ->isArchetypeSpecimen())
    ...
    TR_ResolvedMethod *refinedMethod =
        symRef->getOwningMethod(comp())->getResolvedVirtualMethod(
            comp(), receiverInfo.getClass(), offset);
    ...
    if (refinedMethod) {
        ...
        if (needsAssumptions) {
          classInfo =
              comp()
                  ...
                  ->findClassInfoAfterLocking(receiverInfo.getClass(), comp());

          // if the number of recompile assumptions on this particular
          // class has exceeded a threshold, don't do prex anymore for this
          // class
          //
          if ((comp()->getMethodHotness() == warm) && classInfo &&
              classInfo->getNumPrexAssumptions() >
      ...
      if (!needsAssumptions || addAssumptions) {
        if (!refinedMethod->isSameMethod(resolvedMethod)) {
          if (performTransformation(
                  comp(),
                  "%sspecialize and devirtualize invoke [%p] on currently "
                  "fixed or final parameter %d [%p]\n",
                  optDetailString(), node, receiverParmOrdinal,
                  receiverSymbol)) {
            TR::SymbolReference *newSymRef =
                getSymRefTab()->findOrCreateMethodSymbol(
                    symRef->getOwningMethodIndex(), -1, refinedMethod,
            ...
            isModified = true;
                       ...
                       comp(),
                       "%sdevirtualize invoke [%p] on currently fixed or final "
                       "parameter  %d [%p]\n",
                       optDetailString(), node, receiverParmOrdinal,
                       receiverSymbol)) {
          node->devirtualizeCall(treeTop);
      ...
      if (isModified &&
          !(receiverInfo.classIsRefined() && receiverInfo.classIsFixed()) &&
          receiverSymbol &&
          receiverSymbol->getParmSymbol()->getFixedType() == 0) {
        ...
        TR_ASSERT(receiverInfo.getClass(),
                  "Currently final classes must have a valid class pointer");
        bool inc = comp()->getCHTable()->recompileOnClassExtend(
            comp(), receiverInfo.getClass());
        if (classInfo && inc) classInfo->incNumPrexAssumptions();
        _success = true;
      }
    } else if (!TR::Compiler->cls.isInterfaceClass(comp(),
    ...
    //
    TR_ASSERT((fixedOrFinalInfoExists ||
               (receiverSymbol->getParmSymbol()->getFixedType() == 0)),
              "Symbol %p is fixed ref type - but is not marked to be currently "
              "final (fixedOrFinalInfoExists=%d, fixedType=%p)",
              receiverSymbol, fixedOrFinalInfoExists,
              receiverSymbol->getParmSymbol()->getFixedType());

    if (!isInterface && !resolvedMethod->virtualMethodIsOverridden() &&
      ...
      if (comp()->ilGenRequest().details().supportsInvalidation()) {
        ...
        if ((comp()->getMethodHotness() == warm) &&
            ...
            callInfo->getNumPrexAssumptions() >
                comp()->getOptions()->getMaxNumPrexAssumptions())
          addAssumptions = false;
      }
      ...
      if (addAssumptions &&
          performTransformation(
              comp(),
              "%sdevirtualizing invoke [%p] on preexistent argument %d [%p]\n",
              optDetailString(), node, receiverParmOrdinal, receiverSymbol)) {
        if (trace())
          traceMsg(
              comp(),
              "secs devirtualizing invoke on preexistent argument %d in %s\n",
              receiverParmOrdinal, comp()->signature());

        node->devirtualizeCall(treeTop);
        bool inc = comp()->getCHTable()->recompileOnMethodOverride(
            comp(), resolvedMethod);
      ...
      TR_PersistentCHTable *chTable =
          comp()->getPersistentInfo()->getPersistentCHTable();
      ...
      if (methSymbol->isInterface() || methodSymbol) {
        ...
        bool newMethod = true;
        // There is the risk of invaliding a method repeatedly due to class
        // hierarchy extensions In the following avoid devirtualization based on
        // single implementor if the method has been invalidated once
        //
        TR::Recompilation *recompInfo = comp()->getRecompilationInfo();
        if (recompInfo &&
            recompInfo->getMethodInfo()->getNumberOfInvalidations() >= 1 &&
            !chTable->findSingleConcreteSubClass(receiverInfo.getClass(),
                                                 comp())) {
          ...
          if (comp()
                  ...
                  ->getAssumptionCount(RuntimeAssumptionOnClassExtend) < 100000)
            method = chTable->findSingleInterfaceImplementer(
                receiverInfo.getClass(),
                node->getSymbolReference()->getCPIndex(),
                node->getSymbolReference()->getOwningMethod(comp()), comp());
          // if (method)
          //   fprintf(stderr, "%s assumptios=%d\n", comp()->signature(),
          //   comp()->getPersistentInfo()->getRuntimeAssumptionTable()->getAssumptionCount(RuntimeAssumptionOnClassExtend));
        } else {
          TR_OpaqueClassBlock *clazz = resolvedMethod->containingClass();
          if (clazz) {
            TR_OpaqueClassBlock *thisClazz = receiverInfo.getClass();
            if (thisClazz) {
              TR_YesNoMaybe isInstance =
                  fe()->isInstanceOf(thisClazz, clazz, true);
              if (isInstance == TR_yes) devirtualize = true;
            }
          }
            ...
            if (resolvedMethod->isAbstract())
              method = chTable->findSingleAbstractImplementer(
                  ...
                  node->getSymbolReference()->getOwningMethod(comp()), comp());
                         ...
                         resolvedMethod, receiverInfo.getClass(),
        ...
        if (method && !method->virtualMethodIsOverridden()) {
          ...
          if (comp()->ilGenRequest().details().supportsInvalidation()) {
            ...
            if (classInfo && addAssumptions) {
              ...
              TR_ClassQueries::collectAllSubClasses(classInfo, &subClasses,
                                                    comp());
              ListIterator<TR_PersistentClassInfo> subClassesIt(&subClasses);
              for (TR_PersistentClassInfo *subClassInfo =
                       subClassesIt.getFirst();
                   subClassInfo; subClassInfo = subClassesIt.getNext()) {
          ...
          if (addAssumptions &&
              performTransformation(
                  comp(),
                  "%sspecialize and devirtualize invoke [%p] based on only a "
                  "single implementation for call on parameter %d [%p]\n",
                  optDetailString(), node, receiverParmOrdinal,
                  receiverSymbol)) {
            if (newMethod || !method->isSameMethod(resolvedMethod)) {
              TR::SymbolReference *newSymRef =
                  getSymRefTab()->findOrCreateMethodSymbol(
                      symRef->getOwningMethodIndex(), -1, method,
                      TR::MethodSymbol::Virtual);
              newSymRef->copyAliasSets(symRef, getSymRefTab());
            ...
            else if (treeTop->getNode()->getOpCodeValue() ==
                     TR::ResolveAndNULLCHK)
              TR::Node::recreate(treeTop->getNode(), TR::NULLCHK);

            bool doInc = comp()->getCHTable()->recompileOnNewClassExtend(
                comp(), receiverInfo.getClass());
            ...
            if (classInfo) {
              ...
              for (TR_PersistentClassInfo *subClassInfo =
                       subClassesIt.getFirst();
                   subClassInfo; subClassInfo = subClassesIt.getNext()) {
                TR_OpaqueClassBlock *subClass =
                    (TR_OpaqueClassBlock *)subClassInfo->getClassId();
                subClassInfo->setShouldNotBeNewlyExtended(
                    comp()->getCompThreadID());
                if (comp()->getCHTable()->recompileOnNewClassExtend(comp(),
                                                                    subClass))
  ...
  if (comp()->isPeekingMethod() && receiverInfo.getClass() && !isInterface) {
    ...
    TR_OpaqueClassBlock *originalClazz =
        originalResolvedMethod->containingClass();
    bool canRefine = true;
    if (originalClazz != receiverInfo.getClass()) {
      TR_YesNoMaybe isInstance =
          fe()->isInstanceOf(originalClazz, receiverInfo.getClass(), true);
      if (isInstance == TR_yes) canRefine = false;

      isInstance =
          fe()->isInstanceOf(receiverInfo.getClass(), originalClazz, true);
      if (isInstance == TR_no) canRefine = false;
    }
...
void TR_InvariantArgumentPreexistence::processIndirectLoad(
    TR::Node *node, TR::TreeTop *treeTop, vcount_t visitCount) {
  TR::Node *ttNode = treeTop->getNode();
  TR::Node *addressChild = node->getFirstChild();
  if (!addressChild->getOpCode().isLoadVar()) return;
  ...
  if (addressChild->getSymbolReference()->hasKnownObjectIndex()) {
    somethingMayHaveChanged = TR::TransformUtil::transformIndirectLoadChain(
        comp(), node, addressChild,
        addressChild->getSymbolReference()->getKnownObjectIndex(),
        &removedNode);
  } else if (addressChild->getSymbol()->isFixedObjectRef()) {
    somethingMayHaveChanged = TR::TransformUtil::transformIndirectLoadChainAt(
        comp(), node, addressChild,
        (uintptrj_t *)addressChild->getSymbol()
            ->castToStaticSymbol()
            ->getStaticAddress(),
        &removedNode);
    ...
    if (trace())
      traceMsg(
          comp(),
          "PREX:          Indirect load through incoming Parm %d parmInfo %p\n",
          index, info);
    if (info && info->hasKnownObjectIndex())
      somethingMayHaveChanged = TR::TransformUtil::transformIndirectLoadChain(
          comp(), node, addressChild, info->getKnownObjectIndex(),
          &removedNode);
  }
  ...
  if (removedNode) {
    if (removedNode->getOpCode().isTreeTop())
      ...
      TR::TreeTop::create(comp(), treeTop->getPrevTreeTop(),
                          TR::Node::create(TR::treetop, 1, removedNode));
    removedNode->decReferenceCount();
  }

  if (somethingMayHaveChanged && nodeToNullCheck) {
    // This node was under a NULLCHK and may no longer be a suitable child for
    // the NULLCHK. Put a passThrough there instead to be safe and correct. The
    // old shape was:
    //
    //   ttNode    // NULLCHK (or some other isNullCheck opcode) on
    //   nodeToNullCheck
    //     node
    //       nodeToNullCheck
    //
    // We'll anchor the node right after the NULLCHK, then replace it with a
    // passThrough under the NULLCHK.
    //
    //   ttNode    // NULLCHK (or some other isNullCheck opcode) on
    //   nodeToNullCheck
    //     passThrough
    ...
    //     node
    //
    TR::TreeTop::create(comp(), treeTop,
                        TR::Node::create(TR::treetop, 1, node));
    ttNode->getAndDecChild(0);
    ttNode->setAndIncChild(
        0, TR::Node::create(TR::PassThrough, 1, nodeToNullCheck));
    if (trace())
...
int32_t TR_CheckcastAndProfiledGuardCoalescer::perform() {
  if (comp()->getOption(TR_DisableCheckcastAndProfiledGuardCoalescer)) return 1;
  ...
  for (TR::TreeTop *tt = comp()->getStartTree(); tt;
    ...
    switch (node->getOpCodeValue()) {
      ...
      case TR::checkcastAndNULLCHK:
        ...
        break;
      case TR::NULLCHK:
        if (!checkcastTree || node->getNullCheckReference() !=
                                  checkcastTree->getNode()->getFirstChild()) {
          transformationIsValid = false;
        }
        break;
      case TR::ifacmpne:
        if (node->isProfiledGuard() && node->getFirstChild() && checkcastTree) {
          TR_ASSERT(checkcastTree->getNode()
                        ->getFirstChild()
                        ->getOpCode()
                        .hasSymbolReference(),
                    "the first checkcast child [%p] should have a symref!",
                    checkcastTree->getNode()->getFirstChild());
          TR_ASSERT(checkcastTree->getNode()
                        ->getFirstChild()
                        ->getOpCode()
                        .hasSymbolReference(),
                    "the second checkcast child [%p] should have a symref!",
                    checkcastTree->getNode()->getSecondChild());

          TR_VirtualGuard *virtualGuard = comp()->findVirtualGuardInfo(node);
          TR::Node *vftLoad = node->getFirstChild();
          if (virtualGuard && transformationIsValid &&
              virtualGuard->getTestType() == TR_VftTest &&
              vftLoad->getOpCodeValue() == TR::aloadi &&
              vftLoad->getSymbolReference() ==
                  comp()->getSymRefTab()->findVftSymbolRef() &&
              !checkcastTree->getNode()
                   ->getSecondChild()
                   ->getSymbolReference()
                   ->isUnresolved() &&
              vftLoad->getFirstChild()->getOpCode().hasSymbolReference() &&
              !vftLoad->getFirstChild()->getSymbolReference()->isUnresolved() &&
              vftLoad->getFirstChild()->getSymbolReference() ==
                  checkcastTree->getNode()
                      ->getFirstChild()
                      ->getSymbolReference() &&
              performTransformation(comp(),
                                    "%s Merging checkcast [%p] and profiled "
                                    "guard [%p] (basic case)\n",
                                    optDetailString(), checkcastTree->getNode(),
                                    node)) {
            // Transformation

            // Insert a nullchk if we are dealing with checkcastAndNULLCHK
            //
            if (checkcastTree->getNode()->getOpCodeValue() ==
                TR::checkcastAndNULLCHK) {
              TR::Node *passThroughNode =
                  TR::Node::create(TR::PassThrough, 1,
                                   checkcastTree->getNode()->getFirstChild());
              TR::Node *nullCheckNode = TR::Node::createWithSymRef(
                  TR::NULLCHK, 1, 1, passThroughNode,
                  comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(NULL));
              TR::TreeTop *nullCheckTreeTop =
                  TR::TreeTop::create(comp(), nullCheckNode);
            ...
            checkcastTree = NULL;
            optimizer()->setRequestOptimization(OMR::compactNullChecks, true,
                                                tt->getEnclosingBlock());
          }
        }
        break;
      case TR::ifacmpeq:
        // It's safe to move a checkcast across a null test on the same object
        // if we fall through when non-null
        if (checkcastTree) {
          if (node->getFirstChild() !=
            ...
            // Jump to the next BBStart, next iteration of the loop will start
            // at the tree following the BBStart
            tt = tt->getNextTreeTop()->getNextTreeTop();
            traceMsg(
                comp(),
                "Found suitable ifacmpeq [%p] after checkcast [%p], safe to "
                "continue to next block in search of a profiled guard\n",
...
TR::Node *TR_CheckcastAndProfiledGuardCoalescer::storeObjectInATemporary(
    TR::TreeTop *checkcastTree) {
  TR::Node *obj = checkcastTree->getNode()->getFirstChild();
  TR::SymbolReference *tempCheckcastSymRef =
      comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
                                              obj->getDataType(), false);
...
void TR_CheckcastAndProfiledGuardCoalescer::doBasicCase(
    TR::TreeTop *checkcastTree, TR::TreeTop *profiledGuardTree) {
  TR::Block *slowPathBlock = profiledGuardTree->getNode()
                                 ->getBranchDestination()
                                 ->getNode()
                                 ->getBlock();

  // Spill the first node of the checkcast (the object operand) into a temp to
  // be used in a slow path block.
  //
  ...
  //
  TR::Node *classConst = TR::Node::createWithSymRef(
      checkcastTree->getNode()->getSecondChild(), TR::loadaddr, 0,
      checkcastTree->getNode()->getSecondChild()->getSymbolReference());

  TR::TreeTop *checkCastInsertionPoint = slowPathBlock->getEntry();

  TR::Node *newCheckcastNode = TR::Node::createWithSymRef(
      TR::checkcast, 2, TR::Node::createLoad(storeNode, tempCheckcastSymRef),
      classConst, 0,
      comp()->getSymRefTab()->findOrCreateCheckCastSymbolRef(
          comp()->getMethodSymbol()));
...
const char *TR_CheckcastAndProfiledGuardCoalescer::optDetailString() const
    throw() {
...
int32_t TR_ColdBlockMarker::perform() {
  ...
  /* The ILValidator is only created if the useILValidator Compiler Option is
   * set */
...
bool TR_ColdBlockMarker::hasAnyExistingColdBlocks() {
  TR::Block *block =
      optimizer()->getMethodSymbol()->getFirstTreeTop()->getNode()->getBlock();
  for (; block; block = block->getNextBlock()) {
    if (block->isCold()) return true;
  }
  return false;
}
...
bool TR_ColdBlockMarker::identifyColdBlocks() {
  ...
  for (TR::AllBlockIterator iter(optimizer()->getMethodSymbol()->getFlowGraph(),
                                 comp());
       iter.currentBlock(); ++iter) {
    ...
    if (block->isCold()) {
      ...
      if (comp()->ilGenTrace()) controlled = true;
...
int32_t TR_ColdBlockMarker::isBlockCold(TR::Block *block) {
  ...
  if (_exceptionsAreRare && !block->getExceptionPredecessors().empty() &&
  ...
  // for (TR::PreorderNodeOccurrenceIterator iter(block->getFirstRealTreeTop(),
  // this); iter != block->getExit(); ++iter) for
  // (TR::PostorderNodeOccurrenceIterator iter(block->getFirstRealTreeTop(),
  // this); iter != block->getExit(); ++iter)
  //
  for (TR::PreorderNodeIterator iter(block->getFirstRealTreeTop(), comp());
       iter != block->getExit(); ++iter) {
    TR::Node *node = iter.currentNode();

    // If the block throws an exception, and exceptions are rare in this method,
    // or if the block has some unresolved symbols in it, then it must be a cold
    // block
    //
    // NOTE: return is not considered a cold block: in a loopless methods, a
    // return postdominates everything.
    //
    if (node->getOpCodeValue() == TR::athrow && _exceptionsAreRare &&
        block->getFrequency() <= 0)
      return CATCH_COLD_BLOCK_COUNT;
    ...
    if (_notYetRunMeansCold && node->getOpCode().isCall() &&
      ...
      TR_ASSERT(calleeSymbol,
                "assertion failure");  // cannot reach here otherwise

      if (calleeSymbol->getResolvedMethod()->isCold(
              comp(), node->getOpCode().isCallIndirect(), calleeSymbol)) {
        if (trace()) {
...
bool TR_ColdBlockMarker::hasNotYetRun(TR::Node *node) {
  ...
  if (hasBeenRun != TR_maybe) {
    ...
    if (isUnresolved) {
      ...
      if (!methodSymbol->isInterface()) return true;
    }
    ...
    if (node->getSymbolReference()->getSymbol()->isClassObject() &&
      ...
      int32_t len;
      char *name = TR::Compiler->cls.classNameChars(
          comp(), node->getSymbolReference(), len);
      if (name) {
        TR::HeuristicRegion heuristicRegion(comp());
        char *sig = classNameToSignature(name, len, comp());
        TR_OpaqueClassBlock *classObject = fe()->getClassFromSignature(
            sig, len, node->getSymbolReference()->getOwningMethod(comp()));
        if (classObject &&
            !TR::Compiler->cls.isInterfaceClass(comp(), classObject))
          return true;
      } else
        return true;
    } else {
      if (comp()->compileRelocatableCode() &&
          ...
          !comp()->getOption(TR_DisablePeekAOTResolutions)) {
        bool isUnresolved =
            node->getSymbolReference()->isUnresolvedFieldInCP(comp());
        // currentely node->hasUnresolvedSymbolReference() returns true more
        // often for AOT than non-AOT beacause of literals() vs romLiterals()
        // used in relocatableresolvedj9method this optimiztaion can be removed
        // when work to clean up ram vs rom literals is done
        if (isUnresolved && node->getSymbol()->isConstString()) {
          TR::ResolvedMethodSymbol *rms =
              comp()->getOwningMethodSymbol(node->getOwningMethod());
          isUnresolved = rms->getResolvedMethod()->isUnresolvedString(
              node->getSymbolReference()->getCPIndex(), true);
        }
        return isUnresolved;
      } else {
        if (comp()->compileRelocatableCode() &&
...
const char *TR_ColdBlockMarker::optDetailString() const throw() {
...
void TR_ColdBlockOutlining::reorderColdBlocks() {
  ...
  for (currentTree = comp()->getStartTree(); currentTree != NULL;
  ...
  for (currentTree = comp()->getStartTree(); currentTree != NULL;
    ...
    if (!coldBlock(currentBlock, comp()) /*currentBlock->isCold()*/
        /* && !currentBlock->isRare(comp()) */) {
      ...
      if (!nextExtendedBlock) break;
      if (!nextExtendedBlock->getEntry()) break;
    ...
    // Since gen asm flow block may contains asm branch which we can't change it
    // to long form So we don't want to move the successor of a gen asm flow
    // block if gen asm block itself is not cold Also we don't want to move gen
    // asm flow block if one of it's successor is not cold.

    // First, check if a predeccessor of this block is genAsmFlow, this block is
    // not it's predeccessor' fall through, continue if yes
    TR::CFGEdgeList &predecessors =
        ((TR::CFGNode *)currentBlock)->getPredecessors();

    for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();
         ++predEdge) {
      TR::Block *predBlock = (*predEdge)->getFrom()->asBlock();

      // we can alwasy insert a long jump if currentBlock is fall through of
      // predBlock
      if (predBlock->getExit() && predBlock->getExit()->getNextTreeTop() &&
          predBlock->getExit()->getNextTreeTop()->getNode()->getBlock() ==
              currentBlock &&
    ...
    // nextBlock should always be non-NULL because the if (exitTree == lastTree)
    // checks above guarantee a break out of the loop before this point for the
    // last block (i.e. what was the original last block before any reordering
    // of cold blocks to the end)
    TR_ASSERT(nextBlock, "tree walk error nextBlock should not be NULL");

    if (coldBlock(
            nextBlock,
            comp()) /*nextBlock->isCold()*/ /* || nextBlock->isRare(comp()) */)
      continue;

    if (!performTransformation(comp(),
                               "%soutlined cold block sequence (%d-%d)\n",
                               optDetailString(), startBlock->getNumber(),
    ...
    // We have discovered a linear sequence of blocks starting with 'startBlock'
    // and ending with 'block': snip and place at the end of the method
    //
    TR::Block *prevOfStart = startBlock->getPrevBlock();

    // prevOfStart should be non-null: otherwise the corollary is that method
    // entry block is cold (and hence every single block in the method).
    // FIXME: the following should be an assume, but i am hesitant.. If anybody
    // runs the cold propagtion code ever at fixed opt levels (count=0) then we
    // could results with the entry block as cold
    //
...
const char *TR_ColdBlockOutlining::optDetailString() const throw() {
  return "O^O COLD BLOCK OUTLINING: ";
}

TR::Block *TR_BlockManipulator::breakFallThrough(TR::Block *faller,
                                                 TR::Block *fallee,
                                                 bool isOutlineSuperColdBlock) {
  TR::Node *lastNode = faller->getLastRealTreeTop()->getNode();
  if (lastNode->getOpCode().isCheck() ||
  ...
  if (lastNode->getOpCode().isReturn() || lastNode->getOpCode().isGoto() ||
      (lastNode->getOpCode().isJumpWithMultipleTargets() &&
       lastNode->getOpCode().hasBranchChildren()) ||
  ...
  if (lastNode->getOpCode().isBranch() ||
    ...
    TR::Block *gotoBlock = TR::Block::createEmptyBlock(
        lastNode, comp(),
        (fallee->getFrequency() < faller->getFrequency())
            ? fallee->getFrequency()
            : faller->getFrequency(),
        fallee);
    ...
    if (faller->getStructureOf())
      comp()->getFlowGraph()->addNode(gotoBlock,
                                      faller->getCommonParentStructureIfExists(
                                          fallee, comp()->getFlowGraph()));
    else
      comp()->getFlowGraph()->addNode(gotoBlock);
    comp()->getFlowGraph()->addEdge(
        TR::CFGEdge::createEdge(faller, gotoBlock, trMemory()));
    comp()->getFlowGraph()->addEdge(
        TR::CFGEdge::createEdge(gotoBlock, fallee, trMemory()));
    // remove the edge only if the branch target is not pointing to the next
    // block
    if ((lastNode->getOpCode().isBranch() &&
         lastNode->getBranchDestination() != fallee->getEntry()) ||
        (lastNode->getOpCode().isCall() &&
         lastNode->getOpCode().isJumpWithMultipleTargets()))
    ...
    if (fallee->isCold() || faller->isCold()) {
      ...
      if (faller->isSuperCold()) gotoBlock->setIsSuperCold();
    }
    return gotoBlock;
  }
...
// used by trivialDeadTreeRemoval opt pass and during CodeGenPrep to remove
// nodes treetops that have no intervening def
void TR_TrivialDeadTreeRemoval::preProcessTreetop(
    TR::TreeTop *treeTop, List<TR::TreeTop> &commonedTreeTopList,
    char *optDetails, TR::Compilation *comp) {
  ...
  if (ttNode->getOpCodeValue() == TR::treetop &&
    ...
    if (firstChild->getReferenceCount() == 1) {
      if (!firstChild->getOpCode().hasSymbolReference() &&
          performTransformation(
              comp, "%sUnlink trivial %s (%p) of %s (%p) with refCount==1\n",
              optDetails, treeTop->getNode()->getOpCode().getName(),
              treeTop->getNode(), firstChild->getOpCode().getName(),
              firstChild)) {
        if (trace)
          traceMsg(comp,
                   "\tfound trivially anchored ttNode %p with firstChild %s "
                   "(%p -- refCount == 1)\n",
        ...
        for (int32_t i = 0; i < firstChild->getNumChildren(); i++) {
          ...
          if (!grandChild->getOpCode().isLoadConst() ||
              grandChild->anchorConstChildren()) {
            if (trace)
              traceMsg(comp,
                       "\t\tcreate new treetop for firstChild->getChild(%d) = "
                       "%s (%p)\n",
                       i, grandChild->getOpCode().getName(), grandChild);
            // use insertAfter so the newly anchored trees get visited in the
            // next interation(s)
            treeTop->insertAfter(TR::TreeTop::create(
                comp, TR::Node::create(TR::treetop, 1, grandChild)));
          }
        }
        if (trace)
          traceMsg(comp,
                   "\t\tremove trivially anchored ttNode %p with firstChild %s "
                   "(%p) treetop\n",
                   ttNode, firstChild->getOpCode().getName(), firstChild);
        treeTop->unlink(true);
      }
               ...
               (firstChild->getOpCode().isLoad() &&
                !firstChild->getOpCode().isStore())) {
      if (trace)
        traceMsg(comp,
                 "\tadd ttNode %p with firstChild %s (%p, refCount %d) to "
                 "commonedTreeTopList\n",
                 ttNode, firstChild->getOpCode().getName(), firstChild,
...
void TR_TrivialDeadTreeRemoval::postProcessTreetop(
  ...
  if (treeTop->isPossibleDef()) {
    if (trace)
      traceMsg(comp,
               "\tfound a possible def at node %p so clear "
               "_commonedTreeTopList list\n",
               treeTop->getNode());
    commonedTreeTopList.deleteAll();
  }
}

void TR_TrivialDeadTreeRemoval::processCommonedChild(
    TR::Node *child, TR::TreeTop *treeTop,
    List<TR::TreeTop> &commonedTreeTopList, char *optDetails,
    TR::Compilation *comp) {
  bool trace = comp->getOption(TR_TraceTrivialDeadTreeRemoval);
  if (child->getReferenceCount() > 1) {
    if (!commonedTreeTopList.isEmpty()) {
      ...
      if (trace)
        traceMsg(comp,
                 "commonedTreeTopList is not empty and found a commoned child "
                 "%s (%p, refCount %d)\n",
      ...
      while (listTT) {
        ...
        // if listTT == treeTop then the commoned referenced is under the same
        // treetop as the tracked treetop tt
        //   a
        //   =>a
        //
        if (trace)
          traceMsg(
              comp,
              "\tcomparing listTT %p with firstChild %s (%p) to commoned child "
              "%s (%p, refCount %d) (listTT == _currentTreeTop -- %s)\n",
              listTT->getNode(),
              listTT->getNode()->getFirstChild()->getOpCode().getName(),
              listTT->getNode()->getFirstChild(), child->getOpCode().getName(),
              child, child->getReferenceCount(),
              listTT == treeTop ? "yes" : "no");
        if (listTT->getNode()->getFirstChild() == child) {
          if (listTT != treeTop) {
            if (performTransformation(comp,
                                      "%sFound commoned reference to child %s "
                                      "(%p) so unlink %s (0x%p)\n",
                                      optDetails, child->getOpCode().getName(),
                                      child,
                                      listTT->getNode()->getOpCode().getName(),
                                      listTT->getNode())) {
              listTT->unlink(true);
            }
            ...
            if (treeTop->getNode()->getNumChildren() == 1 &&
              ...
              if (performTransformation(
                      comp,
                      "%sFound commoned reference to single child %s (%p) case "
                      "1 so unlink %s (0x%p)\n",
                      optDetails, child->getOpCode().getName(), child,
                      treeTop->getNode()->getOpCode().getName(),
                      treeTop->getNode()))
                treeTop->unlink(true);
              commonedTreeTopList.removeNext(prevElement);
              return;
      ...
      if (treeTop->getNode()->getNumChildren() == 1 &&
        ...
        if (performTransformation(
                comp,
                "%sFound commoned reference to single child %s (%p) case 2 so "
                "unlink %s (0x%p)\n",
                optDetails, child->getOpCode().getName(), child,
...
void TR_TrivialDeadTreeRemoval::examineNode(TR::Node *node,
                                            vcount_t visitCount) {
  node->setVisitCount(visitCount);
  //   if (trace())
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    //  if (trace())
    //      traceMsg(comp(),"\tlooking at index %d : child %s (%p, refCount %d)
    //      -- childVC %d, VC %d
    ...
    if (child->getVisitCount() != visitCount) {
      ...
      processCommonedChild(child, _currentTreeTop, _commonedTreeTopList,
                           OPT_DETAILS_TDTR, comp());
    }
  }
  // if (trace()) traceMsg(comp(),"\n");
}

void TR_TrivialDeadTreeRemoval::transformBlock(TR::TreeTop *entryTree,
                                               TR::TreeTop *exitTree) {
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  ...
  if (comp()->getOption(TR_TraceTrivialDeadTreeRemoval))
    traceMsg(comp(),
             "TrivialDeadTreeRemoval on block_%d : entryTreeNode %p -> "
             "exitTreeNode %p\n",
             _currentBlock->getNumber(), entryTree->getNode(),
             exitTree->getNode());
  for (TR::TreeTop *currentTree = entryTree->getNextRealTreeTop();
       currentTree != exitTree;
       currentTree = currentTree->getNextRealTreeTop()) {
    _currentTreeTop = currentTree;
    preProcessTreetop(currentTree, _commonedTreeTopList, OPT_DETAILS_TDTR,
                      comp());
    examineNode(currentTree->getNode(), visitCount);
    postProcessTreetop(currentTree, _commonedTreeTopList, OPT_DETAILS_TDTR,
                       comp());
  }
}
...
const char *TR_TrivialDeadTreeRemoval::optDetailString() const throw() {
  return "O^O TRIVIAL DEAD TREE REMOVAL: ";
}

int32_t TR_TrivialBlockExtension::perform() {
  int32_t cost = 0;
  for (TR::Block *block = comp()->getStartTree()->getNode()->getBlock();
       block && block->getEntry(); block = block->getNextBlock())
...
int32_t TR_TrivialBlockExtension::performOnBlock(TR::Block *block) {
  ...
  if (block->isExtensionOfPreviousBlock()) {
    if (trace())
      traceMsg(comp(),
               "BlockExtension: block_%d is already an extension of the "
               "previous block\n",
               block->getNumber());
    ...
    if (pred != block->getPrevBlock()) {
      if (trace())
        traceMsg(
            comp(),
            "BlockExtension: block_%d predecessor is not the previous block\n",
            block->getNumber());
    } else if (!pred->canFallThroughToNextBlock()) {
      if (trace())
        traceMsg(comp(),
                 "BlockExtension: block_%d does not fall through to block_%d\n",
                 pred->getNumber(), block->getNumber());
    } else if (pred->getLastRealTreeTop()
                   ->getNode()
                   ->getOpCode()
                   .isJumpWithMultipleTargets()) {
      if (trace())
        traceMsg(comp(),
                 "BlockExtension: block_%d ends in a switch and so we will not "
                 "mark block_%d as an extension\n",
                 pred->getNumber(), block->getNumber());
    } else {
      if (performTransformation(comp(),
                                "O^O BLOCK EXTENSION: Mark block_%d as an "
                                "extension of block_%d\n",
...
const char *TR_TrivialBlockExtension::optDetailString() const throw() {
