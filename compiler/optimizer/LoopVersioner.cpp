...
int32_t TR_LoopVersioner::performWithoutDominators()
{
  ...
  for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
    ...
    bool nullChecksMayBeEliminated = detectChecksToBeEliminated(naturalLoop, &nullCheckedReferences, &nullCheckTrees,
      &numIndirections, &boundCheckTrees, &spineCheckTrees, &numDimensions, &conditionalTrees, &divCheckTrees,
      &awrtbariTrees, &checkCastTrees, &arrayStoreCheckTrees, &specializedInvariantNodes, invariantNodes,
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases())
      checkCastTreesWillBeEliminated
        = detectInvariantTrees(naturalLoop, &checkCastTrees, false, &containsNonInlineGuard, reverseBranchInLoops);
    else
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases()) {
      ...
      if (comp()->getOption(TR_EnableAggressiveLoopVersioning)) {
        if (trace())
          traceMsg(comp(), "aggressiveLoopVersioning: raising hotnessThreshold for conditionalsWillBeEliminated\n");
      ...
      if (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
        ...
        || checkCastTreesWillBeEliminated || arrayStoreCheckTreesWillBeEliminated || specializedNodesWillBeEliminated
        || invariantNodesWillBeEliminated || _nonInlineGuardConditionalsWillNotBeEliminated || _loopTransferDone) {
        conditionalsWillBeEliminated
          = detectInvariantTrees(naturalLoop, &conditionalTrees, true, &containsNonInlineGuard, reverseBranchInLoops);
      } else {
        conditionalsWillBeEliminated
          = detectInvariantTrees(naturalLoop, &conditionalTrees, false, &containsNonInlineGuard, reverseBranchInLoops);
    ...
    if ((versionToRefineAliases || !refineAliases())
      ...
      versionNaturalLoop(naturalLoop, &nullCheckedReferences, &nullCheckTrees, &boundCheckTrees, &spineCheckTrees,
        ...
        &specializedInvariantNodes, invariantNodes, &invariantTranslationNodesList, &whileLoops, &clonedInnerWhileLoops,
  ...
  for (vgInfo = _virtualGuardInfo.getFirst(); vgInfo; vgInfo = vgInfo->getNext()) {
    ...
    for (nextVirtualGuard = guardsIt.getFirst(); nextVirtualGuard != NULL; nextVirtualGuard = guardsIt.getNext()) {
      ...
      if (hotGuard->getOpCode().isIf() && hotGuard->isTheVirtualGuardForAGuardedInlinedCall()
        ...
        const char* debugCounter
          = TR::DebugCounter::debugCounterName(comp(), "loopVersioner.transfer/(%s)/%s/origin=block_%d",
            comp()->signature(), comp()->getHotnessName(comp()->getMethodHotness()), hotGuardBlock->getNumber());
...
bool TR_LoopVersioner::detectInvariantSpecializedExprs(List<TR::Node>* profiledExprs)
{
  ...
  for (node = profiledExprs->getListHead(); node != NULL;) {
    ...
    if (!isProfiledExprInvariant) {
      if (trace())
        traceMsg(
          comp(), "Non invariant Specialized expr %p (%s)\n", node->getData(), node->getData()->getOpCode().getName());
      ...
      if (trace())
        traceMsg(
          comp(), "Invariant Specialized expr %p (%s)\n", node->getData(), node->getData()->getOpCode().getName());
  ...
  List<TR_NodeParentSymRef>* invariantNodes, List<TR_NodeParentSymRefWeightTuple>* invariantTranslationNodes)
{
  ...
  for (; nextNode;) {
    ...
    // while not technically true, computeCC and overflow compares need to have their children intact and so they can't
    // be marked as invariant a better solution is really needed
...
bool TR_LoopVersioner::detectInvariantTrees(TR_RegionStructure* whileLoop, List<TR::TreeTop>* trees,
  ...
  for (; nextTree;) {
    ...
    if (includeComparisonTree) {
      ...
      if (onlyDetectHighlyBiasedBranches && (thisChild || nextRealNode)
        && node->isTheVirtualGuardForAGuardedInlinedCall() && !node->isHCRGuard() && !node->isBreakpointGuard()) {
        if (!guardInfo)
          guardInfo = comp()->findVirtualGuardInfo(node);
        // TR::Node *thisChild = NULL;
        ...
        if (guardInfo->getKind() == TR_DummyGuard
          ...
          if (!b) {
            ...
            {
              if (!(thisChild->getOpCode().hasSymbolReference()
                    && thisChild->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(thisChild))) {
                isTreeInvariant = false;
              } else {
                if (!guardInfo)
                  guardInfo = comp()->findVirtualGuardInfo(node);
  ...
  for (; nextTree;) {
    ...
    if (!isMaxBoundInvariant) {
      if (boundNode->getOpCode().isArrayLength()) {
        ...
        if (arrayObject->getOpCode().hasSymbolReference() && arrayObject->getSymbolReference()->getSymbol()->isAuto()
          && isDependentOnInvariant(arrayObject))
          isMaxBoundInvariant = true;
      } else {
        if (boundNode->getOpCode().hasSymbolReference() && boundNode->getSymbolReference()->getSymbol()->isAuto()
    ...
    if (isMaxBoundInvariant) {
      indexChild = (node->getOpCodeValue() == TR::BNDCHKwithSpineCHK) ? node->getChild(3) : node->getSecondChild();
        ...
        while (indexSymRef && changedIndexSymRef) {
            ...
            if (_loopTestTree && (_loopTestTree->getNode()->getNumChildren() > 1)
              ...
              if (symbolInCompare) {
                ...
                for (v = _currentNaturalLoop->getFirstInductionVariable(); v; v = v->getNext()) {
                  if ((v->getLocal() == indexSymRef->getSymbol()) && (v->getLocal() == symbolInCompare)
                    && (v->getLocal()->getDataType() == TR::Int32) && (v->isSigned())) {
                    if ((v->getIncr()->getLowInt() == v->getIncr()->getHighInt()) && (v->getIncr()->getLowInt() > 0))
          ...
          if (!isInductionVariable) {
            ...
            indexChild = isDependentOnInductionVariable(indexChild, changedIndexSymRefAtSomePoint,
              isIndexChildMultiplied, mulNode, strideNode, indVarOccursAsSecondChildOfSub);
...
bool TR_LoopVersioner::detectInvariantDivChecks(List<TR::TreeTop>* divideCheckTrees)
{
  ...
  for (; nextTree;) {
    ...
    if ((node->getFirstChild()->getOpCodeValue() == TR::idiv) || (node->getFirstChild()->getOpCodeValue() == TR::irem)
  ...
  if (comp->getMethodHotness() >= hot && callSymRef) {
    if (callSymRef->getSymbol() && callSymRef->getSymbol()->castToMethodSymbol()
      ...
      if (disableLoopCodeRatioCheck || loopCodeRatio < 0.25) {
        if (trace())
          traceMsg(comp,
            "Skipping versioning of profiled guard %p because we found more than 2 JIT'd implementors at warm or above "
            "and the loop code ratio is too low\n",
            guardNode);
        risky = true;
        TR::DebugCounter::incStaticDebugCounter(comp,
          TR::DebugCounter::debugCounterName(comp,
            "profiledVersioning/unsuitableForVersioning/interfaceGuard/(%s)/bci=%d.%d", comp->signature(),
            guardNode->getByteCodeInfo().getCallerIndex(), guardNode->getByteCodeInfo().getByteCodeIndex()));
      }
    } else if (comp->getInlinedResolvedMethod(guardNode->getByteCodeInfo().getCallerIndex())
                 ->isSubjectToPhaseChange(comp)) {
      if (trace()) {
        traceMsg(
          comp, "Found profiled guard %p is for a method subject to phase change - skipping versioning\n", guardNode);
      }
      risky = true;
    }
  }
  ...
  TR_ScratchList<TR::Block>* loopBlocks, TR::Node* node, TR::Compilation* comp)
{
  ...
  if (node->isProfiledGuard()) {
    bool isGetFieldCacheCounts = !strncmp(comp->signature(),
      "org/apache/solr/request/SimpleFacets.getFieldCacheCounts(Lorg/apache/solr/search/SolrIndexSearcher;Lorg/apache/"
      "solr/search/DocSet;Ljava/lang/String;IIIZLjava/lang/String;Ljava/lang/String;)Lorg/apache/solr/common/util/"
      "NamedList;",
      60);
    ...
    if (isGetFieldCacheCounts || disableProfiledGuardVersioning) {
      ...
      if (valueInfo) {
        ...
        if (suitableForVersioning) {
          TR::DebugCounter::incStaticDebugCounter(comp,
            TR::DebugCounter::debugCounterName(comp, "profiledVersioning/suitableForVersioning/probability=%d",
              ((int32_t)(valueInfo->getTopProbability() * 100))));
        } else {
          TR::DebugCounter::incStaticDebugCounter(comp,
            TR::DebugCounter::debugCounterName(comp, "profiledVersioning/unsuitableForVersioning/probability=%d",
              ((int32_t)(valueInfo->getTopProbability() * 100))));
        }
      } else {
        TR::DebugCounter::incStaticDebugCounter(
          comp, TR::DebugCounter::debugCounterName(comp, "profiledVersioning/unsuitableForVersioning/noinfo"));
        suitableForVersioning = false;
      }
    } else {
  ...
  bool& discontinue)
{
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
    ...
    if (nextBlock->isCold()) {
      ...
      if ((!aggressive || comp()->getMethodHotness() <= warm) && blockFrequency >= 0 // frequency must be valid
        ...
        && performTransformation(comp(),
          ...
          nextBlock->getNumber(), blockFrequency, loopFrequency, hotBlock->getNumber(), whileLoop->getNumber())) {
        isUnimportant = true;
          ...
          for (TR::TreeTop* tt = entryTree; isUnimportant && tt != exitTree; tt = tt->getNextTreeTop()) {
            ...
            if (op.isCheck() || op.isCheckCast()
              ...
              if (!performTransformation(comp(), "%s ...disregarding %s n%dn\n", OPT_DETAILS_LOOP_VERSIONER,
                    ttNode->getOpCode().getName(), ttNode->getGlobalIndex()))
                isUnimportant = false;
            }
          }
        }
    ...
    while (!(currentTree == exitTree)) {
      ...
      updateDefinitionsAndCollectProfiledExprs(NULL, currentNode, visitCount, specializedInvariantNodes, invariantNodes,
        invariantTranslationNodesList, NULL, collectProfiledExprs, nextBlock, warmBranchCount);
      ...
      if ((currentNode->getNumChildren() > 0) && currentNode->getFirstChild()->getOpCode().isCall()) {
        if (!currentNode->getFirstChild()->isTheVirtualCallNodeForAGuardedInlinedCall()) {
          ...
          if (!guard) {
            // printf("\t\tnode %p (nextblock = %p) is hte virtual call node for a guarded inlined call without a
      ...
      if (!isUnimportant && !_containsCall && !refineAliases()) {
        if (currentOpCode.isNullCheck()) {
          ...
          if (dupOfThisBlockAlreadyExecutedBeforeLoop)
            _checksInDupHeader.add(currentTree);
          foundPotentialChecks = true;
        } else if (currentOpCode.isBranch() && isBranchSuitableToVersion(&blocksInWhileLoop, currentNode, comp())
        ...
        } else if (currentOpCode.isCheckCast()) // (currentOpCode.getOpCodeValue() == TR::checkcast)
        {
          if (currentNode->getSecondChild()->getOpCodeValue() == TR::loadaddr) {
            void* clazz
              = currentNode->getSecondChild()->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress();
            if (!comp()->fe()->isUnloadAssumptionRequired((TR_OpaqueClassBlock*)clazz, comp()->getCurrentMethod())) {
              ...
              if (trace()) // if we move the checkcast before the inlined body instanceof might fail because the class
        ...
        if (TR::Compiler->om.writeBarrierType() == gc_modron_wrtbar_cardmark_and_oldcheck
          ...
          if ((currentOpCode.getOpCodeValue() != TR::awrtbari) && (possibleAwrtbariNode->getNumChildren() > 0))
  ...
  TR::Block* block, int32_t warmBranchCount)
{
  ...
  if (_inNullCheckReference || !_nullCheckReference) {
    if ((opCode.isCall()) || (opCodeValue == TR::New) || (opCodeValue == TR::newarray) || (opCodeValue == TR::anewarray)
      || (opCodeValue == TR::multianewarray))
      _containsCall = true;
  ...
  if (TR_LocalAnalysis::isSupportedOpCode(node->getOpCode(), comp())) {
    if (node->getType().isInt32()
      ...
      if (profileLongParms && comp()->getMethodHotness() == hot && comp()->getRecompilationInfo()) {
      ...
      if (/* comp()->getRecompilationInfo() && */ collectProfiledExprs
        && ((!node->getType().isInt64()) || (profileLongParms /* && (valueInfo->getTopValue() == 0) */))) {
#ifdef J9_PROJECT_SPECIFIC
        TR_ValueInfo* valueInfo
          = static_cast<TR_ValueInfo*>(TR_ValueProfileInfoManager::getProfiledValueInfo(node, comp(), ValueInfo));
        if (valueInfo) {
          if ((valueInfo->getTopProbability() > MIN_PROFILED_FREQUENCY) && (valueInfo->getTotalFrequency() > 0)
            && !_containsCall && !node->getByteCodeInfo().doNotProfile() && !node->isNodeCreatedByPRE() &&
            // Only collect nodes from unspecialized blocks to avoid specializing the same nodes twice
            !block->isSpecialized()) {
            if ((!node->getType().isInt64()) || (profileLongParms && (valueInfo->getTopValue() == 0))) {
              ...
              if (comp()->requiresSpineChecks() && node->getOpCodeValue() == TR::contigarraylength
                ...
                dumpOptDetails(comp(),
                  "From value profiling, NOT USING inconclusive contigarraylength node %p with value 0 freq %d total "
                  "freq %d\n",
                  node, ((int32_t)(valueInfo->getTopProbability() * ((float)valueInfo->getTotalFrequency()))),
                ...
                dumpOptDetails(comp(), "From value profiling, node %p has value %d freq %d total freq %d\n", node,
                  ...
                  ((int32_t)(valueInfo->getTopProbability() * ((float)valueInfo->getTotalFrequency()))),
                  valueInfo->getTotalFrequency());
  ...
  SharedSparseBitVector& reverseBranchInLoops)
{
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
    ...
    if (!_neitherLoopCold || nextBlock->isCold()) {
      ...
      // Set the flag on blocks in the slow version of the loop, so that in the analysis phase, we don't collect these
      // blocks
      ...
      if (clonedSucc) {
        ...
        if (!fallsThrough)
          ...
          TR_BlockStructure* newGotoBlockStructure
            = new (_cfg->structureRegion()) TR_BlockStructure(comp(), newGotoBlock->getNumber(), newGotoBlock);
  ...
  if (_requiresAdditionalCheckForIncrement
    ...
    if (_constNode->getType().isInt32()) {
      ...
      TR_ASSERT(0,
        "Unable to create test outside of loop for checking if iv update increment is greater than 0 due to unknown "
        "type\n");
    }
  }
  ...
  if (_loopConditionInvariant && _asyncCheckTree) {
...
#ifdef J9_PROJECT_SPECIFIC
      TR_ValueInfo* valueInfo
        = static_cast<TR_ValueInfo*>(TR_ValueProfileInfoManager::getProfiledValueInfo(loopLimit, comp(), ValueInfo));
      if (valueInfo) {
        if (valueInfo->getTotalFrequency()) {
          dumpOptDetails(comp(),
            "From value profiling, loop test node %p has value %d freq %d total freq %d, and max value %d\n", loopLimit,
            valueInfo->getTopValue(), (int32_t)(valueInfo->getTopProbability() * valueInfo->getTotalFrequency()),
          ...
          // if (shouldOnlySpecializeLoops())
          {
            ...
            if (valueInfo->getMaxValue() > profiledLoopLimit && valuedFreq / totalFreq < 0.95f) {
              dumpOptDetails(comp(),
                "Maximum profiled value of %d exceeds profiled loop limit of %d, and value profiled iterations below "
                "limit account for only %5.3f of iterations.  Delaying asyncCheck removal for node %p\n",
    ...
    if (!comp()->getOption(TR_DisableAsyncCheckVersioning) && !refineAliases() && _canPredictIters
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(new (_curLoop->_memRegion) RemoveAsyncCheck(this, prep, _asyncCheckTree));
      }
    }
  }
  ...
  if (!specializedNodes->isEmpty()) {
    TR::SymbolReference** symRefs
      = (TR::SymbolReference**)trMemory()->allocateStackMemory(comp()->getSymRefCount() * sizeof(TR::SymbolReference*));
      ...
      for (TR::TreeTopIterator it(entry, comp()); !it.isAt(exit); it.stepForward()) {
        ...
        if (op.isNullCheck() || op.getOpCodeValue() == TR::DIVCHK) {
          ...
          if (childOp == TR::iconst || childOp == TR::iu2l) {
            dumpOptDetails(
              comp(), "Removing check n%un [%p] because child has been specialized\n", node->getGlobalIndex(), node);
  ...
  if (refineAliases())
    buildAliasRefinementComparisonTrees(
      nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees, &comparisonTrees, clonedLoopInvariantBlock);
  ...
  if (comp()->cg()->performsChecksExplicitly() || (comp()->getMethodHotness() > hotnessThreshold)) {
    ...
    for (ListElement<TR::TreeTop>* elt = head; elt != NULL; elt = elt->getNextElement()) {
      ...
      if (!check->getOpCode().isNullCheck()) {
        TR_ASSERT_FATAL(
          check->getOpCodeValue() == TR::treetop, "Unexpected opcode for n%dn [%p]\n", check->getGlobalIndex(), check);
        continue;
      }
  ...
  if (comp()->getHCRMode() != TR::none && disableLoopHCR == NULL) {
    ...
    if (!safeToRemoveHCRGuards) {
      dumpOptDetails(comp(), "Cannot remove HCR guards in loop %d due to n%un [%p]\n", whileLoop->getNumber(),
        culprit->getGlobalIndex(), culprit);
    ...
    } else if (!performTransformation(comp(), "%sCreating versioned HCR guards\n", OPT_DETAILS_LOOP_VERSIONER)) {
  ...
  if (!awrtbariTrees->isEmpty() && !disableWrtbarVersion && (safeToVersionAwrtbari || enableWrtbarVersion)
  ...
  if (!_curLoop->_privatizationOK && _curLoop->_privatizationsRequested) {
    ...
    LoopBodySearch search(
      comp(), _curLoop->_memRegion, whileLoop, &_curLoop->_optimisticallyRemovableNodes, &_curLoop->_takenBranches);
  ...
  if (comp()->getHCRMode() == TR::osr && disableLoopOSR == NULL) {
    ...
    TR_ASSERT_FATAL(_curLoop->_optimisticallyRemovableNodes.contains(_curLoop->_definitelyRemovableNodes),
      "all _definitelyRemovableNodes should also be _optimisticallyRemovableNodes in loop %d", whileLoop->getNumber());
    ...
    if (seenOSRGuards && safeToRemoveOSRGuards) {
      ...
      if (performTransformation(comp(), "%sCreate versioned OSRGuard\n", OPT_DETAILS_LOOP_VERSIONER)) {
        ...
        if (trace())
          traceMsg(
            comp(), "OSRGuard n%dn has been created to guard against method invalidation\n", guard->getGlobalIndex());
        guard->setBranchDestination(clonedLoopInvariantBlock->getEntry());
        comparisonTrees.add(guard);
      } else {
        safeToRemoveOSRGuards = false;
    ...
    for (; search.hasTreeTop(); search.advance()) {
      ...
      if (op.isNullCheck() || op.getOpCodeValue() == TR::DIVCHK) {
        ...
        if (child->getOpCode().isLoadDirect()) {
          dumpOptDetails(
            comp(), "Removing check n%un [%p] because child has been privatized\n", node->getGlobalIndex(), node);
  ...
  // two versions of the loop, it is necessary that there be at least one test at the top.  Therefore, the following is
  // required to ensure that a test is created.
  ...
  TR_BlockStructure* clonedInvariantBlockStructure = new (_cfg->structureRegion())
  ...
  TR_RegionStructure* properRegion
    = new (_cfg->structureRegion()) TR_RegionStructure(comp(), chooserBlock->getNumber());
  ...
  TR_StructureSubGraphNode* invariantNode
    = new (_cfg->structureRegion()) TR_StructureSubGraphNode(invariantBlockStructure);
  TR_StructureSubGraphNode* clonedInvariantNode
    = new (_cfg->structureRegion()) TR_StructureSubGraphNode(clonedInvariantBlockStructure);
  ...
  while (currComparisonBlock) {
    ...
    TR_StructureSubGraphNode* comparisonNode
      = new (_cfg->structureRegion()) TR_StructureSubGraphNode(comparisonBlockStructure);
    ...
    if (isTest) {
      if (currCriticalEdgeBlock != NULL) {
        TR_StructureSubGraphNode* criticalEdgeNode
          = new (_cfg->structureRegion()) TR_StructureSubGraphNode(currCriticalEdgeBlock->getData()->getStructureOf());
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    ...
    if (subStruct == properRegion) {
      ...
      for (auto changedSuccEdge = succNode->getSuccessors().begin(); changedSuccEdge != succNode->getSuccessors().end();
           ++changedSuccEdge) {
        //
  ...
  for (newGotoBlockStructure = newGotoBlockStructuresIt.getCurrent(); newGotoBlockStructure;
    ...
    TR_StructureSubGraphNode* newGotoBlockNode
      = new (_cfg->structureRegion()) TR_StructureSubGraphNode(newGotoBlockStructure);
  ...
  List<TR::Node>* nullCheckedReferences, List<TR::TreeTop>* nullCheckTrees)
{
  ...
  for (; nextNode; nextNode = nextNode->getNextElement(), nextTree = nextTree->getNextElement()) {
    ...
    if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is null at n%un [%p]\n",
      ...
      TR::Node* nextComparisonNode = TR::Node::createif(
        TR::ifacmpeq, duplicateNullCheckReference, TR::Node::aconst(duplicateNullCheckReference, 0), _exitGotoTarget);
      LoopEntryPrep* prep = createLoopEntryPrep(LoopEntryPrep::TEST, nextComparisonNode);
      if (prep != NULL) {
        TR::Node* checkNode = nextTree->getData()->getNode();
        nodeWillBeRemovedIfPossible(checkNode, prep);
        _curLoop->_loopImprovements.push_back(new (_curLoop->_memRegion) RemoveNullCheck(this, prep, checkNode));
      }
    }
  }
}
  ...
  while (nextTree) {
    ...
    if (performTransformation(comp(),
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(new (_curLoop->_memRegion) RemoveWriteBarrier(this, prep, awrtbariNode));
      }
    }
    ...
    if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is divide by zero\n",
      ...
      if (duplicateDivisor->getType().isInt64())
        ifNode = TR::Node::createif(
          TR::iflcmpeq, duplicateDivisor, TR::Node::create(duplicateDivisor, TR::lconst, 0, 0), _exitGotoTarget);
      else
        ifNode = TR::Node::createif(
          TR::ificmpeq, duplicateDivisor, TR::Node::create(duplicateDivisor, TR::iconst, 0, 0), _exitGotoTarget);

      LoopEntryPrep* prep = createLoopEntryPrep(LoopEntryPrep::TEST, ifNode);
      if (prep != NULL) {
        nodeWillBeRemovedIfPossible(divCheckNode, prep);
        _curLoop->_loopImprovements.push_back(new (_curLoop->_memRegion) RemoveDivCheck(this, prep, divCheckNode));
      }
    }
...
void TR_LoopVersioner::buildCheckCastComparisonsTree(List<TR::TreeTop>* checkCastTrees)
{
  ...
  while (nextTree) {
    ...
    if (!performTransformation(comp(), "%s Creating test outside loop for checking that checkcast n%un [%p] passes\n",
    ...
    TR::Node* instanceofNode
      = TR::Node::createWithSymRef(TR:: instanceof, 2, 2, duplicateCheckedValue, duplicateClassPtr,
    ...
    if (prep != NULL) {
      ...
      _curLoop->_loopImprovements.push_back(new (_curLoop->_memRegion) RemoveCheckCast(this, prep, checkCastTree));
    }
...
void TR_LoopVersioner::RemoveArrayStoreCheck::improveLoop()
{
  ...
  TR::TreeTop* firstNewTree
    = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, arrayStoreCheckNode->getFirstChild()), NULL, NULL);
  ...
  while (nextSpecializedNode) {
    ...
    if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is value profiled\n",
      ...
      if (specializedNode->getType().isInt64()) {
        ...
        comparisonNode = TR::Node::createif(
          TR::ificmpne, duplicateNode, TR::Node::create(duplicateNode, TR::iconst, 0, value), _exitGotoTarget);
      }
      comparisonTrees->add(comparisonNode);
      dumpOptDetails(
        comp(), "The node %p has been created for testing if value profiling check is required\n", comparisonNode);
        ...
        if (specializedNode->getOpCode().isLoadVar()) {
          ...
          if (!tempSymRef) {
            ...
            TR::Node* newStore = TR::Node::createWithSymRef(
              TR::istore, 1, 1, TR::Node::create(TR::l2i, 1, specializedNode->duplicateTree()), tempSymRef);
  ...
  List<TR::TreeTop>* conditionalTrees, SharedSparseBitVector& reverseBranchInLoops)
{
  ...
  while (nextTree) {
    ...
    if (conditionalNode->isTheVirtualGuardForAGuardedInlinedCall() && !conditionalNode->isNonoverriddenGuard()
      && !conditionalNode->isHCRGuard() && !conditionalNode->isBreakpointGuard()) {
      ...
      bool ignoreHeapificationStore = false;
      if (thisChild->getOpCode().hasSymbolReference() && thisChild->getSymbolReference()->getSymbol()->isAutoOrParm())
        ignoreHeapificationStore = true;

      bool b = isExprInvariant(thisChild, ignoreHeapificationStore);
      if (!b && !nodeToBeChecked) {
        if (thisChild->getOpCode().hasSymbolReference() && thisChild->getSymbolReference()->getSymbol()->isAuto()) {
    ...
    if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is conditional\n",
      ...
      if (conditionalNode->isVersionableIfWithMaxExpr() || conditionalNode->isVersionableIfWithMinExpr()) {
        ...
        if (isInductionVar
          ...
          extraIter = TR::Node::create(TR::iand, 2, extraIter, TR::Node::create(parent, TR::iconst, 0, condValue));
          ...
          if (duplicateIndexNode->getOpCode().isLoad()
            ...
            replaceInductionVariable(
              conditionalNode, duplicateIndexNode, indexChildIndex, indexSymRefNum, maxValue, visitCount);
          }
        }
      }
      ...
      if (replaceIndexWithExpr) {
        ...
        if (indexNodeOccursAsSecondChild) {
          if (!reverseBranch)
            duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCodeValue(),
              conditionalNode->getFirstChild()->duplicateTree(comp()), duplicateIndexNode, _exitGotoTarget);
          else
            duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCode().getOpCodeForReverseBranch(),
              conditionalNode->getFirstChild()->duplicateTree(comp()), duplicateIndexNode, _exitGotoTarget);
        } else {
          if (!reverseBranch)
            duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCodeValue(), duplicateIndexNode,
              conditionalNode->getSecondChild()->duplicateTree(comp()), _exitGotoTarget);
          else
            duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCode().getOpCodeForReverseBranch(),
              duplicateIndexNode, conditionalNode->getSecondChild()->duplicateTree(comp()), _exitGotoTarget);
        }
      } else {
        if (!reverseBranch)
          duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCodeValue(),
      ...
      LoopEntryPrep* prep = createLoopEntryPrep(LoopEntryPrep::TEST, duplicateComparisonNode);
    ...
    if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] has spine\n",
      ...
      if (op == TR::SpineCHK) {
        ...
        TR_ASSERT_FATAL(prereqEntry != _curLoop->_boundCheckPrepsWithSpineChecks.end(),
          "missing prep for removal of bound check from BNDCHKwithSpineCHK n%un [%p]", spineCheckNode->getGlobalIndex(),
          spineCheckNode);
  ...
  while (nextTree) {
      ...
      if (arrayLengthNode->getOpCode().isArrayLength()) {
        ...
        if (arrayObject->getOpCode().hasSymbolReference() && arrayObject->getSymbolReference()->getSymbol()->isAuto()) {
          ...
          if (invariantArrayObject) {
            ...
            arrayLengthNode
              = TR::Node::create(arrayLengthNode, arrayLengthNode->getOpCodeValue(), 1, dupInvariantArrayObject);
          } else
    ...
    if (isIndexInvariant) {
      ...
      while (indexSymRef && changedIndexSymRef) {
        ...
        if (!foundInductionVariable) {
          indexChild = isDependentOnInductionVariable(indexChild, changedIndexSymRefAtSomePoint, isIndexChildMultiplied,
      ...
      if (isSpecialInductionVariable) {
        ...
        nextComparisonNode
          = TR::Node::createif(TR::ificmplt, boundCheckNode->getChild(indexChildIndex)->duplicateTreeForCodeMotion(),
            TR::Node::create(boundCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
        if (trace())
        ...
        if ((isAddition && !indVarOccursAsSecondChildOfSub) || (!isAddition && indVarOccursAsSecondChildOfSub)) {
          nextComparisonNode = TR::Node::createif(
            TR::ificmplt, duplicateIndex, TR::Node::create(boundCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
      ...
      if (changedIndexSymRefAtSomePoint
        || (!_unchangedValueUsedInBndCheck->get(boundCheckNode->getGlobalIndex()) && !isSpecialInductionVariable)) {
        if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
          // printf("Changing test in %s\n", comp()->signature());
          TR::Node* storeNode = _storeTrees[origIndexSymRef->getReferenceNumber()]->getNode();
          TR::Node* storeRhs = storeNode->getFirstChild()->duplicateTree();
      ...
      if (isLoopDrivingInductionVariable || isDerivedInductionVariable) {
        ...
        TR::Node* extraIter = TR::Node::create(TR::icmpeq, 2, remNode, zeroNode);
        extraIter
          = TR::Node::create(TR::iand, 2, extraIter, TR::Node::create(boundCheckNode, TR::iconst, 0, condValue));
        numIterations = TR::Node::create(TR::iadd, 2, numIterations, extraIter);
        numIterations = TR::Node::create(TR::imul, 2, numIterations, incrJNode);
      ...
      if (isSpecialInductionVariable) {
        ...
        nextComparisonNode
          = TR::Node::createif(TR::ifiucmpgt, correctCheckNode, arrayLengthNode->duplicateTree(), _exitGotoTarget);
        if (trace())
          traceMsg(comp(), "Special Induction variable added in each iter -> Creating %p (%s)\n", nextComparisonNode,
            nextComparisonNode->getOpCode().getName());
      } else {
          ...
          if (!indVarOccursAsSecondChildOfSub) {
            ...
            if (isLoopDrivingInductionVariable) {
              if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt)
                || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge))
                nextComparisonNode = TR::Node::createif(
                  TR::ificmple, correctCheckNode, TR::Node::create(boundCheckNode, TR::iconst, 0, -1), _exitGotoTarget);
            }

            if (!nextComparisonNode)
              nextComparisonNode = TR::Node::createif(
                TR::ificmplt, correctCheckNode, TR::Node::create(boundCheckNode, TR::iconst, 0, -1), _exitGotoTarget);
      ...
      if (!isSpecialInductionVariable) {
        ...
        if (!isIndexChildMultiplied) {
          ...
          if ((isAddition && !indVarOccursAsSecondChildOfSub) || (!isAddition && indVarOccursAsSecondChildOfSub)) {
            ...
            vcount_t visitCount = comp()->incVisitCount();
            replaceInductionVariable(
              NULL, duplicateMulNode, -1, indexSymRef->getReferenceNumber(), loopLimit->duplicateTree(), visitCount);
            visitCount = comp()->incVisitCount();
            replaceInductionVariable(
              NULL, duplicateMulHNode, -1, indexSymRef->getReferenceNumber(), loopLimit->duplicateTree(), visitCount);
          }

          traceMsg(comp(), " node : %p Loop limit %p )\n", nextComparisonNode, loopLimit);
          // If its negative
          nextComparisonNode = TR::Node::createif(
            TR::ificmplt, duplicateMulNode, TR::Node::create(boundCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
          nextComparisonNode->setIsVersionableIfWithMaxExpr(comp());
          if (comp()->requiresSpineChecks())
            findAndReplaceContigArrayLen(NULL, nextComparisonNode, comp()->incVisitCount());
          ...
          nextComparisonNode = TR::Node::createif(
            TR::ifiucmpgt, duplicateMulHNode, TR::Node::create(boundCheckNode, TR::iconst, 0, 0), _exitGotoTarget);
          nextComparisonNode->setIsVersionableIfWithMaxExpr(comp());
          if (comp()->requiresSpineChecks())
            findAndReplaceContigArrayLen(NULL, nextComparisonNode, comp()->incVisitCount());
  ...
  TR::TreeTop* boundCheckTree, LoopEntryPrep* prep, List<TR::TreeTop>* spineCheckTrees)
{
  ...
  if (op == TR::BNDCHK || op == TR::ArrayCopyBNDCHK) {
    ...
    TR_ASSERT_FATAL(op == TR::BNDCHKwithSpineCHK, "expected BNDCHK, ArrayCopyBNDCHK, or BNDCHKwithSpineCHK, but got %s",
      TR::ILOpCode(op).getName());
  ...
  if (node->isInternalPointer()
    ...
    if (!node->getFirstChild()->isThisPointer()) {
      ...
      TR::Node* ifacmpeqNode
        = TR::Node::createif(TR::ifacmpeq, node->getFirstChild(), TR::Node::aconst(node, 0), _exitGotoTarget);
    ...
    if (node->isInternalPointer()
      && (firstChild->isInternalPointer()
        || (firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbolReference()->getSymbol()->isAuto()
      ...
      if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
        ...
        dumpOptDetails(
          comp(), "For node %p len %d other len %d sig %p other sig %p\n", node, len, otherLen, sig, otherSig);
    ...
    if (instanceOfReqd) {
      if (otherClassObject) {
        ...
        TR::Node* instanceofNode = TR::Node::createWithSymRef(
          TR:: instanceof, 2, 2, node->getFirstChild(), duplicateClassPtr,
          comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(comp()->getMethodSymbol()));
        TR::Node* ificmpeqNode
          = TR::Node::createif(TR::ificmpeq, instanceofNode, TR::Node::create(node, TR::iconst, 0, 0), _exitGotoTarget);
        ...
        TR::Node* vftLoad = TR::Node::createWithSymRef(
          TR::aloadi, 1, 1, node->getFirstChild(), comp()->getSymRefTab()->findOrCreateVftSymbolRef());
        ...
        if (TR::Compiler->target.is32Bit()) {
          classFlag = TR::Node::createWithSymRef(
            TR::iloadi, 1, 1, vftLoad, comp()->getSymRefTab()->findOrCreateClassAndDepthFlagsSymbolRef());
        } else {
          classFlag = TR::Node::createWithSymRef(
            TR::lloadi, 1, 1, vftLoad, comp()->getSymRefTab()->findOrCreateClassAndDepthFlagsSymbolRef());
          classFlag = TR::Node::create(TR::l2i, 1, classFlag);
        }
        TR::Node* andConstNode
          = TR::Node::create(classFlag, TR::iconst, 0, TR::Compiler->cls.flagValueForArrayCheck(comp()));
        ...
        // comp()->getSymRefTab()->findVtableEntrySymbolRef(node->getSymbolReference())), "Not enough information to emit
        // the instanceof test that is reqd\n"); dumpOptDetails(comp(), "otherClassObject is NULL in node %p\n", node);
    ...
    for (int32_t index = sizeof(a) / sizeof(temps) - 1; index >= 0; --index) {
      if (offset->getOpCodeValue() == a[index].addOp && offset->getSecondChild()->getOpCodeValue() == a[index].constOp
    ...
    if (!indexNode) {
      ...
      indexNode = TR::Node::create(
        type.isInt32() ? TR::iushr : TR::lushr, 2, indexNode, TR::Node::create(node, TR::iconst, 0, shiftWidth));
    }
    ...
    TR::Node* ificmpgeNode
      = TR::Node::createif(type.isInt32() ? TR::ificmpge : TR::iflcmpge, indexNode, arrayLengthNode, _exitGotoTarget);
...
bool TR_LoopVersioner::boundCheckUsesUnchangedValue(
  TR::TreeTop* bndCheckTree, TR::Node* node, TR::SymbolReference* symRef, TR_RegionStructure* loopStructure)
{
  ...
  if (entryBlock->getSuccessors().size() == 1) {
    ...
    if (!searchForStore) {
      ...
      //
      if ((succBlock->getSuccessors().size() == 1) && succBlock->getSuccessors().front()->getTo() == currentBlock) {
...
bool TR_LoopVersioner::initExprFromNode(Expr* expr, TR::Node* node, bool onlySearching)
{
  ...
  if (expr->_op.isLoadConst()) {
    ...
    TR_ASSERT_FATAL(!expr->_op.isIf() || node->getBranchDestination() == _exitGotoTarget,
...
bool TR_LoopVersioner::guardOkForExpr(TR::Node* node, bool onlySearching)
{
  ...
  case TR_BreakpointGuard:
    TR_ASSERT_FATAL(
      onlySearching, "guardOkForExpr: should not intern breakpoint guard n%un [%p]", node->getGlobalIndex(), node);
    return false;
  ...
  default:
    ...
    //
    TR_ASSERT_FATAL(
      false, "guardOkForExpr: n%un [%p]: unrecognized nop-able guard kind %d", node->getGlobalIndex(), node, (int)kind);
  }
}
  ...
  if (!initExprFromNode(&expr, node, onlySearching)) {
    ...
    if (shouldAssert) {
      if (node->isNopableInlineGuard()) {
        TR_VirtualGuard* guard = comp()->findVirtualGuardInfo(node);
        TR_ASSERT_FATAL(false, "n%un [%p] is unrepresentable guard kind=%d, test=%d", node->getGlobalIndex(), node,
          (int)guard->getKind(), (int)guard->getTestType());
      } else {
  ...
  TR::TreeTop* tt, TR::Node* node, TR::NodeChecklist* visited)
{
  ...
  if (trace()) {
    traceMsg(
      comp(), "substitutePrivTemps: Canonical n%un [%p] is expr %p\n", node->getGlobalIndex(), node, canonicalExpr);
  }
