...
TR_LoopVersioner::TR_LoopVersioner(TR::OptimizationManager* manager,
  ...
  , _visitedNodes(comp()->getNodeCount() /*an estimate*/,
                  trMemory(),
                  heapAlloc,
                  growable)
...
int32_t
TR_LoopVersioner::performWithDominators()
{
  if (trace())
...
bool
TR_LoopVersioner::loopIsWorthVersioning(TR_RegionStructure* naturalLoop)
{
  ...
  if (aggressive || comp()->getMethodHotness() <= warm) {
    ...
    if (entryBlock->getFrequency() < lvBlockFreqCutoff) {
      if (trace())
        traceMsg(
          comp(),
          "loopIsWorthVersioning returning false based on lvBlockFreqCutoff\n");
      return false;
    }
  }
...
int32_t
TR_LoopVersioner::performWithoutDominators()
{
  ...
  for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL;
    ...
    TR_ScratchList<TR_NodeParentBlockTuple> arrayMemberLoadCandidates(
      trMemory());
    TR_BitVector disqualifiedRefinementCandidates(
      1, trMemory(), stackAlloc, growable);
    ...
    bool nullChecksMayBeEliminated =
      detectChecksToBeEliminated(naturalLoop,
                                 &nullCheckedReferences,
                                 &nullCheckTrees,
                                 &numIndirections,
                                 &boundCheckTrees,
                                 &spineCheckTrees,
                                 ...
                                 &awrtbariTrees,
                                 &checkCastTrees,
                                 &arrayStoreCheckTrees,
                                 &specializedInvariantNodes,
                                 invariantNodes,
                                 &invariantTranslationNodesList,
                                 discontinue);
    ...
    if (_loopTestTree) {
      if ((_loopTestTree->getNode()->getNumChildren() > 1) &&
        ...
        if (trace())
          traceMsg(comp(),
                   "Limit in loop test tree %p is invariant\n",
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases())
      checkCastTreesWillBeEliminated =
        detectInvariantTrees(naturalLoop,
                             &checkCastTrees,
                             false,
    ...
    if ((versionToRefineAliases || !refineAliases()) &&
      ...
      versionNaturalLoop(naturalLoop,
                         &nullCheckedReferences,
                         &nullCheckTrees,
                         &boundCheckTrees,
                         &spineCheckTrees,
    ...
    if (versionedThisLoop) {
      ...
      TR_BitVector* inductionVars =
        new (trStackMemory()) TR_BitVector(symRefCount, trMemory(), stackAlloc);
      ...
      while (versionableInductionVar) {
        ...
        inductionCandidate->addAllBlocksInStructure(
          naturalLoop, comp(), trace() ? "auto" : NULL);
...
void
TR_LoopVersioner::performLoopTransfer()
{
  ...
  dumpOptDetails(comp(),
                 "Loop transfer in %s with size %d\n",
                 comp()->signature(),
                 _virtualGuardInfo.getSize());
    ...
    for (nextVirtualGuard = guardsIt.getFirst(); nextVirtualGuard != NULL;
      ...
      if (hotGuard->getOpCode().isIf() &&
        ...
        const char* debugCounter = TR::DebugCounter::debugCounterName(
          comp(),
          "loopVersioner.transfer/(%s)/%s/origin=block_%d",
          comp()->signature(),
        ...
        if (comp()->getOptions()->dynamicDebugCounterIsEnabled(debugCounter)) {
          TR::Block* newBlock = hotGuardBlock->splitEdge(
            hotGuardBlock,
            coldGuard->getBranchDestination()->getNode()->getBlock(),
            comp(),
            NULL,
            false);
...
//
TR::Block*
TR_LoopVersioner::createClonedHeader(TR::Block* origHeader,
                                     TR::TreeTop** endTree)
{
  ...
  for (auto e = origHeader->getSuccessors().begin();
       e != origHeader->getSuccessors().end();
       ++e) {
    removedEdges.add(*e);
  ...
  for (auto e = origHeader->getExceptionSuccessors().begin();
       e != origHeader->getExceptionSuccessors().end();
       ++e) {
    removedEdges.add(*e);
    TR::Block* dest = toBlock((*e)->getTo());
    _cfg->addEdge(
...
TR::Node*
TR_LoopVersioner::createSwitchNode(TR::Block* clonedHeader,
...
TR::Block*
TR_LoopVersioner::createEmptyGoto(TR::Block* source,
                                  TR::Block* dest,
                                  TR::TreeTop* endTree)
{
  ...
  TR::Node* gotoNode = TR::Node::create(
    destEntry->getNextTreeTop()->getNode(), TR::Goto, 0, destEntry);
...
bool
TR_LoopVersioner::detectInvariantChecks(List<TR::Node>* nullCheckedReferences,
                                        List<TR::TreeTop>* nullCheckTrees)
{
  ...
  for (node = nullCheckedReferences->getListHead(); node != NULL;) {
    ...
    if (!isNullCheckReferenceInvariant ||
      ...
      if (trace())
        traceMsg(comp(),
                 "Non invariant Null check reference %p (%s)\n",
                 node->getData(),
      ...
      if (trace())
        traceMsg(comp(),
                 "Invariant Null check reference %p (%s)\n",
                 node->getData(),
                 node->getData()->getOpCode().getName());
...
bool
TR_LoopVersioner::detectInvariantArrayStoreChecks(
    ...
    if (arrayNode && valueNode) {
      ...
      if (sourceInvariant) {
        ...
        if (!targetInvariant || _checksInDupHeader.find(treetop->getData())) {
          if (trace())
            traceMsg(comp(),
                     "Non invariant Array store check %p (%s)\n",
          ...
          if (trace())
            traceMsg(comp(),
                     "Invariant Array store check %p (%s)\n",
                     treetop->getData()->getNode(),
                     treetop->getData()->getNode()->getOpCode().getName());
          foundInvariantChecks = true;
          prevTreetop = treetop;
        ...
        if (trace())
          traceMsg(comp(),
                   "Non invariant Specialized expr %p (%s)\n",
                   treetop->getData()->getNode(),
                   treetop->getData()->getNode()->getOpCode().getName());
...
bool
TR_LoopVersioner::detectInvariantSpecializedExprs(List<TR::Node>* profiledExprs)
{
  ...
  for (node = profiledExprs->getListHead(); node != NULL;) {
    ...
    if (!isProfiledExprInvariant) {
      ...
      if (trace())
        traceMsg(comp(),
                 "Invariant Specialized expr %p (%s)\n",
                 node->getData(),
                 node->getData()->getOpCode().getName());
...
TR::Node*
nodeTreeGetFirstOpCode(TR::Node* n, TR::ILOpCodes op)
{
...
bool
TR_LoopVersioner::detectInvariantNodes(
...
TR::Node*
TR_LoopVersioner::findCallNodeInBlockForGuard(TR::Node* node)
{
...
bool
TR_LoopVersioner::detectInvariantTrees(
          ...
          if (!isExprInvariant(node->getChild(childNum))) {
            if (node->isVersionableIfWithMaxExpr() ||
              ...
              if (!child || !child->getOpCode().isLoadVarDirect()) {
                ...
                while (versionableInductionVar) {
                  dumpOptDetails(comp(),
                                 "Versionable induction var in if %d\n",
                                 *(versionableInductionVar->getData()));
    ...
    if (!isTreeInvariant) {
      ...
      if (onlyDetectHighlyBiasedBranches || (!node->getOpCode().isBranch()) ||
        ...
        if (!onlyDetectHighlyBiasedBranches && node->getOpCode().isBranch()) {
          ...
          if (trace())
            traceMsg(comp(),
                     "Discarded invariant branch  %p (%s) \n",
                     removedNode,
...
TR::Node*
TR_LoopVersioner::isDependentOnInvariant(TR::Node* useNode)
{
  ...
  if (trace())
    traceMsg(
      comp(),
      "Definition Counts for node [%p] is %d inside isDependentOnInvariant \n",
      useNode,
...
TR::Node*
TR_LoopVersioner::isDependentOnInductionVariable(
  TR::Node* useNode,
...
bool
TR_LoopVersioner::isVersionableArrayAccess(TR::Node* indexChild)
{
...
bool
TR_LoopVersioner::detectInvariantBoundChecks(List<TR::TreeTop>* boundCheckTrees)
{
  bool foundInvariantChecks = false;
  ...
  for (; nextTree;) {
      ...
      if (!isIndexInvariant && _loopConditionInvariant &&
        ...
        while (indexSymRef && changedIndexSymRef) {
          ...
          if (!isInductionVariable) {
            ...
            indexChild =
              isDependentOnInductionVariable(indexChild,
    ...
    if (isInductionVariable && indexSymRef) {
      if (!boundCheckUsesUnchangedValue(nextTree->getData(),
                                        indexChild,
                                        indexSymRef,
                                        _currentNaturalLoop)) {
    ...
    if (!isIndexInvariant && !isInductionVariable) {
      if (trace())
        traceMsg(comp(),
                 "Non invariant Bound check reference %p (%s)\n",
                 node,
                 node->getOpCode().getName());
      if (prevTree) {
        prevTree->setNextElement(nextTree->getNextElement());
      ...
      if (trace())
        traceMsg(comp(),
                 "Invariant Bound check reference %p (%s)\n",
                 node,
                 node->getOpCode().getName());
      foundInvariantChecks = true;
      prevTree = nextTree;
...
bool
TR_LoopVersioner::detectInvariantSpineChecks(List<TR::TreeTop>* spineCheckTrees)
{
  bool foundInvariantChecks = false;
  ...
  for (; nextTree;) {
    ...
    if (!isArrayInvariant) {
      ...
      if (trace())
        traceMsg(comp(),
                 "Non invariant Spine check reference %p (%s)\n",
                 node,
                 node->getOpCode().getName());
      if (prevTree) {
        prevTree->setNextElement(nextTree->getNextElement());
      ...
      if (trace())
        traceMsg(comp(),
                 "Invariant Spine check reference %p (%s)\n",
                 node,
                 node->getOpCode().getName());
      foundInvariantChecks = true;
      prevTree = nextTree;
...
bool
TR_LoopVersioner::detectInvariantDivChecks(List<TR::TreeTop>* divideCheckTrees)
{
  bool foundInvariantChecks = false;
  ...
  for (; nextTree;) {
    ...
    if (!isDivisorInvariant && !isInductionVariable) {
      if (trace())
        traceMsg(comp(),
                 "Non invariant Div check reference %p (%s)\n",
                 node,
                 node->getOpCode().getName());
      ...
      if (trace())
        traceMsg(comp(),
                 "Invariant Div check reference %p (%s)\n",
                 node,
                 node->getOpCode().getName());
      foundInvariantChecks = true;
      prevTree = nextTree;
...
bool
TR_LoopVersioner::isDependentOnAllocation(TR::Node* useNode,
...
bool
TR_LoopVersioner::detectInvariantAwrtbaris(List<TR::TreeTop>* awrtbariTrees)
{
  ...
  for (; nextTree;) {
    ...
    if (node->getOpCodeValue() == TR::awrtbari) {
      ...
      if (baseChild->getOpCode().hasSymbolReference() &&
        ...
        if (trace())
          traceMsg(
            comp(), "base invariant 11 in %p inv %d\n", node, isBaseInvariant);
...
bool
opCodeIsHoistable(TR::Node* node, TR::Compilation* comp)
{
  bool hasEffects = 0;
  ...
  hasEffects =
    (opCode.isCall() || opCodeValue == TR::New || opCodeValue == TR::newarray ||
...
bool
TR_LoopVersioner::isExprInvariant(TR::Node* node, bool ignoreHeapificationStore)
{
  _visitedNodes.empty();
...
bool
TR_LoopVersioner::isExprInvariantRecursive(TR::Node* node,
...
bool
TR_LoopVersioner::hasWrtbarBeenSeen(List<TR::TreeTop>* awrtbariTrees,
...
bool
TR_LoopVersioner::canPredictIters(
  TR_RegionStructure* whileLoop,
  ...
  TR::SymbolReference*& firstChildSymRef)
{
  ...
  if (((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple) ||
       (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt))) {
    returnValue = true;
    isIncreasing = blocksInWhileLoop.find(
      _loopTestTree->getNode()->getBranchDestination()->getNode()->getBlock());
  }
...
int32_t*
computeCallsiteCounts(TR_ScratchList<TR::Block>* loopBlocks,
                      TR::Compilation* comp)
{
...
bool
TR_LoopVersioner::checkProfiledGuardSuitability(
  TR_ScratchList<TR::Block>* loopBlocks,
  TR::Node* guardNode,
  ...
  if (comp->getMethodHotness() >= hot && callSymRef) {
    if (callSymRef->getSymbol() &&
        ...
        && comp->fej9()->maybeHighlyPolymorphic(
             comp,
             callSymRef->getOwningMethod(comp),
      ...
      if (disableLoopCodeRatioCheck || loopCodeRatio < 0.25) {
        ...
        TR::DebugCounter::incStaticDebugCounter(
          ...
          TR::DebugCounter::debugCounterName(
            comp,
            "profiledVersioning/unsuitableForVersioning/interfaceGuard/(%s)/"
            "bci=%d.%d",
            comp->signature(),
...
bool
TR_LoopVersioner::isBranchSuitableToVersion(
  TR_ScratchList<TR::Block>* loopBlocks,
  TR::Node* node,
  TR::Compilation* comp)
{
  ...
  if (node->isProfiledGuard()) {
    bool isGetFieldCacheCounts =
      !strncmp(comp->signature(),
               "org/apache/solr/request/SimpleFacets.getFieldCacheCounts(Lorg/"
               "apache/solr/search/SolrIndexSearcher;Lorg/apache/solr/search/"
               "DocSet;Ljava/lang/String;IIIZLjava/lang/String;Ljava/lang/"
               "String;)Lorg/apache/solr/common/util/NamedList;",
               60);
    ...
    if (isGetFieldCacheCounts || disableProfiledGuardVersioning) {
      ...
      TR_AddressInfo* valueInfo = static_cast<TR_AddressInfo*>(
        TR_ValueProfileInfoManager::getProfiledValueInfo(
          ics._byteCodeInfo, comp, AddressInfo));

      if (valueInfo) {
        if (trace())
          traceMsg(comp,
                   "Profiled guard probability %.2f for guard %p\n",
...
bool
TR_LoopVersioner::isBranchSuitableToDoLoopTransfer(
  TR_ScratchList<TR::Block>* loopBlocks,
  TR::Node* node,
  TR::Compilation* comp)
{
...
bool
TR_LoopVersioner::detectChecksToBeEliminated(
  TR_RegionStructure* whileLoop,
  List<TR::Node>* nullCheckedReferences,
  List<TR::TreeTop>* nullCheckTrees,
  List<int32_t>* numIndirections,
  List<TR::TreeTop>* boundCheckTrees,
  List<TR::TreeTop>* spineCheckTrees,
  List<int32_t>* numDimensions,
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    if (nextBlock->isCold()) {
      ...
      int32_t unimportantFrequencyRatio =
        unimportantFrequencyRatioStr ? atoi(unimportantFrequencyRatioStr) : 20;
    ...
    while (!(currentTree == exitTree)) {
      ...
      updateDefinitionsAndCollectProfiledExprs(NULL,
                                               currentNode,
                                               visitCount,
                                               specializedInvariantNodes,
                                               invariantNodes,
      ...
      if (!isUnimportant && !_containsCall && !refineAliases()) {
        if (currentOpCode.isNullCheck()) {
          ...
          foundPotentialChecks = true;
                   ...
                   isBranchSuitableToVersion(
                     &blocksInWhileLoop, currentNode, comp()) &&
          ...
          if (isConditionalTreeCandidateForElimination(currentTree)) {
            ...
            if (trace()) {
              ...
              if (_postDominators) {
                traceMsg(comp(),
                         "    controls %d out of %d blocks\n",
                         _postDominators->numberOfBlocksControlled(
                           nextBlock->getNumber()),
                         loop_size);

                traceMsg(comp(),
                         "    post dominates loop entry: %s\n",
...
void
TR_LoopVersioner::updateDefinitionsAndCollectProfiledExprs(
  TR::Node* parent,
  TR::Node* node,
  vcount_t visitCount,
  List<TR::Node>* profiledNodes,
  List<TR_NodeParentSymRef>* invariantNodes,
  List<TR_NodeParentSymRefWeightTuple>* invariantTranslationNodesList,
  TR::Node* lastFault,
  bool collectProfiledExprs,
  TR::Block* block,
  int32_t warmBranchCount)
{
    ...
    if (opCode.isStore()) {
      ...
      if (!heapificationStore && _writtenAndNotJustForHeapification)
        defAliases.getAliasesAndUnionWith(
          *_writtenAndNotJustForHeapification); // TODO: avoid getting the same
                                                // aliases twice
    }
  }
  ...
  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++)
    updateDefinitionsAndCollectProfiledExprs(node,
                                             node->getChild(i),
                                             visitCount,
                                             profiledNodes,
                                             invariantNodes,
...
void
TR_LoopVersioner::versionNaturalLoop(
  TR_RegionStructure* whileLoop,
  List<TR::Node>* nullCheckedReferences,
  List<TR::TreeTop>* nullCheckTrees,
  List<TR::TreeTop>* boundCheckTrees,
  ...
  List<TR_Structure>* clonedInnerWhileLoops,
  bool skipVersioningAsynchk,
  SharedSparseBitVector& reverseBranchInLoops)
{
  if (!performTransformation(comp(),
                             "%sVersioning natural loop %d\n",
                             OPT_DETAILS_LOOP_VERSIONER,
                             whileLoop->getNumber()))
    return;
  ...
  TR::Block** correspondingBlocks =
    (TR::Block**)trMemory()->allocateStackMemory(numNodes * sizeof(TR::Block*));
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    while (!(currentTree == exitTree)) {
      ...
      for (int32_t childNum = 0;
           childNum < currentTree->getNode()->getNumChildren();
           childNum++) {
    ...
    if (!disableLT && lastTree->getOpCode().isIf() &&
        ...
        isBranchSuitableToDoLoopTransfer(
          &blocksInWhileLoop, lastTree, comp()) &&
        performTransformation(comp(),
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    for (auto edge = nextBlock->getSuccessors().begin();
         edge != nextBlock->getSuccessors().end();
         ++edge) {
      ...
      if (clonedSucc) {
        ...
        if (!fallsThrough)
          ...
          if (trace())
            traceMsg(comp(),
                     "Creating new goto block : %d for node %p\n",
          ...
          TR::CFGEdge* e1 =
            TR::CFGEdge::createEdge(nextClonedBlock, newGotoBlock, trMemory());
          _cfg->addEdge(e1);
          TR::CFGEdge* e2 =
    ...
    for (auto edge = nextBlock->getExceptionSuccessors().begin();
         edge != nextBlock->getExceptionSuccessors().end();
         ++edge) {
      TR::Block* succ = toBlock((*edge)->getTo());
      TR::Block* nextClonedBlock = correspondingBlocks[nextBlock->getNumber()];
      TR::Block* clonedSucc = correspondingBlocks[succ->getNumber()];
      ...
      if (clonedSucc) {
        _cfg->addEdge(TR::CFGEdge::createExceptionEdge(
          nextClonedBlock, clonedSucc, trMemory()));
      } else {
        _cfg->addEdge(
          TR::CFGEdge::createExceptionEdge(nextClonedBlock, succ, trMemory()));
      }
    }
  }
  ...
  for (auto nextPred = blockAtHeadOfLoop->getPredecessors().begin();
       nextPred != blockAtHeadOfLoop->getPredecessors().end();
       ++nextPred) {
  ...
  TR::Block* clonedLoopInvariantBlock =
    TR::Block::createEmptyBlock(invariantBlock->getEntry()->getNode(),
                                comp(),
  ...
  if (_requiresAdditionalCheckForIncrement &&
      performTransformation(
        comp(),
        "%s Creating test outside loop for checking if loop driving induction "
        "variable is incremented by a loop invariant that is greater than 0\n",
        OPT_DETAILS_LOOP_VERSIONER)) {
    if (_constNode->getType().isInt32()) {
      TR::Node* nextComparisonNode =
        TR::Node::createif(TR::ificmple,
                           _constNode,
  ...
  if (_loopConditionInvariant && _asyncCheckTree) {
    ...
    if (!comp()->getOption(TR_DisableAsyncCheckVersioning) &&
        ...
        performTransformation(comp(),
                              ...
                              OPT_DETAILS_LOOP_VERSIONER)) {
      TR::Node* lowerBound =
        _loopTestTree->getNode()->getFirstChild()->duplicateTreeForCodeMotion();
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(new (
          _curLoop->_memRegion) RemoveAsyncCheck(this, prep, _asyncCheckTree));
      }
    }
  }
  ...
  if (!boundCheckTrees->isEmpty()) {
    ...
    buildBoundCheckComparisonsTree(
      boundCheckTrees, spineCheckTrees, reverseBranch);
  }
  ...
  if (!specializedNodes->isEmpty()) {
    ...
    bool specializedLongs = buildSpecializationTree(nullCheckTrees,
                                                    divCheckTrees,
                                                    checkCastTrees,
                                                    arrayStoreCheckTrees,
                                                    &comparisonTrees,
  ...
  if (refineAliases())
    buildAliasRefinementComparisonTrees(nullCheckTrees,
                                        divCheckTrees,
                                        checkCastTrees,
                                        arrayStoreCheckTrees,
                                        &comparisonTrees,
  ...
  if (comp()->cg()->performsChecksExplicitly() ||
    ...
    for (ListElement<TR::TreeTop>* elt = head; elt != NULL;
      ...
      if (!performTransformation(
            comp(),
            "%sOpportunistically attempting to eliminate null check n%un [%p] "
            "using existing prep %p\n",
            optDetailString(),
            check->getGlobalIndex(),
            check,
            prep)) {
  ...
  if (comp()->getHCRMode() == TR::osr && disableLoopOSR == NULL) {
    ...
    if (trace()) {
      traceMsg(comp(),
               "privatizationOK: %d : removedNodes ",
    ...
    for (; search.hasTreeTop(); search.advance()) {
      ...
      if (comp()->isPotentialOSRPoint(tt->getNode(), NULL, true)) {
        if (trace())
          traceMsg(
            comp(),
            "safeToRemoveOSRGuards false due to potential OSR point at n%dn\n",
            tt->getNode()->getGlobalIndex());
        ...
        if (trace())
          traceMsg(comp(),
                   "adding OSRGuard n%dn to the list of removedNodes\n",
                   osrGuard->getGlobalIndex());
      } else {
        if (trace())
          traceMsg(comp(),
                   "osrGuardSafety traversed n%dn\n",
                   tt->getNode()->getGlobalIndex());
      }
    }
  ...
  if (_curLoop->_privatizationsRequested && !_curLoop->_privTemps.empty()) {
    ...
    LoopBodySearch search(
      comp(), _curLoop->_memRegion, whileLoop, removedNodes, takenBranches);
  ...
  if (testCount == 0) {
    ...
    TR::Node* nextComparisonNode = TR::Node::createif(
      TR::ificmpne, constNode, constNode, clonedLoopInvariantBlock->getEntry());
    comparisonTrees.add(nextComparisonNode);
    dumpOptDetails(
      comp(),
  ...
  TR::DebugCounter::prependDebugCounter(
    ...
    TR::DebugCounter::debugCounterName(
      comp(),
      "loopVersioner.coldLoop/%d/(%s)/%s/bcinfo=%d.%d",
      blockAtHeadOfLoop->getFrequency(),
      comp()->signature(),
      comp()->getHotnessName(comp()->getMethodHotness()),
      bcInfo.getCallerIndex(),
      bcInfo.getByteCodeIndex()),
    clonedLoopInvariantBlock->getEntry()->getNextTreeTop(),
    1,
  ...
  while (currComparisonBlock) {
    ...
    if (isTest)
      debugCounter = TR::DebugCounter::debugCounterName(
        comp(),
        "loopVersioner.fail/(%s)/%s/origin=block_%d",
        comp()->signature(),
        comp()->getHotnessName(comp()->getMethodHotness()),
  ...
  _cfg->addEdge(TR::CFGEdge::createEdge(
    clonedLoopInvariantBlock, blockAtHeadOfClonedLoop, trMemory()));
  ...
  memset(
    correspondingSubNodes, 0, numNodes * sizeof(TR_StructureSubGraphNode*));

  TR_RegionStructure* clonedWhileLoop =
    whileLoop
      ->cloneStructure(correspondingBlocks,
                       correspondingSubNodes,
                       innerWhileLoops,
                       clonedInnerWhileLoops)
      ->asRegion();
  ...
  TR_BlockStructure* clonedInvariantBlockStructure =
    new (_cfg->structureRegion()) TR_BlockStructure(
      comp(), clonedLoopInvariantBlock->getNumber(), clonedLoopInvariantBlock);
  ...
  while (currComparisonBlock) {
    ...
    if (isTest) {
      if (currCriticalEdgeBlock != NULL) {
        ...
        TR::CFGEdge::createEdge(
          prevComparisonNode, criticalEdgeNode, trMemory());
        TR::CFGEdge::createEdge(
          criticalEdgeNode, clonedInvariantNode, trMemory());
        currCriticalEdgeBlock = currCriticalEdgeBlock->getNextElement();
      } else
        TR::CFGEdge::createEdge(
          prevComparisonNode, clonedInvariantNode, trMemory());
    }
  }
  ...
  for (exitEdge = ei.getCurrent(); exitEdge; exitEdge = ei.getNext()) {
    ...
    if (!seenExitNodes.get(node->getNumber())) {
      ...
      if (node->getNumber() == regionEntryNode->getNumber()) {
        ...
        properRegion->addExitEdge(
          whileNode, node->getNumber(), isExceptionEdge);
      }
  ...
  for (exitEdge = ei.getCurrent(); exitEdge; exitEdge = ei.getNext()) {
    ...
    if (!seenExitNodes.get(node->getNumber())) {
      ...
      if (node->getNumber() == regionEntryNode->getNumber()) {
        ...
        properRegion->addExitEdge(
          clonedWhileNode, node->getNumber(), isExceptionEdge);
      }
      seenExitNodes.set(node->getNumber());
    }
  }
  ...
  for (newGotoBlockStructure = newGotoBlockStructuresIt.getCurrent();
       newGotoBlockStructure;
       newGotoBlockStructure = newGotoBlockStructuresIt.getNext()) {
    TR::Block* predBlock = toBlock(
      newGotoBlockStructure->getBlock()->getPredecessors().front()->getFrom());
    clonedWhileLoop->addExternalEdge(
      predBlock->getStructureOf(), newGotoBlockStructure->getNumber(), false);
  }
...
void
TR_LoopVersioner::RemoveAsyncCheck::improveLoop()
{
...
void
TR_LoopVersioner::buildNullCheckComparisonsTree(
  List<TR::Node>* nullCheckedReferences,
  List<TR::TreeTop>* nullCheckTrees)
{
  ...
  for (; nextNode; nextNode = nextNode->getNextElement(),
    ...
    if (!isNextNodeInvariant) {
      bool nullCheckReferenceisField =
        ...
        !_seenDefinedSymbolReferences->get(
          nextNode->getData()->getSymbolReference()->getReferenceNumber()) &&
        nextNode->getData()
          ->getFirstChild()
          ->getOpCode()
          .hasSymbolReference() &&
    ...
    if (performTransformation(comp(),
      ...
      TR::Node* nextComparisonNode =
        TR::Node::createif(TR::ifacmpeq,
                           duplicateNullCheckReference,
                           TR::Node::aconst(duplicateNullCheckReference, 0),
                           _exitGotoTarget);
      LoopEntryPrep* prep =
        createLoopEntryPrep(LoopEntryPrep::TEST, nextComparisonNode);
      if (prep != NULL) {
...
void
TR_LoopVersioner::RemoveNullCheck::improveLoop()
{
  dumpOptDetails(comp(),
...
void
TR_LoopVersioner::buildAwrtbariComparisonsTree(List<TR::TreeTop>* awrtbariTrees)
{
#ifdef J9_PROJECT_SPECIFIC
  ListElement<TR::TreeTop>* nextTree = awrtbariTrees->getListHead();
  while (nextTree) {
    ...
    if (performTransformation(comp(),
      ...
      ifNode =
        TR::Node::createif(TR::ificmpne,
                           TR::Node::create(TR::iand, 2, ifNode1, ifNode2),
                           TR::Node::create(duplicateBase, TR::iconst, 0, 0),
                           _exitGotoTarget);
...
void
TR_LoopVersioner::RemoveWriteBarrier::improveLoop()
{
  dumpOptDetails(comp(),
...
void
TR_LoopVersioner::buildDivCheckComparisonsTree(List<TR::TreeTop>* divCheckTrees)
{
...
void
TR_LoopVersioner::RemoveDivCheck::improveLoop()
{
  dumpOptDetails(comp(),
...
void
TR_LoopVersioner::buildCheckCastComparisonsTree(
  List<TR::TreeTop>* checkCastTrees)
{
  ...
  while (nextTree) {
    ...
    TR::Node* instanceofNode = TR::Node::createWithSymRef(
      TR:: instanceof,
                     2,
                     2,
                     duplicateCheckedValue,
                     duplicateClassPtr,
                     comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(
                       comp()->getMethodSymbol()));
    TR::Node* ificmpeqNode =
      TR::Node::createif(TR::ificmpeq,
                         instanceofNode,
                         TR::Node::create(checkCastNode, TR::iconst, 0, 0),
                         _exitGotoTarget);
...
void
TR_LoopVersioner::RemoveCheckCast::improveLoop()
{
  ...
  TR::TreeTop* firstNewTree = TR::TreeTop::create(
    comp(),
    TR::Node::create(TR::treetop, 1, checkCastNode->getFirstChild()),
    NULL,
    NULL);
  TR::TreeTop* secondNewTree = TR::TreeTop::create(
    comp(),
    TR::Node::create(TR::treetop, 1, checkCastNode->getSecondChild()),
    NULL,
    NULL);
...
void
TR_LoopVersioner::buildArrayStoreCheckComparisonsTree(
  List<TR::TreeTop>* arrayStoreCheckTrees)
{
...
void
TR_LoopVersioner::RemoveArrayStoreCheck::improveLoop()
{
...
void
TR_LoopVersioner::convertSpecializedLongsToInts(TR::Node* node,
                                                vcount_t visitCount,
                                                TR::SymbolReference** symRefs)
{
  if (node->getVisitCount() == visitCount)
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++)
    convertSpecializedLongsToInts(
      node->getChild(childNum), visitCount, symRefs);
}
...
bool
nodeTreeContainsOpCode(TR::Node* n, TR::ILOpCodes op)
{
  if (n->getOpCodeValue() == op)
    return true;
  for (int32_t i = 0; i < n->getNumChildren(); ++i) {
...
bool
TR_LoopVersioner::buildLoopInvariantTree(
  List<TR_NodeParentSymRef>* invariantNodes)
{
...
bool
TR_LoopVersioner::buildSpecializationTree(
  List<TR::TreeTop>* nullCheckTrees,
  List<TR::TreeTop>* divCheckTrees,
  List<TR::TreeTop>* checkCastTrees,
  List<TR::TreeTop>* arrayStoreCheckTrees,
      ...
      if (specializedNode->getType().isInt64()) {
        ...
        comparisonNode =
          TR::Node::createif(TR::iflcmpne,
                             highInt,
                             TR::Node::create(duplicateNode, TR::lconst, 0, 0),
                             _exitGotoTarget);
      } else {
...
static void
cleanseIntegralNodeFlagsInSubtree(TR::Compilation* comp, TR::Node* node)
{
  dumpOptDetails(
    comp,
...
void
TR_LoopVersioner::buildConditionalTree(
  List<TR::TreeTop>* conditionalTrees,
  SharedSparseBitVector& reverseBranchInLoops)
{
  ...
  while (nextTree) {
      ...
      if (guardInfo->getTestType() == TR_VftTest) {
        ...
        if ((conditionalNode->getFirstChild()->getNumChildren() > 0) &&
            (conditionalNode->getFirstChild()
               ...
               ->getNumChildren() > 0)) {
          thisChild =
            conditionalNode->getFirstChild()->getFirstChild()->getFirstChild();
          searchReqd = false;
    ...
    if (performTransformation(comp(),
      ...
      if (trace())
        traceMsg(comp(),
                 "changeConditionalToUnconditionalInBothVersions %d\n",
      ...
      if (conditionalNode->isVersionableIfWithMaxExpr() ||
        ...
        for (int32_t childNum = 0; childNum < conditionalNode->getNumChildren();
          ...
          if (!isExprInvariant(conditionalNode->getChild(childNum))) {
            if (trace())
              traceMsg(comp(),
                       "Versioning if conditional node [%p]\n",
                       conditionalNode);
            while (child->getOpCode().isAdd() || child->getOpCode().isSub() ||
                   child->getOpCode().isMul()) {
              if (child->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if (isInductionVar &&
          ...
          if (duplicateIndexNode->getOpCode().isLoad() &&
            ...
            copyOnWriteNode(origConditionalNode, &conditionalNode);
            replaceInductionVariable(conditionalNode,
                                     duplicateIndexNode,
                                     indexChildIndex,
                                     indexSymRefNum,
                                     maxValue,
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(
          new (_curLoop->_memRegion) FoldConditional(this,
                                                     prep,
                                                     origConditionalNode,
                                                     reverseBranch,
                                                     /* original = */ true));
        ...
        if (changeConditionalToUnconditionalInBothVersions &&
          ...
          _curLoop->_loopImprovements.push_back(new (
            _curLoop->_memRegion) FoldConditional(this,
                                                  prep,
...
void
TR_LoopVersioner::FoldConditional::improveLoop()
{
  dumpOptDetails(comp(),
...
void
TR_LoopVersioner::copyOnWriteNode(TR::Node* original, TR::Node** current)
{
  if (*current != original)
  ...
  *current = original->duplicateTree();
  ...
  if (comp()->getOutFile() != NULL &&
    ...
    comp()->getDebug()->printWithFixedPrefix(
      comp()->getOutFile(), original, 1, true, false, "\t\t");
    dumpOptDetails(comp(), "\n\tduplicate node:\n");
    comp()->getDebug()->printWithFixedPrefix(
      comp()->getOutFile(), *current, 1, true, false, "\t\t");
  }
}

bool
TR_LoopVersioner::replaceInductionVariable(TR::Node* parent,
                                           TR::Node* node,
                                           int childNum,
                                           ...
                                           int visitCount)
{
  ...
  int i;
  for (i = 0; i < node->getNumChildren(); i++) {
    if (replaceInductionVariable(node,
                                 node->getChild(i),
                                 i,
                                 loopDrivingInductionVar,
                                 loopLimit,
...
void
TR_LoopVersioner::buildSpineCheckComparisonsTree(
  List<TR::TreeTop>* spineCheckTrees)
{
  ListElement<TR::TreeTop>* nextTree = spineCheckTrees->getListHead();
  ...
  while (nextTree) {
    ...
    if (performTransformation(
      ...
      TR::Node* nextComparisonNode =
        TR::Node::createif(TR::ificmpne,
                           contigArrayLength,
                           TR::Node::create(spineCheckNode, TR::iconst, 0, 0),
                           _exitGotoTarget);
      ...
      if (op == TR::SpineCHK) {
        ...
        prep = createChainedLoopEntryPrep(
          LoopEntryPrep::TEST, nextComparisonNode, prereq);
      }
...
void
TR_LoopVersioner::RemoveSpineCheck::improveLoop()
{
  ...
  if (strideShift)
    strideShiftNode = is64BitTarget
                        ? TR::Node::lconst(spineCheckNode, (int64_t)strideShift)
  ...
  node = TR::Node::create(
    is64BitTarget ? TR::lshr : TR::ishr, 2, spineIndex, spineShiftNode);
  node =
    TR::Node::create(is64BitTarget ? TR::lshl : TR::ishl, 2, node, shiftNode);
  node =
    TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, node, hdrSize);
  node =
    TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, arrayBase, node);
  ...
  if (is64BitTarget) {
    ...
    leafOffset = TR::Node::create(spineCheckNode,
                                  TR::iconst,
                                  0,
  ...
  if (strideShiftNode) {
    nodeLeafOffset = TR::Node::create(
      is64BitTarget ? TR::lshl : TR::ishl, 2, nodeLeafOffset, strideShiftNode);
  }

  node = TR::Node::create(
    is64BitTarget ? TR::aladd : TR::aiadd, 2, node, nodeLeafOffset);
  TR::TreeTop* compressTree = NULL;
  if (comp()->useCompressedPointers()) {
    compressTree = TR::TreeTop::create(
      comp(),
      TR::Node::createCompressedRefsAnchor(node->getFirstChild()),
      NULL,
      NULL);
  }
...
void
TR_LoopVersioner::buildBoundCheckComparisonsTree(
  List<TR::TreeTop>* boundCheckTrees,
  List<TR::TreeTop>* spineCheckTrees,
  bool reverseBranch)
{
  ...
  while (nextTree) {
      ...
      if (arrayLengthNode->getOpCode().isArrayLength()) {
        ...
        if (arrayObject->getOpCode().hasSymbolReference() &&
          ...
          if (invariantArrayObject) {
            ...
            arrayLengthNode =
              TR::Node::create(arrayLengthNode,
    ...
    if (isIndexInvariant) {
      nextComparisonNode =
        TR::Node::createif(TR::ificmplt,
                           indexNode->duplicateTreeForCodeMotion(),
                           TR::Node::create(boundCheckNode, TR::iconst, 0, 0),
                           _exitGotoTarget);

      if (trace())
        traceMsg(comp(),
                 "Index invariant in each iter -> Creating %p (%s)\n",
                 nextComparisonNode,
                 nextComparisonNode->getOpCode().getName());

      if (comp()->requiresSpineChecks())
        findAndReplaceContigArrayLen(
          NULL, nextComparisonNode, comp()->incVisitCount());
      ...
      if (boundCheckNode->getOpCodeValue() == TR::BNDCHK ||
        ...
        nextComparisonNode =
          TR::Node::createif(TR::ificmpge,
                             indexNode->duplicateTreeForCodeMotion(),
                             arrayLengthNode->duplicateTreeForCodeMotion(),
                             _exitGotoTarget);
      else
        nextComparisonNode =
          TR::Node::createif(TR::ificmpgt,
                             indexNode->duplicateTreeForCodeMotion(),
                             arrayLengthNode->duplicateTreeForCodeMotion(),
                             _exitGotoTarget);
      ...
      prep = createChainedLoopEntryPrep(
        LoopEntryPrep::TEST, nextComparisonNode, prep);
      ...
      if (indexChild->getOpCode().hasSymbolReference())
        ...
        while (
          indexChild->getOpCode().isAdd() || indexChild->getOpCode().isSub() ||
      ...
      while (valueChild->getOpCode().isAdd() ||
        ...
        if (valueChild->getSecondChild()->getOpCode().isLoadConst()) {
          if (valueChild->getOpCode().isAdd())
            additiveConstantInStore =
              additiveConstantInStore + valueChild->getSecondChild()->getInt();
          else
            additiveConstantInStore =
              additiveConstantInStore - valueChild->getSecondChild()->getInt();
          valueChild = valueChild->getFirstChild();
        ...
        if (changedIndexSymRefAtSomePoint ||
          ...
          if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
            ...
            TR::Node* replacementLoad = TR::Node::createWithSymRef(
              storeNode,
              comp()->il.opCodeForDirectLoad(storeNode->getDataType()),
              0,
              indexSymRef);

            if (!storeRhs->getOpCode().isLoad()) {
              int visitCount = comp()->incVisitCount();
              replaceInductionVariable(NULL,
                                       storeRhs,
                                       -1,
      ...
      if (isLoopDrivingInductionVariable || isDerivedInductionVariable) {
        ...
        loopLimit = maxValue;
        dumpOptDetails(
          comp(), "loopLimit has been adjusted to %p\n", loopLimit);
      }
      ...
      if (!duplicateIndex->getOpCode().isLoad()) {
        ...
        */
        replaceInductionVariable(
          NULL, duplicateIndex, -1, indexSymRefNum, loopLimit, visitCount);
      ...
      if (isSpecialInductionVariable) {
        nextComparisonNode = TR::Node::createif(
          TR::ifiucmpgt,
          boundCheckNode->getChild(indexChildIndex)->duplicateTree(),
          correctCheckNode->duplicateTree(),
          _exitGotoTarget);
        if (trace())
          traceMsg(comp(),
        ...
        nextComparisonNode =
          TR::Node::createif(TR::ifiucmpgt,
                             correctCheckNode,
                             arrayLengthNode->duplicateTree(),
                             _exitGotoTarget);
        if (trace())
          traceMsg(comp(),
                   "Special Induction variable added in each iter -> Creating "
...
void
TR_LoopVersioner::createRemoveBoundCheck(TR::TreeTop* boundCheckTree,
                                         LoopEntryPrep* prep,
                                         List<TR::TreeTop>* spineCheckTrees)
{
  _curLoop->_loopImprovements.push_back(
...
void
TR_LoopVersioner::RemoveBoundCheck::improveLoop()
{
  ...
  if (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) {
    ...
    TR::TreeTop* firstNewTree = TR::TreeTop::create(
      comp(),
      TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(0)),
      NULL,
      NULL);
    TR::TreeTop* secondNewTree = TR::TreeTop::create(
      comp(),
      TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(1)),
      NULL,
      NULL);
    prevTree->join(firstNewTree);
    firstNewTree->join(secondNewTree);
 ...
 */
void
TR_LoopVersioner::collectAllExpressionsToBeChecked(
  List<TR::TreeTop>* nullCheckTrees,
  List<TR::TreeTop>* divCheckTrees,
  List<TR::TreeTop>* checkCastTrees,
  List<TR::TreeTop>* arrayStoreCheckTrees,
  ...
  List<TR::Node>* comparisonTrees)
{
  ...
  if (optDetails) {
    ...
    comp()->getDebug()->clearNodeChecklist();
    comp()->getDebug()->printWithFixedPrefix(
      comp()->getOutFile(), node, 1, true, false, "\t\t");
    traceMsg(comp(), "\n");
  }
 ...
 */
bool
TR_LoopVersioner::requiresPrivatization(TR::Node* node)
{
 ...
 */
bool
TR_LoopVersioner::suppressInvarianceAndPrivatization(
...
void
TR_LoopVersioner::dumpOptDetailsCreatingTest(const char* description,
                                             TR::Node* node)
{
  dumpOptDetails(comp(),
...
void
TR_LoopVersioner::dumpOptDetailsFailedToCreateTest(const char* description,
                                                   TR::Node* node)
{
  dumpOptDetails(comp(),
 ...
 */
bool
TR_LoopVersioner::depsForLoopEntryPrep(
  TR::Node* node,
  ...
  bool canPrivatizeRootNode)
{
  ...
  if (node->isInternalPointer() ||
    ...
    if (!node->getFirstChild()->isThisPointer()) {
      ...
      TR::Node* ifacmpeqNode = TR::Node::createif(TR::ifacmpeq,
                                                  node->getFirstChild(),
    ...
    if (node->isInternalPointer() &&
      ...
      if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
        ...
        if (sig && (len > 0)) {
          ...
          cl = fe()->getClassFromSignature(
            sig, len, symRef->getOwningMethod(comp()));
        }
        ...
        if (!instanceOfReqd) {
          ...
          duplicateClassPtr =
            TR::Node::createWithSymRef(node, TR::loadaddr, 0, otherClassSymRef);
        }
      }
    }
      ...
      if (otherClassObject) {
        ...
        TR::Node* ificmpeqNode =
          TR::Node::createif(TR::ificmpeq,
                             ...
                             _exitGotoTarget);
        if (addLoopEntryPrepDep(
              LoopEntryPrep::TEST, ificmpeqNode, deps, visited) == NULL) {
        ...
        TR::Node* andConstNode =
          TR::Node::create(classFlag,
                           TR::iconst,
                           0,
                           TR::Compiler->cls.flagValueForArrayCheck(comp()));
        TR::Node* andNode =
          TR::Node::create(TR::iand, 2, classFlag, andConstNode);
        TR::Node* cmp = TR::Node::createif(
          TR::ificmpne, andNode, andConstNode, _exitGotoTarget);
    ...
    if (!indexNode) {
      if (!childInRequiredForm) {
        ...
        indexNode = TR::Node::create(
          type.isInt32() ? TR::isub : TR::lsub, 2, offset, constNode);
        if (type.isInt64())
      ...
      indexNode =
        TR::Node::create(type.isInt32() ? TR::iushr : TR::lushr,
                         2,
                         indexNode,
    ...
    if (!divisorIsNonzeroConstant) {
      ...
      if (divisor->getType().isInt64())
        ifNode = TR::Node::createif(TR::iflcmpeq,
                                    divisor,
                                    TR::Node::create(node, TR::lconst, 0, 0),
                                    _exitGotoTarget);
      else
        ifNode = TR::Node::createif(TR::ificmpeq,
                                    divisor,
                                    TR::Node::create(node, TR::iconst, 0, 0),
                                    _exitGotoTarget);
...
TR_LoopVersioner::addLoopEntryPrepDep(
...
TR::Node*
TR_LoopVersioner::findLoad(TR::Node* node,
                           TR::SymbolReference* symRef,
...
bool
TR_LoopVersioner::findStore(TR::TreeTop* start,
                            TR::TreeTop* end,
                            TR::Node* node,
                            TR::SymbolReference* symRef,
                            bool needAuxiliaryWalk,
                            bool lastTimeThrough)
{
  ...
  if (needAuxiliaryWalk) {
    ...
    if (!findStore(start->getEnclosingBlock()->getEntry(),
                   start,
                   node,
                   symRef,
                   false,
...
bool
TR_LoopVersioner::boundCheckUsesUnchangedValue(
  TR::TreeTop* bndCheckTree,
  TR::Node* node,
  TR::SymbolReference* symRef,
  TR_RegionStructure* loopStructure)
{
  ...
  if ((currentBlock->getSuccessors().size() == 1) &&
    ...
    if (findStore(bndCheckTree, currentBlock->getExit(), node, symRef, true) ||
        findStore(_loopTestBlock->getEntry(),
                  _loopTestBlock->getExit(),
                  node,
                  symRef,
                  true))
      return true;
  ...
  if (entryBlock->getSuccessors().size() == 1) {
    ...
    if (!searchForStore) {
      ...
      if ((succBlock->getSuccessors().size() == 1) &&
        ...
        if (!findStore(
              succBlock->getEntry(), succBlock->getExit(), node, symRef))
          searchForStore = true;
      }
    }
  ...
  while (flag) {
    ...
    if (cursorBlock == currentBlock) {
      ...
      if (findStore(
            cursorBlock->getEntry(), cursorBlock->getExit(), node, symRef))
        return true;
    }
    ...
    for (auto succ = cursorBlock->getSuccessors().begin();
         succ != cursorBlock->getSuccessors().end();
         ++succ) {
  ...
  while (flag) {
    ...
    for (auto pred = cursorBlock->getPredecessors().begin();
         pred != cursorBlock->getPredecessors().end();
         ++pred) {
    ...
    if (predInsideLoop) {
      ...
      for (auto succ = predInsideLoop->getSuccessors().begin();
           succ != predInsideLoop->getSuccessors().end();
           ++succ) {
        TR::Block* succBlock = (*succ)->getTo()->asBlock();
        if (blocksInRegion.find(succBlock)) {
          if (!onlySuccInsideLoop)
...
int32_t
TR_LoopVersioner::detectCanonicalizedPredictableLoops(
  TR_Structure* loopStructure,
  TR_BitVector** optSetInfo,
  int32_t bitVectorSize)
{
  ...
  if ((nodeCount / (MAX_SIZE_INCREASE_FACTOR / hotnessFactor)) >
    ...
    if (trace())
      traceMsg(comp(),
               "Failing node count %d orig %d factor %d\n",
               nodeCount,
               _origNodeCount,
               nodeCountFactor);
    return -2;
  ...
  if ((comp()->getFlowGraph()->getNodes().getSize() /
    ...
    if (trace())
      traceMsg(comp(),
               "Failing block count %d orig %d factor %d\n",
...
bool
TR_LoopVersioner::isStoreInSpecialForm(int32_t symRefNum,
                                       TR_Structure* loopStructure)
{
  if (!comp()
         ->getSymRefTab()
...
bool
TR_LoopVersioner::isStoreInRequiredForm(int32_t symRefNum,
                                        TR_Structure* loopStructure)
{
...
bool
TR_LoopVersioner::processArrayAliasCandidates()
{
  TR_ASSERT(!refineAliases(), "something wrong");
  return false;
}
void
TR_LoopVersioner::collectArrayAliasCandidates(TR::Node* node,
                                              vcount_t visitCount)
{
  TR_ASSERT(!refineAliases(), "something wrong");
}
void
TR_LoopVersioner::buildAliasRefinementComparisonTrees(
  List<TR::TreeTop>* nullCheckTrees,
  List<TR::TreeTop>* divCheckTrees,
  List<TR::TreeTop>* checkCastTrees,
  List<TR::TreeTop>* arrayStoreCheckTrees,
  TR_ScratchList<TR::Node>* comparisonTrees,
  TR::Block* exitGotoBlock)
{
  TR_ASSERT(!refineAliases(), "something wrong");
}
void
TR_LoopVersioner::initAdditionalDataStructures()
{}
void
TR_LoopVersioner::refineArrayAliases(TR_RegionStructure* r)
{
  TR_ASSERT(!refineAliases(), "something wrong");
}

void
TR_LoopVersioner::findAndReplaceContigArrayLen(TR::Node* parent,
                                               TR::Node* node,
                                               vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
...
bool
TR_LoopVersioner::isInverseConversions(TR::Node* node)
{
 ...
 */
bool
TR_LoopVersioner::initExprFromNode(Expr* expr,
                                   TR::Node* node,
                                   bool onlySearching)
{
...
static bool
containsCommaSeparated(const char* haystack, const char* needle)
{
  ...
  if (haystackLen < needleLen - 2)
    ...
    return true;
  else if (strncmp(haystack + haystackLen - needleLen + 1,
                   needle,
                   needleLen - 1) == 0)
    return true;
  else
 ...
 */
bool
TR_LoopVersioner::guardOkForExpr(TR::Node* node, bool onlySearching)
{
...
TR_LoopVersioner::makeCanonicalExpr(TR::Node* node)
{
  ...
  if (!initExprFromNode(&expr, node, onlySearching)) {
    dumpOptDetails(
      comp(), "n%un [%p] is unrepresentable\n", node->getGlobalIndex(), node);
    ...
    if (shouldAssert) {
      if (node->isNopableInlineGuard()) {
        ...
        TR_ASSERT_FATAL(
          false, "n%un [%p] is unrepresentable", node->getGlobalIndex(), node);
      }
    }
 ...
 */
const TR_LoopVersioner::Expr*
TR_LoopVersioner::substitutePrivTemps(TR::TreeTop* tt,
                                      TR::Node* node,
                                      TR::NodeChecklist* visited)
{
  if (visited->contains(node)) {
 ...
 */
TR::Node*
TR_LoopVersioner::emitExpr(const Expr* expr)
{
...
TR::Node*
TR_LoopVersioner::emitExpr(const Expr* expr, EmitExprMemo& memo)
{
  ...
  if (!op.isLoadConst() && op.hasSymbolReference()) {
    ...
    TR_ASSERT_FATAL(
      numChildren == 2, "expected if %p to have 2 children", expr);
 ...
 */
void
TR_LoopVersioner::emitPrep(LoopEntryPrep* prep, List<TR::Node>* comparisonTrees)
{
  TR_ASSERT_FATAL(
  ...
  if (prep->_kind == LoopEntryPrep::TEST) {
    ...
    TR_ASSERT_FATAL(
      insertResult.second, "_privTemps insert failed for expr %p", prep->_expr);
 ...
 */
void
TR_LoopVersioner::unsafelyEmitAllTests(
  ...
  for (auto it = preps.begin(); it != preps.end(); ++it) {
    ...
    if (prep->_kind == LoopEntryPrep::TEST) {
      ...
      if (!prep->_requiresPrivatization) {
        prep->_emitted = true;
        dumpOptDetails(
          comp(), "This prep happens to be safe (no privatization required)\n");
      }
    }
  }
}
...
void
TR_LoopVersioner::setAndIncChildren(TR::Node* node, int n, TR::Node** children)
{
  for (int i = 0; i < n; i++)
 ...
 */
void
TR_LoopVersioner::nodeWillBeRemovedIfPossible(TR::Node* node,
                                              LoopEntryPrep* prep)
{
 ...
 */
bool
TR_LoopVersioner::Expr::operator<(const Expr& rhs) const
{
  const Expr& lhs = *this;
 ...
 */
TR_LoopVersioner::LoopEntryPrep*
TR_LoopVersioner::createLoopEntryPrep(LoopEntryPrep::Kind kind,
                                      TR::Node* node,
                                      TR::NodeChecklist* visited,
                                      LoopEntryPrep* prev)
{
  ...
  if (optDetails) {
    ...
    if (prev == NULL)
      ...
      dumpOptDetails(
        comp(), "Creating %s prep for tree with prev=%p:\n", kindName, prev);
  ...
  if (visited != NULL) {
    ...
    ok = depsForLoopEntryPrep(
      node, &prep->_deps, &myVisited, canPrivatizeRootNode);
  }
 ...
 */
TR_LoopVersioner::LoopEntryPrep*
TR_LoopVersioner::createChainedLoopEntryPrep(LoopEntryPrep::Kind kind,
                                             TR::Node* node,
                                             LoopEntryPrep* prev)
{
  if (prev == NULL)
...
bool
TR_LoopVersioner::PrepKey::operator<(const PrepKey& rhs) const
{
...
void
TR_LoopVersioner::visitSubtree(TR::Node* node, TR::NodeChecklist* visited)
{
  if (visited->contains(node))
    return;
 ...
 */
TR_LoopVersioner::CurLoop::CurLoop(TR::Compilation* comp,
                                   TR::Region& memRegion,
 ...
 */
void
TR_LoopVersioner::LoopBodySearch::advance()
{
...
void
TR_LoopVersioner::LoopBodySearch::enqueueReachableSuccessorsInLoop()
{
...
bool
TR_LoopVersioner::LoopBodySearch::isBranchConstant(TR::Node* ifNode)
{
...
bool
TR_LoopVersioner::LoopBodySearch::isConstantBranchTaken(TR::Node* ifNode)
{
  TR_ASSERT_FATAL(isBranchConstant(ifNode),
...
void
TR_LoopVersioner::LoopBodySearch::enqueueReachableSuccessorsInLoopFrom(
...
void
TR_LoopVersioner::LoopBodySearch::enqueueBlockIfInLoop(TR::TreeTop* entry)
{
  enqueueBlockIfInLoop(entry->getNode()->getBlock());
}

void
TR_LoopVersioner::LoopBodySearch::enqueueBlockIfInLoop(TR::Block* block)
{
