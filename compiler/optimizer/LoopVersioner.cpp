...
int32_t TR_LoopVersioner::performWithoutDominators()
{
    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        ...
        if (!shouldOnlySpecializeLoops() && !refineAliases())
            checkCastTreesWillBeEliminated = detectInvariantTrees(
                naturalLoop, &checkCastTrees, false, &containsNonInlineGuard, reverseBranchInLoops);
        else
        ...
        if (!shouldOnlySpecializeLoops() && !refineAliases()) {
            ...
            if (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
                ...
                conditionalsWillBeEliminated = detectInvariantTrees(
                        ...
                        if ((!node->getType().isInt64()) || (profileLongParms && (valueInfo->getTopValue() == 0))) {
                            ...
                            if (comp()->requiresSpineChecks() && node->getOpCodeValue() == TR::contigarraylength
                                ...
                                dumpOptDetails(comp(),
                                    ...
                                    node,
                                    ((int32_t)(
                                        valueInfo->getTopProbability() * ((float)valueInfo->getTotalFrequency()))),
    ...
    SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    if (!_curLoop->_privatizationOK && _curLoop->_privatizationsRequested) {
        ...
        //
        LoopBodySearch search(comp(), _curLoop->_memRegion, whileLoop, &_curLoop->_optimisticallyRemovableNodes,
    ...
    while (currComparisonBlock) {
        ...
        if (isTest) {
            if (currCriticalEdgeBlock != NULL) {
                TR_StructureSubGraphNode* criticalEdgeNode = new (_cfg->structureRegion())
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if n%un [%p] is conditional\n",
            ...
            if (conditionalNode->isVersionableIfWithMaxExpr() || conditionalNode->isVersionableIfWithMinExpr()) {
                ...
                if (isInductionVar
                    ...
                    extraIter
                        = TR::Node::create(TR::iand, 2, extraIter, TR::Node::create(parent, TR::iconst, 0, condValue));
                ...
                if (indexNodeOccursAsSecondChild) {
                    if (!reverseBranch)
                        duplicateComparisonNode = TR::Node::createif(conditionalNode->getOpCodeValue(),
                            conditionalNode->getFirstChild()->duplicateTree(comp()), duplicateIndexNode,
                            _exitGotoTarget);
                    else
                        duplicateComparisonNode
                            = TR::Node::createif(conditionalNode->getOpCode().getOpCodeForReverseBranch(),
                                conditionalNode->getFirstChild()->duplicateTree(comp()), duplicateIndexNode,
                                _exitGotoTarget);
                } else {
                    if (!reverseBranch)
                        duplicateComparisonNode
                            = TR::Node::createif(conditionalNode->getOpCodeValue(), duplicateIndexNode,
                                conditionalNode->getSecondChild()->duplicateTree(comp()), _exitGotoTarget);
                    else
                        duplicateComparisonNode = TR::Node::createif(
                            conditionalNode->getOpCode().getOpCodeForReverseBranch(), duplicateIndexNode,
                            conditionalNode->getSecondChild()->duplicateTree(comp()), _exitGotoTarget);
                }
            } else {
                if (!reverseBranch)
        ...
        if (isIndexInvariant) {
            ...
            if (isLoopDrivingInductionVariable || isDerivedInductionVariable) {
                ...
                extraIter = TR::Node::create(
                    TR::iand, 2, extraIter, TR::Node::create(boundCheckNode, TR::iconst, 0, condValue));
                numIterations = TR::Node::create(TR::iadd, 2, numIterations, extraIter);
                numIterations = TR::Node::create(TR::imul, 2, numIterations, incrJNode);
                    ...
                    if (!indVarOccursAsSecondChildOfSub) {
                        ...
                        if (isLoopDrivingInductionVariable) {
                            if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt)
                                || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge))
                                nextComparisonNode = TR::Node::createif(TR::ificmple, correctCheckNode,
                                    TR::Node::create(boundCheckNode, TR::iconst, 0, -1), _exitGotoTarget);
                        }
    ...
    TR::TreeTop* bndCheckTree, TR::Node* node, TR::SymbolReference* symRef, TR_RegionStructure* loopStructure)
{
    ...
    while (flag) {
        ...
        if (predInsideLoop) {
            TR::Block* onlySuccInsideLoop = NULL;
            for (auto succ = predInsideLoop->getSuccessors().begin(); succ != predInsideLoop->getSuccessors().end();
                 ++succ) {
                TR::Block* succBlock = (*succ)->getTo()->asBlock();
                if (blocksInRegion.find(succBlock)) {
                    if (!onlySuccInsideLoop)
