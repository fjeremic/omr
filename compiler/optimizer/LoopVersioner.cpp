...
void printNode(TR::Node *node, TR_BitVector &visitedNodes, int32_t indentation)
   ...
   if (visitedNodes.isSet(node->getGlobalIndex()))
      ...
      //comp()->getDebug()->printNodeInfo(comp()->getDebug()->getOutFile(),
node);
...
const char *TR_LoopSpecializer::optDetailString() const throw() {
  return "O^O LOOP SPECIALIZER: ";
}

TR_LoopVersioner::TR_LoopVersioner(TR::OptimizationManager *manager,
                                   bool onlySpecialize, bool refineAliases)
    : TR_LoopTransformer(manager),
      ...
      _visitedNodes(comp()->getNodeCount() /*an estimate*/, trMemory(),
                    heapAlloc, growable),
...
int32_t TR_LoopVersioner::performWithDominators() {
  if (trace()) traceMsg(comp(), "Building Control Dependencies\n");
...
bool TR_LoopVersioner::loopIsWorthVersioning(TR_RegionStructure *naturalLoop) {
  ...
  if (entryBlock->isCold()) {
    if (trace())
      traceMsg(comp(),
               "loopIsWorthVersioning returning false for cold block\n");
    return false;
  }

  // if aggressive loop versioner flag is set then run at any optlevel,
  // otherwise only at warm or less
  ...
  if (aggressive || comp()->getMethodHotness() <= warm) {
    if (naturalLoop->getParent()) {
      TR_StructureSubGraphNode *loopNode =
          naturalLoop->getParent()->findNodeInHierarchy(
              naturalLoop->getNumber());
      if (loopNode && (loopNode->getPredecessors().size() == 1)) {
        TR_StructureSubGraphNode *loopInvariantNode = toStructureSubGraphNode(
            loopNode->getPredecessors().front()->getFrom());
        if (loopInvariantNode->getStructure()->asBlock() &&
            loopInvariantNode->getStructure()
                ->asBlock()
                ->isLoopInvariantBlock()) {
          TR::Block *loopInvariantBlock =
              loopInvariantNode->getStructure()->asBlock()->getBlock();
          static const char *unimportantLoopCountStr =
              feGetEnv("TR_UnimportantLoopCountThreshold");
          int32_t unimportantLoopCountThreshold =
              unimportantLoopCountStr ? atoi(unimportantLoopCountStr) : 2;

          if ((unimportantLoopCountThreshold *
               loopInvariantBlock->getFrequency()) >
              entryBlock->getFrequency())  // loop does not even run twice
          {
            if (trace())
              traceMsg(comp(),
                       "loopIsWorthVersioning returning false based on "
                       "LoopCountThreshold\n");
            return false;
          }
        }
...
int32_t TR_LoopVersioner::performWithoutDominators() {
  ...
  // From this point on stack memory allocations will die when the function
  // returns
  ...
  _seenDefinedSymbolReferences = new (trStackMemory())
      TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc, growable);
  _writtenAndNotJustForHeapification = new (trStackMemory())
      TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc, growable);
  _additionInfo = new (trStackMemory())
      TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc, growable);

  _whileIndex = 0;
  _counter = 0;
  _guardedCalls.deleteAll();
  for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL;
       nextWhileLoop = whileLoopsIt.getNext()) {
    ...
    TR_RegionStructure *naturalLoop = nextWhileLoop->asRegion();
    TR::Block *entryBlock = naturalLoop->getEntryBlock();
    if (!loopIsWorthVersioning(naturalLoop)) continue;
    ...
    _containsUnguardedCall = false;
    _loopTransferDone = false;
    _unchangedValueUsedInBndCheck = new (trStackMemory())
        TR_BitVector(comp()->getNodeCount(), trMemory(), stackAlloc);

    _checksInDupHeader.deleteAll();

    TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),
              "Loop versioner, expecting natural loop");

    if (trace()) comp()->dumpMethodTrees("Trees after this versioning");
    ...
    TR_ScratchList<TR_NodeParentSymRefWeightTuple>
        invariantTranslationNodesList(trMemory());
    TR_ScratchList<TR::Node> arrayAccesses(trMemory());
    TR_ScratchList<TR_NodeParentBlockTuple> arrayLoadCandidates(trMemory());
    TR_ScratchList<TR_NodeParentBlockTuple> arrayMemberLoadCandidates(
        trMemory());
    TR_BitVector disqualifiedRefinementCandidates(1, trMemory(), stackAlloc,
                                                  growable);
    ...
    if (comp()->getOption(TR_DisableInvariantCodeMotion) ||
    ...
    if (detectCanonicalizedPredictableLoops(naturalLoop, NULL, -1) <
        0)  // if there was a problem detecting, move to next iteration of the
            // loop
    {
      _curLoop = NULL;
      continue;
    ...
    bool nullChecksMayBeEliminated = detectChecksToBeEliminated(
        naturalLoop, &nullCheckedReferences, &nullCheckTrees, &numIndirections,
        &boundCheckTrees, &spineCheckTrees, &numDimensions, &conditionalTrees,
        &divCheckTrees, &awrtbariTrees, &checkCastTrees, &arrayStoreCheckTrees,
        &specializedInvariantNodes, invariantNodes,
    ...
    if (nullChecksMayBeEliminated && !refineAliases() &&
        !shouldOnlySpecializeLoops())
      nullChecksWillBeEliminated =
          detectInvariantChecks(&nullCheckedReferences, &nullCheckTrees);
    else {
    ...
    if (!refineAliases())
      specializedNodesWillBeEliminated =
          detectInvariantSpecializedExprs(&specializedInvariantNodes);
    else
    ...
    if (!refineAliases() && invariantNodes && !invariantNodes->isEmpty())
      invariantNodesWillBeEliminated =
          detectInvariantNodes(invariantNodes, &invariantTranslationNodesList);
    else {
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases())
      boundChecksWillBeEliminated =
          detectInvariantBoundChecks(&boundCheckTrees);
    else
      boundCheckTrees.deleteAll();

    bool spineChecksWillBeEliminated = false;

    if (false && shouldOnlySpecializeLoops() && comp()->requiresSpineChecks() &&
        !refineAliases())
      spineChecksWillBeEliminated =
          detectInvariantSpineChecks(&spineCheckTrees);
    else
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases())
      checkCastTreesWillBeEliminated =
          detectInvariantTrees(naturalLoop, &checkCastTrees, false,
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases())
      arrayStoreCheckTreesWillBeEliminated =
          detectInvariantArrayStoreChecks(&arrayStoreCheckTrees);
    else
    ...
    if (refineAliases()) versionToRefineAliases = processArrayAliasCandidates();
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases()) {
      ...
      if (comp()->getOption(TR_EnableAggressiveLoopVersioning)) {
        if (trace())
          traceMsg(comp(),
                   "aggressiveLoopVersioning: raising hotnessThreshold for "
                   "conditionalsWillBeEliminated\n");
        hotnessThreshold =
            maxHotness;  // threshold which can't be matched by the > operator
      }
      ...
      if (((debug("nullCheckVersion") ||
           ...
           nullChecksWillBeEliminated) ||
          boundChecksWillBeEliminated || divChecksWillBeEliminated ||
          awrtBarisWillBeEliminated || checkCastTreesWillBeEliminated ||
          arrayStoreCheckTreesWillBeEliminated ||
        ...
        conditionalsWillBeEliminated =
            detectInvariantTrees(naturalLoop, &conditionalTrees, true,
                                 &containsNonInlineGuard, reverseBranchInLoops);
      } else {
        conditionalsWillBeEliminated =
            detectInvariantTrees(naturalLoop, &conditionalTrees, false,
                                 &containsNonInlineGuard, reverseBranchInLoops);
        if (containsNonInlineGuard) {
          _neitherLoopCold = true;
          _conditionalTree =
              conditionalTrees.getListHead()->getData()->getNode();
        }
      }
    } else {
    ...
    if ((versionToRefineAliases || !refineAliases()) &&
        (((debug("nullCheckVersion") ||
           comp()->cg()->performsChecksExplicitly() ||
           (comp()->getMethodHotness() >= veryHot)) &&
          (naturalLoop->containsOnlyAcyclicRegions() || !_loopTransferDone) &&
          nullChecksWillBeEliminated) ||
         (_asyncCheckTree && shouldOnlySpecializeLoops()) ||
         boundChecksWillBeEliminated || spineChecksWillBeEliminated ||
         conditionalsWillBeEliminated || divChecksWillBeEliminated ||
         awrtBarisWillBeEliminated || checkCastTreesWillBeEliminated ||
         arrayStoreCheckTreesWillBeEliminated ||
         specializedNodesWillBeEliminated || invariantNodesWillBeEliminated ||
         versionToRefineAliases || _containsGuard)) {
      // if (shouldOnlySpecializeLoops())
      //   printf("Reached here for %s\n", comp()->signature());
      somethingChanged = true;
      versionedThisLoop = true;
      versionNaturalLoop(naturalLoop, &nullCheckedReferences, &nullCheckTrees,
                         &boundCheckTrees, &spineCheckTrees, &conditionalTrees,
                         &divCheckTrees, &awrtbariTrees, &checkCastTrees,
                         &arrayStoreCheckTrees, &specializedInvariantNodes,
                         invariantNodes, &invariantTranslationNodesList,
                         &whileLoops, &clonedInnerWhileLoops,
                         skipAsyncCheckRemoval, reverseBranchInLoops);
      _exitGotoTarget = NULL;
    }
    ...
    if (versionedThisLoop) {
      ...
      if (refineAliases()) refineArrayAliases(naturalLoop);

      TR_BitVector *inductionVars = new (trStackMemory())
          TR_BitVector(symRefCount, trMemory(), stackAlloc);
      ListElement<int32_t> *versionableInductionVar =
          _versionableInductionVariables.getListHead();
      while (versionableInductionVar) {
        TR::SymbolReference *inductionVar = comp()->getSymRefTab()->getSymRef(
            *(versionableInductionVar->getData()));
        // printf("Induction variable %d in loop %d in register for %s\n",
        // inductionVar->getReferenceNumber(), naturalLoop->getNumber(),
        // comp()->signature());
        inductionVars->set(inductionVar->getReferenceNumber());
        TR_RegisterCandidate *inductionCandidate =
            comp()->getGlobalRegisterCandidates()->findOrCreate(inductionVar);
        // dumpOptDetails(comp(), "%s Adding auto %d as a global register
        // candidate in loop %d\n", OPT_DETAILS_LOOP_VERSIONER,
        // inductionCandidate->getSymbolReference()->getReferenceNumber(),
        // naturalLoop->getNumber());
        inductionCandidate->addAllBlocksInStructure(naturalLoop, comp(),
                                                    trace() ? "auto" : NULL);
      ...
      for (v = naturalLoop->getFirstInductionVariable(); v; v = v->getNext()) {
        ...
        for (symRefNumber = symRefTab->getIndexOfFirstSymRef();
             symRefNumber < symRefCount; symRefNumber++) {
        ...
        if (inductionSymRef &&
            !inductionVars->get(inductionSymRef->getReferenceNumber())) {
          // printf("CP Induction variable %d in natural loop %d in register for
          // %s\n", inductionSymRef->getReferenceNumber(),
          // naturalLoop->getNumber(), comp()->signature());
          TR_RegisterCandidate *inductionCandidate =
              comp()->getGlobalRegisterCandidates()->findOrCreate(
                  inductionSymRef);
          // dumpOptDetails(comp(), "%s Adding auto %d as a global register
          // candidate in loop %d\n", OPT_DETAILS_LOOP_VERSIONER,
          // inductionCandidate->getSymbolReference()->getReferenceNumber(),
      ...
      ListIterator<TR_Structure> clonedInnerWhileLoopsIt(
          &clonedInnerWhileLoops);
      ...
      for (; clonedInnerWhileLoop != NULL;
           clonedInnerWhileLoop = clonedInnerWhileLoopsIt.getNext()) {
        prevClonedInnerLoop =
            whileLoops.addAfter(clonedInnerWhileLoop, prevClonedInnerLoop);
      }
  ...
  if (!_virtualGuardInfo.isEmpty()) performLoopTransfer();
  ...
  if (somethingChanged) requestOpt(OMR::andSimplification);
  ...
  if (_invalidateAliasSets) optimizer()->setAliasSetsAreValid(false);
...
void TR_LoopVersioner::performLoopTransfer() {
  ...
  for (vgInfo = _virtualGuardInfo.getFirst(); vgInfo;
       vgInfo = vgInfo->getNext()) {
    ListIterator<VirtualGuardPair> guardsIt(&vgInfo->_virtualGuardPairs);
    VirtualGuardPair *nextVirtualGuard;
    for (nextVirtualGuard = guardsIt.getFirst(); nextVirtualGuard != NULL;
         nextVirtualGuard = guardsIt.getNext()) {
      ...
      if (hotGuard->getOpCode().isIf() &&
        ...
        hotGuardBlock->changeBranchDestination(
            coldGuard->getBranchDestination(), cfg);
        dumpOptDetails(
            comp(),
            "loop transfer, changed target of guard [%p] in [%d] to [%d]\n",
            hotGuard, hotGuardBlock->getNumber(),
            coldGuard->getBranchDestination()
                ->getNode()
                ->getBlock()
                ->getNumber());
        const char *debugCounter = TR::DebugCounter::debugCounterName(
            comp(), "loopVersioner.transfer/(%s)/%s/origin=block_%d",
            comp()->signature(),
            comp()->getHotnessName(comp()->getMethodHotness()),
            hotGuardBlock->getNumber());
        if (comp()->getOptions()->dynamicDebugCounterIsEnabled(debugCounter)) {
          TR::Block *newBlock = hotGuardBlock->splitEdge(
              hotGuardBlock,
              coldGuard->getBranchDestination()->getNode()->getBlock(), comp(),
              NULL, false);
          TR::DebugCounter::prependDebugCounter(
              comp(), debugCounter, newBlock->getEntry()->getNextTreeTop());
        }
      }
    }
  }
...
//
TR::Block *TR_LoopVersioner::createClonedHeader(TR::Block *origHeader,
                                                TR::TreeTop **endTree) {
  ...
  //
  for (auto e = origHeader->getSuccessors().begin();
       e != origHeader->getSuccessors().end(); ++e) {
    removedEdges.add(*e);
    TR::Block *dest = toBlock((*e)->getTo());
    _cfg->addEdge(TR::CFGEdge::createEdge(clonedHeader, dest, trMemory()));
  }

  for (auto e = origHeader->getExceptionSuccessors().begin();
       e != origHeader->getExceptionSuccessors().end(); ++e) {
    removedEdges.add(*e);
    TR::Block *dest = toBlock((*e)->getTo());
    _cfg->addEdge(
        TR::CFGEdge::createExceptionEdge(clonedHeader, dest, trMemory()));
  }
...
TR::Node *TR_LoopVersioner::createSwitchNode(TR::Block *clonedHeader,
                                             TR::SymbolReference *tempSymRef,
                                             int32_t numCase) {
  TR::Node *switchNode = TR::Node::create(
      clonedHeader->getFirstRealTreeTop()->getNode(), TR::lookup, numCase + 2);
  TR::Node *loadTemp =
      TR::Node::createWithSymRef(switchNode, TR::iload, 0, tempSymRef);
  switchNode->setAndIncChild(0, loadTemp);

  for (int32_t i = 0; i < numCase + 1; i++) {
    TR::Node *caseNode =
        TR::Node::createCase(switchNode, clonedHeader->getEntry(), i);
    switchNode->setAndIncChild(i + 1, caseNode);
  }
  return switchNode;
}

TR::Block *TR_LoopVersioner::createEmptyGoto(TR::Block *source, TR::Block *dest,
                                             TR::TreeTop *endTree) {
  ...
  TR::Block *gotoBlock = TR::Block::createEmptyBlock(
      destEntry->getNode(), comp(), dest->getFrequency(), dest);
...
bool TR_LoopVersioner::detectInvariantChecks(
    ...
    if (!isNullCheckReferenceInvariant &&
        ...
        ((node->getData()->getSymbolReference()->getSymbol()->isAuto() &&
         ...
         (node->getData()->getOpCode().isLoadIndirect() &&
          !_seenDefinedSymbolReferences->get(
              node->getData()->getSymbolReference()->getReferenceNumber()) &&
...
bool TR_LoopVersioner::detectInvariantArrayStoreChecks(
          ...
          if (addressNode->getOpCode().isArrayRef()) {
            ...
            if (!childOfAddressNode->isInternalPointer() &&
                (!childOfAddressNode->getOpCode().hasSymbolReference() ||
                 (!childOfAddressNode->getSymbolReference()
                       ->getSymbol()
...
bool TR_LoopVersioner::detectInvariantSpecializedExprs(
    List<TR::Node> *profiledExprs) {
...
TR::Node *nodeTreeGetFirstOpCode(TR::Node *n, TR::ILOpCodes op) {
  if (n->getOpCodeValue() == op) return n;
  ...
  for (; nextNode;) {
    ...
    if (trace())
      traceMsg(comp(), "Looking at node %p parent %p\n\n", node,
               nextNode->getData()->_parent);
    bool isNodeInvariant = isExprInvariant(node);
    // while not technically true, computeCC and overflow compares need to have
    // their children intact and so they can't be marked as invariant a better
...
TR::Node *TR_LoopVersioner::findCallNodeInBlockForGuard(TR::Node *node) {
  ...
  while (tt) {
    ...
    if (ttNode->getOpCodeValue() == TR::BBEnd) break;
    ...
    if (ttNode->getOpCode().isCall()) {
      if (ttNode->getNumChildren() > 0) {
        ...
        if (inlinedCallIndex < comp()->getNumInlinedCallSites()) {
          TR_InlinedCallSite &ics =
              comp()->getInlinedCallSite(inlinedCallIndex);
          if ((ics._byteCodeInfo.getByteCodeIndex() == bcIndex) &&
            ...
            //   printf("Located tree in new routine by jumping over some
            //   trees\n");
            // fflush(stdout);
            return ttNode;
          }
...
bool TR_LoopVersioner::detectInvariantTrees(
    TR_RegionStructure *whileLoop, List<TR::TreeTop> *trees,
    bool onlyDetectHighlyBiasedBranches, bool *containsNonInlineGuard,
  ...
  for (; nextTree;) {
    ...
    if (trace())
      traceMsg(comp(), "guard node %p %d\n", node,
               onlyDetectHighlyBiasedBranches);
    ...
    if (node->getOpCode().isIf() &&
      ...
      if (nextBlock->getEdge(targetBlock)->getFrequency() <=
              MAX_COLD_BLOCK_COUNT + 1 &&
          nextBlock->getFrequency() >= HIGH_FREQ_THRESHOLD &&
          static_cast<double>(nextBlock->getNextBlock()->getFrequency()) /
                  nextBlock->getFrequency() >=
              0.8) {
        if (trace()) traceMsg(comp(), "node %p is highly biased\n", node);
        highlyBiasedBranch = true;
      } else if (node->isVersionableIfWithMaxExpr() ||
                 node->isVersionableIfWithMinExpr() ||
                 node->isMaxLoopIterationGuard()) {
        if (trace()) traceMsg(comp(), "node %p is versionable If\n", node);
        highlyBiasedBranch = true;
      }
#ifdef J9_PROJECT_SPECIFIC
      else if (comp()->hasIntStreamForEach() &&
               ((targetBlock->isCold() &&
                 !nextBlock->getNextBlock()->isCold()) ||
                (!targetBlock->isCold() &&
                 nextBlock->getNextBlock()->isCold()))) {
        highlyBiasedBranch = true;
      }
    ...
    if (onlyDetectHighlyBiasedBranches) {
      if (node->isTheVirtualGuardForAGuardedInlinedCall()) {
        ...
        if (!includeComparisonTree) {
          ...
          if (callNode &&
              blocksInWhileLoop.find(
                  node->getBranchDestination()->getEnclosingBlock())) {
      ...
      if (includeComparisonTree) {
        ...
        if (onlyNonInlineGuardConditional) {
          ...
          for (; nextTempTree != nextTree;
               nextTempTree = nextTempTree->getNextElement()) {
            if (nextTempTree->getData() == onlyNonInlineGuardConditional) {
              ...
              if (nextTempTree == prevTree) prevTree = prevTempTree;
    ...
    if (includeComparisonTree) {
      ...
      if (onlyDetectHighlyBiasedBranches && (thisChild || nextRealNode) &&
          node->isTheVirtualGuardForAGuardedInlinedCall() &&
          !node->isHCRGuard() && !node->isBreakpointGuard()) {
        if (!guardInfo) guardInfo = comp()->findVirtualGuardInfo(node);
        // TR::Node *thisChild = NULL;
        ...
        if (guardInfo->getKind() == TR_DummyGuard ||
            !guardInfo->getInnerAssumptions()
                 .isEmpty())  // inner assumption may have receivers other than
                              // 'thisChild'
          ...
          if (!b) {
            ...
            /*
               ...
               {
               visitCount = comp()->incVisitCount();
               for (int32_t childNum=0;childNum < node->getNumChildren();
            childNum++)
                  {
        ...
        for (int32_t childNum = 0; childNum < node->getNumChildren();
          ...
          if (!isExprInvariant(node->getChild(childNum))) {
            if (node->isVersionableIfWithMaxExpr() ||
                node->isVersionableIfWithMinExpr()) {
              TR::Node *child = node->getChild(childNum);

              while (child->getOpCode().isAdd() || child->getOpCode().isSub() ||
                     child->getOpCode().isMul()) {
                if (child->getSecondChild()->getOpCode().isLoadConst())
                  child = child->getFirstChild();
                else {
                  bool isSecondChildInvariant =
                      isExprInvariant(child->getSecondChild());
                  if (isSecondChildInvariant)
                    child = child->getFirstChild();
                  else {
                    bool isFirstChildInvariant =
                        isExprInvariant(child->getFirstChild());
              ...
              if (!child || !child->getOpCode().isLoadVarDirect()) {
                ...
                while (versionableInductionVar) {
                  ...
                  versionableInductionVar =
                      versionableInductionVar->getNextElement();
                }
        ...
        if (!onlyDetectHighlyBiasedBranches && node->getOpCode().isBranch()) {
          ...
          if (prevTree) {
            ...
            if (!_postDominators ||
              ...
              if (trace())
                traceMsg(comp(),
                         "Keeping invariant branch  %p (%s) in block_%d\n",
                         node, node->getOpCode().getName(),
                         nextTree->getData()->getEnclosingBlock()->getNumber());
...
TR::Node *TR_LoopVersioner::isDependentOnInvariant(TR::Node *useNode) {
  ...
  if (!useDefInfo) return NULL;

  uint16_t useIndex = useNode->getUseDefIndex();
  if (!useIndex || !useDefInfo->isUseIndex(useIndex)) return NULL;
  ...
  if (cursor < useDefInfo->getFirstRealDefIndex()) return NULL;

  TR_ValueNumberInfo *valueNumberInfo = optimizer()->getValueNumberInfo();
  int32_t firstDefValueNumber =
      valueNumberInfo->getValueNumber(useDefInfo->getNode(cursor));
  if (trace())
    traceMsg(comp(),
             "Definition Counts for node [%p] is %d inside "
             "isDependentOnInvariant \n",
                  ...
                  //_writtenExactlyOnce.ValueAt(useNode->getSymbolReference()->getReferenceNumber()))
  {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (defIndex < useDefInfo->getFirstRealDefIndex()) return NULL;
      ...
      if (trace())
        traceMsg(
            comp(),
            "Definition node [%p] value number %d and firstValueNumber %d  \n",
            defNode, valueNumber, firstDefValueNumber);

      if (valueNumber != firstDefValueNumber) return NULL;

      TR::Node *child = defNode->getFirstChild();
      bool isChildInvariant = isExprInvariant(child);

      if (!isChildInvariant) return NULL;

      if (child && child->getOpCode().hasSymbolReference()) childNode = child;
    }
  }
...
TR::Node *TR_LoopVersioner::isDependentOnInductionVariable(
    TR::Node *useNode, bool noArithmeticAllowed, bool &isIndexChildMultiplied,
    TR::Node *&mulNode, TR::Node *&strideNode,
  ...
  if (useDefInfo->getUseDef(defs, useIndex) && (defs.PopulationCount() == 1) &&
      _writtenExactlyOnce.ValueAt(
          useNode->getSymbolReference()->getReferenceNumber())) {
    TR_UseDefInfo::BitVector::Cursor cursor(defs);
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      int32_t defIndex = cursor;
      if (defIndex < useDefInfo->getFirstRealDefIndex()) return NULL;
      ...
      while (!noArithmeticAllowed &&
             (child->getOpCode().isAdd() || child->getOpCode().isSub() ||
              child->getOpCode().isMul())) {
        ...
        if (child->getSecondChild()->getOpCode().isLoadConst()) {
          if (isIndexChildMultiplied) strideNode = child->getSecondChild();
          child = child->getFirstChild();
        } else {
          bool isSecondChildInvariant =
              isExprInvariant(child->getSecondChild());
          if (isSecondChildInvariant) {
            ...
            if (isFirstChildInvariant) {
              ...
              if (isIndexChildMultiplied) strideNode = child->getFirstChild();
              child = child->getSecondChild();
            } else
              return NULL;
          }
...
bool TR_LoopVersioner::isVersionableArrayAccess(TR::Node *indexChild) {
  ...
  if (!indexChild->getOpCode().hasSymbolReference()) {
    while (indexChild->getOpCode().isAdd() || indexChild->getOpCode().isSub() ||
      ...
      if (indexChild->getOpCode().isSub())
        ...
        if (mulNodeFound) goodAccess = false;
        mulNodeFound = true;
      } else if (indexChild->getOpCode().isAdd()) {
        if (addNodeFound) goodAccess = false;
        addNodeFound = true;
      }
      if (indexChild->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        bool isSecondChildInvariant =
            isExprInvariant(indexChild->getSecondChild());
        if (isSecondChildInvariant) {
          indexChild = indexChild->getFirstChild();
        } else {
          bool isFirstChildInvariant =
              isExprInvariant(indexChild->getFirstChild());
          if (isFirstChildInvariant) {
  ...
  if (mulNodeFound) return goodAccess;

  return true;
}

bool TR_LoopVersioner::detectInvariantBoundChecks(
  ...
  for (; nextTree;) {
    ...
    TR::Node *boundNode = (node->getOpCodeValue() == TR::BNDCHKwithSpineCHK)
                              ? node->getChild(2)
                              : node->getFirstChild();
    ...
    // dumpOptDetails(comp(), "maxBound %p (invariant %d) for BNDCHK node %p
    // loop condition invariant %d\n", node->getFirstChild(),
    // isMaxBoundInvariant, node, _loopConditionInvariant);
    bool isLoopDrivingInductionVariable = false;
    bool isDerivedInductionVariable = false;
    if (isMaxBoundInvariant) {
      indexChild = (node->getOpCodeValue() == TR::BNDCHKwithSpineCHK)
                       ? node->getChild(3)
                       : node->getSecondChild();

      isIndexInvariant = isExprInvariant(indexChild);
      if (!isIndexInvariant && _loopConditionInvariant &&
          (node->getOpCodeValue() == TR::BNDCHK ||
           node->getOpCodeValue() == TR::BNDCHKwithSpineCHK)) {
        if (indexChild->getOpCode().hasSymbolReference())
          indexSymRef = indexChild->getSymbolReference();
        else {
          bool goodAccess = isVersionableArrayAccess(indexChild);
          while (indexChild->getOpCode().isAdd() ||
                 indexChild->getOpCode().isSub() ||
        ...
        while (indexSymRef && changedIndexSymRef) {
          ...
          if (!isInductionVariable /* &&
            ...
            if (_loopTestTree &&
                ...
                ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt) ||
                 ...
                 (_loopTestTree->getNode()->getOpCodeValue() ==
                  TR::ificmple))) {
              TR::Symbol *symbolInCompare = NULL;
              TR::Node *childInCompare =
                  _loopTestTree->getNode()->getFirstChild();
              while (childInCompare->getOpCode().isAdd() ||
              ...
              if (childInCompare->getOpCode().hasSymbolReference()) {
                symbolInCompare =
                    childInCompare->getSymbolReference()->getSymbol();
                if (!symbolInCompare->isAutoOrParm()) symbolInCompare = NULL;
              }

              if (symbolInCompare) {
                TR_InductionVariable *v;
                for (v = _currentNaturalLoop->getFirstInductionVariable(); v;
                     v = v->getNext()) {
                  if ((v->getLocal() == indexSymRef->getSymbol()) &&
                      (v->getLocal() == symbolInCompare) &&
                      (v->getLocal()->getDataType() == TR::Int32) &&
                      (v->isSigned())) {
                    if ((v->getIncr()->getLowInt() ==
                         v->getIncr()->getHighInt()) &&
          ...
          if (!isInductionVariable) {
            ...
            indexChild = isDependentOnInductionVariable(
                indexChild, changedIndexSymRefAtSomePoint,
                isIndexChildMultiplied, mulNode, strideNode,
                indVarOccursAsSecondChildOfSub);
            if (!indexChild || !indexChild->getOpCode().hasSymbolReference() ||
                !indexChild->getSymbolReference()
                     ->getSymbol()
                     ->isAutoOrParm() ||
    ...
    if (isInductionVariable && indexSymRef) {
      if (!boundCheckUsesUnchangedValue(nextTree->getData(), indexChild,
                                        indexSymRef, _currentNaturalLoop)) {
      ...
      if (isDerivedInductionVariable &&
          !_hasPredictableExits->get(indexSymRef->getReferenceNumber())) {
        isInductionVariable = false;
    ...
    if (isIndexInvariant && _checksInDupHeader.find(nextTree->getData()))
      isIndexInvariant = false;
...
bool TR_LoopVersioner::detectInvariantSpineChecks(
...
bool TR_LoopVersioner::detectInvariantDivChecks(
    List<TR::TreeTop> *divideCheckTrees) {
  bool foundInvariantChecks = false;
    ...
    if ((node->getFirstChild()->getOpCodeValue() == TR::idiv) ||
      ...
      if (!isDivisorInvariant) {
        ...
        if (divisorChild->getOpCode().hasSymbolReference())
           ...
           {
           while ((divisorChild->getOpCode().isAdd() ||
        divisorChild->getOpCode().isSub()) &&
        ...
        if (indexSymRef)
           ...
           int32_t symRefNum = indexSymRef->getReferenceNumber();
           ListElement<int32_t> *versionableInductionVar =
        _versionableInductionVariables.getListHead(); while
        (versionableInductionVar)
              {
              if (symRefNum == *(versionableInductionVar->getData()))
                 {
...
bool TR_LoopVersioner::isDependentOnAllocation(TR::Node *useNode,
                                               int32_t recursionDepth) {
  if (recursionDepth < 0) return false;

  TR_UseDefInfo *useDefInfo = optimizer()->getUseDefInfo();
  if (!useDefInfo) return false;

  uint16_t useIndex = useNode->getUseDefIndex();
  if (!useIndex || !useDefInfo->isUseIndex(useIndex)) return false;
  ...
  if (useDefInfo->getUseDef(defs, useIndex)) {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (trace()) {
        traceMsg(comp(), "use %p child %p def %p rec %d\n", useNode, child,
                 defNode, recursionDepth);
        traceMsg(comp(), "new %d non new %d\n", pointsToNew, pointsToNonNew);
      }

      bool heapificationStore = defNode->getOpCodeValue() == TR::astore &&
                                defNode->isHeapificationStore();
    ...
    if (!pointsToNew || pointsToNonNew) return false;
    return true;
  }
...
bool TR_LoopVersioner::detectInvariantAwrtbaris(
    List<TR::TreeTop> *awrtbariTrees) {
  if (!awrtbariTrees->getListHead()) return false;
  ...
  for (; nextTree;) {
    ...
    if (node->getOpCodeValue() != TR::awrtbari) node = node->getFirstChild();

    if (trace()) traceMsg(comp(), "base invariant 0 in %p\n", node);

    if (node->getOpCodeValue() == TR::awrtbari) {
      if (trace()) traceMsg(comp(), "base invariant 1 in %p\n", node);
      ...
      if (baseChild->getOpCode().hasSymbolReference() &&
        ...
        if (isBaseInvariant) {
          if (_checksInDupHeader.find(nextTree->getData())) {
            ...
            //
            // recursionDepth cannot be changed to > 1 because isExprInvariant
            // used inside isDependentOnAllocation may not give the right answer
            // if the expr is outside the loop isExprInvariant is relevant
            // mostly in the context of a loop is my worry
            //
...
bool opCodeIsHoistable(TR::Node *node, TR::Compilation *comp) {
  ...
  // CAREFUL: This should probably be a whitelist of side-effect free operators.
  // If we move effectful stuff, things may explode. Boom is bad.
  hasEffects = (opCode.isCall() || opCodeValue == TR::New ||
                opCodeValue == TR::newarray || opCodeValue == TR::anewarray ||
                opCodeValue == TR::multianewarray) ||
               symReference->isUnresolved() ||
               symReference->getSymbol()->isLocalObject() ||
               (symReference->getSymbol()->isArrayShadowSymbol() &&
                comp->requiresSpineChecks());
  return !hasEffects;
}

bool TR_LoopVersioner::isExprInvariant(TR::Node *node,
                                       bool ignoreHeapificationStore) {
  _visitedNodes.empty();
  return isExprInvariantRecursive(node, ignoreHeapificationStore);
}

bool TR_LoopVersioner::isExprInvariantRecursive(TR::Node *node,
                                                bool ignoreHeapificationStore) {
  static const bool paranoid = feGetEnv("TR_paranoidVersioning") != NULL;
  if (paranoid && requiresPrivatization(node)) return false;

  if (_visitedNodes.isSet(node->getGlobalIndex())) return true;
  ...
  if (opCode.hasSymbolReference()) {
    TR::SymbolReference *symReference = node->getSymbolReference();
    if (suppressInvarianceAndPrivatization(symReference)) return false;
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    if (!isExprInvariantRecursive(node->getChild(i))) return false;
  }

  return true;
}

bool TR_LoopVersioner::hasWrtbarBeenSeen(List<TR::TreeTop> *awrtbariTrees,
  ...
  for (; nextTree;) {
    ...
    if (node->getOpCodeValue() == TR::awrtbari) {
      if (node == awrtbariNode) return true;
    }
...
bool TR_LoopVersioner::canPredictIters(
    TR_RegionStructure *whileLoop,
    const TR_ScratchList<TR::Block> &blocksInWhileLoop, bool &isIncreasing,
  ...
  if (((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple) ||
       (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt))) {
    returnValue = true;
    isIncreasing = blocksInWhileLoop.find(_loopTestTree->getNode()
  ...
  if (!whileLoop->containsOnlyAcyclicRegions()) returnValue = false;
  ...
  if (returnValue) {
    ...
    if (firstChildSymRef) {
      ...
      if (!_versionableInductionVariables.isEmpty())
        loopDrivingInductionVar =
            *(_versionableInductionVariables.getListHead()->getData());
...
int32_t *computeCallsiteCounts(TR_ScratchList<TR::Block> *loopBlocks,
                               TR::Compilation *comp) {
  int32_t inlinedCount = comp->getNumInlinedCallSites();
  int32_t *callIndexCounts = (int32_t *)comp->trMemory()->allocateHeapMemory(
      sizeof(int32_t) * (inlinedCount + 1));
  ...
  const bool countAllocationFenceNodes =
      comp->getOption(TR_EnableLoopVersionerCountAllocationFences);
  for (TR::Block *block = loopItr.getCurrent(); block;
       block = loopItr.getNext()) {
    for (OMR::TreeTop *tt = block->getFirstRealTreeTop();
         tt != block->getExit(); tt = tt->getNextTreeTop()) {
      // Do not count allocation fences on PPC (or other weak memory platforms)
      // because it throws the treetop count relative to non-weak memory
      // platforms where we may have tuned our thresholds
      if (!countAllocationFenceNodes &&
          (tt->getNode()->getOpCodeValue() == TR::allocationFence)) {
        continue;
      }
  ...
  // now compute the transitive closure - each method's count is for the method
  // itself and all the methods it inlines
  int32_t *transitiveCounts = (int32_t *)comp->trMemory()->allocateHeapMemory(
      sizeof(int32_t) * (inlinedCount + 2));
  ...
  for (int32_t i = 0; i < inlinedCount; ++i) {
    ...
    while (callIndex > -1) {
      ...
      callIndex =
          comp->getInlinedCallSite(callIndex)._byteCodeInfo.getCallerIndex();
    }
...
bool TR_LoopVersioner::checkProfiledGuardSuitability(
    TR_ScratchList<TR::Block> *loopBlocks, TR::Node *guardNode,
    TR::SymbolReference *callSymRef, TR::Compilation *comp) {
  bool disableLoopCodeRatioCheck =
      feGetEnv("TR_DisableLoopCodeRatioCheck") != NULL;
  bool risky = false;
  if (comp->getMethodHotness() >= hot && callSymRef) {
    if (callSymRef->getSymbol() &&
...
#ifdef J9_PROJECT_SPECIFIC
        &&
        comp->fej9()->maybeHighlyPolymorphic(
            comp, callSymRef->getOwningMethod(comp), callSymRef->getCPIndex(),
      ...
      if (trace()) {
        ...
        char *s = classNameToSignature(method->getMethod()->classNameChars(),
                                       len, comp);
        TR_OpaqueClassBlock *classOfMethod =
            comp->fe()->getClassFromSignature(s, len, owningMethod, true);
      ...
      TR::DebugCounter::incStaticDebugCounter(
          comp, TR::DebugCounter::debugCounterName(
                    comp, "interfaceGuardCheck/(%s)", comp->signature()));
      int32_t *treeTopCounts = computeCallsiteCounts(loopBlocks, comp);
      float loopCodeRatio =
          (float)treeTopCounts[guardNode->getInlinedSiteIndex() + 2] /
          (float)treeTopCounts[0];
      if (trace())
        traceMsg(comp, "  Loop code ratio %d / %d = %.2f\n",
                 treeTopCounts[guardNode->getInlinedSiteIndex() + 2],
                 treeTopCounts[0], loopCodeRatio);
      if (disableLoopCodeRatioCheck || loopCodeRatio < 0.25) {
        if (trace())
          traceMsg(comp,
                   "Skipping versioning of profiled guard %p because we found "
                   "more than 2 JIT'd implementors at warm or above and the "
                   "loop code ratio is too low\n",
                   guardNode);
        risky = true;
        TR::DebugCounter::incStaticDebugCounter(
            comp, TR::DebugCounter::debugCounterName(
                      comp,
                      "profiledVersioning/unsuitableForVersioning/"
                      "interfaceGuard/(%s)/bci=%d.%d",
                      comp->signature(),
                      guardNode->getByteCodeInfo().getCallerIndex(),
                      guardNode->getByteCodeInfo().getByteCodeIndex()));
      }
    } else if (comp->getInlinedResolvedMethod(
                       guardNode->getByteCodeInfo().getCallerIndex())
                   ->isSubjectToPhaseChange(comp)) {
      if (trace()) {
        traceMsg(comp,
                 "Found profiled guard %p is for a method subject to phase "
                 "change - skipping versioning\n",
                 guardNode);
      }
      risky = true;
    }
...
bool TR_LoopVersioner::isBranchSuitableToVersion(
    TR_ScratchList<TR::Block> *loopBlocks, TR::Node *node,
    TR::Compilation *comp) {
  bool suitableForVersioning = true;

  float profiledGuardProbabilityThreshold = 0.98f;
  static char *profiledGuardProbabilityThresholdStr =
      feGetEnv("TR_ProfiledGuardVersioningThreshold");
  static char *disableProfiledGuardVersioning =
      feGetEnv("TR_DisableProfiledGuardVersioning");
  if (profiledGuardProbabilityThresholdStr) {
    profiledGuardProbabilityThreshold =
        ((float)atof(profiledGuardProbabilityThresholdStr));
  }

#ifdef J9_PROJECT_SPECIFIC
  if (node->isProfiledGuard()) {
    bool isGetFieldCacheCounts = !strncmp(
        comp->signature(),
        "org/apache/solr/request/SimpleFacets.getFieldCacheCounts(Lorg/apache/"
        "solr/search/SolrIndexSearcher;Lorg/apache/solr/search/DocSet;Ljava/"
        "lang/String;IIIZLjava/lang/String;Ljava/lang/String;)Lorg/apache/solr/"
        "common/util/NamedList;",
        60);

    TR_InlinedCallSite &ics =
        comp->getInlinedCallSite(node->getByteCodeInfo().getCallerIndex());
    if (isGetFieldCacheCounts || disableProfiledGuardVersioning) {
      suitableForVersioning = false;
    } else if (comp->getInlinedCallerSymRef(
                   node->getByteCodeInfo().getCallerIndex())) {
      TR_AddressInfo *valueInfo = static_cast<TR_AddressInfo *>(
          TR_ValueProfileInfoManager::getProfiledValueInfo(ics._byteCodeInfo,
                                                           comp, AddressInfo));
      ...
      if (valueInfo) {
        ...
        if (valueInfo->getTopProbability() >=
            profiledGuardProbabilityThreshold) {
          suitableForVersioning = checkProfiledGuardSuitability(
              loopBlocks, node,
              comp->getInlinedCallerSymRef(
                  node->getByteCodeInfo().getCallerIndex()),
              comp);
        } else {
          suitableForVersioning = false;
        ...
        TR::DebugCounter::incStaticDebugCounter(
            comp,
            TR::DebugCounter::debugCounterName(
                comp, "profiledVersioning/unsuitableForVersioning/noinfo"));
        suitableForVersioning = false;
      }
    } else {
...
bool TR_LoopVersioner::detectChecksToBeEliminated(
    TR_RegionStructure *whileLoop, List<TR::Node> *nullCheckedReferences,
    List<TR::TreeTop> *nullCheckTrees, List<int32_t> *numIndirections,
    List<TR::TreeTop> *boundCheckTrees, List<TR::TreeTop> *spineCheckTrees,
    List<int32_t> *numDimensions, List<TR::TreeTop> *conditionalTrees,
    List<TR::TreeTop> *divCheckTrees, List<TR::TreeTop> *awrtbariTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    List<TR::Node> *specializedInvariantNodes,
    List<TR_NodeParentSymRef> *invariantNodes,
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    if (nextBlock->isCold()) {
      ...
      static char *unimportantFrequencyRatioStr =
          feGetEnv("TR_loopVersionerControlUnimportantFrequencyRatio");
      int32_t unimportantFrequencyRatio =
          unimportantFrequencyRatioStr ? atoi(unimportantFrequencyRatioStr)
                                       : 20;
      int16_t blockFrequency = nextBlock->getFrequency();

      // If the aggressive loop versioning flag is set, only do the unimportant
      // block test at lower optlevels
      bool aggressive = comp()->getOption(TR_EnableAggressiveLoopVersioning);
      ...
      if ((!aggressive || comp()->getMethodHotness() <= warm) &&
          ...
          performTransformation(comp(),
                                "%sDisregard unimportant block_%d frequency %d "
                                "< %d from block_%d in loop %d\n",
                                OPT_DETAILS_LOOP_VERSIONER,
                                nextBlock->getNumber(), blockFrequency,
                                loopFrequency, hotBlock->getNumber(),
                                whileLoop->getNumber())) {
        isUnimportant = true;
        if (trace() || comp()->getOption(TR_CountOptTransformations)) {
          for (TR::TreeTop *tt = entryTree; isUnimportant && tt != exitTree;
               tt = tt->getNextTreeTop()) {
            TR::Node *ttNode = tt->getNode();
    ...
    if (isUnimportant) collectProfiledExprs = false;
    ...
    while (!(currentTree == exitTree)) {
      ...
      if (currentOpCode.getOpCodeValue() == TR::asynccheck) {
        ...
        if (_loopTestTree && (_loopTestTree->getNode()->getNumChildren() > 1) &&
          ...
          bool isIncreasing;
          TR::SymbolReference *firstChildSymRef;
          bool _canPredictIters = canPredictIters(
              whileLoop, blocksInWhileLoop, isIncreasing, firstChildSymRef);

          if (_canPredictIters) {
            int32_t numIters = whileLoop->getEntryBlock()->getFrequency();
            // if (numIters >
            // 0.90*comp()->getRecompilationInfo()->getMaxBlockCount())
      ...
      vcount_t visitCount =
          comp()->incVisitCount();  //@TODO: unsafe API/use pattern
      updateDefinitionsAndCollectProfiledExprs(
          NULL, currentNode, visitCount, specializedInvariantNodes,
          invariantNodes, invariantTranslationNodesList, NULL,
          collectProfiledExprs, nextBlock, warmBranchCount);

      bool disableLT = comp()->getOption(TR_DisableLoopTransfer);
      if (!disableLT &&
          currentNode->isTheVirtualGuardForAGuardedInlinedCall() &&
          blocksInWhileLoop.find(
              currentNode->getBranchDestination()->getNode()->getBlock())) {
        _loopTransferDone = true;
        if (!_containsUnguardedCall) _containsGuard = true;
      }
      ...
      if ((currentNode->getNumChildren() > 0) &&
        ...
        if (!currentNode->getFirstChild()
          ...
          _containsGuard = false;
        } else if (currentNode->getFirstChild()
                       ->isTheVirtualCallNodeForAGuardedInlinedCall()) {
          TR::CFG *cfg = comp()->getFlowGraph();
          TR::Block *guard = nextBlock->findVirtualGuardBlock(cfg);
          if (!guard) {
            // printf("\t\tnode %p (nextblock = %p) is hte virtual call node for
            // a guarded inlined call without a guard.\n",currentNode,nextBlock);
            _containsUnguardedCall = true;
            _containsGuard = false;
          }
        }
      ...
      if (!isUnimportant && !_containsCall && !refineAliases()) {
        if (currentOpCode.isNullCheck()) {
          ...
          foundPotentialChecks = true;
                   ...
                   currentOpCode.getOpCodeValue() !=
                       TR::arraytranslateAndTest) {
          if (trace())
          ...
          foundPotentialChecks = true;
                   ...
                   isBranchSuitableToVersion(&blocksInWhileLoop, currentNode,
                                             comp()) &&
          ...
          if (isConditionalTreeCandidateForElimination(currentTree)) {
            ...
            if (_postDominators) {
              postDominatesEntry =
                  (_postDominators->dominates(nextBlock,
                                              whileLoop->getEntryBlock()) != 0);
              if (postDominatesEntry) {
                dumpOptDetails(comp(),
                               "   block_%d post dominates loop entry\n",
                               nextBlock->getNumber());
                nextBlock->setIsPRECandidate(true);
              }

              if (nextBlock->getFrequency() ==
                  whileLoop->getEntryBlock()->getFrequency())
                dumpOptDetails(comp(),
                               "   the same frequency as entry block\n");
            }
            ...
            if (trace()) {
              ...
              if (_postDominators) {
                traceMsg(comp(), "    controls %d out of %d blocks\n",
                         _postDominators->numberOfBlocksControlled(
                             nextBlock->getNumber()),
                         loop_size);
          ...
          if (trace()) traceMsg(comp(), "DIVCHK %p\n", currentTree->getNode());

          divCheckTrees->add(currentTree);

          if (dupOfThisBlockAlreadyExecutedBeforeLoop)
            _checksInDupHeader.add(currentTree);
        } else if (currentOpCode.isCheckCast())  // (currentOpCode.getOpCodeValue()
                                                 // == TR::checkcast)
        {
          if (currentNode->getSecondChild()->getOpCodeValue() == TR::loadaddr) {
            ...
            if (!comp()->fe()->isUnloadAssumptionRequired(
              ...
              if (trace())  // if we move the checkcast before the inlined body
                            // instanceof might fail because the class might not
                            // exist in the original code
                traceMsg(comp(),
                         "Class for Checkcast %p is not loaded by the same "
                         "classloader as the compiled method\n",
                         currentTree->getNode());
            }
          } else {
            checkCastTrees->add(currentTree);
        ...
        // SPECpower Work
        if (TR::Compiler->om.writeBarrierType() ==
...
void TR_LoopVersioner::updateDefinitionsAndCollectProfiledExprs(
    TR::Node *parent, TR::Node *node, vcount_t visitCount,
    List<TR::Node> *profiledNodes, List<TR_NodeParentSymRef> *invariantNodes,
    List<TR_NodeParentSymRefWeightTuple> *invariantTranslationNodesList,
    TR::Node *lastFault, bool collectProfiledExprs, TR::Block *block,
    int32_t warmBranchCount) {
  if (node->getVisitCount() == visitCount) return;

  // There will be no definitions or profiled expressions
  // beneath a nopable guard
  if (node->isNopableInlineGuard() || node->isOSRGuard()) return;
  ...
  if (opCode.hasSymbolReference()) {
    ...
    if (node->mightHaveVolatileSymbolReference()) {
      if (symReference->sharesSymbol()) {
        symReference->getUseDefAliases().getAliasesAndUnionWith(
  ...
  if (opCode.isResolveCheck()) defNode = node->getFirstChild();

  if (defNode->isTheVirtualCallNodeForAGuardedInlinedCall() &&
      block->findVirtualGuardBlock(comp()->getFlowGraph()))
    defNode = NULL;  // old code was setting node's symref anyway, is it really
                     // needed?

  if (_guardedCalls.find(node)) defNode = NULL;
  ...
  if (defNode) {
    ...
    if (opCode.isStore()) {
      bool heapificationStore =
          node->getOpCodeValue() == TR::astore && node->isHeapificationStore();
      if (!heapificationStore && _writtenAndNotJustForHeapification)
        defAliases.getAliasesAndUnionWith(
            *_writtenAndNotJustForHeapification);  // TODO: avoid getting the
                                                   // same aliases twice
    }
  }

  // dumpOptDetails(comp(), "After examining node %p\n", node);
  // dumpOptDetails(comp(), "seenDefined is : ");
  // _seenDefinedSymbolReferences->print(comp());
  if (node == _nullCheckReference) _inNullCheckReference = true;

  opCodeValue = opCode.getOpCodeValue();
  if (_inNullCheckReference || !_nullCheckReference) {
    if ((opCode.isCall()) || (opCodeValue == TR::New) ||
        (opCodeValue == TR::newarray) || (opCodeValue == TR::anewarray) ||
        (opCodeValue == TR::multianewarray))
      _containsCall = true;

    if (node->hasUnresolvedSymbolReference()) _containsCall = true;
  ...
  if (TR_LocalAnalysis::isSupportedOpCode(node->getOpCode(), comp())) {
    if (node->getType().isInt32() ||
      ...
      if (profileLongParms && comp()->getMethodHotness() == hot &&
        ...
        if (node->getType().isInt64()) {
          if (node->getSymbolReference()->getSymbol()->isParm()) {
            ...
            // potential opportunity is seen for specializing long parms (which
            // are usually invariant) is seen
            //
      ...
      if (/* comp()->getRecompilationInfo() && */ collectProfiledExprs &&
        ...
        TR_ValueInfo *valueInfo = static_cast<TR_ValueInfo *>(
            TR_ValueProfileInfoManager::getProfiledValueInfo(node, comp(),
                                                             ValueInfo));
        if (valueInfo) {
          if ((valueInfo->getTopProbability() > MIN_PROFILED_FREQUENCY) &&
              ...
              // Only collect nodes from unspecialized blocks to avoid
              // specializing the same nodes twice
              !block->isSpecialized()) {
            if ((!node->getType().isInt64()) ||
                (profileLongParms && (valueInfo->getTopValue() == 0))) {
              // Zero length contiguous array lengths mean further profiling is
              // necessary to find the true array length.
              //
              if (comp()->requiresSpineChecks() &&
                  node->getOpCodeValue() == TR::contigarraylength &&
                  valueInfo->getTopValue() == 0) {
                dumpOptDetails(
                    comp(),
                    "From value profiling, NOT USING inconclusive "
                    "contigarraylength node %p with value 0 freq %d total freq "
                    "%d\n",
                    node,
                ...
                dumpOptDetails(
                    comp(),
                    "From value profiling, node %p has value %d freq %d total "
                    "freq %d\n",
                    node, valueInfo->getTopValue(),
                    ((int32_t)(valueInfo->getTopProbability() *
                               ((float)valueInfo->getTotalFrequency()))),
                    valueInfo->getTotalFrequency());
    ...
    if (invariantNodes && collectProfiledExprs && !_containsCall &&
      ...
      if (trace())
        traceMsg(comp(), "Added invariant node %p %s\n", node,
                 node->getOpCode().getName());
      invariantNodes->add(new (trStackMemory())
                              TR_NodeParentSymRef(node, parent, NULL));
    }
  }
  ...
  for (i = 0; i < node->getNumChildren(); i++)
    updateDefinitionsAndCollectProfiledExprs(
        node, node->getChild(i), visitCount, profiledNodes, invariantNodes,
        invariantTranslationNodesList, lastFault, collectProfiledExprs, block,
        warmBranchCount);

  if (node == _nullCheckReference) _inNullCheckReference = false;
}

void TR_LoopVersioner::versionNaturalLoop(
    TR_RegionStructure *whileLoop, List<TR::Node> *nullCheckedReferences,
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *boundCheckTrees,
    List<TR::TreeTop> *spineCheckTrees, List<TR::TreeTop> *conditionalTrees,
    List<TR::TreeTop> *divCheckTrees, List<TR::TreeTop> *awrtbariTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    List<TR::Node> *specializedNodes, List<TR_NodeParentSymRef> *invariantNodes,
    List<TR_NodeParentSymRefWeightTuple> *invariantTranslationNodesList,
    List<TR_Structure> *innerWhileLoops,
  ...
  TR::Block **correspondingBlocks =
      (TR::Block **)trMemory()->allocateStackMemory(numNodes *
                                                    sizeof(TR::Block *));
  ...
  //
  TR::TreeTop *treeTop;
  for (treeTop = comp()->getStartTree(); treeTop;
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    TR::Block *nextClonedBlock = TR::Block::createEmptyBlock(
        entryNode, comp(), nextBlock->getFrequency(), nextBlock);
    nextClonedBlock->setIsSpecialized(nextBlock->isSpecialized());
    //
    // Set slow version flag so subsequent passes like unrolling (and even
    // inlining) to ignore 'cold' portions of code (like the slow version of the
    // loop).
    //
    if (!_neitherLoopCold || nextBlock->isCold()) {
      nextClonedBlock->setIsCold();
      if (nextBlock->isSuperCold()) nextClonedBlock->setIsSuperCold();
      int32_t frequency = VERSIONED_COLD_BLOCK_COUNT;
      if (nextBlock->isCold()) {
        int32_t nextBlockFreq = nextBlock->getFrequency();
        if (frequency > nextBlockFreq) frequency = nextBlockFreq;
      }
      nextClonedBlock->setFrequency(frequency);
    } else if (_neitherLoopCold && shouldOnlySpecializeLoops()) {
      // nextClonedBlock->setIsRare();
      // Set the flag on blocks in the slow version of the loop, so that in the
      // analysis phase, we don't collect these blocks
      ...
      if (nextBlock->getFrequency() < 0)
        ...
        int32_t specializedBlockFrequency =
            TR::Block::getScaledSpecializedFrequency(nextBlock->getFrequency());
        if (nextBlock->isCold())
    ...
    while (!(currentTree == exitTree)) {
      ...
      for (int32_t childNum = 0;
           childNum < currentTree->getNode()->getNumChildren(); childNum++) {
        TR::Node *duplicateChildNode =
            duplicateExact(currentTree->getNode()->getChild(childNum),
                           &seenNodes, &duplicateNodes);
    ...
    if (!disableLT && lastTree->getOpCode().isIf() &&
        blocksInWhileLoop.find(
            lastTree->getBranchDestination()->getNode()->getBlock()) &&
        lastTree->isTheVirtualGuardForAGuardedInlinedCall() &&
        isBranchSuitableToDoLoopTransfer(&blocksInWhileLoop, lastTree,
                                         comp()) &&
        performTransformation(comp(),
                              "%s Adding loop transfer candidate "
                              "(VirtualGuardPair) for n%un [%p]\n",
                              OPT_DETAILS_LOOP_VERSIONER,
                              lastTree->getGlobalIndex(), lastTree)) {
      ...
      VirtualGuardPair *virtualGuardPair =
          (VirtualGuardPair *)trMemory()->allocateStackMemory(
              sizeof(VirtualGuardPair));
      virtualGuardPair->_hotGuardBlock = nextBlock;
      virtualGuardPair->_coldGuardBlock = nextClonedBlock;
      if (trace())
        traceMsg(comp(),
                 "virtualGuardPair at guard node %p hotGuardBlock %d "
                 "coldGuardBlock %d\n",
                 nextBlock->getLastRealTreeTop()->getNode(),
                 nextBlock->getNumber(), nextClonedBlock->getNumber());
      virtualGuardPair->_isGuarded = false;
      // check if the virtual guard is in an inner loop
      //
      TR_RegionStructure *parent =
          nextBlock->getStructureOf()->getParent()->asRegion();
  ...
  if (vgInfo) {
    ...
    for (VirtualGuardPair *vg = vgIt.getFirst(); vg; vg = vgIt.getNext()) {
      if (vg->_isInsideInnerLoop)
        vg->_coldGuardLoopEntryBlock =
            correspondingBlocks[vg->_coldGuardLoopEntryBlock->getNumber()];
    }
  }
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    //
    for (auto edge = nextBlock->getSuccessors().begin();
         edge != nextBlock->getSuccessors().end(); ++edge) {
      ...
      if (clonedSucc) {
        ...
        //
        TR::CFGEdge *e =
            TR::CFGEdge::createEdge(nextClonedBlock, clonedSucc, trMemory());
        _cfg->addEdge(e);
        ...
        if (_neitherLoopCold) {
          if (!shouldOnlySpecializeLoops())
            ...
            int32_t specializedBlockFrequency =
                TR::Block::getScaledSpecializedFrequency(
                    (*edge)->getFrequency());
        ...
        if (!(lastOpCode.isBranch() ||
              (lastOpCode.isJumpWithMultipleTargets() &&
               lastOpCode.hasBranchChildren()) ||
              lastOpCode.isReturn() ||
              (lastOpCode.getOpCodeValue() == TR::athrow) || callWithException))
          fallsThrough = true;
        ...
        if (!fallsThrough)
          _cfg->addEdge(
              TR::CFGEdge::createEdge(nextClonedBlock, succ, trMemory()));
        else {
          TR::Block *newGotoBlock = TR::Block::createEmptyBlock(
              lastNode, comp(), (*edge)->getFrequency(), nextClonedBlock);
          ...
          if (endTree == clonedExit) endTree = gotoBlockExitTree;

          TR::CFGEdge *e1 = TR::CFGEdge::createEdge(nextClonedBlock,
                                                    newGotoBlock, trMemory());
          _cfg->addEdge(e1);
          TR::CFGEdge *e2 =
              TR::CFGEdge::createEdge(newGotoBlock, succ, trMemory());
          _cfg->addEdge(e2);
          if (_neitherLoopCold) {
            if (!shouldOnlySpecializeLoops()) {
          ...
          TR_BlockStructure *newGotoBlockStructure =
              new (_cfg->structureRegion()) TR_BlockStructure(
                  comp(), newGotoBlock->getNumber(), newGotoBlock);
    ...
    //
    for (auto edge = nextBlock->getExceptionSuccessors().begin();
         edge != nextBlock->getExceptionSuccessors().end(); ++edge) {
      TR::Block *succ = toBlock((*edge)->getTo());
      TR::Block *nextClonedBlock = correspondingBlocks[nextBlock->getNumber()];
      TR::Block *clonedSucc = correspondingBlocks[succ->getNumber()];

      if (clonedSucc) {
        _cfg->addEdge(TR::CFGEdge::createExceptionEdge(nextClonedBlock,
                                                       clonedSucc, trMemory()));
      } else {
        _cfg->addEdge(TR::CFGEdge::createExceptionEdge(nextClonedBlock, succ,
                                                       trMemory()));
      }
    }
  }
  ...
  //
  TR::Block *blockAtHeadOfClonedLoop =
      correspondingBlocks[blockAtHeadOfLoop->getNumber()];
  ...
  TR::Block *clonedLoopInvariantBlock = TR::Block::createEmptyBlock(
      invariantBlock->getEntry()->getNode(), comp(),
  ...
  TR::Node *gotoNode =
      TR::Node::create(blockHeadNode, TR::Goto, 0, blockHeadTreeTop);
  ...
  if (_requiresAdditionalCheckForIncrement &&
      performTransformation(comp(),
                            "%s Creating test outside loop for checking if "
                            "loop driving induction variable is incremented by "
                            "a loop invariant that is greater than 0\n",
                            OPT_DETAILS_LOOP_VERSIONER)) {
    if (_constNode->getType().isInt32()) {
      TR::Node *nextComparisonNode =
          TR::Node::createif(TR::ificmple, _constNode,
                             TR::Node::create(blockHeadNode, TR::iconst, 0, 0),
                             clonedLoopInvariantBlock->getEntry());
      dumpOptDetails(
          comp(),
          "Node %p has been created testing if increment check is required\n",
          nextComparisonNode);
      comparisonTrees.add(nextComparisonNode);
    } else if (_constNode->getType().isInt64()) {
      TR::Node *nextComparisonNode =
          TR::Node::createif(TR::iflcmple, _constNode,
                             TR::Node::create(blockHeadNode, TR::lconst, 0, 0),
                             clonedLoopInvariantBlock->getEntry());
      dumpOptDetails(
          comp(),
          "Node %p has been created testing if increment check is required\n",
      ...
      TR_ASSERT(0,
                "Unable to create test outside of loop for checking if iv "
                "update increment is greater than 0 due to unknown type\n");
    }
  }
  ...
  if (_loopConditionInvariant && _asyncCheckTree) {
    bool isIncreasing;
    TR::SymbolReference *firstChildSymRef;
    bool _canPredictIters = canPredictIters(whileLoop, blocksInWhileLoop,
                                            isIncreasing, firstChildSymRef);
    ...
    if (loopLimit->getType().isInt32() &&
...
#ifdef J9_PROJECT_SPECIFIC
      TR_ValueInfo *valueInfo = static_cast<TR_ValueInfo *>(
          TR_ValueProfileInfoManager::getProfiledValueInfo(loopLimit, comp(),
                                                           ValueInfo));
      if (valueInfo) {
        if (valueInfo->getTotalFrequency()) {
          dumpOptDetails(comp(),
                         "From value profiling, loop test node %p has value %d "
                         "freq %d total freq %d, and max value %d\n",
                         loopLimit, valueInfo->getTopValue(),
                         (int32_t)(valueInfo->getTopProbability() *
                                   valueInfo->getTotalFrequency()),
                         valueInfo->getTotalFrequency(),
                         valueInfo->getMaxValue());
          ...
          // if (shouldOnlySpecializeLoops())
          {
            ...
            ListIterator<TR_ExtraValueInfo> sortedValuesIt(
                &valuesSortedByFrequency);

            float totalFreq = (float)valueInfo->getTotalFrequency();
            float valuedFreq = 0;

            for (TR_ExtraValueInfo *profiledInfo = sortedValuesIt.getFirst();
                 profiledInfo != NULL;
            ...
            if (valueInfo->getMaxValue() > profiledLoopLimit &&
                valuedFreq / totalFreq < 0.95f) {
              dumpOptDetails(
                  comp(),
                  "Maximum profiled value of %d exceeds profiled loop limit of "
                  "%d, and value profiled iterations below limit account for "
                  "only %5.3f of iterations.  Delaying asyncCheck removal for "
                  "node %p\n",
                  valueInfo->getMaxValue(), profiledLoopLimit,
                  valuedFreq / totalFreq, loopLimit);
      ...
      // compjazz 108308 disallow versioning of async checks for long running
      // GPU Loops when forced to compile at scorching without jit value info.
      else if (comp()->getOptions()->getEnableGPU(TR_EnableGPU) &&
               comp()->hasIntStreamForEach() &&
    ...
    if (_canPredictIters && firstChildSymRef) {
      ...
      if (v && v->getEntry() && v->getExit()) {
        ...
        if (entryVal->asIntConstraint() && exitVal->asIntConstraint() &&
          ...
          if (minDelta > profiledLoopLimit) _canPredictIters = false;
        }
      }
    }
    ...
    if (!comp()->getOption(TR_DisableAsyncCheckVersioning) &&
        !refineAliases() && _canPredictIters &&
        comp()->getProfilingMode() != JitProfiling &&
        performTransformation(comp(),
                              "%s Creating test outside loop for deciding if "
                              "async check is required\n",
                              OPT_DETAILS_LOOP_VERSIONER)) {
      TR::Node *lowerBound = _loopTestTree->getNode()
                                 ->getFirstChild()
                                 ->duplicateTreeForCodeMotion();
      TR::Node *upperBound = _loopTestTree->getNode()
                                 ->getSecondChild()
                                 ->duplicateTreeForCodeMotion();
      TR::Node *loopLimit =
          isIncreasing ? TR::Node::create(TR::isub, 2, upperBound, lowerBound)
                       : TR::Node::create(TR::isub, 2, lowerBound, upperBound);

      TR::Node *nextComparisonNode = TR::Node::createif(
          comparisonOpCode, loopLimit,
          TR::Node::create(loopLimit, TR::iconst, 0, profiledLoopLimit),
          clonedLoopInvariantBlock->getEntry());
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(
            new (_curLoop->_memRegion)
                RemoveAsyncCheck(this, prep, _asyncCheckTree));
      }
    }
  }
  ...
  if (!boundCheckTrees->isEmpty()) {
    ...
    if (_loopConditionInvariant &&
        !blocksInWhileLoop.find(_loopTestTree->getNode()
                                    ->getBranchDestination()
                                    ->getNode()
                                    ->getBlock()))
      reverseBranch = true;
    buildBoundCheckComparisonsTree(boundCheckTrees, spineCheckTrees,
                                   reverseBranch);
  }
  ...
  if (!specializedNodes->isEmpty()) {
    TR::SymbolReference **symRefs =
        (TR::SymbolReference **)trMemory()->allocateStackMemory(
            comp()->getSymRefCount() * sizeof(TR::SymbolReference *));
    memset(symRefs, 0,
           comp()->getSymRefCount() * sizeof(TR::SymbolReference *));

    // printf("Reached here in %s\n", comp()->signature());
    bool specializedLongs = buildSpecializationTree(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
    ...
    //
    ListIterator<TR::Block> blocksIt(&blocksInWhileLoop);
    for (TR::Block *block = blocksIt.getCurrent(); block != NULL;
         block = blocksIt.getNext()) {
      TR::TreeTop *entry = block->getEntry();
      TR::TreeTop *exit = block->getExit();
      for (TR::TreeTopIterator it(entry, comp()); !it.isAt(exit);
           it.stepForward()) {
  ...
  if (refineAliases())
    buildAliasRefinementComparisonTrees(
        nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
  ...
  // to be cast.
  //
  if (!checkCastTrees->isEmpty()) buildCheckCastComparisonsTree(checkCastTrees);
  ...
  // If aggressive loop versioning is requested, don't call
  // buildNullCheckComparisonsTree based on hotness
  if (comp()->getOption(TR_EnableAggressiveLoopVersioning)) {
    if (trace())
      traceMsg(comp(),
               "aggressiveLoopVersioning: raising hotnessThreshold for "
               "buildNullCheckComparisonsTree\n");
    hotnessThreshold =
        maxHotness;  // threshold which can't be matched by the > operator
  }
  ...
  if (comp()->cg()->performsChecksExplicitly() ||
    ...
    for (ListElement<TR::TreeTop> *elt = head; elt != NULL;
         elt = elt->getNextElement()) {
      TR::Node *check = elt->getData()->getNode();

      // A NULLCHK might have been specialized away, but will still appear in
      // nullCheckTrees Skip such trees
      if (!check->getOpCode().isNullCheck()) {
        TR_ASSERT_FATAL(check->getOpCodeValue() == TR::treetop,
                        "Unexpected opcode for n%dn [%p]\n",
      ...
      if (nullTestEntry == _curLoop->_nullTestPreps.end()) continue;

      LoopEntryPrep *prep = nullTestEntry->second;
      if (!performTransformation(
              comp(),
              "%sOpportunistically attempting to eliminate null check n%un "
              "[%p] using existing prep %p\n",
  ...
  if (comp()->getHCRMode() != TR::none && disableLoopHCR == NULL) {
    ...
    LoopBodySearch search(comp(), _curLoop->_memRegion, whileLoop,
                          &removedNodes, &_curLoop->_takenBranches);
    ...
    for (; search.hasTreeTop() && safeToRemoveHCRGuards; search.advance()) {
      ...
      TR::Node *ttNode = tt->getNode();
      culprit = ttNode;
      if (removedNodes.contains(ttNode)) continue;
    ...
    if (!safeToRemoveHCRGuards) {
      dumpOptDetails(
          comp(), "Cannot remove HCR guards in loop %d due to n%un [%p]\n",
          whileLoop->getNumber(), culprit->getGlobalIndex(), culprit);
  ...
  if (!awrtbariTrees->isEmpty() && !disableWrtbarVersion &&
  ...
  if (!_curLoop->_privatizationOK && _curLoop->_privatizationsRequested) {
    ...
    for (; search.hasTreeTop(); search.advance()) {
      ...
      if (child->getOpCode().isFunctionCall()) {
        ...
        dumpOptDetails(comp(), "NO PRIVATIZATION in loop %d due to n%un [%p]\n",
                       whileLoop->getNumber(), node->getGlobalIndex(), node);
        break;
      }
    }
  }
  ...
  // will be patched by the same runtime assumptions, so only one OSR guard is
  // added to branch to the slow loop.
  //
  ...
  if (comp()->getHCRMode() == TR::osr && disableLoopOSR == NULL) {
    ...
    TR_ASSERT_FATAL(_curLoop->_optimisticallyRemovableNodes.contains(
                        _curLoop->_definitelyRemovableNodes),
                    "all _definitelyRemovableNodes should also be "
                    "_optimisticallyRemovableNodes in loop %d",
                    whileLoop->getNumber());
    ...
    if (trace()) {
      traceMsg(comp(), "privatizationOK: %d : removedNodes ",
               _curLoop->_privatizationOK);
    ...
    for (; search.hasTreeTop(); search.advance()) {
      TR::TreeTop *tt = search.currentTreeTop();
      if (removedNodes.contains(tt->getNode())) continue;

      if (comp()->isPotentialOSRPoint(tt->getNode(), NULL, true)) {
        if (trace())
          traceMsg(comp(),
                   "safeToRemoveOSRGuards false due to potential OSR point at "
                   "n%dn\n",
                   tt->getNode()->getGlobalIndex());
    ...
    if (seenOSRGuards && safeToRemoveOSRGuards) {
      ...
      if (performTransformation(comp(), "%sCreate versioned OSRGuard\n",
        ...
        if (trace())
          traceMsg(comp(),
                   "OSRGuard n%dn has been created to guard against method "
                   "invalidation\n",
  ...
  if (_curLoop->_privatizationsRequested && !_curLoop->_privTemps.empty()) {
    ...
    LoopBodySearch search(comp(), _curLoop->_memRegion, whileLoop, removedNodes,
                          takenBranches);
  ...
  // Due to RAS changes to make each loop version test a transformation,
  // disableOptTransformations or lastOptTransformationIndex can now potentially
  // remove all the tests above the 2 versioned loops.  When there are two
  // versions of the loop, it is necessary that there be at least one test at
  // the top.  Therefore, the following is required to ensure that a test is
  // created.
  ...
  if (testCount == 0) {
    ...
    TR::Node *nextComparisonNode =
        TR::Node::createif(TR::ificmpne, constNode, constNode,
                           clonedLoopInvariantBlock->getEntry());
    comparisonTrees.add(nextComparisonNode);
    dumpOptDetails(
        comp(),
        "The comparison tree is empty.  The node %p has been created because a "
        "test is required when there are two versions of the loop.\n",
        nextComparisonNode);
  }

  // if (debugBoundCheck)
  //   {
  //   TR::TreeTop *debugBoundCheckTree = TR::TreeTop::create(comp(),
  //   debugBoundCheck, NULL, NULL);
  ...
  while (comparisonNode) {
    ...
    TR::TreeTop *comparisonTree =
        TR::TreeTop::create(comp(), actualComparisonNode, NULL, NULL);
    ...
    if (actualComparisonNode->getOpCode().isStore()) {
      ...
      TR::Block *newGotoBlock = TR::Block::createEmptyBlock(
          invariantBlock->getEntry()->getNode(), comp(), 0, comparisonBlock);
      ...
      //trMemory())); _cfg->addEdge(TR::CFGEdge::createEdge(newGotoBlock,
      //clonedLoopInvariantBlock, trMemory()));
      TR_BlockStructure *newGotoBlockStructure = new (_cfg->structureRegion())
  ...
  if (previousTree->getNode()->getOpCodeValue() != TR::Goto) {
    ...
    TR::TreeTop *gotoTree = TR::TreeTop::create(comp(), gotoNode, NULL, NULL);
    gotoNode->setBranchDestination(
        toBlock(invariantBlock->getSuccessors().front()->getTo())->getEntry());
  ...
  if (!_neitherLoopCold || invariantBlock->isCold()) {
    ...
    if (clonedLoopInvariantBlock->isCold()) {
      ...
      if (frequency > blockFreq) frequency = blockFreq;
    }
    ...
    if (invariantBlock->getFrequency() < 0)
      ...
      clonedLoopInvariantBlock->setFrequency(
          TR::Block::getScaledSpecializedFrequency(
              invariantBlock->getFrequency()));
  }

  TR_ByteCodeInfo &bcInfo =
      clonedLoopInvariantBlock->getEntry()->getNode()->getByteCodeInfo();
  TR::DebugCounter::prependDebugCounter(
      comp(),
      TR::DebugCounter::debugCounterName(
          comp(), "loopVersioner.coldLoop/%d/(%s)/%s/bcinfo=%d.%d",
          blockAtHeadOfLoop->getFrequency(), comp()->signature(),
          comp()->getHotnessName(comp()->getMethodHotness()),
          bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex()),
      clonedLoopInvariantBlock->getEntry()->getNextTreeTop(), 1,
  ...
  ListElement<TR::Block> *currCriticalEdgeBlock =
      criticalEdgeBlocks.getListHead();

  while (currComparisonBlock) {
    TR::Block *currentBlock = currComparisonBlock->getData();
    _cfg->addNode(currentBlock);
    ListElement<TR::Block> *nextComparisonBlock =
        currComparisonBlock->getNextElement();
    bool isTest =
        !currentBlock->getLastRealTreeTop()->getNode()->getOpCode().isStore();
    const char *debugCounter = NULL;
    if (isTest)
      debugCounter = TR::DebugCounter::debugCounterName(
          comp(), "loopVersioner.fail/(%s)/%s/origin=block_%d",
          comp()->signature(),
          comp()->getHotnessName(comp()->getMethodHotness()),
          currentBlock->getNumber());

    if (nextComparisonBlock)
      _cfg->addEdge(TR::CFGEdge::createEdge(
          currentBlock, nextComparisonBlock->getData(), trMemory()));
    else
      _cfg->addEdge(
          TR::CFGEdge::createEdge(currentBlock, invariantBlock, trMemory()));

    if (isTest) {
      if (currCriticalEdgeBlock == NULL) {
        _cfg->addEdge(TR::CFGEdge::createEdge(
            currentBlock, clonedLoopInvariantBlock, trMemory()));
      } else {
        _cfg->addEdge(TR::CFGEdge::createEdge(
            currentBlock, currCriticalEdgeBlock->getData(), trMemory()));
        _cfg->addEdge(TR::CFGEdge::createEdge(currCriticalEdgeBlock->getData(),
                                              clonedLoopInvariantBlock,
                                              trMemory()));
        TR::DebugCounter::prependDebugCounter(
            comp(), debugCounter,
  ...
  _cfg->addEdge(TR::CFGEdge::createEdge(clonedLoopInvariantBlock,
                                        blockAtHeadOfClonedLoop, trMemory()));
  ...
  // will contain all the versioning test blocks, critical edge splitting
  // blocks, and other goto blocks created earlier as simple block structures.
  // We will clone the original natural loop structure and make these natural
  // loop structures successors of some of the block structures mentioned above
  // as appropriate.
  //
  TR_StructureSubGraphNode **correspondingSubNodes =
      (TR_StructureSubGraphNode **)trMemory()->allocateStackMemory(
          numNodes * sizeof(TR_StructureSubGraphNode *));
  memset(correspondingSubNodes, 0,
         numNodes * sizeof(TR_StructureSubGraphNode *));

  TR_RegionStructure *clonedWhileLoop =
      whileLoop
          ->cloneStructure(correspondingBlocks, correspondingSubNodes,
                           innerWhileLoops, clonedInnerWhileLoops)
          ->asRegion();
  ...
  TR_BlockStructure *clonedInvariantBlockStructure =
      new (_cfg->structureRegion())
          TR_BlockStructure(comp(), clonedLoopInvariantBlock->getNumber(),
                            clonedLoopInvariantBlock);
  clonedInvariantBlockStructure->setCreatedByVersioning(true);

  if (!_neitherLoopCold) clonedInnerWhileLoops->deleteAll();
  ...
  TR_RegionStructure *properRegion = new (_cfg->structureRegion())
      TR_RegionStructure(comp(), chooserBlock->getNumber());
  parentStructure->replacePart(invariantBlockStructure, properRegion);

  TR_StructureSubGraphNode *clonedWhileNode =
      new (_cfg->structureRegion()) TR_StructureSubGraphNode(clonedWhileLoop);
  TR_StructureSubGraphNode *whileNode =
      new (_cfg->structureRegion()) TR_StructureSubGraphNode(whileLoop);
  TR_StructureSubGraphNode *invariantNode = new (_cfg->structureRegion())
      TR_StructureSubGraphNode(invariantBlockStructure);
  TR_StructureSubGraphNode *clonedInvariantNode = new (_cfg->structureRegion())
      TR_StructureSubGraphNode(clonedInvariantBlockStructure);
  ...
  while (currComparisonBlock) {
    ...
    bool isTest = !actualComparisonBlock->getLastRealTreeTop()
                       ->getNode()
                       ->getOpCode()
                       .isStore();

    TR_BlockStructure *comparisonBlockStructure =
        new (_cfg->structureRegion()) TR_BlockStructure(
            comp(), actualComparisonBlock->getNumber(), actualComparisonBlock);
    comparisonBlockStructure->setCreatedByVersioning(true);
    TR_StructureSubGraphNode *comparisonNode = new (_cfg->structureRegion())
        TR_StructureSubGraphNode(comparisonBlockStructure);
    ...
    // TR::CFGEdge::createEdge(comparisonNode,  clonedInvariantNode,
    // trMemory());
    ...
    if (isTest) {
      if (currCriticalEdgeBlock != NULL) {
        TR_StructureSubGraphNode *criticalEdgeNode =
            new (_cfg->structureRegion()) TR_StructureSubGraphNode(
                currCriticalEdgeBlock->getData()->getStructureOf());
        properRegion->addSubNode(criticalEdgeNode);
        TR::CFGEdge::createEdge(prevComparisonNode, criticalEdgeNode,
                                trMemory());
        TR::CFGEdge::createEdge(criticalEdgeNode, clonedInvariantNode,
                                trMemory());
        currCriticalEdgeBlock = currCriticalEdgeBlock->getNextElement();
      } else
        TR::CFGEdge::createEdge(prevComparisonNode, clonedInvariantNode,
                                trMemory());
    }
  }
  ...
  // from the parent structure as the natural loop would be moved into the
  // proper region.
  //
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    ...
    if (subStruct == properRegion) {
      ...
      for (auto changedSuccEdge = succNode->getExceptionSuccessors().begin();
           changedSuccEdge != succNode->getExceptionSuccessors().end();
           ++changedSuccEdge)
  ...
  for (exitEdge = ei.getCurrent(); exitEdge != NULL; exitEdge = ei.getNext()) {
    TR_StructureSubGraphNode *fromNode =
        toStructureSubGraphNode(exitEdge->getFrom());
    ...
    if (fromStruct == whileLoop) {
      ...
      for (; succEdge != fromNode->getExceptionSuccessors().end(); ++succEdge) {
        if ((*succEdge)->getTo()->getNumber() == toNum) break;
      }
  ...
  //
  ListIterator<TR_BlockStructure> newGotoBlockStructuresIt(
      &newGotoBlockStructures);
  TR_BlockStructure *newGotoBlockStructure;
  for (newGotoBlockStructure = newGotoBlockStructuresIt.getCurrent();
       newGotoBlockStructure;
       newGotoBlockStructure = newGotoBlockStructuresIt.getNext()) {
    TR_StructureSubGraphNode *newGotoBlockNode = new (_cfg->structureRegion())
        TR_StructureSubGraphNode(newGotoBlockStructure);
  ...
  for (exitEdge = ei.getCurrent(); exitEdge; exitEdge = ei.getNext()) {
    ...
    if (!seenExitNodes.get(node->getNumber())) {
      ...
      if (node->getNumber() == regionEntryNode->getNumber()) {
        ...
        properRegion->addExitEdge(whileNode, node->getNumber(),
                                  isExceptionEdge);
      }
  ...
  for (exitEdge = ei.getCurrent(); exitEdge; exitEdge = ei.getNext()) {
    if (exitEdge->getTo()->getNumber() ==
        invariantBlockStructure->getNumber()) {
      clonedWhileLoop->replaceExitPart(invariantBlockStructure->getNumber(),
  ...
  for (exitEdge = ei.getCurrent(); exitEdge; exitEdge = ei.getNext()) {
    ...
    if (!seenExitNodes.get(node->getNumber())) {
      ...
      if (node->getNumber() == regionEntryNode->getNumber()) {
        ...
        properRegion->addExitEdge(clonedWhileNode, node->getNumber(),
                                  isExceptionEdge);
      }
      seenExitNodes.set(node->getNumber());
    }
  ...
  for (newGotoBlockStructure = newGotoBlockStructuresIt.getCurrent();
       newGotoBlockStructure;
       newGotoBlockStructure = newGotoBlockStructuresIt.getNext()) {
    TR::Block *predBlock = toBlock(newGotoBlockStructure->getBlock()
                                       ->getPredecessors()
                                       .front()
                                       ->getFrom());
    clonedWhileLoop->addExternalEdge(predBlock->getStructureOf(),
                                     newGotoBlockStructure->getNumber(), false);
  }
  ...
  for (newGotoBlockStructure = newGotoBlockStructuresIt.getCurrent();
    ...
    TR::Block *predBlock = toBlock(newGotoBlockStructure->getBlock()
                                       ...
                                       .front()
                                       ->getFrom());
    TR::Block *succBlock = toBlock(
        newGotoBlockStructure->getBlock()->getSuccessors().front()->getTo());
    properRegion->addExternalEdge(
        newGotoBlockStructure, succBlock->getStructureOf()->getNumber(), false);
    properRegion->removeExternalEdgeTo(
        predBlock->getStructureOf(), succBlock->getStructureOf()->getNumber());
  }
  ...
  if (seenOSRGuards && safeToRemoveOSRGuards) {
    ListIterator<TR::Block> blocksIt(&blocksInWhileLoop);
    for (TR::Block *block = blocksIt.getCurrent(); block;
         block = blocksIt.getNext()) {
      TR::TreeTop *lastRealTT = block->getLastRealTreeTop();
      if (lastRealTT && lastRealTT->getNode()->isOSRGuard() &&
          performTransformation(comp(),
                                "%sRemove OSR guard n%dn from hot loop\n",
                                OPT_DETAILS_LOOP_VERSIONER,
                                lastRealTT->getNode()->getGlobalIndex())) {
  ...
  if (safeToRemoveHCRGuards && !hcrGuards.isEmpty()) {
    ...
    for (TR::Block *block = blocksIt.getCurrent(); block;
         block = blocksIt.getNext()) {
      TR::TreeTop *lastRealTT = block->getLastRealTreeTop();
      if (lastRealTT && lastRealTT->getNode()->isHCRGuard() &&
          performTransformation(comp(),
                                "%sRemove HCR guard n%dn from hot loop\n",
                                OPT_DETAILS_LOOP_VERSIONER,
                                lastRealTT->getNode()->getGlobalIndex())) {
        block->removeBranch(comp());
      }
  ...
  if (trace()) {
    ...
    // whileLoop->getNumber()); if (comp()->getFlowGraph()->getStructure())
...
void TR_LoopVersioner::buildNullCheckComparisonsTree(
  ...
  for (; nextNode; nextNode = nextNode->getNextElement(),
                   nextTree = nextTree->getNextElement()) {
    // A NULLCHK might have been specialized away, but will still appear in
    // nullCheckTrees Skip such trees
    ...
    if (!nullChkNode->getOpCode().isNullCheck()) {
      TR_ASSERT_FATAL(nullChkNode->getOpCodeValue() == TR::treetop,
                      "Unexpected opcode for n%dn [%p]\n",
    ...
    if (!isNextNodeInvariant) {
      bool nullCheckReferenceisField =
          nextNode->getData()->getOpCode().isLoadIndirect() &&
          nextNode->getData()->getOpCode().hasSymbolReference() &&
          !_seenDefinedSymbolReferences->get(nextNode->getData()
                                                 ->getSymbolReference()
                                                 ->getReferenceNumber()) &&
          nextNode->getData()
              ->getFirstChild()
      ...
      if ((nextNode->getData()->getOpCode().hasSymbolReference() &&
        ...
        TR::Node *invariantNullCheckReference =
            isDependentOnInvariant(nextNode->getData());

        if (!invariantNullCheckReference && nullCheckReferenceisField)
          invariantNullCheckReference =
              isDependentOnInvariant(nextNode->getData()->getFirstChild());

        if (invariantNullCheckReference) {
          TR::Node *dupInvariantNullCheckReference;
          if (nullCheckReferenceisField) {
            dupInvariantNullCheckReference =
                nextNode->getData()->duplicateTreeForCodeMotion();
    ...
    if (!nodeToBeNullChkd) nodeToBeNullChkd = nextNode->getData();

    if (performTransformation(comp(),
                              "%s Creating test outside loop for checking if "
                              "n%un [%p] is null at n%un [%p]\n",
                              OPT_DETAILS_LOOP_VERSIONER,
                              nextNode->getData()->getGlobalIndex(),
                              nextNode->getData(),
                              nextTree->getData()->getNode()->getGlobalIndex(),
                              nextTree->getData()->getNode())) {
      /// TR::Node *duplicateNullCheckReference =
      /// nextNode->getData()->duplicateTree();
      TR::Node *duplicateNullCheckReference = NULL;
      if (isDependent) {
        if (nullCheckReferenceisField)
          duplicateNullCheckReference = nodeToBeNullChkd;
        else
          duplicateNullCheckReference =
              nodeToBeNullChkd->duplicateTreeForCodeMotion();
      } else
        duplicateNullCheckReference =
            nextNode->getData()->duplicateTreeForCodeMotion();

      // TR::Node *nextComparisonNode = TR::Node::createif(TR::ifacmpeq,
      // duplicateNullCheckReference,
      // TR::Node::create(duplicateNullCheckReference, TR::aconst, 0, 0),
      // _exitGotoTarget);
      TR::Node *nextComparisonNode = TR::Node::createif(
          TR::ifacmpeq, duplicateNullCheckReference,
...
void TR_LoopVersioner::buildAwrtbariComparisonsTree(
    List<TR::TreeTop> *awrtbariTrees) {
#ifdef J9_PROJECT_SPECIFIC
  ListElement<TR::TreeTop> *nextTree = awrtbariTrees->getListHead();
  while (nextTree) {
    ...
    if (performTransformation(comp(),
                              "%s Creating test outside loop for checking if "
                              "n%un [%p] requires a write barrier\n",
                              OPT_DETAILS_LOOP_VERSIONER,
                              awrtbariNode->getGlobalIndex(), awrtbariNode)) {
      TR::Node *duplicateBase =
          awrtbariNode->getLastChild()->duplicateTreeForCodeMotion();
      TR::Node *ifNode, *ifNode1, *ifNode2;

      // If we can guarantee a fixed tenure size we can hardcode size constants
      bool isVariableHeapBase =
          comp()->getOptions()->isVariableHeapBaseForBarrierRange0();
      bool isVariableHeapSize =
          comp()->getOptions()->isVariableHeapSizeForBarrierRange0();

      TR_J9VMBase *fej9 = (TR_J9VMBase *)(comp()->fe());

      if (isVariableHeapBase || isVariableHeapSize) {
        ifNode1 = TR::Node::create(
            TR::acmpge, 2, duplicateBase,
            TR::Node::createWithSymRef(
                TR::aload, 0,
                comp()
                    ->getSymRefTab()
                    ->findOrCreateThreadLowTenureAddressSymbolRef()));
      } else {
        ifNode1 = TR::Node::create(
            TR::acmpge, 2, duplicateBase,
      ...
      if (isVariableHeapBase || isVariableHeapSize) {
        ifNode2 = TR::Node::create(
            TR::acmplt, 2, duplicateBase,
            TR::Node::createWithSymRef(
                TR::aload, 0,
                comp()
                    ->getSymRefTab()
                    ->findOrCreateThreadHighTenureAddressSymbolRef()));
      } else {
        ifNode2 = TR::Node::create(
            TR::acmplt, 2, duplicateBase,
      ...
      ifNode = TR::Node::createif(
          TR::ificmpne, TR::Node::create(TR::iand, 2, ifNode1, ifNode2),
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(new (
            _curLoop->_memRegion) RemoveWriteBarrier(this, prep, awrtbariNode));
      }
    }
...
void TR_LoopVersioner::RemoveWriteBarrier::improveLoop() {
  ...
  TR_ASSERT_FATAL(_awrtbariNode->getOpCodeValue() == TR::awrtbari,
                  "unexpected opcode");
  _awrtbariNode->setSkipWrtBar(true);
}

void TR_LoopVersioner::buildDivCheckComparisonsTree(
    List<TR::TreeTop> *divCheckTrees) {
  ...
  while (nextTree) {
    ...
    if (performTransformation(comp(),
                              "%s Creating test outside loop for checking if "
                              "n%un [%p] is divide by zero\n",
                              OPT_DETAILS_LOOP_VERSIONER,
                              divCheckNode->getGlobalIndex(), divCheckNode)) {
      TR::Node *duplicateDivisor = divCheckNode->getFirstChild()
                                       ->getSecondChild()
                                       ->duplicateTreeForCodeMotion();
      TR::Node *ifNode;
      if (duplicateDivisor->getType().isInt64())
        ifNode = TR::Node::createif(
            TR::iflcmpeq, duplicateDivisor,
            TR::Node::create(duplicateDivisor, TR::lconst, 0, 0),
            _exitGotoTarget);
      else
        ifNode = TR::Node::createif(
            TR::ificmpeq, duplicateDivisor,
            TR::Node::create(duplicateDivisor, TR::iconst, 0, 0),
            _exitGotoTarget);
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(new (
            _curLoop->_memRegion) RemoveDivCheck(this, prep, divCheckNode));
      }
    }
...
void TR_LoopVersioner::RemoveDivCheck::improveLoop() {
  ...
  TR_ASSERT_FATAL(_divCheckNode->getOpCodeValue() == TR::DIVCHK,
                  "unexpected opcode");
  TR::Node::recreate(_divCheckNode, TR::treetop);
}

void TR_LoopVersioner::buildCheckCastComparisonsTree(
  ...
  while (nextTree) {
    ...
    if (!performTransformation(comp(),
                               "%s Creating test outside loop for checking "
                               "that checkcast n%un [%p] passes\n",
                               OPT_DETAILS_LOOP_VERSIONER,
    ...
    TR::Node *duplicateClassPtr =
        checkCastNode->getSecondChild()->duplicateTreeForCodeMotion();
    TR::Node *duplicateCheckedValue =
        checkCastNode->getFirstChild()->duplicateTreeForCodeMotion();
    TR::Node *instanceofNode = TR::Node::createWithSymRef(
        TR:: instanceof, 2, 2, duplicateCheckedValue, duplicateClassPtr,
                       comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(
                           comp()->getMethodSymbol()));
    TR::Node *ificmpeqNode = TR::Node::createif(
        TR::ificmpeq, instanceofNode,
        TR::Node::create(checkCastNode, TR::iconst, 0, 0), _exitGotoTarget);

    LoopEntryPrep *prep =
        createLoopEntryPrep(LoopEntryPrep::TEST, ificmpeqNode);
    if (prep != NULL) {
      nodeWillBeRemovedIfPossible(checkCastNode, prep);
      _curLoop->_loopImprovements.push_back(new (
          _curLoop->_memRegion) RemoveCheckCast(this, prep, checkCastTree));
    }
...
void TR_LoopVersioner::RemoveCheckCast::improveLoop() {
  ...
  TR_ASSERT_FATAL(checkCastNode->getOpCode().isCheckCast(),
                  "unexpected opcode");
  ...
  TR::TreeTop *firstNewTree = TR::TreeTop::create(
      comp(), TR::Node::create(TR::treetop, 1, checkCastNode->getFirstChild()),
      NULL, NULL);
  TR::TreeTop *secondNewTree = TR::TreeTop::create(
      comp(), TR::Node::create(TR::treetop, 1, checkCastNode->getSecondChild()),
      NULL, NULL);
...
void TR_LoopVersioner::buildArrayStoreCheckComparisonsTree(
    List<TR::TreeTop> *arrayStoreCheckTrees) {
  ...
  while (nextTree) {
    ...
    TR::Node *ificmpeqNode = TR::Node::createif(
        TR::ificmpeq, instanceofNode,
        TR::Node::create(arrayStoreCheckNode, TR::iconst, 0, 0),
        _exitGotoTarget);
    ...
    if (prep != NULL) {
      ...
      _curLoop->_loopImprovements.push_back(
          new (_curLoop->_memRegion)
              RemoveArrayStoreCheck(this, prep, arrayStoreCheckTree));
    }
...
void TR_LoopVersioner::RemoveArrayStoreCheck::improveLoop() {
  ...
  TR_ASSERT_FATAL(arrayStoreCheckNode->getOpCodeValue() == TR::ArrayStoreCHK,
                  "unexpected opcode");
  ...
  TR::TreeTop *firstNewTree = TR::TreeTop::create(
      comp(),
      TR::Node::create(TR::treetop, 1, arrayStoreCheckNode->getFirstChild()),
      NULL, NULL);
  TR::Node *child = arrayStoreCheckNode->getFirstChild();
  if (child->getOpCodeValue() == TR::awrtbari &&
      TR::Compiler->om.writeBarrierType() == gc_modron_wrtbar_none &&
      performTransformation(comp(),
                            "%sChanging awrtbari node [%p] to an iastore\n",
  ...
  if (arrayStoreCheckNode->getNumChildren() > 1) {
    TR_ASSERT((arrayStoreCheckNode->getNumChildren() == 2),
              "Unknown array store check tree\n");
    secondNewTree = TR::TreeTop::create(
        comp(),
        TR::Node::create(TR::treetop, 1, arrayStoreCheckNode->getSecondChild()),
        NULL, NULL);
...
void TR_LoopVersioner::convertSpecializedLongsToInts(
    TR::Node *node, vcount_t visitCount, TR::SymbolReference **symRefs) {
  if (node->getVisitCount() == visitCount) return;
  ...
  if ((node->getType().isInt64()) && node->getOpCode().isLoadVar()) {
    TR::SymbolReference *tempSymRef =
        symRefs[node->getSymbolReference()->getReferenceNumber()];
    if (tempSymRef) {
      TR::Node::recreate(node, TR::iu2l);
      TR::Node *newLoad =
          TR::Node::createWithSymRef(node, TR::iload, 0, tempSymRef);
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++)
    convertSpecializedLongsToInts(node->getChild(childNum), visitCount,
                                  symRefs);
}
...
bool nodeTreeContainsOpCode(TR::Node *n, TR::ILOpCodes op) {
  if (n->getOpCodeValue() == op) return true;
  for (int32_t i = 0; i < n->getNumChildren(); ++i) {
    if (nodeTreeContainsOpCode(n->getChild(i), op)) return true;
  }
  return false;
}

bool TR_LoopVersioner::buildLoopInvariantTree(
    List<TR_NodeParentSymRef> *invariantNodes) {
  TR::NodeChecklist seen(comp());

  ListElement<TR_NodeParentSymRef> *nextInvariantNode =
      invariantNodes->getListHead();
  while (nextInvariantNode) {
    ...
    if (performTransformation(
            comp(), "%s Attempting to hoist n%un [%p] out of the loop\n",
            OPT_DETAILS_LOOP_VERSIONER, invariantNode->getGlobalIndex(),
            invariantNode)) {
      LoopEntryPrep *prep = createLoopEntryPrep(LoopEntryPrep::PRIVATIZE,
...
bool TR_LoopVersioner::buildSpecializationTree(
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *divCheckTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    List<TR::Node> *comparisonTrees, List<TR::Node> *specializedNodes,
    TR::Block *loopInvariantBlock, TR::SymbolReference **symRefs) {
  if (!comp()->getRecompilationInfo()) return false;
  ...
  while (nextSpecializedNode) {
    ...
    if (!isNextNodeInvariant) {
      if (specializedNode->getOpCode().hasSymbolReference() &&
        ...
        TR::Node *invariantSpecializedNode =
            isDependentOnInvariant(specializedNode);
        if (invariantSpecializedNode)
          dupSpecializedNode =
              invariantSpecializedNode->duplicateTreeForCodeMotion();
        else
          TR_ASSERT(0,
                    "Should not be considering profiled expr for versioning\n");
      } else
        TR_ASSERT(0,
                  "Should not be considering profiled expr for versioning\n");
    }
    ...
    if (dupSpecializedNode) nodeToBeChecked = dupSpecializedNode;
    ...
    if (performTransformation(comp(),
      ...
      TR_ValueInfo *valueInfo = static_cast<TR_ValueInfo *>(
          TR_ValueProfileInfoManager::getProfiledValueInfo(specializedNode,
                                                           comp(), ValueInfo));
      ...
      if (specializedNode->getType().isInt64()) {
        TR::Node *highInt =
            TR::Node::create(TR::land, 2, duplicateNode,
                             TR::Node::create(duplicateNode, TR::lconst, 0));
        ...
        comparisonNode = TR::Node::createif(
            TR::ificmpne, duplicateNode,
            TR::Node::create(duplicateNode, TR::iconst, 0, value),
            _exitGotoTarget);
      }
      comparisonTrees->add(comparisonNode);
      dumpOptDetails(comp(),
                     "The node %p has been created for testing if value "
                     "profiling check is required\n",
                     comparisonNode);

      int32_t childNum;
      for (childNum = 0; childNum < specializedNode->getNumChildren();
           childNum++)
      ...
      if (specializedNode->getType().isInt64()) {
        if (specializedNode->getOpCode().isLoadVar()) {
          TR::SymbolReference *tempSymRef =
              symRefs[specializedNode->getSymbolReference()
                          ->getReferenceNumber()];
          if (!tempSymRef) {
            specializedLong = true;
            tempSymRef = comp()->getSymRefTab()->createTemporary(
                comp()->getMethodSymbol(), TR::Int32);
            symRefs[specializedNode->getSymbolReference()
                        ->getReferenceNumber()] = tempSymRef;
            TR::Node *newStore = TR::Node::createWithSymRef(
                TR::istore, 1, 1,
                TR::Node::create(TR::l2i, 1, specializedNode->duplicateTree()),
                tempSymRef);
          ...
          TR::Node *newLoad = TR::Node::createWithSymRef(
              specializedNode, TR::iload, 0, tempSymRef);
          ...
          TR_ASSERT(
              0, "Unexpected long opcode being profiled/specialized in loop\n");
      } else {
...
static void cleanseIntegralNodeFlagsInSubtree(TR::Compilation *comp,
                                              TR::Node *node) {
  dumpOptDetails(
      comp,
      "%sCleansing Integer/Unsigned related flags under node n%in subtree\n",
      OPT_DETAILS_LOOP_VERSIONER, node->getGlobalIndex());

  TR::deque<TR::Node *, TR::Region &> queue(
  ...
  while (!queue.empty()) {
    ...
    queue.pop_front();
    if (seen.get(tmp->getGlobalIndex())) continue;
    ...
    for (int i = 0; i < tmp->getNumChildren(); i++) {
      ...
      if (!seen.get(child->getGlobalIndex())) queue.push_back(child);
    }
  }
}
...
void TR_LoopVersioner::buildConditionalTree(
  ...
  while (nextTree) {
    ...
    if (conditionalNode->isTheVirtualGuardForAGuardedInlinedCall() &&
      ...
      TR_VirtualGuard *guardInfo =
          comp()->findVirtualGuardInfo(conditionalNode);
      if (guardInfo->getTestType() == TR_VftTest) {
        ...
        if ((conditionalNode->getFirstChild()->getNumChildren() > 0) &&
            (conditionalNode->getFirstChild()
                 ->getFirstChild()
                 ->getNumChildren() > 0)) {
          thisChild = conditionalNode->getFirstChild()
                          ->getFirstChild()
                          ->getFirstChild();
          searchReqd = false;
      ...
      if (!b && !nodeToBeChecked) {
        if (thisChild->getOpCode().hasSymbolReference() &&
          ...
          if (dupThisChild) {
            ...
            if (guardInfo->getTestType() == TR_VftTest) {
              invariantThisChild =
                  conditionalNode->getFirstChild()->getFirstChild();
              conditionalNode->getFirstChild()->setAndIncChild(
                  0, dupThisChild->duplicateTree());
            } else {
              TR_ASSERT((guardInfo->getTestType() == TR_MethodTest),
                        "Unknown guard type @ node kind %d test %d %p\n",
                        conditionalNode, guardInfo->getKind(),
                        guardInfo->getTestType());
              invariantThisChild = conditionalNode->getFirstChild()
                                       ->getFirstChild()
                                       ->getFirstChild();
            ...
            TR_ASSERT(0,
                      "Cannot version with respect to non-invariant guard\n");
        } else
    ...
    if (performTransformation(comp(),
      ...
      //
      // We pulled out from both on first conditional and all the subsequent
      // conditionals we only pull from fast path This exposed a bug where slow
      // path had an unconditional that removed a valid path. Fix is only
      // allowing unconditioning in both if there is single if conditional tree.
      //
      if ((_conditionalTree == origConditionalNode) &&
          (conditionalTrees->getSize() == 1))
      ...
      TR::Block *fallThroughBlock =
          conditionalTree->getEnclosingBlock()->getNextBlock();

      if (trace())
        traceMsg(comp(),
                 "Frequency Test for conditional node [%p], destination "
                 "Frequency %d, fallThrough frequency %d\n",
                 conditionalNode, destBlock->getFrequency(),
                 fallThroughBlock->getFrequency());
      if (reverseBranchInLoops[origConditionalNode->getGlobalIndex()]) {
        if (trace())
          traceMsg(comp(),
                   "Branch reversed for conditional node [%p], destination "
                   "Frequency %d, fallThrough frequency %d\n",
                   conditionalNode, destBlock->getFrequency(),
                   fallThroughBlock->getFrequency());
        reverseBranch = true;
      ...
      if (conditionalNode->isVersionableIfWithMaxExpr() ||
          conditionalNode->isVersionableIfWithMinExpr()) {
        ...
        for (int32_t childNum = 0; childNum < conditionalNode->getNumChildren();
             childNum++) {
          child = conditionalNode->getChild(childNum);
          // TR::Node *indexNode=NULL;
          if (!isExprInvariant(conditionalNode->getChild(childNum))) {
            if (trace())
              traceMsg(comp(), "Versioning if conditional node [%p]\n",
                       conditionalNode);
            while (child->getOpCode().isAdd() || child->getOpCode().isSub() ||
                   child->getOpCode().isMul()) {
              if (child->getSecondChild()->getOpCode().isLoadConst()) {
            ...
            if (!child || !child->getOpCode().hasSymbolReference()) {
              ...
              ListElement<int32_t> *versionableInductionVar =
                  _versionableInductionVariables.getListHead();
              while (versionableInductionVar) {
                loopDrivingInductionVariable =
                    *(versionableInductionVar->getData());
                if (symRefNum == *(versionableInductionVar->getData())) {
                  if (_additionInfo->get(symRefNum)) isAddition = true;
                  isInductionVar = true;
                  indexNode = child;
            ...
            if (isInductionVar) indexReferenceCount++;
          }
        }
        ...
        if (isInductionVar &&
          ...
          TR::SymbolReference *loopDrivingSymRef =
              indexNode->getSymbolReference();

          TR::Node *loopLimit = NULL;
          TR::Node *range = NULL;
          TR::Node *entryNode =
              TR::Node::createLoad(conditionalNode, loopDrivingSymRef);
          TR::Node *storeNode =
              _storeTrees[indexNode->getSymbolReference()->getReferenceNumber()]
                  ->getNode();
          // int32_t exitValue = exitValue =
          // storeNode->getFirstChild()->getSecondChild()->getInt();

          loopLimit =
              _loopTestTree->getNode()->getSecondChild()->duplicateTree(comp());
          if (isAddition) {
          ...
          while (valueChild->getOpCode().isAdd() ||
                 valueChild->getOpCode().isSub()) {
            if (valueChild->getOpCode().isAdd())
              additiveConstantInStore = additiveConstantInStore +
                                        valueChild->getSecondChild()->getInt();
            else
              additiveConstantInStore = additiveConstantInStore -
                                        valueChild->getSecondChild()->getInt();
          ...
          if (incrementI < 0) incrementI = -incrementI;
          if (incrementJ < 0) incrementJ = -incrementJ;
          TR::Node *incrNode =
              TR::Node::create(parent, TR::iconst, 0, incrementI);
          TR::Node *incrJNode =
              TR::Node::create(parent, TR::iconst, 0, incrementJ);
          ...
          TR::Node *ceilingNode =
              TR::Node::create(TR::icmpne, 2, remNode, zeroNode);
          numIterations =
              TR::Node::create(TR::iadd, 2, numIterations, ceilingNode);
          ...
          TR::Node *extraIter =
              TR::Node::create(TR::icmpeq, 2, remNode, zeroNode);
          extraIter = TR::Node::create(
              TR::iand, 2, extraIter,
              TR::Node::create(parent, TR::iconst, 0, condValue));
          numIterations =
              TR::Node::create(TR::iadd, 2, numIterations, extraIter);
          numIterations =
              TR::Node::create(TR::imul, 2, numIterations, incrJNode);

          TR::Node *adjustMaxValue = NULL;
          if (isAddition)
            adjustMaxValue =
                TR::Node::create(parent, TR::iconst, 0, incrementJ + 1);
          else
            adjustMaxValue =
                TR::Node::create(parent, TR::iconst, 0, incrementJ);
          ...
          TR_ASSERT(indexChildIndex >= 0,
                    "Index child index should be valid at this point\n");
          duplicateIndexNode =
              conditionalNode->getChild(indexChildIndex)->duplicateTree(comp());
          ...
          if (duplicateIndexNode->getOpCode().isLoad() &&
              duplicateIndexNode->getSymbolReference()->getReferenceNumber() ==
                  indexSymRefNum) {
            duplicateIndexNode = maxValue;
          } else {
            copyOnWriteNode(origConditionalNode, &conditionalNode);
            replaceInductionVariable(conditionalNode, duplicateIndexNode,
                                     indexChildIndex, indexSymRefNum, maxValue,
                                     visitCount);
          }
        }
      }
        ...
        if (indexNodeOccursAsSecondChild) {
          if (!reverseBranch)
            duplicateComparisonNode = TR::Node::createif(
                conditionalNode->getOpCodeValue(),
                conditionalNode->getFirstChild()->duplicateTree(comp()),
                duplicateIndexNode, _exitGotoTarget);
          else
            duplicateComparisonNode = TR::Node::createif(
                conditionalNode->getOpCode().getOpCodeForReverseBranch(),
                conditionalNode->getFirstChild()->duplicateTree(comp()),
                duplicateIndexNode, _exitGotoTarget);
        } else {
          if (!reverseBranch)
            duplicateComparisonNode = TR::Node::createif(
                conditionalNode->getOpCodeValue(), duplicateIndexNode,
                conditionalNode->getSecondChild()->duplicateTree(comp()),
                _exitGotoTarget);
          else
            duplicateComparisonNode = TR::Node::createif(
                conditionalNode->getOpCode().getOpCodeForReverseBranch(),
      ...
      if (duplicateComparisonNode->getFirstChild()->getOpCodeValue() == TR::
              instanceof) {
        duplicateComparisonNode->getFirstChild()->getFirstChild()->setIsNull(
            false);
        duplicateComparisonNode->getFirstChild()->getFirstChild()->setIsNonNull(
            false);
      } else {
        if (duplicateComparisonNode->getFirstChild()->getOpCodeValue() !=
            TR::loadaddr) {
          duplicateComparisonNode->getFirstChild()->setIsNull(false);
          duplicateComparisonNode->getFirstChild()->setIsNonNull(false);
        }
        if (duplicateComparisonNode->getSecondChild()->getOpCodeValue() !=
            TR::loadaddr) {
      ...
      if (prep != NULL) {
        ...
        if (reverseBranch) _curLoop->_takenBranches.add(origConditionalNode);

        _curLoop->_loopImprovements.push_back(
            new (_curLoop->_memRegion)
                FoldConditional(this, prep, origConditionalNode, reverseBranch,
                                /* original = */ true));
        ...
        if (changeConditionalToUnconditionalInBothVersions &&
          ...
          _curLoop->_loopImprovements.push_back(
              new (_curLoop->_memRegion) FoldConditional(
                  this, prep, _duplicateConditionalTree, reverseBranch,
                  /* original = */ false));
        }
      }
    }
...
void TR_LoopVersioner::FoldConditional::improveLoop() {
  ...
  if (_conditionalNode->isTheVirtualGuardForAGuardedInlinedCall()) {
    ...
    if (callNode) {
      ...
      if (_original) _versioner->_guardedCalls.add(callNode);
    }
  }
...
void TR_LoopVersioner::copyOnWriteNode(TR::Node *original, TR::Node **current) {
  if (*current != original) return;  // already copied
  ...
  *current = original->duplicateTree();
  ...
  if (comp()->getOutFile() != NULL &&
    ...
    comp()->getDebug()->printWithFixedPrefix(comp()->getOutFile(), original, 1,
                                             true, false, "\t\t");
    dumpOptDetails(comp(), "\n\tduplicate node:\n");
    comp()->getDebug()->printWithFixedPrefix(comp()->getOutFile(), *current, 1,
                                             true, false, "\t\t");
  }
}

bool TR_LoopVersioner::replaceInductionVariable(TR::Node *parent,
                                                TR::Node *node, int childNum,
                                                int loopDrivingInductionVar,
                                                TR::Node *loopLimit,
                                                int visitCount) {
  if (node->getVisitCount() == visitCount) return false;
  ...
  int i;
  for (i = 0; i < node->getNumChildren(); i++) {
    if (replaceInductionVariable(node, node->getChild(i), i,
                                 loopDrivingInductionVar, loopLimit,
                                 visitCount))
      return true;
  }
  return false;
...
void TR_LoopVersioner::buildSpineCheckComparisonsTree(
    List<TR::TreeTop> *spineCheckTrees) {
  ListElement<TR::TreeTop> *nextTree = spineCheckTrees->getListHead();
  ...
  while (nextTree) {
    ...
    if (performTransformation(comp(),
                              ...
                              spineCheckNode)) {
      TR::Node *contigArrayLength = TR::Node::create(
          TR::contigarraylength, 1, arrayBase->duplicateTreeForCodeMotion());
      TR::Node *nextComparisonNode = TR::Node::createif(
          TR::ificmpne, contigArrayLength,
      ...
      if (op == TR::SpineCHK) {
        ...
        TR_ASSERT_FATAL(
            ...
            "missing prep for removal of bound check from BNDCHKwithSpineCHK "
            "n%un [%p]",
            spineCheckNode->getGlobalIndex(), spineCheckNode);

        LoopEntryPrep *prereq = prereqEntry->second;
        prep = createChainedLoopEntryPrep(LoopEntryPrep::TEST,
                                          nextComparisonNode, prereq);
      }
      ...
      if (prep != NULL) {
        ...
        _curLoop->_loopImprovements.push_back(
            new (_curLoop->_memRegion)
                RemoveSpineCheck(this, prep, nextTree->getData()));
      }
    }
...
void TR_LoopVersioner::RemoveSpineCheck::improveLoop() {
  ...
  TR_ASSERT_FATAL(spineCheckNode->getOpCodeValue() == TR::SpineCHK,
                  "unexpected opcode");
  ...
  if (is64BitTarget) {
    ...
    hdrSize->setLongInt(
        (int64_t)TR::Compiler->om.discontiguousArrayHeaderSizeInBytes());
  } else
    hdrSize = TR::Node::create(
        spineCheckNode, TR::iconst, 0,
        (int32_t)TR::Compiler->om.discontiguousArrayHeaderSizeInBytes());

  int32_t shift = TR::TransformUtil::convertWidthToShift(
      TR::Compiler->om.sizeofReferenceField());
  TR::Node *shiftNode = is64BitTarget
                            ? TR::Node::lconst(spineCheckNode, (int64_t)shift)
  ...
  if (strideShift)
    strideShiftNode =
        is64BitTarget ? TR::Node::lconst(spineCheckNode, (int64_t)strideShift)
                      : TR::Node::iconst(spineCheckNode, strideShift);

  int32_t arraySpineShift = fe()->getArraySpineShift(elementSize);
  TR::Node *spineShiftNode =
      is64BitTarget ? TR::Node::lconst(spineCheckNode, (int64_t)arraySpineShift)
  ...
  node = TR::Node::create(is64BitTarget ? TR::lshr : TR::ishr, 2, spineIndex,
                          spineShiftNode);
  node =
      TR::Node::create(is64BitTarget ? TR::lshl : TR::ishl, 2, node, shiftNode);
  node =
      TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, node, hdrSize);
  node = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, arrayBase,
                          node);
  ...
  if (is64BitTarget) {
    ...
    leafOffset = TR::Node::create(spineCheckNode, TR::iconst, 0,
                                  (int32_t)fe()->getArrayletMask(elementSize));
  }

  TR::Node *nodeLeafOffset = TR::Node::create(
      is64BitTarget ? TR::land : TR::iand, 2, leafOffset, spineIndex);

  if (strideShiftNode) {
    nodeLeafOffset = TR::Node::create(is64BitTarget ? TR::lshl : TR::ishl, 2,
                                      nodeLeafOffset, strideShiftNode);
  }

  node = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, node,
                          nodeLeafOffset);
  TR::TreeTop *compressTree = NULL;
  if (comp()->useCompressedPointers()) {
    compressTree = TR::TreeTop::create(
        comp(), TR::Node::createCompressedRefsAnchor(node->getFirstChild()),
        NULL, NULL);
  }
...
void TR_LoopVersioner::buildBoundCheckComparisonsTree(
  ...
  while (nextTree) {
    ...
    int32_t boundChildIndex =
        (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) ? 2 : 0;
      ...
      if (arrayLengthNode->getOpCode().isArrayLength()) {
        ...
        if (arrayObject->getOpCode().hasSymbolReference() &&
          ...
          if (invariantArrayObject) {
            TR::Node *dupInvariantArrayObject =
                invariantArrayObject->duplicateTree();
            arrayLengthNode = TR::Node::create(
                arrayLengthNode, arrayLengthNode->getOpCodeValue(), 1,
                dupInvariantArrayObject);
          } else
            TR_ASSERT(0,
                      "Should not be considering bound check for versioning\n");
        } else
          TR_ASSERT(0,
                    "Should not be considering bound check for versioning\n");
      } else {
    ...
    int32_t indexChildIndex =
        (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) ? 3 : 1;
    ...
    if (isIndexInvariant) {
      nextComparisonNode = TR::Node::createif(
          TR::ificmplt, indexNode->duplicateTreeForCodeMotion(),
      ...
      if (comp()->requiresSpineChecks())
        findAndReplaceContigArrayLen(NULL, nextComparisonNode,
                                     comp()->incVisitCount());
      ...
      if (boundCheckNode->getOpCodeValue() == TR::BNDCHK ||
        ...
        nextComparisonNode = TR::Node::createif(
            TR::ificmpge, indexNode->duplicateTreeForCodeMotion(),
            arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
      else
        nextComparisonNode = TR::Node::createif(
            TR::ificmpgt, indexNode->duplicateTreeForCodeMotion(),
            arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
      ...
      prep = createChainedLoopEntryPrep(LoopEntryPrep::TEST, nextComparisonNode,
                                        prep);
      ...
      if (indexSymRef) origIndexSymRef = indexSymRef;
      ...
      while (indexSymRef && changedIndexSymRef) {
        ...
        while (versionableInductionVar) {
          ...
          if (indexSymRefNum == *(versionableInductionVar->getData())) {
            if (_additionInfo->get(indexSymRefNum)) isAddition = true;
        ...
        if (!foundInductionVariable) {
          ...
          while (versionableInductionVar) {
            if (indexSymRefNum == *(versionableInductionVar->getData())) {
              ...
              exitValue = _storeTrees[indexSymRefNum]
                              ->getNode()
                              ->getFirstChild()
                              ->getSecondChild()
                              ->getInt();
        ...
        if (!foundInductionVariable) {
          ...
          while (versionableInductionVar) {
            if (indexSymRefNum == *(versionableInductionVar->getData())) {
              // printf("Eliminating a BNDCHK on derived induction variable in
              // %s\n", comp()->signature());
              isDerivedInductionVariable = true;
              if (_additionInfo->get(indexSymRefNum)) isAddition = true;
              // exitValue =
        ...
        if ((isDerivedInductionVariable &&
             (loopDrivingInductionVariable == -1)) ||
          ...
          if (isDerivedInductionVariable &&
            ...
            TR_InductionVariable *v =
                _currentNaturalLoop->findMatchingIV(symRefInCompare);
            if (v) {
              if (_additionInfo->get(symRefInCompare->getReferenceNumber())) {
                isLoopDrivingAddition = true;
              }
              loopDrivingInductionVariable =
                  symRefInCompare->getReferenceNumber();
              // printf("Reached here in method %s\n", comp()->signature());
            }
          } else {
            TR_InductionVariable *v =
                _currentNaturalLoop->findMatchingIV(indexSymRef);
            if (v) {
        ...
        if (!foundInductionVariable) {
          indexChild = isDependentOnInductionVariable(
              indexChild, changedIndexSymRefAtSomePoint, isIndexChildMultiplied,
              mulNode, strideNode, indVarOccursAsSecondChildOfSub);
      ...
      if (!isAddition) complexButPredictableForm = false;

      int32_t additiveConstantInStore = 0;
      int32_t additiveConstantInIndex = 0;

      TR::Node *storeNode =
          _storeTrees[indexChild->getSymbolReference()->getReferenceNumber()]
              ->getNode();
      ...
      while (valueChild->getOpCode().isAdd() ||
        ...
        if (valueChild->getSecondChild()->getOpCode().isLoadConst()) {
          ...
          break;
          // while (indexChild->getOpCode().isAdd() ||
          // indexChild->getOpCode().isSub() || indexChild->getOpCode().isAnd())
          //   {
          //   if (indexChild->getSecondChild()->getOpCode().isLoadConst())
          //      indexChild = indexChild->getFirstChild();
          //   else
          ...
          //        visitCount = comp()->incVisitCount();
          //        bool isSecondChildInvariant =
          //        isExprInvariant(indexChild->getSecondChild(), visitCount);
          //        if (isSecondChildInvariant)
          //       indexChild = indexChild->getFirstChild();
          //        else
          //       {
          //           visitCount = comp()->incVisitCount();
          //           bool isFirstChildInvariant =
          //           isExprInvariant(indexChild->getFirstChild(), visitCount);
          //       if (isFirstChildInvariant)
          //              indexChild = indexChild->getSecondChild();
          //       else
          //      break;
      ...
      if (complexButPredictableForm) {
        ...
        while (cursorChild->getOpCode().isAdd() ||
               cursorChild->getOpCode().isSub()) {
          if (cursorChild->getSecondChild()->getOpCode().isLoadConst()) {
            if (cursorChild->getOpCode().isAdd())
              additiveConstantInIndex = additiveConstantInIndex +
                                        cursorChild->getSecondChild()->getInt();
            else
              additiveConstantInIndex = additiveConstantInIndex -
                                        cursorChild->getSecondChild()->getInt();
        ...
        if (cursorChild != indexChild) complexButPredictableForm = false;

        if (additiveConstantInIndex == 0) complexButPredictableForm = false;
      }
      ...
      if (isSpecialInductionVariable) {
        ...
        nextComparisonNode = TR::Node::createif(
            TR::ificmplt,
            boundCheckNode->getChild(indexChildIndex)
                ->duplicateTreeForCodeMotion(),
            TR::Node::create(boundCheckNode, TR::iconst, 0, 0),
            _exitGotoTarget);
        if (trace())
          traceMsg(
              comp(),
              "Induction variable added in each iter -> Creating %p (%s)\n",
              nextComparisonNode, nextComparisonNode->getOpCode().getName());
      } else {
        TR::Node *duplicateIndex = boundCheckNode->getChild(indexChildIndex)
                                       ->duplicateTreeForCodeMotion();

        if (changedIndexSymRefAtSomePoint ||
            !_unchangedValueUsedInBndCheck->get(
                boundCheckNode->getGlobalIndex())) {
          if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
            TR::Node *storeNode =
                _storeTrees[origIndexSymRef->getReferenceNumber()]->getNode();
            TR::Node *storeRhs = storeNode->getFirstChild()->duplicateTree();
            TR::Node *replacementLoad = TR::Node::createWithSymRef(
                storeNode,
                comp()->il.opCodeForDirectLoad(storeNode->getDataType()), 0,
                indexSymRef);

            if (!storeRhs->getOpCode().isLoad()) {
              int visitCount = comp()->incVisitCount();
              replaceInductionVariable(NULL, storeRhs, -1,
                                       changedIndexSymRefNumForStoreRhs,
            ...
            if (!duplicateIndex->getOpCode().isLoad()) {
              vcount_t visitCount = comp()->incVisitCount();
              replaceInductionVariable(NULL, duplicateIndex, -1,
                                       origIndexSymRef->getReferenceNumber(),
                                       storeRhs, visitCount);
            } else
        ...
        if ((isAddition && !indVarOccursAsSecondChildOfSub) ||
          ...
          nextComparisonNode = TR::Node::createif(
              TR::ificmpge, duplicateIndex,
              arrayLengthNode->duplicateTreeForCodeMotion(), _exitGotoTarget);
          nextComparisonNode->setIsVersionableIfWithMaxExpr(comp());
          if (trace())
            traceMsg(
                comp(),
                "Induction variable subed in each iter -> Creating %p (%s)\n",
                nextComparisonNode, nextComparisonNode->getOpCode().getName());
        }
      }
      ...
      prep = createLoopEntryPrep(LoopEntryPrep::TEST, nextComparisonNode);
      dumpOptDetails(
          comp(), "1: Prep %p has been created for testing if exceed bounds\n",
          prep);
      ...
      TR::Node *duplicateIndex =
          boundCheckNode->getChild(indexChildIndex)->duplicateTree();
      if (changedIndexSymRefAtSomePoint ||
          (!_unchangedValueUsedInBndCheck->get(
               boundCheckNode->getGlobalIndex()) &&
      ...
      if (isLoopDrivingInductionVariable || isDerivedInductionVariable) {
        ...
        if (isDerivedInductionVariable)
          loopDrivingSymRef =
              comp()->getSymRefTab()->getSymRef(loopDrivingInductionVariable);

        TR::Node *entryNode =
            TR::Node::createLoad(boundCheckNode, loopDrivingSymRef);
        TR::Node *numIterations = NULL;
        TR::Node *range = NULL;
        ...
        //
        if (isDerivedInductionVariable) {
          TR::Node *indexChild = _storeTrees[loopDrivingInductionVariable]
                                     ->getNode()
                                     ->getFirstChild();
          ...
          while (indexChild->getOpCode().isAdd() ||
            ...
            if (indexChild->getSecondChild()->getOpCode().isLoadConst()) {
              if (indexChild->getOpCode().isAdd())
                incrementI =
                    incrementI + indexChild->getSecondChild()->getInt();
              else
                incrementI =
                    incrementI - indexChild->getSecondChild()->getInt();
              indexChild = indexChild->getFirstChild();
        ...
        if (incrementJ < 0) incrementJ = -incrementJ;
        TR::Node *incrNode =
            TR::Node::create(boundCheckNode, TR::iconst, 0, incrementI);
        TR::Node *incrJNode =
            TR::Node::create(boundCheckNode, TR::iconst, 0, incrementJ);
        ...
        if (isIndexChildMultiplied)
          adjustMaxValue =
              TR::Node::create(boundCheckNode, TR::iconst, 0, incrementJ + 1);
        else
          adjustMaxValue =
              TR::Node::create(boundCheckNode, TR::iconst, 0, incrementJ);
        TR::Node *maxValue = NULL;
        ...
        if (isLoopDrivingInductionVariable) {
          if (isLoopDrivingAddition) addOp = TR::iadd;
        } else  // isDerivedInductionVariable
        {
          if (isAddition) addOp = TR::iadd;
        }
      ...
      if (!duplicateIndex->getOpCode().isLoad()) {
        ...
        */
        replaceInductionVariable(NULL, duplicateIndex, -1, indexSymRefNum,
                                 loopLimit, visitCount);
      ...
      if (isSpecialInductionVariable) {
        ...
        if (trace())
          traceMsg(comp(),
                   "Special Induction variable added in each iter -> Creating "
                   "%p (%s)\n",
                   nextComparisonNode,
        ...
        prep = createChainedLoopEntryPrep(LoopEntryPrep::TEST,
                                          nextComparisonNode, prep);

        nextComparisonNode = TR::Node::createif(
            TR::ifiucmpgt, correctCheckNode, arrayLengthNode->duplicateTree(),
            _exitGotoTarget);
        if (trace())
          traceMsg(comp(),
                   "Special Induction variable added in each iter -> Creating "
        ...
        if (isAddition) {
          if (!indVarOccursAsSecondChildOfSub) {
            if (isLoopDrivingInductionVariable) {
              if ((_loopTestTree->getNode()->getOpCodeValue() ==
                   TR::ificmple) ||
                  (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt))
                nextComparisonNode = TR::Node::createif(
                    TR::ifiucmpge, correctCheckNode,
            ...
            if (isLoopDrivingInductionVariable) {
              if ((_loopTestTree->getNode()->getOpCodeValue() ==
                  ...
                  (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge))
                nextComparisonNode = TR::Node::createif(
                    TR::ificmple, correctCheckNode,
                    TR::Node::create(boundCheckNode, TR::iconst, 0, -1),
                    _exitGotoTarget);
            }

            if (!nextComparisonNode)
              nextComparisonNode = TR::Node::createif(
                  TR::ificmplt, correctCheckNode,
                  TR::Node::create(boundCheckNode, TR::iconst, 0, -1),
                  _exitGotoTarget);
      ...
      dumpOptDetails(
          comp(), "2: Prep %p has been created for testing if exceed bounds\n",
          prep);
        ...
        if (changedIndexSymRefAtSomePoint ||
          ...
          if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
            ...
            if (!firstChild->getOpCode().isLoad()) {
              vcount_t visitCount = comp()->incVisitCount();
              replaceInductionVariable(NULL, firstChild, -1,
                                       origIndexSymRef->getReferenceNumber(),
                                       storeRhs, visitCount);
            } else
              firstChild = storeRhs;
        ...
        if (!isIndexChildMultiplied) {
          ...
          if (isAddition) {
            if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple) ||
                (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt))
              nextComparisonNode = TR::Node::createif(
                  TR::ificmpgt, firstChild, secondChild, _exitGotoTarget);
            else
              nextComparisonNode = TR::Node::createif(
                  TR::ificmpge, firstChild, secondChild, _exitGotoTarget);
            ...
            if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge) ||
                (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt))
              nextComparisonNode = TR::Node::createif(
                  TR::ificmplt, firstChild, secondChild, _exitGotoTarget);
            else
              nextComparisonNode = TR::Node::createif(
                  TR::ificmple, firstChild, secondChild, _exitGotoTarget);
          ...
          if ((isAddition && !indVarOccursAsSecondChildOfSub) ||
              (!isAddition && indVarOccursAsSecondChildOfSub)) {
            traceMsg(comp(),
                     " Its addition indexsymref %d, duplicateMulNode %p \n",
                     indexSymRef->getReferenceNumber(), duplicateMulNode);
            vcount_t visitCount = comp()->incVisitCount();
            replaceInductionVariable(NULL, duplicateMulNode, -1,
                                     indexSymRef->getReferenceNumber(),
                                     loopLimit->duplicateTree(), visitCount);
            visitCount = comp()->incVisitCount();
            replaceInductionVariable(NULL, duplicateMulHNode, -1,
                                     indexSymRef->getReferenceNumber(),
                                     loopLimit->duplicateTree(), visitCount);
          }
          ...
          nextComparisonNode = TR::Node::createif(
              TR::ifiucmpgt, duplicateMulHNode,
              TR::Node::create(boundCheckNode, TR::iconst, 0, 0),
              _exitGotoTarget);
          nextComparisonNode->setIsVersionableIfWithMaxExpr(comp());
          if (comp()->requiresSpineChecks())
          ...
          // Adding multiplicative factor greater than zero check for
          // multiplicative BNDCHKS a.i+b; a>0
          nextComparisonNode = TR::Node::createif(
              TR::ificmple, strideNode->duplicateTree(),
              TR::Node::create(boundCheckNode, TR::iconst, 0, 0),
              _exitGotoTarget);
          if (comp()->requiresSpineChecks())
            findAndReplaceContigArrayLen(NULL, nextComparisonNode,
...
void TR_LoopVersioner::createRemoveBoundCheck(
    TR::TreeTop *boundCheckTree, LoopEntryPrep *prep,
    List<TR::TreeTop> *spineCheckTrees) {
  _curLoop->_loopImprovements.push_back(
      new (_curLoop->_memRegion) RemoveBoundCheck(this, prep, boundCheckTree));
  ...
  if (op == TR::BNDCHK || op == TR::ArrayCopyBNDCHK) {
    ...
    auto insertResult = _curLoop->_boundCheckPrepsWithSpineChecks.insert(
    ...
    TR_ASSERT_FATAL(
        ...
        entryPreventingInsertion->second, prep,
        boundCheckNode->getGlobalIndex(), boundCheckNode);
  }
}
...
void TR_LoopVersioner::RemoveBoundCheck::improveLoop() {
  ...
  TR_ASSERT_FATAL(boundCheckNode->getOpCode().isBndCheck(),
                  "unexpected opcode");
  ...
  if (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK) {
    ...
    TR::TreeTop *firstNewTree = TR::TreeTop::create(
        comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(0)),
        NULL, NULL);
    TR::TreeTop *secondNewTree = TR::TreeTop::create(
        comp(), TR::Node::create(TR::treetop, 1, boundCheckNode->getChild(1)),
        NULL, NULL);
    prevTree->join(firstNewTree);
    firstNewTree->join(secondNewTree);
 ...
 */
void TR_LoopVersioner::collectAllExpressionsToBeChecked(
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *divCheckTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
    TR::Node *node, List<TR::Node> *comparisonTrees, TR::Block *exitGotoBlock,
    vcount_t visitCount) {
 ...
 */
void TR_LoopVersioner::collectAllExpressionsToBeChecked(
    TR::Node *node, List<TR::Node> *comparisonTrees) {
  ...
  bool optDetails = comp()->getOutFile() != NULL &&
                    (trace() || comp()->getOption(TR_TraceOptDetails));

  if (optDetails) {
    dumpOptDetails(comp(), "collectAllExpressionsToBeChecked on tree:\n");
    comp()->getDebug()->clearNodeChecklist();
    comp()->getDebug()->printWithFixedPrefix(comp()->getOutFile(), node, 1,
                                             true, false, "\t\t");
    traceMsg(comp(), "\n");
  }
 ...
 *
 * **For debugging purposes only**, the \c
 * TR_nothingRequiresPrivatizationInVersioner environment variable can be set to
 * cause this method to assume that privatization is unnecessary.
 *
...
bool TR_LoopVersioner::requiresPrivatization(TR::Node *node) {
  ...
  TR::Symbol *sym = symRef->getSymbol();
  if (sym->isAutoOrParm()) return false;

  TR::SymbolReferenceTable *srTab = comp()->getSymRefTab();
  TR::ResolvedMethodSymbol *curMethod = comp()->getMethodSymbol();
  if (symRef == srTab->findOrCreateInstanceOfSymbolRef(curMethod)) return false;
  ...
  if (srTab->isVtableEntrySymbolRef(symRef)) return false;

  if (suppressInvarianceAndPrivatization(symRef)) return false;
 ...
 */
bool TR_LoopVersioner::suppressInvarianceAndPrivatization(
    TR::SymbolReference *symRef) {
  // Be sensitive to other threads' updates to the method overridden bit.
  if (symRef->isOverriddenBitAddress()) return true;
...
void TR_LoopVersioner::dumpOptDetailsCreatingTest(const char *description,
                                                  TR::Node *node) {
  dumpOptDetails(comp(), "Creating %s test for n%un [%p]\n", description,
                 node->getGlobalIndex(), node);
}

void TR_LoopVersioner::dumpOptDetailsFailedToCreateTest(const char *description,
                                                        TR::Node *node) {
...
bool TR_LoopVersioner::depsForLoopEntryPrep(
    TR::Node *node, TR::list<LoopEntryPrep *, TR::Region &> *deps,
  ...
  if (canPrivatizeRootNode && requiresPrivatization(node))
    return addLoopEntryPrepDep(LoopEntryPrep::PRIVATIZE, node, deps, visited) !=
           NULL;
  ...
  if (node->isInternalPointer() ||
    ...
    if (!node->getFirstChild()->isThisPointer()) {
      ...
      TR::Node *ifacmpeqNode =
          TR::Node::createif(TR::ifacmpeq, node->getFirstChild(),
                             TR::Node::aconst(node, 0), _exitGotoTarget);
    ...
    if (node->isInternalPointer() &&
      ...
      if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
        ...
        if (sig && (len > 0)) {
          ...
          cl = fe()->getClassFromSignature(sig, len,
                                           symRef->getOwningMethod(comp()));
        }

        int32_t otherLen;
        char *otherSig =
            otherSymRef->getOwningMethod(comp())->classNameOfFieldOrStatic(
                otherSymRef->getCPIndex(), otherLen);
        dumpOptDetails(comp(),
                       "For node %p len %d other len %d sig %p other sig %p\n",
                       node, len, otherLen, sig, otherSig);
        TR::ResolvedMethodSymbol *owningMethodSym =
            otherSymRef->getOwningMethodSymbol(comp());
        int32_t classCPI = 0;
        int32_t fieldCPI = otherSymRef->getCPIndex();
        instanceOfReqd = false;
        if (otherSymRef->getSymbol()->isShadow() && fieldCPI >= 0) {
          TR_ResolvedMethod *owningMethod =
              owningMethodSym->getResolvedMethod();
          classCPI = owningMethod->classCPIndexOfFieldOrStatic(fieldCPI);
          bool aotOK = true;
          otherClassObject =
              owningMethod->getClassFromConstantPool(comp(), classCPI, aotOK);
          instanceOfReqd = true;
        }
#ifdef J9_PROJECT_SPECIFIC
        else {
          switch (otherSymRef->getReferenceNumber() -
        ...
        if (!instanceOfReqd) {
          ...
          return false;
                   ...
                   fe()->isInstanceOf(cl, otherClassObject, true) == TR_yes) {
          instanceOfReqd = false;
        } else {
          TR::SymbolReference *otherClassSymRef =
              comp()->getSymRefTab()->findOrCreateClassSymbol(
                  owningMethodSym, classCPI, otherClassObject);

          duplicateClassPtr = TR::Node::createWithSymRef(node, TR::loadaddr, 0,
                                                         otherClassSymRef);
        }
      }
    }
    ...
    if (instanceOfReqd) {
      if (otherClassObject) {
        ...
        TR::Node *instanceofNode = TR::Node::createWithSymRef(
            TR:: instanceof
            , 2, 2, node->getFirstChild(), duplicateClassPtr,
            comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(
                comp()->getMethodSymbol()));
        TR::Node *ificmpeqNode = TR::Node::createif(
            TR::ificmpeq, instanceofNode,
            TR::Node::create(node, TR::iconst, 0, 0), _exitGotoTarget);
        if (addLoopEntryPrepDep(LoopEntryPrep::TEST, ificmpeqNode, deps,
                                visited) == NULL) {
        ...
        TR::Node *vftLoad = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, node->getFirstChild(),
            comp()->getSymRefTab()->findOrCreateVftSymbolRef());
        // TR::Node *componentTypeLoad = TR::Node::create(TR::aloadi, 1,
        // vftLoad,
        // comp()->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
        TR::Node *classFlag = NULL;
        if (TR::Compiler->target.is32Bit()) {
          classFlag = TR::Node::createWithSymRef(
              TR::iloadi, 1, 1, vftLoad,
              comp()
                  ->getSymRefTab()
                  ->findOrCreateClassAndDepthFlagsSymbolRef());
        } else {
          classFlag = TR::Node::createWithSymRef(
              TR::lloadi, 1, 1, vftLoad,
              comp()
                  ->getSymRefTab()
                  ->findOrCreateClassAndDepthFlagsSymbolRef());
        ...
        TR::Node *andConstNode =
            TR::Node::create(classFlag, TR::iconst, 0,
                             TR::Compiler->cls.flagValueForArrayCheck(comp()));
        TR::Node *andNode =
            TR::Node::create(TR::iand, 2, classFlag, andConstNode);
        TR::Node *cmp = TR::Node::createif(TR::ificmpne, andNode, andConstNode,
                                           _exitGotoTarget);
        if (addLoopEntryPrepDep(LoopEntryPrep::TEST, cmp, deps, visited) ==
            NULL) {
        ...
        // TR_ASSERT(((node->getSymbolReference() ==
        // comp()->getSymRefTab()->findVftSymbolRef()) ||
        // comp()->getSymRefTab()->findVtableEntrySymbolRef(node->getSymbolReference())),
        // "Not enough information to emit the instanceof test that is reqd\n");
    ...
    static struct temps {
      ...
      int32_t k;
    } a[] = {{TR::iadd, TR::iconst, headerSize},
    ...
    if (!indexNode) {
      if (!childInRequiredForm) {
        TR::Node *constNode = TR::Node::create(
            node, type.isInt32() ? TR::iconst : TR::lconst, 0, 0);
        indexNode = TR::Node::create(type.isInt32() ? TR::isub : TR::lsub, 2,
                                     offset, constNode);
        if (type.isInt64())
      ...
      indexNode =
          TR::Node::create(type.isInt32() ? TR::iushr : TR::lushr, 2, indexNode,
    ...
    TR::Node *ificmpgeNode =
        TR::Node::createif(type.isInt32() ? TR::ificmpge : TR::iflcmpge,
                           indexNode, arrayLengthNode, _exitGotoTarget);
    if (addLoopEntryPrepDep(LoopEntryPrep::TEST, ificmpgeNode, deps, visited) ==
        NULL) {
    ...
    TR::Node *ificmpltNode =
        TR::Node::createif(type.isInt32() ? TR::ificmplt : TR::iflcmplt,
                           indexNode, constNode, _exitGotoTarget);
    if (type.isInt64()) ificmpltNode->getSecondChild()->setLongInt(0);

    if (addLoopEntryPrepDep(LoopEntryPrep::TEST, ificmpltNode, deps, visited) ==
        NULL) {
      dumpOptDetailsFailedToCreateTest("part 2 of bounds", node);
      return false;
    }
  } else if (((node->getOpCodeValue() == TR::idiv) ||
              (node->getOpCodeValue() == TR::irem) ||
              (node->getOpCodeValue() == TR::lrem) ||
              (node->getOpCodeValue() == TR::ldiv))) {
    ...
    if (!divisorIsNonzeroConstant) {
      ...
      if (divisor->getType().isInt64())
        ifNode = TR::Node::createif(TR::iflcmpeq, divisor,
                                    TR::Node::create(node, TR::lconst, 0, 0),
                                    _exitGotoTarget);
      else
        ifNode = TR::Node::createif(TR::ificmpeq, divisor,
                                    TR::Node::create(node, TR::iconst, 0, 0),
                                    _exitGotoTarget);

      if (addLoopEntryPrepDep(LoopEntryPrep::TEST, ifNode, deps, visited) ==
          NULL) {
        ...
        return false;
                 ...
                 .isCheckCast())  //(node->getOpCodeValue() == TR::checkcast)
  {
    TR_ASSERT_FATAL(false, "depsForLoopEntryPrep: n%un is a checkcast",
                    node->getGlobalIndex());
  }

  // Get any further prerequisite LoopEntryPreps beyond those required for the
  // ones already in deps. Doing this last avoids copying transitive
  // dependencies into deps.
  for (int i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    if (!depsForLoopEntryPrep(child, deps, visited, true)) return false;
  }
...
TR_LoopVersioner::LoopEntryPrep *TR_LoopVersioner::addLoopEntryPrepDep(
    LoopEntryPrep::Kind kind, TR::Node *node,
...
TR::Node *TR_LoopVersioner::findLoad(TR::Node *node,
                                     TR::SymbolReference *symRef,
                                     vcount_t origVisitCount) {
  if (node->getVisitCount() >= origVisitCount) return NULL;
...
bool TR_LoopVersioner::findStore(TR::TreeTop *start, TR::TreeTop *end,
                                 TR::Node *node, TR::SymbolReference *symRef,
  ...
  if (needAuxiliaryWalk) {
    ...
    if (!findStore(start->getEnclosingBlock()->getEntry(), start, node, symRef,
                   false, true))
      return true;
    else
      return false;
...
bool TR_LoopVersioner::boundCheckUsesUnchangedValue(
    TR::TreeTop *bndCheckTree, TR::Node *node, TR::SymbolReference *symRef,
  ...
  if ((currentBlock->getSuccessors().size() == 1) &&
    ...
    if (findStore(bndCheckTree, currentBlock->getExit(), node, symRef, true) ||
        findStore(_loopTestBlock->getEntry(), _loopTestBlock->getExit(), node,
                  symRef, true))
      return true;
    else
      return false;
  ...
  if (entryBlock->getSuccessors().size() == 1) {
    TR::Block *succBlock =
        entryBlock->getSuccessors().front()->getTo()->asBlock();
    bool searchForStore = false;
    if (currentBlock == succBlock) searchForStore = true;
    ...
    if (!searchForStore) {
      ...
      if ((succBlock->getSuccessors().size() == 1) &&
        ...
        if (!findStore(succBlock->getEntry(), succBlock->getExit(), node,
                       symRef))
          searchForStore = true;
      }
    }
  ...
  while (flag) {
    ...
    if (cursorBlock == currentBlock) {
      ...
      if (findStore(cursorBlock->getEntry(), cursorBlock->getExit(), node,
                    symRef))
        return true;
    }

    if ((cursorBlock == _loopTestBlock) || (cursorBlock == entryBlock)) break;
  ...
  if ((cursorBlock == _loopTestBlock) || (cursorBlock == entryBlock))
    return false;
  ...
  while (flag) {
    ...
    for (auto pred = cursorBlock->getPredecessors().begin();
         pred != cursorBlock->getPredecessors().end(); ++pred) {
    ...
    if (predInsideLoop) {
      TR::Block *onlySuccInsideLoop = NULL;
      for (auto succ = predInsideLoop->getSuccessors().begin();
           succ != predInsideLoop->getSuccessors().end(); ++succ) {
        TR::Block *succBlock = (*succ)->getTo()->asBlock();
        if (blocksInRegion.find(succBlock)) {
          if (!onlySuccInsideLoop)
            onlySuccInsideLoop = succBlock;
      ...
      if (!onlySuccInsideLoop) predInsideLoop = NULL;
    }
...
int32_t TR_LoopVersioner::detectCanonicalizedPredictableLoops(
    TR_Structure *loopStructure, TR_BitVector **optSetInfo,
    int32_t bitVectorSize) {
  int32_t retval = 1;
  if (!loopStructure->getParent()) return -3;

  TR_ScratchList<TR::Block> blocksInWhileLoop(trMemory());
  loopStructure->getBlocks(&blocksInWhileLoop);
  int32_t loop_size = blocksInWhileLoop.getSize();
  int32_t hotnessFactor = 1;
  if (comp()->getMethodHotness() < warm) hotnessFactor = 2;

  if (loop_size > (MAX_BLOCKS_THRESHOLD / hotnessFactor)) return -2;

  int32_t normalNodeCount = (2 * NORMAL_NODE_COUNT) / hotnessFactor;
  int32_t normalBlockCount = (2 * NORMAL_BLOCK_COUNT) / hotnessFactor;

  int32_t nodeCountFactor =
      std::min(8, std::max(1, (_origNodeCount / normalNodeCount)));
  int32_t blockCountFactor =
      std::min(8, std::max(1, (_origBlockCount / normalBlockCount)));

  ncount_t nodeCount;
  nodeCount = comp()->getAccurateNodeCount();

  if ((nodeCount / (MAX_SIZE_INCREASE_FACTOR / hotnessFactor)) >
      (_origNodeCount / nodeCountFactor)) {
    if (trace())
  ...
  if ((comp()->getFlowGraph()->getNodes().getSize() /
       (MAX_SIZE_INCREASE_FACTOR / hotnessFactor)) >
      (_origBlockCount / blockCountFactor)) {
    if (trace())
      traceMsg(comp(), "Failing block count %d orig %d factor %d\n",
               comp()->getFlowGraph()->getNodes().getSize(), _origBlockCount,
               blockCountFactor);
    return -2;
  }
  ...
  if (loopStructure->getMaxNestingDepth(&currentNestingDepth,
                                        &currentMaxNestingDepth) >
  ...
  TR_RegionStructure *parentStructure =
      regionStructure->getParent()->asRegion();
  TR_StructureSubGraphNode *subNode = NULL;
  TR_RegionStructure::Cursor si(*parentStructure);
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    if (subNode->getNumber() == loopStructure->getNumber()) break;
  }
  ...
  if (loopInvariantBlock) {
    /////if (isInvariantLoop)
    {
      ...
      _hasPredictableExits = new (trStackMemory())
          TR_BitVector(symRefCount, trMemory(), stackAlloc);
      ...
      _isAddition = false;
      _loopTestTree = NULL;
      int32_t isPredictableLoop = checkLoopForPredictability(
          loopStructure, loopInvariantBlock->getBlock(), NULL);
      ...
      if ((isPredictableLoop > 0) &&
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          ...
          _requiresAdditionalCheckForIncrement = false;
          storeInRequiredForm =
              isStoreInRequiredForm(nextInductionVariableNumber, loopStructure);
          // dumpOptDetails(comp(), "Store of %d in reqd form %d\n",
          // nextInductionVariableNumber, storeInRequiredForm);

          if (_isAddition) _additionInfo->set(nextInductionVariableNumber);
          ...
          if (storeInRequiredForm) {
            ...
            if (storeInRequiredForm) {
              TR::Block *currentBlock =
                  _storeTrees[nextInductionVariableNumber]->getEnclosingBlock();
              if (!blockIsAlwaysExecutedInLoop(currentBlock, regionStructure))
                storeInRequiredForm = false;
              if (currentBlock->getStructureOf()->getContainingLoop() !=
                  regionStructure)
                storeInRequiredForm = false;
            }
          ...
          if (storeInRequiredForm /* && !_cannotBeEliminated->get(nextInductionVariableNumber) */)
            ...
            *versionableInductionVariable = nextInductionVariableNumber;
            _derivedVersionableInductionVariables.add(
                versionableInductionVariable);
            TR::Node *loopTestNode = _loopTestTree->getNode();

            if ((loopTestNode->getFirstChild()
                     ->getOpCode()
                     .hasSymbolReference() &&
                 nextInductionVariableNumber == loopTestNode->getFirstChild()
                                                    ->getSymbolReference()
                                                    ->getReferenceNumber()) ||
            ...
            {
              versionableInductionVariable =
                  _derivedVersionableInductionVariables.popHead();
              ...
              if (trace())
                traceMsg(
                    comp(),
                    "Version loop : %d with respect to induction variable %d\n",
                    loopStructure->getNumber(), nextInductionVariableNumber);
            }
          } else if (isStoreInSpecialForm(nextInductionVariableNumber,
                                          loopStructure)) {
            int32_t *versionableInductionVariable =
                (int32_t *)trMemory()->allocateStackMemory(sizeof(int32_t));
            *versionableInductionVariable = nextInductionVariableNumber;
            _specialVersionableInductionVariables.add(
                versionableInductionVariable);
        ...
        // clearing the derived inductionVariables when primary induction
        // variable had a non-const increment.
        ...
        if (loopStructure->asRegion()) {
          ...
          for (TR::Block *b = eb.getFirst(); b; b = eb.getNext()) {
            ...
            if (ifNode->getOpCode().isIf() &&
                !ifNode->getOpCode().isCompBranchOnly()) {
              ...
              if (ivNode->getOpCode().hasSymbolReference())
                ...
                if ((ivNode->getOpCode().isAdd() ||
                     ivNode->getOpCode().isSub()) &&
...
bool TR_LoopVersioner::isStoreInSpecialForm(int32_t symRefNum,
  ...
  if (!storeNode->getType().isInt32()) return false;
  ...
  if (storeNode->getFirstChild()->getOpCode().isAnd() &&
      ...
      (storeNode->getFirstChild()->getFirstChild()->getOpCodeValue() ==
       TR::iload) &&
...
bool TR_LoopVersioner::isStoreInRequiredForm(int32_t symRefNum,
                                             TR_Structure *loopStructure) {
  /* [101214] calls too can kill symRefs via loadaddr */
  if (symRefNum != 0 && _allKilledSymRefs[symRefNum] == true) return false;
  ...
  _constNode =
      containsOnlyInductionVariableAndAdditiveConstant(addNode, symRefNum);
  if (_constNode == NULL) return false;
  ...
  if (!secondChild->getOpCode().isLoadConst() &&
    ...
    if (!allowVariableStep) return false;
    ...
    if (secondChild->getOpCode().isLoadVarDirect() &&
      ...
      if (!isSymbolReferenceWrittenNumberOfTimesInStructure(
              loopStructure,
              secondChild->getSymbolReference()->getReferenceNumber(),
              &timesWritten, 0))
        return false;
...
void TR_LoopVersioner::collectArrayAliasCandidates(TR::Node *node,
                                                   vcount_t visitCount) {
  TR_ASSERT(!refineAliases(), "something wrong");
}
void TR_LoopVersioner::buildAliasRefinementComparisonTrees(
    List<TR::TreeTop> *nullCheckTrees, List<TR::TreeTop> *divCheckTrees,
    List<TR::TreeTop> *checkCastTrees, List<TR::TreeTop> *arrayStoreCheckTrees,
...
void TR_LoopVersioner::refineArrayAliases(TR_RegionStructure *r) {
  TR_ASSERT(!refineAliases(), "something wrong");
}

void TR_LoopVersioner::findAndReplaceContigArrayLen(TR::Node *parent,
                                                    TR::Node *node,
                                                    vcount_t visitCount) {
  if (node->getVisitCount() == visitCount) return;

  if ((node->getOpCodeValue() ==
       TR::contigarraylength))  // future: || (node->getOpCodeValue() ==
                                // TR::discontigarraylength))
  ...
  if (node->getOpCode().isConversion() &&
    ...
    if ((conversion1->getOpCodeValue() == TR::s2i &&
        ...
        (conversion1->getOpCodeValue() == TR::b2i &&
         conversion2->getOpCodeValue() ==
             TR::i2b) ||  // FIXME: why are we ignoring bu2i/i2b here?
...
const char *TR_LoopVersioner::optDetailString() const throw() {
 ...
 */
bool TR_LoopVersioner::initExprFromNode(Expr *expr, TR::Node *node,
                                        bool onlySearching) {
  // Reject nodes that can't be properly represented.
#ifdef J9_PROJECT_SPECIFIC
  if (node->getOpCode().isConversionWithFraction()) return false;
#endif

  if (node->getNumChildren() > Expr::MAX_CHILDREN) return false;
  ...
  if (expr->_op.isLoadConst()) {
    ...
    expr->_symRef =
        node->getSymbolReference()->getOriginalUnimprovedSymRef(comp());
  } else {
    TR_ASSERT_FATAL(
  ...
  for (int i = 0; i < Expr::MAX_CHILDREN; i++) expr->_children[i] = NULL;
...
static bool containsCommaSeparated(const char *haystack, const char *needle) {
  ...
  if (haystackLen < needleLen - 2)
    ...
    return true;
  else if (strncmp(haystack + haystackLen - needleLen + 1, needle,
                   needleLen - 1) == 0)
    return true;
  else
 ...
 * \param onlySearching True when node is not being copied into
 * CurLoop::_exprTable \return true if the guard is allowed, or else false to
 * prevent versioning
 *
 * \see suppressInvarianceAndPrivatization()
...
bool TR_LoopVersioner::guardOkForExpr(TR::Node *node, bool onlySearching) {
  ...
  if (allowEnv != NULL || forbidEnv != NULL) {
    char needle[32];
    int needleLen =
        snprintf(needle, sizeof(needle), ",%d:%d,", (int)kind, (int)test);
    TR_ASSERT_FATAL(0 <= needleLen && needleLen < sizeof(needle),
                    "needle buffer is too small");
 ...
 */
const TR_LoopVersioner::Expr *TR_LoopVersioner::makeCanonicalExpr(
    TR::Node *node) {
  auto cached = _curLoop->_nodeToExpr.find(node);
  if (cached != _curLoop->_nodeToExpr.end()) return cached->second;
  ...
  if (!initExprFromNode(&expr, node, onlySearching)) {
    ...
    if (shouldAssert) {
      if (node->isNopableInlineGuard()) {
        ...
        TR_ASSERT_FATAL(false,
                        ...
                        node->getGlobalIndex(), node, (int)guard->getKind(),
                        (int)guard->getTestType());
      } else {
        TR_ASSERT_FATAL(false, "n%un [%p] is unrepresentable",
                        node->getGlobalIndex(), node);
      }
    }
    ...
    TR::DebugCounter::incStaticDebugCounter(
        comp(),
        TR::DebugCounter::debugCounterName(
            comp(), "loopVersioner.unrepresentable/(%s)/%s/loop=%d/n%un",
            comp()->signature(),
            comp()->getHotnessName(comp()->getMethodHotness()),
            _curLoop->_loop->getNumber(), node->getGlobalIndex()));
 ...
 */
const TR_LoopVersioner::Expr *TR_LoopVersioner::findCanonicalExpr(
    TR::Node *node) {
  auto cached = _curLoop->_nodeToExpr.find(node);
  if (cached != _curLoop->_nodeToExpr.end()) return cached->second;
  ...
  if (node->getOpCode().isLoadVarDirect() &&
      node->getSymbol()->isAutoOrParm() && !isExprInvariant(node))
  ...
  if (defRHS != NULL) {
    ...
    if (!initExprFromNode(&expr, node, onlySearching)) return NULL;
    ...
    auto existing = _curLoop->_exprTable.find(expr);
    if (existing == _curLoop->_exprTable.end()) return NULL;
 ...
 * \p visited The set of nodes within which substitution has already been
 * performed \return the pointer to the canonical Expr instance corresponding to
 * \p node, or null if no such Expr has been created
 */
const TR_LoopVersioner::Expr *TR_LoopVersioner::substitutePrivTemps(
  ...
  if (defRHS != NULL) {
    ...
    if (canonicalExpr == NULL) return NULL;
  } else {
    ...
    Expr expr;
    bool onlySearching = true;
    bool rootMayBePrivatized = !node->getOpCode().isTreeTop() &&
    ...
    if (!rootMayBePrivatized) return NULL;

    auto exprEntry = _curLoop->_exprTable.find(expr);
    if (exprEntry == _curLoop->_exprTable.end()) return NULL;
  ...
  if (trace()) {
    traceMsg(comp(), "substitutePrivTemps: Canonical n%un [%p] is expr %p\n",
             node->getGlobalIndex(), node, canonicalExpr);
  }
  ...
  TR_ASSERT_FATAL(
      ...
      "failed to insert n%un [%p] into _nodeToExpr", node->getGlobalIndex(),
      node);

  auto tempEntry = _curLoop->_privTemps.find(canonicalExpr);
  if (tempEntry == _curLoop->_privTemps.end()) return canonicalExpr;

  // NB. Once the entry has been put into _privTemps, this transformation is
  // required.
  TR::SymbolReference *temp = tempEntry->second._symRef;
  dumpOptDetails(
      comp(), "Transforming n%un [%p] into a load of privatization temp #%d\n",
      node->getGlobalIndex(), node, temp->getReferenceNumber());
  ...
  if (type == TR::Int8 || type == TR::Int16) {
    ...
    node->setAndIncChild(0,
                         TR::Node::createWithSymRef(node, TR::iload, 0, temp));
  } else {
...
TR::Node *TR_LoopVersioner::emitExpr(const Expr *expr, EmitExprMemo &memo) {
  ...
  int numChildren = 0;
  while (numChildren < Expr::MAX_CHILDREN &&
         expr->_children[numChildren] != NULL)
    numChildren++;
  ...
  if (!op.isLoadConst() && op.hasSymbolReference()) {
    ...
    TR_ASSERT_FATAL(numChildren == 2, "expected if %p to have 2 children",
                    expr);
  ...
  if (op.isLoadConst()) node->setConstValue(expr->_constValue);
 ...
 */
void TR_LoopVersioner::emitPrep(LoopEntryPrep *prep,
  ...
  if (prep->_kind == LoopEntryPrep::TEST) {
    ...
    TR_ASSERT_FATAL(prep->_kind == LoopEntryPrep::PRIVATIZE,
    ...
    //
    TR_ASSERT_FATAL(!value->isInternalPointer(),
    ...
    if (tempType == TR::Int8 || tempType == TR::Int16) tempType = TR::Int32;

    TR::SymbolReference *temp = comp()->getSymRefTab()->createTemporary(
        comp()->getMethodSymbol(), tempType);
    ...
    TR_ASSERT_FATAL(insertResult.second, "_privTemps insert failed for expr %p",
                    prep->_expr);
    ...
    dumpOptDetails(comp(), "Emitted prep %p as n%un [%p] storing to temp #%d\n",
                   prep, store->getGlobalIndex(), store,
...
void TR_LoopVersioner::unsafelyEmitAllTests(
  ...
  for (auto it = preps.begin(); it != preps.end(); ++it) {
    ...
    if (prep->_unsafelyEmitted) continue;
...
void TR_LoopVersioner::setAndIncChildren(TR::Node *node, int n,
                                         TR::Node **children) {
  for (int i = 0; i < n; i++) node->setAndIncChild(i, children[i]);
}
 ...
 * \see LoopBodySearch
 */
void TR_LoopVersioner::nodeWillBeRemovedIfPossible(TR::Node *node,
                                                   LoopEntryPrep *prep) {
 ...
 * \param visited The visited set. Do not specify outside of
 * addLoopEntryPrepDep() \param prev A previous preparation to depend on. Do not
 * specify outside of createChainedLoopEntryPrep().
 *
...
TR_LoopVersioner::LoopEntryPrep *TR_LoopVersioner::createLoopEntryPrep(
    LoopEntryPrep::Kind kind, TR::Node *node, TR::NodeChecklist *visited,
    LoopEntryPrep *prev) {
  bool optDetails = comp()->getOutFile() != NULL &&
                    (trace() || comp()->getOption(TR_TraceOptDetails));
  ...
  if (optDetails) {
    const char *kindName =
        kind == LoopEntryPrep::PRIVATIZE ? "PRIVATIZE" : "TEST";
    if (prev == NULL)
    ...
    if (visited == NULL) comp()->getDebug()->clearNodeChecklist();
  ...
  LoopEntryPrep *prep = new (_curLoop->_memRegion)
      LoopEntryPrep(kind, expr, _curLoop->_memRegion);

  bool ok = false;
  bool isPrivatization = kind == LoopEntryPrep::PRIVATIZE;
  bool canPrivatizeRootNode = !isPrivatization;
  if (isPrivatization) _curLoop->_privatizationsRequested = true;

  if (prev != NULL) prep->_deps.push_back(prev);

  if (visited != NULL) {
    ok =
        depsForLoopEntryPrep(node, &prep->_deps, visited, canPrivatizeRootNode);
  } else {
    TR::NodeChecklist myVisited(comp());
    ok = depsForLoopEntryPrep(node, &prep->_deps, &myVisited,
                              canPrivatizeRootNode);
  }
  ...
  if (optDetails) {
    dumpOptDetails(
        comp(), "Prep for n%un [%p] is prep %p %s expr %p%s, deps: ",
        node->getGlobalIndex(), node, prep,
    ...
    if (prep->_deps.empty()) {
      ...
      traceMsg(comp(), "%p", *it);
      while (++it != prep->_deps.end()) traceMsg(comp(), ", %p", *it);
      traceMsg(comp(), "\n");
    }
  }
...
void TR_LoopVersioner::visitSubtree(TR::Node *node,
 ...
 */
TR_LoopVersioner::CurLoop::CurLoop(TR::Compilation *comp, TR::Region &memRegion,
...
TR_LoopVersioner::LoopBodySearch::LoopBodySearch(
    TR::Compilation *comp, TR::Region &memRegion, TR_RegionStructure *loop,
...
void TR_LoopVersioner::LoopBodySearch::enqueueReachableSuccessorsInLoop() {
  ...
  if (_blockHasExceptionPoint)
    enqueueReachableSuccessorsInLoopFrom(
        _currentBlock->getExceptionSuccessors());
}

bool TR_LoopVersioner::LoopBodySearch::isBranchConstant(TR::Node *ifNode) {
  if (_removedNodes->contains(ifNode)) return true;

  // Detect branches that have already been transformed in the current pass of
  // versioner.
  TR::ILOpCodes op = ifNode->getOpCodeValue();
  if (op != TR::ificmpeq && op != TR::ificmpne) return false;

  TR::Node *lhs = ifNode->getChild(0);
  TR::Node *rhs = ifNode->getChild(1);
  return lhs->getOpCodeValue() == TR::iconst &&
         rhs->getOpCodeValue() == TR::iconst;
}

bool TR_LoopVersioner::LoopBodySearch::isConstantBranchTaken(TR::Node *ifNode) {
  TR_ASSERT_FATAL(isBranchConstant(ifNode), "unexpected branch n%un",
                  ifNode->getGlobalIndex());

  if (_removedNodes->contains(ifNode)) return _takenBranches->contains(ifNode);
...
void TR_LoopVersioner::LoopBodySearch::enqueueBlockIfInLoop(
    TR::TreeTop *entry) {
