    ...
    if (naturalLoop->getParent()) {
      ...
      if (loopNode && (loopNode->getPredecessors().size() == 1)) {
        ...
        if (loopInvariantNode->getStructure()->asBlock() &&
          ...
          {
            if (trace())
              traceMsg(comp(), "loopIsWorthVersioning returning false based on "
                               "LoopCountThreshold\n");
            return false;
          }
        }
  ...
  for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL;
    ...
    if (!shouldOnlySpecializeLoops() && !refineAliases()) {
      ...
      if (comp()->getOption(TR_EnableAggressiveLoopVersioning)) {
        if (trace())
          traceMsg(comp(), "aggressiveLoopVersioning: raising hotnessThreshold "
                           "for conditionalsWillBeEliminated\n");
        hotnessThreshold =
...
void TR_LoopVersioner::updateDefinitionsAndCollectProfiledExprs(
  ...
  if (defNode->isTheVirtualCallNodeForAGuardedInlinedCall() &&
    ...
    defNode =
        NULL; // old code was setting node's symref anyway, is it really needed?
...
void TR_LoopVersioner::versionNaturalLoop(
  ...
  if (_requiresAdditionalCheckForIncrement &&
    ...
    if (_constNode->getType().isInt32()) {
      ...
      TR_ASSERT(0, "Unable to create test outside of loop for checking if iv "
  ...
  if (comp()->getOption(TR_EnableAggressiveLoopVersioning)) {
    if (trace())
      traceMsg(comp(), "aggressiveLoopVersioning: raising hotnessThreshold for "
  ...
  while (currComparisonBlock) {
    ...
    TR_BlockStructure *comparisonBlockStructure = new (_cfg->structureRegion())
        TR_BlockStructure(comp(), actualComparisonBlock->getNumber(),
                          actualComparisonBlock);
  ...
  while (nextTree) {
    ...
    if (performTransformation(comp(),
      ...
      if (op == TR::SpineCHK) {
        ...
        TR_ASSERT_FATAL(prereqEntry !=
                            _curLoop->_boundCheckPrepsWithSpineChecks.end(),
                        "missing prep for removal of bound check from "
                        "BNDCHKwithSpineCHK n%un [%p]",
...
bool TR_LoopVersioner::guardOkForExpr(TR::Node *node, bool onlySearching) {
  ...
  default:
    ...
    //
    TR_ASSERT_FATAL(
        false, "guardOkForExpr: n%un [%p]: unrecognized nop-able guard kind %d",
 ...
 */
const TR_LoopVersioner::Expr *
TR_LoopVersioner::makeCanonicalExpr(TR::Node *node) {
 ...
 */
const TR_LoopVersioner::Expr *
TR_LoopVersioner::findCanonicalExpr(TR::Node *node) {
  auto cached = _curLoop->_nodeToExpr.find(node);
  if (cached != _curLoop->_nodeToExpr.end())
    return cached->second;
 ...
 */
const TR_LoopVersioner::Expr *
TR_LoopVersioner::substitutePrivTemps(TR::TreeTop *tt, TR::Node *node,
  ...
  TR_ASSERT_FATAL(insertResult.second ||
                      insertResult.first->second == canonicalExpr,
 ...
 */
TR_LoopVersioner::LoopEntryPrep *
TR_LoopVersioner::createLoopEntryPrep(LoopEntryPrep::Kind kind, TR::Node *node,
                                      TR::NodeChecklist *visited,
                                      LoopEntryPrep *prev) {
  bool optDetails = comp()->getOutFile() != NULL &&
                    (trace() || comp()->getOption(TR_TraceOptDetails));
...
TR_LoopVersioner::CurLoop::CurLoop(TR::Compilation *comp, TR::Region &memRegion,
    ...
    : _memRegion(memRegion), _loop(loop),
      ...
      _definitelyRemovableNodes(comp), _optimisticallyRemovableNodes(comp),
      _takenBranches(comp), _loopImprovements(memRegion),
      _privTemps(std::less<const Expr *>(), memRegion),
      _privatizationsRequested(false), _privatizationOK(false) {}
...
TR_LoopVersioner::LoopBodySearch::LoopBodySearch(
    ...
    : _loop(loop), _removedNodes(removedNodes), _takenBranches(takenBranches),
      _queue(memRegion), _alreadyEnqueuedBlocks(comp),
