    ...
    if (aggressive || comp()->getMethodHotness() <= warm) {
        if (naturalLoop->getParent()) {
            ...
            if (loopNode && (loopNode->getPredecessors().size() == 1)) {
                ...
                if (loopInvariantNode->getStructure()->asBlock() && loopInvariantNode->getStructure()->asBlock()->isLoopInvariantBlock()) {
...
int32_t TR_LoopVersioner::performWithoutDominators()
{
    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        ...
        if (_loopTestTree) {
            if ((_loopTestTree->getNode()->getNumChildren() > 1) && isExprInvariant(_loopTestTree->getNode()->getSecondChild())) {
                if (trace())
        ...
        if (!shouldOnlySpecializeLoops() && !refineAliases()) {
            ...
            if (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly() || (comp()->getMethodHotness() > hotnessThreshold)) && nullChecksWillBeEliminated) || boundChecksWillBeEliminated || divChecksWillBeEliminated || awrtBarisWillBeEliminated || checkCastTreesWillBeEliminated || arrayStoreCheckTreesWillBeEliminated || specializedNodesWillBeEliminated || invariantNodesWillBeEliminated || _nonInlineGuardConditionalsWillNotBeEliminated || _loopTransferDone) {
        ...
        if ((versionToRefineAliases || !refineAliases()) && (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly() || (comp()->getMethodHotness() >= veryHot)) && (naturalLoop->containsOnlyAcyclicRegions() || !_loopTransferDone) && nullChecksWillBeEliminated) || (_asyncCheckTree && shouldOnlySpecializeLoops()) || boundChecksWillBeEliminated || spineChecksWillBeEliminated || conditionalsWillBeEliminated || divChecksWillBeEliminated || awrtBarisWillBeEliminated || checkCastTreesWillBeEliminated || arrayStoreCheckTreesWillBeEliminated || specializedNodesWillBeEliminated || invariantNodesWillBeEliminated || versionToRefineAliases || _containsGuard)) {
...
void TR_LoopVersioner::performLoopTransfer()
{
    ...
    for (vgInfo = _virtualGuardInfo.getFirst(); vgInfo; vgInfo = vgInfo->getNext()) {
        ...
        for (nextVirtualGuard = guardsIt.getFirst(); nextVirtualGuard != NULL; nextVirtualGuard = guardsIt.getNext()) {
            ...
            if (hotGuard->getOpCode().isIf() && hotGuard->isTheVirtualGuardForAGuardedInlinedCall() && coldGuard->getOpCode().isIf() && coldGuard->isTheVirtualGuardForAGuardedInlinedCall()) {
...
bool TR_LoopVersioner::detectInvariantChecks(List<TR::Node>* nullCheckedReferences, List<TR::TreeTop>* nullCheckTrees)
{
    ...
    for (node = nullCheckedReferences->getListHead(); node != NULL;) {
        ...
        if (!isNullCheckReferenceInvariant && node->getData()->getOpCode().hasSymbolReference() && ((node->getData()->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(node->getData())) || (node->getData()->getOpCode().isLoadIndirect() && !_seenDefinedSymbolReferences->get(node->getData()->getSymbolReference()->getReferenceNumber()) && node->getData()->getFirstChild()->getOpCode().hasSymbolReference() && node->getData()->getFirstChild()->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(node->getData()->getFirstChild()))))
...
bool TR_LoopVersioner::detectInvariantArrayStoreChecks(List<TR::TreeTop>* arrayStoreCheckTrees)
{
    ...
    for (treetop = arrayStoreCheckTrees->getListHead(); treetop != NULL;) {
        ...
        if (arrayNode && valueNode) {
            ...
            if (!sourceInvariant) {
                if (valueNode->getOpCode().hasSymbolReference() && valueNode->getSymbolReference()->getSymbol()->isArrayShadowSymbol()) {
                    ...
                    if (addressNode->getOpCode().isArrayRef()) {
                        ...
                        if (!childOfAddressNode->isInternalPointer() && (!childOfAddressNode->getOpCode().hasSymbolReference() || (!childOfAddressNode->getSymbolReference()->getSymbol()->isInternalPointerAuto() && !childOfAddressNode->getSymbolReference()->getSymbol()->isNotCollected()))) {
...
bool TR_LoopVersioner::detectInvariantSpecializedExprs(List<TR::Node>* profiledExprs)
{
    ...
    for (node = profiledExprs->getListHead(); node != NULL;) {
        ...
        if (!isProfiledExprInvariant && node->getData()->getOpCode().hasSymbolReference() && node->getData()->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(node->getData()))
        ...
        if (ttNode->getOpCode().isCall()) {
            if (ttNode->getNumChildren() > 0) {
                ...
                if (inlinedCallIndex < comp()->getNumInlinedCallSites()) {
                    ...
                    if ((ics._byteCodeInfo.getByteCodeIndex() == bcIndex) && (ics._byteCodeInfo.getCallerIndex() == callerIndex)) {
...
bool TR_LoopVersioner::detectInvariantTrees(TR_RegionStructure* whileLoop, List<TR::TreeTop>* trees, bool onlyDetectHighlyBiasedBranches, bool* containsNonInlineGuard, SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    for (; nextTree;) {
        ...
        if (node->getOpCode().isIf() && !node->isTheVirtualGuardForAGuardedInlinedCall()) {
            ...
            if (nextBlock->getEdge(targetBlock)->getFrequency() <= MAX_COLD_BLOCK_COUNT + 1 && nextBlock->getFrequency() >= HIGH_FREQ_THRESHOLD && static_cast<double>(nextBlock->getNextBlock()->getFrequency()) / nextBlock->getFrequency() >= 0.8) {
                if (trace())
...
#ifdef J9_PROJECT_SPECIFIC
            else if (comp()->hasIntStreamForEach() && ((targetBlock->isCold() && !nextBlock->getNextBlock()->isCold()) || (!targetBlock->isCold() && nextBlock->getNextBlock()->isCold()))) {
                highlyBiasedBranch = true;
            }
#endif
        }
        ...
        if (onlyDetectHighlyBiasedBranches) {
            if (node->isTheVirtualGuardForAGuardedInlinedCall()) {
                ...
                if (guardInfo->getTestType() == TR_VftTest) {
                    ...
                    if ((node->getFirstChild()->getNumChildren() > 0) && (node->getFirstChild()->getFirstChild()->getNumChildren() > 0)) {
        ...
        if (includeComparisonTree) {
            ...
            if (onlyDetectHighlyBiasedBranches
                ...
                if (guardInfo->getKind() == TR_DummyGuard || !guardInfo->getInnerAssumptions().isEmpty()) // inner assumption may have receivers other than 'thisChild'
                    ...
                    if (!b) {
                        ...
                        {
                            if (!(thisChild->getOpCode().hasSymbolReference() && thisChild->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(thisChild))) {
                                isTreeInvariant = false;
                            } else {
                                if (!guardInfo)
                                    guardInfo = comp()->findVirtualGuardInfo(node);
            ...
            if (onlyDetectHighlyBiasedBranches || (!node->getOpCode().isBranch()) || (!onlyNonInlineGuardConditional)) {
                ...
                if (!onlyDetectHighlyBiasedBranches && node->getOpCode().isBranch()) {
                    ...
                    if (prevTree) {
                        ...
                        if (!_postDominators || _postDominators->numberOfBlocksControlled(b1) > _postDominators->numberOfBlocksControlled(b2)) {
                            prevTree->setNextElement(nextTree->getNextElement());
    ...
    bool& indVarOccursAsSecondChildOfSub)
{
    ...
    TR_UseDefInfo::BitVector defs(comp()->allocator());
    if (useDefInfo->getUseDef(defs, useIndex) && (defs.PopulationCount() == 1) && _writtenExactlyOnce.ValueAt(useNode->getSymbolReference()->getReferenceNumber())) {
        TR_UseDefInfo::BitVector::Cursor cursor(defs);
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            int32_t defIndex = cursor;
            if (defIndex < useDefInfo->getFirstRealDefIndex())
...
bool TR_LoopVersioner::detectInvariantBoundChecks(List<TR::TreeTop>* boundCheckTrees)
{
    ...
    for (; nextTree;) {
        ...
        TR::Node* boundNode = (node->getOpCodeValue() == TR::BNDCHKwithSpineCHK) ? node->getChild(2) : node->getFirstChild();
        ...
        if (!isMaxBoundInvariant) {
            if (boundNode->getOpCode().isArrayLength()) {
                ...
                if (arrayObject->getOpCode().hasSymbolReference() && arrayObject->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(arrayObject))
                    isMaxBoundInvariant = true;
            } else {
                if (boundNode->getOpCode().hasSymbolReference() && boundNode->getSymbolReference()->getSymbol()->isAuto() && isDependentOnInvariant(boundNode))
                    isMaxBoundInvariant = true;
            }
        }
        ...
        if (isMaxBoundInvariant) {
            ...
            if (!isIndexInvariant && _loopConditionInvariant && (node->getOpCodeValue() == TR::BNDCHK || node->getOpCodeValue() == TR::BNDCHKwithSpineCHK)) {
                if (indexChild->getOpCode().hasSymbolReference())
                    indexSymRef = indexChild->getSymbolReference();
                else {
                    bool goodAccess = isVersionableArrayAccess(indexChild);
                    while (indexChild->getOpCode().isAdd() || indexChild->getOpCode().isSub() || indexChild->getOpCode().isAnd() || indexChild->getOpCode().isMul()) {
                        if (indexChild->getSecondChild()->getOpCode().isLoadConst())
                            indexChild = indexChild->getFirstChild();
                        else {
                            bool isSecondChildInvariant = isExprInvariant(indexChild->getSecondChild());
                ...
                while (indexSymRef && changedIndexSymRef) {
                    ...
                    !strncmp(comp()->signature(), "banshee/scan/encoding/Latin1EncodingSupport.load", 48) */
                        ...
                        if (_loopTestTree && (_loopTestTree->getNode()->getNumChildren() > 1) && ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple))) {
                            ...
                            if (symbolInCompare) {
                                ...
                                for (v = _currentNaturalLoop->getFirstInductionVariable(); v; v = v->getNext()) {
                                    if ((v->getLocal() == indexSymRef->getSymbol()) && (v->getLocal() == symbolInCompare) && (v->getLocal()->getDataType() == TR::Int32) && (v->isSigned())) {
                    ...
                    if (!isInductionVariable) {
                        ...
                        if (!indexChild || !indexChild->getOpCode().hasSymbolReference() || !indexChild->getSymbolReference()->getSymbol()->isAutoOrParm() || (indexChild->getSymbolReference()->getReferenceNumber() == indexSymRef->getReferenceNumber())) {
                            break;
                        } else {
                            indexSymRef = indexChild->getSymbolReference();
...
bool TR_LoopVersioner::detectInvariantDivChecks(List<TR::TreeTop>* divideCheckTrees)
{
    ...
    for (; nextTree;) {
        ...
        if ((node->getFirstChild()->getOpCodeValue() == TR::idiv) || (node->getFirstChild()->getOpCodeValue() == TR::irem) || (node->getFirstChild()->getOpCodeValue() == TR::ldiv) || (node->getFirstChild()->getOpCodeValue() == TR::lrem)) {
...
bool TR_LoopVersioner::isDependentOnAllocation(TR::Node* useNode, int32_t recursionDepth)
{
    ...
    if (useDefInfo->getUseDef(defs, useIndex)) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (heapificationStore || (child->getOpCode().isNew()) || ((child->getOpCodeValue() == TR::loadaddr) && child->getSymbol()->isLocalObject()))
                pointsToNew = true;
            else {
                if (recursionDepth > 0) {
...
bool TR_LoopVersioner::detectInvariantAwrtbaris(List<TR::TreeTop>* awrtbariTrees)
{
    ...
    for (; nextTree;) {
        ...
        if (node->getOpCodeValue() == TR::awrtbari) {
            ...
            if (baseChild->getOpCode().hasSymbolReference() && (baseChild->getOpCodeValue() == TR::aload) && baseChild->getSymbol()->isAutoOrParm()) {
...
bool opCodeIsHoistable(TR::Node* node, TR::Compilation* comp)
{
    ...
    hasEffects = (opCode.isCall() || opCodeValue == TR::New || opCodeValue == TR::newarray || opCodeValue == TR::anewarray || opCodeValue == TR::multianewarray) || symReference->isUnresolved() || symReference->getSymbol()->isLocalObject() || (symReference->getSymbol()->isArrayShadowSymbol() && comp->requiresSpineChecks());
    return !hasEffects;
}
...
bool TR_LoopVersioner::isExprInvariantRecursive(TR::Node* node, bool ignoreHeapificationStore)
{
    ...
    if (opCode.hasSymbolReference()) {
        ...
        if ((_seenDefinedSymbolReferences->get(symReference->getReferenceNumber()) && (!ignoreHeapificationStore || _writtenAndNotJustForHeapification->get(symReference->getReferenceNumber()))) || !opCodeIsHoistable(node, comp()) /* ||
    ...
    bool& isIncreasing, TR::SymbolReference*& firstChildSymRef)
{
    ...
    if (((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt))) {
        returnValue = true;
        isIncreasing = blocksInWhileLoop.find(_loopTestTree->getNode()->getBranchDestination()->getNode()->getBlock());
    }

    if (((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt))) {
        returnValue = true;
        isIncreasing = !blocksInWhileLoop.find(_loopTestTree->getNode()->getBranchDestination()->getNode()->getBlock());
    }
    ...
    if (returnValue) {
        ...
        if (firstChild->getOpCode().hasSymbolReference())
            ...
            if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
...
bool TR_LoopVersioner::detectChecksToBeEliminated(TR_RegionStructure* whileLoop, List<TR::Node>* nullCheckedReferences, List<TR::TreeTop>* nullCheckTrees, List<int32_t>* numIndirections, List<TR::TreeTop>* boundCheckTrees, List<TR::TreeTop>* spineCheckTrees, List<int32_t>* numDimensions, List<TR::TreeTop>* conditionalTrees, List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* awrtbariTrees, List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::Node>* specializedInvariantNodes, List<TR_NodeParentSymRef>* invariantNodes, List<TR_NodeParentSymRefWeightTuple>* invariantTranslationNodesList, bool& discontinue)
{
    ...
    for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        ...
        if (nextBlockStructure && nextBlockStructure->wasHeaderOfCanonicalizedLoop() && nextBlock->hasSuccessor(whileLoop->getEntryBlock()))
            ...
            if ((!aggressive || comp()->getMethodHotness() <= warm)
                ...
                if (trace() || comp()->getOption(TR_CountOptTransformations)) {
                    for (TR::TreeTop* tt = entryTree; isUnimportant && tt != exitTree; tt = tt->getNextTreeTop()) {
                        ...
                        if (op.isCheck() || op.isCheckCast() || (op.isBranch() && ttNode->getNumChildren() >= 2 && (!tt->getNode()->getFirstChild()->getOpCode().isLoadConst() || !tt->getNode()->getSecondChild()->getOpCode().isLoadConst()))) {
        ...
        while (!(currentTree == exitTree)) {
            ...
            if (currentOpCode.getOpCodeValue() == TR::asynccheck) {
                ...
                if (_loopTestTree && (_loopTestTree->getNode()->getNumChildren() > 1) && _asyncCheckTree && shouldOnlySpecializeLoops()) {
                    bool isIncreasing;
            ...
            if (!disableLT && currentNode->isTheVirtualGuardForAGuardedInlinedCall() && blocksInWhileLoop.find(currentNode->getBranchDestination()->getNode()->getBlock())) {
            ...
            if (!isUnimportant && !_containsCall && !refineAliases()) {
                if (currentOpCode.isNullCheck()) {
                    ...
                    if (dupOfThisBlockAlreadyExecutedBeforeLoop)
                        _checksInDupHeader.add(currentTree);
                    foundPotentialChecks = true;
                } else if (currentOpCode.isBranch() && isBranchSuitableToVersion(&blocksInWhileLoop, currentNode, comp()) && (currentOpCode.getOpCodeValue() != TR::Goto)) {
                ...
                } else if (currentOpCode.isCheckCast()) // (currentOpCode.getOpCodeValue() == TR::checkcast)
                {
                    if (currentNode->getSecondChild()->getOpCodeValue() == TR::loadaddr) {
                        void* clazz = currentNode->getSecondChild()->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress();
                ...
                //SPECpower Work
                if (TR::Compiler->om.writeBarrierType() == gc_modron_wrtbar_cardmark_and_oldcheck || TR::Compiler->om.writeBarrierType() == gc_modron_wrtbar_oldcheck) {
                    ...
                    if ((possibleAwrtbariNode->getOpCodeValue() == TR::awrtbari) && !possibleAwrtbariNode->skipWrtBar() && !hasWrtbarBeenSeen(awrtbariTrees, possibleAwrtbariNode)) {
                        if (trace())
    ...
    TR::Node* lastFault, bool collectProfiledExprs, TR::Block* block, int32_t warmBranchCount)
{
    ...
    if (_inNullCheckReference || !_nullCheckReference) {
        ...
        if (_inNullCheckReference && comp()->requiresSpineChecks() && node->getOpCode().hasSymbolReference() && node->getSymbol()->isArrayShadowSymbol())
            _containsCall = true;
    }

    static char* profileLongParms = feGetEnv("TR_ProfileLongParms");
    if (TR_LocalAnalysis::isSupportedOpCode(node->getOpCode(), comp())) {
        if (node->getType().isInt32() || (node->getType().isInt64() && node->getOpCode().isLoadVar() && node->getSymbolReference()->getSymbol()->isAutoOrParm())) {
            ...
            if (/* comp()->getRecompilationInfo() && */ collectProfiledExprs && ((!node->getType().isInt64()) || (profileLongParms /* && (valueInfo->getTopValue() == 0) */))) {
#ifdef J9_PROJECT_SPECIFIC
                TR_ValueInfo* valueInfo = static_cast<TR_ValueInfo*>(TR_ValueProfileInfoManager::getProfiledValueInfo(node, comp(), ValueInfo));
                if (valueInfo) {
                    if ((valueInfo->getTopProbability() > MIN_PROFILED_FREQUENCY) && (valueInfo->getTotalFrequency() > 0) && !_containsCall && !node->getByteCodeInfo().doNotProfile() && !node->isNodeCreatedByPRE() &&
        ...
        if (invariantNodes && collectProfiledExprs && !_containsCall && ((node->getOpCode().getSize() >= 4) || (opCodeValue == TR::computeCC)) && TR_LocalAnalysis::isSupportedNode(node, comp(), NULL) && ((node->getOpCode().isLoadVar() && !node->getSymbolReference()->getSymbol()->isAutoOrParm()) || !node->getOpCode().hasSymbolReference())) {
            if (trace())
...
void TR_LoopVersioner::versionNaturalLoop(TR_RegionStructure* whileLoop, List<TR::Node>* nullCheckedReferences, List<TR::TreeTop>* nullCheckTrees, List<TR::TreeTop>* boundCheckTrees, List<TR::TreeTop>* spineCheckTrees, List<TR::TreeTop>* conditionalTrees, List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* awrtbariTrees, List<TR::TreeTop>* checkCastTrees, List<TR::TreeTop>* arrayStoreCheckTrees, List<TR::Node>* specializedNodes, List<TR_NodeParentSymRef>* invariantNodes, List<TR_NodeParentSymRefWeightTuple>* invariantTranslationNodesList, List<TR_Structure>* innerWhileLoops, List<TR_Structure>* clonedInnerWhileLoops, bool skipVersioningAsynchk, SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    if (_loopConditionInvariant && _asyncCheckTree) {
        ...
        if (!comp()->getOption(TR_DisableAsyncCheckVersioning) && !refineAliases() && _canPredictIters && comp()->getProfilingMode() != JitProfiling && performTransformation(comp(), "%s Creating test outside loop for deciding if async check is required\n", OPT_DETAILS_LOOP_VERSIONER)) {
            ...
            TR::Node* loopLimit = isIncreasing ? TR::Node::create(TR::isub, 2, upperBound, lowerBound) : TR::Node::create(TR::isub, 2, lowerBound, upperBound);
    ...
    if (!boundCheckTrees->isEmpty()) {
        ...
        if (_loopConditionInvariant && !blocksInWhileLoop.find(_loopTestTree->getNode()->getBranchDestination()->getNode()->getBlock()))
            reverseBranch = true;
    ...
    List<TR::TreeTop>* nullCheckTrees)
{
    ...
    for (; nextNode; nextNode = nextNode->getNextElement(), nextTree = nextTree->getNextElement()) {
        ...
        if (!isNextNodeInvariant) {
            bool nullCheckReferenceisField = nextNode->getData()->getOpCode().isLoadIndirect() && nextNode->getData()->getOpCode().hasSymbolReference() && !_seenDefinedSymbolReferences->get(nextNode->getData()->getSymbolReference()->getReferenceNumber()) && nextNode->getData()->getFirstChild()->getOpCode().hasSymbolReference() && nextNode->getData()->getFirstChild()->getSymbolReference()->getSymbol()->isAuto();

            if ((nextNode->getData()->getOpCode().hasSymbolReference() && nextNode->getData()->getSymbolReference()->getSymbol()->isAuto()) || nullCheckReferenceisField) {
...
void TR_LoopVersioner::RemoveArrayStoreCheck::improveLoop()
{
    ...
    if (child->getOpCodeValue() == TR::awrtbari && TR::Compiler->om.writeBarrierType() == gc_modron_wrtbar_none && performTransformation(comp(), "%sChanging awrtbari node [%p] to an iastore\n", OPT_DETAILS_LOOP_VERSIONER, child)) {
    ...
    TR::SymbolReference** symRefs)
{
    ...
    while (nextSpecializedNode) {
        ...
        if (!isNextNodeInvariant) {
            if (specializedNode->getOpCode().hasSymbolReference() && specializedNode->getSymbolReference()->getSymbol()->isAuto()) {
    ...
    SharedSparseBitVector& reverseBranchInLoops)
{
    ...
    while (nextTree) {
        ...
        if (conditionalNode->isTheVirtualGuardForAGuardedInlinedCall() && !conditionalNode->isNonoverriddenGuard() && !conditionalNode->isHCRGuard() && !conditionalNode->isBreakpointGuard()) {
            ...
            if (guardInfo->getTestType() == TR_VftTest) {
                ...
                if ((conditionalNode->getFirstChild()->getNumChildren() > 0) && (conditionalNode->getFirstChild()->getFirstChild()->getNumChildren() > 0)) {
        ...
        if (performTransformation(
            ...
            if (trace())
                traceMsg(comp(), "Frequency Test for conditional node [%p], destination Frequency %d, fallThrough frequency %d\n", conditionalNode, destBlock->getFrequency(), fallThroughBlock->getFrequency());
            if (reverseBranchInLoops[origConditionalNode->getGlobalIndex()]) {
                if (trace())
                    traceMsg(comp(), "Branch reversed for conditional node [%p], destination Frequency %d, fallThrough frequency %d\n", conditionalNode, destBlock->getFrequency(), fallThroughBlock->getFrequency());
                reverseBranch = true;
            }
            bool isInductionVar = false;
            ...
            if (conditionalNode->isVersionableIfWithMaxExpr() || conditionalNode->isVersionableIfWithMinExpr()) {
                ...
                if (isInductionVar && ((conditionalNode->isVersionableIfWithMaxExpr() && isAddition) || (conditionalNode->isVersionableIfWithMinExpr() && !isAddition))) {
                    ...
                    if (duplicateIndexNode->getOpCode().isLoad() && duplicateIndexNode->getSymbolReference()->getReferenceNumber() == indexSymRefNum) {
...
void TR_LoopVersioner::RemoveSpineCheck::improveLoop()
{
    ...
    TR::Node* shiftNode = is64BitTarget ? TR::Node::lconst(spineCheckNode, (int64_t)shift) : TR::Node::iconst(spineCheckNode, shift);

    int32_t strideShift = TR::TransformUtil::convertWidthToShift(elementSize);
    TR::Node* strideShiftNode = NULL;
    if (strideShift)
        strideShiftNode = is64BitTarget ? TR::Node::lconst(spineCheckNode, (int64_t)strideShift) : TR::Node::iconst(spineCheckNode, strideShift);

    int32_t arraySpineShift = fe()->getArraySpineShift(elementSize);
    TR::Node* spineShiftNode = is64BitTarget ? TR::Node::lconst(spineCheckNode, (int64_t)arraySpineShift) : TR::Node::iconst(spineCheckNode, arraySpineShift);
    ...
    if (arrayTT->getFirstChild()->getOpCode().hasSymbolReference() && arrayTT->getFirstChild()->getSymbol()->isArrayShadowSymbol()) {
    ...
    while (nextTree) {
        ...
        if (!isMaxBoundInvariant) {
            if (arrayLengthNode->getOpCode().isArrayLength()) {
                ...
                if (arrayLengthNode->getOpCode().hasSymbolReference() && arrayLengthNode->getSymbolReference()->getSymbol()->isAuto()) {
        ...
        if (isIndexInvariant) {
            ...
            while (indexSymRef && changedIndexSymRef) {
                ...
                if (!foundInductionVariable) {
                    ...
                    if (!indexChild || !indexChild->getOpCode().hasSymbolReference() || !indexChild->getSymbolReference()->getSymbol()->isAutoOrParm() || (indexChild->getSymbolReference()->getReferenceNumber() == indexSymRefNum)) {
                        break;
                    } else {
                        indexSymRef = indexChild->getSymbolReference();
            ...
            if (!valueChild->getOpCode().hasSymbolReference() || (valueChild->getSymbolReference()->getReferenceNumber() != indexChild->getSymbolReference()->getReferenceNumber()))
                complexButPredictableForm = false;
            ...
            if (changedIndexSymRefAtSomePoint || (!_unchangedValueUsedInBndCheck->get(boundCheckNode->getGlobalIndex()) && !isSpecialInductionVariable)) {
                if (_storeTrees[origIndexSymRef->getReferenceNumber()]) {
                    //printf("Changing test in %s\n", comp()->signature());
                    TR::Node* storeNode = _storeTrees[origIndexSymRef->getReferenceNumber()]->getNode();
                    TR::Node* storeRhs = storeNode->getFirstChild()->duplicateTree();
            ...
            if (isSpecialInductionVariable) {
                ...
                if (isAddition) {
                    if (!indVarOccursAsSecondChildOfSub) {
                        if (isLoopDrivingInductionVariable) {
                            if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt))
                        ...
                        if (isLoopDrivingInductionVariable) {
                            if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge))
            ...
            if (!isSpecialInductionVariable) {
                ...
                if (!isIndexChildMultiplied) {
                    ...
                    if (isAddition) {
                        ...
                        if ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt))
    ...
    bool canPrivatizeRootNode)
{
    ...
    //
    if (node->isInternalPointer() || (((node->getOpCode().isIndirect() && node->getOpCode().hasSymbolReference() && !node->getSymbolReference()->getSymbol()->isStatic()) || node->getOpCode().isArrayLength()) && !node->getFirstChild()->isInternalPointer())) {
        ...
        if (node->isInternalPointer() && (firstChild->isInternalPointer() || (firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbolReference()->getSymbol()->isAuto() && firstChild->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer())))
            ...
            if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
                ...
                if (!instanceOfReqd) {
                    ...
                    TR::SymbolReference* otherClassSymRef = comp()->getSymRefTab()->findOrCreateClassSymbol(
                        owningMethodSym,
                        classCPI,
                        otherClassObject);
        ...
        for (int32_t index = sizeof(a) / sizeof(temps) - 1; index >= 0; --index) {
            if (offset->getOpCodeValue() == a[index].addOp && offset->getSecondChild()->getOpCodeValue() == a[index].constOp && offset->getSecondChild()->getInt() == a[index].k) {
        ...
        if (childInRequiredForm) {
            if (childInRequiredForm->getOpCodeValue() == TR::ishl || childInRequiredForm->getOpCodeValue() == TR::lshl) {
                if (childInRequiredForm->getSecondChild()->getOpCode().isLoadConst() && (childInRequiredForm->getSecondChild()->getInt() == shiftWidth))
                ...
                if (childInRequiredForm->getSecondChild()->getOpCode().isLoadConst() && (childInRequiredForm->getSecondChild()->getInt() == dataWidth))
                    indexNode = childInRequiredForm->getFirstChild();
            }
        }
...
bool TR_LoopVersioner::boundCheckUsesUnchangedValue(TR::TreeTop* bndCheckTree, TR::Node* node, TR::SymbolReference* symRef, TR_RegionStructure* loopStructure)
{
    ...
    if ((currentBlock->getSuccessors().size() == 1) && (currentBlock->getSuccessors().front()->getTo() == _loopTestBlock)) {
        if (findStore(bndCheckTree, currentBlock->getExit(), node, symRef, true) || findStore(_loopTestBlock->getEntry(), _loopTestBlock->getExit(), node, symRef, true))
            return true;
        else
            return false;
    }
...
int32_t TR_LoopVersioner::detectCanonicalizedPredictableLoops(TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (loopInvariantBlock) {
        /////if (isInvariantLoop)
        {
            ...
            if ((isPredictableLoop > 0) && ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge) || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple))) {
                if (trace()) {
                ...
                for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                    ...
                    if (storeInRequiredForm /* && !_cannotBeEliminated->get(nextInductionVariableNumber) */) {
                        ...
                        *versionableInductionVariable = nextInductionVariableNumber;
                        ...
                        if ((loopTestNode->getFirstChild()->getOpCode().hasSymbolReference() && nextInductionVariableNumber == loopTestNode->getFirstChild()->getSymbolReference()->getReferenceNumber()) || loopTestNode->getFirstChild() == _storeTrees[nextInductionVariableNumber]->getNode()->getFirstChild())
                    ...
                    for (TR::Block* b = eb.getFirst(); b; b = eb.getNext()) {
                        ...
                        if (ifNode->getOpCode().isIf() && !ifNode->getOpCode().isCompBranchOnly()) {
                            ...
                            if (ivNode->getOpCode().hasSymbolReference())
                                ...
                                if ((ivNode->getOpCode().isAdd() || ivNode->getOpCode().isSub()) && ivNode->getSecondChild()->getOpCode().isLoadConst())
...
bool TR_LoopVersioner::isStoreInSpecialForm(int32_t symRefNum, TR_Structure* loopStructure)
{
    ...
    if (storeNode->getFirstChild()->getOpCode().isAnd() && storeNode->getFirstChild()->getSecondChild()->getOpCode().isLoadConst() && (storeNode->getFirstChild()->getSecondChild()->getInt() > 0) && (storeNode->getFirstChild()->getFirstChild()->getOpCodeValue() == TR::iload) && (storeNode->getFirstChild()->getFirstChild()->getSymbolReference()->getReferenceNumber() == storeNode->getSymbolReference()->getReferenceNumber()))
        return true;
...
bool TR_LoopVersioner::isStoreInRequiredForm(int32_t symRefNum, TR_Structure* loopStructure)
{
    ...
    if (!secondChild->getOpCode().isLoadConst() && (secondChild->getType().isInt32() || secondChild->getType().isInt64())) {
        ...
        if (((secondChild->getType().isInt32()) && (secondChild->getInt() < 0)) || ((secondChild->getType().isInt64()) && (secondChild->getLongInt() < 0))) {
            if (_isAddition)
                _isAddition = false;
            else
                _isAddition = true;
...
{
    if (node->getOpCode().isConversion() && node->getFirstChild()->getOpCode().isConversion() && (node->isNonNegative() || node->isNonPositive())) {
        TR::Node* conversion1 = node;
        TR::Node* conversion2 = node->getFirstChild();
        if ((conversion1->getOpCodeValue() == TR::s2i && conversion2->getOpCodeValue() == TR::i2s) || (conversion1->getOpCodeValue() == TR::b2i && conversion2->getOpCodeValue() == TR::i2b) || // FIXME: why are we ignoring bu2i/i2b here?
