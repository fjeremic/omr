...
class TR_ArrayLoop {
 public:
  ...
  void findIndVarLoads(TR::Node* node,
                       TR::Node* indVarStoreNode,
                       bool& storeFound,
...
class TR_ByteToCharArraycopy : public TR_ArrayLoop {
 public:
  TR_ByteToCharArraycopy(TR::Compilation*,
                         TR_InductionVariable* firstIndVar,
                         TR_InductionVariable* secondIndVar,
                         bool bigEndian);
...
class TR_CharToByteArraycopy : public TR_ArrayLoop {
 public:
  TR_CharToByteArraycopy(TR::Compilation*,
                         TR_InductionVariable* firstIndVar,
                         TR_InductionVariable* secondIndVar,
                         bool isBigEndian);
...
class TR_Arraytranslate : public TR_ArrayLoop {
 public:
  TR_Arraytranslate(TR::Compilation*,
                    TR_InductionVariable* indVar,
                    bool hasBranch,
                    bool hasBreak);

  bool checkGoto(TR::Block* gotoBlock,
                 TR::Node* gotoNode,
                 TR::Block* subsequentBlock);
  bool checkLoad(TR::Node* loadNode);
  bool checkStore(TR::Node* storeNode);
  bool checkBreak(TR::Block* breakBlock,
                  TR::Node* breakNode,
                  TR::Block* nextBlock);
...
class TR_ArraytranslateAndTest : public TR_ArrayLoop {
 public:
  ...
  bool checkFrequency(TR::CodeGenerator* cg,
                      TR::Block* loadBlock,
                      TR::Node* loadNode);
...
class TR_LoopReducer : public TR_LoopTransformer {
 ...
 private:
  ...
  int addBlock(TR::Block* newBlock,
               TR::Block** blocks,
               int numBlock,
               const int maxNumBlock);
  int addRegionBlocks(TR_RegionStructure* region,
                      TR::Block** blocks,
                      int numBlock,
                      const int maxNumBlock);
  ...
  bool mayNeedGlobalDeadStoreElimination(TR::Block* storeBlock,
                                         TR::Block* compareBlock);
  bool replaceInductionVariable(TR::Node* parent,
                                TR::Node* node,
                                int32_t,
                                int32_t,
                                TR::Node*,
