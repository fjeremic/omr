...
//  Copy byte into a portion of an array (C memset) (clearing with value 0
//  special) Copy a portion of an array into a portion of an array (C
//  memcpy/memmove) Translate a portion of an array via a translation table
//  (S/390 TRT/TROT/TRTO/TRTT/TROO instructions)
//
...
class TR_LRAddressTree : public TR_AddressTree {
 public:
  ...
  TR::SymbolReference *getMaterializedIndVarSymRef() {
    return _matIndVarSymRef;
  }
  void setMaterializedIndVarSymRef(TR::SymbolReference *symRef) {
    _matIndVarSymRef = symRef;
  }
...
class TR_ArrayLoop {
 public:
  ...
  TR_ArrayLoop(TR::Compilation *, TR_InductionVariable *firstIndVar,
               TR_InductionVariable *secondIndVar);

  TR::Compilation *comp() { return _comp; }

  bool checkLoopCmp(TR::Node *loopCmpNode, TR::Node *indVarNode,
                    TR_InductionVariable *indVar);

  int32_t checkForPostIncrement(TR::Block *loopHeader,
                                TR::Node *indVarStoreNode,
                                TR::Node *loopCmpNode, TR::Symbol *ivSym);

  void findIndVarLoads(TR::Node *node, TR::Node *indVarStoreNode,
                       bool &storeFound, List<TR::Node> *ivLoads,
                       TR::Symbol *ivSym, vcount_t visitCount);

  TR::Node *updateIndVarStore(TR_ParentOfChildNode *indVarNode,
                              TR::Node *indVarStoreNode, TR_LRAddressTree *tree,
...
class TR_Arrayset : public TR_ArrayLoop {
 public:
  ...
  TR_ParentOfChildNode *getMultiplyNode() {
    return getFirstAddress()->getMultiplyNode();
  }

  TR_ParentOfChildNode *getIndVarNode() {
    return getFirstAddress()->getIndVarNode();
  }
...
class TR_Arraycopy : public TR_ArrayLoop {
 public:
  ...
  TR_ParentOfChildNode *getStoreMultiplyNode() {
    return getFirstAddress()->getMultiplyNode();
  }
  TR_ParentOfChildNode *getLoadMultiplyNode() {
    return (getSecondAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getLoadIndVarNode() {
    return (getFirstAddress()->getIndVarNode());
  }
  TR_ParentOfChildNode *getStoreIndVarNode() {
    return (getSecondAddress()->getIndVarNode());
  }
...
class TR_ByteToCharArraycopy : public TR_ArrayLoop {
 public:
  TR_ByteToCharArraycopy(TR::Compilation *, TR_InductionVariable *firstIndVar,
  ...
  TR_ParentOfChildNode *getStoreMultiplyNode() {
    return (getStoreAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getStoreIndVarNode() {
    return (getStoreAddress()->getIndVarNode());
  }
  TR_ParentOfChildNode *getHighLoadMultiplyNode() {
    return (getHighLoadAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getHighLoadIndVarNode() {
    return (getHighLoadAddress()->getIndVarNode());
  }
  TR_ParentOfChildNode *getLowLoadMultiplyNode() {
    return (getLowLoadAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getLowLoadIndVarNode() {
    return (getLowLoadAddress()->getIndVarNode());
  }
...
class TR_CharToByteArraycopy : public TR_ArrayLoop {
 public:
  TR_CharToByteArraycopy(TR::Compilation *, TR_InductionVariable *firstIndVar,
                         TR_InductionVariable *secondIndVar, bool isBigEndian);
  ...
  TR_ParentOfChildNode *getHighStoreMultiplyNode() {
    return (getHighStoreAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getHighStoreIndVarNode() {
    return (getHighStoreAddress()->getIndVarNode());
  }
  TR_ParentOfChildNode *getLowStoreMultiplyNode() {
    return (getLowStoreAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getLowStoreIndVarNode() {
    return (getLowStoreAddress()->getIndVarNode());
  }
...
class TR_Arraycmp : public TR_ArrayLoop {
 public:
  ...
  TR_ParentOfChildNode *getFirstMultiplyNode() {
    return (getFirstAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getSecondMultiplyNode() {
    return (getSecondAddress()->getMultiplyNode());
  }
  TR_ParentOfChildNode *getSecondIndVarNode() {
    return (getFirstAddress()->getIndVarNode());
  }
  TR_ParentOfChildNode *getFirstIndVarNode() {
    return (getSecondAddress()->getIndVarNode());
  }
...
class TR_Arraytranslate : public TR_ArrayLoop {
 public:
  TR_Arraytranslate(TR::Compilation *, TR_InductionVariable *indVar,
                    bool hasBranch, bool hasBreak);

  bool checkGoto(TR::Block *gotoBlock, TR::Node *gotoNode,
                 TR::Block *subsequentBlock);
  bool checkLoad(TR::Node *loadNode);
  bool checkStore(TR::Node *storeNode);
  bool checkBreak(TR::Block *breakBlock, TR::Node *breakNode,
                  TR::Block *nextBlock);
  bool checkMatIndVarStore(TR::Node *matIndVarStoreNode,
                           TR::Node *indVarStoreNode);
...
class TR_ArraytranslateAndTest : public TR_ArrayLoop {
 public:
  ...
  bool checkFrequency(TR::CodeGenerator *cg, TR::Block *loadBlock,
                      TR::Node *loadNode);
...
class TR_LoopReducer : public TR_LoopTransformer {
 ...
 private:
  ...
  int addBlock(TR::Block *newBlock, TR::Block **blocks, int numBlock,
               const int maxNumBlock);
  int addRegionBlocks(TR_RegionStructure *region, TR::Block **blocks,
                      int numBlock, const int maxNumBlock);
  bool constrainedIndVar(TR_InductionVariable *indVar);

  TR::ILOpCodes convertIf(TR::ILOpCodes ifCmp);

  bool generateArraytranslateAndTest(TR_RegionStructure *whileLoop,
                                     TR_InductionVariable *indVar,
                                     TR::Block *firstBlock,
                                     TR::Block *secondBlock);
  bool generateArraycopy(TR_InductionVariable *indVar, TR::Block *loopHeader);
  bool generateByteToCharArraycopy(TR_InductionVariable *firstIndVar,
                                   TR_InductionVariable *secondIndVar,
                                   TR::Block *loopHeader);
  bool generateCharToByteArraycopy(TR_InductionVariable *firstIndVar,
                                   TR_InductionVariable *secondIndVar,
                                   TR::Block *loopHeader);

  bool generateArraytranslate(TR_RegionStructure *whileLoop,
                              TR_InductionVariable *indVar,
                              TR::Block *firstBlock, TR::Block *secondBlock,
                              TR::Block *thirdBlock, TR::Block *fourthBlock);
  bool generateArrayset(TR_InductionVariable *indVar, TR::Block *loopHeader);
  bool generateArraycmp(TR_RegionStructure *whileLoop,
                        TR_InductionVariable *indVar, TR::Block *compareBlock,
                        TR::Block *incrementBlock);
  bool blockInVersionedLoop(List<TR::CFGEdge> succList, TR::Block *block);

  bool mayNeedGlobalDeadStoreElimination(TR::Block *storeBlock,
                                         TR::Block *compareBlock);
  bool replaceInductionVariable(TR::Node *parent, TR::Node *node, int32_t,
                                int32_t, TR::Node *, vcount_t);
};
