...
public:
  ...
  public:
    SwitchInfo(TR_Memory *m)
        : _kind(Dense), _count(0), _freq(0), _cost(0), _min(INT_MAX),
          _max(INT_MIN) {
      _chain = new (m->trHeapMemory()) TR_LinkHead<SwitchInfo>();
    }

    SwitchInfo(CASECONST_TYPE value, TR::TreeTop *target, int32_t cost)
        : _kind(Unique), _count(1), _freq(0), _min(value), _max(value),
          _target(target), _cost(cost) {}
  ...
  void emit(TR_LinkHead<SwitchInfo> *chain, TR_LinkHead<SwitchInfo> *bound,
            TR_LinkHead<SwitchInfo> *earlyUniques);

  TR::Block *binSearch(SwitchInfo *startNode, SwitchInfo *endNode,
                       int32_t numMajors, CASECONST_TYPE rangeLeft,
                       CASECONST_TYPE rangeRight);
  TR::Block *addIfBlock(TR::ILOpCodes opCode, CASECONST_TYPE val,
                        TR::TreeTop *dest);
  ...
  void printInfo(TR_FrontEnd *, TR::FILE *pOutFile,
                 TR_LinkHead<SwitchInfo> *head);
