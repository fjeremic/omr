...
public:
  TR_CallStack(TR::Compilation *, TR::ResolvedMethodSymbol *,
               TR_ResolvedMethod *, TR_CallStack *, int32_t,
  ...
  bool isAnywhereOnTheStack(
      TR_ResolvedMethod *,
      int32_t); // Even searches calls already inlined by previous opts
...
struct TR_CallTarget : public TR_Link<TR_CallTarget> {
  ...
  TR_CallTarget(TR_CallSite *callsite, TR::ResolvedMethodSymbol *calleeSymbol,
  ...
  TR::ResolvedMethodSymbol *
      _calleeSymbol; // must have this by the time we actually do the inlinining
  ...
  bool _alreadyInlined; // so we don't keep trying to inline the same method
                        // repeatedly
  ...
  TR_PrexArgInfo *_prexArgInfo; // used by computePrexInfo to calculate prex on
                                // generatedIL and transform IL
  TR_PrexArgInfo *_ecsPrexArgInfo; // used by ECS and findInlineTargets to
                                   // assist in choosing correct inline targets
  ...
  TR_InlinerFailureReason getCallTargetFailureReason() {
...
#define TR_CALLSITE_INHERIT_CONSTRUCTOR_COMMON(EXTENDED, BASE)                 \
  EXTENDED(TR_ResolvedMethod *callerResolvedMethod,                            \
           ...
           TR::Method *interfaceMethod, TR_OpaqueClassBlock *receiverClass,    \
           int32_t vftSlot, int32_t cpIndex,                                   \
           TR_ResolvedMethod *initialCalleeMethod,                             \
           TR::ResolvedMethodSymbol *initialCalleeSymbol, bool isIndirectCall, \
           bool isInterface, TR_ByteCodeInfo &bcInfo, TR::Compilation *comp,   \
           int32_t depth = -1, bool allConsts = false)                         \
      : BASE(callerResolvedMethod, callNodeTreeTop, parent, callNode,          \
             interfaceMethod, receiverClass, vftSlot, cpIndex,                 \
             initialCalleeMethod, initialCalleeSymbol, isIndirectCall,         \
             isInterface, bcInfo, comp, depth, allConsts)
 ...
 *    Used by call sites constructors that initialize given fields and do
 * nothing else.
 */
 ...
 *    Used by call sites constructors that do more complicated things other than
 * just initializing given field.
 */
...
struct TR_CallSite : public TR_Link<TR_CallSite> {
  ...
  TR_CallSite(TR_ResolvedMethod *callerResolvedMethod,
              ...
              int32_t cpIndex, TR_ResolvedMethod *initialCalleeMethod,
              TR::ResolvedMethodSymbol *initialCalleeSymbol,
              bool isIndirectCall, bool isInterface, TR_ByteCodeInfo &bcInfo,
              TR::Compilation *comp, int32_t depth = -1,
              bool allConsts = false);

  TR_CallSite(const TR_CallSite &other)
      : _allConsts(other._allConsts), _bcInfo(other._bcInfo),
        _byteCodeIndex(other._byteCodeIndex), _callerBlock(other._callerBlock),
        _callerResolvedMethod(other._callerResolvedMethod),
        _callNode(other._callNode), _callNodeTreeTop(other._callNodeTreeTop),
        _cursorTreeTop(other._cursorTreeTop), _comp(other._comp),
        _cpIndex(other._cpIndex), _depth(other._depth),
        _ecsPrexArgInfo(other._ecsPrexArgInfo),
        _failureReason(other._failureReason), _forceInline(other._forceInline),
        ...
        _isBackEdge(other._isBackEdge), _isIndirectCall(other._isIndirectCall),
        _isInterface(other._isInterface),
        _mytargets(0, other._comp->allocator()),
        _myRemovedTargets(0, other._comp->allocator()), _parent(other._parent),
        _receiverClass(other._receiverClass), _stmtNo(other._stmtNo),
        _unavailableTemps(other._comp->trMemory()),
        _unavailableBlockTemps(other._comp->trMemory()),
        _vftSlot(other._vftSlot), _visitCount(other._visitCount) {}
  ...
  void tagcalltarget(int32_t index, TR_InlinerTracer *tracer,
                     TR_InlinerFailureReason reason);
  void tagcalltarget(TR_CallTarget *ct, TR_InlinerTracer *tracer,
                     TR_InlinerFailureReason reason);
  void removecalltarget(int32_t index, TR_InlinerTracer *tracer,
                        TR_InlinerFailureReason reason);
  void removecalltarget(TR_CallTarget *ct, TR_InlinerTracer *tracer,
                        TR_InlinerFailureReason reason);
  void removeAllTargets(TR_InlinerTracer *tracer,
                        TR_InlinerFailureReason reason);
  void removeTargets(TR_InlinerTracer *tracer, int index,
                     TR_InlinerFailureReason reason);
  TR_CallTarget *getTarget(int32_t i) {
    TR_ASSERT(i >= 0 && i < _mytargets.size(),
              "indexing a target out of range.");
    return _mytargets[i];
  }
  ...
  TR_CallTarget *addTarget(TR_Memory *, TR_InlinerBase *,
                           TR_VirtualGuardSelection *, TR_ResolvedMethod *,
                           TR_OpaqueClassBlock *,
                           TR_AllocationKind allocKind = stackAlloc,
                           float ratio = 1.0);
  bool addTarget0(TR_Memory *, TR_InlinerTracer *, TR_VirtualGuardSelection *,
                  TR_ResolvedMethod *, TR_OpaqueClassBlock *,
  ...
  TR_CallTarget *getRemovedTarget(int32_t i) {
    TR_ASSERT(i >= 0 && i < _myRemovedTargets.size(),
              "indexing a target out of range.");
  ...
  TR::Method *_interfaceMethod; // If we have an interface, we'll only have a
                                // TR::Method until we determine others
  TR_OpaqueClassBlock
      *_receiverClass; // for interface calls, we might know this?
  int32_t _vftSlot;    //
  int32_t _cpIndex;    //
  TR_ResolvedMethod *_initialCalleeMethod; // alot of times we might already
                                           // know the resolved method.
  TR::ResolvedMethodSymbol
      *_initialCalleeSymbol; // and in those cases, we should also know the
                             // methodsymbol..
  ...
  vcount_t
      _visitCount; // a woefully bad choice of name.  The number of times we
                   // have visited this callsite during inlining.  In theory,
                   // when you have multiple targets for a callsite, you will
                   // have multiple visits to a callsite.
  ...
  virtual bool findCallSiteTarget(TR_CallStack *callStack,
                                  TR_InlinerBase *inliner);
  virtual const char *name() { return "TR_CallSite"; }

  static TR_CallSite *
  create(TR::TreeTop *callNodeTreeTop, TR::Node *parent, TR::Node *callNode,
         TR_OpaqueClassBlock *receiverClass, TR::SymbolReference *symRef,
         TR_ResolvedMethod *resolvedMethod, TR::Compilation *comp,
         TR_Memory *trMemory, TR_AllocationKind kind,
         TR_ResolvedMethod *caller = NULL, int32_t depth = -1,
         bool allConsts = false);
...
public:
  TR_CALLSITE_TR_ALLOC_AND_INHERIT_EMPTY_CONSTRUCTOR(TR_DirectCallSite,
                                                     TR_CallSite)
  virtual bool findCallSiteTarget(TR_CallStack *callStack,
                                  TR_InlinerBase *inliner);
...
public:
  TR_CALLSITE_TR_ALLOC_AND_INHERIT_EMPTY_CONSTRUCTOR(TR_IndirectCallSite,
                                                     TR_CallSite)
  virtual bool findCallSiteTarget(TR_CallStack *callStack,
                                  TR_InlinerBase *inliner);
  virtual const char *name() { return "TR_IndirectCallSite"; }
  virtual TR_ResolvedMethod *
  findSingleJittedImplementer(TR_InlinerBase *inliner);
...
protected:
  ...
  bool
  tryToRefineReceiverClassBasedOnResolvedTypeArgInfo(TR_InlinerBase *inliner);
  ...
  bool
  addTargetIfMethodIsNotOverridenInReceiversHierarchy(TR_InlinerBase *inliner);
...
protected:
  TR_CALLSITE_TR_ALLOC_AND_INHERIT_EMPTY_CONSTRUCTOR(TR_FunctionPointerCallSite,
                                                     TR_IndirectCallSite)
};
