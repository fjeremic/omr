...
struct TR_CallTarget : public TR_Link<TR_CallTarget> {
  ...
  TR_CallTarget(TR_CallSite* callsite, TR::ResolvedMethodSymbol* calleeSymbol, TR_ResolvedMethod* calleeMethod,
    TR_VirtualGuardSelection* guard, TR_OpaqueClassBlock* receiverClass, TR_PrexArgInfo* ecsPrexArgInfo,
    float freqAdj = 1.0);
  ...
  TR::ResolvedMethodSymbol* getSymbol()
  {
    return _calleeSymbol;
  }
...
#define TR_CALLSITE_INHERIT_CONSTRUCTOR_COMMON(EXTENDED, BASE)                                                        \
  EXTENDED(TR_ResolvedMethod* callerResolvedMethod, TR::TreeTop* callNodeTreeTop, TR::Node* parent,                   \
    ...
    : BASE(callerResolvedMethod, callNodeTreeTop, parent, callNode, interfaceMethod, receiverClass, vftSlot, cpIndex, \
      initialCalleeMethod, initialCalleeSymbol, isIndirectCall, isInterface, bcInfo, comp, depth, allConsts)
...
struct TR_CallSite : public TR_Link<TR_CallSite> {
  ...
  int32_t numTargets()
  {
    return _mytargets.size();
  }
  int32_t numRemovedTargets()
  {
    return _myRemovedTargets.size();
  }
