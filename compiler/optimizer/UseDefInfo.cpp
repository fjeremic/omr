 ...
 * true.
 */
TR_UseDefInfo::TR_UseDefInfo(TR::Compilation* comp,
                             TR::CFG* cfg,
  ...
  , _useDefInfo(0,
                TR_UseDefInfo::BitVector(comp->allocator(allocatorName)),
                _region)
  ...
  , _useDefs(0, _region)
  , _numMemorySymbols(0)
  , _valueNumbersToMemorySymbolsMap(0,
                                    static_cast<MemorySymbolList*>(NULL),
                                    _region)
...
{
  if (doCompletion)
    prepareUseDefInfo(requiresGlobals,
                      prefersGlobals,
...
void
TR_UseDefInfo::prepareUseDefInfo(bool requiresGlobals,
                                 bool prefersGlobals,
                                 bool cannotOmitTrivialDefs,
                                 bool conversionRegsOnly)
{
  ...
  TR_UseDefInfo::AuxiliaryData aux(comp()->getSymRefCount(),
                                   comp()->getNodeCount(),
  ...
  for (TR::TreeTop* treeTop = comp()->getStartTree(); treeTop != NULL;
    ...
    if (!assignAdjustedNodeIndex(
          block, treeTop->getNode(), NULL, treeTop, aux)) {
      invalidateUseDefInfo();
      optimizer()->setCantBuildGlobalsUseDefInfo(true);
      optimizer()->setCantBuildLocalsUseDefInfo(true);
      return;
  ...
  if (trace()) {
    ...
    traceMsg(
      comp(), "Number of memory symbols       = %d\n", _numMemorySymbols);
    traceMsg(
      comp(), "Number of statics and fields   = %d\n", _numStaticsAndFields);
    ...
    traceMsg(
      comp(), "Total nodes for reaching defs  = %d\n", getExpandedTotalNodes());
    traceMsg(comp(),
             "   Number of defOnly nodes     = %d\n",
             _numExpandedDefOnlyNodes);
    traceMsg(
      comp(), "   Number of defUse nodes      = %d\n", _numExpandedDefUseNodes);
    traceMsg(comp(),
             "   Number of useOnly nodes     = %d\n",
  ...
  aux._expandedAtoms.resize(
    expandedNodeArraySize, std::make_pair<TR::Node*, TR::TreeTop*>(NULL, NULL));
...
void
TR_UseDefInfo::invalidateUseDefInfo()
{
  _isUseDefInfoValid = false;
  _useDefInfo.clear();
  _defUseInfo.clear();
...
bool
TR_UseDefInfo::performAnalysis(AuxiliaryData& aux)
{
...
bool
TR_UseDefInfo::_runReachingDefinitions(
  TR_ReachingDefinitions& reachingDefinitions,
  AuxiliaryData& aux)
{
...
void
TR_UseDefInfo::setVolatileSymbolsIndexAndRecurse(TR::BitVector& volatileSymbols,
                                                 int32_t symRefNum)
{
  ...
  if (volatileSymbols[symRef->getReferenceNumber()]) // already set, do not need
                                                     // to recurse.
    return;
...
void
TR_UseDefInfo::findAndPopulateVolatileSymbolsIndex(
  TR::BitVector& volatileSymbols)
{
  ...
  for (int32_t symRefNumber = comp()->getSymRefTab()->getIndexOfFirstSymRef();
       symRefNumber < comp()->getSymRefCount();
       symRefNumber++) {
...
void
TR_UseDefInfo::fillInDataStructures(AuxiliaryData& aux)
{
  TR::SymbolReferenceTable* symRefTab = comp()->getSymRefTab();
  ...
  TR::SparseBitVector expandedLIndexes(
    comp()
      ->allocator()); // expanded localIndex for variables defined by function
  for (TR::TreeTop* treeTop = comp()->getStartTree(); treeTop != NULL;
       treeTop = treeTop->getNextTreeTop()) {
    if (treeTop->getNode()->getOpCodeValue() == TR::BBStart) {
      block = treeTop->getNode()->getBlock();
...
bool
TR_UseDefInfo::isLoadAddrUse(TR::Node* node)
{
  return node->getOpCodeValue() == TR::loadaddr;
}

void
TR_UseDefInfo::findTrivialSymbolsToExclude(TR::Node* node,
                                           TR::TreeTop* treeTop,
                                           AuxiliaryData& aux)
{
  ...
  if (node->getOpCode().isStoreDirect()) {
    ...
    if (symRef->getSymbol()->isAutoOrParm()) {
      ...
      if (aux._neverWrittenSymbols.get(symRefNum)) {
        ...
        if (trace())
          traceMsg(
            comp(), "Resetting write bit %d at node %p\n", symRefNum, node);
        ...
        if (!aux._onceWrittenSymbolsIndices[symRefNum].IsNull()) {
          ...
          if (trace())
            traceMsg(comp(),
                     "Sym ref %d written once at node %p\n",
                     symRefNum,
                     treeTop->getNode());
        }
    ...
    if (symRef->getSymbol()->isAutoOrParm()) {
      if (aux._neverReadSymbols.get(symRefNum)) {
        ...
        if (trace())
          traceMsg(
            comp(), "Resetting read bit %d at node %p\n", symRefNum, node);
...
bool
TR_UseDefInfo::isTrivialUseDefNode(TR::Node* node, AuxiliaryData& aux)
{
...
bool
TR_UseDefInfo::isTrivialUseDefNodeImpl(TR::Node* node, AuxiliaryData& aux)
{
...
bool
TR_UseDefInfo::isTrivialUseDefSymRef(TR::SymbolReference* symRef,
                                     AuxiliaryData& aux)
{
  // if (!_hasLoadsAsDefs)
  //   return false;
...
bool
TR_UseDefInfo::isValidAutoOrParm(TR::SymbolReference* symRef)
{
  if (!symRef->getSymbol()->isAutoOrParm())
    return false;
...
bool
TR_UseDefInfo::excludedGlobals(TR::Symbol* sym)
{
  if (sym->isStatic() &&
...
void
TR_UseDefInfo::buildValueNumbersToMemorySymbolsMap()
{
...
void
TR_UseDefInfo::findMemorySymbols(TR::Node* node)
{
        ...
        node->getFirstChild()) // value number is shared with some other node
  {
    ...
    if (trace())
      traceMsg(
        comp(),
        "Node %p has memory symbol index %d (%d:%d:%d)\n",
        node,
...
int32_t
TR_UseDefInfo::getMemorySymbolIndex(TR::Node* node)
{
  if (!_useDefForMemorySymbols)
    return -1;
  ...
  TR_ASSERT(false,
            "Could not find memory symbol index for node %p (%d:%d:%d)",
            node,
            valueNumber,
            size,
            offset);
...
bool
TR_UseDefInfo::shouldIndexVolatileSym(TR::SymbolReference* ref,
                                      AuxiliaryData& aux)
{
 ...
 */
bool
TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData& aux)
{
  ...
  if (trace()) {
    traceMsg(comp(),
             "_neverReferencedSymbols[count = %d]: ",
  ...
  while (refIter.hasMoreElements()) {
    ...
    if (symRef &&
      ...
      if (sym && sym->getLocalIndex() == NULL_USEDEF_SYMBOL_INDEX) {
        ...
        if (trace()) {
          traceMsg(
            comp(), "Non Trivial symbol [%p] has index %d\n", sym, _numSymbols);
        }
        _sideTableToSymRefNumMap[_numSymbols] = symRefNumber;
        sym->setLocalIndex(_numSymbols++);
        _numNonTrivialSymbols++;
  ...
  while (refIter.hasMoreElements()) {
    ...
    if (symRef &&
      ...
      if (sym && sym->getLocalIndex() == NULL_USEDEF_SYMBOL_INDEX) {
        ...
        if (trace())
          traceMsg(
            comp(), "Trivial symbol [%p] has index %d\n", sym, _numSymbols);
        sym->setLocalIndex(_numSymbols++);
      }
    }
  }
  ...
  if (_numSymbols + numRegisters >= MAX_SYMBOLS) {
    dumpOptDetails(
      comp(), "   use/def failed, too many symbols (%d)\n", _numSymbols);
    return false;
  }
  ...
  for (treeTop = comp()->getStartTree(); treeTop;
    ...
    if (!findUseDefNodes(
          block, treeTop->getNode(), NULL, treeTop, aux, symRefToLocalIndexMap))
  ...
  if ((getNumDefNodes() - getNumIrrelevantStores()) * getNumUseNodes() >=
      maxDefByUseNodes) {
    dumpOptDetails(comp(),
                   "   use/def failed, too many def/use combinations (%d)\n",
                   (getNumDefNodes() - getNumIrrelevantStores()) *
                     getNumUseNodes());
    return false;
  }
  ...
  if (trace()) {
    traceMsg(
      comp(), "      expanded nodes          = %d\n", getExpandedTotalNodes());
    traceMsg(comp(),
             "      def/use combinations    = %d\n",
             (getNumDefNodes() - getNumIrrelevantStores()) * getNumUseNodes());
    traceMsg(
      comp(), "      irrelevant stores       = %d\n", getNumIrrelevantStores());
    traceMsg(
      comp(), "      expanded use nodes      = %d\n", getNumExpandedUseNodes());
    traceMsg(
      comp(), "      expanded def nodes      = %d\n", getNumExpandedDefNodes());
    traceMsg(
      comp(), "      local index level  = %d\n", _numExpandedDefUseNodes);
...
bool
TR_UseDefInfo::skipAnalyzingForCompileTime(TR::Node* node,
                                           TR::Block* block,
                                           TR::Compilation* comp,
                                           AuxiliaryData& aux)
{
  ...
  bool considerImplicitStores)
{
  ...
  if (useDefIndex > SIDE_TABLE_LIMIT) {
    dumpOptDetails(
      comp(), "   use/def failed, use/def index overflow (%d)\n", useDefIndex);
    return false;
  }
...
bool
TR_UseDefInfo::assignAdjustedNodeIndex(TR::Block* block,
                                       TR::Node* node,
                                       TR::Node* parent,
                                       TR::TreeTop* treeTop,
                                       AuxiliaryData& aux,
                                       bool considerImplicitStores)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    bool shouldConsiderLoadAddrChildrenImplicitStores =
      parentCouldHaveImplicitStoreChildren &&
      childIndexIndicatesImplicitStore(node, i);
    if (!assignAdjustedNodeIndex(block,
                                 node->getChild(i),
                                 node,
                                 treeTop,
                                 aux,
  ...
  if (nodeIndex + adjustment > SIDE_TABLE_LIMIT) {
    dumpOptDetails(comp(),
                   "   use/def failed, use/def index overflow (%d)\n",
  ...
  if (node->getLocalIndex() + expandedAdjustment > MAX_SCOUNT) {
    dumpOptDetails(comp(),
                   "   use/def failed, local index overflow (%d)\n",
...
bool
TR_UseDefInfo::childIndexIndicatesImplicitStore(TR::Node* node,
                                                int32_t childIndex)
{
  return false;
}
...
void
TR_UseDefInfo::insertData(TR::Block* block,
                          TR::Node* node,
                          TR::Node* parent,
                          TR::TreeTop* treeTop,
                          AuxiliaryData& aux,
  ...
  if (trace()) {
    if (!definesMultipleSymbols)
      ...
      traceMsg(
        ...
        node->getUseDefIndex(),
        node->getLocalIndex(),
        node->getLocalIndex() + num_aliases - 1,
        definesMultipleSymbols,
        isTrivialUseDefNode(node, aux),
        adjustArray);
  }
...

  {
    ...
    for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
      ...
      if (trace())
        traceMsg(
          comp(),
          "    symbol (u/d index=%d) is defined by node with localIndex %d \n",
          j,
          k);
    }
...
bool
isSymRefFromInlinedMethod(TR::Compilation* comp,
...
void
TR_UseDefInfo::processReachingDefinition(void* vblockInfo, AuxiliaryData& aux)
{
  buildUseDefs(vblockInfo, aux);
}

void
TR_UseDefInfo::buildUseDefs(void* vblockInfo, AuxiliaryData& aux)
{
  ...
  for (int32_t j = getFirstUseIndex(); j <= lastUseIndex; j++) {
    ...
    if (info.IsZero() && j != lastUseIndex) {
      dumpOptDetails(
        comp(), "   Use #%d[%p] has no definitions\n", j, getNode(j));
...
void
TR_UseDefInfo::dereferenceDefs(int32_t useIndex,
                               TR_UseDefInfo::BitVector& nodesLookedAt,
                               TR_UseDefInfo::BitVector& loadDefs)
{
  if (trace()) {
    traceMsg(comp(),
             "Dereferencing defs for use index %d : ",
  ...
  if (trace()) {
    traceMsg(
      comp(), "New defs for use index %d : ", useIndex + getFirstUseIndex());
    (*comp()) << _useDefInfo[useIndex];
    traceMsg(comp(), "\n");
  }
}
...
//
int32_t
TR_UseDefInfo::setSingleDefiningLoad(int32_t useIndex,
                                     TR_UseDefInfo::BitVector& nodesLookedAt,
                                     TR_UseDefInfo::BitVector& loadDefs)
{
  TR_UseDefInfo::BitVector& useDefInfo = _useDefInfo[useIndex];
  ...
  if (trace()) {
    traceMsg(comp(),
             "   Checking use index %d for single defining load : ",
             useIndex + getFirstUseIndex());
    (*comp()) << useDefInfo;
    traceMsg(comp(), "\n");
  }
  ...
  if (0 && !cursor.Valid()) {
    if (nodesLookedAt.ValueAt(defIndex - getFirstUseIndex())) {
      if (trace())
        traceMsg(comp(),
                 "      Use index %d has circular defining loads\n",
                 useIndex + getFirstUseIndex());
      return -1;
    }
    if (trace())
      traceMsg(comp(),
               "      Use index %d has single defining load %d\n",
               useIndex + getFirstUseIndex(),
               defIndex);
  ...
  for (; cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (!nodesLookedAt.ValueAt(defIndex)) {
      ...
      if (singleDef == -2) {
        ...
        if (trace())
          traceMsg(comp(),
                   "      Use index %d has defining load %d\n",
                   useIndex + getFirstUseIndex(),
                   defIndex + getFirstUseIndex());
 ...
 */
void
TR_UseDefInfo::dereferenceDef(TR_UseDefInfo::BitVector& useDefInfo,
                              int32_t defIndex,
                              TR_UseDefInfo::BitVector& nodesLookedAt)
{
...
// PLX only
bool
TR_UseDefInfo::isChildUse(TR::Node* node, int32_t childIndex)
{
...
void
TR_UseDefInfo::buildUseDefs(TR::Node* node,
                            void* vanalysisInfo,
                            ...
                            AuxiliaryData& aux)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    buildUseDefs(
      node->getChild(i), analysisInfo, nodesToBeDereferenced, node, aux);
  }
  ...
  if (node->getOpCode().hasSymbolReference() &&
    ...
    if (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))) {
      ...
      if (trace())
        traceMsg(comp(),
                 "For node %p index = %d and first use index = %d\n",
                 node,
                 nodeIndex,
                 getFirstUseIndex());
      if (aux._neverWrittenSymbols.get(
        ...
        if (trace())
          traceMsg(comp(), "Reached here (entry) for use node %p\n", node);
                    ...
                    ._onceReadSymbolsIndices[node->getSymbolReference()
                                               ->getReferenceNumber()]
                    .IsNull()) {
        if (trace())
        ...
        if (aux._onceReadSymbols[node->getSymbolReference()
          ...
          _useDefInfo[realIndex] |=
            CS2_TR_BitVector(*(aux._onceReadSymbols[node->getSymbolReference()
                                                      ->getReferenceNumber()]));
        defsKnownTrivially = true;

      } else if (!aux
                    ._onceWrittenSymbolsIndices[node->getSymbolReference()
                                                  ->getReferenceNumber()]
                    .IsNull()) {
        if (trace())
          traceMsg(
            comp(), "Use node %p is of a symbol write only once\n", node);
  ...
  if (isExpandedUseIndex(expandedNodeIndex) ||
    ...
    TR_ASSERT(symIndex != NULL_USEDEF_SYMBOL_INDEX,
              "Use node %p should have an indexed symbol",
              node);
    int32_t numDefs = 0;
      ...
      *defs = *(aux._defsForSymbol[symIndex]);
      ...
      while (cursor.hasMoreElements()) {
        ...
        if (i >= _numDefsOnEntry) {
          ...
          if (trace())
            traceMsg(
              comp(), "reached by expanded index %d [0x%p]\n", i, defNode);
...
TR::Node*
TR_UseDefInfo::getSingleDefiningLoad(TR::Node* node)
{
...
bool
TR_UseDefInfo::getDefiningLoads(BitVector& definingLoads, TR::Node* node)
{
  definingLoads.Or(getDefiningLoads_ref(node));
  return !definingLoads.IsZero();
}

const TR_UseDefInfo::BitVector&
TR_UseDefInfo::getDefiningLoads_ref(TR::Node* node)
{
  TR_UseDefInfo::BitVector& info =
    _useDefInfo[node->getUseDefIndex() - getFirstUseIndex()];
  if (!info.IsZero()) {
...
bool
TR_UseDefInfo::getUseDefIsZero(int32_t useIndex)
{
...
bool
TR_UseDefInfo::getUseDef(BitVector& useDef, int32_t useIndex)
{
...
bool
TR_UseDefInfo::getUseDef_noExpansion(BitVector& useDef, int32_t useIndex)
{
...
const TR_UseDefInfo::BitVector&
TR_UseDefInfo::getUseDef_ref(int32_t useIndex, TR_UseDefInfo::BitVector* defs)
{
...
const TR_UseDefInfo::BitVector&
TR_UseDefInfo::getUseDef_ref_body(int32_t useIndex,
                                  TR_BitVector* visitedDefs,
...
bool
TR_UseDefInfo::getUsesFromDefIsZero(int32_t defIndex, bool loadAsDef)
{
...
bool
TR_UseDefInfo::getUsesFromDef(BitVector& usesFromDef,
                              int32_t defIndex,
                              bool loadAsDef)
{
...
const TR_UseDefInfo::BitVector&
TR_UseDefInfo::getUsesFromDef_ref(int32_t defIndex, bool loadAsDef)
{
...
void
TR_UseDefInfo::setUseDef(int32_t useIndex, int32_t defIndex)
{
...
void
TR_UseDefInfo::resetUseDef(int32_t useIndex, int32_t defIndex)
{
  int32_t realIndex = useIndex - getFirstUseIndex();
...
void
TR_UseDefInfo::clearUseDef(int32_t useIndex)
{
  int32_t realIndex = useIndex - getFirstUseIndex();
...
TR::Node*
TR_UseDefInfo::getNode(int32_t index)
{
...
TR::TreeTop*
TR_UseDefInfo::getTreeTop(int32_t index)
{
  TR_UseDef* ud = &_useDefs[index];
  if (!ud->isDef()) {
...
void
TR_UseDefInfo::buildDefUseInfo(bool loadAsDef)
{
...
bool
TR_UseDefInfo::canComputeReachingDefs()
{
...
int32_t
TR_UseDefInfo::getSymRefIndexFromUseDefIndex(int32_t udIndex)
{
