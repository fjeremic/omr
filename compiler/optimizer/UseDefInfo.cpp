...
TR_UseDefInfo::TR_UseDefInfo(TR::Compilation *comp, TR::CFG *cfg,
    ...
    : _region(comp->trMemory()->heapMemoryRegion()), _compilation(comp),
      ...
      _useDefInfo(0, TR_UseDefInfo::BitVector(comp->allocator(allocatorName)),
                  _region),
      _isUseDefInfoValid(false), _infoCache(_region),
      ...
      _tempsOnly(false), _trace(comp->getOption(TR_TraceUseDefs)),
      _hasLoadsAsDefs(loadsShouldBeDefs), _hasCallsAsUses(callsShouldBeUses),
      _useDefs(0, _region), _numMemorySymbols(0),
      _valueNumbersToMemorySymbolsMap(0, static_cast<MemorySymbolList *>(NULL),
                                      _region),
      _sideTableToSymRefNumMap(comp->getSymRefCount(), _region), _cfg(cfg),
...
void TR_UseDefInfo::prepareUseDefInfo(bool requiresGlobals, bool prefersGlobals,
  ...
  if (requiresGlobals || prefersGlobals) {
    ...
    if (requiresGlobals && !canBuild) {
      ...
      if (trace())
        traceMsg(comp(), "Use/Def info: cannot build global use/def info as it "
  ...
  if (!canBuild) {
    ...
    if (trace())
      traceMsg(comp(), "Use/Def info: cannot build global nor local use/def "
                       "info as it failed in indexSymbolsAndNodes\n");
    return;
  }
...
bool TR_UseDefInfo::_runReachingDefinitions(
  ...
  if (!succeeded) {
    invalidateUseDefInfo();
    if (trace())
      traceMsg(comp(), "Method too complex to perform reaching defs, use/def "
                       "info not built\n");
  } else {
...
const TR_UseDefInfo::BitVector &
TR_UseDefInfo::getUseDef_ref_body(int32_t useIndex, TR_BitVector *visitedDefs,
