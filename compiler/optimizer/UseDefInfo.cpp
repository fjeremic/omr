  ...
  bool requiresGlobals, bool prefersGlobals, bool cannotOmitTrivialDefs, bool conversionRegsOnly)
{
  ...
  if (!canBuild) {
    ...
    if (trace())
      traceMsg(
        comp(), "Use/Def info: cannot build global nor local use/def info as it failed in indexSymbolsAndNodes\n");
    return;
  }
...
void TR_UseDefInfo::findAndPopulateVolatileSymbolsIndex(TR::BitVector& volatileSymbols)
{
  ...
  for (int32_t symRefNumber = comp()->getSymRefTab()->getIndexOfFirstSymRef(); symRefNumber < comp()->getSymRefCount();
       symRefNumber++) {
        ...
        if (!shouldIndexVolatileSym(symRef, aux)) {
          if (trace())
            traceMsg(comp(), "Ignoring Symbol [%p] because it is volatile %d or aliased to a volatile %d\n", sym,
              sym->isVolatile(), aux._volatileOrAliasedToVolatileSymbols.get(symRefNumber));
          continue;
        }
  ...
  AuxiliaryData& aux, TR::deque<uint32_t, TR::Region&>& symRefToLocalIndexMap, bool considerImplicitStores)
{
  ...
  if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
    ...
    localIndex = _numExpandedDefOnlyNodes;
    _numExpandedDefOnlyNodes
      += TR::NumTypes + 1; // == arraylet shadows + read barrier....encode this where?  SymRefTab probably
    useDefIndex = _numDefOnlyNodes++;
  }
  // else
  //   {
  ...
  AuxiliaryData& aux, TR::SparseBitVector& expandedLIndexes, bool considerImplicitStores)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    bool shouldConsiderLoadAddrChildrenImplicitStores
      = parentCouldHaveImplicitStoreChildren && childIndexIndicatesImplicitStore(node, i);
    insertData(
      block, node->getChild(i), node, treeTop, aux, expandedLIndexes, shouldConsiderLoadAddrChildrenImplicitStores);
  }
  ...
  if (trace()) {
    if (!definesMultipleSymbols)
      traceMsg(comp(),
        "Node : %p   opCode = %s useDefIndex = %d localIndex = %d definesMultipleSymbols=%d isTrivialUseDefNode=%d "
        "adjustArray=%d \n",
      ...
      traceMsg(comp(),
        "Node : %p   opCode = %s useDefIndex = %d localIndex = %d-%d definesMultipleSymbols=%d isTrivialUseDefNode=%d "
        "adjustArray=%d \n",
        node, opCode.getName(), node->getUseDefIndex(), node->getLocalIndex(), node->getLocalIndex() + num_aliases - 1,
...
void TR_UseDefInfo::buildUseDefs(void* vblockInfo, AuxiliaryData& aux)
{
  ...
  if (_indexStatics && blockInfo) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      //         traceMsg(comp(), "UDI: setting _useDefInfo[%d}[i=%d] to true getLastUseIndex = %d getFirstUseIndex = %d
      //         numberOfNodes =
  ...
  TR::Node* node, void* vanalysisInfo, TR_BitVector& nodesToBeDereferenced, TR::Node* parent, AuxiliaryData& aux)
{
    ...
    if (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))) {
      ...
      if (aux._neverWrittenSymbols.get(node->getSymbolReference()->getReferenceNumber())) {
        ...
        if (aux._onceWrittenSymbols[node->getSymbolReference()->getReferenceNumber()])
          _useDefInfo[realIndex]
            |= CS2_TR_BitVector(*(aux._onceWrittenSymbols[node->getSymbolReference()->getReferenceNumber()]));
        defsKnownTrivially = true;
      } else {
  ...
  if (isExpandedUseIndex(expandedNodeIndex)
    || ((expandedNodeIndex == 0)
      && (!node->getOpCode().isStore() || !node->getSymbol()->isAutoOrParm() || !node->storedValueIsIrrelevant()))) {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (firstDef < 0) {
        ...
        if ((firstDef < getFirstUseIndex())
          ...
          //               traceMsg(comp(), "UDI, special case firstDef = %d getFirstUseIndex() = %d getNode(useIndex) =
