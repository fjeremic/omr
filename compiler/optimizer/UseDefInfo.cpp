...
void TR_UseDefInfo::prepareUseDefInfo(bool requiresGlobals, bool prefersGlobals, bool cannotOmitTrivialDefs, bool conversionRegsOnly)
{
    ...
    if (_hasLoadsAsDefs && !cannotOmitTrivialDefs && (comp()->getMethodHotness() < hot)) {
...
void TR_UseDefInfo::findTrivialSymbolsToExclude(TR::Node* node, TR::TreeTop* treeTop, AuxiliaryData& aux)
{
    ...
    if (node->getOpCode().hasSymbolReference() && aux._neverReferencedSymbols.get(node->getSymbolReference()->getReferenceNumber())) {
    ...
    if (node->getOpCode().isStoreDirect()) {
        ...
        if (symRef->getSymbol()->isAutoOrParm()) {
            if (aux._neverReadSymbols.get(symRefNum)) {
                ...
                if ((prevLoadNode->getByteCodeIndex() != node->getByteCodeIndex()) || (prevLoadNode->getInlinedSiteIndex() != node->getInlinedSiteIndex())) {
...
bool TR_UseDefInfo::isTrivialUseDefNodeImpl(TR::Node* node, AuxiliaryData& aux)
{
    ...
    if (_hasLoadsAsDefs && symRef->getSymbol()->isAutoOrParm()) {
        if (!aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull() && ((node->getOpCode().isLoadVarDirect() || isLoadAddrUse(node)) || (node->getOpCode().isStoreDirect() && aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].ValueAt(node->getGlobalIndex()))))
            return true;
    } else if (symRef->getSymbol()->isAutoOrParm() && (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))))
        return true;

    if (/* _hasLoadsAsDefs && */ symRef->getSymbol()->isAutoOrParm()) {
        if (!aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].IsNull() && ((node->getOpCode().isLoadVarDirect() || isLoadAddrUse(node)) || (node->getOpCode().isStoreDirect() && aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].ValueAt(node->getGlobalIndex()))))
            return true;
    }
...
void TR_UseDefInfo::findMemorySymbols(TR::Node* node)
{
    ...
    if ((node->getOpCode().isLoadIndirect() || node->getOpCode().isStoreIndirect()) && node->getSymbolReference()->getSymbol()->isShadow() && _valueNumberInfo && _valueNumberInfo->getNext(node->getFirstChild()) != node->getFirstChild()) // value number is shared with some other node
    {
...
int32_t TR_UseDefInfo::getMemorySymbolIndex(TR::Node* node)
{
    ...
    if ((!node->getOpCode().isLoadIndirect() && !node->getOpCode().isStoreIndirect()) || !node->getSymbolReference()->getSymbol()->isShadow() || _valueNumberInfo->getNext(node->getFirstChild()) == node->getFirstChild())
        return -1;
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData& aux)
{
    ...
    while (refIter.hasMoreElements()) {
        ...
        if (symRef) {
            ...
            if (sym) {
                ...
                if (!isTrivialUseDefSymRef(symRef, aux) && ((sym->isParm() && !aux._neverWrittenSymbols.get(symRef->getReferenceNumber())) || (aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull() && aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].IsNull())))
    ...
    while (refIter.hasMoreElements()) {
        ...
        if (symRef && (!isTrivialUseDefSymRef(symRef, aux) && ((symRef->getSymbol()->isParm() && !aux._neverWrittenSymbols.get(symRef->getReferenceNumber())) || (aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull() && aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].IsNull())))) {
            sym = symRef->getSymbol();
            if (trace()) {
    ...
    int32_t maxDefByUseNodes = ((comp()->getMethodHotness() >= hot)) ? MAX_DEF_BY_USE_NODES_FOR_HIGH_OPT : MAX_DEF_BY_USE_NODES_FOR_LOW_OPT;
    ...
    bool considerImplicitStores)
{
    ...
    if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
        if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
            ...
            //   node->setUseDefIndex(0);
            //   }
        } else if (opCode.isStore() || (_useDefForRegs && opCode.isStoreReg()) || (considerImplicitStores && node->getOpCodeValue() == TR::loadaddr)) {
            if (!isTrivialUseDefNode(node, aux)) {
                localIndex = _numExpandedDefOnlyNodes;
                _numExpandedDefOnlyNodes += num_aliases;
            }
...
void TR_UseDefInfo::insertData(TR::Block* block, TR::Node* node, TR::Node* parent, TR::TreeTop* treeTop, AuxiliaryData& aux, TR::SparseBitVector& expandedLIndexes, bool considerImplicitStores)
{
...

    {
        ...
        for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
            ...
            // RTC_50153
            if (symRef->getSymbol() != aliasedSymRef->getSymbol() && (symRef->getSymbol()->isAutoOrParm() || symRef->getSymbol()->isStatic()) && (aliasedSymRef->getSymbol()->isAutoOrParm() || aliasedSymRef->getSymbol()->isStatic()))
                continue;
...
void TR_UseDefInfo::dereferenceDef(TR_UseDefInfo::BitVector& useDefInfo, int32_t defIndex, TR_UseDefInfo::BitVector& nodesLookedAt)
{
    ...
    while (!defIndices.empty()) {
        ...
        if (getNode(defIndex)->getSymbolReference()->getSymbol()->isMethod() || getNode(defIndex)->getOpCode().isCall()) {
...
void TR_UseDefInfo::buildUseDefs(TR::Node* node, void* vanalysisInfo, TR_BitVector& nodesToBeDereferenced, TR::Node* parent, AuxiliaryData& aux)
{
    ...
    //
    if (isExpandedUseIndex(expandedNodeIndex) || ((expandedNodeIndex == 0) && (!node->getOpCode().isStore() || !node->getSymbol()->isAutoOrParm() || !node->storedValueIsIrrelevant()))) {
         ...
         if (trace())
            ...
            while (cursor.hasMoreElements()) {
                ...
                if (i >= _numDefsOnEntry) {
                    ...
                    if (memSymIndex == -1 || getMemorySymbolIndex(defNode) != memSymIndex || i == defNode->getLocalIndex()) {
    ...
    if (symIndex == NULL_USEDEF_SYMBOL_INDEX || node->getOpCode().isCall() || node->getOpCode().isFence() || (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1)) {
...
TR::Node* TR_UseDefInfo::getSingleDefiningLoad(TR::Node* node)
{
    ...
    if (info.PopulationCount() == 1) {
        ...
        if ((firstDef >= getFirstUseIndex()) && (n = getNode(firstDef)) && (n->getUseDefIndex() > 0) && (n->getOpCode().isLoadVar()) && (n->getOpCode().hasSymbolReference() && !n->getSymbol()->isVolatile()))
            return n;
    }
    return NULL;
}
    ...
    if (!info.IsZero()) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (firstDef < 0) {
                ...
                if ((firstDef < getFirstUseIndex()) || (getNode(useIndex) && getNode(useIndex)->getSymbolReference() && getNode(useIndex)->getSymbolReference()->getSymbol()->isMethod())) {
