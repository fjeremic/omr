...
#define MAX_DEF_BY_USE_NODES_FOR_HIGH_OPT \
  (INT_MAX / 100 - 1)  // will be multiplied by 100 for big methods
...
/**
 * Constructs TR_UseDefInfo instance. Note that this should not be called
 * directly. Instead construction is handled by
 * OMR::Optimizer::createUseDefInfo() method.
 *
 ...
 * @param doCompletion
 * @param callsShouldBeUses          Enables inclusion of calls as uses so that
 * the alias analysis can detect when local (stack) variable has been aliased by
 * a function call. A value of false is fine for Java like languages where local
 * (stack) variables cannot be passed by reference to function calls and hence
 * cannot be aliased. However for C like languages this flag should be set to
 * true.
 */
TR_UseDefInfo::TR_UseDefInfo(TR::Compilation *comp, TR::CFG *cfg,
                             TR::Optimizer *optimizer, bool requiresGlobals,
    ...
    : _region(comp->trMemory()->heapMemoryRegion()),
      ...
      _useDefInfo(0, TR_UseDefInfo::BitVector(comp->allocator(allocatorName)),
                  _region),
      _isUseDefInfoValid(false),
      _infoCache(_region),
      _EMPTY(comp->allocator(allocatorName)),
      _useDerefDefInfo(0, static_cast<const BitVector *>(NULL), _region),
      _defUseInfo(0, TR_UseDefInfo::BitVector(comp->allocator(allocatorName)),
                  _region),
      _loadDefUseInfo(
          0, TR_UseDefInfo::BitVector(comp->allocator(allocatorName)), _region),
      _tempsOnly(false),
      ...
      _useDefs(0, _region),
      _numMemorySymbols(0),
      _valueNumbersToMemorySymbolsMap(0, static_cast<MemorySymbolList *>(NULL),
                                      _region),
      _sideTableToSymRefNumMap(comp->getSymRefCount(), _region),
      _cfg(cfg),
      _valueNumberInfo(NULL) {
  if (doCompletion)
    prepareUseDefInfo(requiresGlobals, prefersGlobals, cannotOmitTrivialDefs,
                      conversionRegsOnly);
}

void TR_UseDefInfo::prepareUseDefInfo(bool requiresGlobals, bool prefersGlobals,
                                      bool cannotOmitTrivialDefs,
                                      bool conversionRegsOnly) {
  LexicalTimer tlex("useDefInfo", comp()->phaseTimer());

  TR_UseDefInfo::AuxiliaryData aux(
      comp()->getSymRefCount(), comp()->getNodeCount(),
    ...
    for (int32_t j = 0; j < numSymRefs; j++) {
      if (symRefTab->getSymRef(j) &&
        ...
        aux._onceReadSymbolsIndices[j].GrowTo(
            comp()->getNodeCount()); /* no effect for sparse bit vectors, except
                                        to make non-null */
      }
    }
  }
  ...
  if (_useDefForMemorySymbols) {
    // TODO: This should be a member of TR_UseDefInfo::AuxiliaryData, and
    // discarded when aux is discarded but to do this TR_ValueNumberInfo should
    // be converted to not be dynamically allocated Also clean up deletes for
    // dynamic allocation
  ...
  if (requiresGlobals || prefersGlobals) {
    ...
    if (requiresGlobals && !canBuild) {
      ...
      if (trace())
        traceMsg(comp(),
                 "Use/Def info: cannot build global use/def info as it failed "
                 "in indexSymbolsAndNodes\n");
      return;
    }
  }
  ...
  if (!canBuild) {
    ...
    if (trace())
      traceMsg(comp(),
               "Use/Def info: cannot build global nor local use/def info as it "
               "failed in indexSymbolsAndNodes\n");
    return;
  }
  ...
  for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop != NULL;
       treeTop = treeTop->getNextTreeTop()) {
    if (treeTop->getNode()->getOpCodeValue() == TR::BBStart) {
      block = treeTop->getNode()->getBlock();
    }
    if (!assignAdjustedNodeIndex(block, treeTop->getNode(), NULL, treeTop,
                                 aux)) {
      invalidateUseDefInfo();
      optimizer()->setCantBuildGlobalsUseDefInfo(true);
      optimizer()->setCantBuildLocalsUseDefInfo(true);
  ...
  _useDefInfo.resize(getNumUseNodes(), TR_UseDefInfo::BitVector(
                                           comp()->allocator(allocatorName)));
  //   for (i = getNumUseNodes()-1; i >= 0; --i)
  //      _useDefInfo[i].GrowTo(getNumDefNodes());
  _isUseDefInfoValid = true;

  int numRegisters =
      _useDefForRegs ? comp()->cg()->getNumberOfGlobalRegisters() : 0;
  ...
  aux._expandedAtoms.resize(
      expandedNodeArraySize,
      std::make_pair<TR::Node *, TR::TreeTop *>(NULL, NULL));

  fillInDataStructures(aux);

  // Flatten the info.. keep the node around for uses - and the treetop around
  // for defs
  //
  ...
  if (trace()) traceMsg(comp(), "completed initialization of use/def info\n\n");
...
bool TR_UseDefInfo::performAnalysis(AuxiliaryData &aux) {
  if (!_isUseDefInfoValid)
    // use-def info hasn't been successfully generated so performing reaching
    // definition analysis is not possible
    return false;
  if (trace())
...
bool TR_UseDefInfo::_runReachingDefinitions(
    TR_ReachingDefinitions &reachingDefinitions, AuxiliaryData &aux) {
  // stack memory is prohibited in UseDefInfo, but here we have to use it as
  // reachingDefinitions use stack memory for _blockAnalysisInfo
  ...
  if (!succeeded) {
    ...
    if (trace())
      traceMsg(comp(),
               "Method too complex to perform reaching defs, use/def info not "
               "built\n");
  } else {
    // Build use/def information from the bit vector information from reaching
    // defs
    //
...
void TR_UseDefInfo::setVolatileSymbolsIndexAndRecurse(
    TR::BitVector &volatileSymbols, int32_t symRefNum) {
  TR::SymbolReference *symRef = comp()->getSymRefTab()->getSymRef(symRefNum);

  if (!symRef || !symRef->getSymbol()) return;

  if (volatileSymbols[symRef->getReferenceNumber()])  // already set, do not
                                                      // need to recurse.
    return;

  //  traceMsg(comp(), "JIAG, setting volatile Symbols for symref
  //  %d.\n",symRefNum);
  ...
  for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
    ...
    TR::SymbolReference *aliasedSymRef =
        comp()->getSymRefTab()->getSymRef(aliasesCursor);

    if (!aliasedSymRef || !aliasedSymRef->getSymbol()) continue;

    setVolatileSymbolsIndexAndRecurse(volatileSymbols,
                                      aliasedSymRef->getReferenceNumber());
  }
}

void TR_UseDefInfo::findAndPopulateVolatileSymbolsIndex(
    TR::BitVector &volatileSymbols) {
  //   traceMsg(comp(), "In findAndPopulateVolatileSymbolsIndex\n");
  for (int32_t symRefNumber = comp()->getSymRefTab()->getIndexOfFirstSymRef();
       symRefNumber < comp()->getSymRefCount(); symRefNumber++) {
    //     traceMsg(comp(), "Considering symRef %d: ",symRefNumber);
    TR::SymbolReference *symRef =
        comp()->getSymRefTab()->getSymRef(symRefNumber);

    if (!symRef || !symRef->getSymbol()) continue;
...
void TR_UseDefInfo::fillInDataStructures(AuxiliaryData &aux) {
  ...
  comp()->incVisitCount();
  TR::Block *block = NULL;
  TR::SparseBitVector expandedLIndexes(
      comp()->allocator());  // expanded localIndex for variables defined by
                             // function
  for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop != NULL;
       treeTop = treeTop->getNextTreeTop()) {
    if (treeTop->getNode()->getOpCodeValue() == TR::BBStart) {
  ...
  // If function foo defined variable a, then the expanded localIndex of a
  // related to foo must be in defsForSymbol of all functions aliased to a
  ...
  if (_uniqueIndexForDefsOnEntry) {
    for (int32_t i = getFirstDefIndex(); i < _numDefsOnEntry; i++) {
      TR::SymbolReference *symRef =
          symRefTab->getSymRef(_sideTableToSymRefNumMap[i]);
      if (!symRef) continue;
      TR::SparseBitVector aliases(comp()->allocator());
      symRef->getUseDefAliases().getAliases(aliases);
      ...
      for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
        ...
        if (!aliasedSym || !aliasedSym->isMethod()) continue;
        if (aux._neverReferencedSymbols.get(
                aliasedSymRef->getReferenceNumber()))
          continue;
        if (excludedGlobals(aliasedSymRef->getSymbol())) continue;
        ...
        // traceMsg(comp(),"\n _numDefsOnEntry=%d getNumExpandedDefNodes()=%d
        // j=(u%d s#%d) i=(u%d s#%d)",_numDefsOnEntry, getNumExpandedDefNodes(),
        // j, aliasedSymRef->getReferenceNumber(),i,
...
void TR_UseDefInfo::findTrivialSymbolsToExclude(TR::Node *node,
                                                TR::TreeTop *treeTop,
                                                AuxiliaryData &aux) {
  ...
  if (node->getOpCode().hasSymbolReference() &&
      aux._neverReferencedSymbols.get(
          node->getSymbolReference()->getReferenceNumber())) {
    aux._neverReferencedSymbols.reset(
        node->getSymbolReference()->getReferenceNumber());
  }
      ...
      if (aux._neverWrittenSymbols.get(symRefNum)) {
        ...
        if (!aux._onceWrittenSymbolsIndices[symRefNum].IsNull()) {
          if (symRef->getSymbol()->isParm())
            ...
            aux._onceWrittenSymbolsIndices[symRefNum][node->getGlobalIndex()] =
                true;
          if (trace())
    ...
    if (symRef->getSymbol()->isAutoOrParm()) {
      if (aux._neverReadSymbols.get(symRefNum)) {
        ...
        if ((prevLoadNode->getByteCodeIndex() != node->getByteCodeIndex()) ||
            (prevLoadNode->getInlinedSiteIndex() !=
             node->getInlinedSiteIndex())) {
...
bool TR_UseDefInfo::isTrivialUseDefNode(TR::Node *node, AuxiliaryData &aux) {
  ...
  if (result) aux._isTrivialNode.set(node->getGlobalIndex());

  return result;
}

bool TR_UseDefInfo::isTrivialUseDefNodeImpl(TR::Node *node,
                                            AuxiliaryData &aux) {
  ...
  if (isTrivialUseDefSymRef(symRef, aux)) return true;
    ...
    if (!aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull() &&
        ((node->getOpCode().isLoadVarDirect() || isLoadAddrUse(node)) ||
         (node->getOpCode().isStoreDirect() &&
          aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].ValueAt(
              node->getGlobalIndex()))))
      return true;
  ...
  if (/* _hasLoadsAsDefs && */ symRef->getSymbol()->isAutoOrParm()) {
    if (!aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()]
             .IsNull() &&
        ((node->getOpCode().isLoadVarDirect() || isLoadAddrUse(node)) ||
         (node->getOpCode().isStoreDirect() &&
          aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].ValueAt(
              node->getGlobalIndex()))))
      return true;
  }
...
bool TR_UseDefInfo::isTrivialUseDefSymRef(TR::SymbolReference *symRef,
                                          AuxiliaryData &aux) {
  // if (!_hasLoadsAsDefs)
  //   return false;
...
bool TR_UseDefInfo::isValidAutoOrParm(TR::SymbolReference *symRef) {
  if (!symRef->getSymbol()->isAutoOrParm()) return false;

  if (!_tempsOnly) return true;
...
bool TR_UseDefInfo::excludedGlobals(TR::Symbol *sym) {
  if (sym->isStatic() &&
      (sym->isConst() || sym->castToStaticSymbol()->isConstObjectRef()))
    return true;
...
void TR_UseDefInfo::buildValueNumbersToMemorySymbolsMap() {
  ...
  _valueNumbersToMemorySymbolsMap.resize(_valueNumberInfo->getNumberOfValues(),
                                         NULL);
  for (size_t i = 0; i < _valueNumbersToMemorySymbolsMap.size(); ++i)
    _valueNumbersToMemorySymbolsMap[i] =
        new (_region) MemorySymbolList(_region);
...
void TR_UseDefInfo::findMemorySymbols(TR::Node *node) {
          ...
          node->getFirstChild())  // value number is shared with some other node
  {
    ...
    if (!found)
      _valueNumbersToMemorySymbolsMap[valueNumber]->push_front(
          MemorySymbol(size, offset, _numMemorySymbols++));

    if (trace())
      traceMsg(
          comp(), "Node %p has memory symbol index %d (%d:%d:%d)\n", node,
          _valueNumbersToMemorySymbolsMap[valueNumber]->front()._localIndex,
          valueNumber, size, offset);
  }
}
...
int32_t TR_UseDefInfo::getMemorySymbolIndex(TR::Node *node) {
  ...
  TR_ASSERT(false, "Could not find memory symbol index for node %p (%d:%d:%d)",
            node, valueNumber, size, offset);
  return -1;
}

bool TR_UseDefInfo::shouldIndexVolatileSym(TR::SymbolReference *ref,
                                           AuxiliaryData &aux) {
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData &aux) {
  ...
  if (trace()) {
    traceMsg(
        comp(),
        "Trying to index nodes with _indexFields = %d, _indexStatics = %d\n",
  ...
  if (trace()) {
    traceMsg(comp(), "_neverReferencedSymbols[count = %d]: ",
             aux._neverReferencedSymbols.elementCount());
  ...
  for (symRefNumber = symRefTab->getIndexOfFirstSymRef();
    ...
    if (aux._neverReferencedSymbols.get(symRefNumber)) continue;

    referencedSymRefs.set(symRefNumber);

    if (excludedGlobals(symRef->getSymbol())) continue;

    if (symRef->getSymbol()->isMethod() ||
        symRef->getSymbol()->isRegularShadow())
  ...
  while (refIter.hasMoreElements()) {
    ...
    if (symRef) {
      ...
      if (numMethodsAndShadows != 0) num_aliases++;

      if (symRef->getSymbol()->isShadow()) num_aliases++;
      ...
      if (sym) {
        ...
        if (!shouldIndexVolatileSym(symRef, aux)) {
          if (trace())
            traceMsg(comp(),
                     "Ignoring Symbol [%p] because it is volatile %d or "
                     "aliased to a volatile %d\n",
                     sym, sym->isVolatile(),
                     aux._volatileOrAliasedToVolatileSymbols.get(symRefNumber));
          continue;
        }

        if (excludedGlobals(sym)) continue;
        ...
        if (!isTrivialUseDefSymRef(symRef, aux) &&
            ((sym->isParm() &&
             ...
             (aux._onceReadSymbolsIndices[symRef->getReferenceNumber()]
                  .IsNull() &&
              aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()]
                  .IsNull())))
  ...
  while (refIter.hasMoreElements()) {
    ...
    if (trace()) {
      traceMsg(comp(),
               "Non Trivial symbol Ref [%p:%d] isTrivial=%d isParm=%d "
               "_neverWritten=%d",
    ...
    if (symRef &&
      ...
      if (sym && sym->getLocalIndex() == NULL_USEDEF_SYMBOL_INDEX) {
        if (!shouldIndexVolatileSym(symRef, aux)) continue;

        if (sym->isStatic()) {
          if (!_indexStatics) continue;
        }
        if (sym->isMethod()) {
          if (!_hasCallsAsUses) continue;
  ...
  // unresolved reference contains a separate index for each alias, and the
  // other for the normal set where there is one index per node. The expanded
  // set is used during reaching definitions analysis and the normal set is used
  // to build the final use/def info.< The expanded index is kept in the node's
  // local index and the normal index is kept in the node's use/def index.
  ...
  // A cleared symRefToLocalIndexMap is required for each call to
  // indexSymbolsAndNodes - so we create it as a local variable here, and pass
  // it down to findUseDefNodes, as findUseDefNodes is recursive.
  TR::deque<uint32_t, TR::Region &> symRefToLocalIndexMap(
      comp()->getSymRefCount(), _region);
  ...
  if (getNumExpandedDefNodes() >= maxExpandedDefOrUseNodes) {
    dumpOptDetails(comp(),
                   "   use/def failed, too many expanded def nodes (%d)\n",
                   getNumExpandedDefNodes());
    return false;
  }
  if (getNumExpandedUseNodes() >= maxExpandedDefOrUseNodes) {
    dumpOptDetails(comp(),
                   "   use/def failed, too many expanded use nodes (%d)\n",
                   getNumExpandedUseNodes());
    return false;
  }
  if ((getNumDefNodes() - getNumIrrelevantStores()) * getNumUseNodes() >=
      maxDefByUseNodes) {
    dumpOptDetails(
        comp(), "   use/def failed, too many def/use combinations (%d)\n",
  ...
  if (getExpandedTotalNodes() > MAX_EXPANDED_TOTAL_NODES) {
    dumpOptDetails(comp(),
                   "   use/def failed, too many expanded nodes in total (%d)\n",
                   getExpandedTotalNodes());
    return false;
  }
  ...
  if (trace())
    dumpOptDetails(comp(),
                   "   use/def:  indexing symbols and nodes succeeded\n");
  if (trace()) {
...
bool TR_UseDefInfo::skipAnalyzingForCompileTime(TR::Node *node,
                                                TR::Block *block,
                                                TR::Compilation *comp,
                                                AuxiliaryData &aux) {
  if (isTrivialUseDefNode(node, aux)) return true;

  if (comp->getMethodHotness() >= hot) return false;
...
bool TR_UseDefInfo::findUseDefNodes(
    TR::Block *block, TR::Node *node, TR::Node *parent, TR::TreeTop *treeTop,
    AuxiliaryData &aux,
    TR::deque<uint32_t, TR::Region &> &symRefToLocalIndexMap,
    bool considerImplicitStores) {
  vcount_t visitCount = comp()->getVisitCount();
  if (visitCount == node->getVisitCount()) return true;
  node->setVisitCount(visitCount);
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    bool shouldConsiderLoadAddrChildrenImplicitStores =
        parentCouldHaveImplicitStoreChildren &&
        childIndexIndicatesImplicitStore(node, i);
    if (!findUseDefNodes(block, node->getChild(i), node, treeTop, aux,
                         symRefToLocalIndexMap,
  ...
  if (_useDefForRegs && (opCode.isLoadReg() || opCode.isStoreReg())) {
    ...
    symRef = node->getOpCode().hasSymbolReference() ? node->getSymbolReference()
                                                    : NULL;
    if (symRef == NULL) {
  ...
  if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
    if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
    ...
    } else if (isLoadAddrUse(node) || opCode.isLoadVar() ||
      ...
      if (num_aliases > 0) {
        if (symRefToLocalIndexMap[symRef->getReferenceNumber()] == 0) {
          ...
          // node->getSymbolReference()->getReferenceNumber(), num_aliases,
          // opCode.isFence(),
          // node->getSymbolReference()->getSymbol()->isMethod());
          useDefIndex = _numDefUseNodes++;
          //             traceMsg(comp(), "UDI: setting useDefIndex to %d
          //             _numDefUseNodes = %d\n",useDefIndex,_numDefUseNodes);
    ...
    //   node->setUseDefIndex(0);
    //   }
  } else if (!_tempsOnly && (opCode.isCall() || opCode.isFence()) &&
             num_aliases > 0) {
    if (!aux._volatileOrAliasedToVolatileSymbols.get(
            symRef->getReferenceNumber())) {
      localIndex = _numExpandedDefOnlyNodes;
    ...
    localIndex = _numExpandedDefOnlyNodes;
    _numExpandedDefOnlyNodes +=
        TR::NumTypes + 1;  // == arraylet shadows + read barrier....encode this
                           // where?  SymRefTab probably
    useDefIndex = _numDefOnlyNodes++;
  }
  // else
  //   {
  ...
  // traceMsg(comp(), "UDI: For node %p setting useDefIndex to
  // %d\n",node,useDefIndex);
  node->setUseDefIndex(useDefIndex);
  return true;
}

bool TR_UseDefInfo::assignAdjustedNodeIndex(TR::Block *block, TR::Node *node,
                                            TR::Node *parent,
                                            TR::TreeTop *treeTop,
                                            AuxiliaryData &aux,
                                            bool considerImplicitStores) {
  ...
  if (_useDefForRegs && (opCode.isLoadReg() || opCode.isStoreReg())) {
    ...
    if (!opCode.hasSymbolReference()) return true;

    symRef = node->getSymbolReference();
    if (!symRef) return true;
  ...
  if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
    if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
      ...
      //         traceMsg(comp(), "node %p setting adjustment to %d
      //         _numDefOnlyNodes = %d, _numDefUseNodes = %d\n",node,
      //         adjustment,_numDefOnlyNodes,_numDefUseNodes);
    } else if (opCode.isCall() || opCode.isFence()) {
      if (num_aliases) {
        expandedAdjustment = _numExpandedDefOnlyNodes;
  ...
  if (symRef &&
    ...
    aux._onceReadSymbols[symRef->getReferenceNumber()]->set(nodeIndex +
                                                            adjustment);
  }
  ...
  if (symRef &&
    ...
    aux._onceWrittenSymbols[symRef->getReferenceNumber()]->set(nodeIndex +
                                                               adjustment);
  }
  ...
  //   traceMsg(comp(), "UDI: Adjusting node %p index to
...
bool TR_UseDefInfo::childIndexIndicatesImplicitStore(TR::Node *node,
                                                     int32_t childIndex) {
  return false;
}

void TR_UseDefInfo::insertData(TR::Block *block, TR::Node *node,
  ...
  if (_useDefForRegs && (opCode.isLoadReg() || opCode.isStoreReg())) {
    ...
    if (!opCode.hasSymbolReference()) return;
  ...
  //   traceMsg(comp(), "For node %p nodeIndex = %d symIndex =
  ...
  if (symIndex != NULL_USEDEF_SYMBOL_INDEX) {
    ...
    if (parent && parent->getOpCode().isResolveCheck() && num_aliases > 1) {
      ...
      if (num_aliases > 1) definesMultipleSymbols = true;
  ...
  if (trace()) {
    if (!definesMultipleSymbols)
      traceMsg(
          comp(),
          "Node : %p   opCode = %s useDefIndex = %d localIndex = %d "
          "definesMultipleSymbols=%d isTrivialUseDefNode=%d adjustArray=%d \n",
          node, opCode.getName(), node->getUseDefIndex(), node->getLocalIndex(),
          definesMultipleSymbols, isTrivialUseDefNode(node, aux), adjustArray);
    else
      traceMsg(
          comp(),
          "Node : %p   opCode = %s useDefIndex = %d localIndex = %d-%d "
          "definesMultipleSymbols=%d isTrivialUseDefNode=%d adjustArray=%d \n",
          node, opCode.getName(), node->getUseDefIndex(), node->getLocalIndex(),
          node->getLocalIndex() + num_aliases - 1, definesMultipleSymbols,
...

  {
    ...
    for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
      ...
      if (symRef->getSymbol() != aliasedSymRef->getSymbol() &&
          ...
          (aliasedSymRef->getSymbol()->isAutoOrParm() ||
           aliasedSymRef->getSymbol()->isStatic()))
        continue;
      ...
      if (opCode.isLoadVarDirect()) {
        k = nodeIndex;
      } else if (aliasedSymRef->getSymbol()->isMethod() ||
                 aliasedSymRef->getSymbol()->isRegularShadow()) {
        if (ii == -1) {
          ii = i;
          k = nodeIndex + i;
          i++;
      ...
      if (trace())
        traceMsg(comp(),
                 "    symbol (u/d index=%d) is defined by node with localIndex "
                 "%d \n",
                 j, k);
    }
    ...
    if (node->getOpCode().isStoreDirect()) {
      ...
      for (i = 0; i < num; i++) {
        aux._defsForSymbol[symRef->getSymbol()->getLocalIndex()]->set(
            nodeIndex + i);
...
#endif
  else if (definesSingleSymbol && !isTrivialUseDefNode(node, aux) &&
           adjustArray) {
...
bool isSymRefFromInlinedMethod(TR::Compilation *comp,
                               TR::ResolvedMethodSymbol *currentMethod,
                               TR::SymbolReference *symRef) {
  while (true) {
    ...
    if (methodIndex <= JITTED_METHOD_INDEX) return false;
    symRef = comp->getResolvedMethodSymbolReference(methodIndex);
    if (!symRef) return false;
  }
  return false;
}

void TR_UseDefInfo::processReachingDefinition(void *vblockInfo,
                                              AuxiliaryData &aux) {
...
void TR_UseDefInfo::buildUseDefs(void *vblockInfo, AuxiliaryData &aux) {
  ...
  TR_BitVector nodesToBeDereferenced(getNumUseNodes(),
  ...
  if (_indexStatics && blockInfo) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (i >= _numDefsOnEntry) {
        ...
        if (i == 0 || i > getLastDefIndex()) continue;
      ...
      //         traceMsg(comp(), "UDI: setting _useDefInfo[%d}[i=%d] to true
      //         getLastUseIndex = %d getFirstUseIndex = %d numberOfNodes =
  ...
  if (_hasLoadsAsDefs) {
    ...
    // use - otherwise any defining loads are dereferenced back to real def
    // nodes
    //
    ...
    while (cursor.hasMoreElements()) {
      ...
      if (getNode(useIndex + getFirstUseIndex())
              ->getSymbolReference()
              ->getSymbol()
              ->isMethod())
        continue;
  ...
  if (trace()) {
    traceMsg(comp(),
             "\nUse/Def info: firstDefIndex=%d lastDefIndex=%d "
             "firstUseIndex=%d lastUseIndex=%d getNumDefsOnEntry()=%d\n",
             getFirstDefIndex(), getLastDefIndex(), getFirstUseIndex(),
             getLastUseIndex(), getNumDefsOnEntry());
  }
  ...
  for (int32_t j = getFirstUseIndex(); j <= lastUseIndex; j++) {
    ...
    if (trace()) {
      // I haven't narrowed down the problem but this is basically the same
      // assert as the one above, not guarded by trace().  It looks like that
      // one has been adjusted without a corresponding adjustment here.  So, I'm
      // just taking this one out.
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (defIndex >= getFirstUseIndex())
          traceMsg(comp(), "      Single defining load #%d[%p]", defIndex,
                   getNode(defIndex));
        else
...
void TR_UseDefInfo::dereferenceDefs(int32_t useIndex,
                                    TR_UseDefInfo::BitVector &nodesLookedAt,
                                    TR_UseDefInfo::BitVector &loadDefs) {
  if (trace()) {
    traceMsg(comp(), "Dereferencing defs for use index %d : ",
  ...
  if (!loadDefs.IsZero()) {
    ...
    useDefInfo |= loadDefs;
    if (trace()) {
      // traceMsg(comp(), "      Changing use index %d to have single defining
      // load %d\n", useIndex+getFirstUseIndex(), commonLoad);
      traceMsg(comp(),
               "      Changing use index %d to have defining loads : \n",
               useIndex + getFirstUseIndex());
      (*comp()) << loadDefs;
      traceMsg(comp(), "\n");
    }
  ...
  //
    ...
    for (int32_t i = getFirstUseIndex(); i < getNumDefNodes(); ++i) {
      if (useDefInfo.ValueAt(i)) dereferenceDef(useDefInfo, i, nodesLookedAt);
    }

    // save dereferenced info
    if (_hasLoadsAsDefs) _useDerefDefInfo[useIndex] = &useDefInfo;
  }

  if (trace()) {
    traceMsg(comp(),
             "New defs for use index %d : ", useIndex + getFirstUseIndex());
    (*comp()) << _useDefInfo[useIndex];
    traceMsg(comp(), "\n");
  }
}
...
int32_t TR_UseDefInfo::setSingleDefiningLoad(
    int32_t useIndex, TR_UseDefInfo::BitVector &nodesLookedAt,
    TR_UseDefInfo::BitVector &loadDefs) {
  TR_UseDefInfo::BitVector &useDefInfo = _useDefInfo[useIndex];
  nodesLookedAt[useIndex] = true;

  if (useDefInfo.IsZero()) return -2;
  ...
  if (trace()) {
    traceMsg(comp(), "   Checking use index %d for single defining load : ",
             useIndex + getFirstUseIndex());
    (*comp()) << useDefInfo;
    traceMsg(comp(), "\n");
  }
  ...
  for (; cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (!nodesLookedAt.ValueAt(defIndex)) {
      int32_t singleDef =
          setSingleDefiningLoad(defIndex, nodesLookedAt, loadDefs);
      if (singleDef == -2) {
...
/*
 void TR_UseDefInfo::dereferenceDef(TR_BitVector *useDefInfo, int32_t defIndex,
 TR_BitVector &nodesLookedAt)
 {
 ...
 * in the same block
 */
void TR_UseDefInfo::dereferenceDef(TR_UseDefInfo::BitVector &useDefInfo,
                                   int32_t defIndex,
                                   TR_UseDefInfo::BitVector &nodesLookedAt) {
  TR_ASSERT(defIndex < getTotalNodes(),
            "TR_UseDefInfo::getNode index(%d) is bigger than total(%d)\n",
            defIndex, getTotalNodes());

  TR::list<std::pair<TR::Node *, TR::TreeTop *>, TR::Region &> defIndices(
      _region);
  ...
  while (!defIndices.empty()) {
    ...
    {
      if (trace()) traceMsg(comp(), "      Resetting def index %d\n", defIndex);
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (myDef < getFirstUseIndex() ||
          ...
          if (trace()) traceMsg(comp(), "      Setting def index %d\n", myDef);
          useDefInfo[myDef] = true;
        } else if (!nodesLookedAt.ValueAt(myDef - getFirstUseIndex())) {
          if (trace()) traceMsg(comp(), "      Adding def index %d\n", myDef);
...
void TR_UseDefInfo::buildUseDefs(TR::Node *node, void *vanalysisInfo,
                                 TR_BitVector &nodesToBeDereferenced,
                                 TR::Node *parent, AuxiliaryData &aux) {
  vcount_t visitCount = comp()->getVisitCount();
  if (node->getVisitCount() == visitCount) return;
  if (trace()) traceMsg(comp(), "looking at node %p\n", node);
  ...
  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++) {
    buildUseDefs(node->getChild(i), analysisInfo, nodesToBeDereferenced, node,
                 aux);
  }
  ...
  if (node->getOpCode().hasSymbolReference() &&
    ...
    if (node->getOpCode().isLoadVarDirect() || (isLoadAddrUse(node))) {
      ...
      if (aux._neverWrittenSymbols.get(
              node->getSymbolReference()->getReferenceNumber())) {
        TR_ASSERT(realIndex >= 0,
                  "Out of bounds negative index, realIndex = %d\n", realIndex);
        //           traceMsg(comp(), "UDI: setting _useDefInfo[realIndex=%d][0]
        //           to true\n",realIndex);
        _useDefInfo[realIndex][0] = true;
        defsKnownTrivially = true;
        if (trace())
          traceMsg(comp(), "Reached here (entry) for use node %p\n", node);
      } else if (!aux._onceReadSymbolsIndices[node->getSymbolReference()
                                                  ->getReferenceNumber()]
                      .IsNull()) {
        if (trace())
        ...
        TR_ASSERT(realIndex >= 0,
                  "Out of bounds negative index, realIndex = %d\n", realIndex);
        //            traceMsg(comp(), "UDI: oring in _useDefInfo[realIndex=%d]
        //            with ",realIndex);
        //           *(comp()) <<
        //           aux._onceReadSymbols[node->getSymbolReference()->getReferenceNumber()];
        if (aux._onceReadSymbols[node->getSymbolReference()
                                     ->getReferenceNumber()])
          _useDefInfo[realIndex] |= CS2_TR_BitVector(
              *(aux._onceReadSymbols[node->getSymbolReference()
                                         ->getReferenceNumber()]));
        defsKnownTrivially = true;

      } else if (!aux._onceWrittenSymbolsIndices[node->getSymbolReference()
                                                     ->getReferenceNumber()]
                      .IsNull()) {
        if (trace())
        ...
        TR_ASSERT(realIndex >= 0,
                  "Out of bounds negative index, realIndex = %d\n", realIndex);
        //           traceMsg(comp(), "UDI: oring2 in _useDefInfo[realIndex=%d]
        //           with ",realIndex);
        //           *(comp()) <<
        //           aux._onceWrittenSymbols[node->getSymbolReference()->getReferenceNumber()];
        if (aux._onceWrittenSymbols[node->getSymbolReference()
                                        ->getReferenceNumber()])
          _useDefInfo[realIndex] |= CS2_TR_BitVector(
              *(aux._onceWrittenSymbols[node->getSymbolReference()
                                            ->getReferenceNumber()]));
        defsKnownTrivially = true;
      } else {
        // if
        // (_onceWrittenSymbols[node->getSymbolReference()->getReferenceNumber()])
        //   {
        //   if (trace())
        //      dumpOptDetails(comp(), "Use node %p is of a symbol written only
        //      once\n", node);
        ...
        //   TR_ASSERT(realIndex >= 0, "Out of bounds negative index, realIndex
        //   = %d\n", realIndex);
        //   _useDefInfo[realIndex][storeNode->getUseDefIndex()] = true;
        //    }
      }
    }
    if (_hasLoadsAsDefs || defsKnownTrivially) return;
  }
  ...
  if (isExpandedUseIndex(expandedNodeIndex) ||
    ...
    TR_ASSERT(symIndex != NULL_USEDEF_SYMBOL_INDEX,
              "Use node %p should have an indexed symbol", node);
    int32_t numDefs = 0;
    ...
    if (!aux._defsForSymbol[symIndex]->isEmpty()) {
      ...
      // assume aux._defsForSymbol[symIndex] is never NULL if _possibleDefs is
      // non-NULL
      if (trace()) {
      ...
      *defs = *(aux._defsForSymbol[symIndex]);
      ...
      if (analysisInfo) *defs &= *analysisInfo;
      ...
      while (cursor.hasMoreElements()) {
        ...
        if (i >= _numDefsOnEntry) {
          ...
          if (isUseIndex(j)) hasUseAsDef = true;

          if (memSymIndex == -1 ||
              getMemorySymbolIndex(defNode) != memSymIndex ||
              i == defNode->getLocalIndex()) {
            //              traceMsg(comp(), "UDI: setting
            //              _useDefInfo[realIndex=%d][j=%d] to
            //              true\n",realIndex,j);
            _useDefInfo[realIndex][j] = true;
          }
        } else if (!_uniqueIndexForDefsOnEntry && !ignoreDefsOnEntry) {
          //          traceMsg(comp(), "UDI: setting
          //          _useDefInfo[realIndex=%d][0] to true\n",realIndex);
          _useDefInfo[realIndex][0] = true;
        } else if (!ignoreDefsOnEntry) {
          //          traceMsg(comp(), "UDI: setting
          //          _useDefInfo[realIndex=%d][i=%d] to true\n",realIndex,i);
          _useDefInfo[realIndex][i] = true;
        }
        TR_ASSERT(realIndex >= 0, "realIndex is negative (%d - %d)", nodeIndex,
                  getFirstUseIndex());
        ++numDefs;
      }
    }
    ...
    if (numDefs == 0) {
      if (isLoadAddrUse(node)) {
        //        traceMsg(comp(), "UDI: setting _useDefInfo[realIndex=%d][0] to
        //        true because of loadaddr\n",realIndex);
        _useDefInfo[realIndex][0] = true;
      }
    }
    ...
    //
      ...
      if (!node->getOpCode().isCall()) nodesToBeDereferenced.set(realIndex);
    }
  }

  // Update the analysis info
  //
  if (analysisInfo == NULL) return;  // no analysisInfo to update
  int32_t numDefNodes;

  if (symIndex == NULL_USEDEF_SYMBOL_INDEX || node->getOpCode().isCall() ||
      node->getOpCode().isFence() ||
  ...
  for (i = 0; i < numDefNodes; ++i) analysisInfo->set(expandedNodeIndex + i);
...
TR::Node *TR_UseDefInfo::getSingleDefiningLoad(TR::Node *node) {
  TR_UseDefInfo::BitVector &info =
      _useDefInfo[node->getUseDefIndex() - getFirstUseIndex()];
  if (info.PopulationCount() == 1) {
    ...
    if ((firstDef >= getFirstUseIndex()) && (n = getNode(firstDef)) &&
        (n->getUseDefIndex() > 0) && (n->getOpCode().isLoadVar()) &&
...
const TR_UseDefInfo::BitVector &TR_UseDefInfo::getDefiningLoads_ref(
    TR::Node *node) {
  TR_UseDefInfo::BitVector &info =
      _useDefInfo[node->getUseDefIndex() - getFirstUseIndex()];
  if (!info.IsZero()) {
    ...
    if (useAsDef < getFirstUseIndex()) return _EMPTY;
    TR_UseDefInfo::BitVector temp1(comp()->allocator());
    const TR_UseDefInfo::BitVector &useDefTemp1 =
        getUseDef_ref(useAsDef, &temp1);
    for (cursor.SetToNextOne(); cursor.Valid(); cursor.SetToNextOne()) {
      useAsDef = cursor;
      if (useAsDef < getFirstUseIndex()) return _EMPTY;

      TR_UseDefInfo::BitVector temp2(comp()->allocator());
      const TR_UseDefInfo::BitVector &useDefTemp2 =
          getUseDef_ref(useAsDef, &temp2);
      if (!(useDefTemp1 == useDefTemp2)) return _EMPTY;
    }
    return info;
  }
  return _EMPTY;
...
const TR_UseDefInfo::BitVector &TR_UseDefInfo::getUseDef_ref(
    int32_t useIndex, TR_UseDefInfo::BitVector *defs) {
...
const TR_UseDefInfo::BitVector &TR_UseDefInfo::getUseDef_ref_body(
    int32_t useIndex, TR_BitVector *visitedDefs,
    TR_UseDefInfo::BitVector *defs) {
  TR_ASSERT(isUseIndex(useIndex), "useIndex is invalid");
  bool save = (defs == NULL);

  if (visitedDefs->get(useIndex)) return _EMPTY;
  ...
  if (!info.IsZero()) {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if ((firstDef < getFirstUseIndex()) ||
            (getNode(useIndex) && getNode(useIndex)->getSymbolReference() &&
             getNode(useIndex)
                 ...
                 ->getSymbol()
                 ->isMethod())) {
          //               traceMsg(comp(), "UDI, special case firstDef = %d
          //               getFirstUseIndex() = %d getNode(useIndex) =
      ...
      if (!(info.PopulationCount() > 1)) {
        ...
        if (!defs) {
          const TR_UseDefInfo::BitVector &info =
              getUseDef_ref_body(i, visitedDefs);
          if (_hasLoadsAsDefs)
            _useDerefDefInfo[useIndex - getFirstUseIndex()] = &info;
          return info;
        } else {
  ...
  if (!defs) return _EMPTY;

  return *defs;
}

bool TR_UseDefInfo::getUsesFromDefIsZero(int32_t defIndex, bool loadAsDef) {
  const TR_UseDefInfo::BitVector &usesFromDef =
      getUsesFromDef_ref(defIndex, loadAsDef);
  return usesFromDef.IsZero();
}

bool TR_UseDefInfo::getUsesFromDef(BitVector &usesFromDef, int32_t defIndex,
                                   bool loadAsDef) {
  usesFromDef.Or(getUsesFromDef_ref(defIndex, loadAsDef));
  return !usesFromDef.IsZero();
}

const TR_UseDefInfo::BitVector &TR_UseDefInfo::getUsesFromDef_ref(
    int32_t defIndex, bool loadAsDef) {
  if ((_defUseInfo.size() > 0) && !loadAsDef) {
    //      traceMsg(comp(), "UDI: returning _defUseInfo at index %d
    //      numberOfElements = %d\n_defUseInfo =
    //      ",defIndex,_defUseInfo.NumberOfElements());
    //      *(comp()) << _defUseInfo;
    return _defUseInfo[defIndex];
  } else if ((_loadDefUseInfo.size() > 0) && loadAsDef) {
    //      traceMsg(comp(), "UDI: returning _loadDefUseInfo at index
    //      %d\n",defIndex);
    ...
    for (i = getNumUseNodes() - 1; i >= 0; --i) {
      ...
      if (getNode(useIndex) == NULL) continue;
      bool isDef = false;
      if (loadAsDef) {
        const TR_UseDefInfo::BitVector &defs =
            _useDefInfo[useIndex - getFirstUseIndex()];
        if (defs.ValueAt(defIndex)) isDef = true;
      } else {
...
void TR_UseDefInfo::setUseDef(int32_t useIndex, int32_t defIndex) {
  ...
  //   traceMsg(comp(), "UDI: setUseDef _useDefInfo[realIndex=%d][defIndex=%d]
  //   to true\n",realIndex,defIndex);
...
TR::Node *TR_UseDefInfo::getNode(int32_t index) {
  ...
  if (ud->isDef()) {
    ...
    if ((node->getOpCode().isCheck() ||
      ...
      // Set numChildren of the node to 1 so that it passes the assertion in
      // getFirstChild But we must save and restore the number of children here,
      // because if VP deletes unreachable code, it may expect the number of
      // children to be set to zero.
      //
...
void TR_UseDefInfo::buildDefUseInfo(bool loadAsDef) {
  ...
  _defUseInfo.resize(getNumDefNodes(), TR_UseDefInfo::BitVector(
                                           comp()->allocator(allocatorName)));

  if (loadAsDef)
    _loadDefUseInfo.resize(getNumDefNodes(),
                           TR_UseDefInfo::BitVector(allocator()));

  for (int32_t i = getFirstUseIndex(); i <= getLastUseIndex(); i++) {
    const TR_UseDefInfo::BitVector &defs = getUseDef_ref(i);

    if (!defs.IsZero()) {
      // the getUseDef() returns a bitvector that contains only store
      // definitions
      TR_UseDefInfo::BitVector::Cursor cursor(defs);
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        int32_t defIndex = cursor;
        TR_ASSERT((defIndex < getNumDefNodes()),
                  "USEDEF: found def which is not store or call: useIndex = "
                  "%d, defIndex = %d",
                  i, defIndex);
        //         traceMsg(comp(), "UDI: for bit vector at index %d setting %d
    ...
    if (loadAsDef) {
      const TR_UseDefInfo::BitVector &loadDefs =
          _useDefInfo[i - getFirstUseIndex()];
      if (!loadDefs.IsZero()) {
        TR_UseDefInfo::BitVector::Cursor cursor(loadDefs);
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          int32_t defIndex = cursor;
          // TR_ASSERT((defIndex < getNumDefNodes()), "USEDEF: found def which
          // is not store or call %d", defIndex);
...
bool TR_UseDefInfo::canComputeReachingDefs() {
  ...
  if (costPerSet > REACHING_DEFS_LIMIT) {
    dumpOptDetails(comp(),
                   "   use/def failed, Reaching defs set too large(%d)\n",
                   costPerSet);
    return false;
  }
  return true;
...
int32_t TR_UseDefInfo::getSymRefIndexFromUseDefIndex(int32_t udIndex) {
  TR_ASSERT(udIndex < _sideTableToSymRefNumMap.size(),
            "index into _sideTableToSymRefNumMap out of bounds");
