  ...
  : _compilation(c)
{
  ...
  //
  _info =
    (BBInfoChk*)trMemory()->allocateStackMemory(_numBlocks * sizeof(BBInfoChk));

  memset(_info, 0, _numBlocks * sizeof(BBInfoChk));

  int32_t nextNodeNumber = cfg->getNextNodeNumber();
  _dfNumbers =
    (int32_t*)trMemory()->allocateStackMemory(nextNodeNumber * sizeof(int32_t));
...
//
void
TR_DominatorsChk::findDominators(TR::Block* start)
{
  int32_t i, j;
  BBInfoChk* w;
  bool change = true;
  TR_BitVector* intersection =
  ...
  while (change) {
    ...
    for (i = 2; i < _topDfNum + 1; i++) {
      ...
      TR::CFGEdge* pred;
      BBInfoChk* predinfo;
      for (auto pred = w->_block->getPredecessors().begin();
           pred != w->_block->getPredecessors().end();
           ++pred) {
        predinfo =
          _info + (_dfNumbers[toBlock((*pred)->getFrom())->getNumber()] + 1);
        (*intersection) &= (*(predinfo->_bucket));
      }
      for (auto pred = w->_block->getExceptionPredecessors().begin();
           pred != w->_block->getExceptionPredecessors().end();
           ++pred) {
        predinfo =
          _info + (_dfNumbers[toBlock((*pred)->getFrom())->getNumber()] + 1);
        (*intersection) &= (*(predinfo->_bucket));
...
//
void
TR_DominatorsChk::initialize(TR::Block* start, TR::Block* nullParent)
{
  TR_Array<StackInfo>* stack = new (trStackMemory())
    TR_Array<StackInfo>(trMemory(), _numBlocks / 2, false, stackAlloc);
...
//
void
TR_DominatorsChk::findImmediateDominators()
{
  int32_t i, j, k;
  BBInfoChk *w, *x;
