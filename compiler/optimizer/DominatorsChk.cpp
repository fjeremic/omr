...
TR_DominatorsChk::TR_DominatorsChk(TR::Compilation *c) : _compilation(c) {
  int32_t i, j;
  _topDfNum = 0;
  ...
  //
  _info = (BBInfoChk *)trMemory()->allocateStackMemory(_numBlocks *
                                                       sizeof(BBInfoChk));

  memset(_info, 0, _numBlocks * sizeof(BBInfoChk));

  int32_t nextNodeNumber = cfg->getNextNodeNumber();
  _dfNumbers = (int32_t *)trMemory()->allocateStackMemory(nextNodeNumber *
                                                          sizeof(int32_t));
...
void TR_DominatorsChk::findDominators(TR::Block *start) {
  ...
  while (change) {
    ...
    for (i = 2; i < _topDfNum + 1; i++) {
      ...
      for (j = 0; j < _numBlocks - 1; j++) intersection->set(j);

      TR::CFGEdge *pred;
      BBInfoChk *predinfo;
      for (auto pred = w->_block->getPredecessors().begin();
           pred != w->_block->getPredecessors().end(); ++pred) {
        predinfo =
    ...
    for (j = 1; j < _topDfNum + 1; j++) {
      ...
      if (w->_tmpbucket->get(j - 1)) {
        for (k = 0; k < _numBlocks - 1; k++) {
          if (!((j - 1) == k)) {
            if (x->_tmpbucket->get(k)) w->_tmpbucket->reset(k);
          }
        }
      }
    }
