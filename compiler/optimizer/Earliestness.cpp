...
void TR_Earliestness::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *,
                                  TR_BitVector *) {}

TR_Earliestness::TR_Earliestness(TR::Compilation *comp,
  ...
  _globalAnticipatability = new (comp->allocator())
      TR_GlobalAnticipatability(comp, optimizer, rootStructure, trace);

  if (trace) traceMsg(comp, "Starting Earliestness\n");
  ...
  if (trace)
    traceMsg(comp, "Earl # bits %d, %d\n", _numberOfBits,
             _globalAnticipatability->_numberOfBits);
  if (trace) {
    int32_t i;
...
//
//
void TR_Earliestness::analyzeTreeTopsInBlockStructure(
  ...
  *_temp -= *(
      _globalAnticipatability->_blockAnalysisInfo[blockStructure->getNumber()]);
  *(_blockAnalysisInfo[blockStructure->getNumber()]) &= *_temp;

  _temp->setAll(_numberOfBits);
  *_temp -= *(_globalAnticipatability->_localTransparency.getAnalysisInfo(
