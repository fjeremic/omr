...
typedef TR::typed_allocator<std::pair<TR::CFGEdge *const, TR_BitVector *>,
                            TR::Region &>
    DefinedOnAllPathsMapAllocator;
typedef std::map<TR::CFGEdge *, TR_BitVector *, std::less<TR::CFGEdge *>,
                 DefinedOnAllPathsMapAllocator>
...
private:
  TR::TreeTop *createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree,
  ...
  TR::TreeTop *createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree,
                               TR::SymbolReference *srcRef,
                               TR::SymbolReference *dstRef, TR::Node *lenRef,
                               TR::SymbolReference *srcObjRef,
                               TR::SymbolReference *dstObjRef, bool isForward);

  int64_t arraycopyHighFrequencySpecificLength(TR::Node *byteLenNode);
  TR::TreeTop *createPointerCompareNode(TR::Node *node,
                                        TR::SymbolReference *srcRef,
                                        TR::SymbolReference *dstRef);
  TR::TreeTop *createRangeCompareNode(TR::Node *node,
                                      TR::SymbolReference *srcRef,
                                      TR::SymbolReference *dstRef,
                                      TR::SymbolReference *lenRef);
  //   int shiftAmount(TR::DataType type, TR::Node* node);

  TR::TreeTop *createMultipleArrayNodes(TR::TreeTop *arrayTreeTop,
                                        TR::Node *node);
  TR::TreeTop *tryToSpecializeForLength(TR::TreeTop *tt,
                                        TR::Node *arraycopyNode);
  TR::TreeTop *specializeForLength(TR::TreeTop *tt, TR::Node *arraycopyNode,
...
public:
  ...
  Relationship *createRelationship(int32_t relative,
                                   TR::VPConstraint *constraint);
  ...
  StoreRelationship *createStoreRelationship(TR::Symbol *symbol,
  ...
  void addUnsafeArrayAccessNode(ncount_t index) {
    _unsafeArrayAccessNodes->set(index);
  }
  ...
  public:
    ...
    void initialize(int32_t valueNumber, Relationship *rel,
  ...
  public:
    ValueConstraint(int32_t valueNumber) : _valueNumber(valueNumber) {}
  ...
  public:
    ...
    ValueConstraint *getFirst() {
      _next = _list->getFirst();
      return getNext();
    }
    ValueConstraint *getNext() {
      ValueConstraint *vc = _next;
      if (_next)
        _next = _next->getNext();
      return vc;
    }
  ...
  ValueConstraint *createValueConstraint(int32_t valueNumber,
  ...
  // This table contains the value constraints that apply across the whole
  // method. It is keyed on the value number, and each entry contains a list of
  // relationships.
  //
  ...
  // GlobalConstraint *createGlobalConstraint(TR::CFGEdge *edge, bool
  // keepBlockList);
  ...
  void createExceptionEdgeConstraints(uint32_t exceptions,
  ...
  TR::VPConstraint *addGlobalConstraint(TR::Node *node,
                                        TR::VPConstraint *constraint,
                                        TR::Node *relative = NULL);
  TR::VPConstraint *addGlobalConstraint(TR::Node *node, int32_t valueNumber,
                                        TR::VPConstraint *constraint,
                                        int32_t relative);
  TR::VPConstraint *addBlockConstraint(TR::Node *node,
                                       TR::VPConstraint *constraint,
                                       TR::Node *relative = NULL,
                                       bool mustBeValid = true);
  TR::VPConstraint *addEdgeConstraint(TR::Node *node,
                                      TR::VPConstraint *constraint,
                                      EdgeConstraints *contraints,
                                      TR::Node *relative = NULL);
  TR::VPConstraint *addConstraintToList(TR::Node *node, int32_t valueNumber,
                                        int32_t relative,
                                        TR::VPConstraint *constraint,
                                        ValueConstraints *valueConstraints,
                                        bool replaceExisting = false);
  TR::VPConstraint *addBlockOrGlobalConstraint(TR::Node *node,
                                               TR::VPConstraint *constraint,
                                               bool isGlobal,
                                               TR::Node *relative = NULL);
  void mergeRelationships(TR_LinkHead<Relationship> &fromList,
                          TR_LinkHead<Relationship> &toList,
                          int32_t valueNumber, bool preserveFrom = false,
                          StoreRelationship *mergingStore = NULL,
                          List<TR::Symbol> *storeSymbols = NULL,
                          bool inBothLists = false);
  void mergeStoreRelationships(ValueConstraint *fromvc, ValueConstraint *tovc,
                               bool preserveFrom = false);
  void mergeValueConstraints(ValueConstraint *fromvc, ValueConstraint *tovc,
                             bool preserveFrom = false);
  void mergeEdgeConstraints(EdgeConstraints *fromEdge, EdgeConstraints *toEdge);
  void mergeConstraintIntoEdge(ValueConstraint *constraint,
  ...
  //
  StoreRelationship *findStoreRelationship(TR_LinkHead<StoreRelationship> &list,
                                           TR::Symbol *symbol);
  Relationship *findConstraintInList(TR_LinkHead<Relationship> &list,
                                     int32_t relative);
  Relationship *findConstraint(int32_t valueNumber,
                               int32_t relative = AbsoluteConstraint);
  Relationship *findValueConstraint(int32_t valueNumber,
                                    ValueConstraints &valueConstraints,
                                    int32_t relative = AbsoluteConstraint);
  Relationship *findEdgeConstraint(int32_t valueNumber, EdgeConstraints *edge,
                                   int32_t relative = AbsoluteConstraint);

  StoreRelationship *findStoreConstraint(int32_t valueNumber,
                                         TR::Symbol *symbol);
  StoreRelationship *
  findStoreValueConstraint(int32_t valueNumber, TR::Symbol *symbol,
                           ValueConstraints &valueConstraints);
  StoreRelationship *findStoreEdgeConstraint(int32_t valueNumber,
                                             TR::Symbol *symbol,
                                             EdgeConstraints *edge);
  ...
  //
  TR::VPConstraint *getConstraint(TR::Node *node, bool &isGlobal,
                                  TR::Node *relative = NULL);
  TR::VPConstraint *mergeDefConstraints(TR::Node *node, int32_t relative,
                                        bool &isGlobal,
                                        bool forceMerge = false);
  TR::VPConstraint *applyGlobalConstraints(TR::Node *node, int32_t valueNumber,
                                           TR::VPConstraint *constraint,
                                           int32_t relative);
  void invalidateParmConstraintsIfNeeded(TR::Node *node,
                                         TR::VPConstraint *constraint);
  void checkTypeRelationship(TR::VPConstraint *lhs, TR::VPConstraint *rhs,
                             int32_t &value, bool isInstanceOf,
                             bool isCheckCast);
  TR_YesNoMaybe isCastClassObject(TR::VPClassType *type);
  TR::VPConstraint *getStoreConstraint(TR::Node *node,
                                       TR::Node *relative = NULL);
  ...
  bool hasBeenStored(int32_t valueNumber, TR::Symbol *symbol,
                     ValueConstraints &valueConstraints);

  void generalizeStores(ValueConstraints &stores,
                        ValueConstraints *edgeConstraints);
  void findStoresInBlock(TR::Block *block, ValueConstraints &stores);

  bool propagateConstraint(TR::Node *node, int32_t valueNumber,
                           Relationship *first, Relationship *rel,
                           ValueConstraints *valueConstraints);
  ...
  void replaceByConstant(TR::Node *node, TR::VPConstraint *constraint,
                         bool isGlobal);
  ...
  bool canTransformArrayCopyCallForSmall(TR::Node *node, int32_t &srcLength,
                                         int32_t &dstLength, int32_t &stride,
                                         TR::DataType &type);
  ...
  struct VPTreeTopPair {
    ...
    VPTreeTopPair(TR::TreeTop *treetop1, TR::TreeTop *treetop2)
        : _treetop1(treetop1), _treetop2(treetop2) {}
  ...
  struct TR_ArrayCopySpineCheck {
    ...
    TR_ArrayCopySpineCheck(TR::TreeTop *arraycopyTree,
                           TR::SymbolReference *arraycopySymRef,
                           TR::Node *srcObjNode, TR::Node *srcOffNode,
                           TR::Node *dstObjNode, TR::Node *dstOffNode,
                           TR::Node *copyLenNode)
        : _arraycopyTree(arraycopyTree), _arraycopySymRef(arraycopySymRef),
          _srcObjNode(srcObjNode), _srcOffNode(srcOffNode),
          _dstObjNode(dstObjNode), _dstOffNode(dstOffNode),
          _copyLenNode(copyLenNode), _srcObjRef(NULL), _srcOffRef(NULL),
          _dstObjRef(NULL), _dstOffRef(NULL), _copyLenRef(NULL) {}
  ...
  TR::TreeTop *createSpineCheckNode(TR::Node *node,
                                    TR::SymbolReference *objSymRef);
  TR::TreeTop *createAndInsertStoresForArrayCopySpineCheck(
      TR_ArrayCopySpineCheck *checkInfo);
  TR::TreeTop *
  createArrayCopyCallForSpineCheck(TR_ArrayCopySpineCheck *checkInfo);
  ...
  void transformReferenceArrayCopyWithoutCreatingStoreTrees(
  ...
  TR::TreeTop *createPrimitiveArrayNodeWithoutFlags(
  ...
  TR::TreeTop *createReferenceArrayNodeWithoutFlags(
      TR::TreeTop *tree, TR::TreeTop *newTree,
      TR::SymbolReference *srcObjectRef, TR::SymbolReference *dstObjectRef,
      TR::SymbolReference *lenRef, TR::SymbolReference *srcRef,
      TR::SymbolReference *dstRef, bool useFlagsOnOriginalArraycopy);
#ifdef J9_PROJECT_SPECIFIC
  void generateArrayTranslateNode(TR::TreeTop *vcallTree,
                                  ...
                                  TR::SymbolReference *lenRef,
                                  TR::SymbolReference *tableRef, bool hasTable);
#endif
  TR::TreeTop *createConverterCallNodeAfterStores(
  ...
  TR::TreeTop *buildSameLeafTest(TR::Node *offset, TR::Node *len,
                                 TR::Node *spineShiftNode);
  TR::TreeTop *createArrayCopyVCallNodeAfterStores(
      TR::TreeTop *tree, TR::SymbolReference *srcObjectRef,
      TR::SymbolReference *dstObjectRef, TR::SymbolReference *lenRef,
      TR::SymbolReference *srcRef, TR::SymbolReference *dstRef);
  TR::TreeTop *createSameLeafTestAfterStores(TR::TreeTop *tree,
                                             TR::SymbolReference *child1Ref,
                                             TR::SymbolReference *child2Ref);
  void generateRTArrayNodeWithoutFlags(
      TR_RealTimeArrayCopy *rtArrayCopyTree, TR::TreeTop *dupArraycopyTree,
      TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
      TR::SymbolReference *srcOffRef, TR::SymbolReference *dstOffRef,
   ...
   *        consuming OMR can implement this function to provide
  ...
  void printStructureInfo(TR_Structure *structure, bool starting,
  ...
  bool removeConstraints(int32_t valueNumber, ValueConstraints *vc,
                         bool findStores);
  bool removeConstraints(int32_t valueNumber,
                         ValueConstraints *valueConstraints);
  bool removeConstraints(int32_t valueNumber);
  bool removeStoreConstraints(ValueConstraints *vc, int32_t valueNumber,
                              int32_t relative);
  ...
  struct InductionVariable : public TR_Link<InductionVariable> {
    InductionVariable(TR::Symbol *sym, TR::Node *entryDef, int32_t incrVN,
  ...
  struct VirtualGuardInfo : public TR_Link<VirtualGuardInfo> {
    VirtualGuardInfo(OMR::ValuePropagation *vp, TR_VirtualGuard *vgOld,
  ...
  void createNewBucketForArrayIndex(ArrayLengthToVersion *,
                                    TR_LinkHead<ArrayLengthToVersion> *,
                                    int32_t, TR::Node *, TR::Node *,
                                    TR_OpaqueClassBlock *);
  void collectDefSymRefs(TR::Node *, TR::Node *);
  bool prepareForBlockVersion(TR_LinkHead<ArrayLengthToVersion> *);
  void addToSortedList(TR_LinkHead<ArrayLengthToVersion> *,
                       ArrayLengthToVersion *);
  ...
  CS2::HashTable<uint64_t, TR::list<TR::Node *> *, TR::Allocator>
      _constNodeInfo;
