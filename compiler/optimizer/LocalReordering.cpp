...
int32_t TR_LocalReordering::perform() {
  if (trace()) traceMsg(comp(), "Starting LocalReordering\n");

  TR::TreeTop *treeTop = comp()->getStartTree();
  while (treeTop != NULL) {
    TR::Node *node = treeTop->getNode();
    TR_ASSERT(node->getOpCodeValue() == TR::BBStart,
              "Local reordering detected BBStart treetop");
    TR::Block *block = node->getBlock();
    if (!containsBarriers(block)) transformBlock(block);
...
int32_t TR_LocalReordering::performOnBlock(TR::Block *block) {
  if (block->getEntry() && !containsBarriers(block)) transformBlock(block);
  return 0;
}
...
bool TR_LocalReordering::transformBlock(TR::Block *block) {
  ...
  //
  int32_t i;
  for (i = comp()->getSymRefTab()->getIndexOfFirstSymRef(); i < symRefCount;
       i++)
  ...
  // register spills simply because the value in the register was live for too
  // long
  //
  ...
  while (currentTree != exitTree) {
    if (currentTree->getNode()->getOpCode().isStore()) {
      TR::SymbolReference *symRef =
          currentTree->getNode()->getSymbolReference();
  ...
  _storeTreesAsArray = (TR::TreeTop **)trMemory()->allocateStackMemory(
      _numStoreTreeTops * sizeof(TR::TreeTop *));
    ...
    if (currentNode->getOpCode().isStore()) {
      ...
      if (sym->isAuto() || sym->isParm()) {
        ...
        if (storedValue->getOpCode().hasSymbolReference()) {
          if (storedValue->getSymbolReference()
                  ->getSymbol()
                  ->isMethodMetaData())
        ...
        if (!subtreeCommoned) {
          if (!specialImmovableStore)
            insertDefinitionBetween(
                currentTree, _treeTopsAsArray[symRef->getReferenceNumber()]);
          _counter++;
        } else
          _numStoreTreeTops++;
      }
...
void TR_LocalReordering::collectUses(TR::Block *block) {
  ...
  while (!(currentTree == exitTree)) {
    TR::Node *currentNode = currentTree->getNode();
    moveStoresEarlierIfRhsAnchored(block, currentTree, currentNode, NULL,
                                   visitCount1);
    if (currentNode->getOpCode().isStore()) {
      TR::SymbolReference *symRef = currentNode->getSymbolReference();
      TR::Symbol *sym = symRef->getSymbol();
...
void TR_LocalReordering::moveStoresEarlierIfRhsAnchored(TR::Block *block,
                                                        TR::TreeTop *treeTop,
                                                        TR::Node *node,
                                                        TR::Node *parent,
                                                        vcount_t visitCount) {
  ...
  if (node->getReferenceCount() > 1) {
    for (int32_t i = 0; i < _numStoreTreeTops; i++) {
      if ((_storeTreesAsArray[i] != NULL) &&
          (_storeTreesAsArray[i] != treeTop)) {
        if (_storeTreesAsArray[i]->getNode()->getFirstChild() == node) {
          ///// Try to hoist it earlier
          /////
          TR::SymbolReference *storeSymRef =
              _storeTreesAsArray[i]->getNode()->getSymbolReference();
          _seenSymbols->empty();
          _seenUnpinnedInternalPointer = false;
          _seenSymbols->set(storeSymRef->getReferenceNumber());

          if (storeSymRef->sharesSymbol())
            storeSymRef->getUseDefAliasesIncludingGCSafePoint()
                .getAliasesAndUnionWith(*_seenSymbols);

          storeSymRef->getUseonlyAliases().getAliasesAndUnionWith(
              *_seenSymbols);
  ...
  for (int32_t j = 0; j < node->getNumChildren(); j++) {
    ...
    if ((child->getLocalIndex() == 0) && (child->getReferenceCount() > 1) &&
  ...
  if ((numDeadChildren > 1) && (!parent || !parent->getOpCode().isCheck()) &&
      !node->getOpCode().isTreeTop() && !node->getOpCode().isCall() &&
      ...
      (!cg()->supportsFusedMultiplyAdd() ||
       ((parent->getOpCodeValue() != TR::dadd ||
         node->getOpCodeValue() != TR::dmul) &&
        (parent->getOpCodeValue() != TR::fadd ||
         node->getOpCodeValue() != TR::fmul))) &&
      !noReordering) {
    node->setLocalIndex(node->getLocalIndex() + 1);
...
bool TR_LocalReordering::insertEarlierIfPossible(TR::TreeTop *storeTree,
                                                 TR::TreeTop *treeTop,
                                                 bool strictCheck) {
  ...
  while (currentTree != treeTop) {
    ...
    if (!stopHere) {
      if (currentNode->getOpCode()
              .isCheckCast())  // Moving a store earlier than a checkcast
                               // usually results in suboptimal type info
                               // (because of the way store constraints work,
                               // they only pick up the type info when the store
                               // was encountered)
        stopHere = true;
    }
    ...
    if (stopHere) {
      if (performTransformation(comp(),
                                "\n%sInserting Definition @ 1 : [%p] between "
                                "%p and %p (earlier between %p and %p)\n",
                                OPT_DETAILS, storeTree->getNode(),
                                currentTree->getNode(),
                                currentTree->getNextTreeTop()->getNode(),
                                storeTree->getPrevTreeTop()->getNode(),
                                storeTree->getNextTreeTop()->getNode())) {
        ...
        // dumpOptDetails(comp(), "\n%sInserting Definition @ 1 : [%p] between
        // %p and %p (earlier between %p and %p)\n", OPT_DETAILS,
        // storeTree->getNode(), currentTree->getNode(), nextTree->getNode(),
  ...
  if (!foundInsertionSpot &&
      performTransformation(comp(),
                            "\n%sInserting Definition @ 2 : [%p] between %p "
                            "and %p (earlier between %p and %p)\n",
                            OPT_DETAILS, storeTree->getNode(),
                            currentTree->getNode(),
                            currentTree->getNextTreeTop()->getNode(),
                            storeTree->getPrevTreeTop()->getNode(),
    ...
    // dumpOptDetails(comp(), "\n%sInserting Definition @ 2 : [%p] between %p
    // and %p (earlier between %p and %p)\n", OPT_DETAILS, storeTree->getNode(),
    // currentTree->getNode(), nextTree->getNode(), origPrevTree->getNode(),
...
//
void TR_LocalReordering::setUseTreeForSymbolReferencesIn(TR::TreeTop *treeTop,
                                                         TR::Node *node,
                                                         vcount_t visitCount) {
  ...
  if (opCode.hasSymbolReference()) {
    ...
    if (opCode.isLoadVar() || (opCode.getOpCodeValue() == TR::loadaddr)) {
      ...
      if (!opCode.isStore()) {
        TR_UseOnlyAliasSetInterface useAliasesInterface =
            symRef->getUseonlyAliases();
...
bool TR_LocalReordering::containsBarriers(TR::Block *block) {
  ...
  while (!(currentTree == exitTree)) {
    ...
    TR::ILOpCode &opCode = node->getOpCode();
    if ((opCode.getOpCodeValue() == TR::monent) ||
...
// Inserts the store as close to the use as possible; checks whether the symbol
// is used/defined in the intervening trees before pushing the store down.
//
void TR_LocalReordering::insertDefinitionBetween(TR::TreeTop *treeTop,
                                                 TR::TreeTop *exitTree) {
  if (treeTop == exitTree) return;

  if ((treeTop->getNextTreeTop()->getNode()->getOpCodeValue() ==
       TR::dbgFence) &&
      (treeTop->getNextTreeTop()->getNextTreeTop() == exitTree))
    return;
  ...
  while (!(currentTree == exitTree)) {
    ...
    if (isAnySymInDefinedOrUsedBy(currentNode, visitCount1)) {
      if (performTransformation(comp(),
                                "\n%sInserting Definition : [%p] between %p "
                                "and %p (earlier between %p and %p)\n",
                                OPT_DETAILS, originalNode,
                                currentTree->getPrevTreeTop()->getNode(),
                                currentTree->getNode(),
                                originalTree->getPrevTreeTop()->getNode(),
                                originalTree->getNextTreeTop()->getNode())) {
...
void TR_LocalReordering::collectSymbolsUsedAndDefinedInNode(
...
//
bool TR_LocalReordering::isAnySymInDefinedOrUsedBy(TR::Node *node,
                                                   vcount_t visitCount) {
  if (visitCount == node->getVisitCount()) return false;
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (symReference->getUseDefAliasesIncludingGCSafePoint(isCallDirect)
            .containsAny(*_seenSymbols, comp()))
      return true;
  ...
  if (node->canCauseGC() && _seenUnpinnedInternalPointer) {
    dumpOptDetails(comp(),
                   "\n%sisAnySymInDefinedOrUsedBy : found unpinned internal "
                   "pointer at GC point %p\n",
                   OPT_DETAILS, node);
    return true;
  }
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    if (isAnySymInDefinedOrUsedBy(child, visitCount)) return true;
  }
...
//
bool TR_LocalReordering::isAnySymInDefinedBy(TR::Node *node,
                                             vcount_t visitCount) {
  if (visitCount == node->getVisitCount()) return false;
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if ((!node->getOpCode().isLoadVar() ||
      ...
      if (_seenSymbols->get(symRefNum)) return true;
  ...
  if (node->canCauseGC() && _seenUnpinnedInternalPointer) {
    dumpOptDetails(comp(),
                   "\n%sisAnySymInDefinedBy : found unpinned internal pointer "
                   "at GC point %p\n",
                   OPT_DETAILS, node);
    return true;
  }

  if (_stopNodes->get(node->getGlobalIndex())) return true;

  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    if (isAnySymInDefinedBy(child, visitCount)) return true;
  }
...
bool TR_LocalReordering::isSubtreeCommoned(TR::Node *node) {
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    if (isSubtreeCommoned(child)) return true;
  }

  return false;
}

const char *TR_LocalReordering::optDetailString() const throw() {
