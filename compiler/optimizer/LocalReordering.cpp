...
void TR_LocalReordering::moveStoresEarlierIfRhsAnchored(TR::Block* block, TR::TreeTop* treeTop, TR::Node* node, TR::Node* parent, vcount_t visitCount)
{
    ...
    if ((numDeadChildren > 1) && (!parent || !parent->getOpCode().isCheck()) && !node->getOpCode().isTreeTop() && !node->getOpCode().isCall() && (node->getOpCodeValue() != TR::New) && (node->getOpCodeValue() != TR::newarray) && (node->getOpCodeValue() != TR::anewarray) && (node->getOpCodeValue() != TR::multianewarray) && (node->getOpCodeValue() != TR::MergeNew) && (node->getOpCodeValue() != TR::monent) && (node->getOpCodeValue() != TR::monexit) && !node->getOpCode().isArrayRef() && (!cg()->supportsFusedMultiplyAdd() || ((parent->getOpCodeValue() != TR::dadd || node->getOpCodeValue() != TR::dmul) && (parent->getOpCodeValue() != TR::fadd || node->getOpCodeValue() != TR::fmul))) && !noReordering) {
        node->setLocalIndex(node->getLocalIndex() + 1);
