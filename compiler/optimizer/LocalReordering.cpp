...
int32_t
TR_LocalReordering::performOnBlock(TR::Block* block)
{
...
void
TR_LocalReordering::prePerformOnBlocks()
{
...
void
TR_LocalReordering::postPerformOnBlocks()
{
  comp()->incVisitCount();
}

bool
TR_LocalReordering::transformBlock(TR::Block* block)
{
  int32_t symRefCount = comp()->getSymRefCount();
...
void
TR_LocalReordering::delayDefinitions(TR::Block* block)
{
...
void
TR_LocalReordering::collectUses(TR::Block* block)
{
  ...
  while (!(currentTree == exitTree)) {
    TR::Node* currentNode = currentTree->getNode();
    moveStoresEarlierIfRhsAnchored(
      block, currentTree, currentNode, NULL, visitCount1);
    if (currentNode->getOpCode().isStore()) {
      TR::SymbolReference* symRef = currentNode->getSymbolReference();
      TR::Symbol* sym = symRef->getSymbol();
...
void
TR_LocalReordering::moveStoresEarlierIfRhsAnchored(TR::Block* block,
                                                   TR::TreeTop* treeTop,
                                                   TR::Node* node,
                                                   TR::Node* parent,
                                                   vcount_t visitCount)
...
bool
TR_LocalReordering::insertEarlierIfPossible(TR::TreeTop* storeTree,
                                            TR::TreeTop* treeTop,
                                            bool strictCheck)
{
  ...
  while (currentTree != treeTop) {
    ...
    if (!stopHere) {
      if (currentNode->getOpCode()
            .isCheckCast()) // Moving a store earlier than a checkcast usually
                            // results in suboptimal type info (because of the
                            // way store constraints work, they only pick up the
                            // type info when the store was encountered)
        stopHere = true;
    }
...
//
void
TR_LocalReordering::setUseTreeForSymbolReferencesIn(TR::TreeTop* treeTop,
                                                    TR::Node* node,
                                                    vcount_t visitCount)
{
...
//
bool
TR_LocalReordering::containsBarriers(TR::Block* block)
{
  TR::TreeTop* currentTree = block->getEntry();
  TR::TreeTop* exitTree = block->getExit();
...
//
void
TR_LocalReordering::insertDefinitionBetween(TR::TreeTop* treeTop,
                                            TR::TreeTop* exitTree)
{
  if (treeTop == exitTree)
    return;
...
void
TR_LocalReordering::collectSymbolsUsedAndDefinedInNode(TR::Node* node,
                                                       vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
    return;
...
//
bool
TR_LocalReordering::isAnySymInDefinedOrUsedBy(TR::Node* node,
                                              vcount_t visitCount)
{
  if (visitCount == node->getVisitCount())
    return false;
...
//
bool
TR_LocalReordering::isAnySymInDefinedBy(TR::Node* node, vcount_t visitCount)
{
  if (visitCount == node->getVisitCount())
    return false;
...
//
bool
TR_LocalReordering::isSubtreeCommoned(TR::Node* node)
{
  if (node->getReferenceCount() > 1)
    return true;
