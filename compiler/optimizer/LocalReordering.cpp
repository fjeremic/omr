  ...
  TR::Block* block, TR::TreeTop* treeTop, TR::Node* node, TR::Node* parent, vcount_t visitCount)
{
  ...
  if ((numDeadChildren > 1) && (!parent || !parent->getOpCode().isCheck()) && !node->getOpCode().isTreeTop()
    && !node->getOpCode().isCall() && (node->getOpCodeValue() != TR::New) && (node->getOpCodeValue() != TR::newarray)
    && (node->getOpCodeValue() != TR::anewarray) && (node->getOpCodeValue() != TR::multianewarray)
    && (node->getOpCodeValue() != TR::MergeNew) && (node->getOpCodeValue() != TR::monent)
    && (node->getOpCodeValue() != TR::monexit) && !node->getOpCode().isArrayRef()
...
bool TR_LocalReordering::insertEarlierIfPossible(TR::TreeTop* storeTree, TR::TreeTop* treeTop, bool strictCheck)
{
  ...
  while (currentTree != treeTop) {
    ...
    if (!stopHere) {
      if (currentNode->getOpCode().isCheckCast()) // Moving a store earlier than a checkcast usually results in
                                                  // suboptimal type info (because of the way store constraints work,
                                                  // they only pick up the type info when the store was encountered)
        stopHere = true;
    }
...
void TR_LocalReordering::insertDefinitionBetween(TR::TreeTop* treeTop, TR::TreeTop* exitTree)
{
  ...
  if ((treeTop->getNextTreeTop()->getNode()->getOpCodeValue() == TR::dbgFence)
...
bool TR_LocalReordering::isAnySymInDefinedBy(TR::Node* node, vcount_t visitCount)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if ((!node->getOpCode().isLoadVar() || node->mightHaveVolatileSymbolReference()) && !node->getOpCode().isCheck()) {
      if (_seenSymbols->get(symRefNum))
        return true;
