...
TR_YesNoMaybe TR::VPClassType::isJavaLangClassObject()
{
    ...
    if (_len == 17 && strncmp(_sig, "Ljava/lang/Class;", 17) == 0)
        return TR_maybe;
    if ((_len == 18 && strncmp(_sig, "Ljava/lang/Object;", 18) == 0) || (_len == 22 && strncmp(_sig, "Ljava/io/Serializable;", 22) == 0) || (_len == 36 && strncmp(_sig, "Ljava/lang/reflect/AnnotatedElement;", 36) == 0) || (_len == 38 && strncmp(_sig, "Ljava/lang/reflect/GenericDeclaration;", 38) == 0) || (_len == 24 && strncmp(_sig, "Ljava/lang/reflect/Type;", 24) == 0))
        return TR_maybe;
    ...
    TR::VPPreexistentObject* preexistence, TR::VPArrayInfo* arrayInfo, TR::VPObjectLocation* location)
{
    ...
    // If the constraint does not already exist, create it
    //
    int32_t hash = (((int32_t)(intptrj_t)type) >> 2) + (((int32_t)(intptrj_t)presence) >> 2) + (((int32_t)(intptrj_t)preexistence) >> 2) + (((int32_t)(intptrj_t)arrayInfo) >> 2) + (((int32_t)(intptrj_t)location) >> 2);
    hash = ((uint32_t)hash) % VP_HASH_TABLE_SIZE;
    TR::VPClass* constraint;
    OMR::ValuePropagation::ConstraintsHashTableEntry* entry;
    for (entry = vp->_constraintsHashTable[hash]; entry; entry = entry->next) {
        constraint = entry->constraint->asClass();
        if (constraint && constraint->_type == type && constraint->_presence == presence && constraint->_preexistence == preexistence && constraint->_arrayInfo == arrayInfo && constraint->_location == location)
            return constraint;
    }
...
TR::VPClassType* TR::VPClassType::create(OMR::ValuePropagation* vp, const char* sig, int32_t len, TR_ResolvedMethod* method, bool isFixed, TR_OpaqueClassBlock* classObject)
{
    ...
    if (classObject) {
        ...
        TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(classObject, vp->comp(), allowForAOT);
...
TR::VPConstraint* TR::VPIntConstraint::merge1(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if (otherInt) {
        ...
        if (otherLong) {
            ...
            if (((int64_t)otherLong->getLow() < (int64_t)TR::getMinSigned<TR::Int32>()) || ((int64_t)otherLong->getHigh() > (int64_t)TR::getMaxSigned<TR::Int32>()))
                return NULL;
                ...
                if (lastResultEntry) {
                    ...
                    if (cur && (!otherCur || (int64_t)cur->getLow() <= (int64_t)otherCur->getLow())) {
                        ...
                        if (lastResult->getHigh() == TR::getMaxSigned<TR::Int32>() || (int64_t)otherCur->getLow() <= (int64_t)lastResult->getHigh() + 1) {
                            ...
                            if ((int64_t)otherCur->getLow() < (int64_t)TR::getMinSigned<TR::Int32>() || (int64_t)otherCur->getHigh() > (int64_t)TR::getMaxSigned<TR::Int32>())
                                return NULL;
                            else
...
TR::VPConstraint* TR::VPClass::intersect1(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if (thisClassObjectYes != otherClassObjectYes) {
        ...
        if (!type && _type && otherClass->_type) {
            if (TR::VPConstraint::isSpecialClass((uintptrj_t)_type->getClass()) || TR::VPConstraint::isSpecialClass((uintptrj_t)otherClass->_type->getClass()))
                ; // do nothing
        ...
        if (!type && _type && otherType) {
            if (TR::VPConstraint::isSpecialClass((uintptrj_t)_type->getClass()) || TR::VPConstraint::isSpecialClass((uintptrj_t)otherType->getClass()))
                ; // do nothing
...
TR::VPConstraint* TR::VPResolvedClass::intersect1(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if (otherRes) {
        ...
        if (*thisSig == *otherSig) {
            while (*thisSig == '[') {
                if (*otherSig != *thisSig) {
                    if (!((otherLen == 21 && !strncmp(otherSig, "Ljava/lang/Cloneable;", 21)) || (otherLen == 22 && !strncmp(otherSig, "Ljava/io/Serializable;", 22)) || (otherLen == 18 && !strncmp(otherSig, "Ljava/lang/Object;", 18))))
                        return NULL;
                    break;
                }
                thisSig++;
        ...
        // JLS Section 10.7: "Every array implements the interfaces Cloneable and java.io.Serializable"
            ...
            return this;
        else if (((thisLen == 21 && !strncmp(thisSig, "Ljava/lang/Cloneable;", 21)) || (thisLen == 22 && !strncmp(thisSig, "Ljava/io/Serializable;", 22))) && (*otherSig == '[')) {
            return other;
        }
        ...
        if (classObject != TR_maybe) {
            location = TR::VPObjectLocation::create(vp, classObject == TR_yes ? TR::VPObjectLocation::ClassObject : TR::VPObjectLocation::NotClassObject);
    ...
    if (otherClass) {
        ...
        if (isJavaLangObject(vp)) {
            if (*otherSig == '[') {
                ...
                if (!((otherLen == 21 && !strncmp(otherSig, "Ljava/lang/Cloneable;", 21)) || (otherLen == 22 && !strncmp(otherSig, "Ljava/io/Serializable;", 22)) || (otherLen == 18 && !strncmp(otherSig, "Ljava/lang/Object;", 18))))
                    return NULL;
            } else if (!(other->asUnresolvedClass()->isCloneableOrSerializable() || other->asUnresolvedClass()->isJavaLangObject(vp)))
                return NULL;
            //return other;
        }
...
TR::VPConstraint* TR::VPObjectLocation::intersect1(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if ((_kind == HeapObject && isKindSubset(otherInfo->_kind, ClassObject)) || (isKindSubset(_kind, ClassObject) && otherInfo->_kind == HeapObject))
        result = HeapObject;
...
TR::VPConstraint* TR::VPSync::intersect1(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if ((syncEmitted() == TR_maybe && otherSync->syncEmitted() == TR_yes) || (syncEmitted() == TR_yes && otherSync->syncEmitted() == TR_maybe))
        return TR::VPSync::create(vp, TR_no);

    if ((syncEmitted() == TR_maybe && otherSync->syncEmitted() == TR_no) || (syncEmitted() == TR_no && otherSync->syncEmitted() == TR_maybe))
...
bool TR::VPClass::mustBeEqual(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if (!_preexistence && !_arrayInfo && _type && _type->isFixedClass() && isNonNullObject() && other && otherClass && !otherClass->getArrayInfo() && !otherClass->isPreexistentObject() && otherClass->getClassType() && otherClass->getClassType()->isFixedClass() && otherClass->isNonNullObject() && (isClassObject() == TR_yes) && (other->isClassObject() == TR_yes)) {
...
bool TR::VPShortConstraint::mustBeNotEqual(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if (otherShort) {
        if (isUnsigned() && otherShort->isUnsigned())
            return (((uint16_t)getHigh() < (uint16_t)otherShort->getLow()) || ((uint16_t)getLow() > (uint16_t)otherShort->getHigh()));
        else
...
bool TR::VPIntConstraint::mustBeNotEqual(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if (otherInt) {
        if (isUnsigned() && otherInt->isUnsigned())
            return (((uint32_t)getHigh() < (uint32_t)otherInt->getLow()) || ((uint32_t)getLow() > (uint32_t)otherInt->getHigh()));
        else
