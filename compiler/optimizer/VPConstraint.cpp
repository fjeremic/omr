...
TR::VPConstraint* TR::VPShortConst::createExclusion(OMR::ValuePropagation* vp, int16_t v)
{
  ...
  return TR::VPMergedConstraints::create(vp, TR::VPShortRange::create(vp, TR::getMinSigned<TR::Int16>(), v - 1),
...
TR::VPConstraint* TR::VPIntConst::createExclusion(OMR::ValuePropagation* vp, int32_t v)
{
  ...
  return TR::VPMergedConstraints::create(vp, TR::VPIntRange::create(vp, TR::getMinSigned<TR::Int32>(), v - 1),
  ...
  TR::VPPreexistentObject* preexistence, TR::VPArrayInfo* arrayInfo, TR::VPObjectLocation* location)
{
  ...
  if (knot && location != NULL && location->isJavaLangClassObject() == TR_yes && type != NULL
    && type->asFixedClass() != NULL && type->asKnownObject() == NULL && !isSpecialClass((uintptrj_t)type->getClass())) {
  ...
  // If the constraint does not already exist, create it
  //
  int32_t hash = (((int32_t)(intptrj_t)type) >> 2) + (((int32_t)(intptrj_t)presence) >> 2)
    + (((int32_t)(intptrj_t)preexistence) >> 2) + (((int32_t)(intptrj_t)arrayInfo) >> 2)
  ...
  for (entry = vp->_constraintsHashTable[hash]; entry; entry = entry->next) {
    ...
    if (constraint && constraint->_type == type && constraint->_presence == presence
      && constraint->_preexistence == preexistence && constraint->_arrayInfo == arrayInfo
...
#ifdef J9_PROJECT_SPECIFIC
  {
    ...
    if (vpKnownObjectCriticalSection.hasVMAccess()) {
      TR_OpaqueClassBlock* clazz
        = TR::Compiler->cls.objectClass(vp->comp(), *vp->comp()->getKnownObjectTable()->getPointerLocation(index));
      ...
      if (isJavaLangClass) {
        ...
        clazz = TR::Compiler->cls.classFromJavaLangClass(
          vp->comp(), *vp->comp()->getKnownObjectTable()->getPointerLocation(index));
      } else {
    ...
    for (entry = vp->_constraintsHashTable[hash]; entry; entry = entry->next) {
      ...
      if (constraint) {
        uintptrj_t constraintStaticAddr
          = (uintptrj_t)constraint->_symRef->getSymbol()->castToStaticSymbol()->getStaticAddress();
      ...
      if ((int64_t)otherLong->getLow() <= (int64_t)(getHigh() + 1)) {
        ...
        if ((int64_t)otherLong->getLow() < (int64_t)getLow())
          return TR::VPMergedConstraints::create(
            vp, TR::VPIntRange::create(vp, (int32_t)otherLong->getLow(), (int32_t)otherLong->getHigh()), this);
        else
          return TR::VPMergedConstraints::create(
            vp, this, TR::VPIntRange::create(vp, (int32_t)otherLong->getLow(), (int32_t)otherLong->getHigh()));
      }
    }
  }
  return NULL;
  ...
  if (otherKnownObject) {
    ...
    if (getIndex()
      == knot->getExistingIndexAt(
        (uintptrj_t*)otherConstString->getSymRef()->getSymbol()->castToStaticSymbol()->getStaticAddress())) {
        ...
        if (lastResultEntry) {
          ...
          if (cur && (!otherCur || (int64_t)cur->getLow() <= (int64_t)otherCur->getLow())) {
            ...
            if ((int64_t)otherCur->getLow() < (int64_t)TR::getMinSigned<TR::Int32>()
              ...
              lastResultEntry
                = result.add(TR::VPIntRange::create(vp, (int32_t)otherCur->getLow(), (int32_t)otherCur->getHigh()));
            if (otherNext) {
              otherCur = otherNext->getData()->asLongConstraint();
              TR_ASSERT(otherCur, "Expecting int constraints in intMerge");
              otherNext = otherNext->getNextElement();
      ...
      while (cur || otherCur) {
        if (lastResultEntry) {
          ...
          if (cur && (!otherCur || (int64_t)cur->getLow() <= (int64_t)otherCur->getLow())) {
            ...
            lastResultEntry
              = result.add(TR::VPLongRange::create(vp, (int64_t)otherCur->getLow(), (int64_t)otherCur->getHigh()));
            if (otherNext) {
              otherCur = otherNext->getData()->asIntConstraint();
              // TR_ASSERT(otherCur, "Expecting long constraints in longMerge");
              otherNext = otherNext->getNextElement();
  ...
  if (otherRes) {
    ...
    if (classObject != TR_maybe) {
      location = TR::VPObjectLocation::create(
        vp, classObject == TR_yes ? TR::VPObjectLocation::ClassObject : TR::VPObjectLocation::NotClassObject);
...
TR::VPConstraint* TR::VPUnresolvedClass::intersect1(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
  ...
  if (other->asClassPresence()) {
    ...
    return TR::VPClass::create(
      vp, this, NULL, NULL, NULL, other->asObjectLocation()); // FIXME: unless rtresolve, should be on heap
  return this;
}
  ...
  int16_t low, int16_t high, bool lowCanOverflow, bool highCanOverflow, OMR::ValuePropagation* vp)
{
  ...
  // addition or subtraction operation. The function takes the results of the lower- and upper-bound calculations for the
  // operation, both performed in the same precision arithmetic as that of the underlying operation, along with two flags
  // indicated whether or not the operations producing these two bounds overflowed. From these values, a constraint can
  // be created, with the overflow flags being set as conservatively as possible.
  //
  ...
  if (lowCanOverflow && highCanOverflow) { // Both sides overflow
    ...
    if ((high ^ low) < 0)
      // high and low have different signs, thus high has overflowed above range, low has overflowed below range, result
      // is full range
      return NULL;

    // either both high and low have overflowed above range, or both have overflowed below range, result is guarunteed
    // to overflow every time
...
bool TR::VPIntConstraint::mustBeNotEqual(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
  ...
  if (otherInt) {
    if (isUnsigned() && otherInt->isUnsigned())
      return (
        ((uint32_t)getHigh() < (uint32_t)otherInt->getLow()) || ((uint32_t)getLow() > (uint32_t)otherInt->getHigh()));
    else
  ...
  TR::VPConstraint* constraint, int32_t relative, OMR::ValuePropagation* vp)
{
  ...
  if (constraint->asLongConst()) {
    ...
    if (excludedValue != TR::getMaxSigned<TR::Int64>()) {
      if (newConstraint)
        newConstraint
          = newConstraint->merge(TR::VPLongRange::create(vp, excludedValue + 1, TR::getMaxSigned<TR::Int64>()), vp);
      else
    ...
    if (excludedValue != TR::getMaxSigned<TR::Int32>()) {
      if (newConstraint)
        newConstraint
          = newConstraint->merge(TR::VPIntRange::create(vp, excludedValue + 1, TR::getMaxSigned<TR::Int32>()), vp);
      else
