...
TR::VPShortRange::createWithPrecision(OMR::ValuePropagation *vp,
                                      int32_t precision, bool isNonNegative) {
  int64_t lo, hi;
...
TR::VPIntConstraint *
TR::VPIntRange::createWithPrecision(OMR::ValuePropagation *vp, TR::DataType dt,
                                    int32_t precision, TR_YesNoMaybe isUnsigned,
                                    bool isNonNegative) {
...
#ifdef J9_PROJECT_SPECIFIC
  {
    ...
    if (vpKnownObjectCriticalSection.hasVMAccess()) {
      ...
      if (isJavaLangClass) {
        ...
        TR_ASSERT(clazz != jlClass, "For java/lang/Class instances, caller "
                                    "needs to use createForJavaLangClass.");
      }
      constraint = new (vp->trStackMemory())
...
TR::VPUnresolvedClass::create(OMR::ValuePropagation *vp, const char *sig,
                              int32_t len, TR_ResolvedMethod *method) {
  // If the constraint does not already exist, create it
  //
  int32_t hash =
...
TR::VPConstraint *TR::VPUnresolvedClass::intersect1(TR::VPConstraint *other,
  ...
  if (other->asClassPresence()) {
    ...
    return TR::VPClass::create(
        ...
        other
            ->asObjectLocation()); // FIXME: unless rtresolve, should be on heap
  return this;
}
