...
bool TR_LocalLiveRangeReduction::isWorthMoving(TR_TreeRefInfo* tree)
{
    ...
    if (((numLastRefNodesInt < numFirstRefNodesInt) && (numLastRefNodesFloat <= numFirstRefNodesFloat)) || ((numLastRefNodesFloat < numFirstRefNodesFloat) && (numLastRefNodesInt <= numFirstRefNodesInt)))
        return true;
...
TR_TreeRefInfo* TR_LocalLiveRangeReduction::findLocationToMove(TR_TreeRefInfo* treeToMove)
{
    ...
    for (; i < _numTreeTops; i++) {
        ...
        if (isAnyDataConstraint(currentTree, treeToMove) || isAnySymInDefinedOrUsedBy(currentTree, currentTree->getTreeTop()->getNode(), treeToMove) || (matchFirstOrMidToLastRef(currentTree, treeToMove)))
            return currentTree;
    }
...
{
    if ((node->getOpCodeValue() == TR::monent) || (node->getOpCodeValue() == TR::monexit) || (node->getOpCode().isStore() && node->getSymbol()->holdsMonitoredObject()))
        return true;
    return false;
}
...
bool TR_LocalLiveRangeReduction::isAnySymInDefinedOrUsedBy(TR_TreeRefInfo* currentTreeRefInfo, TR::Node* currentNode, TR_TreeRefInfo* movingTreeRefInfo)
{
    ...
    if (TR::Compiler->target.cpu.isPower() && opCode.getOpCodeValue() == TR::allocationFence) {
        ...
        if (movingNode->getOpCodeValue() == TR::treetop && movingNode->getFirstChild()->getOpCode().isNew() && (currentNode->getAllocation() == NULL || currentNode->getAllocation() == movingNode->getFirstChild())) {
            if (trace()) {
        ...
        if (movingNode->getOpCode().isStoreIndirect() || (movingNode->getOpCode().isStoreDirect() && !movingNode->getSymbol()->isParm() && !movingNode->getSymbol()->isAuto())) {
            if (trace())
    ...
    for (int32_t i = 0; i < currentNode->getNumChildren(); i++) {
        ...
        if (movingTreeRefInfo->getFirstRefNodesList()->find(child)) {
            ...
            if (child->exceptionsRaised() || (child->getOpCode().hasSymbolReference() && child->getSymbolReference()->isUnresolved())) {
                if (trace())
            ...
            else if (TR::Compiler->target.is64Bit() && movingNode->getOpCode().isBndCheck() && ((opCode.getOpCodeValue() == TR::i2l) || (opCode.getOpCodeValue() == TR::iu2l)) && !child->isNonNegative()) {
                if (trace())
...
bool TR_LocalLiveRangeReduction::isNeedToBeInvestigated(TR_TreeRefInfo* treeRefInfo)
{
    ...
    if (opCode.isBranch() || opCode.isReturn() || opCode.isGoto() || opCode.isJumpWithMultipleTargets() || opCode.getOpCodeValue() == TR::BBStart || opCode.getOpCodeValue() == TR::BBEnd)
        return false;
...
{
    if ((node->getOpCode().isCall() && (node->getReferenceCount() == 1 || treeRefInfo->getFirstRefNodesList()->find(node))) || node->getOpCode().isCheck()) {
        return true;
    }
