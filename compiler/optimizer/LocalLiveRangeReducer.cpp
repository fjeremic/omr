...
int32_t TR_LocalLiveRangeReduction::perform()
{
  ...
  for (tt = comp()->getStartTree(); tt; tt = nextTT) {
    ...
    while (
      (nextTT = exitTT->getNextTreeTop()) && (b = nextTT->getNode()->getBlock(), b->isExtensionOfPreviousBlock())) {
  ...
  TR_TreeRefInfo* currentTreeRefInfo, TR::Node* currentNode, TR_TreeRefInfo* movingTreeRefInfo)
{
  ...
  for (int32_t i = 0; i < currentNode->getNumChildren(); i++) {
    ...
    if (movingTreeRefInfo->getFirstRefNodesList()->find(child)) {
      ...
      else if (TR::Compiler->target.is64Bit() && movingNode->getOpCode().isBndCheck()
        && ((opCode.getOpCodeValue() == TR::i2l) || (opCode.getOpCodeValue() == TR::iu2l)) && !child->isNonNegative()) {
        if (trace())
          traceMsg(comp(), "cannot move %p beyond %p - changing the eval point of %p will casue extra cg instruction ",
