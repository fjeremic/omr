...
TR_LocalLiveRangeReduction::TR_LocalLiveRangeReduction(
...
void
TR_LocalLiveRangeReduction::prePerformOnBlocks()
{
  comp()->incVisitCount();
...
void
TR_LocalLiveRangeReduction::postPerformOnBlocks()
{
  comp()->incVisitCount();
}

int32_t
TR_LocalLiveRangeReduction::perform()
{
  ...
  for (tt = comp()->getStartTree(); tt; tt = nextTT) {
    ...
    exitTT = b->getExit();
    _numTreeTops =
      b->getNumberOfRealTreeTops() + 2; // include both BBStart/BBend

    // support for extended blocks
    while (
      (nextTT = exitTT->getNextTreeTop()) &&
    ...
    _treesRefInfoArray = (TR_TreeRefInfo**)trMemory()->allocateStackMemory(
      _numTreeTops * sizeof(TR_TreeRefInfo*));
...
// exitTree is the next treeTop after the relevant range of TreeTops (start of
// next block)
bool
TR_LocalLiveRangeReduction::transformExtendedBlock(TR::TreeTop* entryTree,
                                                   TR::TreeTop* exitTree)
{
  ...
  if (!performTransformation(comp(),
                             "%sBlock %d\n",
                             OPT_DETAILS,
                             entryTree->getNode()->getBlock()->getNumber()))
    return false;
  ...
  // Note that since the list adds elements to the head, we can iterate the
  // list, and by this to cover chain of moves (the later one will be in front)
  // e.g. if a blocked by b and then b moved after c. The list will be b-c;a-b

  ListIterator<DepPair> listIt(&_depPairList);
  for (DepPair* depPair = listIt.getFirst(); depPair != NULL;
       depPair = listIt.getNext()) {
...
//-----------------------------------------
void
TR_LocalLiveRangeReduction::collectInfo(TR::TreeTop* entryTree,
                                        TR::TreeTop* exitTree)
{
  ...
  while (!(currentTree == exitTree)) {
    ...
    collectRefInfo(
      treeRefInfo, currentTree->getNode(), visitCount, &maxRefCount);
...
void
TR_LocalLiveRangeReduction::collectRefInfo(TR_TreeRefInfo* treeRefInfo,
                                           TR::Node* node,
                                           vcount_t compVisitCount,
                                           int32_t* maxRefCount)
{
...
bool
TR_LocalLiveRangeReduction::investigateAndMove(TR_TreeRefInfo* treeRefInfo,
                                               int32_t passNumber)
{
  ...
  if (isWorthMoving(treeRefInfo)) // worth moving
  {
    ...
    if (!moveTreeBefore(
          treeRefInfo, anchor, passNumber)) // Move tree before anchor
      return false;
...
//
bool
TR_LocalLiveRangeReduction::isWorthMoving(TR_TreeRefInfo* tree)
{
...
TR_TreeRefInfo*
TR_LocalLiveRangeReduction::findLocationToMove(TR_TreeRefInfo* treeToMove)
{
  ...
  for (; i < _numTreeTops; i++) {
    ...
    if (opCode.isBranch() || opCode.isReturn() || opCode.isGoto() ||
        opCode.isJumpWithMultipleTargets())
      return currentTree;
    ...
    if (isAnyDataConstraint(currentTree, treeToMove) ||
        isAnySymInDefinedOrUsedBy(
          currentTree, currentTree->getTreeTop()->getNode(), treeToMove) ||
...
void
TR_LocalLiveRangeReduction::initPotentialDeps(TR_TreeRefInfo* tree)
{
  int32_t symRefCount = comp()->getSymRefCount();
  if (tree->getDefSym() == NULL)
    tree->setDefSym(new (trStackMemory())
                      TR_BitVector(symRefCount, trMemory(), stackAlloc));
  if (tree->getUseSym() == NULL)
    tree->setUseSym(new (trStackMemory())
                      TR_BitVector(symRefCount, trMemory(), stackAlloc));
}

void
TR_LocalLiveRangeReduction::populatePotentialDeps(TR_TreeRefInfo* treeRefInfo,
                                                  TR::Node* node)
{
  TR::ILOpCode& opCode = node->getOpCode();
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (opCode.isCall() || opCode.isResolveCheck() || opCode.isStore() ||
      ...
      if (!symRef->getUseDefAliases(isCallDirect).isZero(comp())) {
        ...
        for (aliasCursor.SetToFirstOne(); aliasCursor.Valid();
             aliasCursor.SetToNextOne()) {
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    ...
    if (child->getReferenceCount() == 1 ||
        treeRefInfo->getFirstRefNodesList()->find(child))
...
bool
TR_LocalLiveRangeReduction::isAnyDataConstraint(
  TR_TreeRefInfo* currentTreeRefInfo,
  TR_TreeRefInfo* movingTreeRefInfo)
{
  // check if there is a symbol used or written by the moving tree and written
  // by the current
...
static bool
mayBeObjectHeaderStore(TR::Node* node, TR_FrontEnd* fe)
{
  node = node->getStoreNode();
  TR_ASSERT(fe->getObjectHeaderSizeInBytes() <=
              TR::Compiler->om.contiguousArrayHeaderSizeInBytes(),
            "assertion failure");
  if (node && node->getSymbol()->getOffset() <
                TR::Compiler->om.contiguousArrayHeaderSizeInBytes())
    return true;
  else
    return false;
...
bool
TR_LocalLiveRangeReduction::isAnySymInDefinedOrUsedBy(
  TR_TreeRefInfo* currentTreeRefInfo,
  TR::Node* currentNode,
  TR_TreeRefInfo* movingTreeRefInfo)
{
  ...
  if (nodeMaybeMonitor(currentNode)) {
    if (trace())
      traceMsg(
        comp(), "cannot move %p beyond monitor %p\n", movingNode, currentNode);
    return true;
  }
  ...
  if (movingNode->canGCandReturn() || currentNode->canGCandReturn()) {
    if (trace())
      traceMsg(
        comp(), "cannot move gc points %p past %p\n", movingNode, currentNode);
    return true;
  }
  ...
  if ((currentNode->getOpCode().isWrtBar() || currentNode->canCauseGC()) &&
    ...
    if (trace())
      traceMsg(comp(),
               "cannot move possible object header store %s past GC point %s\n",
               getDebug()->getName(movingNode),
               getDebug()->getName(currentNode));
    return true;
  }
  ...
  if (TR::Compiler->target.cpu.isPower() &&
    ...
    if (movingNode->getOpCodeValue() == TR::treetop &&
      ...
      if (trace()) {
        traceMsg(
          comp(), "cannot move %p beyond flush %p - ", movingNode, currentNode);
        if (currentNode->getAllocation() == NULL)
          traceMsg(comp(), "(flush with null allocation)\n");
        else
          traceMsg(comp(),
                   "(flush for allocation %p)\n",
    ...
    // Exclude all indirect stores, they may be for stack allocs, in which case
    // the flush is needed at least as a scheduling barrier Direct stores to
    // autos and parms are the only safe candidates
    if (movingNode->getOpCode().isStoreIndirect() ||
        (movingNode->getOpCode().isStoreDirect() &&
         !movingNode->getSymbol()->isParm() &&
         !movingNode->getSymbol()->isAuto())) {
      if (trace())
        traceMsg(
          comp(),
          "cannot move %p beyond flush %p - (flush for possible stack alloc)",
          movingNode,
          currentNode);
      return true;
    }
  ...
  for (int32_t i = 0; i < currentNode->getNumChildren(); i++) {
    ...
    // Any node that has side effects (like call and newarrya) cannot be
    // evaluated in the middle of the tree.
    if (movingTreeRefInfo->getFirstRefNodesList()->find(child)) {
      ...
      if (child->exceptionsRaised() ||
        ...
        if (trace())
          traceMsg(comp(),
                   "cannot move %p beyond %p - cannot change evaluation point "
                   "of %p\n ",
                   movingNode,
                   currentTreeRefInfo->getTreeTop()->getNode(),
                   child);
        return true;
      }
        ...
        // TR::SymbolReference *stSymRef = movingNode->getSymbolReference();
        int32_t numHelperSymbols =
          comp()->getSymRefTab()->getNumHelperSymbols();
        if ((comp()->getSymRefTab()->isNonHelper(
              stSymRefNum, TR::SymbolReferenceTable::vftSymbol)) ||
            ...
            (stSymRef->getSymbol() ==
             comp()->getSymRefTab()->findGenericIntShadowSymbol()))
        ...
        if (trace())
          traceMsg(
            comp(),
            "cannot move %p beyond %p - node %p under ResolveOrNullCheck",
            movingNode,
            currentTreeRefInfo->getTreeTop()->getNode(),
            currentNode);
        return true;
               ...
               ((opCode.getOpCodeValue() == TR::i2l) ||
                (opCode.getOpCodeValue() == TR::iu2l)) &&
               !child->isNonNegative()) {
        if (trace())
          traceMsg(comp(),
                   "cannot move %p beyond %p - changing the eval point of %p "
                   "will casue extra cg instruction ",
                   movingNode,
                   currentTreeRefInfo->getTreeTop()->getNode(),
                   currentNode);
        return true;
    ...
    if (child->getReferenceCount() == 1 ||
        currentTreeRefInfo->getFirstRefNodesList()->find(child)) {
      if (isAnySymInDefinedOrUsedBy(
            currentTreeRefInfo, child, movingTreeRefInfo))
        return true;
    }
  }
...
bool
TR_LocalLiveRangeReduction::moveTreeBefore(TR_TreeRefInfo* treeToMove,
                                           TR_TreeRefInfo* anchor,
                                           int32_t passNumber)
{
  ...
  if (!performTransformation(comp(),
                             "%sPass %d: moving tree [%p] before Tree %p\n",
                             OPT_DETAILS,
                             passNumber,
                             treeToMoveTT->getNode(),
                             anchorTT->getNode()))
    return false;
  ...
  for (i = startIndex + 1; i <= endIndex; i++) {
    ...
    updateRefInfo(treeToMove->getTreeTop()->getNode(),
                  currentTreeRefInfo,
                  treeToMove,
                  false);
    treeToMove->resetSyms();
    currentTreeRefInfo->resetSyms();
    populatePotentialDeps(currentTreeRefInfo,
                          currentTreeRefInfo->getTreeTop()->getNode());
...
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
  if (!(comp()->getOption(TR_EnableParanoidOptCheck) ||
  ...
  // verifier
  {
    ...
    TR_TreeRefInfo** treesRefInfoArrayTemp =
      (TR_TreeRefInfo**)trMemory()->allocateStackMemory(
        _numTreeTops * sizeof(TR_TreeRefInfo*));
    ...
    for (int32_t i = 0; i < _numTreeTops - 1; i++) {
      if (!verifyRefInfo(treesRefInfoArrayTemp[i]->getFirstRefNodesList(),
        ...
        TR_ASSERT(0,
                  "fail to verify firstRefNodesList for %p\n",
      ...
      if (!verifyRefInfo(treesRefInfoArrayTemp[i]->getMidRefNodesList(),
        ...
        printOnVerifyError(_treesRefInfoArray[i], treesRefInfoArrayTemp[i]);
        TR_ASSERT(0,
                  "fail to verify midRefNodesList for %p\n",
                  _treesRefInfoArray[i]->getTreeTop()->getNode());
      }
      ...
      if (!verifyRefInfo(treesRefInfoArrayTemp[i]->getLastRefNodesList(),
        ...
        printOnVerifyError(_treesRefInfoArray[i], treesRefInfoArrayTemp[i]);
        TR_ASSERT(0,
                  "fail to verify lastRefNodesList for %p\n",
                  _treesRefInfoArray[i]->getTreeTop()->getNode());
      }
    }
...
void
TR_LocalLiveRangeReduction::addDepPair(TR_TreeRefInfo* dep,
                                       TR_TreeRefInfo* anchor)
{
...
void
TR_LocalLiveRangeReduction::updateDepList()
{
  // at this point we are sure the list is not empty, cause we know some trees
  // as moved.
...
bool
TR_LocalLiveRangeReduction::isNeedToBeInvestigated(TR_TreeRefInfo* treeRefInfo)
{
  ...
  if (opCode.isBranch() || opCode.isReturn() || opCode.isGoto() ||
      opCode.isJumpWithMultipleTargets() ||
      opCode.getOpCodeValue() == TR::BBStart ||
...
bool
TR_LocalLiveRangeReduction::containsCallOrCheck(TR_TreeRefInfo* treeRefInfo,
                                                TR::Node* node)
{
  if ((node->getOpCode().isCall() &&
       (node->getReferenceCount() == 1 ||
        treeRefInfo->getFirstRefNodesList()->find(node))) ||
...
int32_t
TR_LocalLiveRangeReduction::getIndexInArray(TR_TreeRefInfo* treeRefInfo)
{
...
// If there are several nodes with "first reference" in the tree to move, stop
// before the first tree which contain a loast reference to one of those nodes.
bool
TR_LocalLiveRangeReduction::matchFirstOrMidToLastRef(TR_TreeRefInfo* tree,
                                                     TR_TreeRefInfo* treeToMove)
{
  ListIterator<TR::Node> listIt(tree->getLastRefNodesList());
  for (TR::Node* node = listIt.getFirst(); node != NULL;
       node = listIt.getNext()) {
    TR::Node* node1;
...
void
TR_LocalLiveRangeReduction::updateRefInfo(TR::Node* n,
                                          TR_TreeRefInfo* currentTreeRefInfo,
                                          TR_TreeRefInfo* movingTreeRefInfo,
                                          bool underEval)
{
  ...
  if (M_firstList->find(n)) {
    ...
    if (C_firstList->find(n) || C_midList->find(n) || C_lastList->find(n)) {
      ...
      if (C_lastList->find(n) &&
          ((underEval && M_midList->find(n)) || !underEval)) {
        M_midList->remove(n);
        M_lastList->add(n);
        C_lastList->remove(n);
        C_midList->add(n);
...
void
TR_LocalLiveRangeReduction::printRefInfo(TR_TreeRefInfo* treeRefInfo)
{
  if (trace()) {
    TR::Node* n;
...
bool
TR_LocalLiveRangeReduction::verifyRefInfo(List<TR::Node>* verifier,
                                          List<TR::Node>* refList)
{
  ...
  for (node = listIt.getFirst(); node != NULL; node = listIt.getNext()) {
    if (verifier->find(node))
      ...
      if (trace())
        traceMsg(
          comp(),
          "LocalLiveRangeReduction:node %p should not have beed in the List\n",
          node);
      return false;
    }
  }
  ...
  if (!verifier->isEmpty()) {
    if (trace())
      traceMsg(comp(),
               "LocalLiveRangeReduction: there are nodes that should have been "
               "in the List\n");
    return false;
  }
  return true;
...
void
TR_LocalLiveRangeReduction::printOnVerifyError(TR_TreeRefInfo* optRefInfo,
                                               TR_TreeRefInfo* verifier)
{
  if (trace()) {
