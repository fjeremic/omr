...
int32_t TR_LocalLiveRangeReduction::perform() {
  if (TR::Compiler->target.cpu.isZ()) return false;
...
// next block)
bool TR_LocalLiveRangeReduction::transformExtendedBlock(TR::TreeTop *entryTree,
                                                        TR::TreeTop *exitTree) {
  ...
  if (_movedTreesList.isEmpty()) return true;
  ...
  if (_depPairList.isEmpty()) return true;
  ...
  for (DepPair *depPair = listIt.getFirst(); depPair != NULL;
    ...
    if (isNeedToBeInvestigated(treeToMove)) investigateAndMove(treeToMove, 2);
  }
...
//-----------------------------------------
void TR_LocalLiveRangeReduction::collectInfo(TR::TreeTop *entryTree,
                                             TR::TreeTop *exitTree) {
  ...
  while (!(currentTree == exitTree)) {
    ...
    collectRefInfo(treeRefInfo, currentTree->getNode(), visitCount,
                   &maxRefCount);
...
void TR_LocalLiveRangeReduction::collectRefInfo(TR_TreeRefInfo *treeRefInfo,
                                                TR::Node *node,
...
bool TR_LocalLiveRangeReduction::investigateAndMove(TR_TreeRefInfo *treeRefInfo,
                                                    int32_t passNumber) {
  if (isWorthMoving(treeRefInfo))  // worth moving
  {
    TR_TreeRefInfo *anchor = findLocationToMove(treeRefInfo);
    if (!moveTreeBefore(treeRefInfo, anchor,
                        passNumber))  // Move tree before anchor
      return false;
...
TR_TreeRefInfo *TR_LocalLiveRangeReduction::findLocationToMove(
  ...
  for (; i < _numTreeTops; i++) {
    ...
    if (opCode.getOpCodeValue() == TR::BBEnd) return currentTree;
...
void TR_LocalLiveRangeReduction::populatePotentialDeps(
    TR_TreeRefInfo *treeRefInfo, TR::Node *node) {
  TR::ILOpCode &opCode = node->getOpCode();
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (opCode.isCall() || opCode.isResolveCheck() || opCode.isStore() ||
      ...
      if (opCode.isStore()) treeRefInfo->getDefSym()->set(symRefNum);
    }
...
bool TR_LocalLiveRangeReduction::isAnySymInDefinedOrUsedBy(
    TR_TreeRefInfo *currentTreeRefInfo, TR::Node *currentNode,
  ...
  if (TR::Compiler->target.cpu.isPower() &&
    ...
    if (movingNode->getOpCodeValue() == TR::treetop &&
      ...
      if (trace()) {
        traceMsg(comp(), "cannot move %p beyond flush %p - ", movingNode,
                 currentNode);
  ...
  for (int32_t i = 0; i < currentNode->getNumChildren(); i++) {
      ...
      if (child->exceptionsRaised() ||
        ...
        if (trace())
          traceMsg(comp(),
                   ...
                   "of %p\n ",
                   movingNode, currentTreeRefInfo->getTreeTop()->getNode(),
                   child);
        return true;
      }
    ...
    if (child->getReferenceCount() == 1 ||
      ...
      if (isAnySymInDefinedOrUsedBy(currentTreeRefInfo, child,
                                    movingTreeRefInfo))
        return true;
    }
  }
...
bool TR_LocalLiveRangeReduction::moveTreeBefore(TR_TreeRefInfo *treeToMove,
  ...
  if (!performTransformation(
          comp(), "%sPass %d: moving tree [%p] before Tree %p\n", OPT_DETAILS,
          passNumber, treeToMoveTT->getNode(), anchorTT->getNode()))
    return false;
  ...
  for (i = startIndex + 1; i <= endIndex; i++) {
    ...
    updateRefInfo(treeToMove->getTreeTop()->getNode(), currentTreeRefInfo,
                  treeToMove, false);
...
void TR_LocalLiveRangeReduction::addDepPair(TR_TreeRefInfo *dep,
                                            TR_TreeRefInfo *anchor) {
...
void TR_LocalLiveRangeReduction::updateDepList() {
  ...
  if (_depPairList.isEmpty()) return;
...
bool TR_LocalLiveRangeReduction::isNeedToBeInvestigated(
  ...
  if (node->getOpCode().isStoreReg()) return false;
  /*******************************************************************/
  if (_movedTreesList.find(treeRefInfo)) return false;

  if (treeRefInfo->getFirstRefNodesList()->getSize() != 0) return true;

  return false;
}

// returns true if there is first reference of a call or check
bool TR_LocalLiveRangeReduction::containsCallOrCheck(
    TR_TreeRefInfo *treeRefInfo, TR::Node *node) {
...
int32_t TR_LocalLiveRangeReduction::getIndexInArray(
    TR_TreeRefInfo *treeRefInfo) {
  for (int32_t i = _numTreeTops - 1; i >= 0; i--) {
    if (treeRefInfo == _treesRefInfoArray[i]) return i;
  }
  return -1;
}
...
bool TR_LocalLiveRangeReduction::matchFirstOrMidToLastRef(
    TR_TreeRefInfo *tree, TR_TreeRefInfo *treeToMove) {
...
void TR_LocalLiveRangeReduction::updateRefInfo(
    TR::Node *n, TR_TreeRefInfo *currentTreeRefInfo,
  ...
  if (n->getReferenceCount() <= 1) needToRecurse = true;
...
bool TR_LocalLiveRangeReduction::verifyRefInfo(List<TR::Node> *verifier,
  ...
  for (node = listIt.getFirst(); node != NULL; node = listIt.getNext()) {
    if (verifier->find(node))
      ...
      if (trace())
        traceMsg(comp(),
                 "LocalLiveRangeReduction:node %p should not have beed in the "
                 "List\n",
                 node);
      return false;
    }
...
void TR_LocalLiveRangeReduction::printOnVerifyError(TR_TreeRefInfo *optRefInfo,
...
const char *TR_LocalLiveRangeReduction::optDetailString() const throw() {
