...
TR_InlinerBase::TR_InlinerBase(TR::Optimizer *optimizer,
    ...
    : TR_HasRandomGenerator(optimizer->comp()),
      _visitCount(optimizer->comp()->incVisitCount()), _inliningAsWeWalk(false),
      _optimizer(optimizer), _trMemory(optimizer->comp()->trMemory()),
      _availableTemps(_trMemory), _availableBasicBlockTemps(_trMemory),
      _storeToCachedPrivateStatic(NULL), _disableTailRecursion(false),
      _currentNumberOfNodes(optimizer->comp()->getAccurateNodeCount()),
      _disableInnerPrex(false), _aggressivelyInlineInLoops(false) {
...
void TR_InlinerBase::linkOSRCodeBlocks() {
  ...
  for (intptrj_t i = 0; i < methodDataArray.size(); ++i) {
    ...
    TR_ASSERT(lastNode->getOpCode().isGoto() ||
                  lastNode->getOpCode().isReturn(),
              "last treetop of a non-top-level caller OSR code block is "
              "neither a goto nor a return\n");
...
TR_CallStack::TR_CallStack(TR::Compilation *c,
    ...
    : TR_Link<TR_CallStack>(nextCallStack), _comp(c), _trMemory(c->trMemory()),
      _methodSymbol(methodSymbol), _method(method), _currentCallNode(NULL),
      ...
      _autos(c->trMemory()), _temps(c->trMemory()),
      _injectedBasicBlockTemps(c->trMemory()), _inlineFilters(NULL),
      _blockInfo(0), _safeToAddSymRefs(safeToAddSymRefs) {
  TR_FilterBST *filterInfo = NULL;
  TR::CompilationFilters *inlineFilters = NULL;
...
#if !defined(AIXPPC)
  TR_ASSERT(std::uncaught_exception() ||
                (_autos.isEmpty() && _temps.isEmpty() &&
...
void TR_CallStack::addAutomatic(TR::AutomaticSymbol *a) {
  TR_ASSERT(_safeToAddSymRefs, "make sure a constructed callStack is committed "
                               "then add _safeToAddSymRefs to your c-tor call");
  _autos.add(a);
}
...
TR_DumbInliner::TR_DumbInliner(TR::Optimizer *optimizer,
    ...
    : TR_InlinerBase(optimizer, optimization), _initialSize(initialSize),
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if ((byteCodeSize > maxBCSize)) {
      if (tryToInline("overriding getMaxBytecodeIndex check", calltarget)) {
        ...
        if (comp()->trace(OMR::inlining))
          traceMsg(comp(), "inliner: overriding getMaxBytecodeIndex check "
                           "because it's always worth inlining\n");
      } else {
        if (comp()->trace(OMR::inlining))
          traceMsg(comp(),
...
static TR::Node *
findPotentialDecompilationPoint(TR::ResolvedMethodSymbol *calleeSymbol,
                                TR::Compilation *comp) {
...
TR::Node *TR_InlinerBase::createVirtualGuard(
  ...
  case TR_ProfiledGuard:
    if (guard->isHighProbablityProfiledGuard())
      fidelity = TR::DebugCounter::Free; // The taken path is marked isCold, so
                                         // it had better never happen
    else
...
TR_ParameterToArgumentMapper::TR_ParameterToArgumentMapper(
    ...
    : _callerSymbol(callerSymbol), _calleeSymbol(calleeSymbol),
      _callNode(callNode), _argInfo(argInfo), _tempList(temps),
      _availableTemps(availableTemps), _availableTemps2(availableTemps2),
      _vftReplacementSymRef(0), _lastTempTreeTop(0), _firstTempTreeTop(0),
      _inliner(inliner) {}
      ...
      if (parmMap->_isConst) {
        ...
        if (parmMap->_replacementSymRef)
          ...
          if (!disableUseKnownObjectTempsForParms && prexArgument &&
              !parmMap->_parmIsModified &&
              TR_PrexArgument::knowledgeLevel(prexArgument) == KNOWN_OBJECT &&
              prexArgument->isTypeInfoForInlinedBody()) // use known object temp
                                                        // in inlined body only
          {
...
TR_TransformInlinedFunction::TR_TransformInlinedFunction(
    ...
    : _comp(c), _tracer(tracer), _callerSymbol(callerSymbol),
      _calleeSymbol(calleeSymbol), _callNodeTreeTop(callNodeTreeTop),
      _callNode(callNode), _tempList(temps), _availableTemps(availableTemps),
      _availableTemps2(availableTemps2), _parameterMapper(mapper),
      _resultNode(0), _resultTempSymRef(0), _firstBBEnd(0), _firstCatchBlock(0),
      _crossedBasicBlock(guard->_kind != TR_NoGuard), _generatedLastBlock(0),
      _determineIfReturnCanBeReplacedWithCallNodeReference(true),
      _simpleCallReferenceTreeTop(0), _processingExceptionHandlers(false),
      _treeTopsToRemove(c->trMemory()), _blocksWithEdgesToTheEnd(c->trMemory()),
      _traceVIP(c->getOption(TR_TraceVIP)), _favourVftCompare(false),
...
TR_HandleInjectedBasicBlock::TR_HandleInjectedBasicBlock(
    ...
    : _comp(comp), _tracer(tracer), _tempList(temps),
      _injectedBasicBlockTemps(injectedBasicBlockTemps),
      _availableTemps(availableTemps), _methodSymbol(methodSymbol),
      _mappings(mappings) {
...
TR_HandleInjectedBasicBlock::MultiplyReferencedNode::MultiplyReferencedNode(
    ...
    : _node(node), _treeTop(tt), _referencesToBeFound(refsToBeFound),
      _symbolCanBeReloaded(symCanBeReloaded), _replacementSymRef(0),
      _isConst(false) {}
        ...
        if (valueChild->isInternalPointer() &&
          ...
          if (valueChild->getOpCode().isLoadVarDirect() &&
            ...
            if (!newValueArrayRef->getSymbol()
                     ->isParm()) // newValueArrayRef could contain a parm symbol
            {
...
static TR::TreeTop *
cloneAndReplaceCallNodeReference(TR::TreeTop *callNodeReference,
                                 TR::Node *callNode, TR::Node *replacementNode,
                                 TR::TreeTop *insertAfterTreeTop,
                                 TR::Compilation *comp) {
...
TR_ResolvedMethod *
TR_IndirectCallSite::getResolvedMethod(TR_OpaqueClassBlock *klass) {
  return _callerResolvedMethod->getResolvedVirtualMethod(comp(), klass,
                                                         _vftSlot);
}

TR_ResolvedMethod *
TR_IndirectCallSite::findSingleJittedImplementer(TR_InlinerBase *inliner) {
...
bool TR_IndirectCallSite::addTargetIfMethodIsNotOverriden(
  ...
  if (_initialCalleeMethod &&
    ...
    if (comp()->compileRelocatableCode() &&
      ...
      return false; // CHTable is not used when recompilation is disabled, hence
                    // we cannot use assumptions for AOT
...
TR_OpaqueClassBlock *
TR_IndirectCallSite::extractAndLogClassArgument(TR_InlinerBase *inliner) {
...
TR_CallSite *TR_InlinerBase::findAndUpdateCallSiteInGraph(
  ...
  if (!callNode->getOpCode().isCallIndirect()) {
    ...
    if (callsite->numTargets() > 0) {
      if (callsite
              ->isIndirectCall()) // local opts pre-existence has been performed
      {
        if (comp()->compileRelocatableCode())
    ...
    if (callsite->getTarget(i)->_guard->_kind == TR_InterfaceGuard &&
      ...
      if (tracer()->debugLevel())
        tracer()->dumpCallSite(
            ...
            "initialCalleeSYmbol %p for calNode %p",
            callsite->_initialCalleeSymbol, callNode);
      TR_ASSERT(0, " target has an interface guard AND an initial callee "
                   "symbol.. (bad!)\n");
    }
  }
...
bool TR_InlinerBase::inlineCallTarget2(TR_CallStack *callStack,
  ...
  int32_t numberOfNodesBefore =
      comp()->getNodeCount(); // want to know the number of nodes created before
                              // and after genIL
  ...
  if (argInfo) {
    if (comp()->usesPreexistence()) // Mark the preexistent arguments (maybe
                                    // redundant if VP has already done that and
                                    // provided the info in argInfo)
    {
  ...
  */
  ...
  if (tif->crossedBasicBlock()) {
    debugTrace(tracer(), "Inlining caused a basic block to be crossed.  Need "
                         "to fix broken commoning");
...
TR_CallTarget::TR_CallTarget(TR_CallSite *callsite,
    ...
    : _myCallSite(callsite), _calleeSymbol(calleeSymbol),
      _calleeMethod(calleeMethod), _guard(guard), _receiverClass(receiverClass),
      _frequencyAdjustment(freqAdj), _prexArgInfo(NULL),
...
TR_CallSite::TR_CallSite(TR_ResolvedMethod *callerResolvedMethod,
    ...
    : _callerResolvedMethod(callerResolvedMethod),
      _callNodeTreeTop(callNodeTreeTop), _cursorTreeTop(NULL), _parent(parent),
      _callNode(callNode), _interfaceMethod(interfaceMethod),
      _receiverClass(receiverClass), _vftSlot(vftSlot), _cpIndex(cpIndex),
      _initialCalleeMethod(initialCalleeMethod),
      _initialCalleeSymbol(initialCalleeSymbol),
      _isIndirectCall(isIndirectCall), _isInterface(isInterface),
      _bcInfo(bcInfo), _unavailableTemps(comp->trMemory()),
      _unavailableBlockTemps(comp->trMemory()), _comp(comp), _depth(depth),
      _forceInline(false), _isBackEdge(false), _allConsts(allConsts),
      _mytargets(0, comp->allocator()), _myRemovedTargets(0, comp->allocator()),
      _ecsPrexArgInfo(0) {
  _visitCount = 0;
  _failureReason = InlineableTarget;
...
const char *
TR_InlinerTracer::getGuardKindString(TR_VirtualGuardSelection *guard) {
  if (comp()->getDebug())
...
const char *
TR_InlinerTracer::getGuardTypeString(TR_VirtualGuardSelection *guard) {
  if (comp()->getDebug())
...
TR_InnerPreexistenceInfo::TR_InnerPreexistenceInfo(
    ...
    TR_VirtualGuardKind guardKind)
    : _comp(c), _trMemory(c->trMemory()), _methodSymbol(methodSymbol),
      _callStack(callStack), _callTree(treeTop), _callNode(callNode),
      _guardKind(guardKind), _assumptions(c->trMemory()) {}
