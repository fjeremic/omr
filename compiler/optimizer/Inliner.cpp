...
TR::DataType getStoreType(TR::Node* store,
                          TR::Symbol* addrSymbol,
                          TR::Compilation* comp) {
...
static TR::TreeTop* cloneAndReplaceCallNodeReference(TR::TreeTop*,
                                                     TR::Node*,
                                                     TR::Node*,
                                                     TR::TreeTop*,
                                                     TR::Compilation*);
...
void TR_InlinerBase::cloneChildren(TR::Node* to,
                                   TR::Node* from,
...
void TR_InlinerBase::replaceCallNodeReferences(
    ...
    rcount_t& numberOfReferencesToFind,
...
void TR_InlinerBase::addAdditionalGuard(TR::Node* callNode,
                                        TR::ResolvedMethodSymbol* calleeSymbol,
                                        TR_OpaqueClassBlock* thisClass,
                                        TR::Block* prevBlock,
                                        TR::Block* inlinedBody,
                                        TR::Block* slowPath,
                                        TR_VirtualGuardKind kind,
                                        TR_VirtualGuardTestType type,
                                        bool favourVFTCompare,
                                        TR::CFG* callerCFG) {
  TR::Block* guardBlock =
...
TR_TransformInlinedFunction::TR_TransformInlinedFunction(
    ...
    TR::ResolvedMethodSymbol* callerSymbol,
    TR::ResolvedMethodSymbol* calleeSymbol,
    TR::Block* callNodeBlock,
    TR::TreeTop* callNodeTreeTop,
    TR::Node* callNode,
    TR_ParameterToArgumentMapper& mapper,
    TR_VirtualGuardSelection* guard,
    List<TR::SymbolReference>& temps,
    List<TR::SymbolReference>& availableTemps,
    List<TR::SymbolReference>& availableTemps2)
...
void TR_HandleInjectedBasicBlock::findAndReplaceReferences(
    TR::TreeTop* callBBStart,
    TR::Block* replaceBlock1,
...
TR::TreeTop* OMR_InlinerUtil::storeValueInATemp(
    ...
    List<TR::SymbolReference>& availableTemps,
    List<TR::SymbolReference>* availableTemps2,
    bool behavesLikeTemp,
    TR::TreeTop** newStoreValueTreeTop,
    bool isIndirect,
    int32_t offset) {
...
void TR_CallSite::tagcalltarget(int32_t index,
                                TR_InlinerTracer* tracer,
...
void TR_CallSite::removecalltarget(int32_t index,
                                   TR_InlinerTracer* tracer,
                                   TR_InlinerFailureReason reason) {
  heuristicTrace(
...
void TR_CallSite::removeTargets(TR_InlinerTracer* tracer,
                                int index,
                                TR_InlinerFailureReason reason) {
...
TR_CallTarget* TR_CallSite::addTarget(TR_Memory* mem,
                                      TR_InlinerBase* inliner,
                                      TR_VirtualGuardSelection* guard,
                                      TR_ResolvedMethod* implementer,
                                      TR_OpaqueClassBlock* receiverClass,
...
TR_InlinerTracer::TR_InlinerTracer(TR::Compilation* comp,
                                   TR_FrontEnd* fe,
                                   TR::Optimization* opt)
...
void TR_InlinerTracer::dumpCallSite(TR_CallSite* callsite,
                                    const char* fmt,
                                    ...) {
  va_list args;
  va_start(args, fmt);
...
void TR_InlinerTracer::dumpCallStack(TR_CallStack* callStack,
                                     const char* fmt,
                                     ...) {
  va_list args;
  va_start(args, fmt);
 ...
 * possible
 */
void OMR_InlinerUtil::refineInlineGuard(TR::Node* callNode,
                                        TR::Block*& block1,
                                        TR::Block*& block2,
                                        bool& appendTestToBlock1,
                                        TR::ResolvedMethodSymbol* callerSymbol,
                                        TR::TreeTop* cursorTree,
                                        TR::TreeTop*& virtualGuard,
                                        TR::Block* block4) {}
 ...
 * suppressed
 */
bool OMR_InlinerPolicy::suitableForRemat(TR::Compilation* comp,
                                         TR::Node* node,
                                         TR_VirtualGuardSelection* guard) {
  return true;
}
...
void OMR_InlinerUtil::refineColdness(TR::Node* node, bool& isCold) {
  return;
}
 ...
 */
int32_t OMR_InlinerUtil::getCallCount(TR::Node* callNode) {
  return 0;
}
