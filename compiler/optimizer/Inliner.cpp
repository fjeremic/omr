...
{
  // This method is used to increase the threshold when inlining from places other than the Inliner. ex escape analysis,
  // value prop, etc.
...
void OMR_InlinerPolicy::determineInliningHeuristic(TR::ResolvedMethodSymbol* callerSymbol)
{
  return;
}
...
void TR_InlinerBase::linkOSRCodeBlocks()
{
  ...
  for (intptrj_t i = 0; i < methodDataArray.size(); ++i) {
    ...
    if (osrMethodData == NULL || osrMethodData->getOSRCodeBlock() == NULL || osrMethodData->getInlinedSiteIndex() == -1
...
bool OMR_InlinerPolicy::alwaysWorthInlining(TR_ResolvedMethod* calleeMethod, TR::Node* callNode)
{
  return false;
}
  ...
  TR_ResolvedMethod* calleeResolvedMethod, TR::Node* callNode, bool allConsts)
{
  ...
  if (inlineThreshold && (uint32_t)bytecodeSize > inlineThreshold) {
    ...
    if (block) {
      ...
      TR::DebugCounter::prependDebugCounter(comp(), "inliner.callSites/failed/calleeHasTooManyBytecodes:#bytecodeSize",
      ...
      TR::DebugCounter::prependDebugCounter(comp(),
        "inliner.callSites/failed/calleeHasTooManyBytecodes:#localsInCallee", block->getFirstRealTreeTop(), numLocals);
    }
    heuristicTrace(tracer(), "### Exceeding size threshold because bytecodeSize %d > inlineThreshold %d ", bytecodeSize,
      inlineThreshold);
  ...
  , _safeToAddSymRefs(safeToAddSymRefs)
{
  ...
  if (!nextCallStack) {
    ...
    if (inlineFilters) {
      bool inclusive = comp()->getDebug()->methodSigCanBeFound(
        _method->signature(comp()->trMemory()), inlineFilters, filterInfo, _method->convertToMethod()->methodType());
...
{
  TR_ASSERT(
    _safeToAddSymRefs, "make sure a constructed callStack is committed then add _safeToAddSymRefs to your c-tor call");
  _autos.add(a);
}
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
...

    {
      ...
      calltarget->_prexArgInfo
        = new (trHeapMemory()) TR_PrexArgInfo(calltarget->_myCallSite->_callNode->getNumArguments(), trMemory());
...
bool OMR_InlinerPolicy::inlineMethodEvenForColdBlocks(TR_ResolvedMethod* method)
{
  return false;
}
  ...
  TR::ResolvedMethodSymbol* callerSymbol, TR_CallStack* prevCallStack, TR_InnerPreexistenceInfo* innerPrexInfo)
{
  ...
  for (TR::TreeTop* tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
    ...
    if (parent->getNumChildren()) {
      ...
      if (node->getOpCode().isCall() && (!node->getOpCode().isJumpWithMultipleTargets())
        ...
        if ((isCold
          ...
          !comp()->getMethodSymbol()->doJSR292PerfTweaks() && node->getSymbol() && node->getSymbol()->isResolvedMethod()
    ...
    if (!prevCallStack && parent->getOpCodeValue() == TR::BBStart && !parent->getBlock()->isExtensionOfPreviousBlock())
  ...
  TR_VirtualGuardSelection* guard)
{
  ...
  TR::DebugCounter::prependDebugCounter(comp(),
    TR::DebugCounter::debugCounterName(comp(), "virtualGuards.byKind/%s/(%s)/bcinfo=%d.%d",
      tracer()->getGuardKindString(guard), comp()->signature(), bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex()),
    debugCounterInsertionPoint, 1, fidelity);
  TR::DebugCounter::prependDebugCounter(comp(),
    TR::DebugCounter::debugCounterName(comp(), "virtualGuards.byJittedBody/%s/(%s)/(%s)/%s/bcinfo=%d.%d",
      comp()->getHotnessName(comp()->getMethodHotness()), comp()->signature(), calleeSymbol->signature(trMemory()),
      tracer()->getGuardKindString(guard), bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex()),
    debugCounterInsertionPoint, 1, fidelity);
  ...
  if (guard->_kind == TR_MutableCallSiteTargetGuard) {
    ...
    if (knot)
      heuristicTrace(tracer(), "  createVirtualGuard: MutableCallSite.epoch is %p.obj%d (%p.%p)",
        guard->_mutableCallSiteObject, guard->_mutableCallSiteEpoch, *guard->_mutableCallSiteObject,
  ...
  if (suitableForRemat) {
    ...
    for (prevTree = NULL, argStoreTree = tif.getParameterMapper().firstTempTreeTop();
      ...
      if (argStore->chkIsPrivatizedInlinerArg()) {
        debugTrace(
          tracer(), "  considering priv arg store node [%p] - %d - for remat", argStore, argStore->getGlobalIndex());
        if (dumpRematTrees)
        ...
        TR_YesNoMaybe result = RematTools::gatherNodesToCheck(
          comp(), argStore, argStore->getFirstChild(), scanTargets, argSymRefsToCheck, tracer()->debugLevel());
        if (result == TR_yes) {
          debugTrace(
            tracer(), "    priv arg remat may be possible for node [%p] - %d", argStore, argStore->getGlobalIndex());
      ...
      if (!unsafeSymRefs.Intersects(argSafetyInfo.symRefDependencies(i))) {
        ...
        // otherwise we are doing a partial remat where we are going to load from another temp
          ...
          TR::Node* duplicateStore = TR::Node::createStore(
            argStore->getSymbolReference(), TR::Node::createLoad(argStore, rematTree->getNode()->getSymbolReference()));
        ...
        TR::DebugCounter::incStaticDebugCounter(comp(),
          TR::DebugCounter::debugCounterName(comp(), "privatizedInlinerArgs.byMethod/unsafeSymRef/Failed/(%s)/%s",
            comp()->signature(), argStore->getFirstChild()->getOpCode().getName()));
        TR::DebugCounter::incStaticDebugCounter(comp(),
  ...
  TR_VirtualGuardSelection* guard, TR::TreeTop** induceOSRCallTree, TR_CallTarget* calltarget)
{
  ...
  if (guard->_kind != TR_InnerGuard)
    virtualGuard = block1->append(TR::TreeTop::create(comp(),
      createVirtualGuard(callNode, calleeSymbol, block4->getEntry(),
        calleeSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex(), thisClass, tif.favourVftCompare(), guard)));
  ...
  if (!disableHCRGuards && comp()->getHCRMode() != TR::none && guard->_kind != TR_HCRGuard && !skipHCRGuardForCallee) {
    ...
    if (!skipHCRGuardCreation) {
      ...
      hcrTreeTop = hcrBlock->append(TR::TreeTop::create(comp(),
        createVirtualGuard(callNode, calleeSymbol, block4->getEntry(),
          calleeSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex(), methodClass, tif.favourVftCompare(),
          hcrGuard)));
  ...
  TR::Node* newTreeTopNode = (callNodeTreeTop->getNode()->getOpCode().isCheck())
    ? TR::Node::createWithSymRef(
      callNodeTreeTop->getNode()->getOpCodeValue(), 1, callNode, 0, callNodeTreeTop->getNode()->getSymbolReference())
    :
    ...
    while (cursor && comp()->getMethodSymbol()->isOSRRelatedNode(cursor->getNode(), bci)) {
      if (cursor->getNode()->getOpCode().isStoreDirect() && cursor->getNode()->getFirstChild() == callNode) {
        debugTrace(tracer(), "  virtual call node pps: Pending push store of call [%p] found: [%p]", callNode,
          cursor->getNode());
        block4->append(
          TR::TreeTop::create(comp(), TR::Node::createStore(cursor->getNode()->getSymbolReference(), callNode)));
        break;
      }
  ...
  if (tif.simpleCallReferenceTreeTop())
    ...
    OMR_InlinerUtil::storeValueInATemp(comp(), callNode, resultTempSymRef, block4->getLastRealTreeTop(), callerSymbol,
      tempList, _availableTemps, &_availableBasicBlockTemps);
    ...
    if (shouldUseOSR
      ...
      TR::TreeTop* induceTree
        = callingMethod->genInduceOSRCall(guardedCallNodeTreeTop, callNode->getByteCodeInfo().getCallerIndex(),
          (callNode->getNumChildren() - callNode->getFirstArgumentIndex()), false, false, callerSymbol->getFlowGraph());
  ...
  TR::ResolvedMethodSymbol* callerSymbol, bool isIndirectCall)
{
  ...
  int32_t calleeNumStackSlots
    = calleeMethod->numberOfParameterSlots() + calleeMethod->numberOfTemps() + calleeMethod->numberOfPendingPushes();
  ...
  while (osrMethodData) {
    ...
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(comp(),
        "OSR caller at inlined site index %d has %d bytecodes and %d stack slots, total callers bytecodes %d total "
        "callers stack slots %d total callers live stack slots %d total pending push slots %d\n",
...
TR_ParameterToArgumentMapper::TR_ParameterToArgumentMapper(TR::ResolvedMethodSymbol* callerSymbol,
  TR::ResolvedMethodSymbol* calleeSymbol, TR::Node* callNode, TR_PrexArgInfo* argInfo, List<TR::SymbolReference>& temps,
  List<TR::SymbolReference>& availableTemps, List<TR::SymbolReference>& availableTemps2, TR_InlinerBase* inliner)
...
void TR_ParameterToArgumentMapper::printMapping()
{
  ...
  for (TR_ParameterMapping* pm = _mappings.getFirst(); pm; pm = pm->getNext()) {
    debugTrace(tracer(),
      ...
      pm->_replacementSymRef ? pm->_replacementSymRef->getReferenceNumber() : -1, pm->_parameterNode, pm->_argIndex,
      pm->_parmIsModified, pm->_isConst, pm->_addressTaken);
  }
}
...
void TR_ParameterToArgumentMapper::initialize(TR_CallStack* callStack)
{
  ...
  if (_callNode->getOpCode().isCallIndirect()) {
    ...
    if (arg->getReferenceCount() > 1) {
      ...
      _firstTempTreeTop = _lastTempTreeTop = OMR_InlinerUtil::storeValueInATemp(comp(), arg, _vftReplacementSymRef, 0,
        _calleeSymbol, _tempList, _availableTemps, &_availableTemps2, false, &newStoreTreeTop);
          ...
          if (disableParmTempOpt) {
            ...
            if ((arg->getOpCode().isLoadVarDirect() && arg->getSymbol()->isAutoOrParm())
              && ((arg->getReferenceCount() == 1)
                || (_callNode->getOpCode().isCallIndirect() && (argIndex == _callNode->getFirstArgumentIndex())
                  && (_callNode->getFirstChild()->getNumChildren() > 0)
                  && (arg == _callNode->getFirstChild()->getFirstChild()) && arg->getSymbol()->isAuto()
      ...
      if (parmMap->_isConst) {
        ...
        if (parmMap->_replacementSymRef)
          ...
          if (!disableUseKnownObjectTempsForParms && prexArgument && !parmMap->_parmIsModified
            ...
            debugTrace(
              tracer(), "map arg %p into known object temp #%d as priv arg\n", arg, symRef->getReferenceNumber());
          }

          tt = OMR_InlinerUtil::storeValueInATemp(comp(), arg, symRef, 0, _calleeSymbol, _tempList, _availableTemps,
            ...
            && prexArgument->isTypeInfoForInlinedBody()) // use known object temp in inlined body only
          {
            ...
            debugTrace(tracer(),
              "created tree n%dn to store the priv arg #%d into a known object temp #%d on entry of the inlined body\n",
...
{
  if (!comp()->getOption(TR_EnableOSR) || comp()->getOSRMode() != TR::voluntaryOSR || comp()->osrInfrastructureRemoved()
  ...
  for (uint32_t i = 0; i < comp()->getOSRCallSiteRematSize(siteIndex); ++i) {
    ...
    for (; parmMap; parmMap = parmMap->getNext())
      if (symbol == parmMap->_parmSymbol) {
        if (parmMap->_isConst) {
          ...
          TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "osrCallSiteRemat/mapParm/const/(%s)", comp()->signature()));
        } else if (loadSymRef->getOffset() > 0) {
          comp()->setOSRCallSiteRemat(siteIndex, ppSymRef, NULL);
          TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "osrCallSiteRemat/mapParm/addr/(%s)", comp()->signature()));
        } else {
          comp()->setOSRCallSiteRemat(siteIndex, ppSymRef, parmMap->_replacementSymRef);
          TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(comp(), "osrCallSiteRemat/mapParm/success/(%s)", comp()->signature()));
        }
        break;
      }
    ...
    if (!parmMap)
      TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(comp(), "osrCallSiteRemat/mapParm/missing/(%s)", comp()->signature()));
  }
    ...
    if (parm == parmMap->_parmSymbol) {
      ...
      if (!parmMap->_parmIsModified && parmMap->_replacementSymRefForInlinedBody
        && performTransformation(comp(), "%s set symRef on node n%dn to be known object symRef %p\n", OPT_DETAILS,
          node->getGlobalIndex(), parmMap->_replacementSymRefForInlinedBody))
...
void TR_TransformInlinedFunction::transform()
{
  ...
  if (!_resultNode && returnType != TR::NoType && !_simpleCallReferenceTreeTop && _callNode->getReferenceCount() > 1) {
  ...
  TR::Node* node, TR::Node* parent, uint32_t childIndex, TR::NodeChecklist& visitedNodes)
{
  ...
  if (opcode.isReturn()) {
    ...
    if (callThisPointer->getOpCode().hasSymbolReference() && callThisPointer->getSymbolReference()->isThisPointer()) {
    ...
    if (symbol->isParm()) {
      ...
      if (newNode && newNode != node) {
        if (newNode->getOpCode().isLoadConst() && newNode->getType().isInt32() && node->getType().isInt8()) {
          // do not create bstore/iconst -- work-around for minor type incorrectness in spec2006 hmmer (type mismatch
          // but iconst actually fits in a byte)
          ...
          dumpOptDetails(comp(), "%screate %s (0x%p) to resolve type mismatch between %s (%p) and %s (%p)\n",
            OPT_DETAILS, newNode->getOpCode().getName(), newNode, newNode->getFirstChild()->getOpCode().getName(),
            newNode->getFirstChild(), node->getOpCode().getName(), node);
        }
        ...
        if (newIsIntegral && oldIsIntegral && newNode->getDataType() != node->getDataType()) {
          ...
          newNode = TR::Node::create(
            TR::ILOpCode::getProperConversion(newNode->getDataType(), node->getDataType(), false), 1, oldNewNode);
...
void TR_TransformInlinedFunction::transformReturn(TR::Node* returnNode, TR::Node* parent)
{
  ...
  if (returnNode->getNumChildren() && _callNode->getReferenceCount() > 1) {
    ...
    if (_simpleCallReferenceTreeTop) {
      ...
      OMR_InlinerUtil::storeValueInATemp(comp(), returnValue, _resultTempSymRef, _currentTreeTop->getPrevTreeTop(),
        _callerSymbol, _tempList, _availableTemps, &_availableTemps2);
    else
...
void TR_HandleInjectedBasicBlock::createTemps(bool replaceAllReferences)
{
  ...
  for (MultiplyReferencedNode* ref = _multiplyReferencedNodes.getFirst(); ref; ref = ref->getNext()) {
    ...
    if (rematConstant || opcode.getOpCodeValue() == TR::loadaddr) {
      ...
      // The real underlying issue is that just because a symref is on a parmMap, it doesn't mean you can automatically
      // add it to a list of injected basic block temps When I enabled the ParmTempOpt, this is what happened.  An auto
      // symbol got added to the injectedBasicBlockTemps list, and then was reused in a very bad manner.
...
{
  MultiplyReferencedNode* ref = new (comp()->trStackMemory()) MultiplyReferencedNode(
    node, tt, node->getReferenceCount() - 1, node->getOpCode().isLoadVarDirect() && node->getSymbol()->isAutoOrParm());
  //   ref->_node = node;
  //   ref->_treeTop = tt;
...
TR::TreeTop* OMR_InlinerUtil::storeValueInATemp(TR::Compilation* comp, TR::Node* value,
  ...
  List<TR::SymbolReference>* availableTemps2, bool behavesLikeTemp, TR::TreeTop** newStoreValueTreeTop, bool isIndirect,
  int32_t offset)
{
    ...
    if (isInternalPointer) {
      ...
      if (value->getOpCode().isArrayRef()) {
        ...
        if (valueChild->isInternalPointer() && valueChild->getPinningArrayPointer()) {
          ...
          if (valueChild->getOpCode().isLoadVarDirect() && valueChild->getSymbolReference()->getSymbol()->isAuto()) {
        ...
        pinningArray
          = value->getSymbolReference()->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer();
  ...
  if (isIndirect) {
    ...
    storeNode
      = TR::Node::createWithSymRef(comp->il.opCodeForIndirectStore(dataType), 2, 2, storeAddress, value, symRefShadow);
  } else {
...
TR_ResolvedMethod* OMR_InlinerUtil::findSingleJittedImplementer(TR_IndirectCallSite* callsite)
{
  return NULL;
}
 ...
 */
bool OMR_InlinerUtil::addTargetIfThereIsSingleImplementer(TR_IndirectCallSite* callsite)
{
  return false;
}
...
TR_OpaqueClassBlock* TR_IndirectCallSite::extractAndLogClassArgument(TR_InlinerBase* inliner)
{
  if (inliner->tracer()->heuristicLevel()) {
    ...
    heuristicTrace(
      inliner->tracer(), "Receiver to call is constrained by argument propagation to %s.", fixedReceiverClassName);
  }
    ...
    if (!target->_calleeSymbol || !target->_calleeMethod->isSameMethod(target->_calleeSymbol->getResolvedMethod())) {
      target->_calleeMethod->setOwningMethod(
        callsite->_callNode->getSymbolReference()
          ...
          ->getResolvedMethod()); // need to point to the owning method to the same one that got ilgen'd d157939

      target->_calleeSymbol = comp()
                                ->getSymRefTab()
...
void TR_InlinerBase::applyPolicyToTargets(TR_CallStack* callStack, TR_CallSite* callsite)
{
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    int32_t selfInliningLimit = selfInliningLimitStr
    ...
    if (!forceInline(calltarget)
      && exceedsSizeThreshold(callsite, bytecodeSize,
        ...
        bcInfo, numberOfLocalsInCallee, callsite->_callerResolvedMethod, calltarget->_calleeMethod, callsite->_callNode,
        callsite->_allConsts)
...
static void traceRequestToCSI(TR::Compilation* comp, TR_CallSite* callsite)
{
  ...
  for (int32_t depth = stack.size() - 1; depth >= -1; depth--) {
    const char* methodName
      = (depth == -1) ? comp->signature() : comp->getInlinedResolvedMethod(stack[depth])->signature(comp->trMemory());
  ...
  TR::TreeTop* tt, TR::Node* parent, TR::Node* callNode, TR_CallTarget* calltarget)
{
  ...
  for (callsite = calltarget->_myCallees.getFirst(); callsite; callsite = callsite->getNext()) {
    debugTrace(tracer(), "callNode->getByteCodeIndex = %d callsite->_byteCodeIndex = %d", callNode->getByteCodeIndex(),
  ...
  if (comp()->getOptions()->enableDebugCounters()) {
    if (!foundCall) {
      for (callsite = calltarget->_deletedCallees.getFirst(); callsite; callsite = callsite->getNext()) {
        debugTrace(tracer(),
          "considering deleted callee %p callNode->getByteCodeIndex = %d callsite->_byteCodeIndex = %d", callsite,
  ...
  if (!foundCall && !foundDeadCall) {
    if (tracer()->debugLevel())
      tracer()->dumpCallSite(calltarget->_myCallees.getFirst(),
        "findAndUpdateCallsiteInGraaph: could not match call form IL to call in graph for the following callsite. "
        "callTarget %s, numCallees = %d numDeletedCallees = %d falureReason = %d",
  ...
  TR_CallStack* prevCallStack, TR_CallTarget* calltarget, TR_InnerPreexistenceInfo* innerPrexInfo)
{
  ...
  for (TR::TreeTop* tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
    ...
    if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount
      ...
      if (!isCold) {
        debugTrace(tracer(),
          "inlineFromGraph:: about to call findAndUpdateCallSiteInGraph on call target %p for call at node %p",
          calltarget, node);
        TR_CallSite* site
          = findAndUpdateCallSiteInGraph(&callStack, node->getByteCodeInfo(), tt, parent, node, calltarget);
        debugTrace(tracer(),
          "inlineFromGraph: found a call at parent %p child %p, findAndUpdateCallsite returned callsite %p", parent,
          parent->getChild(0), site);
        ...
        if (calleeSymbol->getMethod())
          heuristicTrace(tracer(),
            "Block containing call node %p (callee name: %s) but either caller block or function is cold. Skipping "
            "call.",
  ...
  for (TR::TreeTop* tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
    ...
    for (TR_CallTarget* target = iter.getFirst(); target; target = iter.getNext()) {
      if (target->_myCallSite->_callNodeTreeTop == tt && !target->_alreadyInlined) {
        debugTrace(tracer(), "inlineFromGraph: Calling inlinecallTarget on calltarget %p in callsite %p", target,
          target->_myCallSite);
  ...
  TR_CallStack* callStack, TR_CallTarget* calltarget, TR::TreeTop** cursorTreeTop, bool inlinefromgraph, int32_t)
{
  ...
  debugTrace(tracer(),
    ...
    "temps: ",
    calltarget, calltarget->_myCallSite, calltarget->_myCallSite->_visitCount, calltarget->_myCallSite->numTargets());
    ...
    if (decompPoint) {
      ...
      if (comp()->trace(OMR::inlining)) {
        ...
        traceMsg(comp(), "FSD inlining: decompilation point %s (%s) prevented inlining of %s\n",
          ...
          comp()->getDebug()->getName(decompPoint->getSymbolReference()), tracer()->traceSignature(calleeSymbol));
      }
  ...
  if (argInfo) {
    if (comp()->usesPreexistence()) // Mark the preexistent arguments (maybe redundant if VP has already done that and
  ...
  if (!performTransformation(comp(),
        ...
        calleeSymbol->signature(trMemory()), callerSymbol->signature(trMemory()), tracer()->getGuardKindString(guard),
  ...
  */
  ...
  if (guard->_kind != TR_NoGuard) {
    virtualGuard = addGuardForVirtual(callerSymbol, calleeSymbol, callNodeTreeTop, callNode, calltarget->_receiverClass,
      startOfInlinedCall, previousBBStartInCaller, nextBBEndInCaller, *tif, tempList, guard, &induceOSRCallTree,
      calltarget);
    ...
    if (!_disableInnerPrex && guard->_kind != TR_ProfiledGuard
      ...
      devirtualizedByInnerPreexistence = innerPrexInfo->perform(comp(), virtualGuard->getNode(), disableTailRecursion);
    ...
    if (osrMethodData && osrMethodData->getOSRCatchBlock()) {
      ...
      debugTrace(tracer(), "Splitting block_%d to have distint exception edges for callee %d", mergedBlock->getNumber(),
        comp()->getCurrentInlinedSiteIndex());
      // mergedBlock may have exception successors from the callee and non-OSR exception successors from the caller,
      // after split, `blockOfTreesAfterTheCall` only contains trees from the caller, should not have exception
      // successors from the callee.
      TR::Block* blockOfTreesAfterTheCall
        = mergedBlock->split(callNodeTreeTop, callerCFG, false /*fixupCommoning*/, false /*copyExceptionSuccessors*/);
      ...
      for (auto e1 = mergedBlock->getExceptionSuccessors().begin(); e1 != mergedBlock->getExceptionSuccessors().end();
           ++e1) {
  ...
  if (induceOSRCallTree) {
    ...
    induceOSRCallNode->setNumChildren(
      numOfChildren - firstArgChild); // tricky "swelling" of the node but we allocated it of the right size when we
                                      // inserted the induce OSR call to begin with
  ...
  if (calltarget->_myCallSite->numTargets() > 1
    ...
    if (lastNodeInCallBlock->getOpCode().isReturn() || lastNodeInCallBlock->getOpCode().isGoto()) {
      ...
      nextBBEndInCaller->getNode()->getBlock()->getPrevBlock()->getExit()->insertBefore(TR::TreeTop::create(comp(),
      ...
      TR_ASSERT(
        calltarget->_myCallSite->_visitCount == 1, "We can only back off after inlining the first implementation\n");
  ...
  if (_comp->cg()->traceBCDCodeGen()) {
    ...
    traceMsg(_comp, "q^q : tag to inline %s into %s (callNode %p on line_no=%d)\n", signature(_comp->trMemory()),
      _comp->fe()->sampleSignature(
        _callerResolvedMethod->getPersistentIdentifier(), callerName, 1024, _comp->trMemory()),
...
TR_CallTarget::TR_CallTarget(TR_CallSite* callsite, TR::ResolvedMethodSymbol* calleeSymbol,
...
TR_CallSite::TR_CallSite(TR_ResolvedMethod* callerResolvedMethod, TR::TreeTop* callNodeTreeTop, TR::Node* parent,
  TR::Node* callNode, TR::Method* interfaceMethod, TR_OpaqueClassBlock* receiverClass, int32_t vftSlot, int32_t cpIndex,
  TR_ResolvedMethod* initialCalleeMethod, TR::ResolvedMethodSymbol* initialCalleeSymbol, bool isIndirectCall,
  bool isInterface, TR_ByteCodeInfo& bcInfo, TR::Compilation* comp, int32_t depth, bool allConsts)
  ...
  TR_ResolvedMethod* implementer, TR_OpaqueClassBlock* receiverClass, TR_AllocationKind allocKind, float ratio)
{
  ...
  TR_CallTarget* result = new (mem, allocKind)
    TR_CallTarget(this, _initialCalleeSymbol, implementer, guard, receiverClass, myPrexArgInfo, ratio);
...
{
  const char* name
    = TR::DebugCounter::debugCounterName(comp(), "inliner.callSites/failed/%s", getFailureReasonString(reason));
    ...
    do {
      ...
      if (!currentTarget->_myCallees.isEmpty()) {
        ...
        for (callsite = currentTarget->_myCallees.getFirst(); callsite; callsite = callsite->getNext()) {
          //               traceMsg(comp(),"\t\t\t\tnumtargets = %d numRemovedTargets = %d currtarget->numdeletedcallees
          //               = %d
...
void TR_InlinerTracer::dumpInline(TR_LinkHead<TR_CallTarget>* targets, const char* fmt, ...)
{
  va_list args;
  va_start(args, fmt);
  ...
  for (TR_CallTarget* calltarget = targets->getFirst(); calltarget; calltarget = calltarget->getNext()) {
    ...
    do {
      ...
      if (currentTarget->_failureReason == InlineableTarget || currentTarget->_failureReason == OverrideInlineTarget
...

  {
    ...
    if (arg && arg->getClass()) {
      ...
      traceMsg(comp(), "<Argument no=%d address=%p classIsFixed=%d classIsPreexistent=%d class=%p className= %s/>\n", i,
        arg, arg->classIsFixed(), arg->classIsPreexistent(), arg->getClass(), className);
    } else {
...
void OMR_InlinerUtil::refineInlineGuard(TR::Node* callNode, TR::Block*& block1, TR::Block*& block2,
  bool& appendTestToBlock1, TR::ResolvedMethodSymbol* callerSymbol, TR::TreeTop* cursorTree, TR::TreeTop*& virtualGuard,
  TR::Block* block4)
{}
...
{
  return new (comp()->trStackMemory()) TR_TransformInlinedFunction(comp(), tracer(), callerSymbol, calleeSymbol,
    blockContainingTheCall, callNodeTreeTop, callNode, pam, guard, tempList, availableTemps, availableBasicBlockTemps);
}
 ...
 */
bool OMR_InlinerPolicy::replaceSoftwareCheckWithHardwareCheck(TR_ResolvedMethod* calleeMethod)
{
  return false;
}
