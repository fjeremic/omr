...
// doesn't rely on the "owning method" accurately representing the calling
// method.
//
...
static bool succAndPredAreNotOSRBlocks(TR::CFGEdge *edge) {
  return !toBlock(edge->getTo())->isOSRCodeBlock() &&
...
int32_t OMR_InlinerPolicy::getInitialBytecodeSize(
    TR::ResolvedMethodSymbol *methodSymbol, TR::Compilation *comp) {
  return getInitialBytecodeSize(methodSymbol->getResolvedMethod(), methodSymbol,
                                comp);
}

int32_t OMR_InlinerPolicy::getInitialBytecodeSize(
    TR_ResolvedMethod *feMethod, TR::ResolvedMethodSymbol *methodSymbol,
    TR::Compilation *comp) {
...
TR::DataType getStoreType(TR::Node *store, TR::Symbol *addrSymbol,
                          TR::Compilation *comp) {
...
int32_t TR_TrivialInliner::perform() {
  ...
  if (sym->mayHaveInlineableCall() && optimizer()->isEnabled(OMR::inlining)) {
    ...
    if (comp()->getOption(TR_Randomize) ||
        comp()->getOption(TR_VerbosePseudoRandom)) {
      // In the following we ask the global cmdLineOptions about the
      // getTrivialInlinerMaxSize() because the result of
      // convertNonDeterministicInput is cached the first time is executed and
      // don't want this result to be determined by the first compilation that
      // comes through
      static uint32_t defaultRandomSize = comp()->convertNonDeterministicInput(
          TR::Options::getCmdLineOptions()->getTrivialInlinerMaxSize(), 30,
          randomGenerator(), 0);
...
const char *TR_TrivialInliner::optDetailString() const throw() {
...
TR_InlinerBase::TR_InlinerBase(TR::Optimizer *optimizer,
  ...
  _policy = optimization->manager()->getOptPolicy()
                ? static_cast<OMR_InlinerPolicy *>(
                      optimization->manager()->getOptPolicy())
                : optimizer->getInlinerPolicy();
  _util = optimizer->getInlinerUtil();
  _policy->setInliner(this);
  _util->setInliner(this);
  if (!optimizer->isEnabled(OMR::innerPreexistence)) _disableInnerPrex = true;

  setInlineVirtuals(true);
  if (optimizer->getInlineSynchronized()) setInlineSynchronized(true);
...
void TR_InlinerBase::setSizeThreshold(uint32_t size) {
  // This method is used to increase the threshold when inlining from places
  // other than the Inliner. ex escape analysis, value prop, etc.

  _methodByteCodeSizeThreshold = size;

  heuristicTrace(
      tracer(),
      "Setting method size threshold (_methodByteCodeSizeThreshold) to %d\n",
      _methodByteCodeSizeThreshold);
}

void OMR_InlinerPolicy::determineInliningHeuristic(
    TR::ResolvedMethodSymbol *callerSymbol) {
  return;
}

void TR_InlinerBase::setInlineThresholds(
    TR::ResolvedMethodSymbol *callerSymbol) {
  ...
  if (comp()->getNodeCount() > _nodeCountThreshold) {
    _nodeCountThreshold =
        comp()->getNodeCount() * (1.05F);  // allow a little bit of inlining
                                           // anyways to get smaller methods
  }

  // Code That should go to its frontend
  getUtil()->adjustCallerWeightLimit(callerSymbol, _callerWeightLimit);
  getUtil()->adjustMethodByteCodeSizeThreshold(callerSymbol,
                                               _methodByteCodeSizeThreshold);

  // let the FrontEnds have a chance to tweak inlining thresholds
  getUtil()->refineInliningThresholds(
      comp(), _callerWeightLimit, _maxRecursiveCallByteCodeSizeEstimate,
      _methodByteCodeSizeThreshold, _methodInWarmBlockByteCodeSizeThreshold,
      _methodInColdBlockByteCodeSizeThreshold, _nodeCountThreshold, size);

  // Possible env variable overrides
  static const char *a = feGetEnv("TR_MethodByteCodeSizeThreshold");
  if (a) _methodByteCodeSizeThreshold = atoi(a);

  static const char *b = feGetEnv("TR_MethodInWarmBlockByteCodeSizeThreshold");
  if (b) _methodInWarmBlockByteCodeSizeThreshold = atoi(b);

  static const char *c = feGetEnv("TR_MethodInColdBlockByteCodeSizeThreshold");
  if (c) _methodInColdBlockByteCodeSizeThreshold = atoi(c);

  static const char *d = feGetEnv("TR_CallerWeightLimit");
  if (d) _callerWeightLimit = atoi(d);

  static const char *e = feGetEnv("TR_NodeCountThreshold");
  if (e) _nodeCountThreshold = atoi(e);
  ...
  if (comp()->getOption(TR_EnableOSR) &&
      comp()->getOSRMode() == TR::voluntaryOSR && comp()->supportsInduceOSR()) {
  ...
  if (comp()->getOption(TR_Randomize)) {
    ...
    _methodByteCodeSizeThreshold = comp()->convertNonDeterministicInput(
        _methodByteCodeSizeThreshold, 500, randomGenerator(), 0);
    traceMsg(comp(),
             "\nTR_Randomize Enabled||TR_InlinerBase::inlineCallTarget, "
             "SeedValue:%d",
             comp()->getOptions()->getRandomSeed());
  }

  heuristicTrace(tracer(),
                 "Inlining Limits:\n\tCaller Side Weight Limit "
                 "(_callerWeightLimit) = %d \n\tCall Graph Size Threshold "
                 "(_maxRecursiveCallByteCodeSizeEstimate) = %d "
                 "\n\tMethod size threshold (_methodByteCodeSizeThreshold) = "
                 "%d \n\tMethod size threshold for warm (and above) compiles "
                 "(_methodInWarmBlockByteCodeSizeThreshold) = %d"
                 "\n\tsize threshold for cold Calls "
                 "(_methodInColdBlockByteCodeSizeThreshold) = %d\n\tNode Count "
                 "Threshold (_nodeCountThreshold) = %d "
                 "\n\tSites Size (_maxInliningCallSites) = %d",
                 _callerWeightLimit, _maxRecursiveCallByteCodeSizeEstimate,
                 _methodByteCodeSizeThreshold,
                 _methodInWarmBlockByteCodeSizeThreshold,
                 _methodInColdBlockByteCodeSizeThreshold, _nodeCountThreshold,
...
void TR_InlinerBase::linkOSRCodeBlocks() {
  ...
  const TR_Array<TR_OSRMethodData *> &methodDataArray =
      compData->getOSRMethodDataArray();
  for (intptrj_t i = 0; i < methodDataArray.size(); ++i) {
    ...
    // For a non-top-level caller, we need to replace the return at the end of
    // the OSR code block with a goto to the OSR code block of the caller
    TR::Block *calleeOSRCodeBlock = osrMethodData->getOSRCodeBlock();
    if (calleeOSRCodeBlock->isUnreachable()) continue;

    TR::TreeTop *lastTT = calleeOSRCodeBlock->getLastRealTreeTop();
    TR::Node *lastNode = lastTT->getNode();
    TR_ASSERT(
        lastNode->getOpCode().isGoto() || lastNode->getOpCode().isReturn(),
        "last treetop of a non-top-level caller OSR code block is neither a "
        "goto nor a return\n");

    TR::Block *callerOSRCodeBlock =
        compData->findCallerOSRMethodData(osrMethodData)->getOSRCodeBlock();
    ...
    comp()->getFlowGraph()->removeEdge(
        calleeOSRCodeBlock->getSuccessors().front());
    lastTT->unlink(true);
...
void TR_InlinerBase::performInlining(TR::ResolvedMethodSymbol *callerSymbol) {
  ...
  TR_InnerPreexistenceInfo *innerPrexInfo =
      getUtil()->createInnerPrexInfo(comp(), callerSymbol, 0, 0, 0, TR_NoGuard);
...
void TR_InlinerBase::cleanup(TR::ResolvedMethodSymbol *callerSymbol,
                             bool inlinedSite) {
  ...
  if (inlinedSite) {
    ...
    _optimizer->setRequestOptimization(
        OMR::eachExpensiveGlobalValuePropagationGroup);
    _optimizer->setAliasSetsAreValid(false, true /*reset for WCode as well */);
  }
}

bool OMR_InlinerPolicy::mustBeInlinedEvenInDebug(
    TR_ResolvedMethod *calleeMethod, TR::TreeTop *callNodeTreeTop) {
  return false;
}

bool TR_InlinerBase::alwaysWorthInlining(TR_ResolvedMethod *calleeMethod,
                                         TR::Node *callNode) {
  return getPolicy()->alwaysWorthInlining(calleeMethod, callNode);
}

bool OMR_InlinerPolicy::alwaysWorthInlining(TR_ResolvedMethod *calleeMethod,
                                            TR::Node *callNode) {
  return false;
}

bool TR_InlinerBase::exceedsSizeThreshold(
    TR_CallSite *callsite, int bytecodeSize, TR::Block *block,
    TR_ByteCodeInfo &bcInfo, int32_t numLocals,
    TR_ResolvedMethod *callerResolvedMethod,
    TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode,
    bool allConsts) {
  if (alwaysWorthInlining(calleeResolvedMethod, callNode)) return false;

  heuristicTrace(tracer(),
                 "### Checking inliner base sizeThreshold. bytecodeSize = %d, "
                 "block = %p, numLocals = %p,"
                 "callerResolvedMethod = %s, calleeResolvedMethod = %s",
                 bytecodeSize, block, numLocals,
                 tracer()->traceSignature(callerResolvedMethod),
  ...
  if (block) {
    ...
    if (allowBiggerMethods()) {
      ...
      if (blockFrequency > borderFrequency) {
        int32_t oldSize = 0;
        if (comp()->trace(OMR::inlining)) oldSize = bytecodeSize;

        bytecodeSize = scaleSizeBasedOnBlockFrequency(
            bytecodeSize, blockFrequency, borderFrequency, calleeResolvedMethod,
            callNode);

        if (comp()->trace(OMR::inlining))
          heuristicTrace(tracer(),
                         "exceedsSizeThreshold: Scaled down size for call "
                         "block_%d from %d to %d",
                         block->getNumber(), oldSize, bytecodeSize);
      }
    }
  } else if (getPolicy()->aggressiveSmallAppOpts()) {
    getUtil()->adjustByteCodeSize(calleeResolvedMethod, _isInLoop, block,
                                  bytecodeSize);
  } else {
    if (comp()->trace(OMR::inlining))
  ...
  if (comp()->isServerInlining()) {
    if (blockFrequency > borderFrequency) inlineThreshold = 200;
  }

  // reduce size if your args are consts

  if (callNode) {
    heuristicTrace(tracer(), "In ExceedsSizeThreshold.  Reducing size from %d",
                   bytecodeSize);
    ...
    if (!allconstsfromNode) {
      if (_isInLoop && originalbcSize > inlineThreshold) {
        if (numConstArgs > 0 && (numConstArgs + numNonConstArgs) > 0)
          bytecodeSize = (numConstArgs * originalbcSize) /
                         (numConstArgs + numNonConstArgs);
      } else
  ...
  if (inlineThreshold && (uint32_t)bytecodeSize > inlineThreshold) {
    ...
    if (block) {
      TR::DebugCounter::prependDebugCounter(
          comp(), "inliner.callSites/failed/calleeHasTooManyBytecodes",
          block->getFirstRealTreeTop());
      TR::DebugCounter::prependDebugCounter(
          comp(),
          "inliner.callSites/failed/calleeHasTooManyBytecodes:#bytecodeSize",
          block->getFirstRealTreeTop(), bytecodeSize);
      TR::DebugCounter::prependDebugCounter(
          comp(), "inliner.callSites/failed/calleeHasTooManyBytecodes:#excess",
          block->getFirstRealTreeTop(), bytecodeSize - inlineThreshold);
      TR::DebugCounter::prependDebugCounter(
          comp(),
          "inliner.callSites/failed/calleeHasTooManyBytecodes:#localsInCallee",
          block->getFirstRealTreeTop(), numLocals);
    }
    heuristicTrace(tracer(),
                   "### Exceeding size threshold because bytecodeSize %d > "
                   "inlineThreshold %d ",
  ...
  heuristicTrace(tracer(),
                 "### Did not exceed size threshold, bytecodeSize %d <= "
                 "inlineThreshold %d ",
                 bytecodeSize, inlineThreshold);
  return false;  // Does not exceed threshold
}

void TR_InlinerBase::createParmMap(TR::ResolvedMethodSymbol *calleeSymbol,
...
bool OMR_InlinerPolicy::inlineRecognizedMethod(TR::RecognizedMethod method) {
  return false;
}

// only dumbinliner uses this and includes checking for variable initialization
bool TR_DumbInliner::tryToInline(char *message, TR_CallTarget *calltarget) {
...
bool OMR_InlinerPolicy::tryToInlineGeneral(TR_CallTarget *calltarget,
  ...
  if (regex && TR::SimpleRegex::match(regex, calltarget->_calleeMethod)) {
    if (comp()->trace(OMR::inlining)) {
      traceMsg(comp(), toInline ? "Inliner: tryToInline pattern matched, "
                                : "Inliner: dontInline pattern matched, ");
  ...
  if (callStack && callStack->_inlineFilters) {
    ...
    if (inlineFilters) {
      bool inclusive = comp()->getDebug()->methodSigCanBeFound(
          signature, inlineFilters, filterInfo,
          calltarget->_calleeMethod->convertToMethod()->methodType());

      if (filterInfo) {
        if (toInline && inclusive) return true;
        if (!toInline && !inclusive) return true;
      }
    }
  }
...
bool OMR_InlinerPolicy::tryToInline(TR_CallTarget *calltarget,
                                    TR_CallStack *callStack, bool toInline) {
...
TR_CallStack::TR_CallStack(TR::Compilation *c,
                           TR::ResolvedMethodSymbol *methodSymbol,
                           TR_ResolvedMethod *method,
                           TR_CallStack *nextCallStack, int32_t maxCallSize,
                           bool safeToAddSymRefs)
  ...
  if (!nextCallStack) {
    ...
    if (inlineFilters) {
      bool inclusive = comp()->getDebug()->methodSigCanBeFound(
          _method->signature(comp()->trMemory()), inlineFilters, filterInfo,
...
void TR_CallStack::commit() {
  ...
  ListIterator<TR::SymbolReference> injectedBasicBlockTemps(
      &_injectedBasicBlockTemps);
  TR::AutomaticSymbol *a = autos.getFirst();
  TR::SymbolReference *temp = temps.getFirst();
  TR::SymbolReference *injectedBasicBlockTemp =
      injectedBasicBlockTemps.getFirst();

  if (getNext()) {
    for (; a; a = autos.getNext()) getNext()->_autos.add(a);
    for (; temp; temp = temps.getNext()) getNext()->_temps.add(temp);
    for (; injectedBasicBlockTemp;
         injectedBasicBlockTemp = injectedBasicBlockTemps.getNext())
      getNext()->_injectedBasicBlockTemps.add(injectedBasicBlockTemp);
  } else {
    TR_ASSERT(_methodSymbol, "_methodSymbol has not been set");
    // TR_ASSERT(!temp, "temps should have been added by a call to
    // makeTempsAvailable");
    for (; a; a = autos.getNext()) _methodSymbol->addAutomatic(a);
    for (; injectedBasicBlockTemp;
         injectedBasicBlockTemp = injectedBasicBlockTemps.getNext())
      _methodSymbol->addAutomatic(
          injectedBasicBlockTemp->getSymbol()->castToAutoSymbol());
  }
...
TR_CallStack::~TR_CallStack() {
  ...
  //propagated to a caller if there are still some residual symRefs left it
  // means that we missed a call to commit somewhere
#if !defined(AIXPPC)
  TR_ASSERT(
      std::uncaught_exception() || (_autos.isEmpty() && _temps.isEmpty() &&
                                    _injectedBasicBlockTemps.isEmpty()),
...
void TR_CallStack::addAutomatic(TR::AutomaticSymbol *a) {
  TR_ASSERT(_safeToAddSymRefs,
            "make sure a constructed callStack is committed then add "
            "_safeToAddSymRefs to your c-tor call");
  _autos.add(a);
}
...
void TR_CallStack::makeTempsAvailable(
    List<TR::SymbolReference> &availableTemps) {
  makeTempsAvailable(availableTemps, _temps);
}

void TR_CallStack::makeBasicBlockTempsAvailable(
    List<TR::SymbolReference> &availableTemps) {
...
TR_CallStack *TR_CallStack::isCurrentlyOnTheStack(TR_ResolvedMethod *method,
                                                  int32_t occurrences) {
  int32_t counter = 0;
...
bool TR_CallStack::isAnywhereOnTheStack(TR_ResolvedMethod *method,
                                        int32_t occurrences) {
  int32_t counter = 0;
  for (TR_CallStack *cs = this; cs; cs = cs->getNext()) {
    if (cs->_method->isSameMethod(method) && ++counter == occurrences) {
      ...
      for (int16_t callerIndex =
               cs->_currentCallNode->getByteCodeInfo().getCallerIndex();
           callerIndex != -1;) {
        TR_InlinedCallSite &ics = _comp->getInlinedCallSite(callerIndex);
        TR_ResolvedMethod *caller =
            _comp->getInlinedResolvedMethod(callerIndex);
        if (caller->isSameMethod(method)) {
          if (++counter == occurrences) return true;
        }
...
void TR_CallStack::initializeControlFlowInfo(
    TR::ResolvedMethodSymbol *callerSymbol) {
  ...
  // Walk forward following successor edges to mark blocks that are always
  // reached
  //
  ...
  // Walk backwards following predecessors edges to mark blocks that are always
  // reached
  //
...
TR_InlineCall::TR_InlineCall(TR::Optimizer *optimizer, TR::Optimization *opt)
    : TR_DumbInliner(optimizer, opt,
                     isScorching(optimizer->comp())
                         ? 140
                         : (isHot(optimizer->comp()) ? 70 : 35)) {}

bool TR_InlineCall::inlineRecognizedMethod(TR::RecognizedMethod method) {
  if (getPolicy()->willBeInlinedInCodeGen(method)) return false;
  return true;
}

bool TR_InlineCall::inlineCall(TR::TreeTop *callNodeTreeTop,
                               TR_OpaqueClassBlock *thisClass,
                               bool recursiveInlining, TR_PrexArgInfo *argInfo,
                               int32_t initialMaxSize) {
  TR_InlinerDelimiter delimiter(tracer(), "TR_InlineCall::inlineCall");
  if (!getOptimizer()->isEnabled(OMR::inlining)) return false;

  TR::Node *parent = callNodeTreeTop->getNode();
  if (parent->getNumChildren() != 1) return false;
  TR::Node *callNode = parent->getFirstChild();
  if (!callNode->getOpCode().isCall()) return false;

  TR::SymbolReference *symRef = callNode->getSymbolReference();

  TR::ResolvedMethodSymbol *resolvedCalleeSymbol =
      symRef->getSymbol()->castToResolvedMethodSymbol();
  if (resolvedCalleeSymbol && !getPolicy()->canInlineMethodWhileInstrumenting(
                                  resolvedCalleeSymbol->getResolvedMethod()))
    return false;

  TR::ResolvedMethodSymbol *callerSymbol = comp()->getMethodSymbol();

  if (recursiveInlining && initialMaxSize == 0)
    initialMaxSize = isScorching(comp())
                         ? 140
                         : isHot(comp()) ? 90 : isWarm(comp()) ? 60 : 45;
  ...
  TR_CallStack callStack(comp(), callerSymbol, comp()->getCurrentMethod(), 0,
                         initialMaxSize, true);
  TR_InnerPreexistenceInfo *innerPrexInfo =
      getUtil()->createInnerPrexInfo(comp(), callerSymbol, 0, 0, 0, TR_NoGuard);
  ...
  TR_CallSite *callsite = TR_CallSite::create(
      callNodeTreeTop, parent, callNode, thisClass, symRef,
      (TR_ResolvedMethod *)0, comp(), trMemory(), stackAlloc);

  getSymbolAndFindInlineTargets(&callStack, callsite);

  if (!callsite->numTargets()) return false;  // nothing to callStack::commit
                                              // yet
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if (initialMaxSize > 0 &&
        getPolicy()->getInitialBytecodeSize(calltarget->_calleeSymbol, comp()) >
            initialMaxSize) {
      heuristicTrace(tracer(),
                     "Failed at Inlining. getMaxByteCodeIndex of %d exceeds "
                     "initialMaxSize of %d",
                     getPolicy()->getInitialBytecodeSize(
                         calltarget->_calleeSymbol, comp()),
                     initialMaxSize);
...

    {
      ...
      int16_t currentInlineDepth =
          comp()->adjustInlineDepth(callNode->getByteCodeInfo());

      if (comp()->trace(OMR::inlining))
        traceMsg(comp(), "inliner: Setting current inline depth=%d\n",
                 currentInlineDepth);

      calltarget->_prexArgInfo = new (trHeapMemory()) TR_PrexArgInfo(
          calltarget->_myCallSite->_callNode->getNumArguments(), trMemory());

      // this is called on a case-by-case basis, and can get repeatedly called
      // for recursive methods triggering a loop.  Unfortunately the callStack
      // is not pervasive for all these seperate calls, so check the VM stack to
      // prevent infinite inlining of recursive calls
      //
      if (comp()->foundOnTheStack(
              calltarget->_calleeSymbol->getResolvedMethod(), 2))
        inlinedSite = false;
      else
...
TR_DumbInliner::TR_DumbInliner(TR::Optimizer *optimizer,
                               TR::Optimization *optimization,
  ...
  static const char *p;
  static int32_t dri =
      (p = feGetEnv("TR_DumbReductionIncrement")) ? atoi(p) : -1;
  if (dri >= 0) _dumbReductionIncrement = dri;
}

bool OMR_InlinerPolicy::inlineMethodEvenForColdBlocks(
    TR_ResolvedMethod *method) {
  return false;
}
...
bool TR_DumbInliner::inlineCallTargets(
    TR::ResolvedMethodSymbol *callerSymbol, TR_CallStack *prevCallStack,
    TR_InnerPreexistenceInfo *innerPrexInfo) {
  int32_t maxCallSize;

  if (!comp()->getOption(TR_DisableAdaptiveDumbInliner)) {
    int32_t callerSize =
        callerSymbol->getResolvedMethod()->maxBytecodeIndex();  // getInitialBytecodeSize(callerSymbol,
                                                                // comp());
    maxCallSize =
        prevCallStack == 0 ?
                           // Initial budget should be inverse proportional to
                           // the size of the method
            (int32_t)_initialSize -
                callerSize * (int32_t)_initialSize /
                    comp()->getOptions()->getDumbInlinerBytecodeSizeCutoff()
                           :
                           // If I inlined a bigger method, my budget should
                           // decrease by a bigger amount
            prevCallStack->_maxCallSize -
                callerSize /
                    comp()->getOptions()->getDumbInlinerBytecodeSizeDivisor();
  } else {
    // For top most method start with a budget of _initialSize
    // For every level of inlining decrement the budget by
    // _dumbReductionIncrement
    maxCallSize = prevCallStack == 0
                      ? _initialSize
                      : prevCallStack->_maxCallSize - _dumbReductionIncrement;
  }

  if (maxCallSize <= 0) return false;

  TR_CallStack callStack(comp(), callerSymbol,
                         callerSymbol->getResolvedMethod(), prevCallStack,
                         maxCallSize, true);
  if (innerPrexInfo) callStack._innerPrexInfo = innerPrexInfo;

  bool isCold = false;
  bool prevInliningAsWeWalk = _inliningAsWeWalk;
  int32_t thisCallSite =
      callerSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex();
  uint32_t inlineCount = 0;
  for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt;
    ...
    if (parent->getOpCodeValue() == TR::BBStart) {
      ...
      if (comp()->getMethodHotness() <= cold) {
        veryColdBorderFrequency =
            comp()->getOption(TR_DisableConservativeColdInlining) ? 0 : 1500;
        // Did the user specify a specific value? If so, use that
        if (comp()->getOptions()->getInlinerVeryColdBorderFrequencyAtCold() >=
            0)
          veryColdBorderFrequency =
              comp()->getOptions()->getInlinerVeryColdBorderFrequencyAtCold();
      }

      // dont inline into cold blocks
      if (block->isCold() ||
          (TR::isJ9() &&
           !getPolicy()->inlineMethodEvenForColdBlocks(
               callerSymbol->getResolvedMethod()) &&
           block->getFrequency() >= 0 &&
           block->getFrequency() < veryColdBorderFrequency) ||
    ...
    if (parent->getNumChildren()) {
      ...
      if (node->getOpCode().isCall() &&
          ...
          node->getVisitCount() != _visitCount &&
          node->getInlinedSiteIndex() == thisCallSite) {
        if ((isCold
#ifdef J9_PROJECT_SPECIFIC
             || comp()->getPersistentInfo()->getInlinerTemporarilyRestricted()
#endif
                 ) &&  // getInlinerTemporarilyRestricted is used to avoid
                       // inlining during startup
            !comp()->getMethodSymbol()->doJSR292PerfTweaks() &&
            node->getSymbol() && node->getSymbol()->isResolvedMethod() &&
            !alwaysWorthInlining(node->getSymbol()
                                     ->castToResolvedMethodSymbol()
                                     ->getResolvedMethod(),
                                 node))
          TR::DebugCounter::prependDebugCounter(
              comp(), "inliner.callSites/failed/coldCallee/1", tt);
        else if (analyzeCallSite(&callStack, tt, parent, node)) {
          inlineCount++;
          if (inlineCount >= MAX_INLINE_COUNT) {
            if (comp()->trace(OMR::inlining))
              traceMsg(comp(),
                       "inliner: stopping inlining as max inline count of %d "
                       "reached\n",
                       MAX_INLINE_COUNT);
            break;
          }
...
bool TR_DumbInliner::analyzeCallSite(TR_CallStack *callStack,
                                     TR::TreeTop *callNodeTreeTop,
                                     TR::Node *parent, TR::Node *callNode) {
  ...
  TR_CallSite *callsite = TR_CallSite::create(
      callNodeTreeTop, parent, callNode, (TR_OpaqueClassBlock *)0, symRef,
      (TR_ResolvedMethod *)0, comp(), trMemory(), stackAlloc);

  getSymbolAndFindInlineTargets(callStack, callsite);

  if (!callsite->numTargets()) return false;
  bool success = false;
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    TR_CallTarget *calltarget = callsite->getTarget(i);
    ...
    if ((byteCodeSize > maxBCSize)) {
      ...
      } else if (alwaysWorthInlining(
                     calltarget->_calleeSymbol->getResolvedMethod(),
                     callNode)) {
        if (comp()->trace(OMR::inlining))
          traceMsg(comp(),
                   "inliner: overriding getMaxBytecodeIndex check because it's "
                   "always worth inlining\n");
      } else {
        if (comp()->trace(OMR::inlining))
          traceMsg(comp(),
                   "inliner: failed: getInitialBytecodeSize(%d) > %d for %s\n",
                   byteCodeSize, callStack->_maxCallSize,
                   tracer()->traceSignature(calltarget->_calleeSymbol));
        if (comp()->cg()->traceBCDCodeGen()) {
          traceMsg(comp(),
                   "q^q : failing to inline %s into %s (callNode %p on "
                   "line_no=%d) due to wcode size\n",
                   tracer()->traceSignature(calltarget->_calleeSymbol),
                   tracer()->traceSignature(callStack->_methodSymbol), callNode,
                   comp()->getLineNumber(callNode));
        }
...
static TR::TreeTop *cloneAndReplaceCallNodeReference(TR::TreeTop *, TR::Node *,
                                                     TR::Node *, TR::TreeTop *,
                                                     TR::Compilation *);

static TR::Node *findPotentialDecompilationPoint(
    TR::ResolvedMethodSymbol *calleeSymbol, TR::Compilation *comp) {
  for (TR::PreorderNodeIterator it(calleeSymbol->getFirstTreeTop(), comp);
       it.currentTree() != NULL; ++it) {
    TR::Node *node = it.currentNode();
    if (node->getOpCode().hasSymbolReference() &&
        comp->isPotentialOSRPoint(node))
      return node;
  }
  return NULL;
}

void TR_InlinerBase::cloneChildren(TR::Node *to, TR::Node *from,
                                   uint32_t fromStartIndex) {
  for (uint32_t i = fromStartIndex, j = 0; i < from->getNumChildren();
       ++i, ++j) {
 ...
 */
void TR_InlinerBase::replaceCallNode(TR::ResolvedMethodSymbol *callerSymbol,
                                     TR::Node *resultNode,
                                     rcount_t originalCallNodeReferenceCount,
                                     TR::TreeTop *callNodeTreeTop,
                                     TR::Node *parent, TR::Node *callNode) {
  ...
  if (resultNode) {
    ...
    for (; tt && numberOfReferencesToFind; tt = tt->getNextTreeTop())
      replaceCallNodeReferences(tt->getNode(), 0, 0, callNode, resultNode,
                                numberOfReferencesToFind, visitedNodes);
  } else
 ...
 * replacementNode
 */
void TR_InlinerBase::replaceCallNodeReferences(
    TR::Node *node, TR::Node *parent, uint32_t childIndex, TR::Node *callNode,
    TR::Node *replacementNode, rcount_t &numberOfReferencesToFind,
  ...
  // We check _inliningAsWeWalk since call nodes' visit counts should not be
  // changed if we are inlining as we walk. We might think we have not inlined
  // some call and try to inline it again and again (as was seen during dumb
  // inlining). Note that this routine we are in could be called from code
  // reached from dumb inliner where inline as we walk the trees in
  // inlineCallTargets as well as the call graph inliner inlineCallTargets where
  // we may/may not inline as we walk the trees. Handling both cases is
  // something to consider when changing below code.
  //
  if ((_inliningAsWeWalk && node->getOpCode().isCall() &&
       (node->getVisitCount() == _visitCount)) ||
  ...
  if (!replacedNode) {
    for (int32_t i = 0; i < node->getNumChildren() && numberOfReferencesToFind;
         ++i)
      replaceCallNodeReferences(node->getChild(i), node, i, callNode,
                                replacementNode, numberOfReferencesToFind,
                                visitedNodes);
  }
}

TR::Node *TR_InlinerBase::createVirtualGuard(
    TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol,
    TR::TreeTop *destination, int16_t calleeIndex,
    TR_OpaqueClassBlock *thisClass, bool favourVftCompareGuard,
    TR_VirtualGuardSelection *guard) {
  TR::MethodSymbol *callNodeMethodSymbol =
      callNode->getSymbol()->castToMethodSymbol();
  TR::ResolvedMethodSymbol *callNodeResolvedMethodSymbol =
      callNodeMethodSymbol->getResolvedMethodSymbol();

  TR::TreeTop *debugCounterInsertionPoint =
      destination->getNextTreeTop();  // destination should always be a BBStart
  ...
  switch (guard->_kind) {
    case TR_ProfiledGuard:
      if (guard->isHighProbablityProfiledGuard())
        fidelity = TR::DebugCounter::Free;  // The taken path is marked isCold,
                                            // so it had better never happen
      else
    ...
    case TR_MethodEnterExitGuard:
      ...
      break;
    default:
      // Nopable guards are considered Free (or else we shouldn't be using code
      // patching!)
  ...
  TR::DebugCounter::prependDebugCounter(
      ...
      TR::DebugCounter::debugCounterName(
          comp(), "virtualGuards.byKind/%s/(%s)/bcinfo=%d.%d",
          tracer()->getGuardKindString(guard), comp()->signature(),
          bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex()),
      debugCounterInsertionPoint, 1, fidelity);
  TR::DebugCounter::prependDebugCounter(
      comp(),
      TR::DebugCounter::debugCounterName(
          comp(), "virtualGuards.byJittedBody/%s/(%s)/(%s)/%s/bcinfo=%d.%d",
          comp()->getHotnessName(comp()->getMethodHotness()),
          comp()->signature(), calleeSymbol->signature(trMemory()),
          tracer()->getGuardKindString(guard), bcInfo.getCallerIndex(),
          bcInfo.getByteCodeIndex()),
      debugCounterInsertionPoint, 1, fidelity);

  if (guard->_kind == TR_DummyGuard) {
    return TR_VirtualGuard::createDummyGuard(comp(), calleeIndex, callNode,
                                             destination);
  }

  if (guard->_kind == TR_HCRGuard) {
    return TR_VirtualGuard::createHCRGuard(
        comp(), calleeIndex, callNode, destination, calleeSymbol, thisClass);
  }
  ...
  if (guard->_kind == TR_MutableCallSiteTargetGuard) {
    ...
    if (knot)
      heuristicTrace(
          tracer(),
          "  createVirtualGuard: MutableCallSite.epoch is %p.obj%d (%p.%p)",
          guard->_mutableCallSiteObject, guard->_mutableCallSiteEpoch,
          *guard->_mutableCallSiteObject,
          *knot->getPointerLocation(guard->_mutableCallSiteEpoch));
    return TR_VirtualGuard::createMutableCallSiteTargetGuard(
        comp(), calleeIndex, callNode, destination,
        guard->_mutableCallSiteObject, guard->_mutableCallSiteEpoch);
  }

  if (guard->_kind == TR_DirectMethodGuard) {
    return TR_VirtualGuard::createAOTInliningGuard(
        comp(), calleeIndex, callNode, destination, guard->_kind);
  }

  if (guard->_type == TR_VftTest) {
    // TR_ASSERT(thisClass == info->getThisClass(), "type info mismatch");
    return TR_VirtualGuard::createVftGuard(guard->_kind, comp(), calleeIndex,
                                           callNode, destination, thisClass);
  } else if (guard->_type == TR_MethodTest)
    return TR_VirtualGuard::createMethodGuard(guard->_kind, comp(), calleeIndex,
...
static bool blocksHaveSameStartingByteCodeInfo(TR::Block *aBlock,
                                               TR::Block *bBlock) {
  TR_ByteCodeInfo a = aBlock->getEntry()->getNode()->getByteCodeInfo();
  TR_ByteCodeInfo b = bBlock->getEntry()->getNode()->getByteCodeInfo();
  return (a.getCallerIndex() == b.getCallerIndex()) &&
         (a.getByteCodeIndex() == b.getByteCodeIndex());
}

/**
 * Root function for privatized inliner argument rematerialization - this
 * handles calculating remat safety and performing the remat.
 */
void TR_InlinerBase::rematerializeCallArguments(
    TR_TransformInlinedFunction &tif, TR_VirtualGuardSelection *guard,
    TR::Node *callNode, TR::Block *block1, TR::TreeTop *rematPoint) {
  debugTrace(tracer(),
             "privatizedInlinerArg - consider arguments for rematerialization "
             "for guard [%p] of call node [n%dn]",
             guard, callNode->getGlobalIndex());
  ...
  static char *disableProfiledGuardRemat =
      feGetEnv("TR_DisableProfiledGuardRemat");

  bool suitableForRemat =
      !comp()->getOption(TR_DisableGuardedCallArgumentRemat) &&
  ...
  if (suitableForRemat) {
    if (guard->_kind == TR_NoGuard) {
      ...
      if (disableProfiledGuardRemat) {
        ...
        suitableForRemat =
            getPolicy()->suitableForRemat(comp(), callNode, guard);
        if (!suitableForRemat) {
          debugTrace(
              tracer(),
              "  skipping remat on profiled guard [%p] due to policy decision",
              guard);
        }
      }
    }
  ...
  if (suitableForRemat) {
    ...
    TR::list<TR::TreeTop *> failedArgs(
        getTypedAllocator<TR::TreeTop *>(comp()->allocator()));
    for (prevTree = NULL,
        argStoreTree = tif.getParameterMapper().firstTempTreeTop();
         prevTree != tif.getParameterMapper().lastTempTreeTop();
         prevTree = argStoreTree,
        argStoreTree = argStoreTree->getNextTreeTop()) {
      TR::Node *argStore = argStoreTree->getNode();
      if (argStore->chkIsPrivatizedInlinerArg()) {
        debugTrace(tracer(),
                   "  considering priv arg store node [%p] - %d - for remat",
                   argStore, argStore->getGlobalIndex());
        if (dumpRematTrees)
          comp()->getDebug()->print(comp()->getOutFile(), argStoreTree);
        TR::SparseBitVector argSymRefsToCheck(comp()->allocator());
        TR_YesNoMaybe result = RematTools::gatherNodesToCheck(
            comp(), argStore, argStore->getFirstChild(), scanTargets,
            argSymRefsToCheck, tracer()->debugLevel());
        if (result == TR_yes) {
          debugTrace(tracer(),
                     "    priv arg remat may be possible for node [%p] - %d",
                     argStore, argStore->getGlobalIndex());
          argSafetyInfo.add(argStoreTree, argSymRefsToCheck);
        } else if (result == TR_no) {
          debugTrace(tracer(), "    priv arg remat unsafe for node [%p] - %d",
                     argStore, argStore->getGlobalIndex());
    ...
    // if we have failed to remat any arguments we want to see if there is
    // another store of the argument that we can use for partial remat purposes
    // - hibb often makes these for us
    if (failedArgs.size() > 0) {
      RematTools::walkTreeTopsCalculatingRematFailureAlternatives(
          comp(), block1->getFirstRealTreeTop(),
      ...
      for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
        ...
        TR::DebugCounter::prependDebugCounter(
            comp(),
            TR::DebugCounter::debugCounterName(
                comp(), "privInlinerArg/Failed/%s",
                argStore->getFirstChild()->getOpCode().getName()),
            argStoreTree, 1, TR::DebugCounter::Expensive);
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(
                        comp(), "privatizedInlinerArgs.byMethod/Failed/(%s)/%s",
                        comp()->signature(),
                        argStore->getFirstChild()->getOpCode().getName()));
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(
                        comp(), "privatizedInlinerArgs.byReason/%s",
                        argStore->getFirstChild()->getOpCode().getName()));
      }
    }

    debugTrace(tracer(), "  priv arg remat: symref checking walk required = %d",
               !scanTargets.IsZero());
    ...
    debugTrace(tracer(),
               "  priv arg remat: Proceeding with block_%d rematPoint=[%p]",
               rematPoint->getEnclosingBlock()->getNumber(), rematPoint);

    if (comp()->getOption(TR_DebugInliner)) argSafetyInfo.dumpInfo(comp());
    for (uint32_t i = 0; i < argSafetyInfo.size(); ++i) {
      ...
      if (!unsafeSymRefs.Intersects(argSafetyInfo.symRefDependencies(i))) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(
                        comp(), "privatizedInlinerArgs.byMethod/(%s)/Succeeded",
                        comp()->signature()));
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(
                        comp(), "privatizedInlinerArgs.byReason/Success"));
        TR::DebugCounter::prependDebugCounter(
            comp(),
            TR::DebugCounter::debugCounterName(
                comp(), "privInlinerArg/Succeeded/(%s)",
                argStore->getFirstChild()->getOpCode().getName()),
            argStoreTree, 1, TR::DebugCounter::Expensive);
        // equality of rematTree and argStoreTree means we want to duplicate the
        // computation of the argument and do a full rematerialization
        if (rematTree == argStoreTree) {
          TR::Node *duplicateStore = argStore->duplicateTree();
          if (performTransformation(
                  comp(),
                  "O^O GUARDED CALL REMAT: Rematerialize [%p] as [%p]\n",
                  argStore, duplicateStore)) {
            rematPoint =
                TR::TreeTop::create(comp(), rematPoint, duplicateStore);
          }
        }
        // otherwise we are doing a partial remat where we are going to load
        // from another temp
        else {
          TR::Node *duplicateStore = TR::Node::createStore(
              argStore->getSymbolReference(),
              TR::Node::createLoad(argStore,
                                   rematTree->getNode()->getSymbolReference()));
          duplicateStore->setByteCodeInfo(argStore->getByteCodeInfo());
          if (performTransformation(
                  comp(),
                  "O^O GUARDED CALL REMAT: Partial rematerialize of [%p] as "
                  "[%p] - load of [%d]\n",
                  argStore, duplicateStore,
                  rematTree->getNode()
                      ->getSymbolReference()
        ...
        debugTrace(tracer(),
                   " priv arg remat failed for node [%p] - %d - due to data "
                   "dependencies",
                   argStoreTree->getNode(),
                   argStoreTree->getNode()->getGlobalIndex());
        TR::DebugCounter::incStaticDebugCounter(
            comp(),
            TR::DebugCounter::debugCounterName(
                comp(),
                "privatizedInlinerArgs.byMethod/unsafeSymRef/Failed/(%s)/%s",
                comp()->signature(),
                argStore->getFirstChild()->getOpCode().getName()));
        TR::DebugCounter::incStaticDebugCounter(
            comp(),
            TR::DebugCounter::debugCounterName(
                comp(), "privatizedInlinerArgs.byReason/unsafeSymRef/%s",
                argStore->getFirstChild()->getOpCode().getName()));
        TR::DebugCounter::prependDebugCounter(
            comp(),
            TR::DebugCounter::debugCounterName(
...
void TR_InlinerBase::addAdditionalGuard(
    TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol,
    TR_OpaqueClassBlock *thisClass, TR::Block *prevBlock,
    TR::Block *inlinedBody, TR::Block *slowPath, TR_VirtualGuardKind kind,
    TR_VirtualGuardTestType type, bool favourVFTCompare, TR::CFG *callerCFG) {
  TR::Block *guardBlock =
      TR::Block::createEmptyBlock(callNode, comp(), prevBlock->getFrequency());
  ...
  TR_VirtualGuardSelection *guard =
      new (trStackMemory()) TR_VirtualGuardSelection(kind, type);
  TR::TreeTop *tt = guardBlock->append(TR::TreeTop::create(
      comp(),
      createVirtualGuard(
          callNode, calleeSymbol, slowPath->getEntry(),
...
TR::TreeTop *TR_InlinerBase::addGuardForVirtual(
    TR::ResolvedMethodSymbol *callerSymbol,
    TR::ResolvedMethodSymbol *calleeSymbol, TR::TreeTop *callNodeTreeTop,
    TR::Node *callNode, TR_OpaqueClassBlock *thisClass,
    TR::TreeTop *startOfInlinedCall, TR::TreeTop *previousBBStartInCaller,
    TR::TreeTop *nextBBEndInCaller, TR_TransformInlinedFunction &tif,
    List<TR::SymbolReference> &tempList, TR_VirtualGuardSelection *guard,
    TR::TreeTop **induceOSRCallTree, TR_CallTarget *calltarget) {
  // generate:
  //
  ...
  //  BBEnd     (if the inlined call has blocks then this is the exit for the
  //  last block in the inlined code) BBStart [3]
  //
  ...
  TR::Block *block3 = nextBBEndInCaller->getNode()->getBlock()->split(
      callNodeTreeTop, callerCFG);
  ...
  if (guard->_kind != TR_InnerGuard)
    virtualGuard = block1->append(TR::TreeTop::create(
        comp(),
        createVirtualGuard(
  ...
  const bool skipHCRGuardForCallee =
      getPolicy()->skipHCRGuardForCallee(calleeSymbol->getResolvedMethod());

  bool skipHCRGuardCreation = false;

  // addGuardForVirtual: create an HCRGuard after the original guard
  if (!disableHCRGuards && comp()->getHCRMode() != TR::none &&
      guard->_kind != TR_HCRGuard && !skipHCRGuardForCallee) {
    ...
    // when using OSR to implement HCR we keep the HCR guards distinct since
    // they will undergo special processing later in the compilation
    ...
    if (!skipHCRGuardCreation) {
      hcrBlock =
          TR::Block::createEmptyBlock(callNode, comp(), block1->getFrequency());
      ...
      hcrGuard = new (trStackMemory())
          TR_VirtualGuardSelection(TR_HCRGuard, TR_NonoverriddenTest);
      hcrTreeTop = hcrBlock->append(TR::TreeTop::create(
          comp(),
          createVirtualGuard(
              callNode, calleeSymbol, block4->getEntry(),
              calleeSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex(),
  ...
  if (!disableFSDGuard && comp()->getOption(TR_FullSpeedDebug) &&
      guard->_kind != TR_BreakpointGuard) {
    addAdditionalGuard(callNode, calleeSymbol, thisClass, block1, block2,
                       block4, TR_BreakpointGuard, TR_FSDTest,
                       false /*favourVftCompare*/, callerCFG);
  }

  bool appendTestToBlock1 = false;
  if (guard->_kind == TR_InnerGuard) appendTestToBlock1 = true;
  ...
  getUtil()->refineInlineGuard(callNode, block1, block2, appendTestToBlock1,
                               callerSymbol, cursorTree, virtualGuard, block4);
  ...
  if ((guard->_kind == TR_ProfiledGuard) || (guard->_kind == TR_InnerGuard)) {
    if (block1->getFrequency() < 0)
      ...
      if (guard->isHighProbablityProfiledGuard())
        ...
        block4->setFrequency(
            TR::Block::getScaledSpecializedFrequency(block1->getFrequency()));
    }
  } else {
  ...
  if (tif.resultNode())
    OMR_InlinerUtil::storeValueInATemp(
        comp(), tif.resultNode(), resultTempSymRef,
        block3->getPrevBlock()->getLastRealTreeTop(), callerSymbol, tempList,
        _availableTemps, &_availableBasicBlockTemps);

  //  existing call
  //
  TR::Node *newTreeTopNode =
      (callNodeTreeTop->getNode()->getOpCode().isCheck())
          ? TR::Node::createWithSymRef(
                callNodeTreeTop->getNode()->getOpCodeValue(), 1, callNode, 0,
                callNodeTreeTop->getNode()->getSymbolReference())
          :

          TR::Node::create(TR::treetop, 1, callNode);

  TR::TreeTop *guardedCallNodeTreeTop =
      TR::TreeTop::create(comp(), newTreeTopNode);
  block4->append(guardedCallNodeTreeTop);

  callNode->setIsTheVirtualCallNodeForAGuardedInlinedCall();
  TR::DebugCounter::prependDebugCounter(
      comp(), "inliner.callSites/succeeded:guardedCallee",
      block4->getLastRealTreeTop());

  // if this is postExecution OSR, the call will be followed by a pending push
  // store of its result. this is necessary to ensure the stack has the correct
  // contents when it transitions, therefore, it is necessary to add the store
  // here as well
  //
  if (comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
    ...
    while (cursor && comp()->getMethodSymbol()->isOSRRelatedNode(
                         cursor->getNode(), bci)) {
      if (cursor->getNode()->getOpCode().isStoreDirect() &&
          cursor->getNode()->getFirstChild() == callNode) {
        debugTrace(tracer(),
                   "  virtual call node pps: Pending push store of call [%p] "
                   "found: [%p]",
                   callNode, cursor->getNode());
        block4->append(TR::TreeTop::create(
            comp(), TR::Node::createStore(
                        cursor->getNode()->getSymbolReference(), callNode)));
        break;
      }
      debugTrace(tracer(),
                 "  virtual call node pps: Skipping node [%p] whilst searching "
                 "for store of call [%p]",
  ...
  TR_ASSERT(!resultTempSymRef || callNode->getReferenceCount() > 2,
            "why do we have a resultTempSymRef when the callNode isn't "
            "referenced again?");

  if (tif.simpleCallReferenceTreeTop())
    cloneAndReplaceCallNodeReference(tif.simpleCallReferenceTreeTop(), 0, 0,
                                     block4->getLastRealTreeTop(), comp());

  else if (callNode->getReferenceCount() > 2)
    OMR_InlinerUtil::storeValueInATemp(
        comp(), callNode, resultTempSymRef, block4->getLastRealTreeTop(),
  ...
  if (block4->getLastRealTreeTop()->getNode()->getOpCode().isReturn()) {
    ...
    lastTreeInBlock4 = block4->append(TR::TreeTop::create(
        comp(), TR::Node::create(callNode, TR::Goto, 0, block3->getEntry())));
  ...
  //
  if (resultTempSymRef) {
    TR::Node *loadOfResultTemp =
        TR::Node::createLoad(callNode, resultTempSymRef);
  ...
  debugTrace(tracer(),
             ...
             calltarget->_myCallSite, calltarget->_myCallSite->_callNodeTreeTop,
  ...
  static const char *osrForHCRGuardsOfIndirect =
      feGetEnv("TR_OSRForHCRAndIndirect");
  ...
  // a failed guard can be handled in one of two ways: 1) we simply branch to a
  // block which will do a virtual call on the correct receiver for the method
  // we want to run or 2) we could transfer control from the compiled code back
  // to the interpreter using an On-Stack-Replacement (OSR) mechanism.
  //
  // Hot-Code-Replace or HCR mode is where the compiler is running in a mode
  // that assumes methods could be redefined at runtime. The compiler can
  // support this mode using traditional virtual guards with calls or by using
  // OSR elsewhere in the compiler.
  //
  // When running with HCR implemented using OSR plain HCR guards will be
  // processed later in the compilation and those later processes will handle
  // them using OSR so we don't want to complicate that with additional OSR at
  // this point

  // Late inlining may result in callerSymbol not being the resolved method that
  // actually calls the inlined method This is problematic for linking OSR
  // blocks
  TR::ResolvedMethodSymbol *callingMethod =
      callNode->getByteCodeInfo().getCallerIndex() == -1
          ? comp()->getMethodSymbol()
          : comp()->getInlinedResolvedMethodSymbol(
                callNode->getByteCodeInfo().getCallerIndex());

  if ((comp()->getHCRMode() != TR::osr || guard->_kind != TR_HCRGuard) &&
      callingMethod->supportsInduceOSR(callNode->getByteCodeInfo(), block1,
                                       comp(), false)) {
    bool shouldUseOSR = heuristicForUsingOSR(
        callNode, calleeSymbol, callerSymbol, createdHCRAndVirtualGuard);

    if (shouldUseOSR &&
        (osrAll || comp()->getOption(TR_EnableOSROnGuardFailure) ||
         (createdHCRAndVirtualGuard && shouldAttemptOSR &&
          osrForHCRGuardsOfIndirect) ||
         createdHCRGuard || (osrForNonHCRGuards && shouldAttemptOSR))) {
      TR::TreeTop *induceTree = callingMethod->genInduceOSRCall(
          guardedCallNodeTreeTop, callNode->getByteCodeInfo().getCallerIndex(),
          (callNode->getNumChildren() - callNode->getFirstArgumentIndex()),
          false, false, callerSymbol->getFlowGraph());
      if (induceOSRCallTree) *induceOSRCallTree = induceTree;
    }
  }
...
bool TR_InlinerBase::heuristicForUsingOSR(
    TR::Node *callNode, TR::ResolvedMethodSymbol *calleeSymbol,
  ...
  static int32_t maxOSRDepth =
      (OSRDepth = feGetEnv("TR_MaxOSRDepth")) ? atoi(OSRDepth) : 100000;

  static const char *OSRThreshold;
  static int32_t calleeThresh =
      (OSRThreshold = feGetEnv("TR_OSRCalleeThreshold")) ? atoi(OSRThreshold)
                                                         : 100000;

  static const char *OSRIndirectThreshold;
  static int32_t calleeIndirectThresh =
      (OSRIndirectThreshold = feGetEnv("TR_OSRIndirectCalleeThreshold"))
          ? atoi(OSRIndirectThreshold)
          : 100000;

  static const char *OSRStackThreshold;
  static int32_t calleeStackThresh =
      (OSRStackThreshold = feGetEnv("TR_OSRCalleeStackThreshold"))
          ? atoi(OSRStackThreshold)
          : 100000;

  static const char *OSRThreshold2;
  static int32_t callerThresh =
      (OSRThreshold2 = feGetEnv("TR_OSRCallerThreshold")) ? atoi(OSRThreshold2)
                                                          : 100000;

  static const char *OSRStackThreshold2;
  static int32_t callerStackThresh =
      (OSRStackThreshold2 = feGetEnv("TR_OSRCallerStackThreshold"))
          ? atoi(OSRStackThreshold2)
          : 100000;

  static const char *OSRStackThreshold3;
  static int32_t callerLiveStackThresh =
      (OSRStackThreshold3 = feGetEnv("TR_OSRCallerLiveStackThreshold"))
          ? atoi(OSRStackThreshold3)
          : 100000;

  static const char *OSRStackThreshold4;
  static int32_t callerLivePendingThresh =
      (OSRStackThreshold4 = feGetEnv("TR_OSRCallerLivePendingThreshold"))
          ? atoi(OSRStackThreshold4)
          : 100000;

  TR_ResolvedMethod *calleeMethod = calleeSymbol->getResolvedMethod();
  int32_t calleeNumStackSlots = calleeMethod->numberOfParameterSlots() +
                                calleeMethod->numberOfTemps() +
  ...
  if (calleeNumStackSlots > calleeStackThresh) return false;

  if (getPolicy()->getInitialBytecodeSize(calleeSymbol, comp()) > calleeThresh)
    return false;

  if (disallowNestedOSR) {
    if (callNode->getByteCodeInfo().getCallerIndex() != -1) return false;
  }
  ...
  TR_OSRMethodData *osrMethodData =
      comp()->getOSRCompilationData()->findCallerOSRMethodData(
          comp()->getOSRCompilationData()->findOrCreateOSRMethodData(
              comp()->getCurrentInlinedSiteIndex(), calleeSymbol));
  ...
  while (osrMethodData) {
    ...
    TR::ResolvedMethodSymbol *osrCallerSymbol =
        osrMethodData->getMethodSymbol();
    int32_t osrCallerSize =
        getPolicy()->getInitialBytecodeSize(osrCallerSymbol, comp());
    totalOSRCallersSize = totalOSRCallersSize + osrCallerSize;

    TR_ResolvedMethod *osrCallerMethod = osrCallerSymbol->getResolvedMethod();
    int32_t osrCallerNumStackSlots = osrCallerMethod->numberOfParameterSlots() +
                                     osrCallerMethod->numberOfTemps() +
                                     osrCallerMethod->numberOfPendingPushes();
    int32_t osrCallerNumLiveStackSlots = 0;
    totalOSRCallersStackSlots =
        totalOSRCallersStackSlots + osrCallerNumStackSlots;

    TR_BitVector *deadSymRefs = osrMethodData->getLiveRangeInfo(byteCodeIndex);
    if (deadSymRefs) {
      osrCallerNumLiveStackSlots =
          osrMethodData->getNumSymRefs() - deadSymRefs->elementCount();
      totalOSRCallersLiveStackSlots =
          totalOSRCallersLiveStackSlots + osrCallerNumLiveStackSlots;
    } else {
    ...
    TR_Array<List<TR::SymbolReference> > *pendingPushSymRefs =
        osrCallerSymbol->getPendingPushSymRefs();
    int32_t numPendingSlots = 0;
    int32_t numLivePendingPushSlots = 0;
    if (pendingPushSymRefs) numPendingSlots = pendingPushSymRefs->size();
    ...
    for (int32_t i = 0; i < numPendingSlots; i++) {
      ...
      for (nextSymRef = symRefsIt.getCurrent(); nextSymRef;
           nextSymRef = symRefsIt.getNext()) {
        if (!deadSymRefs || !deadSymRefs->get(nextSymRef->getReferenceNumber()))
          numLivePendingPushSlots++;
      }
    }

    totalOSRLivePendingPushSlots =
        totalOSRLivePendingPushSlots + numLivePendingPushSlots;

    if (comp()->getOption(TR_TraceOSR))
      traceMsg(
          comp(),
          "OSR caller at inlined site index %d has %d bytecodes and %d stack "
          "slots, total callers bytecodes %d total callers stack slots %d "
          "total callers live stack slots %d total pending push slots %d\n",
          osrMethodData->getInlinedSiteIndex(), osrCallerSize,
          osrCallerNumStackSlots, totalOSRCallersSize,
          totalOSRCallersStackSlots, totalOSRCallersLiveStackSlots,
          totalOSRLivePendingPushSlots);
    ...
    if (osrMethodData->getInlinedSiteIndex() > -1) {
      ...
      osrMethodData = comp()->getOSRCompilationData()->findCallerOSRMethodData(
          osrMethodData);
...
TR_ParameterToArgumentMapper::TR_ParameterToArgumentMapper(
    TR::ResolvedMethodSymbol *callerSymbol,
    TR::ResolvedMethodSymbol *calleeSymbol, TR::Node *callNode,
    TR_PrexArgInfo *argInfo, List<TR::SymbolReference> &temps,
    List<TR::SymbolReference> &availableTemps,
...
void TR_ParameterToArgumentMapper::printMapping() {
  if (!tracer()->debugLevel()) return;
  for (TR_ParameterMapping *pm = _mappings.getFirst(); pm; pm = pm->getNext()) {
    debugTrace(tracer(),
               "Mapping at addr %p:\n\tparmSymbol = %p (offset %d) "
               "\treplacementSymRef = %d\t_parameterNode = %p\n"
               "\t_argIndex = %d\t_parmIsModified = %d\t_isConst = "
               "%d\t_addressTaken =%d",
               pm, pm->_parmSymbol, pm->_parmSymbol->getOffset(),
               pm->_replacementSymRef
                   ? pm->_replacementSymRef->getReferenceNumber()
                   : -1,
               pm->_parameterNode, pm->_argIndex, pm->_parmIsModified,
               pm->_isConst, pm->_addressTaken);
  }
}
...
void TR_ParameterToArgumentMapper::initialize(TR_CallStack *callStack) {
  ...
  if (_callNode->getOpCode().isCallIndirect()) {
    ...
    if (arg->getReferenceCount() > 1) {
      ...
      _firstTempTreeTop = _lastTempTreeTop = OMR_InlinerUtil::storeValueInATemp(
          comp(), arg, _vftReplacementSymRef, 0, _calleeSymbol, _tempList,
  ...
  for (TR_CallStack *cs = callStack; hasStaticCallStack && cs;
       cs = cs->getNext())
    hasStaticCallStack = cs->_method->isStatic();

  bool neverNeedPrivatizedArguments =
      _inliner->getPolicy()->dontPrivatizeArgumentsForRecognizedMethod(
  ...
  for (uint32_t argOffset = 0; parmMap; ++argIndex) {
    ...
    if (parmMap->_parmSymbol->getParameterOffset() == argOffset) {
      ...
      if (!parmMap->_parmIsModified &&
          (!arg->getOpCode().isFloatingPoint() ||
           comp()->cg()->getSupportsJavaFloatSemantics())) {
        if (parmMap->_addressTaken) {
          if (arg->getOpCode().isLoadVarDirect() &&
              arg->getReferenceCount() == 1 && arg->getSymbol()->isAutoOrParm())
            parmMap->_replacementSymRef = arg->getSymbolReference();
        } else {
          debugTrace(tracer(),
                     "Setting parameterNode to n%in %s, argOffset=%d, "
                     "argIndex=%d, _callNode n%in",
                     arg->getGlobalIndex(), arg->getOpCode().getName(),
                     argOffset, argIndex, _callNode->getGlobalIndex());
          parmMap->_parameterNode = arg;

          static const char *disableParmTempOpt =
              feGetEnv("TR_DisableParmTempOpt");
            ...
            if ((arg->getOpCode().isLoadVarDirect() &&
                ...
                ((arg->getReferenceCount() == 1) ||
                 (_callNode->getOpCode().isCallIndirect() &&
                  ...
                  (arg->getReferenceCount() == 2)))) {
              debugTrace(
                  tracer(), "Setting parmMap %p -> _replacementSymref to %d",
                  parmMap, arg->getSymbolReference()->getReferenceNumber());
              debugTrace(tracer(),
                         "isCallIndirect = %d getFirstArgumentIndex = %d  arg "
                         "isAuto = %d",
                         _callNode->getOpCode().isCallIndirect(),
                         _callNode->getFirstArgumentIndex(),
                         arg->getSymbol()->isAuto());
              parmMap->_replacementSymRef = arg->getSymbolReference();
            } else if (rematerializeConstant(arg, comp()))
              parmMap->_isConst = true;
      ...
      if (parmMap->_isConst) {
        // create treetop for constant arg so that inliner can later store the
        // arg into correct register
        tt = TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, arg));
      } else {
        if (parmMap->_replacementSymRef)
          tt = TR::TreeTop::create(comp(),
                                   TR::Node::create(TR::treetop, 1, arg));
        else {
          TR::SymbolReference *symRef = NULL;
          const static bool disableUseKnownObjectTempsForParms =
              feGetEnv("TR_DisableUseKnownObjectTempsForParmsInCallee") ? true
                                                                        : false;
          ...
          if (!disableUseKnownObjectTempsForParms && prexArgument &&
            ...
            symRef =
                comp()
                    ->getSymRefTab()
                    ->findOrCreateTemporaryWithKnowObjectIndex(
                        _callerSymbol, prexArgument->getKnownObjectIndex());
            debugTrace(tracer(),
                       "map arg %p into known object temp #%d as priv arg\n",
                       arg, symRef->getReferenceNumber());
          }

          tt = OMR_InlinerUtil::storeValueInATemp(
              comp(), arg, symRef, 0, _calleeSymbol, _tempList, _availableTemps,
              &_availableTemps2, false, &newValueStoreTreeTop);
          ...
          if (!hasStaticCallStack && !neverNeedPrivatizedArguments &&
              tt->getNode()
                  ->getOpCode()
                  .isStoreDirectOrReg())  // compjazz 53912: PLX sometimes
                                          // privatizes using indirect stores
          {
          ...
          if (!disableUseKnownObjectTempsForParms && prexArgument &&
              ...
              TR_PrexArgument::knowledgeLevel(prexArgument) == KNOWN_OBJECT &&
              prexArgument
                  ->isTypeInfoForInlinedBody())  // use known object temp in
                                                 // inlined body only
          {
            ...
            TR::SymbolReference *knownObjectTempSymRef =
                comp()
                    ...
                    ->findOrCreateTemporaryWithKnowObjectIndex(
                        _callerSymbol, prexArgument->getKnownObjectIndex());
            TR::TreeTop *storeToKnownObjTemp = TR::TreeTop::create(
                comp(), TR::Node::createStore(
                            knownObjectTempSymRef,
                            TR::Node::createLoad(
                                _calleeSymbol->getFirstTreeTop()->getNode(),
                                origTempSymRef)));
            storeToKnownObjTemp->insertNewTreeTop(
                _calleeSymbol->getFirstTreeTop(),
                _calleeSymbol->getFirstTreeTop()->getNextTreeTop());
            parmMap->_replacementSymRefForInlinedBody = knownObjectTempSymRef;
            debugTrace(tracer(),
                       "created tree n%dn to store the priv arg #%d into a "
                       "known object temp #%d on entry of the inlined body\n",
                       storeToKnownObjTemp->getNode()->getGlobalIndex(),
...
void TR_ParameterToArgumentMapper::lookForModifiedParameters() {
  ...
  for (TR::PreorderNodeIterator it(_calleeSymbol->getFirstTreeTop(), comp());
       it.currentTree() != NULL; ++it)
    lookForModifiedParameters(it.currentNode());
}

TR_ParameterMapping *TR_ParameterToArgumentMapper::findMapping(
    TR::Symbol *symbol) {
  for (TR_ParameterMapping *pm = _mappings.getFirst(); pm; pm = pm->getNext())
    if (pm->_parmSymbol == symbol) return pm;
  return 0;
}
...
/*
 * The OSRCallSiteRematTables for this inlined method and those inlined within
 * it may contain symbol references for parms that have been mapped to args.
 * Therefore, its necessary to update the tables based on the mapper.
 *
 * This will only be applied in voluntary OSR when induction is still possible.
 */
void TR_ParameterToArgumentMapper::mapOSRCallSiteRematTable(
    uint32_t siteIndex) {
  if (!comp()->getOption(TR_EnableOSR) ||
  ...
  for (uint32_t i = 0; i < comp()->getOSRCallSiteRematSize(siteIndex); ++i) {
    ...
    if (!ppSymRef || !loadSymRef) continue;
    TR::Symbol *symbol = loadSymRef->getSymbol();
    if (!symbol->isParm()) continue;
    ...
    for (; parmMap; parmMap = parmMap->getNext())
      if (symbol == parmMap->_parmSymbol) {
        if (parmMap->_isConst) {
          ...
          TR::DebugCounter::incStaticDebugCounter(
              comp(), TR::DebugCounter::debugCounterName(
                          comp(), "osrCallSiteRemat/mapParm/const/(%s)",
                          comp()->signature()));
        } else if (loadSymRef->getOffset() > 0) {
          comp()->setOSRCallSiteRemat(siteIndex, ppSymRef, NULL);
          TR::DebugCounter::incStaticDebugCounter(
              comp(), TR::DebugCounter::debugCounterName(
                          comp(), "osrCallSiteRemat/mapParm/addr/(%s)",
                          comp()->signature()));
        } else {
          comp()->setOSRCallSiteRemat(siteIndex, ppSymRef,
                                      parmMap->_replacementSymRef);
          TR::DebugCounter::incStaticDebugCounter(
              comp(), TR::DebugCounter::debugCounterName(
                          comp(), "osrCallSiteRemat/mapParm/success/(%s)",
                          comp()->signature()));
        }
        break;
      }
    ...
    if (!parmMap)
      TR::DebugCounter::incStaticDebugCounter(
          comp(), TR::DebugCounter::debugCounterName(
                      comp(), "osrCallSiteRemat/mapParm/missing/(%s)",
                      comp()->signature()));
  }

  // Update the remat tables for calls within the current
  for (int32_t childIndex = 0; childIndex < comp()->getNumInlinedCallSites();
       ++childIndex) {
...
TR::Node *TR_ParameterToArgumentMapper::map(TR::Node *node,
                                            TR::ParameterSymbol *parm,
                                            bool seenBBStart) {
  TR_InlinerDelimiter delimiter(tracer(), "pam.map");

  for (TR_ParameterMapping *parmMap = _mappings.getFirst(); parmMap;
       parmMap = parmMap->getNext())
    if (parm == parmMap->_parmSymbol) {
      ...
      if (parmMap->_isConst) {
        TR_ASSERT(parmMap->_parameterNode,
                  "Inlining, expected to have a replacementNode");
      ...
      if (!parmMap->_parmIsModified &&
          parmMap->_replacementSymRefForInlinedBody &&
          performTransformation(
              comp(),
              "%s set symRef on node n%dn to be known object symRef %p\n",
...
TR::Node *TR_ParameterToArgumentMapper::fixCallNodeArgs(
    bool createNullCheckReference) {
  ...
  for (TR_ParameterMapping *parmMap = _mappings.getFirst(); parmMap;
    ...
    if (parmMap->_replacementSymRef) {
      ...
      TR::Node *arg =
          TR::Node::createLoad(_callNode, parmMap->_replacementSymRef);
      _callNode->setAndIncChild(i, arg);
    }

    if ((i == _callNode->getFirstArgumentIndex()) && !_vftReplacementSymRef &&
        parmMap->_replacementSymRef &&
        _callNode->getOpCode().isCallIndirect()) {
      TR::Node *vftNode = _callNode->getChild(0);
      if (vftNode->getOpCode().hasSymbolReference() &&
          (vftNode->getSymbolReference() ==
    ...
    if (createNullCheckReference && i == _callNode->getFirstArgumentIndex()) {
      if (parmMap->_replacementSymRef) {
        ...
        TR_ASSERT(parmMap->_parameterNode->getInt() == 0,
                  "a const this arg should be an aconst 0");
        nullCheckReference = TR::Node::aconst(_callNode, 0);
      }
    } else if (parmMap->_isConst &&
               _callNode->getChild(i)->getReferenceCount() > 1) {
      _callNode->getChild(i)->decReferenceCount();
...
TR_TransformInlinedFunction::TR_TransformInlinedFunction(
    TR::Compilation *c, TR_InlinerTracer *tracer,
    TR::ResolvedMethodSymbol *callerSymbol,
    TR::ResolvedMethodSymbol *calleeSymbol, TR::Block *callNodeBlock,
    TR::TreeTop *callNodeTreeTop, TR::Node *callNode,
...
/**
 * this is the basic implementation and project specific implementation should
 * always call the basic implementation first
 */
void TR_TransformInlinedFunction::transform() {
  ...
  TR::Block *b = NULL;
  TR::Block *firstBlock =
      _calleeSymbol->getFirstTreeTop()->getNode()->getBlock();
  ...
  // If the first block has exception predecessors or multiply predecessors then
  // we can't merge the first block with the caller's block
  //
  if (comp()->getOption(TR_EnableJProfiling) ||
      (firstBlock->getPredecessors().size() > 1) ||
      firstBlock->hasExceptionSuccessors() ||
      comp()->fe()->isMethodTracingEnabled(
  ...
  // If the last block doesn't end with a return (eg it's a throw or a goto)
  // then we need to keep the BBEnd.
  //
  TR::Node *penultimateNode = _penultimateTreeTop->getNode();
  if (!_penultimateTreeTop->getNode()->getOpCode().isReturn() ||
      _firstCatchBlock)
    _generatedLastBlock = TR::Block::createEmptyBlock(
        penultimateNode, comp(), firstBlock->getFrequency(), firstBlock);

  TR::NodeChecklist visitedNodes(comp());
  for (_currentTreeTop = tt; _currentTreeTop;
       _currentTreeTop = _currentTreeTop->getNextTreeTop()) {
    transformNode(_currentTreeTop->getNode(), 0, 0, visitedNodes);
  }

  _parameterMapper.mapOSRCallSiteRematTable(
  ...
  // if the callee is declared to have a return type, but no return was found
  // while walking the trees [this is possible, for example if the callee ends
  // with a 'throw' instead of returning a value]; then create a zero const node
  // (of the return type of the callee) and make this the result
  TR::DataType returnType = _calleeSymbol->getMethod()->returnType();
  if (!_resultNode && returnType != TR::NoType &&
      !_simpleCallReferenceTreeTop && _callNode->getReferenceCount() > 1) {
    _resultNode = TR::Node::create(penultimateNode,
                                   comp()->il.opCodeForConst(returnType), 0);
  ...
  if (_generatedLastBlock) {
    ...
    if (!_firstBBEnd) _firstBBEnd = _lastMainLineTreeTop;
...
void TR_TransformInlinedFunction::transformNode(
  ...
  if (opcode.isReturn()) {
    ...
    if (!_firstBBEnd && _currentTreeTop != _lastMainLineTreeTop &&
        !_processingExceptionHandlers)
      _firstBBEnd = _currentTreeTop;
  } else if (opcode.isCallIndirect() &&
             node->getFirstArgumentIndex() < node->getNumChildren()) {
  ...
  if (node->getOpCodeValue() == TR::athrow) _crossedBasicBlock = true;
  ...
  if (opcode.hasSymbolReference()) {
    ...
    if (symbol->isParm()) {
      TR::Node *newNode = _parameterMapper.map(node, symbol->getParmSymbol(),
                                               _crossedBasicBlock);

      if (newNode && newNode != node) {
        if (newNode->getOpCode().isLoadConst() &&
            newNode->getType().isInt32() && node->getType().isInt8()) {
          // do not create bstore/iconst -- work-around for minor type
          // incorrectness in spec2006 hmmer (type mismatch but iconst actually
          // fits in a byte)
          ...
          newNode->setReferenceCount(1);
          dumpOptDetails(comp(),
                         "%screate %s (0x%p) to resolve type mismatch between "
                         "%s (%p) and %s (%p)\n",
                         OPT_DETAILS, newNode->getOpCode().getName(), newNode,
                         newNode->getFirstChild()->getOpCode().getName(),
                         newNode->getFirstChild(), node->getOpCode().getName(),
                         node);
        }
        ...
        if (newIsIntegral && oldIsIntegral &&
          ...
          newNode = TR::Node::create(
              TR::ILOpCode::getProperConversion(newNode->getDataType(),
...
void TR_TransformInlinedFunction::transformReturn(TR::Node *returnNode,
                                                  TR::Node *parent) {
  TR_ASSERT(!parent, "Inlining, a return has a parent node?");
  bool isAtEOF = (_currentTreeTop == _penultimateTreeTop && !_firstCatchBlock);

  // traceMsg(TR::comp(), "Transform Return: returnNode = %p, parent =
  // %p\n",returnNode,parent);

  if (returnNode->getNumChildren() && _callNode->getReferenceCount() > 1) {
    TR_ASSERT(returnNode->getNumChildren() == 1,
              "Inlining, a return has more than one child?");
    ...
    if (_determineIfReturnCanBeReplacedWithCallNodeReference) {
      ...
      _simpleCallReferenceTreeTop =
          findSimpleCallReference(_callNodeTreeTop, _callNode);
    }

    TR::Node *returnValue = returnNode->getFirstChild();
    if (_callNode->isNonNegative()) returnValue->setIsNonNegative(true);

    if (_simpleCallReferenceTreeTop) {
      cloneAndReplaceCallNodeReference(
          _simpleCallReferenceTreeTop, _callNode, returnValue,
      ...
      OMR_InlinerUtil::storeValueInATemp(comp(), returnValue, _resultTempSymRef,
                                         _currentTreeTop->getPrevTreeTop(),
                                         _callerSymbol, _tempList,
  ...
  if (!isAtEOF && !wasReturnTo) {
    if (!_generatedLastBlock)
      _generatedLastBlock =
          TR::Block::createEmptyBlock(returnNode, comp(), -1, firstBlock);

    TR::Node *gotoNode = TR::Node::create(returnNode, TR::Goto, 0,
                                          _generatedLastBlock->getEntry());
  ...
  //
  for (auto e = currentBlock->getSuccessors().begin();
       e != currentBlock->getSuccessors().end(); ++e)
...
TR_HandleInjectedBasicBlock::TR_HandleInjectedBasicBlock(
  ...
  if (!_tracer)
    _tracer = new (comp->trHeapMemory()) TR_InlinerTracer(
        comp, comp->fe(), NULL);  // if I wasn't provided a tracer, just use a
                                  // dummy one.  No tracing will occur.
}

void TR_HandleInjectedBasicBlock::printNodesWithMultipleReferences() {
  if (!tracer()->debugLevel()) return;
  for (MultiplyReferencedNode *mn = _multiplyReferencedNodes.getFirst(); mn;
       mn = mn->getNext()) {
    debugTrace(
        tracer(),
        "MultiplyReferencedNode = %p\ttreetop = %p\n\treplacementSymRef "
        "=%d\t_referencesToBeFound = %d"
        "\tisConst = %d\tsymbolCanBeReloaded = %d",
        mn->_node, mn->_treeTop,
        mn->_replacementSymRef ? mn->_replacementSymRef->getReferenceNumber()
                               : -1,
...
void TR_HandleInjectedBasicBlock::findAndReplaceReferences(
    TR::TreeTop *callBBStart, TR::Block *replaceBlock1,
    TR::Block *replaceBlock2) {
  TR_InlinerDelimiter delimiter(tracer(), "hibb.findAndReplaceReferences");
  debugTrace(tracer(),
             "replaceBlock1 = %d replaceBlock2 = %d callBBStart->getNode = %p",
             replaceBlock1 ? replaceBlock1->getNumber() : -1,
             replaceBlock2 ? replaceBlock2->getNumber() : -1,
             callBBStart->getNode());
  ...
  if (replaceBlock2) {
    TR::TreeTop *tt = replaceBlock2->getEntry(),
                *locationForTemp = lastBlock->getLastRealTreeTop();
...
void TR_HandleInjectedBasicBlock::collectNodesWithMultipleReferences(
    TR::TreeTop *tt, TR::Node *parent, TR::Node *node) {
...
void TR_HandleInjectedBasicBlock::replaceNodesReferencedFromAbove(
    TR::Block *block, TR::NodeChecklist &visitedNodes) {
  TR::Block *lastBlock = block;
  while (lastBlock->getNextBlock() &&
         lastBlock->getNextBlock()->isExtensionOfPreviousBlock())
    lastBlock = lastBlock->getNextBlock();
  TR::TreeTop *tt = block->getEntry();
  for (; _multiplyReferencedNodes.getFirst() && tt != lastBlock->getExit();
       tt = tt->getNextTreeTop())
    replaceNodesReferencedFromAbove(tt, tt->getNode(), 0, 0, visitedNodes);
}

void TR_HandleInjectedBasicBlock::replaceNodesReferencedFromAbove(
    TR::TreeTop *tt, TR::Node *node, TR::Node *parent, uint32_t childIndex,
    TR::NodeChecklist &visitedNodes) {
  ...
  if (node->getReferenceCount() > 1 && (found = find(node))) {
    ...
    for (int32_t i = 0; i < node->getNumChildren(); ++i)
      replaceNodesReferencedFromAbove(tt, node->getChild(i), node, i,
                                      visitedNodes);
  }
}
...
void TR_HandleInjectedBasicBlock::createTemps(bool replaceAllReferences) {
  ...
  for (MultiplyReferencedNode *ref = _multiplyReferencedNodes.getFirst(); ref;
       ref = ref->getNext()) {
    ...
    if (rematConstant || opcode.getOpCodeValue() == TR::loadaddr) {
      ...
      static const char *enabletempCreationOpt =
          feGetEnv("TR_EnableTempCreationOpt");
      // This opt seems risky, and quite bad.
      // If this is enabled and TR_DisableParmTempOpt is not set, there will for
      // sure be problems. There possibly could be problems even when
      // TR_EnableParmTempOpt is set. The real underlying issue is that just
      // because a symref is on a parmMap, it doesn't mean you can automatically
      // add it to a list of injected basic block temps When I enabled the
      // ParmTempOpt, this is what happened.  An auto symbol got added to the
      // injectedBasicBlockTemps list, and then was reused in a very bad manner.
      if (enabletempCreationOpt) {
        for (TR_ParameterMapping *parmMap = _mappings; parmMap;
             parmMap = parmMap->getNext()) {
          if (parmMap->_parameterNode == ref->_node) {
            symRef = parmMap->_replacementSymRef;

            debugTrace(_tracer,
                       "\nadding %d to injected basic block temps for node %p "
                       "is in temp list %d",
                       symRef->getReferenceNumber(), ref->_node,
                       _tempList.find(symRef));
      ...
      if (!symRef) {
        ...
        // If this treetop is an OSR point, a store cannot be placed between it
        // and the transition treetop in postExecutionOSR
        ...
        if (comp()->fe()->dataTypeForLoadOrStore(nodeDataType) !=
            nodeDataType) {
          TR::ILOpCodes convOpCode = TR::ILOpCode::getProperConversion(
              nodeDataType, comp()->fe()->dataTypeForLoadOrStore(nodeDataType),
              false);
          TR::Node *valueToTempConv = TR::Node::create(convOpCode, 1, value);
          value = valueToTempConv;
        }

        if (value->getOpCode().hasSymbolReference() &&
            value->getSymbolReference()->hasKnownObjectIndex())
          symRef =
        ...
        OMR_InlinerUtil::storeValueInATemp(
            comp(), value, symRef, tt, _methodSymbol, _injectedBasicBlockTemps,
            _availableTemps, 0);
      }
...
TR::Node *TR_HandleInjectedBasicBlock::findNullCheckReferenceSymbolReference(
    TR::TreeTop *nullCheckTreeTop) {
  TR::Node *nullCheckReference =
      nullCheckTreeTop->getNode()->getNullCheckReference();
  MultiplyReferencedNode *ref;
  for (ref = _fixedNodes.getFirst(); ref; ref = ref->getNext())
    if (ref->_node == nullCheckReference) break;

  // TR_ASSERT(ref && ref->_replacementSymRef, "couldn't find temp for null
  // check reference");
  if (ref == NULL) return NULL;

  nullCheckReference =
      TR::Node::createLoad(ref->_node, ref->_replacementSymRef);
  return nullCheckReference;
}
...
TR_HandleInjectedBasicBlock::find(TR::Node *node) {
  for (MultiplyReferencedNode *ref = _multiplyReferencedNodes.getFirst(); ref;
       ref = ref->getNext())
    if (ref->_node == node) return ref;
  return 0;
}

TR_HandleInjectedBasicBlock::MultiplyReferencedNode::MultiplyReferencedNode(
    TR::Node *node, TR::TreeTop *tt, uint32_t refsToBeFound,
    bool symCanBeReloaded)
    : _node(node),
      _treeTop(tt),
...
void TR_HandleInjectedBasicBlock::add(TR::TreeTop *tt, TR::Node *node) {
  MultiplyReferencedNode *ref = new (comp()->trStackMemory())
      MultiplyReferencedNode(node, tt, node->getReferenceCount() - 1,
                             node->getOpCode().isLoadVarDirect() &&
                                 node->getSymbol()->isAutoOrParm());
  //   ref->_node = node;
  //   ref->_treeTop = tt;
  //   ref->_referencesToBeFound = node->getReferenceCount() - 1;
  //   ref->_symbolCanBeReloaded = node->getOpCode().isLoadVarDirect() &&
  //   node->getSymbol()->isAutoOrParm();

  _multiplyReferencedNodes.add(ref);
}

void TR_HandleInjectedBasicBlock::replace(MultiplyReferencedNode *ref,
                                          TR::TreeTop *tt, TR::Node *parent,
                                          uint32_t childIndex) {
  ...
  if (ref->_isConst) {
    ...
    if (ref->_node->getOpCodeValue() == TR::loadaddr) {
      ...
      TR::TreeTop *newTree =
          TR::TreeTop::create(comp(), treetopNode, NULL, NULL);
    ...
    // Convert the node just loaded back to the original type in case the FE
    // required us to store it as something else
    if (replacementNode->getDataType() != ref->_node->getDataType()) {
      TR::ILOpCodes convOpCode = TR::ILOpCode::getProperConversion(
          replacementNode->getDataType(), ref->_node->getDataType(), false);
      TR::Node *replacementToRefConv =
          TR::Node::create(convOpCode, 1, replacementNode);
  ...
  if (!parent->getChild(childIndex)->getByteCodeInfo().doNotProfile())
    replacementNode->getByteCodeInfo().setDoNotProfile(
        0);  // allow profiling of replacement node if the original could be
             // profiled
  parent->setChild(childIndex, replacementNode);
}
...
static TR::TreeTop *findPinningArrayStore(TR::Compilation *comp,
                                          TR::TreeTop *prevTT,
                                          TR::Node *value) {
  TR::TreeTop *tt = NULL;
  for (tt = prevTT; tt->getNode()->getOpCodeValue() != TR::BBStart;
       tt = tt->getPrevTreeTop())
    ;
  ...
  for (tt = prevTT; tt != lastTT; tt = tt->getNextTreeTop()) {
    ...
    if (node->getOpCode().isStore() &&
      ...
      if (node->getSymbolReference()->getSymbol() ==
...
TR::TreeTop *OMR_InlinerUtil::storeValueInATemp(
    TR::Compilation *comp, TR::Node *value, TR::SymbolReference *&tempSymRef,
    TR::TreeTop *prevTreeTop, TR::ResolvedMethodSymbol *methodSymbol,
    List<TR::SymbolReference> &tempList,
    List<TR::SymbolReference> &availableTemps,
    List<TR::SymbolReference> *availableTemps2, bool behavesLikeTemp,
    TR::TreeTop **newStoreValueTreeTop, bool isIndirect, int32_t offset) {
  ...
  if (value->isInternalPointer() && value->getPinningArrayPointer()) {
    TR_ASSERT(
        !tempSymRef,
        "storing an internal pointer into a possibly non-internal-ptr temp");

    tempSymRef = comp->getSymRefTab()->createTemporary(methodSymbol,
                                                       TR::Address, true, 0);
    TR::Symbol *symbol =
        tempSymRef->getSymbol()
            ->castToInternalPointerAutoSymbol()
            ->setPinningArrayPointer(value->getPinningArrayPointer());
    ...
    if ((value->isNotCollected() && dataType == TR::Address) || isIndirect) {
      ...
      if (tempSymRef != NULL) {
        ...
        valueRef = comp->getSymRefTab()->createTemporary(
            methodSymbol, dataType, false,
#ifdef J9_PROJECT_SPECIFIC
    ...
    if (isInternalPointer) {
      TR::SymbolReference *valueRef = comp->getSymRefTab()->createTemporary(
          methodSymbol, TR::Address, true, 0);

      if (value->getOpCode().hasSymbolReference() &&
          value->getSymbolReference()->getSymbol()->isNotCollected())
        valueRef->getSymbol()->setNotCollected();
      ...
      if (value->getOpCode().isArrayRef()) {
        ...
        if (valueChild->isInternalPointer() &&
          ...
          if (valueChild->getOpCode().isLoadVarDirect() &&
            ...
            if (valueChild->getSymbolReference()
                    ->getSymbol()
                    ->castToAutoSymbol()
                    ->isInternalPointer()) {
            ...
            TR::SymbolReference *newValueArrayRef =
                comp->getSymRefTab()->createTemporary(methodSymbol, TR::Address,
                                                      false, 0);

            TR::Node *newStoreValue =
                TR::Node::createStore(newValueArrayRef, valueChild);
            newStoreValueTree = TR::TreeTop::create(comp, newStoreValue);
            if (!newValueArrayRef->getSymbol()
                     ->isParm())  // newValueArrayRef could contain a parm
                                  // symbol
            {
            ...
            if (!prevTreeTop) {
              if (!newStoreValueTreeTop)
                TR_ASSERT(
                    0, "no previous treetop and no treetop pointer passed in");
      ...
      valueRef->getSymbol()
          ->castToInternalPointerAutoSymbol()
          ->setPinningArrayPointer(pinningArray);
    ...
    if (newStoreValueTree) {
      ...
      // else
      //  TR_ASSERT(0, "Must specify a place to insert the pinning array
      //  store\n");
    }
  }

  if (dataType == TR::Aggregate) {
    TR_ASSERT(tempSymRef == NULL,
              "Aggregate should not be mapped to symbol yet");
    uint32_t size = value->getSize();
    tempSymRef = new (comp->trHeapMemory()) TR::SymbolReference(
        comp->getSymRefTab(),
  ...
  if (!tempSymRef) {
    if (!value->getOpCode().hasSymbolReference() ||
        !value->getSymbolReference()->getSymbol()->isNotCollected()) {
      tempSymRef = comp->getSymRefTab()->findAvailableAuto(
          availableTemps, dataType, behavesLikeTemp);
      if (tempSymRef && tempSymRef->getSymbol()->isNotCollected())
        tempSymRef = NULL;
      if (!tempSymRef && availableTemps2)
        tempSymRef = comp->getSymRefTab()->findAvailableAuto(
            *availableTemps2, dataType, behavesLikeTemp);
      if (tempSymRef && tempSymRef->getSymbol()->isNotCollected())
        tempSymRef = NULL;
    }
  ...
  TR_ASSERT(comp->il.opCodeForDirectStore(dataType) != TR::BadILOp,
            "Inlining, unexpected data type for temporary");

  TR::Node *storeNode = NULL;
  if (isIndirect) {
    TR::Symbol *symShadow = TR::Symbol::createShadow(
        comp->trHeapMemory(), dataType, value->getSize());
    TR::SymbolReference *symRefShadow =
        comp->getSymRefTab()->createSymbolReference(symShadow);
    TR::Node *storeAddress =
        TR::Node::createWithSymRef(value, TR::loadaddr, 0, tempSymRef);
    storeAddress = TR::Node::createAddConstantToAddress(storeAddress, offset);

    storeNode =
        TR::Node::createWithSymRef(comp->il.opCodeForIndirectStore(dataType), 2,
                                   2, storeAddress, value, symRefShadow);
  } else {
    storeNode = TR::Node::createStore(tempSymRef, value);
  }
  if (comp->cg()->traceBCDCodeGen())
    traceMsg(comp,
             "\tcreate storeNode %p of tempSymRef #%d (possibly for node "
             "uncommoning during opcodeExpansion)\n",
...
static TR::Node *cloneAndReplaceCallNodeReference(TR::Node *originalNode,
                                                  TR::Node *callNode,
                                                  TR::Node *replacementNode,
                                                  TR::Compilation *comp) {
  TR::Node *newNode;
  if (originalNode == callNode)
    ...
    for (uint32_t i = 0; i < originalNode->getNumChildren(); ++i) {
      TR::Node *newChild = cloneAndReplaceCallNodeReference(
          originalNode->getChild(i), callNode, replacementNode, comp);
...
static TR::TreeTop *cloneAndReplaceCallNodeReference(
    TR::TreeTop *callNodeReference, TR::Node *callNode,
    TR::Node *replacementNode, TR::TreeTop *insertAfterTreeTop,
    TR::Compilation *comp) {
  return TR::TreeTop::create(
      comp, insertAfterTreeTop,
      cloneAndReplaceCallNodeReference(callNodeReference->getNode(), callNode,
                                       replacementNode, comp));
}

static void cloneAndReplaceReturnValueReference(TR::TreeTop *ifCmpTreeTop,
                                                TR::TreeTop *rvStoreTreeTop,
                                                TR::Block *curBlock,
                                                TR::CFG *cfg,
                                                TR::Compilation *comp) {
  ...
  TR::TreeTop *newTreeTop =
      TR::TreeTop::create(comp, newNode, ifCmpTreeTop->getPrevTreeTop(),
                          ifCmpTreeTop->getNextTreeTop());
  newTreeTop->join(rvStoreTreeTop->getNextTreeTop());
  rvStoreTreeTop->join(newTreeTop);
}

bool TR_TransformInlinedFunction::findCallNodeInTree(TR::Node *callNode,
                                                     TR::Node *node) {
  if (node == callNode) return true;

  // we cannot use visitcounts here to control the recursion
  // (because the outer transformNode uses them)
  // so abandon the walk if the node was not found even x levels deep in this
  // tree
  //
  if (findCallNodeRecursionDepth == 0) return false;
...
bool TR_TransformInlinedFunction::findReturnValueInTree(TR::Node *rvStoreNode,
                                                        TR::Node *node,
                                                        TR::Compilation *comp) {
  ...
  if (node->getOpCode().isLoadIndirect() &&
    ...
    if (symRef == rvSymRef) return true;
...
bool TR_TransformInlinedFunction::onlyMultiRefNodeIsCallNode(TR::Node *callNode,
                                                             TR::Node *node) {
  if (node != callNode) {
    if (node->getReferenceCount() > 1) return false;

    // stop the recursion
    //
    if (onlyMultiRefNodeIsCallNodeRecursionDepth == 0) return false;
...
TR::TreeTop *TR_TransformInlinedFunction::findSimpleCallReference(
    TR::TreeTop *callNodeTreeTop, TR::Node *callNode) {
  // If the call node's only other reference is in the next tree top
  ...
  if (callNode->getReferenceCount() == 2) {
    ...
    onlyMultiRefNodeIsCallNodeRecursionDepth =
        MAX_FIND_SIMPLE_CALL_REFERENCE_DEPTH;
...
bool TR_CallSite::findCallSiteTarget(TR_CallStack *callStack,
                                     TR_InlinerBase *inliner) {
  TR_ASSERT(0, "TR_CallSite::findCallSiteTarget is not implemented in %s",
            name());
  return false;
}

TR_ResolvedMethod *TR_IndirectCallSite::getResolvedMethod(
    TR_OpaqueClassBlock *klass) {
  return _callerResolvedMethod->getResolvedVirtualMethod(comp(), klass,
                                                         _vftSlot);
}

TR_ResolvedMethod *TR_IndirectCallSite::findSingleJittedImplementer(
    TR_InlinerBase *inliner) {
  return inliner->getUtil()->findSingleJittedImplementer(this);
}

TR_ResolvedMethod *OMR_InlinerUtil::findSingleJittedImplementer(
    TR_IndirectCallSite *callsite) {
  return NULL;
}

bool TR_IndirectCallSite::hasFixedTypeArgInfo() {
  return _ecsPrexArgInfo && _ecsPrexArgInfo->get(0) &&
         _ecsPrexArgInfo->get(0)->classIsFixed();
}

bool TR_IndirectCallSite::hasResolvedTypeArgInfo() {
  return _ecsPrexArgInfo && _ecsPrexArgInfo->get(0) &&
         _ecsPrexArgInfo->get(0)->classIsPreexistent() &&
...
TR_OpaqueClassBlock *TR_IndirectCallSite::getClassFromArgInfo() {
  TR_ASSERT(_ecsPrexArgInfo && _ecsPrexArgInfo->get(0) &&
                _ecsPrexArgInfo->get(0)->getClass(),
            "getClassFromArgInfo is NOT guarded by "
            "hasFixedTypeArgInfo,hasResolvedTypeArgInfo");

  return _ecsPrexArgInfo->get(0)->getClass();
}

bool TR_IndirectCallSite::tryToRefineReceiverClassBasedOnResolvedTypeArgInfo(
    TR_InlinerBase *inliner) {
  if (hasResolvedTypeArgInfo()) {
    TR_OpaqueClassBlock *classFromArgInfo = getClassFromArgInfo();
    if (_receiverClass &&
        comp()->fe()->isInstanceOf(classFromArgInfo, _receiverClass, true,
                                   true) == TR_yes) {
      heuristicTrace(inliner->tracer(), "refining _receiverClass %p to %p",
...
bool TR_IndirectCallSite::findCallTargetUsingArgumentPreexistence(
    TR_InlinerBase *inliner) {
  TR_OpaqueClassBlock *klass = extractAndLogClassArgument(inliner);

  // initialClass might be more specific than the initialMethod or
  // interfaceMethod
  TR_OpaqueClassBlock *initialClass =
      _receiverClass ? _receiverClass : getClassFromMethod();
  ...
  if (!initialClass) {
    heuristicTrace(inliner->tracer(),
                   "ARGS PROPAGATION: couldn't get initialClass\n");
    _ecsPrexArgInfo->set(0, NULL);
    return true;
  }
  ...
  if (!fe()->isInstanceOf(klass, initialClass, true, true)) {
    ...
    heuristicTrace(inliner->tracer(),
                   "The preexistence class (%p) is not compatible with initial "
                   "class (%p)\n"
  ...
  if (!targetMethod) {
    heuristicTrace(inliner->tracer(),
                   "ARGS PROPAGATION: couldn't get targetMethod\n");
    _ecsPrexArgInfo->set(0, NULL);
    return true;
  }
  TR_VirtualGuardSelection *guard = new (comp()->trHeapMemory())
      TR_VirtualGuardSelection(TR_ProfiledGuard, TR_VftTest, klass);
  addTarget(comp()->trMemory(), inliner, guard, targetMethod, klass, heapAlloc);
  return true;
}

bool TR_IndirectCallSite::addTargetIfMethodIsNotOverriden(
    TR_InlinerBase *inliner) {
  if (_initialCalleeMethod &&
      !_initialCalleeMethod->virtualMethodIsOverridden()) {
    if (comp()->compileRelocatableCode() &&
        !TR::Options::getCmdLineOptions()->allowRecompilation())
      return false;  // CHTable is not used when recompilation is disabled,
                     // hence we cannot use assumptions for AOT
    ...
    if (_initialCalleeMethod->isSubjectToPhaseChange(comp())) {
      guard = new (comp()->trHeapMemory())
          TR_VirtualGuardSelection(TR_ProfiledGuard, TR_MethodTest);
    } else {
      guard = new (comp()->trHeapMemory())
          TR_VirtualGuardSelection(TR_NonoverriddenGuard, TR_NonoverriddenTest);
    }

    addTarget(comp()->trMemory(), inliner, guard, _initialCalleeMethod,
...
bool TR_IndirectCallSite::addTargetIfMethodIsNotOverridenInReceiversHierarchy(
    TR_InlinerBase *inliner) {
  return inliner->getUtil()
      ->addTargetIfMethodIsNotOverridenInReceiversHierarchy(this);
}
 ...
 */
bool OMR_InlinerUtil::addTargetIfMethodIsNotOverridenInReceiversHierarchy(
    TR_IndirectCallSite *callsite) {
  return false;
}
 ...
 */
bool OMR_InlinerUtil::addTargetIfThereIsSingleImplementer(
    TR_IndirectCallSite *callsite) {
  return false;
}

bool TR_IndirectCallSite::addTargetIfThereIsSingleImplementer(
    TR_InlinerBase *inliner) {
  return inliner->getUtil()->addTargetIfThereIsSingleImplementer(this);
}

bool TR_IndirectCallSite::findCallSiteTarget(TR_CallStack *callStack,
                                             TR_InlinerBase *inliner) {
...
TR_OpaqueClassBlock *TR_IndirectCallSite::extractAndLogClassArgument(
    TR_InlinerBase *inliner) {
  if (inliner->tracer()->heuristicLevel()) {
    int32_t len;
    const char *fixedReceiverClassName =
        TR::Compiler->cls.classNameChars(comp(), getClassFromArgInfo(), len);
    heuristicTrace(
        inliner->tracer(),
        "Receiver to call is constrained by argument propagation to %s.",
...
bool TR_DirectCallSite::findCallSiteTarget(TR_CallStack *callStack,
                                           TR_InlinerBase *inliner) {
  if (inliner->getPolicy()->replaceSoftwareCheckWithHardwareCheck(
  ...
  const bool skipHCRGuardForCallee =
      inliner->getPolicy()->skipHCRGuardForCallee(_initialCalleeMethod);

  static const char *disableFSDGuard = feGetEnv("TR_DisableFSDGuard");
  if (!disableHCRGuards2 && comp()->getHCRMode() != TR::none &&
      !comp()->compileRelocatableCode() && !skipHCRGuardForCallee) {
    ...
    guard = new (comp()->trHeapMemory())
        TR_VirtualGuardSelection(TR_BreakpointGuard, TR_FSDTest);
  } else {
    TR_VirtualGuardKind kind = TR_NoGuard;
    if (comp()->compileRelocatableCode()) kind = TR_DirectMethodGuard;
  ...
  addTarget(comp()->trMemory(), inliner, guard, _initialCalleeMethod,
            tempreceiverClass, heapAlloc);
  return true;
}

void OMR_InlinerUtil::collectCalleeMethodClassInfo(
    TR_ResolvedMethod *calleeMethod) {
  return;
}

void TR_InlinerBase::getSymbolAndFindInlineTargets(TR_CallStack *callStack,
  ...
  if (callsite->_initialCalleeSymbol) {
    ...
    if (comp()->fe()->isInlineableNativeMethod(
            comp(), callsite->_initialCalleeSymbol)) {
      TR_VirtualGuardSelection *guard =
          new (trStackMemory()) TR_VirtualGuardSelection(TR_NoGuard);
      callsite->addTarget(trMemory(), this, guard,
                          callsite->_initialCalleeSymbol->getResolvedMethod(),
                          callsite->_receiverClass);
    }
  } else if ((isInlineable = static_cast<TR_InlinerFailureReason>(
                  checkInlineableWithoutInitialCalleeSymbol(
                      callsite, comp()))) != InlineableTarget) {
    tracer()->insertCounter(isInlineable, callNodeTreeTop);
    callsite->_failureReason = isInlineable;
    callsite->removeAllTargets(tracer(), isInlineable);
    return;
  ...
  if (callsite->_initialCalleeSymbol &&
      callsite->_initialCalleeSymbol->firstArgumentIsReceiver()) {
    if (callsite->_isIndirectCall && !callsite->_receiverClass) {
      if (callsite->_initialCalleeMethod)
        callsite->_receiverClass =
            callsite->_initialCalleeMethod->classOfMethod();

      int32_t len;
      const char *s = callNode->getChild(callNode->getFirstArgumentIndex())
                          ->getTypeSignature(len);
      TR_OpaqueClassBlock *type =
          s ? fe()->getClassFromSignature(s, len,
                                          callsite->_callerResolvedMethod, true)
            : 0;
      if (type && (!callsite->_receiverClass ||
                   (type != callsite->_receiverClass &&
                    fe()->isInstanceOf(type, callsite->_receiverClass, true,
                                       true) == TR_yes)))
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if (!target->_calleeSymbol ||
        !target->_calleeMethod->isSameMethod(
            target->_calleeSymbol->getResolvedMethod())) {
      target->_calleeMethod->setOwningMethod(
          callsite->_callNode->getSymbolReference()
      ...
      target->_calleeSymbol =
          comp()
              ...
              ->findOrCreateMethodSymbol(
                  symRef->getOwningMethodIndex(), -1, target->_calleeMethod,
                  TR::MethodSymbol::
                      Virtual /* will be target->_calleeMethodKind */)
              ->getSymbol()
              ->castToResolvedMethodSymbol();
    }

    TR_InlinerFailureReason checkInlineableTarget =
        getPolicy()->checkIfTargetInlineable(target, callsite, comp());

    if (checkInlineableTarget != InlineableTarget) {
      tracer()->insertCounter(checkInlineableTarget,
                              callsite->_callNodeTreeTop);
    ...
    TR::RecognizedMethod rm =
        target->_calleeSymbol ? target->_calleeSymbol->getRecognizedMethod()
  ...
  if (callsite->numTargets())  // if we still have targets at this point we can
                               // return true as we would have in the loop
    return;
  ...
  if (callsite->numTargets() > 0 && callsite->getTarget(0) &&
      !callsite->getTarget(0)->_calleeMethod && comp()->trace(OMR::inlining)) {
    traceMsg(comp(), "inliner: method is unresolved: %s into %s\n",
             callsite->_interfaceMethod->signature(trMemory()),
             tracer()->traceSignature(callStack->_methodSymbol));
...
bool OMR_InlinerPolicy::canInlineMethodWhileInstrumenting(
    TR_ResolvedMethod *method) {
  return true;
}

void TR_InlinerBase::applyPolicyToTargets(TR_CallStack *callStack,
                                          TR_CallSite *callsite) {
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    TR_CallTarget *calltarget = callsite->getTarget(i);
    ...
    if (!getPolicy()->canInlineMethodWhileInstrumenting(
    ...
    int32_t selfInliningLimit =
        selfInliningLimitStr
            ? atoi(selfInliningLimitStr)
            : comp()->getMethodSymbol()->doJSR292PerfTweaks()
                  ? 1  // JSR292 methods already do plenty of inlining
                  : 3;
    if (callStack && callStack->isAnywhereOnTheStack(calltarget->_calleeMethod,
                                                     selfInliningLimit)) {
    ...
    if (comp()->getHCRMode() != TR::none) {
      if (calltarget->_guard->_kind != TR_HCRGuard) {
        if ((calltarget->_guard->_kind != TR_NoGuard) &&
            (calltarget->_guard->_kind != TR_InnerGuard))
          realGuard = true;
      }
    } else {
      if ((calltarget->_guard->_kind != TR_NoGuard) &&
    ...
    if (realGuard &&
        (!inlineVirtuals() || comp()->getOption(TR_DisableVirtualInlining))) {
      tracer()->insertCounter(Virtual_Inlining_Disabled,
                              callsite->_callNodeTreeTop);
    ...
    if (comp()->getOption(TR_DisableNonvirtualInlining) && !realGuard) {
      tracer()->insertCounter(NonVirtual_Inlining_Disabled,
                              callsite->_callNodeTreeTop);
    ...
    static const char *dontInlineSyncMethods =
        feGetEnv("TR_DontInlineSyncMethods");
    if (calltarget->_calleeMethod->isSynchronized() &&
        (!inlineSynchronized() ||
         comp()->getOption(TR_DisableSyncMethodInlining))) {
      tracer()->insertCounter(Sync_Method_Inlining_Disabled,
                              callsite->_callNodeTreeTop);
    ...
    if (debug("dontInlineEHAware") &&
        calltarget->_calleeMethod->numberOfExceptionHandlers() > 0) {
    ...
    if (!callsite->_callerResolvedMethod->isStrictFP() &&
        calltarget->_calleeMethod->isStrictFP()) {
...

    {
      ...
      if (regex && !TR::SimpleRegex::match(regex, calltarget->_calleeMethod)) {
        tracer()->insertCounter(Not_InlineOnly_Callee,
                                callsite->_callNodeTreeTop);
    ...
    int32_t bytecodeSize = getPolicy()->getInitialBytecodeSize(
        calltarget->_calleeMethod, calltarget->_calleeSymbol, comp());

    if (!forceInline(calltarget))
      getUtil()->estimateAndRefineBytecodeSize(callsite, calltarget, callStack,
                                               bytecodeSize);

    if (calltarget->_calleeSymbol &&
        strstr(calltarget->_calleeSymbol->signature(trMemory()),
               "FloatingDecimal")) {
      bytecodeSize >>= 1;

      if (comp()->trace(OMR::inlining))
        traceMsg(comp(),
                 "Reducing bytecode size to %d because it's method of "
                 "FloatingDecimal\n",
                 bytecodeSize);
    }
    ...
    if (!forceInline(calltarget) &&
        exceedsSizeThreshold(
            ...
            (callsite->_callerBlock != NULL)
                ...
                : (callsite->_callNodeTreeTop
                       ? callsite->_callNodeTreeTop->getEnclosingBlock()
                       : 0),
            bcInfo, numberOfLocalsInCallee, callsite->_callerResolvedMethod,
            calltarget->_calleeMethod, callsite->_callNode,
            callsite->_allConsts)
      ...
      if (toInline) {
        if (comp()->trace(OMR::inlining))
          traceMsg(comp(),
                   "tryToInline pattern matched.  Skipping size check for %s\n",
                   calltarget->_calleeMethod->signature(comp()->trMemory()));
      ...
      if (toInline) {
        if (comp()->trace(OMR::inlining))
          traceMsg(
              comp(),
              "tryToInline pattern matched.  Within the size check for %s\n",
              calltarget->_calleeMethod->signature(comp()->trMemory()));
...
static void traceRequestToCSI(TR::Compilation *comp, TR_CallSite *callsite) {
  ...
  for (int32_t depth = stack.size() - 1; depth >= -1; depth--) {
    const char *methodName = (depth == -1)
                                 ? comp->signature()
                                 : comp->getInlinedResolvedMethod(stack[depth])
                                       ->signature(comp->trMemory());
    uint32_t offsetInInliner = (depth + 1 == stack.size())
                                   ? callsite->_bcInfo.getByteCodeIndex()
                                   : comp->getInlinedCallSite(stack[depth + 1])
                                         ._byteCodeInfo.getByteCodeIndex();
    fprintf(fout, "STACK : %p %s %d [%d]\n", comp, methodName, offsetInInliner,
  ...
  for (;;)
     ...
     fprintf(fout, "%p %s %d\n",
           comp,
           (callerIndex == -1) ? comp->signature() :
  comp->getInlinedResolvedMethod(stack[callerIndex])->signature
  (comp->trMemory()) , calleeByteCodeIndex); fflush(fout);

     if (callerIndex == -1)
        break;

     calleeByteCodeIndex = comp->getInlinedCallSite(stack[
  callerIndex])._byteCodeInfo.getByteCodeIndex(); callerIndex =
  comp->getInlinedCallSite(stack[ callerIndex])._byteCodeInfo.getCallerIndex();
     }
   */
}

TR_CallSite *TR_InlinerBase::findAndUpdateCallSiteInGraph(
    TR_CallStack *callStack, TR_ByteCodeInfo &bcInfo, TR::TreeTop *tt,
    TR::Node *parent, TR::Node *callNode, TR_CallTarget *calltarget) {
  if (calltarget->_myCallees.isEmpty()) {
    debugTrace(
        tracer(),
        "findAndUpdateCallsiteInGraaph: calltarget %p has empty _myCallees",
        calltarget);
    return 0;
  }
  ...
  for (callsite = calltarget->_myCallees.getFirst(); callsite;
       callsite = callsite->getNext()) {
    debugTrace(tracer(),
               "callNode->getByteCodeIndex = %d callsite->_byteCodeIndex = %d",
               callNode->getByteCodeIndex(), callsite->_byteCodeIndex);
    if (callNode->getByteCodeIndex() ==
        callsite->_byteCodeIndex)  // we have the call
    {
      foundCall = true;
      break;
    }
  ...
  if (foundCall && callNode->getSymbolReference()->isUnresolved() &&
      !callsite->isInterface()) {
    debugTrace(tracer(),
               "findAndUpdateCallsiteInGraaph: call at node %p is unresolved. "
               "Bail out",
               callNode);
    return 0;
  }
  ...
  if (callNode->isTheVirtualCallNodeForAGuardedInlinedCall()) {
    heuristicTrace(tracer(),
                   "findAndUpdateCallSiteInGraph: this call %p is on a cold "
                   "side. Bail out",
                   callNode);
    return 0;
  }
  ...
  if (comp()->getOptions()->enableDebugCounters()) {
    if (!foundCall) {
      for (callsite = calltarget->_deletedCallees.getFirst(); callsite;
           callsite = callsite->getNext()) {
        debugTrace(tracer(),
                   "considering deleted callee %p callNode->getByteCodeIndex = "
                   "%d callsite->_byteCodeIndex = %d",
                   callsite, callNode->getByteCodeIndex(),
  ...
  if (!foundCall && !foundDeadCall) {
    if (tracer()->debugLevel())
      tracer()->dumpCallSite(
          ...
          "findAndUpdateCallsiteInGraaph: could not match call form IL to call "
          "in graph for the following callsite. callTarget %s, numCallees = %d "
          "numDeletedCallees = %d falureReason = %d",
          calltarget->_calleeSymbol->getResolvedMethod()->signature(
              comp()->trMemory()),
          calltarget->_numCallees, calltarget->_numDeletedCallees,
  ...
  // updating the fields on the callsite now that I have the info.  The rest
  // should be filled out properly in estimatecodesize.
  callsite->_callNode = callNode;
  callsite->_callNodeTreeTop = tt;
  callsite->_parent = parent;
  callsite->_initialCalleeSymbol = callNode->getSymbolReference()
                                       ->getSymbol()
                                       ->castToMethodSymbol()
  ...
  if (foundDeadCall) {
    ...
    for (k = 0; k < callsite->numRemovedTargets(); k++) {
      tracer()->insertCounter(
          callsite->getRemovedTarget(k)->getCallTargetFailureReason(),
          callsite->_callNodeTreeTop);
    }
    if (k == 0)  // if you never found a target in the first place.
    {
      tracer()->insertCounter(callsite->getCallSiteFailureReason(),
                              callsite->_callNodeTreeTop);
    }
  }

  if (!foundCall) return 0;
  ...
  if (callNode->getSymbolReference()
    ...
    debugTrace(tracer(),
               "findAndUpdateCallSiteInGraph: BAD: Interface call has an "
               "initialCalleeSYmbol %p for calNode %p",
               callsite->_initialCalleeSymbol, callNode);

  if (tracer()->heuristicLevel()) {
    tracer()->dumpCallSite(
        callsite, "before findAndUpdateCallSiteInGraph for %p", callsite);
  }
  ...
  if (!callNode->getOpCode().isCallIndirect()) {
    ...
    // if we are lucky it is the one whose calleeMethod matches the one of
    // callsite's calleeSymbol also it seemed less invasive to assign i to 0
    // rather than change every single occurence of (i)

    // Code to help Inliner cope with the fact that IAP might have refined
    // targets
    if (callsite->numTargets() > 0) {
      if (callsite->isIndirectCall())  // local opts pre-existence has been
                                       // performed
      {
        if (comp()->compileRelocatableCode())
          callsite->getTarget(i)->_guard->_kind = TR_DirectMethodGuard;
        else
          callsite->getTarget(i)->_guard->_kind =
              TR_NoGuard;  // do not need a guard anymore
        ...
        if (!callsite->getTarget(i)->_calleeMethod->isSameMethod(
          ...
          callsite->getTarget(i)->_myCallees.setFirst(
              0);  // preventing furhter inlining as we now don't know what's
                   // beyond the new target
          callsite->getTarget(i)->_numCallees =
              0;  // may want to improve this eventually
        ...
        callsite->getTarget(i)->_calleeMethod =
            callsite->_initialCalleeSymbol->getResolvedMethod();
        callsite->_isIndirectCall = false;
      } else  // if(!callsite->isIndirectCall()) //local opts string peepholes
              // has been performed
      {
        if (!callsite->getTarget(i)->_calleeMethod->isSameMethod(
                callsite->_initialCalleeSymbol->getResolvedMethod())) {
          ...
          callsite->getTarget(i)->_partialInline =
              0;  // Can't do a partial inline anymore as the blocks are wrong.
          callsite->getTarget(i)->_isPartialInliningCandidate = false;
          callsite->getTarget(i)->_calleeSymbol =
              callsite->_initialCalleeSymbol;
          callsite->getTarget(i)->_calleeMethod =
              callsite->_initialCalleeSymbol->getResolvedMethod();
        }
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    if (callsite->getTarget(i)->_isPartialInliningCandidate &&
        callsite->getTarget(i)->_partialInline)
    ...
    if (!callsite->getTarget(i)->_calleeSymbol && !callsite->isInterface()) {
      ...
      // we will fill in calleeSymbol next time we call
      // getSymbolAndFindInlineTargets
      callsite->getTarget(i)->_calleeSymbol = callsite->_initialCalleeSymbol;
    }

    if (callsite->getTarget(i)->_guard->_kind == TR_InterfaceGuard &&
        callsite->getTarget(i)->_guard->_type == TR_MethodTest &&
        callsite->_initialCalleeSymbol) {
      if (tracer()->debugLevel())
        tracer()->dumpCallSite(
            callsite,
            ...
            callsite->_initialCalleeSymbol, callNode);
      TR_ASSERT(0,
                " target has an interface guard AND an initial callee symbol.. "
                "(bad!)\n");
    }
  }
  ...
  if (tracer()->heuristicLevel()) {
    tracer()->dumpCallSite(
        callsite, "after findAndUpdateCallSiteInGraph for %p", callsite);
  }
...
void TR_InlinerBase::inlineFromGraph(TR_CallStack *prevCallStack,
  ...
  TR::ResolvedMethodSymbol *callerSymbol =
      calltarget->_calleeSymbol;  // the current call target

  debugTrace(tracer(), "inlineFromGraph: calltarget %p, symbol %p", calltarget,
             callerSymbol);

  TR_CallStack callStack(comp(), callerSymbol,
                         callerSymbol->getResolvedMethod(), prevCallStack, 0,
                         true);

  if (innerPrexInfo) callStack._innerPrexInfo = innerPrexInfo;

  int32_t thisSiteIndex =
      callerSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex();
  bool isCold = false;
  ...
  for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt;
    ...
    // assume that all call nodes are the first child of a treetop
    bool isCall = tt->getNode()->getNumChildren() &&
                  tt->getNode()->getChild(0)->getOpCode().isCall();
    if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount &&
        ...
        // induceOSR has the same bcIndex and caller index of the call that
        // follows it the following conditions allows up to skip it
        !tt->getNode()
             ->getChild(0)
             ->getSymbolReference()
             ->isOSRInductionHelper()) {
      TR::Node *node = parent->getChild(0);
      TR::Symbol *sym = node->getSymbol();

      TR_CallStack::SetCurrentCallNode sccn(callStack, node);
      TR::SymbolReference *symRef = node->getSymbolReference();
      bool oldIsCold = isCold;  // added to account for the fact that isCold is
                                // changed by refineColdness call
      getUtil()->refineColdness(node, isCold);

      if (!isCold) {
        debugTrace(
            tracer(),
            "inlineFromGraph:: about to call findAndUpdateCallSiteInGraph on "
            "call target %p for call at node %p",
            calltarget, node);
        TR_CallSite *site = findAndUpdateCallSiteInGraph(
            &callStack, node->getByteCodeInfo(), tt, parent, node, calltarget);
        debugTrace(tracer(),
                   "inlineFromGraph: found a call at parent %p child %p, "
                   "findAndUpdateCallsite returned callsite %p",
        ...
        TR::MethodSymbol *calleeSymbol =
            symRef->getSymbol()->castToMethodSymbol();
        if (calleeSymbol->getMethod())
          heuristicTrace(
              tracer(),
              "Block containing call node %p (callee name: %s) but either "
              "caller block or function is cold. Skipping call.",
              node,
  ...
  debugTrace(tracer(),
             "Done First Tree Iteration for updating PrexInfo.  Now starting "
             "inline Loop for calltarget %p",
             calltarget);
  ...
  for (TR::TreeTop *tt = callerSymbol->getFirstTreeTop(); tt;
       tt = tt->getNextTreeTop()) {
    iter.reset();
    for (TR_CallTarget *target = iter.getFirst(); target;
         target = iter.getNext()) {
      if (target->_myCallSite->_callNodeTreeTop == tt &&
          !target->_alreadyInlined) {
        debugTrace(tracer(),
                   "inlineFromGraph: Calling inlinecallTarget on calltarget %p "
                   "in callsite %p",
...
static void updateCallersFlags(TR::ResolvedMethodSymbol *callerSymbol,
  ...
  if (calleeSymbol->hasNews()) callerSymbol->setHasNews(true);
  ...
  if (callerSymbol != optimizer->comp()->getJittedMethodSymbol() &&
      !disablePropagatingCalleeFlagsToTopLevelMethod)
    updateCallersFlags(optimizer->comp()->getJittedMethodSymbol(), calleeSymbol,
                       optimizer);
}

// decide whether to inline some small methods like unsafe and
// getClassAccessFlags
bool TR_InlinerBase::tryToInlineTrivialMethod(TR_CallStack *callStack,
                                              TR_CallTarget *calltarget) {
  return getPolicy()->tryToInlineTrivialMethod(callStack, calltarget);
}

bool OMR_InlinerPolicy::tryToInlineTrivialMethod(TR_CallStack *callStack,
                                                 TR_CallTarget *calltarget) {
  return false;
}

// returns false when inlining fails
// TODO: currently this method returns true in some cases when the inlining
// fails. This needs to be fixed
bool TR_InlinerBase::inlineCallTarget2(TR_CallStack *callStack,
                                       TR_CallTarget *calltarget,
                                       TR::TreeTop **cursorTreeTop,
  ...
  debugTrace(tracer(),
             "Starting Physical Inlining for call target %p at callsite %p "
             "with visit count %d and numtargets = %d \n Available temps: ",
             calltarget, calltarget->_myCallSite,
             calltarget->_myCallSite->_visitCount,
             calltarget->_myCallSite->numTargets());

  static char *teststring = feGetEnv("TR_WHOSINLININGME");
  if (teststring && strncmp(calleeSymbol->signature(trMemory()), teststring,
                            strlen(teststring)) == 0)
    printf(" Inlining %s INTO %s\n", calleeSymbol->signature(trMemory()),
           comp()->getMethodSymbol()->signature(trMemory()));

  if (tryToInlineTrivialMethod(callStack, calltarget)) return true;

  if (comp()->getOption(TR_InlineNativeOnly)) return false;
  ...
  int32_t numberOfNodesBefore =
      comp()->getNodeCount();  // want to know the number of nodes created
                               // before and after genIL
  ...
  TR::Block *blockContainingTheCall =
  ...
  if (calltarget->_myCallSite->numTargets() > 1) {
    if (calltarget->_myCallSite->_visitCount == 1) {
      ...
      // caching will allows us to skip over multiple merge blocks with single
      // gotos that subsequent inlines of the same call will create also, it is
      // better to cache the next treetop before things get too funky.
      calltarget->_myCallSite->_cursorTreeTop =
          callNodeTreeTop->getNextTreeTop();
    } else if (calltarget->_myCallSite->_visitCount ==
               calltarget->_myCallSite->numTargets())
    // /*!blockContainingTheCall->getExit()->getPrevTreeTop()->getNode()->getOpCode().isReturn()*/)
    {
      ...
      *cursorTreeTop = calltarget->_myCallSite->_cursorTreeTop;
      ...
      heuristicTrace(tracer(), "Setting cursorTreeTop %p node %p",
                     cursorTreeTop, (*cursorTreeTop)->getNode());
    }
  }
  ...
  genILSucceeded =
      tryToGenerateILForMethod(calleeSymbol, callerSymbol, calltarget);

  if (wasSynchronized) calleeSymbol->setSynchronised();
  ...
  if (!genILSucceeded) return false;
  ...
  for (TR::CFGNode *node = calleeSymbol->getFlowGraph()->getFirstNode(); node;
       node = node->getNext()) {
  ...
  if (calleeSymbol->getFirstTreeTop()) {
    ...
    while (curTree) {
      ...
      if (curNode->getOpCodeValue() == TR::asynccheck) _numAsyncChecks++;
  ...
  TR_InnerPreexistenceInfo *innerPrexInfo = getUtil()->createInnerPrexInfo(
      comp(), calleeSymbol, callStack, callNodeTreeTop, callNode, guard->_kind);
  if (calleeSymbol->mayHaveInlineableCall()) {
    debugTrace(
        tracer(),
        "** Target %p symbol %p may have inlineable calls signature %s\n",
  ...
  if (comp()->getOption(TR_FullSpeedDebug) &&
      !getPolicy()->mustBeInlinedEvenInDebug(calleeSymbol->getResolvedMethod(),
                                             callNodeTreeTop) &&
      (!comp()->getOption(TR_EnableOSR) ||
       comp()->getOption(TR_MimicInterpreterFrameShape))) {
    TR::Node *decompPoint =
        findPotentialDecompilationPoint(calleeSymbol, comp());
    if (decompPoint) {
      ...
      if (comp()->trace(OMR::inlining)) {
        ...
        traceMsg(comp(),
                 "FSD inlining: decompilation point %s (%s) prevented inlining "
                 "of %s\n",
  ...
  // If InnerPreexistence added inner assumptions on this method -- and this
  // method is being inlined as a non-virtual - add a dummy virtual guard around
  // this
  //
  if (innerPrexInfo->hasInnerAssumptions() && guard->_kind == TR_NoGuard) {
    if (comp()->trace(OMR::inlining)) {
      traceMsg(comp(),
               "%sIPREX virtualize the non-virtual call %s because of inner "
               "preexistence for inner calls\n",
  ...
  if (argInfo) {
    if (comp()->usesPreexistence())  // Mark the preexistent arguments (maybe
                                     // redundant if VP has already done that
                                     // and provided the info in argInfo)
    {
      int32_t firstArgIndex = callNode->getFirstArgumentIndex();
      for (int32_t c = callNode->getNumChildren() - 1; c >= firstArgIndex;
           c--) {
  ...
  if (!performTransformation(comp(),
                             "%sInlining qwerty %s into %s with a virtual "
                             "guard kind=%s type=%s partialInline=%d\n",
                             OPT_DETAILS, calleeSymbol->signature(trMemory()),
                             callerSymbol->signature(trMemory()),
                             tracer()->getGuardKindString(guard),
                             tracer()->getGuardTypeString(guard),
                             !comp()->getOption(TR_DisablePartialInlining) &&
  ...
  if (tracer()->debugLevel()) {
    debugTrace(tracer(),
               "For call target %p available temps before pam.initialize(): ",
               calltarget);
  ...
  TR_ParameterToArgumentMapper pam(
      callerSymbol, calleeSymbol, callNode, calltarget->_prexArgInfo, tempList,
      _availableTemps, _availableBasicBlockTemps, this);
  pam.initialize(callStack);

  pam.printMapping();

  if (calltarget->_myCallSite->_visitCount <
      calltarget->_myCallSite->numTargets()) {
    ListIterator<TR::SymbolReference> temps(&tempList);
    TR::SymbolReference *temp = 0;

    debugTrace(tracer(),
               "For call target %p. Adding the following to unavailable temps:",
               calltarget);
  ...
  TR_TransformInlinedFunction *tif = getUtil()->getTransformInlinedFunction(
      callerSymbol, calleeSymbol, blockContainingTheCall, callNodeTreeTop,
      callNode, pam, guard, tempList, _availableTemps,
      _availableBasicBlockTemps);

  tif->transform();

  /**
   At this point, the return node in the callee function has been transformed
   and replaced with the caller's usage of the call to the callee - in other
   words, a new node has been created in the calleee that wasn' there when we
   gen'd IL for it
  */
  ...
  TR::TreeTop *startOfInlinedCall =
      calleeSymbol->getFirstTreeTop()->getNextTreeTop();
  TR::Block *calleeFirstBlock =
      calleeSymbol->getFirstTreeTop()->getEnclosingBlock();
  TR::Block *calleeLastBlock =
      calleeSymbol->getLastTreeTop()->getEnclosingBlock();
  ...
  //
  TR::Block *firstCalleeBlock =
      calleeSymbol->getFirstTreeTop()->getNode()->getBlock();
  TR::Block *lastCalleeBlock = 0;
  bool treesAfterCallMergedIntoLastCalleeBlock = false;
  if (tif->firstBBEnd()) {
    // hook up the previous BBStart in the caller with the first BBEnd in the
    // callee
    //
    TR_ASSERT(firstCalleeBlock == tif->firstBBEnd()->getNode()->getBlock(),
              "Inliner, firstCalleeBlock ambiguity");
  ...
  // the goto we created for return to inlining might be in the blockcontaining
  // the call cannot remove the edge from the goto to its target
  TR::Block *RTgotoTarget = NULL;

  if (lastCalleeBlock) {
    // copy the successors from the call block as long as they are not the goto
    // target
    ...
    while ((n = calleeCFG->getNodes().pop())) {
      ...
      callerCFG->addNode(n);
      debugTrace(tracer(),
                 "\nAdding callee blocks into caller: callee block %p:%p:%d "
                 "--> %d (caller block) ",
                 n, n->asBlock(), calleeNodeNumber, n->getNumber());
      if (!n->asBlock()->isOSRCatchBlock() && !n->asBlock()->isOSRCodeBlock())
        callerCFG->copyExceptionSuccessors(blockContainingTheCall, n,
                                           succAndPredAreNotOSRBlocks);
      else
        debugTrace(tracer(),
                   "\ndon't add exception edges from callee OSR "
                   "block(block_%d) to caller catch blocks\n",
                   n->getNumber());
    }
  }

  blockContainingTheCall->inheritBlockInfo(firstCalleeBlock,
                                           firstCalleeBlock->isCold());
  ...
  if (guard->_kind != TR_NoGuard) {
    virtualGuard = addGuardForVirtual(
        callerSymbol, calleeSymbol, callNodeTreeTop, callNode,
        calltarget->_receiverClass, startOfInlinedCall, previousBBStartInCaller,
        nextBBEndInCaller, *tif, tempList, guard, &induceOSRCallTree,
        calltarget);

    TR_VirtualGuard *vGuard =
        comp()->findVirtualGuardInfo(virtualGuard->getNode());
    heuristicTrace(tracer(),
                   "calltarget %p vGuard %p calltarget->_receiverClass %p "
                   "callsite->_receiverClass %p\n",
                   calltarget, vGuard, calltarget->_receiverClass,
                   calltarget->_myCallSite->_receiverClass);

    // Now that we have created the virtual guard - see if we can remove it by
    // doing inner preexistence (yah, i know what you are thinking -- why create
    // it when i am going to remove it later? if we are going to remove it, we
    // still need it later so that we can add to the outer guard as an inner
    // assumption )
    //
    // FIXME: the following limitation can be removed if we fixup the trees
    // correctly in TR_InlinerBase::inlineCallTarget don't do inner pre on
    // HCRGuard
    if (!_disableInnerPrex && guard->_kind != TR_ProfiledGuard &&
        guard->_kind != TR_HCRGuard /* && guard->_kind != TR_NoGuard */ &&
        guard->_kind != TR_InnerGuard) {
      devirtualizedByInnerPreexistence = innerPrexInfo->perform(
          comp(), virtualGuard->getNode(), disableTailRecursion);

      if (disableTailRecursion) _disableTailRecursion = true;
    }
  } else if (treesAfterCallMergedIntoLastCalleeBlock &&
             comp()->getOption(TR_EnableOSR) &&
             !comp()->osrInfrastructureRemoved()) {
    // No guard is generated, caller's trees immediately after the call node is
    // merged into inlined callee's last block. The OSR exception edge from
    // block containing the call hasn't been copied onto the callee's last
    // block. Check if the block has to be splitted for distinct exception
    // edges.
    //
    TR::Block *mergedBlock = lastCalleeBlock;
    TR_OSRMethodData *osrMethodData =
        comp()->getOSRCompilationData()->findOSRMethodData(
            comp()->getCurrentInlinedSiteIndex(), calleeSymbol);

    if (osrMethodData && osrMethodData->getOSRCatchBlock()) {
      /**
       * In OSR, we need to split block even for cases without virtual guard.
       * This is because in OSR a block with OSR point must have an exception
       * edge to the osrCatchBlock of correct callerIndex. Split the block here
       * so that the OSR points from callee and from caller are separated.
       *
       * This will not uncommon the return value if the block is split, instead
       * it expects a temporary to have been generated so that nothing is
       * commoned. This is valid under the current inliner behaviour, as the
       * existance of a catch block will result in the required temporary being
       * created.
       */

      debugTrace(
          tracer(),
          "Splitting block_%d to have distint exception edges for callee %d",
          mergedBlock->getNumber(), comp()->getCurrentInlinedSiteIndex());
      // mergedBlock may have exception successors from the callee and non-OSR
      // exception successors from the caller, after split,
      // `blockOfTreesAfterTheCall` only contains trees from the caller, should
      // not have exception successors from the callee.
      TR::Block *blockOfTreesAfterTheCall = mergedBlock->split(
          callNodeTreeTop, callerCFG, false /*fixupCommoning*/,
          false /*copyExceptionSuccessors*/);
      callerCFG->copyExceptionSuccessors(blockContainingTheCall,
                                         blockOfTreesAfterTheCall);
      debugTrace(tracer(), "block_%d is generated as a result of split",
                 blockOfTreesAfterTheCall->getNumber());
    } else {
      for (auto e1 = mergedBlock->getExceptionSuccessors().begin();
           e1 != mergedBlock->getExceptionSuccessors().end(); ++e1) {
        TR_ASSERT_FATAL(!succIsOSRCatchBlock(*e1),
                        "Mismatch between OSR meta data and CFG\n");
      }

      debugTrace(tracer(),
                 "Copy OSR exception edge from caller block_%d to block_%d",
                 blockContainingTheCall->getNumber(), mergedBlock->getNumber());

      // The inlined callee has no OSR point, but the trees after the call from
      // caller may have OSR points. Have to copy the exception edge from caller
      // block containing the call to the merged block, otherwise OSR points
      // from the caller will become invalid.
      callerCFG->copyExceptionSuccessors(blockContainingTheCall, mergedBlock,
                                         succIsOSRCatchBlock);
    }
  }
  ...
  if (callNode->getReferenceCount() > 0)
    replaceCallNode(callerSymbol, tif->resultNode(),
                    originalCallNodeReferenceCount, callNodeTreeTop, parent,
                    callNode);
  ...
  if (tif->crossedBasicBlock()) {
    debugTrace(tracer(),
               "Inlining caused a basic block to be crossed.  Need to fix "
               "broken commoning");
    TR::Block *virtualCallSnippet = virtualGuard ? virtualGuard->getNode()
                                                       ->getBranchDestination()
                                                       ->getNode()
                                                       ->getBlock()
                                                 : 0;
    TR_HandleInjectedBasicBlock hibb(
        comp(), tracer(), callerSymbol, tempList, injectedBasicBlockTemps,
        _availableBasicBlockTemps, pam.getFirstMapping());
    hibb.findAndReplaceReferences(previousBBStartInCaller,
                                  nextBBEndInCaller->getNode()->getBlock(),
                                  virtualCallSnippet);
    if (guard->_kind != TR_NoGuard && nullCheckTreeTop && !nullCheckReference)
      nullCheckReference =
          hibb.findNullCheckReferenceSymbolReference(nullCheckTreeTop);
  }

  // privatized inliner args above a guard without modification limit code
  // motion and optimization so we now go and check the privatized arguments to
  // see if we can create a duplicate of the argument in the cold path - if so
  // we do to decouple the hot path from the cold path and give other opts more
  // room to work NOTE this used to be part of adding a virtual guard, but we
  // need hibb to store copies of surviving values first so we can exploit them
  // so we do it here
  /*if (virtualGuard)
     rematerializeCallArguments(tif, guard,
     previousBBStartInCaller->getNode()->getBlock(),
  ...
  if (induceOSRCallTree) {
    ...
    induceOSRCallNode->setNumChildren(
        numOfChildren -
        firstArgChild);  // tricky "swelling" of the node but we allocated it of
                         // the right size when we inserted the induce OSR call
                         // to begin with
    for (int32_t i = firstArgChild; i < numOfChildren; i++)
      induceOSRCallNode->setAndIncChild(i - firstArgChild,
                                        refCallNode->getChild(i));
  ...
  //   !virtualGuard->getNode()->inlineGuardIndirectsTheThisPointer())
  if (nullCheckReference && !devirtualizedByInnerPreexistence &&
      ((guard->_kind == TR_InnerGuard) ||
       !comp()
            ->findVirtualGuardInfo(virtualGuard->getNode())
            ->indirectsTheThisPointer())) {
    ...
    nullCheckTreeTop->getPrevTreeTop()->join(
        nullCheckTreeTop->getNextTreeTop());
    TR::Block *b =
        virtualGuard->getNextTreeTop()->getNextTreeTop()->getNode()->getBlock();
  ...
  if (calltarget->_myCallSite->numTargets() > 1 &&
    ...
    TR::Block *virtualCallSnippet =
        virtualGuard->getNode()->getBranchDestination()->getNode()->getBlock();
    TR::Node *lastNodeInCallBlock =
        virtualCallSnippet->getExit()->getPrevTreeTop()->getNode();
    if (lastNodeInCallBlock->getOpCode().isReturn() ||
        lastNodeInCallBlock->getOpCode().isGoto()) {
      TR_ASSERT(!virtualCallSnippet->getExit()->getNextTreeTop(),
                "this is usually the last thing to be appended to the end of "
                "the trees");
      virtualCallSnippet->getPrevBlock()->getExit()->join(NULL);
      nextBBEndInCaller->getNode()
          ->getBlock()
          ->getPrevBlock()
          ->getExit()
          ->insertBefore(TR::TreeTop::create(
              comp(),
              TR::Node::create(
                  virtualGuard->getNode(), TR::Goto, 0,
                  nextBBEndInCaller->getNode()
                      ->getBlock()
                      ->getEntry())));  // goto belongs to the callee (?)
      nextBBEndInCaller->getNode()->getBlock()->getPrevBlock()->getExit()->join(
          virtualCallSnippet->getEntry());
      virtualCallSnippet->getExit()->join(
          nextBBEndInCaller->getNode()->getBlock()->getEntry());
      // cursorTreeTop is used to communicate to the caller the treeTop from
      // which it should continue iteration
      if (cursorTreeTop) {
        *cursorTreeTop =
            calltarget->_myCallSite->_callNodeTreeTop->getPrevTreeTop();
        heuristicTrace(tracer(), "Setting cursorTreeTop %p node %p",
                       cursorTreeTop, (*cursorTreeTop)->getNode());
      }
    } else {
      TR_ASSERT(
          calltarget->_myCallSite->_visitCount == 1,
          "We can only back off after inlining the first implementation\n");
      bool swapped = false;  // hopefully, compiler will get rid of this in prod
                             // build as there would be no use of this var except
                             // for the one in TR_ASSERT
      ...
      TR_ASSERT(swapped || calltarget->_myCallSite->getTarget(0) == calltarget,
                "Swapping didn't occur!");
      // remove the rest of targets
      calltarget->_myCallSite->removeTargets(tracer(), 1, Not_Sane);
    }
  }

  // move all the automatics from the callee as well as any generated
  // temporaries the call stack.
  //
  TR::AutomaticSymbol *a;
  ListIterator<TR::AutomaticSymbol> autos(&calleeSymbol->getAutomaticList());
  for (a = autos.getFirst(); a; a = autos.getNext())
    if (!a->isParm()) callStack->addAutomatic(a);
  ...
  for (temp = temps.getFirst(); temp; temp = temps.getNext()) {
    ...
    if (calltarget->_myCallSite->_visitCount !=
      ...
      ListIterator<TR::SymbolReference> unavailabletemps(
          &calltarget->_myCallSite->_unavailableTemps);
      for (unavailabletemp = unavailabletemps.getFirst(); unavailabletemp;
           unavailabletemp = unavailabletemps.getNext())
        if (temp == unavailabletemp) donotadd = true;
    }
    if (!donotadd) {
      debugTrace(tracer(), "\t%d", temp->getReferenceNumber());
  ...
  if (calltarget->_myCallSite->_visitCount ==
      calltarget->_myCallSite->numTargets()) {
    ListIterator<TR::SymbolReference> unavailabletemps(
        &calltarget->_myCallSite->_unavailableBlockTemps);
    for (TR::SymbolReference *unavailabletemp = unavailabletemps.getFirst();
         unavailabletemp; unavailabletemp = unavailabletemps.getNext()) {
      injectedBasicBlockTemps.add(unavailabletemp);
    }
    calltarget->_myCallSite->_unavailableBlockTemps.setListHead(NULL);
  } else {
    // add newly injected basic block temps to the list of unavailableBlockTemps
    // till visit the last target
    ListIterator<TR::SymbolReference> unavailabletemps(
        &injectedBasicBlockTemps);
    for (TR::SymbolReference *unavailabletemp = unavailabletemps.getFirst();
         unavailabletemp; unavailabletemp = unavailabletemps.getNext()) {
  ...
  //   for (uint32_t i = calleeSymbol->getFirstJitTempIndex(); i < lastSlot;
  //   ++i)
  for (uint32_t i = calleeSymbol->getNumParameterSlots(); i < lastSlot; ++i) {
    ...
    if (i >= (uint32_t)calleeSymbol->getFirstJitTempIndex()) {
      ...
      for (TR::SymbolReference *sr = autos.getFirst(); sr;
           sr = autos.getNext()) {
  ...
  //
  if (tif->firstCatchBlock() &&
      callerCFG->getNodes().find(tif->firstCatchBlock()))
    callerSymbol->getLastTreeTop(nextBBEndInCaller->getNode()->getBlock())
        ->join(tif->firstCatchBlock()->getEntry());

  calleeSymbol->setFirstTreeTop(
      0);  // can't reuse the trees once they've been inlined.

  calltarget->_alreadyInlined = true;

  int32_t bytecodeSize = getPolicy()->getInitialBytecodeSize(
      calleeSymbol->getResolvedMethod(), calleeSymbol, comp());
  TR_ResolvedMethod *reportCalleeMethod = calleeSymbol->getResolvedMethod();
  int32_t numberOfLocalsInCallee =
      reportCalleeMethod->numberOfParameterSlots() +
      reportCalleeMethod->numberOfTemps();

  // TODO (Issue #389): Inserting any of these debug counters below causes
  // reference count exceptions during optimization phase
  //
  // TR::DebugCounter::prependDebugCounter(comp(),
  // "inliner.callSites/succeeded", callNodeTreeTop);
  // TR::DebugCounter::prependDebugCounter(comp(),
  // "inliner.callSites/succeeded:#bytecodeSize", callNodeTreeTop,
  // bytecodeSize); TR::DebugCounter::prependDebugCounter(comp(),
  // "inliner.callSites/succeeded:#localsInCallee", callNodeTreeTop,
  // numberOfLocalsInCallee);
...
bool TR_InlinerBase::forceVarInitInlining(TR_CallTarget *calltarget) {
  return false;
}

bool TR_InlinerBase::forceCalcInlining(TR_CallTarget *calltarget) {
  return false;
}

bool OMR_InlinerPolicy::callMustBeInlined(TR_CallTarget *calltarget) {
  return false;
}

bool TR_InlinerBase::forceInline(TR_CallTarget *calltarget) {
  if (getPolicy()->callMustBeInlined(calltarget) ||
      callMustBeInlinedRegardlessOfSize(calltarget->_myCallSite))
    return true;

  return false;
}

void TR_CallSite::tagcalltarget(int32_t index, TR_InlinerTracer *tracer,
                                TR_InlinerFailureReason reason) {
  heuristicTrace(tracer, "Tag Call Target %p from callsite %p for Reason: %s",
                 getTarget(index), this,
                 tracer->getFailureReasonString(reason));
  if (_comp->cg()->traceBCDCodeGen()) {
    char callerName[1024];
    traceMsg(_comp,
             "q^q : tag to inline %s into %s (callNode %p on line_no=%d)\n",
             signature(_comp->trMemory()),
             _comp->fe()->sampleSignature(
                 _callerResolvedMethod->getPersistentIdentifier(), callerName,
                 1024, _comp->trMemory()),
  ...
  TR_ASSERT(index < _mytargets.size(),
            "Index is greater or equal to the number of targets");
}

void TR_CallSite::tagcalltarget(TR_CallTarget *calltarget,
  ...
  TR_ASSERT(foundCallTarget,
            "Call Target not found in CallSite when trying to remove it.\n");
}

void TR_CallSite::removecalltarget(int32_t index, TR_InlinerTracer *tracer,
                                   TR_InlinerFailureReason reason) {
  heuristicTrace(
      tracer, "Removing Call Target %p from callsite %p for Reason: %s",
      getTarget(index), this, tracer->getFailureReasonString(reason));
  if (_comp->cg()->traceBCDCodeGen() && _callNode != NULL) {
    char callerName[1024];
    traceMsg(_comp,
             "q^q : failing to inline %s into %s (callNode %p on line_no=%d)\n",
             signature(_comp->trMemory()),
             _comp->fe()->sampleSignature(
                 _callerResolvedMethod->getPersistentIdentifier(), callerName,
                 1024, _comp->trMemory()),
...
void TR_CallSite::removecalltarget(TR_CallTarget *calltarget,
                                   TR_InlinerTracer *tracer,
                                   TR_InlinerFailureReason reason) {
  bool foundCallTarget = false;
  for (int32_t i = 0; i < _mytargets.size(); ++i) {
...
void TR_CallSite::removeTargets(TR_InlinerTracer *tracer, int index,
                                TR_InlinerFailureReason reason) {
...
void TR_CallSite::removeAllTargets(TR_InlinerTracer *tracer,
                                   TR_InlinerFailureReason reason) {
...
TR_CallSite::TR_CallSite(TR_ResolvedMethod *callerResolvedMethod,
                         TR::TreeTop *callNodeTreeTop, TR::Node *parent,
                         TR::Node *callNode, TR::Method *interfaceMethod,
                         TR_OpaqueClassBlock *receiverClass, int32_t vftSlot,
                         int32_t cpIndex,
                         TR_ResolvedMethod *initialCalleeMethod,
                         TR::ResolvedMethodSymbol *initialCalleeSymbol,
                         bool isIndirectCall, bool isInterface,
                         TR_ByteCodeInfo &bcInfo, TR::Compilation *comp,
                         int32_t depth, bool allConsts)
 ...
 * create and set the TR_PrexArgument of the calltarget's first argument, namely
 * the receiver, with information provided by the \p guard
 */
...
TR_CallTarget *TR_CallSite::addTarget(TR_Memory *mem, TR_InlinerBase *inliner,
                                      TR_VirtualGuardSelection *guard,
                                      TR_ResolvedMethod *implementer,
                                      TR_OpaqueClassBlock *receiverClass,
                                      TR_AllocationKind allocKind,
                                      float ratio) {
  TR_PrexArgInfo *myPrexArgInfo =
      inliner->getUtil()->createPrexArgInfoForCallTarget(guard, implementer);

  TR_CallTarget *result = new (mem, allocKind)
      TR_CallTarget(this, _initialCalleeSymbol, implementer, guard,
                    receiverClass, myPrexArgInfo, ratio);
  ...
  if (inliner->tracer()->heuristicLevel()) {
    ...
    heuristicTrace(
        inliner->tracer(),
        "Creating a call target %p for callsite %p using a %s and %s .  "
        "Signature %s",
        result, this, inliner->tracer()->getGuardKindString(guard),
        inliner->tracer()->getGuardTypeString(guard),
        _comp->fe()->sampleSignature(implementer->getPersistentIdentifier(),
                                     name, 1024, _comp->trMemory()));
  }
...
TR_OpaqueClassBlock *TR_CallSite::calleeClass() {
  if (isInterface()) {
    ...
    char *s = classNameToSignature(_interfaceMethod->classNameChars(), len,
                                   _comp, stackAlloc);
    TR_OpaqueClassBlock *result =
        _comp->fe()->getClassFromSignature(s, len, _callerResolvedMethod, true);
...
TR_InlineBlocks::TR_InlineBlocks(TR_FrontEnd *fe2, TR::Compilation *comp)
    : fe(fe2), _comp(comp) {
  _inlineBlocks =
      new (_comp->trHeapMemory()) List<TR_InlineBlock>(_comp->trMemory());
  _exceptionBlocks =
      new (_comp->trHeapMemory()) List<TR_InlineBlock>(_comp->trMemory());
...
void TR_InlineBlocks::addBlock(TR::Block *block) {
  ...
  _inlineBlocks->add(new (_comp->trHeapMemory()) TR_InlineBlock(
      block->getBlockBCIndex(), block->getNumber()));
}

void TR_InlineBlocks::addExceptionBlock(TR::Block *block) {
  _numExceptionBlocks++;

  _exceptionBlocks->add(new (_comp->trHeapMemory()) TR_InlineBlock(
      block->getBlockBCIndex(), block->getNumber()));
}
bool TR_InlineBlocks::isInList(int32_t index) {
  ...
  for (aBlock = blocksIt.getCurrent(); aBlock; aBlock = blocksIt.getNext()) {
    if (aBlock->_BCIndex == index) return true;
  }
  return false;
}
...
TR_InlinerTracer::TR_InlinerTracer(TR::Compilation *comp, TR_FrontEnd *fe,
                                   TR::Optimization *opt)
    : TR_LogTracer(comp, opt), _fe(fe), _trMemory(_comp->trMemory()) {
  _traceLevel = 0;
  if (opt)  // this means I want tracing on
  {
    if (comp->trace(OMR::inlining)) _traceLevel = trace_heuristic;

    if (comp->getOption(TR_DebugInliner)) _traceLevel = trace_debug;
  }
...
void TR_InlinerTracer::partialTraceM(const char *fmt, ...) {
  ...
  if (!partialLevel() || !comp()->getDebug()) return;
  va_list args;
  va_start(args, fmt);
...
void TR_InlinerTracer::insertCounter(TR_InlinerFailureReason reason,
                                     TR::TreeTop *tt) {
  const char *name = TR::DebugCounter::debugCounterName(
      comp(), "inliner.callSites/failed/%s", getFailureReasonString(reason));
...
void TR_InlinerTracer::dumpCallGraphs(TR_LinkHead<TR_CallTarget> *targets) {
  ...
  if (!targets->getFirst()) return;

  TR_CallTarget *calltarget;
  for (calltarget = targets->getFirst(); calltarget;
    ...
    traceMsg(comp(),
             "Call at node %p\n\tDepth\tP.I.\tcalltarget\tsize\tfailure "
             "reason\t\t\tbc index\t\tSignature\n",
             calltarget->_myCallSite->_callNode);

    do {
      currentTarget = targetsToBeEvaluated.pop();

      TR_ASSERT(currentTarget->_calleeMethod,
                "dumping Call Graph: a target has no resolved method!");
      ...
      traceMsg(comp(), "\t%d\t%d\t%p\t%d\t%s",
               currentTarget->_myCallSite->getDepth(),
               currentTarget->_isPartialInliningCandidate, currentTarget,
               currentTarget->_isPartialInliningCandidate
                   ? currentTarget->_partialSize
                   : currentTarget->_fullSize,
               getFailureReasonString(currentTarget->_failureReason));
      traceMsg(comp(), "\t\t%d\t\t%s\n",
               currentTarget->_myCallSite->_byteCodeIndex,
               comp()->fe()->sampleSignature(
                   currentTarget->_calleeMethod->getPersistentIdentifier(),
                   nameBuffer, SIGNATURE_SIZE, trMemory()));
      ...
      if (!currentTarget->_myCallees.isEmpty()) {
        TR_CallSite *callsite = 0;
        for (callsite = currentTarget->_myCallees.getFirst(); callsite;
             callsite = callsite->getNext()) {
          //               traceMsg(comp(),"\t\t\t\tnumtargets = %d
          //               numRemovedTargets = %d currtarget->numdeletedcallees
          //               = %d
      ...
      if (heuristicLevel() && !currentTarget->_deletedCallees.isEmpty()) {
        TR_CallSite *callsite = 0;
        for (callsite = currentTarget->_deletedCallees.getFirst(); callsite;
             callsite = callsite->getNext()) {
          if (callsite->numTargets()) {
            for (int32_t i = 0; i < callsite->numTargets(); i++)
              targetsToBeEvaluated.push(callsite->getTarget(i));
            ...
            traceMsg(comp(), "\t%d\t%d\t%p\t%d\t%s", callsite->getDepth(), 0, 0,
                     0, getFailureReasonString(callsite->_failureReason));
            traceMsg(comp(), "\t%d\t\t%s\n", callsite->_byteCodeIndex,
                     "No name  Consult bc index");
          }
        }
      }
    ...
    traceMsg(comp(),
             "Total Estimated Size = %d Total Size After Multipliers = %d "
             "Total Weight = %d\n\n",
...
void TR_InlinerTracer::dumpDeadCalls(TR_LinkHead<TR_CallSite> *sites) {
  ...
  for (callsite = sites->getFirst(); callsite; callsite = callsite->getNext()) {
    traceMsg(comp(),
             "^^^ Top Level Dead CallSite %p Node %p bcIndex %p Failure "
             "Reason: %s\n",
             callsite, callsite->_callNode, callsite->_byteCodeIndex,
    ...
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
      ...
      traceMsg(
          comp(), "\t%d\t%d\t%p\t%d\t%s", calltarget->_myCallSite->getDepth(),
          calltarget->_isPartialInliningCandidate, calltarget,
          calltarget->_isPartialInliningCandidate ? calltarget->_partialSize
                                                  : calltarget->_fullSize,
          getFailureReasonString(calltarget->_failureReason));
      traceMsg(comp(), "\t\t%d\t\t%s\n",
               calltarget->_myCallSite->_byteCodeIndex,
               comp()->fe()->sampleSignature(
                   calltarget->_calleeMethod->getPersistentIdentifier(), name,
...
void TR_InlinerTracer::dumpCallSite(TR_CallSite *callsite, const char *fmt,
...

  {
    ...
    traceMsg(comp(), "\t_CallerResolvedMethod = %p",
    ...
    traceMsg(comp(), "\n\t_initialCalleeMethod = %p",
             callsite->_initialCalleeMethod);
    traceMsg(comp(), "\t_initialCalleeSymbol = %p",
             callsite->_initialCalleeSymbol);
    ...
    if (callsite->_receiverClass) {
      char *sig = TR::Compiler->cls.classSignature(
          comp(), callsite->_receiverClass, trMemory());
    ...
    if (callsite->_callerResolvedMethod) {
      ...
      traceMsg(comp(), "\t CALLER signature from method = %s\n",
               fe()->sampleSignature(
                   callsite->_callerResolvedMethod->getPersistentIdentifier(),
                   callerName, 1024, trMemory()));
    }
...
void TR_InlinerTracer::dumpCallTarget(TR_CallTarget *calltarget,
                                      const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  ...
  traceMsg(
      comp(),
      "\tcalltarget= %p\n\t\tguard = %p guard->_kind = %s guard->_type = %s ",
      calltarget, calltarget->_guard, getGuardKindString(calltarget->_guard),
      getGuardTypeString(calltarget->_guard));
  traceMsg(comp(),
           "guard->_thisClass = %p _receiverclass = %p   (enum in "
           "compilation.hpp)\n",
  ...
  if (calltarget->_calleeMethod) {
    ...
    traceMsg(comp(), "\t\t signature from method = %s\n",
             fe()->sampleSignature(
                 calltarget->_calleeMethod->getPersistentIdentifier(),
                 calleeName, 1024, trMemory()));
  } else
    traceMsg(comp(), "\t\tNo callee Method yet.\n");

  if (calltarget->_receiverClass != NULL) {
    char *sig = TR::Compiler->cls.classSignature(
        comp(), calltarget->_receiverClass, trMemory());
    traceMsg(comp(), "\t Call TARGET Class Signature = %s\n", sig);
  }
  if (calltarget->_guard->_thisClass &&
      calltarget->_guard->_thisClass != calltarget->_receiverClass) {
    char *sig = TR::Compiler->cls.classSignature(
        comp(), calltarget->_guard->_thisClass, trMemory());
    traceMsg(comp(), "\t Call TARGET GUARD Class Signature = %s\n", sig);
  }

  traceMsg(comp(),
           "\t\t_size = %d _partialSize = %d _fullSize = %d _weight = %d ",
           calltarget->_size, calltarget->_partialSize, calltarget->_fullSize,
           calltarget->_weight);
  traceMsg(comp(),
           "_callGraphAdjustedWeight = %f \n\t\t_frequencyAdjustment = %f "
           "_isPartialInliningCandidate = %d _partialInline = %p\n",
           calltarget->_callGraphAdjustedWeight,
           calltarget->_frequencyAdjustment,
           calltarget->_isPartialInliningCandidate, calltarget->_partialInline);
  traceMsg(comp(), "\t\t_failureReason = %d (%s)  _alreadyInlined = %d\n",
           calltarget->_failureReason,
           getFailureReasonString(calltarget->_failureReason),
           calltarget->_alreadyInlined);
}
...
void TR_InlinerTracer::dumpInline(TR_LinkHead<TR_CallTarget> *targets,
                                  const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  ...
  for (TR_CallTarget *calltarget = targets->getFirst(); calltarget;
       calltarget = calltarget->getNext()) {
    targetsToBeEvaluated.push(calltarget);
      ...
      if (currentTarget && !currentTarget->_deletedCallees.isEmpty()) {
        ...
        for (callsite = currentTarget->_deletedCallees.getFirst(); callsite;
          ...
          if (callsite->numTargets()) {
            ...
            // traceMsg(comp(),"\t%d\t\t%s\n",callsite->_byteCodeIndex,"No name
            // Consult bc index");
          }
        }
      }
...
const char *TR_InlinerTracer::getGuardKindString(
    TR_VirtualGuardSelection *guard) {
  if (comp()->getDebug())
...
const char *TR_InlinerTracer::getGuardTypeString(
    TR_VirtualGuardSelection *guard) {
  if (comp()->getDebug())
...
void TR_InlinerTracer::dumpPrexArgInfo(TR_PrexArgInfo *argInfo) {
  if (!argInfo || !heuristicLevel()) return;
...

  {
    ...
    if (arg && arg->getClass()) {
      char *className =
          TR::Compiler->cls.classSignature(comp(), arg->getClass(), trMemory());
      traceMsg(comp(),
               "<Argument no=%d address=%p classIsFixed=%d "
               "classIsPreexistent=%d class=%p className= %s/>\n",
               i, arg, arg->classIsFixed(), arg->classIsPreexistent(),
               arg->getClass(), className);
    } else {
      traceMsg(comp(),
               "<Argument no=%d address=%p classIsFixed=%d "
               "classIsPreexistent=%d/>\n",
               i, arg, arg ? arg->classIsFixed() : 0,
               arg ? arg->classIsPreexistent() : 0);
    }
  }
  traceMsg(comp(), "</argInfo>\n");
}

bool OMR_InlinerPolicy::willBeInlinedInCodeGen(TR::RecognizedMethod method) {
  return false;
}
...
/**
 * replace the existing guard with a better one of more specific type if
 * possible
 */
void OMR_InlinerUtil::refineInlineGuard(
    TR::Node *callNode, TR::Block *&block1, TR::Block *&block2,
    bool &appendTestToBlock1, TR::ResolvedMethodSymbol *callerSymbol,
    TR::TreeTop *cursorTree, TR::TreeTop *&virtualGuard, TR::Block *block4) {}

/**
 * stop inlining for this callsite if TR_CallSite#_initialCalleeSymbol is
 * certain specific method
 */
bool OMR_InlinerPolicy::supressInliningRecognizedInitialCallee(
    TR_CallSite *callsite, TR::Compilation *comp) {
  return false;
}
...
/**
 * Do not perform privated inliner argument rematerialization on high
 * probability profiled guards when distrusted
 *
 * @return true if privatized inliner argumetn rematerialization should be
 * suppressed
 */
bool OMR_InlinerPolicy::suitableForRemat(TR::Compilation *comp, TR::Node *node,
                                         TR_VirtualGuardSelection *guard) {
  return true;
}

void OMR_InlinerUtil::estimateAndRefineBytecodeSize(TR_CallSite *callsite,
                                                    TR_CallTarget *target,
                                                    TR_CallStack *callStack,
                                                    int32_t &bytecodeSize) {}

/**
 * collect arguments information for the given target and store into
 * TR_CallTarget::_prexArgInfo the default implementation does nothing
 */
TR_PrexArgInfo *OMR_InlinerUtil::computePrexInfo(TR_CallTarget *target) {
  return NULL;
}

bool OMR_InlinerUtil::needTargetedInlining(TR::ResolvedMethodSymbol *callee) {
  return false;
}
...
void OMR_InlinerUtil::computeMethodBranchProfileInfo(
    TR::Block *cfgBlock, TR_CallTarget *calltarget,
    TR::ResolvedMethodSymbol *callerSymbol) {
  return;
}

TR_TransformInlinedFunction *OMR_InlinerUtil::getTransformInlinedFunction(
    TR::ResolvedMethodSymbol *callerSymbol,
    TR::ResolvedMethodSymbol *calleeSymbol, TR::Block *blockContainingTheCall,
    TR::TreeTop *callNodeTreeTop, TR::Node *callNode,
    TR_ParameterToArgumentMapper &pam, TR_VirtualGuardSelection *guard,
    List<TR::SymbolReference> &tempList,
    List<TR::SymbolReference> &availableTemps,
    List<TR::SymbolReference> &availableBasicBlockTemps) {
  return new (comp()->trStackMemory()) TR_TransformInlinedFunction(
      comp(), tracer(), callerSymbol, calleeSymbol, blockContainingTheCall,
      callNodeTreeTop, callNode, pam, guard, tempList, availableTemps,
      availableBasicBlockTemps);
}

/**
 * return how many times this call has been invoked. The data could come from
 * some project specific intrumentation, profiler etc
 */
int32_t OMR_InlinerUtil::getCallCount(TR::Node *callNode) { return 0; }

TR_InnerPreexistenceInfo *OMR_InlinerUtil::createInnerPrexInfo(
    TR::Compilation *c, TR::ResolvedMethodSymbol *methodSymbol,
    TR_CallStack *callStack, TR::TreeTop *callTree, TR::Node *callNode,
    TR_VirtualGuardKind guardKind) {
  return new (comp()->trStackMemory()) TR_InnerPreexistenceInfo(
      c, methodSymbol, callStack, callTree, callNode, guardKind);
}

bool TR_InnerPreexistenceInfo::perform(TR::Compilation *comp,
                                       TR::Node *guardNode,
                                       bool &disableTailRecursion) {
  return false;
}

TR_InnerPreexistenceInfo::TR_InnerPreexistenceInfo(
    TR::Compilation *c, TR::ResolvedMethodSymbol *methodSymbol,
    TR_CallStack *callStack, TR::TreeTop *treeTop, TR::Node *callNode,
    TR_VirtualGuardKind guardKind)
    : _comp(c),
      _trMemory(c->trMemory()),
      _methodSymbol(methodSymbol),
...
/**
 * Perform any required tree modifications prior to merging with the callee
 * treetops.
 */
void OMR_InlinerUtil::calleeTreeTopPreMergeActions(
    TR::ResolvedMethodSymbol *calleeResolvedMethodSymbol,
    TR_CallTarget *calltarget) {
  /* By default, nothing is requred */
}

bool OMR_InlinerPolicy::dontPrivatizeArgumentsForRecognizedMethod(
    TR::RecognizedMethod recognizedMethod) {
  return false;
}

/**
 * return true if the \p calleeMethod is doing a software overflow check which
 * can be replaced by a hardware check
 */
bool OMR_InlinerPolicy::replaceSoftwareCheckWithHardwareCheck(
    TR_ResolvedMethod *calleeMethod) {
  return false;
}

TR_InlinerTracer *OMR_InlinerUtil::getInlinerTracer(
    TR::Optimization *optimization) {
  return new (comp()->trHeapMemory())
      TR_InlinerTracer(comp(), fe(), optimization);
}

void OMR_InlinerUtil::requestAdditionalOptimizations(
    TR_CallTarget *calltarget) {
  // This code will be removed once the dependent down stream project is merged
  // properly
  if (needTargetedInlining(calltarget->_calleeSymbol)) {
    inliner()->getOptimizer()->setRequestOptimization(
        OMR::methodHandleInvokeInliningGroup);
    if (comp()->trace(OMR::inlining))
      heuristicTrace(tracer(),
                     "Requesting another pass of targeted inlining due to %s\n",
                     tracer()->traceSignature(calltarget->_calleeSymbol));
  }
}
