...
bool
rematerializeConstant(TR::Node* node, TR::Compilation* comp);

static bool
isWarm(TR::Compilation* comp)
{
  return comp->getMethodHotness() >= warm;
}
static bool
isHot(TR::Compilation* comp)
{
  return comp->getMethodHotness() >= hot;
}
static bool
isScorching(TR::Compilation* comp)
{
  return (
...
static bool
succAndPredAreNotOSRBlocks(TR::CFGEdge* edge)
{
...
static bool
succIsOSRCatchBlock(TR::CFGEdge* edge)
{
  return toBlock(edge->getTo())->isOSRCatchBlock();
}

int32_t
OMR_InlinerPolicy::getInitialBytecodeSize(
  TR::ResolvedMethodSymbol* methodSymbol,
  TR::Compilation* comp)
{
  return getInitialBytecodeSize(
    methodSymbol->getResolvedMethod(), methodSymbol, comp);
}
...
TR::DataType
getStoreType(TR::Node* store, TR::Symbol* addrSymbol, TR::Compilation* comp)
{
  ...
  if (sym->mayHaveInlineableCall() && optimizer()->isEnabled(OMR::inlining)) {
    ...
    if (comp()->getOption(TR_Randomize) ||
      ...
      static uint32_t defaultRandomSize = comp()->convertNonDeterministicInput(
        TR::Options::getCmdLineOptions()->getTrivialInlinerMaxSize(),
        30,
        randomGenerator(),
        0);
...
void
OMR_InlinerPolicy::determineInliningHeuristic(
...
TR_InlinerBase::setInlineThresholds(TR::ResolvedMethodSymbol* callerSymbol)
{
  ...
  if (comp()->getNodeCount() > _nodeCountThreshold) {
    _nodeCountThreshold =
      comp()->getNodeCount() *
      (1.05F); // allow a little bit of inlining anyways to get smaller methods
  }
  ...
  getUtil()->refineInliningThresholds(comp(),
                                      _callerWeightLimit,
                                      _maxRecursiveCallByteCodeSizeEstimate,
                                      _methodByteCodeSizeThreshold,
                                      _methodInWarmBlockByteCodeSizeThreshold,
                                      _methodInColdBlockByteCodeSizeThreshold,
                                      _nodeCountThreshold,
                                      size);
...
void
TR_InlinerBase::cleanup(TR::ResolvedMethodSymbol* callerSymbol,
                        bool inlinedSite)
{
...
bool
OMR_InlinerPolicy::mustBeInlinedEvenInDebug(TR_ResolvedMethod* calleeMethod,
...
bool
TR_InlinerBase::alwaysWorthInlining(TR_ResolvedMethod* calleeMethod,
                                    TR::Node* callNode)
{
...
bool
OMR_InlinerPolicy::alwaysWorthInlining(TR_ResolvedMethod* calleeMethod,
                                       TR::Node* callNode)
{
  return false;
}
...
bool
TR_InlinerBase::exceedsSizeThreshold(TR_CallSite* callsite,
                                     int bytecodeSize,
                                     TR::Block* block,
                                     ...
                                     bool allConsts)
{
  ...
  int32_t coldBorderFrequency; // only used in
                               ...
                               // (to date)
  getBorderFrequencies(
    borderFrequency, coldBorderFrequency, calleeResolvedMethod, callNode);
  ...
  if (block) {
    ...
    if (allowBiggerMethods()) {
      ...
      if (blockFrequency > borderFrequency) {
        ...
        bytecodeSize = scaleSizeBasedOnBlockFrequency(bytecodeSize,
                                                      blockFrequency,
                                                      borderFrequency,
    ...
    getUtil()->adjustByteCodeSize(
      calleeResolvedMethod, _isInLoop, block, bytecodeSize);
  } else {
    if (comp()->trace(OMR::inlining))
  ...
  if (callNode) {
    heuristicTrace(tracer(),
                   "In ExceedsSizeThreshold.  Reducing size from %d",
                   bytecodeSize);
    ...
    if (!allconstsfromNode) {
      if (_isInLoop && originalbcSize > inlineThreshold) {
        if (numConstArgs > 0 && (numConstArgs + numNonConstArgs) > 0)
          bytecodeSize =
            (numConstArgs * originalbcSize) / (numConstArgs + numNonConstArgs);
      } else
  ...
  if (inlineThreshold && (uint32_t)bytecodeSize > inlineThreshold) {
    ...
    if (block) {
      TR::DebugCounter::prependDebugCounter(
        comp(),
        "inliner.callSites/failed/calleeHasTooManyBytecodes",
      ...
      TR::DebugCounter::prependDebugCounter(
        comp(),
        "inliner.callSites/failed/calleeHasTooManyBytecodes:#excess",
        block->getFirstRealTreeTop(),
  ...
  heuristicTrace(
    tracer(),
    "### Did not exceed size threshold, bytecodeSize %d <= inlineThreshold %d ",
    bytecodeSize,
    inlineThreshold);
...
void
TR_InlinerBase::createParmMap(TR::ResolvedMethodSymbol* calleeSymbol,
...
TR_DumbInliner::tryToInline(char* message, TR_CallTarget* calltarget)
{
  ...
  if (getPolicy()->tryToInline(calltarget, NULL, true)) {
    if (comp()->trace(OMR::inlining))
      traceMsg(comp(),
               "tryToInline pattern matched; %s for %s\n",
               message,
...
bool
OMR_InlinerPolicy::tryToInlineGeneral(TR_CallTarget* calltarget,
                                      ...
                                      bool toInline)
{
  ...
  if (regex && TR::SimpleRegex::match(regex, calltarget->_calleeMethod)) {
    if (comp()->trace(OMR::inlining)) {
      traceMsg(comp(),
               toInline ? "Inliner: tryToInline pattern matched, "
...
bool
OMR_InlinerPolicy::tryToInline(TR_CallTarget* calltarget,
                               TR_CallStack* callStack,
                               bool toInline)
{
...
void
TR_CallStack::makeBasicBlockTempsAvailable(
...
TR_CallStack*
TR_CallStack::isCurrentlyOnTheStack(TR_ResolvedMethod* method,
                                    int32_t occurrences)
{
  int32_t counter = 0;
...
bool
TR_CallStack::isAnywhereOnTheStack(TR_ResolvedMethod* method,
                                   int32_t occurrences)
{
  int32_t counter = 0;
  for (TR_CallStack* cs = this; cs; cs = cs->getNext()) {
...
bool
TR_InlineCall::inlineCall(TR::TreeTop* callNodeTreeTop,
                          ...
                          int32_t initialMaxSize)
{
  ...
  if (recursiveInlining && initialMaxSize == 0)
    initialMaxSize =
      isScorching(comp()) ? 140 : isHot(comp()) ? 90 : isWarm(comp()) ? 60 : 45;
  ...
  TR_CallStack callStack(
    comp(), callerSymbol, comp()->getCurrentMethod(), 0, initialMaxSize, true);
  TR_InnerPreexistenceInfo* innerPrexInfo =
    getUtil()->createInnerPrexInfo(comp(), callerSymbol, 0, 0, 0, TR_NoGuard);
  ...
  TR_CallSite* callsite = TR_CallSite::create(callNodeTreeTop,
                                              parent,
                                              callNode,
                                              thisClass,
                                              symRef,
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if (initialMaxSize > 0 &&
      ...
      heuristicTrace(
        tracer(),
        "Failed at Inlining. getMaxByteCodeIndex of %d exceeds initialMaxSize "
        "of %d",
        getPolicy()->getInitialBytecodeSize(calltarget->_calleeSymbol, comp()),
        initialMaxSize);
      return false; // nothing to callStack::commit yet
...

    {
      ...
      if (comp()->trace(OMR::inlining))
        traceMsg(comp(),
                 "inliner: Setting current inline depth=%d\n",
                 currentInlineDepth);
...
bool
TR_DumbInliner::inlineCallTargets(TR::ResolvedMethodSymbol* callerSymbol,
  ...
  if (!comp()->getOption(TR_DisableAdaptiveDumbInliner)) {
    int32_t callerSize =
      callerSymbol->getResolvedMethod()
        ->maxBytecodeIndex(); // getInitialBytecodeSize(callerSymbol, comp());
    maxCallSize =
      prevCallStack == 0 ?
                         // Initial budget should be inverse proportional to the
                         // size of the method
...
#endif
               ) && // getInlinerTemporarilyRestricted is used to avoid inlining
                    // during startup
...
bool
TR_DumbInliner::analyzeCallSite(TR_CallStack* callStack,
                                TR::TreeTop* callNodeTreeTop,
                                TR::Node* parent,
                                TR::Node* callNode)
{
      ...
      if (tryToInline("overriding getMaxBytecodeIndex check", calltarget)) {
        if (comp()->trace(OMR::inlining))
          traceMsg(comp(), "inliner: overriding getMaxBytecodeIndex check\n");
                   ...
                   calltarget->_calleeSymbol->getResolvedMethod(), callNode)) {
        if (comp()->trace(OMR::inlining))
          traceMsg(comp(),
...
static TR::TreeTop*
cloneAndReplaceCallNodeReference(TR::TreeTop*,
                                 TR::Node*,
                                 TR::Node*,
                                 TR::TreeTop*,
                                 TR::Compilation*);
...
void
TR_InlinerBase::cloneChildren(TR::Node* to,
                              TR::Node* from,
 ...
 */
void
TR_InlinerBase::replaceCallNode(TR::ResolvedMethodSymbol* callerSymbol,
                                TR::Node* resultNode,
                                ...
                                TR::Node* callNode)
{
  ...
  if (resultNode) {
    ...
    for (; tt && numberOfReferencesToFind; tt = tt->getNextTreeTop())
      replaceCallNodeReferences(tt->getNode(),
                                0,
                                0,
                                callNode,
                                resultNode,
 ...
 */
void
TR_InlinerBase::replaceCallNodeReferences(TR::Node* node,
                                          TR::Node* parent,
                                          uint32_t childIndex,
                                          TR::Node* callNode,
                                          ...
                                          TR::NodeChecklist& visitedNodes)
{
  ...
  if (!replacedNode) {
    for (int32_t i = 0; i < node->getNumChildren() && numberOfReferencesToFind;
         ++i)
      replaceCallNodeReferences(node->getChild(i),
                                node,
                                i,
                                callNode,
                                replacementNode,
...
TR::Node*
TR_InlinerBase::createVirtualGuard(TR::Node* callNode,
                                   ...
                                   TR_VirtualGuardSelection* guard)
{
  ...
  TR::DebugCounter::prependDebugCounter(
    ...
    TR::DebugCounter::debugCounterName(
      comp(),
      "virtualGuards.byJittedBody/%s/(%s)/(%s)/%s/bcinfo=%d.%d",
  ...
  if (guard->_kind == TR_DummyGuard) {
    return TR_VirtualGuard::createDummyGuard(
  ...
  if (guard->_type == TR_VftTest) {
    ...
    return TR_VirtualGuard::createVftGuard(
      guard->_kind, comp(), calleeIndex, callNode, destination, thisClass);
  } else if (guard->_type == TR_MethodTest)
    return TR_VirtualGuard::createMethodGuard(guard->_kind,
                                              comp(),
                                              calleeIndex,
                                              callNode,
                                              destination,
    ...
    return TR_VirtualGuard::createBreakpointGuard(
      comp(), calleeIndex, callNode, destination, calleeSymbol);
  } else {
...
static bool
blocksHaveSameStartingByteCodeInfo(TR::Block* aBlock, TR::Block* bBlock)
{
 ...
 */
void
TR_InlinerBase::rematerializeCallArguments(TR_TransformInlinedFunction& tif,
                                           TR_VirtualGuardSelection* guard,
                                           TR::Node* callNode,
                                           TR::Block* block1,
                                           TR::TreeTop* rematPoint)
{
  ...
  if (suitableForRemat) {
    ...
    for (prevTree = NULL,
      ...
      if (argStore->chkIsPrivatizedInlinerArg()) {
        ...
        TR_YesNoMaybe result =
          RematTools::gatherNodesToCheck(comp(),
                                         argStore,
                                         argStore->getFirstChild(),
                                         scanTargets,
                                         argSymRefsToCheck,
                                         tracer()->debugLevel());
        if (result == TR_yes) {
          ...
          debugTrace(tracer(),
                     "    priv arg remat unsafe for node [%p] - %d",
                     argStore,
                     argStore->getGlobalIndex());
    ...
    if (failedArgs.size() > 0) {
      ...
      for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
        ...
        TR::DebugCounter::incStaticDebugCounter(
          ...
          TR::DebugCounter::debugCounterName(
            comp(),
            "privatizedInlinerArgs.byMethod/Failed/(%s)/%s",
            comp()->signature(),
            argStore->getFirstChild()->getOpCode().getName()));
        TR::DebugCounter::incStaticDebugCounter(
          comp(),
          TR::DebugCounter::debugCounterName(
            comp(),
            "privatizedInlinerArgs.byReason/%s",
            argStore->getFirstChild()->getOpCode().getName()));
      }
    }

    debugTrace(tracer(),
               "  priv arg remat: symref checking walk required = %d",
               !scanTargets.IsZero());
    ...
    debugTrace(tracer(),
               ...
               rematPoint->getEnclosingBlock()->getNumber(),
               rematPoint);
    ...
    for (uint32_t i = 0; i < argSafetyInfo.size(); ++i) {
      ...
      if (!unsafeSymRefs.Intersects(argSafetyInfo.symRefDependencies(i))) {
        TR::DebugCounter::incStaticDebugCounter(
          ...
          TR::DebugCounter::debugCounterName(
            comp(),
            "privatizedInlinerArgs.byMethod/(%s)/Succeeded",
            comp()->signature()));
        TR::DebugCounter::incStaticDebugCounter(
          comp(),
          TR::DebugCounter::debugCounterName(
        ...
        // from another temp
          ...
          if (performTransformation(
                comp(),
                "O^O GUARDED CALL REMAT: Partial rematerialize of [%p] as [%p] "
                "- load of [%d]\n",
                argStore,
                duplicateStore,
                rematTree->getNode()
        ...
        TR::DebugCounter::incStaticDebugCounter(
          ...
          TR::DebugCounter::debugCounterName(
            comp(),
            "privatizedInlinerArgs.byReason/unsafeSymRef/%s",
            argStore->getFirstChild()->getOpCode().getName()));
        TR::DebugCounter::prependDebugCounter(
          comp(),
          TR::DebugCounter::debugCounterName(
...
void
TR_InlinerBase::addAdditionalGuard(TR::Node* callNode,
                                   TR::ResolvedMethodSymbol* calleeSymbol,
                                   TR_OpaqueClassBlock* thisClass,
                                   TR::Block* prevBlock,
...
TR::TreeTop*
TR_InlinerBase::addGuardForVirtual(TR::ResolvedMethodSymbol* callerSymbol,
                                   TR::ResolvedMethodSymbol* calleeSymbol,
                                   TR::TreeTop* callNodeTreeTop,
                                   TR::Node* callNode,
                                   TR_OpaqueClassBlock* thisClass,
                                   ...
                                   TR_CallTarget* calltarget)
{
  ...
  TR::Block* block3 =
    nextBBEndInCaller->getNode()->getBlock()->split(callNodeTreeTop, callerCFG);
    ...
    if (virtualGuard && comp()->cg()->supportsMergingGuards()) {
      ...
      if (guardNode) {
        ...
        if (virtualGuard && virtualGuard->getThisClass() &&
          ...
          if (comp()->trace(OMR::inlining))
            traceMsg(comp(),
                     "Merge HCR guard with virtual guard %p n%dn\n",
                     guardNode,
  ...
  getUtil()->refineInlineGuard(callNode,
                               block1,
                               block2,
                               appendTestToBlock1,
                               callerSymbol,
  ...
  if (tif.resultNode())
    OMR_InlinerUtil::storeValueInATemp(
      ...
      tif.resultNode(),
      resultTempSymRef,
      block3->getPrevBlock()->getLastRealTreeTop(),
      callerSymbol,
      tempList,
      _availableTemps,
  ...
  TR::Node* newTreeTopNode =
    (callNodeTreeTop->getNode()->getOpCode().isCheck())
      ? TR::Node::createWithSymRef(
          callNodeTreeTop->getNode()->getOpCodeValue(),
          1,
          callNode,
          0,
  ...
  TR::DebugCounter::prependDebugCounter(
    comp(),
    "inliner.callSites/succeeded:guardedCallee",
    ...
    while (cursor && comp()->getMethodSymbol()->isOSRRelatedNode(
      ...
      if (cursor->getNode()->getOpCode().isStoreDirect() &&
        ...
        block4->append(TR::TreeTop::create(
          comp(),
          TR::Node::createStore(cursor->getNode()->getSymbolReference(),
                                callNode)));
        break;
      }
      debugTrace(tracer(),
  ...
  if (tif.simpleCallReferenceTreeTop())
    cloneAndReplaceCallNodeReference(tif.simpleCallReferenceTreeTop(),
                                     0,
                                     0,
                                     block4->getLastRealTreeTop(),
                                     comp());
    ...
    OMR_InlinerUtil::storeValueInATemp(comp(),
                                       callNode,
                                       resultTempSymRef,
                                       block4->getLastRealTreeTop(),
                                       callerSymbol,
  ...
  rematerializeCallArguments(tif,
                             guard,
                             callNode,
                             block1,
  ...
  if ((comp()->getHCRMode() != TR::osr || guard->_kind != TR_HCRGuard) &&
      callingMethod->supportsInduceOSR(
        callNode->getByteCodeInfo(), block1, comp(), false)) {
...
bool
TR_InlinerBase::heuristicForUsingOSR(TR::Node* callNode,
                                     TR::ResolvedMethodSymbol* calleeSymbol,
                                     TR::ResolvedMethodSymbol* callerSymbol,
                                     bool isIndirectCall)
{
  ...
  while (osrMethodData) {
    ...
    if (comp()->getOption(TR_TraceOSR))
      traceMsg(
        ...
        "slots, total callers bytecodes %d total callers stack slots %d total "
        "callers live stack slots %d total pending push slots %d\n",
        osrMethodData->getInlinedSiteIndex(),
        osrCallerSize,
        osrCallerNumStackSlots,
        totalOSRCallersSize,
        totalOSRCallersStackSlots,
        totalOSRCallersLiveStackSlots,
        totalOSRLivePendingPushSlots);
    ...
    if (osrMethodData->getInlinedSiteIndex() > -1) {
      ...
      osrMethodData =
        comp()->getOSRCompilationData()->findCallerOSRMethodData(osrMethodData);
    ...
    if (parmMap->_parmSymbol->getParameterOffset() == argOffset) {
        ...
        if (parmMap->_addressTaken) {
          ...
          if (disableParmTempOpt) {
            ...
            if ((arg->getOpCode().isLoadVarDirect() &&
              ...
              debugTrace(tracer(),
                         "Setting parmMap %p -> _replacementSymref to %d",
                         parmMap,
      ...
      if (parmMap->_isConst) {
        ...
        if (parmMap->_replacementSymRef)
          tt =
            TR::TreeTop::create(comp(), TR::Node::create(TR::treetop, 1, arg));
        else {
          ...
          tt = OMR_InlinerUtil::storeValueInATemp(comp(),
                                                  arg,
                                                  symRef,
                                                  0,
                                                  _calleeSymbol,
  ...
  for (uint32_t i = 0; i < comp()->getOSRCallSiteRematSize(siteIndex); ++i) {
      ...
      if (symbol == parmMap->_parmSymbol) {
        if (parmMap->_isConst) {
          ...
          TR::DebugCounter::incStaticDebugCounter(
            ...
            TR::DebugCounter::debugCounterName(
              comp(),
              "osrCallSiteRemat/mapParm/const/(%s)",
              comp()->signature()));
          ...
          TR::DebugCounter::incStaticDebugCounter(
            ...
            TR::DebugCounter::debugCounterName(
              comp(),
              "osrCallSiteRemat/mapParm/addr/(%s)",
              comp()->signature()));
        } else {
          comp()->setOSRCallSiteRemat(
            siteIndex, ppSymRef, parmMap->_replacementSymRef);
          TR::DebugCounter::incStaticDebugCounter(
            comp(),
            TR::DebugCounter::debugCounterName(
              comp(),
              "osrCallSiteRemat/mapParm/success/(%s)",
              comp()->signature()));
        }
        break;
      }
    ...
    if (!parmMap)
      TR::DebugCounter::incStaticDebugCounter(
        ...
        TR::DebugCounter::debugCounterName(
          comp(),
          "osrCallSiteRemat/mapParm/missing/(%s)",
          comp()->signature()));
  }
...
TR::Node*
TR_ParameterToArgumentMapper::map(TR::Node* node,
...
TR_TransformInlinedFunction::transform()
{
  ...
  if (!_resultNode && returnType != TR::NoType &&
    ...
    _resultNode = TR::Node::create(
      penultimateNode, comp()->il.opCodeForConst(returnType), 0);
...
void
TR_TransformInlinedFunction::transformNode(TR::Node* node,
                                           TR::Node* parent,
                                           uint32_t childIndex,
                                           TR::NodeChecklist& visitedNodes)
{
  ...
  if (opcode.hasSymbolReference()) {
    ...
    if (symbol->isParm()) {
      TR::Node* newNode =
        _parameterMapper.map(node, symbol->getParmSymbol(), _crossedBasicBlock);
        ...
        if (newIsIntegral && oldIsIntegral &&
          ...
          newNode = TR::Node::create(
            TR::ILOpCode::getProperConversion(
              newNode->getDataType(), node->getDataType(), false),
            1,
            oldNewNode);
...
void
TR_TransformInlinedFunction::transformReturn(TR::Node* returnNode,
                                             TR::Node* parent)
{
  ...
  if (returnNode->getNumChildren() && _callNode->getReferenceCount() > 1) {
    ...
    if (_simpleCallReferenceTreeTop) {
      cloneAndReplaceCallNodeReference(_simpleCallReferenceTreeTop,
                                       _callNode,
                                       returnValue,
  ...
  if (!isAtEOF && !wasReturnTo) {
    ...
    TR::Node* gotoNode = TR::Node::create(
      returnNode, TR::Goto, 0, _generatedLastBlock->getEntry());
  ...
  for (auto e = currentBlock->getSuccessors().begin();
       e != currentBlock->getSuccessors().end();
       ++e)
...
TR_HandleInjectedBasicBlock::printNodesWithMultipleReferences()
{
  ...
  for (MultiplyReferencedNode* mn = _multiplyReferencedNodes.getFirst(); mn;
    ...
    debugTrace(tracer(),
               ...
               mn->_node,
               mn->_treeTop,
               mn->_replacementSymRef
                 ? mn->_replacementSymRef->getReferenceNumber()
                 : -1,
               mn->_referencesToBeFound,
               mn->_isConst,
               mn->_symbolCanBeReloaded);
  }
}

void
TR_HandleInjectedBasicBlock::findAndReplaceReferences(TR::TreeTop* callBBStart,
...
void
TR_HandleInjectedBasicBlock::collectNodesWithMultipleReferences(
  TR::TreeTop* tt,
  TR::Node* parent,
  TR::Node* node)
{
  ...
  TR::NodeChecklist& visitedNodes)
{
  ...
  if (node->getReferenceCount() > 1 && (found = find(node))) {
    ...
    for (int32_t i = 0; i < node->getNumChildren(); ++i)
      replaceNodesReferencedFromAbove(
        tt, node->getChild(i), node, i, visitedNodes);
  }
}
      ...
      if (!symRef) {
        ...
        if (comp()->fe()->dataTypeForLoadOrStore(nodeDataType) !=
          ...
          TR::ILOpCodes convOpCode = TR::ILOpCode::getProperConversion(
            nodeDataType,
            comp()->fe()->dataTypeForLoadOrStore(nodeDataType),
            false);
...
TR::Node*
TR_HandleInjectedBasicBlock::findNullCheckReferenceSymbolReference(
...
void
TR_HandleInjectedBasicBlock::replace(MultiplyReferencedNode* ref,
                                     TR::TreeTop* tt,
                                     TR::Node* parent,
                                     uint32_t childIndex)
{
...
static TR::TreeTop*
findPinningArrayStore(TR::Compilation* comp,
                      TR::TreeTop* prevTT,
                      TR::Node* value)
{
  TR::TreeTop* tt = NULL;
 ...
 */
TR::TreeTop*
OMR_InlinerUtil::storeValueInATemp(TR::Compilation* comp,
                                   TR::Node* value,
                                   ...
                                   int32_t offset)
{
  ...
  if (value->isInternalPointer() && value->getPinningArrayPointer()) {
    ...
    tempSymRef =
      comp->getSymRefTab()->createTemporary(methodSymbol, TR::Address, true, 0);
    TR::Symbol* symbol =
        ...
        if (valueChild->isInternalPointer() &&
          ...
          if (valueChild->getOpCode().isLoadVarDirect() &&
            ...
            TR::SymbolReference* newValueArrayRef =
              comp->getSymRefTab()->createTemporary(
                methodSymbol, TR::Address, false, 0);
  ...
  if (isIndirect) {
    ...
    storeNode =
      TR::Node::createWithSymRef(comp->il.opCodeForIndirectStore(dataType),
                                 2,
                                 2,
                                 storeAddress,
                                 value,
...
static TR::Node*
cloneAndReplaceCallNodeReference(TR::Node* originalNode,
                                 TR::Node* callNode,
                                 TR::Node* replacementNode,
                                 TR::Compilation* comp)
{
                                 ...
                                 TR::Compilation* comp)
{
  return TR::TreeTop::create(
    ...
    cloneAndReplaceCallNodeReference(
      callNodeReference->getNode(), callNode, replacementNode, comp));
}

static void
cloneAndReplaceReturnValueReference(TR::TreeTop* ifCmpTreeTop,
                                    ...
                                    TR::Compilation* comp)
{
  ...
  TR::TreeTop* newTreeTop = TR::TreeTop::create(comp,
                                                newNode,
                                                ifCmpTreeTop->getPrevTreeTop(),
...
bool
TR_TransformInlinedFunction::findCallNodeInTree(TR::Node* callNode,
                                                TR::Node* node)
{
  if (node == callNode)
    return true;
...
bool
TR_TransformInlinedFunction::findReturnValueInTree(TR::Node* rvStoreNode,
                                                   TR::Node* node,
                                                   TR::Compilation* comp)
{
...
bool
TR_TransformInlinedFunction::onlyMultiRefNodeIsCallNode(TR::Node* callNode,
                                                        TR::Node* node)
{
  if (node != callNode) {
    if (node->getReferenceCount() > 1)
...
TR::TreeTop*
TR_TransformInlinedFunction::findSimpleCallReference(
  TR::TreeTop* callNodeTreeTop,
  TR::Node* callNode)
{
  // If the call node's only other reference is in the next tree top
...
bool
TR_CallSite::findCallSiteTarget(TR_CallStack* callStack,
                                TR_InlinerBase* inliner)
{
  TR_ASSERT(
    0, "TR_CallSite::findCallSiteTarget is not implemented in %s", name());
  return false;
}
...
{
  return _callerResolvedMethod->getResolvedVirtualMethod(
    comp(), klass, _vftSlot);
}
...
bool
TR_IndirectCallSite::hasFixedTypeArgInfo()
{
...
bool
TR_IndirectCallSite::hasResolvedTypeArgInfo()
{
  return _ecsPrexArgInfo && _ecsPrexArgInfo->get(0) &&
...
TR_OpaqueClassBlock*
TR_IndirectCallSite::getClassFromArgInfo()
{
...
bool
TR_IndirectCallSite::tryToRefineReceiverClassBasedOnResolvedTypeArgInfo(
  TR_InlinerBase* inliner)
{
  if (hasResolvedTypeArgInfo()) {
    ...
    if (_receiverClass &&
        comp()->fe()->isInstanceOf(
          classFromArgInfo, _receiverClass, true, true) == TR_yes) {
      heuristicTrace(inliner->tracer(),
                     "refining _receiverClass %p to %p",
                     _receiverClass,
                     classFromArgInfo);
...
bool
TR_IndirectCallSite::findCallTargetUsingArgumentPreexistence(
  TR_InlinerBase* inliner)
{
...
bool
TR_IndirectCallSite::addTargetIfMethodIsNotOverriden(TR_InlinerBase* inliner)
{
...
bool
TR_IndirectCallSite::addTargetIfMethodIsNotOverridenInReceiversHierarchy(
  TR_InlinerBase* inliner)
{
 ...
 */
bool
OMR_InlinerUtil::addTargetIfMethodIsNotOverridenInReceiversHierarchy(
 ...
 */
bool
OMR_InlinerUtil::addTargetIfThereIsSingleImplementer(
  TR_IndirectCallSite* callsite)
{
  return false;
}
...
bool
TR_IndirectCallSite::addTargetIfThereIsSingleImplementer(
  TR_InlinerBase* inliner)
{
...
bool
TR_IndirectCallSite::findCallSiteTarget(TR_CallStack* callStack,
                                        TR_InlinerBase* inliner)
{
...
bool
TR_DirectCallSite::findCallSiteTarget(TR_CallStack* callStack,
                                      TR_InlinerBase* inliner)
{
...
void
OMR_InlinerUtil::collectCalleeMethodClassInfo(TR_ResolvedMethod* calleeMethod)
{
  return;
}

void
TR_InlinerBase::getSymbolAndFindInlineTargets(TR_CallStack* callStack,
                                              ...
                                              bool findNewTargets)
{
  ...
  if (callsite->_initialCalleeSymbol) {
    ...
    if (comp()->fe()->isInlineableNativeMethod(
      ...
      callsite->addTarget(trMemory(),
                ...
                checkInlineableWithoutInitialCalleeSymbol(callsite, comp()))) !=
             InlineableTarget) {
    tracer()->insertCounter(isInlineable, callNodeTreeTop);
    callsite->_failureReason = isInlineable;
    callsite->removeAllTargets(tracer(), isInlineable);
  ...
  if (callsite->_initialCalleeSymbol &&
    ...
    if (callsite->_isIndirectCall && !callsite->_receiverClass) {
      ...
      TR_OpaqueClassBlock* type =
        s ? fe()->getClassFromSignature(
              s, len, callsite->_callerResolvedMethod, true)
          : 0;
      if (type && (!callsite->_receiverClass ||
                   (type != callsite->_receiverClass &&
                    fe()->isInstanceOf(
                      type, callsite->_receiverClass, true, true) == TR_yes)))
  ...
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    ...
    if (!target->_calleeSymbol ||
      ...
      target->_calleeMethod->setOwningMethod(
        callsite->_callNode->getSymbolReference()
          ...
          ->getResolvedMethod()); // need to point to the owning method to the
                                  // same one that got ilgen'd d157939
    ...
    TR::RecognizedMethod rm = target->_calleeSymbol
                                ? target->_calleeSymbol->getRecognizedMethod()
  ...
  if (callsite->numTargets() > 0 && callsite->getTarget(0) &&
    ...
    traceMsg(comp(),
             "inliner: method is unresolved: %s into %s\n",
...
bool
OMR_InlinerPolicy::canInlineMethodWhileInstrumenting(TR_ResolvedMethod* method)
{
  return true;
}

void
TR_InlinerBase::applyPolicyToTargets(TR_CallStack* callStack,
                                     TR_CallSite* callsite)
{
  for (int32_t i = 0; i < callsite->numTargets(); i++) {
    TR_CallTarget* calltarget = callsite->getTarget(i);
    ...
    if (callStack && callStack->isAnywhereOnTheStack(calltarget->_calleeMethod,
                                                     selfInliningLimit)) {
      debugTrace(tracer(),
                 "Don't inline recursive call %p %s\n",
                 calltarget,
    ...
    if (!forceInline(calltarget))
      getUtil()->estimateAndRefineBytecodeSize(
        callsite, calltarget, callStack, bytecodeSize);
    ...
    int32_t numberOfLocalsInCallee =
      calltarget->_calleeMethod
        ->numberOfParameterSlots(); // + calleeResolvedMethod->numberOfTemps();
    if (!forceInline(calltarget) &&
...
bool
TR_InlinerBase::callMustBeInlinedRegardlessOfSize(TR_CallSite* callsite)
{
...
static bool
traceIfMatchesPattern(TR::Compilation* comp)
{
...
static void
traceRequestToCSI(TR::Compilation* comp, TR_CallSite* callsite)
{

  ::FILE* fout = stderr;

  fprintf(fout,
          "%p CALLEE %s indirect %d interface %d in %s\n",
          comp,
...
TR_CallSite*
TR_InlinerBase::findAndUpdateCallSiteInGraph(TR_CallStack* callStack,
                                             TR_ByteCodeInfo& bcInfo,
                                             TR::TreeTop* tt,
                                             TR::Node* parent,
                                             TR::Node* callNode,
                                             TR_CallTarget* calltarget)
{
  ...
  if (foundCall && callNode->getSymbolReference()->isUnresolved() &&
    ...
    debugTrace(
      tracer(),
      "findAndUpdateCallsiteInGraaph: call at node %p is unresolved. Bail out",
      callNode);
    return 0;
  }
  ...
  if (callNode->isTheVirtualCallNodeForAGuardedInlinedCall()) {
    heuristicTrace(
      tracer(),
      "findAndUpdateCallSiteInGraph: this call %p is on a cold side. Bail out",
      callNode);
    return 0;
  }
    ...
    if (tracer()->debugLevel())
      tracer()->dumpCallSite(
        ...
        calltarget->_calleeSymbol->getResolvedMethod()->signature(
          comp()->trMemory()),
        calltarget->_numCallees,
        calltarget->_numDeletedCallees,
...
void
TR_InlinerBase::inlineFromGraph(TR_CallStack* prevCallStack,
                                TR_CallTarget* calltarget,
                                TR_InnerPreexistenceInfo* innerPrexInfo)
{
  ...
  debugTrace(tracer(),
             "inlineFromGraph: calltarget %p, symbol %p",
             calltarget,
             callerSymbol);
    ...
    if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount &&
      ...
      if (!isCold) {
        ...
        if (calleeSymbol->getMethod())
          heuristicTrace(
            tracer(),
            "Block containing call node %p (callee name: %s) but either caller "
            "block or function is cold. Skipping call.",
            node,
...
static void
updateCallersFlags(TR::ResolvedMethodSymbol* callerSymbol,
                   TR::ResolvedMethodSymbol* calleeSymbol,
                   TR::Optimizer* optimizer)
{
  ...
  if (callerSymbol != optimizer->comp()->getJittedMethodSymbol() &&
    ...
    updateCallersFlags(
      optimizer->comp()->getJittedMethodSymbol(), calleeSymbol, optimizer);
}

// decide whether to inline some small methods like unsafe and
// getClassAccessFlags
bool
TR_InlinerBase::tryToInlineTrivialMethod(TR_CallStack* callStack,
                                         TR_CallTarget* calltarget)
{
...
bool
OMR_InlinerPolicy::tryToInlineTrivialMethod(TR_CallStack* callStack,
                                            TR_CallTarget* calltarget)
{
  return false;
}
...
bool
TR_InlinerBase::inlineCallTarget2(TR_CallStack* callStack,
                                  TR_CallTarget* calltarget,
                                  ...
                                  int32_t)
{
  ...
  if (teststring && strncmp(calleeSymbol->signature(trMemory()),
                            teststring,
                            strlen(teststring)) == 0)
  ...
  if (calltarget->_myCallSite->numTargets() > 1) {
    ...
    // /*!blockContainingTheCall->getExit()->getPrevTreeTop()->getNode()->getOpCode().isReturn()*/)
    {
      ...
      *cursorTreeTop = calltarget->_myCallSite->_cursorTreeTop;
      ...
      heuristicTrace(tracer(),
                     "Setting cursorTreeTop %p node %p",
                     cursorTreeTop,
  ...
  if (comp()->trace(OMR::inlining))
    traceMsg(comp(),
             "inlined %i calls so far,  maxInlinedCalls:%i\n",
  ...
  TR_ParameterToArgumentMapper pam(callerSymbol,
                                   calleeSymbol,
                                   callNode,
                                   calltarget->_prexArgInfo,
                                   tempList,
                                   _availableTemps,
                                   _availableBasicBlockTemps,
                                   this);
  ...
  TR_TransformInlinedFunction* tif =
    getUtil()->getTransformInlinedFunction(callerSymbol,
                                           calleeSymbol,
                                           blockContainingTheCall,
                                           callNodeTreeTop,
                                           callNode,
                                           pam,
                                           guard,
  ...
  */
  ...
  if (lastCalleeBlock) {
    ...
    while ((n = calleeCFG->getNodes().pop())) {
      ...
      if (!n->asBlock()->isOSRCatchBlock() && !n->asBlock()->isOSRCodeBlock())
        callerCFG->copyExceptionSuccessors(
          blockContainingTheCall, n, succAndPredAreNotOSRBlocks);
      else
        debugTrace(tracer(),
  ...
  if (guard->_kind != TR_NoGuard) {
    virtualGuard = addGuardForVirtual(callerSymbol,
                                      calleeSymbol,
                                      callNodeTreeTop,
                                      callNode,
                                      calltarget->_receiverClass,
                                      startOfInlinedCall,
                                      previousBBStartInCaller,
                                      nextBBEndInCaller,
                                      *tif,
                                      tempList,
                                      guard,
    ...
    if (osrMethodData && osrMethodData->getOSRCatchBlock()) {
      ...
      debugTrace(tracer(),
                 ...
                 blockContainingTheCall->getNumber(),
                 mergedBlock->getNumber());
      ...
      callerCFG->copyExceptionSuccessors(
        blockContainingTheCall, mergedBlock, succIsOSRCatchBlock);
    }
  }
  ...
  if (callNode->getReferenceCount() > 0)
    replaceCallNode(callerSymbol,
                    tif->resultNode(),
                    originalCallNodeReferenceCount,
                    callNodeTreeTop,
                    parent,
                    callNode);
  ...
  if (tif->crossedBasicBlock()) {
    ...
    TR::Block* virtualCallSnippet =
      virtualGuard
        ...
        : 0;
    TR_HandleInjectedBasicBlock hibb(comp(),
                                     tracer(),
                                     callerSymbol,
                                     tempList,
  ...
  if (induceOSRCallTree) {
    ...
    induceOSRCallNode->setNumChildren(
      ...
      firstArgChild); // tricky "swelling" of the node but we allocated it of
                      // the right size when we inserted the induce OSR call to
                      // begin with
    ...
    if (lastNodeInCallBlock->getOpCode().isReturn() ||
      ...
      nextBBEndInCaller->getNode()
        ...
        ->insertBefore(TR::TreeTop::create(
          comp(),
          TR::Node::create(virtualGuard->getNode(),
                           TR::Goto,
                           0,
                           nextBBEndInCaller->getNode()
                             ->getBlock()
...
void
TR_CallSite::tagcalltarget(int32_t index,
                           TR_InlinerTracer* tracer,
                           TR_InlinerFailureReason reason)
{
  heuristicTrace(tracer,
                 "Tag Call Target %p from callsite %p for Reason: %s",
                 getTarget(index),
                 this,
  ...
  if (_comp->cg()->traceBCDCodeGen()) {
    ...
    traceMsg(_comp,
             ...
             _comp->fe()->sampleSignature(
               _callerResolvedMethod->getPersistentIdentifier(),
               callerName,
               1024,
               _comp->trMemory()),
             _callNode,
             _comp->getLineNumber(_callNode));
  }
...
void
TR_CallSite::tagcalltarget(TR_CallTarget* calltarget,
                           TR_InlinerTracer* tracer,
                           TR_InlinerFailureReason reason)
{
...
void
TR_CallSite::removecalltarget(int32_t index,
                              TR_InlinerTracer* tracer,
                              TR_InlinerFailureReason reason)
{
  heuristicTrace(tracer,
                 "Removing Call Target %p from callsite %p for Reason: %s",
                 getTarget(index),
                 this,
                 tracer->getFailureReasonString(reason));
...
void
TR_CallSite::removecalltarget(TR_CallTarget* calltarget,
                              TR_InlinerTracer* tracer,
                              TR_InlinerFailureReason reason)
{
  bool foundCallTarget = false;
...
void
TR_CallSite::removeTargets(TR_InlinerTracer* tracer,
                           int index,
                           TR_InlinerFailureReason reason)
{
...
void
TR_CallSite::removeAllTargets(TR_InlinerTracer* tracer,
                              TR_InlinerFailureReason reason)
{
 ...
 */
TR_PrexArgInfo*
OMR_InlinerUtil::createPrexArgInfoForCallTarget(TR_VirtualGuardSelection* guard,
...
TR_CallTarget*
TR_CallSite::addTarget(TR_Memory* mem,
                       TR_InlinerBase* inliner,
                       TR_VirtualGuardSelection* guard,
                       TR_ResolvedMethod* implementer,
                       TR_OpaqueClassBlock* receiverClass,
                       ...
                       float ratio)
{
  ...
  TR_CallTarget* result =
    new (mem, allocKind) TR_CallTarget(this,
  ...
  if (inliner->tracer()->heuristicLevel()) {
    ...
    heuristicTrace(
      ...
      result,
      this,
      inliner->tracer()->getGuardKindString(guard),
      inliner->tracer()->getGuardTypeString(guard),
      _comp->fe()->sampleSignature(
        implementer->getPersistentIdentifier(), name, 1024, _comp->trMemory()));
  }
...
TR_CallSite::calleeClass()
{
  if (isInterface()) {
    ...
    char* s = classNameToSignature(
      _interfaceMethod->classNameChars(), len, _comp, stackAlloc);
...
void
TR_InlineBlocks::addBlock(TR::Block* block)
{
  _numBlocks++;
...
void
TR_InlineBlocks::addExceptionBlock(TR::Block* block)
{
  _numExceptionBlocks++;
...
bool
TR_InlineBlocks::isInList(int32_t index)
{
...
bool
TR_InlineBlocks::isInExceptionList(int32_t index)
{
...
void
TR_InlinerTracer::partialTraceM(const char* fmt, ...)
{
...
void
TR_InlinerTracer::insertCounter(TR_InlinerFailureReason reason, TR::TreeTop* tt)
{
...
TR_InlinerTracer::dumpDeadCalls(TR_LinkHead<TR_CallSite>* sites)
{
  ...
  for (callsite = sites->getFirst(); callsite; callsite = callsite->getNext()) {
    traceMsg(
      comp(),
      "^^^ Top Level Dead CallSite %p Node %p bcIndex %p Failure Reason: %s\n",
      callsite,
      callsite->_callNode,
      callsite->_byteCodeIndex,
      getFailureReasonString(callsite->_failureReason));
    ...
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
      ...
      traceMsg(comp(),
               "\t%d\t%d\t%p\t%d\t%s",
               calltarget->_myCallSite->getDepth(),
               calltarget->_isPartialInliningCandidate,
               calltarget,
               calltarget->_isPartialInliningCandidate
...
TR_InlinerTracer::dumpCallSite(TR_CallSite* callsite, const char* fmt, ...)
{
...

  {
    ...
    traceMsg(
      comp(), "\t_CallerResolvedMethod = %p", callsite->_callerResolvedMethod);
    ...
    traceMsg(
      comp(), "\t_initialCalleeSymbol = %p", callsite->_initialCalleeSymbol);
    ...
    if (callsite->_callerResolvedMethod) {
      char callerName[1024];
      traceMsg(comp(),
               "\t CALLER signature from method = %s\n",
               fe()->sampleSignature(
    ...
    if (callsite->_initialCalleeSymbol)
      traceMsg(comp(),
               "\t initial CALLEE signature from initial symbol = %s\n",
...
void
TR_InlinerTracer::dumpCallTarget(TR_CallTarget* calltarget,
                                 const char* fmt,
                                 ...)
{
  va_list args;
  ...
  traceMsg(
    ...
    calltarget,
    calltarget->_guard,
    getGuardKindString(calltarget->_guard),
    getGuardTypeString(calltarget->_guard));
  traceMsg(
    comp(),
    "guard->_thisClass = %p _receiverclass = %p   (enum in compilation.hpp)\n",
    calltarget->_guard->_thisClass,
    calltarget->_receiverClass);

  if (calltarget->_calleeSymbol)
    traceMsg(comp(),
             "\t\t signature from symbol = %s\n",
  ...
  traceMsg(comp(),
           ...
           calltarget->_partialInline);
  traceMsg(comp(),
           "\t\t_failureReason = %d (%s)  _alreadyInlined = %d\n",
           calltarget->_failureReason,
           getFailureReasonString(calltarget->_failureReason),
...
void
TR_InlinerTracer::dumpInline(TR_LinkHead<TR_CallTarget>* targets,
                             const char* fmt,
                             ...)
{
  va_list args;
...
void
TR_InlinerTracer::dumpPartialInline(TR_InlineBlocks* partialInline)
{
  TR_InlineBlock* aBlock = NULL;
...
void
TR_InlinerTracer::dumpPrexArgInfo(TR_PrexArgInfo* argInfo)
{
  if (!argInfo || !heuristicLevel())
    return;

  traceMsg(comp(),
           "<argInfo address = %p numArgs = %d>\n",
           argInfo,
           argInfo->getNumArgs());
...

  {
    ...
    if (arg && arg->getClass()) {
      ...
      traceMsg(comp(),
               ...
               arg,
               arg->classIsFixed(),
               arg->classIsPreexistent(),
               arg->getClass(),
               className);
    } else {
      traceMsg(
        comp(),
        "<Argument no=%d address=%p classIsFixed=%d classIsPreexistent=%d/>\n",
        i,
        arg,
        arg ? arg->classIsFixed() : 0,
 ...
 */
void
OMR_InlinerUtil::refineInlineGuard(TR::Node* callNode,
                                   TR::Block*& block1,
                                   TR::Block*& block2,
 ...
 */
bool
OMR_InlinerPolicy::supressInliningRecognizedInitialCallee(TR_CallSite* callsite,
                                                          TR::Compilation* comp)
{
  return false;
}
...
OMR_InlinerPolicy::checkIfTargetInlineable(TR_CallTarget* target,
                                           TR_CallSite* callsite,
                                           TR::Compilation* comp)
{
 ...
 */
bool
OMR_InlinerPolicy::suitableForRemat(TR::Compilation* comp,
                                    TR::Node* node,
                                    TR_VirtualGuardSelection* guard)
{
  return true;
...
void
OMR_InlinerUtil::estimateAndRefineBytecodeSize(TR_CallSite* callsite,
 ...
 */
void
OMR_InlinerUtil::computeMethodBranchProfileInfo(
  TR::Block* cfgBlock,
  TR_CallTarget* calltarget,
  TR::ResolvedMethodSymbol* callerSymbol)
{
...
TR_TransformInlinedFunction*
OMR_InlinerUtil::getTransformInlinedFunction(
  TR::ResolvedMethodSymbol* callerSymbol,
  TR::ResolvedMethodSymbol* calleeSymbol,
  TR::Block* blockContainingTheCall,
  TR::TreeTop* callNodeTreeTop,
  ...
  List<TR::SymbolReference>& availableBasicBlockTemps)
{
  return new (comp()->trStackMemory())
    TR_TransformInlinedFunction(comp(),
                                tracer(),
                                callerSymbol,
                                calleeSymbol,
                                blockContainingTheCall,
...
OMR_InlinerUtil::createInnerPrexInfo(TR::Compilation* c,
                                     TR::ResolvedMethodSymbol* methodSymbol,
                                     TR_CallStack* callStack,
                                     TR::TreeTop* callTree,
                                     TR::Node* callNode,
...
bool
TR_InnerPreexistenceInfo::perform(TR::Compilation* comp,
                                  TR::Node* guardNode,
 ...
 */
void
OMR_InlinerUtil::calleeTreeTopPreMergeActions(
...
bool
OMR_InlinerPolicy::dontPrivatizeArgumentsForRecognizedMethod(
 ...
 */
bool
OMR_InlinerPolicy::replaceSoftwareCheckWithHardwareCheck(
  TR_ResolvedMethod* calleeMethod)
{
  return false;
}
