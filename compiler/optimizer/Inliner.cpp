...
{
    return !toBlock(edge->getTo())->isOSRCodeBlock() && !toBlock(edge->getFrom())->isOSRCodeBlock() && !toBlock(edge->getTo())->isOSRCatchBlock() && !toBlock(edge->getFrom())->isOSRCatchBlock();
}
...
bool TR_InlinerBase::exceedsSizeThreshold(TR_CallSite* callsite, int bytecodeSize, TR::Block* block, TR_ByteCodeInfo& bcInfo,
    ...
    TR::Node* callNode, bool allConsts)
{
    ...
    if (callNode) {
        ...
        if (callNode->getOpCode().isCall() && !callNode->getSymbolReference()->isUnresolved() && callNode->getSymbolReference()->getSymbol()->getMethodSymbol() && !callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isHelper() && !callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isSystemLinkageDispatch() && !callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isStatic())
            ++i;
...
bool TR_InlineCall::inlineCall(TR::TreeTop* callNodeTreeTop, TR_OpaqueClassBlock* thisClass, bool recursiveInlining, TR_PrexArgInfo* argInfo, int32_t initialMaxSize)
{
...
bool TR_DumbInliner::inlineCallTargets(TR::ResolvedMethodSymbol* callerSymbol, TR_CallStack* prevCallStack, TR_InnerPreexistenceInfo* innerPrexInfo)
{
    int32_t maxCallSize;
    ...
    for (TR::TreeTop* tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (parent->getOpCodeValue() == TR::BBStart) {
            ...
            if (block->isCold() || (TR::isJ9() && !getPolicy()->inlineMethodEvenForColdBlocks(callerSymbol->getResolvedMethod()) && block->getFrequency() >= 0 && block->getFrequency() < veryColdBorderFrequency) || !block->getExceptionPredecessors().empty()) {
                isCold = true;
            }
        }
        ...
        if (parent->getNumChildren()) {
            ...
            if (node->getOpCode().isCall() && (!node->getOpCode().isJumpWithMultipleTargets()) && node->getVisitCount() != _visitCount && node->getInlinedSiteIndex() == thisCallSite) {
                if ((isCold
                    ...
                    !comp()->getMethodSymbol()->doJSR292PerfTweaks() && node->getSymbol() && node->getSymbol()->isResolvedMethod() && !alwaysWorthInlining(node->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod(), node))
...
*/
void TR_InlinerBase::replaceCallNode(
    ...
    TR::Node* callNode, TR::Node* replacementNode, rcount_t& numberOfReferencesToFind, TR::NodeChecklist& visitedNodes)
{
    ...
    //
    if ((_inliningAsWeWalk && node->getOpCode().isCall() && (node->getVisitCount() == _visitCount)) || visitedNodes.contains(node))
        return;
...
void TR_InlinerBase::addAdditionalGuard(TR::Node* callNode, TR::ResolvedMethodSymbol* calleeSymbol, TR_OpaqueClassBlock* thisClass, TR::Block* prevBlock, TR::Block* inlinedBody, TR::Block* slowPath, TR_VirtualGuardKind kind, TR_VirtualGuardTestType type, bool favourVFTCompare, TR::CFG* callerCFG)
{
    ...
    TR_CallTarget* calltarget)
{
    ...
    if (guard->_kind != TR_InnerGuard)
        virtualGuard = block1->append(TR::TreeTop::create(comp(), createVirtualGuard(callNode, calleeSymbol, block4->getEntry(), calleeSymbol->getFirstTreeTop()->getNode()->getInlinedSiteIndex(), thisClass, tif.favourVftCompare(), guard)));
    ...
    //  existing call
    //
    TR::Node* newTreeTopNode = (callNodeTreeTop->getNode()->getOpCode().isCheck()) ? TR::Node::createWithSymRef(callNodeTreeTop->getNode()->getOpCodeValue(), 1, callNode, 0, callNodeTreeTop->getNode()->getSymbolReference()) :
    ...
    TR::ResolvedMethodSymbol* callingMethod = callNode->getByteCodeInfo().getCallerIndex() == -1 ? comp()->getMethodSymbol() : comp()->getInlinedResolvedMethodSymbol(callNode->getByteCodeInfo().getCallerIndex());
    ...
    if ((comp()->getHCRMode() != TR::osr || guard->_kind != TR_HCRGuard)
        ...
        if (shouldUseOSR && (osrAll || comp()->getOption(TR_EnableOSROnGuardFailure) || (createdHCRAndVirtualGuard && shouldAttemptOSR && osrForHCRGuardsOfIndirect) || createdHCRGuard || (osrForNonHCRGuards && shouldAttemptOSR))) {
            ...
            if (!parmMap->_parmIsModified && (!arg->getOpCode().isFloatingPoint() || comp()->cg()->getSupportsJavaFloatSemantics())) {
                if (parmMap->_addressTaken) {
                    ...
                    if (disableParmTempOpt) {
                        ...
                        if ((arg->getOpCode().isLoadVarDirect() && arg->getSymbol()->isAutoOrParm()) && ((arg->getReferenceCount() == 1) || (_callNode->getOpCode().isCallIndirect() && (argIndex == _callNode->getFirstArgumentIndex()) && (_callNode->getFirstChild()->getNumChildren() > 0) && (arg == _callNode->getFirstChild()->getFirstChild()) && arg->getSymbol()->isAuto() && (arg->getReferenceCount() == 2)))) {
...
{
    if (!comp()->getOption(TR_EnableOSR) || comp()->getOSRMode() != TR::voluntaryOSR || comp()->osrInfrastructureRemoved() || comp()->getOption(TR_DisableOSRCallSiteRemat))
        return;
...
TR_ParameterToArgumentMapper::fixCallNodeArgs(bool createNullCheckReference)
{
    ...
    for (TR_ParameterMapping* parmMap = _mappings.getFirst(); parmMap; parmMap = parmMap->getNext()) {
        ...
        if ((i == _callNode->getFirstArgumentIndex()) && !_vftReplacementSymRef && parmMap->_replacementSymRef && _callNode->getOpCode().isCallIndirect()) {
            TR::Node* vftNode = _callNode->getChild(0);
            if (vftNode->getOpCode().hasSymbolReference() && (vftNode->getSymbolReference() == comp()->getSymRefTab()->findVftSymbolRef())) {
...
void TR_TransformInlinedFunction::transform()
{
    ...
    //
    if (comp()->getOption(TR_EnableJProfiling) || (firstBlock->getPredecessors().size() > 1) || firstBlock->hasExceptionSuccessors() || comp()->fe()->isMethodTracingEnabled(calleeResolvedMethod->getPersistentIdentifier()) || TR::Compiler->vm.canMethodEnterEventBeHooked(comp())) {
...
void TR_TransformInlinedFunction::transformNode(TR::Node* node, TR::Node* parent, uint32_t childIndex, TR::NodeChecklist& visitedNodes)
{
    ...
    } else if (opcode.isStore() && node->getFirstChild()->getOpCode().hasSymbolReference() && node->getFirstChild()->getSymbolReference()->isThisPointer()) {
        _favourVftCompare = true;
    }
...
void TR_HandleInjectedBasicBlock::replace(MultiplyReferencedNode* ref, TR::TreeTop* tt, TR::Node* parent, uint32_t childIndex)
{
...
static TR::TreeTop* findPinningArrayStore(TR::Compilation* comp, TR::TreeTop* prevTT, TR::Node* value)
{
    ...
    for (tt = prevTT; tt != lastTT; tt = tt->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStore() && node->getSymbolReference()->getSymbol()->isAuto() && node->getSymbolReference()->getSymbol()->castToAutoSymbol()->isPinningArrayPointer()) {
    ...
    if (!found) {
        ...
        if (tt->getNode()->getOpCode().isBranch() || tt->getNode()->getOpCode().isSwitch() || tt->getNode()->getOpCode().isReturn() || tt->getNode()->getOpCodeValue() == TR::athrow) {
            tt = tt->getPrevTreeTop();
        }
    }
    return tt;
    ...
    bool isIndirect, int32_t offset)
{
    ...
    if (value->isInternalPointer() && value->getPinningArrayPointer()) {
        ...
        TR::Symbol* symbol = tempSymRef->getSymbol()->castToInternalPointerAutoSymbol()->setPinningArrayPointer(value->getPinningArrayPointer());
        internalPtrHasPinningArrayPtr = true;
    } else {
        bool isInternalPointer = false;

        // disable internal pointer symbols for C/C++
        if ((value->hasPinningArrayPointer() && value->computeIsInternalPointer()) || (value->getOpCode().isLoadVarDirect() && value->getSymbolReference()->getSymbol()->isAuto() && value->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer()))
            ...
            if (value->getOpCode().isArrayRef()) {
                ...
                if (valueChild->isInternalPointer() && valueChild->getPinningArrayPointer()) {
                    ...
                    if (valueChild->getOpCode().isLoadVarDirect() && valueChild->getSymbolReference()->getSymbol()->isAuto()) {
                        ...
                        TR::SymbolReference* newValueArrayRef = comp->getSymRefTab()->createTemporary(methodSymbol, TR::Address, false, 0);
...
bool TR_TransformInlinedFunction::findReturnValueInTree(TR::Node* rvStoreNode, TR::Node* node, TR::Compilation* comp)
{
    ...
    // ibload
    //    loadaddr rvSymRef
    if (node->getOpCode().isLoadIndirect() && node->getFirstChild()->getOpCodeValue() == TR::loadaddr && node->getSize() == rvStoreNode->getSize() && node->getNumChildren() == 1) {
        TR::Node* child = node->getFirstChild();
        TR::SymbolReference* symRef = child->getSymbolReference();
        if (symRef == rvSymRef)
            return true;
    } else if (node->getOpCode().isLoadDirect() && node->getOpCode().hasSymbolReference() && node->getSize() == rvStoreNode->getSize() && node->getSymbolReference() == rvSymRef) {
        return true;
    }
...
TR::TreeTop* TR_TransformInlinedFunction::findSimpleCallReference(TR::TreeTop* callNodeTreeTop, TR::Node* callNode)
{
    ...
    if (callNode->getReferenceCount() == 2) {
        ...
        if ((opcode.isReturn() || opcode.isStore()) && findCallNodeInTree(callNode, nextTreeTopNode) && onlyMultiRefNodeIsCallNode(callNode, nextTreeTopNode))
            return nextTreeTop;
    }
    return 0;
}
...
{
    return _ecsPrexArgInfo && _ecsPrexArgInfo->get(0) && _ecsPrexArgInfo->get(0)->classIsPreexistent() && _ecsPrexArgInfo->get(0)->getClass();
}

TR_OpaqueClassBlock* TR_IndirectCallSite::getClassFromArgInfo()
{
    TR_ASSERT(_ecsPrexArgInfo && _ecsPrexArgInfo->get(0) && _ecsPrexArgInfo->get(0)->getClass(), "getClassFromArgInfo is NOT guarded by hasFixedTypeArgInfo,hasResolvedTypeArgInfo");
...
bool TR_IndirectCallSite::findCallSiteTarget(TR_CallStack* callStack, TR_InlinerBase* inliner)
{
    ...
    if (addTargetIfMethodIsNotOverriden(inliner) || addTargetIfMethodIsNotOverridenInReceiversHierarchy(inliner) || addTargetIfThereIsSingleImplementer(inliner) || findCallTargetUsingArgumentPreexistence(inliner)) {
        return true;
    }
    ...
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
        ...
        if (!target->_calleeSymbol || !target->_calleeMethod->isSameMethod(target->_calleeSymbol->getResolvedMethod())) {
            ...
            target->_calleeSymbol = comp()->getSymRefTab()->findOrCreateMethodSymbol(
                                                              symRef->getOwningMethodIndex(), -1, target->_calleeMethod, TR::MethodSymbol::Virtual /* will be target->_calleeMethodKind */)
                                        ->getSymbol()
...
void TR_InlinerBase::applyPolicyToTargets(TR_CallStack* callStack, TR_CallSite* callsite)
{
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
        ...
        if (!forceInline(calltarget) && exceedsSizeThreshold(callsite, bytecodeSize, (callsite->_callerBlock != NULL) ? callsite->_callerBlock : (callsite->_callNodeTreeTop ? callsite->_callNodeTreeTop->getEnclosingBlock() : 0), bcInfo, numberOfLocalsInCallee, callsite->_callerResolvedMethod, calltarget->_calleeMethod, callsite->_callNode, callsite->_allConsts)
...
static void traceRequestToCSI(TR::Compilation* comp, TR_CallSite* callsite)
{
    ...
    fprintf(fout, "%p CALLEE %s indirect %d interface %d in %s\n",
        comp,
        callsite->_isInterface ? callsite->_interfaceMethod->signature(comp->trMemory()) : callsite->_initialCalleeMethod->signature(comp->trMemory()),
    ...
    for (int32_t depth = stack.size() - 1; depth >= -1; depth--) {
        ...
        uint32_t offsetInInliner = (depth + 1 == stack.size()) ? callsite->_bcInfo.getByteCodeIndex() : comp->getInlinedCallSite(stack[depth + 1])._byteCodeInfo.getByteCodeIndex();
...
void TR_InlinerBase::inlineFromGraph(TR_CallStack* prevCallStack, TR_CallTarget* calltarget, TR_InnerPreexistenceInfo* innerPrexInfo)
{
    ...
    for (TR::TreeTop* tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount && tt->getNode()->getChild(0)->getInlinedSiteIndex() == thisSiteIndex && !tt->getNode()->getChild(0)->getSymbolReference()->getSymbol()->castToMethodSymbol()->isInlinedByCG() && !tt->getNode()->getChild(0)->isPotentialOSRPointHelperCall() && !tt->getNode()->getChild(0)->isOSRFearPointHelperCall() &&
    ...
    */
    ...
    if (induceOSRCallTree) {
        ...
        while (guardedCallTree) {
            ...
            if (((guardedCallTree->getNode()->getOpCodeValue() == TR::athrow) && guardedCallTree->getNode()->throwInsertedByOSR()) || (guardedCallTree->getNode()->getOpCodeValue() == TR::BBEnd))
                break;
        }
    }
    ...
    if (nullCheckReference && !devirtualizedByInnerPreexistence && ((guard->_kind == TR_InnerGuard) || !comp()->findVirtualGuardInfo(virtualGuard->getNode())->indirectsTheThisPointer())) {
    ...
    if (calltarget->_myCallSite->numTargets() > 1 && calltarget->_myCallSite->_visitCount != calltarget->_myCallSite->numTargets()) {
        ...
        if (lastNodeInCallBlock->getOpCode().isReturn() || lastNodeInCallBlock->getOpCode().isGoto()) {
            ...
            nextBBEndInCaller->getNode()->getBlock()->getPrevBlock()->getExit()->insertBefore(
                TR::TreeTop::create(comp(), TR::Node::create(virtualGuard->getNode(), TR::Goto, 0, nextBBEndInCaller->getNode()->getBlock()->getEntry()))); //goto belongs to the callee (?)
...
void TR_InlinerTracer::dumpInline(TR_LinkHead<TR_CallTarget>* targets, const char* fmt, ...)
{
    ...
    for (TR_CallTarget* calltarget = targets->getFirst(); calltarget; calltarget = calltarget->getNext()) {
        ...
        do {
            ...
            if (currentTarget->_failureReason == InlineableTarget || currentTarget->_failureReason == OverrideInlineTarget || currentTarget->_failureReason == TryToInlineTarget) {
                traceMsg(comp(), "\t%s", "+");
...
void OMR_InlinerUtil::estimateAndRefineBytecodeSize(TR_CallSite* callsite, TR_CallTarget* target, TR_CallStack* callStack, int32_t& bytecodeSize)
{
}
 ...
 */
void OMR_InlinerUtil::computeMethodBranchProfileInfo(TR::Block* cfgBlock, TR_CallTarget* calltarget, TR::ResolvedMethodSymbol* callerSymbol)
{
    return;
}
 ...
 */
void OMR_InlinerUtil::calleeTreeTopPreMergeActions(TR::ResolvedMethodSymbol* calleeResolvedMethodSymbol, TR_CallTarget* calltarget)
{
