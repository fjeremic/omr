    ...
    TR_PrexArgInfo* argInfo, int32_t initialMaxSize)
{
    ...
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
...

        {
            ...
            calltarget->_prexArgInfo = new (trHeapMemory())
                TR_PrexArgInfo(calltarget->_myCallSite->_callNode->getNumArguments(), trMemory());
    ...
    TR::TreeTop* tt, TR::Node* parent, TR::Node* callNode, TR_CallTarget* calltarget)
{
    ...
    for (callsite = calltarget->_myCallees.getFirst(); callsite; callsite = callsite->getNext()) {
        debugTrace(tracer(), "callNode->getByteCodeIndex = %d callsite->_byteCodeIndex = %d",
    ...
    if (foundDeadCall) {
        ...
        for (k = 0; k < callsite->numRemovedTargets(); k++) {
            tracer()->insertCounter(
                callsite->getRemovedTarget(k)->getCallTargetFailureReason(), callsite->_callNodeTreeTop);
        }
...
void TR_InlinerTracer::dumpPrexArgInfo(TR_PrexArgInfo* argInfo)
{
...

    {
        ...
        if (arg && arg->getClass()) {
            ...
            traceMsg(comp(),
                "<Argument no=%d address=%p classIsFixed=%d classIsPreexistent=%d class=%p className= %s/>\n", i, arg,
                arg->classIsFixed(), arg->classIsPreexistent(), arg->getClass(), className);
        } else {
