...
class TR_ValueNumberInfo {
 public:
  ...
  TR_ValueNumberInfo(TR::Compilation *, TR::Optimizer *,
                     bool requiresGlobals = false, bool prefersGlobals = true,
  ...
  int32_t getValueNumber(TR::Node *node) {
    ...
    if (index >= _numberOfNodes) setUniqueValueNumber(node);
    return _valueNumbers.ElementAt(index);
  }

  /** Get the node at the given index */
  TR::Node *getNode(int32_t index) {
    if (index >= _numberOfNodes) return NULL;
  ...
  TR::Node *getNext(TR::Node *node) {
    int32_t index = node->getGlobalIndex();
    if (index >= _numberOfNodes) return node;
  ...
  /** Change all nodes with the same value number as the given node to the new
   * value number */
 ...
 protected:
  ...
  virtual int32_t getVN(TR::Node *node) {
    return _valueNumbers.ElementAt(node->getGlobalIndex());
  }
...
class TR_HashValueNumberInfo : public TR_ValueNumberInfo {
 public:
  TR_HashValueNumberInfo(TR::Compilation *, TR::Optimizer *,
                         bool requiresGlobals = false,
                         bool prefersGlobals = true, bool noUseDefInfo = false);
  ...
  struct VNHashFunc {
    static CS2::HashValue Hash(
        const VNHashKey &v1,
        const CS2::HashValue hv = CS2::CS2_DEFAULT_INITIALHASHVALUE) {
      return v1._hashVal;
    }
 ...
 private:
  typedef CS2::HashTable<VNHashKey, int32_t, TR::Allocator, VNHashFunc>
      VNHashTable;
