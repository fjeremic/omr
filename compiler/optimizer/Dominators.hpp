...
//
// The algorithm uses an array of information data structures, one per basic
// block. The blocks are ordered in the array in depth-first order. The first
// entry in the array is a dummy node that is used as the root for the forest.
//
...
private:
  struct BBInfo {
    explicit BBInfo(TR::Region &region)
        : _bucket(region), _block(NULL), _parent(-1), _idom(-1), _ancestor(-1),
          _label(-1), _child(-1), _sdno(-1), _size(0) {}
  ...
  struct StackInfo {
    ...
    StackInfo(list_type &list, iterator_type position, int32_t parent)
        : list(list), listPosition(position), parent(parent) {}

    StackInfo(const StackInfo &other)
        : list(other.list), listPosition(other.listPosition),
          parent(other.parent) {}
  ...
  int32_t blockNumber(int32_t index) {
    return _info[index]._block->getNumber();
  }
...
public:
  ...
  TR_PostDominators(TR::Compilation *comp)
      : TR_Dominators(comp, true),
        _directControlDependents(NULL) // comp->getFlowGraph()->getNumberOfNodes()+1,
                                       // comp->allocator("PostDominators"))
  {}
