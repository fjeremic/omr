...
TR_LoopStrider::perform()
{
  ...
  detectCanonicalizedPredictableLoops(
    comp()->getFlowGraph()->getStructure(), NULL, -1);
...
bool
TR_LoopStrider::foundLoad(TR::TreeTop* storeTreeTop,
                          TR::Node* node,
                          int32_t symRefNum,
                          vcount_t visitCount)
{
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    if (foundLoad(
          storeTreeTop, node->getChild(childNum), symRefNum, visitCount))
      return true;
  }
  return false;
}
...
bool
TR_LoopStrider::foundLoad(TR::TreeTop* storeTree,
...
static int32_t
countInternalPointerOrPinningArrayStores(TR::Compilation* comp,
                                         TR::Block* block)
{
  int32_t count = 0;
                                                    ...
                                                    int32_t bitVectorSize)
{
  ...
  if (regionStructure) {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext())
      detectCanonicalizedPredictableLoops(
        node->getStructure(), optSetInfo, bitVectorSize);
  }
  ...
  if (loopInvariantBlock) {
    ...
    if (isPredictableLoop > 0) {
      ...
      _loadUsedInNewLoopIncrement = (TR::Node**)trMemory()->allocateStackMemory(
        _numberOfLinearExprs * sizeof(TR::Node*));
      memset(_loadUsedInNewLoopIncrement,
             0,
             _numberOfLinearExprs * sizeof(TR::Node*));
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          ...
          for (TR::Block* exitBlock = bi.getCurrent(); exitBlock != NULL;
            ...
            if (!exitValueNeeded && !seenStore) {
              ...
              for (auto edge = exitBlock->getSuccessors().begin();
                   edge != exitBlock->getSuccessors().end();
                   ++edge) {
        ...
        if (storeInRequiredForm &&
            ...
            performTransformation(
              comp(),
              "%s Replacing uses of induction var %d in loop %d\n",
              OPT_DETAILS,
              nextInductionVariableNumber,
              loopStructure->getNumber())) {
          ...
          if (replaceAllInductionVariableComputations(
            ...
            for (; k < _nextExpression; k++) {
              ...
              TR::Node* placeHolderNode =
                placeInitializationTreeInLoopInvariantBlock(loopInvariantBlock,
                                                            inductionVarSymRef,
                                                            newSymbolReference,
                                                            k,
                                                            symRefTab);
              ...
              //
              TR::Node* storeNode =
                placeNewInductionVariableIncrementTree(loopInvariantBlock,
                                                       inductionVarSymRef,
                                                       newSymbolReference,
                                                       k,
                                                       symRefTab,
              ...
              if (newSymbolReference->getSymbol()->getDataType() !=
                  TR::Address) {
                TR_InductionVariable* v =
                  loopStructure->asRegion()->findMatchingIV(inductionVarSymRef);
                if (v) {
                  TR::VPConstraint* entryVal = v->getEntry();
                  TR::VPConstraint* exitVal = v->getExit();
                  ...
                  if (newSymbolReference->getSymbol()->getType().isInt64()) {
                    newEntryVal = genVPLongRange(
                      entryVal, mulConst, getAdditiveTermConst(k));
                    newExitVal = genVPLongRange(
                      exitVal, mulConst, getAdditiveTermConst(k));
                    newIncr = genVPLongRange(incr, mulConst, 0);
                  } else {
                    newEntryVal = genVPIntRange(
                      entryVal, mulConst, getAdditiveTermConst(k));
                    newExitVal =
                      genVPIntRange(exitVal, mulConst, getAdditiveTermConst(k));
                    newIncr = genVPIntRange(incr, mulConst, 0);
                  }

                  TR_InductionVariable* iv =
                    new (trHeapMemory()) TR_InductionVariable(
                      newSymbolReference->getSymbol()
                        ->castToRegisterMappedSymbol(),
                      newEntryVal,
                      newExitVal,
                      newIncr,
                      _loopTestTree->getNode()->getOpCode().isUnsignedCompare()
                        ? TR_no
                        : TR_yes);
                  if (bestCandidate == k)
                    strideForPIV = iv;
              ...
              if (trace())
                traceMsg(comp(),
                         "Found an induction var chance in %s\n",
                         comp()->signature());
            ...
            if (isMulTermConst(bestCandidate) && isLoopTestInductionVariable &&
                ...
                performTransformation(comp(),
                                      ...
                                      OPT_DETAILS,
                                      comp()->signature())) {
              changeLoopCondition(loopInvariantBlock,
                                  usingAladd,
                                  bestCandidate,
    ...
    if (trace()) {
      traceMsg(
        comp(), "Found an induction var chance in %s\n", comp()->signature());
    }
    ...
    for (j = 0; j < symRefCount; j++) {
      ...
      if (reassociatedAuto) {
        ...
        if (trace())
          traceMsg(comp(),
                   "Found an reassociated induction var chance in %s\n",
                   comp()->signature());
        ...
        dumpOptDetails(
          ...
          reassociatedAuto->getReferenceNumber());
        ...
        if (performTransformation(
              ...
              OPT_DETAILS,
              newInductionCandidate->getSymbolReference()->getReferenceNumber(),
              loopStructure->getNumber()))
          newInductionCandidate->addAllBlocksInStructure(
            loopStructure, comp(), trace() ? "strider auto" : NULL);
      }
      ...
      if (symRefPair) {
        if (trace())
          traceMsg(comp(),
                   "Found an internal pointer hoisting chance in %s\n",
                   comp()->signature());
        ...
        while (symRefPair) {
            ...
            if (usingAladd)
              arrayRefNode = TR::Node::create(
                ...
                TR::Node::createWithSymRef(
                  byteCodeInfoNode, TR::aload, 0, origAuto),
                TR::Node::createWithSymRef(
                  byteCodeInfoNode, TR::lload, 0, indexSymRef));
            else
              arrayRefNode = TR::Node::create(
                TR::aiadd,
                2,
                TR::Node::createWithSymRef(
                  byteCodeInfoNode, TR::aload, 0, origAuto),
                TR::Node::createWithSymRef(
                  byteCodeInfoNode, TR::iload, 0, indexSymRef));
          }
          ...
          dumpOptDetails(
            comp(),
            "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis "
            "inserted invariant initialization tree : %p for new symRef #%d\n",
            newStoreTreeTop->getNode(),
...
bool
TR_LoopStrider::reassociateAndHoistNonPacked()
{
...
TR::VPLongRange*
TR_LoopStrider::genVPLongRange(TR::VPConstraint* cons,
                               int64_t coeff,
                               int64_t additive)
{
  if (cons) {
...
TR::VPIntRange*
TR_LoopStrider::genVPIntRange(TR::VPConstraint* cons,
                              int64_t coeff,
                              int64_t additive)
{
...
void
TR_LoopStrider::findOrCreateStoreInfo(TR::TreeTop* tree, int32_t i)
{
...
void
TR_LoopStrider::updateStoreInfo(int32_t i, TR::TreeTop* tree)
{
  _storeTrees[i] = tree;
  if (_storeTreesList)
...
TR::Node*
TR_LoopStrider::getNewLoopIncrement(TR::Node* oldLoad,
                                    int32_t k,
                                    int32_t symRefNum)
{
...
TR::Node*
TR_LoopStrider::getNewLoopIncrement(TR_StoreTreeInfo* info, int32_t k)
{
  if (!info)
...
void
TR_LoopStrider::changeBranchFromIntToLong(TR::Node* branch)
{
...
void
TR_LoopStrider::changeLoopCondition(TR_BlockStructure* loopInvariantBlock,
                                    bool usingAladd,
                                    ...
                                    TR::Node* storeOfDerivedInductionVariable)
{
  ...
  if (usingAladd || (secondChildOfLoopTestNode->getType().isInt64())) {
    TR::Node* constantNode =
      duplicateMulTermNode(bestCandidate, secondChildOfLoopTestNode, TR::Int64);
    ...
    if (!secondChildOfLoopTestNode->getType().isInt64()) {
      ...
      mulNode = TR::Node::create(
        TR::lmul, 2, secondChildOfLoopTestNode->duplicateTree(), constantNode);
  } else
    mulNode =
      TR::Node::create(TR::imul,
                       2,
                       secondChildOfLoopTestNode->duplicateTree(),
                       duplicateMulTermNode(
                         bestCandidate, secondChildOfLoopTestNode, TR::Int32));
  ...
  if (getAdditiveTermNode(bestCandidate) != 0) {
    ...
    addNode = TR::Node::create(
      mulNode->getType().isInt64() ? TR::ladd : TR::iadd,
      2,
      mulNode,
      duplicateAdditiveTermNode(
        bestCandidate, secondChildOfLoopTestNode, mulNode->getDataType()));
  ...
  if ((_linearEquations[bestCandidate][4] > -1) &&
    ...
    newStore =
      TR::Node::createWithSymRef(TR::astore, 1, 1, addNode, newSymbolReference);
  } else {
    newSymbolReference = comp()->getSymRefTab()->createTemporary(
    ...
    newStore = TR::Node::createWithSymRef(
      addNode->getType().isInt64() ? TR::lstore : TR::istore,
      1,
      1,
      addNode,
      newSymbolReference);
  ...
  TR::Node* secondOperand = TR::Node::createWithSymRef(
    secondChildOfLoopTestNode,
    comp()->il.opCodeForDirectLoad(newStore->getDataType()),
    0,
    newSymbolReference);
...
bool
TR_LoopStrider::foundValue(TR::Node* node,
                           int32_t symRefNum,
                           vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
...
bool
TR_LoopStrider::unchangedValueNeededIn(TR::Block* exitBlock,
                                       int32_t nextInductionVariableNumber,
                                       bool& seenStore)
{
...
TR::Node*
TR_LoopStrider::genLoad(TR::Node* node,
...
void
TR_LoopStrider::examineOpCodesForInductionVariableUse(
  TR::Node* node,
  TR::Node* parent,
  int32_t& childNum,
  int32_t& index,
  ...
  if ((replacingNode->getOpCodeValue() == TR::iload ||
    ...
    if (differenceInAdditiveConstants != 0) {
      ...
      if (adjustmentNode) {
        ...
        adjustmentNode = TR::Node::create(
          op, 2, adjustmentNode, differenceInAdditiveConstantsNode);
      } else {
    ...
    if (canHoistAdditiveTerm) {
      ...
      TR::Node* newStore = TR::Node::createWithSymRef(
        comp()->il.opCodeForDirectStore(adjustmentNode->getDataType()),
        1,
        1,
        adjustmentNode->duplicateTree(),
        newLoopInvariant);
      ...
      dumpOptDetails(
        comp(),
        "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis "
        "inserted initialization tree : %p for new loop invariant symRef #%d\n",
        newStoreTreeTop->getNode(),
...
bool
TR_LoopStrider::examineTreeForInductionVariableUse(
  TR::Block* loopInvariantBlock,
  TR::Node* parent,
  int32_t childNum,
  TR::Node* node,
  ...
  TR::SymbolReference** newSymbolReference)
{
  ...
  if ((node->getOpCodeValue() == TR::iadd ||
    ...
    if (isExprLoopInvariant(node->getSecondChild())) {
      ...
      if ((node->getFirstChild()->getOpCodeValue() == TR::imul ||
        ...
        if (replacingNode) {
          *newSymbolReference = NULL;
          int32_t index = findNewInductionVariable(
            node, newSymbolReference, true, internalPointerSymbol);
          ...
          if (node->getSecondChild()->getOpCode().isLoadConst()) {
            TR_ASSERT(isAdditiveTermConst(index),
                      "Both terms should be constants %d",
                      index);
          ...
          examineOpCodesForInductionVariableUse(node,
                                                parent,
                                                childNum,
                                                index,
                                                originalNode,
          ...
          dumpOptDetails(
            ...
            node,
            comp()->getDebug()->getName(node->getOpCodeValue()),
            (*newSymbolReference)->getReferenceNumber());
        }
      }
    } else {
    ...
    bool shift =
      node->getOpCodeValue() == TR::ishl || node->getOpCodeValue() == TR::lshl;
    ...
    if (!mulTerm || !isExprLoopInvariant(mulTerm) ||
        !mulTerm->getType()
           .isIntegral() || // I changed isSignedInt->isIntegral -- but I don't
                            // think unsigned types ever reach here
        (!mulTerm->getOpCode().isLoadConst() && shift))
      replacingNode = NULL;
    ...
    if (replacingNode) {
      *newSymbolReference = NULL;
      int32_t index = findNewInductionVariable(
        node, newSymbolReference, false, internalPointerSymbol);
  ...
  if (examineChildren) {
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
      if (examineTreeForInductionVariableUse(loopInvariantBlock,
                                             node,
                                             i,
                                             node->getChild(i),
                                             visitCount,
...
void
TR_LoopStrider::populateLinearEquation(TR::Node* node,
                                       int32_t loopDrivingInductionVar,
                                       int32_t derivedInductionVar,
                                       int32_t internalPointerSymbol,
                                       TR::Node* invariantMultiplicationTerm)
{
  ...
  if (node->getOpCodeValue() == TR::iadd ||
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
      ...
      if (intType)
        addTerm =
          TR::Node::create(node, TR::iconst, 0, -1 * (int32_t)additiveConstant);
      else {
...
void
TR_LoopStrider::setInternalPointer(TR::Symbol* symbol,
                                   TR::AutomaticSymbol* pinningArrayPointer)
{
  _numInternalPointers++;
...
void
TR_LoopStrider::placeStore(TR::Node* newStore, TR::Block* loopInvariantBlock)
{
...
TR::Node*
TR_LoopStrider::findReplacingNode(TR::Node* node, bool usingAladd, int32_t k)
{
...
TR::DataType
TR_LoopStrider::findDataType(TR::Node* node,
                             bool usingAladd,
                             bool isInternalPointer)
{
...
void
TR_LoopStrider::createParmAutoPair(TR::SymbolReference* parmSymRef,
...
void
TR_LoopStrider::addLoad(TR_StoreTreeInfo* info, TR::Node* load, int32_t index)
{
...
TR::Node*
TR_LoopStrider::placeInitializationTreeInLoopInvariantBlock(
  ...
  TR::SymbolReferenceTable* symRefTab)
{
  ...
  if (usingAladd) {
    ...
    mulNode = TR::Node::create(
      newLoad->getType().isInt64() ? TR::lmul : TR::imul,
      2,
      newLoad,
...
TR::Node*
TR_LoopStrider::placeNewInductionVariableIncrementTree(
  TR_BlockStructure* loopInvariantBlock,
  TR::SymbolReference* inductionVarSymRef,
  TR::SymbolReference* newSymbolReference,
  int32_t k,
  ...
  if (lookup != _storeTreesList->end()) {
    ...
    for (; storeTree != NULL;) {
      ...
      TR::Node* newStore = placeNewInductionVariableIncrementTree(
        ...
        storeTree->getData()->_constNode,
        storeTree->getData()->_isAddition);
    ...
    returnValue = placeNewInductionVariableIncrementTree(loopInvariantBlock,
                                                         inductionVarSymRef,
                                                         newSymbolReference,
                                                         k,
                                                         symRefTab,
  ...
  bool isAddition)
{
  ...
  if (isAddition) {
    ...
    if (_linearEquations[k][4] > -1) {
      if (constNode->getOpCode().isLoadConst()) {
        ...
        TR::ILOpCodes op =
          (usingAladd || newMulNode->getType().isInt64()) ? TR::lneg : TR::ineg;
  ...
  if (_linearEquations[k][4] > -1)
    newStore = TR::Node::createWithSymRef(
      TR::astore, 1, 1, loopIncrementer, newSymbolReference);
  else {
    TR::ILOpCodes op = (usingAladd || (loopIncrementer->getType().isInt64()))
                         ? TR::lstore
                         : TR::istore;
    newStore =
      TR::Node::createWithSymRef(op, 1, 1, loopIncrementer, newSymbolReference);
  }
...
int32_t
TR_LoopStrider::findNewInductionVariable(TR::Node* node,
                                         TR::SymbolReference** symRef,
                                         bool hasAdditiveTerm,
...
void
TR_LoopStrider::identifyExpressionsLinearInInductionVariables(
...
bool
TR_LoopStrider::identifyExpressionLinearInInductionVariable(TR::Node* node,
                                                            vcount_t visitCount)
{
  // for aladds
  bool usingAladd = (TR::Compiler->target.is64Bit()) ? true : false;
...
TR::Node*
TR_LoopStrider::setUsesLoadUsedInLoopIncrement(TR::Node* node, int32_t k)
{
  if (_storeTreesList) {
...
TR::Node*
TR_LoopStrider::isExpressionLinearInInductionVariable(TR::Node* node, int32_t k)
{
  // for aladds
  bool usingAladd = (TR::Compiler->target.is64Bit()) ? true : false;
...
bool
TR_LoopStrider::isExpressionLinearInSomeInductionVariable(TR::Node* node)
{
...
bool
TR_LoopStrider::reassociateAndHoistComputations(TR::Block* loopInvariantBlock,
                                                TR_Structure* structure)
{
  ...
  if (structure->asBlock() != NULL) {
    ...
    while (currentTree != exitTree) {
      ...
      if (reassociateAndHoistComputations(
            loopInvariantBlock, NULL, -1, currentNode, comp()->getVisitCount()))
                                                ...
                                                vcount_t visitCount)
{
  ...
  if (((node->getOpCodeValue() == TR::iadd) ||
    ...
    if (node->getSecondChild()->getOpCodeValue() == TR::iconst ||
      ...
      if ((isInternalPointer &&
          ...
          performTransformation(
            comp(),
            "%s Replacing reassociated internal pointer based on symRef #%d\n",
            OPT_DETAILS,
            internalPointerSymbol)) {
        ...
        TR::Node* newLoad =
          TR::Node::createWithSymRef(node, TR::aload, 0, internalPointerSymRef);
        newLoad->setLocalIndex(~0);
  ...
  if (examineChildren) {
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
      if (reassociateAndHoistComputations(
            loopInvariantBlock, node, i, node->getChild(i), visitCount))
        reassociatedComputation = true;
    }
  }
...
bool
TR_LoopStrider::isStoreInRequiredForm(int32_t symRefNum,
...
void
TR_LoopStrider::checkIfIncrementInDifferentExtendedBlock(
  TR::Block* block,
...
bool
TR_LoopStrider::isStoreInRequiredForm(TR::Node* storeNode,
                                      int32_t symRefNum,
                                      TR_Structure* loopStructure)
{
  /* calls too can kill symRefs via loadaddr */
 ...
 */
static bool
enableExpensiveLoopStriderAssertions()
{
#ifdef DEBUG
  return true;
#else
...
static void
orderSensitiveDescendantsRec(TR::Node* node,
 ...
 */
static void
orderSensitiveDescendants(TR::Node* node, TR::NodeChecklist& out)
{
 ...
 */
static bool
substPreservesEvalOrder(TR::Node* orig, TR::Node* replacement)
{
 ...
 */
static void
assertSubstPreservesEvalOrder(TR::Node* orig,
                              TR::Node* replacement,
...
// (below)
static void
assertSubtreeDoesNotMentionOriginals(
  TR::Node* node,
  ...
  TR::NodeChecklist& visited)
{
  ...
  for (int i = 0; i < node->getNumChildren(); i++)
    assertSubtreeDoesNotMentionOriginals(
      node->getChild(i), widenedIVs, visited);
 ...
 */
static void
assertStructureDoesNotMentionOriginals(
  TR_Structure* structure,
  const TR::list<std::pair<int32_t, int32_t>>& widenedIVs)
{
  if (!enableExpensiveLoopStriderAssertions())
 ...
 */
static bool
isLoopRelativelyFrequent(TR_Structure* loop, TR::Block* preheader)
{
 ...
 */
void
TR_LoopStrider::detectLoopsForIndVarConversion(TR_Structure* loopStructure,
 ...
 */
void
TR_LoopStrider::extendIVsOnLoopEntry(
 ...
 */
void
TR_LoopStrider::truncateIVsOnLoopExit(
  const TR::list<std::pair<int32_t, int32_t>>& ivs,
 ...
 */
void
TR_LoopStrider::convertIV(TR::Node* bciNode,
                          TR::TreeTop* nextTT,
                          int32_t ivIn,
                          int32_t ivOut,
                          TR::ILOpCodes op)
...
void
TR_LoopStrider::createConstraintsForNewInductionVariable(
  TR_Structure* loopStructure,
  ...
  TR::SymbolReference* oldSymRef)
{
  ...
  if (v) {
    ...
    TR_InductionVariable* newv = new (trHeapMemory())
      TR_InductionVariable(newSymRef->getSymbol()->castToRegisterMappedSymbol(),
                           newEntryVal,
                           newExitVal,
                           newIncr,
...
void
TR_LoopStrider::morphExpressionsLinearInInductionVariable(
  TR_Structure* structure,
  vcount_t visitCount)
{
  if (structure->asBlock() != NULL) {
    ...
    while (currentTree != exitTree) {
      TR::Node* currentNode = currentTree->getNode();
      _currTree = currentTree;
      morphExpressionLinearInInductionVariable(
        NULL, -1, currentNode, visitCount);
      currentTree = currentTree->getNextTreeTop();
    }
  } else {
    TR_RegionStructure* regionStructure = structure->asRegion();
...
bool
TR_LoopStrider::morphExpressionLinearInInductionVariable(TR::Node* parent,
                                                         int32_t childNum,
                                                         TR::Node* node,
                                                         vcount_t visitCount)
{
  ...
  if (examineChildren) {
    ...
    for (i = 0; i < node->getNumChildren(); i++)
      morphExpressionLinearInInductionVariable(
        node, i, node->getChild(i), visitCount);
  }
  return true;
}

TR::Node*
TR_LoopStrider::getInductionVariableNode(TR::Node* node)
{
 ...
 */
void
TR_LoopStrider::walkTreesAndFixUseDefs(TR_Structure* loopStructure,
                                       TR::SymbolReference* newSymbolReference,
                                       TR::NodeChecklist& exLoads)
{
  ...
  replaceLoadsInStructure(loopStructure,
                          _loopDrivingInductionVar,
                          newIntValue,
                          newSymbolReference,
                          exLoads,
  ...
  createConstraintsForNewInductionVariable(
    loopStructure, newSymbolReference, oldSymRef);
}
 ...
 */
void
TR_LoopStrider::replaceLoadsInStructure(TR_Structure* structure,
                                        int32_t iv,
                                        TR::Node* defRHS,
  ...
  if (structure->asBlock() != NULL) {
    ...
    for (TR_StructureSubGraphNode* n = it.getFirst(); n != NULL;
      ...
      replaceLoadsInStructure(
        n->getStructure(), iv, defRHS, newSR, exLoads, visited);
  }
}

// node-recursive implementation of replaceLoadsInStructure (above)
void
TR_LoopStrider::replaceLoadsInSubtree(TR::Node* node,
                                      int32_t iv,
                                      TR::Node* defRHS,
                                      TR::SymbolReference* newSR,
                                      TR::NodeChecklist& exLoads,
                                      TR::NodeChecklist& visited)
{
  ...
  visited.add(node);
  for (int i = 0; i < node->getNumChildren(); i++)
    replaceLoadsInSubtree(
      node->getChild(i), iv, defRHS, newSR, exLoads, visited);
 ...
 */
void
TR_LoopStrider::widenComparison(TR::Node* node,
                                int32_t iv,
                                TR::Node* defRHS,
                                TR::NodeChecklist& exLoads)
{
 ...
 */
void
TR_LoopStrider::eliminateSignExtensions(TR::NodeChecklist& exLoads)
{
  TR::NodeChecklist visited(comp());
  ...
  for (TR::TreeTop* tt = start; tt != NULL; tt = tt->getNextTreeTop())
    eliminateSignExtensionsInSubtree(
      tt->getNode(), exLoads, visited, signExtMemo);
...
void
TR_LoopStrider::eliminateSignExtensionsInSubtree(TR::Node* parent,
                                                 TR::NodeChecklist& exLoads,
                                                 TR::NodeChecklist& visited,
...
TR_LoopStrider::signExtend(TR::Node* node,
                           TR::NodeChecklist& exLoads,
                           SignExtMemo& memo)
{
...
TR_LoopStrider::SignExtEntry
TR_LoopStrider::signExtendBinOp(TR::ILOpCodes op64,
                                TR::Node* node,
                                TR::NodeChecklist& exLoads,
                                SignExtMemo& memo)
{
 ...
 */
void
TR_LoopStrider::transmuteDescendantsIntoTruncations(TR::Node* intNode,
                                                    TR::Node* longNode)
{
...
bool
TR_LoopStrider::checkStoreOfIndVar(TR::Node* defNode)
{
...
bool
TR_LoopStrider::checkExpressionForInductionVariable(TR::Node* node)
{
...
int64_t
TR_LoopStrider::getAdditiveTermConst(int32_t k)
{
...
bool
TR_LoopStrider::isAdditiveTermEquivalentTo(int32_t k, TR::Node* node)
{
...
TR::Node*
TR_LoopStrider::duplicateMulTermNode(int32_t k,
                                     TR::Node* node,
                                     TR::DataType type)
{
  TR_ASSERT(k < _numberOfLinearExprs,
  ...
  if (new_node->getDataType() != type)
    new_node = TR::Node::create(
      TR::ILOpCode::getProperConversion(
        new_node->getDataType(), type, false /* !wantZeroExtension */),
      1,
      new_node);
  return new_node;
}
...
int64_t
TR_LoopStrider::getMulTermConst(int32_t k)
{
  TR_ASSERT(k < _numberOfLinearExprs,
            "index k %d exceeds _numberOfLinearExprs %d!\n",
            k,
...
bool
TR_LoopStrider::isMulTermEquivalentTo(int32_t k, TR::Node* node)
{
...
ceiling(int64_t numer, int64_t denom)
{
...
int32_t
TR_InductionVariableAnalysis::perform()
{
...
void
TR_InductionVariableAnalysis::removeStaleIVs(TR_RegionStructure* region)
{
...
void
TR_InductionVariableAnalysis::gatherCandidates(TR_Structure* s,
                                               ...
                                               TR_BitVector* allDefs)
{
  if (s->asRegion()) {
    ...
    if (!region->isAcyclic()) {
      ...
      if (trace()) {
        traceMsg(
          comp(), "All Defs inside cyclic region %d: ", region->getNumber());
...
void
TR_InductionVariableAnalysis::perform(TR_RegionStructure* region)
{
...
void
TR_InductionVariableAnalysis::analyzeNaturalLoop(TR_RegionStructure* loop)
{
...

  {
    ...
    if (trace())
      traceMsg(comp(),
               "<analyzeNaturalLoop loop=%d addr=%p>\n",
               loop->getNumber(),
               loop);
...
void
TR_InductionVariableAnalysis::analyzeAcyclicRegion(TR_RegionStructure* region,
                                                   TR_RegionStructure* loop)
{
  if (loop != region)
...
void
TR_InductionVariableAnalysis::analyzeBlock(TR_BlockStructure* structure,
                                           TR_RegionStructure* loop)
{
...
void
TR_InductionVariableAnalysis::analyzeCyclicRegion(TR_RegionStructure* region,
                                                  TR_RegionStructure* loop)
{
...
void
TR_InductionVariableAnalysis::mergeWithBlock(TR::Block* block,
                                             DeltaInfo** inSet,
                                             TR_RegionStructure* loop)
{
...
void
TR_InductionVariableAnalysis::mergeWithSet(DeltaInfo** mySet,
                                           DeltaInfo** inSet,
                                           TR_RegionStructure* loop)
{
  TR_BitVector* candidates =
...
//
bool
TR_InductionVariableAnalysis::isProgressionalStore(TR::Node* node,
                                                   TR_ProgressionKind* kind,
                                                   int64_t* incr)
{
...
//
bool
TR_InductionVariableAnalysis::branchContainsInductionVariable(
  TR_RegionStructure* loop,
  TR::Node* branchNode,
  TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
  ...
  for (iv = it.getFirst(); iv; iv = it.getNext()) {
    ...
    if (trace())
      traceMsg(comp(),
               "\t considering branchnode [%p] and basiciv [%d]\n",
               branchNode,
               index);
    // setting a node count budget for walking the nodes
    int32_t nodeBudget = 100;
    if (branchContainsInductionVariable(
          branchNode, iv->getSymRef(), &nodeBudget)) {
      if (trace())
        traceMsg(comp(),
                 "\tbranchnode [%p] contains basiciv [%d]\n",
                 branchNode,
                 index);
      result = true;
      ...
      if (firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub() ||
        ...
        if (trace())
          traceMsg(comp(),
                   "\tbut branch expr [%p] is not in recognized form\n",
                   firstChild);
      }
    } else {
      if (trace())
        traceMsg(comp(),
                 "\tbranchnode [%p] does not contain basiciv [%d]\n",
                 branchNode,
                 index);
    }
  }
  ...
  int32_t* nodeBudget)
{
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; i--)
    if (branchContainsInductionVariable(
          node->getChild(i), ivSymRef, nodeBudget))
      return true;
...
//
bool
TR_InductionVariableAnalysis::getProgression(TR::Node* expr,
...
void
TR_InductionVariableAnalysis::analyzeLoopExpressions(TR_RegionStructure* loop,
                                                     DeltaInfo** loopSet)
{
  TR_BitVector* candidates =
    ((AnalysisInfo*)loop->getAnalysisInfo())->getLoopLocalDefs();
    ...
    if (info->isUnknownValue()) {
      ...
      if (info->getDelta() == 0) {
        ...
        TR_BasicInductionVariable* biv =
          new (trHeapMemory()) TR_BasicInductionVariable(comp(), loop, symRef);
...
//
static uint16_t
countNodeOccurrencesInSubTree(TR::Node* root,
                              TR::Node* node,
                              vcount_t visitCount,
                              TR::Compilation* comp)
{
  ...
  for (int8_t c = root->getNumChildren() - 1; c >= 0; --c)
    count +=
      countNodeOccurrencesInSubTree(root->getChild(c), node, visitCount, comp);
  return count;
}

static bool
identicalBranchTrees(TR::Node* node1, TR::Node* node2)
{
...
void
TR_InductionVariableAnalysis::appendPredecessors(WorkQueue& dest,
                                                 TR::Block* block)
{
      ...
      if (curNode->getOpCode().isStoreDirect() &&
        ...
        if (trace())
          traceMsg(comp(),
                   ...
                   "a loop test block\n",
                   curNode,
                   candidateIV->getSymbolReference()->getSymbol(),
                   curBlock->getNumber());
        return false;
      }
    }
...
bool
TR_InductionVariableAnalysis::analyzeExitEdges(
  TR_RegionStructure* loop,
  TR_BitVector* candidates,
  TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
  ...
  TR_Array<DeltaInfo*> exitInfos(
    trMemory(), candidates->elementCount(), true, stackAlloc);
  TR_Array<TR::Node*> bounds(
    trMemory(), candidates->elementCount(), true, stackAlloc);
  ...
  for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
    ...
    if (trace())
      traceMsg(comp(),
               "\tLook at node %d exiting the loop\n",
    ...
    if (subNode->getStructure()->asRegion()) {
      ...
      if (!_seenInnerRegionExit.get(toNum)) {
        ...
        for (auto predEdge = end->getPredecessors().begin();
             predEdge != end->getPredecessors().end();
             ++predEdge) {
    ...
    if (!branchContainsInductionVariable(loop, node, basicIVs)) {
      ...
      if (trace())
        traceMsg(
          comp(), "\tBranch [%p] does not involve basic ivs, ignoring\n", node);
      continue;
    }
    ...
    if (!getProgression(node->getFirstChild(), 0, &symRef, &kind, &incr)) {
      if (trace())
        traceMsg(
          comp(), "\tReject - branch node %p in unrecognized form\n", node);
      return false;
    }
    ...
    if (controllingIV == -1) {
      ...
      if (trace())
        traceMsg(comp(),
                 "\tReject - more than one loop test %p and %p\n",
                 node,
  ...
  if (valueNode->getOpCode().isLoadVarDirect()) {
    ...
    if (valueCouldBeEvaluatedBeforeTest || forceILWalk) {
      ...
      if (usesUnchangedValueInLoopTest) {
        if (!isIVUnchangedInLoop(
              loop, controllingBranch->getEnclosingBlock(), valueNode)) {
      ...
      if (curNode->getOpCode().isStoreDirect() &&
        ...
        if (curNode->getFirstChild() != valueNode) {
          if (trace())
            traceMsg(
              comp(),
              "\tRejected - exit value %p maybe not same as tested value %p\n",
              curNode,
              valueNode);
  ...
  if (trace())
    traceMsg(comp(),
             "Found Loop Controlling Induction Variable: %d\n",
  ...
  TR_PrimaryInductionVariable* piv = new (trHeapMemory())
    TR_PrimaryInductionVariable(biv,
                                controllingBlock,
                                boundNode->duplicateTree(),
                                opCode,
                                comp(),
                                optimizer(),
...
bool
TR_InductionVariableAnalysis::findEntryValues(
  TR_RegionStructure* loop,
  TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
  TR_ArrayIterator<TR_BasicInductionVariable> it(&basicIVs);
...
TR::Node*
TR_InductionVariableAnalysis::findEntryValueForSymRef(
  TR_RegionStructure* loop,
  TR::SymbolReference* symRef)
{
  ...
  TR_Array<TR::Node*> cachedValues(
    trMemory(), comp()->getFlowGraph()->getNextNodeNumber(), true, stackAlloc);
...
TR_PrimaryInductionVariable::TR_PrimaryInductionVariable(
  ...
  TR::Compilation* c,
  TR::Optimizer* opt,
  bool usesUnchangedValueInLoopTest,
  bool trace)
  ...
  , _iterCount(-1)
{
                                        ...
                                        TR_TraceAll))) // always dump the info
  {
    ...
    traceMsg(comp(),
             "Loop Controlling Induction Variable %d (%p):\n",
             getSymRef()->getReferenceNumber(),
             this);
    if (_iterCount != -1)
    ...
    traceMsg(comp(),
             "  UsesUnchangedValueInLoopTest: %d\n",
             usesUnchangedValueInLoopTest);
  }
...
TR::Node*
TR_PrimaryInductionVariable::getExitValue()
{
...
TR::Node*
TR_DerivedInductionVariable::getExitValue()
{
...
TR_IVTypeTransformer::perform()
{
  ...
  detectWhileLoops(whileLoopsInnerFirst,
                   whileLoops,
                   doWhileLoopsInnerFirst,
                   doWhileLoops,
                   _rootStructure,
                   true);
...
static bool
isIdentityExpr(TR::Node* node, TR::SymbolReference* symref)
{
  return node->getOpCode().isLoadVarDirect() &&
         node->getSymbolReference() == symref;
}
void
TR_IVTypeTransformer::changeIVTypeFromAddrToInt(TR_RegionStructure* natLoop)
{
  auto cm = comp();
  ...
  while (!SGNQueue.IsEmpty()) {
                    ...
                    // be pessimistic for now.
    {
      if (trace())
        traceMsg(cm,
                 "Null structure encountered at graph node %i\n",
                 SGNode->getNumber());
      return;
    }
    ...
    if (auto region = structure->asRegion()) {
      SGNode =
        region
          ->getEntry(); // Look through inner loops as well. TODO: Memoization
  ...
  if (!performTransformation(
        cm,
        "%s Initializing base and int index in preheader block_%i of loop %i\n",
        optDetailString(),
  ...
  // Add int increment
  if (!performTransformation(
        cm, "%s Adding int increment tree\n", optDetailString()))
    return;
  ...
  auto a2intOp = TR::ILOpCode::getProperConversion(
    TR::Address, TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32, false);
  TR::Node* newIf;
      ...
      firstChildSymRef) // Match if children to original backedgeIf children
  {
    newIf = TR::Node::createif(
      ...
      TR::Node::create(
        TR::Compiler->target.is64Bit() ? TR::lsub : TR::isub,
        2,
  ...
  if (!performTransformation(
        cm,
        "%s Transforming aload's in loop body to base+int form\n",
        optDetailString()))
    return;
  visitedSGN.Clear();
  SGNQueue.Push(natLoop->getEntry());
...
TR::ILOpCodes
TR_IVTypeTransformer::getIntegralIfOpCode(TR::ILOpCodes ifacmp, bool is64bit)
{
  auto op = ifacmp;
  switch (ifacmp) {
    case TR::ifacmpeq:
...
void
TR_IVTypeTransformer::replaceAloadWithBaseIndexInSubtree(TR::Node* node)
{
  if (node->getVisitCount() == comp()->getVisitCount())
    return;
  node->setVisitCount(comp()->getVisitCount());
  ...
  if (child && child->getOpCodeValue() == TR::aload &&
      ...
      performTransformation(
        comp(),
        "%s Replacing n%in aload with base int-index form\n",
        optDetailString(),
        child->getGlobalIndex())) {
    auto arrayRef = TR::Node::recreateWithoutProperties(
      child,
      TR::Compiler->target.is64Bit() ? TR::aladd : TR::aiadd,
      2,
...
// Sets increment
bool
TR_IVTypeTransformer::isInAddrIncrementForm(TR::Node* node, int32_t& increment)
{
  if (node->getVisitCount() == comp()->getVisitCount())
    return false;
  node->setVisitCount(comp()->getVisitCount());
...
TR::SymbolReference*
TR_IVTypeTransformer::findComparandSymRef(TR::Node* node)
{
  if (node->getOpCode().hasSymbolReference())
