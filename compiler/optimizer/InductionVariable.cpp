  ...
  TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
  ...
  if (loopInvariantBlock) {
    ...
    //**)trMemory()->allocateStackMemory(symRefCount*sizeof(List<TR_StoreTreeInfo> *)); _loadUsedInNewLoopIncrementList
    //= (List<TR::Node> **)trMemory()->allocateStackMemory(symRefCount*sizeof(List<TR::Node> *));
    ...
    if (1) {
      ...
      if (prevInvariantNode && prevInvariantNode->getStructure()->asBlock()
        ...
        _numInternalPointerOrPinningArrayTempsInitialized
          += countInternalPointerOrPinningArrayStores(comp(), prevInvariantNode->getStructure()->asBlock()->getBlock());
      }
    }
        ...
        if (storeInRequiredForm
          ...
          if (replaceAllInductionVariableComputations(
              ...
              if (newSymbolReference->getSymbol()->getDataType() != TR::Address) {
                ...
                if (v) {
                  ...
                  TR_InductionVariable* iv = new (trHeapMemory())
                    TR_InductionVariable(newSymbolReference->getSymbol()->castToRegisterMappedSymbol(), newEntryVal,
                      newExitVal, newIncr, _loopTestTree->getNode()->getOpCode().isUnsignedCompare() ? TR_no : TR_yes);
                  if (bestCandidate == k)
                    strideForPIV = iv;
              ...
              !(0 && // no-strict IV
                ...
                && (!isAdditiveTermConst(bestCandidate) || // non-const additive term
                  ...
                  !(strideForPIV->getEntry()->asLongConstraint()
                      ? (strideForPIV->getEntry()->getLowLong() >= 0 || strideForPIV->getExit()->getHighLong() <= 0)
            ...
            if (performTransformation(comp(), "%s Adding auto %d as a global register candidate in loop %d\n",
                  OPT_DETAILS, inductionCandidate->getSymbolReference()->getReferenceNumber(),
                  loopStructure->getNumber()))
    ...
    for (j = 0; j < symRefCount; j++) {
      ...
      TR::SymbolReference* reassociatedAuto
        = reassociatedAutoSearchResult != _reassociatedAutos->end() ? reassociatedAutoSearchResult->second : NULL;
      if (reassociatedAuto) {
        ...
        if (usingAladd) {
          ...
          arrayRefNode = TR::Node::create(
            TR::aladd, 2, TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0, origAuto), constantNode);
        } else
          arrayRefNode
            = TR::Node::create(TR::aiadd, 2, TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0, origAuto),
      ...
      if (symRefPair) {
        ...
        while (symRefPair) {
          ...
          if (symRefPair->_isConst) {
            ...
            if (usingAladd)
              arrayRefNode
                = TR::Node::create(TR::aladd, 2, TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0, origAuto),
          ...
          if (!origAuto->getSymbol()->isInternalPointer()) {
            symRefPair->_derivedSymRef->getSymbol()
              ...
              ->setPinningArrayPointer();
            arrayRefNode->setPinningArrayPointer(
              symRefPair->_derivedSymRef->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
          } else
            arrayRefNode->setPinningArrayPointer(
              origAuto->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
          ...
          dumpOptDetails(comp(),
            "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted invariant initialization tree : "
            "%p for new symRef #%d\n",
  ...
  TR::Node* storeOfDerivedInductionVariable)
{
  ...
  if ((_linearEquations[bestCandidate][4] > -1)
    ...
    if (!newAload->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer()) {
      ...
      addNode->setPinningArrayPointer(
        newAload->getSymbolReference()->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
  ...
  if (firstChildOfLoopTestNode->getOpCode().hasSymbolReference()) {
    ...
    TR::Node* newLoad = getNewLoopIncrement(
      firstChildOfLoopTestNode, bestCandidate, firstChildOfLoopTestNode->getSymbolReference()->getReferenceNumber());
    /*
    ...
    */
    ...
    if (newLoad) {
      ...
      firstOperand = TR::Node::createWithSymRef(firstChildOfLoopTestNode,
        comp()->il.opCodeForDirectLoad(derivedSymbolReference->getSymbol()->getDataType()), 0, derivedSymbolReference);
    }
  } else
  ...
  if ((replacingNode->getOpCodeValue() == TR::iload || replacingNode->getOpCodeValue() == TR::lload)
    ...
    if (downcastNode && node->getOpCodeValue() == TR::lload) {
      ...
      if (parent && parent->getType().isInt64() &&
        ...
        && !parent->getOpCode().isConversion() && parent->getOpCodeValue() != TR::lternary) {
    ...
    if (isInternalPointer) {
      ...
      if (usingAladd) {
        ...
        if (replacingNode->getOpCodeValue() == TR::isub) {
          adjustmentNode = TR::Node::create(TR::imul, 2, adjustmentNode, TR::Node::create(node, TR::iconst, 0, -1));
          adjustmentNode->setLocalIndex(~0);
          adjustmentNode->getSecondChild()->setLocalIndex(~0);
        }
    ...
    if (differenceInAdditiveConstants != 0) {
      ...
      if (adjustmentNode) {
        ...
        if (usingAladd || (replacingNode->getType().isInt64())) {
          ...
          adjustmentNode = TR::Node::create(node, TR::iconst, 0, differenceInAdditiveConstants);
      }
    }
    ...
    if (canHoistAdditiveTerm) {
      TR::SymbolReference* newLoopInvariant
        = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), adjustmentNode->getDataType(), false);
      TR::Node* newLoad = TR::Node::createLoad(node, newLoopInvariant);
      newLoad->setLocalIndex(~0);
      TR::Node* newStore = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(adjustmentNode->getDataType()), 1,
        1, adjustmentNode->duplicateTree(), newLoopInvariant);
      ...
      dumpOptDetails(comp(),
        "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted initialization tree : %p for new loop "
        "invariant symRef #%d\n",
  ...
  int32_t childNum, TR::Node* node, vcount_t visitCount, TR::SymbolReference** newSymbolReference)
{
  ...
  if (cg()->supportsInternalPointers() && (node->isInternalPointer()) && node->getFirstChild()->getOpCode().isLoadVar()
  ...
  if ((node->getOpCodeValue() == TR::iadd || node->getOpCodeValue() == TR::isub)
    ...
    if (isExprLoopInvariant(node->getSecondChild())) {
      ...
      if ((node->getFirstChild()->getOpCodeValue() == TR::imul || node->getFirstChild()->getOpCodeValue() == TR::ishl)
        ...
        bool shift
          = node->getFirstChild()->getOpCodeValue() == TR::ishl || node->getFirstChild()->getOpCodeValue() == TR::lshl;
        ...
        if (!mulTerm || !isExprLoopInvariant(mulTerm) || !mulTerm->getType().isIntegral()
          || // we were checking for signed here -- i made the test a bit weaker, but I don't think we can reach here
             // with unsigned
        ...
        if (replacingNode) {
          *newSymbolReference = NULL;
          ...
          examineOpCodesForInductionVariableUse(node, parent, childNum, index, originalNode, replacingNode, linearTerm,
            mulTerm, newSymbolReference, loopInvariantBlock, pinningArrayPointer, differenceInAdditiveConstants,
      ...
      *newSymbolReference = NULL;
      ...
      if (canCreateNewSymRef) {
        ...
        *newSymbolReference
        ...
        index = _nextExpression;
        populateLinearEquation(
          node, _loopDrivingInductionVar, (*newSymbolReference)->getReferenceNumber(), internalPointerSymbol, mulTerm);
      }
  ...
  TR::SymbolReferenceTable* symRefTab)
{
  ...
  dumpOptDetails(comp(),
    "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted initialization tree : %p for new symRef "
    "#%d\n",
  ...
  TR::SymbolReferenceTable* symRefTab, TR::Node* placeHolderNode, int32_t loopTestHasSymRef)
{
  ...
  if (lookup != _storeTreesList->end()) {
    ...
    returnValue = placeNewInductionVariableIncrementTree(loopInvariantBlock, inductionVarSymRef, newSymbolReference, k,
      symRefTab, placeHolderNode, newLoad, _insertionTreeTop, _constNode, _isAddition);
  }
  ...
  TR::Node* constNode, bool isAddition)
{
  ...
  dumpOptDetails(comp(),
    "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted loop incremental step tree : %p for new "
    "symRef #%d\n",
    newStoreTreeTop->getNode(), newSymbolReference->getReferenceNumber());
  return newStore;
}
    ...
    if ((isConst
      ...
      if (!internalPointerSymRef) {
        TR::SymbolReference* newSymbolReference
          = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address, isInternalPointer);
        if (isInternalPointer && !pinningArrayPointer) {
          TR::SymbolReference* newPinningArray
            = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Address, false);
          pinningArrayPointer = newPinningArray->getSymbol()->castToAutoSymbol();
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (constNode && _storeTrees[defNode->getSymbolReference()->getReferenceNumber()]) {
          checkIfIncrementInDifferentExtendedBlock(
            useDefInfo->getTreeTop(defIndex)->getEnclosingBlock()->startOfExtendedBlock(), inductionVariable);
        } else {
          constNode = NULL;
        }
        return constNode;
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          ...
          if (storeInRequiredForm
            && performTransformation(comp(), "%s Analyzing candidate: %d in loop: %d for conversion\n", OPT_DETAILS,
              _loopDrivingInductionVar, loopStructure->getNumber())) {
...
void TR_LoopStrider::truncateIVsOnLoopExit(const TR::list<std::pair<int32_t, int32_t> >& ivs, TR_RegionStructure* loop)
{
  ...
  for (TR::Block* src = lit.getFirst(); src != NULL; src = lit.getNext()) {
    ...
    for (TR::Block* dest = eit.getFirst(); dest != NULL; dest = eit.getNext()) {
      ...
      int preds = dest->getPredecessors().size() + dest->getExceptionPredecessors().size();
      if (preds > 1) {
        ...
        dumpOptDetails(comp(), "[Sign-Extn] split loop exit: block_%d [-> block_%d] -> block_%d\n", src->getNumber(),
          dest->getNumber(), origDest->getNumber());
      }
  ...
  TR_Structure* loopStructure, TR::SymbolReference* newSymRef, TR::SymbolReference* oldSymRef)
{
  ...
  if (v) {
    ...
    TR_InductionVariable* newv
      = new (trHeapMemory()) TR_InductionVariable(newSymRef->getSymbol()->castToRegisterMappedSymbol(), newEntryVal,
...
TR_LoopStrider::SignExtEntry TR_LoopStrider::signExtend(TR::Node* node, TR::NodeChecklist& exLoads, SignExtMemo& memo)
{
  ...
  case TR::l2i:
    if (exLoads.contains(node)) {
      TR::Node* child = node->getFirstChild();
      TR_ASSERT(
        child->getOpCodeValue() == TR::lload, "exload l2i n%un's child should be an lload\n", node->getGlobalIndex());
...
void TR_LoopStrider::transmuteDescendantsIntoTruncations(TR::Node* intNode, TR::Node* longNode)
{
  if (longNode->getOpCodeValue() == TR::i2l) {
    ...
    TR_ASSERT(child == intNode, "transmuting n%un %s does not match child n%un %s of n%un i2l\n",
      intNode->getGlobalIndex(), intNode->getOpCode().getName(), child->getGlobalIndex(), child->getOpCode().getName(),
...
TR::Node* TR_LoopStrider::duplicateMulTermNode(int32_t k, TR::Node* node, TR::DataType type)
{
  ...
  if (new_node->getDataType() != type)
    new_node = TR::Node::create(
      TR::ILOpCode::getProperConversion(new_node->getDataType(), type, false /* !wantZeroExtension */), 1, new_node);
  return new_node;
}
...
void TR_InductionVariableAnalysis::analyzeBlock(TR_BlockStructure* structure, TR_RegionStructure* loop)
{
  ...
  if (trace()) {
    ...
    while (it.hasMoreElements()) {
      ...
      traceMsg(comp(), "\t%d %d %p symRef=%p symbol=%p: ", refNum, refIndex, inSymbol, symRef, symRef->getSymbol());
      if (inSymbol)
...
bool TR_InductionVariableAnalysis::isProgressionalStore(TR::Node* node, TR_ProgressionKind* kind, int64_t* incr)
{
  ...
  {
    // traceMsg(comp(),"In isProgressionalStore cursor node %p returning false. symRef = %p(%d) is not equal to storeRef
      ...
      if (curNode->getOpCode().isStoreDirect()
        ...
        if (trace())
          traceMsg(comp(), "\t\tFound store %p of symRef %p in block %d, which is not a loop test block\n", curNode,
            candidateIV->getSymbolReference()->getSymbol(), curBlock->getNumber());
        return false;
      }
    }
  ...
  TR_RegionStructure* loop, TR_BitVector* candidates, TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
  ...
  for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
    ...
    if (!node->getOpCode().isJumpWithMultipleTargets()) {
      ...
      // TM: a jump with multiple target (this case, a tstart) is not a 'controlling branch to exit' in terms of the
      // temp induction variable, so we can continue
    ...
    if (valueCouldBeEvaluatedBeforeTest || forceILWalk) {
      ...
      if (usesUnchangedValueInLoopTest) {
        if (!isIVUnchangedInLoop(loop, controllingBranch->getEnclosingBlock(), valueNode)) {
          ...
          if (trace())
            traceMsg(comp(),
              "\tReject - IV node has been changed in one of the blocks that is not part of the loop test in the "
              "loop\n");
          return false;
        }
      }
...
TR::Node* TR_InductionVariableAnalysis::getEntryValue(
  TR::Block* block, TR::SymbolReference* symRef, TR_BitVector* nodesDone, TR_Array<TR::Node*>& cachedValues)
{
  ...
  for (TR::TreeTop* tt = block->getLastRealTreeTop(); tt != entryTree; tt = tt->getPrevRealTreeTop()) {
    ...
    if (node->getOpCode().isStoreDirect()) {
      ...
      // if you find somewhere a call like  foo(&num);  then this kills the num, and we can no longer reason about its
      // value.
...
TR_PrimaryInductionVariable::TR_PrimaryInductionVariable(TR_BasicInductionVariable* biv, TR::Block* branchBlock,
  TR::Node* exitBound, TR::ILOpCodes exitOp, TR::Compilation* c, TR::Optimizer* opt, bool usesUnchangedValueInLoopTest,
  bool trace)
  ...
  , _iterCount(-1)
{
  ...
  if (getEntryValue() && getEntryValue()->getOpCode().isLoadConst()
    ...
    int64_t entryVal = getEntryValue()->getOpCode().isUnsigned() ? getEntryValue()->get64bitIntegralValueAsUnsigned()
...
void TR_IVTypeTransformer::changeIVTypeFromAddrToInt(TR_RegionStructure* natLoop)
{
  ...
  do {
    ...
    && (tt->getNode()->getOpCodeValue() != TR::BBStart || (tt->getNode()->getBlock()->getPredecessors().size() == 1))
  ...
  _intIdxSymRef = cm->getSymRefTab()->createTemporary(
    cm->getMethodSymbol(), TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32);
  auto intIdxStoreTT = TR::TreeTop::create(cm,
    TR::Node::createStore(_intIdxSymRef, TR::Compiler->target.is64Bit() ? TR::Node::lconst(0) : TR::Node::iconst(0)));
  ...
  auto intIncrementTT = TR::TreeTop::create(cm,
    TR::Node::createStore(_intIdxSymRef,
      TR::Node::create(TR::Compiler->target.is64Bit() ? TR::ladd : TR::iadd, 2, TR::Node::createLoad(_intIdxSymRef),
  ...
  auto a2intOp
    = TR::ILOpCode::getProperConversion(TR::Address, TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32, false);
  TR::Node* newIf;
...
void TR_IVTypeTransformer::replaceAloadWithBaseIndexInSubtree(TR::Node* node)
{
  ...
  if (child && child->getOpCodeValue() == TR::aload && child->getSymbolReference() == _addrSymRef
    ...
    auto arrayRef = TR::Node::recreateWithoutProperties(child, TR::Compiler->target.is64Bit() ? TR::aladd : TR::aiadd,
