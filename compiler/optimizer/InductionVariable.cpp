...
TR_LoopStrider::TR_LoopStrider(TR::OptimizationManager *manager)
    : TR_LoopTransformer(manager), _reassociatedNodes(trMemory()),
...
int32_t
TR_LoopStrider::detectCanonicalizedPredictableLoops(TR_Structure *loopStructure,
        ...
        if (!mulTerm || !isExprLoopInvariant(mulTerm) ||
            !mulTerm->getType()
                 .isIntegral() || // we were checking for signed here -- i made
                                  // the test a bit weaker, but I don't think we
                                  // can reach here with unsigned
...
TR::Node *
TR_LoopStrider::updateLoadUsedInLoopIncrement(TR::Node *node,
 ...
 * \see transmuteDescendantsIntoTruncations(TR::Node*, TR::Node*)
 */
TR_LoopStrider::SignExtEntry
TR_LoopStrider::signExtend(TR::Node *node, TR::NodeChecklist &exLoads,
                           SignExtMemo &memo) {
  ...
  TR_ASSERT((result.extended != NULL) ==
                (result.cancelsExt || result.cancelsTrunc),
...
TR_LoopStrider::SignExtEntry
TR_LoopStrider::signExtendBinOp(TR::ILOpCodes op64, TR::Node *node,
...
void TR_LoopStrider::transmuteDescendantsIntoTruncations(TR::Node *intNode,
  ...
  TR_ASSERT(intOp ==
                TR::ILOpCode::integerOpForLongOp(longNode->getOpCodeValue()),
...
bool TR_InductionVariableAnalysis::isIVUnchangedInLoop(TR_RegionStructure *loop,
  ...
  if (disable) {
    if (trace())
      traceMsg(comp(), "\tintermediate value check disabled; assuming no "
  ...
  if (trace())
    traceMsg(comp(), "\tTrying to make sure that candidate IV hasn't been "
                     "modified elsewhere in the loop\n");
...
bool TR_InductionVariableAnalysis::analyzeExitEdges(
  ...
  if (trace())
    traceMsg(comp(), "Trying to analyze the exit edges to determine if this is "
                     "a counted loop\n");
  ...
  for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
    ...
    if (((!node->getOpCode().isBooleanCompare() ||
        ...
        (node->getOpCode().isJumpWithMultipleTargets() &&
         !node->getOpCode().hasBranchChildren()) || // allow tstarts to continue
    ...
    if (!(node->getFirstChild()->getType().isIntegral() ||
        ...
        !node->getOpCode().isJumpWithMultipleTargets()) {
      if (trace())
        traceMsg(comp(), "\tReject - Branch node is not integral nor ordered "
...
TR_PrimaryInductionVariable::TR_PrimaryInductionVariable(
  ...
  if (trace || (comp()->getDebug() && comp()->getOptions()->getAnyOption(
...
void TR_IVTypeTransformer::changeIVTypeFromAddrToInt(
  ...
  if (trace())
    traceMsg(cm, "all ok! Transforming addr accesses to arrayRef form of base "
                 "addr + int\n");
