...
bool TR_LoopStrider::foundLoad(TR::TreeTop* storeTreeTop, TR::Node* node, int32_t symRefNum, vcount_t visitCount)
{
    ...
    if (node->getOpCode().isLoadVar() && (node->getSymbolReference()->getReferenceNumber() == symRefNum) && (node->getReferenceCount() > 1)) {
        if (_storeTreesList) {
...
static int32_t countInternalPointerOrPinningArrayStores(TR::Compilation* comp, TR::Block* block)
{
    ...
    for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStoreDirect() && ((node->getSymbol()->isAuto() && (node->getSymbol()->castToAutoSymbol()->isInternalPointer() || node->getSymbol()->castToAutoSymbol()->isPinningArrayPointer())) || (node->getSymbol()->isParm() && node->getSymbol()->castToParmSymbol()->isPinningArrayPointer())))
            count++;
    }
    return count;
}
...
int32_t TR_LoopStrider::detectCanonicalizedPredictableLoops(TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (!regionStructure || !regionStructure->getParent() || !regionStructure->isNaturalLoop() || regionStructure->getEntryBlock()->isCold())
        return 0;
    ...
    if (loopInvariantBlock) {
        ...
        if (1) {
            ...
            if (prevInvariantNode && prevInvariantNode->getStructure()->asBlock() && prevInvariantNode->getStructure()->asBlock()->isLoopInvariantBlock()) {
        ...
        if (isPredictableLoop > 0) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if (!symRefTab->getSymRef(nextInductionVariableNumber) || !symRefTab->getSymRef(nextInductionVariableNumber)->getSymbol()->isAutoOrParm())
                    continue;
                ...
                if (_loopTestTree->getNode()->getNumChildren() == 2) {
                    ...
                    if ((firstChildOfLoopTestNode->getOpCodeValue() == TR::l2i) || (firstChildOfLoopTestNode->getOpCodeValue() == TR::i2l))
                    ...
                    if ((secondChildOfLoopTestNode->getOpCodeValue() == TR::l2i) || (secondChildOfLoopTestNode->getOpCodeValue() == TR::i2l))
                            ...
                            if (_storeTreesList) {
                                ...
                                if (lookup != _storeTreesList->end()) {
                                    ...
                                    for (storeTree = si.getCurrent(); storeTree != NULL; storeTree = si.getNext()) {
                                        ...
                                        if (firstChildOfLoopTestNode == storeNode->getFirstChild() && storeTree->_loadUsedInLoopIncrement) {
                                            indexSymRef = storeNode->getSymbolReference();
                                            break;
                                        }
                                    }
                    ...
                    if (!secondChildOfLoopTestNode->getOpCode().isLoadConst() && !(secondChildOfLoopTestNode->getOpCode().isLoadVar() && secondChildOfLoopTestNode->getSymbolReference()->getSymbol()->isAutoOrParm() && _neverWritten->get(secondChildOfLoopTestNode->getSymbolReference()->getReferenceNumber()))) {
                        indexSymRef = NULL;
                    }
                ...
                //
                if (storeInRequiredForm && (!_registersScarce || (!exitValueNeeded && (!_cannotBeEliminated->get(nextInductionVariableNumber) || (isLoopTestInductionVariable && loopTestCanBeChangedIfRequired)))) && performTransformation(comp(), "%s Replacing uses of induction var %d in loop %d\n", OPT_DETAILS, nextInductionVariableNumber, loopStructure->getNumber())) {
                    ...
                    if (replaceAllInductionVariableComputations(loopInvariantBlock->getBlock(), loopStructure, &newSymbolReference, inductionVarSymRef)) {
                        ...
                        for (; k < _nextExpression; k++) {
                            ...
                            if (mulConst < 0) {
                                if ((-1 * mulConst > highestFactor) || ((-1 * mulConst == highestFactor) && (_linearEquations[k][4] > -1))) {
                                    bestCandidate = k;
                                    highestFactor = -1 * mulConst;
                                    storeOfDerivedInductionVariable = storeNode;
                                }
                            } else {
                                if ((mulConst > highestFactor) || ((mulConst == highestFactor) && (_linearEquations[k][4] > -1))) {
                                    bestCandidate = k;
                        ...
                        if (isMulTermConst(bestCandidate) && isLoopTestInductionVariable && highestFactor <= maxFactorAllowed &&
                            /*
                       ...
                       *                                                         */
                            !(0 && //no-strict IV
                                ...
                                loopTestNode->getOpCode().isUnsignedCompare() && (!isAdditiveTermConst(bestCandidate) || //non-const additive term
                                                                                     !strideForPIV || //no stride iv
                                                                                     // we have stride, but range not strictly non-neg or non-pos.
                                                                                     !(strideForPIV->getEntry()->asLongConstraint() ? (strideForPIV->getEntry()->getLowLong() >= 0 || strideForPIV->getExit()->getHighLong() <= 0) : (strideForPIV->getEntry()->getLowInt() >= 0 || // int constraint versions
        ...
        for (j = 0; j < symRefCount; j++) {
            ...
            TR::SymbolReference* reassociatedAuto = reassociatedAutoSearchResult != _reassociatedAutos->end() ? reassociatedAutoSearchResult->second : NULL;
            if (reassociatedAuto) {
                ...
                if (!origAuto->getSymbol()->isInternalPointer()) {
                    arrayRefNode->setPinningArrayPointer(reassociatedAuto->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer());
                    reassociatedAuto->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer()->setPinningArrayPointer();
                } else
...
{
    if ((_registersScarce && cg()->getSupportsScaledIndexAddressing()) || (!cg()->getSupportsScaledIndexAddressing() && cg()->getSupportsConstantOffsetInAddressing()))
        return false;
...
void TR_LoopStrider::changeLoopCondition(TR_BlockStructure* loopInvariantBlock, bool usingAladd, int32_t bestCandidate, TR::Node* storeOfDerivedInductionVariable)
{
    ...
    TR::SymbolReference* newSymbolReference = NULL;
    if ((_linearEquations[bestCandidate][4] > -1) && (comp()->getSymRefTab()->getNumInternalPointers() < maxInternalPointersAtPointTooLateToBackOff())) {
...
bool TR_LoopStrider::unchangedValueNeededIn(TR::Block* exitBlock, int32_t nextInductionVariableNumber, bool& seenStore)
{
    ...
    while (cursorTree != exitTree) {
        ...
        if (cursorNode->getOpCode().isStore() && cursorNode->getSymbolReference()->getReferenceNumber() == nextInductionVariableNumber) {
            seenStore = true;
            return false;
        }
...
{
    if ((replacingNode->getOpCodeValue() == TR::iload || replacingNode->getOpCodeValue() == TR::lload) && (differenceInAdditiveConstants == 0)) {
        ...
        if (downcastNode && node->getOpCodeValue() == TR::lload) {
            ...
            if (parent && parent->getType().isInt64() &&
                ...
                (!parent->getOpCode().isShift() || node != parent->getSecondChild()) && !parent->getOpCode().isCall() && !parent->getOpCode().isConversion() && parent->getOpCodeValue() != TR::lternary) {
        ...
        if (replacingNode->getOpCodeValue() != TR::iload && replacingNode->getOpCodeValue() != TR::lload) {
            ...
            if (usingAladd) {
                ...
                if (/* debug("enableRednIndVarElim") && */
                    indexNode->getOpCode().isLoadVarDirect() && indexNode->getSymbol()->isAutoOrParm() && _neverWritten->get(indexNode->getSymbolReference()->getReferenceNumber())) {
...
bool TR_LoopStrider::examineTreeForInductionVariableUse(TR::Block* loopInvariantBlock, TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount, TR::SymbolReference** newSymbolReference)
{
    ...
    if (cg()->supportsInternalPointers() && (node->isInternalPointer()) && node->getFirstChild()->getOpCode().isLoadVar() && node->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm() &&
        //node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto() &&
        _neverWritten->get(node->getFirstChild()->getSymbolReference()->getReferenceNumber()) && (comp()->getStartBlock()->getFrequency() < 500)) {
    ...
    if ((node->getOpCodeValue() == TR::iadd || node->getOpCodeValue() == TR::isub) || (node->getOpCodeValue() == TR::ladd || node->getOpCodeValue() == TR::lsub)) {
        if (isExprLoopInvariant(node->getSecondChild())) {
            ...
            if ((node->getFirstChild()->getOpCodeValue() == TR::imul || node->getFirstChild()->getOpCodeValue() == TR::ishl) || (node->getFirstChild()->getOpCodeValue() == TR::lmul || node->getFirstChild()->getOpCodeValue() == TR::lshl)) {
                bool shift = node->getFirstChild()->getOpCodeValue() == TR::ishl || node->getFirstChild()->getOpCodeValue() == TR::lshl;
                ...
                if (!mulTerm || !isExprLoopInvariant(mulTerm) || !mulTerm->getType().isIntegral() || // we were checking for signed here -- i made the test a bit weaker, but I don't think we can reach here with unsigned
                ...
                if (replacingNode) {
                    *newSymbolReference = NULL;
                    ...
                    if (*newSymbolReference == NULL) {
                        if (isInternalPointer && ((comp()->getSymRefTab()->getNumInternalPointers() >= maxInternalPointers()) || (comp()->cg()->canBeAffectedByStoreTagStalls() && _numInternalPointerOrPinningArrayTempsInitialized >= MAX_INTERNAL_POINTER_AUTOS_INITIALIZED)))
                            return seenInductionVariableComputation;
    ...
    } else if ((node->getOpCodeValue() == TR::imul || node->getOpCodeValue() == TR::ishl) || (node->getOpCodeValue() == TR::lmul || node->getOpCodeValue() == TR::lshl)) {
        ...
        if (!mulTerm || !isExprLoopInvariant(mulTerm) || !mulTerm->getType().isIntegral() || // I changed isSignedInt->isIntegral -- but I don't think unsigned types ever reach here
            (!mulTerm->getOpCode().isLoadConst() && shift))
            replacingNode = NULL;
    ...
    TR::SymbolReferenceTable* symRefTab)
{
    ...
    if (_linearEquations[k][4] > -1) {
        ...
        newAload->setLocalIndex(~0);
        addNode = (usingAladd) ? TR::Node::create(TR::aladd, 2, newAload, addNode) : TR::Node::create(TR::aiadd, 2, newAload, addNode);
        addNode->setIsInternalPointer(true);
...
TR::Node* TR_LoopStrider::placeNewInductionVariableIncrementTree(TR_BlockStructure* loopInvariantBlock, TR::SymbolReference* inductionVarSymRef, TR::SymbolReference* newSymbolReference, int32_t k, TR::SymbolReferenceTable* symRefTab, TR::Node* placeHolderNode, TR::Node* newLoad, TR::TreeTop* insertionTreeTop, TR::Node* constNode, bool isAddition)
{
    ...
    if (isAddition) {
        if (_linearEquations[k][4] > -1) {
            ...
            loopIncrementer = (usingAladd) ? TR::Node::create(TR::aladd, 2, newIvLoad, newMulNode) : TR::Node::create(TR::aiadd, 2, newIvLoad, newMulNode);
            ...
            loopIncrementer = (usingAladd || (newIvLoad->getType().isInt64())) ? TR::Node::create(TR::ladd, 2, newIvLoad, newMulNode) : TR::Node::create(TR::iadd, 2, newIvLoad, newMulNode);
        }
    } else {
        if (_linearEquations[k][4] > -1) {
            if (constNode->getOpCode().isLoadConst()) {
            ...
            loopIncrementer = (usingAladd || (newIvLoad->getType().isInt64())) ? TR::Node::create(TR::lsub, 2, newIvLoad, newMulNode) : TR::Node::create(TR::isub, 2, newIvLoad, newMulNode);
        }
    }
...
int32_t TR_LoopStrider::findNewInductionVariable(TR::Node* node, TR::SymbolReference** symRef, bool hasAdditiveTerm, int32_t internalPointerParentSymbol)
{
    ...
    if (hasAdditiveTerm) {
        ...
        for (; i < _nextExpression; i++) {
            if (!(isAdditiveTermConst(i) && node->getSecondChild()->getOpCode().isLoadConst()) && !isAdditiveTermEquivalentTo(i, node->getSecondChild()))
                continue;
    ...
    if (node->getOpCode().isLoadVar()) {
        ...
        if (isExprLoopInvariant(node->getSecondChild())) {
            if (((node->getFirstChild()->getOpCodeValue() == TR::imul)
                ...
                bool isShift = node->getFirstChild()->getOpCodeValue() == TR::ishl || node->getFirstChild()->getOpCodeValue() == TR::lshl;
...
TR::Node* TR_LoopStrider::isExpressionLinearInInductionVariable(TR::Node* node, int32_t k)
{
    ...
    if ((node->getOpCodeValue() == TR::iload) || (node->getOpCodeValue() == TR::lload)) {
        ...
        if ((node->getFirstChild()->getOpCodeValue() == TR::iload) || (node->getFirstChild()->getOpCodeValue() == TR::lload)) {
            if ((node->getFirstChild()->getSymbolReference()->getReferenceNumber() == _loopDrivingInductionVar) && (node->getSecondChild()->getOpCode().isLoadConst() || (node->getSecondChild()->getOpCode().isLoadVarDirect() && node->getSecondChild()->getSymbol()->isAutoOrParm() && _neverWritten->get(node->getSecondChild()->getSymbolReference()->getReferenceNumber())))) {
...
bool TR_LoopStrider::isExpressionLinearInSomeInductionVariable(TR::Node* node)
{
    ...
    if ((node->getOpCodeValue() == TR::iload) || (node->getOpCodeValue() == TR::lload)) {
        ...
        if ((node->getFirstChild()->getOpCodeValue() == TR::iload) || (node->getFirstChild()->getOpCodeValue() == TR::lload)) {
            if ((_storeTreesList || _writtenExactlyOnce.ValueAt(node->getFirstChild()->getSymbolReference()->getReferenceNumber())) && (node->getSecondChild()->getOpCode().isLoadConst() || (node->getSecondChild()->getOpCode().isLoadVarDirect() && node->getSecondChild()->getSymbol()->isAutoOrParm() && _neverWritten->get(node->getSecondChild()->getSymbolReference()->getReferenceNumber())))) {
                return true;
            }
        }
    } else if ((node->getOpCodeValue() == TR::isub) || (node->getOpCodeValue() == TR::lsub)) {
...
bool TR_LoopStrider::reassociateAndHoistComputations(TR::Block* loopInvariantBlock, TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount)
{
    ...
    TR::Node* originalNode = NULL;
    if (cg()->supportsInternalPointers() && (reassociateAndHoistNonPacked() && node->isInternalPointer()) && node->getFirstChild()->getOpCode().isLoadVar() && node->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm() &&
        //node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto() &&
    ...
    if (isInternalPointer && examineChildren) {
        ...
        if ((isConst || (node->getOpCode().hasSymbolReference() && node->getOpCode().isLoadVar() &&
                            //node->getSymbolReference()->getSymbol()->isAuto() &&
                            node->getSymbolReference()->getSymbol()->isAutoOrParm() && _neverWritten->get(node->getSymbolReference()->getReferenceNumber())))
            && (!_registersScarce || (originalNode->getReferenceCount() > 1)) && (comp()->getSymRefTab()->getNumInternalPointers() < maxInternalPointers()) && (!comp()->cg()->canBeAffectedByStoreTagStalls() || _numInternalPointerOrPinningArrayTempsInitialized < MAX_INTERNAL_POINTER_AUTOS_INITIALIZED) && performTransformation(comp(), "%s Replacing invariant internal pointer %p based on symRef #%d\n", OPT_DETAILS, node, internalPointerSymbol)) {
    ...
    if (((node->getOpCodeValue() == TR::iadd) || (node->getOpCodeValue() == TR::isub)) || ((node->getOpCodeValue() == TR::ladd) || (node->getOpCodeValue() == TR::lsub))) {
        if (node->getSecondChild()->getOpCodeValue() == TR::iconst || node->getSecondChild()->getOpCodeValue() == TR::lconst) {
            bool isAdd = false;
            if (node->getOpCodeValue() == TR::iadd || node->getOpCodeValue() == TR::ladd)
                isAdd = true;
            ...
            int32_t hdrSize = (int32_t)TR::Compiler->om.contiguousArrayHeaderSizeInBytes();
            if ((isInternalPointer && (comp()->getSymRefTab()->getNumInternalPointers() < maxInternalPointers()) && ((isAdd && (constValue == hdrSize)) || (!isAdd && constValue == -hdrSize))) && (!_registersScarce || (node->getReferenceCount() > 1) || _reassociatedNodes.find(node)) && (!comp()->cg()->canBeAffectedByStoreTagStalls() || _numInternalPointerOrPinningArrayTempsInitialized < MAX_INTERNAL_POINTER_AUTOS_INITIALIZED) && performTransformation(comp(), "%s Replacing reassociated internal pointer based on symRef #%d\n", OPT_DETAILS, internalPointerSymbol)) {
...
bool TR_LoopStrider::isStoreInRequiredForm(int32_t symRefNum, TR_Structure* loopStructure)
{
    ...
    if (_storeTreesList) {
        ...
        if (lookup != _storeTreesList->end()) {
            ...
            for (storeTree = si.getCurrent(); storeTree != NULL; storeTree = si.getNext()) {
                TR::Node* storeNode = storeTree->_tt->getNode();
                if ((!storeNode->getType().isInt32() && !storeNode->getType().isInt64()) || (!storeNode->getSymbolReference()->getSymbol()->getType().isInt32() && !storeNode->getSymbolReference()->getSymbol()->getType().isInt64()))
                    return false;
...
TR::Node* TR_LoopStrider::updateLoadUsedInLoopIncrement(TR::Node* node, int32_t inductionVariable)
{
    ...
    if (_indirectInductionVariable && opCode.isLoadVar() && (node->getSymbolReference()->getReferenceNumber() < _numSymRefs) && ((!_storeTreesList && _writtenExactlyOnce.ValueAt(node->getSymbolReference()->getReferenceNumber())) || (_storeTreesList && _storeTreesList->find(node->getSymbolReference()->getReferenceNumber()) != _storeTreesList->end() && (*_storeTreesList)[node->getSymbolReference()->getReferenceNumber()]->isSingleton()))) {
...
bool TR_LoopStrider::isStoreInRequiredForm(TR::Node* storeNode, int32_t symRefNum, TR_Structure* loopStructure)
{
    ...
    if (storeNode->getFirstChild()->getOpCode().isConversion() && storeNode->getFirstChild()->getFirstChild()->getOpCode().isConversion()) {
        TR::Node* conversion1 = storeNode->getFirstChild();
        TR::Node* conversion2 = storeNode->getFirstChild()->getFirstChild();
        if ((conversion1->getOpCodeValue() == TR::s2i && conversion2->getOpCodeValue() == TR::i2s) || (conversion1->getOpCodeValue() == TR::b2i && conversion2->getOpCodeValue() == TR::i2b) || (conversion1->getOpCodeValue() == TR::su2i && conversion2->getOpCodeValue() == TR::i2s))
    ...
    if (_constNode == NULL) {
        ...
        //constNode will be forced to positive in placeNewInductionVariableIncrementTree(...)
        if (secondChild->getOpCode().isLoadConst() && ((secondChild->getType().isInt32() && (secondChild->getInt() < 0)) || (secondChild->getType().isInt64() && (secondChild->getLongInt() < 0)))) {
            if (_isAddition)
                _isAddition = false;
            else
                _isAddition = true;
                    ...
                    if (storeInRequiredForm && performTransformation(comp(), "%s Analyzing candidate: %d in loop: %d for conversion\n", OPT_DETAILS, _loopDrivingInductionVar, loopStructure->getNumber())) {
                        ...
                        if (_isInductionVariableMorphed || stressMode) {
                            if (checkStoreOfIndVar(_storeTrees[nextInductionVariable]->getNode())) {
                                ...
                                TR::SymbolReference* ext = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), TR::Int64);
                                _newTempsCreated = true;
...
bool TR_LoopStrider::morphExpressionLinearInInductionVariable(TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount)
{
    ...
    node->setVisitCount(visitCount);
    if (cg()->supportsInternalPointers() && node->isInternalPointer() && node->getFirstChild()->getOpCode().isLoadVar() && node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto() &&
...
bool TR_LoopStrider::checkExpressionForInductionVariable(TR::Node* node)
{
    ...
    if (node->getOpCodeValue() == TR::iload) {
        if (_writtenExactlyOnce.ValueAt(node->getSymbolReference()->getReferenceNumber()))
            return true;
    } else if (node->getOpCodeValue() == TR::iadd && node->getFirstChild()->getOpCodeValue() == TR::iload && node->getSecondChild()->getOpCodeValue() == TR::iconst) {
        if (_writtenExactlyOnce.ValueAt(node->getFirstChild()->getSymbolReference()->getReferenceNumber()))
            return true;
    } else if (node->getOpCodeValue() == TR::isub && node->getFirstChild()->getOpCodeValue() == TR::iload && node->getSecondChild()->getOpCodeValue() == TR::iconst) {
        if (_writtenExactlyOnce.ValueAt(node->getFirstChild()->getSymbolReference()->getReferenceNumber()))
            return true;
    }
    return false;
...
bool TR_LoopStrider::isAdditiveTermEquivalentTo(int32_t k, TR::Node* node)
{
    if (isAdditiveTermConst(k) && node->getOpCode().isLoadConst()) {
        ...
        if (getAdditiveTermConst(k) == addConst)
            return true;
    } else if (getAdditiveTermNode(k) && getAdditiveTermNode(k)->getOpCode().hasSymbolReference() && node->getOpCode().hasSymbolReference() && getAdditiveTermNode(k)->getSymbolReference() == node->getSymbolReference()) {
...
bool TR_LoopStrider::isMulTermEquivalentTo(int32_t k, TR::Node* node)
{
    if (isMulTermConst(k) && node->getOpCode().isLoadConst()) {
        ...
        if (getMulTermConst(k) == multiplicativeConstant)
            return true;
    } else if (getMulTermNode(k) && getMulTermNode(k)->getOpCode().hasSymbolReference() && node->getOpCode().hasSymbolReference() && getMulTermNode(k)->getSymbolReference() == node->getSymbolReference()) {
...
void TR_InductionVariableAnalysis::gatherCandidates(TR_Structure* s, TR_BitVector* loopLocalDefs, TR_BitVector* allDefs)
{
    ...
    } else if (loopLocalDefs) // ie inside loop
    {
        ...
        for (TR::TreeTop* tt = block->getEntry();
            ...
            if (node->getOpCode().isStoreDirect() && (node->getType().isIntegral() || node->getSymbolReference()->getSymbol()->isInternalPointer())) {
...
bool TR_InductionVariableAnalysis::isProgressionalStore(TR::Node* node, TR_ProgressionKind* kind, int64_t* incr)
{
    ...
    if (!(cursor->getOpCode().isAdd() || cursor->getOpCode().isSub() || cursor->getOpCode().isLeftShift() || cursor->getOpCode().isRightShift())) {
    ...
    TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
    ...
    for (iv = it.getFirst(); iv; iv = it.getNext()) {
        ...
        if (branchContainsInductionVariable(branchNode, iv->getSymRef(), &nodeBudget)) {
            ...
            if (firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub() || firstChild->getOpCode().isLoadDirect()) {
    ...
    TR_RegionStructure* loop, DeltaInfo** loopSet)
{
    ...
    TR_Array<TR_BasicInductionVariable*>* ivs = new (trHeapMemory()) TR_Array<TR_BasicInductionVariable*>(trMemory(), candidates->elementCount(), true, heapAlloc);
    ...
    TR::Node* candidateIV)
{
    ...
    while (!workQueue.empty()) {
        ...
        for (TR::TreeTop* tt = curBlock->getFirstRealTreeTop();
            ...
            if (curNode->getOpCode().isStoreDirect() && curNode->getSymbolReference()->getReferenceNumber() == candidateIV->getSymbolReference()->getReferenceNumber()) {
                if (trace())
...
bool TR_InductionVariableAnalysis::analyzeExitEdges(TR_RegionStructure* loop,
    ...
    TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
    ...
    for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
        ...
        if (!(node->getFirstChild()->getType().isIntegral()
            ...
            && !node->getOpCode().isJumpWithMultipleTargets()) {
            if (trace())
                traceMsg(comp(), "\tReject - Branch node is not integral nor ordered comparison between internal pointers\n");
            return false;
        }
    ...
    if (biv->getEntryValue()) {
        ...
        TR::Node* mulNode = ((entry->getOpCode().isAdd() || entry->getOpCode().isSub()) && entry->getSecondChild()->getOpCode().isLoadConst()) ? mulNode = entry->getFirstChild() : entry;
    ...
    if (biv->getEntryValue() && biv->getSymRef()->getSymbol()->isInternalPointer()) {
        ...
        if (entry->getOpCode().isAdd() && baseLoad && baseLoad->getOpCode().isLoad()) {
            ...
            } else if ((offset->getOpCode().isMul() || offset->getOpCode().isLeftShift()) && offset->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if (newEntryValue && baseLoad && boundNode->getOpCode().isAdd()) {
            ...
            if (loadNode->getOpCode().isLoadVarDirect() && loadNode->getSymbolReference() == baseLoad->getSymbolReference()) {
    ...
    , _iterCount(-1)
{
    ...
    if (getEntryValue() && getEntryValue()->getOpCode().isLoadConst() && (getEntryValue()->getType().isIntegral() || getEntryValue()->getType().isAddress()) && getExitBound()->getOpCode().isLoadConst() && (getExitBound()->getType().isIntegral() || getExitBound()->getType().isAddress())) {
        ...
        int32_t diff = (int32_t)exitVal - (int32_t)entryVal + (getDeltaOnBackEdge() - getDeltaOnExitEdge()); // FIXME: overflow
...
void TR_IVTypeTransformer::changeIVTypeFromAddrToInt(TR_RegionStructure* natLoop)
{
    ...
    do {
        ...
        cm->incVisitCount();
        if (node->getOpCodeValue() == TR::astore && (node->getSymbolReference() == firstChildSymRef || node->getSymbolReference() == secondChildSymRef) && isInAddrIncrementForm(node, increment)) {
            astoreNode = node;
            astoreTT = tt;
            break;
        }
    } while (!astoreNode && (tt->getNode()->getOpCodeValue() != TR::BBStart || (tt->getNode()->getBlock()->getPredecessors().size() == 1)) && (tt = tt->getPrevTreeTop()));
    if (!astoreNode) {
        if (trace())
    ...
    */
    auto cmpChildWithEndAddr = (astoreNode->getSymbolReference() == firstChildSymRef) ? backEdgeIfNode->getChild(1) : backEdgeIfNode->getChild(0);
    auto ptrTestChild = (astoreNode->getSymbolReference() == firstChildSymRef) ? backEdgeIfNode->getChild(0) : backEdgeIfNode->getChild(1);
    ...
    auto intIdxStoreTT = TR::TreeTop::create(cm, TR::Node::createStore(_intIdxSymRef, TR::Compiler->target.is64Bit() ? TR::Node::lconst(0) : TR::Node::iconst(0)));
    ...
    if (!performTransformation(cm, "%s Adding int increment tree\n", optDetailString()))
        return;
    auto intIncrementTT = TR::TreeTop::create(cm, TR::Node::createStore(_intIdxSymRef, TR::Node::create(TR::Compiler->target.is64Bit() ? TR::ladd : TR::iadd, 2, TR::Node::createLoad(_intIdxSymRef), TR::Compiler->target.is64Bit() ? TR::Node::lconst(increment) : TR::Node::iconst(increment))));
    ...
    */
    ...
    auto a2intOp = TR::ILOpCode::getProperConversion(TR::Address, TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32, false);
    TR::Node* newIf;
...
void TR_IVTypeTransformer::replaceAloadWithBaseIndexInSubtree(TR::Node* node)
{
    ...
    if (child && child->getOpCodeValue() == TR::aload && child->getSymbolReference() == _addrSymRef && performTransformation(comp(), "%s Replacing n%in aload with base int-index form\n", optDetailString(), child->getGlobalIndex())) {
...
bool TR_IVTypeTransformer::isInAddrIncrementForm(TR::Node* node, int32_t& increment)
{
    ...
    if (node->getOpCodeValue() == TR::astore && node->getChild(0)->getOpCode().isArrayRef() && (load = node->getChild(0)->getChild(0)) && load->getOpCode().isLoadVar() && load->getOpCode().hasSymbolReference() && load->getSymbolReference() == node->getSymbolReference() && (incChild = node->getChild(0)->getChild(1)) && incChild->getOpCode().isLoadConst()) // TODO: non-const, loop-invariant expr
    {
