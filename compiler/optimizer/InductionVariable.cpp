...
#define GET32BITINT(n) \
  ((n)->getType().isInt32() ? (n)->getInt() : (int32_t)((n)->getLongInt()))
#define GET64BITINT(n) \
  ((n)->getType().isInt32() ? (int64_t)((n)->getInt()) : (n)->getLongInt())

TR_LoopStrider::TR_LoopStrider(TR::OptimizationManager *manager)
    : TR_LoopTransformer(manager),
      _reassociatedNodes(trMemory()),
      _storeTreesSingleton(
          (StoreTreeMapComparator()),
          StoreTreeMapAllocator(comp()->trMemory()->currentStackRegion())) {
  _indirectInductionVariable =
      false;  // TODO: add this c->getMethodHotness() >= scorching;
  _autosAccessed = NULL;
...
int32_t TR_LoopStrider::perform() {
  ...
  _hoistedAutos = new (stackMemoryRegion) SymRefPairMap(
      (SymRefPairMapComparator()), SymRefPairMapAllocator(stackMemoryRegion));
  _reassociatedAutos = new (stackMemoryRegion)
      SymRefMap((SymRefMapComparator()), SymRefMapAllocator(stackMemoryRegion));
  _count = 0;
  ...
  if (usingAladd && !disableSignExtn) {
    ...
    detectLoopsForIndVarConversion(comp()->getFlowGraph()->getStructure(),
                                   exLoads);
  ...
  detectCanonicalizedPredictableLoops(comp()->getFlowGraph()->getStructure(),
                                      NULL, -1);
  ...
  if (_newNonAddressTempsCreated) requestOpt(OMR::loopVersionerGroup);

  return 2;
}

bool TR_LoopStrider::foundLoad(TR::TreeTop *storeTreeTop, TR::Node *node,
                               int32_t symRefNum, vcount_t visitCount) {
  ...
  if (node->getOpCode().isLoadVar() &&
    ...
    if (_storeTreesList) {
      ...
      if (lookup != _storeTreesList->end()) {
        ...
        for (storeTree = si.getCurrent(); storeTree != NULL;
             storeTree = si.getNext()) {
          if (storeTree->_tt == storeTreeTop) {
            if (node != storeTree->_loadUsedInLoopIncrement) return true;
          }
        }
      }
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    if (foundLoad(storeTreeTop, node->getChild(childNum), symRefNum,
                  visitCount))
      return true;
  }
  return false;
...
bool TR_LoopStrider::foundLoad(TR::TreeTop *storeTree,
                               int32_t nextInductionVariableNumber,
                               TR::Compilation *comp) {
  ...
  while (cursorTree != storeTree) {
    if (foundLoad(storeTree, cursorTree->getNode(), nextInductionVariableNumber,
                  visitCount)) {
...
static int32_t countInternalPointerOrPinningArrayStores(TR::Compilation *comp,
                                                        TR::Block *block) {
  int32_t count = 0;
...
int32_t TR_LoopStrider::detectCanonicalizedPredictableLoops(
  ...
  if (regionStructure) {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext())
      detectCanonicalizedPredictableLoops(node->getStructure(), optSetInfo,
                                          bitVectorSize);
  }

  TR_BlockStructure *loopInvariantBlock = NULL;

  if (!regionStructure || !regionStructure->getParent() ||
  ...
  for (nextBlock = blocksIt.getCurrent(); nextBlock;
    ...
    if (nextBlock->hasExceptionPredecessors()) return 0;
  ...
  if (loopInvariantBlock) {
    ...
    if (_storeTreesList == NULL) _storeTreesList = &_storeTreesSingleton;
    ...
    //**)trMemory()->allocateStackMemory(symRefCount*sizeof(List<TR::Node> *));
    // memset(_storeTreesList, 0, symRefCount*sizeof(List<TR::TreeTop> *));
    // int32_t i = 0;
    // while (i < symRefCount)
    //   {
    //   _storeTreesList[i] = new (trStackMemory())
    //   TR_ScratchList<TR_StoreTreeInfo>(trMemory());
    //   //_loadUsedInNewLoopIncrementList[i] = new (trStackMemory())
    //   TR_ScratchList<TR::Node>(trMemory()); i++;
    //   }

    _reassociatedAutos->clear();
    _hoistedAutos->clear();
    _reassociatedNodes.deleteAll();
    // compute the number of internal pointer temps or pinning array temps
    // already initialized in the loop pre-header. on some platforms (z in
    // particular), a long sequence of stores results in "VS Tag Not ready"
    // stalls (aka store buffer stalls). This variable controls whether any
    // induction variables are replaced by internal pointers by the analysis
    //
    _numInternalPointerOrPinningArrayTempsInitialized = 0;
    if (1) {
      _numInternalPointerOrPinningArrayTempsInitialized +=
          countInternalPointerOrPinningArrayStores(
              comp(), loopInvariantBlock->getBlock());
      // check if there are multiple pre-headers (introduced by previous passes
      // of loopCanonicalization)
      //
      TR_StructureSubGraphNode *loopInvariantNode = toStructureSubGraphNode(
          subNode->getPredecessors().front()->getFrom());
      TR_StructureSubGraphNode *prevInvariantNode = NULL;

      if (loopInvariantNode->getPredecessors().size() == 1)
        prevInvariantNode = toStructureSubGraphNode(
            loopInvariantNode->getPredecessors().front()->getFrom());
      if (prevInvariantNode && prevInvariantNode->getStructure()->asBlock() &&
          prevInvariantNode->getStructure()
              ->asBlock()
    ...
    int32_t isPredictableLoop = checkLoopForPredictability(
        loopStructure, loopInvariantBlock->getBlock(), NULL, false);
    ...
    if (isPredictableLoop > 0) {
      ...
      _linearEquations = (int64_t **)trMemory()->allocateStackMemory(
          _numberOfLinearExprs * sizeof(int64_t *));
      memset(_linearEquations, 0, _numberOfLinearExprs * sizeof(int64_t *));
      _loadUsedInNewLoopIncrement =
          (TR::Node **)trMemory()->allocateStackMemory(_numberOfLinearExprs *
                                                       sizeof(TR::Node *));
      memset(_loadUsedInNewLoopIncrement, 0,
             _numberOfLinearExprs * sizeof(TR::Node *));

      int32_t j;
      for (j = 0; j < _numberOfLinearExprs; j++)
        _linearEquations[j] =
            (int64_t *)trMemory()->allocateStackMemory(5 * sizeof(int64_t));
      _nextExpression = 0;
      ...
      if (_autosAccessed) {
        if (_autosAccessed->elementCount() <
            (comp()->cg()->getMaximumNumbersOfAssignableGPRs() - 5))
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!symRefTab->getSymRef(nextInductionVariableNumber) ||
            !symRefTab->getSymRef(nextInductionVariableNumber)
                 ->getSymbol()
                 ->isAutoOrParm())
          continue;
        ...
        bool storeInRequiredForm =
            isStoreInRequiredForm(nextInductionVariableNumber, loopStructure);

        //            traceMsg(comp(), "storeInRequiredForm=%d for ind var=%d
        //            symRef=#%d\n", storeInRequiredForm,
        //            nextInductionVariableNumber,
        //                  symRefTab->getSymRef(nextInductionVariableNumber)->getReferenceNumber());

        //
        // TODO: If there is a use of an induction variable that refers back to
        // a load
        //       that occurs in a tree PRIOR to the increment/decrement,
        //       then we will not attempt to transform into derived induction
        //       variable as we might produce incorrect trees (wanted old value
        //       but will get new inced/deced value) .
        //
        ...
        if (storeInRequiredForm) {
          ...
          if (storeTree)
            storeInRequiredForm =
                foundLoad(storeTree, nextInductionVariableNumber, comp());
          ...
          if (storeInRequiredForm && _storeTreesList) {
            ...
            if (lookup != _storeTreesList->end()) {
              ...
              for (storeTree = si.getCurrent(); storeTree != NULL;
                ...
                storeInRequiredForm = foundLoad(
                    storeTree->_tt, nextInductionVariableNumber, comp());
                if (!storeInRequiredForm) {
        ...
        if (_loopTestTree->getNode()->getNumChildren() == 2) {
          ...
          TR::Node *firstChildOfLoopTestNode =
          ...
          if ((firstChildOfLoopTestNode->getOpCodeValue() == TR::l2i) ||
            ...
            firstChildOfLoopTestNode =
                firstChildOfLoopTestNode->getFirstChild();
          TR::Node *secondChildOfLoopTestNode =
          ...
          if ((secondChildOfLoopTestNode->getOpCodeValue() == TR::l2i) ||
            ...
            secondChildOfLoopTestNode =
                secondChildOfLoopTestNode->getFirstChild();
          if (firstChildOfLoopTestNode->getOpCode().hasSymbolReference()) {
            ...
            TR_ASSERT(storeNode->getOpCode().isStore(),
                      "Expected to find a store\n");

            if (firstChildOfLoopTestNode == storeNode->getFirstChild() &&
                _loadUsedInLoopIncrement)
              indexSymRef = storeNode->getSymbolReference();
            else {
              if (_storeTreesList) {
                auto lookup =
                    _storeTreesList->find(nextInductionVariableNumber);
                if (lookup != _storeTreesList->end()) {
                  List<TR_StoreTreeInfo> *storeTreesList = lookup->second;
                  ListIterator<TR_StoreTreeInfo> si(storeTreesList);
                  TR_StoreTreeInfo *storeTree;
          ...
          if (!secondChildOfLoopTestNode->getOpCode().isLoadConst() &&
              !(secondChildOfLoopTestNode->getOpCode().isLoadVar() &&
                secondChildOfLoopTestNode->getSymbolReference()
                    ->getSymbol()
                    ->isAutoOrParm() &&
                _neverWritten->get(
                    secondChildOfLoopTestNode->getSymbolReference()
          ...
          if (indexSymRef &&
            ...
            if (loopTestHasSymRef != 0) loopTestHasSymRef = 1;
            loopTestCanBeChangedIfRequired = false;
            TR_InductionVariable *v =
                loopStructure->asRegion()->findMatchingIV(indexSymRef);
            ...
            if (v && v->getEntry() && v->getExit()) {
              ...
              if (entryVal->asIntConstraint()) {
                if (entryVal->getHighInt() <= TR::getMaxSigned<TR::Int32>() / 8)
                  maxFactorAllowed = 8;
                else if (entryVal->getHighInt() <=
                         TR::getMaxSigned<TR::Int32>() / 4)
                  maxFactorAllowed = 4;
                else if (entryVal->getHighInt() <=
                         TR::getMaxSigned<TR::Int32>() / 2)
                  maxFactorAllowed = 2;

                if (entryVal->getLowInt() >=
                    TR::getMinSigned<TR::Int32>() / 8) {
                  if (maxFactorAllowed >= 8) maxFactorAllowed = 8;
                } else if (entryVal->getLowInt() >=
                           TR::getMinSigned<TR::Int32>() / 4) {
                  if (maxFactorAllowed >= 4) maxFactorAllowed = 4;
                } else if (entryVal->getLowInt() >=
                           TR::getMinSigned<TR::Int32>() / 2) {
                  if (maxFactorAllowed >= 2) maxFactorAllowed = 2;
                }
              }

              if (exitVal->asIntConstraint()) {
                if (maxFactorAllowed > 1) {
                  if (exitVal->getHighInt() <=
                      TR::getMaxSigned<TR::Int32>() / 8) {
                    if (maxFactorAllowed >= 8) maxFactorAllowed = 8;
                  } else if (exitVal->getHighInt() <=
                             TR::getMaxSigned<TR::Int32>() / 4) {
                    if (maxFactorAllowed >= 4) maxFactorAllowed = 4;
                  } else if (exitVal->getHighInt() <=
                             TR::getMaxSigned<TR::Int32>() / 2) {
                  ...
                  if (exitVal->getLowInt() >=
                      TR::getMinSigned<TR::Int32>() / 8) {
                    if (maxFactorAllowed >= 8) maxFactorAllowed = 8;
                  } else if (exitVal->getLowInt() >=
                             TR::getMinSigned<TR::Int32>() / 4) {
                    if (maxFactorAllowed >= 4) maxFactorAllowed = 4;
                  } else if (exitVal->getLowInt() >=
                             TR::getMinSigned<TR::Int32>() / 2) {
              ...
              if (entryVal->asLongConstraint()) {
                if (entryVal->getHighLong() <=
                    TR::getMaxSigned<TR::Int64>() / 256)
                  maxFactorAllowed = 256;
                else if (entryVal->getHighLong() <=
                         TR::getMaxSigned<TR::Int64>() / 8)
                  maxFactorAllowed = 8;
                else if (entryVal->getHighLong() <=
                         TR::getMaxSigned<TR::Int64>() / 4)
                  maxFactorAllowed = 4;
                else if (entryVal->getHighLong() <=
                         TR::getMaxSigned<TR::Int64>() / 2)
                  maxFactorAllowed = 2;

                if (entryVal->getLowLong() >=
                    TR::getMinSigned<TR::Int64>() / 256) {
                  if (maxFactorAllowed >= 256) maxFactorAllowed = 256;
                } else if (entryVal->getLowLong() >=
                           TR::getMinSigned<TR::Int64>() / 8) {
                  if (maxFactorAllowed >= 8) maxFactorAllowed = 8;
                } else if (entryVal->getLowLong() >=
                           TR::getMinSigned<TR::Int64>() / 4) {
                  if (maxFactorAllowed >= 4) maxFactorAllowed = 4;
                } else if (entryVal->getLowLong() >=
                           TR::getMinSigned<TR::Int64>() / 2) {
                  if (maxFactorAllowed >= 2) maxFactorAllowed = 2;
                }
              }
              ...
              if (exitVal->asLongConstraint()) {
                if (maxFactorAllowed > 1) {
                  if (exitVal->getHighLong() <=
                      TR::getMaxSigned<TR::Int64>() / 256) {
                    if (maxFactorAllowed >= 256) maxFactorAllowed = 256;
                  } else if (exitVal->getHighLong() <=
                             TR::getMaxSigned<TR::Int64>() / 8) {
                    if (maxFactorAllowed >= 8) maxFactorAllowed = 8;
                  } else if (exitVal->getHighLong() <=
                             TR::getMaxSigned<TR::Int64>() / 4) {
                  ...
                  if (exitVal->getLowLong() >=
                      TR::getMinSigned<TR::Int64>() / 256) {
                    if (maxFactorAllowed >= 256) maxFactorAllowed = 256;
                  } else if (exitVal->getLowLong() >=
                             TR::getMinSigned<TR::Int64>() / 8) {
                    if (maxFactorAllowed >= 8) maxFactorAllowed = 8;
                  } else if (exitVal->getLowLong() >=
                             TR::getMinSigned<TR::Int64>() / 4) {
        ...
        // if (_cannotBeEliminated->get(nextInductionVariableNumber))
        {
          ...
          ListIterator<TR::CFGEdge> ei(
              &loopStructure->asRegion()->getExitEdges());
          for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL;
               edge = ei.getNext()) {
            TR_StructureSubGraphNode *target =
                edge->getTo()->asStructureSubGraphNode();
            TR::CFGNode *cfgNode = comp()->getFlowGraph()->getFirstNode();
            while (cfgNode) {
              //                     dumpOptDetails(comp(), "Target %d cfgNode
              //                     %d\n", target->getNumber(),
              //                     cfgNode->getNumber());
          ...
          vcount_t visitCount = comp()->incVisitCount();
          for (TR::Block *exitBlock = bi.getCurrent(); exitBlock != NULL;
               exitBlock = bi.getNext()) {
            bool seenStore = false;
            exitValueNeeded = unchangedValueNeededIn(
                exitBlock, nextInductionVariableNumber, seenStore);
            // dumpOptDetails(comp(), "Exit block is %d and exitValueNeeded
            ...
            if (!exitValueNeeded && !seenStore) {
              ...
              for (auto edge = exitBlock->getSuccessors().begin();
                ...
                exitValueNeeded = unchangedValueNeededIn(
                    next, nextInductionVariableNumber, seenStore);
                // dumpOptDetails(comp(), "Exit succ block is %d and
                // exitValueNeeded %d\n", next->getNumber(), exitValueNeeded);
                if (exitValueNeeded) break;
              }
            }
        ...
        if (storeInRequiredForm &&
             ...
             (!exitValueNeeded &&
              (!_cannotBeEliminated->get(nextInductionVariableNumber) ||
               (isLoopTestInductionVariable &&
                loopTestCanBeChangedIfRequired)))) &&
            performTransformation(
                comp(), "%s Replacing uses of induction var %d in loop %d\n",
                OPT_DETAILS, nextInductionVariableNumber,
                loopStructure->getNumber())) {
          ...
          if (replaceAllInductionVariableComputations(
                  loopInvariantBlock->getBlock(), loopStructure,
            ...
            for (; k < _nextExpression; k++) {
              newSymbolReference =
                  symRefTab->getSymRef((int32_t)_linearEquations[k][1]);
              TR_RegisterCandidate *newInductionCandidate =
                  comp()->getGlobalRegisterCandidates()->findOrCreate(
                      newSymbolReference);
              //
              // Add the derived induction variable as a global register
              // candidate within the loop
              //
              if (performTransformation(
                      comp(),
                      "%s Adding auto %d (created for striding) as a global "
                      "register "
                      "candidate in loop %d\n",
                      OPT_DETAILS,
                      newInductionCandidate->getSymbolReference()
                          ->getReferenceNumber(),
                      loopStructure->getNumber()))
                newInductionCandidate->addAllBlocksInStructure(
                    loopStructure, comp(), trace() ? "strider auto" : NULL);

              // Add the initialization of the derived induction variable before
              // the loop is entered
              //
              TR::Node *placeHolderNode =
                  placeInitializationTreeInLoopInvariantBlock(
                      loopInvariantBlock, inductionVarSymRef,
                      newSymbolReference, k, symRefTab);

              // Add the increment/decrement of the derived induction variable
              // immediately after the increment/decrement of the original
              // induction variable
              //
              TR::Node *storeNode = placeNewInductionVariableIncrementTree(
                  loopInvariantBlock, inductionVarSymRef, newSymbolReference, k,
                  symRefTab, placeHolderNode, loopTestHasSymRef);
              ...
              // todo: create IV even if non-const additive term? need to check
              // loop-invariance and
              //       known constraints
              if (!isAdditiveTermConst(k)) continue;

              if (newSymbolReference->getSymbol()->getDataType() !=
                  TR::Address) {
                TR_InductionVariable *v =
                    loopStructure->asRegion()->findMatchingIV(
                        inductionVarSymRef);
                if (v) {
                  TR::VPConstraint *entryVal = v->getEntry();
                  TR::VPConstraint *exitVal = v->getExit();
                  ...
                  if (newSymbolReference->getSymbol()->getType().isInt64()) {
                    newEntryVal = genVPLongRange(entryVal, mulConst,
                                                 getAdditiveTermConst(k));
                    newExitVal = genVPLongRange(exitVal, mulConst,
                                                getAdditiveTermConst(k));
                    newIncr = genVPLongRange(incr, mulConst, 0);
                  } else {
                    newEntryVal = genVPIntRange(entryVal, mulConst,
                                                getAdditiveTermConst(k));
                    newExitVal = genVPIntRange(exitVal, mulConst,
                                               getAdditiveTermConst(k));
                    newIncr = genVPIntRange(incr, mulConst, 0);
                  }

                  TR_InductionVariable *iv = new (trHeapMemory())
                      TR_InductionVariable(newSymbolReference->getSymbol()
                                               ->castToRegisterMappedSymbol(),
                                           newEntryVal, newExitVal, newIncr,
                                           _loopTestTree->getNode()
                                                   ->getOpCode()
                                                   .isUnsignedCompare()
                                               ? TR_no
                                               : TR_yes);
                  if (bestCandidate == k) strideForPIV = iv;
            ...
            if (isMulTermConst(bestCandidate) && isLoopTestInductionVariable &&
                ...
                /*
                 * don't change loop exit test from IV to stride if it's unsafe
                 * to do so. IE. stride straddles zero(1), test is unsigned
                 * inequality. To determine (1), we need the range constraints
                 * on stride to test if it only contains non-negatives or
                 * non-positives. Reject if constraints are unknown due to
                 * non-const additive term.
                 *                                                         */
                !(0 &&  // no-strict IV
                  loopTestNode->getOpCode()
                      .isCompareForOrder() &&  // unsigned inequality
                  loopTestNode->getOpCode().isUnsignedCompare() &&
                  (!isAdditiveTermConst(
                       bestCandidate) ||  // non-const additive term
                   ...
                   !(strideForPIV->getEntry()->asLongConstraint()
                         ...
                         : (strideForPIV->getEntry()->getLowInt() >=
                                0 ||  // int constraint versions
                            strideForPIV->getExit()->getHighInt() <= 0)))) &&
                performTransformation(comp(),
                                      "%sReplacing loop test node in %s\n",
            ...
            if (performTransformation(comp(),
                                      "%s Adding auto %d as a global register "
                                      "candidate in loop %d\n",
                                      OPT_DETAILS,
                                      inductionCandidate->getSymbolReference()
                                          ->getReferenceNumber(),
                                      loopStructure->getNumber()))
              inductionCandidate->addAllBlocksInStructure(
                  loopStructure, comp(), trace() ? "auto" : NULL);
          }
        }
      }  // loop over the symbols written once
    }

    if (!reassociateAndHoistNonPacked()) return 0;
    ...
    for (j = 0; j < symRefCount; j++) {
      ...
      TR::SymbolReference *reassociatedAuto =
          reassociatedAutoSearchResult != _reassociatedAutos->end()
      ...
      if (reassociatedAuto) {
        ...
        if (usingAladd) {
          TR::Node *constantNode =
              TR::Node::create(byteCodeInfoNode, TR::lconst);
          // constantNode->setLongInt(16);
          constantNode->setLongInt((int64_t)hdrSize);
          arrayRefNode =
              TR::Node::create(TR::aladd, 2,
                               TR::Node::createWithSymRef(
                                   byteCodeInfoNode, TR::aload, 0, origAuto),
                               constantNode);
        } else
          arrayRefNode = TR::Node::create(
              TR::aiadd, 2,
              TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0,
                                         origAuto),
        ...
        TR::Node *newStore = TR::Node::createWithSymRef(
            TR::astore, 1, 1, arrayRefNode, reassociatedAuto);
        ...
        dumpOptDetails(
            ...
            "inserted reassociation initialization tree : %p for new symRef "
            "#%d\n",
      ...
      SymRefPair *symRefPair = symRefPairSearchResult != _hoistedAutos->end()
                                   ? symRefPairSearchResult->second
                                   : NULL;
      ...
      if (symRefPair) {
        ...
        while (symRefPair) {
          ...
          if (symRefPair->_isConst) {
            ...
            if (usingAladd)
              arrayRefNode = TR::Node::create(
                  ...
                  TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0,
                                             origAuto),
                  TR::Node::createWithSymRef(byteCodeInfoNode, TR::lload, 0,
                                             indexSymRef));
            else
              arrayRefNode = TR::Node::create(
                  TR::aiadd, 2,
                  TR::Node::createWithSymRef(byteCodeInfoNode, TR::aload, 0,
                                             origAuto),
                  TR::Node::createWithSymRef(byteCodeInfoNode, TR::iload, 0,
                                             indexSymRef));
          }
          ...
          TR::Node *newStore = TR::Node::createWithSymRef(
              TR::astore, 1, 1, arrayRefNode, symRefPair->_derivedSymRef);
          newStore->setLocalIndex(~0);
          TR::TreeTop *newStoreTreeTop = TR::TreeTop::create(comp(), newStore);
          placeHolderTree->getPrevTreeTop()->join(newStoreTreeTop);
          newStoreTreeTop->join(placeHolderTree);
          ...
          dumpOptDetails(comp(),
                         "\nO^O INDUCTION VARIABLE ANALYSIS: Induction "
                         "variable analysis inserted invariant initialization "
                         "tree : %p for new symRef #%d\n",
                         newStoreTreeTop->getNode(),
                         symRefPair->_derivedSymRef->getReferenceNumber());

          TR_RegisterCandidate *newInductionCandidate =
              comp()->getGlobalRegisterCandidates()->findOrCreate(
                  symRefPair->_derivedSymRef);
          //
          // Add the reassociated auto as a global register candidate
          // within the loop
          //
          if (performTransformation(
                  comp(),
                  "%s Adding auto %d (created for reassociation) as a global "
                  "register candidate in loop %d\n",
                  OPT_DETAILS,
                  newInductionCandidate->getSymbolReference()
                      ->getReferenceNumber(),
                  loopStructure->getNumber()))
...
TR::VPLongRange *TR_LoopStrider::genVPLongRange(TR::VPConstraint *cons,
                                                int64_t coeff,
                                                int64_t additive) {
  if (cons) {
    int64_t low, high;
    ...
    return new (trHeapMemory())
        TR::VPLongRange(low * coeff + additive, high * coeff + additive);
  }
  return NULL;
}

TR::VPIntRange *TR_LoopStrider::genVPIntRange(TR::VPConstraint *cons,
                                              int64_t coeff, int64_t additive) {
  if (cons && cons->asIntConstraint()) {
    int32_t low = cons->getLowInt();
    int32_t high = cons->getHighInt();
    return new (trHeapMemory()) TR::VPIntRange(
        (int32_t)(low * coeff + additive), (int32_t)(high * coeff + additive));
  }
  return NULL;
}
...
void TR_LoopStrider::findOrCreateStoreInfo(TR::TreeTop *tree, int32_t i) {
  ...
  if (lookup != _storeTreesList->end()) {
    ...
    for (storeTree = si.getCurrent(); storeTree != NULL;
         storeTree = si.getNext()) {
      if (storeTree->_tt == tree) return;
    }

    lookup->second->add(new (trStackMemory()) TR_StoreTreeInfo(
        tree, NULL, NULL, NULL, NULL, false, NULL, false));
  } else {
    TR_ScratchList<TR_StoreTreeInfo> *newList =
        new (trStackMemory()) TR_ScratchList<TR_StoreTreeInfo>(trMemory());
    newList->add(new (trStackMemory()) TR_StoreTreeInfo(
        tree, NULL, NULL, NULL, NULL, false, NULL, false));
...
void TR_LoopStrider::updateStoreInfo(int32_t i, TR::TreeTop *tree) {
  _storeTrees[i] = tree;
  if (_storeTreesList) findOrCreateStoreInfo(tree, i);
}

TR::Node *TR_LoopStrider::getNewLoopIncrement(TR::Node *oldLoad, int32_t k,
                                              int32_t symRefNum) {
  ...
  if (!newLoad && (oldLoad == _loadUsedInLoopIncrement) &&
...
TR::Node *TR_LoopStrider::getNewLoopIncrement(TR_StoreTreeInfo *info,
                                              int32_t k) {
  if (!info) return _loadUsedInNewLoopIncrement[k];
...
void TR_LoopStrider::changeLoopCondition(
    TR_BlockStructure *loopInvariantBlock, bool usingAladd,
    int32_t bestCandidate, TR::Node *storeOfDerivedInductionVariable) {
  TR::SymbolReferenceTable *symRefTab = comp()->getSymRefTab();
  ...
  if (!usingAladd && (_linearEquations[bestCandidate][4] > -1) &&
      secondChildOfLoopTestNode->getType().isInt64()) {
    // Cannot let this case proceed since we will have a SInt64 node created
    // below and then we would be in a situation where we would have to create a
    // TR::aladd node on 32-bit, which we are not allowed to do (at least
    // currently)
    //
    return;
  }
  ...
  if (usingAladd || (secondChildOfLoopTestNode->getType().isInt64())) {
    TR::Node *constantNode = duplicateMulTermNode(
        bestCandidate, secondChildOfLoopTestNode, TR::Int64);

    if (!secondChildOfLoopTestNode->getType().isInt64()) {
      TR::Node *i2lNode =
          TR::Node::create(secondChildOfLoopTestNode, TR::i2l, 1);
      ...
      mulNode = TR::Node::create(TR::lmul, 2,
                                 secondChildOfLoopTestNode->duplicateTree(),
                                 constantNode);
  } else
    mulNode = TR::Node::create(
        TR::imul, 2, secondChildOfLoopTestNode->duplicateTree(),
        duplicateMulTermNode(bestCandidate, secondChildOfLoopTestNode,
                             TR::Int32));
  ...
  if (getAdditiveTermNode(bestCandidate) != 0) {
    addNode = TR::Node::create(
        mulNode->getType().isInt64() ? TR::ladd : TR::iadd, 2, mulNode,
        duplicateAdditiveTermNode(bestCandidate, secondChildOfLoopTestNode,
                                  mulNode->getDataType()));
  ...
  if ((_linearEquations[bestCandidate][4] > -1) &&
      (comp()->getSymRefTab()->getNumInternalPointers() <
       maxInternalPointersAtPointTooLateToBackOff())) {
    ...
    if (!newAload->getSymbolReference()
             ->getSymbol()
             ->castToAutoSymbol()
             ->isInternalPointer()) {
      addNode->setPinningArrayPointer(
          newAload->getSymbolReference()->getSymbol()->castToAutoSymbol());
    ...
    newSymbolReference = comp()->getSymRefTab()->createTemporary(
        comp()->getMethodSymbol(), TR::Address, true);
    ...
    if (!pinningArrayPointer->isInternalPointer()) {
      ...
      symbol->castToInternalPointerAutoSymbol()->setPinningArrayPointer(
          pinningArrayPointer->castToInternalPointerAutoSymbol()
              ->getPinningArrayPointer());

    newStore = TR::Node::createWithSymRef(TR::astore, 1, 1, addNode,
                                          newSymbolReference);
  } else {
    newSymbolReference = comp()->getSymRefTab()->createTemporary(
        comp()->getMethodSymbol(), addNode->getDataType(), false);
    _newTempsCreated = true;
    _newNonAddressTempsCreated = true;
    newStore = TR::Node::createWithSymRef(
        addNode->getType().isInt64() ? TR::lstore : TR::istore, 1, 1, addNode,
        newSymbolReference);
  }
  ...
  if (firstChildOfLoopTestNode->getOpCode().hasSymbolReference()) {
    ...
    //
    TR::Node *newLoad = getNewLoopIncrement(
        firstChildOfLoopTestNode, bestCandidate,
    ...
    if (!newLoad)
       ...
       if (_storeTreesList)
          ...
          if (lookup != _storeTreesList->end())
             ...
             ListIterator<TR_StoreTreeInfo> si(storeTreesList);
             TR_StoreTreeInfo *storeTree;
             for (storeTree = si.getCurrent(); storeTree != NULL; storeTree =
    si.getNext())
                {
                if (firstChildOfLoopTestNode ==
    storeTree->_loadUsedInLoopIncrement && storeTree->_load)
                    {
                    newLoad = storeTree->_load;
                    break;
                    }
    ...
    */
    ...
    if (newLoad) {
      ...
      firstOperand = TR::Node::createWithSymRef(
          firstChildOfLoopTestNode,
          comp()->il.opCodeForDirectLoad(
              derivedSymbolReference->getSymbol()->getDataType()),
          0, derivedSymbolReference);
    }
  } else
  ...
  TR::Node *secondOperand = TR::Node::createWithSymRef(
      secondChildOfLoopTestNode,
      comp()->il.opCodeForDirectLoad(newStore->getDataType()), 0,
      newSymbolReference);

  if (firstOperand->getDataType() == TR::Address) {
    TR_ComparisonTypes compareType =
        TR::ILOpCode::getCompareType(loopTestNode->getOpCodeValue());
    TR::ILOpCodes op =
        TR::ILOpCode::compareOpCode(TR::Address, compareType, true);
  ...
  if (firstChildIsConversion && !usingAladd) {
    ...
    if (firstChildIsConversion->getReferenceCount() == 1)
      ...
      TR::Node *dupOfFirstConversionChild = TR::Node::create(
          firstChildIsConversion->getOpCodeValue(), 1, firstOperand);
  ...
  if (secondChildIsConversion && !usingAladd) {
    ...
    if (secondChildIsConversion->getReferenceCount() == 1)
      ...
      TR::Node *dupOfSecondConversionChild = TR::Node::create(
          secondChildIsConversion->getOpCodeValue(), 1, secondOperand);
  ...
  if (prevFirst) prevFirst->recursivelyDecReferenceCount();
  if (prevSecond) prevSecond->recursivelyDecReferenceCount();
  // TR::Node::recreate(_storeTrees[_loopDrivingInductionVar]->getNode(),
  // TR::treetop);

  // In a case of negative stride, exit branch needs to be reversed
  if (getMulTermConst(bestCandidate) < 0) {
    TR::Node::recreate(loopTestNode,
                       loopTestNode->getOpCode().getOpCodeForSwapChildren());
  }
}

bool TR_LoopStrider::foundValue(TR::Node *node, int32_t symRefNum,
                                vcount_t visitCount) {
  if (node->getVisitCount() == visitCount) return false;
...
bool TR_LoopStrider::unchangedValueNeededIn(TR::Block *exitBlock,
                                            int32_t nextInductionVariableNumber,
                                            bool &seenStore) {
...
TR::Node *TR_LoopStrider::genLoad(TR::Node *node, TR::SymbolReference *symRef,
                                  bool isInternalPointer) {
  TR::Node *result;
  result = TR::Node::createWithSymRef(
      node,
      isInternalPointer
          ? TR::aload
          : (symRef->getSymbol()->getType().isInt64() ? TR::lload : TR::iload),
      0, symRef);
...
void TR_LoopStrider::examineOpCodesForInductionVariableUse(
    TR::Node *node, TR::Node *parent, int32_t &childNum, int32_t &index,
    TR::Node *originalNode, TR::Node *replacingNode, TR::Node *linearTerm,
    TR::Node *mulTerm, TR::SymbolReference **newSymbolReference,
    TR::Block *loopInvariantBlock, TR::AutomaticSymbol *pinningArrayPointer,
    int64_t differenceInAdditiveConstants, bool &isInternalPointer,
  ...
  if ((replacingNode->getOpCodeValue() == TR::iload ||
    ...
    if (_usesLoadUsedInLoopIncrement) {
      TR::Node *newLoad =
          getNewLoopIncrement(_storeTreeInfoForLoopIncrement, index);
      ...
      if (newLoad) {
        ...
        if ((newLoad->getOpCodeValue() == TR::lload) && downcastNode) {
          // We are replacing oldNode(node) with newLoad. Must recursivelyDec
          // all children of oldNode
    ...
    if (downcastNode && node->getOpCodeValue() == TR::lload) {
      ...
      if (parent && parent->getType().isInt64() &&
          // Exclude these guys, even though they're long typed their children
          // shouldn't be For shifts, only the 2nd child shouldn't be long
          (!parent->getOpCode().isShift() ||
    ...
    if (replacingNode->getOpCodeValue() != TR::iload &&
      ...
      if (usingAladd) {
        ...
        if (!mulTerm->getOpCode().isLoadConst()) canHoistAdditiveTerm = true;
        ...
        mulNode = TR::Node::create(
            node, replacingNode->getType().isInt64() ? TR::lmul : TR::imul, 2);
        constantNode =
            duplicateMulTermNode(index, node, replacingNode->getDataType());
      }
    ...
    if (isInternalPointer) {
      ...
      if (usingAladd) {
        if (replacingNode->getOpCodeValue() == TR::lsub) {
          ...
          adjustmentNode =
              TR::Node::create(TR::lmul, 2, adjustmentNode, negateConstantNode);
        ...
        if (replacingNode->getOpCodeValue() == TR::isub) {
          adjustmentNode =
              TR::Node::create(TR::imul, 2, adjustmentNode,
      ...
      if (adjustmentNode) {
        if (usingAladd || (adjustmentNode->getType().isInt64())) {
          differenceInAdditiveConstantsNode =
              TR::Node::create(node, TR::lconst);
          differenceInAdditiveConstantsNode->setLongInt(
              (int64_t)differenceInAdditiveConstants);
          op = replacingNode->getOpCode().isAdd() ? TR::ladd : TR::lsub;
        } else {
          differenceInAdditiveConstantsNode = TR::Node::create(
              node, TR::iconst, 0, differenceInAdditiveConstants);
          op = replacingNode->getOpCode().isAdd() ? TR::iadd : TR::isub;
        }
        adjustmentNode = TR::Node::create(op, 2, adjustmentNode,
                                          differenceInAdditiveConstantsNode);
      } else {
    ...
    if (canHoistAdditiveTerm) {
      TR::SymbolReference *newLoopInvariant =
      ...
      TR::Node *newStore = TR::Node::createWithSymRef(
          comp()->il.opCodeForDirectStore(adjustmentNode->getDataType()), 1, 1,
      ...
      dumpOptDetails(comp(),
                     "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable "
                     "analysis inserted initialization tree : %p for new loop "
                     "invariant symRef #%d\n",
                     newStoreTreeTop->getNode(),
                     newLoopInvariant->getReferenceNumber());
      int32_t childNum;
      for (childNum = 0; childNum < adjustmentNode->getNumChildren();
           childNum++)
    ...
    if (downcastNode && newLoad->getType().isInt64()) {
      ...
      if (nodeRememberedInLoadUsed) {
        nodeRememberedInLoadUsed = false;
        _loadUsedInNewLoopIncrement[index] =
            node->getFirstChild()->getFirstChild();
        if (_storeTreeInfoForLoopIncrement)
          addLoad(_storeTreeInfoForLoopIncrement,
                  node->getFirstChild()->getFirstChild(), index);
      } else {
...
bool TR_LoopStrider::examineTreeForInductionVariableUse(
    TR::Block *loopInvariantBlock, TR::Node *parent, int32_t childNum,
  ...
  if (node->getVisitCount() == visitCount) examineChildren = false;
  ...
  if (cg()->supportsInternalPointers() && (node->isInternalPointer()) &&
    ...
    if (node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto())
      pinningArrayPointer = node->getFirstChild()
                                ->getSymbolReference()
                                ->getSymbol()
                                ->castToAutoSymbol();
    else {
      ...
      while (pair) {
        if (pair->_indexSymRef == node->getFirstChild()->getSymbolReference()) {
          pinningArrayPointer =
              pair->_derivedSymRef->getSymbol()->castToAutoSymbol();
  ...
  if ((node->getOpCodeValue() == TR::iadd ||
    ...
    if (isExprLoopInvariant(node->getSecondChild())) {
      // For TR::aladd's, if the additive constant might exceed INT_MAX; do
      // nothing
      if (usingAladd)
      ...
      if ((node->getFirstChild()->getOpCodeValue() == TR::imul ||
        ...
        if ((replacingNode = findReplacingNode(
                 node->getFirstChild()->getFirstChild(), usingAladd, -1))) {
          linearTerm = node->getFirstChild()->getFirstChild();
          mulTerm = node->getFirstChild()->getSecondChild();
        } else if (!shift && (replacingNode = findReplacingNode(
                                  node->getFirstChild()->getSecondChild(),
                                  usingAladd, -1))) {
          linearTerm = node->getFirstChild()->getSecondChild();
          mulTerm = node->getFirstChild()->getFirstChild();
        }

        if (!mulTerm || !isExprLoopInvariant(mulTerm) ||
            !mulTerm->getType()
                 .isIntegral() ||  // we were checking for signed here -- i made
                                   // the test a bit weaker, but I don't think
                                   // we can reach here with unsigned
        ...
        if (replacingNode) {
          *newSymbolReference = NULL;
          int32_t index = findNewInductionVariable(node, newSymbolReference,
                                                   true, internalPointerSymbol);
          ...
          if (*newSymbolReference == NULL) {
            if (isInternalPointer &&
              ...
              return seenInductionVariableComputation;
            else if (!isInternalPointer && !node->isNonNegative() &&
                     !node->isNonPositive())
              return seenInductionVariableComputation;
          ...
          if (canCreateNewSymRef) {
            TR::DataType dataType =
                findDataType(replacingNode, usingAladd, isInternalPointer);
            ...
            *newSymbolReference = comp()->getSymRefTab()->createTemporary(
            ...
            if (isInternalPointer && !pinningArrayPointer) {
              ...
              pinningArrayPointer =
                  newPinningArray->getSymbol()->castToAutoSymbol();
              createParmAutoPair(
                  comp()->getSymRefTab()->getSymRef(internalPointerSymbol),
                  newPinningArray);
          ...
          if (node->getSecondChild()->getOpCode().isLoadConst()) {
            TR_ASSERT(isAdditiveTermConst(index),
                      "Both terms should be constants %d", index);
            int64_t additiveConstant = GET64BITINT(node->getSecondChild());
            if (node->getOpCodeValue() == TR::iadd ||
                node->getOpCodeValue() == TR::ladd)
              differenceInAdditiveConstants =
                  additiveConstant - getAdditiveTermConst(index);
            else  // if (node->getSecondChild()->getOpCodeValue() == TR::isub)
              differenceInAdditiveConstants =
                  (-1 * additiveConstant) - getAdditiveTermConst(index);
          }

          examineOpCodesForInductionVariableUse(
              node, parent, childNum, index, originalNode, replacingNode,
              linearTerm, mulTerm, newSymbolReference, loopInvariantBlock,
              pinningArrayPointer, differenceInAdditiveConstants,
    ...
    TR::Node *replacingNode = NULL, *mulTerm = NULL, *linearTerm = NULL;
    if ((replacingNode =
             findReplacingNode(node->getFirstChild(), usingAladd, -1))) {
      linearTerm = node->getFirstChild();
      mulTerm = node->getSecondChild();
    } else if (!shift && (replacingNode = findReplacingNode(
                              node->getSecondChild(), usingAladd, -1))) {
    ...
    if (!mulTerm || !isExprLoopInvariant(mulTerm) ||
        !mulTerm->getType()
             .isIntegral() ||  // I changed isSignedInt->isIntegral -- but I
                               // don't think unsigned types ever reach here
        (!mulTerm->getOpCode().isLoadConst() && shift))
      replacingNode = NULL;
    ...
    if (replacingNode) {
      *newSymbolReference = NULL;
      int32_t index = findNewInductionVariable(node, newSymbolReference, false,
                                               internalPointerSymbol);
      ...
      if (canCreateNewSymRef) {
        ...
        *newSymbolReference = comp()->getSymRefTab()->createTemporary(
        ...
        if (isInternalPointer && !pinningArrayPointer) {
          ...
          TR::Node *newAload = TR::Node::createLoad(
              node, comp()->getSymRefTab()->getSymRef(internalPointerSymbol));
          newAload->setLocalIndex(~0);
          TR::Node *newStore = TR::Node::createWithSymRef(
              TR::astore, 1, 1, newAload, newPinningArray);
          internalPointerSymbol = newPinningArray->getReferenceNumber();
          ...
          dumpOptDetails(
              comp(),
              "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis "
              "inserted initialization tree : %p for new symRef #%d\n",
              newStore, newPinningArray->getReferenceNumber());
          _numInternalPointerOrPinningArrayTempsInitialized++;
        }
      ...
      if (isAdditiveTermConst(index))
        differenceInAdditiveConstants =
            (int32_t)(-1 * getAdditiveTermConst(index));

      examineOpCodesForInductionVariableUse(
          node, parent, childNum, index, originalNode, replacingNode,
          linearTerm, mulTerm, newSymbolReference, loopInvariantBlock,
          pinningArrayPointer, differenceInAdditiveConstants, isInternalPointer,
          downcastNode, usingAladd);
  ...
  if (examineChildren) {
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
      if (examineTreeForInductionVariableUse(loopInvariantBlock, node, i,
                                             node->getChild(i), visitCount,
                                             newSymbolReference))
        seenInductionVariableComputation = true;
    }
  }
...
void TR_LoopStrider::populateLinearEquation(
    TR::Node *node, int32_t loopDrivingInductionVar,
    int32_t derivedInductionVar, int32_t internalPointerSymbol,
    TR::Node *invariantMultiplicationTerm) {
  //   traceMsg(comp(), "populate node %p number %d\n", node, _nextExpression);

  TR_ASSERT(_nextExpression < _numberOfLinearExprs,
            "Number of linear expressions exceeded the estimate\n");
  ...
  if (node->getOpCodeValue() == TR::iadd ||
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
      int64_t additiveConstant = GET64BITINT(node->getSecondChild());
      if (intType)
        addTerm = TR::Node::create(node, TR::iconst, 0,
                                   -1 * (int32_t)additiveConstant);
      else {
  ...
  if (node->getOpCodeValue() == TR::imul ||
    ...
    setMulTermNode(invariantMultiplicationTerm, _nextExpression);
             ...
             node->getOpCodeValue() == TR::lshl) {
    TR_ASSERT(invariantMultiplicationTerm->getOpCodeValue() == TR::iconst,
              "Handling only constant shifts");
    ...
    if (shiftAmount > 0)
      ...
      mulAmount = 1;
    setMulTermNode(TR::Node::create(node, TR::iconst, 0, mulAmount),
                   _nextExpression);
  }
...
void TR_LoopStrider::setInternalPointer(
    TR::Symbol *symbol, TR::AutomaticSymbol *pinningArrayPointer) {
  _numInternalPointers++;
...
void TR_LoopStrider::placeStore(TR::Node *newStore,
...
TR::Node *TR_LoopStrider::findReplacingNode(TR::Node *node, bool usingAladd,
                                            int32_t k) {
  TR::Node *replacingNode = NULL;
  if (usingAladd) {
    if (node->getOpCodeValue() == TR::i2l)
      replacingNode =
          isExpressionLinearInInductionVariable(node->getFirstChild(), k);
    else
...
TR::DataType TR_LoopStrider::findDataType(TR::Node *node, bool usingAladd,
                                          bool isInternalPointer) {
...
void TR_LoopStrider::createParmAutoPair(TR::SymbolReference *parmSymRef,
                                        TR::SymbolReference *autoSymRef) {
  SymRefPair *pair =
      (SymRefPair *)trMemory()->allocateStackMemory(sizeof(SymRefPair));
...
void TR_LoopStrider::addLoad(TR_StoreTreeInfo *info, TR::Node *load,
                             int32_t index) {
  ...
  TR_NodeIndexPair *pair =
      new (trStackMemory()) TR_NodeIndexPair(load, index, NULL);
  pair->_next = head;
  info->_loads = pair;
  info->_load = load;
}

TR::Node *TR_LoopStrider::placeInitializationTreeInLoopInvariantBlock(
    TR_BlockStructure *loopInvariantBlock,
    TR::SymbolReference *inductionVarSymRef,
    TR::SymbolReference *newSymbolReference, int32_t k,
  ...
  TR::Node *mulNode;
  if (usingAladd) {
    TR::Node *multiplicativeConstant =
        duplicateMulTermNode(k, placeHolderNode, TR::Int64);
    ...
    mulNode = TR::Node::create(
        newLoad->getType().isInt64() ? TR::lmul : TR::imul, 2, newLoad,
  ...
  if (getAdditiveTermNode(k) != NULL) {
    if (usingAladd) {
      ...
      addNode = TR::Node::create(
          mulNode->getType().isInt64() ? TR::ladd : TR::iadd, 2, mulNode,
          duplicateAdditiveTermNode(k, placeHolderNode,
                                    mulNode->getDataType()));
    }
    addNode->setLocalIndex(~0);
    addNode->getSecondChild()->setLocalIndex(~0);
  ...
  if (_linearEquations[k][4] > -1) {
    TR::Node *newAload = TR::Node::createLoad(
        placeHolderNode, symRefTab->getSymRef((int32_t)_linearEquations[k][4]));
    newAload->setLocalIndex(~0);
    ...
    TR::ILOpCodes op =
        (usingAladd || addNode->getType().isInt64()) ? TR::lstore : TR::istore;
    newStore =
        TR::Node::createWithSymRef(op, 1, 1, addNode, newSymbolReference);
  }
...
TR::Node *TR_LoopStrider::placeNewInductionVariableIncrementTree(
    ...
    TR::SymbolReference *inductionVarSymRef,
    TR::SymbolReference *newSymbolReference, int32_t k,
    TR::SymbolReferenceTable *symRefTab, TR::Node *placeHolderNode,
    int32_t loopTestHasSymRef) {
  //
  // Create and add the loop increment step for derived induction variable
  //
  // First check if we MUST common the load in prior uses of the derived
  // induction variable with the current use in loop increment step; this may be
  // required if the load used in the increment of the original induction
  // variable i was commoned with prior loads of i
  //
  TR::Node *newLoad = NULL;
  TR::Node *returnValue = NULL;
  int32_t loopTestBlockNum = _loopTestTree->getNextTreeTop()
                                 ->getNode()
                                 ->getBlock()
                                 ->startOfExtendedBlock()
                                 ->getNumber();
  ...
  if (lookup != _storeTreesList->end()) {
    ...
    for (; storeTree != NULL;) {
      ...
      if (loadNode)
        ...
        if (_linearEquations[k][4] > -1)
          ...
          newLoad = TR::Node::createLoad(placeHolderNode, newSymbolReference);
        newLoad->setLocalIndex(~0);
        // remember this load here in case the loop test tree needs to refer
        // back to this load instead of the new incremented/decremented value

        storeTree->getData()->_load = newLoad;

        TR_NodeIndexPair *head = storeTree->getData()->_loads;
        TR_NodeIndexPair *pair =
            new (trStackMemory()) TR_NodeIndexPair(newLoad, k, NULL);
        pair->_next = head;
      ...
      TR::Node *newStore = placeNewInductionVariableIncrementTree(
          loopInvariantBlock, inductionVarSymRef, newSymbolReference, k,
          symRefTab, placeHolderNode, newLoad,
          storeTree->getData()->_insertionTreeTop,
          storeTree->getData()->_constNode, storeTree->getData()->_isAddition);

      int32_t storeBlockNum = storeTree->getData()
                                  ->_insertionTreeTop->getEnclosingBlock()
                                  ->startOfExtendedBlock()
                                  ->getNumber();
    ...
    if (_loadUsedInNewLoopIncrement[k])
      ...
      if (_linearEquations[k][4] > -1)
        ...
        newLoad = TR::Node::createLoad(placeHolderNode, newSymbolReference);
      newLoad->setLocalIndex(~0);
      // remember this load here in case the loop test tree needs to refer back
      // to this load instead of the new incremented/decremented value
      _loadUsedInNewLoopIncrement[k] = newLoad;
    }
    returnValue = placeNewInductionVariableIncrementTree(
        loopInvariantBlock, inductionVarSymRef, newSymbolReference, k,
        symRefTab, placeHolderNode, newLoad, _insertionTreeTop, _constNode,
        _isAddition);
  }
...
TR::Node *TR_LoopStrider::placeNewInductionVariableIncrementTree(
    ...
    TR::SymbolReference *newSymbolReference, int32_t k,
    TR::SymbolReferenceTable *symRefTab, TR::Node *placeHolderNode,
    TR::Node *newLoad, TR::TreeTop *insertionTreeTop, TR::Node *constNode,
    bool isAddition) {
  bool usingAladd = (TR::Compiler->target.is64Bit()) ? true : false;
  ...
  if (usingAladd) {
    ...
    newMulNode =
        TR::Node::create(TR::lmul, 2, newConstNode, multiplicativeConstant);
    newConstNode->setLocalIndex(~0);
    //_constNode->incReferenceCount();
  } else {
    newMulNode = TR::Node::create(
        newLoad->getType().isInt64() ? TR::lmul : TR::imul, 2, constNode,
        duplicateMulTermNode(k, placeHolderNode, newLoad->getDataType()));
  }
  ...
  if (constNode->getOpCode().isLoadConst()) {
    if (constNode->getType().isInt32()) {
      if (constNode->getInt() < 0) constNode->setInt(-1 * constNode->getInt());
    } else {
      if (constNode->getLongInt() < 0)
  ...
  if (isAddition) {
    if (_linearEquations[k][4] > -1) {
      ...
      loopIncrementer =
          (usingAladd || (newIvLoad->getType().isInt64()))
    ...
    if (_linearEquations[k][4] > -1) {
      if (constNode->getOpCode().isLoadConst()) {
        ...
        // dont dec the reference count for _constNode here if we are in the
        // mood of aladds because it wasnt used to create the newMulNode in the
        // first place... see newConstNode above
        if (!usingAladd) constNode->recursivelyDecReferenceCount();
        newMulNode->setAndIncChild(0, changedConstNode);
      } else {
        TR::ILOpCodes op = (usingAladd || newMulNode->getType().isInt64())
                               ? TR::lneg
                               : TR::ineg;
  ...
  //
  if (_linearEquations[k][4] > -1)
    newStore = TR::Node::createWithSymRef(TR::astore, 1, 1, loopIncrementer,
                                          newSymbolReference);
  else {
    TR::ILOpCodes op = (usingAladd || (loopIncrementer->getType().isInt64()))
                           ? TR::lstore
                           : TR::istore;
    newStore = TR::Node::createWithSymRef(op, 1, 1, loopIncrementer,
                                          newSymbolReference);
  }
  ...
  dumpOptDetails(
      comp(),
      "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted "
      "loop incremental step tree : %p for new symRef #%d\n",
      newStoreTreeTop->getNode(), newSymbolReference->getReferenceNumber());
  return newStore;
}

int32_t TR_LoopStrider::findNewInductionVariable(
    TR::Node *node, TR::SymbolReference **symRef, bool hasAdditiveTerm,
  ...
  if (hasAdditiveTerm) {
            ...
            if (node->getOpCodeValue() == TR::iadd ||
              ...
              *symRef = symRefTab->getSymRef((int32_t)_linearEquations[i][1]);
              return i;
                   ...
                   node->getFirstChild()->getOpCodeValue() == TR::lshl) {
          TR_ASSERT(node->getFirstChild()
                        ->getSecondChild()
                        ->getOpCode()
                        .isLoadConst(),
                    "assertion failure");

          int32_t multiplicativeConstant =
              GET32BITINT(node->getFirstChild()->getSecondChild());
          bool sameFactor = false;
          if (isMulTermConst(i) && getMulTermConst(i) == 1) {
            if (multiplicativeConstant == 0) sameFactor = true;
          } else if (isMulTermConst(i) &&
                     getMulTermConst(i) == (2 << (multiplicativeConstant - 1)))
            sameFactor = true;
    ...
    for (; i < _nextExpression; i++) {
      if (!isAdditiveTermConst(i)) continue;
...
void TR_LoopStrider::identifyExpressionsLinearInInductionVariables(
    TR_Structure *structure, vcount_t visitCount) {
  if (structure->asBlock() != NULL) {
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      identifyExpressionsLinearInInductionVariables(node->getStructure(),
                                                    visitCount);
    }
  }
}

bool TR_LoopStrider::identifyExpressionLinearInInductionVariable(
    TR::Node *node, vcount_t visitCount) {
  // for aladds
  bool usingAladd = (TR::Compiler->target.is64Bit()) ? true : false;
  ...
  if (node->getOpCode().isLoadVar()) {
    ...
    if (_writtenExactlyOnce.ValueAt(symRefNum)) {
      if ((_currTree !=
           _storeTrees[symRefNum]) /* && (_loopTestTree != _currTree) */)
      ...
      if (lookup != _storeTreesList->end()) {
        ...
        if (!flag) _cannotBeEliminated->set(symRefNum);
      }
    }
    ...
    if (isExprLoopInvariant(node->getSecondChild())) {
      if (((node->getFirstChild()->getOpCodeValue() == TR::imul) ||
        ...
        if (usingAladd &&
            node->getFirstChild()->getFirstChild()->getOpCodeValue() == TR::i2l)
        ...
        isLinearInInductionVariable =
            isExpressionLinearInSomeInductionVariable(linearNode);
        ...
        {
          if (usingAladd &&
              node->getFirstChild()->getSecondChild()->getOpCodeValue() ==
                  TR::i2l)
            linearNode =
                node->getFirstChild()->getSecondChild()->getFirstChild();
          else
          ...
          // traceMsg(comp(), "identifying node %p is linear %d mulTermNode
          ...
          if (isLinearInInductionVariable && isExprLoopInvariant(mulTermNode)) {
            ...
            needToVisitChild = false;
             ...
             ((node->getOpCodeValue() == TR::lmul) ||
              (node->getOpCodeValue() == TR::lshl))) {
    bool isShift = node->getOpCodeValue() == TR::ishl ||
                   node->getOpCodeValue() == TR::lshl;
    TR::Node *linearNode, *mulTermNode;
    bool isLinearInInductionVariable;

    if (usingAladd && node->getFirstChild()->getOpCodeValue() ==
                          TR::i2l)  // handle both TR::xmul and TR::xshl
      linearNode = node->getFirstChild()->getFirstChild();
    else
    ...
    //      traceMsg(comp(), "identifying node %p is linear %d linearNode %p
    //      mulTermNode %p\n", node, isLinearInInductionVariable, linearNode,
    //      mulTermNode);
    ...
    {
      if (usingAladd && node->getSecondChild()->getOpCodeValue() ==
                            TR::i2l)  // handle both TR::xmul and TR::xshl
  ...
  if (needToVisitChild) {
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++) {
      identifyExpressionLinearInInductionVariable(node->getChild(i),
                                                  visitCount);
    }
  }
...
TR::Node *TR_LoopStrider::setUsesLoadUsedInLoopIncrement(TR::Node *node,
                                                         int32_t k) {
  if (_storeTreesList) {
    ...
    if (lookup != _storeTreesList->end()) {
      ...
      for (storeTree = si.getCurrent(); storeTree != NULL;
        ...
        if (node == storeTree->_loadUsedInLoopIncrement &&
            !storeTree->_incrementInDifferentExtendedBlock) {
    ...
    if (!_loadUsedInLoopIncrement && node->getReferenceCount() > 1) return NULL;
...
TR::Node *TR_LoopStrider::isExpressionLinearInInductionVariable(TR::Node *node,
                                                                int32_t k) {
  // for aladds
  bool usingAladd = (TR::Compiler->target.is64Bit()) ? true : false;
  ...
  if ((node->getOpCodeValue() == TR::iload) ||
    ...
    if (node->getSymbolReference()->getReferenceNumber() ==
      ...
      if (!setUsesLoadUsedInLoopIncrement(node, k)) return NULL;
    ...
    if ((node->getFirstChild()->getOpCodeValue() == TR::iload) ||
      ...
      if ((node->getFirstChild()->getSymbolReference()->getReferenceNumber() ==
           _loopDrivingInductionVar) &&
          (node->getSecondChild()->getOpCode().isLoadConst() ||
           (node->getSecondChild()->getOpCode().isLoadVarDirect() &&
            node->getSecondChild()->getSymbol()->isAutoOrParm() &&
            _neverWritten->get(node->getSecondChild()
                                   ->getSymbolReference()
                                   ->getReferenceNumber())))) {
...
bool TR_LoopStrider::isExpressionLinearInSomeInductionVariable(TR::Node *node) {
  ...
  if ((node->getOpCodeValue() == TR::iload) ||
    ...
    if ((node->getFirstChild()->getOpCodeValue() == TR::iload) ||
        (node->getFirstChild()->getOpCodeValue() == TR::lload)) {
      if ((_storeTreesList ||
           _writtenExactlyOnce.ValueAt(node->getFirstChild()
                                           ->getSymbolReference()
                                           ->getReferenceNumber())) &&
          (node->getSecondChild()->getOpCode().isLoadConst() ||
           (node->getSecondChild()->getOpCode().isLoadVarDirect() &&
...
bool TR_LoopStrider::reassociateAndHoistComputations(
    TR::Block *loopInvariantBlock, TR_Structure *structure) {
  ...
  if (structure->asBlock() != NULL) {
    ...
    while (currentTree != exitTree) {
      ...
      if (reassociateAndHoistComputations(loopInvariantBlock, NULL, -1,
                                          currentNode, comp()->getVisitCount()))
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
      if (reassociateAndHoistComputations(loopInvariantBlock,
                                          node->getStructure()))
        reassociatedComputation = true;
    }
  }
...
bool TR_LoopStrider::reassociateAndHoistComputations(
  ...
  if (isInternalPointer && examineChildren) {
    bool isConst = false;
    if (node->getOpCode().isLoadConst()) isConst = true;

    if ((isConst || (node->getOpCode().hasSymbolReference() &&
        ...
        performTransformation(
            comp(),
            "%s Replacing invariant internal pointer %p based on symRef #%d\n",
            OPT_DETAILS, node, internalPointerSymbol)) {
      TR::SymbolReference *internalPointerSymRef = NULL;
      auto symRefPairSearchResult =
          _hoistedAutos->find(originalInternalPointerSymbol);
      SymRefPair *symRefPair = symRefPairSearchResult != _hoistedAutos->end()
                                   ? symRefPairSearchResult->second
                                   : NULL;
      while (symRefPair) {
      ...
      if (!internalPointerSymRef) {
        ...
        if (isInternalPointer && !pinningArrayPointer) {
          ...
          dumpOptDetails(
              ...
              "inserted initialization tree : %p for new symRef #%d\n",
              newStoreTreeTop->getNode(),
              newPinningArray->getReferenceNumber());
          _numInternalPointerOrPinningArrayTempsInitialized++;
        }
    ...
    if (node->getSecondChild()->getOpCodeValue() == TR::iconst ||
      ...
      if ((isInternalPointer &&
           ...
           ((isAdd && (constValue == hdrSize)) ||
            (!isAdd && constValue == -hdrSize))) &&
          (!_registersScarce || (node->getReferenceCount() > 1) ||
           _reassociatedNodes.find(node)) &&
          (!comp()->cg()->canBeAffectedByStoreTagStalls() ||
           _numInternalPointerOrPinningArrayTempsInitialized <
               MAX_INTERNAL_POINTER_AUTOS_INITIALIZED) &&
          performTransformation(comp(),
                                "%s Replacing reassociated internal pointer "
                                "based on symRef #%d\n",
                                OPT_DETAILS, internalPointerSymbol)) {
        if (_reassociatedAutos->find(originalInternalPointerSymbol) ==
            _reassociatedAutos->end()) {
          TR::SymbolReference *newSymbolReference =
              comp()->getSymRefTab()->createTemporary(
                  comp()->getMethodSymbol(), TR::Address, isInternalPointer);
          if (isInternalPointer && !pinningArrayPointer) {
            ...
            TR::TreeTop *nextTree = placeHolderTree->getNextTreeTop();
            newStore->setLocalIndex(~0);
            TR::TreeTop *newStoreTreeTop =
                TR::TreeTop::create(comp(), newStore);
            placeHolderTree->join(newStoreTreeTop);
            newStoreTreeTop->join(nextTree);
            dumpOptDetails(comp(),
                           "\nO^O INDUCTION VARIABLE ANALYSIS: Induction "
          ...
          (*_reassociatedAutos)[originalInternalPointerSymbol] =
              newSymbolReference;
        ...
        TR::SymbolReference *internalPointerSymRef =
            (*_reassociatedAutos)[originalInternalPointerSymbol];
        originalNode->getFirstChild()->recursivelyDecReferenceCount();
        ...
        TR::Node *newLoad = TR::Node::createWithSymRef(node, TR::aload, 0,
                                                       internalPointerSymRef);
        newLoad->setLocalIndex(~0);
  ...
  if (examineChildren) {
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++) {
      if (reassociateAndHoistComputations(loopInvariantBlock, node, i,
                                          node->getChild(i), visitCount))
        reassociatedComputation = true;
    }
  }
...
bool TR_LoopStrider::isStoreInRequiredForm(int32_t symRefNum,
  ...
  if (_storeTreesList) {
    ...
    if (lookup != _storeTreesList->end()) {
      ...
      for (storeTree = si.getCurrent(); storeTree != NULL;
        ...
        if (storeTree->_tt->getEnclosingBlock()
                ->getStructureOf()
                ->getContainingLoop() != loopStructure)
          return false;
      }
...
void TR_LoopStrider::checkIfIncrementInDifferentExtendedBlock(
    TR::Block *block, int32_t inductionVariable) {
  _incrementInDifferentExtendedBlock = false;
  if (block != _storeTrees[inductionVariable]
                   ->getEnclosingBlock()
                   ->startOfExtendedBlock()) {
...
TR::Node *TR_LoopStrider::updateLoadUsedInLoopIncrement(
  ...
  if (_indirectInductionVariable && opCode.isLoadVar() &&
      ...
      ((!_storeTreesList &&
        _writtenExactlyOnce.ValueAt(
            node->getSymbolReference()->getReferenceNumber())) ||
       (_storeTreesList &&
        _storeTreesList->find(
            node->getSymbolReference()->getReferenceNumber()) !=
            _storeTreesList->end() &&
        (*_storeTreesList)[node->getSymbolReference()->getReferenceNumber()]
            ->isSingleton()))) {
    ...
    if (useDefInfo->getUseDef(defs, useIndex) &&
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        TR::Node *constNode = containsOnlyInductionVariableAndAdditiveConstant(
            defNode->getFirstChild(), inductionVariable);
        if (constNode &&
            _storeTrees[defNode->getSymbolReference()->getReferenceNumber()]) {
          checkIfIncrementInDifferentExtendedBlock(
              useDefInfo->getTreeTop(defIndex)
                  ->getEnclosingBlock()
                  ->startOfExtendedBlock(),
              inductionVariable);
        } else {
...
bool TR_LoopStrider::isStoreInRequiredForm(TR::Node *storeNode,
                                           int32_t symRefNum,
                                           TR_Structure *loopStructure) {
  /* calls too can kill symRefs via loadaddr */
  if (symRefNum != 0 && _allKilledSymRefs[symRefNum] == true) return false;
  ...
  if (_constNode == NULL) {
    ...
    TR_InductionVariable *v = loopStructure->asRegion()->findMatchingIV(
        comp()->getSymRefTab()->getSymRef(symRefNum));
    if (!v) return false;
    ...
    // constNode will be forced to positive in
    // placeNewInductionVariableIncrementTree(...)
    ...
    if (trace()) {
      traceMsg(
          comp(),
          "Found loop induction variable #%d incremented indirectly by %lld\n",
          symRefNum, low);
    }
  } else {
...
static void orderSensitiveDescendantsRec(TR::Node *node, TR::NodeChecklist &out,
 ...
 */
static void assertSubstPreservesEvalOrder(TR::Node *orig, TR::Node *replacement,
                                          const char *operation) {
  if (enableExpensiveLoopStriderAssertions()) {
    TR_ASSERT_FATAL(substPreservesEvalOrder(orig, replacement),
                    "%s fails to preserve ordering\n", operation);
  }
}

// node-recursive implementation of assertStructureDoesNotMentionOriginals
// (below)
static void assertSubtreeDoesNotMentionOriginals(
    TR::Node *node, const TR::list<std::pair<int32_t, int32_t> > &widenedIVs,
    TR::NodeChecklist &visited) {
  if (visited.contains(node)) return;

  visited.add(node);
  for (int i = 0; i < node->getNumChildren(); i++)
    assertSubtreeDoesNotMentionOriginals(node->getChild(i), widenedIVs,
                                         visited);

  if (!node->getOpCode().hasSymbolReference()) return;

  int32_t symRefNum = node->getSymbolReference()->getReferenceNumber();
  for (auto it = widenedIVs.begin(); it != widenedIVs.end(); ++it) {
    TR_ASSERT_FATAL(symRefNum != it->first,
...
static void assertStructureDoesNotMentionOriginals(
  ...
  if (!enableExpensiveLoopStriderAssertions()) return;
  ...
  if (structure->asBlock() != NULL) {
    ...
    for (TR_StructureSubGraphNode *n = it.getFirst(); n != NULL;
         n = it.getNext())
...
static bool isLoopRelativelyFrequent(TR_Structure *loop, TR::Block *preheader) {
  ...
  if (minFreq <= 0) minFreq = defaultMinFreq;
  ...
  if (minIters <= 0.f) minIters = defaultMinIters;

  // Check the loop and preheader frequencies
  uint16_t loopFreq = loop->getEntryBlock()->getFrequency();
  if (loopFreq <= 0) return true;  // assume frequent when unknown
  ...
  if (entryFreq <= 0) return true;  // assume frequent when unknown

  float estimatedAverageIters = float(loopFreq) / float(entryFreq);
  if (estimatedAverageIters < minIters) {
    dumpOptDetails(TR::comp(),
                   "[Sign-Extn] skip loop %d due to estimated average "
                   "iterations %f < %f\n",
 ...
 * \see replaceLoadsInStructure(TR_Structure*, int32_t, TR::SymbolReference*,
 TR::NodeChecklist&, TR::NodeChecklist&)
 * \see extendIVsOnLoopEntry(const TR::list<std::pair<int32_t, int32_t> >&,
 TR::Block*)
 * \see truncateIVsOnLoopExit(const TR::list<std::pair<int32_t, int32_t> >&,
 TR_RegionStructure*)
 * \see eliminateSignExtensions(TR::NodeChecklist&)
 */
void TR_LoopStrider::detectLoopsForIndVarConversion(
    TR_Structure *loopStructure, TR::NodeChecklist &exLoads) {
  // 64-bit
  bool usingAladd = (TR::Compiler->target.is64Bit()) ? true : false;

  // Stress mode: choose to replace *every* possible candidate 32-bit IV with
  // a 64-bit one, regardless of benefit/detriment.
  static const char *stressModeEnv =
      feGetEnv("TR_stressLoopStriderSignExtension");
  static const bool stressMode =
      stressModeEnv != NULL && stressModeEnv[0] != '\0';
  ...
  if (regionStructure->getEntryBlock()->isCold() && !stressMode) return;
  ...
  for (nextBlock = bi.getCurrent(); nextBlock; nextBlock = bi.getNext()) {
    if (nextBlock->hasExceptionPredecessors()) return;

    // bail out if there is any exception edges to OSRCatchBlock because there
    // is no safe way to split those edges. Also, there is no need to check if
    // the OSRCatchBlock is within the loop because loop strider already skips
    // any loop that contains a catch block
    if (nextBlock->hasExceptionSuccessors()) {
      ...
      for (TR::CFGEdge *e = sit.getFirst(); e != NULL; e = sit.getNext()) {
        ...
        if (dest->isOSRCatchBlock()) {
          if (trace())
            traceMsg(comp(),
                     "reject loop %d because dest (block_%d) of an exit edge "
                     "is OSRCatchBlock\n",
  ...
  if (loopInvariantBlock) {
    ...
    if (!stressMode && !isLoopRelativelyFrequent(
                           loopStructure, loopInvariantBlock->getBlock()))
      return;
    ...
    if (_storeTreesList) _storeTreesList->clear();
    _storeTreesList = NULL;
    //_loadUsedInNewLoopIncrementList = NULL;
    ...
    if (usingAladd && info) {
      if (isPredictableLoop > 0) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          ...
          if (ivType != TR::Int32) continue;

          bool storeInRequiredForm =
              isStoreInRequiredForm(nextInductionVariable, loopStructure);
          // verify induction variable candidate is not a parameter
          if (storeInRequiredForm &&
              performTransformation(
                  comp(),
                  "%s Analyzing candidate: %d in loop: %d for conversion\n",
                  OPT_DETAILS, _loopDrivingInductionVar,
                  loopStructure->getNumber())) {
            TR_ASSERT(_loopDrivingInductionVar == nextInductionVariable,
                      "_loopDrivingInductionVar #%d does not match "
                      "nextInductionVariable #%d\n",
            ...
            // examine expressions containing induction variable that need to be
            // changed
            morphExpressionsLinearInInductionVariable(loopStructure,
                                                      visitCount);

            if (_isInductionVariableMorphed || stressMode) {
              if (checkStoreOfIndVar(
                      _storeTrees[nextInductionVariable]->getNode())) {
                // found a chance to convert the induction variable
                TR::SymbolReference *ext =
                    comp()->getSymRefTab()->createTemporary(
                        comp()->getMethodSymbol(), TR::Int64);
                _newTempsCreated = true;
                walkTreesAndFixUseDefs(loopStructure, ext, exLoads);
                widenedIVs.push_back(std::make_pair(nextInductionVariable,
...
void TR_LoopStrider::extendIVsOnLoopEntry(
    const TR::list<std::pair<int32_t, int32_t> > &ivs, TR::Block *preheader) {
  TR::TreeTop *tt = preheader->getLastRealTreeTop();
  if (!tt->getNode()->getOpCode().isBranch()) tt = tt->getNextTreeTop();
...
void TR_LoopStrider::truncateIVsOnLoopExit(
  ...
  for (TR::Block *src = lit.getFirst(); src != NULL; src = lit.getNext()) {
    ...
    for (TR::CFGEdge *e = sit.getFirst(); e != NULL; e = sit.getNext()) {
      TR::Block *dest = toBlock(e->getTo());
      if (!inLoop.contains(dest)) exits.add(dest);
    }
 ...
 */
void TR_LoopStrider::convertIV(TR::Node *bciNode, TR::TreeTop *nextTT,
                               int32_t ivIn, int32_t ivOut, TR::ILOpCodes op) {
  ...
  dumpOptDetails(
      ...
      ivOut, TR::ILOpCode(op).getName(), ivIn,
...
void TR_LoopStrider::createConstraintsForNewInductionVariable(
    TR_Structure *loopStructure, TR::SymbolReference *newSymRef,
    TR::SymbolReference *oldSymRef) {
  TR_InductionVariable *v =
      loopStructure->asRegion()->findMatchingIV(oldSymRef);
  if (v) {
    // old constraints
    TR::VPConstraint *entryVal = v->getEntry();
    TR::VPConstraint *exitVal = v->getExit();
    ...
    // new constraints
    TR::VPConstraint *newEntryVal =
        genVPLongRange(entryVal, 1, 0);  // create the new entry constraint
    TR::VPConstraint *newExitVal =
        genVPLongRange(entryVal, 1, 0);  // create the new exit constraint
    TR::VPConstraint *newIncr =
        genVPLongRange(incr, 1, 0);  // create the new incr constraint

    TR_InductionVariable *oldIV =
        loopStructure->asRegion()->findMatchingIV(oldSymRef);

    // add the new constraint and new induction variable
    TR_InductionVariable *newv = new (trHeapMemory()) TR_InductionVariable(
        newSymRef->getSymbol()->castToRegisterMappedSymbol(), newEntryVal,
        newExitVal, newIncr, oldIV ? oldIV->isSigned() : TR_maybe);
    loopStructure->asRegion()->addInductionVariable(newv);
  }
}

void TR_LoopStrider::morphExpressionsLinearInInductionVariable(
    TR_Structure *structure, vcount_t visitCount) {
  if (structure->asBlock() != NULL) {
    TR_BlockStructure *blockStructure = structure->asBlock();
    TR::Block *block = blockStructure->getBlock();
    ...
    while (currentTree != exitTree) {
      TR::Node *currentNode = currentTree->getNode();
      _currTree = currentTree;
      morphExpressionLinearInInductionVariable(NULL, -1, currentNode,
                                               visitCount);
      currentTree = currentTree->getNextTreeTop();
    }
  } else {
    ...
    TR_RegionStructure::Cursor si(*regionStructure);
    for (node = si.getCurrent(); node != NULL; node = si.getNext())
      morphExpressionsLinearInInductionVariable(node->getStructure(),
                                                visitCount);
  }
}
...
bool TR_LoopStrider::morphExpressionLinearInInductionVariable(
  ...
  if (node->getOpCodeValue() == TR::ladd ||
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
      ...
      if (firstChild->getOpCodeValue() == TR::lmul ||
        ...
        if (firstChild->getFirstChild()->getOpCodeValue() == TR::i2l) {
          ...
          isLinearInInductionVariable =
              checkExpressionForInductionVariable(indVarTree);
          if (isLinearInInductionVariable &&
            ...
            if (ivLoad != NULL &&
                (ivLoad == indVarTree || indVarTree->cannotOverflow()))
  ...
  if (examineChildren) {
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++)
      morphExpressionLinearInInductionVariable(node, i, node->getChild(i),
                                               visitCount);
  }
  return true;
}
...
/**
 * Replace \c _loopDrivingInductionVar with \p newSymbolReference in \p
 * loopStructure.
 *
...
void TR_LoopStrider::walkTreesAndFixUseDefs(
    TR_Structure *loopStructure, TR::SymbolReference *newSymbolReference,
    TR::NodeChecklist &exLoads) {
  TR::DebugCounter::incStaticDebugCounter(
      comp(), TR::DebugCounter::debugCounterName(
                  comp(), "loopStrider.widen/(%s)/%s/loop=%d/iv=%d",
                  comp()->signature(),
  ...
  TR_ASSERT(storeNode->getOpCodeValue() == TR::istore,
            "storeNode opcode is %s; expected istore\n",
            storeNode->getOpCode().getName());
  TR_ASSERT(storeNode->getSymbolReference()->getReferenceNumber() ==
                _loopDrivingInductionVar,
  ...
  storeNode->setAndIncChild(
      0, TR::Node::create(storeNode, TR::i2l, 1, newIntValue));
  newIntValue->decReferenceCount();
  storeNode->setSymbolReference(newSymbolReference);
  TR::Node::recreate(storeNode, TR::lstore);

  TR::NodeChecklist visited(comp());
  replaceLoadsInStructure(loopStructure, _loopDrivingInductionVar, newIntValue,
                          newSymbolReference, exLoads, visited);

  TR::SymbolReference *oldSymRef =
      comp()->getSymRefTab()->getSymRef(_loopDrivingInductionVar);
  createConstraintsForNewInductionVariable(loopStructure, newSymbolReference,
                                           oldSymRef);
}
 ...
 * \see detectLoopsForIndVarConversion(TR_Structure*, TR::NodeChecklist&)
 */
void TR_LoopStrider::replaceLoadsInStructure(TR_Structure *structure,
                                             int32_t iv, TR::Node *defRHS,
  ...
  if (structure->asBlock() != NULL) {
    ...
    for (TR_StructureSubGraphNode *n = it.getFirst(); n != NULL;
         n = it.getNext())
      replaceLoadsInStructure(n->getStructure(), iv, defRHS, newSR, exLoads,
                              visited);
  }
}

// node-recursive implementation of replaceLoadsInStructure (above)
void TR_LoopStrider::replaceLoadsInSubtree(TR::Node *node, int32_t iv,
                                           TR::Node *defRHS,
                                           TR::SymbolReference *newSR,
                                           TR::NodeChecklist &exLoads,
                                           TR::NodeChecklist &visited) {
  ...
  visited.add(node);
  for (int i = 0; i < node->getNumChildren(); i++)
    replaceLoadsInSubtree(node->getChild(i), iv, defRHS, newSR, exLoads,
                          visited);
 ...
 */
void TR_LoopStrider::widenComparison(TR::Node *node, int32_t iv,
                                     TR::Node *defRHS,
                                     TR::NodeChecklist &exLoads) {
  ...
  if (!isLeftIV && !isRightIV) return;

  if (op.isIf()) lcmp = TR::ILOpCode(lcmp).convertCmpToIfCmp();

  if (!performTransformation(
          comp(), "%s [Sign-Extn] Changing n%un %s into %s\n",
          optDetailString(), node->getGlobalIndex(),
          node->getOpCode().getName(), TR::ILOpCode(lcmp).getName()))
    return;
...
void TR_LoopStrider::eliminateSignExtensions(TR::NodeChecklist &exLoads) {
  ...
  for (TR::TreeTop *tt = start; tt != NULL; tt = tt->getNextTreeTop())
    eliminateSignExtensionsInSubtree(tt->getNode(), exLoads, visited,
                                     signExtMemo);
...
void TR_LoopStrider::eliminateSignExtensionsInSubtree(
    TR::Node *parent, TR::NodeChecklist &exLoads, TR::NodeChecklist &visited,
  ...
  for (int i = 0; i < parent->getNumChildren(); i++) {
    ...
    if (node->getOpCodeValue() != TR::i2l) continue;
    ...
    if (!performTransformation(comp(),
                               "%s [Sign-Extn] Replacing occurrence of n%un "
                               "i2l with n%un as %dth child of n%un\n",
                               optDetailString(), node->getGlobalIndex(),
                               extended->getGlobalIndex(), i,
...
TR_LoopStrider::SignExtEntry TR_LoopStrider::signExtend(
    TR::Node *node, TR::NodeChecklist &exLoads, SignExtMemo &memo) {
  ...
  switch (node->getOpCodeValue()) {
    case TR::iconst:
      result.extended =
          TR::Node::lconst(node, (int64_t)node->getConst<int32_t>());
    ...
    case TR::l2i:
      if (exLoads.contains(node)) {
        TR::Node *child = node->getFirstChild();
        TR_ASSERT(child->getOpCodeValue() == TR::lload,
...
TR_LoopStrider::SignExtEntry TR_LoopStrider::signExtendBinOp(
    TR::ILOpCodes op64, TR::Node *node, TR::NodeChecklist &exLoads,
    SignExtMemo &memo) {
  const SignExtEntry rejected;
  if (!node->cannotOverflow()) return rejected;
 ...
 * structure produced by signExtend(TR::Node*, TR::NodeChecklist&,
 * SignExtMemo&).
 *
 * \param intNode  the original 32-bit int-typed node
 * \param longNode the 64-bit long-typed node corresponding to \p intNode
 */
void TR_LoopStrider::transmuteDescendantsIntoTruncations(TR::Node *intNode,
                                                         TR::Node *longNode) {
  ...
  if (!performTransformation(
          comp(), "%s [Sign-Extn] Transmuting n%un %s into l2i of n%un %s\n",
          optDetailString(), intNode->getGlobalIndex(),
          intNode->getOpCode().getName(), longNode->getGlobalIndex(),
    ...
    if (firstChild->getFirstChild()->getOpCode().hasSymbolReference()) {
      if (firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        if (firstChild->getFirstChild()
                ->getSymbolReference()
                ->getReferenceNumber() == _loopDrivingInductionVar) {
          if (!firstChild->cannotOverflow()) return false;
          return true;
        } else
          return false;
      }
...
bool TR_LoopStrider::checkExpressionForInductionVariable(TR::Node *node) {
  if (node->getOpCodeValue() == TR::iload) {
    if (_writtenExactlyOnce.ValueAt(
            node->getSymbolReference()->getReferenceNumber()))
      return true;
  } else if (node->getOpCodeValue() == TR::iadd &&
...
int64_t TR_LoopStrider::getAdditiveTermConst(int32_t k) {
  TR_ASSERT(k < _numberOfLinearExprs,
            "index k %d exceeds _numberOfLinearExprs %d!\n", k,
            _numberOfLinearExprs);
...
bool TR_LoopStrider::isAdditiveTermEquivalentTo(int32_t k, TR::Node *node) {
  if (isAdditiveTermConst(k) && node->getOpCode().isLoadConst()) {
    ...
    if (getAdditiveTermConst(k) == addConst) return true;
  } else if (getAdditiveTermNode(k) &&
             getAdditiveTermNode(k)->getOpCode().hasSymbolReference() &&
             node->getOpCode().hasSymbolReference() &&
             getAdditiveTermNode(k)->getSymbolReference() ==
...
TR::Node *TR_LoopStrider::duplicateMulTermNode(int32_t k, TR::Node *node,
                                               TR::DataType type) {
  TR_ASSERT(k < _numberOfLinearExprs,
            "index k %d exceeds _numberOfLinearExprs %d!\n", k,
            _numberOfLinearExprs);
  TR::Node *new_node =
      ((TR::Node *)(intptrj_t)_linearEquations[k][2])->duplicateTree();
  ...
  if (new_node->getDataType() != type)
    new_node = TR::Node::create(
        TR::ILOpCode::getProperConversion(new_node->getDataType(), type,
...
bool TR_LoopStrider::isMulTermEquivalentTo(int32_t k, TR::Node *node) {
  if (isMulTermConst(k) && node->getOpCode().isLoadConst()) {
    ...
    if (getMulTermConst(k) == multiplicativeConstant) return true;
  } else if (getMulTermNode(k) &&
             getMulTermNode(k)->getOpCode().hasSymbolReference() &&
             node->getOpCode().hasSymbolReference() &&
             getMulTermNode(k)->getSymbolReference() ==
                 node->getSymbolReference()) {
...
const char *TR_LoopStrider::optDetailString() const throw() {
...
TR_InductionVariableAnalysis::TR_InductionVariableAnalysis(
...
void TR_InductionVariableAnalysis::removeStaleIVs(TR_RegionStructure *region) {
  ...
  for (TR_StructureSubGraphNode *n = it.getFirst(); n != NULL;
    ...
    if (subRegion != NULL) removeStaleIVs(subRegion);
  }
}

void TR_InductionVariableAnalysis::gatherCandidates(TR_Structure *s,
                                                    TR_BitVector *loopLocalDefs,
                                                    TR_BitVector *allDefs) {
  if (s->asRegion()) {
    ...
    if (!region->isAcyclic()) {
      loopLocalDefs = new (trStackMemory())
          TR_BitVector(comp()->getSymRefTab()->getNumSymRefs(), trMemory());
      myAllDefs = new (trStackMemory())
          TR_BitVector(comp()->getSymRefTab()->getNumSymRefs(), trMemory());
    } else
      myAllDefs = allDefs;
    ...
    if (!region->isAcyclic()) {
      region->setAnalysisInfo(new (trStackMemory())
                                  AnalysisInfo(loopLocalDefs, myAllDefs));

      if (trace()) {
        traceMsg(comp(),
                 "All Defs inside cyclic region %d: ", region->getNumber());
        myAllDefs->print(comp());
        traceMsg(comp(), "\nLoopLocalDefs inside cyclic region %d: ",
                 region->getNumber());
...
void TR_InductionVariableAnalysis::perform(TR_RegionStructure *region) {
  ...
  //
  TR_RegionStructure::Cursor it(*region);
  for (TR_StructureSubGraphNode *node = it.getFirst(); node;
       node = it.getNext()) {
...
void TR_InductionVariableAnalysis::analyzeNaturalLoop(
    TR_RegionStructure *loop) {
...

  {
    ...
    // For each of the candidates, clear the local index to avoid polluting
    // analyses of later loops
    //
  ...
  if (trace()) traceMsg(comp(), "</analyzeNaturalLoop>\n");
}

void TR_InductionVariableAnalysis::analyzeAcyclicRegion(
    TR_RegionStructure *region, TR_RegionStructure *loop) {
  if (loop != region) TR_ASSERT(region->isAcyclic(), "assertion failure");
...
void TR_InductionVariableAnalysis::analyzeBlock(TR_BlockStructure *structure,
                                                TR_RegionStructure *loop) {
  TR::Block *block = structure->getBlock();
  DeltaInfo **inSet = getBlockInfo(block);
  TR_BitVector *candidates =
      ((AnalysisInfo *)loop->getAnalysisInfo())->getLoopLocalDefs();

  if (trace()) traceMsg(comp(), "analyzeBlock %d\n", block->getNumber());
  ...
  if (trace()) {
    ...
    while (it.hasMoreElements()) {
      ...
      traceMsg(comp(), "\t%d %d %p symRef=%p symbol=%p: ", refNum, refIndex,
               inSymbol, symRef, symRef->getSymbol());
      if (inSymbol)
    ...
    if (node->getOpCode().isStoreDirect() &&
      ...
      if (candidates->isSet(refNum)) {
        if (trace())
          traceMsg(
              comp(),
              "node %p effects candidate %d (refNum: %d) symRef=%p symbol=%p\n",
        ...
        if (isProgressionalStore(node, &kind, &delta)) {
          ...
          // traceMsg(comp(),"In analyze block loop %d block %d, setting
          // inSymbol %d to unknown value
  ...
  for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
    ...
    if (succ != loop->getEntryBlock()) mergeWithBlock(succ, outSet, loop);
  }

  if (trace()) traceMsg(comp(), "\n");
}

void TR_InductionVariableAnalysis::analyzeCyclicRegion(
    TR_RegionStructure *region, TR_RegionStructure *loop) {
  TR::Block *entryBlock = region->getEntryBlock();
  DeltaInfo **inSet = getBlockInfo(entryBlock);

  TR_BitVector *allInnerDefs =
      ((AnalysisInfo *)region->getAnalysisInfo())->getAllDefs();
  TR_BitVector *loopLocalDefs =
      ((AnalysisInfo *)loop->getAnalysisInfo())->getLoopLocalDefs();
  ...
  for (TR::CFGEdge *edge = eit.getFirst(); edge; edge = eit.getNext()) {
    ...
    TR_StructureSubGraphNode *succ =
        region->getParent()->asRegion()->findNodeInHierarchy(exitNumber);
...
void TR_InductionVariableAnalysis::mergeWithBlock(TR::Block *block,
                                                  DeltaInfo **inSet,
                                                  TR_RegionStructure *loop) {
  DeltaInfo **mySet = getBlockInfo(block);
  if (!mySet) mySet = setBlockInfo(block, newBlockInfo(loop));
  mergeWithSet(mySet, inSet, loop);
}

void TR_InductionVariableAnalysis::mergeWithSet(DeltaInfo **mySet,
                                                DeltaInfo **inSet,
                                                TR_RegionStructure *loop) {
  TR_BitVector *candidates =
      ((AnalysisInfo *)loop->getAnalysisInfo())->getLoopLocalDefs();
...
TR_InductionVariableAnalysis::newBlockInfo(TR_RegionStructure *loop) {
  int32_t numCandidates = ((AnalysisInfo *)loop->getAnalysisInfo())
                              ->getLoopLocalDefs()
                              ->elementCount();  /// FIXME
  DeltaInfo **mySet = (DeltaInfo **)trMemory()->allocateStackMemory(
      numCandidates * sizeof(DeltaInfo *));
  memset(mySet, 0, numCandidates * sizeof(DeltaInfo *));
  return mySet;
}

void TR_InductionVariableAnalysis::initializeBlockInfoArray(
    TR_RegionStructure *loop) {
  int32_t numBlocks = comp()->getFlowGraph()->getNextNodeNumber();
  _blockInfo = (DeltaInfo ***)trMemory()->allocateStackMemory(
      numBlocks * sizeof(DeltaInfo **));
...
//
bool TR_InductionVariableAnalysis::isProgressionalStore(
    TR::Node *node, TR_ProgressionKind *kind, int64_t *incr) {
  ...
  while (cursor->getOpCode().isConversion()) cursor = cursor->getFirstChild();

  if (!(cursor->getOpCode().isAdd() || cursor->getOpCode().isSub() ||
        cursor->getOpCode().isLeftShift() ||
        cursor->getOpCode().isRightShift())) {
    // traceMsg(comp(),"In isProgressionalStore cursor node %p returning false.
    // its not add/sub/shift\n",cursor);
    return false;
  }
  ...
  {
    // traceMsg(comp(),"In isProgressionalStore cursor node %p returning false.
    // symRef = %p(%d) is not equal to storeRef
  ...
  if (((int64_t)(int32_t)(*incr)) != *incr) {
    // traceMsg(comp(),"In isProgressionalStore cursor node %p, delta is not
    // representable in 32bits\n",cursor);
    return false;
  }
  return true;
}
...
bool TR_InductionVariableAnalysis::branchContainsInductionVariable(
    TR_RegionStructure *loop, TR::Node *branchNode,
...
// Finds the progession taking place on an induction variable.  Right now this
// method recognizes arithmetic progressions of all kinds and Geometric
// progressions where the multiplicative constant (r) is a power of 2 (shift).
// If the storeRef argument has a non-null value, this function will find the
// progression taking place on that symref only.
//
// Returns true if it recognizes a progression taking place.  Returns the symbol
// reference of the induction variable in ref, the kind of progression in prog,
// and the progression constant in incr.
//
// For identity progression incr has no meaning.
// For arithmetic progressions, incr contains the increment value.
// For geometric progressions, incr represents the multiplicative constant.
// geometric progression. The multiplicative constant for the series (r) =
// 2^(incr).  Naturally this means
//   that the series is a shrinking series if incr is negative.
//
bool TR_InductionVariableAnalysis::getProgression(TR::Node *expr,
                                                  TR::SymbolReference *storeRef,
                                                  TR::SymbolReference **ref,
                                                  TR_ProgressionKind *prog,
                                                  int64_t *incr) {
  int64_t konst;
  TR_ProgressionKind kind;

  TR::Node *secondChild =
      expr->getNumChildren() > 1 ? expr->getSecondChild() : 0;
  ...
  else if (expr->getOpCode().isLeftShift() &&
  secondChild->getOpCode().isLoadConst())
     {
  ...
  else if (expr->getOpCode().isRightShift() &&
  secondChild->getOpCode().isLoadConst())
     {
     if (!getProgression(expr->getFirstChild(), ref, &kind, &konst))
        return false;
  ...
  */
    ...
    if (expr->getOpCode().hasSymbolReference()) {
      if (!expr->getSymbolReference()->getSymbol()->isAutoOrParm())
        return false;
      if (storeRef && (expr->getSymbolReference() != storeRef ||
                       expr->getVisitCount() == comp()->getVisitCount()))
        return false;
...
void TR_InductionVariableAnalysis::analyzeLoopExpressions(
  ...
  TR_Array<TR_BasicInductionVariable *> *ivs =
      new (trHeapMemory()) TR_Array<TR_BasicInductionVariable *>(
          trMemory(), candidates->elementCount(), true, heapAlloc);
  ...
  while (it.hasMoreElements()) {
    ...
    // TR_ASSERT(info, "missing store/delta information for definitly written
    // symref");

    // It is possible for info to be null if the expression was written to only
    // on a path that exits the loop.
    //
    if (!info) continue;
    if (info->isUnknownValue()) {
      ...
      if (trace())
        traceMsg(comp(),
                 "----> symRef #%d[%p] is using an identity progression\n",
                 refNum, symRef);
      ...
      if (info->getDelta() == 0) {
        ...
        if (trace())
          traceMsg(comp(),
                   "====> Found basic linear induction variable symRef #%d[%p] "
                   "with increment %d\n",
                   refNum, symRef, info->getDelta());

        TR_BasicInductionVariable *biv = new (trHeapMemory())
            TR_BasicInductionVariable(comp(), loop, symRef);
      ...
      if (trace())
        traceMsg(comp(),
                 "====> Found basic geometric induction variable symRef "
                 "#%d[%p] with increment %d\n",
                 refNum, symRef, info->getDelta());
  ...
  //    traceMsg(comp(),"Before findEntryValues basicIVs->isEmpty() =
  //    %d\n",basicIVs.isEmpty());
  ...
  // recognized as the primary iv. collect the others and hang them off
  // structure so that some other opt may utilize this info
  //
  if (!success2) {
    TR_ArrayIterator<TR_BasicInductionVariable> bit(&basicIVs);
    for (TR_BasicInductionVariable *biv = bit.getFirst(); biv;
         biv = bit.getNext()) {
...
//
static uint16_t countNodeOccurrencesInSubTree(TR::Node *root, TR::Node *node,
                                              vcount_t visitCount,
                                              TR::Compilation *comp) {
  uint16_t count = 0;
  ...
  if (root->getVisitCount() == visitCount) return 0;
...
void TR_InductionVariableAnalysis::appendPredecessors(WorkQueue &dest,
                                                      TR::Block *block) {
...
bool TR_InductionVariableAnalysis::isIVUnchangedInLoop(TR_RegionStructure *loop,
  ...
  if (disable) {
    if (trace())
      traceMsg(comp(),
               "\tintermediate value check disabled; assuming no earlier "
               "modifications\n");
    return true;
  }
  ...
  if (trace())
    traceMsg(comp(),
             "\tTrying to make sure that candidate IV hasn't been modified "
  ...
  while (!workQueue.empty()) {
    ...
    if (nodesDone.contains(curBlock)) continue;
    ...
    for (TR::TreeTop *tt = curBlock->getFirstRealTreeTop();
      ...
      if (curNode->getOpCode().isStoreDirect() &&
        ...
        if (trace())
          traceMsg(comp(),
                   "\t\tFound store %p of symRef %p in block %d, which is not "
                   "a loop test block\n",
                   curNode, candidateIV->getSymbolReference()->getSymbol(),
                   curBlock->getNumber());
        return false;
      }
    }

    // if this is the loop header then we don't want to process it's
    // predecessors
    if (loopEntryBlock != curBlock) appendPredecessors(workQueue, curBlock);
  }

  if (trace()) traceMsg(comp(), "\tIV hasn't been modified in the loop body\n");
...
bool TR_InductionVariableAnalysis::analyzeExitEdges(
    TR_RegionStructure *loop, TR_BitVector *candidates,
    TR_Array<TR_BasicInductionVariable *> &basicIVs) {
  ...
  if (trace())
    traceMsg(comp(),
             "Trying to analyze the exit edges to determine if this is a "
             "counted loop\n");

  TR_Array<DeltaInfo *> exitInfos(trMemory(), candidates->elementCount(), true,
                                  stackAlloc);
  TR_Array<TR::Node *> bounds(trMemory(), candidates->elementCount(), true,
                              stackAlloc);
  ...
  for (edge = eit.getFirst(); edge; edge = eit.getNext()) {
    ...
    if (subNode->getStructure()->asRegion()) {
      // OSR blocks can be ignored here as they will not be impacted by a loop
      // controlling condition.
      //
      // Exit edges do not have structure for their destination, rather they
      // only specify the block number. As all OSRInduceBlocks have an edge to
      // the exit, even though they end in a throw, it is possible to search for
      // a block with a matching number. This is potentially expensive, so it is
      // memoized with two bit vectors, _seenInnerRegionExit and
      // _isOSRInduceBlock.
      //
      ...
      if (!_seenInnerRegionExit.get(toNum)) {
        ...
        for (auto predEdge = end->getPredecessors().begin();
             predEdge != end->getPredecessors().end(); ++predEdge) {
      ...
      if (osrInduceExitEdge) {
        if (trace())
          traceMsg(comp(),
                   "\tbranch from inner region is to OSR block, ignoring\n");
        continue;
      }
    ...
    while (isGotoBlock(block) && (block->getPredecessors().size() == 1)) {
      TR::Block *predBlock =
          block->getPredecessors().front()->getFrom()->asBlock();
      if (predBlock->getStructureOf()->getContainingLoop() ==
    ...
    if (((!node->getOpCode().isBooleanCompare() ||
        ...
        (node->getOpCode().isJumpWithMultipleTargets() &&
         !node->getOpCode()
              .hasBranchChildren()) ||  // allow tstarts to continue
    ...
    if (!(node->getFirstChild()->getType().isIntegral() ||
      ...
      if (trace())
        traceMsg(comp(),
                 "\tReject - Branch node is not integral nor ordered "
                 "comparison between internal pointers\n");
      return false;
    }
    ...
    if (!node->getOpCode().isJumpWithMultipleTargets()) {
      ...
      // TM: a jump with multiple target (this case, a tstart) is not a
      // 'controlling branch to exit' in terms of the temp induction variable, so
      // we can continue

      if (trace())
        traceMsg(comp(),
                 "\tbranch is a tstart, not a controlling branch. Ignoring\n");

      continue;
    }

    //      traceMsg(comp(),"Before branchContainsInductionVariable
    //      basicIVs->isEmpty() = %d\n",basicIVs.isEmpty());
    ...
    for (auto excEdge = block->getExceptionSuccessors().begin();
         excEdge != block->getExceptionSuccessors().end(); ++excEdge) {
    ...
    for (auto predEdge = loop->getEntryBlock()->getPredecessors().begin();
         predEdge != loop->getEntryBlock()->getPredecessors().end();
         ++predEdge) {
    ...
    } else if (controllingIV != index ||
               controllingBranchToExit != branchToExit ||
               !identicalBranchTrees(controllingBranch->getNode(),
                                     branchTree->getNode())) {
      if (trace())
  ...
  // Now figure out the delta on the loop controlling exit edge and place the
  // information on all the induction variables
  //
  ...
  if (!controllingBranchToExit) opCode = op.getOpCodeForReverseBranch();
  ...
  if (valueNode->getOpCode().isLoadVarDirect()) {
    ...
    static const bool forceILWalk =
        feGetEnv("TR_forceILWalkForLoadsInIVA") != NULL;
    if (valueCouldBeEvaluatedBeforeTest || forceILWalk) {
      ...
      TR::TreeTop *const start =
          branchBlock->startOfExtendedBlock()->getEntry();
      ...
      for (TR::PostorderNodeIterator it(start, comp()); it != end; ++it) {
        ...
        if (!node->getOpCode().isStoreDirect()) continue;
      ...
      if (!seenStore) {
        ...
        if (trace())
          traceMsg(comp(),
                   "\tRejected - tested value evaluated between stores\n");
        return false;
      }

      if (usesUnchangedValueInLoopTest) {
        if (!isIVUnchangedInLoop(loop, controllingBranch->getEnclosingBlock(),
                                 valueNode)) {
          // The loop test may be looking at an intermediate value.
          if (trace())
            traceMsg(comp(),
                     "\tReject - IV node has been changed in one of the blocks "
                     "that is not part of the loop test in the loop\n");
          return false;
        }
      }
    }
    ...
    for (tt = controllingBranch;; tt = tt->getPrevTreeTop()) {
      ...
      if (curNode->getOpCode().isStoreDirect() &&
        ...
        if (curNode->getFirstChild() != valueNode) {
          if (trace())
            traceMsg(comp(),
                     "\tRejected - exit value %p maybe not same as tested "
                     "value %p\n",
                     curNode, valueNode);
  ...
  if ((boundNode->getOpCode().isLoadVarDirect()) &&
    ...
    TR::Node *actualValue =
        findEntryValueForSymRef(loop, boundNode->getSymbolReference());
    if (actualValue) boundNode = actualValue;
  }
  ...
  // If the entry/exit is non-constant - try to do some pattern matching to
  // detect 1- internal pointer induction variables 2- strided induction
  // variables created by LoopStrider
  ...
  if (biv->getEntryValue()) {
    ...
    TR::Node *mulNode =
        ((entry->getOpCode().isAdd() || entry->getOpCode().isSub()) &&
    ...
    if (mulNode->getOpCode().isMul() &&
      ...
      if (loadNode->getOpCode().isLoadVarDirect()) {
        TR::Node *actualValue =
            findEntryValueForSymRef(loop, loadNode->getSymbolReference());
        if (actualValue && actualValue->getOpCode().isLoadConst()) {
          if (entry->getType().isInt32()) {
            ...
            if (entry->getOpCode()
                    .isAdd())  // second child must be a load const
  ...
  if (biv->getEntryValue() &&
    ...
    if (entry->getNumChildren() > 0) baseLoad = entry->getFirstChild();
    if (entry->getOpCode().isAdd() && baseLoad &&
      ...
      if (offset->getOpCode().isAdd() || offset->getOpCode().isSub()) {
        if (offset->getSecondChild()->getOpCode().isLoadConst()) {
          ...
          if (otherNode->getOpCode().isMul() &&
            ...
            if (loadNode->getOpCode().isLoadVarDirect()) {
              ...
              if (actualValue && actualValue->getOpCode().isLoadConst()) {
                if ((actualValue->getSize() == 8) || foundConversion) {
                  ...
                  // mulConst->getLongInt(); // FIXME: overflow
            ...
            TR::Node *actualValue =
                findEntryValueForSymRef(loop, otherNode->getSymbolReference());
            if (actualValue && actualValue->getOpCode().isLoadConst()) {
      ...
      } else if ((offset->getOpCode().isMul() ||
                  offset->getOpCode().isLeftShift()) &&
  ...
  TR_PrimaryInductionVariable *piv =
      new (trHeapMemory()) TR_PrimaryInductionVariable(
          biv, controllingBlock, boundNode->duplicateTree(), opCode, comp(),
          optimizer(), usesUnchangedValueInLoopTest, trace());
  ...
  for (iv = bit.getFirst(); iv; iv = bit.getNext()) {
    if (iv != piv) {
      int32_t index = iv->getSymRef()->getSymbol()->getLocalIndex();
      TR_DerivedInductionVariable *div =
          new (trHeapMemory()) TR_DerivedInductionVariable(comp(), iv, piv);
      basicIVs[index] = div;
...
bool TR_InductionVariableAnalysis::isGotoBlock(TR::Block *block) {
  ...
  return firstTree == block->getLastRealTreeTop() &&
         firstTree->getNode()->getOpCodeValue() == TR::Goto;
}

bool TR_InductionVariableAnalysis::findEntryValues(
    TR_RegionStructure *loop, TR_Array<TR_BasicInductionVariable *> &basicIVs) {
  TR_ArrayIterator<TR_BasicInductionVariable> it(&basicIVs);
  for (TR_BasicInductionVariable *biv = it.getFirst(); biv;
       biv = it.getNext()) {
...
TR::Node *TR_InductionVariableAnalysis::findEntryValueForSymRef(
    TR_RegionStructure *loop, TR::SymbolReference *symRef) {
  // flow backward until we hit a def on each path

  TR::Block *header = loop->getEntryBlock();
  TR_BitVector nodesDone(comp()->getFlowGraph()->getNextNodeNumber(),
                         trMemory(), stackAlloc);
  TR_Array<TR::Node *> cachedValues(trMemory(),
  ...
  for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
    ...
    if (loop->contains(pred->getStructureOf())) continue;
...
TR::Node *TR_InductionVariableAnalysis::getEntryValue(
    TR::Block *block, TR::SymbolReference *symRef, TR_BitVector *nodesDone,
  ...
  if (!entryTree) {
    if (comp()->isLoopTransferDone()) {
      ...
      TR::Node *dummyNode = TR::Node::create(
          NULL, ((dataType == TR::Int32) ? TR::iconst : TR::lconst), 0);
      ...
      TR_ASSERT(symRef->getSymbol()->isParm(),
                "uninitialized local discovered.");
    }
    return 0;
  }

  for (TR::TreeTop *tt = block->getLastRealTreeTop(); tt != entryTree;
       tt = tt->getPrevRealTreeTop()) {
    TR::Node *node = tt->getNode();
    if (node->getOpCodeValue() == TR::treetop) node = node->getFirstChild();
      ...
      if (ref->getReferenceNumber() == symRef->getReferenceNumber()) {
        ...
        return node->getFirstChild();
               ...
               (node->getNumChildren() > 0 &&
                node->getChild(0)->mayKill(false).contains(symRef, comp()))) {
      // In non-java languages, an auto may be killed if it is passed out by
      // reference to a call. ex, say if your for loop was  for(int i = 0 ; i <
      // num ; i++ ) if you find somewhere a call like  foo(&num);  then this
      // kills the num, and we can no longer reason about its value.
...
TR_PrimaryInductionVariable::TR_PrimaryInductionVariable(
    TR_BasicInductionVariable *biv, TR::Block *branchBlock, TR::Node *exitBound,
    TR::ILOpCodes exitOp, TR::Compilation *c, TR::Optimizer *opt,
  ...
  if (getEntryValue() && getEntryValue()->getOpCode().isLoadConst() &&
      (getEntryValue()->getType().isIntegral() ||
       getEntryValue()->getType().isAddress()) &&
      getExitBound()->getOpCode().isLoadConst() &&
      (getExitBound()->getType().isIntegral() ||
       getExitBound()->getType().isAddress())) {
    int64_t exitVal = getExitBound()->getOpCode().isUnsigned()
                          ? getExitBound()->get64bitIntegralValueAsUnsigned()
    ...
    //
    if (usesUnchangedValueInLoopTest) exitVal += getDeltaOnBackEdge();
    ...
    int32_t diff =
        (int32_t)exitVal - (int32_t)entryVal +
    ...
    // say how about looking at the type signature and guessing the max value
    // based on the return type?
    // TODO: do above
  }

  if (trace ||
      (comp()->getDebug() && comp()->getOptions()->getAnyOption(
                                 TR_TraceAll)))  // always dump the info
  {
    comp()->incVisitCount();
...
TR::Node *TR_PrimaryInductionVariable::getExitValue() {
  // TODO: for internal pointers style variables we do not need to do this
  // computation, the loops should always be well formed in which case we can
  // return the exitBound (or some function thereof)
  int32_t iterCount = getIterationCount();
  if (iterCount > 0 && getEntryValue() &&
...
TR::Node *TR_DerivedInductionVariable::getExitValue() {
  ...
  // the piv and then returning entry + itercount * deltaOnBackEdge +
  // deltaOnExitEdge
  return 0;
}
...
int32_t TR_IVTypeTransformer::perform() {
  ...
  if (trace()) comp()->dumpMethodTrees("Trees before TR_IVTypeTransformation");
  ...
  _nodesInCycle = new (trStackMemory())
      TR_BitVector(_cfg->getNextNodeNumber(), trMemory(), stackAlloc);

  detectWhileLoops(whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst,
                   doWhileLoops, _rootStructure, true);

  if (whileLoops.isEmpty() && doWhileLoops.isEmpty()) {
    if (trace()) traceMsg(comp(), "No while or doWhile loops detected\n");
    return 1;
  }
  ...
  for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL;
    ...
    TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),
              "IVTypeTransformer, expecting natural loop");
...
void TR_IVTypeTransformer::changeIVTypeFromAddrToInt(
    TR_RegionStructure *natLoop) {
  auto cm = comp();
  if (!natLoop->isCanonicalizedLoop()) {
    if (trace())
      traceMsg(
          cm,
          "Not a canonicalized loop. Preheader is needed for transformation\n");
    return;
  }
  ...
  if (trace()) traceMsg(cm, "Looking at Loop %i\n", natLoop->getNumber());

  if (!exitEdge) {
    if (trace()) traceMsg(cm, "exitEdge is null\n");
    return;
  }
  if (it.getNext()) {
    if (trace())
  ...
  if (!backEdgeTT) {
    if (trace()) traceMsg(cm, "Couldn't find backedge\n");
    return;
  }
  auto backEdgeIfNode = backEdgeTT->getNode();
  if (!(backEdgeIfNode->getOpCode().isIf() &&
        backEdgeIfNode->getChild(0)->getOpCode().isRef())) {
    if (trace()) traceMsg(cm, "Backedge is not a if address test node\n");
    return;
  }

  TR::SymbolReference *firstChildSymRef =
      findComparandSymRef(backEdgeIfNode->getChild(0));
  TR::SymbolReference *secondChildSymRef =
      findComparandSymRef(backEdgeIfNode->getChild(1));
  ...
  if (!astoreNode) {
    if (trace()) traceMsg(cm, "Couldn't find astore near back-edge\n");
    return;
  }
  ...
  while (!SGNQueue.IsEmpty()) {
    if (trace()) {
      ...
      QueueOf<TR_StructureSubGraphNode *, TR::Allocator>::Cursor cursor(
          SGNQueue);
    ...
    if (visitedSGN[SGNode->getNumber()]) continue;
    visitedSGN[SGNode->getNumber()] = 1;
    auto structure = SGNode->getStructure();
    if (!structure)  // null structure could mean invalidated structure... let's
    ...
    if (auto region = structure->asRegion()) {
      SGNode = region->getEntry();  // Look through inner loops as well. TODO:
                                    // Memoization
    } else if (auto block = structure->asBlock()) {
      auto trblock = block->getBlock();
      auto exitTT = trblock->getEntry()->getExtendedBlockExitTreeTop();
      for (tt = trblock->getEntry(); tt && tt != exitTT;
           tt = tt->getNextTreeTop()) {
        if (tt == astoreTT) continue;
        // If treetops are strictly ordered by side-effects, I should be ok just
        // checking the def set of the root node
        auto node = tt->getNode();
        if (node->getOpCodeValue() == TR::treetop) node = node->getChild(0);
        ...
        if (node->getOpCode().isLikeDef() &&
          ...
          if (useDefAliases[astoreNode->getSymbolReference()
                                ->getReferenceNumber()]) {
            if (trace())
    ...
    if (SGNode != exitSGN) {
      ...
      for (auto edge = sit.getFirst(); edge; edge = sit.getNext()) {
        ...
        if (visitedSGN[succ->getNumber()] == 0) SGNQueue.Add(succ);
      }
    }
  }

  /* Currently just returns if anchor is required, as other opts don't see
   * anchored IV as counted loop. If test child has rc>1, find if anchor is
   * required simple ex. is whether tmp=ptr comes before or after ptr++ do { tmp
   * = ptr  // means we have to anchor int test child before increment ptr++ }
   * while(tmp != end)
   *
   * do {
   *   tmp_i = i
   ...
   *
   * Walk forward from entry of EBB to see which is found first: referenced
   * child or incr TT. If latter, we can break anchor
   */
  auto cmpChildWithEndAddr =
      (astoreNode->getSymbolReference() == firstChildSymRef)
  ...
  if (!isIdentityExpr(ptrTestChild, astoreNode->getSymbolReference())) {
    if (trace())
      traceMsg(cm,
               "if test containing address IV is not identity expression\n");
    return;
  }
  auto itTT = backEdgeTT->getNextTreeTop()
                  ->getNode()
                  ->getBlock()
                  ->startOfExtendedBlock()
                  ->getEntry();
  ...
  if (trace())
    traceMsg(cm,
             "all ok! Transforming addr accesses to arrayRef form of base addr "
             "+ int\n");

  // Create copy symbol holding base address
  auto preheaderBlock = natLoop->getEntryBlock()->getPrevBlock();
  if (!performTransformation(cm,
                             "%s Initializing base and int index in preheader "
                             "block_%i of loop %i\n",
                             optDetailString(), preheaderBlock->getNumber(),
                             natLoop->getNumber()))
    return;

  _addrSymRef = astoreNode->getSymbolReference();
  _baseSymRef =
      cm->getSymRefTab()->createTemporary(cm->getMethodSymbol(), TR::Address);
  auto baseStoreTT = TR::TreeTop::create(
      cm, TR::Node::createStore(_baseSymRef, TR::Node::createWithSymRef(
                                                 TR::aload, 0, _addrSymRef)));
  _intIdxSymRef = cm->getSymRefTab()->createTemporary(
      cm->getMethodSymbol(),
      TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32);
  auto intIdxStoreTT = TR::TreeTop::create(
      cm, TR::Node::createStore(_intIdxSymRef, TR::Compiler->target.is64Bit()
                                                   ? TR::Node::lconst(0)
                                                   : TR::Node::iconst(0)));

  // Just insert base address copy store in the preheader block. Note that
  // canonicalizer is required.
  ...
  if (!performTransformation(cm, "%s Adding int increment tree\n",
                             optDetailString()))
    return;
  auto intIncrementTT = TR::TreeTop::create(
      cm,
      TR::Node::createStore(
          _intIdxSymRef,
          TR::Node::create(TR::Compiler->target.is64Bit() ? TR::ladd : TR::iadd,
                           2, TR::Node::createLoad(_intIdxSymRef),
                           TR::Compiler->target.is64Bit()
                               ? TR::Node::lconst(increment)
                               : TR::Node::iconst(increment))));
  ...
  if (!canBreakAnchor) {
    ...
    return;
    itTT->insertAfter(TR::TreeTop::create(
        cm, TR::Node::create(TR::treetop, 1, intNodeForBackEdgeTest)));
  }
  auto a2intOp = TR::ILOpCode::getProperConversion(
      TR::Address, TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32,
      false);
  TR::Node *newIf;
  if (astoreNode->getSymbolReference() ==
      firstChildSymRef)  // Match if children to original backedgeIf children
  {
    newIf = TR::Node::createif(
        getIntegralIfOpCode(backEdgeIfNode->getOpCodeValue(),
                            TR::Compiler->target.is64Bit()),
        intNodeForBackEdgeTest,
        TR::Node::create(
            TR::Compiler->target.is64Bit() ? TR::lsub : TR::isub, 2,
...
#if 1  // todo: is transforming loop body aload's desirable? Shouldn't be, it's
       // undoing ptr striding
  // Transform aload access in loop body
  if (!performTransformation(
          cm, "%s Transforming aload's in loop body to base+int form\n",
          optDetailString()))
    return;
  visitedSGN.Clear();
  SGNQueue.Push(natLoop->getEntry());
...
TR::ILOpCodes TR_IVTypeTransformer::getIntegralIfOpCode(TR::ILOpCodes ifacmp,
                                                        bool is64bit) {
  auto op = ifacmp;
  switch (ifacmp) {
    case TR::ifacmpeq:
...
void TR_IVTypeTransformer::replaceAloadWithBaseIndexInSubtree(TR::Node *node) {
  ...
  if (child && child->getOpCodeValue() == TR::aload &&
    ...
    auto arrayRef = TR::Node::recreateWithoutProperties(
        child, TR::Compiler->target.is64Bit() ? TR::aladd : TR::aiadd, 2,
        TR::Node::createLoad(_baseSymRef), TR::Node::createLoad(_intIdxSymRef));
  }
  for (int i = 0; i < node->getNumChildren(); i++) {
...
// Sets increment
bool TR_IVTypeTransformer::isInAddrIncrementForm(TR::Node *node,
                                                 int32_t &increment) {
  if (node->getVisitCount() == comp()->getVisitCount()) return false;
  node->setVisitCount(comp()->getVisitCount());
  TR::Node *load = NULL;
  ...
  if (node->getOpCodeValue() == TR::astore &&
      ...
      load->getOpCode().isLoadVar() && load->getOpCode().hasSymbolReference() &&
      load->getSymbolReference() == node->getSymbolReference() &&
      (incChild = node->getChild(0)->getChild(1)) &&
      incChild->getOpCode()
          .isLoadConst())  // TODO: non-const, loop-invariant expr
  {
    increment = incChild->getConst<int32_t>();
    return true;
  }
  if (trace()) traceMsg(comp(), "Not in address increment form\n");
  return false;
}

TR::SymbolReference *TR_IVTypeTransformer::findComparandSymRef(TR::Node *node) {
  if (node->getOpCode().hasSymbolReference()) return node->getSymbolReference();
