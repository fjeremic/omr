...
void TR_PrefetchInsertion::insertPrefetchInstructions()
{
  ...
  for (ArrayAccessInfo* aai = it.getFirst(); aai; aai = it.getNext()) {
    ...
    if (!skip
      && performTransformation(comp(), "%sInserting prefetch for array access %p in block_%d\n", OPT_DETAILS,
        aai->_aaNode, aai->_treeTop->getEnclosingBlock()->getNumber())) {
      ...
      deltaNode = createDeltaNode(
        aai->_addressNode->getSecondChild()->getFirstChild(), aai->_bivNode, aai->_biv->getDeltaOnBackEdge());
      ...
      typeNode = TR::Node::iconst(addressNode,
        (TR::Compiler->target.cpu.isZ() && foundStore && !disablePrefetchStore) ? (int32_t)PrefetchStore
                                                                                : (int32_t)PrefetchLoad);

      prefetchNode = TR::Node::createWithSymRef(TR::Prefetch, 4, 4, addressNode, offsetNode, sizeNode, typeNode,
      ...
      if (treeTop->getNode()->getOpCode().isBranch())
        treeTop = TR::TreeTop::create(
          comp(), treeTop->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, treeTop->getNode()->getFirstChild()));
...
TR::Node* TR_PrefetchInsertion::createDeltaNode(TR::Node* node, TR::Node* pivNode, int32_t deltaOnBackEdge)
{
  ...
  if (node->getOpCode().hasSymbolReference())
    newNode
      = TR::Node::createWithSymRef(node, node->getOpCodeValue(), node->getNumChildren(), node->getSymbolReference());
  else
...
void TR_PrefetchInsertion::examineNode(
  TR::TreeTop* treeTop, TR::Block* block, TR::Node* node, intptrj_t visitCount, TR_RegionStructure* loop)
{
  ...
  if (symbol && symbol->isArrayShadowSymbol() && node->getOpCodeValue() == TR::aloadi
    ...
    if ((secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub())
      ...
      if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift())
        ...
        if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
          mulConstBytes = (int64_t)((firstChild->getOpCode().isMul()) ? firstChild->getSecondChild()->getInt()
                                                                      : 2 << firstChild->getSecondChild()->getInt());
        else
          mulConstBytes = firstChild->getOpCode().isMul() ? firstChild->getSecondChild()->getLongInt()
      ...
      if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
        && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
          addConst = (int64_t)((firstChild->getOpCode().isAdd()) ? (int64_t)firstChild->getSecondChild()->getInt()
                                                                 : -firstChild->getSecondChild()->getInt());
        else
          addConst = firstChild->getOpCode().isAdd() ? firstChild->getSecondChild()->getLongInt()
      ...
      if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift())
        && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
          mulConst = (int64_t)((firstChild->getOpCode().isMul()) ? firstChild->getSecondChild()->getInt()
                                                                 : 2 << firstChild->getSecondChild()->getInt());
        else
          mulConst = firstChild->getOpCode().isMul() ? firstChild->getSecondChild()->getLongInt()
                                                     : 2 << firstChild->getSecondChild()->getLongInt();
