...
void
TR_PrefetchInsertion::collectLoops(TR_Structure* str)
{
  ...
  if (region->isNaturalLoop()) {
    ...
    if (region->getPrimaryInductionVariable() != NULL ||
      ...
      if (!region->getEntryBlock()->isCold()) {
        ...
        if (trace())
          traceMsg(
            comp(), "\tReject loop %d ==> cold loop\n", region->getNumber());
        return;
      }
    } else {
      if (trace())
        traceMsg(comp(),
                 "\tReject loop %d ==> no basic induction variable\n",
                 region->getNumber());
    }
  }
...
static bool
identicalSubTrees(TR::Node* node1, TR::Node* node2)
{
  if (node1 == node2)
    return true;
...
void
TR_PrefetchInsertion::insertPrefetchInstructions()
{
  ...
  for (ArrayAccessInfo* aai = it.getFirst(); aai; aai = it.getNext()) {
    ...
    if (!skip && performTransformation(
                   comp(),
                   "%sInserting prefetch for array access %p in block_%d\n",
                   OPT_DETAILS,
                   aai->_aaNode,
      ...
      requestOpt(
        OMR::treeSimplification, true, aai->_treeTop->getEnclosingBlock());
      ...
      addressNode = TR::Node::createWithSymRef(
        ...
        1,
        TR::Node::create(
          aai->_addressNode->getOpCodeValue(), 2, aai->_addressNode, deltaNode),
      ...
      typeNode = TR::Node::iconst(
        addressNode,
        (TR::Compiler->target.cpu.isZ() && foundStore && !disablePrefetchStore)
      ...
      if (treeTop->getNode()->getOpCode().isBranch())
        treeTop = TR::TreeTop::create(
          ...
          TR::Node::create(
            TR::treetop, 1, treeTop->getNode()->getFirstChild()));
...
TR::Node*
TR_PrefetchInsertion::createDeltaNode(TR::Node* node,
                                      TR::Node* pivNode,
                                      int32_t deltaOnBackEdge)
{
  ...
  if (node->getOpCode().hasSymbolReference())
    newNode = TR::Node::createWithSymRef(node,
                                         node->getOpCodeValue(),
                                         node->getNumChildren(),
...
//
void
TR_PrefetchInsertion::examineLoop(TR_RegionStructure* loop)
{
...
void
TR_PrefetchInsertion::examineNode(TR::TreeTop* treeTop,
                                  TR::Block* block,
                                  TR::Node* node,
                                  intptrj_t visitCount,
...
TR_PrimaryInductionVariable*
TR_PrefetchInsertion::getClosestPIV(TR::Block* block)
{
  TR_Structure* loop = block->getStructureOf()->getContainingLoop();
...
bool
TR_PrefetchInsertion::isBIV(TR::SymbolReference* symRef,
                            TR::Block* block,
                            TR_BasicInductionVariable*& biv)
{
