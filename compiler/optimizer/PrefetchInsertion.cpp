...
static bool identicalSubTrees(TR::Node* node1, TR::Node* node2)
{
    ...
    if (node1->getOpCode().isLoadVar() && node1->getSymbolReference() != node2->getSymbolReference())
        return false;
...
void TR_PrefetchInsertion::insertPrefetchInstructions()
{
    ...
    for (ArrayAccessInfo* aai = it.getFirst(); aai; aai = it.getNext()) {
        ...
        if (!skip && performTransformation(comp(), "%sInserting prefetch for array access %p in block_%d\n", OPT_DETAILS, aai->_aaNode, aai->_treeTop->getEnclosingBlock()->getNumber())) {
            ...
            for (TR::Block* blk = bIt.getFirst(); blk && !foundStore; blk = bIt.getNext()) {
                ...
                while ((currentTree != exitTree) && !foundStore) {
                    ...
                    if (node->getOpCode().isStoreIndirect()) {
                        // loopup the a[i];
                        if ((node->getFirstChild() == aai->_aaNode) || (node->getSymbol()->isArrayShadowSymbol() && node->getFirstChild()->getOpCode().isArrayRef() && node->getFirstChild()->getFirstChild() == aai->_aaNode))
                            foundStore = true;
                    }
            ...
            typeNode = TR::Node::iconst(addressNode, (TR::Compiler->target.cpu.isZ() && foundStore && !disablePrefetchStore) ? (int32_t)PrefetchStore : (int32_t)PrefetchLoad);
        ...
        if ((secondChild->getOpCode().isAdd() || secondChild->getOpCode().isSub()) && secondChild->getSecondChild()->getOpCode().isLoadConst()) {
            ...
            if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift()) && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
                if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
                    mulConstBytes = (int64_t)((firstChild->getOpCode().isMul()) ? firstChild->getSecondChild()->getInt() : 2 << firstChild->getSecondChild()->getInt());
                else
                    mulConstBytes = firstChild->getOpCode().isMul() ? firstChild->getSecondChild()->getLongInt() : 2 << firstChild->getSecondChild()->getLongInt();
            ...
            if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
                if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
                    addConst = (int64_t)((firstChild->getOpCode().isAdd()) ? (int64_t)firstChild->getSecondChild()->getInt() : -firstChild->getSecondChild()->getInt());
                else
                    addConst = firstChild->getOpCode().isAdd() ? firstChild->getSecondChild()->getLongInt() : -firstChild->getSecondChild()->getLongInt();
            ...
            if ((firstChild->getOpCode().isMul() || firstChild->getOpCode().isLeftShift()) && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
                if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
                    mulConst = (int64_t)((firstChild->getOpCode().isMul()) ? firstChild->getSecondChild()->getInt() : 2 << firstChild->getSecondChild()->getInt());
                else
                    mulConst = firstChild->getOpCode().isMul() ? firstChild->getSecondChild()->getLongInt() : 2 << firstChild->getSecondChild()->getLongInt();
            ...
            if (firstChild->getOpCode().isLoadDirect() && (((closestPIV = getClosestPIV(block)) && (firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbolReference() == closestPIV->getSymRef())) || (closestPIV == NULL && isBIV(firstChild->getSymbolReference(), block, biv)))) {
                if (closestPIV)
                    biv = closestPIV;
                ...
                if (isTreetopInLoop && ((stepInBytes > 0 && stepInBytes <= TR::Compiler->vm.heapTailPaddingSizeInBytes()) || (stepInBytes < 0 && -stepInBytes <= TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))) {
