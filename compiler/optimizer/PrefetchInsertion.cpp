...
TR_PrefetchInsertion::TR_PrefetchInsertion(TR::OptimizationManager *manager)
    : TR_LoopTransformer(manager), _arrayAccessInfos(manager->trMemory()) {}
...
int32_t TR_PrefetchInsertion::perform() {
  if (comp()->requiresSpineChecks()) {
    ...
    if (trace())
      traceMsg(comp(),
               "Spine checks required for array element accesses -- returning "
               "from prefetch insertion.\n");
    return 0;
  }
  ...
  if (!comp()->mayHaveLoops()) {
    if (trace())
      traceMsg(
          comp(),
          "Method does not have loops -- returning from prefetch insertion.\n");
    return 0;
  }
  ...
  if (!_arrayAccessInfos.isEmpty())
    ...
    dumpOptDetails(comp(),
                   "Prefetch insertion completed: no qualifying loops found\n");
...
void TR_PrefetchInsertion::collectLoops(TR_Structure *str) {
  ...
  if (region->isNaturalLoop()) {
    ...
    //
    if (region->getPrimaryInductionVariable() != NULL ||
        !region->getBasicInductionVariables().isEmpty()) {
...
void TR_PrefetchInsertion::insertPrefetchInstructions() {
  ...
  for (ArrayAccessInfo *aai = it.getFirst(); aai; aai = it.getNext()) {
    ...
    for (ArrayAccessInfo *aai1 = it1.getFirst(); !skip && aai1 != aai;
         aai1 = it1.getNext()) {
      TR::Block *block1 = aai1->_treeTop->getEnclosingBlock();
      if (block == block1 &&
          identicalSubTrees(aai->_addressNode, aai1->_addressNode))
        skip = true;
    }
    for (ArrayAccessInfo *aai1 = it1.getNext(); !skip && aai1;
         aai1 = it1.getNext()) {
      TR::Block *block1 = aai1->_treeTop->getEnclosingBlock();
      TR_Structure *loop1 = block1->getStructureOf()->getContainingLoop();
      if (loop == loop1 && block != block1 &&
          identicalSubTrees(aai->_addressNode, aai1->_addressNode))
        skip = true;
    }
    if (!skip &&
        performTransformation(
            comp(), "%sInserting prefetch for array access %p in block_%d\n",
            OPT_DETAILS, aai->_aaNode,
            aai->_treeTop->getEnclosingBlock()->getNumber())) {
      // Enable tree simplification on this block to normalize any address
      // computation so we can produce an optimal instruction sequence during
      // instruction selection
      requestOpt(OMR::treeSimplification, true,
                 aai->_treeTop->getEnclosingBlock());

      TR::Node *prefetchNode, *addressNode, *deltaNode, *offsetNode, *sizeNode,
          *typeNode;

      // First child -- base address
      deltaNode =
          createDeltaNode(aai->_addressNode->getSecondChild()->getFirstChild(),
                          aai->_bivNode, aai->_biv->getDeltaOnBackEdge());
      addressNode = TR::Node::createWithSymRef(
          TR::aloadi, 1, 1,
          TR::Node::create(aai->_addressNode->getOpCodeValue(), 2,
                           aai->_addressNode, deltaNode),
      ...
      // Walk all blocks in the loop and search for storei whose first child
      // includes a[i].
      ...
      for (TR::Block *blk = bIt.getFirst(); blk && !foundStore;
           blk = bIt.getNext()) {
      ...
      typeNode =
          TR::Node::iconst(addressNode, (TR::Compiler->target.cpu.isZ() &&
                                         foundStore && !disablePrefetchStore)
                                            ? (int32_t)PrefetchStore
                                            : (int32_t)PrefetchLoad);

      prefetchNode = TR::Node::createWithSymRef(
          TR::Prefetch, 4, 4, addressNode, offsetNode, sizeNode, typeNode,
...
TR::Node *TR_PrefetchInsertion::createDeltaNode(TR::Node *node,
                                                TR::Node *pivNode,
                                                int32_t deltaOnBackEdge) {
  if (node == pivNode) {
  ...
  if (node->getOpCode().hasSymbolReference())
    newNode = TR::Node::createWithSymRef(node, node->getOpCodeValue(),
                                         node->getNumChildren(),
                                         node->getSymbolReference());
  else
    newNode =
        TR::Node::create(node, node->getOpCodeValue(), node->getNumChildren());

  for (intptrj_t i = 0; i < node->getNumChildren(); i++) {
    newNode->setAndIncChild(
        i, createDeltaNode(node->getChild(i), pivNode, deltaOnBackEdge));
  }
  return newNode;
}
...
void TR_PrefetchInsertion::examineNode(TR::TreeTop *treeTop, TR::Block *block,
                                       TR::Node *node, intptrj_t visitCount,
  ...
  if (symbol && symbol->isArrayShadowSymbol() &&
    ...
    /* Pattern match for reference array accesses
       iaload                                           iaload
          aiadd                                            aladd
             aload <array> / iaload                           aload <array> /
       iaload isub/iadd                                        lsub/ladd
                (imul/ishl)                                      (lmul/lshl)
                   iload/iadd/isub/imul                             i2l    or
       lload/ladd/lsub/lmul iconst iload/iadd/isub/imul iconst lconst/iconst
                                                                 lconst
     */
    int64_t addConst = 0;
    int64_t mulConst = 1;
    ...
    if ((secondChild->getOpCode().isAdd() ||
      ...
      if ((firstChild->getOpCode().isMul() ||
        ...
        if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
          mulConstBytes =
              (int64_t)((firstChild->getOpCode().isMul())
      ...
      if ((firstChild->getOpCode().isAdd() ||
        ...
        if (firstChild->getSecondChild()->getOpCodeValue() == TR::iconst)
          addConst =
              (int64_t)((firstChild->getOpCode().isAdd())
      ...
      TR_PrimaryInductionVariable *closestPIV =
          NULL;  // argument loop is the most outer loop
      TR_BasicInductionVariable *biv =
          NULL;  // argument loop is the most outer loop
      if (firstChild->getOpCode().isLoadDirect() &&
          (((closestPIV = getClosestPIV(block)) &&
            (firstChild->getOpCode().hasSymbolReference() &&
             firstChild->getSymbolReference() == closestPIV->getSymRef())) ||
           (closestPIV == NULL &&
            isBIV(firstChild->getSymbolReference(), block, biv)))) {
        if (closestPIV) biv = closestPIV;

        int64_t stepInBytes =
            mulConstBytes *
            (mulConst * (int64_t)biv->getDeltaOnBackEdge() + addConst);
        TR_Structure *loop1 =
            treeTop->getEnclosingBlock()->getStructureOf()->getContainingLoop();
        bool isTreetopInLoop =
            (loop1 && (loop1->asRegion() == loop)) ? true : false;
        if (isTreetopInLoop &&
            ((stepInBytes > 0 &&
              stepInBytes <= TR::Compiler->vm.heapTailPaddingSizeInBytes()) ||
             (stepInBytes < 0 &&
              -stepInBytes <=
          ...
          //
          ArrayAccessInfo *aai =
              (ArrayAccessInfo *)trMemory()->allocateStackMemory(
                  sizeof(ArrayAccessInfo));
          ...
          if (trace())
            traceMsg(comp(),
                     "Found array access: node %p, access address node %p, biv "
                     "node %p\n",
...
TR_PrimaryInductionVariable *TR_PrefetchInsertion::getClosestPIV(
    TR::Block *block) {
  TR_Structure *loop = block->getStructureOf()->getContainingLoop();
...
bool TR_PrefetchInsertion::isBIV(TR::SymbolReference *symRef, TR::Block *block,
                                 TR_BasicInductionVariable *&biv) {
...
const char *TR_PrefetchInsertion::optDetailString() const throw() {
