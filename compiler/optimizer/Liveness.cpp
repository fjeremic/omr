...
TR_Liveness*
TR_Liveness::asLiveness()
{
  return this;
}

bool
TR_Liveness::supportsGenAndKillSets()
{
  return true;
}

int32_t
TR_Liveness::getNumberOfBits()
{
  return _liveVariableInfo->numLocals();
}

void
TR_Liveness::analyzeNode(TR::Node*, vcount_t, TR_BlockStructure*, TR_BitVector*)
{}
...
TR_Liveness::TR_Liveness(TR::Compilation* comp,
                         ...
                         bool splitLongs,
                         bool includeParms)
  : TR_BackwardUnionBitVectorAnalysis(comp,
                                      comp->getFlowGraph(),
                                      optimizer,
  ...
  , _liveVariableInfo(liveVariableInfo)
{
  ...
  if (liveVariableInfo == NULL)
    ...
    _liveVariableInfo = new (trStackMemory()) TR_LiveVariableInformation(
      comp, optimizer, rootStructure, splitLongs, includeParms, ignoreOSRUses);
  else
    _liveVariableInfo = liveVariableInfo;
...
bool
TR_Liveness::postInitializationProcessing()
{
  if (traceLiveness()) {
    for (int32_t i = 1; i < _numberOfNodes; ++i) {
...
void
TR_Liveness::initializeGenAndKillSetInfo()
{
...
void
TR_Liveness::analyzeTreeTopsInBlockStructure(TR_BlockStructure* blockStructure)
{
