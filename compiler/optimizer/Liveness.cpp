...
void TR_Liveness::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *,
                              TR_BitVector *) {}

TR_Liveness::TR_Liveness(TR::Compilation *comp, TR::Optimizer *optimizer,
                         TR_Structure *rootStructure, bool ignoreOSRUses,
                         TR_LiveVariableInformation *liveVariableInfo,
                         bool splitLongs, bool includeParms)
    : TR_BackwardUnionBitVectorAnalysis(comp, comp->getFlowGraph(), optimizer,
                                        comp->getOption(TR_TraceLiveness)),
      _liveVariableInfo(liveVariableInfo) {
  _traceLiveness = comp->getOption(TR_TraceLiveness);
  if (traceLiveness()) traceMsg(comp, "Starting Liveness analysis\n");
  ...
  if (liveVariableInfo == NULL)
    ...
    _liveVariableInfo = new (trStackMemory())
        TR_LiveVariableInformation(comp, optimizer, rootStructure, splitLongs,
...
void TR_Liveness::analyzeTreeTopsInBlockStructure(
