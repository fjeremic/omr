...
TR_ExpressionsSimplification::TR_ExpressionsSimplification(
...
int32_t TR_ExpressionsSimplification::perform() {
  ...
  if (trace()) {
    comp()->dumpMethodTrees(
        "Trees Before Performing Expression Simplification");
  }
...
int32_t TR_ExpressionsSimplification::perform(TR_Structure *str) {
  if (trace()) traceMsg(comp(), "Analyzing root Structure : %p\n", str);
  ...
  for (TR_StructureSubGraphNode *node = it.getCurrent(); node != 0;
       node = it.getNext()) {
    // Too strict
    /*
    ...
    toStructureSubGraphNode(edge->getFrom()); TR_BlockStructure *b =
    pred->getStructure()->asBlock(); if (b && pred->getSuccessors().size() ==
    1)) perform(node->getStructure());
       }
    */
  ...
  */
  ...
  if (region->isNaturalLoop() && !entryBlock->isCold() &&
    ...
    if (trace())
      traceMsg(comp(),
               "Found candidate non cold loop %p for expression elimination\n",
               region);
...
void TR_ExpressionsSimplification::findAndSimplifyInvariantLoopExpressions(
    TR_RegionStructure *region) {
  ...
  if (candidateBlocksList.getSize() > 1) {
    if (trace())
      traceMsg(comp(),
               "More than 1 blocks in the natural loop, need to remove "
               "uncertain blocks\n");

    removeUncertainBlocks(_currentRegion, &candidateBlocksList);

    if (candidateBlocksList.getSize() < 1) return;
  }
...
void TR_ExpressionsSimplification::simplifyInvariantLoopExpressions(
  ...
  if (!loopInfo) {
    if (trace())
      traceMsg(comp(),
               "Accurate loop info is not found, cannot carry out summation "
               "reduction\n");
  } else {
    if (trace())
      traceMsg(comp(),
               "Accurate loop info has been found, will try to carry out "
               "summation reduction\n");
    ...
    if (loopInfo->getBoundaryNode()) {
      if (trace())
        traceMsg(comp(),
                 "Variable iterations from node %p has not been handled\n",
                 loopInfo->getBoundaryNode());
    } else if (iters <= 0) {
      if (trace()) traceMsg(comp(), "Failed to determine iteration count\n");
    } else {
  ...
  for (TR::Block *currentBlock = blocks.getFirst(); currentBlock;
       currentBlock = blocks.getNext()) {
    if (trace())
      traceMsg(
          comp(),
          "Analyzing block #%d, which must be executed once per iteration\n",
  ...
  if (!_supportedExpressions) {
    _supportedExpressions = new (trStackMemory())
  ...
  for (TR::TreeTop *treeTop = treeTops.getFirst(); treeTop;
       treeTop = treeTops.getNext()) {
    if (trace())
    ...
    if (canReduceSummations) {
      usedCandidate = tranformSummationReductionCandidate(
          treeTop, loopInfo, &isPreheaderBlockInvalid);
    }
...
void TR_ExpressionsSimplification::setSummationReductionCandidates(
    TR::Node *node, TR::TreeTop *tt) {
  ...
  if (node->getOpCodeValue() != TR::istore
    ...
    if (trace())
      traceMsg(comp(),
               "Node %p: The opcode is not istore so not a summation reduction "
               "candidate\n",
               node);
    return;
  }
  ...
  if (opNode->getOpCodeValue() == TR::iadd ||
    ...
    if (firstNode->getOpCode().hasSymbolReference() &&
      ...
      if (!_currentRegion->isExprInvariant(secondNode)) {
        if (trace()) {
          ...
          if (secondNode->getNumChildren() == 1 &&
              secondNode->getOpCode().hasSymbolReference()) {
            TR_InductionVariable *indVar = _currentRegion->findMatchingIV(
                secondNode->getSymbolReference());
            if (indVar) {
      ...
      _candidateTTs->add(tt);
               ...
               opNode->getReferenceCount() == 1 &&
               secondNode->getReferenceCount() == 1 &&
    ...
    if (opNode->getFirstChild()->getOpCode().hasSymbolReference() &&
        node->getSymbolReference() ==
            opNode->getFirstChild()->getSymbolReference() &&
        opNode->getReferenceCount() == 1 &&
        opNode->getFirstChild()->getReferenceCount() == 1 &&
        (opNode->getOpCodeValue() == TR::ineg ||
         _currentRegion->isExprInvariant(opNode->getSecondChild())))
      _candidateTTs->add(tt);
    else if (opNode->getOpCodeValue() == TR::ixor &&
             opNode->getSecondChild()->getOpCode().hasSymbolReference() &&
             node->getSymbolReference() ==
                 opNode->getSecondChild()->getSymbolReference() &&
             opNode->getReferenceCount() == 1 &&
             opNode->getSecondChild()->getReferenceCount() == 1 &&
             _currentRegion->isExprInvariant(opNode->getFirstChild()))
      _candidateTTs->add(tt);
  }
}

void TR_ExpressionsSimplification::setStoreMotionCandidates(TR::Node *node,
                                                            TR::TreeTop *tt) {
  // Must be a store node, of any type but not holding a monitor object
  //
  if (node->getOpCode().isStore() && !node->getSymbol()->isStatic() &&
      !node->getSymbol()->holdsMonitoredObject()) {
    if (trace())
      traceMsg(
          comp(),
          "Node %p: The opcode is a non-static, non-monitor object store\n",
          node);
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
      if (!_currentRegion->isExprInvariant(node->getChild(i))) {
        if (trace())
          traceMsg(comp(),
                   "Node %p: The store is not loop-invariant due to child %p\n",
    ...
    if (trace()) {
      traceMsg(comp(),
               "Node %p: The store's operands are all loop-invariant, adding "
               "candidate\n",
               node);
      traceMsg(
          comp(),
          "Node %p:   - value of isExprInvariant for the store itself is %s\n",
...
bool TR_ExpressionsSimplification::tranformSummationReductionCandidate(
  ...
  if (expNode) {
    ...
    if (loopInfo->getNumIterations() > 0 ||  // make sure that the loop is going
                                             // to be executed at least once
        _currentRegion
            ->isCanonicalizedLoop())  // or that the loop is canonicalized, in
                                      // which case the preheader is
    {  // executed in its first iteration and is protected.
      if (performTransformation(
              comp(), "%sMove out loop-invariant node [%p] to block_%d\n",
...
void TR_ExpressionsSimplification::tranformStoreMotionCandidate(
    TR::TreeTop *treeTop, bool *isPreheaderBlockInvalid) {
  TR::Node *node = treeTop->getNode();

  TR_ASSERT(node->getOpCode().isStore() && !node->getSymbol()->isStatic() &&
                !node->getSymbol()->holdsMonitoredObject(),
            "node %p was expected to be a non-static non-monitored object "
            "store and was not.",
            node);

  // this candidate should be valid, either direct or indirect

  if (trace()) comp()->getDebug()->print(comp()->getOutFile(), node, 0, true);
  ...
  // Earlier post-dominance test ensures that the loop is executed as least
  // once, or is canonicalized. but to be safe we still perform on canonicalized
  // loops only.
  if (_currentRegion
          ->isCanonicalizedLoop())  // make sure that the loop is canonicalized,
                                    // in which case the preheader is
  {  // executed in its first iteration and is protected.
    if (performTransformation(
            comp(), "%sMove out loop-invariant store [%p] to block_%d\n",
            OPT_DETAILS, node, preheaderBlock->getNumber())) {
      TR::Node *newNode = node->duplicateTree();
      transformNode(newNode, preheaderBlock);
      TR::TransformUtil::removeTree(comp(), treeTop);
    ...
    if (trace()) traceMsg(comp(), "No canonicalized loop for this candidate\n");
  }
}
...
void TR_ExpressionsSimplification::invalidateCandidates() {
  ...
  for (TR::Block *currentBlock = blocks.getFirst(); currentBlock;
    ...
    while (tt != exitTreeTop) {
      ...
      if (trace()) traceMsg(comp(), "Looking at treeTop [%p]\n", currentNode);
...
void TR_ExpressionsSimplification::removeUnsupportedCandidates() {
  ...
  for (TR::TreeTop *candidateTT = candidateTTs.getFirst(); candidateTT;
       candidateTT = candidateTTs.getNext()) {
    TR::Node *candidate = candidateTT->getNode();
    if (!_supportedExpressions->get(candidate->getGlobalIndex())) {
      if (trace())
        traceMsg(comp(),
                 "Removing candidate %p which is unsupported or has "
...
void TR_ExpressionsSimplification::removeCandidate(TR::Node *node,
                                                   TR::TreeTop *tt) {
  ...
  for (TR::TreeTop *candidateTT = candidateTTs.getFirst(); candidateTT;
    ...
    if (tt != candidateTT && node->getOpCode().hasSymbolReference() &&
        candidateTT->getNode()->mayKill(true).contains(
            node->getSymbolReference(), comp())) {
      if (trace())
        traceMsg(comp(),
                 "Removing candidate %p which has aliases in the loop\n",
                 candidateTT->getNode());
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    ...
    // candidates child expressions must be invariant and supported. Here we
    // determine if they are supported.
  ...
  if (hasSupportedChildren &&
      isSupportedNodeForExpressionSimplification(node)) {
    _supportedExpressions->set(node->getGlobalIndex());
  } else {
    if (trace())
      traceMsg(comp(), "  Node %p is unsupported expression because %s\n", node,
               !hasSupportedChildren ? "it has unsupported children"
                                     : "it is itself unsupported");
  }
}

bool TR_ExpressionsSimplification::isSupportedNodeForExpressionSimplification(
    TR::Node *node) {
  // types of stores that may be supported by loop invariant store motion
  bool isSupportedStoreNode = node->getOpCode().isStore();
  return TR_LocalAnalysis::isSupportedNodeForFunctionality(
      node, comp(), NULL, isSupportedStoreNode);
}
...
void TR_ExpressionsSimplification::removeUncertainBlocks(
  ...
  for (TR::Block *block = blocks.getFirst(); block; block = blocks.getNext()) {
    ...
    if (!(cfgNode->getExceptionSuccessors().empty()) ||
        blockHasCalls(block, comp())) {
      if (trace())
        traceMsg(comp(),
                 "An exception can be thrown from block_%d. Removing all the "
                 "blocks, since we cannot know the number of iterations.\n",
                 block->getNumber());
  ...
  if (postDominators.isValid()) {
    ...
    for (TR::Block *block = blocks.getFirst(); block;
         block = blocks.getNext()) {
      if (postDominators.dominates(block, entryBlock) == 0) {
        candidateBlocksList->remove(block);
        if (trace())
          traceMsg(comp(),
                   "Block_%d is not guaranteed to be executed at least once. "
    ...
    if (trace())
      traceMsg(comp(),
               "There is no post dominators information. Removing all the "
               "blocks.\n");
    for (TR::Block *block = blocks.getFirst(); block;
         block = blocks.getNext()) {
      candidateBlocksList->remove(block);
...
TR_ExpressionsSimplification::findLoopInfo(TR_RegionStructure *region) {
  ...
  if (!exitNode->getStructure()->asBlock()) {
    if (trace()) traceMsg(comp(), "The exit block can't be found\n");
    return 0;
  }
  ...
  if (!lastTreeInExitBlock->getOpCode().isBranch()) {
    if (trace()) traceMsg(comp(), "The branch node couldn't be found\n");
    return 0;
  }

  if (lastTreeInExitBlock->getNumChildren() < 2) {
    if (trace()) traceMsg(comp(), "The branch node has less than 2 children\n");
    return 0;
  }
  ...
  if (!firstChildOfLastTree->getOpCode().hasSymbolReference()) {
    if (trace())
      traceMsg(comp(),
               "The branch node's first child node %p - its opcode does not "
               "have a symbol reference\n",
  ...
  if (!indVar->getIncr()->asIntConst()) {
    if (trace()) traceMsg(comp(), "Increment is not a constant\n");
    return 0;
  }
  ...
  if (firstChildOfLastTree->getReferenceCount() > 1) {
    ...
    while (cursorTreeTopInExitBlock != exitTreeTopInExitBlock) {
      ...
      if (checkForLoad(cursorNode, firstChildOfLastTree)) loadSeen = true;
  ...
  switch (lastTreeInExitBlock->getOpCodeValue()) {
    ...
    case TR::ificmpge: {
      ...
      if (branchDest->getNumber() != exitTarget) equals = !equals;

      if (!(indVar->getEntry() && indVar->getEntry()->asIntConst())) {
        if (trace()) traceMsg(comp(), "Entry value is not a constant\n");
        return 0;
      }
      ...
      return new (trStackMemory())
          LoopInfo(bound, lowerBound, upperBound, increment, equals);
    }

    default:
      if (trace()) traceMsg(comp(), "The condition has not been implemeted\n");
      return 0;
  }
...
bool TR_ExpressionsSimplification::checkForLoad(TR::Node *node,
                                                TR::Node *load) {
  if (node->getVisitCount() == _visitCount) return false;

  node->setVisitCount(_visitCount);

  if (node == load) return true;

  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    if (checkForLoad(node->getChild(i), load)) return true;
  }

  return false;
}

TR::Node *TR_ExpressionsSimplification::iaddisubSimplifier(
    TR::Node *invariantNode, LoopInfo *loopInfo) {
  TR_ASSERT_FATAL(loopInfo->getBoundaryNode() == NULL,
                  "iteration count must be constant for loop %d",
                  _currentRegion->getNumber());

  int32_t iters = loopInfo->getNumIterations();
  TR_ASSERT_FATAL(iters > 0,
                  "iteration count (%d) must be known and positive for loop %d",
                  iters, _currentRegion->getNumber());

  return TR::Node::create(
      invariantNode, TR::imul, 2, invariantNode->duplicateTree(),
      TR::Node::create(invariantNode, TR::iconst, 0, iters));
}

TR::Node *TR_ExpressionsSimplification::ixorinegSimplifier(TR::Node *node,
                                                           LoopInfo *loopInfo,
                                                           bool *removeOnly) {
  TR::Node *newNode = 0;
  *removeOnly = false;
  ...
  if (loopInfo->getBoundaryNode()) {
    if (trace())
      traceMsg(comp(),
               "Loop has a non constant boundary, but this case is not taken "
               "care of\n");
  } else {
...
TR::Block *TR_ExpressionsSimplification::findPredecessorBlock(
    TR::CFGNode *entryNode) {
  if (!(entryNode->getPredecessors().size() == 2)) return 0;

  TR::Block *block = 0;

  for (auto edge = entryNode->getPredecessors().begin();
       edge != entryNode->getPredecessors().end(); ++edge) {
...
void TR_ExpressionsSimplification::transformNode(TR::Node *srcNode,
                                                 TR::Block *dstBlock) {
...
const char *TR_ExpressionsSimplification::optDetailString() const throw() {
