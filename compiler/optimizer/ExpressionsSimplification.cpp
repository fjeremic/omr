...
void TR_ExpressionsSimplification::setSummationReductionCandidates(TR::Node* node, TR::TreeTop* tt)
{
  ...
  if (opNode->getOpCodeValue() == TR::iadd || opNode->getOpCodeValue() == TR::isub) {
    ...
    if (opNode->getFirstChild()->getOpCode().hasSymbolReference()
      && node->getSymbolReference() == opNode->getFirstChild()->getSymbolReference() && opNode->getReferenceCount() == 1
...
{
  TR_ASSERT_FATAL(
    loopInfo->getBoundaryNode() == NULL, "iteration count must be constant for loop %d", _currentRegion->getNumber());
  ...
  return TR::Node::create(
    invariantNode, TR::imul, 2, invariantNode->duplicateTree(), TR::Node::create(invariantNode, TR::iconst, 0, iters));
}
