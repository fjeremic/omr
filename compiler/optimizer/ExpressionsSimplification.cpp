...
void TR_ExpressionsSimplification::findAndSimplifyInvariantLoopExpressions(
  ...
  if (candidateBlocksList.getSize() > 1) {
    if (trace())
      traceMsg(comp(), "More than 1 blocks in the natural loop, need to remove "
                       "uncertain blocks\n");
...
void TR_ExpressionsSimplification::simplifyInvariantLoopExpressions(
  ...
  if (!loopInfo) {
    if (trace())
      traceMsg(comp(), "Accurate loop info is not found, cannot carry out "
                       "summation reduction\n");
  } else {
    if (trace())
      traceMsg(comp(), "Accurate loop info has been found, will try to carry "
                       "out summation reduction\n");
...
void TR_ExpressionsSimplification::removeUncertainBlocks(
  ...
  if (postDominators.isValid()) {
    ...
    if (trace())
      traceMsg(comp(), "There is no post dominators information. Removing all "
                       "the blocks.\n");
    for (TR::Block *block = blocks.getFirst(); block;
         block = blocks.getNext()) {
      candidateBlocksList->remove(block);
...
TR::Node *
TR_ExpressionsSimplification::iaddisubSimplifier(TR::Node *invariantNode,
                                                 LoopInfo *loopInfo) {
...
TR::Node *TR_ExpressionsSimplification::ixorinegSimplifier(TR::Node *node,
  ...
  *removeOnly = false;
  ...
  if (loopInfo->getBoundaryNode()) {
    if (trace())
      traceMsg(comp(), "Loop has a non constant boundary, but this case is not "
                       "taken care of\n");
  } else {
