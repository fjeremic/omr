...
void
TR_ExpressionsSimplification::findAndSimplifyInvariantLoopExpressions(
  TR_RegionStructure* region)
{
...
void
TR_ExpressionsSimplification::simplifyInvariantLoopExpressions(
  ListIterator<TR::Block>& blocks)
{
  ...
  if (!loopInfo) {
    ...
    if (loopInfo->getBoundaryNode()) {
      ...
      if (trace())
        traceMsg(
          comp(), "Natural Loop %p will run %d times\n", _currentRegion, iters);
    }
  }
...
void
TR_ExpressionsSimplification::setSummationReductionCandidates(TR::Node* node,
                                                              TR::TreeTop* tt)
{
        ...
        if (trace()) {
          ...
          if (secondNode->getNumChildren() == 1 &&
            ...
            TR_InductionVariable* indVar =
              _currentRegion->findMatchingIV(secondNode->getSymbolReference());
            if (indVar) {
...
void
TR_ExpressionsSimplification::setStoreMotionCandidates(TR::Node* node,
                                                       TR::TreeTop* tt)
{
...
bool
TR_ExpressionsSimplification::tranformSummationReductionCandidate(
  TR::TreeTop* treeTop,
  LoopInfo* loopInfo,
  bool* isPreheaderBlockInvalid)
{
  ...
  if (expNode) {
    ...
    if (loopInfo->getNumIterations() >
          0 || // make sure that the loop is going to be executed at least once
        _currentRegion
    ...
    { // executed in its first iteration and is protected.
      if (performTransformation(
            comp(),
            "%sMove out loop-invariant node [%p] to block_%d\n",
            OPT_DETAILS,
            node,
...
void
TR_ExpressionsSimplification::tranformStoreMotionCandidate(
  TR::TreeTop* treeTop,
  bool* isPreheaderBlockInvalid)
{
  TR::Node* node = treeTop->getNode();
  ...
  // loops only.
  if (_currentRegion
        ->isCanonicalizedLoop()) // make sure that the loop is canonicalized, in
                                 // which case the preheader is
  { // executed in its first iteration and is protected.
    if (performTransformation(
          comp(),
          "%sMove out loop-invariant store [%p] to block_%d\n",
          OPT_DETAILS,
          node,
          preheaderBlock->getNumber())) {
      TR::Node* newNode = node->duplicateTree();
...
TR_ExpressionsSimplification::removeCandidate(TR::Node* node, TR::TreeTop* tt)
{
  ...
  if (hasSupportedChildren &&
    ...
    if (trace())
      traceMsg(comp(),
               "  Node %p is unsupported expression because %s\n",
               node,
...
bool
TR_ExpressionsSimplification::isSupportedNodeForExpressionSimplification(
  TR::Node* node)
{
...
static bool
examineNode(TR::Node* node, intptrj_t visitCount)
{
...
static bool
blockHasCalls(TR::Block* block, TR::Compilation* comp)
{
  ...
  List<TR::Block>* candidateBlocksList)
{
  ...
  if (trace())
    traceMsg(comp(),
             "Number of blocks %d, entry block number %d\n",
  ...
  if (postDominators.isValid()) {
    ...
    if (trace())
      traceMsg(
        comp(),
        "There is no post dominators information. Removing all the blocks.\n");
    for (TR::Block* block = blocks.getFirst(); block;
         block = blocks.getNext()) {
      candidateBlocksList->remove(block);
      if (trace())
        traceMsg(
          comp(), "Block_%d is removed from the list\n", block->getNumber());
    }
  }
}
...
TR::Node*
TR_ExpressionsSimplification::ixorinegSimplifier(TR::Node* node,
                                                 LoopInfo* loopInfo,
                                                 bool* removeOnly)
{
  TR::Node* newNode = 0;
...
TR_ExpressionsSimplification::findPredecessorBlock(TR::CFGNode* entryNode)
{
  ...
  for (auto edge = entryNode->getPredecessors().begin();
       edge != entryNode->getPredecessors().end();
       ++edge) {
...
void
TR_ExpressionsSimplification::transformNode(TR::Node* srcNode,
                                            TR::Block* dstBlock)
{
