...
TR::Optimization*
OMR::LocalCSE::create(TR::OptimizationManager* manager)
{
  return new (manager->allocator()) TR::LocalCSE(manager);
}

bool
OMR::LocalCSE::shouldCopyPropagateNode(TR::Node* parent,
                                       TR::Node* node,
                                       int32_t childNum,
                                       TR::Node* storeNode)
{
  ...
  if (_numCopyPropagations >= MAX_COPY_PROP) {
    if (trace())
      traceMsg(comp(),
               "z^z : _numCopyPropagations %d >= max %d\n",
...
bool
OMR::LocalCSE::shouldCommonNode(TR::Node* parent, TR::Node* node)
{
  return isTreetopSafeToCommon();
}

TR::Node*
getRHSOfStoreDefNode(TR::Node* storeNode)
{
  int32_t childAdjust = (storeNode->getOpCode().isWrtBar()) ? 2 : 1;
  int32_t maxChild = storeNode->getNumChildren() - childAdjust;
...
TR::Node*
OMR::LocalCSE::getNode(TR::Node* node)
{
...
bool
OMR::LocalCSE::doExtraPassForVolatiles()
{
...
OMR::LocalCSE::perform()
{
  ...
  for (tt = comp()->getStartTree(); tt; tt = exitTreeTop->getNextTreeTop()) {
    ...
    if (doExtraPassForVolatiles()) {
      if (trace())
        traceMsg(comp(),
                 "LocalCSE entering 2 pass mode for volatile elimination - "
                 "pass 1 for volatiles ONLY\n");
      _volatileState = VOLATILE_ONLY;
      transformBlock(tt, exitTreeTop);
      if (trace())
        traceMsg(comp(),
                 "LocalCSE volatile only pass 1 complete - pass 2 for "
                 "non-volatiles ONLY\n");
...
int32_t
OMR::LocalCSE::performOnBlock(TR::Block* block)
{
  if (block->getEntry()) {
    _volatileState = VOLATILE_AND_NON_VOLATILE;
    if (doExtraPassForVolatiles()) {
...
void
OMR::LocalCSE::prePerformOnBlocks()
{
  TR::Region& stackRegion = comp()->trMemory()->currentStackRegion();
  _storeMap = new (stackRegion)
    StoreMap((StoreMapComparator()), StoreMapAllocator(stackRegion));
...
void
OMR::LocalCSE::postPerformOnBlocks()
{
  _storeMap = NULL;
...
bool
OMR::LocalCSE::isFirstReferenceToNode(TR::Node* parent,
                                      int32_t index,
                                      TR::Node* node,
                                      vcount_t visitCount)
{
...
bool
OMR::LocalCSE::shouldTransformBlock(TR::Block* block)
{
  if (block->isCold())
    return false;
...
void
OMR::LocalCSE::transformBlock(TR::TreeTop* entryTree, TR::TreeTop* exitTree)
{
  ...
  _replacedNodesAsArray =
    (TR::Node**)trMemory()->allocateStackMemory(_numNodes * sizeof(TR::Node*));
  _replacedNodesByAsArray =
    (TR::Node**)trMemory()->allocateStackMemory(_numNodes * sizeof(TR::Node*));
  memset(_replacedNodesAsArray, 0, _numNodes * sizeof(TR::Node*));
  memset(_replacedNodesByAsArray, 0, _numNodes * sizeof(TR::Node*));

  _hashTable =
    new (stackMemoryRegion) HashTable(std::less<int32_t>(), stackMemoryRegion);
  _hashTableWithSyms =
    new (stackMemoryRegion) HashTable(std::less<int32_t>(), stackMemoryRegion);
  _hashTableWithCalls =
    new (stackMemoryRegion) HashTable(std::less<int32_t>(), stackMemoryRegion);
  _hashTableWithConsts =
    new (stackMemoryRegion) HashTable(std::less<int32_t>(), stackMemoryRegion);
  ...
  if (_treeBeingExamined && !rhsOfStoreDefNode->getOpCode().isLoadConst() &&
    ...
    if (cg()->traceBCDCodeGen() &&
      ...
      traceMsg(
        ...
        rhsOfStoreDefNode->getOpCode().getName(),
        rhsOfStoreDefNode);
...
void
OMR::LocalCSE::examineNode(TR::Node* node,
                           TR_BitVector& seenAvailableLoadedSymbolReferences,
                           TR::Node* parent,
                           int32_t childNum,
                           int32_t* nextLoadIndex,
                           bool* parentCanBeAvailable,
                           int32_t depth)
{
  if (depth > MAX_DEPTH) {
    comp()->failCompilation<TR::ExcessiveComplexity>(
      "scratch space in local CSE");
  }

  // If register pressure is high at this point, then nothing should be commoned
  // or copy propagated across this point To achieve that, we clear all the
  // commoning and copy propagation related information here
  ...
  if (symRef && node->getOpCode().isLoadVar()) {
    ...
    if (result != _storeMap->end() &&
        result->second->getSymbolReference()->getReferenceNumber() ==
          symRef->getReferenceNumber()) {
      // Base case: where symrefs match
      doCopyPropagationIfPossible(node,
                                  parent,
                                  childNum,
                                  result->second,
                                  symRef,
      ...
      for (auto itr = _storeMap->begin(), end = _storeMap->end(); itr != end;
        ...
        if (storeNode->getOpCode().isStoreIndirect() &&
            storeNode->getSymbolReference()->getSymbol()->getSize() ==
              symRef->getSymbol()->getSize() &&
            storeNode->getSymbolReference()->getSymbol()->getDataType() ==
          ...
          if (doCopyPropagationIfPossible(node,
                                          parent,
                                          childNum,
                                          storeNode,
                                          symRef,
  ...
  // being available. Essentially we check if ALL symbols in the expression are
  ...
  if (!(doneCommoning || doneCopyPropagation)) {
    makeNodeAvailableForCommoning(
      parent, node, seenAvailableLoadedSymbolReferences, parentCanBeAvailable);
  }
  ...
  // slow commoning and copy propagation info
  //
  if (node->getOpCode().isStore() || hasAliases) {
    ...
    if (trace())
      traceMsg(comp(),
               "\tnode %p isStore = %d or hasAliases = %d\n",
               node,
    ...
    if (hasAliases) {
      ...
      if (UseDefAliases.containsAny(_seenSymRefs, comp())) {
        ...
        // smaller set, Less than 500 nodes should not be too significant as to
        // which set to iterate over.
        if (storeNodesSize >= 500) {
          ...
          // is large and we can make up the cost of getting aliases by
          ...
          if (storeNodesSize < tmpAliases.elementCount()) {
            for (auto itr = _storeMap->begin(), end = _storeMap->end();
                 itr != end;) {
      ...
      if (UseDefAliases.containsAny(tmp, comp())) {
        ...
        // earlier loads should survive note that volatile processing already
        // handles killing symbols when necessary hence the else
    ...
    // for commoning) if the symbol is available now; otherwise there is nothing
    // to kill.
    //
  ...
  killAvailableExpressionsAtGCSafePoints(
    node, parent, seenAvailableLoadedSymbolReferences);
  ...
  if (verboseProcessing) {
    traceMsg(comp(),
             "  after n%dn [%p] seenAvailableLoadedSymbolReferences:",
             node->getGlobalIndex(),
             node);
    seenAvailableLoadedSymbolReferences.print(comp());
    traceMsg(comp(), "\n");
...
void
OMR::LocalCSE::doCommoningAgainIfPreviouslyCommoned(TR::Node* node,
                                                    TR::Node* parent,
                                                    int32_t childNum)
{
  for (int32_t i = 0; i < _nextReplacedNode; i++) {
    ...
    if (_replacedNodesAsArray[i] == node && shouldCommonNode(parent, node) &&
        performTransformation(comp(),
                              "%s   Local Common Subexpression Elimination "
                              "commoning node : %p again\n",
                              optDetailString(),
                              node)) {
...
void
OMR::LocalCSE::doCommoningIfAvailable(TR::Node* node,
                                      TR::Node* parent,
                                      int32_t childNum,
                                      bool& doneCommoning)
{
  ...
  if (availableExpression && (availableExpression != node) &&
      ...
      performTransformation(comp(),
                            "%s   Local Common Subexpression Elimination "
                            "commoning node : %p by available node : %p\n",
                            optDetailString(),
                            node,
                            availableExpression)) {
    if (!node->getOpCode().hasSymbolReference() ||
      ...
      if (parent != NULL) {
        ...
        if (node->getOpCode().isResolveOrNullCheck()) {
          ...
          optimizer()->prepareForNodeRemoval(node);
          for (int32_t index = 0;
               index < _treeBeingExamined->getNode()->getNumChildren();
               index++)
            _treeBeingExamined->getNode()
              ->getChild(index)
              ->recursivelyDecReferenceCount();
      ...
      if (trace())
        traceMsg(
          ...
          node->getGlobalIndex(),
          availableExpression->getGlobalIndex(),
          _volatileState);
...
bool
OMR::LocalCSE::doCopyPropagationIfPossible(TR::Node* node,
                                           TR::Node* parent,
                                           int32_t childNum,
                                           TR::Node* storeNode,
                                           TR::SymbolReference* symRef,
                                           ...
                                           bool& doneCopyPropagation)
{
  ...
  if ((!comp()->getOption(TR_MimicInterpreterFrameShape) ||
      ...
      performTransformation(
        comp(),
        "%s   Local Common Subexpression Elimination propagating local #%d in "
        "node : %p PARENT : %p from node %p\n",
        optDetailString(),
        symRef->getReferenceNumber(),
        node,
        parent,
    ...
    // potential memory overlap created by the optimizer (vs present from the
    // start) for in memory types (i.e. BCD and Aggregate)
    ...
    rhsOfStoreDefNode = replaceCopySymbolReferenceByOriginalIn(
      symRef /*, originalSymbolReference*/,
      storeNode,
      rhsOfStoreDefNode,
      node,
      parent,
...
//
void
OMR::LocalCSE::makeNodeAvailableForCommoning(
  TR::Node* parent,
  TR::Node* node,
  TR_BitVector& seenAvailableLoadedSymbolReferences,
  bool* canBeAvailable)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    if (!seenAvailableLoadedSymbolReferences.get(
          node->getSymbolReference()->getReferenceNumber())) {
...
//
void
OMR::LocalCSE::commonNode(TR::Node* parent,
                          int32_t childNum,
                          TR::Node* node,
...
//
TR::Node*
OMR::LocalCSE::replaceCopySymbolReferenceByOriginalIn(
  TR::SymbolReference*
        ...
        if (rhsOfStoreDefNode->getType().isBCD()) {
          if (defNode && defNode->getDataType() == TR::PackedDecimal &&
            ...
            TR_ASSERT(rhsOfStoreDefNode->getDataType() == TR::PackedDecimal,
                      ...
                      "defNode %p is pd\n",
                      rhsOfStoreDefNode,
                      rhsOfStoreDefNode->getDataType().toString(),
                      defNode);
...
void
OMR::LocalCSE::getNumberOfNodes(TR::Node* node)
{
  _numNodes++;
...
//
bool
OMR::LocalCSE::canBeAvailable(TR::Node* parent,
                              TR::Node* node,
                              TR_BitVector& seenAvailableLoadedSymbolReferences,
                              bool canBeAvailable)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    // volatile only phase This is because those are the only fields we can rely
    // on not changing and therefore we can common volatiles that are based on
    // an indirection chain of such final non volatiles (or autos or parms that
    // are not global by definition) Any non volatile field that could change is
    // a problem since it could be proven that a later volatile load based on an
    // indirection chain involving a non final, non volatile variable must be
    // different than an earlier volatile load based on the same indirection
    // chain.
    //
    if ((!seenAvailableLoadedSymbolReferences.get(
          node->getSymbolReference()->getReferenceNumber())) ||
...
//
bool
OMR::LocalCSE::isAvailableNullCheck(
  TR::Node* node,
  TR_BitVector& seenAvailableLoadedSymbolReferences)
{
  if (node->getOpCode().hasSymbolReference()) {
...
//
TR::Node*
OMR::LocalCSE::getAvailableExpression(TR::Node* parent, TR::Node* node)
{
  if (node->getOpCodeValue() == TR::NULLCHK) {
  ...
  for (auto it = range.first; it != range.second;) {
    ...
    if (areSyntacticallyEquivalent(other, node, &remove)) {
      if (trace())
        traceMsg(comp(),
                 "node %p is syntactically equivalent to other %p\n",
                 node,
                 other);
      return other;
    }
  ...
  if (node->hasPinningArrayPointer() && node->computeIsInternalPointer() &&
...
// loads should surivive This routine kills all prior volatile loads of the same
// (or aliased) sym ref before we add the current volatile load to the available
// expressions
//
bool
OMR::LocalCSE::killExpressionsIfVolatileLoad(
  TR::Node* node,
  TR_BitVector& seenAvailableLoadedSymbolReferences,
...
// in the subtree Note that bucket 0 specifically holds such values and this
// kill operation is therefore quite cheap (its done at many places as there are
// many unresolved (GC safe) points at count=0
//
void
OMR::LocalCSE::killAllAvailableExpressions()
{
...
void
OMR::LocalCSE::killAllInternalPointersBasedOnThisPinningArray(
  TR::SymbolReference* symRef)
{
  ListIterator<TR::Node> arrayRefNodesIt(_arrayRefNodes);
  TR::Node* arrayRefNode = arrayRefNodesIt.getFirst();
...
//
void
OMR::LocalCSE::killAvailableExpressions(int32_t symRefNum)
{
...
void
OMR::LocalCSE::killAvailableExpressionsUsingBitVector(HashTable* hashTable,
                                                      TR_BitVector& vec)
{
...
void
OMR::LocalCSE::killAvailableExpressionsUsingAliases(TR_BitVector& aliases)
{
...
void
OMR::LocalCSE::killAvailableExpressionsUsingAliases(
  TR_UseDefAliasSetInterface& UseDefAliases)
{
  TR_BitVector tmp(_availableLoadExprs);
...
void
OMR::LocalCSE::killAllDataStructures(
  TR_BitVector& seenAvailableLoadedSymbolReferences)
{
  _storeMap->clear();
...
// are aload, aiadd (or aladd) or that contain an aiadd (or an aladd).

void
OMR::LocalCSE::killAvailableExpressionsAtGCSafePoints(
  TR::Node* node,
  TR::Node* parent,
  TR_BitVector& seenAvailableLoadedSymbolReferences)
{
  ...
  if ((node->getOpCodeValue() == TR::MethodEnterHook) ||
    ...
    // -1) causing a crash when using the live local index to index into a bit
    // vector.
    //
    if (trace())
      traceMsg(
        comp(), "Node %p is detected as a method enter/exit point\n", node);
...
int32_t
OMR::LocalCSE::hash(TR::Node* parent, TR::Node* node)
{
  ...
  if (node->getOpCodeValue() == TR::aconst &&
      (!parent || !(parent->isTheVirtualGuardForAGuardedInlinedCall() &&
...
void
OMR::LocalCSE::addToHashTable(TR::Node* node, int32_t hashValue)
{
  if (node->getOpCode().isStore() ||
  ...
  if ((node->getOpCode().isArrayRef()) && cg()->supportsInternalPointers() &&
      (node->getFirstChild()->getOpCodeValue() == TR::aload) &&
      (node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto())) {
    _availablePinningArrayExprs.set(
      node->getFirstChild()->getSymbolReference()->getReferenceNumber());
...
void
OMR::LocalCSE::removeFromHashTable(HashTable* hashTable, int32_t hashValue)
{
  auto range = hashTable->equal_range(hashValue);
  hashTable->erase(range.first, range.second);
}
...
//
bool
OMR::LocalCSE::areSyntacticallyEquivalent(TR::Node* node1,
                                          TR::Node* node2,
                                          bool* remove)
{
...
//
void
OMR::LocalCSE::collectAllReplacedNodes(TR::Node* node, TR::Node* replacingNode)
{
  ...
  if (node->getReferenceCount() > 1) {
    ...
    if (trace())
      traceMsg(comp(),
               "Replaced node : %p Replacing node : %p\n",
               node,
               replacingNode);
...
rcount_t
OMR::LocalCSE::recursivelyIncReferenceCount(TR::Node* node)
{
  rcount_t count;
