  ...
  int32_t childNum, int32_t* nextLoadIndex, bool* parentCanBeAvailable, int32_t depth)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++)
    examineNode(
      node->getChild(i), seenAvailableLoadedSymbolReferences, node, i, nextLoadIndex, &nodeCanBeAvailable, depth + 1);
    ...
    if (result != _storeMap->end()
      ...
      for (auto itr = _storeMap->begin(), end = _storeMap->end(); itr != end; ++itr) {
        ...
        if (storeNode->getOpCode().isStoreIndirect()
          ...
          && allowNodeTypes(storeNode, node) && storeNode->getSymbolReference()->getOffset() == symRef->getOffset()
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (node->getOpCode().isStore() || hasAliases) {
    ...
    if (hasAliases) {
      ...
      if (UseDefAliases.containsAny(tmp, comp())) {
        ...
        if (alreadyKilledAtVolatileLoad) // we want to keep the last load of a particular sym ref alive but no earlier
  ...
  for (int32_t i = 0; i < _nextReplacedNode; i++) {
    ...
    if (_replacedNodesAsArray[i] == node && shouldCommonNode(parent, node)
      && performTransformation(
        comp(), "%s   Local Common Subexpression Elimination commoning node : %p again\n", optDetailString(), node)) {
...
void OMR::LocalCSE::doCommoningIfAvailable(TR::Node* node, TR::Node* parent, int32_t childNum, bool& doneCommoning)
{
  ...
  if (availableExpression && (availableExpression != node) && shouldCommonNode(parent, node)
    ...
    if (!node->getOpCode().hasSymbolReference() || (_volatileState == VOLATILE_ONLY && node->getSymbol()->isVolatile())
      || (_volatileState != VOLATILE_ONLY)) {
        ...
        if (node->getOpCode().isResolveOrNullCheck()) {
          ...
          if (node->getFirstChild()->getOpCode().isTreeTop()) {
            if ((comp()->useAnchors() && node->getFirstChild()->getOpCode().isStoreIndirect())) {
              ...
              TR_ASSERT(
                node->getFirstChild()->getReferenceCount() == 1, "Local CSE, treetop child of NULLCHK has other uses");
      ...
      if ((parent != NULL || !node->getOpCode().isResolveOrNullCheck())
        && !_simulatedNodesAsArray[node->getGlobalIndex()] && !node->getOpCode().isCase()
      ...
      if (trace())
        traceMsg(comp(), "Simulating commoning of node n%dn with n%dn - current mode %n\n", node->getGlobalIndex(),
          availableExpression->getGlobalIndex(), _volatileState);
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (symRef->getReferenceNumber() == copySymbolReference->getReferenceNumber()) {
      ...
      if (
        ...
        TR_ASSERT(convOp != TR::BadILOp, "BadIlOp for conversion from node %p (%s) to node %p (%s)", rhsOfStoreDefNode,
          rhsOfStoreDefNode->getOpCode().getName(), node, node->getOpCode().getName());
        ...
        if (rhsOfStoreDefNode->getType().isBCD()) {
          if (defNode && defNode->getDataType() == TR::PackedDecimal && defNode->getOpCode().isStore()
            ...
            dumpOptDetails(comp(),
              ...
              optDetailString(), defNode->getOpCode().getName(), defNode, origRhsOfStoreDefNode->getOpCode().getName(),
              origRhsOfStoreDefNode, parent->getOpCode().getName(), parent, rhsOfStoreDefNode->getOpCode().getName(),
              rhsOfStoreDefNode);
          }
          ...
          if (rhsOfStoreDefNode->getDecimalPrecision() != nodePrecision) {
            // If the copy propagation is going to change the size of the node being replaced then a size changing pdshl
            // must be prepended in case the new parent requires explicit widening (such as if the parent is a call).
            TR::Node* origRhsOfStoreDefNode = rhsOfStoreDefNode;
            ...
            rhsOfStoreDefNode->setReferenceCount(1);
            dumpOptDetails(comp(),
              "%sPrecision mismatch when propagating %s [" POINTER_PRINTF_FORMAT "] to %s [" POINTER_PRINTF_FORMAT
              "] so create new %s [" POINTER_PRINTF_FORMAT "]\n",
  ...
  TR::Node* parent, TR::Node* node, TR_BitVector& seenAvailableLoadedSymbolReferences, bool canBeAvailable)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    // common volatiles that are based on an indirection chain of such final non volatiles (or autos or parms that are
    // not global by definition) Any non volatile field that could change is a problem since it could be proven that a
    // later volatile load based on an indirection chain involving a non final, non volatile variable must be different
    // than an earlier volatile load based on the same indirection chain.
    //
    if ((!seenAvailableLoadedSymbolReferences.get(node->getSymbolReference()->getReferenceNumber()))
      || ((_volatileState == VOLATILE_ONLY) && !node->getSymbol()->isAutoOrParm() && !node->getSymbol()->isVolatile()
        && !node->getSymbol()->isFinal()))
      return false;
  ...
  if ((node->getOpCode().isArrayRef()) || (node->isGCSafePointWithSymRef() && comp()->getOptions()->realTimeGC())
    || (comp()->getOption(TR_EnableHCR)
      && ((node->getOpCodeValue() == TR::loadaddr)
        || ((node->getOpCodeValue() == TR::aloadi) && node->getSymbolReference()->getSymbol()->isClassObject()))))
    return 0;
