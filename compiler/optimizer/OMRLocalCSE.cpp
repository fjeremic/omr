...
{
    if (_treeBeingExamined && !rhsOfStoreDefNode->getOpCode().isLoadConst() && cg()->IsInMemoryType(rhsOfStoreDefNode->getType())) {
...
bool OMR::LocalCSE::allowNodeTypes(TR::Node* storeNode, TR::Node* node)
{
    if (storeNode->getDataType() == node->getDataType()) {
        return true;
    } else if (storeNode->getType().isIntegral() && node->getType().isAggregate() && storeNode->getSize() == node->getSize()) {
        return true;
    }
    return false;
}
...
void OMR::LocalCSE::examineNode(TR::Node* node, TR_BitVector& seenAvailableLoadedSymbolReferences, TR::Node* parent, int32_t childNum, int32_t* nextLoadIndex, bool* parentCanBeAvailable, int32_t depth)
{
    ...
    if (symRef && node->getOpCode().isLoadVar()) {
        ...
        if (result != _storeMap->end() && result->second->getSymbolReference()->getReferenceNumber() == symRef->getReferenceNumber()) {
            ...
            for (auto itr = _storeMap->begin(), end = _storeMap->end(); itr != end; ++itr) {
                ...
                if (storeNode->getOpCode().isStoreIndirect() && storeNode->getSymbolReference()->getSymbol()->getSize() == symRef->getSymbol()->getSize() && storeNode->getSymbolReference()->getSymbol()->getDataType() == symRef->getSymbol()->getDataType() && allowNodeTypes(storeNode, node) && storeNode->getSymbolReference()->getOffset() == symRef->getOffset()
#ifdef J9_PROJECT_SPECIFIC
                    && (!storeNode->getType().isBCD() || storeNode->getDecimalPrecision() == node->getDecimalPrecision())
#endif
                ) {
    ...
    // to be available.
    //
    if (!doneCopyPropagation && (((node->getOpCodeValue() == TR::NULLCHK) && (isAvailableNullCheck(node, seenAvailableLoadedSymbolReferences))) || (canBeAvailable(parent, node, seenAvailableLoadedSymbolReferences, nodeCanBeAvailable)) || (!_loadaddrAsLoad && node->getOpCodeValue() == TR::loadaddr))) {
...
void OMR::LocalCSE::doCommoningAgainIfPreviouslyCommoned(TR::Node* node, TR::Node* parent, int32_t childNum)
{
    for (int32_t i = 0; i < _nextReplacedNode; i++) {
        ...
        //
        if (_replacedNodesAsArray[i] == node && shouldCommonNode(parent, node) && performTransformation(comp(), "%s   Local Common Subexpression Elimination commoning node : %p again\n", optDetailString(), node)) {
...
void OMR::LocalCSE::doCommoningIfAvailable(TR::Node* node, TR::Node* parent, int32_t childNum, bool& doneCommoning)
{
    ...
    if (availableExpression && (availableExpression != node) && shouldCommonNode(parent, node) && performTransformation(comp(), "%s   Local Common Subexpression Elimination commoning node : %p by available node : %p\n", optDetailString(), node, availableExpression)) {
        if (!node->getOpCode().hasSymbolReference() || (_volatileState == VOLATILE_ONLY && node->getSymbol()->isVolatile()) || (_volatileState != VOLATILE_ONLY)) {
            ...
            if (parent != NULL) {
                ...
                if ((!parent->getOpCode().isResolveOrNullCheck() && (parent->getOpCodeValue() != TR::DIVCHK)) && ((parent->getOpCodeValue() != TR::compressedRefs) || (childNum != 0)))
            ...
            if ((parent != NULL || !node->getOpCode().isResolveOrNullCheck()) && !_simulatedNodesAsArray[node->getGlobalIndex()] && !node->getOpCode().isCase() && node->getReferenceCount() > 1) {
...
bool OMR::LocalCSE::doCopyPropagationIfPossible(TR::Node* node, TR::Node* parent, int32_t childNum, TR::Node* storeNode, TR::SymbolReference* symRef, vcount_t visitCount, bool& doneCopyPropagation)
{
    ...
    if ((!comp()->getOption(TR_MimicInterpreterFrameShape) || !comp()->areSlotsSharedByRefAndNonRef() || !symRef->getSymbol()->isAuto() || !symRef->getSymbol()->castToAutoSymbol()->isSlotSharedByRefAndNonRef()) && shouldCommonNode(parent, node) && isSafeToCommon && canAffordToIncreaseRegisterPressure() && (!node->getOpCode().hasSymbolReference() || node->getSymbolReference() != comp()->getSymRefTab()->findVftSymbolRef()) && (symRef->storeCanBeRemoved() || (!symRef->getSymbol()->isVolatile() && rhsOfStoreDefNode->getDataType() == TR::Float && (rhsOfStoreDefNode->getOpCode().isCall() || (rhsOfStoreDefNode->getOpCode().isLoadConst()) || rhsOfStoreDefNode->getOpCode().isLoadVar()))) && (!parent->getOpCode().isSpineCheck() || childNum != 0) && performTransformation(comp(), "%s   Local Common Subexpression Elimination propagating local #%d in node : %p PARENT : %p from node %p\n", optDetailString(), symRef->getReferenceNumber(), node, parent, storeNode)) {
    ...
    if (node->getOpCode().hasSymbolReference()) {
        if (!seenAvailableLoadedSymbolReferences.get(node->getSymbolReference()->getReferenceNumber())) {
            *canBeAvailable = false;
            ...
            if (node->getOpCode().isLoadVar() || node->getOpCode().isCheck() || node->getOpCode().isCall() || node->getOpCodeValue() == TR:: instanceof || ((node->getOpCodeValue() == TR::loadaddr) && (node->getSymbol()->isNotCollected() || node->getSymbol()->isAutoOrParm()))) {
        ...
        if (symRef->getReferenceNumber() == copySymbolReference->getReferenceNumber()) {
            ...
            if (
...
#endif
                node->getDataType() != rhsOfStoreDefNode->getDataType() && node->getSize() == rhsOfStoreDefNode->getSize()) {
                //sign mismatch
                ...
                if (rhsOfStoreDefNode->getType().isBCD()) {
                    if (defNode && defNode->getDataType() == TR::PackedDecimal && defNode->getOpCode().isStore() && defNode->mustCleanSignInPDStoreEvaluator()) {
...
bool OMR::LocalCSE::canBeAvailable(TR::Node* parent, TR::Node* node, TR_BitVector& seenAvailableLoadedSymbolReferences, bool canBeAvailable)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        //
        if ((!seenAvailableLoadedSymbolReferences.get(node->getSymbolReference()->getReferenceNumber())) || ((_volatileState == VOLATILE_ONLY) && !node->getSymbol()->isAutoOrParm() && !node->getSymbol()->isVolatile() && !node->getSymbol()->isFinal()))
            return false;

        if (comp()->getOption(TR_MimicInterpreterFrameShape) && comp()->areSlotsSharedByRefAndNonRef() && node->getSymbolReference()->getSymbol()->isAuto() && node->getSymbolReference()->getSymbol()->castToAutoSymbol()->isSlotSharedByRefAndNonRef())
            return false;
    }

    if (parent) {
        if (node->getOpCode().isCall() && (!node->getSymbol()->isMethod() || (node->getSymbol()->isMethod() && !node->getSymbol()->castToMethodSymbol()->isPureFunction())) && (parent->getOpCodeValue() == TR::treetop || parent->getOpCode().isResolveOrNullCheck()))
            return false;
    }
    ...
    while (i < numChildren) {
        if (node->getChild(i)->getReferenceCount() == 1) {
            if (node->getChild(i)->getOpCode().isArrayRef()) {
                if ((node->getChild(i)->getFirstChild()->getReferenceCount() == 1) || (node->getChild(i)->getSecondChild()->getReferenceCount() == 1))
                    return false;
            } else
                return false;
        }
...
TR::Node* OMR::LocalCSE::getAvailableExpression(TR::Node* parent, TR::Node* node)
{
    if (node->getOpCodeValue() == TR::NULLCHK) {
        for (int32_t i = 0; i < _numNullCheckNodes; i++) {
            if (!(_nullCheckNodesAsArray[i] == NULL)) {
                if ((_nullCheckNodesAsArray[i]->getSymbolReference()->getReferenceNumber() == node->getSymbolReference()->getReferenceNumber()) && (_nullCheckNodesAsArray[i]->getNullCheckReference() == node->getNullCheckReference()))
    ...
    if (node->hasPinningArrayPointer() && node->computeIsInternalPointer() && cg()->supportsInternalPointers() && (node->getFirstChild()->getOpCodeValue() == TR::aload) && (node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto()) && !_killedPinningArrayExprs.get(node->getFirstChild()->getSymbolReference()->getReferenceNumber())) {
        ...
        for (; arrayRefNode; arrayRefNode = arrayRefNodesIt.getNext()) {
            if ((arrayRefNode != node) && (arrayRefNode->getFirstChild() == node->getFirstChild()) && (arrayRefNode->getSecondChild() == node->getSecondChild())) {
...
void OMR::LocalCSE::killAllInternalPointersBasedOnThisPinningArray(TR::SymbolReference* symRef)
{
    ...
    TR::Node* arrayRefNode = arrayRefNodesIt.getFirst();
    for (; arrayRefNode; arrayRefNode = arrayRefNodesIt.getNext()) {
        if ((arrayRefNode->getNumChildren() > 0) && (arrayRefNode->getFirstChild()->getOpCodeValue() == TR::aload) && (arrayRefNode->getFirstChild()->getSymbolReference()->getSymbol()->isAuto()) && (arrayRefNode->getFirstChild()->getSymbolReference() == symRef)) {
...
{
    if ((node->getOpCode().isArrayRef()) || (node->isGCSafePointWithSymRef() && comp()->getOptions()->realTimeGC()) || (comp()->getOption(TR_EnableHCR) && ((node->getOpCodeValue() == TR::loadaddr) || ((node->getOpCodeValue() == TR::aloadi) && node->getSymbolReference()->getSymbol()->isClassObject()))))
        return 0;

    if (node->getOpCodeValue() == TR::aconst && (!parent || !(parent->isTheVirtualGuardForAGuardedInlinedCall() && parent->isProfiledGuard())))
        return 0;
...
void OMR::LocalCSE::addToHashTable(TR::Node* node, int32_t hashValue)
{
    ...
    if ((node->getOpCode().isArrayRef()) && cg()->supportsInternalPointers() && (node->getFirstChild()->getOpCodeValue() == TR::aload) && (node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto())) {
...
bool OMR::LocalCSE::areSyntacticallyEquivalent(TR::Node* node1, TR::Node* node2, bool* remove)
{
    ...
    for (int32_t i = 0; i < node1->getNumChildren(); i++) {
        ...
        if (child1 != child2) {
            ...
            } else if (node1->getOpCodeValue() == node2->getOpCodeValue() && node1->getOpCode().isCommutative() && node1->getNumChildren() == 2 && node2->getNumChildren() == 2 && getNode(node1->getFirstChild()) == getNode(node2->getSecondChild()) && getNode(node1->getSecondChild()) == getNode(node2->getFirstChild())) {
                return true;
            } else {
                return false;
            }
