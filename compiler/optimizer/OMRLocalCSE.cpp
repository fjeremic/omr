...
OMR::LocalCSE::LocalCSE(TR::OptimizationManager *manager)
    : TR::Optimization(manager), _storeMap(NULL), _arrayRefNodes(NULL) {
...
bool OMR::LocalCSE::shouldCopyPropagateNode(TR::Node *parent, TR::Node *node,
  ...
  if (node->getNumChildren() < maxChild) return false;
  ...
  for (int32_t k = 0; k < maxChild; k++) {
    if (storeNode->getChild(k) != node->getChild(k)) return false;
  }
...
TR::Node *OMR::LocalCSE::getNode(TR::Node *node) {
  if (_volatileState != VOLATILE_ONLY) return node;
...
void OMR::LocalCSE::postPerformOnBlocks() {
  ...
  if (_mayHaveRemovedChecks) requestOpt(OMR::catchBlockRemoval);
}
...
void OMR::LocalCSE::transformBlock(TR::TreeTop *entryTree,
                                   TR::TreeTop *exitTree) {
  if (!shouldTransformBlock(entryTree->getNode()->getBlock())) return;
  ...
  for (currentTree = entryTree->getNextRealTreeTop();
    ...
    if (node->getStoreNode()) numStores++;
    if (node->getOpCodeValue() == TR::NULLCHK) numNullChecks++;
  ...
  _hashTable = new (stackMemoryRegion)
      HashTable(std::less<int32_t>(), stackMemoryRegion);
  _hashTableWithSyms = new (stackMemoryRegion)
      HashTable(std::less<int32_t>(), stackMemoryRegion);
  _hashTableWithCalls = new (stackMemoryRegion)
      HashTable(std::less<int32_t>(), stackMemoryRegion);
  _hashTableWithConsts = new (stackMemoryRegion)
      HashTable(std::less<int32_t>(), stackMemoryRegion);
...
void OMR::LocalCSE::examineNode(
    TR::Node *node, TR_BitVector &seenAvailableLoadedSymbolReferences,
  ...
  if (trace()) traceMsg(comp(), "Examining node %p\n", node);

  if (!isFirstReferenceToNode(parent, childNum, node, visitCount)) {
    if (trace()) traceMsg(comp(), "\tNot first Reference to Node\n");
  ...
  if (symRef && node->getOpCode().isLoadVar()) {
    ...
    if (result != _storeMap->end() &&
      ...
      doCopyPropagationIfPossible(node, parent, childNum, result->second,
      ...
      for (auto itr = _storeMap->begin(), end = _storeMap->end(); itr != end;
        ...
        if (storeNode->getOpCode().isStoreIndirect() &&
          ...
          if (doCopyPropagationIfPossible(node, parent, childNum, storeNode,
                                          symRef, visitCount,
                                          doneCopyPropagation))
            break;
        }
  ...
  if (node->getOpCode().isStore() || hasAliases) {
        ...
        if (storeNodesSize >= 500) {
          ...
          if (storeNodesSize < tmpAliases.elementCount()) {
            ...
            while (bvi.hasMoreElements()) {
              ...
              if (result != _storeMap->end()) _storeMap->erase(sc);
            }
          }
        } else {
          for (auto itr = _storeMap->begin(), end = _storeMap->end();
    ...
    if (previouslyAvailable) {
      if (hasAliases) killAvailableExpressionsUsingAliases(UseDefAliases);
      if (node->getOpCode().isLikeDef()) killAvailableExpressions(symRefNum);
    }
  ...
  if (_isTreeTopNullCheck) {
    if (nullCheckReference != NULL) _inSubTreeOfNullCheckReference = false;
  }
...
bool OMR::LocalCSE::doCopyPropagationIfPossible(
  ...
  if (!shouldCopyPropagateNode(parent, node, childNum, storeNode)) return false;
  ...
  if (!shouldCommonNode(node, rhsOfStoreDefNode)) isSafeToCommon = false;
  ...
  if ((!comp()->getOption(TR_MimicInterpreterFrameShape) ||
      ...
      performTransformation(
          comp(),
          "%s   Local Common Subexpression Elimination propagating local #%d "
          "in node : %p PARENT : %p from node %p\n",
          optDetailString(), symRef->getReferenceNumber(), node, parent,
          storeNode)) {
...
void OMR::LocalCSE::makeNodeAvailableForCommoning(
  ...
  if (comp()->cg()->supportsLengthMinusOneForMemoryOpts() && parent &&
    ...
    if (parent->getLastChild() == node) return;
  }
  ...
  if (node->getOpCode().hasSymbolReference()) {
    if (!seenAvailableLoadedSymbolReferences.get(
      ...
      *canBeAvailable = false;
      if (_inSubTreeOfNullCheckReference) _isAvailableNullCheck = false;
...
void OMR::LocalCSE::commonNode(TR::Node *parent, int32_t childNum,
  ...
  if (replacingNode->getReferenceCount() == 0)
    ...
    replacingNode->incReferenceCount();
  if (node->getReferenceCount() <= 1) optimizer()->prepareForNodeRemoval(node);
  node->recursivelyDecReferenceCount();
}
        ...
        if (rhsOfStoreDefNode->getType().isBCD()) {
          ...
          if (overrideNodePrecision != 0) {
            ...
            TR_ASSERT(node->getType().isBCD(),
                      "node %s (%p) must be a BCD type if "
                      "overrideNodePrecision == 0\n",
...
bool OMR::LocalCSE::canBeAvailable(
    TR::Node *parent, TR::Node *node,
    TR_BitVector &seenAvailableLoadedSymbolReferences, bool canBeAvailable) {
  if (!canBeAvailable) return false;

  if (node->getOpCode().isBranch() || (node->getOpCode().isCase()))
    return false;

  if (!shouldCommonNode(parent, node)) return false;

  if (node->getOpCodeValue() == TR::allocationFence) return false;
  ...
  if (comp()->cg()->supportsLengthMinusOneForMemoryOpts() && parent &&
      parent->getOpCode().isMemToMemOp()) {
    if (parent->getLastChild() == node) return false;
  }
...
bool OMR::LocalCSE::isAvailableNullCheck(
  ...
  if (!_isAvailableNullCheck) return false;
...
void OMR::LocalCSE::killAvailableExpressionsUsingBitVector(HashTable *hashTable,
                                                           TR_BitVector &vec) {
...
void OMR::LocalCSE::removeFromHashTable(HashTable *hashTable,
                                        int32_t hashValue) {
  auto range = hashTable->equal_range(hashValue);
  hashTable->erase(range.first, range.second);
}
...
bool OMR::LocalCSE::areSyntacticallyEquivalent(TR::Node *node1, TR::Node *node2,
  ...
  if (!(node1->getNumChildren() == node2->getNumChildren())) {
    if (node1->getOpCode().isDiv() || node1->getOpCode().isRem()) {
      if (node1->getNumChildren() == 3) return false;
    } else
      return false;
  }
  ...
  for (int32_t i = 0; i < node1->getNumChildren(); i++) {
    ...
    if (child1 != child2) {
      if (child1->getOpCode().isArrayRef() &&
        ...
        if (child2->getReferenceCount() > 1) return false;
...
void OMR::LocalCSE::collectAllReplacedNodes(TR::Node *node,
  ...
  if (node->getOpCode().isCase()) return;
...
const char *OMR::LocalCSE::optDetailString() const throw() {
