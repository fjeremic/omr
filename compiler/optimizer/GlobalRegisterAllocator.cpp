...
TR_GlobalRegisterAllocator::TR_GlobalRegisterAllocator(
    ...
    : TR::Optimization(manager), _pairedSymbols(manager->trMemory()),
      _allocatedPerformCellSymRefs(manager->allocator()), _performCellCount(0),
      _newBlocks(manager->trMemory()), _nodesToBeChecked(manager->trMemory()),
      _nodeTriplesToBeChecked(manager->trMemory()),
      _rejectedSignExtNodes(manager->trMemory()), _storeSymRef(NULL),
      _seenInternalMethods(NULL), _osrCatchSucc(NULL),
...
static void
setAutoContainsRegisterValue(TR_RegisterCandidate *rc,
    ...
    if (!firstRc || firstRc->getSymbolReference()->getSymbol() != symbol) {
      ...
      if (!secondRc || secondRc->getSymbolReference()->getSymbol() != symbol) {
        ...
        if (!thirdRc || thirdRc->getSymbolReference()->getSymbol() != symbol)
          ...
          TR_ASSERT(false, "Current cand is not live on entry and not live on "
                           "exit from basic block\n");
      }
    }
...
TR::TreeTop *
TR_GlobalRegister::optimalPlacementForStore(TR::Block *currentBlock,
                                            TR::Compilation *comp) {
  bool traceGRA =
      comp->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
  if (traceGRA)
...
TR_PairedSymbols *
TR_GlobalRegisterAllocator::findPairedSymbols(TR::SymbolReference *symRef1,
...
StoresInBlockInfo *
TR_GlobalRegisterAllocator::findRegInStoreInfo(TR_GlobalRegister *gr) {
...
TR_LiveRangeSplitter::TR_LiveRangeSplitter(TR::OptimizationManager *manager)
    : TR::Optimization(manager), _splitBlocks(manager->trMemory()),
      _changedSomething(false), _origSymRefs(NULL) {}
            ...
            if (!rc) {
              ...
              if (!dontCreateCandidatesJustForSplitter &&
                ...
                rc->addAllBlocksInStructure(loop->getStructure(), comp(),
                                            trace() ? "range splitter candidate"
                                                    : NULL);
              }
            }
            registerCandidates[origSymRef->getReferenceNumber()] = rc;
