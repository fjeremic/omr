...
public:
  ...
  virtual switchSuccessorIterator* asSwitchSuccessor()
  {
    return NULL;
  }
};
...
public:
  ...
  {}
  switchSuccessorIterator* asSwitchSuccessor()
  {
    return this;
  }
...
static void changeHeapBaseConstToLoad(
  TR::Compilation* comp, TR::SymbolReference*& autoSymRef, TR::Node* node, vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
    return;
...
int32_t TR_GlobalRegisterAllocator::perform()
{
  ...
  candidates->_candidateForSymRefs = new (trStackMemory())
    TR_RegisterCandidates::SymRefCandidateMap((TR_RegisterCandidates::SymRefCandidateMapComparator()),
      (TR_RegisterCandidates::SymRefCandidateMapAllocator(trMemory()->currentStackRegion())));
  ...
  candidates->_startOfExtendedBBForBB.init(
    trMemory(), (uint32_t)(comp()->getFlowGraph()->getNextNodeNumber() * sizeof(TR::Block*) * 1.5), false, stackAlloc);
  ...
  if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") && cg()->prepareForGRA()) {
    ...
    if (TR::Compiler->target.is64Bit() && optimizer()->getUseDefInfo()) {
      _temp
        = new (trStackMemory()) TR_BitVector(optimizer()->getUseDefInfo()->getNumDefNodes(), trMemory(), stackAlloc);
    ...
    if (canAffordAssignment) {
      ...
      if (_lastGlobalRegisterNumber > -1) {
        ...
        for (nodeTriple = nodeTriplesIt.getFirst(); nodeTriple != NULL; nodeTriple = nodeTriplesIt.getNext()) {
          ...
          if ((nodeTriple->_node2->getOpCodeValue() == TR::iRegStore)
            && ((node3->getOpCodeValue() == TR::iRegStore) || (node3->getOpCodeValue() == TR::iRegLoad))
      ...
      if (!splitSymRefs.isEmpty()) {
        ...
        // If source and destination locals of a copy receive registers, in other words, a register-register copy
        // exists, and a value is modified in one of locals, another local must be indentified as 'valueModified'.
        //
        if (trace)
...
void TR_GlobalRegisterAllocator::transformBlock(TR::TreeTop* tt)
{
  ...
  do {
    if (node->getOpCodeValue() == TR::BBStart) {
      ...
      for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        if ((*curRegisters)[i].getRegisterCandidateOnEntry()) {
          (*curRegisters)[i].getRegisterCandidateOnEntry()->getSymbolReference()->getSymbol()->setIsInGlobalRegister(
            true);
        }

        if ((*curRegisters)[i].getRegisterCandidateOnExit()) {
          (*curRegisters)[i].getRegisterCandidateOnExit()->getSymbolReference()->getSymbol()->setIsInGlobalRegister(
            true);
        }
      }
...
static void setAutoContainsRegisterValue(
  TR_RegisterCandidate* rc, TR_Array<TR_GlobalRegister>* extRegisters, int32_t i, TR::Compilation* comp)
{
...
void TR_GlobalRegisterAllocator::transformNode(TR::Node* node, TR::Node* parent, int32_t childIndex, TR::TreeTop* tt,
  TR::Block*& block, TR_Array<TR_GlobalRegister>& registers, TR_NodeMappings* extBlockNodeMapping)
{
  ...
  TR_Array<TR_GlobalRegister>* extRegisters
    = &(_candidates->_startOfExtendedBBForBB[block->getNumber()]->getGlobalRegisters(comp()));
  ...
  if (node->getOpCode().isJumpWithMultipleTargets() && !node->getOpCode().isSwitch()
    ...
    if (changeNode) {
      ...
      if (!value) {
        ...
        if (comp()->cg()->needToAvoidCommoningInGRA() && value
          ...
          && (symbol->getDataType() == TR::Float || symbol->getDataType() == TR::Double
#ifdef J9_PROJECT_SPECIFIC
            || symbol->getDataType() == TR::DecimalFloat || symbol->getDataType() == TR::DecimalDouble
            || symbol->getDataType() == TR::DecimalLongDouble
#endif
            )
          && (!parent->getOpCode().isStore() && (parent->getOpCodeValue() != TR::fRegStore)
            && (parent->getOpCodeValue() != TR::dRegStore) && (parent->getOpCodeValue() != TR::GlRegDeps)
...

        {
          ...
          if (!storeInfo->_origStoreExists) {
            ...
            TR::Node* store = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(rc->getDataType()), 1, 1,
          ...
          dumpOptDetails(comp(),
            "%s did not change load var [%p] of symRef#%d to load reg because float/double issues\n", OPT_DETAILS, node,
            node->getSymbolReference()->getReferenceNumber());
          return;
        }
        ...
        if (!value) {
          ...
          if (node->isDontMoveUnderBranch()) {
            // dumpOptDetails(comp(), "propagating dontMoveUnderBranch flag from original load node [%p] to new load
            // node [%p]\n", node, value);
      ...
      if (comp()->getOption(TR_CheckGRA)
        ...
        TR::Node* zeroIfEqual
          = TR::Node::create(comp()->il.opCodeForCompareNotEquals(oldValue->getDataType()), 2, oldValue, value);
    ...
    if (symbol->isInGlobalRegister() && (gr = getGlobalRegister(symbol, registers, block))) {
      ...
      // traceMsg(comp(), "ptrToGr %p node %p mapping %d\n", ptrToGr, node, ptrToGr ? ptrToGr->getMappings().getTo(node)
      // : 0);
      if (ptrToGr) {
        ...
        TR_RegisterCandidate* rc = ptrToGr->getCurrentRegisterCandidate();
        if (rc && (rc->getSymbolReference()->getSymbol() == symbol)) {
          if (rc->getSymbolReference()->getSymbol()->isMethodMetaData() && rc->getSymbolReference() != origSymRef
            && ptrToGr->getValue() && !ptrToGr->getAutoContainsRegisterValue())
            ptrToGr->createStoreFromRegister(comp()->getVisitCount(), tt->getPrevTreeTop(), 1, comp(), true);
          ptrToGr->setAutoContainsRegisterValue(true);
          ptrToGr->setValue(0);
    ...
    if (block->getNextBlock() && !block->getNextBlock()->isExtensionOfPreviousBlock()
      ...
      if (!regStarTransformDone) {
        addGlRegDepToExit(regDepNodes, exitNode,
          _candidates->_startOfExtendedBBForBB[block->getNumber()]->getGlobalRegisters(comp()), block);
      }
  ...
  TR_Array<TR_GlobalRegister>& registers, TR::Block* successorBlock)
{
  ...
  TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
  addGlRegDepToExit(
    regDepNodes, exitNode, _candidates->_startOfExtendedBBForBB[block->getNumber()]->getGlobalRegisters(comp()), block);
}
...
bool TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors(
  int32_t i, TR_GlobalRegister* extReg, TR_GlobalRegister* reg, TR::Block* block, bool traceIt)
{
...
void TR_GlobalRegisterAllocator::addCandidateReloadsToEntry(
  TR::TreeTop* bbStartTT, TR_Array<TR_GlobalRegister>& registers, TR::Block* block)
{
...
void TR_GlobalRegisterAllocator::addStoresForCatchBlockLoads(
  TR::TreeTop* appendPoint, TR_Array<TR_GlobalRegister>& registers, TR::Block* throwingBlock)
{
...
void TR_GlobalRegisterAllocator::prepareForBlockExit(TR::TreeTop*& exitTreeTop, TR::Node*& exitNode, TR::Block* block,
  TR_Array<TR_GlobalRegister>& registers, TR::Block* successorBlock, TR_Array<TR::Node*>& regDepNodes)
{
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    ///regDepNodes[i], extgr->getAutoContainsRegisterValue());
...
bool TR_GlobalRegisterAllocator::registerIsLiveAcrossEdge(TR::TreeTop* exitTreeTop, TR::Node* exitNode,
  TR::Block* block, TR_GlobalRegister* extgr, TR::Block*& successorBlock, int32_t i)
{
  TR_Array<TR_GlobalRegister>& registers = block->getGlobalRegisters(comp());
  TR_GlobalRegister* gr = &registers[i];
  ...
  if (!predecessors.empty() && !(predecessors.size() == 1)
    ...
    if (liveOnAllPredExits && allPredFreq >= successorBlock->getFrequency()) {
      if (trace())
        traceMsg(comp(),
          "Extended live range of #%d into successor since candidate is available in register on all predecessor's "
          "exits: block=%d succ=%d allPredFreq=%d succFreq=%d\n",
          rc->getSymbolReference()->getReferenceNumber(), block->getNumber(), successorBlock->getNumber(), allPredFreq,
  ...
  if (!keepingItAliveAcrossEdgeRequiresExtraGoto) {
    ...
    if (successorRegister->getRegisterCandidateOnEntry() != 0
      ...
      || (successorRegister->getRegisterCandidateOnExit() && (successorRegister->getRegisterCandidateOnExit() != rc))) {
    ...
    for (nextNewBlock = newBlocksIt.getFirst(); nextNewBlock != NULL; nextNewBlock = newBlocksIt.getNext()) {
      ...
      if (newBlockCanBeReused) {
        ...
        TR_Array<TR_GlobalRegister>& predRegistersEBB
          = _candidates->_startOfExtendedBBForBB[predBlock->getNumber()]->getGlobalRegisters(comp());
    ...
    for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber; ++j) {
      if (successorRegisters[j].getRegisterCandidateOnEntry()) {
        ...
        TR_ASSERT(!blockRegisters[j].getRegisterCandidateOnExit()
            ...
            || (blockRegisters[j].getRegisterCandidateOnEntry() == successorRegisters[j].getRegisterCandidateOnEntry()),
...
TR_GlobalRegister* TR_GlobalRegisterAllocator::getGlobalRegisterWithoutChangingCurrentCandidate(
  TR::Symbol* symbol, TR_Array<TR_GlobalRegister>& registers, TR::Block* block)
{
  TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
...
void TR_GlobalRegister::setCurrentRegisterCandidate(TR_RegisterCandidate* rc, vcount_t visitCount,
  TR::Block* currentBlock, int32_t i, TR::Compilation* comp, bool resetOtherHalfOfLong)
{
  if (_rcCurrent != rc) {
    ...
    if (resetOtherHalfOfLong && _rcCurrent != NULL && _rcCurrent->rcNeeds2Regs(comp)) {
      ...
      //  _candidates->_startOfExtendedBBForBB[currentBlock->getNumber()], "Incorrect value in
    ...
    _rcCurrent = rc;
    setValue(
      0); // regardless of whether or not we created a store we must setValue to NULL, else we can use the wrong value
...
TR::TreeTop* TR_GlobalRegister::optimalPlacementForStore(TR::Block* currentBlock, TR::Compilation* comp)
{
...
TR::Node* TR_GlobalRegister::createLoadFromRegister(TR::Node* n, TR::Compilation* comp)
{
  ...
  if (load->requiresRegisterPair(comp))
    dumpOptDetails(comp, "%s create load [%p] from Register %d (low word) and Register %d (high word)\n", OPT_DETAILS,
      load, rc->getLowGlobalRegisterNumber(), rc->getHighGlobalRegisterNumber());
  else
...
TR::Node* TR_GlobalRegister::createStoreToRegister(
  TR::TreeTop* prevTreeTop, TR::Node* node, vcount_t visitCount, TR::Compilation* comp, TR_GlobalRegisterAllocator* gra)
{
  ...
  if (TR::Compiler->target.is64Bit() && (store->getOpCodeValue() == TR::iRegStore)
  ...
  if (store->requiresRegisterPair(comp))
    dumpOptDetails(comp, "%s create store [%p] of symRef#%d to Register %d (low word) and Register %d (high word)\n",
      OPT_DETAILS, store, rc->getSymbolReference()->getReferenceNumber(), rc->getLowGlobalRegisterNumber(),
...
#ifdef J9_PROJECT_SPECIFIC
              || sym->getDataType() == TR::DecimalFloat || sym->getDataType() == TR::DecimalDouble
...
void TR_GlobalRegisterAllocator::findLoopAutoRegisterCandidates()
{
  ...
  SymRefCandidateMap* registerCandidates = new (trStackMemory())
    SymRefCandidateMap((SymRefCandidateMapComparator()), SymRefCandidateMapAllocator(trMemory()->currentStackRegion()));
    ...
    if (!regionStructure->isAcyclic() && structureNode) {
      ...
      for (nextBlock = blocksIt.getFirst(); nextBlock; nextBlock = blocksIt.getNext()) {
        if (nextBlock->getVisitCount() != visitCount) {
          ...
          if (((++numIter) & 0x3f) == 0
            && comp()->compilationShouldBeInterrupted(GRA_FIND_LOOPS_AND_CORRESPONDING_AUTOS_BLOCK_CONTEXT)) {
      ...
      for (auto succ = structureNode->getSuccessors().begin(); succ != structureNode->getSuccessors().end(); ++succ) {
        ...
        if (0 && exitBlock && (exitBlock != comp()->getFlowGraph()->getEnd())) {
          ...
          while (bvi.hasMoreElements()) {
            ...
            // exitBlock->getNumber(), nextCandidate);
  ...
  TR_BitVector* symsThatShouldNotBeAssignedInCurrentLoop, bool hasCatchBlock)
{
  ...
  if (node->getOpCode().isLoadVarDirect() && node->getSymbolReference()->getSymbol()->isAuto()) {
    ...
    if (TR::Compiler->target.is64Bit() && info && (parent->getOpCodeValue() == TR::i2l) && node->isNonNegative()
      && enableSignExtGRA) {
      ...
      if (info->getUseDef(defs, useIndex)) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          ...
          if (defNode->getOpCode().isStore()) {
            ...
            // TR_ASSERT((sym == node->getSymbolReference()->getSymbol()), "Symbols must match between a use and a
            // def\n"); Comment out the above assume because:
            //   - The assume came from JAVA where there could not be aliasing with autos
            //   - If there is an alias causing a different sym then we will not have a candidate anyways so it does not
            //   matter
    ...
    if (symRef->getSymbol()->isAutoOrParm() && isSymRefAvailable(symRef, blocksInLoop)) {
      ...
      if (block != cfg->getStart()) {
        ...
        if ((node->getOpCode().isStoreDirect() && isSplittingCopy(node))
          ...
          if (TR::Compiler->target.cpu.isZ() && rc->getSymbolReference()->getSymbol()->getDataType() == TR::Address
            && parent
            && (((parent->getOpCode().isStoreIndirect() || parent->getOpCode().isLoadIndirect())
                  && parent->getFirstChild() == node)
              || (parent->getOpCode().isAdd() && grandParent
                && (grandParent->getOpCode().isStoreIndirect() || grandParent->getOpCode().isLoadIndirect())))) {
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    ...
    markAutosUsedIn(node->getChild(childNum), node, parent, currentArrayAccess, block, blocksInLoop, visitCount,
      executionFrequency, registerCandidates, assignedAutosInCurrentLoop, symsThatShouldNotBeAssignedInCurrentLoop,
      hasCatchBlock);
      ...
      for (nextBlock = blocksIt.getFirst(); nextBlock; nextBlock = blocksIt.getNext()) {
        if (nextBlock->getVisitCount() != visitCount) {
          ...
          while (currentTree != exitTree) {
            TR::Node* currentNode = currentTree->getNode();
            TR::Node* arrayAccess = NULL;
            createStoresForSignExt(
              currentNode, NULL, NULL, currentTree, &arrayAccess, nextBlock, &blocksInLoop, visitCount, false);
            currentTree = currentTree->getNextRealTreeTop();
          }
        }
      }
...
void TR_LiveRangeSplitter::splitLiveRanges()
{
  ...
  //**)trMemory()->allocateStackMemory(comp()->getSymRefCount()*sizeof(TR::SymbolReference *)); memset(_origSymRefs, 0,
  ...
  if (structure->asRegion()) {
    ...
    if (regionStructure->isNaturalLoop() && structureNode) {
      ...
      if (loopInvariantBlock) {
        ...
        TR_SymRefCandidatePair** correspondingSymRefs = (TR_SymRefCandidatePair**)trMemory()->allocateStackMemory(
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
          if (nextBlock->getVisitCount() != visitCount) {
            ...
            while (currentTree != exitTree) {
              TR::Node* currentNode = currentTree->getNode();
              replaceAutosUsedIn(currentTree, currentNode, NULL, nextBlock, &blocksInLoop, &exitBlocks, visitCount,
                executionFrequency, *registerCandidates, correspondingSymRefs, replacedAutosInCurrentLoop,
                autosThatCannotBeReplacedInCurrentLoop, structureNode, loopInvariantBlock);
              currentTree = currentTree->getNextRealTreeTop();
            }
          }
        }
        ...
        while ((i < symRefCount) && (prevSymRefCount == 0)) {
          ...
          if (origSymRef && origSymRef->getSymbol()->isAutoOrParm() && comp()->cg()->considerTypeForGRA(origSymRef)
            ...
            if (trace() && origSymRef->getSymbol()->getAutoSymbol())
              traceMsg(comp(), "2 orig sym %p %d live index %d\n", origSymRef->getSymbol(),
                origSymRef->getReferenceNumber(), origSymRef->getSymbol()->getAutoSymbol()->getLiveLocalIndex());
            // if (rc)
            {
          ...
          if (candidateIsLiveOnExit) {
            ...
            if (((!isFloat && (_numberOfGPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalGPRs() - 2)
              ...
              TR_SymRefCandidatePair* correspondingSymRefCandidate = splitAndFixPreHeader(
                symRef, correspondingSymRefs, loopInvariantBlock, loopInvariantBlock->getEntry()->getNode());
              ...
              ///correspondingSymRef->getReferenceNumber(), comp()->signature()); fflush(stdout);
              fixExitsAfterSplit(symRef, correspondingSymRefCandidate, correspondingSymRefs, loopInvariantBlock,
                &blocksInLoop, loopInvariantBlock->getEntry()->getNode(), *registerCandidates, structureNode,
  ...
  if (node->getOpCode().isLoadVarDirect() || (node->getOpCodeValue() == TR::loadaddr)
    ...
    if (comp()->cg()->considerTypeForGRA(symRef) && canSplit(symRef, comp())
      ...
      if (correspondingSymRef) {
        ...
        if (!node->getOpCode().isStoreDirect()
          || (!dontReplaceStores
            || performTransformation(comp(), "%s --- going to replace auto #%d by auto #%d on %s node %p --- \n",
              OPT_DETAILS, symRef->getReferenceNumber(), correspondingSymRef->getReferenceNumber(),
              node->getOpCode().getName(), node))) {
          // traceMsg(comp(), " --- replaced auto #%d by auto #%d on %s node %p --- \n", symRef->getReferenceNumber(),
          // correspondingSymRef->getReferenceNumber(), node->getOpCode().getName(), node);
        ...
        fixExitsAfterSplit(symRef, correspondingSymRefCandidate, correspondingSymRefs, loopInvariantBlock, blocksInLoop,
          node, registerCandidates, loop, replacedAutosInCurrentLoop, origSymRef);
      }
    }
  }
...
void TR_LiveRangeSplitter::fixExitsAfterSplit(TR::SymbolReference* symRef,
  ...
  TR::Block* loopInvariantBlock, List<TR::Block>* blocksInLoop, TR::Node* node, SymRefCandidateMap& registerCandidates,
  TR_StructureSubGraphNode* loop, TR_BitVector* replacedAutosInCurrentLoop, TR::SymbolReference* origSymRef)
{
  ...
  if (correspondingSymRef) {
    ...
    if (!replacedAutosInCurrentLoop->get(symRef->getReferenceNumber())) {
      ...
      if (rc && !correspondingRc) {
        ...
        TR_BitVector* blocksInInnerLoop
          = new (trStackMemory()) TR_BitVector(comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc);
        ListIterator<TR::Block> blocksIt(blocksInLoop);
        TR::Block* nextBlock;
        for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
            ...
            if (!blocksInInnerLoop->get(nextBlock->getNumber())) {
              if (trace())
                traceMsg(comp(), "Adding original candidate #%d in block_%d in outer loop %d (%p)\n",
                  rc->getSymbolReference()->getReferenceNumber(), nextBlock->getNumber(), parentOfLoop->getNumber(),
                  parentOfLoop);
              rc->addBlock(nextBlock, 0);
            }
          }
  ...
  for (exitBlock = exitBlocksIt.getCurrent(); exitBlock; exitBlock = exitBlocksIt.getNext()) {
    if ((exitBlock != comp()->getFlowGraph()->getEnd()) && exitsFromCurrentLoop->get(exitBlock->getNumber())) {
      ...
      if (!(exitBlock->getPredecessors().size() == 1)) {
        ...
        for (auto nextEdge = exitBlock->getPredecessors().begin(); nextEdge != exitBlock->getPredecessors().end();
             ++nextEdge) {
        ...
        for (nextEdge = ei.getFirst(); nextEdge; nextEdge = ei.getNext()) {
          ...
          if (pred->getLastRealTreeTop()->getNode()->getOpCode().isBranch() && !_splitBlocks.isEmpty()) {
            ...
            for (nextSplitBlock = splitBlocksIt.getFirst(); nextSplitBlock != NULL;
              ...
              if (exitBlockIsASplitBlock
                ...
                dumpOptDetails(comp(),
                  " place store-back of auto #%d using auto #%d in (existing) loop exit block_%d between pred %d and "
                  "exit block_%d\n",
          ...
          if (!splitBlock) {
            ...
            dumpOptDetails(comp(),
              " place store-back of auto #%d using auto #%d in (new) loop exit block_%d between pred %d and exit "
              "block_%d\n",
              orig->getReferenceNumber(), replacement->getReferenceNumber(), splitBlock->getNumber(), pred->getNumber(),
              exitBlock->getNumber());
          }
