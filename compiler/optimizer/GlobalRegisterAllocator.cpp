...
#define HAVE_DIFFERENT_MSB_TO_LSB_OFFSETS(r1, r2)   \
  ((((r1)->getHostByteOffset() + (r1)->getSize()) - \
    ((r2)->getHostByteOffset() + (r2)->getSize())) != 0)

static bool dontAssignInColdBlocks(TR::Compilation *comp) {
  return comp->getMethodHotness() >= hot;
}

#define keepAllStores false

// TODO:GRA: if we are going to have two versions of GRA one with Meta Data and
// one without then we can do someting here
// for different compilation level then we can have two version of this, one
// using CPIndex and one  (i.e. with MetaData) using getReferenceNumber
...
class switchSuccessorIterator : public SuccessorIterator {
 public:
  ...
  switchSuccessorIterator(TR::Node *node)
      : node_(node), i_(node_->getCaseIndexUpperBound()) {}
  ...
  TR::Block *getNextSuccessor() {
    for (i_ = (i_ > 0) ? i_ - 1 : 0;
         i_ > 0 && !node_->getChild((int32_t)i_)->getOpCode().isCase(); --i_)
      ;
    if (i_ == 0) {
      return NULL;
    } else {
  ...
  TR::Node *getCaseNode() {
    TR_ASSERT(i_ > 0 && i_ < node_->getCaseIndexUpperBound(),
              "getCaseNode not called on valid successor");
    return node_->getChild((int32_t)i_);
  }
...
class multipleJumpSuccessorIterator : public SuccessorIterator {
 public:
  ...
  TR::Block *getNextSuccessor() {
    if (_iterator != _list->end()) ++_iterator;
    return getNextSuccessor_();
  }
...
TR_GlobalRegisterAllocator::TR_GlobalRegisterAllocator(
...
void TR_GlobalRegisterAllocator::populateSymRefNodes(TR::Node *node,
                                                     vcount_t visitCount) {
...
/**
 * The cg considerTypeForGRA calls below are the functional tests (i.e. avoid
 * BCD and some aggregates) The allocateFor checks are for tuning for
 * scalability when running in limited GRA mode
 */
bool TR_GlobalRegisterAllocator::allocateForSymRef(
    TR::SymbolReference *symRef) {
  return true;
}

bool TR_GlobalRegisterAllocator::isSymRefAvailable(
    TR::SymbolReference *symRef) {
  if (!comp()->cg()->considerTypeForGRA(symRef)) return false;
 ...
 */
bool TR_GlobalRegisterAllocator::isSymRefAvailable(
    TR::SymbolReference *symRef, List<TR::Block> *blocksInLoop) {
  if (!comp()->cg()->considerTypeForGRA(symRef)) return false;

  bool loopHasOneBlock =
      blocksInLoop && (blocksInLoop->getListHead()->getNextElement() == NULL);
...
bool TR_GlobalRegisterAllocator::isNodeAvailable(TR::Node *node) {
  if (!comp()->cg()->considerTypeForGRA(node)) return false;
...
static void changeHeapBaseConstToLoad(TR::Compilation *comp,
  ...
  if ((node->getOpCodeValue() == TR::lconst) &&
    ...
    if (!autoSymRef) {
      autoSymRef = comp->getSymRefTab()->createTemporary(
      ...
      TR::Node *storeNode =
          TR::Node::createWithSymRef(TR::lstore, 1, 1, lconstNode, autoSymRef);
...
void TR_GlobalRegisterAllocator::visitNodeForDataType(TR::Node *node) {
  ...
  if (node->getDataType() != node->getSymbol()->getDataType() &&
      node->getSymbol()->getDataType() == TR::Aggregate) {
    comp()->cg()->addSymbolAndDataTypeToMap(node->getSymbol(),
                                            node->getDataType());
  }
}
...
int32_t TR_GlobalRegisterAllocator::perform() {
  ...
  if (comp()->isGPUCompilation()) return 1;
  ...
  if (comp()->useCompressedPointers() && comp()->cg()->materializesHeapBase() &&
    ...
    if (autoSymRef) comp()->getSymRefTab()->aliasBuilder.createAliasInfo();
  }
  ...
  candidates->_candidateForSymRefs =
      new (trStackMemory()) TR_RegisterCandidates::SymRefCandidateMap(
          (TR_RegisterCandidates::SymRefCandidateMapComparator()),
          (TR_RegisterCandidates::SymRefCandidateMapAllocator(
              trMemory()->currentStackRegion())));
  TR_RegisterCandidate *rc = candidates->getFirst();
  for (; rc; rc = rc->getNext())
    (*candidates->_candidateForSymRefs)[GET_INDEX_FOR_CANDIDATE_FOR_SYMREF(
        rc->getSymbolReference())] = rc;

  candidates->_startOfExtendedBBForBB.init(
      trMemory(),
      (uint32_t)(comp()->getFlowGraph()->getNextNodeNumber() *
                 sizeof(TR::Block *) * 1.5),
      false, stackAlloc);
  TR::Block *lastStartOfExtendedBB = comp()->getStartBlock();
  for (TR::Block *b = lastStartOfExtendedBB; b; b = b->getNextBlock()) {
    lastStartOfExtendedBB =
        b->isExtensionOfPreviousBlock() ? lastStartOfExtendedBB : b;
    candidates->_startOfExtendedBBForBB[b->getNumber()] = lastStartOfExtendedBB;
  }

  comp()->getOptimizer()->setCachedExtendedBBInfoValid(true);

  if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") &&
      cg()->prepareForGRA()) {
    static char *useFreqs = feGetEnv("TR_GRA_UseProfilingFrequencies");
    if (useFreqs) comp()->setUsesBlockFrequencyInGRA();

    TR_BitVector *liveVars = NULL;
    if (!cg()->getLiveLocals()) {
      int32_t numLocals = 0;
      TR::AutomaticSymbol *a;
      ListIterator<TR::AutomaticSymbol> locals(
          &comp()->getMethodSymbol()->getAutomaticList());
      for (a = locals.getFirst(); a != NULL; a = locals.getNext()) ++numLocals;

      if (comp()->getOption(TR_EnableAggressiveLiveness)) {
        TR::ParameterSymbol *p;
        ListIterator<TR::ParameterSymbol> parms(
            &comp()->getMethodSymbol()->getParameterList());
        for (p = parms.getFirst(); p != NULL; p = parms.getNext()) ++numLocals;
      }

      const uint64_t MAX_BITVECTOR_MEMORY_USAGE = 1000000000;
      uint64_t bitvectorMemoryUsage =
          numLocals * comp()->getFlowGraph()->getNextNodeNumber();
      ...
      if (numLocals > 0 &&
        ...
        TR_Liveness liveLocals(
            comp(), optimizer(), comp()->getFlowGraph()->getStructure(), false,
            NULL, false, comp()->getOption(TR_EnableAggressiveLiveness));
        if (comp()->getVisitCount() > HIGH_VISIT_COUNT) {
          comp()->resetVisitCounts(1);
        }

        for (TR::CFGNode *cfgNode = comp()->getFlowGraph()->getFirstNode();
             cfgNode; cfgNode = cfgNode->getNext()) {
        ...
        // Make sure the code generator knows there are live locals for blocks,
        // and create a bit vector of the correct size for it.
        //
        liveVars = new (trHeapMemory()) TR_BitVector(numLocals, trMemory());
    ...
    if (trace())
      comp()->dumpMethodTrees("Trees before tactical global register allocator",
    ...
    _temp2 = new (trStackMemory())
        TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);

    if (TR::Compiler->target.is64Bit() && optimizer()->getUseDefInfo()) {
      _temp = new (trStackMemory())
          TR_BitVector(optimizer()->getUseDefInfo()->getNumDefNodes(),
                       trMemory(), stackAlloc);
      _candidatesNeedingSignExtension = new (trStackMemory())
          TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
      _candidatesSignExtendedInThisLoop = new (trStackMemory())
          TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
    }

    candidates->getReferencedAutoSymRefs(
        comp()->trMemory()->currentStackRegion());
    if (!comp()->mayHaveLoops() ||
        cg()->considerAllAutosAsTacticalGlobalRegisterCandidates())
    ...
    _registerCandidates = new (trStackMemory()) SymRefCandidateMap(
        (SymRefCandidateMapComparator()),
        SymRefCandidateMapAllocator(trMemory()->currentStackRegion()));

    _candidates = comp()->getGlobalRegisterCandidates();

    for (TR_RegisterCandidate *rc = _candidates->getFirst(); rc;
         rc = rc->getNext()) {
      (*_registerCandidates)[rc->getSymbolReference()->getReferenceNumber()] =
          rc;
    }
    ...
    if (!comp()->getOption(TR_ProcessHugeMethods)) {
      ...
      //
      double complexityEstimate = comp()->getFlowGraph()->getNumberOfNodes() *
                                  (double)numCands * numCands;
      if (complexityEstimate / hotnessFactor > (double)GRA_COMPLEXITY_LIMIT)
        canAffordAssignment = false;
    }

    _valueModifiedSymRefs = new (trStackMemory())
        TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
    TR_BitVector splitSymRefs(_origSymRefCount, trMemory(), stackAlloc);
    TR_BitVector nonSplittingCopyStored(_origSymRefCount, trMemory(),
                                        stackAlloc);
    ...
    if (canAffordAssignment) {
      globalFPAssignmentDone = _candidates->assign(cfgBlocks, numberOfBlocks,
                                                   _firstGlobalRegisterNumber,
                                                   _lastGlobalRegisterNumber);

      if (_lastGlobalRegisterNumber > -1) {
        _visitCount = comp()->incVisitCount();
        _gotoCreated = false;

        _signExtAdjustmentReqd = new (trStackMemory())
            TR_BitVector(_lastGlobalRegisterNumber + 1, trMemory(), stackAlloc);
        _signExtAdjustmentNotReqd = new (trStackMemory())
            TR_BitVector(_lastGlobalRegisterNumber + 1, trMemory(), stackAlloc);
        _signExtDifference = new (trStackMemory())
            TR_BitVector(_lastGlobalRegisterNumber + 1, trMemory(), stackAlloc);
        ...
        for (node = nodesIt.getFirst(); node != NULL;
             node = nodesIt.getNext()) {
        ...
        for (nodeTriple = nodeTriplesIt.getFirst(); nodeTriple != NULL;
             nodeTriple = nodeTriplesIt.getNext()) {
          if (trace())
            traceMsg(comp(), "node1 %p node2 %p node3 %p \n",
                     nodeTriple->_node1, nodeTriple->_node2,
                     nodeTriple->_node3);

          TR::Node *node3 = NULL;
          if ((nodeTriple->_node3->getOpCodeValue() == TR::istore) ||
              (nodeTriple->_node3->getOpCodeValue() == TR::iRegStore))
          ...
          if ((nodeTriple->_node2->getOpCodeValue() == TR::iRegStore) &&
              ...
              (nodeTriple->_node2->getGlobalRegisterNumber() ==
               node3->getGlobalRegisterNumber())) {
            ...
            nodeTriple->_node1->getChild(nodeTriple->_childNum)
                ->setSkipSignExtension(false);
          }
        }
      }
      ...
      for (TR_RegisterCandidate *rc = _candidates->getFirst(); rc;
        ...
        if (!rc->getValueModified())
          ...
          _valueModifiedSymRefs->set(
              rc->getSymbolReference()->getReferenceNumber());
          if (store) mayHaveDeadStore = true;
        }
      }
      ...
      if (mayHaveDeadStore) {
        ...
        if (_gotoCreated) requestOpt(OMR::redundantGotoElimination);
      }
      ...
      if (!splitSymRefs.isEmpty()) {
        bool trace =
            comp()->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
        ...
        for (TR_RegisterCandidate *rc = _candidates->getFirst(); rc;
          ...
          while (outerSymRef) {
            TR_RegisterCandidate *outerRc =
                (*_registerCandidates)[outerSymRef->getReferenceNumber()];
            if (!outerRc) break;
            if (_valueModifiedSymRefs->get(
                    rc->getSymbolReference()->getReferenceNumber()))
              _valueModifiedSymRefs->set(
                  outerRc->getSymbolReference()->getReferenceNumber());
            if (outerSymRef == rc->getSplitSymbolReference()) break;
            outerSymRef = outerRc->getRestoreSymbolReference();
          }
        }

        // Propagate the information about 'valueModified'.
        // 'valueModified' flag is not set at the stores (copies) for live-range
        // splitting. If source and destination locals of a copy receive
        // registers, in other words, a register-register copy exists, and a
        // value is modified in one of locals, another local must be indentified
        // as 'valueModified'.
        //
        if (trace)
        ...
        for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
          ...
          if (isSplittingCopy(tt->getNode())) {
            TR::Node *store = tt->getNode(),
                     *load = tt->getNode()->getFirstChild();
            if (store->getOpCode().isStoreDirect() &&
                load->getOpCode().isLoadReg() &&
                !(*_registerCandidates)[store->getSymbolReference()
                                            ->getReferenceNumber()]
                     ->extendedLiveRange()) {
            ...
            nonSplittingCopyStored.set(
                node->getSymbolReference()->getReferenceNumber());
        }

        // Restore original symbols. This can reduce the size of the stack area.
        // In addition, dead store elimination can remove redundant stores
        // (copies) where both of operands (source and destination) do not
        // receive any register.
        //
        if (performTransformation(
                comp(),
                "%s Restoring original symbols from live range splitter\n",
                OPT_DETAILS)) {
          vcount_t visitCount = comp()->incVisitCount();
          for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop())
        ...
        // Remove a store generated at the end of the live range when a value is
        // not modified in the live range. If a value is not modified in a local
        // which receives a register, the stores from the register is redundant.
        //
        ...
        for (tt = itr.getFirst(); tt; tt = itr.getNext()) {
          if (!_valueModifiedSymRefs->isSet(
                  tt->getNode()
                      ->getFirstChild()
                      ->getRegLoadStoreSymbolReference()
                      ->getReferenceNumber()) &&
  ...
  for (; block; block = block->getNextBlock()) block->clearGlobalRegisters();
...
bool TR_GlobalRegisterAllocator::isSplittingCopy(TR::Node *node) {
  ...
  if ((node->getOpCode().isStoreDirect() || node->getOpCode().isStoreReg()) &&
    ...
    TR::SymbolReference *loadSymRef =
        node->getFirstChild()->getSymbolReferenceOfAnyType();
    if (storeSymRef && loadSymRef && storeSymRef != loadSymRef) {
      TR_RegisterCandidate *storeRc =
          (*_registerCandidates)[storeSymRef->getReferenceNumber()];
      TR_RegisterCandidate *loadRc =
          (*_registerCandidates)[loadSymRef->getReferenceNumber()];
      TR::SymbolReference *origStoreSymRef =
          storeRc ? storeRc->getSplitSymbolReference() : NULL;
      TR::SymbolReference *origLoadSymRef =
          loadRc ? loadRc->getSplitSymbolReference() : NULL;
      if ((origStoreSymRef && origLoadSymRef &&
           origStoreSymRef == origLoadSymRef) ||
          (origStoreSymRef && !origLoadSymRef &&
           origStoreSymRef == loadSymRef) ||
          (!origStoreSymRef && origLoadSymRef &&
           storeSymRef == origLoadSymRef)) {
...
void TR_GlobalRegisterAllocator::restoreOriginalSymbol(TR::Node *node,
                                                       vcount_t visitCount) {
  if (node->getVisitCount() == visitCount) return;
  ...
  if (node->getOpCode().hasSymbolReference() || node->getOpCode().isLoadReg() ||
      node->getOpCode().isStoreReg()) {
    if (node->getSymbolReferenceOfAnyType()) {
      int32_t symRefNum =
          node->getSymbolReferenceOfAnyType()->getReferenceNumber();
      TR_RegisterCandidate *rc = (*_registerCandidates)[symRefNum];
      TR::SymbolReference *origSymRef =
          rc ? rc->getRestoreSymbolReference() : NULL;
      bool foundChangeSymRef = false;
      bool setValueModified = false;
      TR::SymbolReference *changeSymRef =
          rc ? rc->getSplitSymbolReference() : NULL;
      while (origSymRef && (origSymRef != rc->getSplitSymbolReference())) {
        TR_RegisterCandidate *origRc =
            (*_registerCandidates)[origSymRef->getReferenceNumber()];

        if (setValueModified)
          _valueModifiedSymRefs->set(
              origRc->getSymbolReference()->getReferenceNumber());

        if (!origRc || origRc->getValueModified() ||
      ...
      TR_RegisterCandidate *oldRc =
          origSymRef ? (*_registerCandidates)[origSymRef->getReferenceNumber()]
                     : 0;
      if (oldRc && oldRc->extendedLiveRange()) {
        _valueModifiedSymRefs->set(
            oldRc->getSymbolReference()->getReferenceNumber());
        changeSymRef = NULL;
      }
...
void TR_GlobalRegisterAllocator::transformBlock(TR::TreeTop *tt) {
  ...
  // Find out if there are any symbols that are in registers on entry and/or
  // exit
  //
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    while (b) {
      ...
      if (curRegisters[i].getRegisterCandidateOnEntry()) found = true;
      if (curRegisters[i].getRegisterCandidateOnExit()) found = true;

      b = b->getNextBlock();
      if (b && !b->isExtensionOfPreviousBlock()) break;
    }
  }
  ...
  //
  // Walk the tree changing all load/stores of register candidates to RegLoads
  // and RegStores. Also add RegLoads at branch points for registers live across
  // the branch.
  //
  ...
  do {
    if (node->getOpCodeValue() == TR::BBStart) {
      ...
      //
      for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber;
           ++i) {
    ...
    transformNode(node, 0, 0, tt, block, *curRegisters, &extBlockNodeMapping);
           ...
           (node = tt->getNode(),
            node->getOpCodeValue() != TR::BBStart ||
  ...
  *_signExtDifference &= *_signExtAdjustmentReqd;
  ...
  if (block == _appendBlock) _appendBlock = NULL;
}

TR::Node *TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::DataType oldType,
                                                          TR::Node *newNode) {
  return resolveTypeMismatch(oldType, NULL, newNode);
}

TR::Node *TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::Node *oldNode,
                                                          TR::Node *newNode) {
...
static void setAutoContainsRegisterValue(
    TR_RegisterCandidate *rc, TR_Array<TR_GlobalRegister> *extRegisters,
...
void TR_GlobalRegisterAllocator::transformNode(
    TR::Node *node, TR::Node *parent, int32_t childIndex, TR::TreeTop *tt,
    TR::Block *&block, TR_Array<TR_GlobalRegister> &registers,
    TR_NodeMappings *extBlockNodeMapping) {
  TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
            "Incorrect value in _startOfExtendedBBForBB");
  TR_Array<TR_GlobalRegister> *extRegisters =
      &(_candidates->_startOfExtendedBBForBB[block->getNumber()]
            ->getGlobalRegisters(comp()));
  ...
  if (opcode.getOpCodeValue() ==
      TR::table)  // (opcode.isSwitch() && debug("putGlRegDepOnSwitch"))
  {
    transformNode(node->getFirstChild(), node, 0, tt, block, registers,
                  extBlockNodeMapping);
  ...
  for (i = 0; i < node->getNumChildren(); ++i)
    transformNode(node->getChild(i), node, i, tt, block, registers,
                  extBlockNodeMapping);

  bool transformDone = false;

  if (node->getOpCode().isJumpWithMultipleTargets() &&
      !node->getOpCode().isSwitch() && node->getOpCodeValue() != TR::tstart)
    transformMultiWayBranch(tt, node, block, registers, transformDone);
  else if (opcode.isBranch()) {
    transformBlockExit(tt, node, block, registers,
    ...
    if (value) {
      ...
      if (gr)
        changeNode =
            true;  // symbol is will be put into register right before exit
    } else {
      TR_GlobalRegister *ptrToGr =
          getGlobalRegisterWithoutChangingCurrentCandidate(symbol, registers,
                                                           block);
      if (ptrToGr) {
        ...
        if (rc && (rc->getSymbolReference()->getSymbol() == symbol)) {
          ...
          if (needs2Regs) {
            TR_RegisterCandidate *currRC =
                ptrToGr->getCurrentRegisterCandidate();
            int32_t highRegNum = currRC->getHighGlobalRegisterNumber();
            if (ptrToGr == &((*extRegisters)[highRegNum])) {
              TR_GlobalRegister *grLow =
                  &((*extRegisters)[currRC->getLowGlobalRegisterNumber()]);
              if (grLow->getCurrentRegisterCandidate() == currRC) {
                grLow->setAutoContainsRegisterValue(true);
                grLow->setValue(0);
              }
            } else {
              TR_GlobalRegister *grHigh =
                  &((*extRegisters)[currRC->getHighGlobalRegisterNumber()]);
    ...
    if (changeNode) {
      ...
      if (!value) {
...

        {
          ...
          TR_ASSERT(storeInfo,
                    "Must have a store before a load in this block\n");
          ...
          if (!storeInfo->_origStoreExists) {
            ...
            TR::Node *store = TR::Node::createWithSymRef(
                comp()->il.opCodeForDirectStore(rc->getDataType()), 1, 1,
                regStoreNode->getFirstChild(), rc->getSymbolReference());
            ...
            TR::TreeTop *storeTreeTop =
                TR::TreeTop::create(comp(), prevTree, store);
            storeInfo->_origStoreExists = true;
          }
          dumpOptDetails(comp(),
                         "%s did not change load var [%p] of symRef#%d to load "
                         "reg because float/double issues\n",
                         OPT_DETAILS, node,
                         node->getSymbolReference()->getReferenceNumber());
          return;
        }
        ...
        dumpOptDetails(
            comp(), "%s change load var [%p] of %s symRef#%d to load reg\n",
            OPT_DETAILS, node,
            node->getSymbolReference()->getSymbol()->isMethodMetaData()
                ? node->getSymbolReference()
                      ->getSymbol()
        ...
        if (!value) {
          if (parent->getOpCode().isStore() &&
            ...
            TR::Node *loadVarNode =
                node->getSymbolReference()->getSymbol()->isMethodMetaData() &&
                    ...
                    ? 0
                    : node;
            value = gr->createStoreToRegister(prevTree, loadVarNode,
                                              _visitCount, comp(), this);
          ...
          if (node->isDontMoveUnderBranch()) {
            // dumpOptDetails(comp(), "propagating dontMoveUnderBranch flag from
            // original load node [%p] to new load node [%p]\n", node, value);
      ...
      if (node->skipSignExtension()) value->setSkipSignExtension(true);
      ...
      if (comp()->getOption(TR_CheckGRA) &&
          performTransformation(
              comp(), "%s CheckGRA inserting BNDCHK to verify %s node %p\n",
              OPT_DETAILS, value->getOpCode().getName(), value)) {
        TR::Node *zeroIfEqual = TR::Node::create(
            comp()->il.opCodeForCompareNotEquals(oldValue->getDataType()), 2,
            oldValue, value);
        TR::TreeTop::create(
            comp(), tt->getPrevTreeTop(),
            TR::Node::createWithSymRef(
                TR::BNDCHK, 2, 2,
                TR::Node::iconst(oldValue,
                                 1),  // pretend we have an array of size=1 so
                                      // only zeroIfEqual==0 is acceptable
                zeroIfEqual,
    ...
    if (symbol->isInGlobalRegister()) {
      ...
      if (gr && gr->getValue() && !gr->getAutoContainsRegisterValue()) {
        gr->createStoreFromRegister(_visitCount, tt->getPrevTreeTop(), -1,
                                    comp());
        bool needs2Regs = false;
        TR_RegisterCandidate *rc = gr->getCurrentRegisterCandidate();
        if (rc->rcNeeds2Regs(comp())) needs2Regs = true;
    ...
    if (symbol->isInGlobalRegister() &&
      ...
      // to keep the store.
      //
      // However if storeCanBeRemoved returns false for (some special) stores
      // created by other optimization passes (like escape analysis), then for
      // non FP stores (FP stores are handled correct as explained above because
      // we will insert store/reload pairs wherever required based on
      // GlobalFPStoreReloadOpt), we must keep the store intact. Another case
      // when we must keep the store to the stack location intact is if it is a
      // pinning array for an internal pointer, as the pinning array for an
      ...
      if ((!fpStore && !symRef->storeCanBeRemoved()) ||
        ...
        if (fpStore && !comp()->cg()->getSupportsJavaFloatSemantics()) {
          ...
          for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber;
            ...
            TR::Block *b = block;
            while (b) {
              TR_Array<TR_GlobalRegister> &curRegisters =
                  b->getGlobalRegisters(comp());
            ...
            if (avoidDuplicateFPStackValues) break;
          }
        }

        if (avoidDuplicateFPStackValues) return;
        ...
        if (_osrCatchSucc || symRef->getSymbol()->dontEliminateStores(comp()) ||
            keepAllStores || rc->isLiveAcrossExceptionEdge() ||
          ...
          // Pinning arrays must be accessible from the stack for GC to work
          // correctly. Candidates live across exception edges must have the
          // correct value in the temp so that the value can be reloaded in code
          // reachable from the catch block. Note that we ensure the candidate
          // is not live on entry in the ctach block. This ensures that it will
          // be re-loaded into the global register from the stack, prior to its
          // use.
          //
          ...
          TR::Node *newNode = NULL;
          {
            ...
            newNode = TR::Node::create(
                comp()->il.opCodeForRegisterStore(regStoreType), 1, child);
          ...
          TR::TreeTop *prevTree = tt->getPrevTreeTop();
          TR::TreeTop *newStore =
              TR::TreeTop::create(comp(), prevTree, newNode);
          node = newNode;
          if (comp()->cg()->needToAvoidCommoningInGRA() && fpStore)
            storeInfo->_origStoreExists = true;
        } else {
          origStoreToMetaData = NULL;
          {
            ...
            TR::Node::recreate(node,
                               comp()->il.opCodeForRegisterStore(regStoreType));
          }
        ...
        // node->getFirstChild());
        if (!storeIntact) gr->setAutoContainsRegisterValue(false);
        ...
        if (needs2Regs) {
          ...
          if (i == highRegNum) {
            int32_t lowRegNum = rc->getLowGlobalRegisterNumber();
            (*extRegisters)[lowRegNum].setValue(
                newValueNode ? newValueNode : node->getFirstChild());
            if (!storeIntact)
              (*extRegisters)[lowRegNum].setAutoContainsRegisterValue(false);
            (*extRegisters)[lowRegNum].setLastRefTreeTop(tt);
          } else {
            (*extRegisters)[highRegNum].setValue(
                newValueNode ? newValueNode : node->getFirstChild());
            if (!storeIntact)
      ...
      // ptrToGr ? ptrToGr->getMappings().getTo(node) : 0);
      if (ptrToGr) {
 ...
 */
void TR_GlobalRegisterAllocator::transformMultiWayBranch(
    TR::TreeTop *exitTreeTop, TR::Node *node, TR::Block *block,
    TR_Array<TR_GlobalRegister> &registers, bool regStarTransformDone) {
  TR_Array<TR::Node *> regDepNodes(trMemory(), _lastGlobalRegisterNumber + 1,
                                   true, stackAlloc);
  ;
  ...
  for (TR::Block *successorBlock = si->getFirstSuccessor();
       successorBlock != NULL; successorBlock = si->getNextSuccessor()) {
    TR::Node *caseNode;
    if (node->getOpCode().isSwitch()) {
    ...
    if (successorBlock->isExtensionOfPreviousBlock() == false) {
      prepareForBlockExit(exitTreeTop, caseNode, block, registers,
                          successorBlock, regDepNodes);
    }
  }
  ...
  if (node->getOpCode().getOpCodeValue() != TR::tstart) {
    TR::Node *exitNode =
        node->getOpCode().isSwitch() ? node->getSecondChild() : node;
...
void TR_GlobalRegisterAllocator::transformBlockExit(
  ...
  prepareForBlockExit(exitTreeTop, exitNode, block, registers, successorBlock,
                      regDepNodes);
...
/**
 * The method below is used to record the GlobalRegister to be reloaded into
 * register is successors of the passed block should we be not able to reload it
 * in the current block for reasons such as stack not being available
 */
bool TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors(
    int32_t i, TR_GlobalRegister *extReg, TR_GlobalRegister *reg,
    TR::Block *block, bool traceIt) {
  ...
  if (traceIt)
    traceMsg(comp(),
             "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors "
             "block=%d GlobalReg=(%d,symRef=#%d)\n",
             block->getNumber(), i,
             rc->getSymbolReference()->getReferenceNumber());
  // Alread visited this block?
  if (reg->isUnavailable()) return (reg->isUnavailableResolved());

  reg->setUnavailable();

  // If it is not live on exit then nothing to do
  if (reg->getRegisterCandidateOnExit() != rc &&
      !block->getNextBlock()->isExtensionOfPreviousBlock())
    return true;
  ...
  if (nextBlock && nextBlock->isExtensionOfPreviousBlock()) {
    if (traceIt)
      traceMsg(comp(),
               ...
               "nextBlock=%d\n",
               nextBlock->getNumber());
    TR_Array<TR_GlobalRegister> &nextRegisters =
        nextBlock->getGlobalRegisters(comp());
    TR_GlobalRegister &nextGr = nextRegisters[i];
    nextRc = nextGr.getRegisterCandidateOnEntry();
    if (nextRc && nextRc != rc)  // Not live anymore so leave. Should be caught
                                 // on previous test on live on exit
    {
    ...
    if (traceIt)
      traceMsg(comp(), "  block_%d marked to reload candidate #%d\n",
               ...
               rc->getSymbolReference()->getReferenceNumber());
    reg->setUnavailableResolved();
    return true;  // Yes I managed to set reload on Entry for this successor
                  // path
  } else {
    if (traceIt)
      traceMsg(comp(),
               "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors "
               "next block is not extension\n");
    // Multiple successors must visit all and set them as needing a reload on
    // entry
    for (auto succ = block->getSuccessors().begin();
         succ != block->getSuccessors().end(); ++succ) {
      ...
      if (nextRc == NULL ||
          nextRc != rc)  // not live on entry in successor, continue
        continue;
      ...
      reg->setUnavailableResolved();
      result =
          true;  // Yes I managed to set reload on Entry for this successor path
    }
  }
...
void TR_GlobalRegisterAllocator::reloadNonRegStarVariables(TR::TreeTop *tt,
                                                           TR::Node *node,
                                                           TR::Block *block,
                                                           bool traceIt) {
  TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
            "Incorrect value in _startOfExtendedBBForBB");
  TR_Array<TR_GlobalRegister> &extRegisters =
      block->startOfExtendedBlock()->getGlobalRegisters(comp());
...
void TR_GlobalRegisterAllocator::addCandidateReloadsToEntry(
    TR::TreeTop *bbStartTT, TR_Array<TR_GlobalRegister> &registers,
    TR::Block *block) {
  TR::Node *bbStart = bbStartTT->getNode();
  int32_t i;
  TR_Array<TR_GlobalRegister> &currRegisters =
      block->getGlobalRegisters(comp());

  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    if (currRegisters[i].getReloadRegisterCandidateOnEntry()) {
      TR_RegisterCandidate *candidateOnEntry =
          currRegisters[i].getRegisterCandidateOnEntry();
      currRegisters[i].setCurrentRegisterCandidate(candidateOnEntry, 0, 0, i,
                                                   comp());
      currRegisters[i].createStoreToRegister(bbStartTT, NULL, _visitCount,
                                             comp(), this);
    }
  }
}
...
void TR_GlobalRegisterAllocator::addStoresForCatchBlockLoads(
    TR::TreeTop *appendPoint, TR_Array<TR_GlobalRegister> &registers,
    TR::Block *throwingBlock) {
  if (!throwingBlock->hasExceptionSuccessors() ||
      !comp()->penalizePredsOfOSRCatchBlocksInGRA())
    return;

  // For most catch block loads, we currently just don't remove any candidate
  // stores, so there's no need to emit stores before taking the exception edge
  // because the auto always has the right value. For OSR, however, we are
  // particularly interested in eliminating those stores, so now when we reach a
  // block with an outgoing OSR exception edge, we must emit the stores.
  //
  _osrCatchSucc = NULL;
  for (auto nextEdge = throwingBlock->getExceptionSuccessors().begin();
       (nextEdge != throwingBlock->getExceptionSuccessors().end()) &&
       !_osrCatchSucc;
       ++nextEdge) {
    ...
    if (succ->asBlock()->isOSRCatchBlock()) {
      ...
      if (trace())
        traceMsg(comp(),
                 "           addStoresForCatchBlockLoads([%p], block_%d) found "
                 "OSR catch block_%d\n",
                 appendPoint->getNode(), throwingBlock->getNumber(),
                 _osrCatchSucc->getNumber());
    }
  }

  if (!_osrCatchSucc) return;

  for (int32_t i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber;
       ++i) {
    ...
    if (_osrCatchSucc && rc && !rc->isLiveAcrossExceptionEdge() &&
      ...
      if (!registers[i].getAutoContainsRegisterValue()) {
        registers[i].createStoreFromRegister(comp()->getVisitCount(),
                                             appendPoint, 1, comp(), true);
...
void TR_GlobalRegisterAllocator::addRegLoadsToEntry(
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    if (rc && !seenCandidates.find(rc) &&
        !currRegisters[i].getReloadRegisterCandidateOnEntry() &&
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    TR_RegisterCandidate *candidateOnEntry =
        registers[i].getRegisterCandidateOnEntry();
    ...
    if (!currRegisters[i].getReloadRegisterCandidateOnEntry() &&
      ...
      if (candidateOnEntry && !seenCandidates.find(candidateOnEntry)) {
        ...
        regDepNode->setAndIncChild(
            numLoads++, registers[i].createLoadFromRegister(bbStart, comp()));
      }
...
void TR_GlobalRegisterAllocator::addGlRegDepToExit(
    TR_Array<TR::Node *> &regDepNodes, TR::Node *exitNode,
    TR_Array<TR_GlobalRegister> &registers, TR::Block *block) {
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    if (regDepNodes[i]) {
      TR_RegisterCandidate *candidate =
          registers[i].getCurrentRegisterCandidate();
      // traceMsg(comp(), "real reg %d exit node %p reg dep node %p candidate
      // %d\n", i, exitNode, regDepNodes[i], (candidate ?
      // candidate->getSymbolReference()->getReferenceNumber() : 0));
      if (candidate && !seenCandidates.find(candidate) &&
          !currRegisters[i].isUnavailable()) {
  ...
  if (numLoads) {
    ...
    for (numLoads = 0, i = _firstGlobalRegisterNumber;
         i <= _lastGlobalRegisterNumber; ++i) {
      if (regDepNodes[i]) {
        TR_RegisterCandidate *candidate =
            registers[i].getCurrentRegisterCandidate();
        if (candidate && !seenCandidates.find(candidate) &&
    ...
    dumpOptDetails(comp(),
                   "%s create TR::GlRegDeps [" POINTER_PRINTF_FORMAT
                   "] on exit node [" POINTER_PRINTF_FORMAT "]\n",
...
void TR_GlobalRegisterAllocator::prepareForBlockExit(
    TR::TreeTop *&exitTreeTop, TR::Node *&exitNode, TR::Block *block,
    TR_Array<TR_GlobalRegister> &registers, TR::Block *successorBlock,
  ...
  TR_ScratchList<TR_RegisterCandidate> seenCurrCandidates(trMemory()),
      seenExitCandidates(trMemory());
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    if (successorBlock->getGlobalRegisters(comp())[i]
            .getRegisterCandidateOnEntry() != gr->getRegisterCandidateOnExit())
    ...
    if (extgr->getValue()) {
      bool mustBeLiveAcrossAllPaths =
          (extgr->getValue()->getOpCode().isFloatingPoint() &&
           !cg()->getSupportsJavaFloatSemantics());

      if ((!extgr->getAutoContainsRegisterValue() ||
           mustBeLiveAcrossAllPaths) &&
          !registerIsLiveAcrossEdge(exitTreeTop, exitNode, block, extgr,
                                    successorBlock, i)) {
        TR_RegisterCandidate *rc = extgr->getCurrentRegisterCandidate();
        TR::CFGEdge *succ;
        bool liveOnSomeSucc = true;
        if (liveOnSomeSucc && !extgr->getAutoContainsRegisterValue() &&
            !currCandidateAlreadySeen) {
          if ((block->getSuccessors().size() == 1) ||
              rc != gr->getRegisterCandidateOnExit()) {
            extgr->createStoreFromRegister(
                _visitCount, extgr->optimalPlacementForStore(block, comp()), i,
                comp());
          } else {
            if (!prevTreeTop)
              prevTreeTop =
                  findPrevTreeTop(exitTreeTop, exitNode, block, successorBlock);
        ...
        if (mustBeLiveAcrossAllPaths)
          extgr->setValue(
              0);  // must ensure the stack is cleared across the edge
      }
    }

    // create reg dep nodes
    //
    // note: successorBlock may have changed during call to
    // registerIsLiveAcrossEdge
    //
    TR_RegisterCandidate *successorRC =
        ...
        !gr->isUnavailable())  // ensure the global reg is currently available)
    {
      ...
      // if (extgr->getCurrentRegisterCandidate() != successorRC)
      {
        extgr->setCurrentRegisterCandidate(
            gr->getRegisterCandidateOnExit(), _visitCount,
        ...
        if (currRC && currRC->rcNeeds2Regs(comp())) {
          ...
          if (i == highRegNum) {
            TR_GlobalRegister *grLow =
                &extRegisters[currRC->getLowGlobalRegisterNumber()];
            grLow->setCurrentRegisterCandidate(
                gr->getRegisterCandidateOnExit(), _visitCount,
                exitTreeTop->getEnclosingBlock(),
                currRC->getLowGlobalRegisterNumber(), comp());
            // grLow->copyCurrentRegisterCandidate(gr);
          } else {
            TR_GlobalRegister *grHigh =
                &extRegisters[currRC->getHighGlobalRegisterNumber()];
            grHigh->setCurrentRegisterCandidate(
                gr->getRegisterCandidateOnExit(), _visitCount,
                exitTreeTop->getEnclosingBlock(),
                currRC->getHighGlobalRegisterNumber(), comp());
        ...
        if (extgr->getCurrentRegisterCandidate() != successorRC) {
          ...
          if (extgr->getCurrentRegisterCandidate())
            traceMsg(comp(), "reg %d current candidate %d\n", i,
                     extgr->getCurrentRegisterCandidate()
                         ->getSymbolReference()
                         ->getReferenceNumber());
          else
        ...
        TR_ASSERT(
            extgr->getCurrentRegisterCandidate() == successorRC,
      ...
      if (!extgr->getValue() && !exitCandidateAlreadySeen) {
        ...
        value = extgr->createStoreToRegister(prevTreeTop, NULL, _visitCount,
                                             comp(), this);
        bool needs2Regs = false;
        if (extgr->getCurrentRegisterCandidate() &&
      ...
      if (!exitCandidateAlreadySeen &&
          !(value->getOpCode().isLoadReg() &&
            extgr->getCurrentRegisterCandidate()->hasSameGlobalRegisterNumberAs(
                value, comp()))) {
        TR::Node *actualValue = value;
        value = TR::Node::create(TR::PassThrough, 1, value);

        if (actualValue->requiresRegisterPair(comp())) {
          value->setLowGlobalRegisterNumber(extgr->getCurrentRegisterCandidate()
                                                ->getLowGlobalRegisterNumber());
          ...
          if (currRC && currRC->getType().isInt64()) {
            int32_t highRegNum = currRC->getHighGlobalRegisterNumber();
            if (i == highRegNum) {
              regDepNodes[extgr->getCurrentRegisterCandidate()
                              ->getLowGlobalRegisterNumber()] = value;
              TR_GlobalRegister *grLow =
                  &extRegisters[extgr->getCurrentRegisterCandidate()
                                    ->getLowGlobalRegisterNumber()];
              grLow->setLastRefTreeTop(exitTreeTop);
              grLow->setValue(actualValue);
            } else {
              regDepNodes[extgr->getCurrentRegisterCandidate()
                              ->getHighGlobalRegisterNumber()] = value;
              TR_GlobalRegister *grHigh =
                  &extRegisters[extgr->getCurrentRegisterCandidate()
                                    ->getHighGlobalRegisterNumber()];
  ...
  if ((successorBlock != originalSuccessorBlock) &&
    ...
    transformBlockExit(succExit, succExit->getNode(), sb,
                       sb->getGlobalRegisters(comp()), originalSuccessorBlock);
  }
}

bool TR_GlobalRegisterAllocator::registerIsLiveAcrossEdge(
    TR::TreeTop *exitTreeTop, TR::Node *exitNode, TR::Block *block,
  ...
  TR_GlobalRegister *successorRegister =
      &successorBlock->getGlobalRegisters(comp())[i];
  if (rc == successorRegister->getRegisterCandidateOnEntry()) return true;

  if (successorBlock && !successorBlock->getEntry()) return false;

  // It may happen that all predecessors have this candidate in register on
  // exit. If there are more than one predecessor and their frequency is higher
  // than that of this block, it's better to extend live range into successor
  TR::CFGEdgeList &predecessors = successorBlock->getPredecessors();
  if (!predecessors.empty() && !(predecessors.size() == 1) &&
      successorBlock->getEntry()->getNode()->getVisitCount() !=
          comp()->getVisitCount()) {
    ...
    for (auto pred = predecessors.begin(); pred != predecessors.end(); ++pred) {
      if (!rc->getBlocksLiveOnExit().get((*pred)->getFrom()->getNumber()) ||
          toBlock((*pred)->getFrom())->getEntry()->getNode()->getVisitCount() ==
              comp()->getVisitCount()) {
    ...
    if (liveOnAllPredExits && allPredFreq >= successorBlock->getFrequency()) {
      if (trace())
        traceMsg(comp(),
                 "Extended live range of #%d into successor since candidate is "
                 "available in register on all predecessor's exits: block=%d "
                 "succ=%d allPredFreq=%d succFreq=%d\n",
                 rc->getSymbolReference()->getReferenceNumber(),
                 block->getNumber(), successorBlock->getNumber(), allPredFreq,
                 successorBlock->getFrequency());

      TR_ASSERT(!successorRegister->getRegisterCandidateOnEntry(),
                "Should not have candidate on entry");
      successorRegister->setRegisterCandidateOnEntry(rc);
      return true;
    }
  }

  // If the register isn't marked as being live across the branch then without
  // some effort we're going to slow down the fall-thru path by inserting a
  // store from the register back into the auto before the branch and/or
  // reloading the float value (on ia32) at the next use in the fall thru.
  //
  // If the target of the branch has only one predecessor then we can prepend a
  // block to the branch target, add the store there and change the branch
  // target to be the new block.
  //
  // If the target of the branch has multiple predecessors and the fallthru path
  // in known to be hotter than the branch target then we can append a new
  // block, change the branch to target the new block, end the live range in the
  // new block, and add a goto to the original block.
  //

  int32_t refNum = extgr->getCurrentRegisterCandidate()
                       ->getSymbolReference()
                       ->getReferenceNumber();

  if (!exitNode->getOpCode().isBranch() ||
      !cg()->allowGlobalRegisterAcrossBranch(rc, exitNode))
    return false;
  ...
  if (!isReferencedInFallThru) return false;

  if (needs2Regs) {
    int32_t highRegNum = rc->getHighGlobalRegisterNumber();
    if (i == highRegNum) {
      int32_t lowRegNum = rc->getLowGlobalRegisterNumber();
      if (extBlock->getGlobalRegisters(comp())[lowRegNum]
              .getCurrentRegisterCandidate() != rc)
        return false;
    } else {
      if (extBlock->getGlobalRegisters(comp())[highRegNum]
              .getCurrentRegisterCandidate() != rc)
        return false;
    }
  }

  int32_t numRegistersLiveOnSuccessor = numberOfRegistersLiveOnEntry(
      successorBlock->getGlobalRegisters(comp()), true);
  if (numRegistersLiveOnSuccessor + (needs2Regs ? 2 : 1) >
      cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block))
    return false;
  ...
  bool keepingItAliveAcrossEdgeRequiresExtraGoto =
      !ttBeforeSuccessor ||
      ttBeforeSuccessor->getNode()->getBlock()->hasSuccessor(successorBlock);

  TR_YesNoMaybe branchIsHotter = TR_no;
  // Try to decide based on cold flag
  //
  branchIsHotter = TR_maybe;
  if (successorBlock->isCold() && block->getNextBlock() &&
      !block->getNextBlock()->isCold())
    branchIsHotter = TR_no;
  else if (!successorBlock->isCold() && block->getNextBlock() &&
           block->getNextBlock()->isCold())
  ...
  if (branchIsHotter == TR_maybe) {
    if (block->getNextBlock()) {
      if (block->getNextBlock()->getFrequency() >
          successorBlock->getFrequency()) {
        if (((successorBlock->getFrequency() == 0) ||
             ((successorBlock->getFrequency() > 0) &&
              ((block->getNextBlock()->getFrequency() * 100 /
                (successorBlock->getFrequency())) > 130))))
          branchIsHotter = TR_no;
      } else if (block->getNextBlock()->getFrequency() <
                 successorBlock->getFrequency()) {
        if (((block->getNextBlock()->getFrequency() == 0) ||
             ((block->getNextBlock()->getFrequency() > 0) &&
              ((successorBlock->getFrequency() * 100 /
                (block->getNextBlock()->getFrequency())) > 130))))
          branchIsHotter = TR_yes;
      }
    }
  }

  if (!successorBlock->getExceptionPredecessors().empty()) return false;

  if (!keepingItAliveAcrossEdgeRequiresExtraGoto) {
    // If an extra branch isn't required to keep the register alive across the
    // edge then it should make sense to keep it alive across the edge unless
    // doing so introduces an extra pop on the branch successor and the branch
    // is the hotter path.
    //
    ...
    if (successorRegister->getRegisterCandidateOnEntry() != 0 ||
      ...
      TR::Block *newBlock = createNewSuccessorBlock(block, successorBlock,
                                                    exitTreeTop, exitNode, rc);
      if (trace())
    ...
    if (trace())
      traceMsg(comp(),
               "Setting candidate %d (real reg %d) on entry to succ block_%d\n",
               rc->getSymbolReference()->getReferenceNumber(), i,
               successorBlock->getNumber());
    successorBlock->getGlobalRegisters(comp())[i].setRegisterCandidateOnEntry(
        rc);
    ...
    if (needs2Regs) {
      ...
      if (i == highRegNum) {
        int32_t lowRegNum = rc->getLowGlobalRegisterNumber();
        successorBlock->getGlobalRegisters(comp())[lowRegNum]
        ...
        if (trace())
          traceMsg(
              comp(),
              "Setting candidate %d (real reg %d) on entry to succ block_%d\n",
              rc->getSymbolReference()->getReferenceNumber(), highRegNum,
              successorBlock->getNumber());
      }
    }
  ...
  // If keeping the register live across the branch requires an extra goto on
  // the branch path then only do this if the fallthru path is hotter.
  //
  if (branchIsHotter == TR_no) {
    ...
    if (!newBlock->getEntry()->getPrevTreeTop()) {
      ...
      // FIXME: appending all the goto blocks clumped up together, (near) the
      // end of the method is far from ideal.  We should try to find the first
      // break in the fall through chain from 'block' onwards, and place it
      // there instead so that the ichace is not contaminated
      //
    ...
    if (trace())
      traceMsg(comp(),
               "Setting candidate %d (real reg %d) on entry to new block_%d\n",
               rc->getSymbolReference()->getReferenceNumber(), i,
               newBlock->getNumber());
    ...
    if (needs2Regs) {
      ...
      if (i == highRegNum) {
        int32_t lowRegNum = rc->getLowGlobalRegisterNumber();
        newBlock->getGlobalRegisters(comp())[lowRegNum]
            .setRegisterCandidateOnEntry(rc);
        rc->setExtendedLiveRange(true);
        if (trace())
          traceMsg(
        ...
        newBlock->getGlobalRegisters(comp())[highRegNum]
            .setRegisterCandidateOnEntry(rc);
        rc->setExtendedLiveRange(true);
        if (trace())
          traceMsg(
...
TR::Block *TR_GlobalRegisterAllocator::extendBlock(TR::Block *block,
                                                   TR::Block *successorBlock) {
  ...
  _candidates->_startOfExtendedBBForBB[newBlock->getNumber()] =
      _candidates->_startOfExtendedBBForBB[block->getNumber()];
...
TR::Block *TR_GlobalRegisterAllocator::createNewSuccessorBlock(
    TR::Block *block, TR::Block *successorBlock, TR::TreeTop *exitTreeTop,
    TR::Node *exitNode, TR_RegisterCandidate *rc) {
  TR_Array<TR_GlobalRegister> &successorRegisters =
      successorBlock->getGlobalRegisters(comp());
  TR_Array<TR_GlobalRegister> &blockRegisters =
      block->getGlobalRegisters(comp());
  TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
            "Incorrect value in _startOfExtendedBBForBB");
  TR_Array<TR_GlobalRegister> &blockRegistersEBB =
      _candidates->_startOfExtendedBBForBB[block->getNumber()]
          ->getGlobalRegisters(comp());
  ...
  if (!_newBlocks.isEmpty()) {
    ...
    for (nextNewBlock = newBlocksIt.getFirst(); nextNewBlock != NULL;
         nextNewBlock = newBlocksIt.getNext()) {
      bool newBlockCanBeReused = true;
      TR::Block *predBlock =
          nextNewBlock->getPredecessors().front()->getFrom()->asBlock();
      ...
      if (newBlockCanBeReused) {
        TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
                  "Incorrect value in _startOfExtendedBBForBB");
        TR_Array<TR_GlobalRegister> &predRegistersEBB =
            _candidates->_startOfExtendedBBForBB[predBlock->getNumber()]
                ->getGlobalRegisters(comp());
        TR_Array<TR_GlobalRegister> &predRegisters =
            predBlock->getGlobalRegisters(comp());
        ...
        if ((nextNewBlock->getSuccessors().size() == 1) &&
          ...
          TR_Array<TR_GlobalRegister> &nextNewRegisters =
              nextNewBlock->getGlobalRegisters(comp());
          int32_t j;
          for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber;
               ++j) {
          ...
          int32_t numRegistersLiveOnNewSuccessor = numberOfRegistersLiveOnEntry(
              nextNewBlock->getGlobalRegisters(comp()), true);
          bool needs2Regs = rc->rcNeeds2Regs(comp());
          if (numRegistersLiveOnNewSuccessor + (needs2Regs ? 2 : 1) >
            ...
            if (trace())
              traceMsg(
                  ...
                  "comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block_"
                  "%d) %d\n",
                  numRegistersLiveOnNewSuccessor, nextNewBlock->getNumber(),
                  block->getNumber(),
                  comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block));
            newBlockCanBeReused = false;
          }
  ...
  if (!newBlock) {
    ...
    TR_Array<TR_GlobalRegister> &newRegisters =
        newBlock->getGlobalRegisters(comp());

    int32_t j;
    for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber; ++j) {
      if (successorRegisters[j].getRegisterCandidateOnEntry()) {
        // traceMsg(comp(), "cand on exit %p\n",
        // blockRegisters[j].getRegisterCandidateOnExit()); traceMsg(comp(),
        // "succ on enter %p\n",
        // successorRegisters[j].getRegisterCandidateOnEntry()); traceMsg(comp(),
        // "cand on ebb enter %p\n",
        ...
        TR_ASSERT(!blockRegisters[j].getRegisterCandidateOnExit() ||
                      ...
                      (blockRegisters[j].getRegisterCandidateOnEntry() ==
                       successorRegisters[j].getRegisterCandidateOnEntry()),
                  "Candidates are different in a global register across a CFG "
                  "edge\n");

        successorRegisters[j]
            .getRegisterCandidateOnEntry()
            ->setExtendedLiveRange(true);
        newRegisters[j].setRegisterCandidateOnEntry(
            successorRegisters[j].getRegisterCandidateOnEntry());
        newRegisters[j].setRegisterCandidateOnExit(
            successorRegisters[j].getRegisterCandidateOnEntry());
      }
    }
  } else {
  ...
  if (exitTreeTop->getNode()->getOpCode().isSwitch()) {
    ...
    for (i = exitTreeTop->getNode()->getCaseIndexUpperBound() - 1; i > 0; --i) {
      ...
      if (child->getBranchDestination()->getNode()->getBlock() ==
          successorBlock)
...
TR::Block *TR_GlobalRegisterAllocator::createBlock(TR::Block *block,
                                                   TR::Block *successorBlock) {
  ...
  TR::Block *newBlock = TR::Block::createEmptyBlock(
      block->getExit()->getNode(), comp(), block->getFrequency(), block);
  ...
  if (block->isCold() || successorBlock->isCold()) {
    ...
    newBlock->setFrequency(
        TR::Block::getMinColdFrequency(block, successorBlock));
  }

  TR::CFG *cfg = comp()->getFlowGraph();
  cfg->addNode(newBlock,
               block->getCommonParentStructureIfExists(successorBlock, cfg));
  ...
  if (_candidates->_startOfExtendedBBForBB[successorBlock->getNumber()] ==
          block &&
      block != successorBlock)
    _candidates->_startOfExtendedBBForBB[successorBlock->getNumber()] =
        newBlock;
...
int32_t TR_GlobalRegisterAllocator::numberOfRegistersLiveOnEntry(
    TR_Array<TR_GlobalRegister> &registers, bool countMachineRegs) {
  int32_t numLoads = 0;
  TR_ScratchList<TR_RegisterCandidate> seenCandidates(trMemory());
  int32_t i;
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    if (rc && !seenCandidates.find(rc) && !registers[i].isUnavailable()) {
      seenCandidates.add(rc);
      ++numLoads;
      if (countMachineRegs && rc->rcNeeds2Regs(comp())) ++numLoads;
    }
  }
...
TR_GlobalRegister *
TR_GlobalRegisterAllocator::getGlobalRegisterWithoutChangingCurrentCandidate(
    TR::Symbol *symbol, TR_Array<TR_GlobalRegister> &registers,
    TR::Block *block) {
  TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
            "Incorrect value in _startOfExtendedBBForBB");
  TR::Block *startOfExtendedBlock =
      _candidates->_startOfExtendedBBForBB[block->getNumber()];
  TR_Array<TR_GlobalRegister> &extRegisters =
      startOfExtendedBlock->getGlobalRegisters(comp());
  int32_t i;
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    TR_GlobalRegister *gr = &registers[i];
    TR_GlobalRegister *extgr = &extRegisters[i];
      ...
      if (!secondRc || secondRc->getSymbolReference()->getSymbol() != symbol) {
        ...
        if (!thirdRc || thirdRc->getSymbolReference()->getSymbol() != symbol)
          ...
          TR_ASSERT(!symbol->isInGlobalRegister(),
                    "Current candidate is not live on entry and not live on "
...
TR_GlobalRegister *TR_GlobalRegisterAllocator::getGlobalRegister(
    TR::Symbol *symbol, TR_Array<TR_GlobalRegister> &registers,
    TR::Block *block) {
  TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(),
            "Incorrect value in _startOfExtendedBBForBB");
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    if (!firstRc || firstRc->getSymbolReference()->getSymbol() != symbol) {
      ...
      if (!secondRc || secondRc->getSymbolReference()->getSymbol() != symbol) {
        ...
        if (!thirdRc || thirdRc->getSymbolReference()->getSymbol() != symbol)
          ...
          TR_ASSERT(false,
                    "Current cand is not live on entry and not live on exit "
                    "from basic block\n");
      }
    }
    ...
    if (rc->rcNeeds2Regs(comp())) {
      ...
      if (i == highRegNum) {
        int32_t lowRegNum = rc->getLowGlobalRegisterNumber();
        TR_GlobalRegister *grLow = &extRegisters[lowRegNum];
        grLow->setCurrentRegisterCandidate(rc, _visitCount, block, lowRegNum,
                                           comp());
        // grLow.copyCurrentRegisterCandidate(gr);
      } else {
        TR_GlobalRegister *grHigh = &extRegisters[highRegNum];
        grHigh->setCurrentRegisterCandidate(rc, _visitCount, block, highRegNum,
                                            comp());
        // grHigh.copyCurrentRegisterCandidate(gr);
      }
    }
...
bool TR_GlobalRegister::getAutoContainsRegisterValue() {
...
void TR_GlobalRegister::setCurrentRegisterCandidate(
    TR_RegisterCandidate *rc, vcount_t visitCount, TR::Block *currentBlock,
    int32_t i, TR::Compilation *comp, bool resetOtherHalfOfLong) {
  if (_rcCurrent != rc) {
    bool liveOnSomeSucc = true;
    if (_rcCurrent && getValue() && !getAutoContainsRegisterValue()) {
      if (liveOnSomeSucc)
        createStoreFromRegister(
            visitCount, optimalPlacementForStore(currentBlock, comp), i, comp);
    }
    if (_rcCurrent)
      _rcCurrent->getSymbolReference()->getSymbol()->setIsInGlobalRegister(
          false);

    if (resetOtherHalfOfLong && _rcCurrent != NULL &&
        _rcCurrent->rcNeeds2Regs(comp)) {
      //  TR_ASSERT(currentBlock->startOfExtendedBlock() ==
      //  _candidates->_startOfExtendedBBForBB[currentBlock->getNumber()],
      //  "Incorrect value in _startOfExtendedBBForBB");
      ...
      if (i == highRegNum) {
        ...
        TR_GlobalRegister *grLow = &extRegisters[lowRegNum];
        grLow->setCurrentRegisterCandidate(NULL, visitCount, currentBlock,
                                           lowRegNum, comp, false);
        // grLow.copyCurrentRegisterCandidate(gr);
      } else {
        TR_GlobalRegister *grHigh = &extRegisters[highRegNum];
        grHigh->setCurrentRegisterCandidate(NULL, visitCount, currentBlock,
                                            highRegNum, comp, false);
        // grHigh.copyCurrentRegisterCandidate(gr);
      }
    }

    _rcCurrent = rc;
    setValue(0);  // regardless of whether or not we created a store we must
                  // setValue to NULL, else we can use the wrong value

    if (currentBlock) comp->setCurrentBlock(currentBlock);
    if (rc && (rc->getSymbol()->dontEliminateStores(comp) || keepAllStores ||
...
void TR_GlobalRegister::copyCurrentRegisterCandidate(TR_GlobalRegister *gr) {
...
TR::TreeTop *TR_GlobalRegister::optimalPlacementForStore(
    TR::Block *currentBlock, TR::Compilation *comp) {
  bool traceGRA =
      comp->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator);
  if (traceGRA)
  ...
  if (lastRefBlock != currentBlock) {
    ...
    if (prevOpCode.isBranch() || prevOpCode.isJumpWithMultipleTargets() ||
        prevOpCode.isReturn() || prevOpCode.getOpCodeValue() == TR::athrow ||
  ...
  if (lastRefBlock == currentBlock) {
    if (traceGRA)
      traceMsg(comp,
               "           - lastRefBlock == currentBlock: returning [%p]\n",
...
TR::Node *TR_GlobalRegister::createLoadFromRegister(TR::Node *n,
                                                    TR::Compilation *comp) {
  ...
  if (!rc->is8BitGlobalGPR()) load->setIsInvalid8BitGlobalRegister(true);
  setValue(load);
  if (load->requiresRegisterPair(comp))
    dumpOptDetails(comp,
                   "%s create load [%p] from Register %d (low word) and "
                   "Register %d (high word)\n",
                   OPT_DETAILS, load, rc->getLowGlobalRegisterNumber(),
...
TR::Node *TR_GlobalRegister::createStoreToRegister(
    TR::TreeTop *prevTreeTop, TR::Node *node, vcount_t visitCount,
  ...
  if (node) {
    ...
    load = TR::Node::createWithSymRef(n, comp->il.opCodeForDirectLoad(dt), 0,
                                      rc->getSymbolReference());
  }
  ...
  if (NULL != doit) enableSignExtGRA = true;

  if (TR::Compiler->target.cpu.isZ()) {
    enableSignExtGRA = true;
    static char *doit2 = feGetEnv("TR_NSIGNEXTGRA");
    if (NULL != doit2) enableSignExtGRA = false;
  }

  if (TR::Compiler->target.is64Bit() &&
      (store->getOpCodeValue() == TR::iRegStore) &&
      gra->candidateCouldNeedSignExtension(
          rc->getSymbolReference()->getReferenceNumber()) &&
      enableSignExtGRA) {
    store->setNeedsSignExtension(true);
  }
  ...
  if (store->requiresRegisterPair(comp))
    dumpOptDetails(
        comp,
        "%s create store [%p] of symRef#%d to Register %d (low word) and "
        "Register %d (high word)\n",
        OPT_DETAILS, store, rc->getSymbolReference()->getReferenceNumber(),
        rc->getLowGlobalRegisterNumber(), rc->getHighGlobalRegisterNumber());
  else
    dumpOptDetails(comp,
                   "%s create store [%p] of %s symRef#%d to Register %d\n",
                   OPT_DETAILS, store,
                   rc->getSymbolReference()->getSymbol()->isMethodMetaData()
                       ? rc->getSymbolReference()
                             ->getSymbol()
...
TR::Node *TR_GlobalRegister::createStoreFromRegister(
    vcount_t visitCount, TR::TreeTop *prevTreeTop, int32_t i,
    TR::Compilation *comp, bool storeUnconditionally) {
  if (!storeUnconditionally) {
    TR_ASSERT(!getAutoContainsRegisterValue(),
              "don't need to store the register back into the auto");
  ...
  if (!prevTreeTop) prevTreeTop = getLastRefTreeTop();
  ...
  TR::Node *store = TR::Node::createWithSymRef(
      comp->il.opCodeForDirectStore(rc->getDataType()), 1, 1, node,
      rc->getSymbolReference());
  ...
  if (i != -1) {
    if (store->requiresRegisterPair(comp))
      dumpOptDetails(comp,
                     "%s create store [%p] from Register %d (low word) and "
                     "Register %d (high word)\n",
                     OPT_DETAILS, store, rc->getLowGlobalRegisterNumber(),
                     rc->getHighGlobalRegisterNumber());
    else
      dumpOptDetails(comp, "%s create store [%p] from Register %d for %s #%d\n",
                     OPT_DETAILS, store, rc->getGlobalRegisterNumber(),
                     rc->getSymbolReference()->getSymbol()->isMethodMetaData()
                         ? rc->getSymbolReference()
                               ->getSymbol()
                               ->castToMethodMetaDataSymbol()
...
void TR_GlobalRegisterAllocator::findIfThenRegisterCandidates() {
  LexicalTimer t("TR_GlobalRegisterAllocator::findIfThenRegisterCandidates",
                 comp()->phaseTimer());
  ...
  ListIterator<TR::ParameterSymbol> paramIterator(
      &(methodSymbol->getParameterList()));
  ListIterator<TR::AutomaticSymbol> autoIterator(
      &(methodSymbol->getAutomaticList()));
  ...
  // It is visiting all parameters and locals and ensuring that the candidate
  // has a BlockInfo entry if it is live in any block.
  if (!debug("oldIfThen")) {
    LexicalTimer t("newFindIfThen", comp()->phaseTimer());

    // First create a bit vector of auto and parm symbols
    // Guess at size of bit vector to use by getting first block's LiveLocals
    // bitvector and check its size
    ...
    TR_BitVector autoAndParmLiveLocalIndex(guessSize, trMemory(), stackAlloc,
                                           growable);
    TR_Array<TR_RegisterCandidate *> registerCandidateByIndex(
        trMemory(), guessSize, false, stackAlloc);
    autoAndParmLiveLocalIndex.empty();
    int32_t i;
    while (paramCursor != NULL) {
      if (paramCursor->isReferencedParameter()) {
        TR_RegisterCandidate *rc =
            comp()->getGlobalRegisterCandidates()->find(paramCursor);
        if (!rc) {
    ...
    while (autoCursor != NULL) {
      TR_RegisterCandidate *rc =
          comp()->getGlobalRegisterCandidates()->find(autoCursor);
      if (!rc) {
    ...
    // Now visit all blocks and intersect each blocks LiveLocals with
    // autoAndParmLiveLocalIndex. For each intersected bit ensure BlockInfo
    // exists for the candidate and initialize it to zero NumberOfLoadsAndStores
    ...
    for (TR::CFGNode *block = cfg->getFirstNode(); block;
         block = block->getNext()) {
      TR_BitVector *liveLocals = toBlock(block)->getLiveLocals();
      if (cg()->getLiveLocals() && liveLocals) {
        if (block != comp()->getStartBlock() && block != cfg->getStart() &&
            block != cfg->getEnd()) {
          ...
          while (bvi.hasMoreElements()) {
            ...
            if (!rc->_blocks.find(block->getNumber()))
              rc->_blocks.setNumberOfLoadsAndStores(toBlock(block)->getNumber(),
                                                    0);
          }
        }
      }
    ...
    while (paramCursor != NULL) {
      if (paramCursor->isReferencedParameter()) {
        ...
        for (TR::CFGNode *block = cfg->getFirstNode(); block;
          ...
          TR_BitVector *liveLocals = toBlock(block)->getLiveLocals();
          if (cg()->getLiveLocals() && liveLocals &&
              !rc->_blocks.find(block->getNumber())) {
            if (block != comp()->getStartBlock() && block != cfg->getStart() &&
                block != cfg->getEnd() &&
  ...
  for (TR::CFGNode *block = cfg->getFirstNode(); block;
    ...
    if ((edges.size() == 2) && currBlock->getExit()) {
      ...
      if (mergeBlock1) {
        if (block2->hasSuccessor(mergeBlock1)) mergeBlock2 = mergeBlock1;
      } else {
      ...
      {
        // TR::Block * mergeBlock1 =
        // toBlock(block1->getSuccessors().front()->getTo()); TR::Block *
        // mergeBlock2 = toBlock(block2->getSuccessors().front()->getTo());
        if (mergeBlock1 && mergeBlock1 == mergeBlock2 &&
            mergeBlock1 != cfg->getEnd()) {
          for (TR::TreeTop *tt1 = block1->getEntry(); tt1 != block1->getExit();
               tt1 = tt1->getNextTreeTop()) {
            TR::Node *storeNode1 = tt1->getNode()->getStoreNode();
            if (storeNode1 && storeNode1->getOpCode().isStoreDirect()) {
              TR_RegisterCandidate *rc = 0;
              TR::SymbolReference *symRef =
                  tt1->getNode()->getSymbolReference();
              if ((symRef->getSymbol()->isAutoOrParm()) &&
                  isSymRefAvailable(symRef))
                for (TR::TreeTop *tt2 = block2->getEntry();
                     tt2 != block2->getExit(); tt2 = tt2->getNextTreeTop()) {
                  ...
                  if (storeNode2 && storeNode2->getOpCode().isStoreDirect() &&
                    ...
                    rc = comp()->getGlobalRegisterCandidates()->findOrCreate(
                        symRef);
                    break;
                  }
                }
              if (rc) {
                int32_t weight = 3;
                if (mergeBlock1->findFirstReference(symRef->getSymbol(),
                                                    comp()->incVisitCount())) {
                ...
                if (toBlock(block)->findFirstReference(
                        symRef->getSymbol(), comp()->incVisitCount())) {
                  weight = 1;
      ...
      if (virtualGuard->isTheVirtualGuardForAGuardedInlinedCall()) {
        TR::Block *branchBlock =
            virtualGuard->getBranchDestination()->getNode()->getBlock();
        ...
        for (; tt1 != currBlock->getExit(); tt1 = tt1->getNextTreeTop()) {
          ...
          if (storeNode1 && storeNode1->getOpCode().isStoreDirect()) {
            ...
            if (rc) {
              int32_t weight = 1;
              if (branchBlock->findFirstReference(symRef->getSymbol(),
                                                  comp()->incVisitCount())) {
...
void TR_GlobalRegisterAllocator::offerAllAutosAndRegisterParmAsCandidates(
  ...
  LexicalTimer t(
      "TR_GlobalRegisterAllocator::offerAllAutosAndRegisterParmsAsCandidates",
      comp()->phaseTimer());
  ...
  TR::CFG *cfg = comp()->getFlowGraph();
  TR::CFGNode *node;
  TR::Block *block, *startBlock = toBlock(cfg->getStart()),
                    *endBlock = toBlock(cfg->getEnd());
  ...
  // Interested blocks consist of all blocks except for entry, exit and
  // exception handlers
  TR_BitVector interestedBlocks(numberOfNodes,
                                comp()->trMemory()->currentStackRegion());
  ...
  //
  for (TR::ParameterSymbol *paramCursor = paramIterator.getFirst();
       paramCursor != NULL; paramCursor = paramIterator.getNext()) {
    ...
    if ((paramCursor->isReferencedParameter() && isTypeAvailable(symRef)) &&
      ...
      if (!isSymRefAvailable(symRef)) continue;
      ...
      if (paramCursor->getLinkageRegisterIndex() >= 0) rc->addAllBlocks();
    }
  }
  ...
  // Offer all autos now
  //
  for (int32_t symRefNumber = symRefTab->getIndexOfFirstSymRef();
      ...
      if (sym) {
        if (candidates->aliasesPreventAllocation(comp(), symRef)) {
          if (comp()->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator))
            traceMsg(comp(),
                     "Leaving candidate #%d because it has use_def_aliases\n",
                     symRefNumber);
          continue;
        }
        ...
        if ((sym->isAuto() &&
          ...
          TR_RegisterCandidate *rc =
              comp()->getGlobalRegisterCandidates()->findOrCreate(symRef);
          if (sym->isMethodMetaData() && rc &&
              rc->initialBlocksWeightComputed())
            continue;
...
void TR_GlobalRegisterAllocator::offerAllFPAutosAndParmsAsCandidates(
    TR::Block **cfgBlocks, int32_t numberOfNodes) {
  LexicalTimer t(
      "TR_GlobalRegisterAllocator::offerAllFPAutosAndParmsAsCandidates",
      comp()->phaseTimer());
      ...
      if (sym) {
        if ((sym->getDataType() == TR::Float || sym->getDataType() == TR::Double
            ...
            ((sym->isAuto() &&
              methodSymbol->getAutomaticList().find(sym->castToAutoSymbol())) ||
             (sym->isParm() &&
              methodSymbol->getParameterList().find(sym->castToParmSymbol()) &&
...
void TR_GlobalRegisterAllocator::findLoopAutoRegisterCandidates() {
  LexicalTimer t("TR_GlobalRegisterAllocator::findLoopAutoRegisterCandidates",
                 comp()->phaseTimer());
  ...
  SymRefCandidateMap *registerCandidates =
      new (trStackMemory()) SymRefCandidateMap(
          (SymRefCandidateMapComparator()),
          SymRefCandidateMapAllocator(trMemory()->currentStackRegion()));
  findLoopsAndCorrespondingAutos(NULL, visitCount, *registerCandidates);
}

TR_GlobalRegisterAllocator::BlockInfo &TR_GlobalRegisterAllocator::blockInfo(
    int32_t i) {
...
void TR_GlobalRegisterAllocator::findLoopsAndCorrespondingAutos(
  ...
  if (structure->asRegion()) {
    ...
    if (!regionStructure->isAcyclic() && structureNode) {
      ...
      TR_BitVector assignedAutosInCurrentLoop(_origSymRefCount, trMemory(),
                                              stackAlloc);
      ...
      bool excludeInvariantsEnabled =
          comp()->cg()->excludeInvariantsFromGRAEnabled() &&
          (!comp()->getOption(TR_DisableRXusage));

      if (excludeInvariantsEnabled) {
        symsThatShouldNotBeAssignedInCurrentLoop = new (trStackMemory())
            TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
        symsThatShouldNotBeAssignedInCurrentLoop->setAll(_origSymRefCount);
        symsThatShouldBeAssignedInCurrentLoop = new (trStackMemory())
            TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
      }
      ...
      for (nextBlock = blocksIt.getFirst(); nextBlock;
           nextBlock = blocksIt.getNext()) {
        if (nextBlock->isCatchBlock()) hasCatchBlock = true;
      }

      TR_BitVector *oldCandidatesSignExtendedInThisLoop = NULL;
      if (_candidatesSignExtendedInThisLoop) {
        oldCandidatesSignExtendedInThisLoop = new (trStackMemory())
            TR_BitVector(_origSymRefCount, trMemory(), stackAlloc);
      ...
      for (nextBlock = blocksIt.getFirst(); nextBlock;
        ...
        if (nextBlock->getVisitCount() != visitCount) {
          ...
          while (currentTree != exitTree) {
            ...
            markAutosUsedIn(currentNode, NULL, NULL, &arrayAccess, nextBlock,
                            &blocksInLoop, visitCount, executionFrequency,
                            registerCandidates, &assignedAutosInCurrentLoop,
      ...
      for (auto succ = structureNode->getSuccessors().begin();
        ...
        if (exitStructure) exitBlock = exitStructure->getEntryBlock();

        // Turning off this code below. Reason : extending the live range till
        // the exit blocks may lead to slowdowns if the loop (whose exit we are
        // extending the live range to) was actually run for only a few
        // iterations and the outer loop was in fact long running. Making the
        // candidate live on entry to an exit out of the inner loop may cause it
        // to be loaded up into the register on some hot path in the outer loop
        // and this extra load could degrade performance. This scenario actually
        // occurred in db shell sort, where the innermost loop is String
        // compareTo code which does not run too long, but the outer loops are
        // very hot.
        //
        if (0 && exitBlock && (exitBlock != comp()->getFlowGraph()->getEnd())) {
          ...
          while (bvi.hasMoreElements()) {
            ...
            // dumpOptDetails(comp(), "For loop %d exit block_%d candidate
            // %d\n", structureNode->getNumber(), exitBlock->getNumber(),
            // nextCandidate);
          ...
          if (!symsThatShouldBeAssignedInCurrentLoop->get(nextCandidate)) {
            *_temp2 = *symsThatShouldBeAssignedInCurrentLoop;
            ...
            for (p = pairs.getFirst(); p; p = pairs.getNext()) {
              ...
              if (otherSymRef && symsThatShouldNotBeAssignedInCurrentLoop->get(
                                     otherSymRef->getReferenceNumber())) {
                TR_RegisterCandidate *otherCandidate =
                    registerCandidates[otherSymRef->getReferenceNumber()];
                if (otherCandidate->countNumberOfLoadsAndStoresInBlocks(
                        &blocksInLoop) <
                    rc->countNumberOfLoadsAndStoresInBlocks(&blocksInLoop))
                  symsThatShouldBeAssignedInCurrentLoop->set(
                      otherSymRef->getReferenceNumber());
                else
...
bool TR_GlobalRegisterAllocator::isDependentStore(
    TR::Node *node, const TR_UseDefInfo::BitVector &defs,
...
void TR_GlobalRegisterAllocator::markAutosUsedIn(
    TR::Node *node, TR::Node *parent, TR::Node *grandParent,
    TR::Node **currentArrayAccess, TR::Block *block,
    List<TR::Block> *blocksInLoop, vcount_t visitCount,
    int32_t executionFrequency, SymRefCandidateMap &registerCandidates,
  ...
  if (node->getOpCode().isLoadVarDirect() &&
    ...
    if (TR::Compiler->target.is64Bit() && info &&
      ...
      _candidatesNeedingSignExtension->set(
          node->getSymbolReference()->getReferenceNumber());
      ...
      if (info->getUseDef(defs, useIndex)) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          ...
          if (defIndex <
              info->getFirstRealDefIndex())  // def is  unseen--can't mark this
                                             // node as skippable
          {
            node->setSkipSignExtension(false);
            // printf("Parm def, so cannot skip sign extension at node %p in
            // %s\n",
          ...
          if (defNode->getOpCode().isStore()) {
            ...
            // TR_ASSERT((sym == node->getSymbolReference()->getSymbol()),
            // "Symbols must match between a use and a def\n"); Comment out the
            // above assume because:
            //   - The assume came from JAVA where there could not be aliasing
            //   with autos
            //   - If there is an alias causing a different sym then we will not
            //   have a candidate anyways so it does not matter
            //   - avoid extra computation if symbols do not match by adding
            //   extra condition below.
            if (sym == node->getSymbolReference()->getSymbol() &&
                (sym->isAuto())) {
              bool seenLoad = false;
              bool dependentStore = isDependentStore(
                  defNode, defs, node->getSymbolReference(), &seenLoad);
              if (dependentStore && defNode->getFirstChild()->isNonNegative() &&
                  seenLoad) {
                ...
                TR_BlockStructure *defBlockStructure =
                    defBlock->getStructureOf();
                if (comp()->getFlowGraph()->getStructure() && blockStructure &&
                    defBlockStructure &&
                    (blockStructure->getContainingLoop() ==
  ...
  if (node->getOpCode().isLoadVarDirect() ||
    ...
    if (symRef->getSymbol()->isAutoOrParm() &&
      ...
      if (!assignedAutosInCurrentLoop->get(symRef->getReferenceNumber())) {
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock;
          ...
          if (!rc->hasBlock(nextBlock)) {
            if (nextBlock != cfg->getStart()) rc->addBlock(nextBlock, 0);
          }
        }
      }

      // dumpOptDetails(comp(), "Adding %d numLoadsStores to candidate %d in
      // block_%d\n", executionFrequency,
      // rc->getSymbolReference()->getReferenceNumber(), block->getNumber());

      if (block != cfg->getStart()) {
        // We should not consider the cost of the copies for live-range
        // splitting, because this cost is meaningful only when one of operand
        // (source or destination) receives a register. Note: Although we need
        // to re-calculate the cost when one of operand receives a register, we
        // do not do so currently.
        //
        if ((node->getOpCode().isStoreDirect() && isSplittingCopy(node)) ||
            (node->getOpCode().isLoadVarDirect() && parent &&
             parent->getOpCode().isStoreDirect() && isSplittingCopy(parent))) {
          rc->addBlock(block, 0);
        } else {
          if (TR::Compiler->target.cpu.isZ() &&
              rc->getSymbolReference()->getSymbol()->getDataType() ==
                  TR::Address &&
              parent &&
            ...
            if (trace())
              dumpOptDetails(comp(),
                             "Increased weight of candidate #%d in block_%d to "
                             "reduce AGI\n",
                             rc->getSymbolReference()->getReferenceNumber(),
                             block->getNumber());
          } else {
      ...
      if (((node->getReferenceCount() > 1) ||
           node->getOpCode().isStoreDirect()) &&
          symsThatShouldNotBeAssignedInCurrentLoop)
        symsThatShouldNotBeAssignedInCurrentLoop->reset(
        ...
        if (parent->getNumChildren() == 2) {
          if (parent->getSecondChild()->getOpCode().isLoadConst())
            ...
            if (otherChild->getOpCode().isLoadVarDirect() &&
              ...
              findOrCreatePairedSymbols(symRef,
                                        otherChild->getSymbolReference());
          }
        }
      }
    }
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    ...
    markAutosUsedIn(node->getChild(childNum), node, parent, currentArrayAccess,
                    block, blocksInLoop, visitCount, executionFrequency,
                    registerCandidates, assignedAutosInCurrentLoop,
  ...
  if (symsThatShouldNotBeAssignedInCurrentLoop && assignedAutosInCurrentLoop &&
      ((opCode.hasSymbolReference() && opCode.isIndirect()) ||
       (opCode.isArrayLength()) ||
       !(opCode.isAdd() || opCode.isSub() || opCode.isMul() ||
         opCode.isBooleanCompare() || opCode.isNullCheck() ||
         opCode.isBndCheck())))
    findSymsUsedInIndirectAccesses(node,
                                   symsThatShouldNotBeAssignedInCurrentLoop,
                                   assignedAutosInCurrentLoop, true);
}

void TR_GlobalRegisterAllocator::signExtendAllDefNodes(
    TR::Node *defNode, List<TR::Node> *defNodes) {
  LexicalTimer t("TR_GlobalRegisterAllocator::signExtendAllDefNodes",
                 comp()->phaseTimer());
...
void TR_GlobalRegisterAllocator::findSymsUsedInIndirectAccesses(
    TR::Node *node, TR_BitVector *symsThatShouldNotBeAssignedInCurrentLoop,
    TR_BitVector *assignedAutosInCurrentLoop, bool examineChildren) {
  if (symsThatShouldNotBeAssignedInCurrentLoop &&
  ...
  *_temp2 &= *assignedAutosInCurrentLoop;
  ...
  if (examineChildren && (node->getNumChildren() > 0) && !_temp2->isEmpty()) {
    ...
    int32_t i;
    for (i = 0; i < node->getNumChildren(); ++i)
      findSymsUsedInIndirectAccesses(node->getChild(i),
                                     symsThatShouldNotBeAssignedInCurrentLoop,
...
TR::Block *TR_GlobalRegisterAllocator::getAppendBlock(TR::Block *block) {
  if (_appendBlock) return _appendBlock;

  TR::Block *inBlock = block;

  // Find the last block that is not a cold block, and does not fall into the
  // next block, or if none such, the first cold block that does not fall into
  // the next
  //
  TR::Block *prevBlock = block->getPrevBlock();
  ;
  if (block->isCold())
    return _appendBlock = comp()
                              ->getMethodSymbol()
                              ->getLastTreeTop()
                              ->getNode()
                              ->getBlock();
...
void TR_GlobalRegisterAllocator::appendGotoBlock(TR::Block *gotoBlock,
                                                 TR::Block *curBlock) {
  ...
  if (nextBlock) gotoBlock->getExit()->join(nextBlock->getEntry());
  _appendBlock = gotoBlock;
}

StoresInBlockInfo *TR_GlobalRegisterAllocator::findRegInStoreInfo(
    TR_GlobalRegister *gr) {
  StoresInBlockInfo *nextStoreInfo;
  for (nextStoreInfo = _storesInBlockInfo.getFirst(); nextStoreInfo;
       nextStoreInfo = nextStoreInfo->getNext()) {
    if (nextStoreInfo->_gr == gr) return nextStoreInfo;
  }
  return NULL;
}
      ...
      for (nextBlock = blocksIt.getFirst(); nextBlock;
        ...
        if (nextBlock->getVisitCount() != visitCount) {
          ...
          while (currentTree != exitTree) {
            TR::Node *currentNode = currentTree->getNode();
            TR::Node *arrayAccess = NULL;
            createStoresForSignExt(currentNode, NULL, NULL, currentTree,
                                   &arrayAccess, nextBlock, &blocksInLoop,
                                   visitCount, false);
            currentTree = currentTree->getNextRealTreeTop();
          }
        }
...
void TR_GlobalRegisterAllocator::createStoresForSignExt(
    TR::Node *node, TR::Node *parent, TR::Node *grandParent,
    TR::TreeTop *treetop, TR::Node **currentArrayAccess, TR::Block *block,
    List<TR::Block> *blocksInLoop, vcount_t visitCount, bool hasCatchBlock) {
  LexicalTimer t("TR_GlobalRegisterAllocator::createStoresForSignExt",
                 comp()->phaseTimer());
  ...
  int32_t childNum;
  for (childNum = 0; childNum < node->getNumChildren(); childNum++)
    createStoresForSignExt(node->getChild(childNum), node, parent, treetop,
                           currentArrayAccess, block, blocksInLoop, visitCount,
                           hasCatchBlock);
}

const char *TR_GlobalRegisterAllocator::optDetailString() const throw() {
...
int32_t TR_LiveRangeSplitter::perform() {
  if (!comp()->getOption(TR_EnableRangeSplittingGRA)) return 0;

  if (!cg()->prepareForGRA()) return 0;
...
/**
 * @todo FIXME: Do live range splitting before GRA pass so that the use/def,
 * liveness etc. are computed after we have created the new (split) variables
 */
void TR_LiveRangeSplitter::splitLiveRanges() {
  ...
  if (!cg()->getLiveLocals()) {
    ...
    ListIterator<TR::AutomaticSymbol> locals(
        &comp()->getMethodSymbol()->getAutomaticList());
    for (p = locals.getFirst(); p != NULL; p = locals.getNext()) ++numLocals;
    ...
    if (numLocals > 0 &&
      ...
      // Perform liveness analysis
      //
      TR_Liveness liveLocals(comp(), optimizer(),
  ...
  if (trace())
    comp()->dumpMethodTrees("Trees before live range splitter ",
                            comp()->getMethodSymbol());

  //_origSymRefs = (TR::SymbolReference
  //**)trMemory()->allocateStackMemory(comp()->getSymRefCount()*sizeof(TR::SymbolReference
  //*)); memset(_origSymRefs, 0,
  ...
  if (_changedSomething) cg()->setLiveLocals(NULL);
}

static bool canSplit(TR::SymbolReference *symRef, TR::Compilation *comp) {
  if (symRef->getSymbol()->getDataType() == TR::Aggregate) return false;

  bool addr_taken = false;

  // FIXME : refine below condition some more to allow these kinds of
  // autos/parms
  //
  if (symRef->getSymbol()->isAutoOrParm() && !addr_taken &&
...
void TR_LiveRangeSplitter::splitLiveRanges(
    TR_StructureSubGraphNode *structureNode) {
  ...
  if (structure->asRegion()) {
    ...
    if (regionStructure->isNaturalLoop() && structureNode) {
      ...
      if (!dontAssignInColdBlocks(comp()) ||
        ...
        if ((structureNode->getPredecessors().size() == 1)) {
          TR_StructureSubGraphNode *loopInvariantNode = toStructureSubGraphNode(
              structureNode->getPredecessors().front()->getFrom());
      ...
      if (loopInvariantBlock) {
        ...
        TR_BitVector *replacedAutosInCurrentLoop = new (trStackMemory())
            TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);
        TR_BitVector *autosThatCannotBeReplacedInCurrentLoop =
            new (trStackMemory())
                TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);
        _storedSymRefs = new (trStackMemory())
            TR_BitVector(comp()->getSymRefCount(), trMemory(), stackAlloc);
        ...
        TR_BitVector *exitsFromCurrentLoop = new (trStackMemory())
            TR_BitVector(comp()->getFlowGraph()->getNextNodeNumber(),
                         trMemory(), stackAlloc);
        for (auto succ = structureNode->getSuccessors().begin();
             succ != structureNode->getSuccessors().end(); ++succ)
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock;
             nextBlock = blocksIt.getNext()) {
          for (auto succ = nextBlock->getSuccessors().begin();
               succ != nextBlock->getSuccessors().end(); ++succ) {
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock;
          ...
          if (nextBlock->getVisitCount() != visitCount) {
            ...
            while (currentTree != exitTree) {
              TR::Node *currentNode = currentTree->getNode();
              replaceAutosUsedIn(currentTree, currentNode, NULL, nextBlock,
                                 &blocksInLoop, &exitBlocks, visitCount,
                                 executionFrequency, *registerCandidates,
                                 correspondingSymRefs,
                                 replacedAutosInCurrentLoop,
                                 autosThatCannotBeReplacedInCurrentLoop,
                                 structureNode, loopInvariantBlock);
              currentTree = currentTree->getNextRealTreeTop();
            }
          }
        }
        ...
        /// if (trace())
        traceMsg(comp(),
                 "Trying to split unused locals in loop %d with starting sym "
                 "ref count %d\n",
        ...
        while ((i < symRefCount) && (prevSymRefCount == 0)) {
          ...
          if (i < prevSymRefCount) origSymRef = prevOrigSymRefs[i];

          if (!origSymRef) origSymRef = symRef;
          ...
          if (origSymRef && origSymRef->getSymbol()->isAutoOrParm() &&
              ...
              canSplit(symRef, comp()) && !replacedAutosInCurrentLoop->get(i) &&
              !autosThatCannotBeReplacedInCurrentLoop->get(i)) {
            TR_RegisterCandidate *rc =
                (*registerCandidates)[origSymRef->getReferenceNumber()];
            if (!rc) {
            ...
            // if (rc)
            {
              ...
              TR::Block *exitBlock;
              for (exitBlock = exitBlocksIt.getCurrent(); exitBlock;
                   exitBlock = exitBlocksIt.getNext()) {
                TR_BitVector *liveLocals = exitBlock->getLiveLocals();
                if (liveLocals && origSymRef->getSymbol()->getAutoSymbol() &&
                    liveLocals->get(origSymRef->getSymbol()
                                        ->getAutoSymbol()
                                        ->getLiveLocalIndex()))
          ...
          if (candidateIsLiveOnExit) {
            ...
            if ((symRef->getSymbol()->getType().isInt64() &&
            ...
            if (((!isFloat && (_numberOfGPRs + numRegsForCandidate) <=
                                  comp()->cg()->getNumberOfGlobalGPRs() - 2) ||
                 (isFloat && (_numberOfFPRs + numRegsForCandidate) <=
                                 comp()->cg()->getNumberOfGlobalFPRs() - 2)) &&
                performTransformation(
                    comp(), "%s replace auto #%d in loop %d (%p)\n",
                    OPT_DETAILS, symRef->getReferenceNumber(),
                    structureNode->getNumber(), structureNode)) {
              ...
              TR_SymRefCandidatePair *correspondingSymRefCandidate =
                  splitAndFixPreHeader(
                      symRef, correspondingSymRefs, loopInvariantBlock,
                      loopInvariantBlock->getEntry()->getNode());
              TR::SymbolReference *correspondingSymRef =
              ...
              fixExitsAfterSplit(symRef, correspondingSymRefCandidate,
                                 correspondingSymRefs, loopInvariantBlock,
                                 &blocksInLoop,
                                 loopInvariantBlock->getEntry()->getNode(),
                                 *registerCandidates, structureNode,
                                 replacedAutosInCurrentLoop, origSymRef);
            }
          }
        ...
        _origSymRefCount =
            comp()->getSymRefCount();  // _origSymRefCount was always zero
                                       // becasue there was no definition
        ...
        while (i < symRefCount) {
          if (correspondingSymRefs[i])
            _origSymRefs[correspondingSymRefs[i]
                             ->_symRef->getReferenceNumber()] =
                _origSymRefs[i] ? _origSymRefs[i]
                                : comp()->getSymRefTab()->getSymRef(i);
          i++;
        }
      } else
        traceMsg(
            comp(),
            " loop %d (%p) is skipped because loop pre-header was not found \n",
...
void TR_LiveRangeSplitter::replaceAutosUsedIn(
    TR::TreeTop *currentTree, TR::Node *node, TR::Node *parent,
    TR::Block *block, List<TR::Block> *blocksInLoop,
    List<TR::Block> *exitBlocks, vcount_t visitCount,
    int32_t executionFrequency, SymRefCandidateMap &registerCandidates,
    TR_SymRefCandidatePair **correspondingSymRefs,
  ...
  if (node->getOpCode().isLoadVarDirect() ||
    ...
    if (comp()->cg()->considerTypeForGRA(symRef) && canSplit(symRef, comp()) &&
        !autosThatCannotBeReplacedInCurrentLoop->get(
            symRef->getReferenceNumber())) {
      TR::SymbolReference *correspondingSymRef = NULL;
      TR_SymRefCandidatePair *correspondingSymRefCandidate =
          correspondingSymRefs[symRef->getReferenceNumber()];
      ...
      if (!correspondingSymRefCandidate) {
        ...
        int32_t numRegsForCandidate = 1;
        if (node->requiresRegisterPair(comp())) numRegsForCandidate = 2;
            ...
            if (!rc) {
              ...
              if (!dontCreateCandidatesJustForSplitter &&
                  performTransformation(comp(),
                                        "%s live range splitter creating "
                                        "candidate for #%d in loop %d\n",
                                        OPT_DETAILS,
                                        origSymRef->getReferenceNumber(),
                                        loop->getNumber())) {
                rc = comp()->getGlobalRegisterCandidates()->findOrCreate(
        ...
        if (((!isFloat && (_numberOfGPRs + numRegsForCandidate) <=
          ...
          correspondingSymRefCandidate = splitAndFixPreHeader(
              symRef, correspondingSymRefs, loopInvariantBlock, node);
          correspondingSymRef = correspondingSymRefCandidate->_symRef;
        } else
          autosThatCannotBeReplacedInCurrentLoop->set(
              symRef->getReferenceNumber());
      } else
        correspondingSymRef = correspondingSymRefCandidate->_symRef;

      if (correspondingSymRef) {
        static const char *dontReplaceStores =
            feGetEnv("TR_disableReplacingOfStores");

        if (!node->getOpCode().isStoreDirect() ||
            (!dontReplaceStores ||
             performTransformation(comp(),
                                   "%s --- going to replace auto #%d by auto "
                                   "#%d on %s node %p --- \n",
                                   OPT_DETAILS, symRef->getReferenceNumber(),
                                   correspondingSymRef->getReferenceNumber(),
          ...
          TR::Node *storeNode = TR::Node::createWithSymRef(
              comp()->il.opCodeForDirectStore(
                  correspondingSymRef->getSymbol()->getDataType()),
              1, 1, node->getFirstChild(), correspondingSymRef);
          traceMsg(comp(),
                   " --- created a store to auto #%d adjacent to existing "
                   "store to auto #%d at %s node %p --- \n",
                   correspondingSymRef->getReferenceNumber(),
                   symRef->getReferenceNumber(), node->getOpCode().getName(),
                   node);
        ...
        fixExitsAfterSplit(symRef, correspondingSymRefCandidate,
                           correspondingSymRefs, loopInvariantBlock,
                           blocksInLoop, node, registerCandidates, loop,
                           replacedAutosInCurrentLoop, origSymRef);
      }
    }
  }
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++)
    replaceAutosUsedIn(
        currentTree, node->getChild(childNum), node, block, blocksInLoop,
        exitBlocks, visitCount, executionFrequency, registerCandidates,
        correspondingSymRefs, replacedAutosInCurrentLoop,
        autosThatCannotBeReplacedInCurrentLoop, loop, loopInvariantBlock);
}

TR_SymRefCandidatePair *TR_LiveRangeSplitter::splitAndFixPreHeader(
    TR::SymbolReference *symRef, TR_SymRefCandidatePair **correspondingSymRefs,
    TR::Block *loopInvariantBlock, TR::Node *node) {
  TR::SymbolReference *correspondingSymRef =
      comp()->getSymRefTab()->createTemporary(
          comp()->getMethodSymbol(), symRef->getSymbol()->getDataType(),
          symRef->getSymbol()->isPinningArrayPointer());
  ...
  if (symRef->isFromLiteralPool()) correspondingSymRef->setFromLiteralPool();
  ...
  //
  dumpOptDetails(comp(),
                 " place initialization of auto #%d by auto #%d in loop "
                 "pre-header block_%d\n",
                 correspondingSymRef->getReferenceNumber(),
                 symRef->getReferenceNumber(), loopInvariantBlock->getNumber());
  appendStoreToBlock(correspondingSymRef, symRef, loopInvariantBlock, node);

  return correspondingSymRefCandidate;
}

void TR_LiveRangeSplitter::fixExitsAfterSplit(
    TR::SymbolReference *symRef,
    TR_SymRefCandidatePair *correspondingSymRefCandidate,
    TR_SymRefCandidatePair **correspondingSymRefs,
    TR::Block *loopInvariantBlock, List<TR::Block> *blocksInLoop,
    TR::Node *node, SymRefCandidateMap &registerCandidates,
    TR_StructureSubGraphNode *loop, TR_BitVector *replacedAutosInCurrentLoop,
    TR::SymbolReference *origSymRef) {
  TR::SymbolReference *correspondingSymRef =
      correspondingSymRefCandidate->_symRef;
  ...
  if (correspondingSymRef) {
    ...
    if (/* node->getOpCode().isStoreDirect() && */ !_storedSymRefs->get(
        symRef->getReferenceNumber())) {
      _storedSymRefs->set(symRef->getReferenceNumber());
      placeStoresInLoopExits(node, loop, blocksInLoop, symRef,
                             correspondingSymRef);
    }
    ...
    if (!replacedAutosInCurrentLoop->get(symRef->getReferenceNumber())) {
      ...
      if (rc && !correspondingRc) {
        correspondingRc = comp()->getGlobalRegisterCandidates()->findOrCreate(
            correspondingSymRef);
        ...
        if (parentOfLoop) {
          ...
          for (nextBlock = blocksIt.getCurrent(); nextBlock;
            ...
            if (!blocksInInnerLoop->get(nextBlock->getNumber())) {
              if (trace())
                traceMsg(comp(),
                         "Adding original candidate #%d in block_%d in outer "
                         "loop %d (%p)\n",
                         rc->getSymbolReference()->getReferenceNumber(),
                         nextBlock->getNumber(), parentOfLoop->getNumber(),
                         parentOfLoop);
...
void TR_LiveRangeSplitter::appendStoreToBlock(TR::SymbolReference *storeSymRef,
                                              TR::SymbolReference *loadSymRef,
                                              TR::Block *block,
                                              TR::Node *node) {
  TR::Node *initNode = TR::Node::createWithSymRef(
      comp()->il.opCodeForDirectStore(storeSymRef->getSymbol()->getDataType()),
      1, 1,
      TR::Node::createWithSymRef(node,
                                 comp()->il.opCodeForDirectLoad(
                                     loadSymRef->getSymbol()->getDataType()),
                                 0, loadSymRef),
      storeSymRef);

  if (trace()) dumpOptDetails(comp(), "creating store node %p\n", initNode);
  ...
  while (placeHolderTreeForLoad &&
    ...
    if ((placeHolderTreeForLoad->getNode()->getSymbolReference() ==
         loadSymRef) ||
        (placeHolderTreeForLoad->getNode()
             ->getSymbolReference()
             ->sharesSymbol() &&
         placeHolderTreeForLoad->getNode()
             ->getSymbolReference()
             ->getUseDefAliases()
             .contains(loadSymRef, comp())))
      break;
  ...
  if (placeHolderTreeForLoad != prevTree) {
    TR::Node *ttNode =
        TR::Node::create(TR::treetop, 1, initNode->getFirstChild());
    if (trace()) dumpOptDetails(comp(), "creating treetop node %p\n", ttNode);
...
void TR_LiveRangeSplitter::prependStoreToBlock(TR::SymbolReference *storeSymRef,
                                               TR::SymbolReference *loadSymRef,
                                               TR::Block *block,
                                               TR::Node *node) {
  for (TR::TreeTop *tt = block->getEntry(); tt != block->getExit();
    ...
    if (node->getOpCode().isStoreDirect() &&
      ...
      TR::SymbolReference *loadSymRef2 =
          node->getFirstChild()->getSymbolReference();
      if (storeSymRef2 && loadSymRef2 && storeSymRef == storeSymRef2 &&
          loadSymRef == loadSymRef2)
        return;
    }
  }
  ...
  while (
    ...
    if (!ttChild->getOpCode().hasSymbolReference() ||
        ...
        (ttChild->getSymbolReference()->sharesSymbol() &&
         ttChild->getSymbolReference()->getUseDefAliases().contains(storeSymRef,
                                                                    comp())))
      break;
...
void TR_LiveRangeSplitter::placeStoresInLoopExits(
    TR::Node *node, TR_StructureSubGraphNode *loop,
    List<TR::Block> *blocksInLoop, TR::SymbolReference *orig,
    TR::SymbolReference *replacement) {
  // traceMsg(comp(), " place initialization of auto #%d by auto #%d in loop
  // pre-header block_%d\n", replacement->getReferenceNumber(),
  // orig->getReferenceNumber(), loopInvariantBlock->getNumber());
  // appendStoreToBlock(replacement, orig, loopInvariantBlock, node);

  TR_ScratchList<TR::Block> loopExitBlocks(trMemory());
  loop->getStructure()->collectExitBlocks(&loopExitBlocks);

  TR_BitVector *exitsFromCurrentLoop = new (trStackMemory()) TR_BitVector(
      comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc);
  for (auto succ = loop->getSuccessors().begin();
       succ != loop->getSuccessors().end(); ++succ)
    exitsFromCurrentLoop->set((*succ)->getTo()->getNumber());
    ...
    if ((exitBlock != comp()->getFlowGraph()->getEnd()) &&
      ...
      if (!(exitBlock->getPredecessors().size() == 1)) {
        ...
        for (auto nextEdge = exitBlock->getPredecessors().begin();
          ...
          if (blocksInLoop->find(pred)) edgesFromLoop.add(*nextEdge);
        }
        ...
        for (nextEdge = ei.getFirst(); nextEdge; nextEdge = ei.getNext()) {
          ...
          if (pred->getLastRealTreeTop()->getNode()->getOpCode().isBranch() &&
            ...
            for (nextSplitBlock = splitBlocksIt.getFirst();
                 nextSplitBlock != NULL;
              ...
              if (exitBlockIsASplitBlock ||
                  ((nextSplitBlock->getSuccessors().size() == 1) &&
                   nextSplitBlock->getSuccessors().front()->getTo() ==
                       exitBlock)) {
                ...
                if (!exitBlockIsASplitBlock) {
                  ...
                  if (pred->getLastRealTreeTop()
                          ->getNode()
                          ->getBranchDestination() == exitBlock->getEntry())
                    pred->getLastRealTreeTop()->getNode()->setBranchDestination(
                        splitBlock->getEntry());
                  if (exitBlockIsFallThrough) {
                    TR::TreeTop *prevTree =
                        splitBlock->getEntry()->getPrevTreeTop();
                    TR::TreeTop *nextTree =
                        splitBlock->getExit()->getNextTreeTop();
                    prevTree->join(nextTree);
                    pred->getExit()->join(splitBlock->getEntry());
                    splitBlock->getExit()->join(exitBlock->getEntry());
                    if (splitBlock->getLastRealTreeTop()
                            ->getNode()
                            ->getOpCodeValue() == TR::Goto) {
                      prevTree =
                          splitBlock->getLastRealTreeTop()->getPrevTreeTop();
                ...
                dumpOptDetails(comp(),
                               " place store-back of auto #%d using auto #%d "
                               "in (existing) loop exit block_%d between pred "
                               "%d and exit block_%d\n",
                               orig->getReferenceNumber(),
                               replacement->getReferenceNumber(),
                               splitBlock->getNumber(), pred->getNumber(),
                               exitBlock->getNumber());
                break;
              }
            }
          ...
          if (!splitBlock) {
            ...
            dumpOptDetails(
                comp(),
                " place store-back of auto #%d using auto #%d in (new) loop "
                "exit block_%d between pred %d and exit block_%d\n",
        ...
        dumpOptDetails(comp(),
                       " place store-back of auto #%d using auto #%d in loop "
                       "exit block_%d \n",
                       orig->getReferenceNumber(),
                       replacement->getReferenceNumber(),
                       exitBlock->getNumber());
  ...
  for (succ = succIt.getFirst(); succ != NULL; succ = succIt.getNext())
     {
     TR_Structure *exitStructure =
  succ->getTo()->asStructureSubGraphNode()->getStructure(); TR::Block *exitBlock
  = NULL; if (exitStructure) exitBlock = exitStructure->getEntryBlock(); if
  (exitBlock && (exitBlock != comp()->getFlowGraph()->getEnd()))
        {
        //rc->removeBlock(exitBlock);
        //rc->removeLoopExitBlock(exitBlock);
        }
...
const char *TR_LiveRangeSplitter::optDetailString() const throw() {
