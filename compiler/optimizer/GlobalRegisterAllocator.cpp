...
static bool
dontAssignInColdBlocks(TR::Compilation* comp)
{
  return comp->getMethodHotness() >= hot;
}
...
void
TR_GlobalRegisterAllocator::populateSymRefNodes(TR::Node* node,
                                                vcount_t visitCount)
{
 ...
 */
bool
TR_GlobalRegisterAllocator::allocateForSymRef(TR::SymbolReference* symRef)
{
  return true;
}

bool
TR_GlobalRegisterAllocator::isSymRefAvailable(TR::SymbolReference* symRef)
{
...
bool
TR_GlobalRegisterAllocator::allocateForType(TR::DataType dt)
{
  return true;
}

bool
TR_GlobalRegisterAllocator::isNodeAvailable(TR::Node* node)
{
...
bool
TR_GlobalRegisterAllocator::isTypeAvailable(TR::SymbolReference* symref)
{
  if (!comp()->cg()->considerTypeForGRA(symref))
    return false;
...
static void
changeHeapBaseConstToLoad(TR::Compilation* comp,
...
TR_GlobalRegisterAllocator::perform()
{
  ...
  if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") &&
      ...
      if (numLocals > 0 &&
          (!trace() ||
           performTransformation(
             comp(),
             "%s Performing liveness for Global Register Allocator\n",
             OPT_DETAILS))) {
        // Perform liveness analysis
        //
        TR_Liveness liveLocals(comp(),
                               optimizer(),
    ...
    if (TR::Compiler->target.is64Bit() && optimizer()->getUseDefInfo()) {
      _temp = new (trStackMemory()) TR_BitVector(
        optimizer()->getUseDefInfo()->getNumDefNodes(), trMemory(), stackAlloc);
    ...
    TR_BitVector nonSplittingCopyStored(
      _origSymRefCount, trMemory(), stackAlloc);

    //
    // Assign registers to candidates
    //
    if (canAffordAssignment) {
      globalFPAssignmentDone = _candidates->assign(cfgBlocks,
                                                   numberOfBlocks,
      ...
      if (!splitSymRefs.isEmpty()) {
        ...
        for (tt = itr.getFirst(); tt; tt = itr.getNext()) {
          if (!_valueModifiedSymRefs->isSet(tt->getNode()
                                              ->getFirstChild()
                                              ->getRegLoadStoreSymbolReference()
                                              ->getReferenceNumber()) &&
              !nonSplittingCopyStored.isSet(tt->getNode()
                                              ->getFirstChild()
                                              ->getRegLoadStoreSymbolReference()
                                              ->getReferenceNumber())) {
            if (trace)
...
void
TR_GlobalRegisterAllocator::restoreOriginalSymbol(TR::Node* node,
                                                  vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
    return;
  ...
  if (node->getOpCode().hasSymbolReference() || node->getOpCode().isLoadReg() ||
    ...
    if (node->getSymbolReferenceOfAnyType()) {
      ...
      if (rc && !rc->extendedLiveRange() && changeSymRef) {
        if (trace)
          traceMsg(comp(),
                   "Restore an original symbol #%d from #%d at %p\n",
                   changeSymRef->getReferenceNumber(),
                   symRefNum,
                   node);
...
TR::Node*
TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::DataType oldType,
                                                TR::Node* newNode)
{
...
TR::Node*
TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::Node* oldNode,
                                                TR::Node* newNode)
{
...
TR::Node*
TR_GlobalRegisterAllocator::resolveTypeMismatch(TR::DataType inputOldType,
                                                TR::Node* oldNode,
                                                TR::Node* newNode)
{
  return newNode;
  ...
  TR_NodeMappings* extBlockNodeMapping)
{
  ...
  {
    transformNode(node->getFirstChild(),
                  node,
                  0,
                  tt,
                  block,
  ...
  for (i = 0; i < node->getNumChildren(); ++i)
    transformNode(
      node->getChild(i), node, i, tt, block, registers, extBlockNodeMapping);
  ...
  if (node->getOpCode().isJumpWithMultipleTargets() &&
    ...
    if (value) {
      ...
      TR_GlobalRegister* ptrToGr =
        getGlobalRegisterWithoutChangingCurrentCandidate(
          symbol, registers, block);
      if (ptrToGr) {
    ...
    if (changeNode) {
...

        {
          ...
          if (!storeInfo->_origStoreExists) {
            ...
            TR::Node* store = TR::Node::createWithSymRef(
              comp()->il.opCodeForDirectStore(rc->getDataType()),
              1,
              1,
              regStoreNode->getFirstChild(),
          ...
          if (parent->getOpCode().isStore() &&
            ...
            TR::Node* loadVarNode =
              node->getSymbolReference()->getSymbol()->isMethodMetaData() &&
                ...
                ? 0
                : node;
            value = gr->createStoreToRegister(
              prevTree, loadVarNode, _visitCount, comp(), this);
      ...
      if (comp()->getOption(TR_CheckGRA) &&
          performTransformation(
            comp(),
            "%s CheckGRA inserting BNDCHK to verify %s node %p\n",
            OPT_DETAILS,
            value->getOpCode().getName(),
            value)) {
        TR::Node* zeroIfEqual = TR::Node::create(
          comp()->il.opCodeForCompareNotEquals(oldValue->getDataType()),
          2,
          oldValue,
          value);
        TR::TreeTop::create(
          ...
          TR::Node::createWithSymRef(
            ...
            TR::Node::iconst(oldValue,
                             1), // pretend we have an array of size=1 so only
                                 // zeroIfEqual==0 is acceptable
            zeroIfEqual,
    ...
    if (symbol->isInGlobalRegister()) {
      ...
      if (gr && gr->getValue() && !gr->getAutoContainsRegisterValue()) {
        gr->createStoreFromRegister(
          _visitCount, tt->getPrevTreeTop(), -1, comp());
        bool needs2Regs = false;
        TR_RegisterCandidate* rc = gr->getCurrentRegisterCandidate();
        if (rc->rcNeeds2Regs(comp()))
          needs2Regs = true;
    ...
    if (symbol->isInGlobalRegister() &&
      ...
      if ((!fpStore && !symRef->storeCanBeRemoved()) ||
        ...
        TR_RegisterCandidate* rc = gr->getCurrentRegisterCandidate();
        dumpOptDetails(comp(),
                       "%s change store var [%p] %s #%d to store reg\n",
                       OPT_DETAILS,
                       node,
  ...
  bool regStarTransformDone)
{
  TR_Array<TR::Node*> regDepNodes(
    trMemory(), _lastGlobalRegisterNumber + 1, true, stackAlloc);
  ;
  ...
  for (TR::Block* successorBlock = si->getFirstSuccessor();
    ...
    if (successorBlock->isExtensionOfPreviousBlock() == false) {
      prepareForBlockExit(
        exitTreeTop, caseNode, block, registers, successorBlock, regDepNodes);
    }
  }
  ...
  TR::Block* successorBlock)
{
  ...
  prepareForBlockExit(
    exitTreeTop, exitNode, block, registers, successorBlock, regDepNodes);
  ...
  bool traceIt)
{
  ...
  if (nextBlock && nextBlock->isExtensionOfPreviousBlock()) {
    ...
    if (traceIt)
      traceMsg(comp(),
               "  block_%d marked to reload candidate #%d\n",
               nextBlock->getNumber(),
               rc->getSymbolReference()->getReferenceNumber());
    reg->setUnavailableResolved();
...
void
TR_GlobalRegisterAllocator::reloadNonRegStarVariables(TR::TreeTop* tt,
                                                      TR::Node* node,
                                                      TR::Block* block,
                                                      bool traceIt)
{
...
void
TR_GlobalRegisterAllocator::addCandidateReloadsToEntry(
  ...
  TR::Block* block)
{
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    if (currRegisters[i].getReloadRegisterCandidateOnEntry()) {
      ...
      currRegisters[i].setCurrentRegisterCandidate(
        candidateOnEntry, 0, 0, i, comp());
      currRegisters[i].createStoreToRegister(
        bbStartTT, NULL, _visitCount, comp(), this);
    }
  }
}

void
TR_GlobalRegisterAllocator::addStoresForCatchBlockLoads(
  ...
  for (int32_t i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber;
    ...
    if (_osrCatchSucc && rc && !rc->isLiveAcrossExceptionEdge() &&
      ...
      if (!registers[i].getAutoContainsRegisterValue()) {
        registers[i].createStoreFromRegister(
          comp()->getVisitCount(), appendPoint, 1, comp(), true);
...
void
TR_GlobalRegisterAllocator::addRegLoadsToEntry(
  TR::TreeTop* bbStartTT,
  TR_Array<TR_GlobalRegister>& registers,
  TR::Block* block)
{
  ...
  dumpOptDetails(comp(),
                 "%s create TR::GlRegDeps [%p] on BBStart [%p]\n",
                 OPT_DETAILS,
                 regDepNode,
                 bbStart);
  numLoads = 0;
...
TR::TreeTop*
TR_GlobalRegisterAllocator::findPrevTreeTop(TR::TreeTop*& exitTreeTop,
                                            TR::Node*& exitNode,
                                            TR::Block* block,
                                            TR::Block* successorBlock)
{
  ...
  TR_Array<TR::Node*>& regDepNodes)
{
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    if (extgr->getValue()) {
      ...
      if ((!extgr->getAutoContainsRegisterValue() ||
          ...
          !registerIsLiveAcrossEdge(
            exitTreeTop, exitNode, block, extgr, successorBlock, i)) {
        ...
        if (liveOnSomeSucc && !extgr->getAutoContainsRegisterValue() &&
          ...
          if ((block->getSuccessors().size() == 1) ||
            ...
            extgr->createStoreFromRegister(
              _visitCount,
              extgr->optimalPlacementForStore(block, comp()),
              i,
              comp());
          } else {
            if (!prevTreeTop)
        ...
        !gr->isUnavailable()) // ensure the global reg is currently available)
    {
      ...
      {
        extgr->setCurrentRegisterCandidate(gr->getRegisterCandidateOnExit(),
                                           _visitCount,
        ...
        if (currRC && currRC->rcNeeds2Regs(comp())) {
          ...
          if (i == highRegNum) {
            ...
            grLow->setCurrentRegisterCandidate(
              ...
              _visitCount,
              exitTreeTop->getEnclosingBlock(),
              currRC->getLowGlobalRegisterNumber(),
              comp());
            ...
            grHigh->setCurrentRegisterCandidate(
              ...
              _visitCount,
              exitTreeTop->getEnclosingBlock(),
              currRC->getHighGlobalRegisterNumber(),
              comp());
        ...
        if (extgr->getCurrentRegisterCandidate() != successorRC) {
          traceMsg(comp(),
                   "block_%d successorBlock %d exitNode %p\n",
                   block->getNumber(),
      ...
      if (!extgr->getValue() && !exitCandidateAlreadySeen) {
        ...
        value = extgr->createStoreToRegister(
          prevTreeTop, NULL, _visitCount, comp(), this);
        bool needs2Regs = false;
        if (extgr->getCurrentRegisterCandidate() &&
  ...
  if ((successorBlock != originalSuccessorBlock) &&
    ...
    transformBlockExit(succExit,
                       succExit->getNode(),
                       sb,
                       sb->getGlobalRegisters(comp()),
                       originalSuccessorBlock);
  }
}

bool
TR_GlobalRegisterAllocator::registerIsLiveAcrossEdge(TR::TreeTop* exitTreeTop,
                                                     TR::Node* exitNode,
                                                     TR::Block* block,
                                                     ...
                                                     int32_t i)
{
  ...
  if (!keepingItAliveAcrossEdgeRequiresExtraGoto) {
    ...
    if (successorRegister->getRegisterCandidateOnEntry() != 0 ||
      ...
      TR::Block* newBlock = createNewSuccessorBlock(
        block, successorBlock, exitTreeTop, exitNode, rc);
      if (trace())
        traceMsg(
          comp(), "Creating new successor block_%d\n", newBlock->getNumber());
  ...
  if (branchIsHotter == TR_no) {
    ...
    TR::Block* newBlock =
      createNewSuccessorBlock(block, successorBlock, exitTreeTop, exitNode, rc);
...
TR::Block*
TR_GlobalRegisterAllocator::extendBlock(TR::Block* block,
                                        TR::Block* successorBlock)
{
...
TR::Block*
TR_GlobalRegisterAllocator::createNewSuccessorBlock(TR::Block* block,
                                                    TR::Block* successorBlock,
                                                    TR::TreeTop* exitTreeTop,
                                                    TR::Node* exitNode,
          ...
          if (numRegistersLiveOnNewSuccessor + (needs2Regs ? 2 : 1) >
            ...
            if (trace())
              traceMsg(
                ...
                "d) %d\n",
                numRegistersLiveOnNewSuccessor,
                nextNewBlock->getNumber(),
                block->getNumber(),
                comp()->cg()->getMaximumNumberOfGPRsAllowedAcrossEdge(block));
            newBlockCanBeReused = false;
...
TR::Block*
TR_GlobalRegisterAllocator::createBlock(TR::Block* block,
                                        TR::Block* successorBlock)
{
...
int32_t
TR_GlobalRegisterAllocator::numberOfRegistersLiveOnEntry(
  TR_Array<TR_GlobalRegister>& registers,
...
TR_GlobalRegister*
TR_GlobalRegisterAllocator::getGlobalRegister(
  TR::Symbol* symbol,
  TR_Array<TR_GlobalRegister>& registers,
  TR::Block* block)
{
  ...
  for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
    ...
    if (rc->rcNeeds2Regs(comp())) {
      ...
      if (i == highRegNum) {
        ...
        TR_GlobalRegister* grLow = &extRegisters[lowRegNum];
        grLow->setCurrentRegisterCandidate(
          rc, _visitCount, block, lowRegNum, comp());
        // grLow.copyCurrentRegisterCandidate(gr);
      } else {
        TR_GlobalRegister* grHigh = &extRegisters[highRegNum];
        grHigh->setCurrentRegisterCandidate(
          rc, _visitCount, block, highRegNum, comp());
        // grHigh.copyCurrentRegisterCandidate(gr);
      }
    }
...
void
TR_GlobalRegister::setCurrentRegisterCandidate(TR_RegisterCandidate* rc,
                                               vcount_t visitCount,
  ...
  if (_rcCurrent != rc) {
    ...
    if (resetOtherHalfOfLong && _rcCurrent != NULL &&
      ...
      if (i == highRegNum) {
        ...
        TR_GlobalRegister* grLow = &extRegisters[lowRegNum];
        grLow->setCurrentRegisterCandidate(
          NULL, visitCount, currentBlock, lowRegNum, comp, false);
        // grLow.copyCurrentRegisterCandidate(gr);
      } else {
        TR_GlobalRegister* grHigh = &extRegisters[highRegNum];
        grHigh->setCurrentRegisterCandidate(
          NULL, visitCount, currentBlock, highRegNum, comp, false);
        // grHigh.copyCurrentRegisterCandidate(gr);
      }
    }
                                            ...
                                            TR::Compilation* comp)
{
  ...
  if (traceGRA)
    traceMsg(comp,
             "           optimalPlacementForStore([%p], block_%d)\n",
             getValue(),
  ...
  if (!lastRefBlock->getStructureOf() || !currentBlock->getStructureOf()) {
    if (traceGRA)
      traceMsg(comp,
               "           - Structure info missing: returning [%p]\n",
               lastRefTreeTop->getNode());
    return lastRefTreeTop;
  }
  ...
  if (lastRefFreq <= currentFreq) // used to be ==
  {
    if (traceGRA)
      traceMsg(comp,
               "           - Frequency is low enough: returning [%p]\n",
               lastRefTreeTop->getNode());
    return lastRefTreeTop;
  }
  ...
  for (TR::Block* b = lastRefBlock->getNextBlock(); b; b = b->getNextBlock()) {
    ...
    if (traceGRA)
      traceMsg(comp,
               "           - Found a suitable block: returning [%p]\n",
               b->getEntry()->getNode());
    return b->getEntry();
  }
...
TR_GlobalRegister::createLoadFromRegister(TR::Node* n, TR::Compilation* comp)
{
  ...
  if (load->requiresRegisterPair(comp))
    dumpOptDetails(comp,
                   ...
                   OPT_DETAILS,
                   load,
                   rc->getLowGlobalRegisterNumber(),
                   rc->getHighGlobalRegisterNumber());
  else
    dumpOptDetails(comp,
                   "%s create load [%p] %s from Register %d\n",
                   OPT_DETAILS,
                   load,
                   rc->getSymbolReference()->getSymbol()->isMethodMetaData()
                     ? rc->getSymbolReference()
...
TR::Node*
TR_GlobalRegister::createStoreToRegister(TR::TreeTop* prevTreeTop,
                                         TR::Node* node,
                                         vcount_t visitCount,
                                         TR::Compilation* comp,
                                         TR_GlobalRegisterAllocator* gra)
{
  ...
  if (dt == TR::Aggregate) {
    switch (rc->getSymbol()->getSize()) {
      ...
      default:
        TR_ASSERT(false,
                  "unsupported size %d for aggregate in GRA\n",
  ...
  if (node) {
    ...
    load = TR::Node::createWithSymRef(
      n, comp->il.opCodeForDirectLoad(dt), 0, rc->getSymbolReference());
  }
  ...
  if (store->requiresRegisterPair(comp))
    dumpOptDetails(comp,
                   "%s create store [%p] of symRef#%d to Register %d (low "
                   "word) and Register %d (high word)\n",
                   OPT_DETAILS,
                   store,
                   rc->getSymbolReference()->getReferenceNumber(),
                   rc->getLowGlobalRegisterNumber(),
...
TR::Node*
TR_GlobalRegister::createStoreFromRegister(vcount_t visitCount,
                                           ...
                                           bool storeUnconditionally)
{
  ...
  TR::Node* store =
    TR::Node::createWithSymRef(comp->il.opCodeForDirectStore(rc->getDataType()),
                               1,
                               1,
                               node,
                               rc->getSymbolReference());
  ...
  if (i != -1) {
    if (store->requiresRegisterPair(comp))
      ...
      dumpOptDetails(comp,
                     "%s create store [%p] from Register %d for %s #%d\n",
                     OPT_DETAILS,
                     store,
                     rc->getGlobalRegisterNumber(),
                     rc->getSymbolReference()->getSymbol()->isMethodMetaData()
...
TR_GlobalRegisterAllocator::findIfThenRegisterCandidates()
{
  ...
  if (!debug("oldIfThen")) {
    ...
    TR_BitVector autoAndParmLiveLocalIndex(
      guessSize, trMemory(), stackAlloc, growable);
...
void
TR_GlobalRegisterAllocator::offerAllAutosAndRegisterParmAsCandidates(
  TR::Block** cfgBlocks,
  ...
  if (structure->asRegion()) {
    ...
    if (!regionStructure->isAcyclic() && structureNode) {
      ...
      for (auto succ = structureNode->getSuccessors().begin();
           succ != structureNode->getSuccessors().end();
           ++succ) {
...
bool
TR_GlobalRegisterAllocator::isDependentStore(
  TR::Node* node,
        ...
        if ((node->getOpCode().isStoreDirect() && isSplittingCopy(node)) ||
          ...
          if (TR::Compiler->target.cpu.isZ() &&
            ...
            if (trace())
              dumpOptDetails(
                comp(),
                "Increased weight of candidate #%d in block_%d to reduce AGI\n",
                rc->getSymbolReference()->getReferenceNumber(),
                block->getNumber());
          } else {
...
void
TR_GlobalRegisterAllocator::signExtendAllDefNodes(TR::Node* defNode,
...
void
TR_GlobalRegisterAllocator::findSymsUsedInIndirectAccesses(
  TR::Node* node,
  TR_BitVector* symsThatShouldNotBeAssignedInCurrentLoop,
  TR_BitVector* assignedAutosInCurrentLoop,
...
TR_PairedSymbols*
TR_GlobalRegisterAllocator::findOrCreatePairedSymbols(
...
TR_GlobalRegisterAllocator::getAppendBlock(TR::Block* block)
{
  ...
  if (block->isCold())
    return _appendBlock =
             comp()->getMethodSymbol()->getLastTreeTop()->getNode()->getBlock();
...
void
TR_GlobalRegisterAllocator::appendGotoBlock(TR::Block* gotoBlock,
                                            TR::Block* curBlock)
{
...
void
TR_GlobalRegisterAllocator::findLoopsAndCreateAutosForSignExt(
  TR_StructureSubGraphNode* structureNode,
  vcount_t visitCount)
{
  TR_Structure* structure;
      ...
      for (nextBlock = blocksIt.getFirst(); nextBlock;
        ...
        if (nextBlock->getVisitCount() != visitCount) {
          ...
          while (currentTree != exitTree) {
            TR::Node* currentNode = currentTree->getNode();
            TR::Node* arrayAccess = NULL;
            createStoresForSignExt(currentNode,
                                   NULL,
                                   NULL,
                                   currentTree,
                                   &arrayAccess,
  ...
  bool hasCatchBlock)
{
  ...
  int32_t childNum;
  for (childNum = 0; childNum < node->getNumChildren(); childNum++)
    createStoresForSignExt(node->getChild(childNum),
                           node,
                           parent,
                           treetop,
                           currentArrayAccess,
 ...
 */
void
TR_LiveRangeSplitter::splitLiveRanges()
{
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  ...
  if (!cg()->getLiveLocals()) {
    ...
    if (numLocals > 0 &&
      ...
      TR_Liveness liveLocals(
        comp(), optimizer(), comp()->getFlowGraph()->getStructure());
      if (comp()->getVisitCount() > HIGH_VISIT_COUNT) {
        comp()->resetVisitCounts(1);
      }
...
static bool
canSplit(TR::SymbolReference* symRef, TR::Compilation* comp)
{
      ...
      if (loopInvariantBlock) {
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock;
          ...
          for (auto succ = nextBlock->getSuccessors().begin();
               succ != nextBlock->getSuccessors().end();
               ++succ) {
        ...
        for (nextBlock = blocksIt.getCurrent(); nextBlock;
          ...
          if (nextBlock->getVisitCount() != visitCount) {
            ...
            while (currentTree != exitTree) {
              TR::Node* currentNode = currentTree->getNode();
              replaceAutosUsedIn(currentTree,
                                 currentNode,
                                 NULL,
                                 nextBlock,
                                 &blocksInLoop,
        ...
        _origSymRefCount =
          comp()->getSymRefCount(); // _origSymRefCount was always zero becasue
          ...
          if (!rc) {
            ...
            if (!rc) {
              ...
              if (!dontCreateCandidatesJustForSplitter &&
                ...
                rc = comp()->getGlobalRegisterCandidates()->findOrCreate(
                  origSymRef);
                rc->addAllBlocksInStructure(loop->getStructure(),
                                            comp(),
...
TR_LiveRangeSplitter::splitAndFixPreHeader(
  TR::SymbolReference* symRef,
  TR_SymRefCandidatePair** correspondingSymRefs,
  TR::Block* loopInvariantBlock,
  TR::Node* node)
...
void
TR_LiveRangeSplitter::fixExitsAfterSplit(
  TR::SymbolReference* symRef,
  TR_SymRefCandidatePair* correspondingSymRefCandidate,
  TR_SymRefCandidatePair** correspondingSymRefs,
  TR::Block* loopInvariantBlock,
  ...
  TR::SymbolReference* origSymRef)
{
  ...
  if (correspondingSymRef) {
    ...
    if (/* node->getOpCode().isStoreDirect() && */ !_storedSymRefs->get(
      ...
      placeStoresInLoopExits(
        node, loop, blocksInLoop, symRef, correspondingSymRef);
    }
    ...
    if (!replacedAutosInCurrentLoop->get(symRef->getReferenceNumber())) {
      ...
      if (rc && !correspondingRc) {
        ...
        TR_BitVector* blocksInInnerLoop = new (trStackMemory()) TR_BitVector(
          comp()->getFlowGraph()->getNextNodeNumber(), trMemory(), stackAlloc);
        ListIterator<TR::Block> blocksIt(blocksInLoop);
        TR::Block* nextBlock;
        for (nextBlock = blocksIt.getCurrent(); nextBlock;
...
void
TR_LiveRangeSplitter::appendStoreToBlock(TR::SymbolReference* storeSymRef,
                                         TR::SymbolReference* loadSymRef,
                                         TR::Block* block,
                                         TR::Node* node)
{
  TR::Node* initNode = TR::Node::createWithSymRef(
    ...
    TR::Node::createWithSymRef(
      node,
      comp()->il.opCodeForDirectLoad(loadSymRef->getSymbol()->getDataType()),
      0,
      loadSymRef),
    storeSymRef);
...
void
TR_LiveRangeSplitter::prependStoreToBlock(TR::SymbolReference* storeSymRef,
                                          TR::SymbolReference* loadSymRef,
                                          TR::Block* block,
                                          TR::Node* node)
{
...
void
TR_LiveRangeSplitter::placeStoresInLoopExits(TR::Node* node,
                                             TR_StructureSubGraphNode* loop,
                                             List<TR::Block>* blocksInLoop,
    ...
    if ((exitBlock != comp()->getFlowGraph()->getEnd()) &&
      ...
      if (!(exitBlock->getPredecessors().size() == 1)) {
        ...
        for (auto nextEdge = exitBlock->getPredecessors().begin();
             nextEdge != exitBlock->getPredecessors().end();
             ++nextEdge) {
        ...
        for (nextEdge = ei.getFirst(); nextEdge; nextEdge = ei.getNext()) {
          ...
          if (pred->getLastRealTreeTop()->getNode()->getOpCode().isBranch() &&
            ...
            for (nextSplitBlock = splitBlocksIt.getFirst();
              ...
              if (exitBlockIsASplitBlock ||
                ...
                dumpOptDetails(
                  comp(),
                  " place store-back of auto #%d using auto #%d in (existing) "
                  "loop exit block_%d between pred %d and exit block_%d\n",
          ...
          if (!splitBlock) {
            ...
            dumpOptDetails(
              comp(),
              " place store-back of auto #%d using auto #%d in (new) loop exit "
              "block_%d between pred %d and exit block_%d\n",
              orig->getReferenceNumber(),
              replacement->getReferenceNumber(),
              splitBlock->getNumber(),
              pred->getNumber(),
