...
static void changeHeapBaseConstToLoad(TR::Compilation* comp, TR::SymbolReference*& autoSymRef, TR::Node* node, vcount_t visitCount)
{
    ...
    if ((node->getOpCodeValue() == TR::lconst) && (node->getLongInt() == TR::Compiler->vm.heapBaseAddress())) {
        if (!autoSymRef) {
...
TR_GlobalRegisterAllocator::perform()
{
    ...
    if (comp()->useCompressedPointers() && comp()->cg()->materializesHeapBase() && TR::Compiler->vm.heapBaseAddress() != 0) {
    ...
    if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") && cg()->prepareForGRA()) {
        ...
        if (comp()->getOptions()->realTimeGC() && comp()->compilationShouldBeInterrupted(GRA_AFTER_FIND_LOOP_AUTO_CONTEXT)) {
        ...
        if (canAffordAssignment) {
            ...
            if (_lastGlobalRegisterNumber > -1) {
                ...
                for (node = nodesIt.getFirst(); node != NULL; node = nodesIt.getNext()) {
                    if (node->skipSignExtension() && (node->getOpCode().isLoadVarDirect() || (((node->getOpCodeValue() == TR::iadd) || (node->getOpCodeValue() == TR::isub)) && node->getFirstChild()->getOpCode().isLoadVarDirect() && node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto() && node->getSecondChild()->getOpCode().isLoadConst())))
                ...
                for (nodeTriple = nodeTriplesIt.getFirst(); nodeTriple != NULL; nodeTriple = nodeTriplesIt.getNext()) {
                    ...
                    if ((nodeTriple->_node2->getOpCodeValue() == TR::iRegStore) && ((node3->getOpCodeValue() == TR::iRegStore) || (node3->getOpCodeValue() == TR::iRegLoad)) && (nodeTriple->_node2->getGlobalRegisterNumber() == node3->getGlobalRegisterNumber())) {
            ...
            if (!splitSymRefs.isEmpty()) {
                ...
                for (tt = itr.getFirst(); tt; tt = itr.getNext()) {
                    if (!_valueModifiedSymRefs->isSet(tt->getNode()->getFirstChild()->getRegLoadStoreSymbolReference()->getReferenceNumber()) && !nonSplittingCopyStored.isSet(tt->getNode()->getFirstChild()->getRegLoadStoreSymbolReference()->getReferenceNumber())) {
                        if (trace)
...
bool TR_GlobalRegisterAllocator::isSplittingCopy(TR::Node* node)
{
    ...
    if ((node->getOpCode().isStoreDirect() || node->getOpCode().isStoreReg()) && (node->getFirstChild()->getOpCode().isLoadVarDirect() || node->getFirstChild()->getOpCode().isLoadReg())) {
        if (trace)
        ...
        if (storeSymRef && loadSymRef && storeSymRef != loadSymRef) {
            ...
            if ((origStoreSymRef && origLoadSymRef && origStoreSymRef == origLoadSymRef) || (origStoreSymRef && !origLoadSymRef && origStoreSymRef == loadSymRef) || (!origStoreSymRef && origLoadSymRef && storeSymRef == origLoadSymRef)) {
...
void TR_GlobalRegisterAllocator::transformBlock(TR::TreeTop* tt)
{
    ...
    } while ((tt = tt->getNextTreeTop()) && (node = tt->getNode(), node->getOpCodeValue() != TR::BBStart || node->getBlock()->isExtensionOfPreviousBlock()));
    ...
    TR::Node* node, TR::Node* parent, int32_t childIndex, TR::TreeTop* tt, TR::Block*& block, TR_Array<TR_GlobalRegister>& registers, TR_NodeMappings* extBlockNodeMapping)
{
    ...
    if (node->getOpCode().isJumpWithMultipleTargets() && !node->getOpCode().isSwitch() && node->getOpCodeValue() != TR::tstart)
        ...
        if (value) {
            ...
            if (ptrToGr) {
                ...
                if (rc && (rc->getSymbolReference()->getSymbol() == symbol)) {
                    ...
                    bool needs2Regs = false;
                    if (ptrToGr->getCurrentRegisterCandidate() && ptrToGr->getCurrentRegisterCandidate()->rcNeeds2Regs(comp()))
                        needs2Regs = true;
        ...
        if (changeNode) {
            ...
            if (!value) {
                value = gr->getValue();
                if (comp()->cg()->needToAvoidCommoningInGRA() && value && (!value->getOpCode().isLoadReg() && !value->getOpCode().isLoadVar()) && (symbol->getDataType() == TR::Float || symbol->getDataType() == TR::Double
#ifdef J9_PROJECT_SPECIFIC
                                                                                                                                                      || symbol->getDataType() == TR::DecimalFloat || symbol->getDataType() == TR::DecimalDouble || symbol->getDataType() == TR::DecimalLongDouble
#endif
                                                                                                                                                      )
                    && (!parent->getOpCode().isStore() && (parent->getOpCodeValue() != TR::fRegStore) && (parent->getOpCodeValue() != TR::dRegStore) && (parent->getOpCodeValue() != TR::GlRegDeps) && (parent->getOpCodeValue() != TR::PassThrough)))
                ...
                if (!value) {
                    if (parent->getOpCode().isStore() && (parent->getSymbolReference() == node->getSymbolReference())) {
                        ...
                        TR::Node* loadVarNode = node->getSymbolReference()->getSymbol()->isMethodMetaData() && node->getSymbolReference() != node->getSymbolReference() ? 0 : node;
            ...
            if (value->getOpCode().isLoadReg() && !(parent->getOpCode().getOpCodeValue() == TR::GlRegDeps) && !value->isSeenRealReference()) {
            ...
            if ((oldValue->getReferenceCount() == 0) && (oldValue->getOpCodeValue() == TR::iload) && (value->getOpCodeValue() == TR::iRegLoad)) {
        ...
        if (symbol->isInGlobalRegister() && (gr = getGlobalRegister(symbol, registers, block))) {
                ...
                if (fpStore && !comp()->cg()->getSupportsJavaFloatSemantics()) {
                    ...
                    for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
                        ...
                        while (b) {
                            ...
                            if (curRegisters[i].getRegisterCandidateOnExit() && (curRegisters[i].getRegisterCandidateOnExit() != rc)) {
            ...
            if (ptrToGr) {
                ...
                TR_RegisterCandidate* rc = ptrToGr->getCurrentRegisterCandidate();
                if (rc && (rc->getSymbolReference()->getSymbol() == symbol)) {
                    if (rc->getSymbolReference()->getSymbol()->isMethodMetaData() && rc->getSymbolReference() != origSymRef && ptrToGr->getValue() && !ptrToGr->getAutoContainsRegisterValue())
                        ptrToGr->createStoreFromRegister(comp()->getVisitCount(), tt->getPrevTreeTop(), 1, comp(), true);
                    ptrToGr->setAutoContainsRegisterValue(true);
                    ptrToGr->setValue(0);
...
bool TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors(int32_t i, TR_GlobalRegister* extReg, TR_GlobalRegister* reg, TR::Block* block, bool traceIt)
{
    ...
    if (traceIt)
        traceMsg(comp(), "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors block=%d GlobalReg=(%d,symRef=#%d)\n", block->getNumber(), i, rc->getSymbolReference()->getReferenceNumber());
    ...
    if (nextBlock && nextBlock->isExtensionOfPreviousBlock()) {
        ...
        if (traceIt)
            traceMsg(comp(), "TR_GlobalRegisterAllocator::markCandidateForReloadInSuccessors next block is not extension\n");
...
void TR_GlobalRegisterAllocator::reloadNonRegStarVariables(TR::TreeTop* tt, TR::Node* node, TR::Block* block, bool traceIt)
{
    TR_ASSERT(comp()->getOptimizer()->cachedExtendedBBInfoValid(), "Incorrect value in _startOfExtendedBBForBB");
...
void TR_GlobalRegisterAllocator::addCandidateReloadsToEntry(TR::TreeTop* bbStartTT, TR_Array<TR_GlobalRegister>& registers, TR::Block* block)
{
...
void TR_GlobalRegisterAllocator::addStoresForCatchBlockLoads(TR::TreeTop* appendPoint, TR_Array<TR_GlobalRegister>& registers, TR::Block* throwingBlock)
{
    ...
    for (int32_t i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        ...
        if (_osrCatchSucc && rc && !rc->isLiveAcrossExceptionEdge() && rc->symbolIsLive(_osrCatchSucc) && registers[i].getValue()) {
...
void TR_GlobalRegisterAllocator::addRegLoadsToEntry(TR::TreeTop* bbStartTT, TR_Array<TR_GlobalRegister>& registers, TR::Block* block)
{
    TR::Node* bbStart = bbStartTT->getNode();
    ...
    for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        ...
        if (rc && !seenCandidates.find(rc) && !currRegisters[i].getReloadRegisterCandidateOnEntry() && !currRegisters[i].isUnavailable()) {
    ...
    TR_Array<TR::Node*>& regDepNodes)
{
    ...
    for (i = _firstGlobalRegisterNumber; i <= _lastGlobalRegisterNumber; ++i) {
        ...
        if (extgr->getValue()) {
            bool mustBeLiveAcrossAllPaths = (extgr->getValue()->getOpCode().isFloatingPoint() && !cg()->getSupportsJavaFloatSemantics());

            if ((!extgr->getAutoContainsRegisterValue() || mustBeLiveAcrossAllPaths) && !registerIsLiveAcrossEdge(exitTreeTop, exitNode, block, extgr, successorBlock, i)) {
        ...
        if (successorRC && !regDepNodes[i] && !gr->isUnavailable()) // ensure the global reg is currently available)
        {
            ...
            if (!exitCandidateAlreadySeen && !(value->getOpCode().isLoadReg() && extgr->getCurrentRegisterCandidate()->hasSameGlobalRegisterNumberAs(value, comp()))) {
    ...
    if ((successorBlock != originalSuccessorBlock) && (successorBlock->getPredecessors().size() == 1) && (successorBlock->getExit()->getPrevTreeTop()->getNode()->getOpCodeValue() != TR::Goto)) {
    ...
    TR_GlobalRegister* extgr, TR::Block*& successorBlock, int32_t i)
{
    ...
    if (!predecessors.empty() && !(predecessors.size() == 1) && successorBlock->getEntry()->getNode()->getVisitCount() != comp()->getVisitCount()) {
        bool liveOnAllPredExits = true;
        int32_t allPredFreq = 0;
        for (auto pred = predecessors.begin(); pred != predecessors.end(); ++pred) {
            if (!rc->getBlocksLiveOnExit().get((*pred)->getFrom()->getNumber()) || toBlock((*pred)->getFrom())->getEntry()->getNode()->getVisitCount() == comp()->getVisitCount()) {
    ...
    bool keepingItAliveAcrossEdgeRequiresExtraGoto = !ttBeforeSuccessor || ttBeforeSuccessor->getNode()->getBlock()->hasSuccessor(successorBlock);
    ...
    if (branchIsHotter == TR_maybe) {
        if (block->getNextBlock()) {
            if (block->getNextBlock()->getFrequency() > successorBlock->getFrequency()) {
                if (((successorBlock->getFrequency() == 0) || ((successorBlock->getFrequency() > 0) && ((block->getNextBlock()->getFrequency() * 100 / (successorBlock->getFrequency())) > 130))))
                    branchIsHotter = TR_no;
            } else if (block->getNextBlock()->getFrequency() < successorBlock->getFrequency()) {
                if (((block->getNextBlock()->getFrequency() == 0) || ((block->getNextBlock()->getFrequency() > 0) && ((successorBlock->getFrequency() * 100 / (block->getNextBlock()->getFrequency())) > 130))))
                    branchIsHotter = TR_yes;
            }
        }
    }
    ...
    if (!keepingItAliveAcrossEdgeRequiresExtraGoto) {
        ...
        if (successorRegister->getRegisterCandidateOnEntry() != 0 || successorBlock->getEntry()->getNode()->getVisitCount() == _visitCount || !(successorBlock->getPredecessors().size() == 1) || (successorRegister->getRegisterCandidateOnExit() && (successorRegister->getRegisterCandidateOnExit() != rc))) {
    ...
    TR::Block* block, TR::Block* successorBlock, TR::TreeTop* exitTreeTop, TR::Node* exitNode, TR_RegisterCandidate* rc)
{
    ...
    if (!_newBlocks.isEmpty()) {
        ...
        for (nextNewBlock = newBlocksIt.getFirst(); nextNewBlock != NULL; nextNewBlock = newBlocksIt.getNext()) {
            ...
            if (!predBlock->getLastRealTreeTop()->getNode()->getOpCode().isBranch() || !block->getLastRealTreeTop()->getNode()->getOpCode().isBranch())
            ...
            if (newBlockCanBeReused) {
                ...
                if ((nextNewBlock->getSuccessors().size() == 1) && nextNewBlock->getSuccessors().front()->getTo() == successorBlock) {
                    ...
                    for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber; ++j) {
                        ...
                        if ((blockRegisters[j].getRegisterCandidateOnExit() != predRegisters[j].getRegisterCandidateOnExit()) || (blockRegistersEBB[j].getCurrentRegisterCandidate() != predRegisters[j].getRegisterCandidateOnExit()) || (blockRegisters[j].getRegisterCandidateOnExit() != nextNewRegisters[j].getRegisterCandidateOnEntry()) || (blockRegistersEBB[j].getCurrentRegisterCandidate() != nextNewRegisters[j].getRegisterCandidateOnEntry())) {
                            newBlockCanBeReused = false;
                            break;
                        }
                    }
    ...
    if (!newBlock) {
        ...
        for (j = _firstGlobalRegisterNumber; j <= _lastGlobalRegisterNumber; ++j) {
            if (successorRegisters[j].getRegisterCandidateOnEntry()) {
                ...
                TR_ASSERT(!blockRegisters[j].getRegisterCandidateOnExit() || (blockRegisters[j].getRegisterCandidateOnExit() == successorRegisters[j].getRegisterCandidateOnEntry()) || (blockRegisters[j].getRegisterCandidateOnEntry() == successorRegisters[j].getRegisterCandidateOnEntry()), "Candidates are different in a global register across a CFG edge\n");
...
TR_GlobalRegisterAllocator::createBlock(TR::Block* block, TR::Block* successorBlock)
{
    ...
    if (_candidates->_startOfExtendedBBForBB[successorBlock->getNumber()] == block && block != successorBlock)
...
TR_GlobalRegister::optimalPlacementForStore(TR::Block* currentBlock, TR::Compilation* comp)
{
    ...
    if (lastRefBlock != currentBlock) {
        ...
        if (prevOpCode.isBranch() || prevOpCode.isJumpWithMultipleTargets() || prevOpCode.isReturn() || prevOpCode.getOpCodeValue() == TR::athrow || prevOpCode.getOpCodeValue() == TR::BBEnd) {
...
TR_GlobalRegister::createStoreToRegister(TR::TreeTop* prevTreeTop, TR::Node* node, vcount_t visitCount, TR::Compilation* comp, TR_GlobalRegisterAllocator* gra)
{
    ...
    if (TR::Compiler->target.is64Bit() && (store->getOpCodeValue() == TR::iRegStore) && gra->candidateCouldNeedSignExtension(rc->getSymbolReference()->getReferenceNumber()) && enableSignExtGRA) {
        store->setNeedsSignExtension(true);
    }
    ...
    if (!debug("oldIfThen")) {
        ...
        while (paramCursor != NULL) {
            if (paramCursor->isReferencedParameter()) {
                ...
                for (TR::CFGNode* block = cfg->getFirstNode(); block; block = block->getNext()) {
                    TR_BitVector* liveLocals = toBlock(block)->getLiveLocals();
                    if (cg()->getLiveLocals() && liveLocals && !rc->_blocks.find(block->getNumber())) {
                        if (block != comp()->getStartBlock() && block != cfg->getStart() && block != cfg->getEnd() && liveLocals->get(paramCursor->getLiveLocalIndex())) {
                            rc->_blocks.setNumberOfLoadsAndStores(toBlock(block)->getNumber(), 0);
                        }
                    }
                }
        ...
        while (autoCursor != NULL) {
            //if (paramCursor->isReferencedParameter())
            {
                ...
                for (TR::CFGNode* block = cfg->getFirstNode(); block; block = block->getNext()) {
                    TR_BitVector* liveLocals = toBlock(block)->getLiveLocals();
                    if (cg()->getLiveLocals() && liveLocals && !rc->_blocks.find(block->getNumber())) {
                        if (block != comp()->getStartBlock() && block != cfg->getStart() && block != cfg->getEnd() && liveLocals->get(autoCursor->getLiveLocalIndex())) {
                            rc->_blocks.setNumberOfLoadsAndStores(toBlock(block)->getNumber(), 0);
                        }
                    }
                }
    ...
    for (int32_t symRefNumber = symRefTab->getIndexOfFirstSymRef(); symRefNumber < symRefCount; symRefNumber++) {
        ...
        if (symRef && isSymRefAvailable(symRef)) {
            ...
            if (sym) {
                ...
                if ((sym->isAuto() && methodSymbol->getAutomaticList().find(sym->castToAutoSymbol()) && !onlySelectedCandidates)) {
        ...
        if (symRef) {
            ...
            if (sym) {
                ...
                if ((sym->getDataType() == TR::Float
                    ...
                    && isTypeAvailable(symRef) && ((sym->isAuto() && methodSymbol->getAutomaticList().find(sym->castToAutoSymbol())) || (sym->isParm() && methodSymbol->getParameterList().find(sym->castToParmSymbol()) && sym->isReferencedParameter()))) {
...
void TR_GlobalRegisterAllocator::findLoopsAndCorrespondingAutos(TR_StructureSubGraphNode* structureNode, vcount_t visitCount, SymRefCandidateMap& registerCandidates)
{
    ...
    if (structure->asRegion()) {
        ...
        if (!regionStructure->isAcyclic() && structureNode) {
            ...
            bool excludeInvariantsEnabled = comp()->cg()->excludeInvariantsFromGRAEnabled() && (!comp()->getOption(TR_DisableRXusage));
    ...
    bool hasCatchBlock)
{
    ...
    if (node->getOpCode().isLoadVarDirect() && node->getSymbolReference()->getSymbol()->isAuto()) {
        ...
        if (TR::Compiler->target.is64Bit() && info && (parent->getOpCodeValue() == TR::i2l) && node->isNonNegative() && enableSignExtGRA) {
                    ...
                    if (defNode->getOpCode().isStore()) {
                        ...
                        if (sym == node->getSymbolReference()->getSymbol() && (sym->isAuto())) {
                            ...
                            if (dependentStore && defNode->getFirstChild()->isNonNegative() && seenLoad) {
                                ...
                                if (comp()->getFlowGraph()->getStructure() && blockStructure && defBlockStructure && (blockStructure->getContainingLoop() == defBlockStructure->getContainingLoop()))
                                    continue;
                            }
    ...
    if (node->getOpCode().isLoadVarDirect() || node->getOpCode().isStoreDirect()) {
        ...
        if (symRef->getSymbol()->isAutoOrParm() && isSymRefAvailable(symRef, blocksInLoop)) {
            ...
            if (block != cfg->getStart()) {
                ...
                //
                if ((node->getOpCode().isStoreDirect() && isSplittingCopy(node)) || (node->getOpCode().isLoadVarDirect() && parent && parent->getOpCode().isStoreDirect() && isSplittingCopy(parent))) {
                    rc->addBlock(block, 0);
                } else {
                    if (TR::Compiler->target.cpu.isZ() && rc->getSymbolReference()->getSymbol()->getDataType() == TR::Address && parent && (((parent->getOpCode().isStoreIndirect() || parent->getOpCode().isLoadIndirect()) && parent->getFirstChild() == node) || (parent->getOpCode().isAdd() && grandParent && (grandParent->getOpCode().isStoreIndirect() || grandParent->getOpCode().isLoadIndirect())))) {
            ...
            if (((node->getReferenceCount() > 1) || node->getOpCode().isStoreDirect()) && symsThatShouldNotBeAssignedInCurrentLoop)
            ...
            if (symsThatShouldNotBeAssignedInCurrentLoop && node->getOpCode().isLoadVarDirect()) {
                if (parent->getNumChildren() == 2) {
                    if (parent->getSecondChild()->getOpCode().isLoadConst())
                        ...
                        if (otherChild->getOpCode().isLoadVarDirect() && (otherChild->getSymbolReference()->getSymbol()->isAutoOrParm()))
    ...
    if (symsThatShouldNotBeAssignedInCurrentLoop && assignedAutosInCurrentLoop && ((opCode.hasSymbolReference() && opCode.isIndirect()) || (opCode.isArrayLength()) || !(opCode.isAdd() || opCode.isSub() || opCode.isMul() || opCode.isBooleanCompare() || opCode.isNullCheck() || opCode.isBndCheck())))
...
void TR_GlobalRegisterAllocator::signExtendAllDefNodes(TR::Node* defNode, List<TR::Node>* defNodes)
{
    ...
    if (((childOfDefNode->getOpCodeValue() == TR::iadd) || (childOfDefNode->getOpCodeValue() == TR::isub)) && childOfDefNode->getFirstChild()->getOpCode().isLoadVarDirect() && childOfDefNode->getFirstChild()->getSymbolReference()->getSymbol()->isAuto() && childOfDefNode->getSecondChild()->getOpCode().isLoadConst() && ((childOfDefNode->getSecondChild()->getInt() <= 32767) && (childOfDefNode->getSecondChild()->getInt() >= -32767))) {
        int32_t useIndex;
        if ((childOfDefNode->getOpCodeValue() == TR::iadd) || (childOfDefNode->getOpCodeValue() == TR::isub)) {
            useIndex = childOfDefNode->getFirstChild()->getUseDefIndex();
            if (((childOfDefNode->getOpCodeValue() == TR::iadd) && (childOfDefNode->getSecondChild()->getInt() < 0)) || ((childOfDefNode->getOpCodeValue() == TR::isub) && (childOfDefNode->getSecondChild()->getInt() > 0)))
                defNode->setNeedsSignExtension(true);
        } else {
            defNode->setNeedsSignExtension(true);
...
void TR_GlobalRegisterAllocator::findSymsUsedInIndirectAccesses(TR::Node* node, TR_BitVector* symsThatShouldNotBeAssignedInCurrentLoop, TR_BitVector* assignedAutosInCurrentLoop, bool examineChildren)
{
    if (symsThatShouldNotBeAssignedInCurrentLoop && node->getOpCode().hasSymbolReference() && (node->getSymbolReference()->getSymbol()->isAutoOrParm())) {
        TR::SymbolReference* symRef = node->getSymbolReference();
        symsThatShouldNotBeAssignedInCurrentLoop->reset(symRef->getReferenceNumber());
    }
...
TR_PairedSymbols* TR_GlobalRegisterAllocator::findPairedSymbols(TR::SymbolReference* symRef1, TR::SymbolReference* symRef2)
{
    ...
    TR_PairedSymbols* p;
    for (p = pairs.getFirst(); p; p = pairs.getNext()) {
        if (((p->_symRef1 == symRef1) && (p->_symRef2 == symRef2)) || ((p->_symRef1 == symRef2) && (p->_symRef2 == symRef1)))
            return p;
    }
    return NULL;
}
...
void TR_GlobalRegisterAllocator::findLoopsAndCreateAutosForSignExt(TR_StructureSubGraphNode* structureNode, vcount_t visitCount)
{
    TR_Structure* structure;
    if (structureNode)
        structure = structureNode->getStructure();
...
static bool canSplit(TR::SymbolReference* symRef, TR::Compilation* comp)
{
    ...
    //
    if (symRef->getSymbol()->isAutoOrParm() && !addr_taken && !symRef->getSymbol()->isInternalPointer() && !symRef->getSymbol()->dontEliminateStores(comp) && !symRef->getSymbol()->isLocalObject() && symRef->getUseonlyAliases().isZero(comp))
        return true;
        ...
        if (regionStructure->isNaturalLoop() && structureNode) {
            ...
            if (loopInvariantBlock) {
                ...
                while ((i < symRefCount) && (prevSymRefCount == 0)) {
                    ...
                    if (origSymRef && origSymRef->getSymbol()->isAutoOrParm() && comp()->cg()->considerTypeForGRA(origSymRef) && canSplit(symRef, comp()) && !replacedAutosInCurrentLoop->get(i) && !autosThatCannotBeReplacedInCurrentLoop->get(i)) {
                        ...
                        //if (rc)
                        {
                            ...
                            for (exitBlock = exitBlocksIt.getCurrent(); exitBlock; exitBlock = exitBlocksIt.getNext()) {
                                ...
                                if (liveLocals && origSymRef->getSymbol()->getAutoSymbol() && liveLocals->get(origSymRef->getSymbol()->getAutoSymbol()->getLiveLocalIndex()))
                    ...
                    if (candidateIsLiveOnExit) {
                        ...
                        if (((!isFloat && (_numberOfGPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalGPRs() - 2) || (isFloat && (_numberOfFPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalFPRs() - 2)) && performTransformation(comp(), "%s replace auto #%d in loop %d (%p)\n", OPT_DETAILS, symRef->getReferenceNumber(), structureNode->getNumber(), structureNode)) {
                ...
                while (i < symRefCount) {
                    if (correspondingSymRefs[i])
                        _origSymRefs[correspondingSymRefs[i]->_symRef->getReferenceNumber()] = _origSymRefs[i] ? _origSymRefs[i] : comp()->getSymRefTab()->getSymRef(i);
                    i++;
                }
            } else
    ...
    TR_StructureSubGraphNode* loop, TR::Block* loopInvariantBlock)
{
    ...
    if (node->getOpCode().isLoadVarDirect() || (node->getOpCodeValue() == TR::loadaddr) || node->getOpCode().isStoreDirect()) {
        ...
        TR::SymbolReference* symRef = node->getSymbolReference();
        comp()->setCurrentBlock(block);
        if (comp()->cg()->considerTypeForGRA(symRef) && canSplit(symRef, comp()) && !autosThatCannotBeReplacedInCurrentLoop->get(symRef->getReferenceNumber())) {
            ...
            if (!correspondingSymRefCandidate) {
                ...
                //
                if (((!isFloat && (_numberOfGPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalGPRs() - 2) || (isFloat && (_numberOfFPRs + numRegsForCandidate) <= comp()->cg()->getNumberOfGlobalFPRs() - 2)) &&
                    /* candidateIsLiveOnExit && */ candidateIsLiveOnEntry && performTransformation(comp(), "%s replace auto #%d in loop %d (%p)\n", OPT_DETAILS, symRef->getReferenceNumber(), loop->getNumber(), loop)) {
                    _changedSomething = true;
            ...
            if (correspondingSymRef) {
                ...
                if (!node->getOpCode().isStoreDirect() || (!dontReplaceStores || performTransformation(comp(), "%s --- going to replace auto #%d by auto #%d on %s node %p --- \n", OPT_DETAILS, symRef->getReferenceNumber(), correspondingSymRef->getReferenceNumber(), node->getOpCode().getName(), node))) {
...
void TR_LiveRangeSplitter::appendStoreToBlock(TR::SymbolReference* storeSymRef, TR::SymbolReference* loadSymRef, TR::Block* block, TR::Node* node)
{
    ...
    if (!placeHolderOpCode.isBranch() && !placeHolderOpCode.isJumpWithMultipleTargets() && !placeHolderOpCode.isReturn() && (placeHolderOpCode.getOpCodeValue() != TR::athrow))
    ...
    while (placeHolderTreeForLoad && placeHolderTreeForLoad->getNode()->getOpCode().isStore()) {
        if ((placeHolderTreeForLoad->getNode()->getSymbolReference() == loadSymRef) || (placeHolderTreeForLoad->getNode()->getSymbolReference()->sharesSymbol() && placeHolderTreeForLoad->getNode()->getSymbolReference()->getUseDefAliases().contains(loadSymRef, comp())))
            break;
...
void TR_LiveRangeSplitter::prependStoreToBlock(TR::SymbolReference* storeSymRef, TR::SymbolReference* loadSymRef, TR::Block* block, TR::Node* node)
{
    ...
    while (placeHolderTreeForStore && (placeHolderTreeForStore->getNode()->getOpCodeValue() == TR::treetop)) {
        ...
        if (!ttChild->getOpCode().hasSymbolReference() || !ttChild->getSymbolReference()->getSymbol()->isAutoOrParm() || (ttChild->getSymbolReference() == storeSymRef) || (ttChild->getSymbolReference()->sharesSymbol() && ttChild->getSymbolReference()->getUseDefAliases().contains(storeSymRef, comp())))
            break;
...
void TR_LiveRangeSplitter::placeStoresInLoopExits(TR::Node* node, TR_StructureSubGraphNode* loop, List<TR::Block>* blocksInLoop, TR::SymbolReference* orig, TR::SymbolReference* replacement)
{
                ...
                for (nextEdge = ei.getFirst(); nextEdge; nextEdge = ei.getNext()) {
                    ...
                    if (pred->getLastRealTreeTop()->getNode()->getOpCode().isBranch() && !_splitBlocks.isEmpty()) {
                        ...
                        for (nextSplitBlock = splitBlocksIt.getFirst(); nextSplitBlock != NULL; nextSplitBlock = splitBlocksIt.getNext()) {
                            ...
                            if (exitBlockIsASplitBlock || ((nextSplitBlock->getSuccessors().size() == 1) && nextSplitBlock->getSuccessors().front()->getTo() == exitBlock)) {
