...
extern TR::TreeTop *createStoresForArraycopyChildren(
    TR::Compilation *comp, TR::TreeTop *arrayTree,
    TR::SymbolReference *&srcObjRef, TR::SymbolReference *&dstObjRef,
    TR::SymbolReference *&srcRef, TR::SymbolReference *&dstRef,
    TR::SymbolReference *&lenRef);

void collectArraylengthNodes(TR::Node *node, vcount_t visitCount,
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++)
    collectArraylengthNodes(node->getChild(childNum), visitCount,
                            arraylengthNodes);
}
...
OMR::ValuePropagation::Relationship *OMR::ValuePropagation::createRelationship(
  ...
  if (!rel) rel = new (trStackMemory()) Relationship;
...
OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::createStoreRelationship(TR::Symbol *symbol,
                                               Relationship *firstRel) {
  StoreRelationship *rel = _storeRelationshipCache.pop();
  if (!rel) rel = new (trStackMemory()) StoreRelationship;
  rel->symbol = symbol;
...
void OMR::ValuePropagation::freeStoreRelationships(
    TR_LinkHead<StoreRelationship> &list) {
...
void OMR::ValuePropagation::freeValueConstraints(
    ValueConstraints &valueConstraints) {
  _vcHandler.empty(valueConstraints);
}

OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::copyValueConstraints(
    ValueConstraints &valueConstraints) {
  return _vcHandler.copyAll(valueConstraints);
}

void OMR::ValuePropagation::addConstraint(TR::VPConstraint *constraint,
                                          int32_t hash) {
  ConstraintsHashTableEntry *entry =
      new (trStackMemory()) ConstraintsHashTableEntry;
...
OMR::ValuePropagation::LoopDefsHashTableEntry *
OMR::ValuePropagation::findLoopDef(TR::Node *node) {
...
static bool worthPropagatingConstraints(TR::Compilation *comp,
                                        bool isGlobalPropagation) {
  if ((!isGlobalPropagation && (comp->getMethodHotness() > cold)) ||
...
bool OMR::ValuePropagation::propagateConstraint(
    TR::Node *node, int32_t valueNumber, Relationship *first, Relationship *rel,
    ValueConstraints *valueConstraints) {
  ...
  for (cur = first;
       cur && worthPropagatingConstraints(comp(), _isGlobalPropagation);
       cur = cur->getNext()) {
    if (cur->relative == rel->relative) continue;
    ...
    if (rel->relative == AbsoluteConstraint) {
      ...
      c = cur->constraint->asRelation()->propagateAbsoluteConstraint(
          rel->constraint, cur->relative, this);
      if (c) {
        c = addConstraintToList(node, cur->relative, AbsoluteConstraint, c,
                                valueConstraints);
        if (!c) return false;
      }
      ...
      c = rel->constraint->asRelation()->propagateAbsoluteConstraint(
          cur->constraint, rel->relative, this);
      if (c) {
        c = addConstraintToList(node, rel->relative, AbsoluteConstraint, c,
                                valueConstraints);
        if (!c) return false;
      }
    } else {
      ...
      if (c) {
        c = addConstraintToList(node, cur->relative, rel->relative, c,
                                valueConstraints);
        if (!c) return false;
      }
    }
  ...
  if (rel->relative != AbsoluteConstraint) {
    TR_ASSERT(rel->constraint->asRelation(), "assertion failure");
    if (!addConstraintToList(node, rel->relative, valueNumber,
                             rel->constraint->asRelation()->getComplement(this),
                             valueConstraints))
      return false;
  }
...
//
TR::VPConstraint *OMR::ValuePropagation::addConstraintToList(
    TR::Node *node, int32_t valueNumber, int32_t relative,
    TR::VPConstraint *constraint, ValueConstraints *valueConstraints,
  ...
  if (!existingGlobalConstraint) {
    if (!cur) cur = _vcHandler.findOrCreate(valueNumber, *valueConstraints);

    // Find the relationship. The list is ordered by relative value number.
    //
    int32_t numRelatives = 0;
    for (rel = cur->relationships.getFirst(), prevRel = NULL;
         rel && rel->relative < relative; prevRel = rel, rel = rel->getNext()) {
      numRelatives++;
    }
    ...
    //
      ...
      c = replaceExisting ? constraint
                          : constraint->intersect(rel->constraint, this);
  ...
  if (cur) {
    for (store = cur->storeRelationships.getFirst(); store;
         store = store->getNext()) {
      if (isUnreachableStore(store)) continue;
      ...
      if (updateStoreConstraint) {
        ...
        if (!storeRel) {
          ...
          if (TR::VPConstraint::isSpecialClass(
                  (uintptrj_t)constraint->getClass())) {
            TR_ASSERT(constraint->asClass(),
                      "special class constraint must be VPClass");
            traceMsg(comp(), "found special class constraint!\n");
            // remove the type information
            storeConstraint = TR::VPClass::create(
                this, NULL, constraint->getClassPresence(),
                constraint->getPreexistence(), constraint->getArrayInfo(),
                constraint->getObjectLocation());
          } else
        ...
        if (storeConstraint) {
          if (storeConstraint != storeRel->constraint) {
            ...
            if (trace() && node) {
              traceMsg(comp(), "   %s [%p] gets new store constraint:",
    ...
    }    // store = cur->storeRelationships.getFirst(); store; store =
         // store->getNext()
  }      // if (cur)

  if (existingGlobalConstraint) return existingGlobalConstraint;

  if (newOrChanged) {
    if (trace() && node) {
      traceMsg(comp(),
               "   n%in %s gets new constraint:", node->getGlobalIndex(),
               node->getOpCode().getName());
      rel->print(this, valueNumber, 1);
      traceMsg(comp(),
               "type of constraint - longConstraint: %p, intConstraint: %p, "
               "shortConstraint: %p\n",
               rel->constraint->asLongConstraint(),
               rel->constraint->asIntConstraint(),
    ...
    if (valueNumber < _firstUnresolvedSymbolValueNumber) {
      ...
      if (!propagateConstraint(node, valueNumber, cur->relationships.getFirst(),
                               rel, valueConstraints))
        propFailed = true;

      static const bool relPropGlobal =
          feGetEnv("TR_enableVPRelPropGlobal") != NULL;
      if (relPropGlobal) {
        ...
        if (!propFailed && globalConstraint != NULL) {
          ...
          if (!propagateConstraint(node, valueNumber, firstReln, rel,
                                   valueConstraints))
            propFailed = true;
        }
      }
...
TR::VPConstraint *OMR::ValuePropagation::addGlobalConstraint(
  ...
  return addGlobalConstraint(node, getValueNumber(node), constraint,
                             relativeVN);
}

TR::VPConstraint *OMR::ValuePropagation::addGlobalConstraint(
    TR::Node *node, int32_t valueNumber, TR::VPConstraint *constraint,
    int32_t relative) {
  ...
  //
  GlobalConstraint *gc = findGlobalConstraint(valueNumber);
  if (!gc) gc = createGlobalConstraint(valueNumber);

  int32_t numRelatives = 0;
  Relationship *rel, *prev;
  for (rel = gc->constraints.getFirst(), prev = NULL;
       rel && rel->relative < relative; prev = rel, rel = rel->getNext()) {
    numRelatives++;
  }
  ...
  if (trace() && c == NULL)
    traceMsg(comp(), "Cannot intersect constraints on %s [%p]",
             node->getOpCode().getName(), node);
  // The assume below is informational in nature and should therefore not be
  // triggered if production builds include assumes (as on WCode or binopt).
  // This condition should rarely show up in WCode or binopt (as Simplifier
  // transformations will homogenize the types) but can happen, for example,
  // when an aggregate symbol is referenced in one place on an oload and another
  // as an iload. MG: Update assert?
  TR_ASSERT(c, "Cannot intersect constraints on %s [%p]",
            node->getOpCode().getName(), node);
  ...
  if (newOrChanged) {
    if (trace() && node) {
      traceMsg(comp(), "   %s [%p] gets new global constraint:",
               node->getOpCode().getName(), node);
      rel->print(this, valueNumber, 1);
    }
    ...
    // associated with this value number
    //
    bool propagateOK = propagateConstraint(
        node, valueNumber, gc->constraints.getFirst(), rel, NULL);
    ...
    if (trace() && node) {
      traceMsg(
          comp(),
          "   %s [%p] found existing global constraint value number %d (%p): ",
...
TR::VPConstraint *OMR::ValuePropagation::addEdgeConstraint(
    TR::Node *node, TR::VPConstraint *constraint, EdgeConstraints *constraints,
    TR::Node *relative) {
  if (!_isGlobalPropagation) return constraint;

  int32_t valueNumber = getValueNumber(node);
  int32_t relativeVN = relative ? getValueNumber(relative) : AbsoluteConstraint;
  constraint = addConstraintToList(node, valueNumber, relativeVN, constraint,
...
TR::VPConstraint *OMR::ValuePropagation::addBlockOrGlobalConstraint(
    TR::Node *node, TR::VPConstraint *constraint, bool isGlobal,
    TR::Node *relative) {
  if (isGlobal) return addGlobalConstraint(node, constraint, relative);

  return addBlockConstraint(node, constraint, relative);
}

void OMR::ValuePropagation::mergeRelationships(
    TR_LinkHead<Relationship> &fromList, TR_LinkHead<Relationship> &toList,
    int32_t valueNumber, bool preserveFrom, StoreRelationship *mergingStore,
    List<TR::Symbol> *storeSymbols, bool inBothLists) {
  if (trace())
  ...
  GlobalConstraint *gc = findGlobalConstraint(valueNumber);
  if (gc) global = gc->constraints.getFirst();
  ...
  //
  bool preserveStoreRelationships =
      true;  // comp()->fe()->aggressiveSmallAppOpts();

  if (!preserveFrom) fromList.setFirst(NULL);
  ...
  while (from || to) {
    if (!to || (from && from->relative < to->relative)) {
      ...
      //
      if (preserveStoreRelationships && mergingStore &&
          (from->relative == AbsoluteConstraint) &&
          (!inBothLists /* || !isToEmpty */))  // commented out the !isToEmpty
                                               // because it is not guaranteed
                                               // to be safe to take an absolute
                                               // constraint that only exists on
                                               // one side
      {
      ...
      rel = from->getNext();
      if (!preserveFrom) freeRelationship(from);
      from = rel;
      continue;
    }
...
void OMR::ValuePropagation::mergeStoreRelationships(ValueConstraint *fromvc,
                                                    ValueConstraint *tovc,
                                                    bool preserveFrom) {
  ...
  StoreRelationship *from =
      fromvc ? fromvc->storeRelationships.getFirst() : NULL;
  ...
  if (fromvc && !preserveFrom) fromvc->storeRelationships.setFirst(NULL);
  ...
  if (_loopInfo && _loopInfo->_backEdgeConstraints) {
    backEdgeStores = _vcHandler.find(
        valueNumber, _loopInfo->_backEdgeConstraints->valueConstraints);
  }
  ...
  while (from || to) {
    ...
    if (!from || (to && from->symbol > to->symbol)) {
      ...
      if (backEdgeStores) {
        store =
            findStoreRelationship(backEdgeStores->storeRelationships, symbol);
        if (store) {
      ...
      if (isUnreachableStore(from)) {
        ...
        //
        store = from->getNext();
        if (!preserveFrom) freeStoreRelationship(from);
        from = store;
        continue;
      }
      symbol = from->symbol;
      ...
      if (preserveFrom)
        store->relationships.setFirst(
            copyRelationships(from->relationships.getFirst()));
      else {
    ...
    if (!fromRel) {
      ...
      //
      if (!lastTimeThrough()) freeRelationships(*toRel);
    } else {
      // Merge the relationship lists
      //
      /// traceMsg(comp(), "before merge inBothLists %d isFromEmpty %d isToEmpty
      /// %d\n", inBothLists, fromRel->isEmpty(), toRel->isEmpty());
      mergeRelationships(*fromRel, *toRel, valueNumber, preserve,
                         prev /*mergingStore*/, &storeSymbols, inBothLists);
    }
  }
}

void OMR::ValuePropagation::mergeValueConstraints(ValueConstraint *fromvc,
                                                  ValueConstraint *tovc,
                                                  bool preserveFrom) {
  // First merge the non-store relationships.
  //
  mergeRelationships(fromvc->relationships, tovc->relationships,
                     fromvc->getValueNumber(), preserveFrom);
...
void OMR::ValuePropagation::mergeEdgeConstraints(EdgeConstraints *fromEdge,
                                                 EdgeConstraints *toEdge) {
  // Merge (i.e. logical OR) the "from" list into the "to" list.
...
void OMR::ValuePropagation::mergeConstraintIntoEdge(ValueConstraint *constraint,
                                                    EdgeConstraints *edge) {
  // Merge (i.e. logical OR) the value constraint into the list.
  //
  ValueConstraint *cur = _vcHandler.findOrCreate(constraint->getValueNumber(),
                                                 edge->valueConstraints);
...
void OMR::ValuePropagation::mergeBackEdgeConstraints(EdgeConstraints *edge) {
  ...
  for (vc = iter.getFirst(); vc; vc = iter.getNext()) {
    ValueConstraint *edgeVc =
        _vcHandler.findOrCreate(vc->getValueNumber(), edge->valueConstraints);
...
//
OMR::ValuePropagation::GlobalConstraint *
OMR::ValuePropagation::GlobalConstraint::create(TR::Compilation *c,
                                                int32_t valueNumber) {
...
//
inline uint32_t OMR::ValuePropagation::hashGlobalConstraint(
    int32_t valueNumber) {
  return valueNumber &
         _globalConstraintsHTMaxBucketIndex;  // exploiting the fact that size
                                              // is power of two
}
...
//
OMR::ValuePropagation::GlobalConstraint *
OMR::ValuePropagation::findGlobalConstraint(int32_t valueNumber) {
  if (!_isGlobalPropagation) return NULL;

  uint32_t hash = hashGlobalConstraint(valueNumber);
  GlobalConstraint *entry;
  for (entry = _globalConstraintsHashTable[hash]; entry; entry = entry->next) {
    if (entry->valueNumber == valueNumber) break;
  }
  return entry;
}
...
//
OMR::ValuePropagation::GlobalConstraint *
OMR::ValuePropagation::createGlobalConstraint(int32_t valueNumber) {
  uint32_t hash = hashGlobalConstraint(valueNumber);
...
//
OMR::ValuePropagation::EdgeConstraints *
OMR::ValuePropagation::EdgeConstraints::create(TR::Compilation *c,
                                               TR::CFGEdge *edge) {
...
OMR::ValuePropagation::EdgeConstraints *
OMR::ValuePropagation::createEdgeConstraints(TR::CFGEdge *edge,
                                             bool keepBlockList) {
  if (!_isGlobalPropagation) return NULL;
  ...
  //
  if (keepBlockList) {
    _vcHandler.setRoot(constraints->valueConstraints,
                       copyValueConstraints(_curConstraints));
  } else {
    _vcHandler.setRoot(constraints->valueConstraints,
                       _vcHandler.getRoot(_curConstraints));
...
// Can add more blocks to handle conservatively (no constraints pushed down) to
// save compile time Would need to modify addEdgeConstraints or
// addExceptionEdgeConstraints
//
...
void OMR::ValuePropagation::createExceptionEdgeConstraints(
  ...
  if (!_isGlobalPropagation) return;

  for (auto edge = _curBlock->getExceptionSuccessors().begin();
       edge != _curBlock->getExceptionSuccessors().end(); ++edge) {
    TR::Block *target = toBlock((*edge)->getTo());
    if (!target->canCatchExceptions(exceptions)) continue;
    ...
    if (isUnreachablePath(constraints)) {
      ...
      for (vc = iter.getFirst(); vc; vc = iter.getNext()) {
        if (!vc->storeRelationships.isEmpty()) {
          ValueConstraint *edgeVc = _vcHandler.findOrCreate(
              vc->getValueNumber(), constraints->valueConstraints);
          mergeStoreRelationships(vc, edgeVc, true);
        }
      }
    }
...
void OMR::ValuePropagation::removeConstraint(int32_t valueNumber,
                                             ValueConstraints &valueConstraints,
                                             int32_t relative) {
  ...
  if (vc) {
    ...
    for (rel = vc->relationships.getFirst(), prevRel = NULL; rel;
         prevRel = rel, rel = next) {
      next = rel->getNext();
      if (rel->relative > relative) break;
      if (rel->relative == relative) {
...
OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::findStoreRelationship(
    TR_LinkHead<StoreRelationship> &list, TR::Symbol *symbol) {
  StoreRelationship *rel;
  for (rel = list.getFirst(); rel; rel = rel->getNext()) {
    if (rel->symbol == symbol) return rel;
    if (rel->symbol > symbol) break;
  }
  return NULL;
}

OMR::ValuePropagation::Relationship *
OMR::ValuePropagation::findConstraintInList(TR_LinkHead<Relationship> &list,
                                            int32_t relative) {
  ...
  Relationship *rel;
  for (rel = list.getFirst(); rel; rel = rel->getNext()) {
    if (rel->relative == relative) return rel;
    if (rel->relative > relative) break;
  }
  return NULL;
}
...
OMR::ValuePropagation::Relationship *OMR::ValuePropagation::findValueConstraint(
  ...
  //
  ValueConstraint *vc = _vcHandler.find(valueNumber, valueConstraints);
  if (vc) return findConstraintInList(vc->relationships, relative);
  return NULL;
}

OMR::ValuePropagation::Relationship *OMR::ValuePropagation::findConstraint(
    int32_t valueNumber, int32_t relative) {
...
OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::findStoreValueConstraint(
  ...
  //
  ValueConstraint *vc = _vcHandler.find(valueNumber, valueConstraints);
  if (vc) return findStoreRelationship(vc->storeRelationships, symbol);
  return NULL;
}

OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::findStoreConstraint(int32_t valueNumber,
                                           TR::Symbol *symbol) {
  return findStoreValueConstraint(valueNumber, symbol, _curConstraints);
}

OMR::ValuePropagation::StoreRelationship *
OMR::ValuePropagation::findStoreEdgeConstraint(int32_t valueNumber,
                                               TR::Symbol *symbol,
                                               EdgeConstraints *edge) {
...
TR::VPConstraint *OMR::ValuePropagation::applyGlobalConstraints(
  ...
  for (rel = gc->constraints.getFirst(); rel; rel = rel->getNext()) {
    ...
    if (rel->relative == AbsoluteConstraint) continue;
    ...
    for (link = gc->constraints.getFirst(); link; link = link->getNext()) {
      ...
      if (link->relative > limit) break;
    }
    if (reverse && follow) {
      TR::VPConstraint *linkedConstraint;
      if (relative == AbsoluteConstraint)
        linkedConstraint =
            reverse->constraint->asRelation()->propagateAbsoluteConstraint(
                follow->constraint, valueNumber, this);
      else
        linkedConstraint =
            reverse->constraint->asRelation()->propagateRelativeConstraint(
...
TR::VPConstraint *OMR::ValuePropagation::getStoreConstraint(
    TR::Node *node, TR::Node *relative) {
  ...
  if (store) rel = findConstraintInList(store->relationships, relativeVN);

  // If there is no local constraint, see if there is a global constraint
  //
  if (!rel) rel = findGlobalConstraint(valueNumber, relativeVN);

  if (rel) {
    if (trace()) {
      traceMsg(comp(), "   %s [%p] has existing store constraint:",
               node->getOpCode().getName(), node);
      rel->print(this, valueNumber, 1);
    }
...
void OMR::ValuePropagation::createStoreConstraints(TR::Node *node) {
  ...
  TR_ASSERT(node->getOpCode().hasSymbolReference(),
            "Can't create store constraints if no symbol");
  TR::Symbol *symbol = node->getSymbol();

  // Find or create the store relationship
  //
  StoreRelationship *store, *prev;
  for (store = vc->storeRelationships.getFirst(), prev = NULL;
  ...
  store->relationships.setFirst(
      copyRelationships(vc->relationships.getFirst()));
}

bool OMR::ValuePropagation::hasBeenStored(int32_t valueNumber,
                                          TR::Symbol *symbol,
                                          ValueConstraints &valueConstraints) {
  // See if the given value number has a store constraint in the given list
  //
  return (findStoreValueConstraint(valueNumber, symbol, valueConstraints) !=
          NULL);
}
...
void OMR::ValuePropagation::setUnreachableStore(StoreRelationship *store) {
  ...
  //
  freeRelationships(store->relationships);
  store->relationships.setFirst(createRelationship(
      AbsoluteConstraint, TR::VPUnreachablePath::create(this)));
}
...
bool OMR::ValuePropagation::isDefInUnreachableBlock(int32_t defIndex) {
  ...
  TR_StructureSubGraphNode *defSubGraphNode =
      comp()->getFlowGraph()->getStructure()->findNodeInHierarchy(
          defBlock->getParentStructureIfExists(comp()->getFlowGraph()),
          defBlock->getNumber());
  ...
  while (defSubGraphNode) {
    ...
    if (!reachableInputEdgesFound &&
        ...
        (defSubGraphNode->getStructure()->getParent()->getNumber() ==
         defSubGraphNode->getNumber()))
      defSubGraphNode =
          comp()->getFlowGraph()->getStructure()->findNodeInHierarchy(
              defSubGraphNode->getStructure()->getParent()->getParent(),
              defBlock->getNumber());
    else
      defSubGraphNode = NULL;
  }

  if (!reachableInputEdgesFound) return true;
...
void OMR::ValuePropagation::checkTypeRelationship(TR::VPConstraint *lhs,
                                                  TR::VPConstraint *rhs,
                                                  int32_t &value,
                                                  bool isInstanceOf,
                                                  bool isCheckCast) {
  ...
  if (lhs->asClass() && rhs->asClass()) {
    ...
    bool noPresenceResult = (!presence && objectClass->getClassPresence() &&
                             castClass->getClassPresence());
    bool noTypeResult =
        (!newType && objectClass->getClassType() && castClass->getClassType());

    if (jlKlass && noPresenceResult) {
      if (trace()) traceMsg(comp(), "presences are incompatible\n");
      result = 0;
    } else if (jlKlass && noTypeResult) {
      if (trace()) traceMsg(comp(), "types are incompatible\n");
      result = 0;
    ...
    TR::VPConstraint *resultType =
        lhs->getClassType()->classTypesCompatible(rhs->getClassType(), this);
    if (jlKlass) {
      // check if intersection failed
      if (!resultType) result = 0;
...
void OMR::ValuePropagation::invalidateParmConstraintsIfNeeded(
    TR::Node *node, TR::VPConstraint *constraint) {
  if (_isGlobalPropagation) return;  // nothing to do

  if (!_parmValues) return;
  ...
  if (symRef && symRef->getSymbol()->getParmSymbol()) {
    ...
    if (parmConstraint) {
      if (trace())
        traceMsg(comp(),
                 "Checking compatibility of store node %p parm %d with value\n",
                 node, parmNum);
      int32_t result = 1;
      ...
      if (!result)  // there is some incompatibility
      {
        if (trace())
          traceMsg(comp(),
                   "   Store node %p to parm %d is not compatible with rhs, "
                   "invalidating _parms entry %p\n",
                   node, parmNum, _parmValues[parmNum]);
        _parmTypeValid[parmNum] =
            false;  // invalidate the info in the parms array
      }
    }
  }
}

TR::VPConstraint *OMR::ValuePropagation::mergeDefConstraints(TR::Node *node,
                                                             int32_t relative,
                                                             bool &isGlobal,
                                                             bool forceMerge) {
  ...
  if (_defMergedNodes->get(node->getGlobalIndex())) {
    if (trace())
      traceMsg(comp(),
               "Node n%dn has already been processed by mergeDefConstraints - "
               "returning NULL\n",
               node->getGlobalIndex());
    return NULL;
  }
  ...
  if (!_isGlobalPropagation) {
    if (relative == AbsoluteConstraint) {
      TR::SymbolReference *symRef = node->getOpCode().hasSymbolReference()
  ...
  if (!useDefIndex || !_useDefInfo->isUseIndex(useDefIndex)) return NULL;
  ...
  if (!forceMerge && defNode) {
    if (trace())
      traceMsg(comp(), "   %s [%p] has singleDefiningLoad [%p] ",
               node->getOpCode().getName(), node, defNode);
    if (defNode->getVisitCount() >= _visitCount) {
      if (trace()) traceMsg(comp(), "  whose constraints already computed\n");
      return NULL;
    }
    if (trace()) traceMsg(comp(), "  going to merge\n");
  }

  TR_UseDefInfo::BitVector defs(comp()->allocator());
  if (!_useDefInfo->getUseDef(defs, useDefIndex)) return NULL;
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
      if (trace())
        traceMsg(comp(), "   %s [%p] looking at method entry def point: ",
                 node->getOpCode().getName(), node);
      ...
      //
      if (defNode == 0 || defNode->getUseDefIndex() == 0) continue;

      defValueNumber = getValueNumber(defNode);
      // before we can look at def nodes to build up a set of contraints we had
      // better know that set of defs dominates the use we are considering - if
      // not we treat the use as an unknown
      if (node->getSymbol()->isAutoOrParm() && _curDefinedOnAllPaths &&
          !_curDefinedOnAllPaths->get(
              node->getSymbolReference()->getReferenceNumber())) {
        if (trace())
          traceMsg(
              comp(),
              "symRef %d is not stored on all paths - treating as unknown\n",
      ...
      //
      if (trace())
        traceMsg(comp(), "   %s [%p] looking at def value %d, %s [%p]: ",
                 node->getOpCode().getName(), node, getValueNumber(defNode),
                 defNode->getOpCode().getName(), defNode);

      if (defNode->getOpCode().isStore()) {
        // TODO : This can be improved to return a value by checking which
        // byte(s) are written to and taking the rhs and applying it properly to
        // get the correct value for (potentially wider) datatype of the use
        //
        if (node->getDataType() != defNode->getDataType()) return NULL;

        // Check for indirect reference
        //
        bool matchingDef = true;
        if (baseValueNumber >= 0) {
          TR_ASSERT(defNode->getOpCode().isIndirect(),
                    "Expected indirect store");
          if (defNode->getOpCode().isIndirect()) {
            matchingDef =
                (baseValueNumber == getValueNumber(defNode->getFirstChild()));
          } else {
            matchingDef = false;
          }
        }
        ...
        if (matchingDef) {
          ...
          if (defValueConstraint) {
            rel = findConstraintInList(defValueConstraint->relationships,
                                       relative);
            if (rel) {
              if (rel->constraint->asUnreachablePath()) {
                ...
                if (trace())
                  traceMsg(comp(),
                           "def is unreachable (constraint), ignored\n");
                continue;
              }
              isGlobal = false;
            } else {
            ...
            if (rel) defConstraint = rel->constraint;
          }
            ...
            if (!isDefInUnreachableBlock(defIndex)) {
              ...
              if (trace())
                traceMsg(
                    comp(),
                    "def is unreachable (defInUnreachableBlock), ignored\n");
              continue;
            }
#if 0
    ...
    if (defIsUnseen) {
      if (trace()) traceMsg(comp(), "def has not been seen yet\n");
    } else if (!defConstraint) {
      if (trace()) traceMsg(comp(), "no constraint\n");
      constraint = NULL;
      unconstrained = true;
    } else {
      if (trace()) {
        ...
        if (defNode->getOpCode().isStoreDirect()) {
          ...
          if ((firstChild->getOpCodeValue() == TR::acall) ||
            ...
            if (!symRef->isUnresolved() &&
                symRef->getSymbol()->isResolvedMethod()) {
              TR::ResolvedMethodSymbol *method =
                  symRef->getSymbol()->getResolvedMethodSymbol();

              if ((method->getRecognizedMethod() ==
      ...
      if (mergeConstraint) {
        ...
        if (constraint) {
          ...
          if (!constraint) unconstrained = true;
  ...
  if (!lastTimeThrough()) {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      defIndex = cursor;
      if (defIndex < _useDefInfo->getFirstRealDefIndex()) continue;

      defNode = _useDefInfo->getNode(defIndex);

      if (defNode) {
        defValueNumber = getValueNumber(defNode);
        if (hasBeenStored(defValueNumber, defNode->getSymbol(),
                          _curConstraints))
          addLoopDef(defNode);
      }
    }
  ...
  // Last time through, and there were unseen defs: if we are already
  // unconstrained then there is no point in looking at the backedge defs
  //
  if (unconstrained) return constraint;
      ...
      if (hasBeenStored(defValueNumber, sym, _curConstraints)) {
        ...
        if (entry) {
          for (; container; container = container->_parent) {
            if (container->_loop == entry->region) break;
          }
        }
      }
    }
  ...
  if (relative != AbsoluteConstraint) {
    if (trace())
      traceMsg(comp(),
               "we are processing a relative constraint and merging with "
               "backedge constraints is not supported\n");
    return NULL;
  }
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (defNode->getUseDefIndex() == 0) continue;

    sym = defNode->getSymbol();
    defValueNumber = getValueNumber(defNode);
    // if we haven't seen a def on this iteration along all paths we must
    // consider the backedge constraints
    if (node->getSymbol()->isAutoOrParm() && !unseenDefsFound &&
        !_curDefinedOnAllPaths->get(
            defNode->getSymbolReference()->getReferenceNumber())) {
      if (trace())
        traceMsg(comp(),
                 "symref %d is not defined on all paths - consulting backedge "
                 "constraints\n",
                 node->getSymbolReference()->getReferenceNumber());
      ...
      unseenDefsFound = true;
      LoopInfo *loopInfo;
      for (loopInfo = _loopInfo; loopInfo && loopInfo != container;
           loopInfo = loopInfo->_parent) {
        if (trace())
          traceMsg(
              comp(),
              "   %s [%p] looking at back edges in loop %d for def %s [%p]: ",
              node->getOpCode().getName(), node, loopInfo->_loop->getNumber(),
              defNode->getOpCode().getName(), defNode);
        ...
        if (!hasBeenStored(defValueNumber, sym,
                           loopInfo->_backEdgeConstraints->valueConstraints)) {
          if (trace()) traceMsg(comp(), "no backedge constraint, ignored\n");
          continue;
        }
        ...
        //
        rel = NULL;
        defValueConstraint = findStoreEdgeConstraint(
            defValueNumber, sym, loopInfo->_backEdgeConstraints);
        // if (defValueConstraint)
        //   rel = findConstraintInList(defValueConstraint->relationships,
        //   getValueNumber(node));
        // if (!rel)
        ...
        if (defConstraint && defConstraint->asEqual() &&
            defConstraint->asEqual()->increment() == 0) {
          if (trace()) traceMsg(comp(), "trivially equal, ignored\n");
          continue;
        }
        ...
        if (defConstraint->asUnreachablePath()) {
          if (trace()) traceMsg(comp(), "def is unreachable, ignored\n");
          continue;
        }
          ...
          if (defNode->getOpCode().isStoreDirect()) {
            ...
            if ((firstChild->getOpCodeValue() == TR::acall) ||
              ...
              if (!symRef->isUnresolved()) {
                ...
                if (method && ((method->getRecognizedMethod() ==
                                TR::java_math_BigDecimal_add) ||
                               (method->getRecognizedMethod() ==
                                TR::java_math_BigDecimal_subtract) ||
                               (method->getRecognizedMethod() ==
        ...
        if (mergeConstraint) {
          ...
          if (constraint) {
            //
            // If its an induction var, check the direction of increment and do
            // not change the bound in the direction opposite to the increment
            //
            InductionVariable *ivInfo;
            for (ivInfo = loopInfo->_inductionVariables.getFirst(); ivInfo;
                 ivInfo = ivInfo->getNext()) {
              if (ivInfo->_symbol == sym) break;
            }
            ...
            if (sym->getType().isInt32()) {
              if (positiveIncrement) {
                if (defConstraint->isUnsigned()) {
                  if ((uint32_t)defConstraint->getHighInt() <
                      TR::getMaxUnsigned<TR::Int32>()) {
                    if ((uint32_t)defConstraint->getLowInt() <
                            (uint32_t)constraint->getLowInt() &&
                        (uint32_t)constraint->getLowInt() <=
                            defConstraint->getHighInt()) {
                      TR::VPConstraint *newDefConstraint =
                          TR::VPIntRange::create(this, constraint->getLowInt(),
                                                 defConstraint->getHighInt());
                      defConstraint =
                          defConstraint->intersect(newDefConstraint, this);
                    }
                  }
                } else if (defConstraint->getHighInt() <
              ...
              if (positiveIncrement) {
                if (defConstraint->isUnsigned()) {
                  if ((uint16_t)defConstraint->getHighShort() <
                      TR::getMaxUnsigned<TR::Int16>()) {
                    if ((uint16_t)defConstraint->getLowShort() <
                            (uint16_t)constraint->getLowShort() &&
                        (uint16_t)constraint->getLowShort() <=
                            defConstraint->getHighShort()) {
                      TR::VPConstraint *newDefConstraint =
                          TR::VPShortRange::create(
                              this, constraint->getLowShort(),
                              defConstraint->getHighShort());
                      defConstraint =
                          defConstraint->intersect(newDefConstraint, this);
                    }
                  }
                } else if (defConstraint->getHighShort() <
                           TR::getMaxSigned<TR::Int16>()) {
                  if (defConstraint->getLowShort() <
                          constraint->getLowShort() &&
                      constraint->getLowShort() <=
                          defConstraint->getHighShort()) {
                    TR::VPConstraint *newDefConstraint =
                        TR::VPShortRange::create(this,
                                                 constraint->getLowShort(),
                                                 defConstraint->getHighShort());
                    defConstraint =
                        defConstraint->intersect(newDefConstraint, this);
                ...
                if (defConstraint->getLowShort() >
                  ...
                  if (defConstraint->getLowShort() <=
                          constraint->getHighShort() &&
                      constraint->getHighShort() <
                          defConstraint->getHighShort()) {
                    TR::VPConstraint *newDefConstraint =
                        TR::VPShortRange::create(this,
                                                 defConstraint->getLowShort(),
                                                 constraint->getHighShort());
                    defConstraint =
                        defConstraint->intersect(newDefConstraint, this);
                  }
              ...
              if (positiveIncrement) {
                if (defConstraint->isUnsigned()) {
                  if ((uint64_t)defConstraint->getHighLong() <
                      TR::getMaxUnsigned<TR::Int64>()) {
                    if ((uint64_t)defConstraint->getLowLong() <
                            (uint64_t)constraint->getLowLong() &&
                        (uint64_t)constraint->getLowLong() <=
                            (uint64_t)defConstraint->getHighLong()) {
                      TR::VPConstraint *newDefConstraint =
                          TR::VPLongRange::create(this,
                                                  constraint->getLowLong(),
                                                  defConstraint->getHighLong());
                      defConstraint =
                          defConstraint->intersect(newDefConstraint, this);
                    }
                  }
                } else if (defConstraint->getHighLong() <
                           TR::getMaxSigned<TR::Int64>()) {
                  if (defConstraint->getLowLong() < constraint->getLowLong() &&
                      constraint->getLowLong() <=
                          defConstraint->getHighLong()) {
                    TR::VPConstraint *newDefConstraint =
                        TR::VPLongRange::create(this, constraint->getLowLong(),
                                                defConstraint->getHighLong());
                    defConstraint =
                        defConstraint->intersect(newDefConstraint, this);
                  }
                ...
                if (defConstraint->getLowLong() >
                    TR::getMinSigned<TR::Int64>()) {
                  if (defConstraint->getLowLong() <=
                          constraint->getHighLong() &&
                      constraint->getHighLong() <
                          defConstraint->getHighLong()) {
                    TR::VPConstraint *newDefConstraint =
                        TR::VPLongRange::create(this,
                                                defConstraint->getLowLong(),
                                                constraint->getHighLong());
                    defConstraint =
                        defConstraint->intersect(newDefConstraint, this);
                  }
...
void OMR::ValuePropagation::replaceByConstant(TR::Node *node,
  ...
  if (!isGlobal && !lastTimeThrough()) return;

  if (!performTransformation(
          comp(), "%sConstant folding %s [" POINTER_PRINTF_FORMAT "]",
  ...
  switch (type) {
    ...
    case TR::Int16:
      ...
      (shortConstraint == NULL)
          ? node->setShortInt(constraint->asIntConst()->getInt())
    ...
    case TR::Address:
      ...
      node->setAddress(0);
      TR_ASSERT(constraint->isNullObject(),
                "Only null address constant supported");
...
void OMR::ValuePropagation::mustTakeException() {
  ...
  if (!performTransformation(
          comp(), "%sRemoving rest of block after %s [%p]\n", OPT_DETAILS,
  ...
  for (auto edge = _curBlock->getSuccessors().begin();
       edge != _curBlock->getSuccessors().end(); ++edge) {
...
bool OMR::ValuePropagation::registerPreXClass(TR::VPConstraint *constraint) {
  if (!constraint->isFixedClass()) return false;
  if (!constraint->isPreexistentObject()) return false;
  TR_OpaqueClassBlock *clazz = constraint->getClass();
  TR_OpaqueClassBlock *assumptionClazz =
...
TR::Node *OMR::ValuePropagation::findThrowInBlock(TR::Block *block,
                                                  TR::TreeTop *&treeTop) {
  // if (comp()->getFlowGraph()->getRemovedNodes().find(block))
  if (block->nodeIsRemoved()) return 0;

  treeTop = block->getLastRealTreeTop();
  TR::Node *node = treeTop->getNode();
  if (node->getOpCodeValue() != TR::athrow) {
    if (node->getOpCodeValue() == TR::Return)
      return 0;  // mustTakeException has been called on the block that use to
                 // contain our throw
    if (node->getNumChildren() != 1)
      return 0;  // inlining messed up the trees so that we can't find the throw
                 // now
...
void OMR::ValuePropagation::checkForInductionVariableIncrement(TR::Node *node) {
  ...
  TR::Symbol *sym = node->getSymbol();
  if (!sym->isAutoOrParm()) return;

  _loopInfo->_seenDefs->set(getValueNumber(node));

  TR::Node *child = node->getFirstChild();
  if (!(child->getOpCode().isAdd() || child->getOpCode().isSub())) return;
  ...
  bool isGlobal;
  TR::VPConstraint *increment =
      getConstraint(child->getSecondChild(), isGlobal);
  if (!increment) return;
  if (!(increment->asIntConst() || increment->asLongConst())) return;

  if (child->getOpCode().isSub()) {
    if (increment->asIntConst())
      increment =
          TR::VPIntConst::create(this, -increment->asIntConst()->getInt());
    else if (increment->asShortConst())
      increment = TR::VPShortConst::create(
          this, -increment->asShortConst()->getShort());
    else
      increment =
          TR::VPLongConst::create(this, -increment->asLongConst()->getLong());
  }
    ...
    if (!_useDefInfo->getUseDef(defs, useIndex))
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
          ...
          if (trace())
            traceMsg(comp(),
                     "      ivInfo is invalid for sym [%p] as method entry is "
                     "def point\n",
                     sym);
          break;
        }
        ...
        if (defNode->getOpCode().isStore()) {
          ...
          if (defTree && comp()->getFlowGraph()->getStructure()) {
            if (defTree->getEnclosingBlock()
                    ->getStructureOf()
                    ->getContainingLoop() != _loopInfo->_loop)
              notInLoop = true;
          }
        }

        if (!notInLoop || _loopInfo->_seenDefs->get(defVN)) continue;
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if ((defVN != incrementVN) && !notInLoop) {
          ...
          if (trace())
            traceMsg(
                comp(),
                "      ivInfo is invalid for sym [%p], def [%d] node %p does "
                "not have the same value number as another increment\n",
                sym, defVN, defNode);
          break;
        }

        if (((defVN != entryVN) && !notInLoop) ||
            ((defVN == entryVN) && notInLoop))
          continue;
        ...
        if (entryVN >= 0) {
          ...
          if (_loopInfo->_seenDefs->get(defVN)) {
            ...
            if (trace())
              traceMsg(comp(),
                       "      ivInfo is invalid for sym [%p], def [%d] already "
                       "seen in loop\n",
                       sym, defVN);
            break;
          } else {
            if (trace()) {
              traceMsg(comp(),
                       "      Sym [%p] has more than two defs, extra def due "
                       "to VN [%d]\n",
                       sym, entryVN);
        ...
        if ((defVN != entryVN) && notInLoop) {
          ...
          if (trace())
            traceMsg(comp(),
                     "      ivInfo is invalid for sym [%p], def [%d] node %p "
                     "is likely in some inner loop\n",
                     sym, defVN, defNode);
          break;
        }
      }
  ...
  if (iv) {
    ...
    //
    if (iv->_onlyIncrValid) invalidEntryInfo = false;
    ...
    //
    iv = new (trStackMemory())
        InductionVariable(sym, entryDef, incrementVN, increment, this);
    ...
    if (maybeInductionVariable) {
      if (checkLoopTestBlock(sym)) {
        ...
        traceMsg(comp(),
                 "      Could not guess the increment for sym [%p], not marked "
    ...
    //
    if (lastTimeThrough()) iv->_entryDef = NULL;
  }

  // If the induction variable is not valid, mark it
  //
  if (!isInductionVariable) iv->_entryDef = NULL;

  // if the entry is invalid, mark it so
  if (iv->_invalidEntryInfo) iv->_entryDef = NULL;

  // If the induction variable is valid, inject a constraint showing a use of
  // the induction variable has been found
  //
  if (iv->_entryDef) {
    // select the right type of constraint for induction variables to prevent
    // merging different types of constraints together
    if (node->getOpCode().isLong())
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                          TR::VPLongConst::create(this, incrementVN),
                          &_curConstraints, true);
    else if (node->getOpCode().isShort())
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                          TR::VPShortConst::create(this, incrementVN),
                          &_curConstraints, true);
    else
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                          TR::VPIntConst::create(this, incrementVN),
                          &_curConstraints, true);
  }
}
...
void OMR::ValuePropagation::checkForInductionVariableLoad(TR::Node *node) {
  ...
  // induction variable.
  //
  if (!_loopInfo || !lastTimeThrough()) return;

  if (!node->getOpCode().isLoadVar()) return;
  ...
  if (iv && iv->_entryDef) {
    ...
    if (node->getOpCode().isLong())
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                          TR::VPLongConst::create(this, useValueNumber),
                          &_curConstraints, true);
    else if (node->getOpCode().isShort())
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                          TR::VPShortConst::create(this, useValueNumber),
                          &_curConstraints, true);
    else
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint,
                          TR::VPIntConst::create(this, useValueNumber),
                          &_curConstraints, true);
  }
}
...
void OMR::ValuePropagation::collectInductionVariableEntryConstraints() {
  ...
  for (iv = _loopInfo->_inductionVariables.getFirst(); iv; iv = next) {
    ...
    //
    if (!iv->_entryDef) _loopInfo->_inductionVariables.remove(iv);
  }
}
...
bool OMR::ValuePropagation::checkLoopTestBlock(TR::Symbol *sym) {
  ...
  if (!_loopInfo) return false;
  TR_RegionStructure *region = _loopInfo->_loop;
  if (!region->isNaturalLoop()) return false;

  // check for a while-do or a do-while loop
  //
  bool isWellFormedLoop = false;

  TR_BlockStructure *entry =
      _loopInfo->_entryBlock ? _loopInfo->_entryBlock->getStructureOf() : NULL;
  TR_StructureSubGraphNode *entryNode = region->getEntry();
  if (entry) {
    for (auto edge = entryNode->getSuccessors().begin();
         edge != entryNode->getSuccessors().end(); ++edge) {
      if (region->isExitEdge(*edge)) isWellFormedLoop = true;
    }
  }
  ...
  if (!isWellFormedLoop) {
    ...
    for (TR_StructureSubGraphNode *sgNode = snIt.getCurrent();
         sgNode && (branchNode == NULL); sgNode = snIt.getNext()) {
      bool hasBackEdge = false;
      bool hasExitEdge = false;
      for (auto edge = sgNode->getSuccessors().begin();
           edge != sgNode->getSuccessors().end(); ++edge) {
        TR_StructureSubGraphNode *dest =
            toStructureSubGraphNode((*edge)->getTo());
        if (region->isExitEdge(*edge)) hasExitEdge = true;
        if (dest == region->getEntry()) {
          hasBackEdge = true;
        }
        if (hasBackEdge && hasExitEdge) isWellFormedLoop = true;
      }
    }
  }
  ...
  if (isWellFormedLoop) {
    ...
    for (TR::CFGEdge *e = cfgIt.getFirst(); e; e = cfgIt.getNext()) {
      loopTestBlock =
          toStructureSubGraphNode(e->getFrom())->getStructure()->asBlock();
      if (loopTestBlock) {
        TR::Node *branchNode =
            loopTestBlock->getBlock()->getLastRealTreeTop()->getNode();
        if (branchNode->getOpCode().isBranch() &&
          ...
          while (firstChild->getOpCode().isAdd() ||
...
void OMR::ValuePropagation::checkBackEdgeCoverage() {
  ...
  for (iv = _loopInfo->_inductionVariables.getFirst(); iv; iv = next) {
    ...
    if (iv->_entryDef) {
      Relationship *rel =
          findEdgeConstraint(iv->_valueNumber, _loopInfo->_backEdgeConstraints);
      if (rel) {
        TR_ASSERT(
            ((rel->constraint->asIntConst() &&
              rel->constraint->asIntConst()->getInt() == iv->_incrementVN) ||
        ...
        removeConstraint(iv->_valueNumber,
                         _loopInfo->_backEdgeConstraints->valueConstraints);
      } else {
        iv->_entryDef = NULL;
      }
    }
...
void OMR::ValuePropagation::setUpInductionVariables(
  ...
  while ((ivInfo = _loopInfo->_inductionVariables.pop())) {
    ...
    for (TR::CFGEdge *edge = exits.getFirst(); edge; edge = exits.getNext()) {
      if (ivInfo->_onlyIncrValid) continue;

      EdgeConstraints *constraints = getEdgeConstraints(edge);

      // Ignore unreachable exit paths
      //
      if (isUnreachablePath(constraints)) continue;

      // Merge in the constraints for all uses of the symbol on this edge.
      //
      Relationship *uses =
          findEdgeConstraint(ivInfo->_valueNumber, constraints);
      if (!uses) {
      ...
      //
      if (!ivInfo->_entryDef) continue;
      if (exitConstraintFound && exitConstraint == NULL) continue;
    ...
    if (!ivInfo->_entryDef && !ivInfo->_onlyIncrValid) continue;
    ...
    if (ivInfo->_symbol->getType().isInt32()) {
      if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
        if (ivInfo->_entryConstraint->asIntConst())
          entry = new (trHeapMemory())
              TR::VPIntConst(ivInfo->_entryConstraint->getLowInt());
        else
          entry = new (trHeapMemory())
              TR::VPIntRange(ivInfo->_entryConstraint->getLowInt(),
                             ivInfo->_entryConstraint->getHighInt());
      } else
        entry = NULL;
      if (ivInfo->_increment->asIntConst())
        incr = new (trHeapMemory())
            TR::VPIntConst(ivInfo->_increment->getLowInt());
      else
        incr = new (trHeapMemory()) TR::VPIntRange(
            ivInfo->_increment->getLowInt(), ivInfo->_increment->getHighInt());
      if (exitConstraint) {
        if (exitConstraint->asIntConst())
          exit =
              new (trHeapMemory()) TR::VPIntConst(exitConstraint->getLowInt());
        else
          exit = new (trHeapMemory()) TR::VPIntRange(
              exitConstraint->getLowInt(), exitConstraint->getHighInt());
      } else
        exit = NULL;
    } else if (ivInfo->_symbol->getType().isInt16()) {
      if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
        if (ivInfo->_entryConstraint->asShortConst())
          entry = new (trHeapMemory())
              TR::VPShortConst(ivInfo->_entryConstraint->getLowShort());
        else
          entry = new (trHeapMemory())
              TR::VPShortRange(ivInfo->_entryConstraint->getLowShort(),
                               ivInfo->_entryConstraint->getHighShort());
      } else
        entry = NULL;
      if (ivInfo->_increment->asShortConst())
        incr = new (trHeapMemory())
            TR::VPShortConst(ivInfo->_increment->getLowShort());
      else
        incr = new (trHeapMemory())
            TR::VPShortRange(ivInfo->_increment->getLowShort(),
                             ivInfo->_increment->getHighShort());
      if (exitConstraint) {
        if (exitConstraint->asShortConst())
          exit = new (trHeapMemory())
              TR::VPShortConst(exitConstraint->getLowShort());
        else
          exit = new (trHeapMemory()) TR::VPShortRange(
              exitConstraint->getLowShort(), exitConstraint->getHighShort());
      } else
        exit = NULL;
      ...
      if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
        if (ivInfo->_entryConstraint->asLongConst())
          entry = new (trHeapMemory())
              TR::VPLongConst(ivInfo->_entryConstraint->getLowLong());
        else
          entry = new (trHeapMemory())
              TR::VPLongRange(ivInfo->_entryConstraint->getLowLong(),
                              ivInfo->_entryConstraint->getHighLong());
      } else
        entry = NULL;
      if (ivInfo->_increment->asLongConst())
        incr = new (trHeapMemory())
            TR::VPLongConst(ivInfo->_increment->getLowLong());
      else
        incr = new (trHeapMemory())
            TR::VPLongRange(ivInfo->_increment->getLowLong(),
                            ivInfo->_increment->getHighLong());
      if (exitConstraint) {
        if (exitConstraint->asLongConst())
          exit = new (trHeapMemory())
              TR::VPLongConst(exitConstraint->getLowLong());
        else
          exit = new (trHeapMemory()) TR::VPLongRange(
              exitConstraint->getLowLong(), exitConstraint->getHighLong());
      } else
        exit = NULL;
    }
    ...
    TR_InductionVariable *iv = new (trHeapMemory())
        TR_InductionVariable(ivInfo->_symbol->castToRegisterMappedSymbol(),
                             entry, exit, incr, isSigned);

    if (trace()) {
      /////printf("\nFound induction variable in %s",comp()->signature());
      traceMsg(comp(), "\nFound induction variable %d [%p]",
               ivInfo->_valueNumber - _firstInductionVariableValueNumber,
               ivInfo->_symbol);
    ...
    for (cur = iter.getFirst(); cur; cur = next) {
      ...
      if (cur->getValueNumber() < _firstInductionVariableValueNumber) {
        ...
        if (cur->storeRelationships.isEmpty()) {
          _vcHandler.remove(cur->getValueNumber(),
                            constraints->valueConstraints);
...
TR::GlobalValuePropagation::GlobalValuePropagation(
...
int32_t TR::GlobalValuePropagation::perform() {
  ...
  if (!cfg) {
    dumpOptDetails(comp(),
                   "Can't do Global Value Propagation - there is no CFG\n");
    return 0;
  }
  if (!optimizer()->getUseDefInfo()) {
    dumpOptDetails(
        comp(), "Can't do Global Value Propagation - no use/def info for %s\n",
        comp()->signature());
    return 0;
  }
  ...
  if (!optimizer()->getValueNumberInfo()) {
    dumpOptDetails(
        comp(), "Can't do Global Value Propagation - no value numbers for %s\n",
        comp()->signature());
    return 0;
  }
  ...
  if ((_firstUnresolvedSymbolValueNumber - 1) <= comp()->getNodeCount()) {
    dumpOptDetails(comp(),
                   "Can't do Global Value Propagation - too many nodes\n");
    return 0;
  }

  // TODO: Use TR_CanReachGivenBlocks from TR_CanBeReachedWithoutExceptionEdges

  static char *skipBlocksThatCannotReachNonColdBlocks =
      feGetEnv("TR_skipBlocksThatCannotReachNonColdBlocks");
  if (skipBlocksThatCannotReachNonColdBlocks) {
    _blocksToProcess = new (trStackMemory()) TR_BitVector(
        comp()->getFlowGraph()->getNumberOfNodes(), comp()->trMemory(),
        stackAlloc, notGrowable, TR_MemoryBase::ValuePropagation);
    TR_CanReachNonColdBlocks(comp()).perform(_blocksToProcess);
  }

  static char *skipBlocksThatCannotReachNormalPaths =
      feGetEnv("TR_skipBlocksThatCannotReachNormalPaths");
  if (skipBlocksThatCannotReachNormalPaths) {
    _blocksToProcess = new (trStackMemory()) TR_BitVector(
        comp()->getFlowGraph()->getNumberOfNodes(), comp()->trMemory(),
        stackAlloc, notGrowable, TR_MemoryBase::ValuePropagation);
    TR_CanBeReachedWithoutExceptionEdges(comp()).perform(_blocksToProcess);
    TR_CanReachGivenBlocks(comp(), _blocksToProcess)
        .perform(_blocksToProcess);  // Bitvector is changed in-place
  }
  ...
  if (checksWereRemoved()) requestOpt(OMR::catchBlockRemoval);
  ...
  if (trace()) comp()->dumpMethodTrees("Trees after Global Value Propagation");

  // Invalidate usedef and value number information if necessary
  //
  if (_useDefInfo && useDefInfoInvalid()) optimizer()->setUseDefInfo(NULL);
...
bool TR::GlobalValuePropagation::buildInputConstraints(TR::CFGNode *node) {
  ...
  if (edge) {
    ...
    //
    _vcHandler.setRoot(_curConstraints,
                       _vcHandler.getRoot(constraints->valueConstraints));
    _vcHandler.setRoot(constraints->valueConstraints, NULL);
  ...
  //
  if (reachableInputEdgesFound || !unreachableInputEdgesFound) return true;
...
void TR::GlobalValuePropagation::propagateOutputConstraints(
    TR_StructureSubGraphNode *node, bool lastTimeThrough, bool isNaturalLoop,
  ...
  if (!outEdges2) outEdges2 = &dummyList;
  ...
  for (parentEdge = si.getFirst(); parentEdge; parentEdge = si.getNext()) {
    ...
    for (edge = exits.getFirst(); edge; edge = exits.getNext()) {
      if (edge->getTo()->getNumber() == parentTo) {
        ...
        //
        if (isUnreachablePath(constraints)) {
          if (parentEdgeIsUnreachable) parentEdgeIsUnreachable = true;
          continue;
        } else
...
void TR::GlobalValuePropagation::processStructure(
  ...
  if (comp()->getOptions()->realTimeGC() && ((++numIter) & 0xf) == 0 &&
    ...
    comp()->failCompilation<TR::CompilationInterrupted>(
        "interrupted when starting processStructure()");
  }
  TR_RegionStructure *region = node->getStructure()->asRegion();
  if (region) {
...
void TR::GlobalValuePropagation::processAcyclicRegion(
    TR_StructureSubGraphNode *node, bool lastTimeThrough, bool insideLoop) {
  if (trace()) printStructureInfo(node->getStructure(), true, lastTimeThrough);

  processRegionSubgraph(node, lastTimeThrough, insideLoop, false);

  if (trace()) printStructureInfo(node->getStructure(), false, lastTimeThrough);
}
...
void TR::GlobalValuePropagation::processNaturalLoop(
  ...
  if (!insideLoop) {
    ...
    _loopInfo->_seenDefs = new (trStackMemory())
        TR_BitVector(_numValueNumbers, trMemory(), stackAlloc);
    ...
    //
    if (trace()) printStructureInfo(region, true, false);

    _visitCount--;
    processRegionSubgraph(node, false, true, true);

    // having processed the loop the first time we want to make sure to wipe out
    // any seenOnAllPaths information on the back edges - we use this notion
    ...
    if (trace()) printStructureInfo(region, false, false);
  ...
  if (trace()) printStructureInfo(region, true, lastTimeThrough);

  // If this is the last time through the loop, collect info for induction
  // variable entry constraints.
  //
  if (lastTimeThrough) collectInductionVariableEntryConstraints();

  processRegionSubgraph(node, lastTimeThrough, true, true);

  // having processed the loop again we again clear out any seenOnAllPaths
  // information on the back edges
  for (auto itr = region->getEntry()->getPredecessors().begin(),
            end = region->getEntry()->getPredecessors().end();
       itr != end; ++itr) {
    (*_definedOnAllPaths)[*itr] = NULL;
  ...
  // the exit edge is a union of the exit edge seenOnAllPaths and the
  // intersection of the entry seen on all paths
  ...
  if (trace()) {
    traceMsg(comp(), "   defined on all paths for entry of loop %d",
             region->getNumber());
  ...
  for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
    if (trace())
      traceMsg(comp(), "   defined on all paths for exit %d->%d:",
    ...
    if ((*_definedOnAllPaths)[edge]) {
      ...
      if (trace()) traceMsg(comp(), " NULL\n");
      (*_definedOnAllPaths)[edge] = inboundDefinedOnAllPaths;
    }

    // the exit edges in the natural loop region do not connect to the parent's
    // nodes so we now find the matching exit edge in the parent region and copy
    // the defined on all paths information onto those edges
    for (auto itr = node->getSuccessors().begin(),
              end = node->getSuccessors().end();
         itr != end; ++itr) {
      if ((*itr)->getTo()->getNumber() == edge->getTo()->getNumber()) {
        ...
        if (parentEdgeDefinedOnAllPaths != NULL) {
          *parentEdgeDefinedOnAllPaths &= (*(*_definedOnAllPaths)[edge]);
          ...
          parentEdgeDefinedOnAllPaths =
              new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc);
...
void TR::GlobalValuePropagation::processImproperLoop(
  ...
  // region when doing GVP (they are apparently created later on in the
  // optimizer). Since handling improper regions correctly is complicated, we
  // simply skip them; however if we wanted to restart analyzin inside them, we
  // must use code under comment after the below loop.
  //
  ...
  */
  ...
  // we do not fold a branch that would have been folded (if blocks were
  // analyzed in proper order) resulting in analysis of unreachable code which
  // might cause problems when intersecting constraints, for example.
  //
  // freeValueConstraints(_curConstraints);
  ...
  //
  propagateOutputConstraints(node, lastTimeThrough, false,
                             region->getExitEdges(), NULL);
...
void OMR::ValuePropagation::generalizeStores(ValueConstraints &stores,
                                             ValueConstraints *vC) {
  ...
  for (vc = iter.getFirst(); vc; vc = iter.getNext()) {
    if (vc->storeRelationships.isEmpty()) continue;
...
void OMR::ValuePropagation::findStoresInBlock(TR::Block *block,
                                              ValueConstraints &stores) {
  ...
  for (treeTop = block->getEntry(); treeTop && treeTop != block->getExit();
    ...
    if (node->getOpCode().isStore()) {
      ValueConstraint *vc =
          _vcHandler.findOrCreate(getValueNumber(node), stores);
      StoreRelationship *rel, *prev;
      for (rel = vc->storeRelationships.getFirst(), prev = NULL;
           rel && rel->symbol < node->getSymbol();
...
void TR::GlobalValuePropagation::getImproperRegionStores(
    TR_StructureSubGraphNode *node, ValueConstraints &stores) {
  TR_RegionStructure *region = node->getStructure()->asRegion();
  if (region) {
...
void TR::GlobalValuePropagation::processRegionSubgraph(
  ...
  for (subNode = si.getCurrent(); subNode; subNode = si.getNext()) {
    if (subNode != entry && subNode->getVisitCount() != _visitCount) {
      ...
      if (_reachedMaxRelationDepth) return;
    }
  }
  ...
  //
  if (isNaturalLoop) collectBackEdgeConstraints();
  propagateOutputConstraints(node, lastTimeThrough, isNaturalLoop,
                             region->getExitEdges(), NULL);
}
  ...
  if (!nodeIsReachable) {
    ...
    if (lastTimeThrough) {
      if (node->getStructure()->asBlock()) {
        ...
        _blocksToBeRemoved->add(
            node->getStructure()->asRegion()->getEntryBlock());
      }
    }
...
TR_BitVector *TR::GlobalValuePropagation::mergeDefinedOnAllPaths(
    TR_StructureSubGraphNode *node) {
  TR_BitVector *mergeResult =
      new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc);

  if (!node->getExceptionPredecessors().empty()) return mergeResult;

  bool first = true;
  for (auto itr = node->getPredecessors().begin(),
            end = node->getPredecessors().end();
       itr != end; ++itr) {
    TR_BitVector *predDefinedOnAllPaths = (*_definedOnAllPaths)[*itr];
    if (trace()) {
      traceMsg(comp(), "   inbound seenOnAllpaths for edge %d->%d",
...
void TR::GlobalValuePropagation::processBlock(TR_StructureSubGraphNode *node,
  ...
  // inside of a loop we track the defined on all paths bitvector so we know
  // when a merge of def constraints may need to consider the backedge - here we
  // compute the block's inbound seen on all paths set by intersecting the sets
  // on the inbound edges
  if (insideLoop) {
    _curDefinedOnAllPaths = mergeDefinedOnAllPaths(node);
  }

  if (insideLoop && trace()) {
    traceMsg(comp(), "   defined on all paths for entry of block %d",
             block->getNumber());
  ...
  if (comp()->getStartBlock()->getNumber() == _curBlock->getNumber()) {
    addConstraintToList(NULL, _syncValueNumber, AbsoluteConstraint,
                        TR::VPSync::create(this, TR_no), &_curConstraints);
  }
  ...
  // inside a loop we will now propagate the seen on all paths definitions
  // computed through the block onto the exit edges
  if (insideLoop) {
    for (auto itr = node->getSuccessors().begin(),
              end = node->getSuccessors().end();
         itr != end; ++itr) {
      if (trace()) {
        traceMsg(comp(), "   outbound seenOnAllpaths for edge %d->%d",
                 (*itr)->getFrom()->getNumber(), (*itr)->getTo()->getNumber());
        _curDefinedOnAllPaths->print(comp());
        traceMsg(comp(), "\n");
      }
  ...
  if (!isUnreachablePath(_curConstraints)) {
    ...
    if (trace())
      traceMsg(comp(),
               "Calling findOutEdge using _curBlock %d and nextBlock %d\n",
  ...
  if (hasIncomingStoreRelationships) {
    TR::Block *nextBlock =
        endTree->getNextTreeTop()
            ? endTree->getNextTreeTop()->getNode()->getBlock()
            : 0;
    for (auto edge = _curBlock->getSuccessors().begin();
         edge != _curBlock->getSuccessors().end(); ++edge) {
      if (getEdgeConstraints(*edge)->valueConstraints.isEmpty()) {
        TR_ASSERT((*edge)->getTo()->getNumber() == 1,
                  "Empty set of constraints on edge to block %d\n",
  ...
  // Create temporary List structures to pass to this function. Will be
  // eliminated once getExitEdges has migrated to STL
  List<TR::CFGEdge> List1(comp()->trMemory());
  List<TR::CFGEdge> List2(comp()->trMemory());
  for (auto iter = _curBlock->getSuccessors().begin();
       iter != _curBlock->getSuccessors().end(); ++iter)
    List1.add(*iter);
  for (auto iter = _curBlock->getExceptionSuccessors().begin();
       iter != _curBlock->getExceptionSuccessors().end(); ++iter)
    List2.add(*iter);
  propagateOutputConstraints(node, lastTimeThrough, false, List1, &List2);
}

const char *TR::GlobalValuePropagation::optDetailString() const throw() {
  return "O^O GLOBAL VALUE PROPAGATION: ";
}

TR::CFGEdge *OMR::ValuePropagation::findOutEdge(TR::CFGEdgeList &edges,
                                                TR::CFGNode *target) {
...
void OMR::ValuePropagation::setUnreachablePath(ValueConstraints &vc) {
  ...
  addConstraintToList(NULL, 0, AbsoluteConstraint,
                      TR::VPUnreachablePath::create(this), &vc);
}
...
void OMR::ValuePropagation::printStructureInfo(TR_Structure *s, bool starting,
                                               bool lastTimeThrough) {
...
void OMR::ValuePropagation::printValueConstraints(
    ValueConstraints &valueConstraints) {
...
void OMR::ValuePropagation::printGlobalConstraints() {
  ...
  for (int32_t i = 0; i <= _globalConstraintsHTMaxBucketIndex; i++) {
    ...
    for (entry = _globalConstraintsHashTable[i]; entry; entry = entry->next) {
      for (Relationship *rel = entry->constraints.getFirst(); rel;
           rel = rel->getNext()) {
...
void OMR::ValuePropagation::printEdgeConstraints(EdgeConstraints *constraints) {
  if (!_isGlobalPropagation || comp()->getOutFile() == NULL) return;
...
void OMR::ValuePropagation::Relationship::print(OMR::ValuePropagation *vp) {
  if (vp->comp()->getOutFile() == NULL) return;
  if (relative == AbsoluteConstraint) {
...
void OMR::ValuePropagation::Relationship::print(OMR::ValuePropagation *vp,
  ...
  if (valueNumber < vp->_firstUnresolvedSymbolValueNumber) {
    trfprintf(vp->comp()->getOutFile(), "%*.svalue %d is ", indent, " ",
              valueNumber);
    print(vp);
    ...
    for (iv = vp->_loopInfo->_inductionVariables.getFirst(); iv;
         iv = iv->getNext()) {
      if (iv->_valueNumber == valueNumber) break;
    }
    if (iv)
      trfprintf(vp->comp()->getOutFile(), "%*.sinduction variable %d [%p]",
                indent, " ",
                valueNumber - vp->_firstInductionVariableValueNumber,
                iv->_symbol);
    else
      trfprintf(vp->comp()->getOutFile(), "%*.sparent induction variable %d",
                indent, " ",
                valueNumber - vp->_firstInductionVariableValueNumber);
...
void OMR::ValuePropagation::StoreRelationship::print(OMR::ValuePropagation *vp,
                                                     int32_t valueNumber,
                                                     int32_t indent) {
  if (vp->comp()->getOutFile() == NULL) return;
  ...
  if (relationships.getFirst())
    for (Relationship *rel = relationships.getFirst(); rel;
         rel = rel->getNext()) {
      trfprintf(vp->comp()->getOutFile(), "%*.ssymbol %p store", indent, " ",
                symbol);
      rel->print(vp, valueNumber, 1);
    }
  else
    trfprintf(vp->comp()->getOutFile(),
              "%*.sptr %p symbol %p has no relationships\n", indent, " ", this,
              symbol);
}

void OMR::ValuePropagation::ValueConstraint::print(OMR::ValuePropagation *vp,
                                                   int32_t indent) {
  if (vp->comp()->getOutFile() == NULL) return;
  for (Relationship *rel = relationships.getFirst(); rel; rel = rel->getNext())
    rel->print(vp, getValueNumber(), indent);
  for (StoreRelationship *storeRel = storeRelationships.getFirst(); storeRel;
       storeRel = storeRel->getNext())
...
bool OMR::ValuePropagation::removeConstraints(int32_t valueNumber,
  ...
  if (trace()) {
    ...
    traceMsg(comp(),
             "   Intersection of constraints failed for valueNumber [%d], "
             "removing constraints\n",
             valueNumber);
  }

  if (!vc) vc = &_curConstraints;

  ValueConstraints stores;
  generalizeStores(stores, vc);

  if (findStores) findStoresInBlock(_curBlock, stores);

#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
  if (_isGlobalPropagation) removeConstraints(valueNumber);
  if (trace()) traceMsg(comp(), "   Setting fall-through as unreachable\n");
...
//
bool OMR::ValuePropagation::removeConstraints(
    int32_t valueNumber, ValueConstraints *valueConstraints) {
  if (trace())
    traceMsg(comp(),
             "   Intersection of constraints failed for valueNumber [%d], "
             "removing constraints\n",
  ...
  //
  if (!valueConstraints) return removeConstraints(valueNumber);
  ...
  if (vc) {
    ...
    for (curStore = vc->storeRelationships.getFirst(); curStore;
         curStore = nextStore) {
      nextStore = curStore->getNext();
      Relationship *rel;
      for (rel = curStore->relationships.getFirst(); rel;
           rel = rel->getNext()) {
        if (rel->relative != AbsoluteConstraint)
...
bool OMR::ValuePropagation::removeConstraints(int32_t valueNumber) {
  if (trace())
    traceMsg(comp(),
             "   Intersection failed for value number [%d], removing global "
             "constraints\n",
             valueNumber);
...
TR::TreeTop *TR::ArraycopyTransformation::createArrayNode(
    TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcRef,
  ...
  return createArrayNode(tree, newTree, srcRef, dstRef, len, srcObjRef,
                         dstObjRef, isForward);
}

TR::TreeTop *TR::ArraycopyTransformation::createArrayNode(
    TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcRef,
    TR::SymbolReference *dstRef, TR::Node *len, TR::SymbolReference *srcObjRef,
  ...
  if (!root->isReferenceArrayCopy()) {
    ...
    if (trace())
      traceMsg(comp(), "Created 3-child arraycopy %s from root node %s, ",
    ...
    if (trace())
      traceMsg(comp(), "Created 5-child arraycopy %s from root node %s, ",
               comp()->getDebug()->getName(node),
               comp()->getDebug()->getName(root));
  }
...
int64_t TR::ArraycopyTransformation::arraycopyHighFrequencySpecificLength(
    ...
    if (TR::Compiler->target.is64Bit()) {
      TR_LongValueInfo *valueInfo = static_cast<TR_LongValueInfo *>(
          TR_ValueProfileInfoManager::getProfiledValueInfo(
              arrayCopyNode, comp(), LongValueInfo));
      if (valueInfo && valueInfo->getTopProbability() >
                           MIN_ARRAYCOPY_FREQ_FOR_SPECIALIZATION) {
        return ((int64_t)valueInfo->getTopValue());
      }
    } else {
      TR_ValueInfo *valueInfo = static_cast<TR_ValueInfo *>(
          TR_ValueProfileInfoManager::getProfiledValueInfo(arrayCopyNode,
                                                           comp(), ValueInfo));
      if (valueInfo && valueInfo->getTopProbability() >
                           MIN_ARRAYCOPY_FREQ_FOR_SPECIALIZATION) {
        return ((int64_t)valueInfo->getTopValue());
...
TR::TreeTop *TR::ArraycopyTransformation::createMultipleArrayNodes(
    TR::TreeTop *arrayTree, TR::Node *node) {
  if (/* node->isReferenceArrayCopy() || */ node
          ->isRarePathForwardArrayCopy() ||
      node->isBackwardArrayCopy()) {
    return arrayTree;
  }
  ...
  // if this is already a forward arraycopy and there is no specific length
  // known, no further processing required.
  if (alreadyForwardArrayCopy &&
      (specificLength < 0 || len->getOpCode().isLoadConst())) {
    return arrayTree;
  }
  ...
  TR::TreeTop *firstInsertedTree = createStoresForArraycopyChildren(
      comp(), arrayTree, srcObjRef, dstObjRef, srcRef, dstRef, lenRef);
  ...
  if (!alreadyForwardArrayCopy) {
    ...
    TR::TreeTop *outerElseTree =
        createRangeCompareNode(node, srcRef, dstRef, lenRef);

    createArrayNode(arrayTree, arraycopyForward, srcRef, dstRef, lenRef,
                    srcObjRef, dstObjRef, true);
    createArrayNode(arrayTree, arraycopyBackward, srcRef, dstRef, lenRef,
                    srcObjRef, dstObjRef, false);

    followOnBlock = arraycopyBlock->createConditionalBlocksBeforeTree(
        arrayTree, compareTree, arraycopyBackward, outerElseTree, cfg);

    TR::Block *forwardArrayCopyBlock = TR::Block::createEmptyBlock(
        node, comp(), arraycopyBlock->getFrequency(), arraycopyBlock);
    ...
    cfg->addEdge(TR::CFGEdge::createEdge(outerElseBlock, forwardArrayCopyBlock,
                                         trMemory()));
    cfg->addEdge(TR::CFGEdge::createEdge(arraycopyBlock, forwardArrayCopyBlock,
                                         trMemory()));
    cfg->addEdge(TR::CFGEdge::createEdge(forwardArrayCopyBlock, followOnBlock,
                                         trMemory()));
    cfg->copyExceptionSuccessors(ifBlock, forwardArrayCopyBlock);

    cfg->removeEdge(outerElseBlock->getSuccessors(),
                    outerElseBlock->getNumber(), followOnBlock->getNumber());
    cfg->removeEdge(arraycopyBlock->getSuccessors(),
                    arraycopyBlock->getNumber(), ifBlock->getNumber());

    outerArraycopyTree = arraycopyForward;
  } else {
    outerArraycopyTree =
        arrayTree;  // if already forward, just use the original arraycopy
  }

  if (trace()) {
    comp()->dumpMethodTrees(
        "Trees after forward/backward arraycopy transformation");
    cfg->comp()->getDebug()->print(cfg->comp()->getOutFile(), cfg);
  }

  arraycopyBlock = outerArraycopyTree->getEnclosingBlock();
  if (specificLength >= 0)
    specializeForLength(outerArraycopyTree, node, (uintptrj_t)specificLength,
                        srcRef, dstRef, lenRef, srcObjRef, dstObjRef);
...
TR::TreeTop *TR::ArraycopyTransformation::tryToSpecializeForLength(
    TR::TreeTop *tt, TR::Node *arraycopyNode) {
  TR::Node *lengthChild = arraycopyNode->getChild(
      arraycopyNode->getNumChildren() - 1);  // last child is length
  if (arraycopyNode->isRarePathForwardArrayCopy() ||
      lengthChild->getOpCode().isLoadConst())
    return tt;  // already specialized

  if (!arraycopyNode->isForwardArrayCopy())
    return tt;  // can only specialize forward arraycopies because the rare path
                // flag is "isRarePathForwardArrayCopy"

  int64_t specificLength = arraycopyHighFrequencySpecificLength(
      arraycopyNode);  // TODO: What about 64-bit lengths on 64-bit platforms?
  if (specificLength >= 0 &&
      performTransformation(
          comp(), "%sSpecializing arraycopy %s for length of %d bytes\n",
          OPT_DETAILS, comp()->getDebug()->getName(arraycopyNode),
          specificLength)) {
    TR::SymbolReference *srcObjRef = NULL;
    TR::SymbolReference *dstObjRef = NULL;
    TR::SymbolReference *srcRef = NULL;
    ...
    TR::SymbolReference *lenRef = NULL;
    TR::TreeTop *firstInsertedTree = createStoresForArraycopyChildren(
        comp(), tt, srcObjRef, dstObjRef, srcRef, dstRef, lenRef);
    specializeForLength(tt, arraycopyNode, (uintptrj_t)specificLength, srcRef,
                        dstRef, lenRef, srcObjRef, dstObjRef);
    return firstInsertedTree;
  } else {
    return tt;
  }
...
static TR::Node *addressSizedConst(TR::Compilation *comp, TR::Node *n,
                                   intptrj_t val) {
  TR::Node *node = TR::Compiler->target.is64Bit() ? TR::Node::lconst(n, val)
                                                  : TR::Node::iconst(n, val);
  if (node->getOpCodeValue() == TR::lconst) node->setLongInt(val);
  return node;
}

TR::TreeTop *TR::ArraycopyTransformation::specializeForLength(
    TR::TreeTop *tt, TR::Node *arraycopyNode, uintptrj_t lengthInBytes,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
    TR::SymbolReference *lenRef, TR::SymbolReference *srcObjRef,
  ...
  createArrayNode(tt, arraycopyOriginal, srcRef, dstRef, lenRef, srcObjRef,
                  dstObjRef, true);
  arraycopyOriginal->getNode()->getFirstChild()->setRarePathForwardArrayCopy(
      true);

  TR::TreeTop *specializedTree =
      createArrayNode(tt, arraycopySpecific, srcRef, dstRef,
                      addressSizedConst(comp(), arraycopyNode, lengthInBytes),
                      srcObjRef, dstObjRef, true);
  if (trace())
    dumpOptDetails(
        comp(), "%s Specialized arraycopy is %s\n", OPT_DETAILS,
        getDebug()->getName(specializedTree->getNode()->getFirstChild()));

  TR::Node *lengthNode = TR::Node::createLoad(arraycopyNode, lenRef);

  TR::TreeTop *specificIfCheck = TR::TreeTop::create(
      comp(),
      TR::Node::createif(
          lengthNode->getType().isInt32() ? TR::ificmpne : TR::iflcmpne,
          lengthNode, addressSizedConst(comp(), arraycopyNode, lengthInBytes)));

  TR::Block *followOnBlock =
      tt->getEnclosingBlock()->createConditionalBlocksBeforeTree(
          tt, specificIfCheck, arraycopyOriginal, arraycopySpecific,
          comp()->getFlowGraph());

  specificIfCheck->getNode()->setBranchDestination(
      arraycopyOriginal->getEnclosingBlock()->getEntry());

  // Scale the frequency of the specialized path
  TR::Block *specificPathBlock = arraycopySpecific->getEnclosingBlock();
  int32_t specializedBlockFrequency = TR::Block::getScaledSpecializedFrequency(
      specificPathBlock->getFrequency());
  ...
  arraycopyOriginal->getEnclosingBlock()->setFrequency(
      specializedBlockFrequency);
  arraycopyOriginal->getEnclosingBlock()->setIsCold(false);
  requestOpt(OMR::basicBlockOrdering);  // Block frequency is reset, so run
                                        // order blocks optimization to reorder
                                        // this path appropriately.

  return specificIfCheck;  // First inserted tree
}

TR::ArraycopyTransformation::ArraycopyTransformation(
    TR::OptimizationManager *manager)
    : TR::Optimization(manager), _changedTrees(false) {}

int32_t TR::ArraycopyTransformation::perform() {
  // this is not an optional transformation - it must be run if arraycopy
  // transformation is done since the code generator can not handle the direct
  // output from ValuePropagation arraycopy creation. it is intended for
  // platforms that want to generate inline code for both forward and backward
  // arraycopy but don't want to do control flow in the platform code
  bool mustSpecializeForDirection =
      comp()->cg()->getSupportsPostProcessArrayCopy();

  TR::CFG *cfg = comp()->getFlowGraph();
  TR::TreeTop *lastTreeTop = cfg->findLastTreeTop();
  TR::TreeTop *firstTreeTop = comp()->getMethodSymbol()->getFirstTreeTop();
  for (TR::TreeTop *tt = lastTreeTop; tt != firstTreeTop;
  ...
  if (!hasChangedTrees() ||
      !performTransformation(
          comp(),
          "%s Arraycopy Transformation for primitive and reference arrays\n",
          OPT_DETAILS))
    return 0;
...
const char *TR::ArraycopyTransformation::optDetailString() const throw() {
  return "O^O ARRAY COPY TRANSFORMATION: ";
}

void OMR::ValuePropagation::createNewBlockInfoForVersioning(TR::Block *block) {
  if (!block->isCatchBlock() && !_bndChecks->isEmpty() &&
      !_bndChecks->isSingleton()) {
    TR_LinkHead<ArrayLengthToVersion> arrayLengthsLocal;

    if (prepareForBlockVersion(&arrayLengthsLocal)) {
      TR_LinkHead<ArrayLengthToVersion> *arrayLengths =
          new (trStackMemory()) TR_LinkHead<ArrayLengthToVersion>();
...
void OMR::ValuePropagation::versionBlocks() {
  ...
  for (cur = _blocksToBeVersioned->getFirst(); cur; cur = cur->getNext()) {
    ...
    for (TR::CFGNode *node = _cfg->getFirstNode(); node;
         node = node->getNext()) {
    ...
    if (!blockIsInCFG) continue;

    // check if all bndchecks are in the block

    TR_ScratchList<TR::Node> temp(trMemory());
    for (ArrayLengthToVersion *arrayLength = cur->_arrayLengths->getFirst();
         arrayLength; arrayLength = arrayLength->getNext()) {
      for (ArrayIndexInfo *arrayIndex =
               arrayLength->_arrayIndicesInfo->getFirst();
           arrayIndex; arrayIndex = arrayIndex->getNext()) {
    ...
    for (TR::TreeTop *tt = block->getFirstRealTreeTop(); tt;
      ...
      if (node->getOpCodeValue() == TR::BNDCHK) {
        if (temp.find(node)) temp.remove(node);
      }
    }
    ...
    if (comparisonNodes.isEmpty()) continue;
    ...
    while (1) {
      ...
      if (!node->getBlock()->isExtensionOfPreviousBlock()) break;
    ...
    while (1) {
      ...
      for (fromTT = fromTT->getNextTreeTop(); fromTT != tempBlock->getExit();
           fromTT = fromTT->getNextTreeTop()) {
        if (fromTT->getNode()->getOpCode().isBranch())
          fromTT->getNode()->setBranchDestination(
              cloner
                  .getToBlock(fromTT->getNode()
    ...
    for (auto edge = lastBlock->getSuccessors().begin();
         edge != lastBlock->getSuccessors().end(); ++edge) {
      ...
      if (!(lastOpCode.isBranch() || lastOpCode.isJumpWithMultipleTargets() ||
            lastOpCode.isReturn() ||
        ...
        if (!((lastNode->getBranchDestination() == succ->getEntry()) ||
              (succ->getEntry() &&
               (lastNode->getBranchDestination() ==
                cloner.getToBlock(succ->getEntry()->getNode()->getBlock())
                    ->getEntry()))))
          fallsThrough = true;
      }
      ...
      if (!fallsThrough) {
            ...
            (!lastOpCode.isBranch() ||
             (lastNode->getBranchDestination() !=
              cloner.getToBlock(succ->getEntry()->getNode()->getBlock())
                  ->getEntry())))
          _cfg->addEdge(
              TR::CFGEdge::createEdge(lastClonedBlock, succ, trMemory()));
        else
          _cfg->addEdge(TR::CFGEdge::createEdge(
              lastClonedBlock, cloner.getToBlock(succ), trMemory()));
      } else {
        TR::Block *newGotoBlock = TR::Block::createEmptyBlock(
            lastNode, comp(), succ->getFrequency(), succ);
        if (trace())
        ...
        _cfg->addEdge(
            TR::CFGEdge::createEdge(lastClonedBlock, newGotoBlock, trMemory()));
    ...
    //
    for (auto edge = lastBlock->getExceptionSuccessors().begin();
         edge != lastBlock->getExceptionSuccessors().end(); ++edge) {
      TR::Block *succ = toBlock((*edge)->getTo());
      _cfg->addEdge(
          TR::CFGEdge::createExceptionEdge(lastClonedBlock, succ, trMemory()));
    }
    ...
    while (comparisonNode) {
      ...
      TR::TreeTop *comparisonTree =
          TR::TreeTop::create(comp(), comparisonNode->getData(), NULL, NULL);
      TR::Block *comparisonBlock = TR::Block::createEmptyBlock(
          insertionPoint->getNode(), comp(), block->getFrequency(), block);
        ...
        for (int32_t i = 0; i < actualComparisonNode->getNumChildren(); i++) {
          ...
          if (objectRef->getOpCode().isLoadIndirect() &&
            ...
            shouldBeCompressed = true;
                     ...
                     TR::TransformUtil::fieldShouldBeCompressed(
                         objectRef->getFirstChild(), comp())) {
            objectRef = objectRef->getFirstChild();
            shouldBeCompressed = true;
          }
          if (shouldBeCompressed) {
            TR::TreeTop *translateTT = TR::TreeTop::create(
                comp(), TR::Node::createCompressedRefsAnchor(objectRef), NULL,
                NULL);
    ...
    while (!block->getPredecessors().empty()) {
      ...
      if (nextPredBlock != _cfg->getStart()) {
        ...
        if (lastTreeInPred)
          lastTreeInPred->adjustBranchOrSwitchTreeTop(comp(), block->getEntry(),
    ...
    //
    ListElement<TR::Block> *currComparisonBlock =
        comparisonBlocks.getListHead();
    while (currComparisonBlock) {
      ...
      if (nextComparisonBlock) {
        ...
        _cfg->addEdge(
            TR::CFGEdge::createEdge(currentBlock, clonedBlock, trMemory()));
      } else {
...
// walk the bndchks collected for this block and add to appropriate bucket based
// on arrayLenght. returns True if to version block and false if not.
bool OMR::ValuePropagation::prepareForBlockVersion(
    TR_LinkHead<ArrayLengthToVersion> *arrayLengths) {
  bool isGlobal;
  TR_BitVector invariantVariables(comp()->getSymRefCount(), trMemory(),
                                  stackAlloc);
  ...
  for (TR::Node *bndchkNode = iter.getFirst(); bndchkNode;
       bndchkNode = iter.getNext()) {
    ...
    if (!(arrayLen->getOpCode().isLoadConst() ||
          (arrayLen->getOpCode().isArrayLength() &&
           (((arrayLen->getFirstChild()->getOpCode().getOpCodeValue() ==
              TR::aloadi) &&
             ((arrayLen->getFirstChild()
                   ->getFirstChild()
                   ->getOpCode()
                   .getOpCodeValue() == TR::aload) &&
              (arrayLen->getFirstChild()
                   ->getFirstChild()
                   ->getSymbolReference()
                   ->getSymbol()
    ...
    if (!arrayLen->getOpCode().isLoadDirect()) {
      ...
      if (arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload)
        ...
        symbolRef = arrayLen->getFirstChild()
                            ->getFirstChild()
                            ->getOpCode()
                            .hasSymbolReference()
                        ? arrayLen->getFirstChild()
      ...
      if (!symbolRef ||
          !invariantVariables.isSet(symbolRef->getReferenceNumber()))
        continue;

      if (arrayLen->getFirstChild()->hasUnresolvedSymbolReference()) continue;
      if ((arrayLen->getFirstChild()->getOpCode().getOpCodeValue() ==
           TR::aloadi) &&
          (arrayLen->getFirstChild()
               ->getFirstChild()
      ...
      if (arrayLen->getFirstChild()->getOpCode().getOpCodeValue() ==
        ...
        int32_t len;
        char *sig =
            symRefField->getOwningMethod(comp())->classNameOfFieldOrStatic(
                symRefField->getCPIndex(), len);
        ...
        if (sig)
          classOfField = fe()->getClassFromSignature(
              sig, len, symRefField->getOwningMethod(comp()));

        if (!classOfField) continue;

        instanceOfClass = classOfField;
      }
    }

    for (ArrayLengthToVersion *arrayFound = arrayLengths->getFirst();
         arrayFound; arrayFound = arrayFound->getNext()) {
    ...
    if (bndchkNode->getSecondChild()->getOpCode().isLoadConst()) {
      ...
      // not a const
      static const char *disableExtendedBoundCheckRemoval =
          feGetEnv("TR_DisableExtendedBoundCheckRemoval");
      ...
      if (index) {
        // check if invariant
        TR::SymbolReference *symRef = index->getOpCode().hasSymbolReference()
                                          ? index->getSymbolReference()
                                          : 0;
        if (symRef) {
          if (invariantVariables.isSet(symRef->getReferenceNumber()))
            ...
            for (FirstLoadOfNonInvariant *firstLoad = _firstLoads->getFirst();
                 firstLoad && !baseNode; firstLoad = firstLoad->getNext()) {
              if (firstLoad->_node != NULL &&
                  (firstLoad->_symRefNum == indexSymRefNum)) {
          ...
          if (baseNode)  // invarinat or trackableNonInvariant
          {
            ...
            if (bndchkNode->getSecondChild() == baseNode)
              ...
              if (getConstraint(bndchkNode->getSecondChild(), isGlobal,
                                baseNode))
                c = getConstraint(bndchkNode->getSecondChild(), isGlobal,
                                  baseNode)
                        ->asEqual()
                        ->increment();
              else
    ...
    if (!array) {
      createNewBucketForArrayIndex(array, arrayLengths, c, baseNode, bndchkNode,
                                   instanceOfClass);
      continue;
    }
    ...
    for (ArrayIndexInfo *arrayIndexInfo = array->_arrayIndicesInfo->getFirst();
         arrayIndexInfo; arrayIndexInfo = arrayIndexInfo->getNext()) {
      if (arrayIndexInfo->_notToVersionBucket) continue;
      ...
      if (baseNode) {
        TR::VPConstraint *rel = getConstraint(
            bndchkNode->getSecondChild(), isGlobal, arrayIndexInfo->_baseNode);
        if (!(rel && rel->asEqual()) &&
            (bndchkNode->getSecondChild() != arrayIndexInfo->_baseNode))
          continue;
      }
    ...
    if (arrayIndexInfoFound) {
      ...
      if (baseNode) {
        if (arrayIndexInfoFound->_max == arrayIndexInfoFound->_min) {
          ...
          // The following tests, using c and max (distance from 0) guarantees
          // delta positive.
        ...
        // if delta is larger than maxInt, one of the accesses will throw an
        // exception. So just don't version this bucket. we need to check that
        // max-c>0 to make sure that delta is positive (less than maxInt)
        if ((c < arrayIndexInfoFound->_min) &&
            (arrayIndexInfoFound->_max - c > 0)) {
          if (c >= 0) {
            updateMin = true;
            arrayIndexInfoFound->_delta += (arrayIndexInfoFound->_min - c);
          }
        } else if ((c > arrayIndexInfoFound->_max) &&
                   (c - arrayIndexInfoFound->_min > 0)) {
          updateMax = true;
          arrayIndexInfoFound->_delta += (c - arrayIndexInfoFound->_max);
        } else if ((arrayIndexInfoFound->_min < c &&
                    c > arrayIndexInfoFound->_max) ||
                   (c == arrayIndexInfoFound->_min ||
                    c == arrayIndexInfoFound->_max))
          removeBndchkFromFastBlock = true;
      }
  ...
  for (ArrayLengthToVersion *array = arrayLengths->getFirst(); array;
       array = array->getNext()) {
    for (ArrayIndexInfo *arrayIndex = array->_arrayIndicesInfo->getFirst();
         arrayIndex; arrayIndex = arrayIndex->getNext()) {
      if (arrayIndex->_versionBucket) return true;
    }
  }
...
void OMR::ValuePropagation::buildBoundCheckComparisonNodes(
    BlockVersionInfo *blockInfo, List<TR::Node> *comparisonNodes) {
  // walk arrayLength and for each one walk all buckets - create tests for min
  // and max.
  TR::Node *nextComparisonNode;
  for (ArrayLengthToVersion *arrayLength = blockInfo->_arrayLengths->getFirst();
       arrayLength; arrayLength = arrayLength->getNext()) {
    ...
    for (ArrayIndexInfo *arrayIndex =
             arrayLength->_arrayIndicesInfo->getFirst();
         arrayIndex; arrayIndex = arrayIndex->getNext()) {
      if (!arrayIndex->_versionBucket) continue;

      if (performTransformation(comp(),
                                "%s Creating tests outside block_%d for "
                                "versioning arraylenth %p \n",
        ...
        if (arrayIndex->_baseNode) {
          maxIndex = TR::Node::create(
              TR::iadd, 2, arrayIndex->_baseNode->duplicateTree(),
              TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0,
                               arrayIndex->_max));
          minIndex = TR::Node::create(
              TR::iadd, 2, arrayIndex->_baseNode->duplicateTree(),
              TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0,
                               arrayIndex->_min));
        } else {
          maxIndex = TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0,
                                      arrayIndex->_max);
          minIndex = TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0,
                                      arrayIndex->_min);
        }

        if (arrayIndex->_baseNode) {
          nextComparisonNode = TR::Node::createif(
              TR::ificmplt, minIndex,
        ...
        nextComparisonNode = TR::Node::createif(
            TR::ifiucmpge, maxIndex, arrayLength->_arrayLen->duplicateTree());
    ...
    if (arrayLengthVersioned) {
      if (arrayLength->_arrayLen->getOpCode().isArrayLength()) {
        ...
        if (objectRefChild && !objectRefChild->isNonNull()) {
          dumpOptDetails(comp(),
                         "%s Creating test for nullCheck of %p outside "
                         "block_%d for versioning arraylenth %p \n",
                         OPT_DETAILS, objectRefChild,
                         blockInfo->_block->getNumber(),
                         arrayLength->_arrayLen);
          TR::Node *nullTestNode =
              TR::Node::createif(TR::ifacmpeq, objectRefChild->duplicateTree(),
        ...
        if (arrayLength->_instanceOfClass && objectRefChild &&
          ...
          dumpOptDetails(comp(),
                         "%s Creating test for instanceof of %p outside "
                         "block_%d for versioning arraylenth %p \n",
                         OPT_DETAILS, objectRefChild,
                         blockInfo->_block->getNumber(),
                         arrayLength->_arrayLen);
          TR::Node *duplicateClassPtr = TR::Node::createWithSymRef(
              objectRefChild, TR::loadaddr, 0,
              comp()->getSymRefTab()->findOrCreateClassSymbol(
                  objectRef->getSymbolReference()->getOwningMethodSymbol(
                      comp()),
                  -1, arrayLength->_instanceOfClass, false));
          TR::Node *instanceofNode = TR::Node::createWithSymRef(
              TR:: instanceof
              , 2, 2, objectRefChild->duplicateTree(), duplicateClassPtr,
              comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(
                  comp()->getMethodSymbol()));
          TR::Node *ificmpeqNode =
              TR::Node::createif(TR::ificmpeq, instanceofNode,
        ...
        if (!objectRef->isNonNull()) {
          dumpOptDetails(comp(),
                         "%s Creating test for nullCheck of %p outside "
                         "block_%d for versioning arraylenth %p \n",
                         OPT_DETAILS, objectRef, blockInfo->_block->getNumber(),
                         arrayLength->_arrayLen);
          TR::Node *nullTestNode =
              TR::Node::createif(TR::ifacmpeq, objectRef->duplicateTree(),
                                 TR::Node::aconst(objectRef, 0));
          comparisonNodes->add(nullTestNode);
        }
      }
...
void OMR::ValuePropagation::removeBndChecksFromFastVersion(
    BlockVersionInfo *blockInfo) {
  for (ArrayLengthToVersion *array = blockInfo->_arrayLengths->getFirst();
       array; array = array->getNext()) {
    for (ArrayIndexInfo *index = array->_arrayIndicesInfo->getFirst(); index;
         index = index->getNext()) {
      if (!index->_bndChecks->isEmpty() && !index->_bndChecks->isSingleton()) {
        ListIterator<TR::Node> iter(index->_bndChecks);
        for (TR::Node *bndchk = iter.getFirst(); bndchk;
             bndchk = iter.getNext()) {
      ...
      ((node->getOpCode().getOpCodeValue() == TR::iload) ||
       ((node->getOpCode().getOpCodeValue() == TR::iloadi) &&
        ((node->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload) &&
         ...
         !(_seenDefinedSymbolReferences->isSet(node->getFirstChild()
                                                   ->getSymbolReference()
                                                   ->getReferenceNumber()))))))
    return node;
  else {
...
// deals with constants of the form (i+5), (i+2-3), but now i doesn't have to be
// an invariant; i has to be any subtree, but its children are walked through,
// and if they have a symbol reference they have to be either a param or an auto
// (we are trying to avoid object fields) and of course they have to be not
// defined before then; see design 1839
    ...
    TR::Node *node)  // ArrayIndexNodeFromOffset(TR::Node *node, int32_t stride)
{
  ...
  if (opCode.hasSymbolReference()) {
    ...
    if (!symReference->getSymbol()->isAutoOrParm() ||
      ...
      if (node->getOpCode().hasSymbolReference() &&
          ...
          ((node->getOpCode().getOpCodeValue() == TR::iload) ||
           ((node->getOpCode().getOpCodeValue() == TR::iloadi) &&
            ((node->getFirstChild()->getOpCode().getOpCodeValue() ==
              TR::aload) &&
             !((node->getFirstChild())->hasUnresolvedSymbolReference()) &&
             !(_seenDefinedSymbolReferences->isSet(
                 node->getFirstChild()
      ...
      for (FirstLoadOfNonInvariant *firstLoad = _firstLoads->getFirst();
           firstLoad; firstLoad = firstLoad->getNext()) {
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    if (!findVarOfSimpleForm(node->getChild(i))) return NULL;
  }

  return node;
}

void OMR::ValuePropagation::createNewBucketForArrayIndex(
    ArrayLengthToVersion *array,
    TR_LinkHead<ArrayLengthToVersion> *arrayLengths, int32_t c,
    TR::Node *baseNode, TR::Node *bndchkNode,
  ...
  if (!array) {  // create a new arrayLengthToVersion
    ...
    array->_arrayIndicesInfo =
        new (trStackMemory()) TR_LinkHead<ArrayIndexInfo>();
...
void OMR::ValuePropagation::collectDefSymRefs(TR::Node *node,
                                              TR::Node *parent) {
  if (!node) return;
  TR::ILOpCode &opCode = node->getOpCode();
  TR::ILOpCodes opCodeValue = opCode.getOpCodeValue();
  if (opCode.hasSymbolReference()) {
    ...
    if (opCode.isResolveCheck()) {
      TR::SymbolReference *childSymRef =
      ...
      childSymRef->getUseDefAliases(isCallDirect)
          .getAliasesAndUnionWith(*_seenDefinedSymbolReferences);
    }

    if (!opCode.isLoadVar() && (opCode.getOpCodeValue() != TR::loadaddr)) {
      bool isCallDirect = false;
      if (opCode.isCallDirect()) isCallDirect = true;

      if (!opCode.isCheck() && !opCode.isStore() /* &&
             !node->isTheVirtualCallNodeForAGuardedInlinedCall() */)
            {
        symReference->getUseDefAliases(isCallDirect)
            .getAliasesAndUnionWith(*_seenDefinedSymbolReferences);
      }
      ...
      if (opCode.isStore()) {
        ...
        if (opCode.getOpCodeValue() == TR::istore) {
          ...
          if (load && symReference->getReferenceNumber() ==
                          load->getSymbolReference()
                              ->getReferenceNumber()) {  // check if this is the
                                                         // first time
            bool firstLoadWasSeen = false;
            for (FirstLoadOfNonInvariant *firstLoad = _firstLoads->getFirst();
                 firstLoad; firstLoad = firstLoad->getNext()) {
              if (firstLoad->_symRefNum ==
                  load->getSymbolReference()->getReferenceNumber()) {
                firstLoadWasSeen = true;
                break;
              }
            }
            if (!firstLoadWasSeen) {
              FirstLoadOfNonInvariant *newFirstLoad =
                  new (trStackMemory()) FirstLoadOfNonInvariant;
              newFirstLoad->_symRefNum =
                  load->getSymbolReference()->getReferenceNumber();
...
TR::TreeTop *OMR::ValuePropagation::createReferenceArrayNodeWithoutFlags(
    TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcObjectRef,
    TR::SymbolReference *dstObjectRef, TR::SymbolReference *lenRef,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
    bool useFlagsOnOriginalArraycopy) {
  TR::Node *root = tree->getNode()->getFirstChild();
  TR::Node *len = TR::Node::createLoad(root, lenRef);
  ...
  TR::Node *node =
      TR::Node::createArraycopy(srcObject, dstObject, src, dst, len);
  node->setNumChildren(5);
...
void OMR::ValuePropagation::
    transformReferenceArrayCopyWithoutCreatingStoreTrees(
        TR_TreeTopWrtBarFlag *arrayTree, TR::SymbolReference *srcObjRef,
        TR::SymbolReference *dstObjRef, TR::SymbolReference *srcRef,
        TR::SymbolReference *dstRef, TR::SymbolReference *lenRef) {
  TR::Node *node = arrayTree->_treetop->getNode();
  if (node->getOpCodeValue() != TR::arraycopy) node = node->getFirstChild();
  ...
  createReferenceArrayNodeWithoutFlags(arrayTree->_treetop,
                                       arraycopyArrayStoreCheck, srcObjRef,
                                       dstObjRef, lenRef, srcRef, dstRef, true);
  ...
  if ((arrayTree->_flag & NEED_WRITE_BARRIER) == 0)
    createPrimitiveArrayNodeWithoutFlags(arrayTree->_treetop,
                                         arraycopyNoArrayStoreCheck, srcRef,
                                         dstRef, lenRef, true, true);
  else {
    createReferenceArrayNodeWithoutFlags(
        arrayTree->_treetop, arraycopyNoArrayStoreCheck, srcObjRef, dstObjRef,
        lenRef, srcRef, dstRef, true);
    arraycopyNoArrayStoreCheck->getNode()
        ->getFirstChild()
        ->setNoArrayStoreCheckArrayCopy(true);
  }
  ...
  arraycopyBlock->createConditionalBlocksBeforeTree(
      arrayTree->_treetop, ifTree, arraycopyArrayStoreCheck,
      arraycopyNoArrayStoreCheck, cfg, false);
  ifTree->getNode()->setBranchDestination(
      arraycopyArrayStoreCheck->getEnclosingBlock()->getEntry());
  if (!arraycopyBlock->isCold()) {
    TR::Block *arraycopyArrayStoreBlock =
        arraycopyArrayStoreCheck->getEnclosingBlock();
    arraycopyArrayStoreBlock->setIsCold(false);
    arraycopyArrayStoreBlock->setFrequency(arraycopyBlock->getFrequency() / 3);

    TR::Block *arraycopyNoArrayStoreBlock =
        arraycopyNoArrayStoreCheck->getEnclosingBlock();
    arraycopyNoArrayStoreBlock->setIsCold(false);
    arraycopyNoArrayStoreBlock->setFrequency(
        (2 * arraycopyBlock->getFrequency()) / 3);

    arraycopyArrayStoreBlock->getSuccessors().front()->setFrequency(
        arraycopyBlock->getFrequency() / 3);
    arraycopyArrayStoreBlock->getPredecessors().front()->setFrequency(
        arraycopyBlock->getFrequency() / 3);

    arraycopyNoArrayStoreBlock->getSuccessors().front()->setFrequency(
        (2 * arraycopyBlock->getFrequency()) / 3);
    arraycopyNoArrayStoreBlock->getPredecessors().front()->setFrequency(
        (2 * arraycopyBlock->getFrequency()) / 3);
  }

  if (trace()) {
    comp()->dumpMethodTrees(
        "Trees after arraycopy array store check specialization");
    cfg->comp()->getDebug()->print(cfg->comp()->getOutFile(), cfg);
  }
}

void OMR::ValuePropagation::transformReferenceArrayCopy(
    TR_TreeTopWrtBarFlag *arrayTree) {
  TR::Node *node = arrayTree->_treetop->getNode();
  if (node->getOpCodeValue() != TR::arraycopy) node = node->getFirstChild();
  ...
  createStoresForArraycopyChildren(comp(), arrayTree->_treetop, srcObjRef,
                                   dstObjRef, srcRef, dstRef, lenRef);

  transformReferenceArrayCopyWithoutCreatingStoreTrees(
      arrayTree, srcObjRef, dstObjRef, srcRef, dstRef, lenRef);
}

void OMR::ValuePropagation::transformUnknownTypeArrayCopy(
    TR_TreeTopWrtBarFlag *arrayTree) {
#ifdef J9_PROJECT_SPECIFIC
  TR::Node *node = arrayTree->_treetop->getNode();
  if (node->getOpCodeValue() != TR::arraycopy) node = node->getFirstChild();
  ...
  // TR_ASSERT(srcRef, "No source address ref\n");
  // TR_ASSERT(dstRef, "No destination address ref\n");
  createPrimitiveArrayNodeWithoutFlags(arrayTree->_treetop, arraycopyPrimitive,
                                       srcRef, dstRef, lenRef, true, false);
  // arraycopyForward->getNode()->getFirstChild()->setRarePathForwardArrayCopy(true,
  // comp());

  TR_ASSERT(srcObjRef, "No source object ref\n");
  TR_ASSERT(dstObjRef, "No destination object ref\n");
  createReferenceArrayNodeWithoutFlags(arrayTree->_treetop, arraycopyReference,
                                       srcObjRef, dstObjRef, lenRef, srcRef,
                                       dstRef, true);
  ...
  arraycopyBlock->createConditionalBlocksBeforeTree(
      arrayTree->_treetop, ifTree, arraycopyReference, arraycopyPrimitive, cfg,
      false);
  ifTree->getNode()->setBranchDestination(
      arraycopyReference->getEnclosingBlock()->getEntry());
  if (!arraycopyBlock->isCold()) {
    TR::Block *arraycopyReferenceBlock =
        arraycopyReference->getEnclosingBlock();
    arraycopyReferenceBlock->setIsCold(false);
    arraycopyReferenceBlock->setFrequency(arraycopyBlock->getFrequency() / 3);

    TR::Block *arraycopyPrimitiveBlock =
        arraycopyPrimitive->getEnclosingBlock();
    arraycopyReferenceBlock->setIsCold(false);
    arraycopyPrimitiveBlock->setFrequency((2 * arraycopyBlock->getFrequency()) /
                                          3);

    arraycopyReferenceBlock->getSuccessors().front()->setFrequency(
        arraycopyBlock->getFrequency() / 3);
    arraycopyReferenceBlock->getPredecessors().front()->setFrequency(
        arraycopyBlock->getFrequency() / 3);

    arraycopyPrimitiveBlock->getSuccessors().front()->setFrequency(
        (2 * arraycopyBlock->getFrequency()) / 3);
    arraycopyPrimitiveBlock->getPredecessors().front()->setFrequency(
        (2 * arraycopyBlock->getFrequency()) / 3);
  }

  if (trace()) {
    comp()->dumpMethodTrees(
        "Trees after arraycopy reference/primitive specialization");
#if DEBUG
    cfg->comp()->getDebug()->print(cfg->comp()->getOutFile(), cfg);
#endif
  }
  ...
  if ((arrayTree->_flag & NEED_ARRAYSTORE_CHECK) == 0)
    arraycopyReference->getNode()
        ->getFirstChild()
        ->setNoArrayStoreCheckArrayCopy(true);
  else
    transformReferenceArrayCopyWithoutCreatingStoreTrees(
        new (trStackMemory())
            TR_TreeTopWrtBarFlag(arraycopyReference, arrayTree->_flag),
        srcObjRef, dstObjRef, srcRef, dstRef, lenRef);
#endif
}

static void changeBranchToGoto(OMR::ValuePropagation *vp, TR::Node *guardNode,
                               TR::Block *guard) {
  ...
  if (fallThroughBlock) {
    for (auto e = guard->getSuccessors().begin();
         e != guard->getSuccessors().end(); ++e) {
...
bool OMR::ValuePropagation::checkAllUnsafeReferences(TR::Node *node,
                                                     vcount_t visitCount) {
  ...
  if (node->getOpCode().hasSymbolReference() &&
    ...
    if (_unsafeArrayAccessNodes->get(node->getGlobalIndex())) {
      comp()->getSymRefTab()->aliasBuilder.unsafeArrayElementSymRefs().set(
          node->getSymbolReference()->getReferenceNumber());
    } else {
      if (trace())
...
void OMR::ValuePropagation::doDelayedTransformations() {
  ...
  for (scalarizedArrayCopy = treesIt1.getFirst(); scalarizedArrayCopy;
    ...
    TR::TransformUtil::scalarizeArrayCopy(comp(), scalarizedArrayCopy->_node,
                                          scalarizedArrayCopy->_treetop, true,
  ...
  // NOTE: the array copy spine checks are processed before the reference,
  // realtime, and
  //       unknown arraycopies because it refactors the CFG at a higher (outer)
  //       level than the others.  To simplify the CFG it should be run first.
  //
  if (comp()->requiresSpineChecks()) {
    ListIterator<TR_ArrayCopySpineCheck> acscIt(&_arrayCopySpineCheck);
    TR_ArrayCopySpineCheck *arrayCopySpineCheck;
    for (arrayCopySpineCheck = acscIt.getFirst(); arrayCopySpineCheck;
  ...
  for (converterCallTree = convIt.getFirst(); converterCallTree;
       converterCallTree = convIt.getNext()) {
  ...
  if (comp()->generateArraylets()) {
    ...
    for (rtArrayCopyTree = tt.getFirst(); rtArrayCopyTree;
         rtArrayCopyTree = tt.getNext()) {
  ...
  if (_blocksToBeRemoved) {
    for (i = _blocksToBeRemoved->size() - 1; i >= 0; --i) {
      ...
      if (performTransformation(comp(),
                                "%sRemoving unreachable block_%d at [%p]\n",
  ...
  if (_edgesToBeRemoved) {
    for (i = _edgesToBeRemoved->size() - 1; i >= 0; --i) {
      ...
      // NB: this following transformation is not conditional - it must be done
      // otherwise the CFG could be incorrect (for example, if a conditional
      // branch was converted to a goto, you have to remove the extra edge in
      // the CFG or else madness will ensue.
      removedEdgeSources.push_back(toBlock(edge->getFrom()));
      if (std::find(edge->getTo()->getPredecessors().begin(),
                    edge->getTo()->getPredecessors().end(),
                    edge) != edge->getTo()->getPredecessors().end()) {
        if (trace())
      ...
      if (node->getSuccessors().empty()) {
        ...
        if (!node->nodeIsRemoved())
          cfg->addEdge(
              TR::CFGEdge::createEdge(node, cfg->getEnd(), trMemory()));
      }
    }
  }
  ...
  if (!_multiLeafCallsToInline.isEmpty()) {
    ...
    for (multiLeafCall = iter.getCurrent(); multiLeafCall != NULL;
         multiLeafCall = iter.getNext()) {
      TR::Node *vcallNode = multiLeafCall->getNode()->getFirstChild();
      // maybe the call node was removed.
      if (vcallNode->getReferenceCount() < 1) continue;
  ...
  if (_unsafeCallsToInline.getFirst() != NULL) {
    ...
    for (CallInfo *uci = _unsafeCallsToInline.getFirst(); uci;
         uci = uci->getNext()) {
      if (uci->_block->nodeIsRemoved()) continue;
  ...
  for (CallInfo *ci = _devirtualizedCalls.getFirst(); ci; ci = ci->getNext()) {
    ...
    if (ci->_block->nodeIsRemoved()) continue;

    TR::Node *callNode;
    TR::Node *parent = ci->_tt->getNode();
    if (parent->getNumChildren() &&
        (callNode = parent->getFirstChild())->getOpCode().isCall()) {
      ...
      if (callGuarded) {
        if (callNode->getOpCode().isCallIndirect()) {
          // TODO : see if we can be smarter about the type of the guard being
          // used
        } else {
          ...
          if (!guardForDifferentCall) {
            ...
            if (!guard) {
              ...
              if (ci->_block->getPredecessors().size() == 1)
                foldedGuard =
                    ci->_block->getPredecessors().front()->getFrom()->asBlock();

              if (foldedGuard && (foldedGuard != cfg->getStart()) &&
                  (foldedGuard->getSuccessors().size() == 1) &&
                  (foldedGuard->getLastRealTreeTop()
                       ->getNode()
                       ->getOpCodeValue() == TR::Goto) &&
                  (foldedGuard->getLastRealTreeTop()
                       ->getNode()
                       ->getBranchDestination() == ci->_block->getEntry()))
                callUnreachable = false;
            }

            if (guard && (guard->getSuccessors().size() == 2) &&
                guard->getExit()) {
              ...
              if (guardNode) {
                ...
                canBeRemoved =
                    ...
                    callNode->getOpCode()
                        .isCallDirect();  // do not remove guards for indirect
                                          // calls. If the ifacmpne handler
                                          // didn't do it, the chances are it is
                                          // illegal to remove the guard
              } else
                canBeRemoved = true;
              ...
              if (guardNode && guardNode->isProfiledGuard()) {
                ...
                if (classTypeNode->getOpCodeValue() == TR::aconst &&
                  ...
                  TR_OpaqueClassBlock *typeClass =
                      (TR_OpaqueClassBlock *)classTypeNode->getAddress();
                  TR_OpaqueClassBlock *receiverClass =
                      callNode->getSymbolReference()
                          ...
                          ->getResolvedMethod()
                          ->containingClass();
                  TR_YesNoMaybe result = comp()->fe()->isInstanceOf(
                      typeClass, receiverClass, true);
                  ...
                  if (ci->_thisType && (ci->_thisType != typeClass)) {
                    isVPClassDerivedFromGuardClass = comp()->fe()->isInstanceOf(
                        ci->_thisType, typeClass, true);
                    if (isVPClassDerivedFromGuardClass == TR_no)
                      isGuardClassDerivedFromVPClass =
                          comp()->fe()->isInstanceOf(typeClass, ci->_thisType,
                                                     true);
                  }

                  if ((result == TR_no) ||
                      (isVPClassDerivedFromGuardClass == TR_yes) ||
                      (isGuardClassDerivedFromVPClass == TR_no)) {
                    typeCompatibleStatus = TR_no;  // types incompatible
                  } else if ((result == TR_yes) &&
                             (ci->_thisType == typeClass) && canBeRemoved) {
                  ...
                  TR_ASSERT(
                      (classTypeNode->getOpCodeValue() == TR::aconst &&
                       classTypeNode->isMethodPointerConstant()),
                      "Devirtualization of guarded profiled calls only works "
                      "currently for vft-guards or method compare guards");
                ...
                if (typeCompatibleStatus == TR_no) {
                  ...
                  if ((guardType == 1) &&
                      performTransformation(
                          comp(),
                          "%sChanging branch (guard) %p in block_%d to goto "
                    ...
                    callUnreachable = false;
                             ...
                             performTransformation(
                                 comp(),
                                 "%sRemoving branch %p in block_%d due to "
                                 "devirtualization\n",
                                 OPT_DETAILS, guardNode, guard->getNumber())) {
                    // remove branch
                  ...
                  if ((guardType == 1) &&
                      performTransformation(comp(),
                                            "%sRemoving branch %p in block_%d "
                                            "due to devirtualization\n",
                                            OPT_DETAILS, guardNode,
                                            guard->getNumber())) {
                    // remove branch
                    guard->removeBranch(comp());
                    // callUnreachable true;
                             ...
                             performTransformation(
                                 comp(),
                                 "%sChanging branch (guard) %p in block_%d to "
                                 "goto due to devirtualization\n",
                                 OPT_DETAILS, guardNode, guard->getNumber())) {
                    // change the if to goto
                    changeBranchToGoto(this, guardNode, guard);
                    // callUnreachable  true;
      ...
      if (!callGuarded || !callUnreachable) {
...
#endif
        {
          if (comp()->getMethodHotness() <= warm &&
              comp()->getOption(TR_DisableInliningDuringVPAtWarm)) {
            if (trace())
            ...
            if (comp()->getMethodHotness() <= warm) {
              initialMaxSize =
                  comp()->getOptions()->getMaxSzForVPInliningWarm();
              newInlineCall.setSizeThreshold(initialMaxSize);
            }

            if (!newInlineCall.inlineCall(ci->_tt, ci->_thisType, true,
                                          ci->_argInfo, initialMaxSize)) {
              ...
              if (!callNode->getSymbolReference()
                ...
                if (callNode->getOpCode().isCallIndirect()) {
                  ...
                  comp()->findOrCreateDevirtualizedCall(callNode,
                                                        ci->_thisType);
                }
              }
            }
  ...
  for (VirtualGuardInfo *cvg = _convertedGuards.getFirst(); cvg;
       cvg = cvg->getNext()) {
    if (cvg->_block->nodeIsRemoved()) continue;

    // IL part
    TR::Node *oldNode = cvg->_currentTree->getNode();

    // !oldNode means that the branch was already removed
    if (!oldNode || !performTransformation(
                        comp(),
                        "%sReplacing the old guard %p with the shiny new "
                        "overridden guard %p at treetop %p\n",
  ...
  for (getComponentCallNode = nodesIt.getFirst(); getComponentCallNode;
       getComponentCallNode = nodesIt.getNext()) {
    TR::Node::recreate(getComponentCallNode, TR::aloadi);
    getComponentCallNode->setSymbolReference(
        comp()->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
    if (getComponentCallNode->getNumChildren() > 1) {
      TR_ASSERT((getComponentCallNode->getNumChildren() == 2),
                "Only expect two children for call\n");
  ...
  for (predictedThrow = throwsIt.getFirst(); predictedThrow;
    ...
    if (predictedCatchBlock->specializedDesyncCatchBlock())
      dumpOptDetails(
          comp(),
          "%sChanging a throw [%p] to a goto for specializedDesyncCatchBlock\n",
          OPT_DETAILS, node);
    else if (!performTransformation(comp(),
                                    "%sChanging a throw [%p] to a goto\n",
                                    OPT_DETAILS, node))
      continue;

    optimizer()->setAliasSetsAreValid(
        false);  // the catch auto must be aliased to the catch block
    TR::SymbolReference *excpSymRef =
    ...
    if (treeTop->getNode()->getOpCodeValue() == TR::NULLCHK) {
      ...
      treeTop = TR::TreeTop::create(
          comp(), treeTop,
          TR::Node::createWithSymRef(TR::astore, 1, 1, node->getFirstChild(),
                                     excpSymRef));
    } else {
    ...
    TR_HandleInjectedBasicBlock hibb(comp(), NULL, currentSymbol, l1, l2, l3,
                                     0);
    hibb.findAndReplaceReferences(predictedCatchBlock->getEntry(),
                                  gotoDestination, 0);
    ListIterator<TR::SymbolReference> newTemps(&l2);
    for (TR::SymbolReference *newTemp = newTemps.getFirst(); newTemp;
         newTemp = newTemps.getNext())
      currentSymbol->addAutomatic(newTemp->getSymbol()->castToAutoSymbol());

    ListElement<TR_Pair<TR::Node, TR::Block> > *e =
        throwsIt.getCurrentElement()->getNextElement();
    ...
    TR::TreeTop::create(
        comp(), treeTop,
        TR::Node::create(node, TR::Goto, 0, gotoDestination->getEntry()));
  ...
  for (TR_OpaqueClassBlock *clazz = cit.getCurrent(); clazz;
       clazz = cit.getNext()) {
  ...
  for (TR_ResolvedMethod *method = mit.getCurrent(); method;
       method = mit.getNext()) {
  ...
  // Commit any preexistence based assumptions
  //
  ListIterator<TR_OpaqueClassBlock> prexCit(
      &_prexClassesThatShouldNotBeNewlyExtended);
  for (TR_OpaqueClassBlock *prexClazz = prexCit.getCurrent(); prexClazz;
       prexClazz = prexCit.getNext()) {
    // printf("---secs--- class assumption in %s\n", comp()->signature());
    ...
    TR_PersistentClassInfo *classInfo =
        comp()
            ...
            ->findClassInfoAfterLocking(prexClazz, comp());
    if (classInfo) {
      ...
      for (TR_PersistentClassInfo *subClassInfo = subClassesIt.getFirst();
  ...
  if (0 && usePreexistence() &&
      _prexClassesThatShouldNotBeNewlyExtended.isEmpty()) {
    if (chTableWasValid()) comp()->setFailCHTableCommit(false);
  }
  ListIterator<TR_PersistentClassInfo> resetCit(
      &_resetClassesThatShouldNotBeNewlyExtended);
  for (TR_PersistentClassInfo *resetClazz = resetCit.getCurrent(); resetClazz;
       resetClazz = resetCit.getNext()) {
    if (!_prexClassesThatShouldNotBeNewlyExtended.find(
            resetClazz->getClassId())) {
  ...
  // Change here
  for (ClassInitInfo *cii = _classesToCheckInit.getFirst(); cii;
       cii = cii->getNext()) {
    if (cii->_block->nodeIsRemoved()) continue;
    ...
    if (ifNode->getOpCodeValue() == TR::ifacmpne) {
      ...
      newBlock->getLastRealTreeTop()->getNode()->setBranchDestination(
    ...
    if (splitBlock->getLastRealTreeTop()->getNode()->getOpCodeValue() !=
        TR::Goto) {
      TR::Node *gotoNode =
          TR::Node::create(ifNode, TR::Goto, 0, origDest->getEntry());
      TR::TreeTop *gotoTree = TR::TreeTop::create(comp(), gotoNode, NULL, NULL);
      splitBlock->append(gotoTree);
    } else {
      splitBlock->getLastRealTreeTop()->getNode()->setBranchDestination(
          origDest->getEntry());
    }
    ...
    if ((origFirst->getOpCodeValue() == TR::aload) &&
      ...
      staticName =
          origFirst->getSymbolReference()->getOwningMethod(comp())->staticName(
              origFirst->getSymbolReference()->getCPIndex(), staticNameLen,
              comp()->trMemory());

      if ((staticName && (staticNameLen > 0) &&
           (!strncmp(staticName,
                     "com/ibm/websphere/ras/TraceComponent.fineTracingEnabled",
                     55) ||
            !strncmp(staticName,
                     "com/ibm/ejs/ras/TraceComponent.anyTracingEnabled", 48) ||
            !strncmp(staticName, "java/lang/String.compressionFlag", 32))) ||
          ((cii->_len == 41) &&
           !strncmp(cii->_sig, "Lcom/ibm/websphere/ras/TraceEnabledToken;",
                    cii->_len)) ||
          ((cii->_len == 35) &&
           !strncmp(cii->_sig, "Lcom/ibm/ejs/ras/TraceEnabledToken;",
                    cii->_len)) ||
          ((cii->_len == 40) &&
           !strncmp(cii->_sig, "Ljava/lang/String$StringCompressionFlag;",
                    cii->_len))) {
        recognizedStatic = true;
        ifNode->setAndIncChild(0, origFirst->duplicateTree());
        ifNode->setAndIncChild(1,
                               TR::Node::create(origSecond, TR::aconst, 0, 0));
      }
    }
    ...
    if (!recognizedStatic) {
      ...
      TR::Node *astoreNode = TR::Node::createWithSymRef(
          TR::astore, 1, 1, ifNode->getFirstChild(), newSymbolReference);
      TR::TreeTop *astoreTree =
          TR::TreeTop::create(comp(), astoreNode, NULL, NULL);

      prevTree->join(astoreTree);
      astoreTree->join(ifTree);
      prevTree = astoreTree;

      ifNode->setAndIncChild(
          0, TR::Node::createWithSymRef(TR::aload, 0, newSymbolReference));
    ...
    TR::Node *guard = comp()->createSideEffectGuard(
        comp(), block->getEntry()->getNode(), splitBlock2->getEntry());
    ...
    bool found = false;
    for (TR_ClassLoadCheck *clc =
             comp()->getClassesThatShouldNotBeLoaded()->getFirst();
         clc; clc = clc->getNext()) {
      if (clc->_length == cii->_len &&
          !strncmp(clc->_name, cii->_sig, cii->_len)) {
        found = true;
        break;
      }
    }
  ...
  if (!_unsafeArrayAccessNodes->isEmpty()) {
    ...
    while (curTree) {
      if (curTree->getNode() &&
        ...
        comp()
            ->getSymRefTab()
            ->aliasBuilder.unsafeArrayElementSymRefs()
            .empty();
        break;
      }
  ...
  if (trace()) {
    traceMsg(comp(),
             "Unsafe references that are only used to access array elements: ");
    comp()->getSymRefTab()->aliasBuilder.unsafeArrayElementSymRefs().print(
        comp());
    traceMsg(comp(), "\n");
  }
}

void constrainRangeByPrecision(const int64_t low, const int64_t high,
                               const int32_t precision, int64_t &lowResult,
  ...
  if (isNonNegative) lowResult = 0;
}

#if USE_TREES
void OMR::ValuePropagation::ValueConstraintHandler::setVP(
    OMR::ValuePropagation *vp) {
  _vp = vp;
}

OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::allocate(int32_t key) {
...
OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::copy(ValueConstraint *vc) {
  Relationship *rel = _vp->copyRelationships(vc->relationships.getFirst());
  StoreRelationship *storeRel =
      _vp->copyStoreRelationships(vc->storeRelationships.getFirst());
  ValueConstraint *newvc =
      _vp->createValueConstraint(vc->getValueNumber(), rel, storeRel);
  return newvc;
}
...
void OMR::ValuePropagation::ValueConstraintHandler::empty(
    ValueConstraints &valueConstraints) {
  ValueConstraint *vc;
  while (vc = valueConstraints.pop()) free(vc);
}

OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::copyAll(
    ValueConstraints &valueConstraints) {
...
OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::getRoot(ValueConstraints &list) {
  return list.getFirst();
}

void OMR::ValuePropagation::ValueConstraintHandler::setRoot(
    ValueConstraints &list, ValueConstraint *vc) {
  list.setFirst(vc);
}

OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::find(int32_t key,
                                                    ValueConstraints &list) {
  ValueConstraint *prev, *cur;
  for (cur = list.getFirst(), prev = NULL; cur;
       prev = cur, cur = cur->getNext()) {
    if (cur->getValueNumber() < key) continue;
    if (cur->getValueNumber() > key) break;
    return cur;
  }
  return NULL;
}
...
OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::findOrCreate(
    int32_t key, ValueConstraints &list) {
  ValueConstraint *prev, *cur;
  for (cur = list.getFirst(), prev = NULL; cur;
       prev = cur, cur = cur->getNext()) {
    if (cur->getValueNumber() < key) continue;
...
OMR::ValuePropagation::ValueConstraint *
OMR::ValuePropagation::ValueConstraintHandler::remove(int32_t key,
                                                      ValueConstraints &list) {
  ValueConstraint *prev, *cur;
  for (cur = list.getFirst(), prev = NULL; cur;
       prev = cur, cur = cur->getNext()) {
...
OMR::ValuePropagation::InductionVariable::InductionVariable(
    TR::Symbol *sym, TR::Node *entryDef, int32_t incrVN, TR::VPConstraint *incr,
    OMR::ValuePropagation *vp)
    : _symbol(sym),
      _entryDef(entryDef),
      _entryConstraint(0),
...
OMR::ValuePropagation::CallInfo::CallInfo(OMR::ValuePropagation *vp,
...
OMR::ValuePropagation::VirtualGuardInfo::VirtualGuardInfo(
    OMR::ValuePropagation *vp, TR_VirtualGuard *vgOld, TR_VirtualGuard *vgNew,
    TR::Node *newGNode, TR::Node *cn)
...
OMR::ValuePropagation::ClassInitInfo::ClassInitInfo(OMR::ValuePropagation *vp,
                                                    char *sig, int32_t len)
