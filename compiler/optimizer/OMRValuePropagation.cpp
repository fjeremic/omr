  ...
  while (from || to) {
    if (!to || (from && from->relative < to->relative)) {
      ...
      if (preserveStoreRelationships && mergingStore && (from->relative == AbsoluteConstraint)
        && (!inBothLists /* || !isToEmpty */)) // commented out the !isToEmpty because it is not guaranteed to be safe
                                               // to take an absolute constraint that only exists on one side
      {
  ...
  TR::Node* node, int32_t valueNumber, TR::VPConstraint* constraint, int32_t relative)
{
  ...
  for (rel = gc->constraints.getFirst(); rel; rel = rel->getNext()) {
    ...
    if (reverse && follow) {
      ...
      if (relative == AbsoluteConstraint)
        linkedConstraint
          = reverse->constraint->asRelation()->propagateAbsoluteConstraint(follow->constraint, valueNumber, this);
      else
  ...
  TR::Node* node, int32_t relative, bool& isGlobal, bool forceMerge)
{
  ...
  if (relative != AbsoluteConstraint) {
    if (trace())
      traceMsg(
        comp(), "we are processing a relative constraint and merging with backedge constraints is not supported\n");
    return NULL;
  }
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (node->getSymbol()->isAutoOrParm() && !unseenDefsFound
      ...
      for (loopInfo = _loopInfo; loopInfo && loopInfo != container; loopInfo = loopInfo->_parent) {
        if (trace())
          traceMsg(comp(), "   %s [%p] looking at back edges in loop %d for def %s [%p]: ", node->getOpCode().getName(),
              ...
              if (positiveIncrement) {
                  ...
                  if ((uint32_t)defConstraint->getHighInt() < TR::getMaxUnsigned<TR::Int32>()) {
                    if ((uint32_t)defConstraint->getLowInt() < (uint32_t)constraint->getLowInt()
                      ...
                      TR::VPConstraint* newDefConstraint
                        = TR::VPIntRange::create(this, constraint->getLowInt(), defConstraint->getHighInt());
                ...
                if (defConstraint->getLowInt() > TR::getMinSigned<TR::Int32>()) {
                  if (defConstraint->getLowInt() <= constraint->getHighInt()
                    ...
                    TR::VPConstraint* newDefConstraint
                      = TR::VPIntRange::create(this, defConstraint->getLowInt(), constraint->getHighInt());
                    defConstraint = defConstraint->intersect(newDefConstraint, this);
                  }
                }
              }
              ...
              if (positiveIncrement) {
                if (defConstraint->isUnsigned()) {
                  if ((uint16_t)defConstraint->getHighShort() < TR::getMaxUnsigned<TR::Int16>()) {
                    if ((uint16_t)defConstraint->getLowShort() < (uint16_t)constraint->getLowShort()
                      ...
                      TR::VPConstraint* newDefConstraint
                        = TR::VPShortRange::create(this, constraint->getLowShort(), defConstraint->getHighShort());
                      defConstraint = defConstraint->intersect(newDefConstraint, this);
                    }
                  }
                ...
                if (defConstraint->getLowShort() > TR::getMinSigned<TR::Int16>()) {
                  if (defConstraint->getLowShort() <= constraint->getHighShort()
                    ...
                    TR::VPConstraint* newDefConstraint
                      = TR::VPShortRange::create(this, defConstraint->getLowShort(), constraint->getHighShort());
                    defConstraint = defConstraint->intersect(newDefConstraint, this);
                  }
                }
              }
              ...
              if (positiveIncrement) {
                if (defConstraint->isUnsigned()) {
                  if ((uint64_t)defConstraint->getHighLong() < TR::getMaxUnsigned<TR::Int64>()) {
                    if ((uint64_t)defConstraint->getLowLong() < (uint64_t)constraint->getLowLong()
                      ...
                      TR::VPConstraint* newDefConstraint
                        = TR::VPLongRange::create(this, constraint->getLowLong(), defConstraint->getHighLong());
                      defConstraint = defConstraint->intersect(newDefConstraint, this);
                    }
                  }
                ...
                if (defConstraint->getLowLong() > TR::getMinSigned<TR::Int64>()) {
                  if (defConstraint->getLowLong() <= constraint->getHighLong()
                    ...
                    TR::VPConstraint* newDefConstraint
                      = TR::VPLongRange::create(this, defConstraint->getLowLong(), constraint->getHighLong());
                    defConstraint = defConstraint->intersect(newDefConstraint, this);
                  }
                }
              }
...
void OMR::ValuePropagation::replaceByConstant(TR::Node* node, TR::VPConstraint* constraint, bool isGlobal)
{
  ...
  if (!performTransformation(
        comp(), "%sConstant folding %s [" POINTER_PRINTF_FORMAT "]", OPT_DETAILS, node->getOpCode().getName(), node))
    return;
...
void OMR::ValuePropagation::checkForInductionVariableIncrement(TR::Node* node)
{
    ...
    if (!_useDefInfo->getUseDef(defs, useIndex))
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if ((defVN != incrementVN) && !notInLoop) {
          ...
          if (trace())
            traceMsg(comp(),
              "      ivInfo is invalid for sym [%p], def [%d] node %p does not have the same value number as another "
              "increment\n",
              sym, defVN, defNode);
          break;
        }
  ...
  if (iv->_entryDef) {
    ...
    if (node->getOpCode().isLong())
      addConstraintToList(
        node, iv->_valueNumber, AbsoluteConstraint, TR::VPLongConst::create(this, incrementVN), &_curConstraints, true);
      ...
      addConstraintToList(
        node, iv->_valueNumber, AbsoluteConstraint, TR::VPIntConst::create(this, incrementVN), &_curConstraints, true);
  }
}
...
void OMR::ValuePropagation::checkForInductionVariableLoad(TR::Node* node)
{
  ...
  if (iv && iv->_entryDef) {
    ...
    // together
    if (node->getOpCode().isLong())
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint, TR::VPLongConst::create(this, useValueNumber),
        &_curConstraints, true);
    else if (node->getOpCode().isShort())
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint, TR::VPShortConst::create(this, useValueNumber),
        &_curConstraints, true);
    else
      addConstraintToList(node, iv->_valueNumber, AbsoluteConstraint, TR::VPIntConst::create(this, useValueNumber),
        &_curConstraints, true);
  }
}
    ...
    for (TR::CFGEdge* e = cfgIt.getFirst(); e; e = cfgIt.getNext()) {
      ...
      if (loopTestBlock) {
        ...
        if (branchNode->getOpCode().isBranch() && branchNode->getOpCode().isBooleanCompare()) {
          ...
          if ((firstChild->getOpCode().isLoadVarDirect() && (firstChild->getSymbolReference()->getSymbol() == sym))
    ...
    if (ivInfo->_symbol->getType().isInt32()) {
      ...
      if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
        if (ivInfo->_entryConstraint->asShortConst())
          ...
          entry = new (trHeapMemory())
            TR::VPShortRange(ivInfo->_entryConstraint->getLowShort(), ivInfo->_entryConstraint->getHighShort());
      } else
        entry = NULL;
      if (ivInfo->_increment->asShortConst())
      ...
      if (ivInfo->_entryConstraint && !ivInfo->_invalidEntryInfo) {
        if (ivInfo->_entryConstraint->asLongConst())
          ...
          entry = new (trHeapMemory())
            TR::VPLongRange(ivInfo->_entryConstraint->getLowLong(), ivInfo->_entryConstraint->getHighLong());
      } else
        entry = NULL;
      if (ivInfo->_increment->asLongConst())
        ...
        incr
          = new (trHeapMemory()) TR::VPLongRange(ivInfo->_increment->getLowLong(), ivInfo->_increment->getHighLong());
      if (exitConstraint) {
  ...
  TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop)
{
  ...
  for (TR::CFGEdge* edge = it.getFirst(); edge; edge = it.getNext()) {
    if (trace())
      traceMsg(
        comp(), "   defined on all paths for exit %d->%d:", edge->getFrom()->getNumber(), edge->getTo()->getNumber());
...
TR::TreeTop* TR::ArraycopyTransformation::createArrayNode(TR::TreeTop* tree, TR::TreeTop* newTree,
  TR::SymbolReference* srcRef, TR::SymbolReference* dstRef, TR::SymbolReference* lenRef, TR::SymbolReference* srcObjRef,
...
TR::TreeTop* TR::ArraycopyTransformation::createMultipleArrayNodes(TR::TreeTop* arrayTree, TR::Node* node)
{
  ...
  if (specificLength >= 0)
    specializeForLength(
      outerArraycopyTree, node, (uintptrj_t)specificLength, srcRef, dstRef, lenRef, srcObjRef, dstObjRef);
...
TR::TreeTop* TR::ArraycopyTransformation::specializeForLength(TR::TreeTop* tt, TR::Node* arraycopyNode,
  uintptrj_t lengthInBytes, TR::SymbolReference* srcRef, TR::SymbolReference* dstRef, TR::SymbolReference* lenRef,
      ...
      for (fromTT = fromTT->getNextTreeTop(); fromTT != tempBlock->getExit(); fromTT = fromTT->getNextTreeTop()) {
        ...
        if (fromTT->getNode()->getOpCode().isBranch())
          fromTT->getNode()->setBranchDestination(
            cloner.getToBlock(fromTT->getNode()->getBranchDestination()->getNode()->getBlock())->getEntry());
      }
    ...
    while (comparisonNode) {
      ...
      if (comp()->useCompressedPointers()) {
        for (int32_t i = 0; i < actualComparisonNode->getNumChildren(); i++) {
          ...
          } else if (objectRef->getOpCode().isArrayLength() && objectRef->getFirstChild()->getOpCode().isLoadIndirect()
  ...
  for (ArrayLengthToVersion* arrayLength = blockInfo->_arrayLengths->getFirst(); arrayLength;
    ...
    for (ArrayIndexInfo* arrayIndex = arrayLength->_arrayIndicesInfo->getFirst(); arrayIndex;
      ...
      if (performTransformation(comp(), "%s Creating tests outside block_%d for versioning arraylenth %p \n",
        ...
        if (arrayIndex->_baseNode) {
          nextComparisonNode
            = TR::Node::createif(TR::ificmplt, minIndex, TR::Node::create(arrayLength->_arrayLen, TR::iconst, 0, 0));

          if (trace())
            traceMsg(
              comp(), "First Test - Creating %p (%s)\n", nextComparisonNode, nextComparisonNode->getOpCode().getName());
      ...
      if (arrayLength->_arrayLen->getOpCode().isArrayLength()) {
        ...
        if (objectRefChild && !objectRefChild->isNonNull()) {
          dumpOptDetails(comp(),
            ...
            objectRefChild, blockInfo->_block->getNumber(), arrayLength->_arrayLen);
          TR::Node* nullTestNode
            = TR::Node::createif(TR::ifacmpeq, objectRefChild->duplicateTree(), TR::Node::aconst(objectRefChild, 0));
        ...
        if (arrayLength->_instanceOfClass && objectRefChild && !comp()->compileRelocatableCode()) {
          ...
          TR::Node* duplicateClassPtr = TR::Node::createWithSymRef(objectRefChild, TR::loadaddr, 0,
            comp()->getSymRefTab()->findOrCreateClassSymbol(
              objectRef->getSymbolReference()->getOwningMethodSymbol(comp()), -1, arrayLength->_instanceOfClass,
              false));
          ...
          TR::Node* ificmpeqNode
            = TR::Node::createif(TR::ificmpeq, instanceofNode, TR::Node::create(objectRef, TR::iconst, 0, 0));
        ...
        if (!objectRef->isNonNull()) {
          dumpOptDetails(comp(),
            "%s Creating test for nullCheck of %p outside block_%d for versioning arraylenth %p \n", OPT_DETAILS,
            objectRef, blockInfo->_block->getNumber(), arrayLength->_arrayLen);
          TR::Node* nullTestNode
...
void OMR::ValuePropagation::transformUnknownTypeArrayCopy(TR_TreeTopWrtBarFlag* arrayTree)
{
  ...
  if ((arrayTree->_flag & NEED_ARRAYSTORE_CHECK) == 0)
    ...
    transformReferenceArrayCopyWithoutCreatingStoreTrees(new (trStackMemory())
...
void OMR::ValuePropagation::doDelayedTransformations()
{
            ...
            if (guard && (guard->getSuccessors().size() == 2) && guard->getExit()) {
              ...
              if (guardNode) {
                ...
                canBeRemoved = virtualGuard->canBeRemoved()
                  && callNode->getOpCode()
                       .isCallDirect(); // do not remove guards for indirect calls. If the ifacmpne handler
              ...
              if (guardNode && guardNode->isProfiledGuard()) {
                ...
                if (classTypeNode->getOpCodeValue() == TR::aconst && classTypeNode->isClassPointerConstant()) {
                  ...
                  TR_ASSERT((classTypeNode->getOpCodeValue() == TR::aconst && classTypeNode->isMethodPointerConstant()),
                    "Devirtualization of guarded profiled calls only works currently for vft-guards or method compare "
                    "guards");
                ...
                if (typeCompatibleStatus == TR_no) {
                  ...
                  if ((guardType == 1)
                    && performTransformation(comp(),
                      "%sChanging branch (guard) %p in block_%d to goto due to devirtualization\n", OPT_DETAILS,
  ...
  for (VirtualGuardInfo* cvg = _convertedGuards.getFirst(); cvg; cvg = cvg->getNext()) {
    ...
    if (!oldNode
      || !performTransformation(comp(),
        "%sReplacing the old guard %p with the shiny new overridden guard %p at treetop %p\n", oldNode,
        cvg->_newGuardNode, cvg->_currentTree)) {
      continue;
    }
...
OMR::ValuePropagation::ValueConstraint* OMR::ValuePropagation::ValueConstraintHandler::copyAll(
...
OMR::ValuePropagation::ValueConstraint* OMR::ValuePropagation::ValueConstraintHandler::find(
...
OMR::ValuePropagation::ValueConstraint* OMR::ValuePropagation::ValueConstraintHandler::findOrCreate(
  int32_t key, ValueConstraints& list)
{
  ValueConstraint *prev, *cur;
  for (cur = list.getFirst(), prev = NULL; cur; prev = cur, cur = cur->getNext()) {
...
OMR::ValuePropagation::ValueConstraint* OMR::ValuePropagation::ValueConstraintHandler::remove(
  int32_t key, ValueConstraints& list)
{
  ValueConstraint *prev, *cur;
  for (cur = list.getFirst(), prev = NULL; cur; prev = cur, cur = cur->getNext()) {
