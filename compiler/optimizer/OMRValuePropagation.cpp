...
OMR::ValuePropagation::createRelationship(int32_t relative,
...
TR::VPConstraint *
OMR::ValuePropagation::addGlobalConstraint(TR::Node *node, int32_t valueNumber,
                                           TR::VPConstraint *constraint,
                                           int32_t relative) {
    ...
    if (!to || (from && from->relative < to->relative)) {
      ...
      if (preserveStoreRelationships && mergingStore &&
          ...
          (!inBothLists /* || !isToEmpty */)) // commented out the !isToEmpty
                                              // because it is not guaranteed to
                                              // be safe to take an absolute
...
//
inline uint32_t
OMR::ValuePropagation::hashGlobalConstraint(int32_t valueNumber) {
  return valueNumber &
         _globalConstraintsHTMaxBucketIndex; // exploiting the fact that size is
                                             // power of two
}
...
OMR::ValuePropagation::Relationship *
OMR::ValuePropagation::findConstraint(int32_t valueNumber, int32_t relative) {
...
TR::VPConstraint *
OMR::ValuePropagation::getStoreConstraint(TR::Node *node, TR::Node *relative) {
...
TR::VPConstraint *OMR::ValuePropagation::mergeDefConstraints(TR::Node *node,
  ...
  if (relative != AbsoluteConstraint) {
    if (trace())
      traceMsg(comp(), "we are processing a relative constraint and merging "
                       "with backedge constraints is not supported\n");
    return NULL;
  }
...
TR::TreeTop *
TR::ArraycopyTransformation::createMultipleArrayNodes(TR::TreeTop *arrayTree,
                                                      TR::Node *node) {
...
TR::TreeTop *
TR::ArraycopyTransformation::tryToSpecializeForLength(TR::TreeTop *tt,
                                                      TR::Node *arraycopyNode) {
...
TR::TreeTop *TR::ArraycopyTransformation::specializeForLength(
  ...
  requestOpt(OMR::basicBlockOrdering); // Block frequency is reset, so run order
                                       // blocks optimization to reorder this
                                       // path appropriately.
                ...
                canBeRemoved =
                    ...
                    callNode->getOpCode()
                        .isCallDirect(); // do not remove guards for indirect
                                         // calls. If the ifacmpne handler didn't
                                         // do it, the chances are it is illegal
                                         // to remove the guard
              } else
                canBeRemoved = true;
...
OMR::ValuePropagation::InductionVariable::InductionVariable(
    ...
    OMR::ValuePropagation *vp)
    : _symbol(sym), _entryDef(entryDef), _entryConstraint(0),
      _incrementVN(incrVN), _increment(incr) {
...
OMR::ValuePropagation::CallInfo::CallInfo(OMR::ValuePropagation *vp,
    ...
    : _tt(vp->_curTree), _block(vp->_curBlock), _thisType(thisType),
      _argInfo(argInfo) {}
...
OMR::ValuePropagation::VirtualGuardInfo::VirtualGuardInfo(
    ...
    : _currentTree(vp->_curTree), _block(vp->_curBlock),
      _oldVirtualGuard(vgOld), _newVirtualGuard(vgNew), _newGuardNode(newGNode),
      _callNode(cn) {}
