    ...
    bool inBothLists)
{
    ...
    while (from || to) {
        if (!to || (from && from->relative < to->relative)) {
            ...
            //
            if (preserveStoreRelationships && mergingStore && (from->relative == AbsoluteConstraint) && (!inBothLists /* || !isToEmpty */)) // commented out the !isToEmpty because it is not guaranteed to be safe to take an absolute constraint that only exists on one side
            {
        ...
        if (!from || from->relative > to->relative) {
            ...
            //
            rel = to->getNext();
            if (!preserveStoreRelationships || !mergingStore || (to->relative != AbsoluteConstraint) || (inBothLists /* && isFromEmpty */)) // but do not remove the storeRel
            {
...
bool OMR::ValuePropagation::isDefInUnreachableBlock(int32_t defIndex)
{
    ...
    while (defSubGraphNode) {
        ...
        if (!reachableInputEdgesFound && defSubGraphNode->getStructure()->getParent() && (defSubGraphNode->getStructure()->getParent()->getNumber() == defSubGraphNode->getNumber()))
    ...
    int32_t& value, bool isInstanceOf, bool isCheckCast)
{
    ...
    if (lhs->asClass() && rhs->asClass()) {
        ...
        if (jlKlass && noPresenceResult) {
            ...
            if (!objectClass->getClassType() && (castIsClassObject == TR_no) && (isInstanceOf || objectClass->isNonNullObject()) && (objectClass->isClassObject() == TR_yes)) {
                if (trace())
         ...
         */
            else if ((castIsClassObject == TR_yes) && !objectClass->getClassType() && (isInstanceOf || objectClass->isNonNullObject()) && (objectClass->isClassObject() == TR_no)) {
                result = 0;
                if (trace())
            ...
            //
            else if ((castIsClassObject == TR_yes) && !objectClass->getClassType() && (objectClass->isNonNullObject() || !isInstanceOf) && (objectClass->isClassObject() == TR_yes)) {
                result = 1;
                if (trace())
                ...
                if (defNode->getOpCode().isStoreDirect()) {
                    ...
                    if ((firstChild->getOpCodeValue() == TR::acall) || (firstChild->getOpCodeValue() == TR::acalli)) {
                        ...
                        if (!symRef->isUnresolved() && symRef->getSymbol()->isResolvedMethod()) {
                            ...
                            if ((method->getRecognizedMethod() == TR::java_math_BigDecimal_add) || (method->getRecognizedMethod() == TR::java_math_BigDecimal_subtract) || (method->getRecognizedMethod() == TR::java_math_BigDecimal_multiply)) {
                                TR::Node* thisChild = firstChild->getFirstChild();
                                if (firstChild->getOpCode().isCallIndirect())
                                    thisChild = firstChild->getSecondChild();

                                if ((thisChild == node) || ((thisChild->getOpCodeValue() == node->getOpCodeValue()) && (thisChild->getSymbolReference() == node->getSymbolReference()))) {
                    ...
                    if (defNode->getOpCode().isStoreDirect()) {
                        ...
                        if ((firstChild->getOpCodeValue() == TR::acall) || (firstChild->getOpCodeValue() == TR::acalli)) {
                            ...
                            if (!symRef->isUnresolved()) {
                                ...
                                if (method && ((method->getRecognizedMethod() == TR::java_math_BigDecimal_add) || (method->getRecognizedMethod() == TR::java_math_BigDecimal_subtract) || (method->getRecognizedMethod() == TR::java_math_BigDecimal_multiply))) {
                                    TR::Node* thisChild = firstChild->getFirstChild();
                                    if (firstChild->getOpCode().isCallIndirect())
                                        thisChild = firstChild->getSecondChild();
                            ...
                            if (ivInfo->_symbol->getType().isInt32()) {
                                if (!defConstraint->isUnsigned()) {
                                    if (ivInfo->_increment->asIntConst()) {
                                        ...
                                        if ((ivInfo->_increment->getLowInt() > 0) && (ivInfo->_increment->getHighInt() > 0))
                                            positiveIncrement = true;
                                        else if ((ivInfo->_increment->getLowInt() < 0) && (ivInfo->_increment->getHighInt() < 0))
                                            negativeIncrement = true;
                                    }
                                } else
                                    positiveIncrement = true;
                                ...
                                if (!defConstraint->isUnsigned()) {
                                    if (ivInfo->_increment->asLongConst()) {
                                        ...
                                        if ((ivInfo->_increment->getLowLong() > 0) && (ivInfo->_increment->getHighLong() > 0))
                                            positiveIncrement = true;
                                        else if ((ivInfo->_increment->getLowLong() < 0) && (ivInfo->_increment->getHighLong() < 0))
                                            negativeIncrement = true;
                                    }
                                } else
                                    positiveIncrement = true;
                                ...
                                if (!defConstraint->isUnsigned()) {
                                    if (ivInfo->_increment->asShortConst()) {
                                        ...
                                        if ((ivInfo->_increment->getLowShort() > 0) && (ivInfo->_increment->getHighShort() > 0))
                                            positiveIncrement = true;
                                        else if ((ivInfo->_increment->getLowShort() < 0) && (ivInfo->_increment->getHighShort() < 0))
                                            negativeIncrement = true;
                                    }
                                } else
                                    positiveIncrement = true;
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint32_t)defConstraint->getHighInt() < TR::getMaxUnsigned<TR::Int32>()) {
                                        if ((uint32_t)defConstraint->getLowInt() < (uint32_t)constraint->getLowInt() && (uint32_t)constraint->getLowInt() <= defConstraint->getHighInt()) {
                                    ...
                                    if (defConstraint->getLowInt() < constraint->getLowInt() && constraint->getLowInt() <= defConstraint->getHighInt()) {
                                        TR::VPConstraint* newDefConstraint = TR::VPIntRange::create(this, constraint->getLowInt(), defConstraint->getHighInt());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                                ...
                                if (defConstraint->getLowInt() > TR::getMinSigned<TR::Int32>()) {
                                    if (defConstraint->getLowInt() <= constraint->getHighInt() && constraint->getHighInt() < defConstraint->getHighInt()) {
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint16_t)defConstraint->getHighShort() < TR::getMaxUnsigned<TR::Int16>()) {
                                        if ((uint16_t)defConstraint->getLowShort() < (uint16_t)constraint->getLowShort() && (uint16_t)constraint->getLowShort() <= defConstraint->getHighShort()) {
                                    ...
                                    if (defConstraint->getLowShort() < constraint->getLowShort() && constraint->getLowShort() <= defConstraint->getHighShort()) {
                                        TR::VPConstraint* newDefConstraint = TR::VPShortRange::create(this, constraint->getLowShort(), defConstraint->getHighShort());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                                ...
                                if (defConstraint->getLowShort() > TR::getMinSigned<TR::Int16>()) {
                                    if (defConstraint->getLowShort() <= constraint->getHighShort() && constraint->getHighShort() < defConstraint->getHighShort()) {
                            ...
                            if (positiveIncrement) {
                                if (defConstraint->isUnsigned()) {
                                    if ((uint64_t)defConstraint->getHighLong() < TR::getMaxUnsigned<TR::Int64>()) {
                                        if ((uint64_t)defConstraint->getLowLong() < (uint64_t)constraint->getLowLong() && (uint64_t)constraint->getLowLong() <= (uint64_t)defConstraint->getHighLong()) {
                                    ...
                                    if (defConstraint->getLowLong() < constraint->getLowLong() && constraint->getLowLong() <= defConstraint->getHighLong()) {
                                        TR::VPConstraint* newDefConstraint = TR::VPLongRange::create(this, constraint->getLowLong(), defConstraint->getHighLong());
                                        defConstraint = defConstraint->intersect(newDefConstraint, this);
                                    }
                                }
                                ...
                                if (defConstraint->getLowLong() > TR::getMinSigned<TR::Int64>()) {
                                    if (defConstraint->getLowLong() <= constraint->getHighLong() && constraint->getHighLong() < defConstraint->getHighLong()) {
...
void OMR::ValuePropagation::replaceByConstant(TR::Node* node, TR::VPConstraint* constraint, bool isGlobal)
{
    ...
    case TR::Int16:
        ...
        (shortConstraint == NULL) ? node->setShortInt(constraint->asIntConst()->getInt()) : node->setShortInt(constraint->asShortConst()->getShort());
...
bool OMR::ValuePropagation::isHighWordZero(TR::Node* node)
{
    ...
    if (constraint) {
        ...
        if (longConstraint && (longConstraint->getLow() >= 0) && ((longConstraint->getHigh() & (((uint64_t)0xffffffff) << 32)) == 0)) {
        ...
        for (TR::CFGEdge* e = cfgIt.getFirst(); e; e = cfgIt.getNext()) {
            ...
            if (loopTestBlock) {
                ...
                if (branchNode->getOpCode().isBranch() && branchNode->getOpCode().isBooleanCompare()) {
                    ...
                    if ((firstChild->getOpCode().isLoadVarDirect() && (firstChild->getSymbolReference()->getSymbol() == sym)) || (secondChild->getOpCode().isLoadVarDirect() && (secondChild->getSymbolReference()->getSymbol() == sym))) {
                        return true;
                    }
                }
            }
...
void OMR::ValuePropagation::checkBackEdgeCoverage()
{
    ...
    for (iv = _loopInfo->_inductionVariables.getFirst(); iv; iv = next) {
        ...
        if (iv->_entryDef) {
            ...
            if (rel) {
                TR_ASSERT(((rel->constraint->asIntConst() && rel->constraint->asIntConst()->getInt() == iv->_incrementVN) || (rel->constraint->asLongConst() && rel->constraint->asLongConst()->getLong() == iv->_incrementVN) || (rel->constraint->asShortConst() && rel->constraint->asShortConst()->getShort() == iv->_incrementVN)), "getLow not equal to incrementVN");
...
void TR::GlobalValuePropagation::processStructure(TR_StructureSubGraphNode* node, bool lastTimeThrough, bool insideLoop)
{
    ...
    if (comp()->getOptions()->realTimeGC() && ((++numIter) & 0xf) == 0 && comp()->compilationShouldBeInterrupted(BEFORE_PROCESS_STRUCTURE_CONTEXT)) {
...
{
    return !valueConstraints.isEmpty() && !_vcHandler.getRoot(valueConstraints)->relationships.isEmpty() && _vcHandler.getRoot(valueConstraints)->relationships.getFirst()->constraint->asUnreachablePath();
}
    ...
    for (cur = _blocksToBeVersioned->getFirst(); cur; cur = cur->getNext()) {
        ...
        for (auto edge = lastBlock->getSuccessors().begin(); edge != lastBlock->getSuccessors().end(); ++edge) {
            ...
            if (!(lastOpCode.isBranch() || lastOpCode.isJumpWithMultipleTargets() || lastOpCode.isReturn() || (lastOpCode.getOpCodeValue() == TR::athrow)))
                ...
                if (!((lastNode->getBranchDestination() == succ->getEntry()) || (succ->getEntry() && (lastNode->getBranchDestination() == cloner.getToBlock(succ->getEntry()->getNode()->getBlock())->getEntry()))))
                    fallsThrough = true;
            }
        ...
        while (comparisonNode) {
            ...
            if (comp()->useCompressedPointers()) {
                for (int32_t i = 0; i < actualComparisonNode->getNumChildren(); i++) {
                    ...
                    if (objectRef->getOpCode().isLoadIndirect() && objectRef->getDataType() == TR::Address && TR::TransformUtil::fieldShouldBeCompressed(objectRef, comp())) {
                        shouldBeCompressed = true;
                    } else if (objectRef->getOpCode().isArrayLength() && objectRef->getFirstChild()->getOpCode().isLoadIndirect() && TR::TransformUtil::fieldShouldBeCompressed(objectRef->getFirstChild(), comp())) {
...
bool OMR::ValuePropagation::prepareForBlockVersion(TR_LinkHead<ArrayLengthToVersion>* arrayLengths)
{
    ...
    for (TR::Node* bndchkNode = iter.getFirst(); bndchkNode; bndchkNode = iter.getNext()) {
        ...
        if (!(arrayLen->getOpCode().isLoadConst() || (arrayLen->getOpCode().isArrayLength() && (((arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aloadi) && ((arrayLen->getFirstChild()->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload) && (arrayLen->getFirstChild()->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm() && !arrayLen->getFirstChild()->getFirstChild()->getSymbol()->isInternalPointerAuto()))) || ((arrayLen->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload) && arrayLen->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm()))))) {
            continue;
        }
        ...
        for (ArrayIndexInfo* arrayIndexInfo = array->_arrayIndicesInfo->getFirst(); arrayIndexInfo; arrayIndexInfo = arrayIndexInfo->getNext()) {
            ...
            if (((arrayIndexInfo->_baseNode == NULL) && (baseNode != NULL)) || ((arrayIndexInfo->_baseNode != NULL) && (baseNode == NULL)))
        ...
        if (arrayIndexInfoFound) {
            ...
            if (baseNode) {
                ...
                if ((c < arrayIndexInfoFound->_min) && (arrayIndexInfoFound->_max - c > 0)) {
                    ...
                    updateMax = true;
                    arrayIndexInfoFound->_delta += (c - arrayIndexInfoFound->_max);
                } else if ((arrayIndexInfoFound->_min < c && c > arrayIndexInfoFound->_max) || (c == arrayIndexInfoFound->_min || c == arrayIndexInfoFound->_max))
                    removeBndchkFromFastBlock = true;
            }
...
{
    if (node->getOpCode().hasSymbolReference() && !node->hasUnresolvedSymbolReference() && ((node->getOpCode().getOpCodeValue() == TR::iload) || ((node->getOpCode().getOpCodeValue() == TR::iloadi) && ((node->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload) && !((node->getFirstChild())->hasUnresolvedSymbolReference()) && !(_seenDefinedSymbolReferences->isSet(node->getFirstChild()->getSymbolReference()->getReferenceNumber()))))))
        return node;
    else {
        ...
        if (node && node->getOpCode().hasSymbolReference() && !node->hasUnresolvedSymbolReference() && ((node->getOpCode().getOpCodeValue() == TR::iload) || ((node->getOpCode().getOpCodeValue() == TR::iloadi) && ((node->getFirstChild()->getOpCode().getOpCodeValue() == TR::aload) && !((node->getFirstChild())->hasUnresolvedSymbolReference()) && !(_seenDefinedSymbolReferences->isSet(node->getFirstChild()->getSymbolReference()->getReferenceNumber()))))))
        ...
        if (!opCode.isLoadVar() && (opCode.getOpCodeValue() != TR::loadaddr)) {
            ...
            if (opCode.isStore()) {
                ...
                if (opCode.getOpCodeValue() == TR::istore) {
                    ...
                    if (load && symReference->getReferenceNumber() == load->getSymbolReference()->getReferenceNumber()) { //check if this is the first time
...
void OMR::ValuePropagation::addToSortedList(TR_LinkHead<ArrayLengthToVersion>* arrayLengths, ArrayLengthToVersion* newArrayLen)
{
    ...
    for (ArrayLengthToVersion* array = arrayLengths->getFirst(); array; array = array->getNext()) {
        if (array->_arrayLen->getOpCode().isLoadConst() && (newArrayLen->_arrayLen->getInt() > array->_arrayLen->getInt())) {
...
bool OMR::ValuePropagation::checkAllUnsafeReferences(TR::Node* node, vcount_t visitCount)
{
    ...
    if (node->getOpCode().hasSymbolReference() && node->getSymbol()->isUnsafeShadowSymbol()) {
...
void OMR::ValuePropagation::doDelayedTransformations()
{
            ...
            if (callGuarded) {
                if (callNode->getOpCode().isCallIndirect()) {
                    ...
                    while (cursorNode->getOpCodeValue() != TR::BBStart) {
                        ...
                        if (cursorNode->getOpCode().isCall() && cursorNode->isTheVirtualCallNodeForAGuardedInlinedCall()) {
                    ...
                    if (!guardForDifferentCall) {
                        ...
                        if (!guard) {
                            ...
                            if (foldedGuard && (foldedGuard != cfg->getStart()) && (foldedGuard->getSuccessors().size() == 1) && (foldedGuard->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto) && (foldedGuard->getLastRealTreeTop()->getNode()->getBranchDestination() == ci->_block->getEntry()))
                        ...
                        if (guard && (guard->getSuccessors().size() == 2) && guard->getExit()) {
                            ...
                            if (guardNode && guardNode->isProfiledGuard()) {
                                ...
                                if (classTypeNode->getOpCodeValue() == TR::aconst && classTypeNode->isClassPointerConstant()) {
                                    ...
                                    TR_ASSERT((classTypeNode->getOpCodeValue() == TR::aconst && classTypeNode->isMethodPointerConstant()), "Devirtualization of guarded profiled calls only works currently for vft-guards or method compare guards");
                                ...
                                if (typeCompatibleStatus == TR_no) {
                                    // ifacmpne
                                    if ((guardType == 1) && performTransformation(comp(), "%sChanging branch (guard) %p in block_%d to goto due to devirtualization\n", OPT_DETAILS, guardNode, guard->getNumber())) {
                                        // change the if to goto
                                        changeBranchToGoto(this, guardNode, guard);
                                        callUnreachable = false;
                                    } else if ((guardType == 0) && performTransformation(comp(), "%sRemoving branch %p in block_%d due to devirtualization\n", OPT_DETAILS, guardNode, guard->getNumber())) {
                                        // remove branch
                                    ...
                                    // ifacmpne
                                    if ((guardType == 1) && performTransformation(comp(), "%sRemoving branch %p in block_%d due to devirtualization\n", OPT_DETAILS, guardNode, guard->getNumber())) {
                                        // remove branch
                                        guard->removeBranch(comp());
                                        //callUnreachable true;
                                    } else if ((guardType == 0) && performTransformation(comp(), "%sChanging branch (guard) %p in block_%d to goto due to devirtualization\n", OPT_DETAILS, guardNode, guard->getNumber())) {
                                        // change the if to goto
                                        changeBranchToGoto(this, guardNode, guard);
                                        //callUnreachable  true;
                                    }
...
#endif
                {
                    if (comp()->getMethodHotness() <= warm && comp()->getOption(TR_DisableInliningDuringVPAtWarm)) {
                        ...
                        if (!newInlineCall.inlineCall(ci->_tt, ci->_thisType, true, ci->_argInfo, initialMaxSize)) {
                            ...
                            if (!callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isInterpreted() || callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->isJITInternalNative()) {
                                if (callNode->getOpCode().isCallIndirect()) {
    ...
    for (ClassInitInfo* cii = _classesToCheckInit.getFirst(); cii; cii = cii->getNext()) {
        ...
        if ((origFirst->getOpCodeValue() == TR::aload) && origFirst->getSymbol()->isStaticField()) {
            ...
            if ((staticName && (staticNameLen > 0) && (!strncmp(staticName, "com/ibm/websphere/ras/TraceComponent.fineTracingEnabled", 55) || !strncmp(staticName, "com/ibm/ejs/ras/TraceComponent.anyTracingEnabled", 48) || !strncmp(staticName, "java/lang/String.compressionFlag", 32))) || ((cii->_len == 41) && !strncmp(cii->_sig, "Lcom/ibm/websphere/ras/TraceEnabledToken;", cii->_len)) || ((cii->_len == 35) && !strncmp(cii->_sig, "Lcom/ibm/ejs/ras/TraceEnabledToken;", cii->_len)) || ((cii->_len == 40) && !strncmp(cii->_sig, "Ljava/lang/String$StringCompressionFlag;", cii->_len))) {
    ...
    if (!_unsafeArrayAccessNodes->isEmpty()) {
        ...
        while (curTree) {
            if (curTree->getNode() && !checkAllUnsafeReferences(curTree->getNode(), visitCount)) {
                comp()->getSymRefTab()->aliasBuilder.unsafeArrayElementSymRefs().empty();
                break;
            }
