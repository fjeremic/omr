...
extern TR::TreeTop*
createStoresForArraycopyChildren(TR::Compilation* comp,
                                 TR::TreeTop* arrayTree,
...
void
collectArraylengthNodes(TR::Node* node,
                        vcount_t visitCount,
                        List<TR::Node>* arraylengthNodes)
{
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++)
    collectArraylengthNodes(
      node->getChild(childNum), visitCount, arraylengthNodes);
}
...
void
OMR::ValuePropagation::freeRelationship(Relationship* rel)
{
  _relationshipCache.add(rel);
}

void
OMR::ValuePropagation::freeRelationships(TR_LinkHead<Relationship>& list)
{
...
void
OMR::ValuePropagation::freeStoreRelationship(StoreRelationship* rel)
{
...
void
OMR::ValuePropagation::freeStoreRelationships(
...
void
OMR::ValuePropagation::freeValueConstraints(ValueConstraints& valueConstraints)
{
  _vcHandler.empty(valueConstraints);
}

OMR::ValuePropagation::ValueConstraint*
OMR::ValuePropagation::copyValueConstraints(ValueConstraints& valueConstraints)
{
  return _vcHandler.copyAll(valueConstraints);
}

void
OMR::ValuePropagation::addConstraint(TR::VPConstraint* constraint, int32_t hash)
{
...
void
OMR::ValuePropagation::addLoopDef(TR::Node* node)
{
...
static bool
worthPropagatingConstraints(TR::Compilation* comp, bool isGlobalPropagation)
{
...
bool
OMR::ValuePropagation::propagateConstraint(TR::Node* node,
                                           int32_t valueNumber,
                                           Relationship* first,
                                           Relationship* rel,
                                           ValueConstraints* valueConstraints)
{
  ...
  if (_propagationDepth > _maxPropagationDepth) {
    ...
    if (trace())
      traceMsg(comp(),
               "===>Reached Max Relational Propagation Depth: %d\n",
               _propagationDepth);
  ...
  for (cur = first;
    ...
    if (rel->relative == AbsoluteConstraint) {
      ...
      if (c) {
        c = addConstraintToList(
          node, cur->relative, AbsoluteConstraint, c, valueConstraints);
        if (!c)
          return false;
      }
      ...
      if (c) {
        c = addConstraintToList(
          node, rel->relative, AbsoluteConstraint, c, valueConstraints);
        if (!c)
          return false;
      }
    } else {
      ...
      if (c) {
        c = addConstraintToList(
          node, cur->relative, rel->relative, c, valueConstraints);
        if (!c)
          return false;
      }
    }
...
//
TR::VPConstraint*
OMR::ValuePropagation::addConstraintToList(TR::Node* node,
                                           int32_t valueNumber,
                                           int32_t relative,
                                           TR::VPConstraint* constraint,
                                           ValueConstraints* valueConstraints,
                                           bool replaceExisting)
{
          ...
          if (TR::VPConstraint::isSpecialClass(
            ...
            storeConstraint =
              TR::VPClass::create(this,
                                  NULL,
                                  constraint->getClassPresence(),
                                  constraint->getPreexistence(),
                                  constraint->getArrayInfo(),
  ...
  if (newOrChanged) {
    ...
    if (valueNumber < _firstUnresolvedSymbolValueNumber) {
      ...
      if (!propagateConstraint(node,
                               valueNumber,
                               cur->relationships.getFirst(),
                               rel,
                               valueConstraints))
        propFailed = true;
      ...
      if (relPropGlobal) {
        ...
        if (!propFailed && globalConstraint != NULL) {
          ...
          if (!propagateConstraint(
                node, valueNumber, firstReln, rel, valueConstraints))
            propFailed = true;
        }
      }
...
TR::VPConstraint*
OMR::ValuePropagation::addGlobalConstraint(TR::Node* node,
                                           TR::VPConstraint* constraint,
                                           TR::Node* relative)
{
  ...
  return addGlobalConstraint(
    node, getValueNumber(node), constraint, relativeVN);
}
                                           ...
                                           int32_t relative)
{
  ...
  if (trace() && c == NULL)
    traceMsg(comp(),
             "Cannot intersect constraints on %s [%p]",
             node->getOpCode().getName(),
             node);
  ...
  if (newOrChanged) {
    if (trace() && node) {
      traceMsg(comp(),
               "   %s [%p] gets new global constraint:",
               node->getOpCode().getName(),
               node);
      rel->print(this, valueNumber, 1);
    }
...
TR::VPConstraint*
OMR::ValuePropagation::addEdgeConstraint(TR::Node* node,
                                         TR::VPConstraint* constraint,
                                         ...
                                         TR::Node* relative)
{
  ...
  int32_t relativeVN = relative ? getValueNumber(relative) : AbsoluteConstraint;
  constraint = addConstraintToList(
    node, valueNumber, relativeVN, constraint, &constraints->valueConstraints);
...
TR::VPConstraint*
OMR::ValuePropagation::addBlockConstraint(TR::Node* node,
                                          TR::VPConstraint* constraint,
                                          TR::Node* relative,
                                          bool mustBeValid)
{
  ...
  int32_t relativeVN = relative ? getValueNumber(relative) : AbsoluteConstraint;
  constraint = addConstraintToList(
    node, valueNumber, relativeVN, constraint, &_curConstraints);
...
TR::VPConstraint*
OMR::ValuePropagation::addBlockOrGlobalConstraint(TR::Node* node,
                                                  TR::VPConstraint* constraint,
                                                  bool isGlobal,
                                                  TR::Node* relative)
{
...
void
OMR::ValuePropagation::mergeRelationships(TR_LinkHead<Relationship>& fromList,
                                          TR_LinkHead<Relationship>& toList,
                                          int32_t valueNumber,
                                          bool preserveFrom,
                                          StoreRelationship* mergingStore,
                                          List<TR::Symbol>* storeSymbols,
                                          bool inBothLists)
{
  if (trace())
    traceMsg(
      comp(), "Merging relationships for value number: %i\n", valueNumber);
...
void
OMR::ValuePropagation::mergeStoreRelationships(ValueConstraint* fromvc,
...
void
OMR::ValuePropagation::mergeValueConstraints(ValueConstraint* fromvc,
                                             ValueConstraint* tovc,
                                             bool preserveFrom)
{
  // First merge the non-store relationships.
  //
  mergeRelationships(fromvc->relationships,
                     tovc->relationships,
                     fromvc->getValueNumber(),
                     preserveFrom);
...
void
OMR::ValuePropagation::mergeEdgeConstraints(EdgeConstraints* fromEdge,
...
void
OMR::ValuePropagation::mergeConstraintIntoEdge(ValueConstraint* constraint,
...
void
OMR::ValuePropagation::mergeBackEdgeConstraints(EdgeConstraints* edge)
{
...
//
static bool
reduceCostOfAnalysisByTreatingBlockConservatively(TR::Block* b)
{
  return b->isOSRCatchBlock();
}

void
OMR::ValuePropagation::createExceptionEdgeConstraints(
  uint32_t exceptions,
  ValueConstraint* extraConstraint,
  TR::Node* reason)
{
  if (!_isGlobalPropagation)
    return;
  ...
  for (auto edge = _curBlock->getExceptionSuccessors().begin();
       edge != _curBlock->getExceptionSuccessors().end();
       ++edge) {
    ...
    if (trace())
      traceMsg(comp(),
               "   %s [%p] can throw exception to block_%d\n",
...
void
OMR::ValuePropagation::removeConstraint(int32_t valueNumber,
                                        ValueConstraints& valueConstraints,
                                        int32_t relative)
{
...
OMR::ValuePropagation::Relationship*
OMR::ValuePropagation::findValueConstraint(int32_t valueNumber,
                                           ValueConstraints& valueConstraints,
                                           int32_t relative)
{
  // Find the constraint for the given value number in the given value
...
OMR::ValuePropagation::Relationship*
OMR::ValuePropagation::findEdgeConstraint(int32_t valueNumber,
                                          EdgeConstraints* edge,
                                          int32_t relative)
{
...
TR::VPConstraint*
OMR::ValuePropagation::applyGlobalConstraints(TR::Node* node,
                                              int32_t valueNumber,
                                              TR::VPConstraint* constraint,
                                              int32_t relative)
{
...
OMR::ValuePropagation::getStoreConstraint(TR::Node* node, TR::Node* relative)
{
  ...
  if (rel) {
    if (trace()) {
      traceMsg(comp(),
               "   %s [%p] has existing store constraint:",
               node->getOpCode().getName(),
               node);
      rel->print(this, valueNumber, 1);
    }
...
void
OMR::ValuePropagation::createStoreConstraints(TR::Node* node)
{
...
bool
OMR::ValuePropagation::hasBeenStored(int32_t valueNumber,
                                     TR::Symbol* symbol,
                                     ValueConstraints& valueConstraints)
{
...
void
OMR::ValuePropagation::setUnreachableStore(StoreRelationship* store)
{
...
bool
OMR::ValuePropagation::isUnreachableStore(StoreRelationship* store)
{
...
bool
OMR::ValuePropagation::isDefInUnreachableBlock(int32_t defIndex)
{
...
TR_YesNoMaybe
OMR::ValuePropagation::isCastClassObject(TR::VPClassType* type)
{
...
void
OMR::ValuePropagation::checkTypeRelationship(TR::VPConstraint* lhs,
                                             TR::VPConstraint* rhs,
                                             int32_t& value,
                                             bool isInstanceOf,
                                             bool isCheckCast)
...
void
OMR::ValuePropagation::invalidateParmConstraintsIfNeeded(
  TR::Node* node,
  TR::VPConstraint* constraint)
{
  if (_isGlobalPropagation)
...
TR::VPConstraint*
OMR::ValuePropagation::mergeDefConstraints(TR::Node* node,
                                           int32_t relative,
                                           bool& isGlobal,
                                           bool forceMerge)
{
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (defIndex < _useDefInfo->getFirstRealDefIndex()) {
      if (trace())
        traceMsg(comp(),
                 "   %s [%p] looking at method entry def point: ",
                 node->getOpCode().getName(),
                 node);
      ...
      if (trace())
        traceMsg(comp(),
                 "   %s [%p] looking at def value %d, %s [%p]: ",
                 node->getOpCode().getName(),
                 node,
                 getValueNumber(defNode),
                 defNode->getOpCode().getName(),
                 defNode);
      ...
      if (defNode->getOpCode().isStore()) {
        ...
        if (matchingDef) {
          ...
          if (defValueConstraint) {
            rel =
              findConstraintInList(defValueConstraint->relationships, relative);
            if (rel) {
  ...
  if (!bigDecimalConstraints.isEmpty()) {
    ...
    if (constraint && mergeDeferredConstraints) {
      ...
      for (c = cIt.getFirst(); c; c = cIt.getNext()) {
        if (trace())
          traceMsg(
            comp(), " doing late merging constraint at def %p\n", defNode);
  ...
  if (!lastTimeThrough()) {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (defNode) {
        defValueNumber = getValueNumber(defNode);
        if (hasBeenStored(
              defValueNumber, defNode->getSymbol(), _curConstraints))
          addLoopDef(defNode);
      }
    }
    return NULL;
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (node->getSymbol()->isAutoOrParm() && !unseenDefsFound &&
      ...
      for (loopInfo = _loopInfo; loopInfo && loopInfo != container;
        ...
        if (!hasBeenStored(defValueNumber,
                           sym,
                  ...
                  if ((uint32_t)defConstraint->getHighInt() <
                    ...
                    if ((uint32_t)defConstraint->getLowInt() <
                      ...
                      TR::VPConstraint* newDefConstraint =
                        TR::VPIntRange::create(this,
                                               constraint->getLowInt(),
                                               defConstraint->getHighInt());
                      defConstraint =
...
void
OMR::ValuePropagation::replaceByConstant(TR::Node* node,
                                         TR::VPConstraint* constraint,
                                         bool isGlobal)
{
  if (isGlobal)
  ...
  if (!performTransformation(comp(),
                             "%sConstant folding %s [" POINTER_PRINTF_FORMAT
                             "]",
                             OPT_DETAILS,
                             node->getOpCode().getName(),
                             node))
  ...
  switch (type) {
    ...
    case TR::Int64:
      ...
      dumpOptDetails(
        comp(), " to lconst " INT64_PRINTF_FORMAT "\n", node->getLongInt());
      break;
    case TR::Float:
    ...
    case TR::Address:
      ...
      dumpOptDetails(comp(),
                     " to aconst " UINT64_PRINTF_FORMAT_HEX "\n",
...
void
OMR::ValuePropagation::removeRestOfBlock()
{
...
void
OMR::ValuePropagation::mustTakeException()
{
  ...
  if (!performTransformation(comp(),
                             "%sRemoving rest of block after %s [%p]\n",
                             OPT_DETAILS,
  ...
  for (auto edge = _curBlock->getSuccessors().begin();
       edge != _curBlock->getSuccessors().end();
       ++edge) {
...
bool
OMR::ValuePropagation::registerPreXClass(TR::VPConstraint* constraint)
{
...
TR::Node*
OMR::ValuePropagation::findThrowInBlock(TR::Block* block, TR::TreeTop*& treeTop)
{
...
bool
OMR::ValuePropagation::isHighWordZero(TR::Node* node)
{
...
void
OMR::ValuePropagation::checkForInductionVariableIncrement(TR::Node* node)
{
  ...
  if (child->getOpCode().isSub()) {
    if (increment->asIntConst())
      ...
      increment =
        TR::VPShortConst::create(this, -increment->asShortConst()->getShort());
    else
      increment =
  ...
  if (trace())
    traceMsg(comp(),
             "   %s [%p] may be induction variable [%p]\n",
             node->getOpCode().getName(),
             node,
             sym);
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if ((defVN != incrementVN) && !notInLoop) {
          ...
          if (trace())
            traceMsg(
              comp(),
              "      ivInfo is invalid for sym [%p], def [%d] node %p does not "
              "have the same value number as another increment\n",
              sym,
              defVN,
              defNode);
          break;
  ...
  if (iv) {
    ...
    if (maybeInductionVariable) {
      if (checkLoopTestBlock(sym)) {
        ...
        if (trace())
          traceMsg(
            comp(), "      Guessed increment of the iv for sym [%p]\n", sym);
      } else if (trace())
        traceMsg(comp(),
...
void
OMR::ValuePropagation::checkForInductionVariableLoad(TR::Node* node)
{
...
void
OMR::ValuePropagation::collectInductionVariableEntryConstraints()
{
...
bool
OMR::ValuePropagation::checkLoopTestBlock(TR::Symbol* sym)
{
  ...
  if (entry) {
    for (auto edge = entryNode->getSuccessors().begin();
         edge != entryNode->getSuccessors().end();
         ++edge) {
...
void
OMR::ValuePropagation::checkBackEdgeCoverage()
{
...
void
OMR::ValuePropagation::setUpInductionVariables(TR_StructureSubGraphNode* node)
{
  ...
  while ((ivInfo = _loopInfo->_inductionVariables.pop())) {
    ...
    if (ivInfo->_symbol->getType().isInt32()) {
      ...
      if (ivInfo->_increment->asIntConst())
        incr =
          new (trHeapMemory()) TR::VPIntConst(ivInfo->_increment->getLowInt());
      else
      ...
      if (ivInfo->_increment->asLongConst())
        ...
        incr = new (trHeapMemory()) TR::VPLongRange(
          ivInfo->_increment->getLowLong(), ivInfo->_increment->getHighLong());
      if (exitConstraint) {
        if (exitConstraint->asLongConst())
          exit =
            new (trHeapMemory()) TR::VPLongConst(exitConstraint->getLowLong());
        else
...
void
OMR::ValuePropagation::collectBackEdgeConstraints()
{
...
TR::GlobalValuePropagation::perform()
{
  ...
  if (!optimizer()->getUseDefInfo()) {
    dumpOptDetails(
      comp(),
      "Can't do Global Value Propagation - no use/def info for %s\n",
      comp()->signature());
    return 0;
  }
  ...
  if (!optimizer()->getValueNumberInfo()) {
    dumpOptDetails(
      comp(),
      "Can't do Global Value Propagation - no value numbers for %s\n",
      comp()->signature());
    return 0;
  }
  ...
  if (skipBlocksThatCannotReachNonColdBlocks) {
    _blocksToProcess = new (trStackMemory())
      TR_BitVector(comp()->getFlowGraph()->getNumberOfNodes(),
                   comp()->trMemory(),
                   stackAlloc,
                   notGrowable,
                   TR_MemoryBase::ValuePropagation);
...
void
OMR::ValuePropagation::getParmValues()
{
  return;
}

bool
OMR::ValuePropagation::isParmInvariant(TR::Symbol* sym)
{
...
bool
TR::GlobalValuePropagation::buildInputConstraints(TR::CFGNode* node)
{
...
void
TR::GlobalValuePropagation::propagateOutputConstraints(
  TR_StructureSubGraphNode* node,
  bool lastTimeThrough,
  bool isNaturalLoop,
  List<TR::CFGEdge>& outEdges1,
  List<TR::CFGEdge>* outEdges2)
{
...
void
TR::GlobalValuePropagation::determineConstraints()
{
...
void
TR::GlobalValuePropagation::processStructure(TR_StructureSubGraphNode* node,
                                             bool lastTimeThrough,
                                             bool insideLoop)
{
...
void
TR::GlobalValuePropagation::processAcyclicRegion(TR_StructureSubGraphNode* node,
                                                 bool lastTimeThrough,
                                                 bool insideLoop)
{
  if (trace())
...
void
TR::GlobalValuePropagation::processNaturalLoop(TR_StructureSubGraphNode* node,
                                               bool lastTimeThrough,
                                               bool insideLoop)
{
  TR_RegionStructure* region = node->getStructure()->asRegion();
  ...
  if (trace()) {
    traceMsg(comp(),
             "   defined on all paths for entry of loop %d",
             region->getNumber());
  ...
  for (TR::CFGEdge* edge = it.getFirst(); edge; edge = it.getNext()) {
    if (trace())
      traceMsg(comp(),
               "   defined on all paths for exit %d->%d:",
...
void
TR::GlobalValuePropagation::processImproperLoop(TR_StructureSubGraphNode* node,
                                                bool lastTimeThrough,
                                                bool insideLoop)
{
  if (trace())
  ...
  */
  ...
  //
  propagateOutputConstraints(
    node, lastTimeThrough, false, region->getExitEdges(), NULL);
...
void
OMR::ValuePropagation::generalizeStores(ValueConstraints& stores,
                                        ValueConstraints* vC)
{
...
void
OMR::ValuePropagation::findStoresInBlock(TR::Block* block,
                                         ValueConstraints& stores)
{
...
void
TR::GlobalValuePropagation::getImproperRegionStores(
  TR_StructureSubGraphNode* node,
  ValueConstraints& stores)
{
  TR_RegionStructure* region = node->getStructure()->asRegion();
...
void
TR::GlobalValuePropagation::processRegionSubgraph(
  TR_StructureSubGraphNode* node,
  bool lastTimeThrough,
  bool insideLoop,
  bool isNaturalLoop)
{
  ...
  propagateOutputConstraints(
    node, lastTimeThrough, isNaturalLoop, region->getExitEdges(), NULL);
}

void
TR::GlobalValuePropagation::processRegionNode(TR_StructureSubGraphNode* node,
                                              bool lastTimeThrough,
                                              bool insideLoop)
{
...
TR_BitVector*
TR::GlobalValuePropagation::mergeDefinedOnAllPaths(
  TR_StructureSubGraphNode* node)
{
  ...
  for (auto itr = node->getPredecessors().begin(),
    ...
    if (trace()) {
      traceMsg(comp(),
               "   inbound seenOnAllpaths for edge %d->%d",
...
void
TR::GlobalValuePropagation::processBlock(TR_StructureSubGraphNode* node,
                                         bool lastTimeThrough,
                                         bool insideLoop)
{
  TR::CFGEdge* edge;
  ...
  if (_blocksToProcess && !_blocksToProcess->isSet(block->getNumber())) {
    if (performTransformation(comp(),
                              "%sSkip block_%d deemed unimportant\n",
                              OPT_DETAILS,
                              block->getNumber()))
      return;
  }
  ...
  if (insideLoop && trace()) {
    traceMsg(comp(),
             "   defined on all paths for entry of block %d",
             block->getNumber());
  ...
  if (comp()->getStartBlock()->getNumber() == _curBlock->getNumber()) {
    addConstraintToList(NULL,
                        _syncValueNumber,
                        AbsoluteConstraint,
                        TR::VPSync::create(this, TR_no),
                        &_curConstraints);
  }
  ...
  if (insideLoop) {
    for (auto itr = node->getSuccessors().begin(),
      ...
      if (trace()) {
        traceMsg(comp(),
                 "   outbound seenOnAllpaths for edge %d->%d",
                 (*itr)->getFrom()->getNumber(),
                 (*itr)->getTo()->getNumber());
        _curDefinedOnAllPaths->print(comp());
        traceMsg(comp(), "\n");
  ...
  if (!isUnreachablePath(_curConstraints)) {
    ...
    if (trace())
      traceMsg(comp(),
               ...
               endTree->getNode()->getOpCode().getName(),
               endTree->getNode());
  ...
  if (hasIncomingStoreRelationships) {
    TR::Block* nextBlock = endTree->getNextTreeTop()
  ...
  for (auto iter = _curBlock->getSuccessors().begin();
       iter != _curBlock->getSuccessors().end();
       ++iter)
    List1.add(*iter);
  for (auto iter = _curBlock->getExceptionSuccessors().begin();
       iter != _curBlock->getExceptionSuccessors().end();
       ++iter)
    List2.add(*iter);
...
TR::CFGEdge*
OMR::ValuePropagation::findOutEdge(TR::CFGEdgeList& edges, TR::CFGNode* target)
{
...
bool
OMR::ValuePropagation::isUnreachablePath(ValueConstraints& valueConstraints)
{
...
bool
OMR::ValuePropagation::isUnreachablePath(EdgeConstraints* constraints)
{
  return isUnreachablePath(constraints->valueConstraints);
}

void
OMR::ValuePropagation::setUnreachablePath()
{
#if 0
   freeValueConstraints(_curConstraints);
...
void
OMR::ValuePropagation::setUnreachablePath(ValueConstraints& vc)
{
  freeValueConstraints(vc);
  addConstraintToList(
    NULL, 0, AbsoluteConstraint, TR::VPUnreachablePath::create(this), &vc);
}

void
OMR::ValuePropagation::setUnreachablePath(TR::CFGEdge* edge)
{
  if (!_isGlobalPropagation)
    return;
...
void
OMR::ValuePropagation::printStructureInfo(TR_Structure* s,
                                          bool starting,
                                          bool lastTimeThrough)
{
...
void
OMR::ValuePropagation::printParentStructure(TR_Structure* s)
{
  if (s->getParent()) {
...
void
OMR::ValuePropagation::printValueConstraints(ValueConstraints& valueConstraints)
{
...
void
OMR::ValuePropagation::printGlobalConstraints()
{
...
void
OMR::ValuePropagation::printEdgeConstraints(EdgeConstraints* constraints)
{
...
void
OMR::ValuePropagation::Relationship::print(OMR::ValuePropagation* vp)
{
...
void
OMR::ValuePropagation::StoreRelationship::print(OMR::ValuePropagation* vp,
                                                int32_t valueNumber,
                                                int32_t indent)
{
  if (vp->comp()->getOutFile() == NULL)
  ...
  if (relationships.getFirst())
    for (Relationship* rel = relationships.getFirst(); rel;
         rel = rel->getNext()) {
      trfprintf(
        vp->comp()->getOutFile(), "%*.ssymbol %p store", indent, " ", symbol);
...
void
OMR::ValuePropagation::ValueConstraint::print(OMR::ValuePropagation* vp,
                                              int32_t indent)
{
  if (vp->comp()->getOutFile() == NULL)
    return;
...
//
bool
OMR::ValuePropagation::removeConstraints()
{
...
bool
OMR::ValuePropagation::removeConstraints(int32_t valueNumber,
                                         ValueConstraints* vc,
                                         bool findStores)
{
  if (trace()) {
  ...
  if (findStores) {
    if (trace())
      traceMsg(comp(),
               "   block following block_%d is unreachable\n",
               _curBlock->getNumber());
    setUnreachablePath();
  }
...
bool
OMR::ValuePropagation::removeStoreConstraints(
  ValueConstraints* valueConstraints,
  int32_t valueNumber,
  int32_t relative)
{
...
TR::TreeTop*
TR::ArraycopyTransformation::createArrayNode(TR::TreeTop* tree,
                                             TR::TreeTop* newTree,
                                             TR::SymbolReference* srcRef,
                                             TR::SymbolReference* dstRef,
                                             TR::SymbolReference* lenRef,
                                             ...
                                             bool isForward)
{
  ...
  return createArrayNode(
    tree, newTree, srcRef, dstRef, len, srcObjRef, dstObjRef, isForward);
}
                                             ...
                                             bool isForward)
{
  ...
  if (!root->isReferenceArrayCopy()) {
    ...
    if (trace())
      traceMsg(comp(),
               "Created 3-child arraycopy %s from root node %s, ",
    ...
    if (trace())
      traceMsg(comp(),
               "Created 5-child arraycopy %s from root node %s, ",
               comp()->getDebug()->getName(node),
               comp()->getDebug()->getName(root));
  }
...
int64_t
TR::ArraycopyTransformation::arraycopyHighFrequencySpecificLength(
...
TR::TreeTop*
TR::ArraycopyTransformation::createPointerCompareNode(
  TR::Node* node,
  TR::SymbolReference* srcRef,
  TR::SymbolReference* dstRef)
{
...
TR::TreeTop*
TR::ArraycopyTransformation::createRangeCompareNode(TR::Node* node,
                                                    TR::SymbolReference* srcRef,
                                                    TR::SymbolReference* dstRef,
                                                    TR::SymbolReference* lenRef)
{
  ...
  if (is64Bit) {
    if (len->getType().isInt32())
      srcEndNode =
        TR::Node::create(TR::aladd, 2, src, TR::Node::create(TR::i2l, 1, len));
    else
                                                      ...
                                                      TR::Node* node)
{
  ...
  if (!alreadyForwardArrayCopy) {
    ...
    cfg->addEdge(TR::CFGEdge::createEdge(
      outerElseBlock, forwardArrayCopyBlock, trMemory()));
    cfg->addEdge(TR::CFGEdge::createEdge(
      arraycopyBlock, forwardArrayCopyBlock, trMemory()));
    cfg->addEdge(TR::CFGEdge::createEdge(
      forwardArrayCopyBlock, followOnBlock, trMemory()));
    cfg->copyExceptionSuccessors(ifBlock, forwardArrayCopyBlock);

    cfg->removeEdge(outerElseBlock->getSuccessors(),
                    outerElseBlock->getNumber(),
                    followOnBlock->getNumber());
    cfg->removeEdge(arraycopyBlock->getSuccessors(),
                    arraycopyBlock->getNumber(),
                    ifBlock->getNumber());
  ...
  if (specificLength >= 0)
    specializeForLength(outerArraycopyTree,
                        node,
                        (uintptrj_t)specificLength,
                        srcRef,
                        dstRef,
                        lenRef,
                        srcObjRef,
                                                      ...
                                                      TR::Node* arraycopyNode)
{
  ...
  if (specificLength >= 0 &&
      performTransformation(
        comp(),
        "%sSpecializing arraycopy %s for length of %d bytes\n",
        OPT_DETAILS,
        comp()->getDebug()->getName(arraycopyNode),
        specificLength)) {
    TR::SymbolReference* srcObjRef = NULL;
    TR::SymbolReference* dstObjRef = NULL;
    TR::SymbolReference* srcRef = NULL;
...
static TR::Node*
addressSizedConst(TR::Compilation* comp, TR::Node* n, intptrj_t val)
{
...
TR::TreeTop*
TR::ArraycopyTransformation::specializeForLength(TR::TreeTop* tt,
                                                 TR::Node* arraycopyNode,
                                                 ...
                                                 TR::SymbolReference* dstObjRef)
{
  ...
  createArrayNode(
    tt, arraycopyOriginal, srcRef, dstRef, lenRef, srcObjRef, dstObjRef, true);
  ...
  int32_t specializedBlockFrequency =
    TR::Block::getScaledSpecializedFrequency(specificPathBlock->getFrequency());
...
int32_t
TR::ArraycopyTransformation::perform()
{
...
void
OMR::ValuePropagation::createNewBlockInfoForVersioning(TR::Block* block)
{
...
OMR::ValuePropagation::versionBlocks()
{
  ...
  for (cur = _blocksToBeVersioned->getFirst(); cur; cur = cur->getNext()) {
    ...
    for (auto edge = lastBlock->getSuccessors().begin();
         edge != lastBlock->getSuccessors().end();
         ++edge) {
    ...
    for (auto edge = lastBlock->getExceptionSuccessors().begin();
         edge != lastBlock->getExceptionSuccessors().end();
         ++edge) {
      TR::Block* succ = toBlock((*edge)->getTo());
      _cfg->addEdge(
        ...
        for (int32_t i = 0; i < actualComparisonNode->getNumChildren(); i++) {
          ...
          if (shouldBeCompressed) {
            TR::TreeTop* translateTT = TR::TreeTop::create(
              comp(),
              TR::Node::createCompressedRefsAnchor(objectRef),
              NULL,
              NULL);
      ...
      if (nextPredBlock != _cfg->getStart()) {
        ...
        if (lastTreeInPred)
          lastTreeInPred->adjustBranchOrSwitchTreeTop(
            comp(), block->getEntry(), chooserBlock->getEntry());
      }
    }
...
bool
OMR::ValuePropagation::prepareForBlockVersion(
  TR_LinkHead<ArrayLengthToVersion>* arrayLengths)
{
  bool isGlobal;
  TR_BitVector invariantVariables(
    comp()->getSymRefCount(), trMemory(), stackAlloc);
  ...
  for (TR::Node* bndchkNode = iter.getFirst(); bndchkNode;
        ...
        if (symRef) {
          ...
          if (baseNode) // invarinat or trackableNonInvariant
          {
            ...
            if (bndchkNode->getSecondChild() == baseNode)
              ...
              if (getConstraint(
                    bndchkNode->getSecondChild(), isGlobal, baseNode))
                c = getConstraint(
                      bndchkNode->getSecondChild(), isGlobal, baseNode)
                      ->asEqual()
                      ->increment();
    ...
    if (!array) {
      createNewBucketForArrayIndex(
        array, arrayLengths, c, baseNode, bndchkNode, instanceOfClass);
      continue;
    }
...
void
OMR::ValuePropagation::buildBoundCheckComparisonNodes(
  BlockVersionInfo* blockInfo,
  List<TR::Node>* comparisonNodes)
{
      ...
      if (performTransformation(comp(),
        ...
        if (arrayIndex->_baseNode) {
          ...
          maxIndex = TR::Node::create(
            TR::iadd,
            2,
            arrayIndex->_baseNode->duplicateTree(),
            TR::Node::create(
              arrayLength->_arrayLen, TR::iconst, 0, arrayIndex->_max));
          minIndex = TR::Node::create(
            TR::iadd,
            2,
            arrayIndex->_baseNode->duplicateTree(),
            TR::Node::create(
              arrayLength->_arrayLen, TR::iconst, 0, arrayIndex->_min));
        } else {
          maxIndex = TR::Node::create(
            arrayLength->_arrayLen, TR::iconst, 0, arrayIndex->_max);
          minIndex = TR::Node::create(
        ...
        if (arrayLength->_instanceOfClass && objectRefChild &&
          ...
          TR::Node* duplicateClassPtr = TR::Node::createWithSymRef(
            ...
            comp()->getSymRefTab()->findOrCreateClassSymbol(
              objectRef->getSymbolReference()->getOwningMethodSymbol(comp()),
              -1,
              arrayLength->_instanceOfClass,
              false));
...
void
OMR::ValuePropagation::removeBndChecksFromFastVersion(
  BlockVersionInfo* blockInfo)
{
    ...
    for (ArrayIndexInfo* index = array->_arrayIndicesInfo->getFirst(); index;
      ...
      if (!index->_bndChecks->isEmpty() && !index->_bndChecks->isSingleton()) {
        ...
        for (TR::Node* bndchk = iter.getFirst(); bndchk;
             bndchk = iter.getNext()) {
          dumpOptDetails(
            comp(), "blockVersioner: removing bndchk %p\n", bndchk);
...
TR::Node*
OMR::ValuePropagation::findVarOfSimpleFormOld(
...
TR::Node*
OMR::ValuePropagation::findVarOfSimpleForm(
  TR::Node* node) // ArrayIndexNodeFromOffset(TR::Node *node, int32_t stride)
{
  if (node->getOpCode().hasSymbolReference() &&
      !node->hasUnresolvedSymbolReference() &&
...
void
OMR::ValuePropagation::createNewBucketForArrayIndex(
...
void
OMR::ValuePropagation::collectDefSymRefs(TR::Node* node, TR::Node* parent)
{
  if (!node)
    return;
  TR::ILOpCode& opCode = node->getOpCode();
          ...
          if (load &&
              symReference->getReferenceNumber() ==
                load->getSymbolReference()
                  ->getReferenceNumber()) { // check if this is the first time
...
void
OMR::ValuePropagation::addToSortedList(
  TR_LinkHead<ArrayLengthToVersion>* arrayLengths,
...
TR::TreeTop*
OMR::ValuePropagation::createReferenceArrayNodeWithoutFlags(
  TR::TreeTop* tree,
  TR::TreeTop* newTree,
  TR::SymbolReference* srcObjectRef,
  TR::SymbolReference* dstObjectRef,
  TR::SymbolReference* lenRef,
  TR::SymbolReference* srcRef,
  TR::SymbolReference* dstRef,
...
void
OMR::ValuePropagation::transformReferenceArrayCopyWithoutCreatingStoreTrees(
  ...
  TR::SymbolReference* lenRef)
{
  ...
  arraycopyBlock->createConditionalBlocksBeforeTree(arrayTree->_treetop,
                                                    ifTree,
                                                    arraycopyArrayStoreCheck,
                                                    arraycopyNoArrayStoreCheck,
                                                    cfg,
  ...
  TR_TreeTopWrtBarFlag* arrayTree)
{
  ...
  createStoresForArraycopyChildren(
    comp(), arrayTree->_treetop, srcObjRef, dstObjRef, srcRef, dstRef, lenRef);
...
void
OMR::ValuePropagation::transformUnknownTypeArrayCopy(
  TR_TreeTopWrtBarFlag* arrayTree)
{
#ifdef J9_PROJECT_SPECIFIC
  TR::Node* node = arrayTree->_treetop->getNode();
...
static void
changeBranchToGoto(OMR::ValuePropagation* vp,
                   TR::Node* guardNode,
                   TR::Block* guard)
{
 ...
 */
void
OMR::ValuePropagation::constrainRecognizedMethod(TR::Node* node)
{}
 ...
 */
bool
OMR::ValuePropagation::transformDirectLoad(TR::Node* node)
{
  return TR::TransformUtil::transformDirectLoad(comp(), node);
}

bool
OMR::ValuePropagation::checkAllUnsafeReferences(TR::Node* node,
                                                vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
    return true;
...
OMR::ValuePropagation::doDelayedTransformations()
{
  ...
  for (scalarizedArrayCopy = treesIt1.getFirst(); scalarizedArrayCopy;
    ...
    TR::TransformUtil::scalarizeArrayCopy(comp(),
                                          scalarizedArrayCopy->_node,
                                          scalarizedArrayCopy->_treetop,
                                          true,
  ...
  if (_edgesToBeRemoved) {
    for (i = _edgesToBeRemoved->size() - 1; i >= 0; --i) {
      ...
      if (std::find(edge->getTo()->getPredecessors().begin(),
        ...
        if (trace())
          traceMsg(comp(),
                   "Removing unreachable edge from %d to %d\n",
                   edge->getFrom()->getNumber(),
                   edge->getTo()->getNumber());
    ...
    for (multiLeafCall = iter.getCurrent(); multiLeafCall != NULL;
      ...
      if (!multiLeafInlineCall.inlineCall(multiLeafCall))
        performTransformation(comp(),
                              "%s WARNING: Inlining of %p failed\n",
                              OPT_DETAILS,
            ...
            if (guard && (guard->getSuccessors().size() == 2) &&
              ...
              if (guardNode) {
                ...
                canBeRemoved =
                  ...
                  callNode->getOpCode()
                    .isCallDirect(); // do not remove guards for indirect calls.
                                     // If the ifacmpne handler didn't do it, the
                                     // chances are it is illegal to remove the
                                     // guard
              } else
                canBeRemoved = true;
              ...
              if (guardNode && guardNode->isProfiledGuard()) {
                ...
                if (classTypeNode->getOpCodeValue() == TR::aconst &&
                  ...
                  TR_YesNoMaybe result =
                    comp()->fe()->isInstanceOf(typeClass, receiverClass, true);
                ...
                if (typeCompatibleStatus == TR_no) {
                  ...
                  if ((guardType == 1) &&
                      performTransformation(
                        comp(),
                        "%sChanging branch (guard) %p in block_%d to goto due "
                        "to devirtualization\n",
                        OPT_DETAILS,
                        guardNode,
                        guard->getNumber())) {
      ...
      if (!callGuarded || !callUnreachable) {
...
#endif
        {
          if (comp()->getMethodHotness() <= warm &&
            ...
            if (!newInlineCall.inlineCall(
                  ci->_tt, ci->_thisType, true, ci->_argInfo, initialMaxSize)) {
  ...
  for (VirtualGuardInfo* cvg = _convertedGuards.getFirst(); cvg;
    ...
    if (!oldNode ||
        !performTransformation(comp(),
                               "%sReplacing the old guard %p with the shiny "
                               "new overridden guard %p at treetop %p\n",
                               oldNode,
                               cvg->_newGuardNode,
                               cvg->_currentTree)) {
      continue;
  ...
  for (predictedThrow = throwsIt.getFirst(); predictedThrow;
    ...
    if (treeTop->getNode()->getOpCodeValue() == TR::NULLCHK) {
      ...
      treeTop = TR::TreeTop::create(
        ...
        TR::Node::createWithSymRef(
          TR::astore, 1, 1, node->getFirstChild(), excpSymRef));
    } else {
    ...
    TR_HandleInjectedBasicBlock hibb(
      comp(), NULL, currentSymbol, l1, l2, l3, 0);
    hibb.findAndReplaceReferences(
      predictedCatchBlock->getEntry(), gotoDestination, 0);
    ...
    if ((origFirst->getOpCodeValue() == TR::aload) &&
      ...
      staticName =
        origFirst->getSymbolReference()->getOwningMethod(comp())->staticName(
          origFirst->getSymbolReference()->getCPIndex(),
          staticNameLen,
          comp()->trMemory());
      ...
      if ((staticName && (staticNameLen > 0) &&
           (!strncmp(staticName,
                     ...
                     55) ||
            !strncmp(staticName,
                     "com/ibm/ejs/ras/TraceComponent.anyTracingEnabled",
                     48) ||
    ...
    if (!recognizedStatic) {
      ...
      TR::SymbolReference* newSymbolReference =
        comp()->getSymRefTab()->createTemporary(
          comp()->getMethodSymbol(), dataType, false, 0);
...
void
constrainRangeByPrecision(const int64_t low,
                          const int64_t high,
...
#if USE_TREES
void
OMR::ValuePropagation::ValueConstraintHandler::setVP(OMR::ValuePropagation* vp)
{
  _vp = vp;
}
...
void
OMR::ValuePropagation::ValueConstraintHandler::free(ValueConstraint* vc)
{
...
TR::Compilation*
OMR::ValuePropagation::ValueConstraintHandler::comp()
{
  return _vp->comp();
}
...
void
OMR::ValuePropagation::ValueConstraintHandler::empty(
  ValueConstraints& valueConstraints)
{
  ValueConstraint* vc;
...
void
OMR::ValuePropagation::ValueConstraintHandler::setRoot(ValueConstraints& list,
                                                       ValueConstraint* vc)
{
  list.setFirst(vc);
}
...
OMR::ValuePropagation::VirtualGuardInfo::VirtualGuardInfo(
  OMR::ValuePropagation* vp,
  TR_VirtualGuard* vgOld,
  TR_VirtualGuard* vgNew,
  TR::Node* newGNode,
  TR::Node* cn)
