...
public:
  ...
  bool setIsAnchored(bool b = true)
  {
    return _isAnchored = b;
  }

  TR::TreeTop* getAnchorLocation()
  {
  ...
  TR::Node* setNode(TR::Node* n)
  {
    return _node = n;
  }

  int32_t getAnchorBlockNumber()
  {
    return _anchorBlockNumber;
  }
  int32_t setAnchorBlockNumber(int32_t b)
  {
    return _anchorBlockNumber = b;
  }
...
public:
  ...
  bool setIsSatisfied(bool b = true)
  {
    return _isSatisfied = b;
  }
...
public:
  ...
  TR::TreeTop* getAnchorTree()
  {
    return _anchorTree;
  }
  TR::TreeTop* setAnchorTree(TR::TreeTop* t)
  {
    return _anchorTree = t;
  }

  TR::Block* getAnchorBlock()
  {
    return _anchorBlock;
  }
  TR::Block* setAnchorBlock(TR::Block* b)
  {
    return _anchorBlock = b;
  }
...
public:
  ...
  bool
    _needsDuplication; // has the store already been duplicated by sinkStorePlacement or should it be done in doSinking
};
...
private:
  virtual bool storeIsSinkingCandidate(TR::Block* block, TR::Node* node, int32_t symIdx, bool sinkIndirectLoads,
    uint32_t& indirectLoadCount, int32_t& depth, bool& isLoadStatic, vcount_t& treeVisitCount, vcount_t& highVisitCount)
    = 0;
...
protected:
  ...
  bool treeIsSinkableStore(TR::Node* node, bool sinkIndirectLoads, uint32_t& indirectLoadCount, int32_t& depth,
  ...
  uint32_t insertAnchoredNodes(TR_MovableStore* store, TR::Node* nodeCopy, TR::Node* nodeOrig, TR::Node* nodeParentCopy,
    int32_t childNum, TR_BitVector* needTempForCommonedLoads, TR_BitVector* blockingUsedSymbols,
