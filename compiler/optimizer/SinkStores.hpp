 ...
 public:
  ...
  TR_FirstUseOfLoad(TR::Node *node, TR::TreeTop *anchorLocation,
      ...
      : _node(node),
        ...
        _anchorBlockNumber(anchorBlockNumber),
        _isAnchored(false) {}
 ...
 public:
  ...
  TR_MovableStore
      *_movableStore;  // all TR_MovableStore's are also TR_UseOrKillInfo's so
                       // this is a pointer to the corresponding movableStore
                       // (if one exists)
};
...
class TR_MovableStore {
 public:
  ...
  int32_t _symIdx;
  TR_BitVector
      *_commonedLoadsUnderTree;  // commoned used syms under this store in the
                                 // IL tree (syms are not first used)
                                 // - syms that have associated temps are
                                 // excluded (only when
                                 // enablePreciseSymbolTracking() == false
                                 // otherwise all syms are included)
  TR_BitVector
...
class TR_StoreInformation {
 public:
  ...
  bool _copy;               // whether original store should be copied or moved
               ...
               //  source block
  bool _needsDuplication;  // has the store already been duplicated by
                           // sinkStorePlacement or should it be done in
                           // doSinking
};
...
class TR_OrderedBlockList : public List<TR_BlockListEntry> {
 public:
  ...
  TR_ALLOC(TR_Memory::DataFlowAnalysis)
  ListElement<TR_BlockListEntry> *addInTraversalOrder(TR::Block *block,
                                                      bool forward,
                                                      TR::CFGEdge *edge);
...
class TR_SinkStores : public TR::Optimization {
 ...
 protected:
  ...
  virtual bool storeCanMoveThroughBlock(
      TR_BitVector *blockKilledSet, TR_BitVector *blockUsedSet, int32_t symIdx,
