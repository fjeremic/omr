...
public:
  ...
  TR_LiveOnNotAllPaths(TR::Compilation *comp, TR_Liveness *liveOnSomePaths,
...
public:
  ...
  TR_FirstUseOfLoad(TR::Node *node, TR::TreeTop *anchorLocation,
                    int32_t anchorBlockNumber)
      : _node(node), _anchorLocation(anchorLocation),
        _anchorBlockNumber(anchorBlockNumber), _isAnchored(false) {}
  ...
  TR::TreeTop *setAnchorLocation(TR::TreeTop *loc) {
    return _anchorLocation = loc;
  }
...
public:
  TR_ALLOC(TR_Memory::DataFlowAnalysis)
  TR_CommonedLoad(TR::Node *node, int32_t symIdx)
      : _node(node), _isSatisfied(false), _isKilled(false), _symIdx(symIdx) {}
...
public:
  ...
  TR_UseOrKillInfo(TR::TreeTop *tt, TR::Block *block, TR_BitVector *usedSymbols,
                   TR_BitVector *commonedSymbols, uint32_t &indirectLoadCount,
                   TR_BitVector *killedSymbols)
      : _tt(tt), _block(block), _usedSymbols(usedSymbols),
        _commonedSymbols(commonedSymbols),
        _indirectLoadCount(indirectLoadCount), _killedSymbols(killedSymbols),
        _movableStore(NULL) {}
  ...
  TR_MovableStore
      *_movableStore; // all TR_MovableStore's are also TR_UseOrKillInfo's so
                      // this is a pointer to the corresponding movableStore (if
                      // one exists)
};
...
public:
  ...
  TR_MovableStore(TR_SinkStores *s, TR_UseOrKillInfo *useOrKillInfo,
                  int32_t symIdx = -1, TR_BitVector *commonedLoadsUnderTree = 0,
                  TR_BitVector *commonedLoadsAfter = 0, int32_t depth = 0,
  ...
  int32_t _symIdx;
  TR_BitVector *
      _commonedLoadsUnderTree; // commoned used syms under this store in the IL
                               // tree (syms are not first used)
                               // - syms that have associated temps are excluded
                               // (only when enablePreciseSymbolTracking() ==
                               // false otherwise all syms are included)
  TR_BitVector
      *_commonedLoadsAfter; // used syms which are commoned following the store
                            // (syms are first used and are commoned later)
  TR::Compilation *comp() { return _comp; }
...
public:
  ...
  TR_IndirectLoadAnchor(TR::Node *node, TR::Block *anchorBlock,
                        TR::TreeTop *anchorTree)
...
public:
  TR_ALLOC(TR_Memory::DataFlowAnalysis)
  TR_StoreInformation(TR::TreeTop *store, bool copy,
                      bool needsDuplication = true)
      : _store(store), _copy(copy), _needsDuplication(needsDuplication),
        _storeTemp(NULL) {}
  ...
  bool _copy;              // whether original store should be copied or moved
              //  it should be copied if the store is placed in a block that
              //  does NOT extend the original source block it should be moved
              //  if it is to be placed in a block that extends the original
              //  source block
  bool
      _needsDuplication; // has the store already been duplicated by
                         // sinkStorePlacement or should it be done in doSinking
};
...
public:
  ...
  TR_BitVector *_symbolsUsedOrKilled; // symbols that are used or killed by
                                      // stores placed along this edge
};
...
public:
  ...
  TR_EdgeStorePlacement(TR_StoreInformation *store, TR_EdgeInformation *edge,
                        TR_Memory *m)
      : _stores(m), _edges(m) {
    _stores.add(store);
    _edges.add(edge);
  ...
  List<TR_EdgeInformation>
      _edges; // list of cfg edges to _block that should be split to place def
              // NOTE: only one of these edges need be split: others can simply
              // be redirected to the new split block
};
...
public:
  ...
  TR_BlockStorePlacement(TR_StoreInformation *store, TR::Block *block,
                         TR_Memory *m)
      : _stores(m) {
    _stores.add(store);
    _block = block;
...
public:
  ...
  addInTraversalOrder(TR::Block *block, bool forward, TR::CFGEdge *edge);
...
public:
  ...
  bool usesDataFlowAnalysis() {
    return _storeSinkingFlags.testAny(UsesDataFlowAnalysis);
  }
  void setUsesDataFlowAnalysis(bool b) {
    _storeSinkingFlags.set(UsesDataFlowAnalysis, b);
  }

  bool sinkStoresWithIndirectLoads() {
    return _storeSinkingFlags.testAny(SinkStoresWithIndirectLoads);
  }
  void setSinkStoresWithIndirectLoads(bool b) {
    _storeSinkingFlags.set(SinkStoresWithIndirectLoads, b);
  }

  bool isExceptionFlagIsSticky() {
    return _storeSinkingFlags.testAny(ExceptionFlagIsSticky);
  }
  void setExceptionFlagIsSticky(bool b) {
    _storeSinkingFlags.set(ExceptionFlagIsSticky, b);
  }

  bool sinkStoresWithStaticLoads() {
    return _storeSinkingFlags.testAny(SinkStoresWithStaticLoads);
  }
  void setSinkStoresWithStaticLoads(bool b) {
    _storeSinkingFlags.set(SinkStoresWithStaticLoads, b);
  }

  TR::RegisterMappedSymbol *getSinkableSymbol(TR::Node *node);

private:
  virtual bool storeIsSinkingCandidate(TR::Block *block, TR::Node *node,
                                       int32_t symIdx, bool sinkIndirectLoads,
  ...
  virtual bool sinkStorePlacement(TR_MovableStore *store,
                                  bool nextStoreWasMoved) = 0;
  void lookForSinkableStores();
  void doSinking();
  TR_EdgeInformation *findEdgeInformation(TR::CFGEdge *edge,
                                          List<TR_EdgeInformation> &edgeList);
  void coalesceSimilarEdgePlacements();
  void placeStoresAlongEdges(List<TR_StoreInformation> &stores,
                             List<TR_EdgeInformation> &edges);
  void placeStoresInBlock(List<TR_StoreInformation> &stores,
                          TR::Block *placementBlock);

protected:
  int32_t performStoreSinking();
  virtual bool
  storeCanMoveThroughBlock(TR_BitVector *blockKilledSet,
                           TR_BitVector *blockUsedSet, int32_t symIdx,
                           TR_BitVector *allBlockUsedSymbols = NULL,
                           TR_BitVector *allBlockKilledSymbols = NULL);
  bool treeIsSinkableStore(TR::Node *node, bool sinkIndirectLoads,
                           uint32_t &indirectLoadCount, int32_t &depth,
                           bool &isLoadStatic, vcount_t visitCount);
  bool checkLiveMergingPaths(TR_BlockListEntry *blockEntry, int32_t symIdx);
  bool shouldSinkStoreAlongEdge(int symIdx, TR::CFGNode *block,
                                TR::CFGNode *succBlock,
                                int32_t sourceBlockFrequency, bool isLoadStatic,
                                vcount_t visitCount,
  ...
  bool
  isSafeToSinkThruEdgePlacement(int symIdx, TR::CFGNode *block,
                                TR::CFGNode *succBlock,
                                TR_BitVector *allEdgeInfoUsedOrKilledSymbols);
  bool isSymUsedInEdgePlacement(TR::CFGNode *block, TR::CFGNode *succBlock);

  uint32_t insertAnchoredNodes(
  ...
  // Set of blocks that have some symbols that are live on not all paths at
  // their exits
  TR_BitVector *_candidateBlocks;

  // One bit vector per block in the method describing which symbols are used or
  // killed
  ...
  // These bit vectors hold the symbols defined and used by the candidate for
  // sinking Right now, there's only one tree (a store) represented in these,
  // but in future the
  //   information for a set of trees could be put into these vectors (imagine
  //   moving an entire loop or an if-then-else statement).
  TR_BitVector *_usedSymbolsToMove;
  TR_BitVector *_killedSymbolsToMove;

  // These lists hold pending placements for sunk stores
  // They are used to detect safe movements after stores are sunk (because sunk
  // stores need to stop other stores)
  ...
  void replaceLoadsWithTempSym(TR::Node *newNode, TR::Node *origNode,
                               TR_BitVector *needTempForCommonedLoads);
  bool isCorrectCommonedLoad(TR::Node *commonedLoad, TR::Node *searchNode);
  void genStoreToTempSyms(TR::TreeTop *storeLocation, TR::Node *node,
  ...
  bool _sinkAllStores;       // all sinkable stores will be moved regardless of
                             // condition
  bool _printSinkStoreStats; // print number of stores removed / sunk / temp
                             // created
...
// store sinking code cannot push store with commoned load properly (problem
// with using _usedSymbolsToMove outside Ebb)
// TODO: make this an instance variable and not dep on ZEMUL flag to clean it up
#define MIN_TREE_DEPTH_TO_MOVE                                                 \
  3 // if temp is used to sink the store, this is the min depth that can be
    // benefit from sinking
// TODO: make this an instance variable and not dep on ZEMUL flag to clean it up
#define MAX_TREE_DEPTH_TO_DUP                                                  \
  12 // set a max depth when is needed duplication so massive uncommoning does
     // not occur in the duplicated tree
