...
public:
  ...
  TR_LiveOnNotAllPaths(TR::Compilation* comp,
...
public:
  ...
  TR_FirstUseOfLoad(TR::Node* node,
                    TR::TreeTop* anchorLocation,
...
public:
  ...
  TR_UseOrKillInfo(TR::TreeTop* tt,
                   TR::Block* block,
                   TR_BitVector* usedSymbols,
                   TR_BitVector* commonedSymbols,
  ...
  TR_MovableStore*
    _movableStore; // all TR_MovableStore's are also TR_UseOrKillInfo's so this
                   // is a pointer to the corresponding movableStore (if one
                   // exists)
};
...
public:
  ...
  TR_IndirectLoadAnchor(TR::Node* node,
                        TR::Block* anchorBlock,
                        TR::TreeTop* anchorTree)
    : _node(node)
    , _anchorBlock(anchorBlock)
    , _anchorTree(anchorTree)
  {}
...
public:
  TR_ALLOC(TR_Memory::DataFlowAnalysis)
  TR_StoreInformation(TR::TreeTop* store,
                      bool copy,
  ...
  bool
    _copy; // whether original store should be copied or moved
           //  it should be copied if the store is placed in a block that does
           //  NOT extend the original source block it should be moved if it is
           //  to be placed in a block that extends the original source block
  bool
...
public:
  ...
  TR_EdgeStorePlacement(TR_StoreInformation* store,
                        TR_EdgeInformation* edge,
                        TR_Memory* m)
    : _stores(m)
    , _edges(m)
  {
  ...
  List<TR_EdgeInformation>
    _edges; // list of cfg edges to _block that should be split to place def
            // NOTE: only one of these edges need be split: others can simply be
            // redirected to the new split block
};
...
public:
  ...
  TR_BlockStorePlacement(TR_StoreInformation* store,
                         TR::Block* block,
                         TR_Memory* m)
    : _stores(m)
  {
...
protected:
  ...
  bool treeIsSinkableStore(TR::Node* node,
                           bool sinkIndirectLoads,
                           uint32_t& indirectLoadCount,
                           int32_t& depth,
                           bool& isLoadStatic,
  ...
  bool shouldSinkStoreAlongEdge(int symIdx,
                                TR::CFGNode* block,
  ...
  void replaceLoadsWithTempSym(TR::Node* newNode,
                               TR::Node* origNode,
                               TR_BitVector* needTempForCommonedLoads);
  ...
  bool
    _sinkAllStores; // all sinkable stores will be moved regardless of condition
