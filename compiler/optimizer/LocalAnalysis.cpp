...
bool TR_LocalAnalysisInfo::isCallLike(TR::Node* node)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    // Note that this change is only needed because javaLangClassFromClass loads are being skipped for performance (see
    // earlier code in this file) and we want to avoid strange side effects of a parent being commoned but a child is
    // not a candidate for commoning.
    //
    if (node->getOpCode().hasSymbolReference()
      && (node->getSymbolReference() == comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef())) {
      return true;
    ...
    if (node1->getChild(i)->getLocalIndex() != node2->getChild(i)->getLocalIndex())
      ...
      if (node1->getChild(i)->getLocalIndex() == MAX_SCOUNT) {
        if ((node1->getChild(i)->getOpCode().isLoadConst() || node1->getChild(i)->getOpCode().isLoadVarDirect())
          && (node2->getChild(i)->getOpCode().isLoadConst() || node2->getChild(i)->getOpCode().isLoadVarDirect())) {
          ...
          for (int32_t j = 0; j < node1->getChild(i)->getNumChildren(); j++) {
            ...
            if (grandChild1->getLocalIndex() != grandChild2->getLocalIndex())
              ...
              if (grandChild1->getLocalIndex() == MAX_SCOUNT) {
                if ((grandChild1->getOpCode().isLoadConst() || grandChild1->getOpCode().isLoadVarDirect())
                  && (grandChild2->getOpCode().isLoadConst() || grandChild2->getOpCode().isLoadVarDirect())) {
...
bool isExceptional(TR::Compilation* comp, TR::Node* node)
{
  ...
  if (comp->cg()->nodeMayCauseException(node)) {
    if (comp->cg()->traceBCDCodeGen())
      traceMsg(
        comp, "d^d: %s (%p) may cause on exception so do not speculate in PRE\n", node->getOpCode().getName(), node);
    return true;
  }
  ...
  if (TR_LocalAnalysis::isSupportedNode(node, _compilation, parent)) {
    ...
    if (opCode.isIndirect() && (opCode.isLoadVar() || opCode.isStore())) {
      ...
      if (node->getFirstChild()->isThisPointer() && node->getFirstChild()->isNonNull()) {
        ...
        int32_t otherLen;
        const char* otherSig
          = otherSymRef->getOwningMethod(_compilation)->classNameOfFieldOrStatic(otherSymRef->getCPIndex(), otherLen);
        if (otherSig) {
          otherSig = classNameToSignature(otherSig, otherLen, _compilation);
          otherClassObject
            = _compilation->fe()->getClassFromSignature(otherSig, otherLen, otherSymRef->getOwningMethod(_compilation));
        }
