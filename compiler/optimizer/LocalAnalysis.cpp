...
bool
TR_LocalAnalysis::isSupportedNodeForFieldPrivatization(TR::Node* node,
                                                       TR::Compilation* comp,
                                                       TR::Node* parent)
{
...
bool
TR_LocalAnalysis::isSupportedNode(TR::Node* node,
                                  TR::Compilation* comp,
                                  TR::Node* parent,
                                  bool isSupportedStoreNode)
{
  return isSupportedNodeForFunctionality(
           node, comp, parent, isSupportedStoreNode) &&
         isSupportedNodeForPREPerformance(node, comp, parent);
}

bool
TR_LocalAnalysis::isSupportedNodeForPREPerformance(TR::Node* node,
                                                   TR::Compilation* comp,
                                                   TR::Node* parent)
{
...
bool
TR_LocalAnalysis::isSupportedNodeForFunctionality(TR::Node* node,
                                                  TR::Compilation* comp,
                                                  TR::Node* parent,
                                                  bool isSupportedStoreNode)
{
  ...
  , _trMemory(c->trMemory())
{
  ...
  if (_numNodes < 0) {
    ...
    while (currentTree) {
      ...
      countSupportedNodes(
        firstNodeInTree, NULL, firstNodeInTreeHasCallsInStoreLhs);
      ...
      if ((opCode->isStore() && !firstNodeInTree->getSymbolReference()
        ...
        if (oldExpressionOnRhs == -1) {
          if (trace()) {
            traceMsg(comp(), "\nExpression #%d is : \n", _numNodes);
            comp()->getDebug()->print(
              comp()->getOutFile(), firstNodeInTree, 6, true);
          }
...
void
TR_LocalAnalysis::initializeLocalAnalysis(bool isSparse, bool lock)
{
...
void
TR_LocalAnalysis::initializeBlocks(TR::Block* block, TR::BitVector& blocksSeen)
{
  ...
  for (auto nextEdge = block->getSuccessors().begin();
       nextEdge != block->getSuccessors().end();
       ++nextEdge) {
  ...
  for (auto nextEdge = block->getExceptionSuccessors().begin();
       nextEdge != block->getExceptionSuccessors().end();
       ++nextEdge) {
    next = toBlock((*nextEdge)->getTo());
    if (!blocksSeen.ValueAt(next->getNumber()))
      initializeBlocks(next, blocksSeen);
...
//
int
TR_LocalAnalysisInfo::hasOldExpressionOnRhs(TR::Node* node,
    ...
    if (node->getOpCode().isStore() &&
      ...
      if (node->getOpCode().isStoreIndirect()) {
        if (seenWriteBarrier) {
          TR::Node::recreate(
            node,
            _compilation->il.opCodeForIndirectArrayLoad(node->getDataType()));
        } else {
          TR::Node::recreate(
            node,
            _compilation->il.opCodeForCorrespondingIndirectStore(
              node->getOpCodeValue()));
        }
    ...
    for (other = cursor.firstNode(); other; other = cursor.nextNode()) {
      ...
      if (other->getOpCode().isStore() &&
        ...
        if (other->getOpCode().isStoreIndirect()) {
          ...
          TR::Node::recreate(
            other, _compilation->il.opCodeForDirectLoad(other->getDataType()));
      ...
      if (seenOtherWriteBarrier) {
        ...
        if (other->getOpCode().isIndirect())
          ...
          TR::Node::recreate(
            other, _compilation->il.opCodeForDirectStore(other->getDataType()));
      }
...
bool
TR_LocalAnalysisInfo::isCallLike(TR::Node* node)
{
...
//
bool
TR_LocalAnalysisInfo::containsCall(TR::Node* node, bool& containsCallInStoreLhs)
{
...
bool
TR_LocalAnalysisInfo::containsCallInTree(TR::Node* node,
...
void
TR_LocalAnalysisInfo::containsCallResetVisitCounts(TR::Node* node)
{
  int32_t i;
...
//
bool
TR_LocalAnalysisInfo::areSyntacticallyEquivalent(TR::Node* node1,
                                                 TR::Node* node2)
{
...
int32_t
TR_LocalAnalysisInfo::HashTable::hash(TR::Node* node)
{
...
TR::Node*
TR_LocalAnalysisInfo::HashTable::Cursor::firstNode()
{
...
TR::Node*
TR_LocalAnalysisInfo::HashTable::Cursor::nextNode()
{
  while (_chunk) {
...
//
bool
TR_LocalAnalysisInfo::countSupportedNodes(TR::Node* node,
                                          TR::Node* parent,
                                          bool& containsCallInStoreLhs)
{
  ...
  if (TR_LocalAnalysis::isSupportedNode(node, _compilation, parent)) {
    ...
    if (oldExpressionOnRhs == -1) {
      if (trace()) {
        traceMsg(comp(), "\nExpression #%d is : \n", _numNodes);
        _compilation->getDebug()->print(
          _compilation->getOutFile(), node, 6, true);
      }
...
bool
isExceptional(TR::Compilation* comp, TR::Node* node)
{
...
//
bool
TR_LocalAnalysisInfo::collectSupportedNodes(TR::Node* node, TR::Node* parent)
{
