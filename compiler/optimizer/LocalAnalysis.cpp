...
bool TR_LocalAnalysis::isSupportedNodeForFieldPrivatization(
    TR::Node *node, TR::Compilation *comp, TR::Node *parent) {
...
bool TR_LocalAnalysis::isSupportedNode(TR::Node *node, TR::Compilation *comp,
                                       TR::Node *parent,
                                       bool isSupportedStoreNode) {
  return isSupportedNodeForFunctionality(node, comp, parent,
                                         isSupportedStoreNode) &&
         isSupportedNodeForPREPerformance(node, comp, parent);
}

bool TR_LocalAnalysis::isSupportedNodeForPREPerformance(TR::Node *node,
                                                        TR::Compilation *comp,
                                                        TR::Node *parent) {
...
bool TR_LocalAnalysis::isSupportedNodeForFunctionality(
  ...
  if (node->isThisPointer() && !node->isNonNull()) return false;

  if ((node->getOpCodeValue() == TR::a2l) ||
      (node->getOpCodeValue() == TR::a2i))
    return false;
  ...
  if (isSupportedOpCode(node->getOpCode(), comp) || isSupportedStoreNode ||
      node->getOpCode().isLoadConst()) {
    if (node->getDataType() == TR::Address) {
      if (!node->addressPointsAtObject()) return false;
    }
    return true;
  }
...
TR_LocalAnalysisInfo::TR_LocalAnalysisInfo(TR::Compilation *c, bool t)
  ...
  // of this class to determine the life of the information by using
  // jitStackMark and jitStackRelease.
  //
  ...
  if (_numNodes < 0) {
    ...
    while (currentTree) {
      ...
      countSupportedNodes(firstNodeInTree, NULL,
                          firstNodeInTreeHasCallsInStoreLhs);

      if ((opCode->isStore() && !firstNodeInTree->getSymbolReference()
                                     ->getSymbol()
                                     ->isAutoOrParm()) ||
          opCode->isCheck()) {
        ...
        if (oldExpressionOnRhs == -1) {
          if (trace()) {
            traceMsg(comp(), "\nExpression #%d is : \n", _numNodes);
            comp()->getDebug()->print(comp()->getOutFile(), firstNodeInTree, 6,
                                      true);
          }
        ...
        if (opCode->isCheck() &&
            (firstNodeInTree->getFirstChild()->getOpCode().isStore() &&
             !firstNodeInTree->getFirstChild()
                  ...
                  ->getSymbol()
                  ->isAutoOrParm())) {
          int oldExpressionOnRhs =
              hasOldExpressionOnRhs(firstNodeInTree->getFirstChild());

          if (oldExpressionOnRhs == -1) {
            if (trace()) {
              traceMsg(comp(), "\nExpression #%d is : \n", _numNodes);
              comp()->getDebug()->print(comp()->getOutFile(),
                                        firstNodeInTree->getFirstChild(), 6,
                                        true);
            }
  ...
  _supportedNodesAsArray = (TR::Node **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(TR::Node *));
  memset(_supportedNodesAsArray, 0, _numNodes * sizeof(TR::Node *));
  _checkExpressions =
      new (trStackMemory()) TR_BitVector(_numNodes, trMemory(), stackAlloc);
  ...
  while (currentTree) {
    ...
    if ((opCode->isStore() &&
         !firstNodeInTree->getSymbolReference()->getSymbol()->isAutoOrParm()) ||
        opCode->isCheck()) {
      if (opCode->isCheck()) {
        _checkSymbolReferences->set(
            firstNodeInTree->getSymbolReference()->getReferenceNumber());
        _checkExpressions->set(firstNodeInTree->getLocalIndex());
      }

      if (!_supportedNodesAsArray[firstNodeInTree->getLocalIndex()])
        _supportedNodesAsArray[firstNodeInTree->getLocalIndex()] =
            firstNodeInTree;
      ...
      if (opCode->isCheck() &&
          ...
          !firstNodeInTree->getFirstChild()
               ...
               ->getSymbol()
               ->isAutoOrParm() &&
          !_supportedNodesAsArray[firstNodeInTree->getFirstChild()
                                      ->getLocalIndex()])
        _supportedNodesAsArray[firstNodeInTree->getFirstChild()
                                   ->getLocalIndex()] =
            firstNodeInTree->getFirstChild();
...
void TR_LocalAnalysis::initializeLocalAnalysis(bool isSparse, bool lock) {
  ...
  for (i = 0; i < _lainfo._numBlocks; i++) {
    ...
    _info[i]._downwardExposedStoreAnalysisInfo =
        allocateContainer(getNumNodes());
  }
}

void TR_LocalAnalysis::initializeBlocks(TR::Block *block,
                                        TR::BitVector &blocksSeen) {
  _info[block->getNumber()]._block = block;
  blocksSeen[block->getNumber()] = true;

  TR::Block *next;
  for (auto nextEdge = block->getSuccessors().begin();
       nextEdge != block->getSuccessors().end(); ++nextEdge) {
    next = toBlock((*nextEdge)->getTo());
    if (!blocksSeen.ValueAt(next->getNumber()))
      initializeBlocks(next, blocksSeen);
  }
  for (auto nextEdge = block->getExceptionSuccessors().begin();
       nextEdge != block->getExceptionSuccessors().end(); ++nextEdge) {
    next = toBlock((*nextEdge)->getTo());
    if (!blocksSeen.ValueAt(next->getNumber()))
      initializeBlocks(next, blocksSeen);
  }
...
//
//
int TR_LocalAnalysisInfo::hasOldExpressionOnRhs(TR::Node *node,
                                                bool recalcContainsCall,
                                                bool storeLhsContainsCall) {
  //
  ...
  if (!recalcContainsCall && (relevantSubtree == node)) {
    // can use pre-calculated value of containsCall and storeLhsContainsCall, to
    // avoid visitCount overflow
  ...
  if (nodeContainsCall) {
    ...
    // the private temp store's RHS simply points at original RHS. But if a
    // call-like sub-expression is present in the LHS of the store, that is
    // inadmissable
    //
    if (!node->getOpCode().isStore() || storeLhsContainsCall) return 0;
  }
  ...
  /////if (node->getOpCode().isNullCheck())
  {
    ...
    for (k = 0; k < _numNullChecks; k++) {
      if (!(_nullCheckNodesAsArray[k] == NULL)) {
        if (areSyntacticallyEquivalent(
                _nullCheckNodesAsArray[k]->getNullCheckReference(),
    ...
    if (node->getOpCode().isStore() &&
      ...
      if (node->getOpCode().isWrtBar()) seenWriteBarrier = true;
#ifdef J9_PROJECT_SPECIFIC
      seenIndirectBCDStore = node->getType().isBCD();
#endif
      if (node->getOpCode().isStoreIndirect()) {
        if (seenWriteBarrier) {
          TR::Node::recreate(node, _compilation->il.opCodeForIndirectArrayLoad(
                                       node->getDataType()));
        } else {
          TR::Node::recreate(
              node, _compilation->il.opCodeForCorrespondingIndirectStore(
                        node->getOpCodeValue()));
        }
        node->setNumChildren(1);
      } else {
        TR::Node::recreate(
            node, _compilation->il.opCodeForDirectLoad(node->getDataType()));
        node->setNumChildren(0);
      }

#ifdef J9_PROJECT_SPECIFIC
      if (seenIndirectBCDStore) node->setBCDStoreIsTemporarilyALoad(true);
#endif
    ...
    for (other = cursor.firstNode(); other; other = cursor.nextNode()) {
      ...
      if (other->getOpCode().isStore() &&
        ...
        if (other->getOpCode().isWrtBar()) seenOtherWriteBarrier = true;
        ...
        if (other->getOpCode().isStoreIndirect()) {
          if (seenOtherWriteBarrier) {
            TR::Node::recreate(other,
                               _compilation->il.opCodeForIndirectArrayLoad(
                                   other->getDataType()));
          } else {
            TR::Node::recreate(
                other, _compilation->il.opCodeForCorrespondingIndirectStore(
                           other->getOpCodeValue()));
          }
          other->setNumChildren(1);
        } else {
          TR::Node::recreate(other, _compilation->il.opCodeForDirectLoad(
                                        other->getDataType()));
      ...
      if (seenOtherWriteBarrier) {
        ...
        if (other->getOpCode().isIndirect())
          TR::Node::recreate(
              other, _compilation->il.opCodeForCorrespondingIndirectLoad(
                         other->getOpCodeValue()));
        else
          TR::Node::recreate(other, _compilation->il.opCodeForDirectStore(
                                        other->getDataType()));
      }
...
#ifdef J9_PROJECT_SPECIFIC
          if (seenIndirectBCDStore) node->setBCDStoreIsTemporarilyALoad(false);
#endif

          if (node->getOpCode().isIndirect())
            TR::Node::recreate(
                node, _compilation->il.opCodeForCorrespondingIndirectLoad(
                          node->getOpCodeValue()));
          else
            TR::Node::recreate(node, _compilation->il.opCodeForDirectStore(
                                         node->getDataType()));
        }
  ...
  if (seenWriteBarrier) {
    ...
    if (node->getOpCode().isIndirect())
      TR::Node::recreate(node,
                         _compilation->il.opCodeForCorrespondingIndirectLoad(
                             node->getOpCodeValue()));
    else
      TR::Node::recreate(
          node, _compilation->il.opCodeForDirectStore(node->getDataType()));
  }
...
bool TR_LocalAnalysisInfo::isCallLike(TR::Node *node) {
  ...
  if ((opCode.isCall() && !node->isPureCall()) || opCodeValue == TR::New ||
  ...
  {
    if (node->hasUnresolvedSymbolReference()) return true;
  }
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (node->isThisPointer() && !node->isNonNull()) return true;
    ...
    // Note that this change is only needed because javaLangClassFromClass loads
    // are being skipped for performance (see earlier code in this file) and we
    // want to avoid strange side effects of a parent being commoned but a child
    // is not a candidate for commoning.
    //
    if (node->getOpCode().hasSymbolReference() &&
        (node->getSymbolReference() ==
         comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef())) {
...
//
// This method iterates over the children of the call, so it needs a visit count
// mechanism to avoid processing nodes twice. However, the trees are already in
// a similar situation so the node visit counts must not be corrupted for those
// iterations. Simply incrementing for each use of containsCall will not work,
// for large programs the visit count will overflow. To get around this, we use
// two visit counts:
//    if the old visit count is the current one, the new visit count is old
//    visit count + 2 otherwise, the new visit count is old visit count + 1
// At the end of processing the node tree is revisited and the visit counts
// reset to the old visit count or to zero. It is guaranteed that there is room
// for old visit count + 2, that is checked before all this starts.
//
bool TR_LocalAnalysisInfo::containsCall(TR::Node *node,
                                        bool &containsCallInStoreLhs) {
  bool result = containsCallInTree(node, containsCallInStoreLhs);
  containsCallResetVisitCounts(node);
  return result;
}
bool TR_LocalAnalysisInfo::containsCallInTree(TR::Node *node,
                                              bool &containsCallInStoreLhs) {
  ...
  if (isCallLike(node)) return true;

  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++) {
    bool childHasCalls =
        containsCallInTree(node->getChild(i), containsCallInStoreLhs);
    if (childHasCalls) {
...
//
// Returns true if the two subtrees are identical syntactically; only special
// case is TR::aiadd (or TR::aladd) which may not participate in PRE;
// expressions containing the TR::aiadd (or TR::aladd) are matched correctly
// though by the special case code.
//
bool TR_LocalAnalysisInfo::areSyntacticallyEquivalent(TR::Node *node1,
                                                      TR::Node *node2) {
  if (!_optimizer->areNodesEquivalent(node1, node2)) return false;
  ...
  for (i = 0; i < node1->getNumChildren(); i++) {
    if (node1->getChild(i)->getLocalIndex() !=
        node2->getChild(i)->getLocalIndex())
      return false;
    else {
      if (node1->getChild(i)->getLocalIndex() == MAX_SCOUNT) {
        if ((node1->getChild(i)->getOpCode().isLoadConst() ||
             node1->getChild(i)->getOpCode().isLoadVarDirect()) &&
            (node2->getChild(i)->getOpCode().isLoadConst() ||
             node2->getChild(i)->getOpCode().isLoadVarDirect())) {
          if (!_optimizer->areNodesEquivalent(node1->getChild(i),
                                              node2->getChild(i)))
            return false;
...
TR_LocalAnalysisInfo::HashTable::Cursor::Cursor(HashTable *table,
                                                int32_t bucket)
    : _table(*table),
      _bucket(bucket),
...
//
bool TR_LocalAnalysisInfo::countSupportedNodes(TR::Node *node, TR::Node *parent,
                                               bool &containsCallInStoreLhs) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (countSupportedNodes(child, node, childHasCallsInStoreLhs)) flag = true;

    if (childHasCallsInStoreLhs) containsCallInStoreLhs = true;
  ...
  if (TR_LocalAnalysis::isSupportedNode(node, _compilation, parent)) {
    int oldExpressionOnRhs =
        hasOldExpressionOnRhs(node, false, containsCallInStoreLhs);

    if (oldExpressionOnRhs == -1) {
      if (trace()) {
        traceMsg(comp(), "\nExpression #%d is : \n", _numNodes);
        _compilation->getDebug()->print(_compilation->getOutFile(), node, 6,
                                        true);
      }
...
bool isExceptional(TR::Compilation *comp, TR::Node *node) {
  ...
  if (comp->cg()->nodeMayCauseException(node)) {
    if (comp->cg()->traceBCDCodeGen())
      traceMsg(
          comp,
          "d^d: %s (%p) may cause on exception so do not speculate in PRE\n",
...
// These nodes require temps.
//
bool TR_LocalAnalysisInfo::collectSupportedNodes(TR::Node *node,
                                                 TR::Node *parent) {
  if (node->getVisitCount() == _visitCount) return false;
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    if (collectSupportedNodes(child, node)) flag = true;

    if (_checkExpressions->get(child->getLocalIndex())) childRelevant = true;
  }
  ...
  if (TR_LocalAnalysis::isSupportedNode(node, _compilation, parent)) {
    ...
    if (opCode.isIndirect() && (opCode.isLoadVar() || opCode.isStore())) {
      ...
      if (node->getFirstChild()->isThisPointer() &&
        ...
        if (sig && (len > 0))
          cl = _compilation->fe()->getClassFromSignature(
              sig, len, symRef->getOwningMethod(_compilation));
    ...
    if (childRelevant || (!indirectionSafe || (opCode.isArrayLength())) ||
