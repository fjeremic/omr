...
{
    return isSupportedNodeForFunctionality(node, comp, parent, isSupportedStoreNode) && isSupportedNodeForPREPerformance(node, comp, parent);
}
...
bool TR_LocalAnalysis::isSupportedNodeForPREPerformance(TR::Node* node, TR::Compilation* comp, TR::Node* parent)
{
    ...
    if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference() == comp->getSymRefTab()->findJavaLangClassFromClassSymbolRef())) {
        return false;
    }
...
bool TR_LocalAnalysis::isSupportedNodeForFunctionality(TR::Node* node, TR::Compilation* comp, TR::Node* parent, bool isSupportedStoreNode)
{
   ...
   if (node->getOpCode().isLoadConst() &&
    ...
    if (comp->requiresSpineChecks() && node->getOpCode().hasSymbolReference() && node->getSymbol()->isArrayShadowSymbol())
        return false;

    if (node->getOpCode().isCall() && !node->getSymbolReference()->isUnresolved() && node->getSymbol()->castToMethodSymbol()->isPureFunction() && (node->getDataType() != TR::NoType))
        return true;

    if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference()->isSideEffectInfo() || node->getSymbolReference()->isOverriddenBitAddress() || node->getSymbolReference()->isUnresolved()))
        return false;
   ...
   comp()->resetVisitCounts(0);
    ...
    if (_numNodes < 0) {
        ...
        while (currentTree) {
            ...
            if (((firstNodeInTree->getOpCodeValue() == TR::treetop) || (comp()->useAnchors() && firstNodeInTree->getOpCode().isAnchor())) && (firstNodeInTree->getFirstChild()->getOpCode().isStore())) {
            ...
            if ((opCode->isStore() && !firstNodeInTree->getSymbolReference()->getSymbol()->isAutoOrParm()) || opCode->isCheck()) {
                ...
                if (opCode->isCheck() && (firstNodeInTree->getFirstChild()->getOpCode().isStore() && !firstNodeInTree->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm())) {
    ...
    while (currentTree) {
        ...
        if ((opCode->isStore() && !firstNodeInTree->getSymbolReference()->getSymbol()->isAutoOrParm()) || opCode->isCheck()) {
            ...
            if (opCode->isCheck() && firstNodeInTree->getFirstChild()->getOpCode().isStore() && !firstNodeInTree->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm() && !_supportedNodesAsArray[firstNodeInTree->getFirstChild()->getLocalIndex()])
...
bool TR_LocalAnalysisInfo::isCallLike(TR::Node* node)
{
    ...
    if ((opCode.isCall() && !node->isPureCall()) || opCodeValue == TR::New || opCodeValue == TR::newarray || opCodeValue == TR::anewarray || opCodeValue == TR::multianewarray)
        return true;
    ...
    if (node->getOpCode().hasSymbolReference()) {
        if (node->getSymbolReference()->getSymbol()->isVolatile() || (node->getSymbolReference()->getSymbol()->isMethodMetaData() && !node->getSymbolReference()->getSymbol()->isImmutableField()))
            return true;
...
bool TR_LocalAnalysisInfo::collectSupportedNodes(TR::Node* node, TR::Node* parent)
{
    ...
    if (TR_LocalAnalysis::isSupportedNode(node, _compilation, parent)) {
        ...
        if (childRelevant || (!indirectionSafe || (opCode.isArrayLength())) || (node->getOpCode().isArrayRef()) || (opCode.hasSymbolReference() && (node->getSymbolReference()->isUnresolved() || node->getSymbol()->isArrayShadowSymbol())) || (opCode.isDiv() || opCode.isRem()))
