...
//
void TR_AsyncCheckInsertion::insertAsyncCheck(TR::Block *block,
                                              TR::Compilation *comp,
                                              const char *counterPrefix) {
  TR::TreeTop *lastTree = block->getLastRealTreeTop();
  TR::TreeTop *asyncTree = TR::TreeTop::create(
      comp, TR::Node::createWithSymRef(
                lastTree->getNode(), TR::asynccheck, 0,
  ...
  const char *const name = TR::DebugCounter::debugCounterName(
      comp, "asynccheck.insert/%s/(%s)/%s/block_%d", counterPrefix,
      comp->signature(), comp->getHotnessName(), block->getNumber());
  TR::DebugCounter::prependDebugCounter(comp, name,
                                        asyncTree->getNextTreeTop());
}

int32_t TR_AsyncCheckInsertion::insertReturnAsyncChecks(
    TR::Optimization *opt, const char *counterPrefix) {
  ...
  for (TR::TreeTop *treeTop = comp->getStartTree(); treeTop;
    ...
    if (block->getLastRealTreeTop()->getNode()->getOpCode().isReturn() &&
        performTransformation(
            comp, "%sInserting return asynccheck (%s) in block_%d\n",
            opt->optDetailString(), counterPrefix, block->getNumber())) {
...
int32_t TR_AsyncCheckInsertion::perform() {
  ...
  //
  static const char *p;
  static uint32_t numNodesInLargeMethod = (p = feGetEnv("TR_LargeMethodNodes"))
                                              ? atoi(p)
  ...
  if (!largeAcyclicMethod && comp()->getLoopWasVersionedWrtAsyncChecks()) {
    ...
    int32_t entry =
        comp()->getStartTree()->getNode()->getBlock()->getFrequency();
    int32_t limit =
        comp()->getOptions()->getLoopyAsyncCheckInsertionMaxEntryFreq();
  ...
  if (largeAcyclicMethod || loopyMethodWithVersionedAsyncChecks) {
    ...
    int32_t numAsyncChecksInserted =
        insertReturnAsyncChecks(this, counterPrefix);
    if (trace())
...
const char *TR_AsyncCheckInsertion::optDetailString() const throw() {
