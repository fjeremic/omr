...
const char *TR::SwitchAnalyzer::optDetailString() const throw() {
...
void TR::SwitchAnalyzer::analyze(TR::Node *node, TR::Block *block) {
  ...
  for (int32_t i = upperBound - 1; i > 1; --i) {
    ...
    if (i == upperBound - 1) highVal = konst;
    if (i == 2) lowVal = konst;
...
TR_LinkHead<TR::SwitchAnalyzer::SwitchInfo> *TR::SwitchAnalyzer::gather(
    TR_LinkHead<SwitchInfo> *chain) {
  SwitchInfo *prev = 0;
  SwitchInfo *cur =
...
TR::SwitchAnalyzer::SwitchInfo *TR::SwitchAnalyzer::getConsecutiveUniques(
    SwitchInfo *info) {
  SwitchInfo *cur = info;
  SwitchInfo *next = cur->getNext();
...
//
void TR::SwitchAnalyzer::chainInsert(TR_LinkHead<SwitchInfo> *chain,
                                     SwitchInfo *info) {
...
void TR::SwitchAnalyzer::denseInsert(SwitchInfo *dense, SwitchInfo *info) {
  ...
  if (info->_kind == Range) {
    ...
    for (CASECONST_TYPE i = info->_min; i <= info->_max; ++i)
      denseInsert(dense, new (trStackMemory())
                             SwitchInfo(i, info->_target, _costUnique));
  ...
  if (info->_min < dense->_min) dense->_min = info->_min;
  if (info->_max > dense->_max) dense->_max = info->_max;
...
void TR::SwitchAnalyzer::printInfo(TR_FrontEnd *fe, TR::FILE *pOutFile,
                                   TR_LinkHead<SwitchInfo> *chain) {
  if (!pOutFile) return;
...
void TR::SwitchAnalyzer::SwitchInfo::print(TR_FrontEnd *fe, TR::FILE *pOutFile,
                                           int32_t indent) {
  if (!pOutFile) return;

  trfprintf(pOutFile, "%*s %0.8g %4d %8d [%4d -%4d] ", indent, " ", _freq,
            _count, _cost, _min, _max);
  switch (_kind) {
    case Unique:
...
TR::SwitchAnalyzer::SwitchInfo *TR::SwitchAnalyzer::getLastInChain(
    TR_LinkHead<SwitchInfo> *chain) {
  if (!chain) return 0;
  SwitchInfo *info = chain->getFirst();
  if (!info) return 0;
  while (info->getNext()) info = info->getNext();
  return info;
}

TR::Block *TR::SwitchAnalyzer::peelOffTheHottestValue(
    TR_LinkHead<SwitchInfo> *chain) {
  if (!_haveProfilingInfo) return NULL;
  ...
  if (first->_kind == Dense) list = first->_chain;
...
TR::Block *TR::SwitchAnalyzer::checkIfDefaultIsDominant(SwitchInfo *start) {
  ...
  if (!hasChildWithDecentFrequency) {
    ...
    for (SwitchInfo *temp = start->getNext(); temp; temp = temp->getNext()) {
      if (absMin > temp->_min) absMin = temp->_min;
      if (absMax < temp->_max) absMax = temp->_max;
    }
...
void TR::SwitchAnalyzer::fixUpUnsigned(TR_LinkHead<SwitchInfo> *chain) {
  ...
  if (csr->_min >= 0) return;  // nothing to do
...
void TR::SwitchAnalyzer::emit(TR_LinkHead<SwitchInfo> *chain,
                              TR_LinkHead<SwitchInfo> *bound,
                              TR_LinkHead<SwitchInfo> *earlyUniques) {
  ...
  if (keepOverflow == false)
    _switch->setCannotOverflow(false);  // Cancel the unneeded range check
               ...
               comp(), "%sUnneeded range check on switch propagated\n",
               OPT_DETAILS))
    _switch->setCannotOverflow(false);  // Cancel the unneeded range check if
  ...
  if (majorsInBound > 0) {
    if (majorsInBound <= MIN_SIZE_FOR_BIN_SEARCH) {
      ...
      if (comp()->isOptServer() && _switch->getOpCodeValue() != TR::lookup) {
        ...
        if (peeledOffBlock) newBlock = peeledOffBlock;
      }
    } else {
      newBlock = binSearch(bound->getFirst(), getLastInChain(bound),
                           majorsInBound, rangeLeft, rangeRight);
      if (comp()->isOptServer()) {
        TR::Block *defaultNewBlock =
            checkIfDefaultIsDominant(bound->getFirst());
        if (defaultNewBlock) newBlock = defaultNewBlock;
      }
    }
  ...
  if (majorsInChain > 0) {
    if (majorsInChain <= MIN_SIZE_FOR_BIN_SEARCH) {
      ...
      newBlock = binSearch(chain->getFirst(), getLastInChain(chain),
                           majorsInChain, rangeLeft, rangeRight);
      if (comp()->isOptServer()) {
        TR::Block *defaultNewBlock =
  ...
  if (majorsInEarly > 0) {
    if (majorsInEarly <= MIN_SIZE_FOR_BIN_SEARCH)
      ...
      newBlock =
          binSearch(earlyUniques->getFirst(), getLastInChain(earlyUniques),
                    majorsInEarly, rangeLeft, rangeRight);
  }
...
TR::Block *TR::SwitchAnalyzer::binSearch(SwitchInfo *startNode,
                                         SwitchInfo *endNode, int32_t numMajors,
  ...
  if (numMajors == 2 && startNode == endNode) {
    ...
    if (endNode->_kind == Range) {
      ...
      addIfBlock(_signed ? TR::ificmpge : TR::ifiucmpge, endNode->_min,
                 endNode->_target);
      cmpOp = _isInt64 ? (_signed ? TR::iflcmpgt : TR::iflucmpgt)
                       : (_signed ? TR::ificmpgt : TR::ifiucmpgt);
  ...
  TR::Block *right = binSearch(pivotNode->getNext(), endNode, numMajors - pivot,
                               pivotValue + 1, rangeRight);
...
TR::SwitchAnalyzer::SwitchInfo *TR::SwitchAnalyzer::sortedListByFrequency(
    SwitchInfo *start) {
...
TR::Block *TR::SwitchAnalyzer::addIfBlock(TR::ILOpCodes opCode,
                                          CASECONST_TYPE val,
                                          TR::TreeTop *dest) {
  TR::Node *constNode =
      TR::Node::create(_switch, _isInt64 ? TR::lconst : TR::iconst, 0);
  constNode->set64bitIntegralValue(val);
  TR::Node *node = TR::Node::createif(
      opCode, TR::Node::createLoad(_switch, _temp), constNode);
...
TR::Block *TR::SwitchAnalyzer::addTableBlock(SwitchInfo *dense) {
  ...
  if (_switch && _switch->chkCannotOverflow())
    node->setCannotOverflow(
        true);  // Pass on info to code gen that table will have all cases
                // covered and not use default case

  node->setAndIncChild(
      0, TR::Node::create(
             TR::isub, 2,
             (_isInt64 ? TR::Node::create(TR::l2i, 1,
  ...
  TR_BitVector seenSuccessors(_cfg->getNextNodeNumber(), trMemory(),
                              stackAlloc);
  ...
  if (branchTable) {
    node->setAndIncChild(2 + upperBound + 1,
                         TR::Node::createWithSymRef(_switch, TR::loadaddr, 0,
                                                    branchTableSymRef));
  }
