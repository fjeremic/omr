...
TR::Block* TR::SwitchAnalyzer::linearSearch(SwitchInfo* start)
{
    ...
    SwitchInfo* cursor = (comp()->isOptServer() && _switch->getOpCodeValue() == TR::lookup) ? sortedListByFrequency(start) : start;
...
TR::Block* TR::SwitchAnalyzer::addTableBlock(SwitchInfo* dense)
{
    ...
    node->setAndIncChild(0, TR::Node::create(TR::isub, 2, (_isInt64 ? TR::Node::create(TR::l2i, 1, TR::Node::createLoad(_switch, _temp)) : TR::Node::createLoad(_switch, _temp)), TR::Node::create(_switch, TR::iconst, 0, dense->_min)));
