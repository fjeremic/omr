  ...
  TR_LinkHead<SwitchInfo>* chain, TR_LinkHead<SwitchInfo>* bound, TR_LinkHead<SwitchInfo>* earlyUniques)
{
  ...
  if (_switch->getOpCodeValue() == TR::lookup
    ...
    // if the number of cases is so small that it's always better to convert the switch to ifs, skip checks for backing
    // out
  ...
  if (majorsInEarly > 0) {
    if (majorsInEarly <= MIN_SIZE_FOR_BIN_SEARCH)
      ...
      newBlock
        = binSearch(earlyUniques->getFirst(), getLastInChain(earlyUniques), majorsInEarly, rangeLeft, rangeRight);
  }
