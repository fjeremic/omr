...
int32_t
TR::SwitchAnalyzer::perform()
{
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());

  _cfg = comp()->getFlowGraph();
  _haveProfilingInfo = (comp()->isOptServer()) ? _cfg->setFrequencies() : false;
  _blocksGeneratedByMe = new (trStackMemory())
    TR_BitVector(_cfg->getNextNodeNumber(), trMemory(), stackAlloc, growable);
...
void
TR::SwitchAnalyzer::analyze(TR::Node* node, TR::Block* block)
{
  if (_blocksGeneratedByMe->isSet(block->getNumber()))
    return;
  if (node->getFirstChild()->getOpCodeValue() == TR::trt)
    return;
  ...
  // Build Switch Info
  //
  TR_LinkHead<SwitchInfo>* chain =
    new (trStackMemory()) TR_LinkHead<SwitchInfo>();
  ...
  for (int32_t i = upperBound - 1; i > 1; --i) {
    ...
    SwitchInfo* info =
      new (trStackMemory()) SwitchInfo(konst, target, _costUnique);
    if (frequencies) {
    ...
    if (trace())
      traceMsg(comp(),
               "Switch info pointing at target tree top 0x%p has frequency "
               "scale of %f\n",
               target->getNode(),
               info->_freq);
  ...
  //                                         [--------------------------|------------------------------------]
  //                                         unsigned
  // If all the cases of the lookup are on the same 'half' of the number lines,
  // it doesn't matter if we use signed or unsigned comparisons, lets use
  // signed. For the signed case, we better use signed if the numbers straddle
  // the 0 boundary; we already do. For the unsigned case, we better use
  // unsigned if the numbers straddle the TR::getMaxSigned<TR::Int32>()
  // boundary. If that's the case, the highVal will be less than (in the signed
  // sense) the lowVal.
  //
  if (highVal < lowVal)
    _signed = false;
  ...
  // Remerge bound nodes back into the primary chain if small // FIXME:
  // implement
  //
  // remerge(chain, bound);

  // Fix-up sorting order. If unsigned, we need to put it back to unsigned
  // sorting order for correct emit since merge part made it signed sorting
  // order. The merge logic can be reused with this fix-up. Caveats are that
  // zero straddling set must be subdivided and TR::getMaxSigned<TR::Int32>()
  // straddling set could be merged back.
  if (!_signed) {
    fixUpUnsigned(chain);
    fixUpUnsigned(bound);
...
void
TR::SwitchAnalyzer::findDenseSets(TR_LinkHead<SwitchInfo>* chain)
{
  // find consecutive uniques (they must have different targets)
  // and group them together into a dense node
  //
  for (SwitchInfo *prev = 0, *cur = chain->getFirst(); cur;
...
bool
TR::SwitchAnalyzer::mergeDenseSets(TR_LinkHead<SwitchInfo>* chain)
{
  SwitchInfo* prev = 0;
...
TR_LinkHead<TR::SwitchAnalyzer::SwitchInfo>*
TR::SwitchAnalyzer::gather(TR_LinkHead<SwitchInfo>* chain)
{
  SwitchInfo* prev = 0;
  SwitchInfo* cur =
    chain->getFirst(); // will be non-null, chain cannot be empty
  SwitchInfo* next = cur->getNext();

  TR_LinkHead<SwitchInfo>* bound =
    new (trStackMemory()) TR_LinkHead<SwitchInfo>();
...
TR::SwitchAnalyzer::SwitchInfo*
TR::SwitchAnalyzer::getConsecutiveUniques(SwitchInfo* info)
{
  SwitchInfo* cur = info;
  SwitchInfo* next = cur->getNext();
...
bool
TR::SwitchAnalyzer::SwitchInfo::operator>(SwitchInfo& other)
{
...
//
void
TR::SwitchAnalyzer::chainInsert(TR_LinkHead<SwitchInfo>* chain,
                                SwitchInfo* info)
{
  TR_ASSERT(info->_kind != Dense, "Not expecting a dense node");
  SwitchInfo *cur, *prev;
  for (prev = 0, cur = chain->getFirst(); cur != NULL;
       prev = cur, cur = cur->getNext())
    if (*cur > *info)
      break;
...
void
TR::SwitchAnalyzer::denseInsert(SwitchInfo* dense, SwitchInfo* info)
{
  ...
  if (info->_kind == Range) {
    ...
    for (CASECONST_TYPE i = info->_min; i <= info->_max; ++i)
      denseInsert(
        dense, new (trStackMemory()) SwitchInfo(i, info->_target, _costUnique));
...
void
TR::SwitchAnalyzer::denseMerge(SwitchInfo* to, SwitchInfo* from)
{
...
void
TR::SwitchAnalyzer::printInfo(TR_FrontEnd* fe,
                              TR::FILE* pOutFile,
                              TR_LinkHead<SwitchInfo>* chain)
{
  if (!pOutFile)
...
void
TR::SwitchAnalyzer::SwitchInfo::print(TR_FrontEnd* fe,
                                      TR::FILE* pOutFile,
                                      int32_t indent)
{
  if (!pOutFile)
...
int32_t
TR::SwitchAnalyzer::countMajorsInChain(TR_LinkHead<SwitchInfo>* chain)
{
  if (!chain)
    return 0;
...
TR::SwitchAnalyzer::SwitchInfo*
TR::SwitchAnalyzer::getLastInChain(TR_LinkHead<SwitchInfo>* chain)
{
  if (!chain)
    return 0;
...
TR::Block*
TR::SwitchAnalyzer::peelOffTheHottestValue(TR_LinkHead<SwitchInfo>* chain)
{
  ...
  if (trace()) {
    traceMsg(comp(),
             "\nLooking to see if we have a value that's more than 33%% of all "
             "cases.\n");
  }
  ...
  if (topNode && (topNode->_kind == Unique) && (maxFreq > cutOffFrequency)) {
    ...
    if (trace()) {
      traceMsg(comp(),
               "Found a dominant entry in a dense node for target 0x%p with "
               "frequency of %f.\n",
               topNode->_target->getNode(),
               maxFreq);
...
TR::Block*
TR::SwitchAnalyzer::checkIfDefaultIsDominant(SwitchInfo* start)
{
  if (!_haveProfilingInfo)
    return NULL;
  ...
  if (trace()) {
    traceMsg(comp(),
             "Looking to see if the default case is dominant. Number of cases "
             "is %d, cut off frequency set to %f\n",
             numCases,
             cutOffFrequency);
  }
  ...
  for (SwitchInfo* temp = start; temp; temp = temp->getNext()) {
    if (temp->_freq >= cutOffFrequency) {
      ...
      if (trace()) {
        traceMsg(comp(),
                 "Found child with frequency of %f. The default case isn't "
                 "that dominant.\n",
                 temp->_freq);
      }
      break;
  ...
  if (!hasChildWithDecentFrequency) {
    ...
    if (trace()) {
      traceMsg(
        comp(),
        "The default case is dominant, we'll generate the range tests.\n");
    }
    ...
    TR::Block* newBlock = NULL;
    cmpOp = _isInt64 ? (_signed ? TR::iflcmplt : TR::iflucmplt)
                     : (_signed ? TR::ificmplt : TR::ifiucmplt);
    addIfBlock(cmpOp, absMin, _defaultDest);
    cmpOp = _isInt64 ? (_signed ? TR::iflcmpgt : TR::iflucmpgt)
                     : (_signed ? TR::ificmpgt : TR::ifiucmpgt);
...
// If unsigned, break consecutive range that straddles 0,
// TR::getMaxUnsigned<TR::Int32>(), and sort by unsigned order. todo: join
// range/dense that straddle
// TR::getMaxSigned<TR::Int32>(),TR::getMaxSigned<TR::Int32>()+1 Assumes that
// it's in signed sorted order
/*
 * [-110,-100]           [0,10]
 * [-10,10]       ==>    [100,110]
 * [100,110]             [-110,-100]
 ...
 */
void
TR::SwitchAnalyzer::fixUpUnsigned(TR_LinkHead<SwitchInfo>* chain)
{
  ...
  for (;; csr = csr->getNext()) {
    if (csr->_max >= 0 && csr->_min < 0) {
      ...
      for (auto info = firstPos->_chain->getFirst(); info;
           info = info->getNext()) {
      ...
      TR_ASSERT(firstPosFirstChainInfo != NULL,
                "Could not find the neg to pos SwitchInfo\n");
  ...
  // set firstPos as first in the chain and replace NULL after csr with
  // firstNeg. [pos range ... ->csr->NULL] -> [neg range]
  if (lastNeg && firstPos) {
...
void
TR::SwitchAnalyzer::emit(TR_LinkHead<SwitchInfo>* chain,
                         TR_LinkHead<SwitchInfo>* bound,
                         TR_LinkHead<SwitchInfo>* earlyUniques)
{
  ...
  if (_switch->getOpCodeValue() == TR::lookup &&
    ...
    if (trace())
      traceMsg(comp(),
               "numMajors %d, majorsInBound %d, numCases %d\n",
               numMajors,
               majorsInBound,
               numCases);

    // if the number of cases is so small that it's always better to convert the
    // switch to ifs, skip checks for backing out
    if (numCases > SWITCH_TO_IFS_THRESHOLD) {
      // Do the tranformation only if we know that the search space is
      // decreasing in size
      if (4 * numMajors >
          3 * numCases) // if nummajors must be no more than 3/4th of numcases
        return;
      if (numCases < MIN_CASES_FOR_OPT) // if numcases is small
        return;
      // Do the transformation only if we know that the primary chain contains
      // most of the cases (majorsInBound == numCasesInBound, since bound does
      // not have range or dense nodes)
      if (majorsInBound * 3 > numCases) // bound-cases must be no more than a
                                        // third of the toal cases
        return;
    }
  }

  if (!performTransformation(comp(),
                             "%soptimized switch in block_%d\n",
                             OPT_DETAILS,
                             _block->getNumber()))
    return;

  // Check if CannotOverflow flag on switch should be propagated
  // This flag indicates that the possible values of the switch operand are
  // withing the min max range of all the case statements
  ...
  if (keepOverflow == false)
    _switch->setCannotOverflow(false); // Cancel the unneeded range check
             ...
             comp(),
             "%sUnneeded range check on switch propagated\n",
             OPT_DETAILS))
    _switch->setCannotOverflow(
      false); // Cancel the unneeded range check if optimization limit reached

  // check if some element needs to be extracted because it is high frequency
  // we can pull them out and test them first before searching the rest of the
  // chain
  //
  // log(numMajors in range) * (probability of range) > 2 for ranges to be
  // extracted log(numMajors in dense) * (probability of dense) > 2 for denses
  // to be extracted uniques cannot be extracted since they don't exist in the
  // primary chain
  //
  // extracting multiple elements will almost never be practical unless they
  // have very high probablity (first = 60%, second = 30%, etc.)
  //
  // FIXME: implement the above
  //
  // We now have a list of elements, earlyUniques, that we want to check early.
  // When the above is implemented, we can simply add the items to this list;
  // code below issues them before anything else.

  // FIXME: the following code doesn't seem to be too graceful for unsigned
  // values
  CASECONST_TYPE rangeLeft, rangeRight;

  switch (_switch->getChild(0)->getDataType()) {
    case TR::Int16:
      rangeLeft = _signed ? TR::getMinSigned<TR::Int16>()
                          : TR::getMinUnsigned<TR::Int16>();
      rangeRight = _signed ? TR::getMaxSigned<TR::Int16>()
  ...
  _temp = comp()->getSymRefTab()->createTemporary(
    comp()->getMethodSymbol(), _isInt64 ? TR::Int64 : TR::Int32);
  ...
  if (majorsInBound > 0) {
    if (majorsInBound <= MIN_SIZE_FOR_BIN_SEARCH) {
      ...
      newBlock = binSearch(bound->getFirst(),
                           getLastInChain(bound),
                           majorsInBound,
                           rangeLeft,
                           rangeRight);
      if (comp()->isOptServer()) {
        TR::Block* defaultNewBlock =
          checkIfDefaultIsDominant(bound->getFirst());
        if (defaultNewBlock)
  ...
  if (majorsInChain > 0) {
    if (majorsInChain <= MIN_SIZE_FOR_BIN_SEARCH) {
      ...
      newBlock = binSearch(chain->getFirst(),
                           getLastInChain(chain),
                           majorsInChain,
                           rangeLeft,
                           rangeRight);
      if (comp()->isOptServer()) {
        TR::Block* defaultNewBlock =
          checkIfDefaultIsDominant(chain->getFirst());
        if (defaultNewBlock)
          newBlock = defaultNewBlock;
      }
    }
  ...
  if (majorsInEarly > 0) {
    if (majorsInEarly <= MIN_SIZE_FOR_BIN_SEARCH)
      ...
      newBlock = binSearch(earlyUniques->getFirst(),
                           getLastInChain(earlyUniques),
                           majorsInEarly,
                           rangeLeft,
                           rangeRight);
  }
  ...
  TR_ASSERT(newBlock,
            "At least one of primary, early unique, and bound chains must not "
            "be empty");
  ...
  for (auto edge = _block->getSuccessors().begin();
       edge != _block->getSuccessors().end();) {
...
TR::Block*
TR::SwitchAnalyzer::binSearch(SwitchInfo* startNode,
                              SwitchInfo* endNode,
                              int32_t numMajors,
                              CASECONST_TYPE rangeLeft,
                              CASECONST_TYPE rangeRight)
{
  ...
  //
  if (numMajors == 1) {
    TR_ASSERT(startNode == endNode && startNode->_kind == Unique,
              "illegal leaf node");
  ...
  if (numMajors == 2 && startNode == endNode) {
    TR_ASSERT(startNode->_kind != Unique,
              "cannot have two majors in a unique leaf");
    ...
    if (endNode->_kind == Range) {
      if (rangeRight == endNode->_max && rangeLeft == endNode->_min) {
        ...
        //
        addGotoBlock(_defaultDest);
        cmpOp = _isInt64 ? (_signed ? TR::iflcmpge : TR::iflucmpge)
                         : (_signed ? TR::ificmpge : TR::ifiucmpge);
        return addIfBlock(cmpOp, endNode->_min, endNode->_target);
      } else if (rangeLeft == endNode->_min) {
        // Lower bound test can be omitted
        //
        addGotoBlock(_defaultDest);
        cmpOp = _isInt64 ? (_signed ? TR::iflcmple : TR::iflucmple)
                         : (_signed ? TR::ificmple : TR::ifiucmple);
        return addIfBlock(cmpOp, endNode->_max, endNode->_target);
      }
      ...
      addIfBlock(_signed ? TR::ificmpge : TR::ifiucmpge,
                 endNode->_min,
                 endNode->_target);
      cmpOp = _isInt64 ? (_signed ? TR::iflcmpgt : TR::iflucmpgt)
                       : (_signed ? TR::ificmpgt : TR::ifiucmpgt);
  ...
  int32_t pivot =
    numMajors /
    2; // FIXME: divide the freq range into two rather than the numMajors
  ...
  TR::Block* right = binSearch(pivotNode->getNext(),
                               endNode,
                               numMajors - pivot,
                               pivotValue + 1,
                               rangeRight);
...
TR::SwitchAnalyzer::SwitchInfo*
TR::SwitchAnalyzer::sortedListByFrequency(SwitchInfo* start)
{
...
TR::Block*
TR::SwitchAnalyzer::linearSearch(SwitchInfo* start)
{
  ...
  if ((_switch->getOpCodeValue() == TR::lookup) && trace()) {
    traceMsg(
      comp(),
      "Laying down linear search sequence. Initial switch values order:\n");
    for (SwitchInfo* temp = start; temp; temp = temp->getNext()) {
  ...
  SwitchInfo* cursor =
    (comp()->isOptServer() && _switch->getOpCodeValue() == TR::lookup)
      ? sortedListByFrequency(start)
      : start;
...
TR::Block*
TR::SwitchAnalyzer::addGotoBlock(TR::TreeTop* dest)
{
  ...
  TR::Block* newBlock = TR::Block::createEmptyBlock(
    node, comp(), dest->getNode()->getBlock()->getFrequency(), _block);
...
TR::Block*
TR::SwitchAnalyzer::addIfBlock(TR::ILOpCodes opCode,
                               CASECONST_TYPE val,
                               TR::TreeTop* dest)
{
  TR::Node* constNode =
    TR::Node::create(_switch, _isInt64 ? TR::lconst : TR::iconst, 0);
  constNode->set64bitIntegralValue(val);
  TR::Node* node =
    TR::Node::createif(opCode, TR::Node::createLoad(_switch, _temp), constNode);

  node->setBranchDestination(dest);
  TR::Block* newBlock =
    TR::Block::createEmptyBlock(node, comp(), _block->getFrequency(), _block);
...
TR::Block*
TR::SwitchAnalyzer::addTableBlock(SwitchInfo* dense)
{
  ...
  TR::Node* node =
    TR::Node::create(_switch, TR::table, 3 + upperBound + branchTable);
  if (_switch && _switch->chkCannotOverflow())
    node->setCannotOverflow(
      true); // Pass on info to code gen that table will have all cases covered
             // and not use default case
  ...
  node->setAndIncChild(
    ...
    TR::Node::create(
      ...
      2,
      (_isInt64
         ? TR::Node::create(TR::l2i, 1, TR::Node::createLoad(_switch, _temp))
  ...
  TR_BitVector seenSuccessors(
    _cfg->getNextNodeNumber(), trMemory(), stackAlloc);
  ...
  for (int32_t caseIndex = 0; caseIndex <= upperBound; ++caseIndex) {
    ...
    node->setAndIncChild(2 + caseIndex,
                         TR::Node::createCase(_switch, dest, caseIndex));
  }

  if (branchTable) {
    node->setAndIncChild(
      2 + upperBound + 1,
      TR::Node::createWithSymRef(_switch, TR::loadaddr, 0, branchTableSymRef));
  }
...
int32_t*
TR::SwitchAnalyzer::setupFrequencies(TR::Node* node)
{
  if (!_haveProfilingInfo)
    return 0;

  int32_t* targetCounts = (int32_t*)trMemory()->allocateStackMemory(
    _cfg->getNextNodeNumber() * sizeof(int32_t));
  memset(targetCounts, 0, sizeof(int32_t) * _cfg->getNextNodeNumber());
  int32_t* frequencies = (int32_t*)trMemory()->allocateStackMemory(
    node->getCaseIndexUpperBound() * sizeof(int32_t));
  memset(frequencies, 0, sizeof(int32_t) * node->getCaseIndexUpperBound());

  // Count the number of cases reaching a particular target, divide target block
  // frequency by the number of cases
  //
  CASECONST_TYPE i;
  for (i = node->getCaseIndexUpperBound() - 1; i > 0; --i) {
    TR::Node* caseNode = node->getChild(i);
    TR::Block* targetBlock =
      caseNode->getBranchDestination()->getNode()->getBlock();
  ...
  for (i = node->getCaseIndexUpperBound() - 1; i > 0; --i) {
    ...
    TR_ASSERT_FATAL(
      targetCount > 0,
      "Successor block_%d of switch statement has non-sense successsor count",
    ...
    if (trace())
      traceMsg(comp(),
               "Switch analyser: Frequency at pos %d is %d\n",
               i,
               frequencies[i]);
  }

  // For each case value, lists the frequency of the selector being of that
  // value Note that the default is at frequencies[1], frequencies[0] will
  // always be null.
  //
  return frequencies;
}
...
// dense set
bool
TR::SwitchAnalyzer::keepAsUnique(SwitchInfo* info, int32_t itemNumber)
{
  return false;
}
