  ...
  : _compilation(findDominators.comp())
{
  ...
  if (expensiveAlgorithmCorrect) {
    if (debug("traceVER"))
      dumpOptDetails(
        comp(), "Dominators computed by the expensive algorithm are correct\n");
  } else {
    if (debug("traceVER"))
      dumpOptDetails(
        comp(),
  ...
  if (bothImplementationsConsistent) {
    ...
    TR_ASSERT(
      0, "Dominators computed by the two implementations are NOT consistent\n");
  }
}

bool
TR_DominatorVerifier::areBothImplementationsConsistent(
  TR_DominatorsChk& findDominatorsChk,
    ...
    if (!(efficientDominator == conservativeDominator)) {
      if (debug("traceVER")) {
        ...
        dumpOptDetails(
          ...
          _dominatorsChkInfo[i]._idom->_block);
        dumpOptDetails(
          comp(),
...
bool
TR_DominatorVerifier::isExpensiveAlgorithmCorrect(
      ...
      if (bucket->get(j)) // dominator according to algorithm
      {
              ...
              _dominatorsChkInfo[i]._block)) // dominator according to the CFG
        {
          if (debug("traceVER")) {
            ...
            dumpOptDetails(
              comp(),
              "   Dominator of [%p] is [%p] as per the algorithm\n",
              _dominatorsChkInfo[i]._block,
              _dominatorsChkInfo[j + 1]._block);
            dumpOptDetails(comp(),
...
//
bool
TR_DominatorVerifier::dominates(TR::Block* dominator, TR::Block* dominated)
{
  for (auto pred = dominated->getPredecessors().begin();
       pred != dominated->getPredecessors().end();
       ++pred) {
    _visitCount = comp()->incVisitCount();
  ...
  for (auto pred = dominated->getExceptionPredecessors().begin();
       pred != dominated->getExceptionPredecessors().end();
       ++pred) {
    _visitCount = comp()->incVisitCount();
    compareWithPredsOf(dominator, toBlock((*pred)->getFrom()));
    *_nodesSeenOnEveryPath &= *_nodesSeenOnCurrentPath;
...
//
void
TR_DominatorVerifier::compareWithPredsOf(TR::Block* dominator,
                                         TR::Block* dominated)
{
