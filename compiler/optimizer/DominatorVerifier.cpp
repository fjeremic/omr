  ...
  for (i = 2; i < _numBlocks - 1; i++) {
    ...
    for (j = 0; j < _numBlocks - 1; j++) {
      if (bucket->get(j)) // dominator according to algorithm
      {
        ...
        if (!dominates(
              _dominatorsChkInfo[j + 1]._block, _dominatorsChkInfo[i]._block)) // dominator according to the CFG
        {
          if (debug("traceVER")) {
            ...
            dumpOptDetails(comp(), "   But [%p] is not an the dominator of [%p] as per the Control Flow Graph",
