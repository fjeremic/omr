...
TR_DominatorVerifier::TR_DominatorVerifier(TR_Dominators &findDominators)
  ...
  if (debug("traceVER")) {
    dumpOptDetails(comp(),
                   "Printing out the TreeTops from DominatorVerifier\n");
    ...
    if (cfg != NULL) comp()->getDebug()->print(comp()->getOutFile(), cfg);
  }
  ...
  if (expensiveAlgorithmCorrect) {
    if (debug("traceVER"))
      dumpOptDetails(
          comp(),
          "Dominators computed by the expensive algorithm are correct\n");
  } else {
    if (debug("traceVER"))
      dumpOptDetails(
          comp(),
          "Dominators computed by the expensive algorithm are NOT correct\n");
    TR_ASSERT(
        0, "Dominators computed by the expensive algorithm are NOT correct\n");
  }

  bothImplementationsConsistent =
      areBothImplementationsConsistent(expensiveAlgorithm, findDominators);

  if (bothImplementationsConsistent) {
    if (debug("traceVER"))
      dumpOptDetails(
          comp(),
          "Dominators computed by the two implementations are consistent\n");
  } else {
    if (debug("traceVER"))
      dumpOptDetails(comp(),
                     "Dominators computed by the two implementations are NOT "
                     "consistent\n");
    TR_ASSERT(
        0,
        "Dominators computed by the two implementations are NOT consistent\n");
  }
}
...
bool TR_DominatorVerifier::areBothImplementationsConsistent(
  ...
  for (i = 2; i < _numBlocks - 1; i++) {
    ...
    TR::Block *efficientDominator =
        findDominators.getDominator(_dominatorsChkInfo[i]._block);
    ...
    if (!(efficientDominator == conservativeDominator)) {
      if (debug("traceVER")) {
        ...
        dumpOptDetails(
            comp(),
            "   Dominator computed by expensive algorithm for [%p] is [%p]\n",
            _dominatorsChkInfo[i]._block, _dominatorsChkInfo[i]._idom->_block);
        dumpOptDetails(
            comp(),
            "   Dominator computed by efficient algorithm for [%p] is [%p]\n",
...
bool TR_DominatorVerifier::isExpensiveAlgorithmCorrect(
    TR_DominatorsChk &expensiveAlgorithm) {
  int32_t i, j;
  _nodesSeenOnEveryPath =
      new (trStackMemory()) TR_BitVector(_numBlocks, trMemory(), stackAlloc);
  _nodesSeenOnCurrentPath =
      new (trStackMemory()) TR_BitVector(_numBlocks, trMemory(), stackAlloc);
  ...
  for (i = 2; i < _numBlocks - 1; i++) {
    ...
    for (j = 0; j < _numBlocks - 1; j++) {
      if (bucket->get(j))  // dominator according to algorithm
      {
        ...
        if (lastBit) _nodesSeenOnCurrentPath->set(_numBlocks - 1);

        if (!dominates(_dominatorsChkInfo[j + 1]._block,
                       _dominatorsChkInfo[i]
                           ._block))  // dominator according to the CFG
        {
          if (debug("traceVER")) {
            dumpOptDetails(
                comp(),
                "   Dominator info for expensive algorithm is incorrect \n");
            dumpOptDetails(
                comp(), "   Dominator of [%p] is [%p] as per the algorithm\n",
                _dominatorsChkInfo[i]._block, _dominatorsChkInfo[j + 1]._block);
            dumpOptDetails(comp(),
                           "   But [%p] is not an the dominator of [%p] as per "
                           "the Control Flow Graph",
                           _dominatorsChkInfo[j + 1]._block,
                           _dominatorsChkInfo[i]._block);
          }
          return false;
        }
      }
...
//
bool TR_DominatorVerifier::dominates(TR::Block *dominator,
                                     TR::Block *dominated) {
  for (auto pred = dominated->getPredecessors().begin();
       pred != dominated->getPredecessors().end(); ++pred) {
    _visitCount = comp()->incVisitCount();
  ...
  for (auto pred = dominated->getExceptionPredecessors().begin();
       pred != dominated->getExceptionPredecessors().end(); ++pred) {
    _visitCount = comp()->incVisitCount();
    compareWithPredsOf(dominator, toBlock((*pred)->getFrom()));
    *_nodesSeenOnEveryPath &= *_nodesSeenOnCurrentPath;
    _nodesSeenOnCurrentPath->empty();
  ...
  for (int32_t i = 0; i < _numBlocks - 1; i++) {
    if (i == _dominators->_dfNumbers[dominator->getNumber()]) {
      if (!_nodesSeenOnEveryPath->get(i)) return false;
    } else {
      if (_nodesSeenOnEveryPath->get(i)) return false;
    }
  }
