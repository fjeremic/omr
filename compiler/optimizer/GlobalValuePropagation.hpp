...
private:
  ...
  void processStructure(TR_StructureSubGraphNode *node, bool lastTimeThrough,
                        bool insideLoop);
  void processAcyclicRegion(TR_StructureSubGraphNode *node,
                            bool lastTimeThrough, bool insideLoop);
  void processNaturalLoop(TR_StructureSubGraphNode *node, bool lastTimeThrough,
                          bool insideLoop);
  void processImproperLoop(TR_StructureSubGraphNode *node, bool lastTimeThrough,
                           bool insideLoop);
  void processRegionSubgraph(TR_StructureSubGraphNode *node,
                             bool lastTimeThrough, bool insideLoop,
                             bool isNaturalLoop);
  void processRegionNode(TR_StructureSubGraphNode *node, bool lastTimeThrough,
                         bool insideLoop);
  void processBlock(TR_StructureSubGraphNode *node, bool lastTimeThrough,
                    bool insideLoop);
  void getImproperRegionStores(TR_StructureSubGraphNode *node,
                               ValueConstraints &stores);
  bool buildInputConstraints(TR::CFGNode *node);
  void propagateOutputConstraints(TR_StructureSubGraphNode *node,
                                  bool lastTimeThrough, bool isNaturalLoop,
  ...
  // NULL bitvector means the info is unavailable, and all blocks should be
  // processed.
  //
