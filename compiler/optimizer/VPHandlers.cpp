...
extern TR::Node*
constrainChildren(OMR::ValuePropagation* vp, TR::Node* node);
extern TR::Node*
constrainVcall(OMR::ValuePropagation* vp, TR::Node* node);
extern void
createGuardSiteForRemovedGuard(TR::Compilation* comp, TR::Node* ifNode);

static void
checkForNonNegativeAndOverflowProperties(OMR::ValuePropagation* vp,
                                         TR::Node* node,
...
static bool
isBoolean(TR::VPConstraint* constraint)
{
  if (constraint) {
    if (constraint->asIntConst()) {
      int32_t low = constraint->asIntConst()->getLowInt();
...
static int32_t
arrayElementSize(const char* signature,
                 int32_t len,
                 TR::Node* node,
...
static void
constrainBaseObjectOfIndirectAccess(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (baseObjectSig) {
    TR_OpaqueClassBlock* baseClass =
      vp->fe()->getClassFromSignature(baseObjectSig, baseObjectSigLen, method);
    if (baseClass &&
...
static bool
tryFoldCompileTimeLoad(OMR::ValuePropagation* vp,
                       TR::Node* node,
                       bool& isGlobal)
{
  isGlobal = true;
  ...
  if (node->getOpCode().isIndirect()) {
    if (vp->trace())
      traceMsg(vp->comp(),
               "  constrainCompileTimeLoad inspecting %s %p\n",
    ...
    for (; !baseExpression;) {
      ...
      if (!constraint) {
        ...
        if (vp->trace())
          traceMsg(vp->comp(),
                   " - %s %p is class object - transforming\n",
                   curNode->getOpCode().getName(),
                   curNode);
...
static bool
constrainCompileTimeLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
  bool isGlobal;
...
static bool
findConstant(OMR::ValuePropagation* vp, TR::Node* node)
{
...
static bool
containsUnsafeSymbolReference(OMR::ValuePropagation* vp, TR::Node* node)
{
...
static bool
owningMethodDoesNotContainNullChecks(OMR::ValuePropagation* vp, TR::Node* node)
{
...
bool
constraintFitsInIntegerRange(OMR::ValuePropagation* vp,
...
bool
canMoveLongOpChildDirectly(TR::Node* node,
                           int32_t numChild,
                           TR::Node* arithNode)
{
...
bool
reduceLongOpToIntegerOp(OMR::ValuePropagation* vp,
                        TR::Node* node,
                        TR::VPConstraint* nodeConstraint)
{
  ...
  if (performTransformation(vp->comp(),
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
      ...
      if (canMoveLongOpChildDirectly(node, i, arithNode)) {
        ...
        dumpOptDetails(vp->comp(),
                       "    Transfer integer child %d %s (0x%p)\n",
                       i,
    ...
    vp->prepareToReplaceNode(
      node,
      TR::ILOpCode::getProperConversion(
        arithNode->getDataType(), node->getDataType(), false));
...
static void
constrainAConst(OMR::ValuePropagation* vp, TR::Node* node, bool isGlobal)
{
  TR::VPConstraint* constraint = NULL;
  if (node->getAddress() == 0) {
    ...
    if (node->isClassPointerConstant()) {
      TR::VPConstraint* typeConstraint = TR::VPClass::create(
        vp,
        TR::VPFixedClass::create(vp, (TR_OpaqueClassBlock*)node->getAddress()),
        NULL,
        NULL,
        NULL,
...
TR::Node*
constrainAConst(OMR::ValuePropagation* vp, TR::Node* node)
{
...
static void
constrainIntAndFloatConstHelper(OMR::ValuePropagation* vp,
                                TR::Node* node,
                                int32_t value,
                                bool isGlobal)
{
...
static void
constrainIntConst(OMR::ValuePropagation* vp, TR::Node* node, bool isGlobal)
{
...
TR::Node*
constrainIntConst(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainFloatConst(OMR::ValuePropagation* vp, TR::Node* node)
{
...
static void
constrainLongConst(OMR::ValuePropagation* vp, TR::Node* node, bool isGlobal)
{
...
TR::Node*
constrainLongConst(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainByteConst(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainShortConst(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainBCDSign(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (node->hasKnownSignCode()) // TODO: track assumed sign codes too? -- these
                                // are tracked on load operations already so not
                                // much benefit
  {
  ...
  if (sign == TR::DataType::getInvalidSignCode() ||
      sign == TR::DataType::getIgnoredSignCode()) // if no specific sign code
                                                  // see if a clean or preferred
                                                  // sign state can be tracked
  {
    ...
    if (constraintType != TR_Sign_Unknown) {
      ...
      vp->addGlobalConstraint(
        node, TR::VP_BCDSign::create(vp, constraintType, node->getDataType()));
    }
  } else {
...
TR::Node*
constrainBCDAggrLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
      ...
      case TR_Sign_Clean:
        if (!node->hasKnownCleanSign() &&
            performTransformation(
              vp->comp(),
              "%sTransfer sign constraint %s to %s (0x%p)\n",
              OPT_DETAILS,
...
TR::Node*
constrainAnyIntLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (index->getOpCode().isLoadConst()) {
      ...
      if (dataType == TR::Int16 && array->getOpCode().isIndirect()) {
        ...
        if (baseVPConstraint && baseVPConstraint->isConstString()) {
          ...
          uintptrj_t chIdx =
            (offset -
             (uintptrj_t)TR::Compiler->om.contiguousArrayHeaderSizeInBytes()) /
            2;
          uint16_t ch = constString->charAt(chIdx, vp->comp());
          if (ch != 0) {
            vp->replaceByConstant(
              node, TR::VPShortConst::create(vp, ch), isGlobal);
            return node;
          }
        }
      }
            ...
            if (constrainAnyIntLoadCriticalSection.hasVMAccess()) {
              ...
              if (validStart <= offset && offset < accessEnd &&
                ...
                uintptrj_t elementAddress =
                  TR::Compiler->om.getAddressOfElement(
                    vp->comp(), arrayObj, offset);
                switch (dataType) {
                  case TR::Int8: {
...
TR::Node*
constrainShortLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
...
TR::Node*
constrainIntLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
...
bool
simplifyJ9ClassFlags(OMR::ValuePropagation* vp, TR::Node* node, bool isLong)
{
#ifdef J9_PROJECT_SPECIFIC
...
static void
checkUnsafeArrayAccess(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (node->getSymbol()->isUnsafeShadowSymbol()) {
    if (vp->trace())
      traceMsg(vp->comp(),
               "Node [%p] has an unsafe symbol reference %d\n",
               node,
               node->getSymbolReference()->getReferenceNumber());
...
//
TR::Node*
constrainLload(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
//
TR::Node*
constrainFload(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (!findConstant(vp, node))
    constrainChildren(vp, node);
...
//
TR::Node*
constrainDload(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (!findConstant(vp, node))
    constrainChildren(vp, node);
...
static const char*
getFieldSignature(OMR::ValuePropagation* vp, TR::Node* node, int32_t& len)
{
  TR::SymbolReference* symRef = node->getSymbolReference();
 ...
 */
static bool
addKnownObjectConstraints(OMR::ValuePropagation* vp, TR::Node* node)
{
  TR::KnownObjectTable* knot = vp->comp()->getKnownObjectTable();
  if (!knot)
    return false;
  ...
  if (objectReferenceLocation) {
...

    {
      ...
      if (isFixedJavaLangClass) {
        ...
        //
        clazz =
          TR::Compiler->cls.classFromJavaLangClass(vp->comp(), objectReference);
      }
...
TR::Node*
constrainAload(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (symRef->getSymbol()->isLocalObject()) {
      vp->addGlobalConstraint(
        node,
        TR::VPObjectLocation::create(vp, TR::VPObjectLocation::StackObject));
    }
        ...
        if (sym->isStatic() && sym->isFinal()) {
          ...
          if (classOfStatic == NULL) {
            ...
            if (classNameOfFieldOrStatic) {
              classNameOfFieldOrStatic =
                classNameToSignature(classNameOfFieldOrStatic, len, vp->comp());
          ...
          TR_PersistentClassInfo* classInfo =
            vp->comp()
              ...
              ->findClassInfoAfterLocking(
                classOfStatic, vp->comp(), allowForAOT);
                  ...
                  if (elementSize != 0) {
                    ...
                    vp->addGlobalConstraint(
                      ...
                      TR::VPArrayInfo::create(
                        vp, arrLength, arrLength, elementSize));
                    vp->addGlobalConstraint(
                      node,
                      TR::VPObjectLocation::create(
                  ...
                  if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                    ...
                    if (elementSize != 0) {
                      ...
                      vp->addGlobalConstraint(
                        node,
                        TR::VPArrayInfo::create(
                          vp, firstDimension, firstDimension, elementSize));
                      vp->addGlobalConstraint(
                        node,
                        TR::VPObjectLocation::create(
                          vp, TR::VPObjectLocation::NotClassObject));
  ...
  if (vp->comp()->getCurrentMethod()->isJ9() && symRef &&
    ...
    if (vp->_curBlock->getExceptionClass()) {
      ...
      sig = classNameToSignature(
        vp->_curBlock->getExceptionClassNameChars(), len, vp->comp());
            ...
            if (classBlock != jlClass) {
              ...
              if (*sig == '[' || sig[0] == 'L') {
                ...
                if (elementSize != 0) {
                  constraint = TR::VPClass::create(
                    vp,
                    (TR::VPClassType*)constraint,
                    NULL,
                    NULL,
                    TR::VPArrayInfo::create(vp,
                                            0,
                                            elementSize == 0
                                              ? TR::getMaxSigned<TR::Int32>()
                                              : TR::getMaxSigned<TR::Int32>() /
...
static TR::Node*
findArrayLengthNode(OMR::ValuePropagation* vp,
                    TR::Node* node,
...
static TR::Node*
findArrayIndexNode(OMR::ValuePropagation* vp, TR::Node* node, int32_t stride)
{
...
TR::Node*
constrainAddressRef(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainIiload(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (field == TR::Symbol::Java_io_ByteArrayOutputStream_count) {
    ...
    vp->addGlobalConstraint(
      node, TR::VPIntRange::create(vp, 0, TR::getMaxSigned<TR::Int32>() >> 1));
    node->setIsNonNegative(true);
    node->setCannotOverflow(true);
  } else
#endif
...
TR::Node*
constrainIaload(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    // hence can be treated as known object.
    {
          ...
          if (recognizedClassName) {
            TR_OpaqueClassBlock* recognizedClazz =
              vp->fe()->getClassFromSignature(
                ...
                symRef->getOwningMethod(vp->comp()));
            if (recognizedClazz &&
                (TR_yes == vp->fe()->isInstanceOf(
                             baseObjectClazz, recognizedClazz, false))) {
      ...
      if (knot &&
          symRef ==
            vp->comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) {
      ...
      case TR::Symbol::Java_lang_invoke_MethodHandle_thunks:
        if (!node->isNonNull() &&
            performTransformation(vp->comp(),
                                  "%s[%p] recognized field is never null: %s\n",
                                  OPT_DETAILS,
                                  node,
          ...
          if (callSiteConstraint && callSiteConstraint->getKnownObject()) {
            ...
            if (bypassOnThisRun) {
              ...
              if (bypassLocation &&
                  performTransformation(
                    vp->comp(),
                    "%s[%p] bypass CallSite load from obj%d\n",
                    OPT_DETAILS,
                    node,
                    callSiteKOI)) {
        ...
        if (!foundInfo) {
          TR_PersistentClassInfo* classInfo =
            vp->comp()
              ...
              ->getPersistentCHTable()
              ->findClassInfoAfterLocking(
                vp->comp()->getCurrentMethod()->containingClass(), vp->comp());
          if (classInfo && classInfo->getFieldInfo()) {
            TR_PersistentFieldInfo* fieldInfo =
              classInfo->getFieldInfo()->findFieldInfo(vp->comp(), node, false);
            if (fieldInfo) {
            ...
            if (jlClass) {
              if (clazz != jlClass) {
                ...
                if (fieldName && (fieldNameLen > 0) &&
                    !strncmp(
                      fieldName, "java/util/TreeMap.valuesCollection", 34)) {
  ...
  if (sig && !(node->getOpCode().hasSymbolReference() &&
    ...
    if (classBlock) {
      ...
      if (jlClass) {
        if (classBlock != jlClass) {
          constraint =
            TR::VPClassType::create(vp, sig, len, method, isFixed, classBlock);
      ...
      if (arraySig && *arraySig == '[') {
        ...
        if (vp->comp()->getMethodHotness() >= scorching) {
          int32_t prefetchOffset =
            vp->comp()->fej9()->findFirstHotFieldTenuredClassOffset(vp->comp(),
                                                                    clazz);
                ...
                if (underlyingArray->getSymbolReference()
                  ...
                  fieldName =
                    underlyingArray->getSymbolReference()
                      ->getOwningMethod(vp->comp())
                      ->fieldName(
                        underlyingArray->getSymbolReference()->getCPIndex(),
                        fieldNameLen,
                        vp->comp()->trMemory());
                else
                  fieldName =
  ...
  if (base && base->getClass() &&
    ...
    vp->comp()->fej9()->markHotField(vp->comp(),
                                     node->getSymbolReference(),
                                     base->getClass(),
                                     base->isFixedClass());
  }
...
TR::Node*
constrainStore(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (globalStore) {
    ...
    if (sync && sync->syncEmitted() == TR_yes) {
      vp->addConstraintToList(NULL,
                              vp->_syncValueNumber,
                              vp->AbsoluteConstraint,
      ...
      if (vp->trace()) {
        if (sync)
          traceMsg(
            vp->comp(), "syncRequired is already setup at node [%p]\n", node);
        else
          traceMsg(
            vp->comp(), "No sync constraint found at node [%p]!\n", node);
      }
    }
  }
...
//
TR::Node*
constrainIntStore(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (loadNode->getOpCode().isLoadVarDirect() &&
      ...
      for (bni = vp->_booleanNegationInfo.getFirst(); bni;
        ...
        if (bni->_storeValueNumber == loadVN) {
          ...
          if (performTransformation(
                vp->comp(),
                "%sRemoving double boolean negation at [%p]\n",
                OPT_DETAILS,
                node)) {
            ...
            node->setNumChildren(1);
            vp->addBlockConstraint(
              node, TR::VPEqual::create(vp, 0), bni->_loadNode);
            return node;
          }
        }
      }
...
TR::Node*
constrainLongStore(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainStore(vp, node);
...
//
TR::Node*
constrainAstore(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainStore(vp, node);
...
void
canRemoveWrtBar(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (constraint) {
    if (constraint->isNullObject() &&
      ...
      if (node->getOpCode().isIndirect()) {
        ...
        if (performTransformation(
              vp->comp(),
              "%sChanging write barrier store into astore [%p]\n",
              OPT_DETAILS,
              node)) {
...
//
TR::Node*
constrainWrtBar(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainGoto(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainIgoto(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  for (auto edge = vp->_curBlock->getSuccessors().begin();
       edge != vp->_curBlock->getSuccessors().end();
       ++edge) {
    auto current = edge;
...
TR::Node*
constrainReturn(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainMonent(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainMonexit(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  vp->createExceptionEdgeConstraints(
    TR::Block::CanCatchMonitorExit, NULL, node);
  ...
  if (sync) {
    if (sync->syncEmitted() == TR_no) {
      ...
      if (vp->trace())
        traceMsg(
          vp->comp(), "syncRequired is already setup at monexit [%p]\n", node);
    }
    ...
    if (vp->trace()) {
      if (sync)
        ...
        traceMsg(
          vp->comp(), "No sync constraint found at monexit [%p]!\n", node);
    }
  }
...
TR::Node*
constrainMonexitfence(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainTstart(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainTfinish(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  return node;
}
...
TR::Node*
constrainTabort(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  return node;
}
...
TR::Node*
constrainThrow(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (node->throwInsertedByOSR()) {
    ...
    //
    vp->createExceptionEdgeConstraints(
      TR::Block::CanCatchUserThrows, NULL, node);
    vp->setUnreachablePath();
    return node;
  }
...
TR::Node*
constrainInstanceOf(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (objectConstraint) {
    ...
    if (objectConstraint->isNullObject()) {
      ...
      TR_YesNoMaybe isInstance =
        vp->fe()->isInstanceOf(objectClass,
                               castClass,
                               objectConstraint->isFixedClass(),
                               castConstraint->isFixedClass(),
                               !disabled);
      ...
      if (isInstance == TR_yes) {
        ...
        if (objectConstraint->asClass() && castConstraint->asClass()) {
          vp->checkTypeRelationship(
            objectConstraint, castConstraint, result, true, false);
...
TR::Node*
constrainCheckcast(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (objectConstraint) {
    if (objectConstraint->isNullObject()) {
      ...
      isInstance = vp->fe()->isInstanceOf(objectClass,
                                          castClass,
                                          objectConstraint->isFixedClass(),
                                          castConstraint->isFixedClass(),
                                          !disabled);
      ...
      if (isInstance == TR_yes) {
        ...
        if (objectConstraint->asClass() && castConstraint->asClass() &&
            objectConstraint->isNonNullObject()) {
          vp->checkTypeRelationship(
            objectConstraint, castConstraint, result, false, true);
  ...
  if (result != 1)
    vp->createExceptionEdgeConstraints(
      TR::Block::CanCatchCheckCast, NULL, node);
  if ((node->getOpCodeValue() == TR::checkcastAndNULLCHK) &&
      (!objectConstraint || !objectConstraint->isNonNullObject()))
    vp->createExceptionEdgeConstraints(
      TR::Block::CanCatchNullCheck, NULL, node);
            ...
            if (classType->getClass() == jlClass) {
              ...
              if ((classType->isClassObject() == TR_maybe) &&
                ...
                newClassType = TR::VPResolvedClass::create(
                  vp,
                  classType
                    ->getClass()); // Do not want to get the 'fixed' property
            }
          }
        } else {
...
TR::Node*
constrainBCDCHK(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  vp->createExceptionEdgeConstraints(TR::Block::CanCatchUserThrows, NULL, node);
  return node;
...
TR::Node*
constrainCheckcastNullChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainCheckcast(vp, node);
}

TR::Node*
constrainNew(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainNewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (sizeConstraint)
    vp->addGlobalConstraint(
      node,
      TR::VPArrayInfo::create(vp,
                              sizeConstraint->getLowInt(),
                              sizeConstraint->getHighInt(),
                              elementSize));
  else
    vp->addGlobalConstraint(
      node,
      TR::VPArrayInfo::create(
        vp, 0, TR::getMaxSigned<TR::Int32>(), elementSize));
...
TR::Node*
constrainVariableNew(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainVariableNewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  TR::Node* typeNode = node->getSecondChild();
  bool isGlobal;
...
TR::Node*
constrainANewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainMultiANewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  int32_t maxSize =
    (int32_t)TR::Compiler->om.maxArraySizeInElementsForAllocation(
      node, vp->comp()); // int32_t is ok because multiANewArray is only useful
                         // in Java, where array sizes are bound by
                         // TR::getMaxSigned<TR::Int32>()
  int64_t maxHeapSize;
  ...
  // Type constraints can be applied to this node.
  //
  TR::VPArrayInfo* arrayInfo = TR::VPArrayInfo::create(
    vp, sizeConstraint->getLowInt(), sizeConstraint->getHighInt(), elementSize);
  vp->addGlobalConstraint(
    node,
    TR::VPClass::create(
      vp,
...
static TR::VPLongRange*
getLongRange(TR::VPConstraint* c)
{
  return c->asLongRange();
}
static TR::VPLongConst*
getLongConst(TR::VPConstraint* c)
{
  return c->asLongConst();
}
static TR::VPIntRange*
getIntRange(TR::VPConstraint* c)
{
  return c->asIntRange();
}
static TR::VPIntConst*
getIntConst(TR::VPConstraint* c)
{
  return c->asIntConst();
}
static void
getLowHighInts(TR::VPIntRange* range, int32_t& low, int32_t& high)
{
  low = range->getLowInt();
  high = range->getHighInt();
}
static void
getInt(TR::VPIntConst* c, int32_t& n)
{
  n = c->getInt();
}
static void
getLowHighLongs(TR::VPLongRange* range, int64_t& low, int64_t& high)
{
  low = range->getLowLong();
  high = range->getHighLong();
}
static void
getLong(TR::VPLongConst* c, int64_t& n)
{
  n = c->getLong();
}

static TR::VPConstraint*
createIntConstConstraint(OMR::ValuePropagation* vp, int32_t n)
{
  return TR::VPIntConst::create(vp, n);
}
static TR::VPConstraint*
createIntRangeConstraint(OMR::ValuePropagation* vp, int32_t l, int32_t h)
{
  return TR::VPIntRange::create(vp, l, h);
}
static TR::VPConstraint*
createLongConstConstraint(OMR::ValuePropagation* vp, int64_t n)
{
  return TR::VPLongConst::create(vp, n);
}
static TR::VPConstraint*
createLongRangeConstraint(OMR::ValuePropagation* vp, int64_t l, int64_t h)
{
  return TR::VPLongRange::create(vp, l, h);
}

static int32_t
integerToPowerOf2(int32_t n)
{
  return (n == 0) ? 0 : floorPowerOfTwo(n);
}
static int32_t
integerNumberOfLeadingZeros(int32_t n)
{
  return leadingZeroes(n);
}
static int32_t
integerNumberOfTrailingZeros(int32_t n)
{
  return trailingZeroes(n);
}
static int32_t
integerBitCount(int32_t n)
{
  return populationCount(n);
}
static int64_t
longToPowerOf2(int64_t n)
{
  return (n == 0) ? 0 : floorPowerOfTwo64(n);
}
static int32_t
longNumberOfLeadingZeros(int64_t n)
{
  return leadingZeroes(n);
}
static int32_t
longNumberOfTrailingZeros(int64_t n)
{
  return trailingZeroes(n);
}
static int32_t
longBitCount(int64_t n)
{
  return populationCount(n);
}
...
void
addValidRangeBlockOrGlobalConstraint(OMR::ValuePropagation* vp,
                                     TR::Node* node,
                                     FUNC createRange,
                                     FUNC2 processValue,
                                     T low,
                                     ...
                                     bool childGlobal)
{
  ...
  vp->addBlockOrGlobalConstraint(
    node, createRange(vp, pLow, pHigh), childGlobal);
}
...
template<typename A,
         ...
         typename T>
static TR::Node*
constrainHighestOneBitAndLeadingZerosHelper(OMR::ValuePropagation* vp,
                                            TR::Node* node,
                                            A getConst,
                                            B getRange,
                                            C getValue,
                                            ...
                                            T MAX_VALUE)
{
  ...
  addValidRangeBlockOrGlobalConstraint(
    vp, node, createRange, processValue, MIN_VALUE, MAX_VALUE, childGlobal);
  return node;
}
...
template<typename A,
         ...
         typename T>
static TR::Node*
constrainLowestOneBitAndTrailingZerosHelper(OMR::ValuePropagation* vp,
                                            TR::Node* node,
                                            A getConst,
                                            B getRange,
                                            C getValue,
...
TR::Node*
constrainIntegerHighestOneBit(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainIntegerNumberOfLeadingZeros(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainHighestOneBitAndLeadingZerosHelper(
    vp,
    node,
...
TR::Node*
constrainLongHighestOneBit(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainHighestOneBitAndLeadingZerosHelper(
    vp,
    node,
...
TR::Node*
constrainLongNumberOfLeadingZeros(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainHighestOneBitAndLeadingZerosHelper(vp,
                                                     node,
                                                     getLongConst,
                                                     getLongRange,
                                                     getLong,
...
TR::Node*
constrainIntegerLowestOneBit(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainIntegerBitCount(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp,
                                                     node,
                                                     getIntConst,
                                                     getIntRange,
                                                     getInt,
...
TR::Node*
constrainIntegerNumberOfTrailingZeros(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(
    vp,
    node,
    ...
    createIntConstConstraint,
    createIntRangeConstraint,
    integerNumberOfTrailingZeros,
    (int32_t)0,
    (int32_t)-1);
}

TR::Node*
constrainLongLowestOneBit(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(
    vp,
    node,
...
TR::Node*
constrainLongNumberOfTrailingZeros(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp,
                                                     node,
                                                     getLongConst,
...
TR::Node*
constrainLongBitCount(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp,
                                                     node,
                                                     getLongConst,
...
//
TR::Node*
constrainArraylength(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
    ...
    if (knot && kobj) {
      ...
      if (constrainArraylengthCriticalSection.hasVMAccess()) {
        ...
        if (vp->comp()->fej9()->isClassArray(
          ...
          vp->replaceByConstant(
            node, TR::VPIntConst::create(vp, length), isGlobal);
          return node;
        }
      }
    }
  ...
  if (!elementSize) {
    ...
    if (elementSize) {
      constraint = TR::VPArrayInfo::create(
        vp, lowerBoundLimit, upperBoundLimit, elementSize);
...
static void
refineSymbolReferenceForProfiledToOverridden(OMR::ValuePropagation* vp,
                                             TR::Node* callNode,
...
refineMethodSymbolInCall(OMR::ValuePropagation* vp,
                         TR::Node* node,
                         TR::SymbolReference* symRef,
                         ...
                         int32_t offset)
{
  ...
  if (vp->trace())
    traceMsg(vp->comp(),
             "Refined method symbol to %s\n",
             resolvedMethod->signature(vp->trMemory()));
  return methodSymbol;
}

static void
devirtualizeCall(OMR::ValuePropagation* vp, TR::Node* node)
{
  TR::SymbolReference* symRef = node->getSymbolReference();
        ...
        if (resolvedMethod) {
          TR::SymbolReference* specimenSymRef =
            vp->getSymRefTab()->findOrCreateMethodSymbol(
              owningMethod->getResolvedMethodIndex(),
              -1,
              resolvedMethod,
          ...
          if (performTransformation(
                ...
                OPT_DETAILS,
                node,
                specimenSymRef->getName(vp->getDebug()))) {
        ...
        if (constr->asKnownObject() && constr->isNonNullObject()) {
          ...
          argInfo->set(c - firstArgIndex,
                       new (vp->trStackMemory())
                         TR_PrexArgument(TR_PrexArgument::ClassIsPreexistent));
          if (tracePrex)
            traceMsg(vp->comp(),
  ...
  if (tracePrex)
    traceMsg(vp->comp(),
             "PREX.vp: Done populating prex argInfo for %s %p\n",
             node->getOpCode().getName(),
             node);
...
TR::Node*
constrainCall(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (symbol) {
    ...
    if (isIntegerAbs || isLongAbs) {
      ...
      if (isIntegerAbs) {
        ...
        couldBeMinValue = (operand == NULL || operand->getLowInt() ==
        ...
        couldBeMinValue = (operand == NULL || operand->getLowLong() ==
                                                TR::getMinSigned<TR::Int64>());
      }
      ...
      if (couldBeMinValue) {
        ...
        absConstraint = TR::VPMergedConstraints::create(
          vp, minValueConstraint, positiveConstraint);
      } else
      ...
      if (jitHelpersClass &&
        ...
        vp->comp()->fej9()->getResolvedMethods(
          vp->trMemory(), jitHelpersClass, &helperMethods);
...
TR::Node*
constrainAcall(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainAdd(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
...
TR::Node*
constrainSubtract(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
...
TR::Node*
constrainImul(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
    ...
    if (lhs->asIntConst() && rhs->asIntConst()) {
      ...
      if (lowest < TR::getMinSigned<TR::Int32>() ||
        ...
        constraint = TR::VPIntRange::create(
          vp, (int32_t)lowest, (int32_t)highest /*, isUnsigned*/);
      }
    }
    if (constraint) {
      if (constraint->asIntConst()) {
...
bool
can64BitUnsignedMultiplyOverflow(uint64_t a, uint64_t b, uint64_t maxValue)
{
...
bool
can64BitSignedMultiplyOverflow(int64_t a, int64_t b)
{
  ...
  if (a > 0 && b > 0)
    return can64BitUnsignedMultiplyOverflow(
      (uint64_t)a, (uint64_t)b, TR::getMaxSigned<TR::Int32>());
  ...
  if (a < 0)
    return can64BitUnsignedMultiplyOverflow(
      (uint64_t)(-1 * a), (uint64_t)b, TR::getMaxSigned<TR::Int32>());
  else if (b < 0)
    return can64BitUnsignedMultiplyOverflow(
      (uint64_t)a, (uint64_t)(-1 * b), TR::getMaxSigned<TR::Int32>());

  return false;
}

bool
can64BitMultiplyOverflow(uint64_t a, uint64_t b, bool isUnsigned)
{
  if (isUnsigned)
    return can64BitUnsignedMultiplyOverflow(
      a, b, TR::getMaxUnsigned<TR::Int32>());
  else
    return can64BitSignedMultiplyOverflow((int64_t)a, (int64_t)b);
}

TR::Node*
constrainLmul(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (lhs && rhs) {
    ...
    if (lhs->asLongConst() && rhs->asLongConst()) {
      ...
      if (!(can64BitMultiplyOverflow(lhs->getLowLong(),
                                     rhs->getLowLong(),
                                     node->getOpCode().isUnsigned()) ||
            can64BitMultiplyOverflow(lhs->getLowLong(),
                                     rhs->getHighLong(),
                                     node->getOpCode().isUnsigned()) ||
            can64BitMultiplyOverflow(lhs->getHighLong(),
                                     rhs->getLowLong(),
                                     node->getOpCode().isUnsigned()) ||
            can64BitMultiplyOverflow(lhs->getHighLong(),
                                     rhs->getHighLong(),
...
static bool
isUnsafeDivide(int64_t dividend, int64_t divisor)
{
...
bool
constrainIntegerDivisionRange(int64_t lhsLow,
                              int64_t lhsHigh,
                              int64_t rhsLow,
                              int64_t rhsHigh,
                              int64_t rangeMin,
...
TR::Node*
cloneDivForDivideByZeroCheck(OMR::ValuePropagation* vp, TR::Node* divNode)
{
...
bool
doesRangeContainZero(int64_t rangeMin, int64_t rangeMax)
{
...
TR::Node*
constrainIdiv(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
    ...
    if (lhs->asIntConst() && rhs->asIntConst()) {
      ...
      if (lhsInt && rhsInt) {
        ...
        if (isUnsigned) {
          ...
          range_min = 0;
          range_max =
            TR::getMaxSigned<TR::Int32>(); // we don't support unsigned range
                                           // constraint yet. Only support upto
                                           // TR::getMaxSigned<TR::Int32>() for
                                           // unsigned
        } else {
...
TR::Node*
constrainLdiv(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
removeRedundantREM(OMR::ValuePropagation* vp,
                   TR::Node* node,
                   TR::VPConstraint* nodeConstraint,
...
TR::Node*
constrainIrem(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainLrem(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node)) {
    return node;
  }
...
TR::Node*
constrainIneg(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (child) {
    if (child->asIntConst()) {
      ...
      TR::VPConstraint* constraint = NULL;
      if (low == TR::getMinSigned<TR::Int32>()) {
        lowConstraint = TR::VPIntRange::create(vp,
                                               TR::getMinSigned<TR::Int32>(),
                                               TR::getMinSigned<TR::Int32>(),
                                               TR_yes);
        low++;
      ...
      if (high == TR::getMinSigned<TR::Int32>()) {
        highConstraint = TR::VPIntRange::create(vp,
                                                TR::getMinSigned<TR::Int32>(),
                                                TR::getMinSigned<TR::Int32>(),
                                                TR_yes);
        high++;
...
TR::Node*
constrainLneg(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (child) {
    if (child->asLongConst()) {
      ...
      if (low == TR::getMinSigned<TR::Int64>()) {
        lowConstraint = TR::VPLongRange::create(vp,
                                                TR::getMinSigned<TR::Int64>(),
                                                TR::getMinSigned<TR::Int64>(),
                                                TR_yes);
        low++;
      ...
      if (high == TR::getMinSigned<TR::Int64>()) {
        highConstraint = TR::VPLongRange::create(vp,
                                                 TR::getMinSigned<TR::Int64>(),
                                                 TR::getMinSigned<TR::Int64>(),
                                                 TR_yes);
        high++;
...
TR::Node*
constrainIabs(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainLabs(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainIshl(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainLshl(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
static TR::Node*
distributeShift(OMR::ValuePropagation* vp,
                TR::Node* node,
                int32_t shiftAmount,
                TR::VPConstraint*& constraint)
{
    ...
    if (replaceNode &&
      ...
      if (c) {
        if (node->getOpCodeValue() == TR::iushr) {
          ...
          constraint = TR::VPIntRange::create(
            vp, c->getLowInt() >> shiftAmount, c->getHighInt() >> shiftAmount);
      }
    }
  }
  return replaceNode;
...
TR::Node*
constrainIshr(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainLshr(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
...
TR::Node*
constrainIushr(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (low == high)
      ...
      constraint = TR::VPIntRange::create(vp,
                                          ...
                                          ((uint32_t)high) >>
                                            shiftAmount /*, isUnsigned*/);
    else if (high < 0 /*&& !isUnsigned*/)
      constraint = TR::VPIntRange::create(
        vp, ((uint32_t)high) >> shiftAmount, ((uint32_t)low) >> shiftAmount);
    // this path is probably never taken for unsigned
    else {
      if (shiftAmount > 0)
        constraint =
...
TR::Node*
constrainLushr(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (low == high)
      ...
      constraint = TR::VPLongRange::create(
        vp, ((uint64_t)low) >> shiftAmount, ((uint64_t)high) >> shiftAmount);
    else if (high < 0)
      constraint = TR::VPLongRange::create(
        vp, ((uint64_t)high) >> shiftAmount, ((uint64_t)low) >> shiftAmount);
    else {
      if (shiftAmount > 0)
        constraint =
...
TR::Node*
constrainIand(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
        ...
        if ((firstChild->getOpCodeValue() == TR::iloadi ||
          ...
          if (vftLoad->getOpCodeValue() == TR::aloadi ||
            ...
            if (baseExpressionConstraint &&
              ...
              if (vp->trace())
                traceMsg(
                  vp->comp(), "Fold isArray test in %p\n", baseExpression);
...
TR::Node*
constrainLand(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainIor(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainLor(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
TR::Node*
constrainIxor(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
  ...
  if (lhs && rhs && rhs->asIntConst()) {
    ...
    if (rhsConst == 1 && lhs->asIntRange()) {
      ...
      //
      constraint = TR::VPIntRange::create(
        vp, (lhs->getLowInt() & ~1), (lhs->getHighInt() | 1) /*, isUnsigned*/);
      if (constraint) {
        vp->addBlockOrGlobalConstraint(node, constraint, lhsGlobal);
      }
    }
...
TR::Node*
constrainLxor(OMR::ValuePropagation* vp, TR::Node* node)
{
  if (findConstant(vp, node))
    return node;
  constrainChildren(vp, node);
...
static TR::Node*
constrainWidenToLong(OMR::ValuePropagation* vp,
                     TR::Node* node,
                     int64_t low,
                     int64_t high,
                     bool isUnsigned)
...
TR::Node*
constrainI2l(OMR::ValuePropagation* vp, TR::Node* node)
{

  if (node->getFirstChild()->isNonNegative())
    node->setIsNonNegative(true);

  return constrainWidenToLong(vp,
                              node,
                              TR::getMinSigned<TR::Int32>(),
                              TR::getMaxSigned<TR::Int32>(),
                              false);
...
TR::Node*
constrainIu2l(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainWidenToLong(vp,
                              node,
...
void
replaceWithSmallerType(OMR::ValuePropagation* vp, TR::Node* node)
{
#if 1
...
static TR::Node*
constrainNarrowIntValue(OMR::ValuePropagation* vp,
                        TR::Node* node,
                        int32_t low,
                        int32_t high)
{
...
TR::Node*
constrainNarrowToByte(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainNarrowIntValue(
    vp, node, TR::getMinSigned<TR::Int8>(), TR::getMaxSigned<TR::Int8>());
}

TR::Node*
constrainNarrowToShort(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainNarrowIntValue(
    vp, node, TR::getMinSigned<TR::Int16>(), TR::getMaxSigned<TR::Int16>());
}

TR::Node*
constrainNarrowToChar(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (constraint) {
    if (constraint->asIntConstraint() || constraint->asMergedConstraints()) {
      ...
      if (constraint->getLowInt() < 0 &&
        ...
        low = high =
          max + constraint->getLowInt() % (TR::getMaxUnsigned<TR::Int16>() + 1);
    } else if (constraint->asLongConstraint() ||
               constraint->asMergedLongConstraints()) {
      if (constraint->getLowInt() > 0 &&
          constraint->getLowInt() < TR::getMaxUnsigned<TR::Int16>() &&
...
TR::Node*
constrainNarrowToInt(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainNarrowIntValue(
    vp, node, TR::getMinSigned<TR::Int32>(), TR::getMaxSigned<TR::Int32>());
}
...
static bool
constrainWidenToInt(OMR::ValuePropagation* vp,
                    TR::Node*& node,
                    int32_t low,
                    int32_t high,
                    bool isUnsigned,
...
TR::Node*
constrainB2i(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToInt(vp,
                      node,
...
TR::Node*
constrainB2s(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToInt(vp,
                      node,
                      TR::getMinSigned<TR::Int8>(),
...
TR::Node*
constrainBu2i(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToInt(vp,
                      node,
                      TR::getMinUnsigned<TR::Int8>(),
...
TR::Node*
constrainBu2s(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToInt(vp,
                      node,
                      TR::getMinUnsigned<TR::Int8>(),
...
TR::Node*
constrainB2l(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToLong(vp,
                       node,
                       TR::getMinSigned<TR::Int8>(),
                       TR::getMaxSigned<TR::Int8>(),
                       false);
...
TR::Node*
constrainBu2l(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToLong(vp,
                       node,
                       TR::getMinUnsigned<TR::Int8>(),
...
TR::Node*
constrainS2i(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToInt(vp,
                      node,
                      TR::getMinSigned<TR::Int16>(),
...
TR::Node*
constrainSu2i(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToInt(vp,
                      node,
                      TR::getMinUnsigned<TR::Int16>(),
                      TR::getMaxUnsigned<TR::Int16>(),
                      true,
                      TR::i2s);
  return node;
}
...
TR::Node*
constrainS2l(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToLong(vp,
                       node,
                       TR::getMinSigned<TR::Int16>(),
...
TR::Node*
constrainSu2l(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainWidenToLong(vp,
                       node,
                       TR::getMinUnsigned<TR::Int16>(),
...
static void
changeConditionalToGoto(OMR::ValuePropagation* vp,
                        TR::Node* node,
...
static void
removeConditionalBranch(OMR::ValuePropagation* vp,
                        TR::Node* node,
                        TR::CFGEdge* branchEdge)
{
#ifdef J9_PROJECT_SPECIFIC
...
static TR::VPConstraint*
passingTypeTestObjectConstraint(OMR::ValuePropagation* vp,
                                TR::VPConstraint* classConstraint,
...
static TR::Node*
getOriginalCallNode(TR_VirtualGuard* vGuard,
                    TR::Compilation* comp,
                    TR::Node* guardNode)
{
...
#ifdef J9_PROJECT_SPECIFIC
static int
numConcreteClasses(List<TR_PersistentClassInfo>* subClasses)
{
...
static void
addDelayedConvertedGuard(TR::Node* node,
                         TR::Node* callNode,
                         ...
                         TR_OpaqueClassBlock* objectClass)
{
  ...
  if (vp->trace()) {
    traceMsg(vp->comp(),
             "P2O: oldGuard %p newGuard %p currentTree %p\n",
             oldVirtualGuard,
             newGuardNode,
             vp->_curTree);
  }
...
#ifdef J9_PROJECT_SPECIFIC
TR_ResolvedMethod*
findSingleImplementer(TR_OpaqueClassBlock* thisClass,
                      ...
                      TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
  ...
  int32_t implCount =
    TR_ClassQueries::collectImplementorsCapped(classInfo,
                                               implArray,
                                               2,
                                               cpIndexOrVftSlot,
                                               callerMethod,
...
//
static TR::Node*
constrainIfcmpeqne(OMR::ValuePropagation* vp,
                   TR::Node* node,
                   bool branchOnEqual)
{
  constrainChildren(vp, node);
    ...
    if (lhs && rhs) {
      ...
      if (lhs->mustBeEqual(rhs, vp) && ignoreVirtualGuard) {
        if (vp->trace())
          traceMsg(
            vp->comp(),
            "   cmp children must be equal by absolute constraints: %p == %p\n",
            lhsChild,
            rhsChild);
        if (branchOnEqual)
          cannotFallThrough = true;
    ...
    if (rel && ignoreVirtualGuard) {
      ...
      if (rel->mustBeEqual()) {
        if (vp->trace())
          traceMsg(
            vp->comp(),
            "   cmp children must be equal by relative constraint: %p == %p\n",
            lhsChild,
            rhsChild);
        if (branchOnEqual)
          cannotFallThrough = true;
  ...
  if (cannotBranch &&
      performTransformation(vp->comp(),
                            "%sRemoving conditional branch [%p] %s\n",
                            OPT_DETAILS,
                            node,
                            node->getOpCode().getName())) {
    ...
    if (callNode && callNode->getOpCode().isCallIndirect()) {
      ...
      if (constraint && constraint->isFixedClass() &&
        ...
        if (resolvedMethodSymbol) {
          ...
          if ((vp->fe()->isInstanceOf(
                 thisType, originalMethodClass, true, true) == TR_yes) &&
  ...
  if (branchOnEqual) {
    ...
    if (notEqualConstraint) {
      if (!vp->addEdgeConstraint(
            constraintChild, notEqualConstraint, edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
          vp->setIntersectionFailed(false);
  ...
  if (instanceofConstraint && instanceofOnBranch) {
    TR::VPConstraint* newConstraint =
      passingTypeTestObjectConstraint(vp,
                                      instanceofConstraint,
                                      instanceofDetectedAndFixedType,
                                      instanceofObjectRefIsVft);

    if (!isVirtualGuardNopable && !vp->addEdgeConstraint(instanceofObjectRef,
                                                         newConstraint,
                                                         edgeConstraints)) {
  ...
  if (node->getOpCodeValue() != TR::ifacmpeq &&
    ...
    if (branchOnEqual) {
      if (!vp->addEdgeConstraint(
            lhsChild, TR::VPEqual::create(vp, 0), edgeConstraints, rhsChild)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
          vp->setIntersectionFailed(false);
      ...
      if (!vp->addEdgeConstraint(lhsChild,
                                 TR::VPNotEqual::create(vp, 0),
                                 edgeConstraints,
                                 rhsChild)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
  ...
  if (branchOnEqual) {
    if (notEqualConstraint) {
      if (!vp->addBlockConstraint(
            constraintChild, notEqualConstraint, NULL, false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
        else
          vp->setIntersectionFailed(false);
  ...
  if (instanceofConstraint && !instanceofOnBranch) {
    ...
    if (!isVirtualGuardNopable &&
        !vp->addBlockConstraint(
          instanceofObjectRef, newConstraint, NULL, false)) {
      TR_ASSERT_FATAL(!vp->intersectionFailed(), "VP intersection error");
      cannotFallThrough = true;
    }
  }
  ...
  if (node->getOpCodeValue() != TR::ifacmpeq &&
    ...
    if (branchOnEqual) {
      if (!vp->addBlockConstraint(
            lhsChild, TR::VPNotEqual::create(vp, 0), rhsChild, false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
        else
          vp->setIntersectionFailed(false);
      ...
      if (!vp->addBlockConstraint(
            lhsChild, TR::VPEqual::create(vp, 0), rhsChild, false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
        else
          vp->setIntersectionFailed(false);
  ...
  if (cannotBranch &&
    ...
    if (vp->trace()) {
      traceMsg(vp->comp(),
               "P2O: Considering a candidate at %p. Object Constraint : ",
               node);
      lhs->print(vp);
    ...
    if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
      ...
      if (rhs && !objectClass && lhs && lhs->getClassType() &&
        ...
        objectClass =
          vp->comp()->fe()->getClassFromSignature(sig, len, owningMethod, true);
        ...
        sig = classNameToSignature(
          interfaceMethod->classNameChars(), len, vp->comp());
        callClass =
          vp->comp()->fe()->getClassFromSignature(sig, len, owningMethod, true);
      }
      ...
      if (objectClass && rhsClass && callClass && !lhs->isFixedClass() &&
          vp->comp()->fe()->isInstanceOf(
            rhsClass, objectClass, true, true, true) == TR_yes &&
          vp->comp()->fe()->isInstanceOf(
            objectClass, callClass, true, true, true) ==
            TR_yes) /*the object class may be less specific than callClass*/
      {
        ...
        if (methodSymbol &&
          ...
          TR_ResolvedMethod* rvm =
            findSingleImplementer(objectClass,
                                  cpIndexOrVftSlot,
                                  symRef->getOwningMethod(vp->comp()),
                                  vp->comp(),
                                  false,
          ...
          if (objectClassInfo) {
            TR_ClassQueries::collectAllSubClasses(
              objectClassInfo, &subClasses, vp->comp());
          ...
          */
          ...
          if (searchSucceeded && numConcreteClasses(&subClasses) < 2 && rvm) {
            TR::ResolvedMethodSymbol* cMethodSymbol =
              vp->comp()
                ->getSymRefTab()
                ->findOrCreateMethodSymbol(
                  symRef->getOwningMethodIndex(), -1, rvm, methodKind)
                ->getSymbol()
...
TR::Node*
constrainIfcmpeq(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainIfcmpeqne(vp, node, true);
}

TR::Node*
constrainIfcmpne(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainIfcmpeqne(vp, node, false);
}

void
getLimits(OMR::ValuePropagation* vp,
          int32_t* zLow,
          int32_t* zHigh,
          TR::Node* symNode,
          bool isGlobal)
...
void
getConstValue(int32_t* cConst, TR::Node* constNode)
{
  *cConst = constNode->getInt();
}

void
getConstValue(int64_t* cConst, TR::Node* constNode)
{
  *cConst = constNode->getLongInt();
}

void
getExtremes(int32_t* lowEnd, int32_t* highEnd)
{
...
void
getExtremes(int64_t* lowEnd, int64_t* highEnd)
{
...
TR::Node*
makeNewRhsNode(OMR::ValuePropagation* vp,
               TR::Node* node,
               TR::Node* symNodeY,
               int32_t constDiff)
{
...
static TR::Node*
simplifyInequality(OMR::ValuePropagation* vp,
                   TR::Node* node,
                   TR::Node* lhsChild,
                   TR::Node* rhsChild,
                   bool isGlobal,
...
static int64_t
clippedDifference(int64_t left, int64_t right)
{
...
//
static TR::Node*
constrainIfcmplessthan(OMR::ValuePropagation* vp,
                       TR::Node* node,
                       TR::Node* lhsChild,
                       TR::Node* rhsChild,
                       bool orEqual)
{
  ...
  //
    ...
    if (lhsChild->getOpCode().isLong() && !isUnsigned) {
      simplifyInequality<int64_t>(
        vp, node, lhsChild, rhsChild, isGlobal, childrenReversed);
    } else if (!isUnsigned) {
      simplifyInequality<int32_t>(
        vp, node, lhsChild, rhsChild, isGlobal, childrenReversed);
    }
    ...
    if (rel) {
      ...
      if (rel->asEqual() && !cannotFallThrough && !cannotBranch &&
        ...
        int64_t maxIncrement =
          0; // If absIncrement is more than this, overflow may occur, and (for
             // example) a==b+k does not imply a>b
  ...
  if (lhsChild->getOpCode().isLong()) {
    ...
    if (longBoundary != TR::getMinSigned<TR::Int64>() && !isUnsigned) {
      if (!vp->addEdgeConstraint(
            rhsChild,
            TR::VPLongRange::create(
              vp, longBoundary, TR::getMaxSigned<TR::Int64>()),
            edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
    ...
    if (longBoundary != TR::getMaxSigned<TR::Int64>()) {
      if (!vp->addEdgeConstraint(
            lhsChild,
            TR::VPLongRange::create(vp, minLongValue, longBoundary),
            edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
    ...
    bool modifiedBoundary =
      /*isUnsigned ? ((uint32_t)intBoundary != TR::getMinUnsigned<TR::Int32>())
        :*/
    ...
    intBoundary -= adjustment;
    modifiedBoundary =
      /*isUnsigned ? ((uint32_t)intBoundary != TR::getMaxUnsigned<TR::Int32>())
        :*/
  ...
  if (lhsChild->getOpCode().isLong() && !isUnsigned) {
    ...
    if (longBoundary != TR::getMaxSigned<TR::Int64>()) {
      if (!vp->addBlockConstraint(
            rhsChild,
            TR::VPLongRange::create(
              vp, TR::getMinSigned<TR::Int64>(), longBoundary),
            NULL,
            false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
...
TR::Node*
constrainIfcmplt(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainIfcmplessthan(
    vp, node, node->getFirstChild(), node->getSecondChild(), false);
}

TR::Node*
constrainIfcmple(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainIfcmplessthan(
    vp, node, node->getFirstChild(), node->getSecondChild(), true);
}

TR::Node*
constrainIfcmpgt(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainIfcmplessthan(
    vp, node, node->getSecondChild(), node->getFirstChild(), false);
}

TR::Node*
constrainIfcmpge(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainIfcmplessthan(
    vp, node, node->getSecondChild(), node->getFirstChild(), true);
}

TR::Node*
constrainCondBranch(OMR::ValuePropagation* vp, TR::Node* node)
{
...
//
static TR::Node*
constrainCmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool testEqual)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainCmpeq(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainCmpeqne(vp, node, true);
}

TR::Node*
constrainCmpne(OMR::ValuePropagation* vp, TR::Node* node)
{
...
//
static TR::Node*
constrainCmplessthan(OMR::ValuePropagation* vp,
                     TR::Node* node,
                     TR::Node* lhsChild,
                     TR::Node* rhsChild,
                     bool orEqual)
{
  ...
  if (result >= 0) {
    if ((lhsGlobal || vp->lastTimeThrough()) &&
        performTransformation(vp->comp(),
                              "%sChanging node [%p] %s into constant %d\n",
                              OPT_DETAILS,
                              node,
                              node->getOpCode().getName(),
                              result)) {
...
TR::Node*
constrainCmplt(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainCmplessthan(
    vp, node, node->getFirstChild(), node->getSecondChild(), false);
}

TR::Node*
constrainCmple(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainCmplessthan(
    vp, node, node->getFirstChild(), node->getSecondChild(), true);
}
...
TR::Node*
constrainCmpgt(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainCmplessthan(
    vp, node, node->getSecondChild(), node->getFirstChild(), false);
}
...
TR::Node*
constrainCmpge(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainCmplessthan(
    vp, node, node->getSecondChild(), node->getFirstChild(), true);
}
...
TR::Node*
constrainCmp(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainFloatCmp(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainSwitch(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (!isInt64 && constraint && constraint->asIntConstraint()) {
      ...
      for (i = node->getNumChildren() - 1; i > 1; i--) {
        ...
        if (/*!isUnsigned &&*/ ((low < value && high < value) ||
          ...
          if (vp->trace())
            traceMsg(vp->comp(),
                     "   Case %d (target %d) is unreachable\n",
                     value,
                     target->getNumber());
          node->removeChild(i);
          casesRemoved = true;
      ...
      if (casesRemoved) {
        ...
        for (auto e = vp->_curBlock->getSuccessors().begin();
             e != vp->_curBlock->getSuccessors().end();
             ++e) {
          if (((*e) == defaultEdge) ||
      ...
      if (low >= minCase && high <= maxCase)
        node->setCannotOverflow(
          true); // Let evaluator know value range of selector is covered by min
                 // and max case
    }            // if IntConstraint
...
TR::Node*
constrainCase(OMR::ValuePropagation* vp, TR::Node* node)
{
  // Put the current list of block constraints on to the edge
  //
  TR::Block* target = node->getBranchDestination()->getNode()->getBlock();
...
static void
constrainClassObjectLoadaddr(OMR::ValuePropagation* vp,
                             TR::Node* node,
                             bool isGlobal)
{
...
TR::Node*
constrainLoadaddr(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (symbol->isAddressOfClassObject()) {
    ...
    TR::VPConstraint* constraint = TR::VPClass::create(
      vp,
      TR::VPClassType::create(vp, symRef, false, true),
      NULL,
      NULL,
      NULL,
      TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject));
...
void
constrainNewlyFoldedConst(OMR::ValuePropagation* vp,
                          TR::Node* node,
                          bool isGlobal)
{
  switch (node->getOpCodeValue()) {
...
//
static int32_t
handleNullCheck(OMR::ValuePropagation* vp, TR::Node* node, bool resolveChkToo)
{
...
//
static bool
handleResolveCheck(OMR::ValuePropagation* vp, TR::Node* node, bool nullChkToo)
{
  ...
  if (nullChkToo)
    vp->createExceptionEdgeConstraints(TR::Block::CanCatchNullCheck |
                                         TR::Block::CanCatchResolveCheck,
                                       NULL,
                                       node);
  else
    vp->createExceptionEdgeConstraints(
      TR::Block::CanCatchResolveCheck, NULL, node);
...
TR::Node*
constrainAsm(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  vp->createExceptionEdgeConstraints(TR::Block::CanCatchUserThrows, NULL, node);
  return node;
...
TR::Node*
constrainNullChk(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainZeroChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  TR::Node* child = node->getFirstChild();
  bool isGlobal;
...
TR::Node*
constrainResolveChk(OMR::ValuePropagation* vp, TR::Node* node)
{
...
TR::Node*
constrainResolveNullChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  // See if the resolve check can be eliminated
  //
...
TR::Node*
constrainOverflowChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  return node;
}
...
TR::Node*
constrainUnsignedOverflowChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  return node;
}
...
TR::Node*
constrainDivChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainTRT(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainBndChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (relativeConstraint && relativeConstraint->mustBeLessThan() &&
      performTransformation(
        vp->comp(),
        "%sRemoving redundant bound check node (subsumed) [%p]\n",
        OPT_DETAILS,
        node)) {
    // Change this node to a treetop containing the index (which is
    // currently the second child of the bounds check).
...
TR::Node*
constrainBndChkWithSpineChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainArrayCopyBndChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (isArrayLengthCheck) {
    ...
    vp->addBlockConstraint(
      objectRef,
      TR::VPArrayInfo::create(
        vp, lhsConstraint->getLowInt(), lhsConstraint->getHighInt(), 0));
  }
  return node;
}

TR::Node*
constrainArrayStoreChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
        ...
        //
          ...
          if (isInstance == TR_yes) {
            ...
            if (vp->trace())
              traceMsg(
                vp->comp(),
                "Setting arrayStoreClass on ArrayStoreChk node [%p] to [%p]\n",
                node,
                objectClass);
            node->setArrayStoreClassInNode(objectClass);
                   ...
                   arrayComponentClass && objectClass &&
                   vp->fe()->isInstanceOf(
                     objectClass, arrayComponentClass, true, true)) {
            if (vp->trace())
              traceMsg(vp->comp(),
  ...
  if (canBeRemoved) {
    ...
    if (performTransformation(
          vp->comp(),
          "%sRemoving redundant arraystore check node [%p]\n",
          OPT_DETAILS,
          node)) {
  ...
  vp->createExceptionEdgeConstraints(
    TR::Block::CanCatchArrayStoreCheck, NULL, node);
...
TR::Node*
constrainArrayChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
...
TR::Node*
constrainArraycopy(OMR::ValuePropagation* vp, TR::Node* node)
{
  constrainChildren(vp, node);
  ...
  if (len && node->getNumChildren() == 3 && len->asIntConst() &&
      len->getLowInt() == 0 &&
      performTransformation(
        vp->comp(), "%sRemoving arraycopy node [%p]\n", OPT_DETAILS, node)) {
...
TR::Node*
constrainIntegralToBCD(OMR::ValuePropagation* vp, TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
  constrainChildren(vp, node);
  ...
  if (performTransformation(
        vp->comp(),
        "%sSetting source precision on node %s [0x%x] to %d\n",
        OPT_DETAILS,
        node->getOpCode().getName(),
        node,
        prec))
...
TR::Node*
constrainBCDToIntegral(OMR::ValuePropagation* vp, TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
  constrainChildren(vp, node);
