...
static void checkForNonNegativeAndOverflowProperties(OMR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* constraint = NULL)
{
    ...
    if (constraint) {
        ...
        if (constraint->asIntRange()) {
            ...
            if ((node->getOpCode().isLoad() && ((low > TR::getMinSigned<TR::Int32>()) || (high < TR::getMaxSigned<TR::Int32>()))) || (node->getOpCode().isArithmetic() && (range->canOverflow() != TR_yes))) {
                node->setCannotOverflow(true);
            ...
            if ((node->getOpCode().isLoad() && ((low > TR::getMinSigned<TR::Int64>()) || (high < TR::getMaxSigned<TR::Int64>()))) || (node->getOpCode().isArithmetic() && (range->canOverflow() != TR_yes))) {
                node->setCannotOverflow(true);
                //dumpOptDetails(vp->comp(), "Node %p cannot overflow\n", node);
            }
            ...
            if ((node->getOpCode().isLoad() && ((low > TR::getMinSigned<TR::Int16>()) || (high < TR::getMaxSigned<TR::Int16>()))) || (node->getOpCode().isArithmetic() && (range->canOverflow() != TR_yes))) {
                node->setCannotOverflow(true);
            }
        }
    }
        ...
        for (; !baseExpression;) {
            ...
            if (!constraint) {
                ...
                if (prevNodeSym->isArrayShadowSymbol() &&
                    ...
                    !prevNodeSym->isUnsafeShadowSymbol() && !vp->comp()->getSymRefTab()->isImmutableArrayShadow(prevNode->getSymbolReference())) {
    ...
    if (constraint) {
        ...
        case TR::Double:
            if (constraint->asLongConst()) {
                ...
                if (!vp->cg()->materializesLargeConstants() || (!node->getType().isInt64()) || (constraint->asLongConst()->getLong() < vp->cg()->getSmallestPosConstThatMustBeMaterialized() && constraint->asLongConst()->getLong() > vp->cg()->getLargestNegConstThatMustBeMaterialized()) || (vp->getCurrentParent()->getOpCode().isMul() && (vp->getCurrentParent()->getSecondChild() == node) && isNonNegativePowerOf2(constraint->asLongConst()->getLong()))) {
        ...
        default:
            if (constraint->asIntConstraint()) {
                ...
                if (constraint->asIntConst()) {
                    bool replacedConst = false;
                    if (!vp->cg()->materializesLargeConstants() || (!node->getType().isInt32()) || (value < vp->cg()->getSmallestPosConstThatMustBeMaterialized() && value > vp->cg()->getLargestNegConstThatMustBeMaterialized()) || (vp->getCurrentParent()->getOpCode().isMul() && (vp->getCurrentParent()->getSecondChild() == node) && isNonNegativePowerOf2(value))) {
                        vp->replaceByConstant(node, constraint, isGlobal);
                        replacedConst = true;
                    }
                    if (value)
                ...
                if ((node->getOpCode().isArithmetic() || node->getOpCode().isLoad()) && ((value > TR::getMinSigned<TR::Int32>()) || (constraint->getHighInt() < TR::getMaxSigned<TR::Int32>()))) {
                    node->setCannotOverflow(true);
                }
                ...
                if (constraint->asShortConst()) {
                    bool replacedConst = false;
                    if (!vp->cg()->materializesLargeConstants() || (!node->getType().isInt16()) || (value < vp->cg()->getSmallestPosConstThatMustBeMaterialized() && value > vp->cg()->getLargestNegConstThatMustBeMaterialized()) || (vp->getCurrentParent()->getOpCode().isMul() && vp->getCurrentParent()->getSecondChild() == node && isNonNegativePowerOf2(value))) {
                        vp->replaceByConstant(node, constraint, isGlobal);
                        replacedConst = true;
                    }
                    if (value)
                ...
                if ((node->getOpCode().isArithmetic() || node->getOpCode().isLoad()) && ((value > TR::getMinSigned<TR::Int16>()) || (constraint->getHighShort() < TR::getMaxSigned<TR::Int16>()))) {
                    node->setCannotOverflow(true);
                }
            }
            break;
...
static bool containsUnsafeSymbolReference(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (node->getSymbolReference()->getSymbol()->isShadow()) {
        ...
        if (node->getSymbolReference() == vp->getSymRefTab()->findInstanceShapeSymbolRef() || node->getSymbolReference() == vp->getSymRefTab()->findInstanceDescriptionSymbolRef() || node->getSymbolReference() == vp->getSymRefTab()->findDescriptionWordFromPtrSymbolRef() ||
#ifdef J9_PROJECT_SPECIFIC
...
TR::Node* constrainBCDAggrLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (signConstraint && TR_ENABLE_BCD_SIGN && signConstraint->getDataType() == node->getDataType()) {
        ...
        case TR_Sign_Clean:
            if (!node->hasKnownCleanSign() && performTransformation(vp->comp(), "%sTransfer sign constraint %s to %s (0x%p)\n", OPT_DETAILS, signConstraint->getName(), node->getOpCode().getName(), node)) {
        ...
        case TR_Sign_Preferred:
            if (!node->hasKnownPreferredSign() && performTransformation(vp->comp(), "%sTransfer sign constraint %s to %s (0x%p)\n", OPT_DETAILS, signConstraint->getName(), node->getOpCode().getName(), node)) {
                if (vp->comp()->cg()->traceBCDCodeGen())
                    traceMsg(vp->comp(), "y^y: VP: Transfer sign constraint %s to %s (0x%p)\n",
                        signConstraint->getName(), node->getOpCode().getName(), node);
            ...
            break;
        case TR_Sign_Plus:
            if (!node->knownSignCodeIs(bcd_plus) && performTransformation(vp->comp(), "%sTransfer sign constraint %s to %s (0x%p)\n", OPT_DETAILS, signConstraint->getName(), node->getOpCode().getName(), node)) {
                if (vp->comp()->cg()->traceBCDCodeGen())
                    traceMsg(vp->comp(), "y^y: VP: Transfer sign constraint %s to %s (0x%p)\n",
                        signConstraint->getName(), node->getOpCode().getName(), node);
            ...
            break;
        case TR_Sign_Minus:
            if (!node->knownSignCodeIs(bcd_minus) && performTransformation(vp->comp(), "%sTransfer sign constraint %s to %s (0x%p)\n", OPT_DETAILS, signConstraint->getName(), node->getOpCode().getName(), node)) {
                if (vp->comp()->cg()->traceBCDCodeGen())
                    traceMsg(vp->comp(), "y^y: VP: Transfer sign constraint %s to %s (0x%p)\n",
                        signConstraint->getName(), node->getOpCode().getName(), node);
        ...
        case TR_Sign_Unsigned:
            if (!node->knownSignCodeIs(bcd_unsigned) && performTransformation(vp->comp(), "%sTransfer sign constraint %s to %s (0x%p)\n", OPT_DETAILS, signConstraint->getName(), node->getOpCode().getName(), node)) {
                if (vp->comp()->cg()->traceBCDCodeGen())
                    traceMsg(vp->comp(), "y^y: VP: Transfer sign constraint %s to %s (0x%p)\n",
                        signConstraint->getName(), node->getOpCode().getName(), node);
        ...
        case TR_Sign_Minus_Clean:
            if (!(node->knownSignCodeIs(bcd_minus) && node->hasKnownCleanSign()) && performTransformation(vp->comp(), "%sTransfer sign constraint %s to %s (0x%p)\n", OPT_DETAILS, signConstraint->getName(), node->getOpCode().getName(), node)) {
                if (vp->comp()->cg()->traceBCDCodeGen())
                    traceMsg(vp->comp(), "y^y: VP: Transfer sign constraint %s to %s (0x%p)\n",
                        signConstraint->getName(), node->getOpCode().getName(), node);
                node->setKnownSignCode(bcd_minus);
    ...
    if (node->getOpCode().isIndirect() && !vp->_curTree->getNode()->getOpCode().isNullCheck() && owningMethodDoesNotContainNullChecks(vp, node)) {
...
TR::Node* constrainAnyIntLoad(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    if (node->getOpCode().isIndirect() && node->getSymbol()->isArrayShadowSymbol() && node->getFirstChild()->getOpCode().isAdd()) {
...
bool simplifyJ9ClassFlags(OMR::ValuePropagation* vp, TR::Node* node, bool isLong)
{
    ...
    if (symRef == vp->comp()->getSymRefTab()->findClassAndDepthFlagsSymbolRef()) {
        if (node->getFirstChild()->getOpCode().hasSymbolReference() && (node->getFirstChild()->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef())) {
            TR::VPConstraint* baseObject = vp->getConstraint(node->getFirstChild()->getFirstChild(), isGlobal);
            if (baseObject && baseObject->getClassType() && baseObject->getClassType()->asFixedClass()) {
...
static void checkUnsafeArrayAccess(OMR::ValuePropagation* vp, TR::Node* node)
{
    if (node->getSymbol()->isUnsafeShadowSymbol()) {
        ...
        if (node->getFirstChild()->getOpCode().isArrayRef() && node->getFirstChild()->getFirstChild()->getOpCode().isRef()) {
            TR::Node* baseAddress = node->getFirstChild()->getFirstChild();
            bool isGlobal;
            TR::VPConstraint* baseConstraint = vp->getConstraint(baseAddress, isGlobal);

            if (baseConstraint && baseConstraint->getClassType() && baseConstraint->getClassType()->isArray()) {
                if (vp->trace())
...
TR::Node* constrainAload(OMR::ValuePropagation* vp, TR::Node* node)
{
        ...
        if (!symRef->getSymbol()->isArrayShadowSymbol()) {
            ...
            if (haveClassLookaheadInfo) {
                ...
                if (sym->isStatic() && sym->isFinal()) {
                    ...
                    TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(classOfStatic, vp->comp(), allowForAOT);
                    if (classInfo && classInfo->isInitialized())
                        isClassInitialized = true;

                    if (classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized && (type == TR::Address)) {
                ...
                if (!foundInfo) {
                    TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(symRef->getOwningMethod(vp->comp())->classOfStatic(symRef->getCPIndex()), vp->comp(), allowForAOT);
    ...
    if (vp->comp()->getCurrentMethod()->isJ9() && symRef && (symRef == vp->comp()->getSymRefTab()->findOrCreateExcpSymbolRef() && vp->_curBlock->isCatchBlock() && vp->_curBlock->getExceptionClassNameChars())) {
...
static TR::Node* findArrayIndexNode(OMR::ValuePropagation* vp, TR::Node* node, int32_t stride)
{
    ...
    if (usingAladd) {
        int32_t constValue;
        if ((offset->getOpCode().isAdd() || offset->getOpCode().isSub()) && offset->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if ((offset->getOpCode().isAdd() && (offset->getSecondChild()->getOpCode().isLoadConst() && (constValue == TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))) || (offset->getOpCode().isSub() && (offset->getSecondChild()->getOpCode().isLoadConst() && (constValue == -(int32_t)TR::Compiler->om.contiguousArrayHeaderSizeInBytes())))) {
        ...
        if ((offset->getOpCode().isAdd() && (offset->getSecondChild()->getOpCode().isLoadConst() && (offset->getSecondChild()->getInt() == TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))) || (offset->getOpCode().isSub() && (offset->getSecondChild()->getOpCode().isLoadConst() && (offset->getSecondChild()->getInt() == -(int32_t)TR::Compiler->om.contiguousArrayHeaderSizeInBytes())))) {
            TR::Node* offset2 = offset->getFirstChild();
            if (offset2->getOpCodeValue() == TR::imul) {
                TR::Node* mulStride = offset2->getSecondChild();
                if (mulStride->getOpCode().isLoadConst() && mulStride->getInt() == stride) {
                    return offset2->getFirstChild();
                }
            } else if (stride == 1)
                return offset2;
...
TR::Node* constrainAddressRef(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    TR::Node* parent = vp->getCurrentParent();
    if (node->getFirstChild()->getOpCode().isLoadVar() && parent && ((parent->getOpCode().isLoadIndirect() || parent->getOpCode().isStoreIndirect()) && (parent->getFirstChild() == node))) {
...
TR::Node* constrainIaload(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if (base && node->getOpCode().hasSymbolReference() && (node->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef())) {
        ...
        // hence can be treated as known object.
        {
...

                {
                    ...
                    if (constrainIaloadCriticalSection.hasVMAccess()) {
                        ...
                        if (TR::Compiler->cls.isClassInitialized(vp->comp(), clazz)) {
                            ...
                            if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), clazz) && (TR::Compiler->cls.isClassFinal(vp->comp(), clazz) || TR::Compiler->cls.isPrimitiveClass(vp->comp(), clazz))) {
            ...
            if (knot && symRef == vp->comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) {
                ...
                // but clearly others make invalid assumptions on seeing this fixed class constraint so be conservative
                // and don't let the world know if we aren't sure the elements match this type
                if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), componentClass) && (TR::Compiler->cls.isClassFinal(vp->comp(), componentClass) || TR::Compiler->cls.isPrimitiveClass(vp->comp(), componentClass))) {
                    vp->addBlockOrGlobalConstraint(node,
                        TR::VPClass::create(vp,
        ...
        if (!symRef->getSymbol()->isArrayShadowSymbol()) {
            TR::Symbol* sym = symRef->getSymbol();
            bool haveClassLookaheadInfo = false;
            if (((sym->isShadow() && node->getFirstChild()->isThisPointer()) || sym->isStatic()) && !symRef->isUnresolved() && (sym->isPrivate() || sym->isFinal()))
                haveClassLookaheadInfo = true;
            ...
            if (haveClassLookaheadInfo) {
                ...
                if (!foundInfo) {
                    TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(vp->comp()->getCurrentMethod()->containingClass(), vp->comp());
                    if (classInfo && classInfo->getFieldInfo()) {
                        TR_PersistentFieldInfo* fieldInfo = classInfo->getFieldInfo()->findFieldInfo(vp->comp(), node, false);
                        if (fieldInfo) {
                            TR_PersistentArrayFieldInfo* arrayFieldInfo = fieldInfo ? fieldInfo->asPersistentArrayFieldInfo() : 0;
                            if (arrayFieldInfo && arrayFieldInfo->isDimensionInfoValid()) {
                                ...
                                if (arrLength >= 0 && sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                                    elementSize = arrayElementSize(sig, len, node, vp);
                                    if (elementSize != 0) {
                                        if (vp->trace())
            ...
            if (constraint && constraint->getClass()) {
                ...
                if (objectSig && (len == 19) && constraint->isFixedClass() && !strncmp(objectSig, "Ljava/util/TreeMap;", 19)) {
                    TR_ResolvedMethod* method = node->getSymbolReference()->getOwningMethod(vp->comp());
                    ...
                    if (clazz) {
                        ...
                        if (jlClass) {
                            if (clazz != jlClass) {
                                ...
                                char* fieldName = NULL;
                                if (node && node->getOpCode().hasSymbolReference() && node->getSymbolReference()->getSymbol()->isShadow()) {
                                ...
                                if (fieldName && (fieldNameLen > 0) && !strncmp(fieldName, "java/util/TreeMap.valuesCollection", 34)) {
    ...
    //
    if (sig && !(node->getOpCode().hasSymbolReference() && node->getSymbol()->isArrayShadowSymbol() && node->getFirstChild()->getOpCode().isArrayRef())) {
        TR_ResolvedMethod* method = node->getSymbolReference()->getOwningMethod(vp->comp());
...
#ifdef J9_PROJECT_SPECIFIC
    if (node->getOpCode().hasSymbolReference() && node->getSymbol()->isArrayShadowSymbol() && node->getFirstChild()->getOpCode().isArrayRef()) {
        TR::Node* underlyingArray = node->getFirstChild()->getFirstChild();
        if (vp->comp()->generateArraylets() && underlyingArray && underlyingArray->getOpCode().hasSymbolReference() && underlyingArray->getSymbol()->isArrayletShadowSymbol() && underlyingArray->getFirstChild()->getOpCode().isArrayRef()) {
                ...
                if (vp->comp()->getMethodHotness() >= scorching) {
                    ...
                    if (prefetchOffset >= 0) {
                        if (vp->comp()->findPrefetchInfo(node) < 0) {
                            TR_Pair<TR::Node, uint32_t>* prefetchInfo = new (vp->comp()->trHeapMemory()) TR_Pair<TR::Node, uint32_t>(node, (uint32_t*)(intptr_t)prefetchOffset);
                ...
                if (clazz) {
                    ...
                    if (jlClass) {
                        if (clazz != jlClass) {
                            int32_t fieldNameLen = -1;
                            char* fieldName = NULL;
                            if (underlyingArray && underlyingArray->getOpCode().hasSymbolReference() && (underlyingArray->getSymbolReference()->getSymbol()->isStaticField() || underlyingArray->getSymbolReference()->getSymbol()->isShadow())) {
                            ...
                            if (fieldName && (fieldNameLen > 0) && (!strncmp(fieldName, "java/math/BigDecimal.CACHE0", 27) || !strncmp(fieldName, "java/math/BigDecimal.CACHE1", 27) || !strncmp(fieldName, "java/math/BigDecimal.CACHE2", 27)))
                                constraint = TR::VPFixedClass::create(vp, clazz);
                            else
    ...
    if (base && base->getClass() && !vp->comp()->getOption(TR_DisableMarkingOfHotFields) && node->getFirstChild()->getOpCode().hasSymbolReference() && node->getFirstChild()->getSymbol()->isCollectedReference() && vp->_curBlock->getGlobalNormalizedFrequency(vp->comp()->getFlowGraph()) >= TR::Options::_hotFieldThreshold) {
        vp->comp()->fej9()->markHotField(vp->comp(), node->getSymbolReference(), base->getClass(), base->isFixedClass());
    }

    if (node->getSymbolReference()) {
        if (!node->getSymbolReference()->isUnresolved() && (node->getSymbolReference()->getSymbol()->getKind() == TR::Symbol::IsShadow) && (node->getSymbolReference()->getCPIndex() >= 0) && node->getSymbolReference()->getOwningMethod(vp->comp())) {
            int32_t fieldSigLen;
            const char* fieldSig = node->getSymbolReference()->getOwningMethod(vp->comp())->fieldSignatureChars(node->getSymbolReference()->getCPIndex(), fieldSigLen);
            int32_t fieldNameLen;
            const char* fieldName = node->getSymbolReference()->getOwningMethod(vp->comp())->fieldNameChars(node->getSymbolReference()->getCPIndex(), fieldNameLen);
...
TR::Node* constrainStore(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    // if the node contains an unsafe symbol reference
    // then don't inject a constraint
    if (containsUnsafeSymbolReference(vp, node) || (node->getSymbol()->isAutoOrParm() && node->storedValueIsIrrelevant()))
        return node;
...
TR::Node* constrainIntStore(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (child->getOpCodeValue() == TR::ixor && child->getSecondChild()->getOpCodeValue() == TR::iconst && child->getSecondChild()->getInt() == 1) {
...
TR::Node* constrainWrtBar(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    if (!vp->getCurrentParent() || (vp->getCurrentParent() && vp->getCurrentParent()->getOpCodeValue() != TR::ArrayStoreCHK))
    ...
    if (doOpt && ((gcMode == gc_modron_wrtbar_cardmark_and_oldcheck) || (gcMode == gc_modron_wrtbar_oldcheck)) && (node->getOpCodeValue() == TR::awrtbari) && !node->skipWrtBar()) {
        ...
        if ((valueChild->getOpCodeValue() == TR::New) || (valueChild->getOpCodeValue() == TR::newarray) || (valueChild->getOpCodeValue() == TR::anewarray) || (valueChild->getOpCodeValue() == TR::multianewarray) || (valueChild && (valueChild->getOpCodeValue() == TR::loadaddr) && valueChild->getSymbolReference()->getSymbol()->isAuto() && valueChild->getSymbolReference()->getSymbol()->isLocalObject()))
            removeWrtBar = true;
        else if (valueChild->getOpCodeValue() == TR::aload) {
            TR_UseDefInfo* useDefInfo = vp->_useDefInfo;

            if (valueChild->isThisPointer() && vp->comp()->getJittedMethodSymbol()->getResolvedMethod()->isConstructor()) {
                removeWrtBar = true;
                ...
                if (useDefInfo->isUseIndex(useIndex)) {
                    ...
                    if (useDefInfo->getUseDef(defs, useIndex)) {
                        ...
                        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                            ...
                            if (defNode && (defNode->getOpCodeValue() == TR::astore) && (defNode->getNumChildren() > 0)) {
                                TR::Node* defChild = defNode->getFirstChild();

                                if ((defChild->getOpCodeValue() == TR::New) || (defChild->getOpCodeValue() == TR::newarray) || (defChild->getOpCodeValue() == TR::anewarray) || (defChild->getOpCodeValue() == TR::multianewarray) || (defChild && (defChild->getOpCodeValue() == TR::loadaddr) && defChild->getSymbolReference()->getSymbol()->isAuto() && defChild->getSymbolReference()->getSymbol()->isLocalObject()))
                                    removeWrtBar = true;
                                /*
        ...
        if (removeWrtBar && performTransformation(vp->comp(), "%sChanging wrtbar to store because the rhs is a new object [%p]\n", OPT_DETAILS, node)) {
    ...
    if ((node->getOpCodeValue() == TR::awrtbari) && !node->getSymbolReference()->getSymbol()->isArrayShadowSymbol() && !vp->comp()->getOption(TR_DisableWriteBarriersRangeCheck)) {
                    ...
                    if (useDefInfo->getUseDef(defs, useIndex)) {
                        ...
                        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                            ...
                            if (defNode && (defNode->getOpCodeValue() == TR::astore) && (defNode->getNumChildren() > 0)) {
                                ...
                                if (defChild && (defChild->getOpCodeValue() == TR::loadaddr) && defChild->getSymbolReference()->getSymbol()->isAuto() && defChild->getSymbolReference()->getSymbol()->isLocalObject()) {
                                    setStackWrtBar = true;
                                    break;
                                } else if (defChild && (defChild->getOpCodeValue() == TR::aload) && defChild->getSymbolReference()->getSymbol()->isAuto()) {
                                    ...
                                    if (useDefInfo->isUseIndex(useIndexA)) {
                                        ...
                                        if (useDefInfo->getUseDef(defsA, useIndexA)) {
                                            ...
                                            for (cursorA.SetToFirstOne(); cursorA.Valid(); cursorA.SetToNextOne()) {
                                                ...
                                                if (defNodeA && (defNodeA->getOpCodeValue() == TR::astore) && (defNodeA->getNumChildren() > 0)) {
                                                    TR::Node* defChildA = defNodeA->getFirstChild();
                                                    if (defChildA && (defChildA->getOpCodeValue() == TR::loadaddr) && defChildA->getSymbolReference()->getSymbol()->isAuto() && defChildA->getSymbolReference()->getSymbol()->isLocalObject()) {
                                                        setStackWrtBar = true;
                                                        break;
                                                    }
                                                }
                        ...
                        if (setStackWrtBar && performTransformation(vp->comp(), "%sSetting wrtbar flag to assume stack object [%p]\n", OPT_DETAILS, node)) {
    ...
    if (node->getOpCode().isWrtBar()) {
        ...
        TR::VPConstraint* constraint = vp->getConstraint(node, isGlobal);
        if (constraint) {
            if (constraint->isHeapObject() == TR_yes && performTransformation(vp->comp(), "%sMarking the wrtbar node [%p] - destination is a heap object\n", OPT_DETAILS, node)) {
                //printf("--wbar-- heap wrtbar in %s\n", vp->comp()->signature());
                node->setIsHeapObjectWrtBar(true);
            } else if (constraint->isHeapObject() == TR_no && performTransformation(vp->comp(), "%sMarking the wrtbar node [%p] - destination is a non-heap object\n", OPT_DETAILS, node)) {
...
TR::Node* constrainMonexit(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (constraint && constraint->getClass()) {
        ...
        if ((clazz || !node->hasMonitorClassInNode()) && performTransformation(vp->comp(), "%sSetting type on MONEXIT  node [%p] to [%p]\n", OPT_DETAILS, node, clazz))
            node->setMonitorClassInNode(clazz);
    }
...
TR::Node* constrainInstanceOf(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (objectConstraint) {
        ...
        if (objectConstraint->isNullObject()) {
            result = 0;
        } else if (objectConstraint->getClassType() && castConstraint && castConstraint->isFixedClass() && objectConstraint->getClassType() == castConstraint->getClassType() && objectConstraint->isNonNullObject() && objectConstraint->isClassObject() != TR_yes) {
            result = 1;
            ...
            TR_YesNoMaybe isInstance = vp->fe()->isInstanceOf(objectClass, castClass,
...
TR::Node* constrainCheckcast(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (objectConstraint) {
        if (objectConstraint->isNullObject()) {
            ...
            } else if (isInstance == TR_no && (objectConstraint->isNonNullObject() || TR::Compiler->cls.isPrimitiveClass(vp->comp(), objectConstraint->getClass()))) {
                vp->registerPreXClass(objectConstraint);
            ...
            if (castConstraint && objectConstraint->isNonNullObject()) {
                if (objectConstraint->asClass() && castConstraint->asClass()) {
                    ...
                    if ((castIsClassObject == TR_no) && !objectConstraint->getClassType() && (objectConstraint->isClassObject() == TR_yes)) {
                        result = 0;
                        if (vp->trace())
                            traceMsg(vp->comp(), "object is a classobject but cast is not java/lang/Class\n");
                    } else if ((castIsClassObject == TR_no) && !objectConstraint->getClassType() &&
    ...
    if (result == 1 && ((node->getOpCodeValue() != TR::checkcastAndNULLCHK) || (objectConstraint && objectConstraint->isNonNullObject())) && performTransformation(vp->comp(), "%sRemoving redundant checkcast node [%p]\n", OPT_DETAILS, node)) {
    ...
    if ((node->getOpCodeValue() == TR::checkcastAndNULLCHK) && (!objectConstraint || !objectConstraint->isNonNullObject()))
        vp->createExceptionEdgeConstraints(TR::Block::CanCatchNullCheck, NULL, node);

    bool exceptionTaken = false;
    if ((result == 0) || ((node->getOpCodeValue() == TR::checkcastAndNULLCHK) && ((objectConstraint && objectConstraint->isNullObject()) || (isInstance == TR_no)))) {
                ...
                if (classType->asResolvedClass()) {
                    ...
                    if (jlClass) {
                        ...
                        if (classType->getClass() == jlClass) {
                            ...
                            if ((classType->isClassObject() == TR_maybe) && (objectConstraint && (objectConstraint->isClassObject() == TR_yes))) {
...
TR::Node* constrainNew(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (classConstraint) {
        ...
        else if (classConstraint->asClass() && classConstraint->asClass()->getClassType() && !(classConstraint->asClass()->getClassType()->isClassObject() == TR_yes))
            vp->addGlobalConstraint(node, classConstraint->asClass()->getClassType());
        else
            vp->addGlobalConstraint(node, classConstraint);
    }

    if (classConstraint && classConstraint->getClassType() && classConstraint->getClassType()->getClass()) {
...
TR::Node* constrainMultiANewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    TR_ASSERT(node->getFirstChild()->getOpCode().isLoadConst() && node->getFirstChild()->getInt() == node->getNumChildren() - 2,
    ...
    vp->addGlobalConstraint(node, TR::VPClass::create(vp, newType, TR::VPNonNullObject::create(vp), NULL, arrayInfo, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::NotClassObject)));
...
TR::Node* constrainArraylength(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (lowerBoundLimit == upperBoundLimit) {
        ...
        //
        if (!(node->getOpCodeValue() == TR::contigarraylength || node->getOpCodeValue() == TR::discontigarraylength) || !TR::Compiler->om.isDiscontiguousArray(lowerBoundLimit, elementSize)) {
    ...
    //
    if (!(node->getOpCodeValue() == TR::contigarraylength || node->getOpCodeValue() == TR::discontigarraylength) || (lowerBoundLimit != upperBoundLimit) || !TR::Compiler->om.isDiscontiguousArray(upperBoundLimit, elementSize)) {
    ...
    if (!node->getArrayStride() && performTransformation(vp->comp(), "%sSetting element width for array [%p] to %d\n", OPT_DETAILS, node, elementSize)) {
...
static void devirtualizeCall(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (!constraint || !(thisType = constraint->getClass())) {
        ...
        if (!dontProfileMore && (vp->comp()->getMethodHotness() == hot) && vp->comp()->getRecompilationInfo() && vp->_isGlobalPropagation && vp->lastTimeThrough() && vp->optimizer()->switchToProfiling()) {
    ...
    if ((constraint->asClass() && (constraint->asClass()->isClassObject() == TR_yes)) || (constraint->asObjectLocation() && (constraint->asObjectLocation()->isClassObject() == TR_yes))) {
#ifdef J9_PROJECT_SPECIFIC
    ...
    if (methodSymbol->isComputed()) {
        ...
        if ((vp->fe()->isInstanceOf(thisType, originalMethodClass, true, true) != TR_yes) || (originalMethodClass && TR::Compiler->cls.isInterfaceClass(vp->comp(), originalMethodClass)))
            return;
...
#ifdef J9_PROJECT_SPECIFIC
        TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(thisType, vp->comp());
        if (!classInfo || !classInfo->isInitialized())
            return;
#else
        return;
    ...
    if (methodSymbol->isVirtual() || methodSymbol->isInterface()) {
        ...
        if (constraint->isFixedClass() || constraint->isPreexistentObject()) {
            ...
            if (!vp->registerPreXClass(constraint) && constraint->isPreexistentObject()) {
                if (!resolvedMethod->virtualMethodIsOverridden() && (constraint->getClass() == constraint->getPreexistence()->getAssumptionClass()))
    ...
    if ((vp->lastTimeThrough() || !node->getOpCode().isIndirect()) && (vp->_isGlobalPropagation || !vp->getLastRun()) && !methodSymbol->isInterface())
...
TR::Node* constrainCall(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (vp->lastTimeThrough() && callIsDirect && TR::Compiler->om.canGenerateArraylets() && node->getSymbol()->getMethodSymbol()->getMethod() && (node->getSymbol()->getMethodSymbol()->getMethod()->isUnsafeWithObjectArg() || (node->getSymbol()->getMethodSymbol()->getMethod()->isUnsafeCAS() && vp->comp()->getOption(TR_EnableInliningOfUnsafeForArraylets))))

    {
        ...
        if (operand && ((operand->isClassObject() == TR_yes) || (operand->getClassType() && ((operand->getClassType()->isArray() == TR_no) || (operand->getClassType()->isClassObject() == TR_yes))))) {
    ...
    if (symbol) {
        ...
        if (isIntegerAbs || isLongAbs) {
            ...
            if (classPointer->getOpCode().hasSymbolReference() && (classPointer->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef())) {
                bool operandGlobal;
        ...
        } else if (vp->cg()->getSupportsNewObjectAlignment() && symbol->getRecognizedMethod() == TR::java_util_TreeMap_rbInsert) {
            ...
            if (jitHelpersClass && TR::Compiler->cls.isClassInitialized(vp->comp(), jitHelpersClass)) {
                ...
                if (vp->comp()->getOption(TR_EnableJITHelpershashCodeImpl) && hashCodeMethodSymRef && getHelpersSymRef && performTransformation(vp->comp(), "%sChanging call to new hashCodeImpl at node [%p]\n", OPT_DETAILS, node)) {
         ...
         */
             ...
             */
                ...
                if (j9class && TR::Compiler->cls.isClassInitialized(vp->comp(), j9class) && performTransformation(vp->comp(), "%s Remove TR::sun_misc_Unsafe_ensureClassInitialized call, class already initialized [%p]\n", OPT_DETAILS, node)) {
...
TR::Node* constrainAdd(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (rhs) {
        constraint = NULL;
        if (rhs->asLongConst()) {
            if (rhs->asLongConst()->getLong() > TR::getMinSigned<TR::Int32>() && rhs->asLongConst()->getLong() < TR::getMaxSigned<TR::Int32>())
            ...
            // type context
            if (rhs->asIntConst()->getInt() > TR::getMinSigned<TR::Int32>() && rhs->asIntConst()->getInt() < TR::getMaxSigned<TR::Int32>())
...
TR::Node* constrainSubtract(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (rhs) {
        ...
        if (rhs->asLongConst()) {
            ...
            if (rhs->asShortConst()->getShort() > TR::getMinSigned<TR::Int16>() && rhs->asShortConst()->getShort() < TR::getMaxSigned<TR::Int16>())
...
TR::Node* constrainLmul(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (lhs && rhs) {
        ...
        if (lhs->asLongConst() && rhs->asLongConst()) {
            ...
            if (!(can64BitMultiplyOverflow(lhs->getLowLong(), rhs->getLowLong(), node->getOpCode().isUnsigned()) || can64BitMultiplyOverflow(lhs->getLowLong(), rhs->getHighLong(), node->getOpCode().isUnsigned()) || can64BitMultiplyOverflow(lhs->getHighLong(), rhs->getLowLong(), node->getOpCode().isUnsigned()) || can64BitMultiplyOverflow(lhs->getHighLong(), rhs->getHighLong(), node->getOpCode().isUnsigned()))) {
...
bool constrainIntegerDivisionRange(int64_t lhsLow, int64_t lhsHigh, int64_t rhsLow, int64_t rhsHigh, int64_t rangeMin, int64_t rangeMax, int64_t& resultLow, int64_t& resultHigh, bool allowZeroInDivisorRange)
{
    ...
    if (isUnsafeDivide(lhsLow, rhsLow) || isUnsafeDivide(lhsLow, rhsHigh) || isUnsafeDivide(lhsHigh, rhsLow) || isUnsafeDivide(lhsHigh, rhsHigh))
        return false;
        ...
        if (lhs->asIntConst() && rhs->asIntConst()) {
            ...
            if (lhsInt && rhsInt) {
                ...
                if (isUnsigned) {
                    ...
                    range_min = 0;
                    range_max = TR::getMaxSigned<TR::Int32>(); // we don't support unsigned range constraint yet. Only support upto TR::getMaxSigned<TR::Int32>() for unsigned
                } else {
...
TR::Node* constrainLdiv(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
        ...
        if (vp->lastTimeThrough() && !isUnsigned && lhsConstraint && (lhsConstraint->getLow() >= ((int64_t)TR::getMinSigned<TR::Int32>())) && (lhsConstraint->getHigh() <= ((int64_t)TR::getMaxSigned<TR::Int32>())) && rhsConstraint && (rhsConstraint->getLow() >= ((int64_t)TR::getMinSigned<TR::Int32>())) && (rhsConstraint->getHigh() <= ((int64_t)TR::getMaxSigned<TR::Int32>())) && ((lhsConstraint->getLow() > TR::getMinSigned<TR::Int32>()) || (rhsConstraint->getLow() > -1) || (rhsConstraint->getHigh() < -1)) && performTransformation(vp->comp(), "%sChange node [" POINTER_PRINTF_FORMAT "] ldiv->i2l of idiv\n", OPT_DETAILS, node)) {
            ...
            if (vp->_curTree->getNode()->getOpCodeValue() == TR::DIVCHK && vp->_curTree->getNode()->getFirstChild() == node) {
                // messier scenario
...
TR::Node* removeRedundantREM(OMR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* nodeConstraint, TR::VPConstraint* firstChildConstraint, TR::VPConstraint* secondChildConstraint)
{
    ...
    if (powerOfTen) {
        ...
        if (!isUnsigned && fcPrecision <= refinedPrecision && performTransformation(vp->comp(), "%sRemove %s [0x%p] as child %s [0x%p] prec %lld <= divisor max prec %d (value %lld)\n", OPT_DETAILS, node->getOpCode().getName(), node, node->getFirstChild()->getOpCode().getName(), node->getFirstChild(), fcPrecision, refinedPrecision, constValue)) {
    ...
    TR::VPConstraint*& constraint)
{
    ...
    if ((nodeOp.isAdd() || nodeOp.isSub()) && node->getFirstChild()->cannotOverflow()) {
        ...
        if (replaceNode && performTransformation(vp->comp(), "%sDistributing shift across add/sub [%p]\n", OPT_DETAILS, node->getFirstChild())) {
...
TR::Node* constrainIshr(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    TR::Node* firstChild = node->getFirstChild();
    if (firstChild->isNonNegative() /*&& firstChild->getType().isSignedInt()*/ && vp->lastTimeThrough() && performTransformation(vp->comp(), "%sChange node [" POINTER_PRINTF_FORMAT "] ishr->iushr\n", OPT_DETAILS, node)) {
...
TR::Node* constrainIand(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (rhs && rhs->asIntConst()) {
        ...
        if (255 == mask && lhs) // loading from byte
        {
            if (lhs->asIntRange()) {
                ...
                if (low >= 0 && high <= 255 && performTransformation(vp->comp(), "%sRemoving node [%p] %s\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        ...
        if (mask == 0) {
            ...
            if (rhs && rhs->asIntConst()) {
                ...
                if ((firstChild->getOpCodeValue() == TR::iloadi || firstChild->getOpCodeValue() == TR::lloadi) && (firstChild->getSymbolReference() == vp->comp()->getSymRefTab()->findClassAndDepthFlagsSymbolRef()) && (rhs->getLowInt() == TR::Compiler->cls.flagValueForArrayCheck(vp->comp()))) {
                    if (vp->trace())
    ...
    if (rhs && lrhs && lrhs->asIntConst()) {
        ...
        if (rhs->asIntRange()) {
            ...
            int32_t low = range->getLowInt();
            int32_t high = range->getHighInt();
            if (node->getFirstChild()->getOpCodeValue() == TR::imul && ((high & 0x80000000) == 0) && ((low & 0x80000000) == 0)) {
                while (!(mask & 0x1)) {
                    shift++;
                    mask >>= 1;
                }
                if (high < 1 << shift)
                    constraint = TR::VPIntConst::create(vp, 0);
            } else if (node->getFirstChild()->getOpCodeValue() == TR::ishl && ((high & 0x80000000) == 0) && ((low & 0x80000000) == 0)) {
                if (high < 1 << mask)
                    constraint = TR::VPIntConst::create(vp, 0);
            }
        } else if (rhs->asIntConst()) {
    ...
    if (canBeRemoved && performTransformation(vp->comp(), "%sRemoving redundant node [%p] %s\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        TR::Node* firstChild = node->getFirstChild();
    ...
    if (TR::ishr == firstChild->getOpCodeValue() && firstChild->getReferenceCount() < 2 /*&&
                                            firstChild->getFirstChild()->getType().isSignedInt()*/
        ) // FIXME: handle unsigned when supported
    {
        ...
        } else if (shiftConstraint && shiftConstraint->asIntRange() && shiftConstraint->asIntRange()->getLowInt() >= 0) {
            bitsShifted = shiftConstraint->asIntRange()->getHighInt();
        }

        if ((bitsShifted < (int32_t)signBitsMaskedOff) && vp->lastTimeThrough() && performTransformation(vp->comp(), "%s Node [" POINTER_PRINTF_FORMAT "]: ishr -> iushr (parent ignores sign bits)\n", OPT_DETAILS, firstChild)) {
...
void replaceWithSmallerType(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (!load->getOpCode().isLoadVar() || load->getReferenceCount() > 1 || !oldRef->getSymbol()->isAuto() || oldRef->getUseDefAliases().hasAliases())
        return;
...
static TR::Node* constrainNarrowIntValue(OMR::ValuePropagation* vp, TR::Node* node, int32_t low, int32_t high)
{
    ...
    if (constraint) {
        if (constraint->asIntConstraint() || constraint->asMergedIntConstraints()) {
            ...
            if (vp->comp()->getOptions()->getDebugEnableFlag(TR_EnableUnneededNarrowIntConversion) && constraint->getLowLong() >= 0 && node->isUnsigned()) {
    ...
    if (low <= high) {
        ...
        case TR::Int8:
            ...
            max = TR::getMaxSigned<TR::Int8>(); // byte nodes are still using Int constraint
        case TR::Int32:
            constraint = TR::VPIntRange::create(vp, low, high);
            break;
        default:
    ...
    TR::ILOpCodes correspondingNarrowingOpCode)
{
   ...
   */
    ...
    if (low <= high) {
        ...
        if ((node->getOpCode().isArithmetic() || node->getOpCode().isLoad()) && ((low > TR::getMinSigned<TR::Int32>()) || (high < TR::getMaxSigned<TR::Int32>())))
            node->setCannotOverflow(true);
    }
    return false;
}
...
static TR::Node* getOriginalCallNode(TR_VirtualGuard* vGuard, TR::Compilation* comp, TR::Node* guardNode)
{
    ...
    for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
        if (tt->getNode()->getNumChildren()) {
            ...
            if (possiblyCall->getOpCode().isCall() && possiblyCall->getOpCode().isIndirect() && possiblyCall->getByteCodeInfo().getCallerIndex() == bci.getCallerIndex() && possiblyCall->getByteCodeIndex() == bci.getByteCodeIndex()) {
                return possiblyCall;
            }
        }
    }
    ...
    TR_OpaqueClassBlock* objectClass)
{
    ...
    if (!callNode->getFirstChild()->getOpCode().hasSymbolReference() || callNode->getFirstChild()->getSymbolReference() != vp->comp()->getSymRefTab()->findVftSymbolRef() || !callNode->getFirstChild()->getFirstChild()->getOpCode().isLoadVarDirect() || //I guess, having a vft symbol implies that there is something underneath it?
    ...
    TR::Node* newGuardNode = TR_VirtualGuard::createMethodGuardWithReceiver(guardKind,
        vp->comp(),
...
static TR::Node* constrainIfcmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool branchOnEqual)
{
    ...
    if (((node->getOpCodeValue() == TR::ifacmpne || node->getOpCodeValue() == TR::ifacmpeq || node->getOpCodeValue() == TR::ificmpne || node->getOpCodeValue() == TR::ificmpeq) && node->isTheVirtualGuardForAGuardedInlinedCall())) {
    ...
    if (cannotBranch && performTransformation(vp->comp(), "%sRemoving conditional branch [%p] %s\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        removeConditionalBranch(vp, node, edge);
        return node;
    }

    if (cannotFallThrough && performTransformation(vp->comp(), "%sChanging node [%p] %s into goto\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        vp->printEdgeConstraints(vp->createEdgeConstraints(edge, false));
    ...
    if (!vp->comp()->compileRelocatableCode() && vp->lastTimeThrough() && vp->comp()->performVirtualGuardNOPing() && !vp->_curBlock->isCold() && !vp->_curBlock->getNextBlock()->isExtensionOfPreviousBlock() && (node->getNumChildren() == 2) && ((node->getOpCodeValue() == TR::ifacmpeq) || (node->getOpCodeValue() == TR::ifacmpne))) {
            ...
            if (objectRefConstraint) {
                ...
                if (typeConstraint) {
                    ...
                    if (resolvedTypeConstraint) {
                        ...
                        TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(clazz, vp->comp(), allowForAOT);
                    ...
                    if (clazzToBeInitialized && (((clazzNameLen == 35) && !strncmp(clazzToBeInitialized, "Lcom/ibm/ejs/ras/TraceEnabledToken;", clazzNameLen)) || ((clazzNameLen == 41) && !strncmp(clazzToBeInitialized, "Lcom/ibm/websphere/ras/TraceEnabledToken;", clazzNameLen)) || ((clazzNameLen == 40) && !strncmp(clazzToBeInitialized, "Ljava/lang/String$StringCompressionFlag;", clazzNameLen))) && performTransformation(vp->comp(), "%sUsing side-effect guard to fold away condition on a load from an uninitializec class  %s [%p]\n", OPT_DETAILS, clazzToBeInitialized, node)) {
    ...
    if (((node->getOpCodeValue() == TR::ifacmpne || node->getOpCodeValue() == TR::ifacmpeq) && node->isTheVirtualGuardForAGuardedInlinedCall())) {
        ...
        if (virtualGuard) {
            ...
            case TR_VftTest: {
                ...
                if (enableJavaLangClassFolding && (instanceofObjectRef->getOpCodeValue() == TR::aloadi) && (instanceofObjectRef->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef())) {
                    ...
                    if (ignoreVirtualGuard && classConstraint && classConstraint->getClassType() && objectConstraint && objectConstraint->isClassObject() == TR_yes && vp->comp()->getClassClassPointer()) {
                ...
                if (!foldedGuard && (instanceofObjectRef->getOpCodeValue() == TR::aloadi) && (instanceofObjectRef->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef())) {
            ...
            case TR_MethodTest: {
                ...
                if ((vtableEntryNode->getOpCodeValue() == TR::aloadi) && vtableEntryNode->getOpCode().hasSymbolReference() && vp->comp()->getSymRefTab()->isVtableEntrySymbolRef(vtableEntryNode->getSymbolReference()))
        ...
        if (callNode && callNode->getOpCode().isCallIndirect()) {
            ...
            if (constraint && constraint->isFixedClass() && (thisType = constraint->getClass()) && methodSymbol->isVirtual()) {
                ...
                if (resolvedMethodSymbol) {
                    ...
                    if ((vp->fe()->isInstanceOf(thisType, originalMethodClass, true, true) == TR_yes) && !originalResolvedMethod->virtualMethodIsOverridden()) {
    ...
    if (cannotBranch && performTransformation(vp->comp(), "%sRemoving conditional branch [%p] %s\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        ...
        removeConditionalBranch(vp, node, edge);
    } else if (cannotFallThrough && performTransformation(vp->comp(), "%sChanging node [%p] %s into goto\n", OPT_DETAILS, node, node->getOpCode().getName())) {
        ...
        intersected = (intersected) ? intersected->intersect(constraintFromMethodPointer, vp) : intersected = constraintFromMethodPointer;
        ...
        if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
            ...
            if (rhs && !objectClass && lhs && lhs->getClassType() && (interfaceMethodSymbol = callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()) && interfaceMethodSymbol->isInterface()) {
                int32_t len;
            ...
            if (objectClass && rhsClass && callClass && !lhs->isFixedClass() && vp->comp()->fe()->isInstanceOf(rhsClass, objectClass, true, true, true) == TR_yes && vp->comp()->fe()->isInstanceOf(objectClass, callClass, true, true, true) == TR_yes) /*the object class may be less specific than callClass*/
            {
      ...
      */
                    ...
                    if (searchSucceeded && numConcreteClasses(&subClasses) < 2 && rvm) {
                        ...
                        TR_VirtualGuardKind guardKind = methodSymbol->isInterface() ? TR_InterfaceGuard : TR::Compiler->cls.isAbstractClass(vp->comp(), objectClass) ? TR_AbstractGuard : TR_HierarchyGuard;
...
static TR::Node* simplifyInequality(OMR::ValuePropagation* vp, TR::Node* node, TR::Node* lhsChild, TR::Node* rhsChild, bool isGlobal, bool childrenReversed)
{
    ...
    //
    if ((lhsChild->getOpCode().isAdd() || lhsChild->getOpCode().isSub()) && (rhsChild->getOpCode().isAdd() || rhsChild->getOpCode().isSub()) && lhsChild->getSecondChild()->getOpCode().isLoadConst() && rhsChild->getSecondChild()->getOpCode().isLoadConst()) {
...
static TR::Node* constrainCmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool testEqual)
{
    ...
    if (result >= 0) {
        ...
        if (secondChild->getOpCode().isLoadConst() && lhsIsBool && vp->getCurrentParent()) {
            ...
            if (((constValue == 1) && ((node->getOpCodeValue() == TR::icmpeq) || (node->getOpCodeValue() == TR::lcmpeq))) || ((constValue == 0) && ((node->getOpCodeValue() == TR::icmpne) || (node->getOpCodeValue() == TR::lcmpne)))) {
            ...
            } else if (((constValue == 1) && ((node->getOpCodeValue() == TR::icmpne) || (node->getOpCodeValue() == TR::lcmpne))) || ((constValue == 0) && ((node->getOpCodeValue() == TR::icmpeq) || (node->getOpCodeValue() == TR::lcmpeq)))) {
                if (firstChild->getOpCodeValue() == node->getOpCodeValue()) {
                    ...
                    if (secondGrandChild->getOpCode().isLoadConst() && firstGrandIsBool && vp->getCurrentParent()) {
                        ...
                        if ((firstChild->getOpCodeValue() == TR::icmpeq) || (firstChild->getOpCodeValue() == TR::icmpne))
...
static TR::Node* constrainCmplessthan(OMR::ValuePropagation* vp, TR::Node* node, TR::Node* lhsChild, TR::Node* rhsChild, bool orEqual)
{
    ...
    TR::VPConstraint* constraint = NULL;
    if (result >= 0) {
        if ((lhsGlobal || vp->lastTimeThrough()) && performTransformation(vp->comp(), "%sChanging node [%p] %s into constant %d\n", OPT_DETAILS, node, node->getOpCode().getName(), result)) {
            vp->removeChildren(node);
...
void constrainNewlyFoldedConst(OMR::ValuePropagation* vp, TR::Node* node, bool isGlobal)
{
    ...
    default:
        ...
        if (node->getDataType() == TR::Address && node->getOpCode().hasSymbolReference() && node->getSymbolReference()->hasKnownObjectIndex()) {
...
TR::Node* constrainNullChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    if (nullCheckResult == 1 && performTransformation(vp->comp(), "%sRemoving redundant null check node [%p]\n", OPT_DETAILS, node)) {
...
TR::Node* constrainResolveChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    // Now remove the check if we can
    //
    if (removeTheCheck && performTransformation(vp->comp(), "%sRemoving redundant resolve check node [%p]\n", OPT_DETAILS, node)) {
...
TR::Node* constrainDivChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (removeDivCheck && performTransformation(vp->comp(), "%sRemoving redundant div check node [%p]\n", OPT_DETAILS, node))
        TR::Node::recreate(node, TR::treetop);
    else {
        if (!complexDivCheck) {
            if (divNode->getOpCode().isDiv() || divNode->getOpCode().isRem())
...
TR::Node* constrainBndChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    if (sizeBefore && index) {
        if (index->getLowInt() >= 0 && index->getHighInt() < sizeBefore->getLowInt() && performTransformation(vp->comp(), "%sRemoving unnecessary bound check node [%p]\n", OPT_DETAILS, node)) {
    ...
    if (relativeConstraint && relativeConstraint->mustBeLessThan() && performTransformation(vp->comp(), "%sRemoving redundant bound check node (subsumed) [%p]\n", OPT_DETAILS, node)) {
        // Change this node to a treetop containing the index (which is
        // currently the second child of the bounds check).
        //
        TR::Node::recreate(node, TR::treetop);
...
TR::Node* constrainBndChkWithSpineChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (indexConstraint && objConstraint) {
        ...
        if (arrayInfo) {
            ...
            if (indexConstraint->getLowInt() >= 0 && indexConstraint->getHighInt() < lowerBoundLimit && performTransformation(vp->comp(), "%sRemoving unnecessary bound check from composite node [%p]\n", OPT_DETAILS, node)) {
                removeBndChk = true;
            }
        }
    }
    ...
    if (sizeConstraint && indexConstraint) {
        if (indexConstraint->getLowInt() >= 0 && indexConstraint->getHighInt() < sizeConstraint->getLowInt() && performTransformation(vp->comp(), "%sRemoving unnecessary bound check from composite node [%p]\n", OPT_DETAILS, node)) {
            removeBndChk = true;
        }
    }
    ...
    if (sizeConstraint && !vp->comp()->generateArraylets()) {
        ...
        if (elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(sizeConstraint->getHighInt(), elementSize) && (sizeConstraint->getLowInt() > 0) && performTransformation(vp->comp(), "%sRemoving unnecessary spine check from composite node [%p] : min arraylength = %d, max arraylength=%d, element size=%d\n", OPT_DETAILS, node, sizeConstraint->getLowInt(), sizeConstraint->getHighInt(), elementSize)) {
            removeSpineChk = true;
        }
    }
    ...
    bool globalFlag;
    TR::VPConstraint* relativeConstraint = vp->getConstraint(indexNode, globalFlag, sizeNode);
    if (!removeBndChk && relativeConstraint && relativeConstraint->mustBeLessThan() && performTransformation(vp->comp(), "%sRemoving redundant bound check from node (subsumed) [%p]\n", OPT_DETAILS, node)) {
        removeBndChk = true;
    }
...
TR::Node* constrainArrayCopyBndChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    //
    if ((vp->getValueNumber(lhsNode) == vp->getValueNumber(rhsNode)) || (lhs && rhs && lhs->getLowInt() >= rhs->getHighInt())) {
...
TR::Node* constrainArrayStoreChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (objectRef->getOpCode().isLoadVar() && objectRef->getOpCode().isIndirect() && objectRef->getFirstChild()->isInternalPointer()) {
            ...
            if (sig && sig[0] == '[') {
                ...
                //
                    ...
                    if (isInstance == TR_yes) {
                        ...
                        vp->registerPreXClass(object);
                        mustFail = true;
                    } else if (arrayComponentClass && objectClass && !TR::Compiler->cls.isClassArray(vp->comp(), arrayComponentClass) && (arrayComponentClass == objectClass) && !vp->comp()->fe()->classHasBeenExtended(objectClass)) {
                        if (vp->trace())
                            traceMsg(vp->comp(), "Setting arrayStoreClass on ArrayStoreChk node [%p] to [%p]\n", node, objectClass);
                        node->setArrayStoreClassInNode(objectClass);
                    }

                    else if (!vp->comp()->compileRelocatableCode() && !(vp->comp()->getOption(TR_DisableArrayStoreCheckOpts)) && arrayComponentClass && objectClass && vp->fe()->isInstanceOf(objectClass, arrayComponentClass, true, true)) {
                        if (vp->trace())
...
TR::Node* constrainArrayChk(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (removeThisNode && performTransformation(vp->comp(), "%sRemoving redundant array check node [%p]\n", OPT_DETAILS, node)) {
...
TR::Node* constrainArraycopy(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (node->getNumChildren() == 5) {
        ...
        if (elementType != TR::NoType && performTransformation(vp->comp(), "%sTransforming arraycopy node [%p]\n", OPT_DETAILS, node)) {
