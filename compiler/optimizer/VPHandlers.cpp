      ...
      if (constraint->isNullObject()) {
        ...
        if (constraint->getKnownObject()) {
          ...
          if (node->getOpCode().isLoadIndirect() &&
            ...
            TR::KnownObjectTable *knot = vp->comp()->getKnownObjectTable();
            TR_ASSERT(knot, "Can't have a known-object constraint without a "
                            "known-object table");
            TR::SymbolReference *improvedSymRef =
            ...
            if (improvedSymRef->hasKnownObjectIndex()) {
              if (performTransformation(vp->comp(),
                                        "%sUsing known-object specific symref "
                                        "#%d for obj%d at [%p]\n",
                                        OPT_DETAILS,
    ...
    case TR_Sign_Clean:
      if (!node->hasKnownCleanSign() &&
        ...
        if (vp->comp()->cg()->traceBCDCodeGen())
          traceMsg(
              vp->comp(), "y^y: VP: Transfer sign constraint %s to %s (0x%p)\n",
            ...
            if (constrainAnyIntLoadCriticalSection.hasVMAccess()) {
              ...
              if (validStart <= offset && offset < accessEnd &&
                ...
                case TR::Int8: {
                  ...
                  vp->replaceByConstant(node, TR::VPIntConst::create(vp, value),
                                        isGlobal);
                  return node;
                }
                case TR::Int16: {
          ...
          if (bypassOnThisRun) {
...

            {
              ...
              if (constrainIaloadCriticalSection.hasVMAccess()) {
                ...
                if (!bypassLocation &&
                    performTransformation(
                        vp->comp(), "%s[%p] create CallSite bypass for obj%d\n",
                ...
                if (vp->trace())
                  traceMsg(vp->comp(),
                           "[%p] Unable to acquire VM access.  Not attempting "
                           "to bypass CallSite\n",
                           node);
              }
            }
            ...
            if (fieldInfo) {
              ...
              if (arrayFieldInfo && arrayFieldInfo->isDimensionInfoValid()) {
                ...
                if (arrLength >= 0 && sig && (len > 0) &&
                  ...
                  if (elementSize != 0) {
                    if (vp->trace())
                      traceMsg(vp->comp(), "Using class lookahead info to find "
                                           "out non null, array dimension\n");
                    // vp->addGlobalConstraint(node,
                    // TR::VPNonNullObject::create(vp));
                    // vp->addGlobalConstraint(node, TR::VPArrayInfo::create(vp,
                    // arrLength, arrLength, elementSize));
        ...
        if (objectConstraint->asClass() && castConstraint->asClass()) {
          ...
          //
            ...
            if (objectConstraint->isNonNullObject()) {
              result = 1;
              if (vp->trace())
                traceMsg(vp->comp(), "object is a non-null classobject and "
                                     "cast is java/lang/Class\n");
            } else {
              TR::Node::recreate(node, TR::acmpne);
              vp->removeNode(node->getChild(1), true);
              node->setAndIncChild(1, TR::Node::create(node, TR::aconst, 0, 0));
      ...
      if (castConstraint && objectConstraint->isNonNullObject()) {
        if (objectConstraint->asClass() && castConstraint->asClass()) {
          ...
          //
            ...
            result = 1;
            if (vp->trace())
              traceMsg(vp->comp(), "object is a non-null classobject and cast "
                                   "is java/lang/Class\n");
...
refineMethodSymbolInCall(OMR::ValuePropagation *vp, TR::Node *node,
                         TR::SymbolReference *symRef,
...
TR::Node *constrainImul(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs) {
    ...
    if (lhs->asIntConst() && rhs->asIntConst()) {
      ...
      //   isUnsigned);
      // else
      constraint = TR::VPIntConst::create(vp, lhs->asIntConst()->getInt() *
                                                  rhs->asIntConst()->getInt());
    } else {
...
TR::Node *constrainIushr(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    TR::VPConstraint *constraint = NULL;
    if (low == high)
      constraint = TR::VPIntConst::create(vp, ((uint32_t)low) >>
                                                  shiftAmount /*, isUnsigned*/);
    else if (low >= 0)
      constraint = TR::VPIntRange::create(vp, ((uint32_t)low) >> shiftAmount,
                                          ((uint32_t)high) >>
                                              shiftAmount /*, isUnsigned*/);
...
#ifdef J9_PROJECT_SPECIFIC
TR_ResolvedMethod *
findSingleImplementer(TR_OpaqueClassBlock *thisClass, int32_t cpIndexOrVftSlot,
                      TR_ResolvedMethod *callerMethod, TR::Compilation *comp,
                      bool locked,
      ...
      case TR_MethodTest: {
           ...
           vp->comp()->getSymRefTab()->findVftSymbolRef()))*/
        {
          ...
          if (ignoreVirtualGuard && classConstraint &&
            ...
            if (vp->trace())
              traceMsg(vp->comp(),
                       "TR_MethodTest: node=%p, vtableEntryNode=%p, clazz=%p, "
                       "vftOffset=%d, vftEntry=%p, childrenAreEqual=%d, "
                       "testForEquality=%d\n",
