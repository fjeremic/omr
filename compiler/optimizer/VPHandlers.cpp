...
static void constrainBaseObjectOfIndirectAccess(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (baseObjectSig) {
    ...
    if (baseClass && TR::Compiler->cls.isInterfaceClass(vp->comp(), baseClass)
    ...
    if (baseClass) {
      ...
      TR::VPConstraint* constraint
        = TR::VPClassType::create(vp, baseObjectSig, baseObjectSigLen, method, false, baseClass);
    ...
    for (; !baseExpression;) {
      ...
      if (!constraint) {
        ...
        if (prevNodeSym->isArrayShadowSymbol() &&
          // UnsafeShadow responds true to above question, but the type of the array element might not match the type of
          // the symbol, it needs to be derived from the array
          ...
          if (constraint->getKnownObject()->isArrayWithConstantElements(vp->comp())) {
            ...
            TR::SymbolReference* improvedSymRef
              = vp->comp()->getSymRefTab()->findOrCreateImmutableArrayShadowSymbolRef(prevNodeSym->getDataType());
            if (vp->trace())
              traceMsg(
                vp->comp(), "Found arrayShadow load %p from array with constant elements %p\n", prevNode, curNode);
            if (performTransformation(vp->comp(), "%sUsing ImmutableArrayShadow symref #%d for node %p\n", OPT_DETAILS,
                  improvedSymRef->getReferenceNumber(), prevNode))
              prevNode->setSymbolReference(improvedSymRef);

            // Alias info of calls are computed in createAliasInfo, which is only called if alias info is not available
            // or is invalid. Invalidate alias info so that it can be recomputed in the next optimization that needs it
        ...
        if (vp->trace())
          traceMsg(vp->comp(), "  - %s %p is string obj%d\n", baseExpression->getOpCode().getName(), baseExpression,
            baseKnownObject);
        break;
      } else if (constraint->isJ9ClassObject() == TR_yes && constraint->isNonNullObject() && constraint->getClassType()
        ...
        if (nodeToRemove) {
          TR_ASSERT(
            didSomething, "How can there be a node to remove if transformIndirectLoadChain didn't do something??");
          vp->removeNode(nodeToRemove, true);
          return true;
        }
        return didSomething;
      } else if (!curNode->getOpCode().isLoadIndirect()) {
        if (curNode->getOpCodeValue() == TR::loadaddr && curNode->getSymbolReference()->getSymbol()->isClassObject()
          && !curNode->getSymbolReference()->isUnresolved()) {
          TR::Node* nodeToRemove = NULL;
          uintptrj_t clazz
            = (uintptrj_t)curNode->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress();
          bool didSomething
            = TR::TransformUtil::transformIndirectLoadChainAt(vp->comp(), node, curNode, &clazz, &nodeToRemove);
          if (nodeToRemove) {
            TR_ASSERT(
    ...
    case TR::Double:
      if (constraint->asLongConst()) {
        ...
        if (!vp->cg()->materializesLargeConstants() || (!node->getType().isInt64())
          ...
          || (vp->getCurrentParent()->getOpCode().isMul() && (vp->getCurrentParent()->getSecondChild() == node)
      ...
      if (constraint->isNullObject()) {
        ...
        if (constraint->getKnownObject()) {
          ...
          if (node->getOpCode().isLoadIndirect() && !node->getSymbolReference()->hasKnownObjectIndex()) {
            ...
            TR::SymbolReference* improvedSymRef = vp->comp()->getSymRefTab()->findOrCreateSymRefWithKnownObject(
...
bool reduceLongOpToIntegerOp(OMR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* nodeConstraint)
{
  ...
  // Adding this check because it is likely suboptimal on machines where you can use long registers to convert things to
  // integers and insert conversion trees. Conversion trees are not free. LoadExtensions (and other codegen peepholes)
  // should in theory catch uneeded conversions (like widening) as described in the case below on 64 bit platforms.
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
      ...
      if (canMoveLongOpChildDirectly(node, i, arithNode)) {
        ...
        dumpOptDetails(vp->comp(), "    Replacing child %d %s (0x%p) with grandchild %s (0x%p)\n", i,
          child->getOpCode().getName(), child, child->getFirstChild()->getOpCode().getName(), child->getFirstChild());
      } else {
  ...
  if (node->getAddress() == 0) {
    ...
    if (node->isClassPointerConstant()) {
      TR::VPConstraint* typeConstraint
        = TR::VPClass::create(vp, TR::VPFixedClass::create(vp, (TR_OpaqueClassBlock*)node->getAddress()), NULL, NULL,
...
TR::Node* constrainBCDSign(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (sign == TR::DataType::getInvalidSignCode()
    || sign == TR::DataType::getIgnoredSignCode()) // if no specific sign code see if a clean or preferred sign state
                                                   // can be tracked
  {
    ...
    if (index->getOpCode().isLoadConst()) {
      ...
      if (dataType == TR::Int16 && array->getOpCode().isIndirect()) {
        ...
        if (baseVPConstraint && baseVPConstraint->isConstString()) {
          ...
          uintptrj_t offset = TR::Compiler->target.is64Bit() ? (uintptrj_t)index->getUnsignedLongInt()
  ...
  if (objectReferenceLocation) {
...

    {
      ...
      uintptrj_t objectReference
        = vp->comp()->fej9()->getStaticReferenceFieldAtAddress((uintptrj_t)objectReferenceLocation);
    ...
    if (isString && symRef->getSymbol()->isStatic()) {
      ...
      vp->addGlobalConstraint(node,
        TR::VPClass::create(vp, TR::VPConstString::create(vp, symRef), TR::VPNonNullObject::create(vp), NULL, NULL,
    ...
    } else if (jlClass) // without a jlClass, we can't tell what kind of constraint to add
    {
      ...
      if (isFixedJavaLangClass) {
        if (performTransformation(vp->comp(),
              ...
              classSig, knownObjectIndex)) {
          constraint = TR::VPClass::create(vp, TR::VPKnownObject::createForJavaLangClass(vp, knownObjectIndex),
        ...
        if (performTransformation(vp->comp(),
              ...
              knownObjectIndex, classSig)) {
          constraint
            = TR::VPClass::create(vp, TR::VPKnownObject::create(vp, knownObjectIndex), TR::VPNonNullObject::create(vp),
...
TR::Node* constrainAload(OMR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (!symRef->getSymbol()->isArrayShadowSymbol()) {
      ...
      if (haveClassLookaheadInfo) {
        ...
        if (sym->isStatic() && sym->isFinal()) {
          ...
          if (classOfStatic == NULL) {
            ...
            classNameOfFieldOrStatic
              = symRef->getOwningMethod(vp->comp())->classNameOfFieldOrStatic(symRef->getCPIndex(), len);
            if (classNameOfFieldOrStatic) {
              classNameOfFieldOrStatic = classNameToSignature(classNameOfFieldOrStatic, len, vp->comp());
              TR_OpaqueClassBlock* curClass
                = vp->fe()->getClassFromSignature(classNameOfFieldOrStatic, len, symRef->getOwningMethod(vp->comp()));
              TR_OpaqueClassBlock* owningClass
                = vp->comp()->getJittedMethodSymbol()->getResolvedMethod()->containingClass();
                ...
                if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                  ...
                  if (elementSize != 0) {
                    ...
                    vp->addGlobalConstraint(
                      node, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::NotClassObject));
                    foundInfo = true;
                  }
                }
              }
        ...
        if (!foundInfo) {
          TR_PersistentClassInfo* classInfo
            = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
              symRef->getOwningMethod(vp->comp())->classOfStatic(symRef->getCPIndex()), vp->comp(), allowForAOT);
                ...
                if (firstDimension >= 0) {
                  ...
                  if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                    ...
                    if (elementSize != 0) {
                      if (vp->trace())
                        traceMsg(vp->comp(),
                          "Using class lookahead info to find out non null, array dimension, and object location\n");
                      vp->addGlobalConstraint(
                        node, TR::VPArrayInfo::create(vp, firstDimension, firstDimension, elementSize));
                      vp->addGlobalConstraint(
                        node, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::NotClassObject));
                    }
                  }
  ...
  if (symRef && symRef->getSymbol()->isStatic()) {
    if (symRef->getSymbol()->isConstString() && !symRef->isUnresolved() && vp->comp()->getStringClassPointer()) {
      constraint = TR::VPClass::create(vp, TR::VPConstString::create(vp, symRef), TR::VPNonNullObject::create(vp), NULL,
      ...
      if (sig) {
        TR_OpaqueClassBlock* classBlock
          = vp->fe()->getClassFromSignature(sig, len, symRef->getOwningMethod(vp->comp()));

        if (classBlock && TR::Compiler->cls.isInterfaceClass(vp->comp(), classBlock)
          && !vp->comp()->getOption(TR_TrustAllInterfaceTypeInfo)) {
          classBlock = NULL;
        }
        ...
        if (classBlock) {
          ...
          if (jlClass) {
            if (classBlock != jlClass) {
              constraint
                = TR::VPClassType::create(vp, sig, len, symRef->getOwningMethod(vp->comp()), isFixed, classBlock);
              if (*sig == '[' || sig[0] == 'L') {
                int32_t elementSize = arrayElementSize(sig, len, node, vp);
                if (elementSize != 0) {
...
TR::Node* constrainIaload(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    // hence can be treated as known object.
    {
          ...
          if (recognizedClassName) {
            ...
            if (recognizedClazz && (TR_yes == vp->fe()->isInstanceOf(baseObjectClazz, recognizedClazz, false))) {
              uintptrj_t fieldObject
                = vp->comp()->fej9()->getReferenceFieldAtAddress(baseObject + node->getSymbolReference()->getOffset());
              if (0 != fieldObject) {
                ...
                if (!TR::Compiler->cls.isClassArray(vp->comp(), fieldObjectClazz)) {
                  ...
                  constraint = TR::VPClass::create(vp, TR::VPKnownObject::create(vp, fieldObjectKnotIndex),
                    TR::VPNonNullObject::create(vp), NULL, NULL,
                    TR::VPObjectLocation::create(vp, TR::VPObjectLocation::HeapObject));
                  vp->addGlobalConstraint(node, constraint);
                } else {
                  ...
                  vp->addGlobalConstraint(node,
                    TR::VPArrayInfo::create(
                      vp, arrLength, arrLength, TR::Compiler->om.getArrayElementWidthInBytes(vp->comp(), fieldObject)));
                }
              }
            }
          }
    ...
    } else if (base && base->isJavaLangClassObject() == TR_yes && base->isNonNullObject() && base->getKnownObject()) {
            ...
            if (TR::Compiler->cls.isClassInitialized(vp->comp(), clazz)) {
              ...
              if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), clazz)
                ...
                vp->addBlockOrGlobalConstraint(node,
                  TR::VPClass::create(vp, TR::VPFixedClass::create(vp, clazz), TR::VPNonNullObject::create(vp), NULL,
                    NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                  isGlobal);
              } else {
                vp->addBlockOrGlobalConstraint(node,
                  TR::VPClass::create(vp, TR::VPResolvedClass::create(vp, clazz), TR::VPNonNullObject::create(vp), NULL,
                    NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                  isGlobal);
              }
              node->setIsNonNull(true);
      ...
      if (knot && symRef == vp->comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) {
        ...
        TR::KnownObjectTable::Index knownObjectIndex
          = knot->getIndexAt((uintptrj_t*)(base->getClass() + fej9->getOffsetOfJavaLangClassFromClassField()));
        vp->addBlockOrGlobalConstraint(node,
        ...
        if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), componentClass)
          ...
          vp->addBlockOrGlobalConstraint(node,
            TR::VPClass::create(vp, TR::VPFixedClass::create(vp, componentClass), TR::VPNonNullObject::create(vp), NULL,
              NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
            isGlobal);
        } else {
          vp->addBlockOrGlobalConstraint(node,
            TR::VPClass::create(vp, TR::VPResolvedClass::create(vp, componentClass), TR::VPNonNullObject::create(vp),
          ...
          if (bypassOnThisRun) {
            ...
            if (bypassLocation
              ...
              node->setSymbolReference(
                vp->comp()->getSymRefTab()->createKnownStaticReferenceSymbolRef(bypassLocation, targetKOI));
              return node;
            }
          }
        }
    ...
    if (!symRef->getSymbol()->isArrayShadowSymbol()) {
      TR::Symbol* sym = symRef->getSymbol();
      bool haveClassLookaheadInfo = false;
      if (((sym->isShadow() && node->getFirstChild()->isThisPointer()) || sym->isStatic()) && !symRef->isUnresolved()
                ...
                if (node && node->getOpCode().hasSymbolReference()
                  ...
                  fieldName
                    = node->getSymbolReference()
                        ...
                        ->fieldName(node->getSymbolReference()->getCPIndex(), fieldNameLen, vp->comp()->trMemory());
                }
              ...
              if (underlyingArray && underlyingArray->getOpCode().hasSymbolReference()
                ...
                if (underlyingArray->getSymbolReference()->getSymbol()->isShadow())
                  fieldName = underlyingArray->getSymbolReference()
                                ->getOwningMethod(vp->comp())
                                ->fieldName(underlyingArray->getSymbolReference()->getCPIndex(), fieldNameLen,
                                  vp->comp()->trMemory());
                else
                  fieldName = underlyingArray->getSymbolReference()
                                ->getOwningMethod(vp->comp())
                                ->staticName(underlyingArray->getSymbolReference()->getCPIndex(), fieldNameLen,
                                  vp->comp()->trMemory());
              }
  ...
  if (node->getSymbolReference()) {
    if (!node->getSymbolReference()->isUnresolved()
      ...
      && (node->getSymbolReference()->getCPIndex() >= 0) && node->getSymbolReference()->getOwningMethod(vp->comp())) {
      int32_t fieldSigLen;
...
#ifdef J9_PROJECT_SPECIFIC
  if (valueChild->getType().isBCD()) // sign state constraints are currently only propagated to loads so only constrain
                                     // on stores vs every BCD node
          ...
          if (useDefInfo->getUseDef(defs, useIndex)) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
              ...
              if (defNode && (defNode->getOpCodeValue() == TR::astore) && (defNode->getNumChildren() > 0)) {
                ...
                if ((defChild->getOpCodeValue() == TR::New) || (defChild->getOpCodeValue() == TR::newarray)
                  || (defChild->getOpCodeValue() == TR::anewarray) || (defChild->getOpCodeValue() == TR::multianewarray)
  ...
  if (node->getOpCode().isWrtBar()) {
    ...
    if (constraint) {
      if (constraint->isHeapObject() == TR_yes
        ...
        && performTransformation(
          vp->comp(), "%sMarking the wrtbar node [%p] - destination is a non-heap object\n", OPT_DETAILS, node)) {
...
TR::Node* constrainThrow(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  for (TR::Block* catchBlock = oehi.getFirst(); catchBlock; catchBlock = oehi.getNext()) {
    ...
    if (catchBlock->getCatchType() == 0) {
      ...
      willCatch
        = vp->fe()->isInstanceOf(constraint->getClass(), catchBlock->getExceptionClass(), constraint->isFixedClass());
      if (willCatch == TR_yes)
          ...
          if (objectConstraint->asClassType() && castConstraint->asClass()
            ...
            if (objectConstraint->isClassObject() == TR_yes)
              intersectConstraint = TR::VPClass::create(
                vp, dilutedType, NULL, NULL, NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::ClassObject));
            else
          ...
          if (jlClass) {
            ...
            if (classType->getClass() == jlClass) {
              ...
              if ((classType->isClassObject() == TR_maybe)
                ...
                newClassType
                  = TR::VPResolvedClass::create(vp, classType->getClass()); // Do not want to get the 'fixed' property
            }
          }
        } else {
          ...
          if (useDefInfo->getUseDef(defs, useIndex)) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
              ...
              if (defNode && (defNode->getOpCodeValue() == TR::astore)) {
                ...
                if (methodSig
                  && !strncmp(methodSig, "java/util/HashMap.get(Ljava/lang/Object;)Ljava/lang/Object;", 59)) {
...
TR::Node* constrainMultiANewArray(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  //
  TR_ASSERT(
    node->getFirstChild()->getOpCode().isLoadConst() && node->getFirstChild()->getInt() == node->getNumChildren() - 2,
  ...
  int32_t maxSize = (int32_t)TR::Compiler->om.maxArraySizeInElementsForAllocation(
    node, vp->comp()); // int32_t is ok because multiANewArray is only useful in Java, where array sizes are bound by
...
static TR::VPConstraint* createIntRangeConstraint(OMR::ValuePropagation* vp, int32_t l, int32_t h)
{
  ...
  B getRange, C getValue, D getValues, E createConst, F createRange, G processValue, T MIN_VALUE, T MAX_VALUE)
{
  ...
  if (childConstraint) {
    if (getConst(childConstraint)) {
      ...
      if (vp->trace()) {
        traceMsg(
          vp->comp(), "The first child's value of %p %lld is replaced with %lld \n", node, value, processValue(value));
      }
      ...
      if (low < 0 && high < 0) {
        ...
        if (vp->trace()) {
          traceMsg(vp->comp(), "Constraint %lld .. %lld of %p 's first child is negative and folded into %lld \n", low,
            high, node, processValue(MAX_VALUE));
        }
...
TR::Node* constrainIntegerHighestOneBit(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainHighestOneBitAndLeadingZerosHelper(vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
    ...
    createIntRangeConstraint, integerToPowerOf2, (int32_t)TR::getMaxSigned<TR::Int32>(),
...
{
  return constrainHighestOneBitAndLeadingZerosHelper(vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
    createIntConstConstraint, createIntRangeConstraint, integerNumberOfLeadingZeros, (int32_t)0, (int32_t)-1);
}
...
{
  return constrainHighestOneBitAndLeadingZerosHelper(vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
    createIntConstConstraint, createIntRangeConstraint, longNumberOfLeadingZeros, (int64_t)0, (int64_t)-1);
}
...
TR::Node* constrainIntegerLowestOneBit(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
    createIntConstConstraint, createIntRangeConstraint, integerToPowerOf2, (int32_t)TR::getMaxSigned<TR::Int32>(),
    (int32_t)TR::getMinSigned<TR::Int32>());
}
...
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
    createIntConstConstraint, createIntRangeConstraint, integerBitCount, (int32_t)0, (int32_t)-1);
}

TR::Node* constrainIntegerNumberOfTrailingZeros(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
    createIntConstConstraint, createIntRangeConstraint, integerNumberOfTrailingZeros, (int32_t)0, (int32_t)-1);
}
...
TR::Node* constrainLongLowestOneBit(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
    createLongConstConstraint, createLongRangeConstraint, longToPowerOf2, (int64_t)TR::getMaxSigned<TR::Int64>(),
...
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
    createIntConstConstraint, createIntRangeConstraint, longNumberOfTrailingZeros, (int64_t)0, (int64_t)-1);
}

TR::Node* constrainLongBitCount(OMR::ValuePropagation* vp, TR::Node* node)
{
  return constrainLowestOneBitAndTrailingZerosHelper(vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
    createIntConstConstraint, createIntRangeConstraint, longBitCount, (int64_t)0, (int64_t)-1);
}
...
TR::Node* constrainArraylength(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  //
  if (!(node->getOpCodeValue() == TR::contigarraylength || node->getOpCodeValue() == TR::discontigarraylength)
    || (lowerBoundLimit != upperBoundLimit) || !TR::Compiler->om.isDiscontiguousArray(upperBoundLimit, elementSize)) {
...
static void refineSymbolReferenceForProfiledToOverridden(
  OMR::ValuePropagation* vp, TR::Node* callNode, TR::SymbolReference* newSymRef)
{
...
static void devirtualizeCall(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (methodSymbol->isComputed()) {
    ...
    if (methodSymbol->getRecognizedMethod() == TR::java_lang_invoke_MethodHandle_invokeExact
      ...
      if (constraint && constraint->getKnownObject() && knot) {
        ...
        resolvedMethod = vp->comp()->fej9()->createMethodHandleArchetypeSpecimen(vp->trMemory(),
          knot->getPointerLocation(constraint->getKnownObject()->getIndex()), owningMethod->getResolvedMethod());
        if (resolvedMethod) {
          ...
          if (performTransformation(vp->comp(), "%sSubstituting more specific method symbol on %p: %s\n", OPT_DETAILS,
                node, specimenSymRef->getName(vp->getDebug()))) {
          ...
          if (vp->optimizer()->getOptimization(OMR::methodHandleInvokeInliningGroup)->requested()) {
            if (vp->trace())
              traceMsg(vp->comp(),
                "Not inlining call [%p] because the MethodHandle.invoke inlining group will do a better job\n", node);
            return;
          }
        } else {
          if (vp->trace())
        ...
        if (vp->trace())
          traceMsg(
            vp->comp(), "Not inlining call [%p] because unable to substitute object-specific method symbol\n", node);
        return;
      }
    }
#endif
    ...
    if (TR::Compiler->cls.isInterfaceClass(vp->comp(), thisType)) {
      if (vp->trace())
        traceMsg(
          vp->comp(), "Not attempting to de-virtualize interface call [%p] with interface-class receiver\n", node);
      return;
    }
    ...
    if (!resolvedMethod) {
      if (vp->trace())
        traceMsg(
          vp->comp(), "Not attempting to de-virtualize interface call [%p] with no resolved method\n", node, len, s);
      return;
    }
    ...
    TR_ASSERT(
      methodSymbol->isVirtual(), "unexpected method symbol kind for %s node %p", node->getOpCode().getName(), node);
  ...
  if (methodSymbol->isVirtual() || methodSymbol->isInterface()) {
    ...
    if (constraint->isFixedClass() || constraint->isPreexistentObject()) {
      if (!performTransformation(vp->comp(), "%sChanging an indirect call %s (%s) to a direct call [%p]\n", OPT_DETAILS,
            resolvedMethod->signature(vp->trMemory()), node->getOpCode().getName(), node))
        return;
  ...
  if (tracePrex)
    traceMsg(
      vp->comp(), "PREX.vp: Value propagation populating prex argInfo for %s %p\n", node->getOpCode().getName(), node);
    ...
    if (argument->getDataType() == TR::Address) {
      ...
      if (constr) {
        ...
        if (constr->asKnownObject() && constr->isNonNullObject()) {
          ...
          argInfo->set(
            c - firstArgIndex, new (vp->trStackMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsPreexistent));
          if (tracePrex)
...
TR::Node* constrainCall(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (symbol) {
    ...
    if (isIntegerAbs || isLongAbs) {
      ...
      // value of the range (TR::getMinSigned<TR::Int32>() or TR::getMinSigned<TR::Int64>()) then the return value of
      // the ABS could be that same minimum value (yes, that's right, it could be negative).
      bool operandGlobal;
      ...
      /*
      n572n       vcall  sun/misc/Unsafe.ensureClassInitialized(Ljava/lang/Class;)V[#631  final native virtual Method
      -728] n577n         aload  java/lang/invoke/MethodHandle$UnsafeGetter.myUnsafe n579n         iaload
    ...
    if (lhs->asIntConst() && rhs->asIntConst()) {
      ...
      if (lhsInt && rhsInt) {
        ...
        if (isUnsigned) {
          ...
          range_min = 0;
          range_max = TR::getMaxSigned<TR::Int32>(); // we don't support unsigned range constraint yet. Only support
        ...
        if (constrainIntegerDivisionRange(
              lhs1, lhs2, rhs1, rhs2, range_min, range_max, lo, hi, vp->computeDivRangeWhenDivisorCanBeZero(node)))
  ...
  TR::VPConstraint* firstChildConstraint, TR::VPConstraint* secondChildConstraint)
{
  ...
  if (powerOfTen) {
    bool isUnsigned = node->getOpCode().isUnsigned();
    // the max value for a remainder result is the divisor value - 1 so when the divisor is a power of ten then -1 this
    // value is one decimal precision value less (e.g. 100 - 1 = 99)
    ...
    if (!isUnsigned && fcPrecision <= refinedPrecision
      ...
      //    iconst 1000
      //
      // this doesn't work for unsigned (iurem) as these ops treat the operands as unsigned so, for example, a prec=1
      // dividend that is negative when interpreted as unsigned will actually have 10 digits of precision (-1 would be
  ...
  if (child) {
    if (child->asIntConst()) {
      ...
      if (low == TR::getMinSigned<TR::Int32>()) {
        lowConstraint
          = TR::VPIntRange::create(vp, TR::getMinSigned<TR::Int32>(), TR::getMinSigned<TR::Int32>(), TR_yes);
        low++;
      }
...
TR::Node* constrainIabs(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (child == NULL) {
    vp->addGlobalConstraint(node,
      TR::VPMergedConstraints::create(vp, TR::VPIntConst::create(vp, TR::getMinSigned<TR::Int32>()),
    ...
    if (low == high) {
      ...
      if (low < 0 && high <= 0) {
        ...
        low = 0;
                   ...
                   OPT_DETAILS, node->getOpCode().getName(), node, node->getFirstChild()->getOpCode().getName(),
                   node->getFirstChild())) {
    ...
    if (replaceNode) {
      ...
      if (lhsORrhs) {
        ...
        if ((low <= 0) && (((int64_t)(high + 1) & (pow2Val - 1))) == 0) {
          if (vp->trace())
            traceMsg(
              vp->comp(), "found opportunity to distribute shift in node [%p] replaceNode [%p]\n", node, replaceNode);
        } else {
          if (vp->trace())
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (constraint) {
      ...
      if (longShr) {
        TR::Node* secondChild = node->getSecondChild();
        didReduction = reduceLongOpToIntegerOp(vp, node, constraint);
        // If the shift amount is between 32 and 63, inclusive, then if we convert the node to an integer shift we must
        // reduce this shift value. Integer shifts are masked by 0x1f in java, and if the first child is integer
        // constrained, a shift by an amount in this range is effectively just a shift by 31.
...
TR::Node* constrainIushr(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (low == high)
      ...
      constraint
        = TR::VPIntRange::create(vp, ((uint32_t)low) >> shiftAmount, ((uint32_t)high) >> shiftAmount /*, isUnsigned*/);
      ...
      if (rhs && rhs->asIntConst()) {
        ...
        if ((firstChild->getOpCodeValue() == TR::iloadi || firstChild->getOpCodeValue() == TR::lloadi)
          ...
          if (vftLoad->getOpCodeValue() == TR::aloadi || vftLoad->getOpCodeValue() == TR::loadaddr) {
            ...
            if (baseExpressionConstraint && baseExpressionConstraint->getClassType()
              && (baseExpressionConstraint->getClassType()->isArray() != TR_maybe)) {
              if (vp->trace())
              ...
              TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                TR::DebugCounter::debugCounterName(vp->comp(), "isArrayTest/hit/(%s)/%s", vp->comp()->signature(),
                  vp->comp()->getHotnessName(vp->comp()->getMethodHotness())));
            } else {
              TR::DebugCounter::incStaticDebugCounter(vp->comp(),
                TR::DebugCounter::debugCounterName(vp->comp(), "isArrayTest/miss/(%s)/%s", vp->comp()->signature(),
                  vp->comp()->getHotnessName(vp->comp()->getMethodHotness())));
            }
          } else {
            TR::DebugCounter::incStaticDebugCounter(vp->comp(),
              TR::DebugCounter::debugCounterName(vp->comp(), "isArrayTest/unsuitable/(%s)/%s", vp->comp()->signature(),
                vp->comp()->getHotnessName(vp->comp()->getMethodHotness())));
          }
        }
      }
...
void replaceWithSmallerType(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (performTransformation(comp,
        "%sReplace [" POINTER_PRINTF_FORMAT "] and [" POINTER_PRINTF_FORMAT "] with store and load of a smaller type\n",
...
TR::Node* constrainNarrowToChar(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (node->getFirstChild()->getOpCodeValue() == TR::b2i || node->getFirstChild()->getOpCodeValue() == TR::bu2i
    || node->getFirstChild()->getOpCodeValue() == TR::b2s || node->getFirstChild()->getOpCodeValue() == TR::bu2s
    || node->getFirstChild()->getOpCodeValue() == TR::b2l || node->getFirstChild()->getOpCodeValue() == TR::bu2l) {
  ...
  if (constraint) {
    if (constraint->asIntConstraint() || constraint->asMergedConstraints()) {
      if (constraint->getLowInt() > 0 && constraint->getLowInt() < TR::getMaxUnsigned<TR::Int16>()
        && constraint->getHighInt() > 0 && constraint->getHighInt() < TR::getMaxUnsigned<TR::Int16>()) {
        if (constraint->getLowInt() > low)
          low = constraint->getLowInt();
        if (constraint->getHighInt() < high)
          high = constraint->getHighInt();
...
static TR::Node* getOriginalCallNode(TR_VirtualGuard* vGuard, TR::Compilation* comp, TR::Node* guardNode)
{
  ...
  for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
    if (tt->getNode()->getNumChildren()) {
      ...
      if (possiblyCall->getOpCode().isCall() && possiblyCall->getOpCode().isIndirect()
...
static void addDelayedConvertedGuard(TR::Node* node, TR::Node* callNode, TR::ResolvedMethodSymbol* methodSymbol,
  TR_VirtualGuard* oldVirtualGuard, OMR::ValuePropagation* vp, TR_VirtualGuardKind guardKind,
  TR_OpaqueClassBlock* objectClass)
{
  ...
  if (!callNode->getFirstChild()->getOpCode().hasSymbolReference()
    ...
    || !callNode->getFirstChild()->getFirstChild()->getOpCode().isLoadVarDirect()
    || // I guess, having a vft symbol implies that there is something underneath it?
    !callNode->getSecondChild()->getOpCode().isLoadVarDirect()
    || // it also implies, there should be a receiver argument?
  ...
  TR::Node* newGuardNode
    = TR_VirtualGuard::createMethodGuardWithReceiver(guardKind, vp->comp(), oldVirtualGuard->getCalleeIndex(), callNode,
      node->getBranchDestination(), methodSymbol, objectClass /*oldVirtualGuard->getThisClass()*/, newReceiver);
  ...
  vp->_convertedGuards.add(new (vp->trStackMemory())
      OMR::ValuePropagation::VirtualGuardInfo(vp, oldVirtualGuard, newGuard, newGuardNode, callNode));
}

#ifdef J9_PROJECT_SPECIFIC
TR_ResolvedMethod* findSingleImplementer(TR_OpaqueClassBlock* thisClass, int32_t cpIndexOrVftSlot,
  TR_ResolvedMethod* callerMethod, TR::Compilation* comp, bool locked, TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
  ...
  int32_t implCount = TR_ClassQueries::collectImplementorsCapped(
    classInfo, implArray, 2, cpIndexOrVftSlot, callerMethod, comp, locked, useGetResolvedInterfaceMethod);
...
static TR::Node* constrainIfcmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool branchOnEqual)
{
    ...
    if (rhsConst == 0 || rhsConst == 1) {
      ...
      if (lhsOp.isInt() || lhsOp.isLong() || lhsOp.isRef()) {
        ...
        if (performTransformation(vp->comp(), "%sChanging n%un (%s (%s ...) %d) to (%s ...)\n", OPT_DETAILS,
              node->getGlobalIndex(), node->getOpCode().getName(), lhsCmp.getName(), rhsConst, newIfOp.getName())) {
  ...
  if (((node->getOpCodeValue() == TR::ifacmpne || node->getOpCodeValue() == TR::ifacmpeq
         || node->getOpCodeValue() == TR::ificmpne || node->getOpCodeValue() == TR::ificmpeq)
  ...
  //
    ...
    if (lhs && rhs) {
      ...
      if (lhs->mustBeEqual(rhs, vp) && ignoreVirtualGuard) {
        ...
        if (vp->trace())
          traceMsg(
            vp->comp(), "   cmp children must be not equal by absolute constraints: %p != %p\n", lhsChild, rhsChild);
        if (branchOnEqual)
          cannotBranch = true;
        else
          cannotFallThrough = true;
  ...
  if (!cannotBranch && !cannotFallThrough) {
    ...
    if (rel && ignoreVirtualGuard) {
      ...
      if (rel->mustBeEqual()) {
        ...
        if (vp->trace())
          traceMsg(
            vp->comp(), "   cmp children must be not equal by relative constraint: %p != %p\n", lhsChild, rhsChild);
        if (branchOnEqual)
          cannotBranch = true;
        else
          cannotFallThrough = true;
        ...
        if (typeConstraint) {
          ...
          if (clazzToBeInitialized
            && (((clazzNameLen == 35)
              ...
              || ((clazzNameLen == 41)
                && !strncmp(clazzToBeInitialized, "Lcom/ibm/websphere/ras/TraceEnabledToken;", clazzNameLen))
              || ((clazzNameLen == 40)
                && !strncmp(clazzToBeInitialized, "Ljava/lang/String$StringCompressionFlag;", clazzNameLen)))
            && performTransformation(vp->comp(),
              "%sUsing side-effect guard to fold away condition on a load from an uninitializec class  %s [%p]\n",
    ...
    if (virtualGuard) {
      ...
      case TR_VftTest: {
        ...
        if (enableJavaLangClassFolding && (instanceofObjectRef->getOpCodeValue() == TR::aloadi)
          ...
          if (ignoreVirtualGuard && classConstraint && classConstraint->getClassType() && objectConstraint
            && objectConstraint->isClassObject() == TR_yes && vp->comp()->getClassClassPointer()) {
      ...
      case TR_MethodTest: {
        ...
        if ((vtableEntryNode->getOpCodeValue() == TR::aloadi) && vtableEntryNode->getOpCode().hasSymbolReference()
        ...
        /*(vtableEntryNode->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef()))*/
        {
          ...
          if (ignoreVirtualGuard && classConstraint && classConstraint->isFixedClass()) {
            ...
            if (vp->trace())
              traceMsg(vp->comp(),
                "TR_MethodTest: node=%p, vtableEntryNode=%p, clazz=%p, vftOffset=%d, vftEntry=%p, childrenAreEqual=%d, "
                "testForEquality=%d\n",
          ...
          if (vtableEntryNode->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef()) {
            TR_OpaqueClassBlock* classFromMethod
              = vp->comp()->fe()->getClassFromMethodBlock((TR_OpaqueMethodBlock*)methodPtrNode->getAddress());
  ...
  if (node->isTheVirtualGuardForAGuardedInlinedCall() && node->isNonoverriddenGuard()) {
    ...
    if (callNode && callNode->getOpCode().isCallIndirect()) {
      ...
      if (constraint && constraint->isFixedClass() && (thisType = constraint->getClass())
  ...
  if (cannotBranch
    ...
    // if constraintFromMethodPointer is set we should be able to assume that lhsChild->getFirstChild()->getFirstChild()
    // is not NULL, since non-NULL constraintFromMethodPointer implies the following trees:
    // n235n     ifacmpne --> block_25 BBStart at n230n (inlineProfiledGuard )                       [0x7fffd5110a40]
    // bci=[0,0,314] rc=0 vc=56 vn=49 sti=- udi=- nc=2 flg=0x1020 n233n       iaload <vtable-entry-symbol> [#552 Shadow
    // +768] [flags 0x10607 0x0 ]              [0x7fffd51109b0] bci=[-1,0,2302] rc=1 vc=56 vn=7 sti=- udi=- nc=1 n232n
    // iaload <vft-symbol> [#449 Shadow] [flags 0x18607 0x0 ] (X>=0 )                  [0x7fffd5110968] bci=[-1,0,2302]
    // rc=1 vc=56 vn=6 sti=- udi=- nc=1 flg=0x100 n3n             ==>aload n234n       aconst 0x7fd070
    // (methodPointerConstant )                                          [0x7fffd51109f8] bci=[0,0,314] rc=1 vc=56 vn=5
    // sti=- udi=- nc=0 flg=0x2000

    bool isGlobal;
    TR::VPConstraint* intersected = vp->getConstraint(lhsChild->getFirstChild()->getFirstChild(), isGlobal);

    intersected = (intersected) ? intersected->intersect(constraintFromMethodPointer, vp)
    ...
    if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
      ...
      if (!callClass && !callNode->getSymbolReference()->isUnresolved()) {
        TR::ResolvedMethodSymbol* resolvedCallMethodSymbol
          = callNode->getSymbolReference()->getSymbol()->getResolvedMethodSymbol();
      ...
      if (objectClass && rhsClass && callClass && !lhs->isFixedClass()
        ...
        && vp->comp()->fe()->isInstanceOf(objectClass, callClass, true, true, true)
          == TR_yes) /*the object class may be less specific than callClass*/
      {
        ...
        if (methodSymbol
          && !(!methodSymbol->isInterface() && TR::Compiler->cls.isInterfaceClass(vp->comp(), objectClass))) {

          int32_t cpIndexOrVftSlot
            = TR::Compiler->cls.isInterfaceClass(vp->comp(), objectClass) ? symRef->getCPIndex() : symRef->getOffset();

          TR_YesNoMaybe useGetResolvedInterfaceMethod = methodSymbol->isInterface() ? TR_yes : TR_no;
          TR::MethodSymbol::Kinds methodKind
            = methodSymbol->isInterface() ? TR::MethodSymbol::Interface : TR::MethodSymbol::Virtual;

          TR_ResolvedMethod* rvm = findSingleImplementer(objectClass, cpIndexOrVftSlot,
            symRef->getOwningMethod(vp->comp()), vp->comp(), false, useGetResolvedInterfaceMethod);
          ...
          // we should treat this case conservatively meaning we assume that there are more than two concrete classes in
          // the given hierarchy most likely rvm will also be NULL in this case, but it is better to not rely on this
          // implicit connection between different CH queries.
          ...
          /*
            ...
            overriding methods in objectClass and if any call was devirtualized by invariantargumentpreexisence
            downstream we will produce the functionally incorrect code by swapping a profiled test with a method test as
          ...
          */
          ...
          if (searchSucceeded && numConcreteClasses(&subClasses) < 2 && rvm) {
            TR::ResolvedMethodSymbol* cMethodSymbol
              = vp->comp()
                  ->getSymRefTab()
                  ->findOrCreateMethodSymbol(symRef->getOwningMethodIndex(), -1, rvm, methodKind)
                  ->getSymbol()
                  ->castToResolvedMethodSymbol();

            TR_VirtualGuardKind guardKind = methodSymbol->isInterface()
              ? TR_InterfaceGuard
  ...
  bool isGlobal, bool childrenReversed)
{
  ...
  if ((lhsChild->getOpCode().isAdd() || lhsChild->getOpCode().isSub())
    ...
    && lhsChild->getSecondChild()->getOpCode().isLoadConst() && rhsChild->getSecondChild()->getOpCode().isLoadConst()) {
  ...
  OMR::ValuePropagation* vp, TR::Node* node, TR::Node* lhsChild, TR::Node* rhsChild, bool orEqual)
{
    ...
    if (rel) {
      ...
      if (rel->asEqual() && !cannotFallThrough && !cannotBranch && (lhs || rhs)) {
        ...
        int64_t maxIncrement
          = 0; // If absIncrement is more than this, overflow may occur, and (for example) a==b+k does not imply a>b
  ...
  if (lhsChild->getOpCode().isLong()) {
    ...
    if (longBoundary != TR::getMinSigned<TR::Int64>() && !isUnsigned) {
      if (!vp->addEdgeConstraint(
            rhsChild, TR::VPLongRange::create(vp, longBoundary, TR::getMaxSigned<TR::Int64>()), edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
          vp->setIntersectionFailed(false);
    ...
    if (modifiedBoundary) {
      ...
      // if (!vp->addEdgeConstraint(rhsChild, TR::VPIntRange::create(vp, intBoundary, TR::getMaxSigned<TR::Int32>()),
      // edgeConstraints))
    ...
    if (modifiedBoundary) {
      ...
      // if (!vp->addEdgeConstraint(lhsChild, TR::VPIntRange::create(vp, TR::getMinSigned<TR::Int32>(), intBoundary),
      // edgeConstraints))
  ...
  if (lhsChild->getOpCode().isLong() && !isUnsigned) {
    ...
    if (modifiedBoundary) {
      TR::VPConstraint* constraint = TR::VPIntRange::create(vp, minIntValue, intBoundary /*, isUnsigned*/);
      // if (!vp->addBlockConstraint(rhsChild, TR::VPIntRange::create(vp, TR::getMinSigned<TR::Int32>(), intBoundary),
      // NULL, false))
    ...
    if (modifiedBoundary) {
      TR::VPConstraint* constraint = TR::VPIntRange::create(vp, intBoundary, maxIntValue /*, isUnsigned*/);
      // if (!vp->addBlockConstraint(lhsChild, TR::VPIntRange::create(vp, intBoundary, TR::getMaxSigned<TR::Int32>()),
      // NULL, false))
        ...
        if (firstChild->getOpCodeValue() == node->getOpCodeValue()) {
          ...
          if (secondGrandChild->getOpCode().isLoadConst() && firstGrandIsBool && vp->getCurrentParent()) {
            ...
            if (constValue == firstGrandConstValue) {
              if (performTransformation(
                    vp->comp(), "%sReduced 2 NOTs of bool in node [" POINTER_PRINTF_FORMAT "] \n", OPT_DETAILS, node)) {
                TR::Node* parent = vp->getCurrentParent();
                vp->invalidateValueNumberInfo();
                vp->invalidateUseDefInfo();
...
static void constrainClassObjectLoadaddr(OMR::ValuePropagation* vp, TR::Node* node, bool isGlobal)
{
  ...
  TR::VPConstraint* constraint = TR::VPClass::create(vp, TR::VPClassType::create(vp, symRef, true),
    TR::VPNonNullObject::create(vp), NULL, NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject));
...
TR::Node* constrainLoadaddr(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (symbol->isAddressOfClassObject()) {
    ...
    TR::VPConstraint* constraint = TR::VPClass::create(vp, TR::VPClassType::create(vp, symRef, false, true), NULL, NULL,
      NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject));
    vp->addGlobalConstraint(node, constraint);
...
TR::Node* constrainZeroChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  TR::Node* child = node->getFirstChild();
  bool isGlobal;
  TR::VPConstraint* nonzeroConstraint = TR::VPLongRange::create(vp, TR::getMinSigned<TR::Int64>(), -1)
...
TR::Node* constrainBndChkWithSpineChk(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (sizeConstraint && !vp->comp()->generateArraylets()) {
    ...
    if (elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(sizeConstraint->getHighInt(), elementSize)
      ...
      && performTransformation(vp->comp(),
        "%sRemoving unnecessary spine check from composite node [%p] : min arraylength = %d, max arraylength=%d, "
        "element size=%d\n",
      ...
      if (sig && sig[0] == '[') {
        ...
        //
          ...
          if (arrayComponentClass)
            isInstance
              = vp->fe()->isInstanceOf(objectClass, arrayComponentClass, object->isFixedClass(), array->isFixedClass());
          // isInstance = vp->fe()->isInstanceOf(object->getClass(), arrayComponentClass, object->isFixedClass(),
          // array->isFixedClass());
          ...
          if (isInstance == TR_yes) {
            ...
            && !TR::Compiler->cls.isClassArray(vp->comp(), arrayComponentClass) && (arrayComponentClass == objectClass)
          ...
          else if (!vp->comp()->compileRelocatableCode() && !(vp->comp()->getOption(TR_DisableArrayStoreCheckOpts))
...
TR::Node* constrainBCDToIntegral(OMR::ValuePropagation* vp, TR::Node* node)
{
  ...
  if (node->getType().isInt64()) {
    ...
    // we know that it can't possibly be represented as a negative number, so the min value for the constraint must be
    // at least 0
