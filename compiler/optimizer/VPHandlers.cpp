...
extern void createGuardSiteForRemovedGuard(TR::Compilation *comp,
                                           TR::Node *ifNode);

static void checkForNonNegativeAndOverflowProperties(
    OMR::ValuePropagation *vp, TR::Node *node,
  ...
  if (constraint) {
    if (constraint->asIntConst()) {
      ...
      if (low >= 0) node->setIsNonNegative(true);
      if (low <= 0) node->setIsNonPositive(true);
    }
    ...
    if (constraint->asIntRange()) {
      ...
      if (high <= 0) node->setIsNonPositive(true);

      /// if ((node->getOpCode().isArithmetic() || node->getOpCode().isLoad())
      /// &&
      ///      ((low > TR::getMinSigned<TR::Int32>()) || (high <
      ...
      /// if ((node->getOpCode().isArithmetic() || node->getOpCode().isLoad())
      /// &&
      ///      ((low > TR::getMinSigned<TR::Int64>()) || (high <
      ///      TR::getMaxSigned<TR::Int64>())))
      if ((node->getOpCode().isLoad() &&
      ...
      if ((node->getOpCode().isLoad() &&
           ((low > TR::getMinSigned<TR::Int16>()) ||
            (high < TR::getMaxSigned<TR::Int16>()))) ||
          (node->getOpCode().isArithmetic() &&
           (range->canOverflow() != TR_yes))) {
        node->setCannotOverflow(true);
      }
...
static int32_t arrayElementSize(const char *signature, int32_t len,
                                TR::Node *node, OMR::ValuePropagation *vp) {
  ...
  if (signature[0] == '[') {
    switch (signature[1]) {
      ...
      case 'J':
        return 8;
      case 'Z':
        return TR::Compiler->om.elementSizeOfBooleanArray();
      case 'L':
      default:
...
static void constrainBaseObjectOfIndirectAccess(OMR::ValuePropagation *vp,
                                                TR::Node *node) {
  ...
  char *baseObjectSig = method->classNameOfFieldOrStatic(
      node->getSymbolReference()->getCPIndex(), baseObjectSigLen);
  if (baseObjectSig)
    baseObjectSig =
        classNameToSignature(baseObjectSig, baseObjectSigLen, vp->comp());
  if (baseObjectSig) {
    TR_OpaqueClassBlock *baseClass = vp->fe()->getClassFromSignature(
        baseObjectSig, baseObjectSigLen, method);
    if (baseClass &&
    ...
    if (baseClass) {
      ...
      bool isGlobal;
      TR::VPConstraint *baseConstraint =
          vp->getConstraint(node->getFirstChild(), isGlobal);
      TR::VPConstraint *constraint = TR::VPClassType::create(
          vp, baseObjectSig, baseObjectSigLen, method, false, baseClass);
      // only if its a class constraint
      if (baseConstraint && baseConstraint->getClassType()) {
        if (vp->trace()) {
          traceMsg(vp->comp(),
                   "Found existing class constraint for node (%p) :\n",
                   node->getFirstChild());
          baseConstraint->print(vp->comp(), vp->comp()->getOutFile());
        }
        // check if it can be refined
        if (!constraint->intersect(baseConstraint, vp)) return;  // nope, bail
      }
...
static bool tryFoldCompileTimeLoad(OMR::ValuePropagation *vp, TR::Node *node,
                                   bool &isGlobal) {
  isGlobal = true;
  ...
  if (node->getOpCode().isIndirect()) {
    ...
    for (; !baseExpression;) {
      // Track last curNode because its symref might be improved to
      // ImmutableArrayShadow
      ...
      if (!constraint) {
        ...
        if (prevNodeSym->isArrayShadowSymbol() &&
            // UnsafeShadow responds true to above question, but the type of the
            // array element might not match the type of the symbol, it needs to
            // be derived from the array
            !prevNodeSym->isUnsafeShadowSymbol() &&
            !vp->comp()->getSymRefTab()->isImmutableArrayShadow(
                prevNode->getSymbolReference())) {
          if (constraint->getKnownObject()->isArrayWithConstantElements(
                  vp->comp())) {
            // Use the DataType of the symbol because it represents the element
            // type of the array
            TR::SymbolReference *improvedSymRef =
                vp->comp()
                    ->getSymRefTab()
                    ->findOrCreateImmutableArrayShadowSymbolRef(
                        prevNodeSym->getDataType());
            if (vp->trace())
              traceMsg(vp->comp(),
                       "Found arrayShadow load %p from array with constant "
                       "elements %p\n",
                       prevNode, curNode);
            if (performTransformation(
                    vp->comp(),
                    "%sUsing ImmutableArrayShadow symref #%d for node %p\n",
                    OPT_DETAILS, improvedSymRef->getReferenceNumber(),
                    prevNode))
              prevNode->setSymbolReference(improvedSymRef);

            // Alias info of calls are computed in createAliasInfo, which is
            // only called if alias info is not available or is invalid.
            // Invalidate alias info so that it can be recomputed in the next
        ...
        baseExpression = curNode;
        baseKnownObject =
            knot->getIndexAt((uintptrj_t *)constraint->getConstString()
                                 ->getSymRef()
                                 ->getSymbol()
                                 ->castToStaticSymbol()
                                 ->getStaticAddress());
        ...
        break;
                 ...
                 constraint->isNonNullObject() && constraint->getClassType() &&
        ...
        if (nodeToRemove) {
          TR_ASSERT(didSomething,
                    "How can there be a node to remove if "
                    "transformIndirectLoadChain didn't do something??");
        ...
        if (curNode->getOpCodeValue() == TR::loadaddr &&
            ...
            !curNode->getSymbolReference()->isUnresolved()) {
          TR::Node *nodeToRemove = NULL;
          uintptrj_t clazz = (uintptrj_t)curNode->getSymbolReference()
                                 ->getSymbol()
                                 ->getStaticSymbol()
                                 ->getStaticAddress();
          bool didSomething = TR::TransformUtil::transformIndirectLoadChainAt(
          ...
          if (vp->trace())
            traceMsg(vp->comp(),
                     "  - FAIL: %s %p is not an indirect load and has "
...
#ifdef J9_PROJECT_SPECIFIC
      else if (vp->comp()->fej9()->isFinalFieldPointingAtJ9Class(
                   curNode->getSymbolReference(), vp->comp())) {
        if (vp->trace())
          traceMsg(vp->comp(),
                   " - FAIL: %s %p points to a j9class but has insufficient "
                   "constraints\n",
                   curNode->getOpCode().getName(), curNode);
        baseExpression = NULL;
        break;
      } else if (vp->comp()->fej9()->canDereferenceAtCompileTime(
                     curNode->getSymbolReference(), vp->comp())) {
...
static bool constrainCompileTimeLoad(OMR::ValuePropagation *vp,
                                     TR::Node *node) {
  bool isGlobal;
  if (!tryFoldCompileTimeLoad(vp, node, isGlobal)) return false;
    ...
    switch (type) {
        ...
        if (constraint->asLongConst()) {
          ...
          if (!vp->cg()->materializesLargeConstants() ||
              ...
              (constraint->asLongConst()->getLong() <
                   vp->cg()->getSmallestPosConstThatMustBeMaterialized() &&
               constraint->asLongConst()->getLong() >
        ...
        if (constraint->isNullObject()) {
          ...
          if (constraint->getKnownObject()) {
            ...
            if (node->getOpCode().isLoadIndirect() &&
              ...
              TR::KnownObjectTable *knot = vp->comp()->getKnownObjectTable();
              TR_ASSERT(knot,
                        "Can't have a known-object constraint without a "
                        "known-object table");
              TR::SymbolReference *improvedSymRef =
              ...
              if (improvedSymRef->hasKnownObjectIndex()) {
                if (performTransformation(vp->comp(),
                                          "%sUsing known-object specific "
                                          "symref #%d for obj%d at [%p]\n",
                                          OPT_DETAILS,
                                          improvedSymRef->getReferenceNumber(),
                                          knownObject->getIndex(), node)) {
      ...
      default:
        if (constraint->asIntConstraint()) {
          ...
          if (constraint->getHighInt() <= 0) node->setIsNonPositive(true);
          ...
          if (constraint->getHighShort() <= 0) node->setIsNonPositive(true);
...
static bool containsUnsafeSymbolReference(OMR::ValuePropagation *vp,
                                          TR::Node *node) {
  if (node->getSymbolReference()->isLitPoolReference()) return true;

  if (node->getSymbolReference()->getSymbol()->isShadow()) {
    if (node->getSymbol()->isUnsafeShadowSymbol()) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "Node [%p] has an unsafe symbol reference %d, no constraint\n",
                 node, node->getSymbolReference()->getReferenceNumber());
      return true;
    }

    if (node->getSymbolReference() ==
            vp->getSymRefTab()->findInstanceShapeSymbolRef() ||
        node->getSymbolReference() ==
            vp->getSymRefTab()->findInstanceDescriptionSymbolRef() ||
        node->getSymbolReference() ==
            vp->getSymRefTab()->findDescriptionWordFromPtrSymbolRef() ||
#ifdef J9_PROJECT_SPECIFIC
        node->getSymbolReference() ==
            vp->getSymRefTab()
                ->findClassFromJavaLangClassAsPrimitiveSymbolRef() ||
#endif
        (node->getSymbolReference()->getSymbol() ==
         vp->comp()->getSymRefTab()->findGenericIntShadowSymbol()))
      return true;
  }
...
static bool owningMethodDoesNotContainNullChecks(OMR::ValuePropagation *vp,
                                                 TR::Node *node) {
  TR::ResolvedMethodSymbol *method =
      node->getSymbolReference()->getOwningMethodSymbol(vp->comp());
  if (method && method->skipNullChecks()) return true;
  return false;
}

bool constraintFitsInIntegerRange(OMR::ValuePropagation *vp,
  ...
  if (longConstraint) {
    int64_t low = longConstraint->getLowLong(),
            high = longConstraint->getHighLong();
...
bool canMoveLongOpChildDirectly(TR::Node *node, int32_t numChild,
                                TR::Node *arithNode) {
  return node->getChild(numChild)->getDataType() == arithNode->getDataType() ||
         (node->getOpCodeValue() == TR::lshr && numChild > 0);
}

bool reduceLongOpToIntegerOp(OMR::ValuePropagation *vp, TR::Node *node,
                             TR::VPConstraint *nodeConstraint) {
  if (!constraintFitsInIntegerRange(vp, nodeConstraint)) return false;

  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    bool isGlobal;
    TR::VPConstraint *childConstraint =
        vp->getConstraint(node->getChild(i), isGlobal);
    if (!constraintFitsInIntegerRange(vp, childConstraint)) return false;
  }

  // Adding this check because it is likely suboptimal on machines where you can
  // use long registers to convert things to integers and insert conversion
  // trees. Conversion trees are not free. LoadExtensions (and other codegen
  // peepholes) should in theory catch uneeded conversions (like widening) as
  // described in the case below on 64 bit platforms.
  ...
  //                           lconst
  TR::ILOpCodes newOp =
      TR::ILOpCode::integerOpForLongOp(node->getOpCodeValue());
  if (newOp == TR::BadILOp) return false;

  if (performTransformation(vp->comp(),
                            "%sReduce %s (0x%p) to integer arithmetic\n",
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
      ...
      if (canMoveLongOpChildDirectly(node, i, arithNode)) {
        ...
        dumpOptDetails(vp->comp(), "    Transfer integer child %d %s (0x%p)\n",
                 ...
                 child->getFirstChild()->getDataType() ==
                     arithNode->getDataType()) {
        if (child->getReferenceCount() > 1) vp->anchorNode(child, vp->_curTree);
        arithNode->setAndIncChild(i, child->getFirstChild());
        dumpOptDetails(
            vp->comp(),
            "    Replacing child %d %s (0x%p) with grandchild %s (0x%p)\n", i,
            child->getOpCode().getName(), child,
            child->getFirstChild()->getOpCode().getName(),
            child->getFirstChild());
      } else {
        TR::Node *newChild = TR::Node::create(
            node,
            TR::ILOpCode::getProperConversion(child->getDataType(),
                                              arithNode->getDataType(), false),
            1);
        newChild->setAndIncChild(0, child);
        arithNode->setAndIncChild(i, newChild);
        dumpOptDetails(vp->comp(),
                       "    Creating new %s (0x%p) above child %d %s (0x%p)\n",
                       newChild->getOpCode().getName(), newChild, i,
                       child->getOpCode().getName(), child);
      }
    }

    vp->prepareToReplaceNode(
        node, TR::ILOpCode::getProperConversion(arithNode->getDataType(),
                                                node->getDataType(), false));
    node->setNumChildren(1);
    node->setAndIncChild(0, arithNode);
    dumpOptDetails(vp->comp(),
                   "  Changed (0x%p) to %s with new child %s (0x%p)\n", node,
                   node->getOpCode().getName(),
                   arithNode->getOpCode().getName(), arithNode);
    return true;
  }
...
static void constrainAConst(OMR::ValuePropagation *vp, TR::Node *node,
                            bool isGlobal) {
  TR::VPConstraint *constraint = NULL;
  if (node->getAddress() == 0) {
    ...
    if (node->isClassPointerConstant()) {
      TR::VPConstraint *typeConstraint = TR::VPClass::create(
          vp,
          TR::VPFixedClass::create(vp,
                                   (TR_OpaqueClassBlock *)node->getAddress()),
          NULL, NULL, NULL,
          TR::VPObjectLocation::create(vp,
                                       TR::VPObjectLocation::J9ClassObject));
...
static void constrainIntAndFloatConstHelper(OMR::ValuePropagation *vp,
                                            TR::Node *node, int32_t value,
                                            bool isGlobal) {
  if (value) {
    node->setIsNonZero(true);
  ...
  vp->addBlockOrGlobalConstraint(node, TR::VPIntConst::create(vp, value),
                                 isGlobal);
}

static void constrainIntConst(OMR::ValuePropagation *vp, TR::Node *node,
                              bool isGlobal) {
...
static void constrainLongConst(OMR::ValuePropagation *vp, TR::Node *node,
                               bool isGlobal) {
  ...
  vp->addBlockOrGlobalConstraint(node, TR::VPLongConst::create(vp, value),
                                 isGlobal);
}
...
// 1)    via a specific known sign tracked on the node itself (this could be for
// any type of BCD node) 2a)   for a setsign operation where the setsign value
// is on the node itself 2b)   for a setsign operation where the setsign value
// is a const child of the node
//
#ifdef J9_PROJECT_SPECIFIC
#define TR_ENABLE_BCD_SIGN (true)
TR::Node *constrainBCDSign(OMR::ValuePropagation *vp, TR::Node *node) {
  if (!TR_ENABLE_BCD_SIGN) return node;

  int32_t sign = TR::DataType::getInvalidSignCode();
  if (node->hasKnownSignCode())  // TODO: track assumed sign codes too? -- these
                                 // are tracked on load operations already so
                                 // not much benefit
  {
    ...
    if (vp->trace())
      traceMsg(vp->comp(),
               "\tconstrainBCDSign from knownSign : %s (%p) sign %s (0x%x)\n",
               node->getOpCode().getName(), node,
               TR::DataType::getName(rawSign), sign);
    ...
    if (vp->trace())
      traceMsg(
          vp->comp(),
          "\tconstrainBCDSign from setSignOnNode : %s (%p) sign %s (0x%x)\n",
          node->getOpCode().getName(), node, TR::DataType::getName(rawSign),
          sign);
  } else if (node->getOpCode().isSetSign()) {
    TR::Node *setSignValue = node->getSetSignValueNode();
    if (setSignValue->getOpCode().isLoadConst() &&
        setSignValue->getType().isIntegral() && setSignValue->getSize() <= 4) {
      sign = setSignValue->get32bitIntegralValue();
      if (vp->trace())
        traceMsg(vp->comp(),
                 "\tconstrainBCDSign from setSignOp : %s (%p) sign 0x%x\n",
  ...
  if (sign == TR::DataType::getInvalidSignCode() ||
      sign ==
          TR::DataType::getIgnoredSignCode())  // if no specific sign code see
                                               // if a clean or preferred sign
                                               // state can be tracked
  {
    ...
    if (constraintType != TR_Sign_Unknown) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "\tnode %s (%p) got clean or preferred constraintType %s\n",
                 node->getOpCode().getName(), node,
                 TR::VP_BCDSign::getName(constraintType));
      vp->addGlobalConstraint(
          node,
          TR::VP_BCDSign::create(vp, constraintType, node->getDataType()));
    }
  } else {
    TR_BCDSignCode normalizedSign =
        TR::DataType::getNormalizedSignCode(node->getDataType(), sign);
    TR_BCDSignConstraint constraintType =
        TR::VP_BCDSign::getSignConstraintFromBCDSign(normalizedSign);

    if (vp->trace())
      traceMsg(vp->comp(),
               "\tnode %s (%p) got constraintType %s for sign 0x%x\n",
               node->getOpCode().getName(), node,
    ...
    if (constraintType == TR_Sign_Minus && node->hasKnownCleanSign()) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "\tpromote constraintType %s->%s as node %s (%p) is clean\n",
                 TR::VP_BCDSign::getName(constraintType),
                 TR::VP_BCDSign::getName(TR_Sign_Minus_Clean),
                 node->getOpCode().getName(), node);
...
TR::Node *constrainBCDAggrLoad(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (findConstant(vp, node)) return node;
  ...
  if (signConstraint && TR_ENABLE_BCD_SIGN &&
      signConstraint->getDataType() == node->getDataType()) {
    switch (signConstraint->getSign()) {
      case TR_Sign_Clean:
        if (!node->hasKnownCleanSign() &&
            performTransformation(
                vp->comp(), "%sTransfer sign constraint %s to %s (0x%p)\n",
                OPT_DETAILS, signConstraint->getName(),
                node->getOpCode().getName(), node)) {
          if (vp->comp()->cg()->traceBCDCodeGen())
            traceMsg(vp->comp(),
                     "y^y: VP: Transfer sign constraint %s to %s (0x%p)\n",
                     signConstraint->getName(), node->getOpCode().getName(),
                     node);
  ...
  if (node->getOpCode().isIndirect() &&
    ...
    vp->addBlockConstraint(node->getFirstChild(),
...
TR::Node *constrainAnyIntLoad(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  // Optimize characters being loaded out of the values array of a constant
  // string
  //
  if (node->getOpCode().isIndirect() &&
    ...
    if (index->getOpCode().isLoadConst()) {
      ...
      if (dataType == TR::Int16 && array->getOpCode().isIndirect()) {
        ...
        if (baseVPConstraint && baseVPConstraint->isConstString()) {
          TR::VPConstString *constString =
              baseVPConstraint->getClassType()->asConstString();

          uintptrj_t offset = TR::Compiler->target.is64Bit()
                                  ? (uintptrj_t)index->getUnsignedLongInt()
                                  : (uintptrj_t)index->getUnsignedInt();
          uintptrj_t chIdx =
              (offset - (uintptrj_t)TR::Compiler->om
                            .contiguousArrayHeaderSizeInBytes()) /
              2;
          uint16_t ch = constString->charAt(chIdx, vp->comp());
          if (ch != 0) {
            vp->replaceByConstant(node, TR::VPShortConst::create(vp, ch),
                                  isGlobal);
            return node;
          }
        }
          ...
          if (kobj->isArrayWithConstantElements(vp->comp()) &&
            ...
            if (constrainAnyIntLoadCriticalSection.hasVMAccess()) {
              ...
              uintptrj_t offset = TR::Compiler->target.is64Bit()
                                      ? (uintptrj_t)index->getUnsignedLongInt()
                                      : (uintptrj_t)index->getUnsignedInt();
              uintptrj_t lengthInElements =
                  TR::Compiler->om.getArrayLengthInElements(vp->comp(),
                                                            arrayObj);
              TR::DataType elementType = kobj->getPrimitiveArrayDataType();
              uintptrj_t lengthInBytes =
                  lengthInElements * TR::DataType::getSize(elementType);
              uintptrj_t validStart =
              ...
              if (validStart <= offset && offset < accessEnd &&
                  accessEnd <= validEnd) {
                uintptrj_t elementAddress =
                    TR::Compiler->om.getAddressOfElement(vp->comp(), arrayObj,
                                                         offset);
                switch (dataType) {
                  case TR::Int8: {
                    uint8_t value = *((uint8_t *)(elementAddress));
                    vp->replaceByConstant(
                        node, TR::VPIntConst::create(vp, value), isGlobal);
                    return node;
                  }
                  case TR::Int16: {
                    uint16_t value = *((uint16_t *)(elementAddress));
                    vp->replaceByConstant(
                        node, TR::VPShortConst::create(vp, value), isGlobal);
                    return node;
                  }
                  case TR::Int32: {
                  ...
                  case TR::Int64: {
                    ...
                    vp->replaceByConstant(
                        node, TR::VPLongConst::create(vp, value), isGlobal);
                    return node;
                  }
                }
              }
  ...
  TR::Symbol *opSym = node->getOpCode().hasSymbolReference()
                          ? node->getSymbolReference()->getSymbol()
                          : NULL;
  if (opSym && (!opSym->isParm() || vp->isParmInvariant(opSym))) {
    ...
    if (shouldConstrain) {
      TR::VPConstraint *constraint = TR::VPIntRange::createWithPrecision(
          vp, dataType, TR_MAX_DECIMAL_PRECISION, TR_maybe);
      if (constraint) vp->addGlobalConstraint(node, constraint);
    }
  }

  if (node->isNonNegative())
    vp->addBlockConstraint(
        node, TR::VPIntRange::create(vp, 0, TR::getMaxSigned<TR::Int32>()));
...
TR::Node *constrainShortLoad(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  TR::VPConstraint *constraint =
      TR::VPShortRange::createWithPrecision(vp, TR_MAX_DECIMAL_PRECISION);
  if (constraint) vp->addGlobalConstraint(node, constraint);

  if (node->isNonNegative())
    vp->addBlockConstraint(
        node, TR::VPShortRange::create(vp, 0, TR::getMaxSigned<TR::Int16>()));
...
TR::Node *constrainIntLoad(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (wasReplacedByConstant) return node;
...
bool simplifyJ9ClassFlags(OMR::ValuePropagation *vp, TR::Node *node,
                          bool isLong) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (symRef == vp->comp()->getSymRefTab()->findClassAndDepthFlagsSymbolRef()) {
    if (node->getFirstChild()->getOpCode().hasSymbolReference() &&
      ...
      TR::VPConstraint *baseObject =
          vp->getConstraint(node->getFirstChild()->getFirstChild(), isGlobal);
      if (baseObject && baseObject->getClassType() &&
          baseObject->getClassType()->asFixedClass()) {
        cdfValue = vp->comp()->fej9()->getClassDepthAndFlagsValue(
            baseObject->getClassType()->getClass());
        // if the type is a java.lang.Object, make the control branch to the
        // call
        //
...
static void checkUnsafeArrayAccess(OMR::ValuePropagation *vp, TR::Node *node) {
  if (node->getSymbol()->isUnsafeShadowSymbol()) {
    ...
    if (node->getFirstChild()->getOpCode().isArrayRef() &&
      ...
      bool isGlobal;
      TR::VPConstraint *baseConstraint =
          vp->getConstraint(baseAddress, isGlobal);

      if (baseConstraint && baseConstraint->getClassType() &&
          baseConstraint->getClassType()->isArray()) {
        if (vp->trace()) traceMsg(vp->comp(), "is an array access\n");
        vp->addUnsafeArrayAccessNode(node->getGlobalIndex());
      } else {
        if (vp->trace()) traceMsg(vp->comp(), "is not an array access\n");
      }
    }
  }
}
...
TR::Node *constrainLload(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (node->getOpCode().isIndirect()) {
    ...
    if (containsUnsafeSymbolReference(vp, node)) return node;

    if (constrainCompileTimeLoad(vp, node)) return node;
  }

  int64_t lo, hi;
  constrainRangeByPrecision(TR::getMinSigned<TR::Int64>(),
                            TR::getMaxSigned<TR::Int64>(),
                            TR_MAX_DECIMAL_PRECISION, lo, hi);
  TR::VPConstraint *constraint = TR::VPLongRange::create(vp, lo, hi);
  if (constraint) vp->addGlobalConstraint(node, constraint);

  if (node->isNonNegative())
    vp->addBlockConstraint(
        node, TR::VPLongRange::create(vp, 0, TR::getMaxSigned<TR::Int64>()));

  checkForNonNegativeAndOverflowProperties(vp, node);

  if (vp->isHighWordZero(node)) node->setIsHighWordZero(true);
...
TR::Node *constrainFload(OMR::ValuePropagation *vp, TR::Node *node) {
  if (!findConstant(vp, node)) constrainChildren(vp, node);
...
static const char *getFieldSignature(OMR::ValuePropagation *vp, TR::Node *node,
                                     int32_t &len) {
  TR::SymbolReference *symRef = node->getSymbolReference();
  ...
  if (cookie > 0)
    return symRef->getOwningMethod(vp->comp())
        ->fieldSignatureChars(cookie, len);
  if (cookie == -1) {
 ...
 */
static bool addKnownObjectConstraints(OMR::ValuePropagation *vp,
                                      TR::Node *node) {
  TR::KnownObjectTable *knot = vp->comp()->getKnownObjectTable();
  if (!knot) return false;

  TR::SymbolReference *symRef = node->getSymbolReference();
  if (symRef->isUnresolved()) return false;

  uintptrj_t *objectReferenceLocation = NULL;
  if (symRef->hasKnownObjectIndex())
    objectReferenceLocation =
        symRef->getKnownObjectReferenceLocation(vp->comp());
  else if (symRef->getSymbol()->isFixedObjectRef())
    objectReferenceLocation = (uintptrj_t *)symRef->getSymbol()
                                  ->castToStaticSymbol()
                                  ->getStaticAddress();
  ...
  if (objectReferenceLocation) {
...

    {
      ...
      uintptrj_t objectReference =
          vp->comp()->fej9()->getStaticReferenceFieldAtAddress(
              (uintptrj_t)objectReferenceLocation);
      ...
      if (isFixedJavaLangClass) {
        // A FixedClass constraint means something different when the class
        // happens to be java/lang/Class. Must add constraints pertaining to the
        // class that the java/lang/Class object represents.
        //
        clazz = TR::Compiler->cls.classFromJavaLangClass(vp->comp(),
                                                         objectReference);
      }
    ...
    if (isString && symRef->getSymbol()->isStatic()) {
      ...
      //
      vp->addGlobalConstraint(
          node, TR::VPClass::create(vp, TR::VPConstString::create(vp, symRef),
                                    TR::VPNonNullObject::create(vp), NULL, NULL,
                                    TR::VPObjectLocation::create(
                                        vp, TR::VPObjectLocation::HeapObject)));
    } else if (jlClass)  // without a jlClass, we can't tell what kind of
                         // constraint to add
    {
      TR::VPConstraint *constraint = NULL;
      const char *classSig =
          TR::Compiler->cls.classSignature(vp->comp(), clazz, vp->trMemory());
      if (isFixedJavaLangClass) {
        if (performTransformation(vp->comp(),
                                  "%sAdd ClassObject constraint to %p based on "
                                  "known java/lang/Class %s =obj%d\n",
                                  OPT_DETAILS, node, classSig,
                                  knownObjectIndex)) {
          constraint = TR::VPClass::create(
              vp,
              TR::VPKnownObject::createForJavaLangClass(vp, knownObjectIndex),
              TR::VPNonNullObject::create(vp), NULL, NULL,
              TR::VPObjectLocation::create(
                  vp, TR::VPObjectLocation::JavaLangClassObject));
          vp->addGlobalConstraint(node, constraint);
        }
      } else {
        if (performTransformation(vp->comp(),
                                  "%sAdd known-object constraint to %p based "
                                  "on known object obj%d of class %s\n",
                                  OPT_DETAILS, node, knownObjectIndex,
                                  classSig)) {
          constraint = TR::VPClass::create(
              vp, TR::VPKnownObject::create(vp, knownObjectIndex),
              TR::VPNonNullObject::create(vp), NULL, NULL,
              TR::VPObjectLocation::create(vp,
                                           TR::VPObjectLocation::HeapObject));
...
TR::Node *constrainAload(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  // before undertaking aload specific handling see if the default load
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (symRef->getSymbol()->isLocalObject()) {
      vp->addGlobalConstraint(node, TR::VPObjectLocation::create(
                                        vp, TR::VPObjectLocation::StackObject));
    }
    ...
    if (addKnownObjectConstraints(vp, node)) return node;

    if (!symRef->getSymbol()->isArrayShadowSymbol()) {
      TR::Symbol *sym = symRef->getSymbol();
      bool haveClassLookaheadInfo = false;
      if (sym->isStatic() && !symRef->isUnresolved() &&
      ...
      if (haveClassLookaheadInfo) {
        ...
        if (sym->isStatic() && sym->isFinal()) {
          ...
          TR_OpaqueClassBlock *classOfStatic =
              symRef->getOwningMethod(vp->comp())
                  ->classOfStatic(symRef->getCPIndex());
          if (classOfStatic == NULL) {
            int len = 0;
            char *classNameOfFieldOrStatic = NULL;
            classNameOfFieldOrStatic =
                symRef->getOwningMethod(vp->comp())
                    ->classNameOfFieldOrStatic(symRef->getCPIndex(), len);
            if (classNameOfFieldOrStatic) {
              classNameOfFieldOrStatic = classNameToSignature(
                  classNameOfFieldOrStatic, len, vp->comp());
              TR_OpaqueClassBlock *curClass = vp->fe()->getClassFromSignature(
                  classNameOfFieldOrStatic, len,
                  symRef->getOwningMethod(vp->comp()));
              TR_OpaqueClassBlock *owningClass = vp->comp()
                                                     ->getJittedMethodSymbol()
                                                     ->getResolvedMethod()
                                                     ->containingClass();
              if (owningClass == curClass) classOfStatic = curClass;
            }
          }
          ...
          TR_PersistentClassInfo *classInfo =
              vp->comp()
                  ...
                  ->findClassInfoAfterLocking(classOfStatic, vp->comp(),
                                              allowForAOT);
          if (classInfo && classInfo->isInitialized())
            isClassInitialized = true;

          if (classOfStatic != vp->comp()->getSystemClassPointer() &&
              isClassInitialized && (type == TR::Address)) {
            TR::VMAccessCriticalSection constrainAloadCriticalSection(
                vp->comp(), TR::VMAccessCriticalSection::tryToAcquireVMAccess);
            if (constrainAloadCriticalSection.hasVMAccess()) {
              uintptrj_t arrayStaticAddress =
                  (uintptrj_t)symbol->getStaticAddress();
              uintptrj_t arrayObject =
                  vp->comp()->fej9()->getStaticReferenceFieldAtAddress(
                      arrayStaticAddress);
              if (arrayObject != 0) {
                int32_t arrLength = TR::Compiler->om.getArrayLengthInElements(
                    vp->comp(), arrayObject);
                int32_t len;
                ...
                if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                  ...
                  if (elementSize != 0) {
                    vp->addGlobalConstraint(node,
                                            TR::VPNonNullObject::create(vp));
                    vp->addGlobalConstraint(
                        node, TR::VPArrayInfo::create(vp, arrLength, arrLength,
                                                      elementSize));
                    vp->addGlobalConstraint(
                        node, TR::VPObjectLocation::create(
                                  vp, TR::VPObjectLocation::NotClassObject));
                    foundInfo = true;
                  }
                }
              }
        ...
        if (!foundInfo) {
          ...
          if (classInfo && classInfo->getFieldInfo()) {
            TR_PersistentFieldInfo *fieldInfo =
                classInfo->getFieldInfo()->findFieldInfo(vp->comp(), node,
                                                         false);
            if (fieldInfo) {
                  ...
                  if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                    ...
                    if (elementSize != 0) {
                      if (vp->trace())
                        traceMsg(
                            vp->comp(),
                            "Using class lookahead info to find out non null, "
                            "array dimension, and object location\n");
                      vp->addGlobalConstraint(
                          node,
                          TR::VPArrayInfo::create(vp, firstDimension,
                                                  firstDimension, elementSize));
                      vp->addGlobalConstraint(
                          node, TR::VPObjectLocation::create(
                                    vp, TR::VPObjectLocation::NotClassObject));
                    }
    ...
    if (vp->_curBlock->getExceptionClass()) {
      ...
      if (vp->_curBlock->specializedDesyncCatchBlock()) {
        ...
        for (predictedThrow = throwsIt.getFirst(); predictedThrow;
          ...
          if (node != predictedNode) continue;

          if (node) {
            // hack we're using the second child of the throw to store the
            // destination block todo: make this less hacky
            //
            node->setNumChildren(2);
            TR::Block *b = (TR::Block *)node->getSecondChild();
            node->setNumChildren(1);
            if (vp->_curBlock == b) {
              constraint = TR::VPFixedClass::create(
                  vp, vp->_curBlock->getExceptionClass());
              break;
            }
          }
        }
      ...
      if (!constraint)
        constraint =
            TR::VPResolvedClass::create(vp, vp->_curBlock->getExceptionClass());
    } else {
      len = vp->_curBlock->getExceptionClassNameLength();
      sig = classNameToSignature(vp->_curBlock->getExceptionClassNameChars(),
                                 len, vp->comp());
      constraint = TR::VPUnresolvedClass::create(
          vp, sig, len, symRef->getOwningMethod(vp->comp()));
    }
    vp->addGlobalConstraint(node, constraint);
  ...
  if (symRef && symRef->getSymbol()->isStatic()) {
    if (symRef->getSymbol()->isConstString() && !symRef->isUnresolved() &&
        vp->comp()->getStringClassPointer()) {
      constraint = TR::VPClass::create(
          vp, TR::VPConstString::create(vp, symRef),
          TR::VPNonNullObject::create(vp), NULL, NULL,
      ...
      if (sig) {
        TR_OpaqueClassBlock *classBlock = vp->fe()->getClassFromSignature(
            sig, len, symRef->getOwningMethod(vp->comp()));
        ...
        if (classBlock) {
          TR_OpaqueClassBlock *jlClass =
              vp->fe()->getClassClassPointer(classBlock);
          if (jlClass) {
            if (classBlock != jlClass) {
              constraint = TR::VPClassType::create(
                  vp, sig, len, symRef->getOwningMethod(vp->comp()), isFixed,
                  classBlock);
              if (*sig == '[' || sig[0] == 'L') {
                int32_t elementSize = arrayElementSize(sig, len, node, vp);
                if (elementSize != 0) {
                  constraint = TR::VPClass::create(
                      vp, (TR::VPClassType *)constraint, NULL, NULL,
                      TR::VPArrayInfo::create(
                          vp, 0,
                          elementSize == 0
                              ? TR::getMaxSigned<TR::Int32>()
              ...
              constraint = TR::VPObjectLocation::create(
                  vp, TR::VPObjectLocation::JavaLangClassObject);
            }
            vp->addGlobalConstraint(node, constraint);
          }
  ...
  bool isGlobal;
  constraint = vp->getConstraint(node, isGlobal);
  if (constraint && constraint->isNonNullObject()) node->setIsNonNull(true);
  return node;
}

static TR::Node *findArrayLengthNode(OMR::ValuePropagation *vp, TR::Node *node,
                                     List<TR::Node> *arraylengthNodes) {
  int32_t nodeVN = vp->getValueNumber(node);

  ListIterator<TR::Node> arraylengthNodesIt(arraylengthNodes);
  TR::Node *nextNode;
  for (nextNode = arraylengthNodesIt.getFirst(); nextNode != NULL;
       nextNode = arraylengthNodesIt.getNext()) {
    //
    // Check if the arraylength node is still in the trees and
    // still looks like an arraylength node
    //
    if (nextNode->getOpCode().isArrayLength() &&
        (nextNode->getReferenceCount() > 0)) {
...
static TR::Node *findArrayIndexNode(OMR::ValuePropagation *vp, TR::Node *node,
                                    int32_t stride) {
  ...
  if (usingAladd) {
    ...
    if ((offset->getOpCode().isAdd() &&
      ...
      if (offset2->getOpCodeValue() == TR::lmul) {
        ...
        if (mulStride->getOpCode().isLoadConst() && constValue == stride) {
...
TR::Node *constrainAddressRef(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (node->getFirstChild()->getOpCode().isLoadVar() && parent &&
    ...
    TR::Node *arraylengthNode =
        findArrayLengthNode(vp, arrayLoad, vp->getArraylengthNodes());
    TR::Node *indexNode = NULL;
    if (arraylengthNode)
      indexNode =
          findArrayIndexNode(vp, node, arraylengthNode->getArrayStride());
...
TR::Node *constrainIiload(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (base && base->isConstString()) {
    ...
    if (success) {
      int32_t realKonst;
      realKonst = *konst;
      if (!base->isNonNullObject() &&
          vp->getCurrentParent()->getOpCodeValue() == TR::NULLCHK) {
        // replace node with a PassThrough in the original tree
        TR::Node *passThroughNode =
            TR::Node::create(TR::PassThrough, 1, node->getFirstChild());
        ...
        TR::TreeTop *newTree = TR::TreeTop::create(
            vp->comp(), TR::Node::create(TR::treetop, 1, node));
  ...
  if (field == TR::Symbol::Java_io_ByteArrayOutputStream_count) {
    ...
    vp->addGlobalConstraint(
        node,
        TR::VPIntRange::create(vp, 0, TR::getMaxSigned<TR::Int32>() >> 1));
    node->setIsNonNegative(true);
    node->setCannotOverflow(true);
  } else
#endif
  ...
  if (simplifyJ9ClassFlags(vp, node, false)) return node;
...
TR::Node *constrainIaload(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  //
  vp->addGlobalConstraint(node, TR::VPObjectLocation::create(
                                    vp, TR::VPObjectLocation::NotStackObject));
  ...
  if (node->getOpCode().hasSymbolReference()) {
    TR::Node *transformedNode =
        TR::TransformUtil::transformIndirectLoad(vp->comp(), node);
    if (transformedNode) {
    ...
    const static char *needInitializedCheck =
        feGetEnv("TR_needInitializedCheck");
    TR::VPConstraint *base = vp->getConstraint(node->getFirstChild(), isGlobal);
    ...
    if (base && node->getOpCode().hasSymbolReference() &&
      ...
      if (clazz != NULL) type = TR::VPFixedClass::create(vp, clazz);
      ...
      vp->addBlockOrGlobalConstraint(
          node, TR::VPClass::create(vp, type, nonnull, NULL, NULL, loc),
          isGlobal);

      node->setIsNonNull(true);
    } else if (base && base->getKnownObject() && !symRef->isUnresolved() &&
               symRef->getSymbol()->isRecognizedKnownObjectShadow() &&
    ...
    // hence can be treated as known object.
    {
      ...
      if (constrainIaloadCriticalSection.hasVMAccess()) {
        ...
        uintptrj_t baseObject =
            knot->getPointer(base->getKnownObject()->getIndex());
        if (baseObject) {
          // Check if baseObject is actually the recognized field Class
          TR_OpaqueClassBlock *baseObjectClazz =
              TR::Compiler->cls.objectClass(vp->comp(), baseObject);
          int32_t recognizedClassNameLength;
          const char *recognizedClassName =
              symRef->getSymbol()->owningClassNameCharsForRecognizedField(
                  recognizedClassNameLength);
          if (recognizedClassName) {
            TR_OpaqueClassBlock *recognizedClazz =
                vp->fe()->getClassFromSignature(
                    recognizedClassName, recognizedClassNameLength,
                    symRef->getOwningMethod(vp->comp()));
            if (recognizedClazz &&
                (TR_yes == vp->fe()->isInstanceOf(baseObjectClazz,
                                                  recognizedClazz, false))) {
              uintptrj_t fieldObject =
                  vp->comp()->fej9()->getReferenceFieldAtAddress(
              ...
              if (0 != fieldObject) {
                ...
                if (!TR::Compiler->cls.isClassArray(vp->comp(),
                                                    fieldObjectClazz)) {
                  traceMsg(vp->comp(),
                           "Recognized known object field node %p \n", node);
                  TR::KnownObjectTable::Index fieldObjectKnotIndex =
                      knot->getIndex(fieldObject);
                  // Global constraints should work here, as field loads get
                  // fresh value numbers
                  constraint = TR::VPClass::create(
                      vp, TR::VPKnownObject::create(vp, fieldObjectKnotIndex),
                      TR::VPNonNullObject::create(vp), NULL, NULL,
                      TR::VPObjectLocation::create(
                          vp, TR::VPObjectLocation::HeapObject));
                  vp->addGlobalConstraint(node, constraint);
                  ...
                  // fresh value numbers
                  vp->addGlobalConstraint(
                      node, TR::VPFixedClass::create(vp, fieldObjectClazz));
                  vp->addGlobalConstraint(node,
                                          TR::VPNonNullObject::create(vp));
                  vp->addGlobalConstraint(
                      node, TR::VPArrayInfo::create(
                                vp, arrLength, arrLength,
                                TR::Compiler->om.getArrayElementWidthInBytes(
                                    vp->comp(), fieldObject)));
                }
              }
            }
    ...
    } else if (base && base->isJavaLangClassObject() == TR_yes &&
               base->isNonNullObject() && base->getKnownObject()) {
      if (symRef ==
          vp->comp()->getSymRefTab()->findClassFromJavaLangClassSymbolRef()) {
        TR::KnownObjectTable *knot = vp->comp()->getOrCreateKnownObjectTable();
        TR_ASSERT(
            knot,
            "Can not have a TR::VPKnownObject without a known-object table");

        {
          ...
          if (constrainIaloadCriticalSection.hasVMAccess()) {
            ...
            TR_OpaqueClassBlock *clazz =
                TR::Compiler->cls.classFromJavaLangClass(vp->comp(), jlclazz);
            if (TR::Compiler->cls.isClassInitialized(vp->comp(), clazz)) {
              // this should be able to be precise because we do know the
              // compenent class pointer precisely but clearly others make
              // invalid assumptions on seeing this fixed class constraint so be
              // conservative and don't let the world know if we aren't sure the
              // elements match this type
              if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), clazz) &&
                ...
                vp->addBlockOrGlobalConstraint(
                    node,
                    TR::VPClass::create(
                        vp, TR::VPFixedClass::create(vp, clazz),
                        TR::VPNonNullObject::create(vp), NULL, NULL,
                        TR::VPObjectLocation::create(
                            vp, TR::VPObjectLocation::J9ClassObject)),
                    isGlobal);
                ...
                vp->addBlockOrGlobalConstraint(
                    node,
                    TR::VPClass::create(
                        vp, TR::VPResolvedClass::create(vp, clazz),
                        TR::VPNonNullObject::create(vp), NULL, NULL,
                        TR::VPObjectLocation::create(
                            vp, TR::VPObjectLocation::J9ClassObject)),
                    isGlobal);
    ...
    } else if (base && base->isJ9ClassObject() == TR_yes &&
               base->isNonNullObject() && base->getClassType() &&
               base->getClassType()->asFixedClass() && base->getClass() &&
               (!needInitializedCheck || TR::Compiler->cls.isClassInitialized(
                                             vp->comp(), base->getClass()))) {
      TR::KnownObjectTable *knot = vp->comp()->getOrCreateKnownObjectTable();
      if (knot && symRef == vp->comp()
                                ->getSymRefTab()
                                ->findJavaLangClassFromClassSymbolRef()) {
        TR_J9VMBase *fej9 = (TR_J9VMBase *)(vp->comp()->fe());
        TR::KnownObjectTable::Index knownObjectIndex = knot->getIndexAt(
            (uintptrj_t *)(base->getClass() +
        ...
        vp->addBlockOrGlobalConstraint(
            ...
            isGlobal);
        node->setIsNonNull(true);
      } else if (symRef == vp->comp()
                               ->getSymRefTab()
                               ->findArrayComponentTypeSymbolRef() &&
                 base->getClassType()->isArray() == TR_yes) {
        attemptCompileTimeLoad = true;
        TR_OpaqueClassBlock *componentClass =
            vp->comp()->fej9()->getComponentClassFromArrayClass(
                base->getClass());
        // this should be able to be precise because we do know the compenent
        // class pointer precisely but clearly others make invalid assumptions
        // on seeing this fixed class constraint so be conservative and don't
        // let the world know if we aren't sure the elements match this type
        if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), componentClass) &&
          ...
          vp->addBlockOrGlobalConstraint(
              node,
              TR::VPClass::create(
                  vp, TR::VPResolvedClass::create(vp, componentClass),
                  TR::VPNonNullObject::create(vp), NULL, NULL,
                  TR::VPObjectLocation::create(
                      vp, TR::VPObjectLocation::J9ClassObject)),
              isGlobal);
      ...
      node->setIsNonNull(true);
                             ...
                             ->getSymRefTab()
                             ->findClassFromJavaLangClassSymbolRef()) {
      vp->addGlobalConstraint(node,
                              TR::VPObjectLocation::create(
                                  vp, TR::VPObjectLocation::J9ClassObject));
      vp->addGlobalConstraint(node, TR::VPNonNullObject::create(vp));
    ...
    switch (sym->getRecognizedField()) {
      ...
      case TR::Symbol::Java_lang_invoke_MethodHandle_thunks:
        if (!node->isNonNull() &&
            performTransformation(
                vp->comp(), "%s[%p] recognized field is never null: %s\n",
      ...
      case TR::Symbol::Java_lang_invoke_MutableCallSite_target: {
        if (!vp->comp()->getOption(TR_DisableMCSBypass) &&
            vp->comp()->getMethodHotness() >= hot) {
          TR::VPConstraint *callSiteConstraint =
              vp->getConstraint(node->getFirstChild(), isGlobal);
          if (callSiteConstraint && callSiteConstraint->getKnownObject()) {
            ...
            if (bypassOnThisRun) {
              TR::KnownObjectTable::Index callSiteKOI =
                  callSiteConstraint->getKnownObject()->getIndex();
...

              {
                ...
                if (constrainIaloadCriticalSection.hasVMAccess()) {
                  uintptrj_t *siteLocation =
                      vp->comp()->getKnownObjectTable()->getPointerLocation(
                          callSiteKOI);
                  bypassLocation =
                      vp->comp()->fej9()->mutableCallSite_bypassLocation(
                          *siteLocation);
                  if (!bypassLocation &&
                      performTransformation(
                          vp->comp(),
                    ...
                    bypassLocation =
                        vp->comp()
                            ->fej9()
                            ->mutableCallSite_findOrCreateBypassLocation(
                                *siteLocation);
                  }
                } else {
                  if (vp->trace())
                    traceMsg(vp->comp(),
                             "[%p] Unable to acquire VM access.  Not "
                             "attempting to bypass CallSite\n",
                             node);
                }
              }
              ...
              if (bypassLocation &&
                  performTransformation(
                      vp->comp(), "%s[%p] bypass CallSite load from obj%d\n",
                      OPT_DETAILS, node, callSiteKOI)) {
                TR::VPConstraint *targetConstraint =
                    vp->getConstraint(node, isGlobal);
                TR::KnownObjectTable::Index targetKOI =
                ...
                node->setSymbolReference(
                    vp->comp()
                        ->getSymRefTab()
                        ->createKnownStaticReferenceSymbolRef(bypassLocation,
                                                              targetKOI));
                return node;
              }
            }
    ...
    if (!symRef->getSymbol()->isArrayShadowSymbol()) {
      TR::Symbol *sym = symRef->getSymbol();
      bool haveClassLookaheadInfo = false;
      if (((sym->isShadow() && node->getFirstChild()->isThisPointer()) ||
           sym->isStatic()) &&
        ...
        if (sym->isStatic() && sym->isFinal()) {
          ...
          if ((classOfStatic != vp->comp()->getSystemClassPointer() &&
            ...
            if (constrainIaloadCriticalSection.hasVMAccess()) {
              ...
              if (arrayObject != 0) {
                /*
                 * This line is left commented because of the commented lines
                 * further below which make use of the arrLength variable. This
                 * can be removed once it is determined that the commented out
                 * lines below will never be needed.
                 */
                // int32_t arrLength =
                // TR::Compiler->om.getArrayLengthInElements(vp->comp(),
                // arrayObject);
                ...
                if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                  ...
                  if (elementSize != 0) {
                    // vp->addGlobalConstraint(node,
                    // TR::VPNonNullObject::create(vp));
                    // vp->addGlobalConstraint(node, TR::VPArrayInfo::create(vp,
                    // arrLength, arrLength, elementSize));
                    foundInfo = true;
                    isFixed = true;
                  }
                }
              ...
              if (arrayFieldInfo && arrayFieldInfo->isDimensionInfoValid()) {
                ...
                if (arrLength >= 0 && sig && (len > 0) &&
                  ...
                  if (elementSize != 0) {
                    if (vp->trace())
                      traceMsg(vp->comp(),
                               "Using class lookahead info to find out non "
                               "null, array dimension\n");
                    // vp->addGlobalConstraint(node,
                    // TR::VPNonNullObject::create(vp));
                    // vp->addGlobalConstraint(node, TR::VPArrayInfo::create(vp,
                    // arrLength, arrLength, elementSize));
                ...
                if (node && node->getOpCode().hasSymbolReference() &&
                  ...
                  fieldName =
                      node->getSymbolReference()
                          ...
                          ->fieldName(node->getSymbolReference()->getCPIndex(),
                                      fieldNameLen, vp->comp()->trMemory());
                }

                if (fieldName && (fieldNameLen > 0) &&
                    !strncmp(fieldName, "java/util/TreeMap.valuesCollection",
                             34)) {
                  TR_OpaqueClassBlock *clazz = vp->fe()->getClassFromSignature(
                      "Ljava/util/TreeMap$2;", 21, method);
                  if (clazz != NULL) {
  ...
  //
  if (sig && !(node->getOpCode().hasSymbolReference() &&
               node->getSymbol()->isArrayShadowSymbol() &&
               node->getFirstChild()->getOpCode().isArrayRef())) {
    TR_ResolvedMethod *method =
        node->getSymbolReference()->getOwningMethod(vp->comp());
    TR_OpaqueClassBlock *classBlock =
        vp->fe()->getClassFromSignature(sig, len, method);
    ...
    if (classBlock) {
      ...
      if (jlClass) {
        if (classBlock != jlClass) {
          constraint = TR::VPClassType::create(vp, sig, len, method, isFixed,
                                               classBlock);
  ...
  if (foundInfo) {
    vp->addGlobalConstraint(
        node, TR::VPArrayInfo::create(vp, arrLength, arrLength, elementSize));
  }
  ...
  if (node->getOpCode().hasSymbolReference() &&
    ...
    if (vp->comp()->generateArraylets() && underlyingArray &&
      ...
      if (arraySig && *arraySig == '[') {
        ...
        if (vp->comp()->getMethodHotness() >= scorching) {
          int32_t prefetchOffset =
              vp->comp()->fej9()->findFirstHotFieldTenuredClassOffset(
                  vp->comp(), clazz);

          if (prefetchOffset >= 0) {
            if (vp->comp()->findPrefetchInfo(node) < 0) {
              TR_Pair<TR::Node, uint32_t> *prefetchInfo =
                  new (vp->comp()->trHeapMemory()) TR_Pair<TR::Node, uint32_t>(
                      node, (uint32_t *)(intptr_t)prefetchOffset);
              vp->comp()->getNodesThatShouldPrefetchOffset().push_front(
                  prefetchInfo);
            }
          }
        }
        ...
        /*
        We removed the check "!TR::Compiler->cls.isInterfaceClass(vp->comp(),
        clazz)" since java treats classes and interfaces in the same way if they
        are array elements. For example, the following two snippets are both
        rejected by javac but accepted by a bytecode verifier. Both throw an
        ...
        this line staticField = new Object(); won't produce any exceptions if
        staticField is Serializable but it will be rejected by the bytecode
        verifier if staticField is of a class type.
        */
        if (clazz) {
          TR_OpaqueClassBlock *jlClass =
              vp->comp()->fej9()->getClassClassPointer(clazz);
          if (jlClass) {
            if (clazz != jlClass) {
              int32_t fieldNameLen = -1;
              char *fieldName = NULL;
              if (underlyingArray &&
                  underlyingArray->getOpCode().hasSymbolReference() &&
                  (underlyingArray->getSymbolReference()
                       ->getSymbol()
                       ->isStaticField() ||
                   underlyingArray->getSymbolReference()
                       ->getSymbol()
                       ->isShadow())) {
                if (underlyingArray->getSymbolReference()
                        ->getSymbol()
                        ->isShadow())
                  fieldName =
                      underlyingArray->getSymbolReference()
                  ...
                  fieldName =
                      underlyingArray->getSymbolReference()
                          ->getOwningMethod(vp->comp())
                          ->staticName(underlyingArray->getSymbolReference()
                                           ->getCPIndex(),
                                       fieldNameLen, vp->comp()->trMemory());
              }
  ...
  if (base && base->getClass() &&
      !vp->comp()->getOption(TR_DisableMarkingOfHotFields) &&
      node->getFirstChild()->getOpCode().hasSymbolReference() &&
      node->getFirstChild()->getSymbol()->isCollectedReference() &&
      vp->_curBlock->getGlobalNormalizedFrequency(vp->comp()->getFlowGraph()) >=
          TR::Options::_hotFieldThreshold) {
    vp->comp()->fej9()->markHotField(vp->comp(), node->getSymbolReference(),
                                     base->getClass(), base->isFixedClass());
  }

  if (node->getSymbolReference()) {
    if (!node->getSymbolReference()->isUnresolved() &&
        (node->getSymbolReference()->getSymbol()->getKind() ==
         TR::Symbol::IsShadow) &&
      ...
      const char *fieldSig =
          node->getSymbolReference()
              ->getOwningMethod(vp->comp())
              ->fieldSignatureChars(node->getSymbolReference()->getCPIndex(),
                                    fieldSigLen);
      int32_t fieldNameLen;
      const char *fieldName =
          node->getSymbolReference()
              ->getOwningMethod(vp->comp())
              ->fieldNameChars(node->getSymbolReference()->getCPIndex(),
                               fieldNameLen);
      char *className = node->getSymbolReference()
                            ->getOwningMethod(vp->comp())
                            ->classNameChars();
      uint16_t classNameLen = node->getSymbolReference()
                                  ->getOwningMethod(vp->comp())
                                  ->classNameLength();
      if (fieldName && !strncmp(fieldName, "this$0", 6) && className &&
          !strncmp(className, "java/util/", 10)) {
        if (vp->trace())
          traceMsg(
              vp->comp(),
              "NonNull node %d className %.*s fieldSig %.*s fieldName %.*s\n",
              node->getGlobalIndex(), classNameLen, className, fieldSigLen,
              fieldSig, fieldNameLen, fieldName);
        vp->addBlockConstraint(node, TR::VPNonNullObject::create(vp));
      }
    }
  }
...
TR::Node *constrainStore(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (!node->getSymbol()->isAutoOrParm()) {
    ...
    if (node->getOpCode().isStore()) {
      if (node->getSymbolReference() ==
          vp->comp()->getSymRefTab()->findThisRangeExtensionSymRef())
        globalStore = false;
    }
  }

  if (globalStore) {
    OMR::ValuePropagation::Relationship *syncRel =
        vp->findConstraint(vp->_syncValueNumber);
    TR::VPSync *sync = NULL;
    if (syncRel && syncRel->constraint) sync = syncRel->constraint->asVPSync();
    if (sync && sync->syncEmitted() == TR_yes) {
      vp->addConstraintToList(
          NULL, vp->_syncValueNumber, vp->AbsoluteConstraint,
...
#ifdef J9_PROJECT_SPECIFIC
  if (valueChild->getType()
          .isBCD())  // sign state constraints are currently only propagated to
                     // loads so only constrain on stores vs every BCD node
...
TR::Node *constrainIntStore(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (child->getOpCodeValue() == TR::ixor &&
    ...
    if (loadNode->getOpCode().isLoadVarDirect() &&
        loadNode->getSymbol() == storeSymbol) {
      ...
      for (bni = vp->_booleanNegationInfo.getFirst(); bni;
           bni = bni->getNext()) {
        if (bni->_storeValueNumber == loadVN) {
          ...
          if (performTransformation(
            ...
            node->setNumChildren(1);
            vp->addBlockConstraint(node, TR::VPEqual::create(vp, 0),
                                   bni->_loadNode);
            return node;
          }
        }
      ...
      //
      bni =
          new (vp->trStackMemory()) OMR::ValuePropagation::BooleanNegationInfo;
  ...
  if (constraint) {
    if (constraint->isNullObject() &&
      ...
      if (node->getOpCode().isIndirect()) {
        if (performTransformation(
                vp->comp(),
                "%sChanging write barrier store into iastore [%p]\n",
                OPT_DETAILS, node)) {
          bool invalidateInfo = false;
          if (node->getChild(2) != node->getFirstChild()) invalidateInfo = true;
...
TR::Node *constrainWrtBar(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (!vp->getCurrentParent() ||
      (vp->getCurrentParent() &&
       vp->getCurrentParent()->getOpCodeValue() != TR::ArrayStoreCHK))
  ...
  if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none) {
    ...
    //     x.a = new A();
    //
    // Most of the time both newly allocated objects would be in the nursery and
    // there is no need for a generational barrier. However, in a rare case that
    // a GC occurs in between the two allocations above the premise can be
    // violated (by tenuring the first, but not the second object). The GC will
    // compensate for the lack of the barrier by auto-remembering all objects
    // that are tenured and directly stack referenced.
    //
    // This optimization is currently disabled when read barriers are required
    // as it is currently unknown whether this optimization is safe under such a
    // GC mode.
    doOpt = false;
  }
  ...
  if (doOpt &&
    ...
    if ((valueChild->getOpCodeValue() == TR::New) ||
      ...
      if (valueChild->isThisPointer() && vp->comp()
                                             ->getJittedMethodSymbol()
                                             ->getResolvedMethod()
                                             ->isConstructor()) {
        removeWrtBar = true;
        ...
        if (vp->_changedThis == TR_maybe) {
          ...
          for (; tt; tt = tt->getNextTreeTop()) {
            ...
            if (storeNode && (storeNode->getSymbolReference() ==
                              valueChild->getSymbolReference())) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid();
              ...
              if (defNode && (defNode->getOpCodeValue() == TR::astore) &&
                ...
                /*
                 ...
                 defChild->getSymbolReference(); if (symRef)
                       {
                       TR::Symbol *sym = symRef->getSymbol();

                       if (!(sym->getKind() == TR::Symbol::IsStatic &&
                 sym->isConstString()))
                           {
                           removeWrtBar = false;
                           break;
    ...
    if (removeWrtBar &&
        performTransformation(
            vp->comp(),
            "%sChanging wrtbar to store because the rhs is a new object [%p]\n",
            OPT_DETAILS, node)) {
      node->setSkipWrtBar(true);
    }
  }

  // If node is still a write barrier then let's find if it's coming from a new
  // in the same method. If that's the case then the chances are this wrtbar is
  // going to be on a new object and we should let the codegen know so it can
        ...
        if (useDefInfo->isUseIndex(useIndex)) {
          ...
          if (useDefInfo->getUseDef(defs, useIndex)) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid();
              ...
              if (defIndex < useDefInfo->getFirstRealDefIndex()) continue;
              ...
              if (defNode && (defNode->getOpCodeValue() == TR::astore) &&
                ...
                if (defChild && (defChild->getOpCodeValue() == TR::loadaddr) &&
                  ...
                  break;
                           ...
                           (defChild->getOpCodeValue() == TR::aload) &&
                           defChild->getSymbolReference()
                               ->getSymbol()
                               ->isAuto()) {
            ...
            if (setStackWrtBar &&
                performTransformation(
                    vp->comp(),
                    "%sSetting wrtbar flag to assume stack object [%p]\n",
                    OPT_DETAILS, node)) {
  ...
  // If the node is still a write barrier - we can mark it to specify the
  // location of the dest object
  //
  if (node->getOpCode().isWrtBar()) {
    TR::Node *destinationChild =
        node->getOpCode().isIndirect() ? node->getChild(2) : node->getChild(1);
    bool isGlobal;
    TR::VPConstraint *constraint = vp->getConstraint(node, isGlobal);
    if (constraint) {
      if (constraint->isHeapObject() == TR_yes &&
          performTransformation(
              vp->comp(),
              "%sMarking the wrtbar node [%p] - destination is a heap object\n",
              OPT_DETAILS, node)) {
        ...
        node->setIsHeapObjectWrtBar(true);
                 ...
                 performTransformation(vp->comp(),
                                       "%sMarking the wrtbar node [%p] - "
                                       "destination is a non-heap object\n",
                                       OPT_DETAILS, node)) {
...
TR::Node *constrainGoto(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (vp->trace())
    traceMsg(vp->comp(),
             "   unconditional branch on node %s (%p), vp->_curBlock block_%d "
             "target block_%d\n",
             node->getOpCode().getName(), node, vp->_curBlock->getNumber(),
             target->getNumber());
...
TR::Node *constrainIgoto(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  bool canFallThrough =
      vp->_curBlock->getNextBlock() &&
      vp->_curBlock->getNextBlock()->isExtensionOfPreviousBlock();

  for (auto edge = vp->_curBlock->getSuccessors().begin();
       edge != vp->_curBlock->getSuccessors().end(); ++edge) {
    auto current = edge;
    bool keepConstraints =
        (++current != vp->_curBlock->getSuccessors().end()) || canFallThrough;
    vp->printEdgeConstraints(vp->createEdgeConstraints(*edge, keepConstraints));
  }

  if (!canFallThrough) vp->setUnreachablePath();
  return node;
}
...
TR::Node *constrainReturn(OMR::ValuePropagation *vp, TR::Node *node) {
  if (node->getDataType() == TR::Address)
    vp->addGlobalConstraint(
        node,
        TR::VPObjectLocation::create(vp, TR::VPObjectLocation::NotStackObject));
...
TR::Node *constrainMonent(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (constraint && constraint->getClass()) {
    ...
    if (node->hasMonitorClassInNode() && clazz &&
        (node->getMonitorClassInNode() != clazz)) {
      TR_YesNoMaybe answer = vp->fe()->isInstanceOf(
          clazz, node->getMonitorClassInNode(), true, true);
      if (answer != TR_yes) clazz = node->getMonitorClassInNode();
    }

    if ((clazz || !node->hasMonitorClassInNode()) &&
        (performTransformation(vp->comp(),
                               "%sSetting type on MONENTER node [%p] to [%p]\n",
...
TR::Node *constrainMonexit(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  //
  vp->createExceptionEdgeConstraints(TR::Block::CanCatchMonitorExit, NULL,
                                     node);
  ...
  if (constraint && constraint->getClass()) {
    ...
    if ((clazz || !node->hasMonitorClassInNode()) &&
        performTransformation(vp->comp(),
                              "%sSetting type on MONEXIT  node [%p] to [%p]\n",
                              OPT_DETAILS, node, clazz))
      node->setMonitorClassInNode(clazz);
  }
...
TR::Node *constrainThrow(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (node->throwInsertedByOSR()) {
    // Propagate constraints so far to the exception edges
    //
    vp->createExceptionEdgeConstraints(TR::Block::CanCatchUserThrows, NULL,
                                       node);
    vp->setUnreachablePath();
    return node;
  }
  ...
  TR_OrderedExceptionHandlerIterator oehi(
      vp->_curBlock, vp->comp()->trMemory()->currentStackRegion());
  for (TR::Block *catchBlock = oehi.getFirst(); catchBlock;
       catchBlock = oehi.getNext()) {
    if (catchBlock->isOSRCatchBlock()) continue;

    TR_YesNoMaybe willCatch = TR_maybe;
    if (catchBlock->getCatchType() == 0) {
      willCatch = TR_yes;
      if (debug("traceThrowToGoto")) printf("\n\n catch type yes ...\n");
    } else if (!constraint || !constraint->getClass()) {
      if (debug("traceThrowToGoto")) printf("\n\n thrown type unknown\n");
    } else if (catchBlock->getExceptionClass()) {
      willCatch = vp->fe()->isInstanceOf(constraint->getClass(),
                                         catchBlock->getExceptionClass(),
                                         constraint->isFixedClass());
      if (willCatch == TR_yes) vp->registerPreXClass(constraint);

      if (debug("traceThrowToGoto"))
        printf("\n\n catch type %s %.*s\n",
               (willCatch == TR_yes ? "yes"
                                    : (willCatch == TR_no ? "no" : "maybe")),
               catchBlock->getExceptionClassNameLength(),
               catchBlock->getExceptionClassNameChars());
    ...
    if (willCatch != TR_no && !debug("aggressiveThrowToGoto")) break;
  }

  if (debug("traceThrowToGoto")) fflush(stdout);
  ...
  if (predictedCatchBlock && !vp->comp()->getOption(TR_DisableThrowToGoto)) {
    ...
    TR_Pair<TR::Node, TR::Block> *newPredictedThrow = new (vp->trStackMemory())
        TR_Pair<TR::Node, TR::Block>(node, vp->_curBlock);
...
TR::Node *constrainInstanceOf(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  TR::VPConstraint *constraint = NULL;
  TR::VPConstraint *objectConstraint =
      vp->getConstraint(node->getFirstChild(), lhsGlobal);
  TR::VPConstraint *castConstraint =
      vp->getConstraint(node->getSecondChild(), rhsGlobal);
  lhsGlobal &= rhsGlobal;
    ...
    if (objectConstraint->isNullObject()) {
      result = 0;
               ...
               objectConstraint->getClassType() ==
                   castConstraint->getClassType() &&
               objectConstraint->isNonNullObject() &&
               objectConstraint->isClassObject() != TR_yes) {
      result = 1;
    } else if (objectConstraint->getClass() && castConstraint &&
               castConstraint->getClass()) {
      ...
      TR_YesNoMaybe isInstance = vp->fe()->isInstanceOf(
          objectClass, castClass, objectConstraint->isFixedClass(),
      ...
      if (castConstraint) {
        if (objectConstraint->asClass() && castConstraint->asClass()) {
          ...
          TR_YesNoMaybe castIsClassObject =
              vp->isCastClassObject(castConstraint->getClassType());
          ...
          //
            ...
            if (objectConstraint->isNonNullObject()) {
              ...
              if (vp->trace())
                traceMsg(vp->comp(),
                         "object is a non-null classobject and cast is "
                         "java/lang/Class\n");
            } else {
              TR::Node::recreate(node, TR::acmpne);
              vp->removeNode(node->getChild(1), true);
            ...
            TR::VPConstraint *intersectConstraint =
                castConstraint->getClassType();
            if (intersectConstraint) {
              if (intersectConstraint->asFixedClass())
                intersectConstraint = TR::VPResolvedClass::create(
                    vp, intersectConstraint->getClass());
...
TR::Node *constrainCheckcast(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  TR::VPConstraint *objectConstraint =
      vp->getConstraint(node->getFirstChild(), isGlobal);
  TR::VPConstraint *castConstraint =
      vp->getConstraint(node->getSecondChild(), isGlobal);
      ...
      if (isInstance == TR_yes) {
        if (castConstraint->isFixedClass()) {
          ...
          if (objectConstraint->isClassObject() != TR_yes) result = 1;
                 ...
                 (objectConstraint->isNonNullObject() ||
                  TR::Compiler->cls.isPrimitiveClass(
                      vp->comp(), objectConstraint->getClass()))) {
        vp->registerPreXClass(objectConstraint);
        ...
        if (objectConstraint->asClass() && castConstraint->asClass()) {
          ...
          if (objectConstraint->asClassType() && castConstraint->asClass() &&
            ...
            if (objectConstraint->isClassObject() == TR_yes)
              intersectConstraint = TR::VPClass::create(
                  vp, dilutedType, NULL, NULL, NULL,
                  TR::VPObjectLocation::create(
                      vp, TR::VPObjectLocation::ClassObject));
            else
  ...
  if (result == 1 &&
      ...
      performTransformation(vp->comp(),
                            "%sRemoving redundant checkcast node [%p]\n",
                            OPT_DETAILS, node)) {
  ...
  if (result != 1)
    vp->createExceptionEdgeConstraints(TR::Block::CanCatchCheckCast, NULL,
                                       node);
      ...
      else  ////if (isInstance == TR_yes)
      {
          ...
          if (jlClass) {
            ...
            if (classType->getClass() == jlClass) {
              ...
              if ((classType->isClassObject() == TR_maybe) &&
                ...
                newClassType = TR::VPResolvedClass::create(
                    vp, classType->getClass());  // Do not want to get the
                                                 // 'fixed' property
            }
          }
        } else {
        ...
        if (applyType) {
          ...
          if ((objectConstraint &&
               (objectConstraint->isClassObject() == TR_yes)) ||
              castIsClassObject)
            newTypeConstraint = TR::VPClass::create(
                vp, newClassType, NULL, NULL, NULL,
                TR::VPObjectLocation::create(
                    vp, TR::VPObjectLocation::JavaLangClassObject));
  ...
  if (vp->cg()->getSupportsTenuredObjectAlignment() &&
    ...
    TR_ASSERT(node->getNumChildren() == 2,
              "checkcast should have only 2 children");
    TR::Node *valueChild = node->getFirstChild();
          ...
          if (useDefInfo->getUseDef(defs, useIndex)) {
            ...
            for (cursor.SetToFirstOne(); cursor.Valid();
              ...
              if (defNode && (defNode->getOpCodeValue() == TR::astore)) {
                ...
                char buf[512];
                const char *methodSig = vp->fe()->sampleSignature(
                    ownMethod, buf, 512, vp->trMemory());
                if (methodSig && !strncmp(methodSig,
                                          "java/util/HashMap.get(Ljava/lang/"
    ...
    if (definitionNode) {
      ...
      if (classSymRef && !classSymRef->isUnresolved()) {
        TR::StaticSymbol *classSym =
            classSymRef->getSymbol()->getStaticSymbol();
        if (classSym) {
          uint32_t size = TR::Compiler->cls.classInstanceSize(
              (TR_OpaqueClassBlock *)classSym->getStaticAddress());
          if (vp->cg()->isObjectOfSizeWorthAligning(size)) {
            vp->comp()->fej9()->markClassForTenuredAlignment(
                vp->comp(), (TR_OpaqueClassBlock *)classSym->getStaticAddress(),
                0);
          }
        }
      }
...
TR::Node *constrainNew(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (classConstraint) {
    ...
    if (classConstraint->getClass() && !classConstraint->isFixedClass())
      vp->addGlobalConstraint(
          node, TR::VPFixedClass::create(vp, classConstraint->getClass()));
    else if (classConstraint->asClass() &&
             classConstraint->asClass()->getClassType() &&
             !(classConstraint->asClass()->getClassType()->isClassObject() ==
               TR_yes))
...
TR::Node *constrainNewArray(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  int64_t maxSize =
      TR::Compiler->om.maxArraySizeInElementsForAllocation(node, vp->comp());
  if (sizeConstraint && (sizeConstraint->getHighInt() < 0 ||
    ...
    dumpOptDetails(vp->comp(),
                   "size node has no known constraint for newarray %p\n",
                   sizeNode);
    // node->setAllocationCanBeRemoved(false)
  } else
    node->setAllocationCanBeRemoved(true);
  ...
  //
  // TODO - should be sizeConstraint = vp->addBlockConstraint(sizeNode,
  // TR::VPIntRange::create(vp, 0, maxSize));
  ...
  if (clazz) vp->addGlobalConstraint(node, TR::VPFixedClass::create(vp, clazz));
  vp->addGlobalConstraint(node, TR::VPNonNullObject::create(vp));
  if (sizeConstraint)
    vp->addGlobalConstraint(
        node,
        TR::VPArrayInfo::create(vp, sizeConstraint->getLowInt(),
                                sizeConstraint->getHighInt(), elementSize));
  else
    vp->addGlobalConstraint(
        node, TR::VPArrayInfo::create(vp, 0, TR::getMaxSigned<TR::Int32>(),
                                      elementSize));
  ...
  if (constraint && constraint->getClassType() &&
    ...
    if (TR::Compiler->cls.isPrimitiveClass(vp->comp(), elementType)) {
      TR::Node::recreateWithoutProperties(
          node, TR::newarray, node->getNumChildren(),
          vp->comp()->getSymRefTab()->findOrCreateNewArraySymbolRef(
              typeNode->getSymbolReference()->getOwningMethodSymbol(
                  vp->comp())));
      TR::Node *typeConst = TR::Node::create(
          TR::iconst, 0,
          vp->comp()->fe()->getNewArrayTypeFromClass(constraint->getClass()));
      vp->_curTree->insertBefore(OMR::TreeTop::create(
          vp->comp(), TR::Node::create(TR::treetop, 1, typeNode)));
...
TR::Node *constrainANewArray(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (sizeConstraint && (sizeConstraint->getHighInt() < 0 ||
    ...
    dumpOptDetails(vp->comp(),
                   "size node has no known constraint for anewarray %p\n",
                   sizeNode);
    // node->setAllocationCanBeRemoved(false)
  } else {
    if (typeConstraint && typeConstraint->getClassType() &&
      ...
      TR_OpaqueClassBlock *arrayClass =
          vp->fe()->getArrayClassFromComponentClass(
              typeConstraint->getClassType()->getClass());
      if (arrayClass) node->setAllocationCanBeRemoved(true);
    }
  }
...
TR::Node *constrainMultiANewArray(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  //
  int32_t maxSize =
      (int32_t)TR::Compiler->om.maxArraySizeInElementsForAllocation(
          node, vp->comp());  // int32_t is ok because multiANewArray is only
                              // useful in Java, where array sizes are bound by
                              // TR::getMaxSigned<TR::Int32>()
  int64_t maxHeapSize;
  ...
  if (maxHeapSize > 0) {
    int64_t maxHeapSizeInPointers =
        maxHeapSize / TR::Compiler->om.sizeofReferenceField();
  ...
  if (numChildren - 2 == 1)  // dims == 1
  {
    ...
    // We need to determine the size of the element type defined by the type
    // node
  ...
  TR::VPArrayInfo *arrayInfo =
      TR::VPArrayInfo::create(vp, sizeConstraint->getLowInt(),
                              sizeConstraint->getHighInt(), elementSize);
  vp->addGlobalConstraint(
      node, TR::VPClass::create(vp, newType, TR::VPNonNullObject::create(vp),
                                NULL, arrayInfo,
                                TR::VPObjectLocation::create(
                                    vp, TR::VPObjectLocation::NotClassObject)));
...
static TR::VPLongRange *getLongRange(TR::VPConstraint *c) {
  return c->asLongRange();
}
static TR::VPLongConst *getLongConst(TR::VPConstraint *c) {
  return c->asLongConst();
}
static TR::VPIntRange *getIntRange(TR::VPConstraint *c) {
  return c->asIntRange();
}
static TR::VPIntConst *getIntConst(TR::VPConstraint *c) {
  return c->asIntConst();
}
...
static void getLowHighLongs(TR::VPLongRange *range, int64_t &low,
                            int64_t &high) {
...
static TR::VPConstraint *createIntConstConstraint(OMR::ValuePropagation *vp,
                                                  int32_t n) {
  return TR::VPIntConst::create(vp, n);
}
static TR::VPConstraint *createIntRangeConstraint(OMR::ValuePropagation *vp,
                                                  int32_t l, int32_t h) {
  return TR::VPIntRange::create(vp, l, h);
}
static TR::VPConstraint *createLongConstConstraint(OMR::ValuePropagation *vp,
                                                   int64_t n) {
  return TR::VPLongConst::create(vp, n);
}
static TR::VPConstraint *createLongRangeConstraint(OMR::ValuePropagation *vp,
                                                   int64_t l, int64_t h) {
  return TR::VPLongRange::create(vp, l, h);
}

static int32_t integerToPowerOf2(int32_t n) {
  return (n == 0) ? 0 : floorPowerOfTwo(n);
}
static int32_t integerNumberOfLeadingZeros(int32_t n) {
  return leadingZeroes(n);
}
static int32_t integerNumberOfTrailingZeros(int32_t n) {
  return trailingZeroes(n);
}
static int32_t integerBitCount(int32_t n) { return populationCount(n); }
static int64_t longToPowerOf2(int64_t n) {
  return (n == 0) ? 0 : floorPowerOfTwo64(n);
}
static int32_t longNumberOfLeadingZeros(int64_t n) { return leadingZeroes(n); }
static int32_t longNumberOfTrailingZeros(int64_t n) {
  return trailingZeroes(n);
}
...
void addValidRangeBlockOrGlobalConstraint(OMR::ValuePropagation *vp,
                                          TR::Node *node, FUNC createRange,
                                          FUNC2 processValue, T low, T high,
                                          bool childGlobal) {
  ...
  if (vp->trace()) {
    traceMsg(vp->comp(),
             "Adding a %s range constraint %lld .. %lld on the node %p\n",
             (childGlobal) ? "global" : "block", pLow, pHigh, node);
  }

  vp->addBlockOrGlobalConstraint(node, createRange(vp, pLow, pHigh),
                                 childGlobal);
}

template <typename A, typename B, typename C, typename D, typename E,
          typename F, typename G, typename T>
static TR::Node *constrainHighestOneBitAndLeadingZerosHelper(
    OMR::ValuePropagation *vp, TR::Node *node, A getConst, B getRange,
    C getValue, D getValues, E createConst, F createRange, G processValue,
    T MIN_VALUE, T MAX_VALUE) {
  TR_ASSERT(node->getNumChildren() == 1,
            "Node has a wrong number of children (i.e. !=1 )! ");
  constrainChildren(vp, node);

  if (vp->trace()) {
    traceMsg(
        vp->comp(),
        "calling constrainHighestOneBitAndLeadingZerosHelper for node %p\n",
        node);
  }

  bool childGlobal;
  TR::VPConstraint *childConstraint =
      vp->getConstraint(node->getFirstChild(), childGlobal);
  ...
  if (childConstraint) {
    if (getConst(childConstraint)) {
      ...
      // RangeConstraint with the same low and high should be folded into a
      // const constraint
      MIN_VALUE = value;
      MAX_VALUE = value;
      ...
      if (vp->trace()) {
        traceMsg(vp->comp(),
                 "The first child's value of %p %lld is replaced with %lld \n",
      ...
      if (low < 0 && high < 0) {
        // RangeConstraint with the same low and high should be folded into a
        // const constraint. All negative numbers have the highest bit set
        MIN_VALUE = MAX_VALUE;
        if (vp->trace()) {
          traceMsg(vp->comp(),
                   "Constraint %lld .. %lld of %p 's first child is negative "
                   "and folded into %lld \n",
  ...
  addValidRangeBlockOrGlobalConstraint(vp, node, createRange, processValue,
                                       MIN_VALUE, MAX_VALUE, childGlobal);
  return node;
}

template <typename A, typename B, typename C, typename D, typename E,
          typename F, typename G, typename T>
static TR::Node *constrainLowestOneBitAndTrailingZerosHelper(
    OMR::ValuePropagation *vp, TR::Node *node, A getConst, B getRange,
    C getValue, D getValues, E createConst, F createRange, G processValue,
    T MIN_VALUE, T MAX_VALUE) {
  TR_ASSERT(node->getNumChildren() == 1,
  ...
  if (vp->trace()) {
    traceMsg(
        vp->comp(),
        "calling constrainLowestOneBitAndTrailingZerosHelper for node %p\n",
        node);
  }
...
TR::Node *constrainIntegerHighestOneBit(OMR::ValuePropagation *vp,
                                        TR::Node *node) {
  return constrainHighestOneBitAndLeadingZerosHelper(
      vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
      createIntConstConstraint,
      // createIntRangeConstraint, integerToPowerOf2, (int32_t) 0, (int32_t)
      // -1);
      createIntRangeConstraint, integerToPowerOf2,
      (int32_t)TR::getMaxSigned<TR::Int32>(),
      (int32_t)TR::getMinSigned<TR::Int32>());
}

TR::Node *constrainIntegerNumberOfLeadingZeros(OMR::ValuePropagation *vp,
                                               TR::Node *node) {
  return constrainHighestOneBitAndLeadingZerosHelper(
      vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
      createIntConstConstraint, createIntRangeConstraint,
      integerNumberOfLeadingZeros, (int32_t)0, (int32_t)-1);
}

TR::Node *constrainLongHighestOneBit(OMR::ValuePropagation *vp,
                                     TR::Node *node) {
  return constrainHighestOneBitAndLeadingZerosHelper(
      vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
      createLongConstConstraint,
      // createLongRangeConstraint, longToPowerOf2, (int64_t) 0, (int64_t) -1);
      createLongRangeConstraint, longToPowerOf2,
      (int64_t)TR::getMaxSigned<TR::Int64>(), TR::getMinSigned<TR::Int64>());
}

TR::Node *constrainLongNumberOfLeadingZeros(OMR::ValuePropagation *vp,
                                            TR::Node *node) {
  return constrainHighestOneBitAndLeadingZerosHelper(
      vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
      createIntConstConstraint, createIntRangeConstraint,
      longNumberOfLeadingZeros, (int64_t)0, (int64_t)-1);
}

TR::Node *constrainIntegerLowestOneBit(OMR::ValuePropagation *vp,
                                       TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
      createIntConstConstraint, createIntRangeConstraint, integerToPowerOf2,
      (int32_t)TR::getMaxSigned<TR::Int32>(),
      (int32_t)TR::getMinSigned<TR::Int32>());
}
...
TR::Node *constrainIntegerBitCount(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
      createIntConstConstraint, createIntRangeConstraint, integerBitCount,
      (int32_t)0, (int32_t)-1);
}

TR::Node *constrainIntegerNumberOfTrailingZeros(OMR::ValuePropagation *vp,
                                                TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getIntConst, getIntRange, getInt, getLowHighInts,
      createIntConstConstraint, createIntRangeConstraint,
      integerNumberOfTrailingZeros, (int32_t)0, (int32_t)-1);
}

TR::Node *constrainLongLowestOneBit(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
      createLongConstConstraint, createLongRangeConstraint, longToPowerOf2,
      (int64_t)TR::getMaxSigned<TR::Int64>(),
      (int64_t)TR::getMinSigned<TR::Int64>());
}

TR::Node *constrainLongNumberOfTrailingZeros(OMR::ValuePropagation *vp,
                                             TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
      createIntConstConstraint, createIntRangeConstraint,
      longNumberOfTrailingZeros, (int64_t)0, (int64_t)-1);
}

TR::Node *constrainLongBitCount(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainLowestOneBitAndTrailingZerosHelper(
      vp, node, getLongConst, getLongRange, getLong, getLowHighLongs,
      createIntConstConstraint, createIntRangeConstraint, longBitCount,
      (int64_t)0, (int64_t)-1);
}
...
TR::Node *constrainArraylength(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (constraint) {
    ...
    if (knot && kobj) {
      TR::VMAccessCriticalSection constrainArraylengthCriticalSection(
          vp->comp(), TR::VMAccessCriticalSection::tryToAcquireVMAccess);
      if (constrainArraylengthCriticalSection.hasVMAccess()) {
        uintptrj_t array = knot->getPointer(kobj->getIndex());
        if (vp->comp()->fej9()->isClassArray(
                vp->comp()->fej9()->getObjectClass(array))) {
          uintptrj_t length =
              vp->comp()->fej9()->getArrayLengthInElements(array);
          vp->replaceByConstant(node, TR::VPIntConst::create(vp, length),
                                isGlobal);
          return node;
        }
      }
  ...
  if (!elementSize) {
    ...
    if (!elementSize && constraint) {
      ...
      if (sig) elementSize = arrayElementSize(sig, len, objectRef, vp);
    }

    if (elementSize) {
      constraint = TR::VPArrayInfo::create(vp, lowerBoundLimit, upperBoundLimit,
                                           elementSize);
  ...
  if (lowerBoundLimit == upperBoundLimit) {
    ...
    if (!(node->getOpCodeValue() == TR::contigarraylength ||
          node->getOpCodeValue() == TR::discontigarraylength) ||
      ...
      if (node->getOpCodeValue() == TR::contigarraylength) intValue = 0;
  ...
  // For hybrid arraylets, a constant can't be used if a contiguity check is
  // still required.
  //
  if (!(node->getOpCodeValue() == TR::contigarraylength ||
        node->getOpCodeValue() == TR::discontigarraylength) ||
    ...
    if (node->getOpCodeValue() == TR::contigarraylength) {
      int32_t maxContiguousArraySizeInElements =
          TR::Compiler->om.maxContiguousArraySizeInBytes() >> shiftAmount;
  ...
  if (!node->getArrayStride() &&
      performTransformation(vp->comp(),
                            "%sSetting element width for array [%p] to %d\n",
...
static void refineSymbolReferenceForProfiledToOverridden(
    OMR::ValuePropagation *vp, TR::Node *callNode,
    TR::SymbolReference *newSymRef) {
  for (OMR::ValuePropagation::VirtualGuardInfo *cvg =
           vp->_convertedGuards.getFirst();
       cvg; cvg = cvg->getNext()) {
    if (cvg->_callNode == callNode) {
      if (vp->trace()) {
        traceMsg(
            vp->comp(),
            "Replaced with newSymRef %d in a Profiled2Overridden guard %p\n",
...
static TR::MethodSymbol *refineMethodSymbolInCall(
    OMR::ValuePropagation *vp, TR::Node *node, TR::SymbolReference *symRef,
    TR_ResolvedMethod *resolvedMethod, int32_t offset) {
  TR::SymbolReference *newSymRef =
...
static void devirtualizeCall(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (!methodSymbol->firstArgumentIsReceiver()) {
    if (vp->trace())
      traceMsg(vp->comp(),
               "Not attempting to de-virtualize call [%p] without first "
               "argument receiver\n",
               node);
    return;
  }

  if (!vp->comp()->fe()->canDevirtualizeDispatch()) return;

  int32_t firstArgIndex = node->getFirstArgumentIndex();
  bool isGlobal;
  TR::VPConstraint *constraint =
      vp->getConstraint(node->getChild(firstArgIndex), isGlobal);
  TR_OpaqueClassBlock *thisType = NULL;
  if (!constraint || !(thisType = constraint->getClass())) {
    if (vp->trace())
      traceMsg(vp->comp(),
               "Interface call [%p] to %s with unknown object type in %s\n",
               node,
               methodSymbol->getMethod()->signature(vp->trMemory(), stackAlloc),
               vp->comp()->signature());

    static bool dontProfileMore =
        feGetEnv("TR_DontProfileMoreAtHot") ? true : false;

    if (!dontProfileMore && (vp->comp()->getMethodHotness() == hot) &&
        vp->comp()->getRecompilationInfo() && vp->_isGlobalPropagation &&
        vp->lastTimeThrough() && vp->optimizer()->switchToProfiling()) {
      dumpOptDetails(
          vp->comp(),
          "%s enabling profiling: would be useful to profile arraycopy calls\n",
          OPT_DETAILS);
    }
  ...
  if (constraint->getArrayInfo()) {
    ...
    if (!thisType) {
      if (vp->trace())
        traceMsg(
            vp->comp(),
            "Not attempting to de-virtualize call [%p] with array receiver\n",
            node);
      return;
    }
  ...
  if ((constraint->asClass() &&
    ...
    if (!thisType) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "Not attempting to de-virtualize call [%p] with class object "
                 "receiver\n",
                 node);
      return;
    }
  ...
  if (argNode->getSymbol()) {
    ...
    if (argSym->isParm()) {
      if (!vp->isParmInvariant(argSym)) {
        if (vp->trace())
          traceMsg(vp->comp(),
                   "Not attempting to de-virtualize call [%p] because receiver "
                   "[%p] is not invariant\n",
                   node, argNode);
        return;
      }
  ...
  if (methodSymbol->isComputed()) {
    ...
    if (methodSymbol->getRecognizedMethod() ==
      ...
      if (constraint && constraint->getKnownObject() && knot) {
        TR::ResolvedMethodSymbol *owningMethod =
            symRef->getOwningMethodSymbol(vp->comp());
        resolvedMethod =
            vp->comp()->fej9()->createMethodHandleArchetypeSpecimen(
                vp->trMemory(),
                knot->getPointerLocation(
                    constraint->getKnownObject()->getIndex()),
                owningMethod->getResolvedMethod());
        if (resolvedMethod) {
          TR::SymbolReference *specimenSymRef =
              vp->getSymRefTab()->findOrCreateMethodSymbol(
                  owningMethod->getResolvedMethodIndex(), -1, resolvedMethod,
                  TR::MethodSymbol::ComputedVirtual);
          if (performTransformation(
                  vp->comp(),
                  "%sSubstituting more specific method symbol on %p: %s\n",
          ...
          if (vp->optimizer()
                  ->getOptimization(OMR::methodHandleInvokeInliningGroup)
                  ->requested()) {
            if (vp->trace())
              traceMsg(vp->comp(),
                       "Not inlining call [%p] because the MethodHandle.invoke "
                       "inlining group will do a better job\n",
                       node);
            return;
          }
        } else {
          if (vp->trace())
            traceMsg(vp->comp(),
                     "Not inlining call [%p] because there is no more specific "
                     "method symbol for obj%d\n",
        ...
        if (vp->trace())
          traceMsg(vp->comp(),
                   "Not inlining call [%p] because unable to substitute "
                   "object-specific method symbol\n",
                   node);
        return;
      }
    }
    ...
    if (TR::Compiler->cls.isInterfaceClass(vp->comp(), thisType)) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "Not attempting to de-virtualize interface call [%p] with "
                 "interface-class receiver\n",
                 node);
      return;
    }
    ...
    if (!originalMethodClass) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "Not attempting to de-virtualize interface call [%p] with "
                 "receiver class %.*s not yet loaded\n",
                 node, len, s);
      return;
    }

    resolvedMethod =
        owningMethod->getResolvedInterfaceMethod(vp->comp(), thisType, cpIndex);
    if (!resolvedMethod) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "Not attempting to de-virtualize interface call [%p] with no "
                 "resolved method\n",
                 node, len, s);
      return;
    }
    ...
    TR_ASSERT(methodSymbol->isVirtual(),
              "unexpected method symbol kind for %s node %p",
              node->getOpCode().getName(), node);

    TR::ResolvedMethodSymbol *resolvedMethodSymbol =
        methodSymbol->getResolvedMethodSymbol();
    if (!resolvedMethodSymbol ||
        symRef == vp->getSymRefTab()->findObjectNewInstanceImplSymbol())
      return;

    originalResolvedMethod = resolvedMethodSymbol->getResolvedMethod();
    originalMethodClass = originalResolvedMethod->classOfMethod();

    if ((vp->fe()->isInstanceOf(thisType, originalMethodClass, true, true) !=
         TR_yes) ||
        (originalMethodClass &&
         TR::Compiler->cls.isInterfaceClass(vp->comp(), originalMethodClass)))
      return;

    if (!constraint->isFixedClass()) {
      if (!constraint->isPreexistentObject()) {
        if (originalMethodClass == thisType) return;

        s = node->getChild(firstArgIndex)->getTypeSignature(len);
        if (s &&
            vp->fe()->getClassFromSignature(s, len, owningMethod) == thisType)
          return;
      }
    }
    ...
    resolvedMethod =
        owningMethod->getResolvedVirtualMethod(vp->comp(), thisType, offset);
  ...
  if (node->isTheVirtualCallNodeForAGuardedInlinedCall()) {
    ...
    TR_PersistentClassInfo *classInfo =
        vp->comp()
            ->getPersistentInfo()
            ->getPersistentCHTable()
            ->findClassInfoAfterLocking(thisType, vp->comp());
    if (!classInfo || !classInfo->isInitialized()) return;
#else
    return;
#endif
  }
  ...
  if (methodSymbol->isVirtual() || methodSymbol->isInterface()) {
    ...
    if (methodSymbol->isVirtual() &&
        (!originalResolvedMethod ||
         !resolvedMethod->isSameMethod(originalResolvedMethod))) {
      // The virtual guard noping code relies on the the virtual call node's
      // symbol to refer to the original devirtualized call.  Not devirtualizing
      // the call shouldn't be a performance concern since the call is in a cold
      // block.
      //
      // Why do we need to stop the devirtualization if its a
      // guarded virtual ? Commenting these two lines out so that a guarded
      // virtual gets added to the devirtualizedCalls list and the guard is
      // eventually removed. This has the benefit that the call no longer
      // interferes with aliasing (and other opts like escape analysis).
      //
      if (performTransformation(
              vp->comp(), "%sDevirtualizing call [%p] to %s\n", OPT_DETAILS,
              node, resolvedMethod->signature(vp->trMemory())))
        methodSymbol =
            refineMethodSymbolInCall(vp, node, symRef, resolvedMethod, offset);
    } else if (!resolvedMethod->virtualMethodIsOverridden() &&
               !constraint->isFixedClass())
      return;
    ...
    if (constraint->isFixedClass() || constraint->isPreexistentObject()) {
      if (!performTransformation(
              vp->comp(),
              "%sChanging an indirect call %s (%s) to a direct call [%p]\n",
              OPT_DETAILS, resolvedMethod->signature(vp->trMemory()),
              node->getOpCode().getName(), node))
        return;
      ...
      if (!vp->registerPreXClass(constraint) &&
          constraint->isPreexistentObject()) {
        if (!resolvedMethod->virtualMethodIsOverridden() &&
            (constraint->getClass() ==
             constraint->getPreexistence()->getAssumptionClass()))
      ...
      //
      TR_ASSERT(firstArgIndex == 1,
                "more than one generated argument on an indirect call");
      node->getChild(0)->recursivelyDecReferenceCount();
      int32_t numChildren = node->getNumChildren();
      for (int32_t i = 1; i < numChildren;
           i = (++i & 0x0000ffff))  // this & with 0x0000ffff is to work around
                                    // an opt bug in the ibm build on ia32
  ...
  TR_PrexArgInfo *argInfo = new (vp->trStackMemory())
      TR_PrexArgInfo(node->getNumChildren() - firstArgIndex, vp->trMemory());
  bool tracePrex = vp->trace() || vp->comp()->trace(OMR::inlining) ||
                   vp->comp()->trace(OMR::invariantArgumentPreexistence);
  if (tracePrex)
    traceMsg(vp->comp(),
             "PREX.vp: Value propagation populating prex argInfo for %s %p\n",
             node->getOpCode().getName(), node);
      ...
      if (constr) {
        ...
        if (constr->asKnownObject() && constr->isNonNullObject()) {
          argInfo->set(c - firstArgIndex,
                       new (vp->trStackMemory()) TR_PrexArgument(
                           constr->asKnownObject()->getIndex(), vp->comp()));
          if (tracePrex)
            traceMsg(vp->comp(),
                     "PREX.vp:    Child %d [%p] arg %p is known object obj%d\n",
                     c, argument, argInfo->get(c - firstArgIndex),
                     constr->asKnownObject()->getIndex());
        } else if (constr->isFixedClass()) {
          argInfo->set(c - firstArgIndex,
                       new (vp->trStackMemory()) TR_PrexArgument(
                           TR_PrexArgument::ClassIsFixed, constr->getClass()));
          if (tracePrex) {
            TR_OpaqueClassBlock *clazz = constr->getClass();
            const char *sig = TR::Compiler->cls.classSignature(
                vp->comp(), clazz, vp->trMemory());
            traceMsg(vp->comp(),
                     "PREX.vp:    Child %d [%p] arg %p has fixed class %p %s\n",
                     c, argument, argInfo->get(c - firstArgIndex), clazz, sig);
          }
        } else if (constr->isPreexistentObject()) {
          argInfo->set(c - firstArgIndex,
                       new (vp->trStackMemory()) TR_PrexArgument(
                           TR_PrexArgument::ClassIsPreexistent));
          if (tracePrex)
            traceMsg(vp->comp(),
                     "PREX.vp:    Child %d [%p] arg %p is preexistent\n", c,
                     argument, argInfo->get(c - firstArgIndex));
        }
      }
    }
  }
  ...
  if ((vp->lastTimeThrough() || !node->getOpCode().isIndirect()) &&
    ...
    vp->_devirtualizedCalls.add(
        new (vp->trStackMemory())
            OMR::ValuePropagation::CallInfo(vp, thisType, argInfo));
  vp->invalidateUseDefInfo();
  vp->invalidateValueNumberInfo();
}
...
TR::Node *constrainCall(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (vp->lastTimeThrough() && callIsDirect &&
      TR::Compiler->om.canGenerateArraylets() &&
      node->getSymbol()->getMethodSymbol()->getMethod() &&
      (node->getSymbol()
           ->getMethodSymbol()
           ->getMethod()
           ->isUnsafeWithObjectArg() ||
...

  {
    ...
    bool operandGlobal;
    TR::VPConstraint *operand =
        vp->getConstraint(node->getChild(1), operandGlobal);
    if (operand && ((operand->isClassObject() == TR_yes) ||
      ...
      vp->_unsafeCallsToInline.add(new (
          vp->trStackMemory()) OMR::ValuePropagation::CallInfo(vp, NULL, NULL));
  ...
  if (node->getOpCode().isIndirect() && !debug("dontDevirtualize")) {
    ...
    if (node->getSymbolReference()->isUnresolved() && symbol->isInterface()) {
      ...
      TR_ResolvedMethod *method =
          node->getSymbolReference()->getOwningMethod(vp->comp());
      TR::VPConstraint *constraint =
          TR::VPUnresolvedClass::create(vp, sig, len, method);
      int32_t firstArgIndex = node->getFirstArgumentIndex();
      bool isGlobal;
      TR::VPConstraint *existingConstraint =
          vp->getConstraint(node->getChild(firstArgIndex), isGlobal);
      if (existingConstraint && existingConstraint->intersect(constraint, vp)) {
        ...
        // dumpOptDetails(vp->comp(), "First time an interface call made using
        // receiver at node %p in %s\n", node, vp->comp()->signature());
  ...
  if (node->getType().isInt32() &&
    ...
    if (method) {
      ...
      if (dataType == TR::Int8 || dataType == TR::Int16 ||
          dataType == TR::Int32 || dataType == TR::Int64) {
        bool isUnsigned = method->returnTypeIsUnsigned();
        TR::VPConstraint *constraint =
            TR::VPIntRange::create(vp, dataType, isUnsigned ? TR_yes : TR_no);
        if (constraint) vp->addGlobalConstraint(node, constraint);
      }
    }
  }
  ...
  if (!node->getOpCode().isCall()) return node;
  ...
  if (symbol) {
    ...
    if (isIntegerAbs || isLongAbs) {
      // one special case to worry about here: if the operand of the ABS could
      // be the minimum value of the range (TR::getMinSigned<TR::Int32>() or
      // TR::getMinSigned<TR::Int64>()) then the return value of the ABS could
      // be that same minimum value (yes, that's right, it could be negative).
      bool operandGlobal;
      TR::VPConstraint *operand =
          vp->getConstraint(node->getFirstChild(), operandGlobal);
      ...
      if (isIntegerAbs) {
        ...
        couldBeMinValue =
            (operand == NULL ||
             operand->getLowInt() == TR::getMinSigned<TR::Int32>());
      } else {
        TR_ASSERT(
            isLongAbs,
            "");  // sanity check that no other cases get in here by mistake
        positiveConstraint =
            TR::VPLongRange::create(vp, 0, TR::getMaxSigned<TR::Int64>());
        couldBeMinValue =
            (operand == NULL ||
             operand->getLowLong() == TR::getMinSigned<TR::Int64>());
      }
      ...
      if (couldBeMinValue) {
        ...
        absConstraint = TR::VPMergedConstraints::create(vp, minValueConstraint,
                                                        positiveConstraint);
      } else
        absConstraint = positiveConstraint;
      vp->addGlobalConstraint(node, absConstraint);
      if (!couldBeMinValue) node->setIsNonNegative(true);
      node->setCannotOverflow(true);
    } else if (isFloatAbs || isDoubleAbs) {
      // TODO: when we implement FP constraints, need to add the 0,MAX_F or
      // 0,MAX_D constraint here
    } else if (symbol->getRecognizedMethod() ==
               TR::java_lang_Class_getComponentType) {
      TR::Node *classPointer = node->getFirstChild();
      if (classPointer->getOpCode().hasSymbolReference() &&
          (classPointer->getSymbolReference() ==
           vp->comp()->getSymRefTab()->findVftSymbolRef())) {
        bool operandGlobal;
        TR::VPConstraint *operand =
            vp->getConstraint(classPointer->getFirstChild(), operandGlobal);
        if (operand) {
          TR::VPArrayInfo *arrayInfo = operand->getArrayInfo();
          if (arrayInfo || (operand->getClassType() &&
                            (operand->getClassType()->isArray() == TR_yes))) {
            vp->_javaLangClassGetComponentTypeCalls.add(node);
          }
        }
      }
    } else if (symbol->getRecognizedMethod() ==
                   TR::java_lang_Integer_numberOfLeadingZeros ||
               symbol->getRecognizedMethod() ==
                   TR::java_lang_Integer_numberOfTrailingZeros) {
      vp->addGlobalConstraint(node, TR::VPIntRange::create(vp, 0, 32));
    } else if (symbol->getRecognizedMethod() ==
                   TR::java_lang_Long_numberOfLeadingZeros ||
               symbol->getRecognizedMethod() ==
                   TR::java_lang_Long_numberOfTrailingZeros) {
      ...
      TR_ASSERT(node->getNumChildren() == 2,
                "call to TreeMap.rbInsert should have only 2 children");
                  ...
                  if (defChild->getOpCodeValue() == TR::New) {
                    ...
                    if (useDefInfo->isUseIndex(useIndexC)) {
                      ...
                      if (useDefInfo->getUseDef(defsC, useIndexC)) {
                        ...
                        for (cursorC.SetToFirstOne(); cursorC.Valid();
                             cursorC.SetToNextOne()) {
                          ...
                          TR::TreeTop *defTreeC =
                              useDefInfo->getTreeTop(defIndexC);
                          TR::Node *defNodeC = defTreeC->getNode();

                          if (defNodeC &&
                              (defNodeC->getOpCodeValue() == TR::astore)) {
                            TR::Node *defChildC = defNodeC->getFirstChild();
                            if (defChildC &&
                                defChildC->getOpCodeValue() == TR::New) {
      ...
      if (definitionNode) {
        definitionNode->setAlignTLHAlloc(true);
               ...
               // symbol->isNative() &&
               (symbol->getRecognizedMethod() ==
                TR::java_lang_J9VMInternals_identityHashCode)) {
      // new hashcode algorithm
      //
      TR_OpaqueClassBlock *jitHelpersClass =
          vp->comp()->getJITHelpersClassPointer();
      if (jitHelpersClass &&
        ...
        vp->comp()->fej9()->getResolvedMethods(vp->trMemory(), jitHelpersClass,
                                               &helperMethods);
        ...
        for (TR_ResolvedMethod *m = it.getCurrent(); m; m = it.getNext()) {
          ...
          if (!strncmp(sig, "hashCodeImpl", 11)) {
            hashCodeMethodSymRef =
                vp->comp()->getSymRefTab()->findOrCreateMethodSymbol(
                    JITTED_METHOD_INDEX, -1, m, TR::MethodSymbol::Virtual);
            hashCodeMethodSymRef->setOffset(TR::Compiler->cls.vTableSlot(
                vp->comp(), m->getPersistentIdentifier(), jitHelpersClass));
        ...
        if (vp->comp()->getOption(TR_EnableJITHelpershashCodeImpl) &&
            hashCodeMethodSymRef && getHelpersSymRef &&
            performTransformation(
                vp->comp(),
                "%sChanging call to new hashCodeImpl at node [%p]\n",
                OPT_DETAILS, node)) {
          // FIXME: add me to the list of calls to be inlined
          //
          TR::Method *method =
              getHelpersSymRef->getSymbol()->castToMethodSymbol()->getMethod();
          TR::Node *helpersCallNode = TR::Node::createWithSymRef(
              node, method->directCallOpCode(), 0, getHelpersSymRef);
          TR::TreeTop *helpersCallTT = TR::TreeTop::create(
              vp->comp(), TR::Node::create(TR::treetop, 1, helpersCallNode));
          vp->_curTree->insertBefore(helpersCallTT);

          method = hashCodeMethodSymRef->getSymbol()
                       ->castToMethodSymbol()
                       ->getMethod();
          ...
          return node;
               ...
               TR::sun_misc_Unsafe_ensureClassInitialized) {
      /*
      n572n       vcall
      sun/misc/Unsafe.ensureClassInitialized(Ljava/lang/Class;)V[#631  final
      native virtual Method -728] n577n         aload
      java/lang/invoke/MethodHandle$UnsafeGetter.myUnsafe n579n         iaload
      java/lang/invoke/MethodHandle.defc

      Because Unsafe.ensureClassInitialized is JNI, can't use direct JNI,
      otherwise can't optimized it at compile time.
      */
      TR::Node *classValueNode = node->getSecondChild();
      bool isGlobal;
      TR::VPConstraint *constraint =
          vp->getConstraint(classValueNode, isGlobal);
      if (constraint && constraint->getKnownObject() &&
          constraint->isClassObject() == TR_yes &&
        ...
        if (j9class &&
            ...
            performTransformation(
                vp->comp(),
                "%s Remove TR::sun_misc_Unsafe_ensureClassInitialized call, "
                "class already initialized [%p]\n",
                OPT_DETAILS, node)) {
          TR::Node *receiver = node->getChild(0);
          TR::TransformUtil::transformCallNodeToPassThrough(
              vp, node, vp->_curTree, receiver);
          return node;
        }
      }
    }
...
TR::Node *constrainAdd(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs) {
    ...
    if (constraint) {
      ...
      if (longAdd) didReduction = reduceLongOpToIntegerOp(vp, node, constraint);

      vp->addBlockOrGlobalConstraint(node, constraint, lhsGlobal);
      // If we reduced long to integer, node is now an i2l; not safe to do any
      // further addition-related things with node
      if (didReduction) return node;
    }
  }
  ...
  if (rhs) {
    ...
    if (rhs->asLongConst()) {
      if (rhs->asLongConst()->getLong() > TR::getMinSigned<TR::Int32>() &&
        ...
        constraint =
            TR::VPEqual::create(vp, (int32_t)rhs->asLongConst()->getLong());
...
TR::Node *constrainSubtract(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs) {
    ...
    if (constraint) {
      ...
      bool didReduction = false;
      if (longSub) didReduction = reduceLongOpToIntegerOp(vp, node, constraint);
      vp->addBlockOrGlobalConstraint(node, constraint, lhsGlobal);
      // If we reduced long to integer, node is now an i2l; not safe to do any
      // further subtract-related things with node
      if (didReduction) return node;
    }
  }
...
TR::Node *constrainImul(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs) {
    ...
    if (lhs->asIntConst() && rhs->asIntConst()) {
      ...
      //   ((uint32_t)lhs->asIntConst()->getInt()*(uint32_t)rhs->asIntConst()->getInt()),
      //   isUnsigned);
      // else
      constraint = TR::VPIntConst::create(
          vp, lhs->asIntConst()->getInt() * rhs->asIntConst()->getInt());
    } else {
      int64_t lowerLowerLimit =
          (int64_t)lhs->getLowInt() * (int64_t)rhs->getLowInt();
      int64_t lowerUpperLimit =
          (int64_t)lhs->getLowInt() * (int64_t)rhs->getHighInt();
      int64_t upperLowerLimit =
          (int64_t)lhs->getHighInt() * (int64_t)rhs->getLowInt();
      int64_t upperUpperLimit =
          (int64_t)lhs->getHighInt() * (int64_t)rhs->getHighInt();
      ...
      if (lowest < TR::getMinSigned<TR::Int32>() ||
          highest > TR::getMaxSigned<TR::Int32>()) {
        constraint = NULL;
      } else {
        constraint = TR::VPIntRange::create(vp, (int32_t)lowest,
                                            (int32_t)highest /*, isUnsigned*/);
      }
    }
    if (constraint) {
      if (constraint->asIntConst()) {
...
// maxValue should either be TR::getMaxUnsigned<TR::Int32>() or
// TR::getMaxSigned<TR::Int32>(); the latter is used when this method is called
// with (positive) signed numbers
bool can64BitUnsignedMultiplyOverflow(uint64_t a, uint64_t b,
                                      uint64_t maxValue) {
  // a must be the lower number
  if (a > b) return can64BitUnsignedMultiplyOverflow(b, a, maxValue);

  // If both numbers are >= 2^32, the product must be >= 2^64
  if (a > TR::getMaxUnsigned<TR::Int32>()) return true;

  // If both numbers are < 2^32, the product must be < 2^64
  if (b <= TR::getMaxUnsigned<TR::Int32>()) return false;

  // a is < 2^32; b is >= 2^32
  // If the high word of b is zero, we're multiplying two numbers both less than
  // 2^32, which is < 2^64, so no overflow. If the high word of b is non-zero,
  // the final product x will be ((a * HIGHWORD(b)) << 32) + (a * LOWWORD(b)) If
  // we compute the highword of the above expression in a 64-bit variable, a
  // value >= 2^32 indicates overflow
  uint64_t highWord = b >> 32;
  uint64_t lowWord = b & 0xFFFFFFFF;
  uint64_t lowWordOverflow =
      (a * lowWord) >>
      32;  // Can't overflow 64 bits because a and lowWord are both < 2^32
  ...
  if (a * highWord > maxValue) return true;

  // a * highWord is < 2^32 and lowWordOverflow is < 2^32, so it's safe to add
  // them If the sum is > 2^32, a * b will overflow
  if ((a * highWord) + lowWordOverflow > maxValue) return true;
...
bool can64BitSignedMultiplyOverflow(int64_t a, int64_t b) {
  ...
  // If both numbers are < 2^32, the product must be < 2^64
  if (a >= TR::getMinSigned<TR::Int32>() &&
      a <= TR::getMaxSigned<TR::Int32>() &&
  ...
  // If both numbers are positive, use the unsigned code, but limit the high
  // word of the result to TR::getMaxSigned<TR::Int32>()
  if (a > 0 && b > 0)
    return can64BitUnsignedMultiplyOverflow((uint64_t)a, (uint64_t)b,
                                            TR::getMaxSigned<TR::Int32>());

  // If both are negative, multiply both by -1 and use the unsigned code
  if (a < 0 && a != TR::getMinSigned<TR::Int64>() && b < 0 &&
      b != TR::getMinSigned<TR::Int64>())
    return can64BitUnsignedMultiplyOverflow(
        (uint64_t)(-1 * a), (uint64_t)(-1 * b), TR::getMaxSigned<TR::Int32>());

  // Only remaining case is one is positive, one is negative, and neither is 0
  // nor TR::getMinSigned<TR::Int64>() Treat both numbers as positive and use
  // the unsigned code
  if (a < 0)
    return can64BitUnsignedMultiplyOverflow((uint64_t)(-1 * a), (uint64_t)b,
                                            TR::getMaxSigned<TR::Int32>());
  else if (b < 0)
    return can64BitUnsignedMultiplyOverflow((uint64_t)a, (uint64_t)(-1 * b),
                                            TR::getMaxSigned<TR::Int32>());
...
bool can64BitMultiplyOverflow(uint64_t a, uint64_t b, bool isUnsigned) {
  if (isUnsigned)
    return can64BitUnsignedMultiplyOverflow(a, b,
...
TR::Node *constrainLmul(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs) {
    ...
    if (lhs->asLongConst() && rhs->asLongConst()) {
      constraint = TR::VPLongConst::create(
          vp, lhs->asLongConst()->getLong() * rhs->asLongConst()->getLong());
      vp->replaceByConstant(node, constraint, lhsGlobal);
    } else {
      // If no combination overflows, it's safe to create a constraint
      if (!(can64BitMultiplyOverflow(lhs->getLowLong(), rhs->getLowLong(),
                                     node->getOpCode().isUnsigned()) ||
            can64BitMultiplyOverflow(lhs->getLowLong(), rhs->getHighLong(),
                                     node->getOpCode().isUnsigned()) ||
            can64BitMultiplyOverflow(lhs->getHighLong(), rhs->getLowLong(),
                                     node->getOpCode().isUnsigned()) ||
...
bool constrainIntegerDivisionRange(int64_t lhsLow, int64_t lhsHigh,
                                   int64_t rhsLow, int64_t rhsHigh,
                                   int64_t rangeMin, int64_t rangeMax,
                                   int64_t &resultLow, int64_t &resultHigh,
                                   bool allowZeroInDivisorRange) {
  // No matter what, division by constant 0 isn't something we'll ever simplify
  if (rhsLow == 0 && rhsHigh == 0) return false;

  if (!allowZeroInDivisorRange && rhsLow <= 0 && rhsHigh >= 0) return false;

  // The range rhsLow, rhsHigh may include 0. If it does, adjust or split the
  // range so it doesn't include 0, since integer division by 0 is meaningless
  // and should be handled by an exception when executed. In other words, this
  // method determines the range of the result assuming the divisor is valid.
  if (rhsLow == 0) rhsLow = 1;

  if (rhsHigh == 0) rhsHigh = -1;
  ...
  if (rhsLow < 0 && rhsHigh > 0) {
    if (isUnsafeDivide(lhsLow, -1) || isUnsafeDivide(lhsHigh, -1)) return false;
    int64_t computed[8];
  ...
  if (resultLow < rangeMin) resultLow = rangeMin;

  if (resultHigh > rangeMax) resultHigh = rangeMax;

  if (resultLow > resultHigh) return false;  // doesn't make sense

  return true;
}

TR::Node *cloneDivForDivideByZeroCheck(OMR::ValuePropagation *vp,
                                       TR::Node *divNode) {
...
TR::Node *constrainIdiv(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs) {
    if (lhs->asIntConst() && rhs->asIntConst()) {
      ...
      if (lhsConst == TR::getMinSigned<TR::Int32>() && rhsConst == -1 &&
        ...
        if (isUnsigned)
          constraint = TR::VPIntConst::create(
              vp, ((uint32_t)lhsConst / (uint32_t)rhsConst));
        else
      ...
      if (lhsInt && rhsInt) {
        ...
        if (isUnsigned) {
          ...
          range_min = 0;
          range_max = TR::getMaxSigned<
              TR::Int32>();  // we don't support unsigned range constraint yet.
                             // Only support upto TR::getMaxSigned<TR::Int32>()
                             // for unsigned
        } else {
        ...
        if (constrainIntegerDivisionRange(
                lhs1, lhs2, rhs1, rhs2, range_min, range_max, lo, hi,
  ...
  if (constraint) {
    if (constraint->asIntConst()) {
      // If constrainIntegerDivisionRange reduced the range to a constant, but
      // the original dividend contained 0 in its range, create a copy of the
      // original div, to be anchored under the original DIVCHK, so that the
      // constant value can be optimized further, but division by zero is still
      // caught
      TR::Node *newDiv = NULL;
      if (doesRangeContainZero(rhs->getLowInt(), rhs->getHighInt()))
        newDiv = cloneDivForDivideByZeroCheck(vp, node);
      vp->replaceByConstant(node, constraint, lhsGlobal);
      if (newDiv != NULL) return newDiv;
      return node;
    }
    vp->addBlockOrGlobalConstraint(node, constraint, lhsGlobal);
  }
...
TR::Node *constrainLdiv(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
    ...
    if (lhsConst == TR::getMinSigned<TR::Int64>() && rhsConst == -1 &&
      ...
      if (isUnsigned)
        constraint = TR::VPLongConst::create(
            vp, ((uint64_t)lhsConst / (uint64_t)rhsConst));
      else
        constraint = TR::VPLongConst::create(
            vp, TR::Compiler->arith.longDivideLong(lhsConst, rhsConst));
    }
    if (constraint) {
      vp->replaceByConstant(node, constraint, lhsGlobal);
    }
    ...
    if (vp->lastTimeThrough() && !isUnsigned && lhsConstraint &&
        ...
        (lhsConstraint->getHigh() <=
         ((int64_t)TR::getMaxSigned<TR::Int32>())) &&
        rhsConstraint &&
        (rhsConstraint->getLow() >= ((int64_t)TR::getMinSigned<TR::Int32>())) &&
        (rhsConstraint->getHigh() <=
         ((int64_t)TR::getMaxSigned<TR::Int32>())) &&
        ((lhsConstraint->getLow() > TR::getMinSigned<TR::Int32>()) ||
         (rhsConstraint->getLow() > -1) || (rhsConstraint->getHigh() < -1)) &&
        performTransformation(vp->comp(),
                              "%sChange node [" POINTER_PRINTF_FORMAT
                              "] ldiv->i2l of idiv\n",
                              OPT_DETAILS, node)) {
      ...
      if (constrainIntegerDivisionRange(
              lhs1, lhs2, rhs1, rhs2, TR::getMinSigned<TR::Int32>(),
              TR::getMaxSigned<TR::Int32>(), lo, hi,
              vp->computeDivRangeWhenDivisorCanBeZero(newIdivNode)))
        idivConstraint = TR::VPIntRange::create(vp, (int32_t)lo, (int32_t)hi);
      if (idivConstraint) {
        // If constrainIntegerDivisionRange reduced the range to a constant, but
        // the original divisor contained 0 in its range, create a copy of the
        // original div, to be anchored under the original DIVCHK, so that the
        // constant value can be optimized further, but division by zero is
        // still caught
        ...
        vp->addBlockOrGlobalConstraint(
            node, TR::VPLongRange::create(vp, lo, hi), lhsGlobal);
      } else {
        // Even with no constraint on idiv, the result of i2l is in int range.
        TR::VPConstraint *i2lConstraint = TR::VPLongRange::create(
            vp, TR::getMinSigned<TR::Int32>(), TR::getMaxSigned<TR::Int32>());
        vp->addBlockOrGlobalConstraint(node, i2lConstraint, lhsGlobal);
      }
    } else if (!isUnsigned) {
      if (lhsConstraint && rhsConstraint) {
        int64_t lhs1 = lhsConstraint->getLowLong(),
                lhs2 = lhsConstraint->getHighLong();
        int64_t rhs1 = rhsConstraint->getLowLong(),
                rhs2 = rhsConstraint->getHighLong();
        int64_t lo, hi;
        if (constrainIntegerDivisionRange(
                lhs1, lhs2, rhs1, rhs2, TR::getMinSigned<TR::Int64>(),
                TR::getMaxSigned<TR::Int64>(), lo, hi,
                vp->computeDivRangeWhenDivisorCanBeZero(node))) {
          TR::VPConstraint *constraint = TR::VPLongRange::create(vp, lo, hi);
          if (constraint) {
            // If constrainIntegerDivisionRange reduced the range to a constant,
            // but the original divisor contained 0 in its range, create a copy
            // of the original div, to be anchored under the original DIVCHK, so
            // that the constant value can be optimized further, but division by
      ...
      if (rhsLow >= 1 && rhsLow <= rhsHigh && lhsLow <= lhsHigh) {
        ...
        vp->addBlockOrGlobalConstraint(
            node,
            TR::VPLongRange::create(vp, lhsLow / rhsHigh, lhsHigh / rhsLow),
            lhsGlobal);
      }
    }
  }
...
TR::Node *removeRedundantREM(OMR::ValuePropagation *vp, TR::Node *node,
                             TR::VPConstraint *nodeConstraint,
                             TR::VPConstraint *firstChildConstraint,
                             TR::VPConstraint *secondChildConstraint) {
  if (!(node->getOpCode().isRem() && node->getType().isIntegral())) return NULL;
  ...
  if (secondChildConstraint->asIntConst() &&
      isPositivePowerOfTen(secondChildConstraint->getLowInt())) {
    powerOfTen = true;
    constValue = secondChildConstraint->getLowInt();
  } else if (secondChildConstraint->asLongConst() &&
             isPositivePowerOfTen(secondChildConstraint->getLowLong())) {
    powerOfTen = true;
  ...
  if (powerOfTen) {
    bool isUnsigned = node->getOpCode().isUnsigned();
    // the max value for a remainder result is the divisor value - 1 so when the
    // divisor is a power of ten then -1 this value is one decimal precision
    // value less (e.g. 100 - 1 = 99)
    int32_t refinedPrecision = trailingZeroes((uint64_t)constValue);
    if (!isUnsigned && fcPrecision <= refinedPrecision &&
        performTransformation(vp->comp(),
                              "%sRemove %s [0x%p] as child %s [0x%p] prec %lld "
                              "<= divisor max prec %d (value %lld)\n",
                              OPT_DETAILS, node->getOpCode().getName(), node,
                              node->getFirstChild()->getOpCode().getName(),
                              node->getFirstChild(), fcPrecision,
                              refinedPrecision, constValue)) {
      // in this case based on the child prec and the divisor constant value
      // that the irem is not needed (999 < 1000) so can remove the irem
      // completely irem
      //    child prec=3 (i.e. max value is 999)
      //    iconst 1000
      //
      // this doesn't work for unsigned (iurem) as these ops treat the operands
      // as unsigned so, for example, a prec=1 dividend that is negative when
      // interpreted as unsigned will actually have 10 digits of precision (-1
      // would be 0xFFffFFff = 4,294,967,295)
      //
...
TR::Node *constrainIrem(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && lhs->asIntConst() && rhs && rhs->asIntConst()) {
    ...
    if (constraint) vp->replaceByConstant(node, constraint, lhsGlobal);
    ...
    int32_t rhsConst = rhs->asIntConst()->getInt();
    if (!isUnsigned && rhsConst < 0) rhsConst *= -1;
  ...
  if (constraint && lhs && lhs->asIntConstraint() && rhs &&
      rhs->asIntConstraint()) {
    TR::Node *newNode = removeRedundantREM(vp, node, constraint, lhs, rhs);
    if (NULL != newNode) node = newNode;
  }
...
TR::Node *constrainLrem(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
    ...
    if (lhsConst == TR::getMinSigned<TR::Int64>() && rhsConst == -1)
      ...
      constraint = TR::VPLongConst::create(
          vp, TR::Compiler->arith.longRemainderLong(lhsConst, rhsConst));
    if (constraint) vp->replaceByConstant(node, constraint, lhsGlobal);
  ...
  if (constraint && lhs && lhs->asLongConstraint() && rhs &&
      rhs->asLongConstraint()) {
    TR::Node *newNode = removeRedundantREM(vp, node, constraint, lhs, rhs);
    if (NULL != newNode) node = newNode;
  }
...
TR::Node *constrainIneg(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (child) {
    if (child->asIntConst()) {
      TR::VPConstraint *constraint =
          TR::VPIntConst::create(vp, -child->asIntConst()->getInt());
      vp->replaceByConstant(node, constraint, isGlobal);
    } else {
      ...
      if (low == TR::getMinSigned<TR::Int32>()) {
        lowConstraint =
            TR::VPIntRange::create(vp, TR::getMinSigned<TR::Int32>(),
                                   TR::getMinSigned<TR::Int32>(), TR_yes);
        low++;
      }
      ...
      if (!highConstraint) {
        ...
        if (lowConstraint)
          constraint =
              TR::VPMergedConstraints::create(vp, lowConstraint, constraint);
      } else
...
TR::Node *constrainLneg(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (child) {
    if (child->asLongConst()) {
      TR::VPConstraint *constraint =
          TR::VPLongConst::create(vp, -child->asLongConst()->getLong());
      vp->replaceByConstant(node, constraint, isGlobal);
    } else {
      ...
      if (low == TR::getMinSigned<TR::Int64>()) {
        lowConstraint =
            TR::VPLongRange::create(vp, TR::getMinSigned<TR::Int64>(),
                                    TR::getMinSigned<TR::Int64>(), TR_yes);
        low++;
      }
  ...
  if (child == NULL) {
    vp->addGlobalConstraint(
        ...
        TR::VPMergedConstraints::create(
            vp, TR::VPIntConst::create(vp, TR::getMinSigned<TR::Int32>()),
            TR::VPIntRange::create(vp, 0, TR::getMaxSigned<TR::Int32>())));
  } else {
    int32_t low = child->getLowInt();
    int32_t high = child->getHighInt();
    if (low == high) {
      int32_t value = low;
      if (value < 0) value = -(uint32_t)value;
      ...
      if (low < 0 && high <= 0) {
        ...
        low = 0;
                     ...
                     vp->comp(),
                     "%sRemoving %s [0x%p] as child %s [0x%p] is known to be "
                     "positive\n",
                     OPT_DETAILS, node->getOpCode().getName(), node,
                     node->getFirstChild()->getOpCode().getName(),
                     node->getFirstChild())) {
        // The range of values for the child is entirely positive, so remove the
        // iabs
        return vp->replaceNode(node, node->getFirstChild(), vp->_curTree);
      }
      ...
      if (low != high || minConstraint != NULL) {
        ...
        if (minConstraint != NULL)
          constraint =
              TR::VPMergedConstraints::create(vp, minConstraint, constraint);
        vp->addBlockOrGlobalConstraint(node, constraint, isGlobal);
      } else {
...
TR::Node *constrainLabs(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (child == NULL) {
    vp->addGlobalConstraint(
        node,
        TR::VPMergedConstraints::create(
            vp, TR::VPLongConst::create(vp, TR::getMinSigned<TR::Int64>()),
            TR::VPLongRange::create(vp, 0, TR::getMaxSigned<TR::Int64>())));
  } else {
    int64_t low = child->getLowLong();
    int64_t high = child->getHighLong();
    if (low == high) {
      int64_t value = low;
      if (value < 0) value = -(uint64_t)value;
...
TR::Node *constrainIshl(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && lhs->asIntConst() && rhs && rhs->asIntConst()) {
    int32_t lhsConst = lhs->asIntConst()->getInt();
    int32_t rhsConst = rhs->asIntConst()->getInt();
    TR::VPConstraint *constraint = TR::VPIntConst::create(
        vp, lhsConst << (rhsConst & 0x01F) /*, isUnsigned*/);
    vp->replaceByConstant(node, constraint, lhsGlobal);
  }
...
TR::Node *constrainLshl(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
    ...
    int64_t rhsConst = rhs->asLongConst()->getLong();
    TR::VPConstraint *constraint =
        TR::VPLongConst::create(vp, lhsConst << (rhsConst & 0x03F));
    vp->replaceByConstant(node, constraint, lhsGlobal);
  }

  if (lhs && lhs->asLongConst() && lhs->asLongConst()->getLong() == 1) {
    TR::VPConstraint *constraint = TR::VPLongRange::create(
        vp, TR::getMinSigned<TR::Int64>(), TR::getMaxSigned<TR::Int64>(), true);
    vp->addBlockConstraint(node, constraint);
  }
...
static TR::Node *distributeShift(OMR::ValuePropagation *vp, TR::Node *node,
                                 int32_t shiftAmount,
                                 TR::VPConstraint *&constraint) {
  ...
  if ((nodeOp.isAdd() || nodeOp.isSub()) &&
      node->getFirstChild()->cannotOverflow()) {
    TR::Node *lhsGrandChild = node->getFirstChild()->getFirstChild();
    TR::Node *rhsGrandChild = node->getFirstChild()->getSecondChild();
    bool isGlobal;
    TR::VPConstraint *lhsGConstraint =
        vp->getConstraint(lhsGrandChild, isGlobal);
    TR::VPConstraint *rhsGConstraint =
        vp->getConstraint(rhsGrandChild, isGlobal);
    ...
    if (replaceNode) {
      ...
      if (lhsORrhs) {
        ...
        if ((low <= 0) && (((int64_t)(high + 1) & (pow2Val - 1))) == 0) {
          if (vp->trace())
            traceMsg(vp->comp(),
                     "found opportunity to distribute shift in node [%p] "
                     "replaceNode [%p]\n",
                     node, replaceNode);
        } else {
          if (vp->trace())
            traceMsg(vp->comp(),
                     "failed additive expr constraint is not within range, "
                     "node [%p] replaceNode [%p]\n",
                     node, replaceNode);
          replaceNode = NULL;
        }
      } else {
        if (vp->trace())
          traceMsg(vp->comp(),
                   "failed no constraint found, node [%p] replaceNode [%p]\n",
                   node, replaceNode);
        replaceNode = NULL;
      }
    }
    ...
    if (replaceNode &&
        performTransformation(vp->comp(),
                              "%sDistributing shift across add/sub [%p]\n",
        ...
        if (node->getOpCodeValue() == TR::iushr) {
          if (c->getLowInt() >= 0)
            constraint = TR::VPIntRange::create(
                vp, ((uint32_t)c->getLowInt()) >> shiftAmount,
                ((uint32_t)c->getHighInt()) >> shiftAmount);
          else if (c->getHighInt() < 0)
            constraint = TR::VPIntRange::create(
                vp, ((uint32_t)c->getHighInt()) >> shiftAmount,
                ((uint32_t)c->getLowInt()) >> shiftAmount);
          // this path is probably never taken for unsigned
          else {
            if (shiftAmount > 0)
              constraint = TR::VPIntRange::create(
                  vp, 0, (uint32_t)0xffffffff >> shiftAmount);
            else
              constraint =
                  TR::VPIntRange::create(vp, 0, TR::getMaxSigned<TR::Int32>());
          }
        } else
          constraint = TR::VPIntRange::create(vp, c->getLowInt() >> shiftAmount,
                                              c->getHighInt() >> shiftAmount);
      }
    }
  }
  return replaceNode;
...
TR::Node *constrainIshr(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    TR::VPConstraint *constraint =
        TR::VPIntRange::create(vp, low >> shiftAmount, high >> shiftAmount);
  ...
  TR::Node *firstChild = node->getFirstChild();
  if (firstChild->isNonNegative() /*&& firstChild->getType().isSignedInt()*/ &&
      vp->lastTimeThrough() &&
      performTransformation(
    ...
    // node->setNumChildren(1);
    // TR::Node *newIushrNode = TR::Node::create(vp->comp(), TR::iushr, 2, lhs,
    // rhs); node->setAndIncChild(0, newIushrNode); lhs->decReferenceCount();
    // rhs->decReferenceCount();
  }
  // this handler is not called for unsigned shifts
  //#ifdef DEBUG
  // else if(!firstChild->getType().isSignedInt()) dumpOptDetails(vp->comp(),
  // "FIXME: Need to support ishr opt for Unsigned datatypes!\n");
  // FIXME
  //#endif
  return node;
}
...
TR::Node *constrainLshr(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    TR::VPConstraint *constraint =
        TR::VPLongRange::create(vp, low >> shiftAmount, high >> shiftAmount);
    if (constraint) {
      if (constraint->asLongConst()) {
        vp->replaceByConstant(node, constraint, lhsGlobal);
        return node;
      ...
      if (longShr) {
        ...
        // If the shift amount is between 32 and 63, inclusive, then if we
        // convert the node to an integer shift we must reduce this shift value.
        // Integer shifts are masked by 0x1f in java, and if the first child is
        // integer constrained, a shift by an amount in this range is effectively
        // just a shift by 31.
        if (didReduction && shiftAmount >= 32) {
          secondChild->decReferenceCount();
          node->getFirstChild()->setAndIncChild(
              1, TR::Node::create(node, TR::iconst, 0, 31));
        }
      }
...
TR::Node *constrainIushr(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (low == high)
      constraint = TR::VPIntConst::create(
          vp, ((uint32_t)low) >> shiftAmount /*, isUnsigned*/);
    else if (low >= 0)
      constraint = TR::VPIntRange::create(
          vp, ((uint32_t)low) >> shiftAmount,
          ((uint32_t)high) >> shiftAmount /*, isUnsigned*/);
    else if (high < 0 /*&& !isUnsigned*/)
      constraint = TR::VPIntRange::create(vp, ((uint32_t)high) >> shiftAmount,
                                          ((uint32_t)low) >> shiftAmount);
    // this path is probably never taken for unsigned
    else {
      if (shiftAmount > 0)
        constraint =
            TR::VPIntRange::create(vp, 0, (uint32_t)0xffffffff >> shiftAmount);
...
TR::Node *constrainLushr(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (low == high)
      ...
      constraint = TR::VPLongRange::create(vp, ((uint64_t)low) >> shiftAmount,
                                           ((uint64_t)high) >> shiftAmount);
    else if (high < 0)
      constraint = TR::VPLongRange::create(vp, ((uint64_t)high) >> shiftAmount,
                                           ((uint64_t)low) >> shiftAmount);
    else {
      if (shiftAmount > 0)
        constraint =
            TR::VPLongRange::create(vp, 0, ((uint64_t)-1L) >> shiftAmount);
      else
...
TR::Node *constrainIand(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (rhs && rhs->asIntConst()) {
    ...
    if (255 == mask && lhs)  // loading from byte
    {
      if (lhs->asIntRange()) {
        ...
        int32_t low = range->getLowInt();
        int32_t high = range->getHighInt();
        // bool withinRange = isUnsigned ? ((uint32_t)high <= 255) : (high <=
        // 255);
        if (low >= 0 && high <= 255 &&
          ...
          if (info &&
              (info->isDefIndex(useIndex) || info->isUseIndex(useIndex))) {
            if (info->getNode(useIndex) == node) info->clearNode(useIndex);
          }
    ...
    if (mask == 0) {
      ...
      // if (isUnsigned)
      //   constraint = TR::VPIntConst::create(vp,
      //   ((uint32_t)lhs->asIntConst()->getInt() & (uint32_t)mask),
      //   isUnsigned);
      // else
      constraint =
          TR::VPIntConst::create(vp, lhs->asIntConst()->getInt() & mask);
    } else {
      ...
      if (rhs && rhs->asIntConst()) {
        ...
        if ((firstChild->getOpCodeValue() == TR::iloadi ||
             firstChild->getOpCodeValue() == TR::lloadi) &&
            (firstChild->getSymbolReference() ==
             vp->comp()->getSymRefTab()->findClassAndDepthFlagsSymbolRef()) &&
            (rhs->getLowInt() ==
             TR::Compiler->cls.flagValueForArrayCheck(vp->comp()))) {
          if (vp->trace())
          ...
          if (vftLoad->getOpCodeValue() == TR::aloadi ||
              vftLoad->getOpCodeValue() == TR::loadaddr) {
            TR::Node *baseExpression = NULL;
            ...
            if (vp->trace())
              traceMsg(vp->comp(),
                       "Base expression node for isArray test is %p\n",
                       baseExpression);

            bool baseExpressionGlobal;
            TR::VPConstraint *baseExpressionConstraint =
                vp->getConstraint(baseExpression, baseExpressionGlobal);
            if (baseExpressionConstraint &&
                baseExpressionConstraint->getClassType() &&
                (baseExpressionConstraint->getClassType()->isArray() !=
                 TR_maybe)) {
              if (vp->trace())
              ...
              if (baseExpressionConstraint->getClassType()->isArray() == TR_yes)
                constraint =
                    TR::VPIntConst::create(vp, rhs->asIntConst()->getLowInt());
              else
                constraint = TR::VPIntConst::create(vp, 0);

              TR::DebugCounter::incStaticDebugCounter(
                  vp->comp(), TR::DebugCounter::debugCounterName(
                                  vp->comp(), "isArrayTest/hit/(%s)/%s",
                                  vp->comp()->signature(),
                                  vp->comp()->getHotnessName(
                                      vp->comp()->getMethodHotness())));
            } else {
              TR::DebugCounter::incStaticDebugCounter(
                  vp->comp(), TR::DebugCounter::debugCounterName(
                                  vp->comp(), "isArrayTest/miss/(%s)/%s",
                                  vp->comp()->signature(),
                                  vp->comp()->getHotnessName(
                                      vp->comp()->getMethodHotness())));
            }
            ...
            TR::DebugCounter::incStaticDebugCounter(
                vp->comp(), TR::DebugCounter::debugCounterName(
                                vp->comp(), "isArrayTest/unsuitable/(%s)/%s",
                                vp->comp()->signature(),
                                vp->comp()->getHotnessName(
                                    vp->comp()->getMethodHotness())));
          }
      ...
      if (!constraint) {
        if (mask != 0xffffffff && isNonNegativePowerOf2(mask + 1)) {
          ...
          if (lhs && lhs->getLowInt() >= 0 && lhs->getHighInt() <= mask) {
            ...
            if (vp->trace())
              traceMsg(vp->comp(),
                       "Removing redundant iand [%p] due to range\n", node);

            if (lhs->getLowInt() > 0) low = lhs->getLowInt();
            if (lhs->getHighInt() >= 0) high = lhs->getHighInt();
          }
          ...
          // if (isUnsigned)
          //   constraint = TR::VPIntRange::create(vp,
          //   TR::getMinUnsigned<TR::Int32>(), mask, isUnsigned);
          // else
          constraint =
              TR::VPIntRange::create(vp, TR::getMinSigned<TR::Int32>(),
                                     mask & TR::getMaxSigned<TR::Int32>());
        }
      }
    }
  } else {
    if (lhs && lhs->getLowInt() > 0)
      constraint =
          TR::VPIntRange::create(vp, 0, lhs->getHighInt() /*, isUnsigned*/);
  }

  // Evaluate to zero optimization: lhs will atleast contain the same number of
  // trailng zeros as lrhs. When lhs & rhs, and rhs < 0b1000..(number of trailing
  // zeros in lhs), then the whole iand node evaluates to a constant zero. The
  // Opt doesn't kick in when rhs is negative, because of MSB being 1.

  bool lrhsGlobal;
  TR::VPConstraint *lrhs = NULL;
  if (node->getFirstChild()->getNumChildren() > 1)
    lrhs =
        vp->getConstraint(node->getFirstChild()->getSecondChild(), lrhsGlobal);
  if (rhs && lrhs && lrhs->asIntConst()) {
    ...
    if (rhs->asIntRange()) {
      ...
      if (node->getFirstChild()->getOpCodeValue() == TR::imul &&
          ((high & 0x80000000) == 0) && ((low & 0x80000000) == 0)) {
        while (!(mask & 0x1)) {
          shift++;
          mask >>= 1;
        }
        if (high < 1 << shift) constraint = TR::VPIntConst::create(vp, 0);
      } else if (node->getFirstChild()->getOpCodeValue() == TR::ishl &&
                 ((high & 0x80000000) == 0) && ((low & 0x80000000) == 0)) {
        if (high < 1 << mask) constraint = TR::VPIntConst::create(vp, 0);
      }
    } else if (rhs->asIntConst()) {
      ...
      if (node->getFirstChild()->getOpCodeValue() == TR::imul &&
        ...
        if (iandMask < 1 << shift) constraint = TR::VPIntConst::create(vp, 0);
      } else if (node->getFirstChild()->getOpCodeValue() == TR::ishl &&
                 ((iandMask & 0x80000000) == 0)) {
        if (iandMask < 1 << mask) constraint = TR::VPIntConst::create(vp, 0);
      }
    }
  }
  ...
  if (!constraint && (lhs || rhs) /* && !isUnsigned*/) {
    int32_t lhsHigh = TR::getMaxSigned<TR::Int32>(),
            rhsHigh = TR::getMaxSigned<TR::Int32>();
    int32_t lhsLow = TR::getMinSigned<TR::Int32>(),
            rhsLow = TR::getMinSigned<TR::Int32>();
    int32_t high, low;
    ...
    if (rhsLow >= 0 || lhsLow >= 0) {
      if (rhsLow < 0)  // punt
      {
        ...
        high = rhsHigh < lhsHigh ? rhsHigh
                                 : lhsHigh;  // intersection, so will be smaller
      }
      if (low > 0) low = 0;
  ...
  if(TR::ishr == firstChild->getOpCodeValue() &&
    ...
    TR::VPConstraint *shiftConstraint =
        vp->getConstraint(shiftAmount, isGlobal);
    ...
    if ((bitsShifted < (int32_t)signBitsMaskedOff) && vp->lastTimeThrough() &&
        performTransformation(vp->comp(),
                              "%s Node [" POINTER_PRINTF_FORMAT
                              "]: ishr -> iushr (parent ignores sign bits)\n",
      ...
      // TR::Node *newIushrNode = TR::Node::create(vp->comp(), TR::iushr, 2,
      // lhs, rhs); firstChild->setAndIncChild(0, newIushrNode);
...
TR::Node *constrainLand(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (rhs && rhs->asLongConst()) {
    ...
    if (mask == 0L) {
      ...
      constraint =
          TR::VPLongConst::create(vp, lhs->asLongConst()->getLong() & mask);
    } else {
      // look for mod by power of 2 case
      if (mask != -1L && isNonNegativePowerOf2(mask + 1)) {
        ...
        if (lhs && lhs->getLowLong() >= 0 && lhs->getHighLong() <= mask) {
          if (lhs->getLowLong() > 0) low = lhs->getLowLong();
          if (lhs->getHighLong() >= 0) high = lhs->getHighLong();
        }
        constraint = TR::VPLongRange::create(vp, low, high);
...
TR::Node *constrainIor(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && lhs->asIntConst() && rhs && rhs->asIntConst()) {
    ...
    TR::VPConstraint *constraint = NULL;
    // if (isUnsigned)
    //   constraint = TR::VPIntConst::create(vp, ((uint32_t)lhsConst |
    //   (uint32_t)rhsConst), isUnsigned);
    // else
    constraint =
        TR::VPIntConst::create(vp, lhsConst | rhsConst /*, isUnsigned*/);
    vp->replaceByConstant(node, constraint, lhsGlobal);
  }
...
TR::Node *constrainLor(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && lhs->asLongConst() && rhs && rhs->asLongConst()) {
    ...
    int64_t rhsConst = rhs->asLongConst()->getLong();
    TR::VPConstraint *constraint =
        TR::VPLongConst::create(vp, lhsConst | rhsConst);
    vp->replaceByConstant(node, constraint, lhsGlobal);
  }
...
TR::Node *constrainIxor(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (lhs && rhs && rhs->asIntConst()) {
    ...
    if (rhsConst == 1 && lhs->asIntRange()) {
      ...
      //
      constraint =
          TR::VPIntRange::create(vp, (lhs->getLowInt() & ~1),
                                 (lhs->getHighInt() | 1) /*, isUnsigned*/);
      if (constraint) {
        vp->addBlockOrGlobalConstraint(node, constraint, lhsGlobal);
      }
    }
...
static TR::Node *constrainWidenToLong(OMR::ValuePropagation *vp, TR::Node *node,
  ...
  if (constraint) {
    if (constraint->asIntConstraint()) {
      if (isUnsigned) {
        if (constraint->getLowInt() > 0) low = constraint->getLowInt();
        if (constraint->getLowInt() >= 0 && constraint->getHighInt() < high)
          high = constraint->getHighInt();
      } else {
        if (constraint->getLowInt() > low) low = constraint->getLowInt();
        if (constraint->getHighInt() < high) high = constraint->getHighInt();
      }
    } else if (constraint->asShortConstraint()) {
      if (isUnsigned) {
        if (constraint->getLowShort() > 0) low = constraint->getLowShort();
        if (constraint->getLowShort() > 0 && constraint->getHighShort() < high)
          high = constraint->getHighShort();
      } else {
        if (constraint->getLowShort() > low) low = constraint->getLowShort();
...
TR::Node *constrainI2l(OMR::ValuePropagation *vp, TR::Node *node) {
  if (node->getFirstChild()->isNonNegative()) node->setIsNonNegative(true);

  return constrainWidenToLong(vp, node, TR::getMinSigned<TR::Int32>(),
                              TR::getMaxSigned<TR::Int32>(), false);
}

TR::Node *constrainIu2l(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainWidenToLong(vp, node, TR::getMinUnsigned<TR::Int32>(),
...
void replaceWithSmallerType(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (!load->getOpCode().isLoadVar() || load->getReferenceCount() > 1 ||
      !oldRef->getSymbol()->isAuto() || oldRef->getUseDefAliases().hasAliases())
    return;

  TR_UseDefInfo *useDefInfo = vp->_useDefInfo;

  if (!useDefInfo) return;

  uint16_t useIndex = load->getUseDefIndex();

  if (!useIndex || !useDefInfo->isUseIndex(useIndex)) return;
  ...
  if (performTransformation(comp,
    ...
    TR::SymbolReference *symRef = comp->getSymRefTab()->createTemporary(
        vp->comp()->getMethodSymbol(), newType);
    defNode->setSymbolReference(symRef);
    node->setSymbolReference(symRef);
  }
}

static TR::Node *constrainNarrowIntValue(OMR::ValuePropagation *vp,
                                         TR::Node *node, int32_t low,
                                         int32_t high) {
  if (findConstant(vp, node)) return node;
  constrainChildren(vp, node);
  bool isGlobal;
    ...
    if (constraint->asIntConstraint() || constraint->asMergedIntConstraints()) {
      if (constraint->getLowInt() > low && constraint->getHighInt() < high) {
        ...
        if (constraint->getHighInt() < high) high = constraint->getHighInt();
               ...
               constraint->asMergedLongConstraints()) {
      ...
      if (vp->comp()->getOptions()->getDebugEnableFlag(
              TR_EnableUnneededNarrowIntConversion) &&
  ...
  if (low <= high) {
    // Make sure the constraint is not trivial
    //
    int64_t min = TR::getMinSigned<TR::Int32>(),
            max = TR::getMaxSigned<TR::Int32>();
    switch (node->getDataType()) {
      case TR::Int16:
        constraint = TR::VPShortRange::create(vp, low, high);
        min = TR::getMinSigned<TR::Int16>();
        max = TR::getMaxSigned<TR::Int16>();
        break;
      case TR::Int8:
        min = TR::getMinSigned<TR::Int8>();
        max = TR::getMaxSigned<TR::Int8>();  // byte nodes are still using Int
                                             // constraint
      case TR::Int32:
        constraint = TR::VPIntRange::create(vp, low, high);
        break;
      default:
        TR_ASSERT(false, "Invalid node datatype");
    }
    if (constraint) {
      vp->addBlockOrGlobalConstraint(node, constraint, isGlobal);
    }
    if (low >= 0) node->setIsNonNegative(true);
    if ((low > min) || (high < max)) node->setCannotOverflow(true);
  }
...
TR::Node *constrainNarrowToByte(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainNarrowIntValue(vp, node, TR::getMinSigned<TR::Int8>(),
                                 TR::getMaxSigned<TR::Int8>());
}

TR::Node *constrainNarrowToShort(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainNarrowIntValue(vp, node, TR::getMinSigned<TR::Int16>(),
                                 TR::getMaxSigned<TR::Int16>());
}
...
TR::Node *constrainNarrowToChar(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (constraint) {
    if (constraint->asIntConstraint() || constraint->asMergedConstraints()) {
      ...
      if (constraint->getLowInt() < 0 &&
          constraint->getLowInt() == constraint->getHighInt())
        low = high = max + constraint->getLowInt() %
                               (TR::getMaxUnsigned<TR::Int16>() + 1);
    } else if (constraint->asLongConstraint() ||
               constraint->asMergedLongConstraints()) {
      if (constraint->getLowInt() > 0 &&
          constraint->getLowInt() < TR::getMaxUnsigned<TR::Int16>() &&
      ...
      if (constraint->getLowLong() < 0 &&
          constraint->getLowLong() == constraint->getHighLong())
        low = high = max + (int32_t)constraint->getLowLong() %
                               (TR::getMaxUnsigned<TR::Int16>() + 1);
    }
  }
...
TR::Node *constrainNarrowToInt(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainNarrowIntValue(vp, node, TR::getMinSigned<TR::Int32>(),
                                 TR::getMaxSigned<TR::Int32>());
}

/*
static int32_t findConstantValue(OMR::ValuePropagation *vp, TR::Node *node,
TR::VPConstraint *constraint)
   {
...
static bool constrainWidenToInt(OMR::ValuePropagation *vp, TR::Node *&node,
                                int32_t low, int32_t high, bool isUnsigned,
                                TR::ILOpCodes correspondingNarrowingOpCode) {
  if (findConstant(vp, node)) return true;
  ...
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *grandChild =
      firstChild->getNumChildren() > 0 ? firstChild->getFirstChild() : 0;
  ...
  */
  ...
  if (firstChild->getOpCodeValue() == correspondingNarrowingOpCode) {
    preNarrowingConstraint =
        vp->getConstraint(firstChild->getFirstChild(), isGlobal);
  ...
  if (  // p &&
    ...
    // printf("found one (sign extension conversion removal) in method %s, node
    // %p, low was %d, high was %d\n", vp->comp()->signature(), grandChild,
    // preNarrowingConstraint->getLowInt(),
  ...
  if (constraint) {
    if (isUnsigned) {
      // Checks below to adjust high have been changed to >= (they were
      // previously just >).
      //
      // The check for constraint->getLowInt() >= 0 is still needed. If the
      // number was defined as PIC S999 (3 digits, signed), the incoming range
      // would be [-999, 999]. Widened to an unsigned integer, this gives the
      // disjoint range [0, 999], [64537, 65535], which, when merged into one
      // constraint, would be [0, 65535]. In this case, low and high should
      // already be 0 and 65535, so we don't want to adjust anything based on
...
TR::Node *constrainB2i(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToInt(vp, node, TR::getMinSigned<TR::Int8>(),
                      TR::getMaxSigned<TR::Int8>(), false, TR::i2b);
  return node;
}

TR::Node *constrainB2s(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToInt(vp, node, TR::getMinSigned<TR::Int8>(),
                      TR::getMaxSigned<TR::Int8>(), false, TR::s2b);
  return node;
}

TR::Node *constrainBu2i(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToInt(vp, node, TR::getMinUnsigned<TR::Int8>(),
                      TR::getMaxUnsigned<TR::Int8>(), true, TR::i2b);
  return node;
}

TR::Node *constrainBu2s(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToInt(vp, node, TR::getMinUnsigned<TR::Int8>(),
                      TR::getMaxUnsigned<TR::Int8>(), true, TR::s2b);
  return node;
}

TR::Node *constrainB2l(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToLong(vp, node, TR::getMinSigned<TR::Int8>(),
                       TR::getMaxSigned<TR::Int8>(), false);
  return node;
}

TR::Node *constrainBu2l(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToLong(vp, node, TR::getMinUnsigned<TR::Int8>(),
                       TR::getMaxUnsigned<TR::Int8>(), true);
  return node;
}

TR::Node *constrainS2i(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToInt(vp, node, TR::getMinSigned<TR::Int16>(),
                      TR::getMaxSigned<TR::Int16>(), false, TR::i2s);
  return node;
}

TR::Node *constrainSu2i(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToInt(vp, node, TR::getMinUnsigned<TR::Int16>(),
                      TR::getMaxUnsigned<TR::Int16>(), true, TR::i2s);
  return node;
}

TR::Node *constrainS2l(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToLong(vp, node, TR::getMinSigned<TR::Int16>(),
                       TR::getMaxSigned<TR::Int16>(), false);
  return node;
}

TR::Node *constrainSu2l(OMR::ValuePropagation *vp, TR::Node *node) {
  constrainWidenToLong(vp, node, TR::getMinUnsigned<TR::Int16>(),
                       TR::getMaxUnsigned<TR::Int16>(), true);
  return node;
}

static void changeConditionalToGoto(OMR::ValuePropagation *vp, TR::Node *node,
                                    TR::CFGEdge *branchEdge) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  TR::Block *fallThrough =
      vp->_curBlock->getExit()->getNextTreeTop()->getNode()->getBlock();
  TR::CFGEdge *edge =
      vp->findOutEdge(vp->_curBlock->getSuccessors(), fallThrough);
  TR::Block *target = node->getBranchDestination()->getNode()->getBlock();
  if (fallThrough != target) vp->_edgesToBeRemoved->add(edge);
  vp->printEdgeConstraints(vp->createEdgeConstraints(edge, true));
}

static void removeConditionalBranch(OMR::ValuePropagation *vp, TR::Node *node,
                                    TR::CFGEdge *branchEdge) {
#ifdef J9_PROJECT_SPECIFIC
  createGuardSiteForRemovedGuard(vp->comp(), node);
#endif
  ...
  TR::Block *fallThrough =
      vp->_curBlock->getExit()->getNextTreeTop()->getNode()->getBlock();
  TR::Block *target = node->getBranchDestination()->getNode()->getBlock();
  if (fallThrough != target) vp->_edgesToBeRemoved->add(branchEdge);
}
...
static TR::VPConstraint *passingTypeTestObjectConstraint(
    OMR::ValuePropagation *vp, TR::VPConstraint *classConstraint,
  ...
  if (constrainVft) {
    ...
    if (sig != NULL && sigLen == 17 &&
        !strncmp(sig, "Ljava/lang/Class;", sigLen)) {
  ...
  if (vp->trace()) {
    traceMsg(vp->comp(),
             "passingTypeTestObjectConstraint returning constraint: ");
...
static TR::Node *getOriginalCallNode(TR_VirtualGuard *vGuard,
                                     TR::Compilation *comp,
                                     TR::Node *guardNode) {
  TR::Block *block = guardNode->getBranchDestination()->getNode()->getBlock();
  TR_ByteCodeInfo &bci =
      comp->getInlinedCallSite(vGuard->getCalleeIndex())._byteCodeInfo;

  for (TR::TreeTop *tt = block->getEntry(); tt != block->getExit();
    ...
    if (tt->getNode()->getNumChildren()) {
      ...
      if (possiblyCall->getOpCode().isCall() &&
          ...
          possiblyCall->getByteCodeInfo().getCallerIndex() ==
              bci.getCallerIndex() &&
...
static int numConcreteClasses(List<TR_PersistentClassInfo> *subClasses) {
  ...
  for (TR_PersistentClassInfo *ptClassInfo = i.getFirst(); ptClassInfo;
       ptClassInfo = i.getNext()) {
...
static void addDelayedConvertedGuard(TR::Node *node, TR::Node *callNode,
  ...
  if (!callNode->getFirstChild()->getOpCode().hasSymbolReference() ||
      callNode->getFirstChild()->getSymbolReference() !=
          vp->comp()->getSymRefTab()->findVftSymbolRef() ||
      !callNode->getFirstChild()
           ->getFirstChild()
           ->getOpCode()
           .isLoadVarDirect() ||  // I guess, having a vft symbol implies that
                                  // there is something underneath it?
      !callNode->getSecondChild()
           ->getOpCode()
           .isLoadVarDirect() ||  // it also implies, there should be a receiver
                                  // argument?
      !callNode->getSecondChild()
           ->getOpCode()
           .hasSymbolReference() ||  // I think its safer to exclude statics
      !callNode->getSecondChild()
           ->getSymbolReference()
           ->getSymbol()
           ->isAutoOrParm()) {
  ...
  TR::Node *newReceiver = TR::Node::createLoad(
      callNode, callNode->getSecondChild()->getSymbolReference());
  TR::Node *newGuardNode = TR_VirtualGuard::createMethodGuardWithReceiver(
      guardKind, vp->comp(), oldVirtualGuard->getCalleeIndex(), callNode,
      node->getBranchDestination(), methodSymbol,
      objectClass /*oldVirtualGuard->getThisClass()*/, newReceiver);
  ...
  // do not add a new guard yet ... it will be added in doDelayedTransformation
  // and we will fix the IL accordingly
  vp->comp()->removeVirtualGuard(newGuard);
  // finish the rest of a transformation in doDelayedTransformation
  vp->_convertedGuards.add(
      new (vp->trStackMemory()) OMR::ValuePropagation::VirtualGuardInfo(
          vp, oldVirtualGuard, newGuard, newGuardNode, callNode));
}

#ifdef J9_PROJECT_SPECIFIC
TR_ResolvedMethod *findSingleImplementer(
    TR_OpaqueClassBlock *thisClass, int32_t cpIndexOrVftSlot,
  ...
  TR_PersistentClassInfo *classInfo =
      comp->getPersistentInfo()
          ->getPersistentCHTable()
          ->findClassInfoAfterLocking(thisClass, comp, true);
  if (!classInfo) {
    return 0;
  }

  TR_ResolvedMethod *implArray[2];  // collect maximum 2 implemeters if you can
  int32_t implCount = TR_ClassQueries::collectImplementorsCapped(
      classInfo, implArray, 2, cpIndexOrVftSlot, callerMethod, comp, locked,
...
//
static TR::Node *constrainIfcmpeqne(OMR::ValuePropagation *vp, TR::Node *node,
                                    bool branchOnEqual) {
  constrainChildren(vp, node);
  ...
  if (fallThrough == target) return node;
  ...
  if (rhsChild->getOpCodeValue() == TR::iconst &&
    ...
    if (rhsConst == 0 || rhsConst == 1) {
      ...
      if (lhsOp.isInt() || lhsOp.isLong() || lhsOp.isRef()) {
        ...
        if (performTransformation(
                vp->comp(), "%sChanging n%un (%s (%s ...) %d) to (%s ...)\n",
                OPT_DETAILS, node->getGlobalIndex(),
                node->getOpCode().getName(), lhsCmp.getName(), rhsConst,
                newIfOp.getName())) {
  ...
  if (((node->getOpCodeValue() == TR::ifacmpne ||
    ...
    if (vGuard && !vGuard->canBeRemoved()) {
      if (vp->trace())
        traceMsg(vp->comp(),
                 "   found virtual guard node %p with inner assumptions "
                 "(cannot be removed)\n",
                 node);
  ...
  if (vp->getValueNumber(lhsChild) == vp->getValueNumber(rhsChild) &&
    ...
    if (vp->trace())
      traceMsg(vp->comp(),
               "   cmp children have same value number: %p = %p = %d\n",
  ...
  //
    ...
    if (lhs && rhs) {
      if (vp->trace()) {
        traceMsg(vp->comp(),
                 "   cmp %p child absolute constraints:\n      %p: ", node,
                 lhsChild);
        lhs->print(vp);
        traceMsg(vp->comp(), "\n      %p: ", rhsChild);
        rhs->print(vp);
      ...
      if (lhs->mustBeEqual(rhs, vp) && ignoreVirtualGuard) {
        if (vp->trace())
          traceMsg(vp->comp(),
                   "   cmp children must be equal by absolute constraints: %p "
                   "== %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotFallThrough = true;
        ...
        if (vp->trace())
          traceMsg(vp->comp(),
                   "   cmp children must be not equal by absolute constraints: "
                   "%p != %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotBranch = true;
  ...
  if (!cannotBranch && !cannotFallThrough) {
    ...
    if (rel && ignoreVirtualGuard) {
      ...
      if (rel->mustBeEqual()) {
        if (vp->trace())
          traceMsg(vp->comp(),
                   "   cmp children must be equal by relative constraint: %p "
                   "== %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotFallThrough = true;
        ...
        if (vp->trace())
          traceMsg(vp->comp(),
                   "   cmp children must be not equal by relative constraint: "
                   "%p != %p\n",
                   lhsChild, rhsChild);
        if (branchOnEqual)
          cannotBranch = true;
  ...
  if (cannotBranch && performTransformation(
  ...
  if (!vp->comp()->compileRelocatableCode() && vp->lastTimeThrough() &&
      vp->comp()->performVirtualGuardNOPing() && !vp->_curBlock->isCold() &&
      !vp->_curBlock->getNextBlock()->isExtensionOfPreviousBlock() &&
    ...
    if ((second->getOpCodeValue() == TR::aconst) &&
      ...
      bool isGlobal;
      TR::VPConstraint *objectRefConstraint =
          vp->getConstraint(first, isGlobal);
      if (objectRefConstraint) {
        TR::VPClassType *typeConstraint = objectRefConstraint->getClassType();
        if (typeConstraint) {
          TR::VPConstraint *resolvedTypeConstraint =
              typeConstraint->asResolvedClass();
          bool allowForAOT =
              vp->comp()->getOption(TR_UseSymbolValidationManager);
          if (resolvedTypeConstraint) {
            ...
            if (vp->trace())
              traceMsg(vp->comp(),
                       "MyDebug: clazz %p classInfo %p "
                       "classInfo->isInitialized() %d\n",
                       clazz, classInfo,
                       classInfo ? classInfo->isInitialized() : -1);
            if (!classInfo || !classInfo->isInitialized()) {
              int32_t len;
            ...
            TR::VPUnresolvedClass *unresolvedTypeConstraint =
                typeConstraint->asUnresolvedClass();
            if (unresolvedTypeConstraint) {
              int32_t len;
              const char *sig =
                  unresolvedTypeConstraint->getClassSignature(len);
              TR_ResolvedMethod *owningMethod =
                  unresolvedTypeConstraint->getOwningMethod();
              if (owningMethod) {
                TR_OpaqueClassBlock *clazz =
                    vp->fe()->getClassFromSignature(sig, len, owningMethod);
                if (!clazz) {
                  clazzToBeInitialized = sig;
                  clazzNameLen = len;
                } else {
                  TR_PersistentClassInfo *classInfo =
                      vp->comp()
                          ->getPersistentInfo()
                          ->getPersistentCHTable()
                          ->findClassInfoAfterLocking(clazz, vp->comp(),
                                                      allowForAOT);
          ...
          if (clazzToBeInitialized &&
              (((clazzNameLen == 35) &&
                !strncmp(clazzToBeInitialized,
                         "Lcom/ibm/ejs/ras/TraceEnabledToken;",
                         clazzNameLen)) ||
               ((clazzNameLen == 41) &&
                !strncmp(clazzToBeInitialized,
                         "Lcom/ibm/websphere/ras/TraceEnabledToken;",
                         clazzNameLen)) ||
               ((clazzNameLen == 40) &&
                !strncmp(clazzToBeInitialized,
                         "Ljava/lang/String$StringCompressionFlag;",
                         clazzNameLen))) &&
              performTransformation(
                  vp->comp(),
                  "%sUsing side-effect guard to fold away condition on a load "
                  "from an uninitializec class  %s [%p]\n",
                  OPT_DETAILS, clazzToBeInitialized, node)) {
            char *clazzToBeInitializedCopy =
                (char *)vp->trMemory()->allocateMemory(clazzNameLen + 1,
                                                       heapAlloc);
            strcpy(clazzToBeInitializedCopy, clazzToBeInitialized);
            vp->_classesToCheckInit.add(
                new (vp->trStackMemory()) OMR::ValuePropagation::ClassInitInfo(
                    vp, clazzToBeInitializedCopy, clazzNameLen));
          }
        }
      }
    }
  ...
  //
  if (vp->trace()) traceMsg(vp->comp(), "   Conditional branch\n");
  OMR::ValuePropagation::EdgeConstraints *edgeConstraints =
      vp->createEdgeConstraints(edge, true);
  ...
  if (lhs) {
    ...
    if (lhs->isNullObject())
      ...
      // if (lhs->isUnsigned())
      //   notEqualConstraint = TR::VPIntConst::createExclusion(vp,
      //   lhs->asIntConst()->getInt(), true);
      // else
      notEqualConstraint =
          TR::VPIntConst::createExclusion(vp, lhs->asIntConst()->getInt());
    } else if (lhs->asLongConst())
      notEqualConstraint =
          TR::VPLongConst::createExclusion(vp, lhs->asLongConst()->getLong());
  }
  if (rhs && !notEqualConstraint) {
    ...
    if (rhs->isNullObject())
      ...
      // if (rhs->isUnsigned())
      //   notEqualConstraint = TR::VPIntConst::createExclusion(vp,
      //   rhs->asIntConst()->getInt(), true);
      // else
      notEqualConstraint =
          TR::VPIntConst::createExclusion(vp, rhs->asIntConst()->getInt());
    } else if (rhs->asLongConst())
      notEqualConstraint =
          TR::VPLongConst::createExclusion(vp, rhs->asLongConst()->getLong());
  }
  ...
  if (lhsChild->getOpCodeValue() == TR:: instanceof && rhs &&
                                                        rhs->asIntConst()) {
    ...
    if (value == 0 || value == 1) {
      ...
      TR::VPConstraint *classConstraint =
          vp->getConstraint(classChild, isGlobal);
      if (classConstraint && classConstraint->getClassType()) {
    ...
    if (virtualGuard) {
      switch (virtualGuard->getTestType()) {
        case TR_VftTest: {
          ...
          static const char *enableJavaLangClassFolding =
              feGetEnv("TR_EnableFoldJavaLangClass");
          if (enableJavaLangClassFolding &&
              (instanceofObjectRef->getOpCodeValue() == TR::aloadi) &&
              (instanceofObjectRef->getSymbolReference() ==
               vp->comp()->getSymRefTab()->findVftSymbolRef())) {
            TR::Node *objectChild = instanceofObjectRef->getFirstChild();
            TR::VPConstraint *objectConstraint =
                vp->getConstraint(objectChild, isGlobal);
            TR::VPConstraint *classConstraint =
                vp->getConstraint(classChild, isGlobal);

            // if objectConstraint is equal to java/lang/Class we can fold the
            // guard
            if (ignoreVirtualGuard && classConstraint &&
                classConstraint->getClassType() && objectConstraint &&
                objectConstraint->isClassObject() == TR_yes &&
                vp->comp()->getClassClassPointer()) {
              bool testForEquality = (node->getOpCodeValue() == TR::ifacmpeq);
              bool childrenAreEqual = (vp->comp()->getClassClassPointer() ==
          ...
          TR::VPConstraint *classConstraint =
              vp->getConstraint(classChild, isGlobal);
          // foldedGuard indicates that we already set cannotBranch or
          // cannotFallThrough
          if (!foldedGuard && classConstraint &&
        ...
        case TR_MethodTest: {
          ...
          if ((vtableEntryNode->getOpCodeValue() == TR::aloadi) &&
              ...
              vp->comp()->getSymRefTab()->isVtableEntrySymbolRef(
                  vtableEntryNode->getSymbolReference()))
          /*(vtableEntryNode->getSymbolReference() ==
             vp->comp()->getSymRefTab()->findVftSymbolRef()))*/
          {
            ...
            TR::VPConstraint *classConstraint =
                vp->getConstraint(classNode, isGlobal);
            if (ignoreVirtualGuard && classConstraint &&
                classConstraint->isFixedClass()) {
              TR_OpaqueClassBlock *clazz = classConstraint->getClass();
              int32_t vftOffset =
                  vtableEntryNode->getSymbolReference()->getOffset();
              intptrj_t vftEntry =
                  TR::Compiler->cls.getVFTEntry(vp->comp(), clazz, vftOffset);
              bool childrenAreEqual = (vftEntry == methodPtrNode->getAddress());
              bool testForEquality = (node->getOpCodeValue() == TR::ifacmpeq);
              if (vp->trace())
                traceMsg(vp->comp(),
                         "TR_MethodTest: node=%p, vtableEntryNode=%p, "
                         "clazz=%p, vftOffset=%d, vftEntry=%p, "
                         "childrenAreEqual=%d, testForEquality=%d\n",
                         node, vtableEntryNode, clazz, vftOffset, vftEntry,
                         childrenAreEqual, testForEquality);
              if (testForEquality == childrenAreEqual)
                cannotFallThrough = true;
              else
                cannotBranch = true;
            ...
            if (vtableEntryNode->getSymbolReference() ==
                vp->comp()->getSymRefTab()->findVftSymbolRef()) {
              TR_OpaqueClassBlock *classFromMethod =
                  vp->comp()->fe()->getClassFromMethodBlock(
                      (TR_OpaqueMethodBlock *)methodPtrNode->getAddress());
              constraintFromMethodPointer =
                  TR::VPResolvedClass::create(vp, classFromMethod);
            }
          }
        } break;
        default:
  ...
  // This code would help in avoiding the virtual call path when creating
  // constraints for a return value (for example) from a guarded virtual call
  //
  ...
  if (node->isTheVirtualGuardForAGuardedInlinedCall() &&
    ...
    if (nextRealNode->getOpCode().isTreeTop() &&
        (nextRealNode->getNumChildren() > 0))
    ...
    if (callNode && callNode->getOpCode().isCallIndirect()) {
      ...
      bool isGlobal;
      TR::VPConstraint *constraint =
          vp->getConstraint(callNode->getChild(firstArgIndex), isGlobal);
      TR_OpaqueClassBlock *thisType;
      // dumpOptDetails(vp->comp(), "Guard %p Call %p constraint %p\n", node,
      // callNode, constraint);
      if (constraint && constraint->isFixedClass() &&
          (thisType = constraint->getClass()) && methodSymbol->isVirtual()) {
        TR::ResolvedMethodSymbol *resolvedMethodSymbol =
            methodSymbol->getResolvedMethodSymbol();
        if (resolvedMethodSymbol) {
          TR_ResolvedMethod *originalResolvedMethod =
              resolvedMethodSymbol->getResolvedMethod();
          TR_OpaqueClassBlock *originalMethodClass =
              originalResolvedMethod->classOfMethod();

          if ((vp->fe()->isInstanceOf(thisType, originalMethodClass, true,
                                      true) == TR_yes) &&
              !originalResolvedMethod->virtualMethodIsOverridden()) {
            virtualGuardConstraint = constraint;
            TR_VirtualGuard *virtualGuard =
                vp->comp()->findVirtualGuardInfo(node);
  ...
  if (branchOnEqual) {
      ...
      if (rhs && rhs->getClassType() && lhs->getClassType()) {
        ...
        if (!result) {
          if (vp->trace())
            traceMsg(
                vp->comp(),
                "   types are inconsistent, result will not be propagated\n");
        }
      }
    ...
    if (notEqualConstraint) {
      if (!vp->addEdgeConstraint(constraintChild, notEqualConstraint,
                                 edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
  ...
  if (instanceofConstraint && instanceofOnBranch) {
    TR::VPConstraint *newConstraint = passingTypeTestObjectConstraint(
        vp, instanceofConstraint, instanceofDetectedAndFixedType,
        instanceofObjectRefIsVft);

    if (!isVirtualGuardNopable &&
        !vp->addEdgeConstraint(instanceofObjectRef, newConstraint,
                               edgeConstraints)) {
  ...
  //
  // Do not create the (in)equality constraint on object comparison.  There is
  // no intrinsic value of the comparison, and instead we end with way too many
  // objects related together because they were all related to null.  This
  // increases the time it takes to propagate constraints, but provides no
  // useful value since we track (non-)nullness using a different mechanism
  // anyway.
  //
  if (node->getOpCodeValue() != TR::ifacmpeq &&
      node->getOpCodeValue() != TR::ifacmpne) {
    if (branchOnEqual) {
      if (!vp->addEdgeConstraint(lhsChild, TR::VPEqual::create(vp, 0),
                                 edgeConstraints, rhsChild)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
          vp->setIntersectionFailed(false);
      ...
      if (!vp->addEdgeConstraint(lhsChild, TR::VPNotEqual::create(vp, 0),
                                 edgeConstraints, rhsChild)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
  ...
  if (vp->trace() && !cannotBranch) vp->printEdgeConstraints(edgeConstraints);

  // Apply new constraints to the fall through edge
  //
  if (branchOnEqual) {
    if (notEqualConstraint) {
      if (!vp->addBlockConstraint(constraintChild, notEqualConstraint, NULL,
                                  false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
        else
  ...
  if (instanceofConstraint && !instanceofOnBranch) {
    ...
    if (!isVirtualGuardNopable &&
        !vp->addBlockConstraint(instanceofObjectRef, newConstraint, NULL,
                                false)) {
      TR_ASSERT_FATAL(!vp->intersectionFailed(), "VP intersection error");
      cannotFallThrough = true;
    }
  ...
  if (node->getOpCodeValue() != TR::ifacmpeq &&
    ...
    //
    if (branchOnEqual) {
      if (!vp->addBlockConstraint(lhsChild, TR::VPNotEqual::create(vp, 0),
                                  rhsChild, false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
        else
      ...
      if (!vp->addBlockConstraint(lhsChild, TR::VPEqual::create(vp, 0),
                                  rhsChild, false)) {
        if (!vp->intersectionFailed())
          cannotFallThrough = true;
        else
  ...
  if (cannotBranch && performTransformation(
    ...
    changeConditionalToGoto(vp, node, edge);
  } else if (constraintFromMethodPointer && lhsChild->getFirstChild()
                                                ->getFirstChild()
                                                ->getOpCode()
                                                .isLoadVarDirect()) {
    // if constraintFromMethodPointer is set we should be able to assume that
    // lhsChild->getFirstChild()->getFirstChild() is not NULL, since non-NULL
    // constraintFromMethodPointer implies the following trees:
    // n235n     ifacmpne --> block_25 BBStart at n230n (inlineProfiledGuard )
    // [0x7fffd5110a40] bci=[0,0,314] rc=0 vc=56 vn=49 sti=- udi=- nc=2
    // flg=0x1020 n233n       iaload <vtable-entry-symbol> [#552 Shadow +768]
    // [flags 0x10607 0x0 ]              [0x7fffd51109b0] bci=[-1,0,2302] rc=1
    // vc=56 vn=7 sti=- udi=- nc=1 n232n         iaload <vft-symbol> [#449
    // Shadow] [flags 0x18607 0x0 ] (X>=0 )                  [0x7fffd5110968]
    // bci=[-1,0,2302] rc=1 vc=56 vn=6 sti=- udi=- nc=1 flg=0x100 n3n ==>aload
    // n234n       aconst 0x7fd070 (methodPointerConstant ) [0x7fffd51109f8]
    // bci=[0,0,314] rc=1 vc=56 vn=5 sti=- udi=- nc=0 flg=0x2000

    bool isGlobal;
    TR::VPConstraint *intersected =
        vp->getConstraint(lhsChild->getFirstChild()->getFirstChild(), isGlobal);
    ...
    if (intersected) {
      if (!branchOnEqual) {
        vp->addBlockConstraint(lhsChild->getFirstChild()->getFirstChild(),
                               intersected);
      } else {
        // not sure about this case (is this the right way to add an edge
        // constraint??
        vp->addEdgeConstraint(lhsChild->getFirstChild()->getFirstChild(),
                              intersected, edgeConstraints);
      }
    }
  } else if (node->isProfiledGuard() &&
             node->getOpCodeValue() == TR::ifacmpne) {
#ifdef J9_PROJECT_SPECIFIC
    ...
    if (vp->trace()) {
      traceMsg(
          vp->comp(),
          "P2O: Considering a candidate at %p. Object Constraint : ", node);
      lhs->print(vp);
    ...
    static const char *disableProfiled2Overridden =
        feGetEnv("TR_DisableProfiled2Overridden");
    if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
      ...
      // interfaces are weird we would create an UnresolvedClass Constraint and
      // then wrap it in a Class constraint
      if (rhs && !objectClass && lhs && lhs->getClassType() &&
          (interfaceMethodSymbol = callNode->getSymbolReference()
                                       ->getSymbol()
                                       ->castToMethodSymbol()) &&
          interfaceMethodSymbol->isInterface()) {
        int32_t len;
        TR_ResolvedMethod *owningMethod =
            callNode->getSymbolReference()->getOwningMethod(vp->comp());
        const char *sig = lhs->getClassType()->getClassSignature(len);
        objectClass = vp->comp()->fe()->getClassFromSignature(
            sig, len, owningMethod, true);

        // The object's class can sometimes be less specific than the class of
        // the call if we ask for a rammethod in such a class we would trigger an
        // assert ("no ram method in the vft slot") to avoid this we would to
        // insert another isInstanceOf (objectClass, callClass)
        ...
        sig = classNameToSignature(interfaceMethod->classNameChars(), len,
                                   vp->comp());
        callClass = vp->comp()->fe()->getClassFromSignature(sig, len,
                                                            owningMethod, true);
      }

      if (!callClass && !callNode->getSymbolReference()->isUnresolved()) {
        TR::ResolvedMethodSymbol *resolvedCallMethodSymbol =
            callNode->getSymbolReference()
                ->getSymbol()
                ->getResolvedMethodSymbol();
        if (resolvedCallMethodSymbol) {
          TR_ResolvedMethod *resolvedCallMethod =
              resolvedCallMethodSymbol->getResolvedMethod();
          callClass = resolvedCallMethod->containingClass();
        }
      }

      // if all the conditions below are satisfied, especially the last one, we
      // can cut some corners and use findSingleImplementer to get a concrete
      // implementation regardless of the type(e.g. virtual vs interface) of a
      // call and the type of a receiver (e.g. abstract, interface, normal
      // classes) if findSingleImplementer returns a method than it must be the
      // implementation Inliner used since it would be the only one available
      if (objectClass && rhsClass && callClass && !lhs->isFixedClass() &&
          vp->comp()->fe()->isInstanceOf(rhsClass, objectClass, true, true,
                                         true) == TR_yes &&
          vp->comp()->fe()->isInstanceOf(objectClass, callClass, true, true,
                                         true) ==
              TR_yes) /*the object class may be less specific than callClass*/
      {
        TR::SymbolReference *symRef = callNode->getSymbolReference();
        TR::MethodSymbol *methodSymbol =
            symRef->getSymbol()->castToMethodSymbol();
        ...
        if (methodSymbol &&
            !(!methodSymbol->isInterface() &&
              TR::Compiler->cls.isInterfaceClass(vp->comp(), objectClass))) {
          ...
          TR_YesNoMaybe useGetResolvedInterfaceMethod =
              methodSymbol->isInterface() ? TR_yes : TR_no;
          TR::MethodSymbol::Kinds methodKind = methodSymbol->isInterface()
                                                   ? TR::MethodSymbol::Interface
                                                   : TR::MethodSymbol::Virtual;

          TR_ResolvedMethod *rvm = findSingleImplementer(
              objectClass, cpIndexOrVftSlot,
              symRef->getOwningMethod(vp->comp()), vp->comp(), false,
              useGetResolvedInterfaceMethod);

          TR_ScratchList<TR_PersistentClassInfo> subClasses(
              vp->comp()->trMemory());

          // in AOT or when CHOpts are disabled findClassInfoAfterLocking
          // returns NULL unconditionally we should treat this case
          // conservatively meaning we assume that there are more than two
          // concrete classes in the given hierarchy most likely rvm will also be
          // NULL in this case, but it is better to not rely on this implicit
          // connection between different CH queries.
          TR_PersistentClassInfo *objectClassInfo =
              vp->comp()
                  ->getPersistentInfo()
                  ->getPersistentCHTable()
                  ->findClassInfoAfterLocking(objectClass, vp->comp());

          bool searchSucceeded = false;

          if (objectClassInfo) {
            TR_ClassQueries::collectAllSubClasses(objectClassInfo, &subClasses,
                                                  vp->comp());
          ...
          /*
            ...
            Namely, we only do a transformation if there is no more than one
            instantiable class in the hierarchy starting from objectClass. If
            there is more than one concrete class in hierarchy overriding
            methods in objectClass and if any call was devirtualized by
            invariantargumentpreexisence downstream we will produce the
            functionally incorrect code by swapping a profiled test with a
            method test as the method test is insufficient for the
            devirtualizations done by invariantargumentpreexisence
          */

          if (searchSucceeded && numConcreteClasses(&subClasses) < 2 && rvm) {
            TR::ResolvedMethodSymbol *cMethodSymbol =
                vp->comp()
                    ->getSymRefTab()
                    ->findOrCreateMethodSymbol(symRef->getOwningMethodIndex(),
                                               -1, rvm, methodKind)
                    ->getSymbol()
            ...
            TR_VirtualGuardKind guardKind =
                methodSymbol->isInterface()
                    ? TR_InterfaceGuard
                    : TR::Compiler->cls.isAbstractClass(vp->comp(), objectClass)
                          ? TR_AbstractGuard
                          : TR_HierarchyGuard;

            addDelayedConvertedGuard(node, callNode, cMethodSymbol, vGuard, vp,
                                     guardKind, objectClass);
          }
        }
      }
    }
...
void getLimits(OMR::ValuePropagation *vp, int32_t *zLow, int32_t *zHigh,
               TR::Node *symNode, bool isGlobal) {
...
void getLimits(OMR::ValuePropagation *vp, int64_t *zLow, int64_t *zHigh,
               TR::Node *symNode, bool isGlobal) {
  TR::VPConstraint *zConstraint = vp->getConstraint(symNode, isGlobal);
...
TR::Node *makeNewRhsNode(OMR::ValuePropagation *vp, TR::Node *node,
                         TR::Node *symNodeY, int32_t constDiff) {
...
TR::Node *makeNewRhsNode(OMR::ValuePropagation *vp, TR::Node *node,
                         TR::Node *symNodeY, int64_t constDiff) {
...
// Decide and execute simplification:  x + a < y + b  changes into  x < y + (b -
// a) .
//
template <class IntType>
static TR::Node *simplifyInequality(OMR::ValuePropagation *vp, TR::Node *node,
                                    TR::Node *lhsChild, TR::Node *rhsChild,
  ...
  if ((lhsChild->getOpCode().isAdd() || lhsChild->getOpCode().isSub()) &&
    ...
    if (lhsChild->getOpCode().isSub()) aConst = -aConst;
    if (rhsChild->getOpCode().isSub()) bConst = -bConst;
    ...
    if (  // no overflow in x + a
        ...
        (aConst > bConst ? yLow >= low + aConst - bConst
                         : yHigh <= high - bConst + aConst)) {
      ...
      TR::Node *newRhsNode =
          makeNewRhsNode(vp, node, rhsChild->getFirstChild(), cConst);
      ...
      // A newly-created node may have a value number larger than the limit
      // (e.g. 200,000 for WCode). Values larger than that limit are used for
      // other purposes, so there can't be any overlap. It's not safe to call
      // constrainChildren on a newly-created node
      // constrainChildren(vp, node);
    }
  }
  return node;
...
static int64_t clippedDifference(int64_t left, int64_t right) {
  ...
  // for Y within the range TR::getMinSigned<TR::Int32>() <= Y <=
  // -TR::getMinSigned<TR::Int32>(). (Note: -TR::getMinSigned<TR::Int32>() =
  // TR::getMaxSigned<TR::Int32>()+1)
  //
  ...
  //
  // (Note that overflow in the negative direction can't occur when left >=
  // right.)
...
//
static TR::Node *constrainIfcmplessthan(OMR::ValuePropagation *vp,
                                        TR::Node *node, TR::Node *lhsChild,
  ...
  //
  bool childrenReversed =
      (rhsChild == node->getFirstChild());  // MUST be before constrainChildren
  constrainChildren(vp, node);
  ...
  //
    ...
    if (lhsChild->getOpCode().isLong() && !isUnsigned) {
      simplifyInequality<int64_t>(vp, node, lhsChild, rhsChild, isGlobal,
                                  childrenReversed);
    } else if (!isUnsigned) {
      simplifyInequality<int32_t>(vp, node, lhsChild, rhsChild, isGlobal,
                                  childrenReversed);
    }
  ...
  if (!cannotBranch && !cannotFallThrough && !isUnsigned) {
    ...
    if (rel) {
      ...
      if (rel->asEqual() && !cannotFallThrough && !cannotBranch &&
          (lhs || rhs)) {
        // If we know lhs == rhs+k, and rhs+k can't overflow, we can tell which
        // way the comparison will go. Likewise, since lhs-k == rhs, if we know
        // lhs-k can't overflow, we're also good to go.

        TR::VPEqual *relation = rel->asEqual();
        int32_t increment = relation->increment();

        int64_t maxIncrement =
            0;  // If absIncrement is more than this, overflow may occur, and
                // (for example) a==b+k does not imply a>b
        int64_t absIncrement = increment;
        if (absIncrement < 0) {
          absIncrement = -absIncrement;
          if (rhs) {
            if (rhsChild->getOpCode().isLong())
              maxIncrement = clippedDifference(rhs->getLowLong(),
                                               TR::getMinSigned<TR::Int64>());
            else
              maxIncrement = clippedDifference(rhs->getLowInt(),
                                               TR::getMinSigned<TR::Int32>());
          } else {
            if (lhsChild->getOpCode().isLong())
              maxIncrement = clippedDifference(TR::getMaxSigned<TR::Int64>(),
                                               lhs->getHighLong());
            else
              maxIncrement = clippedDifference(TR::getMaxSigned<TR::Int32>(),
                                               lhs->getHighInt());
          }
        } else {
          if (rhs) {
            ...
            if (lhsChild->getOpCode().isLong())
              maxIncrement = clippedDifference(lhs->getLowLong(),
                                               TR::getMinSigned<TR::Int64>());
            else
              maxIncrement = clippedDifference(lhs->getLowInt(),
                                               TR::getMinSigned<TR::Int32>());
          }
        }
        ...
        if (vp->trace()) {
          traceMsg(vp->comp(),
                   "   Conditional relation check on %s [%p]: increment=%d, "
                   "absIncrement=" INT64_PRINTF_FORMAT
                   ", maxIncrement=" INT64_PRINTF_FORMAT ", orEqual=%s\n",
                   node->getOpCode().getName(), node, increment, absIncrement,
                   maxIncrement, orEqual ? "true" : "false");
        }

        // At this point we have: TR::getMinSigned<TR::Int32>() <= absIncrement
        // <= -TR::getMinSigned<TR::Int32>(), and maxIncrement comes from
        // clippedDifference, so by the definition of clippedDifference, we can
        ...
        if (cannotBranch || cannotFallThrough)
          if (!performTransformation(
                  vp->comp(),
                  "%sSuccessful conditional relation check on %s [%p]\n",
                  OPT_DETAILS, node->getOpCode().getName(), node)) {
            cannotBranch = false;
  ...
  int32_t minIntValue =
      /*isUnsigned ? TR::getMinUnsigned<TR::Int32>() :*/ TR::getMinSigned<
          TR::Int32>();
  int32_t maxIntValue =
      /*isUnsigned ? TR::getMaxUnsigned<TR::Int32>() :*/ TR::getMaxSigned<
          TR::Int32>();
  int64_t minLongValue = isUnsigned ? TR::getMinUnsigned<TR::Int64>()
                                    : TR::getMinSigned<TR::Int64>();
  ...
  if (lhsChild->getOpCode().isLong()) {
    ...
    if (longBoundary != TR::getMinSigned<TR::Int64>() && !isUnsigned) {
      if (!vp->addEdgeConstraint(
              rhsChild,
              TR::VPLongRange::create(vp, longBoundary,
                                      TR::getMaxSigned<TR::Int64>()),
              edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
    ...
    if (longBoundary != TR::getMaxSigned<TR::Int64>()) {
      if (!vp->addEdgeConstraint(
              lhsChild, TR::VPLongRange::create(vp, minLongValue, longBoundary),
              edgeConstraints)) {
        if (!vp->intersectionFailed())
          cannotBranch = true;
        else
    ...
    bool modifiedBoundary = /*isUnsigned ? ((uint32_t)intBoundary !=
                              TR::getMinUnsigned<TR::Int32>())
                              :*/
        (intBoundary != TR::getMinSigned<TR::Int32>());
    // if (intBoundary != TR::getMinSigned<TR::Int32>())
    if (modifiedBoundary) {
      TR::VPConstraint *constraint =
          TR::VPIntRange::create(vp, intBoundary, maxIntValue /*, isUnsigned*/);
      // if (!vp->addEdgeConstraint(rhsChild, TR::VPIntRange::create(vp,
      // intBoundary, TR::getMaxSigned<TR::Int32>()), edgeConstraints))
    ...
    intBoundary -= adjustment;
    modifiedBoundary = /*isUnsigned ? ((uint32_t)intBoundary !=
                         TR::getMaxUnsigned<TR::Int32>())
                         :*/
    ...
    if (modifiedBoundary) {
      TR::VPConstraint *constraint =
          TR::VPIntRange::create(vp, minIntValue, intBoundary /*, isUnsigned*/);
      // if (!vp->addEdgeConstraint(lhsChild, TR::VPIntRange::create(vp,
      // TR::getMinSigned<TR::Int32>(), intBoundary), edgeConstraints))
  ...
  // Apply relative constraint to the branch edge
  //
  // TR::VPLessThanOrEqual *newConstraint = TR::VPLessThanOrEqual::create(vp,
  // -adjustment); if (newConstraint->increment() != 0)
  //   newConstraint->setHasArtificialIncrement();
  // if (!vp->addEdgeConstraint(lhsChild, newConstraint, edgeConstraints,
  // rhsChild))
  //   cannotBranch = true;
  ...
  if (lhsChild->getOpCode().isLong() && !isUnsigned) {
    ...
    if (modifiedBoundary) {
      TR::VPConstraint *constraint =
          TR::VPIntRange::create(vp, minIntValue, intBoundary /*, isUnsigned*/);
      // if (!vp->addBlockConstraint(rhsChild, TR::VPIntRange::create(vp,
      // TR::getMinSigned<TR::Int32>(), intBoundary), NULL, false))
    ...
    if (modifiedBoundary) {
      TR::VPConstraint *constraint =
          TR::VPIntRange::create(vp, intBoundary, maxIntValue /*, isUnsigned*/);
      // if (!vp->addBlockConstraint(lhsChild, TR::VPIntRange::create(vp,
      // intBoundary, TR::getMaxSigned<TR::Int32>()), NULL, false))
  ...
  if (!lhsChild->getOpCode().isLong() && !rhsChild->getOpCode().isLong()) {
    // TR::VPGreaterThanOrEqual *newConstraint =
    // TR::VPGreaterThanOrEqual::create(vp, 1-adjustment); if
    // (newConstraint->increment() != 0)
    //   newConstraint->setHasArtificialIncrement();
...
TR::Node *constrainIfcmplt(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainIfcmplessthan(vp, node, node->getFirstChild(),
                                node->getSecondChild(), false);
}

TR::Node *constrainIfcmple(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainIfcmplessthan(vp, node, node->getFirstChild(),
                                node->getSecondChild(), true);
}

TR::Node *constrainIfcmpgt(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainIfcmplessthan(vp, node, node->getSecondChild(),
                                node->getFirstChild(), false);
}
...
//
static TR::Node *constrainCmpeqne(OMR::ValuePropagation *vp, TR::Node *node,
                                  bool testEqual) {
  constrainChildren(vp, node);
  ...
  if (result >= 0) {
    ...
    if (lhs && vp->lastTimeThrough()) lhsIsBool = isBoolean(lhs);

    if (secondChild->getOpCode().isLoadConst() && lhsIsBool &&
        vp->getCurrentParent()) {
      int64_t constValue;
      ...
      if (((constValue == 1) && ((node->getOpCodeValue() == TR::icmpeq) ||
                                 (node->getOpCodeValue() == TR::lcmpeq))) ||
          ((constValue == 0) && ((node->getOpCodeValue() == TR::icmpne) ||
                                 (node->getOpCodeValue() == TR::lcmpne)))) {
        if (performTransformation(vp->comp(),
                                  "%sReduced identity operation on bool in "
        ...
        if (firstChild->getOpCodeValue() == node->getOpCodeValue()) {
          ...
          if (secondGrandChild->getOpCode().isLoadConst() && firstGrandIsBool &&
              vp->getCurrentParent()) {
...
//
static TR::Node *constrainCmplessthan(OMR::ValuePropagation *vp, TR::Node *node,
                                      TR::Node *lhsChild, TR::Node *rhsChild,
                                      bool orEqual) {
  TR_ASSERT(!node->getOpCode().isUnsignedCompare(),
            "This code doesn't handle logical comparisons, but it should be "
            "easy to fix it.");
  ...
  //
  bool childrenReversed =
      (rhsChild == node->getFirstChild());  // MUST be before constainChildren
  ...
  if (result >= 0) {
    if ((lhsGlobal || vp->lastTimeThrough()) &&
        performTransformation(
            vp->comp(), "%sChanging node [%p] %s into constant %d\n",
...
TR::Node *constrainCmplt(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainCmplessthan(vp, node, node->getFirstChild(),
                              node->getSecondChild(), false);
}
...
TR::Node *constrainCmpgt(OMR::ValuePropagation *vp, TR::Node *node) {
  return constrainCmplessthan(vp, node, node->getSecondChild(),
                              node->getFirstChild(), false);
}
    ...
    if (!isInt64 && constraint && constraint->asIntConstraint()) {
      ...
      for (i = node->getNumChildren() - 1; i > 1; i--) {
        ...
        if (/*!isUnsigned &&*/ ((low < value && high < value) ||
          ...
          TR::Block *target =
              node->getChild(i)->getBranchDestination()->getNode()->getBlock();
          if (vp->trace())
          ...
          if (minCase > value) minCase = value;
          if (maxCase < value) maxCase = value;
        }
      }
      ...
      if (casesRemoved) {
        ...
        TR::Block *defaultTarget =
            node->getChild(1)->getBranchDestination()->getNode()->getBlock();
        TR::CFGEdge *defaultEdge =
            vp->findOutEdge(vp->_curBlock->getSuccessors(), defaultTarget);
        for (auto e = vp->_curBlock->getSuccessors().begin();
             e != vp->_curBlock->getSuccessors().end(); ++e) {
          if (((*e) == defaultEdge) ||
      ...
      if (low >= minCase && high <= maxCase)
        node->setCannotOverflow(
            true);  // Let evaluator know value range of selector is covered by
                    // min and max case
    }               // if IntConstraint
...
TR::Node *constrainCase(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  //
  TR::Block *target = node->getBranchDestination()->getNode()->getBlock();
  if (vp->trace()) traceMsg(vp->comp(), "   Switch case branch\n");
...
static void constrainClassObjectLoadaddr(OMR::ValuePropagation *vp,
                                         TR::Node *node, bool isGlobal) {
...
TR::Node *constrainLoadaddr(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (symbol->isAddressOfClassObject()) {
    ...
    TR::VPConstraint *constraint = TR::VPClass::create(
        vp, TR::VPClassType::create(vp, symRef, false, true), NULL, NULL, NULL,
        TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject));
    vp->addGlobalConstraint(node, constraint);
    ...
    if (localObj->getOpCodeKind() == TR::New) {
      if (symRef) typeConstraint = TR::VPClassType::create(vp, symRef, true);
      ...
      TR_OpaqueClassBlock *clazz =
          vp->fe()->getClassFromNewArrayType(localObj->getArrayType());
      if (clazz) typeConstraint = TR::VPFixedClass::create(vp, clazz);
    }

    if (typeConstraint) vp->addGlobalConstraint(node, typeConstraint);
    vp->addGlobalConstraint(node, TR::VPNonNullObject::create(vp));
  } else {
    ...
    bool isGlobal;
    constraint =
        vp->mergeDefConstraints(node, vp->AbsoluteConstraint, isGlobal);
    if (constraint) {
      if (constraint->isNullObject())
...
void constrainNewlyFoldedConst(OMR::ValuePropagation *vp, TR::Node *node,
                               bool isGlobal) {
  switch (node->getOpCodeValue()) {
    case TR::iconst:
...
// Perform null check processing (used by TR::NULLCHK and
// TR::ResolveAndNULLCHK). Return 1 if the null check can be removed. Return 2
// if the null check must be taken. Return 0 if the null check may or may not be
// taken.
//
static int32_t handleNullCheck(OMR::ValuePropagation *vp, TR::Node *node,
                               bool resolveChkToo) {
  ...
  //
  if (!node->getNullCheckReference()) return 1;
...
//
static bool handleResolveCheck(OMR::ValuePropagation *vp, TR::Node *node,
                               bool nullChkToo) {
  ...
  int32_t index = child->getSymbolReference()->getUnresolvedIndex() +
                  vp->_firstUnresolvedSymbolValueNumber;
  ...
  //
  if (nullChkToo)
    vp->createExceptionEdgeConstraints(
        TR::Block::CanCatchNullCheck | TR::Block::CanCatchResolveCheck, NULL,
        node);
  else
    vp->createExceptionEdgeConstraints(TR::Block::CanCatchResolveCheck, NULL,
                                       node);

  //
  // This symbol can now be marked as resolved
  //
  // Create an int const == 1 if store is seen; subsequent stores dominated by
  // this one can only be removed if this constraint (int const 1) reaches them.
  // This is because seeing a resolvechk for the load is not sufficient (in
  // every case) to remove the resolvechk on the dominated store (if the
  // unresolved field happened to be final, then the store should raise an
  // illegal access error even though the load will not)
  //
  if (child->getOpCode().isStore())
    vp->addConstraintToList(node, index, vp->AbsoluteConstraint,
                            TR::VPIntConst::create(vp, 1),
                            &vp->_curConstraints);
  else if (!rel)
    vp->addConstraintToList(node, index, vp->AbsoluteConstraint,
                            TR::VPIntConst::create(vp, 0),
                            &vp->_curConstraints);
...
TR::Node *constrainNullChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (nullCheckResult == 1 &&
      performTransformation(vp->comp(),
                            "%sRemoving redundant null check node [%p]\n",
                            OPT_DETAILS, node)) {
    ...
    if (child->getOpCode().isTreeTop()) {
      if (vp->comp()->useCompressedPointers() &&
        ...
        TR_ASSERT(child->getReferenceCount() == 1,
                  "Treetop child of NULLCHK has other uses");
...
TR::Node *constrainZeroChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  TR::VPConstraint *nonzeroConstraint =
      TR::VPLongRange::create(vp, TR::getMinSigned<TR::Int64>(), -1)
          ->merge(TR::VPLongRange::create(vp, 1, TR::getMaxSigned<TR::Int64>()),
                  vp);
  ...
  if (childConstraint) {
    ...
    if (!zeroConstraint->intersect(childConstraint, vp)) {
      ...
      if (performTransformation(vp->comp(), "%sRemoving unnecessary %s [%p]\n",
                                OPT_DETAILS, node->getOpCode().getName(),
                                node)) {
...
TR::Node *constrainResolveChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (removeTheCheck &&
      performTransformation(vp->comp(),
                            "%sRemoving redundant resolve check node [%p]\n",
                            OPT_DETAILS, node)) {
    TR_ASSERT(node->getReferenceCount() == 0, "ResolveCHK has references");
    TR_ASSERT(node->getNumChildren() == 1,
              "ResolveCHK has more than one child");
    ...
    if (child->getOpCode().isTreeTop()) {
      if (vp->comp()->useCompressedPointers() &&
        ...
        TR_ASSERT(child->getReferenceCount() == 1,
                  "Treetop child of ResolveCHK has other uses");
  ...
  if (sync && sync->syncEmitted() == TR_yes) {
    ...
    if (vp->trace()) {
      if (sync)
        ...
        traceMsg(vp->comp(),
                 "check got removed at node [%p], syncRequired unchanged\n",
                 node);
      else
        traceMsg(vp->comp(), "No sync constraint found at node [%p]!\n", node);
    }
...
TR::Node *constrainResolveNullChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (handleNullCheck(vp, node, !removeTheCheck) == 1) {
    if (removeTheCheck) {
      if (performTransformation(
              vp->comp(),
              "%sChanging ResolveAndNULLCHK node into a treetop node [%p]\n",
              OPT_DETAILS, node)) {
        TR::Node::recreate(node, TR::treetop);
        vp->setChecksRemoved();
      }
      ...
      if (performTransformation(
              vp->comp(),
              "%sChanging ResolveAndNULLCHK node into a ResolveCHK node [%p]\n",
              OPT_DETAILS, node)) {
    ...
    if (performTransformation(
            vp->comp(),
            "%sChanging ResolveAndNULLCHK node into a NULLCHK node [%p]\n",
            OPT_DETAILS, node)) {
      TR::Node::recreate(node, TR::NULLCHK);
      node->setSymbolReference(
          vp->comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(
              vp->comp()->getMethodSymbol()));
      vp->setChecksRemoved();
    }
  }
  return node;
...
TR::Node *constrainUnsignedOverflowChk(OMR::ValuePropagation *vp,
                                       TR::Node *node) {
  constrainChildren(vp, node);
  return node;
}
...
TR::Node *constrainDivChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (divNode->getOpCode().isDiv() || divNode->getOpCode().isRem()) {
    bool isGlobal;
    TR::VPConstraint *denominator =
        vp->getConstraint(divNode->getSecondChild(), isGlobal);
    ...
    if (denominator && !denominator->asMergedConstraints()) {
      ...
      ListIterator<TR::VPConstraint> it(
          denominator->asMergedConstraints()->getList());
      TR::VPConstraint *nextConstraint = NULL;
      for (nextConstraint = it.getFirst(); nextConstraint != NULL;
           nextConstraint = it.getNext()) {
        if (divNode->getType().isInt32()) {
    ...
    TR::VPConstraint *numerator =
        vp->getConstraint(divNode->getFirstChild(), isGlobal);
    if (numerator) {
      if (divNode->getType().isInt32()) {
  ...
  if (removeDivCheck &&
      performTransformation(vp->comp(),
                            "%sRemoving redundant div check node [%p]\n",
                            OPT_DETAILS, node))
    TR::Node::recreate(node, TR::treetop);
  else {
    if (!complexDivCheck) {
...
TR::Node *constrainBndChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (sizeBefore && index) {
    if (index->getLowInt() >= 0 &&
        ...
        performTransformation(vp->comp(),
                              "%sRemoving unnecessary bound check node [%p]\n",
                              OPT_DETAILS, node)) {
  ...
  bool globalFlag;
  TR::VPConstraint *relativeConstraint =
      vp->getConstraint(indexNode, globalFlag, sizeNode);
  ...
  if (vp->_enableVersionBlocks && !vp->_disableVersionBlockForThisBlock &&
      vp->lastTimeThrough())
  ...
  // We now know the BNDCHK has succeeded, and can make some assertions about
  // the index value.
  ...
  int32_t minIndex = 0;
  int32_t maxIndex =
      TR::Compiler->om.maxArraySizeInElements(elementSize, vp->comp()) - 1;

  if (sizeBefore) maxIndex = std::min(maxIndex, sizeBefore->getHighInt() - 1);
  ...
  if (minIndex <= maxIndex) {
    ...
    if (index) indexConstraint = index->intersect(indexConstraint, vp);
  } else
  ...
  if (sizeBefore) sizeAfter = sizeBefore->intersect(sizeAfter, vp);
  ...
  if (sizeNode->getOpCode().isArrayLength()) {
    ...
    vp->addBlockConstraint(objectRef,
                           TR::VPArrayInfo::create(vp, minSize, maxSize, 0));
  }
  return node;
}

TR::Node *constrainBndChkWithSpineChk(OMR::ValuePropagation *vp,
                                      TR::Node *node) {
  constrainChildren(vp, node);
  ...
  if (indexConstraint && objConstraint) {
    ...
    if (arrayInfo) {
      ...
      if (indexConstraint->getLowInt() >= 0 &&
          ...
          performTransformation(
              vp->comp(),
              "%sRemoving unnecessary bound check from composite node [%p]\n",
              OPT_DETAILS, node)) {
        removeBndChk = true;
      }
    }
  ...
  if (sizeConstraint && !vp->comp()->generateArraylets()) {
    // If we can determine the element size then we can possibly eliminate the
    // spine check.
    //
    int32_t elementSize = 0;
    ...
    if (elementSize > 0 &&
        !TR::Compiler->om.isDiscontiguousArray(sizeConstraint->getHighInt(),
                                               elementSize) &&
        (sizeConstraint->getLowInt() > 0) &&
        performTransformation(
            vp->comp(),
            "%sRemoving unnecessary spine check from composite node [%p] : min "
            "arraylength = %d, max arraylength=%d, element size=%d\n",
            OPT_DETAILS, node, sizeConstraint->getLowInt(),
            sizeConstraint->getHighInt(), elementSize)) {
      removeSpineChk = true;
    }
  }
  ...
  TR::VPConstraint *relativeConstraint =
      vp->getConstraint(indexNode, globalFlag, sizeNode);
  if (!removeBndChk && relativeConstraint &&
      relativeConstraint->mustBeLessThan() &&
      performTransformation(
          vp->comp(),
          "%sRemoving redundant bound check from node (subsumed) [%p]\n",
          OPT_DETAILS, node)) {
    removeBndChk = true;
  }
  ...
  if ((sizeNode == indexNode) && !((sizeNode->getOpCodeValue() == TR::iconst) &&
                                   (sizeNode->getInt() == 0))) {
    vp->mustTakeException();
    return node;
  }
  ...
  if (isSizeContigArrayLength) {
    ...
    if (!constraint) constraint = TR::VPIntRange::create(vp, low, high);
  ...
  if (/* (!isSizeContigArrayLength || foundArrayInfo) && */
      (!constraint ||
       (indexConstraint && indexConstraint->getLowInt() >= high + 1))) {
    // Everything past this point in the block is dead, since the exception
    // will always be taken.
    //
    vp->mustTakeException();
  ...
  if (!isSizeContigArrayLength) {
    ...
    if (!isSizeContigArrayLength || (low > 0)) low++;
...
//----------------------------------------------------------
  ...
  if (sizeNode->getOpCode().isArrayLength()) {
    TR::Node *objectRef = sizeNode->getFirstChild();
    vp->addBlockConstraint(objectRef,
                           TR::VPArrayInfo::create(vp, low, high, 0));
  }
...
TR::Node *constrainArrayCopyBndChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if ((vp->getValueNumber(lhsNode) == vp->getValueNumber(rhsNode)) ||
    ...
    if (performTransformation(
            vp->comp(),
            "%sRemoving redundant arraycopy bound check node [%p]\n",
            OPT_DETAILS, node)) {
      vp->removeNode(node);
      vp->setChecksRemoved();
      return NULL;
  ...
  int32_t low = 0;
  int32_t high = TR::getMaxSigned<TR::Int32>();
  if (elementSize > 0) high = high / elementSize - 1;

  if (lhs && lhs->getHighInt() < high) high = lhs->getHighInt();

  if (rhs && rhs->getLowInt() > low) low = rhs->getLowInt();
  ...
  if (isArrayLengthCheck) {
    ...
    vp->addBlockConstraint(
        objectRef, TR::VPArrayInfo::create(vp, lhsConstraint->getLowInt(),
                                           lhsConstraint->getHighInt(), 0));
  }
  return node;
}
...
TR::Node *constrainArrayStoreChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (objectRef->getOpCode().isLoadVar() &&
    ...
    if (base == arrayRef) canBeRemoved = true;
  }
      ...
      if (sig && sig[0] == '[') {
        ...
        //
          ...
          TR_OpaqueClassBlock *arrayComponentClass =
              vp->fe()->getComponentClassFromArrayClass(array->getClass());
          ...
          if (array->asClass() && array->isClassObject() == TR_yes)
            arrayComponentClass =
                vp->fe()->getClassClassPointer(array->getClass());

          TR_YesNoMaybe isInstance = TR_maybe;
          if (arrayComponentClass)
            isInstance = vp->fe()->isInstanceOf(
                objectClass, arrayComponentClass, object->isFixedClass(),
                array->isFixedClass());
          // isInstance = vp->fe()->isInstanceOf(object->getClass(),
          // arrayComponentClass, object->isFixedClass(), array->isFixedClass());

          if (isInstance == TR_yes) {
            vp->registerPreXClass(object);
            canBeRemoved = true;
          } else if (isInstance == TR_no &&
                     debug("enableMustFailArrayStoreCheckOpt")) {
            vp->registerPreXClass(object);
            mustFail = true;
          } else if (arrayComponentClass && objectClass &&
                     !TR::Compiler->cls.isClassArray(vp->comp(),
                                                     arrayComponentClass) &&
                     (arrayComponentClass == objectClass) &&
                     !vp->comp()->fe()->classHasBeenExtended(objectClass)) {
            if (vp->trace())
              traceMsg(vp->comp(),
                       "Setting arrayStoreClass on ArrayStoreChk node [%p] to "
                       "[%p]\n",
                       node, objectClass);
            node->setArrayStoreClassInNode(objectClass);
                   ...
                   arrayComponentClass && objectClass &&
                   vp->fe()->isInstanceOf(objectClass, arrayComponentClass,
                                          true, true)) {
            if (vp->trace())
              traceMsg(vp->comp(),
                       "Setting arrayComponentClass on ArrayStoreChk node [%p] "
                       "to [%p]\n",
  ...
  if (canBeRemoved) {
    ...
    if (performTransformation(
            vp->comp(), "%sRemoving redundant arraystore check node [%p]\n",
            OPT_DETAILS, node)) {
      // Child is the iastore. Just change this node to a treetop
      //
      TR::Node::recreate(node, TR::treetop);
      // DemandLiteralPool can add extra aload to arraystore check node, but
      // treetop can only have one child
      if (vp->comp()->cg()->supportsOnDemandLiteralPool() &&
  ...
  vp->createExceptionEdgeConstraints(TR::Block::CanCatchArrayStoreCheck, NULL,
                                     node);
...
TR::Node *constrainArrayChk(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  //
  else if (srcType && srcType == dstType &&
           srcType->isPrimitiveArray(vp->comp())) {
    removeThisNode = true;
  }

  if (removeThisNode &&
      performTransformation(vp->comp(),
                            "%sRemoving redundant array check node [%p]\n",
                            OPT_DETAILS, node)) {
...
TR::Node *constrainArraycopy(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  //
  if (len && node->getNumChildren() == 3 && len->asIntConst() &&
      len->getLowInt() == 0 &&
  ...
  if (node->getNumChildren() == 5) {
    ...
    if (elementType != TR::NoType &&
        performTransformation(vp->comp(),
                              "%sTransforming arraycopy node [%p]\n",
                              OPT_DETAILS, node)) {
  ...
  if (node->getNumChildren() == 3) {
    ListIterator<OMR::ValuePropagation::TR_TreeTopNodePair> treesIt1(
        &vp->_scalarizedArrayCopies);
    bool alreadyAdded = false;
    OMR::ValuePropagation::TR_TreeTopNodePair *scalarizedArrayCopy;
    for (scalarizedArrayCopy = treesIt1.getFirst(); scalarizedArrayCopy;
         scalarizedArrayCopy = treesIt1.getNext()) {
    ...
    if (!alreadyAdded)
      vp->_scalarizedArrayCopies.add(
          new (vp->comp()->trStackMemory())
              OMR::ValuePropagation::TR_TreeTopNodePair(vp->_curTree, node));
  } else {
    vp->createExceptionEdgeConstraints(TR::Block::CanCatchArrayStoreCheck, NULL,
                                       node);
  }
...
TR::Node *constrainIntegralToBCD(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (constraint)
    ...
    if (node->getFirstChild()->getType().isInt16())
      prec =
          std::min(prec, getPrecisionFromValue(TR::getMaxSigned<TR::Int16>()));
    else if (node->getFirstChild()->getType().isInt32())
      prec =
          std::min(prec, getPrecisionFromValue(TR::getMaxSigned<TR::Int32>()));
    else if (node->getFirstChild()->getType().isInt64())
      prec =
          std::min(prec, getPrecisionFromValue(TR::getMaxSigned<TR::Int64>()));
  }

  if (prec > node->getSourcePrecision()) return node;

  if (performTransformation(
          vp->comp(), "%sSetting source precision on node %s [0x%x] to %d\n",
...
TR::Node *constrainBCDToIntegral(OMR::ValuePropagation *vp, TR::Node *node) {
  ...
  if (node->getType().isInt64()) {
    // If a node has precision < the max precision (for its data type) and has
    // the isNonNegative (x >= 0) flag set, we know that it can't possibly be
    // represented as a negative number, so the min value for the constraint
    // must be at least 0
    if (p < node->getMaxIntegerPrecision() && node->isNonNegative())
      constrainRangeByPrecision(TR::getMinSigned<TR::Int64>(),
                                TR::getMaxSigned<TR::Int64>(), p, lo, hi, true);
    else
      constrainRangeByPrecision(TR::getMinSigned<TR::Int64>(),
                                TR::getMaxSigned<TR::Int64>(), p, lo, hi);
  } else {
    if (p < node->getMaxIntegerPrecision() && node->isNonNegative())
      constrainRangeByPrecision(TR::getMinSigned<TR::Int32>(),
                                TR::getMaxSigned<TR::Int32>(), p, lo, hi, true);
    else
      constrainRangeByPrecision(TR::getMinSigned<TR::Int32>(),
                                TR::getMaxSigned<TR::Int32>(), p, lo, hi);
  }
