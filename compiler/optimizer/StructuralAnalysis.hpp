...
private:
  ...
  void simpleIterator(TR_Stack<int32_t> &workStack, StructureBitVector &vector,
                      WorkBitVector &regionNodes, WorkBitVector &nodesInPath,
  ...
  public:
    StructInfo(TR::Region &region)
        : _pred(region), _succ(region), _exceptionPred(region),
    ...
    int32_t getNumber() {
      return _originalBlock ? _originalBlock->getNumber() : -1;
    }
  };

  TR_RegionAnalysis(TR::Compilation *comp, TR_Dominators &dominators,
                    TR::CFG *cfg, TR::Region &workingRegion)
      : _workingRegion(workingRegion),
        _structureRegion(comp->getFlowGraph()->structureRegion()),
        _compilation(comp), _infoTable(NULL), _dominators(dominators),
        _cfg(cfg) {}
  ...
  void addNaturalLoopNodes(StructInfo &node, WorkBitVector &regionNodes,
                           WorkBitVector &nodesInPath, bool &cyclesFound,
                           TR::Block *hdrBlock);
  ...
  TR_RegionStructure *findRegion(StructInfo &node, WorkBitVector &regionNodes,
                                 WorkBitVector &nodesInPath);
  void addRegionNodes(StructInfo &node, WorkBitVector &regionNodes,
                      WorkBitVector &nodesInPath, bool &cyclesFound,
                      TR::Block *hdrBlock);
  ...
  void buildRegionSubGraph(TR_RegionStructure *region, StructInfo &entryNode,
                           WorkBitVector &regionNodes, SubGraphNodes &cfgNodes,
                           TR::Region &memRegion);
};
