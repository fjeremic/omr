...
void
TR_LocalAnticipatability::analyzeBlock(TR::Block* block,
...
bool
TR_LocalAnticipatability::updateAnticipatabilityForSupportedNodes(
  TR::Node* node,
  ContainerType* seenDefinedSymbolReferences,
  ContainerType* seenStoredSymbolReferences,
  TR::Block* block,
  ...
  if (visitCount <= node->getVisitCount()) {
    if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0) &&
      ...
      if (opCode.isLoad() || node->getOpCodeValue() == TR::loadaddr) {
        ...
        if (!adjustInfoForAddressAdd(node,
                                     firstChild,
                                     seenDefinedSymbolReferences,
                                     seenStoredSymbolReferences,
                                     killedExpressions,
 ...
 */
bool
TR_LocalAnticipatability::adjustInfoForAddressAdd(
  TR::Node* node,
  TR::Node* child,
  ContainerType* seenDefinedSymbolReferences,
  ContainerType* seenStoredSymbolReferences,
    ...
    if (killedExpressions->get(child->getLocalIndex())) {
      if (trace())
        (TR::Compiler->target.is64Bit())
          ? traceMsg(comp(),
                     "\n330Definition #%d (aladd) is NOT locally anticipatable "
                     "in block_%d because of child\n",
                     node->getLocalIndex(),
                     block->getNumber())
          : traceMsg(comp(),
                     "\n330Definition #%d (aiadd) is NOT locally anticipatable "
                     "in block_%d because of child\n",
                     node->getLocalIndex(),
                     block->getNumber());
      return false;
...
void
TR_LocalAnticipatability::updateUsesAndDefs(
  TR::Node* node,
  TR::Block* block,
  ContainerType* seenDefinedSymbolReferences,
  ContainerType* seenStoredSymbolReferences,
...
void
TR_LocalAnticipatability::killDownwardExposedExprs(TR::Block* block,
                                                   ContainerType* tempContainer,
                                                   TR::Node* node)
{
