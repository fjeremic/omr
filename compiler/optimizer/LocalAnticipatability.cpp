  ...
  ContainerType* storeNodes, vcount_t visitCount)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (!updateAnticipatabilityForSupportedNodes(child, seenDefinedSymbolReferences, seenStoredSymbolReferences, block,
          killedExpressions, allSymbolReferences, allSymbolReferencesInNullCheckReference, allSymbolReferencesInStore,
...
void TR_LocalAnticipatability::updateUsesAndDefs(TR::Node* node, TR::Block* block,
  ContainerType* seenDefinedSymbolReferences, ContainerType* seenStoredSymbolReferences,
  ContainerType* seenUsedSymbolReferences, ContainerType* tempContainer, TR_BitVector* temp, ContainerType* storeNodes,
  vcount_t visitCount)
{
  if (visitCount <= node->getVisitCount())
    return;
