...
TR_LocalAnticipatability::TR_LocalAnticipatability(TR_LocalAnalysisInfo &info,
                                                   TR_LocalTransparency *lt,
                                                   bool t)
    : _localTransparency(lt), TR_LocalAnalysis(info, t) {
  if (trace()) traceMsg(comp(), "Starting LocalAnticipatability\n");
  ...
  for (auto methodSymRef = itr.getNext(); methodSymRef;
       methodSymRef = itr.getNext()) {
...
void TR_LocalAnticipatability::analyzeBlock(TR::Block *block,
                                            vcount_t visitCount1,
                                            vcount_t visitCount2,
                                            TR_BitVector *temp) {
  ...
  ContainerType *seenDefinedSymbolReferences =
      allocateTempContainer(symRefCount);
  ContainerType *seenStoredSymbolReferences =
      allocateTempContainer(symRefCount);
  ...
  TR_BitVector *allSymbolReferencesInStore =
      new (trStackMemory()) TR_BitVector(symRefCount, trMemory(), stackAlloc);

  // Once an expression is killed, it cannot become anticiptable again. This is
  // to avoid running into problems where there are two loads of the same symbol
  // (one before a def point (say a call) and one after the def point in a given
  // basic block). In this case we MUST say that the symbol load is not
  // anticipatable as we do not want to common the two different uses leading to
  // possibly incorrect code.
  //
  ...
  while (currentTree != exitTree) {
    ...
    if (firstOpCodeInTree.getOpCodeValue() == TR::NULLCHK) {
      ...
      if (currentTree->getNode()->getFirstChild()->getOpCode().isStore()) {
        _isStoreTree = true;
        if (currentTree->getNode()->getFirstChild()->getLocalIndex() ==
            MAX_SCOUNT)
          storeTreeParticipatesInAnalysis = false;
        storeSymRef =
            currentTree->getNode()->getFirstChild()->getSymbolReference();
    ...
    updateAnticipatabilityForSupportedNodes(
        currentTree->getNode(), seenDefinedSymbolReferences,
        seenStoredSymbolReferences, block, killedExpressions,
        allSymbolReferences, allSymbolReferencesInNullCheckReference,
        allSymbolReferencesInStore, storeNodes, visitCount1);
    ...
    if (_isStoreTree) {
      ...
      if (storeSymRef) {
        if (storeSymRef->sharesSymbol(true)) {
          storeSymRefNode->mayKill(true).getAliasesAndSubtractFrom(
              *allSymbolReferencesInStore);
        } else
      ...
      if (!allSymbolReferencesInStore->isEmpty()) {
        ...
        *tempContainer &= *allSymbolReferencesInStore;
        if (!tempContainer->isEmpty()) isCurrentTreeTopAnticipatable = false;
      ...
      if (storeTreeParticipatesInAnalysis) {
        ...
        if (node->getOpCode().isIndirect() &&
            ((node->getFirstChild()->getLocalIndex() != MAX_SCOUNT &&
             ...
             ((_downwardExposedBeforeButNotAnymore->get(
        ...
        if (downwardExposed) {
          _info[block->getNumber()]._downwardExposedStoreAnalysisInfo->set(
              node->getLocalIndex());
          if (trace())
            traceMsg(comp(),
          ...
          _info[block->getNumber()]._downwardExposedAnalysisInfo->set(
              node->getLocalIndex());
          if (trace())
      ...
      if (isCurrentTreeTopAnticipatable) {
        if (!killedExpressions->get(node->getLocalIndex()) ||
            // if store appears in the block and is in its localTransparency
            // set, it has to be locally anticipatable.
            _localTransparency->getAnalysisInfo(block->getNumber())
                ->get(node->getLocalIndex())) {
      ...
      if (_isNullCheckTree) {
        ...
        if (isCurrentTreeTopAnticipatable) {
          if (!killedExpressions->get(treeTopNode->getLocalIndex())) {
            _info[block->getNumber()]._analysisInfo->set(
                treeTopNode->getLocalIndex());
            if (trace())
              traceMsg(
                  comp(),
          ...
          _info[block->getNumber()]._analysisInfo->reset(
              treeTopNode->getLocalIndex());
          if (trace())
            traceMsg(
                comp(),
      ...
      *tempContainer &= *symbolReferencesInCheck;
      ...
      if (!_isNullCheckTree) {
        ...
        for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
          ...
          if ((child->getLocalIndex() != MAX_SCOUNT) &&
              (child->getLocalIndex() != 0)) {
            if ((_downwardExposedBeforeButNotAnymore->get(
                     child->getLocalIndex()) &&
      ...
      if (downwardExposed &&
          !_downwardExposedBeforeButNotAnymore->get(node->getLocalIndex()) &&
          !_notDownwardExposed->get(node->getLocalIndex())) {
        _info[block->getNumber()]._downwardExposedAnalysisInfo->set(
            node->getLocalIndex());
        if (trace())
    ...
    //
    updateUsesAndDefs(currentTree->getNode(), block,
                      seenDefinedSymbolReferences, seenStoredSymbolReferences,
                      seenUsedSymbolReferences, tempContainer, temp, storeNodes,
                      visitCount2);
  ...
  while (i < getNumNodes()) {
    ...
    if (node &&
        _info[block->getNumber()]._analysisInfo->get(node->getLocalIndex())) {
      ...
      while (j < numChildren) {
        ...
        if ((child->getLocalIndex() != MAX_SCOUNT) &&
          ...
          if (killedExpressions->get(child->getLocalIndex()) &&
            ...
            if (trace())
              traceMsg(comp(),
                       "\n55Definition #%d is NOT locally anticipatable in "
                       "block_%d\n",
                       node->getLocalIndex(), block->getNumber());
            killedExpressions->set(node->getLocalIndex());
            _info[block->getNumber()]._analysisInfo->reset(
...
bool TR_LocalAnticipatability::updateAnticipatabilityForSupportedNodes(
    TR::Node *node, ContainerType *seenDefinedSymbolReferences,
    ContainerType *seenStoredSymbolReferences, TR::Block *block,
    ContainerType *killedExpressions, ContainerType *allSymbolReferences,
    ContainerType *allSymbolReferencesInNullCheckReference,
    TR_BitVector *allSymbolReferencesInStore, ContainerType *storeNodes,
    vcount_t visitCount) {
  ...
  if (visitCount <= node->getVisitCount()) {
    if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0) &&
      ...
      if (opCode.hasSymbolReference() &&
        ...
        if (seenStoredSymbolReferences->get(
      ...
      if (opCode.isLoad() || node->getOpCodeValue() == TR::loadaddr) {
        ...
        if (!adjustInfoForAddressAdd(node, firstChild,
                                     seenDefinedSymbolReferences,
                                     seenStoredSymbolReferences,
                                     killedExpressions, storeNodes, block))
          return false;
        if (!adjustInfoForAddressAdd(node, secondChild,
                                     seenDefinedSymbolReferences,
                                     seenStoredSymbolReferences,
                                     killedExpressions, storeNodes, block))
          return false;
  ...
  if (_isNullCheckTree &&
      node == _checkTree->getNode()->getNullCheckReference()) {
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (!updateAnticipatabilityForSupportedNodes(
            child, seenDefinedSymbolReferences, seenStoredSymbolReferences,
            block, killedExpressions, allSymbolReferences,
            allSymbolReferencesInNullCheckReference, allSymbolReferencesInStore,
            storeNodes, visitCount))
      flag = false;
  }
  ...
  if (opCode.hasSymbolReference() &&
    ...
    if (!aliases.IsZero()) {
      *allSymbolReferences |= aliases;
      ...
      if (_inStoreLhsTree)
        allSymbolReferencesInStore->set(
            node->getSymbolReference()->getReferenceNumber());
    }
  }
  ...
  if (_isStoreTree) {
    if (opCode.isStore()) _inStoreLhsTree = false;
  }
  ...
  if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0) &&
    ...
    if (opCode.isLoad() || opCode.getOpCodeValue() == TR::loadaddr) {
      if (opCode.hasSymbolReference() &&
        ...
        if (seenStoredSymbolReferences->get(
                node->getSymbolReference()->getReferenceNumber())) {
          if (!storeNodes->get(node->getLocalIndex())) return false;
          flagToBeReturned = false;
        }
      }
      ...
      TR::Node *firstChild = node->getFirstChild();
      TR::Node *secondChild = node->getSecondChild();
      if (!adjustInfoForAddressAdd(
              node, firstChild, seenDefinedSymbolReferences,
              seenStoredSymbolReferences, killedExpressions, storeNodes, block))
        return false;
      if (!adjustInfoForAddressAdd(
              node, secondChild, seenDefinedSymbolReferences,
              seenStoredSymbolReferences, killedExpressions, storeNodes, block))
        return false;
    } else
      return false;
  ...
  if (!flagToBeReturned) return false;
  return flag;
}
 ...
 */
bool TR_LocalAnticipatability::adjustInfoForAddressAdd(
    TR::Node *node, TR::Node *child, ContainerType *seenDefinedSymbolReferences,
    ContainerType *seenStoredSymbolReferences, ContainerType *killedExpressions,
    ContainerType *storeNodes, TR::Block *block) {
    ...
    if (killedExpressions->get(child->getLocalIndex())) {
      if (trace())
        (TR::Compiler->target.is64Bit())
            ? traceMsg(comp(),
                       "\n330Definition #%d (aladd) is NOT locally "
                       "anticipatable in block_%d because of child\n",
                       node->getLocalIndex(), block->getNumber())
            : traceMsg(comp(),
                       "\n330Definition #%d (aiadd) is NOT locally "
                       "anticipatable in block_%d because of child\n",
                       node->getLocalIndex(), block->getNumber());
      return false;
    }
    ...
    if (childOpCode.isLoad() || child->getOpCodeValue() == TR::loadaddr) {
      if (childOpCode.hasSymbolReference() &&
        ...
        if (seenDefinedSymbolReferences->get(
            ...
            (seenStoredSymbolReferences->get(
                 child->getSymbolReference()->getReferenceNumber()) &&
...
void TR_LocalAnticipatability::updateUsesAndDefs(
    TR::Node *node, TR::Block *block,
    ContainerType *seenDefinedSymbolReferences,
    ContainerType *seenStoredSymbolReferences,
    ContainerType *seenUsedSymbolReferences, ContainerType *tempContainer,
    TR_BitVector *temp, ContainerType *storeNodes, vcount_t visitCount) {
  ...
  if (opCode.hasSymbolReference() &&
    ...
    if (opCode.isResolveCheck()) {
      ...
      TR_UseDefAliasSetInterface aliasSet =
          node->getFirstChild()->mayKill(true);
...
#endif
      seenUsedSymbolReferences->set(
          symReference->getReferenceNumber());  // in case non-reflexive
    } else {
      ...
      if (opCode.isStore()) {
        ...
        if (_registersScarce ||
          ...
          if (!seenStoredSymbolReferences->get(
            ...
            if (!storeNodes->get(node->getLocalIndex()))
              seenDefinedSymbolReferences->set(
                  symReference->getReferenceNumber());
          }
          ...
          if (!aliases.isZero(comp())) {
            ...
            *seenDefinedSymbolReferences |= *temp;
            ...
            if (!bitAlreadySet)
              seenDefinedSymbolReferences->reset(
                  symReference->getReferenceNumber());
          }
        } else {
          storeNodes->set(node->getLocalIndex());
  ...
  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++)
    updateUsesAndDefs(node->getChild(i), block, seenDefinedSymbolReferences,
                      seenStoredSymbolReferences, seenUsedSymbolReferences,
                      tempContainer, temp, storeNodes, visitCount);
}

void TR_LocalAnticipatability::killDownwardExposedExprs(
    TR::Block *block, ContainerType *tempContainer, TR::Node *node) {
  ...
  if (node && (node->getLocalIndex() != MAX_SCOUNT)) {
    if (_info[block->getNumber()]._downwardExposedAnalysisInfo->get(
            node->getLocalIndex()))
      nodeBitShouldBeOn = true;

    if (_info[block->getNumber()]._downwardExposedStoreAnalysisInfo->get(
            node->getLocalIndex()))
  ...
  *_temp = *(_info[block->getNumber()]._downwardExposedAnalysisInfo);
  ...
  for (iter.SetToFirstOne(); iter.Valid(); iter.SetToNextOne()) {
    ...
    ContainerType *tinfo =
        _localTransparency->getTransparencyInfo(nextDefinedSymbolReference);
...
void TR_LocalAnticipatability::killDownwardExposedExprs(TR::Block *block,
                                                        TR::Node *node) {
  bool nodeBitShouldBeOn = false;
  bool storeNodeBitShouldBeOn = false;
  if (node && (node->getLocalIndex() != MAX_SCOUNT)) {
  ...
  *_temp = *(_info[block->getNumber()]._downwardExposedAnalysisInfo);
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ContainerType *tinfo = _localTransparency->getTransparencyInfo(
        node->getSymbolReference()->getReferenceNumber());
    *(_info[block->getNumber()]._downwardExposedAnalysisInfo) &= *tinfo;
    *(_info[block->getNumber()]._downwardExposedStoreAnalysisInfo) &= *tinfo;
  }
