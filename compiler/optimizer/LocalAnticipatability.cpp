    ...
    , TR_LocalAnalysis(info, t)
{
    ...
    for (auto methodSymRef = itr.getNext(); methodSymRef; methodSymRef = itr.getNext()) {
        if (methodSymRef->getSymbol()->getMethodSymbol() && methodSymRef->getSymbol()->getMethodSymbol()->isPureFunction())
    ...
    while (currentTree != exitTree) {
        ...
        if (_isStoreTree) {
            ...
            if (storeTreeParticipatesInAnalysis) {
                ...
                if (node->getOpCode().isIndirect() && ((node->getFirstChild()->getLocalIndex() != MAX_SCOUNT && node->getFirstChild()->getLocalIndex() != 0) && ((_downwardExposedBeforeButNotAnymore->get(node->getFirstChild()->getLocalIndex()) && _visitedNodes->get(node->getFirstChild()->getGlobalIndex())) || !_info[block->getNumber()]._downwardExposedAnalysisInfo->get(node->getFirstChild()->getLocalIndex()))))
            ...
            if (_isNullCheckTree) {
                ...
                bool downwardExposed = true;
                if (((nullCheckReference->getLocalIndex() != MAX_SCOUNT) && (nullCheckReference->getLocalIndex() != 0)) && ((_downwardExposedBeforeButNotAnymore->get(nullCheckReference->getLocalIndex()) && _visitedNodes->get(nullCheckReference->getGlobalIndex())) || _notDownwardExposed->get(nullCheckReference->getLocalIndex()) || !_info[block->getNumber()]._downwardExposedAnalysisInfo->get(nullCheckReference->getLocalIndex())))
                    downwardExposed = false;
            ...
            *tempContainer &= *symbolReferencesInCheck;
            ...
            if (!_isNullCheckTree) {
                ...
                for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
                    ...
                    if ((child->getLocalIndex() != MAX_SCOUNT) && (child->getLocalIndex() != 0)) {
                        if ((_downwardExposedBeforeButNotAnymore->get(child->getLocalIndex()) && _visitedNodes->get(child->getGlobalIndex())) || _notDownwardExposed->get(child->getLocalIndex()) || !_info[block->getNumber()]._downwardExposedAnalysisInfo->get(child->getLocalIndex()))
    ...
    if (childHasSupportedOpCode) {
        ...
        if (childOpCode.isLoad() || child->getOpCodeValue() == TR::loadaddr) {
            if (childOpCode.hasSymbolReference() && (loadaddrAsLoad() || child->getOpCodeValue() != TR::loadaddr)) {
                if (seenDefinedSymbolReferences->get(child->getSymbolReference()->getReferenceNumber()) || (seenStoredSymbolReferences->get(child->getSymbolReference()->getReferenceNumber()) && ((child->getLocalIndex() == MAX_SCOUNT) || (child->getLocalIndex() == 0) || !storeNodes->get(child->getLocalIndex())))) {
                    if (trace())
                        (TR::Compiler->target.is64Bit()) ? traceMsg(comp(), "\n330Definition #%d (aladd) is NOT locally anticipatable in block_%d because of child\n", node->getLocalIndex(), block->getNumber())
                                                         : traceMsg(comp(), "\n330Definition #%d (aiadd) is NOT locally anticipatable in block_%d because of child\n", node->getLocalIndex(), block->getNumber());
                    return false;
        ...
        if ((opCode.isLoadVar() || (loadaddrAsLoad() && opCode.getOpCodeValue() == TR::loadaddr)) && !node->mightHaveVolatileSymbolReference()) {
            ...
            *seenUsedSymbolReferences = *temp;
            ...
            if (opCode.isStore()) {
                ...
                //
                if (_registersScarce || (node->getSymbolReference() == comp()->getSymRefTab()->findVftSymbolRef()) || node->getSymbolReference()->getSymbol()->isAutoOrParm() || symReference->sharesSymbol() || seenStoredSymbolReferences->get(symReference->getReferenceNumber())) {
