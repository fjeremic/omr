...
int32_t
TR_VirtualGuardTailSplitter::perform()
{
...
void
TR_VirtualGuardTailSplitter::initializeDataStructures()
{
...
void
TR_VirtualGuardTailSplitter::splitLinear(TR::Block* start, TR::Block* end)
{
...
//
TR::Block*
TR_VirtualGuardTailSplitter::lookAheadAndSplit(VGInfo* guard,
                                               List<TR::Block>* stack)
{
  VGInfo* lastInfo = 0;
  List<VGInfo> deferred(trMemory());
...
//
void
TR_VirtualGuardTailSplitter::transformLinear(TR::Block* first, TR::Block* last)
{
  ...
  while (next != last) {
    ...
    if (trace()) {
      traceMsg(comp(),
               "$$$ Processing guards: first %d, last %d\n",
               firstInfo->getNumber(),
               last->getNumber());
      traceMsg(comp(),
               "=> Call node %d, next node %d\n",
               call->getNumber(),
               next->getNumber());
    ...
    for (auto e = next->getExceptionSuccessors().begin();
         e != next->getExceptionSuccessors().end();
         ++e)
...
bool
TR_VirtualGuardTailSplitter::isKill(TR::Block* block)
{
  if (!block->getEntry())
    return false;
...
bool
TR_VirtualGuardTailSplitter::isKill(TR::Node* node)
{
...
void
TR_VirtualGuardTailSplitter::putGuard(uint32_t index, VGInfo* info)
{
...
void
TR_VirtualGuardTailSplitter::VGInfo::markRemoved()
{
...
void
TR_VirtualGuardTailSplitter::remergeGuard(TR_BlockCloner& cloner, VGInfo* info)
{
  TR::Block* block = info->getBranchBlock();

  dumpOptDetails(comp(),
                 "%sperforming global split on guard block_%d\n",
                 OPT_DETAILS,
                 block->getNumber());
...
TR::Node*
TR_VirtualGuardTailSplitter::getFirstCallNode(TR::Block* block)
{
...
TR_InnerPreexistence::transform()
{
  ...
  for (i = 0; i < _numInlinedSites; ++i) {
    ...
    if (trace()) {
      ...
      traceMsg(comp(),
               "Site %d (block_%d, parent-block_%d): thisVN: %d, argsVNs: {",
               i,
  ...
  for (i = 0; i < _numInlinedSites; ++i) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (true) // force block scope
      {
        ...
        outerVirtualGuard->addInnerAssumption(
          comp(), ordinal, innerVirtualGuard);
