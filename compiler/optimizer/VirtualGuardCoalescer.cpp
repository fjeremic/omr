...
public:
  ...
  T* pop()
  {
    return List<T>::popHead();
  }
  T* top()
  {
    return List<T>::_pHead ? List<T>::_pHead->getData() : NULL;
  }
};
...
void TR_VirtualGuardTailSplitter::transformLinear(TR::Block* first, TR::Block* last)
{
  ...
  while (next != last) {
    ...
    if (_cfg->getStructure()) {
      next->getStructureOf()->getParent()->asRegion()->addSubNode(new (_cfg->structureRegion())
          TR_StructureSubGraphNode(new (_cfg->structureRegion()) TR_BlockStructure(comp(), clone->getNumber(), clone)));
    }
...
void TR_InnerPreexistence::transform()
{
  ...
  for (i = 0; i < _numInlinedSites; ++i) {
    ...
    if (trace()) {
      ...
      traceMsg(comp(), "Site %d (block_%d, parent-block_%d): thisVN: %d, argsVNs: {", i, info->getBlock()->getNumber(),
  ...
  for (i = 0; i < _numInlinedSites; ++i) {
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (true) // force block scope
      {
        ...
        //
        int16_t ordinal = -1;
        for (int16_t child = outerCall->getNumChildren() - 1; child >= outerCall->getFirstArgumentIndex(); child--) {
