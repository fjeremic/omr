...
TR_VirtualGuardTailSplitter::TR_VirtualGuardTailSplitter(
...
void TR_VirtualGuardTailSplitter::initializeDataStructures() {
  ...
  // not true when there are exception edges in the contents.  To solve this
  // problem we refuse to recognize a guarded call with exception successors as
  // a valid virtual guarded call.
  //
  ...
  while (!dfsList.isEmpty()) {
    ...
    if (top && block == top->getMergeBlock()) {
      ...
      for (TR::CFGEdge *edge = it.getFirst(); edge && !notDone;
           edge = it.getNext()) {
        if (edge->getFrom()->getVisitCount() != visitCount) notDone = true;
      }
    ...
    for (TR::CFGEdge *edge = it.getFirst(); edge; edge = it.getNext()) {
      ...
      if (!to->isOSRInduceBlock() && !to->isOSRCatchBlock() &&
          !to->isOSRCodeBlock())
        dfsList.add(toBlock(edge->getTo()));
    }
  }

  if (trace()) traceMsg(comp(), "Disjoint set forest:\n");

  // Put the information into the virtual guard table
  //
  _numGuards = numGuards;
  _table =
      (VGInfo **)trMemory()->allocateStackMemory(numGuards * sizeof(VGInfo *));
  ...
  for (VGInfo *info = it.getFirst(); info; info = it.getNext()) {
    ...
    if (trace())
      traceMsg(comp(), "%d -> %d\n", info->getNumber(),
               info->getParent()->getNumber());
  }
}

// Given any block, return if is a virtual guard
//
TR_VirtualGuardTailSplitter::VGInfo *
TR_VirtualGuardTailSplitter::getVirtualGuardInfo(TR::Block *block) {
  if (!block->getExit())
    return 0;
  else if (block->getLastRealTreeTop()
               ->getNode()
               ->isTheVirtualGuardForAGuardedInlinedCall())
    return getGuard(block);
  return 0;
}

void TR_VirtualGuardTailSplitter::splitLinear(TR::Block *start,
                                              TR::Block *end) {
  ...
  while (!stack.isEmpty()) {
    ...
    if (block->getVisitCount() == _visitCount) continue;
    block->setVisitCount(_visitCount);

    VGInfo *info = getVirtualGuardInfo(block);
    if (info) {
      if (!info->isLeaf()) deferred.add(info);
...
// control flow)
//
TR::Block *TR_VirtualGuardTailSplitter::lookAheadAndSplit(
    VGInfo *guard, List<TR::Block> *stack) {
  VGInfo *lastInfo = 0;
  List<VGInfo> deferred(trMemory());
    ...
    if (!(succs.size() == 1)) {
      ...
      if ((succs.size() == 2) && !lastTree->getNode()->getOpCode().isSwitch()) {
        ...
        if (succBlock1 == cursor->getNextBlock()) {
          // TR::Block *nextBlock =
          // toBlock(succBlock1->getSuccessors().front()->getTo());
        ...
        if (!newCursor) {
          ...
          if (succBlock2 == cursor->getNextBlock()) {
            // TR::Block *nextBlock =
            // toBlock(succBlock2->getSuccessors().front()->getTo());
...
//    \/  \/  \/                    \__\__\__/
//
void TR_VirtualGuardTailSplitter::transformLinear(TR::Block *first,
                                                  TR::Block *last) {
  ...
  while (next != last) {
      ...
      next->getStructureOf()->getParent()->asRegion()->addSubNode(
          new (_cfg->structureRegion()) TR_StructureSubGraphNode(
              new (_cfg->structureRegion())
                  TR_BlockStructure(comp(), clone->getNumber(), clone)));
    }
    ...
    for (auto e = next->getExceptionSuccessors().begin();
         e != next->getExceptionSuccessors().end(); ++e)
    ...
    if (info) {
      ...
      TR::Node *gotoNode =
          TR::Node::create(next->getLastRealTreeTop()->getNode(), TR::Goto);
      ...
      _cfg->addEdge(clone, dest);
      if (otherDest) _cfg->addEdge(clone, otherDest);
      _cfg->removeEdge(call, next);
      ...
      if (lastTree->getNode()->getOpCode().isBranch()) {
        if (lastTree->getNode()->getOpCodeValue() == TR::Goto)
          ...
          if (!otherDest) {
            ...
            TR::Node *gotoNode =
                TR::Node::create(lastTree->getNode(), TR::Goto);
            gotoNode->setBranchDestination(dest->getEntry());
...
TR_VirtualGuardTailSplitter::VGInfo *
TR_VirtualGuardTailSplitter::recognizeVirtualGuard(TR::Block *block,
                                                   VGInfo *parent) {
  ...
  TR::Node *node = getFirstCallNode(
      first);  //->getFirstRealTreeTop()->getNode()->getFirstChild();
  // TR::Node *node = first->getFirstRealTreeTop()->getNode()->getFirstChild();
  if (node && node->isTheVirtualCallNodeForAGuardedInlinedCall()) {
    call = first;
    inlined = second;
  } else {
    node = getFirstCallNode(
        second);  //->getFirstRealTreeTop()->getNode()->getFirstChild();
  ...
  if (call && guardNode->isProfiledGuard()) {
    ...
    static char *profiledGuardSplitProbabilityThresholdStr =
        feGetEnv("TR_ProfiledGuardSplitProbabilityThreshold");
    if (profiledGuardSplitProbabilityThresholdStr) {
      profiledGuardProbabilityThreshold =
          ((float)atof(profiledGuardSplitProbabilityThresholdStr));
    }
  ...
  // the call-block must not have any exception edges.  exception edges break
  // the assumption that a virtual guard interval is a proper interval.
  //
  ...
  // check if the merge block has back edges, if so exclude such guards from
  // this opt to prevent dead zone in initializeDataStructures()
  bool invalidMergeBlock =
      (merge == _cfg->getEnd() || merge->getPredecessors().size() > 2);
...
bool TR_VirtualGuardTailSplitter::isKill(TR::Block *block) {
  ...
  for (; tree != exit; tree = tree->getNextRealTreeTop()) {
    if (isKill(tree->getNode())) return true;
  }
...
void TR_VirtualGuardTailSplitter::remergeGuard(TR_BlockCloner &cloner,
                                               VGInfo *info) {
...
const char *TR_VirtualGuardTailSplitter::optDetailString() const throw() {
...
int32_t TR_InnerPreexistence::perform() {
  ...
  if (!comp()->performVirtualGuardNOPing()) return 0;

  TR::StackMemoryRegion stackMemoryRegion(*trMemory());

  if (trace()) comp()->dumpMethodTrees("Trees before InnerPreexistence");
...
int32_t TR_InnerPreexistence::initialize() {
  ...
  // array of guard blocks index by the inlined call site index of the guarded
  // method
  TR::Block **guardBlocks = (TR::Block **)trMemory()->allocateStackMemory(
      _numInlinedSites * sizeof(TR::Block *));
  memset(guardBlocks, 0, _numInlinedSites * sizeof(TR::Block *));

  int32_t numGuards = 0;
  TR::TreeTop *treeTop, *exitTree;
  for (treeTop = comp()->getStartTree(); treeTop;
       treeTop = exitTree->getNextTreeTop()) {
    ...
    if (lastNode->isTheVirtualGuardForAGuardedInlinedCall()) {
      ...
      // Both the inner guard and the outer guard must be nop-able for inner
      // preexistence to work. Ie. both the guards must give rise to assumptions
      //
      if (info->isNopable()) {
  ...
  if (numGuards == 0) return 0;

  _guardTable = (GuardInfo **)trMemory()->allocateStackMemory(
      _numInlinedSites * sizeof(GuardInfo *));
  ...
  for (int32_t i = 0; i < _numInlinedSites; ++i) {
    ...
    GuardInfo *info = new (trStackMemory())
        GuardInfo(comp(), guard, outerGuard, _vnInfo, _numInlinedSites);
    if (outerGuard) numInnerGuards++;
    _guardTable[i] = info;
  }
...
TR_InnerPreexistence::GuardInfo::GuardInfo(TR::Compilation *comp,
                                           TR::Block *block, GuardInfo *parent,
  ...
  TR_ASSERT(callNode->getOpCode().isIndirect(),
            "Guarded calls must be indirect");

  _argVNs = new (comp->trStackMemory())
      TR_BitVector(20, comp->trMemory(), stackAlloc, growable);
  _innerSubTree = new (comp->trStackMemory())
      TR_BitVector(numInlinedSites, comp->trMemory(), stackAlloc, notGrowable);
...
void TR_InnerPreexistence::transform() {
  int32_t i;
  for (i = _numInlinedSites - 1; i > 0;
       --i)  // intentionally skipping i = 0 case
  {
  ...
  for (i = 0; i < _numInlinedSites; ++i) {
    ...
    if (trace()) {
      ...
      traceMsg(
          comp(), "Site %d (block_%d, parent-block_%d): thisVN: %d, argsVNs: {",
  ...
  for (i = 0; i < _numInlinedSites; ++i) {
    GuardInfo *info = _guardTable[i];
    if (!info || info->getHasBeenDevirtualized()) continue;
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (descendant->getHasBeenDevirtualized()) continue;

      if (!performTransformation(comp(),
                                 "%sDevirtualizing call guarded by block_%d "
                                 "preexisting on guard %d\n",
                                 OPT_DETAILS,
                                 descendant->getBlock()->getNumber(),
      ...
      if (true)  // force block scope
      {
        ...
        // Find out which argument of the outer call is the same as the this of
        // the inner call
        //
        int16_t ordinal = -1;
        for (int16_t child = outerCall->getNumChildren() - 1;
             child >= outerCall->getFirstArgumentIndex(); child--) {
        ...
        TR_VirtualGuard *outerVirtualGuard =
            comp()->findVirtualGuardInfo(info->getGuardNode());
        TR_VirtualGuard *innerVirtualGuard =
            comp()->findVirtualGuardInfo(descendant->getGuardNode());
        outerVirtualGuard->addInnerAssumption(comp(), ordinal,
                                              innerVirtualGuard);
...
const char *TR_InnerPreexistence::optDetailString() const throw() {
