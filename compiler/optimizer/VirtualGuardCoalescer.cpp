...
void TR_VirtualGuardTailSplitter::transformLinear(TR::Block* first, TR::Block* last)
{
    ...
    while (next != last) {
        ...
        if (_cfg->getStructure()) {
            next->getStructureOf()->getParent()->asRegion()->addSubNode(new (_cfg->structureRegion()) TR_StructureSubGraphNode(new (_cfg->structureRegion()) TR_BlockStructure(comp(), clone->getNumber(), clone)));
        }
...
TR_VirtualGuardTailSplitter::VGInfo* TR_VirtualGuardTailSplitter::recognizeVirtualGuard(TR::Block* block, VGInfo* parent)
{
    if (!block->getExit())
        return 0;
