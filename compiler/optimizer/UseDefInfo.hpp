...
public:
  ...
  class AuxiliaryData {
  private:
    AuxiliaryData(int32_t numSymRefs, ncount_t nodeCount, TR::Region &region,
                  TR::Allocator allocator)
        : _region(region),
          _onceReadSymbols(numSymRefs, static_cast<TR_BitVector *>(NULL),
                           _region),
          _onceWrittenSymbols(numSymRefs, static_cast<TR_BitVector *>(NULL),
                              _region),
          ...
          _onceWrittenSymbolsIndices(numSymRefs, TR::SparseBitVector(allocator),
                                     _region),
          _onceReadSymbolsIndices(numSymRefs, TR::SparseBitVector(allocator),
                                  _region),
          _expandedAtoms(0,
                         std::make_pair<TR::Node *, TR::TreeTop *>(NULL, NULL),
                         _region),
          _sideTableToUseDefMap(_region), _numAliases(numSymRefs, _region),
          _loadsBySymRefNum(numSymRefs, _region),
          _defsForOSR(0, static_cast<TR_BitVector *>(NULL), _region),
          _workBitVector(_region), _doneTrivialNode(_region),
    ...
    TR::vector<std::pair<TR::Node *, TR::TreeTop *>, TR::Region &>
        _expandedAtoms;
...
protected:
   ...
   * @param doCompletion
   * @param callsShouldBeUses          Enables inclusion of calls as uses so
   * that the alias analysis can detect when local (stack) variable has been
   * aliased by a function call. A value of false is fine for Java like
   * languages where local (stack) variables cannot be passed by reference to
   * function calls and hence cannot be aliased. However for C like languages
   * this flag should be set to true.
   */
  TR_UseDefInfo(TR::Compilation *, TR::CFG *cfg, TR::Optimizer *,
                bool requiresGlobals = true, bool prefersGlobals = true,
                bool loadsShouldBeDefs = true,
                bool cannotOmitTrivialDefs = false,
                bool conversionRegsOnly = false, bool doCompletion = true,
                bool callsShouldBeUses = false);

  void prepareUseDefInfo(bool requiresGlobals, bool prefersGlobals,
...
private:
  bool _runReachingDefinitions(TR_ReachingDefinitions &reachingDefinitions,
                               AuxiliaryData &aux);
...
protected:
  ...
  TR_Memory *trMemory() {
    TR_ASSERT(false,
              "trMemory is prohibited in TR_UseDefInfo. Use comp->allocator()");
    return NULL;
  }
...
public:
  ...
  bool getUsesFromDef(BitVector &usesFromDef, int32_t defIndex,
                      bool loadAsDef = false);
...
private:
  ...
  const BitVector &getUseDef_ref_body(int32_t useIndex,
...
public:
  ...
  bool skipAnalyzingForCompileTime(TR::Node *node, TR::Block *block,
                                   TR::Compilation *comp, AuxiliaryData &aux);

private:
  static const char *const allocatorName;

  void findTrivialSymbolsToExclude(TR::Node *node, TR::TreeTop *treeTop,
                                   AuxiliaryData &aux);
  ...
  // For Languages where an auto can alias a volatile, extra care needs to be
  // taken when setting up use-def The conservative answer is to not index autos
  // that have volatile aliases.

  void setVolatileSymbolsIndexAndRecurse(TR::BitVector &volatileSymbols,
                                         int32_t symRefNum);
...
public:
  ...
  int32_t getNumDefsOnEntry() {
    return (_uniqueIndexForDefsOnEntry ? _numDefsOnEntry : 1);
  }
  int32_t getTotalNodes() {
    return _numDefOnlyNodes + _numDefUseNodes + _numUseOnlyNodes;
  }
  ...
  int32_t getFirstRealDefIndex() {
    return getFirstDefIndex() + getNumDefsOnEntry();
  }
  int32_t getLastDefIndex() { return getNumDefNodes() - 1; }

  bool isDefIndex(uint32_t index) {
    return index && index <= getLastDefIndex();
  }
  bool isUseIndex(uint32_t index) {
    return index >= getFirstUseIndex() && index <= getLastUseIndex();
  }
...
public:
  ...
  bool isExpandedUseDefIndex(uint32_t index) {
    return index >= _numExpandedDefOnlyNodes &&
           index < getNumExpandedDefNodes();
  }

  bool getDefsForSymbol(TR_BitVector &defs, int32_t symIndex,
                        AuxiliaryData &aux) {
...
private:
  void dereferenceDefs(int32_t useIndex, BitVector &nodesLookedAt,
                       BitVector &loadDefs);

public:
  void dereferenceDef(BitVector &useDefInfo, int32_t defIndex,
                      BitVector &nodesLookedAt);
...
private:
  ...
  bool findUseDefNodes(TR::Block *block, TR::Node *node, TR::Node *parent,
                       TR::TreeTop *treeTop, AuxiliaryData &aux,
                       TR::deque<uint32_t, TR::Region &> &symRefToLocalIndexMap,
                       bool considerImplicitStores = false);
  bool assignAdjustedNodeIndex(TR::Block *, TR::Node *node, TR::Node *parent,
                               TR::TreeTop *treeTop, AuxiliaryData &aux,
                               bool considerImplicitStores = false);
  bool childIndexIndicatesImplicitStore(TR::Node *node, int32_t childIndex);
  void insertData(TR::Block *, TR::Node *node, TR::Node *parent,
                  TR::TreeTop *treeTop, AuxiliaryData &aux,
                  TR::SparseBitVector &, bool considerImplicitStores = false);
  void buildUseDefs(void *vblockInfo, AuxiliaryData &aux);
  void buildUseDefs(TR::Node *node, void *vanalysisInfo,
                    TR_BitVector &nodesToBeDereferenced, TR::Node *parent,
                    AuxiliaryData &aux);
  int32_t setSingleDefiningLoad(int32_t useIndex, BitVector &nodesLookedAt,
                                BitVector &loadDefs);
...
private:
  ...
  TR::list<BitVector, TR::Region &>
      _infoCache; ///< initially empty bit vectors that are used for caching
  const BitVector _EMPTY; ///< the empty bit vector
  TR::vector<const BitVector *, TR::Region &>
      _useDerefDefInfo; ///< all load defs are dereferenced
  ...
  class MemorySymbol {
    ...
    MemorySymbol(uint32_t size, uint32_t offset, int32_t localIndex)
        : _size(size), _offset(offset), _localIndex(localIndex) {}
