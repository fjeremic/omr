...
protected:
   ...
   *                                   set to true.
   */
  TR_UseDefInfo(TR::Compilation*, TR::CFG* cfg, TR::Optimizer*, bool requiresGlobals = true, bool prefersGlobals = true,
    bool loadsShouldBeDefs = true, bool cannotOmitTrivialDefs = false, bool conversionRegsOnly = false,
    bool doCompletion = true, bool callsShouldBeUses = false);
...
public:
  ...
  bool infoIsValid()
  {
    return _isUseDefInfoValid;
  }
  TR::Node* getNode(int32_t index);
  TR::TreeTop* getTreeTop(int32_t index);
  void clearNode(int32_t index)
  {
    _useDefs[index] = TR_UseDef();
  }
...
public:
  int32_t getNumIrrelevantStores()
  {
    return _numIrrelevantStores;
  }
  int32_t getNumUseNodes()
  {
    return _numUseOnlyNodes + _numDefUseNodes;
  }
  int32_t getNumDefNodes()
  {
    return _numDefOnlyNodes + _numDefUseNodes;
  }
  int32_t getNumDefOnlyNodes()
  {
    return _numDefOnlyNodes;
  }
  ...
  int32_t getFirstUseIndex()
  {
    return _numDefOnlyNodes;
  }
  int32_t getLastUseIndex()
  {
    return getTotalNodes() - 1;
  }
  int32_t getFirstDefIndex()
  {
    return 0;
  }
  ...
  int32_t getLastDefIndex()
  {
    return getNumDefNodes() - 1;
  }
  ...
  bool hasGlobalsUseDefs()
  {
    return _indexFields && _indexStatics;
  }
  ...
  int32_t getNumExpandedDefsOnEntry()
  {
    return _numDefsOnEntry;
  }
...
protected:
  uint32_t getBitVectorSize()
  {
    return getNumExpandedDefNodes();
  }
