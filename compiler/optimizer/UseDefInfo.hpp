...
class TR_UseDefInfo {
 ...
 public:
  ...
  class AuxiliaryData {
   private:
    AuxiliaryData(int32_t numSymRefs,
                  ncount_t nodeCount,
                  TR::Region& region,
                  TR::Allocator allocator)
        : _region(region),
          _onceReadSymbols(numSymRefs,
                           static_cast<TR_BitVector*>(NULL),
                           _region),
          _onceWrittenSymbols(numSymRefs,
                              static_cast<TR_BitVector*>(NULL),
                              _region),
          ...
          _onceWrittenSymbolsIndices(numSymRefs,
                                     TR::SparseBitVector(allocator),
                                     _region),
          _onceReadSymbolsIndices(numSymRefs,
                                  TR::SparseBitVector(allocator),
                                  _region),
          _expandedAtoms(0,
 ...
 protected:
   ...
   */
  TR_UseDefInfo(TR::Compilation*,
                TR::CFG* cfg,
                TR::Optimizer*,
                bool requiresGlobals = true,
                bool prefersGlobals = true,
                bool loadsShouldBeDefs = true,
                bool cannotOmitTrivialDefs = false,
                bool conversionRegsOnly = false,
                bool doCompletion = true,
                bool callsShouldBeUses = false);

  void prepareUseDefInfo(bool requiresGlobals,
                         bool prefersGlobals,
 ...
 public:
  ...
  bool getUsesFromDef(BitVector& usesFromDef,
                      int32_t defIndex,
 ...
 public:
  ...
  bool skipAnalyzingForCompileTime(TR::Node* node,
                                   TR::Block* block,
                                   TR::Compilation* comp,
                                   AuxiliaryData& aux);
 ...
 private:
  ...
  void findTrivialSymbolsToExclude(TR::Node* node,
                                   TR::TreeTop* treeTop,
                                   AuxiliaryData& aux);
 ...
 public:
  ...
  bool getDefsForSymbol(TR_BitVector& defs,
                        int32_t symIndex,
                        AuxiliaryData& aux) {
 ...
 private:
  void dereferenceDefs(int32_t useIndex,
                       BitVector& nodesLookedAt,
                       BitVector& loadDefs);

 public:
  void dereferenceDef(BitVector& useDefInfo,
                      int32_t defIndex,
                      BitVector& nodesLookedAt);
 ...
 private:
  ...
  bool findUseDefNodes(TR::Block* block,
                       TR::Node* node,
                       TR::Node* parent,
                       TR::TreeTop* treeTop,
                       AuxiliaryData& aux,
  ...
  bool assignAdjustedNodeIndex(TR::Block*,
                               TR::Node* node,
                               TR::Node* parent,
                               TR::TreeTop* treeTop,
                               AuxiliaryData& aux,
  ...
  void buildUseDefs(TR::Node* node,
                    void* vanalysisInfo,
                    TR_BitVector& nodesToBeDereferenced,
                    TR::Node* parent,
                    AuxiliaryData& aux);
  int32_t setSingleDefiningLoad(int32_t useIndex,
                                BitVector& nodesLookedAt,
                                BitVector& loadDefs);
