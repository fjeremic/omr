...
// Define OPT_TIMING to collect statistics on the time taken by individual
// optimizations. Also use -Xjit:timing to collect statistics.
//#undef OPT_TIMING
...
enum {
  ...
  IfEnabledAndLoops,
  IfEnabledAndNoLoops,
  IfNoLoopsOREnabledAndLoops, // ie. do this opt if no loops; if loops, it must
                              // be enabled
  ...
  IfAOTAndEnabled,
  IfMethodHandleInvokes, // JSR292: Extra analysis required to optimize
                         // MethodHandle.invoke
  IfNotQuickStart,
...
public:
  ...
  static TR::Optimizer *createOptimizer(TR::Compilation *comp,
   ...
   */
  static void setMockStrategy(const OptimizationStrategy *strategy) {
    _mockStrategy = strategy;
  };
  ...
  void setSymReferencesTable(int32_t *symReferencesTable) {
    _symReferencesTable = symReferencesTable;
  }
  ...
  void setCantBuildGlobalsUseDefInfo(bool v) {
    _cantBuildGlobalsUseDefInfo = v;
  }
   ...
   */
  TR_UseDefInfo *
  createUseDefInfo(TR::Compilation *comp, bool requiresGlobals = true,
                   bool prefersGlobals = true, bool loadsShouldBeDefs = true,
  ...
  TR_ValueNumberInfo *createValueNumberInfo(bool requiresGlobals = false,
                                            bool preferGlobals = true,
                                            bool noUseDefInfo = false);
  TR_ValueNumberInfo *setValueNumberInfo(TR_ValueNumberInfo *v);

  bool cantBuildGlobalsValueNumberInfo() {
    return _cantBuildGlobalsValueNumberInfo;
  }
  bool cantBuildLocalsValueNumberInfo() {
    return _cantBuildLocalsValueNumberInfo;
  }
  void setCantBuildGlobalsValueNumberInfo(bool v) {
    _cantBuildGlobalsValueNumberInfo = v;
  }
  void setCantBuildLocalsValueNumberInfo(bool v) {
    _cantBuildLocalsValueNumberInfo = v;
  }

  bool canRunBlockByBlockOptimizations() {
    return _canRunBlockByBlockOptimizations;
  }
  void setCanRunBlockByBlockOptimizations(bool v) {
    _canRunBlockByBlockOptimizations = v;
  }
   ...
   * and hence cannot be aliased. However for C like languages this flag should
   * be over-ridden by the optimizer in the front-end.
   *
  ...
  bool prepareForNodeRemoval(TR::Node *node,
                             bool deferInvalidatingUseDefInfo = false);
  void prepareForTreeRemoval(TR::TreeTop *treeTop) {
    prepareForNodeRemoval(treeTop->getNode());
  }
  ...
  void setEnclosingFinallyBlock(TR::Block *block) {
    _enclosingFinallyBlock = block;
  }
  ...
  List<TR::Node> &getEliminatedCheckcastNodes() {
    return _eliminatedCheckcastNodes;
  }
  ...
  void setRequestOptimization(OMR::Optimizations optNum, bool value = true,
                              TR::Block *block = NULL);

  int32_t getOptMessageIndex() { return _optMessageIndex; }
  int32_t incOptMessageIndex() { return ++_optMessageIndex; }

  bool optsThatCanCreateLoopsDisabled() {
    return _disableLoopOptsThatCanCreateLoops;
  }

  // allowBCDSignPromotion -- if true and node1 has conservatively 'better' sign
  // state then node2 then also consider nodes equivalent (used only by certain
  // optimizations such as CSE)
  static bool areNodesEquivalent(TR::Node *, TR::Node *, TR::Compilation *,
                                 bool allowBCDSignPromotion = false);
  static bool areBCDAggrConstantNodesEquivalent(TR::Node *, TR::Node *,
                                                TR::Compilation *);
  bool areNodesEquivalent(TR::Node *node1, TR::Node *node2,
                          bool allowBCDSignPromotion = false) {
  ...
  TR::OptimizationManager *getOptimization(OMR::Optimizations i) {
    TR_ASSERT(i < OMR::numGroups, "Optimization index should be less than %d",
              OMR::numGroups);
    return _opts[i];
  }
...
private:
  ...
  int32_t performOptimization(const OptimizationStrategy *,
