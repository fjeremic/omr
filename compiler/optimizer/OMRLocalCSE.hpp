...
public:
  ...
  typedef TR::typed_allocator<std::pair<const int32_t, TR::Node *>,
                              TR::Region &>
      HashTableAllocator;
  typedef std::multimap<int32_t, TR::Node *, std::less<int32_t>,
                        HashTableAllocator>
      HashTable;

protected:
  virtual bool shouldTransformBlock(TR::Block *block);
  virtual bool shouldCopyPropagateNode(TR::Node *parent, TR::Node *node,
                                       int32_t childNum, TR::Node *storeNode);
  ...
  virtual bool canAffordToIncreaseRegisterPressure(TR::Node *node = NULL) {
    return true;
  }
...
protected:
  ...
  TR::Node *replaceCopySymbolReferenceByOriginalIn(
      TR::SymbolReference *, /* TR::SymbolReference *,*/ TR::Node *, TR::Node *,
      TR::Node *, TR::Node *, int32_t);
  void examineNode(TR::Node *, TR_BitVector &, TR::Node *, int32_t, int32_t *,
                   bool *, int32_t);
  ...
  void makeNodeAvailableForCommoning(TR::Node *, TR::Node *, TR_BitVector &,
                                     bool *);
  ...
  void killAvailableExpressionsAtGCSafePoints(TR::Node *, TR::Node *,
                                              TR_BitVector &);
  void killAllAvailableExpressions();
  void killAllDataStructures(TR_BitVector &);
  void killAvailableExpressions(int32_t);
  void killAvailableExpressionsUsingBitVector(HashTable *hashTable,
                                              TR_BitVector &vec);
  ...
  void
  killAllInternalPointersBasedOnThisPinningArray(TR::SymbolReference *symRef);
  ...
  bool isIfAggrToBCDOverride(TR::Node *parent, TR::Node *rhsOfStoreDefNode,
                             TR::Node *nodeBeingReplaced);

  bool doCopyPropagationIfPossible(TR::Node *, TR::Node *, int32_t, TR::Node *,
                                   TR::SymbolReference *, vcount_t, bool &);
  ...
  typedef TR::typed_allocator<std::pair<uint32_t const, TR::Node *>,
                              TR::Region &>
      StoreMapAllocator;
  typedef std::less<uint32_t> StoreMapComparator;
  typedef std::map<uint32_t, TR::Node *, StoreMapComparator, StoreMapAllocator>
      StoreMap;
  StoreMap *_storeMap;
