   ...
   */
  {
    ...
    while (currentTree) {
      ...
      updateUsesAndDefs(currentTree->getNode(), globalDefinedSymbolReferences,
        ...
        symRefsDefinedAfterStoredInBlock[blockNumber], symRefsUsedAfterDefinedInBlock[blockNumber], startVisitCount,
        tempContainer, temp, allStoredSymRefsInMethod);
            ...
            if (allSymbolReferencesInStore->get(newDefinedSymbolReference)) {
              ...
              if (trace())
                traceMsg(comp(), "75757Expression %d killed (%x) by symRef #%d  %d\n", storeNode->getLocalIndex(),
                  storeNode, newDefinedSymbolReference, true);
            }
          }
              ...
              if (allSymbolReferencesInNullCheckReference->get(newDefinedSymbolReference)) {
                if (trace())
                  traceMsg(comp(), "76767Expression %d killed (%x) by symRef #%d  %d\n", currentNode->getLocalIndex(),
                    currentNode, newDefinedSymbolReference, true);
  ...
  if (opCode.hasSymbolReference() && (loadaddrAsLoad() || opCode.getOpCodeValue() != TR::loadaddr)) {
    ...
    if ((opCode.isLoadVar() || (loadaddrAsLoad() && opCode.getOpCodeValue() == TR::loadaddr))
      ...
      if (opCode.isStore()) {
        ...
        //
        if (_registersScarce || node->getSymbolReference()->getSymbol()->isAutoOrParm() || symReference->sharesSymbol()
...
void TR_LocalTransparency::updateInfoForSupportedNodes(TR::Node* node, ContainerType* allSymbolReferences,
  ...
  ContainerType* seenDefinedSymbolReferences, ContainerType* seenStoredSymRefs, ContainerType* allStoredSymRefsInMethod,
  vcount_t visitCount)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    updateInfoForSupportedNodes(child, allSymbolReferences, allSymbolReferencesInNullCheckReference,
      allSymbolReferencesInStore, seenDefinedSymbolReferences, seenStoredSymRefs, allStoredSymRefsInMethod, visitCount);
        ...
        if (!_supportedNodes->get(child->getLocalIndex()))
          ...
          for (i = 0; i < comp()->getMaxAliasIndex(); i++) {
            if (!_transparencyInfo[i]->get(child->getLocalIndex())
              ...
              if (trace())
                traceMsg(comp(),
                  "Expression %d killed by symRef #%d because child %d is already killed by the symRef\n",
          ...
          if (child->getOpCode().hasSymbolReference()
            ...
            if (seenDefinedSymbolReferences->get(childSymRefNum) || seenStoredSymRefs->get(childSymRefNum)) {
              ...
              for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
                ...
                if (trace())
                  traceMsg(comp(), "9999Expression %d killed (%x) by symRef #%d  %d\n", node->getLocalIndex(), node,
                    nextAlias, seenDefinedSymbolReferences->get(nextAlias));
              }
      ...
      if (seenDefinedSymbolReferences->get(symRef->getReferenceNumber())
        ...
        if (trace())
          traceMsg(comp(), "Expression %d killed (%x) by symRef #%d  %d\n", node->getLocalIndex(), node,
            symRef->getReferenceNumber(), seenDefinedSymbolReferences->get(symRef->getReferenceNumber()));
        ...
        if (!_transparencyInfo[i]->get(child->getLocalIndex())) {
          ...
          if (trace()) {
            if (TR::Compiler->target.is64Bit())
              traceMsg(comp(),
                "Expression %d killed by symRef #%d because grandchild (child of aladd) %d is already killed by the "
                "symRef\n",
                node->getLocalIndex(), i, child->getLocalIndex());
            else
              traceMsg(comp(),
                "Expression %d killed by symRef #%d because grandchild (child of aiadd) %d is already killed by the "
                "symRef\n",
                node->getLocalIndex(), i, child->getLocalIndex());
          }
        }
