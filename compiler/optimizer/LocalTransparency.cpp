  ...
  : TR_LocalAnalysis(info, t)
{
   ...
   */
  {
    ...
    ContainerType** definedSymbolReferencesInBlock =
      (ContainerType**)trMemory()->allocateStackMemory(numberOfNodes *
                                                       sizeof(ContainerType*));
    memset(definedSymbolReferencesInBlock,
           0,
    ...
    ContainerType** symRefsDefinedAfterStoredInBlock =
      (ContainerType**)trMemory()->allocateStackMemory(numberOfNodes *
                                                       sizeof(ContainerType*));
    memset(symRefsDefinedAfterStoredInBlock,
           0,
           numberOfNodes * sizeof(ContainerType*));
    ContainerType** symRefsUsedAfterDefinedInBlock =
      (ContainerType**)trMemory()->allocateStackMemory(numberOfNodes *
                                                       sizeof(ContainerType*));
    memset(symRefsUsedAfterDefinedInBlock,
           0,
           numberOfNodes * sizeof(ContainerType*));
    ...
    while (currentTree) {
      ...
      updateUsesAndDefs(currentTree->getNode(),
                        globalDefinedSymbolReferences,
    ...
    *_hasTransparencyInfoFor |= *globalDefinedSymbolReferences;
    ...
    while (currentTree) {
      ...
      updateInfoForSupportedNodes(currentNode,
                                  allSymbolReferences,
                                  allSymbolReferencesInNullCheckReference,
  ...
  ContainerType* allStoredSymRefsInMethod)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++)
    updateUsesAndDefs(node->getChild(i),
                      globallySeenDefinedSymbolReferences,
                      seenDefinedSymbolReferences,
                      seenStoredSymRefs,
                      symRefsDefinedAfterStored,
                      symRefsUsedAfterDefined,
                      visitCount,
                      tempContainer,
                      temp,
                      allStoredSymRefsInMethod);
...
void
TR_LocalTransparency::updateInfoForSupportedNodes(
  TR::Node* node,
  ...
  vcount_t visitCount)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    updateInfoForSupportedNodes(child,
                                allSymbolReferences,
                                allSymbolReferencesInNullCheckReference,
                                allSymbolReferencesInStore,
                                seenDefinedSymbolReferences,
      ...
      if (childHasSupportedOpCode) {
        ...
        if (child->getOpCode().isLoad() ||
          ...
          if (child->getOpCode().isTwoChildrenAddress()) {
            ...
            if (trace())
              traceMsg(comp(),
                       "Expression %d killed (non supported opcode)\n",
                       node->getLocalIndex());
          }
        }
      }
    ...
    if (opCode.hasSymbolReference() &&
      ...
      if (seenDefinedSymbolReferences->get(symRef->getReferenceNumber()) ||
        ...
        if (trace())
          traceMsg(
            comp(),
            "Expression %d killed (%x) by symRef #%d  %d\n",
            node->getLocalIndex(),
            node,
            symRef->getReferenceNumber(),
        ...
        for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
          ...
          if (trace())
            traceMsg(comp(),
                     "888Expression %d killed (%x) by symRef #%d  %d\n",
                     node->getLocalIndex(),
                     node,
                     nextAlias,
                     seenDefinedSymbolReferences->get(nextAlias));
...
void
TR_LocalTransparency::adjustInfoForAddressAdd(
  TR::Node* node,
  TR::Node* child,
  ContainerType* seenDefinedSymbolReferences,
  ContainerType* seenStoredSymRefs)
