    ...
    */
    {
        ...
        *_hasTransparencyInfoFor |= *globalDefinedSymbolReferences;
        ...
        while (currentTree) {
            ...
            } else if ((firstOpCodeInTree.getOpCodeValue() == TR::treetop) || (comp()->useAnchors() && firstOpCodeInTree.isAnchor())) {
                if (currentNode->getFirstChild()->getOpCode().isStore()) {
                    _isStoreTree = true;
                    if (currentNode->getFirstChild()->getLocalIndex() == MAX_SCOUNT)
                        storeTreeParticipatesInAnalysis = false;
            ...
            if (_isStoreTree) {
                ...
                //
                if (!_registersScarce && storeTreeParticipatesInAnalysis && !storeNode->getSymbolReference()->getSymbol()->isAutoOrParm()) {
            ...
            *tempContainer |= *definedSymbolReferencesInBlock[nextBlock->getNumber()];
            ...
            for (iter.SetToFirstOne(); iter.Valid(); iter.SetToNextOne()) {
                ...
                if (_hasTransparencyInfoFor->get(nextDefinedSymbolReference)) {
                    ...
                    //
                    if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]->get(nextDefinedSymbolReference) || (!symRefsDefinedAfterStoredInBlock[nextBlock->getNumber()]->get(nextDefinedSymbolReference) && !symRefsUsedAfterDefinedInBlock[nextBlock->getNumber()]->get(nextDefinedSymbolReference))) {
                        ...
                        for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
                            ...
                            if (_info[nextBlock->getNumber()]._analysisInfo->get(storeNode)) {
                                ...
                                if (storeSymRefNum == nextDefinedSymbolReference)
                                    ...
                                    {
                                        if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]->get(storeSymRefNum) || (!symRefsDefinedAfterStoredInBlock[nextBlock->getNumber()]->get(storeSymRefNum) && !symRefsUsedAfterDefinedInBlock[nextBlock->getNumber()]->get(storeSymRefNum)))
                                            symRefCanSurvive = true;
                                    }
                                }
...
void TR_LocalTransparency::updateUsesAndDefs(TR::Node* node, ContainerType* globallySeenDefinedSymbolReferences, ContainerType* seenDefinedSymbolReferences, ContainerType* seenStoredSymRefs, ContainerType* symRefsDefinedAfterStored, ContainerType* symRefsUsedAfterDefined, vcount_t visitCount, ContainerType* tempContainer, TR_BitVector* temp, ContainerType* allStoredSymRefsInMethod)
{
    ...
    if (opCode.hasSymbolReference() && (loadaddrAsLoad() || opCode.getOpCodeValue() != TR::loadaddr)) {
        ...
        if ((opCode.isLoadVar() || (loadaddrAsLoad() && opCode.getOpCodeValue() == TR::loadaddr)) && !node->mightHaveVolatileSymbolReference()) {
            ...
            if (!opCode.isCheck() && !opCode.isStore()) {
                ...
                if (!tempContainer->isEmpty()) {
                    ...
                    *tempContainer -= *getCheckSymbolReferences();
                    ...
                    bool pureFunction = node->getOpCode().isCall() && symReference && symReference->getSymbol()->castToMethodSymbol() && symReference->getSymbol()->castToMethodSymbol()->isPureFunction();
                    if (pureFunction)
            ...
            if (opCode.isStore()) {
                ...
                //
                if (_registersScarce || node->getSymbolReference()->getSymbol()->isAutoOrParm() || symReference->sharesSymbol() || seenStoredSymRefs->get(symRefNum)) {
...
void TR_LocalTransparency::updateInfoForSupportedNodes(TR::Node* node, ContainerType* allSymbolReferences, ContainerType* allSymbolReferencesInNullCheckReference, ContainerType* allSymbolReferencesInStore, ContainerType* seenDefinedSymbolReferences, ContainerType* seenStoredSymRefs, ContainerType* allStoredSymRefsInMethod, vcount_t visitCount)
{
            ...
            if (childHasSupportedOpCode) {
                if (!_supportedNodes->get(child->getLocalIndex()))
                    ...
                    for (i = 0; i < comp()->getMaxAliasIndex(); i++) {
                        if (!_transparencyInfo[i]->get(child->getLocalIndex()) || (child->getOpCode().hasSymbolReference() && !child->getOpCode().isCall() && (i == child->getSymbolReference()->getReferenceNumber()) && seenStoredSymRefs->get(child->getSymbolReference()->getReferenceNumber()))) {
                ...
                if (child->getOpCode().isLoad() || child->getOpCodeValue() == TR::loadaddr) {
                    if (child->getOpCode().hasSymbolReference() && (loadaddrAsLoad() || child->getOpCodeValue() != TR::loadaddr)) {
                        int32_t childSymRefNum = child->getSymbolReference()->getReferenceNumber();

                        if (seenDefinedSymbolReferences->get(childSymRefNum) || seenStoredSymRefs->get(childSymRefNum)) {
    ...
    if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) && (!(opCode.isStore() || opCode.isCheck()))) {
        ...
        if (opCode.hasSymbolReference() && (loadaddrAsLoad() || opCode.getOpCodeValue() != TR::loadaddr)) {
            TR::SymbolReference* symRef = node->getSymbolReference();
            if (seenDefinedSymbolReferences->get(symRef->getReferenceNumber()) || seenStoredSymRefs->get(symRef->getReferenceNumber())) {
                if (trace())
    ...
    if (childHasSupportedOpCode) {
        ...
        if (child->getOpCode().isLoad() || child->getOpCodeValue() == TR::loadaddr) {
            if (child->getOpCode().hasSymbolReference() && (loadaddrAsLoad() || child->getOpCodeValue() != TR::loadaddr)) {
                ...
                if (seenDefinedSymbolReferences->get(childSymRef->getReferenceNumber()) || seenStoredSymRefs->get(childSymRef->getReferenceNumber())) {
