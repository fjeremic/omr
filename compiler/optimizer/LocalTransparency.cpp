...
TR_LocalTransparency::TR_LocalTransparency(TR_LocalAnalysisInfo &info, bool t)
  ...
  if (trace()) traceMsg(comp(), "Starting LocalTransparency\n");
  ...
  //
  if (comp()->getVisitCount() > 8000) comp()->resetVisitCounts(1);
  ...
  // block
  //
  _transparencyInfo = (ContainerType **)trMemory()->allocateStackMemory(
      symRefCount * sizeof(ContainerType *));
  ...
  /*
   * Caution : we want _transparencyInfo to be around after this routine
   * returns; so we should consider this when moving the stack memory region's
   * scope earlier
   */
  {
    ...
    // that are transparent in this block ? Since an expression is transparent
    // or not based on whether any of its symbols have been written or not in
    ...
    ContainerType **definedSymbolReferencesInBlock =
        (ContainerType **)trMemory()->allocateStackMemory(
            numberOfNodes * sizeof(ContainerType *));
    memset(definedSymbolReferencesInBlock, 0,
           numberOfNodes * sizeof(ContainerType *));
    ...
    ContainerType **symRefsDefinedAfterStoredInBlock =
        (ContainerType **)trMemory()->allocateStackMemory(
            numberOfNodes * sizeof(ContainerType *));
    memset(symRefsDefinedAfterStoredInBlock, 0,
           numberOfNodes * sizeof(ContainerType *));
    ContainerType **symRefsUsedAfterDefinedInBlock =
        (ContainerType **)trMemory()->allocateStackMemory(
            numberOfNodes * sizeof(ContainerType *));
    memset(symRefsUsedAfterDefinedInBlock, 0,
           numberOfNodes * sizeof(ContainerType *));
    ...
    ContainerType *globalDefinedSymbolReferences =
        allocateContainer(symRefCount);
    ContainerType *tempContainer =
        allocateContainer(comp()->getMaxAliasIndex());
    TR_BitVector *temp = new (trStackMemory())
        TR_BitVector(comp()->getMaxAliasIndex(), trMemory(), stackAlloc);
    ...
    while (currentTree) {
      ...
      updateUsesAndDefs(currentTree->getNode(), globalDefinedSymbolReferences,
                        definedSymbolReferencesInBlock[blockNumber],
                        storedSymRefsInBlock[blockNumber],
                        symRefsDefinedAfterStoredInBlock[blockNumber],
                        symRefsUsedAfterDefinedInBlock[blockNumber],
                        startVisitCount, tempContainer, temp,
    ...
    ContainerType *allSymbolReferencesInNullCheckReference =
        allocateContainer(symRefCount);
    ...
    *_hasTransparencyInfoFor |= *globalDefinedSymbolReferences;
    ...
    while (currentTree) {
      ...
      updateInfoForSupportedNodes(
          currentNode, allSymbolReferences,
          allSymbolReferencesInNullCheckReference, allSymbolReferencesInStore,
          globalDefinedSymbolReferences, storedSymRefsInBlock[blockNumber],
      ...
      if (_isStoreTree) {
        ...
        for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
          ...
          if (!allSymbolReferencesInStore->isEmpty() &&
            ...
            if (allSymbolReferencesInStore->get(newDefinedSymbolReference)) {
              _transparencyInfo[newDefinedSymbolReference]->reset(
                  storeNode->getLocalIndex());
              if (trace())
                traceMsg(comp(),
                         "75757Expression %d killed (%x) by symRef #%d  %d\n",
                         storeNode->getLocalIndex(), storeNode,
          ...
          if (_isNullCheckTree) {
            if (!allSymbolReferencesInNullCheckReference->isEmpty()) {
              if (allSymbolReferencesInNullCheckReference->get(
                      newDefinedSymbolReference)) {
                if (trace())
                  traceMsg(comp(),
                           "76767Expression %d killed (%x) by symRef #%d  %d\n",
                           currentNode->getLocalIndex(), currentNode,
                           newDefinedSymbolReference, true);

                _transparencyInfo[newDefinedSymbolReference]->reset(
                    currentNode->getLocalIndex());
              }
            }
          }
        }
        ...
        for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
          ...
          if (symbolReferencesInCheck->get(newDefinedSymbolReference)) {
            if (trace())
              traceMsg(comp(),
                       "777Expression %d killed (%x) by symRef #%d  %d\n",
                       currentNode->getLocalIndex(), currentNode,
                       newDefinedSymbolReference, true);
    ...
    // Now use the defined symbol references to decide which expressions are
    // transparent in this block
    //
    TR::CFGNode *nextNode;
    for (nextNode = comp()->getFlowGraph()->getFirstNode(); nextNode;
         nextNode = nextNode->getNext()) {
      TR::Block *nextBlock = toBlock(nextNode);
      TR_LocalAnalysisInfo::LAInfo *binfo = _info + (nextBlock->getNumber());
      if (binfo->_block == NULL) continue;
      ...
      *(_info[nextBlock->getNumber()]._analysisInfo) &= *_supportedNodes;
      ...
      // We need to kill all expressions dependent on a symbol (other than the
      // load/store) being stored into; this is because we cannot compensate for
      // an expression (o.f+1) where o.f is written in the block. We would
      // adjust the load/store of o.f (say with temp t1) but to adjust the value
      // of the temp t2 that corresponds to (o.f+1) is more complicated
      //
      ...
      for (iter.SetToFirstOne(); iter.Valid(); iter.SetToNextOne()) {
        ...
        if (_hasTransparencyInfoFor->get(nextDefinedSymbolReference)) {
          if (trace()) {
            ...
            traceMsg(comp(), "\nDefined : ");
            definedSymbolReferencesInBlock[nextBlock->getNumber()]->print(
                comp());
            traceMsg(comp(), "\nDefined after stored : ");
            symRefsDefinedAfterStoredInBlock[nextBlock->getNumber()]->print(
                comp());
            traceMsg(comp(), "\nUsed after defined : ");
            symRefsUsedAfterDefinedInBlock[nextBlock->getNumber()]->print(
                comp());
            traceMsg(comp(), "\n");
          }
          ...
          // If the value was not really defined by anything other than a call,
          // ONLY the store/load of the stored value (say o.f) can still
          // survive. All other dependent expressions must be killed though. See
          // discussion above.
          //
          if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]->get(
                  nextDefinedSymbolReference) ||
            ...
            for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
              ...
              // If store of this symRef was transparent so far, then keep it
              // transparent; i.e. do not kill it because of this store as this
              // store is going to be privatized
              //
              if (_info[nextBlock->getNumber()]._analysisInfo->get(storeNode)) {
                int32_t storeSymRefNum = supportedNodesAsArray[storeNode]
                                             ->getSymbolReference()
                                             ->getReferenceNumber();

                bool childKilled = false;
                if (supportedNodesAsArray[storeNode]
                        ->getOpCode()
                        .isIndirect()) {
                  TR::Node *child =
                      supportedNodesAsArray[storeNode]->getFirstChild();
                  if ((child->getLocalIndex() != MAX_SCOUNT) &&
                      (child->getLocalIndex() != 0)) {
                    if (!_transparencyInfo[nextDefinedSymbolReference]->get(
                            child->getLocalIndex()))
                      childKilled = true;
                  } else
                    childKilled = true;
                ...
                else if (supportedNodesAsArray[storeNode]
                             ->mayKill(true)
                             .contains(nextDefinedSymbolReference, comp())) {
                  {
                    if (!definedSymbolReferencesInBlock[nextBlock->getNumber()]
                             ->get(storeSymRefNum) ||
                        (!symRefsDefinedAfterStoredInBlock[nextBlock
                                                               ->getNumber()]
                              ->get(storeSymRefNum) &&
                         !symRefsUsedAfterDefinedInBlock[nextBlock->getNumber()]
                              ->get(storeSymRefNum)))
                      symRefCanSurvive = true;
                  }
                }
          ...
          *(_info[nextBlock->getNumber()]._analysisInfo) &=
              *(_transparencyInfo[nextDefinedSymbolReference]);
          *(_info[nextBlock->getNumber()]._analysisInfo) |=
              *survivingStoreNodes;
...
TR_LocalAnalysisInfo::ContainerType *TR_LocalTransparency::getTransparencyInfo(
    int32_t symRefNum) {
  return _transparencyInfo[symRefNum];
}

void TR_LocalTransparency::updateUsesAndDefs(
    TR::Node *node, ContainerType *globallySeenDefinedSymbolReferences,
    ContainerType *seenDefinedSymbolReferences,
    ContainerType *seenStoredSymRefs, ContainerType *symRefsDefinedAfterStored,
    ContainerType *symRefsUsedAfterDefined, vcount_t visitCount,
    ContainerType *tempContainer, TR_BitVector *temp,
    ContainerType *allStoredSymRefsInMethod) {
  ...
  for (i = 0; i < node->getNumChildren(); i++)
    updateUsesAndDefs(node->getChild(i), globallySeenDefinedSymbolReferences,
                      seenDefinedSymbolReferences, seenStoredSymRefs,
                      symRefsDefinedAfterStored, symRefsUsedAfterDefined,
                      visitCount, tempContainer, temp,
                      allStoredSymRefsInMethod);
...
#else
      tempContainer->empty();
      node->getFirstChild()->mayKill(true).getAliasesAndUnionWith(
          *tempContainer);
...
void TR_LocalTransparency::updateInfoForSupportedNodes(
    TR::Node *node, ContainerType *allSymbolReferences,
    ContainerType *allSymbolReferencesInNullCheckReference,
    ContainerType *allSymbolReferencesInStore,
    ContainerType *seenDefinedSymbolReferences,
    ContainerType *seenStoredSymRefs, ContainerType *allStoredSymRefsInMethod,
    vcount_t visitCount) {
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    updateInfoForSupportedNodes(
        child, allSymbolReferences, allSymbolReferencesInNullCheckReference,
        allSymbolReferencesInStore, seenDefinedSymbolReferences,
        seenStoredSymRefs, allStoredSymRefsInMethod, visitCount);
      ...
      if (childHasSupportedOpCode) {
          ...
          for (i = 0; i < comp()->getMaxAliasIndex(); i++) {
            if (!_transparencyInfo[i]->get(child->getLocalIndex()) ||
                (child->getOpCode().hasSymbolReference() &&
                 ...
                 seenStoredSymRefs->get(
                     child->getSymbolReference()->getReferenceNumber()))) {
              _transparencyInfo[i]->reset(node->getLocalIndex());
              if (trace())
                traceMsg(comp(),
                         "Expression %d killed by symRef #%d because child %d "
                         "is already killed by the symRef\n",
        ...
        if (child->getOpCode().isLoad() ||
          ...
          if (child->getOpCode().hasSymbolReference() &&
            ...
            if (seenDefinedSymbolReferences->get(childSymRefNum) ||
              ...
              for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
                ...
                if (trace())
                  traceMsg(comp(),
                           "9999Expression %d killed (%x) by symRef #%d  %d\n",
                           node->getLocalIndex(), node, nextAlias,
              ...
              if (trace())
                traceMsg(
                    comp(),
                    "Expression %d killed by symRef #%d (loaded in child)\n",
          ...
          if (child->getOpCode().isTwoChildrenAddress()) {
            ...
            adjustInfoForAddressAdd(node, firstGrandChild,
                                    seenDefinedSymbolReferences,
                                    seenStoredSymRefs);
            adjustInfoForAddressAdd(node, secondGrandChild,
                                    seenDefinedSymbolReferences,
                                    seenStoredSymRefs);
          } else {
            _supportedNodes->reset(node->getLocalIndex());
  ...
  if (_isStoreTree) {
    if (node->getOpCode().isStore()) _inStoreLhsTree = true;
  }
  ...
  if (opCode.hasSymbolReference() &&
    ...
    if (!aliases.IsZero()) {
      *allSymbolReferences |= aliases;
      ...
      if (_inStoreLhsTree) *allSymbolReferencesInStore |= aliases;
    } else {
      allSymbolReferences->set(symRef->getReferenceNumber());

      if (_inNullCheckReferenceSubtree)
        allSymbolReferencesInNullCheckReference->set(
  ...
  if (_isStoreTree) {
    if (node->getOpCode().isStore()) _inStoreLhsTree = false;
  }
  ...
  if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) &&
    ...
    if (opCode.hasSymbolReference() &&
      ...
      if (seenDefinedSymbolReferences->get(symRef->getReferenceNumber()) ||
        ...
        if (trace())
          traceMsg(
              comp(), "Expression %d killed (%x) by symRef #%d  %d\n",
              node->getLocalIndex(), node, symRef->getReferenceNumber(),
              seenDefinedSymbolReferences->get(symRef->getReferenceNumber()));
        _transparencyInfo[symRef->getReferenceNumber()]->reset(
            node->getLocalIndex());
...
void TR_LocalTransparency::adjustInfoForAddressAdd(
    TR::Node *node, TR::Node *child, ContainerType *seenDefinedSymbolReferences,
    ContainerType *seenStoredSymRefs) {
  bool childHasSupportedOpCode = false;
  ...
  if (childHasSupportedOpCode) {
        ...
        if (!_transparencyInfo[i]->get(child->getLocalIndex())) {
          ...
          if (trace()) {
            if (TR::Compiler->target.is64Bit())
              traceMsg(comp(),
                       "Expression %d killed by symRef #%d because grandchild "
                       "(child of aladd) %d is already killed by the symRef\n",
                       node->getLocalIndex(), i, child->getLocalIndex());
            else
              traceMsg(comp(),
                       "Expression %d killed by symRef #%d because grandchild "
                       "(child of aiadd) %d is already killed by the symRef\n",
                       node->getLocalIndex(), i, child->getLocalIndex());
          }
        }
      }
    ...
    if (child->getOpCode().isLoad() ||
      ...
      if (child->getOpCode().hasSymbolReference() &&
        ...
        if (seenDefinedSymbolReferences->get(
                childSymRef->getReferenceNumber()) ||
            seenStoredSymRefs->get(childSymRef->getReferenceNumber())) {
          _transparencyInfo[childSymRef->getReferenceNumber()]->reset(
              node->getLocalIndex());
          ...
          for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid();
            ...
            if (trace())
              traceMsg(comp(),
                       "999Expression %d killed (%x) by symRef #%d  %d\n",
                       node->getLocalIndex(), node, nextAlias,
                       seenDefinedSymbolReferences->get(nextAlias));
          }
          if (trace())
            traceMsg(
                comp(),
                "Expression %d killed by symRef #%d (loaded in grandchild)\n",
