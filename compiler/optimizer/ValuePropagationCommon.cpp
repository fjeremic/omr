...
void OMR::ValuePropagation::initialize()
{
    ...
    if (0 && !disablePREX && !disablePREXinVP && comp()->getMethodHotness() >= warm && comp()->couldBeRecompiled() && !comp()->getRecompilationInfo()->isProfilingCompilation() && !comp()->getOption(TR_DisableCHOpts)) {
        enablePreexistence();
    }
    ...
    if (((comp()->getMethodHotness() > warm) || !comp()->isOptServer()) && (!comp()->getOption(TR_DisableBlockVersioner))) {
        if (!comp()->getFlowGraph()->getStructure() && (((comp()->mayHaveLoops() && optimizer()->getLastRun(OMR::loopVersioner)) || !comp()->mayHaveLoops()) && optimizer()->getLastRun(OMR::basicBlockExtension))) {
...
void OMR::ValuePropagation::processTrees(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    if (_enableVersionBlocks && lastTimeThrough() && (startTree->getNode()->getOpCodeValue() == TR::BBStart)) {
        ...
        if (block->isCatchBlock() || (blockStructure && blockStructure->getContainingLoop() && !optimizer()->getLastRun(OMR::loopVersioner)) || (block->isCold()))
            _disableVersionBlockForThisBlock = true;
    ...
    for (treeTop = startTree; ((treeTop != endTree) && (treeTop != _curBlock->getExit())); treeTop = _curTree->getNextTreeTop()) {
        ...
        if (_enableVersionBlocks && !_disableVersionBlockForThisBlock && treeTop == lastRealTreeTop && !lastTtIsBndchk && lastTimeThrough()) {
...
#ifdef J9_PROJECT_SPECIFIC
static TR_ResolvedMethod* findResolvedClassMethod(TR::Compilation* comp, char* className, char* methodName, char* methodSig)
{
...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node* node)
{
    ...
    // removed.
    //
    if ((srcObject && srcObject->isNullObject()) || (dstObject && dstObject->isNullObject()) || srcOffHigh < 0 || dstOffHigh < 0 || copyLenHigh < 0 || (srcObject && srcObject->getClassType() && srcObject->getClassType()->asFixedClass() && (srcObject->getClassType()->asFixedClass()->isArray() == TR_no)) || (dstObject && dstObject->getClassType() && dstObject->getClassType()->asFixedClass() && (dstObject->getClassType()->asFixedClass()->isArray() == TR_no))) {
...
#ifdef J9_PROJECT_SPECIFIC
    bool isStringCompressedArrayCopy = recognizedMethod == TR::java_lang_String_compressedArrayCopy_BIBII || recognizedMethod == TR::java_lang_String_compressedArrayCopy_BICII || recognizedMethod == TR::java_lang_String_compressedArrayCopy_CIBII || recognizedMethod == TR::java_lang_String_compressedArrayCopy_CICII;

    bool isStringDecompressedArrayCopy = recognizedMethod == TR::java_lang_String_decompressedArrayCopy_BIBII || recognizedMethod == TR::java_lang_String_decompressedArrayCopy_BICII || recognizedMethod == TR::java_lang_String_decompressedArrayCopy_CIBII || recognizedMethod == TR::java_lang_String_decompressedArrayCopy_CICII;

    // If it is OK to convert this call to a possible call to the (fast)
    // arraycopy helper, do it.
    //
    if ((primitiveTransform || referenceTransform) && !comp()->getOption(TR_DisableArrayCopyOpts) && !node->isDontTransformArrayCopyCall() && comp()->fej9()->callTheJitsArrayCopyHelper() && !comp()->getOption(TR_DisableInliningOfNatives) && !(srcObject && srcObject->isNullObject()) && !(dstObject && dstObject->isNullObject()) && srcOffHigh >= 0 && dstOffHigh >= 0 && copyLenHigh >= 0) {
        ...
        if (!isRecognizedMultiLeafArrayCopy) {
            ...
            if (isStringCompressedArrayCopy || isStringDecompressedArrayCopy) {
                ...
                if (primitiveArray2) {
                    if (srcObject->getClassType() == dstObject->getClassType() || (srcObject->getClassType() && dstObject->getClassType() && srcObject->getClassType()->asResolvedClass() && dstObject->getClassType()->asResolvedClass() && srcObject->getClassType()->asResolvedClass()->getClass() == dstObject->getClassType()->asResolvedClass()->getClass())) {
                        needArrayCheck = false;
                    } else {
        ...
        if (primitiveArray1 || primitiveArray2) {
            type = primitiveArray1 ? srcObject->getClassType()->getPrimitiveArrayDataType() : dstObject->getClassType()->getPrimitiveArrayDataType();
        ...
        if (comp()->getOptions()->realTimeGC() && comp()->requiresSpineChecks() && (referenceArray1 || referenceArray2 || !(primitiveArray1 || primitiveArray2)))
            transformTheCall = false;

        if (transformTheCall && comp()->generateArraylets()) {
            if (!((primitiveArray1 || primitiveArray2) || (!comp()->getOption(TR_DisableRefArraycopyRT) && (referenceArray1 || referenceArray2)))) {
                transformTheCall = false;
            } else {
                ...
                if (!sameLeafSrc) {
                    if ((copyLenNode->getOpCode().isLoadConst() && srcOffNode->getOpCode().isLoadConst()) && !isRecognizedMultiLeafArrayCopy) {
                ...
                if (!sameLeafDst) {
                    if ((copyLenNode->getOpCode().isLoadConst() && dstOffNode->getOpCode().isLoadConst()) && !isRecognizedMultiLeafArrayCopy) {
                        if (!comp()->getOption(TR_DisableMultiLeafArrayCopy))
                            isMultiLeafArrayCopy = true;
                        else
                            transformTheCall = false;
    ...
    if (transformTheCall && performTransformation(comp(), "%sChanging call %s [%p] to arraycopy\n", OPT_DETAILS, node->getOpCode().getName(), node)) {
        ...
        if (!canSkipAllChecksOnArrayCopy) {
            ...
            //
#ifdef J9_PROJECT_SPECIFIC
            if (srcObjNode->getOpCode().hasSymbolReference() && srcObjNode->getSymbolReference()->getSymbol() && srcObjNode->getSymbolReference()->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_String_value) {
                isSrcPossiblyNull = false;
                isStringFwdArrayCopy = true;
            }

            if (dstObjNode->getOpCode().hasSymbolReference() && dstObjNode->getSymbolReference()->getSymbol() && dstObjNode->getSymbolReference()->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_String_value) {
        ...
        if (isMultiLeafArrayCopy) {
        ...
        } else if (((needSameLeafCheckForSrc || needSameLeafCheckForDst) || (comp()->generateArraylets() && comp()->getOptions()->realTimeGC() && !comp()->getOption(TR_DisableRefArraycopyRT) && (referenceArray1 || referenceArray2)))) {
            uint8_t flag = 0;
            ...
            if (((dstOffHigh == 0) || (srcOffLow >= dstOffHigh) || ((srcOffHigh <= TR::getMaxSigned<TR::Int32>() - copyLenHigh) && ((srcOffHigh + copyLenHigh) <= dstOffLow))) || isStringFwdArrayCopy)
        ...
        if ((dstOffHigh == 0) || (srcOffLow >= dstOffHigh) || ((srcOffHigh <= TR::getMaxSigned<TR::Int32>() - copyLenHigh) && ((srcOffHigh + copyLenHigh) <= dstOffLow)))
            node->setForwardArrayCopy(true);
...
*/
    ...
    if (isISO88591Encoder || isSBCSEncoder || isEncodeUtf16 || (rm == TR::sun_nio_cs_US_ASCII_Encoder_encodeASCII) || (rm == TR::sun_nio_cs_UTF_8_Encoder_encodeUTF_8))
        encode = true;
    ...
    if (isEncodeUtf16) {
        TR::SymbolReference* transformedCallSymRef = comp()->getSymRefTab()->methodSymRefFromName(
            comp()->getMethodSymbol(),
            "com/ibm/jit/JITHelpers",
            const_cast<char*>(TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Big == rm ? "transformedEncodeUTF16Big" : "transformedEncodeUTF16Little"),
            "(JJI)I",
...
void OMR::ValuePropagation::transformRealTimeArrayCopy(TR_RealTimeArrayCopy* rtArrayCopyTree)
{
    ...
    bool doMultiLeafArrayCopy = (!comp()->getOption(TR_DisableMultiLeafArrayCopy) && ((rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC) || (rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_DST)));
...
void OMR::ValuePropagation::transformObjectCloneCall(TR::TreeTop* callTree, OMR::ValuePropagation::ObjCloneInfo* cloneInfo)
{
    ...
    if (!cloneInfo->_isFixed) {
        ...
        TR::TreeTop* compareTree = TR::TreeTop::create(comp(), TR::Node::createif(TR::ifacmpne, TR::Node::createWithSymRef(callNode, TR::aloadi, 1, objNode, comp()->getSymRefTab()->findOrCreateVftSymbolRef()), TR::Node::createWithSymRef(callNode, TR::loadaddr, 0, comp()->getSymRefTab()->findOrCreateClassSymbol(callNode->getSymbolReference()->getOwningMethodSymbol(comp()), 0, j9class)), slowPath->getEntry()));
    ...
    TR::SymbolReference* helperAccessor = comp()->getSymRefTab()->methodSymRefFromName(
        comp()->getMethodSymbol(),
        "com/ibm/jit/JITHelpers",
        const_cast<char*>("jitHelpers"),
        const_cast<char*>("()Lcom/ibm/jit/JITHelpers;"),
        TR::MethodSymbol::Static);
    TR::SymbolReference* objCopySymRef = comp()->getSymRefTab()->methodSymRefFromName(
        comp()->getMethodSymbol(),
        "com/ibm/jit/JITHelpers",
...
void OMR::ValuePropagation::transformArrayCloneCall(TR::TreeTop* callTree, OMR::ValuePropagation::ArrayCloneInfo* cloneInfo)
{
    ...
    TR::Node* lengthInBytes = TR::Compiler->target.is64Bit() ? TR::Node::create(callNode, TR::lmul, 2, TR::Node::create(callNode, TR::i2l, 1, lenNode), TR::Node::lconst(lenNode, elementSize)) : TR::Node::create(callNode, TR::imul, 2, lenNode, TR::Node::iconst(lenNode, elementSize));

    TR::Node* srcStart = TR::Compiler->target.is64Bit() ? TR::Node::create(callNode, TR::aladd, 2, objNode, TR::Node::lconst(objNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes())) : TR::Node::create(callNode, TR::aiadd, 2, objNode, TR::Node::iconst(objNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
    TR::Node* destStart = TR::Compiler->target.is64Bit() ? TR::Node::create(callNode, TR::aladd, 2, newArray, TR::Node::lconst(newArray, TR::Compiler->om.contiguousArrayHeaderSizeInBytes())) : TR::Node::create(callNode, TR::aiadd, 2, newArray, TR::Node::iconst(newArray, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
...
void OMR::ValuePropagation::transformConverterCall(TR::TreeTop* callTree)
{
    ...
    if (callTree->getEnclosingBlock()->isCold() || (callTree->getEnclosingBlock()->getFrequency() == UNKNOWN_COLD_BLOCK_COUNT))
        return;
    ...
    TR::SymbolReference* symRefConvert = getSymRefTab()->createTemporary(comp()->getMethodSymbol(), callNode->getDataType(), false);
    ...
    if (comp()->getOSRMode() != TR::involuntaryOSR && comp()->getHCRMode() != TR::osr) {
        TR::TreeTop* actt = TR::TreeTop::create(comp(), TR::Node::createWithSymRef(callNode, TR::asynccheck, 0, getSymRefTab()->findOrCreateAsyncCheckSymbolRef(comp()->getMethodSymbol())));
...
TR::TreeTop* TR::LocalValuePropagation::processBlock(TR::TreeTop* startTree)
{
    ...
    if (comp()->getOption(TR_EnableLocalVPSkipLowFreqBlock)) {
        if (!((((comp()->getMethodHotness() < warm) && (_curBlock->getFrequency() > 1500)) || ((comp()->getMethodHotness() == warm) && (_curBlock->getFrequency() > 500)) || ((comp()->getMethodHotness() > warm) && (!_curBlock->isCold()))))) {
            if (trace())
