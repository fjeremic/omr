...
extern void collectArraylengthNodes(TR::Node *node, vcount_t visitCount,
...
OMR::ValuePropagation::ValuePropagation(TR::OptimizationManager *manager)
  ...
  // DANGER !!!
  // Virtual methods on ValuePropagation should only be called when the most
  // derived class of VP's constructor is completed for correct behavior. setVP
  // does not call any virtual method on VP, so it's ok to be here.
...
void OMR::ValuePropagation::initialize() {
  ...
  _preexistentObjectConstraint =
      new (trStackMemory()) TR::VPPreexistentObject();
  ...
  _constraintsHashTable =
      (ConstraintsHashTableEntry **)trMemory()->allocateStackMemory(
          VP_HASH_TABLE_SIZE * sizeof(ConstraintsHashTableEntry *));
  memset(_constraintsHashTable, 0,
         VP_HASH_TABLE_SIZE * sizeof(ConstraintsHashTableEntry *));
  ...
  _parmInfo =
      (int32_t *)trMemory()->allocateStackMemory(numParms * sizeof(int32_t));
  _parmTypeValid =
      (bool *)trMemory()->allocateStackMemory(numParms * sizeof(bool));
  // mark all parms as invariant (0) to begin with
  //
  memset(_parmInfo, 0, numParms * sizeof(int32_t));
  // mark info to be true
  for (int32_t i = 0; i < numParms; i++) _parmTypeValid[i] = true;
  ...
  while (curTree) {
    if (curTree->getNode())
      collectArraylengthNodes(curTree->getNode(), visitCount,
                              &_arraylengthNodes);
    curTree = curTree->getNextTreeTop();
  }

  if (_isGlobalPropagation) {
    // Determine the size of the _globalConstraintsHashTable based on the number
    // of nodes
    //
    uint32_t targetSize = (uint32_t)(
        comp()->getNodeCount() / 5);  // that 5 was experimentally determined
    const uint32_t globalConstraintsHashTableSizeUpperBound =
        2048;  // must be power of two
    uint32_t globalConstraintsHashTableSize =
        128;  // starting value; must be power of two
    if (targetSize >=
        globalConstraintsHashTableSizeUpperBound)  // test for upper bound
    ...
    _globalConstraintsHashTable =
        (GlobalConstraint **)trMemory()->allocateStackMemory(
            globalConstraintsHashTableSize * sizeof(GlobalConstraint *));
    memset(_globalConstraintsHashTable, 0,
           globalConstraintsHashTableSize * sizeof(GlobalConstraint *));

    _edgeConstraintsHashTable =
        (EdgeConstraints **)trMemory()->allocateStackMemory(
            VP_HASH_TABLE_SIZE * sizeof(EdgeConstraints *));
    memset(_edgeConstraintsHashTable, 0,
           VP_HASH_TABLE_SIZE * sizeof(EdgeConstraints *));

    _loopDefsHashTable =
        (LoopDefsHashTableEntry **)trMemory()->allocateStackMemory(
            VP_HASH_TABLE_SIZE * sizeof(LoopDefsHashTableEntry *));
    memset(_loopDefsHashTable, 0,
           VP_HASH_TABLE_SIZE * sizeof(LoopDefsHashTableEntry *));
  } else {
  ...
  _edgesToBeRemoved = new (trStackMemory())
      TR_Array<TR::CFGEdge *>(trMemory(), 8, false, stackAlloc);
  _blocksToBeRemoved = new (trStackMemory())
      TR_Array<TR::CFGNode *>(trMemory(), 8, false, stackAlloc);
  _curDefinedOnAllPaths = NULL;
  if (_isGlobalPropagation)
    _definedOnAllPaths = new (trStackMemory()) DefinedOnAllPathsMap(
        std::less<TR::CFGEdge *>(), trMemory()->currentStackRegion());
  else
  ...
  _valueConstraintCache = new (trStackMemory())
      TR_Stack<ValueConstraint *>(trMemory(), 256, false, stackAlloc);
  _loopInfo = NULL;
  ...
  //
  {
    ...
    _firstUnresolvedSymbolValueNumber =
        e ? atoi(e)
          : (comp()->getOption(TR_ProcessHugeMethods) ? 200000 : 100000);
  ...
  if (0 && !disablePREX && !disablePREXinVP &&
      comp()->getMethodHotness() >= warm && comp()->couldBeRecompiled() &&
  ...
  if (((comp()->getMethodHotness() > warm) || !comp()->isOptServer()) &&
    ...
    if (!comp()->getFlowGraph()->getStructure() &&
        (((comp()->mayHaveLoops() &&
           optimizer()->getLastRun(OMR::loopVersioner)) ||
          !comp()->mayHaveLoops()) &&
      ...
      if (doTiming) {
        ...
        if (comp()->getOutFile() != NULL) {
          trfprintf(comp()->getOutFile(),
                    "Time taken for %s = ", myTimer.title());
          trfprintf(comp()->getOutFile(), "%9.6f seconds\n",
                    myTimer.secondsTaken());
        }
#ifdef OPT_TIMING
        statStructuralAnalysisTiming.update(
            (double)myTimer.timeTaken() * 1000.0 /
            TR::Compiler->vm.getHighResClockResolution());
#endif
      }
    }
    ...
    if (comp()->getFlowGraph()->getStructure()) {
      ...
      // The bitVector is growable /bc when we devirtaulize calls we add symbol
      // references for the call and the parameters.
      _seenDefinedSymbolReferences = new (trStackMemory()) TR_BitVector(
          comp()->getSymRefCount(), trMemory(), stackAlloc, growable);
      _blocksToBeVersioned =
          new (trStackMemory()) TR_LinkHead<BlockVersionInfo>();
      _firstLoads =
          new (trStackMemory()) TR_LinkHead<FirstLoadOfNonInvariant>();
      _startEBB = NULL;
    }
  }

  _unsafeArrayAccessNodes = new (trStackMemory())
      TR_BitVector(comp()->getNodeCount(), trMemory(), stackAlloc, growable);
}

OMR::ValuePropagation::Relationship *OMR::ValuePropagation::copyRelationships(
    Relationship *first) {
...
OMR::ValuePropagation::copyStoreRelationships(StoreRelationship *first) {
  TR_LinkHeadAndTail<StoreRelationship> list;
  for (StoreRelationship *rel = first; rel; rel = rel->getNext()) {
    StoreRelationship *newRel = createStoreRelationship(
        rel->symbol, copyRelationships(rel->relationships.getFirst()));
    list.append(newRel);
  }
  return list.getFirst();
}
...
OMR::ValuePropagation::createValueConstraint(
    int32_t valueNumber, Relationship *relationships,
...
int32_t OMR::ValuePropagation::getValueNumber(TR::Node *node) {
  static const char *disableLocalVPConstNumbering =
      feGetEnv("TR_DisableLocalVPConstNumbering");
  if (_isGlobalPropagation)
    ...
    if (node->getOpCode().isStore()) {
      ...
      if (_constNodeInfo.Locate(value, index)) {
        ...
        TR::Node *result = NULL;
        for (auto nodeCursor = nodeList->begin();
             (result == NULL) && (nodeCursor != nodeList->end());
             ++nodeCursor) {
        ...
        TR::list<TR::Node *> *nodeList =
            new (comp()->trHeapMemory()) TR::list<TR::Node *>(
                getTypedAllocator<TR::Node *>(comp()->allocator()));
        nodeList->push_back(node);
...
TR::VPConstraint *OMR::ValuePropagation::getConstraint(TR::Node *node,
                                                       bool &isGlobal,
                                                       TR::Node *relative) {
  /*
    If we return a non-null constraint, isGlobal will be set accordingly.
    If we return a null constraint, isGlobal defaults to true: it is safe to
    globally assume that we know nothing about this node.
   */
  isGlobal = true;
  ...
  if (rel) {
    if (trace()) {
      traceMsg(comp(), "   %s [%p] has existing constraint:",
  ...
  //
  if (constraint) {
    TR::VPConstraint *betterConstraint =
        applyGlobalConstraints(node, valueNumber, constraint, relativeVN);
  ...
  if (rel) {
    if (trace()) {
      traceMsg(comp(), "   %s [%p] has existing global constraint:",
               node->getOpCode().getName(), node);
      rel->print(this, valueNumber, 1);
    }
    isGlobal = true;
...
void OMR::ValuePropagation::removeNode(TR::Node *node, bool anchorIt) {
  ...
  if (node->getReferenceCount() > 1) {
    ...
    //          TR::TreeTop::create(comp(), _curTree, TR::Node::create(
    //          TR::treetop, 1, node));
    //       node->decReferenceCount();

    // The above is wrong, put it _before_ the current tree.  By definition,
    // children must be evaluated before the parent, so they belong before the
    // parent tree. Also, we have to do this even if the curtree is the last
    // tree in the block, since the node may be commoned in the extended basic
    // block following
    //
  ...
  //
    ...
    if (optimizer()->prepareForNodeRemoval(
            node, /* deferInvalidatingUseDefInfo = */ true))
      invalidateUseDefInfo();

    if (node->getOpCode().isCheck()) setChecksRemoved();
...
static bool canCauseOSR(TR::TreeTop *tt, TR::Compilation *comp) {
  if (comp->isPotentialOSRPointWithSupport(tt)) return true;

  return false;
}

void OMR::ValuePropagation::processTrees(TR::TreeTop *startTree,
                                         TR::TreeTop *endTree) {
  TR::TreeTop *treeTop;
  ...
  if (_enableVersionBlocks && lastTimeThrough() &&
    ...
    if (!block->isExtensionOfPreviousBlock()) _startEBB = block;

    if (lastRealTreeTop->getNode()->getOpCode().isBndCheck())
      lastTtIsBndchk = true;

    if (block->isCatchBlock() ||
        (blockStructure && blockStructure->getContainingLoop() &&
         !optimizer()->getLastRun(OMR::loopVersioner)) ||
        (block->isCold()))
      _disableVersionBlockForThisBlock = true;
    ...
    if (nextStartBlock &&
        nextStartBlock->getNode()->getBlock()->isExtensionOfPreviousBlock())
      nextBlockIsExtentionOfThis = true;
  }

  for (treeTop = startTree;
       ((treeTop != endTree) && (treeTop != _curBlock->getExit()));
    ...
    if (_enableVersionBlocks && !_disableVersionBlockForThisBlock &&
        treeTop == lastRealTreeTop && !lastTtIsBndchk && lastTimeThrough()) {
    ...
    //
    if (!treeTop->getNode()) {
      if (_curTree == treeTop) _curTree = treeTop->getPrevTreeTop();
...
bool OMR::ValuePropagation::canTransformArrayCopyCallForSmall(
    TR::Node *node, int32_t &srcLength, int32_t &dstLength,
    int32_t &elementSize, TR::DataType &type) {
  ...
  static TR::DataType primitiveArrayToTRDataType[] = {
      TR::Int8, TR::Int16, TR::Float, TR::Double,
  ...
  // if signature is null then see if array was created in this method
  // (newarray).
  ...
  if (srcSig && srcSigLength >= 2 && *srcSig == '[') {
    ...
    srcLength = srcArrayNode->getFirstChild()->getOpCode().isLoadConst()
                    ? srcArrayNode->getFirstChild()->getInt()
                    : -1;
  } else {
    srcType = -1;
  }
  ...
  if (dstSig && dstSigLength >= 2 && *dstSig == '[') {
    ...
    dstLength = dstArrayNode->getFirstChild()->getOpCode().isLoadConst()
                    ? dstArrayNode->getFirstChild()->getInt()
                    : -1;
  } else {
    dstType = -1;
  }

  // check if both arrays are primitive types and of the same type, if so then
  // can transform the call for small (ie a primitive array copy)
...
#ifdef J9_PROJECT_SPECIFIC
static TR_ResolvedMethod *findResolvedClassMethod(TR::Compilation *comp,
                                                  char *className,
                                                  char *methodName,
                                                  char *methodSig) {
  TR_OpaqueClassBlock *classHandle = comp->fe()->getClassFromSignature(
      className, strlen(className), comp->getCurrentMethod());

  if (classHandle) {
    TR_ScratchList<TR_ResolvedMethod> classMethods(comp->trMemory());
    comp->fej9()->getResolvedMethods(comp->trMemory(), classHandle,
                                     &classMethods);
...
void OMR::ValuePropagation::removeArrayCopyNode(TR::TreeTop *arraycopyTree) {
  ListElement<TR_TreeTopWrtBarFlag> *elem =
      _unknownTypeArrayCopyTrees.getListHead();
  ListElement<TR_TreeTopWrtBarFlag> *prev = NULL;
  while (elem) {
    if (elem->getData()->_treetop == arraycopyTree) break;
    prev = elem;
  ...
  //
  ListElement<TR_ArrayCopySpineCheck> *acscElem =
      _arrayCopySpineCheck.getListHead();
  ListElement<TR_ArrayCopySpineCheck> *acscPrev = NULL;
  while (acscElem) {
    if (acscElem->getData()->_arraycopyTree == arraycopyTree) break;
    acscPrev = acscElem;
...
TR::Node *createHdrSizeNode(TR::Compilation *comp, TR::Node *n) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    hdrSize->setLongInt(
        (int64_t)TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
  } else
...
TR::Node *generateLenForArrayCopy(TR::Compilation *comp, int32_t elementSize,
                                  TR::Node *stride, TR::Node *srcObjNode,
  ...
  if (elementSize == 1) {
...
#ifdef J9_PROJECT_SPECIFIC
    if (!stride)
      stride = TR::TransformUtil::generateArrayElementShiftAmountTrees(
          comp, srcObjNode);
#endif
    ...
    if (is64BitTarget) {
      ...
      if (!stride) stride = TR::Node::create(n, TR::iconst, 0, elementSize);
...
bool OMR::ValuePropagation::canRunTransformToArrayCopy() {
  if (!lastTimeThrough()) return false;

  if ((comp()->getMethodHotness() >= hot) && !_isGlobalPropagation &&
      !getLastRun())
    return false;
...
bool OMR::ValuePropagation::transformUnsafeCopyMemoryCall(
    TR::Node *arraycopyNode) {
  if (!canRunTransformToArrayCopy()) return false;
  ...
  if (comp()->canTransformUnsafeCopyToArrayCopy() &&
      ...
      arraycopyNode->getSymbol()->castToMethodSymbol()->getRecognizedMethod() ==
          TR::sun_misc_Unsafe_copyMemory) {
    if ((ttNode->getOpCodeValue() == TR::treetop ||
         ttNode->getOpCode().isResolveOrNullCheck()) &&
        performTransformation(
            comp(), "%sChanging call Unsafe.copyMemory [%p] to arraycopy\n",
...

    {
      ...
      bool isGlobal;
      TR::VPConstraint *srcOffsetConstraint =
          getConstraint(srcOffset, isGlobal);
      TR::VPConstraint *dstOffsetConstraint =
          getConstraint(destOffset, isGlobal);
      TR::VPConstraint *copyLenConstraint = getConstraint(len, isGlobal);

      srcOffLow = srcOffsetConstraint ? srcOffsetConstraint->getLowInt()
                                      : TR::getMinSigned<TR::Int32>();
      srcOffHigh = srcOffsetConstraint ? srcOffsetConstraint->getHighInt()
                                       : TR::getMaxSigned<TR::Int32>();
      dstOffLow = dstOffsetConstraint ? dstOffsetConstraint->getLowInt()
                                      : TR::getMinSigned<TR::Int32>();
      dstOffHigh = dstOffsetConstraint ? dstOffsetConstraint->getHighInt()
                                       : TR::getMaxSigned<TR::Int32>();
      copyLenLow = copyLenConstraint ? copyLenConstraint->getLowInt()
                                     : TR::getMinSigned<TR::Int32>();
      copyLenHigh = copyLenConstraint ? copyLenConstraint->getHighInt()
                                      : TR::getMaxSigned<TR::Int32>();
...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node *node) {
  ...
  if (!node->isArrayCopyCall()) return;

  if (!canRunTransformToArrayCopy()) return;

  // Sanity check
  TR_ASSERT(node->getNumChildren() == 5,
            "Wrong number of arguments for arraycopy.");
  ...
  srcOffLow =
      srcOffset ? srcOffset->getLowInt() : TR::getMinSigned<TR::Int32>();
  srcOffHigh =
      srcOffset ? srcOffset->getHighInt() : TR::getMaxSigned<TR::Int32>();
  dstOffLow =
      dstOffset ? dstOffset->getLowInt() : TR::getMinSigned<TR::Int32>();
  dstOffHigh =
      dstOffset ? dstOffset->getHighInt() : TR::getMaxSigned<TR::Int32>();
  ...
  if ((srcObject && srcObject->isNullObject()) ||
      (dstObject && dstObject->isNullObject()) || srcOffHigh < 0 ||
      dstOffHigh < 0 || copyLenHigh < 0 ||
      (srcObject && srcObject->getClassType() &&
       srcObject->getClassType()->asFixedClass() &&
       (srcObject->getClassType()->asFixedClass()->isArray() == TR_no)) ||
      (dstObject && dstObject->getClassType() &&
       dstObject->getClassType()->asFixedClass() &&
  ...
  TR::RecognizedMethod recognizedMethod =
      node->getSymbol()->castToMethodSymbol()->getRecognizedMethod();
  ...
  if ((primitiveTransform || referenceTransform) &&
    ...
    if (comp()->generateArraylets() &&
      ...
      if (caller && strncmp(caller->nameChars(), sig, strlen(sig)) == 0) {
        if (trace())
          traceMsg(
              comp(),
              "Detected real-time same leaf arraycopy: %p from java helper\n",
              node);
    ...
    if (!isRecognizedMultiLeafArrayCopy) {
      ...
      if (isStringCompressedArrayCopy || isStringDecompressedArrayCopy) {
        ...
        if (referenceArray2) {
            ...
            if (sig && sig[0] == '[') {
              ...
              //
                ...
                isInstance = fe()->isInstanceOf(
                    srcObject->getClass(), dstObject->getClass(),
                    srcObject->isFixedClass(), dstObject->isFixedClass());

                if (isInstance == TR_yes) needArrayStoreCheck = false;
              }
            }
          }
          ...
          // Array types are different types so the arraycopy will fail
          transformTheCall = false;
        } else if (TR::Compiler->om.writeBarrierType() ==
                   gc_modron_wrtbar_always) {
          transformTheCall = false;
        }
      }
    }
    ...
    if (isStringCompressedArrayCopy) {
      ...
      // VP may not know anything about the types of the objects we are copying,
      // hence primitiveArray1 and and primitiveArray2 would both return false.
      // However because we are dealing with a recognized method we know that
      // the types must be primitive. As such we we can safely set the following
      // two variables without repercussions.
    ...
    if (comp()->getOptions()->realTimeGC() && comp()->requiresSpineChecks() &&
        (referenceArray1 || referenceArray2 ||
         !(primitiveArray1 || primitiveArray2)))
      transformTheCall = false;
      ...
      if (!((primitiveArray1 || primitiveArray2) ||
        ...
        if ((srcOffLow < 0) ||
          ...
          sameLeafSrc = false;
                 ...
                 fe()->getArrayletLeafIndex(srcOffHigh + copyLenHigh,
                                            elementSize))
          sameLeafSrc = true;
        ...
        if ((dstOffLow < 0) ||
          ...
          sameLeafDst = false;
                 ...
                 fe()->getArrayletLeafIndex(dstOffHigh + copyLenHigh,
                                            elementSize))
          sameLeafDst = true;
  ...
  if (transformTheCall &&
    ...
    bool canSkipAllChecksOnArrayCopy =
        methodSymbol->safeToSkipChecksOnArrayCopies() ||
        isRecognizedMultiLeafArrayCopy || isStringCompressedArrayCopy ||
    ...
    if (comp()->getMethodHotness() == hot && comp()->getRecompilationInfo() &&
      ...
      dumpOptDetails(
          comp(),
          "%s enabling profiling: it is useful to profile arraycopy calls\n",
          OPT_DETAILS);
    }
    ...
    if (!canSkipAllChecksOnArrayCopy) {
      ...
      // The String.value char array should never alias with the destination or
      // source array.  The String.value array is also never null.
      //
#ifdef J9_PROJECT_SPECIFIC
      ...
      if (needArrayCheck) {
        tempNode = TR::Node::createWithSymRef(
            TR::ArrayCHK, 2, 2, srcObjNode, dstObjNode,
            comp()->getSymRefTab()->findOrCreateArrayStoreExceptionSymbolRef(
                comp()->getMethodSymbol()));
      ...
      if (copyLenLow < 0) {
        if (!zeroConst) zeroConst = TR::Node::create(node, TR::iconst, 0, 0);
        checkNode =
            TR::Node::create(TR::ArrayCopyBNDCHK, 2, copyLenNode, zeroConst);
      ...
      if (srcOffLow < 0) {
        if (!zeroConst) zeroConst = TR::Node::create(node, TR::iconst, 0, 0);
        checkNode =
            TR::Node::create(TR::ArrayCopyBNDCHK, 2, srcOffNode, zeroConst);
        checkNode->setSymbolReference(bndChkSymRef);
        prevTree = TR::TreeTop::create(comp(), prevTree, checkNode);
        setEnableSimplifier();
      }
      ...
      if (dstOffLow < 0) {
        if (!zeroConst) zeroConst = TR::Node::create(node, TR::iconst, 0, 0);
        checkNode =
            TR::Node::create(TR::ArrayCopyBNDCHK, 2, dstOffNode, zeroConst);
        checkNode->setSymbolReference(bndChkSymRef);
        prevTree = TR::TreeTop::create(comp(), prevTree, checkNode);
        setEnableSimplifier();
      }
      ...
      if (needSrcBndChk) {
        ...
        checkNode =
            TR::Node::create(TR::ArrayCopyBNDCHK, 2, checkNode, srcOffNode);
        checkNode->setSymbolReference(bndChkSymRef);
        prevTree = TR::TreeTop::create(comp(), prevTree, checkNode);
        setEnableSimplifier();
      }
      ...
      if (needDstBndChk) {
        ...
        checkNode =
            TR::Node::create(TR::ArrayCopyBNDCHK, 2, checkNode, dstOffNode);
        checkNode->setSymbolReference(bndChkSymRef);
        prevTree = TR::TreeTop::create(comp(), prevTree, checkNode);
        setEnableSimplifier();
      }
    ...
    if (srcArrayLength) {
      ...
      if (stride != 0) srcArrayLength->setArrayStride(stride);
    }
    ...
    if (dstArrayLength) {
      ...
      if (stride != 0) dstArrayLength->setArrayStride(stride);
    }
    ...
    // If the arraycopy was removed from the trees (can happen if one of the
    // newly inserted check trees was proven to always fail), we should not
    // continue as we may affect the reference counts of srcObjNode etc. (the
    // removed arraycopy could be a parent of a node still in the trees, see
    // code below that uses srcObjNode etc.)
    //
    if (node->getReferenceCount() < 1) {
      return;
    }
    ...
    if (comp()->requiresSpineChecks()) {
      ...
      _arrayCopySpineCheck.add(new (trStackMemory()) TR_ArrayCopySpineCheck(
          arraycopyTree, node->getSymbolReference(), srcObjNode, srcOffNode,
          dstObjNode, dstOffNode, copyLenNode));
    }

    if (isMultiLeafArrayCopy) {
      uint8_t flag = 0;

      _needMultiLeafArrayCopy.add(new (trStackMemory()) TR_RealTimeArrayCopy(
          arraycopyTree, type, flag));
      ...
      goto createExceptionEdgeConstraintsAndReturn;
                ...
                (comp()->generateArraylets() &&
                 ...
                 (referenceArray1 || referenceArray2)))) {
      uint8_t flag = 0;
      if (needSameLeafCheckForSrc) flag |= NEED_RUNTIME_TEST_FOR_SRC;

      if (needSameLeafCheckForDst) flag |= NEED_RUNTIME_TEST_FOR_DST;

      if (needArrayStoreCheck) flag |= NEED_ARRAYSTORE_CHECK;

      if (needWriteBarrier) flag |= NEED_WRITE_BARRIER;
      ...
      _needRunTimeCheckArrayCopy.add(new (trStackMemory()) TR_RealTimeArrayCopy(
          arraycopyTree, type, flag));
    ...
    node->setNodeIsRecognizedArrayCopyCall(
        false);  // flag conflicts with isForwardArrayCopy
    ...
    if (!comp()->generateArraylets()) {
      ...
      src = generateArrayAddressTree(comp(), node, srcOffHigh, srcOffNode,
                                     srcObjNode, elementSize, stride, hdrSize);

      // -------------------------------------------------------------------
      // Calculate destination address node
      // -------------------------------------------------------------------

      dst = generateArrayAddressTree(comp(), node, dstOffHigh, dstOffNode,
                                     dstObjNode, elementSize, stride, hdrSize);
    } else {
      // aiadd
      //   iaload
      //       aiadd
      ...
      TR::Node *shiftNode =
          TR::Node::create(node, TR::iconst, 0, (int32_t)shift);

      int32_t strideShift = TR::TransformUtil::convertWidthToShift(elementSize);
      TR::Node *strideShiftNode =
          strideShift
              ? TR::Node::create(node, TR::iconst, 0, (int32_t)strideShift)
              : NULL;
      ...
      TR::Node *spineShiftNode =
          TR::Node::create(node, TR::iconst, 0, (int32_t)arraySpineShift);

      // -------------------------------------------------------------------
      // Calculate source address node
      // -------------------------------------------------------------------

      src = generateArrayletAddressTree(comp(), node, type, srcOff, srcObjNode,
                                        spineShiftNode, shiftNode,
      ...
      dst = generateArrayletAddressTree(comp(), node, type, dstOff, dstObjNode,
                                        spineShiftNode, shiftNode,
                                        strideShiftNode, hdrSize);
      ...
      if (comp()->useCompressedPointers()) {
        ...
        prevTree = TR::TreeTop::create(
            comp(), prevTree,
            TR::Node::createCompressedRefsAnchor(src->getFirstChild()));
        prevTree = TR::TreeTop::create(
            comp(), prevTree,
            TR::Node::createCompressedRefsAnchor(dst->getFirstChild()));
      }
    }
    ...
    len = generateLenForArrayCopy(comp(), elementSize, stride, srcObjNode,
                                  copyLenNode, node);
    ...
    if (primitiveArray1 || primitiveArray2) {
      ...
      // removeChildren might setNumChildren on src,dst,len if reference count
      // reaches 0
    ...
    if (referenceArray1 || referenceArray2) {
      if (needArrayStoreCheck) {
        _referenceArrayCopyTrees.add(
            new (trStackMemory()) TR_TreeTopWrtBarFlag(arraycopyTree, flag));
      } else {
      ...
      if (elementSize == 0 || needArrayStoreCheck || needWriteBarrier)
        _unknownTypeArrayCopyTrees.add(
            new (trStackMemory()) TR_TreeTopWrtBarFlag(arraycopyTree, flag));
      else
        changeExistingNodeToPrimitiveCopy = true;
    }
    ...
    if (changeExistingNodeToPrimitiveCopy) {
      ...
      if (existingNodeIsReferenceArraycopy)
        ...
        // So we need to make sure we set the element datatype appropriately to
        // enforce that
...
createExceptionEdgeConstraintsAndReturn:
  ...
  srcOffHigh =
      srcArrayInfo ? srcArrayInfo->highBound() : TR::getMaxSigned<TR::Int32>();
  addBlockConstraint(srcOffNode, TR::VPIntRange::create(this, 0, srcOffHigh));
  dstOffHigh =
      dstArrayInfo ? dstArrayInfo->highBound() : TR::getMaxSigned<TR::Int32>();
  addBlockConstraint(dstOffNode, TR::VPIntRange::create(this, 0, dstOffHigh));
  addBlockConstraint(copyLenNode, TR::VPIntRange::create(
                                      this, 0, TR::getMaxSigned<TR::Int32>()));
}

TR::Node *generateArrayAddressTree(TR::Compilation *comp, TR::Node *node,
                                   int32_t offHigh, TR::Node *offNode,
                                   TR::Node *objNode, int32_t elementSize,
                                   TR::Node *&stride, TR::Node *hdrSize) {
  bool is64BitTarget = TR::Compiler->target.is64Bit() ? true : false;
  ...
  if (offHigh > 0) {
    ...
    array = TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, array,
                             hdrSize);
  } else {
    array = hdrSize;
  }

  array = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, objNode,
                           array);
...
TR::TreeTop *OMR::ValuePropagation::createPrimitiveOrReferenceCompareNode(
    TR::Node *node) {
#ifdef J9_PROJECT_SPECIFIC
...
TR::TreeTop *OMR::ValuePropagation::createArrayStoreCompareNode(TR::Node *src,
                                                                TR::Node *dst) {
  TR::Node *vftDst = TR::Node::createWithSymRef(
      TR::aloadi, 1, 1, dst,
      comp()->getSymRefTab()->findOrCreateVftSymbolRef());
  ...
  TR::Node *cmp =
      TR::Node::createif(TR::ificmpeq, instanceofNode,
                         TR::Node::create(dst, TR::iconst, 0, 0), NULL);
  TR::TreeTop *cmpTree = TR::TreeTop::create(comp(), cmp);
  return cmpTree;
}

TR::TreeTop *OMR::ValuePropagation::createPrimitiveArrayNodeWithoutFlags(
  ...
  if (isOptimizedReferenceArraycopy) {
    // If the root node is a reference arraycopy that needs neither array store
    // check nor write barrier, it can be simplified into a three-child
    // arraycopy, but the array element type must be TR::Address.
    node->setArrayCopyElementType(TR::Address);
  } else {
    // If the root node is an arraycopy of unknown type, a run-time check will
    // have been inserted, and on this path we are generating a truly primitive
    // arraycopy, but the element type remains unknown.
    // root->getArrayCopyElementType() CANNOT be used, since the root node does
    // not have that information (we would not be here if it did).
    //
    // FIXME: Ideally we would like to set the type to TR::NoType here, to make
    // it clear that the code generator should not assume the arrays in question
    ...
    // currently not all codegens (e.g. z) handle an unknown-type arraycopy
    // correctly.
  ...
  if (trace())
    traceMsg(
        comp(), "Created 3-child arraycopy %s from root node %s, type = %s\n",
...
TR::TreeTop *createStoresForArraycopyChildren(TR::Compilation *comp,
  ...
  if (storeTree) insertBefore = storeTree;
...
#ifdef J9_PROJECT_SPECIFIC
void OMR::ValuePropagation::generateArrayTranslateNode(
    TR::TreeTop *callTree, TR::TreeTop *arrayTranslateTree,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
    TR::SymbolReference *srcOffRef, TR::SymbolReference *dstOffRef,
    TR::SymbolReference *lenRef, TR::SymbolReference *tableRef, bool hasTable) {
  bool is64BitTarget = TR::Compiler->target.is64Bit();
  ...
  bool isISO88591Encoder =
      (rm == TR::sun_nio_cs_ISO_8859_1_Encoder_encodeISOArray ||
       rm == TR::java_lang_StringCoding_implEncodeISOArray);
  bool isISO88591Decoder =
      (rm == TR::sun_nio_cs_ISO_8859_1_Decoder_decodeISO8859_1);
  bool isSBCSEncoder =
      (rm == TR::sun_nio_cs_ext_SBCS_Encoder_encodeSBCS) ? true : false;
  bool isSBCSDecoder =
      (rm == TR::sun_nio_cs_ext_SBCS_Decoder_decodeSBCS) ? true : false;
  bool isEncodeUtf16 = (rm == TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Big ||
                        rm == TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Little);

  int32_t childId = callNode->getFirstArgumentIndex();
  if (callNode->getChild(childId)->getType().isAddress() &&
      callNode->getChild(childId + 1)->getType().isAddress())
    childId++;

  TR::Node *srcObj = srcRef ? TR::Node::createLoad(callNode, srcRef)
                            : callNode->getChild(childId++)->duplicateTree();
  TR::Node *srcOffNode = srcOffRef
                             ? TR::Node::createLoad(callNode, srcOffRef)
                             : callNode->getChild(childId++)->duplicateTree();
  TR::Node *srcOff = srcOffNode->createLongIfNeeded();
  TR::Node *lenNode = NULL;
  TR::Node *len = NULL;
  if (!isISO88591Encoder) {
    lenNode = lenRef ? TR::Node::createLoad(callNode, lenRef)
                     : callNode->getChild(childId++)->duplicateTree();
    len = lenNode->createLongIfNeeded();
  }

  TR::Node *dstObj = dstRef ? TR::Node::createLoad(callNode, dstRef)
                            : callNode->getChild(childId++)->duplicateTree();
  TR::Node *dstOffNode = dstOffRef
                             ? TR::Node::createLoad(callNode, dstOffRef)
                             : callNode->getChild(childId++)->duplicateTree();
  TR::Node *dstOff = dstOffNode->createLongIfNeeded();
  ...
  // hasTable is set if the table was passed in as an argument to the method,
  // i.e. passed by the java call
  ...
  TR::Node *arrayTranslateNode = TR::Node::create(
      arrayTranslateTree->getNode()->getFirstChild(), TR::arraytranslate, 6);
  removeNode(arrayTranslateTree->getNode()->getFirstChild());

  /*
     TR::Node* arrayTranslateNode =
     arrayTranslateTree->getNode()->getFirstChild();
     TR::Node::recreate(arrayTranslateNode, TR::arraytranslate);
     arrayTranslateNode->setNumChildren(6);
  */
  arrayTranslateNode->setSymbolReference(
  ...
  if (encode) {
    node = TR::Node::create(is64BitTarget ? TR::lmul : TR::imul, 2, srcOff,
                            strideNode);
    node =
        TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, node, hdrSize);
    src = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, srcObj,
                           node);
    node = TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, dstOff,
                            hdrSize);
    dst = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, dstObj,
                           node);
    arrayTranslateNode->setSourceIsByteArrayTranslate(false);
    arrayTranslateNode->setTargetIsByteArrayTranslate(true);
  } else {
    node = TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, srcOff,
                            hdrSize);
    src = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, srcObj,
                           node);
    node = TR::Node::create(is64BitTarget ? TR::lmul : TR::imul, 2, dstOff,
                            strideNode);
    node =
        TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, node, hdrSize);
    dst = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, dstObj,
  ...
  if (encode && !isSBCSEncoder) {
    ...
    if (cg()->getSupportsArrayTranslateTRTO255() ||
        cg()->getSupportsArrayTranslateTRTO()) {
    ...
    {
      bool genSIMD = comp()->cg()->getSupportsVectorRegisters() &&
                     !comp()->getOption(TR_DisableSIMDArrayTranslate);
      stopIndex = isISO88591Encoder ? 255 : 127;
      termchar = isISO88591Encoder ? 0x0B : 0xff;

      if (genSIMD) {
        tableNode =
            TR::Node::create(callNode, TR::aconst, 0,
                             0);  // dummy table node, it's not gonna be used
      } else {
        uint8_t *table =
            (uint8_t *)comp()->trMemory()->allocateMemory(65536, stackAlloc);
        int i;
        for (i = 0; i <= stopIndex; i++) table[i] = (uint8_t)i;
        for (i = stopIndex + 1; i < 65536; i++) table[i] = (uint8_t)termchar;
    ...
    if (cg()->getSupportsArrayTranslateTROTNoBreak() ||
        cg()->getSupportsArrayTranslateTROT()) {  // X or P
      ...
      tableNode =
          TR::Node::create(callNode, TR::iconst, 0,
                           0);  // dummy table node, it's not gonna be used
    } else {                    // Z
      bool genSIMD = comp()->cg()->getSupportsVectorRegisters() &&
                     !comp()->getOption(TR_DisableSIMDArrayTranslate);
      ...
      if (genSIMD) {
        ...
        for (i = 128; i < 256; i++) table[i] = isISO88591Decoder ? i : -1;
  ...
  } else if (isSBCSEncoder)  // only z
  {
    ...
    TR::Node *tableNodeAddr =
        tableRef ? TR::Node::createLoad(callNode, tableRef)
                 : callNode->getChild(childId++)->duplicateTree();
    tableNode = TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2,
                                 tableNodeAddr, hdrSize);
  }
  ...
  if (isEncodeUtf16) {
    TR::SymbolReference *transformedCallSymRef =
        comp()->getSymRefTab()->methodSymRefFromName(
            comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
            const_cast<char *>(TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Big ==
                                       rm
                                   ? "transformedEncodeUTF16Big"
                                   : "transformedEncodeUTF16Little"),
            "(JJI)I", TR::MethodSymbol::Static);

    arrayTranslateNode = TR::Node::createWithSymRef(
        callNode, callNode->getOpCodeValue(), 3, transformedCallSymRef);
  }
  ...
  // arrayTranslateNode->setChild(5, NULL);//* do I need this? what if there are
  // more children?*/
...
TR::TreeTop *OMR::ValuePropagation::createConverterCallNodeAfterStores(
    TR::TreeTop *tree, TR::TreeTop *origTree, TR::SymbolReference *srcRef,
    TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
    TR::SymbolReference *srcOffRef, TR::SymbolReference *dstOffRef,
    TR::SymbolReference *thisRef, TR::SymbolReference *tableRef) {
  TR::Node *root = tree->getNode()->getFirstChild();
  ...
  if (lenRef)
    ...
    len = isISO88591Encoder ? root->getChild(childId + 4)->duplicateTree()
                            : root->getChild(childId + 2)->duplicateTree();

  isISO88591Encoder ? root->setAndIncChild(childId + 4, len)
                    : root->setAndIncChild(childId + 2, len);

  if (dstRef)
    dst = TR::Node::createLoad(root, dstRef);
  else
    dst = isISO88591Encoder ? root->getChild(childId + 2)->duplicateTree()
                            : root->getChild(childId + 3)->duplicateTree();

  isISO88591Encoder ? root->setAndIncChild(childId + 2, dst)
                    : root->setAndIncChild(childId + 3, dst);

  if (dstOffRef)
    dstOff = TR::Node::createLoad(root, dstOffRef);
  else
    dstOff = isISO88591Encoder ? root->getChild(childId + 3)->duplicateTree()
                               : root->getChild(childId + 4)->duplicateTree();

  isISO88591Encoder ? root->setAndIncChild(childId + 3, dstOff)
...
//-------------------------- realtime support
TR::TreeTop *OMR::ValuePropagation::buildSameLeafTest(
    TR::Node *offset, TR::Node *len, TR::Node *spineShiftNode) {
  ...
  child1 = TR::Node::create(is64BitTarget ? TR::lshr : TR::ishr, 2, offset,
                            spineShiftNode);
  child2 =
      TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, offset, len);
  child2 = TR::Node::create(is64BitTarget ? TR::lshr : TR::ishr, 2, child2,
                            spineShiftNode);
  ifNode = TR::Node::createif(is64BitTarget ? TR::iflcmpne : TR::ificmpne,
                              child1, child2);

  // check with this :ifNode = TR::Node::createif(TR::ificmpne,
  // TR::Node::create(offset , TR::iconst, 0, 1), TR::Node::create(offset ,
  // TR::iconst, 1, 1));
...
TR::Node *generateArrayletAddressTree(
    TR::Compilation *comp, TR::Node *vcallNode, TR::DataType type,
    TR::Node *off, TR::Node *obj, TR::Node *spineShiftNode, TR::Node *shiftNode,
    TR::Node *strideShiftNode, TR::Node *hdrSize) {
  bool is64BitTarget = TR::Compiler->target.is64Bit() ? true : false;
  ...
  node = TR::Node::create(is64BitTarget ? TR::lshr : TR::ishr, 2, off,
                          spineShiftNode);
  node =
  ...
  if (is64BitTarget) {
    ...
    leafOffset =
        TR::Node::create(vcallNode, TR::iconst, 0,
                         (int32_t)comp->fe()->getArrayletMask(elementSize));
  }

  TR::Node *nodeLeafOffset =
      TR::Node::create(is64BitTarget ? TR::land : TR::iand, 2, leafOffset, off);
...
void OMR::ValuePropagation::generateRTArrayNodeWithoutFlags(
    TR_RealTimeArrayCopy *rtArrayCopy, TR::TreeTop *dupArraycopyTree,
    TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
    TR::SymbolReference *srcOffRef, TR::SymbolReference *dstOffRef,
    TR::SymbolReference *lenRef, bool primitive) {
  ...
  TR::Node *hdrSize = createHdrSizeNode(comp(), node);
  TR::Node *spineShiftNode = TR::Node::create(
      node, TR::iconst, 0, (int32_t)fe()->getArraySpineShift(elementSize));
  ...
  srcOff = generateArrayletAddressTree(comp(), node, type, srcOff, srcObj,
                                       spineShiftNode, shiftNode,
                                       strideShiftNode, hdrSize);
  dstOff = generateArrayletAddressTree(comp(), node, type, dstOff, dstObj,
                                       spineShiftNode, shiftNode,
                                       strideShiftNode, hdrSize);
  ...
  arraycopyNode->setNodeIsRecognizedArrayCopyCall(
      false);  // flag conflicts with isForwardArrayCopy
...
TR::TreeTop *OMR::ValuePropagation::createArrayCopyVCallNodeAfterStores(
    TR::TreeTop *tree, TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
    TR::SymbolReference *lenRef, TR::SymbolReference *srcOffRef,
  ...
  // duplicate the tree just to copy either the ResolveCHK or the tree-top
  // root->removeAllChildren(); no need because the children were not countable
  // for refCount
...
TR::TreeTop *createStoresForConverterCallChildren(
    TR::Compilation *comp, TR::TreeTop *callTree, TR::SymbolReference *&srcRef,
    TR::SymbolReference *&dstRef, TR::SymbolReference *&srcOffRef,
    TR::SymbolReference *&dstOffRef, TR::SymbolReference *&lenRef,
    TR::SymbolReference *&thisRef, TR::TreeTop *insertBefore) {
  ...
  TR::Node *len = NULL;
  if (!isISO88591Encoder) len = node->getChild(childId++);
  ...
  if (hasReciever) {
    storeTree =
        node->getChild(thisChildId)->createStoresForVar(thisRef, insertBefore);
    if (storeTree) insertBefore = storeTree;
  }
  return insertBefore;
}

TR::TreeTop *createStoresForArraycopyVCallChildren(
    TR::Compilation *comp, TR::TreeTop *vcallTree, TR::SymbolReference *&srcRef,
    TR::SymbolReference *&dstRef, TR::SymbolReference *&srcOffRef,
    TR::SymbolReference *&dstOffRef, TR::SymbolReference *&lenRef,
    TR::TreeTop *insertBefore) {
  TR::Node *node = vcallTree->getNode();
  if (node->getOpCodeValue() != TR::call) node = node->getFirstChild();
...
void createAndInsertTestBlock(TR::Compilation *comp, TR::TreeTop *ifTree,
                              TR::TreeTop *tree, TR::Block *insertAfterBlock,
                              TR::Block *dstBlock) {
  TR::CFG *cfg = comp->getFlowGraph();

  TR::Block *ifBlock =
      TR::Block::createEmptyBlock(tree->getNode(), comp, 0, insertAfterBlock);
  ifBlock->append(ifTree);

  TR::TreeTop::insertTreeTops(comp, insertAfterBlock->getExit(),
                              ifBlock->getEntry(), ifBlock->getExit());
  ifTree->getNode()->setBranchDestination(dstBlock->getEntry());

  cfg->addNode(ifBlock);
  cfg->addEdge(
      TR::CFGEdge::createEdge(insertAfterBlock, ifBlock, comp->trMemory()));
  cfg->addEdge(TR::CFGEdge::createEdge(ifBlock, dstBlock, comp->trMemory()));
  cfg->addEdge(TR::CFGEdge::createEdge(ifBlock, ifBlock->getNextBlock(),
                                       comp->trMemory()));
...
// Check if the contiguous size field is zero, which indicates the array is
// discontiguous.
//
TR::TreeTop *OMR::ValuePropagation::createSpineCheckNode(
    TR::Node *node, TR::SymbolReference *objSymRef) {
  TR::Node *objNode = TR::Node::createLoad(node, objSymRef);
  TR::Node *sizeNode = TR::Node::createWithSymRef(
      TR::iloadi, 1, 1, objNode,
      comp()->getSymRefTab()->findOrCreateContiguousArraySizeSymbolRef());
  TR::Node *cmpNode = TR::Node::createif(
      TR::ificmpeq, sizeNode, TR::Node::create(node, TR::iconst, 0, 0), NULL);
...
TR::TreeTop *OMR::ValuePropagation::createAndInsertStoresForArrayCopySpineCheck(
  ...
  storeTree = checkInfo->_srcObjNode->createStoresForVar(
      checkInfo->_srcObjRef, insertBeforeTree, true);
  storeTree = checkInfo->_srcOffNode->createStoresForVar(checkInfo->_srcOffRef,
                                                         insertBeforeTree);
  storeTree = checkInfo->_dstObjNode->createStoresForVar(
      checkInfo->_dstObjRef, insertBeforeTree, true);
  storeTree = checkInfo->_dstOffNode->createStoresForVar(checkInfo->_dstOffRef,
                                                         insertBeforeTree);
  storeTree = checkInfo->_copyLenNode->createStoresForVar(
      checkInfo->_copyLenRef, insertBeforeTree);
...
TR::TreeTop *OMR::ValuePropagation::createArrayCopyCallForSpineCheck(
  ...
  if (acNode->getOpCodeValue() != TR::arraycopy) {
    ...
    //      TR_ASSERT((acNode->getOpCodeValue() == TR::arraycopy) ||
    //      acNode->getOpCode().isStore(), "unexpected opcode");
  }
  ...
  TR::Node *callNode = TR::Node::createWithSymRef(acNode, TR::call, 5,
                                                  checkInfo->_arraycopySymRef);
...
void OMR::ValuePropagation::transformArrayCopySpineCheck(
    TR_ArrayCopySpineCheck *checkInfo) {
  TR::CFG *cfg = comp()->getFlowGraph();
  ...
  //
  TR::TreeTop *lastTree =
      createAndInsertStoresForArrayCopySpineCheck(checkInfo);
  ...
  //
  TR::TreeTop *srcCheckIfTree =
      createSpineCheckNode(checkInfo->_srcObjNode, checkInfo->_srcObjRef);
  ...
  //
  TR::TreeTop *dstCheckIfTree =
      createSpineCheckNode(checkInfo->_dstObjNode, checkInfo->_dstObjRef);
  ...
  //
  TR::Block *remainderBlock =
      elseBlock2->split(arraycopyTree->getNextTreeTop(), cfg, true);

  // Create the 'if' block to do the arraycopy call.
  //
  TR::Block *ifBlock = TR::Block::createEmptyBlock(arraycopyTree->getNode(),
                                                   comp(), 0, remainderBlock);
  ...
  ifBlock->append(TR::TreeTop::create(
      comp(), TR::Node::create(arraycopyTree->getNode(), TR::Goto, 0,
                               remainderBlock->getEntry())));
...
void OMR::ValuePropagation::transformRealTimeArrayCopy(
  ...
  if (vcallNode->getReferenceCount() < 1) return;
  ...
  // for testing purpose can create a dummy slow version without call (instead
  // of the real call) TR::Node *dummyNode1 = TR::Node::create(TR::treetop, 1,
  // TR::Node::create( vcallNode, TR::iconst, 0, 0));
  ...
  //  TR::Node *dummyNode2 = TR::Node::create(TR::treetop, 1,
  //  TR::Node::create(vcallNode, TR::iconst, 0, 0));
  ...
  // create ifNodes
  TR::Node *spineShiftNode = TR::Node::create(
      vcallNode, TR::iconst, 0, (int32_t)fe()->getArraySpineShift(elementSize));
  TR::TreeTop *ifTree;
  TR::Node *ifNode;
  ...
  if (type != TR::Address || !comp()->getOptions()->realTimeGC()) {
    ...
    // srcOFf dstOff are only indicators for runtime tests needed, so make sure
    // they are not null without incrementing ref count
    if (rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC) {
      //      printf("needsSrc\n");
    ...
    TR::Node *fragmentParent = TR::Node::createWithSymRef(
        vcallNode, TR::aload, 0,
        comp()->getSymRefTab()->findOrCreateFragmentParentSymbolRef());
    if (TR::Compiler->target.is64Bit()) {
      TR::Node *globalFragment = TR::Node::createWithSymRef(
          TR::lloadi, 1, 1, fragmentParent,
          comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
      ifTree = TR::TreeTop::create(
          comp(),
          TR::Node::createif(TR::iflcmpne, globalFragment,
                             TR::Node::create(vcallNode, TR::lconst, 0, 0)));
    } else {
      TR::Node *globalFragment = TR::Node::createWithSymRef(
          TR::iloadi, 1, 1, fragmentParent,
          comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
      ifTree = TR::TreeTop::create(
          comp(),
          TR::Node::createif(TR::ificmpne, globalFragment,
                             TR::Node::create(vcallNode, TR::iconst, 0, 0)));
    }
  ...
  TR::Node *dummyNode = TR::Node::create(
      TR::treetop, 1, TR::Node::create(vcallNode, TR::iconst, 0, 0));
  ...
  origCallBlock->createConditionalBlocksBeforeTree(
      dummyTT, ifTree, dupVCallTree, dupArraycopyTree, cfg, false);

  // if (trace())
  //  comp()->dumpMethodTrees("Trees after createConditional");
  createStoresForArraycopyVCallChildren(comp(), vcallTree, srcRef, dstRef,
                                        srcOffRef, dstOffRef, lenRef,
                                        vcallTree);
  ...
  // if this is tarok, depending on wrtbar we create a primitive arraycopy or
  // reference arraycopy with no store check flag. for realtime the fast path is
  // primitive (if we got there there is no need for wrtbar)
  if (!comp()->getOptions()->realTimeGC() &&
      ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
    // tarok, need wrtbar
    generateRTArrayNodeWithoutFlags(rtArrayCopyTree, dupArraycopyTree, srcRef,
                                    dstRef, srcOffRef, dstOffRef, lenRef, 0);
    dupArraycopyTree->getNode()->getFirstChild()->setNoArrayStoreCheckArrayCopy(
        true);
    if (comp()->useCompressedPointers()) {
      TR::TreeTop *prevTree = dupArraycopyTree->getPrevTreeTop();
      TR_ASSERT((dupArraycopyTree->getNode()
                     ->getFirstChild()
                     ->getChild(2)
                     ->getFirstChild()
      ...
      prevTree = TR::TreeTop::create(
          comp(), prevTree,
          TR::Node::createCompressedRefsAnchor(dupArraycopyTree->getNode()
                                                   ->getFirstChild()
                                                   ->getChild(2)
                                                   ->getFirstChild()));
      prevTree = TR::TreeTop::create(
    ...
    generateRTArrayNodeWithoutFlags(rtArrayCopyTree, dupArraycopyTree, srcRef,
                                    dstRef, srcOffRef, dstOffRef, lenRef, 1);
    if (comp()->useCompressedPointers()) {
      TR::TreeTop *prevTree = dupArraycopyTree->getPrevTreeTop();
      TR_ASSERT((dupArraycopyTree->getNode()
                     ->getFirstChild()
  ...
  createArrayCopyVCallNodeAfterStores(dupVCallTree, srcRef, dstRef, lenRef,
                                      srcOffRef, dstOffRef);
  ...
  if (type == TR::Address && comp()->getOptions()->realTimeGC() &&
      doMultiLeafArrayCopy) {
    createArrayCopyVCallNodeAfterStores(origDupVCallTree, srcRef, dstRef,
                                        lenRef, srcOffRef, dstOffRef);
    slowArraycopyBlock = TR::Block::createEmptyBlock(
        origDupVCallTree->getNode(), comp(), origCallBlock->getFrequency(),
        origCallBlock);
    slowArraycopyBlock->append(origDupVCallTree);

    TR::Block *succ = fastArraycopyBlock->getNextBlock();
    slowArraycopyBlock->append(TR::TreeTop::create(
        comp(), TR::Node::create(origDupVCallTree->getNode(), TR::Goto, 0,
                                 succ->getEntry())));
  ...
  if ((type == TR::Address) &&
    ...
    // We know there is no wrtbar needed, so the fast path will be arraycopy
    // with 3 children, while the slow will be arraycopy with 5 children with
    // arrayStore check.
    ...
    if (!comp()->getOptions()->realTimeGC() &&
        ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
      dupRefArraycopyNode = TR::Node::createArraycopy(
          dupVCallNode->getChild(0)->duplicateTree(),
          dupVCallNode->getChild(2)->duplicateTree(),
          arraycopyNode->getChild(2)->duplicateTree(),
          arraycopyNode->getChild(3)->duplicateTree(),
          arraycopyNode->getChild(4)->duplicateTree());
    } else {  // consturct from primitive
      dupRefArraycopyNode = TR::Node::createArraycopy(
          dupVCallNode->getChild(0)->duplicateTree(),
          dupVCallNode->getChild(2)->duplicateTree(),
          arraycopyNode->getChild(0)->duplicateTree(),
          arraycopyNode->getChild(1)->duplicateTree(),
          arraycopyNode->getChild(2)->duplicateTree());
    }
    dupRefArraycopyNode->setNumChildren(5);
    dupRefArraycopyNode->setSymbolReference(
        arraycopyNode->getSymbolReference());
    dupRefArraycopyNode->setFlags(arraycopyNode->getFlags());
    dupRefArraycopyTree->setNode(
        TR::Node::create(TR::treetop, 1, dupRefArraycopyNode));

    TR::Block *referenceArraycopyBlock = TR::Block::createEmptyBlock(
        dupRefArraycopyNode, comp(), origCallBlock->getFrequency(),
        origCallBlock);
    if (!(rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC ||
          rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_DST)) {
    ...
    if (comp()->useCompressedPointers()) {
      ...
      TR_ASSERT((dupRefArraycopyTree->getNode()
                     ->getFirstChild()
                     ->getChild(2)
                     ->getFirstChild()
                     ->getOpCodeValue() == TR::aloadi),
      ...
      prevTree = TR::TreeTop::create(
          comp(), prevTree,
          TR::Node::createCompressedRefsAnchor(dupRefArraycopyTree->getNode()
                                                   ->getFirstChild()
                                                   ->getChild(2)
                                                   ->getFirstChild()));
      prevTree = TR::TreeTop::create(
    ...
    TR::Block *succ = fastArraycopyBlock->getNextBlock();
    referenceArraycopyBlock->append(TR::TreeTop::create(
        comp(),
        TR::Node::create(dupArraycopyNode, TR::Goto, 0, succ->getEntry())));

    cfg->addNode(referenceArraycopyBlock);
    cfg->findLastTreeTop()->join(referenceArraycopyBlock->getEntry());
    cfg->addEdge(
        TR::CFGEdge::createEdge(referenceArraycopyBlock, succ, trMemory()));
    ...
    createAndInsertTestBlock(comp(), newIfTree, dupArraycopyTree, origCallBlock,
                             referenceArraycopyBlock);
  }
  ...
  if (dstOff && ((type == TR::Address) || srcOff)) {
    TR::Node *spineShiftNode2 =
        TR::Node::create(vcallNode, TR::iconst, 0,
                         (int32_t)fe()->getArraySpineShift(elementSize));
    ...
    createAndInsertTestBlock(comp(), ifDstTree, dupArraycopyTree, origCallBlock,
                             slowArraycopyBlock);
  }
  ...
  if ((type == TR::Address) && srcOff && comp()->getOptions()->realTimeGC()) {
    ...
    createAndInsertTestBlock(comp(), ifSrcTree, dupArraycopyTree, origCallBlock,
                             slowArraycopyBlock);
  }
  ...
  if (doMultiLeafArrayCopy)
    _needMultiLeafArrayCopy.add(new (trStackMemory()) TR_RealTimeArrayCopy(
        origDupVCallTree, type, flag));
  else
    origDupVCallTree->getNode()
        ->getFirstChild()
        ->setDontTransformArrayCopyCall();
...
#ifdef J9_PROJECT_SPECIFIC
void OMR::ValuePropagation::transformRTMultiLeafArrayCopy(
    TR_RealTimeArrayCopy *rtArrayCopyTree) {
  ...
  TR::ResolvedMethodSymbol *methodSymbol =
      comp()->getOwningMethodSymbol(method);
  TR_OpaqueClassBlock *helperClass =
      comp()->getSystemClassPointer();  // fe()->getClassFromSignature("RTArrayCopy",
                                        // 11, method);

  if (trace()) traceMsg(comp(), " class = %p\n", helperClass);
  ...
  for (TR_ResolvedMethod *m = it.getCurrent(); m && !helperSymRef;
       m = it.getNext()) {
    char *sig = m->nameChars();
    if (trace()) traceMsg(comp(), " sig = %s\n", sig);

    if (!strncmp(sig, "multiLeafArrayCopy", 18))
      helperSymRef = getSymRefTab()->findOrCreateMethodSymbol(
          JITTED_METHOD_INDEX, -1, m, TR::MethodSymbol::Static);
  }

  if (trace()) traceMsg(comp(), " helper sym = %p\n", helperSymRef);
  ...
  TR::Node *newCallNode = TR::Node::createWithSymRef(
      TR::call, 7, 7, vcallNode->getChild(0), vcallNode->getChild(1),
      vcallNode->getChild(2), vcallNode->getChild(3), vcallNode->getChild(4),
      extraChild1, extraChild2, helperSymRef);
  TR::TransformUtil::removeTree(comp(), vcallTree);
  TR::TreeTop *newCallTree = TR::TreeTop::create(
      comp(), prevTree, TR::Node::create(TR::treetop, 1, newCallNode));
...
void OMR::ValuePropagation::transformObjectCloneCall(
  ...
  if (disableObjectCloneOpt) return;
  ...
  if (method->getRecognizedMethod() ==
      TR::java_lang_J9VMInternals_primitiveClone)
    objNode = callNode->getLastChild();

  if (!performTransformation(comp(),
                             "%sInlining object clone call [%p] as new object "
                             "and JITHelpers object copy\n",
  ...
  TR::DebugCounter::prependDebugCounter(
      ...
      TR::DebugCounter::debugCounterName(
          comp(), "inlineClone.location/object/(%s)", comp()->signature()),
      callTree);
  int32_t classNameLength;
  char *className =
      TR::Compiler->cls.classNameChars(comp(), j9class, classNameLength);
  TR::DebugCounter::prependDebugCounter(
      comp(),
      TR::DebugCounter::debugCounterName(
          comp(), "inlineClone.type/(%s)/(%s)/%s", className,
          comp()->signature(),
  ...
  prepareToReplaceNode(
      callNode);  // This will remove the usedef info, valueNumber info and all
                  // children of the node

  if (callTree->getNode()->getOpCode().isCheck()) {
    TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK,
              "it only makes sense to do this object clone transform for "
              "checks with a NULLCHK - how did we get here otherwise??");
    TR::Node *passthrough =
        TR::Node::create(callNode, TR::PassThrough, 1, objNode);
    callTree->insertBefore(TR::TreeTop::create(
        comp(), TR::Node::createWithSymRef(
                    callNode, callTree->getNode()->getOpCodeValue(), 1,
                    passthrough, callTree->getNode()->getSymbolReference())));
  ...
  if (!cloneInfo->_isFixed) {
    ...
    TR::Block *remainder =
        fastPath->split(callTree->getNextTreeTop(), cfg, true, true);

    TR_BlockCloner *cloner = new (trStackMemory()) TR_BlockCloner(cfg);
    TR::Block *slowPath = cloner->cloneBlocks(fastPath, fastPath);
    slowPath->append(TR::TreeTop::create(
        comp(),
        TR::Node::create(callNode, TR::Goto, 0, remainder->getEntry())));
    ...
    TR::TreeTop *compareTree = TR::TreeTop::create(
        ...
        TR::Node::createif(
            TR::ifacmpne,
            TR::Node::createWithSymRef(
                callNode, TR::aloadi, 1, objNode,
                comp()->getSymRefTab()->findOrCreateVftSymbolRef()),
            TR::Node::createWithSymRef(
                callNode, TR::loadaddr, 0,
                comp()->getSymRefTab()->findOrCreateClassSymbol(
                    callNode->getSymbolReference()->getOwningMethodSymbol(
                        comp()),
                    0, j9class)),
            slowPath->getEntry()));
  ...
  TR::Node *loadaddr = TR::Node::createWithSymRef(
      callNode, TR::loadaddr, 0,
      comp()->getSymRefTab()->findOrCreateClassSymbol(
          callNode->getSymbolReference()->getOwningMethodSymbol(comp()), 0,
          j9class));
  ...
  TR::SymbolReference *objCopySymRef =
      comp()->getSymRefTab()->methodSymRefFromName(
          comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
          const_cast<char *>(TR::Compiler->target.is64Bit()
                                 ? "unsafeObjectShallowCopy64"
                                 : "unsafeObjectShallowCopy32"),
          const_cast<char *>(TR::Compiler->target.is64Bit()
                                 ? "(Ljava/lang/Object;Ljava/lang/Object;J)V"
                                 : "(Ljava/lang/Object;Ljava/lang/Object;I)V"),
          TR::MethodSymbol::Static);
  TR::Node *getHelpers =
      TR::Node::createWithSymRef(callNode, TR::acall, 0, helperAccessor);
  callTree->insertBefore(TR::TreeTop::create(
      comp(), TR::Node::create(callNode, TR::treetop, 1, getHelpers)));
  TR::Node *objCopy =
      TR::Node::createWithSymRef(callNode, TR::call, 4, objCopySymRef);
  ...
  objCopy->setAndIncChild(
      3, TR::Node::create(callNode,
                          (TR::Compiler->target.is64Bit() ? TR::a2l : TR::a2i),
                          1, loadaddr));

  callTree->insertBefore(TR::TreeTop::create(
      comp(), TR::Node::create(callNode, TR::treetop, 1, callNode)));
  callTree->insertBefore(TR::TreeTop::create(
      comp(), TR::Node::create(callNode, TR::treetop, 1, objCopy)));
...
void OMR::ValuePropagation::transformArrayCloneCall(
  ...
  if (disableArrayCloneOpt || TR::Compiler->om.canGenerateArraylets()) return;
  ...
  TR_OpaqueClassBlock *j9class =
      comp()->fe()->getComponentClassFromArrayClass(j9arrayClass);
  ...
  if ((isPrimitiveClass && !cg()->getSupportsPrimitiveArrayCopy()) ||
    ...
    if (trace())
      traceMsg(comp(),
               "\nNot transforming array clone call [%p] because %s array copy "
               "is not supported\n",
  ...
  if (!performTransformation(
          comp(),
          "%sInlining array clone call [%p] as new array and arraycopy\n",
          OPT_DETAILS, callNode))
    return;
  ...
  TR::DebugCounter::prependDebugCounter(
      comp(),
      TR::DebugCounter::debugCounterName(
          comp(), "inlineClone.location/array/(%s)", comp()->signature()),
      callTree);
  int32_t classNameLength;
  char *className =
      TR::Compiler->cls.classNameChars(comp(), j9arrayClass, classNameLength);
  TR::DebugCounter::prependDebugCounter(
      comp(),
      TR::DebugCounter::debugCounterName(
          comp(), "inlineClone.type/(%s)/(%s)/%s", className,
  ...
  if (callTree->getNode()->getOpCode().isCheck()) {
    TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK,
              "it only makes sense to do this array clone transform for checks "
              "with a NULLCHK - how did we get here otherwise??");
    TR::Node *passthrough =
        TR::Node::create(callNode, TR::PassThrough, 1, objNode);
    callTree->insertBefore(TR::TreeTop::create(
        comp(), TR::Node::createWithSymRef(
  ...
  callTree->insertBefore(TR::TreeTop::create(
      comp(), TR::Node::create(callNode, TR::treetop, 1, lenNode)));

  if (isPrimitiveClass) {
    TR::Node *typeConst = TR::Node::iconst(
        callNode, comp()->fe()->getNewArrayTypeFromClass(j9arrayClass));
    static char *disableSkipZeroInitInVP =
        feGetEnv("TR_disableSkipZeroInitInVP");
    if (disableSkipZeroInitInVP) {
      TR::SymbolReference *symRef =
          comp()->getSymRefTab()->findOrCreateNewArraySymbolRef(
              objNode->getSymbolReference()->getOwningMethodSymbol(comp()));
      TR::Node::recreateWithoutProperties(callNode, TR::newarray, 2, lenNode,
                                          typeConst, symRef);
    } else {
      TR::SymbolReference *symRef =
    ...
    if (isFixedClass) {
      classNode = TR::Node::createWithSymRef(
          callNode, TR::loadaddr, 0,
          comp()->getSymRefTab()->findOrCreateClassSymbol(
              callNode->getSymbolReference()->getOwningMethodSymbol(comp()), 0,
              j9class));
    } else {
      // Load the component class of the cloned array as 2nd child of anewarray
      // as expected by the opcode.
      TR::Node *arrayClassNode = TR::Node::createWithSymRef(
          callNode, TR::aloadi, 1, objNode,
          comp()->getSymRefTab()->findOrCreateVftSymbolRef());
      classNode = TR::Node::createWithSymRef(
          callNode, TR::aloadi, 1, arrayClassNode,
          comp()->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
    }
    TR::SymbolReference *symRef =
        comp()->getSymRefTab()->findOrCreateANewArraySymbolRef(
            objNode->getSymbolReference()->getOwningMethodSymbol(comp()));
    TR::Node::recreateWithoutProperties(callNode, TR::anewarray, 2, lenNode,
                                        classNode, symRef);
  }
  ...
  TR::Node *lengthInBytes =
      TR::Compiler->target.is64Bit()
          ? TR::Node::create(callNode, TR::lmul, 2,
                             TR::Node::create(callNode, TR::i2l, 1, lenNode),
                             TR::Node::lconst(lenNode, elementSize))
          : TR::Node::create(callNode, TR::imul, 2, lenNode,
                             TR::Node::iconst(lenNode, elementSize));

  TR::Node *srcStart =
      TR::Compiler->target.is64Bit()
          ? TR::Node::create(
                callNode, TR::aladd, 2, objNode,
                TR::Node::lconst(
                    objNode,
                    TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))
          : TR::Node::create(
                callNode, TR::aiadd, 2, objNode,
                TR::Node::iconst(
                    objNode,
                    TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
  TR::Node *destStart =
      TR::Compiler->target.is64Bit()
          ? TR::Node::create(
                callNode, TR::aladd, 2, newArray,
                TR::Node::lconst(
                    newArray,
                    TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))
          : TR::Node::create(
                callNode, TR::aiadd, 2, newArray,
                TR::Node::iconst(
                    newArray,
                    TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
  ...
  if (isPrimitiveClass)
    ...
    arraycopy = TR::Node::createArraycopy(objNode, newArray, srcStart,
                                          destStart, lengthInBytes);
  ...
  callTree->insertBefore(TR::TreeTop::create(
      comp(), TR::Node::create(callNode, TR::treetop, 1, newArray)));
  callTree->insertBefore(TR::TreeTop::create(
      comp(), TR::Node::create(callNode, TR::treetop, 1, arraycopy)));
}

void OMR::ValuePropagation::transformConverterCall(TR::TreeTop *callTree) {
  if (callTree->getEnclosingBlock()->isCold() ||
      (callTree->getEnclosingBlock()->getFrequency() ==
       UNKNOWN_COLD_BLOCK_COUNT))
    return;

  // maybe the call node was removed.
  TR::Node *callNode = callTree->getNode()->getFirstChild();
  if (callNode->getReferenceCount() < 1) return;
  ...
  dumpOptDetails(comp(),
                 "Insert runtime tests for converter call transformation to "
                 "arraytranslate (%p)\n",
                 callNode);

  TR::Node *child1 = TR::Node::create(TR::iadd, 2, srcOff, len);
  TR::Node *child2 = TR::Node::create(TR::arraylength, 1, srcObjNode);
  // the source for the decompressed string version of this in StringCoding
  // takes a byte array used as a char array so convert the size appropriately
  // for the check >> 1
  ...
  createStoresForConverterCallChildren(comp(), callTree, srcRef, dstRef,
                                       srcOffRef, dstOffRef, lenRef, thisRef,
                                       callTree);
  if (hasTable) tableNode->createStoresForVar(tableRef, callTree, true);

  // create arraytranslate and cold call
  generateArrayTranslateNode(callTree, arrayTranslateTree, srcRef, dstRef,
                             srcOffRef, dstOffRef, lenRef, tableRef, hasTable);
  createConverterCallNodeAfterStores(dupCallTree, callTree, srcRef, dstRef,
                                     lenRef, srcOffRef, dstOffRef, thisRef,
                                     tableRef);
  arrayTranslateNode = arrayTranslateTree->getNode()->getFirstChild();

  // transmute original call node to iload of a temporary (where we will store
  // the result), so that any references to it pick up the result regardless of
  // execution path
  TR::SymbolReference *symRefConvert = getSymRefTab()->createTemporary(
      comp()->getMethodSymbol(), callNode->getDataType(), false);
  ...
  // split blocks
  origCallBlock->createConditionalBlocksBeforeTree(
      callTree, ifTree, dupCallTree, arrayTranslateTree, cfg, false);
  ...
  TR::Node *storeArrayTranslate = TR::Node::createWithSymRef(
      TR::istore, 1, 1, arrayTranslateNode, symRefConvert);
  arrayTranslateTree->insertAfter(
      TR::TreeTop::create(comp(), storeArrayTranslate, NULL, NULL));

  // create store of cold call result
  TR::Node *storeCall = TR::Node::createWithSymRef(
      TR::istore, 1, 1, dupCallTree->getNode()->getFirstChild(), symRefConvert);
  dupCallTree->insertAfter(TR::TreeTop::create(comp(), storeCall, NULL, NULL));

  // The async check is inserted because the converter call might have been the
  // only yield point in a loop due to asyn check removel.
  if (comp()->getOSRMode() != TR::involuntaryOSR &&
      comp()->getHCRMode() != TR::osr) {
    TR::TreeTop *actt = TR::TreeTop::create(
        comp(), TR::Node::createWithSymRef(
                    callNode, TR::asynccheck, 0,
  ...
  TR::TreeTop *ifTreeb = TR::TreeTop::create(
      comp(), TR::Node::createif(TR::ificmpgt, child1b, child2b));
  createAndInsertTestBlock(comp(), ifTreeb, callTree, origCallBlock,
                           slowArraytranslateBlock);

  // Null check on source
  TR::TreeTop *ifTreec = TR::TreeTop::create(
      comp(),
      TR::Node::createif(TR::ifacmpeq, TR::Node::createLoad(callNode, srcRef),
                         TR::Node::aconst(callNode, 0)));
  createAndInsertTestBlock(comp(), ifTreec, callTree, origCallBlock,
                           slowArraytranslateBlock);

  // Null check on desitantion
  TR::TreeTop *ifTreed = TR::TreeTop::create(
      comp(),
      TR::Node::createif(TR::ifacmpeq, TR::Node::createLoad(callNode, dstRef),
                         TR::Node::aconst(callNode, 0)));
  createAndInsertTestBlock(comp(), ifTreed, callTree, origCallBlock,
                           slowArraytranslateBlock);

  if (TR::Compiler->target.cpu.isZ()) {
    // Task 110060:
    //
    // Threshold represents the length of the input array for which executing
    // the Java method is faster than the hardware accelerated arraytranslate.
    // The constants used below are deduced from an empirical study of all 11
    // converters on Z.
    //
    // It is also noteworthy to say that we use ifacmpeq and ificmpgt trees to
    // perform the nullchecks and the "bound checks" because the actual
    // implementation in Java translates the arrays on an element by element
    // basis. This means if an ArrayIndexOutOfBounds were to occur we would have
    // an intermediate translation result. This cannot be achieved if we have a
    // pre-emptive BNDCHK node before the arraytranslate tree. Thus we must fall
    // back to the Java implementation in such cases.
    ...
    static const char *overrideThreshold =
        feGetEnv("TR_ArrayTranslateOverrideThreshold");
    ...
    if (threshold > 0) {
      TR::TreeTop *thresholdTestTree = TR::TreeTop::create(
          comp(), TR::Node::createif(
                      TR::ificmplt, TR::Node::createLoad(callNode, lenRef),
                      TR::Node::create(TR::iconst, 0, threshold)));
      createAndInsertTestBlock(comp(), thresholdTestTree, callTree,
                               origCallBlock, slowArraytranslateBlock);

      TR_InlineCall inlineFallback(optimizer(), this);
      // Inline the duplicate call as this block is potentially not cold
      // depending on input string length
  ...
  if (comp()->requiresSpineChecks()) {
    ...
    createAndInsertTestBlock(comp(), ifTreeSrcContig, callTree, origCallBlock,
                             slowArraytranslateBlock);

    TR::TreeTop *ifTreeDstContig = createSpineCheckNode(callNode, dstRef);
    createAndInsertTestBlock(comp(), ifTreeDstContig, callTree, origCallBlock,
                             slowArraytranslateBlock);
  }

  if (trace())
    comp()->dumpMethodTrees(
        "Trees after reducing converter call to intrinsic arraytranslate");
...
TR::LocalValuePropagation::LocalValuePropagation(
...
int32_t TR::LocalValuePropagation::perform() {
  if ((_firstUnresolvedSymbolValueNumber - 1) <= comp()->getNodeCount()) {
    ...
    while (treeTop) {
      TR_ASSERT(treeTop->getNode()->getOpCodeValue() == TR::BBStart,
                "assertion failure");
      treeTop = processBlock(treeTop);
      if (_reachedMaxRelationDepth) break;
    }
  }
...
int32_t TR::LocalValuePropagation::performOnBlock(TR::Block *block) {
  ...
  if ((_firstUnresolvedSymbolValueNumber - 1) <= comp()->getNodeCount()) {
    ...
    while (treeTop) {
      ...
      if ((blk != block) && !blk->isExtensionOfPreviousBlock()) break;
...
void TR::LocalValuePropagation::prePerformOnBlocks() {
  ...
  if (!cfg) {
    dumpOptDetails(comp(),
                   "Can't do Local Value Propagation - there is no CFG\n");
    return;
  }
  _useDefInfo = NULL;
  _valueNumberInfo = NULL;

  _bestRun = comp()->getMethodHotness() <=
             cold;  // && getLastRun(); getLastRun doesn't work well yet for VP
...
void TR::LocalValuePropagation::postPerformOnBlocks() {
  ...
  if (_enableVersionBlocks) versionBlocks();
  ...
  if (trace()) comp()->dumpMethodTrees("Trees after Local Value Propagation");
...
TR::TreeTop *TR::LocalValuePropagation::processBlock(TR::TreeTop *startTree) {
  ...
  if (comp()->getOption(TR_EnableLocalVPSkipLowFreqBlock)) {
    if (!((((comp()->getMethodHotness() < warm) &&
            (_curBlock->getFrequency() > 1500)) ||
           ((comp()->getMethodHotness() == warm) &&
            (_curBlock->getFrequency() > 500)) ||
      ...
      _curBlock =
          startTree->getExtendedBlockExitTreeTop()->getNode()->getBlock();
      TR::TreeTop *tt = _curBlock->getExit()->getNextTreeTop();
      if (tt) _curBlock = _curBlock->getNextBlock();
      return tt;
    }
  }
  ...
  while (1) {
    ...
    _curBlock = node->getBlock();
    if (!_curBlock->isExtensionOfPreviousBlock()) break;
...
const char *TR::LocalValuePropagation::optDetailString() const throw() {
  return "O^O LOCAL VALUE PROPAGATION: ";
}

void OMR::ValuePropagation::launchNode(TR::Node *node, TR::Node *parent,
                                       int32_t whichChild) {
  TR::Node *newNode = node;
  if (node && node->getVisitCount() != _visitCount) {
    int32_t valueNumber = getValueNumber(node);
    TR_ASSERT(valueNumber < _firstUnresolvedSymbolValueNumber,
              "value number too big, valueNumber:%d "
              "_firstUnresolvedSymbolValueNumber:%d",
    ...
    if (_isGlobalPropagation) {
      ...
      if (useDefIndex && _useDefInfo->isDefIndex(useDefIndex)) {
        if (node->getOpCode().isStore()) createStoreConstraints(node);

        if (lastTimeThrough() && _loopInfo) {
          LoopDefsHashTableEntry *entry = findLoopDef(node);
          if (entry) entry->region = _loopInfo->_loop;
        }
      }
    }
  }
  ...
  if (newNode && newNode->getOpCodeValue() == TR::PassThrough) {
    ...
    if (!parent->getOpCode().isNullCheck() &&
        performTransformation(comp(),
                              "%sReplace PassThrough node [%p] with its child "
                              "in its parent [%p]\n",
