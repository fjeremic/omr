...
OMR::ValuePropagation::ValuePropagation(TR::OptimizationManager *manager)
    : TR::Optimization(manager), _parmValues(NULL), _currentParent(NULL),
      _arraylengthNodes(trMemory()),
      _javaLangClassGetComponentTypeCalls(trMemory()),
      _unknownTypeArrayCopyTrees(trMemory()),
      _scalarizedArrayCopies(trMemory()), _predictedThrows(trMemory()),
      _prexClasses(trMemory()), _prexMethods(trMemory()),
      ...
      _needMultiLeafArrayCopy(trMemory()), _arrayCopySpineCheck(trMemory()),
      _multiLeafCallsToInline(trMemory()), _converterCalls(trMemory()),
      _objectCloneCalls(trMemory()), _arrayCloneCalls(trMemory()),
      _objectCloneTypes(trMemory()), _arrayCloneTypes(trMemory()),
      _parmInfo(NULL), _parmTypeValid(NULL),
...
void OMR::ValuePropagation::initialize() {
  ...
  if (_isGlobalPropagation) {
    ...
    // of nodes
    //
    uint32_t targetSize = (uint32_t)(comp()->getNodeCount() /
                                     5); // that 5 was experimentally determined
...
OMR::ValuePropagation::copyRelationships(Relationship *first) {
...
TR::TreeTop *
OMR::ValuePropagation::createPrimitiveOrReferenceCompareNode(TR::Node *node) {
#ifdef J9_PROJECT_SPECIFIC
...
//-------------------------- realtime support
TR::TreeTop *
OMR::ValuePropagation::buildSameLeafTest(TR::Node *offset, TR::Node *len,
                                         TR::Node *spineShiftNode) {
...
TR::Node *
generateArrayletAddressTree(TR::Compilation *comp, TR::Node *vcallNode,
                            TR::DataType type, TR::Node *off, TR::Node *obj,
                            TR::Node *spineShiftNode, TR::Node *shiftNode,
...
//
TR::TreeTop *
OMR::ValuePropagation::createSpineCheckNode(TR::Node *node,
                                            TR::SymbolReference *objSymRef) {
...
void OMR::ValuePropagation::transformRealTimeArrayCopy(
  ...
  if ((type == TR::Address) &&
    ...
    referenceArraycopyBlock->append(
        TR::TreeTop::create(comp(), TR::Node::create(dupArraycopyNode, TR::Goto,
                                                     0, succ->getEntry())));
