...
void
OMR::ValuePropagation::initialize()
{
  ...
  while (curTree) {
    if (curTree->getNode())
      collectArraylengthNodes(
        curTree->getNode(), visitCount, &_arraylengthNodes);
  ...
  if (_isGlobalPropagation) {
    ...
    memset(_globalConstraintsHashTable,
           0,
           globalConstraintsHashTableSize * sizeof(GlobalConstraint*));
    ...
    memset(_edgeConstraintsHashTable,
           0,
           VP_HASH_TABLE_SIZE * sizeof(EdgeConstraints*));
    ...
    if (!comp()->getFlowGraph()->getStructure() &&
      ...
      if (doTiming) {
        ...
        if (comp()->getOutFile() != NULL) {
          trfprintf(
            comp()->getOutFile(), "Time taken for %s = ", myTimer.title());
          trfprintf(
            comp()->getOutFile(), "%9.6f seconds\n", myTimer.secondsTaken());
        }
#ifdef OPT_TIMING
...
void
OMR::ValuePropagation::freeValueConstraint(ValueConstraint* vc)
{
...
int32_t
OMR::ValuePropagation::getValueNumber(TR::Node* node)
{
...
TR::VPConstraint*
OMR::ValuePropagation::getConstraint(TR::Node* node,
                                     bool& isGlobal,
                                     TR::Node* relative)
{
  /*
  ...
  if (rel) {
    if (trace()) {
      traceMsg(comp(),
               "   %s [%p] has existing global constraint:",
               node->getOpCode().getName(),
               node);
      rel->print(this, valueNumber, 1);
    }
...
void
OMR::ValuePropagation::removeNode(TR::Node* node, bool anchorIt)
{
...
void
OMR::ValuePropagation::removeChildren(TR::Node* node, bool anchorIt)
{
...
static bool
canCauseOSR(TR::TreeTop* tt, TR::Compilation* comp)
{
...
void
OMR::ValuePropagation::processTrees(TR::TreeTop* startTree,
                                    TR::TreeTop* endTree)
{
  TR::TreeTop* treeTop;
...
int32_t
OMR::ValuePropagation::getPrimitiveArrayType(char primitiveArrayChar)
{
...
bool
OMR::ValuePropagation::canTransformArrayCopyCallForSmall(TR::Node* node,
                                                         int32_t& srcLength,
                                                         int32_t& dstLength,
                                                         int32_t& elementSize,
                                                         TR::DataType& type)
{
  ...
  static TR::DataType primitiveArrayToTRDataType[] = { TR::Int8,  TR::Int16,
                                                       TR::Float, TR::Double,
                                                       TR::Int8,  TR::Int16,
                                                       TR::Int32, TR::Int64 };
...
#ifdef J9_PROJECT_SPECIFIC
static TR_ResolvedMethod*
findResolvedClassMethod(TR::Compilation* comp,
                        char* className,
                        char* methodName,
                        char* methodSig)
{
  ...
  if (classHandle) {
    ...
    comp->fej9()->getResolvedMethods(
      comp->trMemory(), classHandle, &classMethods);
...
void
OMR::ValuePropagation::removeArrayCopyNode(TR::TreeTop* arraycopyTree)
{
...
TR::Node*
createHdrSizeNode(TR::Compilation* comp, TR::Node* n)
{
...
TR::Node*
generateLenForArrayCopy(TR::Compilation* comp,
                        int32_t elementSize,
                        TR::Node* stride,
                        TR::Node* srcObjNode,
                        TR::Node* copyLenNode,
...
bool
OMR::ValuePropagation::canRunTransformToArrayCopy()
{
  if (!lastTimeThrough())
    return false;
...
bool
OMR::ValuePropagation::transformUnsafeCopyMemoryCall(TR::Node* arraycopyNode)
{
  ...
  if (comp()->canTransformUnsafeCopyToArrayCopy() &&
    ...
    if ((ttNode->getOpCodeValue() == TR::treetop ||
        ...
        performTransformation(
          comp(),
          "%sChanging call Unsafe.copyMemory [%p] to arraycopy\n",
          OPT_DETAILS,
          arraycopyNode))
...
void
OMR::ValuePropagation::transformArrayCopyCall(TR::Node* node)
{
          ...
          if (dstObject && dstObject->getClass()) {
            ...
            if (sig && sig[0] == '[') {
              ...
              //
                ...
                isInstance = fe()->isInstanceOf(srcObject->getClass(),
                                                dstObject->getClass(),
                                                srcObject->isFixedClass(),
                                                dstObject->isFixedClass());
  ...
  if (transformTheCall &&
      performTransformation(comp(),
                            "%sChanging call %s [%p] to arraycopy\n",
                            OPT_DETAILS,
                            node->getOpCode().getName(),
                            node)) {
    ...
    if (zeroLengthCopy &&
        performTransformation(comp(),
                              "%sRemoving zero length call %s [%p]\n",
                              OPT_DETAILS,
                              node->getOpCode().getName(),
                              node)) {
    ...
    if (comp()->requiresSpineChecks()) {
      ...
      _arrayCopySpineCheck.add(
        new (trStackMemory()) TR_ArrayCopySpineCheck(arraycopyTree,
    ...
    if (isMultiLeafArrayCopy) {
      ...
      _needRunTimeCheckArrayCopy.add(
        new (trStackMemory()) TR_RealTimeArrayCopy(arraycopyTree, type, flag));
    ...
    if (!comp()->generateArraylets()) {
      ...
      src = generateArrayAddressTree(comp(),
                                     node,
                                     srcOffHigh,
                                     srcOffNode,
                                     srcObjNode,
      ...
      dst = generateArrayAddressTree(comp(),
                                     node,
                                     dstOffHigh,
                                     dstOffNode,
                                     dstObjNode,
      ...
      src = generateArrayletAddressTree(comp(),
                                        node,
                                        type,
                                        srcOff,
                                        srcObjNode,
      ...
      dst = generateArrayletAddressTree(comp(),
                                        node,
                                        type,
                                        dstOff,
                                        dstObjNode,
    ...
    len = generateLenForArrayCopy(
      comp(), elementSize, stride, srcObjNode, copyLenNode, node);
...
createExceptionEdgeConstraintsAndReturn:
  ...
  addBlockConstraint(
    copyLenNode,
    TR::VPIntRange::create(this, 0, TR::getMaxSigned<TR::Int32>()));
}

TR::Node*
generateArrayAddressTree(TR::Compilation* comp,
                         TR::Node* node,
                         int32_t offHigh,
                         TR::Node* offNode,
                         TR::Node* objNode,
                         ...
                         TR::Node* hdrSize)
{
  ...
  if (offHigh > 0) {
    ...
    array =
      TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, array, hdrSize);
  } else {
    array = hdrSize;
  }

  array =
    TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, objNode, array);
...
OMR::ValuePropagation::createPrimitiveOrReferenceCompareNode(TR::Node* node)
{
  ...
  TR::Node* vftLoad = TR::Node::createWithSymRef(
    TR::aloadi, 1, 1, node, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
...
TR::TreeTop*
OMR::ValuePropagation::createArrayStoreCompareNode(TR::Node* src, TR::Node* dst)
{
  TR::Node* vftDst = TR::Node::createWithSymRef(
    TR::aloadi, 1, 1, dst, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
  TR::Node* instanceofNode = TR::Node::createWithSymRef(
    TR:: instanceof,
                   ...
                   comp()->getSymRefTab()->findOrCreateInstanceOfSymbolRef(
                     comp()->getMethodSymbol()));
  TR::Node* cmp = TR::Node::createif(TR::ificmpeq,
                                     instanceofNode,
                                     TR::Node::create(dst, TR::iconst, 0, 0),
                                     NULL);
  TR::TreeTop* cmpTree = TR::TreeTop::create(comp(), cmp);
  return cmpTree;
}
...
TR::TreeTop*
OMR::ValuePropagation::createPrimitiveArrayNodeWithoutFlags(
  TR::TreeTop* tree,
  TR::TreeTop* newTree,
  ...
  bool isOptimizedReferenceArraycopy)
{
  ...
  if (trace())
    traceMsg(comp(),
             "Created 3-child arraycopy %s from root node %s, type = %s\n",
...
#ifdef J9_PROJECT_SPECIFIC
void
OMR::ValuePropagation::generateArrayTranslateNode(
  TR::TreeTop* callTree,
  ...
  bool hasTable)
{
  ...
  */
  ...
  if (encode) {
    node = TR::Node::create(
      is64BitTarget ? TR::lmul : TR::imul, 2, srcOff, strideNode);
    node =
      TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, node, hdrSize);
    src =
      TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, srcObj, node);
    node =
      TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, dstOff, hdrSize);
    dst =
      TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, dstObj, node);
    arrayTranslateNode->setSourceIsByteArrayTranslate(false);
    arrayTranslateNode->setTargetIsByteArrayTranslate(true);
  } else {
    node =
      TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, srcOff, hdrSize);
    src =
      TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, srcObj, node);
    node = TR::Node::create(
      is64BitTarget ? TR::lmul : TR::imul, 2, dstOff, strideNode);
    node =
      TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, node, hdrSize);
    dst =
      TR::Node::create(is64BitTarget ? TR::aladd : TR::aiadd, 2, dstObj, node);
  ...
  if (encode && !isSBCSEncoder) {
    ...
    if (cg()->getSupportsArrayTranslateTROTNoBreak() ||
      ...
      tableNode = TR::Node::create(
        callNode, TR::iconst, 0, 0); // dummy table node, it's not gonna be used
    } else {                         // Z
      bool genSIMD = comp()->cg()->getSupportsVectorRegisters() &&
                     !comp()->getOption(TR_DisableSIMDArrayTranslate);
  ...
  } else if (isSBCSEncoder) // only z
  {
    ...
    TR::Node* tableNodeAddr =
      tableRef ? TR::Node::createLoad(callNode, tableRef)
               : callNode->getChild(childId++)->duplicateTree();
    tableNode = TR::Node::create(
      is64BitTarget ? TR::aladd : TR::aiadd, 2, tableNodeAddr, hdrSize);
  }
...
TR::TreeTop*
OMR::ValuePropagation::createConverterCallNodeAfterStores(
  TR::TreeTop* tree,
  TR::TreeTop* origTree,
  TR::SymbolReference* srcRef,
  TR::SymbolReference* dstRef,
...
//-------------------------- realtime support
TR::TreeTop*
OMR::ValuePropagation::buildSameLeafTest(TR::Node* offset,
                                         TR::Node* len,
                                         TR::Node* spineShiftNode)
{
  ...
  child1 = TR::Node::create(
    is64BitTarget ? TR::lshr : TR::ishr, 2, offset, spineShiftNode);
  child2 =
    TR::Node::create(is64BitTarget ? TR::ladd : TR::iadd, 2, offset, len);
  child2 = TR::Node::create(
    is64BitTarget ? TR::lshr : TR::ishr, 2, child2, spineShiftNode);
  ifNode = TR::Node::createif(
    is64BitTarget ? TR::iflcmpne : TR::ificmpne, child1, child2);
...
TR::Node*
generateArrayletAddressTree(TR::Compilation* comp,
                            TR::Node* vcallNode,
                            TR::DataType type,
                            TR::Node* off,
                            TR::Node* obj,
                            ...
                            TR::Node* hdrSize)
{
  ...
  node = TR::Node::create(
    is64BitTarget ? TR::lshr : TR::ishr, 2, off, spineShiftNode);
  node =
...
void
OMR::ValuePropagation::generateRTArrayNodeWithoutFlags(
  ...
  bool primitive)
{
  ...
  srcOff = generateArrayletAddressTree(comp(),
                                       node,
                                       type,
                                       srcOff,
                                       srcObj,
                                       ...
                                       strideShiftNode,
                                       hdrSize);
  dstOff = generateArrayletAddressTree(comp(),
                                       node,
                                       type,
                                       dstOff,
                                       dstObj,
...
TR::TreeTop*
OMR::ValuePropagation::createArrayCopyVCallNodeAfterStores(
  TR::TreeTop* tree,
  TR::SymbolReference* srcRef,
  TR::SymbolReference* dstRef,
  TR::SymbolReference* lenRef,
...
TR::TreeTop*
createStoresForConverterCallChildren(TR::Compilation* comp,
                                     TR::TreeTop* callTree,
                                     TR::SymbolReference*& srcRef,
                                     TR::SymbolReference*& dstRef,
                                     TR::SymbolReference*& srcOffRef,
...
TR::TreeTop*
createStoresForArraycopyVCallChildren(TR::Compilation* comp,
...
void
createAndInsertTestBlock(TR::Compilation* comp,
                         TR::TreeTop* ifTree,
                         TR::TreeTop* tree,
                         ...
                         TR::Block* dstBlock)
{
  ...
  TR::TreeTop::insertTreeTops(
    comp, insertAfterBlock->getExit(), ifBlock->getEntry(), ifBlock->getExit());
  ...
  cfg->addEdge(TR::CFGEdge::createEdge(
    ifBlock, ifBlock->getNextBlock(), comp->trMemory()));
...
//
TR::TreeTop*
OMR::ValuePropagation::createAndInsertStoresForArrayCopySpineCheck(
...
TR::TreeTop*
OMR::ValuePropagation::createArrayCopyCallForSpineCheck(
  TR_ArrayCopySpineCheck* checkInfo)
{
  ...
  TR::Node* callNode = TR::Node::createWithSymRef(
    acNode, TR::call, 5, checkInfo->_arraycopySymRef);
...
void
OMR::ValuePropagation::transformArrayCopySpineCheck(
  TR_ArrayCopySpineCheck* checkInfo)
{
  ...
  //
  TR::Block* ifBlock = TR::Block::createEmptyBlock(
    arraycopyTree->getNode(), comp(), 0, remainderBlock);
  ...
  ifBlock->append(TR::TreeTop::create(
    comp(),
    TR::Node::create(
      arraycopyTree->getNode(), TR::Goto, 0, remainderBlock->getEntry())));
...
void
OMR::ValuePropagation::transformRealTimeArrayCopy(
  TR_RealTimeArrayCopy* rtArrayCopyTree)
{
  ...
  createStoresForArraycopyVCallChildren(
    comp(), vcallTree, srcRef, dstRef, srcOffRef, dstOffRef, lenRef, vcallTree);
  ...
  if (!comp()->getOptions()->realTimeGC() &&
    ...
    // tarok, need wrtbar
    generateRTArrayNodeWithoutFlags(rtArrayCopyTree,
                                    dupArraycopyTree,
                                    srcRef,
                                    dstRef,
                                    srcOffRef,
  ...
  createArrayCopyVCallNodeAfterStores(
    dupVCallTree, srcRef, dstRef, lenRef, srcOffRef, dstOffRef);
  ...
  if (type == TR::Address && comp()->getOptions()->realTimeGC() &&
    ...
    createArrayCopyVCallNodeAfterStores(
      origDupVCallTree, srcRef, dstRef, lenRef, srcOffRef, dstOffRef);
    slowArraycopyBlock =
      TR::Block::createEmptyBlock(origDupVCallTree->getNode(),
                                  comp(),
                                  origCallBlock->getFrequency(),
                                  origCallBlock);
    ...
    slowArraycopyBlock->append(TR::TreeTop::create(
      comp(),
      TR::Node::create(
        origDupVCallTree->getNode(), TR::Goto, 0, succ->getEntry())));
  ...
  if ((type == TR::Address) &&
    ...
    if (!comp()->getOptions()->realTimeGC() &&
        ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
      dupRefArraycopyNode =
        TR::Node::createArraycopy(dupVCallNode->getChild(0)->duplicateTree(),
    ...
    TR::Block* referenceArraycopyBlock =
      TR::Block::createEmptyBlock(dupRefArraycopyNode,
                                  comp(),
                                  origCallBlock->getFrequency(),
                                  origCallBlock);
    if (!(rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC ||
  ...
  if (dstOff && ((type == TR::Address) || srcOff)) {
    TR::Node* spineShiftNode2 = TR::Node::create(
      vcallNode, TR::iconst, 0, (int32_t)fe()->getArraySpineShift(elementSize));
    ...
    createAndInsertTestBlock(
      comp(), ifDstTree, dupArraycopyTree, origCallBlock, slowArraycopyBlock);
  }
  ...
  if ((type == TR::Address) && srcOff && comp()->getOptions()->realTimeGC()) {
    ...
    createAndInsertTestBlock(
      comp(), ifSrcTree, dupArraycopyTree, origCallBlock, slowArraycopyBlock);
  }
  ...
  if (doMultiLeafArrayCopy)
    _needMultiLeafArrayCopy.add(
      new (trStackMemory()) TR_RealTimeArrayCopy(origDupVCallTree, type, flag));
  else
...
#ifdef J9_PROJECT_SPECIFIC
void
OMR::ValuePropagation::transformRTMultiLeafArrayCopy(
  TR_RealTimeArrayCopy* rtArrayCopyTree)
{
  ...
  TR::Node* newCallNode = TR::Node::createWithSymRef(TR::call,
                                                     7,
                                                     7,
...
static const char*
transformedTargetName(TR::RecognizedMethod rm)
{
  switch (rm) {
#ifdef J9_PROJECT_SPECIFIC
...
#ifdef J9_PROJECT_SPECIFIC
void
OMR::ValuePropagation::transformObjectCloneCall(
  TR::TreeTop* callTree,
  OMR::ValuePropagation::ObjCloneInfo* cloneInfo)
{
  ...
  if (callTree->getNode()->getOpCode().isCheck()) {
    ...
    callTree->insertBefore(TR::TreeTop::create(
      comp(),
      TR::Node::createWithSymRef(callNode,
                                 callTree->getNode()->getOpCodeValue(),
                                 1,
                                 passthrough,
  ...
  if (!cloneInfo->_isFixed) {
    ...
    slowPath->append(TR::TreeTop::create(
      comp(), TR::Node::create(callNode, TR::Goto, 0, remainder->getEntry())));
...
void
OMR::ValuePropagation::transformArrayCloneCall(
  TR::TreeTop* callTree,
  OMR::ValuePropagation::ArrayCloneInfo* cloneInfo)
{
  ...
  if (isPrimitiveClass) {
    ...
    if (disableSkipZeroInitInVP) {
      TR::SymbolReference* symRef =
        comp()->getSymRefTab()->findOrCreateNewArraySymbolRef(
          objNode->getSymbolReference()->getOwningMethodSymbol(comp()));
      TR::Node::recreateWithoutProperties(
        callNode, TR::newarray, 2, lenNode, typeConst, symRef);
    } else {
      TR::SymbolReference* symRef =
    ...
    TR::SymbolReference* symRef =
      comp()->getSymRefTab()->findOrCreateANewArraySymbolRef(
        objNode->getSymbolReference()->getOwningMethodSymbol(comp()));
    TR::Node::recreateWithoutProperties(
      callNode, TR::anewarray, 2, lenNode, classNode, symRef);
  }
  ...
  TR::Node* srcStart =
    TR::Compiler->target.is64Bit()
      ...
      : TR::Node::create(
          ...
          objNode,
          TR::Node::iconst(
            objNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
  TR::Node* destStart =
    TR::Compiler->target.is64Bit()
      ? TR::Node::create(
          callNode,
      ...
      : TR::Node::create(
          ...
          newArray,
          TR::Node::iconst(
            newArray, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
  ...
  if (isPrimitiveClass)
    ...
    arraycopy = TR::Node::createArraycopy(
      objNode, newArray, srcStart, destStart, lengthInBytes);
...
void
OMR::ValuePropagation::transformConverterCall(TR::TreeTop* callTree)
{
  ...
  if (!performTransformation(comp(),
                             "%sChanging call %s [%p] to %s \n",
                             OPT_DETAILS,
                             callNode->getOpCode().getName(),
                             callNode,
                             transformedTargetName(rm)))
    return;
  ...
  createStoresForConverterCallChildren(comp(),
                                       callTree,
                                       srcRef,
                                       dstRef,
                                       srcOffRef,
  ...
  generateArrayTranslateNode(callTree,
                             arrayTranslateTree,
                             srcRef,
                             dstRef,
                             srcOffRef,
                             ...
                             tableRef,
                             hasTable);
  createConverterCallNodeAfterStores(dupCallTree,
                                     callTree,
                                     srcRef,
                                     dstRef,
                                     lenRef,
  ...
  createAndInsertTestBlock(
    comp(), ifTreeb, callTree, origCallBlock, slowArraytranslateBlock);

  // Null check on source
  TR::TreeTop* ifTreec = TR::TreeTop::create(
    comp(),
    TR::Node::createif(TR::ifacmpeq,
                       TR::Node::createLoad(callNode, srcRef),
                       TR::Node::aconst(callNode, 0)));
  createAndInsertTestBlock(
    comp(), ifTreec, callTree, origCallBlock, slowArraytranslateBlock);

  // Null check on desitantion
  TR::TreeTop* ifTreed = TR::TreeTop::create(
    comp(),
    TR::Node::createif(TR::ifacmpeq,
                       TR::Node::createLoad(callNode, dstRef),
                       TR::Node::aconst(callNode, 0)));
  createAndInsertTestBlock(
    comp(), ifTreed, callTree, origCallBlock, slowArraytranslateBlock);
  ...
  if (TR::Compiler->target.cpu.isZ()) {
    ...
    if (overrideThreshold) {
      ...
      traceMsg(comp(),
               "Overriding arraytranslate fallback threshold to %d\n",
               threshold);
    }
    ...
    if (threshold > 0) {
      TR::TreeTop* thresholdTestTree = TR::TreeTop::create(
        ...
        TR::Node::createif(TR::ificmplt,
                           TR::Node::createLoad(callNode, lenRef),
...
int32_t
TR::LocalValuePropagation::perform()
{
...
int32_t
TR::LocalValuePropagation::performOnBlock(TR::Block* block)
{
...
void
TR::LocalValuePropagation::prePerformOnBlocks()
{
  TR::CFG* cfg = comp()->getFlowGraph();
...
TR::TreeTop*
TR::LocalValuePropagation::processBlock(TR::TreeTop* startTree)
{
...
void
OMR::ValuePropagation::launchNode(TR::Node* node,
                                  TR::Node* parent,
                                  int32_t whichChild)
{
  ...
  if (newNode && newNode->getOpCodeValue() == TR::PassThrough) {
    ...
    if (!parent->getOpCode().isNullCheck() &&
        performTransformation(
          comp(),
          "%sReplace PassThrough node [%p] with its child in its parent [%p]\n",
          OPT_DETAILS,
          newNode,
          parent)) {
