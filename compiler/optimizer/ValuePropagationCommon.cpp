...
void OMR::ValuePropagation::initialize()
{
  ...
  if (_isGlobalPropagation) {
    ...
    _globalConstraintsHashTable
      = (GlobalConstraint**)trMemory()->allocateStackMemory(globalConstraintsHashTableSize * sizeof(GlobalConstraint*));
    ...
    _loopDefsHashTable
      = (LoopDefsHashTableEntry**)trMemory()->allocateStackMemory(VP_HASH_TABLE_SIZE * sizeof(LoopDefsHashTableEntry*));
...
int32_t OMR::ValuePropagation::getValueNumber(TR::Node* node)
{
  ...
  if (_isGlobalPropagation)
    ...
    if (node->getOpCode().isStore()) {
    ...
    } else if (node->getOpCode().isLoadConst() && node->canGet64bitIntegralValue() && !disableLocalVPConstNumbering) {
      ...
      if (_constNodeInfo.Locate(value, index)) {
        ...
        TR::list<TR::Node*>* nodeList
          = new (comp()->trHeapMemory()) TR::list<TR::Node*>(getTypedAllocator<TR::Node*>(comp()->allocator()));
        nodeList->push_back(node);
...
bool OMR::ValuePropagation::canTransformArrayCopyCallForSmall(
  TR::Node* node, int32_t& srcLength, int32_t& dstLength, int32_t& elementSize, TR::DataType& type)
{
  ...
  static TR::DataType primitiveArrayToTRDataType[]
    = { TR::Int8, TR::Int16, TR::Float, TR::Double, TR::Int8, TR::Int16, TR::Int32, TR::Int64 };
  ...
  // check if both arrays are primitive types and of the same type, if so then can transform the call for small (ie a
...
#ifdef J9_PROJECT_SPECIFIC
  if (comp()->canTransformUnsafeCopyToArrayCopy() && arraycopyNode->getOpCode().isCall()
...
void OMR::ValuePropagation::transformArrayCopyCall(TR::Node* node)
{
  ...
  if ((primitiveTransform || referenceTransform) && !comp()->getOption(TR_DisableArrayCopyOpts)
    && !node->isDontTransformArrayCopyCall() && comp()->fej9()->callTheJitsArrayCopyHelper()
    && !comp()->getOption(TR_DisableInliningOfNatives) && !(srcObject && srcObject->isNullObject())
    && !(dstObject && dstObject->isNullObject()) && srcOffHigh >= 0 && dstOffHigh >= 0 && copyLenHigh >= 0) {
      ...
      if (isStringCompressedArrayCopy || isStringDecompressedArrayCopy) {
        ...
        if (primitiveArray2) {
          if (srcObject->getClassType() == dstObject->getClassType()
            || (srcObject->getClassType() && dstObject->getClassType() && srcObject->getClassType()->asResolvedClass()
            ...
            if (sig && sig[0] == '[') {
              ...
              //
                ...
                isInstance = fe()->isInstanceOf(
                  srcObject->getClass(), dstObject->getClass(), srcObject->isFixedClass(), dstObject->isFixedClass());
  ...
  if (transformTheCall
    ...
    bool canSkipAllChecksOnArrayCopy = methodSymbol->safeToSkipChecksOnArrayCopies() || isRecognizedMultiLeafArrayCopy
    ...
    if (comp()->requiresSpineChecks()) {
      ...
      _arrayCopySpineCheck.add(new (trStackMemory()) TR_ArrayCopySpineCheck(
        arraycopyTree, node->getSymbolReference(), srcObjNode, srcOffNode, dstObjNode, dstOffNode, copyLenNode));
    }
...
TR::TreeTop* OMR::ValuePropagation::createArrayStoreCompareNode(TR::Node* src, TR::Node* dst)
{
  ...
  TR::Node* instanceofNode
    = TR::Node::createWithSymRef(TR:: instanceof, 2, 2, src, vftDst,
  ...
  bool useFlagsOnOriginalArraycopy, bool isOptimizedReferenceArraycopy)
{
  ...
  if (trace())
    traceMsg(comp(), "Created 3-child arraycopy %s from root node %s, type = %s\n", comp()->getDebug()->getName(node),
      comp()->getDebug()->getName(root), TR::DataType::getName(node->getArrayCopyElementType()));
  ...
  TR::SymbolReference* dstOffRef, TR::SymbolReference* lenRef, TR::SymbolReference* tableRef, bool hasTable)
{
  ...
  bool isISO88591Encoder
    = (rm == TR::sun_nio_cs_ISO_8859_1_Encoder_encodeISOArray || rm == TR::java_lang_StringCoding_implEncodeISOArray);
  ...
  */
  ...
  if (isISO88591Encoder || isSBCSEncoder || isEncodeUtf16 || (rm == TR::sun_nio_cs_US_ASCII_Encoder_encodeASCII)
  ...
  if (isEncodeUtf16) {
    TR::SymbolReference* transformedCallSymRef
      = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
        const_cast<char*>(TR::sun_nio_cs_UTF_16_Encoder_encodeUTF16Big == rm ? "transformedEncodeUTF16Big"
...
TR::TreeTop* OMR::ValuePropagation::createConverterCallNodeAfterStores(TR::TreeTop* tree, TR::TreeTop* origTree,
  TR::SymbolReference* srcRef, TR::SymbolReference* dstRef, TR::SymbolReference* lenRef, TR::SymbolReference* srcOffRef,
  TR::SymbolReference* dstOffRef, TR::SymbolReference* thisRef, TR::SymbolReference* tableRef)
{
  TR::Node* root = tree->getNode()->getFirstChild();
  ...
  if (root->getOpCode().isCallIndirect()) {
    TR::Node* vftLoad
      = TR::Node::createWithSymRef(TR::aloadi, 1, 1, thisNode, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
  ...
  if (lenRef)
    ...
    len
      = isISO88591Encoder ? root->getChild(childId + 4)->duplicateTree() : root->getChild(childId + 2)->duplicateTree();
  ...
  if (dstRef)
    ...
    dst
      = isISO88591Encoder ? root->getChild(childId + 2)->duplicateTree() : root->getChild(childId + 3)->duplicateTree();
  ...
  if (dstOffRef)
    ...
    dstOff
      = isISO88591Encoder ? root->getChild(childId + 3)->duplicateTree() : root->getChild(childId + 4)->duplicateTree();
...
void OMR::ValuePropagation::transformArrayCopySpineCheck(TR_ArrayCopySpineCheck* checkInfo)
{
  ...
  ifBlock->append(
    TR::TreeTop::create(comp(), TR::Node::create(arraycopyTree->getNode(), TR::Goto, 0, remainderBlock->getEntry())));
...
void OMR::ValuePropagation::transformRealTimeArrayCopy(TR_RealTimeArrayCopy* rtArrayCopyTree)
{
  ...
  bool doMultiLeafArrayCopy = (!comp()->getOption(TR_DisableMultiLeafArrayCopy)
    && ((rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC) || (rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_DST)));
  ...
  if (type != TR::Address || !comp()->getOptions()->realTimeGC()) {
    ...
    // srcOFf dstOff are only indicators for runtime tests needed, so make sure they are not null without incrementing
    // ref count
    if (rtArrayCopyTree->_flag & NEED_RUNTIME_TEST_FOR_SRC) {
      //      printf("needsSrc\n");
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      ifTree = TR::TreeTop::create(
        comp(), TR::Node::createif(TR::iflcmpne, globalFragment, TR::Node::create(vcallNode, TR::lconst, 0, 0)));
    } else {
      TR::Node* globalFragment = TR::Node::createWithSymRef(
        TR::iloadi, 1, 1, fragmentParent, comp()->getSymRefTab()->findOrCreateGlobalFragmentSymbolRef());
      ifTree = TR::TreeTop::create(
        comp(), TR::Node::createif(TR::ificmpne, globalFragment, TR::Node::create(vcallNode, TR::iconst, 0, 0)));
    }
  ...
  if (!comp()->getOptions()->realTimeGC() && ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
    ...
    if (comp()->useCompressedPointers()) {
      ...
      TR_ASSERT(
        (dupArraycopyTree->getNode()->getFirstChild()->getChild(2)->getFirstChild()->getOpCodeValue() == TR::aloadi),
        "didn't find the src arraylet pointer");
      TR_ASSERT(
        (dupArraycopyTree->getNode()->getFirstChild()->getChild(3)->getFirstChild()->getOpCodeValue() == TR::aloadi),
    ...
    if (comp()->useCompressedPointers()) {
      TR::TreeTop* prevTree = dupArraycopyTree->getPrevTreeTop();
      TR_ASSERT(
        (dupArraycopyTree->getNode()->getFirstChild()->getChild(0)->getFirstChild()->getOpCodeValue() == TR::aloadi),
        "didn't find the src arraylet pointer");
      TR_ASSERT(
        (dupArraycopyTree->getNode()->getFirstChild()->getChild(1)->getFirstChild()->getOpCodeValue() == TR::aloadi),
        "didn't find the dst arraylet pointer");
  ...
  if (type == TR::Address && comp()->getOptions()->realTimeGC() && doMultiLeafArrayCopy) {
    ...
    slowArraycopyBlock
      = TR::Block::createEmptyBlock(origDupVCallTree->getNode(), comp(), origCallBlock->getFrequency(), origCallBlock);
  ...
  if ((type == TR::Address) && (rtArrayCopyTree->_flag & NEED_ARRAYSTORE_CHECK)) {
    ...
    if (comp()->useCompressedPointers()) {
      ...
      TR_ASSERT(
        (dupRefArraycopyTree->getNode()->getFirstChild()->getChild(2)->getFirstChild()->getOpCodeValue() == TR::aloadi),
        "didn't find the src arraylet pointer");
      TR_ASSERT(
        (dupRefArraycopyTree->getNode()->getFirstChild()->getChild(3)->getFirstChild()->getOpCodeValue() == TR::aloadi),
        "didn't find the dst arraylet pointer");
  ...
  TR::TreeTop* callTree, OMR::ValuePropagation::ObjCloneInfo* cloneInfo)
{
  ...
  if (!performTransformation(
        comp(), "%sInlining object clone call [%p] as new object and JITHelpers object copy\n", OPT_DETAILS, callNode))
    return;
  ...
  if (callTree->getNode()->getOpCode().isCheck()) {
    ...
    callTree->insertBefore(TR::TreeTop::create(comp(),
      TR::Node::createWithSymRef(
        callNode, callTree->getNode()->getOpCodeValue(), 1, passthrough, callTree->getNode()->getSymbolReference())));
  ...
  TR::SymbolReference* helperAccessor
    = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
      const_cast<char*>("jitHelpers"), const_cast<char*>("()Lcom/ibm/jit/JITHelpers;"), TR::MethodSymbol::Static);
  TR::SymbolReference* objCopySymRef
    = comp()->getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers",
      ...
      const_cast<char*>(TR::Compiler->target.is64Bit() ? "(Ljava/lang/Object;Ljava/lang/Object;J)V"
  ...
  objCopy->setAndIncChild(
    3, TR::Node::create(callNode, (TR::Compiler->target.is64Bit() ? TR::a2l : TR::a2i), 1, loadaddr));
  ...
  TR::TreeTop* callTree, OMR::ValuePropagation::ArrayCloneInfo* cloneInfo)
{
  ...
  if (isPrimitiveClass) {
    ...
    if (isFixedClass) {
      ...
      classNode = TR::Node::createWithSymRef(
        callNode, TR::aloadi, 1, arrayClassNode, comp()->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
    }
  ...
  TR::Node* lengthInBytes = TR::Compiler->target.is64Bit()
    ? TR::Node::create(
      callNode, TR::lmul, 2, TR::Node::create(callNode, TR::i2l, 1, lenNode), TR::Node::lconst(lenNode, elementSize))
    : TR::Node::create(callNode, TR::imul, 2, lenNode, TR::Node::iconst(lenNode, elementSize));

  TR::Node* srcStart = TR::Compiler->target.is64Bit()
    ? TR::Node::create(
      callNode, TR::aladd, 2, objNode, TR::Node::lconst(objNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))
    : TR::Node::create(
      callNode, TR::aiadd, 2, objNode, TR::Node::iconst(objNode, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
  TR::Node* destStart = TR::Compiler->target.is64Bit()
    ? TR::Node::create(
      callNode, TR::aladd, 2, newArray, TR::Node::lconst(newArray, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()))
    : TR::Node::create(callNode, TR::aiadd, 2, newArray,
      TR::Node::iconst(newArray, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()));
...
void OMR::ValuePropagation::transformConverterCall(TR::TreeTop* callTree)
{
  ...
  if (comp()->getOSRMode() != TR::involuntaryOSR && comp()->getHCRMode() != TR::osr) {
    TR::TreeTop* actt = TR::TreeTop::create(comp(),
      TR::Node::createWithSymRef(
        callNode, TR::asynccheck, 0, getSymRefTab()->findOrCreateAsyncCheckSymbolRef(comp()->getMethodSymbol())));
  ...
  TR::TreeTop* ifTreec = TR::TreeTop::create(
    comp(), TR::Node::createif(TR::ifacmpeq, TR::Node::createLoad(callNode, srcRef), TR::Node::aconst(callNode, 0)));
  ...
  TR::TreeTop* ifTreed = TR::TreeTop::create(
    comp(), TR::Node::createif(TR::ifacmpeq, TR::Node::createLoad(callNode, dstRef), TR::Node::aconst(callNode, 0)));
...
void OMR::ValuePropagation::launchNode(TR::Node* node, TR::Node* parent, int32_t whichChild)
{
  ...
  if (newNode && newNode->getOpCodeValue() == TR::PassThrough) {
    ...
    if (!parent->getOpCode().isNullCheck()
      && performTransformation(
        comp(), "%sReplace PassThrough node [%p] with its child in its parent [%p]\n", OPT_DETAILS, newNode, parent)) {
