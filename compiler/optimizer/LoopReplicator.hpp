...
class TR_LoopReplicator : public TR_LoopTransformer {
 public:
  ...
  int32_t replicateLoop(TR_RegionStructure *region,
  ...
  void gatherRestOfLoop(TR_RegionStructure *region, TR::Block *b,
                        TR_Queue<TR::Block> &n);
  ...
  TR::Block *nextCandidate(TR::Block *X, TR_RegionStructure *region,
                           bool doSucc);
  void processBlock(TR::Block *X, TR_RegionStructure *region, LoopInfo *lInfo);
  bool computeWeight(TR::CFGEdge *edge);
  TR::Block *bestSuccessor(TR_RegionStructure *r, TR::Block *b,
                           TR::CFGEdge **edge);
  ...
  void modifyLoops();
  TR_StructureSubGraphNode *findNodeInHierarchy(TR_RegionStructure *r,
                                                int32_t num);
  void nextSuccessor(TR_RegionStructure *r, TR::Block **cand,
                     TR::CFGEdge **edge);
  void fixUpLoopEntry(LoopInfo *lInfo, TR::Block *loopHeader);
  bool isBackEdgeOrLoopExit(TR::CFGEdge *e, TR_RegionStructure *region,
                            bool testSGraph = false);
  ...
  int32_t deriveFrequencyFromPreds(TR_StructureSubGraphNode *curNode,
                                   TR_RegionStructure *region);
  bool predecessorsNotYetVisited(TR_RegionStructure *r,
                                 TR_StructureSubGraphNode *curNode);

  bool isSwitchBlock(TR::Block *b);
  void gatherTargets(TR_Queue<TR::Block> &q, TR_Queue<TR::Block> &sn,
                     LoopInfo *l);
