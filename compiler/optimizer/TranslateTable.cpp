...
TR_TranslateTable::TR_TranslateTableData* TR_TranslateTable::matchTable(uint32_t startA, uint32_t endA, uint32_t startB, uint32_t endB, uint8_t inputSize, uint8_t outputSize, uint16_t defaultValue)
{
    ...
    while (curr) {
        if (curr->startA == startA && curr->endA == endA && curr->startB == startB && curr->endB == endB && curr->inSize == inputSize && curr->outSize == outputSize && curr->defaultValue == defaultValue) {
            return curr;
        }
        curr = curr->next;
    }
...
TR_RangeTranslateTable::TR_RangeTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint16_t start, uint16_t end, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
    createTable(start, end, inputSize, outputSize, defaultValue);
}
TR_RangeTranslateTable::TR_RangeTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint16_t startA, uint16_t endA, uint16_t startB, uint16_t endB, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
    createTable(startA, endA, startB, endB, inputSize, outputSize, defaultValue);
}

TR_RangeTranslateTable::TR_RangeTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint32_t start, uint32_t end, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
    createTable(start, end, inputSize, outputSize, defaultValue);
}

TR_RangeTranslateTable::TR_RangeTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, uint32_t startA, uint32_t endA, uint32_t startB, uint32_t endB, uint16_t defaultValue)
    : TR_TranslateTable(comp)
{
    createTable(startA, endA, startB, endB, inputSize, outputSize, defaultValue);
}
...
TR_SetTranslateTable::TR_SetTranslateTable(TR::Compilation* comp, uint8_t inputSize, uint8_t outputSize, void* set, int copyElemSize)
    : TR_TranslateTable(comp) // input and output size in bits
{
    int tableSizeParm = tableSize(inputSize, outputSize);
    int tableByteSize = tableSizeParm * (outputSize / 8);
