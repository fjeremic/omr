...
void TR_TranslateTable::dumpTable() {
  ...
  dumpOptDetails(comp(), "\n  Range %d to %d and %d to %d default value %d\n",
                 table()->startA, table()->endA, table()->startB, table()->endB,
                 table()->defaultValue);
  if (table()->outSize == 16) {
...
TR::SymbolReference *TR_TranslateTable::createSymbolRef() {
  if (!_symRef) {
    _symRef = _comp->getSymRefTab()->createKnownStaticDataSymbolRef(
        data(), TR::Address);
  }
  return _symRef;
}
...
TR_TranslateTable::TR_TranslateTableData *
TR_TranslateTable::createTable(uint32_t startA, uint32_t endA,
                               uint8_t inputSize, uint8_t outputSize,
                               uint16_t defaultValue) {
  return createTable(startA, endA, 0, 0, inputSize, outputSize, defaultValue);
}

TR_TranslateTable::TR_TranslateTableData *
TR_TranslateTable::createTable(uint16_t startA, uint16_t endA,
                               uint8_t inputSize, uint8_t outputSize,
                               uint16_t defaultValue) {
  return createTable((uint32_t)startA, (uint32_t)endA, 0, 0, inputSize,
                     outputSize, defaultValue);
}

TR_TranslateTable::TR_TranslateTableData *
TR_TranslateTable::createTable(uint16_t startA, uint16_t endA, uint16_t startB,
                               uint16_t endB, uint8_t inputSize,
                               uint8_t outputSize, uint16_t defaultValue) {
  return createTable((uint32_t)startA, (uint32_t)endA, (uint32_t)startB,
                     (uint32_t)endB, inputSize, outputSize, defaultValue);
}

TR_TranslateTable::TR_TranslateTableData *
TR_TranslateTable::createTable(uint32_t startA, uint32_t endA, uint32_t startB,
                               uint32_t endB, uint8_t inputSize,
                               uint8_t outputSize, uint16_t defaultValue) {
  TR_TranslateTableData *match = matchTable(
      startA, endA, startB, endB, inputSize, outputSize, defaultValue);
  if (match) {
    _table = match;
    return _table;
  }
  ...
  // msf - methods that update _head will need a lock when we move to a true
  // async compilation (re-entrant) model
...
// can do something smarter if number of tables gets big - expected to be quite
// small (1 or 2 tables in practice)
TR_TranslateTable::TR_TranslateTableData *
TR_TranslateTable::matchTable(uint32_t startA, uint32_t endA, uint32_t startB,
                              uint32_t endB, uint8_t inputSize,
                              uint8_t outputSize, uint16_t defaultValue) {
...
TR_TranslateTable::TR_TranslateTableData *
TR_TranslateTable::matchTable(uint8_t inputSize, uint8_t outputSize,
                              void *array) {
...
TR_RangeTranslateTable::TR_RangeTranslateTable(TR::Compilation *comp,
                                               uint8_t inputSize,
                                               uint8_t outputSize,
...
TR_SetTranslateTable::TR_SetTranslateTable(TR::Compilation *comp,
                                           uint8_t inputSize,
                                           uint8_t outputSize, uint8_t set[])
    : TR_TranslateTable(comp) // input and output size in bits
{
  createTable((uint16_t)0, tableSize(inputSize, outputSize), inputSize,
              outputSize, (uint16_t)0);
...
TR_SetTranslateTable::TR_SetTranslateTable(TR::Compilation *comp,
                                           uint8_t inputSize,
                                           uint8_t outputSize, uint16_t set[])
    : TR_TranslateTable(comp) // input and output size in bits
{
  createTable((uint32_t)0, tableSize(inputSize, outputSize), inputSize,
              outputSize, (uint32_t)0);
  for (int i = 0; set[i] > 0; ++i) {
    ...
    : TR_TranslateTable(comp) // input and output size in bits
{
  ...
  if (tableByteSize == copyByteSize) {
    TR_TranslateTableData *match =
        TR_TranslateTable::matchTable(inputSize, outputSize, set);
    if (match) {
      setTable(match);
      return;
    }
