...
public:
  ...
  TR_StoreTreeInfo(TR::TreeTop* tt,
                   TR::Node* load,
                   TR_NodeIndexPair* loads,
                   TR::TreeTop* insertionTree,
                   TR::Node* loadUsedInLoopIncrement,
                   bool b,
                   TR::Node* constNode,
                   bool isAddition)
    : _tt(tt)
...
public:
  ...
  TR::DataType findDataType(TR::Node* node,
                            bool usingAladd,
  ...
  void populateLinearEquation(TR::Node* node,
  ...
  typedef std::
    map<ncount_t, SignExtEntry, std::less<ncount_t>, SignExtMemoAllocator>
      SignExtMemo;

  void morphExpressionsLinearInInductionVariable(TR_Structure*, vcount_t);
  bool morphExpressionLinearInInductionVariable(TR::Node*,
                                                int32_t,
                                                TR::Node*,
                                                vcount_t);
  TR::Node* getInductionVariableNode(TR::Node*);
  void walkTreesAndFixUseDefs(TR_Structure*,
                              TR::SymbolReference*,
                              TR::NodeChecklist&);
  void replaceLoadsInStructure(TR_Structure*,
                               int32_t,
                               TR::Node*,
                               TR::SymbolReference*,
                               TR::NodeChecklist&,
                               TR::NodeChecklist&);
  void replaceLoadsInSubtree(TR::Node*,
                             int32_t,
                             TR::Node*,
                             TR::SymbolReference*,
                             TR::NodeChecklist&,
  ...
  void eliminateSignExtensionsInSubtree(TR::Node*,
                                        TR::NodeChecklist&,
                                        TR::NodeChecklist&,
                                        SignExtMemo&);
  SignExtEntry signExtend(TR::Node*, TR::NodeChecklist&, SignExtMemo&);
  SignExtEntry signExtendBinOp(TR::ILOpCodes,
                               TR::Node*,
                               TR::NodeChecklist&,
                               SignExtMemo&);
  ...
  TR::Node* placeNewInductionVariableIncrementTree(TR_BlockStructure*,
                                                   TR::SymbolReference*,
                                                   TR::SymbolReference*,
                                                   int32_t,
                                                   TR::SymbolReferenceTable*,
  ...
  bool reassociateAndHoistComputations(TR::Block*,
                                       TR::Node*,
                                       int32_t,
                                       TR::Node*,
                                       vcount_t);
  ...
  TR::Node* duplicateAdditiveTermNode(int32_t k,
                                      TR::Node* node,
                                      TR::DataType type)
  {
    TR_ASSERT(k < _numberOfLinearExprs,
  ...
  typedef std::
    map<uint32_t, TR::SymbolReference*, SymRefMapComparator, SymRefMapAllocator>
      SymRefMap;
  ...
  typedef std::
    map<uint32_t, SymRefPair*, SymRefPairMapComparator, SymRefPairMapAllocator>
      SymRefPairMap;
...
private:
  ...
  TR::VPLongRange* genVPLongRange(TR::VPConstraint* cons,
                                  int64_t coeff,
                                  int64_t additive);
  TR::VPIntRange* genVPIntRange(TR::VPConstraint* cons,
                                int64_t coeff,
                                int64_t additive);
};
...
public:
  ...
  TR_BasicInductionVariable(TR::Compilation* c,
...
private:
  ...
  bool analyzeExitEdges(TR_RegionStructure* loop,
  ...
  TR::Node* findEntryValueForSymRef(TR_RegionStructure* loop,
                                    TR::SymbolReference* symRef);
  TR::Node* getEntryValue(TR::Block* block,
                          TR::SymbolReference* symRef,
                          TR_BitVector* nodesDone,
                          TR_Array<TR::Node*>& cachedVales);

  bool isProgressionalStore(TR::Node* node,
                            TR_ProgressionKind* kind,
                            int64_t* incr);
  bool getProgression(TR::Node* expr,
  ...
  bool branchContainsInductionVariable(TR::Node* branchNode,
                                       ...
                                       int32_t* nodeBudget);
  bool isIVUnchangedInLoop(TR_RegionStructure* loop,
  ...
  void mergeWithBlock(TR::Block* block,
                      DeltaInfo** info,
                      TR_RegionStructure* loop);
