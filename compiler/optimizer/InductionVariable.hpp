...
public:
  ...
  TR_StoreTreeInfo(TR::TreeTop *tt, TR::Node *load, TR_NodeIndexPair *loads,
                   TR::TreeTop *insertionTree,
                   TR::Node *loadUsedInLoopIncrement, bool b,
                   TR::Node *constNode, bool isAddition)
      : _tt(tt), _load(load), _loads(loads), _insertionTreeTop(insertionTree),
        _loadUsedInLoopIncrement(loadUsedInLoopIncrement),
        _incrementInDifferentExtendedBlock(b), _constNode(constNode),
...
public:
  ...
  virtual int32_t detectCanonicalizedPredictableLoops(TR_Structure *,
                                                      TR_BitVector **, int32_t);

  // bool replaceAllInductionVariableComputations(TR::Block *loopInvariantBlock,
  // TR_Structure *, TR::SymbolReference **, TR::SymbolReference *);
  virtual bool examineTreeForInductionVariableUse(TR::Block *loopInvariantBlock,
                                                  TR::Node *, int32_t,
                                                  TR::Node *, vcount_t,
                                                  TR::SymbolReference **);
  virtual void updateStoreInfo(int32_t i, TR::TreeTop *tree);
  virtual void
  checkIfIncrementInDifferentExtendedBlock(TR::Block *block,
                                           int32_t inductionVariable);
  virtual bool isStoreInRequiredForm(int32_t symRefNum,
                                     TR_Structure *loopStructure);
  virtual TR::Node *updateLoadUsedInLoopIncrement(TR::Node *, int32_t);

  bool isStoreInRequiredForm(TR::Node *storeNode, int32_t symRefNum,
                             TR_Structure *loopStructure);
  TR::Node *setUsesLoadUsedInLoopIncrement(TR::Node *node, int32_t k);

  TR::Node *getNewLoopIncrement(TR_StoreTreeInfo *info, int32_t k);
  TR::Node *getNewLoopIncrement(TR::Node *oldLoad, int32_t k,
                                int32_t symRefNum);
  void addLoad(TR_StoreTreeInfo *info, TR::Node *load, int32_t index);
  void findOrCreateStoreInfo(TR::TreeTop *tree, int32_t i);

  int32_t findNewInductionVariable(TR::Node *, TR::SymbolReference **, bool,
                                   int32_t);
  ...
  bool foundLoad(TR::TreeTop *, int32_t nextInductionVariableNumber,
                 TR::Compilation *);
  bool foundValue(TR::Node *, int32_t, vcount_t);
  bool unchangedValueNeededIn(TR::Block *, int32_t, bool &seenStore);
  void changeLoopCondition(TR_BlockStructure *loopInvariantBlock,
                           bool usingAladd, int32_t bestCandidate,
                           TR::Node *storeOfDerivedInductionVariable);
  void createParmAutoPair(TR::SymbolReference *parmSymRef,
                          TR::SymbolReference *autoSymRef);

  TR::Node *findReplacingNode(TR::Node *node, bool usingAladd, int32_t k);
  TR::DataType findDataType(TR::Node *node, bool usingAladd,
                            bool isInternalPointer);
  void placeStore(TR::Node *newStore, TR::Block *loopInvariantBlock);
  void setInternalPointer(TR::Symbol *symbol,
                          TR::AutomaticSymbol *pinningArrayPointer);
  void populateLinearEquation(TR::Node *node, int32_t loopDrivingInductionVar,
  ...
  typedef TR::typed_allocator<std::pair<ncount_t const, SignExtEntry>,
                              TR::Allocator>
      SignExtMemoAllocator;
  typedef std::map<ncount_t, SignExtEntry, std::less<ncount_t>,
                   SignExtMemoAllocator>
      SignExtMemo;

  void morphExpressionsLinearInInductionVariable(TR_Structure *, vcount_t);
  bool morphExpressionLinearInInductionVariable(TR::Node *, int32_t, TR::Node *,
                                                vcount_t);
  TR::Node *getInductionVariableNode(TR::Node *);
  void walkTreesAndFixUseDefs(TR_Structure *, TR::SymbolReference *,
                              TR::NodeChecklist &);
  void replaceLoadsInStructure(TR_Structure *, int32_t, TR::Node *,
                               TR::SymbolReference *, TR::NodeChecklist &,
                               TR::NodeChecklist &);
  void replaceLoadsInSubtree(TR::Node *, int32_t, TR::Node *,
                             TR::SymbolReference *, TR::NodeChecklist &,
                             TR::NodeChecklist &);
  void widenComparison(TR::Node *, int32_t, TR::Node *, TR::NodeChecklist &);
  void eliminateSignExtensions(TR::NodeChecklist &);
  void eliminateSignExtensionsInSubtree(TR::Node *, TR::NodeChecklist &,
                                        TR::NodeChecklist &, SignExtMemo &);
  SignExtEntry signExtend(TR::Node *, TR::NodeChecklist &, SignExtMemo &);
  SignExtEntry signExtendBinOp(TR::ILOpCodes, TR::Node *, TR::NodeChecklist &,
                               SignExtMemo &);
  ...
  void extendIVsOnLoopEntry(const TR::list<std::pair<int32_t, int32_t>> &,
                            TR::Block *);
  void truncateIVsOnLoopExit(const TR::list<std::pair<int32_t, int32_t>> &,
                             TR_RegionStructure *);
  void convertIV(TR::Node *, TR::TreeTop *, int32_t, int32_t, TR::ILOpCodes);
  void createConstraintsForNewInductionVariable(TR_Structure *,
                                                TR::SymbolReference *,
                                                TR::SymbolReference *);
  ...
  TR::Node *placeInitializationTreeInLoopInvariantBlock(
      TR_BlockStructure *, TR::SymbolReference *, TR::SymbolReference *,
      int32_t, TR::SymbolReferenceTable *);
  ...
  bool reassociateAndHoistComputations(TR::Block *, TR::Node *, int32_t,
                                       TR::Node *, vcount_t);
  ...
  int32_t maxInternalPointers() {
    // was int32_t
    // reservedForLoops=comp()->getOption(TR_ProcessHugeMethods)?6:2; increased
    // to 8 since we did see the limit being reached and believe 2 would be too
    // little as a buffer for the internal pointer spills. 8 is a hypothetical
  ...
  void setAdditiveTermNode(TR::Node *node, int32_t k) {
    ...
    _linearEquations[k][3] = (intptrj_t)node;
  }
  TR::Node *getAdditiveTermNode(int32_t k) {
    TR_ASSERT(k < _numberOfLinearExprs,
              "index k %d exceeds _numberOfLinearExprs %d!\n", k,
              _numberOfLinearExprs);
    return (TR::Node *)(intptrj_t)_linearEquations[k][3];
  }
  TR::Node *duplicateAdditiveTermNode(int32_t k, TR::Node *node,
                                      TR::DataType type) {
    TR_ASSERT(k < _numberOfLinearExprs,
              "index k %d exceeds _numberOfLinearExprs %d!\n", k,
              _numberOfLinearExprs);
    TR::Node *new_node =
        ((TR::Node *)(intptrj_t)_linearEquations[k][3])->duplicateTree();
  ...
  bool isAdditiveTermConst(int32_t k) {
    ...
    return (((TR::Node *)(intptrj_t)_linearEquations[k][3]) == NULL ||
            ((TR::Node *)(intptrj_t)_linearEquations[k][3])
                ->getOpCode()
                .isLoadConst());
  }
  ...
  void setMulTermNode(TR::Node *node, int32_t k) {
    TR_ASSERT(k < _numberOfLinearExprs,
              "index k %d exceeds _numberOfLinearExprs %d!\n", k,
              _numberOfLinearExprs);
    _linearEquations[k][2] = (intptrj_t)node;
  }
  TR::Node *getMulTermNode(int32_t k) {
    TR_ASSERT(k < _numberOfLinearExprs,
              "index k %d exceeds _numberOfLinearExprs %d!\n", k,
              _numberOfLinearExprs);
    return (TR::Node *)(intptrj_t)_linearEquations[k][2];
  }
  ...
  bool isExprLoopInvariant(TR::Node *node) {
    ...
    if (TR::ILOpCode::isOpCodeAnImplicitNoOp(node->getOpCode()) &&
  ...
  typedef TR::typed_allocator<std::pair<uint32_t const, TR::SymbolReference *>,
                              TR::Region &>
      SymRefMapAllocator;
  typedef std::less<uint32_t> SymRefMapComparator;
  typedef std::map<uint32_t, TR::SymbolReference *, SymRefMapComparator,
                   SymRefMapAllocator>
      SymRefMap;
  ...
  typedef TR::typed_allocator<
      std::pair<uint32_t const, List<TR_StoreTreeInfo> *>, TR::Region &>
      StoreTreeMapAllocator;
  typedef std::less<uint32_t> StoreTreeMapComparator;
  typedef std::map<uint32_t, List<TR_StoreTreeInfo> *, StoreTreeMapComparator,
                   StoreTreeMapAllocator>
      StoreTreeMap;
  ...
  typedef TR::typed_allocator<std::pair<uint32_t const, SymRefPair *>,
                              TR::Region &>
      SymRefPairMapAllocator;
  typedef std::less<uint32_t> SymRefPairMapComparator;
  typedef std::map<uint32_t, SymRefPair *, SymRefPairMapComparator,
                   SymRefPairMapAllocator>
      SymRefPairMap;
...
private:
  TR::Node *genLoad(TR::Node *node, TR::SymbolReference *symRef,
                    bool isInternalPointer);
  void examineOpCodesForInductionVariableUse(
  ...
  TR::VPLongRange *genVPLongRange(TR::VPConstraint *cons, int64_t coeff,
                                  int64_t additive);
  TR::VPIntRange *genVPIntRange(TR::VPConstraint *cons, int64_t coeff,
                                int64_t additive);
};
...
public:
  ...
  TR_BasicInductionVariable(TR::Compilation *c, TR_RegionStructure *loop,
                            TR::SymbolReference *symRef)
      : _comp(c), _loop(loop), _symRef(symRef), _entryValue(0),
        _deltaOnBackEdge(0), _deltaOnExitEdge(0), _increment(0),
        _onlyIncrementValid(false) {}
  TR_BasicInductionVariable(TR::Compilation *c, TR_BasicInductionVariable *biv)
      : _comp(c), _loop(biv->_loop), _symRef(biv->_symRef),
        _entryValue(biv->_entryValue), _deltaOnBackEdge(biv->_deltaOnBackEdge),
        _deltaOnExitEdge(biv->_deltaOnExitEdge), _increment(biv->_increment) {
    _onlyIncrementValid = false;
  }
...
public:
  ...
  TR_PrimaryInductionVariable(TR_BasicInductionVariable *biv,
                              TR::Block *branchBlock, TR::Node *exitBound,
                              TR::ILOpCodes exitOp, TR::Compilation *comp,
                              TR::Optimizer *opt,
  ...
  void setUsesUnchangedValueInLoopTest(bool v) {
    _usesUnchangedValueInLoopTest = v;
  }
...
public:
  TR_ALLOC(TR_Memory::InductionVariableAnalysis);
  TR_DerivedInductionVariable(TR::Compilation *c,
...
private:
  ...
  public:
    ...
    DeltaInfo(DeltaInfo *other)
        : _delta(other->_delta), _unknown(other->_unknown),
          _kind(other->_kind) {}
  ...
  void analyzeAcyclicRegion(TR_RegionStructure *region,
                            TR_RegionStructure *loop);
  void analyzeCyclicRegion(TR_RegionStructure *region,
                           TR_RegionStructure *loop);
  ...
  bool isProgressionalStore(TR::Node *node, TR_ProgressionKind *kind,
                            int64_t *incr);
  bool getProgression(TR::Node *expr, TR::SymbolReference *storeRef,
                      TR::SymbolReference **ref, TR_ProgressionKind *prog,
                      int64_t *incr);
  ...
  bool branchContainsInductionVariable(
      ...
      TR_Array<TR_BasicInductionVariable *> &basicIVs);
  bool branchContainsInductionVariable(TR::Node *branchNode,
                                       TR::SymbolReference *ivSymRef,
                                       int32_t *nodeBudget);
  bool isIVUnchangedInLoop(TR_RegionStructure *loop, TR::Block *loopTestBlock,
                           TR::Node *ivNode);
  ...
  void mergeWithBlock(TR::Block *block, DeltaInfo **info,
                      TR_RegionStructure *loop);
...
/*
 * This pass attempts to transforms loops with single back-edge testing address
 * nodes into ones with integer node tests. The main motivation is that IVA does
 * not recognize address type node. Since a lot of loop-based optimizations and
 * analysis are written for loops with integral IV, this is a simple solution to
 * catch many address incrementing loops. The other, full-fledged solution is to
 * recognize address IV in IVA and everywhere else, but that may entail much
 * more work.
 *
 * Address IV is also a fairly common coding pattern in C (pointer manip) and
 * C++ with iterators of contiguous memory. This pass is analogous to loop
 * strider in that it undo's the pointer incr which strides by size of
 * underlying type.
 */
class TR_IVTypeTransformer : public TR_LoopTransformer {
public:
  TR_IVTypeTransformer(TR::OptimizationManager *manager)
      : TR_LoopTransformer(manager), _addrSymRef(NULL), _baseSymRef(NULL),
        _intIdxSymRef(NULL) {}
  static TR::Optimization *create(TR::OptimizationManager *manager) {
