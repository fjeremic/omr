...
public:
    ...
    {
        TR_ASSERT(k < _numberOfLinearExprs, "index k %d exceeds _numberOfLinearExprs %d!\n", k, _numberOfLinearExprs);
        return (((TR::Node*)(intptrj_t)_linearEquations[k][3]) == NULL || ((TR::Node*)(intptrj_t)_linearEquations[k][3])->getOpCode().isLoadConst());
    }
    ...
    {
        if (node->getOpCode().isLoadConst() || (node->getOpCode().isLoadVarDirect() && node->getSymbol()->isAutoOrParm() && _neverWritten->get(node->getSymbolReference()->getReferenceNumber())))
            return true;
