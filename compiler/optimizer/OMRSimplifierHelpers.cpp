...
void orderChildren(TR::Node* node, TR::Node*& firstChild, TR::Node*& secondChild, TR::Simplifier* s)
{
    ...
    // R2:
    else if ((region = s->containingStructure()) && !isExprInvariant(region, secondChild) && isExprInvariant(region, firstChild)) {
...
TR::Node* foldRedundantAND(TR::Node* node, TR::ILOpCodes andOpCode, TR::ILOpCodes constOpCode, int64_t andVal, TR::Simplifier* s)
{
    ...
    if (((val & andVal) == andVal) && (andChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sFolding redundant AND node [%s] and its children [%s, %s]\n", s->optDetailString(), node->getName(s->getDebug()), lhsChild->getName(s->getDebug()), constChild->getName(s->getDebug()))) {
...
bool isNaNFloat(TR::Node* node)
{
    ...
    return ((value >= FLOAT_NAN_1_LOW && value <= FLOAT_NAN_1_HIGH) || (value >= FLOAT_NAN_2_LOW && value <= FLOAT_NAN_2_HIGH));
}
...
TR::Node* replaceExpWithMult(TR::Node* node, TR::Node* valueNode, TR::Node* exponentNode, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (exponentNode->getOpCode().isLoadConst() && kMaxPositiveExponent >= 0 && kMaxPositiveExponent <= TR::getMaxSigned<TR::Int32>() && kMaxNegativeExponent >= TR::getMinSigned<TR::Int32>() && kMaxNegativeExponent <= 0) {
        ...
        case TR::dcall: // Math.pow(D)
        {
            ...
            if (exponentNode->getType().isIntegral()) {
                ...
                if (isNaNDouble(exponentNode) && performTransformation(s->comp(), "%sReplacing Math.pow(X,NaN) call with dconst NaN [%p]\n", s->optDetailString(), node)) {
                    s->prepareToReplaceNode(node, TR::dconst);
        ...
        if (isPowAndReasonableIntExponent && performTransformation(s->comp(), "%sStrength reduce %s [" POINTER_PRINTF_FORMAT "] with power = %d to a series of multiplications\n", s->optDetailString(), node->getOpCode().getName(), node, (int32_t)powExponent)) {
