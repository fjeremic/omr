...
static bool shouldSwapChildren(TR::Node *firstChild, TR::Node *secondChild) {
  ...
  if (firstOrdinal < secondOrdinal) return false;
  if (firstOrdinal > secondOrdinal) return true;
  if (firstChild->getNumChildren() == 0) return false;
  if (secondChild->getNumChildren() == 0) return true;
  return shouldSwapChildren(firstChild->getFirstChild(),
...
static bool swapChildren(TR::Node *node, TR::Simplifier *s) {
  // NB: although one might be tempted, do not turn this child swap
  // dumpOptDetails into a performTransformation because we require that a tree
  // with a constant node have that constant as the second child, unless both
...
bool performTransformationSimplifier(TR::Node *node, TR::Simplifier *s) {
  return performTransformation(
      s->comp(), "%sConstant folding node [%s] %s", s->optDetailString(),
...
TR::Node *_gotoSimplifier(TR::Node *node, TR::Block *block,
                          TR::TreeTop *curTree, TR::Optimization *s) {
  if (branchToFollowingBlock(node, block, s->comp())) {
    if (node->getNumChildren() > 0) {
      TR_ASSERT(node->getFirstChild()->getOpCodeValue() == TR::GlRegDeps,
                "Expecting TR::GlRegDeps");

      // has GlRegDeps(after GRA), can be removed only if BBExit has exactly the
      // same GlRegDeps
      if (block->getExit()->getNode()->getNumChildren() == 0) return node;
    ...
    if (performTransformation(s->comp(),
                              "%sRemoving goto [" POINTER_PRINTF_FORMAT
                              "] to following block\n",
...
void foldIntConstant(TR::Node *node, int32_t value, TR::Simplifier *s,
...
void foldUIntConstant(TR::Node *node, uint32_t value, TR::Simplifier *s,
                      bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
...
void foldLongIntConstant(TR::Node *node, int64_t value, TR::Simplifier *s,
                         bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
  ...
  s->prepareToReplaceNode(node,
                          node->getOpCode().isRef() ? TR::aconst : TR::lconst);
...
void foldByteConstant(TR::Node *node, int8_t value, TR::Simplifier *s,
                      bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
...
void foldShortIntConstant(TR::Node *node, int16_t value, TR::Simplifier *s,
                          bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
...
bool swapChildren(TR::Node *node, TR::Node *&firstChild, TR::Node *&secondChild,
                  TR::Simplifier *s) {
...
//
void orderChildren(TR::Node *node, TR::Node *&firstChild,
  ...
  // R2:
    ...
    if (performTransformation(s->comp(),
                              "%sApplied reassociation rule 2 to node 0x%p\n",
                              s->optDetailString(), node))
      swapChildren(node, firstChild, secondChild, s);
  }
  // R2:
...
TR::Node *foldRedundantAND(TR::Node *node, TR::ILOpCodes andOpCode,
                           TR::ILOpCodes constOpCode, int64_t andVal,
                           TR::Simplifier *s) {
  TR::Node *andChild = node->getFirstChild();

  if (andChild->getOpCodeValue() != andOpCode) return 0;
  ...
  if (((val & andVal) == andVal) && (andChild->getReferenceCount() == 1) &&
      performTransformation(
          s->comp(),
          "%sFolding redundant AND node [%s] and its children [%s, %s]\n",
          s->optDetailString(), node->getName(s->getDebug()),
          lhsChild->getName(s->getDebug()),
...
/** \brief
 *     Attempts to fold a logical and operation whose first child is a widening
 * operation and whose second child is a constant node such that no bits in the
 * constant node could ever overlap with the bits in the widened value.
 *
 *  \param simplifier
  ...
  if (node->getOpCode().isAnd()) {
    ...
    if (rhsNode->getOpCode().isLoadConst()) {
      ...
      if (lhsNode->getOpCode().isZeroExtension() ||
          (lhsNode->getOpCode().isSignExtension() &&
           lhsNode->isNonNegative())) {
        ...
        TR_ASSERT(node->getSize() > extensionValueNode->getSize(),
                  "Extended value datatype size must be smaller than the "
                  "parent's datatype size.");

        // Produce a mask of 1 bits of the same width as the value being
        // extended
        int64_t mask = (1ll << (8ll * static_cast<int64_t>(
                                          extensionValueNode->getSize()))) -
                       1ll;

        if ((rhsNode->getConstValue() & mask) == 0) {
          if (performTransformation(
                  simplifier->comp(),
                  "%sConstant folding widened and node [%p] to zero\n",
            ...
            simplifier->prepareToReplaceNode(
                node, TR::ILOpCode::constOpCode(node->getDataType()));
...
//
bool branchToFollowingBlock(TR::Node *node, TR::Block *block,
                            TR::Compilation *comp) {
  ...
  if (treeTop->getNode() != node) return false;
  return true;
}

// If the first child is a constant but the second isn't, swap them.
//
void makeConstantTheRightChild(TR::Node *node, TR::Node *&firstChild,
                               TR::Node *&secondChild, TR::Simplifier *s) {
...
void makeConstantTheRightChildAndSetOpcode(TR::Node *node,
                                           TR::Node *&firstChild,
                                           TR::Node *&secondChild,
                                           TR::Simplifier *s) {
  if (firstChild->getOpCode().isLoadConst() &&
...
TR::Node *replaceChild(int32_t childIndex, TR::Node *node, TR::Node *newChild,
                       TR::Simplifier *s) {
...
TR::Node *postWalkLowerTreeSimplifier(TR::TreeTop *tt, TR::Node *node,
                                      TR::Block *block, TR::Simplifier *s) {
  TR::TreeTop *newTree = s->comp()->cg()->lowerTree(node, tt);
  if (newTree != s->_curTree)
    s->_curTree = newTree =
        newTree->getPrevTreeTop();  // set it to the previous treetop so
                                    // simplier will next walk the new tree

  return node;
}

void foldFloatConstantEmulate(TR::Node *node, uint32_t value,
                              TR::Simplifier *s) {
  TR_ASSERT(false, "foldFloatConstantEmulate not implemented\n");
  return;
}

void foldDoubleConstantEmulate(TR::Node *node, uint64_t value,
                               TR::Simplifier *s) {
...
bool isNZFloatPowerOfTwo(float value) {
  ...
  if (float_exp != 0 && float_exp != 0xff && float_frac == 0) return true;
  return false;
}
...
bool isNZDoublePowerOfTwo(double value) {
  ...
  if (double_exp != 0 && double_exp != 0x7ff && double_frac == 0) return true;
  return false;
}

// Exponentiation operations must be sensitive to the signedness of the exponent
// (the base signedness does not matter) If the exp operation itself is
// unsigned, then the exponent value is interpreted as an unsigned number. This
// matters because otherwise (for example) an 8 bit exponent with the encoding
// 0xFF would be interpreted as base ** -1 instead of base ** 255
bool isIntegralExponentInRange(TR::Node *parent, TR::Node *exponent,
                               int64_t maxNegativeExponent,
                               int64_t maxPositiveExponent, TR::Simplifier *s) {
  TR_ASSERT(exponent->getType().isIntegral(),
            "isIntegralExponentInRange only valid for integral exponents and "
            "not type %s\n",
            exponent->getDataType().toString());
  TR_ASSERT(
      parent->getOpCode().isExponentiation(),
      "isIntegralExponentInRange only valid for exponentiation operations\n");
  bool exponentInRange = false;
  bool isUnsignedExpOp = parent->getOpCode().isUnsignedExponentiation();
  if (exponent->getType().isIntegral()) {
    if (isUnsignedExpOp) {
      uint64_t unsignedExponentValue =
          exponent->get64bitIntegralValueAsUnsigned();
...
TR::Node *reduceExpTwoAndGreaterToMultiplication(
    int32_t exponentValue, TR::Node *baseNode, TR::ILOpCodes multOp,
    TR::Block *block, TR::Simplifier *s, int32_t maxExponent) {
  if (exponentValue <= 1) {
    TR_ASSERT(false,
              "reduceExpTwoAndGreaterToMultiplication only valid for values >= "
              "2 and not value=%d\n",
              exponentValue);
    return NULL;
  }
  ...
  if (s->comp()->cg()->multiplyIsDestructive()) {
    int32_t bitPosOfLeftMostOne =
        32 - leadingZeroes(exponentValue) -
        1;  // bitPos=0 is the least significant bit so for exponentValue=7 : 32
            // - 29 - 1 = 2
    resultNode = baseNode;
    if (bitPosOfLeftMostOne != 0) {
      for (int32_t i = bitPosOfLeftMostOne - 1; i >= 0; i--) {
        ...
        dumpOptDetails(s->comp(),
                       "%screated %s [" POINTER_PRINTF_FORMAT
                       "] operation for exponentiation strength reduction "
                       "(algorithmA/caseA)\n",
                       s->optDetailString(), resultNode->getOpCode().getName(),
                       resultNode);
        if (((exponentValue >> i) & 0x1) != 0) {
          ...
          dumpOptDetails(s->comp(),
                         ...
                         "(algorithmA/caseB)\n",
                         s->optDetailString(),
                         resultNode->getOpCode().getName(), resultNode);
        }
      }
    }
  } else {
    int32_t maxCeiling = ceilingPowerOfTwo(
        maxExponent);  // if maxExponent=31 then maxCeiling = 32
    int32_t maxCeilingExp =
        trailingZeroes(maxCeiling);  // (2^x=maxCeiling) so if maxCeiling = 32
                                     // then x=maxCeilingExp=5
    TR::Node **subTrees =
        (TR::Node **)s->comp()->trMemory()->allocateStackMemory(
            (maxCeilingExp + 1) * sizeof(TR::Node *));
    subTrees[0] = baseNode;
    int32_t i = 0;
    for (i = 1; exponentValue >= (CONSTANT64(1) << i);
         ++i)  // i can reach maxCeilingExp+1
    {
      int32_t j = i - 1;
      resultNode = subTrees[i] =
          TR::Node::create(multOp, 2, subTrees[j], subTrees[j]);
      dumpOptDetails(s->comp(),
                     "%screated %s [" POINTER_PRINTF_FORMAT
                     "] operation for exponentiation strength reduction "
                     "(algorithmB/caseA)\n",
...
TR::Node *replaceExpWithMult(TR::Node *node, TR::Node *valueNode,
                             TR::Node *exponentNode, TR::Block *block,
                             TR::Simplifier *s) {
  ...
  // negative power inlining generates the divide 1/pow(base,abs(exponent)) so
  // if base==0 then there are fe/language specific rules on what the
  // result/behaviour will be (e.g. Inf, hardware expection). The current
  // expansion below generates the divide ignorant of any of these rules so it
  // cannot be enabled globally.
  const int64_t kMaxNegativeExponent = 0;

  if (exponentNode->getOpCode().isLoadConst() && kMaxPositiveExponent >= 0 &&
      kMaxPositiveExponent <= TR::getMaxSigned<TR::Int32>() &&
      kMaxNegativeExponent >= TR::getMinSigned<TR::Int32>() &&
      kMaxNegativeExponent <= 0) {
    ...
    switch (node->getOpCodeValue()) {
      ...
      case TR::fexp:  // only integer exponents currently handled for fexp
      {
        ...
        if (exponentNode->getType().isIntegral()) {
          isPowAndReasonableIntExponent = isIntegralExponentInRange(
              node, exponentNode, kMaxNegativeExponent, kMaxPositiveExponent,
              s);
          if (isUnsignedExpOp)
            powExponent =
                (int64_t)exponentNode->get64bitIntegralValueAsUnsigned();
          else
      ...
      case TR::dcall:  // Math.pow(D)
      {
        ...
        if (exponentNode->getType().isIntegral()) {
          ...
          if (isNaNDouble(exponentNode) &&
              performTransformation(
                  s->comp(),
                  "%sReplacing Math.pow(X,NaN) call with dconst NaN [%p]\n",
                  s->optDetailString(), node)) {
            s->prepareToReplaceNode(node, TR::dconst);
    ...
    if (isPowAndReasonableIntExponent &&
        performTransformation(
            s->comp(),
            "%sStrength reduce %s [" POINTER_PRINTF_FORMAT
            "] with power = %d to a series of multiplications\n",
            s->optDetailString(), node->getOpCode().getName(), node,
            (int32_t)powExponent)) {
      TR::Node *origNode = node;
      bool isExponentNegative = powExponent < 0;
      int32_t absPowExponent =
          (int32_t)(isExponentNegative ? -powExponent : powExponent);
      if (0 == absPowExponent) {
        ...
        if (isExponentNegative) {
          valueNode->incReferenceCount();  // keep node alive across
                                           // prepareToReplaceNode call
          s->prepareToReplaceNode(origNode, divideOp);
          origNode->setNumChildren(2);
          origNode->setAndIncChild(
              0, TR::Node::createConstOne(node, node->getDataType()));
        ...
        TR_ASSERT(
            kMaxPositiveExponent >= 0 &&
                kMaxPositiveExponent <= TR::getMaxSigned<TR::Int32>(),
            "kMaxPositiveExponent should not exceed integer limits\n");  // checked
                                                                         // above
        TR_ASSERT(
            kMaxNegativeExponent >= TR::getMinSigned<TR::Int32>() &&
                kMaxNegativeExponent <= 0,
            "kMaxNegativeExponent should not exceed integer limits\n");  // checked
                                                                         // above
        TR_ASSERT(
            absPowExponent >= TR::getMinSigned<TR::Int32>() &&
                absPowExponent <= TR::getMaxSigned<TR::Int32>(),
            "exponent should not exceed integer limits\n");  // checked above
        int32_t maxExponent = isExponentNegative
                                  ? (int32_t)kMaxNegativeExponent
                                  : (int32_t)kMaxPositiveExponent;

        node = reduceExpTwoAndGreaterToMultiplication(
            absPowExponent, valueNode, multiplyOp, block, s, maxExponent);
...
// NOTE: This function only (and should only) decodes opcodes found in
// conversionMap table!!!
bool decodeConversionOpcode(TR::ILOpCode op, TR::DataType nodeDataType,
                            TR::DataType &sourceDataType,
  ...
  if (!op.isConversion()) {
    ...
    for (int i = 0; i < TR::NumTypes; i++) {
      ...
      if (opValue ==
          TR::ILOpCode::getProperConversion(sourceDataType, targetDataType,
...
void removePaddingNode(TR::Node *node, TR::Simplifier *s) {
  ...
  if (paddingNode) {
    ...
    dumpOptDetails(s->comp(),
                   "remove paddingNode %s (0x%p) from %s (0x%p) and dec "
                   "numChildren %d->%d\n",
                   paddingNode->getOpCode().getName(), paddingNode,
                   node->getOpCode().getName(), node, oldNumChildren,
                   oldNumChildren - 1);
  }
}

// the usual behaviour is to remove the padding node except in cases where the
// caller is already dealing with this
void stopUsingSingleNode(TR::Node *node, bool removePadding,
                         TR::Simplifier *s) {
  TR_ASSERT(node->getReferenceCount() == 1,
            "stopUsingSingleNode only valid for nodes with a referenceCount of "
            "1 and not %d\n",
            node->getReferenceCount());
  if (removePadding) removePaddingNode(node, s);
  if (node->getReferenceCount() <= 1) {
    if (s->optimizer()->prepareForNodeRemoval(
            node, /* deferInvalidatingUseDefInfo = */ true))
      s->_invalidateUseDefInfo = true;
    s->_alteredBlock = true;
  }
  node->decReferenceCount();
  if (node->getReferenceCount() > 0) node->setVisitCount(0);
}

TR::TreeTop *findTreeTop(TR::Node *callNode, TR::Block *block) {
  // Walk the extended block to find this node - has to be child of store or
  // tree-top
  TR::Block *b = block->startOfExtendedBlock();
  if (!b) return NULL;
  do {
    for (TR::TreeTop *tt = b->getEntry(); tt != b->getExit();
         tt = tt->getNextRealTreeTop()) {
      if (tt->getNode()->getNumChildren() == 1 &&
          tt->getNode()->getFirstChild() == callNode) {
        return tt;
      }
    }
    b = b->getNextBlock();
...
TR::Node *removeIfToFollowingBlock(TR::Node *node, TR::Block *block,
                                   TR::Simplifier *s) {
  if (branchToFollowingBlock(node, block, s->comp())) {
    ...
    if (performTransformation(
            s->comp(),
            "%sRemoving %s [" POINTER_PRINTF_FORMAT "] to following block\n",
            s->optDetailString(), node->getOpCode().getName(), node)) {
