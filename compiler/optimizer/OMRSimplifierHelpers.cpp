...
{
  return performTransformation(s->comp(), "%sConstant folding node [%s] %s", s->optDetailString(),
...
TR::Node* _gotoSimplifier(TR::Node* node, TR::Block* block, TR::TreeTop* curTree, TR::Optimization* s)
{
  if (branchToFollowingBlock(node, block, s->comp())) {
    ...
    if (performTransformation(
          s->comp(), "%sRemoving goto [" POINTER_PRINTF_FORMAT "] to following block\n", s->optDetailString(), node)) {
...
{
  if (firstChild->getOpCode().isLoadConst() && !secondChild->getOpCode().isLoadConst()) {
    TR_ASSERT(node->getOpCode().getOpCodeForSwapChildren() != TR::BadILOp, "cannot swap children of irreversible op");
...
bool isNaNFloat(TR::Node* node)
{
  ...
  return (
    (value >= FLOAT_NAN_1_LOW && value <= FLOAT_NAN_1_HIGH) || (value >= FLOAT_NAN_2_LOW && value <= FLOAT_NAN_2_HIGH));
}
...
// This matters because otherwise (for example) an 8 bit exponent with the encoding 0xFF would be interpreted as base **
// -1 instead of base ** 255
bool isIntegralExponentInRange(
  TR::Node* parent, TR::Node* exponent, int64_t maxNegativeExponent, int64_t maxPositiveExponent, TR::Simplifier* s)
{
  TR_ASSERT(exponent->getType().isIntegral(),
    "isIntegralExponentInRange only valid for integral exponents and not type %s\n",
    exponent->getDataType().toString());
  TR_ASSERT(
    parent->getOpCode().isExponentiation(), "isIntegralExponentInRange only valid for exponentiation operations\n");
  ...
  if (exponent->getType().isIntegral()) {
    if (isUnsignedExpOp) {
      ...
      if (signedExponentValue >= maxNegativeExponent && signedExponentValue <= maxPositiveExponent) {
        exponentInRange = true;
      }
    }
  }
...
TR::Node* reduceExpTwoAndGreaterToMultiplication(int32_t exponentValue, TR::Node* baseNode, TR::ILOpCodes multOp,
  TR::Block* block, TR::Simplifier* s, int32_t maxExponent)
{
  if (exponentValue <= 1) {
    TR_ASSERT(
      false, "reduceExpTwoAndGreaterToMultiplication only valid for values >= 2 and not value=%d\n", exponentValue);
    return NULL;
  }
  ...
  if (s->comp()->cg()->multiplyIsDestructive()) {
    int32_t bitPosOfLeftMostOne = 32 - leadingZeroes(exponentValue)
      - 1; // bitPos=0 is the least significant bit so for exponentValue=7 : 32 - 29 - 1 = 2
    resultNode = baseNode;
    if (bitPosOfLeftMostOne != 0) {
      for (int32_t i = bitPosOfLeftMostOne - 1; i >= 0; i--) {
        ...
        dumpOptDetails(s->comp(),
          ...
          "] operation for exponentiation strength reduction (algorithmA/caseA)\n",
        ...
        if (((exponentValue >> i) & 0x1) != 0) {
          ...
          dumpOptDetails(s->comp(),
            "%screated %s [" POINTER_PRINTF_FORMAT
            "] operation for exponentiation strength reduction (algorithmA/caseB)\n",
            s->optDetailString(), resultNode->getOpCode().getName(), resultNode);
        }
      }
    }
    ...
    TR::Node** subTrees
      = (TR::Node**)s->comp()->trMemory()->allocateStackMemory((maxCeilingExp + 1) * sizeof(TR::Node*));
    subTrees[0] = baseNode;
    int32_t i = 0;
    for (i = 1; exponentValue >= (CONSTANT64(1) << i); ++i) // i can reach maxCeilingExp+1
    {
      ...
      dumpOptDetails(s->comp(),
        "%screated %s [" POINTER_PRINTF_FORMAT "] operation for exponentiation strength reduction (algorithmB/caseA)\n",
        s->optDetailString(), resultNode->getOpCode().getName(), resultNode);
    }
    int32_t j = -1;
    uint32_t mask = 1;
    for (i = 0; i < maxCeilingExp; ++i) {
      if (exponentValue & (mask << i)) {
        if (j != -1) {
          ...
          dumpOptDetails(s->comp(),
            "%screated %s [" POINTER_PRINTF_FORMAT
            "] operation for exponentiation strength reduction (algorithmB/caseA))\n",
            s->optDetailString(), resultNode->getOpCode().getName(), resultNode);
        }
        j = i;
      }
...
TR::Node* replaceExpWithMult(
  TR::Node* node, TR::Node* valueNode, TR::Node* exponentNode, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (exponentNode->getOpCode().isLoadConst() && kMaxPositiveExponent >= 0
    && kMaxPositiveExponent <= TR::getMaxSigned<TR::Int32>() && kMaxNegativeExponent >= TR::getMinSigned<TR::Int32>()
    ...
    case TR::fexp: // only integer exponents currently handled for fexp
    {
      ...
      if (exponentNode->getType().isIntegral()) {
        isPowAndReasonableIntExponent
          = isIntegralExponentInRange(node, exponentNode, kMaxNegativeExponent, kMaxPositiveExponent, s);
        if (isUnsignedExpOp)
      ...
      if (exponentNode->getType().isIntegral()) {
        ...
        if (isNaNDouble(exponentNode)
          && performTransformation(
            s->comp(), "%sReplacing Math.pow(X,NaN) call with dconst NaN [%p]\n", s->optDetailString(), node)) {
          s->prepareToReplaceNode(node, TR::dconst);
          node->setLongInt(exponentNode->getLongInt());
          return node;
        }

        if (exponentValue >= kMaxNegativeExponent && exponentValue <= kMaxPositiveExponent) {
    ...
    if (isPowAndReasonableIntExponent
      && performTransformation(s->comp(),
        "%sStrength reduce %s [" POINTER_PRINTF_FORMAT "] with power = %d to a series of multiplications\n",
      ...
      if (0 == absPowExponent) {
        ...
        TR_ASSERT(kMaxPositiveExponent >= 0 && kMaxPositiveExponent <= TR::getMaxSigned<TR::Int32>(),
          "kMaxPositiveExponent should not exceed integer limits\n"); // checked above
        TR_ASSERT(kMaxNegativeExponent >= TR::getMinSigned<TR::Int32>() && kMaxNegativeExponent <= 0,
          "kMaxNegativeExponent should not exceed integer limits\n"); // checked above
        TR_ASSERT(absPowExponent >= TR::getMinSigned<TR::Int32>() && absPowExponent <= TR::getMaxSigned<TR::Int32>(),
