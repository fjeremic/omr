...
//
static bool
shouldSwapChildren(TR::Node* firstChild, TR::Node* secondChild)
{
...
//
static bool
swapChildren(TR::Node* node, TR::Simplifier* s)
{
...
//
void
simplifyChildren(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
//
bool
performTransformationSimplifier(TR::Node* node, TR::Simplifier* s)
{
  return performTransformation(s->comp(),
                               "%sConstant folding node [%s] %s",
                               s->optDetailString(),
                               node->getName(s->getDebug()),
                               node->getOpCode().getName());
}
...
void
setIsHighWordZero(TR::Node* node, TR::Simplifier* s)
{
...
void
foldLongIntConstant(TR::Node* node,
                    int64_t value,
                    TR::Simplifier* s,
                    bool anchorChildrenP)
{
...
void
foldFloatConstant(TR::Node* node, float value, TR::Simplifier* s)
{
  if (performTransformationSimplifier(node, s)) {
...
void
foldDoubleConstant(TR::Node* node, double value, TR::Simplifier* s)
{
  if (performTransformationSimplifier(node, s)) {
...
void
foldShortIntConstant(TR::Node* node,
                     int16_t value,
                     TR::Simplifier* s,
                     bool anchorChildrenP)
{
...
bool
isExprInvariant(TR_RegionStructure* region, TR::Node* node)
{
              ...
              TR::Simplifier* s)
{
  ...
  // R2:
    ...
    if (performTransformation(s->comp(),
                              "%sOrdering children of node 0x%p\n",
                              s->optDetailString(),
                              node))
      swapChildren(node, firstChild, secondChild, s);
  }
...
TR::Node*
foldRedundantAND(TR::Node* node,
                 TR::ILOpCodes andOpCode,
                 TR::ILOpCodes constOpCode,
                 int64_t andVal,
                 TR::Simplifier* s)
{
 ...
 */
TR::Node*
tryFoldAndWidened(TR::Simplifier* simplifier, TR::Node* node)
{
  if (node->getOpCode().isAnd()) {
    ...
    if (rhsNode->getOpCode().isLoadConst()) {
      ...
      if (lhsNode->getOpCode().isZeroExtension() ||
        ...
        // extended
        int64_t mask =
          (1ll << (8ll * static_cast<int64_t>(extensionValueNode->getSize()))) -
          1ll;
...
//
bool
branchToFollowingBlock(TR::Node* node, TR::Block* block, TR::Compilation* comp)
{
...
//
void
makeConstantTheRightChild(TR::Node* node,
                          TR::Node*& firstChild,
                          TR::Node*& secondChild,
                          TR::Simplifier* s)
{
...
void
makeConstantTheRightChildAndSetOpcode(TR::Node* node,
                                      TR::Node*& firstChild,
                                      TR::Node*& secondChild,
                                      TR::Simplifier* s)
{
...
TR::Node*
replaceChild(int32_t childIndex,
             TR::Node* node,
             TR::Node* newChild,
             TR::Simplifier* s)
{
...
TR::Node*
postWalkLowerTreeSimplifier(TR::TreeTop* tt,
                            TR::Node* node,
                            TR::Block* block,
                            TR::Simplifier* s)
{
  ...
  if (newTree != s->_curTree)
    s->_curTree = newTree =
      newTree->getPrevTreeTop(); // set it to the previous treetop so simplier
                                 // will next walk the new tree

  return node;
}

void
foldFloatConstantEmulate(TR::Node* node, uint32_t value, TR::Simplifier* s)
{
...
void
foldDoubleConstantEmulate(TR::Node* node, uint64_t value, TR::Simplifier* s)
{
...
bool
isNZFloatPowerOfTwo(float value)
{
...
bool
isNZDoublePowerOfTwo(double value)
{
...
bool
isIntegralExponentInRange(TR::Node* parent,
                          TR::Node* exponent,
                          int64_t maxNegativeExponent,
                          int64_t maxPositiveExponent,
                          TR::Simplifier* s)
{
...
TR::Node*
reduceExpTwoAndGreaterToMultiplication(int32_t exponentValue,
                                       TR::Node* baseNode,
                                       TR::ILOpCodes multOp,
                                       TR::Block* block,
                                       TR::Simplifier* s,
                                       int32_t maxExponent)
{
  ...
  if (s->comp()->cg()->multiplyIsDestructive()) {
    int32_t bitPosOfLeftMostOne = 32 - leadingZeroes(exponentValue) -
                                  1; // bitPos=0 is the least significant bit so
                                     // for exponentValue=7 : 32 - 29 - 1 = 2
    resultNode = baseNode;
    ...
    int32_t maxCeiling =
      ceilingPowerOfTwo(maxExponent); // if maxExponent=31 then maxCeiling = 32
    int32_t maxCeilingExp =
      trailingZeroes(maxCeiling); // (2^x=maxCeiling) so if maxCeiling = 32 then
                                  // x=maxCeilingExp=5
    TR::Node** subTrees =
...
TR::Node*
replaceExpWithMult(TR::Node* node,
                   TR::Node* valueNode,
                   TR::Node* exponentNode,
                   TR::Block* block,
                   TR::Simplifier* s)
{
      ...
      case TR::fexp: // only integer exponents currently handled for fexp
      {
        ...
        if (exponentNode->getType().isIntegral()) {
          isPowAndReasonableIntExponent = isIntegralExponentInRange(
            node, exponentNode, kMaxNegativeExponent, kMaxPositiveExponent, s);
          if (isUnsignedExpOp)
            powExponent =
    ...
    if (isPowAndReasonableIntExponent &&
      ...
      if (0 == absPowExponent) {
        switch (node->getDataType()) {
          ...
          default: {
            TR_ASSERT(false,
                      "unexpected exponent datatype %s\n",
...
// conversionMap table!!!
bool
decodeConversionOpcode(TR::ILOpCode op,
                       TR::DataType nodeDataType,
    ...
    for (int i = 0; i < TR::NumTypes; i++) {
      ...
      if (opValue ==
          TR::ILOpCode::getProperConversion(
            sourceDataType, targetDataType, false /*!wantZeroExtension*/)) {
        return true;
      }
    }
    return false;
...
int32_t
floatToInt(float value, bool roundUp)
{
...
int32_t
doubleToInt(double value, bool roundUp)
{
...
void
removePaddingNode(TR::Node* node, TR::Simplifier* s)
{
...
void
stopUsingSingleNode(TR::Node* node, bool removePadding, TR::Simplifier* s)
{
...
TR::TreeTop*
findTreeTop(TR::Node* callNode, TR::Block* block)
{
...
TR::Node*
removeIfToFollowingBlock(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (branchToFollowingBlock(node, block, s->comp())) {
    // Branch to the immediately following block. The branch can be removed
    //
    if (performTransformation(s->comp(),
                              "%sRemoving %s [" POINTER_PRINTF_FORMAT
                              "] to following block\n",
                              s->optDetailString(),
                              node->getOpCode().getName(),
                              node)) {
