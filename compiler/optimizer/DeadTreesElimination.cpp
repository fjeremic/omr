...
static bool fixUpTree(TR::Node* node, TR::TreeTop* treeTop, TR::NodeChecklist& visited, bool& highGlobalIndex, TR::Optimization* opt, vcount_t evaluatedVisitCount)
{
    ...
    if (node->getOpCodeValue() == TR::arraycmp && !node->isArrayCmpLen() && TR::Compiler->target.cpu.isX86()) {
    ...
    bool* cantMoveUnderBranch)
{
    ...
    if (cantMoveUnderBranch && (node->getOpCode().isLoadVarDirect() || node->getOpCode().isLoadReg()) && node->isDontMoveUnderBranch())
    ...
    if (seenArraylet) {
        if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()->getSymbol()->isArrayletShadowSymbol() && node->getReferenceCount() > 1) {
            *seenArraylet = true;
        }
    }
...
static bool containsNode(TR::Node* containerNode, TR::Node* node, vcount_t visitCount, TR::Compilation* comp, int32_t* height, int32_t* maxHeight, bool& canMoveIfVolatile)
{
    ...
    if (containerNode->getOpCode().hasSymbolReference() && (containerNode->getSymbol()->isShadow() || containerNode->getSymbol()->isStatic()))
    ...
    LongestPathMap& longestPaths)
{
...

    {
        ...
        if (containsNode(node, currentNode, visitCount, comp, &tempHeight, &maxHeight, canMoveIfVolatile)) {
            ...
            //
            if (isUnresolvedReference && node->getFirstChild()->getOpCode().isCall() && node->getFirstChild()->getSymbol()->castToMethodSymbol()->isJNI())
                return false;
        ...
        if ((node->getOpCode().isBranch() && (node->getOpCodeValue() != TR::Goto)) || (node->getOpCode().isJumpWithMultipleTargets() && node->getOpCode().hasBranchChildren()))
            *seenConditionalBranch = true;

        if (node->getOpCodeValue() == TR::treetop || node->getOpCode().isNullCheck() || node->getOpCode().isResolveCheck() || node->getOpCodeValue() == TR::ArrayStoreCHK || node->getOpCode().isSpineCheck()) {
            node = node->getFirstChild();
        }
...
void TR::DeadTreesElimination::prePerformOnBlocks()
{
    ...
    */
    ...
    for (TR::TreeTop* tt = comp()->getStartTree();
        ...
        if (node->getOpCodeValue() == TR::treetop && node->getFirstChild()->getVisitCount() == visitCount && performTransformation(comp(), "%sRemove trivial dead tree: %p\n", optDetailString(), node)) {
        ...
        if (comp()->useCompressedPointers() && !removed && isReadBarrierUnderTreetop(node) && node->getFirstChild()->getType() == TR::Address && node->getFirstChild()->getOpCode().isLoadIndirect()) {
        ...
        for (TR::Block* b = comp()->getStartBlock(); b; b = b->getNextBlock()) {
            ...
            if (startNode->getNumChildren() > 0 && !debug("disableEliminationOfGlRegDeps")) {
                ...
                for (int32_t i = glRegDeps->getNumChildren() - 1; i >= 0; --i) {
                    ...
                    if (dep->getReferenceCount() == 1 && (!dep->getOpCode().isFloatingPoint() || cg()->getSupportsJavaFloatSemantics()) && performTransformation(comp(), "%sRemove GlRegDep : %p\n", optDetailString(), glRegDeps->getChild(i)))
                        ...
                        for (auto e = b->getPredecessors().begin(); e != b->getPredecessors().end(); ++e) {
                            ...
                            if (parent->getOpCode().isJumpWithMultipleTargets() && parent->getOpCode().hasBranchChildren()) {
                                for (int32_t j = parent->getCaseIndexUpperBound() - 1; j > 0; --j) {
                                    ...
                                    if (caseNode->getBranchDestination() == startTT && caseNode->getNumChildren() > 0 && 0) // can't do this now that all glRegDeps are hung off the default branch
                                        removeGlRegDep(caseNode, registerNum, pred, this);
                                }
                            } else if (!parent->getOpCode().isReturn() && parent->getOpCodeValue() != TR::igoto && !(parent->getOpCode().isJumpWithMultipleTargets() && parent->getOpCode().hasBranchChildren()) && !(parent->getOpCodeValue() == TR::treetop && parent->getFirstChild()->getOpCode().isCall() && parent->getFirstChild()->getOpCode().isIndirect()))
...
static bool treeCanPossiblyBeRemoved(TR::Node* node)
{
    ...
    // other uses exist
    if (node->getOpCodeValue() != TR::treetop) {
        return (node->getOpCode().isAnchor() && node->getFirstChild()->getReferenceCount() == 1) || (node->getOpCode().isStoreReg() && node->getFirstChild()->getReferenceCount() == 1);
    }
...
int32_t TR::DeadTreesElimination::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    for (TR::TreeTopIterator iter(startTree, comp()); iter != endTree; ++iter) {
        ...
        //
        if (!treeCanPossiblyBeRemoved(node) && (delayedRegStoresBeforeThisPass || (iter.currentTree() == block->getLastRealTreeTop()) || !node->getOpCode().isStoreReg() || (node->getVisitCount() == visitCount))) {
            /*
        ...
        if (visitCount == child->getVisitCount()) {
            ...
            bool callWithNoSideEffects = child->getOpCode().isCall() && child->getSymbolReference()->getSymbol()->isResolvedMethod() && child->getSymbolReference()->getSymbol()->castToResolvedMethodSymbol()->isSideEffectFree();

            if (callWithNoSideEffects) {
                treeTopCanBeEliminated = true;
            } else if (!((childOpCode.isCall() && !callWithNoSideEffects) || childOpCode.isStore() || ((opCodeValue == TR::New || opCodeValue == TR::anewarray || opCodeValue == TR::newarray) && child->getReferenceCount() > 1) || opCodeValue == TR::multianewarray || opCodeValue == TR::MergeNew || opCodeValue == TR::checkcast || opCodeValue == TR::Prefetch || opCodeValue == TR::iu2l || ((childOpCode.isDiv() || childOpCode.isRem()) && child->getNumChildren() == 3))) {
                ...
                if (safeToReplaceNode) {
                    if (childOpCode.hasSymbolReference()) {
                        ...
                        if (symRef->getSymbol()->isAuto() || symRef->getSymbol()->isParm())
                            ...
                            if (childOpCode.isLoad() || (opCodeValue == TR::loadaddr) || (opCodeValue == TR:: instanceof) || (((opCodeValue == TR::New) || (opCodeValue == TR::anewarray || opCodeValue == TR::newarray)) &&
            ...
            if (treeTopCanBeEliminated && seenConditionalBranch) {
                if (!cg()->getSupportsJavaFloatSemantics()) {
                    if (child->getOpCode().isConversion() || child->getOpCode().isBooleanCompare()) {
                        if (child->getFirstChild()->getOpCode().isFloatingPoint() && !child->getOpCode().isFloatingPoint())
            ...
            if (!node->getOpCode().isStoreReg() || (node->getFirstChild()->getReferenceCount() == 1)) {
                ...
                if (performTransformation(comp(), "%sMove tree : [" POINTER_PRINTF_FORMAT "]([" POINTER_PRINTF_FORMAT "] = %s) to end of block\n", optDetailString(), node, node->getFirstChild(), node->getFirstChild()->getOpCode().getName())) {
                    ...
                    while (cursorTreeTop != lastTree) {
                        if (cursorTreeTop->getNode()->getOpCode().isStoreReg() && (cursorTreeTop->getNode()->getGlobalRegisterNumber() == iter.currentTree()->getNode()->getGlobalRegisterNumber())) {
                    ...
                    if (lastNode->getOpCode().isIf() && !lastNode->getOpCode().isCompBranchOnly() && prevLastNode->getOpCode().isStoreReg() && ((prevLastNode->getFirstChild() == lastNode->getFirstChild()) || (prevLastNode->getFirstChild() == lastNode->getSecondChild()))) {
