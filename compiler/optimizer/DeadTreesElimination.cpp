...
static OMR::TreeInfo *findOrCreateTreeInfo(TR::TreeTop *treeTop,
                                           List<OMR::TreeInfo> *targetTrees,
                                           TR::Compilation *comp) {
  ListIterator<OMR::TreeInfo> trees(targetTrees);
  OMR::TreeInfo *t;
  for (t = trees.getFirst(); t; t = trees.getNext()) {
    if (t->getTreeTop() == treeTop) return t;
  }
...
static bool fixUpTree(TR::Node *node, TR::TreeTop *treeTop,
                      TR::NodeChecklist &visited, bool &highGlobalIndex,
                      TR::Optimization *opt, vcount_t evaluatedVisitCount) {
  if (node->getVisitCount() == evaluatedVisitCount) return false;
  ...
  if (node->getOpCodeValue() == TR::arraycmp && !node->isArrayCmpLen() &&
      TR::Compiler->target.cpu.isX86()) {
    anchorArrayCmp = false;
  }

  if ((node->getReferenceCount() > 1) && !node->getOpCode().isLoadConst() &&
      anchorLoadaddr && anchorArrayCmp) {
    if (!opt->comp()->getOption(TR_ProcessHugeMethods)) {
      ...
      if (nodeCount > nodeCountLimit) {
        dumpOptDetails(opt->comp(), "%snode count %d exceeds limit %d\n",
    ...
    for (int32_t i = 0; i < node->getNumChildren(); ++i) {
      ...
      if (fixUpTree(child, treeTop, visited, highGlobalIndex, opt,
                    evaluatedVisitCount))
...
static inline bool isReadBarrierUnderTreetop(TR::Node *node) {
  return node->getOpCodeValue() == TR::treetop &&
         node->getFirstChild()->getOpCode().isReadBar();
}

bool collectSymbolReferencesInNode(
    TR::Node *node, TR::SparseBitVector &symbolReferencesInNode,
    int32_t *numDeadSubNodes, vcount_t visitCount, TR::Compilation *comp,
    bool *seenInternalPointer, bool *seenArraylet, bool *cantMoveUnderBranch) {
  ...
  // diagnostic("Walking node %p, height=%d, oldVisitCount=%d, visitCount=%d,
  // compVisitCount=%d\n", node, *height, oldVisitCount,
  // visitCount,comp->getVisitCount());
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; i--) {
    TR::Node *child = node->getChild(i);
    if (child->getFutureUseCount() == 1 && child->getReferenceCount() > 1 &&
        !child->getOpCode().isLoadConst())
      *numDeadSubNodes = (*numDeadSubNodes) + 1;

    collectSymbolReferencesInNode(
        child, symbolReferencesInNode, numDeadSubNodes, visitCount, comp,
        seenInternalPointer, seenArraylet, cantMoveUnderBranch);
  }

  // detect if this is a direct load that shouldn't be moved under a branch
  // (because an update was moved past this load by treeSimplification)
  ...
  if (seenInternalPointer && node->isInternalPointer() &&
  ...
  if (node->getOpCode().hasSymbolReference()) {
    symbolReferencesInNode[node->getSymbolReference()->getReferenceNumber()] =
        true;
  }
...
typedef std::map<TR::Node *, int32_t, std::less<TR::Node *>, LPAlloc>
    LongestPathMap;

static int32_t getLongestPathOfDAG(TR::Node *node, LongestPathMap &memo) {
  if (node->getNumChildren() == 0) return 0;
...
static bool containsNode(TR::Node *containerNode, TR::Node *node,
                         vcount_t visitCount, TR::Compilation *comp,
                         int32_t *height, int32_t *maxHeight,
                         bool &canMoveIfVolatile) {
  ...
  if (containerNode->getOpCode().hasSymbolReference() &&
      (containerNode->getSymbol()->isShadow() ||
       containerNode->getSymbol()->isStatic()))
    canMoveIfVolatile = false;

  (*height)++;
  if (*height > *maxHeight) *maxHeight = *height;
  for (int32_t i = 0; i < containerNode->getNumChildren(); ++i) {
    if (containsNode(containerNode->getChild(i), node, visitCount, comp, height,
                     maxHeight, canMoveIfVolatile))
      return true;
  }
  (*height)--;
...
static bool isSafeToReplaceNode(TR::Node *currentNode, TR::TreeTop *curTreeTop,
                                bool *seenConditionalBranch,
                                vcount_t visitCount, TR::Compilation *comp,
                                List<OMR::TreeInfo> *targetTrees,
                                bool &cannotBeEliminated,
  ...
  collectSymbolReferencesInNode(
      currentNode, symbolReferencesInNode, &numDeadSubNodes, visitCount, comp,
      &seenInternalPointer, &seenArraylet, &cantMoveUnderBranch);
  ...
  OMR::TreeInfo *curTreeInfo =
      findOrCreateTreeInfo(curTreeTop, targetTrees, comp);
  ...
  if (isUnresolvedReference) return false;
...

  {
    ...
    if (node->canGCandReturn() && seenInternalPointer) return false;

    int32_t tempHeight = 0;
    int32_t maxHeight = 0;
    bool canMoveIfVolatile = true;
    if (containsNode(node, currentNode, visitCount, comp, &tempHeight,
                     &maxHeight, canMoveIfVolatile)) {
      // TEMPORARY
      ...
      if (curTreeInfo) {
        OMR::TreeInfo *treeInfo =
            findOrCreateTreeInfo(treeTop, targetTrees, comp);
        int32_t height = treeInfo->getHeight();
        int32_t maxHeightUsed = maxHeight;
        if (maxHeightUsed < curMaxHeight) maxHeightUsed = curMaxHeight;
    ...
    if (mayBeVolatileReference && !canMoveIfVolatile) return false;
    ...
    if (node->getOpCodeValue() == TR::treetop ||
        node->getOpCode().isNullCheck() || node->getOpCode().isResolveCheck() ||
    ...
    if (node->getOpCode().isStore()) {
      ...
      //
      if (symbolReferencesInNode.ValueAt(
...
static void removeGlRegDep(TR::Node *parent,
                           TR_GlobalRegisterNumber registerNum,
                           TR::Block *containingBlock, TR::Optimization *opt) {
  if (parent->getNumChildren() == 0) return;

  TR_ASSERT(parent->getNumChildren() > 0, "expected TR::GlRegDeps %p", parent);
  TR::Node *predGlRegDeps = parent->getLastChild();

  if (predGlRegDeps->getOpCodeValue() !=
      TR::GlRegDeps)  // could be already removed
    return;

  TR_ASSERT(predGlRegDeps->getOpCodeValue() == TR::GlRegDeps,
            "expected TR::GlRegDeps");
  ...
  if (predGlRegDeps->getNumChildren() == 0) parent->removeLastChild();
}

// Note: the future use counts are incremented with visit counts but are
// decremented here without using visit counts so they cannot be trusted for any
// functional purpose but only to guide heuristics.
...
TR::Optimization *TR::DeadTreesElimination::create(
...
TR::DeadTreesElimination::DeadTreesElimination(TR::OptimizationManager *manager)
    : TR::Optimization(manager), _targetTrees(manager->trMemory()) {
...
typedef TR::typed_allocator<ReadBarToTreeTopMapEntry, TR::Region &>
    ReadBarToTreeTopMapAlloc;
typedef std::map<ncount_t, TR::TreeTop *, std::less<ncount_t>,
                 ReadBarToTreeTopMapAlloc>
    ReadBarToTreeTopMap;

static void findReadBarInSubTree(TR::Node *node,
                                 TR::NodeChecklist &visitedNodesInCurrentTree,
                                 TR::list<TR::Node *> &rdbarsInCurrentSubTree) {
  if (visitedNodesInCurrentTree.contains(node)) return;
  visitedNodesInCurrentTree.add(node);
  if (node->getOpCode().isReadBar()) rdbarsInCurrentSubTree.push_back(node);
  for (int i = 0; i < node->getNumChildren(); i++)
    findReadBarInSubTree(node->getChild(i), visitedNodesInCurrentTree,
                         rdbarsInCurrentSubTree);
}
...
void TR::DeadTreesElimination::prePerformOnBlocks() {
  ...
  /*
   * Walk through all the blocks to remove trivial dead trees in the following
   * forms:
   *
   * case 1:
   * treetop
   ...
   * 1. Case 2 is seen very often because ilgen creates trees in the following
   * form: NULLCHK ardbari aload anchor
   *      => ardbari
   *    And the NULLCHK can be optimized away by optimizations like value
   * propagation and turned into a treetop.
   * 2. We do not remove the treetop node if there is any other tree whose
   * subtree references to that rdbar before the anchor node, like the
   * following: treetop ardbari SOMETREE (that's not anchor nor treetop)
   *      => ardbari
   *    anchor
   *      => ardbari
   *    Because those SOMETREE would need a treetop to anchor the rdbar node as
   * the first evaluation point.
   *
   */
  vcount_t visitCount = comp()->incOrResetVisitCount();
  ReadBarToTreeTopMap rdbar2ttMap(std::less<ncount_t>(),
  ...
  for (TR::TreeTop *tt = comp()->getStartTree(); tt != 0;
    ...
    if (node->getOpCodeValue() == TR::treetop &&
      ...
      removed = true;
               ...
               rdbar2ttMap.find(node->getFirstChild()->getGlobalIndex()) !=
                   rdbar2ttMap.end()) {
      TR::TreeTop *ttToRemove =
          rdbar2ttMap[node->getFirstChild()->getGlobalIndex()];
      if (performTransformation(comp(),
                                "%sRemove trivial dead tree (rdbar under "
                                "treetop before compressedrefs): %p\n",
      ...
      if (comp()->useCompressedPointers() && !node->getOpCode().isAnchor() &&
          !rdbar2ttMap.empty()) {
        TR::NodeChecklist visitedNodesInCurrentTree(comp());
        TR::list<TR::Node *> rdbarsInSubTree(
            getTypedAllocator<TR::Node *>(comp()->allocator()));
        findReadBarInSubTree(node, visitedNodesInCurrentTree, rdbarsInSubTree);
        for (auto it = rdbarsInSubTree.begin(); it != rdbarsInSubTree.end();
             it++) {
          TR::Node *rdbarNode = *it;
          if (rdbar2ttMap.find(rdbarNode->getGlobalIndex()) !=
              rdbar2ttMap.end())
    ...
    if (node->getVisitCount() >= visitCount) continue;

    if (comp()->useCompressedPointers() && !removed &&
        isReadBarrierUnderTreetop(node) &&
        node->getFirstChild()->getType() == TR::Address &&
        node->getFirstChild()->getOpCode().isLoadIndirect()) {
  ...
  // the BBStart and remove the corresponding dependency node from each of the
  ...
  while (1) {
    ...
    for (TR::Block *b = comp()->getStartBlock(); b; b = b->getNextBlock()) {
      ...
      if (startNode->getNumChildren() > 0 &&
          !debug("disableEliminationOfGlRegDeps")) {
        TR::Node *glRegDeps = startNode->getFirstChild();
        TR_ASSERT(glRegDeps->getOpCodeValue() == TR::GlRegDeps,
                  "expected TR::GlRegDeps");
        for (int32_t i = glRegDeps->getNumChildren() - 1; i >= 0; --i) {
...

          {
            ...
            TR_GlobalRegisterNumber registerNum =
                dep->getGlobalRegisterNumber();
            for (auto e = b->getPredecessors().begin();
                 e != b->getPredecessors().end(); ++e) {
              ...
              if (parent->getOpCode().isJumpWithMultipleTargets() &&
                ...
                for (int32_t j = parent->getCaseIndexUpperBound() - 1; j > 0;
                     --j) {
                  ...
                  if (caseNode->getBranchDestination() == startTT &&
                      ...
                      0)  // can't do this now that all glRegDeps are hung off
                          // the default branch
        ...
        if (glRegDeps->getNumChildren() == 0) startNode->removeChild(0);
      }
    }

    if (!glRegDepRemoved) break;
  }
}
...
/** \brief
 *       Tells whether it is possible to remove a tree node without considering
 * actual side effect. Only the child's reference count is taken into
 ...
 *
 *  \note
 *       In general, anchoring nodes with its child's reference count == 1 might
 * be removed, like compressedrefs, reg store, rdbar under a treetop. Any
 * TR::treetop node (except rdbar under a treetop) can be considered for
 * removing no matter what the child's reference count is.
 */
static bool treeCanPossiblyBeRemoved(TR::Node *node) {
  // If the tree node is not TR::treetop, it can only be removed if it's
  // anchoring node and no other uses exist
  if (node->getOpCodeValue() != TR::treetop) {
    return (node->getOpCode().isAnchor() &&
            node->getFirstChild()->getReferenceCount() == 1) ||
  ...
  return (!isReadBarrierUnderTreetop(node) ||
          node->getFirstChild()->getReferenceCount() == 1);
}

int32_t TR::DeadTreesElimination::process(TR::TreeTop *startTree,
                                          TR::TreeTop *endTree) {
  ...
  for (TR::TreeTopIterator iter(startTree, comp()); iter != endTree; ++iter) {
    ...
    if (node->getOpCodeValue() == TR::BBStart) {
      block = node->getBlock();
      if (!block->isExtensionOfPreviousBlock()) longestPaths.clear();
    }
    ...
    if (!treeCanPossiblyBeRemoved(node) &&
       ...
       * Given the following trees, the anchoring node can still be removed  if
       * the first treetop is removed
       *
       * treetop
       *    xloadi #x
       * anchor
    ...
    if (node->getOpCode().isStoreReg()) _delayedRegStores = true;
    ...
    if (visitCount == child->getVisitCount()) {
      ...
      if (callWithNoSideEffects) {
        treeTopCanBeEliminated = true;
                   ...
                   childOpCode.isStore() ||
                   ((opCodeValue == TR::New || opCodeValue == TR::anewarray ||
                   ...
                   ((childOpCode.isDiv() || childOpCode.isRem()) &&
        ...
        if (child->getReferenceCount() == 1) {
          ...
          if (opCodeValue == TR::loadaddr) treeTopCanBeEliminated = true;
        } else if (!_cannotBeEliminated) {
          safeToReplaceNode = isSafeToReplaceNode(
              child, iter.currentTree(), &seenConditionalBranch, visitCount,
              comp(), &_targetTrees, _cannotBeEliminated, longestPaths);
        }
      ...
      if (treeTopCanBeEliminated) {
        ...
        for (int32_t i = 0; i < child->getNumChildren(); ++i) {
          ...
          if (fixUpTree(child->getChild(i), iter.currentTree(), visited,
                        highGlobalIndex, self(), visitCount))
            containsFloatingPoint = true;
          if (highGlobalIndex) {
    ...
    if (treeTopCanBeEliminated) {
      ...
      if (!node->getOpCode().isStoreReg() ||
        ...
        if (performTransformation(
                comp(),
                "%sRemove tree : [" POINTER_PRINTF_FORMAT
                "] ([" POINTER_PRINTF_FORMAT "] = %s)\n",
                optDetailString(), node, node->getFirstChild(),
        ...
        if (performTransformation(
                comp(),
                "%sMove tree : [" POINTER_PRINTF_FORMAT
                "]([" POINTER_PRINTF_FORMAT "] = %s) to end of block\n",
                optDetailString(), node, node->getFirstChild(),
                node->getFirstChild()->getOpCode().getName())) {
          prevTree->join(nextTree);
          ...
          if (lastNode->getOpCode().isIf() &&
              !lastNode->getOpCode().isCompBranchOnly() &&
  ...
  for (auto it = anchors.begin(); it != anchors.end(); ++it) {
    ...
    if (load->getReferenceCount() > 1) continue;
    ...
    if (!heapBase->getOpCode().isLoadConst()) {
      it->tree->insertAfter(TR::TreeTop::create(
          comp(), TR::Node::create(heapBase, TR::treetop, 1, heapBase)));
    }
...
TR::TreeTop *TR::DeadTreesElimination::findLastTreetop(TR::Block *block,
                                                       TR::TreeTop *prevTree) {
  return block->getLastRealTreeTop();
}

const char *TR::DeadTreesElimination::optDetailString() const throw() {
