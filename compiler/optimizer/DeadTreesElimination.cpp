...
static OMR::TreeInfo*
findOrCreateTreeInfo(TR::TreeTop* treeTop,
          ...
          vcount_t evaluatedVisitCount)
{
  ...
  if ((node->getReferenceCount() > 1) && !node->getOpCode().isLoadConst() &&
    ...
    TR::TreeTop* anchorTreeTop =
      TR::TreeTop::create(opt->comp(), TR::Node::create(TR::treetop, 1, node));
    ...
    for (int32_t i = 0; i < node->getNumChildren(); ++i) {
      ...
      if (fixUpTree(
            child, treeTop, visited, highGlobalIndex, opt, evaluatedVisitCount))
        containsFloatingPoint = true;
    }
  }
  return containsFloatingPoint;
...
static inline bool
isReadBarrierUnderTreetop(TR::Node* node)
{
...
bool
collectSymbolReferencesInNode(TR::Node* node,
                              ...
                              bool* cantMoveUnderBranch)
{
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; i--) {
    ...
    collectSymbolReferencesInNode(child,
                                  symbolReferencesInNode,
                                  numDeadSubNodes,
                                  visitCount,
                                  comp,
                                  seenInternalPointer,
...
static int32_t
getLongestPathOfDAG(TR::Node* node, LongestPathMap& memo)
{
...
static bool
containsNode(TR::Node* containerNode,
             TR::Node* node,
             vcount_t visitCount,
             TR::Compilation* comp,
             int32_t* height,
             ...
             bool& canMoveIfVolatile)
{
  ...
  for (int32_t i = 0; i < containerNode->getNumChildren(); ++i) {
    if (containsNode(containerNode->getChild(i),
                     node,
                     visitCount,
                     comp,
                     height,
...
static bool
isSafeToReplaceNode(TR::Node* currentNode,
                    ...
                    LongestPathMap& longestPaths)
{
  ...
  collectSymbolReferencesInNode(currentNode,
                                symbolReferencesInNode,
                                &numDeadSubNodes,
                                visitCount,
                                comp,
...
static scount_t
recursivelyDecFutureUseCount(TR::Node* node)
{
...
int32_t
TR::DeadTreesElimination::perform()
{
...
int32_t
TR::DeadTreesElimination::performOnBlock(TR::Block* block)
{
  if (block->getEntry())
...
static void
findReadBarInSubTree(TR::Node* node,
                     ...
                     TR::list<TR::Node*>& rdbarsInCurrentSubTree)
{
  ...
  for (int i = 0; i < node->getNumChildren(); i++)
    findReadBarInSubTree(
      node->getChild(i), visitedNodesInCurrentTree, rdbarsInCurrentSubTree);
}

void
TR::DeadTreesElimination::prePerformOnBlocks()
{
  _cannotBeEliminated = false;
  _delayedRegStores = false;
    ...
    if (node->getOpCodeValue() == TR::treetop &&
      ...
      if (node->getOpCode().isCheck() &&
          ...
          performTransformation(
            comp(),
            "%sRemove dead check of side-effect free call: %p\n",
            optDetailString(),
            node)) {
        TR::TransformUtil::removeTree(comp(), tt);
        removed = true;
 ...
 */
static bool
treeCanPossiblyBeRemoved(TR::Node* node)
{
...
int32_t
TR::DeadTreesElimination::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    if (visitCount == child->getVisitCount()) {
      ...
      if (callWithNoSideEffects) {
        ...
        if (child->getReferenceCount() == 1) {
          ...
          safeToReplaceNode = isSafeToReplaceNode(child,
                                                  iter.currentTree(),
...
TR::TreeTop*
TR::DeadTreesElimination::findLastTreetop(TR::Block* block,
                                          TR::TreeTop* prevTree)
{
