...
static bool fixUpTree(TR::Node* node, TR::TreeTop* treeTop, TR::NodeChecklist& visited, bool& highGlobalIndex,
  ...
  bool* cantMoveUnderBranch)
{
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; i--) {
    ...
    collectSymbolReferencesInNode(child, symbolReferencesInNode, numDeadSubNodes, visitCount, comp, seenInternalPointer,
      seenArraylet, cantMoveUnderBranch);
  }
...
static bool containsNode(TR::Node* containerNode, TR::Node* node, vcount_t visitCount, TR::Compilation* comp,
  ...
  LongestPathMap& longestPaths)
{
...

  {
    ...
    if (node->getOpCodeValue() == TR::treetop || node->getOpCode().isNullCheck() || node->getOpCode().isResolveCheck()
      || node->getOpCodeValue() == TR::ArrayStoreCHK || node->getOpCode().isSpineCheck()) {
      node = node->getFirstChild();
    }
...

          {
            ...
            for (auto e = b->getPredecessors().begin(); e != b->getPredecessors().end(); ++e) {
              ...
              if (parent->getOpCode().isJumpWithMultipleTargets() && parent->getOpCode().hasBranchChildren()) {
                ...
                && !(parent->getOpCodeValue() == TR::treetop && parent->getFirstChild()->getOpCode().isCall()
    ...
    if (visitCount == child->getVisitCount()) {
      ...
      if (callWithNoSideEffects) {
        treeTopCanBeEliminated = true;
                   ...
                   || opCodeValue == TR::multianewarray || opCodeValue == TR::MergeNew || opCodeValue == TR::checkcast
        ...
        if (child->getReferenceCount() == 1) {
          ...
          safeToReplaceNode = isSafeToReplaceNode(child, iter.currentTree(), &seenConditionalBranch, visitCount, comp(),
            &_targetTrees, _cannotBeEliminated, longestPaths);
        }
        ...
        if (safeToReplaceNode) {
          if (childOpCode.hasSymbolReference()) {
            ...
            if (symRef->getSymbol()->isAuto() || symRef->getSymbol()->isParm())
              ...
              if (childOpCode.isLoad() || (opCodeValue == TR::loadaddr) || (opCodeValue == TR:: instanceof)
                || (((opCodeValue == TR::New) || (opCodeValue == TR::anewarray || opCodeValue == TR::newarray)) &&
    ...
    if (treeTopCanBeEliminated) {
      ...
      if (!node->getOpCode().isStoreReg() || (node->getFirstChild()->getReferenceCount() == 1)) {
        ...
        if (performTransformation(comp(),
              ...
              node, node->getFirstChild(), node->getFirstChild()->getOpCode().getName())) {
          ...
          if (nextTree->getNode()->getOpCodeValue() == TR::Goto && prevTree->getNode()->getOpCodeValue() == TR::BBStart
            && !prevTree->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
            requestOpt(OMR::redundantGotoElimination, prevTree->getNode()->getBlock());
          }
        }
      } else {
        if (performTransformation(comp(),
