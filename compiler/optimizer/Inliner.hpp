...
#define MIN_PROFILED_CALL_FREQUENCY \
  (.65f) // lowered this from .80f since opportunities were being missed in WAS; in those cases getting rid of the call
         // even in 65% of the cases was beneficial probably due to the improved icache impact
...
protected:
  virtual bool supportsMultipleTargetInlining()
  {
    return false;
  }
...
public:
  ...
  void setPolicy(OMR_InlinerPolicy* policy)
  {
    _policy = policy;
  }
  void setUtil(OMR_InlinerUtil* util)
  {
    _util = util;
  }
  ...
  bool allowBiggerMethods()
  {
    return comp()->isServerInlining();
  }
  ...
  int32_t getCurrentNumberOfNodes()
  {
    return _currentNumberOfNodes;
  }
  int32_t getMaxInliningCallSites()
  {
    return _maxInliningCallSites;
  }
  int32_t getNumAsyncChecks()
  {
    return _numAsyncChecks;
  }
  int32_t getNodeCountThreshold()
  {
    return _nodeCountThreshold;
  }
  int32_t getMaxRecursiveCallByteCodeSizeEstimate()
  {
    return _maxRecursiveCallByteCodeSizeEstimate;
  }
  void incCurrentNumberOfNodes(int32_t i)
  {
    _currentNumberOfNodes += i;
  }

  bool inlineVirtuals()
  {
    return _flags.testAny(InlineVirtuals);
  }
  void setInlineVirtuals(bool b)
  {
    _flags.set(InlineVirtuals, b);
  }
  bool inlineSynchronized()
  {
    return _flags.testAny(InlineSynchronized);
  }
  void setInlineSynchronized(bool b)
  {
    _flags.set(InlineSynchronized, b);
  }
  bool firstPass()
  {
    return _flags.testAny(FirstPass);
  }
  void setFirstPass(bool b = true)
  {
    _flags.set(FirstPass, b);
  }
  uint32_t getSizeThreshold()
  {
    return _methodByteCodeSizeThreshold;
  }
...
protected:
  ...
  void rematerializeCallArguments(TR_TransformInlinedFunction& tif, TR_VirtualGuardSelection* guard, TR::Node* callNode,
  ...
  TR::Node* createVirtualGuard(
    TR::Node*, TR::ResolvedMethodSymbol*, TR::TreeTop*, int16_t, TR_OpaqueClassBlock*, bool, TR_VirtualGuardSelection*);
...
public:
  ...
  TR::TreeTop* firstTempTreeTop()
  {
    return _firstTempTreeTop;
  }
  TR::TreeTop* lastTempTreeTop()
  {
    return _lastTempTreeTop;
  }
  TR_ParameterMapping* getFirstMapping()
  {
    return _mappings.getFirst();
  }
...
protected:
  TR_InlinerBase* _inliner;
  void setInliner(TR_InlinerBase* inliner)
  {
    _inliner = inliner;
  }
};
...
public:
   ...
   * information of the next call, which is usually the receiver or the method of the next call. Knowing the receiver of
   * the first call means we can devirtualize all the calls along the chain.
   *
   *    Each method except the last one in the chain usually contains simple code that manipulates the arguments for the
   * next call. The last method in the chain is usually the most important one, it does the real job and can contain
   * arbitrary code.
   *
   *    The goal of targeted inlining is to inline along the call chain to get the most important method inlined, which
   * is the last one in the chain. It does not care about other callsites.
   */
...
protected:
  ...
  virtual TR_TransformInlinedFunction* getTransformInlinedFunction(TR::ResolvedMethodSymbol*, TR::ResolvedMethodSymbol*,
    TR::Block*, TR::TreeTop*, TR::Node*, TR_ParameterToArgumentMapper&, TR_VirtualGuardSelection*,
...
public:
  ...
  bool aggressiveSmallAppOpts()
  {
    return comp()->getOption(TR_AggressiveOpts);
  }
...
public:
  ...
  TR::TreeTop* firstBBEnd()
  {
    return _firstBBEnd;
  }
  TR::TreeTop* lastBBStart()
  {
    return _lastBBStart;
  }
  TR::TreeTop* lastMainLineTreeTop()
  {
    return _lastMainLineTreeTop;
  }
  TR::Block* firstCatchBlock()
  {
    return _firstCatchBlock;
  }
  bool crossedBasicBlock()
  {
    return _crossedBasicBlock;
  }
  TR::TreeTop* simpleCallReferenceTreeTop()
  {
    return _simpleCallReferenceTreeTop;
  }
  TR::SymbolReference* resultTempSymRef()
  {
    return _resultTempSymRef;
  }
  List<TR::TreeTop>& treeTopsToRemove()
  {
    return _treeTopsToRemove;
  }
  ...
  {
    return _callNodeTreeTop;
  }
  TR::Node* getCallNode()
  {
    return _callNode;
  }
  void setResultNode(TR::Node* n)
  {
    _resultNode = n;
  }
  bool favourVftCompare()
  {
    return _favourVftCompare;
  }

  TR::ResolvedMethodSymbol* getCallerSymbol()
  {
    return _callerSymbol;
  }
