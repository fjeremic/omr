...
#define MIN_PROFILED_CALL_FREQUENCY                                            \
  (.65f) // lowered this from .80f since opportunities were being missed in WAS;
         // in those cases getting rid of the call even in 65% of the cases was
         // beneficial probably due to the improved icache impact
...
public:
  TR_InlinerTracer(TR::Compilation *comp, TR_FrontEnd *fe,
                   TR::Optimization *opt);
  ...
  bool partialLevel() {
    return comp()->getOption(TR_TracePartialInlining);
  } // the > ensures partial tracing gets turned on for debug

  // trace statements for specific tracing levels
  void partialTraceM(
      const char *fmt,
      ...); // trace statement for investigating partial ininling heuristic
  ...
  const char *getFailureReasonString(TR_InlinerFailureReason reason) {
    return TR_InlinerFailureReasonStr[reason];
  }
...
public:
  ...
  TR_InnerPreexistenceInfo(TR::Compilation *c,
                           TR::ResolvedMethodSymbol *methodSymbol,
                           TR_CallStack *callStack, TR::TreeTop *callTree,
                           TR::Node *callNode, TR_VirtualGuardKind _guardKind);
  ...
  virtual bool perform(TR::Compilation *comp, TR::Node *guardNode,
...
protected:
  ...
  TR_CallStack
      *_callStack; // the stack of callers (not including _methodSymbol)
...
struct TR_VirtualGuardSelection {
  ...
  TR_VirtualGuardSelection(TR_VirtualGuardKind kind,
                           TR_VirtualGuardTestType type = TR_NonoverriddenTest,
                           TR_OpaqueClassBlock *thisClass = 0)
      : _kind(kind), _type(type), _thisClass(thisClass),
...
public:
  ...
  bool isEDODisableInlinedProfilingInfo() {
    return _EDODisableInlinedProfilingInfo;
  }
  ...
  int32_t getMaxRecursiveCallByteCodeSizeEstimate() {
    return _maxRecursiveCallByteCodeSizeEstimate;
  }
  ...
  void createParmMap(TR::ResolvedMethodSymbol *calleeSymbol,
  ...
  void setStoreToCachedPrivateStatic(TR::Node *node) {
    _storeToCachedPrivateStatic = node;
  }
  TR::Node *getStoreToCachedPrivateStatic() {
    return _storeToCachedPrivateStatic;
  }
  bool alwaysWorthInlining(TR_ResolvedMethod *calleeMethod, TR::Node *callNode);

  void getSymbolAndFindInlineTargets(TR_CallStack *, TR_CallSite *,
                                     bool findNewTargets = true);
...
protected:
  ...
  TR_PrexArgInfo *
  buildPrexArgInfoForOutermostMethod(TR::ResolvedMethodSymbol *methodSymbol);
  bool inlineCallTarget(TR_CallStack *, TR_CallTarget *calltarget,
                        bool inlinefromgraph, TR_PrexArgInfo *argInfo = 0,
                        TR::TreeTop **cursorTreeTop = NULL);
  bool inlineCallTarget2(TR_CallStack *, TR_CallTarget *calltarget,
                         TR::TreeTop **cursorTreeTop, bool inlinefromgraph,
                         int32_t);

  // inlineCallTarget2 methods
  bool tryToInlineTrivialMethod(TR_CallStack *callStack,
                                TR_CallTarget *calltarget);

  bool tryToGenerateILForMethod(TR::ResolvedMethodSymbol *calleeSymbol,
                                TR::ResolvedMethodSymbol *callerSymbol,
                                TR_CallTarget *calltarget);

  void inlineFromGraph(TR_CallStack *, TR_CallTarget *calltarget,
                       TR_InnerPreexistenceInfo *);
  TR_CallSite *findAndUpdateCallSiteInGraph(TR_CallStack *callStack,
  ...
  int checkInlineableWithoutInitialCalleeSymbol(TR_CallSite *callsite,
                                                TR::Compilation *comp);

  void getBorderFrequencies(int32_t &hotBorderFrequency,
                            int32_t &coldBorderFrequency,
                            TR_ResolvedMethod *calleeResolvedMethod,
                            TR::Node *callNode);
  int32_t scaleSizeBasedOnBlockFrequency(
      int32_t bytecodeSize, int32_t frequency, int32_t borderFrequency,
      TR_ResolvedMethod *calleeResolvedMethod, TR::Node *callNode,
      int32_t coldBorderFrequency =
          0); // not virtual because we want base class always calling base
              // version of this
  virtual bool
  exceedsSizeThreshold(TR_CallSite *callSite, int bytecodeSize,
                       TR::Block *callNodeBlock, TR_ByteCodeInfo &bcInfo,
                       int32_t numLocals = 0, TR_ResolvedMethod *caller = 0,
                       TR_ResolvedMethod *calleeResolvedMethod = 0,
                       TR::Node *callNode = 0, bool allConsts = false);
  virtual bool inlineCallTargets(TR::ResolvedMethodSymbol *, TR_CallStack *,
                                 TR_InnerPreexistenceInfo *info) {
    TR_ASSERT(0, "invalid call to TR_InlinerBase::inlineCallTargets");
    return false;
  }
  TR::TreeTop *addGuardForVirtual(TR::ResolvedMethodSymbol *,
                                  TR::ResolvedMethodSymbol *, TR::TreeTop *,
                                  TR::Node *, TR_OpaqueClassBlock *,
                                  TR::TreeTop *, TR::TreeTop *, TR::TreeTop *,
                                  TR_TransformInlinedFunction &,
                                  List<TR::SymbolReference> &,
                                  TR_VirtualGuardSelection *, TR::TreeTop **,
                                  TR_CallTarget *calltarget = 0);

  void addAdditionalGuard(TR::Node *callNode,
                          TR::ResolvedMethodSymbol *calleeSymbol,
                          TR_OpaqueClassBlock *thisClass, TR::Block *prevBlock,
                          TR::Block *inlinedBody, TR::Block *slowPath,
                          TR_VirtualGuardKind kind,
                          TR_VirtualGuardTestType type, bool favourVftCompare,
                          TR::CFG *callerCFG);
  void rematerializeCallArguments(TR_TransformInlinedFunction &tif,
                                  TR_VirtualGuardSelection *guard,
                                  TR::Node *callNode, TR::Block *block1,
                                  TR::TreeTop *rematPoint);
  void replaceCallNode(TR::ResolvedMethodSymbol *, TR::Node *, rcount_t,
                       TR::TreeTop *, TR::Node *, TR::Node *);
  void linkOSRCodeBlocks();
  bool IsOSRFramesSizeUnderThreshold();
  bool heuristicForUsingOSR(TR::Node *, TR::ResolvedMethodSymbol *,
                            TR::ResolvedMethodSymbol *, bool);

  TR::Node *createVirtualGuard(TR::Node *, TR::ResolvedMethodSymbol *,
                               TR::TreeTop *, int16_t, TR_OpaqueClassBlock *,
                               bool, TR_VirtualGuardSelection *);

private:
  void replaceCallNodeReferences(TR::Node *, TR::Node *, uint32_t, TR::Node *,
                                 TR::Node *, rcount_t &,
...
public:
  TR_DumbInliner(TR::Optimizer *, TR::Optimization *, uint32_t initialSize,
                 uint32_t dumbReductionIncrement = 5);
  virtual bool inlineCallTargets(TR::ResolvedMethodSymbol *, TR_CallStack *,
                                 TR_InnerPreexistenceInfo *);
  bool tryToInline(char *message, TR_CallTarget *calltarget);

protected:
  virtual bool analyzeCallSite(TR_CallStack *, TR::TreeTop *, TR::Node *,
                               TR::Node *);
...
public:
  ...
  virtual bool inlineRecognizedMethod(TR::RecognizedMethod method);
  bool inlineCall(TR::TreeTop *callNodeTreeTop,
                  TR_OpaqueClassBlock *thisType = 0,
                  bool recursiveInlining = false, TR_PrexArgInfo *argInfo = 0,
                  int32_t initialMaxSize = 0);
};

struct TR_ParameterMapping : TR_Link<TR_ParameterMapping> {
  TR_ParameterMapping(TR::ParameterSymbol *ps)
      : _parmSymbol(ps), _replacementSymRef(NULL),
        _replacementSymRefForInlinedBody(NULL), _parameterNode(NULL),
   ...
   *
   * Used for replacing both the parm symbol references in the inlined body and
   * args on the taken side of the guard.
   */
  TR::SymbolReference *_replacementSymRef;
  /*
   * Used for replacing parm symbol reference in the inlined body when this
   * field not NULL. Otherwise, the \ref _replacementSymRef is used.
   */
  TR::SymbolReference *_replacementSymRefForInlinedBody;

  TR::Node *_parameterNode; // The Node under the call which matches the
...
public:
  ...
  TR_ParameterToArgumentMapper(TR::ResolvedMethodSymbol *,
                               TR::ResolvedMethodSymbol *, TR::Node *,
                               TR_PrexArgInfo *, List<TR::SymbolReference> &,
                               List<TR::SymbolReference> &,
...
public:
  ...
  static TR::TreeTop *storeValueInATemp(
      TR::Compilation *comp, TR::Node *, TR::SymbolReference *&, TR::TreeTop *,
      TR::ResolvedMethodSymbol *, List<TR::SymbolReference> &tempList,
      List<TR::SymbolReference> &availableTemps,
      List<TR::SymbolReference> *moreTemps, bool behavesLikeTemp = true,
      TR::TreeTop **newStoreValueATreeTop = NULL, bool isIndirect = false,
      int32_t offset = 0);
  virtual bool addTargetIfMethodIsNotOverridenInReceiversHierarchy(
      TR_IndirectCallSite *callsite);
  virtual TR_ResolvedMethod *
  findSingleJittedImplementer(TR_IndirectCallSite *callsite);
  virtual bool
  addTargetIfThereIsSingleImplementer(TR_IndirectCallSite *callsite);
  virtual TR_PrexArgInfo *
  createPrexArgInfoForCallTarget(TR_VirtualGuardSelection *guard,
                                 TR_ResolvedMethod *implementer);
  virtual TR_InnerPreexistenceInfo *
  createInnerPrexInfo(TR::Compilation *c,
                      TR::ResolvedMethodSymbol *methodSymbol,
                      TR_CallStack *callStack, TR::TreeTop *callTree,
                      TR::Node *callNode, TR_VirtualGuardKind guardKind);
  virtual TR_InlinerTracer *getInlinerTracer(TR::Optimization *optimization);
  virtual void adjustByteCodeSize(TR_ResolvedMethod *calleeResolvedMethod,
                                  bool isInLoop, TR::Block *block,
                                  int &bytecodeSize) {
    return;
  }
  virtual void adjustCallerWeightLimit(TR::ResolvedMethodSymbol *callSymbol,
                                       int &callerWeightLimit) {
    return;
  }
  virtual void
  adjustMethodByteCodeSizeThreshold(TR::ResolvedMethodSymbol *callSymbol,
  ...
  /*
   * Implemented by down stream projects to request for certain optimizations
   * based on \parm target
   *
   * \parm calltarget
  ...
  /**
   * \brief
   *    Check if another pass of targeted inlining is needed if the given method
   * is inlined.
   *
   * \parm callee
   ...
   *
   * \note
   *    Targeted inlining is meant to deal with method call chains where the
   * receiver of current call stores the information of the next call, which is
   * usually the receiver or the method of the next call. Knowing the receiver
   * of the first call means we can devirtualize all the calls along the chain.
   *
   *    Each method except the last one in the chain usually contains simple
   * code that manipulates the arguments for the next call. The last method in
   * the chain is usually the most important one, it does the real job and can
   * contain arbitrary code.
   *
   *    The goal of targeted inlining is to inline along the call chain to get
   * the most important method inlined, which is the last one in the chain. It
   * does not care about other callsites.
   */
...
protected:
  ...
  virtual void
  computeMethodBranchProfileInfo(TR::Block *cfgBlock, TR_CallTarget *calltarget,
                                 TR::ResolvedMethodSymbol *callerSymbol);
  ...
  virtual void calleeTreeTopPreMergeActions(
      ...
      TR_CallTarget *calltarget);
  virtual void refineInlineGuard(TR::Node *callNode, TR::Block *&block1,
                                 TR::Block *&block2, bool &appendTestToBlock1,
                                 TR::ResolvedMethodSymbol *callerSymbol,
                                 TR::TreeTop *cursorTree,
                                 TR::TreeTop *&virtualGuard, TR::Block *block4);
  virtual void
  refineInliningThresholds(TR::Compilation *comp, int32_t &callerWeightLimit,
                           int32_t &maxRecursiveCallByteCodeSizeEstimate,
                           int32_t &methodByteCodeSizeThreshold,
                           int32_t &methodInWarmBlockByteCodeSizeThreshold,
                           int32_t &methodInColdBlockByteCodeSizeThreshold,
                           int32_t &nodeCountThreshold, int32_t size) {}
  virtual void estimateAndRefineBytecodeSize(TR_CallSite *callsite,
                                             TR_CallTarget *target,
                                             TR_CallStack *callStack,
                                             int32_t &bytecodeSize);
  virtual TR_TransformInlinedFunction *getTransformInlinedFunction(
      TR::ResolvedMethodSymbol *, TR::ResolvedMethodSymbol *, TR::Block *,
      TR::TreeTop *, TR::Node *, TR_ParameterToArgumentMapper &,
      TR_VirtualGuardSelection *, List<TR::SymbolReference> &,
      List<TR::SymbolReference> &, List<TR::SymbolReference> &);
};

class OMR_InlinerPolicy : public TR::OptimizationPolicy,
...
public:
  ...
  virtual bool inlineRecognizedMethod(TR::RecognizedMethod method);
  int32_t getInitialBytecodeSize(TR::ResolvedMethodSymbol *methodSymbol,
                                 TR::Compilation *comp);
  virtual int32_t getInitialBytecodeSize(TR_ResolvedMethod *feMethod,
                                         TR::ResolvedMethodSymbol *methodSymbol,
                                         TR::Compilation *comp);
  ...
  /** \brief
   *     Determines whether to skip generation of HCR guards for a particular
   * callee during inlining.
   *
   *  \param callee
   *     The callee to be inlined.
   *
   *  \return
   *     true if we can skip generating HCR guards for this \p callee; false
   * otherwise.
   */
  ...
  virtual bool dontPrivatizeArgumentsForRecognizedMethod(
      TR::RecognizedMethod recognizedMethod);
  virtual bool
  replaceSoftwareCheckWithHardwareCheck(TR_ResolvedMethod *calleeMethod);

  /**
   * \brief Answers whether the provided ResolvedMethodSymbol is a native method
   * that can be inlined
   *
   ...
   */
  virtual bool
  isInlineableNativeMethod(TR::Compilation *comp,
                           TR::ResolvedMethodSymbol *methodSymbol) {
    return false;
  }

  /**
   * \brief sometimes we choose not to inline certain callsite because they will
   * be handled specially in other optimizations or in codegen
   *
   * \return true if the method shouldn't be inlined in inliner
   */
  virtual bool supressInliningRecognizedInitialCallee(TR_CallSite *callsite,
                                                      TR::Compilation *comp);

protected:
  virtual bool tryToInlineTrivialMethod(TR_CallStack *callStack,
                                        TR_CallTarget *calltarget);
  virtual bool alwaysWorthInlining(TR_ResolvedMethod *calleeMethod,
                                   TR::Node *callNode);
  virtual void
  ...
  bool mustBeInlinedEvenInDebug(TR_ResolvedMethod *calleeMethod,
                                TR::TreeTop *callNodeTreeTop);
  virtual TR_InlinerFailureReason
  checkIfTargetInlineable(TR_CallTarget *target, TR_CallSite *callsite,
                          TR::Compilation *comp);
  virtual bool suitableForRemat(TR::Compilation *comp, TR::Node *node,
                                TR_VirtualGuardSelection *guard);
};
...
public:
  TR_TransformInlinedFunction(
      TR::Compilation *, TR_InlinerTracer *tracer, TR::ResolvedMethodSymbol *,
      TR::ResolvedMethodSymbol *, TR::Block *, TR::TreeTop *, TR::Node *,
      TR_ParameterToArgumentMapper &, TR_VirtualGuardSelection *,
      List<TR::SymbolReference> &, List<TR::SymbolReference> &,
      List<TR::SymbolReference> &);
  ...
  TR::TreeTop *simpleCallReferenceTreeTop() {
    return _simpleCallReferenceTreeTop;
  }
  TR::SymbolReference *resultTempSymRef() { return _resultTempSymRef; }
  List<TR::TreeTop> &treeTopsToRemove() { return _treeTopsToRemove; }
  List<TR::Block> &blocksWithEdgesToTheEnd() {
    return _blocksWithEdgesToTheEnd;
  }
  ...
  TR_ParameterToArgumentMapper &getParameterMapper() {
    return _parameterMapper;
  }

  bool traceVIP() { return _traceVIP; }

#define MAX_FIND_SIMPLE_CALL_REFERENCE_DEPTH 12
  void setFindCallNodeRecursionDepthToMax() {
  ...
  TR::TreeTop *findSimpleCallReference(TR::TreeTop *callNodeTreeTop,
                                       TR::Node *callNode);
  bool findReturnValueInTree(TR::Node *rvStoreNode, TR::Node *node,
                             TR::Compilation *comp);
...
public:
  TR_HandleInjectedBasicBlock(TR::Compilation *, TR_InlinerTracer *,
...
private:
  ...
  void collectNodesWithMultipleReferences(TR::TreeTop *, TR::Node *,
                                          TR::Node *);
  void replaceNodesReferencedFromAbove(TR::Block *,
                                       TR::NodeChecklist &visitedNodes);
  void replaceNodesReferencedFromAbove(TR::TreeTop *, TR::Node *, TR::Node *,
                                       uint32_t,
                                       TR::NodeChecklist &visitedNodes);
  void createTemps(bool);

  struct MultiplyReferencedNode : TR_Link<MultiplyReferencedNode> {
    MultiplyReferencedNode(TR::Node *node, TR::TreeTop *tt,
