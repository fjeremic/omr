    ...
    TR::Compilation* comp, TR::Node* parent, TR::ILOpCodes value, TR::ILOpCodes leftChildValue, TR::ILOpCodes rightChildValue, const char* msg)
{
    ...
    if (child->getOpCodeValue() != value || child->getFirstChild()->getOpCodeValue() != leftChildValue || child->getSecondChild()->getOpCodeValue() != rightChildValue) {
        if (msg)
            dumpOptDetails(comp, msg);
        return NULL;
    } else {
...
{
    TR::Node* child = parent->getFirstChild();
    if (child->getOpCodeValue() != value || child->getFirstChild()->getOpCodeValue() != leftChildValue || child->getSecondChild()->getOpCodeValue() != TR::iconst || child->getSecondChild()->getInt() != rightChildValue) {
        if (msg)
            dumpOptDetails(comp, msg);
        return NULL;
    } else {
...
bool TR_Arraycopy::checkArrayStore(TR::Node* storeNode)
{
    ...
    if (storeFirstChild->getNumChildren() == 0 || loadFirstChild->getNumChildren() == 0 || !storeFirstChild->getFirstChild()->getOpCode().hasSymbolReference() || !loadFirstChild->getFirstChild()->getOpCode().hasSymbolReference() || storeFirstChild->getFirstChild()->getSymbol()->getRegisterMappedSymbol() == loadFirstChild->getFirstChild()->getSymbol()->getRegisterMappedSymbol()) {
...
bool TR_ByteToCharArraycopy::checkByteLoads(TR::Node* loadNodes)
{
    ...
    if (highByteNode->getOpCodeValue() != TR::imul && (highByteNode->getOpCodeValue() == TR::bu2i && lowByteNode->getOpCodeValue() == TR::imul)) {
...
bool TR_ArrayLoop::checkLoopCmp(TR::Node* loopCmpNode, TR::Node* indVarStoreNode, TR_InductionVariable* indVar)
{
    ...
    // on exit
    if (compareOp == TR::ificmpeq || compareOp == TR::ificmpge || compareOp == TR::ificmple || compareOp == TR::ifiucmpge || compareOp == TR::ifiucmple) {
        _addInc = true;
    }

    if (compareOp == TR::ificmplt || compareOp == TR::ificmple || compareOp == TR::ifiucmplt || compareOp == TR::ifiucmple) {
        _forwardLoop = true;
    }
...
void TR_ArrayLoop::findIndVarLoads(TR::Node* node, TR::Node* indVarStoreNode, bool& storeFound,
...
bool TR_Arraycmp::checkElementCompare(TR::Node* elemCmpNode)
{
    ...
    if (elemOp != TR::ificmpne && elemOp != TR::iffcmpne && elemOp != TR::iffcmpneu && elemOp != TR::ifdcmpne && elemOp != TR::ifdcmpneu && elemOp != TR::iflcmpne) {
...
bool TR_Arraycmp::checkGoto(TR::Block* gotoBlock, TR::Node* gotoNode, TR::Node* finalNode)
{
    ...
    TR::RegisterMappedSymbol* comparisonSym = (comparisonNode->getOpCode().hasSymbolReference()) ? comparisonNode->getSymbol()->getRegisterMappedSymbol() : NULL;
    TR::RegisterMappedSymbol* firstChildSym = targetNode->getFirstChild()->getSymbol()->getRegisterMappedSymbol();
    TR::RegisterMappedSymbol* secondChildSym = (opCodeSecondChild == TR::iload) ? targetNode->getSecondChild()->getSymbol()->getRegisterMappedSymbol() : NULL;
    ...
    } else if (!(firstChildSym == indSym && secondChildSym == comparisonSym) && !(firstChildSym == comparisonSym && secondChildSym == indSym) && !(firstChildSym == indSym && secondChildSym == NULL && comparisonNode->getInt() == targetNode->getSecondChild()->getInt())) {
...
TR_Arraytranslate::getMulChild(TR::Node* child)
{
    ...
    if ((child->getOpCodeValue() == TR::imul || child->getOpCodeValue() == TR::lmul) && (child->getSecondChild()->getOpCodeValue() == TR::iconst || child->getSecondChild()->getOpCodeValue() == TR::lconst)) {
...
static void swapIfNecessary(TR::Node*& aiaddFirstChild, TR::Node*& aiaddSecondChild)
{
    ...
    if (aiaddFirstChild->getOpCodeValue() == TR::l2i && (aiaddFirstChild->getFirstChild()->getOpCodeValue() == TR::lloadi || aiaddFirstChild->getFirstChild()->getOpCodeValue() == TR::lload)) {
...
bool TR_Arraytranslate::checkLoad(TR::Node* loadNode)
{
    ...
    //
    if (transLoadNode->getOpCodeValue() == TR::i2b || transLoadNode->getOpCodeValue() == TR::i2s || transLoadNode->getOpCodeValue() == TR::s2b) {
    ...
    if (aiaddNode->getOpCodeValue() != TR::aiadd && aiaddNode->getOpCodeValue() != TR::aladd && aiaddNode->getOpCodeValue() != TR::iadd && aiaddNode->getOpCodeValue() != TR::ladd) {
...
bool TR_Arraytranslate::checkStore(TR::Node* storeNode)
{
    ...
    if (hasBranch()) {
        ...
        if (shrinkNode->getOpCodeValue() != TR::i2s && shrinkNode->getOpCodeValue() != TR::i2b && shrinkNode->getOpCodeValue() != TR::cconst && shrinkNode->getOpCodeValue() != TR::bconst) {
...
bool TR_Arraytranslate::checkBreak(TR::Block* breakBlock, TR::Node* breakNode, TR::Block* nextBlock)
{
    ...
    if (loadNode->getSymbolReference() != _resultUnconvertedNode->getSymbolReference() && loadNode->getSymbolReference() != _resultNode->getFirstChild()->getSymbolReference() && loadNode->getSymbolReference() != _resultNode->getSymbolReference()) {
    ...
    TR::Block* branchBlock, TR::Block* incrementBlock)
{
    ...
    if (
        (arraycmpLoop.getFirstAddress()->getIncrement() * arraycmpLoop.getFirstAddress()->getMultiplier() < 0) || (arraycmpLoop.getSecondAddress()->getIncrement() * arraycmpLoop.getSecondAddress()->getMultiplier() < 0)) {
    ...
    TR::Block* firstBlock, TR::Block* secondBlock, TR::Block* thirdBlock, TR::Block* fourthBlock)
{
    ...
    if (
        ((firstNum == 3 || firstNum == 4 || firstNum == 5) && secondNum == 0) || ((firstNum == 3 || firstNum == 2) && secondNum == 3) || ((firstNum == 3 || firstNum == 2) && secondNum == 1 && thirdNum == 2 && fourthNum == 2)) {
    ...
    if (inputBase && outputBase) {
        ...
        if ((inputBase == outputBase) || (inputBase->getOpCode().hasSymbolReference() && outputBase->getOpCode().hasSymbolReference() && inputBase->getSymbolReference()->getSymbol() == outputBase->getSymbolReference()->getSymbol())) {
    ...
    // should match
    //
    if (!arraytranslateLoop.compilerGeneratedTable() && (arraytranslateLoop.getResultUnconvertedNode()->getDataType() != storeNode->getDataType())) {
   ...
   */
    ...
    if (hasBreak) {
        ...
        TR::Node* dupFinalNode = arraytranslateLoop.getFinalNode()->duplicateTree();
        TR::TreeTop* lengthTestTree = TR::TreeTop::create(comp(), TR::Node::createif(TR::ificmplt, lengthTestNode, dupFinalNode, loadElementBlock->getEntry()));
...
bool TR_ArraytranslateAndTest::checkFrequency(TR::CodeGenerator* cg, TR::Block* loadBlock, TR::Node* loadNode)
{
    ...
    if (((fallThroughFrequency <= 0) && ((loopHeaderFrequency > 0))) || (fallThroughFrequency < breakFrequency * cg->arrayTranslateAndTestMinimumNumberOfIterations())) {
        return false;
    }
    return true;
}
...
bool TR_LoopReducer::generateByteToCharArraycopy(TR_InductionVariable* byteIndVar, TR_InductionVariable* charIndVar, TR::Block* loopHeader)
{
    if (!comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
        dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
        return false;
...
bool TR_LoopReducer::generateCharToByteArraycopy(TR_InductionVariable* byteIndVar, TR_InductionVariable* charIndVar, TR::Block* loopHeader)
{
    if (!comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
        dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
        return false;
...
int TR_LoopReducer::addRegionBlocks(TR_RegionStructure* region, TR::Block** blockList, int numBlocks, const int maxNumBlocks)
{
    TR::CFGEdge* edge;
    TR::Block* b;
...
void TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure* whileLoop)
{
    ...
    if ((block2 && (loopHeader->getNextBlock() != block2)) || (block2 && block3 && (block2->getNextBlock() != block3)) || (block3 && block4 && (block3->getNextBlock() != block4))) {
    ...
    {
        if (((numBlocks == 1) && generateByteToCharArraycopy(indVar, nextIndVar, loopHeader)) || ((numBlocks == 1) && generateCharToByteArraycopy(indVar, nextIndVar, loopHeader))) {
        ...
        if (((numBlocks == 1) && generateArrayset(indVar, loopHeader)) || ((numBlocks == 1) && generateArraycopy(indVar, loopHeader)) || (((numBlocks == 2) /* || (numBlocks == 3 && block3->endsInGoto()) */) && generateArraycmp(whileLoop, indVar, loopHeader, block2)) || ((numBlocks == 1) && generateArraytranslate(whileLoop, indVar, loopHeader, NULL, NULL, NULL)) || (((numBlocks == 2) /* || (numBlocks == 3 && block3->endsInGoto()) */) && generateArraytranslate(whileLoop, indVar, loopHeader, block2, NULL, NULL)) || ((numBlocks == 3) && generateArraytranslate(whileLoop, indVar, loopHeader, block4, block3, NULL)) || ((numBlocks == 4) && generateArraytranslate(whileLoop, indVar, loopHeader, block4, block3, block2)) || ((numBlocks == 2) && generateArraytranslateAndTest(whileLoop, indVar, loopHeader, block2))) {
            removeSelfEdge(loopHeader->getSuccessors(), whileLoop->getNumber());
        } else {
...
TR_LoopReducer::perform()
{
    ...
    if (!comp()->cg()->getSupportsArraySet() && !comp()->cg()->getSupportsReferenceArrayCopy() && !comp()->cg()->getSupportsPrimitiveArrayCopy() && !comp()->cg()->getSupportsArrayCmp() && !comp()->cg()->getSupportsArrayTranslateTRxx() && !comp()->cg()->getSupportsArrayTranslateAndTest()) {
