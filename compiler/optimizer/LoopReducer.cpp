...
bool TR_LRAddressTree::checkAiadd(TR::Node* aiaddNode, int32_t elementSize)
{
  ...
  if (_multiplyNode.isNull() && ((elementSize != _increment) && (elementSize != -_increment))) {
    dumpOptDetails(comp(),
      "checkAiadd: sub-tree does not have induction variable change consistent with increment of multiplier (%d %d)\n",
...
{
  if (!storeNode->getOpCode().isStoreIndirect()
    && !(storeNode->getOpCodeValue() == TR::treetop && storeNode->getFirstChild()->getOpCodeValue() == TR::awrtbari)) {
...
bool TR_CharToByteArraycopy::checkArrayStores(TR::Node* origHighStoreNode, TR::Node* origLowStoreNode)
{
  ...
  if (lowStoreNode->getOpCodeValue() != TR::bstorei) {
    dumpOptDetails(
      comp(), "checkArrayStores: char to byte arraycopy low arraystore tree does not have an indirect store as root\n");
    return false;
  }
  ...
  TR::Node* ishr
    = testNode(comp(), origHighStoreNode->getSecondChild(), TR::i2b, "checkArrayStores: high store child is not i2b\n");
  if (!ishr)
    return false;
  ...
  TR::Node* c2i = testBinaryIConst(
    comp(), iand, TR::iand, TR::su2i, 0xFF00, "checkArrayStores: high store child is not iand of su2i and 0xFF00\n");
  if (!c2i)
    return false;
  TR::Node* icload
    = testUnary(comp(), c2i->getFirstChild(), TR::cloadi, "checkArrayStores: high store child is not icload\n");
  if (!icload)
    return false;
  ...
  iand
    = testNode(comp(), origLowStoreNode->getSecondChild(), TR::i2b, "checkArrayStores: low store child is not i2b\n");
  if (!iand)
    return false;
  ...
  if (!c2i)
    return false;
  TR::Node* icloadDup
    = testUnary(comp(), c2i->getFirstChild(), TR::cloadi, "checkArrayStores: low store child is not icload\n");
  if (!icloadDup)
    return false;
...
bool TR_ArrayLoop::checkLoopCmp(TR::Node* loopCmpNode, TR::Node* indVarStoreNode, TR_InductionVariable* indVar)
{
  ...
  // on exit
  if (compareOp == TR::ificmpeq || compareOp == TR::ificmpge || compareOp == TR::ificmple || compareOp == TR::ifiucmpge
...
TR::Node* TR_LRAddressTree::updateMultiply(TR_ParentOfChildNode* multiplyNode)
{
  ...
  if (!multiplyNode->isNull()) {
    ...
    if (multiplyNode->getParent()->getType().isInt32()) {
      TR::Node* initMulVal
        = TR::Node::create(multiplyNode->getParent(), TR::iconst, 0, (_increment > 0 ? _increment : -_increment));
...
bool TR_LoopReducer::generateArraycopy(TR_InductionVariable* indVar, TR::Block* loopHeader)
{
  ...
  if (arraycopyLoop.hasWriteBarrier() && needWriteBarrier && !comp()->cg()->getSupportsReferenceArrayCopy()) {
    dumpOptDetails(comp(),
      "arraycopy arraystore tree has write barrier as root and write barriers are enabled but no support for this "
  ...
  if (storeNode->getType().isBCD() && !(storeSize == 1 || storeSize == 2 || storeSize == 4 || storeSize == 8)) {
    dumpOptDetails(comp(),
      "arraycopy storeNode %p is a BCD type (%s) and the storeSize (%d) is not 1,2,4 or 8 so do not reduce arraycopy\n",
...
bool TR_LoopReducer::generateArrayset(TR_InductionVariable* indVar, TR::Block* loopHeader)
{
  ...
  if (!performTransformation(comp(),
        "%sReducing arrayset %d from storeNode [" POINTER_PRINTF_FORMAT "] and copyValueNode [" POINTER_PRINTF_FORMAT
        "]\n",
...
bool TR_Arraycmp::checkGoto(TR::Block* gotoBlock, TR::Node* gotoNode, TR::Node* finalNode)
{
  ...
  TR::RegisterMappedSymbol* secondChildSym
    = (opCodeSecondChild == TR::iload) ? targetNode->getSecondChild()->getSymbol()->getRegisterMappedSymbol() : NULL;
...
bool TR_Arraytranslate::checkLoad(TR::Node* loadNode)
{
  ...
  if (inLoadNode->getOpCodeValue() != TR::cloadi && inLoadNode->getOpCodeValue() != TR::bloadi) {
    dumpOptDetails(
      comp(), "...load tree does not have 2nd icload/ibload - check if compiler-generated table lookup match\n");
...
bool TR_Arraytranslate::checkStore(TR::Node* storeNode)
{
  ...
  if (hasBranch()) {
    ...
    if (shrinkNode->getOpCodeValue() != TR::i2s && shrinkNode->getOpCodeValue() != TR::i2b
      && shrinkNode->getOpCodeValue() != TR::cconst && shrinkNode->getOpCodeValue() != TR::bconst) {
    ...
    if (shrinkNode->getOpCodeValue() == TR::i2b || shrinkNode->getOpCodeValue() == TR::i2s) {
      ...
      if (iloadNode->getSymbolReference() != _resultNode->getSymbolReference()) {
        dumpOptDetails(
          comp(), "...store tree reference does not match load tree reference - no arraytranslate reduction\n");
        return false;
      }
    }
  } else {
...
bool TR_Arraytranslate::checkBreak(TR::Block* breakBlock, TR::Node* breakNode, TR::Block* nextBlock)
{
  ...
  if (valNode->getOpCodeValue() != TR::iconst
    ...
    dumpOptDetails(
      comp(), "...break tree does not have bconst/cconst/iconst, or not in range - no arraytranslate reduction\n");
    return false;
  }
...
bool TR_Arraytranslate::checkMatIndVarStore(TR::Node* matIndVarStoreNode, TR::Node* indVarStoreNode)
{
  if (!matIndVarStoreNode->getOpCode().isStoreDirect()) {
    dumpOptDetails(
      comp(), "materialized induction variable tree %p does not have a direct store as root\n", matIndVarStoreNode);
    return false;
  }
  ...
  if (storeFirstChild->getOpCodeValue() != TR::iadd && storeFirstChild->getOpCodeValue() != TR::isub) {
    dumpOptDetails(
      comp(), "first child %p of materialized induction variable store is not TR::iadd/TR::isub\n", storeFirstChild);
    return false;
  }
  ...
  TR::Block* firstBlock, TR::Block* secondBlock, TR::Block* thirdBlock, TR::Block* fourthBlock)
{
  ...
  */
  ...
  int32_t mask
    = cg()->arrayTranslateTableRequiresAlignment(arraytranslateLoop.getByteInput(), arraytranslateLoop.getByteOutput());
  if (mask && !arraytranslateLoop.compilerGeneratedTable()) // compiler-generated tables aligned at allocation
  {
    ...
    if (arraytranslateLoop.tableBackedByRawStorage()) {
      ...
      TR::Node* hdrSizeNode
        = TR::Node::create(loadNode, TR::iconst, 0, TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
  ...
  if (hasBreak) {
    ...
    if (ivLoadNode->getOpCode().isArrayRef()) {
      vcount_t visitCount = comp()->incVisitCount();
      TR::Node* decNode
        = TR::Node::create(TR::isub, 2, TR::Node::createLoad(loadNode, indVarSymRef), TR::Node::iconst(loadNode, 1));
  ...
  TR_RegionStructure* whileLoop, TR_InductionVariable* indVar, TR::Block* firstBlock, TR::Block* secondBlock)
{
  ...
  if (firstNum != 1 || (secondNum != 2 && secondNum != 3)) {
    dumpOptDetails(
      comp(), "Loop has wrong number of tree tops (%d,%d) - no arraytranslateAndTest reduction\n", firstNum, secondNum);
    return false;
  }
  ...
  if (cmpTargetBlockNumber != nextBlock->getNumber()) {
    TR::Node* compareNode
      = TR::Node::createif(TR::ificmpne, arraytranslateAndTestLoop.getStoreAddress()->getIndVarLoad()->duplicateTree(),
  ...
  TR_InductionVariable* byteIndVar, TR_InductionVariable* charIndVar, TR::Block* loopHeader)
{
  ...
  {
    dumpOptDetails(comp(),
      "Loop does not have an increment of +/-1 and +/-2, but instead %d and %d - no byte to char arraycopy reduction\n",
      incrA, incrB);
    return false;
  }
  ...
  if (!arraycopyLoop.checkByteLoads(storeNode->getSecondChild())) {
    dumpOptDetails(
      comp(), "second child of store does not look like OR of 2 byte loads - no byte to char arraycopy performed\n");
    return false;
  }
  ...
  if (!arraycopyLoop.getStoreAddress()->checkIndVarStore(charIndVarStoreNode)) {
    ...
    if (swap && arraycopyLoop.getStoreAddress()->checkIndVarStore(byteIndVarStoreNode)) {
      ...
      dumpOptDetails(
        comp(), "Did not encounter byte array induction variable increment - no byte to char arraycopy performed\n");
      return false;
    }
  } else if (!arraycopyLoop.getHighLoadAddress()->checkIndVarStore(byteIndVarStoreNode)) {
    dumpOptDetails(
      comp(), "Did not encounter char array induction variable increment - no byte to char arraycopy performed\n");
    return false;
  }
  ...
  if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode, charIndVar)) {
    ...
    //   { ... }
    dumpOptDetails(
      comp(), "Loop comparison does not match byte or char induction variable - no byte to char arraycopy performed\n");
    return false;
  }
  ...
  TR_InductionVariable* byteIndVar, TR_InductionVariable* charIndVar, TR::Block* loopHeader)
{
  ...
  if (!arraycopyLoop.getLoadAddress()->checkIndVarStore(charIndVarStoreNode)) {
    ...
    if (swap && arraycopyLoop.getLoadAddress()->checkIndVarStore(byteIndVarStoreNode)) {
      ...
      dumpOptDetails(
        comp(), "Did not encounter byte array induction variable increment - no char to byte arraycopy performed\n");
      return false;
    }
  } else if (!arraycopyLoop.getHighStoreAddress()->checkIndVarStore(byteIndVarStoreNode)) {
    dumpOptDetails(
      comp(), "Did not encounter char array induction variable increment - no char to byte arraycopy performed\n");
    return false;
  }
  ...
  if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode, charIndVar)) {
    ...
    //   { ... }
    dumpOptDetails(
      comp(), "Loop comparison does not match byte or char induction variable - no char to byte arraycopy performed\n");
    return false;
  }
...
int32_t TR_LoopReducer::perform()
{
  ...
  if (!comp()->cg()->getSupportsArraySet() && !comp()->cg()->getSupportsReferenceArrayCopy()
    ...
    && !comp()->cg()->getSupportsArrayTranslateTRxx() && !comp()->cg()->getSupportsArrayTranslateAndTest()) {
