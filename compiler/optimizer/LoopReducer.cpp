...
static TR::Node *testNode(TR::Compilation *comp, TR::Node *node,
                          TR::ILOpCodes value, const char *msg) {
...
static TR::Node *testUnary(TR::Compilation *comp, TR::Node *parent,
                           TR::ILOpCodes value, const char *msg) {
  return testNode(comp, parent->getFirstChild(), value, msg);
}

static TR::Node *testBinary(TR::Compilation *comp, TR::Node *parent,
                            TR::ILOpCodes value, TR::ILOpCodes leftChildValue,
                            TR::ILOpCodes rightChildValue, const char *msg) {
  TR::Node *child = parent->getFirstChild();
  if (child->getOpCodeValue() != value ||
      child->getFirstChild()->getOpCodeValue() != leftChildValue ||
...
static TR::Node *testBinaryIConst(TR::Compilation *comp, TR::Node *parent,
                                  TR::ILOpCodes value,
                                  TR::ILOpCodes leftChildValue,
...
TR_LRAddressTree::TR_LRAddressTree(TR::Compilation *comp,
                                   TR_InductionVariable *indVar)
    : TR_AddressTree(heapAlloc, comp), _indVar(indVar),
      _increment(indVar->getIncr()->getLowInt()), _indVarLoad(NULL) {}

TR_ArrayLoop::TR_ArrayLoop(TR::Compilation *comp,
                           TR_InductionVariable *firstIndVar,
                           TR_InductionVariable *secondIndVar)
    : _comp(comp), _firstAddress(comp, firstIndVar),
      _secondAddress(comp, secondIndVar), _thirdAddress(comp, secondIndVar),
...
TR_ArrayLoop::TR_ArrayLoop(TR::Compilation *comp, TR_InductionVariable *indVar)
    : _comp(comp), _firstAddress(comp, indVar), _secondAddress(comp, indVar),
      _thirdAddress(comp, indVar), _finalNode(NULL), _addInc(false),
      _forwardLoop(false) {}
...
TR_ByteToCharArraycopy::TR_ByteToCharArraycopy(
    TR::Compilation *comp, TR_InductionVariable *firstIndVar,
...
TR_Arraycmp::TR_Arraycmp(TR::Compilation *comp, TR_InductionVariable *indVar)
    : _targetOfGotoBlock(NULL), _firstLoad(NULL), _secondLoad(NULL),
      TR_ArrayLoop(comp, indVar) {}

TR_Arraytranslate::TR_Arraytranslate(TR::Compilation *comp,
                                     TR_InductionVariable *indVar,
                                     bool hasBranch, bool hasBreak)
    : TR_ArrayLoop(comp, indVar), _inputNode(NULL), _outputNode(NULL),
      _termCharNode(NULL), _tableNode(NULL), _resultNode(NULL),
      _resultUnconvertedNode(NULL), _usesRawStorage(false),
      _compilerGeneratedTable(false), _hasBranch(hasBranch),
      _hasBreak(hasBreak), _compareOp(TR::BadILOp) {}

TR_ArraytranslateAndTest::TR_ArraytranslateAndTest(TR::Compilation *comp,
                                                   TR_InductionVariable *indVar)
...
bool TR_LRAddressTree::checkAiadd(TR::Node *aiaddNode, int32_t elementSize) {
  if (!process(aiaddNode)) {
    dumpOptDetails(
        comp(), "checkAiadd: base processing of node did not match criteria\n");
    return false;
  }

  TR::RegisterMappedSymbol *indSym = _indVar->getLocal();

  if (_indVarNode.isNull() || (_indVarNode.getChild()
                                   ->skipConversions()
                                   ->getSymbol()
                                   ->getRegisterMappedSymbol() != indSym)) {
    dumpOptDetails(
        comp(),
        "checkAiadd: induction variable does not match index variable\n");
    return false;
  }

  TR::RegisterMappedSymbol *loadSym = _indVarNode.getChild()
                                          ->skipConversions()
                                          ->getSymbol()
                                          ->getRegisterMappedSymbol();
  if (loadSym != indSym) {
    if (_matIndVarSymRef) {
      if (loadSym != _matIndVarSymRef->getSymbol()->getRegisterMappedSymbol()) {
        dumpOptDetails(comp(), "checkAiadd: load in the aiadd tree does not "
                               "match materialized induction variable\n");
        return false;
      }
    } else {
      dumpOptDetails(
  ...
  if (_multiplyNode.isNull() &&
      ((elementSize != _increment) && (elementSize != -_increment))) {
    dumpOptDetails(comp(),
                   "checkAiadd: sub-tree does not have induction variable "
                   "change consistent with increment of multiplier (%d %d)\n",
...
bool TR_Arrayset::checkArrayStore(TR::Node *storeNode) {
  if (!storeNode->getOpCode().isStoreIndirect()) {
    dumpOptDetails(comp(),
                   "arraystore tree does not have an indirect store as root\n");
    return false;
  }
  ...
  if (opCodeStoreSecondChild == TR::iload &&
      storeSecondChild->getSymbol()->getRegisterMappedSymbol() ==
          getIndVar()->getLocal()) {
  ...
  if (!storeSecondChild->getOpCode().isLoadDirectOrReg()) {
    dumpOptDetails(comp(), "arraystore tree does not have a constant load, or "
                           "constant load is an address\n");
    return false;
  }
...
bool TR_Arraycopy::checkArrayStore(TR::Node *storeNode) {
  if (!storeNode->getOpCode().isStoreIndirect() &&
    ...
    dumpOptDetails(
        comp(),
        "arraycopy arraystore tree does not have an indirect store as root\n");
    return false;
  }
  ...
  if (!loadNode->getOpCode().isLoadIndirect()) {
    dumpOptDetails(comp(), "arraycopy arraystore tree does not have an "
                           "indirect load as the second child\n");
    return false;
  }
  if (loadNode->getSize() != storeNode->getSize()) {
    dumpOptDetails(comp(),
                   "arraycopy src and dst trees are not of the same size\n");
    return false;
  }
  ...
  if (storeFirstChild->getNumChildren() == 0 ||
      loadFirstChild->getNumChildren() == 0 ||
      !storeFirstChild->getFirstChild()->getOpCode().hasSymbolReference() ||
      !loadFirstChild->getFirstChild()->getOpCode().hasSymbolReference() ||
      storeFirstChild->getFirstChild()
              ->getSymbol()
              ->getRegisterMappedSymbol() == loadFirstChild->getFirstChild()
                                                 ->getSymbol()
                                                 ->getRegisterMappedSymbol()) {
    dumpOptDetails(
        comp(),
        "arraycopy src and dst are against same object - punt for now\n");
    return false;
  }

  bool checkStore =
      getStoreAddress()->checkAiadd(storeFirstChild, storeNode->getSize());
  bool checkLoad =
      getLoadAddress()->checkAiadd(loadFirstChild, loadNode->getSize());
...
bool TR_ByteToCharArraycopy::checkArrayStore(TR::Node *storeNode) {
  if (storeNode->getOpCodeValue() != TR::cstorei) {
    dumpOptDetails(comp(), "byte to char arraycopy arraystore tree does not "
                           "have an indirect store as root\n");
    return false;
  }
  TR::Node *storeFirstChild = storeNode->getFirstChild();
  TR::ILOpCodes opCodeStoreFirstChild = storeFirstChild->getOpCodeValue();
...
bool TR_ByteToCharArraycopy::checkByteLoads(TR::Node *loadNodes) {
  if (loadNodes->getOpCodeValue() != TR::i2s) {
    dumpOptDetails(comp(), "checkByteLoads: byte to char arraycopy byte loads "
                           "is not headed with i2c\n");
    return false;
  }
  TR::Node *joinNode = loadNodes->getFirstChild();
  if (joinNode->getOpCodeValue() != TR::ior &&
      joinNode->getOpCodeValue() != TR::iadd) {
    dumpOptDetails(comp(), "checkByteLoads: byte to char arraycopy byte loads "
                           "not joined with OR or ADD\n");
    return false;
  }
  ...
  if (highByteNode->getOpCodeValue() != TR::imul &&
      (highByteNode->getOpCodeValue() == TR::bu2i &&
       lowByteNode->getOpCodeValue() == TR::imul)) {
    // operands may be reversed - swap
    dumpOptDetails(comp(),
                   "checkByteLoads: try swapping the 2 OR/ADD children\n");
    TR::Node *tmpNode = highByteNode;
    highByteNode = lowByteNode;
    lowByteNode = tmpNode;
  } else {
    if (highByteNode->getOpCodeValue() != TR::imul ||
        lowByteNode->getOpCodeValue() != TR::bu2i) {
      dumpOptDetails(comp(), "checkByteLoads: byte to char arraycopy byte "
                             "loads do not have imul/bu2i children\n");
      return false;
    }
  }

  if (highByteNode->getFirstChild()->getOpCodeValue() != TR::bu2i ||
      highByteNode->getFirstChild()->getFirstChild()->getOpCodeValue() !=
          TR::bloadi) {
    dumpOptDetails(
        comp(), "checkByteLoads: high byte load does not have bu2i/ibload\n");
    return false;
  }
  if (lowByteNode->getFirstChild()->getOpCodeValue() != TR::bloadi) {
    dumpOptDetails(comp(),
                   "checkByteLoads: low byte load does not have ibload\n");
    return false;
  }
  ...
  if (highByteNode->getSecondChild()->getOpCodeValue() != TR::iconst ||
    ...
    dumpOptDetails(comp(),
                   "checkByteLoads: multiplier for high value is not 256\n");
    return false;
  }

  TR::Node *lowByteAddressNode = lowByteNode->getFirstChild()->getFirstChild();
  TR::Node *highByteAddressNode =
      highByteNode->getFirstChild()->getFirstChild()->getFirstChild();
  ...
  if (getLowLoadAddress()->getOffset() !=
      getHighLoadAddress()->getOffset() + 1) {
    dumpOptDetails(comp(),
                   "checkByteLoads: second offset is not one greater than "
                   "first offset (%d %d)\n",
                   (int32_t)getLowLoadAddress()->getOffset(),
                   (int32_t)getHighLoadAddress()->getOffset());
    return false;
  }

  TR::RegisterMappedSymbol *hbs =
      getHighLoadAddress()->getBaseVarNode()->getChild()
          ? getHighLoadAddress()
                ->getBaseVarNode()
                ->getChild()
                ->getSymbol()
                ->getRegisterMappedSymbol()
          : NULL;
  TR::RegisterMappedSymbol *lbs =
      getLowLoadAddress()->getBaseVarNode()->getChild()
          ? getLowLoadAddress()
                ->getBaseVarNode()
                ->getChild()
                ->getSymbol()
  ...
  if (hbs || lbs) {
    ...
    if (!hbs || !lbs || hbs != lbs) {
      dumpOptDetails(comp(),
                     "checkByteLoads: at least one tree has a base sym, but "
                     "both trees do not have the same sym (%p %p)\n",
                     lbs, hbs);
      return false;
    }
  }
...
bool TR_CharToByteArraycopy::checkArrayStores(TR::Node *origHighStoreNode,
                                              TR::Node *origLowStoreNode) {
  ...
  if (highStoreNode->getOpCodeValue() != TR::bstorei) {
    dumpOptDetails(comp(),
                   "checkArrayStores: char to byte arraycopy high arraystore "
                   "tree does not have an indirect store as root\n");
    return false;
  }
  if (lowStoreNode->getOpCodeValue() != TR::bstorei) {
    dumpOptDetails(comp(),
                   "checkArrayStores: char to byte arraycopy low arraystore "
                   "tree does not have an indirect store as root\n");
    return false;
  }
  ...
  bool checkHighStore =
      getHighStoreAddress()->checkAiadd(highStoreAddressNode, 2);
  ...
  if ((int32_t)getLowStoreAddress()->getOffset() !=
      (int32_t)getHighStoreAddress()->getOffset() + 1) {
    dumpOptDetails(comp(),
                   "checkArrayStores: second offset is not 1 greater than "
                   "first offset (%d %d)\n",
                   (int32_t)getLowStoreAddress()->getOffset(),
                   (int32_t)getHighStoreAddress()->getOffset());
    return false;
  }

  TR::Node *ishr =
      testNode(comp(), origHighStoreNode->getSecondChild(), TR::i2b,
               "checkArrayStores: high store child is not i2b\n");
  if (!ishr)
    return false;
  TR::Node *iand = testBinaryIConst(
      comp(), ishr, TR::ishr, TR::iand, 8,
      "checkArrayStores: high store child is not ishr of iand and 8\n");
  if (!iand)
    return false;
  TR::Node *c2i = testBinaryIConst(
      comp(), iand, TR::iand, TR::su2i, 0xFF00,
      "checkArrayStores: high store child is not iand of su2i and 0xFF00\n");
  if (!c2i)
    return false;
  TR::Node *icload =
      testUnary(comp(), c2i->getFirstChild(), TR::cloadi,
                "checkArrayStores: high store child is not icload\n");
  if (!icload)
    return false;
  ...
  c2i = testBinaryIConst(
      comp(), iand, TR::iand, TR::su2i, 0xFF,
      "checkArrayStores: low store child is not iand of su2i and 0xFF\n");
  if (!c2i)
    return false;
  TR::Node *icloadDup =
      testUnary(comp(), c2i->getFirstChild(), TR::cloadi,
                "checkArrayStores: low store child is not icload\n");
  if (!icloadDup)
    return false;

  if (icloadDup != icload) {
    dumpOptDetails(comp(),
                   "checkArrayStores: two icload addresses are not the same\n");
    return false;
  }
...
//
// This method verifies that the sub-tree passed in conforms to an increment of
// a variable and that the variable being incremented is the induction variable
// for the loop.
//
// istore <ind var>
//   iadd
...
bool TR_LRAddressTree::checkIndVarStore(TR::Node *indVarNode) {
  if (!indVarNode->getOpCode().isStoreDirect()) {
    dumpOptDetails(
        comp(),
        "induction variable tree does not have a direct store as root\n");
    return false;
  }
  ...
  if (opCodeIaddFirstChild != TR::iload ||
      opCodeIaddSecondChild != TR::iconst) {
    dumpOptDetails(comp(), "first child of iadd is not TR::iload or second "
                           "child is not TR::iconst\n");
    return false;
  }
  TR::RegisterMappedSymbol *indSym = _indVar->getLocal();
  if (indSym != iaddFirstChild->getSymbol()->getRegisterMappedSymbol()) {
    dumpOptDetails(
        comp(), "iload symbol for aload does not match induction variable\n");
    return false;
  }
  ...
  if (_increment != realIncr) {
    dumpOptDetails(comp(),
                   "increment does not match induction variable increment\n");
    return false;
  } else {
...
bool TR_ArrayLoop::checkLoopCmp(TR::Node *loopCmpNode,
                                TR::Node *indVarStoreNode,
                                TR_InductionVariable *indVar) {
  ...
  // if the comparison to leave the loop is equality, add (subtract) one to ind
  // var on exit
  if (compareOp == TR::ificmpeq || compareOp == TR::ificmpge ||
      compareOp == TR::ificmple || compareOp == TR::ifiucmpge ||
  ...
  if (opCodeCmpFirstChild != TR::iload &&
      cmpFirstChild != indVarStoreNode->getFirstChild()) {
    dumpOptDetails(
        comp(),
        "loop compare does not have iload or indvarnode expr as first child\n");
    return false;
  }

  if (opCodeCmpSecondChild != TR::iconst && opCodeCmpSecondChild != TR::iload &&
      !cmpSecondChild->getOpCode().isArrayLength()) {
    dumpOptDetails(comp(), "loop compare does not have "
                           "iconst/iload/arraylength as second child\n");
    return false;
  }
...
int32_t TR_ArrayLoop::checkForPostIncrement(TR::Block *loopHeader,
                                            TR::Node *indVarStoreNode,
                                            TR::Node *loopCmpNode,
                                            TR::Symbol *ivSym) {
  ...
  for (TR::TreeTop *tt = startTree; !storeFound && tt != loopHeader->getExit();
       tt = tt->getNextTreeTop())
    findIndVarLoads(tt->getNode(), indVarStoreNode, storeFound, &ivLoads, ivSym,
                    visitCount);
...
void TR_ArrayLoop::findIndVarLoads(TR::Node *node, TR::Node *indVarStoreNode,
                                   bool &storeFound, List<TR::Node> *ivLoads,
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++)
    findIndVarLoads(node->getChild(i), indVarStoreNode, storeFound, ivLoads,
                    ivSym, visitCount);
}
...
TR::Node *TR_ArrayLoop::updateIndVarStore(TR_ParentOfChildNode *indVarNode,
                                          TR::Node *indVarStoreNode,
                                          TR_LRAddressTree *tree,
                                          int32_t postIncrement) {
  ...
  //
  TR::Node *isub = TR::Node::create(TR::isub, 2, endNode->duplicateTree(),
                                    startNode->duplicateTree());
  if (postIncrement != 0)
    isub =
        TR::Node::create(TR::iadd, 2, isub,
                         TR::Node::create(isub, TR::iconst, 0, postIncrement));
  if (_addInc) {
    int32_t positiveIncrement = (tree->getIncrement() < 0)
                                    ? -(tree->getIncrement())
                                    : tree->getIncrement();
    TR::Node *elemSizeInc =
        TR::Node::create(_finalNode, TR::iconst, 0, positiveIncrement);
  ...
  // before we change the child of the isub, check if it is commoned, and create
  // a copy if so.
  if (indVarStoreNode->getFirstChild()->getReferenceCount() > 1) {
    replacedNode = indVarStoreNode->getFirstChild();
    // duplicate the isub tree
    indVarStoreNode->setAndIncChild(
        0, indVarStoreNode->getFirstChild()->duplicateTree());
  }

  indVarStoreNode->getFirstChild()->setAndIncChild(0,
                                                   _finalNode->duplicateTree());
...
void TR_LRAddressTree::updateAiaddSubTree(TR_ParentOfChildNode *indVarNode,
                                          TR_ArrayLoop *loop) {
  ...
  if (endInc < 0 && !indVarNode->isNull()) {
    ...
    if (indVarNode->getParent()->getType().isInt64() &&
        !finalNode->getType().isInt64()) {
      TR::Node *i2lNode =
          TR::Node::create(TR::i2l, 1, finalNode->duplicateTree());
    ...
    if (!addInc) {
      TR::Node *elemSizeInc =
          TR::Node::create(finalNode, TR::iconst, 0, endInc);
      TR::Node *isub = TR::Node::create(TR::isub, 2, finalNode->duplicateTree(),
                                        elemSizeInc);
      // (64-bit)
...
TR::Node *TR_LRAddressTree::updateMultiply(TR_ParentOfChildNode *multiplyNode) {
  ...
  if (!multiplyNode->isNull()) {
    ...
    if (multiplyNode->getParent()->getType().isInt32()) {
      TR::Node *initMulVal =
          TR::Node::create(multiplyNode->getParent(), TR::iconst, 0,
                           (_increment > 0 ? _increment : -_increment));
      newMul =
          TR::Node::create(TR::imul, 2, multiplyNode->getChild(), initMulVal);
    } else {
      TR::Node *initMulVal =
          TR::Node::create(multiplyNode->getParent(), TR::lconst);
      initMulVal->setLongInt((_increment > 0 ? _increment : -_increment));

      newMul =
          TR::Node::create(TR::lmul, 2, multiplyNode->getChild(), initMulVal);
    }
...
// The code recognizes the following arraycopy patterns is similar to arrayset
// pattern: iTstore #ArrayStore Shadow[<array-shadow>] (where T is the type -
// one of b, c, s, i, l, d, f)
//   aiadd
...
//       iconst -16
//   iXload #ArrayLoad Shadow[<array-shadow>] (where T is the type - one of b,
//   c, s, i, l, d, f)
//     aiadd
...
//   iconst 0 (or iload <val> if loop invariant)
//
bool TR_LoopReducer::generateArraycopy(TR_InductionVariable *indVar,
                                       TR::Block *loopHeader) {
  // for aladds
  if (!comp()->cg()->getSupportsReferenceArrayCopy() &&
  ...
  if (loopHeader->getNumberOfRealTreeTops() !=
      3) // array store, induction variable store, loop comparison
  {
  ...
  if (!arraycopyLoop.checkLoopCmp(
          loopCmpNode, indVarStoreNode,
          arraycopyLoop.getStoreAddress()->getIndVar())) {
    return false;
  }
  ...
  // FUTURE: can eliminate wrtbar when src and dest are equal. Currently we
  // don't reduce arraycopy like this.
  if (arraycopyLoop.hasWriteBarrier() && needWriteBarrier &&
      !comp()->cg()->getSupportsReferenceArrayCopy()) {
    dumpOptDetails(comp(), "arraycopy arraystore tree has write barrier as "
                           "root and write barriers are enabled but no support "
                           "for this platform- no arraycopy reduction\n");
    return false;
  }

  int32_t postIncrement = arraycopyLoop.checkForPostIncrement(
      loopHeader, indVarStoreNode, loopCmpNode,
  ...
  if (storeNode->getType().isBCD() &&
    ...
    dumpOptDetails(
        comp(),
        "arraycopy storeNode %p is a BCD type (%s) and the storeSize (%d) is "
        "not 1,2,4 or 8 so do not reduce arraycopy\n",
  ...
  // Create a new tree with a primitive arraycopy call node followed by an
  // istore node to store the induction variable The parameters will be:
  //   The address to store into (the store aiadd tree - with the iload variable
  //   changed if negative increment) The address to load from  (the load aiadd
  //   tree - with the iload variable changed if negative increment) The number
  //   of bytes to copy(induction variable initial value - comparison value) *
  //   stride
  //
  // Tree-Top
  // ArrayCopy call
  ...
  //     aload #ArrayStoreBase (loop invariant)
  //     isub
  //       imul (this imul may need to be generated if not in original code and
  //       stride not 1)
  ...
  //     iconst increment
  //
  arraycopyLoop.getStoreAddress()->updateAiaddSubTree(
      arraycopyLoop.getStoreIndVarNode(), &arraycopyLoop);
  arraycopyLoop.getLoadAddress()->updateAiaddSubTree(
      arraycopyLoop.getLoadIndVarNode(), &arraycopyLoop);
  TR::Node *imul = arraycopyLoop.updateIndVarStore(
      arraycopyLoop.getStoreIndVarNode(), indVarStoreNode,
      arraycopyLoop.getStoreAddress(), postIncrement);
  arraycopyLoop.getStoreAddress()->updateMultiply(
      arraycopyLoop.getStoreMultiplyNode());

  TR::Node *storeAddr = arraycopyLoop.getStoreNode()->getFirstChild();
  TR::Node *loadAddr =
      arraycopyLoop.getStoreNode()->getSecondChild()->getFirstChild();
  TR::Node *arraycopy;
  if (!(arraycopyLoop.hasWriteBarrier() && needWriteBarrier)) {
    ...
    TR::ILOpCodes op_const =
        TR::Compiler->target.is64Bit() ? TR::lconst : TR::iconst;

    offset = arraycopyLoop.getStoreNode()->getSymbolReference()->getOffset();
    if (offset != 0)
      dst = TR::Node::create(op_add, 2, dst,
                             TR::Node::create(dst, op_const, 0, offset));

    offset = arraycopyLoop.getStoreNode()
                 ->getSecondChild()
                 ->getSymbolReference()
                 ->getOffset();
    if (offset != 0)
      src = TR::Node::create(op_add, 2, src,
                             TR::Node::create(src, op_const, 0, offset));
    ...
    if (arraycopyElementType.isBCD()) {
      ...
      case 8:
        ...
        break;
      default:
        // a pre-transformation check just above guarantees only sizes 1,2,4 or
        // 8 are transformed
    ...
    arraycopy = TR::Node::createArraycopy(loadAddr->getFirstChild(),
                                          storeAddr->getFirstChild(), loadAddr,
  ...
  TR::TreeTop *deadCmpChild1 =
      TR::TreeTop::create(comp(), indVarStoreTree, loopCmpNode);
  TR::TreeTop *deadCmpChild2 =
      TR::TreeTop::create(comp(), deadCmpChild1, loopCmpNode);

  deadCmpChild1->setNode(TR::Node::create(
      TR::treetop, 1, loopCmpTree->getNode()->getFirstChild()));
  deadCmpChild2->setNode(TR::Node::create(
      TR::treetop, 1, loopCmpTree->getNode()->getSecondChild()));
  ...
  if (arraycopyLoop.hasWriteBarrier()) {
    TR::TreeTop *deadCmpChild =
        TR::TreeTop::create(comp(), deadCmpChild2, loopCmpNode);
    deadCmpChild->setNode(TR::Node::create(
        TR::treetop, 1, arraycopyLoop.getStoreNode()->getChild(2)));
...
// iTstore #ArrayStore Shadow[<array-shadow>] (where T is the type - one of b,
// c, s, i, l, d, f
//   aiadd
...
//   iconst 0 (or iload <val> if loop invariant)
//
bool TR_LoopReducer::generateArrayset(TR_InductionVariable *indVar,
                                      TR::Block *loopHeader) {
  // for aladds
  ...
  if (copyValueNode->getType().isFloatingPoint()) {
    // TODO: Evaluators need to support these types for an arrayset, if they're
    // intended to be supported
    dumpOptDetails(
        comp(),
        "Loop has unsupported copyValueNode type %s so do not transform\n",
  ...
  if (!performTransformation(
          ...
          OPT_DETAILS, loopHeader->getNumber(), storeNode,
          storeNode->getSecondChild())) {
    return false;
  }

  //
  // At this point, this is an ArraySet loop.
  // Create a new tree with an ArraySet call node followed by an istore node to
  // store the induction variable The parameters will be:
  //   The address to store into (the aiadd tree - with the iload variable
  //   changed if negative increment) The value to store (the result of the 2nd
  //   child of the iTstore tree) The number of elements to set to the value
  //   (induction variable initial value - comparison value) / increment /
  //   multiplier
  ...
  //     iconst increment
  //
  arraysetLoop.getStoreAddress()->updateAiaddSubTree(
      arraysetLoop.getIndVarNode(), &arraysetLoop);
  TR::Node *imul = arraysetLoop.updateIndVarStore(
      arraysetLoop.getIndVarNode(), indVarStoreNode,
      arraysetLoop.getStoreAddress());
  arraysetLoop.getStoreAddress()->updateMultiply(
      arraysetLoop.getMultiplyNode());
  ...
  TR::Node *arrayset = TR::Node::create(TR::arrayset, 3, dst, copyValueNode,
                                        imul->duplicateTree());

  storeNode->getFirstChild()->decReferenceCount();
  storeNode->getSecondChild()->decReferenceCount();

  arrayset->setSymbolReference(
...
bool TR_Arraycmp::checkElementCompare(TR::Node *elemCmpNode) {
  ...
  if (elemOp != TR::ificmpne && elemOp != TR::iffcmpne &&
      elemOp != TR::iffcmpneu && elemOp != TR::ifdcmpne &&
      elemOp != TR::ifdcmpneu && elemOp != TR::iflcmpne) {
    dumpOptDetails(comp(),
                   "element compare tree does not have an ifxcmpne as root\n");
    return false;
  }
  ...
  if (cmpFirstChild->getOpCode().isLoadIndirect()) {
    ...
    dumpOptDetails(comp(),
                   "no array element load encountered on first cmp child\n");
    return false;
  }
  ...
  if (cmpSecondChild->getOpCode().isLoadIndirect()) {
    ...
    dumpOptDetails(comp(),
                   "no array element load encountered on second cmp child\n");
    return false;
  }

  if (!getFirstAddress()->checkAiadd(cmpFirstChild->getFirstChild(),
                                     cmpFirstChild->getSize())) {
    dumpOptDetails(comp(),
                   "firstAddress check failed on checkElementCompare\n");
    return false;
  }
  if (!getFirstAddress()->checkAiadd(cmpSecondChild->getFirstChild(),
                                     cmpSecondChild->getSize())) {
    dumpOptDetails(comp(),
                   "secondAddress check failed on checkElementCompare\n");
    return false;
  }
...
bool TR_Arraycmp::checkGoto(TR::Block *gotoBlock, TR::Node *gotoNode,
                            TR::Node *finalNode) {
  ...
  if (gotoOpCode == TR::istore) {
    if (gotoBlock->getNumberOfRealTreeTops() != 2) {
      dumpOptDetails(comp(),
                     "goto tree has istore but too many subsequent nodes\n");
      return false;
    }
  ...
  if (targetNode->getOpCode().getOpCodeValue() != TR::ificmpne) {
    if (targetNode->getOpCode().getOpCodeValue() == TR::istore) {
      ...
      if (targetNode->getOpCode().getOpCodeValue() != TR::icmpeq) {
        dumpOptDetails(comp(),
                       "target of goto is not an ificmpne/istore. It is %s\n",
  ...
  TR::ILOpCodes opCodeFirstChild =
      targetNode->getFirstChild()->getOpCode().getOpCodeValue();
  if (opCodeFirstChild != TR::iload) {
    dumpOptDetails(comp(), "first child: goto comparison does have iload\n");
    return false;
  }
  TR::ILOpCodes opCodeSecondChild =
      targetNode->getSecondChild()->getOpCode().getOpCodeValue();
  if (opCodeSecondChild != TR::iload && opCodeSecondChild != TR::iconst) {
    dumpOptDetails(comp(),
                   "second child: goto comparison does have iload/iconst\n");
    return false;
  }

  TR::RegisterMappedSymbol *indSym = getFirstAddress()->getIndVar()->getLocal();
  TR::RegisterMappedSymbol *comparisonSym =
      (comparisonNode->getOpCode().hasSymbolReference())
          ? comparisonNode->getSymbol()->getRegisterMappedSymbol()
          : NULL;
  TR::RegisterMappedSymbol *firstChildSym =
      targetNode->getFirstChild()->getSymbol()->getRegisterMappedSymbol();
  TR::RegisterMappedSymbol *secondChildSym =
      (opCodeSecondChild == TR::iload)
  ...
  if (extraStoreNode != NULL) {
    if (!(firstChildSym == comparisonSym && secondChildSym == NULL)) {
      dumpOptDetails(comp(), "first/second child: alternate goto comparison "
                             "not to comparison sym\n");
      return false;
    }
             ...
             !(firstChildSym == indSym && secondChildSym == NULL &&
               comparisonNode->getInt() ==
                   targetNode->getSecondChild()->getInt())) {
    dumpOptDetails(comp(), "first/second child: goto comparison are not "
                           "final/induction variable syms\n");
    return false;
  }
...
static void swapIfNecessary(TR::Node *&aiaddFirstChild,
                            TR::Node *&aiaddSecondChild) {
...
//        isub (add displacement to get to start of array with index into
//        translation table)
//          imul (multiply by stride of target - may not be present if
//          byte-to-byte)
...
bool TR_Arraytranslate::checkLoad(TR::Node *loadNode) {
  ...
  if (hasBranch()) {
    if (loadNode->getOpCodeValue() != TR::istore) {
      dumpOptDetails(
          comp(),
          "...load tree does not have store - no arraytranslate reduction\n");
      return false;
    }
  ...
  if (transLoadNode->getOpCodeValue() != TR::cloadi &&
      transLoadNode->getOpCodeValue() != TR::bloadi) {
    dumpOptDetails(comp(), "...load tree does not have ibload/icload - no "
                           "arraytranslate reduction\n");
    return false;
  }
  ...
  if (aiaddNode->getOpCodeValue() != TR::aiadd &&
    ...
    dumpOptDetails(comp(), "...load tree does not have aiadd/aladd/iadd/ladd - "
                           "no arraytranslate reduction\n");
    return false;
  }
  ...
  if (aiaddNode->getOpCodeValue() == TR::aiadd ||
      aiaddNode->getOpCodeValue() == TR::aladd) {
    if (aiaddFirstChild->getOpCodeValue() != TR::aload &&
        aiaddFirstChild->getOpCodeValue() != TR::aloadi) {
      dumpOptDetails(comp(), "...aiadd load tree does not have aload - no "
                             "arraytranslate reduction\n");
      return false;
    }
    _tableNode = aiaddFirstChild;

    if (aiaddSecondChild->getOpCodeValue() != TR::isub &&
        aiaddSecondChild->getOpCodeValue() != TR::lsub) {
      dumpOptDetails(
          comp(),
          "...load tree does not have isub - no arraytranslate reduction\n");
      return false;
    }
    ...
    if (aiaddSecondChild->getOpCodeValue() ==
        TR::l2i) // skip over l2i if present
    {
      aiaddSecondChild = aiaddSecondChild->getFirstChild();
    }
    if (aiaddSecondChild->getOpCodeValue() != TR::lloadi &&
        aiaddSecondChild->getOpCodeValue() != TR::lload) {
      dumpOptDetails(comp(), "...iadd load tree does not have ilload - no "
                             "arraytranslate reduction\n");
      return false;
    }
  ...
  if (inLoadNode->getOpCodeValue() != TR::cloadi &&
      inLoadNode->getOpCodeValue() != TR::bloadi) {
    dumpOptDetails(comp(), "...load tree does not have 2nd icload/ibload - "
                           "check if compiler-generated table lookup match\n");
...
bool TR_Arraytranslate::checkStore(TR::Node *storeNode) {
  if (storeNode->getOpCodeValue() != TR::cstorei &&
      storeNode->getOpCodeValue() != TR::bstorei) {
    dumpOptDetails(comp(), "...store tree does not have icstore/ibstore - no "
                           "arraytranslate reduction\n");
    return false;
  }
  ...
  if (aiaddNode->getOpCodeValue() != TR::aiadd &&
      aiaddNode->getOpCodeValue() != TR::aladd) {
    dumpOptDetails(comp(), "...store tree does not have aiadd/aladd - no "
                           "arraytranslate reduction\n");
    return false;
  }
  ...
  if (hasBranch()) {
    ...
    if (shrinkNode->getOpCodeValue() != TR::i2s &&
      ...
      dumpOptDetails(comp(),
                     "...store tree does not have i2c/i2b/cconst/bconst - no "
                     "arraytranslate reduction\n");
      return false;
    }

    if (shrinkNode->getOpCodeValue() == TR::i2b ||
        shrinkNode->getOpCodeValue() == TR::bconst) {
      _byteOutput = true;
    } else {
      _byteOutput = false;
    ...
    if (shrinkNode->getOpCodeValue() == TR::i2b ||
        shrinkNode->getOpCodeValue() == TR::i2s) {
      TR::Node *iloadNode = shrinkNode->getFirstChild();
      if (iloadNode->getOpCodeValue() != TR::iload) {
        dumpOptDetails(comp(), "...store tree does not have iload - no "
                               "arraytranslate reduction\n");
        return false;
      }

      if (iloadNode->getSymbolReference() !=
          _resultNode->getSymbolReference()) {
        dumpOptDetails(comp(),
                       "...store tree reference does not match load tree "
                       "reference - no arraytranslate reduction\n");
        return false;
      }
    }
  } else {
...
//
bool TR_Arraytranslate::checkBreak(TR::Block *breakBlock, TR::Node *breakNode,
                                   TR::Block *nextBlock) {
  if (breakNode->getOpCodeValue() != TR::ificmpeq) {
    if (compilerGeneratedTable()) {
      // if the compiler is generating the table, we can handle any compare, but
      // only to a constant
      if (!breakNode->getOpCode().isBooleanCompare()) {
        dumpOptDetails(
            comp(), "...break tree does not have expected compare operator\n");
        return false;
      }
    } else {
  ...
  if (loadNode->getSymbolReference() !=
          _resultUnconvertedNode->getSymbolReference() &&
      loadNode->getSymbolReference() !=
          _resultNode->getFirstChild()->getSymbolReference() &&
      loadNode->getSymbolReference() != _resultNode->getSymbolReference()) {
    dumpOptDetails(comp(), "...break tree reference does not match load tree "
                           "reference - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (valNode->getOpCodeValue() != TR::iconst ||
      (compilerGeneratedTable() &&
       (valNode->getInt() > 32766 || valNode->getInt() < -32766))) {
    // msf - this can be loosened up to just ensure the expression is a byte or
    // character value
    //      that is loop invariant iff it is not a compilerGeneratedTable() loop
    //      we also conservatively ensure the range is inside the represented
    //      ranges such that we can add or subtract 1 from the value and still
    //      represent it in 2**16 bits for a default value if we need to
    //      generate a table.
    dumpOptDetails(comp(), "...break tree does not have bconst/cconst/iconst, "
                           "or not in range - no arraytranslate reduction\n");
    return false;
  }
...
//
bool TR_Arraytranslate::checkMatIndVarStore(TR::Node *matIndVarStoreNode,
                                            TR::Node *indVarStoreNode) {
  if (!matIndVarStoreNode->getOpCode().isStoreDirect()) {
    dumpOptDetails(comp(),
                   "materialized induction variable tree %p does not have a "
                   "direct store as root\n",
                   matIndVarStoreNode);
    return false;
  }
  ...
  if (storeFirstChild->getOpCodeValue() != TR::iadd &&
    ...
    dumpOptDetails(comp(),
                   "first child %p of materialized induction variable store is "
                   "not TR::iadd/TR::isub\n",
                   storeFirstChild);
    return false;
  }
  ...
  if (iaddFirstChild->getOpCodeValue() != TR::iadd &&
    ...
    dumpOptDetails(
        comp(),
        "materialized variable is not expressed in terms of primary iv %p\n",
        iaddFirstChild);
    return false;
  }
  ...
  if (indVarLoad->getOpCodeValue() != TR::iload ||
    ...
    dumpOptDetails(comp(),
                   "primary iv in the materialized tree is not TR::iload %p or "
                   "second child is not TR::iconst %p\n",
  ...
  if (indVarLoad->getSymbol()->getRegisterMappedSymbol() !=
      indVar->getLocal()) {
    dumpOptDetails(comp(),
                   "materialized iv is not actually materialized at all\n");
    return false;
  }
  ...
  if (iaddSecondChild->getOpCodeValue() != TR::iload &&
    ...
    dumpOptDetails(
        comp(),
        "second child of the materialized tree is not delta or a constant %p\n",
        iaddSecondChild);
    return false;
  }

  getStoreAddress()->setMaterializedIndVarSymRef(
      matIndVarStoreNode->getSymbolReference());

  return true;
}

bool TR_Arraytranslate::checkGoto(TR::Block *gotoBlock, TR::Node *gotoNode,
                                  TR::Block *nextBlock) {
  TR::ILOpCodes gotoOpCode = gotoNode->getOpCode().getOpCodeValue();
  ...
  TR::Block *targetBlock =
      gotoNode->getBranchDestination()->getEnclosingBlock();
  if (targetBlock != nextBlock) {
    dumpOptDetails(
        comp(),
        "...goto tree does not goto the first block after the loop %p %p\n",
...
bool TR_LoopReducer::blockInVersionedLoop(
    List<TR::CFGEdge> otherLoopVersionBlockList, TR::Block *block) {
...
bool TR_LoopReducer::generateArraycmp(TR_RegionStructure *whileLoop,
                                      TR_InductionVariable *indVar,
                                      TR::Block *branchBlock,
                                      TR::Block *incrementBlock) {
  //
  // The general loop that we need to recognize for an array compare is as
  // follows:
  //
  // BBStart (block #1)
  ...
  //      aiadd
  //        aload <array #1 base>
  //        isub (this could be variable based on internal pointers, striding,
  //        type being loaded) [common base offset]
  ...
  int32_t incrNum =
      incrementBlock ? incrementBlock->getNumberOfRealTreeTops() : 0;
  if (compNum != 1 || incrNum != 2) {
    dumpOptDetails(
        comp(),
        "Loop has wrong number of tree tops (%d,%d) - no arraycmp reduction\n",
        compNum, incrNum);
    return false;
  }
  ...
  //
  // The following check is to ensure someone doesn't read the induction
  // variable other than for comparison to the 'final' value (which is in the
  // known blocks I've examined) in any successor block to the loop. If the
  // induction variable is written to first, then I can ignore that path. The
  // reason this is important is because the reduced arraycmp will not calculate
  // the induction variable's final value correctly if it terminates early, only
  // if it runs to completion, so I have to ensure that I don't reduce a loop
  // like: for (int i=0; i<len; ++i)
  //    {
  ...
  while (!queue.isEmpty()) {
    ...
    if (block != _cfg->getEnd() && block != branchBlock &&
        block != incrementBlock) {
      TR_TransformerDefUseState blockState =
          getSymbolDefUseStateInBlock(block, indVarSym);
      if (blockState == transformerReadFirst) {
        dumpOptDetails(comp(),
                       "induction variable is read before write in block_%d "
                       "after compare loop - no arraycmp reduction\n",
                       block->getNumber());
        return false;
      }
      ...
      if (blockState == transformerWrittenFirst) {
        dumpOptDetails(comp(),
                       "induction variable is written before read in block_%d "
                       "after compare loop - pruning successor\n",
                       block->getNumber());
        continue;
      }
    }
  ...
  if ((arraycmpLoop.getFirstAddress()->getIncrement() *
           arraycmpLoop.getFirstAddress()->getMultiplier() <
       0) ||
      (arraycmpLoop.getSecondAddress()->getIncrement() *
           arraycmpLoop.getSecondAddress()->getMultiplier() <
       0)) {
    dumpOptDetails(comp(),
                   "Can not reduce an arraycmp loop that runs backwards\n");
    return false;
  }
  ...
  //     (the istore can not be moved above the arraycmp because the ind var may
  //     be used in the arraycmp) (the istore must be after the conditional
  //     branch since it is used in exactly one subsequent compare)
  //
  // The parameters to ArrayCmp will be:
  //   The first address to compare to (the aiadd tree - with the iload variable
  //   changed if negative increment) The second address to compare to (the
  //   aiadd tree - this may reference the first iload in which case it can't be
  //   updated) The number of elements to compare (induction variable initial
  //   value - comparison value) / increment / multiplier
  //
  // Tree-Top
  // ArrayCmp call
  ...
  //     iconst increment
  //
  arraycmpLoop.getFirstAddress()->updateAiaddSubTree(
      arraycmpLoop.getFirstIndVarNode(), &arraycmpLoop);
  arraycmpLoop.getSecondAddress()->updateAiaddSubTree(
      arraycmpLoop.getSecondIndVarNode(), &arraycmpLoop);
  TR::Node *imul = arraycmpLoop.updateIndVarStore(
      arraycmpLoop.getFirstIndVarNode(), indVarStoreNode,
      arraycmpLoop.getFirstAddress());

  arraycmpLoop.getFirstAddress()->updateMultiply(
      arraycmpLoop.getFirstMultiplyNode());
  arraycmpLoop.getFirstAddress()->updateMultiply(
      arraycmpLoop.getSecondMultiplyNode());

  TR::Node *firstBase =
      branchNode->getFirstChild()->skipConversions()->getFirstChild();
  TR::Node *secondBase =
      branchNode->getSecondChild()->skipConversions()->getFirstChild();

  TR_ASSERT(arraycmpLoop.getFirstLoad(),
            "firstLoad is null for arraycmp reduction\n");
  TR_ASSERT(arraycmpLoop.getSecondLoad(),
            "secondLoad is null for arraycmp reduction\n");
  TR_ASSERT(arraycmpLoop.getFirstLoad()->getOpCode().isLoadVar(),
            "firstLoad %s (%p) is not a loadVar for arraycmp reduction\n",
            arraycmpLoop.getFirstLoad()->getOpCode().getName(),
            arraycmpLoop.getFirstLoad());
  TR_ASSERT(arraycmpLoop.getSecondLoad()->getOpCode().isLoadVar(),
            "secondLoad %s (%p) is not a loadVar for arraycmp reduction\n",
            arraycmpLoop.getSecondLoad()->getOpCode().getName(),
            arraycmpLoop.getSecondLoad());

  TR::Node *arraycmp =
      TR::Node::create(TR::arraycmp, 3, firstBase, secondBase, imul);

  TR::SymbolReference *arraycmpSymRef =
      comp()->getSymRefTab()->findOrCreateArrayCmpSymbol();
  arraycmp->setSymbolReference(arraycmpSymRef);

  firstBase->decReferenceCount();
  secondBase->decReferenceCount();

  TR::SymbolReference *compareTemp = comp()->getSymRefTab()->createTemporary(
  ...
  TR::Node *constBoolNode =
      TR::Node::create(arraycmp, TR::iconst, 0, (int32_t)0);
  ...
  int32_t cmpTargetBlockNumber =
      cmpTargetTree->getEnclosingBlock()->getNumber();
  TR::Node *compareNode = TR::Node::createif(TR::ificmpne, loadCompare,
                                             constBoolNode, cmpTargetTree);
  ...
  _cfg->removeEdge(incrementBlock->getSuccessors(), incrementBlock->getNumber(),
                   branchBlock->getNumber());
...
TR::Node *TR_Arraytranslate::getTableNode() {
  if (compilerGeneratedTable() && (_tableNode == NULL)) {
    ...
    if (TR::ILOpCode::isLessCmp(_compareOp)) {
      if (defaultValue < 0) {
        // need 2 ranges to initialize to non-default, 0 to signed-max and  max
        // + termVal
        startA = 0;
        endA = signedMax;
      ...
      if (defaultValue < 0) {
        ...
        // need 2 ranges to initialize to non-default, 0 to termVal and
        // signed-max+1 to max
        startA = 0;
        endA = defaultValue;
        startB = signedMax;
    ...
    if (inputBits == 8) {
      if (startB != endB) {
        TR_RangeTranslateTable rangeTable(
            comp(), inputBits, outputBits, (uint16_t)startA, (uint16_t)endA,
            (uint16_t)startB, (uint16_t)endB, defaultValue);
        tableSymRef = rangeTable.createSymbolRef();
        rangeTable.dumpTable();
      } else {
        TR_RangeTranslateTable rangeTable(comp(), inputBits, outputBits,
                                          (uint16_t)startA, (uint16_t)endA,
                                          defaultValue);
        tableSymRef = rangeTable.createSymbolRef();
        rangeTable.dumpTable();
      }
      ...
      if (startB != endB) {
        TR_RangeTranslateTable rangeTable(comp(), inputBits, outputBits, startA,
                                          endA, startB, endB, defaultValue);
        tableSymRef = rangeTable.createSymbolRef();
        rangeTable.dumpTable();
      } else {
        TR_RangeTranslateTable rangeTable(comp(), inputBits, outputBits, startA,
...
bool TR_LoopReducer::replaceInductionVariable(TR::Node *parent, TR::Node *node,
                                              int32_t childNum,
                                              int32_t indVarSymRefNumber,
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    if (replaceInductionVariable(node, node->getChild(i), i, indVarSymRefNumber,
                                 replacingNode, visitCount))
      return true;
  }
  return false;
}

bool TR_LoopReducer::generateArraytranslate(TR_RegionStructure *whileLoop,
                                            TR_InductionVariable *indVar,
                                            TR::Block *firstBlock,
                                            TR::Block *secondBlock,
                                            TR::Block *thirdBlock,
                                            TR::Block *fourthBlock) {
  // The following is the general loop you can expect for a translate type
  // instruction. Note that you will probably need to run with recompilation for
  // all the bounds checks to get eliminated - the profile-directed feedback is
  // required to eliminate the bounds check on the translation table reference.
  //
  ...
  //      {
  //      item = in[i];
  //      if (item <comparison> <termination-character>) break; [<comparison>
  //      can be <, <=, >, >=] out[i] = item;
  //      }
  // BBStart <load-char>
  // istore #transchar (store translated character)
  ...
  if (((firstNum == 3 || firstNum == 4 || firstNum == 5) && secondNum == 0) ||
      ...
      ((firstNum == 3 || firstNum == 2) && secondNum == 1 && thirdNum == 2 &&
       fourthNum == 2)) {
    ; // this is ok - matches one of our loop forms
  } else {
    dumpOptDetails(comp(),
                   "Loop has wrong number of tree tops (%d,%d,%d,%d) - no "
                   "arraytranslate reduction\n",
  ...
  if (hasBreak) {
    ...
    if (hasBranch) {
      ...
      storeNode = (storeTree->getNode()->getNumChildren() == 2)
                      ? storeTree->getNode()
                      : NULL;
      loadTree = (storeNode != NULL && storeNode->getNumChildren() == 2)
                     ? storeTree
                     : NULL;
  ...
  if (!nextBlock) {
    dumpOptDetails(
        comp(),
        "Loop exit block is method exit - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (!arraytranslateLoop.checkLoad(loadNode)) {
    dumpOptDetails(
        comp(), "Loop does not have load tree - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (arraytranslateLoop.compilerGeneratedTable() && !hasBranch && hasBreak) {
    dumpOptDetails(comp(),
                   "Attempt to reduce loop marked with no branch but also comp "
                   "gen'ed table - no arraytranslate reduction\n");
    return false;
  }

  // if hasBreak and hasBranch are both false, the loop is a simple byte->char
  // or char->byte conversion the termChar can be ignored by the instruction if
  // only the hardware supports it. if there is no support for ignoring the
  // termChar, then char->byte conversions cannot be reduced (since the termChar
  // is treated as byte from 0x00 - 0xFF, this effectively means there is no
  // good defaultValue that can be specified)
  //
  if (!hasBranch && !arraytranslateLoop.getByteInput() &&
      !cg()->getSupportsTestCharComparisonControl()) {
    dumpOptDetails(comp(), "Cannot reduce char->byte loops since hardware does "
                           "not support termination character control\n");
    return false;
  }

  if (!arraytranslateLoop.getStoreAddress()->checkIndVarStore(
          indVarStoreNode)) {
    dumpOptDetails(
        comp(),
        "Loop does not have indvar tree - no arraytranslate reduction\n");
    return false;
  }

  if (mayHaveMaterializedIV && !arraytranslateLoop.checkMatIndVarStore(
                                   matIndVarStoreNode, indVarStoreNode)) {
    dumpOptDetails(comp(), "Loop does not match materialized indvar "
                           "requirements - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (!arraytranslateLoop.checkStore(storeNode)) {
    dumpOptDetails(
        comp(),
        "Loop does not have store tree - no arraytranslate reduction\n");
    return false;
  }

  // check for src & dest arrays being different
  // if they are same, the range of indices may overlap. in this case,
  // the result of the arrayTranslate instruction in the hardware is
  // unpredictable
  //
  ...
  if (inputBase && outputBase) {
    ...
    if ((inputBase == outputBase) ||
        (inputBase->getOpCode().hasSymbolReference() &&
         outputBase->getOpCode().hasSymbolReference() &&
         inputBase->getSymbolReference()->getSymbol() ==
             outputBase->getSymbolReference()->getSymbol())) {
      dumpOptDetails(
          comp(),
          "Loop has same src and dst arrays -- no arraytranslate reduction\n");
      return false;
    }
  }

  if (hasBranch) {
    if (!arraytranslateLoop.checkBreak(storeBlock, branchAroundNode,
                                       nextBlock)) {
      dumpOptDetails(comp(), "Loop does not have branch-around tree - no "
                             "arraytranslate reduction\n");
      return false;
    }
  } else {
    if (arraytranslateLoop.getTableNode() == NULL) {
      dumpOptDetails(comp(), "Loop does not have a translation table - no "
                             "arraytranslate reduction\n");
      return false;
    }
  }
  ...
  if (!arraytranslateLoop.compilerGeneratedTable() &&
      (arraytranslateLoop.getResultUnconvertedNode()->getDataType() !=
       storeNode->getDataType())) {
    dumpOptDetails(comp(), "Output array and translation table datatype "
                           "mismatch - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (!arraytranslateLoop.checkLoopCmp(loopCmpNode, indVarStoreNode, indVar)) {
    dumpOptDetails(
        comp(),
        "Loop does not have loopcmp tree - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (!arraytranslateLoop.forwardLoop()) {
    dumpOptDetails(
        comp(),
        "Loop does not have forward loop - no arraytranslate reduction\n");
    return false;
  }
  ...
  // ificmpne --> block <original-code>  *** this is only generated for s2b/s2s
  // if required ***
  //  iand
  //     <table address>
  ...
  // establish new 'load' tree and block and connect it up to the old load/break
  // code
  ...
  TR::Block *newLoadBlock = TR::Block::createEmptyBlock(
      loadNode, comp(), firstBlock->getFrequency(), firstBlock);
  ...
  TR::Node *hardwareMinimumNode =
      TR::Node::create(loadNode, TR::iconst, 0, minimumSize);
  TR::Node *finalNode = arraytranslateLoop.getFinalNode()->duplicateTree();
  TR::Node *compareNode = TR::Node::createif(TR::ificmple, finalNode,
                                             hardwareMinimumNode, newLoadEntry);
  TR::TreeTop *branchNewOldTreeTop =
      loadTree; // necessary to keep old edges consistent
  ...
  TR::Block *translateBlock = TR::Block::createEmptyBlock(
      loadNode, comp(), firstBlock->getFrequency(), firstBlock);
  ...
  /* this is what is going to happen if hasBreak is true
     ...
     What does this mean?
     a) if the entire array was successfully translated, the return value is
     length-1. So the exit values of
        --> i must be: i = length
        --> c must be: c = charA[i-1]

     b) if the break condition was triggered (entire array not traversed), the
     return value is the index of the last element translated (call it j). So
     exit values of
        --> i must be: i = j
        --> c must be: c = charA[i]
     ...
     This means that the loop should look like:
         ...
         if (i < length) {         // lengthTestTree
            c = charA[i]           // load the element read at index i
     (loadElementBlock)
         }
         else {
            c = charA[i-1]         // load the last element
     (loadLastElementBlock)
         }
         if (c > termChar) exit    // check for the termChar (originalbreak
     condition) (matchBlock) goto normal_loop_exit     // gotoBlock
  */

  TR::Block *loadElementBlock = NULL;
  TR::Block *loadLastElementBlock = NULL;
  if (hasBreak) {
    loadElementBlock = TR::Block::createEmptyBlock(
        loadNode, comp(), firstBlock->getFrequency(), firstBlock);
    loadLastElementBlock = TR::Block::createEmptyBlock(
        loadNode, comp(), firstBlock->getFrequency(), firstBlock);
  ...
  int32_t mask = cg()->arrayTranslateTableRequiresAlignment(
      arraytranslateLoop.getByteInput(), arraytranslateLoop.getByteOutput());
  if (mask &&
      !arraytranslateLoop.compilerGeneratedTable()) // compiler-generated tables
                                                    // aligned at allocation
  {
    alignmentBlock = TR::Block::createEmptyBlock(
        loadNode, comp(), firstBlock->getFrequency(), firstBlock);
    ...
    if (arraytranslateLoop.tableBackedByRawStorage()) {
      if (TR::Compiler->target.is64Bit()) {
        ...
        TR::Node *andNode =
            TR::Node::create(TR::land, 2, dupTableNode, pageNode);

        compareNode =
            TR::Node::createif(TR::iflcmpne, zeroNode, andNode, newLoadEntry);
      } else {
        ...
        TR::Node *andNode =
            TR::Node::create(TR::iand, 2, dupTableNode, pageNode);

        compareNode =
            TR::Node::createif(TR::ificmpne, zeroNode, andNode, newLoadEntry);
      }
    } else {
      TR::Node *zeroNode = TR::Node::create(loadNode, TR::iconst, 0, 0);
      TR::Node *pageNode = TR::Node::create(loadNode, TR::iconst, 0, mask);
      ...
      TR::Node *hdrSizeNode =
          TR::Node::create(loadNode, TR::iconst, 0,
                           TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
      TR::Node *addNode =
          TR::Node::create(TR::iadd, 2, dupTableNode, hdrSizeNode);
  ...
  TR::Node *termCharNode =
      arraytranslateLoop.getTermCharNode()->duplicateTree();
  ...
  TR::Node *stoppingNode =
      TR::Node::create(loadNode, TR::iconst, 0, 0xffffffff);

  TR::Node *translateNode = TR::Node::create(loadNode, TR::arraytranslate, 6);
  translateNode->setSymbolReference(
      comp()->getSymRefTab()->findOrCreateArrayTranslateSymbol());
  ...
  TR::SymbolReference *indVarSymRef =
      arraytranslateLoop.getStoreAddress()->getIndVarSymRef();
  TR::Node *indVarLengthNode =
      arraytranslateLoop.getStoreAddress()->getIndVarLoad()->duplicateTree();
  TR::Node *lengthNode =
      TR::Node::create(TR::isub, 2, dupFinalNode, indVarLengthNode);

  translateNode->setAndIncChild(4, lengthNode);
  translateNode->setAndIncChild(5, stoppingNode);

  translateNode->setSourceIsByteArrayTranslate(
      arraytranslateLoop.getByteInput());
  translateNode->setTargetIsByteArrayTranslate(
  ...
  translateNode->setTableBackedByRawStorage(
      arraytranslateLoop.tableBackedByRawStorage() ||
      arraytranslateLoop.compilerGeneratedTable());

  TR::SymbolReference *translateTemp = comp()->getSymRefTab()->createTemporary(
      comp()->getMethodSymbol(), TR::Int32);
  TR::Node *storeTranslateNode =
      TR::Node::createStore(translateTemp, translateNode);
  TR::TreeTop *translateTreeTop =
      TR::TreeTop::create(comp(), storeTranslateNode);

  // add number of elements translated to induction variable total

  TR::Node *indVarValueNode =
      arraytranslateLoop.getStoreAddress()->getIndVarLoad()->duplicateTree();
  TR::Node *addCountNode =
      TR::Node::create(TR::iadd, 2, indVarValueNode, translateNode);

  TR::Node *indVarUpdateNode =
      TR::Node::createWithSymRef(TR::istore, 1, 1, addCountNode, indVarSymRef);

  TR::TreeTop *indVarUpdateTreeTop =
      TR::TreeTop::create(comp(), indVarUpdateNode);
  ...
  //
  TR::TreeTop *indVarTreeTop =
      indVarUpdateTreeTop; // TR::TreeTop::create(comp(), indVarDupNode);
  ...
  if (hasBranch) {
    ...
    if (!hasBreak) {
      ...
      TR::Node *icmpNode =
          TR::Node::create(TR::icmpeq, 2, iNode,
                           arraytranslateLoop.getFinalNode()->duplicateTree());
      TR::Node *decNode = TR::Node::create(TR::isub, 2, iNode, icmpNode);
      vcount_t visitCount = comp()->incVisitCount();
      replaceInductionVariable(
          NULL, resCharDupNode->getFirstChild()->skipConversions(), -1,
    ...
    if (mayHaveMaterializedIV) {
      ...
      // the tree generated here is the dup of the matIV store ie. matIV =
      // primaryIV + delta + 1 so subtract by 1 here to get the right final
      // value
      //
      ...
      TR::Node *decNode = TR::Node::create(
          TR::isub, 2, firstChild,
          TR::Node::create(updatedMatIndVar, TR::iconst, 0, 1));
  ...
  if (hasBreak) {
    ...
    // setup matchBlock
    //
    matchBlock = TR::Block::createEmptyBlock(
        loadNode, comp(), firstBlock->getFrequency(), firstBlock);
    _cfg->addNode(matchBlock);
    TR::Node *matchBreakNode = branchAroundNode->duplicateTree();
    TR::Node *matchResultNode = TR::Node::createLoad(
        branchAroundNode,
        arraytranslateLoop.getResultNode()->getSymbolReference());
    ...
    /// lengthTestNode = TR::Node::create(TR::iadd, 2, lengthTestNode,
    /// TR::Node::create(loadNode, TR::iconst, 0, 1));
    TR::Node *dupFinalNode = arraytranslateLoop.getFinalNode()->duplicateTree();
    TR::TreeTop *lengthTestTree = TR::TreeTop::create(
        comp(), TR::Node::createif(TR::ificmplt, lengthTestNode, dupFinalNode,
                                   loadElementBlock->getEntry()));
    ...
    //
    for (; !ivLoadNode->getOpCode().isArrayRef() &&
           ivLoadNode->getNumChildren() >= 1;
    ...
    if (ivLoadNode->getOpCode().isArrayRef()) {
      vcount_t visitCount = comp()->incVisitCount();
      TR::Node *decNode = TR::Node::create(
          TR::isub, 2, TR::Node::createLoad(loadNode, indVarSymRef),
          TR::Node::iconst(loadNode, 1));
      replaceInductionVariable(NULL, ivLoadNode, -1,
                               indVarSymRef->getReferenceNumber(), decNode,
                               visitCount);
    }
    TR::TreeTop *dupResCharTreeTop =
        TR::TreeTop::create(comp(), dupResCharNode);
    loadLastElementBlock->getEntry()->join(dupResCharTreeTop);
    TR::TreeTop *gotoMatchTree =
        TR::TreeTop::create(comp(), TR::Node::create(loadNode, TR::Goto, 0,
                                                     matchBlock->getEntry()));
  ...
  if (alignmentBlock) {
    ...
    _cfg->addEdge(
        TR::CFGEdge::createEdge(loadBlock, alignmentBlock, trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(alignmentBlock, newLoadBlock, trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(alignmentBlock, translateBlock, trMemory()));
  } else {
    _cfg->addEdge(TR::CFGEdge::createEdge(loadBlock, newLoadBlock, trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(loadBlock, translateBlock, trMemory()));
  }
  ...
  if (hasBreak)
    ...
    _cfg->addEdge(
        TR::CFGEdge::createEdge(translateBlock, gotoBlock, trMemory()));
  _cfg->addEdge(TR::CFGEdge::createEdge(gotoBlock, nextBlock, trMemory()));

  if (hasBreak) {
    _cfg->addEdge(
        TR::CFGEdge::createEdge(translateBlock, loadElementBlock, trMemory()));
    _cfg->addEdge(TR::CFGEdge::createEdge(translateBlock, loadLastElementBlock,
                                          trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(loadElementBlock, matchBlock, trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(loadLastElementBlock, matchBlock, trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(matchBlock, breakExitBlock, trMemory()));

    _cfg->addEdge(
        TR::CFGEdge::createEdge(newLoadBlock, storeBlock, trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(newLoadBlock, breakExitBlock, trMemory()));
    _cfg->addEdge(
        TR::CFGEdge::createEdge(storeBlock, newLoadBlock, trMemory()));
  } else {
    if (hasBranch) {
      _cfg->addEdge(
          TR::CFGEdge::createEdge(newLoadBlock, storeBlock, trMemory()));
      _cfg->addEdge(
          TR::CFGEdge::createEdge(newLoadBlock, otherStoreBlock, trMemory()));
      _cfg->addEdge(
          TR::CFGEdge::createEdge(cmpBlock, newLoadBlock, trMemory()));
    } else {
      _cfg->addEdge(
          TR::CFGEdge::createEdge(newLoadBlock, newLoadBlock, trMemory()));
      _cfg->addEdge(
          TR::CFGEdge::createEdge(newLoadBlock, nextBlock, trMemory()));
    }
  }
  ...
  if (hasBranch) {
    ...
    _cfg->removeEdge(loadSuccList, loadBlock->getNumber(),
                     storeBlock->getNumber());

    if (hasBreak) {
      _cfg->removeEdge(loadSuccList, loadBlock->getNumber(),
                       breakExitBlock->getNumber());
    } else {
      _cfg->removeEdge(loadSuccList, loadBlock->getNumber(),
                       otherStoreBlock->getNumber());
      _cfg->removeEdge(loadSuccList, loadBlock->getNumber(),
                       storeBlock->getNumber());
      _cfg->removeEdge(storeSuccList, storeBlock->getNumber(),
                       loadBlock->getNumber());

      _cfg->removeEdge(cmpSuccList, cmpBlock->getNumber(),
                       loadBlock->getNumber());
    }
...
bool TR_ArraytranslateAndTest::checkLoad(TR::Block *loadBlock,
                                         TR::Node *loadNode) {
  if (loadNode->getOpCodeValue() != TR::ifbcmpeq &&
      loadNode->getOpCodeValue() != TR::ificmpeq) {
    dumpOptDetails(comp(), "...load tree does not have ifbcmpeq/ificmpeq - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (loadNode->getOpCodeValue() == TR::ificmpeq) {
    if (loadNode->getFirstChild()->getOpCodeValue() != TR::b2i) {
      dumpOptDetails(comp(), "...load tree has ificmpeq but no widening from "
                             "byte - no arraytranslateAndTest reduction\n");
      return false;
    } else {
  ...
  if (bLoadNode->getOpCodeValue() != TR::bloadi) {
    dumpOptDetails(comp(), "...load tree does not have ibload - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (aiaddNode->getOpCodeValue() != TR::aiadd &&
      aiaddNode->getOpCodeValue() != TR::aladd) {
    dumpOptDetails(comp(), "...load tree does not have aiadd/aladd - no "
                           "arraytranslate reduction\n");
    return false;
  }
  ...
  if (aiaddFirstChild->getOpCodeValue() != TR::aload &&
      aiaddFirstChild->getOpCodeValue() != TR::aloadi) {
    dumpOptDetails(comp(), "...load tree does not have aload - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }

  TR::Node *cmpVal = loadNode->getSecondChild();
  if (cmpVal->getOpCodeValue() != TR::bconst &&
      cmpVal->getOpCodeValue() != TR::iconst) {
    dumpOptDetails(comp(), "...load tree does not have bconst/iconst - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }
...
bool TR_ArraytranslateAndTest::checkFrequency(TR::CodeGenerator *cg,
                                              TR::Block *loadBlock,
                                              TR::Node *loadNode) {
  int16_t loopHeaderFrequency = loadBlock->getFrequency();
  int16_t fallThroughFrequency = loadBlock->getNextBlock()->getFrequency();
  int16_t breakFrequency =
      loadNode->getBranchDestination()->getEnclosingBlock()->getFrequency();

  if (((fallThroughFrequency <= 0) && ((loopHeaderFrequency > 0))) ||
      (fallThroughFrequency <
       breakFrequency * cg->arrayTranslateAndTestMinimumNumberOfIterations())) {
    return false;
  }
  return true;
}
...
bool TR_LoopReducer::generateArraytranslateAndTest(
    TR_RegionStructure *whileLoop, TR_InductionVariable *indVar,
    TR::Block *firstBlock, TR::Block *secondBlock) {
  if (!cg()->getSupportsArrayTranslateAndTest()) {
    dumpOptDetails(comp(),
                   "arrayTranslateAndTest not enabled for this platform\n");
    return false;
  }
  ...
  if (firstNum != 1 || (secondNum != 2 && secondNum != 3)) {
    dumpOptDetails(comp(),
                   "Loop has wrong number of tree tops (%d,%d) - no "
                   "arraytranslateAndTest reduction\n",
                   firstNum, secondNum);
    return false;
  }
  ...
  if (secondNum == 2) {
    ...
    if (!asyncFound) {
      dumpOptDetails(comp(),
                     "Loop has wrong number of tree tops for no async-check "
                     "(%d,%d) - no arraytranslateAndTest reduction\n",
                     firstNum, secondNum);
      return false;
    }
  ...
  nextBlock = (nextTree) ? nextTree->getEnclosingBlock() : NULL;
  if (!nextBlock) {
    dumpOptDetails(comp(), "Loop exit block is method exit - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (!arraytranslateAndTestLoop.checkLoad(loadBlock, loadNode)) {
    dumpOptDetails(
        comp(),
        "Loop does not have load tree - no arraytranslateAndTest reduction\n");
    return false;
  }

  if (!arraytranslateAndTestLoop.getStoreAddress()->checkIndVarStore(
          indVarStoreNode)) {
    dumpOptDetails(comp(), "Loop does not have indvar tree - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }

  if (!arraytranslateAndTestLoop.checkLoopCmp(loopCmpNode, indVarStoreNode,
                                              indVar)) {
    dumpOptDetails(comp(), "Loop does not have loopcmp tree - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }

  if (!arraytranslateAndTestLoop.forwardLoop()) {
    dumpOptDetails(comp(), "Loop does not have forward loop - no "
                           "arraytranslateAndTest reduction\n");
    return false;
  }

  if (!arraytranslateAndTestLoop.checkFrequency(cg(), loadBlock, loadNode)) {
    dumpOptDetails(comp(), "Loop frequency on fall-through not high enough - "
                           "no arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  // reduce the loop to a call to arrayTranslateAndTest, returning number of
  // bytes translated, stored into indVar
  TR::Node *termCharNode =
      arraytranslateAndTestLoop.getTermCharNode()->duplicateTree();
  TR::Node *inputNode =
      arraytranslateAndTestLoop.getInputNode()->duplicateTree();
  TR::Node *finalNode = arraytranslateAndTestLoop.updateIndVarStore(
      arraytranslateAndTestLoop.getIndVarNode(), indVarStoreNode,
      arraytranslateAndTestLoop.getFirstAddress());

  TR::Node *translateNode =
      TR::Node::create(loadNode, TR::arraytranslateAndTest, 3);
  translateNode->setSymbolReference(
      comp()->getSymRefTab()->findOrCreateArrayTranslateAndTestSymbol());
  ...
  TR::SymbolReference *indVarSymRef =
      arraytranslateAndTestLoop.getStoreAddress()->getIndVarSymRef();
  TR::Node *indVarLengthNode = arraytranslateAndTestLoop.getStoreAddress()
                                   ->getIndVarLoad()
                                   ->duplicateTree();
  TR::Node *addNode = TR::Node::create(
      TR::iadd, 2, translateNode, TR::Node::createLoad(loadNode, indVarSymRef));
  TR::Node *storeToIndVar =
      TR::Node::createWithSymRef(TR::istore, 1, 1, addNode, indVarSymRef);
  ...
  if (cmpTargetBlockNumber != nextBlock->getNumber()) {
    TR::Node *compareNode = TR::Node::createif(
        TR::ificmpne,
        arraytranslateAndTestLoop.getStoreAddress()
            ->getIndVarLoad()
            ->duplicateTree(),
        arraytranslateAndTestLoop.getFinalNode()->duplicateTree(),
        cmpTargetTree);
    TR::TreeTop *compareTreeTop = TR::TreeTop::create(comp(), compareNode);
  ...
  _cfg->removeEdge(loadBlock->getSuccessors(), loadBlock->getNumber(),
                   cmpBlock->getNumber());
  _cfg->removeEdge(cmpBlock->getSuccessors(), cmpBlock->getNumber(),
                   nextBlock->getNumber());
  return true;
}

// The following is the type of loop that needs to be reduced to an arraycopy
// (byte to char arraycopy)
//
...
bool TR_LoopReducer::generateByteToCharArraycopy(
    TR_InductionVariable *byteIndVar, TR_InductionVariable *charIndVar,
    TR::Block *loopHeader) {
  if (!comp()->cg()->getSupportsReferenceArrayCopy() &&
      !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
    dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
  ...
  {
    dumpOptDetails(comp(),
                   "Loop does not have an increment of +/-1 and +/-2, but "
                   "instead %d and %d - no byte to char arraycopy reduction\n",
                   incrA, incrB);
    return false;
  }
  ...
  if (loopHeader->getNumberOfRealTreeTops() !=
      4) // array store, induction variable store, induction variable store,
         // loop comparison
  {
    dumpOptDetails(
        comp(), "Loop has %d tree tops - no byte to char arraycopy reduction\n",
        loopHeader->getNumberOfRealTreeTops());
    return false;
  }
  ...
  TR_ByteToCharArraycopy arraycopyLoop(comp(), charIndVar, byteIndVar,
                                       TR::Compiler->target.cpu.isBigEndian());
  ...
  if (!arraycopyLoop.checkByteLoads(storeNode->getSecondChild())) {
    dumpOptDetails(comp(),
                   "second child of store does not look like OR of 2 byte "
                   "loads - no byte to char arraycopy performed\n");
    return false;
  }
  ...
  if (!arraycopyLoop.getStoreAddress()->checkIndVarStore(charIndVarStoreNode)) {
    bool swap = arraycopyLoop.getHighLoadAddress()->checkIndVarStore(
        charIndVarStoreNode);
    if (swap && arraycopyLoop.getStoreAddress()->checkIndVarStore(
                    byteIndVarStoreNode)) {
      ...
      dumpOptDetails(comp(),
                     "Did not encounter byte array induction variable "
                     "increment - no byte to char arraycopy performed\n");
      return false;
    }
  } else if (!arraycopyLoop.getHighLoadAddress()->checkIndVarStore(
                 byteIndVarStoreNode)) {
    dumpOptDetails(comp(), "Did not encounter char array induction variable "
                           "increment - no byte to char arraycopy performed\n");
    return false;
  }

  if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode,
                                  charIndVar)) {
    // For now, we require that the comparison is for the character array
    // induction variable. We should make this work if it was comparing the byte
    // array induction variable as an enhancement, e.g. for (byteInd=start;
    // byteInd<end; byteInd+=2)
    //   { ... }
    dumpOptDetails(comp(),
                   "Loop comparison does not match byte or char induction "
                   "variable - no byte to char arraycopy performed\n");
    return false;
  }
  ...
  // In addition to the arraycopy we generate, we also will need to generate an
  // update of the second induction variable - this will require a new node.
  // There is no 'final' value as in the case of the primary induction variable,
  // so the final value will have to be calculated. Ignoring the fact that they
  // could have different starting values, the primary induction variable is
  // either twice the value of the secondary induction variable, or vice versa.
  // The final value of the secondary induction variable is thus (primaryEnd -
  // primaryStart) <operator> 2, where <operator> may be multiplication or
  // division.

  // For details on arraycopy call to be generated, see generateArrayCopy. The
  // core tree is repeated here for convenience. Tree-Top
  // ArrayCopy call
  //     ArrayStoreBase tree
  //     ArrayLoadBase tree
  //     number of bytes to copy
  // first induction variable update tree (this tree is new - see note above)
  // second induction variable update tree (this re-uses comparison node from
  // original tree)

  // First, set the increment for the byte array addresses to 1 so that the code
  // doesn't insert a multiplier in to the base address calculation of the src
  // node on the arraycopy.
  arraycopyLoop.getHighLoadAddress()->setIncrement(1);
  arraycopyLoop.getLowLoadAddress()->setIncrement(1);

  arraycopyLoop.getStoreAddress()->updateAiaddSubTree(
      arraycopyLoop.getStoreIndVarNode(), &arraycopyLoop);
  arraycopyLoop.getHighLoadAddress()->updateAiaddSubTree(
      arraycopyLoop.getHighLoadIndVarNode(), &arraycopyLoop);
  TR::Node *charImul = arraycopyLoop.updateIndVarStore(
      arraycopyLoop.getStoreIndVarNode(), charIndVarStoreNode,
      arraycopyLoop.getStoreAddress());
  TR::Node *byteImul = arraycopyLoop.updateIndVarStore(
      arraycopyLoop.getHighLoadIndVarNode(), byteIndVarStoreNode,
      arraycopyLoop.getHighLoadAddress());
  arraycopyLoop.getStoreAddress()->updateMultiply(
      arraycopyLoop.getStoreMultiplyNode());
  arraycopyLoop.getHighLoadAddress()->updateMultiply(
      arraycopyLoop.getHighLoadMultiplyNode());

  TR::Node *loadAddr = arraycopyLoop.getHighLoadAddress()->getRootNode();
  TR::Node *storeAddr = arraycopyLoop.getStoreAddress()->getRootNode();
  TR::Node *arraycopy =
      TR::Node::createArraycopy(loadAddr, storeAddr, charImul->duplicateTree());

  loadAddr->decReferenceCount();
  storeAddr->decReferenceCount();
  arraycopyLoop.getLowLoadAddress()
      ->getRootNode()
      ->recursivelyDecReferenceCount();

  arraycopy->setSymbolReference(
      comp()->getSymRefTab()->findOrCreateArrayCopySymbol());
  arraycopy->setForwardArrayCopy(true);
  ...
  TR::TreeTop *deadCmpChild1 =
      TR::TreeTop::create(comp(), lastIndVarStoreTree, loopCmpNode);
  TR::TreeTop *deadCmpChild2 =
      TR::TreeTop::create(comp(), deadCmpChild1, loopCmpNode);

  deadStore->setNode(
      TR::Node::create(TR::treetop, 1, byteIndVarStoreNode->getFirstChild()));
  deadCmpChild1->setNode(TR::Node::create(
      TR::treetop, 1, loopCmpTree->getNode()->getFirstChild()));
  deadCmpChild2->setNode(TR::Node::create(
      TR::treetop, 1, loopCmpTree->getNode()->getSecondChild()));
  ...
  TR::SymbolReference *charIndVarTemp = comp()->getSymRefTab()->createTemporary(
      comp()->getMethodSymbol(), TR::Int32);
  TR::Node *charIndVarTempLoadNode = TR::Node::createLoad(
      storeAddr, arraycopyLoop.getStoreAddress()->getIndVarSymRef());
  TR::Node *charIndVarTempStoreNode =
      TR::Node::createStore(charIndVarTemp, charIndVarTempLoadNode);

  TR::TreeTop *charIndVarTempStoreTree =
      TR::TreeTop::create(comp(), charIndVarTempStoreNode);
  arrayStoreTree->insertAfter(charIndVarTempStoreTree);

  TR::Node *origTempReLoadNode =
      TR::Node::createLoad(storeAddr, charIndVarTemp);
  TR::Node *charIndVarReLoadNode = TR::Node::createLoad(
      storeAddr, arraycopyLoop.getStoreAddress()->getIndVarSymRef());
  TR::Node *subNode =
      TR::Node::create(TR::isub, 2, charIndVarReLoadNode, origTempReLoadNode);
  ...
  TR::Node *byteIndVarReLoadNode = TR::Node::createLoad(
      storeAddr, arraycopyLoop.getHighLoadAddress()->getIndVarSymRef());
  TR::Node *addNode =
      TR::Node::create(TR::iadd, 2, byteIndVarReLoadNode, mulNode);
  TR::Node *byteIndVarReStoreNode = TR::Node::createStore(
      arraycopyLoop.getHighLoadAddress()->getIndVarSymRef(), addNode);

  TR::TreeTop *byteIndVarReStoreTree =
      TR::TreeTop::create(comp(), byteIndVarReStoreNode);
...
bool TR_LoopReducer::generateCharToByteArraycopy(
  ...
  if (loopHeader->getNumberOfRealTreeTops() !=
      5) // array store low byte, array store high byte, induction variable
         // store, induction variable store, loop comparison
  {
    dumpOptDetails(
        comp(), "Loop has %d tree tops - no char to byte arraycopy reduction\n",
        loopHeader->getNumberOfRealTreeTops());
    return false;
  }
  ...
  TR_CharToByteArraycopy arraycopyLoop(comp(), charIndVar, byteIndVar,
                                       TR::Compiler->target.cpu.isBigEndian());

  if (!arraycopyLoop.checkArrayStores(highStoreNode, lowStoreNode)) {
    dumpOptDetails(comp(), "... no match - switch around high and low array "
                           "store trees and try again...\n");
  ...
  if (!arraycopyLoop.getLoadAddress()->checkIndVarStore(charIndVarStoreNode)) {
    bool swap = arraycopyLoop.getHighStoreAddress()->checkIndVarStore(
        charIndVarStoreNode);
    if (swap &&
        arraycopyLoop.getLoadAddress()->checkIndVarStore(byteIndVarStoreNode)) {
      dumpOptDetails(comp(), "try swapping the 2 induction variables\n");
      TR::TreeTop *tmpTree = byteIndVarStoreTree;
      TR::Node *tmpNode = byteIndVarStoreNode;
      byteIndVarStoreTree = charIndVarStoreTree;
      ...
      dumpOptDetails(comp(),
                     "Did not encounter byte array induction variable "
                     "increment - no char to byte arraycopy performed\n");
      return false;
    }
  } else if (!arraycopyLoop.getHighStoreAddress()->checkIndVarStore(
                 byteIndVarStoreNode)) {
    dumpOptDetails(comp(), "Did not encounter char array induction variable "
                           "increment - no char to byte arraycopy performed\n");
    return false;
  ...
  if (!arraycopyLoop.checkLoopCmp(loopCmpNode, charIndVarStoreNode,
                                  charIndVar)) {
    // msf - for now, we require that the comparison is for the character array
    // induction variable. We should make this work if it was comparing the byte
    // array induction variable as an enhancement, e.g. for (byteInd=start;
    // byteInd<end; byteInd+=2)
    //   { ... }
    dumpOptDetails(comp(),
                   "Loop comparison does not match byte or char induction "
                   "variable - no char to byte arraycopy performed\n");
    return false;
  }
  ...
  // First, set the increment for the byte array address to 1 so that the code
  // doesn't insert a multiplier in to the base address calculation of the src
  // node on the arraycopy.
  arraycopyLoop.getHighStoreAddress()->setIncrement(1);

  arraycopyLoop.getHighStoreAddress()->updateAiaddSubTree(
      arraycopyLoop.getHighStoreIndVarNode(), &arraycopyLoop);
  arraycopyLoop.getLoadAddress()->updateAiaddSubTree(
      arraycopyLoop.getLoadIndVarNode(), &arraycopyLoop);
  TR::Node *charImul = arraycopyLoop.updateIndVarStore(
      arraycopyLoop.getLoadIndVarNode(), charIndVarStoreNode,
      arraycopyLoop.getLoadAddress());
  TR::Node *byteImul = arraycopyLoop.updateIndVarStore(
      arraycopyLoop.getHighStoreIndVarNode(), byteIndVarStoreNode,
      arraycopyLoop.getHighStoreAddress());
  arraycopyLoop.getHighStoreAddress()->updateMultiply(
      arraycopyLoop.getHighStoreMultiplyNode());
  arraycopyLoop.getLoadAddress()->updateMultiply(
      arraycopyLoop.getLoadMultiplyNode());

  TR::Node *loadAddr =
      arraycopyLoop.getLoadAddress()->getRootNode()->duplicateTree();
  TR::Node *storeAddr =
      arraycopyLoop.getHighStoreAddress()->getRootNode()->duplicateTree();
  ...
  TR::TreeTop *treeList[] = {byteIndVarStoreTree, charIndVarStoreTree,
                             highArrayStoreTree,  lowArrayStoreTree,
                             loopCmpTree,         NULL};

  TR::TreeTop::removeDeadTrees(
      comp(), treeList); // make these trees into nops, but leave them around to
                         // keep ref counts right

  TR::SymbolReference *charIndVarTemp = comp()->getSymRefTab()->createTemporary(
      comp()->getMethodSymbol(), TR::Int32);
  TR::Node *charIndVarTempLoadNode = TR::Node::createLoad(
      storeAddr, arraycopyLoop.getLoadAddress()->getIndVarSymRef());
  TR::Node *charIndVarTempStoreNode =
      TR::Node::createStore(charIndVarTemp, charIndVarTempLoadNode);
  ...
  // following line is a bit messy - need to skip over the multiply-by-2, since
  // want an unstrided value for ind var
  TR::Node *charIndVarReStoreNode =
      TR::Node::createStore(arraycopyLoop.getLoadAddress()->getIndVarSymRef(),
                            charImul->getFirstChild()->duplicateTree());
  TR::TreeTop *charIndVarReStoreTree =
      TR::TreeTop::create(comp(), charIndVarReStoreNode);
  ...
  TR::Node *byteIndVarReLoadNode = TR::Node::createLoad(
      storeAddr, arraycopyLoop.getHighStoreAddress()->getIndVarSymRef());
  TR::Node *addNode =
      TR::Node::create(TR::iadd, 2, byteIndVarReLoadNode, mulNode);
  TR::Node *byteIndVarReStoreNode = TR::Node::createStore(
      arraycopyLoop.getHighStoreAddress()->getIndVarSymRef(), addNode);
...
bool TR_LoopReducer::constrainedIndVar(TR_InductionVariable *indVar) {
  ...
  if (!indVar->getIncr()->asIntConst() && !indVar->getIncr()->asLongConst()) {
    dumpOptDetails(comp(),
                   "Loop has non-constant induction variable increment\n");
    return false;
  }
  ...
  default:
    dumpOptDetails(
        comp(), "Loop has constant induction variable other than +/-1/2/4/8\n");
    return false;
  }
  return true;
}

void TR_LoopReducer::removeSelfEdge(TR::CFGEdgeList &succList,
                                    int32_t selfNumber) {
  _cfg->removeEdge(succList, selfNumber, selfNumber);
}

int TR_LoopReducer::addBlock(TR::Block *newBlock, TR::Block **blockList,
                             int numBlocks, const int maxNumBlocks) {
  if (numBlocks > maxNumBlocks) {
    dumpOptDetails(comp(),
                   "Loop has more than 4 blocks. Punting after block:%d\n",
                   newBlock->getNumber());
  } else {
...
int TR_LoopReducer::addRegionBlocks(TR_RegionStructure *region,
                                    TR::Block **blockList, int numBlocks,
  ...
  for (node = it.getFirst(); node != NULL; node = it.getNext()) {
    TR_BlockStructure *sbs =
        node->getStructure() ? node->getStructure()->asBlock() : NULL;
    if (sbs) {
      numBlocks = addBlock(sbs->getBlock(), blockList, numBlocks, maxNumBlocks);
    } else {
      // numBlocks = addRegionBlocks(node->getStructure()->asRegion(),
      // blockList, numBlocks, maxNumBlocks);
...
bool TR_LoopReducer::mayNeedGlobalDeadStoreElimination(
    TR::Block *storeBlock, TR::Block *compareBlock) {
  ...
  if (storeNum != 3 || compareNum != 3) {
    ...
    // compareNum);
    return false; // for now, just the pattern for arraytranslate is what we are
                  // worried about
  }
  ...
  for (int b = 0; b < numBlocks; ++b) {
    ...
    for (int t = 0; t < numTreeTops; ++t) {
      ...
      if (node->getOpCodeValue() != pattern[b][t]) {
        ...
        // node->getOpCodeValue(), pattern[b][t]);
        return false;
      }
...
void TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure *whileLoop) {
  ...
  if (trace()) {
    dumpOptDetails(comp(), "Blocks in loop %p,%d ( ", loopHeader,
                   loopHeader->getNumber());
  ...
  //
  // note that the blocks are in backwards order (4,3,2) because they are popped
  // from the stack
  //
  if (nextIndVar) /* these reductions require 2 induction variables to be
                     considered */
  {
    if (((numBlocks == 1) &&
         generateByteToCharArraycopy(indVar, nextIndVar, loopHeader)) ||
        ((numBlocks == 1) &&
         generateCharToByteArraycopy(indVar, nextIndVar, loopHeader))) {
      removeSelfEdge(loopHeader->getSuccessors(), whileLoop->getNumber());
    } else {
      dumpOptDetails(comp(),
                     "Multiple Induction Variable loop %d has %d blocks and is "
                     "not reduced\n",
    ...
    if ((numBlocks == 2 /* || (numBlocks == 3 && block3->endsInGoto()) */) &&
      ...
      dumpOptDetails(comp(), "Loop matches possible arraytranslate - global "
                             "deadstore elimination to be performed\n");
    ...
    if (((numBlocks == 1) && generateArrayset(indVar, loopHeader)) ||
        ...
        ((numBlocks == 1) &&
         generateArraytranslate(whileLoop, indVar, loopHeader, NULL, NULL,
                                NULL)) ||
        (((numBlocks == 2) /* || (numBlocks == 3 && block3->endsInGoto()) */) &&
         generateArraytranslate(whileLoop, indVar, loopHeader, block2, NULL,
                                NULL)) ||
        ((numBlocks == 3) &&
         generateArraytranslate(whileLoop, indVar, loopHeader, block4, block3,
                                NULL)) ||
        ((numBlocks == 4) &&
         generateArraytranslate(whileLoop, indVar, loopHeader, block4, block3,
                                block2)) ||
        ((numBlocks == 2) && generateArraytranslateAndTest(
                                 whileLoop, indVar, loopHeader, block2))) {
      removeSelfEdge(loopHeader->getSuccessors(), whileLoop->getNumber());
    } else {
...
int32_t TR_LoopReducer::perform() {
  ...
  if (!comp()->cg()->getSupportsArraySet() &&
    ...
    dumpOptDetails(
        comp(), "No Loop Reduction Optimizations Enabled for this platform\n");
    return 0;
  }
  ...
  for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL;
    ...
    TR_ASSERT(naturalLoop && naturalLoop->isNaturalLoop(),
              "Loop reducer, expecting natural loop");
...
const char *TR_LoopReducer::optDetailString() const throw() {
