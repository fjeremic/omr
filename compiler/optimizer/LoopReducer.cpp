...
static TR::Node* testNode(TR::Compilation* comp,
                          TR::Node* node,
                          TR::ILOpCodes value,
                          const char* msg) {
...
static TR::Node* testUnary(TR::Compilation* comp,
                           TR::Node* parent,
                           TR::ILOpCodes value,
                           const char* msg) {
...
static TR::Node* testBinary(TR::Compilation* comp,
                            TR::Node* parent,
                            TR::ILOpCodes value,
                            TR::ILOpCodes leftChildValue,
                            TR::ILOpCodes rightChildValue,
                            const char* msg) {
...
static TR::Node* testBinaryIConst(TR::Compilation* comp,
                                  TR::Node* parent,
                                  TR::ILOpCodes value,
                                  TR::ILOpCodes leftChildValue,
                                  int32_t rightChildValue,
                                  const char* msg) {
  TR::Node* child = parent->getFirstChild();
  if (child->getOpCodeValue() != value ||
      child->getFirstChild()->getOpCodeValue() != leftChildValue ||
...
void TR_ArrayLoop::findIndVarLoads(TR::Node* node,
                                   TR::Node* indVarStoreNode,
                                   bool& storeFound,
                                   List<TR::Node>* ivLoads,
                                   TR::Symbol* ivSym,
                                   vcount_t visitCount) {
...
bool TR_Arraycmp::checkGoto(TR::Block* gotoBlock,
                            TR::Node* gotoNode,
                            TR::Node* finalNode) {
...
//
bool TR_Arraytranslate::checkBreak(TR::Block* breakBlock,
                                   TR::Node* breakNode,
                                   TR::Block* nextBlock) {
...
bool TR_Arraytranslate::checkGoto(TR::Block* gotoBlock,
                                  TR::Node* gotoNode,
                                  TR::Block* nextBlock) {
  TR::ILOpCodes gotoOpCode = gotoNode->getOpCode().getOpCodeValue();
...
bool TR_LoopReducer::blockInVersionedLoop(
    List<TR::CFGEdge> otherLoopVersionBlockList,
    TR::Block* block) {
...
bool TR_LoopReducer::replaceInductionVariable(TR::Node* parent,
                                              TR::Node* node,
                                              int32_t childNum,
...
bool TR_LoopReducer::generateByteToCharArraycopy(
    TR_InductionVariable* byteIndVar,
    TR_InductionVariable* charIndVar,
    TR::Block* loopHeader) {
  if (!comp()->cg()->getSupportsReferenceArrayCopy() &&
      !comp()->cg()->getSupportsPrimitiveArrayCopy()) {
    dumpOptDetails(comp(), "arraycopy not enabled for this platform\n");
...
int TR_LoopReducer::addBlock(TR::Block* newBlock,
                             TR::Block** blockList,
                             int numBlocks,
