...
static TR::Node*
testBinary(TR::Compilation* comp,
           TR::Node* parent,
           TR::ILOpCodes value,
...
static TR::Node*
testBinaryIConst(TR::Compilation* comp,
                 TR::Node* parent,
                 TR::ILOpCodes value,
                 TR::ILOpCodes leftChildValue,
...
TR_LRAddressTree::checkAiadd(TR::Node* aiaddNode, int32_t elementSize)
{
  ...
  if (_indVarNode.isNull() || (_indVarNode.getChild()
    ...
    dumpOptDetails(
      comp(), "checkAiadd: induction variable does not match index variable\n");
    return false;
  }
  ...
  if (storeFirstChild->getNumChildren() == 0 ||
      ...
      storeFirstChild->getFirstChild()
          ...
          ->getRegisterMappedSymbol() == loadFirstChild->getFirstChild()
                                           ...
                                           ->getRegisterMappedSymbol()) {
    dumpOptDetails(
      comp(), "arraycopy src and dst are against same object - punt for now\n");
    return false;
  }
...
TR_ByteToCharArraycopy::checkByteLoads(TR::Node* loadNodes)
{
  ...
  if (!checkHighLoad || !checkLowLoad) {
    dumpOptDetails(comp(),
                   "checkByteLoads: aiadd tree in error (%d,%d)\n",
                   checkHighLoad,
                   checkLowLoad);
    return false;
  }
...
bool
TR_CharToByteArraycopy::checkArrayStores(TR::Node* origHighStoreNode,
                                         TR::Node* origLowStoreNode)
{
  ...
  TR::Node* ishr = testNode(comp(),
                            origHighStoreNode->getSecondChild(),
                            TR::i2b,
  ...
  iand = testNode(comp(),
                  origLowStoreNode->getSecondChild(),
                  TR::i2b,
...
TR_LRAddressTree::checkIndVarStore(TR::Node* indVarNode)
{
  if (!indVarNode->getOpCode().isStoreDirect()) {
    dumpOptDetails(
      comp(), "induction variable tree does not have a direct store as root\n");
    return false;
  }
...
bool
TR_ArrayLoop::checkLoopCmp(TR::Node* loopCmpNode,
                           ...
                           TR_InductionVariable* indVar)
{
  ...
  if (opCodeCmpSecondChild != TR::iconst && opCodeCmpSecondChild != TR::iload &&
    ...
    dumpOptDetails(
      comp(),
      "loop compare does not have iconst/iload/arraylength as second child\n");
    return false;
  }
...
int32_t
TR_ArrayLoop::checkForPostIncrement(TR::Block* loopHeader,
                                    TR::Node* indVarStoreNode,
                                    TR::Node* loopCmpNode,
                                    TR::Symbol* ivSym)
{
  ...
  for (TR::TreeTop* tt = startTree; !storeFound && tt != loopHeader->getExit();
    ...
    findIndVarLoads(
      tt->getNode(), indVarStoreNode, storeFound, &ivLoads, ivSym, visitCount);
...
void
TR_ArrayLoop::findIndVarLoads(TR::Node* node,
                              TR::Node* indVarStoreNode,
                              bool& storeFound,
                              ...
                              vcount_t visitCount)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++)
    findIndVarLoads(node->getChild(i),
                    indVarStoreNode,
                    storeFound,
                    ivLoads,
                    ivSym,
...
TR::Node*
TR_ArrayLoop::updateIndVarStore(TR_ParentOfChildNode* indVarNode,
                                TR::Node* indVarStoreNode,
                                ...
                                int32_t postIncrement)
{
  ...
  //
  TR::Node* isub = TR::Node::create(
    TR::isub, 2, endNode->duplicateTree(), startNode->duplicateTree());
  if (postIncrement != 0)
    isub = TR::Node::create(
      TR::iadd, 2, isub, TR::Node::create(isub, TR::iconst, 0, postIncrement));
  if (_addInc) {
...
void
TR_LRAddressTree::updateAiaddSubTree(TR_ParentOfChildNode* indVarNode,
                                     TR_ArrayLoop* loop)
{
  ...
  if (endInc < 0 && !indVarNode->isNull()) {
    ...
    if (!addInc) {
      ...
      TR::Node* isub =
        TR::Node::create(TR::isub, 2, finalNode->duplicateTree(), elemSizeInc);
      // (64-bit)
...
TR_LRAddressTree::updateMultiply(TR_ParentOfChildNode* multiplyNode)
{
  ...
  if (!multiplyNode->isNull()) {
    ...
    if (multiplyNode->getParent()->getType().isInt32()) {
      TR::Node* initMulVal =
        TR::Node::create(multiplyNode->getParent(),
                         TR::iconst,
                         0,
...
//
bool
TR_LoopReducer::generateArraycopy(TR_InductionVariable* indVar,
                                  TR::Block* loopHeader)
{
  // for aladds
  ...
  {
    dumpOptDetails(comp(),
                   "Loop has %d tree tops - no arrayset reduction\n",
  ...
  if (arraycopyLoop.hasWriteBarrier() && needWriteBarrier &&
    ...
    dumpOptDetails(
      comp(),
      "arraycopy arraystore tree has write barrier as root and write barriers "
      "are enabled but no support for this platform- no arraycopy reduction\n");
    return false;
  }
  ...
  if (storeNode->getType().isBCD() &&
    ...
    dumpOptDetails(
      comp(),
      "arraycopy storeNode %p is a BCD type (%s) and the storeSize (%d) is not "
      "1,2,4 or 8 so do not reduce arraycopy\n",
      storeNode,
      storeNode->getDataType().toString(),
      storeSize);
    return false;
  }
#endif
  ...
  TR::Node* imul =
    arraycopyLoop.updateIndVarStore(arraycopyLoop.getStoreIndVarNode(),
                                    indVarStoreNode,
  ...
  if (!(arraycopyLoop.hasWriteBarrier() && needWriteBarrier)) {
    ...
    if (offset != 0)
      dst = TR::Node::create(
        op_add, 2, dst, TR::Node::create(dst, op_const, 0, offset));
    ...
    if (offset != 0)
      src = TR::Node::create(
        op_add, 2, src, TR::Node::create(src, op_const, 0, offset));
    ...
    if (arraycopyElementType.isBCD()) {
      switch (storeNode->getSize()) {
        ...
        default:
          ...
          TR_ASSERT(false,
                    "unsupported size (%d) for reduced arraycopy\n",
                    storeNode->getSize());
      }
    }
#endif
  ...
  deadCmpChild1->setNode(
    TR::Node::create(TR::treetop, 1, loopCmpTree->getNode()->getFirstChild()));
  deadCmpChild2->setNode(
    TR::Node::create(TR::treetop, 1, loopCmpTree->getNode()->getSecondChild()));
...
//
bool
TR_LoopReducer::generateArrayset(TR_InductionVariable* indVar,
                                 TR::Block* loopHeader)
{
  // for aladds
  ...
  TR::Node* imul =
    arraysetLoop.updateIndVarStore(arraysetLoop.getIndVarNode(),
                                   indVarStoreNode,
  ...
  TR::Node* arrayset = TR::Node::create(
    TR::arrayset, 3, dst, copyValueNode, imul->duplicateTree());
...
bool
TR_Arraycmp::checkGoto(TR::Block* gotoBlock,
                       TR::Node* gotoNode,
                       TR::Node* finalNode)
{
...
static void
swapIfNecessary(TR::Node*& aiaddFirstChild, TR::Node*& aiaddSecondChild)
{
  ...
  if (hasBranch()) {
    ...
    if (shrinkNode->getOpCodeValue() == TR::i2b ||
      ...
      if (iloadNode->getOpCodeValue() != TR::iload) {
        dumpOptDetails(
          comp(),
          "...store tree does not have iload - no arraytranslate reduction\n");
        return false;
      }
...
//
bool
TR_Arraytranslate::checkBreak(TR::Block* breakBlock,
                              TR::Node* breakNode,
                              TR::Block* nextBlock)
{
...
//
bool
TR_Arraytranslate::checkMatIndVarStore(TR::Node* matIndVarStoreNode,
                                       TR::Node* indVarStoreNode)
{
...
bool
TR_Arraytranslate::checkGoto(TR::Block* gotoBlock,
                             TR::Node* gotoNode,
                             TR::Block* nextBlock)
{
  TR::ILOpCodes gotoOpCode = gotoNode->getOpCode().getOpCodeValue();
...
bool
TR_LoopReducer::blockInVersionedLoop(
...
bool
TR_LoopReducer::generateArraycmp(TR_RegionStructure* whileLoop,
                                 TR_InductionVariable* indVar,
                                 ...
                                 TR::Block* incrementBlock)
{
  ...
  TR::Node* imul =
    arraycmpLoop.updateIndVarStore(arraycmpLoop.getFirstIndVarNode(),
                                   indVarStoreNode,
  ...
  TR::Node* compareNode =
    TR::Node::createif(TR::ificmpne, loadCompare, constBoolNode, cmpTargetTree);
  ...
  _cfg->removeEdge(incrementBlock->getSuccessors(),
                   incrementBlock->getNumber(),
                   branchBlock->getNumber());
...
TR_Arraytranslate::getTableNode()
{
  if (compilerGeneratedTable() && (_tableNode == NULL)) {
    ...
    if (inputBits == 8) {
      if (startB != endB) {
        TR_RangeTranslateTable rangeTable(comp(),
                                          inputBits,
                                          outputBits,
                                          (uint16_t)startA,
                                          (uint16_t)endA,
      ...
      if (startB != endB) {
        ...
        TR_RangeTranslateTable rangeTable(
          comp(), inputBits, outputBits, startA, endA, defaultValue);
        tableSymRef = rangeTable.createSymbolRef();
        rangeTable.dumpTable();
      }
    }
...
bool
TR_LoopReducer::replaceInductionVariable(TR::Node* parent,
                                         TR::Node* node,
                                         int32_t childNum,
...
bool
TR_LoopReducer::generateArraytranslate(TR_RegionStructure* whileLoop,
                                       TR_InductionVariable* indVar,
                                       TR::Block* firstBlock,
                                       ...
                                       TR::Block* fourthBlock)
{
  ...
  if (hasBreak) {
    ...
    if (hasBranch) {
      ...
      if (storeNode == NULL || loadNode == NULL) {
        dumpOptDetails(comp(),
                       "Load node is %p and store node is %p\n",
                       loadNode,
                       storeNode);
        return false;
      }
  ...
  if (!nextBlock) {
    dumpOptDetails(
      comp(), "Loop exit block is method exit - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (!arraytranslateLoop.getStoreAddress()->checkIndVarStore(
        indVarStoreNode)) {
    dumpOptDetails(
      comp(), "Loop does not have indvar tree - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (!arraytranslateLoop.checkStore(storeNode)) {
    dumpOptDetails(
      comp(), "Loop does not have store tree - no arraytranslate reduction\n");
    return false;
  }
  ...
  if (hasBranch) {
    if (!arraytranslateLoop.checkBreak(
          storeBlock, branchAroundNode, nextBlock)) {
      dumpOptDetails(comp(),
  ...
  TR::Node* compareNode = TR::Node::createif(
    TR::ificmple, finalNode, hardwareMinimumNode, newLoadEntry);
  ...
  */
  ...
  if (hasBranch) {
    ...
    if (!hasBreak) {
      ...
      replaceInductionVariable(
        NULL,
        resCharDupNode->getFirstChild()->skipConversions(),
        -1,
  ...
  if (hasBreak) {
    ...
    if (ivLoadNode->getOpCode().isArrayRef()) {
      ...
      TR::Node* decNode =
        TR::Node::create(TR::isub,
                         2,
                         TR::Node::createLoad(loadNode, indVarSymRef),
    ...
    TR::TreeTop* gotoMatchTree = TR::TreeTop::create(
      comp(), TR::Node::create(loadNode, TR::Goto, 0, matchBlock->getEntry()));
  ...
  if (hasBreak) {
    ...
    _cfg->addEdge(TR::CFGEdge::createEdge(
      translateBlock, loadLastElementBlock, trMemory()));
    _cfg->addEdge(
  ...
  if (hasBranch) {
    ...
    _cfg->removeEdge(
      loadSuccList, loadBlock->getNumber(), storeBlock->getNumber());

    if (hasBreak) {
      _cfg->removeEdge(
        loadSuccList, loadBlock->getNumber(), breakExitBlock->getNumber());
    } else {
      _cfg->removeEdge(
        loadSuccList, loadBlock->getNumber(), otherStoreBlock->getNumber());
      _cfg->removeEdge(
        loadSuccList, loadBlock->getNumber(), storeBlock->getNumber());
      _cfg->removeEdge(
        storeSuccList, storeBlock->getNumber(), loadBlock->getNumber());

      _cfg->removeEdge(
        cmpSuccList, cmpBlock->getNumber(), loadBlock->getNumber());
    }
    ...
    _cfg->removeEdge(
      loadSuccList, loadBlock->getNumber(), nextBlock->getNumber());
    return true;
  }
}
...
TR_ArraytranslateAndTest::checkLoad(TR::Block* loadBlock, TR::Node* loadNode)
{
  ...
  if (aiaddNode->getOpCodeValue() != TR::aiadd &&
    ...
    dumpOptDetails(
      comp(),
      "...load tree does not have aiadd/aladd - no arraytranslate reduction\n");
    return false;
  }
...
bool
TR_ArraytranslateAndTest::checkFrequency(TR::CodeGenerator* cg,
                                         TR::Block* loadBlock,
                                         TR::Node* loadNode)
{
...
// BBEnd (block 16)
bool
TR_LoopReducer::generateArraytranslateAndTest(TR_RegionStructure* whileLoop,
                                              TR_InductionVariable* indVar,
                                              TR::Block* firstBlock,
                                              TR::Block* secondBlock)
{
  ...
  if (!nextBlock) {
    dumpOptDetails(
      comp(),
      "Loop exit block is method exit - no arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (!arraytranslateAndTestLoop.getStoreAddress()->checkIndVarStore(
    ...
    dumpOptDetails(
      comp(),
      "Loop does not have indvar tree - no arraytranslateAndTest reduction\n");
    return false;
  }

  if (!arraytranslateAndTestLoop.checkLoopCmp(
        loopCmpNode, indVarStoreNode, indVar)) {
    dumpOptDetails(
      comp(),
      "Loop does not have loopcmp tree - no arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (!arraytranslateAndTestLoop.forwardLoop()) {
    dumpOptDetails(
      comp(),
      "Loop does not have forward loop - no arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (!performTransformation(comp(),
                             "%sReducing arraytranslateAndTest %d\n",
                             OPT_DETAILS,
                             loadBlock->getNumber())) {
    return false;
  }
  ...
  TR::Node* finalNode = arraytranslateAndTestLoop.updateIndVarStore(
    arraytranslateAndTestLoop.getIndVarNode(),
    indVarStoreNode,
  ...
  _cfg->removeEdge(
    loadBlock->getSuccessors(), loadBlock->getNumber(), cmpBlock->getNumber());
  _cfg->removeEdge(
    cmpBlock->getSuccessors(), cmpBlock->getNumber(), nextBlock->getNumber());
  return true;
}
...
bool
TR_LoopReducer::generateByteToCharArraycopy(TR_InductionVariable* byteIndVar,
                                            ...
                                            TR::Block* loopHeader)
{
         ...
         // loop comparison
  {
    dumpOptDetails(
      comp(),
      "Loop has %d tree tops - no byte to char arraycopy reduction\n",
      loopHeader->getNumberOfRealTreeTops());
    return false;
  }
  ...
  TR_ByteToCharArraycopy arraycopyLoop(
    comp(), charIndVar, byteIndVar, TR::Compiler->target.cpu.isBigEndian());
  ...
  if (!arraycopyLoop.getStoreAddress()->checkIndVarStore(charIndVarStoreNode)) {
    bool swap =
      arraycopyLoop.getHighLoadAddress()->checkIndVarStore(charIndVarStoreNode);
  ...
  if (!arraycopyLoop.checkLoopCmp(
        loopCmpNode, charIndVarStoreNode, charIndVar)) {
  ...
  if (!performTransformation(comp(),
                             "%sReducing byte to char arraycopy %d\n",
                             OPT_DETAILS,
                             loopHeader->getNumber())) {
    return false;
  }
  ...
  TR::Node* charImul =
    arraycopyLoop.updateIndVarStore(arraycopyLoop.getStoreIndVarNode(),
                                    ...
                                    arraycopyLoop.getStoreAddress());
  TR::Node* byteImul =
    arraycopyLoop.updateIndVarStore(arraycopyLoop.getHighLoadIndVarNode(),
                                    byteIndVarStoreNode,
...
bool
TR_LoopReducer::generateCharToByteArraycopy(TR_InductionVariable* byteIndVar,
                                            TR_InductionVariable* charIndVar,
                                            TR::Block* loopHeader)
{
  if (!comp()->cg()->getSupportsReferenceArrayCopy() &&
         ...
         // store, induction variable store, loop comparison
  {
    dumpOptDetails(
      comp(),
      "Loop has %d tree tops - no char to byte arraycopy reduction\n",
      loopHeader->getNumberOfRealTreeTops());
    return false;
  }
  ...
  TR_CharToByteArraycopy arraycopyLoop(
    comp(), charIndVar, byteIndVar, TR::Compiler->target.cpu.isBigEndian());
  ...
  if (!performTransformation(comp(),
                             "%sReducing char to byte arraycopy %d\n",
                             OPT_DETAILS,
                             loopHeader->getNumber())) {
    return false;
  }
  ...
  arraycopyLoop.getLoadAddress()->updateAiaddSubTree(
    arraycopyLoop.getLoadIndVarNode(), &arraycopyLoop);
  TR::Node* charImul =
    arraycopyLoop.updateIndVarStore(arraycopyLoop.getLoadIndVarNode(),
                                    charIndVarStoreNode,
                                    arraycopyLoop.getLoadAddress());
  TR::Node* byteImul =
    arraycopyLoop.updateIndVarStore(arraycopyLoop.getHighStoreIndVarNode(),
                                    byteIndVarStoreNode,
...
int
TR_LoopReducer::addBlock(TR::Block* newBlock,
                         TR::Block** blockList,
                         int numBlocks,
...
int
TR_LoopReducer::addRegionBlocks(TR_RegionStructure* region,
                                TR::Block** blockList,
                                int numBlocks,
                                const int maxNumBlocks)
{
...
bool
TR_LoopReducer::mayNeedGlobalDeadStoreElimination(TR::Block* storeBlock,
...
TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure* whileLoop)
{
  ...
  if (trace()) {
    dumpOptDetails(
      comp(), "Blocks in loop %p,%d ( ", loopHeader, loopHeader->getNumber());
                     ...
                     considered */
  {
    ...
    if (((numBlocks == 1) && generateArrayset(indVar, loopHeader)) ||
        ...
        ((numBlocks == 1) &&
         generateArraytranslate(
           whileLoop, indVar, loopHeader, NULL, NULL, NULL)) ||
        (((numBlocks == 2) /* || (numBlocks == 3 && block3->endsInGoto()) */) &&
         generateArraytranslate(
           whileLoop, indVar, loopHeader, block2, NULL, NULL)) ||
        ((numBlocks == 3) &&
         generateArraytranslate(
           whileLoop, indVar, loopHeader, block4, block3, NULL)) ||
        ((numBlocks == 4) &&
         generateArraytranslate(
           whileLoop, indVar, loopHeader, block4, block3, block2)) ||
        ((numBlocks == 2) && generateArraytranslateAndTest(
                               whileLoop, indVar, loopHeader, block2))) {
      removeSelfEdge(loopHeader->getSuccessors(), whileLoop->getNumber());
    } else {
      dumpOptDetails(comp(),
                     "Loop %d has %d blocks and is not reduced\n",
                     loopHeader->getNumber(),
                     numBlocks);
    }
  }
