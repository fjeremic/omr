  ...
  if (loadSym != indSym) {
    if (_matIndVarSymRef) {
      if (loadSym != _matIndVarSymRef->getSymbol()->getRegisterMappedSymbol()) {
        dumpOptDetails(comp(),
                       "checkAiadd: load in the aiadd tree does not match "
                       "materialized induction variable\n");
        return false;
      }
    } else {
      dumpOptDetails(
...
bool TR_Arrayset::checkArrayStore(TR::Node *storeNode) {
  ...
  if (!storeSecondChild->getOpCode().isLoadDirectOrReg()) {
    dumpOptDetails(comp(),
                   "arraystore tree does not have a constant load, or constant "
                   "load is an address\n");
    return false;
  }
...
bool TR_Arraycopy::checkArrayStore(TR::Node *storeNode) {
  ...
  if (!loadNode->getOpCode().isLoadIndirect()) {
    dumpOptDetails(comp(),
                   "arraycopy arraystore tree does not have an indirect load "
                   "as the second child\n");
    return false;
  }
...
bool TR_ByteToCharArraycopy::checkArrayStore(TR::Node *storeNode) {
  if (storeNode->getOpCodeValue() != TR::cstorei) {
    dumpOptDetails(comp(),
                   "byte to char arraycopy arraystore tree does not have an "
                   "indirect store as root\n");
    return false;
  }
  TR::Node *storeFirstChild = storeNode->getFirstChild();
  TR::ILOpCodes opCodeStoreFirstChild = storeFirstChild->getOpCodeValue();
...
bool TR_ByteToCharArraycopy::checkByteLoads(TR::Node *loadNodes) {
  if (loadNodes->getOpCodeValue() != TR::i2s) {
    dumpOptDetails(comp(),
                   "checkByteLoads: byte to char arraycopy byte loads is not "
                   "headed with i2c\n");
    return false;
  }
  ...
  if (joinNode->getOpCodeValue() != TR::ior &&
    ...
    dumpOptDetails(comp(),
                   "checkByteLoads: byte to char arraycopy byte loads not "
                   "joined with OR or ADD\n");
    return false;
  }
  ...
  if (highByteNode->getOpCodeValue() != TR::imul &&
    ...
    if (highByteNode->getOpCodeValue() != TR::imul ||
      ...
      dumpOptDetails(comp(),
                     "checkByteLoads: byte to char arraycopy byte loads do not "
                     "have imul/bu2i children\n");
      return false;
    }
  }
...
bool TR_LRAddressTree::checkIndVarStore(TR::Node *indVarNode) {
  ...
  if (opCodeIaddFirstChild != TR::iload ||
    ...
    dumpOptDetails(comp(),
                   "first child of iadd is not TR::iload or second child is "
                   "not TR::iconst\n");
    return false;
  }
  TR::RegisterMappedSymbol *indSym = _indVar->getLocal();
...
bool TR_ArrayLoop::checkLoopCmp(TR::Node *loopCmpNode,
  ...
  if (opCodeCmpSecondChild != TR::iconst && opCodeCmpSecondChild != TR::iload &&
    ...
    dumpOptDetails(comp(),
                   "loop compare does not have iconst/iload/arraylength as "
                   "second child\n");
    return false;
  }
...
int32_t TR_ArrayLoop::checkForPostIncrement(TR::Block *loopHeader,
  ...
  // of the iv
  //
  if (storeIvLoad == cmpFirstChild) return 1;

  // the loopCmp uses some load of the iv that
  // was commoned
  //
  if (ivLoads.find(cmpFirstChild)) return 1;
...
void TR_ArrayLoop::findIndVarLoads(TR::Node *node, TR::Node *indVarStoreNode,
  ...
  if (node == indVarStoreNode) storeFound = true;

  if (node->getOpCodeValue() == TR::iload &&
      node->getSymbolReference()->getSymbol() == ivSym) {
    if (!ivLoads->find(node)) ivLoads->add(node);
  }
...
bool TR_LoopReducer::generateArraycopy(TR_InductionVariable *indVar,
  ...
  if (arraycopyLoop.hasWriteBarrier() && needWriteBarrier &&
    ...
    dumpOptDetails(comp(),
                   "arraycopy arraystore tree has write barrier as root and "
                   "write barriers are enabled but no support for this "
                   "platform- no arraycopy reduction\n");
    return false;
  }
    ...
    if (arraycopyElementType.isBCD()) {
      switch (storeNode->getSize()) {
        ...
        case 8:
          ...
          break;
        default:
          // a pre-transformation check just above guarantees only sizes 1,2,4
          // or 8 are transformed
...
bool TR_Arraycmp::checkGoto(TR::Block *gotoBlock, TR::Node *gotoNode,
  ...
  if (extraStoreNode != NULL) {
    if (!(firstChildSym == comparisonSym && secondChildSym == NULL)) {
      dumpOptDetails(comp(),
                     "first/second child: alternate goto comparison not to "
                     "comparison sym\n");
      return false;
    }
    ...
    dumpOptDetails(comp(),
                   "first/second child: goto comparison are not "
...
bool TR_Arraytranslate::checkLoad(TR::Node *loadNode) {
  ...
  if (transLoadNode->getOpCodeValue() != TR::cloadi &&
    ...
    dumpOptDetails(comp(),
                   "...load tree does not have ibload/icload - no "
  ...
  if (aiaddNode->getOpCodeValue() != TR::aiadd &&
    ...
    dumpOptDetails(comp(),
                   "...load tree does not have aiadd/aladd/iadd/ladd - no "
                   "arraytranslate reduction\n");
    return false;
  }
  ...
  if (aiaddNode->getOpCodeValue() == TR::aiadd ||
    ...
    if (aiaddFirstChild->getOpCodeValue() != TR::aload &&
      ...
      dumpOptDetails(comp(),
                     "...aiadd load tree does not have aload - no "
                     "arraytranslate reduction\n");
      return false;
    }
    ...
    if (aiaddSecondChild->getOpCodeValue() != TR::lloadi &&
      ...
      dumpOptDetails(comp(),
                     "...iadd load tree does not have ilload - no "
                     "arraytranslate reduction\n");
      return false;
    }
  ...
  if (inLoadNode->getOpCodeValue() != TR::cloadi &&
    ...
    dumpOptDetails(comp(),
                   "...load tree does not have 2nd icload/ibload - check if "
                   "compiler-generated table lookup match\n");
...
bool TR_Arraytranslate::checkStore(TR::Node *storeNode) {
  if (storeNode->getOpCodeValue() != TR::cstorei &&
    ...
    dumpOptDetails(comp(),
                   "...store tree does not have icstore/ibstore - no "
                   "arraytranslate reduction\n");
    return false;
  }
  ...
  if (aiaddNode->getOpCodeValue() != TR::aiadd &&
      aiaddNode->getOpCodeValue() != TR::aladd) {
    dumpOptDetails(comp(),
                   "...store tree does not have aiadd/aladd - no "
                   "arraytranslate reduction\n");
    return false;
  }
  ...
  if (hasBranch()) {
    ...
    if (shrinkNode->getOpCodeValue() == TR::i2b ||
      ...
      if (iloadNode->getOpCodeValue() != TR::iload) {
        dumpOptDetails(comp(),
                       "...store tree does not have iload - no arraytranslate "
                       "reduction\n");
        return false;
      }
...
bool TR_Arraytranslate::checkBreak(TR::Block *breakBlock, TR::Node *breakNode,
  ...
  if (loadNode->getSymbolReference() !=
    ...
    dumpOptDetails(comp(),
                   "...break tree reference does not match load tree reference "
  ...
  if (valNode->getOpCodeValue() != TR::iconst ||
    ...
    //      generate a table.
    dumpOptDetails(comp(),
                   "...break tree does not have bconst/cconst/iconst, or not "
                   "in range - no arraytranslate reduction\n");
    return false;
  }
...
int32_t TR_Arraytranslate::getTermValue() {
  ...
  if (_termCharNode) defaultValue = _termCharNode->getInt();
...
bool TR_LoopReducer::generateArraytranslate(TR_RegionStructure *whileLoop,
  ...
  if (!hasBranch && !arraytranslateLoop.getByteInput() &&
    ...
    dumpOptDetails(comp(),
                   "Cannot reduce char->byte loops since hardware does not "
                   "support termination character control\n");
    return false;
  }
  ...
  if (mayHaveMaterializedIV && !arraytranslateLoop.checkMatIndVarStore(
    ...
    dumpOptDetails(comp(),
                   "Loop does not match materialized indvar requirements - no "
                   "arraytranslate reduction\n");
    return false;
  }
  ...
  if (hasBranch) {
    if (!arraytranslateLoop.checkBreak(storeBlock, branchAroundNode,
                                       nextBlock)) {
      dumpOptDetails(comp(),
                     "Loop does not have branch-around tree - no "
                     "arraytranslate reduction\n");
      return false;
    }
  } else {
    if (arraytranslateLoop.getTableNode() == NULL) {
      dumpOptDetails(comp(),
                     "Loop does not have a translation table - no "
                     "arraytranslate reduction\n");
      return false;
    }
  }
  ...
  if (!arraytranslateLoop.compilerGeneratedTable() &&
    ...
    dumpOptDetails(comp(),
                   "Output array and translation table datatype mismatch - no "
                   "arraytranslate reduction\n");
    return false;
  }
  ...
  if (mayHaveMaterializedIV) loadBlock->getEntry()->join(branchNewOldTreeTop);
  ...
  */
  ...
  int32_t mask = cg()->arrayTranslateTableRequiresAlignment(
      arraytranslateLoop.getByteInput(), arraytranslateLoop.getByteOutput());
  if (mask && !arraytranslateLoop
                   .compilerGeneratedTable())  // compiler-generated tables
  ...
  if (hasBreak) {
    ...
    TR::TreeTop *gotoMatchTree = TR::TreeTop::create(
        comp(),
        TR::Node::create(loadNode, TR::Goto, 0, matchBlock->getEntry()));
...
bool TR_ArraytranslateAndTest::checkLoad(TR::Block *loadBlock,
  ...
  if (loadNode->getOpCodeValue() != TR::ifbcmpeq &&
    ...
    dumpOptDetails(comp(),
                   "...load tree does not have ifbcmpeq/ificmpeq - no "
  ...
  if (loadNode->getOpCodeValue() == TR::ificmpeq) {
    if (loadNode->getFirstChild()->getOpCodeValue() != TR::b2i) {
      dumpOptDetails(comp(),
                     "...load tree has ificmpeq but no widening from byte - no "
                     "arraytranslateAndTest reduction\n");
      return false;
    } else {
  ...
  if (bLoadNode->getOpCodeValue() != TR::bloadi) {
    dumpOptDetails(comp(),
                   "...load tree does not have ibload - no "
                   "arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (aiaddNode->getOpCodeValue() != TR::aiadd &&
      aiaddNode->getOpCodeValue() != TR::aladd) {
    dumpOptDetails(comp(),
                   "...load tree does not have aiadd/aladd - no arraytranslate "
                   "reduction\n");
    return false;
  }
  ...
  if (aiaddFirstChild->getOpCodeValue() != TR::aload &&
      aiaddFirstChild->getOpCodeValue() != TR::aloadi) {
    dumpOptDetails(comp(),
                   "...load tree does not have aload - no "
                   "arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (cmpVal->getOpCodeValue() != TR::bconst &&
    ...
    dumpOptDetails(comp(),
                   "...load tree does not have bconst/iconst - no "
                   "arraytranslateAndTest reduction\n");
    return false;
  }
...
bool TR_LoopReducer::generateArraytranslateAndTest(
  ...
  if (!nextBlock) {
    dumpOptDetails(comp(),
                   "Loop exit block is method exit - no arraytranslateAndTest "
                   "reduction\n");
    return false;
  }
  ...
  if (!arraytranslateAndTestLoop.getStoreAddress()->checkIndVarStore(
          indVarStoreNode)) {
    dumpOptDetails(comp(),
                   "Loop does not have indvar tree - no arraytranslateAndTest "
                   "reduction\n");
    return false;
  }
  ...
  if (!arraytranslateAndTestLoop.checkLoopCmp(loopCmpNode, indVarStoreNode,
                                              indVar)) {
    dumpOptDetails(comp(),
                   "Loop does not have loopcmp tree - no arraytranslateAndTest "
                   "reduction\n");
    return false;
  }
  ...
  if (!arraytranslateAndTestLoop.forwardLoop()) {
    dumpOptDetails(comp(),
                   "Loop does not have forward loop - no arraytranslateAndTest "
                   "reduction\n");
    return false;
  }
  ...
  if (!arraytranslateAndTestLoop.checkFrequency(cg(), loadBlock, loadNode)) {
    dumpOptDetails(comp(),
                   "Loop frequency on fall-through not high enough - no "
                   "arraytranslateAndTest reduction\n");
    return false;
  }
  ...
  if (!arraycopyLoop.getStoreAddress()->checkIndVarStore(charIndVarStoreNode)) {
    ...
    if (swap && arraycopyLoop.getStoreAddress()->checkIndVarStore(
      ...
      return false;
                 ...
                 byteIndVarStoreNode)) {
    dumpOptDetails(comp(),
                   "Did not encounter char array induction variable increment "
                   "- no byte to char arraycopy performed\n");
    return false;
  }
...
bool TR_LoopReducer::generateCharToByteArraycopy(
  ...
  if (!arraycopyLoop.checkArrayStores(highStoreNode, lowStoreNode)) {
    dumpOptDetails(comp(),
                   "... no match - switch around high and low array store "
                   "trees and try again...\n");
  ...
  if (!arraycopyLoop.getLoadAddress()->checkIndVarStore(charIndVarStoreNode)) {
    ...
    dumpOptDetails(comp(),
                   "Did not encounter char array induction variable increment "
                   "- no char to byte arraycopy performed\n");
    return false;
  }
  ...
  TR::TreeTop::removeDeadTrees(
      comp(), treeList);  // make these trees into nops, but leave them around
                          // to keep ref counts right
...
bool TR_LoopReducer::constrainedIndVar(TR_InductionVariable *indVar) {
  ...
  switch (increment) {
    ...
    default:
      dumpOptDetails(
          comp(),
          "Loop has constant induction variable other than +/-1/2/4/8\n");
      return false;
  }
  return true;
}
...
bool TR_LoopReducer::mayNeedGlobalDeadStoreElimination(
  ...
  if (storeNum != 3 || compareNum != 3) {
    ...
    // compareNum);
    return false;  // for now, just the pattern for arraytranslate is what we
                   // are worried about
  }
...
void TR_LoopReducer::reduceNaturalLoop(TR_RegionStructure *whileLoop) {
                     ...
                     considered */
  {
    ...
    if ((numBlocks == 2 /* || (numBlocks == 3 && block3->endsInGoto()) */) &&
      ...
      dumpOptDetails(comp(),
                     "Loop matches possible arraytranslate - global deadstore "
                     "elimination to be performed\n");
