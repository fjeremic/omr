...
void
TR::RegDepCopyRemoval::rememberNodeChoice(TR_GlobalRegisterNumber reg,
                                          TR::Node* selected)
{
...
TR::RegDepCopyRemoval::readRegDeps()
{
  for (int i = 0; i < _regDeps->getNumChildren(); i++) {
    ...
    TR_ASSERT(dep.state == REGDEP_ABSENT,
              "register %s is multiply-specified\n",
              registerName(reg));
    dep.node = depNode;
    dep.value = depValue;
...
TR::RegDepCopyRemoval::selectNodesToReuse(TR::NodeChecklist& usedNodes)
{
  for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
    ...
    if (trace())
      traceMsg(comp(),
               "\t%s: prefer to reuse previous choice n%un\n",
               registerName(reg),
...
TR::RegDepCopyRemoval::selectNodesToCopy(TR::NodeChecklist& usedNodes)
{
  for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
    ...
    if (!usedNodes.contains(dep.value)) {
      ...
      if (trace())
        traceMsg(comp(),
                 "\t%s: prefer to keep the original node n%un\n",
                 registerName(reg),
      ...
      if (trace())
        traceMsg(comp(),
                 "\t%s: prefer to make a new copy of n%un\n",
                 registerName(reg),
                 dep.value->getGlobalIndex());
    }
  }
...
void
TR::RegDepCopyRemoval::updateSingleRegDep(TR_GlobalRegisterNumber reg,
