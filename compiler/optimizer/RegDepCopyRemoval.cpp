...
{
  TR_ASSERT(
    _regBegin <= _regEnd, "upper and lower global register number bounds out of order: [%d, %d)\n", _regBegin, _regEnd);
    ...
    default:
      if (node->getOpCode().isSwitch()) {
        ...
        for (int i = 0; i < nChildren - 1; i++)
          TR_ASSERT(
            node->getChild(i)->getOpCodeValue() != TR::GlRegDeps, "GlRegDeps for branch is not the last child\n");
        if (nChildren > 0) {
...
void TR::RegDepCopyRemoval::selectNodesToCopy(TR::NodeChecklist& usedNodes)
{
  for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
    ...
    if (!usedNodes.contains(dep.value)) {
      ...
      if (trace())
        traceMsg(
          comp(), "\t%s: prefer to keep the original node n%un\n", registerName(reg), dep.value->getGlobalIndex());
    } else {
