...
TR::RegDepCopyRemoval::RegDepCopyRemoval(TR::OptimizationManager *manager)
  ...
  TR_ASSERT(
      _regBegin <= _regEnd,
      "upper and lower global register number bounds out of order: [%d, %d)\n",
      _regBegin, _regEnd);
...
int32_t TR::RegDepCopyRemoval::perform() {
  if (!cg()->supportsPassThroughCopyToNewVirtualRegister()) return 0;
    ...
    switch (node->getOpCodeValue()) {
      case TR::BBStart:
        if (!node->getBlock()->isExtensionOfPreviousBlock()) {
          if (trace())
            traceMsg(comp(),
                     "clearing remembered node choices at start of extended "
                     "block at block_%d\n",
...
const char *TR::RegDepCopyRemoval::registerName(TR_GlobalRegisterNumber reg) {
  ...
  TR_RegisterSizes defaultSize =
      TR::Compiler->target.is64Bit() ? TR_DoubleWordReg : TR_WordReg;
...
void TR::RegDepCopyRemoval::rangeCheckRegister(TR_GlobalRegisterNumber reg) {
  TR_ASSERT(reg >= _regBegin && reg < _regEnd,
            "global register number %d out of range\n", (int)reg);
}

TR::RegDepCopyRemoval::RegDepInfo &TR::RegDepCopyRemoval::getRegDepInfo(
    TR_GlobalRegisterNumber reg) {
  rangeCheckRegister(reg);
  return _regDepInfoTable.at(reg - _regBegin);
}

TR::RegDepCopyRemoval::NodeChoice &TR::RegDepCopyRemoval::getNodeChoice(
    TR_GlobalRegisterNumber reg) {
  rangeCheckRegister(reg);
...
void TR::RegDepCopyRemoval::rememberNodeChoice(TR_GlobalRegisterNumber reg,
                                               TR::Node *selected) {
...
void TR::RegDepCopyRemoval::processRegDeps(TR::Node *deps, TR::TreeTop *depTT) {
  ...
  TR_ASSERT(deps->getOpCodeValue() == TR::GlRegDeps,
            "processDeps: deps is not GlRegDeps\n");
...
void TR::RegDepCopyRemoval::readRegDeps() {
  for (int i = 0; i < _regDeps->getNumChildren(); i++) {
    ...
    if (depValue->getOpCodeValue() == TR::PassThrough) {
      ...
      TR_ASSERT(depNode->getOpCode().isLoadReg(),
                "invalid GlRegDeps child opcode n%un %s\n",
                depNode->getGlobalIndex(), depNode->getOpCode().getName());
    }

    // Avoid register pairs for simplicity, at least for now
    bool isRegPairDep =
        depNode->getHighGlobalRegisterNumber() != (TR_GlobalRegisterNumber)-1;
    bool valueNeedsRegPair = depValue->requiresRegisterPair(comp());
    TR_ASSERT(isRegPairDep == valueNeedsRegPair,
              "mismatch on number of registers required for n%un\n",
...
void TR::RegDepCopyRemoval::ignoreRegister(TR_GlobalRegisterNumber reg) {
  ...
  RegDepInfo &dep = getRegDepInfo(reg);
  TR_ASSERT(dep.state == REGDEP_ABSENT,
            "attempted to ignore previously specified register %s\n",
            registerName(reg));
...
void TR::RegDepCopyRemoval::selectNodesToReuse(TR::NodeChecklist &usedNodes) {
  for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
    RegDepInfo &dep = getRegDepInfo(reg);
    if (dep.state != REGDEP_UNDECIDED) continue;

    NodeChoice &prevChoice = getNodeChoice(reg);
    if (dep.value != prevChoice.original) continue;  // can't reuse
    ...
    TR_ASSERT(!usedNodes.contains(prevChoice.selected),
              "attempted to reuse the same node more than once\n");
  ...
  for (TR_GlobalRegisterNumber reg = _regBegin; reg < _regEnd; reg++) {
    ...
    switch (dep.state) {
      ...
      case REGDEP_UNDECIDED:
        TR_ASSERT(false,
                  "undecided preference for %s survived to updateRegDeps()\n",
                  registerName(reg));
        break;
      default:
...
void TR::RegDepCopyRemoval::reuseCopy(TR_GlobalRegisterNumber reg) {
  RegDepInfo &dep = getRegDepInfo(reg);
  NodeChoice &prevChoice = getNodeChoice(reg);
  TR_ASSERT(prevChoice.original == dep.value,
            "previous copy for %s doesn't match original\n", registerName(reg));
  TR_ASSERT(prevChoice.selected != dep.value,
            "previous copy is the same as original for %s\n",
            registerName(reg));
  if (performTransformation(
          comp(),
          "%schange %s in GlRegDeps n%un to use previous copy n%un of n%un\n",
          optDetailString(), registerName(reg), _regDeps->getGlobalIndex(),
...
void TR::RegDepCopyRemoval::makeFreshCopy(TR_GlobalRegisterNumber reg) {
  RegDepInfo &dep = getRegDepInfo(reg);
  if (!performTransformation(
          comp(), "%schange %s in GlRegDeps n%un to an explicit copy of n%un\n",
          optDetailString(), registerName(reg), _regDeps->getGlobalIndex(),
  ...
  if (curNode->getOpCodeValue() == TR::BBEnd) {
    ...
    if (curBlock->getLastRealTreeTop() !=
        curBlock->getLastNonControlFlowTreeTop()) {
      TR::Block *fallthrough = curBlock->getNextBlock();
      fallthrough = curBlock->splitEdge(curBlock, fallthrough, comp());
      TR_ASSERT(curBlock->getNextBlock() == fallthrough,
                "bad block placement from splitEdge\n");
      ...
      if (trace())
        traceMsg(comp(),
                 "\tsplit fallthrough edge to insert copy, created block_%d\n",
  ...
  if (trace()) traceMsg(comp(), "\tcopy is n%un\n", copyNode->getGlobalIndex());

  updateSingleRegDep(reg, copyNode);
}

void TR::RegDepCopyRemoval::updateSingleRegDep(TR_GlobalRegisterNumber reg,
                                               TR::Node *newValueNode) {
  RegDepInfo &dep = getRegDepInfo(reg);
  ...
  TR_ASSERT(prevChild == dep.node,
            "childIndex and node inconsistent in RegDepInfo for %s\n",
            registerName(reg));
  TR_ASSERT(prevChild->getGlobalRegisterNumber() == reg,
            "childIndex and reg inconsistent in RegDepInfo for %s\n",
            registerName(reg));
  ...
  if (newValueNode->getOpCode().isLoadReg() &&
    ...
    TR::Node *newOutgoingPassThroughNode =
        TR::Node::create(TR::PassThrough, 1, newValueNode);
...
void TR::RegDepCopyRemoval::generateRegcopyDebugCounter(const char *category) {
  ...
  const char *fullName = TR::DebugCounter::debugCounterName(
      comp(), "regcopy/RegDepCopyRemoval/%s/(%s)/%s/block_%d", category,
...
const char *TR::RegDepCopyRemoval::optDetailString() const throw() {
