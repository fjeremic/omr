...
public:
  ...
  bool optimizeArraysetIfPossible(
    TR::Node*, TR::Node*, TR::TreeTop*, TR::Node*, TR_BitVector*, TR_BitVector*, TR_BitVector*, vcount_t, TR::TreeTop*);
  virtual const char* optDetailString() const throw();
};
...
public:
  ...
  void removeNodeFromList(TR::Node* node, List<TR::Node>* nodes, List<List<TR::Node> >* parents, bool checkSymRefs,
    List<TR::Node>* loadsAlreadyVisited = NULL, List<TR::Node>* loadsAlreadyVisitedThatCannotBeRematerialized = NULL)
  {
...
private:
  ...
  public:
    ...
    void setSymbol(TR::ParameterSymbol* s)
    {
      _symbol = s;
    }
    TR::ParameterSymbol* getSymbol()
    {
      return _symbol;
    }

    void setNotInvariant()
    {
      _isNotInvariant = true;
    }
    bool isInvariant()
    {
      return !_isNotInvariant;
    }
    ...
    bool classIsFixed()
    {
      return _classIsFixed;
    }

    void setClassIsCurrentlyFinal()
    {
      _classIsCurrentlyFinal = true;
    }
    bool classIsCurrentlyFinal()
    {
      return _classIsCurrentlyFinal;
    }

    void setClassIsRefined()
    {
      _classIsRefined = true;
    }
    bool classIsRefined()
    {
      return _classIsRefined;
    }

    void setClass(TR_OpaqueClassBlock* clazz)
    {
      _clazz = clazz;
    }
...
public:
  ...
  static void preProcessTreetop(
    TR::TreeTop* treeTop, List<TR::TreeTop>& commonedTreeTopList, char* optDetails, TR::Compilation* comp);
  static void postProcessTreetop(
    TR::TreeTop* treeTop, List<TR::TreeTop>& commonedTreeTopList, char* optDetails, TR::Compilation* comp);
  static void processCommonedChild(TR::Node* child, TR::TreeTop* treeTop, List<TR::TreeTop>& commonedTreeTopList,
