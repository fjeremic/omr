...
bool collectSymbolReferencesInNode(TR::Node *node,
                                   ...
                                   vcount_t visitCount, TR::Compilation *c,
                                   bool *seenInternalPointer = NULL,
                                   bool *seenArraylet = NULL,
...
public:
  ...
  int32_t performChecksAndTreesMovement(TR::Block *, TR::Block *, TR::Block *,
                                        TR::TreeTop *, vcount_t,
                                        TR::Optimizer *);
  ...
  TR::Block *breakFallThrough(TR::Block *faller, TR::Block *fallee,
                              bool isOutlineSuperColdBlock = true);
...
private:
  ...
  void renumberExitEdges(TR_RegionStructure *region, int32_t oldN,
                         int32_t newN);
  void redirectPredecessors(TR::Block *block, TR::Block *destBlock,
                            const TR::CFGEdgeList &preds, bool emptyBlock,
...
public:
  ...
  virtual int32_t performOnBlock(TR::Block *);
  int32_t process(TR::TreeTop *, TR::TreeTop *);
  void reduceArraysetStores(TR::Block *, TR_BitVector *, TR_BitVector *,
                            TR_BitVector *);
  bool optimizeArraysetIfPossible(TR::Node *, TR::Node *, TR::TreeTop *,
                                  TR::Node *, TR_BitVector *, TR_BitVector *,
 ...
 * (null checks are implicit, i.e. no code is generated for performing null
 * checks; instead we use hardware trap on IA32). Sometimes (say as a result of
 * dead store removal of a store to a field) an explicit null check node may
 * need to be created in the IL trees (if the original dead store was
 * responsible for checking nullness implicitly); this pass attempts to find
 * another expression in the same block that can be used to perform the null
 * check implicitly again.
 */
...
public:
  ...
  bool replacePassThroughIfPossible(TR::Node *, TR::Node *, TR::Node *,
                                    TR::Node *, bool *, TR_BitVector *,
                                    vcount_t, vcount_t, TR::TreeTop *);

  bool replaceNullCheckIfPossible(TR::Node *cursorNode, TR::Node *objectRef,
 ...
 *
 * if (a == null)     goto blockA; // test emitted for the null check on the
 * read of a[i] if (a.length <= N) goto blockA; // test emitted for the bound
 * check on the read of a[i] if (i < 0)         goto blockA; // second test
 * emitted for the bound check on the read of a[i] if (b == null)     goto
 * blockA; // test emitted for the null check on the read of b[i] if (b.length
 * <= N) goto blockA; // test emitted for the bound check on the read of b[i] if
 * (i < 0)         goto blockA; // second test emitted for the bound check on
 * the read of b[i] if (a == null)     goto blockA; // test emitted for the null
 * check on the write of a[i] if (a.length <= N) goto blockA; // test emitted
 * for the bound check on the write of a[i] if (i < 0)         goto blockA; //
 * second test emitted for the bound check on the write of a[i]
 *
 * There are obviously some redundant tests emitted here by the loop versioner
 * but that optimization simply relies on later cleanup passes to eliminate
 * those redundant tests. This is where and simplification comes in. The above
 * control flow construct is similar to a "logical and" of all those conditions
 * and this is where the name of the optimization comes from.
 *
 * And simplification runs after basic block extension has been done and a long
 * extended block created containing the above cascade of tests and local CSE
 * has commoned up expressions within that extended block. This means that there
 ...
 * of commoned expressions is more than the available registers on that
 * platform, it un-commons one of the expressions if possible (after making sure
 * this will not change the program's behaviour). This would cause the
 * expression to be evaluated again (i.e. the expression must be
 * "rematerialized") and in the case of our load of an auto this should be
 * cheaper than spilling to a stack slot and reloading from that slot. Some
 * cheap arithmetic operations are also considered cheap enough to re-evaluate
 * instead of spilling, e.g. iadd.
 *
...
public:
  ...
  void addParentToList(TR::Node *, List<TR::Node> *, TR::Node *,
                       List<List<TR::Node>> *);
  ...
  void removeNodeFromList(
      TR::Node *node, List<TR::Node> *nodes, List<List<TR::Node>> *parents,
      bool checkSymRefs, List<TR::Node> *loadsAlreadyVisited = NULL,
    ...
    removeNodeFromList(node, nodes, parents, checkSymRefs, loadsAlreadyVisited,
                       loadsAlreadyVisitedThatCannotBeRematerialized,
                       EMPTY_SET);
  }

  void findSymsUsedInIndirectAccesses(TR::Node *, List<TR::Node> *,
                                      List<List<TR::Node>> *);

  // bool examineNode(TR::TreeTop *, TR::Node *, TR::Node *, int32_t,
  // List<TR::Node> *, List<TR::Node> *, List< List<TR::Node> > *, List<TR::Node>
  // *, List< List<TR::Node> > *, List<TR::Node> *, List<TR::Node> *);
  bool examineNode(TR::TreeTop *, TR::Node *, TR::Node *, vcount_t,
                   TR_RematState *, TR_RematAdjustments &);
  void rematerializeNode(TR::TreeTop *, TR::Node *, TR::Node *, vcount_t,
                         List<TR::Node> *, List<TR::Node> *,
                         List<List<TR::Node>> *, List<TR::Node> *,
                         List<List<TR::Node>> *, List<TR::Node> *,
                         List<TR::Node> *, bool);

  void rematerializeAddresses(TR::Node *indirectNode, TR::TreeTop *treeTop,
                              vcount_t visitCount);
  bool isRematerializable(TR::Node *parent, TR::Node *node,
                          bool onlyConsiderOpCode = false);
...
private:
  ...
  int32_t pruneAndPopulateBlockMapper(TR_LinkHeadAndTail<BlockMapper> *bMap,
                                      int32_t depth);
  int32_t synergisticDepthCalculator(TR_LinkHeadAndTail<BlockMapper> *bMap,
                                     TR::Block *startPoint);
  ...
  int32_t processNode(TR::Node *node, int32_t blockIndex,
  ...
  public:
    ...
    uint32_t remove(TR_IndexedBinaryHeapElement *t) {
      TR_ASSERT(false,
                "Don't call remove for a specific object on a binaryHeap!\n");
      return 0;
    }

    uint32_t add(TR_IndexedBinaryHeapElement *t) {
      if (_array[0] && _maxSize > 0 && _nextIndex >= _maxSize &&
      ...
      for (i = _nextIndex; i > 0 && _array[i / 2]->objectGT(t->getObject());
           i /= 2) {
    ...
    uint32_t removeMin() {
      ...
      while (2 * i + 1 < _nextIndex) {
        ...
        if (child + 1 < _nextIndex &&
            _array[child + 1]->objectLT(_array[child]->getObject())) {
          child++;
        }
    ...
    void dumpList(TR::Compilation *comp) {
      ...
      for (uint32_t i = 0; i < _nextIndex; i++) {
        traceMsg(comp, "%d [idx %d], ", _array[i]->getObject()->getNumber(),
                 _array[i]->getIndex());
      }
  ...
  void heapElementQuickSort(TR_Array<TR_IndexedBinaryHeapElement *> *array,
                            int32_t left, int32_t right);
  void quickSortSwap(TR_Array<TR_IndexedBinaryHeapElement *> *array,
                     int32_t left, int32_t right);
...
private:
  ...
  public:
    ...
    void setKnownObjectIndex(TR::KnownObjectTable::Index index) {
      _knownObjectIndex = index;
    }
  ...
  void processIndirectCall(TR::Node *node, TR::TreeTop *treeTop,
                           vcount_t visitCount);
  void processIndirectLoad(TR::Node *node, TR::TreeTop *treeTop,
                           vcount_t visitCount);
...
public:
  TR_CheckcastAndProfiledGuardCoalescer(TR::OptimizationManager *manager)
      : TR::Optimization(manager){};
  static TR::Optimization *create(TR::OptimizationManager *manager) {
    return new (manager->allocator())
        TR_CheckcastAndProfiledGuardCoalescer(manager);
  }
...
public:
  ...
  // Use this as follows:
  //
  // TR_ByteCodeInfo oldInfo =
  // temporarilySetProfilingBcInfoOnNewArrayLengthChild(newArray); (do whatever)
  // newArray->getFirstChild()->setByteCodeInfo(oldIfdo);
  //
  static TR_ByteCodeInfo
  temporarilySetProfilingBcInfoOnNewArrayLengthChild(TR::Node *newArray,
                                                     TR::Compilation *comp);
};

// Look for simple anchored treetops that can be removed
// This is important to clean up the noOpt code for some WCode frontends that
// generate a lot of temporary symbols
class TR_TrivialDeadTreeRemoval : public TR::Optimization {
public:
  TR_TrivialDeadTreeRemoval(TR::OptimizationManager *manager)
      : TR::Optimization(manager), _currentTreeTop(NULL), _currentBlock(NULL),
  ...
  static void preProcessTreetop(TR::TreeTop *treeTop,
                                List<TR::TreeTop> &commonedTreeTopList,
                                char *optDetails, TR::Compilation *comp);
  static void postProcessTreetop(TR::TreeTop *treeTop,
                                 List<TR::TreeTop> &commonedTreeTopList,
                                 char *optDetails, TR::Compilation *comp);
  static void processCommonedChild(TR::Node *child, TR::TreeTop *treeTop,
                                   List<TR::TreeTop> &commonedTreeTopList,
                                   char *optDetails, TR::Compilation *comp);
