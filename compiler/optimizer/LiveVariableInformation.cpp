  ...
  TR_BitVector* commonedLoads, bool belowCommonedNode)
{
  ...
  if (traceLiveVarInfo()) {
    if (movingForwardThroughTrees)
      ...
      traceMsg(comp(),
        ...
        node->getFutureUseCount(), node->getReferenceCount());
  }
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i)
    visitTreeForLocals(node->getChild(i), blockGenSetInfo, blockKillSetInfo, movingForwardThroughTrees, visitEntireTree,
      visitCount, commonedLoads, belowCommonedNode);
        ...
        if (!indirectMethodMetaUses.IsZero()) {
          ...
          for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
            ...
            if (usedSymbol && usedSymbol->getDataType() != TR::NoType) {
              ...
              if (usedSymbolIndex != INVALID_LIVENESS_INDEX && usedSymbolIndex < _numLocals
                && (!blockKillSetInfo || !blockKillSetInfo->get(usedSymbolIndex))) {
                if (traceLiveVarInfo())
