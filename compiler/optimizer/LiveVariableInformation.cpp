...
void
TR_LiveVariableInformation::createGenAndKillSetCaches()
{
...
void
TR_LiveVariableInformation::initializeGenAndKillSetInfo(
...
void
TR_LiveVariableInformation::trackLiveCommonedLoads()
{
  _liveCommonedLoads =
    new (trStackMemory()) TR_BitVector(_numLocals, trMemory());
}
...
void
TR_LiveVariableInformation::findCommonedLoads(TR::Node* node,
                                              ...
                                              vcount_t visitCount)
{
  ...
  visitTreeForLocals(node,
                     NULL,
                     NULL,
                     movingForwardThroughTrees,
                     true,
                     visitCount,
                     commonedLoads,
                     false);
...
void
TR_LiveVariableInformation::findAllUseOfLocal(TR::Node* node,
                                              int32_t blockNum,
...
void
TR_LiveVariableInformation::findLocalUsesInBackwardsTreeWalk(
  TR::Node* node,
  int32_t blockNum,
  TR_BitVector* genSetInfo,
  TR_BitVector* killSetInfo,
...
void
TR_LiveVariableInformation::findUseOfLocal(TR::Node* node,
                                           int32_t blockNum,
                                           TR_BitVector** genSetInfo,
                                           TR_BitVector** killSetInfo,
                                           TR_BitVector* commonedLoads,
...
//    efficiently
void
TR_LiveVariableInformation::visitTreeForLocals(TR::Node* node,
                                               ...
                                               bool belowCommonedNode)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i)
    visitTreeForLocals(node->getChild(i),
                       blockGenSetInfo,
                       blockKillSetInfo,
                       movingForwardThroughTrees,
                       visitEntireTree,
  ...
  if (local || !indirectMethodMetaUses.IsZero() || !regStarAsmUses.IsZero()) {
    ...
    if (belowCommonedNode) {
      ...
      if (_liveCommonedLoads != NULL) {
        if (traceLiveVarInfo())
          traceMsg(comp(),
                   "              Marking %d as live commoned load\n",
                   localIndex);
...

    {
      ...
      //! blockKillSetInfo->get(localIndex))
      {
        if (traceLiveVarInfo() && local)
          traceMsg(comp(),
                   "            Gening symbol with local index %d\n",
                   localIndex);
 ...
 */
void
TR_OSRLiveVariableInformation::findUseOfLocal(TR::Node* node,
                                              int32_t blockNum,
                                              TR_BitVector** genSetInfo,
                                              TR_BitVector** killSetInfo,
                                              TR_BitVector* commonedLoads,
                                              ...
                                              vcount_t visitCount)
{
  TR_LiveVariableInformation::findUseOfLocal(node,
                                             blockNum,
                                             genSetInfo,
                                             killSetInfo,
                                             commonedLoads,
 ...
 */
void
TR_OSRLiveVariableInformation::buildLiveSymbolsBitVector(
 ...
 */
TR_BitVector*
TR_OSRLiveVariableInformation::getLiveSymbolsInInterpreter(
