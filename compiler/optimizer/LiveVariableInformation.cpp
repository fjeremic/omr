    ...
    TR_BitVector* commonedLoads, bool belowCommonedNode)
{
    ...
    if (local || !indirectMethodMetaUses.IsZero() || !regStarAsmUses.IsZero()) {
...

        {
            ...
            // for it
            //
            if (!movingForwardThroughTrees || blockKillSetInfo == NULL || (local && !blockKillSetInfo->get(localIndex)) || !regStarAsmUses.IsZero() || !indirectMethodMetaUses.IsZero())
 ...
 */
void TR_OSRLiveVariableInformation::findUseOfLocal(TR::Node* node, int32_t blockNum,
    TR_BitVector** genSetInfo, TR_BitVector** killSetInfo,
    TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount)
{
 ...
 */
void TR_OSRLiveVariableInformation::buildLiveSymbolsBitVector(TR_OSRMethodData* osrMethodData, int32_t byteCodeIndex, TR_BitVector* liveLocals)
{
