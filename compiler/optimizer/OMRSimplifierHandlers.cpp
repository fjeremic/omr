    ...
    if (konst == 0) {                                                          \
      ...
      if (grandChild->getReferenceCount() == 1)                                \
        ...
        secondChild->setAndIncChild(                                           \
          1,                                                                   \
          TR::Node::create(                                                    \
            grandChild, grandChild->getOpCodeValue(), 0, (int32_t)konst));     \
      }                                                                        \
    }                                                                          \
  ...
  if (secondChild->getOpCode().isLoadConst()) {                                \
    ...
    if (value == ZeroValue) {                                                  \
      if (performTransformation(                                               \
            s->comp(),                                                         \
            "%sFound op with iconst in node [" POINTER_PRINTF_FORMAT "]\n",    \
            s->optDetailString(),                                              \
            node)) {                                                           \
...
static TR::ILOpCodes addOps[TR::NumTypes] = {
  TR::BadILOp, TR::badd,    TR::sadd,    TR::iadd,    TR::ladd,
  TR::fadd,    TR::dadd,    TR::BadILOp, TR::BadILOp, TR::BadILOp,
  TR::BadILOp, TR::BadILOp, TR::BadILOp
};

static TR::ILOpCodes subOps[TR::NumTypes] = {
  TR::BadILOp, TR::bsub,    TR::ssub,   TR::isub,    TR::lsub,
  TR::fsub,    TR::dsub,    TR::asub,   TR::BadILOp, TR::BadILOp,
  TR::BadILOp, TR::BadILOp, TR::BadILOp
};

static TR::ILOpCodes constOps[TR::NumTypes] = {
  TR::BadILOp, TR::bconst,  TR::sconst, TR::iconst,  TR::lconst,
  TR::fconst,  TR::dconst,  TR::aconst, TR::BadILOp, TR::BadILOp,
  TR::BadILOp, TR::BadILOp, TR::BadILOp
};

static TR::ILOpCodes negOps[TR::NumTypes] = {
  TR::BadILOp, TR::bneg,    TR::sneg,    TR::ineg,    TR::lneg,
  TR::fneg,    TR::dneg,    TR::BadILOp, TR::BadILOp, TR::BadILOp,
  TR::BadILOp, TR::BadILOp, TR::BadILOp
};
...
template<typename T>
inline void
setCCAddSigned(T value,
               T operand1,
               T operand2,
               TR::Node* node,
               TR::Simplifier* s)
{
  // for an add (T = A + B) an overflow occurs iff the sign(A) == sign(B) and
  // sign(T) != sign(A)
...
template<typename T>
inline void
setCCAddUnsigned(T value, T operand1, TR::Node* node, TR::Simplifier* s)
{
...
template<typename T>
inline void
setCCSubSigned(T value,
               T operand1,
               T operand2,
               TR::Node* node,
               TR::Simplifier* s)
{
  // for a sub (T = A - B) an overflow occurs iff the sign(A) != sign(B) and
  // sign(T) == sign(B)
...
template<typename T>
inline void
setCCSubUnsigned(T value, T operand1, TR::Node* node, TR::Simplifier* s)
{
...
template<typename T>
inline void
setCCOr(T value, TR::Node* node, TR::Simplifier* s)
{
  if (value == (T)0)
    s->setCC(node, OMR::ConditionCode0);
  else
...
static void
convertToTestUnderMask(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if ((iand = node->getChild(0))->getOpCodeValue() == TR::iand &&
      ...
      (constVal = iand->getChild(1)->getConst<int32_t>()) >=
        TR::getMinSigned<TR::Int8>() &&
      constVal <= TR::getMaxSigned<TR::Int8>() &&
      ...
      !load->getSymbol()->isParm() &&
      load->getOpCode()
        .isLoadIndirect() && // Limit it to non-parm, indirect cases
      load->getReferenceCount() == 1 &&
      performTransformation(cm,
                            "%sTransforming iand/iushr to byte test under mask "
                            "[" POINTER_PRINTF_FORMAT "]\n",
                            s->optDetailString(),
                            node)) {
    // anchor iand subtree
    auto anchor =
      TR::TreeTop::create(s->comp(), TR::Node::create(TR::treetop, 1, iand));
    s->_curTree->insertBefore(anchor);

    auto bloadi = TR::Node::createWithSymRef(
      TR::bloadi, 1, load->getChild(0), 0, load->getSymbolReference());
    node->setAndIncChild(
      0,
      TR::Node::create(
        TR::butest,
...
static bool
isBitwiseIntComplement(TR::Node* n)
{
...
static void
setExprInvariant(TR_RegionStructure* region, TR::Node* node)
{
...
template<typename T>
inline void
foldConstant(TR::Node* node, T value, TR::Simplifier* s, bool anchorChildrenP)
{
...
template<typename T>
static TR::Node*
binaryIdentityOp(TR::Node* node, T nil, TR::Simplifier* s)
{
...
static bool
isIMulComposerOpCode(TR::Node* node)
{
  TR::ILOpCodes childOp = node->getOpCodeValue();

  if (childOp == TR::iadd || childOp == TR::isub || childOp == TR::imul ||
      childOp == TR::ineg)
    return true;
...
bool
imulComposer(TR::Node* node, int* value, TR::Node** multiplier)
{
  TR::Node* firstChild;
  TR::Node* secondChild;
  ...
  *value = 0;
  ...
  if (node->getOpCodeValue() == TR::iadd ||
    ...
    *multiplier = lMultiplier;
    ...
    if (secondChild->getOpCodeValue() == TR::iconst &&
        isNonNegativePowerOf2(secondChild->getInt())) {
...
static bool
isSmallConstant(TR::Node* node, TR::Simplifier* s)
{
...
static void
reassociateBigConstants(TR::Node* node, TR::Simplifier* s)
{
  if (s->reassociate() &&
      ...
      if (base_node != node && base_node->getReferenceCount() >= 1 &&
        ...
        if (!s->comp()->cg()->shouldValueBeInACommonedNode(diff) &&
            performTransformation(
              s->comp(),
              "%sReusing big constant from node 0x%p in node 0x%p\n",
              s->optDetailString(),
              base_node,
              node)) {
          ...
          TR::Node* diff_node =
            TR::Node::create(node, node->getSecondChild()->getOpCodeValue(), 0);
...
static TR::Node*
addSimplifierCommon(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (s->reassociate() && node->getOpCode().isArrayRef()) {
    ...
    if (firstChild->getOpCodeValue() == node->getOpCodeValue() &&
      ...
      if (performTransformation(s->comp(),
                                "%sReordering constant terms in node 0x%p\n",
                                s->optDetailString(),
                                node)) {
        TR::Node* newFirstChild =
          TR::Node::create(firstChild, node->getOpCodeValue(), 2);
  ...
  if (s->reassociate() && node->getOpCode().isAdd()) {
    ...
    if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) &&
      ...
      if (performTransformation(s->comp(),
                                "%sApplied reassociation rule 9 to node 0x%p\n",
                                s->optDetailString(),
                                node)) {
        TR::Node* newSecondChild = TR::Node::create(
          secondChild,
        ...
        if (c1->getDataType() != secondChild->getDataType()) {
          TR::ILOpCodes conversionOpCode =
            TR::ILOpCode::getProperConversion(c1->getDataType(),
    ...
    //         / \        / \
      ...
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 9_1 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node* newFirstChild =
          TR::Node::create(firstChild, node->getOpCodeValue(), 2);
      ...
      //    c  t1              t1 t2
             ...
             (!isExprInvariant(region, secondChild) ||
              (node->getOpCode().isArrayRef() &&
               isSmallConstant(secondChild, s))) &&
             firstChild->getSecondChild()->getDataType() ==
               secondChild->getDataType()) {
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 9_1_1 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node* newSecondChild =
          TR::Node::create(secondChild, addOps[secondChild->getDataType()], 2);
    ...
    //     / \            / \
      //    c1  t         c1  c2
             ...
             firstChild->getFirstChild()->getOpCode().isLoadConst() &&
             secondChild->getOpCode().isLoadConst() &&
             firstChild->getFirstChild()->getDataType() ==
               secondChild->getDataType()) {
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 9_1_2 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node* newFirstChild =
          TR::Node::create(firstChild, node->getOpCodeValue(), 2);
    ...
    //     / \   / \        / \   / \       / \   / \       / \   / \
      ...
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 9_1_3 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node* newFirstChild =
          TR::Node::create(firstChild, node->getOpCodeValue(), 2);
        ...
        TR::Node* newSecondChild =
          TR::Node::create(secondChild, node->getOpCodeValue(), 2);
        newSecondChild->setAndIncChild(0, firstChild->getSecondChild());
    ...
    //         / \        / \
      //        t   c2     c1 c2
    else if (node->getOpCode().isAdd() && !node->getOpCode().isArrayRef() &&
             isExprInvariant(region, firstChild) &&
             secondChild->getOpCode().isSub() &&
             firstChild->getDataType() ==
               secondChild->getSecondChild()->getDataType() &&
             !isExprInvariant(region, secondChild->getFirstChild()) &&
             isExprInvariant(region, secondChild->getSecondChild())) {
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 9_2 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node* newFirstChild =
          TR::Node::create(firstChild, subOps[firstChild->getDataType()], 2);
        newFirstChild->setAndIncChild(0, firstChild);
        newFirstChild->setAndIncChild(1, secondChild->getSecondChild());
    ...
    //                         \
      //                          c2
             ...
             !secondChild->getFirstChild()
                ->getOpCode()
                .isRef() && // can't move ref to the RHS

             (!isExprInvariant(region, secondChild->getFirstChild()) ||
              isSmallConstant(secondChild->getFirstChild(),
                              s)) && // better address expression

             isExprInvariant(region, secondChild->getSecondChild()) &&
             !isSmallConstant(secondChild->getSecondChild(), s)) {
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 9_3 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node* newFirstChild =
          TR::Node::create(firstChild, node->getOpCodeValue(), 2);
        TR::Node* newNeg =
          TR::Node::create(secondChild->getSecondChild(),
                           negOps[secondChild->getSecondChild()->getDataType()],
                           1);
        TR::Node* zero = TR::Node::create(
          secondChild->getSecondChild(),
          constOps[secondChild->getSecondChild()->getDataType()],
          0);
...
template<class T>
static TR::Node*
addSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      ...
      if (node->nodeRequiresConditionCodes())
        setCCAddSigned<T>(value,
                          firstChild->getConst<T>(),
                          secondChild->getConst<T>(),
                          node,
                          s);
    }
  ...
  // normalize adds of positive constants to be isubs.  Have to do it this way
  // because more negative value than positive in a 2's complement signed
  // representation
  if (nodeOp.isAdd() && secondChildOp.isLoadConst() &&
      secondChild->getConst<T>() > 0) {
    if (performTransformation(
          s->comp(),
          "%sNormalized xadd of xconst > 0 in node [%s] to xsub of -xconst\n",
          s->optDetailString(),
    ...
    if (TR::DataType::isSignedInt32<T>() && // this code has not been ported to
                                            // templates yet
        secondChildOp.isLoadConst() && secondChild->getConst<T>() == -1) {
      if (performTransformation(s->comp(),
                                "%sReduced xadd of -1 and an xneg in node [%s] "
                                "to bitwise complement\n",
                                s->optDetailString(),
                                node->getName(s->getDebug()))) {
        s->anchorChildren(node, s->_curTree);
    ...
    // Turn add with first child negated into a subtract
               ...
               s->comp(),
               "%sReduced iadd with negated first child in node [%s] to isub\n",
               s->optDetailString(),
               node->getName(s->getDebug()))) {
      s->anchorChildren(node, s->_curTree);
      TR::Node::recreate(node, TR::ILOpCode::getSubOpCode<T>());
  ...
  // turn add with other child negated in to a subtract
    ...
    if (performTransformation(
          s->comp(),
          "%sReduced iadd with negated second child in node [%s] to isub\n",
          s->optDetailString(),
          node->getName(s->getDebug()))) {
      s->anchorChildren(node, s->_curTree);
  ...
  } else if (TR::DataType::isSignedInt32<T>() && // this code has not been
                                                 // ported to templates yet
             imulComposer(node, (int*)&iMulComposerValue, &iMulComposerChild)) {
    bool iMulDecomposeReport =
      s->comp()->getOptions()->getTraceSimplifier(TR_TraceMulDecomposition);
    ...
    if (s->getLastRun() &&
        s->comp()->cg()->codegenMulDecomposition((int64_t)iMulComposerValue) &&
        performTransformation(s->comp(),
                              "%sFactored iadd with distributed imul with a "
                              "codegen decomposible constant in node [%s]\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      ...
      if (iMulDecomposeReport)
        dumpOptDetails(
          s->comp(),
          "Putting the node back to imul with %d, for node [%s]. \n",
          iMulComposerValue,
      ...
      constNode = TR::Node::create(
        node, TR::ILOpCode::getConstOpCode<T>(), 0, iMulComposerValue);
  ...
  // reduce imul operations by factoring
  else if (firstChildOp.isMul() && firstChild->getReferenceCount() == 1 &&
           secondChildOp.isMul() && secondChild->getReferenceCount() == 1) {
    ...
    // if (performTransformation(s->comp(), "%sFactored iadd with distributed
    // imul in node [%s]\n", s->optDetailString(),
    // node->getName(s->getDebug())))
    {
      if (llChild == rlChild) {
      ...
      if (factorChild && !s->getLastRun() &&
          (region = s->containingStructure()) &&
          (isExprInvariant(region, secondChild->getFirstChild()) !=
             isExprInvariant(region, secondChild->getSecondChild()) ||
           isExprInvariant(region,
                           factorChild))) // to avoid conflict with Rule 13
      {
        factorChild = NULL;
      }
      ...
      if (!factorChild ||
          !performTransformation(
            s->comp(),
            "%sFactored iadd with distributed imul in node [%s]\n",
            s->optDetailString(),
            node->getName(s->getDebug()))) {
        ...
        node->setChild(1, s->simplify(secondChild, block));
             ...
             !((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() &&
                ...
                s->_curTree->getNode()
                  ...
                  ->getOpCode()
                  .isMemToMemOp()) &&
               (s->_curTree->getNode()->getFirstChild()->getLastChild() ==
                node)) &&
    ...
    if (lrChild->getOpCode().isLoadConst()) {
      if (secondChildOp.isLoadConst()) {
        if (!s->reassociate() && // use new rules
            performTransformation(s->comp(),
                                  "%sFound xadd of xconst with xadd or xsub of "
                                  "x and const in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
            // it makes sense to uncommon it in this situation
            TR::Node* newFirstChild =
              TR::Node::create(node, firstChildOp.getOpCodeValue(), 2);
            newFirstChild->setAndIncChild(0, firstChild->getFirstChild());
          ...
          if (secondChild->getReferenceCount() == 1) {
            ...
            TR::Node* foldedConstChild = TR::Node::create(
              secondChild, TR::ILOpCode::getConstOpCode<T>(), 0);
          ...
          s->_alteredBlock = true;
                 ...
                 (firstChild->getReferenceCount() == 1) &&
                 performTransformation(s->comp(),
                                       "%sFound xadd of non-xconst with xadd "
                                       "or isub of x and const in node [%s]\n",
                                       s->optDetailString(),
                                       node->getName(s->getDebug()))) {
    ...
    if ((lrChild->getOpCode().isLoadConst()) &&
      ...
      if (performTransformation(
            s->comp(),
            "%sFound aiadd of iconst with aiadd x and iconst in node [%s]\n",
            s->optDetailString(),
            node->getName(s->getDebug()))) {
        T value = secondChild->getConst<T>();
        value += lrChild->getConst<T>();
...
template<class T>
TR::Node*
subSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  scount_t futureUseCountOfFirstChild =
    node->getFirstChild()->getFutureUseCount();
  ...
  if (!node->nodeRequiresConditionCodes()) // don't do identity op on cc nodes
                                           // on zemulator
  {
    TR::Node* identity = 0;
    if (0 != (identity = binaryIdentityOp<T>(node, 0, s)))
  ...
  if (firstChild == secondChild) {
    if (node->nodeRequiresConditionCodes()) {
      setCCSubSigned<T>(
        0, firstChild->getConst<T>(), secondChild->getConst<T>(), node, s);
    }
  ...
  if (node->getOpCodeValue() != TR::aiadd &&
    ...
    if ((rrChild->getOpCode().isLoadConst()) && (rlChild == firstChild) &&
        node->cannotOverflow() && secondChild->cannotOverflow()) {
      if (performTransformation(s->comp(),
                                "%sFolded isub with children related through "
                                "iconst in node [%s] to iconst \n",
                                s->optDetailString(),
                                node->getName(s->getDebug()))) {
        node->setChild(0, 0);
        node->setChild(1, 0);
  ...
  // normalize subtracts of positive constants to be iadds.  Have to do it this
  // way because more negative value than positive in a 2's complement signed
  // representation
  else if (secondChildOp.isLoadConst() && secondChild->getConst<T>() > 0) {
    if (performTransformation(
          s->comp(),
          "%sNormalized isub of iconst > 0 in node [%s] to iadd of -iconst \n",
          s->optDetailString(),
          node->getName(s->getDebug()))) {
  ...
  // turn isub with second child ineg into an iadd
    ...
    if (performTransformation(
          s->comp(),
          "%sReduced xsub with negated second child in node [%s] to xadd\n",
          s->optDetailString(),
          node->getName(s->getDebug()))) {
      TR::Node* newSecondChild = secondChild->getFirstChild();
      TR::Node::recreate(node, TR::ILOpCode::getAddOpCode<T>());
  ...
  // turn isub with first child ineg into an ineg with iadd child.  isn't
  // quicker, but normalizes expressions and drives inegs up the tree
  else if (firstChildOp.isNeg()) {
    if (performTransformation(s->comp(),
                              "%sReduced xsub with negated first child in node "
                              "[%s] to ineg of xadd\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      TR::Node* newFirstChild = firstChild->getFirstChild();
      ...
      TR::Node* newNode =
        TR::Node::create(node, TR::ILOpCode::getAddOpCode<T>(), 2);
  ...
  else if (isBitwiseIntComplement(firstChild) && secondChildOp.isLoadConst() &&
           secondChild->getConst<T>() == -1) {
    if (performTransformation(s->comp(),
                              "%sReduced isub of bitwise complement and iconst "
                              "-1 in node [%s] to 2s complement negation\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      ...
      s->_alteredBlock = true;
                                                 ...
                                                 // ported to templates yet
             imulComposer(
               node, (int*)&MulComposerValue, &MulComposerChild)) // TODO
  {
    bool iMulDecomposeReport =
      s->comp()->getOptions()->getTraceSimplifier(TR_TraceMulDecomposition);
    if (iMulDecomposeReport)
    ...
    if (s->getLastRun() &&
        s->comp()->cg()->codegenMulDecomposition((int64_t)MulComposerValue) &&
        performTransformation(s->comp(),
                              "%sFactored iadd with distributed imul with a "
                              "codegen decomposible constant in node [%s]\n",
                              s->optDetailString(),
      ...
      constNode = TR::Node::create(
        node, TR::ILOpCode::getConstOpCode<T>(), 0, MulComposerValue);
      node->setAndIncChild(1, constNode);
  ...
  // reduce imul operations by factoring
    ...
    TR::Node* factorChild = NULL;
    // moved this check down:
    // if (performTransformation(s->comp(), "%sFactored isub with distributed
    // imul in node [%s]\n", s->optDetailString(),
    // node->getName(s->getDebug())))
    {
      if (llChild == rlChild) {
      ...
      if (!factorChild ||
          !performTransformation(
            s->comp(),
            "%sFactored xsub with distributed xmul in node [%s]\n",
            s->optDetailString(),
            node->getName(s->getDebug()))) {
        secondChild->setChild(0, rlChild);
        secondChild->setChild(1, rrChild);
        ...
        TR::Node::recreate(node, TR::ILOpCode::getMulOpCode<T>());
        node
          ->setChild(factorChild->getOpCode().isLoadConst() ? 1 : 0,
                     factorChild)
          ->decReferenceCount();
        ...
        node->setVisitCount(0);
        s->_alteredBlock = true;
        node->setChild(factorChild->getOpCode().isLoadConst() ? 0 : 1,
                       s->simplify(secondChild, block));
      }
    }
  }
  ...
  //   that problem, the test should be moved back down there.
    ...
    if (lrChild->getOpCode().isLoadConst()) {
      if (secondChildOp.isLoadConst()) {
        if (performTransformation(s->comp(),
                                  "%sFound xsub of xconst with xadd or xsub of "
                                  "x and const in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
        ...
        if ((firstChild->getReferenceCount() == 1) &&
            (performTransformation(s->comp(),
                                   "%sFound xsub of non-xconst with xadd or "
                                   "xsub of x and const in node [%s]\n",
                                   s->optDetailString(),
                                   node->getName(s->getDebug())))) {
        // move constants up the tree so they will tend to get merged together
        node->setChild(1, lrChild);
...
// normalize shift constant so that we can common, fold and strength reduce more
// reliably
static void
normalizeConstantShiftAmount(TR::Node* node,
                             int32_t shiftMask,
                             TR::Node*& secondChild,
                             TR::Simplifier* s)
{
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (shiftValue != secondChild->getInt()) {
      if (performTransformation(
            s->comp(),
            "%sReducing constant of node [%s] from %d to %d\n",
            s->optDetailString(),
            secondChild->getName(s->getDebug()),
            secondChild->getInt(),
            shiftValue)) {
        if (secondChild->getReferenceCount() > 1) {
...
static void
normalizeShiftAmount(TR::Node* node,
                     int32_t normalizationConstant,
                     TR::Simplifier* s)
{
  if (s->comp()->cg()->needsNormalizationBeforeShifts() &&
    ...
    // to obey Java semantics. This can be captured in the IL and
    // commoning/hoisting can be done (look at Compressor.compress).
    //
    if ((secondChild->getOpCodeValue() != TR::iconst) &&
      ...
      if (performTransformation(
            s->comp(),
            "%sPlatform specific normalization of shift node [%s]\n",
            s->optDetailString(),
            node->getName(s->getDebug()))) {
        //
        // Not normalized yet
...
static void
orderChildrenByHighWordZero(TR::Node* node,
                            TR::Node*& firstChild,
                            TR::Node*& secondChild,
                            TR::Simplifier* s)
{
  if (!secondChild->getOpCode().isLoadConst() &&
      secondChild->isHighWordZero() && !firstChild->isHighWordZero()) {
    swapChildren(node, firstChild, secondChild, s);
  }
}

static TR::Node*
foldDemotionConversion(TR::Node* node,
                       TR::ILOpCodes opcode,
                       TR::ILOpCodes foldedOpCode,
                       TR::Simplifier* s)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (firstChild->getOpCodeValue() == opcode &&
      performTransformation(
        s->comp(),
        "%sFolding conversion node [%s] %s and its child [%s] %s\n",
        s->optDetailString(),
        node->getName(s->getDebug()),
        node->getOpCode().getName(),
        firstChild->getName(s->getDebug()),
        firstChild->getOpCode().getName())) {
...
//---------------------------------------------------------------------
// Reduce long operations to int operations if the result is going to be
// converted back to an int result.
//
static bool
reduceLongOp(TR::Node* node,
             TR::Block* block,
             TR::Simplifier* s,
             TR::ILOpCodes newConversionOp)
{
  ...
  switch (firstChild->getOpCodeValue()) {
    ...
    case TR::lshr:
      if (node->getOpCodeValue() == TR::l2s &&
          firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        if ((firstChild->getSecondChild()->get64bitIntegralValue() &
             LONG_SHIFT_MASK) <= 16) {
    ...
    case TR::lshl:
      if (firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        if ((firstChild->getSecondChild()->get64bitIntegralValue() &
          ...
          if (!performTransformation(
                s->comp(),
                "%sReducing long operation in node [" POINTER_PRINTF_FORMAT
                "] to an int operation\n",
                s->optDetailString(),
                node))
            return false;
          ...
          if (newConversionOp == TR::BadILOp) {
            ...
            TR::Node::recreate(firstChild,
                               isUnsigned ? TR::iuconst : TR::iconst);
    ...
    case TR::luneg: {
      ...
      if (newConversionOp == TR::BadILOp) {
        ...
        TR::Node* temp =
          TR::Node::create(TR::l2i, 1, firstChild->getFirstChild());
...
static void
fold2SmallerIntConstant(TR::Node* node,
                        TR::Node* child,
                        TR::DataType sdt,
                        TR::DataType tdt,
                        TR::Simplifier* s)
  ...
  switch (tdt) {
    ...
    case TR::Int16:
      foldShortIntConstant(
        node, (val << 16) >> 16, s, false /* !anchorChildren */);
      break;
    case TR::Int32:
...
static TR::Node*
intDemoteSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);

  TR::ILOpCode op = node->getOpCode();

  TR::DataType sourceDataType(TR::DataTypes::NoType),
    targetDataType(TR::DataTypes::NoType);

  if (!decodeConversionOpcode(
        op, node->getDataType(), sourceDataType, targetDataType))
    return node;

  TR::ILOpCodes inverseOp1 = TR::ILOpCode::getProperConversion(
    targetDataType, sourceDataType, false); // sign extend
  TR::ILOpCodes inverseOp2 = TR::ILOpCode::getProperConversion(
    targetDataType, sourceDataType, true); // zero extend
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    fold2SmallerIntConstant(
      node, firstChild, sourceDataType, targetDataType, s);
    return node;
  }
  ...
  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, inverseOp1)))
    return result;

  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, inverseOp2)))
    return result;
  ...
  //     i)  Tx is 64 bit type
  //     ii) Tz is integral
  //    iii) Tx 2 Tz would be a demotion (and not promotion) which is checked
  //    for
  //
  ...
  TR::DataType childSourceDataType(TR::DataTypes::NoType),
    childTargetDataType(TR::DataTypes::NoType);

  if (sourceIs64Bit &&
      decodeConversionOpcode(childOp,
                             firstChild->getDataType(),
                             childSourceDataType,
                             childTargetDataType) &&
      (childSourceDataType != targetDataType)) {
    bool wantZeroExtension = childOp.isZeroExtension();
    uint32_t childSourceSize =
      TR::ILOpCode::getSize(TR::ILOpCode::getProperConversion(
        childTargetDataType,
        childSourceDataType,
        wantZeroExtension)); // size of childSourceDataType (hack)
    if (childSourceDataType.isIntegral() && (childSourceSize < sourceSize)) {
      bool wantZeroExtension = childOp.isZeroExtension();
      TR::ILOpCodes foldOp = TR::ILOpCode::getProperConversion(
        childSourceDataType, targetDataType, wantZeroExtension);
      result =
        foldDemotionConversion(node, childOp.getOpCodeValue(), foldOp, s);
      if (result)
        return result;
    }
  }
...
static bool
hasCommonedChild(TR::Node* node, TR::SparseBitVector& isVisited)
{
  TR::ILOpCode& op = node->getOpCode();
...
getUniqueSymRef(TR::Node* node, bool& isLegal, TR::SparseBitVector& isVisited)
{
  TR::ILOpCode& op = node->getOpCode();
  isVisited[node->getGlobalIndex()] = true;
...
// 1. an ifcmp is comparing with a const (or variable a), it is the first real
// instruction in the block
//    the other operand is defined in another blocks, at least one of the def is
//    const (or a), We move this ifcmp to after that def, hopefully the ifcmp
//    can be resolved at compile time
// 2. TODO: for cmp,
static void
partialRedundantCompareElimination(TR::Node* node,
                                   TR::Block* block,
                                   TR::Simplifier* s)
{
  // return;
  ...
  if (!disableSPMD) {
    // PRBE prevents unrolling the kernel loop for SIMDization by confusing loop
    // canonizalization if the range is known at the compile time
  ...
  if (node->getSecondChild()->getOpCode().isLoadConst() &&
    ...
    TR::SymbolReference* symRef =
      getUniqueSymRef(node->getFirstChild(), isLegal, isVisited);
    ...
    if (!onlyOnePred) {
      ...
      for (auto predEdge = predecessors.begin();
           predEdge != predecessors.end();) {
        ...
        if (gotoTree) {
          bool predEndWithGoto =
            (gotoTree->getNode()->getOpCodeValue() == TR::Goto);
          bool isImmediatePred =
            (predBlock->getExit()->getNextTreeTop() == block->getEntry());
          // Don't need to do anything if only has on pred and it's immmediate
          // pred
          if (predEndWithGoto || isImmediatePred) {
            TR::TreeTop* storeTree =
              predEndWithGoto ? gotoTree->getPrevTreeTop() : gotoTree;
            TR::Node* storeNode = storeTree->getNode();
            TR::SymbolReference* storeSymRef =
              storeNode->getOpCode().hasSymbolReference()
            ...
            if (storeNode->getOpCode().isStoreDirect() &&
              ...
              // ...
              //
              // also consider the case of block_A is the immediate predecessor
              // of block_B check if it is feeding the ifcmp if (num >
              // comp->getOptions()->getLastIpaOptTransformationIndex()) return;
              // num++;
              if (performTransformation(
                    comp,
                    "%sDuplicating %p and insert it after %p\n",
                    s->optDetailString(),
                    node,
                    storeNode)) {
                TR::Block* ifCmpBlock = block;
                TR_RegionStructure* parent =
                  predBlock->getCommonParentStructureIfExists(fallThroughBB,
                                                              cfg);
                TR::Block* branchDestBB =
                  node->getBranchDestination()->getNode()->getBlock();

                // Create new ifcmp block
                TR::Block* newIfCmpBlock =
                  TR::Block::createEmptyBlock(block->getEntry()->getNode(),
                                              comp,
                                              predBlock->getFrequency(),
                                              predBlock);

                // Create new gotoBlock
                TR::Block* gotoBlock =
                  TR::Block::createEmptyBlock(gotoTree->getNode(),
                                              comp,
                                              predBlock->getFrequency(),
                                              predBlock);
                ...
                // traceMsg(comp, "\n AAA num=%d adding edge between block %d to
                // block %d\n", num, predBlock->getNumber(),
                // newIfCmpBlock->getNumber());
    ...
    if (didSomething) {
      ...
      // it's safe to unset this flags since ifcmp doesn't have any child with
      // refrerenceCount >1
...
static void
addressCompareConversion(TR::Node* node, TR::Simplifier* s)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (firstOp == TR::a2i &&
      firstChild->getFirstChild()->getType().isAddress() &&
      TR::Compiler->target.is32Bit() && firstChild->getReferenceCount() == 1) {
    if ((secondOp == TR::iconst && secondChild->getInt() == 0) ||
      ...
      if (secondOp == TR::a2i) {
        ...
        dumpOptDetails(
          s->comp(),
          "Address Compare Conversion: found both children a2i in node %p\n",
          node);
      } else if (secondOp == TR::iconst) {
        if (secondChild->getReferenceCount() > 1) {
          TR::Node* newSecondChild =
            TR::Node::aconst(secondChild, secondChild->getInt(), 4);
          secondChild->recursivelyDecReferenceCount();
          node->setAndIncChild(1, newSecondChild);
        ...
        dumpOptDetails(s->comp(),
                       "Address Compare Conversion: found child 1 a2i and "
                       "child 2 iconst in node %p\n",
                       node);
      }
    }
  } else if (firstOp == TR::a2l &&
             firstChild->getFirstChild()->getType().isAddress() &&
    ...
    if ((secondOp == TR::lconst && secondChild->getLongInt() == 0) ||
      ...
      if (secondOp == TR::a2l) {
        ...
        dumpOptDetails(
          s->comp(),
          "Address Compare Conversion: found both children a2l in node %p\n",
          node);
      } else if (secondOp == TR::lconst) {
        if (secondChild->getReferenceCount() > 1) {
          TR::Node* newSecondChild =
            TR::Node::aconst(secondChild, secondChild->getLongInt(), 8);
          secondChild->recursivelyDecReferenceCount();
          node->setAndIncChild(1, newSecondChild);
          newSecondChild->setIsClassPointerConstant(false);
        } else {
        ...
        dumpOptDetails(s->comp(),
                       "Address Compare Conversion: found child 1 a2l and "
                       "child 2 lconst in node %p\n",
                       node);
      }
    }
  }
...
static void
longCompareNarrower(TR::Node* node,
                    TR::Simplifier* s,
                    TR::ILOpCodes intOp,
                    TR::ILOpCodes charOp,
                    TR::ILOpCodes shortOp,
                    TR::ILOpCodes byteOp)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  //    performTransformation(s->comp(), "%sLong compare narrower for node
  //    [%p]\n", s->optDetailString(), node))
  {
    ...
    if ((firstOp == TR::i2l) &&
        performTransformation(s->comp(),
                              "%sLong compare narrower for node [%p]\n",
                              s->optDetailString(),
                              node)) {
      if (secondOp == TR::iconst || secondOp == TR::i2l ||
        ...
        if (secondOp == TR::i2l) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(
              s->comp(),
              "Long Compare Narrower: found both children i2l in method %s\n",
              s->comp()->signature());
          }
        } else if (secondOp == TR::lconst) {
          if (secondChild->getReferenceCount() == 1) {
            ...
            secondChild->decReferenceCount();
            TR::Node* iConstNode = TR::Node::create(
              node, TR::iconst, 0, (int32_t)secondChild->getLongInt());
            node->setAndIncChild(1, iConstNode);
          }
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Long Compare Narrower: found child 1 i2l and child "
                           "2 lconst in iconst range in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(),
                         "Long Compare Narrower: found child 1 i2l and child 2 "
                         "iconst in method %s\n",
                         s->comp()->signature());
        }
      }
      ...
      if ((val1 & 0xffffffff00000000ull) == 0 &&
          ...
          performTransformation(
            s->comp(),
            "%sLong compare narrower: equality with no upper bits [%p]\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, intOp);
        node->notifyChangeToValueOfNode();
      ...
      if (firstOp == TR::su2l) {
        if (secondOp == TR::cconst || secondOp == TR::su2l ||
          ...
          if (secondOp == TR::su2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(
                s->comp(),
                "Long Compare Narrower: found both children c2l in method %s\n",
                s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
            if (secondChild->getReferenceCount() == 1) {
              TR::Node::recreate(secondChild, TR::cconst);
              secondChild->setConst<uint16_t>(
                (uint16_t)secondChild->getLongInt());
              secondChild->notifyChangeToValueOfNode();
            } else {
              secondChild->decReferenceCount();
              TR::Node* cConstNode =
                TR::Node::cconst(node, (uint16_t)secondChild->getLongInt());
              node->setAndIncChild(1, cConstNode);
            }
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                             "Long Compare Narrower: found child 1 c2l and "
                             "child 2 lconst in cconst range in method %s\n",
                             s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Long Compare Narrower: found child 1 c2l and child "
                           "2 cconst in method %s\n",
                           s->comp()->signature());
          }
        }
        ...
        if (secondOp == TR::sconst || secondOp == TR::s2l ||
            (secondOp == TR::lconst &&
             secondChild->getLongInt() >=
               s->cg()->getMinShortForLongCompareNarrower() &&
          ...
          if (secondOp == TR::s2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(
                s->comp(),
                "Long Compare Narrower: found both children s2l in method %s\n",
                s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
            if (secondChild->getReferenceCount() == 1) {
              ...
              secondChild->decReferenceCount();
              TR::Node* sConstNode =
                TR::Node::sconst(node, (int16_t)secondChild->getLongInt());
              node->setAndIncChild(1, sConstNode);
            }

            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                             "Long Compare Narrower: found child 1 s2l and "
                             "child 2 lconst in sconst range in method %s\n",
                             s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Long Compare Narrower: found child 1 s2l and child "
                           "2 sconst in method %s\n",
                           s->comp()->signature());
          }
        }
        ...
        if (secondOp == TR::bconst || secondOp == TR::b2l ||
            (secondOp == TR::lconst &&
             secondChild->getLongInt() >=
               s->cg()->getMinByteForLongCompareNarrower() &&
          ...
          if (secondOp == TR::b2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(
                s->comp(),
                "Long Compare Narrower: found both children b2l in method %s\n",
                s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
            if (secondChild->getReferenceCount() == 1) {
              ...
              secondChild->decReferenceCount();
              TR::Node* bConstNode =
                TR::Node::bconst(node, (int8_t)secondChild->getLongInt());
              node->setAndIncChild(1, bConstNode);
            }
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                             "Long Compare Narrower: found child 1 b2l and "
                             "child 2 lconst in bconst range in method %s\n",
                             s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Long Compare Narrower: found child 1 b2l and child "
                           "2 bconst in method %s\n",
                           s->comp()->signature());
          }
        }
      }
...
static void
ifjlClassSimplifier(TR::Node* node, TR::Simplifier* s)
{
  static char* disableJLClassSimplification =
    feGetEnv("TR_DisableJLClassSimplification");
  ...
  if (node->getFirstChild()->getOpCode().hasSymbolReference() &&
      ...
      (node->getFirstChild()->getSymbolReference() ==
       node->getSecondChild()->getSymbolReference()) &&
      (node->getFirstChild()->getSymbolReference() ==
       s->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) &&
      performTransformation(s->comp(),
                            "%sSimplify test on j/l/Class children [%p]\n",
                            s->optDetailString(),
                            node)) {
...
static void
transformToLongDivBy10Bitwise(TR::Node* origNode,
                              TR::Node* node,
                              TR::Simplifier* s)
{
  ...
  */
  ...
  // n = n + (n>>63 & 9);  (nodes n1, n2, n3)
  TR::Node* n2Node = TR::Node::create(
    TR::lshr, 2, firstChild, TR::Node::create(secondChild, TR::iconst, 0, 63));
  TR::Node* temp = NULL;
  TR::Node* n3Node = TR::Node::create(
    TR::land, 2, n2Node, temp = TR::Node::create(secondChild, TR::lconst, 0));
  temp->setLongInt(9);
  TR::Node* n1Node = TR::Node::create(TR::ladd, 2, firstChild, n3Node);

  // q = (n >> 1) + (n >> 2);  (nodes q1, q2, q3)
  TR::Node* q1Node = TR::Node::create(
    TR::lshr, 2, n1Node, TR::Node::create(secondChild, TR::iconst, 0, 1));
  TR::Node* q3Node = TR::Node::create(
    TR::lshr, 2, n1Node, TR::Node::create(secondChild, TR::iconst, 0, 2));
  TR::Node* q2Node = TR::Node::create(TR::ladd, 2, q1Node, q3Node);

  // q = q + (q >> 4);  (nodes q4, q5)
  TR::Node* q5Node = TR::Node::create(
    TR::lshr, 2, q2Node, TR::Node::create(secondChild, TR::iconst, 0, 4));
  TR::Node* q4Node = TR::Node::create(TR::ladd, 2, q2Node, q5Node);

  // q = q + (q >> 8);  (nodes q6, q7)
  TR::Node* q7Node = TR::Node::create(
    TR::lshr, 2, q4Node, TR::Node::create(secondChild, TR::iconst, 0, 8));
  TR::Node* q6Node = TR::Node::create(TR::ladd, 2, q4Node, q7Node);

  // q = q + (q >> 16); (nodes q8, q9)
  TR::Node* q9Node = TR::Node::create(
    TR::lshr, 2, q6Node, TR::Node::create(secondChild, TR::iconst, 0, 16));
  TR::Node* q8Node = TR::Node::create(TR::ladd, 2, q6Node, q9Node);

  // q = q + (q >> 32); (nodes q10, q11)
  TR::Node* q11Node = TR::Node::create(
    TR::lshr, 2, q8Node, TR::Node::create(secondChild, TR::iconst, 0, 32));
  TR::Node* q10Node = TR::Node::create(TR::ladd, 2, q8Node, q11Node);

  // q = q >> 3;  (nodes 12)
  TR::Node* q12Node = TR::Node::create(
    TR::lshr, 2, q10Node, TR::Node::create(secondChild, TR::iconst, 0, 3));

  // r = n - q*10; (nodes r1, r2)
  TR::Node* r2Node = TR::Node::create(
    TR::lmul, 2, q12Node, temp = TR::Node::create(secondChild, TR::lconst, 0));
  temp->setLongInt(10);
  TR::Node* r1Node = TR::Node::create(TR::lsub, 2, n1Node, r2Node);

  // return q + ((r + 6) >> 4); nodes (ret1, ret2, ret3)
  TR::Node* ret2Node = TR::Node::create(
    TR::ladd, 2, r1Node, temp = TR::Node::create(secondChild, TR::lconst, 0));
  temp->setLongInt(6);
  TR::Node* ret3Node = TR::Node::create(
    TR::lshr, 2, ret2Node, TR::Node::create(secondChild, TR::iconst, 0, 4));
...
static int
decomposeConstant(char bitPosition[],
                  char operationType[],
                  int64_t value,
                  int wordSize)
{
  ...
  while (value != 0 && (pos < (wordSize))) {
    ...
    if (currentBit) {
      ...
      if (subsequentCount > 1) {
        // there are more then 1 bits set in a row set the operation as
        // substract
...
static TR::Node*
generateDecomposedTree(TR::Node* parentNode,
                       TR::Node* multiplierChild,
                       TR::Simplifier* s,
                       char bitPosition[],
                       char operationType[],
                       int from,
                       ...
                       bool isLong)
{
  ...
  if (count <= 2) {
    if (count == 1) {
      if (operationType[from] == OP_MINUS) {
        ...
        if (bitPosition[from] != 0) {
          TR::Node* constNode1 =
            isLong ? (TR::Node::lconst(
                       parentNode, (int64_t)1 << (int32_t)(bitPosition[from])))
                   : (TR::Node::iconst(parentNode,
                                       1 << (int32_t)(bitPosition[from])));
          TR::Node* shlNode =
            TR::Node::create(mul_op, 2, multiplierChild, constNode1);
      ...
      if (operationType[from + 1] == OP_MINUS) {
        ...
        if (operationType[from] == OP_MINUS) {
          if (bitPosition[from] == 0) {
            ...
            TR::Node* constNode1 =
              isLong
                ? (TR::Node::lconst(parentNode,
                                    (int64_t)1 << (int32_t)(bitPosition[from])))
                : (TR::Node::iconst(parentNode,
                                    1 << (int32_t)(bitPosition[from])));
            TR::Node* shlNode =
              TR::Node::create(mul_op, 2, multiplierChild, constNode1);
        ...
        if (bitPosition[from + 1] == 0) {
          ...
          TR::Node* constNode2 =
            isLong
              ? (TR::Node::lconst(
                  parentNode, (int64_t)1 << (int32_t)(bitPosition[from + 1])))
              : (TR::Node::iconst(parentNode,
                                  1 << (int32_t)(bitPosition[from + 1])));
          secondChild =
            TR::Node::create(mul_op, 2, multiplierChild, constNode2);
        ...
        if (operationType[from] == OP_MINUS) {
          ...
          TR::Node* thisNode =
            TR::Node::create(sub_op, 2, firstChild, secondChild);
          ...
          TR::Node* thisNode =
            TR::Node::create(add_op, 2, firstChild, secondChild);
    ...
    firstChild = generateDecomposedTree(parentNode,
                                        multiplierChild,
                                        s,
                                        bitPosition,
                                        operationType,
                                        ...
                                        tabspace + 1,
                                        isLong);
    secondChild = generateDecomposedTree(parentNode,
                                         multiplierChild,
                                         s,
                                         bitPosition,
                                         operationType,
  ...
  if (count <= 2) {
    if (count == 1) {
      if (operationType[from] == OP_MINUS) {
        ...
        operationType[from] =
          (operationType[from] == OP_MINUS) ? OP_PLUS : OP_MINUS;
        for (i = 0; i < tabspace; i++)
          traceMsg(s->comp(), "\t");
        traceMsg(s->comp(),
                 "\t-> %cn<<%d ",
      ...
      if (operationType[from + 1] == OP_MINUS) {
        ...
        operationType[from + 1] =
          (operationType[from + 1] == OP_MINUS) ? OP_PLUS : OP_MINUS;
        for (i = 0; i < tabspace; i++)
          traceMsg(s->comp(), "\t");
        traceMsg(s->comp(),
                 "\t-> %cn<<%d \n",
    ...
    printTree(
      s, bitPosition, operationType, from, from + mid, tabspace + 1, isLong);
    printTree(
      s, bitPosition, operationType, from + mid, to, tabspace + 1, isLong);
  }
  traceMsg(s->comp(), "\n");
}

static void
decomposeMultiply(TR::Node* node, TR::Simplifier* s, bool isLong)
{
  auto comp = s->comp();
  TR::Node *firstChild = node->getFirstChild(),
           *secondChild = node->getSecondChild();
  ...
  count = decomposeConstant(bitPosition,
                            operationType,
                            isLong ? secondChild->getLongInt()
                                   : secondChild->getInt(),
                            isLong ? 64 : 32);
  ...
  bool iMulDecomposeReport =
    comp->getOptions()->getTraceSimplifier(TR_TraceMulDecomposition);
  if (iMulDecomposeReport) {
    dumpOptDetails(comp,
  ...
  if ((!s->containingStructure() || s->getLastRun()) &&
    ...
    TR::Node* decomposedMulNode = generateDecomposedTree(
      node, firstChild, s, bitPosition, operationType, 0, count, 0, isLong);
...
static void
addSimplifierJavaPacked(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return;
}

static void
foldAddressConstant(TR::Node* node,
                    int64_t value,
                    TR::Simplifier* s,
                    bool anchorChildrenP)
{
  if (!performTransformationSimplifier(node, s))
    return;
...
//
TR::Node*
getQuotientUsingMagicNumberMultiply(TR::Node* node,
                                    TR::Block* block,
                                    TR::Simplifier* s)
{
  TR::Node *firstChild = node->getFirstChild(),
  ...
  if (node->getOpCodeValue() == TR::idiv ||
      node->getOpCodeValue() == TR::irem) {
    ...
    if (shiftAmount == 0) {
      if (divisor > 0) {
        node4 =
          TR::Node::create(TR::iushr,
                           2,
                           firstChild,
                           TR::Node::create(firstChild, TR::iconst, 0, 31));
      } else {
        node4 = TR::Node::create(
          TR::iushr, 2, node2, TR::Node::create(node2, TR::iconst, 0, 31));
      }

      replacementNode = TR::Node::create(TR::iadd, 2, node2, node4);
    } else {
      node3 =
        TR::Node::create(TR::ishr,
                         2,
                         node2,
      ...
      if (divisor > 0)
        ...
        node4 = TR::Node::create(
          TR::iushr, 2, node3, TR::Node::create(node3, TR::iconst, 0, 31));
      replacementNode = TR::Node::create(TR::iadd, 2, node3, node4);
    }
  } else if (node->getOpCodeValue() == TR::ldiv ||
             node->getOpCodeValue() == TR::lrem) {
    //
    ...
    if (shiftAmount == 0) {
      if (divisor > 0) {
        node4 =
          TR::Node::create(TR::lushr,
                           2,
                           firstChild,
                           TR::Node::create(firstChild, TR::iconst, 0, 63));
      } else {
        node4 = TR::Node::create(
          TR::lushr, 2, node2, TR::Node::create(node2, TR::iconst, 0, 63));
      }

      replacementNode = TR::Node::create(TR::ladd, 2, node2, node4);
    } else {
      node3 =
        TR::Node::create(TR::lshr,
                         2,
                         node2,
                         TR::Node::create(node2, TR::iconst, 0, shiftAmount));
      ...
      if (divisor > 0) {
        ...
        node4 = TR::Node::create(
          TR::lushr, 2, node3, TR::Node::create(node3, TR::iconst, 0, 63));
      }
    ...
    TR_ASSERT(0,
              "getQuotientUsingMagicNumberMultiply called with incorrect "
              "opcode in node %p, need TR::idiv/TR::ldiv/TR::irem/TR::lrem.",
              node);
  }
...
/*
 * Run down tree and change the searchOp to its unsigned equivalent.  Only
 * change uncommoned nodes.  Only valid if we don't see any ops that could shift
 * the bits around ie any ops other than loads, ands, ors or convs
 */
static void
changeConverts2Unsigned(TR::Node* node,
                        TR::ILOpCodes searchOp,
                        TR::Simplifier* s)
{
  if (node->getReferenceCount() > 1)
    return;
  ...
  if (node->getOpCodeValue() == searchOp) {
    ...
    if (performTransformation(s->comp(),
                              "%sConverted x2i [%s] to unsigned xu2i\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      TR::Node::recreate(node, newOp);
      return;
...
static void
simplifyIntBranchArithmetic(TR::Node* node,
                            TR::Node*& firstChild,
                            TR::Node*& secondChild,
                            TR::Simplifier* s)
{
...
static void
simplifyLongBranchArithmetic(TR::Node* node,
                             TR::Node*& firstChild,
                             TR::Node*& secondChild,
                             TR::Simplifier* s)
{
...
//
static bool
conditionalBranchFold(int32_t takeBranch,
                      TR::Node*& node,
                      TR::Node* firstChild,
                      TR::Node* secondChild,
                      TR::Block* block,
...
static bool
conditionalZeroComparisonBranchFold(TR::Node*& node,
                                    TR::Node* firstChild,
                                    TR::Node* secondChild,
                                    TR::Block* block,
                                    TR::Simplifier* s)
{
  ...
  if ((firstChild->isNonZero() || firstChild->isNonNull()) &&
    ...
    s->conditionalToUnconditional(
      node, block, !node->getOpCode().isCompareTrueIfEqual());
    return true;
  }
...
//
static TR::Node*
binaryNanDoubleOp(TR::Node* node,
                  TR::Node* firstChild,
                  TR::Node* secondChild,
                  TR::Simplifier* s)
{
...
floatRecip(float value)
{
  ...
  TR_ASSERT(float_exp != 0 && float_exp != 0xff && (u.i & 0x7fffff) == 0,
            "bad argument to floatRecip");
...
doubleRecip(double value)
{
  ...
  TR_ASSERT(double_exp != 0 && double_exp != 0x7ff &&
              (u.i & CONSTANT64(0xfffffffffffff)) == 0,
...
static bool
isLegalToMerge(TR::Node* node,
               TR::Block* block,
               TR::Block* nextBlock,
               TR::CFGEdgeList& nextExcpOut,
               TR::Node* bbStartNode,
               TR::CFGEdgeList& inEdge,
               TR::Simplifier* s,
               bool& blockIsEmpty)
{
  ...
  blockIsEmpty = (block->getEntry() != NULL &&
                  block->getEntry()->getNextTreeTop() == block->getExit());
  if (inEdge.empty() ||
      (!blockIsEmpty && (inEdge.front() != outEdge || (inEdge.size() > 1))))
    return false;

  // Can't merge block with nextBlock if block is entry point and inEdge has
  // more than one edge Since there may be a goto/branch which go around entry
  // point, in that case merging will skip the prologue for that empty entry
  // point See ey1f520, block 5(INTERNAL4) and block 5
  if (blockIsEmpty && (inEdge.size() > 1) &&
      (block == s->comp()->getStartBlock() ||
  ...
  // If nextBlock has predecessor whose target can't be changed, then don't
  // merge
  if (!(inEdge.empty()) && (inEdge.front() != outEdge || (inEdge.size() > 1))) {
    for (auto ie = inEdge.begin(); ie != inEdge.end(); ++ie) {
      if ((*ie) != outEdge && (*ie)->getFrom() != cfg->getStart()) {
        ...
        // ie->getData()->getFrom()->getNumber(),s->comp()->getFlowGraph()->getStart(),
        // nextBlock->getNumber());
        ...
        bool isLegal = (*ie)
                         ...
                         ->asBlock()
                         ->getLastRealTreeTop()
                         ->isLegalToChangeBranchDestination(s->comp());
        if (!isLegal)
          return false;
      }
    }
  ...
  if (cfg &&
      ...
      ((block->isLoopInvariantBlock()) || (nextBlock->isLoopInvariantBlock())))
    return false;
  ...
  if (block->getLastRealTreeTop()
      ...
      (block->getLastRealTreeTop()->getNode()->getOpCodeValue() ==
         TR::treetop &&
  ...
  if (block->getNumber() >= 0 && nextBlock->getNumber()) {
    if (!performTransformation(s->comp(),
                               "%sMerge blocks: block_%d and block_%d\n",
                               s->optDetailString(),
                               block->getNumber(),
                               nextBlock->getNumber()))
      return false;
    ...
    if (!performTransformation(s->comp(),
                               "%sMerge blocks [" POINTER_PRINTF_FORMAT
...
//
static void
changeBranchDestinationsForMergeBlocks(TR::Block* block,
                                       TR::Block* nextBlock,
                                       TR::Node* bbStartNode,
                                       TR::CFGEdgeList& inEdge,
                                       TR::CFGEdgeList::iterator& inEdgeIter,
                                       TR::Simplifier* s)
{
  ...
  // if (inEdge->getNextElement() != NULL)
  {
    ...
    if (cfg->getStructure() != NULL) {
      // merging structures does not handle multiple predecessors to the second
      // block
      dumpOptDetails(
        s->comp(), "%s Resetting structure\n", s->optDetailString());
...
//     iconst
//
// requires that you know the number of digits of precision on the iload
// expression and can be sure that the precision will be the same with or
// without the i2f. Consider if the iload expression had the value 33554432.
// After the i2f, the value would be 3355430 because the float representation of
// the value does not as much precision as the integer representation. If the
// range of the value was known to be within the range of the float value, the
// conversion could still be done.
//
// This same argument applies for l2f and l2d since long has a higher precision
// than float and double.
//
// Without knowing the precision, we can still do the reduction if we know the
// constant is within the precision of the data type and therefore lost
// precision won't affect the comparison.
//
static bool
floatExactlyRepresented(float constant)
{
  const float positiveFloatExactMax = (float)((1 << 24) - 1);
  const float negativeFloatExactMax = (float)-((1 << 24) - 1);
  return (constant >= negativeFloatExactMax &&
          constant <= positiveFloatExactMax);
}

static bool
doubleExactlyRepresented(double constant)
{
  const double positiveDoubleExactMax = (double)(((int64_t)1 << 53) - 1);
  const double negativeDoubleExactMax = (double)-(((int64_t)1 << 53) - 1);
  return (constant >= negativeDoubleExactMax &&
          constant <= positiveDoubleExactMax);
}

static bool
intValueInFloatRange(TR::Node* conversionNode, float constant)
{
...
static bool
longValueInDoubleRange(TR::Node* conversionNode, double constant)
{
...
static bool
longValueInFloatRange(TR::Node* conversionNode, float constant)
{
  if (floatExactlyRepresented(constant)) {
    return true;
  } else {
...
static TR::ILOpCodes
doubleToFloatOp(TR::ILOpCodes dOp)
{
...
static TR::ILOpCodes
doubleToIntegerOp(TR::ILOpCodes dOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (dOp) {
    case TR::ifdcmpeq:
...
static TR::ILOpCodes
doubleToLongOp(TR::ILOpCodes dOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (dOp) {
    case TR::ifdcmpeq:
...
static TR::ILOpCodes
doubleToShortOp(TR::ILOpCodes dOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (dOp) {
    case TR::ifdcmpeq:
...
static TR::ILOpCodes
doubleToCharOp(TR::ILOpCodes dOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (dOp) {
    case TR::ifdcmpeq:
...
static TR::ILOpCodes
doubleToByteOp(TR::ILOpCodes dOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (dOp) {
    case TR::ifdcmpeq:
...
static TR::ILOpCodes
floatToIntegerOp(TR::ILOpCodes fOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (fOp) {
    case TR::iffcmpeq:
...
static TR::ILOpCodes
floatToLongOp(TR::ILOpCodes fOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (fOp) {
    case TR::iffcmpeq:
...
static TR::ILOpCodes
floatToShortOp(TR::ILOpCodes fOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (fOp) {
    case TR::iffcmpeq:
...
static TR::ILOpCodes
floatToCharOp(TR::ILOpCodes fOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (fOp) {
    case TR::iffcmpeq:
...
static TR::ILOpCodes
floatToByteOp(TR::ILOpCodes fOp)
{
  TR::ILOpCodes op = TR::BadILOp;
  switch (fOp) {
    case TR::iffcmpeq:
...
static bool
doubleConstIsRepresentableExactlyAsFloat(double dValue, float& fValue)
{
...
static bool
doubleConstIsRepresentableExactlyAsInt(double dValue, int32_t& iValue)
{
...
static bool
doubleConstIsRepresentableExactlyAsLong(double dValue, int64_t& lValue)
{
...
static bool
doubleConstIsRepresentableExactlyAsShort(double dValue, int16_t& sValue)
{
...
static bool
doubleConstIsRepresentableExactlyAsChar(double dValue, uint16_t& cValue)
{
...
static bool
doubleConstIsRepresentableExactlyAsByte(double dValue, int8_t& bValue)
{
...
static bool
floatConstIsRepresentableExactlyAsInt(float fValue, int32_t& iValue)
{
...
static bool
floatConstIsRepresentableExactlyAsLong(float fValue, int64_t& lValue)
{
...
static bool
floatConstIsRepresentableExactlyAsShort(float fValue, int16_t& sValue)
{
...
static bool
floatConstIsRepresentableExactlyAsChar(float fValue, uint16_t& cValue)
{
...
static bool
floatConstIsRepresentableExactlyAsByte(float fValue, int8_t& bValue)
{
...
static void
intCompareNarrower(TR::Node* node,
                   TR::Simplifier* s,
                   TR::ILOpCodes ushortOp,
                   TR::ILOpCodes shortOp,
                   TR::ILOpCodes byteOp)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (s->cg()->getSupportsEfficientNarrowIntComputation()) {
    ...
    if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
      if (secondOp == TR::su2i ||
          (secondOp == TR::iconst && secondChild->getInt() >= 0 &&
        ...
        if (secondOp == TR::su2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Integer Compare Narrower: found both children c2i "
                           "in method %s\n",
                           s->comp()->signature());
          }
        } else if (secondOp == TR::iconst) {
          if (secondChild->getReferenceCount() > 1) {
            TR::Node* newSecondChild =
              TR::Node::cconst(secondChild, (uint16_t)secondChild->getInt());
            secondChild->recursivelyDecReferenceCount();
            node->setAndIncChild(1, newSecondChild);
          } else {
            TR::Node::recreate(secondChild, TR::cconst);
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Integer Compare Narrower: found child 1 c2i and "
                           "child 2 iconst in cconst range in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(),
                         "Integer Compare Narrower: found child 1 c2i and "
                         "child 2 cconst in method %s\n",
                         s->comp()->signature());
        }
      }
    } else if (firstOp == TR::s2i && firstChild->getReferenceCount() == 1) {
      if (secondOp == TR::s2i ||
          (secondOp == TR::iconst && secondChild->getInt() >= SHRT_MIN &&
        ...
        if (secondOp == TR::s2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Integer Compare Narrower: found both children s2i "
                           "in method %s\n",
                           s->comp()->signature());
          }
        } else if (secondOp == TR::iconst) {
          if (secondChild->getReferenceCount() > 1) {
            TR::Node* newSecondChild =
              TR::Node::sconst(secondChild, (int16_t)secondChild->getInt());
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Integer Compare Narrower: found child 1 s2i and "
                           "child 2 iconst in sconst range in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(),
                         "Integer Compare Narrower: found child 1 s2i and "
                         "child 2 sconst in method %s\n",
                         s->comp()->signature());
        }
      }
      ...
      if (secondOp == TR::b2i ||
        ...
        if (secondOp == TR::b2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Integer Compare Narrower: found both children b2i "
                           "in method %s\n",
                           s->comp()->signature());
          }
        } else if (secondOp == TR::iconst) {
          if (secondChild->getReferenceCount() > 1) {
            TR::Node* newSecondChild =
              TR::Node::bconst(secondChild, (int8_t)secondChild->getInt());
            secondChild->recursivelyDecReferenceCount();
            node->setAndIncChild(1, newSecondChild);
          } else {
            TR::Node::recreate(secondChild, TR::bconst);
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Integer Compare Narrower: found child 1 b2i and "
                           "child 2 iconst in bconst range in method %s\n",
                           s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
          dumpOptDetails(s->comp(),
                         "Integer Compare Narrower: found child 1 b2i and "
                         "child 2 bconst in method %s\n",
                         s->comp()->signature());
        }
      }
    }
...
static void
unsignedIntCompareNarrower(TR::Node* node,
                           TR::Simplifier* s,
                           TR::ILOpCodes ushortOp,
                           TR::ILOpCodes ubyteOp)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  static char* reportCompareDemotions = feGetEnv("TR_ReportCompareDemotions");
  if (s->cg()->getSupportsEfficientNarrowUnsignedIntComputation()) {
    ...
    if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
      if (secondOp == TR::su2i ||
        ...
        if (secondOp == TR::su2i) {
          ...
          if (secondChild->getReferenceCount() > 1) {
            ...
            TR::Node::recreate(secondChild, TR::cconst);
            secondChild->setConst<uint16_t>(
              (uint16_t)secondChild->getUnsignedInt());
          }
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
                           "Integer Compare Narrower: found child 1 c2i and "
      ...
      if (secondOp == TR::s2i || (secondOp == TR::iconst &&
      ...
      if (secondOp == TR::b2i || (secondOp == TR::iuconst &&
        ...
        if (secondOp == TR::b2i) {
          ...
          if (secondChild->getReferenceCount() > 1) {
            TR::Node* newSecondChild = TR::Node::bconst(
              secondChild, (int8_t)secondChild->getUnsignedInt());
            secondChild->recursivelyDecReferenceCount();
            node->setAndIncChild(1, newSecondChild);
          } else {
            TR::Node::recreate(secondChild, TR::bconst);
...
//---------------------------------------------------------------------
// Helper function to convert int to float.  Should never be added to
// SimplifierTable!
//
static TR::Node*
integerToFloatHelper(uint32_t absValue,
                     bool isUnsignedOperation,
                     TR::Node* node,
                     TR::Simplifier* s)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= 8) {
    ...
    foldFloatConstant(node,
                      isUnsignedOperation ? (float)firstChild->getUnsignedInt()
                                          : (float)firstChild->getInt(),
                      s);
    return node;
  } else {
    ...
    if (isUnsignedOperation)
      ...
      foldFloatConstant(
        node, firstChild->getInt() < 0 ? -(float)absValue : (float)absValue, s);
...
static bool
containsLoad(TR::Node* node, TR::SymbolReference* symRef, vcount_t visitCount)
{
...
static bool
canMovePastTree(TR::TreeTop* tree,
                TR::SymbolReference* symRef,
                TR::Compilation* c,
                bool blockHasCatchers)
{
  ...
  if (blockHasCatchers && tree->getNode()->exceptionsRaised())
    return false; // can't move it past any tree that could cause an exception
                  // if we have catchers
  ...
  //
  if (definingSymRef != 0 &&
      definingSymRef->getUseDefAliases().contains(symRef, c))
    return false;
...
static bool
containsNode(TR::Node* tree, TR::Node* searchNode, vcount_t visitCount)
{
  if (tree == searchNode)
    return true;
...
static bool
isBitwiseLongComplement(TR::Node* n)
{
...
//       iload
//       iconst
// Since we are simplifying children first, in all likelyhood the ishl will get
// transformed like this:
// n380n     istore  <auto slot 8>[id=232:"b"] [#60  Auto] [flags 0x3 0x0 ]
// (Int32) n379n       ior n373n         imul n371n           iload  <auto slot
// 8>[id=232:"b"] [#60  Auto] [flags 0x3 0x0 ] (Int32) n372n           iconst
// 0x400000 n378n         iushr n374n           iload  <auto slot 8>[id=232:"b"]
// [#60  Auto] [flags 0x3 0x0 ] (Int32) n377n           iconst 10
//

template<typename T>
static bool
checkAndReplaceRotation(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  static char* disableROLSimplification =
    feGetEnv("TR_DisableROLSimplification");
  ...
  TR_ASSERT(node->getOpCode().isOr() || node->getOpCode().isXor(),
            ...
            node); // I believe this is general enough to allow other opcodes,
                   // like add's, xor's. But just want to be safe for now.

  // traceMsg(TR::comp(), "In checkAndReplaceRotation for node %p firstChild =
  // %p secondChild = %p\n",node,firstChild,secondChild);

  if (!firstChild->getOpCode().isShift() &&
      !firstChild->getOpCode().isShiftLogical() &&
      !firstChild->getOpCode().isMul())
    return false;

  if (!secondChild->getOpCode().isShift() &&
      !secondChild->getOpCode().isShiftLogical() &&
      !secondChild->getOpCode().isMul())
    return false;

  // To simplify my code, I am not going to handle these two cases on the
  // assumption that simplifier will already have simplified these away.

  //   if (firstChild->getOpCode().isLeftShift() &&
  //   !secondChild->getOpCode().isShiftLogical() &&
  //   !secondChild->getOpCode().isRightShift())
  //      return false;

  //   if ( secondChild->getOpCode().isLeftShift() &&
  //   !firstChild->getOpCode().isShiftLogical() &&
  //   !firstChild->getOpCode().isRightShift())
  //      return false;

  TR::Node* mulNode = firstChild->getOpCode().isMul()
                        ? firstChild
                        : (secondChild->getOpCode().isMul() ? secondChild : 0);
  TR::Node* shiftNode = firstChild->getOpCode().isMul()
                          ? secondChild
                          : (secondChild->getOpCode().isMul() ? firstChild : 0);

  if (!mulNode || !shiftNode ||
      !(shiftNode->getOpCode().isShiftLogical() &&
        shiftNode->getOpCode().isRightShift()))
    return false;

  TR::Node* mulConstNode =
    mulNode->getFirstChild()->getOpCode().isLoadConst()
      ? mulNode->getFirstChild()
      : (mulNode->getSecondChild()->getOpCode().isLoadConst()
           ? mulNode->getSecondChild()
           : 0);
  TR::Node* shiftConstNode =
    shiftNode->getFirstChild()->getOpCode().isLoadConst()
      ? shiftNode->getFirstChild()
      : (shiftNode->getSecondChild()->getOpCode().isLoadConst()
           ? shiftNode->getSecondChild()
           : 0);
  TR::Node* mulNonConstNode =
    mulNode->getFirstChild()->getOpCode().isLoadConst()
      ? mulNode->getSecondChild()
      : (mulNode->getSecondChild()->getOpCode().isLoadConst()
           ? mulNode->getFirstChild()
           : 0);
  TR::Node* shiftNonConstNode =
    shiftNode->getFirstChild()->getOpCode().isLoadConst()
      ? shiftNode->getSecondChild()
      : (shiftNode->getSecondChild()->getOpCode().isLoadConst()
           ? shiftNode->getFirstChild()
  ...
  if (!mulConstNode || !shiftConstNode ||
      mulNonConstNode != shiftNonConstNode) // requires commoning to have
                                            // commoned the non const value.
    return false;

  // traceMsg(TR::comp(), "firstChild: isMul = %d isShiftLogical = %d
  // isRightShift = %d secondChild: isMul = %d isShiftLogical = %d isRightShift =
  // %d\n",firstChild->getOpCode().isMul(),
  // firstChild->getOpCode().isShiftLogical(),
  // firstChild->getOpCode().isRightShift(), secondChild->getOpCode().isMul(),
  // firstChild->getOpCode().isShiftLogical(),
  // firstChild->getOpCode().isRightShift());
  ...
  // traceMsg(TR::comp(), "Spot 2.5 correctLeftShiftAmount = %d mulConst = %d
  // shiftConst = %d sizeof(T)*8 =
  ...
  T expectedMulConst = correctLeftShiftAmount < static_cast<T>(sizeof(T) * 8)
                         ? 1ULL << correctLeftShiftAmount
                         : 0;
  ...
  if (!performTransformation(
        s->comp(),
        "%sReduced or/xor/add in node [" POINTER_PRINTF_FORMAT "] to rol\n",
        s->optDetailString(),
        node))
    return false;

  TR::Node* newConstNode =
    TR::Node::iconst(mulConstNode, correctLeftShiftAmount);
  TR::Node::recreate(
    node,
    TR::ILOpCode::getRotateOpCodeFromDt(
      mulConstNode->getDataType())); // this line will have to change based on T

  // Set the common nonconst node (child of mul and shift) as the child of rol
  // node, and then recursively decrement both mul and shift node. The nonconst
  // node will survive as it is attached to the rol node before
  // recursivelyDecRefCount is called.
...
//---------------------------------------------------------------------
// Long helper to convert to float -- should never be called from Simplifier
// Table!
//
static TR::Node*
longToFloatHelper(uint64_t absValue,
                  bool isUnsignedOperation,
                  TR::Node* node,
                  TR::Simplifier* s)
{
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 24)) {
    foldFloatConstant(node,
                      isUnsignedOperation
                        ? (float)firstChild->getUnsignedLongInt()
                        : (float)firstChild->getLongInt(),
                      s);
    return node;
  } else {
    ...
    if (isUnsignedOperation)
      ...
      foldFloatConstant(node,
                        firstChild->getLongInt() < 0 ? -(float)absValue
                                                     : (float)absValue,
                        s);
    return node;
  }
...
//---------------------------------------------------------------------
// This method should never be added to SimplifierTable! It is used to convert
// values to doubles taking into account whether the operation is signed or not.
//
static TR::Node*
longToDoubleHelper(uint64_t absValue,
                   bool isUnsignedOperation,
                   TR::Node* node,
                   TR::Simplifier* s)
{
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 53)) {
    foldDoubleConstant(node,
                       isUnsignedOperation
                         ? (double)firstChild->getUnsignedLongInt()
                         : (double)firstChild->getLongInt(),
                       s);
    return node;
  } else // probably the native compile rounds to ieee-754, but to be sure we
         // round it manually
  {
    uint64_t roundBit = CONSTANT64(0x8000000000000000);
    ...
    if (isUnsignedOperation)
      ...
      foldDoubleConstant(node,
                         firstChild->getLongInt() < 0 ? -(double)absValue
                                                      : (double)absValue,
                         s);
    return node;
  }
...
//
static int64_t
floatToLong(float value, bool roundUp)
{
...
//
static int64_t
doubleToLong(double value, bool roundUp)
{
  int64_t pattern = *(int64_t*)&value;
  int64_t result;
  if ((pattern & DOUBLE_ORDER(CONSTANT64(0x7ff0000000000000))) ==
        DOUBLE_ORDER(CONSTANT64(0x7ff0000000000000)) &&
 ...
 */
static bool
isDeletedLabelLoadaddr(TR::Node* node)
{
  return false;
}
...
static void
bitTestingOp(TR::Node* node, TR::Simplifier* s)
{
  ...
  int32_t shiftAmount = (int32_t)firstChild->getFirstChild()
                          ->getSecondChild()
                          ->get64bitIntegralValue();
  int64_t bitAmount =
    (int64_t)firstChild->getSecondChild()->get64bitIntegralValue();
  ...
  if (!performTransformation(s->comp(),
                             "%sRemoving shift node [" POINTER_PRINTF_FORMAT
                             "] \n",
                             s->optDetailString(),
  ...
  switch (dt) {
    ...
    case TR::Int64:
      foldLongIntConstant(firstChild->getSecondChild(),
                          ...
                          false /* !anchorChildren */);
      foldLongIntConstant(
        secondChild, cmpAmount >> shiftAmount, s, false /* !anchorChildren */);
      break;
    default:
...
/**
 * analyse expression and determine if it's result will exceed the precision of
 the node's
 * type.  The motivation is to see if using FMA whose intermediate result has
 higher precision
 * than the arguments, would be identical to an FP strict implementation.  If
 so, then
 * we can substitute (a*b)+c expression trees at codegeneration time with fma
 instructions,
 * if supported on the platform.

   In general we can use fmadd/fmsub if.
     - Multiply and add/subtract are in same method and
           - One child of multiply is double and other is power of 2 and method
 is not strictfp
           - One child of multiply is f2d or i2d and other is double const of
 <(53-24) or <(53-31) bits precision ,but result might overflow a double range
 or denormalised precision
     - Children of multiplies are both f2d
     - One child of multiply is f2d or i2d and other is double const of <(53-24)
 or <(53-31) bits precision and result cannot overflow a double range or
 precision.  This is currently not handled as it requires value propagation of
 floats.

  NOTE: rounding of very large values can cause overflow, but these constants
 would have to be: i2d: |constant| must be < 2^993 f2d: |constant| must be <
 2^896 and >= 2^-901
 */

static bool
isOperationFPCompliant(TR::Node* parent,
                       TR::Node* arithExprNode,
                       TR::Simplifier* s)
{
  ...
  if (!s->cg()->supportsFusedMultiplyAdd())
    return false; // no point if FMA not supported

  if (!arithExprNode->getOpCode().isMul())
    return false;
  ...
  if (mulNode->getDataType() == TR::Double &&
      mulNode->getFirstChild()->getOpCode().isConversion() &&
      mulNode->getSecondChild()
        ->getOpCode()
        .isConversion()) { // long->double is only case where we must give up
    if (mulNode->getFirstChild()->getOpCode().isLong()) {
      if (traceIt)
        traceMsg(s->comp(),
                 "Node: %p fails because conversion type is long\n",
                 mulNode->getFirstChild());
      return false;
    }
    ...
    if (traceIt)
      traceMsg(s->comp(),
               "Node: %p passes because conversion type is <long\n",
               mulNode);
    return true;
  }

  bool sameCaller =
    mulNode->getInlinedSiteIndex() == parent->getInlinedSiteIndex();
  ...
  if (!mulChildIsConst && s->cg()->supportsOnDemandLiteralPool()) {
    if (mulNonConstChild->getOpCode()
          .isLoadIndirect() && // the "nonconst' node is actually the const
    ...
    if (mulChildIsConst ||
        (mulConstChild->getOpCode().isLoadIndirect() &&
         mulConstChild->getSymbolReference()->isLiteralPoolAddress())) {
      // offset in the sym ref points to the constant node
      TR::Node* constNode =
        (TR::Node*)(mulConstChild->getSymbolReference()->getOffset());
      if (traceIt)
        traceMsg(s->comp(),
                 "LiteralPool load detected:%p points to %p\n",
                 mulConstChild,
                 constNode);
  ...
  if (!mulChildIsConst) {
    if (traceIt)
      traceMsg(s->comp(), "Node %p fails since not a const\n", mulNode);
    return false;
  }
  ...
  if (!sameCaller) {
    if (traceIt)
      traceMsg(s->comp(),
               "Node: %p fails because split between two methods \n",
               mulNode);
    return false;
  }

  // we look at trailing zeros of the constant ie how 'small' is the number, and
  // that number indicates the kind of precision of the result.  Subtract this
  // number from the precision of current type and if smaller than the precision
  // limit then a fused multiply will have same results as lower-precision mult.
  // So, for example  i2f: float has 24 bits of precision, so the most precision
  // we have 53-24 bits of precision 'available'--anything requiring more bits
  // will give different results between fused multiply and double multiply So
  // the constant must not cause the precision to increase by more than this
  // 'available' bits.  So, if the constant is 1000...010000000000, then there
  // are 10 zeros and hence can increase precision requirements by 53-10=43
  // bits. available precision is 53-24 = 19, 43 > 19 (=14) so can't use fmul
  // This boils down to whether precision of the constant is greater than the
  // precision of the non-const argument (aka the conversion operator).
  //
  if (mulNonConstChild->getOpCode().isConversion()) {
    uint32_t precisionLimit =
      53; // bit count must be more than this to assure no rounding issues
    switch (mulNonConstChild->getOpCodeValue()) {
      ...
      default:
        break;
        /*
         these nodes are actually never generated by J9, so no chance to test
        them case TR::b2d: precisionLimit = 8; break; //unsigned case TR::su2d:
        precisionLimit = 16; break; //unsigned case TR::s2d: precisionLimit =
        15; break; //signed
        */
    }

    double possibleRoundedValue =
      1; // if not a float or double value, doesn't matter
    ///
    /// The Java language spec
    /// lists bit sizes for doubles.  53 bits used for precision exponent, 11
    /// for magnitude and leading one for sign.  So check # bits in precision
    /// only by masking off these first 11+1 bits
    uint32_t bitcount;
    switch (mulConstChild->getDataType()) {
      // I don't believe it is possible to reach here with anything other than a
      // floating point const child; even in the case of a literal pool load.
      case TR::Float: {
        union
        {
          float f;
      ...
      case TR::Double: // rely on fact same storage used for longs
        ...
        if (32 == bitcount)
          bitcount +=
            trailingZeroes(0x000fffff & mulConstChild->getLongIntHigh());

        if (traceIt)
          traceMsg(s->comp(),
                   "Node:%p  bitcount:%d hex:%x%x value:%g\n",
                   mulConstChild,
                   bitcount,
                   mulConstChild->getLongIntHigh(),
                   mulConstChild->getLongIntLow(),
    ...
    if (i2dConv) {
      if (possibleRoundedValue >= FMA_CONST_HIGHBOUNDI2D) {
        if (traceIt)
          traceMsg(s->comp(),
                   "Fails because |constant| %e exceeds %e\n",
                   possibleRoundedValue,
                   FMA_CONST_HIGHBOUNDI2D);
        return false;
      }
    } else if (f2dConv) {
      if (possibleRoundedValue >= FMA_CONST_HIGHBOUNDF2D ||
          possibleRoundedValue < FMA_CONST_LOWBOUND) {
        if (traceIt)
          traceMsg(s->comp(),
                   "Fails because |constant| %e exceeds range %e-%e \n",
                   possibleRoundedValue,
                   FMA_CONST_LOWBOUND,
    ...
    if (bitcount > precisionLimit)
      return true;
    else if (traceIt)
      traceMsg(s->comp(),
               "Node:%p failed bitcount:%d >= precision:%d\n",
               mulConstChild,
               bitcount,
               precisionLimit);
  }

  bool loadConstIsPowerOfTwoDbl =
    (mulConstChild->getDataType() == TR::Double) &&
    isNZDoublePowerOfTwo(mulConstChild->getDouble());

  bool loadConstIsPowerOfTwoFloat =
    (mulConstChild->getDataType() == TR::Float) &&
    isNZFloatPowerOfTwo(mulConstChild->getFloat());

  bool isStrictfp = s->comp()->getCurrentMethod()->isStrictFP() ||
                    s->comp()->getOption(TR_StrictFP);

  if ((loadConstIsPowerOfTwoDbl || loadConstIsPowerOfTwoFloat) && !isStrictfp)
    return true;
  else if (traceIt)
    traceMsg(
      s->comp(), "Fails because not power of 2 or !strict\n", mulConstChild);

  if (traceIt)
    traceMsg(s->comp(),
             "Fails because strict, no const or bitcount exceeded\n");
  return false;
}
...
//
static void
bitwiseToLogical(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  // FIXME: We will return if the next block extends the current block to ensure
  // that it does not have references to the current block. The smarter way is
  // to uncommon all the references in the next block referencing the current
  // block. The class TR_HandleInjectedBasicBlocks in the inliner will be useful
  //
  ...
  if (!performTransformation(
        s->comp(),
        "%sConvert comparison with bitwise ops [" POINTER_PRINTF_FORMAT
        "] to logical control flow\n",
        s->optDetailString(),
        node))
    return;
  ...
  TR::Node::recreate(
    ...
    (disjunctive) ? cmp1->getOpCode().convertCmpToIfCmp()
                  : TR::ILOpCode(cmp1->getOpCode().getOpCodeForReverseBranch())
                      .convertCmpToIfCmp());
...
static bool
skipRemLoweringForPositivePowersOfTen(TR::Simplifier* s)
{
  return false;
}

static bool
skipRemLowering(int64_t divisor, TR::Simplifier* s)
{
  return false;
}

/** \brief
 *     Reduce ifxcmpge to ifxcmpeq if possible to simplify the trees so that a
 * code generator may have the opportunity to carry out in-memory bit tests to
 * evaluate the respective tree.
 *
 *  \details
 ...
 * and we can reduce ificmpge to ificmpeq. In other words, the result of iand
 * cannot be greater than the constant, and we can simply perform an equality
 *     check, which provides further optimization opportunities.
 *
 *  \note
 *     The following special case. We need to check that either the comparison
 * is unsigned, or the mask has the high / sign bit clear. Otherwise it's
 * possible for the result of iand to compare greater. For example, consider the
 *     following tree:
 *
 *     <code>
 *     (0) ificmpge --> block_7
 ...
 *     </code>
 *
 *     If <c>y</c> is non-negative, then the result of iand will also be
 * non-negative, and therefore greater than -2.
 */
...
public:
  ...
  TR::Node* reduce()
  {
    if (performTransformation(_simplifier->comp(),
                              "%sReduce an ifxcmpge node [%p] to ifxcmpeq\n",
                              _simplifier->optDetailString(),
                              _node)) {
      TR::ILOpCodes ifcmpeqOpCode =
        TR::ILOpCode::ifcmpeqOpCode(_node->getSecondChild()->getDataType());
...
private:
  ...
  bool isReducibleSpecific() const
  {
    ...
    if (lhsNode->getOpCode().isAnd() &&
        lhsNode->getSecondChild()->getOpCode().isIntegralConst() &&
        rhsNode->getOpCode().isIntegralConst()) {
      T andConst = lhsNode->getSecondChild()->getConst<T>();

      if (andConst == rhsNode->getConst<T>() &&
          (_node->getOpCode().isUnsignedCompare() || andConst >= 0)) {
        return true;
      }
    }
...
//
TR::Node*
dftSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (node->getOpCode().isBranch() &&
      (removeIfToFollowingBlock(node, block, s) == NULL))
    return NULL;
  simplifyChildren(node, block, s);
  return node;
}
...
TR::Node*
lstoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  return node;
}
...
TR::Node*
directLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(
    node, block, s); // For all those direct loads with child nodes...
  TR::TransformUtil::transformDirectLoad(s->comp(), node);
  return node;
}

TR::Node*
indirectLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  //
  TR::Node* transformed =
    TR::TransformUtil::transformIndirectLoad(s->comp(), node);
  if (transformed) {
    if (node == transformed)
      return node;
    else
  ...
  if (firstChild->getOpCodeValue() == TR::loadaddr) {
    ...
    TR::DataType loadDataType =
      newOType ? node->getDataType()
               : node->getSymbolReference()->getSymbol()->getDataType();
    intptrj_t loadSize = newOType
                           ? node->getSize()
                           : node->getSymbolReference()->getSymbol()->getSize();
    intptrj_t loadSymSize = node->getSymbolReference()->getSymbol()->getSize();

    TR::DataType addrDataType =
      firstChild->getSymbolReference()->getSymbol()->getDataType();
    intptrj_t addrSize =
      firstChild->getSymbolReference()->getSymbol()->getSize();
    TR::Symbol* addrSymbol = firstChild->getSymbolReference()->getSymbol();
    bool localStatic = false;

    TR::ILOpCodes convOpCode =
      TR::ILOpCode::getProperConversion(addrDataType, loadDataType, false);
    ...
    if (newOType)
      allowLoadSize =
        (loadSize == loadSymSize) &&
        (loadSize == 1 || loadSize == 2 || loadSize == 4 || loadSize == 8);
    else
    ...
    if (isSame &&
        ...
        (node->getSymbol()->isVolatile() ==
         firstChild->getSymbol()->isVolatile()) &&
        performTransformation(
          s->comp(),
          "%sReplace indirect load %s [" POINTER_PRINTF_FORMAT "] with ",
          s->optDetailString(),
          node->getOpCode().getName(),
          node)) {
      ...
      if (loadDataType != addrDataType) {
        uint8_t precision = node->getType().isAddress()
      ...
      if (!newOType && addrDataType == TR::Aggregate &&
          node->getDataType() != TR::Aggregate)
        addrDataType = node->getDataType();

      TR::Node::recreateWithoutProperties(
        loadNode, s->comp()->il.opCodeForDirectLoad(addrDataType), 0, childRef);

      dumpOptDetails(s->comp(),
                     "%s [" POINTER_PRINTF_FORMAT
                     "] (load %s [" POINTER_PRINTF_FORMAT "])\n",
                     node->getOpCode().getName(),
                     node,
                     loadNode->getOpCode().getName(),
                     loadNode);
    ...
    if ((node->getType().isIntegral() || node->getType().isDouble()) &&
        ((addressNode->getOpCode().isArrayRef() &&
         ...
         (addressNode->getOpCode().hasSymbolReference() &&
          addressNode->getSymbol()->getType().isVector())) &&
        performTransformation(s->comp(),
                              "%sReplace indirect load [" POINTER_PRINTF_FORMAT
                              "] with getvelem",
                              s->optDetailString(),
                              node)) {
      int32_t offset = 0;
      ...
      TR::SymbolReference* newSymRef =
        s->comp()->getSymRefTab()->createSymbolReference(
          TR::Symbol::createShadow(s->comp()->trHeapMemory(),
                                   addressNode->getSymbol()->getDataType()),
          0);
      TR::Node* vloadiNode =
        TR::Node::createWithSymRef(TR::vloadi, 1, 1, addressNode, newSymRef);
      TR::Node* indexNode = TR::Node::iconst(offset / (node->getSize()));

      TR::Node* getvelemNode =
        TR::Node::create(TR::getvelem, 2, vloadiNode, indexNode);
...
TR::Node*
indirectStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (isDeletedLabelLoadaddr(secondChild) &&
      performTransformation(s->comp(),
                            "%sRemoving indirect store [" POINTER_PRINTF_FORMAT
                            "] to deleted label [" POINTER_PRINTF_FORMAT "]\n",
                            s->optDetailString(),
                            node,
                            secondChild->getSymbol())) {
  ...
  if (firstChild->getOpCodeValue() == TR::loadaddr) {
    ...
    TR::ILOpCodes convOpCode =
      TR::ILOpCode::getProperConversion(storeDataType, addrDataType, false);

    bool allowStoreSize = false;
    if (newOType)
      allowStoreSize =
        (storeSize == storeSymSize) &&
    ...
    if ((storeDataType == addrDataType && isSameType) &&
        ...
        performTransformation(
          s->comp(),
          "%sReplace indirect store %s [" POINTER_PRINTF_FORMAT "] with ",
          s->optDetailString(),
          node->getOpCode().getName(),
          node)) {
      ...
      TR::Node::recreate(node,
                         s->comp()->il.opCodeForDirectStore(addrDataType));
  ...
  if ((node->getOpCodeValue() == TR::awrtbari) && 0 &&
    ...
    if ((valueChild->getOpCodeValue() == TR::New) ||
      ...
      if (removeWrtBar && !s->comp()->getOptions()->realTimeGC() &&
          performTransformation(
            s->comp(),
            "%sFolded indirect write barrier to iastore because GC could not "
            "have occurred enough times to require iwrtbar "
            "[" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(),
            node)) {
  ...
  if (s->comp()->cg()->getSupportsVectorRegisters()) {
    ...
    // Can convert to vsetelem if it is a scalar store to a vector, or it's an
    // array ref to a vector with a constant offset
    if ((node->getType().isIntegral() || node->getType().isDouble()) &&
        ...
        ((addressNode->getOpCode().isArrayRef() &&
         ...
         (addressNode->getOpCode().hasSymbolReference() &&
          addressNode->getSymbol()->getType().isVector())) &&
        performTransformation(s->comp(),
                              "%sReplace indirect store [" POINTER_PRINTF_FORMAT
                              "] with vsetelem",
                              s->optDetailString(),
                              node)) {
      int32_t offset = 0;
      ...
      TR::Node* vloadiNode =
        TR::Node::createWithSymRef(TR::vloadi, 1, 1, addressNode, newSymRef);
      TR::Node* indexNode = TR::Node::iconst(
        offset / (node->getSize())); // idx = byte offset / element size
      TR::Node* vsetelemNode =
        TR::Node::create(TR::vsetelem, 3, vloadiNode, indexNode, valueNode);
      auto newVStorei = TR::Node::createWithSymRef(
        TR::vstorei, 2, 2, addressNode, vsetelemNode, newSymRef);
      dumpOptDetails(s->comp(), "[" POINTER_PRINTF_FORMAT "]\n", newVStorei);
...
TR::Node*
astoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (isDeletedLabelLoadaddr(node->getFirstChild()) &&
      performTransformation(s->comp(),
                            "%sRemoving astore [" POINTER_PRINTF_FORMAT
                            "] to deleted label [" POINTER_PRINTF_FORMAT "]\n",
                            s->optDetailString(),
                            node,
...
TR::Node*
directStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* child = node->getFirstChild();
  TR::SymbolReference* symRef = node->getSymbolReference();
  if (child->getOpCode().isLoadVar() && child->getReferenceCount() == 1 &&
      symRef == child->getSymbolReference() &&
      performTransformation(s->comp(),
                            "%sFolded direct store of load of same symbol on "
                            "node [" POINTER_PRINTF_FORMAT "]\n",
                            s->optDetailString(),
                            node)) {
  ...
  // when we find an update form where the Yload has ref count > 1, we'd like to
  // swing this tree down below the node that uses it to avoid ugly register
  // copying (because both the original and incremented value are required)
  // NOTE: this likely won't improve register pressure; it's purely a codegen
  // quality improvement
    ...
    bool firstLoadsSymRef =
      (firstGrandchild->getOpCode().isLoadVar() &&
       symRef ==
         firstGrandchild
           ->getSymbolReference()); // && firstGrandchild->getReferenceCount() >
                                    // 1);
    ...
    bool secondLoadsSymRef =
      (secondGrandchild->getOpCode().isLoadVar() &&
       symRef ==
         secondGrandchild
           ->getSymbolReference()); // && secondGrandchild->getReferenceCount()
                                    // > 1);
    bool secondIsLoadConst = (secondGrandchild->getOpCode().isLoadConst());

    if ((firstLoadsSymRef && secondIsLoadConst) ||
        (firstIsLoadConst && secondLoadsSymRef)) {
      // found a candidate update form tree: now walk trees forward until either
      // something stores to <sym> or end of block remember tree where last use
      // of node was on this walk: insert our update tree following that last
      // use tree

      // first, find the actual update tree (this is obviously our first
      // potential insertion point for the update tree since it's already here)
      TR::TreeTop* updateTree = block->getEntry();
      while (updateTree->getNode() != node) {
        updateTree = updateTree->getNextRealTreeTop();

        // if we reach the end of the block, then the update tree is probably
        // under a ResolveCHK node and we don't really want to move that
        if (updateTree == block->getExit())
          return node;
      }

      // dumpOptDetails(s->comp(), "found update form treetop ["
      // POINTER_PRINTF_FORMAT "] node [" POINTER_PRINTF_FORMAT "] for symbol
      // #%d[" POINTER_PRINTF_FORMAT "]\n", updateTree->getNode(), node,
      // symRef->getReferenceNumber());

      // only proceed if previous tree is a treetop holding an iload of <sym>
      ...
      if (loadTree->getNode()->getOpCodeValue() == TR::treetop) {
        ...
        if (loadNode->getOpCode().isLoadVarDirect() &&
            loadNode->getSymbolReference() == symRef) {
          // dumpOptDetails(s->comp(), "found load [" POINTER_PRINTF_FORMAT "]
          // of symbol in previous tree\n", loadNode);

          // now, walk trees forward to end of block or a tree that accesses
          // <sym> when we find a tree containing a use of <sym>, update
          // insertion point with tree following the use tree
          ...
          while (walkingTree != block->getExit()) {
            if (!canMovePastTree(
                  walkingTree, symRef, s->comp(), blockHasCatchers))
              break;

            if (containsNode(
                  walkingTree->getNode(), loadNode, s->comp()->getVisitCount()))
              insertionTree = walkingTree;

            walkingTree = walkingTree->getNextRealTreeTop();
          }

          // if we found a better insertion point, snip tree out of its original
          // place and insert it after insertionTree
          if (insertionTree != NULL &&
              performTransformation(s->comp(),
                                    "%smove update tree [" POINTER_PRINTF_FORMAT
                                    "] to after [" POINTER_PRINTF_FORMAT "]\n",
                                    s->optDetailString(),
                                    node,
                                    insertionTree->getNode())) {
            // make sure we still simplify trees following the original update
            // tree's position
            ...
            updateTree->getPrevRealTreeTop()->join(
              updateTree->getNextRealTreeTop());

            // and re-connect at new insertion point
            updateTree->join(insertionTree->getNextRealTreeTop());
            insertionTree->join(updateTree);

            // So we decided to swing this update tree below the load, but
            // eliminateDeadTrees might subsequently try to undo it by moving
            // the load below the update tree (bad pass!).  We'll allow it to do
            // that (because we can come along and re-swing the update below the
            // load) so long as it doesn't move the load under a branch tree. If
            // the load gets moved into a branch tree, then there's no place we
            // could put the update tree that's below the load but still in the
            // block (executive summary: all our careful work above would be for
            // naught).  So mark the load here so that eliminateDeadTrees won't
            // make us look (as?) bad.
...
TR::Node*
vsetelemSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  // try to simplify multiple vsetelems to vsplats if they are storing the same
  // value to the same vector
  ...
  //    iconst 1
  //    ==> dload
  // Since the value being set is the same for all elements in the vector, we
  // can convert it into a single vsplats

  for (TR::Node* vsetelemNode = node;
       vsetelemNode && vsetelemNode->getOpCodeValue() == TR::vsetelem;
    ...
    // If we've seen this index before, then the outer set element overrides the
    // inner set element, so it's safe to skip this node
...
TR::Node*
v2vSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
gotoSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return _gotoSimplifier(node, block, s->_curTree, s);
}

TR::Node*
ifdCallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lcallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
acallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
vcallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  return node;
}
...
TR::Node*
treetopSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  // The treetop node should have a single child.
  //
  TR_ASSERT(
    node->getNumChildren() == 1, "Simplifier, bad treetop node %p", node);
  TR::Node* child = node->getFirstChild();
  ...
  if ((s->comp()->useCompressedPointers() && child->getOpCode().isStore() &&
       child->getType().isAddress() && child->getReferenceCount() > 1)) {
    // if the child is a store, then compressedRefs may still refer to it
    // so cannot remove it

    // compjazz 54194: This code makes the store and all its children swing
    // down, which is illegal.
...
TR::Node*
anchorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
iaddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      uint32_t value =
        firstChild->getUnsignedInt() + secondChild->getUnsignedInt();
      ...
      if (node->nodeRequiresConditionCodes())
        setCCAddSigned(
          value, firstChild->getInt(), secondChild->getInt(), node, s);
    }
    return node;
  }
  ...
  // because more negative value than positive in a 2's complement signed
  // representation
  if (nodeOp == TR::iadd && secondChildOp == TR::iconst &&
      secondChild->getInt() > 0) {
    if (performTransformation(
          s->comp(),
          "%sNormalized iadd of iconst > 0 in node [%s] to isub of -iconst\n",
          s->optDetailString(),
          node->getName(s->getDebug()))) {
      TR::Node* newSecondChild =
        (secondChild->getReferenceCount() == 1)
          ? secondChild
          : TR::Node::create(secondChild, TR::iconst, 0);
      newSecondChild->setInt(-secondChild->getInt());
      TR::Node::recreateWithoutProperties(
        node, TR::isub, 2, firstChild, newSecondChild);
      firstChild->recursivelyDecReferenceCount();
      secondChild->recursivelyDecReferenceCount();
      node->setVisitCount(0);
      s->_alteredBlock = true;
    ...
    if (secondChildOp == TR::iconst && secondChild->getInt() == -1) {
      if (performTransformation(s->comp(),
                                "%sReduced iadd of -1 and an ineg in node [%s] "
                                "to bitwise complement\n",
                                s->optDetailString(),
                                node->getName(s->getDebug()))) {
        s->anchorChildren(node, s->_curTree);
  ...
  // reduce imul operations by factoring
  else if (firstChildOp == TR::imul && firstChild->getReferenceCount() == 1 &&
           secondChildOp == TR::imul && secondChild->getReferenceCount() == 1) {
    TR::Node* llChild = firstChild->getFirstChild();
    TR::Node* lrChild = firstChild->getSecondChild();
    TR::Node* rlChild = secondChild->getFirstChild();
    TR::Node* rrChild = secondChild->getSecondChild();
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        if (!s->reassociate() && // use new rules
            performTransformation(s->comp(),
                                  "%sFound iadd of iconst with iadd or isub of "
                                  "x and const in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
          ...
          s->_alteredBlock = true;
                 ...
                 (firstChild->getReferenceCount() == 1) &&
                 performTransformation(s->comp(),
                                       "%sFound iadd of non-iconst with iadd "
                                       "or isub of x and const in node [%s]\n",
                                       s->optDetailString(),
                                       node->getName(s->getDebug()))) {
        // move constants up the tree so they will tend to get merged together
        ...
        s->_alteredBlock = true;
  ...
  } else if (nodeOp == TR::aiadd && firstChildOp == TR::aiadd &&
             ...
             firstChild->getFirstChild()->getReferenceCount() <
               (MAX_RCOUNT - 16)) {
    TR::Node* lrChild = firstChild->getSecondChild();
    if ((lrChild->getOpCodeValue() == TR::iconst) &&
        ((firstChild->getFutureUseCount() == 0) ||
      ...
      if (performTransformation(
        ...
        s->_alteredBlock = true;
             ...
             (node->getFirstChild()->getOpCodeValue() ==
              TR::a2i) && // node is iadd, so it has two children
             (node->getFirstChild()->getReferenceCount() ==
              1) && // a2i should be unique
             (node->getSecondChild()
                ->getOpCode()
                .isLoadConst()) && // iadd second child should be const to be
                                   // pushed down
             (node->getFirstChild()->getFirstChild()->getOpCodeValue() ==
              TR::aiadd) && // iadd -> a2i -> aiadd
             (node->getFirstChild()->getFirstChild()->getReferenceCount() ==
              1) // aiadd should be unique
  ) {
    // Push down the iadd into the aiadd's second child:
    //
    // n1.3083n   (  0)      iadd (in *GPR_6512) (profilingCode ) [0x2BB38260]
    // loc=[-1,2848,1153] rc=0 vc=631 vn=- sti=- udi=14723 nc=2 flg=0x80
    // n1.3081n   (  0)        a2i (in *GPR_6512) (profilingCode unneededConv )
    // [0x2BB381C0] loc=[-1,2847,1153] rc=0 vc=631 vn=- sti=- udi=14723 nc=1
    // addr=4 flg=0x8080 n1.26322n  (  0)          aiadd (in *GPR_6512)
    // (profilingCode X>=0 internalPtr )                       [0x374931DC]
    // loc=[-1,2844,1153] rc=0 vc=1859 vn=- sti=- udi=14723 nc=2 addr=4
    // flg=0x8180 n1.3077n   (  2)            ==>aload (in GPR_6513)
    // (profilingCode ) n1.3086n   (  0)            ==>iconst 692 (profilingCode
    // X!=0 X>=0 ) n1.3082n   (  0)        iconst -24 (profilingCode X!=0 X<=0 )
    // [0x2BB38210] loc=[-1,2848,1153] rc=0 vc=631 vn=- sti=- udi=- nc=0
    // flg=0x284
    //
    ...
    if ((node_aiadd->getSecondChild()->getOpCodeValue() == TR::iconst) &&
        (node->getSecondChild()->getReferenceCount() ==
         1) && // iadd iconst should be unique
        (node_aiadd->getSecondChild()->getReferenceCount() ==
         1) && // aiadd iconst should be unique
        performTransformation(s->comp(),
                              "%sPushing down iadd [%p] iconst [%p] into aiadd "
                              "[%p] iconst [%p] below\n",
                              s->optDetailString(),
                              node,
                              node->getSecondChild(),
      ...
      int64_t v = node_iadd_iconst->get64bitIntegralValue() +
                  node_aiadd->getSecondChild()->get64bitIntegralValue();
      ...
      if (passesPrecisionTest &&
          performTransformation(
            s->comp(),
            "%sPushing down iadd [%p] iconst [%p] into aiadd [%p] second child "
            "[%p], creating new iadd\n",
            s->optDetailString(),
            node,
            node->getSecondChild(),
...
TR::Node*
laddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst() &&
      performTransformation(s->comp(),
                            "%sSimplified ladd in node [" POINTER_PRINTF_FORMAT
                            "] to lconst\n",
                            s->optDetailString(),
                            node)) {
    if (node->nodeRequiresConditionCodes()) {
    ...
    int64_t val1 = (firstChild->getDataType() == TR::Address)
                     ? firstChild->getAddress()
                     : firstChild->getLongInt();
    int64_t val2 = (secondChild->getDataType() == TR::Address)
                     ? secondChild->getAddress()
                     : secondChild->getLongInt();
  ...
  // because more negative value than positive in a 2's complement signed
  // representation
  if (node->getOpCodeValue() == TR::ladd && secondChildOp == TR::lconst &&
      secondChild->getLongInt() > 0) {
    if (performTransformation(
          s->comp(),
          "%sNormalized ladd of lconst > 0 in node [" POINTER_PRINTF_FORMAT
          "] to lsub of -lconst\n",
          s->optDetailString(),
          node)) {
        ...
        secondChild->getLongInt() == -1) // -1LL works for IBM but not for MS
    {
      if (performTransformation(
            s->comp(),
            "%sReduced ladd of -1 and an lneg in node [" POINTER_PRINTF_FORMAT
            "] to bitwise complement\n",
            s->optDetailString(),
            node)) {
        s->anchorChildren(node, s->_curTree);
    ...
    // Turn add with first child negated into a subtract
      ...
      if (performTransformation(s->comp(),
                                "%sReduced ladd with negated first child in "
                                "node [" POINTER_PRINTF_FORMAT "] to lsub\n",
                                s->optDetailString(),
                                node)) {
        s->anchorChildren(node, s->_curTree);
        TR::Node::recreate(node, TR::lsub);
  ...
  // turn add with other child negated in to a subtract
    ...
    if (performTransformation(s->comp(),
                              "%sReduced ladd with negated second child in "
                              "node [" POINTER_PRINTF_FORMAT "] to lsub\n",
                              s->optDetailString(),
                              node)) {
      s->anchorChildren(node, s->_curTree);
  ...
  // reduce imul operations by factoring
  else if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1 &&
           secondChildOp == TR::lmul && secondChild->getReferenceCount() == 1) {
    TR::Node* llChild = firstChild->getFirstChild();
    TR::Node* lrChild = firstChild->getSecondChild();
    TR::Node* rlChild = secondChild->getFirstChild();
    TR::Node* rrChild = secondChild->getSecondChild();
    TR::Node* factorChild = NULL;
    if (performTransformation(s->comp(),
                              "%sFactored ladd with distributed lmul in node "
                              "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(),
                              node)) {
      if (llChild == rlChild) {
    ...
    if (secondChildOp == TR::lconst) {
      ...
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (performTransformation(s->comp(),
                                  "%sFound ladd of lconst with ladd or lsub of "
                                  "x and const in node [" POINTER_PRINTF_FORMAT
                                  "]\n",
                                  s->optDetailString(),
                                  node)) {
          if (firstChild->getReferenceCount() > 1) {
          ...
          if (secondChild->getReferenceCount() == 1) {
            ...
            TR::Node* foldedConstChild =
              TR::Node::create(secondChild, TR::lconst, 0);
            node->setAndIncChild(1, foldedConstChild);
      ...
      } else // move constants up the tree so they will tend to get merged
             // together
      {
        if ((firstChild->getReferenceCount() == 1) &&
            performTransformation(
              s->comp(),
              "%sFound ladd of non-lconst with ladd or lsub of x and lconst in "
              "node [" POINTER_PRINTF_FORMAT "]\n",
              s->optDetailString(),
              node)) {
          node->setChild(1, lrChild);
    ...
    if ((lrChild->getOpCodeValue() == TR::lconst) &&
      ...
      if (performTransformation(s->comp(),
                                "%sFound aladd of lconst with aladd x and "
                                "lconst in node [" POINTER_PRINTF_FORMAT "]\n",
                                s->optDetailString(),
                                node)) {
        int64_t value = secondChild->getLongInt();
        value += lrChild->getLongInt();
        ...
        s->_alteredBlock = true;
               ...
               performTransformation(
                 s->comp(),
                 "%sFound aladd of non-lconst with aladd x and lconst in node "
                 "[" POINTER_PRINTF_FORMAT "]\n",
                 s->optDetailString(),
                 node)) {
...
TR::Node*
faddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldFloatConstant(node,
                      TR::Compiler->arith.floatAddFloat(
                        firstChild->getFloat(), secondChild->getFloat()),
                      s);
    return node;
  }
  ...
  // If we would have ranges about f we might use the simplification in some
  // cases
  ...
  if (isOperationFPCompliant(node, firstChild, s))
    firstChild->setIsFPStrictCompliant(true);
  if (isOperationFPCompliant(node, secondChild, s))
    secondChild->setIsFPStrictCompliant(true);
  return node;
}

TR::Node*
daddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldDoubleConstant(node,
                       TR::Compiler->arith.doubleAddDouble(
                         firstChild->getDouble(), secondChild->getDouble()),
                       s);
    return node;
  }
...
TR::Node*
baddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return addSimplifier<int8_t>(node, block, s);
}

TR::Node*
saddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
isubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  scount_t futureUseCountOfFirstChild =
    node->getFirstChild()->getFutureUseCount();
  ...
  if (node->getOpCodeValue() != TR::aiadd &&
    ...
    if ((rrChild->getOpCodeValue() == TR::iconst) && (rlChild == firstChild) &&
        node->cannotOverflow() && secondChild->cannotOverflow()) {
      if (performTransformation(s->comp(),
                                "%sFolded isub with children related through "
                                "iconst in node [%s] to iconst \n",
  ...
  // turn isub with second child ineg into an iadd
    ...
    if (performTransformation(
          s->comp(),
          "%sReduced isub with negated second child in node [%s] to iadd\n",
          s->optDetailString(),
          node->getName(s->getDebug()))) {
      TR::Node* newSecondChild = secondChild->getFirstChild();
      TR::Node::recreate(node, TR::iadd);
  ...
  // quicker, but normalizes expressions and drives inegs up the tree
    ...
    if (performTransformation(s->comp(),
                              "%sReduced isub with negated first child in node "
                              "[%s] to ineg of iadd\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      TR::Node* newFirstChild = firstChild->getFirstChild();
  ...
  // turn bitwise complement + 1 into a 2's complement negate
  else if (isBitwiseIntComplement(firstChild) && secondChildOp == TR::iconst &&
           secondChild->getInt() == -1) {
    if (performTransformation(s->comp(),
                              "%sReduced isub of bitwise complement and iconst "
                              "-1 in node [%s] to 2s complement negation\n",
  ...
  // reduce imul operations by factoring
    ...
    // node->getName(s->getDebug())))
    {
      ...
      if (!factorChild ||
          !performTransformation(
            s->comp(),
            "%sFactored isub with distributed imul in node [%s]\n",
            s->optDetailString(),
            node->getName(s->getDebug()))) {
        secondChild->setChild(0, rlChild);
        secondChild->setChild(1, rrChild);
  ...
  //   that problem, the test should be moved back down there.
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        if (performTransformation(s->comp(),
                                  "%sFound isub of iconst with iadd or isub of "
                                  "x and const in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          if (firstChild->getReferenceCount() > 1) {
        ...
        if ((firstChild->getReferenceCount() == 1) &&
            (performTransformation(s->comp(),
                                   "%sFound isub of non-iconst with iadd or "
                                   "isub of x and const in node [%s]\n",
                                   s->optDetailString(),
                                   node->getName(s->getDebug())))) {
        // move constants up the tree so they will tend to get merged together
        node->setChild(1, lrChild);
...
TR::Node*
lsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldLongIntConstant(node,
                        firstChild->getLongInt() - secondChild->getLongInt(),
                        s,
                        false /* !anchorChildren */);
  ...
  if (node->getOpCodeValue() != TR::aladd &&
    ...
    if ((rrChild->getOpCodeValue() == TR::lconst) && (rlChild == firstChild) &&
        node->cannotOverflow() && secondChild->cannotOverflow()) {
      if (performTransformation(s->comp(),
                                "%sFolded lsub with children related through "
                                "lconst in node [" POINTER_PRINTF_FORMAT
                                "] to lconst \n",
                                s->optDetailString(),
                                node)) {
  ...
  // normalize subtracts of positive constants to be ladds.  Have to do it this
  // way because more negative value than positive in a 2's complement signed
  // representation
  else if (secondChildOp == TR::lconst && secondChild->getLongInt() > 0) {
    if (performTransformation(
          s->comp(),
          "%sNormalized lsub of lconst > 0 in node [" POINTER_PRINTF_FORMAT
          "] to ladd of -lconst \n",
          s->optDetailString(),
          node)) {
  ...
  // turn lsub with second child lneg into an ladd
    ...
    if (performTransformation(s->comp(),
                              "%sReduced lsub with negated second child in "
                              "node [" POINTER_PRINTF_FORMAT "] to ladd\n",
                              s->optDetailString(),
                              node)) {
      TR::Node* newSecondChild = secondChild->getFirstChild();
      TR::Node::recreate(node, TR::ladd);
  ...
  // turn lsub with first child lneg into an lneg with ladd child.  isn't
  // quicker, but normalizes expressions and drives lnegs up the tree
  else if (firstChildOp == TR::lneg) {
    if (performTransformation(s->comp(),
                              "%sReduced lsub with negated first child in node "
                              "[" POINTER_PRINTF_FORMAT "] to lneg of ladd\n",
                              s->optDetailString(),
                              node)) {
      TR::Node* newFirstChild = firstChild->getFirstChild();
  ...
  {
    if (performTransformation(s->comp(),
                              "%sReduced lsub of bitwise complement and lconst "
                              "-1 in node [" POINTER_PRINTF_FORMAT
  ...
  // reduce imul operations by factoring
           ...
           secondChildOp == TR::lmul && secondChild->getReferenceCount() == 1) {
    if (performTransformation(s->comp(),
                              "%sFactored lsub with distributed lmul in node "
                              "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(),
                              node)) {
      TR::Node* llChild = firstChild->getFirstChild();
    ...
    if (secondChildOp == TR::lconst) {
      ...
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (performTransformation(s->comp(),
                                  "%sFound lsub of lconst with ladd or lsub of "
                                  "x and lconst in node [" POINTER_PRINTF_FORMAT
                                  "]\n",
                                  s->optDetailString(),
                                  node)) {
          if (firstChild->getReferenceCount() > 1) {
             ...
             // together
      {
        if ((firstChild->getReferenceCount() == 1) &&
            performTransformation(
              s->comp(),
              "%sFound lsub of non-lconst with ladd or lsub of x and lconst in "
              "node [" POINTER_PRINTF_FORMAT "]\n",
              s->optDetailString(),
              node)) {
          node->setChild(1, lrChild);
  ...
  // 113726: turn lsub to lxor if first operand fits in 32-bit imm and is all
  // ones and the second operand is smaller than the first.
    ...
    // Check to see if the constants fit in 32-bit imm. and that all low bits in
    // lSubValue are set.
    // i.e., we are checking for something like: 63 - i%64
    if ((lSubValue & 0xFFFFFFFF) == lSubValue &&
        (lSubValue & (lSubValue + 1)) == 0 && lAndValue <= lSubValue) {
      TR::Node::recreate(node, TR::lxor);
      node->setVisitCount(0);
      s->_alteredBlock = true;
      node = s->simplify(node, block);
  ...
  // 184251
    ...
    if (shftAmnt > 0 &&
        (firstChild->getOpCodeValue() == TR::iadd ||
         firstChild->getOpCodeValue() == TR::isub) &&
      ...
      if ((iValue << shftAmnt) == (int32_t)(lValue << shftAmnt) &&
          performTransformation(
            s->comp(),
            "%sFound lsub/lmul of +ve powOf2 lconst with i2l of iadd or isub "
            "of x and iconst in node [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(),
            node)) {
      ...
      if (secondChildOp == TR::lconst) {
        ...
        if (lrChild->getOpCodeValue() == TR::iconst) {
          if (node->isNonNegative() && i2lNode->isNonNegative()) {
            // need to ensure no overflow happens on current isub operation to
            // be able to make this transformation we can loosen this up in the
            // future if we have more information on children nodes but not the
            // parents
            int64_t value = (int64_t)lrChild->getInt();
            value = -secondChild->getLongInt();
            if (performTransformation(
                  s->comp(),
                  "%sFound lsub of lconst with i2l of iadd or isub of x and "
                  "iconst in node [" POINTER_PRINTF_FORMAT "]\n",
                  s->optDetailString(),
                  node)) {
              // Reset the flags on i2lNode to reflect the new child - llChild
              ...
              if (node->getFirstChild()->getReferenceCount() > 1) {
                ...
                i2lChild->setAndIncChild(
                  0, node->getFirstChild()->getFirstChild());
                node->getFirstChild()->recursivelyDecReferenceCount();
              ...
              if (firstChild->getReferenceCount() > 1) {
                // it makes sense to uncommon it in this situation
                TR::Node* newFirstChild =
                  TR::Node::create(node->getFirstChild(), firstChildOp, 2);
                newFirstChild->setAndIncChild(0, firstChild->getFirstChild());
                newFirstChild->setAndIncChild(1, firstChild->getSecondChild());
                firstChild->recursivelyDecReferenceCount();
  ...
  if (!node->nodeRequiresConditionCodes() &&
    ...
    if (((mask + 1) & mask) == 0 && // mask + 1 power of two?
        performTransformation(
          s->comp(),
          "%sNext lower pwr of 2 using land [" POINTER_PRINTF_FORMAT "]\n",
          s->optDetailString(),
          node)) {
...
TR::Node*
fsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldFloatConstant(node,
                      TR::Compiler->arith.floatSubtractFloat(
                        firstChild->getFloat(), secondChild->getFloat()),
                      s);
    return node;
  }
  ...
  // the result of f-(-0.0) is +0.0 (and not f)
  // If we would have ranges about f we might use the simplification in some
  // cases BINARY_IDENTITY_OP(Int, FLOAT_NEG_ZERO)
...
TR::Node*
dsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldDoubleConstant(node,
                       TR::Compiler->arith.doubleSubtractDouble(
                         firstChild->getDouble(), secondChild->getDouble()),
                       s);
    return node;
  }
...
TR::Node*
bsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return subSimplifier<int8_t>(node, block, s);
}

TR::Node*
ssubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
imulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    if (node->getOpCode().isUnsigned()) {
      foldUIntConstant(node,
                       firstChild->getUnsignedInt() *
                         secondChild->getUnsignedInt(),
                       s,
                       false /* !anchorChildren*/);
    } else {
      foldIntConstant(node,
  ...
  if (firstChildOp == TR::imul && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        ...
        if (performTransformation(
              s->comp(),
              "%sFound (a*c1)*c2 => a*(c1*c2) in imul [%s]\n",
              s->optDetailString(),
              node->getName(s->getDebug()))) {
          if (secondChild->getReferenceCount() == 1) {
      ...
      // (a*c1)*b => (a*b)*c1
                 ...
                 s->comp(),
                 "%sFound (a*c1)*b => (a*b)*c1 in imul [%s]\n",
                 s->optDetailString(),
                 node->getName(s->getDebug()))) {
        // move constants up the tree so they will tend to get merged together
        node->setChild(1, lrChild);
  ...
  // see if we can distribute multiply by constant over add or sub with a
  // constant. makes for better addressing mode trees for multiple array
  // accesses using indices which differ only by constants, ex. a[i] + a[i+2]
    ...
    if (lrChild->getOpCodeValue() == TR::iconst &&
        performTransformation(s->comp(),
                              "%sFound (a+c1)*c2 = a*c2 + c1*c2 in imul [%s]\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
  ...
  } else if (secondChildOp == TR::iconst && !s->getLastRun() &&
             secondChild->getInt() != 0 &&
             !isNonNegativePowerOf2(secondChild->getInt()) &&
             secondChild->getInt() != TR::getMinSigned<TR::Int32>()) {
  ...
  if (s->getLastRun() && node->isNonNegative() && node->getOpCode().isMul() &&
    ...
    if (performTransformation(
          s->comp(),
          "%sFolded mul and div by the same constant 0x%p\n",
          s->optDetailString(),
          node)) {
      TR::Node* firstChild = node->getFirstChild();
      TR::Node* secondChild = node->getSecondChild();
  ...
  if (s->reassociate()) {
   ...
   //      *  c2    =>    t   *
    //     / \                / \
   //    t   c1             c2 c1
    if (region && node->getOpCodeValue() == TR::imul &&
      ...
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 10 to node 0x%p\n",
            s->optDetailString(),
            node)) {
   ...
   //    e   c1          e   c2
    if (region && node->getOpCodeValue() == TR::imul &&
      ...
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 11 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::iadd);
   ...
   //    e1  e2          e1  c e2  c
      ...
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 13 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::iadd);
        TR::Node* firstChild = node->getFirstChild();
   ...
   //    e1  e2          e1  c e2  c
      ...
      if (performTransformation(
            s->comp(),
            "%sApplied reassociation rule 15 to node 0x%p\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::isub);
        TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
lmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::lconst) {
      if (secondChildOp == TR::lconst) {
        if (performTransformation(s->comp(),
                                  "%sFound lmul of lconst with lmul of x and "
                                  "lconst in node [" POINTER_PRINTF_FORMAT
                                  "]\n",
                                  s->optDetailString(),
                                  node)) {
          if (secondChild->getReferenceCount() == 1) {
            ...
            TR::Node* foldedConstChild =
              TR::Node::create(secondChild, TR::lconst, 0);
            node->setAndIncChild(1, foldedConstChild);
            foldedConstChild->setLongInt(secondChild->getLongInt() *
                                         lrChild->getLongInt());
            secondChild->recursivelyDecReferenceCount();
      ...
      {
        if (performTransformation(s->comp(),
                                  "%sFound lmul of non-lconst with lmul of x "
                                  "and lconst in node [" POINTER_PRINTF_FORMAT
                                  "]\n",
                                  s->optDetailString(),
                                  node)) {
          node->setChild(1, lrChild);
    ...
    if (lrChild->getOpCodeValue() == TR::lconst &&
        performTransformation(s->comp(),
                              "%sDistributed lmul with lconst over lsub or "
                              "ladd with lconst in node [" POINTER_PRINTF_FORMAT
                              "]\n",
                              s->optDetailString(),
                              node)) {
  ...
  // the simplification is then invalid
    ...
    if (((i2lNode->getFirstChild()->getOpCodeValue() == TR::isub) ||
      ...
      if (lrChild->getOpCodeValue() == TR::iconst &&
          performTransformation(
            s->comp(),
            "%sDistributed lmul with lconst over isub or iadd of with iconst "
            "in node [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(),
            node)) {
        int64_t product =
          (int64_t)lrChild->getInt() * secondChild->getLongInt();
        if (firstChildOp == TR::isub)
          product = -product;
        TR::Node* productNode = secondChild;
        int64_t scale = secondChild->getLongInt();
  ...
  } else if (TR::Compiler->target.is64Bit() && secondChildOp == TR::lconst &&
             !s->getLastRun() && secondChild->getLongInt() != 0 &&
...
TR::Node*
fmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldFloatConstant(node,
                      TR::Compiler->arith.floatMultiplyFloat(
                        firstChild->getFloat(), secondChild->getFloat()),
                      s);
    return node;
  }
  ...
  if (bothChildrenNegated) {
    if (performTransformation(s->comp(),
                              "%sTransforming [" POINTER_PRINTF_FORMAT
                              "] (-A)*(-B) -> A*B\n",
                              s->optDetailString(),
                              node)) { // remove negates
      TR::Node* newFirstChild =
        s->replaceNode(firstChild, firstChild->getFirstChild(), s->_curTree);
      TR::Node* newSecondChild =
        s->replaceNode(secondChild, secondChild->getFirstChild(), s->_curTree);
...
TR::Node*
dmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldDoubleConstant(node,
                       TR::Compiler->arith.doubleMultiplyDouble(
                         firstChild->getDouble(), secondChild->getDouble()),
                       s);
    return node;
  }
  ...
  if (s->comp()->getOption(TR_IgnoreIEEERestrictions) &&
      ...
      !s->comp()
         ->cg()
         ->supportsNegativeFusedMultiplyAdd()) // we can create x*1.0 when
                                               // negFma is available.  Don't
                                               // undo it
  {
...
TR::Node*
bmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldByteConstant(node,
                     firstChild->getByte() * secondChild->getByte(),
                     s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
smulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node,
                         firstChild->getShortInt() * secondChild->getShortInt(),
                         s,
                         false /* !anchorChildren */);
    return node;
  }
...
TR::Node*
idivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (divisor != 0) {
      if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCode().isUnsigned()) {
          ...
          if (divisor == -1 && dividend == TR::getMinSigned<TR::Int32>())
            return s->replaceNode(node, firstChild, s->_curTree);
          foldIntConstant(
            node, dividend / divisor, s, false /* !anchorChildren*/);
        }
        ...
        if (performTransformation(
              s->comp(),
              "%sReduced idiv by -1 with ineg in node [%s]\n",
              s->optDetailString(),
              node->getName(s->getDebug()))) {
      ...
      // Design 1790
               ...
               ((shftAmnt =
                   TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(
                     divisor)) > 0) &&
               (secondChild->getReferenceCount() == 1) &&
               performTransformation(s->comp(),
                                     "%sPwr of 2 idiv opt node %p\n",
                                     s->optDetailString(),
                                     node)) {
        secondChild->decReferenceCount();
        TR::Node* newNode1;
        if (shftAmnt - 1 != 0) {
          ...
          newNode1->setSecond(TR::Node::create(
            node, TR::iconst, 0, shftAmnt - 1)); // shift amount, test for 0
        ...
        TR::Node* newNode2 =
          TR::Node::create(node, TR::iushr, 2); // another shift
        newNode2->setFirst(newNode1);
        newNode2->setSecond(TR::Node::create(
          node, TR::iconst, 0, 32 - shftAmnt)); // shift amount, test for 0
      ...
      } else if (s->cg()->getSupportsLoweringConstIDiv() &&
                 !isPowerOf2(divisor) &&
                 performTransformation(s->comp(),
                                       "%sMagic number idiv opt in node %p\n",
                                       s->optDetailString(),
                                       node)) {
        // leave idiv as is if the divisor is 2^n. CodeGen generates a fast
        // instruction squence for it. otherwise, expose the magic number
        // squence to allow optimization lowered tree will look like this: iadd
        // (node)
        //   ishr (node3)
        //     iadd (node2)
        //       iload [ dividend ]
...
// Disabled pending approval of 1055.
        ...
        // Generate a new node that has the quotient calculated using the magic
        // number method.
        TR::Node* quotient =
          getQuotientUsingMagicNumberMultiply(node, block, s);
...
TR::Node*
ldivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (divisor != 0) {
      if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (performTransformation(
              s->comp(),
              "%sReduced ldiv by -1 with lneg in node [%p]\n",
              s->optDetailString(),
              node)) {
          // make sure we don't lose grandchildren in prepareToReplaceNode
          firstChild->incReferenceCount();
      ...
      } else if (s->cg()->getSupportsLoweringConstLDivPower2() &&
                 isPowerOf2(divisor)) {
        ...
        if (firstChild->isNonNegative()) {
          if (divisor < 0) {
            if (performTransformation(
                  s->comp(),
                  "%sReduced ldiv power of 2 with lneg lshr in node [%p]\n",
                  s->optDetailString(),
                  node)) {
              TR::Node* newChild = TR::Node::create(secondChild, TR::iconst, 0);
              ...
              TR::Node* node1 =
                TR::Node::create(TR::lshr, 2, firstChild, newChild);
              s->prepareToReplaceNode(node);
              TR::Node::recreate(node, TR::lneg);
            ...
            if (performTransformation(
                  s->comp(),
                  "%sReduced ldiv power of 2 with lshr in node [%p]\n",
                  s->optDetailString(),
                  node)) {
          ...
          TR::Node* node1;
          TR::Node* shiftNode =
            TR::Node::create(secondChild, TR::iconst, 0, shiftAmount);
          // dividend proven negative
          if (divisor < 0) {
            if (performTransformation(s->comp(),
                                      "%sReduced ldiv power of 2 - neg "
                                      "nominator with lshr lneg in node [%p]\n",
                                      s->optDetailString(),
                                      node)) {
            ...
            if (performTransformation(s->comp(),
                                      "%sReduced ldiv power of 2 - neg "
                                      "nominator with lneg lneg in node [%p]\n",
                                      s->optDetailString(),
                                      node)) {
              if (s->cg()->canUseImmedInstruction(divisor - 1)) {
                TR::Node* newChild =
                  TR::Node::create(firstChild, TR::lconst, 0);
                ...
                TR::Node::recreate(node, TR::ladd);
                node->setAndIncChild(
                  0, TR::Node::create(TR::lshr, 2, firstChild, shiftNode));
                ...
                TR::Node *node_const0or1, *node_sign, *node_divisor_1,
                  *node_and, *node_add, *node_shl;
                node_divisor_1 = TR::Node::create(secondChild, TR::lconst, 0);
                node_divisor_1->setLongInt(divisor - 1);
                node_and =
                  TR::Node::create(TR::land, 2, firstChild, node_divisor_1);

                node_add =
                  TR::Node::create(TR::ladd, 2, node_and, node_divisor_1);
                ...
                node_sign = TR::Node::create(
                  TR::lshr, 2, node_shl, TR::Node::iconst(firstChild, 63));
        ...
        // Design 1790
          ...
          if (shftAmnt - 1 != 0) {
            ...
            newNode1->setFirst(firstChild);
            newNode1->setSecond(
              TR::Node::create(node, TR::iconst, 0, shftAmnt - 1));
            newNode1->getSecondChild()->incReferenceCount();
            newNode1->incReferenceCount();
          } else
            newNode1 = firstChild;
          ...
          newNode2->setFirst(newNode1);
          newNode2->setSecond(
            TR::Node::create(node, TR::iconst, 0, 64 - shftAmnt));
          newNode2->getSecondChild()->incReferenceCount();
          ...
          if (divisor > 0) {
            ...
            newNode4->setFirst(newNode3);
            newNode4->setSecond(
              TR::Node::create(node, TR::iconst, 0, shftAmnt));
            newNode4->getFirstChild()->incReferenceCount();
            newNode4->getSecondChild()->incReferenceCount();
            TR::Node::recreate(node, TR::lneg);
            node->setNumChildren(1);
      ...
      else if (s->cg()->getSupportsLoweringConstLDiv() &&
               !isPowerOf2(divisor)) {
  ...
  if (node->getOpCodeValue() == TR::ldiv) {
    ...
    if ((firstChild->getOpCodeValue() == TR::i2l) &&
        (secondChild->getOpCodeValue() == TR::i2l) &&
        performTransformation(
          s->comp(),
          "%sReduced ldiv [%p] of two i2l children to i2l of idiv \n",
          s->optDetailString(),
          node)) {
      ...
      TR::Node* divNode = TR::Node::create(
        TR::idiv, 2, firstChild->getFirstChild(), secondChild->getFirstChild());
    ...
    if (secondChild->getOpCode().isLoadConst() &&
        secondChild->getLongInt() == 10 &&
        !skipRemLoweringForPositivePowersOfTen(s) &&
        node->getFirstChild()
          ->getOpCode()
          .isLoadVar() && // transformToLongDiv creates a tonne of nodes.  Not
                          // good to do this recursively if you have chained
                          // divides
        performTransformation(s->comp(),
                              "%sReduced ldiv by 10 [%p] to bitwise ops\n",
                              s->optDetailString(),
                              node)) {
      TR::TreeTop* curTree = s->_curTree;
      TR::Node* divCheckParent = NULL;
...
TR::Node*
fdivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (firstChild->getOpCode().isLoadConst()) {
      foldFloatConstant(node,
                        TR::Compiler->arith.floatDivideFloat(
                          firstChild->getFloat(), secondChild->getFloat()),
                        s);
      return node;
    }
...
TR::Node*
ddivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    if (firstChild->getOpCode().isLoadConst() &&
        secondChild->getOpCode().isLoadConst()) {
      foldDoubleConstant(node,
                         TR::Compiler->arith.doubleDivideDouble(
                           firstChild->getDouble(), secondChild->getDouble()),
                         s);
      return node;
    }
...
TR::Node*
bdivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldByteConstant(node,
                     (int8_t)(firstChild->getByte() / secondChild->getByte()),
                     s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
sdivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldShortIntConstant(
      node,
      (int16_t)(firstChild->getShortInt() / secondChild->getShortInt()),
      s,
      false /* !anchorChildren */);
    return node;
  }
...
TR::Node*
iremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (divisor != 0) {
      if (divisor == 1 || (!isUnsigned && (divisor == -1))) {
        ...
        return node;
                 ...
                 (!isUnsigned || isNonNegativePowerOf2(divisor)) &&
                 ((shftAmnt =
                     TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(
                       divisor)) > 0) &&
                 (secondChild->getReferenceCount() == 1) &&
                 performTransformation(s->comp(),
                                       "%sPwr of 2 irem opt node %p\n",
                                       s->optDetailString(),
                                       node)) {
        ...
        if (node->getOpCodeValue() == TR::iurem) {
          secondChild->decReferenceCount();
          TR::Node* newNode =
            TR::Node::create(node, TR::iconst, 0, ((uint32_t)divisor) - 1);
          TR::Node::recreate(node, TR::iand);
          ...
          newNode2->setFirst(newNode1);
          newNode2->setSecond(
            TR::Node::create(node, TR::iconst, 0, 32 - shftAmnt));
          newNode2->getSecondChild()->incReferenceCount();
          TR::Node* newNode3 = TR::Node::create(node, TR::iadd, 2);
          newNode3->setFirst(newNode2);
          newNode3->setSecond(firstChild);
          ...
          newNode4->setFirst(newNode3);
          newNode4->setSecond(TR::Node::create(
            node, TR::iconst, 0, divisor > 0 ? -divisor : divisor));
          newNode4->getFirstChild()->incReferenceCount();
          newNode4->getSecondChild()->incReferenceCount();
          TR::Node::recreate(node, TR::isub);
          ...
          return node;
                 ...
                 s->cg()->getSupportsLoweringConstIDiv() &&
                 !isPowerOf2(divisor) && !skipRemLowering(divisor, s) &&
                 performTransformation(s->comp(),
                                       "%sMagic number irem opt in node %p\n",
                                       s->optDetailString(),
                                       node)) {
        // leave irem as is if the divisor is 2^n. CodeGen generates a fast
        // instruction squence for it. otherwise, expose the magic number
        // squence to allow optimization lowered tree will look like this:
...
TR::Node*
lremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (node->getOpCodeValue() == TR::lrem) {
    ...
    if ((firstChild->getOpCodeValue() == TR::i2l) &&
        ...
        performTransformation(
          s->comp(),
          "%sReduced lrem [%p] of two i2l children to i2l of irem \n",
          s->optDetailString(),
          node)) {
      TR::TreeTop* curTree = s->_curTree;
      TR::Node* divCheckParent = NULL;
      ...
      TR::Node* remNode = TR::Node::create(
        TR::irem, 2, firstChild->getFirstChild(), secondChild->getFirstChild());
    ...
    if (secondChild->getOpCode().isLoadConst() &&
        ...
        performTransformation(
          s->comp(),
          "%sReduced lrem by 10 [%p] to sequence of bitwise operations\n",
          s->optDetailString(),
          node)) {
      TR::TreeTop* curTree = s->_curTree;
      TR::Node* divCheckParent = NULL;
      ...
      node->setAndIncChild(0, firstChild);
      node->setAndIncChild(
        1, TR::Node::create(TR::lmul, 2, ldivNode, secondChild));
      firstChild->recursivelyDecReferenceCount();
      secondChild->recursivelyDecReferenceCount();
...
TR::Node*
fremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (firstChild->getOpCode().isLoadConst() &&
        secondChild->getOpCode().isLoadConst()) {
      foldFloatConstant(node,
                        TR::Compiler->arith.floatRemainderFloat(
                          firstChild->getFloat(), secondChild->getFloat()),
                        s);
      return node;
    }
...
TR::Node*
dremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldDoubleConstant(node,
                       TR::Compiler->arith.doubleRemainderDouble(
                         firstChild->getDouble(), secondChild->getDouble()),
                       s);
    return node;
  }
...
TR::Node*
bremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
inegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (opCode == TR::ineg) {
    ...
    if (performTransformation(
          s->comp(),
          "%sReduced ineg with isub child in node [" POINTER_PRINTF_FORMAT
          "] to isub\n",
          s->optDetailString(),
          node)) {
      TR::Node::recreate(node, TR::isub);
    ...
    if (shiftBy == 63 &&
        performTransformation(s->comp(),
                              "%sReplaced ineg of lushr by 63 with lshr node "
                              "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(),
                              node)) {
...
TR::Node*
lnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(
      node, -firstChild->getLongInt(), s, false /* !anchorChildren */);
    return node;
  }
  ...
  if (opCode == TR::lneg) {
    if (performTransformation(
          s->comp(),
          "%sCancelled lneg with lneg child in node [" POINTER_PRINTF_FORMAT
          "]\n",
          s->optDetailString(),
          node)) {
      TR::Node* grandChild = firstChild->getFirstChild();
...
TR::Node*
fnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldFloatConstant(
      node, TR::Compiler->arith.floatNegate(firstChild->getFloat()), s);
    return node;
  }
  ...
  if (firstChild->getOpCodeValue() == TR::fneg) {
    if (performTransformation(s->comp(),
                              ...
                              node)) {
      node = s->replaceNode(
        node, firstChild->getFirstChild(), s->_curTree); // remove neg
    }
  }
  // don't just do -A op B -> -(A op B) as not always a win, so catch
  // opportunity here
    ...
    if (child2Change && child2Change->getReferenceCount() == 1 &&
        performTransformation(s->comp(),
                              ...
                              node)) {
      // remove neg
      TR::Node* newChild = s->replaceNode(
        child2Change, child2Change->getFirstChild(), s->_curTree);
...
TR::Node*
dnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldDoubleConstant(
      node, TR::Compiler->arith.doubleNegate(firstChild->getDouble()), s);
    return node;
  }
  ...
  if (firstChild->getOpCodeValue() == TR::dneg) {
    ...
    if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) &&
        ...
        performTransformation(s->comp(),
                              ...
                              node)) {
      TR::Node* newConst =
        TR::Node::create(firstChild->getFirstChild(), TR::dconst, 0);
...
TR::Node*
bnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(
      node, -firstChild->getByte(), s, false /* !anchorChildren*/);
  }
...
TR::Node*
snegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(
      node, -firstChild->getShortInt(), s, false /* !anchorChildren */);
  }
  return node;
}

TR::Node*
constSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lconstSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  setIsHighWordZero(node, s);
  return node;
}
...
TR::Node*
ilfdabsSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  switch (abs) {
    ...
    case TR::fabs:
      ...
      performFlagBasedTransformation =
        AllowFlagBasedTransformationForFloatOrDouble;
      break;
    case TR::dabs:
      neg = TR::dneg;
      performFlagBasedTransformation =
  ...
  if (child->isNonNegative() && performFlagBasedTransformation &&
      performTransformation(
        s->comp(),
        "%sSimplify abs of non-negative child at [" POINTER_PRINTF_FORMAT "]\n",
        s->optDetailString(),
        node)) {
    return s->replaceNodeWithChild(node, child, s->_curTree, block);
             ...
             performTransformation(
               s->comp(),
               "%sSimplify abs of non-positive child at [" POINTER_PRINTF_FORMAT
               "]\n",
               s->optDetailString(),
               node)) {
    TR::Node::recreate(node, neg);
    return s->simplify(node, block);
  } else if ((child->getOpCodeValue() == abs ||
              child->getOpCodeValue() == neg) &&
             performTransformation(
               s->comp(),
               "%sSimplify abs of abs/neg child at [" POINTER_PRINTF_FORMAT
               "]\n",
               s->optDetailString(),
               node)) {
...
TR::Node*
ishlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
      performTransformation(
        s->comp(),
        "%sChanged ishl by const into imul by const in node [%s]\n",
        s->optDetailString(),
        node->getName(s->getDebug()))) {
...
TR::Node*
lshlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldLongIntConstant(node,
                        firstChild->getLongInt()
                          << (secondChild->getInt() & LONG_SHIFT_MASK),
                        s,
                        false /* !anchorChildren */);
    return node;
  }
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    //
    performTransformation(s->comp(),
                          "%sCanonicalize long left shift by constant in node "
                          "[" POINTER_PRINTF_FORMAT
                          "] to long multiply by power of 2\n",
                          s->optDetailString(),
                          node);
    TR::Node::recreate(node, TR::lmul);
    int64_t multiplier = (int64_t)CONSTANT64(1)
                         << (secondChild->getInt() & LONG_SHIFT_MASK);
    if (secondChild->getReferenceCount() > 1) {
      secondChild->decReferenceCount();
...
TR::Node*
bshlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sshlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
ishrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lshrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bshrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sshrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
iushrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
    ...
    if (constGrandChild->getOpCodeValue() == TR::iconst &&
      ...
      if (subTree->getReferenceCount() == 1) {
        if (opCode == TR::s2i && rightShiftValue == 16) {
          if (performTransformation(
                s->comp(),
                "%sReduced left shift followed by iushr equivalent to zero "
                "extend short in node [" POINTER_PRINTF_FORMAT "] to su2i\n",
                s->optDetailString(),
                node)) {
          ...
          if (performTransformation(
                s->comp(),
                "%sReduced left shift followed by iushr equivalent to zero "
                "extend byte in node [" POINTER_PRINTF_FORMAT "] to bu2i\n",
                s->optDetailString(),
                node)) {
      ...
      //
      if (performTransformation(s->comp(),
                                "%sReduced left shift followed by iushr in "
                                "node [" POINTER_PRINTF_FORMAT
                                "] to iand with mask\n",
                                s->optDetailString(),
                                node)) {
...
TR::Node*
lushrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  TR::Node *firstChild = node->getFirstChild(),
           *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
      ...
      firstChild->getSecondChild()->getOpCode().isLoadConst()) {
    int64_t left =
      firstChild->getSecondChild()->get64bitIntegralValue() & LONG_SHIFT_MASK;
    int64_t right = secondChild->get64bitIntegralValue() & LONG_SHIFT_MASK;

    if (right >= left && (right == 56 || right == 48 || right == 32) &&
        performTransformation(s->comp(),
                              "%sshift left followed by shift right %p of %d "
                              "can become a shift + conversion\n",
                              s->optDetailString(),
                              node,
                              right)) {
      ...
      TR::Node* shift =
        TR::Node::create(TR::lushr, 2, firstChild->getFirstChild(), amount);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node,
                        ((uint64_t)firstChild->getLongInt()) >>
                          (secondChild->getInt() & LONG_SHIFT_MASK),
                        s,
                        false /* !anchorChildren */);
    return node;
    ...
    if (constGrandChild->getOpCodeValue() == TR::lconst &&
      ...
      if (subTree->getReferenceCount() == 1) {
        if (opCode == TR::i2l && rightShiftValue == 32) {
          if (performTransformation(
                s->comp(),
                "%sReduced left shift followed by lushr equivalent to zero "
                "extend int in node [" POINTER_PRINTF_FORMAT "] to iu2l\n",
                s->optDetailString(),
                node)) {
            foundZeroExtension = true;
          ...
          if (performTransformation(
                s->comp(),
                "%sReduced left shift followed by lushr equivalent to zero "
                "extend byte in node [" POINTER_PRINTF_FORMAT "] to bu2l\n",
                s->optDetailString(),
                node)) {
            foundZeroExtension = true;
      ...
      //
      if (performTransformation(s->comp(),
                                "%sReduced left shift followed by lushr in "
                                "node [" POINTER_PRINTF_FORMAT
                                "] to land with mask\n",
                                s->optDetailString(),
                                node)) {
...
TR::Node*
bushrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sushrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node,
                         firstChild->getUnsignedShortInt() >>
                           (secondChild->getInt() & INT_SHIFT_MASK),
                         s,
                         false /* !anchorChildren */);
    return node;
...
TR::Node*
irolSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    int32_t value = (firstChild->getUnsignedInt() << rotateAmount) |
                    (firstChild->getUnsignedInt() >> (32 - rotateAmount));
...
TR::Node*
lrolSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
iandSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  // look for demorgan's law.  if found can remove one bitwise complement
  // operation
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseIntComplement(firstChild) &&
      ...
      if (performTransformation(s->comp(),
                                "%sReduced iand with two complemented children "
                                "in node [%s] to complemented ior\n",
                                s->optDetailString(),
                                node->getName(s->getDebug()))) {
        TR::Node* orNode = TR::Node::create(TR::ior,
                                            2,
                                            firstChild->getFirstChild(),
                                            secondChild->getFirstChild());
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::iconst) {
        if (secondChildOp == TR::iconst ||
            (secondChildOp == TR::iuconst && lrChild->chkUnsigned())) {
          if (performTransformation(s->comp(),
                                    "%sFound iand of iconst with iand of x and "
                                    "iconst in node [%s]\n",
                                    s->optDetailString(),
                                    node->getName(s->getDebug()))) {
            if (secondChild->getReferenceCount() == 1) {
              ...
              TR::Node* foldedConstChild =
                TR::Node::create(secondChild, TR::iconst, 0);
              foldedConstChild->setInt(secondChild->getInt() &
                                       lrChild->getInt());
              node->setSecond(
                s->replaceNode(secondChild, foldedConstChild, s->_curTree));
            }
            node->setFirst(s->replaceNode(
              firstChild, firstChild->getFirstChild(), s->_curTree));
            s->_alteredBlock = true;
          }
        } else // move constants up the tree so they will tend to get merged
               // together
        {
          if (performTransformation(s->comp(),
                                    "%sFound iand of non-iconst with iand x "
                                    "and iconst in node [%s]\n",
                                    s->optDetailString(),
                                    node->getName(s->getDebug()))) {
            node->setChild(1, lrChild);
            firstChild->setChild(1, secondChild);
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (maskValue == 255 && firstChildOp == TR::b2i) {
      if (performTransformation(
            s->comp(),
            "%sReduced iand with iconst 255 in node [%s] to bu2i\n",
            s->optDetailString(),
            node->getName(s->getDebug()))) {
        TR::Node::recreate(node, TR::bu2i);
        foundZeroExtension = true;
      }
    } else if (maskValue == 65535 &&
               (firstChildOp == TR::s2i || firstChildOp == TR::su2i)) {
      if (performTransformation(
            s->comp(),
            "%sReduced iand with iconst 65536 in node [%s] to %s\n",
            s->optDetailString(),
            node->getName(s->getDebug()),
            "su2i")) {
        TR::Node::recreate(node, TR::su2i);
  ...
  if ((node->getOpCodeValue() == TR::iand) &&
      node->getSecondChild()->getOpCode().isLoadConst()) {
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
...
TR::Node*
landSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) &&
      ...
      if (performTransformation(s->comp(),
                                "%sReduced land with two complemented children "
                                "in node [" POINTER_PRINTF_FORMAT
                                "] to complemented lor\n",
                                s->optDetailString(),
                                node)) {
        TR::Node* orNode = TR::Node::create(TR::lor,
                                            2,
                                            firstChild->getFirstChild(),
                                            secondChild->getFirstChild());
        TR::Node* constNode = firstChild->getSecondChild();
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (secondChildOp == TR::lconst) {
          if (performTransformation(s->comp(),
                                    "%sFound land of lconst with land of x and "
                                    "lconst in node [" POINTER_PRINTF_FORMAT
                                    "]\n",
                                    s->optDetailString(),
                                    node)) {
        ...
        {
          if (performTransformation(s->comp(),
                                    "%sFound land of non-lconst with land of x "
                                    "and lconst in node [" POINTER_PRINTF_FORMAT
                                    "]\n",
                                    s->optDetailString(),
                                    node)) {
    ...
    // look for opportunities to perform zero extending conversions
      ...
      if (maskValue == 255 && firstChildOp == TR::b2i) {
        if (performTransformation(
              s->comp(),
              "%sReduced land with lconst 255 in node [" POINTER_PRINTF_FORMAT
              "] to bu2l\n",
              s->optDetailString(),
              node)) {
          TR::Node::recreate(node, TR::bu2l);
        ...
        if (performTransformation(
              s->comp(),
              "%sReduced land with lconst 65536 in node [" POINTER_PRINTF_FORMAT
              "] to su2l\n",
              s->optDetailString(),
              node)) {
          TR::Node::recreate(node, TR::su2l);
        ...
        if (performTransformation(s->comp(),
                                  "%sReduced land with lconst 0xffffffff in "
                                  "node [" POINTER_PRINTF_FORMAT "] to iu2l\n",
                                  s->optDetailString(),
                                  node)) {
          TR::Node::recreate(node, TR::iu2l);
          foundZeroExtension = true;
  ...
  if (node->getOpCodeValue() == TR::land &&
    ...
    if (secondChild->isHighWordZero() &&
        ((int32_t)secondChild->getLongIntLow()) > 0) {
      TR::ILOpCodes firstChildOp = firstChild->getOpCodeValue();

      if (firstChildOp == TR::iu2l &&
          performTransformation(s->comp(),
                                "%sReduced land with lconst and iu2l child in "
                                "node [" POINTER_PRINTF_FORMAT "] to iand\n",
                                s->optDetailString(),
                                node)) {
        ...
        TR::Node* iandChild = TR::Node::create(
          TR::iand, 2, firstChild->getFirstChild(), constChild);
        TR::Node::recreate(node, firstChildOp);
        node->setNumChildren(1);
  ...
  if ((node->getOpCodeValue() == TR::land) &&
    ...
    if ((firstChild->getOpCodeValue() == TR::lor) &&
        firstChild->getSecondChild()->getOpCode().isLoadConst()) {
      if ((firstChild->getSecondChild()->getLongInt() &
           secondChild->getLongInt()) == 0) {
...
TR::Node*
bandSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sandSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
bool
isChildOrConstRedundant(TR::Node* parentOrConstNode,
                        TR::Node* childOrConstNode,
                        TR::Simplifier* s)
{
  TR_ASSERT(parentOrConstNode->getOpCode().isIntegralConst(),
            "parentOrConstNode %s (%p) not an integral const\n",
            parentOrConstNode->getOpCode().getName(),
            parentOrConstNode);
  TR_ASSERT(childOrConstNode->getOpCode().isIntegralConst(),
            "childOrConstNode %s (%p) not an integral const\n",
            childOrConstNode->getOpCode().getName(),
            childOrConstNode);
  ...
  // if the result of or'ing the two or constants is the same as the parent (int
  // or) constant then can remove child oor operation
...
//    lconst 0xF0F0F0
//
// remove lor_b as it's constant 'or' value is redundant in this subtree given
// the lor_a constant value also works for ior pairs returns the unchanged or
// new firstChild
//
// after tree
//
...
//
TR::Node*
removeRedundantIntegralOrPattern1(TR::Node* node,
                                  TR::Block* block,
                                  TR::Simplifier* s)
{
  ...
  if (!performTransformation(
        s->comp(),
        "%sRemove redundant %s 0x%llx [" POINTER_PRINTF_FORMAT
        "] under %s 0x%llx [" POINTER_PRINTF_FORMAT "]\n",
        s->optDetailString(),
        childOr->getOpCode().getName(),
        childOrConstNode->get64bitIntegralValueAsUnsigned(),
        childOr,
        parentOr->getOpCode().getName(),
        parentOrConstNode->get64bitIntegralValueAsUnsigned(),
        parentOr)) {
    return firstChild;
  }
...
//    iconst 0xF0F0F0F0
//
// remove bor as it's constant 'or' value is redundant in this subtree given the
// ior constant value and zero extension returns the unchanged or new firstChild
//
// after tree
//
//  ior
...
//
TR::Node*
removeRedundantIntegralOrPattern2(TR::Node* node,
                                  TR::Block* block,
                                  TR::Simplifier* s)
{
  TR::Node* parentOr = node;
  ...
  if (!(firstChild->getOpCodeValue() == TR::bu2i ||
        firstChild->getOpCodeValue() == TR::su2i ||
  ...
  // create a newFirstChild so we're not limited to cases where zeroExtConv has
  // a refCount=1
  TR::Node* newFirstChild = TR::Node::create(
    zeroExtConv->getOpCodeValue(), 1, childOr->getFirstChild());
  dumpOptDetails(
    s->comp(),
    "%sCreate new zero extension conversion %s [" POINTER_PRINTF_FORMAT
    "] of childOr child %s [" POINTER_PRINTF_FORMAT "]\n",
    s->optDetailString(),
    newFirstChild->getOpCode().getName(),
    newFirstChild,
    childOr->getFirstChild()->getOpCode().getName(),
    childOr->getFirstChild());
...
TR::Node*
iorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseIntComplement(firstChild) &&
        ...
        isBitwiseIntComplement(secondChild)) {
      if (performTransformation(s->comp(),
                                "%sReduced ior with two complemented children "
                                "in node [" POINTER_PRINTF_FORMAT
                                "] to complemented iand\n",
                                s->optDetailString(),
                                node)) {
        TR::Node* andNode = TR::Node::create(TR::iand,
                                             2,
                                             firstChild->getFirstChild(),
                                             secondChild->getFirstChild());
        TR::Node* constNode = firstChild->getSecondChild();
      ...
      if (lrChild->getOpCodeValue() == TR::iconst) {
        if (secondChildOp == TR::iconst) {
          ...
          if ((((~and_const) | or_const) == or_const) &&
              performTransformation(s->comp(),
                                    "%sFound ior of iconst with iand of x and "
                                    "iconst in node [" POINTER_PRINTF_FORMAT
                                    "]\n",
                                    s->optDetailString(),
                                    node)) {
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::iconst) {
        if (secondChildOp == TR::iconst) {
          if (performTransformation(s->comp(),
                                    "%sFound ior of iconst with ior of x and "
                                    "iconst in node [" POINTER_PRINTF_FORMAT
                                    "]\n",
                                    s->optDetailString(),
                                    node)) {
        ...
        {
          if (performTransformation(s->comp(),
                                    "%sFound ior of non-iconst with ior x and "
                                    "iconst in node [" POINTER_PRINTF_FORMAT
                                    "]\n",
                                    s->optDetailString(),
                                    node)) {
  ...
  firstChild =
    node->setChild(0, removeRedundantIntegralOrPattern1(node, block, s));
  firstChild =
    node->setChild(0, removeRedundantIntegralOrPattern2(node, block, s));

  return node;
}

TR::Node*
lorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) &&
        ...
        isBitwiseLongComplement(secondChild)) {
      if (performTransformation(s->comp(),
                                "%sReduced lor with two complemented children "
                                "in node [" POINTER_PRINTF_FORMAT
                                "] to complemented land\n",
                                s->optDetailString(),
                                node)) {
        TR::Node* andNode = TR::Node::create(TR::land,
                                             2,
                                             firstChild->getFirstChild(),
                                             secondChild->getFirstChild());
        TR::Node* constNode = firstChild->getSecondChild();
    ...
    // look for reassociation to fold constants
      ...
      if (lrChild->getOpCodeValue() == TR::lconst) {
        if (secondChildOp == TR::lconst) {
          if (performTransformation(s->comp(),
                                    "%sFound lor of lconst with lor of x and "
                                    "lconst in node [" POINTER_PRINTF_FORMAT
                                    "]\n",
                                    s->optDetailString(),
                                    node)) {
        ...
        {
          if (performTransformation(s->comp(),
                                    "%sFound lor of non-lconst with lor of x "
                                    "and lconst in node [" POINTER_PRINTF_FORMAT
                                    "]\n",
                                    s->optDetailString(),
                                    node)) {
  ...
  if (node->getOpCodeValue() == TR::lor &&
    ...
    if (secondChild->isHighWordZero() &&
      ...
      if (firstChildOp == TR::iu2l &&
          performTransformation(s->comp(),
                                "%sReduced lor with lconst and iu2l child in "
                                "node [" POINTER_PRINTF_FORMAT "] to ior\n",
                                s->optDetailString(),
                                node)) {
        TR::Node* constChild = NULL;
        ...
        TR::Node* iorChild =
          TR::Node::create(TR::ior, 2, firstChild->getFirstChild(), constChild);
        TR::Node::recreate(node, firstChildOp);
        node->setNumChildren(1);
  ...
  // Disable transformation if rotate evaluator does not support 64-bit
  // registers on 32-bit platform
  if ((TR::Compiler->target.is64Bit() ||
       s->comp()->cg()->use64BitRegsOn32Bit()) &&
...
TR::Node*
borSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isAnd() && firstChild->getReferenceCount() == 1 &&
      secondChild->getOpCode().isLoadConst() &&
      firstChild->getSecondChild()->getOpCode().isLoadConst() &&
      (((secondChild->getByte() | firstChild->getSecondChild()->getByte()) &
        0xFF) == 0xFF) &&
      performTransformation(s->comp(),
                            "%sReplacing bor [" POINTER_PRINTF_FORMAT
                            "] child with band child [" POINTER_PRINTF_FORMAT
                            "] \n",
                            s->optDetailString(),
                            node,
                            firstChild->getFirstChild())) {
...
TR::Node*
sorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
ixorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChildOp == TR::ixor && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        if (performTransformation(
              s->comp(),
              "%sFound ixor of iconst with ixor of x and iconst in node [%s]\n",
              s->optDetailString(),
              node->getName(s->getDebug()))) {
          if (secondChild->getReferenceCount() == 1) {
            secondChild->setInt(secondChild->getInt() ^ lrChild->getInt());
      ...
      {
        if (performTransformation(s->comp(),
                                  "%sFound ixor of non-iconst with ixor x and "
                                  "iconst in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          node->setChild(1, lrChild);
...
TR::Node*
lxorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChildOp == TR::lxor && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::lconst) {
      if (secondChildOp == TR::lconst) {
        if (performTransformation(s->comp(),
                                  "%sFound lxor of lconst with lxor of x and "
                                  "lconst in node [" POINTER_PRINTF_FORMAT
                                  "]\n",
                                  s->optDetailString(),
                                  node)) {
      ...
      {
        if (performTransformation(s->comp(),
                                  "%sFound lxor of non-lconst with lxor x and "
                                  "lconst in node [" POINTER_PRINTF_FORMAT
                                  "]\n",
                                  s->optDetailString(),
                                  node)) {
  ...
  if (node->getOpCodeValue() == TR::lxor &&
    ...
    if (secondChild->isHighWordZero() &&
      ...
      if (firstChildOp == TR::iu2l &&
          performTransformation(s->comp(),
                                "%sReduced lxor with lconst and iu2l child in "
                                "node [" POINTER_PRINTF_FORMAT "] to ixor\n",
                                s->optDetailString(),
                                node)) {
        TR::Node* constChild = NULL;
        ...
        TR::Node* ixorChild = TR::Node::create(
          TR::ixor, 2, firstChild->getFirstChild(), constChild);
        TR::Node::recreate(node, firstChildOp);
        node->setNumChildren(1);
...
TR::Node*
bxorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sxorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
template<class T>
inline TR::Node*
sqrtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (child->getOpCode().isLoadConst() &&
      performTransformation(
        s->comp(),
        "%sSimplify sqrt of const child at [" POINTER_PRINTF_FORMAT "]\n",
        s->optDetailString(),
        node)) {
    foldConstant<T>(node,
                    TR::Compiler->arith.fpSquareRoot(child->getConst<T>()),
                    s,
                    false /* !anchorChilren */);
  }
  return node;
...
TR::Node*
fsqrtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return sqrtSimplifier<float>(node, block, s);
}

TR::Node*
dsqrtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
i2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(
      node, firstChild->getInt(), s, false /* !anchorChildren */);
    return node;
  }
  ...
  if (firstChild->getReferenceCount() == 1) {
    ...
    if (childOp == TR::su2i) {
      if (performTransformation(
            s->comp(),
            "%sReduced i2l with su2i child in node [" POINTER_PRINTF_FORMAT
            "] to su2l\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::su2l);
      ...
      if (performTransformation(
            s->comp(),
            "%sReduced i2l with s2i child in node [" POINTER_PRINTF_FORMAT
            "] to s2l\n",
            s->optDetailString(),
            node)) {
      ...
      if (performTransformation(
            s->comp(),
            "%sReduced i2l with b2i child in node [" POINTER_PRINTF_FORMAT
            "] to b2l\n",
            s->optDetailString(),
            node)) {
  ...
  if (firstChild->getOpCodeValue() == TR::l2i &&
    ...
    if (shiftBy >= 57 &&
        performTransformation(
          s->comp(),
          "%sRemove i2l/l2i from lshr node [" POINTER_PRINTF_FORMAT "]\n",
          s->optDetailString(),
          node)) {
      TR::Node::recreate(node, TR::lshr);
      node->setNumChildren(2);
...
TR::Node*
i2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCodeValue() == TR::iconst) {
    uint32_t absValue =
      firstChild->getInt() >= 0 ? firstChild->getInt() : -firstChild->getInt();
...
TR::Node*
i2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
i2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  TR::Node* result;
  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, TR::b2i)))
    return result;

  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, TR::bu2i)))
    return result;
...
TR::Node*
i2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(
      node, (int16_t)firstChild->getInt(), s, false /* !anchorChildren */);
    return node;
  }

  TR::Node* result;
  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, TR::s2i)))
    return result;

  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, TR::su2i)))
    return result;
...
TR::Node*
i2aSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    ...
    foldAddressConstant(
      node, firstChild->getInt(), s, false /* !anchorChildren */);
    return node;
  } else if (firstChild->getOpCode()
               .isConversion()) // remove redundant conversions
  {
    while (firstChild->getOpCode().isConversion())
      firstChild = firstChild->getFirstChild();

    if (firstChild->getDataType() == TR::Address &&
        !firstChild->getOpCode().isLoadAddr() && TR::Compiler->target.is32Bit())
      return s->replaceNode(node, firstChild, s->_curTree);
    else
  ...
  if ((firstChild->getOpCodeValue() == TR::isub ||
       firstChild->getOpCodeValue() == TR::iadd) &&
      (s->comp()->cg()->supports32bitAiadd() ||
       node
         ->isNonNegative()) && // todo: do we know NonNegativity in simplifier?
      firstChild->getFirstChild() &&
    ...
    if (firstChild->getReferenceCount() == 1 &&
      ...
      TR::Node* newSecondChild =
        TR::Node::create(firstChild->getSecondChild(), TR::iconst, 0);
...
TR::Node*
iu2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(
      node, firstChild->getUnsignedInt(), s, false /* !anchorChildren */);
    return node;
  }
  ...
  if (firstChild->getReferenceCount() == 1) {
    ...
    if (childOp == TR::su2i) {
      if (performTransformation(
            s->comp(),
            "%sReduced iu2l with su2i child in node [" POINTER_PRINTF_FORMAT
            "] to su2l\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::su2l);
      ...
      if (performTransformation(
            s->comp(),
            "%sReduced iu2l with bu2i child in node [" POINTER_PRINTF_FORMAT
            "] to bu2l\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::bu2l);
...
TR::Node*
iu2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
iu2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
l2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return intDemoteSimplifier(node, block, s);
}

TR::Node*
l2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCodeValue() == TR::lconst) {
    ...
    uint64_t absValue = firstChild->getLongInt() >= 0
                          ? firstChild->getLongInt()
                          : -firstChild->getLongInt();
...
TR::Node*
l2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
l2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return intDemoteSimplifier(node, block, s);
}

TR::Node*
l2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return intDemoteSimplifier(node, block, s);
}

TR::Node*
l2aSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    uint8_t addrPr = (TR::Compiler->target.is64Bit() ? 8 : 4);
    foldAddressConstant(node,
                        firstChild->getLongInt() & 0xFFFFFFFF,
                        s,
                        false /* !anchorChildren */);
    return node;
  } else if (firstChild->getOpCode()
               .isConversion()) // remove redundant conversions
  {
    ...
    if (firstChild->getDataType() == TR::Address &&
        !firstChild->getOpCode().isLoadAddr() && TR::Compiler->target.is64Bit())
      return s->replaceNode(node, firstChild, s->_curTree);
    else
      firstChild = node->getFirstChild(); // reset firstChild
  }
  ...
  //   lconst -X
  // -----------------------
  if ((firstChild->getOpCodeValue() == TR::lsub ||
       firstChild->getOpCodeValue() == TR::ladd) &&
      firstChild->getFirstChild() &&
      ...
      firstChild->getSecondChild() &&
      firstChild->getSecondChild()->getOpCodeValue() == TR::lconst &&
      ((firstChild->getOpCodeValue() == TR::lsub &&
        firstChild->getSecondChild()->getLongInt() <
          0) || // todo: is positive offset only a correct soln?
       (firstChild->getOpCodeValue() == TR::ladd &&
        firstChild->getSecondChild()->getLongInt() >= 0))) {
    TR::Node* constChild = firstChild->getSecondChild();
    bool is64Bit = TR::Compiler->target.is64Bit();
    TR::ILOpCodes addressAddOp =
      ((constChild->getReferenceCount() ==
        1) && // is firstChild the only parent of constChild?
       (constChild->get64bitIntegralValue() <=
          (int64_t)CONSTANT64(
            0x000000000FFFFFFF) && // Does 64bit value fit in 32bit?
        constChild->get64bitIntegralValue() >=
          (int64_t)CONSTANT64(0xFFFFFFFFF0000000)) &&
       TR::Compiler->target.is32Bit())
        ? TR::aiadd
        : TR::aladd;

    if (!(addressAddOp == TR::aladd &&
          !is64Bit) && // aladd only allowed in 64bit
        performTransformation(s->comp(),
                              "%sTransforming %s [%s] to address add\n",
                              s->optDetailString(),
                              node->getOpCode().getName(),
                              node->getName(s->getDebug()))) {
      ...
      node->setNumChildren(
        2); // upto 2 children do not require node extension. this should
            // suffice. Any more requires Node::addChildren
      node->setAndIncChild(
        0, firstChild->getFirstChild()->getFirstChild()); // aload

      int64_t newVal = (firstChild->getOpCodeValue() == TR::lsub)
                         ? -constChild->getLongInt()
                         : constChild->getLongInt();
      constChild = TR::Node::create(
        constChild, (addressAddOp == TR::aladd) ? TR::lconst : TR::iconst, 0);
      if (addressAddOp == TR::aladd)
...
TR::Node*
lu2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lu2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
f2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldIntConstant(node,
                    floatToInt(firstChild->getFloat(), false),
                    s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
f2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node,
                        floatToLong(firstChild->getFloat(), false),
                        s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node*
f2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
f2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(node,
                     (floatToInt(firstChild->getFloat(), false) << 24) >> 24,
                     s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
f2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node,
                         (floatToInt(firstChild->getFloat(), false) << 16) >>
                           16,
                         s,
                         false /* !anchorChildren */);
    return node;
...
TR::Node*
d2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    if (node->getOpCode().isUnsigned()) {
      if (firstChild->getDouble() > 0.0) {
        foldUIntConstant(node,
                         doubleToInt(firstChild->getDouble(), false),
                         s,
                         false /* !anchorChildren */);
      }
    } else {
...
TR::Node*
d2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node,
                        doubleToLong(firstChild->getDouble(), false),
                        s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node*
d2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    {
      foldFloatConstant(
        node, TR::Compiler->arith.doubleToFloat(firstChild->getDouble()), s);
      return node;
    }
  }
  ...
  if (resultNode != NULL) {
    ...
    TR_ASSERT(firstChild->getOpCodeValue() == TR::fcall,
              "Unexpected opcode %d\n",
              firstChild->getOpCodeValue());
  }

  // because of the above, this and subsequent d2f's can become parents of
  // fcalls, so clean it up when seen
...
TR::Node*
d2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(node,
                     (doubleToInt(firstChild->getDouble(), false) << 24) >> 24,
                     s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
d2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node,
                         (doubleToInt(firstChild->getDouble(), false) << 16) >>
                           16,
                         s,
                         false /* !anchorChildren */);
    return node;
...
TR::Node*
b2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
b2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
b2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
  if (firstChild->getOpCode().isLoadConst()) {
...
TR::Node*
b2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
b2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
bu2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldIntConstant(
      node, firstChild->getUnsignedByte(), s, false /* !anchorChildren*/);
    return node;
  }
  ...
  if (firstChild->getOpCodeValue() == TR::i2b &&
      ...
      firstChild->getFirstChild()
        ...
        ->getOpCode()
        .isLoadConst()) {
    uint64_t val = firstChild->getFirstChild()
                     ->getSecondChild()
                     ->get64bitIntegralValueAsUnsigned();
    if (((val & 0xFFUL) == 0UL) &&
        performTransformation(s->comp(),
                              "%sReplacing bu2i [" POINTER_PRINTF_FORMAT
                              "] with i2b child [" POINTER_PRINTF_FORMAT
                              "] of iand [" POINTER_PRINTF_FORMAT
                              "] with mask 0, with iconst 0\n",
                              s->optDetailString(),
                              node,
                              firstChild,
                              firstChild->getFirstChild(),
      ...
      // if we're going to zero extend a zero, then we can just replace this
      // entire expression with a 0
      s->anchorNode(
        firstChild->getFirstChild()->getFirstChild(),
        s->_curTree); // only really need to anchor this child, everything else
                      // is guaranteed to not be a load
      s->prepareToReplaceNode(node, TR::iconst);
      node->setInt(0);
      return node;
    } else if (val <= (uint64_t)TR::getMaxUnsigned<TR::Int8>() &&
               performTransformation(s->comp(),
                                     "%sRemove bu2i [" POINTER_PRINTF_FORMAT
                                     "] with i2b child [" POINTER_PRINTF_FORMAT
                                     "] of iand [" POINTER_PRINTF_FORMAT
                                     "] with mask %d <= %d\n",
                                     s->optDetailString(),
                                     ...
                                     val,
                                     TR::getMaxUnsigned<TR::Int8>())) {
      // the bu2i zero extension is not needed when the grandchild iand will
      // zero out the top 3 bytes. simplify: bu2i
      //    i2b
      //       iand
      //          x
      //          iconst 0-255
      ...
      bool anchorChildren = false; // no loads are being removed (just bu2i/i2b)
                                   // so there is nothing that needs anchoring
      ...
      return grandChild;
             ...
             performTransformation(
               ...
               node,
               firstChild,
               firstChild->getFirstChild()->getOpCode().getName(),
               firstChild->getFirstChild())) {
    TR::Node* grandChild = firstChild->getFirstChild();
    grandChild->incReferenceCount();
    bool anchorChildren = false; // no loads are being removed (just bu2i/i2b)
...
TR::Node*
bu2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (firstChild->getOpCodeValue() == TR::i2b &&
    ...
    if (((val & 0xFFUL) == 0UL) &&
        performTransformation(s->comp(),
                              "%sReplacing bu2l [" POINTER_PRINTF_FORMAT
                              "] with i2b child [" POINTER_PRINTF_FORMAT
                              "] of iand [" POINTER_PRINTF_FORMAT
                              "] with mask 0, with iconst 0\n",
                              s->optDetailString(),
      ...
      return node;
               ...
               performTransformation(s->comp(),
                                     "%sReplace bu2l [" POINTER_PRINTF_FORMAT
                                     "] with i2b child [" POINTER_PRINTF_FORMAT
                                     "] of iand [" POINTER_PRINTF_FORMAT
                                     "] with mask %d <= %d with i2l\n",
                                     s->optDetailString(),
                                     node,
                                     firstChild,
                                     firstChild->getFirstChild(),
                                     val,
                                     TR::getMaxUnsigned<TR::Int8>())) {
      // the bu2l zero extension is not needed when the grandchild iand will
      // zero out the top 3 bytes. simplify: bu2l
      //    i2b
      //       iand
      //          x
      //          iconst 0-255
      ...
      node->setChild(0, grandChild);
             ...
             (firstChild->getFirstChild()->getOpCodeValue() == TR::icmpeq ||
              ...
              firstChild->getFirstChild()->getOpCodeValue() == TR::lcmpne) &&
             performTransformation(s->comp(),
                                   "%sRemove bu2l [" POINTER_PRINTF_FORMAT
                                   "] with i2b child [" POINTER_PRINTF_FORMAT
...
TR::Node*
bu2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
  if (firstChild->getOpCode().isLoadConst()) {
...
TR::Node*
bu2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bu2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
s2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getReferenceCount() == 1) {
    ...
    if (childOp == TR::bu2s) {
      if (performTransformation(
            s->comp(),
            "%sReduced s2i with bu2s child in node [" POINTER_PRINTF_FORMAT
            "] to bu2i\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::bu2i);
      ...
      if (performTransformation(
            s->comp(),
            "%sReduced s2i with b2s child in node [" POINTER_PRINTF_FORMAT
            "] to b2i\n",
            s->optDetailString(),
            node)) {
...
TR::Node*
s2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (firstChild->getReferenceCount() == 1) {
    ...
    if (childOp == TR::bu2s) {
      if (performTransformation(
            s->comp(),
            "%sReduced s2l with bu2s child in node [" POINTER_PRINTF_FORMAT
            "] to bu2l\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::bu2l);
      ...
      if (performTransformation(
            s->comp(),
            "%sReduced s2l with b2s child in node [" POINTER_PRINTF_FORMAT
            "] to b2l\n",
            s->optDetailString(),
            node)) {
        TR::Node::recreate(node, TR::b2l);
...
TR::Node*
s2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
s2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
s2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(
      node, (int8_t)firstChild->getShortInt(), s, false /* !anchorChildren*/);
    return node;
  }

  TR::Node* result;
  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, TR::b2s)))
    return result;

  if ((result =
         s->unaryCancelOutWithChild(node, firstChild, s->_curTree, TR::bu2s)))
    return result;
...
TR::Node*
su2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldIntConstant(
      node, firstChild->getUnsignedShortInt(), s, false /* !anchorChildren*/);
    return node;
  }

  TR::ILOpCodes childOp = firstChild->getOpCodeValue();
  if (firstChild->getReferenceCount() == 1) {
    bool foundFoldableConversion =
      false; // Haven't implemented any of these cases yet
    ...
    if (childOp == TR::i2s) {
      ...
      if (firstGrandChild->getReferenceCount() == 1 &&
        ...
        if (performTransformation(s->comp(),
                                  "%sReduced su2i node [" POINTER_PRINTF_FORMAT
                                  "] and i2s child [" POINTER_PRINTF_FORMAT
                                  "] to no-op\n",
                                  s->optDetailString(),
                                  node,
                                  firstChild)) {
    ...
    TR_ASSERT(
      !(foundFoldableConversion && foundDoubleFoldableConversion),
...
TR::Node*
su2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
su2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
...
TR::Node*
su2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node* firstChild = node->getFirstChild();
 ...
 * \endverbatim
 *
 * Also note that for unsigned comparison types other than equality and
 * inequality, add/sub simplifications are not possible due to how values are
 * interpreted in the current IL nodes. Essentially, the value of an iconst node
 * is neither signed nor unsigned; and its parent is free to interpret its
 * value.
 *
 ...
 * \endverbatim
 *
 * If x were -1, the first tree would evaluate to true since it's comparing if
 * zero is less than 100. But in the second tree, ifiucmplt interprets the value
 * of x as UINT_MAX. Thus, the second tree is comparing UINT_MAX with 99; and
 * the whole comparison node evalutes to false.
 *
 * Unsigned equality and inequality comparisons are not sensitive to sign
 ...
 */
TR::Node*
removeArithmeticsUnderIntegralCompare(TR::Node* node, TR::Simplifier* s)
{
  if (s->comp()->getOption(TR_DisableIntegerCompareSimplification) ||
      !(node->getOpCode()
          .isBooleanCompare() // The node must be an integer comparison node.
  ...
  // Get recognized operation types. Unsigned IL OpCodes are deprecated and not
  // supported here.
  bool isAddOp =
    (op == TR::iadd || op == TR::ladd || op == TR::sadd || op == TR::badd);
  bool isSubOp =
    (op == TR::isub || op == TR::lsub || op == TR::ssub || op == TR::bsub);

  bool isUnsignedCompare = node->getOpCode().isUnsignedCompare();
  bool isEqOrNotEqCompare = node->getOpCode().isCompareForEquality();

  bool isSignednessTypeSupported =
    isEqOrNotEqCompare || (!isUnsignedCompare && opNode->cannotOverflow());

  if ((isAddOp || isSubOp) && isSignednessTypeSupported &&
      opNode->getSecondChild()->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()
      // Only perform the simplification on the first occurance of the add/sub
      // node
    ...
    if (opNode->getOpCode().is1Byte()) {
      ...
      if (s->trace())
        traceMsg(s->comp(),
                 "\nEliminating add/sub under compare node n%dn failed due to "
                 "opcode data type\n",
                 node->getGlobalIndex());
    ...
    if (!isEqOrNotEqCompare) {
      ...
      // arithmnetic operations are defined. Need to make sure that signed
      // integer results don't overflow.
      int64_t oldConst1 = static_cast<int64_t>(oldUConst1);
      int64_t oldConst2 = static_cast<int64_t>(oldUConst2);

      bool canTransformAdd =
        isAddOp && !(oldConst1 > 0 && (oldConst2 < signedMin + oldConst1)) &&
        !(oldConst1 < 0 && (oldConst2 > signedMax + oldConst1));

      bool canTransformSub =
      ...
      if (!(canTransformAdd || canTransformSub)) {
        if (s->trace())
          traceMsg(s->comp(),
                   "\nEliminating add/sub under order compare node n%dn failed "
                   "due to overflow\n",
                   node->getGlobalIndex());
    ...
    // For both signed and unsigned comparisons, the new values are calculated
    // using the old unsigned values. The reason being that signed integer
    // overflow is undefined behavior in C++, and that unsigned integer overflow
    // is defined behavior: they shall obey the laws of arithmetic modulo 2^n
    // and produce wrapped values The IL expects integer wrapping in case of
    // overflow; and this matches unsigned integer behaviors in C++.
    newUConst = isAddOp ? (oldUConst2 - oldUConst1) : (oldUConst2 + oldUConst1);

    // Done checking constant values. Transform the tree.
    if (performTransformation(s->comp(),
                              "%sEliminating add/sub operation under integer "
                              "comparison node n%dn %s\n",
                              s->optDetailString(),
                              node->getGlobalIndex(),
                              node->getOpCode().getName())) {
      // It's not recommended to mutate the value of an existing constant node
      // by calling constNode->setConstValue() Hence, creating a new node here
      // and copy the old node's internal info.
      TR::Node* newConstNode =
        TR::Node::create(secondChild, secondChild->getOpCodeValue(), 0);
...
TR::Node*
ificmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      conditionalBranchFold((firstChild->getInt() == secondChild->getInt()),
                            node,
                            firstChild,
                            secondChild,
                            block,
  ...
  if (conditionalZeroComparisonBranchFold(
  ...
  if (firstChild->getOpCode().isRightShift() && // First child is a right shift
      ...
      firstChild->getSecondChild()->getInt() >=
        0 && // Shift value is a positive constant of reasonable size

      (firstChild->getOpCodeValue() == TR::iushr ||
       firstChild->getFirstChild()
         ->isNonNegative()) && // Either a logical shift or a positive first
                               // child to guaruntee zero-extend

      secondChild->getOpCode().isLoadConst() &&
      secondChild->getInt() == 0 && // Second child is a const 0
      performTransformation(s->comp(),
                            "%sTransform (a >> C == 0) to (a < 2^C)\n",
                            s->optDetailString())) {
    // Change if type
    TR::Node::recreate(node, TR::ifiucmplt);

    TR::Node* newSecondChild = TR::Node::create(
      node, TR::iconst, 0, 1 << firstChild->getSecondChild()->getInt());
    node->setAndIncChild(1, newSecondChild);
  ...
  if (firstChild->getOpCode().isBooleanCompare() &&
      ...
      (s->comp()->cg()->getSupportsJavaFloatSemantics() ||
       !(firstChild->getNumChildren() > 1 &&
         firstChild->getFirstChild()->getOpCode().isFloatingPoint())) &&
      performTransformation(s->comp(),
                            "%sChanging if opcode %p because first child %p is "
                            "a comparison opcode\n",
                            s->optDetailString(),
                            node,
                            firstChild)) {
  ...
  if ((firstChild->getOpCodeValue() == TR::lcmp) &&
      ...
      performTransformation(
        s->comp(),
        "%sChanging if opcode %p because first child %p is an lcmp\n",
        s->optDetailString(),
        node,
        firstChild)) {
    TR::Node::recreate(
      node, TR::iflcmpeq); // change to iflcmp since operands are longs
    node->setAndIncChild(0, firstChild->getFirstChild());
    node->setAndIncChild(1, firstChild->getSecondChild());
    firstChild->recursivelyDecReferenceCount();
    secondChild->recursivelyDecReferenceCount();
  ...
  if ((firstChild->getOpCodeValue() == TR::lcmpeq) &&
      ...
      performTransformation(
        s->comp(),
        "%sChanging if opcode %p because first child %p is an lcmpeq\n",
        s->optDetailString(),
        node,
        firstChild)) {
    TR::Node::recreate(
      node, TR::iflcmpne); // change to iflcmpne since operands are longs
    node->setAndIncChild(0, firstChild->getFirstChild());
    node->setAndIncChild(1, firstChild->getSecondChild());
    firstChild->recursivelyDecReferenceCount();
    secondChild->recursivelyDecReferenceCount();
...
TR::Node*
ificmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (firstChild->getOpCode().isRightShift() && // First child is a right shift
      ...
      (firstChild->getOpCodeValue() == TR::iushr ||
       firstChild->getFirstChild()
         ->isNonNegative()) && // Either a logical shift or a positive first
                               // child to guarantee zero-extend
  ...
  if ((firstChild->getOpCodeValue() == TR::lcmp) &&
      ...
      performTransformation(
        ...
        firstChild)) {
    TR::Node::recreate(
      node, TR::iflcmpne); // change to iflcmp since operands are longs
    node->setAndIncChild(0, firstChild->getFirstChild());
    node->setAndIncChild(1, firstChild->getSecondChild());
    firstChild->recursivelyDecReferenceCount();
    secondChild->recursivelyDecReferenceCount();
...
TR::Node*
ificmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (node->getOpCodeValue() == TR::ificmplt) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold(
          (originalFirst->getInt() < originalSecond->getInt()),
          node,
          firstChild,
          secondChild,
          block,
    ...
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold((originalFirstVal < originalSecondVal),
                              node,
                              firstChild,
                              secondChild,
                              block,
...
TR::Node*
ificmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
...
TR::Node*
ificmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
...
TR::Node*
ificmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
...
TR::Node*
iflcmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpeq) {
    longCompareNarrower(
      node, s, TR::ificmpeq, TR::ifsucmpeq, TR::ifscmpeq, TR::ifbcmpeq);
  }
...
TR::Node*
iflcmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpne) {
    longCompareNarrower(
      node, s, TR::ificmpne, TR::ifsucmpne, TR::ifscmpne, TR::ifbcmpne);
  }
  addressCompareConversion(node, s);
  removeArithmeticsUnderIntegralCompare(node, s);
...
TR::Node*
iflcmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmplt) {
    if (firstChild->getOpCode().isLoadConst() &&
      ...
      return node;
    longCompareNarrower(
      node, s, TR::ificmplt, TR::ifsucmplt, TR::ifscmplt, TR::ifbcmplt);
  } else if (node->getOpCodeValue() == TR::iflucmplt) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold((((uint64_t)originalFirst->getLongInt()) <
                               ((uint64_t)originalSecond->getLongInt())),
                              node,
                              firstChild,
                              secondChild,
                              block,
...
TR::Node*
iflcmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmple) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold(
          (originalFirst->getLongInt() <= originalSecond->getLongInt()),
          node,
          firstChild,
          secondChild,
          block,
      ...
      return node;
    longCompareNarrower(
      node, s, TR::ificmple, TR::ifsucmple, TR::ifscmple, TR::ifbcmple);
...
TR::Node*
iflcmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpgt) {
    if (firstChild->getOpCode().isLoadConst() &&
      ...
      return node;
    longCompareNarrower(
      node, s, TR::ificmpgt, TR::ifsucmpgt, TR::ifscmpgt, TR::ifbcmpgt);
...
TR::Node*
iflcmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpge) {
    if (firstChild->getOpCode().isLoadConst() &&
      ...
      return node;
    longCompareNarrower(
      node, s, TR::ificmpge, TR::ifsucmpge, TR::ifscmpge, TR::ifbcmpge);
...
TR::Node*
normalizeCmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (node->getOpCode().isBranch() &&
      (removeIfToFollowingBlock(node, block, s) == NULL))
    return NULL;
  ...
  if (node->getOpCode().isBranch()) {
    ...
    if (firstChild->getOpCode().isDouble() &&
      ...
      if (firstChild->getOpCodeValue() == TR::f2d &&
          ...
          performTransformation(s->comp(),
                                "%sDemoted double compare of TR::f2d to dconst "
                                "to float compare at node [%p]\n",
                                s->optDetailString(),
                                node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(s->comp(),
                                       "%sDemoted double compare of TR::i2d to "
                                       "dconst to int compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
      ...
      } else if (longValueInDoubleRange(node, dValue) &&
                 firstChild->getOpCodeValue() == TR::l2d &&
                 doubleConstIsRepresentableExactlyAsLong(dValue, lValue) &&
                 performTransformation(s->comp(),
                                       "%sDemoted double compare of TR::l2d to "
                                       "dconst to long compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(s->comp(),
                                       "%sDemoted double compare of TR::s2d to "
                                       "dconst to short compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(
                   s->comp(),
                   "%sDemoted double compare of TR::su2d to dconst to char "
                   "compare at node [%p]\n",
                   s->optDetailString(),
                   node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(s->comp(),
                                       "%sDemoted double compare of TR::b2d to "
                                       "dconst to byte compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
      ...
      if (intValueInFloatRange(node, fValue) &&
          firstChild->getOpCodeValue() == TR::i2f &&
          floatConstIsRepresentableExactlyAsInt(fValue, iValue) &&
          performTransformation(s->comp(),
                                "%sDemoted float compare of TR::i2f to fconst "
                                "to int compare at node [%p]\n",
                                s->optDetailString(),
                                node)) {
      ...
      } else if (longValueInFloatRange(node, fValue) &&
                 firstChild->getOpCodeValue() == TR::l2f &&
                 floatConstIsRepresentableExactlyAsLong(fValue, lValue) &&
                 performTransformation(s->comp(),
                                       "%sDemoted float compare of TR::l2f to "
                                       "fconst to long compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(s->comp(),
                                       "%sDemoted float compare of TR::s2f to "
                                       "fconst to short compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(s->comp(),
                                       "%sDemoted float compare of TR::su2f to "
                                       "fconst to char compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
        ...
        if (op != TR::BadILOp) {
          ...
          secondChild->recursivelyDecReferenceCount();
                 ...
                 performTransformation(s->comp(),
                                       "%sDemoted float compare of TR::b2f to "
                                       "fconst to byte compare at node [%p]\n",
                                       s->optDetailString(),
                                       node)) {
...
TR::Node*
ifacmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCodeValue() == TR::aconst &&
      conditionalBranchFold(
        (firstChild->getAddress() == secondChild->getAddress()),
        node,
        firstChild,
        secondChild,
        block,
  ...
  if (!(firstChild->getOpCode().hasSymbolReference() &&
        firstChild->getSymbol()->isWeakSymbol()) &&
      conditionalZeroComparisonBranchFold(
        node, firstChild, secondChild, block, s))
    return node;
...
//
TR::Node*
ifacmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  simplifyChildren(node, block, s);
...
TR::Node*
ifCmpWithEqualitySimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  simplifyChildren(node, block, s);
  ...
  TR::ILOpCode op = node->getOpCode();
  IfxcmpgeToIfxcmpeqReducer ifxcmpReducer(s, node);
  if (op.isCompareForOrder() && op.isCompareTrueIfGreater() &&
      ifxcmpReducer.isReducible())
    node = ifxcmpReducer.reduce();
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    switch (opCode) {
      case TR::ifbcmpeq:
      case TR::ifscmpeq:
        takeBranch = firstChild->get64bitIntegralValue() ==
      ...
      case TR::ifbucmple:
      case TR::ifsucmple:
        takeBranch = firstChild->get64bitIntegralValueAsUnsigned() <=
                     secondChild->get64bitIntegralValueAsUnsigned();
        foldBranch = true;
        break;
    ...
    if (foldBranch && conditionalBranchFold(
                        takeBranch, node, firstChild, secondChild, block, s)) {
      return node;
    }
  }
  ...
  if (!disableFoldIfSet && node->getOpCodeValue() == TR::ifbcmpeq) {
    if (secondChild->getOpCodeValue() == TR::bconst &&
      ...
      if (op != TR::BadILOp &&
          performTransformation(s->comp(),
                                "%sFolding ifbcmpeq of bconst 0 to boolean "
                                "compare at node [" POINTER_PRINTF_FORMAT
...
TR::Node*
ifCmpWithoutEqualitySimplifier(TR::Node* node,
                               TR::Block* block,
                               TR::Simplifier* s)
{
  if (removeIfToFollowingBlock(node, block, s) == NULL)
...
TR::Node*
icmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if ((firstChild->getOpCode().isBooleanCompare() &&
      ...
      ((secondChild->getOpCode().isLoadConst()) &&
       secondChild->getInt() == 0) &&
      performTransformation(s->comp(),
                            "%sChanging icmpeq opcode %p because first child "
                            "%p is an int compare\n",
                            s->optDetailString(),
                            node,
                            firstChild)) {
    TR::Node::recreate(node,
                       firstChild->getOpCode().getOpCodeForReverseBranch());
    node->setAndIncChild(0, firstChild->getFirstChild());
    node->setAndIncChild(1, firstChild->getSecondChild());
    firstChild->recursivelyDecReferenceCount();
    secondChild->recursivelyDecReferenceCount();
  ...
  if (firstChild->getOpCodeValue() == TR::iand &&
    ...
    if ((val & (val - 1)) == 0 && val == mask &&
        performTransformation(
          s->comp(),
          "%s Changing icmpeq (x&2**c) to 2**c node [" POINTER_PRINTF_FORMAT
          "] to iand\n",
          s->optDetailString(),
          node)) {
...
TR::Node*
icmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
icmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
icmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
icmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
icmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lcmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCodeValue() == TR::land &&
      ...
      secondChild->getOpCodeValue() == TR::lconst) {
    int64_t val1 =
      firstChild->getSecondChild()->getLongInt() & 0xffffffff80000000ull;
    int64_t val2 = secondChild->getLongInt() & 0xffffffff80000000ull;
    if (val1 == 0 && val2 == 0 &&
        performTransformation(
          s->comp(),
          "%sChanging lcmpeq %p to icmpeq because there are no upper bits\n",
          s->optDetailString(),
          node)) {
  ...
  if (firstChild->getOpCodeValue() == TR::land &&
    ...
    if ((val & (val - 1)) == 0 && val == mask &&
        performTransformation(
          s->comp(),
          "%s Changing lcmpeq (x&2**c) to 2**c node [" POINTER_PRINTF_FORMAT
          "] to land\n",
          s->optDetailString(),
          node)) {
      uint32_t shiftBy = trailingZeroes(mask);
...
TR::Node*
lcmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (firstSecond->getOpCodeValue() == TR::lshl &&
        ...
        performTransformation(
          s->comp(),
          "%slcmpne of x & (1 << y) to 0 opt node [" POINTER_PRINTF_FORMAT
          "]\n",
          s->optDetailString(),
          node)) {
      TR::Node::recreate(node, TR::iand);
      ...
      if ((val & (val - 1)) == 0 && // power of two?
          performTransformation(
            s->comp(),
            "%slcmpne of (x & 2**c) to 0 node [" POINTER_PRINTF_FORMAT "]\n",
            s->optDetailString(),
            node)) {
...
TR::Node*
lcmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
      ...
      performTransformation(
        s->comp(),
        "%sReplace lcmplt to 0 with lushr node [" POINTER_PRINTF_FORMAT "]\n",
        s->optDetailString(),
        node)) {
    TR::Node* shiftBy = TR::Node::create(node, TR::iconst, 0);
    shiftBy->setInt(63);
...
TR::Node*
lcmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lcmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lcmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lucmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(node,
                    firstChild->getUnsignedLongInt() <
                      secondChild->getUnsignedLongInt(),
                    s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
lucmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lucmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lucmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
acmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
acmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bcmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bcmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bcmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bcmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bcmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
bcmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
scmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
scmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
scmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
scmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
scmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
scmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sucmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(node,
                    firstChild->getConst<uint16_t>() ==
                      secondChild->getConst<uint16_t>(),
                    s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
sucmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sucmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sucmpgeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sucmpgtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
sucmpleSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lcmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
passThroughSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
endBlockSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  TR_ASSERT(node->getBlock() == block,
            "Simplification, BBEnd %s points to block_%d (should be %d)",
  ...
  TR_ASSERT(bbStartNode->getOpCodeValue() == TR::BBStart,
            "Simplification, BBEnd not followed by BBStart");
  ...
  bool moreThanOnePred = (!(nextBlock->getPredecessors().empty()) &&
                          (nextBlock->getPredecessors().size() > 1));

  // Exclude all the illegal cases first since we don't want to return in the
  // middle of merging
  bool blockIsEmpty;
  if (!isLegalToMerge(node,
                      block,
                      nextBlock,
                      nextExcpOut,
                      bbStartNode,
  ...
  if (moreThanOnePred)
    changeBranchDestinationsForMergeBlocks(
      block, nextBlock, bbStartNode, inEdge, inEdgeIter, s);
  ...
  if (cfg) {
    ...
    if (rootStructure != NULL) {
      ...
      // it's needed since we may merge block 2 to block 3, but first block must
      // be numbered as 2 However mergeBlock only handle structure edges and
      // numbers This is inconsistent with what we do for CFG in the following,
      // it's better for mergeBlock to merge nextBlock to block, this is a TODO
    ...
    for (auto outEdge = nextBlock->getSuccessors().begin();
         outEdge != nextBlock->getSuccessors().end();
         ++outEdge) {
      (*outEdge)->setFrom(block);
    }
    for (auto outEdge = nextExcpOut.begin(); outEdge != nextExcpOut.end();
         ++outEdge) {
      if (rootStructure != NULL)
  ...
  // it is important to copy the bytecode and frequency information in a
  // consistent manner with how it is done in other block merge operations (in
  // particular, goto elimination) so that frequency information attached to
  // bytecode isn't lost in the process of block merging. But Wcode compiler
  // which generates asm file will need the line number info to be correct for
  // BBstart node of Prolog so no copy.
  //
  // dont copy the bytecodeinfo for catch blocks. liveMonitor analysis relies on
  // accurate callerIndex information to propagate locked objects. copying can
  // result in the wrong bytecode info on the catch block, in turn incorrect gc
  // stack maps
  //
  if (!block->isCatchBlock())
    block->getEntry()->getNode()->copyByteCodeInfo(
      nextBlock->getEntry()->getNode());
...
TR::Node*
ternarySimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (node->getChild(1)->getOpCode().isLoadConst() &&
    ...
    if (node->getChild(1)->getOpCode().isInteger() &&
        node->getChild(2)->getOpCode().isInteger())
      if (node->getChild(1)->get64bitIntegralValue() ==
          node->getChild(2)->get64bitIntegralValue())
        return s->replaceNode(node, node->getChild(1), s->_curTree);
...
TR::Node*
a2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    if (firstChild->getType().isAddress())
      foldIntConstant(
        node, firstChild->getAddress(), s, false /* !anchorChildren */);
    else
      foldIntConstant(node,
                      firstChild->get64bitIntegralValue(),
                      s,
                      false /* !anchorChildren */);
  } else {
...
TR::Node*
a2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    if (firstChild->getType().isAddress()) {
      // on Z the address cast evaluation of a2l of a 4 byte addr child will
      // always zero out bit 32, need to do equivalently here
      if (TR::Compiler->target.cpu.isZ() &&
          node->getFirstChild()->getSize() == 4)
        foldLongIntConstant(node,
                            firstChild->getAddress() & CLEARBIT32,
                            s,
                            false /* !anchorChildren */);
      else
        foldLongIntConstant(
                 ...
                 TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    ...
    TR::CFG* cfg = s->comp()->getFlowGraph();
    if (cfg) {
      TR_BitVector removedTargets(
        cfg->getNumberOfNodes(), s->comp()->trMemory(), stackAlloc, growable);
      for (caseIndex = numCases + 1; caseIndex >= 1; caseIndex--) {
        ...
        if (dest != target) {
          ...
          if (!removedTargets.get(destBlock->getNumber()) &&
              performTransformation(s->comp(),
                                    "%sRemoving edge block_%d->block_%d in CFG "
                                    "because switch value is known\n",
                                    s->optDetailString(),
                                    block->getNumber(),
                                    destBlock->getNumber())) {
    ...
    // If the first child is a shift node (possibly with a conversion node as a
    // parent), we can eliminate the shift by performing the reverse shift on
    // all the case values (eg. instead of left-shifting the first child by two,
    ...
    if (node->getOpCode().getOpCodeValue() == TR::lookup && NULL != shiftNode &&
        shiftNode->getOpCode().isShift()) {
      ...
      if (NULL != shiftAmtNode && shiftAmtNode->getOpCode().isLoadConst()) {
        // In order for the transformation to be accurate, any bits shifted out
        // of either the selector node or the case constants must be 0. For the
        // constants, we can check the bits to be shifted out at compile time.
        // For the shift node, which is non-constant, the only way we know those
        // bits are 0 is if they are explicitly zeroed with an AND node.
        //
        // Example: if the selector is 3 (binary 11) and we shift it right 1,
        // and have case values 0 and 1, we'll match 1 (since 11b >> 1 is 1b) If
        // we shift 0 and 1 left 1 bit, we'll compare 11b to 00b and 10b, and
        // not have a match.
        ...
        highNBits64 = lowNBits
                      << ((shiftNode->getOpCode().getSize() * 8) - shiftAmount);

        // Make sure all the case nodes can be shifted safely
        for (int caseIndex = 0; caseIndex < numCases; caseIndex++) {
          TR::Node* caseNode = node->getChild(caseIndex + 2);
          if ((isLeftShift &&
               ((caseNode->getCaseConstant() & lowNBits) != 0)) ||
              (!isLeftShift &&
               ((caseNode->getCaseConstant() & highNBits32) != 0))) {
            canTransform = false;
            break;
          }
        }
        ...
        if (NULL != andNode && andNode->getOpCode().isAnd() &&
            NULL != andNode->getSecondChild() &&
          ...
          // If we're left-shifting, it's the high bits that need to be set
          // instead
          if (isLeftShift)
        ...
        if (!canTransform || !performTransformation(
                               s->comp(),
                               "%sRemoving shift node [" POINTER_PRINTF_FORMAT
                               "] from lookup node [" POINTER_PRINTF_FORMAT
        ...
        for (int caseIndex = 0; caseIndex < numCases; caseIndex++) {
          TR::Node* caseNode = node->getChild(caseIndex + 2);
          caseNode->setCaseConstant(
            isLeftShift ? (caseNode->getCaseConstant() >> shiftAmount)
  ...
  if (convertToGoto) {
    ...
    //
    if (!performTransformation(s->comp(),
                               "%sChanging node [" POINTER_PRINTF_FORMAT
                               "] %s into goto\n",
                               s->optDetailString(),
                               node,
                               node->getOpCode().getName()))
    ...
    // Reduce cases that are just gotos, ie make the case go where the goto
    // does.
    bool blocksWereRemoved = false;
    TR::CFG* cfg = s->comp()->getFlowGraph();
    int32_t numChildren = node->getCaseIndexUpperBound();
    for (int childIndex = 1; childIndex < numChildren;
         childIndex++) // loop from default to last case
    {
      TR::Node* childNode = node->getChild(childIndex);
      TR::Block* destBlock =
        childNode->getBranchDestination()->getNode()->getBlock();
      ...
      if (destInst->getOpCodeValue() == TR::Goto) {
        ...
        if (!performTransformation(
              s->comp(),
              "%sRedirecting switch [" POINTER_PRINTF_FORMAT
              "] child %d from block_%d to block_%d\n",
              s->optDetailString(),
              node,
              childIndex,
              destBlock->getNumber(),
              newDestBlock->getNumber()))
          return node;

        // Removing a block containing a goto shouldn't be logged for deleted
        // line processing

        childNode->setBranchDestination(newDest);
        if (cfg) {
          // Add edge from this block to new destination block it doesn't
          // already exist.
          if (!block->hasSuccessor(newDestBlock))
            cfg->addEdge(block, newDestBlock);
          // Remove edge between this block and old destination block if this is
          // the only case that leads to the block.
          int otherChildIndex;
          for (otherChildIndex = 1; otherChildIndex < numChildren;
               ++otherChildIndex) // Look for another case/default that leads to
                                  // destBlock.
            if (otherChildIndex !=
                childIndex) // Another case/default, not this case.
              if (destBlock == node->getChild(otherChildIndex)
                                 ->getBranchDestination()
                                 ->getNode()
                                 ->getBlock())
                break; // If there's another case/default to destBlock.
          if (otherChildIndex ==
              numChildren) // If another case/default to destBlock wasn't found,
            blocksWereRemoved |=
              cfg->removeEdge(block, destBlock); // remove the edge.
        }
      }
    }
    if (blocksWereRemoved)
...
static void
foldUnsignedLongIntConstant(TR::Node* node,
                            uint64_t value,
                            TR::Simplifier* s,
                            bool anchorChildrenP)
{
...
inline OMR::TR_ConditionCodeNumber
calculateSignedCC(int64_t result, bool overflow)
{
...
inline OMR::TR_ConditionCodeNumber
calculateUnsignedCC(uint64_t result, bool carryNotBorrow)
{
...
//
static uint64_t
lmulhu(uint64_t op1, uint64_t op2)
{
...
static void
foldUByteConstant(TR::Node* node,
                  uint8_t value,
                  TR::Simplifier* s,
                  bool anchorChildrenP)
{
  ...
  dumpOptDetails(s->comp(),
                 " to %s %d\n",
                 node->getOpCode().getName(),
                 node->getUnsignedByte());
}

static void
foldCharConstant(TR::Node* node,
                 uint16_t value,
                 TR::Simplifier* s,
                 bool anchorChildrenP)
{
  ...
  dumpOptDetails(s->comp(),
                 " to %s %d\n",
                 node->getOpCode().getName(),
                 node->getConst<uint16_t>());
}

static void
removeRestOfBlock(TR::TreeTop* curTree, TR::Compilation* compilation)
{
...
static bool
isNodeMulHigh(TR::Node* node)
{
  TR::ILOpCodes mulOp = node->getOpCodeValue();
  return node->getOpCode().isMul() &&
         (mulOp == TR::imulh || mulOp == TR::lmulh || mulOp == TR::lumulh ||
          mulOp == TR::iumulh);
}
...
TR::Node*
checkcastSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
      ...
      if ((prevNode->getOpCodeValue() == TR::ificmpeq) ||
        ...
        if (firstChild->getOpCodeValue() == TR:: instanceof
            && secondChild->getOpCodeValue() == TR::iconst &&
              ...
              performTransformation(
                s->comp(),
                "%sRemoving checkcast node [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(),
                node)) {
...
TR::Node*
checkcastAndNULLCHKSimplifier(TR::Node* node,
                              TR::Block* block,
                              TR::Simplifier* s)
{
...
TR::Node*
variableNewSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (node->getOpCodeValue() == TR::variableNew &&
      ...
      performTransformation(s->comp(),
                            "%sReplacing TR::variableNew %p with TR::New\n",
                            s->optDetailString(),
                            node)) {
...
TR::Node*
caddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
csubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
imulhSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  TR::Node *firstChild = node->getFirstChild(),
           *secondChild = node->getSecondChild();
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (src2 == 0 &&
        performTransformation(s->comp(),
                              "%ssecond child [%p] of node [%p] is 0, setting "
                              "the result of imulh to 0\n",
                              s->optDetailString(),
                              secondChild,
                              node)) {
      s->prepareToReplaceNode(node);
      ...
      if (firstChild->isNegative() &&
          performTransformation(s->comp(),
                                "%sfirst child [%p] of node [%p] is negative, "
                                "setting the result of imulh to -1\n",
                                s->optDetailString(),
                                firstChild,
                                node)) {
        s->prepareToReplaceNode(node);
        ...
        node->setInt(-1);
                 ...
                 performTransformation(
                   s->comp(),
                   "%sfirst child [%p] of node [%p] is non-negative, setting "
                   "the result of imulh to 0\n",
                   s->optDetailString(),
                   firstChild,
                   node)) {
    ...
    //  /    \   =>      /  \
      // src1  src2      src1 32-n
             ...
             performTransformation(s->comp(),
                                   "%ssecond child [%p] of node [%p] is 2's "
                                   "power, converting imulh to ishr\n",
                                   s->optDetailString(),
                                   secondChild,
                                   node)) {
      ...
      TR::Node::recreate(node, TR::ishr);
      TR::Node* newSecondChild = TR::Node::create(
        TR::iconst, 0); // Create a new node because changing the value of the
                        // node might affect trees referencing this node
...
TR::Node*
lmulhSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (performTransformationSimplifier(node, s)) {
      ...
      if (node->getOpCode().isUnsigned()) {
        TR::Node::recreate(node, TR::lconst);
        node->setUnsignedLongInt(lmulhu(firstChild->getUnsignedLongInt(),
                                        secondChild->getUnsignedLongInt()));
      } else {
        TR::Node::recreate(node, TR::lconst);
        node->setLongInt(
          lmulh(firstChild->getLongInt(), secondChild->getLongInt()));
      }
    }
  }
  return node;
...
TR::Node*
f2cSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldCharConstant(node,
                     floatToInt(firstChild->getFloat(), false) & 0x0000FFFF,
                     s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
d2cSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldCharConstant(node,
                     doubleToInt(firstChild->getDouble(), false) & 0x0000FFFF,
                     s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node*
expSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  return replaceExpWithMult(
    node, node->getFirstChild(), node->getSecondChild(), block, s);
}
...
//
TR::Node*
ibits2fSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
lbits2dSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
fbits2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
dbits2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
ifxcmpoSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  simplifyChildren(node, block, s);
  ...
  TR::ILOpCodes opCode = node->getOpCodeValue();
  TR_ASSERT((opCode == TR::ificmpo) || (opCode == TR::ificmpno) ||
  ...
  if (node->getFirstChild()->getOpCode().isLoadConst() &&
    ...
    int64_t res = ((opCode == TR::iflcmpo) || (opCode == TR::iflcmpno))
                    ? op1 - op2
                    : (int32_t)(op1 - op2);
...
TR::Node*
ifxcmnoSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (removeIfToFollowingBlock(node, block, s) == NULL)
    return NULL;
  simplifyChildren(node, block, s);
  ...
  if (node->getFirstChild()->getOpCode().isLoadConst() &&
    ...
    int64_t op1 = node->getFirstChild()->get64bitIntegralValue();
    int64_t op2 = node->getSecondChild()->get64bitIntegralValue();
    int64_t res = ((opCode == TR::iflcmno) || (opCode == TR::iflcmnno))
                    ? op1 + op2
                    : (int32_t)(op1 + op2);
    bool overflow = ((op1 < 0) == (op2 < 0)) && ((op1 < 0) != (res < 0));
...
//
TR::Node*
lookupSwitchSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
//
TR::Node*
tableSwitchSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
//
TR::Node*
nullchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  TR::ILOpCodes nullCheckRefOp =
    node->getNullCheckReference()->getOpCodeValue();
  if (nullCheckRefOp == TR::New || nullCheckRefOp == TR::newarray ||
      nullCheckRefOp == TR::anewarray || nullCheckRefOp == TR::multianewarray) {
    TR::Node::recreate(node, TR::treetop);
    simplifyChildren(node, block, s);
    return node;
  }
  ...
  if (node->getFirstChild()->getNumChildren() == 0) {
    dumpOptDetails(s->comp(),
                   "%sRemoving nullchk with no grandchildren in node [%s]\n",
                   s->optDetailString(),
                   node->getName(s->getDebug()));
    TR::Node::recreate(node, TR::treetop);
    s->_alteredBlock = true;
    ...
    if (refNode->isNonNull() &&
        performTransformation(s->comp(),
                              "%sRemoving redundant NULLCHK in node [%s]\n",
                              s->optDetailString(),
                              node->getName(s->getDebug())))
      TR::Node::recreate(node, TR::treetop);

    if ((refNode->isNull() || ((refNode->getOpCodeValue() == TR::aconst) &&
                               (refNode->getAddress() == 0))) &&
        performTransformation(
          s->comp(),
          "%sRemoving rest of the block past a NULLCHK that will fail [%s]\n",
          s->optDetailString(),
          node->getName(s->getDebug()))) {
      ...
      //
      TR::Node* retNode =
        TR::Node::create(s->_curTree->getNode(), TR::Return, 0);
      ...
      TR::CFG* cfg = s->comp()->getFlowGraph();
      for (auto edge = block->getSuccessors().begin();
           edge != block->getSuccessors().end();
           ++edge) {
      ...
      for (auto edge = list.begin(); edge != list.end(); ++edge) {
        if ((*edge)->getTo() != cfg->getEnd())
          s->_blockRemoved |= cfg->removeEdge(*edge);
               ...
               performTransformation(
                 s->comp(),
                 "%sNULLCHK passthrough simplification on n%dn\n",
                 s->optDetailString(),
                 node->getGlobalIndex())) {
...
//
TR::Node*
divchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (divisor->getOpCode().isLoadConst()) {
    if (((divisor->getOpCode().isLong() && divisor->getLongInt() != 0) ||
        ...
        performTransformation(
          s->comp(),
          "%sRemoved divchk with constant non-zero divisor in node[%s]\n",
          s->optDetailString(),
          node->getName(s->getDebug()))) {
      // s->removeNode(node, s->_curTree);
      // return NULL;
...
//
TR::Node*
bndchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if ((boundOp == TR::iconst) && (boundOp == indexOp) &&
  ...
  if (removableBoundCheck ||
    ...
    if (removableBoundCheck) {
      if (performTransformation(
            s->comp(),
            "%sRemoved bndchk with constant arguments in node [%s]\n",
            s->optDetailString(),
            node->getName(s->getDebug()))) {
        s->removeNode(node, s->_curTree);
        return NULL;
      ...
      if (performTransformation(s->comp(),
                                "%sRemoved bndchk with irem with arraylength "
                                "as denominator in node [%s]\n",
                                s->optDetailString(),
                                node->getName(s->getDebug()))) {
        s->removeNode(node, s->_curTree);
        return NULL;
  ...
  //        iconst k2
  //
  // which may result as a consequence of the array-length-simplification
  // optimization. In this case we are guaranteed that neither of the
  // multiplications was going to overflow, so we can safely remove them
  //
  // if the index child is a mul-high, the above transformations may not be
  // valid
  //
  if ((boundChild->getOpCode().isMul() && !isNodeMulHigh(boundChild) &&
    ...
    if (k1 == k2 && k1 > 0) {
      if (performTransformation(s->comp(),
                                "%ssimplified algebra in BNDCHK [%s]\n",
                                s->optDetailString(),
                                node->getName(s->getDebug()))) {
...
//
TR::Node*
arraycopybndchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  //
  bool removableBoundCheck = false;
  if ((lhsOp == TR::iconst) && (lhsOp == rhsOp) &&
    ...
    removableBoundCheck = true;
  else if ((rhsChild->getOpCode().isArrayLength()) && (lhsOp == TR::imul) &&
  ...
  if (removableBoundCheck) {
    if (performTransformation(s->comp(),
                              "%sRemoved arraycopy bndchk node [%s]\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      s->removeNode(node, s->_curTree);
      return NULL;
  ...
  if (rhsChild->getOpCodeValue() == TR::iadd) {
    ...
    if (lengthChild == boundChild ||
      ...
      if (indexChild->isZero()) {
        if (performTransformation(
              s->comp(),
              "%sRemoved arraycopy bndchk with zero index in node [%s]\n",
              s->optDetailString(),
              node->getName(s->getDebug()))) {
          s->removeNode(node, s->_curTree);
          return NULL;
      ...
      if (indexChild->getOpCodeValue() == TR::isub) {
        if ((indexChild->getFirstChild() == boundChild ||
          ...
          if (performTransformation(
                s->comp(),
                "%sRemoved arraycopy bndchk with len-1 index in node [%s]\n",
                s->optDetailString(),
                node->getName(s->getDebug()))) {
            s->removeNode(node, s->_curTree);
            return NULL;
        ...
        if (boundChild->getOpCodeValue() == TR::iadd &&
          ...
          if (performTransformation(
                s->comp(),
                "%sRemoved arraycopy bndchk with arrayLength index in node "
                "[%s] when bound is sum of length and index arraylengths\n",
                s->optDetailString(),
                node->getName(s->getDebug()))) {
            s->removeNode(node, s->_curTree);
            return NULL;
  ...
  // multiplications was going to overflow, so we can safely remove them
  //
  if ((lhsChild->getOpCode().isMul() &&
       lhsChild->getSecondChild()->getOpCode().isLoadConst()) &&
      (rhsChild->getOpCode().isMul() &&
...
//
TR::Node*
bndchkwithspinechkSimplifier(TR::Node* node,
                             TR::Block* block,
                             TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (boundChild->getOpCode().isLoadConst() &&
      indexChild->getOpCode().isLoadConst() &&
      (boundChild->getInt() > indexChild->getInt()) &&
      (indexChild->getInt() >= 0)) {
    if (performTransformation(
          s->comp(),
  ...
  } else if (indexOp == TR::irem &&
             indexChild->getFirstChild()->isNonNegative() &&
             boundChild == indexChild->getSecondChild()) {
    // If the bndchk is checking whether the result of an irem is in an array
    // where the length of the array was used as the denominator of the
    // remainder then the bndchk is guaranteed not to trap, so can remove it.
    // This code tends to come from hash table implementations that use arrays
    // of buckets
    //
    if (performTransformation(s->comp(),
                              "%sRemoved bndchk with irem with arraylength as "
                              "denominator in node [%s]\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
      removeBoundCheck = true;
    }
  ...
  if (boundChild->getOpCode().isLoadConst() &&
    ...
    if (elementSize > 0 &&
        !TR::Compiler->om.isDiscontiguousArray(boundChild->getInt(),
                                               elementSize) &&
        performTransformation(s->comp(),
                              "%sRemoving spine check because constant "
                              "arraylength is contiguous in node [%s]\n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
  ...
  if (removeSpineCheck) {
    ...
    if (s->trace()) {
      traceMsg(
        s->comp(),
        "removing spine check from node %p, anchoring element child to %p\n",
        node,
        treeTopNode);
    }
  }
  ...
  if ((boundChild->getOpCode().isMul() && !isNodeMulHigh(boundChild) &&
    ...
    if (k2 > 0 && ((k1 >= k2) && ((k1 % k2) == 0)) &&
        performTransformation(s->comp(),
                              ...
                              node->getName(s->getDebug()))) {
      if (boundChild->getReferenceCount() > 1) {
        node->setAndIncChild(boundChildNum,
                             TR::Node::create(node, TR::iconst, 0, k1 / k2));
        boundChild->decReferenceCount();
      } else
        boundChild->setInt(k1 / k2);
      node->setAndIncChild(indexChildNum, indexChild->getFirstChild());
...
// -------------------------------------------------------------------
TR::Node*
bcmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
bucmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
scmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
sucmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
icmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
iucmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
lucmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
...
TR::Node*
imaxminSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  bool is_signed =
    node->getOpCodeValue() == TR::imax || node->getOpCodeValue() == TR::imin;
  bool max_opcode =
    node->getOpCodeValue() == TR::imax || node->getOpCodeValue() == TR::iumax;
  TR::Node* constNode = NULL;
  ...
  if (has_const) {
    ...
    if (is_all_const) {
      if (is_signed)
        foldIntConstant(
          node, max_opcode ? max : min, s, false /* !anchorChildren*/);
      else
        foldUIntConstant(
          node, max_opcode ? umax : umin, s, false /* !anchorChildren*/);
    } else {
      if (is_signed)
...
TR::Node*
lmaxminSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  bool is_signed =
    node->getOpCodeValue() == TR::lmax || node->getOpCodeValue() == TR::lmin;
  bool max_opcode =
    node->getOpCodeValue() == TR::lmax || node->getOpCodeValue() == TR::lumax;
  TR::Node* constNode = NULL;
...
TR::Node*
fmaxminSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  for (i = 0; i < num_children; i++) {
    if (node->getChild(i)->getOpCode().isLoadConst()) {
      ...
      if (jiagblah)
        TR_ASSERT(0,
                  "Should never be considering non float children in fmaxmin "
                  "simplifier!\n");
...
TR::Node*
dmaxminSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
computeCCSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  TR_ASSERT(node->getNumChildren() == 1,
            "computeCC node should only have 1 child");
  ...
  if (cc != OMR::ConditionCodeInvalid) {
    ...
    TR_ASSERT(
      ccOriginalChildOp.getOpCodeValue() == firstChild->getOpCodeValue(),
      "computeCC's child node's opcode was changed to a different opcode");
  }

  // NOTE: we allow simplification of anchored computeCC nodes, so MUST return
  // the same node that was passed in
  return node;
}

TR::Node*
arraysetSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);

  // Reduce Int64 fill to lower size fill - this generates much more efficient
  // code on z, and there is no reason not to do this if elem child has
  // repeating pattern.
  ...
  if (fill->getOpCode().isLoadConst() && fill->getDataType() == TR::Int64) {
    ...
    if ((fillVal & 0x0FFFFFFFFL) == ((fillVal >> 32) & 0x0FFFFFFFFL) &&
        performTransformation(s->comp(),
                              "%sTransform large fill arrayset to 4byte fill "
                              "arrayset [" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(),
                              node)) {
      node->setAndIncChild(1,
                           TR::Node::iconst((int32_t)(fillVal & 0x0FFFFFFFFL)));
...
//
TR::Node*
bitOpMemSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
//
TR::Node*
bitOpMemNDSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  return node;
}
...
//
TR::Node*
arrayCmpWithPadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
...
TR::Node*
NewSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  return node;
}
...
TR::Node*
lowerTreeSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  if (node->getOpCodeValue() == TR::MethodExitHook ||
      node->getOpCodeValue() == TR::MethodEnterHook) {
...
TR::Node*
arrayLengthSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  simplifyChildren(node, block, s);
  ...
  if (node->getOpCodeValue() == TR::contigarraylength) {
    if (firstChildOp == TR::newarray || firstChildOp == TR::anewarray) {
      ...
      if (arraySize->getOpCode().isLoadConst() && elementSize > 0 &&
          !TR::Compiler->om.isDiscontiguousArray(arraySize->getInt(),
                                                 elementSize) &&
          performTransformation(s->comp(),
                                "%sReducing contiguous arraylength of newarray "
                                "or anewarray in node [%s]\n",
                                s->optDetailString(),
                                node->getName(s->getDebug()))) {
  ...
  // the first child of a newarray or anewarray is the same as the arrayLength
  // of that array
  if ((firstChildOp == TR::newarray || firstChildOp == TR::anewarray) &&
      performTransformation(
        s->comp(),
        "%sReducing arraylength of newarray or anewarray in node [%s]\n",
        s->optDetailString(),
        node->getName(s->getDebug()))) {
    TR::Node* arraySize = firstChild->getFirstChild();
    arraySize->incReferenceCount();
