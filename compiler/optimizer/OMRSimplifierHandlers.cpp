...
#define SIMPLIFY_BRANCH_ARITHMETIC(Type, Width)                                                                                                                                                                                                                                                                                                                           \
    if ((firstChild->getOpCode().isSub() || firstChild->getOpCode().isAdd()) && (firstChild->getSecondChild()->getOpCode().isLoadConst()) && (secondChild->getOpCode().isSub() || secondChild->getOpCode().isAdd()) && (secondChild->getSecondChild()->getOpCode().isLoadConst()) && (firstChild->getReferenceCount() == 1) && (secondChild->getReferenceCount() == 1)) { \
        Width konst;                                                                                                                                                                                                                                                                                                                                                      \
...
static void convertToTestUnderMask(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    */
    ...
    int32_t constVal;
    if ((iand = node->getChild(0))->getOpCodeValue() == TR::iand && node->getChild(1)->getOpCode().isLoadConst() && (constVal = node->getChild(1)->getConst<int32_t>()) == 0 && (iushr = iand->getChild(0))->getOpCodeValue() == TR::iushr && iand->getChild(1)->getOpCode().isLoadConst() && (constVal = iand->getChild(1)->getConst<int32_t>()) >= TR::getMinSigned<TR::Int8>() && constVal <= TR::getMaxSigned<TR::Int8>() && (load = iushr->getChild(0))->getOpCode().isLoadVar() && iushr->getChild(1)->getOpCode().isLoadConst() && (constVal = iushr->getChild(1)->getConst<int32_t>()) == 24 && !load->getSymbol()->isParm() && load->getOpCode().isLoadIndirect() && // Limit it to non-parm, indirect cases
        load->getReferenceCount() == 1 && performTransformation(cm, "%sTransforming iand/iushr to byte test under mask [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
        // anchor iand subtree
        ...
        node->setAndIncChild(0, TR::Node::create(TR::butest, 2, bloadi, TR::Node::bconst(iand->getChild(1)->getConst<int8_t>())));
...
{
    if (s->reassociate() && (node->getOpCode().isAdd() || node->getOpCode().isSub()) && node->getFirstChild()->getReferenceCount() > 1 && node->getSecondChild()->getOpCode().isLoadConst() && s->comp()->cg()->isMaterialized(node->getSecondChild())) {
        TR_HashId index;
        if (!s->_hashTable.locate(node->getFirstChild()->getGlobalIndex(), index)) {
            ...
            if (base_node != node && base_node->getReferenceCount() >= 1 && (base_node->getOpCodeValue() == node->getOpCodeValue()) && base_node->getFirstChild() == node->getFirstChild() &&
                // need to double check in case base_node has changed
                base_node->getSecondChild()->getOpCode().isLoadConst() && s->comp()->cg()->isMaterialized(base_node->getSecondChild())) {
                int64_t diff = node->getSecondChild()->get64bitIntegralValue() - base_node->getSecondChild()->get64bitIntegralValue();

                if (!s->comp()->cg()->shouldValueBeInACommonedNode(diff) && performTransformation(s->comp(), "%sReusing big constant from node 0x%p in node 0x%p\n", s->optDetailString(), base_node, node)) {
...
static TR::Node* addSimplifierCommon(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() && (s->_curTree->getNode()->getNumChildren() > 0) && s->_curTree->getNode()->getFirstChild()->getOpCode().isMemToMemOp()) && (s->_curTree->getNode()->getFirstChild()->getLastChild() == node))
        return node;
   ...
   //    t1  c1         t1 t2
    if (s->reassociate() && node->getOpCode().isArrayRef()) {
        firstChild = node->getFirstChild();
        secondChild = node->getSecondChild();
        if (firstChild->getOpCodeValue() == node->getOpCodeValue() && firstChild->getSecondChild()->getOpCode().isLoadConst() && !s->comp()->cg()->isMaterialized(firstChild->getSecondChild())) {
    ...
    if (s->reassociate() && node->getOpCode().isAdd()) {
      ...
      //      +-  c2    =>   t   +-
        //     / \                / \
      //    t   c1             c2 c1
        if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && ((!isExprInvariant(region, firstChild->getFirstChild()) && isExprInvariant(region, firstChild->getSecondChild()) && isExprInvariant(region, secondChild)) || (!firstChild->getFirstChild()->getOpCode().isLoadConst() && firstChild->getSecondChild()->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst())) &&

            !(firstChild->getReferenceCount() > 1 && secondChild->getOpCode().isLoadConst() && !s->comp()->cg()->isMaterialized(secondChild))) {
            if (performTransformation(s->comp(), "%sApplied reassociation rule 9 to node 0x%p\n", s->optDetailString(), node)) {
                TR::Node* newSecondChild = TR::Node::create(secondChild,
                    firstChild->getOpCode().isAdd() ? addOps[secondChild->getDataType()] : subOps[secondChild->getDataType()], 2);
      ...
      //      c1  +    =>    +   t
        //         / \        / \
      //        t   c2     c1 c2
        else if (isExprInvariant(region, firstChild) && secondChild->getOpCode().isAdd() && !isExprInvariant(region, secondChild->getFirstChild()) && isExprInvariant(region, secondChild->getSecondChild()) && !isSmallConstant(secondChild->getSecondChild(), s)) {
      ...
      //      +  t2     =>   c   +
        //     / \                / \
      //    c  t1              t1 t2
        else if (firstChild->getOpCode().isAdd() && isExprInvariant(region, firstChild->getFirstChild()) && !isExprInvariant(region, firstChild->getSecondChild()) && (!isExprInvariant(region, secondChild) || (node->getOpCode().isArrayRef() && isSmallConstant(secondChild, s))) && firstChild->getSecondChild()->getDataType() == secondChild->getDataType()) {
      ...
      //      -  c2     =>   +   t
        //     / \            / \
      //    c1  t         c1  c2
        else if (firstChild->getOpCode().isSub() && firstChild->getFirstChild()->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst() && firstChild->getFirstChild()->getDataType() == secondChild->getDataType()) {
      ...
      //    t1  c1 t2 c2     t1 t2  c1 c2    t1 c1 t2 c2     t1 t2 c1 c2
        else if (!node->getOpCode().isRef() && firstChild->getOpCodeValue() == secondChild->getOpCodeValue() && (firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && !isExprInvariant(region, firstChild->getFirstChild()) && !isExprInvariant(region, secondChild->getFirstChild()) && isExprInvariant(region, firstChild->getSecondChild()) && isExprInvariant(region, secondChild->getSecondChild())) {
      ...
      //      c1  -    =>    -   t
        //         / \        / \
      //        t   c2     c1 c2
        else if (node->getOpCode().isAdd() && !node->getOpCode().isArrayRef() && isExprInvariant(region, firstChild) && secondChild->getOpCode().isSub() && firstChild->getDataType() == secondChild->getSecondChild()->getDataType() && !isExprInvariant(region, secondChild->getFirstChild()) && isExprInvariant(region, secondChild->getSecondChild())) {
      ...
      //         t   c2     c1 ineg
        //                         \
      //                          c2
        else if (node->getOpCode().isArrayRef() && isExprInvariant(region, firstChild) && secondChild->getOpCode().isSub() && !secondChild->getFirstChild()->getOpCode().isRef() && // can't move ref to the RHS

            (!isExprInvariant(region, secondChild->getFirstChild()) || isSmallConstant(secondChild->getFirstChild(), s)) && // better address expression

            isExprInvariant(region, secondChild->getSecondChild()) && !isSmallConstant(secondChild->getSecondChild(), s)) {
...
static TR::Node* addSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // reduce imul operations by factoring
    else if (firstChildOp.isMul() && firstChild->getReferenceCount() == 1 && secondChildOp.isMul() && secondChild->getReferenceCount() == 1) {
        ...
        // if (performTransformation(s->comp(), "%sFactored iadd with distributed imul in node [%s]\n", s->optDetailString(), node->getName(s->getDebug())))
        {
            ...
            if (factorChild && !s->getLastRun() && (region = s->containingStructure()) && (isExprInvariant(region, secondChild->getFirstChild()) != isExprInvariant(region, secondChild->getSecondChild()) || isExprInvariant(region, factorChild))) // to avoid conflict with Rule 13
            {
                factorChild = NULL;
            }

            if (!factorChild || !performTransformation(s->comp(), "%sFactored iadd with distributed imul in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
    ...
    } else if (nodeOp.getOpCodeValue() != TR::aiadd && !((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() && (s->_curTree->getNode()->getNumChildren() > 0) && s->_curTree->getNode()->getFirstChild()->getOpCode().isMemToMemOp()) && (s->_curTree->getNode()->getFirstChild()->getLastChild() == node)) && (firstChildOp.isAdd() || firstChildOp.isSub())) {
        TR::Node* lrChild = firstChild->getSecondChild();
        if (lrChild->getOpCode().isLoadConst()) {
            if (secondChildOp.isLoadConst()) {
                ...
                (firstChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sFound xadd of non-xconst with xadd or isub of x and const in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
    ...
    } else if (nodeOp.getOpCodeValue() == TR::aiadd && firstChildOp.getOpCodeValue() == TR::aiadd && secondChildOp.isLoadConst()) {
        TR::Node* lrChild = firstChild->getSecondChild();
        if ((lrChild->getOpCode().isLoadConst()) && ((firstChild->getFutureUseCount() == 0) || (!s->comp()->cg()->areAssignableGPRsScarce() && (lrChild->getConst<T>() == -1 * secondChild->getConst<T>())))) {
...
TR::Node* subSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() != TR::aiadd && !((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() && (s->_curTree->getNode()->getNumChildren() > 0) && s->_curTree->getNode()->getFirstChild()->getOpCode().isMemToMemOp()) && (s->_curTree->getNode()->getFirstChild()->getLastChild() == node)) && ((node->getOpCode().isSub()) && ((secondChildOp.isAdd()) || (secondChildOp.isSub())))) {
        TR::Node* rlChild = secondChild->getFirstChild();
        TR::Node* rrChild = secondChild->getSecondChild();

        if ((rrChild->getOpCode().isLoadConst()) && (rlChild == firstChild) && node->cannotOverflow() && secondChild->cannotOverflow()) {
    ...
    // reduce imul operations by factoring
        ...
        // if (performTransformation(s->comp(), "%sFactored isub with distributed imul in node [%s]\n", s->optDetailString(), node->getName(s->getDebug())))
        {
            ...
            // and non-invariant terms together
            TR_RegionStructure* region;
            if (factorChild && (region = s->containingStructure()) && !s->getLastRun() && (isExprInvariant(region, secondChild->getFirstChild()) != isExprInvariant(region, secondChild->getSecondChild()) || isExprInvariant(region, factorChild))) // to avoid conflict with rule 13
            {
                factorChild = NULL;
            }

            if (!factorChild || !performTransformation(s->comp(), "%sFactored xsub with distributed xmul in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
                secondChild->setChild(0, rlChild);
                secondChild->setChild(1, rrChild);
            } else {
                TR::Node::recreate(node, TR::ILOpCode::getMulOpCode<T>());
    ...
    else if (!((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() && (s->_curTree->getNode()->getNumChildren() > 0) && s->_curTree->getNode()->getFirstChild()->getOpCode().isMemToMemOp()) && (s->_curTree->getNode()->getFirstChild()->getLastChild() == node)) && (firstChildOp.isAdd() || firstChildOp.isSub())) {
        TR::Node* lrChild = firstChild->getSecondChild();
        if (lrChild->getOpCode().isLoadConst()) {
            if (secondChildOp.isLoadConst()) {
    ...
    } else if (firstChild->getOpCode().isLoadConst() && firstChild->getConst<T>() == 0 && performTransformation(s->comp(), "%sReduce xsub from 0 [%s] to xneg \n", s->optDetailString(), node->getName(s->getDebug()))) {
...
static void normalizeShiftAmount(TR::Node* node, int32_t normalizationConstant, TR::Simplifier* s)
{
    if (s->comp()->cg()->needsNormalizationBeforeShifts() && !node->isNormalizedShift()) {
        ...
        //
        if ((secondChild->getOpCodeValue() != TR::iconst) && ((secondChild->getOpCodeValue() != TR::iand) || (secondChild->getSecondChild()->getOpCodeValue() != TR::iconst) || (secondChild->getSecondChild()->getInt() != normalizationConstant))) {
...
static TR::Node* foldDemotionConversion(TR::Node* node, TR::ILOpCodes opcode, TR::ILOpCodes foldedOpCode, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == opcode && performTransformation(s->comp(), "%sFolding conversion node [%s] %s and its child [%s] %s\n", s->optDetailString(), node->getName(s->getDebug()), node->getOpCode().getName(), firstChild->getName(s->getDebug()), firstChild->getOpCode().getName())) {
...
static bool reduceLongOp(TR::Node* node, TR::Block* block, TR::Simplifier* s, TR::ILOpCodes newConversionOp)
{
    ...
    if (newConversionOp == TR::BadILOp) {
        ...
        node->setAndIncChild(1, convertSecondChild ? TR::Node::create(TR::l2i, 1, firstChild->getSecondChild()) : firstChild->getSecondChild());
            ...
            for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();) {
                ...
                if (gotoTree) {
                    ...
                    if (predEndWithGoto || isImmediatePred) {
                        ...
                        if (storeNode->getOpCode().isStoreDirect() && storeSymRef == symRef && storeNode->getFirstChild()->getOpCode().isLoadConst()) {
...
static void addressCompareConversion(TR::Node* node, TR::Simplifier* s)
{
    ...
    if (firstOp == TR::a2i && firstChild->getFirstChild()->getType().isAddress() && TR::Compiler->target.is32Bit() && firstChild->getReferenceCount() == 1) {
    ...
    } else if (firstOp == TR::a2l && firstChild->getFirstChild()->getType().isAddress() && TR::Compiler->target.is64Bit() && firstChild->getReferenceCount() == 1) {
...
static void longCompareNarrower(TR::Node* node, TR::Simplifier* s, TR::ILOpCodes intOp, TR::ILOpCodes charOp, TR::ILOpCodes shortOp, TR::ILOpCodes byteOp)
{
    ...
    //    performTransformation(s->comp(), "%sLong compare narrower for node [%p]\n", s->optDetailString(), node))
    {
        TR::ILOpCodes firstOp = firstChild->getOpCodeValue();
        TR::ILOpCodes secondOp = secondChild->getOpCodeValue();
        if ((firstOp == TR::i2l) && performTransformation(s->comp(), "%sLong compare narrower for node [%p]\n", s->optDetailString(), node)) {
            if (secondOp == TR::iconst || secondOp == TR::i2l || (secondOp == TR::lconst && secondChild->getLongInt() >= INT_MIN && secondChild->getLongInt() <= INT_MAX)) {
                node->setAndIncChild(0, firstChild->getFirstChild());
        ...
        } else if (node->getOpCode().isCompareForEquality() && firstOp == TR::land && firstChild->getSecondChild()->getOpCodeValue() == TR::lconst && secondOp == TR::lconst) {
            uint64_t val1 = firstChild->getSecondChild()->getLongInt();
            uint64_t val2 = secondChild->getLongInt();
            if ((val1 & 0xffffffff00000000ull) == 0 && (val2 & 0xffffffff00000000ull) == 0 && performTransformation(s->comp(), "%sLong compare narrower: equality with no upper bits [%p]\n", s->optDetailString(), node)) {
                TR::Node::recreate(node, intOp);
                node->notifyChangeToValueOfNode();
        ...
        } else if (s->cg()->getSupportsEfficientNarrowIntComputation() && performTransformation(s->comp(), "%sLong compare narrower for node [%p]\n", s->optDetailString(), node)) {
            if (firstOp == TR::su2l) {
                if (secondOp == TR::cconst || secondOp == TR::su2l || (secondOp == TR::lconst && secondChild->getLongInt() >= 0 && secondChild->getLongInt() <= USHRT_MAX)) {
                    node->setAndIncChild(0, firstChild->getFirstChild());
                ...
                if (secondOp == TR::sconst || secondOp == TR::s2l || (secondOp == TR::lconst && secondChild->getLongInt() >= s->cg()->getMinShortForLongCompareNarrower() && secondChild->getLongInt() <= SHRT_MAX)) {
                    node->setAndIncChild(0, firstChild->getFirstChild());
                ...
                if (secondOp == TR::bconst || secondOp == TR::b2l || (secondOp == TR::lconst && secondChild->getLongInt() >= s->cg()->getMinByteForLongCompareNarrower() && secondChild->getLongInt() <= SCHAR_MAX)) {
                    node->setAndIncChild(0, firstChild->getFirstChild());
...
static void ifjlClassSimplifier(TR::Node* node, TR::Simplifier* s)
{
    ...
    if (node->getFirstChild()->getOpCode().hasSymbolReference() && node->getSecondChild()->getOpCode().hasSymbolReference() && (node->getFirstChild()->getSymbolReference() == node->getSecondChild()->getSymbolReference()) && (node->getFirstChild()->getSymbolReference() == s->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) && performTransformation(s->comp(), "%sSimplify test on j/l/Class children [%p]\n", s->optDetailString(), node)) {
        ...
        if (count == 1) {
            if (operationType[from] == OP_MINUS) {
                ...
                if (bitPosition[from] != 0) {
                    TR::Node* constNode1 = isLong ? (TR::Node::lconst(parentNode, (int64_t)1 << (int32_t)(bitPosition[from]))) : (TR::Node::iconst(parentNode, 1 << (int32_t)(bitPosition[from])));
            ...
            if (operationType[from + 1] == OP_MINUS) {
                ...
                if (bitPosition[from + 1] == 0) {
                    ...
                    TR::Node* constNode2 = isLong ? (TR::Node::lconst(parentNode, (int64_t)1 << (int32_t)(bitPosition[from + 1]))) : (TR::Node::iconst(parentNode, 1 << (int32_t)(bitPosition[from + 1])));
                ...
                if (operationType[from] == OP_MINUS) {
                    ...
                    if (bitPosition[from + 1] == 0) {
                        ...
                        TR::Node* constNode1 = isLong ? (TR::Node::lconst(parentNode, (int64_t)1 << (int32_t)(bitPosition[from + 1]))) : (TR::Node::iconst(parentNode, 1 << (int32_t)(bitPosition[from + 1])));
                        firstChild = TR::Node::create(mul_op, 2, multiplierChild, constNode1);
                    }
                    ...
                    if (bitPosition[from] == 0) {
                        ...
                        TR::Node* constNode2 = isLong ? (TR::Node::lconst(parentNode, (int64_t)1 << (int32_t)(bitPosition[from]))) : (TR::Node::iconst(parentNode, 1 << (int32_t)(bitPosition[from])));
                        secondChild = TR::Node::create(mul_op, 2, multiplierChild, constNode2);
                    }
...
static void decomposeMultiply(TR::Node* node, TR::Simplifier* s, bool isLong)
{
    ...
    if ((!s->containingStructure() || s->getLastRun()) && comp->cg()->mulDecompositionCostIsJustified(count, bitPosition, operationType, isLong ? secondChild->getLongInt() : secondChild->getInt()) && performTransformation(comp, "%sDecomposing mul with a constant, to shift left, "
    ...
    TR::Node*& node, TR::Node* firstChild, TR::Node* secondChild, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((firstChild->isNonZero() || firstChild->isNonNull()) && secondChild->getOpCode().isLoadConst() && secondChild->getIntegerNodeValue<int64_t>() == 0) {
    ...
    TR::Node* bbStartNode, TR::CFGEdgeList& inEdge, TR::Simplifier* s, bool& blockIsEmpty)
{
    ...
    if (block->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets() || (block->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::treetop && block->getLastRealTreeTop()->getNode()->getFirstChild()->getOpCode().isJumpWithMultipleTargets()))
        return false;
...
static void intCompareNarrower(TR::Node* node, TR::Simplifier* s, TR::ILOpCodes ushortOp, TR::ILOpCodes shortOp, TR::ILOpCodes byteOp)
{
    ...
    if (s->cg()->getSupportsEfficientNarrowIntComputation()) {
        ...
        if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
            if (secondOp == TR::su2i || (secondOp == TR::iconst && secondChild->getInt() >= 0 && secondChild->getInt() <= USHRT_MAX)) {
                node->setAndIncChild(0, firstChild->getFirstChild());
            ...
            if (secondOp == TR::s2i || (secondOp == TR::iconst && secondChild->getInt() >= SHRT_MIN && secondChild->getInt() <= SHRT_MAX)) {
                node->setAndIncChild(0, firstChild->getFirstChild());
                TR::Node::recreate(node, shortOp);
                firstChild->recursivelyDecReferenceCount();
                if (secondOp == TR::s2i) {
            ...
            if (secondOp == TR::b2i || (secondOp == TR::iconst && secondChild->getInt() >= SCHAR_MIN && secondChild->getInt() <= SCHAR_MAX)) {
                node->setAndIncChild(0, firstChild->getFirstChild());
                TR::Node::recreate(node, byteOp);
                firstChild->recursivelyDecReferenceCount();
                if (secondOp == TR::b2i) {
...
static void unsignedIntCompareNarrower(TR::Node* node, TR::Simplifier* s, TR::ILOpCodes ushortOp, TR::ILOpCodes ubyteOp)
{
    ...
    if (s->cg()->getSupportsEfficientNarrowUnsignedIntComputation()) {
        ...
        TR::ILOpCodes secondOp = secondChild->getOpCodeValue();
        if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
            if (secondOp == TR::su2i || (secondOp == TR::iuconst && secondChild->getUnsignedInt() <= USHRT_MAX)) {
                node->setAndIncChild(0, firstChild->getFirstChild());
                TR::Node::recreate(node, ushortOp);
                firstChild->recursivelyDecReferenceCount();
                if (secondOp == TR::su2i) {
            ...
            if (secondOp == TR::b2i || (secondOp == TR::iuconst && secondChild->getUnsignedInt() <= SCHAR_MAX)) {
                node->setAndIncChild(0, firstChild->getFirstChild());
...
static int64_t doubleToLong(double value, bool roundUp)
{
    ...
    int64_t result;
    if ((pattern & DOUBLE_ORDER(CONSTANT64(0x7ff0000000000000))) == DOUBLE_ORDER(CONSTANT64(0x7ff0000000000000)) && (pattern & DOUBLE_ORDER(CONSTANT64(0x000fffffffffffff))) != 0)
...
static void bitTestingOp(TR::Node* node, TR::Simplifier* s)
{
    ...
    if (TR::ILOpCode::isEqualCmp(node->getOpCode().getOpCodeValue()) && TR::ILOpCode::isNotEqualCmp(node->getOpCode().getOpCodeValue()))
        return;
...
static bool isOperationFPCompliant(TR::Node* parent, TR::Node* arithExprNode, TR::Simplifier* s)
{
    ...
    * less precision to double precision then it's safe to convert them */
    if (mulNode->getDataType() == TR::Double && mulNode->getFirstChild()->getOpCode().isConversion() && mulNode->getSecondChild()->getOpCode().isConversion()) { // long->double is only case where we must give up
    ...
    if (!mulChildIsConst && s->cg()->supportsOnDemandLiteralPool()) {
        ...
        if (mulChildIsConst || (mulConstChild->getOpCode().isLoadIndirect() && mulConstChild->getSymbolReference()->isLiteralPoolAddress())) {
    ...
    if (mulNonConstChild->getOpCode().isConversion()) {
        ...
        case TR::Double: // rely on fact same storage used for longs
            ...
            if (32 == bitcount)
                bitcount += trailingZeroes(0x000fffff & mulConstChild->getLongIntHigh());
        ...
        if (i2dConv) {
            ...
            if (possibleRoundedValue >= FMA_CONST_HIGHBOUNDF2D || possibleRoundedValue < FMA_CONST_LOWBOUND) {
                if (traceIt)
                    traceMsg(s->comp(), "Fails because |constant| %e exceeds range %e-%e \n", possibleRoundedValue, FMA_CONST_LOWBOUND,
    ...
    bool loadConstIsPowerOfTwoDbl = (mulConstChild->getDataType() == TR::Double) && isNZDoublePowerOfTwo(mulConstChild->getDouble());

    bool loadConstIsPowerOfTwoFloat = (mulConstChild->getDataType() == TR::Float) && isNZFloatPowerOfTwo(mulConstChild->getFloat());
    ...
    if (traceIt)
        traceMsg(s->comp(), "Fails because strict, no const or bitcount exceeded\n");
    return false;
}
...
static void bitwiseToLogical(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR::Node::recreate(node, (disjunctive) ? cmp1->getOpCode().convertCmpToIfCmp() : TR::ILOpCode(cmp1->getOpCode().getOpCodeForReverseBranch()).convertCmpToIfCmp());
    ...
    TR::SymbolReference* temp1 = s->comp()->getSymRefTab()->createTemporary(s->comp()->getMethodSymbol(), cmp2->getFirstChild()->getDataType());
    prevTree = TR::TreeTop::create(s->comp(), prevTree, TR::Node::createStore(temp1, cmp2->getFirstChild()));
    TR::SymbolReference* temp2 = s->comp()->getSymRefTab()->createTemporary(s->comp()->getMethodSymbol(), cmp2->getSecondChild()->getDataType());
    ...
    TR::ILOpCodes newOp = ((isZero) ? TR::ILOpCode(cmp2->getOpCode().getOpCodeForReverseBranch()).convertCmpToIfCmp() : TR::ILOpCode(cmp2->getOpCodeValue()).convertCmpToIfCmp());
...
private:
    ...
    */
    ...
    bool isReducibleSpecific() const
    {
        ...
        if (lhsNode->getOpCode().isAnd() && lhsNode->getSecondChild()->getOpCode().isIntegralConst() && rhsNode->getOpCode().isIntegralConst()) {
...
TR::Node* indirectLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::loadaddr) {
        ...
        if (isSame && (node->getSymbol()->getSize() == firstChild->getSymbol()->getSize()) && (firstChild->getSymbolReference()->getSymbol()->isAutoOrParm() || localStatic) && node->getSymbolReference()->getOffset() == 0 && (node->getSymbol()->isVolatile() == firstChild->getSymbol()->isVolatile()) && performTransformation(s->comp(), "%sReplace indirect load %s [" POINTER_PRINTF_FORMAT "] with ", s->optDetailString(), node->getOpCode().getName(), node)) {
    ...
    if (s->comp()->cg()->getSupportsVectorRegisters()) {
        ...
        if ((node->getType().isIntegral() || node->getType().isDouble()) && ((addressNode->getOpCode().isArrayRef() && addressNode->getSecondChild()->getOpCode().isLoadConst() && addressNode->getFirstChild()->getOpCode().hasSymbolReference() && addressNode->getFirstChild()->getSymbol()->getType().isVector()) || (addressNode->getOpCode().hasSymbolReference() && addressNode->getSymbol()->getType().isVector())) && performTransformation(s->comp(), "%sReplace indirect load [" POINTER_PRINTF_FORMAT "] with getvelem", s->optDetailString(), node)) {
            int32_t offset = 0;
...
TR::Node* indirectStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (isDeletedLabelLoadaddr(secondChild) && performTransformation(s->comp(), "%sRemoving indirect store [" POINTER_PRINTF_FORMAT "] to deleted label [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node, secondChild->getSymbol())) {
    ...
    if (firstChild->getOpCodeValue() == TR::loadaddr) {
        ...
        if ((storeDataType == addrDataType && isSameType) && (firstChild->getSymbolReference()->getSymbol()->isAutoOrParm() || localStatic) && node->getSymbolReference()->getOffset() == 0 && (node->getSymbol()->isVolatile() == firstChild->getSymbol()->isVolatile()) && performTransformation(s->comp(), "%sReplace indirect store %s [" POINTER_PRINTF_FORMAT "] with ", s->optDetailString(), node->getOpCode().getName(), node)) {
    ...
    if ((node->getOpCodeValue() == TR::awrtbari) && 0 && !node->getSymbolReference()->getSymbol()->isArrayShadowSymbol()) {
        TR::Node* valueChild = node->getSecondChild();
        if ((valueChild->getOpCodeValue() == TR::New) || (valueChild->getOpCodeValue() == TR::newarray) || (valueChild->getOpCodeValue() == TR::anewarray) || (valueChild->getOpCodeValue() == TR::MergeNew) || (valueChild->getOpCodeValue() == TR::multianewarray)) {
            ...
            if (removeWrtBar && !s->comp()->getOptions()->realTimeGC() && performTransformation(s->comp(), "%sFolded indirect write barrier to iastore because GC could not have occurred enough times to require iwrtbar [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
    ...
    if (s->comp()->cg()->getSupportsVectorRegisters()) {
        ...
        if ((node->getType().isIntegral() || node->getType().isDouble()) && (!valueNode->getOpCode().isLoadConst() || s->getLastRun()) && ((addressNode->getOpCode().isArrayRef() && addressNode->getSecondChild()->getOpCode().isLoadConst() && addressNode->getFirstChild()->getOpCode().hasSymbolReference() && addressNode->getFirstChild()->getSymbol()->getType().isVector()) || (addressNode->getOpCode().hasSymbolReference() && addressNode->getSymbol()->getType().isVector())) && performTransformation(s->comp(), "%sReplace indirect store [" POINTER_PRINTF_FORMAT "] with vsetelem", s->optDetailString(), node)) {
            int32_t offset = 0;
            TR::SymbolReference* oldSymRef = node->getSymbolReference();
...
TR::Node* astoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // Check if we are storing an address of a deleted label
    if (isDeletedLabelLoadaddr(node->getFirstChild()) && performTransformation(s->comp(), "%sRemoving astore [" POINTER_PRINTF_FORMAT "] to deleted label [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node, node->getFirstChild()->getSymbol())) {
        s->removeNode(node, s->_curTree);
        return NULL;
    }
    return node;
...
TR::Node* directStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (child->getOpCode().isLoadVar() && child->getReferenceCount() == 1 && symRef == child->getSymbolReference() && performTransformation(s->comp(), "%sFolded direct store of load of same symbol on node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* treetopSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((s->comp()->useCompressedPointers() && child->getOpCode().isStore() && child->getType().isAddress() && child->getReferenceCount() > 1)) {
...
TR::Node* iaddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // reduce imul operations by factoring
    else if (firstChildOp == TR::imul && firstChild->getReferenceCount() == 1 && secondChildOp == TR::imul && secondChild->getReferenceCount() == 1) {
        TR::Node* llChild = firstChild->getFirstChild();
        TR::Node* lrChild = firstChild->getSecondChild();
        TR::Node* rlChild = secondChild->getFirstChild();
        TR::Node* rrChild = secondChild->getSecondChild();
    ...
    } else if (nodeOp != TR::aiadd && !((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() && (s->_curTree->getNode()->getNumChildren() > 0) && s->_curTree->getNode()->getFirstChild()->getOpCode().isMemToMemOp()) && (s->_curTree->getNode()->getFirstChild()->getLastChild() == node)) && (firstChildOp == TR::iadd || firstChildOp == TR::isub)) {
        TR::Node* lrChild = firstChild->getSecondChild();
        if (lrChild->getOpCodeValue() == TR::iconst) {
            if (secondChildOp == TR::iconst) {
                ...
                (firstChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sFound iadd of non-iconst with iadd or isub of x and const in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
                // move constants up the tree so they will tend to get merged together
                node->setChild(1, lrChild);
                firstChild->setChild(1, secondChild);
        ...
        TR::Node* lrChild = firstChild->getSecondChild();
        if ((lrChild->getOpCodeValue() == TR::iconst) && ((firstChild->getFutureUseCount() == 0) || (!s->comp()->cg()->areAssignableGPRsScarce() && (lrChild->getInt() == -1 * secondChild->getInt())))) {
            if (performTransformation(s->comp(), "%sFound aiadd of iconst with aiadd x and iconst in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
                int32_t value = secondChild->getInt();
                value += lrChild->getInt();
                if (secondChild->getReferenceCount() == 1) {
        ...
        if ((node_aiadd->getSecondChild()->getOpCodeValue() == TR::iconst) && (node->getSecondChild()->getReferenceCount() == 1) && // iadd iconst should be unique
            ...
            if (passesPrecisionTest && performTransformation(s->comp(), "%sPushing down iadd [%p] iconst [%p] into aiadd [%p] second child [%p], creating new iadd\n", s->optDetailString(), node, node->getSecondChild(), node_aiadd, node_aiadd->getSecondChild())) {
...
TR::Node* laddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst() && performTransformation(s->comp(), "%sSimplified ladd in node [" POINTER_PRINTF_FORMAT "] to lconst\n", s->optDetailString(), node)) {
        if (node->nodeRequiresConditionCodes()) {
    ...
    // reduce imul operations by factoring
    else if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1 && secondChildOp == TR::lmul && secondChild->getReferenceCount() == 1) {
        TR::Node* llChild = firstChild->getFirstChild();
        TR::Node* lrChild = firstChild->getSecondChild();
        TR::Node* rlChild = secondChild->getFirstChild();
        TR::Node* rrChild = secondChild->getSecondChild();
        ...
        if (secondChildOp == TR::lconst) {
            ...
            {
                if ((firstChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sFound ladd of non-lconst with ladd or lsub of x and lconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
                    node->setChild(1, lrChild);
                    firstChild->setChild(1, secondChild);
                    TR::Node::recreate(node, firstChildOp);
        ...
        TR::Node* lrChild = firstChild->getSecondChild();
        if ((lrChild->getOpCodeValue() == TR::lconst) && ((firstChild->getFutureUseCount() == 0) || (lrChild->getLongInt() == -1 * secondChild->getLongInt()))) {
        ...
        } else if ((firstChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sFound aladd of non-lconst with aladd x and lconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* isubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() != TR::aiadd && !((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() && (s->_curTree->getNode()->getNumChildren() > 0) && s->_curTree->getNode()->getFirstChild()->getOpCode().isMemToMemOp()) && (s->_curTree->getNode()->getFirstChild()->getLastChild() == node)) && ((node->getOpCodeValue() == TR::isub) && ((secondChildOp == TR::iadd) || (secondChildOp == TR::isub)))) {
        TR::Node* rlChild = secondChild->getFirstChild();
        TR::Node* rrChild = secondChild->getSecondChild();

        if ((rrChild->getOpCodeValue() == TR::iconst) && (rlChild == firstChild) && node->cannotOverflow() && secondChild->cannotOverflow()) {
            if (performTransformation(s->comp(), "%sFolded isub with children related through iconst in node [%s] to iconst \n", s->optDetailString(), node->getName(s->getDebug()))) {
                node->setChild(0, 0);
                node->setChild(1, 0);
    ...
    // reduce imul operations by factoring
        ...
        // if (performTransformation(s->comp(), "%sFactored isub with distributed imul in node [%s]\n", s->optDetailString(), node->getName(s->getDebug())))
        {
            ...
            if (!factorChild || !performTransformation(s->comp(), "%sFactored isub with distributed imul in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
                secondChild->setChild(0, rlChild);
                secondChild->setChild(1, rrChild);
            } else {
                TR::Node::recreate(node, TR::imul);
    ...
    //   a lot slower for register usage problems.  when we have a solution for
    //   that problem, the test should be moved back down there.
    else if (!((s->comp()->cg()->supportsLengthMinusOneForMemoryOpts() && (s->_curTree->getNode()->getNumChildren() > 0) && s->_curTree->getNode()->getFirstChild()->getOpCode().isMemToMemOp()) && (s->_curTree->getNode()->getFirstChild()->getLastChild() == node)) && (firstChildOp == TR::iadd || firstChildOp == TR::isub)) {
        TR::Node* lrChild = firstChild->getSecondChild();
        if (lrChild->getOpCodeValue() == TR::iconst) {
            if (secondChildOp == TR::iconst) {
    ...
    } else if (node->getFirstChild()->getOpCode().isLoadConst() && node->getFirstChild()->get64bitIntegralValue() == 0 && performTransformation(s->comp(), "%sReduce isub from 0 [%s] to ineg \n", s->optDetailString(), node->getName(s->getDebug()))) {
        TR::Node::recreate(node, TR::ILOpCode::negateOpCode(node->getDataType()));
        node->setVisitCount(0);
        node->getFirstChild()->recursivelyDecReferenceCount();
        node->setChild(0, node->getSecondChild());
     ...
     */
        ...
        if (firstChild->getChild(0)->getOpCode().isArrayRef() && firstChild->getChild(0)->getChild(0)->getOpCode().isLoadAddr()) {
        ...
        if (secondChild->getChild(0)->getOpCode().isArrayRef() && secondChild->getChild(0)->getChild(0)->getOpCode().isLoadAddr()) {
        ...
        if (LBase && RBase && LBase == RBase && performTransformation(s->comp(), "%sRemove loadaddr in address computation in [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
            if (!exprA)
...
TR::Node* lsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() != TR::aladd && ((node->getOpCodeValue() == TR::lsub) && ((secondChildOp == TR::ladd) || (secondChildOp == TR::lsub)))) {
        TR::Node* rlChild = secondChild->getFirstChild();
        TR::Node* rrChild = secondChild->getSecondChild();

        if ((rrChild->getOpCodeValue() == TR::lconst) && (rlChild == firstChild) && node->cannotOverflow() && secondChild->cannotOverflow()) {
    ...
    // turn bitwise complement + 1 into a 2's complement negate
    else if (isBitwiseLongComplement(firstChild) && secondChildOp == TR::lconst && secondChild->getLongInt() == -1) // -1LL works for IBM but not MS
    {
    ...
    // reduce imul operations by factoring
        ...
        if (secondChildOp == TR::lconst) {
            ...
            {
                if ((firstChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sFound lsub of non-lconst with ladd or lsub of x and lconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
                    node->setChild(1, lrChild);
                    firstChild->setChild(1, secondChild);
                    TR::Node::recreate(node, firstChildOp);
    ...
    else if (firstChildOp == TR::lconst && secondChildOp == TR::land && secondChild->getSecondChild()->getOpCodeValue() == TR::lconst) {
    ...
    // 184251
    else if (firstChildOp == TR::lmul && firstChild->getSecondChild()->getOpCodeValue() == TR::lconst && firstChild->getFirstChild()->getOpCodeValue() == TR::i2l && secondChildOp == TR::lconst) {
        ...
        if (shftAmnt > 0 && (firstChild->getOpCodeValue() == TR::iadd || firstChild->getOpCodeValue() == TR::isub) && firstChild->isNonNegative() && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst) {
            int32_t iValue = firstChild->getSecondChild()->getInt();
            int64_t lValue = (int64_t)iValue;

            // Make sure we don't overflow
            if ((iValue << shftAmnt) == (int32_t)(lValue << shftAmnt) && performTransformation(s->comp(), "%sFound lsub/lmul of +ve powOf2 lconst with i2l of iadd or isub of x and iconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
    ...
    */
    ...
    if (!node->nodeRequiresConditionCodes() && secondChild->getOpCodeValue() == TR::land && secondChild->getFirstChild() == firstChild && secondChild->getSecondChild()->getOpCode().isLoadConst()) {
...
TR::Node* imulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChildOp == TR::iconst && secondChild->getInt() < 0 && secondChild->getInt() != TR::getMinSigned<TR::Int32>() && performTransformation(s->comp(), "%sFound a*(-c) = -(a*c) in imul [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
        TR::Node::recreate(node, TR::ineg);
        node->setNumChildren(1);
        if (secondChild->getReferenceCount() == 1) {
            secondChild->setInt(-secondChild->getInt());
    ...
    // which differ only by constants, ex. a[i] + a[i+2]
        ...
        // (a+c1)*c2 = a*c2 + c1*c2
        if (lrChild->getOpCodeValue() == TR::iconst && performTransformation(s->comp(), "%sFound (a+c1)*c2 = a*c2 + c1*c2 in imul [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
    ...
    } else if (secondChildOp == TR::iconst && !s->getLastRun() && secondChild->getInt() != 0 && !isNonNegativePowerOf2(secondChild->getInt()) && secondChild->getInt() != TR::getMinSigned<TR::Int32>()) {
   ...
   //    e   c1
    ...
    if (s->getLastRun() && node->isNonNegative() && node->getOpCode().isMul() && node->getFirstChild()->getOpCode().isDiv() && node->getFirstChild()->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getOpCode().isLoadConst() && (size = node->getSecondChild()->get64bitIntegralValue()) == node->getFirstChild()->getSecondChild()->get64bitIntegralValue() && ((size & (size - 1)) == 0) && // size is  power of 2
        size > 0) {
   ...
   //      *  c2    =>    t   *
        //     / \                / \
   //    t   c1             c2 c1
        if (region && node->getOpCodeValue() == TR::imul && isExprInvariant(region, node->getSecondChild()) && node->getFirstChild()->getOpCodeValue() == TR::imul && !isExprInvariant(region, node->getFirstChild()->getFirstChild()) && isExprInvariant(region, node->getFirstChild()->getSecondChild()) && node->getFirstChild()->getReferenceCount() == 1) {
   ...
   //      +  c2    =>     *  c1*c2
        //     / \             / \
   //    e   c1          e   c2
        if (region && node->getOpCodeValue() == TR::imul && isExprInvariant(region, node->getSecondChild()) && node->getFirstChild()->getOpCodeValue() == TR::iadd && !isExprInvariant(region, node->getFirstChild()->getFirstChild()) && isExprInvariant(region, node->getFirstChild()->getSecondChild()) && node->getFirstChild()->getReferenceCount() == 1) {
   ...
   //      +   c    =>     *     *
        //     / \             / \   / \
   //    e1  e2          e1  c e2  c
        else if (region && node->getOpCodeValue() == TR::imul && isExprInvariant(region, node->getSecondChild()) && node->getFirstChild()->getOpCodeValue() == TR::iadd) {
   ...
   //    e1  e2          e1  c e2  c
            ...
            isExprInvariant(region, node->getSecondChild()) && !isExprInvariant(region, node->getFirstChild()) && node->getFirstChild()->getOpCodeValue() == TR::isub) {
...
TR::Node* lmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1) {
        ...
        firstChildOp = firstChild->getOpCodeValue();
        TR::Node* lrChild = firstChild->getSecondChild();
        if (lrChild->getOpCodeValue() == TR::lconst && performTransformation(s->comp(), "%sDistributed lmul with lconst over lsub or ladd with lconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
    ...
    // the simplification is then invalid
        ...
        TR::Node* i2lNode = firstChild;
        if (((i2lNode->getFirstChild()->getOpCodeValue() == TR::isub) || (i2lNode->getFirstChild()->getOpCodeValue() == TR::iadd)) && i2lNode->getFirstChild()->cannotOverflow()) {
            firstChild = firstChild->getFirstChild();
            firstChildOp = firstChild->getOpCodeValue();
            TR::Node* lrChild = firstChild->getSecondChild();
            if (lrChild->getOpCodeValue() == TR::iconst && performTransformation(s->comp(), "%sDistributed lmul with lconst over isub or iadd of with iconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* dmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (s->comp()->getOption(TR_IgnoreIEEERestrictions) && !node->isFPStrictCompliant() && !s->comp()->cg()->supportsNegativeFusedMultiplyAdd()) // we can create x*1.0 when negFma is available.  Don't undo it
    {
...
TR::Node* idivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (divisor != 0) {
            ...
            // Design 1790
            else if ((!disableILDivPwr2Opt) && ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(divisor)) > 0) && (secondChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sPwr of 2 idiv opt node %p\n", s->optDetailString(), node)) {
                secondChild->decReferenceCount();
                TR::Node* newNode1;
                if (shftAmnt - 1 != 0) {
            ...
            } else if (s->cg()->getSupportsLoweringConstIDiv() && !isPowerOf2(divisor) && performTransformation(s->comp(), "%sMagic number idiv opt in node %p\n", s->optDetailString(), node)) {
...
TR::Node* ldivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (divisor != 0) {
            if (firstChild->getOpCode().isLoadConst()) {
                ...
                // Design 1790
                else if ((!disableILDivPwr2Opt) && ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(divisor)) > 0) && (secondChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sPwr of 2 ldiv opt node %p\n", s->optDetailString(), node)) {
                    secondChild->decReferenceCount();
                    TR::Node* newNode1;
                    if (shftAmnt - 1 != 0) {
    ...
    if (node->getOpCodeValue() == TR::ldiv) {
        firstChild = node->getFirstChild();
        secondChild = node->getSecondChild();
        if ((firstChild->getOpCodeValue() == TR::i2l) && (secondChild->getOpCodeValue() == TR::i2l) && performTransformation(s->comp(), "%sReduced ldiv [%p] of two i2l children to i2l of idiv \n", s->optDetailString(), node)) {
            TR::TreeTop* curTree = s->_curTree;
            TR::Node* divCheckParent = NULL;
            if ((curTree->getNode()->getOpCodeValue() == TR::DIVCHK) && (curTree->getNode()->getFirstChild() == node))
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 10 && !skipRemLoweringForPositivePowersOfTen(s) && node->getFirstChild()->getOpCode().isLoadVar() && //transformToLongDiv creates a tonne of nodes.  Not good to do this recursively if you have chained divides
...
TR::Node* fdivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getFloatBits() != FLOAT_POS_ZERO && secondChild->getFloatBits() != FLOAT_NEG_ZERO) {
        if (firstChild->getOpCode().isLoadConst()) {
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (divisor != 0) {
            if (divisor == 1 || (!isUnsigned && (divisor == -1))) {
                ...
                int32_t result = (node->getOpCodeValue() == TR::iurem) ? (uint32_t)dividend % (uint32_t)divisor : dividend % divisor;

                foldIntConstant(node, result, s, false /* !anchorChildren */);
                return node;
            } else if ((!disableILRemPwr2Opt) && (!isUnsigned || isNonNegativePowerOf2(divisor)) && ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(divisor)) > 0) && (secondChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sPwr of 2 irem opt node %p\n", s->optDetailString(), node)) {
            ...
            } else if (node->getOpCodeValue() == TR::irem && s->cg()->getSupportsLoweringConstIDiv() && !isPowerOf2(divisor) && !skipRemLowering(divisor, s) && performTransformation(s->comp(), "%sMagic number irem opt in node %p\n", s->optDetailString(), node)) {
...
TR::Node* lremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (divisor != 0) {
            ...
            // Design 1792
            else if ((!disableILRemPwr2Opt) && ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(divisor)) > 0) && (secondChild->getReferenceCount() == 1) && performTransformation(s->comp(), "%sPwr of 2 lrem opt node %p\n", s->optDetailString(), node)) {
                secondChild->decReferenceCount();
                TR::Node* newNode1;
                if (shftAmnt - 1 != 0) {
                    newNode1 = TR::Node::create(node, TR::lshr, 2);
    ...
    if (node->getOpCodeValue() == TR::lrem) {
        ...
        if ((firstChild->getOpCodeValue() == TR::i2l) && (secondChild->getOpCodeValue() == TR::i2l) && performTransformation(s->comp(), "%sReduced lrem [%p] of two i2l children to i2l of irem \n", s->optDetailString(), node)) {
            TR::TreeTop* curTree = s->_curTree;
            TR::Node* divCheckParent = NULL;
            if ((curTree->getNode()->getOpCodeValue() == TR::DIVCHK) && (curTree->getNode()->getFirstChild() == node))
                divCheckParent = curTree->getNode();
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 10 && !skipRemLoweringForPositivePowersOfTen(s) && node->getFirstChild()->getOpCode().isLoadVar() && performTransformation(s->comp(), "%sReduced lrem by 10 [%p] to sequence of bitwise operations\n", s->optDetailString(), node)) {
            TR::TreeTop* curTree = s->_curTree;
            TR::Node* divCheckParent = NULL;
            if ((curTree->getNode()->getOpCodeValue() == TR::DIVCHK) && (curTree->getNode()->getFirstChild() == node))
                divCheckParent = curTree->getNode();
...
TR::Node* inegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    } else if (opCode == TR::l2i && firstChild->getFirstChild()->getOpCodeValue() == TR::lushr && firstChild->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        TR::Node* firstFirst = firstChild->getFirstChild();
        uint32_t shiftBy = firstFirst->getSecondChild()->getInt();
        if (shiftBy == 63 && performTransformation(s->comp(), "%sReplaced ineg of lushr by 63 with lshr node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* fnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // don't just do -A op B -> -(A op B) as not always a win, so catch opportunity here
    else if ((firstChild->getOpCodeValue() == TR::fmul || firstChild->getOpCodeValue() == TR::fdiv || firstChild->getOpCodeValue() == TR::frem)) {
        ...
        if (firstOpChild->getOpCodeValue() == TR::fneg) {
            ...
            childNo = 0;
        } else if (secondOpChild->getOpCodeValue() == TR::fneg && firstChild->getOpCodeValue() != TR::frem) // not valid for rem
        {
        ...
        if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && !(firstChild->getFirstChild()->isFPStrictCompliant() || firstChild->getSecondChild()->isFPStrictCompliant()) && performTransformation(s->comp(), "%sTransforming [" POINTER_PRINTF_FORMAT "] -(-A +/- B) -> -((A*1)+/-B)\n", s->optDetailString(), node)) {
        ...
        } else if (firstChild->getOpCode().isMul() && performTransformation(s->comp(), "%sTransforming [" POINTER_PRINTF_FORMAT "] -(A*B) -> -((A*B)-0)\n", s->optDetailString(), node)) {
            TR::Node* newConst = TR::Node::create(firstChild, TR::fconst, 0);
            newConst->setFloat(0);
...
TR::Node* ilfdabsSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (child->isNonNegative() && performFlagBasedTransformation && performTransformation(s->comp(), "%sSimplify abs of non-negative child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
        return s->replaceNodeWithChild(node, child, s->_curTree, block);
    } else if (child->isNegative() && performFlagBasedTransformation && performTransformation(s->comp(), "%sSimplify abs of non-positive child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
        TR::Node::recreate(node, neg);
        return s->simplify(node, block);
    } else if ((child->getOpCodeValue() == abs || child->getOpCodeValue() == neg) && performTransformation(s->comp(), "%sSimplify abs of abs/neg child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* ishlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && performTransformation(s->comp(), "%sChanged ishl by const into imul by const in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node* lushrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    //
    if (secondChild->getOpCode().isLoadConst() && firstChild->getOpCodeValue() == TR::lshl && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if (right >= left && (right == 56 || right == 48 || right == 32) && performTransformation(s->comp(), "%sshift left followed by shift right %p of %d can become a shift + conversion\n", s->optDetailString(), node, right)) {
    ...
    if (secondChild->getOpCodeValue() == TR::iconst && firstChild->getOpCodeValue() == TR::lmul) {
        ...
        if (constGrandChild->getOpCodeValue() == TR::lconst && constGrandChild->getLongInt() == (CONSTANT64(1) << rightShiftValue)) {
            TR::Node* subTree = firstChild->getFirstChild();
            TR::ILOpCodes opCode = subTree->getOpCodeValue();
            bool foundZeroExtension = false;
            if (subTree->getReferenceCount() == 1) {
...
TR::Node* iandSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getReferenceCount() == 1) {
        if (isBitwiseIntComplement(firstChild) && secondChild->getReferenceCount() == 1 && isBitwiseIntComplement(secondChild)) {
...
TR::Node* landSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // look for demorgan's law.  if found can remove one bitwise complement operation
    if (firstChild->getReferenceCount() == 1) {
        if (isBitwiseLongComplement(firstChild) && secondChild->getReferenceCount() == 1 && isBitwiseLongComplement(secondChild)) {
    ...
    if (node->getOpCodeValue() == TR::land && secondChild->getOpCodeValue() == TR::lconst && firstChild->isHighWordZero()) {
        ...
        if (secondChild->isHighWordZero() && ((int32_t)secondChild->getLongIntLow()) > 0) {
            ...
            if (firstChildOp == TR::iu2l && performTransformation(s->comp(), "%sReduced land with lconst and iu2l child in node [" POINTER_PRINTF_FORMAT "] to iand\n", s->optDetailString(), node)) {
        ...
        //
            ...
            if (lrChild->getOpCodeValue() == TR::iconst) {
                if (secondChildOp == TR::iconst) {
                    ...
                    if ((((~and_const) | or_const) == or_const) && performTransformation(s->comp(), "%sFound ior of iconst with iand of x and iconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
                        node->setAndIncChild(0, firstChild->getFirstChild());
                        firstChild->recursivelyDecReferenceCount();
                        node->setVisitCount(0);
                        s->_alteredBlock = true;
...
TR::Node* lorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::lor && secondChild->getOpCodeValue() == TR::lconst && firstChild->isHighWordZero()) {
        ...
        if (secondChild->isHighWordZero() && ((int32_t)secondChild->getLongIntLow()) > 0) {
            ...
            if (firstChildOp == TR::iu2l && performTransformation(s->comp(), "%sReduced lor with lconst and iu2l child in node [" POINTER_PRINTF_FORMAT "] to ior\n", s->optDetailString(), node)) {
                TR::Node* constChild = NULL;
...
TR::Node* borSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isAnd() && firstChild->getReferenceCount() == 1 && secondChild->getOpCode().isLoadConst() && firstChild->getSecondChild()->getOpCode().isLoadConst() && (((secondChild->getByte() | firstChild->getSecondChild()->getByte()) & 0xFF) == 0xFF) && performTransformation(s->comp(), "%sReplacing bor [" POINTER_PRINTF_FORMAT "] child with band child [" POINTER_PRINTF_FORMAT "] \n", s->optDetailString(), node, firstChild->getFirstChild())) {
...
TR::Node* lxorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::lxor && secondChild->getOpCodeValue() == TR::lconst && firstChild->isHighWordZero()) {
        ...
        if (secondChild->isHighWordZero() && ((int32_t)secondChild->getLongIntLow()) > 0) {
            ...
            if (firstChildOp == TR::iu2l && performTransformation(s->comp(), "%sReduced lxor with lconst and iu2l child in node [" POINTER_PRINTF_FORMAT "] to ixor\n", s->optDetailString(), node)) {
                TR::Node* constChild = NULL;
...
inline TR::Node* sqrtSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (child->getOpCode().isLoadConst() && performTransformation(s->comp(), "%sSimplify sqrt of const child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* i2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::l2i && firstChild->getFirstChild()->getOpCodeValue() == TR::lshr && firstChild->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        TR::Node* firstFirst = firstChild->getFirstChild();
        uint32_t shiftBy = firstFirst->getSecondChild()->getInt();
        if (shiftBy >= 57 && performTransformation(s->comp(), "%sRemove i2l/l2i from lshr node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
            TR::Node::recreate(node, TR::lshr);
            node->setNumChildren(2);
...
TR::Node* i2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        foldByteConstant(node,
            node->getOpCode().isUnsigned() ? (firstChild->getInt() & 0xff) : (firstChild->getInt() << 24) >> 24,
            s, false /* !anchorChildren*/);
        return node;
    }
...
TR::Node* i2aSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    } else if (firstChild->getOpCode().isConversion()) // remove redundant conversions
    {
        ...
        if (firstChild->getDataType() == TR::Address && !firstChild->getOpCode().isLoadAddr() && TR::Compiler->target.is32Bit())
            return s->replaceNode(node, firstChild, s->_curTree);
        else
    ...
    //   iconst -X
    //
    if ((firstChild->getOpCodeValue() == TR::isub || firstChild->getOpCodeValue() == TR::iadd) && (s->comp()->cg()->supports32bitAiadd() || node->isNonNegative()) && // todo: do we know NonNegativity in simplifier?
        firstChild->getFirstChild() && firstChild->getFirstChild()->getOpCodeValue() == TR::a2i && firstChild->getSecondChild() && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && ((firstChild->getOpCodeValue() == TR::isub && firstChild->getSecondChild()->getInt() <= 0) || // todo: is positive offset only a correct soln?
...
TR::Node* l2aSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && ((int32_t)(firstChild->getLongIntHigh() & 0xffffffff) == (int32_t)0) && ((int64_t)firstChild->getLongInt() >= (int64_t)0)) {
        uint8_t addrPr = (TR::Compiler->target.is64Bit() ? 8 : 4);
    ...
    } else if (firstChild->getOpCode().isConversion()) // remove redundant conversions
    {
        ...
        if (firstChild->getDataType() == TR::Address && !firstChild->getOpCode().isLoadAddr() && TR::Compiler->target.is64Bit())
            return s->replaceNode(node, firstChild, s->_curTree);
        else
            firstChild = node->getFirstChild(); // reset firstChild
    }
    ...
    //   lconst -X
    // -----------------------
    if ((firstChild->getOpCodeValue() == TR::lsub || firstChild->getOpCodeValue() == TR::ladd) && firstChild->getFirstChild() && firstChild->getFirstChild()->getOpCode().isConversion() && firstChild->getFirstChild()->getFirstChild() && firstChild->getFirstChild()->getFirstChild()->getType().isAddress() && firstChild->getSecondChild() && firstChild->getSecondChild()->getOpCodeValue() == TR::lconst && ((firstChild->getOpCodeValue() == TR::lsub && firstChild->getSecondChild()->getLongInt() < 0) || // todo: is positive offset only a correct soln?
...
TR::Node* bu2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::i2b && firstChild->getFirstChild()->getOpCodeValue() == TR::iand && firstChild->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        uint64_t val = firstChild->getFirstChild()->getSecondChild()->get64bitIntegralValueAsUnsigned();
        if (((val & 0xFFUL) == 0UL) && performTransformation(s->comp(), "%sReplacing bu2i [" POINTER_PRINTF_FORMAT "] with i2b child [" POINTER_PRINTF_FORMAT "] of iand [" POINTER_PRINTF_FORMAT "] with mask 0, with iconst 0\n", s->optDetailString(), node, firstChild, firstChild->getFirstChild(), val, TR::getMaxUnsigned<TR::Int8>())) {
            ...
            node->setInt(0);
            return node;
        } else if (val <= (uint64_t)TR::getMaxUnsigned<TR::Int8>() && performTransformation(s->comp(), "%sRemove bu2i [" POINTER_PRINTF_FORMAT "] with i2b child [" POINTER_PRINTF_FORMAT "] of iand [" POINTER_PRINTF_FORMAT "] with mask %d <= %d\n", s->optDetailString(), node, firstChild, firstChild->getFirstChild(), val, TR::getMaxUnsigned<TR::Int8>())) {
    ...
    } else if (firstChild->getOpCodeValue() == TR::i2b && (firstChild->getFirstChild()->getOpCodeValue() == TR::butest || firstChild->getFirstChild()->getOpCodeValue() == TR::arraycmp || firstChild->getFirstChild()->getOpCodeValue() == TR::trt || firstChild->getFirstChild()->getOpCodeValue() == TR::trtSimple || firstChild->getFirstChild()->getOpCodeValue() == TR::icmpeq || firstChild->getFirstChild()->getOpCodeValue() == TR::lcmpeq || firstChild->getFirstChild()->getOpCodeValue() == TR::icmpne || firstChild->getFirstChild()->getOpCodeValue() == TR::lcmpne) && performTransformation(s->comp(), "%sRemove bu2i [" POINTER_PRINTF_FORMAT "] with i2b child [" POINTER_PRINTF_FORMAT "] with %s grandchild [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node, firstChild, firstChild->getFirstChild()->getOpCode().getName(), firstChild->getFirstChild())) {
        TR::Node* grandChild = firstChild->getFirstChild();
        grandChild->incReferenceCount();
        bool anchorChildren = false; // no loads are being removed (just bu2i/i2b) so there is nothing that needs anchoring
        s->prepareToStopUsingNode(node, s->_curTree, anchorChildren);
    ...
    if (firstChild->getOpCodeValue() == TR::l2b && firstChild->getFirstChild()->getOpCodeValue() == TR::lushr && firstChild->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        uint32_t shiftBy = firstChild->getFirstChild()->getSecondChild()->getInt();
        if (shiftBy >= 56 && performTransformation(s->comp(), "%sReplace bu2i/l2b of lushr with l2i node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* bu2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::i2b && firstChild->getFirstChild()->getOpCodeValue() == TR::iand && firstChild->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        uint64_t val = firstChild->getFirstChild()->getSecondChild()->get64bitIntegralValueAsUnsigned();
        if (((val & 0xFFUL) == 0UL) && performTransformation(s->comp(), "%sReplacing bu2l [" POINTER_PRINTF_FORMAT "] with i2b child [" POINTER_PRINTF_FORMAT "] of iand [" POINTER_PRINTF_FORMAT "] with mask 0, with iconst 0\n", s->optDetailString(), node, firstChild, firstChild->getFirstChild(), val, TR::getMaxUnsigned<TR::Int8>())) {
            // if we're going to zero extend a zero, then we can just replace this entire expression with a 0
            s->anchorNode(firstChild->getFirstChild()->getFirstChild(), s->_curTree); //only really need to anchor this child, everything else is guaranteed to not be a load
            s->prepareToReplaceNode(node, TR::lconst);
            node->setLongInt(0);
            return node;
        } else if (val <= (uint64_t)TR::getMaxUnsigned<TR::Int8>() && performTransformation(s->comp(), "%sReplace bu2l [" POINTER_PRINTF_FORMAT "] with i2b child [" POINTER_PRINTF_FORMAT "] of iand [" POINTER_PRINTF_FORMAT "] with mask %d <= %d with i2l\n", s->optDetailString(), node, firstChild, firstChild->getFirstChild(), val, TR::getMaxUnsigned<TR::Int8>())) {
    ...
    } else if (firstChild->getOpCodeValue() == TR::i2b && (firstChild->getFirstChild()->getOpCodeValue() == TR::icmpeq || firstChild->getFirstChild()->getOpCodeValue() == TR::icmpne || firstChild->getFirstChild()->getOpCodeValue() == TR::lcmpeq || firstChild->getFirstChild()->getOpCodeValue() == TR::lcmpne) && performTransformation(s->comp(), "%sRemove bu2l [" POINTER_PRINTF_FORMAT "] with i2b child [" POINTER_PRINTF_FORMAT "] of compare [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node, firstChild, firstChild->getFirstChild())) {
        TR::Node* grandChild = firstChild->getFirstChild();
        node->recreate(node, TR::i2l);
        grandChild->incReferenceCount();
        firstChild->recursivelyDecReferenceCount();
        node->setChild(0, grandChild);
    } else if (firstChild->getOpCodeValue() == TR::l2b && firstChild->getFirstChild()->getOpCodeValue() == TR::lushr && firstChild->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        uint32_t shiftBy = firstChild->getFirstChild()->getSecondChild()->getInt();
        if (shiftBy >= 56 && performTransformation(s->comp(), "%sReplace bu2l/l2b of lushr with lushr node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* removeArithmeticsUnderIntegralCompare(TR::Node* node,
    TR::Simplifier* s)
{
    if (s->comp()->getOption(TR_DisableIntegerCompareSimplification) || !(node->getOpCode().isBooleanCompare() // The node must be an integer comparison node.
...
TR::Node* ificmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isRightShift() && //First child is a right shift
        ...
        firstChild->getSecondChild()->getOpCode().isLoadConst() && firstChild->getSecondChild()->getInt() <= 31 && firstChild->getSecondChild()->getInt() >= 0 && //Shift value is a positive constant of reasonable size

        (firstChild->getOpCodeValue() == TR::iushr || firstChild->getFirstChild()->isNonNegative()) && //Either a logical shift or a positive first child to guaruntee zero-extend
    ...
    if (firstChild->getOpCode().isBooleanCompare() && (secondChild->getOpCode().isLoadConst()) && ((secondChild->getInt() == 0) || (secondChild->getInt() == 1)) && (firstChild->getOpCode().convertCmpToIfCmp() != TR::BadILOp) && (s->comp()->cg()->getSupportsJavaFloatSemantics() || !(firstChild->getNumChildren() > 1 && firstChild->getFirstChild()->getOpCode().isFloatingPoint())) && performTransformation(s->comp(), "%sChanging if opcode %p because first child %p is a comparison opcode\n", s->optDetailString(), node, firstChild)) {
    ...
    if ((firstChild->getOpCodeValue() == TR::lcmp) && ((secondChild->getOpCode().isLoadConst()) && secondChild->getInt() == 0) && performTransformation(s->comp(), "%sChanging if opcode %p because first child %p is an lcmp\n", s->optDetailString(), node, firstChild)) {
    ...
    if ((firstChild->getOpCodeValue() == TR::lcmpeq) && ((secondChild->getOpCode().isLoadConst()) && secondChild->getInt() == 0) && performTransformation(s->comp(), "%sChanging if opcode %p because first child %p is an lcmpeq\n", s->optDetailString(), node, firstChild)) {
...
TR::Node* ificmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isRightShift() && //First child is a right shift
        ...
        (firstChild->getOpCodeValue() == TR::iushr || firstChild->getFirstChild()->isNonNegative()) && //Either a logical shift or a positive first child to guarantee zero-extend
...
TR::Node* ificmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::ificmplt) {
        ...
        if (firstChild->getOpCode().isLoadConst() && conditionalBranchFold((originalFirstVal < originalSecondVal), node, firstChild, secondChild, block, s))
            return node;
...
TR::Node* normalizeCmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCode().isBranch()) {
        ...
        if (firstChild->getOpCode().isDouble()
            ...
            uint16_t cValue;
            int8_t bValue;
            if (firstChild->getOpCodeValue() == TR::f2d && doubleConstIsRepresentableExactlyAsFloat(dValue, fValue) && performTransformation(s->comp(), "%sDemoted double compare of TR::f2d to dconst to float compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::i2d && doubleConstIsRepresentableExactlyAsInt(dValue, iValue) && performTransformation(s->comp(), "%sDemoted double compare of TR::i2d to dconst to int compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (longValueInDoubleRange(node, dValue) && firstChild->getOpCodeValue() == TR::l2d && doubleConstIsRepresentableExactlyAsLong(dValue, lValue) && performTransformation(s->comp(), "%sDemoted double compare of TR::l2d to dconst to long compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::s2d && doubleConstIsRepresentableExactlyAsShort(dValue, sValue) && performTransformation(s->comp(), "%sDemoted double compare of TR::s2d to dconst to short compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::su2d && doubleConstIsRepresentableExactlyAsChar(dValue, cValue) && performTransformation(s->comp(), "%sDemoted double compare of TR::su2d to dconst to char compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::b2d && doubleConstIsRepresentableExactlyAsByte(dValue, bValue) && performTransformation(s->comp(), "%sDemoted double compare of TR::b2d to dconst to byte compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            if (intValueInFloatRange(node, fValue) && firstChild->getOpCodeValue() == TR::i2f && floatConstIsRepresentableExactlyAsInt(fValue, iValue) && performTransformation(s->comp(), "%sDemoted float compare of TR::i2f to fconst to int compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (longValueInFloatRange(node, fValue) && firstChild->getOpCodeValue() == TR::l2f && floatConstIsRepresentableExactlyAsLong(fValue, lValue) && performTransformation(s->comp(), "%sDemoted float compare of TR::l2f to fconst to long compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::s2f && floatConstIsRepresentableExactlyAsShort(fValue, sValue) && performTransformation(s->comp(), "%sDemoted float compare of TR::s2f to fconst to short compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::su2f && floatConstIsRepresentableExactlyAsChar(fValue, cValue) && performTransformation(s->comp(), "%sDemoted float compare of TR::su2f to fconst to char compare at node [%p]\n", s->optDetailString(), node)) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::b2f && floatConstIsRepresentableExactlyAsByte(fValue, bValue) && performTransformation(s->comp(), "%sDemoted float compare of TR::b2f to fconst to byte compare at node [%p]\n", s->optDetailString(), node)) {
...
TR::Node* ifacmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (!(firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbol()->isWeakSymbol()) && conditionalZeroComparisonBranchFold(node, firstChild, secondChild, block, s))
        return node;
...
TR::Node* ifCmpWithEqualitySimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (!disableFoldIfSet && node->getOpCodeValue() == TR::ifbcmpeq) {
        if (secondChild->getOpCodeValue() == TR::bconst && secondChild->getByte() == 0 && firstChild->getOpCode().isBooleanCompare() == true && firstChild->getOpCode().isBranch() == false && firstChild->getReferenceCount() == 1) {
            TR::ILOpCodes op = firstChild->getOpCode().convertCmpToIfCmp();
            if (op != TR::BadILOp && performTransformation(s->comp(), "%sFolding ifbcmpeq of bconst 0 to boolean compare at node [" POINTER_PRINTF_FORMAT "] to equivalent if?cmp??\n", s->optDetailString(), node)) {
                TR::Node::recreate(node, op);
                TR::Node::recreate(node, node->getOpCode().getOpCodeForReverseBranch());
                secondChild->recursivelyDecReferenceCount();
                node->setAndIncChild(0, firstChild->getFirstChild());
...
TR::Node* icmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((firstChild->getOpCode().isBooleanCompare() && firstChild->getOpCode().isInteger()) && ((secondChild->getOpCode().isLoadConst()) && secondChild->getInt() == 0) && performTransformation(s->comp(), "%sChanging icmpeq opcode %p because first child %p is an int compare\n", s->optDetailString(), node, firstChild)) {
    ...
    if (node->getOpCode().isSignum() && secondChild->getOpCode().isLoadConst()) {
        ...
        if (newOpCode != TR::BadILOp && performTransformation(s->comp(), "%sChanging icmpeq opcode %p because first child %p is an %s opcode\n", s->optDetailString(), node, firstChild, firstChild->getOpCode().getName())) {
    ...
    if (firstChild->getOpCodeValue() == TR::iand && secondChild->getOpCode().isLoadConst() && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        uint32_t val = secondChild->getInt();
        uint32_t mask = firstChild->getSecondChild()->getInt();
        if ((val & (val - 1)) == 0 && val == mask && performTransformation(s->comp(), "%s Changing icmpeq (x&2**c) to 2**c node [" POINTER_PRINTF_FORMAT "] to iand\n", s->optDetailString(), node)) {
...
TR::Node* lcmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::land && firstChild->getSecondChild()->getOpCodeValue() == TR::lconst && secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (firstChild->getOpCodeValue() == TR::land && secondChild->getOpCode().isLoadConst() && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
        uint64_t val = secondChild->getLongInt();
        uint64_t mask = firstChild->getSecondChild()->getLongInt();
        if ((val & (val - 1)) == 0 && val == mask && performTransformation(s->comp(), "%s Changing lcmpeq (x&2**c) to 2**c node [" POINTER_PRINTF_FORMAT "] to land\n", s->optDetailString(), node)) {
            uint32_t shiftBy = trailingZeroes(mask);
            TR::Node* byNode = TR::Node::create(node, TR::iconst, 0);
            byNode->setInt(shiftBy);
...
TR::Node* lcmpneSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0 && firstChild->getOpCodeValue() == TR::land) {
        TR::Node* firstFirst = firstChild->getFirstChild();
        TR::Node* firstSecond = firstChild->getSecondChild();
        if (firstSecond->getOpCodeValue() == TR::lshl && firstSecond->getFirstChild()->getOpCode().isLoadConst() && firstSecond->getFirstChild()->getLongInt() == 1 && performTransformation(s->comp(), "%slcmpne of x & (1 << y) to 0 opt node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
            TR::Node::recreate(node, TR::iand);
            TR::Node* one = TR::Node::create(node, TR::iconst, 0);
            one->setInt(1);
...
TR::Node* lcmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0ll && performTransformation(s->comp(), "%sReplace lcmplt to 0 with lushr node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
        TR::Node* shiftBy = TR::Node::create(node, TR::iconst, 0);
        shiftBy->setInt(63);
...
TR::Node* endBlockSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (cfg) {
        ...
        if (lastTree->getNode()->getOpCode().isBranch() || (lastTree->getNode()->getOpCode().isJumpWithMultipleTargets() && lastTree->getNode()->getOpCode().hasBranchChildren())) {
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (cfg) {
            ...
            for (caseIndex = numCases + 1; caseIndex >= 1; caseIndex--) {
                ...
                if (dest != target) {
                    TR::Block* destBlock = dest->getNode()->getBlock();
                    if (!removedTargets.get(destBlock->getNumber()) && performTransformation(s->comp(), "%sRemoving edge block_%d->block_%d in CFG because switch value is known\n", s->optDetailString(), block->getNumber(), destBlock->getNumber())) {
...
TR::Node* checkcastSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((node->getFirstChild()->getReferenceCount() > 1) && (node->getSecondChild()->getReferenceCount() > 1)) {
        ...
        while (prevTree && ((prevTree->getNode()->getOpCodeValue() == TR::BBStart) || (prevTree->getNode()->getOpCodeValue() == TR::BBEnd)))
        ...
        if (prevTree) {
            ...
            if ((prevNode->getOpCodeValue() == TR::ificmpeq) || (prevNode->getOpCodeValue() == TR::ificmpne)) {
                ...
                if (firstChild->getOpCodeValue() == TR:: instanceof && secondChild->getOpCodeValue() == TR::iconst && ((secondChild->getInt() == 0 && equalityTest) || (secondChild->getInt() == 1 && !equalityTest)) && firstChild->getFirstChild() == node->getFirstChild() && firstChild->getSecondChild() == node->getSecondChild() && performTransformation(s->comp(), "%sRemoving checkcast node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* nullchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (nullCheckRefOp == TR::New || nullCheckRefOp == TR::newarray || nullCheckRefOp == TR::anewarray || nullCheckRefOp == TR::multianewarray) {
        TR::Node::recreate(node, TR::treetop);
        simplifyChildren(node, block, s);
        return node;
    }
    ...
    if (node->getFirstChild()->getNumChildren() == 0) {
        ...
        if ((refNode->isNull() || ((refNode->getOpCodeValue() == TR::aconst) && (refNode->getAddress() == 0))) && performTransformation(s->comp(), "%sRemoving rest of the block past a NULLCHK that will fail [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node* divchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (child != originalChild || !(child->getOpCode().isDiv() || child->getOpCode().isRem())) {
        TR::Node::recreate(node, TR::treetop);
        node->setFirst(child);
        return node;
    }
    ...
    if (divisor->getOpCode().isLoadConst()) {
        if (((divisor->getOpCode().isLong() && divisor->getLongInt() != 0) || ((!divisor->getOpCode().isLong()) && divisor->getInt() != 0)) && performTransformation(s->comp(), "%sRemoved divchk with constant non-zero divisor in node[%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
            //s->removeNode(node, s->_curTree);
            //return NULL;
            TR::Node::recreate(node, TR::treetop);
            return node;
...
TR::Node* bndchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((boundOp == TR::iconst) && (boundOp == indexOp) && (boundChild->getInt() > indexChild->getInt()) && (indexChild->getInt() >= 0))
    ...
    if (removableBoundCheck || (indexOp == TR::irem && indexChild->getFirstChild()->isNonNegative() && boundChild == indexChild->getSecondChild())) {
    ...
    //
    if ((boundChild->getOpCode().isMul() && !isNodeMulHigh(boundChild) && boundChild->getSecondChild()->getOpCode().isLoadConst()) && (indexChild->getOpCode().isMul() && !isNodeMulHigh(indexChild) && indexChild->getSecondChild()->getOpCode().isLoadConst())) {
    ...
    } else if (boundChild->getOpCode().isLoadConst() && (indexChild->getOpCode().isMul() && !isNodeMulHigh(indexChild) && indexChild->getSecondChild()->getOpCode().isLoadConst())) {
...
TR::Node* arraycopybndchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((lhsOp == TR::iconst) && (lhsOp == rhsOp) && (lhsChild->getInt() >= rhsChild->getInt()))
        removableBoundCheck = true;
    else if ((lhsChild == rhsChild) || ((lhsChild->getNumChildren() == rhsChild->getNumChildren()) && (lhsChild->getNumChildren() == 1) && s->optimizer()->areNodesEquivalent(lhsChild, rhsChild) && lhsChild->getFirstChild() == rhsChild->getFirstChild()))
        removableBoundCheck = true;
    else if ((rhsChild->getOpCode().isArrayLength()) && (lhsOp == TR::imul) && (lhsChild->getFirstChild() == rhsChild) && lhsChild->getSecondChild()->getOpCode().isLoadConst() && (lhsChild->getSecondChild()->getInt() > 0) && (lhsChild->getSecondChild()->getInt() <= rhsChild->getArrayStride()))
        removableBoundCheck = true;
    ...
    if (rhsChild->getOpCodeValue() == TR::iadd) {
        ...
        if (lengthChild == boundChild || s->isBoundDefinitelyGELength(boundChild, lengthChild)) {
            ...
            if (indexChild->getOpCodeValue() == TR::isub) {
                if ((indexChild->getFirstChild() == boundChild || indexChild->getFirstChild() == lengthChild) && indexChild->getSecondChild()->getOpCodeValue() == TR::iconst && indexChild->getSecondChild()->getInt() == 1) {
                ...
                if (boundChild->getOpCodeValue() == TR::iadd && (boundChild->getFirstChild() == indexChild || boundChild->getSecondChild() == indexChild)) {
    ...
    // overflow, so we can safely remove them
    //
    if ((lhsChild->getOpCode().isMul() && lhsChild->getSecondChild()->getOpCode().isLoadConst()) && (rhsChild->getOpCode().isMul() && rhsChild->getSecondChild()->getOpCode().isLoadConst())) {
...
TR::Node* bndchkwithspinechkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (boundChild->getOpCode().isLoadConst() && indexChild->getOpCode().isLoadConst() && (boundChild->getInt() > indexChild->getInt()) && (indexChild->getInt() >= 0)) {
        if (performTransformation(s->comp(), "%sRemoved bndchk with constant arguments in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
    ...
    if (boundChild->getOpCode().isLoadConst() && !s->comp()->generateArraylets()) {
        ...
        if (elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(boundChild->getInt(), elementSize) && performTransformation(s->comp(), "%sRemoving spine check because constant arraylength is contiguous in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node* arraysetSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (fill->getOpCode().isLoadConst() && fill->getDataType() == TR::Int64) {
        ...
        if ((fillVal & 0x0FFFFFFFFL) == ((fillVal >> 32) & 0x0FFFFFFFFL) && performTransformation(s->comp(), "%sTransform large fill arrayset to 4byte fill arrayset [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* arrayLengthSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::contigarraylength) {
        if (firstChildOp == TR::newarray || firstChildOp == TR::anewarray) {
            ...
            if (arraySize->getOpCode().isLoadConst() && elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(arraySize->getInt(), elementSize) && performTransformation(s->comp(), "%sReducing contiguous arraylength of newarray or anewarray in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
    ...
    if ((firstChildOp == TR::newarray || firstChildOp == TR::anewarray) && performTransformation(s->comp(), "%sReducing arraylength of newarray or anewarray in node [%s]\n", s->optDetailString(), node->getName(s->getDebug()))) {
        TR::Node* arraySize = firstChild->getFirstChild();
        arraySize->incReferenceCount();
        firstChild->setVisitCount(0);
        node->setVisitCount(0);
