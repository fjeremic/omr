  ...
  if (secondChild->getOpCode().isLoadConst()) {                                                                       \
    ...
    if (value == ZeroValue) {                                                                                         \
      if (performTransformation(                                                                                      \
            s->comp(), "%sFound op with iconst in node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) { \
...
static TR::ILOpCodes addOps[TR::NumTypes] = { TR::BadILOp, TR::badd, TR::sadd, TR::iadd, TR::ladd, TR::fadd, TR::dadd,
  TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp };

static TR::ILOpCodes subOps[TR::NumTypes] = { TR::BadILOp, TR::bsub, TR::ssub, TR::isub, TR::lsub, TR::fsub, TR::dsub,
  TR::asub, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp };

static TR::ILOpCodes constOps[TR::NumTypes] = { TR::BadILOp, TR::bconst, TR::sconst, TR::iconst, TR::lconst, TR::fconst,
  TR::dconst, TR::aconst, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp };

static TR::ILOpCodes negOps[TR::NumTypes] = { TR::BadILOp, TR::bneg, TR::sneg, TR::ineg, TR::lneg, TR::fneg, TR::dneg,
  TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp, TR::BadILOp };
  ...
  if (s->reassociate() && (node->getOpCode().isAdd() || node->getOpCode().isSub())
    ...
    if (!s->_hashTable.locate(node->getFirstChild()->getGlobalIndex(), index)) {
      ...
      if (base_node != node && base_node->getReferenceCount() >= 1
        ...
        int64_t diff
          = node->getSecondChild()->get64bitIntegralValue() - base_node->getSecondChild()->get64bitIntegralValue();

        if (!s->comp()->cg()->shouldValueBeInACommonedNode(diff)
          && performTransformation(
            s->comp(), "%sReusing big constant from node 0x%p in node 0x%p\n", s->optDetailString(), base_node, node)) {
...
static TR::Node* addSimplifierCommon(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (s->reassociate() && node->getOpCode().isAdd()) {
    ...
    if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
      && ((!isExprInvariant(region, firstChild->getFirstChild())
        ...
        || (!firstChild->getFirstChild()->getOpCode().isLoadConst()
          && firstChild->getSecondChild()->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()))
      &&
    ...
    //     / \                / \
      ...
      && (!isExprInvariant(region, secondChild) || (node->getOpCode().isArrayRef() && isSmallConstant(secondChild, s)))
    ...
    //     / \   / \        / \   / \       / \   / \       / \   / \
      ...
      && !isExprInvariant(region, firstChild->getFirstChild()) && !isExprInvariant(region, secondChild->getFirstChild())
      && isExprInvariant(region, firstChild->getSecondChild())
    ...
    //         / \        / \
      ...
      && secondChild->getOpCode().isSub() && firstChild->getDataType() == secondChild->getSecondChild()->getDataType()
      && !isExprInvariant(region, secondChild->getFirstChild())
      && isExprInvariant(region, secondChild->getSecondChild())) {
      if (performTransformation(
    ...
    //                         \
      //                          c2
    else if (node->getOpCode().isArrayRef() && isExprInvariant(region, firstChild) && secondChild->getOpCode().isSub()
      ...
      if (performTransformation(
        ...
        TR::Node* newFirstChild = TR::Node::create(firstChild, node->getOpCodeValue(), 2);
        TR::Node* newNeg
          = TR::Node::create(secondChild->getSecondChild(), negOps[secondChild->getSecondChild()->getDataType()], 1);
        TR::Node* zero
          = TR::Node::create(secondChild->getSecondChild(), constOps[secondChild->getSecondChild()->getDataType()], 0);
...
TR::Node* subSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  // turn isub with first child ineg into an ineg with iadd child.  isn't quicker, but normalizes expressions and drives
  // inegs up the tree
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (shiftValue != secondChild->getInt()) {
      if (performTransformation(s->comp(), "%sReducing constant of node [%s] from %d to %d\n", s->optDetailString(),
            secondChild->getName(s->getDebug()), secondChild->getInt(), shiftValue)) {
        if (secondChild->getReferenceCount() > 1) {
...
static void normalizeShiftAmount(TR::Node* node, int32_t normalizationConstant, TR::Simplifier* s)
{
  if (s->comp()->cg()->needsNormalizationBeforeShifts() && !node->isNormalizedShift()) {
    ...
    if ((secondChild->getOpCodeValue() != TR::iconst)
      && ((secondChild->getOpCodeValue() != TR::iand) || (secondChild->getSecondChild()->getOpCodeValue() != TR::iconst)
...
static bool reduceLongOp(TR::Node* node, TR::Block* block, TR::Simplifier* s, TR::ILOpCodes newConversionOp)
{
  ...
  if (newConversionOp == TR::BadILOp) {
    ...
    node->setAndIncChild(1,
      convertSecondChild ? TR::Node::create(TR::l2i, 1, firstChild->getSecondChild()) : firstChild->getSecondChild());
...
static TR::Node* intDemoteSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (sourceIs64Bit) {
    ...
    if (targetDataType == TR::Int32) {
      ...
      reduceOp
        = TR::ILOpCode::getProperConversion(TR::Int32, targetDataType, false); // appropriate integer to integer op
    }
          ...
          if (predEndWithGoto || isImmediatePred) {
            ...
            if (storeNode->getOpCode().isStoreDirect() && storeSymRef == symRef
              ...
              if (performTransformation(
                ...
                TR::Block* gotoBlock
                  = TR::Block::createEmptyBlock(gotoTree->getNode(), comp, predBlock->getFrequency(), predBlock);
                ...
                if (predEndWithGoto) {
                  ...
                  gotoTree = TR::TreeTop::create(
                    comp, gotoBlock->getEntry(), TR::Node::create(storeNode, TR::Goto, 0, fallThroughTT));
...
static void addressCompareConversion(TR::Node* node, TR::Simplifier* s)
{
  ...
  if (firstOp == TR::a2i && firstChild->getFirstChild()->getType().isAddress() && TR::Compiler->target.is32Bit()
    && firstChild->getReferenceCount() == 1) {
  ...
  } else if (firstOp == TR::a2l && firstChild->getFirstChild()->getType().isAddress() && TR::Compiler->target.is64Bit()
    && firstChild->getReferenceCount() == 1) {
  ...
  //    performTransformation(s->comp(), "%sLong compare narrower for node [%p]\n", s->optDetailString(), node))
  {
    ...
    if ((firstOp == TR::i2l)
      ...
      if (secondOp == TR::iconst || secondOp == TR::i2l
        || (secondOp == TR::lconst && secondChild->getLongInt() >= INT_MIN && secondChild->getLongInt() <= INT_MAX)) {
        node->setAndIncChild(0, firstChild->getFirstChild());
        ...
        if (secondOp == TR::i2l) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(
              s->comp(), "Long Compare Narrower: found both children i2l in method %s\n", s->comp()->signature());
          }
        } else if (secondOp == TR::lconst) {
          if (secondChild->getReferenceCount() == 1) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
              "Long Compare Narrower: found child 1 i2l and child 2 lconst in iconst range in method %s\n",
              s->comp()->signature());
          }
      ...
      if ((val1 & 0xffffffff00000000ull) == 0 && (val2 & 0xffffffff00000000ull) == 0
        && performTransformation(
          s->comp(), "%sLong compare narrower: equality with no upper bits [%p]\n", s->optDetailString(), node)) {
        TR::Node::recreate(node, intOp);
        node->notifyChangeToValueOfNode();
      ...
      if (firstOp == TR::su2l) {
        if (secondOp == TR::cconst || secondOp == TR::su2l
          || (secondOp == TR::lconst && secondChild->getLongInt() >= 0 && secondChild->getLongInt() <= USHRT_MAX)) {
          node->setAndIncChild(0, firstChild->getFirstChild());
          ...
          if (secondOp == TR::su2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(
                s->comp(), "Long Compare Narrower: found both children c2l in method %s\n", s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
            if (secondChild->getReferenceCount() == 1) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                "Long Compare Narrower: found child 1 c2l and child 2 lconst in cconst range in method %s\n",
                s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
        ...
        if (secondOp == TR::sconst || secondOp == TR::s2l
          || (secondOp == TR::lconst && secondChild->getLongInt() >= s->cg()->getMinShortForLongCompareNarrower()
          ...
          if (secondOp == TR::s2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(
                s->comp(), "Long Compare Narrower: found both children s2l in method %s\n", s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
            if (secondChild->getReferenceCount() == 1) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                "Long Compare Narrower: found child 1 s2l and child 2 lconst in sconst range in method %s\n",
                s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
        ...
        if (secondOp == TR::bconst || secondOp == TR::b2l
          || (secondOp == TR::lconst && secondChild->getLongInt() >= s->cg()->getMinByteForLongCompareNarrower()
          ...
          if (secondOp == TR::b2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(
                s->comp(), "Long Compare Narrower: found both children b2l in method %s\n", s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
            if (secondChild->getReferenceCount() == 1) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                "Long Compare Narrower: found child 1 b2l and child 2 lconst in bconst range in method %s\n",
                s->comp()->signature());
            }
          } else if (reportCompareDemotions) {
    ...
    if (count == 1) {
      if (operationType[from] == OP_MINUS) {
        ...
        for (i = 0; i < tabspace; i++)
          traceMsg(s->comp(), "\t");
        traceMsg(
          s->comp(), "\t-> %cn<<%d ", (operationType[from] == OP_MINUS) ? '-' : '+', (int32_t)(bitPosition[from]));
      } else
...
TR::Node* getQuotientUsingMagicNumberMultiply(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (node->getOpCodeValue() == TR::idiv || node->getOpCodeValue() == TR::irem) {
    ...
    if (shiftAmount == 0) {
      ...
      node3 = TR::Node::create(TR::lshr, 2, node2, TR::Node::create(node2, TR::iconst, 0, shiftAmount));
...
static double doubleRecip(double value)
{
  ...
  TR_ASSERT(
    double_exp != 0 && double_exp != 0x7ff && (u.i & CONSTANT64(0xfffffffffffff)) == 0, "bad argument to doubleRecip");
  ...
  TR::Node* bbStartNode, TR::CFGEdgeList& inEdge, TR::Simplifier* s, bool& blockIsEmpty)
{
  ...
  // Since there may be a goto/branch which go around entry point, in that case merging will skip the prologue for that
  // empty entry point See ey1f520, block 5(INTERNAL4) and block 5
    ...
    if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
      if (secondOp == TR::su2i
        ...
        if (secondOp == TR::su2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(
              s->comp(), "Integer Compare Narrower: found both children c2i in method %s\n", s->comp()->signature());
          }
        } else if (secondOp == TR::iconst) {
          if (secondChild->getReferenceCount() > 1) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
              "Integer Compare Narrower: found child 1 c2i and child 2 iconst in cconst range in method %s\n",
              s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
      ...
      if (secondOp == TR::s2i
        ...
        if (secondOp == TR::s2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(
              s->comp(), "Integer Compare Narrower: found both children s2i in method %s\n", s->comp()->signature());
          }
        } else if (secondOp == TR::iconst) {
          if (secondChild->getReferenceCount() > 1) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
              "Integer Compare Narrower: found child 1 s2i and child 2 iconst in sconst range in method %s\n",
              s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
      ...
      if (secondOp == TR::b2i
        || (secondOp == TR::iconst && secondChild->getInt() >= SCHAR_MIN && secondChild->getInt() <= SCHAR_MAX)) {
        node->setAndIncChild(0, firstChild->getFirstChild());
        TR::Node::recreate(node, byteOp);
        firstChild->recursivelyDecReferenceCount();
        if (secondOp == TR::b2i) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(
              s->comp(), "Integer Compare Narrower: found both children b2i in method %s\n", s->comp()->signature());
          }
        } else if (secondOp == TR::iconst) {
          if (secondChild->getReferenceCount() > 1) {
          ...
          if (reportCompareDemotions) {
            dumpOptDetails(s->comp(),
              "Integer Compare Narrower: found child 1 b2i and child 2 iconst in bconst range in method %s\n",
              s->comp()->signature());
          }
        } else if (reportCompareDemotions) {
...
static bool checkAndReplaceRotation(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  TR_ASSERT(node->getOpCode().isOr() || node->getOpCode().isXor(),
    ...
    node); // I believe this is general enough to allow other opcodes, like add's, xor's. But just want to be safe for
           // now.
  ...
  // isShiftLogical = %d isRightShift = %d\n",firstChild->getOpCode().isMul(), firstChild->getOpCode().isShiftLogical(),
  // firstChild->getOpCode().isRightShift(), secondChild->getOpCode().isMul(), firstChild->getOpCode().isShiftLogical(),
  // firstChild->getOpCode().isRightShift());
  ...
  TR::Node::recreate(
    node, TR::ILOpCode::getRotateOpCodeFromDt(mulConstNode->getDataType())); // this line will have to change based on T
...
static void bitTestingOp(TR::Node* node, TR::Simplifier* s)
{
  ...
  switch (dt) {
  case TR::Int8:
    foldByteConstant(firstChild->getSecondChild(), (bitAmount & 0xFF) >> shiftAmount, s, false /* !anchorChildren */);
...
static bool isOperationFPCompliant(TR::Node* parent, TR::Node* arithExprNode, TR::Simplifier* s)
{
  ...
  if (mulNode->getDataType() == TR::Double && mulNode->getFirstChild()->getOpCode().isConversion()
    && mulNode->getSecondChild()->getOpCode().isConversion()) { // long->double is only case where we must give up
  ...
  if (!mulChildIsConst && s->cg()->supportsOnDemandLiteralPool()) {
    ...
    if (mulChildIsConst
      || (mulConstChild->getOpCode().isLoadIndirect() && mulConstChild->getSymbolReference()->isLiteralPoolAddress())) {
    ...
    case TR::Double: // rely on fact same storage used for longs
      ...
      if (traceIt)
        traceMsg(s->comp(), "Node:%p  bitcount:%d hex:%x%x value:%g\n", mulConstChild, bitcount,
          mulConstChild->getLongIntHigh(), mulConstChild->getLongIntLow(), mulConstChild->getDouble());
      ...
      if (possibleRoundedValue >= FMA_CONST_HIGHBOUNDF2D || possibleRoundedValue < FMA_CONST_LOWBOUND) {
        if (traceIt)
          traceMsg(s->comp(), "Fails because |constant| %e exceeds range %e-%e \n", possibleRoundedValue,
            FMA_CONST_LOWBOUND, FMA_CONST_HIGHBOUNDF2D);
        return false;
      }
    }
  ...
  bool loadConstIsPowerOfTwoDbl
    = (mulConstChild->getDataType() == TR::Double) && isNZDoublePowerOfTwo(mulConstChild->getDouble());

  bool loadConstIsPowerOfTwoFloat
    = (mulConstChild->getDataType() == TR::Float) && isNZFloatPowerOfTwo(mulConstChild->getFloat());
...
static void bitwiseToLogical(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  TR::SymbolReference* temp1
    = s->comp()->getSymRefTab()->createTemporary(s->comp()->getMethodSymbol(), cmp2->getFirstChild()->getDataType());
  prevTree = TR::TreeTop::create(s->comp(), prevTree, TR::Node::createStore(temp1, cmp2->getFirstChild()));
  TR::SymbolReference* temp2
    = s->comp()->getSymRefTab()->createTemporary(s->comp()->getMethodSymbol(), cmp2->getSecondChild()->getDataType());
...
public:
  ...
  {
    if (performTransformation(
          _simplifier->comp(), "%sReduce an ifxcmpge node [%p] to ifxcmpeq\n", _simplifier->optDetailString(), _node)) {
...
TR::Node* indirectLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (s->comp()->cg()->getSupportsVectorRegisters()) {
    ...
    if ((node->getType().isIntegral() || node->getType().isDouble())
      ...
      && performTransformation(
        s->comp(), "%sReplace indirect load [" POINTER_PRINTF_FORMAT "] with getvelem", s->optDetailString(), node)) {
      int32_t offset = 0;
...
TR::Node* indirectStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if ((node->getOpCodeValue() == TR::awrtbari) && 0
    ...
    if ((valueChild->getOpCodeValue() == TR::New) || (valueChild->getOpCodeValue() == TR::newarray)
      || (valueChild->getOpCodeValue() == TR::anewarray) || (valueChild->getOpCodeValue() == TR::MergeNew)
      ...
      if (removeWrtBar && !s->comp()->getOptions()->realTimeGC()
        && performTransformation(s->comp(),
          "%sFolded indirect write barrier to iastore because GC could not have occurred enough times to require "
          "iwrtbar [" POINTER_PRINTF_FORMAT "]\n",
          s->optDetailString(), node)) {
  ...
  if (s->comp()->cg()->getSupportsVectorRegisters()) {
    ...
    if ((node->getType().isIntegral() || node->getType().isDouble())
      ...
      && ((addressNode->getOpCode().isArrayRef() && addressNode->getSecondChild()->getOpCode().isLoadConst()
        ...
        || (addressNode->getOpCode().hasSymbolReference() && addressNode->getSymbol()->getType().isVector()))
      && performTransformation(
        s->comp(), "%sReplace indirect store [" POINTER_PRINTF_FORMAT "] with vsetelem", s->optDetailString(), node)) {
      int32_t offset = 0;
      TR::SymbolReference* oldSymRef = node->getSymbolReference();
...
TR::Node* directStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  // NOTE: this likely won't improve register pressure; it's purely a codegen quality improvement
    ...
    if ((firstLoadsSymRef && secondIsLoadConst) || (firstIsLoadConst && secondLoadsSymRef)) {
      // found a candidate update form tree: now walk trees forward until either something stores to <sym> or end of
      // block remember tree where last use of node was on this walk: insert our update tree following that last use
      // tree

      // first, find the actual update tree (this is obviously our first potential insertion point for the update tree
      ...
      while (updateTree->getNode() != node) {
        ...
        // if we reach the end of the block, then the update tree is probably under a ResolveCHK node and we don't
        // really want to move that
        if (updateTree == block->getExit())
          return node;
      }

      // dumpOptDetails(s->comp(), "found update form treetop [" POINTER_PRINTF_FORMAT "] node [" POINTER_PRINTF_FORMAT
      // "] for symbol #%d[" POINTER_PRINTF_FORMAT "]\n", updateTree->getNode(), node, symRef->getReferenceNumber());
      ...
      if (loadTree->getNode()->getOpCodeValue() == TR::treetop) {
        ...
        if (loadNode->getOpCode().isLoadVarDirect() && loadNode->getSymbolReference() == symRef) {
          ...
          if (insertionTree != NULL
            ...
            // So we decided to swing this update tree below the load, but eliminateDeadTrees might subsequently try to
            // undo it by moving the load below the update tree (bad pass!).  We'll allow it to do that (because we can
            // come along and re-swing the update below the load) so long as it doesn't move the load under a branch
            // tree.  If the load gets moved into a branch tree, then there's no place we could put the update tree
            // that's below the load but still in the block (executive summary: all our careful work above would be for
            // naught).  So mark the load here so that eliminateDeadTrees won't make us look (as?) bad.
...
TR::Node* vsetelemSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  for (TR::Node* vsetelemNode = node; vsetelemNode && vsetelemNode->getOpCodeValue() == TR::vsetelem;
    ...
    // If we've seen this index before, then the outer set element overrides the inner set element, so it's safe to skip
    // this node
...
TR::Node* iaddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  // reduce imul operations by factoring
    ...
    if ((lrChild->getOpCodeValue() == TR::iconst)
      && ((firstChild->getFutureUseCount() == 0)
        || (!s->comp()->cg()->areAssignableGPRsScarce() && (lrChild->getInt() == -1 * secondChild->getInt())))) {
      if (performTransformation(s->comp(), "%sFound aiadd of iconst with aiadd x and iconst in node [%s]\n",
            s->optDetailString(), node->getName(s->getDebug()))) {
        int32_t value = secondChild->getInt();
        value += lrChild->getInt();
    ...
    //
    // n1.3083n   (  0)      iadd (in *GPR_6512) (profilingCode ) [0x2BB38260] loc=[-1,2848,1153] rc=0 vc=631 vn=- sti=-
    // udi=14723 nc=2 flg=0x80 n1.3081n   (  0)        a2i (in *GPR_6512) (profilingCode unneededConv ) [0x2BB381C0]
    // loc=[-1,2847,1153] rc=0 vc=631 vn=- sti=- udi=14723 nc=1 addr=4 flg=0x8080 n1.26322n  (  0)          aiadd (in
    // *GPR_6512) (profilingCode X>=0 internalPtr )                       [0x374931DC] loc=[-1,2844,1153] rc=0 vc=1859
    // vn=- sti=- udi=14723 nc=2 addr=4 flg=0x8180 n1.3077n   (  2)            ==>aload (in GPR_6513) (profilingCode )
    // n1.3086n   (  0)            ==>iconst 692 (profilingCode X!=0 X>=0 )
    // n1.3082n   (  0)        iconst -24 (profilingCode X!=0 X<=0 ) [0x2BB38210] loc=[-1,2848,1153] rc=0 vc=631 vn=-
    // sti=- udi=- nc=0 flg=0x284
    //
...
TR::Node* laddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()
    && performTransformation(
      s->comp(), "%sSimplified ladd in node [" POINTER_PRINTF_FORMAT "] to lconst\n", s->optDetailString(), node)) {
    if (node->nodeRequiresConditionCodes()) {
...
TR::Node* lsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (node->getOpCodeValue() != TR::aladd
    ...
    if ((rrChild->getOpCodeValue() == TR::lconst) && (rlChild == firstChild) && node->cannotOverflow()
      && secondChild->cannotOverflow()) {
      if (performTransformation(s->comp(),
            "%sFolded lsub with children related through lconst in node [" POINTER_PRINTF_FORMAT "] to lconst \n",
            s->optDetailString(), node)) {
        node->setChild(0, 0);
        node->setChild(1, 0);
  ...
  // turn lsub with first child lneg into an lneg with ladd child.  isn't quicker, but normalizes expressions and drives
  // lnegs up the tree
      ...
      if (secondChildOp == TR::lconst) {
        ...
        if (lrChild->getOpCodeValue() == TR::iconst) {
          if (node->isNonNegative() && i2lNode->isNonNegative()) {
            ...
            if (performTransformation(s->comp(),
                  "%sFound lsub of lconst with i2l of iadd or isub of x and iconst in node [" POINTER_PRINTF_FORMAT
                  "]\n",
                  s->optDetailString(), node)) {
              // Reset the flags on i2lNode to reflect the new child - llChild
  ...
  if (!node->nodeRequiresConditionCodes() && secondChild->getOpCodeValue() == TR::land
    ...
    if (((mask + 1) & mask) == 0 && // mask + 1 power of two?
      performTransformation(
        s->comp(), "%sNext lower pwr of 2 using land [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* imulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (secondChildOp == TR::iconst && secondChild->getInt() < 0 && secondChild->getInt() != TR::getMinSigned<TR::Int32>()
    && performTransformation(
  ...
  if (s->getLastRun() && node->isNonNegative() && node->getOpCode().isMul()
    && node->getFirstChild()->getOpCode().isDiv() && node->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()
...
TR::Node* lmulSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  } else if (TR::Compiler->target.is64Bit() && secondChildOp == TR::lconst && !s->getLastRun()
...
TR::Node* ldivSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (divisor != 0) {
      if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (firstChild->isNonNegative()) {
          if (divisor < 0) {
            if (performTransformation(
                  s->comp(), "%sReduced ldiv power of 2 with lneg lshr in node [%p]\n", s->optDetailString(), node)) {
              TR::Node* newChild = TR::Node::create(secondChild, TR::iconst, 0);
            ...
            if (performTransformation(
                  s->comp(), "%sReduced ldiv power of 2 with lshr in node [%p]\n", s->optDetailString(), node)) {
        ...
        node3 = TR::Node::create(TR::lshr, 2, node2, TR::Node::create(node2, TR::iconst, 0, (int32_t)shiftAmount));
  ...
  if (node->getOpCodeValue() == TR::ldiv) {
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 10
      ...
      && // transformToLongDiv creates a tonne of nodes.  Not good to do this recursively if you have chained divides
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (divisor != 0) {
      if (divisor == 1 || (!isUnsigned && (divisor == -1))) {
        ...
        int32_t result
          = (node->getOpCodeValue() == TR::iurem) ? (uint32_t)dividend % (uint32_t)divisor : dividend % divisor;
      ...
      } else if (node->getOpCodeValue() == TR::irem && s->cg()->getSupportsLoweringConstIDiv() && !isPowerOf2(divisor)
...
TR::Node* lremSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (node->getOpCodeValue() == TR::lrem) {
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 10
      && !skipRemLoweringForPositivePowersOfTen(s) && node->getFirstChild()->getOpCode().isLoadVar()
      && performTransformation(
        s->comp(), "%sReduced lrem by 10 [%p] to sequence of bitwise operations\n", s->optDetailString(), node)) {
      TR::TreeTop* curTree = s->_curTree;
      TR::Node* divCheckParent = NULL;
      if ((curTree->getNode()->getOpCodeValue() == TR::DIVCHK) && (curTree->getNode()->getFirstChild() == node))
        divCheckParent = curTree->getNode();
...
TR::Node* fnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  // don't just do -A op B -> -(A op B) as not always a win, so catch opportunity here
    ...
    if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub())
      ...
      && performTransformation(
        s->comp(), "%sTransforming [" POINTER_PRINTF_FORMAT "] -(A*B) -> -((A*B)-0)\n", s->optDetailString(), node)) {
      TR::Node* newConst = TR::Node::create(firstChild, TR::fconst, 0);
      newConst->setFloat(0);
...
TR::Node* ilfdabsSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (child->isNonNegative() && performFlagBasedTransformation
    && performTransformation(
      s->comp(), "%sSimplify abs of non-negative child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
    return s->replaceNodeWithChild(node, child, s->_curTree, block);
  } else if (child->isNegative() && performFlagBasedTransformation
    && performTransformation(
      s->comp(), "%sSimplify abs of non-positive child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
    TR::Node::recreate(node, neg);
    return s->simplify(node, block);
  } else if ((child->getOpCodeValue() == abs || child->getOpCodeValue() == neg)
    && performTransformation(
      s->comp(), "%sSimplify abs of abs/neg child at [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* lshlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(
      node, firstChild->getLongInt() << (secondChild->getInt() & LONG_SHIFT_MASK), s, false /* !anchorChildren */);
    return node;
  }
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    //
    performTransformation(s->comp(),
      "%sCanonicalize long left shift by constant in node [" POINTER_PRINTF_FORMAT "] to long multiply by power of 2\n",
      s->optDetailString(), node);
    TR::Node::recreate(node, TR::lmul);
...
TR::Node* sshlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(
      node, firstChild->getShortInt() << (secondChild->getInt() & INT_SHIFT_MASK), s, false /* !anchorChildren */);
    return node;
  }
  BINARY_IDENTITY_OP(Int, 0)
  return node;
...
TR::Node* iushrSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
    foldUIntConstant(
      node, firstChild->getUnsignedInt() >> (secondChild->getInt() & INT_SHIFT_MASK), s, false /* !anchorChildren*/);
    return node;
  }
    ...
    if (constGrandChild->getOpCodeValue() == TR::iconst && constGrandChild->getInt() == (1 << rightShiftValue)) {
      ...
      if (subTree->getReferenceCount() == 1) {
        if (opCode == TR::s2i && rightShiftValue == 16) {
          if (performTransformation(s->comp(),
                "%sReduced left shift followed by iushr equivalent to zero extend short in node [" POINTER_PRINTF_FORMAT
                "] to su2i\n",
                s->optDetailString(), node)) {
          ...
          if (performTransformation(s->comp(),
                "%sReduced left shift followed by iushr equivalent to zero extend byte in node [" POINTER_PRINTF_FORMAT
                "] to bu2i\n",
                s->optDetailString(), node)) {
    ...
    if (constGrandChild->getOpCodeValue() == TR::lconst
      ...
      if (subTree->getReferenceCount() == 1) {
        if (opCode == TR::i2l && rightShiftValue == 32) {
          if (performTransformation(s->comp(),
                "%sReduced left shift followed by lushr equivalent to zero extend int in node [" POINTER_PRINTF_FORMAT
                "] to iu2l\n",
                s->optDetailString(), node)) {
            foundZeroExtension = true;
          ...
          if (performTransformation(s->comp(),
                "%sReduced left shift followed by lushr equivalent to zero extend byte in node [" POINTER_PRINTF_FORMAT
                "] to bu2l\n",
                s->optDetailString(), node)) {
            foundZeroExtension = true;
...
TR::Node* lrolSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
    ...
    uint64_t value
      = (firstChild->getUnsignedLongInt() << rotateAmount) | (firstChild->getUnsignedLongInt() >> (64 - rotateAmount));
...
TR::Node* landSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) && secondChild->getReferenceCount() == 1
      ...
      if (performTransformation(s->comp(),
            "%sReduced land with two complemented children in node [" POINTER_PRINTF_FORMAT "] to complemented lor\n",
            s->optDetailString(), node)) {
...
bool isChildOrConstRedundant(TR::Node* parentOrConstNode, TR::Node* childOrConstNode, TR::Simplifier* s)
{
  ...
  // if the result of or'ing the two or constants is the same as the parent (int or) constant then can remove child oor
  // operation
...
TR::Node* removeRedundantIntegralOrPattern2(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  dumpOptDetails(s->comp(),
    "%sCreate new zero extension conversion %s [" POINTER_PRINTF_FORMAT "] of childOr child %s [" POINTER_PRINTF_FORMAT
    "]\n",
...
TR::Node* iorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseIntComplement(firstChild) && secondChild->getReferenceCount() == 1
      && isBitwiseIntComplement(secondChild)) {
      if (performTransformation(s->comp(),
            "%sReduced ior with two complemented children in node [" POINTER_PRINTF_FORMAT "] to complemented iand\n",
            s->optDetailString(), node)) {
...
TR::Node* lorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) && secondChild->getReferenceCount() == 1
      && isBitwiseLongComplement(secondChild)) {
      if (performTransformation(s->comp(),
            "%sReduced lor with two complemented children in node [" POINTER_PRINTF_FORMAT "] to complemented land\n",
            s->optDetailString(), node)) {
  ...
  if ((TR::Compiler->target.is64Bit() || s->comp()->cg()->use64BitRegsOn32Bit())
...
TR::Node* borSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isAnd() && firstChild->getReferenceCount() == 1 && secondChild->getOpCode().isLoadConst()
    && firstChild->getSecondChild()->getOpCode().isLoadConst()
...
TR::Node* i2lSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCodeValue() == TR::l2i && firstChild->getFirstChild()->getOpCodeValue() == TR::lshr
    ...
    if (shiftBy >= 57
      && performTransformation(
        s->comp(), "%sRemove i2l/l2i from lshr node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
      TR::Node::recreate(node, TR::lshr);
      node->setNumChildren(2);
...
TR::Node* l2aSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if ((firstChild->getOpCodeValue() == TR::lsub || firstChild->getOpCodeValue() == TR::ladd)
    ...
    if (!(addressAddOp == TR::aladd && !is64Bit) && // aladd only allowed in 64bit
      ...
      node->setNumChildren(
        2); // upto 2 children do not require node extension. this should suffice. Any more requires Node::addChildren
  ...
  if (firstChild->getReferenceCount() == 1) {
    ...
    if (childOp == TR::i2s) {
      ...
      if (firstGrandChild->getReferenceCount() == 1 && firstGrandChild->getOpCodeValue() == node->getOpCodeValue()) {
        if (performTransformation(s->comp(),
              "%sReduced su2i node [" POINTER_PRINTF_FORMAT "] and i2s child [" POINTER_PRINTF_FORMAT "] to no-op\n",
...
TR::Node* ificmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCode().isLoadConst()
    && conditionalBranchFold((firstChild->getInt() == secondChild->getInt()), node, firstChild, secondChild, block, s))
    return node;
...
TR::Node* iflcmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (node->getOpCodeValue() == TR::iflcmplt) {
    ...
    if (firstChild->getOpCode().isLoadConst()
      && conditionalBranchFold((((uint64_t)originalFirst->getLongInt()) < ((uint64_t)originalSecond->getLongInt())),
...
TR::Node* normalizeCmpSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (node->getOpCode().isBranch()) {
    ...
    if (firstChild->getOpCode().isDouble() && secondChild->getOpCodeValue() == TR::dconst) {
      ...
      } else if (firstChild->getOpCodeValue() == TR::s2d && doubleConstIsRepresentableExactlyAsShort(dValue, sValue)
        && performTransformation(s->comp(),
      ...
      } else if (firstChild->getOpCodeValue() == TR::su2d && doubleConstIsRepresentableExactlyAsChar(dValue, cValue)
        && performTransformation(s->comp(),
      ...
      } else if (firstChild->getOpCodeValue() == TR::b2d && doubleConstIsRepresentableExactlyAsByte(dValue, bValue)
        && performTransformation(s->comp(),
      ...
      } else if (firstChild->getOpCodeValue() == TR::s2f && floatConstIsRepresentableExactlyAsShort(fValue, sValue)
        && performTransformation(s->comp(),
      ...
      } else if (firstChild->getOpCodeValue() == TR::su2f && floatConstIsRepresentableExactlyAsChar(fValue, cValue)
        && performTransformation(s->comp(),
...
TR::Node* lcmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (firstChild->getOpCodeValue() == TR::land && firstChild->getSecondChild()->getOpCodeValue() == TR::lconst
    ...
    if (val1 == 0 && val2 == 0
      && performTransformation(
        s->comp(), "%sChanging lcmpeq %p to icmpeq because there are no upper bits\n", s->optDetailString(), node)) {
  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0
    ...
    if (firstSecond->getOpCodeValue() == TR::lshl && firstSecond->getFirstChild()->getOpCode().isLoadConst()
      ...
      if ((val & (val - 1)) == 0 && // power of two?
        performTransformation(
          s->comp(), "%slcmpne of (x & 2**c) to 0 node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* lcmpltSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0ll
    && performTransformation(
      s->comp(), "%sReplace lcmplt to 0 with lushr node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
    TR::Node* shiftBy = TR::Node::create(node, TR::iconst, 0);
    shiftBy->setInt(63);
...
TR::Node* endBlockSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  if (cfg) {
    ...
    if (rootStructure != NULL) {
      ...
      // This is inconsistent with what we do for CFG in the following, it's better for mergeBlock to merge nextBlock to
      // block, this is a TODO
  ...
  if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (dest != target) {
          ...
          if (!removedTargets.get(destBlock->getNumber())
            && performTransformation(s->comp(),
              ...
              block->getNumber(), destBlock->getNumber())) {
    ...
    if (node->getOpCode().getOpCodeValue() == TR::lookup && NULL != shiftNode && shiftNode->getOpCode().isShift()) {
      ...
      if (NULL != shiftAmtNode && shiftAmtNode->getOpCode().isLoadConst()) {
        ...
        //
        // Example: if the selector is 3 (binary 11) and we shift it right 1, and have case values 0 and 1, we'll match
        // 1 (since 11b >> 1 is 1b) If we shift 0 and 1 left 1 bit, we'll compare 11b to 00b and 10b, and not have a
        // match.
        ...
        for (int caseIndex = 0; caseIndex < numCases; caseIndex++) {
          ...
          caseNode->setCaseConstant(
            isLeftShift ? (caseNode->getCaseConstant() >> shiftAmount) : (caseNode->getCaseConstant() << shiftAmount));
        }
      }
    }
  }
    ...
    if (prevTree) {
      ...
      if ((prevNode->getOpCodeValue() == TR::ificmpeq) || (prevNode->getOpCodeValue() == TR::ificmpne)) {
        ...
        if (firstChild->getOpCodeValue() == TR:: instanceof && secondChild->getOpCodeValue() == TR::iconst
              ...
              && performTransformation(
                s->comp(), "%sRemoving checkcast node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* ifxcmpoSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
  ...
  TR::ILOpCodes opCode = node->getOpCodeValue();
  TR_ASSERT((opCode == TR::ificmpo) || (opCode == TR::ificmpno) || (opCode == TR::iflcmpo) || (opCode == TR::iflcmpno),
    ...
    if (lengthChild == boundChild || s->isBoundDefinitelyGELength(boundChild, lengthChild)) {
      ...
      if (indexChild->getOpCodeValue() == TR::isub) {
        ...
        if (boundChild->getOpCodeValue() == TR::iadd
          ...
          if (performTransformation(s->comp(),
                "%sRemoved arraycopy bndchk with arrayLength index in node [%s] when bound is sum of length and index "
                "arraylengths\n",
                s->optDetailString(), node->getName(s->getDebug()))) {
            s->removeNode(node, s->_curTree);
            return NULL;
  ...
  if (node->getOpCodeValue() == TR::contigarraylength) {
    if (firstChildOp == TR::newarray || firstChildOp == TR::anewarray) {
      ...
      if (arraySize->getOpCode().isLoadConst() && elementSize > 0
        ...
        && performTransformation(s->comp(), "%sReducing contiguous arraylength of newarray or anewarray in node [%s]\n",
          s->optDetailString(), node->getName(s->getDebug()))) {
