...
static void decomposeMultiply(TR::Node *node, TR::Simplifier *s, bool isLong) {
  ...
  count = decomposeConstant(bitPosition, operationType,
...
static void bitTestingOp(TR::Node *node, TR::Simplifier *s) {
  ...
  case TR::Int32:
    ...
    break;
  case TR::Int64:
    foldLongIntConstant(firstChild->getSecondChild(), bitAmount >> shiftAmount,
...
TR::Node *fmaxminSimplifier(TR::Node *node, TR::Block *block,
  ...
  for (i = 0; i < num_children; i++) {
    if (node->getChild(i)->getOpCode().isLoadConst()) {
      ...
      if (jiagblah)
        TR_ASSERT(0, "Should never be considering non float children in "
                     "fmaxmin simplifier!\n");
