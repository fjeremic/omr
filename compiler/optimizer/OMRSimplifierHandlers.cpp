    ...
    if (konst == 0) {                                                          \
      ...
      if (grandChild->getReferenceCount() == 1)                                \
        ...
        secondChild->setAndIncChild(                                           \
            1, TR::Node::create(grandChild, grandChild->getOpCodeValue(), 0,   \
                                (int32_t)konst));                              \
      }                                                                        \
    }                                                                          \
...
template <typename T>
inline void setCCAddSigned(T value, T operand1, T operand2, TR::Node *node,
                           TR::Simplifier *s) {
...
template <typename T>
inline void setCCAddUnsigned(T value, T operand1, TR::Node *node,
                             TR::Simplifier *s) {
...
template <typename T>
inline void setCCSubSigned(T value, T operand1, T operand2, TR::Node *node,
                           TR::Simplifier *s) {
...
template <typename T>
inline void setCCSubUnsigned(T value, T operand1, TR::Node *node,
                             TR::Simplifier *s) {
...
static void convertToTestUnderMask(TR::Node *node, TR::Block *block,
                                   TR::Simplifier *s) {
  // butest evaluator is only implemented on Z
  if (!TR::Compiler->target.cpu.isZ()) return;

  if (debug("disableConvertToTestUnderMask")) return;
  auto cm = s->comp();
  ...
  if ((iand = node->getChild(0))->getOpCodeValue() == TR::iand &&
    ...
    auto bloadi = TR::Node::createWithSymRef(TR::bloadi, 1, load->getChild(0),
                                             0, load->getSymbolReference());
    node->setAndIncChild(
        0, TR::Node::create(
               TR::butest, 2, bloadi,
...
template <typename T>
inline void foldConstant(TR::Node *node, T value, TR::Simplifier *s,
  ...
  *value = 0;
  ...
  if (node->getOpCodeValue() == TR::iadd ||
    ...
    if (isIMulComposerOpCode(secondChild)) {
      ...
      if (!rAnswer) return false;
    } else {
      rMultiplier = secondChild;
      rcount = 1;
    }

    if (lMultiplier != rMultiplier) return false;

    if (!lAnswer && !rAnswer) return false;
    ...
    if (!s->_hashTable.locate(node->getFirstChild()->getGlobalIndex(), index)) {
      ...
      if (base_node != node && base_node->getReferenceCount() >= 1 &&
        ...
        if (!s->comp()->cg()->shouldValueBeInACommonedNode(diff) &&
          ...
          TR::Node *diff_node = TR::Node::create(
              node, node->getSecondChild()->getOpCodeValue(), 0);
...
static TR::Node *addSimplifierCommon(TR::Node *node, TR::Block *block,
                                     TR::Simplifier *s) {
      ...
      if (performTransformation(s->comp(),
        ...
        if (c1->getDataType() != secondChild->getDataType()) {
          TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(
              c1->getDataType(), secondChild->getDataType(),
    ...
    //     / \                / \
      ...
      if (performTransformation(
        ...
        TR::Node *newSecondChild = TR::Node::create(
            secondChild, addOps[secondChild->getDataType()], 2);
...
template <class T>
static TR::Node *addSimplifier(TR::Node *node, TR::Block *block,
                               TR::Simplifier *s) {
  simplifyChildren(node, block, s);

  if (node->getOpCodeValue() == TR::iuaddc) return node;
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      ...
      if (node->nodeRequiresConditionCodes())
        setCCAddSigned<T>(value, firstChild->getConst<T>(),
                          secondChild->getConst<T>(), node, s);
    }
  ...
  TR::Node *identity = 0;
  if (0 != (identity = binaryIdentityOp<T>(node, 0, s))) return identity;
  ...
  if (nodeOp.isAdd() && secondChildOp.isLoadConst() &&
    ...
    // Turn add with first child negated into a subtract
                                   ...
                                   "%sReduced iadd with negated first child in "
                                   "node [%s] to isub\n",
                                   s->optDetailString(),
                                   node->getName(s->getDebug()))) {
      s->anchorChildren(node, s->_curTree);
  ...
  // turn add with other child negated in to a subtract
    ...
    if (performTransformation(
      ...
      node = s->simplify(node, block);
             ...
             imulComposer(node, (int *)&iMulComposerValue,
                          &iMulComposerChild)) {
    ...
    if (s->getLastRun() &&
      ...
      constNode = TR::Node::create(node, TR::ILOpCode::getConstOpCode<T>(), 0,
                                   iMulComposerValue);
  ...
  // reduce imul operations by factoring
    ...
    // node->getName(s->getDebug())))
    {
      ...
      if (!factorChild ||
          !performTransformation(
              s->comp(), "%sFactored iadd with distributed imul in node [%s]\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node *subSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() == TR::iusubb) return node;
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      ...
      if (node->nodeRequiresConditionCodes())
        setCCSubSigned<T>(value, firstChild->getConst<T>(),
                          secondChild->getConst<T>(), node, s);
    }
    return node;
  }
  ...
  if (firstChild == secondChild) {
    if (node->nodeRequiresConditionCodes()) {
      setCCSubSigned<T>(0, firstChild->getConst<T>(),
                        secondChild->getConst<T>(), node, s);
    }
  ...
  // representation
    ...
    if (performTransformation(s->comp(),
                              "%sNormalized isub of iconst > 0 in node [%s] to "
                              "iadd of -iconst \n",
                              s->optDetailString(),
                              node->getName(s->getDebug()))) {
  ...
  // turn bitwise complement + 1 into a 2's complement negate
    ...
    if (performTransformation(s->comp(),
      ...
      s->_alteredBlock = true;
                                                  ...
                                                  // ported to templates yet
             imulComposer(node, (int *)&MulComposerValue,
                          &MulComposerChild))  // TODO
  {
    bool iMulDecomposeReport =
        s->comp()->getOptions()->getTraceSimplifier(TR_TraceMulDecomposition);
    if (iMulDecomposeReport)
  ...
  // reduce imul operations by factoring
    ...
    // node->getName(s->getDebug())))
    {
      ...
      if (!factorChild ||
          !performTransformation(
              s->comp(), "%sFactored xsub with distributed xmul in node [%s]\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
        secondChild->setChild(0, rlChild);
        secondChild->setChild(1, rrChild);
      } else {
        TR::Node::recreate(node, TR::ILOpCode::getMulOpCode<T>());
        node->setChild(factorChild->getOpCode().isLoadConst() ? 1 : 0,
                       factorChild)
            ->decReferenceCount();
...
// reliably
static void normalizeConstantShiftAmount(TR::Node *node, int32_t shiftMask,
                                         TR::Node *&secondChild,
                                         TR::Simplifier *s) {
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (shiftValue != secondChild->getInt()) {
      if (performTransformation(
              s->comp(), "%sReducing constant of node [%s] from %d to %d\n",
              s->optDetailString(), secondChild->getName(s->getDebug()),
              secondChild->getInt(), shiftValue)) {
        if (secondChild->getReferenceCount() > 1) {
...
static void normalizeShiftAmount(TR::Node *node, int32_t normalizationConstant,
                                 TR::Simplifier *s) {
    ...
    if ((secondChild->getOpCodeValue() != TR::iconst) &&
      ...
      if (performTransformation(
        ...
        TR::Node *normalizedNode =
            TR::Node::create(TR::iand, 2, secondChild,
...
static void orderChildrenByHighWordZero(TR::Node *node, TR::Node *&firstChild,
                                        TR::Node *&secondChild,
                                        TR::Simplifier *s) {
  if (!secondChild->getOpCode().isLoadConst() &&
...
static TR::Node *foldDemotionConversion(TR::Node *node, TR::ILOpCodes opcode,
                                        TR::ILOpCodes foldedOpCode,
                                        TR::Simplifier *s) {
  TR::Node *firstChild = node->getFirstChild();

  if (!s->isLegalToFold(node, firstChild)) return NULL;
  ...
  if (firstChild->getOpCodeValue() == opcode &&
      performTransformation(
          ...
          s->optDetailString(), node->getName(s->getDebug()),
          node->getOpCode().getName(), firstChild->getName(s->getDebug()),
...
//
static bool reduceLongOp(TR::Node *node, TR::Block *block, TR::Simplifier *s,
  ...
  if (newOp == TR::BadILOp) return false;
...
static void fold2SmallerIntConstant(TR::Node *node, TR::Node *child,
                                    TR::DataType sdt, TR::DataType tdt,
                                    TR::Simplifier *s) {
  int32_t val;
  switch (sdt) {
    case TR::Int16:
  ...
  switch (tdt) {
    case TR::Int8:
      ...
      break;
    case TR::Int16:
      foldShortIntConstant(node, (val << 16) >> 16, s,
...
static TR::Node *intDemoteSimplifier(TR::Node *node, TR::Block *block,
                                     TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (!decodeConversionOpcode(op, node->getDataType(), sourceDataType,
                              targetDataType))
    return node;
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    fold2SmallerIntConstant(node, firstChild, sourceDataType, targetDataType,
                            s);
    return node;
  }
  ...
  if ((result = s->unaryCancelOutWithChild(node, firstChild, s->_curTree,
                                           inverseOp1)))
    return result;
  ...
  if (sourceIs64Bit &&
      decodeConversionOpcode(childOp, firstChild->getDataType(),
                             childSourceDataType, childTargetDataType) &&
    ...
    uint32_t childSourceSize =
        TR::ILOpCode::getSize(TR::ILOpCode::getProperConversion(
            childTargetDataType, childSourceDataType,
...
static bool hasCommonedChild(TR::Node *node, TR::SparseBitVector &isVisited) {
  ...
  if (node->getReferenceCount() > 1) return true;

  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);

    if (!isVisited[child->getGlobalIndex()]) {
      if (hasCommonedChild(child, isVisited)) return true;
      ;
    }
  }
  return false;
...
static TR::SymbolReference *getUniqueSymRef(TR::Node *node, bool &isLegal,
                                            TR::SparseBitVector &isVisited) {
  TR::ILOpCode &op = node->getOpCode();
  isVisited[node->getGlobalIndex()] = true;
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    ...
    if (!isVisited[child->getGlobalIndex()]) {
      ...
      if (symRef != NULL) {
        if (savedSymRef != NULL && savedSymRef != symRef) isLegal = false;
        savedSymRef = symRef;
      }
...
// 2. TODO: for cmp,
static void partialRedundantCompareElimination(TR::Node *node, TR::Block *block,
                                               TR::Simplifier *s) {
  // return;
  ...
  if (comp->cg()->getGRACompleted()) return;
  ...
  if (!opcode.isBranch() || !opcode.isBooleanCompare()) return;
        ...
        if (gotoTree) {
          ...
          if (predEndWithGoto || isImmediatePred) {
            ...
            if (storeNode->getOpCode().isStoreDirect() &&
              ...
              // num++;
              if (performTransformation(
                      comp, "%sDuplicating %p and insert it after %p\n",
...
static void longCompareNarrower(TR::Node *node, TR::Simplifier *s,
                                TR::ILOpCodes intOp, TR::ILOpCodes charOp,
      ...
      if (firstOp == TR::su2l) {
        if (secondOp == TR::cconst || secondOp == TR::su2l ||
          ...
          if (secondOp == TR::su2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                             "Long Compare Narrower: found both children c2l "
                             "in method %s\n",
                             s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
        ...
        if (secondOp == TR::sconst || secondOp == TR::s2l ||
          ...
          if (secondOp == TR::s2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                             "Long Compare Narrower: found both children s2l "
                             "in method %s\n",
                             s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
        ...
        if (secondOp == TR::bconst || secondOp == TR::b2l ||
          ...
          if (secondOp == TR::b2l) {
            ...
            if (reportCompareDemotions) {
              dumpOptDetails(s->comp(),
                             "Long Compare Narrower: found both children b2l "
                             "in method %s\n",
                             s->comp()->signature());
            }
          } else if (secondOp == TR::lconst) {
...
static void ifjlClassSimplifier(TR::Node *node, TR::Simplifier *s) {
  ...
  if (disableJLClassSimplification) return;
...
static void transformToLongDivBy10Bitwise(TR::Node *origNode, TR::Node *node,
                                          TR::Simplifier *s) {
  ...
  */
  ...
  // n = n + (n>>63 & 9);  (nodes n1, n2, n3)
  TR::Node *n2Node =
      TR::Node::create(TR::lshr, 2, firstChild,
                       TR::Node::create(secondChild, TR::iconst, 0, 63));
  TR::Node *temp = NULL;
  ...
  // r = n - q*10; (nodes r1, r2)
  TR::Node *r2Node =
      TR::Node::create(TR::lmul, 2, q12Node,
                       temp = TR::Node::create(secondChild, TR::lconst, 0));
  temp->setLongInt(10);
...
static int decomposeConstant(char bitPosition[], char operationType[],
                             int64_t value, int wordSize) {
  int32_t count = 0;
  int32_t pos = 0;
...
static TR::Node *generateDecomposedTree(TR::Node *parentNode,
                                        TR::Node *multiplierChild,
                                        TR::Simplifier *s, char bitPosition[],
                                        char operationType[], int from, int to,
                                        int tabspace, bool isLong) {
  int count = to - from;
  int i;
  ...
  if (count <= 2) {
      ...
      if (operationType[from + 1] == OP_MINUS) {
        ...
        if (operationType[from] == OP_MINUS) {
          if (bitPosition[from] == 0) {
            ...
            TR::Node *constNode1 =
                isLong ? (TR::Node::lconst(parentNode, (int64_t)1 << (int32_t)(
                                                           bitPosition[from])))
                       : (TR::Node::iconst(parentNode,
                                           1 << (int32_t)(bitPosition[from])));
            TR::Node *shlNode =
        ...
        if (bitPosition[from + 1] == 0) {
          ...
          TR::Node *constNode2 =
              isLong
                  ? (TR::Node::lconst(parentNode, (int64_t)1 << (int32_t)(
                                                      bitPosition[from + 1])))
                  : (TR::Node::iconst(parentNode,
    ...
    firstChild = generateDecomposedTree(parentNode, multiplierChild, s,
                                        bitPosition, operationType, from,
                                        from + mid, tabspace + 1, isLong);
    secondChild = generateDecomposedTree(parentNode, multiplierChild, s,
                                         bitPosition, operationType, from + mid,
                                         to, tabspace + 1, isLong);
...
static void printTree(TR::Simplifier *s, char bitPosition[],
                      char operationType[], int from, int to, int tabspace,
                      bool isLong) {
  int count = to - from;
  int i;
  ...
  if (count <= 2) {
    ...
    printTree(s, bitPosition, operationType, from, from + mid, tabspace + 1,
              isLong);
    printTree(s, bitPosition, operationType, from + mid, to, tabspace + 1,
              isLong);
  }
  traceMsg(s->comp(), "\n");
}
...
static void decomposeMultiply(TR::Node *node, TR::Simplifier *s, bool isLong) {
  ...
  if ((!s->containingStructure() || s->getLastRun()) &&
      comp->cg()->mulDecompositionCostIsJustified(
          count, bitPosition, operationType,
          isLong ? secondChild->getLongInt() : secondChild->getInt()) &&
...
static void addSimplifierJavaPacked(TR::Node *node, TR::Block *block,
                                    TR::Simplifier *s) {
  return;
}

static void foldAddressConstant(TR::Node *node, int64_t value,
                                TR::Simplifier *s, bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
  ...
  if (!node->getOpCode().isRef()) setIsHighWordZero(node, s);
...
//
TR::Node *getQuotientUsingMagicNumberMultiply(TR::Node *node, TR::Block *block,
                                              TR::Simplifier *s) {
  TR::Node *firstChild = node->getFirstChild(),
           *secondChild = node->getSecondChild();
  ...
  if (node->getOpCodeValue() == TR::idiv ||
    ...
    if (shiftAmount == 0) {
      if (divisor > 0) {
        node4 =
            TR::Node::create(TR::iushr, 2, firstChild,
      ...
      node3 =
          TR::Node::create(TR::ishr, 2, node2,
    ...
    if (shiftAmount == 0) {
      if (divisor > 0) {
        node4 =
            TR::Node::create(TR::lushr, 2, firstChild,
      ...
      node3 =
          TR::Node::create(TR::lshr, 2, node2,
                           TR::Node::create(node2, TR::iconst, 0, shiftAmount));
 ...
 */
static void changeConverts2Unsigned(TR::Node *node, TR::ILOpCodes searchOp,
                                    TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() == searchOp) {
    ...
    if (performTransformation(
            s->comp(), "%sConverted x2i [%s] to unsigned xu2i\n",
            s->optDetailString(), node->getName(s->getDebug()))) {
      TR::Node::recreate(node, newOp);
      return;
    }
...
static void simplifyIntBranchArithmetic(TR::Node *node, TR::Node *&firstChild,
                                        TR::Node *&secondChild,
                                        TR::Simplifier *s) {
  SIMPLIFY_BRANCH_ARITHMETIC(Int, int32_t);
}

static void simplifyLongBranchArithmetic(TR::Node *node, TR::Node *&firstChild,
                                         TR::Node *&secondChild,
                                         TR::Simplifier *s) {
...
//
static bool conditionalBranchFold(int32_t takeBranch, TR::Node *&node,
...
static bool conditionalZeroComparisonBranchFold(TR::Node *&node,
                                                TR::Node *firstChild,
                                                TR::Node *secondChild,
                                                TR::Block *block,
                                                TR::Simplifier *s) {
  ...
  if ((firstChild->isNonZero() || firstChild->isNonNull()) &&
    ...
    s->conditionalToUnconditional(node, block,
                                  !node->getOpCode().isCompareTrueIfEqual());
    return true;
  }
...
//
static TR::Node *binaryNanFloatOp(TR::Node *node, TR::Node *firstChild,
                                  TR::Node *secondChild, TR::Simplifier *s) {
...
// If either child is a NaN replace this node with that child.
//
static TR::Node *binaryNanDoubleOp(TR::Node *node, TR::Node *firstChild,
                                   TR::Node *secondChild, TR::Simplifier *s) {
...
static bool isLegalToMerge(TR::Node *node, TR::Block *block,
                           TR::Block *nextBlock, TR::CFGEdgeList &nextExcpOut,
                           TR::Node *bbStartNode, TR::CFGEdgeList &inEdge,
                           TR::Simplifier *s, bool &blockIsEmpty) {
  TR::CFGEdge *outEdge = block->getSuccessors().front();

  // Skip if there are glRegDeps inbetween
  if (block->getExit()->getNode()->getNumChildren() > 0) return false;
  if (nextBlock->getEntry()->getNode()->getNumChildren() > 0) return false;
  if (nextBlock->isOSRCodeBlock()) return false;
  ...
  //
  if (nextBlock->hasExceptionPredecessors()) return false;

  TR::CFGEdgeList &excpOut = block->getExceptionSuccessors();
  if (excpOut.empty()) {
    if (!nextExcpOut.empty()) return false;
  } else {
    if (excpOut.size() != nextExcpOut.size()) return false;
    for (auto outEdge = excpOut.begin(); outEdge != excpOut.end(); ++outEdge) {
      for (auto nextOutEdge = nextExcpOut.begin();; ++nextOutEdge) {
        if (nextOutEdge == nextExcpOut.end()) return false;
        if ((*nextOutEdge)->getTo() == (*outEdge)->getTo()) break;
      }
    }
  }
  ...
  if (block->getNumber() >= 0 && nextBlock->getNumber()) {
    if (!performTransformation(
            s->comp(), "%sMerge blocks: block_%d and block_%d\n",
...
//
static void changeBranchDestinationsForMergeBlocks(
    TR::Block *block, TR::Block *nextBlock, TR::Node *bbStartNode,
    TR::CFGEdgeList &inEdge, TR::CFGEdgeList::iterator &inEdgeIter,
    TR::Simplifier *s) {
      ...
      if ((*ie)->getFrom()->hasSuccessor(block)) {
        ...
        if ((predBlock->getSuccessors().size() == 2) &&
          ...
          (*ie)
              ...
              ->asBlock()
              ->getLastRealTreeTop()
              ->adjustBranchOrSwitchTreeTop(s->comp(), nextBlock->getEntry(),
                                            block->getEntry());
...
static bool doubleConstIsRepresentableExactlyAsFloat(double dValue,
                                                     float &fValue) {
...
static bool doubleConstIsRepresentableExactlyAsInt(double dValue,
                                                   int32_t &iValue) {
...
static bool doubleConstIsRepresentableExactlyAsLong(double dValue,
                                                    int64_t &lValue) {
...
static bool doubleConstIsRepresentableExactlyAsShort(double dValue,
                                                     int16_t &sValue) {
...
static bool doubleConstIsRepresentableExactlyAsChar(double dValue,
                                                    uint16_t &cValue) {
...
static bool doubleConstIsRepresentableExactlyAsByte(double dValue,
                                                    int8_t &bValue) {
...
static bool floatConstIsRepresentableExactlyAsInt(float fValue,
                                                  int32_t &iValue) {
...
static bool floatConstIsRepresentableExactlyAsLong(float fValue,
                                                   int64_t &lValue) {
...
static bool floatConstIsRepresentableExactlyAsShort(float fValue,
                                                    int16_t &sValue) {
...
static bool floatConstIsRepresentableExactlyAsChar(float fValue,
                                                   uint16_t &cValue) {
...
static bool floatConstIsRepresentableExactlyAsByte(float fValue,
                                                   int8_t &bValue) {
...
static void intCompareNarrower(TR::Node *node, TR::Simplifier *s,
                               TR::ILOpCodes ushortOp, TR::ILOpCodes shortOp,
                               TR::ILOpCodes byteOp) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  static char *reportCompareDemotions = feGetEnv("TR_ReportCompareDemotions");
...
static void unsignedIntCompareNarrower(TR::Node *node, TR::Simplifier *s,
...
// SimplifierTable!
//
static TR::Node *integerToFloatHelper(uint32_t absValue,
                                      bool isUnsignedOperation, TR::Node *node,
                                      TR::Simplifier *s) {
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= 8) {
    ...
    if (isUnsignedOperation)
      ...
      foldFloatConstant(
          node, firstChild->getInt() < 0 ? -(float)absValue : (float)absValue,
          s);
...
static bool containsLoad(TR::Node *node, TR::SymbolReference *symRef,
                         vcount_t visitCount) {
  ...
  for (; i >= 0; i--) {
    TR::Node *child = node->getChild(i);
    if (child->getReferenceCount() == visitCount) continue;
    if (containsLoad(child, symRef, visitCount)) return true;
  }

  return false;
}

static bool canMovePastTree(TR::TreeTop *tree, TR::SymbolReference *symRef,
                            TR::Compilation *c, bool blockHasCatchers) {
  ...
  if (thisNode->getOpCode().isBranch() ||
    ...
    if (definingSymRef == symRef) return false;
...
static bool containsNode(TR::Node *tree, TR::Node *searchNode,
                         vcount_t visitCount) {
  if (tree == searchNode) return true;

  int32_t i = tree->getNumChildren() - 1;
  for (; i >= 0; i--) {
    TR::Node *child = tree->getChild(i);
    if (child->getReferenceCount() == visitCount) continue;
    if (containsNode(child, searchNode, visitCount)) return true;
  }
...
template <typename T>
static bool checkAndReplaceRotation(TR::Node *node, TR::Block *block,
                                    TR::Simplifier *s) {
  static char *disableROLSimplification =
      feGetEnv("TR_DisableROLSimplification");
  if (disableROLSimplification) return false;
  ...
  T expectedMulConst = correctLeftShiftAmount < static_cast<T>(sizeof(T) * 8)
                           ...
                           : 0;
  if (expectedMulConst != mulConst) return false;
  ...
  TR::Node::recreate(
      ...
      TR::ILOpCode::getRotateOpCodeFromDt(
          mulConstNode
              ->getDataType()));  // this line will have to change based on T
...
// Table!
//
static TR::Node *longToFloatHelper(uint64_t absValue, bool isUnsignedOperation,
                                   TR::Node *node, TR::Simplifier *s) {
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (leadingZeroes(absValue) + trailingZeroes(absValue) >= (64 - 24)) {
    ...
    if (isUnsignedOperation)
      ...
      foldFloatConstant(
          node,
          firstChild->getLongInt() < 0 ? -(float)absValue : (float)absValue, s);
    return node;
  }
}
...
//
static TR::Node *longToDoubleHelper(uint64_t absValue, bool isUnsignedOperation,
                                    TR::Node *node, TR::Simplifier *s) {
  TR::Node *firstChild = node->getFirstChild();
          ...
          // round it manually
  {
    ...
    if (isUnsignedOperation)
      ...
      foldDoubleConstant(
          node,
          firstChild->getLongInt() < 0 ? -(double)absValue : (double)absValue,
          s);
    return node;
  }
}
...
static void bitTestingOp(TR::Node *node, TR::Simplifier *s) {
  ...
  if (!firstChild->getOpCode().isBitwiseLogical()) return;
  if (!secondChild->getOpCode().isLoadConst()) return;
  if (!firstChild->getFirstChild()->getOpCode().isLeftShift()) return;
  if (!firstChild->getFirstChild()->getSecondChild()->getOpCode().isLoadConst())
    return;
  if (!firstChild->getSecondChild()->getOpCode().isLoadConst()) return;
  ...
  if ((bitAmount >> shiftAmount) << shiftAmount != bitAmount) return;

  if ((cmpAmount >> shiftAmount) << shiftAmount != cmpAmount) return;

  if (!performTransformation(
          s->comp(), "%sRemoving shift node [" POINTER_PRINTF_FORMAT "] \n",
  ...
  switch (dt) {
    case TR::Int8:
      foldByteConstant(firstChild->getSecondChild(),
                       (bitAmount & 0xFF) >> shiftAmount, s,
                       false /* !anchorChildren */);
      foldByteConstant(secondChild, (cmpAmount & 0xFF) >> shiftAmount, s,
                       false /* !anchorChildren */);
      break;
    case TR::Int16:
      foldShortIntConstant(firstChild->getSecondChild(),
                           (bitAmount & 0xFFFF) >> shiftAmount, s,
                           false /* !anchorChildren */);
      foldShortIntConstant(secondChild, (cmpAmount & 0xFFFF) >> shiftAmount, s,
                           false /* !anchorChildren */);
      break;
    case TR::Int32:
      foldIntConstant(firstChild->getSecondChild(),
                      (bitAmount & 0xFFFFFFFF) >> shiftAmount, s,
                      false /* !anchorChildren */);
      foldIntConstant(secondChild, (cmpAmount & 0xFFFFFFFF) >> shiftAmount, s,
                      false /* !anchorChildren */);
      break;
    case TR::Int64:
      foldLongIntConstant(firstChild->getSecondChild(),
                          bitAmount >> shiftAmount, s,
                          false /* !anchorChildren */);
      foldLongIntConstant(secondChild, cmpAmount >> shiftAmount, s,
                          false /* !anchorChildren */);
      break;
    default:
...
static bool isOperationFPCompliant(TR::Node *parent, TR::Node *arithExprNode,
                                   TR::Simplifier *s) {
  ...
  if (mulNode->getDataType() == TR::Double &&
    ...
    if (mulNode->getFirstChild()->getOpCode().isLong()) {
      if (traceIt)
        traceMsg(s->comp(), "Node: %p fails because conversion type is long\n",
                 mulNode->getFirstChild());
      return false;
    }
    ...
    if (traceIt)
      traceMsg(s->comp(), "Node: %p passes because conversion type is <long\n",
               mulNode);
    return true;
  }
  ...
  if (!mulChildIsConst && s->cg()->supportsOnDemandLiteralPool()) {
    ...
    if (mulChildIsConst ||
      ...
      if (traceIt)
        traceMsg(s->comp(), "LiteralPool load detected:%p points to %p\n",
  ...
  if (!sameCaller) {
    if (traceIt)
      traceMsg(s->comp(), "Node: %p fails because split between two methods \n",
               mulNode);
    return false;
  }
  ...
  if (mulNonConstChild->getOpCode().isConversion()) {
    ...
    switch (mulConstChild->getDataType()) {
      ...
      case TR::Double:  // rely on fact same storage used for longs
        ...
        if (traceIt)
          traceMsg(s->comp(), "Node:%p  bitcount:%d hex:%x%x value:%g\n",
                   mulConstChild, bitcount, mulConstChild->getLongIntHigh(),
                   mulConstChild->getLongIntLow(), mulConstChild->getDouble());
    ...
    if (i2dConv) {
      if (possibleRoundedValue >= FMA_CONST_HIGHBOUNDI2D) {
        if (traceIt)
          traceMsg(s->comp(), "Fails because |constant| %e exceeds %e\n",
      ...
      if (possibleRoundedValue >= FMA_CONST_HIGHBOUNDF2D ||
        ...
        if (traceIt)
          traceMsg(
              s->comp(), "Fails because |constant| %e exceeds range %e-%e \n",
              possibleRoundedValue, FMA_CONST_LOWBOUND, FMA_CONST_HIGHBOUNDF2D);
        return false;
      }
    }
...
//
static void bitwiseToLogical(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  TR::ILOpCodes opCode = node->getOpCodeValue();
  if (opCode != TR::ificmpne && opCode != TR::ificmpeq) return;
  // the first child must be an ior or an iand
  TR::Node *firstChild = node->getFirstChild();
  if (firstChild->getOpCodeValue() != TR::ior &&
      firstChild->getOpCodeValue() != TR::iand)
    return;
  if (firstChild->getReferenceCount() != 1) return;
  if (firstChild->getFirstChild()->getOpCodeValue() != TR::b2i) return;
  if (firstChild->getSecondChild()->getOpCodeValue() != TR::b2i) return;
  TR::Node *cmp1 = firstChild->getFirstChild()->getFirstChild();
  TR::Node *cmp2 = firstChild->getSecondChild()->getFirstChild();
  if (!cmp1->getOpCode().isBooleanCompare()) return;
  if (!cmp2->getOpCode().isBooleanCompare()) return;

  // second child must be int
  TR::Node *secondChild = node->getSecondChild();
  if (secondChild->getOpCodeValue() != TR::iconst) return;
  // second child must be 0 or 1
  int32_t konst = secondChild->getInt();
  if (konst != 0 && konst != 1) return;
  ...
  //
  if (nextBlock->isExtensionOfPreviousBlock()) return;
  ...
  if (node->getOpCodeValue() == TR::ificmpne) isZero = !isZero;
  ...
  prevTree =
      TR::TreeTop::create(s->comp(), prevTree,
                          TR::Node::createStore(temp2, cmp2->getSecondChild()));
  ...
  TR::Node *newCmp = TR::Node::createif(
      newOp, TR::Node::createLoad(cmp2->getFirstChild(), temp1),
...
TR::Node *lstoreSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  return node;
}

TR::Node *directLoadSimplifier(TR::Node *node, TR::Block *block,
                               TR::Simplifier *s) {
  simplifyChildren(node, block,
                   s);  // For all those direct loads with child nodes...
  TR::TransformUtil::transformDirectLoad(s->comp(), node);
  return node;
}

TR::Node *indirectLoadSimplifier(TR::Node *node, TR::Block *block,
                                 TR::Simplifier *s) {
  ...
  if (firstChild->getOpCodeValue() == TR::loadaddr) {
    ...
    if (addrDataType == TR::Aggregate && loadSize != addrSize) isSame = false;
    ...
    if (isSame &&
      ...
      TR::Node::recreateWithoutProperties(
          loadNode, s->comp()->il.opCodeForDirectLoad(addrDataType), 0,
          childRef);
      ...
      dumpOptDetails(s->comp(),
                     ...
                     loadNode->getOpCode().getName(), loadNode);
...
TR::Node *indirectStoreSimplifier(TR::Node *node, TR::Block *block,
                                  TR::Simplifier *s) {
  ...
  if (firstChild->getOpCodeValue() == TR::loadaddr) {
    ...
    if (newOType)
      allowStoreSize =
          (storeSize == storeSymSize) && (storeSize == 1 || storeSize == 2 ||
                                          storeSize == 4 || storeSize == 8);
    else
  ...
  if ((node->getOpCodeValue() == TR::awrtbari) && 0 &&
    ...
    if ((valueChild->getOpCodeValue() == TR::New) ||
      ...
      while (cursor) {
        ...
        if (cursorNode->getOpCodeValue() == TR::BBStart) break;
        ...
        if (cursorNode->canGCandReturn()) seenGCPoint = true;
...
TR::Node *astoreSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *directStoreSimplifier(TR::Node *node, TR::Block *block,
                                TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  // quality improvement
    ...
    if ((firstLoadsSymRef && secondIsLoadConst) ||
      ...
      while (updateTree->getNode() != node) {
        ...
        if (updateTree == block->getExit()) return node;
      }
      ...
      if (loadTree->getNode()->getOpCodeValue() == TR::treetop) {
        ...
        if (loadNode->getOpCode().isLoadVarDirect() &&
          ...
          while (walkingTree != block->getExit()) {
            if (!canMovePastTree(walkingTree, symRef, s->comp(),
                                 blockHasCatchers))
              break;

            if (containsNode(walkingTree->getNode(), loadNode,
                             s->comp()->getVisitCount()))
...
TR::Node *vsetelemSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  ...
  for (TR::Node *vsetelemNode = node;
    ...
    if (!vsetelemNode->getSecondChild()->getOpCode().isLoadConst()) break;
    ...
    if (valueNode != vsetelemNode->getThirdChild()) break;
...
TR::Node *ifdCallSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *treetopSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  // The treetop node should have a single child.
  //
  TR_ASSERT(node->getNumChildren() == 1, "Simplifier, bad treetop node %p",
            node);
  TR::Node *child = node->getFirstChild();
...
TR::Node *anchorSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (!s->comp()->useAnchors()) return node;
...
TR::Node *iaddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->chkClassPointerConstant()) return node;
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      ...
      if (node->nodeRequiresConditionCodes())
        setCCAddSigned(value, firstChild->getInt(), secondChild->getInt(), node,
                       s);
    }
    return node;
  }
  ...
  if (nodeOp == TR::iadd && secondChildOp == TR::iconst &&
    ...
    if (performTransformation(
      ...
      TR::Node::recreateWithoutProperties(node, TR::isub, 2, firstChild,
                                          newSecondChild);
      firstChild->recursivelyDecReferenceCount();
      secondChild->recursivelyDecReferenceCount();
      node->setVisitCount(0);
  ...
  // reduce imul operations by factoring
    ...
    if ((node_aiadd->getSecondChild()->getOpCodeValue() == TR::iconst) &&
        ...
        performTransformation(s->comp(),
                              ...
                              "[%p] iconst [%p] below\n",
                              s->optDetailString(), node,
                              node->getSecondChild(), node_aiadd,
      ...
      if (passesPrecisionTest &&
          performTransformation(
              s->comp(),
              "%sPushing down iadd [%p] iconst [%p] into aiadd [%p] second "
              "child [%p], creating new iadd\n",
              s->optDetailString(), node, node->getSecondChild(), node_aiadd,
              node_aiadd->getSecondChild())) {
...
TR::Node *laddSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() == TR::luaddc) return node;
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->nodeRequiresConditionCodes()) {
      setCCAddSigned(firstChild->getLongInt() + secondChild->getLongInt(),
                     firstChild->getLongInt(), secondChild->getLongInt(), node,
                     s);
    }
    ...
    if (secondChildOp == TR::lconst) {
              ...
              // together
      {
        if ((firstChild->getReferenceCount() == 1) &&
            performTransformation(
                s->comp(),
                "%sFound ladd of non-lconst with ladd or lsub of x and lconst "
    ...
    if ((lrChild->getOpCodeValue() == TR::lconst) &&
      ...
      if (performTransformation(s->comp(),
        ...
        s->_alteredBlock = true;
               ...
               performTransformation(
                   s->comp(),
                   "%sFound aladd of non-lconst with aladd x and lconst in "
                   "node [" POINTER_PRINTF_FORMAT "]\n",
                   s->optDetailString(), node)) {
...
TR::Node *isubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      ...
      if (node->nodeRequiresConditionCodes())
        setCCSubSigned(value, firstChild->getInt(), secondChild->getInt(), node,
                       s);
    }
    return node;
  }
  ...
  // reduce imul operations by factoring
    ...
    // node->getName(s->getDebug())))
    {
      ...
      if (!factorChild ||
          !performTransformation(
              s->comp(), "%sFactored isub with distributed imul in node [%s]\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
        secondChild->setChild(0, rlChild);
        secondChild->setChild(1, rrChild);
      } else {
   ...
   */
    ...
    if (LBase && RBase && LBase == RBase &&
        performTransformation(s->comp(),
                              "%sRemove loadaddr in address computation in "
                              "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(), node)) {
      if (!exprA) exprA = TR::Node::createConstZeroValue(NULL, TR::Int32);
      if (!exprB) exprB = TR::Node::createConstZeroValue(NULL, TR::Int32);
...
TR::Node *lsubSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() == TR::lusubb) return node;
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldLongIntConstant(node,
                        firstChild->getLongInt() - secondChild->getLongInt(), s,
                        false /* !anchorChildren */);

    return node;
  }

  if (!node->nodeRequiresConditionCodes()) BINARY_IDENTITY_OP(LongInt, 0L)
    ...
    if (secondChildOp == TR::lconst) {
              ...
              // together
      {
        if ((firstChild->getReferenceCount() == 1) &&
            performTransformation(
                s->comp(),
                "%sFound lsub of non-lconst with ladd or lsub of x and lconst "
                "in node [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), node)) {
          node->setChild(1, lrChild);
          firstChild->setChild(1, secondChild);
   ...
   */
    ...
    if (LBase && RBase && LBase == RBase &&
        performTransformation(s->comp(),
                              ...
                              "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(), node)) {
      if (!exprA) exprA = TR::Node::createConstZeroValue(NULL, TR::Int64);
      if (!exprB) exprB = TR::Node::createConstZeroValue(NULL, TR::Int64);
      node->setAndIncChild(0, exprA);
      node->setAndIncChild(1, exprB);
      firstChild->recursivelyDecReferenceCount();
      secondChild->recursivelyDecReferenceCount();
...
TR::Node *imulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      foldUIntConstant(
          node, firstChild->getUnsignedInt() * secondChild->getUnsignedInt(), s,
          false /* !anchorChildren*/);
    } else {
      foldIntConstant(node, firstChild->getInt() * secondChild->getInt(), s,
                      false /* !anchorChildren*/);
    }
    return node;
  }
  ...
  if (firstChildOp == TR::imul && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        ...
        if (performTransformation(
                s->comp(), "%sFound (a*c1)*c2 => a*(c1*c2) in imul [%s]\n",
                s->optDetailString(), node->getName(s->getDebug()))) {
          if (secondChild->getReferenceCount() == 1) {
      ...
      // (a*c1)*b => (a*b)*c1
                   ...
                   s->comp(), "%sFound (a*c1)*b => (a*b)*c1 in imul [%s]\n",
                   s->optDetailString(), node->getName(s->getDebug()))) {
        // move constants up the tree so they will tend to get merged together
        node->setChild(1, lrChild);
        firstChild->setChild(1, secondChild);
  ...
  // accesses using indices which differ only by constants, ex. a[i] + a[i+2]
    ...
    if (lrChild->getOpCodeValue() == TR::iconst &&
        performTransformation(
            s->comp(), "%sFound (a+c1)*c2 = a*c2 + c1*c2 in imul [%s]\n",
            s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node *lmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChildOp == TR::lmul && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::lconst &&
      ...
      if (firstChildOp == TR::lsub) product = -product;
      TR::Node *productNode = secondChild;
  ...
  // the simplification is then invalid
    ...
    if (((i2lNode->getFirstChild()->getOpCodeValue() == TR::isub) ||
      ...
      if (lrChild->getOpCodeValue() == TR::iconst &&
        ...
        if (firstChildOp == TR::isub) product = -product;
        TR::Node *productNode = secondChild;
        int64_t scale = secondChild->getLongInt();
        if (secondChild->getReferenceCount() != 1) {
          productNode = TR::Node::create(secondChild, TR::lconst, 0);
...
TR::Node *fmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (bothChildrenNegated) {
    if (performTransformation(s->comp(),
      ...
      TR::Node *newSecondChild = s->replaceNode(
          secondChild, secondChild->getFirstChild(), s->_curTree);
...
TR::Node *bmulSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldByteConstant(node, firstChild->getByte() * secondChild->getByte(), s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *idivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() == TR::iudiv) {
    if (!node->getChild(0)->isNonNegative()) return node;
    if (!node->getChild(1)->isNonNegative()) return node;
  }
      ...
      if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCode().isUnsigned()) {
          ...
          if (divisor == -1 && dividend == TR::getMinSigned<TR::Int32>())
            return s->replaceNode(node, firstChild, s->_curTree);
          foldIntConstant(node, dividend / divisor, s,
                          false /* !anchorChildren*/);
        }
        ...
        if (performTransformation(
                s->comp(), "%sReduced idiv by -1 with ineg in node [%s]\n",
                s->optDetailString(), node->getName(s->getDebug()))) {
...
TR::Node *ldivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
    ...
    if (divisor != 0) {
      if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (divisor == -1 && dividend == TR::getMinSigned<TR::Int64>())
          return s->replaceNode(node, firstChild, s->_curTree);
        foldLongIntConstant(node, dividend / divisor, s,
                            false /* !anchorChildren */);
      } else if (divisor == 1)
        return s->replaceNode(node, firstChild, s->_curTree);
      else if (divisor == -1) {
        ...
        while ((value = (value >> 1))) ++shiftAmount;
          ...
          if (divisor < 0) {
            ...
            if (performTransformation(s->comp(),
              ...
              if (s->cg()->canUseImmedInstruction(divisor - 1)) {
                ...
                node_sign = TR::Node::create(TR::lshr, 2, node_shl,
                                             TR::Node::iconst(firstChild, 63));
  ...
  if (node->getOpCodeValue() == TR::ldiv) {
    ...
    if ((firstChild->getOpCodeValue() == TR::i2l) &&
      ...
      TR::Node *divNode =
          TR::Node::create(TR::idiv, 2, firstChild->getFirstChild(),
                           secondChild->getFirstChild());
...
TR::Node *sdivSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldShortIntConstant(
        node, (int16_t)(firstChild->getShortInt() / secondChild->getShortInt()),
        s, false /* !anchorChildren */);
    return node;
  }
...
TR::Node *lremSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (node->getOpCodeValue() == TR::lrem) {
    ...
    if ((firstChild->getOpCodeValue() == TR::i2l) &&
      ...
      TR::Node *remNode =
          TR::Node::create(TR::irem, 2, firstChild->getFirstChild(),
                           secondChild->getFirstChild());
...
TR::Node *inegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (opCode == TR::ineg) {
    if (performTransformation(s->comp(),
                              "%sCancelled out ineg with ineg child in node "
                              "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(), node)) {
      TR::Node *grandChild = firstChild->getFirstChild();
...
TR::Node *lnegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, -firstChild->getLongInt(), s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *fnegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCodeValue() == TR::fneg) {
    if (performTransformation(
            ...
            s->optDetailString(), node)) {
      node = s->replaceNode(node, firstChild->getFirstChild(),
                            s->_curTree);  // remove neg
    }
  }
...
TR::Node *bnegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(node, -firstChild->getByte(), s,
                     false /* !anchorChildren*/);
  }
...
TR::Node *snegSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node, -firstChild->getShortInt(), s,
                         false /* !anchorChildren */);
  }
  return node;
}
...
TR::Node *lconstSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  setIsHighWordZero(node, s);
  return node;
}

TR::Node *ilfdabsSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (child->isNonNegative() && performFlagBasedTransformation &&
    ...
    return s->replaceNodeWithChild(node, child, s->_curTree, block);
             ...
             performTransformation(s->comp(),
                                   "%sSimplify abs of non-positive child at "
                                   "[" POINTER_PRINTF_FORMAT "]\n",
                                   s->optDetailString(), node)) {
...
TR::Node *ishlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(
        node, firstChild->getInt() << (secondChild->getInt() & INT_SHIFT_MASK),
        s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *bshlSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldByteConstant(
        node, firstChild->getByte() << (secondChild->getInt() & INT_SHIFT_MASK),
        s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *lshrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldLongIntConstant(
        node,
        firstChild->getLongInt() >> (secondChild->getInt() & LONG_SHIFT_MASK),
        s, false /* !anchorChildren */);
    return node;
  }
...
TR::Node *sshrSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldShortIntConstant(
        node,
        firstChild->getShortInt() >> (secondChild->getInt() & INT_SHIFT_MASK),
        s, false /* !anchorChildren */);
    return node;
  }
  BINARY_IDENTITY_OP(Int, 0)
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseIntComplement(firstChild) &&
      ...
      if (performTransformation(s->comp(),
                                ...
                                node->getName(s->getDebug()))) {
        TR::Node *orNode =
            TR::Node::create(TR::ior, 2, firstChild->getFirstChild(),
                             secondChild->getFirstChild());
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) &&
      ...
      if (performTransformation(s->comp(),
                                ...
                                s->optDetailString(), node)) {
        TR::Node *orNode =
            TR::Node::create(TR::lor, 2, firstChild->getFirstChild(),
                             secondChild->getFirstChild());
        TR::Node *constNode = firstChild->getSecondChild();
        TR::Node::recreate(node, TR::lxor);
        node->setAndIncChild(0, orNode);
    ...
    // look for opportunities to perform zero extending conversions
      ...
      if (maskValue == 255 && firstChildOp == TR::b2i) {
        ...
        if (performTransformation(s->comp(),
                                  "%sReduced land with lconst 65536 in node "
                                  "[" POINTER_PRINTF_FORMAT "] to su2l\n",
                                  s->optDetailString(), node)) {
          TR::Node::recreate(node, TR::su2l);
          foundZeroExtension = true;
        }
...
bool isChildOrConstRedundant(TR::Node *parentOrConstNode,
                             TR::Node *childOrConstNode, TR::Simplifier *s) {
...
//    lconst 0xF0F0F0
//
TR::Node *removeRedundantIntegralOrPattern1(TR::Node *node, TR::Block *block,
                                            TR::Simplifier *s) {
  ...
  if (!parentOr->getSecondChild()->getOpCode().isLoadConst()) return firstChild;
  TR::Node *parentOrConstNode = parentOr->getSecondChild();

  if (!childOr->getSecondChild()->getOpCode().isLoadConst()) return firstChild;
  ...
  if (!performTransformation(
          ...
          s->optDetailString(), childOr->getOpCode().getName(),
          childOrConstNode->get64bitIntegralValueAsUnsigned(), childOr,
          parentOr->getOpCode().getName(),
          parentOrConstNode->get64bitIntegralValueAsUnsigned(), parentOr)) {
    return firstChild;
  }
...
//    iconst 0xF0F0F0F0
//
TR::Node *removeRedundantIntegralOrPattern2(TR::Node *node, TR::Block *block,
                                            TR::Simplifier *s) {
  TR::Node *parentOr = node;
  TR::Node *firstChild = parentOr->getFirstChild();
  ...
  if (!zeroExtConv->getFirstChild()->getOpCode().isOr()) return firstChild;
  ...
  // a refCount=1
  TR::Node *newFirstChild = TR::Node::create(zeroExtConv->getOpCodeValue(), 1,
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->getOpCode().isUnsigned()) {
      uint32_t value =
          firstChild->getUnsignedInt() | secondChild->getUnsignedInt();
      foldUIntConstant(node, value, s, false /*!anchorChildren*/);
      if (node->nodeRequiresConditionCodes()) setCCOr(value, node, s);
    } else {
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseIntComplement(firstChild) &&
      ...
      if (performTransformation(s->comp(),
                                ...
                                s->optDetailString(), node)) {
        TR::Node *andNode =
            TR::Node::create(TR::iand, 2, firstChild->getFirstChild(),
                             secondChild->getFirstChild());
        TR::Node *constNode = firstChild->getSecondChild();
        TR::Node::recreate(node, TR::ixor);
  ...
  if (firstChild->getReferenceCount() == 1) {
    if (isBitwiseLongComplement(firstChild) &&
      ...
      if (performTransformation(s->comp(),
                                ...
                                s->optDetailString(), node)) {
        TR::Node *andNode =
            TR::Node::create(TR::land, 2, firstChild->getFirstChild(),
                             secondChild->getFirstChild());
        TR::Node *constNode = firstChild->getSecondChild();
        TR::Node::recreate(node, TR::lxor);
        node->setAndIncChild(0, andNode);
  ...
  if (node->getOpCodeValue() == TR::lor &&
    ...
    if (secondChild->isHighWordZero() &&
      ...
      if (firstChildOp == TR::iu2l &&
        ...
        TR::Node *iorChild = TR::Node::create(
            TR::ior, 2, firstChild->getFirstChild(), constChild);
        TR::Node::recreate(node, firstChildOp);
        node->setNumChildren(1);
...
TR::Node *borSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (node->nodeRequiresConditionCodes()) setCCOr(byte, node, s);
    return node;
  }
...
TR::Node *ixorSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild == secondChild) {
    if (firstChild->getOpCode().isUnsigned())
      ...
      foldIntConstant(node, 0, s, true /* !anchorChildren*/);
    if (node->nodeRequiresConditionCodes()) setCCOr(0, node, s);
    return node;
  }
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    if (firstChild->getOpCode().isUnsigned())
      foldUIntConstant(node, firstChild->getInt() ^ secondChild->getInt(), s,
                       false /* !anchorChildren*/);
    else
      foldIntConstant(node, firstChild->getInt() ^ secondChild->getInt(), s,
                      false /* !anchorChildren*/);
  ...
  if (node->nodeRequiresConditionCodes()) return node;
  ...
  if (firstChildOp == TR::ixor && firstChild->getReferenceCount() == 1) {
    ...
    if (lrChild->getOpCodeValue() == TR::iconst) {
      if (secondChildOp == TR::iconst) {
        if (performTransformation(s->comp(),
                                  "%sFound ixor of iconst with ixor of x and "
                                  "iconst in node [%s]\n",
                                  s->optDetailString(),
                                  node->getName(s->getDebug()))) {
          if (secondChild->getReferenceCount() == 1) {
...
//
template <class T>
inline TR::Node *sqrtSimplifier(TR::Node *node, TR::Block *block,
                                TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (child->getOpCode().isLoadConst() &&
      performTransformation(
          ...
          s->optDetailString(), node)) {
    foldConstant<T>(node,
                    TR::Compiler->arith.fpSquareRoot(child->getConst<T>()), s,
                    false /* !anchorChilren */);
  }
  return node;
}
...
TR::Node *i2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, firstChild->getInt(), s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *i2fSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCodeValue() == TR::iconst) {
    uint32_t absValue = firstChild->getInt() >= 0 ? firstChild->getInt()
                                                  : -firstChild->getInt();
...
TR::Node *i2sSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node, (int16_t)firstChild->getInt(), s,
                         false /* !anchorChildren */);
    return node;
  }
...
TR::Node *i2aSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    ...
    foldAddressConstant(node, firstChild->getInt(), s,
                        false /* !anchorChildren */);
    return node;
  ...
  if ((firstChild->getOpCodeValue() == TR::isub ||
      ...
      (s->comp()->cg()->supports32bitAiadd() ||
       node->isNonNegative()) &&  // todo: do we know NonNegativity in
                                  // simplifier?
      firstChild->getFirstChild() &&
...
TR::Node *iu2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, firstChild->getUnsignedInt(), s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *l2aSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    uint8_t addrPr = (TR::Compiler->target.is64Bit() ? 8 : 4);
    foldAddressConstant(node, firstChild->getLongInt() & 0xFFFFFFFF, s,
                        false /* !anchorChildren */);
    return node;
  } else if (firstChild->getOpCode()
                 .isConversion())  // remove redundant conversions
  ...
  if ((firstChild->getOpCodeValue() == TR::lsub ||
    ...
    if (!(addressAddOp == TR::aladd &&
        ...
        performTransformation(s->comp(),
                              ...
                              s->optDetailString(), node->getOpCode().getName(),
                              node->getName(s->getDebug()))) {
...
TR::Node *f2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, floatToInt(firstChild->getFloat(), false), s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *f2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, floatToLong(firstChild->getFloat(), false), s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *f2bSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(node,
                     (floatToInt(firstChild->getFloat(), false) << 24) >> 24, s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *f2sSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(
        node, (floatToInt(firstChild->getFloat(), false) << 16) >> 16, s,
        false /* !anchorChildren */);
    return node;
  }
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    if (node->getOpCode().isUnsigned()) {
      if (firstChild->getDouble() > 0.0) {
        foldUIntConstant(node, doubleToInt(firstChild->getDouble(), false), s,
                         false /* !anchorChildren */);
      }
    } else {
      foldIntConstant(node, doubleToInt(firstChild->getDouble(), false), s,
                      false /* !anchorChildren */);
    }
    return node;
  }
...
TR::Node *d2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, doubleToLong(firstChild->getDouble(), false), s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *d2sSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(
        node, (doubleToInt(firstChild->getDouble(), false) << 16) >> 16, s,
        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *b2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, firstChild->getByte(), s,
                        false /* !anchorChildren */);
  }
  return node;
}
...
TR::Node *b2sSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node, firstChild->getByte(), s,
                         false /* !anchorChildren */);
  }
  return node;
}
...
TR::Node *bu2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getUnsignedByte(), s,
                    false /* !anchorChildren*/);
    return node;
  }
  ...
  if (firstChild->getOpCodeValue() == TR::i2b &&
    ...
    if (((val & 0xFFUL) == 0UL) &&
        performTransformation(s->comp(),
                              ...
                              firstChild->getFirstChild(), val,
                              TR::getMaxUnsigned<TR::Int8>())) {
      // if we're going to zero extend a zero, then we can just replace this
      // entire expression with a 0
      s->anchorNode(
          firstChild->getFirstChild()->getFirstChild(),
          s->_curTree);  // only really need to anchor this child, everything
                         // else is guaranteed to not be a load
      ...
      bool anchorChildren =
          false;  // no loads are being removed (just bu2i/i2b) so there is
                  // nothing that needs anchoring
...
TR::Node *bu2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, firstChild->getUnsignedByte(), s,
                        false /* !anchorChildren */);
    return node;
  }
  ...
  if (firstChild->getOpCodeValue() == TR::i2b &&
    ...
    if (shiftBy >= 56 &&
        performTransformation(s->comp(),
                              "%sReplace bu2l/l2b of lushr with lushr node "
                              "[" POINTER_PRINTF_FORMAT "]\n",
                              s->optDetailString(), node)) {
...
TR::Node *bu2sSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldShortIntConstant(node, firstChild->getUnsignedByte(), s,
                         false /* !anchorChildren */);
  }
  return node;
}
...
TR::Node *s2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getShortInt(), s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *s2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, firstChild->getShortInt(), s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *s2bSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldByteConstant(node, (int8_t)firstChild->getShortInt(), s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *su2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getUnsignedShortInt(), s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *su2lSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldLongIntConstant(node, firstChild->getUnsignedShortInt(), s,
                        false /* !anchorChildren */);
    return node;
  }
...
TR::Node *ificmpeqSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (conditionalZeroComparisonBranchFold(node, firstChild, secondChild, block,
                                          s))
    return node;
  ...
  if (s->getLastRun()) convertToTestUnderMask(node, block, s);
...
TR::Node *ificmpneSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
...
TR::Node *ificmpltSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (node->getOpCodeValue() == TR::ificmplt) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold(
            (originalFirst->getInt() < originalSecond->getInt()), node,
            firstChild, secondChild, block, s))
      return node;
    ...
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold((originalFirstVal < originalSecondVal), node,
                              firstChild, secondChild, block, s))
      return node;
...
TR::Node *ificmpleSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
...
TR::Node *ificmpgtSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
...
TR::Node *ificmpgeSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (ifxcmpReducer.isReducible()) node = ifxcmpReducer.reduce();
...
TR::Node *iflcmpeqSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpeq) {
    longCompareNarrower(node, s, TR::ificmpeq, TR::ifsucmpeq, TR::ifscmpeq,
                        TR::ifbcmpeq);
  }
...
TR::Node *iflcmpneSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpne) {
    longCompareNarrower(node, s, TR::ificmpne, TR::ifsucmpne, TR::ifscmpne,
                        TR::ifbcmpne);
  }
  addressCompareConversion(node, s);
  removeArithmeticsUnderIntegralCompare(node, s);
...
TR::Node *iflcmpltSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmplt) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold(
            ...
            firstChild, secondChild, block, s))
      return node;
    longCompareNarrower(node, s, TR::ificmplt, TR::ifsucmplt, TR::ifscmplt,
                        TR::ifbcmplt);
...
TR::Node *iflcmpleSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmple) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold(
            (originalFirst->getLongInt() <= originalSecond->getLongInt()), node,
            firstChild, secondChild, block, s))
      return node;
    longCompareNarrower(node, s, TR::ificmple, TR::ifsucmple, TR::ifscmple,
                        TR::ifbcmple);
...
TR::Node *iflcmpgtSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpgt) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold(
            ...
            firstChild, secondChild, block, s))
      return node;
    longCompareNarrower(node, s, TR::ificmpgt, TR::ifsucmpgt, TR::ifscmpgt,
                        TR::ifbcmpgt);
...
TR::Node *iflcmpgeSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  ...
  if (node->getOpCodeValue() == TR::iflcmpge) {
    if (firstChild->getOpCode().isLoadConst() &&
        conditionalBranchFold(
            ...
            firstChild, secondChild, block, s))
      return node;
    longCompareNarrower(node, s, TR::ificmpge, TR::ifsucmpge, TR::ifscmpge,
                        TR::ifbcmpge);
...
TR::Node *normalizeCmpSimplifier(TR::Node *node, TR::Block *block,
                                 TR::Simplifier *s) {
  if (node->getOpCode().isBranch() &&
      (removeIfToFollowingBlock(node, block, s) == NULL))
    return NULL;
...
TR::Node *ifacmpeqSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCodeValue() == TR::aconst &&
      conditionalBranchFold(
          (firstChild->getAddress() == secondChild->getAddress()), node,
          firstChild, secondChild, block, s))
    return node;
  ...
  if (!(firstChild->getOpCode().hasSymbolReference() &&
      ...
      conditionalZeroComparisonBranchFold(node, firstChild, secondChild, block,
                                          s))
    return node;
...
//
TR::Node *ifacmpneSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  simplifyChildren(node, block, s);
...
TR::Node *ifCmpWithEqualitySimplifier(TR::Node *node, TR::Block *block,
                                      TR::Simplifier *s) {
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    if (foldBranch && conditionalBranchFold(takeBranch, node, firstChild,
                                            secondChild, block, s)) {
      return node;
    }
  }
...
TR::Node *ifCmpWithoutEqualitySimplifier(TR::Node *node, TR::Block *block,
                                         TR::Simplifier *s) {
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  simplifyChildren(node, block, s);
...
TR::Node *icmpeqSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *icmpneSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *icmpltSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *icmpleSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *icmpgtSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *icmpgeSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lcmpeqSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getLongInt() == secondChild->getLongInt(),
                    s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *lcmpneSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lcmpltSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lcmpleSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lcmpgtSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lcmpgeSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lucmpltSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldIntConstant(
        node,
        firstChild->getUnsignedLongInt() < secondChild->getUnsignedLongInt(), s,
        false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *lucmpleSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldIntConstant(
        node,
        firstChild->getUnsignedLongInt() <= secondChild->getUnsignedLongInt(),
        s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *lucmpgtSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lucmpgeSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *acmpeqSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *acmpneSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *bcmpeqSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
      secondChild->getOpCode().isLoadConst()) {
    foldIntConstant(node, firstChild->getByte() == secondChild->getByte(), s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *bcmpneSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *bcmpltSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *bcmpleSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *bcmpgtSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *bcmpgeSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *scmpeqSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldIntConstant(node,
                    firstChild->getShortInt() == secondChild->getShortInt(), s,
                    false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *scmpneSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *scmpltSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *scmpleSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *scmpgtSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *scmpgeSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *sucmpeqSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldIntConstant(
        node,
        firstChild->getConst<uint16_t>() == secondChild->getConst<uint16_t>(),
        s, false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *sucmpneSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *sucmpltSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    foldIntConstant(
        node,
        firstChild->getConst<uint16_t>() < secondChild->getConst<uint16_t>(), s,
        false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *sucmpgeSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *sucmpgtSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *sucmpleSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *passThroughSimplifier(TR::Node *node, TR::Block *block,
                                TR::Simplifier *s) {
...
TR::Node *endBlockSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  TR_ASSERT(node->getBlock() == block,
            "Simplification, BBEnd %s points to block_%d (should be %d)",
            s->comp()->getDebug()->getName(node), node->getBlock()->getNumber(),
            block->getNumber());

  if (s->comp()->getProfilingMode() == JitProfiling) return node;

  // See if this block has a single successor by looking at the CFG
  //
  if (block->getSuccessors().size() != 1) return node;
  ...
  if (bbStartTree == NULL) return node;
  ...
  // middle of merging
  bool blockIsEmpty;
  if (!isLegalToMerge(node, block, nextBlock, nextExcpOut, bbStartNode, inEdge,
                      s, blockIsEmpty))
    return node;

  if (moreThanOnePred)
    changeBranchDestinationsForMergeBlocks(block, nextBlock, bbStartNode,
                                           inEdge, inEdgeIter, s);
  ...
  if (cfg) {
    ...
    for (auto outEdge = nextBlock->getSuccessors().begin();
         outEdge != nextBlock->getSuccessors().end(); ++outEdge) {
  ...
  if (nextBlock->hasCalls()) block->setHasCalls(true);
  if (nextBlock->hasCallToSuperCold()) block->setHasCallToSuperCold(true);
...
TR::Node *ternarySimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *a2iSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    if (firstChild->getType().isAddress())
      foldIntConstant(node, firstChild->getAddress(), s,
                      false /* !anchorChildren */);
    else
      foldIntConstant(node, firstChild->get64bitIntegralValue(), s,
                      false /* !anchorChildren */);
  } else {
    if (firstChild->isNonNull()) node->setIsNonZero(true);
  }
  return node;
}
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    if (firstChild->getType().isAddress()) {
      ...
      if (TR::Compiler->target.cpu.isZ() &&
        ...
        foldLongIntConstant(node, firstChild->getAddress() & CLEARBIT32, s,
                            false /* !anchorChildren */);
      else
        foldLongIntConstant(node, firstChild->getAddress(), s,
                            false /* !anchorChildren */);
    } else
      foldLongIntConstant(node, firstChild->get64bitIntegralValue(), s,
                          false /* !anchorChildren */);
  } else {
    if (firstChild->isNonNull()) node->setIsNonZero(true);
  }
...
//
TR::Node *switchSimplifier(TR::Node *node, TR::Block *block, bool isTableSwitch,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    ...
    TR::CFG *cfg = s->comp()->getFlowGraph();
    if (cfg) {
      TR_BitVector removedTargets(cfg->getNumberOfNodes(),
                                  s->comp()->trMemory(), stackAlloc, growable);
      for (caseIndex = numCases + 1; caseIndex >= 1; caseIndex--) {
    ...
    if (node->getOpCode().getOpCodeValue() == TR::lookup && NULL != shiftNode &&
      ...
      if (NULL != shiftAmtNode && shiftAmtNode->getOpCode().isLoadConst()) {
        ...
        for (int i = 0; i < shiftAmount; i++) lowNBits = (lowNBits << 1) + 1;
        ...
        if (NULL != andNode && andNode->getOpCode().isAnd() &&
          ...
          // instead
          if (isLeftShift) shiftConst = highNBits64;

          if ((shiftConst & andConst) != 0) canTransform = false;
        } else
          canTransform = false;
  ...
  if (convertToGoto) {
    ...
    if (!performTransformation(
            s->comp(),
            "%sChanging node [" POINTER_PRINTF_FORMAT "] %s into goto\n",
         ...
         childIndex++)  // loop from default to last case
    {
      ...
      if (destInst->getOpCodeValue() == TR::Goto) {
        ...
        if (!performTransformation(
                ...
                s->optDetailString(), node, childIndex, destBlock->getNumber(),
        ...
        if (cfg) {
          ...
          for (otherChildIndex = 1; otherChildIndex < numChildren;
               ++otherChildIndex)  // Look for another case/default that leads
                                   // to destBlock.
            if (otherChildIndex !=
          ...
          if (otherChildIndex == numChildren)  // If another case/default to
                                               // destBlock wasn't found,
            blocksWereRemoved |=
...
static void foldUnsignedLongIntConstant(TR::Node *node, uint64_t value,
                                        TR::Simplifier *s,
                                        bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
...
inline OMR::TR_ConditionCodeNumber calculateSignedCC(int64_t result,
                                                     bool overflow) {
...
inline OMR::TR_ConditionCodeNumber calculateUnsignedCC(uint64_t result,
                                                       bool carryNotBorrow) {
...
static void foldUByteConstant(TR::Node *node, uint8_t value, TR::Simplifier *s,
                              bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
...
static void foldCharConstant(TR::Node *node, uint16_t value, TR::Simplifier *s,
                             bool anchorChildrenP) {
  if (!performTransformationSimplifier(node, s)) return;
...
static void removeRestOfBlock(TR::TreeTop *curTree,
  ...
  for (treeTop = curTree->getNextTreeTop();
       treeTop->getNode()->getOpCodeValue() != TR::BBEnd; treeTop = next) {
...
TR::Node *checkcastSimplifier(TR::Node *node, TR::Block *block,
                              TR::Simplifier *s) {
  ...
  if ((node->getFirstChild()->getReferenceCount() > 1) &&
    ...
    if (prevTree) {
      ...
      if ((prevNode->getOpCodeValue() == TR::ificmpeq) ||
        ...
        if (prevNode->getOpCodeValue() == TR::ificmpne) equalityTest = false;
...
TR::Node *checkcastAndNULLCHKSimplifier(TR::Node *node, TR::Block *block,
                                        TR::Simplifier *s) {
...
TR::Node *variableNewSimplifier(TR::Node *node, TR::Block *block,
                                TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *imulhSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst() &&
    ...
    //  /    \   =>      /  \
      ...
      TR::Node::recreate(node, TR::ishr);
      TR::Node *newSecondChild = TR::Node::create(
          TR::iconst, 0);  // Create a new node because changing the value of
                           // the node might affect trees referencing this node
...
TR::Node *f2cSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  ...
  if (firstChild->getOpCode().isLoadConst()) {
    foldCharConstant(node,
                     floatToInt(firstChild->getFloat(), false) & 0x0000FFFF, s,
                     false /* !anchorChildren*/);
    return node;
  }
...
TR::Node *expSimplifier(TR::Node *node, TR::Block *block, TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  return replaceExpWithMult(node, node->getFirstChild(), node->getSecondChild(),
                            block, s);
}
...
//
TR::Node *ibits2fSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lbits2dSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *fbits2iSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *dbits2lSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *ifxcmpoSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  simplifyChildren(node, block, s);
...
TR::Node *ifxcmnoSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  if (removeIfToFollowingBlock(node, block, s) == NULL) return NULL;
  simplifyChildren(node, block, s);
...
// Lookup switch
//
TR::Node *lookupSwitchSimplifier(TR::Node *node, TR::Block *block,
                                 TR::Simplifier *s) {
...
// Table switch
//
TR::Node *tableSwitchSimplifier(TR::Node *node, TR::Block *block,
                                TR::Simplifier *s) {
...
// nullchk operator
//
TR::Node *nullchkSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  ...
  if (node->getFirstChild()->getNumChildren() == 0) {
    ...
    if (refNode->isNonNull() &&
        performTransformation(
            s->comp(), "%sRemoving redundant NULLCHK in node [%s]\n",
            s->optDetailString(), node->getName(s->getDebug())))
      TR::Node::recreate(node, TR::treetop);
    ...
    if ((refNode->isNull() || ((refNode->getOpCodeValue() == TR::aconst) &&
      ...
      for (auto edge = block->getSuccessors().begin();
           edge != block->getSuccessors().end(); ++edge) {
      ...
      if (!edgeExistsToEnd) cfg->addEdge(block, cfg->getEnd());
      ...
      for (auto edge = list.begin(); edge != list.end(); ++edge) {
        if ((*edge)->getTo() != cfg->getEnd())
          s->_blockRemoved |= cfg->removeEdge(*edge);
               ...
               performTransformation(
                   s->comp(), "%sNULLCHK passthrough simplification on n%dn\n",
                   s->optDetailString(), node->getGlobalIndex())) {
...
//
TR::Node *divchkSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
...
// Bound check
//
TR::Node *bndchkSimplifier(TR::Node *node, TR::Block *block,
                           TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if ((boundChild->getOpCode().isMul() && !isNodeMulHigh(boundChild) &&
    ...
    if (k1 == k2 && k1 > 0) {
      if (performTransformation(
              s->comp(), "%ssimplified algebra in BNDCHK [%s]\n",
              s->optDetailString(), node->getName(s->getDebug()))) {
...
//
TR::Node *arraycopybndchkSimplifier(TR::Node *node, TR::Block *block,
                                    TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (removableBoundCheck) {
    if (performTransformation(
            s->comp(), "%sRemoved arraycopy bndchk node [%s]\n",
            s->optDetailString(), node->getName(s->getDebug()))) {
      s->removeNode(node, s->_curTree);
      return NULL;
    }
...
//
TR::Node *bndchkwithspinechkSimplifier(TR::Node *node, TR::Block *block,
                                       TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *imaxminSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (has_const) {
    ...
    if (is_all_const) {
      if (is_signed)
        foldIntConstant(node, max_opcode ? max : min, s,
                        false /* !anchorChildren*/);
      else
        foldUIntConstant(node, max_opcode ? umax : umin, s,
                         false /* !anchorChildren*/);
    } else {
      if (is_signed)
...
TR::Node *lmaxminSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
  ...
  if (has_const) {
    ...
    if (is_all_const) {
      if (is_signed)
        foldLongIntConstant(node, max_opcode ? max : min, s,
                            false /* !anchorChildren */);
      else
        foldUnsignedLongIntConstant(node, max_opcode ? umax : umin, s,
                                    false /* !anchorChildren */);
    } else {
      if (is_signed)
...
TR::Node *fmaxminSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *dmaxminSimplifier(TR::Node *node, TR::Block *block,
                            TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *computeCCSimplifier(TR::Node *node, TR::Block *block,
                              TR::Simplifier *s) {
...
TR::Node *arraysetSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
//
TR::Node *bitOpMemSimplifier(TR::Node *node, TR::Block *block,
                             TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
//
TR::Node *bitOpMemNDSimplifier(TR::Node *node, TR::Block *block,
                               TR::Simplifier *s) {
  return node;
}
...
//
TR::Node *arrayCmpWithPadSimplifier(TR::Node *node, TR::Block *block,
                                    TR::Simplifier *s) {
  simplifyChildren(node, block, s);
...
TR::Node *lowerTreeSimplifier(TR::Node *node, TR::Block *block,
                              TR::Simplifier *s) {
...
TR::Node *arrayLengthSimplifier(TR::Node *node, TR::Block *block,
                                TR::Simplifier *s) {
  simplifyChildren(node, block, s);
