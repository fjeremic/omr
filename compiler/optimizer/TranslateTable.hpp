...
class TR_TranslateTable {
 public:
  ...
  TR_TranslateTable(TR::Compilation* comp)
      : _comp(comp), _table(NULL), _symRef(NULL) {}
 ...
 protected:
  ...
  TR_TranslateTableData* createTable(uint32_t startA, uint32_t endA,
  ...
  TR_TranslateTableData* createTable(uint32_t startA, uint32_t endA,
                                     uint8_t inputSize, uint8_t outputSize,
                                     uint16_t defaultValue);
  TR_TranslateTableData* createTable(uint16_t startA, uint16_t endA,
                                     uint8_t inputSize, uint8_t outputSize,
                                     uint16_t defaultValue);
  TR_TranslateTableData* createTable(uint16_t startA, uint16_t endA,
  ...
  TR_TranslateTableData* matchTable(uint8_t inputSize, uint8_t outputSize,
                                    void* array);

 private:
  // msf - methods that update head will need a lock when we move to an async
  // compilation model
  ...
  // can do something smarter if number of tables gets big - expected to be
  // quite small (1 or 2 tables in practice)
  TR_TranslateTableData* matchTable(uint32_t startA, uint32_t endA,
                                    uint32_t startB, uint32_t endB,
                                    uint8_t inputSize, uint8_t outputSize,
                                    uint16_t defaultValue);
};
...
{
 public:
  TR_RangeTranslateTable(TR::Compilation*, uint8_t, uint8_t, uint32_t, uint32_t,
                         uint16_t);
  TR_RangeTranslateTable(TR::Compilation*, uint8_t, uint8_t, uint32_t, uint32_t,
                         uint32_t, uint32_t, uint16_t);
  TR_RangeTranslateTable(TR::Compilation*, uint8_t, uint8_t, uint16_t, uint16_t,
                         uint16_t);
  TR_RangeTranslateTable(TR::Compilation*, uint8_t, uint8_t, uint16_t, uint16_t,
...
{
 public:
  TR_SetTranslateTable(TR::Compilation*, uint8_t, uint8_t,
                       uint8_t set[]);  // input and output size in bits
  TR_SetTranslateTable(TR::Compilation*, uint8_t, uint8_t,
                       uint16_t set[]);  // input and output size in bits
  TR_SetTranslateTable(TR::Compilation*, uint8_t, uint8_t, void* set,
                       int copySize);  // input and output size in bits
};
