...
inline bool TR_PartialRedundancy::isSupportedOpCode(TR::Node *node,
                                                    TR::Node *parent) {
...
#ifdef J9_PROJECT_SPECIFIC
static void correctDecimalPrecision(TR::Node *store, TR::Node *child,
                                    TR::Compilation *comp) {
  if (child->getType().isBCD() &&
      child->getDecimalPrecision() != store->getDecimalPrecision()) {
    TR::ILOpCodes modPrecOp =
        TR::ILOpCode::modifyPrecisionOpCode(child->getDataType());
    TR_ASSERT(modPrecOp != TR::BadILOp,
              "no bcd modify precision opcode found\n");
    TR::Node *modPrecNode = TR::Node::create(child, modPrecOp, 1);
    bool isTruncation =
        store->getDecimalPrecision() < child->getDecimalPrecision();
    if (comp->cg()->traceBCDCodeGen())
      traceMsg(comp,
               "%screating %s (%p) to correctDecimalPrecision (%d->%d : "
               "isTruncation=%s) on node %s (%p)\n",
               OPT_DETAILS, modPrecNode->getOpCode().getName(), modPrecNode,
               child->getDecimalPrecision(), store->getDecimalPrecision(),
               isTruncation ? "yes" : "no", child->getOpCode().getName(),
               child);
...
static bool shouldAllowOptimalSubNodeReplacement(TR::Compilation *comp) {
  if ((comp->getOptions()->getLastOptTransformationIndex() <
    ...
    if (!comp->getOption(TR_TracePREForOptimalSubNodeReplacement)) return false;
  }
...
TR_PartialRedundancy::TR_PartialRedundancy(TR::OptimizationManager *manager)
  ...
  if (_loadaddrPRE && _ignoreLoadaddrOfLitPool)
    _loadaddrPRE =
        false;  // need to check each loadaddr and cannot ignore them outright
}

TR_PartialRedundancy::ContainerType *TR_PartialRedundancy::allocateContainer(
    int32_t size) {
  return new (trStackMemory()) TR_BitVector(size, trMemory(), stackAlloc);
}

static bool ignoreValueOfMultipleReturnNode(TR::Node *node,
                                            TR::SparseBitVector &seenNodes) {
  auto npIdx = node->getGlobalIndex();
  if (seenNodes[npIdx]) return false;
  seenNodes[npIdx] = 1;
...
int32_t TR_PartialRedundancy::perform() {
  ...
  // within some limits
  //
  _numProfilingsAllowed = (((USHRT_MAX / 2 - 1000) - comp()->getNodeCount()) /
                           NUMBER_OF_NODES_PER_PROFILING_EXPANSION);
  if (_numProfilingsAllowed < 0) _numProfilingsAllowed = 0;
  ...
  if (comp()->getProfilingMode() == JitProfiling &&
      comp()->getHCRMode() != TR::none && _numProfilingsAllowed == 0)
    return 0;
  ...
  if (comp()->incompleteOptimizerSupportForReadWriteBarriers()) return 0;
  ...
  //
  _isolatedness = new (comp()->allocator())
      TR_Isolatedness(comp(), optimizer(), rootStructure, trace());

  // Obtain local anticipatable info; used in redundant set calculation
  //
  TR_LocalAnticipatability &localAnticipatability =
      _isolatedness->_latestness->_delayedness->_earliestness
          ->_globalAnticipatability->_localAnticipatability;

  if (trace()) traceMsg(comp(), "Starting PartialRedundancy\n");

  _numNodes = cfg->getNextNodeNumber();
  TR_ASSERT(_numNodes > 0, "Partial Redundancy, node numbers not assigned");

  if (trace()) comp()->dumpMethodTrees("Trees after PRE node numbering\n");

  int i;
  _numberOfBits = _isolatedness->_latestness->_numberOfBits;
  _optSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(ContainerType *));
  memset(_optSetInfo, 0, _numNodes * sizeof(ContainerType *));
  _rednSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(ContainerType *));
  memset(_rednSetInfo, 0, _numNodes * sizeof(ContainerType *));
  _unavailableSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(ContainerType *));
  memset(_unavailableSetInfo, 0, _numNodes * sizeof(ContainerType *));
  _origOptSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(ContainerType *));
  memset(_origOptSetInfo, 0, _numNodes * sizeof(ContainerType *));
  _globalRegisterWeights =
      (int32_t *)trMemory()->allocateStackMemory(_numNodes * sizeof(int32_t));
  ...
  TR::CFGNode *nextNode;
  for (nextNode = cfg->getFirstNode(); nextNode;
       nextNode = nextNode->getNext()) {
    TR_Structure *blockStructure = (toBlock(nextNode))->getStructureOf();
    if (blockStructure == NULL) continue;
  ...
  //
  _newSymbols = (TR::Symbol **)trMemory()->allocateStackMemory(
      _numberOfBits * sizeof(TR::Symbol *));
  _newSymbolReferences =
      (TR::SymbolReference **)trMemory()->allocateStackMemory(
          _numberOfBits * sizeof(TR::SymbolReference *));
  _newSymbolsMap = (int32_t *)trMemory()->allocateStackMemory(_numberOfBits *
                                                              sizeof(int32_t));
  _registerCandidates =
      (TR_RegisterCandidate **)trMemory()->allocateStackMemory(
          _numberOfBits * sizeof(TR_RegisterCandidate *));

  memset(_newSymbols, 0, _numberOfBits * sizeof(TR::Symbol *));
  memset(_newSymbolReferences, 0,
         _numberOfBits * sizeof(TR::SymbolReference *));
  memset(_registerCandidates, 0,
         _numberOfBits * sizeof(TR_RegisterCandidate *));
  ...
  for (nextNode = cfg->getFirstNode(); nextNode;
    ...
    _isolatedness->copyFromInto(_isolatedness->_latestness->_inSetInfo[i],
                                _optSetInfo[i]);
    ...
    *negation -= *temp;
    ...
    // _rednSetInfo[i]);
    *_rednSetInfo[i] = *localAnticipatability.getDownwardExposedAnalysisInfo(
        blockStructure->asBlock()->getBlock()->getNumber());
  ...
  if (!optimals->isEmpty()) {
    _exceptionCheckMotion = new (comp()->allocator())
        TR_ExceptionCheckMotion(comp(), optimizer(), this);
  ...
  for (nextNode = cfg->getFirstNode(); nextNode;
    ...
    *optimals |= *(_optSetInfo[i]);
    ...
    calculateGlobalRegisterWeightsBasedOnStructure(
        blockStructure, (_globalRegisterWeights + i));
  ...
  for (bvi1.SetToFirstOne(); bvi1.Valid(); bvi1.SetToNextOne()) {
    ...
    if (nextRedundantComputation != 0) {
      supportedNodesAsArray[nextRedundantComputation] =
          supportedNodesAsArray[nextRedundantComputation]
              ->duplicateTreeWithCommoning(comp()->allocator());
      supportedNodesAsArray[nextRedundantComputation]
          ->resetFlagsForCodeMotion();
    }
  }
    ...
    if (nextOptimalComputation != 0) {
      ...
      if ((_newSymbolsMap[nextOptimalComputation] < 0) &&
        ...
        if (nextOptimalNode->getOpCode().isLoadVarDirect() &&
          ...
          TR::SymbolReference *optimalSymRef =
              nextOptimalNode->getSymbolReference();
          _newSymbols[nextOptimalComputation] = optimalSymRef->getSymbol();
          _newSymbolsMap[nextOptimalComputation] =
              optimalSymRef->getReferenceNumber();
          ...
          if (trace()) {
            traceMsg(comp(),
                     "Creating new symbol for optimal expr number %d node %p\n",
          ...
          if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType()) !=
                  nextOptimalNode
                      ->getDataType() &&  // only do it for Java where we use
                                          // istore for small intergral types
          ...
          comp()->AddCopyPropagationRematerializationCandidate(
              newSymbolReference);
  ...
  for (bvi3.SetToFirstOne(); bvi3.Valid(); bvi3.SetToNextOne()) {
    ...
    if (nextOptimalNode->getOpCode().isCheck() ||
        (comp()->requiresSpineChecks() &&
         nextOptimalNode->getOpCode().hasSymbolReference() &&
    ...
    for (nextNode = cfg->getFirstNode(); nextNode;
      ...
      if (!nextNode->asBlock()->isCold()) {
        if (_rednSetInfo[nextNode->getNumber()]->get(nextOptimalComputation)) {
          ...
          int32_t weight = 1;
          nextNode->asBlock()->getStructureOf()->calculateFrequencyOfExecution(
              &weight);
    ...
    if (cost > benefit) {
      if (trace())
        traceMsg(comp(),
                 "Cost-benefit analysis (cost %d > benefit %d) said ignore "
                 "expression #%d\n",
  ...
  TR::Block *block;
  for (block = comp()->getStartBlock(); block != NULL;
    ...
    // still avoid inserting a store in a non-cold block simply for a load in a
    // cold block.
    //
    if ((block->getEntry() != NULL) /* &&
                                         !block->isCold() */)
      ...
      eliminateRedundantComputations(block, supportedNodesAsArray, _rednSetInfo,
                                     block->getEntry());
    }
  }
...
void TR_PartialRedundancy::invalidateOptimalComputation(
...
// Routine used in duplicating nodes for optimal placement; dulpicates
// respecting visitCounts. Unlike TR::Node::duplicateTree()
//
TR::Node *TR_PartialRedundancy::duplicateExact(TR::Node *node,
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    if (node->getChild(i)->getVisitCount() != visitCount)
      newRoot->setChild(i, duplicateExact(node->getChild(i), seenNodes,
                                          duplicateNodes, visitCount));
    else {
...
bool TR_PartialRedundancy::isNodeAnImplicitNoOp(TR::Node *node) {
  ...
  if (ignoreNode(node)) return true;

  if (TR::ILOpCode::isOpCodeAnImplicitNoOp(
          node->getOpCode()))  // FIXME: disables 190546!!!
    return true;

  if (node->getType().isAggregate() && node->getSize() > 8) {
    // if (trace())
    //   traceMsg(comp(),"skipping placing aggr %s
    //   (%p)\n",node->getOpCode().getName(),node);
    return true;
  }

  if (TR::TransformUtil::isNoopConversion(comp(), node)) return true;

#ifdef J9_PROJECT_SPECIFIC
  if (!TR::Compiler->cls.romClassObjectsMayBeCollected() &&
      node->getOpCode().hasSymbolReference() &&
      ((node->getSymbolReference() ==
        comp()->getSymRefTab()->findArrayClassRomPtrSymbolRef()) ||
       (node->getSymbolReference() ==
        comp()->getSymRefTab()->findClassRomPtrSymbolRef())))
    return true;
#endif
...
// Sign state does not have to be tracked on BCD nodes. This can lead to a BCD
// node with a particular sign setting to be stored to a temp and then the load
// of this temp will lose this sign info and subsequent generated code may
// clobber an unsigned sign code (0xf -> 0xc for example) pdSetSignA sign=?
//    x
// ...
// pdSetSignB
//    x
...
// pdload <temp1> sign=?
//
// To prevent this mark the pdload (reused from the original pdSetSignB) as
// containing some sign state info so the code generator will conservatively not
// alter this sign code.
void TR_PartialRedundancy::processReusedNode(TR::Node *node,
  ...
  if (comp()->cg()->traceBCDCodeGen())
    traceMsg(comp(), "reusing %s (%p) as op ", node->getOpCode().getName(),
             node);
...
TR::TreeTop *TR_PartialRedundancy::placeComputationsOptimally(
    TR::Block *block, TR::Node ***supportedNodesAsArray) {
  if (_optSetInfo[block->getStructureOf()->getNumber()]->isEmpty()) return NULL;
  ...
  for (nextOptimalComputation = orderedOptNumbersList[currOptimalElement];
       seenOptimalElements < numOptimalElements; currOptimalElement++) {
    nextOptimalComputation = orderedOptNumbersList[currOptimalElement];
    if (trace())
      traceMsg(comp(),
               "Optimal computation %d in block number %d insert after %p\n",
               nextOptimalComputation, block->getStructureOf()->getNumber(),
               placeToInsertOptimalComputations->getNode());
    ...
    if (_optSetInfo[block->getNumber()]->get(nextOptimalComputation)) {
      ...
      //
      if (nextOptimalComputation == 0) continue;
    } else
      continue;

    // Its a local variable/parm load, in which case nothing to do
    //
    TR::Node *nextOptimalNode =
        (*supportedNodesAsArray)[nextOptimalComputation];
    if ((nextOptimalNode->getOpCode().isLoadVarDirect() &&
         !nextOptimalNode->getSymbol()->isStatic() &&
         !nextOptimalNode->getSymbol()->isMethodMetaData()) ||
    ...
    if (anticipatabilityInfo->get(nextOptimalComputation)) {
      ...
      while (placeToInsertOptimalComputations != exitTree) {
        if (nextOptimalNode->getOpCode().isCheck()) {
              ...
              if (nextNode->getOpCode().isNullCheck()) {
                if (nextNode->getFirstChild()->exceptionsRaised() != 0) {
                  TR::TreeTop *prevTreeTop =
                      placeToInsertOptimalComputations->getPrevTreeTop();

                  // vcount_t visitCount1 = comp()->incVisitCount();
                  // List<TR::Node> seenNodes, duplicateNodes;
                  // TR::Node *duplicateOptimalNode =
                  // duplicateExact(nextOptimalNode->getNullCheckReference(),
                  // &seenNodes, &duplicateNodes, visitCount1);

                  // duplicateOptimalNode->setReferenceCount(0);
                  TR::SymbolReference *newSymRef =
                      nextOptimalNode->getSymbolReference();
                  TR::Node *passThroughNode = TR::Node::create(
                      TR::PassThrough, 1, nextNode->getNullCheckReference());
                  TR::Node *nodeInTreeTop = TR::Node::createWithSymRef(
                      TR::NULLCHK, 1, 1, passThroughNode, newSymRef);
                  TR::TreeTop *duplicateOptimalTree =
                      TR::TreeTop::create(comp(), nodeInTreeTop);
                  nodeInTreeTop->setLocalIndex(
                      nextOptimalNode->getLocalIndex());
                  ...
                  if (!shouldAllowLimitingOfOptimalPlacement(comp()) ||
                    ...
                    if (comp()->useCompressedPointers()) {
                      ...
                      if (reference->getOpCode().isLoadIndirect() &&
                          ...
                          TR::TransformUtil::fieldShouldBeCompressed(reference,
                                                                     comp())) {
                        translateTT = TR::TreeTop::create(
                            comp(),
                            TR::Node::createCompressedRefsAnchor(reference),
                            NULL, NULL);
                      }
                    }
                    ...
                    if (translateTT) {
                      ...
                      duplicateOptimalTree->join(
                          placeToInsertOptimalComputations);
                    placeToInsertOptimalComputations = duplicateOptimalTree;

                    manager()->setAlteredCode(true);
                  } else
                    nodeInTreeTop->getFirstChild()
          ...
          TR::Node *optimalNode = getAlreadyPresentOptimalNode(
              placeToInsertOptimalComputations->getNode(),
              nextOptimalComputation, comp()->getVisitCount(), rhsOfStore);

          // don't create stores for a void type
          if (optimalNode && optimalNode->getDataType() == TR::NoType) continue;

          if (optimalNode &&
              ((optimalNode->getLocalIndex() != nextOptimalComputation) ||
               rhsOfStore || !lastInsertionMade)) {
            //
            // Insert a store if none has been inserted already;
            // if a store has been inserted already then only insert a store
            // for privatization of global stores (for whom the optimalNode
            // returned will have a different index than nextOptimalComputation
            //
            TR::SymbolReference *newSymbolReference =
                _newSymbolReferences[nextOptimalComputation];

            if (!shouldAllowLimitingOfOptimalPlacement(comp()) ||
                performTransformation(
                    comp(), "%s0Placing %s optimally : %p using symRef #%d\n",
                    OPT_DETAILS, optimalNode->getOpCode().getName(),
                    optimalNode, newSymbolReference->getReferenceNumber())) {
              _useAliasSetsNotGuaranteedToBeCorrect = true;

              TR::Node *convertedOptimalNode = optimalNode;
              if (fe()->dataTypeForLoadOrStore(
                      nextOptimalNode->getDataType()) !=
                  nextOptimalNode->getDataType()) {
                TR::ILOpCodes conversionOpCode =
                    TR::ILOpCode::getProperConversion(
                        nextOptimalNode->getDataType(),
                        fe()->dataTypeForLoadOrStore(
                            nextOptimalNode->getDataType()),
                        false /* !wantZeroExtension */);
                convertedOptimalNode =
                    TR::Node::create(conversionOpCode, 1, optimalNode);
              }
              ...
              TR::Node *storeForCommonedNode = TR::Node::createWithSymRef(
                  comp()->il.opCodeForDirectStore(
                      nextOptimalNode->getDataType()),
                  1, 1, convertedOptimalNode, newSymbolReference);

#ifdef J9_PROJECT_SPECIFIC
              correctDecimalPrecision(storeForCommonedNode,
                                      convertedOptimalNode, comp());
#endif

              TR::TreeTop *duplicateOptimalTree =
                  TR::TreeTop::create(comp(), storeForCommonedNode);

              // dumpOptDetails(comp(), "%s0Placing %s optimally : %p using
              // symRef #%d\n", OPT_DETAILS, optimalNode->getOpCode().getName(),
              // optimalNode, newSymbolReference->getReferenceNumber());
              ...
              if (placeHolderOpCode.isBranch() ||
                ...
                placeToInsertOptimalComputations =
                    translateTT ? translateTT : duplicateOptimalTree;
                _prevTree = translateTT ? translateTT : duplicateOptimalTree;
              } else {
                TR::TreeTop *nextTreeInBlock =
              ...
              if (notLastInsertionInBlock)
                TR::Node::recreate(notLastInsertionInBlock->getNode(),
                                   TR::treetop);
        ...
        placeToInsertOptimalComputations =
            placeToInsertOptimalComputations->getNextRealTreeTop();
        if (placeToInsertOptimalComputations == startingPoint) break;

        if (placeToInsertOptimalComputations == block->getExit()) {
          if (startingPoint == block->getEntry()) break;

          if (!lastInsertionCanBeRemoved) notLastInsertionInBlock = NULL;
          placeToInsertOptimalComputations = block->getEntry();
          if (!nextOptimalNode->getOpCode().isCheck())
            comp()->incOrResetVisitCount();
        }
      ...
      if (!placeToInsertUnanticipatableOptimalComputations)
        ...
        placeToInsertOptimalComputations =
            placeToInsertUnanticipatableOptimalComputations;
    }
    ...
    if (nextOptimalNode->getOpCode().isStore()) {
      ...
      ///duplicateOptimalNode, NULL, NULL); /dumpOptDetails(comp(), "%sPlacing
      ...
      ////placeToInsertOptimalComputations = duplicateOptimalTree;
    } else if (nextOptimalNode->getOpCode().isCheck() ||
               (nextOptimalNode->getOpCodeValue() == TR::treetop)) {
      ...
      if ((nextOptimalNode->getOpCodeValue() == TR::NULLCHK) ||
        ...
        // optimally.
        //
        TR::ILOpCode &nullCheckReferenceOpCode =
            duplicateOptimalNode->getOpCode();
        TR::DataType nullCheckReferenceDataType =
            duplicateOptimalNode->getDataType();

        TR::Node *nullCheckReferenceNode =
            nextOptimalNode->getNullCheckReference();
        if (isSupportedOpCode(nullCheckReferenceNode, NULL) &&
            !nullCheckReferenceOpCode.isLoadVarDirect()) {
          if (((nullCheckReferenceNode->getLocalIndex() != MAX_SCOUNT) &&
               (nullCheckReferenceNode->getLocalIndex() != 0))) {
            if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] >
                 -1) &&
                (((!_unavailableSetInfo[block->getNumber()]->get(
                      nullCheckReferenceNode->getLocalIndex())) &&
                  (_rednSetInfo[block->getNumber()]->get(
                       nullCheckReferenceNode->getLocalIndex()) ||
                   _optSetInfo[block->getNumber()]->get(
                       nullCheckReferenceNode->getLocalIndex()))) ||
                 (comp()->requiresSpineChecks() &&
                  nullCheckReferenceNode->getOpCode().hasSymbolReference() &&
                  nullCheckReferenceNode->getSymbol()
                      ->isArrayShadowSymbol()))) {
              replacedNullCheckReference = true;
              TR::SymbolReference *newSymbolReference =
                  _newSymbolReferences[nullCheckReferenceNode->getLocalIndex()];
              TR::Node *newLoad = NULL;
              newLoad = TR::Node::createWithSymRef(
                  nullCheckReferenceNode,
                  comp()->il.opCodeForDirectLoad(nullCheckReferenceDataType), 0,
                  newSymbolReference);
              newLoad->setLocalIndex(-1);

              if (trace())
                traceMsg(comp(),
                         "Duplicate null check had its new null check "
                         "reference %p replaced by %p with symRef #%d\n",
        ...
        if (!replacedNullCheckReference) {
          ...
          int32_t k;
          for (k = 0;
               k < nextOptimalNode->getNullCheckReference()->getNumChildren();
               k++) {
            vcount_t visitCount4 = comp()->incVisitCount();
            placeToInsertOptimalComputations = replaceOptimalSubNodes(
                placeToInsertOptimalComputations,
                nextOptimalNode->getNullCheckReference(),
                nextOptimalNode->getNullCheckReference()->getChild(k), k,
                duplicateOptimalNode, duplicateOptimalNode->getChild(k), false,
                block->getNumber(), visitCount4);
          }
        }
        ...
        TR::Node *passThroughNode =
            TR::Node::create(TR::PassThrough, 1, duplicateOptimalNode);
        TR::Node *nodeInTreeTop = TR::Node::createWithSymRef(
            TR::NULLCHK, 1, 1, passThroughNode, newSymRef);
        TR::TreeTop *duplicateOptimalTree =
            TR::TreeTop::create(comp(), nodeInTreeTop);
        ...
        if (!shouldAllowLimitingOfOptimalPlacement(comp()) ||
          ...
          if (comp()->useCompressedPointers()) {
            ...
            if (reference->getOpCode().isLoadIndirect() &&
              ...
              translateTT = TR::TreeTop::create(
                  comp(), TR::Node::createCompressedRefsAnchor(reference), NULL,
                  NULL);
            }
          }
          manager()->setAlteredCode(true);
        ...
        TR::Node *duplicateOptimalNode = duplicateExact(
            nextOptimalNode, &seenNodes, &duplicateNodes, visitCount1);
        ...
        for (k = 0; k < nextOptimalNode->getNumChildren(); k++) {
          vcount_t visitCount4 = comp()->incVisitCount();
          placeToInsertOptimalComputations = replaceOptimalSubNodes(
              placeToInsertOptimalComputations, nextOptimalNode,
              nextOptimalNode->getChild(k), k, duplicateOptimalNode,
              duplicateOptimalNode->getChild(k), false, block->getNumber(),
              visitCount4);
        }

        duplicateOptimalNode->setReferenceCount(0);
        TR::TreeTop *duplicateOptimalTree =
            TR::TreeTop::create(comp(), duplicateOptimalNode, NULL, NULL);
        duplicateOptimalNode->setLocalIndex(nextOptimalNode->getLocalIndex());

        if (!shouldAllowLimitingOfOptimalPlacement(comp()) ||
            performTransformation(
                comp(), "%sPlacing %s optimally : %p\n", OPT_DETAILS,
                duplicateOptimalTree->getNode()->getOpCode().getName(),
                duplicateOptimalTree->getNode())) {
          TR::TreeTop *translateTT = NULL;
          if (comp()->useCompressedPointers()) {
      ...
      TR::Node *duplicateOptimalNode = duplicateExact(
          nextOptimalNode, &seenNodes, &duplicateNodes, visitCount2);
      ...
      if (fe()->dataTypeForLoadOrStore(duplicateOptimalNode->getDataType()) !=
        ...
        TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(
            duplicateOptimalNode->getDataType(),
            fe()->dataTypeForLoadOrStore(duplicateOptimalNode->getDataType()),
            false /* !wantZeroExtension */);
        duplicateOptimalNode->setReferenceCount(0);
        convertedOptimalNode =
            TR::Node::create(conversionOpCode, 1, duplicateOptimalNode);
      }

      TR::DataType type = nextOptimalNode->getDataType();
      TR::ILOpCodes storeOp = type == TR::NoType
                                  ? TR::treetop
                                  : comp()->il.opCodeForDirectStore(type);

      TR::Node *storeForCommonedNode = TR::Node::createWithSymRef(
          storeOp, 1, 1, convertedOptimalNode, newSymbolReference);
      convertedOptimalNode->setReferenceCount(1);
#ifdef J9_PROJECT_SPECIFIC
      correctDecimalPrecision(storeForCommonedNode, convertedOptimalNode,
                              comp());
#endif
      ...
      dumpOptDetails(comp(), "%s1Placing %s optimally : %p using symRef #%d\n",
                     OPT_DETAILS, convertedOptimalNode->getOpCode().getName(),
                     convertedOptimalNode,
                     newSymbolReference->getReferenceNumber());
      ...
      if (duplicateOptimalNode->getOpCode().isCall() &&
          ...
          duplicateOptimalNode->getSymbol()
              ->castToMethodSymbol()
              ->isPureFunction()) {
        TR::TreeTop *anchorTT = TR::TreeTop::create(
            comp(), TR::Node::create(TR::treetop, 1, duplicateOptimalNode),
            NULL, NULL);
...
TR::Node *TR_PartialRedundancy::getAlreadyPresentOptimalNode(
    TR::Node *node, int32_t nextOptimalComputation, vcount_t visitCount,
    bool &rhsOfStore) {
  if (node->getVisitCount() == visitCount) return NULL;
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    TR::Node *child = getAlreadyPresentOptimalNode(node->getChild(childNum),
                                                   nextOptimalComputation,
                                                   visitCount, rhsOfStore);
...
static bool isExpressionRedundant(
    TR::Node *node, TR_PartialRedundancy::ContainerType *redundantComputations,
    TR_PartialRedundancy::ContainerType *anticipatable) {
  int32_t preIndex2 = 1000000;
  static char *c1 = feGetEnv("TR_PreIndex2");
  if (c1) preIndex2 = atoi(c1);

  if (redundantComputations && node->getLocalIndex() != MAX_SCOUNT &&
      node->getLocalIndex() != 0 &&
      redundantComputations->get(node->getLocalIndex()) &&
      (node->getOpCode().isStore() ||
       anticipatable->get(node->getLocalIndex()))) {
    if (node->getLocalIndex() < preIndex2) return true;
  }
  return false;
}
...
void TR_PartialRedundancy::eliminateRedundantComputations(
  ...
  if (trace())
    traceMsg(comp(),
             "Eliminating redundant computations in block number %d visit "
             "count %d\n",
  ...
  ContainerType *redundantComputations =
      rednSetInfo[block->getStructureOf()->getNumber()];
  ContainerType *anticipatabilityInfo =
      _isolatedness->_latestness->_delayedness->_earliestness
          ->_globalAnticipatability->_localAnticipatability.getAnalysisInfo(
              block->getNumber());

  {
            ...
            if (region->isNaturalLoop() && piv) {
              ...
              if (callerIndex != -1) do {
                  ...
                  if (method->getRecognizedMethod() ==
                      TR::java_util_stream_IntPipeline_forEach) {
                    shouldHaveRedundantComputations = true;
                    break;
                  }
                  TR_InlinedCallSite site =
                      comp()->getInlinedCallSite(callerIndex);
    ...
    while (currentTree != exitTree) {
      ...
      //
      eliminateRedundantSupportedNodes(
          NULL, currentTree->getNode(), false, currentTree, block->getNumber(),
          visitCount,
          shouldHaveRedundantComputations ? redundantComputations : NULL,
          anticipatabilityInfo, supportedNodesAsArray);

      // Now handle stores
      //
      // Stores could be either treetops themselves or be under a TR::treetop or
      // a null check/resolve check
      //
      if ((firstOpCodeInTree.isStore() && !currentTree->getNode()
                                               ->getSymbolReference()
                                               ->getSymbol()
                                               ->isAutoOrParm()) ||
        ...
        //
        if (isExpressionRedundant(currentTree->getNode(), redundantComputations,
          ...
          if (!currentTree->getNode()->getOpCode().isStore()) {
            if (0 && performTransformation(
                         comp(),
                         "%sEliminating redundant check computation (resolve "
                         "or null check) : %p\n",
                         OPT_DETAILS,
                         currentTree->getNode()))  // Look at check-in comment
                                                   // for version 1.296
            {
              if (firstOpCodeInTree.getOpCodeValue() == TR::NULLCHK)
                TR::Node::recreate(currentTree->getNode(), TR::treetop);
              else if (firstOpCodeInTree.getOpCodeValue() ==
              ...
              // dumpOptDetails(comp(), "%sEliminating redundant check
              // computation (resolve or null check) : %p\n", OPT_DETAILS,
              // currentTree->getNode());
            }
          } else {
            TR::SymbolReference *newSymbolReference =
                _newSymbolReferences[nodeIndex];
            if (newSymbolReference /* &&
              ...
              TR::ILOpCodes opCodeValue =
                  comp()->il.opCodeForDirectStore(redundantNode->getDataType());
                ...
                if (fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType()) !=
                  ...
                  TR::ILOpCodes conversionOpCode =
                      TR::ILOpCode::getProperConversion(
                          ...
                          false /* !wantZeroExtension */);
                  convertedOptimalNode =
                      TR::Node::create(conversionOpCode, 1, rhsOfStore);
                }

                storeForCommonedNode = TR::Node::createWithSymRef(
                    opCodeValue, 1, 1, convertedOptimalNode,
                    newSymbolReference);
              } else
                storeForCommonedNode = TR::Node::createWithSymRef(
                    opCodeValue, 1, 1, currentTree->getNode()->getFirstChild(),
                    newSymbolReference);
#ifdef J9_PROJECT_SPECIFIC
              correctDecimalPrecision(storeForCommonedNode,
                                      storeForCommonedNode->getFirstChild(),
                                      comp());
#endif
              ...
              dumpOptDetails(
                  comp(), "%sEliminating redundant computation (store) : %p\n",
                  OPT_DETAILS, currentTree->getNode());

              if (storeForCommonedNode->getDataType() != TR::Address)
                optimizer()->setRequestOptimization(OMR::loopVersionerGroup,
                                                    true);
        ...
        if ((currentTree->getNode()->getNumChildren() > 0) &&
            isExpressionRedundant(currentTree->getNode()->getFirstChild(),
                                  redundantComputations,
                                  anticipatabilityInfo) &&
            currentTree->getNode()->getFirstChild()->getOpCode().isStore()) {
          scount_t nodeIndex =
              currentTree->getNode()->getFirstChild()->getLocalIndex();
          TR::Node *redundantNode = supportedNodesAsArray[nodeIndex];
		  ...
		  performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()->getFirstChild()) */)
            ...
            TR::Node *storeForCommonedNode = NULL;
            if (currentTree->getNode()
                    ->getFirstChild()
                    ->getOpCode()
                    .isStoreIndirect()) {
              TR::Node *rhsOfStore =
                  currentTree->getNode()->getFirstChild()->getSecondChild();
              TR::Node *convertedOptimalNode = rhsOfStore;
              if (fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType()) !=
                  rhsOfStore->getDataType()) {
                TR::ILOpCodes conversionOpCode =
              ...
              storeForCommonedNode = TR::Node::createWithSymRef(
                  opCodeValue, 1, 1,
                  currentTree->getNode()->getFirstChild()->getFirstChild(),
                  newSymbolReference);
#ifdef J9_PROJECT_SPECIFIC
            correctDecimalPrecision(storeForCommonedNode,
                                    storeForCommonedNode->getFirstChild(),
            ...
            manager()->setAlteredCode(true);
                 ...
                 firstOpCodeInTree
                     .isCheck())  // Look at check-in comment for version 1.296
      {
        ...
        if (isExpressionRedundant(currentTree->getNode(), redundantComputations,
          ...
          if (firstOpCodeInTree.isResolveOrNullCheck()) {
            if (performTransformation(
                    comp(),
                    "%sEliminating redundant check computation (resolve or "
                    "null check) : %p\n",
                    OPT_DETAILS, currentTree->getNode())) {
              if (firstOpCodeInTree.getOpCodeValue() == TR::NULLCHK)
                TR::Node::recreate(currentTree->getNode(), TR::treetop);
            ...
            if (performTransformation(
                    comp(),
                    "%sEliminating redundant check computation (%s) : %p\n",
                    OPT_DETAILS, currentTree->getNode()->getOpCode().getName(),
                    currentTree->getNode())) {
              ...
              collectAllNodesToBeAnchored(
                  &anchoredNodes, currentTree->getNode(), newVisitCount);
              ListIterator<TR::Node> anchorIt(&anchoredNodes);
              TR::Node *nextAnchor;
              for (nextAnchor = anchorIt.getFirst(); nextAnchor != NULL;
                   nextAnchor = anchorIt.getNext()) {
                TR::TreeTop *anchorTreeTop = TR::TreeTop::create(
                    comp(), TR::Node::create(TR::treetop, 1, nextAnchor));
              ...
              /// dumpOptDetails(comp(), "%sEliminating redundant check
              /// computation (%s) : %p\n", OPT_DETAILS,
              /// currentTree->getNode()->getOpCode().getName(),
              /// currentTree->getNode());
    ...
    if (_exceptionCheckMotion) _profilingWalk = false;
  }
...
void TR_PartialRedundancy::printTrees() {
  ...
  while (!(currentTree == NULL)) {
    if (trace()) getDebug()->print(comp()->getOutFile(), currentTree);
...
//
bool TR_PartialRedundancy::eliminateRedundantSupportedNodes(
    TR::Node *parent, TR::Node *node, bool firstComputation,
    TR::TreeTop *currentTree, int32_t blockNum, vcount_t visitCount,
    ContainerType *redundantComputations, ContainerType *anticipatabilityInfo,
    TR::Node **supportedNodesAsArray) {
  // If the node has been seen before, its visit count could be visitCount or
  // visitCount+1
  if (visitCount <= node->getVisitCount()) {
    if (isExpressionRedundant(node, redundantComputations,
                              anticipatabilityInfo)) {
      //
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    if (!eliminateRedundantSupportedNodes(
            node, child, flag, currentTree, blockNum, visitCount,
  ...
  if (isSupportedOpCode(node, parent)) {
    ...
    if (_profilingWalk &&
        ((_exceptionCheckMotion &&
          _exceptionCheckMotion->getOptimisticRednSetInfo()[blockNum]->get(
              node->getLocalIndex())) ||
         isExpressionRedundant(node, redundantComputations,
                               anticipatabilityInfo))) {
      ...
      // may be able to specialize a loop based on the (possibly loop invariant)
      // values being profiled here
      //
      if (comp()->isProfilingCompilation() &&
        ...
        TR::TreeTop *prevTree = currentTree->getPrevTreeTop();
        TR_ValueProfiler *valueProfiler =
            comp()->getRecompilationInfo()->getValueProfiler();
        ...
        if ((node->getType().isInt64()) && profileLongParms) {
    ...
    if (isExpressionRedundant(node, redundantComputations,
      ...
      if (!isNodeAnImplicitNoOp(node)) {
        ...
        for (j = 0; j < node->getNumChildren(); j++)
          collectAllNodesToBeAnchored(&anchoredNodes, node->getChild(j),
                                      visitCount + 1);
        ...
        if (node->getOpCode().isLoadConst()) {
          if ((parent->getOpCode().isDiv() || parent->getOpCode().isRem()) &&
            ...
            optimizer()->setRequestOptimization(OMR::globalValuePropagation,
                                                true);
        }

        // dumpOptDetails(comp(), "%sEliminating redundant computation (%s) :
        ...
        if (newSymbolReference &&
            performTransformation(comp(),
                                  "%sEliminating redundant computation (%s) : "
                                  "%p in block_%d, visit count %d\n",
                                  OPT_DETAILS, node->getOpCode().getName(),
                                  node, blockNum, node->getVisitCount())) {
          if (node->getDataType() == TR::NoType) {
            currentTree->unlink(true);  // don't create loads of a void type
          } else if (fe()->dataTypeForLoadOrStore(node->getDataType()) !=
                     node->getDataType()) {
            if (parent && parent->getOpCode().isConversion() &&
                (parent->getOpCodeValue() ==
                 TR::ILOpCode::getProperConversion(
                     parent->getDataType(), node->getDataType(),
                     false /*!wantZeroExtension*/))) {
              if (node->getReferenceCount() > 1) {
                TR::TreeTop *anchorTreeTop = TR::TreeTop::create(
              ...
              processReusedNode(parent,
                                comp()->il.opCodeForDirectLoad(nodeDataType),
                                newSymbolReference, 0);
            } else {
              for (j = 0; j < node->getNumChildren(); j++)
                node->getChild(j)->recursivelyDecReferenceCount();

              TR::Node *newLoad = TR::Node::createWithSymRef(
                  node, comp()->il.opCodeForDirectLoad(node->getDataType()), 0,
                  newSymbolReference);
              ...
              if (conversionOpCode == TR::v2v) {
                node = TR::Node::createVectorConversion(newLoad,
                                                        node->getDataType());
              } else {
                processReusedNode(node, conversionOpCode, NULL, 1);
#ifdef J9_PROJECT_SPECIFIC
                if (node->getType().isBCD()) node->resetSignState();
#endif
...
bool TR_PartialRedundancy::isNotPrevTreeStoreIntoFloatTemp(TR::Symbol *symbol) {
  if (_prevTree &&
      (_prevTree->getNode()->getOpCode().isFloat() ||
       _prevTree->getNode()->getOpCode().isDouble()) &&
...
//
TR::TreeTop *TR_PartialRedundancy::replaceOptimalSubNodes(
    TR::TreeTop *curTree, TR::Node *parent, TR::Node *node, int32_t childNum,
    TR::Node *duplicateParent, TR::Node *duplicateOptimalNode, bool isNullCheck,
    int32_t blockNum, vcount_t visitCount) {
  if (node->getVisitCount() == visitCount) return curTree;
  ...
  if (isSupportedOpCode(node, parent) &&
      (!(opCode.isLoadVarDirect() && !node->getSymbol()->isStatic() &&
         !node->getSymbol()->isMethodMetaData())) &&
      (!isNodeAnImplicitNoOp(node))) {
    if (trace())
      traceMsg(comp(),
               "Node %p has parent %p and we are considering replacing it\n",
               node, parent);
    if (((node->getLocalIndex() != MAX_SCOUNT) &&
         (node->getLocalIndex() != 0)) &&
        !(isNullCheck && (_nullCheckNode->getNullCheckReference() == node))) {
      // We do not want to do optimal subnode replacement under tracePRE because
      // it can cause spurious failures as a result of earlier optimal node
      // placement not having been done (because of opt transformation index)
      // that interfere with last opt transformation index based search. This is
      // done because optimal sub node replacement is rarely the cause of bugs
      // on it's own and this issue is just making it harder to debug problems
      // in other PRE transformations. We created another option
      // TR_TracePREForOptimalSubNodeReplacement to continue to allow optimal
      // sub node replacement to be done regardless but in order to make this
      // more useful we turn off limiting optimal placements when this new
      // option is in effect (so that it becomes safe to narrow down to a
      // optimal sub node replacement in the unlikely event that this is the
      // cause of some failure)
      //
      if (shouldAllowOptimalSubNodeReplacement(comp()) &&
          (((_newSymbolsMap[node->getLocalIndex()] > -1) &&
            ((/* _optSetInfo[blockNum]->get(node->getLocalIndex()) || */ (
                  (!parent || !parent->getOpCode().isSpineCheck() ||
                   (childNum != 0)) &&
                  comp()->requiresSpineChecks() &&
                  node->getOpCode().hasSymbolReference() &&
        ...
        TR::SymbolReference *newSymbolReference =
            _newSymbolReferences[node->getLocalIndex()];
        TR::Node *newLoad = TR::Node::createWithSymRef(
            node, comp()->il.opCodeForDirectLoad(nodeDataType), 0,
            newSymbolReference);
        if (fe()->dataTypeForLoadOrStore(node->getDataType()) !=
            node->getDataType())
          newLoad =
              TR::Node::create(TR::ILOpCode::getProperConversion(
                                   newLoad->getDataType(), node->getDataType(),
                                   false /* !wantZeroExtension */),
                               1, newLoad);
        ...
        if (trace())
          traceMsg(comp(),
                   "Duplicate parent %p had its old child %p replaced by %p "
                   "with symRef #%d\n",
        ...
        if (trace())
          traceMsg(comp(),
                   "Note : Duplicate parent %p wanted to replace its child %p "
                   "by possibly already available symRef but FAILED to do so\n",
                   duplicateParent, duplicateOptimalNode);
        int32_t i;
        for (i = 0; i < node->getNumChildren(); i++) {
          TR::Node *child = node->getChild(i);
          TR::Node *duplicateChild = duplicateOptimalNode->getChild(i);
          curTree = replaceOptimalSubNodes(curTree, node, child, i,
                                           duplicateOptimalNode, duplicateChild,
...
const char *TR_PartialRedundancy::optDetailString() const throw() {
...
TR_ExceptionCheckMotion::TR_ExceptionCheckMotion(
    TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_PartialRedundancy *partialRedundancy)
    : TR_DataFlowAnalysis(comp, comp->getFlowGraph(), optimizer,
                          partialRedundancy->trace()),
  ...
  _orderedOptNumbersList = (int32_t **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(int32_t *));
  memset(_orderedOptNumbersList, 0, _numberOfNodes * sizeof(int32_t *));

  _actualOptSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(ContainerType *));
  memset(_actualOptSetInfo, 0, _numNodes * sizeof(ContainerType *));
  _actualRednSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(ContainerType *));
  memset(_actualRednSetInfo, 0, _numNodes * sizeof(ContainerType *));
  _killedGenExprs = (ContainerType **)trMemory()->allocateStackMemory(
      _numNodes * sizeof(ContainerType *));
  ...
  for (nextNode = cfg->getFirstNode(); nextNode;
    ...
    ///// Changed to initialize here too to save multiple calls to elementCount
    ///... Dave S
    ...
    //
    int32_t numElements = _optimisticOptSetInfo[i]->elementCount() +
                          _optimisticRednSetInfo[i]->elementCount();
    _orderedOptNumbersList[i] = (int32_t *)trMemory()->allocateStackMemory(
  ...
  for (auto nextEdge = block->getPredecessors().begin();
    ...
    switch (result) {
      ...
      case FIRST_LARGER: {
        ...
        if (trace()) traceMsg(comp(), "Fence at entry to %d\n", blockNum);
        break;
      }
      case SECOND_LARGER: {
...
int32_t TR_ExceptionCheckMotion::areExceptionSuccessorsIdentical(
    TR::CFGNode *node1, TR::CFGNode *node2) {
  int32_t result = -1;
  _firstSucc->empty();
  _secondSucc->empty();
  _comparison->empty();
  for (auto nextEdge = node1->getExceptionSuccessors().begin();
       nextEdge != node1->getExceptionSuccessors().end(); ++nextEdge) {
    TR::CFGNode *succ = (*nextEdge)->getTo();
    _firstSucc->set(succ->getNumber());
  }

  for (auto nextEdge = node2->getExceptionSuccessors().begin();
       nextEdge != node2->getExceptionSuccessors().end(); ++nextEdge) {
    TR::CFGNode *succ = (*nextEdge)->getTo();
...
int32_t TR_ExceptionCheckMotion::perform() {
  ...
  _orderedOptList =
      (List<TR::Node> **)trMemory()->allocateStackMemory(arraySize);
  ...
  for (nextNode = cfg->getFirstNode(); nextNode;
       nextNode = nextNode->getNext()) {
    TR::Block *block = toBlock(nextNode);
    if (block->getStructureOf() == NULL) continue;
  ...
  //
  // Try at most NUM_ITERATIONS iterations of expression dominance and redundant
  // expression adjustment; basically keep iterating till some list changes If
  // there is no change in less than NUM_ITERATIONS iterations we will detect
  // that we have a converged solution and return
  //
  int32_t numIterations;
  for (numIterations = 0; numIterations < NUM_ITERATIONS; numIterations++) {
    if (!_tryAnotherIteration) break;
    ...
    if (_firstIteration) {
      ...
      //
      int32_t i;
      for (i = 0; i < _numberOfNodes; i++) _currentList[i]->deleteAll();
    ...
    for (nextNode = cfg->getFirstNode(); nextNode;
      ...
      *_tempContainer &= *_exprsUnaffectedByOrder;
      ...
      if (trace()) traceMsg(comp(), "Block Number (ordered list) : %d\n", i);
      ...
      if (_orderedOptList[i] != NULL) {
        ...
        if (orderedListSize > 0) {
          ListElement<TR::Node> *nextElement =
              _orderedOptList[i]->getListHead();
          orderedListSize = orderedListSize + size;
          int32_t numElements2 = _optimisticOptSetInfo[i]->elementCount() +
                                 _optimisticRednSetInfo[i]->elementCount();
          int32_t j;
      ...
      // computed by PRE and the actual optimal info computed as a result of
      // applying expression ordering constraints in expression dominance, then
    ...
    // is to place it optimally in those blocks (where the expression was
    // redundant) that dominate other blocks where it is redundant
    //
    ...
    if (redundantSetAdjustmentRequired || _trySecondBestSolution) {
      ...
      //
      ContainerType **unavailableSetInfo =
          _partialRedundancy->getUnavailableSetInfo();
      TR_RedundantExpressionAdjustment *redundantExpressionAdjustment =
          new (comp()->allocator()) TR_RedundantExpressionAdjustment(
              comp(), optimizer(), comp()->getFlowGraph()->getStructure(),
              this);

      // TR_BitVector **globalRegisterSetInfo = NULL;
      // if (_trySecondBestSolution)
      //   globalRegisterSetInfo =
      //   _partialRedundancy->getGlobalRegisterSetInfo();
      //
      // TR_BitVector **globalRegisterSetInfo =
      // _partialRedundancy->getGlobalRegisterSetInfo();
      ...
      for (nextNode = cfg->getFirstNode(); nextNode;
        ...
        if (block->getStructureOf() == NULL) continue;
        int32_t i = block->getNumber();
        *_actualRednSetInfo[i] &=
            *(redundantExpressionAdjustment->_blockAnalysisInfo[i]);
        *_actualRednSetInfo[i] -= *(_actualOptSetInfo[i]);
        unavailableSetInfo[i]->setAll(_partialRedundancy->getNumberOfBits());
        *unavailableSetInfo[i] -=
            *(redundantExpressionAdjustment->_blockAnalysisInfo[i]);
        // if (_trySecondBestSolution)
        //   {
        //   *(globalRegisterSetInfo[i]) =
        //   *(redundantExpressionAdjustment->_blockAnalysisInfo[i]);
        //   }
      }
    }
...
void TR_ExceptionCheckMotion::copyListFromInto(List<TR::Node> *fromList,
                                               List<TR::Node> *toList) {
  if (!fromList->isEmpty()) {
    ...
    for (; oldNode != NULL;) {
      if (newNode == NULL) {
        newNode = (ListElement<TR::Node> *)trMemory()->allocateStackMemory(
            sizeof(ListElement<TR::Node>));
...
void TR_ExceptionCheckMotion::composeLists(List<TR::Node> *targetList,
                                           List<TR::Node> *sourceList,
  ...
  if (!sourceList->isEmpty() && !targetList->isEmpty()) {
    ...
    while (oldNode && !oldNode->getData()->getOpCode().isCheck()) {
      ...
      if (!markerNode) markerNode = oldNode;
    ...
    for (newNode = sourceList->getListHead(); newNode != NULL;) {
      cursorOldNode = NULL;
      if (oldNode) cursorOldNode = oldNode->getNextElement();
      ...
      while (newNode && !newNode->getData()->getOpCode().isCheck()) {
        if (!_composeHelper->get(newNode->getData()->getLocalIndex())) {
          ListElement<TR::Node> *newSourceNode =
              (ListElement<TR::Node> *)trMemory()->allocateStackMemory(
                  sizeof(ListElement<TR::Node>));
          ...
          if (!markerNode) markerNode = newSourceNode;
      ...
      if ((oldNode == NULL) || (newNode == NULL) ||
          (oldNode->getData()->getLocalIndex() !=
           newNode->getData()->getLocalIndex())) {
        ListElement<TR::Node> *prevCursorNode = cutoffNode;
        ListElement<TR::Node> *cursorNode =
            cutoffNode ? cutoffNode->getNextElement() : markerNode;
        while (cursorNode) {
          if (_definitelyNotKilled->get(
                  cursorNode->getData()->getLocalIndex()) ||
              ((cursorNode->getData()->getOpCode().isIndirect() ||
                cursorNode->getData()->getOpCode().isArrayLength()) &&
               availableExprs->get(
                   cursorNode->getData()->getFirstChild()->getLocalIndex())))
        ...
        while (oldNode &&
               ((oldNode->getData()->getOpCode().isIndirect() ||
                ...
                availableExprs->get(
                    oldNode->getData()->getFirstChild()->getLocalIndex()))) {
          ...
          if (!_composeHelper->get(oldNode->getData()->getLocalIndex())) {
            ...
            if (prevCursorNode) {
              ...
              // prevCursorNode, prevCursorNode->getData(), oldNode,
              // oldNode->getData());
            } else
        ...
        if (newNode) newNode = newNode->getNextElement();
        while (newNode &&
               ((newNode->getData()->getOpCode().isIndirect() ||
                 newNode->getData()->getOpCode().isArrayLength()) &&
                availableExprs->get(
                    newNode->getData()->getFirstChild()->getLocalIndex()))) {
          if (!_composeHelper->get(newNode->getData()->getLocalIndex())) {
            ListElement<TR::Node> *newSourceNode =
                (ListElement<TR::Node> *)trMemory()->allocateStackMemory(
                    sizeof(ListElement<TR::Node>));
    ...
    if (!prevCursorNode) targetList->deleteAll();
...
void TR_ExceptionCheckMotion::appendLists(List<TR::Node> *firstList,
                                          List<TR::Node> *secondList) {
  if (!secondList->isEmpty()) {
    ...
    for (; secondNode != NULL;) {
      if (!_appendHelper->get(secondNode->getData()->getLocalIndex())) {
        newFirstNode = (ListElement<TR::Node> *)trMemory()->allocateStackMemory(
            sizeof(ListElement<TR::Node>));
...
bool TR_ExceptionCheckMotion::compareLists(List<TR::Node> *firstList,
                                           List<TR::Node> *secondList) {
  bool same = true;
...
TR_ExceptionCheckMotion::ExprDominanceInfo *
TR_ExceptionCheckMotion::getAnalysisInfo(TR_Structure *s) {
...
TR_ExceptionCheckMotion::ExprDominanceInfo *
TR_ExceptionCheckMotion::createAnalysisInfo() {
  ExprDominanceInfo *analysisInfo = new (trStackMemory()) ExprDominanceInfo;
  analysisInfo->_outList = (List<TR::Node> **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(List<TR::Node> *));
  memset(analysisInfo->_outList, 0, _numberOfNodes * sizeof(List<TR::Node> *));
  return analysisInfo;
}

void TR_ExceptionCheckMotion::initializeAnalysisInfo(ExprDominanceInfo *info,
                                                     TR_Structure *s) {
...
void TR_ExceptionCheckMotion::initializeAnalysisInfo(ExprDominanceInfo *info,
  ...
  for (auto succ = block->getSuccessors().begin();
    ...
    info->_outList[succBlock->getNumber()] =
        new (trStackMemory()) TR_ScratchList<TR::Node>(trMemory());
  }

  for (auto succ = block->getExceptionSuccessors().begin();
       succ != block->getExceptionSuccessors().end(); ++succ) {
    succBlock = (*succ)->getTo();
    info->_outList[succBlock->getNumber()] =
        new (trStackMemory()) TR_ScratchList<TR::Node>(trMemory());
  }
...
TR_ExceptionCheckMotion::ContainerType *
TR_ExceptionCheckMotion::allocateContainer(int32_t size) {
  return new (trStackMemory()) TR_BitVector(size, trMemory(), stackAlloc);
}

void TR_ExceptionCheckMotion::initializeAnalysisInfo(
    ExprDominanceInfo *info, TR_RegionStructure *region) {
  ContainerType *exitNodes = allocateContainer(_numberOfNodes);

  info->_inList = new (trStackMemory()) TR_ScratchList<TR::Node>(trMemory());
  //
  // Copy the current out set for comparison the next time we analyze this
  // region
  //
  //
  if (region != comp()->getFlowGraph()->getStructure()) {
    ...
    for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
      ...
      if (!exitNodes->get(toStructureNumber)) {
        info->_outList[toStructureNumber] =
            new (trStackMemory()) TR_ScratchList<TR::Node>(trMemory());
...
void TR_ExceptionCheckMotion::initializeGenAndKillSetInfo() {
  ...
  TR::CFGNode *cfgNode;
  for (cfgNode = comp()->getFlowGraph()->getFirstNode(); cfgNode;
       cfgNode = cfgNode->getNext()) {
    TR::Block *block = toBlock(cfgNode);
    int32_t blockNum = block->getNumber();
    if (_firstIteration) {
      _regularGenSetInfo[blockNum] =
          new (trStackMemory()) TR_ScratchList<TR::Node>(trMemory());
      _blockInfo[blockNum] =
          new (trStackMemory()) TR_ScratchList<TR::Node>(trMemory());
    } else {
    ...
    if (!block->getExceptionSuccessors().empty()) _hasExceptionSuccessor = true;
    ...
    for (; treeTop != block->getExit(); treeTop = treeTop->getNextTreeTop()) {
      ...
      if (_hasExceptionSuccessor) {
        if (node->exceptionsRaised() != 0) _atLeastOneExceptionPoint = true;
      ...
      analyzeNodeToInitializeGenAndKillSets(treeTop, visitCount,
                                            block->getStructureOf());
    ...
    if (trace()) {
      if (!_regularGenSetInfo[blockNum]->isEmpty()) {
        ...
        for (listElem = _regularGenSetInfo[blockNum]->getListHead();
             listElem != NULL; listElem = listElem->getNextElement())
          traceMsg(comp(), "Expr %d (representative) Node %p in Block : %d\n",
                   listElem->getData()->getLocalIndex(), listElem->getData(),
                   blockNum);
      } else
  ...
  if (treeHasChecks(tt) || node->getOpCode().isStore()) {
    ...
    if (node->getOpCode().isCheck()) {
      ...
      if (opCodeValue == TR::NULLCHK) {
        ...
        includeRelevantNodes(node->getNullCheckReference(), visitCount,
                             blockNum);
        ...
        if (!_atLeastOneExceptionPoint) {
          if ((_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) ||
            ...
            // markNodeAsSurvivor(node->getNullCheckReference(),
            // _indirectAccessesThatSurvive);
          }
        }
        ...
        int32_t i;
        for (i = firstChildNum; i < node->getFirstChild()->getNumChildren();
             i++)
          includeRelevantNodes(node->getFirstChild()->getChild(i), visitCount,
                               blockNum);

        bool nodeKilledByChild = false;
        for (i = firstChildNum; i < node->getFirstChild()->getNumChildren();
             i++) {
          if (isNodeKilledByChild(node, node->getFirstChild()->getChild(i),
                                  blockNum)) {
            nodeKilledByChild = true;
            break;
          }
        ...
        if (!_atLeastOneExceptionPoint) {
          if ((_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) ||
            ...
            // _unresolvedAccessesThatSurvive);
          }
        }
      ...
      } else if (node->getOpCode().isBndCheck() ||
                 node->getOpCode().isSpineCheck()) {
        checkNode = true;
        bool nodeKilledByChild = false;

        int32_t i = 0;
        if (node->getOpCode().isSpineCheck()) i = 1;
        ...
        if (!_atLeastOneExceptionPoint) {
          if ((_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) ||
              ...
              (!_genSetHelper->get(node->getLocalIndex()))) {
            createAndAddListElement(node, blockNum);
            // markNodeAsSurvivor(node->getFirstChild()->getSecondChild(),
            // _dividesThatSurvive);
          }
        }
        ...
        if ((!_optimisticRednSetInfo[blockNum]->get(
            ...
            !checkIfNodeCanSurvive(node->getFirstChild()->getSecondChild(),
                                   _dividesThatSurvive)) {
          _killedGenExprs[blockNum]->set(node->getLocalIndex());
          _nullCheckKilled->set(blockNum);
          _resolveCheckKilled->set(blockNum);
        ...
        if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
          ...
          _seenImmovableExceptionPoint = true;
                     ...
                     .isCheckCast())  //(opCodeValue == TR::checkcast)
      {
        checkNode = true;
        bool nodeKilledByChild = false;
        int32_t i;
      ...
      if (_hasExceptionSuccessor) _atLeastOneExceptionPoint = true;
    } else if (node->getOpCode().isStore()) {
...
void TR_ExceptionCheckMotion::createAndAddListElement(TR::Node *node,
                                                      int32_t blockNum) {
...
bool TR_ExceptionCheckMotion::isNodeKilledByChild(TR::Node *parent,
                                                  TR::Node *child,
                                                  int32_t blockNum) {
  ...
  if (((child->getLocalIndex() != MAX_SCOUNT) &&
    ...
    if (_exprsContainingUnresolvedAccess->get(child->getLocalIndex())) {
      ...
      if (_unresolvedAccessesKilled->get(blockNum)) {
        if (!checkIfNodeCanSomehowSurvive(child,
                                          _unresolvedAccessesThatSurvive))
          parentKilled = true;
      }
    }
...
bool TR_ExceptionCheckMotion::includeRelevantNodes(TR::Node *node,
                                                   vcount_t visitCount,
                                                   int32_t blockNum) {
  if (node->getVisitCount() == visitCount) {
    if (((node->getLocalIndex() != MAX_SCOUNT) &&
         (node->getLocalIndex() !=
          0)) /* && (!node->getOpCode().isStore()) */) {
      if (_relevantNodes->get(node->getLocalIndex())) return true;
    } else if (node->getOpCode().isTwoChildrenAddress()) {
      TR::Node *firstChild = node->getFirstChild();
      TR::Node *secondChild = node->getSecondChild();
      if (((firstChild->getLocalIndex() != MAX_SCOUNT) &&
           (firstChild->getLocalIndex() !=
            0)) /* && (!firstChild->getOpCode().isStore()) */) {
        if (_relevantNodes->get(firstChild->getLocalIndex())) return true;
      }

      if (((secondChild->getLocalIndex() != MAX_SCOUNT) &&
           (secondChild->getLocalIndex() !=
            0)) /* && (!secondChild->getOpCode().isStore()) */) {
        if (_relevantNodes->get(secondChild->getLocalIndex())) return true;
      }
    }
    return false;
  }
  ...
  bool childRelevant = false, containsIndirectAccess = false,
       containsArrayAccess = false, containsDivide = false,
    ...
    if (includeRelevantNodes(node->getChild(i), visitCount, blockNum) &&
      ...
      if (child->getOpCode().isTwoChildrenAddress()) {
        ...
        if (((firstChild->getLocalIndex() != MAX_SCOUNT) &&
          ...
          if (_exprsContainingDivide->get(firstChild->getLocalIndex()))
            containsDivide = true;
          if (_exprsContainingUnresolvedAccess->get(
                  firstChild->getLocalIndex()))
            containsUnresolvedAccess = true;
        }
  ...
  if (((node->getLocalIndex() != MAX_SCOUNT) &&
    ...
    if ((!_actualRednSetInfo[blockNum]->get(node->getLocalIndex())) &&
      ...
      if (containsArrayAccess || node->getOpCode().isTwoChildrenAddress()) {
        _exprsContainingArrayAccess->set(node->getLocalIndex());
        if (_arrayAccessesKilled->get(blockNum)) {
          // if (!checkIfNodeCanSomehowSurvive(node,
          // _indirectAccessesThatSurvive))
          relevantNodeKilled = true;
        }
      }
      ...
      if (!(childRelevant ||
        ...
        */
        ...
        if (containsIndirectAccess ||
            ((opCode.isIndirect() &&
              (opCode.isLoadVar() || opCode.isStore())) ||
             opCode.isArrayLength()))
          _exprsContainingIndirectAccess->set(node->getLocalIndex());
        if (containsArrayAccess || node->getOpCode().isTwoChildrenAddress())
          _exprsContainingArrayAccess->set(node->getLocalIndex());
...
void TR_ExceptionCheckMotion::initializeOutLists(List<TR::Node> **lists) {
  int32_t i;
  for (i = 0; i < _numberOfNodes; i++) lists[i]->deleteAll();
}
...
bool TR_ExceptionCheckMotion::analyzeRegionStructure(
  ...
  /////if (regionStructure != comp()->getFlowGraph()->getStructure())
  {
    ...
    for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
      ...
      if (analysisInfo->_outList[toStructureNumber] != NULL) {
        copyListFromInto(_currentList[toStructureNumber],
                         analysisInfo->_outList[toStructureNumber]);
      }
...

  {
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
      if (subNode->getSuccessors().empty() &&
          subNode->getExceptionSuccessors().empty())
        exitNodes->set(subNode->getNumber());
    }
  }

  TR_BitVector *pendingList = new (trStackMemory())
      TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  TR_BitVector *originalPendingList = new (trStackMemory())
      TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  ...
  while (changed) {
    ...
    *pendingList |= *originalPendingList;
...

    {
      ei.reset();
      for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
        TR_StructureSubGraphNode *edgeFrom =
            toStructureSubGraphNode(edge->getFrom());
        addToAnalysisQueue(edgeFrom, 0);
        if (analyzeNodeIfSuccessorsAnalyzed(edgeFrom, regionStructure,
                                            pendingList, exitNodes))
          changed = true;
      }
    }

    {
      ...
      for (subNode = si.getCurrent(); subNode; subNode = si.getNext()) {
        if (subNode->getSuccessors().empty() &&
          ...
          if (analyzeNodeIfSuccessorsAnalyzed(subNode, regionStructure,
                                              pendingList, exitNodes))
            changed = true;
        }
      }
  ...
  List<TR::Node> *entryList =
      getAnalysisInfo(regionStructure->getEntry()->getStructure())->_inList;
  ...
  while (_analysisQueue.getListHead() &&
    ...
    if (_analysisInterrupted) {
      comp()->failCompilation<TR::CompilationInterrupted>(
          "interrupted in forward bit vector analysis");
    }
    ...
    ExprDominanceInfo *analysisInfo =
        getAnalysisInfo(nodeStructure->getStructure());
    ...
    {
      for (auto succ = nodeStructure->getSuccessors().begin();
           succ != nodeStructure->getSuccessors().end(); ++succ) {
        if (!regionStructure->isExitEdge(*succ)) {
          TR_StructureSubGraphNode *succNode =
              toStructureSubGraphNode((*succ)->getTo());
          TR_Structure *succStructure = succNode->getStructure();

          if (!(isNaturalLoop && (succNode == regionStructure->getEntry()))) {
            if (pendingList->get(succNode->getNumber()) &&
                (!alreadyVisitedNode)) {
              removeHeadFromAnalysisQueue();
              addToAnalysisQueue(succNode, 0);
              stopAnalyzingThisNode = true;
              break;
              // return analyzeNodeIfSuccessorsAnalyzed(succNode,
              // regionStructure, pendingList, exitNodes);
            }

            if (!(pendingList->get(succNode->getNumber()) &&
                  !succStructure->hasBeenAnalyzedBefore())) {
              if (!seenNodes.ValueAt(succStructure->getNumber()))
                initializeOutList(_currentList[succNode->getNumber()]);
              List<TR::Node> *succInList =
                  getAnalysisInfo(succStructure)->_inList;
      ...
      if (stopAnalyzingThisNode) continue;
        ...
        if (!regionStructure->isExitEdge(*succ)) {
          ...
          if (!(isNaturalLoop && (succNode == regionStructure->getEntry()))) {
            ...
            if (!(pendingList->get(succNode->getNumber()) &&
              ...
              if (!seenNodes.ValueAt(succStructure->getNumber()))
                initializeOutList(_currentList[succNode->getNumber()]);
              List<TR::Node> *esuccInList =
                  getAnalysisInfo(succStructure)->_inList;
              copyListFromInto(esuccInList,
                               _currentList[succNode->getNumber()]);
            } else
              _currentList[succNode->getNumber()]->deleteAll();
          }
    ...
    if (exitNodes->get(nodeStructure->getNumber())) {
      if (!(nodeStructure->getSuccessors().empty() &&
            nodeStructure->getExceptionSuccessors().empty())) {
        ExprDominanceInfo *regionExtraAnalysisInfo =
            getAnalysisInfo(regionStructure);
        int32_t i;
        for (i = 0; i < _numberOfNodes; i++) {
          if ((regionExtraAnalysisInfo->_outList[i] != NULL) &&
              (analysisInfo->_outList[i] != NULL)) {
            copyListFromInto(regionExtraAnalysisInfo->_outList[i],
                             _currentList[i]);
          }
        }
      } else {
        int32_t i;
        for (i = 0; i < _numberOfNodes; i++) {
          if (analysisInfo->_outList[i] != NULL) _currentList[i]->deleteAll();
        }
      }
    }
    ...
    _nodesInCycle->empty();
    bool inSetChanged =
        nodeStructure->getStructure()->doDataFlowAnalysis(this, checkForChange);
    if (pendingList->get(node->getNumber())) inSetChanged = true;
    ...
    for (auto pred = nodeStructure->getPredecessors().begin();
         pred != nodeStructure->getPredecessors().end(); ++pred) {
      TR_StructureSubGraphNode *predNode =
          (TR_StructureSubGraphNode *)(*pred)->getFrom();
      TR_Structure *predStructure = predNode->getStructure();
      if (pendingList->get(predStructure->getNumber()) || inSetChanged) {
        // comp()->incVisitCount();
        _nodesInCycle->empty();
        // if (analyzeNodeIfSuccessorsAnalyzed(pred->getFrom(), regionStructure,
        // pendingList, exitNodes))
        //   changed = true;
    ...
    if (changed) anyNodeChanged = true;
  }

  return anyNodeChanged;
}

bool TR_ExceptionCheckMotion::analyzeBlockStructure(
    TR_BlockStructure *blockStructure, bool checkForChange) {
  ...
  *_tempContainer &= *_partialRedundancy->_isolatedness->_latestness
                          ->_delayedness->_earliestness->_globalAnticipatability
                          ->_localTransparency.getAnalysisInfo(blockNum);
  TR::Node **supportedNodesAsArray =
      _partialRedundancy->_isolatedness->_supportedNodesAsArray;
  _composeHelper->empty();
  ContainerType::Cursor bvi(*_tempContainer);
  for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
    int32_t nextAvailableComputation = bvi;
    if (nextAvailableComputation == 0) continue;

    TR::Node *nextAvailableNode =
        supportedNodesAsArray[nextAvailableComputation];
    TR::ILOpCode &opCode = nextAvailableNode->getOpCode();

    if (opCode.getOpCodeValue() == TR::NULLCHK) {
      _composeHelper->set(
          nextAvailableNode->getNullCheckReference()->getLocalIndex());
    }
  }
  ...
  *_tempContainer = *_composeHelper;
  ...
  if (_nullCheckKilled->get(blockNum)) nullCheckKilled = true;
  bool resolveCheckKilled = false;
  if (_resolveCheckKilled->get(blockNum)) resolveCheckKilled = true;
  bool boundCheckKilled = false;
  if (_boundCheckKilled->get(blockNum)) boundCheckKilled = true;
  bool divCheckKilled = false;
  if (_divCheckKilled->get(blockNum)) divCheckKilled = true;
  bool checkCastKilled = false;
  if (_checkCastKilled->get(blockNum)) checkCastKilled = true;
  bool indirectAccessesKilled = false;
  if (_indirectAccessesKilled->get(blockNum)) indirectAccessesKilled = true;
  bool arrayAccessesKilled = false;
  if (_arrayAccessesKilled->get(blockNum)) arrayAccessesKilled = true;
  bool unresolvedAccessesKilled = false;
  if (_unresolvedAccessesKilled->get(blockNum)) unresolvedAccessesKilled = true;
  bool dividesKilled = false;
  if (_dividesKilled->get(blockNum)) dividesKilled = true;
  bool arrayStoreCheckKilled = false;
  if (_arrayStoreCheckKilled->get(blockNum)) arrayStoreCheckKilled = true;
  bool arrayCheckKilled = false;
  if (_arrayCheckKilled->get(blockNum)) arrayCheckKilled = true;
  ...
  *_tempContainer &= *_partialRedundancy->_isolatedness->_latestness
  ...
  for (bvi2.SetToFirstOne(); bvi2.Valid(); bvi2.SetToNextOne()) {
    ...
    if (opCode.getOpCodeValue() == TR::NULLCHK) {
      markNodeAsSurvivor(nextAvailableNode->getNullCheckReference(),
                         _knownIndirectAccessesThatSurvive);
    } else if (opCode.isIndirect() || opCode.isArrayLength()) {
      markNodeAsSurvivor(nextAvailableNode->getFirstChild(),
                         _knownIndirectAccessesThatSurvive);
    } else if (opCode.isResolveCheck()) {
      markNodeAsSurvivor(nextAvailableNode->getFirstChild(),
                         _knownUnresolvedAccessesThatSurvive);
    } else if (opCode.getOpCodeValue() == TR::DIVCHK) {
      markNodeAsSurvivor(nextAvailableNode->getFirstChild()->getSecondChild(),
                         _knownDividesThatSurvive);
    } else if (opCode.isDiv() || opCode.isRem()) {
      markNodeAsSurvivor(nextAvailableNode->getSecondChild(),
                         _knownDividesThatSurvive);
    }
    /*
  ...
  *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
  ...
  // Use the information about this block collected earlier (during gen/kill
  // list generation) to prune the incoming ordered list from the successors.
  //
  ListElement<TR::Node> *prevElem = NULL;
  ListElement<TR::Node> *listElem;
  for (listElem = _workingList.getListHead(); listElem != NULL;
    ...
    // kills indirect accesses then remove this expression from the incoming
    // list; also mark this expression as one that will kill its parent (because
    // it was killed) if the parent is a check taking part in this analysis
    //
    ...
    // kinds cannot move past this point. The code below implements this kind of
    // logic for all the different check opcodes.
    //
    if (opCode.getOpCodeValue() == TR::NULLCHK) {
      ...
      if (opCode.getOpCodeValue() == TR::ResolveAndNULLCHK) {
        if (nullCheckKilled) nodeKilled = true;
      }
    ...
    } else if (opCode.isCheckCast())  /// opCode.getOpCodeValue() ==
                                      /// TR::checkcast)
    {
    ...
    if ((prevElem && (prevElem->getNextElement() == listElem)) ||
        (_workingList.getListHead() == listElem))
      prevElem = listElem;
  }
  ...
  // iterations of expression dominance) in which case we would like to append
  // the current solution to the already existing opt list. Also we would like
  // to prune the ordered list going out of this block (OUT in dataflow terms)
  // based on the check expressions already in the ordered opt list for this
  // block; this is reqd as these expressions in the existing ordered opt list
  // for this block were not taken into account in gen and kill
  // generation/initialization time
  //
  ...
  if (_orderedOptList[blockNum]) {
    ...
    for (nextOptElement = _orderedOptList[blockNum]->getListHead();
         nextOptElement != NULL;
  ...
  *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
  ...
  for (listElem = _blockInfo[blockNum]->getListHead(); listElem != NULL;
       listElem = listElem->getNextElement()) {
    ...
    if (!_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())) {
      ...
      if (placedEarlierCheckOptimally) {
        //
        // If a null check has been placed as an opt expression earlier in this
        // block then kill this expression if it contains an indirect access as
        // it is not allowed to cross this block (it is not allowed to move past
        // the check)
        //
        if (_exprsContainingIndirectAccess->get(node->getLocalIndex())) {
          if (indirectAccessesKilled) {
      ...
      if (opCode.getOpCodeValue() == TR::NULLCHK) {
        ...
        if (placedEarlierCheckOptimally &&
            (nullCheckKilled || checkKilledByChild)) {
          removeFromList(listElem, _blockInfo[blockNum], prevElem);
          boundCheckKilled = true;
          divCheckKilled = true;
          arrayStoreCheckKilled = true;
        ...
        if (placedEarlierCheckOptimally &&
            (boundCheckKilled || checkKilledByChild)) {
          removeFromList(listElem, _blockInfo[blockNum], prevElem);
          nullCheckKilled = true;
          resolveCheckKilled = true;
          divCheckKilled = true;
        ...
        if (placedEarlierCheckOptimally &&
            (divCheckKilled || checkKilledByChild)) {
          removeFromList(listElem, _blockInfo[blockNum], prevElem);
          resolveCheckKilled = true;
          nullCheckKilled = true;
          boundCheckKilled = true;
        ...
        if (placedEarlierCheckOptimally &&
            (arrayStoreCheckKilled || checkKilledByChild)) {
          removeFromList(listElem, _blockInfo[blockNum], prevElem);
          nullCheckKilled = true;
          resolveCheckKilled = true;
          boundCheckKilled = true;
                                        ...
                                        /// TR::checkcast)
      {
        bool alreadyRemoved = false;
        if (placedEarlierCheckOptimally &&
            (checkCastKilled || checkKilledByChild)) {
          removeFromList(listElem, _blockInfo[blockNum], prevElem);
          resolveCheckKilled = true;
          nullCheckKilled = true;
          boundCheckKilled = true;
      ...
      // that requires a check (and it has not been placed optimally in this
      // block). Note that if the required check has been placed optimally in
      // this block or earlier, then we will go ahead and place this expression
      // optimally in this block. This is driven by the optimalACCESSKilled
      // flags rather than the ACCESSKilled flags (which are really used to
      // limit which expressions can stay in the out list for this block
      // eventually)
      //
      if (_exprsContainingIndirectAccess->get(
              listElem->getData()->getLocalIndex()) &&
          optimalIndirectAccessesKilled)
        canPlaceOptimally = false;
      else if (_exprsContainingArrayAccess->get(
                   listElem->getData()->getLocalIndex()) &&
               optimalArrayAccessesKilled)
        canPlaceOptimally = false;
      else if (_exprsContainingDivide->get(
                   listElem->getData()->getLocalIndex()) &&
               optimalDividesKilled)
        canPlaceOptimally = false;
      else if (_exprsContainingUnresolvedAccess->get(
                   listElem->getData()->getLocalIndex()) &&
      ...
      //
      if (canPlaceOptimally && !_actualOptSetInfo[blockNum]->get(
                                   listElem->getData()->getLocalIndex())) {
        _tryAnotherIteration = true;
        if (trace())
          traceMsg(comp(),
                   "IN ORDER Expr %d (representative) Node %p (listElem %p) in "
                   "Block : %d\n",
                   listElem->getData()->getLocalIndex(), listElem->getData(),
                   listElem, blockNum);
        ...
        if (checkElement) {
                         ...
                         .isCheckCast())  /// opCodeValue == TR::checkcast)
          {
            ...
            divCheckKilledByOptList = true;
            indirectAccessesKilledByOptList = true;
            if (listElem->getData()->getOpCodeValue() ==
    ...
    if ((prevElem && (prevElem->getNextElement() == listElem)) ||
        (_blockInfo[blockNum]->getListHead() == listElem))
      prevElem = listElem;
  }
  ...
  *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
  ...
  // This is the list for this block; note that this is not what we can flow
  // backwards from this point Some expressions that are computed in this block
  // are okay to keep in this block but not okay to "move" any further back.
  //
  _workingList.deleteAll();
  ...
  for (listElem = _blockInfo[blockNum]->getListHead(); listElem != NULL;
    ...
    if (_exprsContainingArrayAccess->get(
            listElem->getData()->getLocalIndex())) {
    ...
    if (opCode.getOpCodeValue() == TR::NULLCHK) {
      if (nullCheckKilledByOptList || checkKilledByChild ||
          genExprThatIsKilled ||
          _actualOptSetInfo[blockNum]->get(
              listElem->getData()->getLocalIndex())) {
        removeFromList(listElem, _blockInfo[blockNum], prevElem);
        boundCheckKilledByOptList = true;
        resolveCheckKilledByOptList = true;
      ...
      if (opCode.getOpCodeValue() == TR::ResolveAndNULLCHK) {
        if (nullCheckKilledByOptList) nodeKilledByOptList = true;
      }
      ...
      if (boundCheckKilledByOptList || checkKilledByChild ||
          genExprThatIsKilled ||
          _actualOptSetInfo[blockNum]->get(
              listElem->getData()->getLocalIndex())) {
        removeFromList(listElem, _blockInfo[blockNum], prevElem);
      ...
      if (divCheckKilledByOptList || checkKilledByChild ||
          genExprThatIsKilled ||
          _actualOptSetInfo[blockNum]->get(
              listElem->getData()->getLocalIndex())) {
        removeFromList(listElem, _blockInfo[blockNum], prevElem);
      ...
      if (arrayStoreCheckKilledByOptList || checkKilledByChild ||
          genExprThatIsKilled ||
          _actualOptSetInfo[blockNum]->get(
              listElem->getData()->getLocalIndex())) {
        removeFromList(listElem, _blockInfo[blockNum], prevElem);
    ...
    {
      bool alreadyRemoved = false;
      if (checkCastKilledByOptList || checkKilledByChild ||
          genExprThatIsKilled ||
          _actualOptSetInfo[blockNum]->get(
              listElem->getData()->getLocalIndex())) {
        removeFromList(listElem, _blockInfo[blockNum], prevElem);
  ...
  if (checkForChange &&
      !compareLists(analysisInfo->_inList, _blockInfo[blockNum]))
    changed = true;
...
bool TR_ExceptionCheckMotion::checkIfNodeCanSomehowSurvive(
    TR::Node *node, ContainerType *nodesThatSurvive) {
  ...
  if (!_exprsUnaffectedByOrder->get(node->getLocalIndex())) {
    ...
    if (((opCode.isIndirect() || (opCode.isArrayLength())) &&
        ...
        (opCode.hasSymbolReference() &&
         node->getSymbolReference()->isUnresolved()) ||
        ((opCode.isDiv() || opCode.isRem()) &&
         !checkIfNodeCanSurvive(node->getSecondChild(), nodesThatSurvive)))
      return false;
  }
    ...
    if (child->getOpCode().isTwoChildrenAddress()) {
      ...
      if (!checkIfNodeCanSurvive(secondChild, nodesThatSurvive)) {
        ...
        break;
               ...
               (node->getOpCode().isDiv() || node->getOpCode().isRem()) &&
...
bool TR_ExceptionCheckMotion::checkIfNodeCanSurvive(
    TR::Node *node, ContainerType *nodesThatSurvive) {
...
void TR_ExceptionCheckMotion::markNodeAsSurvivor(
    TR::Node *node, ContainerType *nodesThatSurvive) {
  if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) {
    nodesThatSurvive->set(node->getLocalIndex());
  }
...
bool TR_ExceptionCheckMotion::isNodeValueZero(TR::Node *node) {
  ...
  if (opCode.isConversion())
    ...
    switch (node->getDataType()) {
      case TR::Int8:
        if (node->getByte() == 0) return true;
        break;
      case TR::Int16:
        if (node->getShortInt() == 0) return true;
        break;
      case TR::Int32:
        if (node->getInt() == 0) return true;
        break;
      case TR::Int64:
        if (node->getLongInt() == 0L) return true;
        break;
      case TR::Float:
        if (node->getFloat() == 0.0) return true;
        break;
      case TR::Double:
        if (node->getDouble() == 0.0) return true;
        break;
      case TR::Address:
        if (node->getAddress() == 0) return true;
        break;
      default:
        break;
    }
...
void TR_ExceptionCheckMotion::removeFromList(ListElement<TR::Node> *listElem,
                                             List<TR::Node> *list,
...
TR_RedundantExpressionAdjustment *
TR_RedundantExpressionAdjustment::asRedundantExpressionAdjustment() {
  return this;
}
...
void TR_RedundantExpressionAdjustment::analyzeNode(TR::Node *, vcount_t,
                                                   TR_BlockStructure *,
                                                   ContainerType *) {}
...
//
TR_RedundantExpressionAdjustment::TR_RedundantExpressionAdjustment(
    TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_Structure *rootStructure, TR_ExceptionCheckMotion *exceptionCheckMotion)
    : TR_IntersectionBitVectorAnalysis(comp, comp->getFlowGraph(), optimizer,
                                       exceptionCheckMotion->trace()) {
  if (trace()) traceMsg(comp, "Starting Redundant expression adjustment\n");
...
void TR_RedundantExpressionAdjustment::initializeGenAndKillSetInfo() {
  // For each block in the CFG build the gen and kill set for this analysis.
  //
  ContainerType **actualOptSetInfo =
      _exceptionCheckMotion->getActualOptSetInfo();
  ...
  for (i = 1; i < _numberOfNodes; ++i) {
    ...
    if (actualOptSetInfo[i]) {
      //
      // The gen sets are all those expressions that are actually optimal in
      // this block, thus making them available in the successor blocks where
      // some of those expressions would be redundant
      //
      *_regularGenSetInfo[i] = *(origOptSetInfo[i]);
      *_regularGenSetInfo[i] -= *(_partialRedundancy->getSymOptimalNodes());
      *_regularGenSetInfo[i] &=
          *_partialRedundancy->_isolatedness->_latestness->_delayedness
               ->_earliestness->_globalAnticipatability->_localAnticipatability
               .getDownwardExposedAnalysisInfo(i);
      ...
      *_optSetHelper -=
          *_partialRedundancy->_isolatedness->_latestness->_delayedness
               ->_earliestness->_globalAnticipatability->_localTransparency
               .getAnalysisInfo(i);

      // We ran into a problem where we were caching the array length too early
      // and later
      //              reallocating a larger array thus resulting an a stale
      //              assumption which produces an AIOOB.
      // *_optSetHelper -=
...
bool TR_RedundantExpressionAdjustment::analyzeBlockStructure(
    TR_BlockStructure *blockStructure, bool checkForChange) {
  ...
  if (_exceptionCheckMotion->getTrySecondBestSolution()) {
    ContainerType **actualOptSetInfo =
        _exceptionCheckMotion->getActualOptSetInfo();
    int32_t **orderedOptNumbersList =
        _exceptionCheckMotion->getOrderedOptNumbersList();
    List<TR::Node> **genSetInfo = _exceptionCheckMotion->getGenSetList();
    ContainerType **optimisticRednSetInfo =
        _partialRedundancy->getRednSetInfo();
    ContainerType **optimisticOptSetInfo = _partialRedundancy->getOptSetInfo();
    ContainerType *exprsUnaffectedByOrder =
        _exceptionCheckMotion->getExprsUnaffectedByOrder();
    int32_t totalMaxSize = optimisticOptSetInfo[blockNum]->elementCount() +
                           optimisticRednSetInfo[blockNum]->elementCount();
    ...
    int32_t *orderedOldOptNumbersList =
        (int32_t *)trMemory()->allocateStackMemory(oldMaxSize *
                                                   sizeof(int32_t));
    j = 0;
    _optSetHelper->empty();
    while (j < oldMaxSize) {
    ...
    ListElement<TR::Node> *prevElem = NULL;
    ListElement<TR::Node> *listElem;
    for (listElem = genSetInfo[blockNum]->getListHead(); listElem != NULL;
         listElem = listElem->getNextElement()) {
      /*
      int32_t weight = 1;
      ...
      */
      ...
      if (trace())
        traceMsg(comp(),
                 "CONSIDERING Expr %d (representative) Node %p (listElem %p) "
                 "in Block : %d\n",
                 listElem->getData()->getLocalIndex(), listElem->getData(),
                 listElem, blockNum);
      if (/*_optSetHelper->get(listElem->getData()->getLocalIndex()) || */
          ((optimisticRednSetInfo[blockNum]->get(
               listElem->getData()->getLocalIndex())) &&
           (!_currentInSetInfo->get(listElem->getData()->getLocalIndex())) &&
           (!actualOptSetInfo[blockNum]->get(
               listElem->getData()->getLocalIndex())))) {
        if (trace())
          traceMsg(comp(),
                   "IN ORDER Expr %d (representative) Node %p (listElem %p) in "
        ...
        _exceptionGenSetInfo[blockNum]->set(
            listElem->getData()->getLocalIndex());
        ...
        orderedOptNumbersList[blockNum][j++] =
            listElem->getData()->getLocalIndex();
      }
    ...
    for (; j < totalMaxSize;) orderedOptNumbersList[blockNum][j++] = -1;
  }
  ...
  if (blockStructure->getNumber() == 0)
    ...
    if (_regularGenSetInfo) {
      ...
      copyFromInto(analysisInfo->_inSetInfo,
                   _blockAnalysisInfo[blockStructure->getNumber()]);
    } else
  ...
  if ((block->getSuccessors().size() == 2) && block->getExit()) {
    ...
    if (((branch->getOpCodeValue() == TR::ifacmpne) ||
      ...
      if (((nullCheckReferenceIndex != MAX_SCOUNT) &&
           (nullCheckReferenceIndex != 0))) {
        if (_regularInfo->get(nullCheckReferenceIndex)) {
          ...
          while (j < numberOfBits) {
            ...
            if (nextOptimalNode->getOpCodeValue() == TR::NULLCHK) {
              if (nextOptimalNode->getNullCheckReference()->getLocalIndex() ==
                  nullCheckReferenceIndex) {
                nullCheckNumber = j;
                if (branch->getOpCodeValue() == TR::ifacmpne)
                  nullConstraintBlock =
                      branch->getBranchDestination()->getNode()->getBlock();
                else
  ...
  for (auto succ = block->getSuccessors().begin();
       succ != block->getSuccessors().end(); ++succ) {
    outSetInfo = analysisInfo->getContainer(analysisInfo->_outSetInfo,
                                            (*succ)->getTo()->getNumber());
    bool addedNullConstraint = false;

    if (nullConstraintBlock && (nullConstraintBlock == (*succ)->getTo()) &&
    ...
    *outSetInfo = *_regularInfo;
    ...
    if (addedNullConstraint) _regularInfo->reset(nullCheckNumber);
  }
  ...
  if (trace()) {
    ...
    for (pair = analysisInfo->_outSetInfo->getFirst(); pair;
         pair = pair->getNext()) {
