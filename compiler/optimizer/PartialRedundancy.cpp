...
inline bool
TR_PartialRedundancy::isSupportedOpCode(TR::Node* node, TR::Node* parent)
{
  return (TR_LocalAnalysis::isSupportedNode(node, comp(), parent));
}

static void
resetChildrensVisitCounts(TR::Node* node, vcount_t count)
{
...
#ifdef J9_PROJECT_SPECIFIC
static void
correctDecimalPrecision(TR::Node* store, TR::Node* child, TR::Compilation* comp)
{
  if (child->getType().isBCD() &&
    ...
    if (comp->cg()->traceBCDCodeGen())
      traceMsg(comp,
               ...
               OPT_DETAILS,
               modPrecNode->getOpCode().getName(),
               modPrecNode,
               child->getDecimalPrecision(),
               store->getDecimalPrecision(),
               isTruncation ? "yes" : "no",
...
static bool
shouldAllowOptimalSubNodeReplacement(TR::Compilation* comp)
{
...
static bool
shouldAllowLimitingOfOptimalPlacement(TR::Compilation* comp)
{
...
TR_PartialRedundancy::ContainerType*
TR_PartialRedundancy::allocateContainer(int32_t size)
{
  return new (trStackMemory()) TR_BitVector(size, trMemory(), stackAlloc);
}

static bool
ignoreValueOfMultipleReturnNode(TR::Node* node, TR::SparseBitVector& seenNodes)
{
...
bool
TR_PartialRedundancy::ignoreNode(TR::Node* node)
{
  TR::ILOpCodes op = node->getOpCodeValue();
  TR::CodeGenerator* cg = comp()->cg();
  TR::SymbolReference* symRef =
    node->getOpCode().hasSymbolReference() ? node->getSymbolReference() : NULL;
...
TR_PartialRedundancy::perform()
{
  ...
  _newSymbolReferences = (TR::SymbolReference**)trMemory()->allocateStackMemory(
    _numberOfBits * sizeof(TR::SymbolReference*));
  _newSymbolsMap =
    (int32_t*)trMemory()->allocateStackMemory(_numberOfBits * sizeof(int32_t));
  _registerCandidates = (TR_RegisterCandidate**)trMemory()->allocateStackMemory(
      ...
      if ((_newSymbolsMap[nextOptimalComputation] < 0) &&
        ...
        if (nextOptimalNode->getOpCode().isLoadVarDirect() &&
          ...
          if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType()) !=
                nextOptimalNode
                  ->getDataType() && // only do it for Java where we use istore
                                     // for small intergral types
          ...
          TR::SymbolReference* newSymbolReference =
            comp()->getSymRefTab()->createTemporary(
              comp()->getMethodSymbol(), dataType, false, size);
  ...
  for (block = comp()->getStartBlock(); block != NULL;
    ...
    if ((block->getEntry() != NULL) /* &&
      ...
      eliminateRedundantComputations(
        block, supportedNodesAsArray, _rednSetInfo, block->getEntry());
    }
  }
...
void
TR_PartialRedundancy::invalidateOptimalComputation(
...
//
TR::Node*
TR_PartialRedundancy::duplicateExact(TR::Node* node,
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    if (node->getChild(i)->getVisitCount() != visitCount)
      newRoot->setChild(
        i,
        duplicateExact(
          node->getChild(i), seenNodes, duplicateNodes, visitCount));
    else {
...
void
TR_PartialRedundancy::calculateGlobalRegisterWeightsBasedOnStructure(
...
bool
TR_PartialRedundancy::isNodeAnImplicitNoOp(TR::Node* node)
{
...
//
bool
TR_PartialRedundancy::isOpCodeAnImplicitNoOp(TR::ILOpCode& opCode)
{
...
void
TR_PartialRedundancy::processReusedNode(TR::Node* node,
                                        ...
                                        int newNumChildren)
{
  ...
  if (comp()->cg()->traceBCDCodeGen())
    traceMsg(
      comp(), "reusing %s (%p) as op ", node->getOpCode().getName(), node);
...
TR::TreeTop*
TR_PartialRedundancy::placeComputationsOptimally(
  TR::Block* block,
  TR::Node*** supportedNodesAsArray)
{
  ...
  if (trace())
    traceMsg(comp(),
             "Placing computations optimally in block number %d\n",
  ...
  for (nextOptimalComputation = orderedOptNumbersList[currOptimalElement];
       seenOptimalElements < numOptimalElements;
       currOptimalElement++) {
        ...
        if (nextOptimalNode->getOpCode().isCheck()) {
          ...
          if (optimalNode &&
            ...
            if (!shouldAllowLimitingOfOptimalPlacement(comp()) ||
                performTransformation(
                  comp(),
                  "%s0Placing %s optimally : %p using symRef #%d\n",
                  OPT_DETAILS,
                  optimalNode->getOpCode().getName(),
                  optimalNode,
                  newSymbolReference->getReferenceNumber())) {
              ...
              if (comp()->useCompressedPointers()) {
                if (convertedOptimalNode->getOpCode().isLoadIndirect() &&
                  ...
                  translateTT = TR::TreeTop::create(
                    comp(),
                    TR::Node::createCompressedRefsAnchor(convertedOptimalNode),
                    NULL,
                    NULL);
                }
              }
              ...
              TR::Node* storeForCommonedNode = TR::Node::createWithSymRef(
                comp()->il.opCodeForDirectStore(nextOptimalNode->getDataType()),
                1,
                1,
                convertedOptimalNode,
                newSymbolReference);
              ...
              correctDecimalPrecision(
                storeForCommonedNode, convertedOptimalNode, comp());
#endif
    ...
    if (nextOptimalNode->getOpCode().isStore()) {
      ...
      if ((nextOptimalNode->getOpCodeValue() == TR::NULLCHK) ||
          ...
          if (((nullCheckReferenceNode->getLocalIndex() != MAX_SCOUNT) &&
            ...
            if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] >
              ...
              newLoad = TR::Node::createWithSymRef(
                ...
                comp()->il.opCodeForDirectLoad(nullCheckReferenceDataType),
                0,
                newSymbolReference);
        ...
        if (!replacedNullCheckReference) {
          if (trace())
            traceMsg(comp(),
                     "Trying to replace OPTIMAL SUBNODES in : %p\n",
                     nextOptimalNode);
          int32_t k;
          for (k = 0;
            ...
            placeToInsertOptimalComputations = replaceOptimalSubNodes(
              placeToInsertOptimalComputations,
              nextOptimalNode->getNullCheckReference(),
              nextOptimalNode->getNullCheckReference()->getChild(k),
              k,
              duplicateOptimalNode,
              duplicateOptimalNode->getChild(k),
              false,
              block->getNumber(),
              visitCount4);
          }
        ...
        for (k = 0; k < nextOptimalNode->getNumChildren(); k++) {
          vcount_t visitCount4 = comp()->incVisitCount();
          placeToInsertOptimalComputations =
            replaceOptimalSubNodes(placeToInsertOptimalComputations,
                                   nextOptimalNode,
                                   nextOptimalNode->getChild(k),
                                   k,
                                   duplicateOptimalNode,
                                   duplicateOptimalNode->getChild(k),
                                   false,
      ...
      dumpOptDetails(comp(),
                     "%s1Placing %s optimally : %p using symRef #%d\n",
                     OPT_DETAILS,
                     convertedOptimalNode->getOpCode().getName(),
                     convertedOptimalNode,
                     newSymbolReference->getReferenceNumber());
      ...
      if (comp()->useCompressedPointers()) {
        ...
        if (reference->getOpCode().isLoadIndirect() ||
          ...
          if (reference->getFirstChild()->getOpCode().isLoadIndirect() &&
            ...
            translateTT = TR::TreeTop::create(
              comp(),
              TR::Node::createCompressedRefsAnchor(reference->getFirstChild()),
              NULL,
              NULL);
            placeToInsertOptimalComputations->join(translateTT);
            translateTT->join(secondTreeInBlock);
      ...
      if (duplicateOptimalNode->getOpCode().isCall() &&
        ...
        TR::TreeTop* anchorTT = TR::TreeTop::create(
          comp(),
          TR::Node::create(TR::treetop, 1, duplicateOptimalNode),
          NULL,
          NULL);
...
//
TR::Node*
TR_PartialRedundancy::getAlreadyPresentOptimalNode(
  TR::Node* node,
  int32_t nextOptimalComputation,
  vcount_t visitCount,
  bool& rhsOfStore)
{
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    TR::Node* child = getAlreadyPresentOptimalNode(
      node->getChild(childNum), nextOptimalComputation, visitCount, rhsOfStore);
    if (child)
      return child;
  }
...
isExpressionRedundant(
  TR::Node* node,
  TR_PartialRedundancy::ContainerType* redundantComputations,
...
//
void
TR_PartialRedundancy::eliminateRedundantComputations(
  TR::Block* block,
  TR::Node** supportedNodesAsArray,
  ContainerType** rednSetInfo,
  TR::TreeTop* startTree)
{
  ...
  if (trace())
    traceMsg(
      comp(),
      "Eliminating redundant computations in block number %d visit count %d\n",
      block->getStructureOf()->getNumber(),
      visitCount);
...

  {
    ...
    while (currentTree != exitTree) {
      ...
      if ((firstOpCodeInTree.isStore() && !currentTree->getNode()
        ...
        //
        if (isExpressionRedundant(currentTree->getNode(),
          ...
          if (!currentTree->getNode()->getOpCode().isStore()) {
            if (0 &&
                performTransformation(
                  comp(),
                  "%sEliminating redundant check computation (resolve or null "
                  "check) : %p\n",
                  OPT_DETAILS,
                  currentTree
                    ->getNode())) // Look at check-in comment for version 1.296
            {
            ...
            if (newSymbolReference /* &&
              ...
              dumpOptDetails(
                comp(),
                "%sEliminating redundant computation (store) : %p\n",
                OPT_DETAILS,
                currentTree->getNode());
...
void
TR_PartialRedundancy::printTrees()
{
  comp()->incVisitCount();
...
//
void
TR_PartialRedundancy::collectAllNodesToBeAnchored(List<TR::Node>* anchoredNodes,
                                                  TR::Node* node,
                                                  vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
...
//
bool
TR_PartialRedundancy::eliminateRedundantSupportedNodes(
  TR::Node* parent,
  TR::Node* node,
  bool firstComputation,
  ...
  TR::Node** supportedNodesAsArray)
{
  ...
  if (visitCount <= node->getVisitCount()) {
    if (isExpressionRedundant(
          node, redundantComputations, anticipatabilityInfo)) {
      //
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    TR::Node* child = node->getChild(i);
    if (!eliminateRedundantSupportedNodes(node,
                                          child,
                                          flag,
                                          currentTree,
                                          blockNum,
    ...
    if (_profilingWalk &&
      ...
      if (comp()->isProfilingCompilation() &&
        ...
        if ((node->getType().isInt64()) && profileLongParms) {
          ...
          if (trace())
            traceMsg(comp(),
                     "Added profiling instrumentation for %p(%d)\n",
                     node,
    ...
    if (isExpressionRedundant(
      ...
      if (!isNodeAnImplicitNoOp(node)) {
        ...
        for (j = 0; j < node->getNumChildren(); j++)
          collectAllNodesToBeAnchored(
            &anchoredNodes, node->getChild(j), visitCount + 1);
        ...
        if (newSymbolReference &&
          ...
          if (node->getDataType() == TR::NoType) {
            ...
            if (parent && parent->getOpCode().isConversion() &&
                (parent->getOpCodeValue() == TR::ILOpCode::getProperConversion(
                                               parent->getDataType(),
                                               node->getDataType(),
                                               false /*!wantZeroExtension*/))) {
              if (node->getReferenceCount() > 1) {
              ...
              TR::Node* newLoad = TR::Node::createWithSymRef(
                node,
                comp()->il.opCodeForDirectLoad(node->getDataType()),
                0,
                newSymbolReference);
...
bool
TR_PartialRedundancy::isNotPrevTreeStoreIntoFloatTemp(TR::Symbol* symbol)
{
  if (_prevTree &&
...
//
TR::TreeTop*
TR_PartialRedundancy::replaceOptimalSubNodes(TR::TreeTop* curTree,
                                             TR::Node* parent,
                                             TR::Node* node,
                                             int32_t childNum,
                                             TR::Node* duplicateParent,
                                             TR::Node* duplicateOptimalNode,
                                             bool isNullCheck,
                                             int32_t blockNum,
                                             vcount_t visitCount)
    ...
    if (((node->getLocalIndex() != MAX_SCOUNT) &&
      ...
      if (shouldAllowOptimalSubNodeReplacement(comp()) &&
        ...
        if (fe()->dataTypeForLoadOrStore(node->getDataType()) !=
            node->getDataType())
          newLoad = TR::Node::create(
            TR::ILOpCode::getProperConversion(newLoad->getDataType(),
                                              node->getDataType(),
                                              false /* !wantZeroExtension */),
            1,
            newLoad);
        ...
        for (i = 0; i < node->getNumChildren(); i++) {
          ...
          curTree = replaceOptimalSubNodes(curTree,
                                           node,
                                           child,
                                           i,
                                           duplicateOptimalNode,
  ...
  , _workingList(comp->trMemory())
{
  ...
  for (nextNode = cfg->getFirstNode(); nextNode;
    ...
    _orderedOptNumbersList[i] =
      (int32_t*)trMemory()->allocateStackMemory(numElements * sizeof(int32_t));
...
TR_ExceptionCheckMotion::getKind()
{
  return ExceptionCheckMotion;
}

void
TR_ExceptionCheckMotion::setBlockFenceStatus(TR::Block* block)
{
...
int32_t
TR_ExceptionCheckMotion::areExceptionSuccessorsIdentical(TR::CFGNode* node1,
                                                         TR::CFGNode* node2)
{
  int32_t result = -1;
  _firstSucc->empty();
  ...
  for (auto nextEdge = node1->getExceptionSuccessors().begin();
       nextEdge != node1->getExceptionSuccessors().end();
       ++nextEdge) {
  ...
  for (auto nextEdge = node2->getExceptionSuccessors().begin();
       nextEdge != node2->getExceptionSuccessors().end();
       ++nextEdge) {
    TR::CFGNode* succ = (*nextEdge)->getTo();
...
int32_t
TR_ExceptionCheckMotion::perform()
{
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
    ...
    if (redundantSetAdjustmentRequired || _trySecondBestSolution) {
      ...
      TR_RedundantExpressionAdjustment* redundantExpressionAdjustment =
        new (comp()->allocator()) TR_RedundantExpressionAdjustment(
          comp(), optimizer(), comp()->getFlowGraph()->getStructure(), this);
...
void
TR_ExceptionCheckMotion::copyListFromInto(List<TR::Node>* fromList,
                                          List<TR::Node>* toList)
{
  if (!fromList->isEmpty()) {
...
void
TR_ExceptionCheckMotion::composeLists(List<TR::Node>* targetList,
...
void
TR_ExceptionCheckMotion::appendLists(List<TR::Node>* firstList,
...
bool
TR_ExceptionCheckMotion::compareLists(List<TR::Node>* firstList,
                                      List<TR::Node>* secondList)
{
  bool same = true;
...
void
TR_ExceptionCheckMotion::initializeAnalysisInfo(ExprDominanceInfo* info,
                                                TR_Structure* s)
{
                                                ...
                                                TR::Block* block)
{
  ...
  for (auto succ = block->getExceptionSuccessors().begin();
       succ != block->getExceptionSuccessors().end();
       ++succ) {
    succBlock = (*succ)->getTo();
    info->_outList[succBlock->getNumber()] =
      new (trStackMemory()) TR_ScratchList<TR::Node>(trMemory());
...
void
TR_ExceptionCheckMotion::initializeGenAndKillSetInfo()
{
  ...
  for (cfgNode = comp()->getFlowGraph()->getFirstNode(); cfgNode;
    ...
    for (; treeTop != block->getExit(); treeTop = treeTop->getNextTreeTop()) {
      ...
      analyzeNodeToInitializeGenAndKillSets(
        treeTop, visitCount, block->getStructureOf());
    ...
    if (trace()) {
      if (!_regularGenSetInfo[blockNum]->isEmpty()) {
        ...
        for (listElem = _regularGenSetInfo[blockNum]->getListHead();
          ...
          traceMsg(comp(),
                   "Expr %d (representative) Node %p in Block : %d\n",
...
void
TR_ExceptionCheckMotion::analyzeNodeToInitializeGenAndKillSets(
  TR::TreeTop* tt,
  vcount_t visitCount,
  ...
  if (treeHasChecks(tt) || node->getOpCode().isStore()) {
    ...
    if (node->getOpCode().isCheck()) {
      ...
      if (opCodeValue == TR::NULLCHK) {
        ...
        includeRelevantNodes(
          node->getNullCheckReference(), visitCount, blockNum);
        ...
        for (i = firstChildNum; i < node->getFirstChild()->getNumChildren();
             i++)
          includeRelevantNodes(
            node->getFirstChild()->getChild(i), visitCount, blockNum);

        bool nodeKilledByChild = false;
        for (i = firstChildNum; i < node->getFirstChild()->getNumChildren();
             i++) {
          if (isNodeKilledByChild(
                node, node->getFirstChild()->getChild(i), blockNum)) {
            nodeKilledByChild = true;
            break;
          }
        }
        ...
        if (
          (!_optimisticRednSetInfo[blockNum]->get(
             node
               ->getLocalIndex()) /* &&
                                         !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())*/
...
void
TR_ExceptionCheckMotion::createAndAddListElement(TR::Node* node,
                                                 int32_t blockNum)
{
...
bool
TR_ExceptionCheckMotion::isNodeKilledByChild(TR::Node* parent,
                                             TR::Node* child,
                                             int32_t blockNum)
{
...
bool
TR_ExceptionCheckMotion::includeRelevantNodes(TR::Node* node,
                                              vcount_t visitCount,
                                              int32_t blockNum)
{
  if (node->getVisitCount() == visitCount) {
...
void
TR_ExceptionCheckMotion::initializeOutLists(List<TR::Node>** lists)
{
  int32_t i;
  for (i = 0; i < _numberOfNodes; i++)
    lists[i]->deleteAll();
...
void
TR_ExceptionCheckMotion::initializeOutList(List<TR::Node>* list)
{
  list->deleteAll();
}

bool
TR_ExceptionCheckMotion::analyzeRegionStructure(
  TR_RegionStructure* regionStructure,
  bool checkForChange)
{
  ...
  TR_BitVector* pendingList =
    new (trStackMemory()) TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
  TR_BitVector* originalPendingList =
    new (trStackMemory()) TR_BitVector(_numberOfNodes, trMemory(), stackAlloc);
    ...
    *pendingList |= *originalPendingList;
...

    {
      ...
      for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
        ...
        if (analyzeNodeIfSuccessorsAnalyzed(
              edgeFrom, regionStructure, pendingList, exitNodes))
          changed = true;
      }
    }

    {
      ...
      for (subNode = si.getCurrent(); subNode; subNode = si.getNext()) {
        if (subNode->getSuccessors().empty() &&
          ...
          if (analyzeNodeIfSuccessorsAnalyzed(
                subNode, regionStructure, pendingList, exitNodes))
            changed = true;
        }
      }
    }
...
bool
TR_ExceptionCheckMotion::analyzeNodeIfSuccessorsAnalyzed(
  TR::CFGNode* cfgNode,
  TR_RegionStructure* regionStructure,
  ContainerType* pendingList,
  ContainerType* exitNodes)
{
  ...
  while (_analysisQueue.getListHead() &&
    ...
    if (trace())
      traceMsg(comp(),
               "Begin analyzing node %p numbered %d\n",
               node,
               node->getNumber());
    ...
    //////bool examiningFirstSuccessor = true;
    {
      for (auto succ = nodeStructure->getSuccessors().begin();
           succ != nodeStructure->getSuccessors().end();
           ++succ) {
      ...
      for (auto succ = nodeStructure->getExceptionSuccessors().begin();
           succ != nodeStructure->getExceptionSuccessors().end();
           ++succ) {
        if (!regionStructure->isExitEdge(*succ)) {
          TR_StructureSubGraphNode* succNode =
            toStructureSubGraphNode((*succ)->getTo());
    ...
    if (trace()) {
      ...
      traceMsg(comp(),
               "In List for Region or Block : %x numbered %d is : \n",
               nodeStructure->getStructure(),
               nodeStructure->getStructure()->getNumber());
    ...
    for (auto pred = nodeStructure->getPredecessors().begin();
         pred != nodeStructure->getPredecessors().end();
         ++pred) {
    ...
    for (auto pred = nodeStructure->getExceptionPredecessors().begin();
         pred != nodeStructure->getExceptionPredecessors().end();
         ++pred) {
      TR_StructureSubGraphNode* predNode =
        (TR_StructureSubGraphNode*)(*pred)->getFrom();
      TR_Structure* predStructure = predNode->getStructure();
...
bool
TR_ExceptionCheckMotion::analyzeBlockStructure(
  TR_BlockStructure* blockStructure,
  bool checkForChange)
{
  ...
  *_tempContainer &=
    *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness
    ...
    if (opCode.getOpCodeValue() == TR::NULLCHK) {
      ...
      if (arrayCheckKilled || checkKilledByChild) {
        ...
        arrayStoreCheckKilled = true;
                 ...
                 .isCheckCast()) /// opCode.getOpCodeValue() == TR::checkcast)
    {
  ...
  *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
  ...
  if (trace()) {
    traceMsg(
      comp(), "Following gen exprs for block_%d cannot survive\n", blockNum);
    _killedGenExprs[blockNum]->print(comp());
    traceMsg(
      comp(), "Known indirect exprs for block_%d that survive\n", blockNum);
...
bool
TR_ExceptionCheckMotion::checkIfNodeCanSomehowSurvive(
  TR::Node* node,
  ContainerType* nodesThatSurvive)
{
...
bool
TR_ExceptionCheckMotion::checkIfNodeCanSurvive(TR::Node* node,
                                               ContainerType* nodesThatSurvive)
{
...
void
TR_ExceptionCheckMotion::markNodeAsSurvivor(TR::Node* node,
                                            ContainerType* nodesThatSurvive)
{
  if ((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) {
    nodesThatSurvive->set(node->getLocalIndex());
...
bool
TR_ExceptionCheckMotion::isNodeValueZero(TR::Node* node)
{
  TR::ILOpCode& opCode = node->getOpCode();
...
void
TR_ExceptionCheckMotion::removeFromList(ListElement<TR::Node>* listElem,
                                        List<TR::Node>* list,
...
TR_DataFlowAnalysis::Kind
TR_RedundantExpressionAdjustment::getKind()
{
...
bool
TR_RedundantExpressionAdjustment::supportsGenAndKillSets()
{
  return true;
}

bool
TR_RedundantExpressionAdjustment::supportsGenAndKillSetsForStructures()
{
  return false;
}

int32_t
TR_RedundantExpressionAdjustment::getNumberOfBits()
{
  return _partialRedundancy->getNumberOfBits();
}

void
TR_RedundantExpressionAdjustment::analyzeNode(TR::Node*,
                                              vcount_t,
                                              TR_BlockStructure*,
                                              ContainerType*)
{}
...
TR_RedundantExpressionAdjustment::TR_RedundantExpressionAdjustment(
  ...
  TR_ExceptionCheckMotion* exceptionCheckMotion)
  : TR_IntersectionBitVectorAnalysis(comp,
                                     comp->getFlowGraph(),
                                     optimizer,
...
bool
TR_RedundantExpressionAdjustment::postInitializationProcessing()
{
  if (trace()) {
    int32_t i;
    for (i = 1; i < _numberOfNodes; ++i) {
...
void
TR_RedundantExpressionAdjustment::initializeGenAndKillSetInfo()
{
  // For each block in the CFG build the gen and kill set for this analysis.
  //
...
bool
TR_RedundantExpressionAdjustment::analyzeBlockStructure(
  TR_BlockStructure* blockStructure,
  bool checkForChange)
{
  ...
  if (_exceptionCheckMotion->getTrySecondBestSolution()) {
    ...
    int32_t* orderedOldOptNumbersList =
      (int32_t*)trMemory()->allocateStackMemory(oldMaxSize * sizeof(int32_t));
    j = 0;
    _optSetHelper->empty();
    while (j < oldMaxSize) {
    ...
    for (listElem = genSetInfo[blockNum]->getListHead(); listElem != NULL;
      ...
      */
      ...
      if (/*_optSetHelper->get(listElem->getData()->getLocalIndex()) || */
        ...
        if (trace())
          traceMsg(comp(),
                   "Affected by order <%d> will be at index %d\n",
                   listElem->getData()->getLocalIndex(),
                   j);
