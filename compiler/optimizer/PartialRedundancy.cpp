...
{
    if ((comp->getOptions()->getLastOptTransformationIndex() < comp->getOptions()->getMaxLastOptTransformationIndex()) || (comp->getOptions()->getFirstOptTransformationIndex() > comp->getOptions()->getMinFirstOptTransformationIndex())) {
...
int32_t TR_PartialRedundancy::perform()
{
    ...
    for (bvi2.SetToFirstOne(); bvi2.Valid(); bvi2.SetToNextOne()) {
        ...
        if (nextOptimalComputation != 0) {
            ...
            if ((_newSymbolsMap[nextOptimalComputation] < 0) && !nextOptimalNode->getOpCode().isCheck() && !nextOptimalNode->getOpCode().isAnchor() && !(nextOptimalNode->getOpCodeValue() == TR::treetop)) {
                if (nextOptimalNode->getOpCode().isLoadVarDirect() && !nextOptimalNode->getSymbol()->isStatic() && !nextOptimalNode->getSymbol()->isMethodMetaData()) {
                ...
                } else if ((nextOptimalNode->getOpCode().isLoadVarDirect() && (nextOptimalNode->getSymbol()->isStatic() || nextOptimalNode->getSymbol()->isMethodMetaData())) || !isNodeAnImplicitNoOp(nextOptimalNode)) {
                    if (trace()) {
    ...
    for (bvi3.SetToFirstOne(); bvi3.Valid(); bvi3.SetToNextOne()) {
        ...
        TR::Node* nextOptimalNode = supportedNodesAsArray[nextOptimalComputation];
        if (nextOptimalNode->getOpCode().isCheck() || (comp()->requiresSpineChecks() && nextOptimalNode->getOpCode().hasSymbolReference() && nextOptimalNode->getSymbol()->isArrayShadowSymbol()))
            continue;
        ...
        for (nextNode = cfg->getFirstNode(); nextNode; nextNode = nextNode->getNext()) {
            ...
            if (((_rednSetInfo[nextNode->getNumber()]->get(nextOptimalComputation)) || (_optSetInfo[nextNode->getNumber()]->get(nextOptimalComputation))) && (false)) {
...
bool TR_PartialRedundancy::isNodeAnImplicitNoOp(TR::Node* node)
{
    ...
    if (node->getOpCode().isArrayRef() && node->getSecondChild()->getOpCode().isLoadConst() && !comp()->cg()->isMaterialized(node->getSecondChild()))
        return true;
...
#ifdef J9_PROJECT_SPECIFIC
    if (!TR::Compiler->cls.romClassObjectsMayBeCollected() && node->getOpCode().hasSymbolReference() && ((node->getSymbolReference() == comp()->getSymRefTab()->findArrayClassRomPtrSymbolRef()) || (node->getSymbolReference() == comp()->getSymRefTab()->findClassRomPtrSymbolRef())))
        return true;
#endif
...
TR::TreeTop* TR_PartialRedundancy::placeComputationsOptimally(TR::Block* block, TR::Node*** supportedNodesAsArray)
{
    ...
    for (nextOptimalComputation = orderedOptNumbersList[currOptimalElement]; seenOptimalElements < numOptimalElements; currOptimalElement++) {
        ...
        TR::Node* nextOptimalNode = (*supportedNodesAsArray)[nextOptimalComputation];
        if (
            (nextOptimalNode->getOpCode().isLoadVarDirect() && !nextOptimalNode->getSymbol()->isStatic() && !nextOptimalNode->getSymbol()->isMethodMetaData())
        ...
        if (anticipatabilityInfo->get(nextOptimalComputation)) {
            ...
            while (placeToInsertOptimalComputations != exitTree) {
                if (nextOptimalNode->getOpCode().isCheck()) {
                                ...
                                if (nextNode->getFirstChild()->exceptionsRaised() != 0) {
                                    ...
                                    if (!shouldAllowLimitingOfOptimalPlacement(comp()) || performTransformation(comp(), "%sPlacing NULLCHK optimally : %p\n", OPT_DETAILS, nodeInTreeTop)) {
                                        ...
                                        if (comp()->useCompressedPointers()) {
                                            ...
                                            if (reference->getOpCode().isLoadIndirect() && reference->getDataType() == TR::Address && TR::TransformUtil::fieldShouldBeCompressed(reference, comp())) {
                    ...
                    if (optimalNode && ((optimalNode->getLocalIndex() != nextOptimalComputation) || rhsOfStore || !lastInsertionMade)) {
                        //
                        ...
                        if (!shouldAllowLimitingOfOptimalPlacement(comp()) || performTransformation(comp(), "%s0Placing %s optimally : %p using symRef #%d\n", OPT_DETAILS, optimalNode->getOpCode().getName(), optimalNode, newSymbolReference->getReferenceNumber())) {
                            ...
                            if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType()) != nextOptimalNode->getDataType()) {
                                TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(nextOptimalNode->getDataType(),
                            ...
                            TR::TreeTop* translateTT = NULL;
                            if (comp()->useCompressedPointers()) {
                                if (convertedOptimalNode->getOpCode().isLoadIndirect() && convertedOptimalNode->getDataType() == TR::Address && TR::TransformUtil::fieldShouldBeCompressed(convertedOptimalNode, comp())) {
        ...
        if (nextOptimalNode->getOpCode().isStore()) {
            ...
            if ((nextOptimalNode->getOpCodeValue() == TR::NULLCHK) || (nextOptimalNode->getOpCodeValue() == TR::treetop)) {
                ...
                if (isSupportedOpCode(nullCheckReferenceNode, NULL) && !nullCheckReferenceOpCode.isLoadVarDirect()) {
                    if (((nullCheckReferenceNode->getLocalIndex() != MAX_SCOUNT) && (nullCheckReferenceNode->getLocalIndex() != 0))) {
                        if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] > -1) && (((!_unavailableSetInfo[block->getNumber()]->get(nullCheckReferenceNode->getLocalIndex())) && (_rednSetInfo[block->getNumber()]->get(nullCheckReferenceNode->getLocalIndex()) || _optSetInfo[block->getNumber()]->get(nullCheckReferenceNode->getLocalIndex()))) || (comp()->requiresSpineChecks() && nullCheckReferenceNode->getOpCode().hasSymbolReference() && nullCheckReferenceNode->getSymbol()->isArrayShadowSymbol()))) {
            ...
            if (comp()->useCompressedPointers()) {
                ...
                if (reference->getOpCode().isLoadIndirect() || reference->getOpCode().isArrayLength()) {
                    if (reference->getFirstChild()->getOpCode().isLoadIndirect() && reference->getFirstChild()->getDataType() == TR::Address && TR::TransformUtil::fieldShouldBeCompressed(reference->getFirstChild(), comp())) {
                    ...
                    if (reference->getDataType() == TR::Address && TR::TransformUtil::fieldShouldBeCompressed(reference, comp())) {
                        translateTT = TR::TreeTop::create(comp(), TR::Node::createCompressedRefsAnchor(reference), NULL, NULL);
                        placeToInsertOptimalComputations->join(translateTT);
                        translateTT->join(secondTreeInBlock);
                        placeToInsertOptimalComputations = translateTT;
            ...
            if (duplicateOptimalNode->getOpCode().isCall() && !duplicateOptimalNode->getSymbolReference()->isUnresolved() && duplicateOptimalNode->getSymbol()->castToMethodSymbol()->isPureFunction()) {
...
static bool isExpressionRedundant(TR::Node* node, TR_PartialRedundancy::ContainerType* redundantComputations, TR_PartialRedundancy::ContainerType* anticipatable)
{
    ...
    if (redundantComputations && node->getLocalIndex() != MAX_SCOUNT && node->getLocalIndex() != 0 && redundantComputations->get(node->getLocalIndex()) && (node->getOpCode().isStore() || anticipatable->get(node->getLocalIndex()))) {
...
void TR_PartialRedundancy::eliminateRedundantComputations(TR::Block* block, TR::Node** supportedNodesAsArray, ContainerType** rednSetInfo, TR::TreeTop* startTree)
{
...

    {
        ...
        while (currentTree != exitTree) {
            ...
            //
            if ((firstOpCodeInTree.isStore() && !currentTree->getNode()->getSymbolReference()->getSymbol()->isAutoOrParm()) || ((firstOpCodeInTree.isResolveOrNullCheck() || (firstOpCodeInTree.getOpCodeValue() == TR::ArrayStoreCHK) || firstOpCodeInTree.isAnchor() || (firstOpCodeInTree.getOpCodeValue() == TR::treetop)) && (currentTree->getNode()->getFirstChild()->getOpCode().isStore() && !currentTree->getNode()->getFirstChild()->getSymbolReference()->getSymbol()->isAutoOrParm()))) {
		      ...
		      performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()) */
                ...
                if ((currentTree->getNode()->getNumChildren() > 0) && isExpressionRedundant(currentTree->getNode()->getFirstChild(), redundantComputations, anticipatabilityInfo) && currentTree->getNode()->getFirstChild()->getOpCode().isStore()) {
...
bool TR_PartialRedundancy::eliminateRedundantSupportedNodes(TR::Node* parent, TR::Node* node, bool firstComputation, TR::TreeTop* currentTree, int32_t blockNum, vcount_t visitCount, ContainerType* redundantComputations, ContainerType* anticipatabilityInfo, TR::Node** supportedNodesAsArray)
{
    ...
    if (isSupportedOpCode(node, parent)) {
#ifdef J9_PROJECT_SPECIFIC
        if (_profilingWalk && ((_exceptionCheckMotion && _exceptionCheckMotion->getOptimisticRednSetInfo()[blockNum]->get(node->getLocalIndex())) || isExpressionRedundant(node, redundantComputations, anticipatabilityInfo))) {
            ...
            if (comp()->isProfilingCompilation() && ((node->getType().isInt32()) || ((node->getType().isInt64()))) && /* &&
               ...
               node->getSymbolReference()->getSymbol()->isAutoOrParm())) && */
                !node->getOpCode().isLoadConst() && (_numProfilingsAllowed > 0) && !node->getByteCodeInfo().doNotProfile()) {
                ...
                if ((node->getType().isInt64()) && profileLongParms) {
                    ...
                    if (currentOpCode.isBranch() || currentOpCode.isJumpWithMultipleTargets() || currentOpCode.isReturn() || currentOpCode.getOpCodeValue() == TR::athrow)
        ...
        //
        if ((node->getOpCode().isLoadVarDirect() && !node->getSymbol()->isStatic() && !node->getSymbol()->isMethodMetaData())
            || (ignoreNode(node)))
            return flag;
        ...
        if (isExpressionRedundant(node, redundantComputations, anticipatabilityInfo)) {
            ...
            if (!isNodeAnImplicitNoOp(node)) {
                ...
                if (node->getOpCode().isLoadConst()) {
                    if ((parent->getOpCode().isDiv() || parent->getOpCode().isRem()) && (parent->getSecondChild() == node))
                        return flag;
                    else
                        optimizer()->setRequestOptimization(OMR::globalValuePropagation, true);
                }
                ...
                TR::SymbolReference* newSymbolReference = _newSymbolReferences[nodeIndex];
                if (newSymbolReference && performTransformation(comp(), "%sEliminating redundant computation (%s) : %p in block_%d, visit count %d\n", OPT_DETAILS, node->getOpCode().getName(), node, blockNum, node->getVisitCount())) {
                    if (node->getDataType() == TR::NoType) {
                        ...
                        if (parent && parent->getOpCode().isConversion() && (parent->getOpCodeValue() == TR::ILOpCode::getProperConversion(parent->getDataType(), node->getDataType(), false /*!wantZeroExtension*/))) {
                            if (node->getReferenceCount() > 1) {
...
TR::TreeTop* TR_PartialRedundancy::replaceOptimalSubNodes(TR::TreeTop* curTree, TR::Node* parent, TR::Node* node, int32_t childNum, TR::Node* duplicateParent, TR::Node* duplicateOptimalNode, bool isNullCheck, int32_t blockNum, vcount_t visitCount)
{
    ...
    if (isSupportedOpCode(node, parent) && (!(opCode.isLoadVarDirect() && !node->getSymbol()->isStatic() && !node->getSymbol()->isMethodMetaData())) && (!isNodeAnImplicitNoOp(node))) {
        if (trace())
        ...
        if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) && !(isNullCheck && (_nullCheckNode->getNullCheckReference() == node))) {
            ...
            //
            if (shouldAllowOptimalSubNodeReplacement(comp()) && (((_newSymbolsMap[node->getLocalIndex()] > -1) && ((/* _optSetInfo[blockNum]->get(node->getLocalIndex()) || */ ((!parent || !parent->getOpCode().isSpineCheck() || (childNum != 0)) && comp()->requiresSpineChecks() && node->getOpCode().hasSymbolReference() && node->getSymbol()->isArrayShadowSymbol()) || !_unavailableSetInfo[blockNum]->get(node->getLocalIndex())))))) {
    ...
    if (!sourceList->isEmpty() && !targetList->isEmpty()) {
        ...
        for (newNode = sourceList->getListHead(); newNode != NULL;) {
            ...
            if ((oldNode == NULL) || (newNode == NULL) || (oldNode->getData()->getLocalIndex() != newNode->getData()->getLocalIndex())) {
                ...
                while (cursorNode) {
                    if (_definitelyNotKilled->get(cursorNode->getData()->getLocalIndex()) || ((cursorNode->getData()->getOpCode().isIndirect() || cursorNode->getData()->getOpCode().isArrayLength()) && availableExprs->get(cursorNode->getData()->getFirstChild()->getLocalIndex())))
                ...
                while (oldNode && ((oldNode->getData()->getOpCode().isIndirect() || oldNode->getData()->getOpCode().isArrayLength()) && availableExprs->get(oldNode->getData()->getFirstChild()->getLocalIndex()))) {
                ...
                if (newNode)
                    newNode = newNode->getNextElement();
                while (newNode && ((newNode->getData()->getOpCode().isIndirect() || newNode->getData()->getOpCode().isArrayLength()) && availableExprs->get(newNode->getData()->getFirstChild()->getLocalIndex()))) {
                    if (!_composeHelper->get(newNode->getData()->getLocalIndex())) {
                        ListElement<TR::Node>* newSourceNode = (ListElement<TR::Node>*)trMemory()->allocateStackMemory(sizeof(ListElement<TR::Node>));
                        newSourceNode->setNextElement(NULL);
                        if (prevCursorNode)
    ...
    if (treeHasChecks(tt) || node->getOpCode().isStore()) {
        ...
        if (node->getOpCode().isCheck()) {
            ...
            if (opCodeValue == TR::NULLCHK) {
                ...
                if (!_atLeastOneExceptionPoint) {
                    if ((_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) || _optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())) && (!_actualRednSetInfo[blockNum]->get(node->getLocalIndex())) && (!_actualOptSetInfo[blockNum]->get(node->getLocalIndex())) && (!_genSetHelper->get(node->getLocalIndex()))) {
                ...
                !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */
                        ...
                        || _nullCheckKilled->get(blockNum) || nodeKilledByChild)
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                    ...
                    || _boundCheckKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _nullCheckKilled->set(blockNum);
                    _resolveCheckKilled->set(blockNum);
                    _divCheckKilled->set(blockNum);
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                    ...
                    || _arrayStoreCheckKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _resolveCheckKilled->set(blockNum);
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                    ...
                    || _arrayCheckKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _resolveCheckKilled->set(blockNum);
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
            ...
            } else if (node->getOpCode().isCheckCast()) //(opCodeValue == TR::checkcast)
            {
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                    ...
                    || _checkCastKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _resolveCheckKilled->set(blockNum);
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
            ...
            if ((node->getSymbol()->isStatic() || node->getSymbol()->isShadow()) || !node->getSymbolReference()->getUseonlyAliases().isZero(comp())) {
                if (!_atLeastOneExceptionPoint && node->getOpCode().isIndirect() && !node->getSymbolReference()->getSymbol()->isArrayletShadowSymbol() && (!node->getFirstChild()->isThisPointer() || !node->getFirstChild()->isNonNull())) {
                    if ((_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) || _optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())) && (!_actualRednSetInfo[blockNum]->get(node->getLocalIndex())) && (!_actualOptSetInfo[blockNum]->get(node->getLocalIndex())) && (!_genSetHelper->get(node->getLocalIndex()))) {
                        createAndAddListElement(node, blockNum);
                    }
                }
...
bool TR_ExceptionCheckMotion::includeRelevantNodes(TR::Node* node, vcount_t visitCount, int32_t blockNum)
{
    ...
    int32_t i;
    for (i = 0; i < node->getNumChildren(); i++) {
        if (includeRelevantNodes(node->getChild(i), visitCount, blockNum) && (!node->getOpCode().isStore() || (i != rhsChild))) {
            TR::Node* child = node->getChild(i);
            childRelevant = true;
    ...
    if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0)) /* && (!opCode.isStore()) */) {
        if ((!_actualRednSetInfo[blockNum]->get(node->getLocalIndex())) && (!_actualOptSetInfo[blockNum]->get(node->getLocalIndex())) && (childRelevant || ((opCode.isIndirect() && (opCode.isLoadVar() || opCode.isStore()) && (!(opCode.hasSymbolReference() && node->getSymbolReference()->getSymbol()->isArrayletShadowSymbol())) && (!node->getFirstChild()->isThisPointer() || !node->getFirstChild()->isNonNull())) || opCode.isArrayLength()) || (node->getOpCode().isTwoChildrenAddress()) || (opCode.hasSymbolReference() && node->getSymbolReference()->isUnresolved()) || (opCode.isDiv() || opCode.isRem()))) {
            _relevantNodes->set(node->getLocalIndex());
            ...
            if (!_genSetHelper->get(node->getLocalIndex()) && (_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) || _optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()))) {
                ListElement<TR::Node>* newElement = (ListElement<TR::Node>*)trMemory()->allocateStackMemory(sizeof(ListElement<TR::Node>));
                newElement->setData(node);
                newElement->setNextElement(NULL);
                if (_lastGenSetElement)
...
bool TR_ExceptionCheckMotion::analyzeNodeIfSuccessorsAnalyzed(TR::CFGNode* cfgNode, TR_RegionStructure* regionStructure, ContainerType* pendingList, ContainerType* exitNodes)
{
    ...
    while (_analysisQueue.getListHead() && (_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        if (!pendingList->get(nodeStructure->getStructure()->getNumber()) && (*(_changedSetsQueue.getListHead()->getData()) == 0)) {
...
bool TR_ExceptionCheckMotion::analyzeBlockStructure(TR_BlockStructure* blockStructure, bool checkForChange)
{
    ...
    *_unresolvedAccessesThatSurvive = *_knownUnresolvedAccessesThatSurvive;
    ...
    for (listElem = _blockInfo[blockNum]->getListHead(); listElem != NULL; listElem = listElem->getNextElement()) {
        ...
        if (opCode.getOpCodeValue() == TR::NULLCHK) {
            if (nullCheckKilledByOptList || checkKilledByChild || genExprThatIsKilled || _actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) {
                removeFromList(listElem, _blockInfo[blockNum], prevElem);
                boundCheckKilledByOptList = true;
                resolveCheckKilledByOptList = true;
                divCheckKilledByOptList = true;
            ...
            if (nodeKilledByOptList || checkKilledByChild || genExprThatIsKilled || _actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) {
                removeFromList(listElem, _blockInfo[blockNum], prevElem);
                boundCheckKilledByOptList = true;
                nullCheckKilledByOptList = true;
                divCheckKilledByOptList = true;
            ...
            if (boundCheckKilledByOptList || checkKilledByChild || genExprThatIsKilled || _actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) {
                removeFromList(listElem, _blockInfo[blockNum], prevElem);
                nullCheckKilledByOptList = true;
                resolveCheckKilledByOptList = true;
                divCheckKilledByOptList = true;
            ...
            if (divCheckKilledByOptList || checkKilledByChild || genExprThatIsKilled || _actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) {
                removeFromList(listElem, _blockInfo[blockNum], prevElem);
                nullCheckKilledByOptList = true;
                resolveCheckKilledByOptList = true;
                boundCheckKilledByOptList = true;
            ...
            if (arrayStoreCheckKilledByOptList || checkKilledByChild || genExprThatIsKilled || _actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) {
                removeFromList(listElem, _blockInfo[blockNum], prevElem);
                nullCheckKilledByOptList = true;
                resolveCheckKilledByOptList = true;
                boundCheckKilledByOptList = true;
        ...
        {
            bool alreadyRemoved = false;
            if (checkCastKilledByOptList || checkKilledByChild || genExprThatIsKilled || _actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) {
                removeFromList(listElem, _blockInfo[blockNum], prevElem);
                nullCheckKilledByOptList = true;
                resolveCheckKilledByOptList = true;
                boundCheckKilledByOptList = true;
...
bool TR_ExceptionCheckMotion::checkIfNodeCanSomehowSurvive(TR::Node* node, ContainerType* nodesThatSurvive)
{
    ...
    if (!_exprsUnaffectedByOrder->get(node->getLocalIndex())) {
        ...
        if (((opCode.isIndirect() || (opCode.isArrayLength())) && (!node->getFirstChild()->isThisPointer() || !node->getFirstChild()->isNonNull()) && (!(opCode.hasSymbolReference() && node->getSymbolReference()->getSymbol()->isArrayletShadowSymbol())) && !checkIfNodeCanSurvive(node->getFirstChild(), nodesThatSurvive))
            || (node->getOpCode().isTwoChildrenAddress()) || (opCode.hasSymbolReference() && node->getSymbolReference()->isUnresolved()) || ((opCode.isDiv() || opCode.isRem()) && !checkIfNodeCanSurvive(node->getSecondChild(), nodesThatSurvive)))
            return false;
    }
...
bool TR_RedundantExpressionAdjustment::analyzeBlockStructure(TR_BlockStructure* blockStructure, bool checkForChange)
{
    ...
    if (_exceptionCheckMotion->getTrySecondBestSolution()) {
         ...
         */
            ...
            if (/*_optSetHelper->get(listElem->getData()->getLocalIndex()) || */
                ((optimisticRednSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())) && (!_currentInSetInfo->get(listElem->getData()->getLocalIndex())) && (!actualOptSetInfo[blockNum]->get(listElem->getData()->getLocalIndex())))) {
                if (trace())
                    traceMsg(comp(), "IN ORDER Expr %d (representative) Node %p (listElem %p) in Block : %d\n", listElem->getData()->getLocalIndex(), listElem->getData(), listElem, blockNum);
    ...
    if ((block->getSuccessors().size() == 2) && block->getExit()) {
        ...
        if (((branch->getOpCodeValue() == TR::ifacmpne) || (branch->getOpCodeValue() == TR::ifacmpeq)) && (branch->getSecondChild()->getOpCodeValue() == TR::aconst) && (branch->getSecondChild()->getAddress() == 0)) {
