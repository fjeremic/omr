...
int32_t TR_PartialRedundancy::perform()
{
  ...
  _registerCandidates
    = (TR_RegisterCandidate**)trMemory()->allocateStackMemory(_numberOfBits * sizeof(TR_RegisterCandidate*));
...
TR::TreeTop* TR_PartialRedundancy::placeComputationsOptimally(TR::Block* block, TR::Node*** supportedNodesAsArray)
{
  ...
  ContainerType* anticipatabilityInfo = _isolatedness->_latestness->_delayedness->_earliestness->_globalAnticipatability
                                          ->_localAnticipatability.getDownwardExposedAnalysisInfo(block->getNumber());
      ...
      while (placeToInsertOptimalComputations != exitTree) {
        if (nextOptimalNode->getOpCode().isCheck()) {
            ...
            if (nextNode->getLocalIndex() == nextOptimalComputation) {
              if (nextNode->getOpCode().isNullCheck()) {
                if (nextNode->getFirstChild()->exceptionsRaised() != 0) {
                  ...
                  // TR::Node *duplicateOptimalNode = duplicateExact(nextOptimalNode->getNullCheckReference(),
          ...
          TR::Node* optimalNode = getAlreadyPresentOptimalNode(
            placeToInsertOptimalComputations->getNode(), nextOptimalComputation, comp()->getVisitCount(), rhsOfStore);
          ...
          if (optimalNode
            && ((optimalNode->getLocalIndex() != nextOptimalComputation) || rhsOfStore || !lastInsertionMade)) {
            //
            ...
            if (!shouldAllowLimitingOfOptimalPlacement(comp())
              || performTransformation(comp(), "%s0Placing %s optimally : %p using symRef #%d\n", OPT_DETAILS,
                optimalNode->getOpCode().getName(), optimalNode, newSymbolReference->getReferenceNumber())) {
              ...
              if (fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType()) != nextOptimalNode->getDataType()) {
                TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(nextOptimalNode->getDataType(),
                  fe()->dataTypeForLoadOrStore(nextOptimalNode->getDataType()), false /* !wantZeroExtension */);
              ...
              TR::Node* storeForCommonedNode
                = TR::Node::createWithSymRef(comp()->il.opCodeForDirectStore(nextOptimalNode->getDataType()), 1, 1,
    ...
    if (nextOptimalNode->getOpCode().isStore()) {
      ...
      if ((nextOptimalNode->getOpCodeValue() == TR::NULLCHK) || (nextOptimalNode->getOpCodeValue() == TR::treetop)) {
        ...
        TR::Node* duplicateOptimalNode
          = duplicateExact(nextOptimalNode->getNullCheckReference(), &seenNodes, &duplicateNodes, visitCount1);
            ...
            if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] > -1)
              && (((!_unavailableSetInfo[block->getNumber()]->get(nullCheckReferenceNode->getLocalIndex()))
                    && (_rednSetInfo[block->getNumber()]->get(nullCheckReferenceNode->getLocalIndex())
                      || _optSetInfo[block->getNumber()]->get(nullCheckReferenceNode->getLocalIndex())))
                || (comp()->requiresSpineChecks() && nullCheckReferenceNode->getOpCode().hasSymbolReference()
              ...
              if (trace())
                traceMsg(comp(),
                  "Duplicate null check had its new null check reference %p replaced by %p with symRef #%d\n",
        ...
        if (!replacedNullCheckReference) {
          ...
          for (k = 0; k < nextOptimalNode->getNullCheckReference()->getNumChildren(); k++) {
            ...
            placeToInsertOptimalComputations = replaceOptimalSubNodes(placeToInsertOptimalComputations,
              nextOptimalNode->getNullCheckReference(), nextOptimalNode->getNullCheckReference()->getChild(k), k,
              duplicateOptimalNode, duplicateOptimalNode->getChild(k), false, block->getNumber(), visitCount4);
          }
        }
        ...
        for (k = 0; k < nextOptimalNode->getNumChildren(); k++) {
          ...
          placeToInsertOptimalComputations
            = replaceOptimalSubNodes(placeToInsertOptimalComputations, nextOptimalNode, nextOptimalNode->getChild(k), k,
              duplicateOptimalNode, duplicateOptimalNode->getChild(k), false, block->getNumber(), visitCount4);
        }
      ...
      dumpOptDetails(comp(), "%s1Placing %s optimally : %p using symRef #%d\n", OPT_DETAILS,
        convertedOptimalNode->getOpCode().getName(), convertedOptimalNode, newSymbolReference->getReferenceNumber());
      ...
      if (duplicateOptimalNode->getOpCode().isCall() && !duplicateOptimalNode->getSymbolReference()->isUnresolved()
  ...
  TR::Block* block, TR::Node** supportedNodesAsArray, ContainerType** rednSetInfo, TR::TreeTop* startTree)
{
  ...
  ContainerType* anticipatabilityInfo = _isolatedness->_latestness->_delayedness->_earliestness->_globalAnticipatability
                                          ->_localAnticipatability.getAnalysisInfo(block->getNumber());

  {
    ...
    while (currentTree != exitTree) {
      ...
      //
      eliminateRedundantSupportedNodes(NULL, currentTree->getNode(), false, currentTree, block->getNumber(), visitCount,
        shouldHaveRedundantComputations ? redundantComputations : NULL, anticipatabilityInfo, supportedNodesAsArray);

      // Now handle stores
      //
      // Stores could be either treetops themselves or be under a TR::treetop or a null check/resolve check
      //
      if ((firstOpCodeInTree.isStore() && !currentTree->getNode()->getSymbolReference()->getSymbol()->isAutoOrParm())
        || ((firstOpCodeInTree.isResolveOrNullCheck() || (firstOpCodeInTree.getOpCodeValue() == TR::ArrayStoreCHK)
              || firstOpCodeInTree.isAnchor() || (firstOpCodeInTree.getOpCodeValue() == TR::treetop))
            ...
            if (newSymbolReference /* &&
              ...
              if (currentTree->getNode()->getOpCode().isStoreIndirect()) {
                ...
                if (fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType()) != rhsOfStore->getDataType()) {
                  TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(rhsOfStore->getDataType(),
                    fe()->dataTypeForLoadOrStore(rhsOfStore->getDataType()), false /* !wantZeroExtension */);
                  convertedOptimalNode = TR::Node::create(conversionOpCode, 1, rhsOfStore);
                }

                storeForCommonedNode
                  = TR::Node::createWithSymRef(opCodeValue, 1, 1, convertedOptimalNode, newSymbolReference);
              } else
        ...
        if ((currentTree->getNode()->getNumChildren() > 0)
		  ...
		  performTransformation(comp(), "%sEliminating redundant computation (store) : %p\n", OPT_DETAILS, currentTree->getNode()->getFirstChild()) */)
            ...
            if (currentTree->getNode()->getFirstChild()->getOpCode().isStoreIndirect()) {
              ...
              storeForCommonedNode = TR::Node::createWithSymRef(
                opCodeValue, 1, 1, currentTree->getNode()->getFirstChild()->getFirstChild(), newSymbolReference);
#ifdef J9_PROJECT_SPECIFIC
            correctDecimalPrecision(storeForCommonedNode, storeForCommonedNode->getFirstChild(), comp());
#endif
        ...
        if (isExpressionRedundant(currentTree->getNode(), redundantComputations, anticipatabilityInfo)) {
          if (firstOpCodeInTree.isResolveOrNullCheck()) {
            ...
            if (performTransformation(comp(), "%sEliminating redundant check computation (%s) : %p\n", OPT_DETAILS,
                  currentTree->getNode()->getOpCode().getName(), currentTree->getNode())) {
  ...
  ContainerType* anticipatabilityInfo, TR::Node** supportedNodesAsArray)
{
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    TR::Node* child = node->getChild(i);
    if (!eliminateRedundantSupportedNodes(node, child, flag, currentTree, blockNum, visitCount, redundantComputations,
...
node->getSymbolReference()->getSymbol()->isAutoOrParm())) && */
        ...
        if ((node->getType().isInt64()) && profileLongParms) {
          ...
          if (currentOpCode.isBranch() || currentOpCode.isJumpWithMultipleTargets() || currentOpCode.isReturn()
  ...
  vcount_t visitCount)
{
  ...
  if (isSupportedOpCode(node, parent)
    ...
    if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0))
      ...
      // We do not want to do optimal subnode replacement under tracePRE because it can cause spurious failures as a
      // result of earlier optimal node placement not having been done (because of opt transformation index) that
      // interfere with last opt transformation index based search. This is done because optimal sub node replacement is
      // rarely the cause of bugs on it's own and this issue is just making it harder to debug problems in other PRE
      // transformations. We created another option TR_TracePREForOptimalSubNodeReplacement to continue to allow optimal
      // sub node replacement to be done regardless but in order to make this more useful we turn off limiting optimal
      // placements when this new option is in effect (so that it becomes safe to narrow down to a optimal sub node
      // replacement in the unlikely event that this is the cause of some failure)
      //
      if (shouldAllowOptimalSubNodeReplacement(comp())
        && (((_newSymbolsMap[node->getLocalIndex()] > -1)
          && ((/* _optSetInfo[blockNum]->get(node->getLocalIndex()) || */ (
                 (!parent || !parent->getOpCode().isSpineCheck() || (childNum != 0)) && comp()->requiresSpineChecks()
        ...
        TR::Node* newLoad
          = TR::Node::createWithSymRef(node, comp()->il.opCodeForDirectLoad(nodeDataType), 0, newSymbolReference);
        if (fe()->dataTypeForLoadOrStore(node->getDataType()) != node->getDataType())
          newLoad = TR::Node::create(TR::ILOpCode::getProperConversion(
                                       newLoad->getDataType(), node->getDataType(), false /* !wantZeroExtension */),
            1, newLoad);
        ...
        if (trace())
          traceMsg(comp(),
            "Note : Duplicate parent %p wanted to replace its child %p by possibly already available symRef but FAILED "
            "to do so\n",
      ...
      if ((oldNode == NULL) || (newNode == NULL)
        ...
        while (cursorNode) {
          if (_definitelyNotKilled->get(cursorNode->getData()->getLocalIndex())
            || ((cursorNode->getData()->getOpCode().isIndirect() || cursorNode->getData()->getOpCode().isArrayLength())
...
void TR_RedundantExpressionAdjustment::initializeGenAndKillSetInfo()
{
  ...
  for (i = 1; i < _numberOfNodes; ++i) {
    ...
    if (actualOptSetInfo[i]) {
      ...
      *_regularGenSetInfo[i] &= *_partialRedundancy->_isolatedness->_latestness->_delayedness->_earliestness
                                   ->_globalAnticipatability->_localAnticipatability.getDownwardExposedAnalysisInfo(i);
...
bool TR_RedundantExpressionAdjustment::analyzeBlockStructure(TR_BlockStructure* blockStructure, bool checkForChange)
{
  ...
  if ((block->getSuccessors().size() == 2) && block->getExit()) {
    ...
    if (((branch->getOpCodeValue() == TR::ifacmpne) || (branch->getOpCodeValue() == TR::ifacmpeq))
      && (branch->getSecondChild()->getOpCodeValue() == TR::aconst) && (branch->getSecondChild()->getAddress() == 0)) {
