...
void TR_LiveOnAllPaths::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *,
                                    TR_BitVector *) {}

TR_LiveOnAllPaths::TR_LiveOnAllPaths(
    TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_Structure *rootStructure, TR_LiveVariableInformation *liveVariableInfo,
    bool splitLongs, bool includeParms)
    : TR_BackwardIntersectionBitVectorAnalysis(
          comp, comp->getFlowGraph(), optimizer,
          comp->getOption(TR_TraceLiveness)) {
  if (trace()) traceMsg(comp, "Starting LiveOnAllPaths analysis\n");
  ...
  if (liveVariableInfo == NULL)
    _liveVariableInfo = new (trStackMemory()) TR_LiveVariableInformation(
        comp, optimizer, rootStructure, splitLongs, includeParms);
  else
...
void TR_LiveOnAllPaths::initializeGenAndKillSetInfo() {
  _liveVariableInfo->initializeGenAndKillSetInfo(
      _regularGenSetInfo, _regularKillSetInfo, _exceptionGenSetInfo,
      _exceptionKillSetInfo);
}

void TR_LiveOnAllPaths::analyzeTreeTopsInBlockStructure(
