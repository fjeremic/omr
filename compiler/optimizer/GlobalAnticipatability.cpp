...
TR_GlobalAnticipatability *
TR_GlobalAnticipatability::asGlobalAnticipatability() {
  return this;
}
...
void TR_GlobalAnticipatability::analyzeNode(TR::Node *, vcount_t,
                                            TR_BlockStructure *,
                                            TR_BitVector *) {}

TR_GlobalAnticipatability::TR_GlobalAnticipatability(
    TR::Compilation *comp, TR::Optimizer *optimizer,
    TR_Structure *rootStructure, bool trace)
    : TR_BackwardIntersectionBitVectorAnalysis(comp, comp->getFlowGraph(),
                                               optimizer, trace),
  ...
  if (trace) traceMsg(comp, "Starting GlobalAnticipatability\n");
...

  {
    ...
    for (j = 0; j < _numberOfNodes; j++)
      *_blockAnalysisInfo[j] |=
          *(_localAnticipatability.getDownwardExposedAnalysisInfo(j));
...
bool TR_GlobalAnticipatability::postInitializationProcessing() {
  ...
  // bit vector analysis.
  //
  _outSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(ContainerType *));

  int32_t i;
  for (i = 0; i < _numberOfNodes; i++) allocateContainer(_outSetInfo + i);
  ...
  _checkExpressionsInBlock = (ContainerType **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(ContainerType *));
...
static bool nodeCanSurvive(TR::Node *nextNode, TR::Node *lastNodeFirstChild,
                           TR::Node *lastNodeSecondChild, TR::Compilation *comp,
                           bool trace) {
  if (isFieldAccess(nextNode)) {
    ...
    if (lastNodeFirstChild) {
      if (lastNodeFirstChild->getFirstChild()->getLocalIndex() ==
    ...
    if (lastNodeSecondChild) {
      if (lastNodeSecondChild->getFirstChild()->getLocalIndex() ==
          firstChild->getLocalIndex()) {
        if (similarOffset <
            lastNodeSecondChild->getSymbolReference()->getOffset())
          similarOffset =
              lastNodeSecondChild->getSymbolReference()->getOffset();
    ...
    if (trace) traceMsg(comp, "seen similar access %d\n", seenSimilarAccess);
    ...
    if (seenSimilarAccess) {
      ...
      if (sig && (len > 0))
        cl = comp->fe()->getClassFromSignature(
            sig, len, otherSymRef->getOwningMethod(comp));
      ...
      if (otherSig) {
        ...
        otherClassObject = comp->fe()->getClassFromSignature(
            otherSig, otherLen, otherSymRef->getOwningMethod(comp));
      }

      if (trace) traceMsg(comp, "cl %p other cl %p\n", cl, otherClassObject);

      if (cl && otherClassObject &&
          (comp->fe()->isInstanceOf(cl, otherClassObject, true) == TR_yes))
        return true;
    }
  }
...
extern bool isExceptional(TR::Compilation *,
                          TR::Node *);  // in LocalAnalysis.cpp

bool TR_GlobalAnticipatability::isExceptionalInBlock(
    TR::Node *node, int32_t blockNumber, ContainerType *alreadyInBlock,
    vcount_t visitCount) {
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (isExceptionalInBlock(child, blockNumber, alreadyInBlock, visitCount)) {
      node->setVisitCount(visitCount -
                          1);  // so that node gets visited again for another
...
TR_GlobalAnticipatability::ContainerType *
TR_GlobalAnticipatability::getCheckExpressionsInBlock(int32_t blockNumber) {
...
bool isRareEdge(TR::Compilation *comp, TR::CFGEdge *edge) {
  return false;  // has to be performance tested with Java
                 ...
                 toBlock(edge->getFrom())->getNumber(),                toBlock(edge->getTo())->getNumber(),
                                                                      edge->getFrequency());
                 #endif
...
void TR_GlobalAnticipatability::killBasedOnSuccTransparency(TR::Block *block) {
  TR::Block *nextSucc;
  for (auto succEdge = block->getSuccessors().begin();
       succEdge != block->getSuccessors().end(); ++succEdge) {
    nextSucc = toBlock((*succEdge)->getTo());
    *_scratch = *(_localTransparency.getAnalysisInfo(nextSucc->getNumber()));
    *_scratch |=
        *(_localAnticipatability.getAnalysisInfo(nextSucc->getNumber()));
...
//
//
void TR_GlobalAnticipatability::analyzeTreeTopsInBlockStructure(
  ...
  if (block != comp()->getFlowGraph()->getEnd()) {
    ...
    for (auto edge = block->getSuccessors().begin();
      ...
      if (hasNonColdSuccessor && !analyzedSucc) break;
    }
    ...
    if (analyzedSucc) {
      ...
      if (lastTree) {
        ...
        if (lastNode->getOpCode().isIf()) {
          ...
          if (!isFieldAccess(lastNodeFirstChild)) lastNodeFirstChild = NULL;

          lastNodeSecondChild = lastNode->getSecondChild();
          if (!isFieldAccess(lastNodeSecondChild)) lastNodeSecondChild = NULL;
        }
      }
      ...
      for (auto edge = block->getSuccessors().begin();
           edge != block->getSuccessors().end(); ++edge) {
        next = toBlock((*edge)->getTo());
        ExtraAnalysisInfo *analysisInfo =
            getAnalysisInfo(next->getStructureOf());
        ...
        if ((!rare && !next->isCold() && blockWeight > 1) ||
          ...
          *_scratch2 &= *_localTransparency.getCheckExpressions();
          //*_scratch2 &=
          //*(_localAnticipatability.getAnalysisInfo(next->getNumber()));
          _scratch3->empty();
          ...
          if ((lastNodeFirstChild || lastNodeSecondChild) &&
            ...
            for (bvi.SetToFirstOne(); bvi.Valid(); bvi.SetToNextOne()) {
              ...
              if (nodeCanSurvive(nextNode, lastNodeFirstChild,
                                 lastNodeSecondChild, comp(), trace())) {
      ...
      // make expressions available on both sides of 'if' if it post-dominates
      // loop entry
      int32_t blockWeight = 1;
      blockStructure->calculateFrequencyOfExecution(&blockWeight);
      if (blockWeight > 1 &&
          block->getFrequency() ==
              blockStructure->getParent()->getEntryBlock()->getFrequency() &&
          ...
          block->isPRECandidate())  // set in loop versioner
      {
        ...
        for (auto edge = block->getSuccessors().begin();
          ...
          if (!rare && blockWeight > 1) {
            dumpOptDetails(comp(),
                           "Ignoring ifcmp within loop in block_%d and adding "
                           "available expressions from block_%d\n",
    ...
    *_regularInfo &= *(_localTransparency.getAnalysisInfo(
        blockStructure->getBlock()->getNumber()));
    *_regularInfo |= *(_localAnticipatability.getDownwardExposedAnalysisInfo(
        blockStructure->getBlock()->getNumber()));
  }
  ...
  if (trace()) {
    traceMsg(comp(), "\nLocal Anticipatability of Block : %d\n",
             blockStructure->getBlock()->getNumber());
    _localAnticipatability
        .getDownwardExposedAnalysisInfo(blockStructure->getBlock()->getNumber())
        ->print(comp());
