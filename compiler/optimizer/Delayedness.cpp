...
void TR_Delayedness::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *,
                                 ContainerType *) {}

TR_Delayedness::TR_Delayedness(TR::Compilation *comp, TR::Optimizer *optimizer,
                               TR_Structure *rootStructure, bool trace)
    : TR_IntersectionBitVectorAnalysis(comp, comp->getFlowGraph(), optimizer,
                                       trace) {
  _earliestness = new (comp->allocator())
      TR_Earliestness(comp, optimizer, rootStructure, trace);

  if (trace) traceMsg(comp, "Starting Delayedness\n");
...
bool TR_Delayedness::postInitializationProcessing() {
  _inSetInfo = (ContainerType **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(ContainerType *));

  int32_t i;
  for (i = 0; i < _numberOfNodes; i++) allocateContainer(_inSetInfo + i);
  return true;
}
...
//
//
void TR_Delayedness::analyzeTreeTopsInBlockStructure(
  ...
  if (_temp == NULL) allocateBlockInfoContainer(&_temp);
  copyFromInto(_earliestness->_globalAnticipatability
  ...
  *_currentInSetInfo |= *_temp;
  ...
  copyFromInto(_currentInSetInfo,
               _blockAnalysisInfo[blockStructure->getNumber()]);

  _temp->setAll(_numberOfBits);
  *_temp -= *(_earliestness->_globalAnticipatability->_localAnticipatability
                  .getDownwardExposedAnalysisInfo(
  ...
  *(_blockAnalysisInfo[blockStructure->getNumber()]) &= *_temp;
  ...
  while (!(currentTree == exitTree)) {
    if (notSeenTreeWithChecks) {
      ...
      if (currentTreeHasChecks) {
        ...
        copyFromInto(_blockAnalysisInfo[blockStructure->getNumber()],
                     _exceptionInfo);
      }
    } else
      break;
  ...
  getAnalysisInfo(blockStructure)->_containsExceptionTreeTop =
      _containsExceptionTreeTop;
}
