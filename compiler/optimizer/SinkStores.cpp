    ...
    for (int32_t i = 0; i < cfg->getBackwardTraversalLength(); i++) {
        ...
        if (block->getEntry() != NULL) {
            ...
            do {
                ...
                while (bvi.hasMoreElements()) {
                    ...
                    while (storeElement != NULL) {
                        ...
                        if (trace()) {
                            ...
                            if (store->_movable && store->_commonedLoadsUnderTree && store->_commonedLoadsUnderTree->get(killedSymIdx)) {
                ...
                if (!usedSymbols->isEmpty() || !killedSymbols->isEmpty() || (!movedStorePinsCommonedSymbols() && !treeCommonedLoads->isEmpty())) {
                    if (trace())
...
{
    return (_lastSinkOptTransformationIndex == -1) || (_firstSinkOptTransformationIndex <= _numTransformations && _numTransformations <= _lastSinkOptTransformationIndex);
}
...
void TR_SinkStores::placeStoresAlongEdges(List<TR_StoreInformation>& stores, List<TR_EdgeInformation>& edges)
{
    ...
    if (!to->getExceptionPredecessors().empty()) {
        ...
        if (edgeInfo == NULL) {
            if (from->getExit()->getNextTreeTop() == placementBlock->getEntry() && from->canFallThroughToNextBlock() && !from->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets())
            ...
            for (; edgeInfo != NULL; edgeInfo = edgeInfoIt.getNext()) {
                ...
                if ((placementBlock->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::Goto) && !placementBlock->isExtensionOfPreviousBlock() && (from == thisTo->getPrevBlock()) && from->getLastRealTreeTop()->getNode()->getOpCode().isIf()) {
...
bool TR_SinkStores::treeIsSinkableStore(TR::Node* node, bool sinkIndirectLoads, uint32_t& indirectLoadCount, int32_t& depth, bool& isLoadStatic, vcount_t visitCount)
{
    ...
    if (numChildren == 0) {
        ...
        if (node->getOpCode().isStore() && (node->dontEliminateStores() || (node->getSymbolReference()->getSymbol()->isAuto() && (/*node->dontEliminateStores(comp()) ||*/
    ...
    if (!comp()->cg()->getSupportsJavaFloatSemantics() && node->getOpCode().isFloatingPoint() && (underCommonedNode || node->getReferenceCount() > 1)) {
        if (trace())
            traceMsg(comp(), "         fp store failure\n");
        return false;
    }

    if (numChildren == 0 && node->getOpCode().isLoadVarDirect() && node->getSymbolReference()->getSymbol()->isStatic() && (underCommonedNode || node->getReferenceCount() > 1)) {
        if (trace())
...
bool TR_GeneralSinkStores::storeIsSinkingCandidate(TR::Block* block, TR::Node* node, int32_t symIdx, bool sinkIndirectLoads, uint32_t& indirectLoadCount, int32_t& depth, bool& isLoadStatic, vcount_t& treeVisitCount, vcount_t& highVisitCount)
{
    ...
    return (symIdx >= 0 && _liveOnNotAllPaths->_outSetInfo[b]->get(symIdx) && treeIsSinkableStore(node, sinkIndirectLoads, indirectLoadCount, depth, isLoadStatic, comp()->getVisitCount()));
}
...
bool TR_SinkStores::isSafeToSinkThruEdgePlacement(int symIdx, TR::CFGNode* block, TR::CFGNode* succBlock, TR_BitVector* allEdgeInfoUsedOrKilledSymbols)
{
    ...
    if (_placementsForEdgesToBlock[succBlockNumber] != NULL) {
        ...
        for (TR_EdgeStorePlacement* placement = edgeIt.getFirst(); placement != NULL; placement = edgeIt.getNext()) {
            ...
            if (isSafe && from == block && (edgeInfo->_symbolsUsedOrKilled->intersects(*_usedSymbolsToMove) || edgeInfo->_symbolsUsedOrKilled->intersects(*_killedSymbolsToMove)))
                isSafe = false;
        }
    }
    return isSafe;
...
bool nodeContainsCall(TR::Node* node, vcount_t visitCount)
{
    ...
    if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit || (node->getOpCode().isStore() && node->getSymbolReference()->getSymbol()->isStatic()) || (node->getOpCode().hasSymbolReference() && node->getSymbolReference()->isUnresolved()) || (node->getOpCode().hasSymbolReference() && node->getSymbol()->isVolatile()))
        return true;
...
bool TR_SinkStores::shouldSinkStoreAlongEdge(int symIdx, TR::CFGNode* block, TR::CFGNode* succBlock, int32_t sourceBlockFrequency, bool isLoadStatic, vcount_t visitCount, TR_BitVector* allEdgeInfoUsedOrKilledSymbols)
{
    ...
    if (containingLoop != NULL && containingLoop->getEntryBlock() == succBlock) {
        ...
        if (containingLoop->asRegion()->numSubNodes() > 1 || !storeCanMoveThroughBlock(_symbolsKilledInBlock[succBlock->getNumber()], _symbolsUsedInBlock[succBlock->getNumber()], symIdx)) {
            return false;
        }
    }

    // finally, check that the symbol isn't referenced by some store placed along this edge
    return (isSafeToSinkThruEdgePlacement(symIdx, block, succBlock, allEdgeInfoUsedOrKilledSymbols) && !(allEdgeInfoUsedOrKilledSymbols->intersects(*_usedSymbolsToMove) || allEdgeInfoUsedOrKilledSymbols->intersects(*_killedSymbolsToMove)));
}
...
void TR_SinkStores::genStoreToTempSyms(TR::TreeTop* storeLocation,
    TR::Node* node,
    ...
    List<TR_MovableStore>& potentiallyMovableStores)
{
    if (node->getOpCode().isLoadVarDirect() && node->getOpCode().hasSymbolReference()) {
        ...
        if (symIdx != INVALID_LIVENESS_INDEX && commonedLoadsUnderStore->get(symIdx) && !findTempSym(node) && isCorrectCommonedLoad(node, store->getFirstChild()))
        ...
        // FIXME: does isCorrectCommonedLoad need to be called for all symbols or just TR_MethodMetaData symbols
        {
            ...
            if (performTransformation(comp(),
                ...
                while (storeElement != NULL) {
                    ...
                    if (store->_movable && store->_needTempForCommonedLoads && isCorrectCommonedLoad(node, store->_useOrKillInfo->_tt->getNode()->getFirstChild())) {
                        store->_movable = false;
                        if (trace())
...
void TR_SinkStores::replaceLoadsWithTempSym(TR::Node* newNode, TR::Node* origNode, TR_BitVector* needTempForCommonedLoads)
{
    if (newNode->getOpCode().isLoadVarDirect() && newNode->getOpCode().hasSymbolReference() && !newNode->getSymbolReference()->getSymbol()->isStatic()) {
...
*/
    else if (nodeOrig->getOpCode().isLoadVarDirect() && nodeOrig->getOpCode().hasSymbolReference() && !nodeOrig->getSymbolReference()->getSymbol()->isStatic() && getSinkableSymbol(nodeOrig)->getLiveLocalIndex() != INVALID_LIVENESS_INDEX) {
...
{
    bool canMove = (blockKilledSet == NULL || (!blockKilledSet->intersects(*_usedSymbolsToMove) && !blockKilledSet->get(symIdx)))
    ...
    while (!worklist.isEmpty()) {
        ...
        if (!checkLiveMergingPaths(blockEntry, symIdx)) {
            ...
            for (TR::CFGEdge* predEdgeLONAP = predItLONAP.getFirst(); predEdgeLONAP != NULL; predEdgeLONAP = predItLONAP.getNext()) {
                ...
                bool areAllBlocksInEBB = isCurrentBlockInEBB && predBlock->asBlock()->startOfExtendedBlock() == sourceBlock->startOfExtendedBlock();
            ...
            if (_liveOnNotAllPaths->_inSetInfo[blockNumber]->get(symIdx) && storeCanMoveThroughBlock(_symbolsKilledInBlock[blockNumber], _symbolsUsedInBlock[blockNumber], symIdx, allBlockUsedSymbols, allBlockKilledSymbols) && (isCurrentBlockInEBB || (!allBlockKilledSymbols->intersects(*_usedSymbolsToMove) && !allBlockKilledSymbols->get(symIdx) && !allBlockUsedSymbols->intersects(*_killedSymbolsToMove) && !allBlockUsedSymbols->get(symIdx)))) {
                ...
                for (auto succEdge = outEdges.getFirst(); succEdge; succEdge = outEdges.getNext()) {
                    TR::CFGNode* succBlock = succEdge->getTo();
                    int32_t succBlockNumber = succBlock->getNumber();
                    bool isSuccBlockInEBB = isCurrentBlockInEBB && (succBlock->asBlock()->startOfExtendedBlock() == sourceBlock->startOfExtendedBlock());
                    _usedSymbolsToMove = isSuccBlockInEBB ? usedSymbols : allUsedSymbols;
