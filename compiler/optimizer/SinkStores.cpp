    ...
    for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
      ...
      if (liveOnSomePaths->_blockAnalysisInfo[toBlockNumber]) {
        ...
        if (isBackEdge) {
          ...
          if (trace)
            traceMsg(
              comp(), "    Adding backedge live vars from block_%d to LiveOnAllPaths for block_%d\n", toBlockNumber, b);
...
bool TR_OrderedBlockList::removeBlockFromList(TR::Block* block, TR::CFGEdge* edge)
{
  ...
  while (ptr != NULL) {
    ...
    // if block is already in list: remove pred from its list of preds, decrement its count, remove it from the list if
  ...
  if (_placementsForEdgesToBlock[toBlockNumber] != NULL) {
    ...
    while (ptr != NULL) {
      ...
      if (placementEdgeInfo != NULL) {
        ...
        // NOTE that since stores are encountered in backwards order and "add" just puts new stores at the beginning of
        // the list then when we ultimately place the stores on this edge, they will be stored in the correct order
        // (i.e. a store X that originally appeared before another store Y in the code will be on _placement->_stores
        // list in the order X, Y
...
void TR_SinkStores::recordPlacementForDefInBlock(TR_BlockStorePlacement* blockPlacement)
{
  ...
  TR_ASSERT(
    _symbolsKilledInBlock[blockNumber] != NULL, "_symbolsKilledInBlock[%d] should have been initialized!", blockNumber);
...
int32_t TR_SinkStores::performStoreSinking()
{
...

  {
    ...
    //
    _liveVarInfo
      = new (trStackMemory()) TR_LiveVariableInformation(comp(), optimizer(), rootStructure, false, /* !splitLongs */
        true, /* includeParms */
        false);
    ...
    if (usesDataFlowAnalysis()) {
      _liveOnSomePaths
        = new (comp()->allocator()) TR_Liveness(comp(), optimizer(), rootStructure, false, _liveVarInfo, false, true);
      _liveOnAllPaths
        = new (comp()->allocator()) TR_LiveOnAllPaths(comp(), optimizer(), rootStructure, _liveVarInfo, false, true);
    ...
    if (sinkStoresWithIndirectLoads()) {
      ...
      _indirectLoadAnchorMap = new (trStackMemory())
        TR_HashTab(comp()->trMemory(), stackAlloc, comp()->getFlowGraph()->getNextNodeNumber() / 2);
      if (trace())
        traceMsg(
          comp(), "creating _firstUseOfLoadMap with initSize = %d\n", comp()->getFlowGraph()->getNextNodeNumber() / 4);
      _firstUseOfLoadMap = new (trStackMemory())
        TR_HashTab(comp()->trMemory(), stackAlloc, comp()->getFlowGraph()->getNextNodeNumber() / 4);
  ...
  for (int32_t i = 0; i < cfg->getBackwardTraversalLength(); i++) {
    ...
    if (block->getEntry() != NULL) {
      ...
      do {
        ...
        if (trace())
          traceMsg(
            comp(), "      calling findLocalUses on node %p with treeVisitCount %d\n", tt->getNode(), treeVisitCount);
        ...
        if (local != NULL) {
          ...
          if (symIdx != INVALID_LIVENESS_INDEX) {
            TR_ASSERT(symIdx < numLocals,
              "Found store to local with larger index than my liveness information can deal with, idx = %d\n", symIdx);
            if (trace())
        ...
        if (local && (savedLiveCommonedLoads->get(symIdx))) {
          ...
          if (trace())
            traceMsg(
              comp(), "      updating killedLiveCommonedLoads with savedLiveCommonedLoads, setting index %d\n", symIdx);
        ...
        while (bvi.hasMoreElements()) {
          ...
          if (trace())
            traceMsg(comp(),
              "      symbol stores to a live commoned reference, searching stores below that load this symbol via "
              "commoned reference ...\n");
          _killMarkWalks++;
          ...
          while (storeElement != NULL) {
            ...
            if (store->_movable && store->_commonedLoadsUnderTree
              ...
              if (trace()) {
                if (!store->_movable)
                  traceMsg(comp(),
                    "         marking store [" POINTER_PRINTF_FORMAT "] as unmovable because its tree depth is %d\n",
          ...
          while (storeElement != NULL) {
            ...
            if (store->_movable && store->_needTempForCommonedLoads) {
              if (store->_needTempForCommonedLoads->intersects(*firstRefsToKilledSymbols)) {
                // looking at bit vectors alone may not be enough to guarantee that a temp should actually be placed
                // here see the comment in isCorrectCommonedLoad for more information
                ...
                if (trace()) {
                  traceMsg(comp(),
                    "            found store [" POINTER_PRINTF_FORMAT "] below that may require a temp\n",
                    store->_useOrKillInfo->_tt->getNode());
        ...
        if (local != NULL && node->getOpCode().isStoreDirect()) {
          ...
          if (sym && sym->isMethodMetaData()
            ...
            canMoveStore = false;
          } else if (!storeIsSinkingCandidate(block, node, symIdx, false, indirectLoadCount, treeDepth, isLoadStatic,
                       treeVisitCount, highestVisitCount)) // treeDepth is initialized when call return
          {
            if (trace())
          ...
          if (canMoveStore && usedSymbols->intersects(*killedLiveCommonedLoads)) {
            ...
            if (treeDepth >= MIN_TREE_DEPTH_TO_MOVE) {
              ...
              if (trace())
                traceMsg(comp(),
                  "         store will not be moved because its tree depth of %d is too small (MIN_DEPTH = %d)\n",
        ...
        if (!usedSymbols->isEmpty() || !killedSymbols->isEmpty()
          ...
          if (trace())
            traceMsg(comp(),
              "      creating use or kill info on %s node [" POINTER_PRINTF_FORMAT "] to track kills and uses\n",
        ...
        if (movableStore == NULL) {
          if (local && treeCommonedLoads->get(symIdx)) {
            if (trace())
              traceMsg(
                comp(), "      updating killedLiveCommonedLoads with treeCommonedLoads, setting index %d\n", symIdx);
            // this store kills the currently live commoned loads
            killedLiveCommonedLoads->set(symIdx);
          }
        }
        ...
        //   since the kill is below this use, even if we move a store higher up, this use will preserve he old value of
        //   the local
    ...
    if (!block->isExtensionOfPreviousBlock()) {
      ...
      while (!useOrKillInfoList.isEmpty()) {
        ...
        //_liveVarInfo->findAllUseOfLocal(tt->getNode(), storeBlockNumber, _usedSymbolsToMove, _killedSymbolsToMove,
        //NULL, false);

        TR_MovableStore* store = NULL;
        if (isMovableStore) {
          TR_ASSERT(
            !potentiallyMovableStores.isEmpty(), "potentiallyMovableStores should be a subset of useOrKillInfoList!");
          ...
          if (performTransformation(comp(),
                ...
                tt->getNode(), sym ? sym->castToMethodMetaDataSymbol()->getName() : "", store->_depth)
            && performThisTransformation()) {
            int32_t symIdx = store->_symIdx;
            TR_ASSERT(_killedSymbolsToMove->isSet(symIdx),
              "_killedSymbolsToMove is inconsistent with potentiallyMovableStores entry for this store (symIdx=%d)!",
              symIdx);
        ...
        if (movedStore) {
          // Even though the store is pushed outside of the Ebb, we should still set its used sym to be used if it is
          // commoned within the Ebb. For now, just set the sym to be if it is commoned below so that stores from
          // earlier node can push thru this block. Perhaps a better way to do this is to allow earlier store to be sunk
          // along the Ebb path
          if (store->_commonedLoadsAfter) {
            if (usesDataFlowAnalysis()) {
              ...
              if (0 && trace()) {
                traceMsg(
                  comp(), "      Update for movable store due to commonedLoad: LOAP->blockInfo[%d]=", blockNumber);
            ...
            if (trace()) {
              traceMsg(comp(),
                "        symbolsUsed in block_%d is updated due to commoned loads.  Symbols USED BEF is ", blockNumber);
              _symbolsUsedInBlock[blockNumber]->print(comp());
              traceMsg(comp(), "\n");
            }
            ...
            if (trace()) {
              traceMsg(comp(),
                "        symbolsUsed in block_%d is updated due to commoned loads.  Symbols USED AFT is ", blockNumber);
              _symbolsUsedInBlock[blockNumber]->print(comp());
              traceMsg(comp(), "\n");
            }
          }
...
void TR_SinkStores::coalesceSimilarEdgePlacements()
{
  ...
  while (firstEdgePtr != NULL) {
    ...
    // redirected) not really expecting there to be a large number of edges on this list, so just do the stupidest N^2
    // thing for now
    ...
    while (secondEdgePtr != NULL) {
      ...
      if (trace()) {
        traceMsg(comp(), "    Comparing to edge placement (%d,%d) with stores:", secondEdge->getFrom()->getNumber(),
        ...
        while (secondStorePtr != NULL) {
          traceMsg(comp(), " [" POINTER_PRINTF_FORMAT "](copy=%d)", secondStorePtr->getData()->_store->getNode(),
      ...
      if (secondToBlock->getNumber() == firstToBlock->getNumber()) {
        ...
        // the stores should be added to the lists in the same order, so first mismatch in a parallel walk is the exit
        // test
    ...
    for (edgeInfo = edgeInfoIt.getNext(); edgeInfo != NULL; edgeInfo = edgeInfoIt.getNext()) {
      ...
      if (trace())
        traceMsg(comp(), "      changing exception edge (%d,%d) to (%d,%d)\n", from->getNumber(), to->getNumber(),
          from->getNumber(), placementBlock->getNumber());
    ...
    if (!placement->_stores.isEmpty()) {
      ...
      while (!placement->_stores.isEmpty()) {
        ...
        if (storeInfo->_copy) {
          ...
          //               if (trace())
          //                traceMsg(comp(), "    adding store [" POINTER_PRINTF_FORMAT "] to movedStores (edge
          //                placement)\n", store);
  ...
  int symIdx, TR::CFGNode* block, TR::CFGNode* succBlock, TR_BitVector* allEdgeInfoUsedOrKilledSymbols)
{
  ...
  if (_placementsForEdgesToBlock[succBlockNumber] != NULL) {
    ...
    for (TR_EdgeStorePlacement* placement = edgeIt.getFirst(); placement != NULL; placement = edgeIt.getNext()) {
      ...
      TR_ASSERT(
        placement->_edges.getListHead()->getNextElement() == NULL, "edge placement list should only have one edge!");
  ...
  int32_t sourceBlockFrequency, bool isLoadStatic, vcount_t visitCount, TR_BitVector* allEdgeInfoUsedOrKilledSymbols)
{
  ...
  if (isLoadStatic && blockContainsCall(succBlock->asBlock(), comp())) {
    if (trace())
      traceMsg(
        comp(), "            Can't push sym %d to successor block_%d (static load)\n", symIdx, succBlock->getNumber());
    return false;
  }
...
// A single store that is being replicated to a side exit may have 0 or more nodes replaced with anchored nodes for
// different reasons insertAnchoredNodes will take care of all possible node replacements so only a single recursive
// walk of a store is required
uint32_t TR_SinkStores::insertAnchoredNodes(TR_MovableStore* store, TR::Node* nodeCopy, TR::Node* nodeOrig,
  TR::Node* nodeParentCopy, int32_t childNum, TR_BitVector* needTempForCommonedLoads, TR_BitVector* blockingUsedSymbols,
  TR_BitVector* blockingCommonedSymbols, TR::Block* currentBlock,
  List<TR_IndirectLoadAnchor>* indirectLoadAnchorsForThisStore, vcount_t visitCount)
{
  ...
  //    this function.
  // 2. When creating anchors for the fall through store. In this case, nodeParentCopy == NULL and all nodes need only
  // be
  ...
  // In this case only anchors for blockingUsedSymbols and blockingCommonedSymbols need to be created as indirect load
  // nodes are naturally commoned correctly for the fall through sunk store
  if (indirectLoadAnchorsForThisStore && nodeOrig->getOpCode().isLoadIndirect()) {
    ...
    indirectLoadAnchorsForThisStore->add(
      new (trStackMemory()) TR_IndirectLoadAnchor(anchoredNode, currentBlock, store->_useOrKillInfo->_tt));
    if (trace())
  ...
  */
    ...
    && getSinkableSymbol(nodeOrig)->getLiveLocalIndex() != INVALID_LIVENESS_INDEX) {
    ...
    if (blockingUsedSymbols && blockingUsedSymbols->isSet(symIdx)) {
      ...
      if (!firstUseOfLoad) {
        firstUseOfLoad = new (trStackMemory())
          TR_FirstUseOfLoad(nodeOrig, store->_useOrKillInfo->_tt, store->_useOrKillInfo->_block->getNumber());
        TR_HashId addID = 0;
        _firstUseOfLoadMap->add(nodeOrig, addID, firstUseOfLoad);
        if (trace())
          traceMsg(comp(),
            "      insertAnchoredNodes usedBlocked create/add for symIdx %d  firstUse %p with node %p and anchor "
            "treetop\n",
            symIdx, firstUseOfLoad, nodeOrig, store->_useOrKillInfo->_tt->getNode());
      } else {
        if (trace())
          traceMsg(comp(),
            "      insertAnchoredNodes usedBlocked found for symIdx %d firstUse %p with node %p and anchor treetop "
            "%p\n",
            symIdx, firstUseOfLoad, nodeOrig, firstUseOfLoad->getAnchorLocation());
      }
    }
    TR_ASSERT(!firstUseOfLoad || (firstUseOfLoad->getNode() == nodeOrig),
  ...
  for (int32_t i = nodeOrig->getNumChildren() - 1; i >= 0; i--) {
    increment += insertAnchoredNodes(store, nodeCopy ? nodeCopy->getChild(i) : NULL, nodeOrig->getChild(i), nodeCopy, i,
      needTempForCommonedLoads, blockingUsedSymbols, blockingCommonedSymbols, currentBlock,
      indirectLoadAnchorsForThisStore, visitCount);
  }
  if (nodeCopy)
...
bool TR_GeneralSinkStores::sinkStorePlacement(TR_MovableStore* movableStore, bool nextStoreWasMoved)
{
  ...
  for (auto succEdge = outEdges.getFirst(); succEdge; succEdge = outEdges.getNext()) {
    ...
    if (_liveOnSomePaths->_blockAnalysisInfo[succBlock->getNumber()]->get(symIdx)
      ...
      if (!canSinkOutOfEBB && !isSuccBlockInEBB) {
        if (trace())
          traceMsg(
            comp(), "              attempt to sink out of Ebb but commoned syms are killed in the Ebb => don't sink\n");
      ...
      if (shouldSinkStoreAlongEdge(symIdx, sourceBlock, succBlock, sourceBlockFrequency, movableStore->_isLoadStatic,
            visitCount, allEdgeInfoUsedOrKilledSymbols)) {
        if (trace())
  ...
  while (!worklist.isEmpty()) {
    ...
    if (trace())
      traceMsg(
        comp(), "            TT[" POINTER_PRINTF_FORMAT "] trying to sink into block_%d\n", tt->getNode(), blockNumber);
    ...
    if (isCurrentBlockInEBB)
      canSinkOutOfEBB
        = canSinkOutOfEBB && (!commonedSymbols || !commonedSymbols->intersects(*_symbolsKilledInBlock[blockNumber]));
    ...
    if (!checkLiveMergingPaths(blockEntry, symIdx)) {
      ...
      // NOTE: def should go along edge (not at end of pred) because each pred may have other successors to which def
      // was
      ...
      for (TR::CFGEdge* predEdgeLONAP = predItLONAP.getFirst(); predEdgeLONAP != NULL;
        ...
        bool areAllBlocksInEBB
          = isCurrentBlockInEBB && predBlock->asBlock()->startOfExtendedBlock() == sourceBlock->startOfExtendedBlock();
        _usedSymbolsToMove = areAllBlocksInEBB ? usedSymbols : allUsedSymbols;
        bool copyStore = !areAllBlocksInEBB;

        // recordPlacementForDefAlongEdge(tt, predEdgeLONAP, true); // block can't be an extension of source block so
        // copy
        TR_StoreInformation* storeInfo = new (trStackMemory()) TR_StoreInformation(tt, copyStore);
        // init _storeTemp field
        if (!needTempForCommonedLoads)
      ...
      // the liveness of the symbols used by the store can be determined from the liveness of the symbol(s) killed by
      // the store
      ...
      //        the exception flow occurs.  Otherwise, a store might be live at an exception successor from some
      //        exception points within the block but not others, which would force us to split the block to safely push
      //        the store to
      ...
      if (_liveOnNotAllPaths->_inSetInfo[blockNumber]->get(symIdx)
        && storeCanMoveThroughBlock(_symbolsKilledInBlock[blockNumber], _symbolsUsedInBlock[blockNumber], symIdx,
        ...
        && (isCurrentBlockInEBB
          || (!allBlockKilledSymbols->intersects(*_usedSymbolsToMove) && !allBlockKilledSymbols->get(symIdx)
            && !allBlockUsedSymbols->intersects(*_killedSymbolsToMove) && !allBlockUsedSymbols->get(symIdx)))) {
        // 1) the store is not live on all paths at the beginning of block
        // 2) it's safe to push this store to the end of block and the store is live on som successor paths (but not
        // all) so: push the store down to those successors where it is live if we can't push it all the way to the
        // beginning of the successor block, then place def along the edge
        if (trace())
        ...
        for (auto succEdge = outEdges.getFirst(); succEdge; succEdge = outEdges.getNext()) {
          ...
          if (_liveOnSomePaths->_blockAnalysisInfo[succBlockNumber]->get(symIdx)
            || !isSafeToSinkThruEdgePlacement(
              symIdx, block, succBlock, allEdgeInfoUsedOrKilledSymbols) // FIXME:: without this sanity will fail?
          ) {
            if (!canSinkOutOfEBB && !isSuccBlockInEBB) {
              if (trace())
                traceMsg(comp(),
            ...
            if (shouldSinkStoreAlongEdge(symIdx, block, succBlock, sourceBlockFrequency, movableStore->_isLoadStatic,
                  visitCount, allEdgeInfoUsedOrKilledSymbols)) {
              if (trace())
                traceMsg(comp(), "              added %d to worklist\n", succBlock->getNumber());
            ...
            //   if (trace())
            //      traceMsg(comp(), "              attempt to sink out of Ebb but commoned syms are killed in the Ebb
            //      => don't sink\n");
            //   placeDefInBlock = true;
            //
        ...
        if (placeDefInBlock) {
          if (numOfEdgePlacements) {
            if (trace())
              traceMsg(
                comp(), "              all previously added edge store placements from this block will be removed\n");
  ...
  if (sunkPastNonLivePath || _usedSymbolsToMove->isEmpty() || _sinkAllStores) {
    ...
    //       It's true that we could miss an opportunity to move a store that should be LONAP because we moved *this*
    //       store, but let's leave that case on the table until there's a need to catch it
