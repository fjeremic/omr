...
int32_t TR_FieldPrivatizer::detectCanonicalizedPredictableLoops(TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (loopInvariantBlock && ((loopInvariantBlock->getBlock()->getPredecessors().size() == 1) || loopInvariantBlock->getBlock()->getFirstRealTreeTop() == loopInvariantBlock->getBlock()->getLastRealTreeTop())) {
        ...
        if (_stringPeepholeTree && !_stringSymRef->getUseonlyAliases().hasAliases() && !_valueOfSymRef->getUseonlyAliases().hasAliases()) {
            _toStringSymRef = 0;
            _initSymRef = 0;
            _appendSymRef = 0;
        ...
        if (currentNode->getOpCodeValue() == TR::call && !currentNode->getSymbolReference()->isUnresolved()) {
            ...
            if (method->isConstructor()) {
                ...
                if (!strncmp(sig, "(Ljava/lang/String;C)", 21)) {
                    ...
                    if (valueOfString->getOpCode().hasSymbolReference() && currentNode->getFirstChild()->getOpCodeValue() == TR::New) {
                        ...
                        if (prevNode->getOpCode().isStore()) {
                            ...
                            while (prevTree->getNode()->getOpCodeValue() != TR::BBStart) {
                                ...
                                if (prevNode->getOpCode().isStore() && prevNode->getSymbolReference() == _valueOfSymRef && prevNode->getFirstChild()->getOpCode().isLoadVarDirect() && prevNode->getFirstChild()->getSymbolReference() == _stringSymRef) {
                            ...
                            if (foundStringPeephole) {
                                if (!_writtenExactlyOnce.ValueAt(_stringSymRef->getReferenceNumber()) || !_readExactlyOnce.ValueAt(_stringSymRef->getReferenceNumber()) || !_writtenExactlyOnce.ValueAt(_valueOfSymRef->getReferenceNumber()) || !_readExactlyOnce.ValueAt(_valueOfSymRef->getReferenceNumber()))
...
void TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(TR::Node* node, vcount_t visitCount)
{
    ...
    if (opCode.isStore() || opCode.isLoadVar()) {
        ...
        if ((opCode.isIndirect() || staticSym)) {
            ...
            if (canPrivatize && !sym->isArrayShadowSymbol() && !sym->isVolatile() && comp()->cg()->considerTypeForGRA(symRef) && !_neverWritten->get(symRef->getReferenceNumber()) && (opCode.isIndirect() ? subtreeIsInvariantInLoop(node->getFirstChild()) : true) && !isFieldAliasAccessed(symRef) && (symSize <= 8 || sym->getType().isVector())) {
...
bool TR_FieldPrivatizer::bothSubtreesMatch(TR::Node* node1, TR::Node* node2)
{
    ...
    if (node1->getOpCodeValue() == node2->getOpCodeValue()) {
        if (node1->getOpCode().isLoadVar() || (node1->getOpCodeValue() == TR::loadaddr && node1->getSymbolReference()->getSymbol()->isNotCollected())) {
...
bool TR_FieldPrivatizer::subtreeIsInvariantInLoop(TR::Node* node)
{
    ...
    if ((node->getNumChildren() > 1) || !node->getOpCode().isLoadVar() || !_neverWritten->get(node->getSymbolReference()->getReferenceNumber()) || ((node->getNumChildren() > 0) && !subtreeIsInvariantInLoop(node->getFirstChild())))
        return false;
...
void TR_FieldPrivatizer::placeStoresBackInExits(List<TR::Block>* exitBlocks, List<TR::Block>* blocksInLoop)
{
    ...
    for (exitBlock = si.getCurrent(); exitBlock; exitBlock = si.getNext()) {
        ...
        for (auto edge = exitBlock->getSuccessors().begin(); edge != exitBlock->getSuccessors().end();) {
            ...
            if (!blocksInsideLoop->get(next->getNumber()) && storesBackMustBePlacedInExitBlock(exitBlock, next, blocksInsideLoop)) {
...
void TR_FieldPrivatizer::addStringInitialization(TR::Block* block)
{
    ...
    TR::Node* initCall = TR::Node::create(TR::treetop, 1, TR::Node::createWithSymRef(TR::call, 2, 2, initNode->getFirstChild(), TR::Node::createWithSymRef(initNode->getFirstChild(), TR::aload, 0, _stringSymRef), initSymRef));
...
void TR_FieldPrivatizer::cleanupStringPeephole()
{
    ...
    if (!_appendSymRef) {
        ...
        if (prevNode->getOpCode().isStore()) {
            ...
            while (cursorTree) {
                if ((cursorTree->getNode()->getNumChildren() > 0) && (cursorTree->getNode()->getFirstChild() == firstChild)) {
