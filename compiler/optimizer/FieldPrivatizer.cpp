...
TR_FieldPrivatizer::perform()
{
  ...
  detectCanonicalizedPredictableLoops(
    comp()->getFlowGraph()->getStructure(), 0, -1);
...
bool
TR_FieldPrivatizer::isFieldAliasAccessed(TR::SymbolReference* symRef)
{
...
int32_t
TR_FieldPrivatizer::detectCanonicalizedPredictableLoops(
  ...
  int32_t bitVectorSize)
{
  ...
  for (auto predecessor = loopEntryBlock->getPredecessors().begin();
       predecessor != loopEntryBlock->getPredecessors().end();
       ++predecessor) {
  ...
  if (canTransform && !loopInvariantBlock &&
    ...
    TR::TreeTop* gotoTreeTop =
      TR::TreeTop::create(comp(),
                          TR::Node::create(entryBlock->getEntry()->getNode(),
                                           TR::Goto,
                                           0,
                                           entryBlock->getEntry()));
  ...
  if (loopInvariantBlock &&
    ...
    if (isPredictableLoop > 0) {
      if (!loopContainsEscapePoints) {
        ...
        privatizeNonEscapingLoop(
          loopStructure, regionStructure->getEntryBlock(), visitCount);
            ...
            if (next != invariantBlock) {
              ...
              if (next->getSuccessors().size() == 1) {
                ...
                for (auto predEdge = join->getPredecessors().begin();
                     predEdge != join->getPredecessors().end();
                     ++predEdge) {
...
bool
TR_FieldPrivatizer::containsEscapePoints(TR_Structure* structure,
...
bool
TR_FieldPrivatizer::isStringPeephole(TR::Node* currentNode,
...
void
TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(TR_Structure* structure,
                                                       vcount_t visitCount)
{
  if (structure->asBlock() != 0) {
    TR_BlockStructure* blockStructure = structure->asBlock();
...
void
TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(TR::Node* node,
                                                       vcount_t visitCount)
{
  ...
  if (opCode.isStore() || opCode.isLoadVar()) {
    ...
    if ((opCode.isIndirect() || staticSym)) {
      ...
      if (!TR_LocalAnalysis::isSupportedNodeForFieldPrivatization(
            node, comp(), NULL))
        canPrivatize = false;
...
void
TR_FieldPrivatizer::privatizeNonEscapingLoop(TR_Structure* structure,
                                             TR::Block* entryBlock,
                                             vcount_t visitCount)
{
  if (structure->asBlock() != 0) {
...
void
TR_FieldPrivatizer::privatizeFields(TR::Node* node,
                                    bool postDominatesEntry,
                                    vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
    ...
    if (!_fieldsThatCannotBePrivatized->get(symRef->getReferenceNumber())) {
      ...
      bool canPrivatizeBasedOnThisNode =
        performTransformation(comp(),
                              "%s Field access %p using sym ref %d privatized ",
                              optDetailString(),
                              node,
      ...
      if (canPrivatizeBasedOnThisNode) {
        if (!autoForField) {
          ...
          _privatizedFieldSymRefs.add(
            symRef->getReferenceNumber(), index, autoForField);
          _privatizedRegCandidates.add(
            comp()->getGlobalRegisterCandidates()->findOrCreate(autoForField));
          _privatizedFieldNodes.add(node->duplicateTree());
...
bool
TR_FieldPrivatizer::canPrivatizeFieldSymRef(TR::Node* node)
{
...
TR::SymbolReference*
TR_FieldPrivatizer::getPrivatizedFieldAutoSymRef(TR::Node* node)
{
  TR::SymbolReference* symRef = node->getSymbolReference();
  TR_HashId index = 0;
...
bool
TR_FieldPrivatizer::bothSubtreesMatch(TR::Node* node1, TR::Node* node2)
{
  if (node1 == node2)
    return true;
...
bool
TR_FieldPrivatizer::subtreeIsInvariantInLoop(TR::Node* node)
{
...
void
TR_FieldPrivatizer::placeInitializersInLoopInvariantBlock(TR::Block* block)
{
  ListElement<TR::Node>* currentNodeElem = _privatizedFieldNodes.getListHead();
  ...
  while (currentNodeElem) {
    ...
    if (duplicateNode->getOpCode().isStore()) {
      if (duplicateNode->getOpCode().isIndirect()) {
        ...
        TR::Node::recreate(
          duplicateNode,
          comp()->il.opCodeForDirectLoad(duplicateNode->getDataType()));
    ...
    dumpOptDetails(comp(),
                   "%s  Privatizing field #%d with temp #%d\n",
                   optDetailString(),
    ...
    // currentSymRefElem->getData()->getReferenceNumber()))
    {
      TR::Node* initNode = TR::Node::createWithSymRef(
        comp()->il.opCodeForDirectStore(duplicateNode->getDataType()),
        1,
        1,
        duplicateNode,
        currentSymRef);
...
void
TR_FieldPrivatizer::placeStoresBackInExits(List<TR::Block>* exitBlocks,
  ...
  for (exitBlock = si.getCurrent(); exitBlock; exitBlock = si.getNext()) {
    ...
    for (auto edge = exitBlock->getSuccessors().begin();
      ...
      if (!blocksInsideLoop->get(next->getNumber()) &&
          storesBackMustBePlacedInExitBlock(
            exitBlock, next, blocksInsideLoop)) {
        ...
        if (newBlocksHash.locate(next->getNumber(), index)) {
          ...
          newBlock = true;
          nnext = TR::Block::createEmptyBlock(
            next->getEntry()->getNode(), comp(), 0, next);
        ...
        if (newBlock) {
          ...
          if (trace())
            traceMsg(comp(),
                     "placeStoresBackInExits: added block %d freq %d\n",
                     nnext->getNumber(),
                     nnext->getFrequency());
        }
        TR::CFGEdge* newEdge =
        ...
        if (trace()) {
          ...
          traceMsg(comp(),
                   "instead of orig edge %d(%d) -> %d(%d) freq %d\n",
                   from->getNumber(),
                   from->getFrequency(),
                   to->getNumber(),
                   to->getFrequency(),
...
void
TR_FieldPrivatizer::placeStoresBackInExit(TR::Block* block, bool placeAtEnd)
{
  ListElement<TR::Node>* currentNodeElem = _privatizedFieldNodes.getListHead();
  TR::SymbolReference* currentSymRef = NULL;
  TR_HashId index = 0;
...
bool
TR_FieldPrivatizer::storesBackMustBePlacedInExitBlock(
  TR::Block* exitBlock,
  TR::Block* toBlock,
  TR_BitVector* blocksInsideLoop)
{
  ...
  for (auto edge = toBlock->getPredecessors().begin();
       edge != toBlock->getPredecessors().end();
       ++edge) {
    next = (*edge)->getFrom();
    if (!blocksInsideLoop->get(next->getNumber()) &&
...
void
TR_FieldPrivatizer::addPrivatizedRegisterCandidates(TR_Structure* structure)
{
  ListElement<TR_RegisterCandidate>* privatizedCandidate =
    _privatizedRegCandidates.getListHead();
...
void
TR_FieldPrivatizer::addStringInitialization(TR::Block* block)
{
#ifdef J9_PROJECT_SPECIFIC
  ...
  TR::SymbolReference* stringBufferSymRef =
    comp()->getSymRefTab()->findOrCreateClassSymbol(
      methodSymbol, -1, _stringBufferClass);
  ...
  if (!_initSymRef) {
    ...
    comp()->fej9()->getResolvedMethods(
      trMemory(), _stringBufferClass, &stringBufferMethods);
  ...
  if (!performTransformation(comp(),
                             "%s  Inserted string init into symRef #%d\n",
                             optDetailString(),
  ...
  TR::SymbolReference* initSymRef =
    _initSymRef
      ? getSymRefTab()->findOrCreateMethodSymbol(initNode->getFirstChild()
                                                   ->getSymbolReference()
  ...
  TR::Node* initCall = TR::Node::create(
    ...
    TR::Node::createWithSymRef(
      ...
      TR::Node::createWithSymRef(
        initNode->getFirstChild(), TR::aload, 0, _stringSymRef),
      initSymRef));
...
void
TR_FieldPrivatizer::placeStringEpilogueInExits(List<TR::Block>* exitBlocks,
                                               List<TR::Block>* blocksInLoop)
{
#ifdef J9_PROJECT_SPECIFIC
  TR::CFG* cfg = comp()->getFlowGraph();
...
void
TR_FieldPrivatizer::placeStringEpiloguesBackInExit(TR::Block* block,
                                                   bool placeAtEnd)
{
  ...
  TR::SymbolReference* toStringSymRef =
    _toStringSymRef
      ? comp()->getSymRefTab()->findOrCreateMethodSymbol(
          tempStringBufferLoad->getSymbolReference()->getOwningMethodIndex(),
          -1,
...
void
TR_FieldPrivatizer::cleanupStringPeephole()
{
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (!_appendSymRef) {
    ...
    if (prevNode->getOpCode().isStore()) {
      ...
      if (firstChild->getOpCodeValue() == TR::New) {
        ...
        TR::Node* load =
          TR::Node::createWithSymRef(prevNode, TR::aload, 0, _tempStringSymRef);
