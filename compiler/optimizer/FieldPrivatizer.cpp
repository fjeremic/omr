          ...
          for (auto edge = pred->getSuccessors().begin(); edge != pred->getSuccessors().end(); ++edge) {
            ...
            if (next != invariantBlock) {
              ...
              if (next->getSuccessors().size() == 1) {
                ...
                for (auto predEdge = join->getPredecessors().begin(); predEdge != join->getPredecessors().end();
                     ++predEdge) {
...
void TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(TR::Node* node, vcount_t visitCount)
{
  ...
  if (opCode.isStore() || opCode.isLoadVar()) {
    ...
    if ((opCode.isIndirect() || staticSym)) {
      ...
      if (canPrivatize && !sym->isArrayShadowSymbol() && !sym->isVolatile() && comp()->cg()->considerTypeForGRA(symRef)
...
void TR_FieldPrivatizer::privatizeFields(TR::Node* node, bool postDominatesEntry, vcount_t visitCount)
{
  ...
  if (opCode.isStore() || opCode.isLoadVar()) {
    ...
    if (!_fieldsThatCannotBePrivatized->get(symRef->getReferenceNumber())) {
      ...
      bool canPrivatizeBasedOnThisNode = performTransformation(comp(),
        "%s Field access %p using sym ref %d privatized ", optDetailString(), node, symRef->getReferenceNumber());
      if (canPrivatizeBasedOnThisNode) {
        if (!autoForField) {
          ...
          autoForField
            = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), nodeDataType, false, symSize);
...
void TR_FieldPrivatizer::placeInitializersInLoopInvariantBlock(TR::Block* block)
{
  ...
  while (currentNodeElem) {
    ...
    if (_privatizedFieldSymRefs.locate(symRef->getReferenceNumber(), index)) {
      ...
      TR_ASSERT(false,
        "\n field #%d should have been privatized but didn't find corresponding new auto in _privatizedFieldSymRefs",
    ...
    // duplicateNode->getSymbolReference()->getReferenceNumber(), currentSymRefElem->getData()->getReferenceNumber()))
    {
    ...
    for (auto edge = exitBlock->getSuccessors().begin(); edge != exitBlock->getSuccessors().end();) {
      ...
      if (!blocksInsideLoop->get(next->getNumber())
        ...
        if (newBlock) {
          ...
          if (trace())
            traceMsg(
              comp(), "placeStoresBackInExits: added block %d freq %d\n", nnext->getNumber(), nnext->getFrequency());
        }
...
void TR_FieldPrivatizer::addStringInitialization(TR::Block* block)
{
  ...
  TR::SymbolReference* initSymRef = _initSymRef
    ? getSymRefTab()->findOrCreateMethodSymbol(initNode->getFirstChild()->getSymbolReference()->getOwningMethodIndex(),
      -1, _initSymRef->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(), TR::MethodSymbol::Special)
    : 0;
    ...
    if (prevNode->getOpCode().isStore()) {
      ...
      if (firstChild->getOpCodeValue() == TR::New) {
        ...
        TR::SymbolReference* appendSymRef = _appendSymRef
          ? comp()->getSymRefTab()->findOrCreateMethodSymbol(firstChild->getSymbolReference()->getOwningMethodIndex(),
            -1, _appendSymRef->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod(), TR::MethodSymbol::Special)
          : 0;
