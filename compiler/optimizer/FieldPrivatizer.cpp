...
int32_t TR_FieldPrivatizer::perform() {
  ...
  detectCanonicalizedPredictableLoops(comp()->getFlowGraph()->getStructure(), 0,
                                      -1);
...
bool TR_FieldPrivatizer::isFieldAliasAccessed(TR::SymbolReference *symRef) {
  ...
  if (symRef->getUseDefAliases().hasAliases()) {
    TR_ASSERT(_allSymRefs[symRef->getReferenceNumber()] == true,
              "symRef should have been seen in the loop");
...
int32_t TR_FieldPrivatizer::detectCanonicalizedPredictableLoops(
  ...
  for (auto predecessor = loopEntryBlock->getPredecessors().begin();
    ...
    if (predBlock == comp()->getFlowGraph()->getStart()->asBlock()) return 0;
  }
  ...
  if (!loopInvariantBlock &&
    ...
    for (auto pred = entryBlock->getPredecessors().begin();
         pred != entryBlock->getPredecessors().end(); ++pred) {
      TR::Block *predBlock = toBlock((*pred)->getFrom());
      if (blocksInRegion.find(predBlock)) continue;
      ...
      if (node->getOpCode().isJumpWithMultipleTargets() &&
          !node->getOpCode().hasBranchChildren()) {
        canTransform = false;
        break;
      }
    }
  ...
  if (canTransform && !loopInvariantBlock &&
      (regionStructure->getEntryBlock() != comp()->getStartBlock())) {
    TR::Block *entryBlock = regionStructure->getEntryBlock();
    ...
    for (auto pred = entryBlock->getPredecessors().begin();
         pred != entryBlock->getPredecessors().end();) {
      TR::Block *predBlock = toBlock((*pred)->getFrom());
    ...
    TR::TreeTop *gotoTreeTop = TR::TreeTop::create(
        comp(), TR::Node::create(entryBlock->getEntry()->getNode(), TR::Goto, 0,
                                 entryBlock->getEntry()));
  ...
  if (loopInvariantBlock &&
    ...
    _fieldsThatCannotBePrivatized =
        new (trStackMemory()) TR_BitVector(symRefCount, trMemory(), stackAlloc);

    _needToStoreBack = new (trStackMemory())
        TR_BitVector(symRefCount, trMemory(), stackAlloc, growable);
    ...
    int32_t isPredictableLoop = checkLoopForPredictability(
        loopStructure, loopInvariantBlock->getBlock(), 0);
    ...
    bool loopContainsEscapePoints =
        containsEscapePoints(loopStructure, loopContainsStringPeephole);
    if (_stringPeepholeTree &&
      ...
      _initSymRef = 0;
      _appendSymRef = 0;
      _stringBufferClass = fe()->getClassFromSignature(
          "java/lang/StringBuffer", 22, comp()->getCurrentMethod());
    ...
    if (isPredictableLoop > 0) {
      if (!loopContainsEscapePoints) {
        ...
        privatizeNonEscapingLoop(loopStructure,
                                 regionStructure->getEntryBlock(), visitCount);
        ...
        if (!_privatizedFieldNodes.isEmpty()) {
          ...
          TR::Block *pred =
              toBlock(invariantBlock->getPredecessors().front()->getFrom());
          TR::Block *next;
          for (auto edge = pred->getSuccessors().begin();
               edge != pred->getSuccessors().end(); ++edge) {
            next = toBlock((*edge)->getTo());
            if (next != invariantBlock) {
              bool joinHasPredInLoop = false;
              if (next->getSuccessors().size() == 1) {
                TR::Block *join =
                    next->getSuccessors().front()->getTo()->asBlock();
...
bool TR_FieldPrivatizer::containsEscapePoints(TR_Structure *structure,
                                              bool &containsStringPeephole) {
  bool result = false;
  if (structure->asBlock() != 0) {
    ...
    while (currentTree != exitTree) {
      ...
      if (currentNode->exceptionsRaised()) result = true;
...
bool TR_FieldPrivatizer::isStringPeephole(TR::Node *currentNode,
  ...
  if (currentNode->getOpCode().isTreeTop() &&
    ...
    if (currentNode->getOpCodeValue() == TR::call &&
      ...
      TR_ResolvedMethod *method = currentNode->getSymbolReference()
                                      ->getSymbol()
            ...
            if (prevNode->getOpCode().isStore()) {
              ...
              while (prevTree->getNode()->getOpCodeValue() != TR::BBStart) {
                ...
                if (prevNode->getOpCode().isStore() &&
                    ...
                    prevNode->getFirstChild()->getSymbolReference() ==
                        _stringSymRef) {
              ...
              if (foundStringPeephole) {
                if (!_writtenExactlyOnce.ValueAt(
                        _stringSymRef->getReferenceNumber()) ||
                    !_readExactlyOnce.ValueAt(
                        _stringSymRef->getReferenceNumber()) ||
                    !_writtenExactlyOnce.ValueAt(
                        _valueOfSymRef->getReferenceNumber()) ||
                    !_readExactlyOnce.ValueAt(
                        _valueOfSymRef->getReferenceNumber()))
...
void TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(
...
void TR_FieldPrivatizer::detectFieldsThatCannotBePrivatized(
  ...
  if (opCode.isStore() || opCode.isLoadVar()) {
    ...
    if ((opCode.isIndirect() || staticSym)) {
      ...
      if (!TR_LocalAnalysis::isSupportedNodeForFieldPrivatization(node, comp(),
                                                                  NULL))
        canPrivatize = false;
      ...
      if (canPrivatize && !sym->isArrayShadowSymbol() && !sym->isVolatile() &&
          ...
          (opCode.isIndirect() ? subtreeIsInvariantInLoop(node->getFirstChild())
                               : true) &&
...
void TR_FieldPrivatizer::privatizeNonEscapingLoop(TR_Structure *structure,
                                                  TR::Block *entryBlock,
                                                  vcount_t visitCount) {
  if (structure->asBlock() != 0) {
    TR_BlockStructure *blockStructure = structure->asBlock();
    ...
    bool postDominatesEntry =
        _postDominators && _postDominators->dominates(block, entryBlock);
...
void TR_FieldPrivatizer::privatizeFields(TR::Node *node,
                                         bool postDominatesEntry,
                                         vcount_t visitCount) {
  if (node->getVisitCount() == visitCount) return;
  node->setVisitCount(visitCount);
  ...
  if (opCode.isStore() || opCode.isLoadVar()) {
    ...
    if (!_fieldsThatCannotBePrivatized->get(symRef->getReferenceNumber())) {
      ...
      bool canPrivatizeBasedOnThisNode = performTransformation(
          comp(), "%s Field access %p using sym ref %d privatized ",
          optDetailString(), node, symRef->getReferenceNumber());
      if (canPrivatizeBasedOnThisNode) {
        if (!autoForField) {
          ...
          _privatizedFieldSymRefs.add(symRef->getReferenceNumber(), index,
                                      autoForField);
          _privatizedRegCandidates.add(
              comp()->getGlobalRegisterCandidates()->findOrCreate(
                  autoForField));
          _privatizedFieldNodes.add(node->duplicateTree());
        } else if (!_privatizedFields->get(symRef->getReferenceNumber())) {
          _privatizedFields->set(symRef->getReferenceNumber());
          // Even we are reusing temp, need to save this for initialization and
          // store back
          _privatizedFieldNodes.add(node->duplicateTree());
        }
        ...
        if (opCode.isIndirect()) {
          if (opCode.isStore()) {
            ...
            TR::Node::recreate(node,
                               comp()->il.opCodeForDirectStore(nodeDataType));
...
bool TR_FieldPrivatizer::canPrivatizeFieldSymRef(TR::Node *node) {
  ...
  while (currentNodeElem) {
    ...
    TR::SymbolReference *symRefInCurrentNode =
        currentNode->getSymbolReference();
    TR::SymbolReference *symRef = node->getSymbolReference();

    if (symRefInCurrentNode->getReferenceNumber() ==
        symRef->getReferenceNumber()) {
      // Found a node that looks likely to have been privatized
      //
      if (!node->getOpCode().isIndirect()) return true;

      if (bothSubtreesMatch(currentNode->getFirstChild(),
                            node->getFirstChild()))
        return true;
      else
        return false;
  ...
  //    1. If the symref is in _privatizedFields then we add this node to the
  //    list of those that can't be privatized.
  //    2. If the symref is NOT in _privatizedFields we add it to privatized
  //    fields, so essentially it's a true.

  return false;
}

TR::SymbolReference *TR_FieldPrivatizer::getPrivatizedFieldAutoSymRef(
    TR::Node *node) {
  TR::SymbolReference *symRef = node->getSymbolReference();
  TR_HashId index = 0;
...
bool TR_FieldPrivatizer::subtreeIsInvariantInLoop(TR::Node *node) {
  if (node->getOpCodeValue() == TR::loadaddr &&
      node->getSymbolReference()->getSymbol()->isNotCollected()) {
    return true;
  }
...
void TR_FieldPrivatizer::placeInitializersInLoopInvariantBlock(
    TR::Block *block) {
  ListElement<TR::Node> *currentNodeElem = _privatizedFieldNodes.getListHead();
  ...
  while (currentNodeElem) {
    ...
    if (duplicateNode->getOpCode().isStore()) {
      if (duplicateNode->getOpCode().isIndirect()) {
        TR::Node::recreate(duplicateNode,
                           comp()->il.opCodeForCorrespondingIndirectStore(
                               duplicateNode->getOpCodeValue()));
        duplicateNode->setNumChildren(1);
      } else {
        TR::Node::recreate(duplicateNode, comp()->il.opCodeForDirectLoad(
                                              duplicateNode->getDataType()));
    ...
    if (_privatizedFieldSymRefs.locate(symRef->getReferenceNumber(), index)) {
      currentSymRef =
          ((TR::SymbolReference *)_privatizedFieldSymRefs.getData(index));
    } else
      TR_ASSERT(false,
                "\n field #%d should have been privatized but didn't find "
                "corresponding new auto in _privatizedFieldSymRefs",
                symRef->getReferenceNumber());

    dumpOptDetails(comp(), "%s  Privatizing field #%d with temp #%d\n",
                   optDetailString(),
                   duplicateNode->getSymbolReference()->getReferenceNumber(),
                   currentSymRef->getReferenceNumber());

    // if (performTransformation(comp(), "%s  Privatizing field #%d with temp
    // #%d\n", optDetailString(),
    // duplicateNode->getSymbolReference()->getReferenceNumber(),
    // currentSymRefElem->getData()->getReferenceNumber()))
    {
      TR::Node *initNode = TR::Node::createWithSymRef(
          comp()->il.opCodeForDirectStore(duplicateNode->getDataType()), 1, 1,
          duplicateNode, currentSymRef);
...
void TR_FieldPrivatizer::placeStoresBackInExits(List<TR::Block> *exitBlocks,
                                                List<TR::Block> *blocksInLoop) {
  TR::CFG *cfg = comp()->getFlowGraph();
  TR_BitVector *seenExitBlocks = new (trStackMemory())
      TR_BitVector(cfg->getNextNodeNumber(), trMemory(), stackAlloc, growable);
  TR_BitVector *blocksInsideLoop = new (trStackMemory())
      TR_BitVector(cfg->getNextNodeNumber(), trMemory(), stackAlloc);
  ...
  for (exitBlock = si.getCurrent(); exitBlock; exitBlock = si.getNext()) {
    ...
    for (auto edge = exitBlock->getSuccessors().begin();
      ...
      if (!blocksInsideLoop->get(next->getNumber()) &&
          storesBackMustBePlacedInExitBlock(exitBlock, next,
                                            blocksInsideLoop)) {
        ...
        if (newBlocksHash.locate(next->getNumber(), index)) {
          ...
          newBlock = true;
          nnext = TR::Block::createEmptyBlock(next->getEntry()->getNode(),
                                              comp(), 0, next);
          newBlocksHash.add(next->getNumber(), index, nnext);
        }

        if (!parent)
          parent = exitBlock->getCommonParentStructureIfExists(
              next, comp()->getFlowGraph());
        ...
        if (exitBlock->getExit()->getNextTreeTop() == next->getEntry()) {
          ...
          // The block nnext must be appended to the end before, now we can move
          // it to here-pefect
          else {
          ...
          if (newBlock) {
            ...
            TR::Node *gotoNode =
                TR::Node::create(next->getEntry()->getNode(), TR::Goto);
        ...
        // D191793, there are cases while we goto next block, so update
        // branchDestination conditionaly for safety
        ...
        TR::CFGEdge *newEdge =
            comp()->getFlowGraph()->addEdge(exitBlock, nnext);
        newEdge->setFrequency(current->getFrequency());
        if (trace()) {
          ...
          traceMsg(comp(), "new edge %d(%d) -> %d(%d) freq %d\n",
                   from->getNumber(), from->getFrequency(), to->getNumber(),
                   to->getFrequency(), current->getFrequency());
...
void TR_FieldPrivatizer::placeStoresBackInExit(TR::Block *block,
                                               bool placeAtEnd) {
  ListElement<TR::Node> *currentNodeElem = _privatizedFieldNodes.getListHead();
  TR::SymbolReference *currentSymRef = NULL;
  TR_HashId index = 0;
  ListElement<TR_RegisterCandidate> *privatizedCandidate =
      _privatizedRegCandidates.getListHead();

  int32_t blockWeight = 1;
  optimizer()->getStaticFrequency(block, &blockWeight);

  TR::TreeTop *insertionPoint = block->getEntry();
  if (placeAtEnd) insertionPoint = block->getLastRealTreeTop();
  ...
  while (currentNodeElem) {
    ...
    if (_needToStoreBack->get(currentSymRef->getReferenceNumber())) {
      if (currentNode->getOpCode().isIndirect()) {
        if (!duplicateNode->getOpCode().isStore()) {
          TR::Node::recreate(duplicateNode,
                             comp()->il.opCodeForCorrespondingIndirectLoad(
                                 duplicateNode->getOpCodeValue()));
        }
        ...
        TR::Node *storedBackValue = TR::Node::createWithSymRef(
            duplicateNode,
            comp()->il.opCodeForDirectLoad(duplicateNode->getDataType()), 0,
            currentSymRef);
        duplicateNode->setAndIncChild(1, storedBackValue);
      } else {
        if (!duplicateNode->getOpCode().isStore())
          TR::Node::recreate(duplicateNode, comp()->il.opCodeForDirectStore(
                                                duplicateNode->getDataType()));
        if (duplicateNode->getOpCode().isWrtBar())
          duplicateNode->setNumChildren(2);
        else
      ...
      TR::TreeTop *storeBackTree =
          TR::TreeTop::create(comp(), duplicateNode, 0, 0);

      if (placeAtEnd && insertionPoint &&
          (insertionPoint->getNode()->getOpCodeValue() != TR::BBStart)) {
...
bool TR_FieldPrivatizer::storesBackMustBePlacedInExitBlock(
  ...
  TR::CFGNode *next;
  TR::CFGEdge *edge;
  for (auto edge = toBlock->getPredecessors().begin();
       edge != toBlock->getPredecessors().end(); ++edge) {
    next = (*edge)->getFrom();
    if (!blocksInsideLoop->get(next->getNumber()) &&
...
void TR_FieldPrivatizer::addPrivatizedRegisterCandidates(
    TR_Structure *structure) {
  ListElement<TR_RegisterCandidate> *privatizedCandidate =
      _privatizedRegCandidates.getListHead();
  while (privatizedCandidate) {
    if (performTransformation(comp(),
                              "%s Adding auto %d (created for privatization) "
                              "as a global register candidate in loop %d\n",
                              optDetailString(),
                              privatizedCandidate->getData()
                                  ->getSymbolReference()
                                  ->getReferenceNumber(),
                              structure->getNumber()))
...
void TR_FieldPrivatizer::addStringInitialization(TR::Block *block) {
  ...
  TR::ResolvedMethodSymbol *methodSymbol =
      comp()->getOwningMethodSymbol(vmMethod);

  TR::TreeTop *insertionPoint = block->getEntry();

  _stringBufferClass = fe()->getClassFromSignature("java/lang/StringBuffer", 22,
                                                   comp()->getCurrentMethod());
  if (!_stringBufferClass) return;
  // the call to findOrCreateClassSymbol is safe even though we pass CPI of -1
  // since it is guarded by the getClassFromSignature call for _stringBuffer
  TR::SymbolReference *stringBufferSymRef =
      comp()->getSymRefTab()->findOrCreateClassSymbol(methodSymbol, -1,
                                                      _stringBufferClass);
  TR::Node *classObject = TR::Node::createWithSymRef(
      insertionPoint->getNode(), TR::loadaddr, 0, stringBufferSymRef);
  TR::Node *initNode = TR::Node::createWithSymRef(
      TR::New, 1, 1, classObject,
  ...
  if (!_initSymRef) {
    ...
    comp()->fej9()->getResolvedMethods(trMemory(), _stringBufferClass,
                                       &stringBufferMethods);
    ListIterator<TR_ResolvedMethod> it(&stringBufferMethods);
    for (TR_ResolvedMethod *method = it.getCurrent(); method;
         method = it.getNext()) {
      if (method->isConstructor()) {
        char *sig = method->signatureChars();
  ...
  if (!performTransformation(
          comp(), "%s  Inserted string init into symRef #%d\n",
          optDetailString(), _tempStringSymRef->getReferenceNumber()))
    return;

  TR::SymbolReference *initSymRef =
      _initSymRef ? getSymRefTab()->findOrCreateMethodSymbol(
                        initNode->getFirstChild()
                            ->getSymbolReference()
                            ->getOwningMethodIndex(),
                        -1,
                        _initSymRef->getSymbol()
                            ->getResolvedMethodSymbol()
                            ->getResolvedMethod(),
                        TR::MethodSymbol::Special)
                  : 0;
  TR::Node *initCall = TR::Node::create(
      TR::treetop, 1,
      TR::Node::createWithSymRef(
          TR::call, 2, 2, initNode->getFirstChild(),
          TR::Node::createWithSymRef(initNode->getFirstChild(), TR::aload, 0,
                                     _stringSymRef),
          initSymRef));
  ...
  initNode = TR::Node::createWithSymRef(
      TR::astore, 1, 1, initNode->getFirstChild(), _tempStringSymRef);
...
void TR_FieldPrivatizer::placeStringEpilogueInExits(
    List<TR::Block> *exitBlocks, List<TR::Block> *blocksInLoop) {
#ifdef J9_PROJECT_SPECIFIC
  TR::CFG *cfg = comp()->getFlowGraph();
  TR_BitVector *seenExitBlocks = new (trStackMemory())
      TR_BitVector(cfg->getNextNodeNumber(), trMemory(), stackAlloc);
...
void TR_FieldPrivatizer::placeStringEpiloguesBackInExit(TR::Block *block,
                                                        bool placeAtEnd) {
  ...
  TR::Node *tempStringBufferLoad = TR::Node::createWithSymRef(
      insertionPoint->getNode(), TR::aload, 0, _tempStringSymRef);

  TR::SymbolReference *toStringSymRef =
      _toStringSymRef ? comp()->getSymRefTab()->findOrCreateMethodSymbol(
                            tempStringBufferLoad->getSymbolReference()
                                ->getOwningMethodIndex(),
                            -1,
                            _toStringSymRef->getSymbol()
                                ->getResolvedMethodSymbol()
                                ->getResolvedMethod(),
                            TR::MethodSymbol::Virtual)
                      : 0;
  TR::Node *toStringCall = TR::Node::createWithSymRef(
      TR::acall, 1, 1, tempStringBufferLoad, toStringSymRef);

  TR::Node *treetopNode = TR::Node::create(TR::treetop, 1, toStringCall);
  TR::TreeTop *treetop = TR::TreeTop::create(comp(), treetopNode, 0, 0);

  TR::Node *storeBack =
      TR::Node::createWithSymRef(TR::astore, 1, 1, toStringCall, _stringSymRef);
  ...
  if (!_appendSymRef) {
    ...
    if (prevNode->getOpCode().isStore()) {
      ...
      if (firstChild->getOpCodeValue() == TR::New) {
        ...
        TR::SymbolReference *appendSymRef =
            _appendSymRef
                ? comp()->getSymRefTab()->findOrCreateMethodSymbol(
                      firstChild->getSymbolReference()->getOwningMethodIndex(),
                      -1,
                      _appendSymRef->getSymbol()
                          ->getResolvedMethodSymbol()
                          ->getResolvedMethod(),
                      TR::MethodSymbol::Special)
                : 0;
        ...
        TR::Node *load = TR::Node::createWithSymRef(prevNode, TR::aload, 0,
                                                    _tempStringSymRef);
...
const char *TR_FieldPrivatizer::optDetailString() const throw() {
