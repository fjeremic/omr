...
class TR_StripMiner : public TR_LoopTransformer {
 ...
 private:
  ...
  void examineLoop(LoopInfo *li, TR_ClonedLoopType type,
                   bool checkClone = true);
  void examineNode(LoopInfo *li, TR::Node *parent, TR::Node *node,
                   TR::SymbolReference *oldSymRef, vcount_t visitCount,
                   TR_ClonedLoopType type, bool checkClone, int32_t childNum);
  void replaceLoopPivs(LoopInfo *li, TR::ILOpCodes newOpCode,
                       TR::Node *newConst, TR::SymbolReference *newSymRef,
