 ...
 *
 * See: "Efficient Computation of LALR(1) Look-Ahead Sets" p.625, DeRemer and
 * Penello 1982.
 *
...
class TR_ReachabilityAnalysis {
 ...
 protected:
  ...
  void traverse(blocknum_t blockNum, int32_t depth, blocknum_t *stack,
                blocknum_t *depth_map, TR_BitVector *closure);

  virtual void propagateInputs(blocknum_t blockNum, int32_t depth,
                               blocknum_t *stack, blocknum_t *depth_map,
                               TR_BitVector *closure) = 0;

  void propagateOneInput(blocknum_t inputBlockNum, blocknum_t blockNum,
...
// Forward Reachability answers "which blocks can be reached from an origin
// block"
//
...
class TR_ForwardReachabilityWithoutExceptionEdges
    : public TR_ReachabilityAnalysis {
 protected:
  virtual void propagateInputs(blocknum_t blockNum, int32_t depth,
                               blocknum_t *stack, blocknum_t *depth_map,
                               TR_BitVector *closure);
...
class TR_CanReachGivenBlocks : public TR_BackwardReachability {
 ...
 public:
  TR_CanReachGivenBlocks(TR::Compilation *comp, TR_BitVector *originBlocks)
      : TR_BackwardReachability(comp), _originBlocks(originBlocks) {}
};
...
class TR_CanBeReachedWithoutExceptionEdges
    : public TR_ForwardReachabilityWithoutExceptionEdges {
 protected:
  virtual bool isOrigin(TR::Block *block);
