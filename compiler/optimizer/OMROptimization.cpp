...
void OMR::Optimization::requestOpt(OMR::Optimizations optNum, bool value,
                                   TR::Block *block) {
  TR::OptimizationManager *manager =
      self()->optimizer()->getOptimization(optNum);
  if (manager) manager->setRequested(value, block);
}
 ...
 */
void OMR::Optimization::anchorChildren(TR::Node *node, TR::TreeTop *anchorTree,
                                       uint32_t depth, bool hasCommonedAncestor,
                                       TR::Node *replacement) {
 ...
 */
bool OMR::Optimization::removeOrconvertIfToGoto(
    TR::Node *&node, TR::Block *block, int takeBranch, TR::TreeTop *curTree,
    TR::TreeTop *&reachableTarget, TR::TreeTop *&unreachableTarget,
 ...
 */
TR::Node *OMR::Optimization::replaceNode(TR::Node *node, TR::Node *other,
                                         TR::TreeTop *anchorTree,
                                         bool anchorChildren) {
  if (!performTransformation(self()->comp(),
...
bool OMR::Optimization::nodeIsOrderDependent(TR::Node *node, uint32_t depth,
