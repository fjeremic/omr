...
void OMR::Optimization::anchorAllChildren(TR::Node* node, TR::TreeTop* anchorTree)
{
  ...
  if (self()->trace())
    traceMsg(
      self()->comp(), "%sanchoring children of node [" POINTER_PRINTF_FORMAT "]\n", self()->optDetailString(), node);
  ...
  if (correctBCDPrecision && node->getType().isBCD() && child->getType().isBCD()
    ...
    if (node->getReferenceCount() > 1) {
      ...
      dumpOptDetails(self()->comp(),
        ...
        self()->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child);
      TR_ASSERT(
        node->getReferenceCount() == 1, "node %p refCount should be 1 and not %d\n", node, node->getReferenceCount());
...
TR::Node* OMR::Optimization::replaceNode(TR::Node* node, TR::Node* other, TR::TreeTop* anchorTree, bool anchorChildren)
{
  if (!performTransformation(self()->comp(),
        ...
        node, node->getOpCode().getName(), other, other->getOpCode().getName())) {
