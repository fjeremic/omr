 ...
 */
void OMR::Optimization::anchorChildren(TR::Node* node, TR::TreeTop* anchorTree, uint32_t depth, bool hasCommonedAncestor, TR::Node* replacement)
{
 ...
 */
bool OMR::Optimization::removeOrconvertIfToGoto(TR::Node*& node, TR::Block* block, int takeBranch, TR::TreeTop* curTree, TR::TreeTop*& reachableTarget, TR::TreeTop*& unreachableTarget, const char* opt_details)
{
...
{
#ifdef J9_PROJECT_SPECIFIC
    if (correctBCDPrecision && node->getType().isBCD() && child->getType().isBCD() && node->getDecimalPrecision() != child->getDecimalPrecision()) {
        ...
        for (int32_t i = 0; i < node->getNumChildren(); i++) {
            ...
            } else if (!node->getChild(i)->getOpCode().isLoadConst() && node->getChild(i)->getOpCodeValue() != TR::loadaddr) {
...
#endif
    {
        ...
        for (int32_t i = 0; i < node->getNumChildren(); i++) {
            ...
            if (node->getChild(i) != child && (!node->getChild(i)->getOpCode().isLoadConst() || node->getChild(i)->anchorConstChildren()) && node->getChild(i)->getOpCodeValue() != TR::loadaddr) {
                needAnchor = true;
                break;
            }
        }
...
bool OMR::Optimization::nodeIsOrderDependent(TR::Node* node, uint32_t depth, bool hasCommonedAncestor)
{
    ...
    return ((node->getOpCode().isLoad() && node->getOpCode().hasSymbolReference() && ((node->getReferenceCount() > 1) || hasCommonedAncestor)) || ((!node->getOpCode().isLoadConst() || constNeedsAnchor) && depth >= MAX_DEPTH_FOR_SMART_ANCHORING));
}
