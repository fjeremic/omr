...
void
OMR::Optimization::requestOpt(OMR::Optimizations optNum,
                              bool value,
                              TR::Block* block)
{
 ...
 */
void
OMR::Optimization::anchorChildren(TR::Node* node,
 ...
 */
bool
OMR::Optimization::removeOrconvertIfToGoto(TR::Node*& node,
                                           TR::Block* block,
                                           int takeBranch,
                                           TR::TreeTop* curTree,
 ...
 */
TR::CFGEdge*
OMR::Optimization::changeConditionalToUnconditional(TR::Node*& node,
                                                    TR::Block* block,
                                                    int takeBranch,
                                                    TR::TreeTop* curTree,
                                                    const char* opt_details)
{
  ...
  if (!self()->removeOrconvertIfToGoto(node,
                                       block,
                                       takeBranch,
                                       curTree,
                                       reachableTarget,
 ...
 */
void
OMR::Optimization::prepareToStopUsingNode(TR::Node* node,
                                          TR::TreeTop* anchorTree,
 ...
 */
TR::Node*
OMR::Optimization::replaceNodeWithChild(TR::Node* node,
                                        TR::Node* child,
                                        TR::TreeTop* anchorTree,
                                        TR::Block* block,
  ...
  if (correctBCDPrecision && node->getType().isBCD() &&
    ...
    if (node->getReferenceCount() > 1) {
      ...
      dumpOptDetails(
        ...
        newNode);
      return self()->replaceNode(
        node, newNode, anchorTree, false); // needAnchor=false
    } else {
      dumpOptDetails(
        self()->comp(),
        "%sPrecision mismatch when replacing parent %s [" POINTER_PRINTF_FORMAT
...
#endif
  {
    TR_ASSERT(node->hasChild(child),
              "node %p is not a child of node %p\n",
              child,
              node);
 ...
 */
TR::Node*
OMR::Optimization::replaceNode(TR::Node* node,
                               TR::Node* other,
                               TR::TreeTop* anchorTree,
                               bool anchorChildren)
{
...
OMR::Optimization::removeNode(TR::Node* node, TR::TreeTop* anchorTree)
{
  ...
  //
  if (performTransformation(self()->comp(),
                            "%sRemoving redundant node [" POINTER_PRINTF_FORMAT
                            "] %s\n",
                            self()->optDetailString(),
                            node,
                            node->getOpCode().getName())) {
...
bool
OMR::Optimization::nodeIsOrderDependent(TR::Node* node,
                                        uint32_t depth,
                                        bool hasCommonedAncestor)
{
