...
TR_Latestness::getKind()
{
  return Latestness;
}

TR_Latestness*
TR_Latestness::asLatestness()
{
  return this;
}

int32_t
TR_Latestness::getNumberOfBits()
{
  return _delayedness->_numberOfBits;
}

TR_Latestness::TR_Latestness(TR::Compilation* comp,
                                             ...
                                             trace)
{
  ...
  for (nextNode = cfg->getFirstNode(); nextNode;
    ...
    for (auto succ = nextNode->getSuccessors().begin();
         succ != nextNode->getSuccessors().end();
         ++succ) {
    ...
    /////if (getAnalysisInfo(blockStructure)->_containsExceptionTreeTop)
    {
      for (auto succ = nextNode->getExceptionSuccessors().begin();
           succ != nextNode->getExceptionSuccessors().end();
           ++succ) {
        TR::CFGNode* succBlock = (*succ)->getTo();
        compose(intersection, _delayedness->_inSetInfo[succBlock->getNumber()]);
      }
...
//
void
TR_Latestness::analyzeNode(TR::Node*,
                           vcount_t,
                           TR_BlockStructure*,
                           ContainerType*)
{}
...
void
TR_Latestness::analyzeTreeTopsInBlockStructure(
