...
TR_Latestness::TR_Latestness(TR::Compilation *comp, TR::Optimizer *optimizer,
                             TR_Structure *rootStructure, bool trace)
    : TR_BackwardIntersectionBitVectorAnalysis(comp, comp->getFlowGraph(),
                                               optimizer, trace) {
  _delayedness = new (comp->allocator())
      TR_Delayedness(comp, optimizer, rootStructure, trace);

  _supportedNodesAsArray = _delayedness->_supportedNodesAsArray;

  if (trace) traceMsg(comp, "Starting Latestness\n");
  ...
  // Allocate temp bit vectors from block info, since it is local to this
  // analysis
  ...
  for (nextNode = cfg->getFirstNode(); nextNode;
    ...
    for (auto succ = nextNode->getSuccessors().begin();
         succ != nextNode->getSuccessors().end(); ++succ) {
    ...
    *(_inSetInfo[blockStructure->getNumber()]) |=
        *(_delayedness->_earliestness->_globalAnticipatability
              ->_localAnticipatability.getDownwardExposedAnalysisInfo(
                  blockStructure->getBlock()->getNumber()));
    *(_inSetInfo[blockStructure->getNumber()]) &=
        *(_delayedness->_inSetInfo[blockStructure->getNumber()]);
...
//
//
void TR_Latestness::analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *,
                                ContainerType *) {}

void TR_Latestness::analyzeTreeTopsInBlockStructure(
