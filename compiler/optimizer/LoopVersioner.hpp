...
public:
  ...
  TR_NodeParentSymRef(TR::Node *node, TR::Node *parent,
...
public:
  ...
  TR_NodeParentSymRefWeightTuple(TR::Node *node, TR::Node *parent,
                                 TR::SymbolReference *symRef, int32_t weight)
...
struct VGLoopInfo {
  VGLoopInfo()
      : _count(0), _coldGuardLoopInvariantBlock(NULL), _loopTempSymRef(NULL),
        _targetNum(-1) {}
  int32_t _count;
...
struct VirtualGuardInfo : public TR_Link<VirtualGuardInfo> {
  VirtualGuardInfo(TR::Compilation *c) : _virtualGuardPairs(c->trMemory()) {
...
public:
  TR_LoopVersioner(TR::OptimizationManager *manager,
                   bool onlySpecialize = false, bool refineAliases = false);
...
protected:
  ...
  struct LoopEntryPrep {
    ...
    LoopEntryPrep(Kind kind, const Expr *expr, TR::Region &memoryRegion)
        : _kind(kind), _expr(expr), _deps(memoryRegion), _emitted(false),
          _requiresPrivatization(false), _unsafelyEmitted(false) {}
    ...
    /// True when either this LoopEntryPrep or a (transitive) dependency
    /// privatizes.
  ...
  public:
    LoopBodySearch(TR::Compilation *comp, TR::Region &memRegion,
                   TR_RegionStructure *loop, TR::NodeChecklist *removedNodes,
  ...
  typedef TR::typed_allocator<std::pair<const Expr, const Expr *>, TR::Region &>
      ExprTableAlloc;
  typedef std::map<Expr, const Expr *, std::less<Expr>, ExprTableAlloc>
      ExprTable;

  typedef TR::typed_allocator<std::pair<TR::Node *const, const Expr *>,
                              TR::Region &>
      NodeToExprAlloc;
  typedef std::map<TR::Node *, const Expr *, std::less<TR::Node *>,
                   NodeToExprAlloc>
      NodeToExpr;

  typedef TR::typed_allocator<std::pair<const PrepKey, LoopEntryPrep *>,
                              TR::Region &>
      PrepTableAlloc;
  typedef std::map<PrepKey, LoopEntryPrep *, std::less<PrepKey>, PrepTableAlloc>
      PrepTable;

  typedef TR::typed_allocator<std::pair<const Expr *const, LoopEntryPrep *>,
                              TR::Region &>
      NullTestPrepMapAlloc;
  typedef std::map<const Expr *, LoopEntryPrep *, std::less<const Expr *>,
                   NullTestPrepMapAlloc>
      NullTestPrepMap;

  typedef TR::typed_allocator<std::pair<TR::Node *const, LoopEntryPrep *>,
                              TR::Region &>
      NodePrepMapAlloc;
  typedef std::map<TR::Node *, LoopEntryPrep *, std::less<TR::Node *>,
                   NodePrepMapAlloc>
      NodePrepMap;

  typedef TR::typed_allocator<std::pair<const Expr *const, PrivTemp>,
                              TR::Region &>
      PrivTempMapAlloc;
  typedef std::map<const Expr *, PrivTemp, std::less<const Expr *>,
                   PrivTempMapAlloc>
      PrivTempMap;

  typedef TR::typed_allocator<std::pair<const Expr *const, TR::Node *>,
                              TR::Region &>
      EmitExprMemoAlloc;
  typedef std::map<const Expr *, TR::Node *, std::less<const Expr *>,
                   EmitExprMemoAlloc>
      EmitExprMemo;
   ...
   */
  struct CurLoop {
    CurLoop(TR::Compilation *comp, TR::Region &memRegion,
            TR_RegionStructure *loop);

    /// A memory region that is freed when versioner finishes processing this
    /// loop.
    ...
    /**
     * \brief Map from \c BNDCHKwithSpineCheck node to bound check
     * LoopEntryPrep.
     *
  ...
  public:
    ...
    RemoveAsyncCheck(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                     TR::TreeTop *asyncCheckTree)
        : LoopImprovement(versioner, prep), _asyncCheckTree(asyncCheckTree) {}
  ...
  public:
    ...
    RemoveNullCheck(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                    TR::Node *nullCheckNode)
        : LoopImprovement(versioner, prep), _nullCheckNode(nullCheckNode) {}
  ...
  public:
    ...
    RemoveBoundCheck(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                     TR::TreeTop *boundCheckTree)
        : LoopImprovement(versioner, prep), _boundCheckTree(boundCheckTree) {}
  ...
  public:
    ...
    RemoveSpineCheck(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                     TR::TreeTop *spineCheckTree)
        : LoopImprovement(versioner, prep), _spineCheckTree(spineCheckTree) {}
  ...
  public:
    ...
    RemoveDivCheck(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                   TR::Node *divCheckNode)
        : LoopImprovement(versioner, prep), _divCheckNode(divCheckNode) {}
  ...
  public:
    ...
    RemoveWriteBarrier(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                       TR::Node *awrtbariNode)
        : LoopImprovement(versioner, prep), _awrtbariNode(awrtbariNode) {}
  ...
  public:
    ...
    RemoveCheckCast(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                    TR::TreeTop *checkCastTree)
        : LoopImprovement(versioner, prep), _checkCastTree(checkCastTree) {}
  ...
  public:
    ...
    RemoveArrayStoreCheck(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
  ...
  public:
    ...
    FoldConditional(TR_LoopVersioner *versioner, LoopEntryPrep *prep,
                    TR::Node *conditionalNode, bool reverseBranch,
                    bool original)
        : LoopImprovement(versioner, prep), _conditionalNode(conditionalNode),
          _reverseBranch(reverseBranch), _original(original) {}
   ...
   */
  bool refineAliases() {
    return _refineLoopAliases && !comp()->generateArraylets();
  }
  ...
  virtual void
  buildAliasRefinementComparisonTrees(List<TR::TreeTop> *, List<TR::TreeTop> *,
                                      List<TR::TreeTop> *, List<TR::TreeTop> *,
                                      TR_ScratchList<TR::Node> *, TR::Block *);
  ...
  TR::Node *isDependentOnInductionVariable(TR::Node *, bool, bool &,
                                           TR::Node *&, TR::Node *&, bool &);
  TR::Node *isDependentOnInvariant(TR::Node *);
  bool boundCheckUsesUnchangedValue(TR::TreeTop *, TR::Node *,
                                    TR::SymbolReference *,
                                    TR_RegionStructure *);
  bool findStore(TR::TreeTop *start, TR::TreeTop *end, TR::Node *node,
                 TR::SymbolReference *symRef, bool ignoreLoads = false,
                 bool lastTimeThrough = false);
  TR::Node *findLoad(TR::Node *node, TR::SymbolReference *symRef,
                     vcount_t origVisitCount);
  void versionNaturalLoop(
      TR_RegionStructure *, List<TR::Node> *, List<TR::TreeTop> *,
      List<TR::TreeTop> *, List<TR::TreeTop> *, List<TR::TreeTop> *,
      List<TR::TreeTop> *, List<TR::TreeTop> *, List<TR::TreeTop> *,
      List<TR::TreeTop> *, List<TR::Node> *, List<TR_NodeParentSymRef> *,
  ...
  bool detectInvariantTrees(TR_RegionStructure *whileLoop, List<TR::TreeTop> *,
                            bool, bool *,
                            SharedSparseBitVector &reverseBranchInLoops);
  ...
  bool isExprInvariant(TR::Node *,
                       bool ignoreHeapificationStore =
                           false); // ignoreHeapificationStore flags for both
  bool isExprInvariantRecursive(
      TR::Node *,
      bool ignoreHeapificationStore = false); // methods need to be in sync!

  bool isDependentOnAllocation(TR::Node *, int32_t);
  bool hasWrtbarBeenSeen(List<TR::TreeTop> *, TR::Node *);

  bool checkProfiledGuardSuitability(TR_ScratchList<TR::Block> *loopBlocks,
                                     TR::Node *guardNode,
                                     TR::SymbolReference *callSymRef,
                                     TR::Compilation *comp);
  bool isBranchSuitableToVersion(TR_ScratchList<TR::Block> *loopBlocks,
                                 TR::Node *node, TR::Compilation *comp);
  bool isBranchSuitableToDoLoopTransfer(TR_ScratchList<TR::Block> *loopBlocks,
                                        TR::Node *node, TR::Compilation *comp);

  bool detectChecksToBeEliminated(TR_RegionStructure *, List<TR::Node> *,
                                  List<TR::TreeTop> *, List<int32_t> *,
                                  List<TR::TreeTop> *, List<TR::TreeTop> *,
                                  List<int32_t> *, List<TR::TreeTop> *,
                                  List<TR::TreeTop> *, List<TR::TreeTop> *,
                                  List<TR::TreeTop> *, List<TR::TreeTop> *,
                                  List<TR::Node> *, List<TR_NodeParentSymRef> *,
                                  List<TR_NodeParentSymRefWeightTuple> *,
                                  bool &);

  void buildNullCheckComparisonsTree(List<TR::Node> *, List<TR::TreeTop> *);
  void buildBoundCheckComparisonsTree(List<TR::TreeTop> *, List<TR::TreeTop> *,
                                      bool);
  void createRemoveBoundCheck(TR::TreeTop *, LoopEntryPrep *,
                              List<TR::TreeTop> *);
  ...
  void buildConditionalTree(List<TR::TreeTop> *,
                            SharedSparseBitVector &reverseBranchInLoops);
  void buildArrayStoreCheckComparisonsTree(List<TR::TreeTop> *);
  bool buildSpecializationTree(List<TR::TreeTop> *, List<TR::TreeTop> *,
                               List<TR::TreeTop> *, List<TR::TreeTop> *,
                               List<TR::Node> *, List<TR::Node> *, TR::Block *,
                               TR::SymbolReference **);
  bool buildLoopInvariantTree(List<TR_NodeParentSymRef> *);
  void convertSpecializedLongsToInts(TR::Node *, vcount_t,
                                     TR::SymbolReference **);
  void collectAllExpressionsToBeChecked(
      List<TR::TreeTop> *, List<TR::TreeTop> *, List<TR::TreeTop> *,
  ...
  bool depsForLoopEntryPrep(TR::Node *,
                            TR::list<LoopEntryPrep *, TR::Region &> *,
                            TR::NodeChecklist *, bool);
  LoopEntryPrep *addLoopEntryPrepDep(LoopEntryPrep::Kind, TR::Node *,
                                     TR::list<LoopEntryPrep *, TR::Region &> *,
                                     TR::NodeChecklist *);

  void copyOnWriteNode(TR::Node *original, TR::Node **current);

  void updateDefinitionsAndCollectProfiledExprs(
      TR::Node *, TR::Node *, vcount_t, List<TR::Node> *,
      List<TR_NodeParentSymRef> *, List<TR_NodeParentSymRefWeightTuple> *,
      TR::Node *, bool, TR::Block *, int32_t);
  void findAndReplaceContigArrayLen(TR::Node *, TR::Node *, vcount_t);
  void performLoopTransfer();

  bool replaceInductionVariable(TR::Node *, TR::Node *, int, int, TR::Node *,
                                int);

  TR::Block *createEmptyGoto(TR::Block *source, TR::Block *dest,
                             TR::TreeTop *endTree);
  TR::Block *createClonedHeader(TR::Block *origHeader, TR::TreeTop **endTree);
  TR::Node *createSwitchNode(TR::Block *clonedHeader,
                             TR::SymbolReference *tempSymRef, int32_t numCase);
  bool canPredictIters(TR_RegionStructure *whileLoop,
  ...
  const Expr *substitutePrivTemps(TR::TreeTop *tt, TR::Node *node,
  ...
  void unsafelyEmitAllTests(const TR::list<LoopEntryPrep *, TR::Region &> &,
                            List<TR::Node> *);
  ...
  LoopEntryPrep *createLoopEntryPrep(LoopEntryPrep::Kind, TR::Node *,
                                     TR::NodeChecklist * = NULL,
                                     LoopEntryPrep * = NULL);
  LoopEntryPrep *createChainedLoopEntryPrep(LoopEntryPrep::Kind, TR::Node *,
                                            LoopEntryPrep *);
