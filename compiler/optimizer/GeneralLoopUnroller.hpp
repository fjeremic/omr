...
public:
  ...
  static bool unroll(TR::Compilation *comp, TR_RegionStructure *loop,
                     int32_t unrollCount, int32_t peelCount,
                     TR::Optimizer *optimizer = NULL);
  static TR_StructureSubGraphNode *findNodeInHierarchy(TR_RegionStructure *,
                                                       int32_t);

protected:
  TR_LoopUnroller(TR::Compilation *comp, TR::Optimizer *optimizer,
  ...
  TR_LoopUnroller(TR::Compilation *comp, TR::Optimizer *optimizer,
                  TR_RegionStructure *loop, TR_PrimaryInductionVariable *piv,
                  UnrollKind unrollKind, int32_t unrollCount, int32_t peelCount,
                  TR::Block *invariantBlock, int32_t vectorSize = 1);

  TR::Compilation *comp() { return _comp; }
  TR::Optimizer *optimizer() { return _optimizer; }
  bool trace() {
    return comp()->trace(OMR::generalLoopUnroller) ||
           comp()->trace(OMR::SPMDKernelParallelization);
  }
  ...
  TR::DataType getTestChildType() {
    return getTestNode()->getFirstChild()->getType();
  }
  bool getTestIsUnsigned() { return getTestOpCode2().isUnsignedCompare(); }

  static bool isWellFormedLoop(TR_RegionStructure *loop, TR::Compilation *,
                               TR::Block *&loopInvariantBlock);
  static bool isTransactionStartLoop(TR_RegionStructure *loop,
                                     TR::Compilation *);
  ...
  void getLoopPreheaders(TR_RegionStructure *loop,
  ...
  int32_t unroll(TR_RegionStructure *loop,
                 TR_StructureSubGraphNode *branchNode);
  void unrollLoopOnce(TR_RegionStructure *loop,
                      TR_StructureSubGraphNode *branchNode, bool finalUnroll);
  bool shouldConnectToNextIteration(TR_StructureSubGraphNode *subNode,
                                    TR_RegionStructure *loop);
  ...
  bool cfgEdgeAlreadyExists(TR::Block *from, TR::Block *to,
                            EdgeContext edgeContext = InvalidContext);
  ...
  void addExitEdgeAndFixEverything(TR_RegionStructure *region,
                                   ...
                                   EdgeContext context = InvalidContext);
  void addEdgeAndFixEverything(TR_RegionStructure *region,
                               TR::CFGEdge *originalEdge,
  ...
  TR::CFGEdge *findEdge(TR_StructureSubGraphNode *from,
                        TR_StructureSubGraphNode *to);

  TR::Node *createIfNodeForSpillLoop(TR::Node *ifNode);

  void
  swingBlocks(TR::Block *from,
              TR::Block *to); // Adds an edge to the queue of to-be-swung blocks
...
private:
  void collectNonColdInnerLoops(TR_RegionStructure *region,
                                List<TR_RegionStructure> &innerLoops);

  // candidates for being made static
  void gatherStatistics(TR_Structure *str, int32_t &numNodes,
                        int32_t &numBlocks, int32_t &numBranches,
  ...
  bool canUnrollUnCountedLoop(TR_RegionStructure *loop, int32_t numBlocks,
                              int32_t numNodes, int32_t entryBlockFrequency);

  bool branchContainsInductionVariable(TR_RegionStructure *loop,
                                       TR::Node *branchNode,
                                       bool checkOnlyPiv = true);
  bool branchContainsInductionVariable(TR::Node *branchNode,
                                       TR::SymbolReference *ivSymRef);

  void countNodesAndSubscripts(TR::Node *node, int32_t &numNodes,
                               int32_t &numSubscripts, LoopWeightProbe &lwp);
  ...
  struct UnrollInfo {
    ...
    UnrollInfo(TR_RegionStructure *loop, int32_t w, int32_t c,
               TR_LoopUnroller::UnrollKind unrollKind, int32_t unrollCount,
               int32_t peelCount)
