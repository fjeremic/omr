...
public:
  ...
  //
  static bool unroll(TR::Compilation* comp, TR_RegionStructure* loop, TR_PrimaryInductionVariable* piv, UnrollKind kind,
...
protected:
  ...
  bool trace()
  {
    return comp()->trace(OMR::generalLoopUnroller) || comp()->trace(OMR::SPMDKernelParallelization);
  }
  TR_Debug* getDebug()
  {
  ...
  TR::ILOpCode getTestOpCode2()
  {
    return getTestNode()->getOpCode();
  }
  TR::DataType getTestChildType()
  {
    return getTestNode()->getFirstChild()->getType();
  }
  bool getTestIsUnsigned()
  {
    return getTestOpCode2().isUnsignedCompare();
  }
  ...
  void addEdgeForSpillLoop(TR_RegionStructure* region, TR::CFGEdge* originalEdge, TR_StructureSubGraphNode* newFromNode,
    TR_StructureSubGraphNode* newToNode, bool removeOriginal = false, EdgeContext context = InvalidContext,
    bool notLoopBranchNode = false);
  void addExitEdgeAndFixEverything(TR_RegionStructure* region, TR::CFGEdge* originalEdge,
    TR_StructureSubGraphNode* newFromNode, TR_StructureSubGraphNode* exitNode = NULL, TR::Block* newExitBlock = NULL,
...
private:
  ...
  int32_t weighNaturalLoop(
    TR_RegionStructure* loop, TR_LoopUnroller::UnrollKind&, int32_t& unrollCount, int32_t& peelCount, int32_t& cost);
