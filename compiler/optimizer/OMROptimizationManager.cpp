...
OMR::OptimizationManager::OptimizationManager(
    TR::Optimizer *o, OptimizationFactory factory, OMR::Optimizations optNum,
    const OptimizationStrategy *groupOfOpts)
    : _optimizer(o), _factory(factory), _id(optNum), _groupOfOpts(groupOfOpts),
      _numPassesCompleted(0), _optData(NULL), _optPolicy(NULL),
      _enabled(!o->comp()->isDisabled(optNum)), _requested(false),
      _requestedBlocks(o->trMemory()),
      _trace(o->comp()->getOptions()->trace(optNum)) {
  if (_id < OMR::Optimizations::numGroups)
    TR_ASSERT_SAFE_FATAL(_id < OMR::Optimizations::numGroups,
                         "The optimization id requested (%d) is too high", _id);

  // set flags if necessary
  switch (self()->id()) {
  case OMR::inlining:
    _flags.set(doesNotRequireAliasSets | canAddSymbolReference | verifyTrees |
               verifyBlocks | checkTheCFG | requiresAccurateNodeCount);
    break;
  ...
  case OMR::globalCopyPropagation:
    _flags.set(requiresStructure | requiresLocalsUseDefInfo |
               doesNotRequireLoadsAsDefs);
    break;
  ...
  case OMR::osrDefAnalysis:
    if (self()->comp()->getOption(TR_DisableOSRSharedSlots))
      _flags.set(doesNotRequireAliasSets | doesNotRequireTreeDumps |
                 supportsIlGenOptLevel);
    else
      _flags.set(doesNotRequireAliasSets |
                 /* requiresStructure | */ supportsIlGenOptLevel);
    break;
    ...
    if (block) {
      ...
      if (!_requestedBlocks.find(block)) {
        ...
        if (self()->id() == loopVersionerGroup)
          self()->optimizer()->setRequestOptimization(lastLoopVersionerGroup,
                                                      requested, block);
      }
    } else {
      ...
      // was done before
      if (!_requestedBlocks.find(
              toBlock(self()->comp()->getFlowGraph()->getStart())))
        ...
        // to be handled efficiently as we won't iterate through blocks each
        // time
        _requestedBlocks.add(
            toBlock(self()->comp()->getFlowGraph()->getStart()));
      if (self()->id() == loopVersionerGroup)
        self()->optimizer()->setRequestOptimization(
            lastLoopVersionerGroup, requested,
            toBlock(self()->comp()->getFlowGraph()->getStart()));
    }
...
void OMR::OptimizationManager::performChecks() {
  ...
  if (self()->getVerifyTrees() ||
      self()->comp()->getOption(TR_EnableParanoidOptCheck) ||
