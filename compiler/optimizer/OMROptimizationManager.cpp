  ...
  , _trace(o->comp()->getOptions()->trace(optNum))
{
  if (_id < OMR::Optimizations::numGroups)
    TR_ASSERT_SAFE_FATAL(_id < OMR::Optimizations::numGroups,
                         "The optimization id requested (%d) is too high",
                         _id);
...
bool
OMR::OptimizationManager::requested(TR::Block* block)
{
  if (block == NULL)
    return _requested;
...
void
OMR::OptimizationManager::setRequested(bool requested, TR::Block* block)
{
      ...
      if (!_requestedBlocks.find(block)) {
        ...
        if (self()->id() == loopVersionerGroup)
          self()->optimizer()->setRequestOptimization(
            lastLoopVersionerGroup, requested, block);
      }
    } else {
