...
bool TR_BasicDFSetAnalysis<Container *>::supportsGenAndKillSets() {
  return false;
}

template <class Container>
void TR_BasicDFSetAnalysis<Container *>::initializeGenAndKillSetInfo() {}

template <class Container>
TR_DataFlowAnalysis::Kind TR_BasicDFSetAnalysis<Container *>::getKind() {
  return BasicDFSetAnalysis;
}

// Perform the data flow analysis, including initialization
template <class Container>
bool TR_BasicDFSetAnalysis<Container *>::performAnalysis(
    TR_Structure *rootStructure, bool checkForChanges) {
  ...
  if (!postInitializationProcessing()) return false;
  doAnalysis(rootStructure, checkForChanges);
  return true;
}

template <class Container>
void TR_BasicDFSetAnalysis<Container *>::allocateContainer(Container **result,
                                                           bool, bool) {
  *result =
      new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}

template <class Container>
void TR_BasicDFSetAnalysis<Container *>::allocateBlockInfoContainer(
    Container **result, bool, bool) {
  *result =
      new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}
...
template <class Container>
void TR_BasicDFSetAnalysis<Container *>::allocateTempContainer(
    Container **result, Container *) {
  *result =
      new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
}
...
template <class Container>
void TR_BasicDFSetAnalysis<Container *>::initializeBlockInfo(
    bool allocateLater) {
  if (_blockAnalysisInfo) return;
  ...
  _blockAnalysisInfo = (Container **)trMemory()->allocateStackMemory(
      _numberOfNodes * sizeof(Container *));
...
template <class Container>
void TR_BasicDFSetAnalysis<Container *>::initializeBasicDFSetAnalysis() {
  if (_blockAnalysisInfo == NULL) initializeBlockInfo();

  _hasImproperRegion =
      _cfg->getStructure()->markStructuresWithImproperRegions();

  if (comp()->getMethodSymbol()->mayHaveNestedLoops() &&
      !comp()->getOption(TR_DisableNewBVA)) {
    _hasImproperRegion = false;  // Probably use a run time option here to
                                 // enable old flow analysis behav
  } else
  ...
  if (comp()->getVisitCount() > HIGH_VISIT_COUNT) {
    ...
    dumpOptDetails(comp(),
                   "\nResetting visit counts for this method before bit vector "
                   "analysis\n");
  }
  ...
  _nodesInCycle = new (trMemory()->currentStackRegion())
      TR_BitVector(trMemory()->currentStackRegion());

  if (supportsGenAndKillSets()) {
    int32_t arraySize = _numberOfNodes * sizeof(Container *);
    _regularGenSetInfo =
        (Container **)trMemory()->allocateStackMemory(arraySize);
    memset(_regularGenSetInfo, 0, arraySize);
    _regularKillSetInfo =
        (Container **)trMemory()->allocateStackMemory(arraySize);
    memset(_regularKillSetInfo, 0, arraySize);
    _exceptionGenSetInfo =
        (Container **)trMemory()->allocateStackMemory(arraySize);
    ...
    if (!_hasImproperRegion) {
      ...
      if (traceBVA())
        dumpOptDetails(comp(),
                       "\n ************** Completed initialization of gen and "
                       "kill sets for all structures ************* \n");
    }
  } else {
...
template <class Container>
void TR_BasicDFSetAnalysis<
    Container *>::initializeGenAndKillSetInfoForStructures() {
...
template <class Container>
void TR_BasicDFSetAnalysis<
    Container *>::initializeGenAndKillSetInfoForStructure(TR_Structure *s) {
  ...
  if (region) {
    if (region->containsImproperRegion() ||
      ...
      typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
          *analysisInfo = getAnalysisInfo(s);
    } else {
...
template <class Container>
void TR_BasicDFSetAnalysis<Container *>::
    initializeGenAndKillSetInfoPropertyForStructure(TR_Structure *s,
                                                    bool inLoop) {
  bool b = canGenAndKillForStructure(s);
  TR_RegionStructure *region = s->asRegion();
  if (region) {
    if (region->isNaturalLoop()) inLoop = true;
  }

  if (!inLoop) s->setContainsImproperRegion(true);

  if (region) {
    TR_RegionStructure::Cursor si(*region);
    TR_StructureSubGraphNode *subNode;
    for (subNode = si.getCurrent(); subNode; subNode = si.getNext()) {
      initializeGenAndKillSetInfoPropertyForStructure(subNode->getStructure(),
                                                      inLoop);
    }
  }
}
...
template <class Container>
bool TR_ForwardDFSetAnalysis<Container *>::canGenAndKillForStructure(
    TR_Structure *nodeStructure) {
  if (!this->supportsGenAndKillSetsForStructures()) return false;
  ...
  if (naturalLoop && naturalLoop->isNaturalLoop()) {
    ...
    for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL;
      ...
      if (seenExitNodes.get(toNode->getNumber())) continue;
      ...
      for (auto pred = predecessors.getFirst(); pred;
           pred = predecessors.getNext()) {
...
template <class Container>
void TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForRegion(
  ...
  typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *analysisInfo =
      this->getAnalysisInfo(region);
  if (!region->hasBeenAnalyzedBefore())
    region->setAnalyzedStatus(true);
  else
    return;

  analysisInfo->_regularGenSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  analysisInfo->_regularKillSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  analysisInfo->_exceptionGenSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  analysisInfo->_exceptionKillSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  analysisInfo
      ->_currentRegularGenSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  analysisInfo
      ->_currentRegularKillSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  analysisInfo
      ->_currentExceptionGenSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  analysisInfo
      ->_currentExceptionKillSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
  ...
  for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
    ...
    if (!exitNodes.get(toStructureNumber)) {
      ...
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
          *pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
              Container *>::TR_ContainerNodeNumberPair(b, toStructureNumber);
  ...
  if (region->isNaturalLoop()) {
    typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
        *analysisInfo = this->getAnalysisInfo(region);
    Container *killBitVector = analysisInfo->getContainer(
        analysisInfo->_regularKillSetInfo, region->getNumber());
    Container *genBitVector = analysisInfo->getContainer(
        analysisInfo->_regularGenSetInfo, region->getNumber());
    Container *exceptionKillBitVector = analysisInfo->getContainer(
        analysisInfo->_exceptionKillSetInfo, region->getNumber());
    Container *exceptionGenBitVector = analysisInfo->getContainer(
        analysisInfo->_exceptionGenSetInfo, region->getNumber());
    typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
        *pair;
    if (analysisInfo->_regularGenSetInfo) {
      for (pair = analysisInfo->_regularGenSetInfo->getFirst(); pair;
           pair = pair->getNext()) {
        if (pair->_container && (pair->_nodeNumber != region->getNumber())) {
          this->_temp->empty();
          if (genBitVector) *this->_temp |= *genBitVector;

          if (exceptionGenBitVector) *this->_temp |= *exceptionGenBitVector;

          Container *killForThisExit = analysisInfo->getContainer(
              analysisInfo->_regularKillSetInfo, pair->_nodeNumber);
          if (killForThisExit) *this->_temp -= *killForThisExit;

          Container *genForThisExit = analysisInfo->getContainer(
              analysisInfo->_regularGenSetInfo, pair->_nodeNumber);
          if (genForThisExit) *this->_temp |= *genForThisExit;
    ...
    if (analysisInfo->_regularKillSetInfo) {
      for (pair = analysisInfo->_regularKillSetInfo->getFirst(); pair;
           pair = pair->getNext()) {
        if (pair->_container && (pair->_nodeNumber != region->getNumber())) {
          this->_temp->empty();
          if (killBitVector) *this->_temp |= *killBitVector;
          if (exceptionKillBitVector) *this->_temp |= *exceptionKillBitVector;
    ...
    if (analysisInfo->_exceptionGenSetInfo) {
      for (pair = analysisInfo->_exceptionGenSetInfo->getFirst(); pair;
           pair = pair->getNext()) {
        if (pair->_container && (pair->_nodeNumber != region->getNumber())) {
          this->_temp->empty();
          if (genBitVector) *this->_temp |= *genBitVector;
          if (exceptionGenBitVector) *this->_temp |= *exceptionGenBitVector;
          Container *killForThisExit = analysisInfo->getContainer(
              analysisInfo->_exceptionKillSetInfo, pair->_nodeNumber);
          if (killForThisExit) *this->_temp -= *killForThisExit;
          Container *genForThisExit = analysisInfo->getContainer(
              analysisInfo->_exceptionGenSetInfo, pair->_nodeNumber);
          if (genForThisExit) *this->_temp |= *genForThisExit;
          compose((pair->_container), this->_temp);
        }
      }
    ...
    if (analysisInfo->_exceptionKillSetInfo) {
      for (pair = analysisInfo->_exceptionKillSetInfo->getFirst(); pair;
           pair = pair->getNext()) {
        if (pair->_container && (pair->_nodeNumber != region->getNumber())) {
          this->_temp->empty();
          if (killBitVector) *this->_temp |= *killBitVector;
...
void TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfo(
  ...
  while (this->_analysisQueue.getListHead() &&
         (this->_analysisQueue.getListHead()->getData()->getStructure() !=
          regionStructure)) {
    if (!this->_analysisInterrupted) {
      numIterations++;
      if ((numIterations % 20) == 0) {
        numIterations = 0;
        if (this->comp()->compilationShouldBeInterrupted(
    ...
    if (this->traceBVA())
      traceMsg(this->comp(),
               "Gen : Begin analyzing node %p numbered %d in region %p (%d)\n",
               nodeStructure->getStructure(), node->getNumber(),
    ...
    if (node != regionStructure->getEntry()) initializeCurrentGenKillSetInfo();
    ...
    if (node != regionStructure->getEntry()) {
      ...
      for (auto pred = predecessors.getFirst(); pred;
            ...
            !predStructure->hasBeenAnalyzedBefore())  //_firstIteration)
        {
          ...
          typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
              *predInfo = this->getAnalysisInfo(predStructure);
          int32_t nodeNumber = predStructure->asRegion()
                                   ? node->getNumber()
                                   : predNode->getNumber();
          TR_LinkHead<typename TR_BasicDFSetAnalysis<
              Container *>::TR_ContainerNodeNumberPair>
              *predCurrentGenSetInfo =
                  normalPred ? predInfo->_currentRegularGenSetInfo
                             : predInfo->_currentExceptionGenSetInfo,
              *predCurrentKillSetInfo =
                  normalPred ? predInfo->_currentRegularKillSetInfo
                             : predInfo->_currentExceptionKillSetInfo;

          predBitVector =
              predInfo->getContainer(predCurrentGenSetInfo, nodeNumber);
          if (predBitVector) compose(_currentRegularGenSetInfo, predBitVector);

          predBitVector =
              predInfo->getContainer(predCurrentKillSetInfo, nodeNumber);
          if (predBitVector)
        ...
        if (this->traceBVA()) {
          dumpOptDetails(this->comp(), "Node %p (%d) pred %p (%d)\n",
                         ...
                         predStructure, predStructure->getNumber());
    ...
    *temp2 = *_currentRegularKillSetInfo;
    ...
    this->_nodesInCycle->empty();
    this->initializeGenAndKillSetInfoForStructure(
        nodeStructure->getStructure());
    ...
    if (this->traceBVA()) {
      ...
      if (nodeInfo->_regularGenSetInfo) {
        traceMsg(this->comp(),
                 ...
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
        for (pair = nodeInfo->_regularGenSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          if (pair->_container) {
            traceMsg(this->comp(),
                     "Exit or Succ numbered %d : ", pair->_nodeNumber);
      ...
      if (nodeInfo->_regularKillSetInfo) {
        traceMsg(this->comp(),
                 ...
                 nodeStructure->getStructure(),
                 nodeStructure->getStructure()->getNumber());
        for (pair = nodeInfo->_regularKillSetInfo->getFirst(); pair;
             pair = pair->getNext()) {
          if (pair->_container) {
            traceMsg(this->comp(),
                     "Exit or Succ numbered %d : ", pair->_nodeNumber);
    ...
    *this->_temp2 = *_currentRegularKillSetInfo;
    ...
    for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
      ...
      *_currentRegularKillSetInfo = *this->_temp2;
      TR_LinkHead<typename TR_BasicDFSetAnalysis<
          Container *>::TR_ContainerNodeNumberPair>
          *_nodeGenSetInfo = normalSucc ? nodeInfo->_regularGenSetInfo
                                        : nodeInfo->_exceptionGenSetInfo,
          *_nodeKillSetInfo = normalSucc ? nodeInfo->_regularKillSetInfo
                                         : nodeInfo->_exceptionKillSetInfo;
      Container *genBitVector = NULL, *killBitVector = NULL;
      int32_t nodeNumber = nodeStructure->getStructure()->asRegion()
                               ? succNode->getNumber()
                               : node->getNumber();
      ...
      if (killBitVector) *_currentRegularGenSetInfo -= *killBitVector;
      if (genBitVector) *_currentRegularGenSetInfo |= *genBitVector;
      if (genBitVector) *_currentRegularKillSetInfo -= *genBitVector;
      if (killBitVector) *_currentRegularKillSetInfo |= *killBitVector;

      if (regionStructure->isExitEdge(succ) ||
          (succNode == regionStructure->getEntry())) {
        TR_LinkHead<typename TR_BasicDFSetAnalysis<
            Container *>::TR_ContainerNodeNumberPair>
            *_genSetInfo = normalSucc ? analysisInfo->_regularGenSetInfo
                                      : analysisInfo->_exceptionGenSetInfo,
            *_killSetInfo = normalSucc ? analysisInfo->_regularKillSetInfo
                                       : analysisInfo->_exceptionKillSetInfo;
        typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
            *pair = analysisInfo->getContainerNodeNumberPair(
                _genSetInfo, succNode->getNumber());
        if (!pair) {
          pair = _genSetInfo->getFirst();
          TR_ASSERT(!pair->getNext(),
                    "There should be only one successor info\n");
          TR_ASSERT((pair->_nodeNumber == node->getNumber()),
                    "Node number inconsistent\n");
        }
        ...
        pair = analysisInfo->getContainerNodeNumberPair(_killSetInfo,
                                                        succNode->getNumber());
        if (!pair) {
        ...
        TR_LinkHead<typename TR_BasicDFSetAnalysis<
            Container *>::TR_ContainerNodeNumberPair>
            *_currentGenSetInfo =
                normalSucc ? nodeInfo->_currentRegularGenSetInfo
                           : nodeInfo->_currentExceptionGenSetInfo,
            *_currentKillSetInfo = normalSucc
                                       ? nodeInfo->_currentRegularKillSetInfo
                                       : nodeInfo->_currentExceptionKillSetInfo;
        if (!_currentGenSetInfo)
          TR_ASSERT(0,
                    "Gen set info should have been allocated in initialization "
                    "step\n");
        typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
            *pair = NULL;
        pair = nodeInfo->getContainerNodeNumberPair(_currentGenSetInfo,
                                                    nodeNumber);
        ...
        compose(pair->_container, _currentRegularGenSetInfo);
        pair = nodeInfo->getContainerNodeNumberPair(_currentKillSetInfo,
                                                    nodeNumber);
...
template <class Container>
void TR_ForwardDFSetAnalysis<Container *>::initializeGenAndKillSetInfoForBlock(
    TR_BlockStructure *s) {
  typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *analysisInfo =
      this->getAnalysisInfo(s);
  ...
  typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
      *pair = new (this->trStackMemory())
          typename TR_BasicDFSetAnalysis<Container *>::
...
template <class Container>
typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *
TR_BasicDFSetAnalysis<Container *>::getAnalysisInfo(TR_Structure *s) {
  typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *analysisInfo =
      (typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *)
          s->getAnalysisInfo();
  if (!s->hasBeenAnalyzedBefore()) {
...
template <class Container>
typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *
TR_BasicDFSetAnalysis<Container *>::createAnalysisInfo() {
  typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo *analysisInfo =
      new (this->trStackMemory())
          typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo;
  analysisInfo->_inSetInfo = initializeInfo(NULL);
  analysisInfo->_outSetInfo = new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair>;
...
void TR_BasicDFSetAnalysis<Container *>::initializeAnalysisInfo(
  ...
  for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
    ...
    typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair
        *pair = new (this->trStackMemory()) typename TR_BasicDFSetAnalysis<
            Container *>::TR_ContainerNodeNumberPair(b, succBlock->getNumber());
...
template <class Container>
void TR_ForwardDFSetAnalysis<Container *>::initializeDFSetAnalysis() {
...
bool TR_ForwardDFSetAnalysis<Container *>::analyzeRegionStructure(
  ...
  while (changed) {
...

    {
      ...
      if (this->analyzeNodeIfPredecessorsAnalyzed(regionStructure,
                                                  pendingList)) {
        if (!this->supportsGenAndKillSets() ||
  ...
  if (regionStructure != this->_cfg->getStructure()) {
    ...
    for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL;
      ...
      int32_t toStructureNumber = edge->getTo()->getNumber();
      Container *toBitVector = analysisInfo->getContainer(
          analysisInfo->_outSetInfo, toStructureNumber);

      typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
          *fromAnalysisInfo = this->getAnalysisInfo(edgeFrom->getStructure());
      Container *fromBitVector = fromAnalysisInfo->getContainer(
          fromAnalysisInfo->_outSetInfo, toStructureNumber);
        ...
        if (this->getKind() == TR_DataFlowAnalysis::ReachingDefinitions) {
          ...
          *this->_temp -= *toBitVector;
          if (!this->_temp->isEmpty()) {
            ...
            // not all containers have getFirstElement, so we no longer print
            // it.
...
bool TR_ForwardDFSetAnalysis<Container *>::analyzeNodeIfPredecessorsAnalyzed(
  ...
  while (this->_analysisQueue.getListHead() &&
    ...
    typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
        *analysisInfo = this->getAnalysisInfo(nodeStructure->getStructure());
    ...
    for (auto pred = predecessors.getFirst(); pred;
      ...
      {
        if (firstPred) initializeInfo(_currentInSetInfo);
      } else {
        typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
            *predInfo = this->getAnalysisInfo(predStructure);
        Container *predBitVector =
            predInfo->getContainer(predInfo->_outSetInfo, node->getNumber());
    ...
    if (node == regionStructure->getEntry()) {
      if (regionStructure != this->_cfg->getStructure())
        compose(_currentInSetInfo,
                this->getAnalysisInfo(regionStructure)->_inSetInfo);
      else
        compose(_currentInSetInfo, _originalInSetInfo);
    }

    typename TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo
        *regionAnalysisInfo = this->getAnalysisInfo(regionStructure);
    ...
    if (this->supportsGenAndKillSets() &&
      ...
      if (nodeStructure->getStructure()->asRegion() &&
        ...
        *this->_temp = *_currentInSetInfo;
        ...
        bitVector = analysisInfo->getContainer(
            analysisInfo->_regularKillSetInfo, node->getNumber());
        if (bitVector)
          *this->_temp -= *bitVector;
        else {
          bitVector = analysisInfo->getContainer(
              analysisInfo->_exceptionKillSetInfo, node->getNumber());
          if (bitVector) *this->_temp -= *bitVector;
        }

        bitVector = analysisInfo->getContainer(analysisInfo->_regularGenSetInfo,
                                               node->getNumber());
        if (bitVector)
          *this->_temp |= *bitVector;
        else {
          bitVector = analysisInfo->getContainer(
              analysisInfo->_exceptionGenSetInfo, node->getNumber());
          if (bitVector) *this->_temp |= *bitVector;
        }
    ...
    if (this->supportsGenAndKillSets() &&
      ...
      TR_SuccessorIterator successors(node);
      int count = 0;
      for (auto succ = successors.getFirst(); succ;
           succ = successors.getNext()) {
        bool normalSucc = (++count <= node->getSuccessors().size());
        Container *_info =
            normalSucc ? this->_regularInfo : this->_exceptionInfo;
        TR_LinkHead<typename TR_BasicDFSetAnalysis<
            Container *>::TR_ContainerNodeNumberPair>
            *_killSetInfo = normalSucc ? analysisInfo->_regularKillSetInfo
                                       : analysisInfo->_exceptionKillSetInfo,
        ...
        int32_t nodeNumber = nodeStructure->getStructure()->asRegion()
                                 ...
                                 : node->getNumber();
        Container *bitVector =
            analysisInfo->getContainer(_killSetInfo, nodeNumber);
        if (bitVector) {
          *_info -= *bitVector;
          if (this->traceBVA()) {
            traceMsg(this->comp(),
                     "\n%sKill Set Info for Region or Block : %p numbered %d "
                     "and exit %d is : \n",
            ...
            traceMsg(this->comp(),
                     "\n%s Info for Region or Block : %p numbered %d and exit "
                     "%d is : \n",
        ...
        if (bitVector) {
          *_info |= *bitVector;
          if (this->traceBVA()) {
            traceMsg(this->comp(),
                     "\n%sGen Set Info for Region or Block : %p numbered %d "
                     "and exit %d is : \n",
                     (normalSucc ? "" : "E"), nodeStructure->getStructure(),
                     nodeStructure->getStructure()->getNumber(),
                     succNode->getNumber());
            bitVector->print(this->comp());
            traceMsg(this->comp(),
                     "\n1%s Info for Region or Block : %p numbered %d and exit "
                     "%d is : \n",
                     (normalSucc ? "Normal" : "Exception"),
                     nodeStructure->getStructure(),
                     nodeStructure->getStructure()->getNumber(),
        ...
        bitVector = analysisInfo->getContainer(analysisInfo->_outSetInfo,
                                               succNode->getNumber());
        // Don't copy vectors if they are the same since comparison is much
        // cheaper and can save compile time
        if (*_info != *bitVector) {
          this->copyFromInto(_info, bitVector);
          if (checkForChange && !outSetChanged) outSetChanged = true;
        }
      }
    }
    ...
    if (b || pendingList.get(node->getNumber())) outSetChanged = true;
    pendingList.reset(nodeStructure->getStructure()->getNumber());
    this->removeHeadFromAnalysisQueue();
    ...
    for (auto succ = node->getSuccessors().begin();
         succ != node->getSuccessors().end(); ++succ) {
      TR::CFGNode *succNode = (*succ)->getTo();
      if ((!regionStructure->isExitEdge(*succ)) &&
          (needToIterate || (succNode != regionStructure->getEntry())) &&
          (pendingList.get(succNode->getNumber()) || outSetChanged)) {
        this->_nodesInCycle->empty();
        this->addToAnalysisQueue(toStructureSubGraphNode(succNode),
                                 outSetChanged ? 1 : 0);
      }
    }

    for (auto succ = node->getExceptionSuccessors().begin();
         succ != node->getExceptionSuccessors().end(); ++succ) {
      TR::CFGNode *succNode = (*succ)->getTo();
      if ((!regionStructure->isExitEdge(*succ)) &&
          (needToIterate || (succNode != regionStructure->getEntry())) &&
          (pendingList.get(succNode->getNumber()) || outSetChanged)) {
...
template <class Container>
bool TR_ForwardDFSetAnalysis<Container *>::analyzeBlockStructure(
  ...
  if (this->supportsGenAndKillSets() &&
    ...
    if (!this->_blockAnalysisInfo[blockStructure->getNumber()])
      this->allocateBlockInfoContainer(
          &this->_blockAnalysisInfo[blockStructure->getNumber()],
          _currentInSetInfo);
    this->copyFromInto(_currentInSetInfo,
                       this->_blockAnalysisInfo[blockStructure->getNumber()]);
  ...
  if (blockStructure->getNumber() == 0) {
    ...
    if (this->_regularGenSetInfo) {
      ...
      this->copyFromInto(analysisInfo->_inSetInfo,
                         this->_blockAnalysisInfo[blockStructure->getNumber()]);
    } else {
  ...
  for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
    ...
    Container *outSetInfo = analysisInfo->getContainer(
        analysisInfo->_outSetInfo, succ->getTo()->getNumber());
    Container *_info = normalSucc ? this->_regularInfo : this->_exceptionInfo;

    if (checkForChange && !changed && !(*_info == *outSetInfo)) changed = true;
...
template <class Container>
void TR_ForwardDFSetAnalysis<Container *>::analyzeBlockZeroStructure(
...
void TR_ForwardDFSetAnalysis<Container *>::analyzeNode(
    TR::Node *node, vcount_t visitCount, TR_BlockStructure *blockStructure,
    Container *analysisInfo) {}

template <class Container>
void TR_ForwardDFSetAnalysis<Container *>::analyzeTreeTopsInBlockStructure(
    TR_BlockStructure *blockStructure) {
  TR::Block *block = blockStructure->getBlock();
  ...
  while (!(currentTree == exitTree)) {
    ...
    analyzeNode(currentTree->getNode(), visitCount, blockStructure,
                this->_regularInfo);
    if (currentTreeHasChecks) compose(this->_exceptionInfo, this->_regularInfo);
    if (!(currentTree == exitTree)) currentTree = currentTree->getNextTreeTop();
  }
}

template <class Container>
void TR_ForwardDFSetAnalysis<Container *>::compose(Container *, Container *) {}

template <class Container>
void TR_ForwardDFSetAnalysis<Container *>::inverseCompose(Container *,
                                                          Container *) {}

template <class Container>
TR_DataFlowAnalysis::Kind TR_ForwardDFSetAnalysis<Container *>::getKind() {
...
Container *TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo::getContainer(
  ...
  for (pair = list->getFirst(); pair; pair = pair->getNext()) {
    if (pair->_nodeNumber == n) return pair->_container;
  }
...
template <class Container>
typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair *
TR_BasicDFSetAnalysis<Container *>::ExtraAnalysisInfo::
    getContainerNodeNumberPair(
        TR_LinkHead<typename TR_BasicDFSetAnalysis<
            Container *>::TR_ContainerNodeNumberPair> *list,
        int32_t n) {
  typename TR_BasicDFSetAnalysis<Container *>::TR_ContainerNodeNumberPair *pair;
  for (pair = list->getFirst(); pair; pair = pair->getNext()) {
    if (pair->_nodeNumber == n) return pair;
  }
