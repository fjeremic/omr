...
bool TR_BasicDFSetAnalysis<Container*>::
...
template <class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeBlockInfo(bool allocateLater)
{
...
template <class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForStructures()
{
...
template <class Container>
void TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(TR_RegionStructure* regionStructure, TR_BitVector& pendingList)
{
    while (this->_analysisQueue.getListHead() && (this->_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        if (!pendingList.get(nodeStructure->getStructure()->getNumber()) && (*(this->_changedSetsQueue.getListHead()->getData()) == 0)) {
        ...
        for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
            ...
            *_currentRegularKillSetInfo = *this->_temp2;
            ...
            if (regionStructure->isExitEdge(succ) || (succNode == regionStructure->getEntry())) {
                ...
                typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair = analysisInfo->getContainerNodeNumberPair(_genSetInfo, succNode->getNumber());
                if (!pair) {
...
template <class Container>
typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* TR_BasicDFSetAnalysis<Container*>::getAnalysisInfo(TR_Structure* s)
{
...
template <class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeAnalysisInfo(typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info, TR_Structure* s)
{
    TR_RegionStructure* region = s->asRegion();
    if (region)
...
template <class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeAnalysisInfo(typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info, TR::Block* block)
{
...
template <class Container>
void TR_BasicDFSetAnalysis<Container*>::initializeAnalysisInfo(typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info, TR_RegionStructure* region)
{
...
template <class Container>
void TR_BasicDFSetAnalysis<Container*>::clearAnalysisInfo(typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info)
{
...
template <class Container>
bool TR_ForwardDFSetAnalysis<Container*>::analyzeRegionStructure(TR_RegionStructure* regionStructure, bool checkForChange)
{
...
template <class Container>
bool TR_ForwardDFSetAnalysis<Container*>::analyzeNodeIfPredecessorsAnalyzed(TR_RegionStructure* regionStructure, TR_BitVector& pendingList)
{
    ...
    while (this->_analysisQueue.getListHead() && (this->_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
            if (nodeStructure->getStructure()->asRegion() && nodeStructure->getStructure()->asRegion()->isNaturalLoop()) {
                Container* bitVector;
        ...
        for (auto succ = node->getSuccessors().begin(); succ != node->getSuccessors().end(); ++succ) {
            TR::CFGNode* succNode = (*succ)->getTo();
            if ((!regionStructure->isExitEdge(*succ)) && (needToIterate || (succNode != regionStructure->getEntry())) && (pendingList.get(succNode->getNumber()) || outSetChanged)) {
                this->_nodesInCycle->empty();
...
template <class Container>
void TR_ForwardDFSetAnalysis<Container*>::analyzeNode(TR::Node* node, vcount_t visitCount, TR_BlockStructure* blockStructure, Container* analysisInfo)
{
}
...
template <class Container>
void TR_ForwardDFSetAnalysis<Container*>::inverseCompose(Container*, Container*)
{
}
...
template <class Container>
typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo::getContainerNodeNumberPair(TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>* list, int32_t n)
{
    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair;
    for (pair = list->getFirst(); pair; pair = pair->getNext()) {
        if (pair->_nodeNumber == n)
