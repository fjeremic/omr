...
template <class Container>
void TR_BasicDFSetAnalysis<Container*>::allocateContainer(Container** result, bool, bool)
{
...
void TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForRegion(TR_RegionStructure* region)
{
  ...
  analysisInfo->_regularGenSetInfo
    = new (this->trStackMemory()) TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
    ...
    if (node != regionStructure->getEntry()) {
      ...
      for (auto pred = predecessors.getFirst(); pred; pred = predecessors.getNext()) {
        ...
        if (pendingList.get(predStructure->getNumber()) && !predStructure->hasBeenAnalyzedBefore()) //_firstIteration)
        {
          ...
          TR_LinkHead<typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>*predCurrentGenSetInfo
          ...
          *_info -= *bitVector;
          if (this->traceBVA()) {
            traceMsg(this->comp(), "\n%sKill Set Info for Region or Block : %p numbered %d and exit %d is : \n",
              (normalSucc ? "" : "E"), nodeStructure->getStructure(), nodeStructure->getStructure()->getNumber(),
              succNode->getNumber());
