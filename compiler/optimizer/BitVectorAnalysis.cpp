...
bool
TR_BasicDFSetAnalysis<Container*>::supportsGenAndKillSets()
{
  return false;
}
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::initializeGenAndKillSetInfo()
{}

template<class Container>
TR_DataFlowAnalysis::Kind
TR_BasicDFSetAnalysis<Container*>::getKind()
{
...
template<class Container>
bool
TR_BasicDFSetAnalysis<Container*>::performAnalysis(TR_Structure* rootStructure,
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::allocateContainer(Container** result,
                                                     bool,
                                                     bool)
{
  *result =
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::allocateBlockInfoContainer(
  Container** result,
  bool,
  bool)
{
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::allocateTempContainer(Container** result,
                                                         Container*)
{
  *result =
    new (trStackMemory()) Container(_numberOfBits, trMemory(), stackAlloc);
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::initializeBlockInfo(bool allocateLater)
{
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::initializeBasicDFSetAnalysis()
{
  ...
  if (comp()->getMethodSymbol()->mayHaveNestedLoops() &&
    ...
    _hasImproperRegion = false; // Probably use a run time option here to enable
                                // old flow analysis behav
  } else
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForStructures()
{
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForStructure(
  TR_Structure* s)
{
...
template<class Container>
void
TR_BasicDFSetAnalysis<
  Container*>::initializeGenAndKillSetInfoPropertyForStructure(TR_Structure* s,
                                                               bool inLoop)
{
...
template<class Container>
bool
TR_ForwardDFSetAnalysis<Container*>::canGenAndKillForStructure(
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForRegion(
  TR_RegionStructure* region)
{
  ...
  analysisInfo->_exceptionKillSetInfo = new (this->trStackMemory()) TR_LinkHead<
    typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
  analysisInfo->_currentRegularGenSetInfo =
    new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
  analysisInfo->_currentRegularKillSetInfo =
    new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
  analysisInfo->_currentExceptionGenSetInfo =
    new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
  analysisInfo->_currentExceptionKillSetInfo =
    new (this->trStackMemory()) TR_LinkHead<
      typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair>;
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(
      ...
      for (auto pred = predecessors.getFirst(); pred;
            ...
            !predStructure->hasBeenAnalyzedBefore()) //_firstIteration)
        {
          ...
          TR_LinkHead<typename TR_BasicDFSetAnalysis<
            Container*>::TR_ContainerNodeNumberPair>*predCurrentGenSetInfo =
             normalPred ? predInfo->_currentRegularGenSetInfo
                        : predInfo->_currentExceptionGenSetInfo,
           *predCurrentKillSetInfo = normalPred
                                       ? predInfo->_currentRegularKillSetInfo
    ...
    if (this->traceBVA()) {
      ...
      if (nodeInfo->_regularGenSetInfo) {
        ...
        for (pair = nodeInfo->_regularGenSetInfo->getFirst(); pair;
          ...
          if (pair->_container) {
            traceMsg(
              this->comp(), "Exit or Succ numbered %d : ", pair->_nodeNumber);
    ...
    for (auto succ = successors.getFirst(); succ; succ = successors.getNext()) {
      ...
      *_currentRegularKillSetInfo = *this->_temp2;
      ...
      if (regionStructure->isExitEdge(succ) ||
        ...
        TR_LinkHead<typename TR_BasicDFSetAnalysis<
          Container*>::TR_ContainerNodeNumberPair>*_genSetInfo =
           normalSucc ? analysisInfo->_regularGenSetInfo
        ...
        TR_LinkHead<typename TR_BasicDFSetAnalysis<
          Container*>::TR_ContainerNodeNumberPair>*_currentGenSetInfo =
        ...
        if (!_currentGenSetInfo)
          TR_ASSERT(
            0,
            "Gen set info should have been allocated in initialization step\n");
        typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair*
          pair = NULL;
        pair =
          nodeInfo->getContainerNodeNumberPair(_currentGenSetInfo, nodeNumber);
        ...
        compose(pair->_container, _currentRegularGenSetInfo);
        pair =
          nodeInfo->getContainerNodeNumberPair(_currentKillSetInfo, nodeNumber);
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForBlock(
  TR_BlockStructure* s)
{
  typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* analysisInfo =
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::initializeAnalysisInfo(
...
template<class Container>
void
TR_BasicDFSetAnalysis<Container*>::clearAnalysisInfo(
  typename TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo* info)
{
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::initializeDFSetAnalysis()
{
...
template<class Container>
bool
TR_ForwardDFSetAnalysis<Container*>::analyzeRegionStructure(
  TR_RegionStructure* regionStructure,
  bool checkForChange)
{
...
template<class Container>
bool
TR_ForwardDFSetAnalysis<Container*>::analyzeNodeIfPredecessorsAnalyzed(
  TR_RegionStructure* regionStructure,
  TR_BitVector& pendingList)
{
  ...
  while (this->_analysisQueue.getListHead() &&
    ...
    if (this->supportsGenAndKillSets() &&
      ...
      for (auto succ = successors.getFirst(); succ;
        ...
        TR_LinkHead<typename TR_BasicDFSetAnalysis<
          Container*>::TR_ContainerNodeNumberPair>*_killSetInfo =
           normalSucc ? analysisInfo->_regularKillSetInfo
                      : analysisInfo->_exceptionKillSetInfo,
    ...
    for (auto succ = node->getExceptionSuccessors().begin();
         succ != node->getExceptionSuccessors().end();
         ++succ) {
      TR::CFGNode* succNode = (*succ)->getTo();
      if ((!regionStructure->isExitEdge(*succ)) &&
          (needToIterate || (succNode != regionStructure->getEntry())) &&
...
template<class Container>
bool
TR_ForwardDFSetAnalysis<Container*>::analyzeBlockStructure(
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::analyzeBlockZeroStructure(
  TR_BlockStructure* blockStructure)
{
  analyzeTreeTopsInBlockStructure(blockStructure);
}
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::analyzeNode(
  TR::Node* node,
  vcount_t visitCount,
  TR_BlockStructure* blockStructure,
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::analyzeTreeTopsInBlockStructure(
  TR_BlockStructure* blockStructure)
{
  TR::Block* block = blockStructure->getBlock();
  ...
  while (!(currentTree == exitTree)) {
    ...
    analyzeNode(
      currentTree->getNode(), visitCount, blockStructure, this->_regularInfo);
...
template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::compose(Container*, Container*)
{}

template<class Container>
void
TR_ForwardDFSetAnalysis<Container*>::inverseCompose(Container*, Container*)
{}

template<class Container>
TR_DataFlowAnalysis::Kind
TR_ForwardDFSetAnalysis<Container*>::getKind()
{
...
template<class Container>
Container*
TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo::getContainer(
  TR_LinkHead<typename TR_BasicDFSetAnalysis<
...
TR_BasicDFSetAnalysis<Container*>::ExtraAnalysisInfo::
  getContainerNodeNumberPair(TR_LinkHead<typename TR_BasicDFSetAnalysis<
                               Container*>::TR_ContainerNodeNumberPair>* list,
                             int32_t n)
{
  typename TR_BasicDFSetAnalysis<Container*>::TR_ContainerNodeNumberPair* pair;
