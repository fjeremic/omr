...
protected:
    ...
    type_t subWithOverflow(type_t a, type_t b, bool& overflow)
    {
        ...
        // when both operand bounds have a differing sign and the resulting value has the same sign as the first
        // operand. A non-negative result from xoring two numbers indicates that their sign bits are either both set, or
        // both unset, meaning they have the same sign, while a negative result from xoring two numbers means that
        // exactly one of them has its sign bit set, meaning the signs of the two numbers differ.
