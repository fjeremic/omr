...
public:
  ...
  static TR::VPIntConstraint* create(
    OMR::ValuePropagation* vp,
    TR::DataTypes dt,
    TR_YesNoMaybe isUnsigned); // Takes a TR::DataTypes instead of TR::DataType
                               // to work around ambiguous overloads
...
public:
  VPUnresolvedClass(const char* sig, int32_t len, TR_ResolvedMethod* method)
    : TR::VPClassType(UnresolvedClassPriority)
    , _method(method)
  {
    _sig = sig;
    _len = len;
...
public:
  VPMergedConstraints(ListElement<TR::VPConstraint>* first, TR_Memory* m)
    ...
    , _type((first && first->getData()->asShortConstraint()
               ? TR::Int16
               : ((first && first->getData()->asLongConstraint()) ? TR::Int64
                                                                  : TR::Int32)))
    , _constraints(m)
  {
