...
public:
  ...
  // fall-through.
  // Changes the "node" argument to be the replaced branch node, NULL if the
  // branch has been eliminated. Returns true if blocks were removed as a result
  // of the change.
  //
  bool conditionalToUnconditional(TR::Node *&node, TR::Block *block,
                                  int takeBranch);
  ...
  void anchorOrderDependentNodesInSubtree(TR::Node *node, TR::Node *replacement,
   ...
   */
  virtual bool isLegalToUnaryCancel(TR::Node *node, TR::Node *firstChild,
                                    TR::ILOpCodes opcode) {
    return true;
  }
   ...
   * NULL otherwise
   */
  virtual TR::Node *unaryCancelOutWithChild(TR::Node *node,
                                            TR::Node *firstChild,
                                            TR::TreeTop *anchorTree,
                                            TR::ILOpCodes opcode,
   ...
   */
  virtual bool isBoundDefinitelyGELength(TR::Node *boundChild,
                                         TR::Node *lengthChild);
   ...
   */
  virtual void setNodePrecisionIfNeeded(TR::Node *baseNode, TR::Node *node,
