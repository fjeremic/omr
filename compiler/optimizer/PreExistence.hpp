...
public:
  ...
  bool classIsFixed()
  {
    return _classKind == ClassIsFixed;
  }
  ...
  bool classIsPreexistent()
  {
    return _classKind == ClassIsPreexistent;
  }

  bool usedProfiledInfo()
  {
    return _profiledClazz != NULL;
  }
  ...
  TR_OpaqueClassBlock* getFixedProfiledClass()
  {
    return _profiledClazz;
  }
  ...
  bool isTypeInfoForInlinedBody()
  {
    return _isTypeInfoForInlinedBody;
  }
  void setTypeInfoForInlinedBody()
  {
    _isTypeInfoForInlinedBody = true;
  }
...
public:
  ...
  static void propagateReceiverInfoIfAvailable(
    TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_InlinerTracer* tracer);

  static void propagateArgsFromCaller(
    TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_InlinerTracer* tracer);

  static bool validateAndPropagateArgsFromCalleeSymbol(
    TR_PrexArgInfo* argsFromSymbol, TR_PrexArgInfo* argsFromTarget, TR_InlinerTracer* tracer);
  ...
  void set(int32_t index, TR_PrexArgument* info)
  {
    _args[index] = info;
  }
  TR_PrexArgument* get(int32_t index)
  {
    return _args[index];
  }
...
private:
  ...
  static TR::Node* getCallNode(
    TR::ResolvedMethodSymbol* methodSymbol, class TR_CallSite* callsite, class TR_InlinerTracer* tracer);
