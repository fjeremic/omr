...
public:
  ...
  TR_PrexArgument(ClassKind classKind, TR_OpaqueClassBlock *clazz = 0,
                  TR_OpaqueClassBlock *profiledClazz = 0)
      : _classKind(classKind), _class(clazz), _profiledClazz(profiledClazz),
  ...
  TR_PrexArgument(TR::KnownObjectTable::Index knownObjectIndex,
                  TR::Compilation *comp);

  bool classIsFixed() { return _classKind == ClassIsFixed; }

  void setClassIsFixed(TR_OpaqueClassBlock *fixedClass = 0,
                       TR_OpaqueClassBlock *profiledClazz = 0) {
...
private:
  ...
  bool _isTypeInfoForInlinedBody; // The prex arg info only apply to the inlined
                                  // body but not on the taken side
};

class TR_PrexArgInfo {
public:
#ifdef J9_PROJECT_SPECIFIC
  static TR_PrexArgInfo *enhance(TR_PrexArgInfo *dest, TR_PrexArgInfo *source,
                                 TR::Compilation *comp);

  static void propagateReceiverInfoIfAvailable(
      TR::ResolvedMethodSymbol *methodSymbol, TR_CallSite *callsite,
      TR_PrexArgInfo *argInfo, TR_InlinerTracer *tracer);

  static void propagateArgsFromCaller(TR::ResolvedMethodSymbol *methodSymbol,
                                      TR_CallSite *callsite,
                                      TR_PrexArgInfo *argInfo,
                                      TR_InlinerTracer *tracer);

  static bool
  validateAndPropagateArgsFromCalleeSymbol(TR_PrexArgInfo *argsFromSymbol,
                                           TR_PrexArgInfo *argsFromTarget,
                                           TR_InlinerTracer *tracer);

  static TR_PrexArgInfo *
  buildPrexArgInfoForMethodSymbol(TR::ResolvedMethodSymbol *methodSymbol,
                                  TR_InlinerTracer *tracer);
  void
  clearArgInfoForNonInvariantArguments(TR::ResolvedMethodSymbol *methodSymbol,
                                       TR_InlinerTracer *tracer);
#endif

  TR_ALLOC(TR_Memory::LocalOpts);
  TR_PrexArgInfo(int32_t numArgs, TR_Memory *m) : _numArgs(numArgs) {
    _args = (TR_PrexArgument **)m->allocateHeapMemory(
        sizeof(TR_PrexArgument *) * numArgs);
...
private:
  ...
  //
#ifdef J9_PROJECT_SPECIFIC
  static TR::Node *getCallNode(TR::ResolvedMethodSymbol *methodSymbol,
                               class TR_CallSite *callsite,
                               class TR_InlinerTracer *tracer);
  static bool hasArgInfoForChild(TR::Node *child, TR_PrexArgInfo *argInfo);
  static TR_PrexArgument *getArgForChild(TR::Node *child,
                                         TR_PrexArgInfo *argInfo);
#endif
};
