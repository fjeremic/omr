...
class TR_LoopEstimator {
 ...
 private:
  ...
  bool isRecognizableExitEdge(TR::CFGEdge* edge,
                              TR::ILOpCodes* op,
  ...
  bool getProgression(TR::Node* node,
                      TR::SymbolReference** ref,
                      TR_ProgressionKind* kind,
                      int32_t* incr);

  EntryInfo* getEntryValueForSymbol(TR::SymbolReference* ref);
  EntryInfo* getEntryValue(TR::Block* block,
  ...
  class ExitCondition {
   public:
    TR_ALLOC(TR_Memory::RedundantAsycCheckRemoval)
    ExitCondition(TR::ILOpCodes opCode,
                  TR::SymbolReference* local,
                  int64_t limit)
