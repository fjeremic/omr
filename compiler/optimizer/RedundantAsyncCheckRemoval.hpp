...
private:
  ...
  public:
    ...
    IncrementInfo(int32_t incr)
        : _incr(incr), _unknown(false), _kind(Identity) {}
  ...
  public:
    ...
    EntryInfo(EntryInfo *other)
        : _val(other->_val), _unknown(other->_unknown) {}
  ...
  void getLoopIncrements(TR_BitVector &candidates,
  ...
  bool getProgression(TR::Node *node, TR::SymbolReference **ref,
                      TR_ProgressionKind *kind, int32_t *incr);

  EntryInfo *getEntryValueForSymbol(TR::SymbolReference *ref);
  EntryInfo *getEntryValue(TR::Block *block, TR::SymbolReference *symRef,
                           TR_BitVector &nodesDone, EntryInfo **entryInfos);

  void mergeWithLoopIncrements(TR::Block *block,
                               IncrementInfo **loopIncrements);
  ...
  public:
    TR_ALLOC(TR_Memory::RedundantAsycCheckRemoval)
    ExitCondition(TR::ILOpCodes opCode, TR::SymbolReference *local,
                  int64_t limit)
...
private:
  ...
  private:
    ...
    List<TR_StructureSubGraphNode>
        _children; // nodes that we are are greater than
  ...
  int32_t processNaturalLoop(TR_RegionStructure *region,
                             bool isInsideImproperRegion = false);
  int32_t processImproperRegion(TR_RegionStructure *region);

  void propagateBackwards(TR_StructureSubGraphNode *node,
                          TR_StructureSubGraphNode *entry);
  void computeCoverageInfo(TR_StructureSubGraphNode *node,
                           TR_StructureSubGraphNode *entry);
  void markAncestors(TR_StructureSubGraphNode *node,
                     TR_StructureSubGraphNode *entry);
  void getNearestAncestors(TR_StructureSubGraphNode *node,
                           TR_StructureSubGraphNode *current,
                           TR_StructureSubGraphNode *entry);
  TR_StructureSubGraphNode *findSmallestAncestor();
  void insertAsyncCheckOnSubTree(TR_StructureSubGraphNode *node,
                                 TR_StructureSubGraphNode *entry);
  void enqueueSinks(TR_RegionStructure *region,
                    TR_Queue<TR_StructureSubGraphNode> *q, bool in);
  ...
  int32_t findShallowestCommonCaller(int32_t callSiteIndex1,
