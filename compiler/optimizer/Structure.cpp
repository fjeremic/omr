...
public:
  ExitExtraction(TR::Compilation *const comp, TR::Region &memRegion)
      : _comp(comp), _cfg(_comp->getFlowGraph()),
        _trace(_comp->getOption(TR_TraceExitExtraction)), _memRegion(memRegion),
        _structureRegion(_cfg->structureRegion()), _workStack(_memRegion),
        _queued(std::less<TR_Structure *>(), _memRegion),
        _regionContents(RcCmp(), _memRegion), _allPreds(_memRegion),
        _succs(_memRegion), _excSuccs(_memRegion) {}

  void
  extractUnconditionalExits(const TR::list<TR::Block *, TR::Region &> &blocks);
...
TR_Structure *TR_BlockStructure::cloneStructure(
  ...
  TR_BlockStructure *clonedBlockStructure = new (cfg()->structureRegion())
      TR_BlockStructure(comp(), correspondingBlock->getNumber(),
                        correspondingBlock);
...
void TR_RegionStructure::checkStructure(TR_BitVector *_blockNumbers) {
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    ...
    for (auto edge = node->getPredecessors().begin();
      ...
      TR_ASSERT_FATAL(isConsistent, "predecessor of this subGraph node not not "
                                    "contain this node in its successors");
    }
    for (auto edge = node->getExceptionPredecessors().begin();
         edge != node->getExceptionPredecessors().end(); ++edge) {
      pred = toStructureSubGraphNode((*edge)->getFrom());
    ...
    for (auto edge = node->getSuccessors().begin();
      ...
      TR_ASSERT_FATAL(isConsistent, "successor of this subGraph node not not "
                                    "contain this node in its predecessors");
    }
    for (auto edge = node->getExceptionSuccessors().begin();
         edge != node->getExceptionSuccessors().end(); ++edge) {
      succ = toStructureSubGraphNode((*edge)->getTo());
