...
//
TR_Structure::Kind
TR_Structure::getKind()
{
  return Blank;
}
TR_Structure::Kind
TR_BlockStructure::getKind()
{
  return Block;
}
TR_Structure::Kind
TR_RegionStructure::getKind()
{
  return Region;
}

TR_BlockStructure::TR_BlockStructure(TR::Compilation* comp,
                                     int32_t index,
                                     TR::Block* b)
...
bool
TR_BlockStructure::doDataFlowAnalysis(TR_DataFlowAnalysis* dfa,
...
bool
TR_RegionStructure::doDataFlowAnalysis(TR_DataFlowAnalysis* dfa,
                                       bool checkForChanges)
{
...
void
TR_BlockStructure::clearAnalysisInfo()
{
  setAnalysisInfo(NULL);
}

void
TR_RegionStructure::clearAnalysisInfo()
{
  setAnalysisInfo(NULL);
...
bool
TR_Structure::contains(TR_Structure* other, TR_Structure* commonParent)
{
...
//
TR_RegionStructure*
TR_Structure::findCommonParent(TR_Structure* other, TR::CFG* cfg)
{
...
void
TR_BlockStructure::replacePart(TR_Structure* from, TR_Structure* to)
{
  TR_ASSERT(0, "bad structure found trying to replace a substructure");
}

void
TR_RegionStructure::addSubNode(TR_StructureSubGraphNode* subNode)
{
...
void
TR_RegionStructure::cleanupAfterEdgeRemoval(TR::CFGNode* node)
{
 ...
 */
/* static */ void
TR_RegionStructure::extractUnconditionalExits(
...
void
TR_RegionStructure::ExitExtraction::extractUnconditionalExits(
  const TR::list<TR::Block*, TR::Region&>& blocks)
{
  collectWork(blocks);
...
void
TR_RegionStructure::ExitExtraction::enqueue(TR_Structure* const s)
{
  if (_trace)
...
void
TR_RegionStructure::ExitExtraction::collectWork(
  const TR::list<TR::Block*, TR::Region&>& blocks)
{
...
void
TR_RegionStructure::ExitExtraction::collectWorkFromRegion(
  TR_RegionStructure* region,
  const StructureSet& relevant)
{
...
void
TR_RegionStructure::ExitExtraction::extractStructure(
...
TR_BitVector&
TR_RegionStructure::ExitExtraction::regionContents(
...
void
TR_RegionStructure::ExitExtraction::removeContentsFromRegion(
  ...
  TR_RegionStructure* const region)
{
  ...
  if (_trace) {
    traceMsg(
      _comp, "adjusted contents of region %d:%p:", region->getNumber(), region);
...
void
TR_RegionStructure::ExitExtraction::traceBitVector(TR_BitVector& bv)
{
...
void
TR_RegionStructure::ExitExtraction::moveNodeIntoParent(
...
void
TR_RegionStructure::ExitExtraction::moveOutgoingEdgeToParent(
  TR_RegionStructure* const region,
  TR_RegionStructure* const parent,
  TR_StructureSubGraphNode* const node,
...
void
TR_RegionStructure::cleanupAfterNodeRemoval()
{
...
void
TR_RegionStructure::removeSubNodeWithoutCleanup(TR_StructureSubGraphNode* node)
{
...
void
TR_RegionStructure::removeSubNode(TR_StructureSubGraphNode* node)
{
...
TR_RegionStructure::findSubNodeInRegion(int32_t num)
{
...
TR_StructureSubGraphNode*
TR_RegionStructure::subNodeFromStructure(TR_Structure* const structure)
{
...
void
TR_RegionStructure::replacePart(TR_Structure* from, TR_Structure* to)
{
  ...
  TR_ASSERT(node != NULL,
            "structure %d doesn't include replacee node %d",
            _nodeIndex,
            from->getNumber());
  TR_ASSERT(node != getEntry() || _nodeIndex == to->getNumber(),
            "cannot replace the entry node in %d with node %d",
            _nodeIndex,
            to->getNumber());
  ...
  if (from->getNumber() != to->getNumber()) {
    ...
    for (auto edge = node->getExceptionPredecessors().begin();
         edge != node->getExceptionPredecessors().end();
         ++edge) {
      TR_RegionStructure* pred =
        toStructureSubGraphNode((*edge)->getFrom())->getStructure()->asRegion();
      if (pred)
...
void
TR_RegionStructure::replaceExitPart(int32_t fromNumber, int32_t toNumber)
{
...
void
TR_RegionStructure::collectExitBlocks(List<TR::Block>* exitBlocks,
...
void
TR_BlockStructure::collectExitBlocks(List<TR::Block>* exitBlocks,
                                     List<TR::CFGEdge>* exitEdges)
{
...
void
TR_RegionStructure::addGlobalRegisterCandidateToExits(
...
static bool
findCycle(TR_StructureSubGraphNode* node,
          TR_BitVector& regionNodes,
          TR_BitVector& nodesSeenOnPath,
          ...
          int32_t entryNode)
{
  ...
  for (auto edge = node->getExceptionSuccessors().begin();
       edge != node->getExceptionSuccessors().end();
       ++edge) {
    TR_ASSERT(
      (*edge)->getTo()->asStructureSubGraphNode(),
      "Expecting a CFG node which can be downcast to StructureSubGraphNode");
...
void
TR_RegionStructure::checkForInternalCycles()
{
  ...
  setContainsInternalCycles(findCycle(
    getEntry(), regionNodes, nodesSeenOnPath, nodesCleared, getNumber()));
}

bool
TR_RegionStructure::hasExceptionOutEdges()
{
...
void
TR_BlockStructure::removeEdge(TR_Structure* from, TR_Structure* to)
{
...
void
TR_RegionStructure::removeEdgeWithoutCleanup(TR::CFGEdge* edge, bool isExitEdge)
{
...
void
TR_RegionStructure::removeEdge(TR::CFGEdge* edge, bool isExitEdge)
{
...
void
TR_RegionStructure::removeEdge(TR_Structure* from, TR_Structure* to)
{
...
int
TR_BlockStructure::removeExternalEdgeTo(TR_Structure* from, int32_t toNumber)
{
...
int
TR_RegionStructure::removeExternalEdgeTo(TR_Structure* from, int32_t toNumber)
{
...
void
TR_Structure::setNestingDepths(int32_t* currentNestingDepth)
{
  if (getParent())
...
void
TR_Structure::setAnyCyclicRegionNestingDepths(int32_t* currentNestingDepth)
{
  if (getParent())
...
int32_t
TR_Structure::getMaxNestingDepth(int32_t* currentNestingDepth,
...
int32_t
TR_RegionStructure::getMaxNestingDepth(int32_t* currentNestingDepth,
                                       int32_t* currentMaxNestingDepth)
{
...
void
TR_Structure::calculateFrequencyOfExecution(int32_t* currentWeight)
{
  if (getParent())
...
void
TR_Structure::setConditionalityWeight(int32_t* currentWeight)
{
  TR_RegionStructure* region = asRegion();
  bool isAcyclicRegion = false;
  if (region && (!region->isAcyclic() ||
                 (region == comp()->getFlowGraph()->getStructure()))) {
    adjustWeightForBranches(
      region->getEntry(), region->getEntry(), currentWeight);
...
void
TR_Structure::adjustWeightForBranches(TR_StructureSubGraphNode* node,
                                      ...
                                      int32_t* currentWeight)
{
  ...
  for (auto edge = node->getSuccessors().begin();
    ...
    if ((*edge)->getTo() != entry)
      adjustWeightForBranches(
        toStructureSubGraphNode((*edge)->getTo()), entry, &weight);
  }
...
TR_StructureSubGraphNode*
TR_Structure::findNodeInHierarchy(TR_RegionStructure* region, int32_t num)
{
  if (!region)
    return NULL;
...
bool
TR_RegionStructure::containsOnlyAcyclicRegions()
{
  TR_StructureSubGraphNode* subNode;
  TR_Structure* subStruct = NULL;
  TR_RegionStructure::Cursor si(*this);
...
TR_Structure*
TR_BlockStructure::cloneStructure(
...
TR_Structure*
TR_RegionStructure::cloneStructure(
  TR::Block** correspondingBlocks,
  ...
  List<TR_Structure>* correspondingWhileLoops)
{
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    ...
    TR_Structure* clonedSubStruct =
      subStruct->cloneStructure(correspondingBlocks,
                                correspondingSubNodes,
                                whileLoops,
                                correspondingWhileLoops);
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    ...
    for (auto edge = subNode->getExceptionSuccessors().begin();
         edge != subNode->getExceptionSuccessors().end();
         ++edge) {
      TR_StructureSubGraphNode* succ =
        toStructureSubGraphNode((*edge)->getTo());
      if (!isExitEdge(*edge)) {
        TR::CFGEdge::createExceptionEdge(
          correspondingSubNode,
          correspondingSubNodes[succ->getNumber()],
          trMemory());
      } else {
        clonedRegionStructure->addExitEdge(
          correspondingSubNode, succ->getNumber(), 1);
      }
    }
  }
  ...
  while (currInductionVariable) {
    ...
    memcpy(currClonedInductionVariable,
           currInductionVariable,
...
void
TR_BlockStructure::cloneStructureEdges(TR::Block** correspondingBlocks)
{}

void
TR_RegionStructure::cloneStructureEdges(TR::Block** correspondingBlocks)
{
  TR_RegionStructure::Cursor si(*this);
  TR_StructureSubGraphNode* subNode;
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
...
List<TR::Block>*
TR_BlockStructure::getBlocks(List<TR::Block>* blocksInRegion)
{
...
List<TR::Block>*
TR_RegionStructure::getBlocks(List<TR::Block>* blocksInRegion)
{
  vcount_t visitCount = comp()->incVisitCount();
  return getBlocks(blocksInRegion, visitCount);
}
...
void
TR_Structure::addEdge(TR::CFGEdge* edge, bool isExceptionEdge)
{
  TR_ASSERT(false, "addEdge called for non-region");
}

void
TR_RegionStructure::addEdge(TR::CFGEdge* edge, bool isExceptionEdge)
{
  ...
  if (fromStruct->asRegion()) {
    ...
    //
    fromStruct->addExternalEdge(
      from->getStructureOf(), to->getNumber(), isExceptionEdge);
  }
...
void
TR_BlockStructure::addExternalEdge(TR_Structure* from,
                                   int32_t toNumber,
                                   bool isExceptionEdge)
{}

void
TR_RegionStructure::addExternalEdge(TR_Structure* from,
                                    int32_t toNumber,
                                    bool isExceptionEdge)
{
...
void
TR_RegionStructure::collapseIntoParent()
{
        ...
        for (target = pTargets.getCurrent(); target;
          ...
          if (target->getNumber() == toNum) {
            ...
            for (auto pred = target->getPredecessors().begin();
                 pred != target->getPredecessors().end();
                 ++pred) {
        ...
        for (target = pTargets.getCurrent(); target;
          ...
          if (target->getNumber() == toNum) {
            for (auto pred = target->getExceptionPredecessors().begin();
                 pred != target->getExceptionPredecessors().end();
                 ++pred) {
              if (((*pred)->getFrom()->getNumber() == getNumber()) &&
  ...
  for (auto edge = nodeInParent->getPredecessors().begin();
       edge != nodeInParent->getPredecessors().end();
       ++edge)
    (*edge)->setTo(getEntry());
  ...
  for (auto edge = nodeInParent->getExceptionPredecessors().begin();
       edge != nodeInParent->getExceptionPredecessors().end();
       ++edge)
...
TR::CFGEdge*
TR_RegionStructure::addExitEdge(TR_StructureSubGraphNode* from,
                                int32_t to,
                                bool isExceptionEdge,
                                TR::CFGEdge* origEdge)
{
...
TR_StructureSubGraphNode*
TR_StructureSubGraphNode::asStructureSubGraphNode()
{
  return this;
}

void
TR_BlockStructure::checkStructure(TR_BitVector* _blockNumbers)
{
  TR_ASSERT_FATAL(
...
void
TR_RegionStructure::checkStructure(TR_BitVector* _blockNumbers)
{
  TR_StructureSubGraphNode* node;
  TR_ASSERT_FATAL(
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    ...
    for (auto edge = node->getPredecessors().begin();
      ...
      for (auto succEdge = pred->getSuccessors().begin();
           succEdge != pred->getSuccessors().end();
           ++succEdge) {
        if (*succEdge == *edge) {
          isConsistent = true;
          break;
    ...
    for (auto edge = node->getExceptionPredecessors().begin();
      ...
      for (auto succEdge = pred->getExceptionSuccessors().begin();
           succEdge != pred->getExceptionSuccessors().end();
           ++succEdge) {
        if ((*succEdge) == (*edge)) {
          isConsistent = true;
          break;
    ...
    for (auto edge = node->getSuccessors().begin();
      ...
      for (auto predEdge = succ->getPredecessors().begin();
           predEdge != succ->getPredecessors().end();
           ++predEdge) {
        if (*predEdge == *edge) {
          isConsistent = true;
          break;
    ...
    for (auto edge = node->getExceptionSuccessors().begin();
      ...
      for (auto predEdge = succ->getExceptionPredecessors().begin();
           predEdge != succ->getExceptionPredecessors().end();
           ++predEdge) {
        if ((*predEdge) == *edge) {
          isConsistent = true;
          break;
...
void
TR_BlockStructure::resetAnalysisInfo()
{
  setAnalysisInfo(NULL);
}

void
TR_RegionStructure::resetAnalysisInfo()
{
  TR_StructureSubGraphNode* node;
  TR_RegionStructure::Cursor si(*this);
  for (node = si.getCurrent(); node != NULL; node = si.getNext())
...
void
TR_BlockStructure::resetAnalyzedStatus()
{
  setAnalyzedStatus(false);
}

void
TR_RegionStructure::resetAnalyzedStatus()
{
  TR_StructureSubGraphNode* node;
  TR_RegionStructure::Cursor si(*this);
  for (node = si.getCurrent(); node != NULL; node = si.getNext())
...
bool
TR_BlockStructure::markStructuresWithImproperRegions()
{
...
bool
TR_RegionStructure::markStructuresWithImproperRegions()
{
  bool b = false;
  TR_StructureSubGraphNode* node;
  TR_RegionStructure::Cursor si(*this);
...
TR::Block*
TR_BlockStructure::setBlock(TR::Block* b)
{
  _block = b;
...
bool
TR_BlockStructure::changeContinueLoopsToNestedLoops(TR_RegionStructure* root)
{
  return false;
}

bool
TR_RegionStructure::changeContinueLoopsToNestedLoops(TR_RegionStructure* root)
{
  ...
  if (isNaturalLoop()) {
    ...
    if (numNewHeaders == 1 &&
        performTransformation(
          ...
          OPT_DETAILS,
          getEntryBlock()->asBlock()->getNumber(),
          numNewHeaders)) {
      ...
      for (TR::CFGEdge* splitEdge = seIterator.getFirst(); splitEdge;
        ...
        TR::Block* newHeader =
          TR::Block::createEmptyBlock(entryBlock->getEntry()->getNode(),
                                      comp(),
                                      splitEdge->getFrequency(),
                                      prevHeader);
        ...
        TR::Block::redirectFlowToNewDestination(
          comp(), splitEdge, newHeader, false);
      ...
      for (TR::CFGEdge* incomingEdge = ieIterator.getFirst(); incomingEdge;
        ...
        TR::Block::redirectFlowToNewDestination(
          comp(), incomingEdge, newHeader, false);
      }
...
//               be numbered as 2
void
TR_Structure::mergeBlocks(TR::Block* merged, TR::Block* mergedInto)
{
...
void
TR_Structure::mergeInto(TR::Block* merged, TR::Block* mergedInto)
{
...
//               be numbered as 2
void
TR_RegionStructure::mergeInto(TR::Block* merged, TR::Block* mergedInto)
{
...
void
TR_Structure::removeMergedBlock(TR::Block* merged, TR::Block* mergedInto)
{
  TR_ASSERT(false, "Structure: bad structure when merging blocks");
}

void
TR_RegionStructure::removeMergedBlock(TR::Block* merged, TR::Block* mergedInto)
{
  // Find the subnode containing the block to be removed
  //
  TR_StructureSubGraphNode* node;
...
void
TR_BlockStructure::collectCFGEdgesTo(int32_t toNum, List<TR::CFGEdge>* cfgEdges)
{
  TR::CFGEdge* edge;
...
void
TR_RegionStructure::collectCFGEdgesTo(int32_t toNum,
...
void
TR_BlockStructure::renumber(int32_t num)
{
  _nodeIndex = num;
...
void
TR_RegionStructure::renumber(int32_t num)
{
...
void
TR_BlockStructure::resetVisitCounts(vcount_t num)
{}

void
TR_RegionStructure::resetVisitCounts(vcount_t num)
{
  TR_StructureSubGraphNode* node;
  TR_Structure* s = NULL;
  TR_RegionStructure::Cursor si(*this);
...
bool
TR_BlockStructure::renumberRecursively(int32_t origNum, int32_t num)
{
...
bool
TR_RegionStructure::renumberRecursively(int32_t origNum, int32_t num)
{
  TR_StructureSubGraphNode* node;
  TR_Structure* s = NULL;
  TR_RegionStructure::Cursor si(*this);
...
void
TR_RegionStructure::computeInvariantSymbols()
{
...
void
TR_RegionStructure::updateInvariantSymbols(TR::Node* node, vcount_t visitCount)
{
...
void
TR_RegionStructure::computeInvariantExpressions()
{
...
void
TR_RegionStructure::updateInvariantExpressions(TR::Node* node,
                                               vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
    return;
...
void
TR_RegionStructure::resetInvariance()
{
...
bool
TR_RegionStructure::isExprInvariant(TR::Node* expr, bool usePrecomputed)
{
...
bool
TR_RegionStructure::isExprTreeInvariant(TR::Node* expr)
{
...
bool
TR_RegionStructure::isSymbolRefInvariant(TR::SymbolReference* symRef)
{
  if (!_invariantSymbols)
    computeInvariantSymbols();
...
bool
TR_RegionStructure::isSubtreeInvariant(TR::Node* node, vcount_t visitCount)
{
  if (node->getVisitCount() == visitCount)
    return true;
...
void
TR_BlockStructure::hoistInvariantsOutOfNestedLoops(
  TR_LocalTransparency* transparencyInfo,
  TR_BitVector** optSetInfo,
  bool insideLoop,
  TR_BlockStructure* outerLoopInvariantBlock,
  ...
  int32_t bitVectorSize)
{
  ...
  if (isCanonicalizedLoop() && getEntry()->getSuccessors().size() == 2) {
    for (auto succ = getEntry()->getSuccessors().begin();
         succ != getEntry()->getSuccessors().end();
         ++succ) {
...
bool
TR_BlockStructure::isExpressionTransparentIn(
...
bool
TR_RegionStructure::isExpressionTransparentIn(
  int32_t nextOptimalComputation,
  TR_LocalTransparency* transparencyInfo)
{
  TR_StructureSubGraphNode* node;
...
TR_RegionStructure::findMatchingIV(TR::SymbolReference* symRef)
{
