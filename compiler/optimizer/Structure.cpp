...
// The "getKind" methods are made non-inline to identify this as the home for
// the vfts for this class hierarchy.
//
...
TR_BlockStructure::TR_BlockStructure(TR::Compilation *comp, int32_t index,
                                     TR::Block *b)
    : TR_Structure(comp, index), _block(b) {
  b->setStructureOf(this);
}

bool TR_BlockStructure::doDataFlowAnalysis(TR_DataFlowAnalysis *dfa,
                                           bool checkForChanges) {
  // LexicalTimer tlex("blockStructure_doDFA", comp()->phaseTimer());
  return dfa->analyzeBlockStructure(this, checkForChanges);
}

bool TR_RegionStructure::doDataFlowAnalysis(TR_DataFlowAnalysis *dfa,
                                            bool checkForChanges) {
...
TR_RegionStructure *TR_Structure::getContainingLoop() {
  for (TR_RegionStructure *p = getParent(); p != NULL; p = p->getParent())
    if (p->isNaturalLoop()) return p;
  return NULL;
}

// Note that if A contains B, the common parent is parent(A)
//
TR_RegionStructure *TR_Structure::findCommonParent(TR_Structure *other,
                                                   TR::CFG *cfg) {
  for (TR_RegionStructure *parent = getParent()->asRegion(); parent;
       parent = parent->getParent()->asRegion()) {
    if (parent->contains(other)) return parent;
  }
  TR_ASSERT(
      this == cfg->getStructure() || other == cfg->getStructure(),
...
void TR_RegionStructure::cleanupAfterEdgeRemoval(TR::CFGNode *node) {
  TR_BlockStructure *blockSt =
      toStructureSubGraphNode(node)->getStructure()->asBlock();
  if (node != getEntry() && node->getPredecessors().empty() &&
  ...
  if (containsInternalCycles())
    ...
    TR_ASSERT(si.getFirst() == getEntry(),
              "bad structure found trying to remove a block");
...
class TR_RegionStructure::ExitExtraction {
 public:
  ...
  void extractUnconditionalExits(
 ...
 private:
  ...
  typedef std::set<TR_Structure *, std::less<TR_Structure *>, StructureAlloc>
      StructureSet;
  ...
  typedef std::map<TR_RegionStructure *, TR_BitVector, RcCmp, RcAlloc>
      RegionContents;

  void enqueue(TR_Structure *);
  void collectWork(const TR::list<TR::Block *, TR::Region &> &blocks);
  void collectWorkFromRegion(TR_RegionStructure *region,
                             const StructureSet &relevant);
  void extractStructure(TR_Structure *);
  TR_BitVector &regionContents(TR_RegionStructure *loop);
  void removeContentsFromRegion(TR_Structure *removed,
                                TR_RegionStructure *region);
  void traceBitVector(TR_BitVector &bv);
  void moveNodeIntoParent(TR_StructureSubGraphNode *node,
                          TR_RegionStructure *region,
                          TR_RegionStructure *parent);
  void moveOutgoingEdgeToParent(TR_RegionStructure *region,
                                TR_RegionStructure *parent,
                                TR_StructureSubGraphNode *node,
                                TR::CFGEdge *edge, bool isExceptionEdge);
 ...
 * - \c checkStructureDuringExitExtraction to \c checkStructure() in non-debug
 builds
...
/* static */ void TR_RegionStructure::extractUnconditionalExits(
  ...
  if (comp->getOption(TR_DisableExitExtraction)) return;

  if (comp->getFlowGraph()->getStructure() == NULL) return;
...
void TR_RegionStructure::ExitExtraction::extractUnconditionalExits(
  ...
  while (_workStack.size() > 0) {
    ...
    if (_cfg->getStructure() == NULL) return;
  }
}

void TR_RegionStructure::ExitExtraction::enqueue(TR_Structure *const s) {
  if (_trace) traceMsg(_comp, "enqueueing %d:%p\n", s->getNumber(), s);
    ...
    while (s != NULL && relevant.find(s) == relevant.end()) {
      ...
      if (_trace) {
        traceMsg(_comp, "found relevant structure %d:%p, parent %d:%p\n",
                 s->getNumber(), s, parent == NULL ? -1 : parent->getNumber(),
                 parent);
      }
...
void TR_RegionStructure::ExitExtraction::collectWorkFromRegion(
    TR_RegionStructure *region, const StructureSet &relevant) {
  ...
  for (auto node = it.getFirst(); node != NULL; node = it.getNext()) {
    ...
    if (relevant.find(s) == relevant.end()) continue;
...
void TR_RegionStructure::ExitExtraction::extractStructure(
  ...
  TR_StructureSubGraphNode *node =
      region->subNodeFromStructure(initialStructure);
  for (;;) {
    TR_ASSERT_FATAL(
    ...
    if (!region->isNaturalLoop() && !region->isAcyclic()) return;
    ...
    if (region != loop) {
      ...
      for (auto e = sit.getFirst(); e != NULL; e = sit.getNext()) {
        if (loopBlocks.isSet(e->getTo()->getNumber())) return;
      }
    }
    ...
    if (!performTransformation(_comp,
                               ...
                               OPT_DETAILS, node->getNumber(),
                               region->getNumber(), region, parent->getNumber(),
                               parent))
      return;
    ...
    if (_trace && _comp->getOutFile() != NULL) {
      traceMsg(_comp,
               "Structure after moving node into parent:\n<structure>\n");
    ...
    if (region == loop) {
      ...
      if (loop == NULL) return;  // no more containing loops
    }
...
TR_BitVector &TR_RegionStructure::ExitExtraction::regionContents(
  ...
  if (existing != _regionContents.end()) return existing->second;
...
void TR_RegionStructure::ExitExtraction::removeContentsFromRegion(
  ...
  if (entry == _regionContents.end()) return;
  ...
  if (_trace) {
    traceMsg(_comp, "adjusted contents of region %d:%p:", region->getNumber(),
             region);
...
void TR_RegionStructure::ExitExtraction::traceBitVector(TR_BitVector &bv) {
  ...
  while (bvi.hasMoreElements()) traceMsg(_comp, " %d", bvi.getNextElement());

  traceMsg(_comp, "\n");
}

void TR_RegionStructure::ExitExtraction::moveNodeIntoParent(
    TR_StructureSubGraphNode *const node, TR_RegionStructure *const region,
  ...
  for (auto e = _allPreds.begin(); e != _allPreds.end(); ++e) {
    ...
    if (_trace) {
      traceMsg(_comp, "removed edge (%d->%d):%p from region %d:%p\n",
               (*e)->getFrom()->getNumber(), (*e)->getTo()->getNumber(), *e,
  ...
  if (_trace) traceMsg(_comp, "moved node into parent\n");
  ...
  for (auto e = _allPreds.begin(); e != _allPreds.end(); ++e) {
    ...
    if (_trace) {
      traceMsg(_comp, "added exit edge (%d->%d) to region %d:%p\n",
               src->getNumber(), node->getNumber(), region->getNumber(),
               region);
    }
  }
  ...
  if (_trace) {
    traceMsg(_comp, "added %sedge (%d->%d) to region %d:%p\n",
             nodeIsHandler ? "exception " : "", regionNode->getNumber(),
             node->getNumber(), parent->getNumber(), parent);
  }
...
void TR_RegionStructure::ExitExtraction::moveOutgoingEdgeToParent(
    ...
    TR_StructureSubGraphNode *const node, TR::CFGEdge *const edge,
...
void TR_RegionStructure::cleanupAfterNodeRemoval() {
  ...
  if ((numSubNodes() == 1) && !isNaturalLoop() &&
    ...
    TR_ASSERT(si.getFirst() == getEntry(),
              "bad structure found trying to remove a block");
    if (getParent()) getParent()->replacePart(this, getEntry()->getStructure());
  }
}

void TR_RegionStructure::removeSubNodeWithoutCleanup(
    TR_StructureSubGraphNode *node) {
...
void TR_RegionStructure::replacePart(TR_Structure *from, TR_Structure *to) {
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    if (node->getStructure() == from) break;
  }

  TR_ASSERT(node != NULL, "structure %d doesn't include replacee node %d",
            _nodeIndex, from->getNumber());
  TR_ASSERT(node != getEntry() || _nodeIndex == to->getNumber(),
            "cannot replace the entry node in %d with node %d", _nodeIndex,
            to->getNumber());
  ...
  if (from->getNumber() != to->getNumber()) {
    for (auto edge = node->getPredecessors().begin();
      ...
      TR_RegionStructure *pred = toStructureSubGraphNode((*edge)->getFrom())
                                     ->getStructure()
                                     ->asRegion();
      if (pred) pred->replaceExitPart(from->getNumber(), to->getNumber());
    }
    for (auto edge = node->getExceptionPredecessors().begin();
         edge != node->getExceptionPredecessors().end(); ++edge) {
      TR_RegionStructure *pred = toStructureSubGraphNode((*edge)->getFrom())
                                     ->getStructure()
                                     ->asRegion();
      if (pred) pred->replaceExitPart(from->getNumber(), to->getNumber());
...
void TR_RegionStructure::replaceExitPart(int32_t fromNumber, int32_t toNumber) {
  ...
  for (edge = ei.getCurrent(); edge; edge = ei.getNext()) {
    if (edge->getTo()->getNumber() == fromNumber) {
      TR_RegionStructure *pred =
          toStructureSubGraphNode(edge->getFrom())->getStructure()->asRegion();
      if (pred) pred->replaceExitPart(fromNumber, toNumber);
    }
  }
  ...
  TR_ASSERT(haveReplaced,
            "bad structure found trying to replace a substructure");
}

void TR_RegionStructure::collectExitBlocks(List<TR::Block> *exitBlocks,
                                           List<TR::CFGEdge> *exitEdges) {
  TR_BitVector *seenNodes =
      new (trStackMemory()) TR_BitVector(1, trMemory(), stackAlloc, growable);
  TR::CFGEdge *edge;
  ListIterator<TR::CFGEdge> ei(&_exitEdges);
  for (edge = ei.getCurrent(); edge; edge = ei.getNext()) {
    if (!seenNodes->get(edge->getFrom()->getNumber())) {
      ...
      TR_Structure *s =
          edge->getFrom()->asStructureSubGraphNode()->getStructure();
      ...
      if (s->asBlock() && exitEdges) {
        ...
        for (TR::CFGEdge *exitEdge = exitIt.getFirst(); exitEdge != NULL;
             exitEdge = exitIt.getNext()) {
          if (toNum == exitEdge->getTo()->getNumber()) exitEdges->add(exitEdge);
        }
      }
    }
  }
...
void TR_BlockStructure::collectExitBlocks(List<TR::Block> *exitBlocks,
                                          List<TR::CFGEdge> *exitEdges) {
  exitBlocks->add(getBlock());
}

void TR_RegionStructure::addGlobalRegisterCandidateToExits(
...
static bool findCycle(TR_StructureSubGraphNode *node, TR_BitVector &regionNodes,
                      TR_BitVector &nodesSeenOnPath, TR_BitVector &nodesCleared,
                      int32_t entryNode) {
  ...
  for (auto edge = node->getExceptionSuccessors().begin();
       edge != node->getExceptionSuccessors().end(); ++edge) {
    TR_ASSERT(
        (*edge)->getTo()->asStructureSubGraphNode(),
        "Expecting a CFG node which can be downcast to StructureSubGraphNode");
    TR_StructureSubGraphNode *succ = toStructureSubGraphNode((*edge)->getTo());
    if (/* succ->getNumber() != entryNode && */ regionNodes.get(
            succ->getNumber()) &&
        findCycle(succ, regionNodes, nodesSeenOnPath, nodesCleared, entryNode))
      return true;
  }
...
void TR_RegionStructure::checkForInternalCycles() {
  ...
  setContainsInternalCycles(findCycle(getEntry(), regionNodes, nodesSeenOnPath,
                                      nodesCleared, getNumber()));
}
...
bool TR_RegionStructure::hasExceptionOutEdges() {
  ...
  for (TR::CFGEdge *edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
    ...
    if (!edgeTo->getExceptionPredecessors().empty()) return true;
  }
  return false;
}
...
void TR_BlockStructure::removeEdge(TR_Structure *from, TR_Structure *to) {
  ...
  if (parent) parent->removeEdge(from, to);
}

void TR_RegionStructure::removeEdgeWithoutCleanup(TR::CFGEdge *edge,
                                                  bool isExitEdge) {
  // Remove this edge from the region's subgraph
  //
  TR::CFGNode *fromNode = edge->getFrom();
  TR::CFGNode *toNode = edge->getTo();
  if (std::find(fromNode->getSuccessors().begin(),
                fromNode->getSuccessors().end(),
                edge) != fromNode->getSuccessors().end()) {
    fromNode->getSuccessors().remove(edge);
    TR_ASSERT((std::find(toNode->getPredecessors().begin(),
                         toNode->getPredecessors().end(),
                         edge) != toNode->getPredecessors().end()),
              "bad structure found trying to remove an out edge");
    toNode->getPredecessors().remove(edge);
  } else {
    TR_ASSERT((std::find(fromNode->getExceptionSuccessors().begin(),
                         fromNode->getExceptionSuccessors().end(),
                         edge) != fromNode->getExceptionSuccessors().end()),
              "bad structure found trying to remove an out edge");
    fromNode->getExceptionSuccessors().remove(edge);
    TR_ASSERT((std::find(toNode->getExceptionPredecessors().begin(),
                         toNode->getExceptionPredecessors().end(),
                         edge) != toNode->getExceptionPredecessors().end()),
              "bad structure found trying to remove an out edge");
    toNode->getExceptionPredecessors().remove(edge);
  }

  if (isExitEdge) _exitEdges.remove(edge);
}
...
void TR_RegionStructure::removeEdge(TR::CFGEdge *edge, bool isExitEdge) {
  ...
  if (!isExitEdge) {
    ...
    if (edge->getTo() == edge->getFrom()) cleanupAlreadyDone = true;
  }

  if (!cleanupAlreadyDone) cleanupAfterEdgeRemoval(edge->getFrom());
}
...
void TR_RegionStructure::removeEdge(TR_Structure *from, TR_Structure *to) {
  ...
  TR_RegionStructure::Cursor si(*this);
  for (TR_StructureSubGraphNode *node = si.getCurrent(); node != NULL;
       node = si.getNext()) {
  ...
  //
  if (fromNode == NULL) {
    TR_ASSERT(toNode == getEntry(),
              "bad structure found trying to remove a block");
    if (getParent()) getParent()->removeEdge(from, to);
    return;
  }
  ...
  //
  int removedEdgeType =
      fromNode->getStructure()->removeExternalEdgeTo(from, to->getNumber());
  ...
  if (removedEdgeType == NO_MORE_EDGES_EXIST) {
    ...
    for (; edge != fromNode->getSuccessors().end(); ++edge)
      if ((*edge)->getTo()->getNumber() == to->getNumber()) break;
...
int TR_BlockStructure::removeExternalEdgeTo(TR_Structure *from,
                                            int32_t toNumber) {
  TR_ASSERT(getNumber() == from->getNumber(),
            "bad structure found trying to remove an out edge");

  return NO_MORE_EDGES_EXIST;
}

int TR_RegionStructure::removeExternalEdgeTo(TR_Structure *from,
                                             int32_t toNumber) {
  ...
  for (TR::CFGNode *n = ni.getCurrent(); n; n = ni.getNext()) {
    ...
    if (node->getStructure()->contains(from, this)) break;
  }
  ...
  for (edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
    if (edge->getTo()->getNumber() == toNumber) {
      ...
      if (oneExitEdge) moreThanOneExitEdgeToSameToNumber = true;
      oneExitEdge = true;
    }
  }

  TR_ASSERT(relevantEdge != NULL,
            "bad structure found trying to remove an out edge");
...
void TR_Structure::setNestingDepths(int32_t *currentNestingDepth) {
  if (getParent()) getParent()->setNestingDepths(currentNestingDepth);
...
void TR_Structure::setAnyCyclicRegionNestingDepths(
    int32_t *currentNestingDepth) {
  if (getParent())
...
int32_t TR_Structure::getMaxNestingDepth(int32_t *currentNestingDepth,
                                         int32_t *currentMaxNestingDepth) {
  return *currentMaxNestingDepth;
}

int32_t TR_RegionStructure::getMaxNestingDepth(
    int32_t *currentNestingDepth, int32_t *currentMaxNestingDepth) {
  bool isLoop = isNaturalLoop();
  if (isLoop) *currentNestingDepth = (*currentNestingDepth) + 1;
...
void TR_Structure::calculateFrequencyOfExecution(int32_t *currentWeight) {
  if (getParent()) getParent()->calculateFrequencyOfExecution(currentWeight);

  TR_RegionStructure *region = asRegion();
  if (region && !region->isAcyclic() &&
      (*currentWeight < (INT_MAX / 10)))  // careful: don't overflow 32b int!
...
void TR_Structure::setConditionalityWeight(int32_t *currentWeight) {
  ...
  if (region && (!region->isAcyclic() ||
    ...
    adjustWeightForBranches(region->getEntry(), region->getEntry(),
                            currentWeight);
...
void TR_Structure::adjustWeightForBranches(TR_StructureSubGraphNode *node,
  ...
  if (node->getStructure() && (weight > node->getStructure()->getWeight())) {
    ...
    // node->getNumber(), node->getStructure(), weight);
  } else
    return;
  ...
  if (node->getStructure()->asRegion()) {
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
      ...
      if (!subStruct->asRegion()) {
        ...
        adjustWeightForBranches(subStruct->asRegion()->getEntry(),
                                subStruct->asRegion()->getEntry(), &weight);
    }
  }
  ...
  for (auto edge = node->getSuccessors().begin();
    ...
    weight = startingWeight;
    if ((*edge)->getTo() != entry)
      adjustWeightForBranches(toStructureSubGraphNode((*edge)->getTo()), entry,
                              &weight);
  }
...
TR_StructureSubGraphNode *TR_Structure::findNodeInHierarchy(
...
bool TR_RegionStructure::containsOnlyAcyclicRegions() {
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    ...
    if (region &&
        (!region->isAcyclic() || !region->containsOnlyAcyclicRegions()))
      return false;
  }
...
TR_Structure *TR_BlockStructure::cloneStructure(
    TR::Block **correspondingBlocks,
    TR_StructureSubGraphNode **correspodingSubNodes,
  ...
  TR_BlockStructure *clonedBlockStructure =
      new (cfg()->structureRegion()) TR_BlockStructure(
          comp(), correspondingBlock->getNumber(), correspondingBlock);
...
TR_Structure *TR_RegionStructure::cloneStructure(
    TR::Block **correspondingBlocks,
    TR_StructureSubGraphNode **correspondingSubNodes,
    List<TR_Structure> *whileLoops,
    List<TR_Structure> *correspondingWhileLoops) {
  TR::Region &structureRegion = cfg()->structureRegion();
  TR_RegionStructure *clonedRegionStructure = new (structureRegion)
      TR_RegionStructure(comp(), correspondingBlocks[getNumber()]->getNumber());
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    subStruct = subNode->getStructure();
    TR_Structure *clonedSubStruct =
        subStruct->cloneStructure(correspondingBlocks, correspondingSubNodes,
                                  whileLoops, correspondingWhileLoops);
    TR_StructureSubGraphNode *clonedSubNode =
        new (structureRegion) TR_StructureSubGraphNode(clonedSubStruct);
    clonedRegionStructure->addSubNode(clonedSubNode);
    if (subNode == getEntry()) clonedRegionStructure->setEntry(clonedSubNode);
  ...
  si.reset();
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    TR_StructureSubGraphNode *correspondingSubNode =
        correspondingSubNodes[subNode->getNumber()];
    ...
    for (auto edge = subNode->getSuccessors().begin();
      ...
      if (!isExitEdge(*edge)) {
        TR::CFGEdge::createEdge(correspondingSubNode,
                                correspondingSubNodes[succ->getNumber()],
                                trMemory());
      } else {
        clonedRegionStructure->addExitEdge(correspondingSubNode,
                                           succ->getNumber());
      }
    }

    for (auto edge = subNode->getExceptionSuccessors().begin();
         edge != subNode->getExceptionSuccessors().end(); ++edge) {
      TR_StructureSubGraphNode *succ =
          toStructureSubGraphNode((*edge)->getTo());
      if (!isExitEdge(*edge)) {
        TR::CFGEdge::createExceptionEdge(
            correspondingSubNode, correspondingSubNodes[succ->getNumber()],
            trMemory());
      } else {
        clonedRegionStructure->addExitEdge(correspondingSubNode,
                                           succ->getNumber(), 1);
  ...
  while (currInductionVariable) {
    TR_InductionVariable *currClonedInductionVariable =
        new (structureRegion) TR_InductionVariable();
    memcpy(currClonedInductionVariable, currInductionVariable,
           sizeof(TR_InductionVariable));
    if (!prevClonedInductionVariable)
      clonedRegionStructure->addInductionVariable(currClonedInductionVariable);
    else
      clonedRegionStructure->addAfterInductionVariable(
  ...
  for (edge = ei.getFirst(); edge; edge = ei.getNext()) {
    if (isExitEdge(edge)) {
      ...
      if (!seenExitNodes.find(exitNode)) {
        ...
        TR::Block *correspondingBlock =
            correspondingBlocks[exitNode->getNumber()];
...
List<TR::Block> *TR_RegionStructure::getBlocks(
    List<TR::Block> *blocksInRegion) {
  vcount_t visitCount = comp()->incVisitCount();
  return getBlocks(blocksInRegion, visitCount);
}
...
List<TR::Block> *TR_BlockStructure::getBlocks(List<TR::Block> *blocksInRegion,
  ...
  if (block->getEntry()) {
    ...
    if (nextBlock && block->getStructureOf() != NULL &&
        nextBlock->getStructureOf() &&
        block->getStructureOf()->getParent() ==
            nextBlock->getStructureOf()->getParent() &&
...
void TR_RegionStructure::addEdge(TR::CFGEdge *edge, bool isExceptionEdge) {
  ...
  for (fromNode = si.getCurrent(); fromNode != NULL; fromNode = si.getNext()) {
    ...
    if (fromStruct->contains(from->getStructureOf(), this)) break;
  }
  TR_ASSERT(fromStruct != NULL, "Structure: bad structure when adding an edge");
  TR_ASSERT(fromNode != NULL,
            "Structure: fromNode of the edge not found in the structure");
  ...
  if (fromStruct->asRegion()) {
    ...
    //
    fromStruct->addExternalEdge(from->getStructureOf(), to->getNumber(),
                                isExceptionEdge);
  }
  ...
  while (1) {
    ...
    for (toNode = si.getCurrent(); toNode != NULL; toNode = si.getNext()) {
      ...
      if (toStruct->contains(to->getStructureOf(), this)) break;
    }
    ...
    if (!toNode) {
      if (to == comp()->getFlowGraph()->getEnd() /* &&
        ...
        if (!toStruct)
          toStruct = new ((structureRegion))
              TR_BlockStructure(comp(), to->getNumber(), to);
    ...
    //
    if (toStruct->getNumber() == to->getNumber()) break;
  ...
  if (iter == list->end()) {
    ...
    //
    if (!containsInternalCycles()) checkForInternalCycles();
  }
}

void TR_BlockStructure::addExternalEdge(TR_Structure *from, int32_t toNumber,
                                        bool isExceptionEdge) {}

void TR_RegionStructure::addExternalEdge(TR_Structure *from, int32_t toNumber,
                                         bool isExceptionEdge) {
  ...
  for (fromNode = si.getCurrent(); fromNode != NULL; fromNode = si.getNext()) {
    fromStruct = fromNode->getStructure();
    if (fromStruct->contains(from, this)) break;
  }
...
void TR_RegionStructure::collapseIntoParent() {
  ...
  //
  TR_BitVector regionNodes(comp()->getFlowGraph()->getNextNodeNumber(),
                           trMemory(), stackAlloc);
  ...
  TR_BitVector parentNodes(comp()->getFlowGraph()->getNextNodeNumber(),
                           trMemory(), stackAlloc);
  ...
  for (node = pNodes.getCurrent(); node; node = pNodes.getNext()) {
    ...
    if (node->getNumber() == getNumber()) nodeInParent = node;
  }
  ...
  for (node = subNodes.getCurrent(); node; node = subNodes.getNext()) {
    ...
    for (auto edge = node->getSuccessors().begin();
      ...
      if (regionNodes.get(toNum)) {
        ...
        for (target = pTargets.getCurrent(); target;
             target = pTargets.getNext()) {
          if (target->getNumber() == toNum) {
            ...
            // removed it.
            //
            for (auto pred = target->getPredecessors().begin();
                 pred != target->getPredecessors().end(); ++pred) {
              if ((*pred)->getFrom()->getNumber() == getNumber() &&
                  toStructureSubGraphNode((*pred)->getFrom())->getStructure() ==
                      this) {
        ...
        for (target = pTargets.getCurrent(); target;
          ...
          if (target->getNumber() == toNum) {
            for (auto pred = target->getExceptionPredecessors().begin();
              ...
              if (((*pred)->getFrom()->getNumber() == getNumber()) &&
                  (toStructureSubGraphNode((*pred)->getFrom())
                       ->getStructure() == this)) {
  ...
  //
  for (auto edge = nodeInParent->getPredecessors().begin();
       edge != nodeInParent->getPredecessors().end(); ++edge)
    (*edge)->setTo(getEntry());
  ...
  //
  for (auto edge = nodeInParent->getSuccessors().begin();
       edge != nodeInParent->getSuccessors().end();) {
  ...
  //
  if (parent->getEntry() == nodeInParent) parent->setEntry(getEntry());
...
TR::CFGEdge *TR_RegionStructure::addExitEdge(TR_StructureSubGraphNode *from,
                                             int32_t to, bool isExceptionEdge,
                                             TR::CFGEdge *origEdge) {
  ListIterator<TR::CFGEdge> it(&getExitEdges());
  TR::CFGEdge *cursor;
  for (cursor = it.getFirst(); cursor; cursor = it.getNext()) {
    if (cursor->getTo()->getNumber() == to) break;
  }
  ...
  TR::CFGNode *toNode =
      cursor ? cursor->getTo()
             : new (cfg()->structureRegion())
                   TR_StructureSubGraphNode(to, cfg()->structureRegion());
...
TR_StructureSubGraphNode *TR_StructureSubGraphNode::create(
    int32_t num, TR_RegionStructure *region) {
  ListIterator<TR::CFGEdge> it(&region->getExitEdges());
  TR::CFGEdge *edge;
  for (edge = it.getFirst(); edge; edge = it.getNext()) {
    if (edge->getTo()->getNumber() == num) break;
  }
  if (edge) return edge->getTo()->asStructureSubGraphNode();

  return new (region->cfg()->structureRegion())
      TR_StructureSubGraphNode(num, region->cfg()->structureRegion());
}

TR_StructureSubGraphNode *TR_StructureSubGraphNode::asStructureSubGraphNode() {
  return this;
}

void TR_BlockStructure::checkStructure(TR_BitVector *_blockNumbers) {
  TR_ASSERT_FATAL(
      this->getNumber() == _block->getNumber(),
      "Number of BlockStructure is NOT the same as that of the block");
  TR_ASSERT_FATAL(_blockNumbers->get(this->getNumber()) == 0,
                  "Structure, Two blocks with the same number");
...
void TR_RegionStructure::checkStructure(TR_BitVector *_blockNumbers) {
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    ...
    for (auto edge = node->getPredecessors().begin();
      ...
      TR_ASSERT_FATAL(isConsistent,
                      "predecessor of this subGraph node not not contain this "
                      "node in its successors");
    }
    for (auto edge = node->getExceptionPredecessors().begin();
         edge != node->getExceptionPredecessors().end(); ++edge) {
      pred = toStructureSubGraphNode((*edge)->getFrom());
      ...
      TR_ASSERT_FATAL(isConsistent,
                      "exception predecessor of this subGraph node not not "
                      "contain this node in its exception successors");
    }
    ...
    for (auto edge = node->getSuccessors().begin();
      ...
      isConsistent = false;
      for (auto predEdge = succ->getPredecessors().begin();
           predEdge != succ->getPredecessors().end(); ++predEdge) {
        if (*predEdge == *edge) {
          isConsistent = true;
          break;
        }
      ...
      TR_ASSERT_FATAL(isConsistent,
                      "successor of this subGraph node not not contain this "
    ...
    for (auto edge = node->getExceptionSuccessors().begin();
      ...
      TR_ASSERT_FATAL(isConsistent,
                      "exception successor of this subGraph node not not "
                      "contain this node in its exception predecessors");
    }
    ...
    if (node->getStructure()->asRegion() != NULL) {
      ...
      for (auto edge = succs.begin(); edge != succs.end(); ++edge) {
        ...
        for (exit = exits.getFirst(); exit != NULL; exit = exits.getNext()) {
          if (exit->getTo()->getNumber() == num) break;
        }
      ...
      // All exception successors of node must have corresponding exception
      // exits
      ...
      for (auto exit = exits.getFirst(); exit != NULL; exit = exits.getNext()) {
        ...
        for (; found != succs.end(); ++found) {
          if ((*found)->getTo()->getNumber() == num) break;
        }
  ...
  for (edge = exitEdges.getFirst(); edge; edge = exitEdges.getNext()) {
    ...
    for (seenNode = exitNodesIt.getFirst(); seenNode;
         seenNode = exitNodesIt.getNext()) {
      if ((seenNode->getNumber() == toNode->getNumber()) &&
          (seenNode != toNode))
        TR_ASSERT_FATAL(
            0,
            "Exit edges to the same node number %d in region %p must have a "
            "unique structure subgraph node (node1 %p node2 %p)\n",
            seenNode->getNumber(), this, seenNode, toNode);
    }
    if (!exitNodes.find(toNode)) exitNodes.add(toNode);
  }
}
...
bool TR_RegionStructure::markStructuresWithImproperRegions() {
  ...
  TR_RegionStructure::Cursor si(*this);
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    if (node->getStructure()->markStructuresWithImproperRegions()) b = true;
  }

  if (containsInternalCycles()) b = true;
...
TR::Block *TR_BlockStructure::setBlock(TR::Block *b) {
  ...
  if (old) {
    if (old->isEntryOfShortRunningLoop()) setIsEntryOfShortRunningLoop();
  }
  return b;
}

bool TR_BlockStructure::changeContinueLoopsToNestedLoops(
    TR_RegionStructure *root) {
  return false;
}

bool TR_RegionStructure::changeContinueLoopsToNestedLoops(
    TR_RegionStructure *root) {
  ...
  if (isNaturalLoop()) {
    ...
    for (auto nextEdge = entryBlock->getPredecessors().begin();
      ...
      if (contains(pred->getStructureOf(), getParent())) {
        // special case backedge: if back-edge is a fall-through, don't do
        // anything to this loop
        ...
        if (traceMyself)
          diagnostic("found incoming edge from %d\n",
                     pred->asBlock()->getNumber());
        TR_ASSERT(
            (*nextEdge)->getTo()->asBlock() == entryBlock,
    ...
    if (numNewHeaders == 1 &&
        performTransformation(
            comp(),
            "%s transforming continues in loop %d to %d new nested loops\n",
            OPT_DETAILS, getEntryBlock()->asBlock()->getNumber(),
            numNewHeaders)) {
      TR::Block *newHeader = NULL;
      TR::Block *prevHeader = entryBlock;
      ListIterator<TR::CFGEdge> seIterator(&splitEdges);
      // for each split edge, change the branch to point to one of the new
      // header blocks
      for (TR::CFGEdge *splitEdge = seIterator.getFirst(); splitEdge;
           splitEdge = seIterator.getNext()) {
        TR::Block *newHeader = TR::Block::createEmptyBlock(
            entryBlock->getEntry()->getNode(), comp(),
        ...
        if (traceMyself)
          diagnostic("Rerouting edge %d->old header %d to new header %d\n",
                     splitEdge->getFrom()->asBlock()->getNumber(),
                     splitEdge->getTo()->asBlock()->getNumber(),
                     newHeader->getNumber());

        // change destination at the end of splitEdge->getFrom()->asBlock() to
        // branch to newHeader rather than entryBlock, updating edges as needed
        TR::Block::redirectFlowToNewDestination(comp(), splitEdge, newHeader,
                                                false);

        // paste trees for this header ahead of prevHeader
        TR::TreeTop *beforePrevHeader =
            prevHeader->getEntry()->getPrevTreeTop();
      ...
      // for each incoming edge to the original header, redirect to the
      // outermost new header block note: newHeader here already holds the
      // header for the outermost loop
      ListIterator<TR::CFGEdge> ieIterator(&incomingEdges);
      for (TR::CFGEdge *incomingEdge = ieIterator.getFirst(); incomingEdge;
           incomingEdge = ieIterator.getNext()) {
        if (traceMyself) {
          diagnostic("Incoming [%p] edge %d->%d\n", incomingEdge,
                     incomingEdge->getFrom()->asBlock()->getNumber(),
                     incomingEdge->getTo()->asBlock()->getNumber());
          diagnostic("\tassuming it should be ->%d\n", entryBlock->getNumber());
          diagnostic("\tredirecting to ->%d\n", newHeader->getNumber());
        }

        TR::Block::redirectFlowToNewDestination(comp(), incomingEdge, newHeader,
                                                false);
      }
...
//               reason: we may merge block 2 to block 3, but first block should
//               be numbered as 2
void TR_Structure::mergeBlocks(TR::Block *merged, TR::Block *mergedInto) {
  ...
  //
  int16_t newFrequency =
      std::max(merged->getFrequency(), mergedInto->getFrequency());
  merged->setFrequency(newFrequency);

  mergeInto(merged, mergedInto);

  // Make the block structure for the merged into block represent the merged
  // block.
  //
  ...
  TR_ASSERT(s->getNumber() == merged->getNumber(),
            "Structure: bad structure when merging blocks");
  if (debug("dumpStructure"))
...
void TR_RegionStructure::mergeInto(TR::Block *merged, TR::Block *mergedInto) {
  ...
  if (merged->isCatchBlock()) mergedIsCatch = true;
  ...
  for (fromNode = si.getCurrent(); fromNode != NULL; fromNode = si.getNext()) {
    fromStruct = fromNode->getStructure();
    if (fromStruct->contains(merged->getStructureOf(), this)) break;
  }
  ...
  if (fromStruct->contains(mergedInto->getStructureOf(), this)) {
    ...
    if (fromStruct->getKind() == TR_Structure::Region &&
        fromStruct->getNumber() == mergedIntoNum) {
  ...
  si.reset();
  for (toNode = si.getCurrent(); toNode != NULL; toNode = si.getNext()) {
    if (toNode->getStructure()->getNumber() == toBlockNum) break;
  }
  ...
  if (fromNode == getEntry()) setEntry(toNode);
  ...
  for (edge = exitEdges.getFirst(); edge;) {
    ...
    if (edge->getFrom() == fromNode) removeEdge(edge, true);
    edge = nextEdge;
  }
  ...
  if (!toNode->getStructure()->asBlock()) {
    for (auto edge = toNode->getSuccessors().begin();
         edge != toNode->getSuccessors().end();) {
    ...
    for (auto edge = toNode->getExceptionSuccessors().begin();
         edge != toNode->getExceptionSuccessors().end();) {
      if ((*edge)->getTo() == toNode)
        removeEdge(*(edge++), true);
      else
        ++edge;
  ...
  //
  if (fromNode->getStructure()->getParent()) removeSubNode(fromNode);
}
...
void TR_RegionStructure::removeMergedBlock(TR::Block *merged,
                                           TR::Block *mergedInto) {
  // Find the subnode containing the block to be removed
  //
  TR_StructureSubGraphNode *node;
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    s = node->getStructure();
    if (s->contains(merged->getStructureOf(), this)) break;
  }
  ...
  if (bs == NULL) {
    ...
    //
    for (edge = _exitEdges.getListHead(); edge != NULL;
         edge = edge->getNextElement()) {
...
void TR_BlockStructure::collectCFGEdgesTo(int32_t toNum,
                                          List<TR::CFGEdge> *cfgEdges) {
  TR::CFGEdge *edge;
  TR_SuccessorIterator sit(getBlock());
  for (edge = sit.getFirst(); edge; edge = sit.getNext()) {
    TR::CFGNode *succ = edge->getTo();
    if (succ->getNumber() == toNum) cfgEdges->add(edge);
  }
}

void TR_RegionStructure::collectCFGEdgesTo(int32_t toNum,
                                           List<TR::CFGEdge> *cfgEdges) {
  ListElement<TR::CFGEdge> *edge;
  for (edge = _exitEdges.getListHead(); edge != NULL;
       edge = edge->getNextElement()) {
    if (edge->getData()->getTo()->getNumber() == toNum)
      toStructureSubGraphNode(edge->getData()->getFrom())
          ->getStructure()
          ->collectCFGEdgesTo(toNum, cfgEdges);
  }
}
...
void TR_RegionStructure::renumber(int32_t num) {
  ...
  // for (edge = _exitEdges.getListHead(); edge != NULL; prevEdge = edge, edge =
  // nextEdge)
  while (edge) {
    ...
    if (edge->getData()->getTo()->getNumber() == num) {
      ...
      if (std::find(
              edge->getData()->getFrom()->getExceptionSuccessors().begin(),
...
nextEdge)
      ...
      if (edge->getData()->getTo()->getNumber() == num)
         ...
         bool isExceptionEdge = true;
         if
(!edge->getData()->getFrom()->getExceptionSuccessors().find(edge->getData()))
            isExceptionEdge = false;
...
bool TR_RegionStructure::renumberRecursively(int32_t origNum, int32_t num) {
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    s = node->getStructure();
    if (s->renumberRecursively(origNum, num)) node->setNumber(num);
  }
...
void TR_RegionStructure::updateInvariantSymbols(TR::Node *node,
                                                vcount_t visitCount) {
  ...
  if (opCode.hasSymbolReference()) {
    ...
    if (opCode.isResolveCheck()) {
      ...
      childSymRef->getUseDefAliases().getAliasesAndSubtractFrom(
          *_invariantSymbols);
    }

    if (!opCode.isLoadVar() && (opCode.getOpCodeValue() != TR::loadaddr)) {
      if (!opCode.isCheck()) {
        symReference->getUseDefAliases().getAliasesAndSubtractFrom(
            *_invariantSymbols);
      }
...
void TR_RegionStructure::computeInvariantExpressions() {
  ...
  _invariantExpressions = new (trStackMemory())
      TR_BitVector(nodeCount, trMemory(), stackAlloc, growable);
...
void TR_RegionStructure::updateInvariantExpressions(TR::Node *node,
                                                    vcount_t visitCount) {
  if (node->getVisitCount() == visitCount) return;
...
bool TR_RegionStructure::isExprTreeInvariant(TR::Node *expr) {
  if (!_invariantSymbols) computeInvariantSymbols();
...
bool TR_RegionStructure::isSubtreeInvariant(TR::Node *node,
                                            vcount_t visitCount) {
  if (node->getVisitCount() == visitCount) return true;

  if (node->getOpCode().isCall()) return false;

  if (node->getOpCode().hasSymbolReference()) {
    if (!_invariantSymbols->get(
  ...
  int32_t i;
  for (i = 0; i < node->getNumChildren(); i++) {
    if (!isSubtreeInvariant(node->getChild(i), visitCount)) return false;
  }
...
void TR_BlockStructure::hoistInvariantsOutOfNestedLoops(
    TR_LocalTransparency *transparencyInfo, TR_BitVector **optSetInfo,
    bool insideLoop, TR_BlockStructure *outerLoopInvariantBlock,
...
void TR_RegionStructure::hoistInvariantsOutOfNestedLoops(
    TR_LocalTransparency *transparencyInfo, TR_BitVector **optSetInfo,
    bool insideLoop, TR_BlockStructure *outerLoopInvariantBlock,
    TR_RegionStructure *outerLoop, int32_t bitVectorSize) {
  bool isInvariantLoop = false;
  ...
  if (isCanonicalizedLoop() && getEntry()->getSuccessors().size() == 2) {
    for (auto succ = getEntry()->getSuccessors().begin();
         succ != getEntry()->getSuccessors().end(); ++succ) {
    ...
    if (loopInvariantBlock) {
      if (loopInvariantNode->getSuccessors().size() == 1) {
        loopStructure =
            ((TR_StructureSubGraphNode *)loopInvariantNode->getSuccessors()
                 .front()
                 ->getTo())
                ->getStructure()
                ->asRegion();
    ...
    if (isInvariantLoop) {
      ...
      for (node = si.getCurrent(); node != NULL; node = si.getNext())
        node->getStructure()->hoistInvariantsOutOfNestedLoops(
            transparencyInfo, optSetInfo, true, loopInvariantBlock,
            loopStructure, bitVectorSize);

      if (insideLoop) {
        if (!(optSetInfo[loopInvariantBlock->getNumber()])->isEmpty()) {
          TR_BitVector *optMask = new (trStackMemory())
              TR_BitVector(bitVectorSize, trMemory(), stackAlloc);

          TR_BitVectorIterator bvi(
              *(optSetInfo[loopInvariantBlock->getNumber()]));
          int32_t nextOptimalComputation = -1;
          while (bvi.hasMoreElements()) {
            nextOptimalComputation = bvi.getNextElement();
            if (!(optSetInfo[outerLoopInvariantBlock->getNumber()])
                     ->get(nextOptimalComputation)) {
              if (outerLoop->isExpressionTransparentIn(nextOptimalComputation,
                                                       transparencyInfo)) {
                TR::Block *innerLoopInvariantBlock =
                ...
                while (currentTree != exitTree) {
                  if (currentTree->getNode()->getOpCode().isCheck()) {
                    if (currentTree->getNode()->getLocalIndex() ==
                    ...
                    if (currentNode->getSymbolReference()
                            ->getSymbol()
                            ->isAuto()) {
                ...
                dumpOptDetails(
                    comp(),
                    "\nO^O PARTIAL REDUNDANCY ELIMINATION: Hoisting loop "
                    "invariant computation %d over nested loops (lifted from "
                    "inner block_%d to outer block_%d)\n",
  ...
  if (!isInvariantLoop) {
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext())
      node->getStructure()->hoistInvariantsOutOfNestedLoops(
          transparencyInfo, optSetInfo, insideLoop, outerLoopInvariantBlock,
          outerLoop, bitVectorSize);
  }
}

bool TR_BlockStructure::isExpressionTransparentIn(
    int32_t nextOptimalComputation, TR_LocalTransparency *transparencyInfo) {
  TR_LocalTransparency::ContainerType *info =
      transparencyInfo->getAnalysisInfo(getNumber());
  if (info && info->get(nextOptimalComputation)) return true;
...
bool TR_RegionStructure::isExpressionTransparentIn(
  ...
  TR_RegionStructure::Cursor si(*this);
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    if (!node->getStructure()->isExpressionTransparentIn(nextOptimalComputation,
                                                         transparencyInfo))
      return false;
  }
...
TR_InductionVariable *TR_RegionStructure::findMatchingIV(
    TR::SymbolReference *symRef) {
  TR::Symbol *symbol = symRef->getSymbol();
  for (TR_InductionVariable *v = getFirstInductionVariable(); v;
       v = v->getNext()) {
    if (symbol == v->getLocal()) return v;
  }
  return NULL;
}
