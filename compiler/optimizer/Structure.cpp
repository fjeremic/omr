...
void TR_RegionStructure::cleanupAfterEdgeRemoval(TR::CFGNode* node)
{
    ...
    else if ((numSubNodes() == 1) && !isNaturalLoop() && !getEntry()->hasSuccessor(getEntry()) && !getEntry()->hasExceptionSuccessor(getEntry())) {
...
void TR_RegionStructure::cleanupAfterNodeRemoval()
{
    ...
    //
    if ((numSubNodes() == 1) && !isNaturalLoop() && !getEntry()->hasSuccessor(getEntry()) && !getEntry()->hasExceptionSuccessor(getEntry())) {
        TR_RegionStructure::Cursor si(*this);
        TR_ASSERT(si.getFirst() == getEntry(), "bad structure found trying to remove a block");
        if (getParent())
            getParent()->replacePart(this, getEntry()->getStructure());
...
static bool findCycle(TR_StructureSubGraphNode* node, TR_BitVector& regionNodes, TR_BitVector& nodesSeenOnPath, TR_BitVector& nodesCleared, int32_t entryNode)
{
    ...
    for (auto edge = node->getSuccessors().begin(); edge != node->getSuccessors().end(); ++edge) {
        ...
        if (succ->getNumber() != entryNode && regionNodes.get(succ->getNumber()) && findCycle(succ, regionNodes, nodesSeenOnPath, nodesCleared, entryNode))
            return true;
    }
...
void TR_RegionStructure::removeEdgeWithoutCleanup(TR::CFGEdge* edge, bool isExitEdge)
{
    ...
    if (std::find(fromNode->getSuccessors().begin(), fromNode->getSuccessors().end(), edge) != fromNode->getSuccessors().end()) {
        ...
        TR_ASSERT((std::find(toNode->getPredecessors().begin(), toNode->getPredecessors().end(), edge) != toNode->getPredecessors().end()), "bad structure found trying to remove an out edge");
        toNode->getPredecessors().remove(edge);
    } else {
        TR_ASSERT((std::find(fromNode->getExceptionSuccessors().begin(), fromNode->getExceptionSuccessors().end(), edge) != fromNode->getExceptionSuccessors().end()), "bad structure found trying to remove an out edge");
        fromNode->getExceptionSuccessors().remove(edge);
        TR_ASSERT((std::find(toNode->getExceptionPredecessors().begin(), toNode->getExceptionPredecessors().end(), edge) != toNode->getExceptionPredecessors().end()), "bad structure found trying to remove an out edge");
...
List<TR::Block>* TR_BlockStructure::getBlocks(List<TR::Block>* blocksInRegion, vcount_t visitCount)
{
    ...
    if (block->getEntry()) {
        ...
        if (nextBlock && block->getStructureOf() != NULL && nextBlock->getStructureOf() && block->getStructureOf()->getParent() == nextBlock->getStructureOf()->getParent() && nextBlock->getVisitCount() != visitCount) {
                ...
                for (target = pTargets.getCurrent(); target; target = pTargets.getNext()) {
                    if (target->getNumber() == toNum) {
                        ...
                        for (auto pred = target->getPredecessors().begin(); pred != target->getPredecessors().end(); ++pred) {
                            if ((*pred)->getFrom()->getNumber() == getNumber() && toStructureSubGraphNode((*pred)->getFrom())->getStructure() == this) {
...
TR_RegionStructure::addExitEdge(TR_StructureSubGraphNode* from, int32_t to, bool isExceptionEdge, TR::CFGEdge* origEdge)
{
    ...
    if (origEdge) {
        ...
        edge = isExceptionEdge ? TR::CFGEdge::createExceptionEdge(from, toNode, trMemory()) : TR::CFGEdge::createEdge(from, toNode, trMemory());
    }
    _exitEdges.add(edge);
    return edge;
}
