...
private:
  ...
  void moveOutgoingEdgeToParent(TR_RegionStructure* region, TR_RegionStructure* parent, TR_StructureSubGraphNode* node,
...
void TR_RegionStructure::ExitExtraction::moveOutgoingEdgeToParent(TR_RegionStructure* const region,
  TR_RegionStructure* const parent, TR_StructureSubGraphNode* const node, TR::CFGEdge* const edge, bool isExceptionEdge)
{
  ...
  if (redirect == NULL) {
    ...
    if (_trace) {
      traceMsg(
        _comp, "parent region contains %d - created internal %sedge\n", tgtNum, isExceptionEdge ? "exception " : "");
    }
  }
}
...
static bool findCycle(TR_StructureSubGraphNode* node, TR_BitVector& regionNodes, TR_BitVector& nodesSeenOnPath,
  TR_BitVector& nodesCleared, int32_t entryNode)
{
  ...
  List<TR_Structure>* correspondingWhileLoops)
{
  ...
  TR_BlockStructure* clonedBlockStructure
    = new (cfg()->structureRegion()) TR_BlockStructure(comp(), correspondingBlock->getNumber(), correspondingBlock);
  ...
  List<TR_Structure>* correspondingWhileLoops)
{
  ...
  TR_RegionStructure* clonedRegionStructure
    = new (structureRegion) TR_RegionStructure(comp(), correspondingBlocks[getNumber()]->getNumber());
  ...
  for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
    ...
    TR_Structure* clonedSubStruct
      = subStruct->cloneStructure(correspondingBlocks, correspondingSubNodes, whileLoops, correspondingWhileLoops);
  ...
  TR_StructureSubGraphNode* from, int32_t to, bool isExceptionEdge, TR::CFGEdge* origEdge)
{
  ...
  TR::CFGEdge* edge;
  TR::CFGNode* toNode
    = cursor ? cursor->getTo() : new (cfg()->structureRegion()) TR_StructureSubGraphNode(to, cfg()->structureRegion());
...
void TR_RegionStructure::checkStructure(TR_BitVector* _blockNumbers)
{
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    ...
    for (auto edge = node->getExceptionPredecessors().begin(); edge != node->getExceptionPredecessors().end(); ++edge) {
      pred = toStructureSubGraphNode((*edge)->getFrom());
      isConsistent = false;
      for (auto succEdge = pred->getExceptionSuccessors().begin(); succEdge != pred->getExceptionSuccessors().end();
           ++succEdge) {
        if ((*succEdge) == (*edge)) {
          isConsistent = true;
          break;
    ...
    for (auto edge = node->getExceptionSuccessors().begin(); edge != node->getExceptionSuccessors().end(); ++edge) {
      ...
      isConsistent = false;
      TR::CFGEdge* predEdge;
      for (auto predEdge = succ->getExceptionPredecessors().begin(); predEdge != succ->getExceptionPredecessors().end();
           ++predEdge) {
        if ((*predEdge) == *edge) {
          isConsistent = true;
          break;
  ...
  for (edge = exitEdges.getFirst(); edge; edge = exitEdges.getNext()) {
    ...
    for (seenNode = exitNodesIt.getFirst(); seenNode; seenNode = exitNodesIt.getNext()) {
      if ((seenNode->getNumber() == toNode->getNumber()) && (seenNode != toNode))
        TR_ASSERT_FATAL(0,
          "Exit edges to the same node number %d in region %p must have a unique structure subgraph node (node1 %p "
          "node2 %p)\n",
  ...
  if (isNaturalLoop()) {
    ...
    if (numNewHeaders == 1
      && performTransformation(comp(), "%s transforming continues in loop %d to %d new nested loops\n", OPT_DETAILS,
        getEntryBlock()->asBlock()->getNumber(), numNewHeaders)) {
      ...
      for (TR::CFGEdge* incomingEdge = ieIterator.getFirst(); incomingEdge; incomingEdge = ieIterator.getNext()) {
        if (traceMyself) {
          diagnostic("Incoming [%p] edge %d->%d\n", incomingEdge, incomingEdge->getFrom()->asBlock()->getNumber(),
...
void TR_BlockStructure::hoistInvariantsOutOfNestedLoops(TR_LocalTransparency* transparencyInfo,
  TR_BitVector** optSetInfo, bool insideLoop, TR_BlockStructure* outerLoopInvariantBlock, TR_RegionStructure* outerLoop,
          ...
          while (bvi.hasMoreElements()) {
            ...
            if (!(optSetInfo[outerLoopInvariantBlock->getNumber()])->get(nextOptimalComputation)) {
              if (outerLoop->isExpressionTransparentIn(nextOptimalComputation, transparencyInfo)) {
                ...
                dumpOptDetails(comp(),
                  "\nO^O PARTIAL REDUNDANCY ELIMINATION: Hoisting loop invariant computation %d over nested loops "
                  "(lifted from inner block_%d to outer block_%d)\n",
                  nextOptimalComputation, loopInvariantBlock->getNumber(), outerLoopInvariantBlock->getNumber());
