...
struct TR_PairedSymbols {
  ...
  TR_PairedSymbols(TR::SymbolReference *symRef1, TR::SymbolReference *symRef2)
      : _symRef1(symRef1), _symRef2(symRef2) {}
...
struct TR_SymRefCandidatePair {
  ...
  TR_SymRefCandidatePair(TR::SymbolReference *symRef, TR_RegisterCandidate *rc)
      : _symRef(symRef), _rc(rc) {}
...
struct TR_NodeTriple {
  ...
  TR_NodeTriple(TR::Node *node1, TR::Node *node2, TR::Node *node3,
                int32_t childNum)
      : _node1(node1), _node2(node2), _node3(node3), _childNum(childNum) {}
...
public:
  ...
  typedef TR::typed_allocator<std::pair<uint32_t const, TR_RegisterCandidate *>,
                              TR::Region &>
      SymRefCandidateMapAllocator;
  typedef std::less<uint32_t> SymRefCandidateMapComparator;
  typedef std::map<uint32_t, TR_RegisterCandidate *,
                   SymRefCandidateMapComparator, SymRefCandidateMapAllocator>
      SymRefCandidateMap;

  void splitLiveRanges();
  void splitLiveRanges(TR_StructureSubGraphNode *structureNode);
  void replaceAutosUsedIn(TR::TreeTop *currentTree, TR::Node *node,
                          TR::Node *parent, TR::Block *block,
  ...
  void placeStoresInLoopExits(TR::Node *node, TR_StructureSubGraphNode *loop,
                              List<TR::Block> *blocksInLoop,
                              TR::SymbolReference *orig,
                              TR::SymbolReference *replacement);
  TR_SymRefCandidatePair *
  splitAndFixPreHeader(TR::SymbolReference *symRef,
                       TR_SymRefCandidatePair **correspondingSymRefs,
                       TR::Block *loopInvariantBlock, TR::Node *node);
  void fixExitsAfterSplit(TR::SymbolReference *symRef,
                          TR_SymRefCandidatePair *correspondingSymRefCandidate,
                          TR_SymRefCandidatePair **correspondingSymRefs,
                          TR::Block *loopInvariantBlock,
                          List<TR::Block> *blocksInLoop, TR::Node *node,
                          SymRefCandidateMap &registerCandidates,
                          TR_StructureSubGraphNode *loop,
                          TR_BitVector *replacedAutosInCurrentLoop,
                          TR::SymbolReference *origSymRef);
  void appendStoreToBlock(TR::SymbolReference *storeSymRef,
                          TR::SymbolReference *loadSymRef, TR::Block *block,
                          TR::Node *node);
  void prependStoreToBlock(TR::SymbolReference *storeSymRef,
                           TR::SymbolReference *loadSymRef, TR::Block *block,
                           TR::Node *node);
...
public:
  ...
  TR::Node *resolveTypeMismatch(TR::DataType inputOldType, TR::Node *oldNode,
                                TR::Node *newNode);
...
private:
  ...
  void findLoopsAndCorrespondingAutos(TR_StructureSubGraphNode *, vcount_t,
                                      SymRefCandidateMap &);
  void
  findLoopsAndAutosNoStructureInfo(vcount_t visitCount,
                                   TR_RegisterCandidate **registerCandidates);
  void initializeControlFlowInfo();
  void markAutosUsedIn(TR::Node *, TR::Node *, TR::Node *, TR::Node **,
                       TR::Block *, List<TR::Block> *, vcount_t, int32_t,
                       SymRefCandidateMap &, TR_BitVector *, TR_BitVector *,
                       bool);
  void signExtendAllDefNodes(TR::Node *, List<TR::Node> *);
  void findSymsUsedInIndirectAccesses(TR::Node *, TR_BitVector *,
                                      TR_BitVector *, bool);

  void
  offerAllAutosAndRegisterParmAsCandidates(TR::Block **, int32_t,
                                           bool onlySelectedCandidates = false);
  void offerAllFPAutosAndParmsAsCandidates(TR::Block **, int32_t);

  bool isDependentStore(TR::Node *, const TR_UseDefInfo::BitVector &,
                        TR::SymbolReference *, bool *);
  ...
  void transformNode(TR::Node *, TR::Node *, int32_t, TR::TreeTop *,
                     TR::Block *&, TR_Array<TR_GlobalRegister> &,
                     TR_NodeMappings *);
  TR_GlobalRegister *
  getGlobalRegister(TR::Symbol *, TR_Array<TR_GlobalRegister> &, TR::Block *);
  TR_GlobalRegister *getGlobalRegisterWithoutChangingCurrentCandidate(
      TR::Symbol *, TR_Array<TR_GlobalRegister> &, TR::Block *);
  void transformBlockExit(TR::TreeTop *, TR::Node *, TR::Block *,
                          TR_Array<TR_GlobalRegister> &, TR::Block *);
  void transformMultiWayBranch(TR::TreeTop *, TR::Node *, TR::Block *,
                               TR_Array<TR_GlobalRegister> &,
                               bool regStarTransformDone = false);
  void addCandidateReloadsToEntry(TR::TreeTop *, TR_Array<TR_GlobalRegister> &,
                                  TR::Block *);
  void addRegLoadsToEntry(TR::TreeTop *, TR_Array<TR_GlobalRegister> &,
                          TR::Block *);
  void addGlRegDepToExit(TR_Array<TR::Node *> &, TR::Node *,
                         TR_Array<TR_GlobalRegister> &, TR::Block *);
  void
  addStoresForCatchBlockLoads(TR::TreeTop *appendPoint,
                              TR_Array<TR_GlobalRegister> &extBlockRegisters,
                              TR::Block *throwingBlock);
  TR::TreeTop *findPrevTreeTop(TR::TreeTop *&, TR::Node *&, TR::Block *,
                               TR::Block *);
  void prepareForBlockExit(TR::TreeTop *&, TR::Node *&, TR::Block *,
                           TR_Array<TR_GlobalRegister> &, TR::Block *,
                           TR_Array<TR::Node *> &);
  bool markCandidateForReloadInSuccessors(int32_t, TR_GlobalRegister *,
                                          TR_GlobalRegister *, TR::Block *,
                                          bool);
  void reloadNonRegStarVariables(TR::TreeTop *, TR::Node *, TR::Block *, bool);

  bool registerIsLiveAcrossEdge(TR::TreeTop *, TR::Node *, TR::Block *,
                                TR_GlobalRegister *, TR::Block *&, int32_t);
  TR::Block *createNewSuccessorBlock(TR::Block *, TR::Block *, TR::TreeTop *,
                                     TR::Node *, TR_RegisterCandidate *rc);
  ...
  CS2::HashTable<int32_t, TR::SymbolReference *, TR::Allocator>
      _defIndexToTempMap;
  ...
  TR_PairedSymbols *findOrCreatePairedSymbols(TR::SymbolReference *symRef1,
                                              TR::SymbolReference *symRef2);
  TR_PairedSymbols *findPairedSymbols(TR::SymbolReference *symRef1,
                                      TR::SymbolReference *symRef2);
  TR_ScratchList<TR_PairedSymbols> *getPairedSymbols() {
    return &_pairedSymbols;
  }
  ...
  void
  findLoopsAndCreateAutosForSignExt(TR_StructureSubGraphNode *structureNode,
                                    vcount_t visitCount);
  void createStoresForSignExt(TR::Node *node, TR::Node *parent,
                              TR::Node *grandParent, TR::TreeTop *treetop,
  ...
  /*
  void splitLiveRanges();
  void splitLiveRanges(TR_StructureSubGraphNode *structureNode, vcount_t
  visitCount, TR_RegisterCandidate **registerCandidates); void
  replaceAutosUsedIn(TR::Node *node, TR::Node *parent, TR::Block * block,
  List<TR::Block> *blocksInLoop, vcount_t visitCount, int32_t
  executionFrequency, TR_RegisterCandidate **registerCandidates,
  TR_SymRefCandidatePair **correspondingSymRefs, TR_BitVector
  *replacedAutosInCurrentLoop, TR_StructureSubGraphNode *loop, TR::Block
  *loopInvariantBlock); void placeStoresInPreHeaderAndInLoopExits(TR::Node
  *node, TR_StructureSubGraphNode *loop, TR::Block *loopInvariantBlock,
  List<TR::Block> *blocksInLoop, TR::SymbolReference *orig, TR::SymbolReference
  *replacement); void appendStoreToBlock(TR::SymbolReference *storeSymRef,
