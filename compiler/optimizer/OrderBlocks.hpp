...
class TR_BlockOrderingOptimization : public TR::Optimization {
 public:
  void dumpBlockOrdering(
      TR::TreeTop *tt, char *title = NULL);  // This is called from other opts!
 ...
 protected:
  ...
  TR::Block *insertGotoFallThroughBlock(TR::TreeTop *fallThroughTT,
                                        TR::Node *node, TR::CFGNode *prevBlock,
                                        TR::CFGNode *origSucc,
...
class TR_OrderBlocks : public TR_BlockOrderingOptimization {
 ...
 public:
  TR_OrderBlocks(TR::OptimizationManager *manager,
                 bool beforeExtension = false);
  ...
  void doPeepHoleOptimizations(bool before, bool after) {
    if (before) _doPeepHoleOptimizationsBefore = true;
    if (after) _doPeepHoleOptimizationsAfter = true;
  }
 ...
 private:
  bool needBetterChoice(TR::CFG *cfg, TR::CFGNode *block,
  ...
  TR::CFGNode *findSuitablePathInList(List<TR::CFGNode> &list,
                                      TR::CFGNode *prevBlock);
  ...
  bool candidateIsBetterSuccessorThanBest(TR::CFGEdge *candidateEdge,
                                          TR::CFGEdge *currentEdge);
  TR::CFGNode *chooseBestFallThroughSuccessor(TR::CFG *cfg, TR::CFGNode *block,
                                              int32_t &);
  void addToOrderedBlockList(TR::CFGNode *block, TR_BlockList &list,
                             bool useNumber);
  void removeFromOrderedBlockLists(TR::CFGNode *block);
  void addRemainingSuccessorsToList(TR::CFGNode *block,
                                    TR::CFGNode *excludeBlock);

  bool peepHoleGotoToFollowing(TR::CFG *cfg, TR::Block *block,
                               TR::Block *followingBlock, char *title);
  bool peepHoleGotoToGoto(TR::CFG *cfg, TR::Block *block, TR::Node *gotoNode,
                          TR::Block *destOfGoto, char *title,
                          TR::BitVector &skippedGotoBlocks);
  bool peepHoleGotoToEmpty(TR::CFG *cfg, TR::Block *block, TR::Node *gotoNode,
                           TR::Block *destOfGoto, char *title);
  void peepHoleGotoBlock(TR::CFG *cfg, TR::Block *block, char *title);
  void removeRedundantBranch(TR::CFG *cfg, TR::Block *block,
                             TR::Node *branchNode, TR::Block *takenBlock);
  void peepHoleBranchBlock(TR::CFG *cfg, TR::Block *block, char *title);
  void peepHoleBranchAroundSingleGoto(TR::CFG *cfg, TR::Block *block,
                                      char *title);
  bool peepHoleBranchToFollowing(TR::CFG *cfg, TR::Block *block,
                                 TR::Block *followingBlock, char *title);
  bool peepHoleBranchToLoopHeader(TR::CFG *cfg, TR::Block *block,
                                  TR::Block *fallThrough, TR::Block *dest,
                                  char *title);
  void removeEmptyBlock(TR::CFG *cfg, TR::Block *block, char *title);
  bool doPeepHoleBlockCorrections(TR::Block *block, char *title);
  void addRemainingSuccessorsToListHWProfile(TR::CFGNode *block,
                                             TR::CFGNode *excludeBlock);
 ...
 * in the method without changing the CFG in any way. This is more of a RAS
 * pass.
 */
...
class TR_BlockShuffling : public TR_BlockOrderingOptimization {
 ...
 private:
  ...
  void riffle(
      TR::Block **blocks);  // Break into two parts and interleave them
                            // randomly.  Blocks generally end up in the same
                            // order, but with cold interleaved with hot.
