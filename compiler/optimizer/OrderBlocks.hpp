...
public:
  ...
  void dontReorderBlocks()
  {
    _reorderBlocks = false;
  }
  void extendBlocks()
  {
    _extendBlocks = true;
  }
  void dontExtendBlocks()
  {
    _extendBlocks = false;
  }
  ...
  {
    _needInvalidateStructure = true;
  }
  bool needInvalidateStructure()
  {
    return _needInvalidateStructure;
  }
...
private:
  ...
  bool peepHoleGotoToGoto(TR::CFG* cfg, TR::Block* block, TR::Node* gotoNode, TR::Block* destOfGoto, char* title,
...
private:
  ...
  void riffle(TR::Block** blocks); // Break into two parts and interleave them randomly.  Blocks generally end up in the
                                   // same order, but with cold interleaved with hot.
