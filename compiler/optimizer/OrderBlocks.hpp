...
private:
  bool needBetterChoice(TR::CFG* cfg,
                        TR::CFGNode* block,
                        TR::CFGNode* prevBlock);
  ...
  TR::CFGNode* chooseBestFallThroughSuccessor(TR::CFG* cfg,
                                              TR::CFGNode* block,
                                              int32_t&);
  void addToOrderedBlockList(TR::CFGNode* block,
                             TR_BlockList& list,
                             bool useNumber);
  ...
  bool peepHoleGotoToFollowing(TR::CFG* cfg,
                               TR::Block* block,
                               TR::Block* followingBlock,
                               char* title);
  bool peepHoleGotoToGoto(TR::CFG* cfg,
                          TR::Block* block,
                          TR::Node* gotoNode,
                          TR::Block* destOfGoto,
                          char* title,
                          TR::BitVector& skippedGotoBlocks);
  bool peepHoleGotoToEmpty(TR::CFG* cfg,
                           TR::Block* block,
                           TR::Node* gotoNode,
                           TR::Block* destOfGoto,
                           char* title);
  void peepHoleGotoBlock(TR::CFG* cfg, TR::Block* block, char* title);
  void removeRedundantBranch(TR::CFG* cfg,
                             TR::Block* block,
                             TR::Node* branchNode,
                             TR::Block* takenBlock);
  void peepHoleBranchBlock(TR::CFG* cfg, TR::Block* block, char* title);
  void peepHoleBranchAroundSingleGoto(TR::CFG* cfg,
                                      TR::Block* block,
                                      char* title);
  bool peepHoleBranchToFollowing(TR::CFG* cfg,
                                 TR::Block* block,
                                 TR::Block* followingBlock,
                                 char* title);
  bool peepHoleBranchToLoopHeader(TR::CFG* cfg,
                                  TR::Block* block,
...
private:
  ...
  void riffle(
    TR::Block** blocks); // Break into two parts and interleave them randomly.
                         // Blocks generally end up in the same order, but with
                         // cold interleaved with hot.
