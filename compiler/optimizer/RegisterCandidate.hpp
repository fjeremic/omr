...
class GlobalSet {
 public:
  ...
  TR_BitVector *operator[](uint32_t symRefNum) {
    if (!_collected) collectBlocks();

    auto lookup = _blocksPerAuto.find(symRefNum);
    if (lookup != _blocksPerAuto.end()) return lookup->second;
    return &_EMPTY;
  }
 ...
 private:
  void collectBlocks();
  void collectReferencedAutoSymRefs(TR::Node *node,
                                    TR_BitVector &referencedAutoSymRefs,
                                    TR::NodeChecklist &visited);

  TR::Region &_region;
  TR_BitVector _empty;
  typedef TR::typed_allocator<std::pair<uint32_t const, TR_BitVector *>,
                              TR::Region &>
      RefMapAllocator;
  typedef std::less<uint32_t> RefMapComparator;
  typedef std::map<uint32_t, TR_BitVector *, RefMapComparator, RefMapAllocator>
      RefMap;
...
class TR_RegisterCandidate : public TR_Link<TR_RegisterCandidate> {
 public:
  ...
  class BlockInfo {
    typedef TR::typed_allocator<std::pair<uint32_t const, uint32_t>,
                                TR::Region &>
        InfoMapAllocator;
    typedef std::less<uint32_t> InfoMapComparator;
    typedef std::map<uint32_t, uint32_t, InfoMapComparator, InfoMapAllocator>
        InfoMap;
    InfoMap _blockMap;
   ...
   public:
    ...
    void incNumberOfLoadsAndStores(uint32_t block, uint32_t count) {
      _candidateBlocks.set(block);
      if (count > 0) _blockMap[block] += count;
    }
    ...
    bool find(uint32_t block) { return _candidateBlocks.get(block); }
  ...
  struct LoopInfo : TR_Link<LoopInfo> {
    LoopInfo(TR_Structure *l, int32_t n)
        : _loop(l), _numberOfLoadsAndStores(n) {}
    TR_Structure *_loop;
  ...
  void addAllBlocksInStructure(TR_Structure *, TR::Compilation *,
                               const char *description,
                               vcount_t count = MAX_VCOUNT,
  ...
  void setSplitSymbolReference(TR::SymbolReference *symRef) {
    _splitSymRef = symRef;
  }
  TR::SymbolReference *getRestoreSymbolReference() { return _restoreSymRef; }
  void setRestoreSymbolReference(TR::SymbolReference *symRef) {
    _restoreSymRef = symRef;
  }
  ...
  void setLowGlobalRegisterNumber(TR_GlobalRegisterNumber n) {
    _lowRegNumber = n;
  }
  void setHighGlobalRegisterNumber(TR_GlobalRegisterNumber n) {
    _highRegNumber = n;
  }

  void setWeight(TR::Block **, int32_t *, TR::Compilation *,
                 TR_Array<int32_t> &, TR_Array<int32_t> &, TR_Array<int32_t> &,
                 TR_BitVector *, TR_Array<TR::Block *> &startOfExtendedBB,
                 TR_BitVector &, TR_BitVector &);
  ...
  void processLiveOnEntryBlocks(TR::Block **, int32_t *, TR::Compilation *,
                                TR_Array<int32_t> &, TR_Array<int32_t> &,
                                TR_Array<int32_t> &, TR_BitVector *,
                                TR_Array<TR::Block *> &startOfExtendedBB,
                                bool removeUnusedLoops = false);
  void removeAssignedCandidateFromLoop(TR::Compilation *, TR_Structure *,
                                       int32_t, TR_BitVector *, TR_BitVector *,
                                       bool);

  void extendLiveRangesForLiveOnExit(
  ...
  void recalculateWeight(TR::Block **, int32_t *, TR::Compilation *,
                         ...
                         TR_Array<int32_t> &, TR_BitVector *,
                         TR_Array<TR::Block *> &startOfExtendedBB);
  bool prevBlockTooRegisterConstrained(TR::Compilation *comp, TR::Block *,
                                       TR_Array<int32_t> &, TR_Array<int32_t> &,
                                       TR_Array<int32_t> &);
  ...
  void addLoopWithHole(TR_Structure *s) {
    if (!_loopsWithHoles.find(s)) _loopsWithHoles.add(s);
  }
  ...
  bool initialBlocksWeightComputed() {
    return _flags.testAny(initialBlocksWeightComputedFlag);
  }
  bool getValueModified() { return _flags.testAny(valueModified); }
  bool isHighWordZero() { return _flags.testAny(highWordZero); }
  bool isLiveAcrossExceptionEdge() {
    return _flags.testAny(liveAcrossExceptionEdge);
  }
  ...
  void setLiveAcrossExceptionEdge(bool b) {
    _flags.set(liveAcrossExceptionEdge, b);
  }
 ...
 private:
  ...
  void setInitialBlocksWeightComputed(bool b) {
    _flags.set(initialBlocksWeightComputedFlag, b);
  }
...
class TR_RegisterCandidates {
 public:
  ...
  TR::GlobalSet &getReferencedAutoSymRefs(TR::Region &region) {
    if (_referencedAutoSymRefsInBlock == NULL) {
      ...
      _referencedAutoSymRefsInBlock =
          new (memory) TR::GlobalSet(comp(), region);
    }
    return *_referencedAutoSymRefsInBlock;
  }
  TR_BitVector *getBlocksReferencingSymRef(uint32_t symRefNum) {
    if (_referencedAutoSymRefsInBlock == NULL) return NULL;
    return (*_referencedAutoSymRefsInBlock)[symRefNum];
  }

  bool assign(TR::Block **, int32_t, int32_t &, int32_t &);
  void computeAvailableRegisters(TR_RegisterCandidate *, int32_t, int32_t,
                                 TR::Block **, TR_BitVector *);
 ...
 private:
  bool candidatesOverlap(TR::Block *, TR_RegisterCandidate *,
                         TR_RegisterCandidate *, bool);
  void lookForCandidates(TR::Node *, TR::Symbol *, TR::Symbol *, bool &,
                         bool &);
  bool prioritizeCandidate(TR_RegisterCandidate *, TR_RegisterCandidate *&);
  TR_RegisterCandidate *reprioritizeCandidates(
      TR_RegisterCandidate *, TR::Block **, int32_t *, int32_t, TR::Block *,
      TR::Compilation *, bool reprioritizeFP, bool onlyReprioritizeLongs,
  ...
  bool aliasesPreventAllocation(TR::Compilation *comp,
...
#ifdef TRIM_ASSIGNED_CANDIDATES
  TR_Array<TR_BitVector>
      _availableBlocks;  // blocks available due to empty loops
#endif
 ...
 public:
  ...
  typedef TR::typed_allocator<std::pair<int32_t const, struct coordinates>,
                              TR::Region &>
      CoordinatesAllocator;
  typedef std::less<int32_t> CoordinatesComparator;
  typedef std::map<int32_t, struct coordinates, CoordinatesComparator,
                   CoordinatesAllocator>
      Coordinates;

  typedef TR::typed_allocator<std::pair<uint32_t const, Coordinates *>,
                              TR::Region &>
      ReferenceTableAllocator;
  typedef std::less<uint32_t> ReferenceTableComparator;
  typedef std::map<uint32_t, Coordinates *, ReferenceTableComparator,
                   ReferenceTableAllocator>
      ReferenceTable;
...
class TR_GlobalRegister {
 public:
  ...
  void setRegisterCandidateOnEntry(TR_RegisterCandidate *rc) {
    _rcOnEntry = rc;
  }
  void setRegisterCandidateOnExit(TR_RegisterCandidate *rc) { _rcOnExit = rc; }
  void setCurrentRegisterCandidate(TR_RegisterCandidate *rc, vcount_t,
                                   TR::Block *, int32_t, TR::Compilation *,
                                   bool resetOtherHalfOfLong = true);
  // void setCurrentLongRegisterCandidate(TR_RegisterCandidate * rc,
  // TR_GlobalRegister *otherReg);
  ...
  void setReloadRegisterCandidateOnEntry(TR_RegisterCandidate *rc,
                                         bool flag = true) {
    if (rc == _rcOnEntry) _reloadOnEntry = flag;
  }
  ...
  TR::Node *createStoreFromRegister(vcount_t, TR::TreeTop *, int32_t,
                                    TR::Compilation *,
                                    bool storeUnconditionally = false);
  TR::Node *createStoreToRegister(TR::TreeTop *, TR::Node *, vcount_t,
                                  TR::Compilation *,
 ...
 private:
  ...
  bool _reloadOnEntry;  // reload the register as this is first block with stack
                        // available to reload killed reg
  bool _unavailable;    // Live register killed but stack unavailable making it
                        // impossible to reload
  bool _unavailableResolved;  // register is unavailable but a reload was found
                              // in all successors
