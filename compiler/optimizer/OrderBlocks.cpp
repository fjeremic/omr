...
TR_OrderBlocks::TR_OrderBlocks(TR::OptimizationManager *manager,
                               bool beforeExtension)
...
bool TR_OrderBlocks::needBetterChoice(TR::CFG *cfg, TR::CFGNode *block,
                                      TR::CFGNode *bestSucc) {
  ...
  // if (trace()) traceMsg(comp(), "\t\tneedBetterChoice: block_%d:sec_%d:
  // bestSucc_%d:sec_%d\n",block->getNumber(),
  // block->asBlock()->getSectionNumber(), bestSucc->getNumber(),
  // bestSucc->asBlock()->getSectionNumber());

  // Choose a better one if a cold block follows a hot block
  if (bestSucc != comp()->getStartBlock() &&
      bestSucc->asBlock()->isSuperCold() && !block->asBlock()->isSuperCold()) {
    if (trace())
      traceMsg(comp(),
               "\t\tneedBetterChoice: hot block_%d:cold_%d: follows a cold "
               "block bestSucc_%d:cold_%d\n",
               block->getNumber(), block->asBlock()->isSuperCold(),
               bestSucc->getNumber(), block->asBlock()->isSuperCold());
    return true;
  }
...
// returns TRUE if block absolutely cannot follow prevBlock in any legal
// ordering
bool TR_OrderBlocks::cannotFollowBlock(TR::Block *block, TR::Block *prevBlock) {
  TR::TreeTop *nextTT = NULL;
  // if block extends its predecessor and that's not prevBlock, then we cannot
  // choose block
...
bool TR_OrderBlocks::mustFollowBlock(TR::Block *block, TR::Block *prevBlock) {
  // if block is prevBlock's fall-through successor then we're stuck, we have to
  // choose it
  if (block->isExtensionOfPreviousBlock()) {
    if (block->getEntry()->getPrevTreeTop() == prevBlock->getExit()) {
      if (trace())
...
// Return the first valid block in the list. If HW Profile info exists return
// the first valid block only if there are no valid hot blocks.
TR::CFGNode *TR_OrderBlocks::findSuitablePathInList(List<TR::CFGNode> &list,
                                                    TR::CFGNode *prevBlock) {
  ...
  while (ptr != NULL) {
    ...
    if (block->getVisitCount() == _visitCount) {
      ...
      if (prevBlock == NULL ||
          !cannotFollowBlock(block->asBlock(), prevBlock->asBlock())) {
...
// Return true if a valid block that can follow prevBlock exists in the list.
// Otherwise return false.
bool TR_OrderBlocks::hasValidCandidate(List<TR::CFGNode> &list,
                                       TR::CFGNode *prevBlock) {
  ListElement<TR::CFGNode> *prev = NULL;
  ListElement<TR::CFGNode> *ptr = list.getListHead();
  TR::CFGNode *block = NULL;
...
// simplest implementation for now: later, look at frequencies, nesting depth,
// maybe even distance from predecessors
...
bool TR_OrderBlocks::endPathAtBlock(TR::CFGNode *block, TR::CFGNode *bestSucc,
                                    TR::CFG *cfg) {
  if (block == NULL || bestSucc == NULL) return true;

  if (block->asBlock()->getExit()) {
    TR::Block *nb = block->asBlock()->getNextBlock();

    if (nb && nb->isExtensionOfPreviousBlock()) return false;
  }

  if (_superColdBlockOnly) {
    if (block->asBlock()->isSuperCold() != bestSucc->asBlock()->isSuperCold() &&
        block->asBlock()->getExit() &&
        block->asBlock()->getExit()->getNextTreeTop()) {
      // end path if coldness changes
      if (trace())
        traceMsg(comp(),
                 "\t\tEnd path because coldness changed from block_%d to "
                 "block_%d block->asBlock()->getExit()=%p "
                 "block->asBlock()->getExit()->getNextTreeTop()=%p\n",
                 block->asBlock()->getNumber(),
                 bestSucc->asBlock()->getNumber(), block->asBlock()->getExit(),
                 block->asBlock()->getExit()->getNextTreeTop());
      return true;
    }
  }
  ...
  if (!_changeBlockOrderBasedOnHWProfile &&
    ...
    if (trace())
      traceMsg(
          comp(),
          "\t\tEnd path because coldness changed from block_%d to block_%d \n",
  ...
  if (block->getSuccessors().size() == 1) return false;
  ...
  if (bestSuccStructure == NULL || blockStructure == NULL) {
    if (trace())
      traceMsg(comp(),
               "\t\tEnd path because structure is NULL block_%d=%p to "
               "block_%d=%p \n",
               block->asBlock()->getNumber(), blockStructure,
               bestSucc->asBlock()->getNumber(), bestSuccStructure);
    return true;
  }

  if (bestSuccStructure->getContainingLoop() !=
    ...
    if (bestSuccLoop != blockLoop) {
      if (trace())
        traceMsg(
            comp(),
            "\t\tEnd path because different loop block_%d=%p to block_%d=%p \n",
            block->asBlock()->getNumber(), blockLoop,
            bestSucc->asBlock()->getNumber(), bestSuccLoop);
      return true;
    } else
      return false;
  } else
    return false;
  /// return (bestSuccStructure->getContainingLoop() !=
  /// blockStructure->getContainingLoop());
}
...
bool TR_OrderBlocks::analyseForHazards(TR::CFGNode *block) {
  ...
  if (tt) {
    for (; tt != block->asBlock()->getExit(); tt = tt->getNextTreeTop()) {
      ...
      if (node != NULL) {
        if (node->getOpCode().isCall()) return true;
        if (node->getOpCode().isReturn()) return true;
        if (node->getOpCodeValue() == TR::athrow) return true;
      }
    }
  }
  return false;
...
bool TR_OrderBlocks::isCandidateReallyBetter(TR::CFGEdge *candEdge,
                                             TR::Compilation *comp) {
  ...
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();
    ...
    float numFactor = 1.5;
    for (auto predSuccEdge = predSuccessors.begin();
         predSuccEdge != predSuccessors.end(); ++predSuccEdge) {
      if (_changeBlockOrderBasedOnHWProfile &&
          ((*predSuccEdge)->getFrom()->getVisitCount() == _visitCount))
        continue;

      // traceMsg(comp, "comparing pred edge  %d -> %d freq %d\n",
      // predSuccEdge->getFrom()->getNumber(),
      // predSuccEdge->getTo()->getNumber(), predSuccEdge->getFrequency());
      // traceMsg(comp, "cand edge %d -> %d freq %d\n",
      // candEdge->getFrom()->getNumber(), candEdge->getTo()->getNumber(),
      // candEdge->getFrequency());
      if (((*predSuccEdge) != candEdge) &&
          ((float)(*predSuccEdge)->getFrequency() >
           (float)(numFactor * (float)candEdge->getFrequency())))
...
static bool isCandidateTheHottestSuccessor(TR::CFGEdge *candEdge,
                                           TR::Compilation *comp) {
  /// return true;
  ...
  if (!comp->getFlowGraph()
           ->getStructure())  // we cannot trust below checks if someone
                              // invalidated structure earlier in this opt
    return true;
  ...
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();
    ...
    if (((*predEdge)->getFrequency() > candEdge->getFrequency()) &&
      ...
      {
        // traceMsg(comp, "really iCHS rejecting cand block_%d numTransforms
        // %d\n", candEdge->getTo()->getNumber(), numTransforms);
        return false;
      }
    }
...
bool TR_OrderBlocks::candidateIsBetterSuccessorThanBest(
    TR::CFGEdge *candidateEdge, TR::CFGEdge *currentBestEdge) {
  ...
  TR_ASSERT(prevBlock == currentBestEdge->getFrom(),
            "edges are from different blocks");

  // is succBlock a better choice than current best?
  if (trace())
    traceMsg(comp(),
             "\tComparing candidate %d(%d) to current best %d(%d) as successor "
             "for %d(%d)\n",
             candidate->getNumber(), candidate->getFrequency(),
             currentBest->getNumber(), currentBest->getFrequency(),
  ...
  if (!comp()->getOption(TR_DisableInterpreterProfiling)) {
    if (candidateEdge->getFrequency() >= 0) {
      if (candidateEdge->getFrequency() == currentBestEdge->getFrequency()) {
        OMR::TreeTop *lexicalSuccessorEntry =
            candidateEdge->getFrom()->asBlock()->getExit()->getNextTreeTop();
        if (lexicalSuccessorEntry != NULL &&
            lexicalSuccessorEntry->getNode()->getBlock() ==
                candidateEdge->getTo()) {
          if (trace())
            traceMsg(comp(),
                     "\t\tis equally hot, but is currently the lexical "
                     "successor, making it my best choice\n");
          return true;
        }
      }
      if (candidateEdge->getFrequency() > currentBestEdge->getFrequency()) {
        ...
        return true;
      } else if (candidateEdge->getFrequency() <
                 currentBestEdge->getFrequency()) {
        if (trace())
          traceMsg(comp(),
                   "\t\thas colder edge than my best choice, so discarding\n");
        return false;
      }
    }
  }
  ...
  if (candidate->hasSuccessor(currentBest)) {
    if (candidate->getFrequency() >
        (prevBlock->getFrequency() - candidate->getFrequency())) {
      if (trace())
        traceMsg(comp(),
                 "\t\thas has current best succ as a succ, detecting an "
                 "if-then structure and making the if block my best choice\n");
      return true;
    }
  }
  ...
  if (!_changeBlockOrderBasedOnHWProfile) {
    ...
    if (!candidate->asBlock()->isCold() && currentBest->asBlock()->isCold()) {
      if (trace())
        traceMsg(comp(),
                 "\t\tcurrent best choice is cold but this one isn't, making "
                 "it my best choice\n");
      return true;
    } else if (candidate->asBlock()->isCold() &&
               !currentBest->asBlock()->isCold()) {
      if (trace())
        traceMsg(comp(),
                 "\t\tis cold while current best choice isn't cold, so "
                 "discarding\n");
      return false;
    }
  }
  ...
  if (candidate->asBlock()->getNestingDepth() >
      currentBest->asBlock()->getNestingDepth()) {
    if (trace())
      traceMsg(comp(),
               "\t\thas deeper nesting level, making it my best choice\n");
    return true;
  } else if (candidate->asBlock()->getNestingDepth() <
             currentBest->asBlock()->getNestingDepth()) {
    if (trace())
      traceMsg(
          comp(),
          "\t\thas lower nesting level than my best choice, so discarding\n");
    return false;
  }

  // ok, we're left to static heuristics now
  // TR::Node *branchNode =
  // prevBlock->asBlock()->getLastRealTreeTop()->getNode();
  ...
  if (bestHasHazards && !candHasHazards) {
    if (trace())
      traceMsg(comp(),
               "\t\tbest choice has hazards but candidate doesn't, making it "
               "my best choice\n");
    // numberHazardCandidates++;
    return true;
  }

  // we've exhausted good ideas for choosing, so now try to choose simply based
  // on extension opportunity
  if (!(currentBest->getPredecessors().size() == 1) &&
      (candidate->getPredecessors().size() == 1)) {
    if (trace())
      traceMsg(
          comp(),
          "\t\tbetter candidate for extension, making it my best choice\n");
    return true;
  }
...
TR::CFGNode *TR_OrderBlocks::chooseBestFallThroughSuccessor(
    TR::CFG *cfg, TR::CFGNode *block, int32_t &numCandidates) {
  ...
  for (auto succEdge = successors.begin(); succEdge != successors.end();
    ...
    if (reallyNewReordering == NULL &&
        succBlock->getVisitCount() == _visitCount) {
    ...
    if (block->asBlock()->getExit() &&
      ...
      if (trace())
        traceMsg(comp(),
                 "\t\tfound original fall-through successor %d: making it "
                 "initial best successor\n",
                 succBlock->getNumber());
    }
  ...
  if (numCandidates == 1) {
    ...
    if (!_changeBlockOrderBasedOnHWProfile) {
      ...
      if (candBlock->isSuperCold()) {
        if (!_hotPathList.isEmpty()) {
          if (trace())
            traceMsg(comp(),
                     "\tcandidate %d is cold, discard because there are still "
                     "hot choices \n",
                     theCandidate->getNumber());
          return NULL;
        } else if (!_coldPathList.isEmpty() &&
                   !_coldPathList.getHeadData()->asBlock()->isSuperCold()) {
          if (trace())
            traceMsg(comp(),
                     "\tcandidate %d is super cold, discard because there are "
                     "still choices which are not so cold \n",
                     theCandidate->getNumber());
          return NULL;
        }
      }
      ...
      if (candBlock->getFrequency() == 0 &&
          hasValidCandidate(_hotPathList, block)) {
        if (trace())
          traceMsg(comp(),
                   "HW Profile: candidate %d has freq zero, discard because "
                   "there are still hot choices \n",
                   candBlock->getNumber());
        return NULL;
      }
    }
    ...
    // if candBlock is a loop exit but the loop has multiple exits, choose the
    // hottest exit instead of candBlock
    if (!envp && cfg->getStructure())
    // Make sure we still have structure info
    {
      TR_Structure *innerLoop =
          block->asBlock()->getStructureOf()->getContainingLoop();

      // the block is contained in the inner loop
      while (innerLoop) {
        TR_Structure *outerLoop =
            candBlock->getStructureOf()->getContainingLoop();

        // outer loop does not exist or contains the inner loop
        if (!outerLoop ||
            ((innerLoop != outerLoop) && outerLoop->contains(innerLoop))) {
          TR::CFGEdgeList &successors = block->asBlock()->getSuccessors();
          if (successors.size() != 2) break;
          // get exit edge frequency
          int16_t exitFrequency = 0;
          for (auto succEdge = successors.begin(); succEdge != successors.end();
               ++succEdge) {
            if ((*succEdge)->getTo()->asBlock()->getNumber() ==
                theCandidate->getNumber()) {
          ...
          if (exitEdges.getSize() > 1) {
            ...
            for (TR::CFGEdge *edge = exitIt.getFirst(); edge;
                 edge = exitIt.getNext()) {
              if (edge->getTo()->asBlock()->getVisitCount() != _visitCount) {
                if (trace())
                  traceMsg(comp(),
                           "\t -> block_%d\tfrequency %4d, current exit "
                           "frequency %4d\n",
                           edge->getTo()->getNumber(), edge->getFrequency(),
                           exitFrequency);
                ...
                if (edge->getFrequency() > exitFrequency) {
                  ...
                  if (trace())
                    traceMsg(comp(),
                             "\t Loop %d has multiple exits, consider hotter "
                             "exit %d\n",
  ...
  // we now know there are multiple candidates, so we have to choose the best
  // one
  ListIterator<TR::CFGEdge> succCandEdgeIt(&candidateEdges);
  if (bestSuccessorEdge == NULL) {
    for (TR::CFGEdge *succEdge = succCandEdgeIt.getFirst(); succEdge;
         succEdge = succCandEdgeIt.getNext()) {
      if (isCandidateReallyBetter(succEdge, comp())) {
        bestSuccessorEdge = succEdge;
        break;
      }
    }
    if (trace()) traceMsg(comp(), "iCRB initial best %p\n", bestSuccessorEdge);
  ...
  for (TR::CFGEdge *succEdge = succCandEdgeIt.getFirst(); succEdge;
       succEdge = succCandEdgeIt.getNext()) {
    if (succEdge == bestSuccessorEdge) continue;
  ...
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();
    ...
    if (pred != block) {
      ...
      if (predSuccEdge == successors.end()) {
        if (trace())
          traceMsg(comp(),
                   "\t\tbut it has another predecessor %d for which it is the "
                   "hottest successor\n",
                   pred->getNumber());
        if (trace())
          traceMsg(comp(),
                   "\t\tcounting this block as a candidate for replication\n",
                   pred->getNumber());
...
void TR_OrderBlocks::removeFromOrderedBlockLists(TR::CFGNode *block) {
  ...
  if (_changeBlockOrderBasedOnHWProfile && block->getFrequency() > 0) {
    ...
    if (trace())
      traceMsg(comp(), "\t_numUnschedHotBlocks remove %d (blockNum:%d) \n",
               _numUnschedHotBlocks, block->getNumber());
  }
}

void TR_OrderBlocks::addToOrderedBlockList(TR::CFGNode *block,
                                           TR_BlockList &list, bool useNumber) {
  ...
  for (listBlock = iter.getFirst(); listBlock != NULL;
    ...
    if (!_superColdBlockOnly) {
      if (cBlock->isCold() && !cBlock->isSuperCold() && lBlock->isSuperCold())
        break;
      else if (cBlock->isSuperCold() && lBlock->isCold() &&
               !lBlock->isSuperCold())
        continue;
      ...
      if (useNumber && block->getNumber() < listBlock->getNumber()) break;
    }
  }
...
void TR_OrderBlocks::addRemainingSuccessorsToList(TR::CFGNode *block,
                                                  TR::CFGNode *excludeBlock) {
  ...
  for (auto excSuccEdge = excSuccessors.begin();
       excSuccEdge != excSuccessors.end(); ++excSuccEdge) {
    TR::CFGNode *succBlock = (*excSuccEdge)->getTo();
    if (succBlock->getVisitCount() != _visitCount) {
      if (succBlock->asBlock()->isCold() &&
          succBlock->asBlock()->getFrequency() <= 0) {
        if (trace())
        ...
        if (trace())
          traceMsg(comp(),
                   "\t\tAdding unvisited non-cold exception successor %d\n",
                   succBlock->getNumber());
        addToOrderedBlockList(succBlock, _hotPathList, false);
      }
    }
...
// Take the successor's frequencies when considering which successor to add to
// the hot list
void TR_OrderBlocks::addRemainingSuccessorsToListHWProfile(
    TR::CFGNode *block, TR::CFGNode *excludeBlock) {
  if (trace())
    traceMsg(comp(), "\tadding remaining successors of block_%d to queue\n",
             block->getNumber());
...
bool TR_OrderBlocks::peepHoleGotoToFollowing(TR::CFG *cfg, TR::Block *block,
                                             TR::Block *followingBlock,
                                             char *title) {
  // pattern: block has goto to fall-through block
  ...
  if (branchToFollowingBlock(gotoNode, block, comp())) {
    ...
    if (performTransformation(comp(),
                              "%s dest of goto in block_%d is the following "
                              "block_%d, removing the goto node\n",
                              title, block->getNumber(),
                              followingBlock->getNumber())) {
...
//   block assumed to end in a goto
// returns TRUE if the pattern was found and replaced
bool TR_OrderBlocks::peepHoleGotoToGoto(TR::CFG *cfg, TR::Block *block,
                                        TR::Node *gotoNode,
                                        TR::Block *destOfGoto, char *title,
                                        TR::BitVector &skippedGotoBlocks) {
  if (comp()->getProfilingMode() == JitProfiling) return false;

  if (destOfGoto->isGotoBlock(comp(), true)) {
    // redirect first goto to the dest of the second goto
    TR::Block *finalGotoBlock =
        destOfGoto->getSuccessors().front()->getTo()->asBlock();
    ...
    if (performTransformation(
            comp(),
            "%s in block_%d, dest of goto (%d) is also goto block, forwarding "
            "destination (%d) back into goto\n",
            title, block->getNumber(), destOfGoto->getNumber(),
            finalGotoBlock->getNumber())) {
      TR_RegionStructure *parent = destOfGoto->getCommonParentStructureIfExists(
          finalGotoBlock, comp()->getFlowGraph());
      if (parent && parent->isNaturalLoop() &&
          (parent->getNumber() == destOfGoto->getNumber()))
        cfg->setStructure(
            0);  // to avoid automatic structure repair creating an improper
                 // region due to temporary state after adding the first edge
...
//   block assumed to end in a goto
// returns TRUE if the pattern was found and replaced
bool TR_OrderBlocks::peepHoleGotoToEmpty(TR::CFG *cfg, TR::Block *block,
                                         TR::Node *gotoNode,
                                         TR::Block *destOfGoto, char *title) {
  if (comp()->getProfilingMode() == JitProfiling) return false;

  if (destOfGoto->isEmptyBlock() &&
      !(destOfGoto->getStructureOf() &&
        destOfGoto->getStructureOf()->isLoopInvariantBlock()) &&
      !(block->getStructureOf() &&
        block->getStructureOf()->isLoopInvariantBlock())) {
    ...
    if (destFallThroughDest != NULL) {
      TR::Block *destFallThroughBlock =
          destFallThroughDest->getNode()->getBlock();
      if (performTransformation(comp(),
                                "%s in block_%d, dest of goto is empty block, "
                                "forwarding destination (%d) back into goto\n",
                                title, block->getNumber(),
                                destFallThroughBlock->getNumber())) {
        TR_RegionStructure *parent =
            destOfGoto->getCommonParentStructureIfExists(
                destFallThroughBlock, comp()->getFlowGraph());
        if (parent && parent->isNaturalLoop() &&
            (parent->getNumber() == destOfGoto->getNumber()))
          cfg->setStructure(
              0);  // to avoid automatic structure repair creating an improper
...
static bool peepHoleGotoToLoopHeader(TR::CFG *cfg, TR::Block *block,
                                     TR::Block *dest, char *title) {
  ...
  if (destStructure && blockStructure) {
    ...
    if (isSingleton) {
      ...
      if (predBlock && predBlock->getStructureOf())
        predBlockIsSibling = (predBlock->getStructureOf()->getParent() ==
    ...
    if (parent && parent->asRegion() && destIsSibling && predBlockIsSibling &&
      ...
      TR::Block *branchDest =
          branchNode->getBranchDestination()->getNode()->getBlock();
      ...
      if (!branchNode->isTheVirtualGuardForAGuardedInlinedCall() &&
          ...
          (branchNode->getOpCodeValue() != TR::ibranch &&
           branchNode->getOpCodeValue() != TR::mbranch) &&
          performTransformation(
              cfg->comp(),
              "%s applied goto-loop header peephole for block_%d dest %d\n",
              title, block->getNumber(), dest->getNumber())) {
        success = true;
        block->getLastRealTreeTop()->getNode()->setBranchDestination(
            branchDest->getEntry());
        cfg->addEdge(
            TR::CFGEdge::createEdge(block, branchDest, cfg->trMemory()));
...
// repeatedly apply peephole optimizations to blocks that end in a goto until no
// more can be done assume block->endsInGoto() returns TRUE if block was removed
//   currently always returns false because this code can't remove block itself
void TR_OrderBlocks::peepHoleGotoBlock(TR::CFG *cfg, TR::Block *block,
                                       char *title) {
  TR_ASSERT(block->endsInGoto(),
            "peepHoleGotoBlock called on block that doesn't end in a goto!");
  bool madeAChange;
  TR::Node *gotoNode = block->getLastRealTreeTop()->getNode();
  ...
  do {
    ...
    if (peepHoleGotoToGoto(cfg, block, gotoNode, destOfGoto, title,
                           skippedGotoBlocks))
      madeAChange = true;
...
void TR_OrderBlocks::removeRedundantBranch(TR::CFG *cfg, TR::Block *block,
                                           TR::Node *branchNode,
                                           TR::Block *takenBlock) {
...
bool TR_OrderBlocks::peepHoleBranchToLoopHeader(TR::CFG *cfg, TR::Block *block,
  ...
  if (destStructure && fallThroughStructure && blockStructure) {
    ...
    if (parent && parent->asRegion() && fallThroughIsSibling &&
        !destIsSibling &&
        ...
        performTransformation(comp(),
                              "%s applied loop header peephole for block_%d "
                              "fall through %d dest %d\n",
                              title, block->getNumber(),
                              fallThrough->getNumber(), dest->getNumber())) {
      success = true;

      TR::TreeTop *fallThroughTT = fallThrough->getEntry();

      TR::Block *gotoBlock = insertGotoFallThroughBlock(
          dest->getEntry(), dest->getEntry()->getNode(), block, dest);
...
// apply peephole optimizations to blocks that end in a branch and branch around
// a single goto
void TR_OrderBlocks::peepHoleBranchAroundSingleGoto(TR::CFG *cfg,
                                                    TR::Block *block,
                                                    char *title) {
  TR_ASSERT(
      block->endsInBranch(),
      "peepHoleBranchBlock called on block that doesn't end in a branch!");
  TR::Node *branchNode = block->getLastRealTreeTop()->getNode();
...
#if 1
  // if we branch around a jump (only a jump), then redirect our branch
  // direction
  if (!branchNode->isTheVirtualGuardForAGuardedInlinedCall() &&
      fallThroughBlock->isGotoBlock(comp(), true) &&
      (fallThroughBlock->getPredecessors().size() == 1) &&
      fallThroughBlock->getExit()->getNextTreeTop()) {
    TR::Block *blockAfterFallThrough =
        fallThroughBlock->getExit()->getNextTreeTop()->getNode()->getBlock();
    ...
    if (!disableSPMD) {
      TR_BlockStructure *blockStructure =
          blockAfterFallThrough->getStructureOf();
      if (blockStructure) {
        if (blockStructure->isLoopInvariantBlock()) {
          if (trace())
            traceMsg(comp(),
                     "\t\tavoid redirecting a jump to a pre-header block %d "
                     "for later SPMD optimization\n",
    ...
    if (!doNotRemove)
      ...
      if (takenBlock == blockAfterFallThrough &&
          (fallThroughBlock->getLastRealTreeTop() ==
           fallThroughBlock->getFirstRealTreeTop())) {
        ...
        if (fallThroughSuccessors.front()->getTo()->asBlock() !=
            blockAfterFallThrough) {
          if (performTransformation(
                  comp(),
                  "%s in block_%d, branch taken dest (%d) is a block after a "
                  "single goto, so redirecting to its goto's destination "
                  "(%d)\n",
                  title, block->getNumber(), takenBlock->getNumber(),
                  gotoDestBlock->getNumber())) {
            TR_ASSERT(branchNode->getOpCode().isBranch(),
                      "expected branch at end of prevBlock to reverse, but "
                      "found something else");
            branchNode->reverseBranch(gotoDest);
            if (!block->hasSuccessor(gotoDestBlock)) {
              cfg->addEdge(block, gotoDestBlock);
              block->getSuccessors().front()->setFrequency(std::min(
                  block->getEdge(fallThroughBlock)->getFrequency(),
...
// no looping here because there are only two possible optimizations done but we
// know which order to try them in assume block->endsInBranch()
void TR_OrderBlocks::peepHoleBranchBlock(TR::CFG *cfg, TR::Block *block,
                                         char *title) {
  TR_ASSERT(
      block->endsInBranch(),
      "peepHoleBranchBlock called on block that doesn't end in a branch!");
  ...
  // if taken block is still a goto block, then redirect our taken branch
  // direction
  while (takenBlock->isGotoBlock(comp(), true)) {
    ...
    if ((!takenBlock->getStructureOf() ||
         !takenBlock->getStructureOf()->isLoopInvariantBlock()) &&
        performTransformation(
            comp(),
            "%s in block_%d, branch taken dest (%d) is a goto block, so "
            "redirecting to its destination (%d)\n",
            title, block->getNumber(), takenBlock->getNumber(),
      ...
      if (isDoubleton && !block->hasSuccessor(takenGotoDestBlock)) {
        ...
        cfg->updateBlockFrequency(takenBlock,
                                  takenBlock->getFrequency() - oldEdgeFreq);
        cfg->updateBlockFrequencyFromEdges(takenBlock);
        cfg->updateBlockFrequency(
            takenGotoDestBlock,
            takenGotoDestBlock->getFrequency() + oldEdgeFreq);
        if (trace()) {
          traceMsg(comp(), "\t\t\ttakenBlock (b_%d) new Frequency: %d\n",
                   takenBlock->getNumber(), takenBlock->getFrequency());
          traceMsg(comp(),
                   "\t\t\ttakenGotoDestBlock (b_%d) new Frequency: %d\n",
                   takenGotoDestBlock->getNumber(),
        ...
        if (isDoubleton) cfg->removeEdge(block, takenBlock);
      }
    } else
      break;
...
bool TR_OrderBlocks::peepHoleBranchToFollowing(TR::CFG *cfg, TR::Block *block,
                                               TR::Block *followingBlock,
                                               char *title) {
  TR_ASSERT(block->endsInBranch(),
            "peepHoleBranchToFollowing called on block that doesn't end in a "
            "branch!");
  TR::Node *branchNode = block->getLastRealTreeTop()->getNode();
  TR::TreeTop *takenEntry = branchNode->getBranchDestination();
  TR::Block *takenBlock = takenEntry->getNode()->getBlock();

  // this pattern is not one that can be legally left in the CFG, so no
  // performTransformation here
...
void TR_OrderBlocks::removeEmptyBlock(TR::CFG *cfg, TR::Block *block,
                                      char *title) {
  ...
  if (performTransformation(comp(),
                            "%s empty block_%d, redirecting edges around this "
                            "block then removing it\n",
                            title, block->getNumber())) {
    bool isLoopEntryBlock =
        block->getStructureOf() &&
    ...
    if (!block->getSuccessors().empty()) {
      ...
      if (trace())
        traceMsg(
            comp(),
            "\t\t\tredirecting edges to block's fall-through successor %d\n",
      ...
      for (auto predEdge = block->getPredecessors().begin();
           predEdge != block->getPredecessors().end();) {
        TR::CFGEdge *current = *(predEdge++);
        TR::Block *fromBlock = current->getFrom()->asBlock();
        if (trace())
          traceMsg(comp(), "\t\t\tredirecting edge (%d,%d) to new dest %d\n",
                   fromBlock->getNumber(), current->getTo()->getNumber(),
                   fallThroughBlock->getNumber());
        fromBlock->redirectFlowToNewDestination(comp(), current,
                                                fallThroughBlock, false);
      }
      // If we removed the loop entry block then invalidate structure to avoid
      // false improper regions
      if (isLoopEntryBlock) invalidateStructure();
    } else if (!block->getPredecessors().empty()) {
      if (trace())
        traceMsg(comp(),
                 "\t\tblock has no successors so no edges to redirect, just "
                 "removing predecessor edges\n");
      for (auto predEdge = block->getPredecessors().begin();
           predEdge != block->getPredecessors().end();) {
      ...
      // false improper regions
      if (isLoopEntryBlock) invalidateStructure();
      // the last removeEdge will remove block so nothing left to do at this
      // point
      return;
    } else if (trace())
      traceMsg(comp(),
               "\t\tblock has no successors edges to redirect, no predecessor "
               "edges to remove\n");

    TR_ASSERT(block->getPredecessors().empty(),
              "how can there be any more predecessors?");
    ...
    if (prevTT != NULL)
      ...
      comp()->getMethodSymbol()->setFirstTreeTop(
          block->getExit()->getNextTreeTop());
...
// apply a small set of transformations to try to clean up blocks before
// ordering
...
bool TR_OrderBlocks::doPeepHoleBlockCorrections(TR::Block *block, char *title) {
  ...
  // pattern: block has nothing in it and no exceptional predecessors (can
  // redirect edges around it and remove it)
  ...
  // TODO enable for PLX, currently disabled because we cannot re-direct edges
  // for ASM flows
  {
  ...
  if (block->getPredecessors().empty() &&
      (block->hasExceptionPredecessors() == false) &&
      block->getEntry() != NULL) {
    if (performTransformation(comp(),
                              "%s block_%d has no predecessors so removing it "
                              "and its out edges from the flow graph\n",
                              title, block->getNumber())) {
      // disconnect any out edges
      TR_SuccessorIterator outEdges(block);
      for (auto succeedingEdge = outEdges.getFirst(); succeedingEdge;
           succeedingEdge = outEdges.getNext())
  ...
  while (madeAChange) {
    TR::TreeTop *prevTT = block->getEntry()->getPrevTreeTop();
    TR::Block *prevBlock =
        (prevTT != NULL) ? prevTT->getNode()->getBlock() : NULL;

    madeAChange = false;

    if (prevBlock != NULL) {
      if (prevBlock->endsInGoto() &&
          peepHoleGotoToFollowing(cfg, prevBlock, block, title))
        madeAChange = true;

      else if (prevBlock->endsInBranch() &&
               peepHoleBranchToFollowing(cfg, prevBlock, block, title))
        madeAChange = true;

      // if we made either of the above changes, then prevBlock might be empty
      // now (remove it if it is)
      if (madeAChange && prevBlock->isEmptyBlock() &&
          (prevBlock->hasExceptionPredecessors() == false) &&
          comp()->getProfilingMode() != JitProfiling &&
  ...
  // if block ends in goto or branch, look for opportunities to optimize the
  // destination
  if (block->endsInGoto())
...
bool TR_OrderBlocks::lookForPeepHoleOpportunities(char *title) {
  ...
  TR_ASSERT(tt->getNode()->getOpCodeValue() == TR::BBStart,
            "first tree isn't BBStart");
  TR::Block *lastBlock = NULL;

  if (trace()) traceMsg(comp(), "Looking for peephole opportunities:\n");
  while (tt != NULL) {
    TR_ASSERT(tt->getNode()->getOpCodeValue() == TR::BBStart,
              "tree walk reached something that isn't BBStart");
    ...
    tt = nextBlockTT;
    if (!blockStillExists) blocksWereRemoved = true;
  }
...
void TR_OrderBlocks::insertBlocksToList() {
  ...
  while (block != NULL) {
    if (_visitCount != block->getVisitCount()) {
      ...
      if (block->getFrequency() > 0) _numUnschedHotBlocks++;

      addToOrderedBlockList(block, _coldPathList, false);
    }
    if (block->asBlock()->getExit() == NULL ||
        block->asBlock()->getExit()->getNextTreeTop() == NULL)
      block = NULL;
    else
      block =
          block->asBlock()->getExit()->getNextTreeTop()->getNode()->getBlock();
  }
  TR::CFGNode *startBlock = comp()->getFlowGraph()->getStart();
  TR::CFGNode *endBlock = comp()->getFlowGraph()->getEnd();
  if (startBlock->getFrequency() > 0 &&
      startBlock->getVisitCount() != _visitCount)
    _numUnschedHotBlocks++;
...
void TR_OrderBlocks::generateNewOrder(TR_BlockList &newBlockOrder) {
  ...
  if (_superColdBlockOnly) {
    //     lastElementInOrder = newBlockOrder.addAfter(cfg->getStart(),
    //     lastElementInOrder);
    while (block != NULL) {
      // traceMsg(comp(), "Checking Block %d [%p] getEntry()=%p
      // methodSymbol->getFirstTreeTop()=%p\n", block->getNumber(),
  ...
  block = cfg->getStart();
  while (block != NULL ||
         !(_hotPathList.isEmpty() && _coldPathList.isEmpty())) {
    if (trace()) {
      if (block) {
      ...
      for (TR::CFGNode *hotBlock = hotList.getFirst(); hotBlock != NULL;
           hotBlock = hotList.getNext())
        traceMsg(comp(), "%d ", hotBlock->getNumber());
      traceMsg(comp(), "\n\tForest of cold paths: ");
      TR_BlockListIterator coldList(&_coldPathList);
      for (TR::CFGNode *coldBlock = coldList.getFirst(); coldBlock != NULL;
           coldBlock = coldList.getNext())
        traceMsg(comp(), "%d ", coldBlock->getNumber());
      traceMsg(comp(), "\n");
    }

    // if block is NULL, then we need to pick a new path to walk from the forest
    // else if block is non-NULL and its best successor is in a different
    // structure, then we should see if there's a better path
    if (block == NULL) {
      TR::CFGNode *lastBlock = NULL;
      if (lastElementInOrder != NULL) lastBlock = lastElementInOrder->getData();
    ...
    if (block->asBlock()->getExit() &&
      ...
      if (trace())
        traceMsg(
            comp(), "Lexical order block_%d visitCount=%d _visitCount=%d\n",
    ...
    if (needBetterChoice(cfg, block, bestSucc) &&
        performTransformation(
            comp(), "%s choose best successor for block_%d (freq:%d)\n",
            OPT_DETAILS, block->getNumber(), block->getFrequency()))
      bestSucc = chooseBestFallThroughSuccessor(cfg, block, numCandidates);

    // TR::CFGNode *bestSucc = chooseBestFallThroughSuccessor(cfg, block,
    // numCandidates);

    addRemainingSuccessorsToList(block, bestSucc);

    // if bestSucc is in a different structure, then just put bestSucc into the
    // right forest of paths and set block to NULL so that we'll choose the best
    // (possibly higher frequency) path to follow
    if (bestSucc != NULL && (numCandidates != 1) &&
        endPathAtBlock(block, bestSucc, cfg) &&
        performTransformation(
            comp(), "%s Reordering blocks to optimize fall-through paths\n",
            OPT_DETAILS)) {
      if (!_superColdBlockOnly) {
        if (trace())
          traceMsg(
              comp(),
              "Choosing to end path here, block_% will be added into list\n",
              bestSucc->asBlock()->getNumber());
        ...
        if (trace())
          traceMsg(
              comp(),
              "Choosing to end path here, no need to add block_% into list\n",
              bestSucc->asBlock()->getNumber());
      }
      block = NULL;
    } else
...
// prevBlock's fall-through successor used to be "origSucc" but now it is some
// other block so: insert a block following prevBLock that contains a goto node
// to "origSucc"
TR::Block *TR_BlockOrderingOptimization::insertGotoFallThroughBlock(
    TR::TreeTop *fallThroughTT, TR::Node *node, TR::CFGNode *prevBlock,
    TR::CFGNode *origSucc, TR_RegionStructure *parent) {
  // insert a goto so that control gets to prevBlock's original fall-through
  // successor
  TR::CFG *cfg = comp()->getFlowGraph();
  ...
  TR::Block *gotoBlock = TR::Block::createEmptyBlock(
      fallThroughTT->getNode(), comp(), frequency, prevBlock->asBlock());
  TR::TreeTop::create(comp(), gotoBlock->getEntry(),
                      TR::Node::create(node, TR::Goto, 0, fallThroughTT));

  if (!parent)
    parent = prevBlock->asBlock()->getCommonParentStructureIfExists(
        origSucc->asBlock(), comp()->getFlowGraph());
  ...
  gotoBlock->asBlock()->inheritBlockInfo(prevBlock->asBlock(),
                                         prevBlock->asBlock()->isCold());
...
// connect all the trees together according to the order of blocks in
// newBlockOrder
void TR_BlockOrderingOptimization::connectTreesAccordingToOrder(
    TR_BlockList &newBlockOrder) {
  TR::CFG *cfg = comp()->getFlowGraph();
  TR::ResolvedMethodSymbol *methodSymbol = optimizer()->getMethodSymbol();

  // now put the trees in the order dictated by newBlockOrder (they are stored
  // on the list IN ORDER) right now, this code just assumes that it can safely
  // rewrite the trees...should probably put some assumes in here
  ...
  if (trace())
    traceMsg(comp(),
             "block =%d prevBlock->getEntry()->getNode()=%p "
             "methodSymbol->getFirstTreeTop()->getNode()=%p\n",
             prevBlock->getNumber(), prevBlock->getEntry()->getNode(),
             methodSymbol->getFirstTreeTop()->getNode());

  TR_ASSERT(prevBlock->getEntry() == methodSymbol->getFirstTreeTop(),
            "first tree shouldn't change!");
  ...
  while (!newBlockOrder.isEmpty()) {
    ...
    if (fallThroughTT != block->getEntry()) {
      ...
      // if prevBlock is exit, or prevBlock was originally the last block, then
      // there's no special work to do also, if there is no flow edge from
      // prevBlock to the original fall through block, nothing to worry about
      if (prevBlock->getEntry() != NULL && fallThroughTT != NULL &&
        ...
        if (trace())
          traceMsg(comp(),
                   "\t%d did not originally follow %d in trees, might need to "
                   "move trees\n",
                   block->getNumber(), prevBlock->getNumber());
        // 5 possibilities, the first two of which require some fix-up:
        //     1) block is not a successor of prevBlock, but prevBlock's
        //     fall-through successor has already been placed elsewhere 2) block
        //     is a different successor block than the original fall-through
        //        similar to 5, but we can reverse the branch in this case
        //     3) block is the only successor of prevBlock, but prevBlock ends
        //     in a goto (straightening should clean-up later) 4) block is not a
        //     successor of prevBlock, but it begins the path we (arbitrarily?)
        //     chose to follow prevBlock 5) block is a successor of prevBlock,
        //     but it's not the original fall through (due to endPathAtBlock)
        //        and we can't reverse branch (TR::branch and
        //        isJumpWithMulitpleTargets), we need a go to for this case
        TR::Node *branchNode = prevBlock->getLastRealTreeTop()->getNode();
        if (branchNode->getOpCodeValue() == TR::treetop)
          branchNode = branchNode->getFirstChild();

        if (branchNode->getOpCodeValue() != TR::Goto &&
            (!branchNode->getOpCode()
                  .isJumpWithMultipleTargets() ||  // don't worry about igotos
             (branchNode->getOpCode().isJumpWithMultipleTargets() &&
              branchNode->getOpCode().isCall()))) {
          ...
          if (!isSucc ||
            ...
            if (trace())
              traceMsg(comp(),
                       "\tneed to add extra goto block so that %d will "
                       "fall-through to %d\n",
                       prevBlock->getNumber(), origSucc->getNumber());
            TR::Block *gotoBlock = insertGotoFallThroughBlock(
                fallThroughTT, branchNode, prevBlock, origSucc);
            ...
            if (trace())
              traceMsg(comp(),
                       "\tdecided to reverse the branch at the end of %d\n",
                       prevBlock->getNumber());
            TR::Node *branchNode = prevBlock->getLastRealTreeTop()->getNode();
            TR_ASSERT(branchNode->getOpCode().isBranch(),
                      "expected branch at end of prevBlock to reverse, but "
        ...
        if (trace())
          traceMsg(comp(),
                   "\t%d did not originally follow %d in trees, but no trees "
                   "manipulation needed\n",
                   block->getNumber(), prevBlock->getNumber());
      }
    ...
    if (block->getExit()) currentLastTreeTop = block->getExit();
  ...
  if (reorderingStats)
    fprintf(stderr, "%d replication candidates in method %s (has %d blocks)\n",
            numberMethodReplicationCandidates, comp()->signature(),
            numberOfBlocks);
...
bool TR_OrderBlocks::doBlockExtension() {
  ...
  while (tt != NULL) {
    ...
    // IvanB: see if this condition can be relaxed to not exclude jumps with
    // multiple targets
    if ((block->getPredecessors().size() == 1) &&
        ...
        !prevBlock->getLastRealTreeTop()
             ->getNode()
             ->getOpCode()
             .isJumpWithMultipleTargets() &&
        !(prevBlock->getLastRealTreeTop()->getNode()->getOpCodeValue() ==
              TR::treetop &&
          prevBlock->getLastRealTreeTop()
              ->getNode()
              ->getFirstChild()
      ...
      if (performTransformation(comp(),
                                "%s block_%d is extension of previous block\n",
...
void TR_OrderBlocks::doReordering() {
  // if (!performTransformation(comp(), "%s ORDER BLOCK: Reordering blocks to
  // optimize fall-through paths\n", OPT_DETAILS))
  //   return;
  ...
  // if (performTransformation(comp(), "%s Reordering blocks to optimize
  // fall-through paths\n", OPT_DETAILS))
...
bool TR_OrderBlocks::shouldPerform() {
  if (comp()->getOption(TR_DisableNewBlockOrdering)) return false;
...
int32_t TR_OrderBlocks::perform() {
  ...
  if (_reorderBlocks &&
      performTransformation(comp(), "%s Propagating coldness information\n",
                            OPT_DETAILS))
  ...
  if (_reorderBlocks) doReordering();

  // comp()->getFlowGraph()->setFrequencies();
  // comp()->getDebug()->verifyCFG(getOptimizer()->getMethodSymbol());

  // block extension must be the last thing we do...particularly after peephole
  // opts because they can change a block's predecessors
  if (_extendBlocks) {
  ...
  if (_donePeepholeGotoToLoopHeader) comp()->getFlowGraph()->setStructure(0);
...
const char *TR_OrderBlocks::optDetailString() const throw() {
  return "O^O ORDER BLOCKS: ";
}

void checkOrderingConsistency(TR::Compilation *comp) {
  static char *debugConsistencyCheck =
      feGetEnv("TR_debugBlockOrderingConsistencyCheck");
    ...
    if (!block->isExtensionOfPreviousBlock()) {
      ...
      for (auto succEdge = successors.begin(); succEdge != successors.end();
        ...
        if (succ->getVisitCount() == visitCount)
          continue;  // if laid out before us, it's not really a candidate for
                     // fall-through

        if (succ == block) prevFlowsToBlock = true;

        if (succ->getFrequency() > block->getFrequency())
          prevsHotterSuccessor = succ;
        else if (rootStructure &&
                 succ->getFrequency() == block->getFrequency() &&
                 succ->asBlock()->getNestingDepth() >
                     block->asBlock()->getNestingDepth())
          prevsHotterSuccessor = succ;
      }
    }

    if (debugConsistencyCheck && prevFlowsToBlock &&
        prevsHotterSuccessor != NULL) {
      if (rootStructure)
        fprintf(stderr,
                "Block %d(%d) doesn't look like the best successor compared to "
                "%d(%d)\n",
                block->getNumber(), block->getFrequency(),
                prevsHotterSuccessor->getNumber(),
                prevsHotterSuccessor->getFrequency());
      else
        fprintf(stderr,
                "Block %d(%d,%d) doesn't look like the best successor compared "
                "to %d(%d,%d)\n",
                block->getNumber(), block->getFrequency(),
                block->asBlock()->getNestingDepth(),
                prevsHotterSuccessor->getNumber(),
...
void TR_BlockOrderingOptimization::dumpBlockOrdering(TR::TreeTop *tt,
                                                     char *title) {
  ...
  while (tt != NULL) {
    ...
    if (node && node->getOpCodeValue() == TR::BBStart) {
      ...
      traceMsg(comp(),
               "block_%-4d\t[ " POINTER_PRINTF_FORMAT "]\tfrequency %4d",
               block->getNumber(), block, block->getFrequency());
...
TR::Block **TR_BlockShuffling::allocateBlockArray() {
  return (TR::Block **)trMemory()->allocateStackMemory(_numBlocks *
                                                       sizeof(TR::Block *));
}
...
int32_t TR_BlockShuffling::perform() {
  ...
  TR::Block *firstShufflableBlock =
      firstExecutableBlock
          ->getNextBlock();  // first executable block must be physically first
  ...
  for (i = 0, block = firstShufflableBlock; block;
       block = block->getNextBlock(), i++) {
  }
  ...
  if (trace()) dumpBlockOrdering(firstTree, "Initial block order");

  // Do the requested shuffling operations
  //
  char *sequence = comp()->getOptions()->getBlockShufflingSequence();
  if (trace()) traceMsg(comp(), "Using shuffling sequence <%s>\n", sequence);
  ...
  for (int32_t i = _numBlocks - 1; i >= 0; i--) newBlockOrder.add(blocks[i]);
  for (block = firstShufflableBlock->getPrevBlock(); block;
  ...
  if (trace()) dumpBlockOrdering(firstTree, "Final block order");
...
void TR_BlockShuffling::riffle(TR::Block **blocks) {
  ...
  int32_t split = 0;
  int32_t i;
  for (i = 0; i < 5; i++) split += randomInt(_numBlocks - 1);
  split /= i;

  if (!performTransformation(comp(),
                             "O^O BLOCK SHUFFLING: Performing riffle shuffle, "
                             "splitting at #%d/%d = block_%d\n",
  ...
  while (upper < split || lower < _numBlocks) {
    ...
    if (decision < 0) {
      if (performTransformation(
              comp(),
              "O^O BLOCK SHUFFLING:   move to [%3d] upper (%3d) block_%d\n",
              target, upper, originalBlocks[upper]->getNumber()))
        blocks[target++] = originalBlocks[upper++];
    } else {
      if (performTransformation(
              comp(),
              "O^O BLOCK SHUFFLING:   move to [%3d] lower (%3d) block_%d\n",
...
void TR_BlockShuffling::reverse(TR::Block **blocks) {
  ...
  for (upper = 0, lower = _numBlocks - 1; upper < lower; upper++, lower--)
    if (performTransformation(comp(),
                              "O^O BLOCK SHUFFLING:   swap [%3d] and [%3d] "
                              "(block_%d and block_%d)\n",
                              upper, lower, blocks[upper]->getNumber(),
                              blocks[lower]->getNumber()))
      swap(blocks, upper, lower);
}

const char *TR_BlockShuffling::optDetailString() const throw() {
