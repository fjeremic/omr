...
void
printReorderingStatistics(void)
{
  if (numberOfCompiles++) {
...
void
checkOrderingConsistency(TR::Compilation* comp);
...
bool
TR_OrderBlocks::needBetterChoice(TR::CFG* cfg,
                                 TR::CFGNode* block,
                                 TR::CFGNode* bestSucc)
{
...
// ordering
bool
TR_OrderBlocks::cannotFollowBlock(TR::Block* block, TR::Block* prevBlock)
{
...
bool
TR_OrderBlocks::mustFollowBlock(TR::Block* block, TR::Block* prevBlock)
{
...
TR::CFGNode*
TR_OrderBlocks::findSuitablePathInList(List<TR::CFGNode>& list,
...
bool
TR_OrderBlocks::hasValidCandidate(List<TR::CFGNode>& list,
                                  TR::CFGNode* prevBlock)
{
  ListElement<TR::CFGNode>* prev = NULL;
  ListElement<TR::CFGNode>* ptr = list.getListHead();
...
TR::CFGNode*
TR_OrderBlocks::findBestPath(TR::CFGNode* prevBlock)
{
...
bool
TR_OrderBlocks::endPathAtBlock(TR::CFGNode* block,
                               TR::CFGNode* bestSucc,
                               TR::CFG* cfg)
{
  ...
  if (bestSuccStructure == NULL || blockStructure == NULL) {
    if (trace())
      traceMsg(
        comp(),
        "\t\tEnd path because structure is NULL block_%d=%p to block_%d=%p \n",
        block->asBlock()->getNumber(),
        blockStructure,
        bestSucc->asBlock()->getNumber(),
        bestSuccStructure);
...
bool
TR_OrderBlocks::analyseForHazards(TR::CFGNode* block)
{
...
bool
TR_OrderBlocks::isCandidateReallyBetter(TR::CFGEdge* candEdge,
                                        TR::Compilation* comp)
{
  ...
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end();
    ...
    for (auto predSuccEdge = predSuccessors.begin();
         predSuccEdge != predSuccessors.end();
         ++predSuccEdge) {
...
static bool
isCandidateTheHottestSuccessor(TR::CFGEdge* candEdge, TR::Compilation* comp)
{
  /// return true;
...
bool
TR_OrderBlocks::candidateIsBetterSuccessorThanBest(TR::CFGEdge* candidateEdge,
                                                   TR::CFGEdge* currentBestEdge)
{
  ...
  if (trace())
    traceMsg(comp(),
             ...
             currentBest->getNumber(),
             currentBest->getFrequency(),
             prevBlock->getNumber(),
             prevBlock->getFrequency());
  ...
  if (!_changeBlockOrderBasedOnHWProfile) {
    ...
    if (!candidate->asBlock()->isCold() && currentBest->asBlock()->isCold()) {
      ...
      if (trace())
        traceMsg(
          comp(),
          "\t\tis cold while current best choice isn't cold, so discarding\n");
      return false;
    }
  }
  //}
...
TR::CFGNode*
TR_OrderBlocks::chooseBestFallThroughSuccessor(TR::CFG* cfg,
                                               TR::CFGNode* block,
                                               int32_t& numCandidates)
{
  ...
  if (trace())
    traceMsg(
      comp(), "Block %d: looking for best successor\n", block->getNumber());
...
void
TR_OrderBlocks::removeFromOrderedBlockLists(TR::CFGNode* block)
{
  ...
  if (_changeBlockOrderBasedOnHWProfile && block->getFrequency() > 0) {
    ...
    if (trace())
      traceMsg(comp(),
               "\t_numUnschedHotBlocks remove %d (blockNum:%d) \n",
               _numUnschedHotBlocks,
               block->getNumber());
  }
}
...
void
TR_OrderBlocks::addToOrderedBlockList(TR::CFGNode* block,
                                      TR_BlockList& list,
                                      bool useNumber)
{
...
void
TR_OrderBlocks::addRemainingSuccessorsToList(TR::CFGNode* block,
                                             TR::CFGNode* excludeBlock)
{
  ...
  if (trace())
    traceMsg(comp(),
             "\tadding remaining successors of block_%d to queue\n",
             block->getNumber());
  ...
  for (auto succEdge = successors.begin(); succEdge != successors.end();
    ...
    if (succBlock != excludeBlock &&
      ...
      if (succBlock->asBlock()->isCold()) {
        if (trace())
          traceMsg(comp(),
                   "\t\tAdding unvisited cold successor %d\n",
                   succBlock->getNumber());
        addToOrderedBlockList(succBlock, _coldPathList, true);
      } else {
        if (trace())
          traceMsg(comp(),
                   "\t\tAdding unvisited non-cold successor %d\n",
                   succBlock->getNumber());
  ...
  for (auto excSuccEdge = excSuccessors.begin();
    ...
    if (succBlock->getVisitCount() != _visitCount) {
      if (succBlock->asBlock()->isCold() &&
        ...
        if (trace())
          traceMsg(comp(),
                   "\t\tAdding unvisited cold exception successor %d\n",
                   succBlock->getNumber());
        addToOrderedBlockList(succBlock, _coldPathList, true);
      } else {
        if (trace())
...
// the hot list
void
TR_OrderBlocks::addRemainingSuccessorsToListHWProfile(TR::CFGNode* block,
                                                      TR::CFGNode* excludeBlock)
{
...
bool
TR_OrderBlocks::peepHoleGotoToFollowing(TR::CFG* cfg,
                                        TR::Block* block,
...
// returns TRUE if the pattern was found and replaced
bool
TR_OrderBlocks::peepHoleGotoToGoto(TR::CFG* cfg,
                                   TR::Block* block,
                                   TR::Node* gotoNode,
                                   TR::Block* destOfGoto,
                                   char* title,
  ...
  if (destOfGoto->isGotoBlock(comp(), true)) {
    ...
    if (performTransformation(
      ...
      if (parent && parent->isNaturalLoop() &&
        ...
        cfg->setStructure(
          0); // to avoid automatic structure repair creating an improper region
              // due to temporary state after adding the first edge
...
// returns TRUE if the pattern was found and replaced
bool
TR_OrderBlocks::peepHoleGotoToEmpty(TR::CFG* cfg,
                                    TR::Block* block,
                                    TR::Node* gotoNode,
                                    TR::Block* destOfGoto,
                                    char* title)
    ...
    if (destFallThroughDest != NULL) {
      ...
      if (performTransformation(comp(),
        ...
        TR_RegionStructure* parent =
          destOfGoto->getCommonParentStructureIfExists(destFallThroughBlock,
                                                       comp()->getFlowGraph());
        if (parent && parent->isNaturalLoop() &&
            (parent->getNumber() == destOfGoto->getNumber()))
          cfg->setStructure(
...
static bool
peepHoleGotoToLoopHeader(TR::CFG* cfg,
                         TR::Block* block,
                         TR::Block* dest,
                         char* title)
{
...
void
TR_OrderBlocks::peepHoleGotoBlock(TR::CFG* cfg, TR::Block* block, char* title)
{
  ...
  do {
    ...
    if (peepHoleGotoToGoto(
          cfg, block, gotoNode, destOfGoto, title, skippedGotoBlocks))
      madeAChange = true;
...
void
TR_OrderBlocks::removeRedundantBranch(TR::CFG* cfg,
                                      TR::Block* block,
                                      TR::Node* branchNode,
                                      TR::Block* takenBlock)
{
...
bool
TR_OrderBlocks::peepHoleBranchToLoopHeader(TR::CFG* cfg,
                                           TR::Block* block,
...
// a single goto
void
TR_OrderBlocks::peepHoleBranchAroundSingleGoto(TR::CFG* cfg,
                                               TR::Block* block,
                                               char* title)
{
  TR_ASSERT(
      ...
      if (takenBlock == blockAfterFallThrough &&
        ...
        if (fallThroughSuccessors.front()->getTo()->asBlock() !=
          ...
          if (performTransformation(
                ...
                "single goto, so redirecting to its goto's destination (%d)\n",
                title,
                block->getNumber(),
...
void
TR_OrderBlocks::peepHoleBranchBlock(TR::CFG* cfg, TR::Block* block, char* title)
{
  TR_ASSERT(
    block->endsInBranch(),
    "peepHoleBranchBlock called on block that doesn't end in a branch!");
  ...
  while (takenBlock->isGotoBlock(comp(), true)) {
    ...
    if ((!takenBlock->getStructureOf() ||
      ...
      if (isDoubleton && !block->hasSuccessor(takenGotoDestBlock)) {
        ...
        if (trace()) {
          traceMsg(comp(),
                   "\t\t\tcreating new edge (b_%d -> b_%d) freq: %d\n",
                   block->getNumber(),
                   takenGotoDestBlock->getNumber(),
                   newEdge->getFrequency());
          traceMsg(comp(),
                   "\t\t\tinstead of edge (b_%d -> b_%d) freq: %d \n",
                   block->getNumber(),
                   takenBlock->getNumber(),
                   newEdge->getFrequency());
        }
        ...
        cfg->updateBlockFrequency(
          takenGotoDestBlock, takenGotoDestBlock->getFrequency() + oldEdgeFreq);
        if (trace()) {
          traceMsg(comp(),
                   "\t\t\ttakenBlock (b_%d) new Frequency: %d\n",
                   takenBlock->getNumber(),
...
bool
TR_OrderBlocks::peepHoleBranchToFollowing(TR::CFG* cfg,
                                          TR::Block* block,
                                          TR::Block* followingBlock,
                                          char* title)
{
  TR_ASSERT(
    block->endsInBranch(),
    "peepHoleBranchToFollowing called on block that doesn't end in a branch!");
  TR::Node* branchNode = block->getLastRealTreeTop()->getNode();
  TR::TreeTop* takenEntry = branchNode->getBranchDestination();
  TR::Block* takenBlock = takenEntry->getNode()->getBlock();
  ...
  if (takenBlock == followingBlock) {
    if (trace())
      dumpOptDetails(comp(),
                     "block_%d ends in redundant branch to %d\n",
                     block->getNumber(),
                     takenBlock->getNumber());
...
void
TR_OrderBlocks::removeEmptyBlock(TR::CFG* cfg, TR::Block* block, char* title)
{
  ...
  if (performTransformation(comp(),
    ...
    bool isLoopEntryBlock =
      block->getStructureOf() && block->getStructureOf()->getContainingLoop() &&
    ...
    if (!block->getSuccessors().empty()) {
      ...
      for (auto predEdge = block->getPredecessors().begin();
        ...
        if (trace())
          traceMsg(comp(),
                   "\t\t\tredirecting edge (%d,%d) to new dest %d\n",
                   fromBlock->getNumber(),
                   current->getTo()->getNumber(),
                   fallThroughBlock->getNumber());
        fromBlock->redirectFlowToNewDestination(
          comp(), current, fallThroughBlock, false);
      }
...
bool
TR_OrderBlocks::doPeepHoleBlockCorrections(TR::Block* block, char* title)
{
...
bool
TR_OrderBlocks::lookForPeepHoleOpportunities(char* title)
{
...
TR_OrderBlocks::initialize()
{
  ...
  for (node = cfg->getFirstNode(); node; node = node->getNext()) {
    ...
    if (node->getFrequency() <= 0) {
      if (trace())
        traceMsg(comp(),
                 "\t\tForcing original frequency %d to 0\n",
                 node->getFrequency());
      node->setFrequency(0);
    }
...
void
TR_OrderBlocks::insertBlocksToList()
{
  ...
  if (trace())
    traceMsg(comp(),
             "\t_numUnschedHotBlocks %s %d\n",
             comp()->signature(),
             _numUnschedHotBlocks);
}

void
TR_OrderBlocks::generateNewOrder(TR_BlockList& newBlockOrder)
{
  TR::CFG* cfg = comp()->getFlowGraph();
  ...
  while (block != NULL ||
    ...
    if (trace()) {
      traceMsg(comp(),
               "\t\tset visit count for block_%d to %d\n",
               block->getNumber(),
               _visitCount);

      if (lastElementInOrder)
        traceMsg(comp(),
                 "\tadding %d to order after %d\n",
                 block->getNumber(),
    ...
    if (block->asBlock()->getExit() &&
      ...
      if (trace())
        traceMsg(comp(),
                 "Lexical order block_%d visitCount=%d _visitCount=%d\n",
                 nextBlock->getNumber(),
                 nextBlock->getVisitCount(),
                 _visitCount);
      if (nextBlock->getVisitCount() != _visitCount) {
        bestSucc = nextBlock;
        if (trace())
          traceMsg(comp(),
                   "Choosing to default lexical order block\n",
                   bestSucc->asBlock()->getNumber());
      }
    ...
    if (needBetterChoice(cfg, block, bestSucc) &&
        performTransformation(
          comp(),
          "%s choose best successor for block_%d (freq:%d)\n",
          OPT_DETAILS,
          block->getNumber(),
          block->getFrequency()))
    ...
    if (bestSucc != NULL && (numCandidates != 1) &&
        ...
        performTransformation(
          comp(),
          "%s Reordering blocks to optimize fall-through paths\n",
          OPT_DETAILS)) {
      if (!_superColdBlockOnly) {
        if (trace())
          traceMsg(
...
// to "origSucc"
TR::Block*
TR_BlockOrderingOptimization::insertGotoFallThroughBlock(
  TR::TreeTop* fallThroughTT,
  TR::Node* node,
  TR::CFGNode* prevBlock,
  TR::CFGNode* origSucc,
  TR_RegionStructure* parent)
...
// newBlockOrder
void
TR_BlockOrderingOptimization::connectTreesAccordingToOrder(
  TR_BlockList& newBlockOrder)
{
  ...
  if (fallThroughTT != NULL) {
    ...
    if (prevBlock->hasSuccessor(origSucc) && !prevBlock->endsInGoto() &&
      ...
      if (trace())
        traceMsg(
          comp(),
          "\tneed to add extra goto block so that %d will fall-through to %d\n",
          prevBlock->getNumber(),
          origSucc->getNumber());
      TR::Node* branchNode = prevBlock->getLastRealTreeTop()->getNode();
      TR::Block* gotoBlock = insertGotoFallThroughBlock(
  ...
  if (reorderingStats)
    fprintf(stderr,
            "%d replication candidates in method %s (has %d blocks)\n",
            numberMethodReplicationCandidates,
            comp()->signature(),
            numberOfBlocks);
...
bool
TR_OrderBlocks::doBlockExtension()
{
...
TR_OrderBlocks::perform()
{
  ...
  if (_reorderBlocks &&
      performTransformation(
        comp(), "%s Propagating coldness information\n", OPT_DETAILS))
...
checkOrderingConsistency(TR::Compilation* comp)
{
  ...
  if (debugConsistencyCheck)
    fprintf(stderr,
            "Checking ordering consistency for method %s\n",
            comp->signature());
...
void
TR_BlockOrderingOptimization::dumpBlockOrdering(TR::TreeTop* tt, char* title)
{
...
TR::Block**
TR_BlockShuffling::allocateBlockArray()
{
...
void
TR_BlockShuffling::traceBlocks(TR::Block** blocks)
{
...
void
TR_BlockShuffling::scramble(TR::Block** blocks)
{
  if (!performTransformation(
...
void
TR_BlockShuffling::riffle(TR::Block** blocks)
{
  ...
  while (upper < split || lower < _numBlocks) {
    ...
    if (decision < 0) {
      if (performTransformation(
            ...
            target,
            upper,
            originalBlocks[upper]->getNumber()))
      ...
      if (performTransformation(
            ...
            target,
            lower,
            originalBlocks[lower]->getNumber()))
...
void
TR_BlockShuffling::reverse(TR::Block** blocks)
{
