...
bool TR_OrderBlocks::isCandidateReallyBetter(TR::CFGEdge* candEdge, TR::Compilation* comp)
{
  ...
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
    ...
    for (auto predSuccEdge = predSuccessors.begin(); predSuccEdge != predSuccessors.end(); ++predSuccEdge) {
      ...
      // predSuccEdge->getTo()->getNumber(), predSuccEdge->getFrequency()); traceMsg(comp, "cand edge %d -> %d freq
...
bool TR_OrderBlocks::candidateIsBetterSuccessorThanBest(TR::CFGEdge* candidateEdge, TR::CFGEdge* currentBestEdge)
{
  ...
  if (candidate->hasSuccessor(currentBest)) {
    if (candidate->getFrequency() > (prevBlock->getFrequency() - candidate->getFrequency())) {
      if (trace())
        traceMsg(comp(),
          "\t\thas has current best succ as a succ, detecting an if-then structure and making the if block my best "
          "choice\n");
      return true;
    }
  }
  ...
  for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
    ...
    if (pred != block) {
      ...
      if (predSuccEdge == successors.end()) {
        if (trace())
          traceMsg(
            comp(), "\t\tbut it has another predecessor %d for which it is the hottest successor\n", pred->getNumber());
        if (trace())
...
bool TR_OrderBlocks::peepHoleGotoToGoto(TR::CFG* cfg, TR::Block* block, TR::Node* gotoNode, TR::Block* destOfGoto,
  char* title, TR::BitVector& skippedGotoBlocks)
{
  ...
  if (destOfGoto->isGotoBlock(comp(), true)) {
    ...
    if (performTransformation(comp(),
          ...
          block->getNumber(), destOfGoto->getNumber(), finalGotoBlock->getNumber())) {
      ...
      if (parent && parent->isNaturalLoop() && (parent->getNumber() == destOfGoto->getNumber()))
        cfg->setStructure(0); // to avoid automatic structure repair creating an improper region due to temporary state
...
bool TR_OrderBlocks::peepHoleGotoToEmpty(
  TR::CFG* cfg, TR::Block* block, TR::Node* gotoNode, TR::Block* destOfGoto, char* title)
{
  if (comp()->getProfilingMode() == JitProfiling)
    return false;
  ...
  if (destOfGoto->isEmptyBlock()
    ...
    if (destFallThroughDest != NULL) {
      ...
      if (performTransformation(comp(),
        ...
        if (parent && parent->isNaturalLoop() && (parent->getNumber() == destOfGoto->getNumber()))
          cfg->setStructure(0); // to avoid automatic structure repair creating an improper region due to temporary
                                // state after adding the first edge
...
static bool peepHoleGotoToLoopHeader(TR::CFG* cfg, TR::Block* block, TR::Block* dest, char* title)
{
  ...
  if (destStructure && blockStructure) {
    ...
    if (parent && parent->asRegion() && destIsSibling && predBlockIsSibling && predBlock && predBlock->endsInBranch()
      ...
      if (!branchNode->isTheVirtualGuardForAGuardedInlinedCall() && (branchDest != dest) && blockIsFallThrough
        && (branchNode->getOpCodeValue() != TR::ibranch && branchNode->getOpCodeValue() != TR::mbranch)
        && performTransformation(cfg->comp(), "%s applied goto-loop header peephole for block_%d dest %d\n", title,
          block->getNumber(), dest->getNumber())) {
        success = true;
...
bool TR_OrderBlocks::peepHoleBranchToLoopHeader(
  TR::CFG* cfg, TR::Block* block, TR::Block* fallThrough, TR::Block* dest, char* title)
{
  bool success = false;
  TR_Structure* destStructure = dest->getStructureOf();
  ...
  if (destStructure && fallThroughStructure && blockStructure) {
    ...
    if (parent && parent->asRegion() && fallThroughIsSibling && !destIsSibling
      ...
      performTransformation(comp(), "%s applied loop header peephole for block_%d fall through %d dest %d\n", title,
        block->getNumber(), fallThrough->getNumber(), dest->getNumber())) {
      success = true;

      TR::TreeTop* fallThroughTT = fallThrough->getEntry();

      TR::Block* gotoBlock = insertGotoFallThroughBlock(dest->getEntry(), dest->getEntry()->getNode(), block, dest);
    ...
    if (!doNotRemove)
      ...
      if (takenBlock == blockAfterFallThrough
        ...
        if (fallThroughSuccessors.front()->getTo()->asBlock() != blockAfterFallThrough) {
          ...
          if (performTransformation(comp(),
                "%s in block_%d, branch taken dest (%d) is a block after a single goto, so redirecting to its goto's "
                "destination (%d)\n",
            ...
            if (!block->hasSuccessor(gotoDestBlock)) {
              ...
              block->getSuccessors().front()->setFrequency(std::min(block->getEdge(fallThroughBlock)->getFrequency(),
  ...
  while (takenBlock->isGotoBlock(comp(), true)) {
    ...
    if ((!takenBlock->getStructureOf() || !takenBlock->getStructureOf()->isLoopInvariantBlock())
      && performTransformation(comp(),
        ...
        block->getNumber(), takenBlock->getNumber(), takenGotoDestBlock->getNumber())) {
...
void TR_OrderBlocks::removeEmptyBlock(TR::CFG* cfg, TR::Block* block, char* title)
{
  ...
  if (performTransformation(comp(), "%s empty block_%d, redirecting edges around this block then removing it\n", title,
    ...
    if (!block->getSuccessors().empty()) {
      ...
      if (trace())
        traceMsg(
          comp(), "\t\t\tredirecting edges to block's fall-through successor %d\n", fallThroughBlock->getNumber());
  ...
  while (madeAChange) {
    ...
    if (prevBlock != NULL) {
      ...
      if (madeAChange && prevBlock->isEmptyBlock() && (prevBlock->hasExceptionPredecessors() == false)
        && comp()->getProfilingMode() != JitProfiling && prevBlock->isTargetOfJumpWhoseTargetCanBeChanged(comp())) {
  ...
  while (block != NULL || !(_hotPathList.isEmpty() && _coldPathList.isEmpty())) {
    ...
    if (bestSucc != NULL && (numCandidates != 1) && endPathAtBlock(block, bestSucc, cfg)
      ...
      if (!_superColdBlockOnly) {
        if (trace())
          traceMsg(
            comp(), "Choosing to end path here, block_% will be added into list\n", bestSucc->asBlock()->getNumber());
        ...
        if (trace())
          traceMsg(
            comp(), "Choosing to end path here, no need to add block_% into list\n", bestSucc->asBlock()->getNumber());
      }
      block = NULL;
    } else
...
TR::Block* TR_BlockOrderingOptimization::insertGotoFallThroughBlock(
  TR::TreeTop* fallThroughTT, TR::Node* node, TR::CFGNode* prevBlock, TR::CFGNode* origSucc, TR_RegionStructure* parent)
{
  ...
  while (!newBlockOrder.isEmpty()) {
    ...
    if (fallThroughTT != block->getEntry()) {
      ...
      if (prevBlock->getEntry() != NULL && fallThroughTT != NULL
        ...
        //     1) block is not a successor of prevBlock, but prevBlock's fall-through successor has already been placed
        //     elsewhere 2) block is a different successor block than the original fall-through
        //        similar to 5, but we can reverse the branch in this case
        //     3) block is the only successor of prevBlock, but prevBlock ends in a goto (straightening should clean-up
        //     later) 4) block is not a successor of prevBlock, but it begins the path we (arbitrarily?) chose to follow
        //     prevBlock 5) block is a successor of prevBlock, but it's not the original fall through (due to
        //     endPathAtBlock)
...
bool TR_OrderBlocks::doBlockExtension()
{
  ...
  while (tt != NULL) {
    ...
    if ((block->getPredecessors().size() == 1) && prevBlock->hasSuccessor(block)
      && prevBlock->canFallThroughToNextBlock() && !prevBlock->getLastRealTreeTop()->getNode()->getOpCode().isReturn()
  ...
  for (; block; prevBlock = block, block = block->getNextBlock()) {
    ...
    if (!block->isCold()) {
      if (seenColdBlock) {
        ...
        char* buffer
          = (char*)comp->trMemory()->allocateStackMemory((strlen(fmt) + strlen(comp->signature()) + 15) * sizeof(char));
    ...
    if (debugConsistencyCheck && prevFlowsToBlock && prevsHotterSuccessor != NULL) {
      if (rootStructure)
        fprintf(stderr, "Block %d(%d) doesn't look like the best successor compared to %d(%d)\n", block->getNumber(),
          block->getFrequency(), prevsHotterSuccessor->getNumber(), prevsHotterSuccessor->getFrequency());
      else
...
void TR_BlockShuffling::reverse(TR::Block** blocks)
{
  ...
  for (upper = 0, lower = _numBlocks - 1; upper < lower; upper++, lower--)
    if (performTransformation(comp(), "O^O BLOCK SHUFFLING:   swap [%3d] and [%3d] (block_%d and block_%d)\n", upper,
          lower, blocks[upper]->getNumber(), blocks[lower]->getNumber()))
