...
bool TR_OrderBlocks::endPathAtBlock(TR::CFGNode* block, TR::CFGNode* bestSucc, TR::CFG* cfg)
{
    ...
    if (_superColdBlockOnly) {
        if (block->asBlock()->isSuperCold() != bestSucc->asBlock()->isSuperCold() && block->asBlock()->getExit() && block->asBlock()->getExit()->getNextTreeTop()) {
            // end path if coldness changes
            if (trace())
                traceMsg(comp(), "\t\tEnd path because coldness changed from block_%d to block_%d block->asBlock()->getExit()=%p block->asBlock()->getExit()->getNextTreeTop()=%p\n", block->asBlock()->getNumber(), bestSucc->asBlock()->getNumber(), block->asBlock()->getExit(), block->asBlock()->getExit()->getNextTreeTop());
            return true;
        }
    }
...
bool TR_OrderBlocks::isCandidateReallyBetter(TR::CFGEdge* candEdge, TR::Compilation* comp)
{
    ...
    for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
        ...
        for (auto predSuccEdge = predSuccessors.begin(); predSuccEdge != predSuccessors.end(); ++predSuccEdge) {
            ...
            if (((*predSuccEdge) != candEdge) && ((float)(*predSuccEdge)->getFrequency() > (float)(numFactor * (float)candEdge->getFrequency())))
...
static bool isCandidateTheHottestSuccessor(TR::CFGEdge* candEdge, TR::Compilation* comp)
{
    ...
    if (candBlock && candBlock->getContainingLoop() && candBlock->getContainingLoop()->getNumber() == candBlock->getNumber())
        return true;
    if (candBlock)
    ///if (1)
    {
...
bool TR_OrderBlocks::candidateIsBetterSuccessorThanBest(TR::CFGEdge* candidateEdge, TR::CFGEdge* currentBestEdge)
{
        ...
        if (candidateEdge->getFrequency() >= 0) {
            if (candidateEdge->getFrequency() == currentBestEdge->getFrequency()) {
                ...
                if (lexicalSuccessorEntry != NULL && lexicalSuccessorEntry->getNode()->getBlock() == candidateEdge->getTo()) {
                    if (trace())
                        traceMsg(comp(), "\t\tis equally hot, but is currently the lexical successor, making it my best choice\n");
                    return true;
                }
            }
            if (candidateEdge->getFrequency() > currentBestEdge->getFrequency()) {
    ...
    if (candidate->hasSuccessor(currentBest)) {
        if (candidate->getFrequency() > (prevBlock->getFrequency() - candidate->getFrequency())) {
            if (trace())
                traceMsg(comp(), "\t\thas has current best succ as a succ, detecting an if-then structure and making the if block my best choice\n");
            return true;
        }
    }
...
TR::CFGNode* TR_OrderBlocks::chooseBestFallThroughSuccessor(TR::CFG* cfg, TR::CFGNode* block, int32_t& numCandidates)
{
    ...
    for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
        ...
        if (block->asBlock()->getExit() && block->asBlock()->getExit()->getNextTreeTop() && succBlock == block->asBlock()->getExit()->getNextTreeTop()->getNode()->getBlock()) {
        ...
        if (!_changeBlockOrderBasedOnHWProfile) {
            ...
            if (candBlock->isSuperCold()) {
                if (!_hotPathList.isEmpty()) {
                    ...
                    if (trace())
                        traceMsg(comp(), "\tcandidate %d is super cold, discard because there are still choices which are not so cold \n", theCandidate->getNumber());
                    return NULL;
                }
            }
        } else {
    ...
    for (TR::CFGEdge* succEdge = succCandEdgeIt.getFirst(); succEdge; succEdge = succCandEdgeIt.getNext()) {
        ...
        if (candidateIsBetterSuccessorThanBest(succEdge, bestSuccessorEdge) && isCandidateReallyBetter(succEdge, comp())) {
            bestSuccessorEdge = succEdge;
        } else {
            if (trace())
...
void TR_OrderBlocks::addRemainingSuccessorsToList(TR::CFGNode* block, TR::CFGNode* excludeBlock)
{
    ...
    for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
        ...
        if (succBlock != excludeBlock && succBlock->getVisitCount() != _visitCount && isCandidateTheHottestSuccessor(*succEdge, comp())) {
...
bool TR_OrderBlocks::peepHoleGotoToEmpty(TR::CFG* cfg, TR::Block* block, TR::Node* gotoNode, TR::Block* destOfGoto, char* title)
{
    ...
    if (destOfGoto->isEmptyBlock() && !(destOfGoto->getStructureOf() && destOfGoto->getStructureOf()->isLoopInvariantBlock()) && !(block->getStructureOf() && block->getStructureOf()->isLoopInvariantBlock())) {
...
static bool peepHoleGotoToLoopHeader(TR::CFG* cfg, TR::Block* block, TR::Block* dest, char* title)
{
    ...
    if (destStructure && blockStructure) {
        ...
        if (parent && parent->asRegion() && destIsSibling && predBlockIsSibling && predBlock && predBlock->endsInBranch() && (destStructure->getNumber() == parent->getNumber())) {
            ...
            if (!branchNode->isTheVirtualGuardForAGuardedInlinedCall() && (branchDest != dest) && blockIsFallThrough && (branchNode->getOpCodeValue() != TR::ibranch && branchNode->getOpCodeValue() != TR::mbranch) && performTransformation(cfg->comp(), "%s applied goto-loop header peephole for block_%d dest %d\n", title, block->getNumber(), dest->getNumber())) {
                success = true;
...
bool TR_OrderBlocks::peepHoleBranchToLoopHeader(TR::CFG* cfg, TR::Block* block, TR::Block* fallThrough, TR::Block* dest, char* title)
{
    ...
    if (destStructure && fallThroughStructure && blockStructure) {
        ...
        if (parent && parent->asRegion() && fallThroughIsSibling && !destIsSibling && (fallThroughStructure->getNumber() == parent->getNumber()) && !branchNode->isTheVirtualGuardForAGuardedInlinedCall() && (branchNode->getOpCodeValue() != TR::ibranch && branchNode->getOpCodeValue() != TR::mbranch) &&
            /* !dest->isCold() &&
 */
...
void TR_OrderBlocks::peepHoleBranchAroundSingleGoto(TR::CFG* cfg, TR::Block* block, char* title)
{
    ...
    if (!branchNode->isTheVirtualGuardForAGuardedInlinedCall() && fallThroughBlock->isGotoBlock(comp(), true) && (fallThroughBlock->getPredecessors().size() == 1) && fallThroughBlock->getExit()->getNextTreeTop()) {
        ...
        if (!disableSPMD) {
            ...
            if (blockStructure) {
                if (blockStructure->isLoopInvariantBlock()) {
                    if (trace())
                        traceMsg(comp(), "\t\tavoid redirecting a jump to a pre-header block %d for later SPMD optimization\n", blockAfterFallThrough->getNumber());
                    doNotRemove = true;
                }
            }
        }
...
void TR_OrderBlocks::peepHoleBranchBlock(TR::CFG* cfg, TR::Block* block, char* title)
{
    ...
    while (takenBlock->isGotoBlock(comp(), true)) {
        ...
        if ((!takenBlock->getStructureOf() || !takenBlock->getStructureOf()->isLoopInvariantBlock()) && performTransformation(comp(), "%s in block_%d, branch taken dest (%d) is a goto block, so redirecting to its destination (%d)\n", title, block->getNumber(), takenBlock->getNumber(), takenGotoDestBlock->getNumber())) {
...
void TR_OrderBlocks::removeEmptyBlock(TR::CFG* cfg, TR::Block* block, char* title)
{
    ...
    if (performTransformation(comp(), "%s empty block_%d, redirecting edges around this block then removing it\n", title, block->getNumber())) {
        bool isLoopEntryBlock = block->getStructureOf() && block->getStructureOf()->getContainingLoop() && block->getStructureOf()->getContainingLoop()->getEntryBlock() == block;
        if (!block->getSuccessors().empty()) {
            ...
            if (trace())
                traceMsg(comp(), "\t\tblock has no successors so no edges to redirect, just removing predecessor edges\n");
            for (auto predEdge = block->getPredecessors().begin(); predEdge != block->getPredecessors().end();) {
...
bool TR_OrderBlocks::doPeepHoleBlockCorrections(TR::Block* block, char* title)
{
    ...
    while (madeAChange) {
        ...
        if (prevBlock != NULL) {
            ...
            if (madeAChange && prevBlock->isEmptyBlock() && (prevBlock->hasExceptionPredecessors() == false) && comp()->getProfilingMode() != JitProfiling && prevBlock->isTargetOfJumpWhoseTargetCanBeChanged(comp())) {
...
void TR_OrderBlocks::generateNewOrder(TR_BlockList& newBlockOrder)
{
    ...
    while (block != NULL || !(_hotPathList.isEmpty() && _coldPathList.isEmpty())) {
        ...
        if (needBetterChoice(cfg, block, bestSucc) && performTransformation(comp(), "%s choose best successor for block_%d (freq:%d)\n", OPT_DETAILS, block->getNumber(), block->getFrequency()))
        ...
        if (bestSucc != NULL && (numCandidates != 1) && endPathAtBlock(block, bestSucc, cfg) && performTransformation(comp(), "%s Reordering blocks to optimize fall-through paths\n", OPT_DETAILS)) {
            if (!_superColdBlockOnly) {
                if (trace())
...
void TR_BlockOrderingOptimization::connectTreesAccordingToOrder(TR_BlockList& newBlockOrder)
{
    ...
    if (trace())
        traceMsg(comp(), "block =%d prevBlock->getEntry()->getNode()=%p methodSymbol->getFirstTreeTop()->getNode()=%p\n", prevBlock->getNumber(), prevBlock->getEntry()->getNode(), methodSymbol->getFirstTreeTop()->getNode());
    ...
    while (!newBlockOrder.isEmpty()) {
        ...
        if (fallThroughTT != block->getEntry()) {
            ...
            if (prevBlock->getEntry() != NULL && fallThroughTT != NULL && prevBlock->hasSuccessor(fallThroughTT->getNode()->getBlock())) {
                ...
                if (branchNode->getOpCodeValue() != TR::Goto && (!branchNode->getOpCode().isJumpWithMultipleTargets() || // don't worry about igotos
                    ...
                    if (!isSucc
                        ...
                        // reverse the branch
                        if (trace())
                            traceMsg(comp(), "\tdecided to reverse the branch at the end of %d\n", prevBlock->getNumber());
                        TR::Node* branchNode = prevBlock->getLastRealTreeTop()->getNode();
                        TR_ASSERT(branchNode->getOpCode().isBranch(), "expected branch at end of prevBlock to reverse, but found something else");
                        branchNode->reverseBranch(fallThroughTT);
                    }
    ...
    if (fallThroughTT != NULL) {
        ...
        if (prevBlock->hasSuccessor(origSucc) && !prevBlock->endsInGoto() && (maybeBranchNode == NULL || !(maybeBranchNode->getOpCode().isJumpWithMultipleTargets()) || (maybeBranchNode->getOpCode().isJumpWithMultipleTargets() && maybeBranchNode->getOpCode().isCall()))) {
            if (trace())
                traceMsg(comp(), "\tneed to add extra goto block so that %d will fall-through to %d\n", prevBlock->getNumber(), origSucc->getNumber());
            TR::Node* branchNode = prevBlock->getLastRealTreeTop()->getNode();
            TR::Block* gotoBlock = insertGotoFallThroughBlock(fallThroughTT, branchNode, prevBlock, origSucc);
...
bool TR_OrderBlocks::doBlockExtension()
{
   ...
   comp()->incOptIndex();
    ...
    while (tt != NULL) {
        ...
        if ((block->getPredecessors().size() == 1) && prevBlock->hasSuccessor(block) && prevBlock->canFallThroughToNextBlock() && !prevBlock->getLastRealTreeTop()->getNode()->getOpCode().isReturn() && !prevBlock->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets() && !(prevBlock->getLastRealTreeTop()->getNode()->getOpCodeValue() == TR::treetop && prevBlock->getLastRealTreeTop()->getNode()->getFirstChild()->getOpCode().isJumpWithMultipleTargets()) && !block->isOSRCodeBlock()) {
