...
TR_Dominators::TR_Dominators(TR::Compilation *c, bool post)
  ...
  for (i = _topDfNum; i > 1; i--) {
    ...
    if (trace())
      traceMsg(comp(), "   %sDominator of block_%d is block_%d\n",
               _postDominators ? "post-" : "", dominated->getNumber(),
               dominator->getNumber());
  }
  ...
  if (_topDfNum != _numNodes - 1) {
    if (_postDominators) {
      ...
      TR_ASSERT(false, "Unreachable block in the CFG %d %d", _topDfNum,
                _numNodes - 1);
  }
...
void TR_Dominators::findDominators(TR::Block *start) {
  ...
  for (i = 2; i <= _topDfNum; i++) {
    BBInfo &w = getInfo(i);
    if (w._idom != w._sdno) w._idom = getInfo(w._idom)._idom;
  }
}
...
int32_t TR_Dominators::eval(int32_t index) {
  ...
  if (bbInfo._ancestor == 0) return bbInfo._label;
  compress(index);
...
void TR_Dominators::BBInfo::print(TR_FrontEnd *fe, TR::FILE *pOutFile) {
  ...
  trfprintf(pOutFile,
            ...
            "_size=%d, _child=%d\n",
            _parent, _idom, _sdno, _ancestor, _label, _size, _child);
}
#endif
...
void TR_PostDominators::findControlDependents() {
  ...
  _directControlDependents = (TR_BitVector **)_region.allocate(
      nextNodeNumber * sizeof(TR_BitVector *));
  // Initialize the table of direct control dependents
  for (i = 0; i < nextNodeNumber; i++)
    _directControlDependents[i] = new (_region) TR_BitVector(
        nextNodeNumber,
...
int32_t TR_PostDominators::countBlocksControlled(int32_t block,
                                                 BitVector &seen) {
  int32_t number = 0;
