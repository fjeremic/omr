...
TR_Dominators::TR_Dominators(TR::Compilation* c, bool post)
  ...
  , _dominators(c->getFlowGraph()->getNextNodeNumber() + 1,
                static_cast<TR::Block*>(NULL),
                _region)
{
  ...
  if (trace()) {
    traceMsg(comp(),
             "Starting %sdominator calculation\n",
             _postDominators ? "post-" : "");
  ...
  for (i = _topDfNum; i > 1; i--) {
    ...
    if (trace())
      traceMsg(comp(),
               "   %sDominator of block_%d is block_%d\n",
               _postDominators ? "post-" : "",
               dominated->getNumber(),
               dominator->getNumber());
  }
  ...
  if (_topDfNum != _numNodes - 1) {
    if (_postDominators) {
      ...
      if (trace())
        traceMsg(comp(),
                 "Some blocks are not reachable from exit. Post-dominator info "
                 "is invalid.\n");
      return;
    } else
      TR_ASSERT(
        false, "Unreachable block in the CFG %d %d", _topDfNum, _numNodes - 1);
  }

#if DEBUG
  for (block = toBlock(cfg->getFirstNode()); block;
       block = toBlock(block->getNext())) {
    TR_ASSERT(_dfNumbers[block->getNumber()] >= 0,
              "Unreachable block in the CFG");
  }
#endif

  if (trace())
    traceMsg(comp(),
             "End of %sdominator calculation\n",
             _postDominators ? "post-" : "");
...
TR::Block*
TR_Dominators::getDominator(TR::Block* block)
{
...
int
TR_Dominators::dominates(TR::Block* block, TR::Block* other)
{

  if (other == block)
    return 1;
  for (TR::Block* d = other; d != NULL && _dfNumbers[d->getNumber()] >=
                                            _dfNumbers[block->getNumber()];
       d = getDominator(d)) {
    if (d == block)
      return 1;
  }
...
void
TR_Dominators::findDominators(TR::Block* start)
{
  int32_t i;
...
void
TR_Dominators::initialize(TR::Block* start, BBInfo* nullParent)
{
  ...
  /*
   * Seed an initial list of successors using a dummy loop edge connecting the
   * start to itself.
   */
  ...
  while (!stack.empty()) {
    ...
    stack.pop_back();
    TR_ASSERT(current.listPosition != current.list.end(),
              "Successor list has already been exhausted.");
    TR::CFGNode* succ = _postDominators ? (*current.listPosition)->getFrom()
                                        : (*current.listPosition)->getTo();
      ...
      if (next != current.list.end()) {
        if (trace()) {
          traceMsg(
            ...
            toBlock(_postDominators ? (*next)->getFrom() : (*next)->getTo())
              ->getNumber(),
            stack.size());
        }
    ...
    if (trace())
      traceMsg(comp(),
               "At level %d block_%d becomes block_%d\n",
               stack.size(),
               block->getNumber(),
               _topDfNum);
    ...
    StackInfo::list_type& exceptionSuccessors =
      _postDominators ? block->getExceptionPredecessors()
                      : block->getExceptionSuccessors();
    StackInfo::iterator_type firstExceptionSuccessor =
      exceptionSuccessors.begin();
    if (firstExceptionSuccessor != exceptionSuccessors.end()) {
      if (trace()) {
        traceMsg(comp(),
                 "Insert block_%d at level %d\n",
                 toBlock(_postDominators ? (*firstExceptionSuccessor)->getFrom()
                                         : (*firstExceptionSuccessor)->getTo())
                   ->getNumber(),
                 stack.size());
      }

      stack.push_back(
        StackInfo(exceptionSuccessors, firstExceptionSuccessor, _topDfNum));
    }

    StackInfo::list_type& successors =
      _postDominators ? block->getPredecessors() : block->getSuccessors();
    StackInfo::iterator_type firstSuccessor = successors.begin();
    if (firstSuccessor != successors.end()) {
      if (trace()) {
        traceMsg(comp(),
                 "Insert block_%d at level %d\n",
                 toBlock(_postDominators ? (*firstSuccessor)->getFrom()
                                         : (*firstSuccessor)->getTo())
                   ->getNumber(),
                 stack.size());
      }
...
// Determine the ancestor of the block at the given index whose semidominator
// has the minimal depth-first number.
//
int32_t
TR_Dominators::eval(int32_t index)
{
...
// Compress ancestor path of the block at the given index to the block whose
// label has the maximal semidominator number.
//
void
TR_Dominators::compress(int32_t index)
{
  BBInfo& bbInfo = getInfo(index);
  BBInfo& ancestor = getInfo(bbInfo._ancestor);
...
//
void
TR_Dominators::link(int32_t parentIndex, int32_t childIndex)
{
...
#ifdef DEBUG
void
TR_Dominators::BBInfo::print(TR_FrontEnd* fe, TR::FILE* pOutFile)
{
  if (pOutFile == NULL)
    return;
  trfprintf(pOutFile, "BBInfo %d:\n", getIndex());
  trfprintf(pOutFile,
            "   _parent=%d, _idom=%d, _sdno=%d, _ancestor=%d, _label=%d, "
            "_size=%d, _child=%d\n",
            _parent,
            _idom,
            _sdno,
...
void
TR_PostDominators::findControlDependents()
{
  int32_t nextNodeNumber = _cfg->getNextNodeNumber();
  int32_t i;

  _directControlDependents =
    (TR_BitVector**)_region.allocate(nextNodeNumber * sizeof(TR_BitVector*));
  // Initialize the table of direct control dependents
  for (i = 0; i < nextNodeNumber; i++)
    _directControlDependents[i] = new (_region)
      TR_BitVector(nextNodeNumber,
                   _region); //.AddEntry(comp()->allocator("PostDominators"));
  ...
  if (trace()) {
    for (i = 0; i < nextNodeNumber; i++) {
      ...
      traceMsg(
        comp(), "} \t\t%d blocks in total\n", numberOfBlocksControlled(i));
    }
  }
}

int32_t
TR_PostDominators::numberOfBlocksControlled(int32_t block)
{
...
int32_t
TR_PostDominators::countBlocksControlled(int32_t block, BitVector& seen)
{
  int32_t number = 0;
