...
struct UnsafeSubexpressionRemoval // PR69613
{
  ...
  {
    TR_ASSERT(
      isVisited(node), "Cannot call isUnsafe on n%dn before anchorSafeChildrenOfUnsafeNodes", node->getGlobalIndex());
...
int32_t TR_IsolatedStoreElimination::performWithUseDefInfo()
{
  ...
  // "ShouldNotBeRemoved") should be set. The 4th bitvector is inTransit vector. So "NotToBeExamine should set the bits
  // in "Visited" and also on "ShouldNotBeRemoved". After finding the group set the right bits in "ToBeRemoved" or
  // "ShouldNotBeRemoved and update Visited vector. Also, emtpy InTransit vector
  //*******************************************************************************************
  ...
  if (tt) {
    ...
    if (node->getValueChild()->getOpCode().isBCDLoadVar() && node->getValueChild()->getOpCode().hasSymbolReference()
      ...
      for (TR::TreeTop* ttTemp = tt->getNextTreeTop(); ttTemp && (valueChild->getFutureUseCount() > 0);
        ...
        TR::Node* defNode
          = ttTemp->getNode()->getOpCodeValue() == TR::treetop ? ttTemp->getNode()->getFirstChild() : ttTemp->getNode();
  ...
  vcount_t visitCount, TR_BitVector* nodesInStructure, TR_BitVector* defsInStructure, bool* propagateRemovalToParent)
{
  ...
  if (canRemoveStructure) {
    ...
    if (regionStructure &&
      ...
      for (auto nextEdge = entryBlock->getPredecessors().begin(); nextEdge != entryBlock->getPredecessors().end();
        ...
        if (nextBlock != loopInvariantBlock->getBlock()) {
          ...
          if (loopTestNode->getOpCode().isIf()) {
            if ((loopTestNode->getOpCodeValue() == TR::ificmplt) || (loopTestNode->getOpCodeValue() == TR::iflcmplt)
              || (loopTestNode->getOpCodeValue() == TR::ificmple) || (loopTestNode->getOpCodeValue() == TR::iflcmple)) {
              ...
              || (loopTestNode->getOpCodeValue() == TR::iflcmpgt) || (loopTestNode->getOpCodeValue() == TR::ificmpge)
      ...
      if (symbolInCompare) {
        ...
        for (v = regionStructure->getFirstInductionVariable(); v; v = v->getNext()) {
          if ((v->getLocal()->getDataType() == TR::Int32)
            || // I have made int checks a bit weaker.. the code below doesn't make sense for unsigned operations // it
               // might make sense to guard this with a test that makes sure that the induction variable is being
               // checked with an signed comparison only
            ...
            if ((v->getLocal() == symbolInCompare)
              ...
              if (isIncreasing) {
                if (((v->getLocal()->getDataType() == TR::Int32) && (v->getIncr()->getLowInt() > 0))
                  || ((v->getLocal()->getDataType() == TR::Int64) && (v->getIncr()->getLowLong() > 0)))
      ...
      for (TR::TreeTop *treeTop = entryBlock->getEntry()->getNextTreeTop(), *next; (treeTop != entryBlock->getExit());
           treeTop = next) {
...
void TR_IsolatedStoreElimination::analyzeSingleBlockLoop(TR_RegionStructure* regionStructure, TR::Block* block)
{
  ...
  if (((lastTreeInLoopHeader->getOpCodeValue() == TR::ificmple)
    ...
    && inductionSymRef
    && (secondChildOfLastTree->getOpCode().isLoadConst() || regionStructure->isExprInvariant(secondChildOfLastTree))) {
                ...
                if (loopIncrement && (loopStride == 1)
                  ...
                  if ((lastTreeInLoopHeader->getOpCodeValue() == TR::ificmpgt)
                    ...
                    numIterations = TR::Node::create(TR::isub, 2,
                      TR::Node::createWithSymRef(secondChildOfLastTree, TR::iload, 0, inductionSymRef),
                      secondChildOfLastTree->duplicateTree());
                    if (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmpge)
                      numIterations = TR::Node::create(
                        TR::iadd, 2, numIterations, TR::Node::create(secondChildOfLastTree, TR::iconst, 0, 1));
                  } else {
                    numIterations = TR::Node::create(TR::isub, 2, secondChildOfLastTree->duplicateTree(),
                      TR::Node::createWithSymRef(secondChildOfLastTree, TR::iload, 0, inductionSymRef));
                    if (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmple)
                      numIterations = TR::Node::create(
                        TR::iadd, 2, numIterations, TR::Node::create(secondChildOfLastTree, TR::iconst, 0, 1));
      ...
      while (treeTop != exit) {
        ...
        if (node->getOpCode().isStoreDirect()) {
          ...
          if ((rhsOfStoreDefNode->getOpCode().getOpCodeValue() == TR::ixor)
            ...
            TR::Node* remNode
              = TR::Node::create(TR::irem, 2, numIterations, TR::Node::create(rhsOfStoreDefNode, TR::iconst, 0, 2));
  ...
  TR::Node* node, vcount_t visitCount, TR_BitVector* nodesSeen, TR_BitVector* defsSeen)
{
  ...
  if (node->getOpCode().isLikeDef() && node->getUseDefIndex()) {
    ...
    if (!(node->getOpCode().isStoreDirect() && node->getSymbolReference()->getSymbol()->isAutoOrParm()
      ...
      if (trace())
        traceMsg(
          comp(), "Marking useDefIndex %d as seendef at node n%dn\n", node->getUseDefIndex(), node->getGlobalIndex());
