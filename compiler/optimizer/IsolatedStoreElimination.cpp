...
TR_IsolatedStoreElimination::TR_IsolatedStoreElimination(
    ...
    : TR::Optimization(manager), _mustUseUseDefInfo(false),
      _deferUseDefInfoInvalidation(false), _currentTree(NULL),
      _usedSymbols(NULL), _storeNodes(NULL), _defParentOfUse(NULL),
      _defStatus(NULL), _groupsOfStoreNodes(NULL), _trivialDefs(NULL) {}
...
struct UnsafeSubexpressionRemoval // PR69613
{
  ...
  UnsafeSubexpressionRemoval(TR_IsolatedStoreElimination *ise)
      : _ise(ise), _visitedNodes(ise->comp()->getNodeCount(),
...
bool TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(
  ...
  if (structure->asRegion()) {
    ...
    if (!(nextBlock->getSuccessors().size() == 1) ||
      ...
      if (trace())
        traceMsg(comp(), "cannot remove structure, pred is empty or succ is "
                         "not singleton\n");
