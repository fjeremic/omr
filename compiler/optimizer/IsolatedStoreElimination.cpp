...
TR_IsolatedStoreElimination::TR_IsolatedStoreElimination(
...
struct UnsafeSubexpressionRemoval  // PR69613
{
  ...
  bool isVisited(TR::Node *node) {
    return _visitedNodes.isSet(node->getGlobalIndex());
  }
  bool isUnsafe(TR::Node *node) {
    TR_ASSERT(
        isVisited(node),
        "Cannot call isUnsafe on n%dn before anchorSafeChildrenOfUnsafeNodes",
        node->getGlobalIndex());
  ...
  UnsafeSubexpressionRemoval(TR_IsolatedStoreElimination *ise)
      : _ise(ise),
        _visitedNodes(ise->comp()->getNodeCount(), comp()->trMemory(),
                      stackAlloc),
        _unsafeNodes(ise->comp()->getNodeCount(), comp()->trMemory(),
                     stackAlloc) {}
  ...
  void anchorSafeChildrenOfUnsafeNodes(TR::Node *node,
                                       TR::TreeTop *anchorPoint) {
    if (isVisited(node))
      return;
    else
      _visitedNodes.set(node->getGlobalIndex());
    ...
    for (int32_t i = 0; i < node->getNumChildren(); ++i) {
      ...
      if (isUnsafe(child)) {
        ...
        if (trace()) {
          ...
          traceMsg(
              comp(),
              "        (Marked %s n%dn unsafe due to dead child #%d %s n%dn)\n",
  ...
  bool anchorIfSafe(TR::Node *node, TR::TreeTop *anchorPoint) {
    if (!isVisited(node)) anchorSafeChildrenOfUnsafeNodes(node, anchorPoint);

    if (isUnsafe(node)) {
      return false;
    } else {
      anchorPoint->insertBefore(
...
int32_t TR_IsolatedStoreElimination::perform() {
  ...
  _storeNodes = new (trStackMemory())
      TR_Array<TR::Node *>(trMemory(), 64, true, stackAlloc);
  ...
  if (useDefInfo) {
    if (trace())
      traceMsg(comp(),
               "Starting Global Store Elimination (using use/def info)\n");
    cost = performWithUseDefInfo();
  } else {
    if (trace())
      traceMsg(
          comp(),
          "Starting Global Store Elimination (without using use/def info)\n");
  ...
  if (useDefInfo) {
    ...
    for (uint32_t groupIndex = 0; groupIndex < _groupsOfStoreNodes->size();
         groupIndex++) {
      ...
      if (groupOfStores) {
        ...
        while (storeIter.hasMoreElements()) {
          ...
          for (useCursor.SetToFirstOne(); useCursor.Valid();
            ...
            int32_t useIndex =
                (int32_t)useCursor + useDefInfo->getFirstUseIndex();
    ...
    for (uint32_t groupIndex = 0; groupIndex < _groupsOfStoreNodes->size();
      ...
      if (groupOfStores &&
        ...
        while (bviUses.hasMoreElements()) {
          int32_t index =
              bviUses.getNextElement() + useDefInfo->getFirstDefIndex();
          ...
          dumpOptDetails(
              comp(), "%sRemoving %s n%dn [" POINTER_PRINTF_FORMAT "] %s\n",
              optDetailString(), node->getOpCode().getName(),
              node->getGlobalIndex(), node,
              node->getSymbolReference()->getSymbol()->isShadow() ? "(shadow)"
                                                                  : "");

#ifdef J9_PROJECT_SPECIFIC
          if (node->getOpCode().isBCDStore()) eliminatedBCDStore = true;
#endif

          _trivialDefs->reset(
              index);  // It's not a trivial def if we're handling it as a
                       // group.  If we try to do both, BOOM!
          ...
          // Eliminate the store
          //
          if (treeTop->getNode()->getOpCode().isSpineCheck() &&
              treeTop->getNode()->getFirstChild() == node) {
            ...
            if (usr.isUnsafe(child)) {
              ...
              TR::Node *dummyChild = node->setAndIncChild(
                  0, TR::Node::createConstDead(child, TR::Int32,
                                               0xbad1 /* eyecatcher */));
              if (trace())
                traceMsg(
                    comp(),
                    "  - replace unsafe child %s n%dn with dummy %s n%dn\n",
                    child->getOpCode().getName(), child->getGlobalIndex(),
                    dummyChild->getOpCode().getName(),
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (node) {
        ...
        if (useDefInfo->getUseDef(defsOfRhs,
                                  node->getFirstChild()->getUseDefIndex()) &&
            useDefInfo->getUsesFromDef(nodeUses, useDefIndex)) {
          TR_UseDefInfo::BitVector::Cursor cursor1(defsOfRhs);
          for (cursor1.SetToFirstOne(); cursor1.Valid();
               cursor1.SetToNextOne()) {
            int32_t nextDef = cursor1;
            TR_UseDefInfo::BitVector::Cursor cursor2(nodeUses);
            for (cursor2.SetToFirstOne(); cursor2.Valid();
                 cursor2.SetToNextOne()) {
              int32_t nextUse =
                  (int32_t)cursor2 + useDefInfo->getFirstUseIndex();
    ...
    for (int32_t i = _storeNodes->size() - 1; i >= 0; --i) {
      ...
      if (node &&
        ...
        if (useDefInfo) useDefInfo->clearNode(node->getUseDefIndex());
        if (node->getReferenceCount() < 1) {
          node->setFlags(0);
          TR::Node::recreate(node, TR::treetop);
        } else
  ...
  if (eliminatedStore) {
    ...
    if (eliminatedBCDStore)
      requestOpt(OMR::treeSimplification);  // some of the more complex BCD
                                            // simplifications work best when
                                            // refCounts == 1
#endif
  ...
  if (_deferUseDefInfoInvalidation) {
    _deferUseDefInfoInvalidation = false;
    // useDefInfo was invalidated (deferred) so invalidate it now and release
    // its memory
...
bool TR_IsolatedStoreElimination::canRemoveStoreNode(TR::Node *node) {
  ...
  if (node->getSymbolReference()->getSymbol()->isVolatile()) return false;

  if (node->dontEliminateStores()) return false;
      ...
      if (useDefInfo->getUseDef(defs, useIndex)) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
          ...
          if (defIndex >= useDefInfo->getFirstRealDefIndex() &&
              ...
              defNode->getFirstChild()->getSymbolReference() ==
                  node->getSymbolReference()) {
...
int32_t TR_IsolatedStoreElimination::performWithUseDefInfo() {
  ...
  _defParentOfUse = new (trStackMemory())
      TR_Array<int32_t>(trMemory(), info->getNumUseNodes(), true, stackAlloc);
  for (i = 0; i < info->getNumUseNodes(); i++) _defParentOfUse->add(-1);
  //*******************************************************************************************
  ...
  // For those that were visited the corresponding bit in one of the bitvectors
  // for ("ToBeRemoved" and "ShouldNotBeRemoved") should be set. The 4th
  // bitvector is inTransit vector. So "NotToBeExamine should set the bits in
  // "Visited" and also on "ShouldNotBeRemoved". After finding the group set the
  // right bits in "ToBeRemoved" or "ShouldNotBeRemoved and update Visited
  // vector. Also, emtpy InTransit vector
  //*******************************************************************************************

  _defStatus = new (trStackMemory()) TR_Array<defStatus>(
      trMemory(), info->getNumDefOnlyNodes(), true, stackAlloc);
  _defStatus->setSize(info->getNumDefOnlyNodes());

  _trivialDefs = new (trStackMemory())
      TR_BitVector(info->getNumDefOnlyNodes(), trMemory(), stackAlloc);

  _groupsOfStoreNodes = new (trStackMemory())
      TR_Array<TR_BitVector *>(trMemory(), 4, true, stackAlloc);
  ...
  for (i = info->getNumDefOnlyNodes() - 1; i >= 0; --i) {
    ...
    if (node && node->getOpCode().isStore()) {
      ...
      if (sym->isAuto() || sym->isParm() || sym->isAutoField() ||
          sym->isParmField() ||
  ...
  for (i = info->getNumDefOnlyNodes() - 1; i >= 0; --i) {
    if (_defStatus->element(i) == notVisited) {
      ...
      foundGroupOfStores = groupIsolatedStores(i, &currentGroupOfStores,
      ...
      if (foundGroupOfStores) {
        TR_BitVector *newGroupOfStoresToRemove = new (trStackMemory())
...
void TR_IsolatedStoreElimination::collectDefParentInfo(int32_t defIndex,
                                                       TR::Node *node,
                                                       TR_UseDefInfo *info) {
...
static void examChildrenForValueNode(TR::TreeTop *tt, TR::Node *parent,
                                     TR::Node *valueNode, vcount_t visitCount) {
  if (parent->getVisitCount() == visitCount) return;
  ...
  for (int32_t childCount = 0; childCount < parent->getNumChildren();
    ...
    if (child == valueNode) child->decFutureUseCount();
...
bool TR_IsolatedStoreElimination::groupIsolatedStores(
  ...
  if (status == inTransit || status == toBeRemoved) {
    if (trace())
      traceMsg(comp(),
               "groupIsolated - DEF %d is inTransit or toBeRemoved - \n",
               defIndex);
    return true;
  ...
  if (tt) {
    ...
    if (node->getValueChild()->getOpCode().isBCDLoadVar() &&
      ...
      for (TR::TreeTop *ttTemp = tt->getNextTreeTop();
           ttTemp && (valueChild->getFutureUseCount() > 0);
           ttTemp = ttTemp->getNextTreeTop()) {
        TR::Node *defNode = ttTemp->getNode()->getOpCodeValue() == TR::treetop
                                ? ttTemp->getNode()->getFirstChild()
                                : ttTemp->getNode();
        bool defNodeHasSymRef = defNode->getOpCode().hasSymbolReference() &&
                                defNode->getSymbolReference();
        if (defNodeHasSymRef && defNode->getSymbolReference()->canKill(
                                    valueChild->getSymbolReference())) {
          if (trace())
            traceMsg(comp(),
                     "%s groupIsolated - DEF %d cannot be removed due to "
                     "InMemoryLoadStoreMarking \n",
  ...
  // Test that all uses of the def are under other store nodes (i.e. set in the
  // defParent array). If even one of the uses is not under store node, then it
  // imposible to remove this def, and thus return.

  TR_UseDefInfo::BitVector usesOfThisDef(comp()->allocator());
  if (!info->getUsesFromDef(usesOfThisDef,
                            defIndex + info->getFirstDefIndex())) {
    if (trace())
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (trace())
      traceMsg(comp(),
               "groupIsolated - recursing for Def %d (parent of %d) - \n",
...
int32_t TR_IsolatedStoreElimination::performWithoutUseDefInfo() {
  ...
  _usedSymbols =
      new (trStackMemory()) TR_BitVector(numSymbols, trMemory(), stackAlloc);
  ...
  for (i = _storeNodes->size() - 1; i >= 0; --i) {
    TR::Node *node = _storeNodes->element(i);
    if (node && _usedSymbols->get(
                    node->getSymbolReference()->getSymbol()->getLocalIndex()))
...
void TR_IsolatedStoreElimination::examineNode(
    TR::Node *node, vcount_t visitCount, bool seenMultiplyReferencedParent) {
  if (node->getVisitCount() == visitCount) return;
  node->setVisitCount(visitCount);

  if (node->getReferenceCount() > 1) seenMultiplyReferencedParent = true;
  ...
  if (symRef == NULL) return;
  TR::Symbol *sym = symRef->getSymbol();
  if (!sym || !sym->getLocalIndex()) return;
  ...
  if (node->getOpCode().isStore()) {
    if (!_usedSymbols->get(sym->getLocalIndex())) {
      ...
      //
      if (canRemoveStoreNode(node)) _storeNodes->add(node);
    }
  } else {
    if (seenMultiplyReferencedParent ||
        (!(_currentTree->getNode()->getOpCode().isStore() &&
           (_currentTree->getNode()->getSymbolReference()->getSymbol() ==
            sym)))) {
...
void TR_IsolatedStoreElimination::performDeadStructureRemoval(
    TR_UseDefInfo *info) {
  TR::CFG *cfg = comp()->getFlowGraph();

  if (info->getTotalNodes() > MAX_TOTAL_NODES ||
      cfg->getNumberOfNodes() > MAX_CFG_SIZE)
    return;
  ...
  TR_BitVector *nodesInStructure =
      new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc, growable);
  TR_BitVector *defsInStructure =
      new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc, growable);

  findStructuresAndNodesUsedIn(info, rootStructure, visitCount,
                               nodesInStructure, defsInStructure,
                               &propagateRemovalToParent);
}

bool TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(
    TR_UseDefInfo *info, TR_Structure *structure, vcount_t visitCount,
    TR_BitVector *nodesInStructure, TR_BitVector *defsInStructure,
    bool *propagateRemovalToParent) {
  ...
  if (structure->asRegion()) {
    ...
    if (onlyExitEdge == -1) {
      ...
      // If the parent can be removed then it doesn't matter how many exit edges
      // there are for this structure.
      *propagateRemovalToParent = true;
      if (regionStructure->getExitEdges().isDoubleton()) {
        TR::CFGNode *firstNode =
            regionStructure->getExitEdges().getListHead()->getData()->getTo();

        TR::CFGNode *secondNode = regionStructure->getExitEdges()
                                      .getLastElement()
                                      ->getData()
                                      ->getTo();
        TR::CFG *cfg = comp()->getFlowGraph();
        ...
        for (TR::CFGNode *node = cfg->getFirstNode(); node;
          ...
          if (firstExitBlock && secondExitBlock) break;
        }
        ...
        if (firstExitBlock->isGotoBlock(comp()) &&
            (firstExitBlock->getSuccessors()
                 .front()
                 ->getTo()
                 ->asBlock()
                 ->getNumber() == secondExitBlock->getNumber()))
          onlyExitEdge = secondExitBlock->getNumber();
        else if (secondExitBlock->isGotoBlock(comp()) &&
                 (secondExitBlock->getSuccessors()
                      .front()
                      ->getTo()
                      ->asBlock()
                      ->getNumber() == firstExitBlock->getNumber()))
          onlyExitEdge = firstExitBlock->getNumber();

        // found one target, no need to propagate
        if (onlyExitEdge != -1) *propagateRemovalToParent = false;
      }
    }

    bool subStructureHasSideEffect = false;

    TR_BitVector *nodesInSubStructure =
        new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc, growable);
    TR_BitVector *defsInSubStructure =
        new (trStackMemory()) TR_BitVector(0, trMemory(), stackAlloc, growable);
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
      ...
      if (findStructuresAndNodesUsedIn(info, subStruct, visitCount,
                                       nodesInSubStructure, defsInSubStructure,
                                       &toPropagateRemoval)) {
    ...
    if (!(nextBlock->getSuccessors().size() == 1) ||
      ...
      if (trace())
        traceMsg(comp(),
                 "cannot remove structure, pred is empty or succ is not "
                 "singleton\n");
    ...
    while (currentTree != exitTree) {
      ...
      if (markNodesAndLocateSideEffectIn(currentNode, visitCount,
                                         nodesInStructure, defsInStructure))
        hasSideEffects = true;
  ...
  if (canRemoveStructure) {
    ...
    while (canRemoveStructure && defs.hasMoreElements()) {
      ...
      if (trace()) traceMsg(comp(), "Checking defIndex %d\n", defIndex);
      ...
      for (useCursor.SetToFirstOne(); canRemoveStructure && useCursor.Valid();
           useCursor.SetToNextOne()) {
        ...
        if (useNode && useNode->getReferenceCount() > 0 &&
            !nodesInStructure->get(useNode->getUseDefIndex())) {
          if (trace()) {
            if (structure->asRegion())
              traceMsg(comp(),
                       "Use Node %d invalidates region structure %d - will "
          ...
          canRemoveStructure = false;
          if (structure->asRegion()) *propagateRemovalToParent = true;
        }
      }
    }
    // if we can remove the strucutre then these defs are ok here and in all
    // parent structures so we don't need to check them again
    if (canRemoveStructure) defsInStructure->empty();
  }

  TR_RegionStructure *regionStructure = structure->asRegion();
  if (canRemoveStructure) {
    if (*propagateRemovalToParent) return true;
    ...
    if (regionStructure &&
      ...
      for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
        if (subNode->getNumber() == regionStructure->getNumber()) break;
      }
      ...
      if (!loopInvariantBlock) return true;
        ...
        if (nextBlock != loopInvariantBlock->getBlock()) {
          ...
          if (loopTestNode->getOpCode().isIf()) {
            if ((loopTestNode->getOpCodeValue() == TR::ificmplt) ||
              ...
              if (loopTestNode->getBranchDestination()->getNode()->getBlock() ==
                  entryBlock)
                isIncreasing = true;
      ...
      while (childInCompare->getOpCode().isConversion() ||
             childInCompare->getOpCode().isAdd() ||
      ...
      if (symbolInCompare) {
        ...
        for (v = regionStructure->getFirstInductionVariable(); v;
             v = v->getNext()) {
          if ((v->getLocal()->getDataType() ==
               TR::Int32) ||  // I have made int checks a bit weaker.. the code
                              // below doesn't make sense for unsigned
                              // operations // it might make sense to guard this
                              // with a test that makes sure that the induction
                              // variable is being checked with an signed
                              // comparison only
        ...
        if (!foundProperIndVar) return true;
      } else
        return true;
      ...
      for (TR::TreeTop *treeTop = entryBlock->getEntry()->getNextTreeTop(),
                       *next;
           (treeTop != entryBlock->getExit()); treeTop = next) {
        next = treeTop->getNextTreeTop();
        TR::TransformUtil::removeTree(comp(), treeTop);
        if (next == entryBlock->getExit()) break;
      }

      if (targetExitBlock->getEntry() !=
          entryBlock->getExit()->getNextTreeTop()) {
        TR::TreeTop *gotoBlockEntryTree = entryBlock->getEntry();
        TR::TreeTop *gotoBlockExitTree = entryBlock->getExit();
        TR::Node *gotoNode =
            TR::Node::create(entryBlock->getEntry()->getNode(), TR::Goto);
        TR::TreeTop *gotoTree =
      ...
      TR::CFGEdge *newEdge =
          TR::CFGEdge::createEdge(entryBlock, targetExitBlock, trMemory());
      if (!entryBlock->hasSuccessor(targetExitBlock)) cfg->addEdge(newEdge);

      for (auto edge = entryBlock->getSuccessors().begin();
           edge != entryBlock->getSuccessors().end();) {
        if ((*edge) != newEdge)
          cfg->removeEdge(*(edge++));
        else
          ++edge;
      ...
      // structure->asBlock()->getNumber()); printf("Found a removable block in
      // %s\n", signature(comp()->getCurrentMethod()));
    }
  } else if (regionStructure && !regionStructure->isAcyclic()) {
    if (trace()) traceMsg(comp(), "region is not acyclic\n");
    return true;
  }
...
static TR::Node *getLoopIncrementStep(TR::Block *block,
                                      TR::SymbolReference *symRef,
                                      bool shouldBeSub, int32_t *loopStride) {
  ...
  while (currentTree != exitTree) {
    TR::Node *currentNode = currentTree->getNode();
    if (currentNode->getOpCode().isStoreDirect() &&
        (currentNode->getSymbolReference() == symRef)) {
      if (seenStore) return NULL;
...
void TR_IsolatedStoreElimination::analyzeSingleBlockLoop(
    TR_RegionStructure *regionStructure, TR::Block *block) {
  TR_InductionVariable *v;
      ...
      if (block->getPredecessors().size() == 2) {
        for (auto nextEdge = block->getPredecessors().begin();
          ...
          if ((pred != block) && (pred->getSuccessors().size() == 1)) {
            ...
            // if (lastRealNode->getOpCode().isStoreDirect() &&
            // (lastRealNode->getSymbolReference() == inductionSymRef))
            {
              {
                ...
                TR::Node *loopIncrement = getLoopIncrementStep(
                    block, inductionSymRef, shouldBeSub, &loopStride);
                ...
                if (loopIncrement && (loopStride == 1) &&
                    ((loopIncrement->getFirstChild() == firstChildOfLastTree) ||
                     (firstChildOfLastTree->getOpCode().isLoadVarDirect() &&
                      ((firstChildOfLastTree->getSymbolReference()
                            ->getReferenceNumber() ==
                        inductionSymRef->getReferenceNumber()))))) {
                  // initializationBlock = pred;
                  // isPredictableLoop = true;
                  if ((lastTreeInLoopHeader->getOpCodeValue() ==
                       TR::ificmpgt) ||
                      (lastTreeInLoopHeader->getOpCodeValue() ==
                       TR::ificmpge)) {
                    numIterations = TR::Node::create(
                        TR::isub, 2,
                        TR::Node::createWithSymRef(secondChildOfLastTree,
                                                   TR::iload, 0,
                                                   inductionSymRef),
                        secondChildOfLastTree->duplicateTree());
                    if (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmpge)
                      numIterations = TR::Node::create(
                          TR::iadd, 2, numIterations,
                          TR::Node::create(secondChildOfLastTree, TR::iconst, 0,
                                           1));
                  } else {
                    numIterations = TR::Node::create(
                        TR::isub, 2, secondChildOfLastTree->duplicateTree(),
                        TR::Node::createWithSymRef(secondChildOfLastTree,
                                                   TR::iload, 0,
                                                   inductionSymRef));
                    if (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmple)
      ...
      while (treeTop != exit) {
        ...
        if (node->getOpCode().isStoreDirect()) {
          ...
          if ((rhsOfStoreDefNode->getOpCode().getOpCodeValue() == TR::ixor) &&
              ...
              (rhsOfStoreDefNode->getFirstChild()
                   ->getOpCode()
                   .hasSymbolReference() &&
               ((rhsOfStoreDefNode->getFirstChild()
                     ->getSymbolReference()
...
bool TR_IsolatedStoreElimination::markNodesAndLocateSideEffectIn(
    TR::Node *node, vcount_t visitCount, TR_BitVector *nodesSeen,
    TR_BitVector *defsSeen) {
  if (node->getVisitCount() == visitCount) return false;

  node->setVisitCount(visitCount);
  bool toReturn = false;

  if (node->exceptionsRaised() || nodeHasSideEffect(node) ||
      node->getOpCode().isReturn() || node->getOpCode().isLoadReg() ||
  ...
  if (node->getOpCode().isLikeDef() && node->getUseDefIndex()) {
    // we don't need to record defs marked as stored value is irrelevant - there
    // will be no uses of the dead values
    if (!(node->getOpCode().isStoreDirect() &&
  ...
  int32_t childNum;
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    if (markNodesAndLocateSideEffectIn(node->getChild(childNum), visitCount,
                                       nodesSeen, defsSeen))
      toReturn = true;
  }
...
const char *TR_IsolatedStoreElimination::optDetailString() const throw() {
