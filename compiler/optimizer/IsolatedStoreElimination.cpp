  ...
  void anchorSafeChildrenOfUnsafeNodes(TR::Node* node, TR::TreeTop* anchorPoint)
  {
    ...
    if (isUnsafe(node)) {
      for (int32_t i = 0; i < node->getNumChildren(); ++i) {
        ...
        if (didIt && trace()) {
          traceMsg(comp(),
                   "  - Anchored child #%d %s n%d of %s n%d\n",
                   i,
                   child->getOpCode().getName(),
                   child->getGlobalIndex(),
                   node->getOpCode().getName(),
...
int32_t
TR_IsolatedStoreElimination::perform()
{
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());

  _storeNodes =
    new (trStackMemory()) TR_Array<TR::Node*>(trMemory(), 64, true, stackAlloc);
  ...
  if (useDefInfo) {
    ...
    for (uint32_t groupIndex = 0; groupIndex < _groupsOfStoreNodes->size();
      ...
      if (trace())
        traceMsg(
          comp(), "  Scanning store group %d for dead uses\n", groupIndex);
      if (groupOfStores) {
        ...
        while (storeIter.hasMoreElements()) {
          ...
          if (trace()) {
            ...
            traceMsg(comp(),
                     "    Scanning def %d %s n%dn for dead uses\n",
                     defIndex,
    ...
    for (uint32_t groupIndex = 0; groupIndex < _groupsOfStoreNodes->size();
      ...
      if (groupOfStores &&
          performTransformation(
            comp(),
            "%sGlobal Store Elimination eliminating group %d:\n",
            optDetailString(),
            groupIndex)) {
        ...
        while (bviUses.hasMoreElements()) {
          ...
          dumpOptDetails(comp(),
                         "%sRemoving %s n%dn [" POINTER_PRINTF_FORMAT "] %s\n",
                         optDetailString(),
                         node->getOpCode().getName(),
                         node->getGlobalIndex(),
                         node,
          ...
          _trivialDefs->reset(
            index); // It's not a trivial def if we're handling it as a group.
                    // If we try to do both, BOOM!
          ...
          if (treeTop->getNode()->getOpCode().isSpineCheck() &&
            ...
            if (usr.isUnsafe(child)) {
              ...
              TR::Node* dummyChild = node->setAndIncChild(
                0,
                TR::Node::createConstDead(
                  child, TR::Int32, 0xbad1 /* eyecatcher */));
              if (trace())
                traceMsg(
                  comp(),
    ...
    while (bvi.hasMoreElements()) {
      ...
      if (node) {
        if (trace()) {
          traceMsg(comp(),
                   "removing trivial node %p %s n%dn udi=%d\n",
                   node,
                   node->getOpCode().getName(),
                   node->getGlobalIndex(),
                   useDefIndex);
    ...
    for (int32_t i = _storeNodes->size() - 1; i >= 0; --i) {
      ...
      if (node && performTransformation(
                    comp(),
                    "%s   Global Store Elimination eliminating : %p\n",
                    optDetailString(),
                    node)) {
        if (useDefInfo)
          useDefInfo->clearNode(node->getUseDefIndex());
...
bool
TR_IsolatedStoreElimination::canRemoveStoreNode(TR::Node* node)
{
  if (_currentTree)
...
void
TR_IsolatedStoreElimination::removeRedundantSpills()
{
...
int32_t
TR_IsolatedStoreElimination::performWithUseDefInfo()
{
    ...
    if (node && node->getOpCode().isStore()) {
      ...
      if (sym->isAuto() || sym->isParm() || sym->isAutoField() ||
        ...
        if (firstChild->getOpCode().isLoadVarDirect() &&
          ...
          if (trace())
            traceMsg(comp(),
                     "Found trivial node %p %s n%dn udi=%d\n",
                     node,
                     node->getOpCode().getName(),
                     node->getGlobalIndex(),
                     i);
  ...
  for (i = info->getNumDefOnlyNodes() - 1; i >= 0; --i) {
    if (_defStatus->element(i) == notVisited) {
      ...
      foundGroupOfStores = groupIsolatedStores(
        i, &currentGroupOfStores, optimizer()->getUseDefInfo());
...
void
TR_IsolatedStoreElimination::collectDefParentInfo(int32_t defIndex,
                                                  TR::Node* node,
                                                  TR_UseDefInfo* info)
{
  if (node->getReferenceCount() > 1)
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    ...
    if ((child->getReferenceCount() == 1) && child->getOpCode().isLoadVar()) {
      ...
      if (index > 0) // parm or auto
      {
        ...
        if (trace())
          traceMsg(
            comp(), "DefParent - use %d has parent %d\n", useIndex, defIndex);
      }
    }
...
static void
examChildrenForValueNode(TR::TreeTop* tt,
                         TR::Node* parent,
                         TR::Node* valueNode,
                         vcount_t visitCount)
{
...
bool
TR_IsolatedStoreElimination::groupIsolatedStores(
  int32_t defIndex,
  TR_BitVector* currentGroupOfStores,
  TR_UseDefInfo* info)
{
  ...
  if (status == inTransit || status == toBeRemoved) {
    ...
    if (trace())
      traceMsg(
        comp(), "groupIsolated - DEF %d is notToBeRemoved - \n", defIndex);
    return false;
    ...
    if (trace())
      traceMsg(
        comp(), "groupIsolated - DEF %d is now investigated - \n", defIndex);
  }
  ...
  if (!info->getUsesFromDef(usesOfThisDef,
    ...
    if (trace())
      traceMsg(comp(),
               "groupIsolated - DEF %d has no uses - can be removed \n",
               defIndex);
    return true;
  } else {
    if (trace()) {
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (_defParentOfUse->element(useIndex) == -1) {
      if (trace())
        traceMsg(
          comp(), "groupIsolated - Use %d has no def parent - \n", useIndex);
      return false;
    }
  }
...
//------------------------------------------------------------------------
int32_t
TR_IsolatedStoreElimination::performWithoutUseDefInfo()
{
...
void
TR_IsolatedStoreElimination::examineNode(TR::Node* node,
                                         vcount_t visitCount,
                                         bool seenMultiplyReferencedParent)
{
...
void
TR_IsolatedStoreElimination::performDeadStructureRemoval(TR_UseDefInfo* info)
{
  ...
  findStructuresAndNodesUsedIn(info,
                               rootStructure,
                               visitCount,
                               nodesInStructure,
                               defsInStructure,
...
bool
TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(
  TR_UseDefInfo* info,
  ...
  bool* propagateRemovalToParent)
{
  ...
  if (trace()) {
    if (structure->asRegion())
      traceMsg(
        comp(), "Inspecting region structure %d\n", structure->getNumber());
    else
      traceMsg(
        comp(), "Inspecting block structure %d\n", structure->getNumber());
  }
  ...
  if (structure->asRegion()) {
    ...
    if (onlyExitEdge == -1) {
      ...
      *propagateRemovalToParent = true;
      if (regionStructure->getExitEdges().isDoubleton()) {
        ...
        TR::CFGNode* secondNode =
          regionStructure->getExitEdges().getLastElement()->getData()->getTo();
        TR::CFG* cfg = comp()->getFlowGraph();
    ...
    for (subNode = si.getCurrent(); subNode != NULL; subNode = si.getNext()) {
      ...
      if (findStructuresAndNodesUsedIn(info,
                                       subStruct,
                                       visitCount,
                                       nodesInSubStructure,
                                       defsInSubStructure,
    ...
    if (subStructureHasSideEffect) {
      *propagateRemovalToParent = false;
      if (trace())
        traceMsg(
          comp(), "end of region %d (hasSideEffect)\n", structure->getNumber());
      return true;
    }
  } // if structure->asRegion()
  else {
    ...
    if (!(nextBlock->getSuccessors().size() == 1) ||
      ...
      if (trace())
        traceMsg(
          comp(),
          "cannot remove structure, pred is empty or succ is not singleton\n");
    ...
    while (currentTree != exitTree) {
      ...
      if (markNodesAndLocateSideEffectIn(
            currentNode, visitCount, nodesInStructure, defsInStructure))
        hasSideEffects = true;
        ...
        if (useNode && useNode->getReferenceCount() > 0 &&
          ...
          if (trace()) {
            if (structure->asRegion())
              ...
              traceMsg(comp(),
                       "Use Node %d invalidates block structure %d\n",
                       useIndex,
                       structure->getNumber());
          }
          canRemoveStructure = false;
        ...
        for (v = regionStructure->getFirstInductionVariable(); v;
          ...
          if ((v->getLocal()->getDataType() ==
               TR::Int32) || // I have made int checks a bit weaker.. the code
                             // below doesn't make sense for unsigned operations
                             // // it might make sense to guard this with a test
                             // that makes sure that the induction variable is
                             // being checked with an signed comparison only
...
static TR::Node*
getLoopIncrementStep(TR::Block* block,
...
void
TR_IsolatedStoreElimination::analyzeSingleBlockLoop(
                  ...
                  if ((lastTreeInLoopHeader->getOpCodeValue() ==
                    ...
                    if (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmpge)
                      numIterations = TR::Node::create(
                        ...
                        numIterations,
                        TR::Node::create(
                          secondChildOfLastTree, TR::iconst, 0, 1));
                  } else {
                    numIterations = TR::Node::create(
                      TR::isub,
                      2,
        ...
        if (node->getOpCode().isStoreDirect()) {
          ...
          if ((rhsOfStoreDefNode->getOpCode().getOpCodeValue() == TR::ixor) &&
              ...
              (rhsOfStoreDefNode->getFirstChild()
               ...
               ((rhsOfStoreDefNode->getFirstChild()
                   ->getSymbolReference()
                   ->getReferenceNumber() == symRef->getReferenceNumber())))) {
...
bool
TR_IsolatedStoreElimination::markNodesAndLocateSideEffectIn(
  TR::Node* node,
  vcount_t visitCount,
  TR_BitVector* nodesSeen,
  TR_BitVector* defsSeen)
{
  ...
  if (node->getOpCode().isLikeDef() && node->getUseDefIndex()) {
    ...
    if (!(node->getOpCode().isStoreDirect() &&
      ...
      if (trace())
        traceMsg(comp(),
                 "Marking useDefIndex %d as seendef at node n%dn\n",
                 node->getUseDefIndex(),
                 node->getGlobalIndex());
  ...
  for (childNum = 0; childNum < node->getNumChildren(); childNum++) {
    if (markNodesAndLocateSideEffectIn(
          node->getChild(childNum), visitCount, nodesSeen, defsSeen))
      toReturn = true;
  }
