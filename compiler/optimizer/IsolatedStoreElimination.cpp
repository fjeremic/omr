...
void TR_IsolatedStoreElimination::removeRedundantSpills()
{
    ...
    for (TR::TreeTop* tt = comp()->getStartTree(); tt != 0; tt = tt->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStoreDirect() && node->getFirstChild()->getOpCode().isLoadReg() && node->getSymbolReference() == node->getFirstChild()->getRegLoadStoreSymbolReference()) {
            ...
            if (useDefInfo->getUseDef(defs, useIndex)) {
                ...
                for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                    ...
                    if (defIndex >= useDefInfo->getFirstRealDefIndex() && (defNode = useDefInfo->getNode(defIndex)) && defNode->getOpCode().isStoreReg() && defNode->getFirstChild()->getOpCode().isLoadVarDirect() && defNode->getFirstChild()->getSymbolReference() == node->getSymbolReference()) {
                        // traceMsg(comp(), "Redundant store node=%p defNode=%p \n", node, defNode);
                    } else {
                        redundantStore = false;
                    }
                }
                if (redundantStore && performTransformation(comp(), "%s Removing redundant spill:  (%p)\n", optDetailString(), node)) {
                    TR::Node::recreate(node, TR::treetop);
                    node->setFlags(0);
                }
            }
...
int32_t TR_IsolatedStoreElimination::performWithUseDefInfo()
{
    ...
    for (i = info->getNumDefOnlyNodes() - 1; i >= 0; --i) {
        ...
        if (node && node->getOpCode().isStore()) {
            ...
            if (sym->isAuto() || sym->isParm() || sym->isAutoField() || sym->isParmField() || sym->isStatic() // the ones that reach exit will have it as  a use
            ) {
                //into array of the size of uses walking the defs
                collectDefParentInfo(useDefIndex, node, info);
                TR::Node* firstChild = node->getFirstChild();
                if (firstChild->getOpCode().isLoadVarDirect() && firstChild->getReferenceCount() == 1 && (firstChild->getSymbolReference() == node->getSymbolReference())) {
                    _trivialDefs->set(i);
                    if (trace())
...
bool TR_IsolatedStoreElimination::groupIsolatedStores(int32_t defIndex, TR_BitVector* currentGroupOfStores, TR_UseDefInfo* info)
{
    ...
    if (tt) {
        ...
        if (node->getValueChild()->getOpCode().isBCDLoadVar() && node->getValueChild()->getOpCode().hasSymbolReference() && comp()->cg()->IsInMemoryType(node->getValueChild()->getType()) && node->getValueChild()->getReferenceCount() > 1) {
...
void TR_IsolatedStoreElimination::examineNode(TR::Node* node, vcount_t visitCount, bool seenMultiplyReferencedParent)
{
    ...
    if (node->getOpCode().isStore()) {
        ...
        if (seenMultiplyReferencedParent || (!(_currentTree->getNode()->getOpCode().isStore() && (_currentTree->getNode()->getSymbolReference()->getSymbol() == sym)))) {
...
bool TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(TR_UseDefInfo* info, TR_Structure* structure, vcount_t visitCount, TR_BitVector* nodesInStructure, TR_BitVector* defsInStructure, bool* propagateRemovalToParent)
{
    ...
    if (structure->asRegion()) {
        ...
        if (regionStructure->isNaturalLoop() && regionStructure->numSubNodes() == 1 && (regionStructure->getParent() && regionStructure->getParent()->asRegion()->isCanonicalizedLoop())) {
        ...
        if (onlyExitEdge == -1) {
            ...
            *propagateRemovalToParent = true;
            if (regionStructure->getExitEdges().isDoubleton()) {
                ...
                if (firstExitBlock->isGotoBlock(comp()) && (firstExitBlock->getSuccessors().front()->getTo()->asBlock()->getNumber() == secondExitBlock->getNumber()))
                    onlyExitEdge = secondExitBlock->getNumber();
                else if (secondExitBlock->isGotoBlock(comp()) && (secondExitBlock->getSuccessors().front()->getTo()->asBlock()->getNumber() == firstExitBlock->getNumber()))
    ...
    if (canRemoveStructure) {
        ...
        if (regionStructure && regionStructure->isNaturalLoop() && (regionStructure->getParent() /* &&
            ...
            for (auto nextEdge = entryBlock->getPredecessors().begin(); nextEdge != entryBlock->getPredecessors().end(); ++nextEdge) {
                ...
                if (nextBlock != loopInvariantBlock->getBlock()) {
                    ...
                    if (loopTestNode->getOpCode().isIf()) {
                        if ((loopTestNode->getOpCodeValue() == TR::ificmplt) || (loopTestNode->getOpCodeValue() == TR::iflcmplt) || (loopTestNode->getOpCodeValue() == TR::ificmple) || (loopTestNode->getOpCodeValue() == TR::iflcmple)) {
                            if (loopTestNode->getBranchDestination()->getNode()->getBlock() == entryBlock)
                                isIncreasing = true;
                            loopTestTree = nextBlock->getLastRealTreeTop();
                        } else if ((loopTestNode->getOpCodeValue() == TR::ificmpgt) || (loopTestNode->getOpCodeValue() == TR::iflcmpgt) || (loopTestNode->getOpCodeValue() == TR::ificmpge) || (loopTestNode->getOpCodeValue() == TR::iflcmpge)) {
                            if (loopTestNode->getBranchDestination()->getNode()->getBlock() != entryBlock)
                                isIncreasing = true;
                            loopTestTree = nextBlock->getLastRealTreeTop();
                        }
            ...
            if (symbolInCompare) {
                ...
                for (v = regionStructure->getFirstInductionVariable(); v; v = v->getNext()) {
                    if ((v->getLocal()->getDataType() == TR::Int32) || // I have made int checks a bit weaker.. the code below doesn't make sense for unsigned operations // it might make sense to guard this with a test that makes sure that the induction variable is being checked with an signed comparison only
                        ...
                        if ((v->getLocal() == symbolInCompare) && (((v->getLocal()->getDataType() == TR::Int32) && (v->getIncr()->getLowInt() == v->getIncr()->getHighInt())) || ((v->getLocal()->getDataType() == TR::Int64) && (v->getIncr()->getLowLong() == v->getIncr()->getHighLong())))) {
                            if (isIncreasing) {
                                if (((v->getLocal()->getDataType() == TR::Int32) && (v->getIncr()->getLowInt() > 0)) || ((v->getLocal()->getDataType() == TR::Int64) && (v->getIncr()->getLowLong() > 0)))
...
static TR::Node* getLoopIncrementStep(TR::Block* block, TR::SymbolReference* symRef, bool shouldBeSub, int32_t* loopStride)
{
    ...
    while (currentTree != exitTree) {
        ...
        if (currentNode->getOpCode().isStoreDirect() && (currentNode->getSymbolReference() == symRef)) {
            ...
            if ((firstChild->getOpCode().isAdd() || firstChild->getOpCode().isSub()) && firstChild->getFirstChild()->getOpCode().isLoadVarDirect() && (firstChild->getFirstChild()->getSymbolReference() == symRef) && (firstChild->getSecondChild()->getOpCode().isLoadConst())) {
...
void TR_IsolatedStoreElimination::analyzeSingleBlockLoop(TR_RegionStructure* regionStructure, TR::Block* block)
{
    ...
    if (lastTreeInLoopHeader->getNumChildren() > 1) {
        ...
        if (!inductionSymRef) {
            ...
            else if ((nodeWithSymRef->getNumChildren() > 0) && nodeWithSymRef->getFirstChild()->getOpCode().hasSymbolReference())
    ...
    if (((lastTreeInLoopHeader->getOpCodeValue() == TR::ificmple) || (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmplt) || (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmpgt) || (lastTreeInLoopHeader->getOpCodeValue() == TR::ificmpge)) && inductionSymRef && (secondChildOfLastTree->getOpCode().isLoadConst() || regionStructure->isExprInvariant(secondChildOfLastTree))) {
                        ...
                        {
                            {
                                ...
                                if (loopIncrement && (loopStride == 1) && ((loopIncrement->getFirstChild() == firstChildOfLastTree) || (firstChildOfLastTree->getOpCode().isLoadVarDirect() && ((firstChildOfLastTree->getSymbolReference()->getReferenceNumber() == inductionSymRef->getReferenceNumber()))))) {
        ...
        if (numIterations && loopInvariantBlock) {
            ...
            while (treeTop != exit) {
                ...
                if (node->getOpCode().isStoreDirect()) {
                    ...
                    if ((rhsOfStoreDefNode->getOpCode().getOpCodeValue() == TR::ixor) && (rhsOfStoreDefNode->getReferenceCount() == 1) && rhsOfStoreDefNode->getSecondChild()->getOpCode().isLoadConst() && (rhsOfStoreDefNode->getSecondChild()->getInt() == 1) && (rhsOfStoreDefNode->getFirstChild()->getReferenceCount() == 1) && (rhsOfStoreDefNode->getFirstChild()->getOpCode().hasSymbolReference() && ((rhsOfStoreDefNode->getFirstChild()->getSymbolReference()->getReferenceNumber() == symRef->getReferenceNumber())))) {
...
nodeHasSideEffect(TR::Node* node)
{
    ...
    case TR::bitOpMem:
        return true;
    default:
        if (node->getOpCode().isCall() && node->getSymbolReference()->getSymbol()->getResolvedMethodSymbol() && !node->getSymbolReference()->getSymbol()->getResolvedMethodSymbol()->isSideEffectFree())
            return true;
    }
    return false;
}

bool TR_IsolatedStoreElimination::markNodesAndLocateSideEffectIn(TR::Node* node, vcount_t visitCount, TR_BitVector* nodesSeen, TR_BitVector* defsSeen)
{
    if (node->getVisitCount() == visitCount)
        return false;
    ...
    if (node->exceptionsRaised() || nodeHasSideEffect(node) || node->getOpCode().isReturn() || node->getOpCode().isLoadReg() || node->getOpCode().isStoreReg() || (node->getOpCode().hasSymbolReference() && node->getSymbolReference()->getSymbol()->isVolatile()) || ((node->getOpCode().isStore() || (node->getOpCode().hasSymbolReference() && node->getSymbolReference()->getSymbol()->isVolatile())) && (node->getSymbolReference()->getSymbol()->isShadow() || node->getSymbolReference()->getSymbol()->isStatic()))) {
        toReturn = true;
