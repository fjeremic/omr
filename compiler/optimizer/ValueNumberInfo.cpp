  ...
  if (node->getType().isBCD()) {
    ...
    if (!node->isSignStateEquivalent(entryNode)) {
      if (trace() || comp()->cg()->traceBCDCodeGen())
        traceMsg(comp(),
          "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign state mismatch -- do not consider as matching\n",
          node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
      return false;
    }
    ...
    if (trace())
      traceMsg(comp(),
        "fracConv node %s (%p) and fracConv entryNode %s (%p) have fraction mismatch -- do not consider as matching\n",
        node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
    return false;
  } else if (node->chkOpsCastedToBCD() && node->castedToBCD() != entryNode->castedToBCD()) {
    if (trace())
      traceMsg(comp(),
        "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s (%p) castedToBCD %d -- do not consider as "
        "matching\n",
...
void TR_ValueNumberInfo::initializeNode(TR::Node* node, int32_t& negativeValueNumber)
{
  ...
  for (entry = _hashTable._buckets[hashValue]; entry; entry = entry->_next) {
    ...
    if (node->getOpCodeValue() != entryNode->getOpCodeValue() || node->getNumChildren() != entryNode->getNumChildren())
      continue;
#ifdef J9_PROJECT_SPECIFIC
    if (node->getOpCode().isSetSignOnNode() && node->getSetSign() != entryNode->getSetSign()) {
      if (trace())
        traceMsg(comp(),
          "Nodes %s (%p) and entryNode %s (%p) with setSignOnNode have sign mismatch -- do not consider as matching\n",
          node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
      continue;
    }
    ...
    if (node->getType().isBCD()) {
      ...
      if (trace())
        traceMsg(comp(),
          "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s (%p) castedToBCD %d -- do not consider "
          "as matching\n",
          node->getOpCode().getName(), node, node->castedToBCD(), entryNode->getOpCode().getName(), entryNode,
          entryNode->castedToBCD());
      continue;
      ...
      if (trace())
        traceMsg(comp(),
          "DFP node %s (%p) and entryNode %s (%p) have different precisions -- do not consider as matching\n",
          node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
      continue;
    }
#endif
...
void TR_ValueNumberInfo::allocateValueNumber(TR::Node* node)
{
  ...
  //
  else if (node->getOpCode().isLoadVar() && !(node->getSymbol()->isConst() || node->getSymbol()->isConstObjectRef())) {
    ...
    if (baseVN == -1 || (defNode->getOpCode().isIndirect() && baseVN == getVN(defNode->getFirstChild()))) {
      ...
      if (trace()) {
        traceMsg(comp(), "  Use value number %d of dominating load %d at [%p] for load %d at [%p]\n", getVN(defNode),
          defNode->getGlobalIndex(), defNode, node->getGlobalIndex(), node);
      }
      return defNode;
    }
  }
            ...
            if (_useDefInfo->getDefiningLoads(otherUseDefiningLoads, useNode)
              ...
              if (baseVN == -1 || (useNode->getOpCode().isIndirect() && baseVN == getVN(useNode->getFirstChild()))) {
                ...
                if (value >= 0) {
                  ...
                  if (trace()) {
                    traceMsg(comp(),
                      "  Change value number for load %d at [%p] to value number %d of load %d at [%p] reached by same "
                      "dominating loads (defs)\n",
  ...
  , _VN(VN)
{
  ...
  for (uint32_t childNum = 0; childNum < node->getNumChildren(); childNum++) {
    ...
    TR_ASSERT(
      childVN > 0, "Allocating VN for node %p without allocating VN for child %p", node, node->getChild(childNum));
    hash(childVN);
  }
}
...
bool TR_HashValueNumberInfo::VNHashKey::operator==(const VNHashKey& other) const
{
  ...
  if (_node->getOpCodeValue() == otherNode->getOpCodeValue()
    ...
    for (uint32_t childNum = 0; (childNum < _node->getNumChildren() && childVNEqual); childNum++)
      childVNEqual
        = (_VN->getValueNumber(_node->getChild(childNum)) == _VN->getValueNumber(otherNode->getChild(childNum)));
    if (childVNEqual) {
