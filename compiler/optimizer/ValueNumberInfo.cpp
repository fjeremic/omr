...
TR_ValueNumberInfo::TR_ValueNumberInfo(TR::Compilation *comp,
  ...
  //
  TR_ASSERT(!(requiresGlobals || prefersGlobals),
            "Don't do global value numbering");
  ...
  if (noUseDefInfo) {
    ...
    if (_useDefInfo == NULL && !_optimizer->cantBuildLocalsUseDefInfo()) {
      if (!(requiresGlobals && _optimizer->cantBuildGlobalsUseDefInfo())) {
        TR::LexicalMemProfiler mp("use defs (value numbers - I)",
                                  comp->phaseMemProfiler());

        _useDefInfo =
            optimizer->createUseDefInfo(comp, requiresGlobals, prefersGlobals);
    ...
    if (_useDefInfo == NULL) {
      ...
      if (!requiresGlobals) _optimizer->setCantBuildLocalsValueNumberInfo(true);
      return;
    }
  ...
  _hashTable._buckets = (CollisionEntry **)trMemory()->allocateStackMemory(
      _hashTable._numBuckets * sizeof(CollisionEntry *),
      TR_Memory::ValueNumberInfo);
  memset(_hashTable._buckets, 0,
         _hashTable._numBuckets * sizeof(CollisionEntry *));
  _matchingNodes = new (trStackMemory())
      TR_Array<CollisionEntry *>(trMemory(), _numberOfNodes, false, stackAlloc);
  ...
  if (trace()) {
    ...
    for (i = 0; i < _numberOfNodes; i++) {
      ...
      if (node == NULL || traced.get(node->getGlobalIndex())) continue;
      if (getNext(node) == node) continue;
    ...
    for (i = _hashTable._numBuckets - 1; i >= 0; i--) {
      if (_hashTable._buckets[i] != NULL) {
        ...
        for (CollisionEntry *entry = _hashTable._buckets[i]; entry;
             entry = entry->_next)
          for (NodeEntry *nodeEntry = entry->_nodes; nodeEntry;
               nodeEntry = nodeEntry->_next)
            numInBucket++;
        numEntries += numInBucket;
        if (numInBucket > maxBucketSize) maxBucketSize = numInBucket;
      }
    }
    traceMsg(
        comp,
        "   HashTable entries = %d, buckets used = %d, max bucket size = %d\n",
  ...
  if (trace()) {
    ...
    for (i = 0; i < _numberOfNodes; i++) {
      ...
      traceMsg(comp, "node %4d [%p] has value number %4d", i, node,
               getValueNumber(node));
...
bool TR_ValueNumberInfo::congruentNodes(TR::Node *node, TR::Node *entryNode) {
  ...
  if (node->getOpCode().isSetSignOnNode() &&
      node->getSetSign() != entryNode->getSetSign())
    return false;
  ...
  if (node->getType().isBCD()) {
    if (!node->isDecimalSizeAndShapeEquivalent(entryNode)) {
      if (trace())
        traceMsg(comp(),
                 "BCD node %s (%p) and BCD entryNode %s (%p) have size/shape "
                 "mismatch -- do not consider as matching\n",
    ...
    if (!node->isSignStateEquivalent(entryNode)) {
      if (trace() || comp()->cg()->traceBCDCodeGen())
        traceMsg(comp(),
                 "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign "
                 "state mismatch -- do not consider as matching\n",
                 node->getOpCode().getName(), node,
                 entryNode->getOpCode().getName(), entryNode);
      return false;
    }
    ...
    if (trace())
      traceMsg(comp(),
               "fracConv node %s (%p) and fracConv entryNode %s (%p) have "
               "fraction mismatch -- do not consider as matching\n",
               node->getOpCode().getName(), node,
               entryNode->getOpCode().getName(), entryNode);
    return false;
    ...
    if (trace())
      traceMsg(
          comp(),
          "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s "
          "(%p) castedToBCD %d -- do not consider as matching\n",
          node->getOpCode().getName(), node, node->castedToBCD(),
          entryNode->getOpCode().getName(), entryNode,
          entryNode->castedToBCD());
    return false;
  } else if (node->getType().isDFP() && node->getOpCode().isModifyPrecision() &&
             node->getDFPPrecision() != entryNode->getDFPPrecision()) {
    return false;
  }
#endif
  ...
  */
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (symRef && entrySymRef &&
      ...
      if (!_useDefInfo || !_useDefInfo->isUseIndex(udIndex)) return true;
      ...
      if (_useDefInfo->getUseDef(udef, udIndex) &&
          _useDefInfo->getUseDef(entryudef, entryUdIndex)) {
        return (udef == entryudef);
      } else {
        if (trace()) TR_ASSERT(0, "No defs for a use\n");
      }
    }
  }
  ...
  if (node->getOpCode().isLoadConst()) {
    ...
    switch (node->getDataType()) {
      ...
      case TR::DecimalLongDouble:
        isSame = (node->getLongDouble() == entryNode->getLongDouble());
        break;
#endif
#endif
      case TR::Address:
...
void TR_ValueNumberInfo::initializeNode(TR::Node *node,
                                        int32_t &negativeValueNumber) {
  int32_t index = node->getGlobalIndex();
  if (_nodes.ElementAt(index) != NULL) {
    // Node has already been initialized
    //
    TR_ASSERT(_nodes.ElementAt(index) == node,
              "Value Numbering initialization error");
    return;
  }
  ...
  for (entry = _hashTable._buckets[hashValue]; entry; entry = entry->_next) {
    ...
    if (node->getOpCode().isSetSignOnNode() &&
      ...
      if (trace())
        traceMsg(comp(),
                 "Nodes %s (%p) and entryNode %s (%p) with setSignOnNode have "
                 "sign mismatch -- do not consider as matching\n",
                 node->getOpCode().getName(), node,
                 entryNode->getOpCode().getName(), entryNode);
      continue;
    }
    ...
    if (node->getType().isBCD()) {
      ...
      if (trace())
        traceMsg(
            comp(),
            "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode "
            "%s (%p) castedToBCD %d -- do not consider as matching\n",
            node->getOpCode().getName(), node, node->castedToBCD(),
            entryNode->getOpCode().getName(), entryNode,
            entryNode->castedToBCD());
      continue;
    } else if (node->getType().isDFP() &&
               node->getOpCode().isModifyPrecision() &&
               node->getDFPPrecision() != entryNode->getDFPPrecision()) {
      if (trace())
        traceMsg(comp(),
                 "DFP node %s (%p) and entryNode %s (%p) have different "
                 "precisions -- do not consider as matching\n",
                 node->getOpCode().getName(), node,
                 entryNode->getOpCode().getName(), entryNode);
      continue;
    }
    ...
    // other nodes in the chain.
    //
    if (node->getOpCode().isStore() || node->getOpCode().isLoadVar()) break;
    ...
    if (node->getOpCode().hasSymbolReference()) {
      ...
      if (symRef && entrySymRef &&
        ...
        uint16_t udIndex = node->getUseDefIndex();
        uint16_t entryUdIndex = entryNode->getUseDefIndex();
        if (!_useDefInfo || !_useDefInfo->isUseIndex(udIndex)) break;
        TR_ASSERT(_useDefInfo->isUseIndex(entryUdIndex), "Expecting use index");
        //
        // In rare cases when Jsrs exist in the method, use def info could be
        // NULL
        //
        TR_UseDefInfo::BitVector udef(comp()->allocator());
        TR_UseDefInfo::BitVector entryudef(comp()->allocator());
      ...
      if (nodeStride == entryNodeStride) break;
    } else
      break;
  }
...
int32_t TR_ValueNumberInfo::hash(TR::Node *node) {
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (node->getOpCode().is8Byte()) {
      ...
      h = (h << 4) + (node->getOpCodeValue() == TR::fconst
                          ? node->getFloatBits()
                          : node->getInt());
      g = h & 0xF0000000;
      h ^= g >> 24;
...
void TR_ValueNumberInfo::allocateParmValueNumbers() {
  ...
  if (_numberOfParms > 0) {
    _parmSymbols = (TR::ParameterSymbol **)trMemory()->allocateHeapMemory(
        _numberOfParms * sizeof(TR::ParameterSymbol *),
        TR_Memory::ValueNumberInfo);
    parms.reset();
    int32_t i = 0;
    for (p = parms.getFirst(); p; p = parms.getNext()) {
...
void TR_ValueNumberInfo::allocateValueNumber(TR::Node *node,
  ...
  if (canShareValueNumber(node)) allocateValueNumber(node);
}
...
void TR_ValueNumberInfo::allocateValueNumber(TR::Node *node) {
  ...
  if (_valueNumbers.ElementAt(index) >= 0) {
    ...
    if (trace()) {
      traceMsg(comp(), "Value of node %d at depth %d is %d\n", index,
               _recursionDepth, _valueNumbers.ElementAt(index));
    }
#endif
    return;
  }
  ...
  if (node->getOpCode().isStore()) {
    ...
    //
    if (getVN(valueChild) <= -3) changeValueNumber(valueChild, _nextValue++);
  ...
  //
    ...
    for (entry = head->_nodes; entry; entry = entry->_next) {
      ...
      if (node->getOpCode().hasSymbolReference()) {
        ...
        //
        if (!entryNode->getOpCode().hasSymbolReference()) continue;
      ...
      // A match found - we can reuse the value number of this other node. At
      // the same time, remove this node from the node chain.
      //
      setValueNumber(node, entryNode);
      NodeEntry *next;
      for (next = entry->_next; next->_node != node;
           entry = next, next = next->_next) {
      }
  ...
  //
  NodeEntry *prev;
  for (entry = head->_nodes, prev = NULL; entry->_node != node;
       prev = entry, entry = entry->_next) {
  }
  if (prev) {
    ...
    if (removeFromHashTable) head->_nodes = entry->_next;
  }
...
TR::Node *TR_ValueNumberInfo::getValueNumberForLoad(TR::Node *node) {
  ...
  if (!_useDefInfo || !_useDefInfo->isUseIndex(useDefIndex) ||
      (_recursionDepth > 50))
    return NULL;
  ...
  int32_t index = 0;
  int32_t baseVN =
      node->getOpCode().isIndirect() ? getVN(node->getFirstChild()) : -1;
  int32_t value = 0;
  int32_t defValue = 0;
  ...
  if (defNode) {
    ...
    if (baseVN == -1 || (defNode->getOpCode().isIndirect() &&
                         baseVN == getVN(defNode->getFirstChild()))) {
      ...
      if (value >= 0) {
        ...
        if (value != defValue) changeValueNumber(node, defValue);
        if (trace()) {
          traceMsg(comp(),
                   "  Change value number for load %d at [%p] to value number "
                   "%d of dominating load %d at [%p]\n",
                   node->getGlobalIndex(), node, defValue,
                   defNode->getGlobalIndex(), defNode);
        }
        return NULL;
      }
      ...
      if (trace()) {
        traceMsg(comp(),
                 "  Use value number %d of dominating load %d at [%p] for load "
                 "%d at [%p]\n",
                 getVN(defNode), defNode->getGlobalIndex(), defNode,
  ...
  if (isNonZero && node->getOpCode().isLoadVarDirect()) {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (defNode) {
        TR_ASSERT(defNode->getOpCode().isLoadVar(),
                  "Defining load is not a load");
        allocateValueNumber(defNode);
    ...
    if (defValue >= 0) {
      if (value >= 0) {
        ...
        if (trace()) {
          traceMsg(comp(),
                   "  2Use value number %d of dominating load %d at [%p] for "
                   "load %d at [%p]\n",
                   getVN(defNode), defNode->getGlobalIndex(), defNode,
                   node->getGlobalIndex(), node);
        }
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (_useDefInfo->getUsesFromDef(usesFromDefs, index, true)) {
        ...
        for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne()) {
          ...
          TR::Node *useNode =
              _useDefInfo->getNode(useIndex + _useDefInfo->getFirstUseIndex());
          ...
          if (useNode && (useNode != node) &&
            ...
            if (_useDefInfo->getDefiningLoads(otherUseDefiningLoads, useNode) &&
              ...
              // TR_ASSERT(useNode->getOpCode().isLoadVar(), "Defining load is
              // not a load");
              ...
              if (baseVN == -1 || (useNode->getOpCode().isIndirect() &&
                                   baseVN == getVN(useNode->getFirstChild()))) {
                // If this node now has a value number, we can equate the value
                // numbers of it and its dominating load
                //
                value = getVN(node);
                if (value >= 0) {
                  int32_t useValue = getVN(useNode);
                  if (value != useValue) changeValueNumber(node, useValue);
                  if (trace()) {
                    traceMsg(comp(),
                             "  Change value number for load %d at [%p] to "
                             "value number %d of load %d at [%p] reached by "
                             "same dominating loads (defs)\n",
                             node->getGlobalIndex(), node, useValue,
                             useNode->getGlobalIndex(), useNode);
                  }
                  return NULL;
                }
                ...
                if (trace()) {
                  traceMsg(comp(),
                           "  Use value number %d of dominating load %d at "
                           "[%p] for load %d at [%p]\n",
                           getVN(useNode), useNode->getGlobalIndex(), useNode,
                           node->getGlobalIndex(), node);
                }
                return useNode;
              }
  ...
  //
  if (!_useDefInfo->getUseDef(defsForLoad, useDefIndex)) return NULL;
...
void TR_ValueNumberInfo::setValueNumber(TR::Node *node, TR::Node *other) {
  ...
  if (index >= _numberOfNodes) {
    ...
    if (_nextInRing.ElementAt(index) != index) {
      int32_t i;
      for (i = _nextInRing.ElementAt(index); _nextInRing.ElementAt(i) != index;
           i = _nextInRing.ElementAt(i))
        ;
...
void TR_ValueNumberInfo::changeValueNumber(TR::Node *node, int32_t newVN) {
  ...
  if (index >= _numberOfNodes) {
    ...
    //
    for (int32_t i = _nextInRing.ElementAt(index); i != index;
         i = _nextInRing.ElementAt(i))
      _valueNumbers.ElementAt(i) = newVN;
    _valueNumbers.ElementAt(index) = newVN;
  }
  if (newVN >= _nextValue) _nextValue = newVN + 1;
}
...
void TR_ValueNumberInfo::printValueNumberInfo(TR::Node *node) {
  traceMsg(comp(), "Node : %p    Index = %d    Value number = %d\n", node,
           node->getUseDefIndex(), getVN(node));
...
TR_HashValueNumberInfo::VNHashKey::VNHashKey(TR::Node *node,
                                             TR_ValueNumberInfo *VN)
    : _node(node), _VN(VN) {
  _hashVal = 2166136261;
  ...
  for (uint32_t childNum = 0; childNum < node->getNumChildren(); childNum++) {
    ...
    TR_ASSERT(childVN > 0,
              "Allocating VN for node %p without allocating VN for child %p",
              node, node->getChild(childNum));
    hash(childVN);
  }
}

bool TR_HashValueNumberInfo::VNHashKey::operator==(
    const VNHashKey &other) const {
  ...
  if (_node->getOpCodeValue() == otherNode->getOpCodeValue() &&
    ...
    for (uint32_t childNum = 0;
         (childNum < _node->getNumChildren() && childVNEqual); childNum++)
...
TR_HashValueNumberInfo::TR_HashValueNumberInfo(TR::Compilation *comp,
                                               TR::Optimizer *optimizer,
                                               bool requiresGlobals,
                                               bool prefersGlobals,
                                               bool noUseDefInfo)
    : TR_ValueNumberInfo(comp), _nodeHash(comp->allocator()) {
  _compilation = comp;
  _optimizer = optimizer;
  _trace = comp->getOption(TR_TraceValueNumbers);
  ...
  if (noUseDefInfo) {
    ...
    if (_useDefInfo == NULL && !_optimizer->cantBuildLocalsUseDefInfo()) {
      if (!(requiresGlobals && _optimizer->cantBuildGlobalsUseDefInfo())) {
        TR::LexicalMemProfiler mp("use defs (value numbers - II)",
                                  comp->phaseMemProfiler());
        _useDefInfo =
            optimizer->createUseDefInfo(comp, requiresGlobals, prefersGlobals);
        if (_useDefInfo->infoIsValid()) {
  ...
  // Any stack allocations made past this point will die when the function
  // returns
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  ...
  if (trace()) {
    ...
    for (i = 0; i < _numberOfNodes; i++) {
      TR::Node *node = getNode(i);
      if (node == NULL) continue;
      traceMsg(comp, "HVN: node %4d [%p] has value number %4d", i, node,
               getValueNumber(node));
      if (getNext(node) != node) {
        traceMsg(comp, ", shared with ");
        for (TR::Node *next = getNext(node); next != node; next = getNext(next))
...
void TR_HashValueNumberInfo::initializeNode(TR::Node *node,
                                            int32_t &negativeValueNumber) {
  int32_t index = node->getGlobalIndex();
  if (_nodes.ElementAt(index) != NULL) {
    // Node has already been initialized
...
void TR_HashValueNumberInfo::allocateValueNumber(TR::Node *node) {
  ...
  for (i = node->getNumChildren() - 1; i >= 0; --i) {
    ...
    //         else
    //            {
    //            traceMsg(comp(), "Did not recurse on node %p because
    //            getGloblalIndex = -2\n",node->getChild(i));
    //            }
  }
  ...
  //
    ...
    if (defNode) {
      setValueNumber(node, defNode);
      // traceMsg(comp(), "For load node %p found defNode %p, setting value
      // number of node to %d ",node,defNode,_valueNumbers.ElementAt(index));
    } else {
      changeValueNumber(node, _nextValue++);
      // traceMsg(comp(), "For load node %p found no defNode , setting value
      // number of node to %d ",node,defNode,_valueNumbers.ElementAt(index));
    }
  }
  ...
  //
    ...
    if (isValidToLookIntoHash) {
      ...
      if (_nodeHash.Locate(nodeKey, loc)) {
        ...
        TR_ASSERT(
            (otherNode != NULL),
            "HASHVN :Non-null nodeTable entry expected for globalIndex:%d",
            otherNodeIndex);

        //              traceMsg(comp(),"JIAG1: Setting node %p value number to
        //              same number at node %p\n",node,otherNode);
  ...
  if (trace()) {
    traceMsg(comp(),
             "Done processing node %p Value of node %d at depth %d is %d\n",
