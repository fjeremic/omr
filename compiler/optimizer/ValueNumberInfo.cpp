...
bool TR_ValueNumberInfo::congruentNodes(TR::Node* node, TR::Node* entryNode)
{
    ...
    } else if (node->getOpCode().isConversionWithFraction() && node->getDecimalFraction() != entryNode->getDecimalFraction()) {
        if (trace())
    ...
    */
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if (symRef && entrySymRef && symRef->getSymbol() == entrySymRef->getSymbol() && symRef->getOffset() == entrySymRef->getOffset()) {
...
{
    if (node->getOpCode().canShareValueNumber() && !node->getOpCode().isResolveCheck() && !node->hasUnresolvedSymbolReference()) {
        return true;
    }
    return false;
}
