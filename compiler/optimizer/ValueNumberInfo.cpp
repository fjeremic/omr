  ...
  , _nextInRing(comp->allocator())
{
  ...
  memset(
    _hashTable._buckets, 0, _hashTable._numBuckets * sizeof(CollisionEntry*));
...
void
TR_ValueNumberInfo::buildValueNumberInfo()
{
...
bool
TR_ValueNumberInfo::congruentNodes(TR::Node* node, TR::Node* entryNode)
{
...
void
TR_ValueNumberInfo::initializeNode(TR::Node* node, int32_t& negativeValueNumber)
{
 ...
 */
int32_t
TR_ValueNumberInfo::hash(TR::Node* node)
{
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (node->getOpCode().is8Byte()) {
      ...
      h =
        (h << 4) + (node->getOpCodeValue() == TR::fconst ? node->getFloatBits()
                                                         : node->getInt());
      g = h & 0xF0000000;
      h ^= g >> 24;
    }
 ...
 */
void
TR_ValueNumberInfo::allocateNonShareableValueNumbers()
{
...
void
TR_ValueNumberInfo::allocateParmValueNumbers()
{
  _numberOfParms = 0;
 ...
 */
void
TR_ValueNumberInfo::allocateShareableValueNumbers()
{
...
void
TR_ValueNumberInfo::allocateValueNumber(TR::Node* node, vcount_t visitCount)
{
...
bool
TR_ValueNumberInfo::canShareValueNumber(TR::Node* node)
{
...
void
TR_ValueNumberInfo::allocateValueNumber(TR::Node* node)
{
  ...
  if (trace()) {
    traceMsg(
      comp(), "Processing node %d at depth %d\n", index, _recursionDepth);
  }
#endif
  ++_recursionDepth;
  ...
  if (_valueNumbers.ElementAt(index) >= 0) {
    ...
    if (trace()) {
      traceMsg(comp(),
               "Value of node %d at depth %d is %d\n",
               index,
               _recursionDepth,
...
TR::Node*
TR_ValueNumberInfo::getValueNumberForLoad(TR::Node* node)
{
  ...
  if (isNonZero && node->getOpCode().isLoadVarDirect()) {
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
      ...
      if (defNode) {
        ...
        if (trace())
          traceMsg(comp(),
                   "node %p defNode %p newDefValue %d\n",
                   node,
                   defNode,
                   newDefValue);
        if (newDefValue < 0) {
...
void
TR_ValueNumberInfo::setValueNumber(TR::Node* node, TR::Node* other)
{
  int32_t index = node->getGlobalIndex();
...
void
TR_ValueNumberInfo::setUniqueValueNumber(TR::Node* node)
{
  int32_t index = node->getGlobalIndex();
...
void
TR_ValueNumberInfo::changeValueNumber(TR::Node* node, int32_t newVN)
{
...
void
TR_ValueNumberInfo::removeNodeInfo(TR::Node* node)
{
  // Remove the node from its existing value number chain
  //
  int32_t index = node->getGlobalIndex();
...
void
TR_ValueNumberInfo::growTo(int32_t index)
{
...
void
TR_ValueNumberInfo::printValueNumberInfo(TR::Node* node)
{
  traceMsg(comp(),
           "Node : %p    Index = %d    Value number = %d\n",
           node,
           node->getUseDefIndex(),
           getVN(node));
...
bool
TR_HashValueNumberInfo::VNHashKey::operator==(const VNHashKey& other) const
{
  ...
  if (_node->getOpCodeValue() == otherNode->getOpCodeValue() &&
    ...
    for (uint32_t childNum = 0;
         (childNum < _node->getNumChildren() && childVNEqual);
         childNum++)
  ...
  if (trace()) {
    ...
    for (i = 0; i < _numberOfNodes; i++) {
      ...
      if (node == NULL)
        continue;
      traceMsg(comp,
               "HVN: node %4d [%p] has value number %4d",
               i,
               node,
               getValueNumber(node));
      if (getNext(node) != node) {
...
void
TR_HashValueNumberInfo::initializeNode(TR::Node* node,
...
void
TR_HashValueNumberInfo::allocateValueNumber(TR::Node* node)
{
  int32_t index = node->getGlobalIndex();
  if (_valueNumbers.ElementAt(index) >= 0 ||
      _valueNumbers.ElementAt(index) <= -3)
