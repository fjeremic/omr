...
TR_ReachingDefinitions::TR_ReachingDefinitions(
    TR::Compilation *comp, TR::CFG *cfg, TR::Optimizer *optimizer,
...
int32_t TR_ReachingDefinitions::perform() {
  ...
  if (traceRD()) traceMsg(comp(), "Starting ReachingDefinitions\n");

  // Allocate the block info, allowing the bit vectors to be allocated on the
  // fly
  //
...
void TR_ReachingDefinitions::analyzeBlockZeroStructure(
...
void TR_ReachingDefinitions::initializeGenAndKillSetInfo() {
  ...
  for (TR::TreeTop *treeTop = comp()->getStartTree(); treeTop;
    ...
    initializeGenAndKillSetInfoForNode(node, defsKilled, seenException,
                                       blockNum, NULL);

    if (!seenException && treeHasChecks(treeTop)) seenException = true;
  }
}

void TR_ReachingDefinitions::initializeGenAndKillSetInfoForNode(
    TR::Node *node, TR_BitVector &defsKilled, bool seenException,
  ...
  for (i = node->getNumChildren() - 1; i >= 0; --i) {
    initializeGenAndKillSetInfoForNode(node->getChild(i), defsKilled,
                                       seenException, blockNum, node);
  }
  ...
  if (_useDefInfo->_useDefForRegs &&
      (opCode.isLoadReg() || opCode.isStoreReg())) {
    sym = NULL;
    symRef = NULL;
  ...
  if (symIndex == NULL_USEDEF_SYMBOL_INDEX || node->getOpCode().isCall() ||
    ...
    if (node->getOpCode().isCall()) foundDefsToKill = false;
    ...
    if (!irrelevantStore) {
      numDefNodes = num_aliases;
      numDefNodes =
          _useDefInfo->isExpandedUseDefIndex(nodeIndex) ? 1 : numDefNodes;
  ...
  } else  // fake up the method entry def as the def index to "gen" to avoid a
          // use without a def completely
  {
