...
TR_ReachingDefinitions::getKind()
{
  return ReachingDefinitions;
}

bool
TR_ReachingDefinitions::supportsGenAndKillSets()
{
  return true;
}
...
int32_t
TR_ReachingDefinitions::perform()
{
...
int32_t
TR_ReachingDefinitions::getNumberOfBits()
{
  return _useDefInfo->getNumExpandedDefNodes();
}

void
TR_ReachingDefinitions::analyzeBlockZeroStructure(
...
void
TR_ReachingDefinitions::initializeGenAndKillSetInfo()
{
  ...
  for (TR::TreeTop* treeTop = comp()->getStartTree(); treeTop;
    ...
    initializeGenAndKillSetInfoForNode(
      node, defsKilled, seenException, blockNum, NULL);
...
void
TR_ReachingDefinitions::initializeGenAndKillSetInfoForNode(
  TR::Node* node,
  ...
  TR::Node* parent)
{
  ...
  for (i = node->getNumChildren() - 1; i >= 0; --i) {
    initializeGenAndKillSetInfoForNode(
      node->getChild(i), defsKilled, seenException, blockNum, node);
  }
