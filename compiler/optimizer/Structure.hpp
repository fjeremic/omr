...
public:
  ...
  TR_Structure(TR::Compilation *c, int32_t index)
      : _comp(c), _nodeIndex(index), _parent(NULL), _weight(-1),
        _versionedStructure(NULL), _trMemory(c->trMemory()),
        _maxNestingDepth(0), _nestingDepth(0), _analyzedBefore(false),
...
public:
  ...
  TR_RegionStructure(TR::Compilation *c, int32_t index)
      : TR_Structure(c, index), _invariantSymbols(NULL),
        _blocksAtSameNestingLevel(NULL), _piv(NULL),
  ...
  static void
  extractUnconditionalExits(TR::Compilation *const comp,
