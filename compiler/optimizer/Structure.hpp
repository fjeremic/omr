...
class TR_Structure {
 public:
  ...
  // Does this structure contain the other? If the "commonParent" is known it
  // can be specified to make the search faster.
  //
  ...
  //
  virtual bool doDataFlowAnalysis(TR_DataFlowAnalysis *,
                                  bool checkForChanges) = 0;
  ...
  virtual void addExternalEdge(TR_Structure *from, int32_t toNumber,
                               bool isExceptionEdge) = 0;
  ...
  virtual void collectExitBlocks(List<TR::Block> *exitBlocks,
                                 List<TR::CFGEdge> *exitEdges = NULL) = 0;
  virtual TR_Structure *cloneStructure(TR::Block **,
  ...
  void adjustWeightForBranches(TR_StructureSubGraphNode *,
                               TR_StructureSubGraphNode *, int32_t *);
  ...
  virtual void setContainsImproperRegion(bool b) {
    _containsImproperRegion = b;
  }

  bool hasBeenAnalyzedBefore() { return _analyzedBefore; }
  void setAnalyzedStatus(bool analyzedStatus) {
    _analyzedBefore = analyzedStatus;
  }
  ...
  void setNestingDepth(int16_t f) {
    if (f > SHRT_MAX - 1) {
      ...
      comp()->failCompilation<TR::CompilationException>(
          "nesting depth must be less than or equal to SHRT_MAX-1");
    }
    _nestingDepth = f;
  }

  int32_t getAnyCyclicRegionNestingDepth() {
    return _anyCyclicRegionNestingDepth;
  }
  ...
  virtual void hoistInvariantsOutOfNestedLoops(TR_LocalTransparency *,
                                               TR_BitVector **, bool,
                                               TR_BlockStructure *,
                                               TR_RegionStructure *,
                                               int32_t) = 0;
...
#if DEBUG
  TR_ASSERT(node->asStructureSubGraphNode() != NULL,
            "Bad downcast from TR::CFGNode to TR_StructureSubGraphNode");
#endif
...
class TR_StructureSubGraphNode : public TR::CFGNode {
 public:
  ...
  TR_StructureSubGraphNode(TR_Structure *s)
      : TR::CFGNode(s->getNumber(), s->cfg()->structureRegion()),
        _structure(s) {
  ...
  static TR_StructureSubGraphNode *create(int32_t num,
...
class TR_BlockStructure : public TR_Structure {
 public:
  ...
  virtual void collectExitBlocks(List<TR::Block> *exitBlocks,
                                 List<TR::CFGEdge> *exitEdges = NULL);
  ...
  bool isEntryOfShortRunningLoop() {
    return _block->isEntryOfShortRunningLoop();
  }
  void setIsEntryOfShortRunningLoop() {
    _block->setIsEntryOfShortRunningLoop();
  }

  bool wasHeaderOfCanonicalizedLoop() {
    return _block->wasHeaderOfCanonicalizedLoop();
  }
  void setWasHeaderOfCanonicalizedLoop(bool b) {
    _block->setWasHeaderOfCanonicalizedLoop(b);
  }

  virtual TR::Block *getEntryBlock() { return getBlock(); }

  TR_BlockStructure *getDuplicatedBlock() {
    if (_versionedStructure) return _versionedStructure->asBlock();
    return NULL;
  }
...
class TR_InductionVariable : public TR_Link<TR_InductionVariable> {
 public:
  ...
  TR_InductionVariable(TR::RegisterMappedSymbol *s, TR::VPConstraint *entry,
                       TR::VPConstraint *exit, TR::VPConstraint *incr,
                       TR_YesNoMaybe isSigned)
 ...
 private:
  ...
  TR_YesNoMaybe _isSigned;  // FIXME// CRITICAL Value Propgation should set this
                            // up.. right now this is not being done
};
...
class TR_RegionStructure : public TR_Structure {
 public:
  ...
  TR_RegionStructure(TR::Compilation *c, int32_t index)
      : TR_Structure(c, index),
        ...
        _piv(NULL),
        _basicIVs(c->getFlowGraph()->structureRegion()),
        _exitEdges(c->getFlowGraph()->structureRegion()),
        _subNodes(c->getFlowGraph()->structureRegion()),
  ...
  TR_StructureSubGraphNode *setEntry(TR_StructureSubGraphNode *entry) {
    ...
    if (entryBlock != NULL) _unrollFactor = getEntryBlock()->getUnrollFactor();
    return entry;
  }
  ...
  //
  TR::CFGEdge *addExitEdge(TR_StructureSubGraphNode *from, int32_t to,
                           bool isExceptionEdge = false,
                           TR::CFGEdge *origEdge = 0);
  ...
  static void extractUnconditionalExits(
  ...
  //
  bool containsInternalCycles() {
    return _regionFlags.testAny(_containsInternalCycles);
  }
  void setContainsInternalCycles(bool b) {
    _regionFlags.set(_containsInternalCycles, b);
  }
  ...
  //
  bool isNaturalLoop() {
    return !containsInternalCycles() && !_entryNode->getPredecessors().empty();
  }

  TR_RegionStructure *getVersionedLoop() {
    if (_versionedStructure) return _versionedStructure->asRegion();
    return NULL;
  }
  ...
  //
  TR_InductionVariable *getFirstInductionVariable() {
    return _inductionVariables.getFirst();
  }
  void addInductionVariable(TR_InductionVariable *v) {
    _inductionVariables.add(v);
  }
  void addAfterInductionVariable(TR_InductionVariable *prev,
  ...
  void setPrimaryInductionVariable(TR_PrimaryInductionVariable *piv) {
    _piv = piv;
  }

  void addInductionVariable(TR_BasicInductionVariable *biv) {
    _basicIVs.add(biv);
  }
  List<TR_BasicInductionVariable> &getBasicInductionVariables() {
    return _basicIVs;
  }
  ...
  bool isExitEdge(TR::CFGEdge *exitEdge) {
    TR_StructureSubGraphNode *toNode =
        exitEdge->getTo()->asStructureSubGraphNode();
    if (toNode->getStructure()) return false;
  ...
  bool isCanonicalizedLoop() {
    return _regionFlags.testAny(_isCanonicalizedLoop);
  }
  void setAsCanonicalizedLoop(bool b) {
    _regionFlags.set(_isCanonicalizedLoop, b);
  }
  ...
  TR_BitVector *getBlocksAtSameNestingLevel() {
    return _blocksAtSameNestingLevel;
  }
  void setBlocksAtSameNestingLevel(TR_BitVector *blocksInLoop) {
    _blocksAtSameNestingLevel = blocksInLoop;
  }

  virtual TR::Block *getEntryBlock() {
    TR_RegionStructure *region = _entryNode->getStructure()->asRegion();
    if (!region) return _entryNode->getStructure()->asBlock()->getBlock();
 ...
 public:
  ...
  // The Cursor provides an immutable view of the elements of the region unlike
  // the normal iterators above, note that there is a non-trivial cost to this
  // Cursor because it must copy the structure's sublist into itself for the
  // purposes of isolation
  class Cursor {
   public:
    Cursor(TR_RegionStructure &region)
        : _nodes(region._subNodes), _iter(_nodes.begin()) {}

    TR_StructureSubGraphNode *getCurrent() {
      return (_iter != _nodes.end()) ? *_iter : NULL;
    }
