...
int32_t OMR::CFGSimplifier::perform() {
  if (trace()) traceMsg(comp(), "Starting CFG Simplification\n");
...

  {
    ...
    if (_cfg != NULL) {
      for (TR::CFGNode *cfgNode = _cfg->getFirstNode(); cfgNode;
...
bool OMR::CFGSimplifier::simplify() {
  ...
  //
  if (_block->getEntry() == NULL) return false;
...
bool OMR::CFGSimplifier::simplifyIfStructure() {
  ...
  //
  if (_next1 == NULL || _next2 == NULL) return false;
  if (_succ2 == NULL) return false;
  if (_block->getSuccessors().size() > 2) return false;
  if (_next1->getEntry() == NULL || _next2->getEntry() == NULL) return false;
  ...
  if (_next1->getPredecessors().empty()) return false;
  if (!(_next1->getPredecessors().front()->getFrom() == _block &&
        (_next1->getPredecessors().size() == 1)))
    needToDuplicateTree = true;
  if (_next2->getPredecessors().empty()) return false;
  if (!(_next2->getPredecessors().front()->getFrom() == _block &&
        (_next2->getPredecessors().size() == 1)))
    needToDuplicateTree = true;
  ...
  if (!compareNode->getOpCode().isIf()) return false;
...
bool OMR::CFGSimplifier::hasExceptionPoint(TR::Block *block, TR::TreeTop *end) {
  return true;
  if (!block->getExceptionSuccessors().empty()) return true;

  for (TR::TreeTop *itr = block->getEntry(); itr && itr != end;
       itr = itr->getNextTreeTop()) {
    if (itr->getNode()->exceptionsRaised() != 0) return true;
  }
  return false;
}

bool OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(
    bool needToDuplicateTree) {
  static char *disableSimplifyInstanceOfTestToCheckcast =
      feGetEnv("TR_disableSimplifyInstanceOfTestToCheckcast");
  if (disableSimplifyInstanceOfTestToCheckcast != NULL) return false;

  if (comp()->getOSRMode() == TR::involuntaryOSR) return false;

  if (_block->isCatchBlock()) return false;
  ...
  if (trace())
    traceMsg(comp(),
             "   Found an ificmp[eq/ne] of an instanceof against 0 n%dn\n",
             compareNode->getGlobalIndex());

  if (compareNode->getFirstChild()->getSecondChild()->getOpCodeValue() !=
      TR::loadaddr)
    return false;

  if (trace())
    traceMsg(comp(),
             "   Found an ificmp[eq/new] of an instanceof a constant class "
             "against zero n%dn\n",
             compareNode->getGlobalIndex());
  ...
  if (compareNode->getOpCodeValue() == TR::ificmpeq) {
    if (_next2->getLastRealTreeTop()->getNode()->getNumChildren() != 1 ||
        _next2->getLastRealTreeTop()
                ->getNode()
                ->getFirstChild()
                ->getOpCodeValue() != TR::athrow)
      return false;

    if (trace())
      traceMsg(comp(),
               "   Found an ificmpeq of an instanceof against zero which "
               "throws on taken size\n");
    throwBlock = _next2;
    fallthroughBlock = _next1;
  } else {
    if (_next1->getLastRealTreeTop()->getNode()->getNumChildren() != 1 ||
        _next1->getLastRealTreeTop()
                ->getNode()
                ->getFirstChild()
                ->getOpCodeValue() != TR::athrow)
      return false;
    ...
    if (trace())
      traceMsg(comp(),
               "   Found an ificmpne of an instance of against zero which "
               "throws on the fallthrough path\n");
    throwBlock = _next1;
    fallthroughBlock = _next2;
  }

  if (!performTransformation(comp(),
                             "%sReplace ificmp of instanceof with throw "
                             "failure with checkcastAndNULLCHK in block_%d\n",
  ...
  TR::Block *catchBlock = TR::Block::createEmptyBlock(
      compareNode, comp(), throwBlock->getFrequency());
  catchBlock->setHandlerInfo(0, comp()->getInlineDepth(), 0,
                             comp()->getCurrentMethod(), comp());
  ...
  TR::Node *checkcastAndNULLCHKNode = TR::Node::createWithSymRef(
      compareNode->getFirstChild(), TR::checkcastAndNULLCHK, 2,
  ...
  TR_Pair<TR_ByteCodeInfo, TR::Node> *bcInfo =
      new (trHeapMemory()) TR_Pair<TR_ByteCodeInfo, TR::Node>(
          &compareNode->getFirstChild()->getByteCodeInfo(),
          checkcastAndNULLCHKNode);
  ...
  TR::TreeTop *checkcastAndNULLCHKTree =
      TR::TreeTop::create(comp(), checkcastAndNULLCHKNode);
  ...
  if (trace()) traceMsg(comp(), "End simplifyInstanceOfTestToCheckcast.\n");

  TR::DebugCounter::incStaticDebugCounter(
      comp(), TR::DebugCounter::debugCounterName(
                  comp(), "cfgSimpCheckcast/(%s)", comp()->signature()));

  return true;
}

static bool containsIndirectOperationImpl(TR::Node *node,
  ...
  if (!(node->getOpCode().isArithmetic() && !node->getOpCode().isDiv()) &&
      !node->getOpCode().isLoadVarDirect() && !node->getOpCode().isTernary() &&
...
static bool containsIndirectOperation(TR::Compilation *comp,
                                      TR::TreeTop *treetop) {
  TR::NodeChecklist visited(comp);
  return containsIndirectOperationImpl(treetop->getNode()->getFirstChild(),
                                       &visited, 3);
}

bool OMR::CFGSimplifier::simplifyCondStoreSequence(bool needToDuplicateTree) {
  static char *disableSimplifyCondStoreSequence =
      feGetEnv("TR_disableSimplifyCondStoreSequence");
  if (disableSimplifyCondStoreSequence != NULL) return false;
  ...
  if (!performTransformation(comp(),
                             "%sReplace conditional stores in block_%d with "
                             "stores of appropriate ternary at nodes\n",
                             OPT_DETAILS, toCheck->getNumber()))
    return false;

  _cfg->invalidateStructure();

  TR::Node *condition = TR::Node::create(
      compareNode, compareNode->getOpCode().convertIfCmpToCmp(), 2,
  ...
  while (treeCursor->getNode()->getOpCode().isStoreDirect() &&
    ...
    TR::Node *trueValue =
        triangle1
            ...
            : (needToDuplicateTree ? storeNode->getFirstChild()->duplicateTree()
    ...
    TR::Node *select = TR::Node::create(
        storeNode, comp()->il.opCodeForTernarySelect(storeNode->getDataType()),
        3);
    if (trace())
    ...
    TR::TreeTop *insTree = TR::TreeTop::create(
        comp(),
        TR::Node::createWithSymRef(storeNode, storeNode->getOpCodeValue(), 1,
                                   select, storeNode->getSymbolReference()));

    if (storeNode->getOpCodeValue() == TR::astore &&
        storeNode->isHeapificationStore())
  ...
  if (trace()) traceMsg(comp(), "End simplifyCondStoreSequence.\n");
  TR::DebugCounter::incStaticDebugCounter(
      comp(), TR::DebugCounter::debugCounterName(
                  comp(), "cfgSimpMovSeq/%d/(%s)", count, comp()->signature()));
  return true;
}

bool OMR::CFGSimplifier::simplifySimpleStore(bool needToDuplicateTree) {
  static char *disableSimplifySimpleStore =
      feGetEnv("TR_disableSimplifySimpleStore");
  if (disableSimplifySimpleStore != NULL) return false;
  ...
  bool diamond = _next2->getSuccessors().size() == 1 &&
                 ...
                 _next1->getExceptionSuccessors().size() == 0 &&
                 toBlock(_next1->getSuccessors().front()->getTo()) ==
                     toBlock(_next2->getSuccessors().front()->getTo());
  ...
  static char *disableSimplifySimpleStoreTriangle =
      feGetEnv("TR_disableSimplifySimpleStoreTriangle");
  if ((triangle1 || triangle2) && disableSimplifySimpleStoreTriangle != NULL)
    return false;

  static char *disableSimplifySimpleStoreDiamond =
      feGetEnv("TR_disableSimplifySimpleStoreDiamond");
  if ((diamond) && disableSimplifySimpleStoreDiamond != NULL) return false;

  if (!triangle1 && !triangle2 && !diamond) return false;

  if (trace()) traceMsg(comp(), "   compareTree has correctType\n");
  ...
  if (triangle2 || diamond) {
    ...
    if (trace())
      traceMsg(comp(),
               "   Take side has an appropriate store as the first tree\n");

    storeNode = treeCursor->getNode();
    isHeapificationStore = storeNode->getOpCodeValue() == TR::astore &&
                           storeNode->isHeapificationStore();

    if (treeCursor->getNextTreeTop()->getNode()->getOpCodeValue() !=
            TR::BBEnd &&
    ...
    if (trace()) traceMsg(comp(), "   Taken side checks out\n");
  }
  ...
  if (triangle1 || diamond) {
    ...
    if (trace())
      traceMsg(
          comp(),
          "   Fallthrough side has an appropriate store as the first tree\n");
    ...
    if (trace()) traceMsg(comp(), "   Fallthrough checks out\n");
  }
  ...
  if (trace()) traceMsg(comp(), "   StoreNode data type checks out\n");

  if (!diamond && !storeNode->getSymbolReference()->getSymbol()->isAutoOrParm())
    return false;

  if (trace()) traceMsg(comp(), "   StoreNode symRef checks out\n");

  if (!performTransformation(comp(),
                             "%sReplace conditional store with store of an "
                             "appropriate ternary at node [%p]\n",
  ...
  TR::Node *select = TR::Node::create(
      storeNode, comp()->il.opCodeForTernarySelect(storeNode->getDataType()),
      3);
  select->setAndIncChild(
      0, TR::Node::create(
             compareNode, compareNode->getOpCode().convertIfCmpToCmp(), 2,
             compareNode->getFirstChild(), compareNode->getSecondChild()));
  select->setAndIncChild(
      1, trueValue
             ? (needToDuplicateTree ? trueValue->duplicateTree() : trueValue)
             : TR::Node::createWithSymRef(
                   comp()->il.opCodeForDirectLoad(storeNode->getDataType()), 0,
                   storeNode->getSymbolReference()));
  select->setAndIncChild(
      2, falseValue
             ? (needToDuplicateTree ? falseValue->duplicateTree() : falseValue)
             : TR::Node::createWithSymRef(
                   comp()->il.opCodeForDirectLoad(storeNode->getDataType()), 0,
                   storeNode->getSymbolReference()));
  TR::TreeTop *cmov = TR::TreeTop::create(
      comp(),
      TR::Node::createWithSymRef(storeNode, storeNode->getOpCodeValue(), 1,
                                 select, storeNode->getSymbolReference()));
  compareTree->insertBefore(cmov);
  if (isHeapificationStore) cmov->getNode()->setHeapificationStore(true);
  ...
  TR::DebugCounter::incStaticDebugCounter(
      ...
      TR::DebugCounter::debugCounterName(
          comp(), "cfgSimpCMOV/%s/(%s)",
          (diamond ? "diamond" : (triangle1 ? "triangle1" : "triangle2")),
          comp()->signature()));
  return true;
}

bool OMR::CFGSimplifier::simplifyNullToException(bool needToDuplicateTree) {
  static char *disableSimplifyExplicitNULLTest =
      feGetEnv("TR_disableSimplifyExplicitNULLTest");
  static char *disableSimplifyNullToException =
      feGetEnv("TR_disableSimplifyNullToException");
  if (disableSimplifyExplicitNULLTest != NULL ||
      disableSimplifyNullToException != NULL)
    return false;

  if (comp()->getOSRMode() == TR::involuntaryOSR) return false;

  if (trace()) traceMsg(comp(), "Start simplifyNullToException\n");
  ...
  TR::Block *nullBlock =
      compareNode->getOpCodeValue() == TR::ifacmpeq ? _next2 : _next1;
  ...
  if (!performTransformation(
          comp(),
          "%sReplace ifacmpeq/ifacmpne of NULL node n%dn [%p] to a blcok "
          "ending in throw with a NULLCHK to a catch which goes to block_%d\n",
          OPT_DETAILS, compareNode->getGlobalIndex(), compareNode,
          nullBlock->getNumber()))
    return false;
  ...
  TR::DebugCounter::incStaticDebugCounter(
      comp(),
      TR::DebugCounter::debugCounterName(
          comp(), "cfgSimpNULLCHK/nullToException/(%s)", comp()->signature()));
  ...
  if (compareBlock->getNextBlock() == nullBlock) {
    ...
    gotoNode->setBranchDestination(
        (nullBlock == _next1 ? _next2 : _next1)->getEntry());
    compareBlock->append(TR::TreeTop::create(comp(), gotoNode));
  }

  TR::Node *nullchkNode = TR::Node::createWithSymRef(
      TR::NULLCHK, 1, 1,
      TR::Node::create(compareNode, TR::PassThrough, 1,
  ...
  TR::Block *catchBlock = TR::Block::createEmptyBlock(
      compareNode, comp(), nullBlock->getFrequency());
  catchBlock->setHandlerInfo(0, comp()->getInlineDepth(), 0,
                             comp()->getCurrentMethod(), comp());
  TR::Node *gotoNode = TR::Node::create(compareNode, TR::Goto, 0);
...
//
// Also look specifically for the following pattern using the S390 condition
// code:
//
//    if (conditionCode)
//       x = 0;
...
bool OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree) {
  static char *enableSimplifyBooleanStore =
      feGetEnv("TR_enableSimplifyBooleanStore");
  if (enableSimplifyBooleanStore == NULL) return false;

  if (trace()) traceMsg(comp(), "Start simplifyBooleanStore\n");

  if (_next1->getSuccessors().empty()) return false;
  if (_next1->getSuccessors().size() != 1) return false;
  if (_next2->getSuccessors().empty()) return false;
  if (_next2->getSuccessors().size() != 1) return false;
  if (_next1->getSuccessors().front()->getTo() !=
      _next2->getSuccessors().front()->getTo())
    return false;

  if (trace()) traceMsg(comp(), "   Control flow checks out\n");
  ...
  if (compareNode->getOpCode().convertIfCmpToCmp() == TR::BadILOp) return false;
  ...
  if (!store1->getOpCode().isStore()) return false;
  if (trace())
  ...
  if (!store2->getOpCode().isStore()) return false;
  if (trace())
    traceMsg(comp(), "   Successor block_%d is single store\n",
             _next2->getNumber());
  ...
  //
  if (store1->getOpCodeValue() != store2->getOpCodeValue()) return false;
  if (!store1->getOpCode().isInt() && !store1->getOpCode().isByte())
    return false;
  if (store1->getSymbolReference()->getSymbol() !=
      store2->getSymbolReference()->getSymbol())
    return false;
  if (trace())
  ...
  if (valueIndex == 1)  // indirect store, check base objects
  {
    ...
    if (!base1->getOpCode().hasSymbolReference() ||
      ...
      return false;
    if (base1->getSymbolReference()->getReferenceNumber() !=
        base2->getSymbolReference()->getReferenceNumber())
      return false;
    if (trace())
      traceMsg(comp(),
               "   Indirect store base node opcode and symref checks out\n");
  }
  ...
  if (value1->getOpCode().isLoadConst()) {
    ...
    // Is this code really valid when the trees guarded by the if rely on the
    // condition checked in the 'if' (e.g. we could have an indirect access
    // without any checking in guarded code, because the test checked if the
    // value was NULL, in which case by performing the simplification we could
    // end up with a crash when the object is dereferenced)
    // else if (int1 == 0)
    //   swapCompare = true;
    else {
      return false;
  ...
  // Is this code really valid when the trees guarded by the if rely on the
  // condition checked in the 'if' (e.g. we could have an indirect access
  // without any checking in guarded code, because the test checked if the value
  // was NULL, in which case by performing the simplification we could end up
  // with a crash when the object is dereferenced)
  // else if (!(value2->getOpCode().isLoadConst() && value2->getInt() == 0))
  //   return false;
  else
    return false;

  if (trace()) traceMsg(comp(), "   Comparison values check out\n");
...
#else
  if (isBranchOnCondCode) return false;
#endif
  ...
  if (!performTransformation(
          comp(),
          "%sReplace compare-and-branch node [%p] with boolean compare\n",
          OPT_DETAILS, compareNode))
    return false;
  ...
  if (swapCompare) {
    ...
    //
    TR::Node::recreate(compareNode,
                       compareNode->getOpCode().getOpCodeForReverseBranch());
  } else {
    if (needToDuplicateTree) {
      storeTreeTop = NULL;
  ...
  if (succ1Freq > 0) {
    _next1->setFrequency(std::max(MAX_COLD_BLOCK_COUNT + 1,
                                  (_next1->getFrequency() - succ1Freq)));
    if (!_next1->getSuccessors().empty()) {
      TR::CFGEdge *successorEdge = _next1->getSuccessors().front();
      successorEdge->setFrequency(
          std::max(MAX_COLD_BLOCK_COUNT + 1,
                   (successorEdge->getFrequency() - succ1Freq)));
    }
  }

  if (succ2Freq > 0) {
    _next2->setFrequency(
        std::max(MAX_COLD_BLOCK_COUNT, (_next2->getFrequency() - succ2Freq)));
    if (!_next2->getSuccessors().empty()) {
      TR::CFGEdge *successorEdge = _next2->getSuccessors().front();
      successorEdge->setFrequency(
          std::max(MAX_COLD_BLOCK_COUNT + 1,
                   (successorEdge->getFrequency() - succ2Freq)));
    }
  }
  ...
  if (getFallThroughBlock(_block) != joinBlock) {
    ...
    _block->append(TR::TreeTop::create(
        comp(),
...
TR::TreeTop *OMR::CFGSimplifier::getLastRealTreetop(TR::Block *block) {
  ...
  if (treeTop->getNode()->getOpCodeValue() == TR::BBStart) return NULL;
  return treeTop;
}
...
/* So far, we've matched the normal pattern, except that instead of an ifcmpxx,
   we have a branch that uses the condCode. To fully match the pattern, we need
   the following:

   1) The final block must start with a compare-and-branch
   2) One of the things being compared must be the same variable that was stored
   to previously (eg. we conditionally store 0 or another value to x; we need to
   be comparing x to something here)
 */
bool OMR::CFGSimplifier::simplifyCondCodeBooleanStore(TR::Block *joinBlock,
                                                      TR::Node *branchNode,
                                                      TR::Node *store1Node,
                                                      TR::Node *store2Node) {
  ...
  if (compareTreeTop == NULL) return false;
  TR::Node *compareNode = compareTreeTop->getNode();
  if (compareNode == NULL ||
      compareNode->getOpCode().convertIfCmpToCmp() == TR::BadILOp)
    return false;
  else if (compareNode == NULL)
    return false;

  int valueIndex = 0;
  if (store1Node->getOpCode().isIndirect()) valueIndex = 1;
  ...
  TR::Node *loadNode = NULL, *constNode = NULL,
           *child1 = compareNode->getFirstChild(),
           *child2 = compareNode->getSecondChild();
  if (NULL != child1 && child1->getOpCode().isInteger()) {
    // Child can be an AND
    if (child1->getOpCode().isAnd()) {
      // First child must be a load; second child must be a const equal to the
      // non-zero const that we stored
      TR::Node *c1 = child1->getFirstChild(), *c2 = child1->getSecondChild();
      if (NULL != c1 && c1->getOpCode().isLoad()) loadNode = c1;
      if (NULL != c2 && c2->getOpCode().isLoadConst()) {
        int32_t constVal = c2->get32bitIntegralValue();
        if (constVal == 0 || (constVal != value1Node->get32bitIntegralValue() &&
                              constVal != value2Node->get32bitIntegralValue()))
          loadNode = NULL;
      }
    }
    // Child can be a conversion
    else if (child1->getOpCode().isConversion() &&
             child1->getFirstChild() != NULL) {
  ...
  if (NULL != child2 && child2->getOpCode().isLoadConst()) constNode = child2;

  if (NULL == loadNode || NULL == constNode) return false;

  // The load node must load from the variable that was previously stored to
  if (store1Node->getSymbolReference()->getSymbol() !=
      loadNode->getSymbolReference()->getSymbol())
    return false;
  ...
  int32_t value1 = value1Node->get32bitIntegralValue(),
          value2 = value2Node->get32bitIntegralValue(),
          compareConst = constNode->get32bitIntegralValue();
  if (compareConst != value1 && compareConst != value2) return false;
  ...
  // The simpler case is when the comparison check matches the taken branch (eg.
  // x = 0; if (x == 0) then ...)
  ...
  // TODO: Handle cases where there's a test for order; sometimes normal,
  // sometimes swap, sometimes can guarantee that only one path will be taken
  {
    traceMsg(comp(),
             "CFGSimplifier condCode pattern matches but uses test for "
  ...
  if (!performTransformation(
          comp(),
          "%sReplace (branch on condition code [%p] -> boolean stores -> "
          "branch-and-compare using stored boolean) with single branch on "
          "condition code\n",
          OPT_DETAILS, branchNode))
    return false;

  // Find the fallthrough and taken blocks
  TR::Block *fallthroughBlock = getFallThroughBlock(joinBlock),
            *takenBlock = NULL;
  TR::CFGEdgeList &joinBlockSuccessors = joinBlock->getSuccessors();

  TR_ASSERT(
      joinBlockSuccessors.size() == 2,
  ...
  TR_ASSERT(fallthroughBlock != NULL && takenBlock != NULL &&
                fallthroughBlock != takenBlock,
  ...
  /* At this point, we have the following structure:
     ...
     if (x == 1)
        ...
        do some other thing
     We can't use the structure changes described above and maintain
     correctness; we need to either reverse the cond code mask, if that's
     allowed, or swap the taken and fallthrough blocks so that we branch to the
     old fallthrough block and fall through to the old taken block. If it's safe
     to invert the mask bits, we can do so and behave as normal. Otherwise, we
     can reverse the blocks, but we also need to add a block containing only a
     goto node to ensure that block A falls through to block F without having to
     reorder the list of blocks. The resulting structure will be the following:
     Block A: ends with the instruction preceding the branch
                       |
     Block D: branch replaces the if
                /             \
  ...
  if (needToSwap && !canReverseBranchMask()) {
    ...
    TR::Block *newGotoBlock = TR::Block::createEmptyBlock(
        lastNode, comp(), fallthroughBlock->getFrequency(), NULL);

    // Create a goto node in the new block, and have it branch to the new
    // fallthrough block
    ...
    _cfg->addNode(newGotoBlock,
                  fallthroughBlock->getParentStructureIfExists(_cfg));
    _cfg->addEdge(TR::CFGEdge::createEdge(joinBlock, newGotoBlock,
                                          trMemory()));  // Add edge from D to G
    _cfg->addEdge(TR::CFGEdge::createEdge(newGotoBlock, fallthroughBlock,
                                          trMemory()));  // Add edge from G to E
...
const char *OMR::CFGSimplifier::optDetailString() const throw() {
