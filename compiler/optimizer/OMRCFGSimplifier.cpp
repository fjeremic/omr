...
bool OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(bool needToDuplicateTree)
{
  ...
  TR::Node* checkcastAndNULLCHKNode = TR::Node::createWithSymRef(compareNode->getFirstChild(), TR::checkcastAndNULLCHK,
    2, comp()->getSymRefTab()->findOrCreateCheckCastSymbolRef(comp()->getMethodSymbol()));
...
static bool containsIndirectOperationImpl(TR::Node* node, TR::NodeChecklist* visited, int32_t depth)
{
  ...
  if (!(node->getOpCode().isArithmetic() && !node->getOpCode().isDiv()) && !node->getOpCode().isLoadVarDirect()
    && !node->getOpCode().isTernary() && !node->getOpCode().isLoadConst())
    return true;
...
bool OMR::CFGSimplifier::simplifyCondStoreSequence(bool needToDuplicateTree)
{
  ...
  if (!performTransformation(comp(),
        "%sReplace conditional stores in block_%d with stores of appropriate ternary at nodes\n", OPT_DETAILS,
        toCheck->getNumber()))
    return false;
...
bool OMR::CFGSimplifier::simplifyNullToException(bool needToDuplicateTree)
{
  ...
  if (compareNode->getSecondChild()->getOpCodeValue() != TR::aconst || compareNode->getSecondChild()->getAddress() != 0)
    return false;
  ...
  if (!performTransformation(comp(),
        "%sReplace ifacmpeq/ifacmpne of NULL node n%dn [%p] to a blcok ending in throw with a NULLCHK to a catch which "
        "goes to block_%d\n",
...
bool OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree)
{
  ...
  // Is this code really valid when the trees guarded by the if rely on the condition checked in the 'if' (e.g. we could
  // have an indirect access without any checking in guarded code, because the test checked if the value was NULL, in
  // which case by performing the simplification we could end up with a crash when the object is dereferenced)
  ...
  TR::Block* joinBlock, TR::Node* branchNode, TR::Node* store1Node, TR::Node* store2Node)
{
  ...
  if (NULL != child1 && child1->getOpCode().isInteger()) {
    ...
    if (child1->getOpCode().isAnd()) {
      ...
      if (NULL != c2 && c2->getOpCode().isLoadConst()) {
        ...
        if (constVal == 0
          || (constVal != value1Node->get32bitIntegralValue() && constVal != value2Node->get32bitIntegralValue()))
          loadNode = NULL;
      }
    }
  ...
  // Everything matches
  if (!performTransformation(comp(),
        "%sReplace (branch on condition code [%p] -> boolean stores -> branch-and-compare using stored boolean) with "
        "single branch on condition code\n",
