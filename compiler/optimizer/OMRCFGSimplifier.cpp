...
bool OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(
  ...
  if (compareNode->getOpCodeValue() == TR::ificmpeq) {
    ...
    if (trace())
      traceMsg(comp(), "   Found an ificmpeq of an instanceof against zero "
                       "which throws on taken size\n");
    throwBlock = _next2;
    ...
    if (trace())
      traceMsg(comp(), "   Found an ificmpne of an instance of against zero "
                       "which throws on the fallthrough path\n");
    throwBlock = _next1;
...
bool OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree) {
  ...
  if (getFallThroughBlock(_block) != joinBlock) {
    ...
    _block->append(
        TR::TreeTop::create(comp(), TR::Node::create(compareNode, TR::Goto, 0,
                                                     joinBlock->getEntry())));
  }
  if (trace())
...
bool OMR::CFGSimplifier::simplifyCondCodeBooleanStore(TR::Block *joinBlock,
  ...
  {
    traceMsg(comp(), "CFGSimplifier condCode pattern matches but uses test for "
