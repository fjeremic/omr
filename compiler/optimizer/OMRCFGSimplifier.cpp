...
TR::Optimization*
OMR::CFGSimplifier::create(TR::OptimizationManager* manager)
{
...
bool
OMR::CFGSimplifier::simplifyIfStructure()
{
  if (trace())
    traceMsg(
      comp(), "Attempting if simpliciaton on block_%d\n", _block->getNumber());
...
bool
OMR::CFGSimplifier::simplifyIfPatterns(bool needToDuplicateTree)
{
...
bool
OMR::CFGSimplifier::hasExceptionPoint(TR::Block* block, TR::TreeTop* end)
{
  return true;
...
bool
OMR::CFGSimplifier::simplifyInstanceOfTestToCheckcast(bool needToDuplicateTree)
{
  ...
  if (trace())
    traceMsg(comp(),
             "Start simplifyInstanceOfTestToCheckcast block_%d\n",
             _block->getNumber());
  ...
  if (trace())
    traceMsg(comp(),
             "   Found an ificmp[eq/ne] against zero n%dn\n",
             compareNode->getGlobalIndex());
  ...
  catchBlock->setHandlerInfo(
    0, comp()->getInlineDepth(), 0, comp()->getCurrentMethod(), comp());
  ...
  if (trace())
    traceMsg(comp(),
             "Create checkcastAndNULLCHK Node n%dn\n",
...
static bool
containsIndirectOperationImpl(TR::Node* node,
...
static bool
containsIndirectOperation(TR::Compilation* comp, TR::TreeTop* treetop)
{
  TR::NodeChecklist visited(comp);
  return containsIndirectOperationImpl(
    treetop->getNode()->getFirstChild(), &visited, 3);
}

bool
OMR::CFGSimplifier::simplifyCondStoreSequence(bool needToDuplicateTree)
{
  ...
  if (trace())
    traceMsg(comp(),
             "Start simplifyCondStoreSequence block_%d\n",
             _block->getNumber());
  ...
  if (trace()) {
    traceMsg(comp(),
             "   block%d triangle1: %d triangle2: %d\n",
             _block->getNumber(),
             triangle1,
             triangle2);
  }
  ...
  while (treeCursor->getNode()->getOpCode().isStoreDirect() &&
    ...
    if (trace())
      traceMsg(comp(),
               "   Store node n%dn data type checks out\n",
    ...
    if (trace())
      traceMsg(comp(),
               "   Store node n%dn symRef checks out\n",
               treeCursor->getNode()->getGlobalIndex());
  ...
  TR::Node* condition =
    TR::Node::create(compareNode,
                     compareNode->getOpCode().convertIfCmpToCmp(),
                     2,
                     compareNode->getFirstChild(),
  ...
  while (treeCursor->getNode()->getOpCode().isStoreDirect() &&
    ...
    TR::Node* select = TR::Node::create(
      storeNode,
      comp()->il.opCodeForTernarySelect(storeNode->getDataType()),
      3);
    if (trace())
    ...
    TR::TreeTop* insTree = TR::TreeTop::create(
      comp(),
      TR::Node::createWithSymRef(storeNode,
                                 storeNode->getOpCodeValue(),
                                 1,
                                 select,
                                 storeNode->getSymbolReference()));
...
bool
OMR::CFGSimplifier::simplifySimpleStore(bool needToDuplicateTree)
{
  ...
  if (trace())
    traceMsg(
      comp(), "Start simplifySimpleStore block_%d\n", _block->getNumber());
  ...
  if (trace()) {
    traceMsg(comp(),
             "   block%d triangle1: %d triangle2: %d diamond: %d\n",
             _block->getNumber(),
             triangle1,
             triangle2,
             diamond);
  ...
  TR::Node* select = TR::Node::create(
    storeNode, comp()->il.opCodeForTernarySelect(storeNode->getDataType()), 3);
  ...
  if (trace())
    traceMsg(comp(),
             "End simplifySimpleStore. New ternary node is n%dn\n",
             select->getGlobalIndex());
...
bool
OMR::CFGSimplifier::simplifyNullToException(bool needToDuplicateTree)
{
  ...
  if (!performTransformation(
        comp(),
        "%sReplace ifacmpeq/ifacmpne of NULL node n%dn [%p] to a blcok ending "
        "in throw with a NULLCHK to a catch which goes to block_%d\n",
        OPT_DETAILS,
        compareNode->getGlobalIndex(),
        compareNode,
        nullBlock->getNumber()))
  ...
  TR::Node* nullchkNode = TR::Node::createWithSymRef(
    ...
    1,
    TR::Node::create(
      compareNode, TR::PassThrough, 1, compareNode->getFirstChild()),
    comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(
      comp()->getMethodSymbol()));
  if (trace())
    traceMsg(comp(),
             "End simplifyNullToException. New NULLCHK node is n%dn\n",
             nullchkNode->getGlobalIndex());
  compareTreeTop->insertBefore(TR::TreeTop::create(comp(), nullchkNode));

  TR::Block* catchBlock =
    TR::Block::createEmptyBlock(compareNode, comp(), nullBlock->getFrequency());
  catchBlock->setHandlerInfo(
    0, comp()->getInlineDepth(), 0, comp()->getCurrentMethod(), comp());
  TR::Node* gotoNode = TR::Node::create(compareNode, TR::Goto, 0);
...
//
bool
OMR::CFGSimplifier::simplifyBooleanStore(bool needToDuplicateTree)
{
  ...
  if (trace())
    traceMsg(
      comp(), "   Found a Compare node n%dn\n", compareNode->getGlobalIndex());
  ...
  if (trace())
    traceMsg(
      comp(), "   Successor block_%d is single store\n", _next1->getNumber());
  ...
  if (succ1Freq > 0) {
    _next1->setFrequency(
      std::max(MAX_COLD_BLOCK_COUNT + 1, (_next1->getFrequency() - succ1Freq)));
    if (!_next1->getSuccessors().empty()) {
      TR::CFGEdge* successorEdge = _next1->getSuccessors().front();
      successorEdge->setFrequency(std::max(
        MAX_COLD_BLOCK_COUNT + 1, (successorEdge->getFrequency() - succ1Freq)));
    }
  }
  ...
  if (succ2Freq > 0) {
    ...
    if (!_next2->getSuccessors().empty()) {
      ...
      successorEdge->setFrequency(std::max(
        MAX_COLD_BLOCK_COUNT + 1, (successorEdge->getFrequency() - succ2Freq)));
    }
  }
  ...
  if (trace())
    traceMsg(comp(),
             "End simplifyBooleanStore. New store node is n%dn\n",
             storeNode->getGlobalIndex());
  return true;
}

TR::TreeTop*
OMR::CFGSimplifier::getNextRealTreetop(TR::TreeTop* treeTop)
{
...
TR::TreeTop*
OMR::CFGSimplifier::getLastRealTreetop(TR::Block* block)
{
...
TR::Block*
OMR::CFGSimplifier::getFallThroughBlock(TR::Block* block)
{
 ...
 */
bool
OMR::CFGSimplifier::simplifyCondCodeBooleanStore(TR::Block* joinBlock,
                                                 TR::Node* branchNode,
                                                 TR::Node* store1Node,
                                                 TR::Node* store2Node)
{
  ...
  if (needToSwap && !canReverseBranchMask()) {
    ...
    _cfg->addEdge(TR::CFGEdge::createEdge(
      joinBlock, newGotoBlock, trMemory())); // Add edge from D to G
    _cfg->addEdge(TR::CFGEdge::createEdge(
      newGotoBlock, fallthroughBlock, trMemory())); // Add edge from G to E
...
bool
OMR::CFGSimplifier::canReverseBranchMask()
{
  return false;
}
