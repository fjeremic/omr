...
// Basically tries to schedule the instructions so it reduces live range of
// registers.
//
 ...
 * at a time and aims to reduce the live range of commoned nodes (virtual
 * registers in the code generator) with a view to reducing the likelihood of
 * register spilling. Entire trees are moved if necessary and also subtrees are
 * extracted and anchored under treetop nodes with a view of keeping all the
 * references to a particular node as close together as possible (while also
 * trying to do the same wrt live ranges of the children of the node). Since the
 * optimization moves nodes around it needs to consult aliasing to ensure that
 * the semantics of the original program are not changed. Some other IL rules
 * that are not expressed as aliasing relationships (e.g. an idiv node must be
 * under a DIVCHK in Java) need to be enforced by checking explicitly that the
 * code motion would still result in correct IL.
 *
 */
...
private:
  ...
  bool isAnySymInDefinedOrUsedBy(TR_TreeRefInfo *, TR::Node *,
                                 TR_TreeRefInfo *);
  ...
  void updateRefInfo(TR::Node *n, TR_TreeRefInfo *currentTreeRefInfo,
  ...
  List<TR_TreeRefInfo>
      _movedTreesList; // List of moved trees during pass 1 of the optimization
  TR_BitVector *_temp;
...
public:
  ...
  TR_TreeRefInfo(TR::TreeTop *treeTop, TR_Memory *m)
      : _treeTop(treeTop), _firstRefNodes(m), _midRefNodes(m),
        _lastRefNodes(m) {
    _defSym = NULL;
    _useSym = NULL;
