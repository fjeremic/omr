...
// upper bound is determined by numChunk (defined as uint16_t) of
// interferenceMatrix BitVector (2800^2 / 2 / 64 < 64K)
...
// Compact Locals
//
// Reduce local stack slot usage by having non-interfering locals share the same
// slot. Uses the graph colouring mechanism.
//
...
int32_t TR_CompactLocals::perform() {
  ...
  if (((int64_t)comp()->getFlowGraph()->getNumberOfNodes() * numLocals *
       numLocals / hotnessFactor) > COMPACT_LOCALS_COMPLEXITY_LIMIT) {
    if (!comp()->getOption(TR_ProcessHugeMethods)) canAffordAnalysis = false;
  }
  ...
  //
  _localIndexToIGNode =
      new (trStackMemory()) TR::vector<TR_IGNode *, TR::Region &>(
          numLocals, static_cast<TR_IGNode *>(NULL),
  ...
  for (p = locals.getFirst(); p != NULL; p = locals.getNext()) {
    ...
    if (eligibleLocal(p)) {
      ...
      // pinning array autos
      //
      if (!p->isInternalPointer() && !p->isPinningArrayPointer() &&
  ...
  // Build the live on exit sets for each block and determine interferences
  // between the live ranges of all locals.
  //
  _liveVars =
      new (trStackMemory()) TR_BitVector(numLocals, trMemory(), stackAlloc);
  _prevLiveVars =
  ...
  while (block) {
    ...
    for (; tt != firstTT; tt = tt->getPrevTreeTop()) {
      ...
      processNodeInPreorder(tt->getNode(), visitCount, &liveLocals, block,
                            true);
    }
    ...
    TR_ASSERT(
        _localsIG->getNumNodes() >= MAX_NUMBER_OF_LOCALS ||
...
void TR_CompactLocals::processNodeInPreorder(TR::Node *node,
  ...
  if (node->getOpCode().isStoreDirect() /* && directChildOfTreeTop */) {
    ...
    *_temp -= *_prevLiveVars;
    ...
    if (!_temp->isEmpty()) createInterferenceBetween(_liveVars);
  }
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
    processNodeInPreorder(node->getChild(i), visitCount, liveLocals, block,
                          false);
  }
}
...
//
void TR_CompactLocals::createInterferenceBetween(TR_BitVector *bv1,
                                                 TR_BitVector *bv2) {
...
void TR_CompactLocals::createInterferenceBetweenLocals(int32_t localIndex) {
  ...
  while (lvi.hasMoreElements()) {
    ...
    if (liveLocalIndex == localIndex) continue;
...
bool TR_CompactLocals::eligibleLocal(TR::AutomaticSymbol *localSym) {
  if (localSym->getLiveLocalIndex() == INVALID_LIVENESS_INDEX) return false;

  if (localSym->isLocalObject()) return false;
...
const char *TR_CompactLocals::optDetailString() const throw() {
