...
TR_CompactLocals::perform()
{
  ...
  if (trace()) {
    ...
    traceMsg(comp(),
             "after initial size interferences numChunks=%d\n",
  ...
  while (block) {
    ...
    for (; tt != firstTT; tt = tt->getPrevTreeTop()) {
      ...
      processNodeInPreorder(
        tt->getNode(), visitCount, &liveLocals, block, true);
    }
    ...
    TR_ASSERT(_localsIG->getNumNodes() >= MAX_NUMBER_OF_LOCALS ||
                *_liveVars ==
                  *liveLocals._blockAnalysisInfo[block->getNumber()],
...
void
TR_CompactLocals::processNodeInPreorder(TR::Node* node,
                                        vcount_t visitCount,
                                        TR_Liveness* liveLocals,
                                        TR::Block* block,
                                        bool directChildOfTreeTop)
{
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
    processNodeInPreorder(
      node->getChild(i), visitCount, liveLocals, block, false);
  }
}
...
TR_CompactLocals::createInterferenceBetweenLocals(int32_t localIndex)
{
  ...
  while (lvi.hasMoreElements()) {
    ...
    if (ig1 && ig2) {
      if (trace() && !_localsIG->hasInterference(ig1, ig2)) {
        traceMsg(comp(),
                 "Adding interference between %d and %d\n",
                 liveLocalIndex,
                 localIndex);
      }
      _localsIG->addInterferenceBetween(ig1, ig2);
...
bool
TR_CompactLocals::eligibleLocal(TR::AutomaticSymbol* localSym)
{
...
void
TR_CompactLocals::assignColorsToSymbols(TR_BitVector* bv)
{
  ;
}
...
TR_CompactLocals::doCompactLocals()
{
  ...
  if (trace()) {
    ...
    traceMsg(comp(),
             ...
             _localsIG->getNumberOfColoursUsedToColour(),
             comp()->signature());
  }
