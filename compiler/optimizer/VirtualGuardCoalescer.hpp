...
private:
  ...
  public:
    ...
    void addChild(VGInfo*)
    {
      ++_referenceCount;
    }
    void removeChild(VGInfo*)
    {
      --_referenceCount;
    }
    ...
    {
      return _call->getExceptionSuccessors().empty();
    }
    ...
    TR::Block* getMergeBlock()
    {
      return _merge;
    }
    TR::Block* getCallBlock()
    {
      return _call;
    }
    TR::Block* getFirstInlinedBlock()
    {
      return _inlined;
    }
...
public:
  ...
  public:
    TR_ALLOC(TR_Memory::VirtualGuardTailSplitter)
    GuardInfo(
      TR::Compilation*, TR::Block* block, GuardInfo* parent, TR_ValueNumberInfo* vnInfo, uint32_t numInlinedSites);
    ...
    {
      return _block->getLastRealTreeTop()->getNode();
    }
    TR::Node* getCallNode()
    {
    ...
    void setVisible(uint32_t inner)
    {
      _innerSubTree->set(inner);
    }
    bool isVisible(uint32_t inner)
    {
      return _innerSubTree->isSet(inner);
    }
    TR_BitVector* getVisibleGuards()
    {
      return _innerSubTree;
    }

    void setHasBeenDevirtualized()
    {
      _hasBeenDevirtualized = true;
    }
    bool getHasBeenDevirtualized()
    {
      return _hasBeenDevirtualized;
    }
