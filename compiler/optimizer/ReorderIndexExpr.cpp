...
void TR_IndexExprManipulator::rewriteIndexExpression(TR_PrimaryInductionVariable* primeIV, TR::Node* parentNode, TR::Node* node, bool parentIsAiadd)
{
        ...
        if (parentIsAiadd) {
            if (childNode->getOpCode().hasSymbolReference() && childNode->getSymbol() == primeIV->getSymRef()->getSymbol()) {
                ...
                if (childNode->cannotOverflow() && // no wrapping of index can happen, hence safe to move around
                    ...
                    node->getReferenceCount() < 2 && node->getOpCodeValue() == parentNode->getOpCodeValue() && node->getOpCode().isCommutative()) {
