...
/**
 * For non-trivial array index expressions, it's important to regroup them so
 * that as much of the array invarient portion is in a subtree as possible so
 * PRE hoists it e.g.
 *
 *   for (i=0;i < endI;++i)
 *     for (j=0; j < endJ; ++j)
 ...
 */
TR_IndexExprManipulator::TR_IndexExprManipulator(
...
int32_t
TR_IndexExprManipulator::perform()
{
...
void
TR_IndexExprManipulator::rewriteIndexExpression(
  TR_PrimaryInductionVariable* primeIV,
  TR::Node* parentNode,
  TR::Node* node,
  bool parentIsAiadd)
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i) {
    ...
    if (parentIsAiadd) {
      if (childNode->getOpCode().hasSymbolReference() &&
        ...
        if (_debug)
          traceMsg(comp(),
                   "Found reference [%p] to primeiv %p\n",
                   childNode,
                   childNode->getSymbol());
        if (childNode->cannotOverflow() && // no wrapping of index can happen,
                                           // hence safe to move around
            parentNode->getReferenceCount() < 2 && // safe to swap nodes
            node->getReferenceCount() < 2 &&
            node->getOpCodeValue() == parentNode->getOpCodeValue() &&
          ...
          if (performTransformation(comp(),
                                    "%sSwapping nodes [%p] and [%p] to create "
                                    "larger loop invariant sub-expression\n",
                                    OPT_DETAILS,
                                    childNode,
                                    parentNode->getChild(swapChildIdx))) {
            if (false)
              printf("%sSwapping nodes [%p] and [%p] to create larger loop "
