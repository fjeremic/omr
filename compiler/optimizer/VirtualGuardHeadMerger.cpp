...
int32_t TR_VirtualGuardHeadMerger::perform()
{
    ...
    for (TR::Block* block = optimizer()->getMethodSymbol()->getFirstTreeTop()->getNode()->getBlock();
        ...
        if (isMergeableGuard(guard1)) {
            ...
            if (!disableVGHeadMergerTailSplitting && (cold1->getSuccessors().size() == 1) && cold1->hasSuccessor(block->getNextBlock()) && cold1->getLastRealTreeTop()->getNode()->getOpCode().isGoto()) {
                // TODO handle moving code earlier in the block down below the guard
                // tail split
                if ((block->getNextBlock()->getSuccessors().size() == 1 || (block->getNextBlock()->getSuccessors().size() == 2 && block->getNextBlock()->getLastRealTreeTop()->getNode()->getOpCode().isBranch())) && performTransformation(comp(), "%sCloning block_%d and placing clone after block_%d to reduce HCR guard nops\n", OPT_DETAILS, block->getNextBlock()->getNumber(), cold1->getNumber()))
            ...
            for (TR::Block* nextBlock = block->getNextBlock(); nextBlock; nextBlock = nextBlock->getNextBlock()) {
                ...
                } else if (guard2->getInlinedSiteIndex() > -1 && guard1->getInlinedSiteIndex() == TR::comp()->getInlinedCallSite(guard2->getInlinedSiteIndex())._byteCodeInfo.getCallerIndex()) {
                    if (trace())
...
void TR_VirtualGuardHeadMerger::tailSplitBlock(TR::Block* block, TR::Block* cold1)
{
    ...
    if (tailSplitEnd->getOpCode().isGoto()) {
        ...
        !tailSplitEnd->getOpCode().isReturn() && !tailSplitEnd->getOpCode().isJumpWithMultipleTargets() && tailSplitEnd->getOpCodeValue() != TR::athrow && !(tailSplitEnd->getNumChildren() >= 1 && tailSplitEnd->getFirstChild()->getOpCodeValue() == TR::athrow)) {
