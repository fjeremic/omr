...
static bool safeToMoveGuard(
  TR::Block* destination, TR::TreeTop* guardCandidate, TR::TreeTop* branchDest, TR_BitVector& privArgSymRefs)
{
      ...
      if (tt->getNode()->chkIsPrivatizedInlinerArg()
        && (disablePrivArgMovement ||
          ...
          (guardCandidate->getNode()->getInlinedSiteIndex() > -1 &&
            // if priv arg store does not have the same inlined site index as the guard's caller, that means it is not a
            // priv arg for this guard, then we cannot move the guard and its priv args up across other calls' priv args
            tt->getNode()->getInlinedSiteIndex()
              != TR::comp()
                   ->getInlinedCallSite(guardCandidate->getNode()->getInlinedSiteIndex())
  ...
  for (TR::Block* block = optimizer()->getMethodSymbol()->getFirstTreeTop()->getNode()->getBlock(); block;
    ...
    if (isMergeableGuard(guard1)) {
      ...
      if (!disableVGHeadMergerTailSplitting && (cold1->getSuccessors().size() == 1)
        && cold1->hasSuccessor(block->getNextBlock()) && cold1->getLastRealTreeTop()->getNode()->getOpCode().isGoto()) {
      ...
      for (TR::Block* nextBlock = block->getNextBlock(); nextBlock; nextBlock = nextBlock->getNextBlock()) {
        ...
        if (guard1->getInlinedSiteIndex() == guard2->getInlinedSiteIndex()) {
          ...
          && guard1->getInlinedSiteIndex()
            == TR::comp()->getInlinedCallSite(guard2->getInlinedSiteIndex())._byteCodeInfo.getCallerIndex()) {
          if (trace())
            traceMsg(
              comp(), "  Guard1 [%p] is the caller of Guard2 [%p] - proceeding with guard merging\n", guard1, guard2);
          ...
          if (trace())
            traceMsg(comp(),
              "  Cold1 block_%d and cold2 block_%d of guard2 [%p] in unknown relationship - abandon the merge "
              "attempt\n",
        ...
        if (!isStopTheWorldGuard(guard2)) {
          ...
          if (coldPathLoads.intersects(privArgSymRefs)) {
            if (trace())
              traceMsg(comp(),
                "  Recycled temp live in cold1 block_%d and used as privarg before guard2 [%p] - stop guard merging",
        ...
        if (!performTransformation(comp(), "%sRedirecting %s guard [%p] in block_%d to parent guard cold block_%d\n",
              OPT_DETAILS, isStopTheWorldGuard(guard2) ? "stop the world" : "runtime", guard2, guard2Block->getNumber(),
              cold1->getNumber()))
          continue;
...
void TR_VirtualGuardHeadMerger::tailSplitBlock(TR::Block* block, TR::Block* cold1)
{
  ...
  if (tailSplitEnd->getOpCode().isGoto()) {
    ...
    cfg->addEdge(
      tailSplitBlock, tailSplitBlock->getLastRealTreeTop()->getNode()->getBranchDestination()->getEnclosingBlock());
