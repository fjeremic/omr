...
static void countReplicationFailure(char* failureReason, int32_t regionNum)
{
  ...
  TR::DebugCounter::incStaticDebugCounter(TR::comp(),
    TR::DebugCounter::debugCounterName(TR::comp(), "LoopReplicator/%s/%s/(%s)/region_%d", failureReason,
      TR::comp()->getHotnessName(TR::comp()->getMethodHotness()), TR::comp()->signature(), regionNum));
}
...
int32_t TR_LoopReplicator::perform(TR_Structure* str)
{
  ...
  for (TR_StructureSubGraphNode* sgNode = snIt.getCurrent(); sgNode && (branchNode == NULL); sgNode = snIt.getNext()) {
  ...
  for (TR_RegionStructure* loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    if (isInnerLoopHot) {
      ...
      if (!searchList(innerLoopHeader, common, lInfo)) {
        ...
        traceMsg(
          comp(), "not going to replicate loop because hot inner loop %d is not on the trace\n", loop->getNumber());
        return false;
      }
    }
  }
...
TR::Block* TR_LoopReplicator::bestSuccessor(TR_RegionStructure* region, TR::Block* node, TR::CFGEdge** edge)
{
  ...
  for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
    ...
    if (!notExitEdge || destStructure == region->getEntry()->getStructure()) {
      if (trace())
        traceMsg(
          comp(), "      isRegionExit? (%s) successor structure %p\n", (!notExitEdge ? "yes" : "no"), destStructure);
      continue;
    }
