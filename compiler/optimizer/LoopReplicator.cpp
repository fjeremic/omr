...
static void
countReplicationFailure(char* failureReason, int32_t regionNum)
{
  ...
  TR::DebugCounter::incStaticDebugCounter(
    ...
    TR::DebugCounter::debugCounterName(
      TR::comp(),
      "LoopReplicator/%s/%s/(%s)/region_%d",
      failureReason,
...
int32_t
TR_LoopReplicator::perform(TR_Structure* str)
{
  ...
  if (!region->isNaturalLoop()) {
    dumpOptDetails(
      comp(), "region (%d) is not a natural loop\n", region->getNumber());
    return 0;
  }
...
bool
TR_LoopReplicator::isWellFormedLoop(TR_RegionStructure* region,
                                    TR_Structure* condBlock)
{
  ...
  for (b = bilIt.getCurrent(); b; b = bilIt.getNext(), numBlocks++) {
    ...
    if (b->hasExceptionSuccessors()) // try
    {
      if (trace())
        traceMsg(
          comp(), "block (%d) has exception successors\n", b->getNumber());
    }
  }
  ...
  if (_maxNestingDepth > MAX_REPLICATION_NESTING_DEPTH) {
    if (traceAny())
      traceMsg(comp(),
               "for loop (%d), max nest depth thresholds exceeded\n",
               region->getNumber());
    return false;
  }
  ...
  if ((numBlocks * MAX_REPLICATION_GROWTH_FACTOR) >
    ...
    if (traceAny())
      traceMsg(comp(),
               "for loop (%d), loop too big, thresholds exceeded\n",
               region->getNumber());
    return false;
  }
...
int32_t
TR_LoopReplicator::replicateLoop(TR_RegionStructure* region,
                                 TR_StructureSubGraphNode* branchNode)
{
  ...
  if (!lastTT->getNode()->getOpCode().isBranch()) {
    ...
    if (trace())
      traceMsg(comp(),
               "no branch condition found in loop (%d)\n",
               region->getNumber());
    return false;
  }

  dumpOptDetails(
    comp(), "picking trace in loop (%d)...\n", region->getNumber());
  ...
  if (trace())
    traceMsg(
      comp(), "gathered information for loop (%d)\n", lInfo->_regionNumber);
  return true;
}
...
int32_t
TR_LoopReplicator::countChildren(TR::Node* node, vcount_t visitCount)
{
...
bool
TR_LoopReplicator::checkInnerLoopFrequencies(TR_RegionStructure* region,
                                             LoopInfo* lInfo)
{
  ...
  for (TR_RegionStructure* loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    if (trace())
      traceMsg(comp(),
               "\t  outerloop relative frequency = %.3g\n",
...
bool
TR_LoopReplicator::shouldReplicateWithHotInnerLoops(
  TR_RegionStructure* region,
  LoopInfo* lInfo,
  TR_ScratchList<TR::Block>* hotInnerLoopHeaders)
{
  ...
  for (;;) {
    ...
    for (auto e = tracePrefixCursor->getSuccessors().begin();
         e != tracePrefixCursor->getSuccessors().end();
         ++e) {
    ...
    if (trace())
      traceMsg(comp(),
               "Checking for cold side-entries targeting block_%d\n",
    ...
    for (auto e = tracePrefixCursor->getPredecessors().begin();
         e != tracePrefixCursor->getPredecessors().end();
         ++e) {
...
bool
TR_LoopReplicator::heuristics(LoopInfo* lInfo)
{
  ...
  if (!lInfo->_replicated)
    dumpOptDetails(
      comp(),
      "no side entrance found into trace; no replication will be performed\n");

  return lInfo->_replicated;
}

void
TR_LoopReplicator::logTrace(LoopInfo* lInfo)
{
  if (!trace())
    return;
...
TR::Block*
TR_LoopReplicator::nextCandidate(TR::Block* X,
                                 TR_RegionStructure* region,
                                 bool doSucc)
{
  ...
  if (Y) {
    ...
    if (!edge) {
      ...
      if (trace())
        traceMsg(comp(),
                 "   candidate (%d) satisfied weight computation\n",
                 Y->getNumber());
      cand = Y;
    }
  }
...
void
TR_LoopReplicator::processBlock(TR::Block* X,
                                TR_RegionStructure* region,
                                LoopInfo* lInfo)
{
  TR::Block* seed = region->getEntryBlock();
...
bool
TR_LoopReplicator::isBackEdgeOrLoopExit(TR::CFGEdge* e,
                                        TR_RegionStructure* region,
                                        bool testSGraph)
{
...
bool
TR_LoopReplicator::computeWeight(TR::CFGEdge* edge)
{
  ...
  if (trace()) {
    traceMsg(comp(),
             "   weighing candidate : %d (Y)  predeccessor : %d (X)\n",
             Y->getNumber(),
             X->getNumber());
    traceMsg(comp(),
...
TR::Block*
TR_LoopReplicator::bestSuccessor(TR_RegionStructure* region,
                                 TR::Block* node,
                                 TR::CFGEdge** edge)
{
  TR::Block* cand = NULL;
  ...
  for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end();
    ...
    if (trace())
      traceMsg(
        comp(), "   analyzing successor block : %d\n", dest->getNumber());
    ...
    if (!notExitEdge || destStructure == region->getEntry()->getStructure()) {
      if (trace())
        traceMsg(comp(),
                 "      isRegionExit? (%s) successor structure %p\n",
                 (!notExitEdge ? "yes" : "no"),
                 destStructure);
      continue;
    }
  ...
  if (cand) {
    ...
    if (trace())
      traceMsg(
        comp(), "   next candidate chosen : %d (Y)\n", cand->getNumber());
  }
  return cand;
}
...
// exit
void
TR_LoopReplicator::nextSuccessor(TR_RegionStructure* region,
                                 TR::Block** cand,
                                 TR::CFGEdge** edge)
{
  ...
  if ((parentStructure != region) &&
    ...
    if (trace())
      traceMsg(comp(),
               "   inner loop detected : %p , exit edges are :\n",
               parentStructure);
    TR::CFGEdge* e = NULL;
...
// is replicated
bool
TR_LoopReplicator::gatherBlocksToBeCloned(LoopInfo* lInfo)
{
  TR_RegionStructure* region = lInfo->_region;
...
void
TR_LoopReplicator::doTailDuplication(LoopInfo* lInfo)
{
  TR_RegionStructure* region = lInfo->_region;
  ...
  for (be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
          ...
          _blockMapper[n->getNumber()])*/
    {
      ...
      for (auto e = newBlock->getExceptionSuccessors().begin();
           e != newBlock->getExceptionSuccessors().end();
           ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                     100)); // cast explicitly
      ...
      for (auto e = newBlock->getExceptionPredecessors().begin();
           e != newBlock->getExceptionPredecessors().end();
           ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                     100)); // cast explicitly
      ...
      for (auto e = newBlock->getPredecessors().begin();
           e != newBlock->getPredecessors().end();
           ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                     100)); // cast explicitly
  ...
  for (auto e = clonedHeader->getSuccessors().begin();
       e != clonedHeader->getSuccessors().end();
       ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100)); // cast explicitly
  ...
  for (auto e = clonedHeader->getExceptionSuccessors().begin();
       e != clonedHeader->getExceptionSuccessors().end();
       ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100)); // cast explicitly
  ...
  for (auto e = clonedHeader->getExceptionPredecessors().begin();
       e != clonedHeader->getExceptionPredecessors().end();
       ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100)); // cast explicitly
  ...
  for (auto e = clonedHeader->getPredecessors().begin();
       e != clonedHeader->getPredecessors().end();
       ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100)); // cast explicitly
...
void
TR_LoopReplicator::addBlocksAndFixEdges(LoopInfo* lInfo)
{
  TR_RegionStructure* region = lInfo->_region;
...
bool
TR_LoopReplicator::checkForSuccessor(TR::Block* dest, TR::Block* loopHeader)
{
...
TR::Block*
TR_LoopReplicator::createEmptyGoto(TR::Block* source,
                                   TR::Block* dest,
                                   bool redirect)
{
  ...
  if (trace())
    traceMsg(comp(),
             "goto block %p freq %d src freq %d dst freq %d\n",
             gotoBlock,
             freq,
             source->getFrequency(),
             dest->getFrequency());
  ...
  TR::Node* gotoNode = TR::Node::create(
    destEntry->getNextTreeTop()->getNode(), TR::Goto, 0, target);
...
TR::TreeTop*
TR_LoopReplicator::findEndTreeTop(TR_RegionStructure* region)
{
...
TR_LoopReplicator::LoopInfo*
TR_LoopReplicator::findLoopInfo(int32_t regionNumber)
{
  LoopInfo* lInfo = NULL;
...
TR_LoopReplicator::BlockEntry*
TR_LoopReplicator::searchList(TR::Block* block, listType lT, LoopInfo* lInfo)
{
...
void
TR_LoopReplicator::modifyLoops()
{
  for (LoopInfo* lInfo = _loopInfo.getFirst(); lInfo;
       lInfo = lInfo->getNext()) {
    // clone blocks
    if (lInfo->_replicated && performTransformation(comp(),
                                                    "%sreplicating loop - %d\n",
                                                    OPT_DETAILS,
                                                    lInfo->_regionNumber)) {
      if (trace()) {
      ...
      dumpOptDetails(
        comp(), "loop (%d) will not be replicated\n", lInfo->_regionNumber);
  }
}

// clone the original loop header and fixup edges
void
TR_LoopReplicator::fixUpLoopEntry(LoopInfo* lInfo, TR::Block* loopHeader)
{
  ...
  for (auto e = loopHeader->getPredecessors().begin();
       e != loopHeader->getPredecessors().end();
       ++e) {
    TR::Block* predBlock = toBlock((*e)->getFrom());
    if (!region->contains(predBlock->getStructureOf(), region->getParent()))
      continue;
  ...
  if (trace())
    traceMsg(comp(),
             "adding successors for new loop header %d\n",
             clonedHeader->getNumber());
  for (auto e = loopHeader->getSuccessors().begin();
       e != loopHeader->getSuccessors().end();
       ++e) {
    EdgeEntry* eE = new (trStackMemory()) EdgeEntry;
    eE->_edge = *e;
    eE->_removeOnly = true;
  ...
  if (trace())
    traceMsg(comp(),
             "added edge orig header(%d) -> new header(%d)\n",
             loopHeader->getNumber(),
             clonedHeader->getNumber());
  ...
  if (trace())
    traceMsg(comp(),
             "fixing back-edges for new loop header %d\n",
             clonedHeader->getNumber());
...
TR_LoopReplicator::findNodeInHierarchy(TR_RegionStructure* region, int32_t num)
{
  if (!region)
    return NULL;
...
void
TR_LoopReplicator::calculateBlockWeights(TR_RegionStructure* region)
{
  ...
  do {
    ...
    if ((node->getStructure()->getParent()->asRegion() == region) &&
      ...
      if ((bFreq > 0) || (node == region->getEntry())) {
        dumpOptDetails(comp(),
                       "positive non-zero frequency %d for block_%d\n",
                       bFreq,
                       node->getNumber());
...
int32_t
TR_LoopReplicator::deriveFrequencyFromPreds(TR_StructureSubGraphNode* curNode,
                                            TR_RegionStructure* region)
{
  ...
  for (auto e = curNode->getPredecessors().begin();
    ...
    if (from->getStructure()->getParent()->asRegion() == region) {
      ...
      // pbr
      dumpOptDetails(
        comp(), "adding block as preds: %d %p\n", from->getNumber(), from);
    }
  }
  ...
  for (TR::Block* b = bIt.getFirst(); b; b = bIt.getNext()) {
    ...
    // pbr
    dumpOptDetails(
      comp(), "cumulative freq for block (%d) is : %d\n", b->getNumber(), f);
    if (!(b->getSuccessors().size() == 1)) {
      ...
      for (auto e = b->getSuccessors().begin(); e != b->getSuccessors().end();
        ...
        // pbr
        dumpOptDetails(
          comp(), "after %d diffing dFreq (%d), f = %d\n", numSuccs, dFreq, f);
      }
      if (scale) {
        f = f / numSuccs;
      }
  ...
  // pbr
  dumpOptDetails(
    comp(), "returned freq for block (%d): %d\n", curNode->getNumber(), freq);
  return freq;
}

bool
TR_LoopReplicator::predecessorsNotYetVisited(TR_RegionStructure* r,
...
int32_t
TR_LoopReplicator::getSeedFreq(TR_RegionStructure* r)
{
...
int32_t
TR_LoopReplicator::getBlockFreq(TR::Block* X)
{
...
int32_t
TR_LoopReplicator::getScaledFreq(TR_ScratchList<TR::Block>& blocks,
                                 TR::Block* X)
{
  int32_t freq = 0;
...
bool
TR_LoopReplicator::heuristics(LoopInfo* lInfo, bool dumb)
{
  TR_RegionStructure* region = lInfo->_region;
