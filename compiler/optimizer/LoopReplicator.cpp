...
static void countReplicationFailure(char* failureReason, int32_t regionNum)
{
    ...
    TR::DebugCounter::incStaticDebugCounter(TR::comp(), TR::DebugCounter::debugCounterName(TR::comp(), "LoopReplicator/%s/%s/(%s)/region_%d", failureReason, TR::comp()->getHotnessName(TR::comp()->getMethodHotness()), TR::comp()->signature(), regionNum));
}
    ...
    for (BlockEntry* be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
        ...
        for (auto e = origBlock->getSuccessors().begin(); e != origBlock->getSuccessors().end(); ++e) {
            ...
            // correspondingly add C1->C2 where C1 & C2 are the respective clones
                ...
                else if (newBlock->getNextBlock() != dest && !branchNode->getOpCode().isJumpWithMultipleTargets() && !branchNode->getOpCode().isGoto()) {
                    // goto required
            ...
            else if (/*(dest->getNumber() < _nodesInCFG) &&*/ dest == origBlock->getNextBlock() && checkForSuccessor(dest, loopHeader)) {
    ...
    for (ee = (lInfo->_removedEdges).getFirst(); ee; ee = ee->getNext()) {
        ...
        if (source->getNextBlock() == dest) {
            ...
            if (branchNode->getOpCode().isSwitch()) {
                ...
                if ((branchNode->getOpCode().isGoto() || branchNode->getOpCode().isBranch()) && (branchNode->getBranchDestination()->getNode()->getBlock()->getNumber() == dest->getNumber())) {
                    // B2 is a goto block
...
void TR_LoopReplicator::modifyLoops()
{
    for (LoopInfo* lInfo = _loopInfo.getFirst(); lInfo; lInfo = lInfo->getNext()) {
        // clone blocks
        if (lInfo->_replicated && performTransformation(comp(), "%sreplicating loop - %d\n", OPT_DETAILS, lInfo->_regionNumber)) {
            if (trace()) {
...
void TR_LoopReplicator::calculateBlockWeights(TR_RegionStructure* region)
{
    ...
    do {
        ...
        if ((node->getStructure()->getParent()->asRegion() == region) && !(node->getStructure()->asRegion() && node->getStructure()->asRegion()->isNaturalLoop())) {
        ...
        for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
            ...
            if (!isBackEdgeOrLoopExit(*e, region, true)) {
                bool innerLoop = ((_seenBlocks[dest->getNumber()] == COLOR_BLACK) && (dest->getStructure()->asRegion() && dest->getStructure()->asRegion()->isNaturalLoop()));
