...
TR_LoopReplicator::TR_LoopReplicator(TR::OptimizationManager *manager)
    : TR_LoopTransformer(manager), _blocksCloned(trMemory()) {
  _nodeCount = 0;
  _nodesInCFG = 0;
  _switchTree = NULL;
  _maxNestingDepth = 0;
...
static void countReplicationFailure(char *failureReason, int32_t regionNum) {
  ...
  TR::DebugCounter::incStaticDebugCounter(
      ...
      TR::DebugCounter::debugCounterName(
          TR::comp(), "LoopReplicator/%s/%s/(%s)/region_%d", failureReason,
...
int32_t TR_LoopReplicator::perform() {
  ...
  if (!_haveProfilingInfo) {
    dumpOptDetails(comp(),
                   "Need profiling information in order to replicate...\n");
    if (trace()) traceMsg(comp(), "method is %s \n", comp()->signature());
    if (!testLR) return 0;
  }
  ...
  _blocksInCFG = (TR::Block **)trMemory()->allocateStackMemory(
      _nodesInCFG * sizeof(TR::Block *));
  memset(_blocksInCFG, 0, _nodesInCFG * sizeof(TR::Block *));
  _blockWeights =
      (int32_t *)trMemory()->allocateStackMemory(_nodesInCFG * sizeof(int32_t));
  ...
  //
  _blocksVisited =
      new (trStackMemory()) TR_BitVector(_nodesInCFG, trMemory(), stackAlloc);
...
int32_t TR_LoopReplicator::perform(TR_Structure *str) {
  ...
  if (!(region = str->asRegion())) return 0;
  ...
  _blockMapper = (TR::Block **)trMemory()->allocateStackMemory(
      _nodesInCFG * sizeof(TR::Block *));
  memset(_blockMapper, 0, _nodesInCFG * sizeof(TR::Block *));

  // analyze 2 kinds of loops
  // (a) do-while loops
  //  contain a block that has 2 edges - one edge exits the region and another
  //  edge is the loop back-edge region
  // (b) while loops
  //  entry block (of the region) has an edge that exits the region. One of the
  //  predecessor edges of the entry is the loop back-edge
  //
  ...
  if (trace()) traceMsg(comp(), "analyzing loop (%d)\n", region->getNumber());
    ...
    for (auto edge = entryNode->getSuccessors().begin();
      ...
      if (region->isExitEdge(*edge)) {
        if (isWellFormedLoop(region, entry)) {
          if (trace()) traceMsg(comp(), "found while loop\n");
          _loopType = whileDo;
  ...
  for (TR_StructureSubGraphNode *sgNode = snIt.getCurrent();
    ...
    for (auto edge = sgNode->getSuccessors().begin();
      ...
      if (dest == region->getEntry()) hasBackEdge = true;
  ...
  if (branchNode) {
    if (trace()) traceMsg(comp(), "found do-while loop\n");
    _loopType = doWhile;
  ...
  dumpOptDetails(
      comp(),
      "loop (%d) does not conform to required form & will not be replicated\n",
      region->getNumber());
  return false;
}

bool TR_LoopReplicator::isWellFormedLoop(TR_RegionStructure *region,
                                         TR_Structure *condBlock) {
  // FIXME:very simple checks for now
  //
  vcount_t visitCount = comp()->incVisitCount();
  // (1)
  // condition has to be in a block
  if (!condBlock->asBlock()) return false;
  ...
  for (b = bilIt.getCurrent(); b; b = bilIt.getNext(), numBlocks++) {
    if (b->hasExceptionPredecessors())  // catch
    {
      if (trace())
        traceMsg(
            comp(),
            "block (%d) has exception predecessors - currently not supported\n",
            b->getNumber());
      return false;
    }
  ...
  for (b = bilIt.getCurrent(); b; b = bilIt.getNext()) {
    ...
    for (TR::TreeTop *tt = entryTT->getNextRealTreeTop(); tt != exitTT;
  ...
  _maxNestingDepth =
      region->getMaxNestingDepth(&currentNestingDepth, &currentMaxNestingDepth);
  ...
  //
  if ((numBlocks * MAX_REPLICATION_GROWTH_FACTOR) >
      MAX_REPLICATION_GROWTH_SIZE) {
    if (traceAny())
...
int32_t TR_LoopReplicator::replicateLoop(TR_RegionStructure *region,
  ...
  if (!testLR) {
    ...
    if (!heuristics(lInfo)) {
      dumpOptDetails(comp(),
                     "failed...unable to select trace inside the loop\n");
      return false;
    }
  } else {
...
int32_t TR_LoopReplicator::countChildren(TR::Node *node, vcount_t visitCount) {
  ...
  if (node->getVisitCount() == visitCount) return 0;
...
static void collectNonColdLoops(TR::Compilation *c, TR_RegionStructure *region,
                                List<TR_RegionStructure> &innerLoops);

// Find non-cold loops strictly nested within region.
static void collectNonColdInnerLoops(TR::Compilation *c,
                                     TR_RegionStructure *region,
                                     List<TR_RegionStructure> &innerLoops) {
...
bool TR_LoopReplicator::checkInnerLoopFrequencies(TR_RegionStructure *region,
                                                  LoopInfo *lInfo) {
  // dont bother with small traces
  //
  int32_t count = 0;
  for (BlockEntry *be = (lInfo->_nodesCommon).getFirst(); be;
       be = be->getNext())
    count++;
  if (count < MIN_BLOCKS_IN_TRACE) return true;

  if (comp()->getFlowGraph()->getMaxFrequency() <= 0) {
    if (trace()) traceMsg(comp(), "no frequency info\n");
    return true;  // maybe this should actually be false...
  }

  if (trace()) traceMsg(comp(), "inspecting non-cold inner loops\n");
  ...
  if (innerLoops.isEmpty()) {
    if (trace())
      traceMsg(
          comp(),
          "failed to find non-cold inner loops; will attempt to replicate\n");
    return true;
  }
  ...
  for (TR_RegionStructure *loop = it.getFirst(); loop; loop = it.getNext()) {
    ...
    if (!isInnerLoopHot && outerLoopFrequency == 6) {
      ...
      if (trace())
        traceMsg(comp(),
                 "\t  considered hot because outer loop has frequency 6\n");
    }

    if (isInnerLoopHot) {
      if (trace()) traceMsg(comp(), "\t  this is a hot inner loop\n");
      ...
      if (!searchList(innerLoopHeader, common, lInfo)) {
        ...
        traceMsg(comp(),
                 "not going to replicate loop because hot inner loop %d is not "
                 "on the trace\n",
                 loop->getNumber());
        return false;
      }
...
bool TR_LoopReplicator::shouldReplicateWithHotInnerLoops(
    TR_RegionStructure *region, LoopInfo *lInfo,
  ...
  if (hotInnerLoopHeaders->isEmpty()) return true;
  ...
  if (trace())
    traceMsg(comp(),
             "Loop has hot inner loops. Looking for early cold side-entry.\n");
  ...
  for (;;) {
    ...
    for (auto e = tracePrefixCursor->getSuccessors().begin();
         e != tracePrefixCursor->getSuccessors().end(); ++e) {
      TR::Block *const succ = toBlock((*e)->getTo());
      if (succ == outerLoopHeader) continue;  // Don't follow back-edges.
      if (!searchList(succ, common, lInfo)) continue;  // Don't leave the trace.
      if (tracePrefixNext != NULL) {
        // Stop due to branching paths within the trace.
        countReplicationFailure("HotInnerLoopHitBranchWithoutColdSideEntry",
                                region->getNumber());
        if (trace())
          traceMsg(comp(),
                   "Hit a branch without finding a cold side-entry. Will not "
                   "replicate.\n");
        return false;
      }
    ...
    if (tracePrefixNext == NULL) {
      ...
      if (trace())
        traceMsg(comp(),
                 "Ran out of trace without finding a cold side-entry. Will not "
                 "replicate.\n");
      return false;
    }
    ...
    for (auto e = tracePrefixCursor->getPredecessors().begin();
         e != tracePrefixCursor->getPredecessors().end(); ++e) {
      TR::Block *const pred = toBlock((*e)->getFrom());
      if (pred->isCold() && !searchList(pred, common, lInfo)) {
        if (trace())
          traceMsg(comp(),
                   "Found a cold side-entry into block_%d from block_%d. Will "
                   "replicate.\n",
    ...
    if (hotInnerLoopHeaders->find(tracePrefixCursor)) {
      countReplicationFailure("HotInnerLoopNoColdSideEntry",
                              region->getNumber());
      if (trace())
        traceMsg(comp(),
                 "Hit a hot inner loop without finding a cold side-entry. Will "
                 "not replicate.\n");
      return false;
    }
  }
...
bool TR_LoopReplicator::heuristics(LoopInfo *lInfo) {
  ...
  // any algorithm can be implemented here; however it should probably have the
  // side effect of populating lInfo->_nodesCommon for a given loop
  //
  ...
  _bStack = new (trStackMemory())
      TR_Stack<TR::Block *>(trMemory(), 32, false, stackAlloc);
  BlockEntry *bE = NULL;
  for (bE = (lInfo->_nodesCommon).getFirst(); bE; bE = bE->getNext())
    _bStack->push(bE->_block);

  if (trace()) traceMsg(comp(), "attempting to extend trace...\n");
  ...
  if (!lInfo->_replicated)
    dumpOptDetails(comp(),
                   "no side entrance found into trace; no replication will be "
                   "performed\n");
...
TR::Block *TR_LoopReplicator::nextCandidate(TR::Block *X,
                                            TR_RegionStructure *region,
                                            bool doSucc) {
  ...
  if (Y) {
    ...
    if (!edge) {
      if (trace()) traceMsg(comp(), "   candidate is %d\n", Y->getNumber());
      cand = Y;
...
void TR_LoopReplicator::processBlock(TR::Block *X, TR_RegionStructure *region,
                                     LoopInfo *lInfo) {
  TR::Block *seed = region->getEntryBlock();
  for (auto e = X->getSuccessors().begin(); e != X->getSuccessors().end();
       ++e) {
    ...
    if (computeWeight(*e)) {
      if (trace())
        traceMsg(
            comp(),
            "   candidate (%d) satisfied weight computation, extending trace\n",
            dest->getNumber());
...
bool TR_LoopReplicator::isBackEdgeOrLoopExit(TR::CFGEdge *e,
                                             TR_RegionStructure *region,
                                             bool testSGraph) {
...
bool TR_LoopReplicator::computeWeight(TR::CFGEdge *edge) {
  ...
  if (trace()) {
    ...
    traceMsg(comp(),
             "      w(Y): %d w(X): %d w(seed): %d w(Y)/w(X): %.4f "
             "w(Y)/w(seed): %.4f\n",
...
TR::Block *TR_LoopReplicator::bestSuccessor(TR_RegionStructure *region,
                                            TR::Block *node,
                                            TR::CFGEdge **edge) {
  TR::Block *cand = NULL;
  ...
  for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end();
    ...
    if (trace())
      traceMsg(comp(),
               "      found parent %p  is block a direct descendent? (%s)\n",
               destStructure->getParent(),
               (region->contains(parentStructure, region->getParent()) ? "yes"
                                                                       : "no"));
    // dumpOptDetails(comp(), "destStruct - %p\n", findNodeInHierarchy(region,
    // destStructure->getNumber()));
    ...
    if (dest->isCold()) continue;

    int32_t destFreq = dest->getFrequency();
    static char *pEnv = feGetEnv("TR_NewLRTracer");
    if (pEnv) destFreq = _blockWeights[dest->getNumber()];
...
// if the current candidate is the start of an inner loop [which would have
// already been analyzed]; this routine skips the loop, picking the inner loop
// exit
void TR_LoopReplicator::nextSuccessor(TR_RegionStructure *region,
                                      TR::Block **cand, TR::CFGEdge **edge) {
  ...
  if ((parentStructure != region) &&
    ...
    for (e = eIt.getFirst(); e; e = eIt.getNext()) {
      ...
      TR_Structure *dest =
          (_blocksInCFG[e->getTo()->getNumber()])->getStructureOf();
      TR_Structure *source =
          (_blocksInCFG[e->getFrom()->getNumber()])->getStructureOf();
      if (trace())
      ...
      if (region->contains(dest, region->getParent())) {
        if (trace())
          traceMsg(comp(), "   found edge to %p (%d)\n", dest,
                   _blocksInCFG[e->getTo()->getNumber()]);
        succEdge = e;
        break;
      }
    }
...
bool TR_LoopReplicator::gatherBlocksToBeCloned(LoopInfo *lInfo) {
  ...
  if (trace()) traceMsg(comp(), "checking for side-entrances :\n");
  ...
  for (be = (lInfo->_nodesCommon).getFirst(); be; be = be->getNext()) {
    ...
    for (auto e = b->getPredecessors().begin(); e != b->getPredecessors().end();
      ...
      if (!sourceBE || _blockMapper[source->getNumber()] || be->_nonLoop) {
        ...
        if (enableInnerLoopChecks && be->_nonLoop && sourceBE &&
  ...
  if (sideEntrance) {
    if (trace()) {
      ...
      traceMsg(comp(), "{ ");
      for (BlockEntry *be = (lInfo->_blocksCloned).getFirst(); be;
           be = be->getNext())
        traceMsg(comp(), " %d ", (be->_block)->getNumber());
      traceMsg(comp(), " }\n");
    }
...
#endif
  if (sideEntrance) {
    if (trace()) traceMsg(comp(), "found a rather cooler backedge\n");
    return true;
  }

  countReplicationFailure("NoSideEntryFound", region->getNumber());
  if (trace()) traceMsg(comp(), "   no side-entrance found\n");
  return false;
}
...
void TR_LoopReplicator::doTailDuplication(LoopInfo *lInfo) {
  ...
  for (be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
          ...
          _blockMapper[n->getNumber()])*/
    {
      ...
      newBlock->setFrequency(
          (int32_t)((block->getFrequency() * BLOCK_THRESHOLD_PERCENTAGE) /
                    100));  // cast explicitly
      if (newBlock->getFrequency() < defaultFrequency)
        newBlock->setFrequency(block->getFrequency());

      for (auto e = newBlock->getSuccessors().begin();
           e != newBlock->getSuccessors().end(); ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                     100));  // cast explicitly
        if ((*e)->getFrequency() < defaultFrequency)
          (*e)->setFrequency(edgeFreq);
      }

      for (auto e = newBlock->getExceptionSuccessors().begin();
           e != newBlock->getExceptionSuccessors().end(); ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                     100));  // cast explicitly
        if ((*e)->getFrequency() < defaultFrequency)
      ...
      for (auto e = newBlock->getExceptionPredecessors().begin();
           e != newBlock->getExceptionPredecessors().end(); ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                     100));  // cast explicitly
        if ((*e)->getFrequency() < defaultFrequency)
      ...
      for (auto e = newBlock->getPredecessors().begin();
           e != newBlock->getPredecessors().end(); ++e) {
        int32_t edgeFreq = (*e)->getFrequency();
        (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                     100));  // cast explicitly
        if ((*e)->getFrequency() < defaultFrequency)
  ...
  loopHeader->setFrequency(
      (int32_t)((origLoopHeaderFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                100));  // cast explicitly
  if (loopHeader->getFrequency() < defaultFrequency)
    loopHeader->setFrequency(origLoopHeaderFreq);

  for (auto e = clonedHeader->getSuccessors().begin();
       e != clonedHeader->getSuccessors().end(); ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100));  // cast explicitly
    if ((*e)->getFrequency() < defaultFrequency) (*e)->setFrequency(edgeFreq);
  }

  for (auto e = clonedHeader->getExceptionSuccessors().begin();
       e != clonedHeader->getExceptionSuccessors().end(); ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100));  // cast explicitly
    if ((*e)->getFrequency() < defaultFrequency) (*e)->setFrequency(edgeFreq);
  ...
  for (auto e = clonedHeader->getExceptionPredecessors().begin();
       e != clonedHeader->getExceptionPredecessors().end(); ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100));  // cast explicitly
    if ((*e)->getFrequency() < defaultFrequency) (*e)->setFrequency(edgeFreq);
  ...
  for (auto e = clonedHeader->getPredecessors().begin();
       e != clonedHeader->getPredecessors().end(); ++e) {
    int32_t edgeFreq = (*e)->getFrequency();
    (*e)->setFrequency((int32_t)((edgeFreq * BLOCK_THRESHOLD_PERCENTAGE) /
                                 100));  // cast explicitly
    if ((*e)->getFrequency() < defaultFrequency) (*e)->setFrequency(edgeFreq);
  ...
  for (be = (lInfo->_blocksCloned).getFirst(); be; be = be->getNext()) {
    TR::Block *n = be->_block;
    if (trace()) traceMsg(comp(), "processing block : %d\n", n->getNumber());
    TR::Block *block = toBlock(n);
    ...
    if (trace()) traceMsg(comp(), "   predecessors : {");
    for (auto e = block->getPredecessors().begin();
         e != block->getPredecessors().end(); ++e) {
      TR::Block *source = toBlock((*e)->getFrom());
      if (trace()) traceMsg(comp(), " %d ", source->getNumber());
      if (!searchList(source, common, lInfo)) {
  ...
  if (trace()) {
    ...
    for (EdgeEntry *ee = (lInfo->_removedEdges).getFirst(); ee;
         ee = ee->getNext()) {
      TR::CFGEdge *e = ee->_edge;
      traceMsg(comp(), "   %d -> %d ; ", e->getFrom()->getNumber(),
               e->getTo()->getNumber());
    }
    traceMsg(comp(), "\n");
  }
...
void TR_LoopReplicator::addBlocksAndFixEdges(LoopInfo *lInfo) {
  ...
  // successors
  // fixup edges and add goto's if required; analyze blocks only on the picked
  // trace B - original block on the trace C - its clone OLH - original loop
  // header (now entry of the outer loop) CLH - cloned loop header (now entry of
  // the inner loop) G - goto block
  for (BlockEntry *be = (lInfo->_blocksCloned).getFirst(); be;
       be = be->getNext()) {
    ...
    for (auto e = origBlock->getSuccessors().begin();
         e != origBlock->getSuccessors().end(); ++e) {
      TR::Block *dest = toBlock((*e)->getTo());
      if (trace())
      ...
      if (dest == loopHeader) {
        ...
        if (branchNode->getOpCode().isBranch()) {
          ...
          if (branchNode->getBranchDestination()->getNode()->getBlock() ==
              loopHeader) {
            // OLH is the target; set the right target in C
            branchNode->setBranchDestination(loopHeader->getEntry());
            _cfg->addEdge(
                TR::CFGEdge::createEdge(dest, loopHeader, trMemory()));
            if (trace())
            ...
            _cfg->addEdge(
                TR::CFGEdge::createEdge(gotoBlock, loopHeader, trMemory()));
            if (trace())
          ...
          if (trace())
            traceMsg(
                comp(),
                "      added  back-edge for jumpwithmultipletarget %d -> %d\n",
                newBlock->getNumber(), dest->getNumber());
          _cfg->addEdge(TR::CFGEdge::createEdge(dest, loopHeader, trMemory()));
        } else {
          // B simply fall-throughs to OLH; add a goto node to the end of C to
          // allow it to branch to OLH and add edge C->OLH
          TR::Node *gotoNode =
              TR::Node::create(branchNode, TR::Goto, 0, loopHeader->getEntry());
      ...
      // correspondingly add C1->C2 where C1 & C2 are the respective clones
        ...
        TR::Node *branchNode =
            newBlock->getExit()->getPrevRealTreeTop()->getNode();
          ...
          if (branchNode->getOpCode().isBranch()) {
            ...
            if (saveDest == target) {
              ...
              if (trace())
                traceMsg(comp(), "      added  edge %d -> %d\n",
                         newBlock->getNumber(), dest->getNumber());
              _cfg->addEdge(
                  TR::CFGEdge::createEdge(newBlock, dest, trMemory()));
            }
          } else if (branchNode->getOpCode().isSwitch()) {
            // B1 is a switch block; fix up the children in C1 so they have
            // correct branch destination targets to C2
            for (int32_t i = branchNode->getCaseIndexUpperBound() - 1; i > 0;
                 i--) {
              TR::Block *target = branchNode->getChild(i)
            ...
            for (int32_t i = 0; i < branchNode->getNumChildren() - 1; i++) {
              ...
              if (target->getNumber() == saveDest->getNumber()) {
                ...
                if (trace())
                  traceMsg(comp(),
                           "   fixed jumpwithmultipletarget child %d -> %d\n",
                           i, dest->getNumber());
              }
            }
            if (trace())
        ...
        // case 3a C1 falls-through to C2; but if C2 is not the next block of C1
        // in the trees, add C1->G->C2
      ...
      // case 4
      // B1->B2 where B1 is on the trace and B2 is not on the trace ie. it is a
      // trace exit and B2 is the next block of B1 in the trees add C1->B2 where
      // C1 is clone of B1
      else if (/*(dest->getNumber() < _nodesInCFG) &&*/ dest ==
                   origBlock->getNextBlock() &&
        ...
        if ((branchNode->getOpCode().isJumpWithMultipleTargets() &&
             branchNode->getOpCode().hasBranchChildren()) ||
            branchNode->getOpCode().isGoto()) {
          _cfg->addEdge(TR::CFGEdge::createEdge(newBlock, dest, trMemory()));
          if (trace())
      ...
      // add C1->B2
               ...
               checkForSuccessor(dest,
                                 loopHeader))  // && region->isExitEdge(e))
      {
  ...
  if (trace())
    traceMsg(comp(),
             "removing edges and adding predecessor edges if required\n");
  EdgeEntry *ee = NULL;
  for (ee = (lInfo->_removedEdges).getFirst(); ee; ee = ee->getNext()) {
    TR::CFGEdge *e = ee->_edge;
    if (trace()) traceMsg(comp(), "processing edge \n");
    if (ee->_removeOnly) {
      if (trace())
    ...
    if (source->getNextBlock() == dest) {
      ...
      if (branchNode->getOpCode().isSwitch()) {
        ...
        if ((branchNode->getOpCode().isGoto() ||
            ...
            (branchNode->getBranchDestination()
                 ->getNode()
                 ->getBlock()
                 ->getNumber() == dest->getNumber())) {
          // B2 is a goto block
          ...
          _cfg->addEdge(
              TR::CFGEdge::createEdge(gotoBlock, newDest, trMemory()));
          if (trace())
            traceMsg(comp(), "   added goto block_%d->%d->%d\n",
  ...
  if (trace()) traceMsg(comp(), "actually removing above edges\n");
  for (ee = (lInfo->_removedEdges).getFirst(); ee; ee = ee->getNext()) {
    TR::CFGEdge *e = ee->_edge;
    _cfg->removeEdge(e);
  }
...
bool TR_LoopReplicator::checkForSuccessor(TR::Block *dest,
                                          TR::Block *loopHeader) {
  ...
  if (dest->getNumber() >= _nodesInCFG) {
    ...
    for (auto e = dest->getSuccessors().begin();
         e != dest->getSuccessors().end(); ++e)
      if ((*e)->getTo()->getNumber() == cH->getNumber()) return false;
  }
  return true;
}

TR::Block *TR_LoopReplicator::createEmptyGoto(TR::Block *source,
                                              TR::Block *dest, bool redirect) {
  ...
  TR::Block *gotoBlock =
      TR::Block::createEmptyBlock(destEntry->getNode(), comp(), freq, source);
  if (trace())
...
TR_LoopReplicator::LoopInfo *TR_LoopReplicator::findLoopInfo(
    int32_t regionNumber) {
  LoopInfo *lInfo = NULL;
  for (lInfo = _loopInfo.getFirst(); lInfo; lInfo = lInfo->getNext())
    if (lInfo->_regionNumber == regionNumber) break;
  return lInfo;
}

// find element in the specified list
TR_LoopReplicator::BlockEntry *TR_LoopReplicator::searchList(TR::Block *block,
                                                             listType lT,
                                                             LoopInfo *lInfo) {
...
void TR_LoopReplicator::modifyLoops() {
  for (LoopInfo *lInfo = _loopInfo.getFirst(); lInfo;
       lInfo = lInfo->getNext()) {
    // clone blocks
    if (lInfo->_replicated &&
...
void TR_LoopReplicator::fixUpLoopEntry(LoopInfo *lInfo, TR::Block *loopHeader) {
  ...
  TR_ScratchList<TR::CFGEdge> backEdges(trMemory());
  TR::CFGEdge *e = NULL;
  for (auto e = loopHeader->getPredecessors().begin();
       e != loopHeader->getPredecessors().end(); ++e) {
    TR::Block *predBlock = toBlock((*e)->getFrom());
    if (!region->contains(predBlock->getStructureOf(), region->getParent()))
      continue;
    // backedge found
  ...
  for (auto e = loopHeader->getExceptionSuccessors().begin();
       e != loopHeader->getExceptionSuccessors().end(); ++e) {
  ...
  if (trace())
    traceMsg(comp(), "adding successors for new loop header %d\n",
             clonedHeader->getNumber());
  for (auto e = loopHeader->getSuccessors().begin();
       e != loopHeader->getSuccessors().end(); ++e) {
    EdgeEntry *eE = new (trStackMemory()) EdgeEntry;
    eE->_edge = *e;
    eE->_removeOnly = true;
    (lInfo->_removedEdges).add(eE);
  ...
  for (e = bIt.getFirst(); e; e = bIt.getNext()) {
    ...
    if (searchList(predBlock, common, lInfo)) {
      ...
      if (predBlock == loopHeader) {
        _cfg->addEdge(
            TR::CFGEdge::createEdge(clonedHeader, clonedHeader, trMemory()));
        if (trace())
          traceMsg(comp(), "   added edge %d -> %d\n", predBlock->getNumber(),
                   clonedHeader->getNumber());
        TR::Node *branchNode =
            clonedHeader->getExit()->getPrevRealTreeTop()->getNode();
        if (branchNode->getOpCode().isBranch() ||
            branchNode->getOpCode().isGoto()) {
      ...
      TR::Node *branchNode =
          predBlock->getExit()->getPrevRealTreeTop()->getNode();
      if (branchNode->getOpCode().isSwitch()) {
        ...
        for (int32_t i = branchNode->getCaseIndexUpperBound() - 1; i > 0; i--) {
          ...
          if (target == loopHeader) {
            branchNode->getChild(i)->setBranchDestination(
                clonedHeader->getEntry());
            if (trace())
              traceMsg(comp(), "   fixed switch child %d -> %d\n", i,
                       clonedHeader->getNumber());
            if (!edgeFixed) {
              ...
              _cfg->addEdge(
                  TR::CFGEdge::createEdge(predBlock, clonedHeader, trMemory()));
              edgeFixed = true;
            }
          }
        }
        ...
        if (branchNode->getBranchDestination()->getNode()->getBlock() ==
          ...
          _cfg->addNode(gotoBlock);
          _cfg->addEdge(
              TR::CFGEdge::createEdge(predBlock, gotoBlock, trMemory()));
          _cfg->addEdge(
              TR::CFGEdge::createEdge(gotoBlock, clonedHeader, trMemory()));
          if (trace())
            traceMsg(comp(), "   added goto-block_%d->%d->%d\n",
        ...
        // block simply falls-through into loopheader; add a goto to the cloned
        // header
        TR::Node *gotoNode =
            TR::Node::create(branchNode, TR::Goto, 0, clonedHeader->getEntry());
        TR::TreeTop *gotoTT = TR::TreeTop::create(comp(), gotoNode);
  ...
  //
  TR::TreeTop *asyncTT = TR::TreeTop::create(
      comp(), TR::Node::createWithSymRef(
                  bbstartNode, TR::asynccheck, 0,
...
TR_StructureSubGraphNode *TR_LoopReplicator::findNodeInHierarchy(
...
void TR_LoopReplicator::calculateBlockWeights(TR_RegionStructure *region) {
  ...
  do {
    ...
    if (predecessorsNotYetVisited(region, node)) {
      dumpOptDetails(comp(),
                     "predecessors not yet visited for block: %d, requeue\n",
                     node->getNumber());
      blockq.enqueue(node);
      continue;
    }
    ...
    if ((node->getStructure()->getParent()->asRegion() == region) &&
      ...
      if ((bFreq > 0) || (node == region->getEntry())) {
        ...
        _blockWeights[node->getNumber()] =
            deriveFrequencyFromPreds(node, region);
        // pbr
...
int32_t TR_LoopReplicator::deriveFrequencyFromPreds(
    TR_StructureSubGraphNode *curNode, TR_RegionStructure *region) {
  TR::Block *curBlock = curNode->getStructure()->asBlock()->getBlock();
  TR_ScratchList<TR::Block> blocks(trMemory());
  for (auto e = curNode->getPredecessors().begin();
       e != curNode->getPredecessors().end(); ++e) {
    ...
    if (from->getStructure()->getParent()->asRegion() == region) {
      if (from->getStructure()->asRegion() &&
        ...
        for (TR::CFGEdge *e = eIt.getFirst(); e; e = eIt.getNext()) {
          TR_Structure *dest =
              toStructureSubGraphNode(e->getTo())->getStructure();
          if (region->contains(dest, region->getParent()))
  ...
  for (TR::Block *b = bIt.getFirst(); b; b = bIt.getNext()) {
    ...
    if (!(b->getSuccessors().size() == 1)) {
      bool scale = true;
      int32_t numSuccs = 0;
      for (auto e = b->getSuccessors().begin(); e != b->getSuccessors().end();
           ++e) {
        numSuccs++;
        ...
        if (d->isCold() || (d->getFrequency() > 0)) scale = false;
...
bool TR_LoopReplicator::predecessorsNotYetVisited(
    TR_RegionStructure *r, TR_StructureSubGraphNode *curNode) {
  if (r->getEntry() == curNode) return false;
  for (auto e = curNode->getPredecessors().begin();
       e != curNode->getPredecessors().end(); ++e) {
    TR_StructureSubGraphNode *from = toStructureSubGraphNode((*e)->getFrom());
...
int32_t TR_LoopReplicator::getSeedFreq(TR_RegionStructure *r) {
  ...
  // if (seedFreq > 0)
  if (seedFreq) return seedFreq;

  TR_ScratchList<TR::Block> blocks(trMemory());
  for (auto e = seed->getPredecessors().begin();
       e != seed->getPredecessors().end(); ++e) {
    TR::Block *source = toBlock((*e)->getFrom());
    if (!r->contains(source->getStructureOf(), r->getParent())) continue;
    blocks.add(source);
  }
...
int32_t TR_LoopReplicator::getBlockFreq(TR::Block *X) {
  ...
  static char *pEnv = feGetEnv("TR_NewLRTracer");
  if (pEnv) return _blockWeights[X->getNumber()];
  // if (freq >= 0)
  // if (freq)
  if ((freq != (MAX_COLD_BLOCK_COUNT + 1)) && (freq != 0)) return freq;

  if (X == _curLoopInfo->_region->getEntryBlock())
    return _curLoopInfo->_seedFreq;

  TR_ScratchList<TR::Block> blocks(trMemory());
  for (auto e = X->getPredecessors().begin(); e != X->getPredecessors().end();
       ++e)
...
int32_t TR_LoopReplicator::getScaledFreq(TR_ScratchList<TR::Block> &blocks,
                                         TR::Block *X) {
  int32_t freq = 0;
...
bool TR_LoopReplicator::heuristics(LoopInfo *lInfo, bool dumb) {
  ...
  if (trace()) traceMsg(comp(), "analyzing region - %p\n", region);
  ...
  for (b = bilIt.getFirst(); b; b = bilIt.getNext()) {
    if (trace()) traceMsg(comp(), "   current cand - %d ", b->getNumber());
    if (!searchList(b, common, lInfo)) {
      if (trace()) traceMsg(comp(), "\n");
      TR::Block *cand = b;
      TR::CFGEdge *edge = NULL;
      nextSuccessor(region, &cand, &edge);
      if (cand != b) {
        if (trace()) traceMsg(comp(), "   inner loop found bypassing\n");
        b = cand;
      }
      if ((b->getNumber() % 2) == 0) {
        if (!searchList(b, common, lInfo)) {
      ...
      if (trace()) traceMsg(comp(), "is already visited\n");
    }
...
const char *TR_LoopReplicator::optDetailString() const throw() {
