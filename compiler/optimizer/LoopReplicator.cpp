...
bool TR_LoopReplicator::shouldReplicateWithHotInnerLoops(
  ...
  for (;;) {
    ...
    for (auto e = tracePrefixCursor->getSuccessors().begin();
      ...
      if (tracePrefixNext != NULL) {
        ...
        countReplicationFailure("HotInnerLoopHitBranchWithoutColdSideEntry",
                                region->getNumber());
        if (trace())
          traceMsg(comp(), "Hit a branch without finding a cold side-entry. "
                           "Will not replicate.\n");
        return false;
      }
    ...
    if (tracePrefixNext == NULL) {
      ...
      if (trace())
        traceMsg(comp(), "Ran out of trace without finding a cold side-entry. "
                         "Will not replicate.\n");
      return false;
    }
    ...
    if (hotInnerLoopHeaders->find(tracePrefixCursor)) {
      countReplicationFailure("HotInnerLoopNoColdSideEntry",
                              region->getNumber());
      if (trace())
        traceMsg(comp(), "Hit a hot inner loop without finding a cold "
                         "side-entry. Will not replicate.\n");
      return false;
    }
  }
}
...
bool TR_LoopReplicator::heuristics(LoopInfo *lInfo) {
  ...
  if (!lInfo->_replicated)
    dumpOptDetails(comp(), "no side entrance found into trace; no replication "
                           "will be performed\n");
...
TR_LoopReplicator::findNodeInHierarchy(TR_RegionStructure *region,
                                       int32_t num) {
  if (!region)
    return NULL;
...
bool TR_LoopReplicator::heuristics(LoopInfo *lInfo, bool dumb) {
  ...
  if (trace()) {
    ...
    traceMsg(comp(), "\n");
    if (!lInfo->_replicated)
      traceMsg(comp(), "no side entrance found into trace; no replication will "
                       "be performed\n");
  }
