...
private:
  void replaceTargetNodeBySourceTree()
  {
    ...
    TR_ASSERT(_parentOfTargetNode != 0,
      "could not find _targetNode " POINTER_PRINTF_FORMAT " parent, _targetTreeTop root node " POINTER_PRINTF_FORMAT
      "!",
  ...
  // The difference between this function and replaceTargetNodeBySourceTree is that this function does not set any class
  // fields.
  ...
  void fixupBCDPrecisionIfRequired()
  {
    ...
    if (oldNode->getType().isBCD() && newNode->getType().isBCD()) {
      ...
      if (newNode->getNumChildren() == 2) {
        ...
        // below pass then can reduce the newPrecision to 30 so a pdModPrec correction is not needed (as the propagated
        // symbol will not have the top nibble 30->31 set).
        if (newNode->getType().isAnyPacked()) {
            ...
            oldSize == newSymSize) // 16 == 16
          {
            if (trace() || comp()->cg()->traceBCDCodeGen()) {
              traceMsg(comp(),
                "reduce newPrecision %d->%d for odd to even truncation (origNode %s (%p) prec=%d, node %s (%p) "
                "prec=%d\n",
                newPrecision, oldPrecision, newNode->getOpCode().getName(), newNode, newNode->getDecimalPrecision(),
                oldNode->getOpCode().getName(), oldNode, oldPrecision);
            }

            newPrecision = oldPrecision; // 31->30 -- no correction needed for same byte odd to even truncation if the
                                         // origNode was only storing out the even $ of digits
          }
        }
      }
      ...
      if (needsClean || needsPrecisionCorrection) {
        dumpOptDetails(comp(), "node %p precision %d != propagated symRef #%d precision %d and/or needsClean (%s)\n",
          oldNode, oldPrecision, newNode->getSymbolReference()->getReferenceNumber(), newPrecision,
          needsClean ? "yes" : "no");

        if (needsPrecisionCorrection) {
          TR::ILOpCodes modPrecOp
            = TR::ILOpCode::modifyPrecisionOpCode(_parentOfTargetNode->getChild(_targetChildIndex)->getDataType());
  ...
  void propagateThroughToCommoningPoints()
  {
    ...
    if (oldNode->getReferenceCount() > 0) {
      ...
      // commoned the old node. We want to replace those commonings with a commoning to the new RHS of the node because
      // the value is the same.

      // Get a list of load sym refs for the new rematerialized RHS - use this to see if any part of the new node is
      // killed during tree traversal.
      ...
      // Start at _targetTreeTop to replace other instances of _targetNode in _targetTreeTop as well as to the end of
      // the block.
      ...
      while (cur_tt && (cur_tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
        ...
        while ((cur_tt->getNode()->getOpCodeValue() == TR::BBEnd)
          && (cur_tt->getNextTreeTop() && (cur_tt->getNextTreeTop()->getNode()->getOpCodeValue() == TR::BBStart)
...

  {
      ...
      if (storeLookup != storeMap.end()) {
        for (auto itr = storeLookup->second->begin(), end = storeLookup->second->end(); itr != end; ++itr) {
          ...
          if (prevDefNode->getType().isBCD()) {
            ...
            if (defNode->getType().isBCD() && defNode->getDataType() == prevDefNode->getDataType()
              ...
              && (defNode->getDataType() != TR::PackedDecimal
                || prevDefNode->mustCleanSignInPDStoreEvaluator() == defNode->mustCleanSignInPDStoreEvaluator())) {
              dumpOptDetails(comp(), "%s   Def nodes %p (precision = %d) and %p (precision = %d) are equivalent\n",
                OPT_DETAILS, defNode, defNode->getDecimalPrecision(), prevDefNode, prevDefNode->getDecimalPrecision());
...
#endif
          {
            ...
            auto equivalentDefLookup = equivalentDefs.find(j);
            if ((!thisSymRefIsPreferred && (prevSymRefIsPreferred || (equivalentDefLookup != equivalentDefs.end())))) {
              if (trace())
                traceMsg(comp(), "000setting i %d to j %d\n", i, j);
    ...
    for (auto itr = equivalentDefs.begin(), end = equivalentDefs.end(); itr != end; ++itr) {
      ...
      for (useCursor.SetToFirstOne(); useCursor.Valid(); useCursor.SetToNextOne()) {
        ...
        if (performTransformation(comp(), "%s   Copy1 Propagation replacing Copy symRef #%d %p %p\n", OPT_DETAILS,
              copySymbolReference->getReferenceNumber(), defNode, equivalentDefNode)) {
    ...
    for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
      ...
      if (useDefInfo->getUseDef(defs, i)) {
        ...
        int32_t regNumber;
        TR::Node* defNode
          = areAllDefsInCorrectForm(useNode, defs, useDefInfo, firstRealDefIndex, loadNode, regNumber, baseNode);
        if (!defNode)
          continue;
        ...
        if (performTransformation(comp(), "%s   Copy2 Propagation replacing Copy symRef #%d \n", OPT_DETAILS,
          ...
          if (!isRegLoad && (loadNode->getOpCodeValue() != TR::loadaddr) && // loadaddr doesn't need to be anchored
            (loadNode->getOpCode().hasSymbolReference()) && (loadNode->getSymbolReference() == copySymbolReference)
            && (rhsOfStoreDefNode
              != loadNode)) // Modifications to this condition should be mirrored in the NOTEMPS check above.
          {
            TR::TreeTop* anchorTree = findAnchorTree(defNode, loadNode);

            // Need to create a temp for case like i = i + 1
            TR::SymbolReference* newSymbolReference = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(),
              copySymbolReference->getSymbol()->getDataType(), false, copySymbolReference->getSymbol()->getSize());

            dumpOptDetails(
              comp(), "%s      created new temp #%d\n", OPT_DETAILS, newSymbolReference->getReferenceNumber());

            TR::Node* store
              = TR::Node::createStore(newSymbolReference, TR::Node::createLoad(defNode, copySymbolReference));
          ...
          if (_propagatingWholeExpression) {
            ...
            dumpOptDetails(
              comp(), "%s   By Original expression in %p Use node : %p\n", OPT_DETAILS, rhsOfStoreDefNode, useNode);
          } else {
          ...
          if (_propagatingWholeExpression) {
            ...
            TR::Node* rematExpression
              = rhsOfStoreDefNode->getOpCode().isStore() ? rhsOfStoreDefNode->getValueChild() : rhsOfStoreDefNode;
...
void TR_CopyPropagation::rematerializeIndirectLoadsFromAutos()
{
  ...
  while (currentTree) {
    ...
    if (node->getOpCode().isStoreIndirect() && (node->getType().isIntegral() || node->getType().isAddress())
      && node->getFirstChild()->getOpCodeValue() == TR::loadaddr && node->getFirstChild()->getSymbol()->isAutoOrParm()
      && nextNode->getOpCode().isStoreDirect() && nextNode->getSymbol()->isAutoOrParm()
...
#endif
      {
        ...
        if (origNode->getOpCode().isLoadIndirect()) {
          ...
          if (origNode->getNumChildren() == 2) {
            ...
            // for example for a node with prec=30 and an origSymPrecision with prec=31 then if the symbol sizes match
            // and the other checks below pass then can reduce the newPrecision to 30 so a pdModPrec correction is not
            // needed (as the propagated symbol will not have the top nibble 30->31 set).
            ...
            if (node->getType().isAnyPacked()) {
                ...
                nodeSize == origSymSize) // 16 == 16
              {
                ...
                newPrecision = nodePrecision; // 31->30 -- no correction needed for same byte odd to even truncation if
                                              // the origNode was only storing out the even $ of digits
              }
            }
          }

          // if newPrecision < currentPrecision then one or more digits will be lost during the copyPropagation -- the
          // isCorrect* routines disallow this
          ...
          //
          if (needsClean || needsPrecisionCorrection) {
            // If the copy propagation is going to change the size of the node being replaced then a modifyPrecision
            // node must be prepended in case the new parent requires explicit widening (such as if the parent is a
            // call). Also if the defNode was going to clean must also insert a pdclean on top of the node with the
            // propagated symRef
            ...
            dumpOptDetails(comp(),
              "node %p precision %d != propagated symRef #%d precision %d and/or needsClean (%s) so create nodeCopy %p "
              "(isTwoChildrenCase=%s)\n",
            ...
            if (needsPrecisionCorrection) {
              ...
              if (needsClean) // i.e. if a clean will be generated below then do not modify node op yet, let the top
                              // level op (the coming clean) do this
              {
            ...
            dumpOptDetails(comp(),
              "attach new first %s (%p) and second %s (%p) to node %s (%p) in the twoChildren=true case\n",
              firstChild->getOpCode().getName(), firstChild, secondChild->getOpCode().getName(), secondChild,
...
bool TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node* useNode, TR::Node* storeNode, TR::Node* loadNode)
{
  ...
  if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
    dumpOptDetails(comp(),
      "isLoadNodeSuitableForPropagation : useNode %p (prec %d), loadNode %p (prec %d) -- isCorrect=%s (only correct "
      "when useNodePrec <= loadNodePrec)\n",
  ...
  TR_UseDefInfo* useDefInfo, int32_t firstRealDefIndex, TR::Node*& loadNode, int32_t& regNumber, TR::Node*& baseNode)
{
  ...
  //...exception point that could reach a catch
  // catch block
  // load b  //  copy propagating "a" here is incorrect since we will end up propagating rhs2 value (in "a") instead of
  // rhs1 value (in "b") changing program semantics
  //
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if ((loadNode = isCheapRematerializationCandidate(defNode, rhsOfStoreDefNode))
      || (loadNode = isLoadVarWithConst(rhsOfStoreDefNode)) || (loadNode = isIndirectLoadFromAuto(rhsOfStoreDefNode))
  ...
  if (node->getOpCode().isLoadIndirect()) {
    ...
    if (!cheapExpression && addr->getOpCode().isAdd()) {
      if ((addr->getFirstChild()->getOpCodeValue() == TR::loadaddr)
        && addr->getFirstChild()->getSymbol()->isAutoOrParm() && addr->getSecondChild()->getOpCode().isLoadConst()) {
        cheapExpression = true;
      }
    }
  }
...
bool TR_CopyPropagation::isNodeAvailableInBlock(TR::TreeTop* useTree, TR::Node* loadNode)
{
  ...
  while (treeTop->getNode()->getOpCode().getOpCodeValue() != TR::BBStart
    ...
    if (loadNode->getOpCode().hasSymbolReference() && node->mayKill().contains(loadNode->getSymbolReference(), comp()))
      return false;
