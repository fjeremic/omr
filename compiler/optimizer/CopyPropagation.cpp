...
void collectNodesForIsCorrectChecks(TR::Node *n,
                                    TR::list<TR::Node *> &checkNodes,
                                    TR::SparseBitVector &refsToCheckIfKilled,
                                    vcount_t vc);
...
TR_CopyPropagation::TR_CopyPropagation(TR::OptimizationManager *manager)
    : TR::Optimization(manager),
      ...
      _useTreeTops(
          (UseTreeMapComparator()),
          UseTreeMapAllocator(comp()->trMemory()->currentStackRegion())) {}

template <class T>
TR::Node *getNodeParent(TR::Node *lookForNode, TR::Node *curNode,
                        vcount_t visitCount, T &ci) {
  if (lookForNode == curNode) return 0;  // problematic

  vcount_t oldVisitCount = curNode->getVisitCount();

  if (oldVisitCount == visitCount) return 0;
  ...
  for (size_t i = 0; i < curNode->getNumChildren(); i++) {
    if (curNode->getChild(i) == lookForNode) {
      ...
      TR::Node *ret =
          getNodeParent(lookForNode, curNode->getChild(i), visitCount, ci);
...
bool foundInterferenceBetweenCurrentNodeAndPropagation(
    TR::Compilation *comp, bool trace, TR::Node *currentNode,
    TR::Node *storeNode, TR::list<TR::Node *> &checkNodes,
    TR::SparseBitVector &refsToCheckIfKilled) {
  bool result = false;
  vcount_t save_vc = comp->getVisitCount();
  TR::Node *valueNode =
      storeNode->getOpCode().isStore() ? storeNode->getValueChild() : storeNode;
  ...
  if (trace) {
    comp->getDebug()->trace(
        "foundInterferenceBetweenCurrentNodeAndPropagation: currentNode %p "
        "storeNode %p checkNodes [",
        currentNode, storeNode);
    for (auto checkNodesCursor = checkNodes.begin();
         checkNodesCursor != checkNodes.end(); ++checkNodesCursor) {
...
class TR_ExpressionPropagation {
 ...
 public:
  ...
  void setTargetNode(TR::Node *tn) {
    ...
    TR_ASSERT(_targetTreeTop->getNode()->containsNode(
                  _targetNode, comp()->incOrResetVisitCount()),
  ...
  void perform() {
    ...
    TR_ASSERT(_originalDefNodeForSourceTree != 0,
              "_originalDefNodeForSourceTree is 0!");
 ...
 private:
  void replaceTargetNodeBySourceTree() {
    _parentOfTargetNode =
        getNodeParent(_targetNode, _targetTreeTop->getNode(),
                      comp()->incOrResetVisitCount(), _targetChildIndex);
  ...
  // The difference between this function and replaceTargetNodeBySourceTree is
  // that this function does not set any class fields.
  void replaceAllInstancesOfTargetNodeInTreeBySourceTree(
      TR::TreeTop *targetTreeTop, TR::Node *targetNode, TR::Node *sourceTree) {
    while (targetTreeTop->getNode()->containsNode(
        targetNode, comp()->incOrResetVisitCount())) {
      size_t childIndex;
      TR::Node *parentNode =
          getNodeParent(targetNode, targetTreeTop->getNode(),
                        comp()->incOrResetVisitCount(), childIndex);
  ...
  void fixupBCDPrecisionIfRequired() {
    ...
    TR::Node *newNode =
        _parentOfTargetNode->getChild(_targetChildIndex);  // propagated
    ...
    if (oldNode->getType().isBCD() && newNode->getType().isBCD()) {
      ...
      if (newNode->getNumChildren() == 2) {
        ...
        int32_t newSymSize =
            newNode->getOpCode().hasSymbolReference()
                ...
                : newNode->getSize();
        newPrecision =
            TR::DataType::getBCDPrecisionFromSize(newDataType, newSymSize);

        // for a oldNode with prec=30 and an newNode with prec=31 then if the
        // symbol sizes match and the other checks below pass then can reduce
        // the newPrecision to 30 so a pdModPrec correction is not needed (as
        ...
        if (newNode->getType().isAnyPacked()) {
              ...
              oldSize == newSymSize)                             // 16 == 16
          {
            if (trace() || comp()->cg()->traceBCDCodeGen()) {
              traceMsg(comp(),
                       "reduce newPrecision %d->%d for odd to even truncation "
                       "(origNode %s (%p) prec=%d, node %s (%p) prec=%d\n",
                       newPrecision, oldPrecision,
                       newNode->getOpCode().getName(), newNode,
                       newNode->getDecimalPrecision(),
                       oldNode->getOpCode().getName(), oldNode, oldPrecision);
            }

            newPrecision =
                oldPrecision;  // 31->30 -- no correction needed for same byte
                               // odd to even truncation if the origNode was
                               // only storing out the even $ of digits
          }
        }
      }
      ...
      TR_ASSERT(
          newPrecision >= oldPrecision,
          "newPrecision < oldPrecision (%d < %d) during copyPropagation\n",
          newPrecision, oldPrecision);

      bool needsClean = _originalDefNodeForSourceTree &&
                        _originalDefNodeForSourceTree->mustClean();
      bool needsPrecisionCorrection = oldPrecision != newPrecision;

      if (needsClean || needsPrecisionCorrection) {
        dumpOptDetails(comp(),
                       "node %p precision %d != propagated symRef #%d "
                       "precision %d and/or needsClean (%s)\n",
                       oldNode, oldPrecision,
                       newNode->getSymbolReference()->getReferenceNumber(),
                       newPrecision, needsClean ? "yes" : "no");
        ...
        if (needsPrecisionCorrection) {
          ...
          TR_ASSERT(
              modPrecOp != TR::BadILOp,
              "no bcd modify precision opcode found for node %p type %s\n",
              _parentOfTargetNode->getChild(_targetChildIndex),
              _parentOfTargetNode->getChild(_targetChildIndex)
                  ->getDataType()
                  .toString());

          TR::Node *pdModPrecNode = TR::Node::create(
              modPrecOp, 1, _parentOfTargetNode->getChild(_targetChildIndex));
          pdModPrecNode->setDecimalPrecision(
              oldPrecision);  // set to existing node's precision

          _parentOfTargetNode->setAndIncChild(_targetChildIndex, pdModPrecNode);
        }

        if (needsClean) {
          TR::ILOpCodes cleanOp = TR::ILOpCode::cleanOpCode(
              _parentOfTargetNode->getChild(_targetChildIndex)->getDataType());

          TR_ASSERT(cleanOp != TR::BadILOp,
                    "no bcd clean opcode found for node %p type %s\n",
                    _parentOfTargetNode->getChild(_targetChildIndex),
                    _parentOfTargetNode->getChild(_targetChildIndex)
                        ->getDataType()
                        .toString());

          TR::Node *pdCleanNode = TR::Node::create(
              cleanOp, 1, _parentOfTargetNode->getChild(_targetChildIndex));
  ...
  void addConversionIfRequired() {
    ...
    if (newNode->getType().getDataType() != oldNode->getType().getDataType()) {
      ...
      TR::ILOpCodes convOp = TR::ILOpCode::getProperConversion(
          newNode->getDataType(), oldNode->getDataType(), true);

      TR_ASSERT(convOp != TR::BadILOp,
                "no conversion opcode found for old type %s to new type %s!\n",
                oldNode->getDataType().toString(),
                newNode->getDataType().toString());

      TR::Node *convNode = TR::Node::create(
          convOp, 1, _parentOfTargetNode->getChild(_targetChildIndex));
  ...
  void propagateThroughToCommoningPoints() {
    ...
    // 1) Propagate this RHS change to anything with a commoned old node further
    // down the block.
    //    Common the rematerialization instead - it should have the same value
    //    at that evaluation.
    // 2) Make sure evaluation point did not change when old node was replaced.

    // If there are more commoning sites for the old node, then take care of
    // them.
    if (oldNode->getReferenceCount() > 0) {
      // Step 1: Propagate this RHS change to anything with a commoned old node
      // further down the block. Common the rematerialization instead.

      // The idea is that we have replaced the RHS of some node, but other nodes
      // further down the block could have commoned the old node. We want to
      // replace those commonings with a commoning to the new RHS of the node
      // because the value is the same.

      // Get a list of load sym refs for the new rematerialized RHS - use this
      // to see if any part of the new node is killed during tree traversal.
      TR::list<TR::Node *> newCheckNodes(
          getTypedAllocator<TR::Node *>(comp()->allocator()));
      TR::SparseBitVector newRefsToCheckIfKilled(comp()->allocator());

      collectNodesForIsCorrectChecks(newNode, newCheckNodes,
                                     newRefsToCheckIfKilled,
                                     comp()->incOrResetVisitCount());

      // Start at _targetTreeTop to replace other instances of _targetNode in
      // _targetTreeTop as well as to the end of the block.
      ...
      while (cur_tt && (cur_tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
        ...
        replaceAllInstancesOfTargetNodeInTreeBySourceTree(cur_tt, oldNode,
                                                          newNode);
        ...
        while ((cur_tt->getNode()->getOpCodeValue() == TR::BBEnd) &&
               (cur_tt->getNextTreeTop() &&
                (cur_tt->getNextTreeTop()->getNode()->getOpCodeValue() ==
                 TR::BBStart) &&
                (cur_tt->getNextTreeTop()
                     ->getNode()
                     ->getBlock()
...
int32_t TR_CopyPropagation::perform() {
  if (trace()) traceMsg(comp(), "Starting CopyPropagation\n");
...

  {
    ...
    typedef TR::typed_allocator<std::pair<int32_t const, int32_t>, TR::Region &>
        UDMapAllocator;
    typedef std::less<int32_t> UDMapComparator;
    typedef std::map<int32_t, int32_t, UDMapComparator, UDMapAllocator>
        UsesToBeFixedMap;
    typedef std::map<int32_t, int32_t, UDMapComparator, UDMapAllocator>
        EquivalentDefMap;

    UsesToBeFixedMap usesToBeFixed(
        (UDMapComparator()), UDMapAllocator(trMemory()->currentStackRegion()));
    EquivalentDefMap equivalentDefs(
        (UDMapComparator()), UDMapAllocator(trMemory()->currentStackRegion()));

    typedef TR::typed_allocator<
        std::pair<TR::Node *const, TR::deque<TR::Node *, TR::Region &> *>,
        TR::Region &>
        StoreNodeMapAllocator;
    typedef std::less<TR::Node *> StoreNodeMapComparator;
    typedef std::map<TR::Node *, TR::deque<TR::Node *, TR::Region &> *,
                     StoreNodeMapComparator, StoreNodeMapAllocator>
        StoreNodeMap;

    StoreNodeMap storeMap(
        (StoreNodeMapComparator()),
        StoreNodeMapAllocator(trMemory()->currentStackRegion()));

    TR::NodeChecklist checklist(comp());

    _lookForOriginalDefs = false;
    for (TR::TreeTop *tree = comp()->getStartTree(); tree;
         tree = tree->getNextTreeTop()) {
      TR::Node *defNode = tree->getNode();
      if (defNode->getOpCodeValue() == TR::BBStart &&
          !defNode->getBlock()->isExtensionOfPreviousBlock()) {
        storeMap.clear();
        continue;
      }
      ...
      if (storeLookup != storeMap.end()) {
        for (auto itr = storeLookup->second->begin(),
                  end = storeLookup->second->end();
             itr != end; ++itr) {
          ...
          if (thisDefSlot < 0)  // pending pushes
            thisSymRefIsPreferred = true;
                   ...
                   comp()
                       ->getOwningMethodSymbol(defNode->getSymbolReference()
                                                   ->getOwningMethodIndex())
          ...
          if (prevDefSlot < 0)  // pending pushes
            prevSymRefIsPreferred = true;
                   ...
                   comp()
                       ->getOwningMethodSymbol(prevDefNode->getSymbolReference()
                                                   ->getOwningMethodIndex())
                       ->getFirstJitTempIndex())  // autos and parms
            prevSymRefIsPreferred = true;
          ...
          if (prevDefNode->getType().isBCD()) {
            TR_ASSERT(defNode->getType().isBCD(),
                      "expecting defNode to be a BCD type (dt=%s) when "
                      "prevDefNode is a BCD type (dt=%s)\n",
                      defNode->getDataType().toString(),
                      prevDefNode->getDataType().toString());
            if (defNode->getType().isBCD() &&
              ...
              dumpOptDetails(comp(),
                             "%s   Def nodes %p (precision = %d) and %p "
                             "(precision = %d) are equivalent\n",
                             OPT_DETAILS, defNode,
                             defNode->getDecimalPrecision(), prevDefNode,
                             prevDefNode->getDecimalPrecision());

              auto equivalentDefLookup = equivalentDefs.find(j);
              if ((!thisSymRefIsPreferred &&
                   (prevSymRefIsPreferred ||
                    (equivalentDefLookup != equivalentDefs.end())))) {
                if (trace())
                  traceMsg(comp(), "000setting i %d to j %d\n", i, j);
                if (equivalentDefLookup == equivalentDefs.end()) {
                  if (i != j) equivalentDefs[i] = j;
...
#endif
          {
            ...
            if ((!thisSymRefIsPreferred &&
                 (prevSymRefIsPreferred ||
                  (equivalentDefLookup != equivalentDefs.end())))) {
              if (trace()) traceMsg(comp(), "000setting i %d to j %d\n", i, j);
              ...
              if (trace()) traceMsg(comp(), "111setting j %d to i %d\n", j, i);
              if (i != j) equivalentDefs[j] = i;
            }
          }
          break;
      ...
      if (storeLookup == storeMap.end()) {
        storeMap[rhsOfStoreDefNode] =
            new (trStackMemory()) TR::deque<TR::Node *, TR::Region &>(
    ...
    for (auto itr = equivalentDefs.begin(), end = equivalentDefs.end();
         itr != end; ++itr) {
      int32_t defIndex = itr->first;
      TR::Node *defNode = useDefInfo->getNode(defIndex);
      TR::TreeTop *defTree = useDefInfo->getTreeTop(defIndex);
      TR::TreeTop *extendedBlockEntry =
          defTree->getEnclosingBlock()->startOfExtendedBlock()->getEntry();
      TR::TreeTop *extendedBlockExit =
          extendedBlockEntry->getExtendedBlockExitTreeTop();
      ...
      useDefInfo->getUsesFromDef(uses, itr->first,
                                 useDefInfo->hasLoadsAsDefs());
      TR_UseDefInfo::BitVector::Cursor useCursor(uses);
      for (useCursor.SetToFirstOne(); useCursor.Valid();
           useCursor.SetToNextOne()) {
        int32_t i = useCursor + firstUseIndex;

        TR_UseDefInfo::BitVector defs(comp()->allocator());
        useDefInfo->getUseDef(defs, i);

        // could use "if (defs.PopulationCount() == 1) continue", but this is
        // more efficient
        ...
        if (!useNode || useNode->getReferenceCount() == 0) continue;

        // Now we know there is a single definition for this use node
        //
        TR::SymbolReference *copySymbolReference =
            defNode->getSymbolReference();
        if (performTransformation(
                comp(),
                "%s   Copy1 Propagation replacing Copy symRef #%d %p %p\n",
                OPT_DETAILS, copySymbolReference->getReferenceNumber(), defNode,
                equivalentDefNode)) {
          comp()->setOsrStateIsReliable(
              false);  // could check if the propagation was done to a child of
                       // the OSR helper call here
          ...
          if (defSeenFirst) {
            ...
            while (cursorTree != equivalentDefTree) {
              if (!cursorTree->getNode()->getOpCode().isStore() ||
                  (cursorTree->getNode()->getFirstChild() !=
          ...
          TR::SymbolReference *originalSymbolReference =
              equivalentDefNode->getSymbolReference();

          dumpOptDetails(
              comp(), "%s   By Original symRef #%d in Use node : %p\n",
              OPT_DETAILS, originalSymbolReference->getReferenceNumber(),
              useNode);
    ...
    for (auto itr = usesToBeFixed.begin(), end = usesToBeFixed.end();
         itr != end;) {
      ...
      if (useDefInfo->hasLoadsAsDefs()) {
        for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex;
             i++) {
          TR::Node *useNode = useDefInfo->getNode(i);
          if (!useNode || useNode->getReferenceCount() == 0) continue;
    ...
    if (_cleanupTemps) rematerializeIndirectLoadsFromAutos();
    ...
    for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
      ...
      if (useDefInfo->getUseDef(defs, i)) {
        ...
        int32_t regNumber;
        TR::Node *defNode = areAllDefsInCorrectForm(useNode, defs, useDefInfo,
                                                    firstRealDefIndex, loadNode,
                                                    regNumber, baseNode);
        if (!defNode) continue;
        ...
        TR::TreeTop *defTree =
            useDefInfo->getTreeTop(defNode->getUseDefIndex());
        if (defTree->getEnclosingBlock()->startOfExtendedBlock() ==
            _useTree->getEnclosingBlock()->startOfExtendedBlock()) {
          // PR62462: LocalCSE is a fine and trustworthy opt, and we're not
          // trying to do its job here. Our analysis (particularly of RegLoads)
          // assumes it is propagating copies into successor blocks, and doesn't
          // handle propagation within an extended block properly.
          //
          continue;
        }

        if (baseNode) {
          TR_ASSERT(baseNode->getOpCode().hasSymbolReference(),
                    "baseNode should have a symbol reference\n");
          if (!_cleanupTemps ||
        ...
        TR::Node *rhsOfStoreDefNode = defNode->getOpCode().isStoreIndirect()
                                          ? defNode->getSecondChild()
                                          : defNode->getFirstChild();
        TR::SymbolReference *copySymbolReference =
            defNode->getOpCode().hasSymbolReference()
                ? defNode->getSymbolReference()
                : NULL;
        ...
        if (performTransformation(
                comp(), "%s   Copy2 Propagation replacing Copy symRef #%d \n",
          ...
          for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            dumpOptDetails(comp(), "%s      Def #%d[%p]\n", OPT_DETAILS,
                           defIndexTrace, useDefInfo->getNode(defIndexTrace));
          }

          if (!isRegLoad &&
              (loadNode->getOpCodeValue() !=
               TR::loadaddr) &&  // loadaddr doesn't need to be anchored
              (loadNode->getOpCode().hasSymbolReference()) &&
              (loadNode->getSymbolReference() == copySymbolReference) &&
              (rhsOfStoreDefNode !=
               loadNode))  // Modifications to this condition should be mirrored
                           // in the NOTEMPS check above.
          {
            ...
            TR::SymbolReference *newSymbolReference =
                comp()->getSymRefTab()->createTemporary(
                    ...
                    copySymbolReference->getSymbol()->getDataType(), false,
          ...
          if (_propagatingWholeExpression) {
            dumpOptDetails(
                comp(),
                "%s   Rematerializing whole expression %p to replace %p\n",
                OPT_DETAILS, rhsOfStoreDefNode, useNode);
          } else if (!isRegLoad) {
            dumpOptDetails(comp(),
                           "%s   By Original expression in %p Use node : %p\n",
                           OPT_DETAILS, rhsOfStoreDefNode, useNode);
          } else {
            dumpOptDetails(comp(),
                           "%s   By register number %d in Use node : %p\n",
          ...
          if (_propagatingWholeExpression) {
            ...
            replaceCopySymbolReferenceByOriginalIn(
                copySymbolReference, rhsOfStoreDefNode, useNode, defNode,
                baseAddr, baseAddrAvail);
            usesToBeFixed[useNode->getUseDefIndex()] =
                loadNode->getUseDefIndex();
          } else {
            TR::Node *regLoadNode = NULL;
            TR::Block *useBlock = _useTree->getEnclosingBlock();
            TR::Node *bbstartNode =
                useBlock->startOfExtendedBlock()->getEntry()->getNode();
            if (bbstartNode->getNumChildren() > 0) {
              for (int i = 0;
                   i < bbstartNode->getFirstChild()->getNumChildren(); i++) {
              ...
              if (regLoadNode) {
                ...
                TR::TreeTop *exitTree =
                    cursorTree->getExtendedBlockExitTreeTop()->getNextTreeTop();
                while (cursorTree != exitTree) {
                  replaceCopySymbolReferenceByOriginalRegLoadIn(
                      regLoadNode, useNode, copySymbolReference,
                      cursorTree->getNode(), NULL, -1);
                  cursorTree = cursorTree->getNextTreeTop();
                }
              }
            }
    ...
    if (_cleanupTemps) commonIndirectLoadsFromAutos();
    ...
    if (invalidateDefUseInfo) useDefInfo->resetDefUseInfo();
...
void TR_CopyPropagation::collectUseTrees(TR::TreeTop *tree, TR::Node *node,
  ...
  TR_UseDefInfo *useDefInfo = optimizer()->getUseDefInfo();
  if (useIndex >= useDefInfo->getFirstUseIndex() &&
      useIndex <= useDefInfo->getLastUseIndex()) {
    auto useTreeLookup = _useTreeTops.find(node);
    if (useTreeLookup == _useTreeTops.end()) _useTreeTops[node] = tree;
  }
...
void TR_CopyPropagation::rematerializeIndirectLoadsFromAutos() {
  ...
  while (currentTree) {
    ...
    if (!currentTree->getNextTreeTop()) break;
    ...
    if (node->getOpCode().isStoreIndirect() &&
        ...
        performTransformation(
            comp(),
            "%s   Rematerializing indirect load from auto in node %p (temp "
            "#%d)\n",
            OPT_DETAILS, nextNode->getFirstChild(),
            nextNode->getSymbolReference()->getReferenceNumber())) {
      comp()->setOsrStateIsReliable(
          false);  // could check if the propagation was done to a child of the
                   // OSR helper call here
      TR::Node *newChild = TR::Node::create(
          node, comp()->il.opCodeForIndirectLoad(node->getDataType()), 1);
  ...
  if (trace())
    comp()->dumpMethodTrees(
        "Trees after rematerialization of indirect loads from autos");
}
...
void TR_CopyPropagation::commonIndirectLoadsFromAutos() {
  ...
  while (currentTree) {
    ...
    if (node->getOpCode().isStoreIndirect() &&
        ...
        performTransformation(
            comp(), "%s   Commoning indirect load from auto in node %p \n",
            OPT_DETAILS, nextNode->getFirstChild())) {
      comp()->setOsrStateIsReliable(
          false);  // could check if the propagation was done to a child of the
                   // OSR helper call here
  ...
  if (trace())
    comp()->dumpMethodTrees(
        "Trees after commoning of indirect loads from autos");
}

/*
   replaceCopySymbolReferenceByOriginalIn uses a slightly different visitCount
idiom -- The visit count for a node is set iff the node has NOT been changed.
This is done to make sure that we will replace ALL qualified nodes (see checks
below).
...
TR::treetop
   iload #378
*/
void TR_CopyPropagation::replaceCopySymbolReferenceByOriginalIn(
    TR::SymbolReference *copySymbolReference, TR::Node *origNode,
    TR::Node *node, TR::Node *defNode, TR::Node *baseAddrNode,
    bool baseAddrAvail) {
  vcount_t curVisit = comp()->getVisitCount();

#ifdef J9_PROJECT_SPECIFIC
  if (_useTree && !node->getOpCode().isLoadConst() &&
      cg()->IsInMemoryType(node->getType())) {
    if (cg()->traceBCDCodeGen() &&
        _useTree->getNode()->chkOpsIsInMemoryCopyProp() &&
        !_useTree->getNode()->isInMemoryCopyProp())
      traceMsg(comp(), "\tset IsInMemoryCopyProp on %s (%p), useNode %s (%p)\n",
               _useTree->getNode()->getOpCode().getName(), _useTree->getNode(),
  ...
  if (node->getVisitCount() != curVisit) {
...
#endif
      {
        ...
        if (origNode->getOpCode().isLoadIndirect()) {
          if (baseAddrAvail && baseAddrNode != NULL) {
            ...
            TR::Node *baseAddrNodeCopy =
                TR::Node::copy(origNode->getFirstChild());
          ...
          if (trace() || comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                     "node %s (%p) #%d->#%d, origNode %s (%p) #%d, defNode %s "
                     "(%p) #%d,copySymRef #%d\n",
                     node->getOpCode().getName(), node,
          ...
          if (origNode->getNumChildren() == 2) {
            ...
            if (origNode->getOpCode().hasSymbolReference() &&
                node->getOpCode().hasSymbolReference())
              node->setSymbolReference(origNode->getSymbolReference());
            int32_t nodePrecision = node->getDecimalPrecision();
            int32_t nodeSize = node->getSize();
            int32_t origSymSize =
                origNode->getSymbolReference()->getSymbol()->getSize();
            int32_t origSymPrecision = TR::DataType::getBCDPrecisionFromSize(
                origNode->getDataType(), origSymSize);
            // for example for a node with prec=30 and an origSymPrecision with
            // prec=31 then if the symbol sizes match and the other checks below
            // pass then can reduce the newPrecision to 30 so a pdModPrec
            // correction is not needed (as the propagated symbol will not have
            ...
            if (node->getType().isAnyPacked()) {
              if (origSymPrecision > nodePrecision &&  // 31 > 30
                  nodePrecision ==
                      origNode->getDecimalPrecision() &&  // 30 == 30
                  ...
                  nodeSize == origSymSize)                // 16 == 16
              {
                if (trace() || comp()->cg()->traceBCDCodeGen())
                  traceMsg(
                      ...
                      "reduce newPrecision %d->%d for odd to even truncation "
                      "(origNode %s (%p) prec=%d, node %s (%p) prec=%d\n",
                      newPrecision, nodePrecision,
                      origNode->getOpCode().getName(), origNode,
                      origNode->getDecimalPrecision(),
                      node->getOpCode().getName(), node, nodePrecision);
                newPrecision =
                    nodePrecision;  // 31->30 -- no correction needed for same
                                    // byte odd to even truncation if the
                                    // origNode was only storing out the even $
                                    // of digits
              }
            }
          }

          // if newPrecision < currentPrecision then one or more digits will be
          // lost during the copyPropagation -- the isCorrect* routines disallow
          // this
          TR_ASSERT(newPrecision >= currentPrecision,
                    "newPrecision < currentPrecision (%d < %d) during "
                    "copyPropagation\n",
          ...
          if (trace() || comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                     "needsClean = %s, needsPrecisionCorrection = %s (cur %d, "
                     "new %d)\n",
                     needsClean ? "yes" : "no",
                     needsPrecisionCorrection ? "yes" : "no", currentPrecision,
                     newPrecision);
          ...
          //
          // may need a pdclean and/or pdModPrec on top of the propagated to
          // node
          //
          ...
          if (needsClean || needsPrecisionCorrection) {
            ...
            // being replaced then a modifyPrecision node must be prepended in
            // case the new parent requires explicit widening (such as if the
            // parent is a call). Also if the defNode was going to clean must
            // also insert a pdclean on top of the node with the propagated
            // symRef
            ...
            dumpOptDetails(comp(),
                           "node %p precision %d != propagated symRef #%d "
                           "precision %d and/or needsClean (%s) so create "
                           "nodeCopy %p (isTwoChildrenCase=%s)\n",
                           node, currentPrecision,
                           origNode->getSymbolReference()->getReferenceNumber(),
                           newPrecision, needsClean ? "yes" : "no", nodeCopy,
                           twoChildrenCase ? "yes" : "no");

            if (needsPrecisionCorrection) {
              TR::ILOpCodes modPrecOp =
                  TR::ILOpCode::modifyPrecisionOpCode(nodeCopy->getDataType());
              TR_ASSERT(modPrecOp != TR::BadILOp,
                        "no bcd modify precision opcode found for node %p with "
                        "type %s\n",
                        nodeCopy, nodeCopy->getDataType().toString());
              if (needsClean)  // i.e. if a clean will be generated below then
                               // do not modify node op yet, let the top level
                               // op (the coming clean) do this
              {
                TR::Node *pdModPrecNode =
                    TR::Node::create(modPrecOp, 1, nodeCopy);
                pdModPrecNode->setDecimalPrecision(currentPrecision);
                nodeCopy = pdModPrecNode;
                dumpOptDetails(comp(),
                               "create %s (0x%p) to correct precision %d->%d\n",
                               pdModPrecNode->getOpCode().getName(),
                               pdModPrecNode, newPrecision, currentPrecision);
              } else {
                ...
                dumpOptDetails(
                    comp(),
                    "modify load 0x%p to %s to correct precision %d->%d\n",
                    node, node->getOpCode().getName(), newPrecision,
                    currentPrecision);

                node = nodeCopy;  // so the recursion continues from the correct
                                  // point below
              }
            }
            ...
            if (needsClean) {
              TR::ILOpCodes cleanOp =
                  TR::ILOpCode::cleanOpCode(nodeCopy->getDataType());
              TR_ASSERT(cleanOp != TR::BadILOp,
                        "no bcd modify precision cleanOp found for node %p "
                        "with type %s\n",
                        nodeCopy, nodeCopy->getDataType().toString());
              // can only clean up to maxPackedPrec -- case allow should have
              // been guaranteed by isCorrectToPropagate
              TR_ASSERT(node->getDecimalPrecision() <=
            ...
            dumpOptDetails(comp(),
                           "attach new first %s (%p) and second %s (%p) to "
                           "node %s (%p) in the twoChildren=true case\n",
    ...
    for (int i = 0; i < node->getNumChildren(); i++) {
      ...
      replaceCopySymbolReferenceByOriginalIn(copySymbolReference, origNode,
                                             child, defNode);
    }
#endif
  }
...
/*
   replaceCopySymbolReferenceByOriginalRegLoadIn uses a slightly different
visitCount idiom -- The visit count for a node is set iff the node has NOT been
replaced with a regLoadNode. This is done to make sure that we will replace ALL
qualified nodes with the regLoadNode.
...
TR::treetop
   iload #378
*/
void TR_CopyPropagation::replaceCopySymbolReferenceByOriginalRegLoadIn(
    TR::Node *regLoadNode, TR::Node *useNode,
    TR::SymbolReference *copySymbolReference, TR::Node *node, TR::Node *parent,
    int32_t childNum) {
  vcount_t curVisit = comp()->getVisitCount();
  ...
  if (node->getVisitCount() != curVisit) {
    ...
    if (!changedNode) node->setVisitCount(curVisit);

    for (int i = 0; i < node->getNumChildren(); i++) {
      TR::Node *child = node->getChild(i);
      replaceCopySymbolReferenceByOriginalRegLoadIn(
          regLoadNode, useNode, copySymbolReference, child, node, i);
    }
  }
}

bool TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node *useNode,
                                                          TR::Node *storeNode,
                                                          TR::Node *loadNode) {
#ifdef J9_PROJECT_SPECIFIC
  if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
    dumpOptDetails(
        comp(),
        "isLoadNodeSuitableForPropagation : useNode %p (prec %d), loadNode %p "
        "(prec %d) -- isCorrect=%s (only correct when useNodePrec <= "
        "loadNodePrec)\n",
        useNode, useNode->getDecimalPrecision(), loadNode,
        loadNode->getDecimalPrecision(),
        useNode->getDecimalPrecision() <= loadNode->getDecimalPrecision()
            ? "yes"
            : "no");
    if (useNode->getDecimalPrecision() > loadNode->getDecimalPrecision()) {
      return false;
    } else if (storeNode && storeNode->mustClean() &&
               (useNode->getDecimalPrecision() >
                TR::DataType::getMaxPackedDecimalPrecision())) {
      // have to disallow this case because will not be able to clean useNode
      // after propagation for prec > max
      dumpOptDetails(comp(),
                     "isLoadNodeSuitableForPropagation=false for useNode %s "
                     "(%p) prec %d > max %d and mustClean store %p\n",
                     useNode->getOpCode().getName(), useNode,
                     useNode->getDecimalPrecision(),
...
// isCorrectToPropagate should be called for any load on the RHS with a symbol
// reference.
void collectNodesForIsCorrectChecks(TR::Node *n,
                                    TR::list<TR::Node *> &checkNodes,
                                    TR::SparseBitVector &refsToCheckIfKilled,
  ...
  for (size_t i = 0; i < n->getNumChildren(); i++) {
    collectNodesForIsCorrectChecks(n->getChild(i), checkNodes,
                                   refsToCheckIfKilled, vc);
  }
}

// If there are load nodes (loadNode or baseNode) in the RHS, and some parent of
// those nodes has ref count > 1, then add to the list of nodes to perform
// isSafeToPropagate on.
void collectNodesForIsSafeChecks(TR::Node *n,
                                 TR::list<TR::Node *> &anchorCheckNodes,
                                 vcount_t vc, bool sawRefCountLargerThanOne) {
  if (n->getVisitCount() == vc) {
    return;
  }
  ...
  // If either the ref count of the load node itself or a parent node is larger
  // than one, add it to the list of nodes to call isSafeToPropagate on because
  // it could be anchored earlier in the block. Only consider nodes with symbol
  // references, because isSafeToPropagate calls loadNode->getSymbolReference().
  //
  // Note that we want to check any load node - the entire RHS could be
  // propagated.
  //
  ...
  for (size_t i = 0; i < n->getNumChildren(); i++) {
    collectNodesForIsSafeChecks(n->getChild(i), anchorCheckNodes, vc,
                                sawRefCountLargerThanOne);
  }
}

void TR_CopyPropagation::adjustUseDefInfo(TR::Node *defNode, TR::Node *useNode,
...
TR::Node *TR_CopyPropagation::areAllDefsInCorrectForm(
    TR::Node *useNode, const TR_UseDefInfo::BitVector &defs,
    TR_UseDefInfo *useDefInfo, int32_t firstRealDefIndex, TR::Node *&loadNode,
    int32_t &regNumber, TR::Node *&baseNode) {
  ...
  //...exception point that could reach a catch
  // catch block
  // load b  //  copy propagating "a" here is incorrect since we will end up
  // propagating rhs2 value (in "a") instead of rhs1 value (in "b") changing
  // program semantics
  //
  if (defs.PopulationCount() > 1) return NULL;
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
    ...
    if (!defNode->getOpCode().isStore()) return NULL;
#else
    if (!defNode->getOpCode().isStoreDirect()) return NULL;
#endif

#ifdef PROPAGATE_GLOBALS
    if (!useDefInfo->isPreciseDef(defNode, useNode)) return NULL;
#else
    ...
    if ((loadNode =
             isCheapRematerializationCandidate(defNode, rhsOfStoreDefNode)) ||
...

    {
      ...
      if (regNumber == -1) {
        ...
        collectNodesForIsSafeChecks(rhsOfStoreDefNode, anchorCheckNodes,
                                    comp()->incOrResetVisitCount(), false);
        for (auto cur = anchorCheckNodes.begin(); cur != anchorCheckNodes.end();
             ++cur) {
      ...
      if (lastDefNode == NULL) {
        ...
        if (regNumber != -1) return NULL;  // TODO: handle multiple regLoads
  ...
  if (lastDefNode != NULL) {
    ...
    collectNodesForIsCorrectChecks(loadNode, checkNodes, refsToCheckIfKilled,
                                   comp()->incOrResetVisitCount());

    // Before setting the def node's symbol ref in refsToCheckIfKilled, check to
    // see that the store does not kill the RHS (see inner comment)
    if (_propagatingWholeExpression) {
      /*
      ...
      if (foundInterferenceBetweenCurrentNodeAndPropagation(
              comp(), trace(), currentNode, lastDefNode, checkNodes,
              refsToCheckIfKilled)) {
        if (trace()) {
          comp()->getDebug()->trace(
    ...
    // Both the load node and def node cannot be redefined between the two
    // points.
    //
    // x <- y
    // ...
    ...
    // z <- x + ...
    //
    refsToCheckIfKilled[lastDefNode->getSymbolReference()
                            ->getReferenceNumber()] = 1;

    if (!isCorrectToPropagate(useNode, lastDefNode, checkNodes,
                              refsToCheckIfKilled, regNumber, defs,
                              useDefInfo)) {
      return NULL;
    }
  }
...
TR::TreeTop *TR_CopyPropagation::findAnchorTree(TR::Node *storeNode,
                                                TR::Node *loadNode) {
  comp()->incOrResetVisitCount();
  ...
  if (lookup != _storeTreeTops.end()) {
    ...
    if (loadNode) {
      ...
      comp()->incOrResetVisitCount();
      while (
          !((treeTop->getNode()->getOpCode().getOpCodeValue() == TR::BBStart) &&
            !treeTop->getNode()->getBlock()->isExtensionOfPreviousBlock())) {
        comp()->incOrResetVisitCount();
        if (containsNode(treeTop->getNode(), loadNode)) anchor = treeTop;
...
//
//
bool TR_CopyPropagation::isSafeToPropagate(TR::Node *storeNode,
                                           TR::Node *loadNode) {
  ...
  if (lookup != _storeTreeTops.end()) {
    ...
    if (loadNode) {
      ...
      // Walk the tree backward to first treetop inside extended basic block,
      // reject if org sym is killed
      while (!((_storeTreeTop->getNode()->getOpCode().getOpCodeValue() ==
                TR::BBStart) &&
               !_storeTreeTop->getNode()
                    ->getBlock()
                    ->isExtensionOfPreviousBlock())) {
...
TR::Node *TR_CopyPropagation::skipTreeTopAndGetNode(TR::TreeTop *tt) {
  TR_ASSERT(tt,
            "tt should always be set no matter if it comes from _storeTreeTops "
            "an iterator");
  return (tt->getNode()->getOpCodeValue() == TR::treetop)
             ? tt->getNode()->getFirstChild()
             : tt->getNode();
}

bool TR_CopyPropagation::containsNode(TR::Node *node1, TR::Node *node2,
                                      bool searchClones) {
  vcount_t curVisit = comp()->getVisitCount();
  ...
  if (node1->getVisitCount() != curVisit) {
    ...
    for (int i = 0; i < node1->getNumChildren(); i++) {
      ...
      if (containsNode(child, node2)) return true;
    }
  }
...
bool TR_CopyPropagation::containsLoadOfSymbol(TR::Node *node,
  ...
  if (node->getVisitCount() != curVisit) {
    ...
    for (int i = 0; i < node->getNumChildren(); i++) {
      TR::Node *child = node->getChild(i);
      if (containsLoadOfSymbol(child, symRef, loadNode)) return true;
    }
  }
...
void TR_CopyPropagation::findUseTree(TR::Node *useNode) {
  ...
  if (lookup != _useTreeTops.end()) _useTree = lookup->second;
}

bool TR_CopyPropagation::isCorrectToPropagate(
    TR::Node *useNode, TR::Node *storeNode, TR::list<TR::Node *> &checkNodes,
    TR::SparseBitVector &refsToCheckIfKilled, int32_t regNumber,
  ...
  if (!isLoadNodeSuitableForPropagation(useNode, storeNode,
                                        storeNode->getValueChild())) {
    return false;
  }

  // The logic for propagating iRegLoads does not work if the def and use are in
  // the same extended block
  ...
  while (
      !(currentTree->getNode()->getOpCode().getOpCodeValue() == TR::BBStart)) {
    // TR_ASSERT(_storeTree->getNode()->getOpCode().isStoreDirect(), "Def node
    // should be a direct store\n");
    TR::Node *currentNode = skipTreeTopAndGetNode(currentTree);
    ...
    if (currentTree == _storeTree) {
      return true;
             ...
             defs.ValueAt(currentNode->getUseDefIndex() -
                          useDefInfo->getFirstDefIndex()))
#else
    else if (_lookForOriginalDefs && currentNode->getOpCode().isStoreDirect() &&
             currentNode->getSymbolReference() ==
...
#endif
    {
      ...
      if (regNumber == -1) {
        if (foundInterferenceBetweenCurrentNodeAndPropagation(
                comp(), trace(), currentNode, storeNode, checkNodes,
                refsToCheckIfKilled)) {
  ...
  for (auto nextEdge = block->getPredecessors().begin();
    ...
    if ((next->getVisitCount() != visitCount) && (next != cfg->getStart()) &&
      ...
      if (isRedefinedBetweenStoreTreeAnd(checkNodes, refsToCheckIfKilled,
                                         storeNode, next->getExit(), regNumber,
                                         defs, useDefInfo)) {
        return false;
      }
    }
...
bool TR_CopyPropagation::isRedefinedBetweenStoreTreeAnd(
    TR::list<TR::Node *> &checkNodes, TR::SparseBitVector &refsToCheckIfKilled,
    TR::Node *storeNode, TR::TreeTop *exitTree, int32_t regNumber,
    const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo) {
  static char *useRecursiveIsRedefinedBetweenStoreTreeAnd =
      feGetEnv("TR_useRecursiveIsRedefinedBetweenStoreTreeAnd");
  if (useRecursiveIsRedefinedBetweenStoreTreeAnd)
    return recursive_isRedefinedBetweenStoreTreeAnd(
        checkNodes, refsToCheckIfKilled, storeNode, exitTree, regNumber, defs,
        useDefInfo);
  ...
  while (currentTree != NULL) {
    ...
    while (!(currentTree->getNode()->getOpCode().getOpCodeValue() ==
             TR::BBStart)) {
      TR::Node *currentNode = skipTreeTopAndGetNode(currentTree);
    ...
    TR_ASSERT(saved_vc == comp()->getVisitCount(),
              "visit count changed in loop!");
    ...
    if (visitPredecessors) {
      ...
      for (auto precedingEdge = precedingEdges.getFirst(); precedingEdge;
        ...
        if ((next->getVisitCount() != visitCount) &&
            (next != cfg->getStart()) &&
            ((regNumber == -1) ||
             (next->startOfExtendedBlock() != _storeBlock))) {
...
bool TR_CopyPropagation::recursive_isRedefinedBetweenStoreTreeAnd(
    TR::list<TR::Node *> &checkNodes, TR::SparseBitVector &refsToCheckIfKilled,
    TR::Node *storeNode, TR::TreeTop *exitTree, int32_t regNumber,
    const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo) {
  TR::TreeTop *currentTree = exitTree;
  ...
  while (
...
#endif
    {
      ...
      if (regNumber == -1) {
        ...
        if (currentNode->getOpCode().isStoreReg()) {
          if (currentNode->getGlobalRegisterNumber() == regNumber) return true;
        }
      }
    }
  ...
  for (auto nextEdge = block->getPredecessors().begin();
    ...
    if ((next->getVisitCount() != visitCount) && (next != cfg->getStart()) &&
      ...
      if (recursive_isRedefinedBetweenStoreTreeAnd(
              checkNodes, refsToCheckIfKilled, storeNode, next->getExit(),
...
bool TR_CopyPropagation::isCorrectToReplace(
    TR::Node *useNode, TR::Node *storeNode,
    const TR_UseDefInfo::BitVector &defs, TR_UseDefInfo *useDefInfo) {
#ifdef J9_PROJECT_SPECIFIC
  if (useNode->getType().isBCD() && storeNode->getType().isBCD()) {
    dumpOptDetails(
        comp(),
        "isCorrectToReplace : useNode %p (prec %d), storeNode %p (prec %d) -- "
        "isCorrect=%s (only correct when useNodePrec <= storeNodePrec)\n",
        useNode, useNode->getDecimalPrecision(), storeNode,
        storeNode->getDecimalPrecision(),
        useNode->getDecimalPrecision() <= storeNode->getDecimalPrecision()
            ? "yes"
            : "no");
  ...
  collectNodesForIsCorrectChecks(storeNode, checkNodes, refsToCheckIfKilled,
                                 comp()->incOrResetVisitCount());

  // x <- y
  // ..
  // z <- x + a
  refsToCheckIfKilled[storeNode->getSymbolReference()->getReferenceNumber()] =
      1;
...
TR::Node *TR_CopyPropagation::isIndirectLoadFromRegister(TR::Node *node,
                                                         TR::Node *&baseNode) {
  baseNode = NULL;
...
TR::Node *TR_CopyPropagation::isCheapRematerializationCandidate(
    TR::Node *defNode, TR::Node *node) {
  // Only run if GRA has already run - do not rematerialize if GRA hasn't had
  // the chance to work with the temps.
  if (!comp()->cg()->getGRACompleted()) {
    return NULL;
  }

  if ((defNode->getSymbolReference() == NULL) ||
      !(comp()->IsCopyPropagationRematerializationCandidate(
          defNode->getSymbolReference()))) {
    return NULL;
  }
  ...
  //
  //  iaload <refined-array-shadow> [id=238:"PSALIT2"] [#1.64 Shadow] [flags
  //  0x80000607 0x0 ]  [0x2B026BFC] bci=[-1,37,329] rc=2 vc=9438 vn=- sti=-
  //  udi=331 nc=1 addr=4
  //    aiadd (internalPtr ) [0x2B016B20] bci=[-1,37,329] rc=1 vc=9438 vn=-
  //    sti=- udi=- nc=2 addr=4 flg=0x8000
  //      loadaddr <auto slot 40> [id=152:"PSA"] [#1.63 Auto] [flags 0x4000008
  //      0x0 ]    [0x2B016AD0] bci=[-1,37,329] rc=1 vc=9438 vn=- sti=- udi=-
  //      nc=0 addr=4 iconst 1212 (X!=0 X>=0 ) [0x2B016B70] bci=[-1,37,329] rc=1
  //      vc=9438 vn=- sti=- udi=- nc=0 flg=0x104
  //
  if (node->getOpCode().isLoadIndirect()) {
...
bool TR_CopyPropagation::isNodeAvailableInBlock(TR::TreeTop *useTree,
                                                TR::Node *loadNode) {
  ...
  while (treeTop->getNode()->getOpCode().getOpCodeValue() != TR::BBStart ||
    ...
    TR::Node *dummyNode;
    if (containsLoadOfSymbol(node, loadNode->getSymbolReference(),
                             &dummyNode)) {
      return true;
    }
...
TR::Node *TR_CopyPropagation::isValidRegLoad(TR::Node *rhsOfStoreDefNode,
                                             TR::TreeTop *defTree,
                                             int32_t &regNumber) {
  regNumber = -1;
  ...
  if (!isRegLoad) regNumber = -1;

  return (isRegLoad ? rhsOfStoreDefNode : NULL);
}

const char *TR_CopyPropagation::optDetailString() const throw() {
