    ...
    void propagateThroughToCommoningPoints()
    {
        ...
        if (oldNode->getReferenceCount() > 0) {
            ...
            while (cur_tt && (cur_tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
                ...
                while ((cur_tt->getNode()->getOpCodeValue() == TR::BBEnd) && (cur_tt->getNextTreeTop() && (cur_tt->getNextTreeTop()->getNode()->getOpCodeValue() == TR::BBStart) && (cur_tt->getNextTreeTop()->getNode()->getBlock()->isExtensionOfPreviousBlock()))) {
            ...
            if (storeLookup != storeMap.end()) {
                for (auto itr = storeLookup->second->begin(), end = storeLookup->second->end(); itr != end; ++itr) {
                    ...
                    if (prevDefNode->getType().isBCD()) {
                        ...
                        if (defNode->getType().isBCD() && defNode->getDataType() == prevDefNode->getDataType() && prevDefNode->getDecimalPrecision() == defNode->getDecimalPrecision() && (defNode->getDataType() != TR::PackedDecimal || prevDefNode->mustCleanSignInPDStoreEvaluator() == defNode->mustCleanSignInPDStoreEvaluator())) {
                ...
                if (performTransformation(comp(), "%s   Copy1 Propagation replacing Copy symRef #%d %p %p\n", OPT_DETAILS, copySymbolReference->getReferenceNumber(), defNode, equivalentDefNode)) {
                    ...
                    if (defSeenFirst) {
                        ...
                        while (cursorTree != equivalentDefTree) {
                            if (!cursorTree->getNode()->getOpCode().isStore() || (cursorTree->getNode()->getFirstChild() != defNode->getFirstChild())) {
                                safe = false;
                                break;
                            }
        ...
        for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
            ...
            if (useDefInfo->getUseDef(defs, i)) {
                ...
                if (baseNode) {
                    ...
                    if (!_cleanupTemps || baseNode->getSymbolReference() == defNode->getSymbolReference() || !isNodeAvailableInBlock(_useTree, baseNode))
                        continue;
                }
                ...
                if (performTransformation(comp(), "%s   Copy2 Propagation replacing Copy symRef #%d \n", OPT_DETAILS, copySymbolReference->getReferenceNumber())) {
                    ...
                    if (!isRegLoad && (loadNode->getOpCodeValue() != TR::loadaddr) && // loadaddr doesn't need to be anchored
                        (loadNode->getOpCode().hasSymbolReference()) && (loadNode->getSymbolReference() == copySymbolReference) && (rhsOfStoreDefNode != loadNode)) // Modifications to this condition should be mirrored in the NOTEMPS check above.
                    {
...
void TR_CopyPropagation::rematerializeIndirectLoadsFromAutos()
{
    ...
    while (currentTree) {
        ...
        if (node->getOpCode().isStoreIndirect() && (node->getType().isIntegral() || node->getType().isAddress()) && node->getFirstChild()->getOpCodeValue() == TR::loadaddr && node->getFirstChild()->getSymbol()->isAutoOrParm() && nextNode->getOpCode().isStoreDirect() && nextNode->getSymbol()->isAutoOrParm() && nextNode->getFirstChild() == node->getSecondChild() && performTransformation(comp(), "%s   Rematerializing indirect load from auto in node %p (temp #%d)\n", OPT_DETAILS, nextNode->getFirstChild(), nextNode->getSymbolReference()->getReferenceNumber())) {
            comp()->setOsrStateIsReliable(false); // could check if the propagation was done to a child of the OSR helper call here
...
void TR_CopyPropagation::commonIndirectLoadsFromAutos()
{
    ...
    while (currentTree) {
        ...
        if (node->getOpCode().isStoreIndirect() && (node->getType().isIntegral() || node->getType().isAddress()) && node->getFirstChild()->getOpCodeValue() == TR::loadaddr && node->getFirstChild()->getSymbol()->isAutoOrParm() && nextNode->getOpCode().isStoreDirect() && nextNode->getSymbol()->isAutoOrParm() && nextNode->getFirstChild()->getOpCode().isLoadIndirect() && nextNode->getFirstChild()->getSymbolReference() == node->getSymbolReference() && nextNode->getFirstChild()->getFirstChild() == node->getFirstChild() && performTransformation(comp(), "%s   Commoning indirect load from auto in node %p \n", OPT_DETAILS, nextNode->getFirstChild())) {
            comp()->setOsrStateIsReliable(false); // could check if the propagation was done to a child of the OSR helper call here
...
void collectNodesForIsSafeChecks(TR::Node* n, TR::list<TR::Node*>& anchorCheckNodes, vcount_t vc, bool sawRefCountLargerThanOne)
{
    ...
    //
    if (sawRefCountLargerThanOne && (n->getOpCode().isLoadVar() || n->getOpCode().isLoadAddr()) && (n->getSymbolReference() != NULL)) {
...
TR::Node* TR_CopyPropagation::areAllDefsInCorrectForm(TR::Node* useNode, const TR_UseDefInfo::BitVector& defs, TR_UseDefInfo* useDefInfo, int32_t firstRealDefIndex, TR::Node*& loadNode, int32_t& regNumber, TR::Node*& baseNode)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if ((loadNode = isCheapRematerializationCandidate(defNode, rhsOfStoreDefNode)) || (loadNode = isLoadVarWithConst(rhsOfStoreDefNode)) || (loadNode = isIndirectLoadFromAuto(rhsOfStoreDefNode)) || (loadNode = isValidRegLoad(rhsOfStoreDefNode, defTree, regNumber)) || (loadNode = isIndirectLoadFromRegister(rhsOfStoreDefNode, baseNode)))
...
TR::TreeTop* TR_CopyPropagation::findAnchorTree(TR::Node* storeNode, TR::Node* loadNode)
{
    ...
    if (lookup != _storeTreeTops.end()) {
        ...
        if (loadNode) {
            ...
            comp()->incOrResetVisitCount();
            while (!((treeTop->getNode()->getOpCode().getOpCodeValue() == TR::BBStart) && !treeTop->getNode()->getBlock()->isExtensionOfPreviousBlock())) {
                comp()->incOrResetVisitCount();
...
bool TR_CopyPropagation::isSafeToPropagate(TR::Node* storeNode, TR::Node* loadNode)
{
    ...
    if (lookup != _storeTreeTops.end()) {
        ...
        if (loadNode) {
            ...
            while (!((_storeTreeTop->getNode()->getOpCode().getOpCodeValue() == TR::BBStart) && !_storeTreeTop->getNode()->getBlock()->isExtensionOfPreviousBlock())) {
    ...
    TR::SparseBitVector& refsToCheckIfKilled, int32_t regNumber, const TR_UseDefInfo::BitVector& defs, TR_UseDefInfo* useDefInfo)
{
...
#ifdef PROPAGATE_GLOBALS
        else if (_lookForOriginalDefs && currentNode->getOpCode().isStore() && defs.ValueAt(currentNode->getUseDefIndex() - useDefInfo->getFirstDefIndex()))
#else
        else if (_lookForOriginalDefs && currentNode->getOpCode().isStoreDirect() && currentNode->getSymbolReference() == storeNode->getSymbolReference())
#endif
        {
            return true;
        } else {
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        ...
        if ((next->getVisitCount() != visitCount) && (next != cfg->getStart()) && ((regNumber == -1) || (next->startOfExtendedBlock() != _storeBlock))) {
...
TR::Node* TR_CopyPropagation::isIndirectLoadFromAuto(TR::Node* node)
{
    ...
    if (node->getOpCode().isLoadIndirect() && node->getFirstChild()->getOpCodeValue() == TR::loadaddr && node->getFirstChild()->getSymbol()->isAutoOrParm()) {
        return node;
    }
...
TR::Node* TR_CopyPropagation::isCheapRematerializationCandidate(TR::Node* defNode, TR::Node* node)
{
    ...
    if ((defNode->getSymbolReference() == NULL) || !(comp()->IsCopyPropagationRematerializationCandidate(defNode->getSymbolReference()))) {
        return NULL;
    }
    ...
    if (node->getOpCode().isLoadIndirect()) {
        ...
        if (!cheapExpression && addr->getOpCode().isAdd()) {
            if ((addr->getFirstChild()->getOpCodeValue() == TR::loadaddr) && addr->getFirstChild()->getSymbol()->isAutoOrParm() && addr->getSecondChild()->getOpCode().isLoadConst()) {
                cheapExpression = true;
            }
        }
    }
