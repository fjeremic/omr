...
void
collectNodesForIsCorrectChecks(TR::Node* n,
...
TR_CopyPropagation::TR_CopyPropagation(TR::OptimizationManager* manager)
  ...
  , _useTreeTops((UseTreeMapComparator()),
                 UseTreeMapAllocator(comp()->trMemory()->currentStackRegion()))
{}

template<class T>
TR::Node*
getNodeParent(TR::Node* lookForNode,
              TR::Node* curNode,
              vcount_t visitCount,
              T& ci)
{
...
bool
foundInterferenceBetweenCurrentNodeAndPropagation(
  TR::Compilation* comp,
  bool trace,
  TR::Node* currentNode,
  TR::Node* storeNode,
  ...
  TR::SparseBitVector& refsToCheckIfKilled)
{
  ...
  if (trace) {
    ...
    for (auto checkNodesCursor = checkNodes.begin();
         checkNodesCursor != checkNodes.end();
         ++checkNodesCursor) {
...
private:
  ...
  {
    _parentOfTargetNode = getNodeParent(_targetNode,
                                        _targetTreeTop->getNode(),
    ...
    TR::Node* sourceTree)
  {
    while (targetTreeTop->getNode()->containsNode(
      ...
      size_t childIndex;
      TR::Node* parentNode = getNodeParent(targetNode,
                                           targetTreeTop->getNode(),
                                           comp()->incOrResetVisitCount(),
                                           childIndex);
              ...
              oldSize == newSymSize)                            // 16 == 16
          {
            if (trace() || comp()->cg()->traceBCDCodeGen()) {
              traceMsg(comp(),
                       ...
                       oldNode->getOpCode().getName(),
                       oldNode,
                       oldPrecision);
            }

            newPrecision =
              oldPrecision; // 31->30 -- no correction needed for same byte odd
                            // to even truncation if the origNode was only
  ...
  void propagateThroughToCommoningPoints()
  {
    ...
    if (oldNode->getReferenceCount() > 0) {
      ...
      collectNodesForIsCorrectChecks(newNode,
                                     newCheckNodes,
      ...
      while (cur_tt && (cur_tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
        ...
        replaceAllInstancesOfTargetNodeInTreeBySourceTree(
          cur_tt, oldNode, newNode);
...
TR_CopyPropagation::perform()
{
...

  {
        ...
        for (auto itr = storeLookup->second->begin(),
          ...
          if (thisDefSlot < 0) // pending pushes
            thisSymRefIsPreferred = true;
                   ...
                   comp()
                     ->getOwningMethodSymbol(
                       defNode->getSymbolReference()->getOwningMethodIndex())
          ...
          {
            dumpOptDetails(comp(),
                           "%s   Def nodes %p and %p are equivalent\n",
                           OPT_DETAILS,
                           defNode,
                           prevDefNode);
    ...
    for (auto itr = equivalentDefs.begin(), end = equivalentDefs.end();
      ...
      useDefInfo->getUsesFromDef(
        uses, itr->first, useDefInfo->hasLoadsAsDefs());
      ...
      for (useCursor.SetToFirstOne(); useCursor.Valid();
        ...
        if (performTransformation(
          ...
          dumpOptDetails(comp(),
                         "%s   By Original symRef #%d in Use node : %p\n",
                         OPT_DETAILS,
                         originalSymbolReference->getReferenceNumber(),
                         useNode);
          dumpOptDetails(comp(),
                         "%s   Use #%d[%p] is defined by:\n",
                         OPT_DETAILS,
                         i,
                         useNode);
          dumpOptDetails(comp(),
          ...
          if (trace()) {
            ...
            traceMsg(comp(),
                     "      Does NOT get Defined by #%d[%p] from now\n",
    ...
    for (int32_t i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
      ...
      if (useDefInfo->getUseDef(defs, i)) {
        ...
        int32_t regNumber;
        TR::Node* defNode = areAllDefsInCorrectForm(useNode,
                                                    defs,
                                                    useDefInfo,
                                                    firstRealDefIndex,
                                                    loadNode,
        ...
        if (performTransformation(
              comp(),
              "%s   Copy2 Propagation replacing Copy symRef #%d \n",
              OPT_DETAILS,
              copySymbolReference->getReferenceNumber())) {
          comp()->setOsrStateIsReliable(
            false); // could check if the propagation was done to a child of the
                          ...
                          // in the NOTEMPS check above.
          {
            ...
            TR::SymbolReference* newSymbolReference =
              comp()->getSymRefTab()->createTemporary(
                ...
                copySymbolReference->getSymbol()->getDataType(),
                false,
                copySymbolReference->getSymbol()->getSize());

            dumpOptDetails(comp(),
                           "%s      created new temp #%d\n",
                           OPT_DETAILS,
          ...
          if (_propagatingWholeExpression) {
            ...
            replaceCopySymbolReferenceByOriginalIn(copySymbolReference,
                                                   rhsOfStoreDefNode,
                                                   useNode,
                                                   defNode,
                                                   baseAddr,
            ...
            if (bbstartNode->getNumChildren() > 0) {
              for (int i = 0;
                   i < bbstartNode->getFirstChild()->getNumChildren();
                   i++) {
...
void
TR_CopyPropagation::collectUseTrees(TR::TreeTop* tree,
                                    TR::Node* node,
...
void
TR_CopyPropagation::rematerializeIndirectLoadsFromAutos()
{
  TR::TreeTop* currentTree = comp()->getStartTree();
  while (currentTree) {
    ...
    if (node->getOpCode().isStoreIndirect() &&
      ...
      comp()->setOsrStateIsReliable(
        false); // could check if the propagation was done to a child of the OSR
                // helper call here
...
void
TR_CopyPropagation::commonIndirectLoadsFromAutos()
{
  TR::TreeTop* currentTree = comp()->getStartTree();
  while (currentTree) {
    TR::Node* node = currentTree->getNode();
    ...
    if (node->getOpCode().isStoreIndirect() &&
        ...
        performTransformation(
          comp(),
          "%s   Commoning indirect load from auto in node %p \n",
          OPT_DETAILS,
          nextNode->getFirstChild())) {
      comp()->setOsrStateIsReliable(
        false); // could check if the propagation was done to a child of the OSR
...
*/
void
TR_CopyPropagation::replaceCopySymbolReferenceByOriginalIn(
  TR::SymbolReference* copySymbolReference,
  TR::Node* origNode,
  TR::Node* node,
  TR::Node* defNode,
  ...
  bool baseAddrAvail)
{
  ...
  if (_useTree && !node->getOpCode().isLoadConst() &&
    ...
    if (cg()->traceBCDCodeGen() &&
      ...
      traceMsg(comp(),
               "\tset IsInMemoryCopyProp on %s (%p), useNode %s (%p)\n",
               _useTree->getNode()->getOpCode().getName(),
               _useTree->getNode(),
  ...
  if (node->getVisitCount() != curVisit) {
        ...
        if (origNode->getOpCode().isLoadIndirect()) {
                  ...
                  nodeSize == origSymSize)             // 16 == 16
              {
                if (trace() || comp()->cg()->traceBCDCodeGen())
                  traceMsg(
                    ...
                    nodePrecision);
                newPrecision =
                  nodePrecision; // 31->30 -- no correction needed for same byte
          ...
          if (trace() || comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                     ...
                     "new %d)\n",
                     needsClean ? "yes" : "no",
                     needsPrecisionCorrection ? "yes" : "no",
                     currentPrecision,
                     newPrecision);
          ...
          if (needsClean || needsPrecisionCorrection) {
            ...
            if (needsClean) {
              ...
              node->setFlags(0);
              dumpOptDetails(comp(),
                             "modify load 0x%p to %s to clean\n",
                             node,
                             node->getOpCode().getName());
    ...
    for (int i = 0; i < node->getNumChildren(); i++) {
      ...
      replaceCopySymbolReferenceByOriginalIn(
        copySymbolReference, origNode, child, defNode);
    }
#endif
  }
}
...
*/
void
TR_CopyPropagation::replaceCopySymbolReferenceByOriginalRegLoadIn(
  TR::Node* regLoadNode,
  TR::Node* useNode,
  TR::SymbolReference* copySymbolReference,
  TR::Node* node,
...
bool
TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node* useNode,
                                                     TR::Node* storeNode,
                                                     TR::Node* loadNode)
{
#ifdef J9_PROJECT_SPECIFIC
  if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
    dumpOptDetails(
      ...
      useNode->getDecimalPrecision() <= loadNode->getDecimalPrecision() ? "yes"
                                                                        : "no");
                               ...
                               vcount_t vc)
{
  ...
  for (size_t i = 0; i < n->getNumChildren(); i++) {
    collectNodesForIsCorrectChecks(
      n->getChild(i), checkNodes, refsToCheckIfKilled, vc);
  }
}
...
void
collectNodesForIsSafeChecks(TR::Node* n,
                            ...
                            bool sawRefCountLargerThanOne)
{
  ...
  for (size_t i = 0; i < n->getNumChildren(); i++) {
    collectNodesForIsSafeChecks(
      n->getChild(i), anchorCheckNodes, vc, sawRefCountLargerThanOne);
  }
}

void
TR_CopyPropagation::adjustUseDefInfo(TR::Node* defNode,
                                     TR::Node* useNode,
...
TR::Node*
TR_CopyPropagation::areAllDefsInCorrectForm(
  TR::Node* useNode,
  ...
  TR::Node*& baseNode)
{
  ...
  for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
...

    {
      ...
      if (regNumber == -1) {
        ...
        collectNodesForIsSafeChecks(rhsOfStoreDefNode,
                                    anchorCheckNodes,
                                    comp()->incOrResetVisitCount(),
                                    false);
  ...
  if (lastDefNode != NULL) {
    ...
    collectNodesForIsCorrectChecks(loadNode,
                                   checkNodes,
                                   refsToCheckIfKilled,
                                   comp()->incOrResetVisitCount());
    ...
    if (!isCorrectToPropagate(useNode,
                              lastDefNode,
                              checkNodes,
                              refsToCheckIfKilled,
                              regNumber,
...
TR::TreeTop*
TR_CopyPropagation::findAnchorTree(TR::Node* storeNode, TR::Node* loadNode)
{
  comp()->incOrResetVisitCount();
...
//
bool
TR_CopyPropagation::isSafeToPropagate(TR::Node* storeNode, TR::Node* loadNode)
{
  ...
  if (lookup != _storeTreeTops.end()) {
    ...
    if (loadNode) {
      ...
      while (!(
        (_storeTreeTop->getNode()->getOpCode().getOpCodeValue() ==
         TR::BBStart) &&
...
TR::Node*
TR_CopyPropagation::skipTreeTopAndGetNode(TR::TreeTop* tt)
{
  TR_ASSERT(tt,
...
bool
TR_CopyPropagation::containsNode(TR::Node* node1,
                                 TR::Node* node2,
                                 bool searchClones)
{
  vcount_t curVisit = comp()->getVisitCount();
...
bool
TR_CopyPropagation::containsLoadOfSymbol(TR::Node* node,
...
void
TR_CopyPropagation::findUseTree(TR::Node* useNode)
{
  if (_useTree)
    return;
...
bool
TR_CopyPropagation::isCorrectToPropagate(
  TR::Node* useNode,
  TR::Node* storeNode,
  TR::list<TR::Node*>& checkNodes,
  TR::SparseBitVector& refsToCheckIfKilled,
  ...
  TR_UseDefInfo* useDefInfo)
{
  ...
  if (!isLoadNodeSuitableForPropagation(
        useNode, storeNode, storeNode->getValueChild())) {
    return false;
  }
  ...
  for (auto nextEdge = block->getPredecessors().begin();
    ...
    if ((next->getVisitCount() != visitCount) && (next != cfg->getStart()) &&
      ...
      if (isRedefinedBetweenStoreTreeAnd(checkNodes,
                                         refsToCheckIfKilled,
                                         storeNode,
                                         next->getExit(),
                                         regNumber,
  ...
  for (auto nextEdge = block->getExceptionPredecessors().begin();
       nextEdge != block->getExceptionPredecessors().end();
       ++nextEdge) {
    TR::Block* next = toBlock((*nextEdge)->getFrom());
    if ((next->getVisitCount() != visitCount) && (next != cfg->getStart()) &&
        ((regNumber == -1) || (next->startOfExtendedBlock() != _storeBlock))) {
...
bool
TR_CopyPropagation::isRedefinedBetweenStoreTreeAnd(
  TR::list<TR::Node*>& checkNodes,
  TR::SparseBitVector& refsToCheckIfKilled,
  TR::Node* storeNode,
  TR::TreeTop* exitTree,
  ...
  TR_UseDefInfo* useDefInfo)
{
  ...
  if (useRecursiveIsRedefinedBetweenStoreTreeAnd)
    return recursive_isRedefinedBetweenStoreTreeAnd(checkNodes,
                                                    refsToCheckIfKilled,
                                                    storeNode,
                                                    exitTree,
                                                    regNumber,
...
bool
TR_CopyPropagation::recursive_isRedefinedBetweenStoreTreeAnd(
  TR::list<TR::Node*>& checkNodes,
  TR::SparseBitVector& refsToCheckIfKilled,
  TR::Node* storeNode,
  TR::TreeTop* exitTree,
  ...
  TR_UseDefInfo* useDefInfo)
{
  ...
  for (auto nextEdge = block->getPredecessors().begin();
    ...
    if ((next->getVisitCount() != visitCount) && (next != cfg->getStart()) &&
        ((regNumber == -1) || (next->startOfExtendedBlock() != _storeBlock))) {
      if (recursive_isRedefinedBetweenStoreTreeAnd(checkNodes,
                                                   refsToCheckIfKilled,
                                                   storeNode,
                                                   next->getExit(),
                                                   regNumber,
...
bool
TR_CopyPropagation::isCorrectToReplace(TR::Node* useNode,
                                       TR::Node* storeNode,
                                       const TR_UseDefInfo::BitVector& defs,
                                       TR_UseDefInfo* useDefInfo)
{
  ...
  collectNodesForIsCorrectChecks(
    storeNode, checkNodes, refsToCheckIfKilled, comp()->incOrResetVisitCount());
...
TR::Node*
TR_CopyPropagation::isLoadVarWithConst(TR::Node* node)
{
...
TR::Node*
TR_CopyPropagation::isIndirectLoadFromAuto(TR::Node* node)
{
  if (!_cleanupTemps)
    return NULL;
...
TR::Node*
TR_CopyPropagation::isIndirectLoadFromRegister(TR::Node* node,
                                               TR::Node*& baseNode)
{
  baseNode = NULL;
...
bool
nodeContainsLoadReg(TR::Compilation* comp, TR::Node* n, vcount_t vc)
{
  if (n->getVisitCount() == vc) {
    return false;
  }
...
bool
TR_CopyPropagation::isNodeAvailableInBlock(TR::TreeTop* useTree,
                                           TR::Node* loadNode)
{
  ...
  while (treeTop->getNode()->getOpCode().getOpCodeValue() != TR::BBStart ||
    ...
    TR::Node* dummyNode;
    if (containsLoadOfSymbol(
          node, loadNode->getSymbolReference(), &dummyNode)) {
      return true;
    }
...
TR::Node*
TR_CopyPropagation::isValidRegLoad(TR::Node* rhsOfStoreDefNode,
                                   TR::TreeTop* defTree,
                                   int32_t& regNumber)
{
  regNumber = -1;
