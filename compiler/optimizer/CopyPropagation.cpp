...
bool foundInterferenceBetweenCurrentNodeAndPropagation(
  ...
  if (trace) {
    comp->getDebug()->trace("foundInterferenceBetweenCurrentNodeAndPropagation:"
                            " currentNode %p storeNode %p checkNodes [",
...
public:
  TR_ExpressionPropagation(TR::Compilation *c, bool t)
      : _compilation(c), _trace(t), _targetTreeTop(0), _targetNode(0),
        _sourceTree(0), _originalDefNodeForSourceTree(0), _targetChildIndex(-1),
              ...
              oldSize == newSymSize)                            // 16 == 16
          {
            ...
            newPrecision =
                oldPrecision; // 31->30 -- no correction needed for same byte
                              // odd to even truncation if the origNode was only
                              // storing out the even $ of digits
          }
        }
      }
        ...
        if (origNode->getOpCode().isLoadIndirect()) {
          ...
          if (needsClean || needsPrecisionCorrection) {
            ...
            if (needsPrecisionCorrection) {
              ...
              if (needsClean) // i.e. if a clean will be generated below then do
                              // not modify node op yet, let the top level op
                              // (the coming clean) do this
              {
...
bool TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node *useNode,
  ...
  if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
    dumpOptDetails(comp(),
                   "isLoadNodeSuitableForPropagation : useNode %p (prec %d), "
                   "loadNode %p (prec %d) -- isCorrect=%s (only correct when "
                   "useNodePrec <= loadNodePrec)\n",
...
TR::Node *TR_CopyPropagation::skipTreeTopAndGetNode(TR::TreeTop *tt) {
  TR_ASSERT(tt, "tt should always be set no matter if it comes from "
                "_storeTreeTops an iterator");
...
TR::Node *
TR_CopyPropagation::isCheapRematerializationCandidate(TR::Node *defNode,
                                                      TR::Node *node) {
