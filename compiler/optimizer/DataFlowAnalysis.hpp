...
class TR_BackwardIntersectionDFSetAnalysis<Container *>;
...
public:
  ...
  static void operator delete(void *ptr, TR::Allocator a) {
    ...
    // will be aborted, and all memory associated with that compilation will get
    // freed.
  }
  ...
  TR_DataFlowAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                      TR::Optimizer *optimizer, bool trace)
      : _comp(comp), _cfg(cfg), _optimizer(optimizer), _trace(trace),
        _analysisQueue(comp->trMemory()), _changedSetsQueue(comp->trMemory()) {
  ...
  TR_PersistentMemory *trPersistentMemory() {
    return _comp->trPersistentMemory();
  }
...
#include "optimizer/DataFlowAnalysis.enum"
  ...
  // virtual TR_IntersectionBitVectorAnalysis
  // *asIntersectionBitVectorAnalysis(); virtual TR_BackwardBitVectorAnalysis
  // *asBackwardBitVectorAnalysis(); virtual
  // TR_BackwardIntersectionBitVectorAnalysis
  // *asBackwardIntersectionBitVectorAnalysis(); virtual
  // TR_BackwardUnionBitVectorAnalysis *asBackwardUnionBitVectorAnalysis();
  ...
  bool collectCallSymbolReferencesInTreeInto(TR::Node *,
                                             List<TR::SymbolReference> *);
  bool collectAllSymbolReferencesInTreeInto(TR::Node *,
                                            List<TR::SymbolReference> *);
  ...
  template <class Container>
  static void copyFromInto(Container *from, Container *to) {
    if (from)
      *to = *from;
    else
      to->empty();
...
//
template <class Container>
class TR_BasicDFSetAnalysis<Container *> : public TR_DataFlowAnalysis {
public:
  TR_BasicDFSetAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                        TR::Optimizer *optimizer, bool trace)
  ...
  class TR_ContainerNodeNumberPair
      : public TR_Link<TR_ContainerNodeNumberPair> {
  public:
  ...
  struct ExtraAnalysisInfo {
    ...
    Container *getContainer(TR_LinkHead<TR_ContainerNodeNumberPair> *list,
                            int32_t n);
    TR_ContainerNodeNumberPair *
    getContainerNodeNumberPair(TR_LinkHead<TR_ContainerNodeNumberPair> *list,
                               int32_t n);
  ...
  void initializeAnalysisInfo(ExtraAnalysisInfo *info,
  ...
  virtual void allocateContainer(Container **result, bool isSparse = true,
                                 bool lock = false);
  virtual void allocateBlockInfoContainer(Container **result,
                                          bool isSparse = true,
                                          bool lock = false);
...
//
template <class Container>
class TR_ForwardDFSetAnalysis<Container *>
    : public TR_BasicDFSetAnalysis<Container *> {
public:
  TR_ForwardDFSetAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                          TR::Optimizer *optimizer, bool trace)
  ...
  virtual void analyzeNode(TR::Node *, vcount_t, TR_BlockStructure *,
                           Container *);

  bool analyzeNodeIfPredecessorsAnalyzed(TR_RegionStructure *, TR_BitVector &);

  virtual void initializeGenAndKillSetInfo(TR_RegionStructure *,
                                           TR_BitVector &);
...
//
template <class Container>
class TR_IntersectionDFSetAnalysis<Container *>
    : public TR_ForwardDFSetAnalysis<Container *> {
public:
  TR_IntersectionDFSetAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                               TR::Optimizer *optimizer, bool trace)
...
class TR_IntersectionBitVectorAnalysis
    : public TR_IntersectionDFSetAnalysis<TR_BitVector *> {
public:
  typedef TR_BitVector ContainerType;
  TR_IntersectionBitVectorAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                                   TR::Optimizer *optimizer, bool trace)
      : TR_IntersectionDFSetAnalysis<TR_BitVector *>(comp, cfg, optimizer,
                                                     trace) {}
...
//
template <class Container>
class TR_UnionDFSetAnalysis<Container *>
    : public TR_ForwardDFSetAnalysis<Container *> {
public:
  TR_UnionDFSetAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                        TR::Optimizer *optimizer, bool trace)
      : TR_ForwardDFSetAnalysis<Container *>(comp, cfg, optimizer, trace) {}
...
public:
  typedef TR_BitVector ContainerType;
  TR_UnionBitVectorAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                            TR::Optimizer *optimizer, bool trace)
      : TR_UnionDFSetAnalysis<TR_BitVector *>(comp, cfg, optimizer, trace) {}
};

class TR_UnionSingleBitContainerAnalysis
    : public TR_UnionDFSetAnalysis<TR_SingleBitContainer *> {
public:
  typedef TR_SingleBitContainer ContainerType;
  TR_UnionSingleBitContainerAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                                     TR::Optimizer *optimizer, bool trace)
      : TR_UnionDFSetAnalysis<TR_SingleBitContainer *>(comp, cfg, optimizer,
                                                       trace) {}
};

class TR_ReachingDefinitions : public TR_UnionBitVectorAnalysis {
public:
  TR_ReachingDefinitions(TR::Compilation *comp, TR::CFG *cfg,
                         TR::Optimizer *optimizer, TR_UseDefInfo *,
...
private:
  void initializeGenAndKillSetInfoForNode(TR::Node *node,
                                          TR_BitVector &defsKilled,
                                          bool seenException, int32_t blockNum,
                                          TR::Node *parent);
...
//
template <class Container>
class TR_BackwardDFSetAnalysis<Container *>
    : public TR_BasicDFSetAnalysis<Container *> {
public:
  TR_BackwardDFSetAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                           TR::Optimizer *optimizer, bool trace)
      : TR_BasicDFSetAnalysis<Container *>(comp, cfg, optimizer, trace),
  ...
  bool analyzeNodeIfSuccessorsAnalyzed(TR_RegionStructure *, TR_BitVector &,
                                       TR_BitVector &);
...
public:
  TR_BackwardIntersectionDFSetAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                                       TR::Optimizer *optimizer, bool trace)
...
public:
  typedef TR_BitVector ContainerType;
  TR_BackwardIntersectionBitVectorAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                                           TR::Optimizer *optimizer, bool trace)
      : TR_BackwardIntersectionDFSetAnalysis<TR_BitVector *>(comp, cfg,
                                                             optimizer, trace) {
  }
};
...
//
template <class Container>
class TR_BackwardUnionDFSetAnalysis<Container *>
    : public TR_BackwardDFSetAnalysis<Container *> {
public:
  TR_BackwardUnionDFSetAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                                TR::Optimizer *optimizer, bool trace)
      : TR_BackwardDFSetAnalysis<Container *>(comp, cfg, optimizer, trace) {}
...
public:
  TR_BackwardUnionBitVectorAnalysis(TR::Compilation *comp, TR::CFG *cfg,
                                    TR::Optimizer *optimizer, bool trace)
      : TR_BackwardUnionDFSetAnalysis<TR_BitVector *>(comp, cfg, optimizer,
                                                      trace) {}
};
...
public:
  TR_BackwardUnionSingleBitContainerAnalysis(TR::Compilation *comp,
                                             TR::CFG *cfg,
                                             TR::Optimizer *optimizer,
                                             bool trace)
      : TR_BackwardUnionDFSetAnalysis<TR_SingleBitContainer *>(
            comp, cfg, optimizer, trace) {}
};
...
public:
  TR_GlobalAnticipatability(TR::Compilation *comp, TR::Optimizer *optimizer,
                            TR_Structure *, bool trace);
  ...
  TR_LocalTransparency _localTransparency; // _localTransparency should be
                                           // before _localAnticipatability
...
public:
  TR_Earliestness(TR::Compilation *comp, TR::Optimizer *optimizer,
                  TR_Structure *, bool trace);
...
public:
  TR_Delayedness(TR::Compilation *comp, TR::Optimizer *optimizer,
                 TR_Structure *, bool trace);
...
public:
  TR_Latestness(TR::Compilation *comp, TR::Optimizer *optimizer, TR_Structure *,
                bool trace);
...
public:
  TR_Isolatedness(TR::Compilation *comp, TR::Optimizer *optimizer,
                  TR_Structure *, bool trace);
...
public:
  ...
  TR_LiveVariableInformation(TR::Compilation *comp, TR::Optimizer *,
                             TR_Structure *, bool splitLongs = false,
  ...
  void initializeGenAndKillSetInfo(TR_BitVector **genSetInfo,
...
protected:
  virtual void
  findUseOfLocal(TR::Node *node, int32_t blockNum, TR_BitVector **genSetInfo,
                 TR_BitVector **killSetInfo, TR_BitVector *commonedLoads,
                 bool movingForwardThroughTrees, vcount_t visitCount);

private:
  void visitTreeForLocals(TR::Node *node, TR_BitVector **blockGenSetInfo,
                          TR_BitVector *blockKillSetInfo,
                          bool movingForwardThroughTrees, bool visitEntireTree,
                          vcount_t visitCount, TR_BitVector *commonedLoads,
  ...
  // these are caches to generate this information once and share among multiple
  // analyses once they are allocated via createGenAndKillSetCaches(), the next
  // call to initializeGenAndKillSetInfo() will store
  //   the sets into these caches and set _haveCachedGenAndKillSets.  Subsequent
  //   calls to initializeGenAndKillSetInfo() will change the arrays passed in
  //   to refer to the bit vectors in these caches.
  ...
  // this bit vector keeps track of locals that are associated with local
  // objects
...
public:
  ...
  TR_OSRLiveVariableInformation(TR::Compilation *comp, TR::Optimizer *,
                                TR_Structure *, bool splitLongs = false,
                                bool includeParms = false,
                                bool ignoreOSRUses = false);
...
private:
  ...
  void buildLiveSymbolsBitVector(TR_OSRMethodData *osrMethodData,
                                 int32_t byteCodeIndex, TR_BitVector *);

  typedef TR::typed_allocator<std::pair<const int32_t, TR_BitVector *>,
                              TR::Region &>
      TR_BCLiveSymbolsMapAllocator;
  typedef std::less<int32_t> TR_BCLiveSymbolsMapComparator;
  typedef std::map<int32_t, TR_BitVector *, TR_BCLiveSymbolsMapComparator,
                   TR_BCLiveSymbolsMapAllocator>
      TR_BCLiveSymbolsMap;
...
public:
  TR_Liveness(TR::Compilation *comp, TR::Optimizer *optimizer, TR_Structure *,
              bool ignoreOSRUses = false,
              TR_LiveVariableInformation *liveVariableInfo = NULL,
              bool splitLongs = false, bool includeParms = false);

  TR_LiveVariableInformation *getLiveVariableInfo() {
    return _liveVariableInfo;
  }
  virtual Kind getKind();
  virtual TR_Liveness *asLiveness();
...
// Live on all paths (LOAP) - analysis that identifies when a variable
// definition is live on all subsequent paths. Backward Intersection BitVector
// analysis.
//
class TR_LiveOnAllPaths : public TR_BackwardIntersectionBitVectorAnalysis {
public:
  TR_LiveOnAllPaths(TR::Compilation *comp, TR::Optimizer *optimizer,
                    TR_Structure *,
                    TR_LiveVariableInformation *liveVariableInfo = 0,
                    bool splitLongs = false, bool includeParms = false);
...
public:
  TR_FlowSensitiveEscapeAnalysis(TR::Compilation *comp,
                                 TR::Optimizer *optimizer,
...
private:
  ...
  // void initializeGenAndKillSetInfoForNode(TR::Node *node, bool seenException,
  ...
  TR_BitVector *_blocksWithFlushes,
      *_blocksWithSyncs; //, *_blocksThatNeedFlush;
