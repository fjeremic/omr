...
void
TR_ReachabilityAnalysis::propagateOneInput(blocknum_t inputBlockNum,
                                           blocknum_t blockNum,
                                           int32_t depth,
                                           blocknum_t* stack,
                                           blocknum_t* depthMap,
                                           TR_BitVector* result)
{
  ...
  if (result->isSet(inputBlockNum)) {
    if (comp()->getOption(TR_TraceReachability))
      traceMsg(comp(),
               "    Propagate block_%d to block_%d\n",
               blockNum,
               inputBlockNum);
    result->set(blockNum);
  } else {
    if (comp()->getOption(TR_TraceReachability))
      traceMsg(comp(),
               "    No change to block_%d from block_%d\n",
               blockNum,
               inputBlockNum);
  }
}
...
void
TR_ReachabilityAnalysis::traverse(blocknum_t blockNum,
                                  int32_t depth,
                                  blocknum_t* stack,
                                  blocknum_t* depthMap,
                                  TR_BitVector* result)
  ...
  if (depthMap[blockNum] == depth) {
    while (1) {
      ...
      if (top == blockNum) {
        ...
        if (trace)
          traceMsg(comp(),
                   "    Loop: propagate block_%d to block_%d\n",
                   blockNum,
                   top);
...
void
TR_ForwardReachability::propagateInputs(blocknum_t blockNum,
                                        int32_t depth,
                                        blocknum_t* stack,
                                        ...
                                        TR_BitVector* closure)
{
  ...
  for (TR::CFGEdge* edge = bi.getFirst(); edge != NULL; edge = bi.getNext()) {
    ...
    propagateOneInput(
      inputBlock->getNumber(), blockNum, depth, stack, depth_map, closure);
  }
}

void
TR_ForwardReachabilityWithoutExceptionEdges::propagateInputs(
  blocknum_t blockNum,
  int32_t depth,
  blocknum_t* stack,
  blocknum_t* depth_map,
...
void
TR_BackwardReachability::propagateInputs(blocknum_t blockNum,
                                         int32_t depth,
                                         blocknum_t* stack,
                                         blocknum_t* depth_map,
                                         TR_BitVector* closure)
