...
bool TR::LocalDeadStoreElimination::isNonRemovableStore(TR::Node* storeNode, bool& seenIdentityStore)
{
  ...
  // except for placeholder stores for monexit, we need to see those in order to pop the live monitor stack the correct
  // number of times
...
void TR::LocalDeadStoreElimination::transformBlock(TR::TreeTop* entryTree, TR::TreeTop* exitTree)
{
  ...
  while (!(_curTree == entryTree)) {
    ...
    if (storeNode) {
      ...
      && (currentNode->getFirstChild()->getOpCode().isCall() || currentNode->getFirstChild()->getOpCode().isStore())) {
    ...
    while (!(_curTree == exitTree)) {
      ...
      if (firstChild
        && !(callWithSideEffects || firstChild->getOpCodeValue() == TR::New
               || firstChild->getOpCodeValue() == TR::newarray || firstChild->getOpCodeValue() == TR::anewarray
               || firstChild->getOpCodeValue() == TR::multianewarray || firstChild->getOpCodeValue() == TR::MergeNew ||
...
void TR::LocalDeadStoreElimination::adjustStoresInfo(TR::Node* node, TR_BitVector& deadSymbolReferences)
{
  ...
  } else if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit
  ...
  TR::Node* parentNode, int32_t nodeIndex, TR::Node* node, TR::TreeTop* treeTop, TR::NodeChecklist& visited)
{
  ...
  if (node->getReferenceCount() > 1) {
    ...
    // (node->getNumChildren()-1) so the first reference flag is propagated correctly with setIsFirstReferenceToNode in
    // the code just above. When not using TR_FirstNodeReferenceTracker the future use count is decremented below to
    // handle this case. NOTE: TR_FirstNodeReferenceTracker is now permanently disabled.
  ...
  for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    if (storeNode && storeNode->getOpCode().isStoreIndirect() && !storeNode->getSymbolReference()->isUnresolved()) {
      if (storeNode->getFirstChild()->getOpCode().hasSymbolReference()
        ...
        && (storeNode->getFirstChild()->getSymbolReference()->getSymbol()->getLocalObjectSymbol()->getOpCodeKind()
          == TR::New)
  ...
  List<TR::Node>* currentNews, List<TR::Node>* removedNews, TR::Node* parent, int32_t childNum, vcount_t visitCount)
{
  ...
  if (currentNews->find(newNode)
    && ((
      ((parent->getOpCode().isIndirect() || parent->getOpCode().isArrayLength())) || parent->getOpCode().isCall()))) {
    if (trace())
