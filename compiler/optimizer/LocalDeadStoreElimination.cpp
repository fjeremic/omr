...
TR::Optimization*
TR::LocalDeadStoreElimination::create(TR::OptimizationManager* manager)
{
...
void
TR::LocalDeadStoreElimination::setIsFirstReferenceToNode(TR::Node* parent,
                                                         int32_t index,
                                                         TR::Node* node)
{
...
bool
TR::LocalDeadStoreElimination::isFirstReferenceToNode(TR::Node* parent,
                                                      int32_t index,
                                                      TR::Node* node)
{
...
int32_t
TR::LocalDeadStoreElimination::perform()
{
  if (trace())
...
int32_t
TR::LocalDeadStoreElimination::performOnBlock(TR::Block* block)
{
  if (block->getEntry()) {
...
void
TR::LocalDeadStoreElimination::prePerformOnBlocks()
{
...
void
TR::LocalDeadStoreElimination::postPerformOnBlocks()
{}

bool
TR::LocalDeadStoreElimination::isNonRemovableStore(TR::Node* storeNode,
...
void
TR::LocalDeadStoreElimination::transformBlock(TR::TreeTop* entryTree,
                                              TR::TreeTop* exitTree)
{
  _curTree = entryTree;
      ...
      if (firstChild &&
        ...
        if (firstChild->getReferenceCount() == 0 &&
            performTransformation(comp(),
                                  "%sRemoving Dead Anchor : %s [0x%p]\n",
                                  optDetailString(),
                                  currentNode->getOpCode().getName(),
                                  currentNode)) {
          _treesChanged = true;
...
TR::TreeTop*
TR::LocalDeadStoreElimination::removeStoreTree(TR::TreeTop* treeTop)
{
  ...
  if (isEntireNodeRemovable(storeNode)) {
    ...
    if (performTransformation(
          comp(),
          "%sAnchoring rhs of store : %s [0x%p] in a treetop\n",
          optDetailString(),
          storeNode->getOpCode().getName(),
          storeNode)) {
      bool removedTree = false;
...
//
bool
TR::LocalDeadStoreElimination::isIdentityStore(TR::Node* storeNode)
{
...
void
TR::LocalDeadStoreElimination::examineNode(TR::Node* parent,
                                           int32_t childNum,
                                           TR::Node* node,
...
//
bool
TR::LocalDeadStoreElimination::isEntireNodeRemovable(TR::Node* storeNode)
{
...
void
TR::LocalDeadStoreElimination::setExternalReferenceCountToTree(
  TR::Node* node,
...
bool
TR::LocalDeadStoreElimination::seenIdenticalStore(TR::Node* node)
{
...
bool
TR::LocalDeadStoreElimination::areLhsOfStoresSyntacticallyEquivalent(
  TR::Node* node1,
  TR::Node* node2)
{
  int32_t childAdjust;
...
void
TR::LocalDeadStoreElimination::adjustStoresInfo(
  TR::Node* node,
  TR_BitVector& deadSymbolReferences)
{
...
//
TR::Node*
TR::LocalDeadStoreElimination::getAnchorNode(TR::Node* parentNode,
                                             int32_t nodeIndex,
                                             TR::Node* node,
                                             TR::TreeTop* treeTop,
...
void
TR::LocalDeadStoreElimination::setupReferenceCounts(TR::Node* node)
{
...
void
TR::LocalDeadStoreElimination::eliminateDeadObjectInitializations()
{
  //
  ...
  for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop())
    findLocallyAllocatedObjectUses(
      usedLocalObjectSymbols, NULL, -1, tt->getNode(), visitCount);
  ...
  for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    if (examineNewUsesForKill(
          node, storeNode, &currentNews, &removedNews, NULL, -1, visitCount)) {
...
void
TR::LocalDeadStoreElimination::findLocallyAllocatedObjectUses(
  LDSBitVector& usedLocalObjectSymbols,
  TR::Node* parent,
  int32_t childNum,
  TR::Node* node,
  vcount_t visitCount)
{
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i)
    findLocallyAllocatedObjectUses(
      usedLocalObjectSymbols, node, i, node->getChild(i), visitCount);
}

bool
TR::LocalDeadStoreElimination::examineNewUsesForKill(
  TR::Node* node,
  TR::Node* storeNode,
  List<TR::Node>* currentNews,
  List<TR::Node>* removedNews,
  TR::Node* parent,
  int32_t childNum,
  vcount_t visitCount)
{
  ...
  //
  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
    if (examineNewUsesForKill(node->getChild(i),
                              storeNode,
                              currentNews,
                              removedNews,
                              node,
...
void
TR::LocalDeadStoreElimination::killStoreNodes(TR::Node* node)
{
