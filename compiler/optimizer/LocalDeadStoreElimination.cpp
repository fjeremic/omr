...
bool TR::LocalDeadStoreElimination::isNonRemovableStore(TR::Node* storeNode, bool& seenIdentityStore)
{
    ...
    */
    ...
    if (!storeNode->getSymbolReference()->getSymbol()->holdsMonitoredObject()) {
        ...
        if (prevStoreNode && !storeNode->getOpCode().isIndirect() && !prevStoreNode->getOpCode().isIndirect() && storeNode->getFirstChild() == prevStoreNode->getFirstChild() && storeNode->getSymbolReference() == prevStoreNode->getSymbolReference()) {
    ...
    */
    ...
    while (!(_curTree == entryTree)) {
       ...
       */
        ...
        if (storeNode) {
            ...
            if (!nonRemovableStore && (seenIdentityStore || // 1)
                                          (deadSymbolReferences.get(symRefNum) && // 2)
                                              !symRef->getSymbol()->isVolatile() && ((_blockContainsReturn && //   a)
        ...
        } else if (currentNode->getNumChildren() > 0 && (currentNode->getFirstChild()->getOpCode().isCall() || currentNode->getFirstChild()->getOpCode().isStore())) {
        ...
        //
        if (currentNode->getOpCode().isBranch() || currentNode->getOpCode().isJumpWithMultipleTargets() || (currentNode->getOpCodeValue() == TR::treetop && currentNode->getFirstChild()->getOpCode().isJumpWithMultipleTargets())) {
            _blockContainsReturn = false;
    ...
    if (_treesAnchored) {
        ...
        while (!(_curTree == exitTree)) {
            ...
            if (firstChild && firstChild->getOpCode().isCall()) {
                ...
                if (firstChild->getSymbolReference()->getSymbol()->isResolvedMethod() && firstChild->getSymbolReference()->getSymbol()->castToResolvedMethodSymbol()->isSideEffectFree())
                    callWithSideEffects = false;
            }

            if (firstChild && !(callWithSideEffects || firstChild->getOpCodeValue() == TR::New || firstChild->getOpCodeValue() == TR::newarray || firstChild->getOpCodeValue() == TR::anewarray || firstChild->getOpCodeValue() == TR::multianewarray || firstChild->getOpCodeValue() == TR::MergeNew ||
                                      ///firstChild->getOpCodeValue() == TR::checkcast ||
                                      firstChild->getOpCode().isCheckCast() || firstChild->getOpCodeValue() == TR::
                                      instanceof || firstChild->getOpCodeValue() == TR::monent || firstChild->getOpCodeValue() == TR::monexit)) {
                if (firstChild->getReferenceCount() == 0 && performTransformation(comp(), "%sRemoving Dead Anchor : %s [0x%p]\n", optDetailString(), currentNode->getOpCode().getName(), currentNode)) {
                    _treesChanged = true;
        ...
        if (performTransformation(comp(), "%sAnchoring rhs of store : %s [0x%p] in a treetop\n", optDetailString(), storeNode->getOpCode().getName(), storeNode)) {
            ...
            if (comp()->useAnchors()) {
                ...
                for (; curTree->getNode()->getOpCodeValue() != TR::BBEnd; curTree = curTree->getNextTreeTop())
                    if (curTree->getNode()->getOpCode().isAnchor() && curTree->getNode()->getFirstChild() == storeNode) {
            ...
            if ((curTree != treeTop) || !removedTree) {
                if (nodeInTreeTop->getOpCode().isAnchor() || (nodeInTreeTop->getOpCode().isCheck() && !nodeInTreeTop->getOpCode().isNullCheck()))
...
bool TR::LocalDeadStoreElimination::isIdentityStore(TR::Node* storeNode)
{
    ...
    if (storedValue->getSymbol() != storeNode->getSymbol())
        return false;
    if ((storeNode->getOpCode().isIndirect() && !storedValue->getOpCode().isIndirect()) || (!storeNode->getOpCode().isIndirect() && storedValue->getOpCode().isIndirect()))
        return false;
    ...
    if (comp()->useCompressedPointers() && (storedValue->getOpCodeValue() == TR::aloadi) && storedValue->getReferenceCount() == 2) {
    ...
    if (doChecksForAnchors) {
        ...
        //
        TR::TreeTop* prevTree = _curTree->getPrevTreeTop();
        if ((prevTree->getNode()->getOpCodeValue() == anchorOp) && (prevTree->getNode()->getFirstChild() == storedValue))
            return true;
    }
...
void TR::LocalDeadStoreElimination::examineNode(TR::Node* parent, int32_t childNum, TR::Node* node, TR_BitVector& deadSymbolReferences)
{
    ...
    if (node->getOpCode().isCall() || node->getOpCode().isCheck() || node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::multianewarray ||
            ///node->getOpCodeValue() == TR::checkcast ||
            node->getOpCode().isCheckCast() || node->getOpCodeValue() == TR::
            instanceof || node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit || node->mightHaveVolatileSymbolReference()) {
        int32_t symRefNum = symRef->getReferenceNumber();
...
void TR::LocalDeadStoreElimination::adjustStoresInfo(TR::Node* node, TR_BitVector& deadSymbolReferences)
{
    ...
    } else if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit || (node->isGCSafePointWithSymRef() && comp()->getOptions()->realTimeGC()) || node->mightHaveVolatileSymbolReference()) {
...
void TR::LocalDeadStoreElimination::eliminateDeadObjectInitializations()
{
    ...
    for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (storeNode && storeNode->getOpCode().isStoreIndirect() && !storeNode->getSymbolReference()->isUnresolved()) {
            if (storeNode->getFirstChild()->getOpCode().hasSymbolReference() && storeNode->getFirstChild()->getSymbolReference()->getSymbol()->isLocalObject() && (storeNode->getFirstChild()->getSymbolReference()->getSymbol()->getLocalObjectSymbol()->getOpCodeKind() == TR::New) && !usedLocalObjectSymbols.get(storeNode->getFirstChild()->getSymbolReference()->getSymbol()->getLocalIndex()))
                removableLocalObjectStore = true;
            else if (currentNews.find(storeNode->getFirstChild()) || (storeNode->getFirstChild()->getOpCode().isArrayRef() && currentNews.find(storeNode->getFirstChild()->getFirstChild()) && storeNode->getFirstChild()->getSecondChild()->getOpCode().isLoadConst())) {
                    ...
                    if (storeNode->getFirstChild()->getOpCode().isArrayRef()) {
                        if (TR::Compiler->target.is64Bit()) {
                            if (storeNode->getFirstChild()->getSecondChild()->getLongInt() > INT_MAX)
                                ...
                                offset = storeNode->getSymbolReference()->getOffset() + (int32_t)storeNode->getFirstChild()->getSecondChild()->getLongInt() - TR::Compiler->om.contiguousArrayHeaderSizeInBytes();
                        } else
                            offset = storeNode->getSymbolReference()->getOffset() + storeNode->getFirstChild()->getSecondChild()->getInt() - TR::Compiler->om.contiguousArrayHeaderSizeInBytes();
                    } else
        ...
        if ((removableZeroStore || removableLocalObjectStore) && performTransformation(comp(), "%sRemoving Dead Store : %s [0x%p]\n", optDetailString(), tt->getNode()->getOpCode().getName(), tt->getNode())) {
...
{
    if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference()->getSymbol()->isLocalObject() && node->getSymbolReference()->getSymbol()->getLocalObjectSymbol()->getOpCodeKind() == TR::New) && !(parent->getOpCode().isStoreIndirect() && childNum == 0 && ((uint32_t)parent->getSymbolReference()->getOffset() < fe()->getObjectHeaderSizeInBytes())))
...
bool TR::LocalDeadStoreElimination::examineNewUsesForKill(TR::Node* node, TR::Node* storeNode, List<TR::Node>* currentNews, List<TR::Node>* removedNews, TR::Node* parent, int32_t childNum, vcount_t visitCount)
{
    ...
    if (node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray)
        newNode = node;

    node = origNode;

    if (currentNews->find(newNode) && ((((parent->getOpCode().isIndirect() || parent->getOpCode().isArrayLength())) || parent->getOpCode().isCall()))) {
        if (trace())
