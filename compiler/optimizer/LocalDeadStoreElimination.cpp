...
TR::Optimization *TR::LocalDeadStoreElimination::create(
    TR::OptimizationManager *manager) {
  return new (manager->allocator()) TR::LocalDeadStoreElimination(manager);
}

TR::LocalDeadStoreElimination::LocalDeadStoreElimination(
    TR::OptimizationManager *manager)
    : TR::Optimization(manager) {}

void TR::LocalDeadStoreElimination::setIsFirstReferenceToNode(TR::Node *parent,
                                                              int32_t index,
                                                              TR::Node *node) {
  parent ? node->setLocalIndex(1) : node->setLocalIndex(0);
}

bool TR::LocalDeadStoreElimination::isFirstReferenceToNode(TR::Node *parent,
                                                           int32_t index,
                                                           TR::Node *node) {
...
int32_t TR::LocalDeadStoreElimination::perform() {
  if (trace()) traceMsg(comp(), "Starting LocalDeadStoreElimination\n");
  ...
  if (_treesChanged) requestDeadTreesCleanup();
...
void TR::LocalDeadStoreElimination::prePerformOnBlocks() {
  ...
  // As we walk backwards in the main analysis, the local index will keep
  // getting decremented giving us an idea about how many more references exist
...
bool TR::LocalDeadStoreElimination::isNonRemovableStore(
    TR::Node *storeNode, bool &seenIdentityStore) {
   ...
   */
  if (currentNode->getOpCode().isResolveCheck()) nonRemovableStore = true;

  // NOTE: The sym->usedInNestedProc() check for LocalDeadStoreElimination is
  // too conservative as the seenIdenticalStore check can legally eliminate even
  // nested symbols i.e. istore "a"  <- even with nested "a" this store can be
  // eliminated due to the redefine of "a" below
  //    x
  // istore "a"
  //    y
  // The usedInNestedProc check is to prevent a auto/parm store to a nested
  // symbol in a nested routine being removed when there is only a return and no
  // uses from the store to the end of the block: istore "b"  // not legal to
  // remove this store as the auto/parm 'escapes' to the parent (owning) routine
  // as they share a stack
  //    z
  // return
  if (storeNode->dontEliminateStores(true)) nonRemovableStore = true;
  ...
  // except for placeholder stores for monexit, we need to see those in order to
  // pop the live monitor stack the correct number of times
  if (!storeNode->getSymbolReference()->getSymbol()->holdsMonitoredObject()) {
    TR::Node *prevStoreNode =
        _curTree->getPrevTreeTop()->getNode()->getStoreNode();
    if (prevStoreNode && !storeNode->getOpCode().isIndirect() &&
        !prevStoreNode->getOpCode().isIndirect() &&
        storeNode->getFirstChild() == prevStoreNode->getFirstChild() &&
        storeNode->getSymbolReference() ==
            prevStoreNode->getSymbolReference()) {
...
void TR::LocalDeadStoreElimination::transformBlock(TR::TreeTop *entryTree,
                                                   TR::TreeTop *exitTree) {
  _curTree = entryTree;
  ...
  while (!(_curTree == exitTree)) {
    if (_curTree->getNode()->getStoreNode()) numStores++;
  ...
  while (!(_curTree == entryTree)) {
    ...
    if (comp()->getVisitCount() ==
        MAX_VCOUNT - 2)  // Inc visit count will assert at MAX_VCOUNT - 1, so
                         // this gives us the one extra incVisitCount that might
                         // occur if we remove a store tree.
    {
      if (trace())
        traceMsg(
            comp(),
            "Bailing out of local deadstore to avoid visit count overflow\n");
      break;
    }
    ...
    if (currentNode->getOpCode().isReturn()) {
      ...
      for (int32_t symRefNumber =
               comp()->getSymRefTab()->getIndexOfFirstSymRef();
    ...
    if (storeNode) {
      ...
      bool nonRemovableStore =
          isNonRemovableStore(storeNode, seenIdentityStore);

      TR::SymbolReference *symRef = storeNode->getSymbolReference();
      int32_t symRefNum = symRef->getReferenceNumber();

      // the logic below is:
      // when nonRemovableStore=false then removal of the store can happen if
      // one of the following conditions is true: 1) seenIdentityStore=true 2)
      // haven't seen a use and not volatile and one of the following is also
      // true
      //    a) the block contains a return and not nested and sym is an auto or
      //    parm (i.e. val doesn't escape) b) an identical store has been seen
      if (!nonRemovableStore && (seenIdentityStore ||                     // 1)
  ...
  if (_treesAnchored) {
    ...
    while (!(_curTree == exitTree)) {
      currentNode = _curTree->getNode();
      TR::Node *firstChild = currentNode->getOpCodeValue() == TR::treetop
                                 ? currentNode->getFirstChild()
                                 : 0;
      ...
      if (firstChild &&
          !(callWithSideEffects || firstChild->getOpCodeValue() == TR::New ||
                ...
                instanceof || firstChild->getOpCodeValue() == TR::monent ||
                               firstChild->getOpCodeValue() == TR::monexit)) {
        if (firstChild->getReferenceCount() == 0 &&
            performTransformation(
                comp(), "%sRemoving Dead Anchor : %s [0x%p]\n",
                optDetailString(), currentNode->getOpCode().getName(),
                currentNode)) {
          _treesChanged = true;
...
TR::TreeTop *TR::LocalDeadStoreElimination::removeStoreTree(
    TR::TreeTop *treeTop) {
  ...
  if ((storeNode != nodeInTreeTop) &&
      (nodeInTreeTop->getOpCodeValue() == TR::NULLCHK)) {
    ...
    TR::Node *passThroughNode =
        TR::Node::create(TR::PassThrough, 1, referenceNode);
  ...
  } else if ((storeNode != nodeInTreeTop) &&
             (nodeInTreeTop->getOpCodeValue() == TR::BNDCHKwithSpineCHK)) {
    ...
    TR::Node *replacementNode =
        TR::Node::createConstZeroValue(storeNode, storeNode->getDataType());
  ...
  if (isEntireNodeRemovable(storeNode)) {
    ...
    if (performTransformation(
            comp(), "%sAnchoring rhs of store : %s [0x%p] in a treetop\n",
            optDetailString(), storeNode->getOpCode().getName(), storeNode)) {
      bool removedTree = false;
      TR::TreeTop *curTree = NULL;

      // if the store is dead, remove the corresponding compressRefs node as
      // well
      //
      if (comp()->useAnchors()) {
        ...
        for (; curTree->getNode()->getOpCodeValue() != TR::BBEnd;
        ...
        if (translateNode) {
          dumpOptDetails(comp(),
                         "removing corresponding translation [%p] for [%p]\n",
...
bool TR::LocalDeadStoreElimination::isIdentityStore(TR::Node *storeNode) {
  ...
  if (!storedValue->getOpCode().hasSymbolReference()) return false;
  if (storeNode->getSymbolReference() == NULL) return false;
  if (storedValue->getSymbolReference() == NULL) return false;
  if (storedValue->getSymbol() != storeNode->getSymbol()) return false;
  if ((storeNode->getOpCode().isIndirect() &&
       !storedValue->getOpCode().isIndirect()) ||
      (!storeNode->getOpCode().isIndirect() &&
       storedValue->getOpCode().isIndirect()))
    return false;
  if (storedValue->getSymbol()->isVolatile()) return false;
  if (!storedValue->getOpCode().isLoadVar()) return false;
  if (storeNode->getOpCode().isIndirect() &&
      storeNode->getFirstChild() != storedValue->getFirstChild())
    return false;
  if (storeNode->getSymbolReference()->getOffset() !=
      storedValue->getSymbolReference()->getOffset())
    return false;
#ifdef J9_PROJECT_SPECIFIC
  if (storeNode->getType().isBCD() &&
      !storeNode->isDecimalSizeAndShapeEquivalent(storedValue))
    return false;
#endif
...
void TR::LocalDeadStoreElimination::examineNode(
    TR::Node *parent, int32_t childNum, TR::Node *node,
  ...
  if (node->getOpCode().isLoadVar() ||
    ...
    if (symRef->sharesSymbol()) {
      symRef->getUseDefAliases().getAliasesAndSubtractFrom(
          deadSymbolReferences);
    }

    killStoreNodes(node);
  }

  if (node->getOpCode().isCall() || node->getOpCode().isCheck() ||
          ...
          instanceof || node->getOpCodeValue() == TR::monent ||
    ...
    if (symRef->sharesSymbol()) {
      symRef->getUseDefAliases(isCallDirect)
          .getAliasesAndSubtractFrom(deadSymbolReferences);
    }
  }
}
...
//
// enhancement made to see if reference counts of children arise out of this
// node only count of edges will equal the accumulated reference count using
// externalReferenceCount = (accumulated reference count) - (edge count
// traversing tree from this edge) node is removable if externalReferenceCount
// is zero
//
bool TR::LocalDeadStoreElimination::isEntireNodeRemovable(TR::Node *storeNode) {
  if (storeNode->getReferenceCount() > 1) return false;
...
void TR::LocalDeadStoreElimination::setExternalReferenceCountToTree(
    TR::Node *node, rcount_t *externalReferenceCount) {
...
bool TR::LocalDeadStoreElimination::areLhsOfStoresSyntacticallyEquivalent(
    TR::Node *node1, TR::Node *node2) {
  int32_t childAdjust;
  ...
  for (int32_t i = 0; i < firstNonCheckChildOfFirstNode; i++) {
    if (node1->getChild(i) != node2->getChild(i)) return false;
  }

  return true;
}

void TR::LocalDeadStoreElimination::adjustStoresInfo(
    TR::Node *node, TR_BitVector &deadSymbolReferences) {
...
TR::Node *TR::LocalDeadStoreElimination::getAnchorNode(
    TR::Node *parentNode, int32_t nodeIndex, TR::Node *node,
  ...
  if (node->getReferenceCount() > 1) {
    ...
    TR::TreeTop *anchorTreeTop =
        TR::TreeTop::create(comp(), TR::Node::create(node, TR::treetop, 1));
    anchorTreeTop->getNode()->setAndIncChild(0, node);
    setIsFirstReferenceToNode(NULL, 0, anchorTreeTop->getNode());
    // first refs may be tracked by parent nodes so we must transfer the first
    // ref flag from the original parent (parentNode) to the newly created
    // treetop parent node
    ...
    // TR_FirstNodeReferenceTracker (when used) will mark the earliest reference
    // of a tree commoned node (> 1 ref in same tree) as the first overall
    // reference therefore visit children nodes from first (0) to last
    // (node->getNumChildren()-1) so the first reference flag is propagated
    // correctly with setIsFirstReferenceToNode in the code just above. When not
    // using TR_FirstNodeReferenceTracker the future use count is decremented
    // below to handle this case. NOTE: TR_FirstNodeReferenceTracker is now
    // permanently disabled.
    for (int i = 0; i < node->getNumChildren();
         i++)  // visit from 0->numChildren-1 see above comment
    {
      ...
      if (!visited.contains(child)) {
        ...
        if (child->getLocalIndex() > 1) {
          // A tree commoned node (> 1 ref in same tree) must have its future
          // use count decremented here so the anchored node previously created
          // in this function will be correctly recognized as a first reference
          // in examineNode()
          child->decLocalIndex();
        }
      }
...
void TR::LocalDeadStoreElimination::eliminateDeadObjectInitializations() {
  ...
  for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop())
    findLocallyAllocatedObjectUses(usedLocalObjectSymbols, NULL, -1,
                                   tt->getNode(), visitCount);
  ...
  for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    if (storeNode && storeNode->getOpCode().isStoreIndirect() &&
      ...
      if (storeNode->getFirstChild()->getOpCode().hasSymbolReference() &&
          ...
          (storeNode->getFirstChild()
               ...
               ->getOpCodeKind() == TR::New) &&
          !usedLocalObjectSymbols.get(storeNode->getFirstChild()
                                          ->getSymbolReference()
                                          ->getSymbol()
                                          ->getLocalIndex()))
        ...
        if (valueNode->isConstZeroValue()) {
          ...
          // dumpOptDetails(comp(), "Removing new %p because of cause node
          // %p\n", currentNewNode, node);
    ...
    if (examineNewUsesForKill(node, storeNode, &currentNews, &removedNews, NULL,
                              -1, visitCount)) {
      if (!removableZeroStore) {
        currentNews.remove(currentNewNode);
        if (!removedNews.find(currentNewNode)) removedNews.add(currentNewNode);
        ...
        if (initInfo->zeroInitSlots || (initInfo->numZeroInitSlots > 0)) {
          ...
          if (storeNode->getFirstChild()->getOpCode().isArrayRef()) {
            if (TR::Compiler->target.is64Bit()) {
              if (storeNode->getFirstChild()->getSecondChild()->getLongInt() >
                  INT_MAX)
                removableZeroStore = false;
              else
                offset = storeNode->getSymbolReference()->getOffset() +
                         (int32_t)storeNode->getFirstChild()
                             ->getSecondChild()
                             ->getLongInt() -
    ...
    if ((removableZeroStore || removableLocalObjectStore) &&
        performTransformation(
            comp(), "%sRemoving Dead Store : %s [0x%p]\n", optDetailString(),
...
void TR::LocalDeadStoreElimination::findLocallyAllocatedObjectUses(
    LDSBitVector &usedLocalObjectSymbols, TR::Node *parent, int32_t childNum,
  ...
  if (node->getOpCode().hasSymbolReference() &&
      ...
      !(parent->getOpCode().isStoreIndirect() && childNum == 0 &&
        ((uint32_t)parent->getSymbolReference()->getOffset() <
         fe()->getObjectHeaderSizeInBytes())))
    usedLocalObjectSymbols.set(
  ...
  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i)
    findLocallyAllocatedObjectUses(usedLocalObjectSymbols, node, i,
                                   node->getChild(i), visitCount);
}

bool TR::LocalDeadStoreElimination::examineNewUsesForKill(
    TR::Node *node, TR::Node *storeNode, List<TR::Node> *currentNews,
    List<TR::Node> *removedNews, TR::Node *parent, int32_t childNum,
    vcount_t visitCount) {
  ...
  if (currentNews->find(newNode) && ((((parent->getOpCode().isIndirect() ||
    ...
    if ((childNum == 0) && (storeNode == parent))
      ...
      if (!removedNews->find(newNode)) removedNews->add(newNode);
      // dumpOptDetails(comp(), "Removing new %p because of cause node %p\n",
      // newNode, node);
    }
  }
  ...
  //
  for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
    if (examineNewUsesForKill(node->getChild(i), storeNode, currentNews,
                              removedNews, node, i, visitCount))
      result = true;
  }
  return result;
}
...
void TR::LocalDeadStoreElimination::killStoreNodes(TR::Node *node) {
  for (auto it = _storeNodes->begin(); it != _storeNodes->end(); ++it) {
    ...
    if (storeNode && node->getSymbolReference()->sharesSymbol()) {
      ...
      if (node->getSymbolReference()->getUseDefAliases().contains(storeSymRef,
                                                                  comp()))
        *it = NULL;
    }
  }
