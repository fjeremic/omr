...
int32_t TR_LoadExtensions::perform()
{
  ...
  excludedNodes
    = new (stackMemoryRegion) NodeToIntTable(NodeToIntTableComparator(), NodeToIntTableAllocator(stackMemoryRegion));
...
const bool TR_LoadExtensions::isSupportedType(TR::Node* node) const
{
  ...
  if (!TR::comp()->cg()->getAccessStaticsIndirectly() && node->getOpCode().isLoadDirect()
    && node->getOpCode().hasSymbolReference() && node->getSymbol()->isStatic() && !node->getOpCode().isInt()
    && !node->getOpCode().isLong()) {
    result = false;
  }
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
              ...
              if (defRegLoad != NULL) {
                ...
                if (defRegLoad->getOpCode().isStoreReg() && isSupportedType(defRegLoadChild)
                  ...
                  if (trace()) {
                    traceMsg(comp(),
                      "\t\tPeeked through use %s [%p] and found def %s [%p] with child %s [%p] - Counting [%p]\n",
                      useRegLoad->getOpCode().getName(), useRegLoad, defRegLoad->getOpCode().getName(), defRegLoad,
                  ...
                  if (trace()) {
                    traceMsg(comp(),
                      "\t\tPeeked through use %s [%p] and found def %s [%p] with child %s [%p] - Excluding [%p]\n",
                      useRegLoad->getOpCode().getName(), useRegLoad, defRegLoad->getOpCode().getName(), defRegLoad,
                      defRegLoadChild != NULL ? defRegLoadChild->getOpCode().getName() : "NULL", defRegLoadChild,
                      parent);
                  }
  ...
  if (isSupportedType(parent) && parent->getOpCode().isConversion()) {
    ...
    if (isSupportedType(child)) {
      if (parent->getSize() == child->getSize()) {
        ...
        if (childOpCode.isLoadReg() && !(parent->getSize() > 4 && TR::Compiler->target.is32Bit())
            ...
            if (useDefInfo->getUseDef(info, useRegLoad->getUseDefIndex())) {
              ...
              for (cursor.SetToFirstOne(); cursor.Valid() && canSkipConversion; cursor.SetToNextOne()) {
                ...
                if (defRegLoad != NULL) {
                  ...
                  if (trace()) {
                    traceMsg(comp(),
                      "\t\tPeeked through %s [%p] and found %s [%p] with child %s [%p] - conversion %s be skipped\n",
                      useRegLoad->getOpCode().getName(), useRegLoad, defRegLoad->getOpCode().getName(), defRegLoad,
                      defRegLoadChild->getOpCode().getName(), defRegLoadChild, canSkipConversion ? "can" : "cannot");
                  }
                }
              }
              ...
              if (canSkipConversion
                ...
                TR::DebugCounter::incStaticDebugCounter(comp(),
                  TR::DebugCounter::debugCounterName(
                    comp(), "codegen/LoadExtensions/success/unneededConversion/GRA/%s", comp()->signature()));
                ...
                if (parent->getType().isInt64() && parent->getSize() > child->getSize()) {
                  if (trace()) {
                    traceMsg(comp(),
                      "\t\t\tSet global register %s in getExtendedToInt64GlobalRegisters for child %s [%p] with parent "
                      "node %s [%p]\n",
                  ...
                  // getExtendedToInt64GlobalRegisters is used by the evaluators to force a larger virtual register to
                  // be used when evaluating the regload so any instructions generated by local RA are the correct size
                  // to preserve the upper bits
