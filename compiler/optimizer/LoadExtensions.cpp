...
const bool TR_LoadExtensions::canSkipConversion(TR::Node* conversion, TR::Node* child, bool& forceExtension)
{
    ...
    if (isSupportedType(child) && excludedNodes->count(child) == 0) {
        ...
        if (isSupportedLoad(child) &&
            ...
            ((loadPrefersSignExtension && loadPrefersSignExtension == conversionOpCode.isSignExtension()) || (loadPrefersZeroExtension && loadPrefersZeroExtension == conversion->isZeroExtension()))) {
            if (trace()) {
...
const bool TR_LoadExtensions::isSupportedType(TR::Node* node) const
{
    ...
    if (!TR::comp()->cg()->getAccessStaticsIndirectly() && node->getOpCode().isLoadDirect() && node->getOpCode().hasSymbolReference() && node->getSymbol()->isStatic() && !node->getOpCode().isInt() && !node->getOpCode().isLong()) {
        result = false;
    }
