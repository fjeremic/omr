...
TR_RedundantAsyncCheckRemoval::processNaturalLoop(TR_RegionStructure *region,
  ...
  if (!isShortRunning) {
    ...
    if (GET_ASYNC_INFO(region->getEntry())->getCoverage() == FullyCovered) {
      if (trace())
        traceMsg(comp(), "Region is completely covered.  No need to perform "
                         "POSet analysis.\n");
...
TR_LoopEstimator::getEntryValue(TR::Block *block, TR::SymbolReference *symRef,
  ...
  if (!entryTree) {
    if (symRef->getSymbol()->isParm()) {
      ...
      TR_ASSERT(entryTree, "unintialized local discovered.  Uninitialized "
                           "locals cannot exist in Java");
