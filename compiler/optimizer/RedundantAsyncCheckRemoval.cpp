...
static TR_RegionStructure *getOuterImproperRegion(TR_Structure *region) {
  if (region->getParent() == 0) return 0;
  TR_RegionStructure *parent = region->getParent()->asRegion();
  if (parent->containsInternalCycles()) return parent;
...
TR_RedundantAsyncCheckRemoval::TR_RedundantAsyncCheckRemoval(
...
int32_t TR_RedundantAsyncCheckRemoval::perform() {
  ...
  if (trace()) comp()->dumpMethodTrees("Before analysis:");
  ...
  // If this is a large acyclic method - add a yield point at each return from
  // this method so that sampling will realize that we are actually in this
  // method.
  //
  ...
  if (comp()->getMethodHotness() <= warm || !comp()->mayHaveLoops() ||
    ...
    static uint32_t numNodesInLargeMethod =
        (p = feGetEnv("TR_LargeMethodNodes")) ? atoi(p)
                                              : NUMBER_OF_NODES_IN_LARGE_METHOD;
    if (!p && 0) {
      if (comp()->getMethodHotness() <= warm && !comp()->mayHaveLoops())
    ...
    if ((uint32_t)comp()->getNodeCount() > numNodesInLargeMethod ||
      ...
      _numAsyncChecksInserted +=
          TR_AsyncCheckInsertion::insertReturnAsyncChecks(
              this, "redundantAsyncCheckRemoval/returns");
    }
  ...
  //
  if ((comp()->getMethodHotness() <
       scorching) &&  // make sure we don't add unnecessary checks at scorching
      (comp()->getLoopWasVersionedWrtAsyncChecks() ||
    ...
    _numAsyncChecksInserted += TR_AsyncCheckInsertion::insertReturnAsyncChecks(
        this, "redundantAsyncCheckRemoval/returns");
  }

  if (trace()) comp()->dumpMethodTrees("After analysis:");

  return count;
}

int32_t TR_RedundantAsyncCheckRemoval::perform(TR_Structure *str,
                                               bool insideImproperRegion) {
  TR_RegionStructure *region = str->asRegion();
  if (region == 0) return processBlockStructure(str->asBlock());
  ...
  if (region->containsInternalCycles()) {
    ...
    if (origAsyncCheckFlag) _asyncCheckInCurrentLoop = true;
    return retValue;
  }

  bool asyncCheckFlag = false;
  if (region->isNaturalLoop()) _asyncCheckInCurrentLoop = false;
  ...
  for (node = it.getFirst(); node != 0; node = it.getNext()) {
    ...
    if (_asyncCheckInCurrentLoop) asyncCheckFlag = true;
...
int32_t TR_RedundantAsyncCheckRemoval::processBlockStructure(
    TR_BlockStructure *b) {
  // We remove all explicit async checks from the block.  All calls other than
  // INL calls cause a yield point as well.  We must ignore unresolved calls as
  // well because they might resolve to INL calls.
  //
  ...
  // Induction Step: Assume that the caller has correct yield points in it (it
  // is safe).
  //   If you are returning to the caller, you need not do a yield, since you
  //   know that the caller will do it.
  //
  // By Induction, on the call depth, omitting yield points on paths that return
  // is safe.
  //
  ...
  for (treeTop = firstBlockEntry; treeTop != block->getExit();
    ...
    if (node->getOpCodeValue() == TR::BBStart) {
      ...
      TR_RegionStructure *outerImproperRegion =
          getOuterImproperRegion(curBlock->getStructureOf());
      TR_Structure *outerLoop = curBlock->getStructureOf()->getContainingLoop();

      if ((node->getVisitCount() == comp()->getVisitCount()) ||
          (outerImproperRegion &&
           (!outerLoop || outerLoop->contains(outerImproperRegion))) ||
          (curBlock->getStructureOf()->getContainingLoop() !=
           b->getContainingLoop())) {
    ...
    if (info->canHaveAYieldPoint() && containsImplicitInternalPointer(node)) {
      ...
      // where there were no gc-points before.  We cannot add a async check is
      // such regions of code - since it will invalidate that optimization.
      // Instead we recognize portions of extended basic blocks as "grey-areas"
      // where its going to be dangerous to put a new async check.
      //
      ...
      switch (node->getOpCodeValue()) {
        case TR::asynccheck:
          ...
          if (performTransformation(comp(),
                                    "%sremoving async check from block_%d\n",
        ...
        default:
          if (node->getOpCode().isReturn()) hasAYieldPoint = true;
          break;
      }
    }
  }
  ...
  if (hasAYieldPoint) info->setHardYieldPoint();

  return 0;
}

bool TR_RedundantAsyncCheckRemoval::callDoesAnImplicitAsyncCheck(
    TR::Node *callNode) {
  ...
  // This covers jitProfileValue, singlePrecisionSQRT and
  // currentTimeMaxPrecision
  if (symbol->isHelper()) return false;
  ...
  if (symbol->isNative() &&
      ((symbol->getRecognizedMethod() ==
        TR::sun_misc_Unsafe_compareAndSwapInt_jlObjectJII_Z) ||
       (symbol->getRecognizedMethod() ==
        TR::sun_misc_Unsafe_compareAndSwapLong_jlObjectJJJ_Z) ||
       (symbol->getRecognizedMethod() ==
        TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z)))
    return false;

  if ((symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPPerformHysteresis) ||
      (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPUseDFP) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPHWAvailable) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPIntConstructor) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPLongConstructor) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPLongExpConstructor) ||
      ...
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPScaledAdd) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPScaledSubtract) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPScaledMultiply) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPScaledDivide) ||
      ...
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPCompareTo) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPSignificance) ||
      (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPExponent) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPBCDDigits) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPUnscaledValue) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPConvertPackedToDFP) ||
      (symbol->getRecognizedMethod() ==
       TR::java_math_BigDecimal_DFPConvertDFPToPacked))
    return false;
  if ((symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_JITIntrinsicsEnabled) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_DFPFacilityAvailable) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_DFPUseDFP) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_DFPConvertPackedToDFP) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_DFPConvertDFPToPacked) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_createZeroBigDecimal) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_getlaside) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_setlaside) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_getflags) ||
      (symbol->getRecognizedMethod() ==
       TR::com_ibm_dataaccess_DecimalData_setflags))
    return false;
#endif
  return true;
}

int32_t TR_RedundantAsyncCheckRemoval::processAcyclicRegion(
    TR_RegionStructure *region) {
  // We do not need to look inside an acyclic region, if it is
  // not inside a loop.
  //
  if (getOuterLoop(region) == 0) return 0;
  ...
  if (GET_ASYNC_INFO(region->getEntry())->getCoverage() != FullyCovered) {
    ...
    for (node = it.getFirst(); node; node = it.getNext()) {
      ...
      if (info->hasYieldPoint()) markAncestors(node, region->getEntry());
    }
...
// Recursively propagates coverage information about nodes in a loop or acylic
// region
//
void TR_RedundantAsyncCheckRemoval::computeCoverageInfo(
  ...
  if (!info->hasYieldPoint()) {
    ...
    for (auto edge = node->getSuccessors().begin();
         edge != node->getSuccessors().end(); ++edge) {
...
bool TR_RedundantAsyncCheckRemoval::isMaxLoopIterationGuardedLoop(
  ...
  TR_StructureSubGraphNode *node =
      parent->findSubNodeInRegion(loop->getNumber());
  if (!(node->getPredecessors().size() == 1)) return false;
  TR_StructureSubGraphNode *pred =
      node->getPredecessors().front()->getFrom()->asStructureSubGraphNode();

  // If parent is an improper region, give up
  //
  if (parent->containsInternalCycles()) return false;

  // The following is a rather intricate structure walk, and requires some
  // explanation we start at the node representing the loop in the parent
  // region.  What we want to do is to find a dominating if-block that contains
  // the max-loop-iteration guard for this loop.  Traditionally, the guard is
  // not the parent region, but infact there are several other versioning guards
  // intervening between the loop and the iter guard.  Each of these guards
  // forms its own acyclic region, nesting the inner guards.
  //
  // We start the search in the parent region, walking the predecessors of the
  // loop region. By definition there must exist an acyclic path between the
  // loop and the guard.  Ignore backedges and just walk the preds.  There
  // cannot be a region structure en route since that would imply that there was
  // some control flow not typical of guarded versioned loops.  If we hit the
  // entry of the parent region, we start the same process in the parent of the
  // parent region, until we hit the start-node, which implies failure.
  //
  // Each node en route must have exactly one predecessor, otherwise we have
  // control flow not typical of a versioned loop.
  //
  // The walk below uses a queue for traversal just to look like a canonical
  // backwards reverse post order walk, however there is never more than one
  // element in the queue. NOTE: there is no use of visit counts in this loop, a
  // node can never be visited twice in the following walk
  //
  ...
  while (!q.isEmpty()) {
    ...
    //
    if (cursor->getStructure()->asRegion()) return false;

    // Check the cursor block
    //
    TR::Block *block = cursor->getStructure()->asBlock()->getBlock();
    if (block == _cfg->getStart()->asBlock()) return false;

    TR::TreeTop *tt = block->getLastRealTreeTop();
    if (tt->getNode()->isMaxLoopIterationGuard()) return true;

    // if we have hit the entry of the parent, we need to start looking in the
    // parent's parent
    //
    TR_RegionStructure *cursorParent =
        cursor->getStructure()->getParent()->asRegion();
    if (cursor == cursorParent->getEntry()) {
      ...
      //
      if (cursorParent->isNaturalLoop()) return false;

      // if we hit the start of the cfg, we have failed
      //
      if (!cursorParent->getParent()) return false;
      cursorParent = cursorParent->getParent()->asRegion();

      // If the parent is improper, give up
      //
      if (cursorParent->containsInternalCycles()) return false;
    ...
    //
    if (!(cursor->getPredecessors().size() == 1)) return false;
    TR_StructureSubGraphNode *pred =
        cursor->getPredecessors().front()->getFrom()->asStructureSubGraphNode();
    q.enqueue(pred);
  }
...
bool TR_RedundantAsyncCheckRemoval::containsImplicitInternalPointer(
    TR::Node *node) {
  ...
  if (node->getOpCode().isArrayRef() && node->getReferenceCount() > 1 &&
      (!comp()->cg()->supportsInternalPointers() ||
       !node->isInternalPointer() || !node->getPinningArrayPointer())) {
    result = true;
  } else {
...
void TR_RedundantAsyncCheckRemoval::markExtendees(TR::Block *block,
                                                  bool canHaveAYieldPoint) {
  for (TR::Block *cursor = block->getNextBlock();
    ...
    if (trace()) {
      traceMsg(comp(), "    block_%d canHaveAYieldPoint %s -> %s\n",
               cursor->getNumber(), ai->canHaveAYieldPoint() ? "true" : "false",
...
#define FIND_LOOP_ITERATIONS(Type, Name, NAME)              \
  ...
  if (incr == 0) continue;                                  \
  if (entryVal && exitVal && entryVal->as##Name##Const() && \
      exitVal->as##Name##Const()) {                         \
    ...
    continue;                                               \
  diff = in - out;                                          \
  if (diff == TR::getMinSigned<NAME>()) continue;           \
  iters = (diff < 0) ? -diff / incr : diff / -incr;

int32_t TR_RedundantAsyncCheckRemoval::estimateLoopIterations(
    TR_RegionStructure *loop) {
  int32_t estimate = INT_MAX;

  TR_InductionVariable *indVar;
  for (indVar = loop->getFirstInductionVariable(); indVar != 0;
    ...
    if (incrVal->asLongConst() || (entryVal && entryVal->asLongConst()) ||
        (exitVal && exitVal->asLongConst())) {
      FIND_LOOP_ITERATIONS(int64_t, Long, TR::Int64);

      if (iters < estimate) estimate = (int32_t)iters;
    } else {
      FIND_LOOP_ITERATIONS(int32_t, Int, TR::Int32);

      if (iters < estimate) estimate = iters;
    }
  }
...
int32_t TR_RedundantAsyncCheckRemoval::findShallowestCommonCaller(
    int32_t callSiteIndex1, int32_t callSiteIndex2) {
  while ((callSiteIndex1 != -1) && (callSiteIndex1 != -1) &&
         (callSiteIndex1 != callSiteIndex2)) {
    if (callSiteIndex1 > callSiteIndex2)
      callSiteIndex1 = comp()
                           ->getInlinedCallSite(callSiteIndex1)
                           ._byteCodeInfo.getCallerIndex();
    else
      callSiteIndex2 = comp()
                           ->getInlinedCallSite(callSiteIndex2)
                           ._byteCodeInfo.getCallerIndex();
  }
  if (callSiteIndex1 == callSiteIndex2)
...
bool TR_RedundantAsyncCheckRemoval::originatesFromShortRunningMethod(
    TR_RegionStructure *region) {
  // Loops originating from java/lang/String methods are assumed to be short
  // running unless they contain loops that are inlined from methods outsides
  // the above classes. The following is an algorithm to detect such loops. In
  // the algorithm, we assume that if all branches of all basic blocks of a loop
  ...
  for (TR::Block *block = blocksIt.getCurrent(); block;
    ...
    if (lastTreeTop != block->getEntry() &&
        lastTreeTop->getNode()->getOpCode().isBranch())
  ...
  int32_t commonCaller =
      branchNodeIt.getFirst()->getByteCodeInfo().getCallerIndex();
  for (TR::Node *branchNode = branchNodeIt.getNext(); branchNode;
       branchNode = branchNodeIt.getNext()) {
    commonCaller = findShallowestCommonCaller(
        commonCaller, branchNode->getByteCodeInfo().getCallerIndex());
  }

  while ((commonCaller != -1) && !comp()->isShortRunningMethod(commonCaller))
    commonCaller =
        comp()->getInlinedCallSite(commonCaller)._byteCodeInfo.getCallerIndex();
  if (commonCaller == -1) return false;
  ...
  for (TR::Node *branchNode = branchNodeIt.getFirst(); branchNode;
       branchNode = branchNodeIt.getNext()) {
    ...
    while (callerIndex != -1) {
      ...
      if (!comp()->isShortRunningMethod(callerIndex) &&
          TR::Compiler->mtd.hasBackwardBranches(
              (TR_OpaqueMethodBlock *)ics._vmMethodInfo))
        break;
      // set callerIndex to its caller
      callerIndex = comp()
                        ->getInlinedCallSite(callerIndex)
                        ._byteCodeInfo.getCallerIndex();
    }
...
bool TR_RedundantAsyncCheckRemoval::hasEarlyExit(TR_RegionStructure *region) {
  ...
  for (TR::CFGEdge *edge2 = eit2.getCurrent(); edge2 != 0;
       edge2 = eit2.getNext()) {
    TR_StructureSubGraphNode *pred2 =
        edge2->getFrom()->asStructureSubGraphNode();
    bool earlyExit = true;
...
int32_t TR_RedundantAsyncCheckRemoval::processNaturalLoop(
    TR_RegionStructure *region, bool isInsideImproperRegion) {
  if (trace())
  ...
  if (!isInsideImproperRegion) {
    ...
    for (auto edge = entryBlock->getPredecessors().begin();
         (edge != entryBlock->getPredecessors().end()) && !isShortRunning;
         ++edge) {
      if ((*edge)->getCreatedByTailRecursionElimination()) {
        isShortRunning = true;
        if (trace())
          traceMsg(comp(),
                   "Loop %d was created by TailRecursionElim.  Skipping\n",
                   region->getNumber());
      }
    }

    if ((comp()->getMethodHotness() == scorching) &&
        originatesFromShortRunningMethod(region)) {
      isShortRunning = true;
      if (trace())
        traceMsg(comp(),
                 "Loop %d originates from a trusted method, and therefore, is "
                 "tagged as short running. Skipping\n",
                 region->getNumber());
    }
    ...
    // Spill Loops generated by the General Loop Unroller do not need any async
    // checks either since they are guaranteed to be short running
    //
    ...
    if (!isShortRunning &&
        (estimateLoopIterations(region) < SHORT_RUNNING_LOOP_BOUND)) {
      isShortRunning = true;
      if (trace())
  ...
  if (!isShortRunning) {
    ...
    if (GET_ASYNC_INFO(region->getEntry())->getCoverage() == FullyCovered) {
      if (trace())
        traceMsg(comp(),
                 "Region is completely covered.  No need to perform POSet "
                 "analysis.\n");
      ...
      if (_ancestors.isEmpty()) {
        ...
        for (auto edge = region->getEntry()->getPredecessors().begin();
          ...
          if (!info->canHaveAYieldPoint() ||
              (pred->getStructure()->asRegion() &&
               pred->getStructure()->asRegion()->isNaturalLoop() &&
        ...
        if (placeInEntry) {
          ...
          TR_ASSERT(
              GET_ASYNC_INFO(region->getEntry())->getCoverage() == NotCovered,
        ...
        if (trace()) {
          ...
          for (node = ait.getFirst(); node; node = ait.getNext()) {
            ...
            for (relative = nit.getFirst(); relative;
                 relative = nit.getNext()) {
    ...
    if (hasEarlyExit(region)) {
      if (trace())
        traceMsg(comp(),
                 "found earlyExit in region %d, so cannotClaimFullCoverage\n",
                 region->getNumber());
    ...
    if (!isInsideImproperRegion) _foundShortRunningLoops = true;
  }
...
TR_RedundantAsyncCheckRemoval::findSmallestAncestor() {
...
static TR_StructureSubGraphNode *findNodeInHierarchy(TR_RegionStructure *region,
                                                     int32_t num) {
  if (!region) return 0;
  TR_RegionStructure::Cursor it(*region);
  TR_StructureSubGraphNode *node;
...
void TR_RedundantAsyncCheckRemoval::insertAsyncCheckOnSubTree(
  ...
  if (info->getCoverage() == FullyCovered) return;

  TR_RegionStructure *outerLoop =
      getOuterLoop(entry->getStructure()->getParent()->asRegion());
  ...
  for (auto edge = node->getSuccessors().begin();
    ...
    if (!succ->getStructure()) {
      ...
      //
      if (outerLoop == 0) continue;

      TR_StructureSubGraphNode *realNode = findNodeInHierarchy(
          entry->getStructure()->getParent()->asRegion(), succ->getNumber());
...
void TR_RedundantAsyncCheckRemoval::getNearestAncestors(
    TR_StructureSubGraphNode *node, TR_StructureSubGraphNode *current,
    TR_StructureSubGraphNode *entry) {
  //   printf("node %d, current %d\n", node->getNumber(), current->getNumber());
  if (current == entry || node == entry) return;

  for (auto edge = current->getPredecessors().begin();
       edge != current->getPredecessors().end(); ++edge) {
    TR_StructureSubGraphNode *pred =
...
void TR_RedundantAsyncCheckRemoval::markAncestors(
    TR_StructureSubGraphNode *node, TR_StructureSubGraphNode *entry) {
  return;  // Disable it for performance. For more details, refer to
           // https://github.com/eclipse/omr/pull/1138
  ...
  for (auto edge = node->getPredecessors().begin();
       edge != node->getPredecessors().end(); ++edge) {
    TR_StructureSubGraphNode *pred =
        toStructureSubGraphNode((*edge)->getFrom());
    ...
    //
    // FIXME: this can be improved slightly - if its a block which can have a
    // yield point in it - and the next block cannot have a yield point in it -
    // then mark it even if it is not a branch node
    //
    if (info->getCoverage() == PartiallyCovered &&
        !(pred->getSuccessors().size() == 1) && !info->isAncestor() &&
        info->canHaveAYieldPoint()) {
      bool canMakeAncestor = true;
      for (auto succEdge = pred->getSuccessors().begin();
           succEdge != pred->getSuccessors().end(); ++succEdge) {
        TR_StructureSubGraphNode *succ =
    ...
    if (trace())
      traceMsg(comp(),
               "<===markAncestors recursion=== ssg pred: %d, ssg entry: %d\n",
...
int32_t TR_RedundantAsyncCheckRemoval::processImproperRegion(
    TR_RegionStructure *region) {
  // We are conservative with improper regions.  We do not attempt to find an
  // optimal solution for an improper region, and leave it in its original form,
  // hence being conservatively correct.  However, any inner loops of the region
  // are analyzed normally: they execute an AC definitely, before and after the
  // transformation, so we should be correct.
  //
...
void TR_RedundantAsyncCheckRemoval::enqueueSinks(
    TR_RegionStructure *region, TR_Queue<TR_StructureSubGraphNode> *q,
    bool in) {
  if (region->isAcyclic()) {
    ...
    for (node = ni.getFirst(); node; node = ni.getNext()) {
      ...
      for (auto edge = node->getSuccessors().begin();
           !seenInternalEdge && (edge != node->getSuccessors().end()); ++edge) {
        if (!region->isExitEdge(*edge)) seenInternalEdge = true;
      }
...
// Performs a linear backwards AC redundancy analysis using a flow algorithm to
// BFS, with the only difference that we analyzed a node only after all its
// successors have been analyzed
//
bool TR_RedundantAsyncCheckRemoval::performRegionalBackwardAnalysis(
    TR_RegionStructure *region, bool inInfo) {
  bool earlyExit = hasEarlyExit(region);

  TR_Queue<TR_StructureSubGraphNode> *q =
      new (trHeapMemory()) TR_Queue<TR_StructureSubGraphNode>(trMemory());
  ...
  // originates.  In an acylic region, a sink is a node with only exit
  // successors.
  //
  ...
  while (!q->isEmpty()) {
    ...
    if (node->getStructure()->hasBeenAnalyzedBefore()) continue;

    // We can process a node only after all its successors have been analyzed
    //
    bool hasUnanalyzedChildren = false;
    for (auto edge = node->getSuccessors().begin();
         edge != node->getSuccessors().end() && !hasUnanalyzedChildren;
         ++edge) {
      TR_StructureSubGraphNode *succ =
          toStructureSubGraphNode((*edge)->getTo());
      if (!succ->getStructure() || succ == region->getEntry()) continue;
      if (!succ->getStructure()->hasBeenAnalyzedBefore())
        hasUnanalyzedChildren = true;
    }
    if (hasUnanalyzedChildren) continue;
    ...
    bool backedge = false;
    for (auto edge = node->getSuccessors().begin();
         edge != node->getSuccessors().end() && !notCoveredOnSomePaths;
         ++edge) {
      TR_StructureSubGraphNode *succ =
          toStructureSubGraphNode((*edge)->getTo());
      ...
      if (succ == region->getEntry())
        ...
        TR_ASSERT(succ->getStructure()->hasBeenAnalyzedBefore(),
                  "unvisited successor encountered");
    ...
    if (coveredOnSomePaths &&
        !notCoveredOnSomePaths)  // i.e covered on all paths
      in = true;
    ...
    if (region && region->isAcyclic())
      out =
          performRegionalBackwardAnalysis(node->getStructure()->asRegion(), in);
    else if (region && region->getAnalysisInfo() &&
             ((AsyncInfo *)region->getAnalysisInfo())->hasYieldPoint() &&
             GET_ASYNC_INFO(region->getEntry())->getReverseCoverageInfo())
      out = true;  // natural loop (except for short running) or improper region

    // If this is a block, is covered on all in paths, and has a soft yield
    // point, we can remove the soft yield point.
    //
    AsyncInfo *info = GET_ASYNC_INFO(node);
    ...
    node->getStructure()->setAnalyzedStatus(
        true);  // mark it as visited (black)
...
void TR_RedundantAsyncCheckRemoval::solidifySoftAsyncChecks(
    TR_StructureSubGraphNode *node) {
  ...
  if (b) {
    if (GET_ASYNC_INFO(node)->hasSoftYieldPoint()) {
      if (performTransformation(comp(), "%sinserted async check in block_%d\n",
                                OPT_DETAILS, b->getNumber())) {
        TR::Block *block = b->getBlock();
        TR_AsyncCheckInsertion::insertAsyncCheck(
            block, comp(), "redundantAsyncCheckRemoval/solidify");
        _numAsyncChecksInserted++;
      }
    }
  } else {
    ...
    if (region->isAcyclic()) {
      if (GET_ASYNC_INFO(node) && GET_ASYNC_INFO(node)->hasSoftYieldPoint()) {
        ...
        if (performTransformation(
                comp(),
                "%sinserted async check in acyclic region entry block %d\n",
                OPT_DETAILS, entryBlock->getNumber())) {
          TR_AsyncCheckInsertion::insertAsyncCheck(
              entryBlock, comp(), "redundantAsyncCheckRemoval/solidify");
          _numAsyncChecksInserted++;
        }
      }

      TR_RegionStructure::Cursor it(*region);
      for (TR_StructureSubGraphNode *subNode = it.getFirst(); subNode;
           subNode = it.getNext())
        if (subNode->getStructure()) solidifySoftAsyncChecks(subNode);
    }
  }
}
...
uint32_t TR_LoopEstimator::estimateLoopIterationsUpperBound() {
  ...
  for (TR::CFGEdge *edge = eit.getCurrent(); edge != 0; edge = eit.getNext()) {
    ...
    if (isRecognizableExitEdge(edge, &op, &symRef, &kind, &limit)) {
      ...
      if (trace())
        traceMsg(comp(),
                 "found candidate symbol #%d (%d) in condition block_%d\n",
                 refNum, symRef->getSymbol()->getLocalIndex(),
  ...
  // iterations.  An increaing loop may have two conditions, say (i>20) and
  // (i>40). Our analysis does not tell us which one will be executed, or when.
  ...
  // at the rest of the loop controlling variables, else, simply return
  // TR::getMaxSigned<TR::Int32>() as the number of iterations. (here we are
  // being conservative as well, there might be an exit edge that causes us to
    ...
    if (candidates.isSet(refNum)) {
      ...
      if (einfo->isUnknownValue() && iinfo->getKind() != Geometric) {
        ...
        if (iinfo->getKind() == Geometric) {
          // For geometric induction variables with a well formed loop, the
          // number of loop iterations in bounded by 32.  We don't really care
          // about the limit value and the entry values.
          //

          // Get rid of the increasing infinite loop (// FIXME: is this an
          // infinite loop?)
          //
          ...
          // Get rid of the decreasing infinite loop
                   ...
                   (opCode == TR::ificmpgt || opCode == TR::ificmpge))
            estimate = TR::getMaxSigned<TR::Int32>();

          else {
            if (trace())
              traceMsg(comp(),
                       "found geometric induction variable symbol #%d\n",
                       refNum);
          ...
          // messed up induction variable info
            ...
            int32_t diff = in - lim;
            int32_t iters =
                (diff < 0) ? ceiling(-diff, incr) : ceiling(diff, -incr);

            // The above calculation is approximate, it is possible that the
            // number of iterations is calculated to be a negative number,
            // because of commoning. Ignore these cases. Usually the diff is off
            // the actual exact value by a difference of incr.
            //
            if (iters < 0) iters = 0;

            if (trace()) {
              traceMsg(comp(),
                       "loop iterations estimate based upon symbol #%d: %d\n",
                       refNum, iters);
  ...
  if (estimate == -1) return TR::getMaxSigned<TR::Int32>();

  /**
  if (estimate < TR::getMaxSigned<TR::Int32>())
     printf(">>found good estimate in loop %d (%d times) in %s\n",
  _loop->getNumber(), estimate, signature(comp()->getCurrentMethod()));
  **/

  return estimate;
}

void TR_LoopEstimator::getLoopIncrements(TR_BitVector &candidates,
                                         IncrementInfo **loopIncrements) {
  ...
  while (!queue.isEmpty()) {
    ...
    if (block->getVisitCount() == visitCount) continue;

    // Make sure the predecessors of this Block have been visited (except for
    // entry)
    //
    if (block->getNumber() != headerIndex) {
      bool notDone = false;
      TR_PredecessorIterator pit(block);
      for (TR::CFGEdge *edge = pit.getFirst(); edge && !notDone;
           edge = pit.getNext()) {
        TR::Block *pred = toBlock(edge->getFrom());
        if (pred->getVisitCount() != visitCount) notDone = true;
      }
...
void TR_LoopEstimator::processBlock(TR::Block *block,
  ...
  for (TR::TreeTop *treeTop = block->getFirstRealTreeTop();
       treeTop != exitTreeTop; treeTop = treeTop->getNextRealTreeTop()) {
    TR::Node *node = treeTop->getNode();
    if (node->getOpCode().isStoreDirect()) {
      ...
      if (candidates.isSet(refNum)) {
        ...
        if (!inSymbol)
          inSymbol = inSymbols[refIndex] =
              new (trStackMemory()) IncrementInfo(0);
...
void TR_LoopEstimator::mergeWithLoopIncrements(TR::Block *block,
                                               IncrementInfo **loopIncrements) {
...
void TR_LoopEstimator::IncrementInfo::merge(IncrementInfo *other) {
  if (other->isUnknownValue() ||
    ...
    if (_kind == Identity) _kind = other->_kind;
...
bool TR_LoopEstimator::isRecognizableExitEdge(TR::CFGEdge *edge,
                                              TR::ILOpCodes *op,
  ...
  if (subNode->getStructure()->asRegion()) return false;
  ...
  if (node->getOpCode().isJumpWithMultipleTargets()) return false;
  ...
  if (!(opValue == TR::ificmplt || opValue == TR::ificmpge ||
        opValue == TR::ificmpgt || opValue == TR::ificmple))
  ...
  if (kind != Geometric) {
    ...
    // but the number of loop iterations is bounded even when we don't the
    // limiting var.
    //
    TR::Node *secondChild = node->getSecondChild();
...
TR_LoopEstimator::IncrementInfo ***TR_LoopEstimator::getBlockInfoArray() {
  IncrementInfo ***array = (IncrementInfo ***)trMemory()->allocateStackMemory(
      _numBlocks * sizeof(IncrementInfo **));
...
TR_LoopEstimator::IncrementInfo **TR_LoopEstimator::getIncrementInfoArray() {
  IncrementInfo **array = (IncrementInfo **)trMemory()->allocateStackMemory(
      _numCandidates * sizeof(IncrementInfo *));
  memset(array, 0, _numCandidates * sizeof(IncrementInfo *));
  return array;
}

TR_LoopEstimator::EntryInfo **TR_LoopEstimator::getEntryInfoArray() {
  EntryInfo **array = (EntryInfo **)trMemory()->allocateStackMemory(
      _numBlocks * sizeof(EntryInfo *));
...
//
bool TR_LoopEstimator::isProgressionalStore(TR::Node *node,
                                            TR_ProgressionKind *kind,
                                            int32_t *incr) {
  ...
  TR::SymbolReference *symRef;
  if (!getProgression(cursor, &symRef, kind, incr)) return false;
...
//
bool TR_LoopEstimator::getProgression(TR::Node *expr, TR::SymbolReference **ref,
                                      TR_ProgressionKind *prog, int32_t *incr) {
  int32_t konst;
  TR_ProgressionKind kind;
...
TR_LoopEstimator::EntryInfo *TR_LoopEstimator::getEntryValue(
  ...
  if (nodesDone.isSet(blockIndex)) return entryInfos[blockIndex];
  ...
  if (!entryTree) {
    if (symRef->getSymbol()->isParm()) {
      entryInfos[blockIndex] = myInfo = new (trStackMemory())
          EntryInfo();  // parameters have unintialized values
    } else
      TR_ASSERT(entryTree,
                "unintialized local discovered.  Uninitialized locals cannot "
                "exist in Java");
    ...
    if (node->getOpCode().isStoreDirect()) {
      ...
      if (ref->getReferenceNumber() == symRef->getReferenceNumber()) {
        if (node->getFirstChild()->getOpCode().isLoadConst())
          entryInfos[blockIndex] = myInfo =
              new (trStackMemory()) EntryInfo(node->getFirstChild()->getInt());
        else
          entryInfos[blockIndex] = myInfo =
              new (trStackMemory()) EntryInfo();  // unknown value
      }
    }
  }
  ...
  if (myInfo == 0) {
    ...
    for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
      ...
      if (predInfo) {
        if (!myInfo)
          entryInfos[blockIndex] = myInfo =
              new (trStackMemory()) EntryInfo(predInfo);
        else
...
static bool internalEdge(TR_RegionStructure *loop, TR::Block *block) {
  ...
  for (TR_StructureSubGraphNode *node = it.getCurrent(); node;
       node = it.getNext()) {
    TR_BlockStructure *b = node->getStructure()->asBlock();
    if (!b) {
      if (internalEdge(node->getStructure()->asRegion(), block)) return true;
    } else {
...
TR_LoopEstimator::EntryInfo *TR_LoopEstimator::getEntryValueForSymbol(
    TR::SymbolReference *symRef) {
  ...
  for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
    ...
    // ignore backedges
    //
    if (internalEdge(_loop, pred)) continue;
    ...
    if (predInfo) {
      ...
      if (!myInfo) TR_ASSERT(0, "Couldn't find store for entry value");
    }
  }
...
void TR_LoopEstimator::EntryInfo::merge(EntryInfo *other) {
  if (other->_unknown)
    ...
    if (other->_val != _val) setUnknownValue();
}
