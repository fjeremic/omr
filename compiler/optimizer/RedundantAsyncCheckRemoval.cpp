...
//
static TR_RegionStructure*
getOuterLoop(TR_RegionStructure* region)
{
...
static TR_RegionStructure*
getOuterImproperRegion(TR_Structure* region)
{
  if (region->getParent() == 0)
    return 0;
  TR_RegionStructure* parent = region->getParent()->asRegion();
...
ceiling(int32_t numer, int32_t denom)
{
...
bool
TR_RedundantAsyncCheckRemoval::shouldPerform()
{
...
int32_t
TR_RedundantAsyncCheckRemoval::perform()
{
...
int32_t
TR_RedundantAsyncCheckRemoval::perform(TR_Structure* str,
...
//
void
TR_RedundantAsyncCheckRemoval::initialize(TR_Structure* str)
{
...
int32_t
TR_RedundantAsyncCheckRemoval::processBlockStructure(TR_BlockStructure* b)
{
    ...
    if (!stopTree) {
      switch (node->getOpCodeValue()) {
        case TR::asynccheck:
          ...
          if (trace())
            traceMsg(
              comp(), "removing async check from block_%d\n", b->getNumber());
...
bool
TR_RedundantAsyncCheckRemoval::callDoesAnImplicitAsyncCheck(TR::Node* callNode)
{
...
int32_t
TR_RedundantAsyncCheckRemoval::processAcyclicRegion(TR_RegionStructure* region)
{
...
//
void
TR_RedundantAsyncCheckRemoval::computeCoverageInfo(
  TR_StructureSubGraphNode* node,
  TR_StructureSubGraphNode* entry)
{
  ...
  if (trace())
    traceMsg(comp(),
             "for node: %d coverage: %d\n",
             node->getNumber(),
             info->getCoverage());
}

bool
TR_RedundantAsyncCheckRemoval::isMaxLoopIterationGuardedLoop(
...
TR_RedundantAsyncCheckRemoval::containsImplicitInternalPointer(TR::Node* node)
{
  ...
  if (trace())
    traceMsg(comp(),
             "    containsImplicitInternalPointer(%p) = %s\n",
             node,
             result ? "true" : "false");
  return result;
}
...
void
TR_RedundantAsyncCheckRemoval::markExtendees(TR::Block* block,
                                             bool canHaveAYieldPoint)
{
  for (TR::Block* cursor = block->getNextBlock();
    ...
    if (trace()) {
      traceMsg(comp(),
               "    block_%d canHaveAYieldPoint %s -> %s\n",
               cursor->getNumber(),
...
int32_t
TR_RedundantAsyncCheckRemoval::estimateLoopIterations(TR_RegionStructure* loop)
{
...
int32_t
TR_RedundantAsyncCheckRemoval::findShallowestCommonCaller(
  int32_t callSiteIndex1,
  int32_t callSiteIndex2)
{
...
bool
TR_RedundantAsyncCheckRemoval::originatesFromShortRunningMethod(
  TR_RegionStructure* region)
{
  ...
  for (TR::Node* branchNode = branchNodeIt.getFirst(); branchNode;
    ...
    while (callerIndex != -1) {
      ...
      callerIndex =
        comp()->getInlinedCallSite(callerIndex)._byteCodeInfo.getCallerIndex();
    }
    if (!callerMatch)
      return false;
  }
...
bool
TR_RedundantAsyncCheckRemoval::hasEarlyExit(TR_RegionStructure* region)
{
  ...
  for (TR::CFGEdge* edge2 = eit2.getCurrent(); edge2 != 0;
    ...
    for (auto edge = region->getEntry()->getPredecessors().begin();
         edge != region->getEntry()->getPredecessors().end();
         ++edge) {
    ...
    if (earlyExit) {
      if (trace())
        traceMsg(
          comp(), "found earlyExit in region %d \n", region->getNumber());
      return true;
    }
  }
...
int32_t
TR_RedundantAsyncCheckRemoval::processNaturalLoop(TR_RegionStructure* region,
                                                  bool isInsideImproperRegion)
{
  if (trace())
    traceMsg(
      comp(), "==> Forward Processing natural loop %d\n", region->getNumber());
  ...
  if (!isInsideImproperRegion) {
    ...
    if (!_asyncCheckInCurrentLoop) {
      ...
      if (trace())
        traceMsg(comp(),
                 "Loop %d is a Short running loop. Skipping\n",
                 region->getNumber());
    }
    ...
    if (!isShortRunning &&
      ...
      if (trace())
        traceMsg(
          comp(), "Loop %d is short running. Skipping\n", region->getNumber());
    }
  }
  ...
  if (trace())
    traceMsg(
      comp(), "==> Finished processing region %d\n", region->getNumber());
...
static TR_StructureSubGraphNode*
findNodeInHierarchy(TR_RegionStructure* region, int32_t num)
{
  if (!region)
    return 0;
  TR_RegionStructure::Cursor it(*region);
...
void
TR_RedundantAsyncCheckRemoval::insertAsyncCheckOnSubTree(
  TR_StructureSubGraphNode* node,
  TR_StructureSubGraphNode* entry)
{
  AsyncInfo* info = GET_ASYNC_INFO(node);
  ...
  for (auto edge = node->getSuccessors().begin();
    ...
    if (!succ->getStructure()) {
      ...
      if (block) {
        if (trace())
          traceMsg(comp(),
                   "- added exit yield point in block_%d\n",
                   block->getNumber());
...
void
TR_RedundantAsyncCheckRemoval::getNearestAncestors(
  TR_StructureSubGraphNode* node,
  TR_StructureSubGraphNode* current,
  TR_StructureSubGraphNode* entry)
{
  ...
  for (auto edge = current->getPredecessors().begin();
       edge != current->getPredecessors().end();
       ++edge) {
    TR_StructureSubGraphNode* pred =
...
void
TR_RedundantAsyncCheckRemoval::markAncestors(TR_StructureSubGraphNode* node,
                                             TR_StructureSubGraphNode* entry)
{
  ...
  if (trace())
    traceMsg(comp(),
             "<===markAncestors start=== ssg node: %d, ssg entry: %d\n",
             node->getNumber(),
             entry->getNumber());
...
int32_t
TR_RedundantAsyncCheckRemoval::processImproperRegion(TR_RegionStructure* region)
{
...
void
TR_RedundantAsyncCheckRemoval::enqueueSinks(
  TR_RegionStructure* region,
  ...
  bool in)
{
  if (region->isAcyclic()) {
    ...
    for (node = ni.getFirst(); node; node = ni.getNext()) {
      ...
      for (auto edge = node->getSuccessors().begin();
           !seenInternalEdge && (edge != node->getSuccessors().end());
           ++edge) {
...
//
bool
TR_RedundantAsyncCheckRemoval::performRegionalBackwardAnalysis(
  TR_RegionStructure* region,
  bool inInfo)
{
  ...
  if (trace())
    traceMsg(comp(),
             "<== Start processing region %d, in = %d\n",
             region->getNumber(),
             inInfo);
  ...
  while (!q->isEmpty()) {
    ...
    if (coveredOnSomePaths && !notCoveredOnSomePaths) // i.e covered on all
                                                      // paths
      in = true;
    ...
    if (!region && in == true && info->hasSoftYieldPoint()) {
      ...
      if (trace())
        traceMsg(
          comp(), "\t\tremoved yield point from node %d\n", node->getNumber());
  ...
  if (trace())
    traceMsg(comp(),
             "<== Finished processing region %d, out = %d\n",
             region->getNumber(),
             GET_ASYNC_INFO(region->getEntry())->getReverseCoverageInfo());
...
void
TR_RedundantAsyncCheckRemoval::solidifySoftAsyncChecks(
  TR_StructureSubGraphNode* node)
{
  ...
  if (b) {
    if (GET_ASYNC_INFO(node)->hasSoftYieldPoint()) {
      if (performTransformation(comp(),
                                "%sinserted async check in block_%d\n",
                                OPT_DETAILS,
                                b->getNumber())) {
        TR::Block* block = b->getBlock();
...
uint32_t
TR_LoopEstimator::estimateLoopIterationsUpperBound()
{
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  ...
  if (trace())
    traceMsg(comp(),
             "==> Begin Processing Loop %d for iteration estimate\n",
             _loop->getNumber());
    ...
    if (isRecognizableExitEdge(edge, &op, &symRef, &kind, &limit)) {
      ...
      if (trace())
        traceMsg(comp(),
                 ...
                 refNum,
                 symRef->getSymbol()->getLocalIndex(),
...
void
TR_LoopEstimator::getLoopIncrements(TR_BitVector& candidates,
...
void
TR_LoopEstimator::processBlock(TR::Block* block, TR_BitVector& candidates)
{
...
void
TR_LoopEstimator::mergeWithLoopIncrements(TR::Block* block,
                                          IncrementInfo** loopIncrements)
{
...
void
TR_LoopEstimator::IncrementInfo::merge(IncrementInfo* other)
{
...
bool
TR_LoopEstimator::isRecognizableExitEdge(TR::CFGEdge* edge,
                                         TR::ILOpCodes* op,
...
TR_LoopEstimator::IncrementInfo***
TR_LoopEstimator::getBlockInfoArray()
{
...
TR_LoopEstimator::IncrementInfo**
TR_LoopEstimator::getIncrementInfoArray()
{
  IncrementInfo** array = (IncrementInfo**)trMemory()->allocateStackMemory(
...
TR_LoopEstimator::EntryInfo**
TR_LoopEstimator::getEntryInfoArray()
{
...
//
bool
TR_LoopEstimator::isProgressionalStore(TR::Node* node,
                                       TR_ProgressionKind* kind,
                                       int32_t* incr)
{
...
//
bool
TR_LoopEstimator::getProgression(TR::Node* expr,
                                 TR::SymbolReference** ref,
                                 TR_ProgressionKind* prog,
                                 int32_t* incr)
{
...
TR_LoopEstimator::EntryInfo*
TR_LoopEstimator::getEntryValue(TR::Block* block,
                                TR::SymbolReference* symRef,
...
static bool
internalEdge(TR_RegionStructure* loop, TR::Block* block)
{
...
TR_LoopEstimator::EntryInfo*
TR_LoopEstimator::getEntryValueForSymbol(TR::SymbolReference* symRef)
{
...
void
TR_LoopEstimator::EntryInfo::merge(EntryInfo* other)
{
  if (other->_unknown)
    setUnknownValue();
  else if (!_unknown)
