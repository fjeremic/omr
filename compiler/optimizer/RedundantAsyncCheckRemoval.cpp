...
int32_t TR_RedundantAsyncCheckRemoval::perform()
{
    ...
    if ((comp()->getMethodHotness() < scorching) && // make sure we don't add unnecessary checks at scorching
        (comp()->getLoopWasVersionedWrtAsyncChecks() || (_numAsyncChecksInserted == 0 && _foundShortRunningLoops && comp()->getRecompilationInfo() &&
...
int32_t TR_RedundantAsyncCheckRemoval::processBlockStructure(TR_BlockStructure* b)
{
    ...
    for (treeTop = firstBlockEntry; treeTop != block->getExit(); treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            if ((node->getVisitCount() == comp()->getVisitCount()) || (outerImproperRegion && (!outerLoop || outerLoop->contains(outerImproperRegion))) || (curBlock->getStructureOf()->getContainingLoop() != b->getContainingLoop())) {
...
bool TR_RedundantAsyncCheckRemoval::callDoesAnImplicitAsyncCheck(TR::Node* callNode)
{
...
#ifdef J9_PROJECT_SPECIFIC
    if ((symbol->getRecognizedMethod() == TR::java_lang_Math_sqrt) || (symbol->getRecognizedMethod() == TR::java_lang_StrictMath_sqrt) || (symbol->getRecognizedMethod() == TR::java_lang_Math_max_I) || (symbol->getRecognizedMethod() == TR::java_lang_Math_min_I) || (symbol->getRecognizedMethod() == TR::java_lang_Math_max_L) || (symbol->getRecognizedMethod() == TR::java_lang_Math_min_L) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_L) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_D) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_F) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_I) || (symbol->getRecognizedMethod() == TR::java_lang_System_hiresClockImpl) || (symbol->getRecognizedMethod() == TR::java_lang_Integer_numberOfLeadingZeros) || (symbol->getRecognizedMethod() == TR::java_lang_Long_numberOfLeadingZeros) || (symbol->getRecognizedMethod() == TR::java_lang_Integer_numberOfTrailingZeros) || (symbol->getRecognizedMethod() == TR::java_lang_Long_numberOfTrailingZeros) || (symbol->getRecognizedMethod() == TR::java_lang_Integer_highestOneBit) || (symbol->getRecognizedMethod() == TR::java_lang_Long_reverseBytes) || (symbol->getRecognizedMethod() == TR::java_lang_Short_reverseBytes) || (symbol->getRecognizedMethod() == TR::java_lang_Integer_reverseBytes) ||

        (symbol->getRecognizedMethod() == TR::java_lang_Long_highestOneBit) || (symbol->getRecognizedMethod() == TR::java_lang_Integer_rotateLeft) || (symbol->getRecognizedMethod() == TR::java_lang_Long_rotateLeft) || (symbol->getRecognizedMethod() == TR::java_lang_Integer_rotateRight) || (symbol->getRecognizedMethod() == TR::java_lang_Long_rotateRight))
        return false;
    ...
    // versions or not.
    if (symbol->isNative() && ((symbol->getRecognizedMethod() == TR::sun_misc_Unsafe_compareAndSwapInt_jlObjectJII_Z) || (symbol->getRecognizedMethod() == TR::sun_misc_Unsafe_compareAndSwapLong_jlObjectJJJ_Z) || (symbol->getRecognizedMethod() == TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z)))
        return false;

    if ((symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPPerformHysteresis) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPUseDFP) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPHWAvailable) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPIntConstructor) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPLongConstructor) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPLongExpConstructor) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPAdd) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPSubtract) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPMultiply) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPDivide) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPScaledAdd) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPScaledSubtract) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPScaledMultiply) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPScaledDivide) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPRound) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPSetScale) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPCompareTo) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPSignificance) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPExponent) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPBCDDigits) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPUnscaledValue) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPConvertPackedToDFP) || (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPConvertDFPToPacked))
        return false;
    if ((symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_JITIntrinsicsEnabled) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_DFPFacilityAvailable) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_DFPUseDFP) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_DFPConvertPackedToDFP) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_DFPConvertDFPToPacked) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_createZeroBigDecimal) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_getlaside) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_setlaside) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_getflags) || (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_setflags))
        return false;
#endif
    return true;
}
...
bool TR_RedundantAsyncCheckRemoval::containsImplicitInternalPointer(TR::Node* node)
{
    ...
    bool result = false;
    if (node->getOpCode().isArrayRef() && node->getReferenceCount() > 1 && (!comp()->cg()->supportsInternalPointers() || !node->isInternalPointer() || !node->getPinningArrayPointer())) {
        result = true;
    } else {
...
bool TR_RedundantAsyncCheckRemoval::originatesFromShortRunningMethod(TR_RegionStructure* region)
{
    ...
    for (TR::Node* branchNode = branchNodeIt.getFirst(); branchNode; branchNode = branchNodeIt.getNext()) {
        ...
        while (callerIndex != -1) {
            ...
            if (!comp()->isShortRunningMethod(callerIndex) && TR::Compiler->mtd.hasBackwardBranches((TR_OpaqueMethodBlock*)ics._vmMethodInfo))
                break;
        ...
        if (needsForwardAnalysis) {
            ...
            if (_ancestors.isEmpty()) {
                ...
                for (auto edge = region->getEntry()->getPredecessors().begin(); edge != region->getEntry()->getPredecessors().end(); ++edge) {
                    ...
                    if (!info->canHaveAYieldPoint() || (pred->getStructure()->asRegion() && pred->getStructure()->asRegion()->isNaturalLoop() && (info->getCoverage() != FullyCovered))) {
                        placeInEntry = true;
                        break;
                    }
                }
...
void TR_RedundantAsyncCheckRemoval::markAncestors(TR_StructureSubGraphNode* node, TR_StructureSubGraphNode* entry)
{
    ...
    for (auto edge = node->getPredecessors().begin(); edge != node->getPredecessors().end(); ++edge) {
        ...
        //
        if (info->getCoverage() == PartiallyCovered && !(pred->getSuccessors().size() == 1) && !info->isAncestor() && info->canHaveAYieldPoint()) {
...
{
    if (other->isUnknownValue() || (_kind == Arithmetic && other->_kind == Geometric) || (_kind == Geometric && other->_kind == Arithmetic))
        _unknown = true;
    else if (!_unknown) {
        if (_kind == Identity)
            _kind = other->_kind;
