  ...
  { // list of flags
    ...
    IsAotLoad =
      0x00010000, // AOT loads have a compilation object now
                  // This flag will allow the compilation object know that it is
                  // only used for relocation, so that some of the structures
                  // that are not needed will be skipped (e.g. jittedBodyInfo,
    ...
    DontFailOnPurpose =
      0x00100000, // Mostly needed to avoid failing for the purpose of upgrading
                  // to a higher opt level
...
private:
  ...
  flags32_t _flags;
  int32_t
    _perceivedCPUUtil; // percentage*10 (so that we can use fractional values)
...
public:
  ...
  virtual bool adjustOptimizationPlan(TR_MethodToBeCompiled* entry, int32_t adj)
  {
    return false;
  }
