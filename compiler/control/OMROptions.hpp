...
enum TR_CompilationOptions {
  ...
  TR_DisableVSSStackCompaction =
      0x00100000 +
      17, // never compact variable size symbols -- if not force or disable then
          // use heuristic based on opt level and VSS total size
  ...
  TR_DisableOutlinedNew =
      0x00200000 + 18, // Expand object allocation logic inline instead of using
                       // a fast jit helper
...
enum TR_VerboseFlags {
  ...
  TR_VerboseSampling,
  TR_VerboseMMap,
  TR_VerboseCompileExclude, // prints a message if a method cannot be translated
                            // due to filters
  ...
  TR_VerboseSCHints,
  TR_VerboseCounts,
  TR_VerboseCompFailure, // print extra message when a first time compilation is
                         // going to be continue interpreted Must leave this as
                         // a separate option because sometimes the error do not
...
#define DEFAULT_ABSTRACT_TIME_SAMPLES_GRACE_PERIOD                             \
  (2 * 60 * 100) // 1200 global samples (2 minutes single threaded, 1 minute for
                 // two thread
...
public:
  ...
  static int32_t _lowCodeCacheThreshold; // Turn off Iprofiler if available code
                                         // cache space is lower than this value

  static uint32_t _memExpensiveCompThreshold; // threshold for when compilations
                                              // are considered memory hungry
  static uint32_t _cpuExpensiveCompThreshold; // threshold for when compilations
                                              // are considered cpu hungry
  ...
  void disableCHOpts(); // disable CHOpts, but also IPA and prex which depend on
                        // the chtable
...
protected:
  ...
  char *_optFileName;
  int32_t *_customStrategy; // Actually array of TR_OptimizerImpl::Optimizations
  ...
  bool
      _insertGCRTrees; // more like a flag than an option; cannot be set by user
  ...
  char *_objectFileName; // Name of the relocatable ELF file *.o if one is to be
                         // generated
...
public:
  ...
  TR_MCTLogs(int32_t compThreadID, TR::Options *options)
      : _next(NULL), _compThreadID(compThreadID), _options(options),
        _logFile(NULL) {}
...
private:
  ...
  TR::FILE *
      _logFile; ///< file descriptor for this log; comp thread will create these
  TR::Options *
      _options; ///< pointer back to the TR::Options obj that contains this list
  int32_t
      _compThreadID; ///< the ID of the compilation thread that can use this log
