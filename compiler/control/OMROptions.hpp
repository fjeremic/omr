...
/**
 * The TR_BUILD_NAME constant defines a unique name for each compiler version
 * generated by the automated build tasks. It is declared as an external
 * constant and it is defined in a make-time generated file called
 * TRBuildName.cpp. This file is generated in the generateVersion.pl script
 * which runs as part of the make process. We could also supply this definition
 * via a compiler define macro flag (-DTR_BUILD_NAME) however that would mean
 * that from one automated build to the next the command line arguments used to
 * invoke the compiler would be different. This is problematic for the use of
 * ccache to cache the object results of a compilation. This is why we had to
 * refactor this definition into a make-time generated file.
 */
...
enum TR_CompilationOptions {
  ...
  TR_CountOptTransformations =
      0x00800000,  // Must be same option word as TR_TraceOptDetails to allow
  ...
  TR_AlwaysSafeFatal =
      0x00000800 + 2,  ///< Always fire a safe fatal assert for testing purposes
  ...
  TR_EnableAnnotations =
      0x00002000 + 4,  // change to disable when on by default
  TR_UnresolvedAreNotColdAtCold =
      0x00004000 + 4,  // cold block marker marks unresolved blocks as cold at
                       // hotness cold or less
  ...
  TR_DisableUpgradingColdCompilations =
      0x00400000 + 4,  // for cold methods due to classLoadPhase for instance
  ...
  TR_EnableNewAllocationProfiling =
      0x00010000 + 5,  // enable tracing of fields load and store
  TR_IgnoreIEEERestrictions =
      0x00020000 + 5,  // enable more aggressive, nonIEEE compliant xforms
  ...
  TR_DisableLoopTransfer =
      0x00000400 + 6,  // disables the loop transfer part of loop versioner
  TR_EnableRangeSplittingGRA = 0x00000800 + 6,
  TR_ForceUsePreexistence = 0x00001000 + 6,
  TR_TLHPrefetch = 0x00002000 + 6,
  TR_ReservingLocks =
      0x00004000 + 6,  // Can be merged with TR_DisableLockResevation when lock
                       // reservation is enabled on all platforms
  ...
  TR_EnableJCLInline =
      0x00008000 + 7,  // enable JCL Integer and Long methods inline
  ...
  TR_DisableNoServerDuringStartup =
      0x04000000 + 9,  // set TR_NoOptServer during startup and insert GCR trees
  ...
  TR_ForceVSSStackCompaction =
      0x00080000 + 17,  // always compact variable size symbols
  TR_DisableVSSStackCompaction =
      0x00100000 +
      17,  // never compact variable size symbols -- if not force or disable
           // then use heuristic based on opt level and VSS total size
  TR_EnableThisLiveRangeExtension =
      0x00200000 + 17,  // "this" live range extension
  ...
  TR_DisableOutlinedNew =
      0x00200000 + 18,  // Expand object allocation logic inline instead of
                        // using a fast jit helper
  // Available                                       = 0x00400000 + 18,
  // Available                                       = 0x00800000 + 18,
  // Available                                       = 0x01000000 + 18,
  TR_TraceOpts =
      0x02000000 + 18,  // list optimization group in its dynamic order
  TR_DisableDecimalFormatPeephole =
      0x04000000 +
      18,  // optimize DecimalFormat.format(BigDecimal.doubleValue())
  ...
  TR_DisableAOTResolveDiffCLMethods =
      0x00001000 + 20,  // disable passing nested autos as parameters
  TR_DisableMethodHandleInvokeOpts = 0x00002000 + 20,  // JSR292
  TR_InhibitRecompilation = 0x00004000 + 20,
  // Available                                       = 0x00008000 + 20,
  TR_PaintDataCacheOnFree =
      0x00010000 + 20,  // Enable painting of data caches on reclamation
  TR_DisableDataCacheReclamation =
      0x00020000 + 20,  // Disable reaping of data caches
  ...
  TR_DisableWrtBarSrcObjCheck =
      0x00000080 + 21,  // enable srcobj range check for wrtBar
  TR_DisableCodeCacheReclamation =
      0x00000100 + 21,  // Disable the freeing of compiled methods
  TR_DisableClearCodeCacheFullFlag =
      0x00000200 + 21,  // Disable the re-enabling of the code cache when a
                        // method body is freed after the cache has become full
  ...
  TR_DisableMCSBypass =
      0x00000200 + 23,  // JSR292: MutableCallSite bypass optimization
...
enum TR_VerboseFlags {
  ...
  TR_VerbosePerformance,  // Print options, compilation
                          // start/end/failure/time/memory
  TR_VerboseFilters,
  TR_VerboseSampling,
  TR_VerboseMMap,
  TR_VerboseCompileExclude,  // prints a message if a method cannot be
                             // translated due to filters
  TR_VerboseLink,            // Trace linking progress on the zEmulator JIT
  TR_VerboseCLP,             // classLoadPhase info printed in vlog
  TR_VerboseGCcycle,  // Info about comp thread waiting for GC cycle to finish;
                      // only used in RTJ
  ...
  TR_VerboseSCHints,
  TR_VerboseCounts,
  TR_VerboseCompFailure,  // print extra message when a first time compilation
                          // is going to be continue interpreted Must leave this
                          // as a separate option because sometimes the error do
                          // not show when full blown verbose options are
                          // specified
  TR_VerboseJitState,
  TR_VerboseJitMemory,  // Print JIT memory usage every 5 minutes; I should add
                        // this to end of run too
  ...
  TR_VerboseJ2IThunks,
  TR_VerboseVMemAvailable,  // print virtual memory available to user at end of
                            // compilation; currntly used only for win32
  TR_VerboseCodeCache,
...
enum TR_SamplingJProfilingFlags {
  ...
  // Any new option added here must be added also to
  // _samplingJProfilingOptionNames
...
#define TR_EXCLUDED_OPTIONSET_INDEX \
  10  // Normal options sets have single-digit indexes
...
#define TR_QUICKSTART_SMALLER_INITIAL_COUNT \
  500  // Used for Windows under Xquickstart
#define TR_QUICKSTART_SMALLER_INITIAL_BCOUNT 250
#define TR_INITIAL_COLDRUN_COUNT \
  450  // we want this smaller than TR_QUICKSTART_INITIAL_COUNT
#define TR_INITIAL_COLDRUN_BCOUNT \
  125  // we want this smaller than TR_QUICKSTART_INITIAL_BCOUNT
...
#define TR_UNRESOLVED_IMPLIES_COLD_COUNT \
  200  // If a method has run this many times, we can assume unresolved symbols
       // indicate cold paths
...
#define DEFAULT_ABSTRACT_TIME_SAMPLES_GRACE_PERIOD \
  (2 * 60 * 100)  // 1200 global samples (2 minutes single threaded, 1 minute
                  // for two thread
#define DEFAULT_ABSTRACT_TIME_SAMPLES_TO_REDUCE_INLINING_AGGRESSIVENESS \
  (4 * 60 * 100)
#define DEFAULT_ABSTRACT_TIME_MS_CPU_GRACE_PERIOD \
  (2 * 60 * 1000)  // 2 minutes single threaded
#define DEFAULT_ABSTRACT_TIME_MS_CPU_TO_REDUCE_INLINING_AGGRESSIVENESS \
  (4 * 60 * 1000)
...
#else
#define JSR292_SCRATCH_SPACE_FACTOR \
  6  // Based on a need for 1.5GB and default of 256MB
#endif
...
class OMR_EXTENSIBLE Options {
 ...
 public:
  ...
  Options() : _optionSets(0), _logListForOtherCompThreads(NULL) {}
  Options(TR_Memory *, int32_t index, int32_t lineNumber,
  ...
  static TR::OptionSet *findOptionSet(TR_Memory *, int32_t index,
                                      int32_t lineNum, TR_ResolvedMethod *,
                                      TR_Hotness, bool);
  static TR::OptionSet *findOptionSet(TR_Memory *, TR_ResolvedMethod *, bool);
  static TR::OptionSet *findOptionSet(int32_t index, int32_t lineNum,
                                      const char *, TR_Hotness, bool);

  static char *getDefaultOptions();
  static char *validateOptions(void *feBase, TR_FrontEnd *fe);
  static char *processOptionsJIT(char *jitOptions, void *feBase,
                                 TR_FrontEnd *fe);
  static char *processOptionsAOT(char *aotOptions, void *feBase,
                                 TR_FrontEnd *fe);
  static char *processOptions(char *options, char *envOptions, void *feBase,
                              TR_FrontEnd *fe, TR::Options *cmdLineOptions);
  static char *latePostProcessJIT(void *jitConfig);
  static char *latePostProcessAOT(void *jitConfig);
  static char *latePostProcess(TR::Options *options, void *jitConfig,
                               bool isAOT);
  static char *processOptions(char *options, char *moreOptions,
  ...
  bool getAnyOption(uint32_t mask) {
    return (_options[mask & TR_OWM] & (mask & ~TR_OWM)) != 0;
  }
  bool getAllOptions(uint32_t mask) {
    return (_options[mask & TR_OWM] & (mask & ~TR_OWM)) == mask;
  }
  ...
  static void setSamplingJProfilingOption(TR_SamplingJProfilingFlags op) {
    _samplingJProfilingOptionFlags.set(op);
  }
  static void resetSamplingJProfilingOption(TR_SamplingJProfilingFlags op) {
    _samplingJProfilingOptionFlags.reset(op);
  }
  static bool isAnySamplingJProfilingOptionSet() {
    return !_samplingJProfilingOptionFlags.isEmpty();
  }

  static bool getVerboseOption(TR_VerboseFlags op) {
    return _verboseOptionFlags.isSet(op);
  }
  static void setVerboseOption(TR_VerboseFlags op) {
    _verboseOptionFlags.set(op);
  }
  static void setVerboseOptions(uint64_t mask) {
    _verboseOptionFlags.maskWord(0, mask);
  }
  static void resetVerboseOption(TR_VerboseFlags op) {
    _verboseOptionFlags.reset(op);
  }
  static char *getVerboseOptionName(TR_VerboseFlags op) {
    return _verboseOptionNames[op];
  }
  ...
  bool getAddressEnumerationOption(uint32_t mask) {
    return (_addressToEnumerate & mask) != 0;
  }

  bool getRegisterAssignmentTraceOption(uint32_t mask) {
    return (_raTrace & mask) != 0;
  }
  bool getTraceRAOption(uint32_t mask);
  bool getTraceLRA(uint32_t mask) { return (_traceLRA & mask) != 0; }
  bool getTraceSpillCosts(uint32_t mask) {
    return (_traceSpillCosts & mask) != 0;
  }
  bool getTraceSimplifier(uint32_t mask) {
    return (_traceSimplifier & mask) != 0;
  }
  bool getDebugEnableFlag(uint32_t mask) {
    return (_debugEnableFlags & mask) != 0;
  }
  bool getTraceILDeadCode(uint32_t mask) {
    return (_traceILDeadCode & mask) != 0;
  }
  ...
  int32_t getLastIpaOptTransformationIndex() {
    return _lastIpaOptTransformationIndex;
  }
  int32_t getNumInterfaceCallCacheSlots() {
    return _numInterfaceCallCacheSlots;
  }
  int32_t getNumInterfaceCallStaticSlots() {
    return _numInterfaceCallStaticSlots;
  }
  int32_t getStoreSinkingLastOpt() { return _storeSinkingLastOpt; }
  int32_t getFirstOptTransformationIndex() {
    return _firstOptTransformationIndex;
  }
  int32_t getLastOptTransformationIndex() {
    return _lastOptTransformationIndex;
  }
  ...
  void setDisabled(OMR::Optimizations o, bool b) {
    _disabledOptimizations[o] = b;
  }
  ...
  int32_t getMaxStaticPICSlots(TR_Hotness h) {
    return (h >= hot) ? _hotMaxStaticPICSlots : _maxStaticPICSlots;
  }
  ...
  bool anOptionSetContainsACountValue() {
    return _anOptionSetContainsACountValue;
  }
  void setAnOptionSetContainsACountValue(bool b) {
    _anOptionSetContainsACountValue = b;
  }
  ...
  bool dynamicDebugCounterIsEnabled(
      const char *name, int8_t fidelity = TR::DebugCounter::Undetermined);
  bool debugCounterHistogramIsEnabled(const char *name, int8_t fidelity);
  int32_t getDebugCounterBucketGranularity() {
    return _counterBucketGranularity;
  }
  int64_t getDebugCounterWarmupSeconds() { return _debugCounterWarmupSeconds; }
  const char *debugCounterInsertedFormat(TR_Memory *mem, const char *name,
                                         const char *format) {
    int nameLen = strlen(name);
    int formatLen = strlen(format);
    char *result =
        (char *)mem->allocateMemory(nameLen + formatLen + 2, heapAlloc);
  ...
  TR::SimpleRegex *getDisabledOptTransformations() {
    return _disabledOptTransformations;
  }
  ...
  TR::SimpleRegex *getVerboseOptTransformationsRegex() {
    return _verboseOptTransformationsRegex;
  }
  TR::SimpleRegex *getPackedTestRegex() { return _packedTest; }
  TR::SimpleRegex *getClassesWithFoldableFinalFields() {
    return _classesWithFolableFinalFields;
  }
  ...
  int32_t getBigCalleeThresholdForColdCallsAtWarm() const {
    return _bigCalleeThresholdForColdCallsAtWarm;
  }
  int32_t getBigCalleeFrequencyCutoffAtWarm() const {
    return _bigCalleeFreqCutoffAtWarm;
  }
  int32_t getBigCalleeHotOptThreshold() const {
    return _bigCalleeHotOptThreshold;
  }
  int32_t getBigCalleeThresholdForColdCallsAtHot() const {
    return _bigCalleeThresholdForColdCallsAtHot;
  }
  int32_t getBigCalleeFrequencyCutoffAtHot() const {
    return _bigCalleeFreqCutoffAtHot;
  }
  int32_t getBigCalleeScorchingOptThreshold() const {
    return _bigCalleeScorchingOptThreshold;
  }
  int32_t getLargeCompiledMethodExemptionFreqCutoff() const {
    return _largeCompiledMethodExemptionFreqCutoff;
  }
  int32_t getMaxSzForVPInliningWarm() const { return _maxSzForVPInliningWarm; }
  int32_t getInlinerVeryLargeCompiledMethodThreshold() const {
    return _inlinerVeryLargeCompiledMethodThreshold;
  }
  ...
  static bool getCountsAreProvidedByUser() {
    return _countsAreProvidedByUser;
  }  // set very late in setCounts()
  static TR_YesNoMaybe startupTimeMatters() {
    return _startupTimeMatters;
  }  // set very late in setCounts()
  ...
  int32_t getStackPCDumpNumberOfBuffers() {
    return _stackPCDumpNumberOfBuffers;
  }
  ...
  int32_t getInlineCntrCalleeTooBigBucketSize() {
    return _inlineCntrCalleeTooBigBucketSize;
  }
  int32_t getInlineCntrColdAndNotTinyBucketSize() {
    return _inlineCntrColdAndNotTinyBucketSize;
  }
  int32_t getInlineCntrWarmCalleeTooBigBucketSize() {
    return _inlineCntrWarmCalleeTooBigBucketSize;
  }
  int32_t getInlineCntrRanOutOfBudgetBucketSize() {
    return _inlineCntrRanOutOfBudgetBucketSize;
  }
  int32_t getInlineCntrCalleeTooDeepBucketSize() {
    return _inlineCntrCalleeTooDeepBucketSize;
  }
  ...
  int32_t getInlineCntrDepthExceededBucketSize() {
    return _inlineCntrDepthExceededBucketSize;
  }
  ...
  int32_t getDumbInlinerBytecodeSizeMaxCutoff() const {
    return _dumbInlinerBytecodeSizeMaxCutoff;
  }
  int32_t getDumbInlinerBytecodeSizeMinCutoff() const {
    return _dumbInlinerBytecodeSizeMinCutoff;
  }
  int32_t getDumbInlinerBytecodeSizeCutoff() const {
    return _dumbInlinerBytecodeSizeCutoff;
  }
  void setDumbInlinerBytecodeSizeCutoff(int32_t n) {
    _dumbInlinerBytecodeSizeCutoff = n;
  }
  int32_t getDumbInlinerBytecodeSizeDivisor() const {
    return _dumbInlinerBytecodeSizeDivisor;
  }
  ...
  int32_t getInlinerVeryColdBorderFrequencyAtCold() {
    return _inlinerVeryColdBorderFrequencyAtCold;
  }
  void setInlinerVeryColdBorderFrequencyAtCold(int32_t n) {
    _inlinerVeryColdBorderFrequencyAtCold = n;
  }
  ...
  int32_t getInlinerColdBorderFrequency() {
    return _inlinerColdBorderFrequency;
  }
  void setInlinerColdBorderFrequency(int32_t n) {
    _inlinerColdBorderFrequency = n;
  }
  int32_t getInlinerVeryColdBorderFrequency() {
    return _inlinerVeryColdBorderFrequency;
  }
  void setInlinerVeryColdBorderFrequency(int32_t n) {
    _inlinerVeryColdBorderFrequency = n;
  }
  ...
  int32_t getInlinerCGColdBorderFrequency() {
    return _inlinerCGColdBorderFrequency;
  }
  void setInlinerCGColdBorderFrequency(int32_t n) {
    _inlinerCGColdBorderFrequency = n;
  }
  int32_t getInlinerCGVeryColdBorderFrequency() {
    return _inlinerCGVeryColdBorderFrequency;
  }
  void setInlinerCGVeryColdBorderFrequency(int32_t n) {
    _inlinerCGVeryColdBorderFrequency = n;
  }
  int32_t getAlwaysWorthInliningThreshold() const {
    return _alwaysWorthInliningThreshold;
  }
  ...
  int32_t getProfilingCompNodecountThreshold() {
    return _profilingCompNodecountThreshold;
  }
 ...
 public:
  ...
  static int32_t getNumUsableCompilationThreads() {
    return _numUsableCompilationThreads;
  }

  static int32_t getTrampolineSpacePercentage() {
    return _trampolineSpacePercentage;
  }
  static size_t getScratchSpaceLimit() { return _scratchSpaceLimit; }
  static void setScratchSpaceLimit(size_t newScratchSpaceLimit) {
    _scratchSpaceLimit = newScratchSpaceLimit;
  }
  ...
  static int32_t
      _lowCodeCacheThreshold;  // Turn off Iprofiler if available code cache
                               // space is lower than this value

  static uint32_t
      _memExpensiveCompThreshold;  // threshold for when compilations are
                                   // considered memory hungry
  static uint32_t
      _cpuExpensiveCompThreshold;  // threshold for when compilations are
                                   // considered cpu hungry

  static int32_t _maxPeekedBytecodeSize;

  static int32_t
      _queueWeightThresholdForAppThreadYield;  // used only for LINUX; app
                                               // threads will sleep sometimes
  ...
  static uint32_t
      _minBytesToLeaveAllocatedInSharedPool;  // 0 to disable the feature and
                                              // revert to old behavior
  static uint32_t
      _maxBytesToLeaveAllocatedInSharedPool;  // 0 to disable the feature and
                                              // revert to old behavior
  ...
  void setIsVariableHeapBaseForBarrierRange0(uintptr_t b) {
    _isVariableHeapBaseForBarrierRange0 = b ? true : false;
  }
  bool isVariableHeapBaseForBarrierRange0() {
    return _isVariableHeapBaseForBarrierRange0;
  }

  void setIsVariableHeapSizeForBarrierRange0(uintptr_t b) {
    _isVariableHeapSizeForBarrierRange0 = b ? true : false;
  }
  bool isVariableHeapSizeForBarrierRange0() {
    return _isVariableHeapSizeForBarrierRange0;
  }

  void setIsVariableActiveCardTableBase(uintptr_t b) {
    _isVariableActiveCardTableBase = b ? true : false;
  }
  bool isVariableActiveCardTableBase() {
    return _isVariableActiveCardTableBase;
  }

  void setHeapAddressToCardAddressShift(uintptr_t s) {
    _heapAddressToCardAddressShift = (int32_t)s;
  }
  int32_t getHeapAddressToCardAddressShift() {
    return (int32_t)_heapAddressToCardAddressShift;
  }

  void setHeapSizeForBarrierRange0(uintptr_t b) {
    _heapSizeForBarrierRange0 = b;
  }
  uintptr_t getHeapSizeForBarrierRange0() { return _heapSizeForBarrierRange0; }

  void setHeapBaseForBarrierRange0(uintptr_t b) {
    _heapBaseForBarrierRange0 = b;
  }
  ...
  /**   \brief Returns a threshold on the profiling method invocations to trip
   * recompilation
   */
  int32_t getJProfilingMethodRecompThreshold() {
    return _jProfilingMethodRecompThreshold;
  }
  /**   \brief Returns a base threshold for loop to trip recompilation
   */
  int32_t getJProfilingLoopRecompThreshold() {
    return _jProfilingLoopRecompThreshold;
  }

  inline static float getMinProfiledCheckcastFrequency() {
    return _minProfiledCheckcastFrequency / ((float)100.0);
  }
  static bool isQuickstartDetected() { return _quickstartDetected; }
  void disableCHOpts();  // disable CHOpts, but also IPA and prex which depend
                         // on the chtable
 ...
 private:
  ...
  template <typename Enum, int NUM_FLAGS>
  class OptionFlagArray {
    // It is important not to add any more fields to this, because sometimes
    // ordinary option words are typecast to one of these (eg.
    // TR_JitPrivateConfig->verboseFlags).
    //
    uint64_t _words[(NUM_FLAGS + 63) / 64];

    inline uint64_t &word(int flagIndex) { return _words[flagIndex / 64]; }
    inline uint64_t bit(int flagIndex) {
      return ((uint64_t)1) << (flagIndex % 64);
    }

   public:
    OptionFlagArray() { memset(_words, 0, sizeof(_words)); }
    void operator=(const OptionFlagArray &other) {
      memcpy(_words, other.words, sizeof(_words));
    }

    bool isEmpty() {
      for (int i = 0; i < sizeof(_words) / sizeof(_words[0]); i++)
        if (_words[i]) return false;
      return true;
    }

    bool isSet(int flagIndex) {
      return (word(flagIndex) & bit(flagIndex)) != 0;
    }
    ...
    void replaceWord(int wordIndex, uint64_t newValue) {
      _words[wordIndex] = newValue;
    }
  };
  typedef OptionFlagArray<TR_VerboseFlags, TR_NumVerboseOptions>
      VerboseOptionFlagArray;

  static bool validateOptionsTables(void *feBase, TR_FrontEnd *fe);

  static bool compareOptionsForBinarySearch(const TR::OptionTable &a,
                                            const TR::OptionTable &b);

  static char *processOptionSet(char *options, char *envOptions,
                                TR::Options *jitBase, bool isAOT);
  static char *processOptionSet(char *options, char *envOptions,
                                TR::OptionSet *optionSet);
  static char *processOptionSet(char *options, TR::OptionSet *optionSet,
                                void *jitBase, bool isAOT);
  static char *processOption(char *option, TR::OptionTable *table, void *base,
                             int32_t numEntries, TR::OptionSet *optionSet);
  ...
  TR_MCTLogs *getLogListForOtherCompThreads() {
    return _logListForOtherCompThreads;
  }
  void setLogListForOtherCompThreads(TR_MCTLogs *l) {
    _logListForOtherCompThreads = l;
  }
  TR_MCTLogs *findLogFileForCompilationThread(int32_t compThreadID);
  void setLogForCompilationThread(int32_t compThreadID,
                                  TR::Options *masterOptions);
  static void safelyCloseLogs(TR::Options *options, TR_MCTLogs *&closedLogs,
                              TR_FrontEnd *fe);
 ...
 private:
  ...
  static char *setVerboseBitsInJitPrivateConfig(char *option, void *base,
                                                TR::OptionTable *entry);
  // Helper method used by the two methods above
  static char *setVerboseBitsHelper(char *option,
                                    VerboseOptionFlagArray *verboseOptionFlags,
                                    uintptrj_t defaultVerboseFlags);

  // Set samplingjprofiling bits
  //
  static char *setSamplingJProfilingBits(char *option, void *base,
                                         TR::OptionTable *entry);
  ...
  //
  static char *disableOptimization(char *option, void *base,
                                   TR::OptionTable *entry);
  static char *enableOptimization(char *option, void *base,
                                  TR::OptionTable *entry);

  // Trace an optimization
  //
  static char *traceOptimization(char *option, void *base,
                                 TR::OptionTable *entry);
  static char *dontTraceOptimization(char *option, void *base,
                                     TR::OptionTable *entry);

  // Scan the option for a numeric value and set word at offset "offset"
  // from the base to that value. That "word" is of 32bit or 64bit, depending on
  // JIT.
  //
  ...
  //
  static char *set32BitNumeric(char *option, void *base,
                               TR::OptionTable *entry);

  /**
   * \brief Option processing function for 32 bit numeric fields stored in
   * JitConfig
   *
   ...
   * JITConfig word 32-bit that needs to be set \return A pointer to the option
   * parameter
   */
  static char *set32BitNumericInJitConfig(char *option, void *base,
                                          TR::OptionTable *entry);

  // Scan the option for a hexadecimal value and set 32bit word at offset
  // "offset" from the base to that value.
  //
  static char *set32BitHexadecimal(char *option, void *base,
                                   TR::OptionTable *entry);

  // Scan the option for a numeric value and set 32bit word at offset "offset"
  // from the 'private' base (derived from the 'base' passed in) to that value.
  //
  static char *setStaticNumericKBAdjusted(char *option, void *base,
                                          TR::OptionTable *entry);

  // Scan the option for a string and set the value at offset "offset"
  // from the 'private' base (derived from the 'base' passed in) to that value.
  //
  static char *setStringForPrivateBase(char *option, void *base,
                                       TR::OptionTable *entry);

  // Scan the option for a (possibly negative) numeric value and set 32bit
  // word at offset "offset" from the base to that value.
  //
  static char *set32BitSignedNumeric(char *option, void *base,
                                     TR::OptionTable *entry);

  // Scan the option for a (possibly negative) numeric value and set 32bit
  // word at offset "offset" from the base to that value.
  //
  static char *set64BitSignedNumeric(char *option, void *base,
                                     TR::OptionTable *entry);

  // Scan the option for a numeric value and set word at parm1 to that value
  //
  static char *setStaticNumeric(char *option, void *base,
                                TR::OptionTable *entry);

  // Scan the option for a hexadecimal value and set word at parm1 to that value
  //
  static char *setStaticHexadecimal(char *option, void *base,
                                    TR::OptionTable *entry);

  // Set word at parm1 to 32-bit value at parm2
  //
  static char *setStatic32BitValue(char *option, void *base,
                                   TR::OptionTable *entry);
  ...
  //
  static char *setStaticString(char *option, void *base,
                               TR::OptionTable *entry);
   ...
   * \param [in] base    Not used
   * \param [in] entry   OptionTable entry specifying the offset of the
   * JITConfig field (a char pointer) that needs to be set \return A pointer to
   * the option parameter
   */
  static char *setStringInJitConfig(char *option, void *base,
                                    TR::OptionTable *entry);
  ...
  //
  static char *setAddressEnumerationBits(char *option, void *base,
                                         TR::OptionTable *entry);
 ...
 public:
  static char *setBitsFromStringSet(char *option, void *base,
                                    TR::OptionTable *entry);
  static char *clearBitsFromStringSet(char *option, void *base,
                                      TR::OptionTable *entry);

  static char *configureOptReporting(char *option, void *base,
                                     TR::OptionTable *entry);
  ...
  bool counterIsEnabled(const char *name, int8_t fidelity,
 ...
 protected:
  ...
  char *_optFileName;
  int32_t
      *_customStrategy;  // Actually array of TR_OptimizerImpl::Optimizations
                         // numbers read from optFileName
  ...
  static bool _quickstartDetected;  // set when Quickstart was specified on the
                                    // command line

  typedef OptionFlagArray<TR_SamplingJProfilingFlags,
                          TR_NumSamplingJProfilingFlags>
  ...
  bool _compileExcludedmethods;  // If true, "excluded" methods are compiled
                                 // using TR_EXCLUDED_OPTIONSET_INDEX
  ...
  int32_t _dumbInlinerBytecodeSizeCutoff;  // not configurable; can change
                                           // between max and min above
  ...
  bool _insertGCRTrees;  // more like a flag than an option; cannot be set by
                         // user
  ...
  static bool
      _dualLogging;  // a log file is used in two different option sets, or in
  ...
  char *_objectFileName;  // Name of the relocatable ELF file *.o if one is to
                          // be generated
...
class TR_MCTLogs {
 ...
 private:
  ...
  TR::FILE *_logFile;     ///< file descriptor for this log; comp thread will
                          ///< create these
  TR::Options *_options;  ///< pointer back to the TR::Options obj that contains
                          ///< this list
  int32_t _compThreadID;  ///< the ID of the compilation thread that can use
                          ///< this log
