...
//-----------------------------------------------------------------------------
void
TR_OptimizationPlan::freeOptimizationPlan(TR_OptimizationPlan* plan)
{
  ...
  if (!plan->getIsStackAllocated()) {
    ...
    TR_ASSERT(_totalNumAllocatedPlans >= _poolSize,
              "_totalNumAllocatedPlans should be greater than _poolSize");
    ...
    if (_poolSize > POOL_THRESHOLD) {
      do {
        ...
        TR_ASSERT(_totalNumAllocatedPlans >= _poolSize,
                  "Error: _totalNumAllocatedPlans=%d poolSize=%d\n",
                  _totalNumAllocatedPlans,
                  _poolSize);
...
//--------------------------------------------------------------------------
void*
TR_OptimizationPlan::operator new(size_t size) NOTHROW
{
  ...
  {
    TR_ASSERT(_poolSize == 0,
              "Some error regarding optimization plan accounting\n");
...
//-----------------------------------------------------------------------------
int32_t
TR_OptimizationPlan::freeEntirePool()
{
  ...
  if (TR::CompilationController::verbose() >= TR::CompilationController::LEVEL1)
    fprintf(stderr,
            "TR_OptimizationPlan allocations=%lu releases=%lu\n",
            _numAllocOp,
            _numFreeOp);
