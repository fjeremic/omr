...
static void writePerfToolEntry(void* start, uint32_t size, const char* name)
{
  ...
  if (firstAttempt) {
    ...
    int numCharsWritten
      = snprintf(perfFilename, maxPerfFilenameSize, "/tmp/perf-%" OMR_PRId64 ".map", static_cast<int64_t>(jvmPid));
  ...
  if (perfFile) {
    ...
    //    {
    //    j9jit_fprintf(getPerfFile(), "%lX %lX %s_%s\n", (intptr_t) getMetadata()->startColdPC, getMetadata()->endPC -
    //    getMetadata()->startColdPC,
    //                  TR::comp()()->signature(),
    //                  getCompilation()->getHotnessName(getCompilation()->getMethodHotness())); // should we change the
    //                  name of the method?
    //    }
...
static void printCompFailureInfo(TR::JitConfig* jitConfig, TR::Compilation* comp, const char* reason)
{
  if (comp) {
    ...
    if (debug("traceCompilationException")) {
      ...
      fprintf(
        stderr, "JIT: terminated compile of %s: %s\n", comp->signature(), reason ? reason : "<no reason provided>");
      fflush(stderr);
    }
  ...
  OMR_VMThread* omrVMThread, TR::IlGeneratorMethodDetails& details, TR_Hotness hotness, int32_t& rc)
{
  ...
  TR_ASSERT(TR::comp() == &compiler,
    "the TLS TR::Compilation object %p for this thread does not match the one %p just created.", TR::comp(), &compiler);
    ...
    if (rc == COMPILATION_SUCCEEDED) // success!
    {
      ...
      if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd, TR_VerbosePerformance)) {
        ...
        TR_VerboseLog::writeLine(TR_Vlog_COMP, "(%s) %s @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT,
          compiler.getHotnessName(compiler.getMethodHotness()), signature, startPC, compiler.cg()->getCodeEnd());
