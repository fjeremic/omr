...
static void
generatePerfToolEntry(uint8_t* startPC,
                      uint8_t* endPC,
                      const char* sig,
                      const char* hotness)
{
...
int32_t
commonJitInit(OMR::FrontEnd& fe, char* cmdLineOptions)
{
...
int32_t
init_options(TR::JitConfig* jitConfig, char* cmdLineOptions)
{
  ...
  if (*endOptions) {
    fprintf(
      stderr, "JIT: fatal error: invalid command line at %s\n", endOptions);
    return -1;
  }
...
static bool
methodCanBeCompiled(OMR::FrontEnd* fe,
                    ...
                    TR_Memory* trMemory)
{
  ...
  return TR::Options::getDebug()->methodCanBeCompiled(
    trMemory, &method, filter);
}
...
static void
printCompFailureInfo(TR::JitConfig* jitConfig,
                     TR::Compilation* comp,
                     const char* reason)
{
  if (comp) {
    ...
    if (jitConfig->options.verboseFlags != 0) {
      TR_VerboseLog::writeLineLocked(
        TR_Vlog_COMPFAIL, "%s failed compilation", comp->signature());
    }
...
uint8_t*
compileMethod(OMR_VMThread* omrVMThread,
...
uint8_t*
compileMethodFromDetails(OMR_VMThread* omrVMThread,
                         ...
                         int32_t& rc)
{
  ...
  TR::Options options(
    &trMemory, optionSetIndex, lineNumber, &compilee, 0, plan, false);
  ...
  try {
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(
      ...
      TR_VerboseLog::writeLineLocked(
        TR_Vlog_COMPSTART, "compiling %s", signature);
    }

    if (compiler.getOutFile() != NULL && compiler.getOption(TR_TraceAll)) {
      const char* signature = compilee.signature(&trMemory);
      traceMsg((&compiler),
               "<compile hotness=\"%s\" method=\"%s\">\n",
    ...
    if (rc == COMPILATION_SUCCEEDED) // success!
    {
      ...
      if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd,
        ...
        if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
          TR_VerboseLog::write(" time=%llu mem=%lluKB",
                               translationTime,
