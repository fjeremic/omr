...
static void writePerfToolEntry(void *start, uint32_t size, const char *name) {
  ...
  if (firstAttempt) {
    ...
    pid_t jvmPid = getpid();
#endif
    static const int maxPerfFilenameSize =
        15 + sizeof(jvmPid) * 3;  // "/tmp/perf-%ld.map"
    char perfFilename[maxPerfFilenameSize] = {0};

    int numCharsWritten =
        snprintf(perfFilename, maxPerfFilenameSize,
                 "/tmp/perf-%" OMR_PRId64 ".map", static_cast<int64_t>(jvmPid));
  ...
  if (perfFile) {
    // perf does not want 0x leading the hex start address and length of the
    // compiled code region the rest of the line is considered to be the name of
    // the region

    // j9jit_fprintf(getPerfFile(), "%lX %lX %s_%s\n", (intptr_t)
    // getMetadata()->startPC, getMetadata()->endWarmPC -
    // getMetadata()->startPC,
    ...
    //    {
    //    j9jit_fprintf(getPerfFile(), "%lX %lX %s_%s\n", (intptr_t)
    //    getMetadata()->startColdPC, getMetadata()->endPC -
...
static void generatePerfToolEntry(uint8_t *startPC, uint8_t *endPC,
                                  const char *sig, const char *hotness) {
  char buffer[1024];
  char *name;
...
int32_t commonJitInit(OMR::FrontEnd &fe, char *cmdLineOptions) {
  ...
  if (init_options(jitConfig, cmdLineOptions) < 0) return -1;
  ...
  // some silliness  here needed to support projects like JitBuilder where the
  // JIT can be initialized, shutdown, then initialized again. If we're
  // initializing the JIT a second (or later) time, the TOC cannot be
  // re-initialized (it will throw an assertion), but we can reuse the older
  // pseudoTOC. This code has never been thread safe on its own (it relies on
  // the caller to ensure thread safety if required), so relying on a static
  // boolean doesn't make this code any less safe.
  if (initializedTOC) {
    TR_PPCTableOfConstants *tocManagement =
        static_cast<TR_PPCTableOfConstants *>(
            fe.getPersistentInfo()->getPersistentTOC());
    pseudoTOC = tocManagement->getTOCBase();
  } else {
    pseudoTOC = (void *)TR_PPCTableOfConstants::initTOC(
        &fe, fe.getPersistentInfo(), jitConfig->getInterpreterTOC());
    initializedTOC = true;
  }

  if (pseudoTOC == (void *)0x1) pseudoTOC = NULL;
#endif
...
int32_t init_options(TR::JitConfig *jitConfig, char *cmdLineOptions) {
  ...
  char *endOptions =
      TR::Options::processOptionsJIT(cmdLineOptions, jitConfig, fe);
  if (*endOptions) {
...
static bool methodCanBeCompiled(OMR::FrontEnd *fe, TR_ResolvedMethod &method,
                                TR_FilterBST *&filter, TR_Memory *trMemory) {
  if (!method.isCompilable(trMemory)) return false;

  if (!TR::Options::getDebug()) return true;

  return TR::Options::getDebug()->methodCanBeCompiled(trMemory, &method,
                                                      filter);
}
...
static void printCompFailureInfo(TR::JitConfig *jitConfig,
                                 TR::Compilation *comp, const char *reason) {
  if (comp) {
    traceMsg(comp, "\n=== EXCEPTION THROWN (%s) ===\n", reason);
    ...
    if (jitConfig->options.verboseFlags != 0) {
      TR_VerboseLog::writeLineLocked(TR_Vlog_COMPFAIL, "%s failed compilation",
                                     comp->signature());
    }

    if (comp->getOutFile() != NULL && comp->getOption(TR_TraceAll))
      traceMsg(comp,
               "<result success=\"false\">exception thrown by the "
               "compiler</result>\n");
  }
}
...
uint8_t *compileMethodFromDetails(OMR_VMThread *omrVMThread,
  ...
  TR::SegmentAllocator &scratchSegmentProvider =
      TR::Options::getCmdLineOptions()->getOption(
          TR_EnableScratchMemoryDebugging)
  ...
  if (0 == (plan = TR_OptimizationPlan::alloc(hotness, false, false))) {
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(
            TR_VerboseCompileExclude)) {
      TR_VerboseLog::write(
          "<JIT: %s out-of-memory allocating optimization plan>\n",
          compilee.signature(&trMemory));
    }
    return 0;
  }
  ...
  TR::Options options(&trMemory, optionSetIndex, lineNumber, &compilee, 0, plan,
                      false);
  ...
  TR_ASSERT(
      TR::comp() == NULL,
      "there seems to be a current TLS TR::Compilation object %p for this "
      "thread. At this point there should be no current TR::Compilation object",
      TR::comp());
  TR::Compilation compiler(0, omrVMThread, &fe, &compilee, request, options,
                           dispatchRegion, &trMemory, plan);
  TR_ASSERT(TR::comp() == &compiler,
            "the TLS TR::Compilation object %p for this thread does not match "
            "the one %p just created.",
            TR::comp(), &compiler);

  try {
    // fprintf(stderr,"loading JIT debug\n");
    if (TR::Options::requiresDebugObject() || options.getLogFileName() ||
    ...
    if (rc == COMPILATION_SUCCEEDED)  // success!
    {
      ...
      uint64_t translationTime =
          TR::Compiler->vm.getUSecClock() - translationStartTime;

      if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompileEnd,
                                             TR_VerbosePerformance)) {
        const char *signature = compilee.signature(&trMemory);
        TR_VerboseLog::vlogAcquire();
        TR_VerboseLog::writeLine(
            TR_Vlog_COMP,
            "(%s) %s @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT,
            compiler.getHotnessName(compiler.getMethodHotness()), signature,
            startPC, compiler.cg()->getCodeEnd());

        if (TR::Options::getVerboseOption(TR_VerbosePerformance)) {
          TR_VerboseLog::write(" time=%llu mem=%lluKB", translationTime,
                               static_cast<unsigned long long>(
                                   scratchSegmentProvider.bytesAllocated()) /
                                   1024);
        }
      ...
      if (compiler.getOption(TR_PerfTool) ||
        ...
        if (compiler.getOption(TR_PerfTool)) {
          generatePerfToolEntry(
              startPC, codeGenerator.getCodeEnd(), compiler.signature(),
      ...
      if (compiler.getOutFile() != NULL && compiler.getOption(TR_TraceAll))
        traceMsg(
            (&compiler),
            "<result success=\"true\" startPC=\"%#p\" time=\"%lld.%lldms\"/>\n",
  ...
  TR::CodeCache *codeCache =
      compiler.cg() ? compiler.cg()->getCodeCache() : NULL;
