...
//
typedef char *(*OptionFunctionPtr)(char *option, void *base,
                                   OptionTable *entry);
...
struct OptionTable {
   ...
   *     'I' ==> internal options (not reported by -Xjit:help) usually for
   * experimentation or testing '0' - '9' ==> reserved for VM-specific
   * categories
   *
   ...
   *     'F' ==> only print if option found (msgInfo has the OPTION_FOUND bit
   * set) 'P' ==> only print if option value is non-zero (or non-null)
   */
  const char *msg;
   ...
   * this option can only be used as a top-level option, and not in an option
   * subset. For the VM-specific table this is not required since none of these
   * options can appear in an option subset.
   *
   * If the option was found in the option string the OPTION_FOUND bit will be
   * set
   */
...
public:
  ...
  intptrj_t getIndex() {
    intptrj_t i = (intptrj_t)_methodRegex;
    return (i & 1) ? i >> 1 : 0;
  }
