...
TR::OptionTable OMR::Options::_jitOptions[] = {
    ...
    { "assumeStartupPhaseUntilToldNotTo", "M\tUse compiler.Command("
                                          "endOfStartup"
...
bool OMR::Options::requiresDebugObject()
{
    ...
    if (OMR::Options::hasSomeLogFile() || OMR::Options::isOptionSetForAnyMethod(TR_EntryBreakPoints) || OMR::Options::isOptionSetForAnyMethod(TR_DebugBeforeCompile) || OMR::Options::isOptionSetForAnyMethod(TR_DebugOnEntry)) {
        return true;
    }
...
bool OMR::Options::jitLatePostProcess(TR::OptionSet* optionSet, void* jitConfig)
{
    ...
    if (!optionSet) {
        ...
        if (!(TR::Options::getCmdLineOptions() && TR::Options::getCmdLineOptions()->getOption(TR_DisableRampupImprovements) || TR::Options::getAOTCmdLineOptions() && TR::Options::getAOTCmdLineOptions()->getOption(TR_DisableRampupImprovements))) {
        ...
        if (TR::Options::sharedClassCache()) // If AOT shared classes is used
        {
            ...
            {
                if (TR::Options::getAOTCmdLineOptions()->getOption(TR_DisablePersistIProfile) || TR::Options::getCmdLineOptions()->getOption(TR_DisablePersistIProfile)) {
            ...
            if (TR::Options::isQuickstartDetected()) {
                ...
                if (!self()->getOption(TR_DisableGuardedCountingRecompilations) && _coldUpgradeSampleThreshold == TR_DEFAULT_COLD_UPGRADE_SAMPLE_THRESHOLD)
        ...
        {
            OMR::Options::_abstractTimeGracePeriod = self()->getOption(TR_UseVmTotalCpuTimeAsAbstractTime) ? DEFAULT_ABSTRACT_TIME_MS_CPU_GRACE_PERIOD : DEFAULT_ABSTRACT_TIME_SAMPLES_GRACE_PERIOD;
        }
        if (OMR::Options::_abstractTimeToReduceInliningAggressiveness == -1) // not set
        {
            OMR::Options::_abstractTimeToReduceInliningAggressiveness = self()->getOption(TR_UseVmTotalCpuTimeAsAbstractTime) ? DEFAULT_ABSTRACT_TIME_MS_CPU_TO_REDUCE_INLINING_AGGRESSIVENESS : DEFAULT_ABSTRACT_TIME_SAMPLES_TO_REDUCE_INLINING_AGGRESSIVENESS;
        }
...
void OMR::Options::jitPreProcess()
{
    ...
    if (TR::isJ9()) {
        ...
        if (((TR::Compiler->target.cpu.isPower() && TR::Compiler->target.isLinux()) && TR::Compiler->target.numberOfProcessors() >= 8) || ((TR::Compiler->target.cpu.isX86() && TR::Compiler->target.isLinux()) && TR::Compiler->target.numberOfProcessors() >= 4))
...
char* OMR::Options::processOptionSet(
    char* options,
    char* envOptions,
    TR::Options* jitBase,
    bool isAOT)
...
bool OMR::Options::requiresLogFile()
{
    ...
    //
    if (self()->getAnyOption(TR_TraceAll) || self()->getAnyOption(TR_TraceBBVA) || self()->getAnyOption(TR_TraceBVA) || self()->getAnyOption(TR_TraceUseDefs) || self()->getAnyOption(TR_TraceNodeFlags) || self()->getAnyOption(TR_TraceValueNumbers) || self()->getAnyOption(TR_TraceLiveness) || self()->getAnyOption(TR_TraceILGen) || self()->getAnyOption(TR_TraceILValidator))
        return true;
...
char* OMR::Options::setCounts()
{
    ...
    } else // no counts string specified
    {
        ...
        {
            if (TR::Options::getJITCmdLineOptions()->getOption(TR_UseLowerMethodCounts) || (TR::Options::getAOTCmdLineOptions() && TR::Options::getAOTCmdLineOptions()->getOption(TR_UseLowerMethodCounts)))
                _startupTimeMatters = TR_yes;
            else if (TR::Options::getJITCmdLineOptions()->getOption(TR_UseHigherMethodCounts) || (TR::Options::getAOTCmdLineOptions() && TR::Options::getAOTCmdLineOptions()->getOption(TR_UseHigherMethodCounts)))
                _startupTimeMatters = TR_no;
            else if (TR::Options::isQuickstartDetected())
                _startupTimeMatters = TR_yes;
        }

        bool startupTimeMatters = (_startupTimeMatters == TR_yes || (_startupTimeMatters == TR_maybe && TR::Options::sharedClassCache()));
    ...
    if ((TR::Options::getAOTCmdLineOptions() == this || TR::Options::getJITCmdLineOptions() == this) && (_initialCount == 0) && (_initialBCount == 0))
        self()->setOption(TR_DisableInterpreterProfiling, true);
...
char* OMR::Options::setCount(char* option, void* base, TR::OptionTable* entry)
{
    ...
    *((int32_t*)((char*)base + offset)) = countValue;
    ...
    if ((base != _jitCmdLineOptions && base != _aotCmdLineOptions)) // the option is specified in a subset.
    {
        ...
        //
        if ((offset == offsetof(OMR::Options, _initialCount) || offset == offsetof(OMR::Options, _initialBCount)) && ((TR::Options*)base)->_initialMILCount > countValue)
...
char* OMR::Options::setVerboseBitsHelper(char* option, VerboseOptionFlagArray* verboseOptionFlags, uintptrj_t defaultVerboseFlags)
{
...
{
    TR::Options::processOptionSet(NoOptString "," DisableAllocationInliningString "," DisableInlineCheckCastString "," DisableInlineIfInstanceOfString "," DisableInlineInstanceOfString "," DisableInlineMonEntString "," DisableInlineMonExitString "," DisableInliningOfNativesString "," DisableNewInstanceImplOptString "," DisableFastStringIndexOfString,
        0, _currentOptionSet);
