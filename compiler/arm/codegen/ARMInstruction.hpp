...
inline bool
constantIsUnsignedImmed8(int32_t intValue)
{
  if (abs(intValue) < 256)
    return true;
  else
...
inline bool
constantIsImmed8r(int32_t intValue)
{
  return intValue < 128;
}
...
inline bool
constantIsImmed10(int32_t intValue)
{
...
inline bool
constantIsImmed12(int32_t intValue)
{
...
inline bool
isSinglePrecision(TR::RealRegister::RegNum registerNumber)
{
...
public:
  ...
  ARMVirtualGuardNOPInstruction(TR::Node* node,
                                ...
                                TR::CodeGenerator* cg)
    : TR::ARMLabelInstruction(precedingInstruction,
                              ARMOp_vgdnop,
                              node,
                              cond,
                              sym,
...
public:
  ARMLoadStartPCInstruction(TR::Node* node,
    ...
    : TR::ARMTrg1Src2Instruction(
        ...
        cg)
    , /* The value 0xde does not mean anything. It will be replaced in the
...
class ARMMulInstruction : public TR::Instruction
{
  ...
  TR::Register*
    _targetHiRegister; // for mull instructions only, NULL for mul instructions
  TR::Register* _source1Register;
...
public:
  ARMControlFlowInstruction(TR::Node* node, TR::CodeGenerator* cg)
    : TR::Instruction(node, cg)
  {}
  ARMControlFlowInstruction(TR_ARMOpCodes op,
                            TR::Node* node,
                            TR::CodeGenerator* cg)
    : TR::Instruction(op, node, cg)
    , _numSources(0)
