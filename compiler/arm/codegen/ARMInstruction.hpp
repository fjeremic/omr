...
inline bool constantIsImmed8r(int32_t intValue, uint32_t *base,
                              uint32_t *rotate, bool *negated) {
...
inline bool isSinglePrecision(TR::RealRegister::RegNum registerNumber) {
  ...
  return ((registerNumber >= TR::RealRegister::FirstFSR) &&
          (registerNumber <= TR::RealRegister::LastFSR));
#else
  return false;
#endif
}
...
public:
  ...
  ARMImmInstruction(TR_ARMOpCodes op, TR::Node *node, uint32_t imm,
                    TR::CodeGenerator *cg)
      : TR::Instruction(node, cg), _sourceImmediate(imm),
        _reloKind(TR_NoRelocation), _symbolReference(NULL) {
    setOpCodeValue(op);
  }

  ARMImmInstruction(TR_ARMOpCodes op, TR::Node *node,
                    TR::RegisterDependencyConditions *cond, uint32_t imm,
                    TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cond, cg), _sourceImmediate(imm),
        _reloKind(TR_NoRelocation), _symbolReference(NULL) {}

  ARMImmInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                    TR::Node *node, uint32_t imm, TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _sourceImmediate(imm), _reloKind(TR_NoRelocation),
        _symbolReference(NULL) {}

  ARMImmInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                    TR::Node *node, TR::RegisterDependencyConditions *cond,
  ...
  ARMImmInstruction(TR_ARMOpCodes op, TR::Node *node, uint32_t imm,
      ...
      : TR::Instruction(op, node, cg), _sourceImmediate(imm),
        _reloKind(relocationKind), _symbolReference(NULL) {
    setNeedsAOTRelocation(true);
  }

  ARMImmInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                    TR::Node *node, uint32_t imm,
                    TR_ExternalRelocationTargetKind relocationKind,
                    TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _sourceImmediate(imm), _reloKind(relocationKind),
        _symbolReference(NULL) {
    setNeedsAOTRelocation(true);
  }
  ...
  ARMImmInstruction(TR_ARMOpCodes op, TR::Node *node, uint32_t imm,
      ...
      : TR::Instruction(op, node, cg), _sourceImmediate(imm),
        _reloKind(relocationKind), _symbolReference(sr) {
    setNeedsAOTRelocation(true);
  }
  ...
  void setReloKind(TR_ExternalRelocationTargetKind reloKind) {
    _reloKind = reloKind;
  }
...
public:
  ...
  ARMLabelInstruction(TR_ARMOpCodes op, TR::Node *node, TR::LabelSymbol *sym,
                      TR::CodeGenerator *cg, TR::Register *t1reg = NULL,
  ...
  ARMLabelInstruction(TR_ARMOpCodes op, TR::Node *node,
                      TR::RegisterDependencyConditions *cond,
                      TR::LabelSymbol *sym, TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cond, cg), _symbol(sym), _t1reg(NULL),
        _s1reg(NULL) {
    if (sym != NULL && op == ARMOp_label)
      sym->setInstruction(this);
  }

  ARMLabelInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                      TR::Node *node, TR::LabelSymbol *sym,
                      TR::CodeGenerator *cg, TR::Register *t1reg = NULL,
                      TR::Register *s1reg = NULL)
      : TR::Instruction(precedingInstruction, op, node, cg), _symbol(sym),
        _t1reg(t1reg), _s1reg(s1reg) {
    if (t1reg)
      t1reg->incTotalUseCount();
    if (s1reg)
      s1reg->incTotalUseCount();
  ...
  ARMLabelInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                      TR::Node *node, TR::RegisterDependencyConditions *cond,
                      TR::LabelSymbol *sym, TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cond, cg), _symbol(sym),
        _t1reg(NULL), _s1reg(NULL) {
    if (sym != NULL && op == ARMOp_label)
      sym->setInstruction(this);
  }
  ...
  TR::Register *setTarget1Register(TR::Register *treg1) {
    return (_t1reg = treg1);
  }

  TR::Register *getSource1Register() { return _s1reg; }
  TR::Register *setSource1Register(TR::Register *sreg1) {
    return (_s1reg = sreg1);
  }
...
public:
  ARMConditionalBranchInstruction(TR::Node *node, TR::CodeGenerator *cg)
      : TR::ARMLabelInstruction(node, cg), _estimatedBinaryLocation(0),
        _farRelocation(false) {
    setConditionCode(ARMConditionCodeIllegal);
  }

  ARMConditionalBranchInstruction(TR_ARMOpCodes op, TR::Node *node,
                                  TR::LabelSymbol *sym, TR_ARMConditionCode cc,
                                  TR::CodeGenerator *cg)
      : TR::ARMLabelInstruction(op, node, sym, cg), _estimatedBinaryLocation(0),
        _farRelocation(false) {
    setConditionCode(cc);
  }
  ...
  ARMConditionalBranchInstruction(TR_ARMOpCodes op, TR::Node *node,
      ...
      : TR::ARMLabelInstruction(op, node, cond, sym, cg),
        _estimatedBinaryLocation(0), _farRelocation(false) {
    setConditionCode(cc);
  }

  ARMConditionalBranchInstruction(TR::Instruction *precedingInstruction,
                                  TR_ARMOpCodes op, TR::Node *node,
                                  TR::LabelSymbol *sym, TR_ARMConditionCode cc,
                                  TR::CodeGenerator *cg)
  ...
  int32_t setEstimatedBinaryLocation(int32_t l) {
    return (_estimatedBinaryLocation = l);
  }

  bool getFarRelocation() { return _farRelocation; }
  bool setFarRelocation(bool b) { return (_farRelocation = b); }

  virtual TR::ARMConditionalBranchInstruction *
  getARMConditionalBranchInstruction();
...
public:
  ARMVirtualGuardNOPInstruction(TR::Node *node, TR_VirtualGuardSite *site,
                                TR::RegisterDependencyConditions *cond,
                                TR::LabelSymbol *sym, TR::CodeGenerator *cg)
  ...
  ARMVirtualGuardNOPInstruction(TR::Node *node, TR_VirtualGuardSite *site,
                                ...
                                TR::CodeGenerator *cg)
      : TR::ARMLabelInstruction(precedingInstruction, ARMOp_vgdnop, node, cond,
                                sym, cg),
        _site(site) {}
...
public:
  ARMAdminInstruction(TR_ARMOpCodes op, TR::Node *node, TR::Node *fenceNode,
                      TR::CodeGenerator *cg)
  ...
  ARMAdminInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                      TR::Node *node, TR::Node *fenceNode,
                      TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
...
public:
  ARMImmSymInstruction(TR::Node *node, TR::CodeGenerator *cg)
      : TR::ARMImmInstruction(node, cg), _symbolReference(NULL),
        _snippet(NULL) {}

  ARMImmSymInstruction(TR_ARMOpCodes op, TR::Node *node, uint32_t imm,
                       TR::RegisterDependencyConditions *cond,
                       TR::SymbolReference *sr, TR::CodeGenerator *cg,
                       TR::Snippet *s = NULL,
                       TR_ARMConditionCode cc = ARMConditionCodeAL);

  ARMImmSymInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                       TR::Node *node, uint32_t imm,
                       TR::RegisterDependencyConditions *cond,
                       TR::SymbolReference *sr, TR::CodeGenerator *cg,
                       TR::Snippet *s = NULL,
...
public:
  ...
  ARMTrg1Src2Instruction(TR_ARMOpCodes op, TR::Node *node,
                         TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _target1Register(0), _source1Register(0),
        _source2Operand(0) {}
  ...
  ARMTrg1Src2Instruction(TR::Instruction *precedingInstruction,
                         ...
                         TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _target1Register(0), _source1Register(0), _source2Operand(0) {}

  ARMTrg1Src2Instruction(TR_ARMOpCodes op, TR::Node *node,
                         TR::RegisterDependencyConditions *cond,
                         TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cond, cg), _target1Register(0),
        _source1Register(0), _source2Operand(0) {}

  ARMTrg1Src2Instruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *treg,
                         TR::Register *s1reg, TR_ARMOperand2 *s2op,
                         TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _target1Register(treg),
        _source1Register(s1reg), _source2Operand(s2op) {
  ...
  ARMTrg1Src2Instruction(TR::Instruction *precedingInstruction,
                         TR_ARMOpCodes op, TR::Node *node, TR::Register *treg,
                         TR::Register *s1reg, TR_ARMOperand2 *s2op,
                         TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _target1Register(treg), _source1Register(s1reg), _source2Operand(s2op) {
    treg->incTotalUseCount();
    s1reg->incTotalUseCount();
    s2op->incTotalUseCount();
  }
  ...
  ARMTrg1Src2Instruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *treg,
    ...
    TR_ARMOperand2 *s2op =
        new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2Reg, s2reg);
  ...
  ARMTrg1Src2Instruction(TR::Instruction *precedingInstruction,
                         ...
                         TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _target1Register(treg), _source1Register(s1reg) {
    TR_ARMOperand2 *s2op =
        new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2Reg, s2reg);
    setSource2Operand(s2op);
    treg->incTotalUseCount();
  ...
  TR::Register *setTarget1Register(TR::Register *treg1) {
    return (_target1Register = treg1);
  }

  TR::Register *getSource1Register() { return _source1Register; }
  TR::Register *setSource1Register(TR::Register *sreg1) {
    return (_source1Register = sreg1);
  }

  TR_ARMOperand2 *getSource2Operand() { return _source2Operand; }
  TR_ARMOperand2 *setSource2Operand(TR_ARMOperand2 *sop) {
    return (_source2Operand = sop);
  }
...
public:
  ARMLoadStartPCInstruction(TR::Node *node, TR::Register *treg,
                            TR::SymbolReference *symRef, TR::CodeGenerator *cg)
      : TR::ARMTrg1Src2Instruction(
            ARMOp_sub, node, treg,
            cg->machine()->getRealRegister(TR::RealRegister::gr15),
            new (cg->trHeapMemory()) TR_ARMOperand2(0xde, 24),
            cg), /* The value 0xde does not mean anything. It will be replaced
                    in the binary encoding phase. */
  ...
  ARMLoadStartPCInstruction(TR::Instruction *precedingInstruction,
                            ...
                            TR::SymbolReference *symRef, TR::CodeGenerator *cg)
      : TR::ARMTrg1Src2Instruction(
            precedingInstruction, ARMOp_sub, node, treg,
            cg->machine()->getRealRegister(TR::RealRegister::gr15),
            new (cg->trHeapMemory()) TR_ARMOperand2(0xde, 0),
            cg), /* The value 0xde does not mean anything. It will be replaced
                    in the binary encoding phase. */
        _symbolReference(symRef) {}
...
public:
  ...
  ARMSrc2Instruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *s1reg,
  ...
  ARMSrc2Instruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                     TR::Node *node, TR::Register *s1reg, TR_ARMOperand2 *s2op,
                     TR::CodeGenerator *cg)
...
public:
  ...
  ARMTrg1Src1Instruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *treg,
                         TR_ARMOperand2 *sop, TR::CodeGenerator *cg)
      : TR::ARMTrg1Src2Instruction(op, node, cg) {
  ...
  ARMTrg1Src1Instruction(TR_ARMOpCodes op, TR::Node *node,
                         TR::RegisterDependencyConditions *cond,
                         TR::Register *treg, TR_ARMOperand2 *sop,
                         TR::CodeGenerator *cg)
  ...
  ARMTrg1Src1Instruction(TR::Instruction *precedingInstruction,
      ...
      : TR::ARMTrg1Src2Instruction(precedingInstruction, op, node, cg) {
    TR_ARMOperand2 *sop =
        new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2Reg, sreg);
    setTarget1Register(treg);
    setSource2Operand(sop);
    treg->incTotalUseCount();
    sop->incTotalUseCount();
...
public:
  ...
  ARMTrg1Instruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *treg,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _target1Register(treg) {
    treg->incTotalUseCount();
  }

  ARMTrg1Instruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                     TR::Node *node, TR::Register *treg, TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
  ...
  TR::Register *setTargetRegister(TR::Register *treg) {
    return _target1Register = treg;
  }
...
public:
  ARMTrg2Src1Instruction(TR::Node *node, TR::CodeGenerator *cg)
      : TR::ARMTrg1Src2Instruction(node, cg), _target2Register(0) {}

  ARMTrg2Src1Instruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *t1reg,
                         TR::Register *t2reg, TR::Register *sreg,
                         TR::CodeGenerator *cg)
  ...
  ARMTrg2Src1Instruction(TR::Instruction *precedingInstruction,
                         TR_ARMOpCodes op, TR::Node *node, TR::Register *t1reg,
                         TR::Register *t2reg, TR::Register *sreg,
                         TR::CodeGenerator *cg)
      : TR::ARMTrg1Src2Instruction(precedingInstruction, op, node, cg),
  ...
  TR::Register *setTarget2Register(TR::Register *treg2) {
    return (_target2Register = treg2);
  }
...
class ARMMulInstruction : public TR::Instruction {
  ...
  TR::Register *_targetHiRegister; // for mull instructions only, NULL for mul
                                   // instructions
  TR::Register *_source1Register;
...
public:
  ...
  ARMMulInstruction(TR_ARMOpCodes op, TR::Node *node, TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _targetLoRegister(0),
        _targetHiRegister(0), _source1Register(0), _source2Register(0) {}

  ARMMulInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                    TR::Node *node, TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _targetLoRegister(0), _targetHiRegister(0), _source1Register(0),
        _source2Register(0) {}

  ARMMulInstruction(TR_ARMOpCodes op, TR::Node *node,
                    TR::RegisterDependencyConditions *cond,
                    TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cond, cg), _targetLoRegister(0),
        _targetHiRegister(0), _source1Register(0), _source2Register(0) {}

  ARMMulInstruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *tregLo,
                    TR::Register *s1reg, TR::Register *s2reg,
                    TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _targetLoRegister(tregLo),
        _targetHiRegister(0), _source1Register(s1reg), _source2Register(s2reg) {
  ...
  ARMMulInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                    TR::Node *node, TR::Register *tregLo, TR::Register *s1reg,
                    TR::Register *s2reg, TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _targetLoRegister(tregLo), _targetHiRegister(0),
        _source1Register(s1reg), _source2Register(s2reg) {
    tregLo->incTotalUseCount();
    s1reg->incTotalUseCount();
    s2reg->incTotalUseCount();
  }
  // below 2 for mull instructions
  ARMMulInstruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *tregHi,
                    TR::Register *tregLo, TR::Register *s1reg,
                    TR::Register *s2reg, TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _targetLoRegister(tregLo),
        _targetHiRegister(tregHi), _source1Register(s1reg),
  ...
  ARMMulInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                    TR::Node *node, TR::Register *tregHi, TR::Register *tregLo,
                    TR::Register *s1reg, TR::Register *s2reg,
                    TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _targetLoRegister(tregLo), _targetHiRegister(tregHi),
        _source1Register(s1reg), _source2Register(s2reg) {
    tregLo->incTotalUseCount();
    tregHi->incTotalUseCount();
    s1reg->incTotalUseCount();
  ...
  TR::Register *setTargetLoRegister(TR::Register *tregLo) {
    return (_targetLoRegister = tregLo);
  }

  TR::Register *getTargetHiRegister() { return _targetHiRegister; }
  TR::Register *setTargetHiRegister(TR::Register *tregHi) {
    return (_targetHiRegister = tregHi);
  }
  ...
  TR::Register *setSource2Register(TR::Register *sreg2) {
    return (_source2Register = sreg2);
  }
...
public:
  ...
  ARMMemInstruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *treg,
                    TR::MemoryReference *mf, bool memRead,
                    TR::CodeGenerator *cg)
      : TR::ARMTrg1Instruction(op, node, treg, cg), _memoryReference(mf),
        _memoryRead(memRead) {
    mf->incRegisterTotalUseCounts(cg);
  }

  ARMMemInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                    TR::Node *node, TR::Register *treg, TR::MemoryReference *mf,
                    bool memRead, TR::CodeGenerator *cg)
      : TR::ARMTrg1Instruction(precedingInstruction, op, node, treg, cg),
        _memoryReference(mf), _memoryRead(memRead) {
    mf->incRegisterTotalUseCounts(cg);
  }
  ...
  virtual TR::Snippet *getSnippetForGC() {
    return getMemoryReference()->getUnresolvedSnippet();
  }
...
public:
  ...
  ARMMemSrc1Instruction(TR_ARMOpCodes op, TR::Node *node,
                        TR::MemoryReference *mf, TR::Register *sreg,
                        TR::CodeGenerator *cg)
      : TR::ARMMemInstruction(op, node, sreg, mf, true, cg) {}

  ARMMemSrc1Instruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                        TR::Node *node, TR::MemoryReference *mf,
                        TR::Register *sreg, TR::CodeGenerator *cg)
      : TR::ARMMemInstruction(precedingInstruction, op, node, sreg, mf, true,
                              cg) {}
  ...
  TR::Register *setSourceRegister(TR::Register *sr) {
    return setTargetRegister(sr);
  }

  void insertSourceRegister(uint32_t *instruction) {
    insertTargetRegister(instruction);
  }
...
public:
  ...
  ARMTrg1MemInstruction(TR_ARMOpCodes op, TR::Node *node, TR::Register *sreg,
                        TR::MemoryReference *mf, TR::CodeGenerator *cg)
      : TR::ARMMemInstruction(op, node, sreg, mf, false, cg) {}

  ARMTrg1MemInstruction(TR::Instruction *precedingInstruction, TR_ARMOpCodes op,
                        TR::Node *node, TR::Register *sreg,
                        TR::MemoryReference *mf, TR::CodeGenerator *cg)
      : TR::ARMMemInstruction(precedingInstruction, op, node, sreg, mf, false,
                              cg) {}
...
public:
  ...
  ARMTrg1MemSrc1Instruction(TR_ARMOpCodes op, TR::Node *node,
  ...
  ARMTrg1MemSrc1Instruction(TR::Instruction *precedingInstruction,
                            ...
                            TR::Register *treg, TR::MemoryReference *mf,
                            TR::Register *sreg, TR::CodeGenerator *cg)
      : TR::ARMMemInstruction(precedingInstruction, op, node, treg, mf, false,
                              cg),
        _source1Register(sreg) {}
  ...
  TR::Register *setSourceRegister(TR::Register *sreg1) {
    return (_source1Register = sreg1);
  }
...
// If we are using a register allocator that cannot handle registers being alive
// across basic block boundaries then we use TR::ARMControlFlowInstruction as a
// placeholder.  It contains a summary of register usage information and enough
// other information to generate the correct instruction sequence.
...
public:
  ARMControlFlowInstruction(TR::Node *node, TR::CodeGenerator *cg)
      : TR::Instruction(node, cg) {}
  ARMControlFlowInstruction(TR_ARMOpCodes op, TR::Node *node,
                            TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _numSources(0), _numTargets(0),
        _label(NULL), _opCode2(ARMOp_bad) {}

  ARMControlFlowInstruction(TR_ARMOpCodes op, TR::Node *node,
                            TR::RegisterDependencyConditions *deps,
                            TR::CodeGenerator *cg)
      : TR::Instruction(op, node, deps, cg), _numSources(0), _numTargets(0),
        _label(NULL), _opCode2(ARMOp_bad) {}
  ...
  TR_ARMOpCodes setOpCode2Value(TR_ARMOpCodes op) {
    return (_opCode2.setOpCodeValue(op));
  }

  TR_ARMOpCode &getOpCode3() { return _opCode3; }
  TR_ARMOpCodes getOpCode3Value() { return _opCode3.getOpCodeValue(); }
  TR_ARMOpCodes setOpCode3Value(TR_ARMOpCodes op) {
    return (_opCode3.setOpCodeValue(op));
  }

  TR_ARMOpCode &getCmpOp() { return _cmpOp; }
  TR_ARMOpCodes getCmpOpValue() { return _cmpOp.getOpCodeValue(); }
  TR_ARMOpCodes setCmpOpValue(TR_ARMOpCodes op) {
    return (_cmpOp.setOpCodeValue(op));
  }
...
public:
  ...
  ARMMultipleMoveInstruction(TR_ARMOpCodes op, TR::Node *node,
                             TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _memoryBaseRegister(0), _registerList(0),
        _writeBack(false), _increment(false), _preIndex(false) {}

  ARMMultipleMoveInstruction(TR::Instruction *precedingInstruction,
                             TR_ARMOpCodes op, TR::Node *node,
                             TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _memoryBaseRegister(0), _registerList(0), _writeBack(false),
        _increment(false), _preIndex(false) {}

  ARMMultipleMoveInstruction(TR_ARMOpCodes op, TR::Node *node,
                             TR::RegisterDependencyConditions *cond,
                             TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cond, cg), _memoryBaseRegister(0),
        _registerList(0), _writeBack(false), _increment(false),
        _preIndex(false) {}

  ARMMultipleMoveInstruction(TR_ARMOpCodes op, TR::Node *node,
                             TR::Register *mbr, uint16_t rl,
                             TR::CodeGenerator *cg)
      : TR::Instruction(op, node, cg), _memoryBaseRegister(mbr),
        _registerList(rl), _writeBack(false), _increment(false),
        _preIndex(false) {
  ...
  ARMMultipleMoveInstruction(TR::Instruction *precedingInstruction,
                             ...
                             TR::CodeGenerator *cg)
      : TR::Instruction(precedingInstruction, op, node, cg),
        _memoryBaseRegister(mbr), _registerList(rl), _writeBack(false),
        _increment(false), _preIndex(false) {
    mbr->incTotalUseCount();
  }
  ...
  TR::Register *setMemoryBaseRegister(TR::Register *mbr) {
    return (_memoryBaseRegister = mbr);
  }
