...
TR_Debug::print(TR::FILE* pOutFile, TR::ARMImmInstruction* instr)
{
  printPrefix(pOutFile, instr);
  trfprintf(
    pOutFile, "%s\t0x%08x", fullOpCodeName(instr), instr->getSourceImmediate());
  dumpDependencies(pOutFile, instr);
  trfflush(pOutFile);
}
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARMImmSymInstruction* instr)
{
  ...
  if (bufferPos != NULL && longJump) {
    if (*(int32_t*)bufferPos == 0xe1a0e00f) {
      ...
      if (name)
        trfprintf(
          pOutFile, "DCD\t0x%x\t; %s (" POINTER_PRINTF_FORMAT ")", imm, name);
      else
        trfprintf(pOutFile, "DCD\t0x%x", imm);
      return;
    } else {
      printARMHelperBranch(instr->getSymbolReference(),
                           bufferPos,
                           pOutFile,
    ...
    if (name) {
      ...
      trfprintf(
        pOutFile, "%s\t" POINTER_PRINTF_FORMAT, fullOpCodeName(instr), imm);
    }
  }
  dumpDependencies(pOutFile, instr);
  trfflush(pOutFile);
...
void
TR_Debug::printARMHelperBranch(TR::SymbolReference* symRef,
                               uint8_t* bufferPos,
                               TR::FILE* pOutFile,
  ...
  switch (op->_opType) {
    ...
    case ARMOp2Immed8r:
      if (op->_immedShift)
        trfprintf(pOutFile,
                  "#0x%08x (0x%x << %d)",
                  op->_shiftOrImmed << op->_immedShift,
                  op->_shiftOrImmed,
                  op->_immedShift);
      else
...
// TODO FLOAT
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile,
            "dd\t" POINTER_PRINTF_FORMAT "\t\t; return address",
            snippet->getCallRA());
  bufferPos += 4;
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARMUnresolvedCallSnippet* snippet)
{
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile,
            "dd\t0x%08x\t\t; offset|flag|cpIndex",
            (helperLookupOffset << 24) | methodSymRef->getCPIndex());
  bufferPos += 4;

  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile,
            "dd\t" POINTER_PRINTF_FORMAT "\t\t; cpAddress",
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARMInterfaceCallSnippet* snippet)
{
  ...
  const char* comment[] = { "resolved interface class",
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARMHelperCallSnippet* snippet)
{
  ...
  if (restartLabel) {
    ...
    trfprintf(pOutFile,
              "b \t" POINTER_PRINTF_FORMAT "\t\t; Return to %s",
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARMStackCheckFailureSnippet* snippet)
{
  ...
  if (constantIsImmed8r(frameSize, &base, &rotate)) {
    ...
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(
      pOutFile, "add\tgr11, gr11, #0x%08x", (frameSize >> 8 & 0xFF) << 8);
    bufferPos += 4;
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(
      pOutFile, "add\tgr11, gr11, #0x%08x", (frameSize >> 16 & 0xFF) << 16);
    bufferPos += 4;
...
TR_Debug::print(TR::FILE* pOutFile, TR::UnresolvedDataSnippet* snippet)
{
  ...
  printARMHelperBranch(
    _cg->getSymRef(snippet->getHelper()), bufferPos, pOutFile);
  bufferPos += 4;
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile,
            "dd\t0x%08x\t\t; memory reference offset",
            symRef->getSymbol()->isConstObjectRef() ? 0 : mr->getOffset());
  bufferPos += 4;

  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile,
            "dd\t0xe3a0%x000\t\t; instruction template",
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARMRecompilationSnippet* snippet)
{
  ...
  trfprintf(pOutFile,
            "dd \t0x%08x\t\t;%s",
            _comp->getRecompilationInfo()->getMethodInfo(),
            "methodInfo");
  cursor += 4;
...
void
TR_Debug::printARMDelayedOffsetInstructions(TR::FILE* pOutFile,
                                            TR::ARMMemInstruction* instr)
{
  ...
  sprintf(
    regName, "gr%d", (*(uint32_t*)bufferPos >> TR::RealRegister::pos_RD) & 0xf);
  ...
  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(pOutFile,
            "mov\t%s, 0x%08x (0x%x << 24)",
            regName,
            localVal.getByte3() << 24,
            localVal.getByte3());
  bufferPos += 4;

  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(pOutFile,
            "add\t%s, %s, 0x%08x (0x%x << 16)",
            regName,
            regName,
            localVal.getByte2() << 16,
            localVal.getByte2());
  bufferPos += 4;

  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(pOutFile,
            "add\t%s, %s, 0x%08x (0x%x << 8)",
            regName,
            regName,
            localVal.getByte1() << 8,
            localVal.getByte1());
  ...
  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(
    pOutFile, "add\t%s, %s, 0x%08x", regName, regName, localVal.getByte0());
  bufferPos += 4;
