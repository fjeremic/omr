...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARMAdminInstruction* instr)
{
    ...
    if (instr->getBinaryEncoding() && instr->getNode()->getOpCodeValue() != TR::BBStart && instr->getNode()->getOpCodeValue() != TR::BBEnd)
        return;
...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARMImmSymInstruction* instr)
{
    ...
    bool longJump = imm && _cg->directCallRequiresTrampoline((intptrj_t)imm, (intptrj_t)bufferPos) && !_comp->isRecursiveMethodTarget(sym);
...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARMTrg1Src2Instruction* instr)
{
    ...
    if (instr->getOpCodeValue() == ARMOp_fmrs) {
        ...
        trfprintf(pOutFile, ", ");
        print(pOutFile, instr->getSource1Register(), TR_WordReg);
    } else if (instr->getOpCodeValue() == ARMOp_fcvtds || instr->getOpCodeValue() == ARMOp_fsitod || instr->getOpCodeValue() == ARMOp_fuitod) {
        print(pOutFile, instr->getTarget1Register(), TR_DoubleReg);
        trfprintf(pOutFile, ", ");
        print(pOutFile, instr->getSource2Operand(), TR_WordReg);
    } else if (instr->getOpCodeValue() == ARMOp_fcvtsd || instr->getOpCodeValue() == ARMOp_ftosizd || instr->getOpCodeValue() == ARMOp_ftouizd) {
        print(pOutFile, instr->getTarget1Register(), TR_WordReg);
        trfprintf(pOutFile, ", ");
        print(pOutFile, instr->getSource2Operand(), TR_DoubleReg);
...
{
    if (instr->getBinaryEncoding() && instr->getMemoryReference()->hasDelayedOffset() && !instr->getMemoryReference()->getUnresolvedSnippet()) {
...
void TR_Debug::printARMHelperBranch(TR::SymbolReference* symRef, uint8_t* bufferPos, TR::FILE* pOutFile, const char* opcodeName)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARMStackCheckFailureSnippet* snippet)
{
    ...
    bool saveLR = (_cg->getSnippetList().size() <= 1 && !bodySymbol->isEHAware() && !_cg->machine()->getLinkRegisterKilled()) ? true : false;
...
void TR_Debug::printARMDelayedOffsetInstructions(TR::FILE* pOutFile, TR::ARMMemInstruction* instr)
{
    ...
    if (op == ARMOp_str || op == ARMOp_strh || op == ARMOp_strb || toRealRegister(instr->getMemoryDataRegister())->getRegisterNumber() == base->getRegisterNumber()) {
        regSpilled = true;
