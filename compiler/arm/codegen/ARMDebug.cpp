...
void TR_Debug::dumpDependencyGroup(TR::FILE* pOutFile, TR_ARMRegisterDependencyGroup* group, int32_t numConditions,
...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARMImmSymInstruction* instr)
{
  ...
  bool longJump = imm && _cg->directCallRequiresTrampoline((intptrj_t)imm, (intptrj_t)bufferPos)
...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARMTrg1Src2Instruction* instr)
{
  ...
  if (instr->getOpCodeValue() == ARMOp_fmrs) {
    ...
    trfprintf(pOutFile, ", ");
    print(pOutFile, instr->getSource1Register(), TR_WordReg);
  } else if (instr->getOpCodeValue() == ARMOp_fcvtds || instr->getOpCodeValue() == ARMOp_fsitod
  ...
  } else if (instr->getOpCodeValue() == ARMOp_fcvtsd || instr->getOpCodeValue() == ARMOp_ftosizd
    || instr->getOpCodeValue() == ARMOp_ftouizd) {
    print(pOutFile, instr->getTarget1Register(), TR_WordReg);
    trfprintf(pOutFile, ", ");
    print(pOutFile, instr->getSource2Operand(), TR_DoubleReg);
  } else if (instr->getOpCodeValue() == ARMOp_fcmpzd || instr->getOpCodeValue() == ARMOp_fcmpzs) {
...
void TR_Debug::print(TR::FILE* pOutFile, TR_ARMOperand2* op, TR_RegisterSizes size)
{
  ...
  case ARMOp2Immed8r:
    if (op->_immedShift)
      trfprintf(
        pOutFile, "#0x%08x (0x%x << %d)", op->_shiftOrImmed << op->_immedShift, op->_shiftOrImmed, op->_immedShift);
    else
...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARMHelperCallSnippet* snippet)
{
  ...
  if (restartLabel) {
    ...
    trfprintf(pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t\t; Return to %s", (intptrj_t)(restartLabel->getCodeLocation()),
...
void TR_Debug::print(TR::FILE* pOutFile, TR::UnresolvedDataSnippet* snippet)
{
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(
    pOutFile, "dd\t0x%08x\t\t; memory reference offset", symRef->getSymbol()->isConstObjectRef() ? 0 : mr->getOffset());
  bufferPos += 4;
...
static const char* opCodeToNameMap[] = {
  ...
  "smull_r", "umull", "umull_r", "smlal", "smlal_r", "umlal", "umlal_r", "mov", "mov_r", "mvn", "mvn_r", "orr", "orr_r",
  "teq", "tst", "sub", "sub_r", "sbc", "sbc_r", "rsb", "rsb_r", "rsc", "rsc_r", "stfs", "stfd", "str", "strb", "strh",
  "stm", "stmdb", "swp", "sxtb", "sxth", "uxtb", "uxth", "fence", "ret", "wrtbar", "proc", "dd", "dmb_v6", "dmb",
  "dmb_st", "ldrex", "strex", "iflong", "setblong", "setbool", "setbflt", "lcmp", "flcmpl", "flcmpg", "idiv", "irem",
  "label", "vgdnop", "nop",
#if defined(__VFP_FP__) && !defined(__SOFTFP__)
