...
static const char *ARMConditionNames[] = {"eq", "ne", "cs", "cc", "mi", "pl",
                                          "vs", "vc", "hi", "ls", "ge", "lt",
                                          "gt", "le", "al", "nv"};
...
static const char *getExtraVFPInstrSpecifiers(TR_ARMOpCode *opCode) {
#define FIRST_VFP_INSTR ARMOp_fabsd

#if defined(__ARM_ARCH_7A__) && defined(__VFP_FP__) && !defined(__SOFTFP__)
  uint32_t index =
      (uint32_t)opCode->getOpCodeValue() - (uint32_t)(FIRST_VFP_INSTR);
...
char *TR_Debug::fullOpCodeName(TR::Instruction *instr) {
  ...
  sprintf(nameBuf, "%s%s%s", getOpCodeName(&instr->getOpCode()),
          (instr->getConditionCode() != ARMConditionCodeAL)
              ? ARMConditionNames[instr->getConditionCode()]
              : "",
          instr->getOpCode().isVFPOp()
              ? getExtraVFPInstrSpecifiers(&instr->getOpCode())
              : "");
#else
...
void TR_Debug::dumpDependencyGroup(TR::FILE *pOutFile,
  ...
  for (i = 0; i < numConditions; ++i) {
    ...
    if (omitNullDependencies && !virtReg) continue;

    TR::RealRegister::RegNum r =
        group->getRegisterDependency(i)->getRealRegister();
...
void TR_Debug::dumpDependencies(TR::FILE *pOutFile, TR::Instruction *instr) {
  ...
  if (pOutFile == NULL || _cg->getStackAtlas()) return;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMLabelInstruction *instr) {
  ...
  } else if (instr->getOpCodeValue() == ARMOp_b ||
             instr->getOpCodeValue() == ARMOp_bl) {
    trfprintf(pOutFile, "%s\t", fullOpCodeName(instr));
    print(pOutFile, label);
    if (snippet) trfprintf(pOutFile, " (%s)", getName(snippet));
  } else {
    trfprintf(pOutFile, "%s\t", fullOpCodeName(instr));
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::ARMVirtualGuardNOPInstruction *instr) {
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s Site:" POINTER_PRINTF_FORMAT ", ",
            getOpCodeName(&instr->getOpCode()), instr->getSite());
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMAdminInstruction *instr) {
  // Omit admin instructions from post-binary dumps unless they mark basic block
  // boundaries.
  ...
  if (instr->getOpCodeValue() == ARMOp_fence && instr->getFenceNode()) {
    ...
    for (uint32_t i = 0; i < fenceNode->getNumRelocations(); i++) {
      trfprintf(pOutFile, " " POINTER_PRINTF_FORMAT,
                fenceNode->getRelocationDestination(i));
    }
    trfprintf(pOutFile, " ]");
  } else {
    trfprintf(pOutFile, "\t\t");
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMImmInstruction *instr) {
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s\t0x%08x", fullOpCodeName(instr),
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMImmSymInstruction *instr) {
  ...
  if (bufferPos != NULL && longJump) {
    if (*(int32_t *)bufferPos == 0xe1a0e00f) {
      ...
      if (name)
        trfprintf(pOutFile, "DCD\t0x%x\t; %s (" POINTER_PRINTF_FORMAT ")", imm,
                  name);
      else
        trfprintf(pOutFile, "DCD\t0x%x", imm);
      return;
    } else {
      printARMHelperBranch(instr->getSymbolReference(), bufferPos, pOutFile,
                           fullOpCodeName(instr));
    }
  } else {
    const char *name = getName(symRef);
    printPrefix(pOutFile, instr);
    if (name) {
      ...
      TR::Snippet *snippet =
          sym->getLabelSymbol() ? sym->getLabelSymbol()->getSnippet() : NULL;
      if (snippet)
        trfprintf(pOutFile, "(%s)", getName(snippet));
      else
        trfprintf(pOutFile, "(" POINTER_PRINTF_FORMAT ")", imm);
    } else {
      trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT, fullOpCodeName(instr),
                imm);
    }
  }
  dumpDependencies(pOutFile, instr);
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMTrg1Src2Instruction *instr) {
  ...
  TR_RegisterSizes source1Size =
      instr->doubleFPOp() ? TR_DoubleReg : TR_WordReg;
  TR_RegisterSizes source2Size =
      instr->doubleFPOp() ? TR_DoubleReg : TR_WordReg;
#else
  ...
  if (instr->getOpCodeValue() == ARMOp_fmrs) {
    ...
    print(
        pOutFile, instr->getTarget1Register(),
        (instr->getOpCodeValue() == ARMOp_fcmpzd) ? TR_DoubleReg : TR_WordReg);
#if defined(__ARM_ARCH_7A__) && defined(__VFP_FP__) && !defined(__SOFTFP__)
  ...
  if (!instr->getOpCode().isVFPOp()) {
    for (int i = 0; regList && i < 16; i++) {
      if (regList & 1) {
        ...
        regList >>= 1;
        if (regList && i != 15) trfprintf(pOutFile, ",");
      } else
        regList >>= 1;
    }
  } else {
    ...
    for (int i = 0; i < nreg; i++) {
      TR::RealRegister::RegNum r = (TR::RealRegister::RegNum)(
          (int)TR::RealRegister::FirstFPR + startReg + i);
      print(pOutFile, machine->getRealRegister(r), size);
      if (i != (nreg - 1)) trfprintf(pOutFile, ",");
    }
  }
  trfprintf(pOutFile, "}");
...
void TR_Debug::printARMHelperBranch(TR::SymbolReference *symRef,
                                    uint8_t *bufferPos, TR::FILE *pOutFile,
  ...
  if (_cg->directCallRequiresTrampoline(target, (intptrj_t)bufferPos)) {
    ...
    if (refNum < TR_ARMnumRuntimeHelpers) {
      target = TR::CodeCacheManager::instance()->findHelperTrampoline(
          refNum, (void *)bufferPos);
    ...
    } else if (*((uintptr_t *)bufferPos) == 0xe28fe004)  // This is a JNI method
    {
      ...
      target =
          _comp->fe()->methodTrampolineLookup(_comp, symRef, (void *)bufferPos);
      info = " through trampoline";
    }
  }
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  if (name)
    trfprintf(pOutFile, "%s\t%s\t;%s (" POINTER_PRINTF_FORMAT ")", opcodeName,
              name, info, target);
  else
    trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t;%s", opcodeName,
              target, info);
}
...
void TR_Debug::print(TR::FILE *pOutFile, TR::MemoryReference *mr) {
  ...
  if (mr->getBaseRegister() != NULL) {
    ...
    if (mr->isPostIndexed()) trfprintf(pOutFile, "]");
    trfprintf(pOutFile, ", ");
  }
...
void TR_Debug::print(TR::FILE *pOutFile, TR_ARMOperand2 *op,
  ...
  switch (op->_opType) {
    ...
    case ARMOp2Immed8r:
      if (op->_immedShift)
        trfprintf(pOutFile, "#0x%08x (0x%x << %d)",
                  op->_shiftOrImmed << op->_immedShift, op->_shiftOrImmed,
                  op->_immedShift);
      else
...
void TR_Debug::printARMGCRegisterMap(TR::FILE *pOutFile,
  ...
  for (int i = 15; i >= 0; i--) {
    if (map->getMap() & (1 << i))
      trfprintf(pOutFile, "%s ",
                getName(machine->getRealRegister((TR::RealRegister::RegNum)(
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMCallSnippet *snippet) {
  ...
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    getName(snippet), getName(methodSymRef));

  TR::Machine *machine = _cg->machine();
  const TR::ARMLinkageProperties &linkage =
      _cg->getLinkage(methodSymbol->getLinkageConvention())->getProperties();
  ...
  for (int i = callNode->getFirstArgumentIndex();
    ...
    switch (callNode->getChild(i)->getOpCode().getDataType()) {
      ...
      case TR::Address:
        ...
        break;
      case TR::Int64:
        if (!linkage.getRightToLeft()) offset -= 8;
        if (numIntArgs < linkage.getNumIntArgRegs()) {
          printPrefix(pOutFile, NULL, bufferPos, 4);
          trfprintf(pOutFile, "str\t[gr7, %+d], ", offset);
          print(pOutFile, machine->getRealRegister(
          ...
          if (numIntArgs < linkage.getNumIntArgRegs() - 1) {
            ...
            print(pOutFile,
                  machine->getRealRegister(
                      linkage.getIntegerArgumentRegister(numIntArgs + 1)));
            bufferPos += 4;
          }
        }
        numIntArgs += 2;
...
// TODO FLOAT
  ...
  if (methodSymRef->isUnresolved())
    ...
    trfprintf(pOutFile, "dd\t" POINTER_PRINTF_FORMAT
                        "\t\t; method address (interpreted)");
  bufferPos += 4;

  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile,
            "dd\t" POINTER_PRINTF_FORMAT "\t\t; lock word for compilation");
#endif
}

void TR_Debug::print(TR::FILE *pOutFile,
                     TR::ARMUnresolvedCallSnippet *snippet) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile, "dd\t0x%08x\t\t; offset|flag|cpIndex",
            (helperLookupOffset << 24) | methodSymRef->getCPIndex());
  bufferPos += 4;

  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile, "dd\t" POINTER_PRINTF_FORMAT "\t\t; cpAddress",
            getOwningMethod(methodSymRef)->constantPool());
#endif
}

void TR_Debug::print(TR::FILE *pOutFile,
                     TR::ARMVirtualUnresolvedSnippet *snippet) {
#ifdef J9_PROJECT_SPECIFIC
  TR::SymbolReference *callSymRef = snippet->getNode()->getSymbolReference();

  uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    getName(snippet), getName(callSymRef));
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile, "dd\t" POINTER_PRINTF_FORMAT "\t\t; cpAddress",
            getOwningMethod(callSymRef)->constantPool());
  bufferPos += 4;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMInterfaceCallSnippet *snippet) {
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile, "dd\t0x%08x\t\t; return address",
            (uintptr_t)snippet->getReturnLabel()->getCodeLocation());
  bufferPos += 4;

  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile, "dd\t0x%08x\t\t; cpAddress",
            (uintptr_t)getOwningMethod(callSymRef)->constantPool());
  bufferPos += 4;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMHelperCallSnippet *snippet) {
  ...
  if (restartLabel) {
    ...
    trfprintf(pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t\t; Return to %s",
              (intptrj_t)(restartLabel->getCodeLocation()),
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::ARMStackCheckFailureSnippet *snippet) {
  uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    getName(snippet));
  ...
  TR::SymbolReference *sofRef =
      _comp->getSymRefTab()->element(TR_stackOverflow);

  bool saveLR =
      (_cg->getSnippetList().size() <= 1 && !bodySymbol->isEHAware() &&
       !_cg->machine()->getLinkRegisterKilled())
          ? true
          : false;
  ...
  if (constantIsImmed8r(frameSize, &base, &rotate)) {
    ...
    bufferPos += 4;
  } else if (!constantIsImmed8r(frameSize - offset, &base, &rotate) &&
    ...
    bufferPos += 4;
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(pOutFile, "add\tgr11, gr11, #0x%08x",
              (frameSize >> 8 & 0xFF) << 8);
    bufferPos += 4;
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(pOutFile, "add\tgr11, gr11, #0x%08x",
              (frameSize >> 16 & 0xFF) << 16);
    bufferPos += 4;
    // There is no need in another add, since a stack frame can never be that
    // big.
  }
...
void TR_Debug::print(TR::FILE *pOutFile, TR::UnresolvedDataSnippet *snippet) {
  ...
  uint8_t *bufferPos = snippet->getSnippetLabel()->getCodeLocation();
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    getName(snippet), getName(symRef));

  printARMHelperBranch(_cg->getSymRef(snippet->getHelper()), bufferPos,
                       pOutFile);
  bufferPos += 4;

  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile, "dd\t" POINTER_PRINTF_FORMAT "\t\t; return address",
            snippet->getAddressOfDataReference());
  bufferPos += 4;
  ...
  printPrefix(pOutFile, NULL, bufferPos, 4);
  trfprintf(pOutFile, "dd\t" POINTER_PRINTF_FORMAT "\t\t; cpAddress",
            getOwningMethod(symRef)->constantPool());
  bufferPos += 4;
  ...
  trfprintf(pOutFile, "dd\t0xe3a0%x000\t\t; instruction template",
            toRealRegister(mr->getModBase())->getRegisterNumber() -
...
void TR_Debug::print(TR::FILE *pOutFile, TR::ARMRecompilationSnippet *snippet) {
  ...
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), cursor,
                    "Counting Recompilation Snippet");
  ...
  trfprintf(pOutFile, "dd \t0x%08x\t\t;%s",
            _comp->getRecompilationInfo()->getMethodInfo(), "methodInfo");
  cursor += 4;
...
void TR_Debug::printARMDelayedOffsetInstructions(TR::FILE *pOutFile,
  ...
  TR::RealRegister *base =
      toRealRegister(instr->getMemoryReference()->getBaseRegister());

  intParts localVal(offset);
  char *regName = (char *)_comp->trMemory()->allocateHeapMemory(6);
  sprintf(regName, "gr%d",
          (*(uint32_t *)bufferPos >> TR::RealRegister::pos_RD) & 0xf);

  if (op == ARMOp_str || op == ARMOp_strh || op == ARMOp_strb ||
      toRealRegister(instr->getMemoryDataRegister())->getRegisterNumber() ==
  ...
  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(pOutFile, "mov\t%s, 0x%08x (0x%x << 24)", regName,
            localVal.getByte3() << 24, localVal.getByte3());
  bufferPos += 4;

  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(pOutFile, "add\t%s, %s, 0x%08x (0x%x << 16)", regName, regName,
            localVal.getByte2() << 16, localVal.getByte2());
  bufferPos += 4;

  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(pOutFile, "add\t%s, %s, 0x%08x (0x%x << 8)", regName, regName,
            localVal.getByte1() << 8, localVal.getByte1());
  bufferPos += 4;

  printPrefix(pOutFile, instr, bufferPos, 4);
  trfprintf(pOutFile, "add\t%s, %s, 0x%08x", regName, regName,
            localVal.getByte0());
  bufferPos += 4;
...
static const char *opCodeToNameMap[] = {
    "bad",     "add",     "add_r",  "adc",     "adc_r",  "and",      "and_r",
    "b",       "bl",      "bic",    "bic_r",   "cmp",    "cmn",      "eor",
    "eor_r",   "ldfs",    "ldfd",   "ldm",     "ldmia",  "ldr",      "ldrb",
    "ldrsb",   "ldrh",    "ldrsh",  "mul",     "mul_r",  "mla",      "mla_r",
    "smull",   "smull_r", "umull",  "umull_r", "smlal",  "smlal_r",  "umlal",
    "umlal_r", "mov",     "mov_r",  "mvn",     "mvn_r",  "orr",      "orr_r",
    "teq",     "tst",     "sub",    "sub_r",   "sbc",    "sbc_r",    "rsb",
    "rsb_r",   "rsc",     "rsc_r",  "stfs",    "stfd",   "str",      "strb",
    "strh",    "stm",     "stmdb",  "swp",     "sxtb",   "sxth",     "uxtb",
    "uxth",    "fence",   "ret",    "wrtbar",  "proc",   "dd",       "dmb_v6",
    "dmb",     "dmb_st",  "ldrex",  "strex",   "iflong", "setblong", "setbool",
    "setbflt", "lcmp",    "flcmpl", "flcmpg",  "idiv",   "irem",     "label",
    "vgdnop",  "nop",
#if defined(__VFP_FP__) && !defined(__SOFTFP__)
...
const char *TR_Debug::getOpCodeName(TR_ARMOpCode *opCode) {
