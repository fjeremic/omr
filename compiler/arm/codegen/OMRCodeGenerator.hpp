...
extern TR::Instruction *armLoadConstant(TR::Node *node, int32_t value,
                                        TR::Register *targetRegister,
                                        TR::CodeGenerator *codeGen,
                                        TR::Instruction *cursor = NULL);

extern TR::Instruction *
loadAddressConstantInSnippet(TR::CodeGenerator *cg, TR::Node *node,
                             intptrj_t address, TR::Register *targetRegister,
                             bool isUnloadablePicSite = false,
                             TR::Instruction *cursor = NULL);

extern TR::Instruction *loadAddressConstantFixed(
    ...
    int16_t typeAddress = -1, bool doAOTRelocation = true);

extern TR::Instruction *
loadAddressConstant(TR::CodeGenerator *cg, TR::Node *node, intptrj_t value,
                    TR::Register *targetRegister,
                    TR::Instruction *cursor = NULL, bool isPicSite = false,
                    int16_t typeAddress = -1, uint8_t *targetAddress = NULL,
...
extern uint8_t *storeArgumentItem(TR_ARMOpCodes op, uint8_t *buffer,
                                  TR::RealRegister *reg, int32_t offset,
                                  TR::CodeGenerator *cg);
extern uint8_t *loadArgumentItem(TR_ARMOpCodes op, uint8_t *buffer,
                                 TR::RealRegister *reg, int32_t offset,
                                 TR::CodeGenerator *cg);
...
public:
  ...
  int32_t findOrCreateAddressConstant(void *v, TR::DataType t,
                                      ...
                                      TR::Instruction *n4, TR::Node *node,
                                      bool isUnloadablePicSite);

  TR::Register *gprClobberEvaluate(TR::Node *node);

  const TR::ARMLinkageProperties &getProperties() {
    return *_linkageProperties;
  }

  TR::RealRegister *getFrameRegister() { return _frameRegister; }
  TR::RealRegister *setFrameRegister(TR::RealRegister *r) {
    return (_frameRegister = r);
  }
  ...
  int64_t getLargestNegConstThatMustBeMaterialized() {
    return -32769;
  } // minimum 16-bit signed int minus 1
  ...
  // This is needed by register simulation pickRegister in CodeGenRA common
  // code.
  TR_GlobalRegisterNumber
  pickRegister(TR_RegisterCandidate *, TR::Block **,
               TR_BitVector &availableRegisters,
               TR_GlobalRegisterNumber &globalRegisterNumber,
  ...
  List<TR_ARMOutOfLineCodeSection> &getARMOutOfLineCodeSectionList() {
