...
extern TR::Instruction* armLoadConstant(TR::Node* node, int32_t value, TR::Register* targetRegister,
  TR::CodeGenerator* codeGen, TR::Instruction* cursor = NULL);
...
extern TR::Instruction* loadAddressConstantFixed(TR::CodeGenerator* cg, TR::Node* node, intptrj_t value,
...
extern uint8_t* storeArgumentItem(
  TR_ARMOpCodes op, uint8_t* buffer, TR::RealRegister* reg, int32_t offset, TR::CodeGenerator* cg);
...
public:
  ...
  bool hasHardFloatReturn()
  {
    return _flags.testAny(HasHardFloatReturn);
  }
  ...
  void setHasHardFloatReturn()
  {
    _flags.set(HasHardFloatReturn);
  }
  void setHasCall()
  {
    _flags.set(HasCall);
  ...
  TR_GlobalRegisterNumber pickRegister(TR_RegisterCandidate*, TR::Block**, TR_BitVector& availableRegisters,
    TR_GlobalRegisterNumber& globalRegisterNumber, TR_LinkHead<TR_RegisterCandidate>* candidates);
