...
OMR::ARM::Instruction::Instruction(TR::Instruction* precedingInstruction, TR_ARMOpCodes op, TR::Node* node,
...
TR::ARMConditionalBranchInstruction* OMR::ARM::Instruction::getARMConditionalBranchInstruction()
{
  return NULL;
}
...
void TR::ARMConditionalBranchInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
  ...
  if (getLabelSymbol()->isStartOfColdInstructionStream()) {
    ...
    TR_ASSERT(
      oi, "Could not find ARMOutOfLineCodeSection stream from label.  instr=%p, label=%p\n", this, getLabelSymbol());
    if (cg()->getDebug())
...
TR::ARMImmSymInstruction::ARMImmSymInstruction(TR_ARMOpCodes op, TR::Node* node, uint32_t imm,
...
TR::ARMImmSymInstruction::ARMImmSymInstruction(TR::Instruction* precedingInstruction, TR_ARMOpCodes op, TR::Node* node,
  uint32_t imm, TR::RegisterDependencyConditions* cond, TR::SymbolReference* sr, TR::CodeGenerator* cg, TR::Snippet* s,
  TR_ARMConditionCode cc)
...
static TR::Instruction* expandBlongLessThan(TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* cursor,
  TR::Register* temp, TR::Register* result, TR::Register* aHi, TR::Register* aLo, TR::Register* bHi, TR::Register* bLo,
  bool trueValue)
{
...
void TR::ARMControlFlowInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
  ...
  for (i = 0; i < getNumTargets(); i++) {
    TR::Register* targetVirtual = getTargetRegister(i);
    // Because of OOL codegen, if the target virtual is defined at this instruction and this instruction is in the hot
    // path of an OOL section, the virtual's future use count may still be >0 at this point because of outstsanding uses
    // in the cold path. Therefore we have to check if this is the start of it's live range and unlatch it.
    targetVirtual->unblock();
  ...
  case ARMOp_setblong:
    ...
    case ARMConditionCodeNE:
      ...
      cursor
        = generateSrc2Instruction(cg(), ARMOp_cmp, currentNode, getSourceRegister(0), getSourceRegister(2), cursor);
      cursor
        = generateSrc2Instruction(cg(), ARMOp_cmp, currentNode, getSourceRegister(1), getSourceRegister(3), cursor);
  ...
  case ARMOp_setbflt:
    ...
    (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor, ARMOp_label, currentNode, label2, cg()); break;
    */
  case ARMOp_lcmp:
    /*
     * correct:
    ...
    /*
       ...
       TR::ARMConditionalBranchInstruction(cursor, ARMOp_beq, currentNode, label2, getTargetRegister(0), cg()); cursor =
       new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), -1,
       cg()); cursor = new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor, ARMOp_label, currentNode, label2,
       cg()); break; case ARMOp_flcmpg: cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor,
       ARMOp_fcmpu, currentNode, getTargetRegister(0), getSourceRegister(0), getSourceRegister(2), cg()); cursor = new
       (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), -1, cg());
             cursor = new (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor, ARMOp_blt, currentNode,
       label2, getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor,
       ARMOp_li, currentNode, getTargetRegister(1), 0, cg()); cursor = new (cg()->trHeapMemory())
       TR::ARMConditionalBranchInstruction(cursor, ARMOp_beq, currentNode, label2, getTargetRegister(0), cg()); cursor =
       new (cg()->trHeapMemory()) TR::ARMTrg1ImmInstruction(cursor, ARMOp_li, currentNode, getTargetRegister(1), 1,
       cg()); cursor = new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor, ARMOp_label, currentNode, label2,
       cg()); break; case ARMOp_idiv: cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_eqv,
       currentNode, getTargetRegister(2), getTargetRegister(1), getSourceRegister(1), cg()); cursor = new
       (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_and, currentNode, getTargetRegister(2),
       getSourceRegister(2), getTargetRegister(2), cg()); cursor = new (cg()->trHeapMemory())
       TR::ARMTrg1Src1ImmInstruction(cursor, ARMOp_cmpi4, currentNode, getTargetRegister(0), getTargetRegister(2), -1,
       cg()); cursor = new (cg()->trHeapMemory()) TR::ARMConditionalBranchInstruction(cursor, ARMOp_beq, currentNode,
       label2, getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor,
       ARMOp_divw, currentNode, getTargetRegister(1), getSourceRegister(1), getSourceRegister(2), cg()); cursor = new
       (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor, ARMOp_label, currentNode, label2, cg()); break; case
       ARMOp_irem: cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src1ImmInstruction(cursor, ARMOp_cmpi4, currentNode,
       ...
       getTargetRegister(0), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_divw,
       currentNode, getTargetRegister(2), getSourceRegister(1), getSourceRegister(2), cg()); cursor = new
       (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(cursor, ARMOp_mullw, currentNode, getTargetRegister(2),
       getSourceRegister(2), getTargetRegister(2), cg()); cursor = new (cg()->trHeapMemory())
       TR::ARMTrg1Src2Instruction(cursor, ARMOp_subf, currentNode, getTargetRegister(1), getTargetRegister(2),
       getSourceRegister(1), cg()); cursor = new (cg()->trHeapMemory()) TR::ARMLabelInstruction(cursor, ARMOp_label,
