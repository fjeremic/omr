...
OMR::ARM::Instruction::Instruction(TR::CodeGenerator *cg,
                                   TR::InstOpCode::Mnemonic op, TR::Node *node)
    : OMR::Instruction(cg, op, node), _conditions(0), _asyncBranch(false) {
  self()->setBlockIndex(cg->getCurrentBlockIndex());
}
...
OMR::ARM::Instruction::Instruction(TR_ARMOpCodes op, TR::Node *node,
                                   TR::CodeGenerator *cg)
    : OMR::InstructionConnector(cg, TR::InstOpCode::BAD, node) {
...
OMR::ARM::Instruction::Instruction(TR_ARMOpCodes op, TR::Node *node,
  ...
  if (cond) cond->incRegisterTotalUseCounts(cg);
}
...
int32_t OMR::ARM::Instruction::getMachineOpCode() {
...
void TR::ARMConditionalBranchInstruction::assignRegisters(
...
TR::ARMImmSymInstruction::ARMImmSymInstruction(
    TR_ARMOpCodes op, TR::Node *node, uint32_t imm,
    TR::RegisterDependencyConditions *cond, TR::SymbolReference *sr,
...
TR::ARMImmSymInstruction::ARMImmSymInstruction(
    TR::Instruction *precedingInstruction, TR_ARMOpCodes op, TR::Node *node,
    uint32_t imm, TR::RegisterDependencyConditions *cond,
    TR::SymbolReference *sr, TR::CodeGenerator *cg, TR::Snippet *s,
...
void TR::ARMLabelInstruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
...
void TR::ARMTrg1Src2Instruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  TR::Machine *machine = cg()->machine();
  TR::Register *target1Virtual = getTarget1Register();
  TR::Register *source1Virtual = getSource1Register();
...
void TR::ARMTrg2Src1Instruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  TR::Machine *machine = cg()->machine();
  TR::Register *target1Virtual = getTarget1Register();
...
void TR::ARMMemSrc1Instruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  TR::Machine *machine = cg()->machine();
...
void TR::ARMTrg1Instruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  if (getDependencyConditions())
    getDependencyConditions()->assignPostConditionRegisters(
        this, kindToBeAssigned, cg());
...
void TR::ARMTrg1MemInstruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  TR::Register *targetVirtual = getTargetRegister();
  TR::Register *mBaseVirtual = getMemoryReference()->getModBase();
...
void TR::ARMTrg1MemSrc1Instruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  TR::Register *targetVirtual = getTargetRegister();
  TR::Register *mBaseVirtual = getMemoryReference()->getModBase();
  TR::Register *sourceVirtual = getSourceRegister();
...
bool TR::ARMControlFlowInstruction::refsRegister(TR::Register *reg) {
  ...
  for (i = 0; i < getNumTargets(); i++) {
    if (reg == getTargetRegister(i)) return true;
  }
  for (i = 0; i < getNumSources(); i++) {
    if (reg == getSourceRegister(i)) return true;
  }
  return false;
}
...
bool TR::ARMControlFlowInstruction::defsRealRegister(TR::Register *reg) {
  for (int i = 0; i < getNumTargets(); i++) {
    if (reg == getTargetRegister(i)->getAssignedRegister()) return true;
  }
  return false;
}
...
static TR::Instruction *expandBlongLessThan(
    TR::Node *node, TR::CodeGenerator *cg, TR::Instruction *cursor,
    TR::Register *temp, TR::Register *result, TR::Register *aHi,
    TR::Register *aLo, TR::Register *bHi, TR::Register *bLo, bool trueValue) {
  ...
  // are zero!).
  cursor = generateTrg1ImmInstruction(cg, ARMOp_mov, node, result, !trueValue,
                                      0, cursor);
  cursor = generateTrg1Src2Instruction(cg, ARMOp_sub_r, node, temp, aLo, bLo,
                                       cursor);
  cursor = generateTrg1Src2Instruction(cg, ARMOp_sbc_r, node, temp, aHi, bHi,
                                       cursor);
  cursor = generateTrg1ImmInstruction(cg, ARMOp_mov, node, result, trueValue, 0,
                                      cursor);
  cursor->setConditionCode(ARMConditionCodeLT);
  return cursor;
}

void TR::ARMControlFlowInstruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  TR::Machine *machine = cg()->machine();
  ...
  for (i = 0; i < getNumSources(); i++) getSourceRegister(i)->block();

  for (i = 0; i < getNumTargets(); i++) getTargetRegister(i)->block();
  ...
  switch (getOpCode().getOpCodeValue()) {
    case ARMOp_iflong: {
      ...
      cursor = generateSrc2Instruction(cg(), ARMOp_cmp, currentNode,
                                       getSourceRegister(0),
                                       getSourceRegister(2), cursor);
      cursor = generateConditionalBranchInstruction(
          cg(), currentNode, isGT ? ARMConditionCodeGT : ARMConditionCodeLT,
          getLabelSymbol(), cursor);
      cursor = generateConditionalBranchInstruction(
          cg(), currentNode, isGT ? ARMConditionCodeLT : ARMConditionCodeGT,
          label2, cursor);
      cursor = generateSrc2Instruction(cg(), ARMOp_cmp, currentNode,
                                       getSourceRegister(1),
                                       getSourceRegister(3), cursor);
      cursor = generateConditionalBranchInstruction(
          cg(), currentNode, isGT ? ARMConditionCodeHI : ARMConditionCodeLS,
          getLabelSymbol(), cursor);
      cursor = generateLabelInstruction(cg(), ARMOp_label, currentNode, label2,
                                        cursor);
    } break;
    case ARMOp_setblong:
      switch (getConditionCode()) {
        case ARMConditionCodeLT:
          cursor = expandBlongLessThan(
              currentNode, cg(), cursor, getTargetRegister(1),
              getTargetRegister(0), getSourceRegister(1), getSourceRegister(0),
              getSourceRegister(3), getSourceRegister(2), 1);
          break;
        case ARMConditionCodeGT:
          // A > B ==> B < A
          cursor = expandBlongLessThan(
              currentNode, cg(), cursor, getTargetRegister(1),
              getTargetRegister(0), getSourceRegister(3), getSourceRegister(2),
              getSourceRegister(1), getSourceRegister(0), 1);
          break;
        case ARMConditionCodeGE:
          ...
          cursor = expandBlongLessThan(
              currentNode, cg(), cursor, getTargetRegister(1),
              getTargetRegister(0), getSourceRegister(1), getSourceRegister(0),
              getSourceRegister(3), getSourceRegister(2), 0);
          break;
        case ARMConditionCodeLE:
          ...
          cursor = expandBlongLessThan(
              currentNode, cg(), cursor, getTargetRegister(1),
              getTargetRegister(0), getSourceRegister(3), getSourceRegister(2),
              getSourceRegister(1), getSourceRegister(0), 0);
          break;
    ...
    case ARMOp_lcmp:
      ...
      cursor = generateSrc2Instruction(cg(), ARMOp_cmp, currentNode,
                                       getSourceRegister(1),
                                       getSourceRegister(3), cursor);
      cursor = generateTrg1ImmInstruction(cg(), ARMOp_mov, currentNode,
                                          getTargetRegister(0), 1, 0, cursor);
      cursor->setConditionCode(ARMConditionCodeGT);
      cursor = generateTrg1ImmInstruction(cg(), ARMOp_mvn, currentNode,
                                          getTargetRegister(0), 0, 0, cursor);
      cursor->setConditionCode(ARMConditionCodeLT);
      cursor = generateConditionalBranchInstruction(
...
void TR::ARMMultipleMoveInstruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  if (getDependencyConditions())
    getDependencyConditions()->assignPostConditionRegisters(
        this, kindToBeAssigned, cg());
