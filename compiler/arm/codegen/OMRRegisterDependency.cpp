...
OMR::ARM::RegisterDependencyConditions::RegisterDependencyConditions(
    TR::Node *node, uint32_t extranum, TR::Instruction **cursorPtr,
    TR::CodeGenerator *cg) {
  ...
  _preConditions =
      TR_ARMRegisterDependencyGroup::create(totalNum, cg->trMemory());
  _postConditions =
      TR_ARMRegisterDependencyGroup::create(totalNum, cg->trMemory());
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (regList.find(reg) || (highReg && regList.find(highReg))) {
      ...
      if (regList.find(reg)) {
        ...
        cursor =
            generateTrg1Src1Instruction(cg, opCode, node, copyReg, reg, cursor);
        reg = copyReg;
      }
      ...
      if (highReg && regList.find(highReg)) {
        ...
        cursor = generateTrg1Src1Instruction(cg, opCode, node, highCopyReg,
                                             highReg, cursor);
        highReg = highCopyReg;
      }
    }
  ...
  if (cursor != NULL && cursorPtr != NULL) *cursorPtr = cursor;
}
...
void OMR::ARM::RegisterDependencyConditions::incRegisterTotalUseCounts(
    TR::CodeGenerator *cg) {
  for (int i = 0; i < _addCursorForPre; i++) {
    _preConditions->getRegisterDependency(i)->getRegister()->incTotalUseCount();
  }
  for (int j = 0; j < _addCursorForPost; j++) {
    _postConditions->getRegisterDependency(j)
        ->getRegister()
        ->incTotalUseCount();
  }
}

void TR_ARMRegisterDependencyGroup::assignRegisters(
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (dependentRegNum == TR::RealRegister::SpilledReg) {
        TR_ASSERT(virtReg->getBackingStorage(),
                  "should have a backing store if dependentRegNum == "
                  "spillRegIndex()\n");
        if (virtReg->getAssignedRealRegister()) {
          ...
          traceMsg(comp,
                   "\nOOL: Found register spilled in main line and re-assigned "
                   "inside OOL");
      ...
      // the OOL during backwards RA assignment
        ...
        TR::ARMLabelInstruction *labelInstr =
            (TR::ARMLabelInstruction *)currentInstruction;
        TR_BackingStore *location = virtReg->getBackingStorage();
        TR_RegisterKinds rk = virtReg->getKind();
        int32_t dataSize;
        if (labelInstr->getLabelSymbol()->isStartOfColdInstructionStream() &&
            location) {
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (virtReg->getAssignedRealRegister() != NULL) {
      if (dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
        ...
        dependentRegNum = toRealRegister(virtReg->getAssignedRealRegister())
                              ->getRegisterNumber();
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    if (dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
      ...
      if (realOne != NULL && excludeGPR0 &&
          toRealRegister(realOne)->getRegisterNumber() ==
              TR::RealRegister::gr0) {
        if ((assignedRegister = machine->findBestFreeRegister(
                 virtReg->getKind(), true)) == NULL) {
          assignedRegister = machine->freeBestRegister(
              currentInstruction, virtReg->getKind(), NULL, true);
        }
        ...
        if (virtReg->getTotalUseCount() == virtReg->getFutureUseCount()) {
          if ((assignedRegister = machine->findBestFreeRegister(
                   virtReg->getKind(), excludeGPR0, true)) == NULL) {
            assignedRegister = machine->freeBestRegister(
                currentInstruction, virtReg->getKind(), NULL, excludeGPR0);
          }
        } else {
          assignedRegister = machine->reverseSpillState(
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (dependentRegister->getAssignedRegister()) {
      ...
      if (dependentRegister->decFutureUseCount() == 0) {
        ...
        assignedRegister->setState(
            TR::RealRegister::Unlatched);  // Was setting to Free
      }
    }
  }
}
...
TR::RegisterDependencyConditions *OMR::ARM::RegisterDependencyConditions::clone(
  ...
  result = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(
      preNum + addPre, postNum + addPost, cg->trMemory());
  ...
  TR_ASSERT(preGPR <= max_gpr && postGPR <= max_gpr,
            "Over the limit of available global registers.");
#endif
  return result;
}

TR::RegisterDependencyConditions *
OMR::ARM::RegisterDependencyConditions::cloneAndFix(
    TR::CodeGenerator *cg, TR::RegisterDependencyConditions *added) {
  TR::RegisterDependencyConditions *result;
  TR_ARMRegisterDependency *singlePair;
  int32_t idx, preNum, postNum, addPre = 0, addPost = 0;
  ...
  for (idx = 0; idx < preNum; idx++) {
    ...
    rnum = singlePair->getRealRegister();
    tempReg = singlePair->getRegister();
    if (rnum == TR::RealRegister::gr0) tempReg = postReg;
...
void OMR::ARM::RegisterDependencyConditions::unionNoRegPostCondition(
