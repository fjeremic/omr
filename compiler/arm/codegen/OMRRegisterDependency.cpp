    ...
    if (regList.find(reg) || (highReg && regList.find(highReg))) {
      ...
      if (highReg && regList.find(highReg)) {
        ...
        cursor = generateTrg1Src1Instruction(
          cg, opCode, node, highCopyReg, highReg, cursor);
        highReg = highCopyReg;
      }
    }
...
bool
OMR::ARM::RegisterDependencyConditions::refsRegister(TR::Register* r)
{
...
bool
OMR::ARM::RegisterDependencyConditions::defsRegister(TR::Register* r)
{
  for (int i = 0; i < _addCursorForPre; i++) {
    if (_preConditions->getRegisterDependency(i)->getRegister() == r &&
...
bool
OMR::ARM::RegisterDependencyConditions::defsRealRegister(TR::Register* r)
{
  for (int i = 0; i < _addCursorForPre; i++) {
...
bool
OMR::ARM::RegisterDependencyConditions::usesRegister(TR::Register* r)
{
  for (int i = 0; i < _addCursorForPre; i++) {
    if (_preConditions->getRegisterDependency(i)->getRegister() == r &&
...
void
OMR::ARM::RegisterDependencyConditions::incRegisterTotalUseCounts(
  TR::CodeGenerator* cg)
{
  for (int i = 0; i < _addCursorForPre; i++) {
...
void
TR_ARMRegisterDependencyGroup::assignRegisters(
        ...
        if (virtReg->getAssignedRealRegister()) {
          ...
          if (comp->getDebug())
            cg->traceRegisterAssignment(
              ...
              "%p\n",
              cg->comp()->getDebug()->getName(virtReg),
              currentInstruction);
...
OMR::ARM::RegisterDependencyConditions::clone(
  TR::CodeGenerator* cg,
