...
static int32_t identifyFarConditionalBranches(int32_t estimate, TR::CodeGenerator* cg)
{
    ...
    for (int32_t i = candidateBranches.lastIndex(); i >= 0; i--) {
        ...
        {
            for (j = i - 1; j >= 0 && targetLocation <= candidateBranches[j]->getEstimatedBinaryLocation();
                 j--)
                ;
...
{
    return !TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(targetAddress, sourceAddress) || self()->comp()->getOption(TR_StressTrampolines);
}
