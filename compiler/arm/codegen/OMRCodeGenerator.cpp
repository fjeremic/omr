...
TR::Linkage*
OMR::ARM::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
  TR::Linkage* linkage;
  switch (lc) {
    case TR_Private:
  ...
  , _internalControlFlowSafeNestingDepth(0)
{
  ...
  if (TR::Compiler->target.isLinux()) {
    ...
    // With -msoft-float any floating point return value is placed in gr0 for
    // single precision and gr0 and gr1 for double precision.  This is where the
    // jit expects the result following a directToJNI call. The floating point
    // return value on non-hardhat linux-arm builds is through the coprocessor
    // register f0. Therefore the HasHardFloatReturn flag must be set to force
    // generation of instructions to move the result from f0 to gr0/gr1
    // following a directToJNI dispatch.

#if !defined(HARDHAT) &&                                                       \
  !defined(__VFP_FP__) // gcc does not support VFP with -mfloat-abi=hard yet
    self()->setHasHardFloatReturn();
#endif
  }

  if (!debug("hasFramePointer"))
    self()->setFrameRegister(self()->machine()->getRealRegister(
      _linkageProperties->getStackPointerRegister()));
  else
    self()->setFrameRegister(self()->machine()->getRealRegister(
      _linkageProperties->getFramePointerRegister()));
  ...
  self()->setLast8BitGlobalGPR(
    TR::Machine::getLast8BitGlobalGPRRegisterNumber());
  ...
  // TODO: Disable FP-GRA since current GRA does not work well with ARM linkage
  // (where Float register usage is limited).
  ...
  self()->setAheadOfTimeCompile(new (self()->trHeapMemory())
                                  TR::AheadOfTimeCompile(self()));
#endif
  self()->getLinkage()->initARMRealRegisterLinkage();
  // To enable this, we must change OMR::ARM::Linkage::saveArguments to support
  // GRA registers
                          ...
                          TR::CodeGenerator* cg)
{
  ...
  switch (methodSymbol->getMethod()->returnType()) {
    case TR::NoType:
      return sync ? TR_ARMicallVMprJavaSendStaticSync0
                  : TR_ARMicallVMprJavaSendStatic0;
    case TR::Int8:
    case TR::Int16:
    case TR::Int32:
      return sync ? TR_ARMicallVMprJavaSendStaticSync1
                  : TR_ARMicallVMprJavaSendStatic1;
    case TR::Address:
      if (TR::Compiler->target.is64Bit())
        return sync ? TR_ARMicallVMprJavaSendStaticSyncJ
                    : TR_ARMicallVMprJavaSendStaticJ;
      else
        return sync ? TR_ARMicallVMprJavaSendStaticSync1
                    : TR_ARMicallVMprJavaSendStatic1;
    case TR::Int64:
      return sync ? TR_ARMicallVMprJavaSendStaticSyncJ
                  : TR_ARMicallVMprJavaSendStaticJ;
    case TR::Float:
      return sync ? TR_ARMicallVMprJavaSendStaticSyncF
                  : TR_ARMicallVMprJavaSendStaticF;
    case TR::Double:
      return sync ? TR_ARMicallVMprJavaSendStaticSyncD
                  : TR_ARMicallVMprJavaSendStaticD;
    default:
...
TR::Instruction*
OMR::ARM::CodeGenerator::generateSwitchToInterpreterPrePrologue(
  ...
  TR::Node* node)
{
  ...
  TR::Register* lr =
    self()->machine()->getRealRegister(TR::RealRegister::gr14); // link register
  TR::ResolvedMethodSymbol* methodSymbol = comp->getJittedMethodSymbol();
  TR::SymbolReference* revertToInterpreterSymRef =
    self()->symRefTab()->findOrCreateRuntimeHelper(
      TR_ARMrevertToInterpreterGlue, false, false, false);
  ...
  cursor = new (self()->trHeapMemory())
    TR::ARMTrg1Src1Instruction(cursor, ARMOp_mov, node, gr4, lr, self());
  cursor = self()->getLinkage()->flushArguments(cursor);
  cursor = generateImmSymInstruction(
    self(),
    ARMOp_bl,
    node,
    (uintptrj_t)revertToInterpreterSymRef->getMethodAddress(),
    new (self()->trHeapMemory())
      TR::RegisterDependencyConditions((uint8_t)0, 0, self()->trMemory()),
    revertToInterpreterSymRef,
    NULL,
    cursor);
  cursor = generateImmInstruction(
    self(), ARMOp_dd, node, (int32_t)ramMethod, TR_RamMethod, cursor);
...
static void
removeGhostRegistersFromGCMaps(TR::CodeGenerator* cg,
                               TR::Instruction* branchOOL)
{
  // If a virtual is live at the end of the hot path and dead at the beginning
  // it will not be killed immediatey after it's first use in the hot path if
  // it's also used on the cold path, since RA still needs to be done on the
  // cold path (which is where it's future use count will drop to 0), which
  // means it will be incorrectly seen as live between it's first use and the
  // top of the hot path. If there are any GC points between the top of the hot
  // path and the first use the real reg holding the virtual will be included,
  // so we need to fix this.
  TR::Instruction* instr = branchOOL->getNext();
  while (!instr->isLabel() || !((TR::ARMLabelInstruction*)instr)
                                 ->getLabelSymbol()
                                 ->isEndOfColdInstructionStream()) {
    if (instr->needsGCMap()) {
      TR_GCStackMap* map = instr->getGCMap();
      TR_ASSERT(map, "Instruction should have a GC map");

      // This instruction has a GC map, for every register in the register map
      // check if that register is unassigned at the beginning of the hot path.
      for (uint32_t regNum = TR::RealRegister::FirstGPR;
           regNum < TR::RealRegister::LastGPR;
           ++regNum) {
        uint32_t regMask = cg->registerBitMask(regNum);
        if (map->getRegisterMap() & regMask) {
          TR::RealRegister* regInRegMap =
            cg->machine()->getRealRegister((TR::RealRegister::RegNum)regNum);
          if (regInRegMap->getState() == TR::RealRegister::Free) {
            // This register is unassigned, check if it was defined before the
            // GC point.
            TR::Instruction* prevInstr = instr->getPrev();
            while (prevInstr != branchOOL &&
                   !prevInstr->defsRealRegister(regInRegMap))
              prevInstr = prevInstr->getPrev();
            // If it wasn't defined before the GC point it died on the cold path
            // and it's first use on the hot path was after the GC point i.e. it
            // shouldn't be in the register map.
...
void
OMR::ARM::CodeGenerator::beginInstructionSelection()
{
  TR::Compilation* comp = self()->comp();
  ...
  if (comp->getMethodSymbol()->getLinkageConvention() == TR_Private) {
    ...
    if (methodSymbol->isJNI()) {
      uintptrj_t JNIMethodAddress =
        (uintptrj_t)methodSymbol->getResolvedMethod()->startAddressForJNIMethod(
          comp);
      cursor = new (self()->trHeapMemory()) TR::ARMImmInstruction(
        cursor, ARMOp_dd, startNode, (int32_t)JNIMethodAddress, self());
    }

    _returnTypeInfoInstruction = new (self()->trHeapMemory())
      TR::ARMImmInstruction(cursor, ARMOp_dd, startNode, 0, self());
    new (self()->trHeapMemory()) TR::ARMAdminInstruction(
      _returnTypeInfoInstruction, ARMOp_proc, startNode, NULL, self());

  } else {
    _returnTypeInfoInstruction = NULL;
    new (self()->trHeapMemory()) TR::ARMAdminInstruction(
      (TR::Instruction*)NULL, ARMOp_proc, startNode, NULL, self());
  }
}

void
OMR::ARM::CodeGenerator::endInstructionSelection()
{
...
void
OMR::ARM::CodeGenerator::doRegisterAssignment(TR_RegisterKinds kindsToAssign)
{
  TR::Compilation* comp = self()->comp();
  ...
  while (instructionCursor) {
    ...
    if (comp->getOption(TR_TraceCG)) {
      diagnostic("\nassigning registers for [" POINTER_PRINTF_FORMAT "]:",
                 instructionCursor);
    ...
    } else if (instructionCursor->getKind() ==
               TR::Instruction::IsConditionalBranch) {
      TR::ARMConditionalBranchInstruction* bi =
        (TR::ARMConditionalBranchInstruction*)instructionCursor;

      if (bi->getLabelSymbol() &&
          bi->getLabelSymbol()->isStartOfColdInstructionStream()) {
    ...
    if (comp->getOption(TR_TraceCG)) {
      ...
      TR::Instruction* instr =
        prevInstruction ? prevInstruction->getNext() : instructionCursor;
...
void
OMR::ARM::CodeGenerator::doBinaryEncoding()
{
  TR::Compilation* comp = self()->comp();
  int32_t estimate = 0;
...
bool
OMR::ARM::CodeGenerator::hasDataSnippets()
{
  return (_constantData == NULL) ? false : true;
}

void
OMR::ARM::CodeGenerator::emitDataSnippets()
{
  self()->setBinaryBufferCursor(_constantData->emitSnippetBody());
}

int32_t
OMR::ARM::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
...
#ifdef DEBUG
void
OMR::ARM::CodeGenerator::dumpDataSnippets(TR::FILE* outFile)
{
  if (outFile == NULL)
    return;
...
int32_t
OMR::ARM::CodeGenerator::findOrCreateAddressConstant(void* v,
                                                     TR::DataType t,
                                                     TR::Instruction* n0,
                                                     TR::Instruction* n1,
                                                     TR::Instruction* n2,
                                                     ...
                                                     bool isUnloadablePicSite)
{
  if (_constantData == NULL)
    _constantData =
      new (self()->trHeapMemory()) TR::ARMConstantDataSnippet(self());
  return (_constantData->addConstantRequest(
    v, t, n0, n1, n2, n3, n4, node, isUnloadablePicSite));
}

// different from evaluate in that it returns a clobberable register
TR::Register*
OMR::ARM::CodeGenerator::gprClobberEvaluate(TR::Node* node)
{
  if (node->getReferenceCount() > 1) {
    if (node->getOpCode().isLong()) {
      ...
      generateTrg1Src1Instruction(
        self(), ARMOp_mov, node, lowReg, temp->getLowOrder());
      generateTrg1Src1Instruction(
        self(), ARMOp_mov, node, highReg, temp->getHighOrder());

      return longReg;
    } else {
      TR::Register* targetRegister = self()->allocateRegister();
      generateTrg1Src1Instruction(
        self(), ARMOp_mov, node, targetRegister, self()->evaluate(node));
...
identifyFarConditionalBranches(int32_t estimate, TR::CodeGenerator* cg)
{
  TR_Array<TR::ARMConditionalBranchInstruction*> candidateBranches(
    cg->trMemory(), 256);
  TR::Instruction* cursorInstruction = cg->getFirstInstruction();

  while (cursorInstruction) {
    TR::ARMConditionalBranchInstruction* branch =
      cursorInstruction->getARMConditionalBranchInstruction();
    if (branch != NULL) {
...
void
OMR::ARM::CodeGenerator::buildRegisterMapForInstruction(TR_GCStackMap* map)
{
...
void
OMR::ARM::CodeGenerator::apply24BitLabelRelativeRelocation(
  int32_t* cursor,
...
void
OMR::ARM::CodeGenerator::apply8BitLabelRelativeRelocation(
  int32_t* cursor,
  TR::LabelSymbol* label)
{
  int32_t delta = (intptr_t)label->getCodeLocation() - ((intptr_t)cursor + 8);
...
TR_ARMOutOfLineCodeSection*
OMR::ARM::CodeGenerator::findOutLinedInstructionsFromLabel(
  TR::LabelSymbol* label)
{
  ListIterator<TR_ARMOutOfLineCodeSection> oiIterator(
    &self()->getARMOutOfLineCodeSectionList());
...
OMR::ARM::CodeGenerator::pickRegister(
...
{
  // Tactical GRA
  // We delegate the decision to use register pressure simulation to common
  // code. if (!comp()->getOption(TR_DisableRegisterPressureSimulation))
...
bool
OMR::ARM::CodeGenerator::allowGlobalRegisterAcrossBranch(
...
int32_t
OMR::ARM::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
  if (node->getOpCode().isSwitch()) {
    ...
    if (node->getOpCodeValue() == TR::table) {
      // The constant below comes from
      // ControlFlowEvaluator.cpp#tableEvaluator(), number of dependencies
      // required.
      return _numGPR - 2;
    } else {
      ...
      // from ControlFlowEvaluator.cpp#switchDispatch() and lookupScheme
      // implementations.
      if (total <= 15) {
...
#if 0 // As noted above, these checks are synched with
      // ControlFlowEvaluator.cpp#switchDispatch(), and this section is unused
      // as of now.
         if (total <= 8)
            return _numGPR - 3;
#endif
...
int32_t
OMR::ARM::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(TR::Node* node)
{
  return _numFPR; // pickRegister will ensure that the number of free registers
                  // isn't exceeded
}

#if 1
TR_GlobalRegisterNumber
OMR::ARM::CodeGenerator::getLinkageGlobalRegisterNumber(
...
#endif
int32_t
OMR::ARM::CodeGenerator::getMaximumNumbersOfAssignableGPRs()
{
  return TR::RealRegister::LastGPR - TR::RealRegister::FirstGPR + 1;
}

int32_t
OMR::ARM::CodeGenerator::getMaximumNumbersOfAssignableFPRs()
{
 ...
 */
bool
OMR::ARM::CodeGenerator::shouldValueBeInACommonedNode(int64_t value)
{
...
bool
OMR::ARM::CodeGenerator::isGlobalRegisterAvailable(TR_GlobalRegisterNumber i,
                                                   TR::DataType dt)
{
  return self()
           ->machine()
...
bool
OMR::ARM::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress,
                                                      intptrj_t sourceAddress)
{
  return !TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(
