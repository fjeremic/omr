  ...
  TR::Instruction* cursor, TR::Node* node)
{
  ...
  TR::SymbolReference* revertToInterpreterSymRef
    = self()->symRefTab()->findOrCreateRuntimeHelper(TR_ARMrevertToInterpreterGlue, false, false, false);
  ...
  cursor = generateImmSymInstruction(self(), ARMOp_bl, node, (uintptrj_t)revertToInterpreterSymRef->getMethodAddress(),
  ...
  cursor = generateImmInstruction(
    self(), ARMOp_dd, node, (int32_t)helperAddr, TR_AbsoluteHelperAddress, helperSymRef, cursor);
...
static void removeGhostRegistersFromGCMaps(TR::CodeGenerator* cg, TR::Instruction* branchOOL)
{
  ...
  // it's first use in the hot path if it's also used on the cold path, since RA still needs to be done on the cold path
  // (which is where it's future use count will drop to 0), which means it will be incorrectly seen as live between it's
  // first use and the top of the hot path. If there are any GC points between the top of the hot path and the first use
  // the real reg holding the virtual will be included, so we need to fix this.
  ...
  while (!instr->isLabel() || !((TR::ARMLabelInstruction*)instr)->getLabelSymbol()->isEndOfColdInstructionStream()) {
    if (instr->needsGCMap()) {
      ...
      // This instruction has a GC map, for every register in the register map check if that register is unassigned at
      // the beginning of the hot path.
      for (uint32_t regNum = TR::RealRegister::FirstGPR; regNum < TR::RealRegister::LastGPR; ++regNum) {
        ...
        if (map->getRegisterMap() & regMask) {
          ...
          if (regInRegMap->getState() == TR::RealRegister::Free) {
            ...
            // If it wasn't defined before the GC point it died on the cold path and it's first use on the hot path was
            // after the GC point i.e. it shouldn't be in the register map.
  ...
  if (comp->getMethodSymbol()->getLinkageConvention() == TR_Private) {
    ...
    if (methodSymbol->isJNI()) {
      ...
      cursor = new (self()->trHeapMemory())
        TR::ARMImmInstruction(cursor, ARMOp_dd, startNode, (int32_t)JNIMethodAddress, self());
    }

    _returnTypeInfoInstruction
      = new (self()->trHeapMemory()) TR::ARMImmInstruction(cursor, ARMOp_dd, startNode, 0, self());
    new (self()->trHeapMemory())
      TR::ARMAdminInstruction(_returnTypeInfoInstruction, ARMOp_proc, startNode, NULL, self());
...
TR_GlobalRegisterNumber OMR::ARM::CodeGenerator::pickRegister(TR_RegisterCandidate* regCan, TR::Block** barr,
...
bool OMR::ARM::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress, intptrj_t sourceAddress)
{
  return !TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(targetAddress, sourceAddress)
