...
#if !defined(HARDHAT) && \
    !defined(            \
        __VFP_FP__)  // gcc does not support VFP with -mfloat-abi=hard yet
...
OMR::ARM::CodeGenerator::generateSwitchToInterpreterPrePrologue(
  ...
  TR::Register *lr = self()->machine()->getRealRegister(
      TR::RealRegister::gr14);  // link register
  ...
  cursor = generateImmInstruction(self(), ARMOp_dd, node, (int32_t)ramMethod,
                                  TR_RamMethod, cursor);
  ...
  cursor =
      generateImmInstruction(self(), ARMOp_dd, node, (int32_t)helperAddr,
...
static void removeGhostRegistersFromGCMaps(TR::CodeGenerator *cg,
  ...
  while (!instr->isLabel() || !((TR::ARMLabelInstruction *)instr)
    ...
    if (instr->needsGCMap()) {
      ...
      for (uint32_t regNum = TR::RealRegister::FirstGPR;
           regNum < TR::RealRegister::LastGPR; ++regNum) {
  ...
  if (comp->getMethodSymbol()->getLinkageConvention() == TR_Private) {
    ...
    if (methodSymbol->isJNI()) {
      uintptrj_t JNIMethodAddress =
          (uintptrj_t)methodSymbol->getResolvedMethod()
              ->startAddressForJNIMethod(comp);
...
void OMR::ARM::CodeGenerator::doRegisterAssignment(
  ...
  while (instructionCursor) {
    ...
    if (instructionCursor->isLabel()) {
      TR::ARMLabelInstruction *li =
          (TR::ARMLabelInstruction *)instructionCursor;
...
int32_t OMR::ARM::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
...
int32_t OMR::ARM::CodeGenerator::findOrCreateAddressConstant(
    void *v, TR::DataType t, TR::Instruction *n0, TR::Instruction *n1,
    TR::Instruction *n2, TR::Instruction *n3, TR::Instruction *n4,
    TR::Node *node, bool isUnloadablePicSite) {
  if (_constantData == NULL)
    _constantData =
        new (self()->trHeapMemory()) TR::ARMConstantDataSnippet(self());
  return (_constantData->addConstantRequest(v, t, n0, n1, n2, n3, n4, node,
                                            isUnloadablePicSite));
}
...
TR::Register *OMR::ARM::CodeGenerator::gprClobberEvaluate(TR::Node *node) {
  if (node->getReferenceCount() > 1) {
    if (node->getOpCode().isLong()) {
      ...
      generateTrg1Src1Instruction(self(), ARMOp_mov, node, lowReg,
                                  temp->getLowOrder());
      generateTrg1Src1Instruction(self(), ARMOp_mov, node, highReg,
                                  temp->getHighOrder());

      return longReg;
    } else {
      TR::Register *targetRegister = self()->allocateRegister();
      generateTrg1Src1Instruction(self(), ARMOp_mov, node, targetRegister,
                                  self()->evaluate(node));
...
void OMR::ARM::CodeGenerator::buildRegisterMapForInstruction(
    TR_GCStackMap *map) {
...
void OMR::ARM::CodeGenerator::apply24BitLabelRelativeRelocation(
...
void OMR::ARM::CodeGenerator::apply8BitLabelRelativeRelocation(
    int32_t *cursor, TR::LabelSymbol *label) {
  int32_t delta = (intptr_t)label->getCodeLocation() - ((intptr_t)cursor + 8);
...
OMR::ARM::CodeGenerator::findOutLinedInstructionsFromLabel(
  ...
  while (oiCursor) {
    if (oiCursor->getEntryLabel() == label) return oiCursor;
...
TR_GlobalRegisterNumber OMR::ARM::CodeGenerator::pickRegister(
...
bool OMR::ARM::CodeGenerator::allowGlobalRegisterAcrossBranch(
...
int32_t OMR::ARM::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
    TR::Node *node) {
  if (node->getOpCode().isSwitch()) {
    ...
    if (node->getOpCodeValue() == TR::table) {
      ...
      if (total <= 15) {
        ...
        for (i = 2; i < total &&
                    constantIsImmed8r(node->getChild(i)->getCaseConstant(),
                                      &base, &rotate);
             i++)
          ;
        if (i == total) return _numGPR - 1;
      }
...
int32_t OMR::ARM::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(
    TR::Node *node) {
...
bool OMR::ARM::CodeGenerator::isGlobalRegisterAvailable(
...
bool OMR::ARM::CodeGenerator::directCallRequiresTrampoline(
