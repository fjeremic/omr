...
void TR::ARMSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                              uint32_t &stackIndex) {
...
void TR::ARMSystemLinkage::createPrologue(TR::Instruction *cursor) {
  ...
  if (comp()->getOption(TR_EntryBreakPoints)) {
    cursor = new (trHeapMemory())
        TR::Instruction(cursor, ARMOp_bad, firstNode, cg());
  }
  ...
  for (TR::ParameterSymbol *parameter = parameterIterator.getFirst();
    ...
    case TR::Address:
      if (nextIntArgReg < getProperties().getNumIntArgRegs()) {
        cursor = generateMemSrc1Instruction(
            cg(), ARMOp_str, firstNode, stackSlot,
    ...
    case TR::Double:
      if (nextFltArgReg < getProperties().getNumFloatArgRegs()) {
        cursor = generateMemSrc1Instruction(
            cg(), ARMOp_fstd, firstNode, stackSlot,
            machine->getRealRegister((TR::RealRegister::RegNum)(
  ...
  auto *stackSlot = new (trHeapMemory()) TR::MemoryReference(
      stackPtr, bodySymbol->getLocalMappingCursor(), codeGen);
  cursor = generateMemSrc1Instruction(
      cg(), ARMOp_str, firstNode, stackSlot,
...
void TR::ARMSystemLinkage::createEpilogue(TR::Instruction *cursor) {
  ...
  auto *stackSlot = new (trHeapMemory()) TR::MemoryReference(
      stackPtr, bodySymbol->getLocalMappingCursor(), codeGen);
  cursor = generateMemSrc1Instruction(
      cg(), ARMOp_ldr, lastNode, stackSlot,
      machine->getRealRegister(TR::RealRegister::gr14), cursor);
  ...
  cursor = generateTrg1Src1Instruction(codeGen, ARMOp_mov, lastNode, gr15, gr14,
                                       cursor);
}

TR::MemoryReference *TR::ARMSystemLinkage::getOutgoingArgumentMemRef(
    int32_t totalSize, int32_t offset, TR::Register *argReg,
    TR_ARMOpCodes opCode, TR::ARMMemoryArgument &memArg) {
