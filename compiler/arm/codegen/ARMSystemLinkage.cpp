...
TR::ARMLinkageProperties TR::ARMSystemLinkage::properties = {
  ...
  {
    // preserved registers
    TR::RealRegister::gr4,  TR::RealRegister::gr5,  TR::RealRegister::gr6,
    TR::RealRegister::gr7,  TR::RealRegister::gr8,  TR::RealRegister::gr9,
    TR::RealRegister::gr10, TR::RealRegister::gr11, TR::RealRegister::gr12,
    TR::RealRegister::gr13, TR::RealRegister::gr14, TR::RealRegister::gr15,
#if defined(__VFP_FP__) && !defined(__SOFTFP__)
    TR::RealRegister::fp8,  TR::RealRegister::fp9,  TR::RealRegister::fp10,
    TR::RealRegister::fp11, TR::RealRegister::fp12, TR::RealRegister::fp13,
    TR::RealRegister::fp14, TR::RealRegister::fp15,
#endif
  },
  {
    // argument registers
...
void
TR::ARMSystemLinkage::initARMRealRegisterLinkage()
{
  ...
  // make r9 unavailable for RA (just in case, because it's meaning is platform
  // defined)
  ...
  for (int32_t r = TR::RealRegister::gr0; r <= TR::RealRegister::gr8; ++r) {
    machine->getRealRegister(static_cast<TR::RealRegister::RegNum>(r))
      ->setWeight(0xf000);
  }
...
uint32_t
TR::ARMSystemLinkage::getRightToLeft()
{
  return getProperties().getRightToLeft();
}

static void
mapSingleParameter(TR::ParameterSymbol* parameter, uint32_t& stackIndex)
{
 ...
 */
void
TR::ARMSystemLinkage::mapStack(TR::ResolvedMethodSymbol* method)
{
  ...
  // allocate space for preserved registers and link register (9 registers
  // total)
  stackIndex += 9 * 4;
    ...
    switch (parameter->getDataType()) {
      ...
      case TR::Float:
        comp()->failCompilation<UnsupportedParameterType>(
          "Compiling methods with a single precision floating point parameter "
          "is not supported");
        break;
      case TR::Double:
...
void
TR::ARMSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p,
                                         uint32_t& stackIndex)
{
...
TR::ARMLinkageProperties&
TR::ARMSystemLinkage::getProperties()
{
  return properties;
}

void
TR::ARMSystemLinkage::createPrologue(TR::Instruction* cursor)
{
  ...
  if (comp()->getOption(TR_EntryBreakPoints)) {
    cursor =
      new (trHeapMemory()) TR::Instruction(cursor, ARMOp_bad, firstNode, cg());
  }

  // allocate stack space
  auto frameSize = codeGen->getFrameSizeInBytes();
  cursor = generateTrg1Src1ImmInstruction(
    codeGen, ARMOp_sub, firstNode, stackPtr, stackPtr, frameSize, 0, cursor);
  ...
  for (TR::ParameterSymbol* parameter = parameterIterator.getFirst();
       ...
       (nextIntArgReg < getProperties().getNumIntArgRegs() ||
        nextFltArgReg < getProperties().getNumFloatArgRegs());
       parameter = parameterIterator.getNext()) {
    auto* stackSlot = new (trHeapMemory())
      TR::MemoryReference(stackPtr, parameter->getParameterOffset(), codeGen);
    switch (parameter->getDataType()) {
      case TR::Int8:
      case TR::Int16:
      case TR::Int32:
        ...
        if (nextIntArgReg < getProperties().getNumIntArgRegs()) {
          cursor = generateMemSrc1Instruction(
            ...
            machine->getRealRegister((TR::RealRegister::RegNum)(
              TR::RealRegister::gr0 + nextIntArgReg)),
            cursor);
          nextIntArgReg++;
        } else {
          nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
      ...
      case TR::Int64:
        ...
        if (nextIntArgReg + 1 < getProperties().getNumIntArgRegs()) {
          ...
          stackSlot = new (trHeapMemory()) TR::MemoryReference(
            stackPtr, parameter->getParameterOffset() + 4, codeGen);
          cursor = generateMemSrc1Instruction(
            cg(),
            ARMOp_str,
            firstNode,
            stackSlot,
            machine->getRealRegister((TR::RealRegister::RegNum)(
              TR::RealRegister::gr0 + nextIntArgReg + 1)),
            cursor);
          nextIntArgReg += 2;
        } else {
          nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
      ...
      case TR::Double:
        if (nextFltArgReg < getProperties().getNumFloatArgRegs()) {
          cursor = generateMemSrc1Instruction(
            ...
            stackSlot,
            machine->getRealRegister((TR::RealRegister::RegNum)(
              TR::RealRegister::fp0 + nextFltArgReg)),
            cursor);
          nextFltArgReg += 1;
        } else {
          nextFltArgReg = getProperties().getNumFloatArgRegs() + 1;
  ...
  auto* stackSlot = new (trHeapMemory())
    TR::MemoryReference(stackPtr, bodySymbol->getLocalMappingCursor(), codeGen);
  cursor =
    generateMemSrc1Instruction(cg(),
                               ARMOp_str,
                               firstNode,
                               stackSlot,
                               machine->getRealRegister(TR::RealRegister::gr14),
                               cursor);
}

void
TR::ARMSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
  TR::CodeGenerator* codeGen = cg();
  const TR::ARMLinkageProperties& properties = getProperties();
  TR::Machine* machine = codeGen->machine();
  ...
  auto frameSize = codeGen->getFrameSizeInBytes();
  cursor = generateTrg1Src1ImmInstruction(
    codeGen, ARMOp_add, lastNode, stackPtr, stackPtr, frameSize, 0, cursor);
  ...
  cursor = generateTrg1Src1Instruction(
    codeGen, ARMOp_mov, lastNode, gr15, gr14, cursor);
}

TR::MemoryReference*
TR::ARMSystemLinkage::getOutgoingArgumentMemRef(int32_t totalSize,
                                                int32_t offset,
                                                TR::Register* argReg,
                                                TR_ARMOpCodes opCode,
                                                TR::ARMMemoryArgument& memArg)
{
  int32_t spOffset = offset - (getProperties().getNumIntArgRegs() *
  ...
  TR::MemoryReference* result =
    new (trHeapMemory()) TR::MemoryReference(sp, spOffset, cg());
...
int32_t
TR::ARMSystemLinkage::buildArgs(TR::Node* callNode,
...
TR::Register*
TR::ARMSystemLinkage::buildDirectDispatch(TR::Node* callNode)
{
  return buildARMLinkageDirectDispatch(callNode, true);
}

TR::Register*
TR::ARMSystemLinkage::buildIndirectDispatch(TR::Node* callNode)
{
  TR_UNIMPLEMENTED();
  return NULL;
}
...
intptrj_t
TR::ARMSystemLinkage::entryPointFromCompiledMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}

intptrj_t
TR::ARMSystemLinkage::entryPointFromInterpretedMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}
