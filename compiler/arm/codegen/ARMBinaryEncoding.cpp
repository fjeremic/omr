...
void
OMR::ARM::Instruction::generateConditionBinaryEncoding(
  uint8_t* instructionStart)
{
  *((uint32_t*)instructionStart) |= ((uint32_t)self()->getConditionCode())
                                    << 28;
}

uint8_t*
OMR::ARM::Instruction::generateBinaryEncoding()
{
...
int32_t
OMR::ARM::Instruction::estimateBinaryLength(int32_t currentEstimate)
{
...
uint32_t
encodeBranchDistance(uint32_t from, uint32_t to)
{
...
uint32_t
encodeHelperBranchAndLink(TR::SymbolReference* symRef,
                          uint8_t* cursor,
                          TR::Node* node,
                          TR::CodeGenerator* cg)
{
...
uint32_t
encodeHelperBranch(bool isBranchAndLink,
                   ...
                   TR::CodeGenerator* cg)
{
  ...
  if (cg->directCallRequiresTrampoline(target, (intptrj_t)cursor)) {
    ...
    TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(
                      target, (intptrj_t)cursor),
                    "Target address is out of range");
  }

  cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(
                              cursor, (uint8_t*)symRef, TR_HelperAddress, cg),
                            __FILE__,
                            __LINE__,
                            node);

  return (isBranchAndLink ? 0x0B000000 : 0x0A000000) |
         encodeBranchDistance((uintptr_t)cursor, (uint32_t)target) |
         (((uint32_t)cc) << 28);
}

uint8_t*
TR::ARMLabelInstruction::generateBinaryEncoding()
{
  ...
  if (getOpCode().isLabel()) // LABEL
  {
    ...
    if (getOpCode().isBranchOp()) {
      if (destination != 0)
        *(int32_t*)cursor |=
          encodeBranchDistance((uintptr_t)cursor, destination);
      else
...
int32_t
TR::ARMLabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  if (getOpCode().isLabel()) // LABEL
  {
...
uint8_t*
TR::ARMConditionalBranchInstruction::generateBinaryEncoding()
{
  return TR::ARMLabelInstruction::generateBinaryEncoding();
}

int32_t
TR::ARMConditionalBranchInstruction::estimateBinaryLength(
...
uint8_t*
TR::ARMAdminInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint32_t i;
  ...
  if (getOpCodeValue() == ARMOp_fence) {
    ...
    if (rtype == TR_AbsoluteAddress) {
      ...
      for (i = 0; i < fenceNode->getNumRelocations(); ++i) {
        *(uint16_t*)(fenceNode->getRelocationDestination(i)) =
          (uint16_t)cg()->getCodeLength();
      }
    } else if (rtype == TR_EntryRelative32Bit) {
      for (i = 0; i < fenceNode->getNumRelocations(); ++i) {
        *(uint32_t*)(fenceNode->getRelocationDestination(i)) =
          cg()->getCodeLength();
      }
...
int32_t
TR::ARMAdminInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(0);
...
uint8_t*
TR::ARMImmInstruction::generateBinaryEncoding()
{
...
uint8_t*
TR::ARMImmSymInstruction::generateBinaryEncoding()
{
  TR::Compilation* comp = cg()->comp();
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  ...
  if (getOpCodeValue() == ARMOp_bl) {
    ...
    TR::ResolvedMethodSymbol* sym =
      getSymbolReference()->getSymbol()->getResolvedMethodSymbol();
    ...
    if (comp->isRecursiveMethodTarget(sym)) {
      ...
      *(int32_t*)cursor =
        (*(int32_t*)cursor) |
        encodeBranchDistance((uintptr_t)cursor, jitTojitStart);
    } else if (label != NULL) {
      cg()->addRelocation(new (cg()->trHeapMemory())
                            TR::LabelRelative24BitRelocation(cursor, label));
#ifdef J9_PROJECT_SPECIFIC
      ...
      if (((method = getSymbolReference()->getSymbol()->getMethodSymbol()) !=
           NULL) &&
          method->isHelper()) {
        *(int32_t*)cursor = encodeHelperBranch(true,
                                               getSymbolReference(),
                                               cursor,
                                               getConditionCode(),
                                               getNode(),
        ...
        if (TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(
              (intptrj_t)imm, (intptrj_t)cursor)) {
          *(int32_t*)cursor |=
            encodeBranchDistance((uintptr_t)cursor, (uint32_t)imm);
        } else {
          ...
          *(int32_t*)cursor = imm;
          ...
          if (sym->isNative() || sym->isJNI() && node &&
            ...
            *(int32_t*)cursor = imm;
            ...
            intptrj_t targetAddress = cg()->fe()->methodTrampolineLookup(
              comp, getSymbolReference(), (void*)cursor);

            TR_ASSERT_FATAL(
              TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(
                targetAddress, (intptrj_t)cursor),
              "Target address is out of range");

            *(int32_t*)cursor |=
              encodeBranchDistance((uintptr_t)cursor, (uintptr_t)targetAddress);
          }
#endif
...
int32_t
TR::ARMImmSymInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  int32_t length;
  TR::ResolvedMethodSymbol* sym =
    getSymbolReference()->getSymbol()->getResolvedMethodSymbol();
...
uint8_t*
TR::ARMTrg1Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::ARMTrg1Src2Instruction::generateBinaryEncoding()
{
  TR::Compilation* comp = cg()->comp();
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
...
uint8_t*
TR::ARMLoadStartPCInstruction::generateBinaryEncoding()
{
  /* Calculates the offset from the current instruction to startPCAddr, then set
   * it to the operand2 */
  uint8_t* startPCAddr = (uint8_t*)getSymbolReference()
                           ->getSymbol()
                           ->getStaticSymbol()
                           ->getStaticAddress();
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  ...
  if (constantIsImmed8r(offset, &base, &rotate)) {
    ...
    if ((base & 0xFFFF0000) == 0) {
      TR_ARMOperand2* op2_1 = new (cg()->trHeapMemory())
        TR_ARMOperand2((base >> 8) & 0x000000FF, 8 + bitTrailing);
      TR_ARMOperand2* op2_0 = new (cg()->trHeapMemory())
        TR_ARMOperand2(base & 0x000000FF, bitTrailing);
      setSource2Operand(op2_1);
      TR::ARMTrg1Src2Instruction* secondInstruction =
        (TR::ARMTrg1Src2Instruction*)getNext();
      ...
      TR::ARMTrg1Src2Instruction* thirdInstruction =
        (TR::ARMTrg1Src2Instruction*)secondInstruction->getNext();
      thirdInstruction->setSource2Operand(op2_1);

      TR::ARMTrg1Src2Instruction* fourthInstruction =
        (TR::ARMTrg1Src2Instruction*)thirdInstruction->getNext();
...
uint8_t*
TR::ARMTrg2Src1Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::ARMMulInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::ARMMemInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
int32_t
TR::ARMMemInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(
    getMemoryReference()->estimateBinaryLength(getOpCodeValue()));
  return (currentEstimate + getEstimatedBinaryLength());
}

uint8_t*
TR::ARMTrg1MemSrc1Instruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
uint8_t*
TR::ARMControlFlowInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
int32_t
TR::ARMControlFlowInstruction::estimateBinaryLength(int32_t currentEstimate)
{
...
uint8_t*
TR::ARMMultipleMoveInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;
  cursor = getOpCode().copyBinaryToBuffer(instructionStart);
...
#ifdef J9_PROJECT_SPECIFIC
uint8_t*
TR::ARMVirtualGuardNOPInstruction::generateBinaryEncoding()
{
...
int32_t
TR::ARMVirtualGuardNOPInstruction::estimateBinaryLength(int32_t currentEstimate)
{
