...
uint32_t encodeHelperBranchAndLink(TR::SymbolReference* symRef,
                                   uint8_t* cursor,
                                   TR::Node* node,
                                   TR::CodeGenerator* cg) {
...
uint32_t encodeHelperBranch(bool isBranchAndLink,
        ...
        if (TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(
          ...
          *(int32_t*)cursor = imm;
          ...
          if (sym->isNative() || sym->isJNI() && node &&
            ...
            *(int32_t*)cursor |= encodeBranchDistance((uintptr_t)cursor,
