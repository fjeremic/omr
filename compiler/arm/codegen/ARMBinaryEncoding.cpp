...
void OMR::ARM::Instruction::generateConditionBinaryEncoding(
...
uint32_t encodeHelperBranchAndLink(TR::SymbolReference *symRef, uint8_t *cursor,
                                   TR::Node *node, TR::CodeGenerator *cg) {
  return encodeHelperBranch(true, symRef, cursor, ARMConditionCodeAL, node, cg);
}

uint32_t encodeHelperBranch(bool isBranchAndLink, TR::SymbolReference *symRef,
                            uint8_t *cursor, TR_ARMConditionCode cc,
                            TR::Node *node, TR::CodeGenerator *cg) {
  ...
  cg->addExternalRelocation(
      new (cg->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t *)symRef,
                                                      TR_HelperAddress, cg),
...
int32_t TR::ARMConditionalBranchInstruction::estimateBinaryLength(
  ...
  if (getOpCodeValue() == ARMOp_fence) {
    ...
    if (rtype == TR_AbsoluteAddress) {
      for (i = 0; i < fenceNode->getNumRelocations(); ++i) {
        *(uint8_t **)(fenceNode->getRelocationDestination(i)) =
            instructionStart;
      }
    ...
    if (comp->isRecursiveMethodTarget(sym)) {
      ...
      *(int32_t *)cursor =
      ...
      if (((method = getSymbolReference()->getSymbol()->getMethodSymbol()) !=
          ...
          method->isHelper()) {
        *(int32_t *)cursor =
            encodeHelperBranch(true, getSymbolReference(), cursor,
                               getConditionCode(), getNode(), cg());
      } else {
        ...
        if (TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(
          ...
          *(int32_t *)cursor = imm;
          ...
          if (sym->isNative() || sym->isJNI() && node &&
            ...
            *(int32_t *)cursor |= encodeBranchDistance(
                (uintptr_t)cursor, (uintptr_t)targetAddress);
          }
#endif
...
int32_t TR::ARMImmSymInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  int32_t length;
  TR::ResolvedMethodSymbol *sym =
      getSymbolReference()->getSymbol()->getResolvedMethodSymbol();
...
int32_t TR::ARMControlFlowInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
uint8_t *TR::ARMMultipleMoveInstruction::generateBinaryEncoding() {
  ...
  if (isWriteBack()) *(int32_t *)cursor |= 1 << 21;
  if (isPreIndex()) *(int32_t *)cursor |= 1 << 24;
  if (isIncrement()) *(int32_t *)cursor |= 1 << 23;
  cursor += ARM_INSTRUCTION_LENGTH;
  setBinaryLength(ARM_INSTRUCTION_LENGTH);
  setBinaryEncoding(instructionStart);
  return cursor;
...
int32_t TR::ARMVirtualGuardNOPInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
