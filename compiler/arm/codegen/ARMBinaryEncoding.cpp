  ...
  TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  cg->addExternalRelocation(new (cg->trHeapMemory())
                              TR::ExternalRelocation(cursor, (uint8_t*)symRef, TR_HelperAddress, cg),
        ...
        *(int32_t*)cursor = encodeHelperBranch(true, getSymbolReference(), cursor, getConditionCode(), getNode(), cg());
        ...
        if (TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange((intptrj_t)imm, (intptrj_t)cursor)) {
          ...
          *(int32_t*)cursor = imm;
          ...
          if (sym->isNative()
            || sym->isJNI() && node && node->isPreparedForDirectJNI() && getConditionCode() == ARMConditionCodeAL) {
            ...
            *(int32_t*)cursor = imm;
            ...
            TR_ASSERT_FATAL(
              TR::Compiler->target.cpu.isTargetWithinBranchImmediateRange(targetAddress, (intptrj_t)cursor),
              "Target address is out of range");
