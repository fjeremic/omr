...
namespace ARM {
    ...
    public:
        ...
        {
            *instruction |= (((fullRegBinaryEncodings[_registerNumber] >> 1) << pos_RD) | ((fullRegBinaryEncodings[_registerNumber] & 1) << pos_D));
        }
        void setRegisterFieldSM(uint32_t* instruction)
        {
            *instruction |= (((fullRegBinaryEncodings[_registerNumber] >> 1) << pos_RM) | ((fullRegBinaryEncodings[_registerNumber] & 1) << pos_M));
        }
        void setRegisterFieldSN(uint32_t* instruction)
        {
            *instruction |= (((fullRegBinaryEncodings[_registerNumber] >> 1) << pos_RN) | ((fullRegBinaryEncodings[_registerNumber] & 1) << pos_N));
        }
#endif
