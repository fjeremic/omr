...
unsigned
getField(unsigned data, int loc, int len)
{
  return (data >> loc) & ((1 << len) - 1);
}

uint32_t
rotateRight(uint32_t uval, uint32_t n)
{
...
uint32_t
concatImm4Imm4(uint32_t hi4, uint32_t low4)
{
  return hi4 << 4 | low4;
}

const char*
regStr(int regNum, bool W) // PC is a single purpose register.
...
#ifdef __VFP_FP__
const char*
doubleRegStr(uint32_t regNum, bool W)
{ // If a register value is to be updated by the instruction, it is printed in
  // capitals.
  switch (regNum >> 1) {
    case 0:
...
const char*
singleRegStr(uint32_t regNum, bool W)
{ // If a register value is to be updated by the instruction, it is printed in
  // capitals.
  switch (regNum) {
    case 0:
...
void
constant32(int32_t* instrPtr, char* mBuf, char* iBuf)
{
#ifdef LINUX
  sprintf(mBuf, ".long");
#else
...
void
branchExchange(int32_t* instrPtr, char* mBuf, char* iBuf)
{
...
void
branch(int32_t* instrPtr, char* mBuf, char* iBuf)
{
...
void
moveStatusReg(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  ...
  if (!instr->to) // Move from status register
  {
    ...
    sprintf(
      iBuf, "%s, %cPSR", regStr(instr->Rd, true), instrBits->bit22 ? 'S' : 'C');
...
void
miscInstr(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    case 1:
      if (!instrBits->bit22) {
        ...
        if (instr->SBO1 == 0xf && instr->SBO2 == 0xf) {
          ...
          sprintf(
            iBuf, "%s, %s", regStr(instr->Rd, true), regStr(instr->Rm, false));
        } else {
          constant32(instrPtr, mBuf, iBuf);
        }
      }
...
void
dataProcessing(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  ...
  if (test_cmp || mov)
    ...
    sprintf(
      dest_src1, "%s, %s", regStr(instr->Rd, true), regStr(instr->Rn, false));
...
void
swap(int32_t* instrPtr, char* mBuf, char* iBuf)
{
...
void
multiply(int32_t* instrPtr, char* mBuf, char* iBuf)
{
...
void
loadStore(int32_t* instrPtr, char* mBuf, char* iBuf, bool extra)
{
  InstructionBits* instrBits = (InstructionBits*)instrPtr;
  ...
  if ((!extra && !instrBits->bit25) ||
    ...
    sprintf(iBuf,
            "%s, [%s%s, #%s%u%s%s",
            regStr(instr->Rd, instr->L),
            regStr(instr->Rn, (!instr->P || instr->W)),
            instr->P ? "" : "]",
            instr->U ? "" : "-",
...
void
mulAndExtraLoadStore(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  InstructionBits* instrBits = (InstructionBits*)instrPtr;
...
void
loadStoreMultiple(int32_t* instrPtr, char* mBuf, char* iBuf)
{
...
#ifdef __VFP_FP__
static uint32_t
getVFPDestination(VFPInstruction* instr)
{
  return (instr->Fd << 1 | instr->D);
}

static uint32_t
getVFPFirstOperand(VFPInstruction* instr)
{
  return (instr->Fn << 1 | instr->N);
}

static uint32_t
getVFPSecondOperand(VFPInstruction* instr)
{
  return (instr->Fm << 1 | instr->M);
}

void
vfpTwoRegTransfer(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  ...
  if (instr->D == 1 && instr->N == 0 && instr->s == 0 && instr->xfer == 1) {
    if (isDouble) {
      ...
      if (instr->r) // L-bit
      {
        ...
        sprintf(iBuf,
                "{%s, %s}, %s, %s",
                singleRegStr(first, true),
                singleRegStr(next, true),
                regStr(instr->Fd, false),
                regStr(instr->Fn, false));
      }
    }
...
void
loadStoreCoprocessor(int32_t* instrPtr, char* mBuf, char* iBuf)
{
...
#ifdef __VFP_FP__
void
vfpExtension(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  VFPInstruction* instr = (VFPInstruction*)instrPtr;
  bool isDouble = (instr->cp == 11); // Coprocessor 11 is double, 10 is float
  switch (instr->Fn) {
    ...
    case 7:
      if (instr->N) {
        // FCVTDS/FCVTSD
        sprintf(
          mBuf, "%s%s", isDouble ? "fcvtsd" : "fcvtds", condName[instr->cond]);
        sprintf(iBuf,
                "%s, %s",
...
void
vfpSingleRegTransfer(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  VFPInstruction* instr = (VFPInstruction*)instrPtr;
  ...
  if (opCode == 0) {
    ...
    if (isDouble) {
      ...
      if (instr->r) // L-bit
        ...
        sprintf(iBuf,
                "%s, %s",
                singleRegStr(getVFPFirstOperand(instr), true),
                regStr(instr->Fd, false));
    }
  } else if (opCode == 1) {
    if (isDouble) {
      sprintf(
        mBuf, "%s%s", instr->r ? "fmrdh" : "fmdhr", condName[instr->cond]);
      if (instr->r) // L-bit
        sprintf(iBuf,
                "%s, %s[63:32]",
                regStr(instr->Fd, true),
...
void
vfpDataProcessing(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  VFPInstruction* instr = (VFPInstruction*)instrPtr;
  uint32_t opCode =
...
void
disassemble(int32_t* instrPtr, char* mBuf, char* iBuf)
{
