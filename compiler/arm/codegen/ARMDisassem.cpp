...
void multiply(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  ...
  {
    sprintf(
      mBuf, "%c%sl%s%s", instr->U ? 's' : 'u', instr->A ? "mla" : "mul", condName[instr->cond], instr->A ? "s" : "");
...
void loadStore(int32_t* instrPtr, char* mBuf, char* iBuf, bool extra)
{
  ...
  if ((!extra && !instrBits->bit25) || (extra && instrBits->bit22)) { // Immediate offset
    ...
    sprintf(iBuf, "%s, [%s%s, %s%s%s%s", regStr(instr->Rd, instr->L), regStr(instr->Rn, (!instr->P || instr->W)),
      instr->P ? "" : "]", instr->U ? "" : "-", regStr(instr->Rm, false), instr->P ? "]" : "", instr->W ? "!" : "");
  } else // Scaled register offset/index
  {
    if (instr->shift == 3 && instr->shift_imm == 0) {
      sprintf(iBuf, "%s, [%s%s, %s%s, RRX%s%s", regStr(instr->Rd, instr->L), regStr(instr->Rn, (!instr->P || instr->W)),
        instr->P ? "" : "]", instr->U ? "" : "-", regStr(instr->Rm, false), instr->P ? "]" : "", instr->W ? "!" : "");
    } else {
      static char* sh_rot[] = { "LSL", "LSR", "ASR", "ROR" };
      sprintf(iBuf, "%s, [%s%s, %s%s, %s #%u%s%s", regStr(instr->Rd, instr->L),
        regStr(instr->Rn, (!instr->P || instr->W)), instr->P ? "" : "]", instr->U ? "" : "-", regStr(instr->Rm, false),
        sh_rot[instr->shift],
  ...
  if (instr->D == 1 && instr->N == 0 && instr->s == 0 && instr->xfer == 1) {
    if (isDouble) {
      ...
      if (instr->r) // L-bit
      {
        ...
        sprintf(iBuf, "%s, %s, {%s, %s}", regStr(instr->Fd, true), regStr(instr->Fn, true), singleRegStr(first, false),
          singleRegStr(next, false));
      } else {
        // FMSRR
        sprintf(mBuf, "%s%s", "mfsrr", condName[instr->cond]);
        sprintf(iBuf, "{%s, %s}, %s, %s", singleRegStr(first, true), singleRegStr(next, true), regStr(instr->Fd, false),
          regStr(instr->Fn, false));
      }
    }
  } else
...
void vfpSingleRegTransfer(int32_t* instrPtr, char* mBuf, char* iBuf)
{
  ...
  if (opCode == 0) {
    sprintf(
      mBuf, "%s%s", isDouble ? (instr->r ? "fmrs" : "fmsr") : (instr->r ? "fmrdl" : "fmdlr"), condName[instr->cond]);
    if (isDouble) {
      if (instr->r) // L-bit
    ...
    if (!isDouble) {
      ...
      if (instr->r) // L-bit
        sprintf(iBuf, "%s, %%s", regStr(instr->Fd, true),
          ((instr->Fn << 1 | instr->N) == 0) ? "fpsid" : ((instr->Fn << 1 | instr->N) == 1) ? "fpscr" : "fpexc");
      else
        sprintf(iBuf, "%s, %s",
          ((instr->Fn << 1 | instr->N) == 0) ? "fpsid" : (((instr->Fn << 1 | instr->N) == 1) ? "fpscr" : "fpexc"),
