...
{ // If a register value is to be updated by the instruction, it is printed in
  // capitals.
  switch (regNum) {
  case 0:
    return (W ? "R0" : "r0");
...
#ifdef __VFP_FP__
const char *
doubleRegStr(uint32_t regNum,
             bool W) { // If a register value is to be updated by the
                       // instruction, it is printed in capitals.
  switch (regNum >> 1) {
  case 0:
    return (W ? "D0" : "d0");
  case 1:
...
void moveStatusReg(int32_t *instrPtr, char *mBuf, char *iBuf) {
  ...
  {
    if (!instr->c || !instr->x || !instr->s || !instr->f || instr->rotate_imm ||
        instr->immLo4) {
      constant32(instrPtr, mBuf, iBuf);
      return;
    }
    ...
    sprintf(iBuf, "%s, %cPSR", regStr(instr->Rd, true),
            instrBits->bit22 ? 'S' : 'C');
  } else // Move to status register
  {
    ...
    if (!instr->immForm) {
      ...
      sprintf(last_oprnd, "0x%x",
              rotateRight(concatImm4Imm4(instr->immHi4, instr->immLo4),
                          instr->rotate_imm * 2));
    }
    sprintf(iBuf, "%cPSR_%s%s%s%s, %s", instr->R ? 'S' : 'C',
            instr->c ? "c" : "", instr->x ? "x" : "", instr->s ? "s" : "",
            instr->f ? "f" : "", last_oprnd);
  }
}
  ...
  case 1:
    if (!instrBits->bit22) {
      ...
      if (instr->SBO1 == 0xf && instr->SBO2 == 0xf) {
        ...
        sprintf(iBuf, "%s, %s", regStr(instr->Rd, true),
                regStr(instr->Rm, false));
      } else {
        constant32(instrPtr, mBuf, iBuf);
      }
    }
...
void dataProcessing(int32_t *instrPtr, char *mBuf, char *iBuf) {
  DataProcessingInstruction *instr =
      (DataProcessingInstruction *)(uint32_t *)instrPtr;
  InstructionBits *instrBits = (InstructionBits *)(uint32_t *)instrPtr;

  static char *opcodeName[] = {"and", "eor", "sub", "rsb", "add", "adc",
                               "sbc", "rsc", "tst", "teq", "cmp", "cmn",
                               "orr", "mov", "bic", "mvn"};
  bool test_cmp =
      instrBits->bits23_24 == 2; // instr->opcode: 0b1000 || 0b1001 || 0b1010 ||
                                 // 0b1011 (tst, teq, cmp, cmn)
  bool mov =
      instrBits->bit21 &&
      (instrBits->bits23_24 == 3); // instr->opcode: 0b1101 || 0b1111 (mov, mvn)
  ...
  sprintf(mBuf, "%s%s%s", opcodeName[instr->opcode], condName[instr->cond],
          (!test_cmp && instr->S) ? "s" : "");
  ...
  if (test_cmp || mov)
    ...
    sprintf(dest_src1, "%s, %s", regStr(instr->Rd, true),
  ...
  {
    DataProcessingImmedInstruction *instr =
        (DataProcessingImmedInstruction *)(uint32_t *)instrPtr;
  ...
  } else if (instr->shifter == 6) //  Rotate right with extend
  {
    ...
    {
      DataProcessingRegShiftInstruction *instr =
          (DataProcessingRegShiftInstruction *)(uint32_t *)instrPtr;
      sprintf(sh_oprnd, "%s, %s %s", regStr(instr->Rm, false),
              shift_rotate[instr->shift], regStr(instr->Rs, false));
    } else // Immediate shift
    {
      DataProcessingImmShiftInstruction *instr =
          (DataProcessingImmShiftInstruction *)(uint32_t *)instrPtr;
      sprintf(sh_oprnd, "%s, %s #%u", regStr(instr->Rm, false),
              shift_rotate[instr->shift],
              !instr->shift_imm && (instr->shift == 1 || instr->shift == 2)
                  ? 32
                  : instr->shift_imm);
    }
  }
...
void swap(int32_t *instrPtr, char *mBuf, char *iBuf) {
  ...
  if (instr->SBZ) {
    ...
    sprintf(iBuf, "%s, %s, [%s]", regStr(instr->Rd, true),
            regStr(instr->Rm, false), regStr(instr->Rn, false));
  }
}
...
void multiply(int32_t *instrPtr, char *mBuf, char *iBuf) {
  ...
  if (!instr->L) // MLA and MUL
  {
    ...
    sprintf(mBuf, "%s%s%s", instr->A ? "mla" : "mul", condName[instr->cond],
            instr->S ? "s" : "");
    char accumReg[9];
    sprintf(accumReg, ", %s", regStr(instr->Rn, false));
    sprintf(iBuf, "%s, %s, %s%s", regStr(instr->Rd, true),
            regStr(instr->Rm, false), regStr(instr->Rs, false),
            instr->A ? accumReg : "");
  } else // long multiplies: SMLAL, SMULL, UMLAL, UMULL
  {
    sprintf(mBuf, "%c%sl%s%s", instr->U ? 's' : 'u', instr->A ? "mla" : "mul",
            condName[instr->cond], instr->A ? "s" : "");
    sprintf(iBuf, "%s, %s, %s, %s", regStr(instr->Rn, true),
            regStr(instr->Rd, true), regStr(instr->Rm, false),
...
void loadStore(int32_t *instrPtr, char *mBuf, char *iBuf, bool extra) {
  ...
  if (extra && !instrBits->bit22 &&
      instr->shift_imm !=
          1) { // this is a register form; bits11..7 should be 00001
    constant32(instrPtr, mBuf, iBuf);
    return;
  }

  if (!extra)
    sprintf(mBuf, "%s%s%s%s", instr->L ? "ldr" : "str", condName[instr->cond],
  ...
  if ((!extra && !instrBits->bit25) ||
      (extra && instrBits->bit22)) { // Immediate offset
    LoadStoreImmInstruction *instrImm = (LoadStoreImmInstruction *)instrPtr;
    LoadStoreImmMiscInstruction *instrImmMisc =
        (LoadStoreImmMiscInstruction *)instrPtr;
    sprintf(iBuf, "%s, [%s%s, #%s%u%s%s", regStr(instr->Rd, instr->L),
            regStr(instr->Rn, (!instr->P || instr->W)), instr->P ? "" : "]",
            instr->U ? "" : "-",
            extra ? concatImm4Imm4(instrImmMisc->immH, instrImmMisc->immL)
                  : instrImm->immed,
            instr->P ? "]" : "", instr->W && instr->P ? "!" : "");
  } else if (extra || (instr->shift == 0 &&
                       instr->shift_imm == 0)) { //   Register offset/index
    sprintf(iBuf, "%s, [%s%s, %s%s%s%s", regStr(instr->Rd, instr->L),
            regStr(instr->Rn, (!instr->P || instr->W)), instr->P ? "" : "]",
            instr->U ? "" : "-", regStr(instr->Rm, false), instr->P ? "]" : "",
            instr->W ? "!" : "");
  } else // Scaled register offset/index
  {
    if (instr->shift == 3 && instr->shift_imm == 0) {
      sprintf(iBuf, "%s, [%s%s, %s%s, RRX%s%s", regStr(instr->Rd, instr->L),
              regStr(instr->Rn, (!instr->P || instr->W)), instr->P ? "" : "]",
              instr->U ? "" : "-", regStr(instr->Rm, false),
              instr->P ? "]" : "", instr->W ? "!" : "");
    } else {
      static char *sh_rot[] = {"LSL", "LSR", "ASR", "ROR"};
      sprintf(
          iBuf, "%s, [%s%s, %s%s, %s #%u%s%s", regStr(instr->Rd, instr->L),
          regStr(instr->Rn, (!instr->P || instr->W)), instr->P ? "" : "]",
          instr->U ? "" : "-", regStr(instr->Rm, false), sh_rot[instr->shift],
          (instr->shift_imm == 0 && (instr->shift == 1 || instr->shift == 2))
              ? 32
              : instr->shift_imm,
          instr->P ? "]" : "", instr->W ? "!" : "");
    }
...
void loadStoreMultiple(int32_t *instrPtr, char *mBuf, char *iBuf) {
  MultipleLoadStoreInstruction *instr =
      (MultipleLoadStoreInstruction *)instrPtr;
  ...
  sprintf(mBuf, "%s%s%s", instr->L ? "ldm" : "stm", condName[instr->cond],
          instr->W ? addr_mode[PU] : "");
  sprintf(iBuf, "%s%s, {%s}%s", regStr(instr->Rn, instr->W),
          instr->W ? "!" : "", registers, instr->S ? "^" : "");
}
  ...
  if (instr->D == 1 && instr->N == 0 && instr->s == 0 && instr->xfer == 1) {
    if (isDouble) {
      if (instr->r) // L-bit
      {
        ...
        sprintf(iBuf, "%s, %s, %s", regStr(instr->Fd, true),
                regStr(instr->Fn, true),
                doubleRegStr(getVFPSecondOperand(instr), false));
      } else {
        // MFDRR
        sprintf(mBuf, "%s%s", "mfdrr", condName[instr->cond]);
        sprintf(iBuf, "%s, %s, %s",
                doubleRegStr(getVFPSecondOperand(instr), true),
      ...
      if (instr->r) // L-bit
      {
        ...
        sprintf(iBuf, "%s, %s, {%s, %s}", regStr(instr->Fd, true),
                regStr(instr->Fn, true), singleRegStr(first, false),
                singleRegStr(next, false));
      } else {
        // FMSRR
        sprintf(mBuf, "%s%s", "mfsrr", condName[instr->cond]);
        sprintf(iBuf, "{%s, %s}, %s, %s", singleRegStr(first, true),
                singleRegStr(next, true), regStr(instr->Fd, false),
...
void loadStoreCoprocessor(int32_t *instrPtr, char *mBuf, char *iBuf) {
  CoprocessorLoadStoreInstruction *instr =
      (CoprocessorLoadStoreInstruction *)instrPtr;
#ifdef __VFP_FP__
  char tempStr[32] = "";
  ...
  case 6:
    ...
    sprintf(mBuf, "%s%s%s", instr->L ? "fld" : "fst", condName[instr->cond],
            isDouble ? "d" : "s");
    if (instr->offset8)
  ...
  case 5: {
    ...
    if (isDouble) {
      sprintf(mBuf, "%s%s%s%s", instr->L ? "fldm" : "fstm",
              (puw != 5) ? "ia" : "db", (instr->offset8 % 2 == 0) ? "d" : "x",
              condName[instr->cond]);
      sprintf(iBuf, "%s%s {%s%s%s}",
              (puw == 2) ? regStr(instr->Rn, false) : regStr(instr->Rn, true),
              (puw != 2) ? "!," : ",", singleRegStr(start, false),
              (instr->offset8 >= 2 * 2) ? ".." : "",
              (instr->offset8 >= 2 * 2)
                  ? singleRegStr(start + (instr->offset8 >> 1), false)
                  : "");
    } else {
      sprintf(mBuf, "%s%s%s", instr->L ? "fldm" : "fstm",
              (puw != 5) ? "ias" : "dbs", condName[instr->cond]);
      sprintf(iBuf, "%s%s {%s%s%s}",
              (puw == 2) ? regStr(instr->Rn, false) : regStr(instr->Rn, true),
              (puw != 2) ? "!," : ",", singleRegStr(start, false),
              (instr->offset8 >= 2 * 2) ? ".." : "",
              (instr->offset8 >= 2 * 2)
                  ? singleRegStr(start + instr->offset8, false)
                  : "");
    }
    break;
  }
  ...
  case 7:
    constant32(instrPtr, mBuf, iBuf);
    break;
    // Case 0 is the two-register transfer instruction, captured at the
    // beginning
  }
#else
  sprintf(mBuf, "%s%s%s", instr->L ? "ldc" : "stc", condName[instr->cond],
          instr->N ? "l" : "");
  sprintf(iBuf, "p%d, CR%d, [%s%s %s%s%d%s%s%s", instr->cp, instr->CR,
          regStr(instr->Rn, instr->W), instr->P ? "," : "],",
          (instr->P || instr->W) ? "#" : "{",
...
void vfpExtension(int32_t *instrPtr, char *mBuf, char *iBuf) {
  ...
  case 0:
    ...
    sprintf(mBuf, "%s%s%s", instr->N ? "fabs" : "fcpy", isDouble ? "d" : "s",
            condName[instr->cond]);
    sprintf(iBuf, "%s, %s",
            isDouble ? doubleRegStr(getVFPDestination(instr), true)
  ...
  case 1:
    ...
    sprintf(mBuf, "%s%s%s", instr->N ? "fsqrt" : "fneg", isDouble ? "d" : "s",
            condName[instr->cond]);
    sprintf(iBuf, "%s, %s",
            isDouble ? doubleRegStr(getVFPDestination(instr), true)
                     : singleRegStr(getVFPDestination(instr), true),
  ...
  case 4:
    ...
    sprintf(mBuf, "%s%s%s", instr->N ? "fcmpe" : "fcmp", isDouble ? "d" : "s",
            condName[instr->cond]);
    sprintf(iBuf, "%s, %s",
  ...
  case 7:
    if (instr->N) {
      // FCVTDS/FCVTSD
      sprintf(mBuf, "%s%s", isDouble ? "fcvtsd" : "fcvtds",
              condName[instr->cond]);
      sprintf(iBuf, "%s, %s",
  ...
  case 8:
    ...
    sprintf(mBuf, "%s%s%s", instr->N ? "fsito" : "fuito", isDouble ? "d" : "s",
            condName[instr->cond]);
    sprintf(iBuf, "%s, %s",
            isDouble ? doubleRegStr(getVFPDestination(instr), true)
                     : singleRegStr(getVFPDestination(instr), true),
  ...
  case 12:
    ...
    sprintf(mBuf, "%s%s%s", instr->N ? "ftouiz" : "ftoui", isDouble ? "d" : "s",
            condName[instr->cond]);
  ...
  case 13:
    ...
    sprintf(mBuf, "%s%s%s", instr->N ? "ftosiz" : "ftosi", isDouble ? "d" : "s",
            condName[instr->cond]);
    sprintf(iBuf, "%s, %s", singleRegStr(getVFPDestination(instr), true),
            isDouble ? doubleRegStr(getVFPSecondOperand(instr), false)
                     : singleRegStr(getVFPSecondOperand(instr), false));
...
void vfpSingleRegTransfer(int32_t *instrPtr, char *mBuf, char *iBuf) {
  ...
  if (opCode == 0) {
    sprintf(mBuf, "%s%s",
            isDouble ? (instr->r ? "fmrs" : "fmsr")
                     : (instr->r ? "fmrdl" : "fmdlr"),
            condName[instr->cond]);
    if (isDouble) {
      if (instr->r) // L-bit
        sprintf(iBuf, "%s, %s[31:0]", regStr(instr->Fd, true),
                doubleRegStr(getVFPFirstOperand(instr), false));
      else
        sprintf(iBuf, "%s[31:0], %s",
                doubleRegStr(getVFPFirstOperand(instr), true),
      ...
      if (instr->r) // L-bit
        ...
        sprintf(iBuf, "%s, %s", singleRegStr(getVFPFirstOperand(instr), true),
                regStr(instr->Fd, false));
    }
  } else if (opCode == 1) {
    if (isDouble) {
      sprintf(mBuf, "%s%s", instr->r ? "fmrdh" : "fmdhr",
              condName[instr->cond]);
      if (instr->r) // L-bit
        sprintf(iBuf, "%s, %s[63:32]", regStr(instr->Fd, true),
                doubleRegStr(getVFPFirstOperand(instr), false));
      else
        sprintf(iBuf, "%s[63:32], %s",
                doubleRegStr(getVFPFirstOperand(instr), true),
                regStr(instr->Fd, false));
    } else
...
void vfpDataProcessing(int32_t *instrPtr, char *mBuf, char *iBuf) {
  VFPInstruction *instr = (VFPInstruction *)instrPtr;
  uint32_t opCode =
      (uint32_t)(instr->p << 3 | instr->q << 2 | instr->r << 1 | instr->s);
  bool isDouble = (instr->cp == 11); // Coprocessor 11 is double, 10 is float
  ...
  if (opCode == 15)
    ...
    sprintf(iBuf, "%s, %s, %s",
            isDouble ? doubleRegStr(getVFPDestination(instr), true)
                     : singleRegStr(getVFPDestination(instr), true),
            isDouble ? singleRegStr(getVFPFirstOperand(instr), false)
                     : doubleRegStr(getVFPFirstOperand(instr), false),
            isDouble ? singleRegStr(getVFPSecondOperand(instr), false)
                     : doubleRegStr(getVFPSecondOperand(instr), false));
  }
