...
#ifdef __VFP_FP__
const char* doubleRegStr(
    uint32_t regNum, bool W) {  // If a register value is to be updated by the
...
void dataProcessing(int32_t* instrPtr, char* mBuf, char* iBuf) {
  ...
  bool mov =
      instrBits->bit21 && (instrBits->bits23_24 ==
                           3);  // instr->opcode: 0b1101 || 0b1111 (mov, mvn)
...
void vfpExtension(int32_t* instrPtr, char* mBuf, char* iBuf) {
  ...
  switch (instr->Fn) {
    ...
    case 8:
      ...
      sprintf(mBuf, "%s%s%s", instr->N ? "fsito" : "fuito",
              isDouble ? "d" : "s", condName[instr->cond]);
      sprintf(iBuf, "%s, %s",
              isDouble ? doubleRegStr(getVFPDestination(instr), true)
                       : singleRegStr(getVFPDestination(instr), true),
    ...
    case 12:
      ...
      sprintf(mBuf, "%s%s%s", instr->N ? "ftouiz" : "ftoui",
              isDouble ? "d" : "s", condName[instr->cond]);
    ...
    case 13:
      ...
      sprintf(mBuf, "%s%s%s", instr->N ? "ftosiz" : "ftosi",
              isDouble ? "d" : "s", condName[instr->cond]);
      sprintf(iBuf, "%s, %s", singleRegStr(getVFPDestination(instr), true),
              isDouble ? doubleRegStr(getVFPSecondOperand(instr), false)
                       : singleRegStr(getVFPSecondOperand(instr), false));
