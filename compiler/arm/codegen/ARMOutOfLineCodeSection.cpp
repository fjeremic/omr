...
TR_ARMOutOfLineCodeSection::TR_ARMOutOfLineCodeSection(
...
void TR_ARMOutOfLineCodeSection::generateARMOutOfLineCodeSectionDispatch() {
  ...
  if (_targetReg) {
    ...
    generateTrg1Src1Instruction(_cg, ARMOp_mov, _callNode, _targetReg,
                                resultReg);
  }
  ...
  generateLabelInstruction(_cg, ARMOp_label, _callNode,
                           generateLabelSymbol(_cg));
...
void TR_ARMOutOfLineCodeSection::assignRegisters(
  ...
  // This prevents the OOL entry label from resetting all register's
  // startOfranges during RA
  _cg->toggleIsInOOLSection();
  TR::RegisterDependencyConditions *liveRealRegDeps =
      _cg->machine()->createCondForLiveAndSpilledGPRs(
          true, _cg->getSpilledRegisterList());

  if (liveRealRegDeps)
    _firstInstruction->setDependencyConditions(liveRealRegDeps);
  _cg->toggleIsInOOLSection();  // toggle it back because
                                // swapInstructionListsWithCompilation() also
                                // calls toggle...
  ...
  // If a real reg is assigned at the end of the hot path and free at the
  // beginning, but unused on the cold path it won't show up in the
  // liveRealRegDeps. If a dead virtual occupies that real reg we need to clean
  // it up.
  for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastFPR;
       ++i) {
    TR::RealRegister *r =
        _cg->machine()->getRealRegister((TR::RealRegister::RegNum)i);
