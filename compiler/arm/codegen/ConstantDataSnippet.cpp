...
int32_t TR::ARMConstantDataSnippet::addConstantRequest(
    void *v, TR::DataType type, TR::Instruction *nibble0,
    TR::Instruction *nibble1, TR::Instruction *nibble2,
    TR::Instruction *nibble3, TR::Instruction *nibble4, TR::Node *node,
    ...
    case TR::Address: {
      ...
      if (acursor == NULL) {
        acursor = new (_cg->trHeapMemory())
            TR::ARMConstant<intptrj_t>(_cg, ain, node, isUnloadablePicSite);
        _addressConstants.add(acursor);
      }
      acursor->addValueRequest(nibble0, nibble1, nibble2, nibble3);
    } break;
    default:
      // TR_ASSERT(0, "Only float and address constants are supported. Data type
      // is %d.\n", type);
...
bool TR::ARMConstantDataSnippet::getRequestorsFromNibble(
    TR::Instruction *nibble, TR::Instruction **q, bool remove) {
      ...
      *(intptrj_t *)codeCursor = acursor->getConstantValue();
      if (cg()->profiledPointersRequireRelocation()) {
        ...
        if (acursor->isUnloadablePicSite()) {
          // Register an unload assumption on the lower 32bit of the class
          // constant. The patching code thinks it's low bit tagging an
          // instruction not a class pointer!!
          cg()->jitAddPicToPatchOnClassUnload(
              (void *)acursor->getConstantValue(),
              (void *)(codeCursor +
                       ((TR::Compiler->target.is64Bit()) ? 4 : 0)));
        }
      }
      ...
      for (i = 0; i < size; i += count) {
        ...
        if (!cg()->comp()->compileRelocatableCode()) {
          ...
          *(uint16_t *)iloc1 =
              ((*((uint16_t *)iloc1)) & 0xf000) | 0x400 |
              localVal.getByte3();  // 0x400 means rotate right by 8bit
          *(uint16_t *)iloc2 =
              ((*((uint16_t *)iloc2)) & 0xf000) | 0x800 | localVal.getByte2();
          *(uint16_t *)iloc3 =
              ((*((uint16_t *)iloc3)) & 0xf000) | 0xc00 | localVal.getByte1();
          *(uint16_t *)iloc4 =
              ((*((uint16_t *)iloc4)) & 0xf000) | localVal.getByte1();
        } else {
          cg()->addExternalRelocation(
              new (cg()->trHeapMemory())
                  TR::BeforeBinaryEncodingExternalRelocation(
                      requestors[i], (uint8_t *)(addr),
                      (uint8_t *)fixedSequence4, TR_FixedSequenceAddress2,
                      cg()),
              __FILE__, __LINE__, requestors[i]->getNode());
        }
      }
