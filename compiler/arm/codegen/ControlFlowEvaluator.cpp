  ...
  TR_ARMConditionCode branchType, TR::Node* node, TR::CodeGenerator* cg, TR::SymbolReference* sr = NULL)
{
  ...
  if (!cg->comp()->getOption(TR_OptimizeForSpace) && firstChild->getOpCodeValue() == TR:: instanceof
      && firstChild->getRegister() == NULL && node->getReferenceCount() <= 1 && (value == 0 || value == 1)) {
    TR_ASSERT(node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ificmpne, "Unexpected opcode");
  ...
  if (!sr) {
    ...
    result = generateImmSymInstruction(
      cg, ARMOp_bl, node, (uintptr_t)sr->getMethodAddress(), deps, sr, NULL, NULL, branchType);
...
static TR::Register* compareLongsForOrder(TR_ARMConditionCode branchOp, TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (cg->comp()->getOption(TR_DisableOOL) || disableOOLForLongCompares) {
    ...
    TR::Instruction* cursor
      = generateSrc2Instruction(cg, ARMOp_cmp, node, src1Reg->getHighOrder(), src2Reg->getHighOrder());
    ...
    cursorOOL
      = generateLabelInstruction(cg, ARMOp_label, node, returnLabel, oolDeps, cursorOOL); // Need deps 3/21 change
...
static void lookupScheme3(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced)
{
  ...
  int32_t* dataTable
    = (int32_t*)cg->allocateCodeMemory(numberOfEntries * sizeof(int32_t), cg->getCurrentEvaluationBlock()->isCold());
...
static void lookupScheme4(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  /*  TODO implement lookups
     ...
     *pivotRegister = cg->allocateRegister(); TR::RegisterDependencyConditions *conditions = new (cg->trHeapMemory())
     ...
     new (cg->trHeapMemory()) TR::ARMTrg1Src1Imm2Instruction(0xfffffffc, 31, pivotRegister, pivotRegister, ARMOp_rlwinm,
     cg); generateTrg1MemInstruction(cg, ARMOp_lwzx, dataRegister, new (cg->trHeapMemory())
     ...
     cndRegister); generateLabelInstruction(node, cg, ARMOp_bl, linkLabel); generateLabelInstruction(node, cg,
     ARMOp_label, linkLabel); new (cg->trHeapMemory()) TR::ARMTrg1Instruction(ARMOp_mflr, dataRegister, cg);
...
TR::Register* OMR::ARM::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (numBranchTableEntries >= 5) {
    ...
    // the matching GlDepRegs free checks in OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..) must be
    // updated to synch.
  ...
  if (numBranchTableEntries < 5) {
    ...
    generateLabelInstruction(
      cg, ARMOp_b, node, node->getChild(i)->getBranchDestination()->getNode()->getLabel(), conditions);
  }
...
#error("NULL is not 0, must fix");
    ...
    TR::Instruction* instr1 = generateImmSymInstruction(cg, ARMOp_bl, node,
...
TR::Register* OMR::ARM::TreeEvaluator::DIVCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!secondChild->getOpCode().isLoadConst() || (dtype.isInt32() && secondChild->getInt() == 0)
    ...
    TR::Instruction* instr1 = generateImmSymInstruction(cg, ARMOp_bl, node,
      (uintptr_t)arithmeticException->getMethodAddress(), NULL, arithmeticException, NULL, NULL, ARMConditionCodeEQ);
    instr1->ARMNeedsGCMap(0xFFFFFFFF);
    cg->machine()->setLinkRegisterKilled(true);
  }
  ...
  if (/* !isSmall() && */ firstChild->getOpCode().isLoadConst()) {
    if (secondChild->getOpCode().isLoadConst()) {
      if (firstChild->getInt() < secondChild->getInt()) {
        ...
        instr = generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)BNDCHKException->getMethodAddress(), NULL,
          BNDCHKException, NULL, NULL, ARMConditionCodeAL);
        cg->machine()->setLinkRegisterKilled(true);
      } else {
  ...
  TR::Node* node, TR::Register* srcReg, TR::Register* dstReg, bool srcIsNonNull, TR::CodeGenerator* cg)
{
  ...
  TR::Instruction* gcPoint = generateImmSymInstruction(
    cg, ARMOp_bl, node, (uintptr_t)arrayStoreChkHelper->getMethodAddress(), deps, arrayStoreChkHelper);
