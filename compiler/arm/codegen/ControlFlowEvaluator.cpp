...
static void lookupScheme1(TR::CodeGenerator *cg, TR::Node *node,
                          bool unbalanced);
static void lookupScheme2(TR::CodeGenerator *cg, TR::Node *node,
                          bool unbalanced);
static void lookupScheme3(TR::CodeGenerator *cg, TR::Node *node,
                          bool unbalanced);
...
TR::Register *OMR::ARM::TreeEvaluator::gotoEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::LabelSymbol *gotoLabel =
      node->getBranchDestination()->getNode()->getLabel();
  if (node->getNumChildren() > 0) {
    TR::Node *child = node->getFirstChild();
    cg->evaluate(child);
    generateLabelInstruction(
        cg, ARMOp_b, node, gotoLabel,
...
TR::Register *OMR::ARM::TreeEvaluator::ireturnEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  ...
  TR::addDependency(deps, returnRegister,
                    cg->getProperties().getIntegerReturnRegister(), TR_GPR, cg);
...
TR::Register *OMR::ARM::TreeEvaluator::lreturnEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Register *returnRegister = cg->evaluate(node->getFirstChild());
  ...
  TR::addDependency(deps, lowReg,
                    cg->getProperties().getLongLowReturnRegister(), TR_GPR, cg);
  TR::addDependency(deps, highReg,
...
TR::Register *OMR::ARM::TreeEvaluator::returnEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
static TR::Instruction *compareIntsForOrder(TR_ARMConditionCode branchType,
  ...
  TR::LabelSymbol *branchTarget =
      sr ? NULL : node->getBranchDestination()->getNode()->getLabel();
  ...
  if (!cg->comp()->getOption(TR_OptimizeForSpace) &&
      ...
      && firstChild->getRegister() == NULL && node->getReferenceCount() <= 1 &&
          (value == 0 || value == 1)) {
    TR_ASSERT(node->getOpCodeValue() == TR::ificmpeq ||
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    negated = cmpValue < 0 &&
              cmpValue != 0x80000000;  // the negative of the maximum negative
                                       // int is itself so can not use cmn here
    if (constantIsImmed8r(negated ? -cmpValue : cmpValue, &base, &rotate)) {
      generateSrc1ImmInstruction(cg, negated ? ARMOp_cmn : ARMOp_cmp, node,
                                 src1Reg, base, rotate);
      foundConst = true;
    }
  }
  if (!foundConst)
    generateSrc2Instruction(cg, ARMOp_cmp, node, src1Reg,
  ...
  if (node->getNumChildren() == 3) {
    ...
    thirdChild->decReferenceCount();  // IS it correct to decReferenceCount
                                      // before the instruction generation ?!!!
  }

  if (!sr) {
    result = generateConditionalBranchInstruction(cg, node, branchType,
                                                  branchTarget, deps);
  } else {
    result = generateImmSymInstruction(cg, ARMOp_bl, node,
                                       (uintptr_t)sr->getMethodAddress(), deps,
                                       sr, NULL, NULL, branchType);
...
TR::Instruction *OMR::ARM::TreeEvaluator::compareIntsForEquality(
    TR_ARMConditionCode branchType, TR::Node *node, TR::CodeGenerator *cg,
...
TR::Register *OMR::ARM::TreeEvaluator::ificmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForEquality(TR::ificmpeq == node->getOpCodeValue()
                             ? ARMConditionCodeEQ
                             : ARMConditionCodeNE,
                         node, cg);
  return NULL;
...
TR::Register *OMR::ARM::TreeEvaluator::ificmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeLT, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::ificmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeGE, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::ificmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeGT, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::ificmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeLE, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::ifiucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeCC, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::ifiucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeCS, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::ifiucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeHI, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::ifiucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  compareIntsForOrder(ARMConditionCodeLS, node, cg);
  return NULL;
}

TR::Register *OMR::ARM::TreeEvaluator::iflcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  TR_ARMConditionCode condition = node->getOpCodeValue() == TR::iflcmpeq
                                      ? ARMConditionCodeEQ
                                      : ARMConditionCodeNE;
  ...
  cursor = generateSrc2Instruction(cg, ARMOp_cmp, node, src1Reg->getHighOrder(),
                                   src2Reg->getHighOrder());
  cursor = generateSrc2Instruction(cg, ARMOp_cmp, node, src1Reg->getLowOrder(),
                                   src2Reg->getLowOrder());
  cursor->setConditionCode(
      ARMConditionCodeEQ);  // make the second instruction conditional
  cursor = generateConditionalBranchInstruction(cg, node, condition,
                                                destinationLabel, deps);
...
static TR::Register *compareLongAndSetOrderedBoolean(
  ...
  if (true) {
    TR::Register *src2Reg = cg->evaluate(secondChild);
    TR::ARMControlFlowInstruction *cfop =
        generateControlFlowInstruction(cg, ARMOp_setblong, node, deps);
...
static TR::Register *compareLongsForOrder(TR_ARMConditionCode branchOp,
                                          TR::Node *node,
                                          TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  // allocator that can handle registers being alive across basic block
  // boundaries. For now we just generate pessimistic code.
  static bool disableOOLForLongCompares =
      (feGetEnv("TR_DisableOOLForLongCompares") != NULL);
  TR::Register *src2Reg = cg->evaluate(secondChild);
  if (cg->comp()->getOption(TR_DisableOOL) || disableOOLForLongCompares) {
    TR::ARMControlFlowInstruction *cfop =
        generateControlFlowInstruction(cg, ARMOp_iflong, node, deps);
    cfop->addSourceRegister(src1Reg->getHighOrder());
    cfop->addSourceRegister(src1Reg->getLowOrder());
    cfop->addSourceRegister(src2Reg->getHighOrder());
    cfop->addSourceRegister(src2Reg->getLowOrder());
    ...
    TR::LabelSymbol *startOOLLabel =
        TR::LabelSymbol::create(cg->trHeapMemory(), cg);
    TR::LabelSymbol *doneOOLLabel =
        TR::LabelSymbol::create(cg->trHeapMemory(), cg);
    ...
    TR::Instruction *cursor = generateSrc2Instruction(
        cg, ARMOp_cmp, node, src1Reg->getHighOrder(), src2Reg->getHighOrder());
    cursor = generateConditionalBranchInstruction(
        cg, node, isGT ? ARMConditionCodeLE : ARMConditionCodeGE, startOOLLabel,
        deps, cursor);
    if (debugObj)
    ...
    outlinedSlowPath = new (cg->trHeapMemory())
        TR_ARMOutOfLineCodeSection(startOOLLabel, doneOOLLabel, cg);
    cg->getARMOutOfLineCodeSectionList().add(outlinedSlowPath);

    // Hot path
    cursor = generateTrg1ImmInstruction(cg, ARMOp_mov, node, tempReg, 1, 0,
                                        cursor);  // Setting tempReg to 1

    // Start OOL
    // The compare result has to be live until the end of the last instruction
    // in the mainline, so put it as a dependency
    TR::RegisterDependencyConditions *tempDeps = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(1, 1, cg->trMemory());
    ...
    // Need the two sources for compare to survive until the returnLabel.  Use a
    // dependency to hold that, and hold until the compare for them are done.
    TR::RegisterDependencyConditions *newDeps = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(2, 2, cg->trMemory());
    TR::addDependency(newDeps, src1Reg->getLowOrder(), TR::RealRegister::NoReg,
                      TR_GPR, cg);
    TR::addDependency(newDeps, src2Reg->getLowOrder(), TR::RealRegister::NoReg,
                      TR_GPR, cg);

    TR::RegisterDependencyConditions *oolDeps = deps->clone(cg, newDeps);

    outlinedSlowPath
        ->swapInstructionListsWithCompilation();  // OOL code to follow
    TR::Instruction *cursorOOL =
        generateLabelInstruction(cg, ARMOp_label, node, startOOLLabel);
    if (debugObj)
      debugObj->addInstructionComment(cursorOOL,
                                      "Denotes start of OOL iflong sequence");
    // XXX: Temporary fix, OOL instruction stream does not pick up live locals
    // or monitors correctly.
    TR_ASSERT(!cursorOOL->getLiveLocals() && !cursorOOL->getLiveMonitors(),
              "Expecting first OOL instruction to not have live "
              "locals/monitors info");
    cursorOOL->setLiveLocals(OOLBranchInstr->getLiveLocals());
    cursorOOL->setLiveMonitors(OOLBranchInstr->getLiveMonitors());

    cursorOOL = generateTrg1ImmInstruction(cg, ARMOp_mov, node, tempReg, 0, 0,
                                           cursorOOL);  // Setting tempReg to 0
    cursorOOL = generateConditionalBranchInstruction(
        cg, node, isGT ? ARMConditionCodeLT : ARMConditionCodeGT, returnLabel,
        cursorOOL);
    cursorOOL =
        generateSrc2Instruction(cg, ARMOp_cmp, node, src1Reg->getLowOrder(),
                                src2Reg->getLowOrder(), cursorOOL);
    cursorOOL = generateTrg1ImmInstruction(cg, ARMOp_mov, node, tempReg, 1, 0,
                                           cursorOOL);  // Setting tempReg to 1
    cursorOOL->setConditionCode(isGT ? ARMConditionCodeHI : ARMConditionCodeLS);
    cursorOOL =
        generateLabelInstruction(cg, ARMOp_label, node, returnLabel, oolDeps,
                                 cursorOOL);  // Need deps 3/21 change
    cursorOOL =
        generateLabelInstruction(cg, ARMOp_b, node, doneOOLLabel, cursorOOL);
    if (debugObj)
      debugObj->addInstructionComment(
          cursorOOL, "Denotes end of OOL iflong sequence: return to mainline");
    outlinedSlowPath
        ->swapInstructionListsWithCompilation();  // Done with OOL code
    ...
    cursor =
        generateSrc1ImmInstruction(cg, ARMOp_tst, node, tempReg, 1, 0, cursor);
    cursor = generateConditionalBranchInstruction(
        cg, node, ARMConditionCodeNE, destinationLabel, deps, cursor);
...
TR::Register *OMR::ARM::TreeEvaluator::iflcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  generateTrg1Src2Instruction(cg, ARMOp_sub_r, node, tempReg,
                              src1Reg->getLowOrder(), src2Reg->getLowOrder());
  generateTrg1Src2Instruction(cg, ARMOp_sbc_r, node, tempReg,
                              src1Reg->getHighOrder(), src2Reg->getHighOrder());
  generateConditionalBranchInstruction(cg, node, ARMConditionCodeLT,
                                       destinationLabel, deps);
...
TR::Register *OMR::ARM::TreeEvaluator::iflcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  generateTrg1Src2Instruction(cg, ARMOp_sbc_r, node, tempReg,
                              src1Reg->getHighOrder(), src2Reg->getHighOrder());
  generateConditionalBranchInstruction(cg, node, ARMConditionCodeGE,
                                       destinationLabel, deps);
...
TR::Register *OMR::ARM::TreeEvaluator::iflucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  generateTrg1Src2Instruction(cg, ARMOp_sbc_r, node, tempReg,
                              src1Reg->getHighOrder(), src2Reg->getHighOrder());
  generateConditionalBranchInstruction(cg, node, ARMConditionCodeCC,
                                       destinationLabel, deps);
...
TR::Register *OMR::ARM::TreeEvaluator::iflucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  generateTrg1Src2Instruction(cg, ARMOp_sbc_r, node, tempReg,
                              src1Reg->getHighOrder(), src2Reg->getHighOrder());
  generateConditionalBranchInstruction(cg, node, ARMConditionCodeCS,
                                       destinationLabel, deps);
...
TR::Register *OMR::ARM::TreeEvaluator::iflcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongsForOrder(ARMConditionCodeGT, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::iflcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongsForOrder(ARMConditionCodeLE, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::iflucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongsForOrder(ARMConditionCodeHI, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::iflucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongsForOrder(ARMConditionCodeLS, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::ifacmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
static TR::Register *icmpHelper(TR::Node *node, TR_ARMConditionCode cond,
                                TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateRegister();
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
...
TR::Register *OMR::ARM::TreeEvaluator::icmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
TR::Register *OMR::ARM::TreeEvaluator::icmpneEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeNE, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::icmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeLT, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::icmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeLE, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::icmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeGE, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::icmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeGT, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::iucmpltEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeCC, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::iucmpleEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeLS, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::iucmpgeEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeCS, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::iucmpgtEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return icmpHelper(node, ARMConditionCodeHI, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lcmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeEQ, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lcmpneEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeNE, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lcmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeLT, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lcmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeGE, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lcmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeGT, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lcmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeLE, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lucmpltEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeCC, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lucmpgeEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeCS, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lucmpgtEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeHI, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lucmpleEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(ARMConditionCodeLS, node, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::lcmpEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateRegister();
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  TR::Register *src2Reg = cg->evaluate(secondChild);
  TR::ARMControlFlowInstruction *cfop =
      generateControlFlowInstruction(cg, ARMOp_lcmp, node, deps);
  cfop->addTargetRegister(trgReg);
  cfop->addSourceRegister(src1Reg->getLowOrder());
  cfop->addSourceRegister(src1Reg->getHighOrder());
  cfop->addSourceRegister(src2Reg->getLowOrder());
...
TR::Register *OMR::ARM::TreeEvaluator::acmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
TR::Register *OMR::ARM::TreeEvaluator::lookupEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
static bool isGlDepsUnBalanced(TR::CodeGenerator *cg, TR::Node *node) {
  ...
  TR::Node *defaultDeps =
      defaultNode->getNumChildren() > 0 ? defaultNode->getFirstChild() : NULL;
  ...
  for (i = 2; i < numChildren; i++) {
    ...
    TR::Node *caseDeps =
        caseNode->getNumChildren() > 0 ? caseNode->getFirstChild() : NULL;
    if ((!caseDeps && numDefDeps != 0) ||
  ...
  for (i = 2; i < numChildren; i++) {
    ...
    if (!(defDepMap == caseDepMap)) return true;
  }

  return false;
}

// Note: When the checks and/or lookupScheme implementations are changed with
// regards to number of used registers (or dependency), the matching GlDepRegs
// free checks in
// OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..) must be
// updated to synch.
static void switchDispatch(TR::CodeGenerator *cg, TR::Node *node) {
  ...
  if (!unbalanced) {
    for (i = 2; i < total; i++) {
      if (node->getChild(i)->getNumChildren() > 0) {
        ...
        if (caseDepsNode != NULL) cg->evaluate(caseDepsNode);
      }
    }
  }
  ...
  if (total <= 15) {
    uint32_t base, rotate;
    for (i = 2;
         i < total && constantIsImmed8r(node->getChild(i)->getCaseConstant(),
                                        &base, &rotate);
         i++)
      ;
    if (i == total) {
  ...
  if (total <= 9) {
    ...
    for (i = 3; i < total; i++) {
      ...
      uint32_t base, rotate;
      preInt += diff;
      if (diff < 0 || !constantIsImmed8r(diff, &base, &rotate)) break;
    }
    if (i >= total) {
  ...
  //       OMRCodeGeneratorExt.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..)
  //       must be updated.
  lookupScheme3(cg, node, unbalanced);
#endif
}
...
// free checks in
// OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..) must be
// updated to synch. Called by switchDispatch().
static void lookupScheme1(TR::CodeGenerator *cg, TR::Node *node,
                          bool unbalanced) {
  ...
  if (defDepsNode && !unbalanced) {
    ...
    defaultDeps = defaultDeps->clone(
        cg, generateRegisterDependencyConditions(cg, defDepsNode, 0));
  }

  for (int i = 2; i < node->getNumChildren(); i++) {
    TR::Node *caseNode = node->getChild(i);
    TR::LabelSymbol *caseTargetLabel =
    ...
    if (unbalanced) {
      ...
      if (caseNode->getNumChildren() > 0) {
        ...
        caseDeps = caseDeps->clone(
            cg, generateRegisterDependencyConditions(cg, caseDepsNode, 0));
      }
      generateConditionalBranchInstruction(cg, node, ARMConditionCodeEQ,
                                           caseTargetLabel, caseDeps);
    } else {
      generateConditionalBranchInstruction(cg, node, ARMConditionCodeEQ,
                                           caseTargetLabel);
    }
  ...
  TR::LabelSymbol *defaultTargetLabel =
      defaultNode->getBranchDestination()->getNode()->getLabel();
  ...
  for (int i = 2; i < numChildren; i++) {
    ...
    if (unbalanced) {
      ...
      if (caseNode->getNumChildren() > 0) {
        ...
        caseDeps = caseDeps->clone(cg, generateRegisterDependencyConditions(
    ...
    if (i < numChildren - 1) {
      ...
      generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, caseConstReg,
                                     caseConstReg, base, rotate);
    }
  }
...
static void lookupScheme3(TR::CodeGenerator *cg, TR::Node *node,
  ...
  int32_t *dataTable = (int32_t *)cg->allocateCodeMemory(
      numberOfEntries * sizeof(int32_t),
  ...
  TR::Node *defaultNode = node->getSecondChild();
  TR::Node *defDepsNode =
      defaultNode->getNumChildren() > 0 ? defaultNode->getFirstChild() : 0;
...
static void lookupScheme4(TR::Node *node, TR::CodeGenerator *cg) {
  cg->comp()->failCompilation<TR::CompilationException>(
      "Automatically failing on lookup scheme 4");

  /*  TODO implement lookups
     int32_t  total = node->getNumChildren();
     int32_t  numberOfEntries = total - 2;
     int32_t *dataTable = (int32_t *)cg->allocateCodeMemory(numberOfEntries *
     sizeof(int), cg->getCurrentEvaluationBlock()->isCold()); int32_t  address =
     (int32_t)dataTable; TR::Register *selector =
     cg->evaluate(node->getFirstChild()); TR::Register *cndRegister =
     cg->allocateRegister(TR_CCR); TR::Register *addrRegister =
     cg->allocateRegister(); TR::Register *dataRegister =
     cg->allocateRegister(); TR::Register *lowRegister = cg->allocateRegister();
     ...
     cg->machine()->setLinkRegisterKilled(true);
     TR::addDependency(conditions, dataRegister, TR::RealRegister::NoReg,
     TR_GPR, cg); TR::addDependency(conditions, addrRegister,
     TR::RealRegister::NoReg, TR_GPR, cg); TR::addDependency(conditions,
     selector, TR::RealRegister::NoReg, TR_GPR, cg);
     TR::addDependency(conditions, lowRegister, TR::RealRegister::NoReg, TR_GPR,
     cg); TR::addDependency(conditions, highRegister, TR::RealRegister::NoReg,
     TR_GPR, cg); TR::addDependency(conditions, pivotRegister,
     TR::RealRegister::NoReg, TR_GPR, cg); TR::addDependency(conditions,
     cndRegister, TR::RealRegister::NoReg, TR_CCR, cg);
     ...
     TR::LabelSymbol *backLabel =
     TR::LabelSymbol::create(cg->trHeapMemory(),cg); TR::LabelSymbol
     *searchLabel = TR::LabelSymbol::create(cg->trHeapMemory(),cg);
     TR::LabelSymbol *biggerLabel =
     TR::LabelSymbol::create(cg->trHeapMemory(),cg); TR::LabelSymbol *linkLabel
     = TR::LabelSymbol::create(cg->trHeapMemory(),cg);

     generateLabelInstruction(node, cg, ARMOp_label, backLabel);
     generateTrg1Src2Instruction(node, cg, ARMOp_add, pivotRegister,
     highRegister, lowRegister); new (cg->trHeapMemory())
     TR::ARMTrg1Src1Imm2Instruction(0xfffffffc, 31, pivotRegister,
     pivotRegister, ARMOp_rlwinm, cg); generateTrg1MemInstruction(cg,
     ARMOp_lwzx, dataRegister, new (cg->trHeapMemory())
     TR_ARMMemoryReference(addrRegister, pivotRegister, cg));
     generateTrg1Src2Instruction(node, cg, ARMOp_cmp4, cndRegister,
     dataRegister, selector); generateConditionalBranchInstruction(node, cg,
     ARMOp_bne, searchLabel, cndRegister); generateLabelInstruction(node, cg,
     ARMOp_bl, linkLabel); generateLabelInstruction(node, cg, ARMOp_label,
     linkLabel); new (cg->trHeapMemory()) TR::ARMTrg1Instruction(ARMOp_mflr,
     dataRegister, cg); generateTrg1Src1ImmInstruction(node, cg, ARMOp_addi,
     pivotRegister, pivotRegister, 20); generateTrg1Src2Instruction(node, cg,
     ARMOp_add, dataRegister, pivotRegister, dataRegister); new
     (cg->trHeapMemory()) TR::ARMSrc1Instruction(ARMOp_mtctr, dataRegister, cg);
     new (cg->trHeapMemory()) TR::Instruction(ARMOp_bctr, cg);
     for (int32_t ii=2; ii<total; ii++)
        {
        generateLabelInstruction(node, cg, ARMOp_b,
     ...
     generateConditionalBranchInstruction(node, cg, ARMOp_bgt, biggerLabel,
     cndRegister); generateTrg1Src2Instruction(node, cg, ARMOp_cmp4,
     cndRegister, pivotRegister, highRegister);
     generateConditionalBranchInstruction(node, cg, ARMOp_beq,
     node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
     cndRegister); generateTrg1Src1ImmInstruction(node, cg, ARMOp_addi,
     lowRegister, pivotRegister, 4); generateLabelInstruction(node, cg, ARMOp_b,
     backLabel);

     generateLabelInstruction(node, cg, ARMOp_label, biggerLabel);
     generateTrg1Src2Instruction(node, cg, ARMOp_cmp4, cndRegister,
     pivotRegister, lowRegister); generateConditionalBranchInstruction(node, cg,
     ARMOp_beq,
     node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
     cndRegister); generateTrg1Src1ImmInstruction(node, cg, ARMOp_addi,
     highRegister, pivotRegister, -4); new (cg->trHeapMemory())
     TR::ARMDepLabelInstruction(ARMOp_b, backLabel, conditions, cg);
     */
}

TR::Register *OMR::ARM::TreeEvaluator::tableEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  int32_t total = node->getNumChildren();
  ...
  if (numBranchTableEntries >= 5) {
    // Note: When implementation is changed with regards to number of used
    // registers (or dependency), the matching GlDepRegs free checks in
    // OMRCodeGenerator.cpp#getMaximumNumberOfGPRsAllowedAcrossEdge(..) must be
    // updated to synch.
    conditions = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(2, 2, cg->trMemory());
    t1Register = cg->allocateRegister();
    TR::addDependency(conditions, t1Register, TR::RealRegister::NoReg, TR_GPR,
                      cg);
  } else {
    conditions = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(1, 1, cg->trMemory());
  ...
  TR::addDependency(conditions, selectorReg, TR::RealRegister::NoReg, TR_GPR,
                    cg);
  ...
  if (numBranchTableEntries < 5) {
    for (i = 0; i < numBranchTableEntries; i++) {
      ...
      generateConditionalBranchInstruction(
          ...
          node->getChild(2 + i)->getBranchDestination()->getNode()->getLabel());
    }
    generateConditionalBranchInstruction(
        cg, node, ARMConditionCodeAL,
        secondChild->getBranchDestination()->getNode()->getLabel(), conditions);
  } else {
    TR::RealRegister *machineIP =
    ...
    if (!constantIsImmed8r(numBranchTableEntries, &base, &rotate)) {
      ...
      generateSrc1ImmInstruction(cg, ARMOp_cmp, node, selectorReg, base,
                                 rotate);
    }
    generateConditionalBranchInstruction(
        cg, node, ARMConditionCodeCS,
    ...
    generateTrg1Src1ImmInstruction(cg, ARMOp_sub, node, t1Register, selectorReg,
                                   1, 0);
    generateTrg1Src2Instruction(cg, ARMOp_add, node, machineIP, machineIP,
                                new (cg->trHeapMemory()) TR_ARMOperand2(
                                    ARMOp2RegLSLImmed, t1Register, 2));
...
static TR::Register *evaluateNULLCHKWithPossibleResolve(TR::Node *node,
                                                        bool needsResolve,
                                                        TR::CodeGenerator *cg) {
  // NOTE:
...
#if (NULLVALUE == 0)
    generateSrc2Instruction(cg, ARMOp_tst, node, targetRegister,
                            targetRegister);
#else
#error("NULL is not 0, must fix");
    ...
    TR::Instruction *instr1 = generateImmSymInstruction(
        cg, ARMOp_bl, node, (uintptr_t)NULLCHKException->getMethodAddress(),
        NULL, NULLCHKException, NULL, NULL, ARMConditionCodeEQ);
  ...
  if (!needCode) {
    ...
    if (faultingInstruction) faultingInstruction->setNeedsGCMap();
  }
...
TR::Register *OMR::ARM::TreeEvaluator::NULLCHKEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return evaluateNULLCHKWithPossibleResolve(node, false, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  ...
  TR_ARMOutOfLineCodeSection *outlinedHelperCall =
      new (cg->trHeapMemory()) TR_ARMOutOfLineCodeSection(
...
TR::Register *OMR::ARM::TreeEvaluator::resolveAndNULLCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return evaluateNULLCHKWithPossibleResolve(node, true, cg);
}

TR::Register *OMR::ARM::TreeEvaluator::DIVCHKEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  ...
  if (!secondChild->getOpCode().isLoadConst() ||
    ...
    if (dtype.isInt64()) {
      TR::Register *trgReg = cg->allocateRegister();
      generateTrg1Src2Instruction(cg, ARMOp_orr, node, trgReg,
                                  srcReg->getHighOrder(),
                                  srcReg->getLowOrder());
      srcReg = trgReg;
    }
    ...
    TR::Instruction *instr1 = generateImmSymInstruction(
        cg, ARMOp_bl, node, (uintptr_t)arithmeticException->getMethodAddress(),
        NULL, arithmeticException, NULL, NULL, ARMConditionCodeEQ);
    instr1->ARMNeedsGCMap(0xFFFFFFFF);
    cg->machine()->setLinkRegisterKilled(true);
  }
...
TR::Register *OMR::ARM::TreeEvaluator::BNDCHKEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
TR::Register *OMR::ARM::TreeEvaluator::ArrayCopyBNDCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (/* !isSmall() && */ firstChild->getOpCode().isLoadConst()) {
    if (secondChild->getOpCode().isLoadConst()) {
      if (firstChild->getInt() < secondChild->getInt()) {
        ...
        instr = generateImmSymInstruction(
            cg, ARMOp_bl, node, (uintptr_t)BNDCHKException->getMethodAddress(),
            NULL, BNDCHKException, NULL, NULL, ARMConditionCodeAL);
        cg->machine()->setLinkRegisterKilled(true);
      } else {
      ...
      node->swapChildren();
      instr =
          compareIntsForOrder(ARMConditionCodeGT, node, cg, BNDCHKException);
      node->swapChildren();
    }
  } else {
...
static void VMoutlinedHelperArrayStoreCHKEvaluator(TR::Node *node,
                                                   TR::Register *srcReg,
                                                   TR::Register *dstReg,
                                                   bool srcIsNonNull,
                                                   TR::CodeGenerator *cg) {
  ...
  TR::Instruction *gcPoint = generateImmSymInstruction(
      cg, ARMOp_bl, node, (uintptr_t)arrayStoreChkHelper->getMethodAddress(),
      deps, arrayStoreChkHelper);
  gcPoint->ARMNeedsGCMap(0xFFFFFFFF);

  cg->machine()->setLinkRegisterKilled(true);
}

TR::Register *OMR::ARM::TreeEvaluator::ArrayStoreCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (!srcNode->isNull()) {
    VMoutlinedHelperArrayStoreCHKEvaluator(node, srcReg, dstReg,
                                           srcNode->isNonNull(), cg);
  }

  TR::MemoryReference *storeMR = new (cg->trHeapMemory()) TR::MemoryReference(
      node->getFirstChild(), TR::Compiler->om.sizeofReferenceAddress(), cg);
...
TR::Register *OMR::ARM::TreeEvaluator::ArrayCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
static TR::Register *generateMaxMin(TR::Node *node, TR::CodeGenerator *cg,
  ...
  if (cg->canClobberNodesRegister(child)) {
    ...
    if (node->getOpCodeValue() == TR::iumax ||
        node->getOpCodeValue() == TR::lumax ||
        node->getOpCodeValue() == TR::iumin ||
        node->getOpCodeValue() == TR::lumin) {
      isUnsigned = true;
    }
    if (two_reg) {
      generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg->getLowOrder(),
                                  reg->getLowOrder());
      generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg->getHighOrder(),
                                  reg->getHighOrder());
    } else {
  ...
  TR_ARMConditionCode cond =
      max ? (isUnsigned ? ARMConditionCodeHI : ARMConditionCodeGT)
  ...
  for (; i < n; i++) {
    ...
    if (two_reg) {
      ...
      cursor = generateSrc2Instruction(
          cg, ARMOp_cmp, node, trgReg->getHighOrder(), reg->getHighOrder());
      cursor = generateConditionalBranchInstruction(cg, node, cond, doneLabel,
                                                    deps, cursor);

      cursor = generateTrg1Src1Instruction(cg, ARMOp_mov, node,
                                           trgReg->getLowOrder(),
                                           reg->getLowOrder(), cursor);
      cursor->setConditionCode(ARMConditionCodeNE);
      cursor = generateTrg1Src1Instruction(cg, ARMOp_mov, node,
                                           trgReg->getHighOrder(),
                                           reg->getHighOrder(), cursor);
      cursor->setConditionCode(ARMConditionCodeNE);
      cursor = generateConditionalBranchInstruction(
          cg, node, ARMConditionCodeNE, doneLabel, deps, cursor);
      // Second compare
      cursor =
          generateSrc2Instruction(cg, ARMOp_cmp, node, trgReg->getLowOrder(),
                                  reg->getLowOrder(), cursor);
      cursor = generateTrg1Src1Instruction(cg, ARMOp_mov, node,
                                           trgReg->getLowOrder(),
                                           reg->getLowOrder(), cursor);
      ...
      cursor->setConditionCode(max ? ARMConditionCodeLS : ARMConditionCodeHI);
      // doneLabel
      cursor = generateLabelInstruction(cg, ARMOp_label, node, doneLabel, deps,
                                        cursor);
    } else {
      ...
      cursor =
          generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg, reg, cursor);
...
TR::Register *OMR::ARM::TreeEvaluator::maxEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  return generateMaxMin(node, cg, true);
}

TR::Register *OMR::ARM::TreeEvaluator::minEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  return generateMaxMin(node, cg, false);
}

TR::Register *OMR::ARM::TreeEvaluator::igotoEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  ...
  TR::Instruction *instr =
      generateTrg1Src1Instruction(cg, ARMOp_mov, node, gr15, addrReg);
