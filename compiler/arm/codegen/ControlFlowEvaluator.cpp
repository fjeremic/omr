...
static void
lookupScheme1(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced);
static void
lookupScheme2(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced);
static void
lookupScheme3(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced);
static void
lookupScheme4(TR::CodeGenerator* cg, TR::Node* node);
static bool
isGlDepsUnBalanced(TR::CodeGenerator* cg, TR::Node* node);
static void
switchDispatch(TR::CodeGenerator* cg, TR::Node* node);
static bool
...
TR::Register*
OMR::ARM::TreeEvaluator::ireturnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::ARM::TreeEvaluator::lreturnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* returnRegister = cg->evaluate(node->getFirstChild());
  ...
  TR::addDependency(
    deps, lowReg, cg->getProperties().getLongLowReturnRegister(), TR_GPR, cg);
  TR::addDependency(
    deps, highReg, cg->getProperties().getLongHighReturnRegister(), TR_GPR, cg);
...
compareIntsForOrder(TR_ARMConditionCode branchType,
                    TR::Node* node,
                    TR::CodeGenerator* cg,
                    TR::SymbolReference* sr = NULL)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (constantIsImmed8r(negated ? -cmpValue : cmpValue, &base, &rotate)) {
      generateSrc1ImmInstruction(
        cg, negated ? ARMOp_cmn : ARMOp_cmp, node, src1Reg, base, rotate);
      foundConst = true;
    }
  }
  if (!foundConst)
    generateSrc2Instruction(
      cg, ARMOp_cmp, node, src1Reg, cg->evaluate(secondChild));
  ...
  if (!sr) {
    result = generateConditionalBranchInstruction(
      cg, node, branchType, branchTarget, deps);
  } else {
    result = generateImmSymInstruction(cg,
                                       ARMOp_bl,
                                       node,
                                       (uintptr_t)sr->getMethodAddress(),
                                       deps,
                                       sr,
                                       NULL,
                                       NULL,
...
TR::Instruction*
OMR::ARM::TreeEvaluator::compareIntsForEquality(TR_ARMConditionCode branchType,
                                                TR::Node* node,
                                                TR::CodeGenerator* cg,
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  cursor = generateSrc2Instruction(
    cg, ARMOp_cmp, node, src1Reg->getLowOrder(), src2Reg->getLowOrder());
  ...
  cursor = generateConditionalBranchInstruction(
    cg, node, condition, destinationLabel, deps);

  return NULL;
}

static TR::Register*
compareLongAndSetOrderedBoolean(TR_ARMConditionCode branchOp,
                                TR::Node* node,
                                TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
...
static TR::Register*
compareLongsForOrder(TR_ARMConditionCode branchOp,
                     TR::Node* node,
                     TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (cg->comp()->getOption(TR_DisableOOL) || disableOOLForLongCompares) {
    ...
    TR::Instruction* cursor = generateSrc2Instruction(
      cg, ARMOp_cmp, node, src1Reg->getHighOrder(), src2Reg->getHighOrder());
    cursor = generateConditionalBranchInstruction(cg,
                                                  node,
                                                  isGT ? ARMConditionCodeLE
                                                       : ARMConditionCodeGE,
                                                  startOOLLabel,
                                                  deps,
                                                  cursor);
    ...
    cursor = generateTrg1ImmInstruction(
      cg, ARMOp_mov, node, tempReg, 1, 0, cursor); // Setting tempReg to 1
    ...
    TR::RegisterDependencyConditions* newDeps = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(2, 2, cg->trMemory());
    TR::addDependency(
      newDeps, src1Reg->getLowOrder(), TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(
      newDeps, src2Reg->getLowOrder(), TR::RealRegister::NoReg, TR_GPR, cg);
    ...
    TR_ASSERT(
      !cursorOOL->getLiveLocals() && !cursorOOL->getLiveMonitors(),
    ...
    cursorOOL = generateTrg1ImmInstruction(
      cg, ARMOp_mov, node, tempReg, 0, 0, cursorOOL); // Setting tempReg to 0
    cursorOOL = generateConditionalBranchInstruction(cg,
                                                     node,
                                                     isGT ? ARMConditionCodeLT
                                                          : ARMConditionCodeGT,
                                                     returnLabel,
                                                     cursorOOL);
    cursorOOL = generateSrc2Instruction(cg,
                                        ARMOp_cmp,
                                        node,
                                        src1Reg->getLowOrder(),
                                        src2Reg->getLowOrder(),
                                        cursorOOL);
    cursorOOL = generateTrg1ImmInstruction(
      cg, ARMOp_mov, node, tempReg, 1, 0, cursorOOL); // Setting tempReg to 1
    cursorOOL->setConditionCode(isGT ? ARMConditionCodeHI : ARMConditionCodeLS);
    cursorOOL = generateLabelInstruction(cg,
                                         ARMOp_label,
                                         node,
                                         returnLabel,
                                         oolDeps,
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  generateConditionalBranchInstruction(
    cg, node, ARMConditionCodeLT, destinationLabel, deps);
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src2Instruction(cg,
                              ...
                              src2Reg->getHighOrder());
  generateConditionalBranchInstruction(
    cg, node, ARMConditionCodeGE, destinationLabel, deps);
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src2Instruction(cg,
                              ...
                              src2Reg->getHighOrder());
  generateConditionalBranchInstruction(
    cg, node, ARMConditionCodeCC, destinationLabel, deps);
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src2Instruction(cg,
                              ...
                              src2Reg->getHighOrder());
  generateConditionalBranchInstruction(
    cg, node, ARMConditionCodeCS, destinationLabel, deps);
...
static TR::Register*
icmpHelper(TR::Node* node, TR_ARMConditionCode cond, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = cg->allocateRegister();
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
...
TR::Register*
OMR::ARM::TreeEvaluator::icmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::ARM::TreeEvaluator::icmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeNE, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::icmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeLT, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::icmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeLE, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::icmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeGE, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::icmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeGT, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::iucmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeCC, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::iucmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeLS, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::iucmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeCS, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::iucmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return icmpHelper(node, ARMConditionCodeHI, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeEQ, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeNE, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeLT, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeGE, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeGT, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeLE, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lucmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeCC, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lucmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeCS, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lucmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeHI, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lucmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return compareLongAndSetOrderedBoolean(ARMConditionCodeLS, node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = cg->allocateRegister();
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
...
TR::Register*
OMR::ARM::TreeEvaluator::lookupEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
static void
mapGlRegDeps(TR::Node* node, TR_BitVector* vector)
{
...
switchDispatch(TR::CodeGenerator* cg, TR::Node* node)
{
  ...
  if (total <= 15) {
    uint32_t base, rotate;
    for (i = 2;
         i < total && constantIsImmed8r(
                        node->getChild(i)->getCaseConstant(), &base, &rotate);
         i++)
      ;
    if (i == total) {
...
lookupScheme1(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced)
{
  ...
  for (int i = 2; i < node->getNumChildren(); i++) {
    ...
    if (unbalanced) {
      ...
      generateConditionalBranchInstruction(
        cg, node, ARMConditionCodeEQ, caseTargetLabel, caseDeps);
    } else {
      generateConditionalBranchInstruction(
        cg, node, ARMConditionCodeEQ, caseTargetLabel);
    }
  }
  ...
  for (int i = 2; i < numChildren; i++) {
    ...
    if (i < numChildren - 1) {
      ...
      generateTrg1Src1ImmInstruction(
        cg, ARMOp_add, node, caseConstReg, caseConstReg, base, rotate);
    }
  }
...
lookupScheme3(TR::CodeGenerator* cg, TR::Node* node, bool unbalanced)
{
  ...
  int32_t* dataTable =
    (int32_t*)cg->allocateCodeMemory(numberOfEntries * sizeof(int32_t),
...
TR::Register*
OMR::ARM::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  int32_t total = node->getNumChildren();
  ...
  if (numBranchTableEntries >= 5) {
    ...
    TR::addDependency(
      conditions, t1Register, TR::RealRegister::NoReg, TR_GPR, cg);
  } else {
    conditions = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(1, 1, cg->trMemory());
  }
  ...
  TR::addDependency(
    conditions, selectorReg, TR::RealRegister::NoReg, TR_GPR, cg);
  ...
  if (numBranchTableEntries < 5) {
    ...
    if (!constantIsImmed8r(numBranchTableEntries, &base, &rotate)) {
      ...
      generateSrc1ImmInstruction(
        cg, ARMOp_cmp, node, selectorReg, base, rotate);
    }
    generateConditionalBranchInstruction(
      cg,
      node,
    ...
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_sub, node, t1Register, selectorReg, 1, 0);
    generateTrg1Src2Instruction(cg,
                                ARMOp_add,
                                node,
                                machineIP,
...
static TR::Register*
evaluateNULLCHKWithPossibleResolve(TR::Node* node,
                                   bool needsResolve,
                                   TR::CodeGenerator* cg)
{
  // NOTE:
...
#if (NULLVALUE == 0)
    generateSrc2Instruction(
      cg, ARMOp_tst, node, targetRegister, targetRegister);
#else
#error("NULL is not 0, must fix");
    ...
    TR::Instruction* instr1 =
      generateImmSymInstruction(cg,
                                ARMOp_bl,
                                node,
                                (uintptr_t)NULLCHKException->getMethodAddress(),
                                NULL,
                                NULLCHKException,
                                NULL,
                                NULL,
...
TR::Register*
OMR::ARM::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
/* We need to change compareIntsForOrder and compareIntsForEquality for the optimization code below to work */
...
#endif
  {
    ...
    generateConditionalBranchInstruction(
      cg, node, ARMConditionCodeEQ, slowPathLabel);
...
TR::Register*
OMR::ARM::TreeEvaluator::DIVCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!secondChild->getOpCode().isLoadConst() ||
    ...
    TR::Instruction* instr1 = generateImmSymInstruction(
      ...
      ARMOp_bl,
      node,
      (uintptr_t)arithmeticException->getMethodAddress(),
      NULL,
      arithmeticException,
      NULL,
      NULL,
...
TR::Register*
OMR::ARM::TreeEvaluator::BNDCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Instruction* instr1;
    ...
    if (secondChild->getOpCode().isLoadConst()) {
      if (firstChild->getInt() < secondChild->getInt()) {
        ...
        instr = generateImmSymInstruction(
          ...
          ARMOp_bl,
          node,
          (uintptr_t)BNDCHKException->getMethodAddress(),
          NULL,
          BNDCHKException,
          NULL,
          NULL,
...
static void
VMoutlinedHelperArrayStoreCHKEvaluator(TR::Node* node,
                                       TR::Register* srcReg,
                                       TR::Register* dstReg,
                                       bool srcIsNonNull,
                                       TR::CodeGenerator* cg)
                                                ...
                                                TR::CodeGenerator* cg)
{
  ...
  if (!srcNode->isNull()) {
    VMoutlinedHelperArrayStoreCHKEvaluator(
      node, srcReg, dstReg, srcNode->isNonNull(), cg);
  }
...
generateMaxMin(TR::Node* node, TR::CodeGenerator* cg, bool max)
{
  ...
  if (cg->canClobberNodesRegister(child)) {
    ...
    if (two_reg) {
      generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, trgReg->getLowOrder(), reg->getLowOrder());
      generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, trgReg->getHighOrder(), reg->getHighOrder());
    } else {
  ...
  for (; i < n; i++) {
    ...
    if (two_reg) {
      ...
      cursor = generateConditionalBranchInstruction(
        cg, node, cond, doneLabel, deps, cursor);

      cursor = generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, trgReg->getLowOrder(), reg->getLowOrder(), cursor);
      ...
      // Second compare
      cursor = generateSrc2Instruction(
        cg, ARMOp_cmp, node, trgReg->getLowOrder(), reg->getLowOrder(), cursor);
      cursor = generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, trgReg->getLowOrder(), reg->getLowOrder(), cursor);
      ...
      cursor->setConditionCode(max ? ARMConditionCodeLS : ARMConditionCodeHI);
      // doneLabel
      cursor = generateLabelInstruction(
        cg, ARMOp_label, node, doneLabel, deps, cursor);
    } else {
...
TR::Register*
OMR::ARM::TreeEvaluator::maxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return generateMaxMin(node, cg, true);
}

TR::Register*
OMR::ARM::TreeEvaluator::minEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return generateMaxMin(node, cg, false);
}

TR::Register*
OMR::ARM::TreeEvaluator::igotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
