    ...
    TR::SymbolReference* sr = NULL)
{
    ...
    if (!cg->comp()->getOption(TR_OptimizeForSpace) && firstChild->getOpCodeValue() == TR:: instanceof && firstChild->getRegister() == NULL && node->getReferenceCount() <= 1 && (value == 0 || value == 1)) {
...
TR::Register* OMR::ARM::TreeEvaluator::DIVCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!secondChild->getOpCode().isLoadConst() || (dtype.isInt32() && secondChild->getInt() == 0) || (dtype.isInt64() && secondChild->getLongInt() == 0)) {
...
static TR::Register* generateMaxMin(TR::Node* node, TR::CodeGenerator* cg, bool max)
{
    ...
    if (cg->canClobberNodesRegister(child)) {
        ...
        if (node->getOpCodeValue() == TR::iumax || node->getOpCodeValue() == TR::lumax || node->getOpCodeValue() == TR::iumin || node->getOpCodeValue() == TR::lumin) {
            isUnsigned = true;
        }
        if (two_reg) {
