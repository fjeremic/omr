...
TR::Instruction* OMR::ARM::Linkage::saveArguments(TR::Instruction* cursor)
{
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        bool hasToBeOnStack = paramCursor->isReferencedParameter() || paramCursor->isParmHasToBeOnStack() || ((numIntArgs == 0) && paramCursor->isCollectedReference());
...
#ifdef DEBUG_ARM_LINKAGE
                        printf("pushing %s word of 64-bit arg %d %d %d %d\n", bigEndian ? "low" : "high", numIntegerArgs, memArg, totalSize, argSize);
                        fflush(stdout);
#endif
...
TR::Register* OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node* callNode, bool isSystem)
{
    ...
    TR::RegisterDependencyConditions* dependencies = new (self()->trHeapMemory()) TR::RegisterDependencyConditions(pp.getNumberOfDependencyGPRegisters() + 8 /*pp.getNumFloatArgRegs()*/,
    ...
    if ((callSymbol->isJITInternalNative() || (!callSymRef->isUnresolved() && !callSymbol->isInterpreted() && ((self()->comp()->compileRelocatableCode() && callSymbol->isHelper()) || !self()->comp()->compileRelocatableCode())))) {
