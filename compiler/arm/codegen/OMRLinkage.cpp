...
void OMR::ARM::Linkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                           uint32_t &stackIndex) {}
...
void OMR::ARM::Linkage::setParameterLinkageRegisterIndex(
...
TR::Instruction *OMR::ARM::Linkage::saveArguments(TR::Instruction *cursor) {
  ...
  TR::RealRegister *stackPtr = machine->getRealRegister(
      self()->getProperties().getStackPointerRegister());
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    if (numIntArgs >= properties.getNumIntArgRegs()) break;

    //      if (lri >=  0)
    //         {
    // We cannot have linkageRegisterIndex as it breaks GRA. Using numIntArgs ==
    // 0 to distinguish "this" pointer.
    bool hasToBeOnStack =
    ...
    switch (paramCursor->getDataType()) {
      ...
      case TR::Address:
        if (hasToBeOnStack) {
          argRegister = machine->getRealRegister(
              properties.getIntegerArgumentRegister(numIntArgs));
          cursor = generateMemSrc1Instruction(
              codeGen, ARMOp_str, firstNode,
              new (self()->trHeapMemory())
                  TR::MemoryReference(stackPtr, offset, codeGen),
              argRegister, cursor);
        }
        numIntArgs++;
        break;
      ...
      case TR::Int64:
        if (hasToBeOnStack) {
          ...
          if (numIntArgs < properties.getNumIntArgRegs() - 1) {
            argRegister = machine->getRealRegister(
                properties.getIntegerArgumentRegister(numIntArgs + 1));
            cursor = generateMemSrc1Instruction(
                codeGen, ARMOp_str, firstNode,
                new (self()->trHeapMemory())
                    TR::MemoryReference(stackPtr, offset + 4, codeGen),
                argRegister, cursor);
          }
        }
        numIntArgs += 2;
...
TR::Instruction *OMR::ARM::Linkage::loadUpArguments(TR::Instruction *cursor) {
  ...
  while (paramCursor != NULL && numIntArgs < properties.getNumIntArgRegs()) {
    TR::RealRegister *argRegister;
    int32_t offset = paramCursor->getParameterOffset();
    bool hasToLoadFromStack = paramCursor->isReferencedParameter() ||
                              paramCursor->isParmHasToBeOnStack();
      ...
      case TR::Int32:
        if (hasToLoadFromStack && numIntArgs < properties.getNumIntArgRegs()) {
          ...
          cursor = generateTrg1MemInstruction(
              codeGen, ARMOp_ldr, firstNode, argRegister,
              new (self()->trHeapMemory())
                  TR::MemoryReference(stackPtr, offset, codeGen),
              cursor);
        }
...
TR::Instruction *OMR::ARM::Linkage::flushArguments(TR::Instruction *cursor) {
  ...
  while (paramCursor != NULL && numIntArgs < properties.getNumIntArgRegs()) {
    TR::RealRegister *argRegister;
    int32_t offset = paramCursor->getParameterOffset();
    bool hasToSaveToStack = paramCursor->isReferencedParameter() ||
                            paramCursor->isParmHasToBeOnStack();
...
TR::Register *OMR::ARM::Linkage::pushJNIReferenceArg(TR::Node *child) {
  ...
  if (child->getOpCodeValue() == TR::loadaddr) {
    ...
    if (sym) {
      if (sym->isAddressOfClassObject()) {
        ...
        TR::MemoryReference *tempMR = new (self()->trHeapMemory())
            TR::MemoryReference(addrReg, 0, self()->cg());

        // statics are non-collectable
        pushReg = self()->cg()->allocateRegister();

        // pass NULL if ref is NULL, else pass &ref
        generateTrg1MemInstruction(self()->cg(), ARMOp_ldr, child, pushReg,
                                   tempMR);
        generateSrc1ImmInstruction(self()->cg(), ARMOp_cmp, child, pushReg, 0,
                                   0);
        TR::Instruction *instr = generateTrg1Src1Instruction(
            self()->cg(), ARMOp_mov, child, pushReg, addrReg);
    ...
    } else  // must be loadaddr of parm or local
    {
      if (child->pointsToNonNull()) {
        ...
        pushReg = self()->cg()->allocateRegister();
        generateTrg1ImmInstruction(self()->cg(), ARMOp_mov, child, pushReg, 0,
                                   0);
        self()->cg()->decReferenceCount(child);
        clobberable = true;
      } else {
        ...
        TR::MemoryReference *tempMR = new (self()->trHeapMemory())
            TR::MemoryReference(addrReg, 0, self()->cg());
        generateTrg1MemInstruction(self()->cg(), ARMOp_ldr, child, objReg,
                                   tempMR);

        if (child->getReferenceCount() > 1) {
          // Since this points at a parm or local location, it is
          // non-collectable.
          TR::Register *tempReg = self()->cg()->allocateRegister();
          generateTrg1Src1Instruction(self()->cg(), ARMOp_mov, child, tempReg,
                                      addrReg);
          pushReg = tempReg;
        } else {
          pushReg = addrReg;
        ...
        generateSrc1ImmInstruction(self()->cg(), ARMOp_cmp, child, objReg, 0,
                                   0);
        generateConditionalBranchInstruction(self()->cg(), child,
                                             ARMConditionCodeNE, doneLabel);
        generateTrg1ImmInstruction(self()->cg(), ARMOp_mov, child, pushReg, 0,
                                   0);

        TR::RegisterDependencyConditions *deps = new (self()->trHeapMemory())
            TR::RegisterDependencyConditions(2, 2, self()->trMemory());
        TR::addDependency(deps, pushReg, TR::RealRegister::NoReg, TR_GPR,
                          self()->cg());
        TR::addDependency(deps, objReg, TR::RealRegister::NoReg, TR_GPR,
                          self()->cg());

        generateLabelInstruction(self()->cg(), ARMOp_label, child, doneLabel,
                                 deps);
        self()->cg()->decReferenceCount(child);
      }
    }
  ...
  if (!clobberable && child->getReferenceCount() > 0) {
    TR::Register *tempReg =
        pushReg->containsCollectedReference()
            ? self()->cg()->allocateCollectedReferenceRegister()
...
TR::Register *OMR::ARM::Linkage::pushIntegerWordArg(TR::Node *child) {
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst() &&
      (child->getDataType() !=
       TR::Float)) /* XXX: We need to fix buildARMLinkageArgs(). */
  {
...
TR::Register *OMR::ARM::Linkage::pushAddressArg(TR::Node *child) {
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    ...
    pushRegister = codeGen->allocateRegister();
    if (isClass && self()->cg()->wantToPatchClassPointer(
                       (TR_OpaqueClassBlock *)child->getAddress(), child)) {
      loadAddressConstantInSnippet(self()->cg(), child, child->getAddress(),
                                   pushRegister);
    } else {
      if (child->isMethodPointerConstant())
        loadAddressConstant(self()->cg(), child, child->getAddress(),
                            pushRegister, NULL, false, TR_RamMethodSequence);
      else
        loadAddressConstant(self()->cg(), child, child->getAddress(),
                            pushRegister);
    }
...
TR::Register *OMR::ARM::Linkage::pushLongArg(TR::Node *child) {
  ...
  if (child->getRegister() == NULL &&
      child->getOpCode()
          .isLoadConst() /* && (child->getDataType() != TR::Double) */) {
...
int32_t OMR::ARM::Linkage::buildARMLinkageArgs(
    TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::Register *&vftReg, TR_LinkageConventions conventions,
    bool isVirtualOrJNI) {
  const TR::ARMLinkageProperties &properties = self()->getProperties();
  ...
  if (isVirtual) {
    ...
    TR::addDependency(dependencies, vftReg, TR::RealRegister::NoReg, TR_GPR,
                      self()->cg());  // dep 0

#ifndef LOCK_R14
    TR::addDependency(dependencies, NULL, TR::RealRegister::gr14, TR_GPR,
                      cg());  // dep 2
#endif
  }
  ...
  if (specialArgReg != TR::RealRegister::NoReg) {
    if (comp->getOption(TR_TraceCG)) {
      traceMsg(comp, "Special arg %s in %s\n",
               ...
               comp->getDebug()->getName(
                   self()->machine()->getRealRegister(specialArgReg)));
    }
  ...
  for (i = from; (isHelper && i > to) || (!isHelper && i < to); i += step) {
    switch (callNode->getChild(i)->getDataType()) {
      ...
      case TR::Float:
#endif
        if (numIntegerArgs >= numIntArgRegs) numMemArgs++;
        numIntegerArgs++;
        totalSize += 4;
        break;
      case TR::Int64:
...
#ifndef __VFP_FP__
      case TR::Float:
        if (numFloatArgs >= numFloatArgRegs) numMemArgs++;
        numFloatArgs++;
        totalSize += 4;
        break;
      case TR::Double:
...
#ifdef DEBUG_ARM_LINKAGE
  // const char *sig =
  // callNode->getSymbol()->getResolvedMethodSymbol()->signature();
  const char *sig = "CALL";
  ...
  // From here, down, all stack memory allocations will expire / die when the
  // function returns.
  TR::StackMemoryRegion stackMemoryRegion(*self()->trMemory());
  if (numMemArgs > 0) {
    pushToMemory =
        new (self()->trStackMemory()) TR::ARMMemoryArgument[numMemArgs];
  }
  ...
  for (i = from; (isHelper && i > to) || (!isHelper && i < to); i += step) {
    ...
    switch (childType) {
      ...
      case TR::Float:
#endif
        if (childType == TR::Address) reg = self()->pushAddressArg(child);
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
        else if (childType == TR::Float) {
          TR::Register *tempReg = self()->cg()->allocateRegister();
          reg = self()->pushFloatArg(child);
          /* Passing a float argument in an integer register.*/
          generateTrg1Src1Instruction(self()->cg(), ARMOp_fmrs, callNode,
                                      tempReg, reg);
          reg = tempReg;
        }
#endif
        ...
        if (isSpecialArg) {
          if (specialArgReg == properties.getIntegerReturnRegister(0)) {
            ...
            dependencies->addPostCondition(
                resultReg, properties.getIntegerReturnRegister(0));
          } else {
            TR::addDependency(dependencies, reg, specialArgReg, TR_GPR,
                              self()->cg());
          }
        } else {
          if (numIntegerArgs < numIntArgRegs) {
            if (!self()->cg()->canClobberNodesRegister(child, 0) &&
              ...
              generateTrg1Src1Instruction(self()->cg(), ARMOp_mov, child,
                                          tempReg, reg);
              reg = tempReg;
            }
            if (numIntegerArgs == 0 /* &&
            ...
            } else if (numIntegerArgs == 1 &&
                       (resType.isInt64() || resType.isDouble())) {
...
#ifdef DEBUG_ARM_LINKAGE
            printf("pushing 32-bit arg %d %d %d %d\n", numIntegerArgs, memArg,
                   totalSize, argSize);
            fflush(stdout);
#endif
            tempMR = self()->getOutgoingArgumentMemRef(
                totalSize, argSize, reg, ARMOp_str, pushToMemory[memArg++]);
          }
          numIntegerArgs++;
          argSize += 4;
        }
      ...
      case TR::Double:
        if (childType == TR::Double) {
          ...
          TR::RegisterPair *tempReg =
              self()->cg()->allocateRegisterPair(lowReg, highReg);
          generateTrg2Src1Instruction(self()->cg(), ARMOp_fmrrd, callNode,
                                      lowReg, highReg, reg);
          reg = tempReg;
        } else
#endif
        ...
        if (isSpecialArg) {
          ...
          TR::Register *specialArgRegister =
              reg->getRegisterPair() ? reg->getLowOrder() : reg;
          if (specialArgReg == properties.getIntegerReturnRegister(0)) {
            TR::Register *resultReg;
            if (resType.isAddress())
              resultReg = self()->cg()->allocateCollectedReferenceRegister();
            ...
            TR::addDependency(dependencies, specialArgRegister, specialArgReg,
                              TR_GPR, self()->cg());
          }
        } else {
          if (numIntegerArgs < numIntArgRegs) {
            if (!self()->cg()->canClobberNodesRegister(child, 0) &&
              ...
              if (bigEndian) {
                generateTrg1Src1Instruction(
                    self()->cg(), ARMOp_mov, child, tempReg,
                    reg->getRegisterPair()->getHighOrder());
                reg = self()->cg()->allocateRegisterPair(
                    reg->getRegisterPair()->getLowOrder(), tempReg);
              } else {
                generateTrg1Src1Instruction(
                    self()->cg(), ARMOp_mov, child, tempReg,
                    reg->getRegisterPair()->getLowOrder());
                reg = self()->cg()->allocateRegisterPair(
                    tempReg, reg->getRegisterPair()->getHighOrder());
              }
            }

            dependencies->addPreCondition(
                bigEndian ? reg->getRegisterPair()->getHighOrder()
                          : reg->getRegisterPair()->getLowOrder(),
            ...
            if (numIntegerArgs + 1 < numIntArgRegs) {
              ...
              printf("pushing %s word of 64-bit arg %d %d %d %d\n",
                     bigEndian ? "low" : "high", numIntegerArgs, memArg,
                     totalSize, argSize);
              fflush(stdout);
#endif
              tempMR = self()->getOutgoingArgumentMemRef(
  ...
  getProperties().getComputedCallTargetRegister());
        dependencites->getPostConditions()->setDependencyInfo(0, vftReg,
  getProperties().getComputedCallTargetRegister());
        }
     }
  */
  for (i = 0; i < properties.getNumIntArgRegs(); i++) {
    TR::RealRegister::RegNum realReg =
        (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::gr0 + i);
    ...
    if (!dependencies->searchPreConditionRegister(realReg)) {
      if (realReg == properties.getIntegerArgumentRegister(0) &&
          resType.isAddress()) {
        dependencies->addPreCondition(self()->cg()->allocateRegister(),
                                      TR::RealRegister::gr0);
        dependencies->addPostCondition(
            self()->cg()->allocateCollectedReferenceRegister(),
            TR::RealRegister::gr0);
      } else if ((realReg == TR::RealRegister::gr1) &&
                 (resType.isInt64() || resType.isDouble())) {
        dependencies->addPreCondition(self()->cg()->allocateRegister(),
                                      TR::RealRegister::gr1);
  ...
  for (i = 0; i < 8 /*properties.getNumFloatArgRegs()*/; i++) {
    TR::addDependency(
        dependencies, NULL,
        (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 + i), TR_FPR,
        self()->cg());
  }
#endif
  ...
  switch (conventions) {
    ...
    case TR_Helper:
      TR::addDependency(dependencies, NULL, TR::RealRegister::gr4, TR_GPR,
                        self()->cg());
      TR::addDependency(dependencies, NULL, TR::RealRegister::gr5, TR_GPR,
                        self()->cg());
      // deliberate fall-through
    case TR_System:
#ifndef LOCK_R14
  ...
  if (numMemArgs > 0) {
    ...
    {
#ifdef DEBUG_ARM_LINKAGE
      printf("pushing mem arg %d of %d (in reg %x) to [sp + %d]... ", i,
             numMemArgs, pushToMemory[i].argRegister,
             pushToMemory[i].argMemory->getOffset());
      fflush(stdout);
#endif
      generateMemSrc1Instruction(self()->cg(), pushToMemory[i].opCode, callNode,
...
TR::Register *OMR::ARM::Linkage::buildARMLinkageDirectDispatch(
    TR::Node *callNode, bool isSystem) {
  TR::CodeGenerator *codeGen = self()->cg();

  const TR::ARMLinkageProperties &pp = self()->getProperties();
  TR::RegisterDependencyConditions *dependencies =
      new (self()->trHeapMemory()) TR::RegisterDependencyConditions(
          pp.getNumberOfDependencyGPRegisters() + 8 /*pp.getNumFloatArgRegs()*/,
          pp.getNumberOfDependencyGPRegisters() + 8 /*pp.getNumFloatArgRegs()*/,
          self()->trMemory());
  ...
  if (callSymRef->getReferenceNumber() >= TR_ARMnumRuntimeHelpers)
    self()->fe()->reserveTrampolineIfNecessary(self()->comp(), callSymRef,
                                               false);
  ...
  if ((callSymbol->isJITInternalNative() ||
    ...
    if (callSymRef->isUnresolved() ||
        self()->comp()->compileRelocatableCode()) {
      snippet = new (self()->trHeapMemory())
          TR::ARMUnresolvedCallSnippet(codeGen, callNode, label, argSize);
    } else {
      snippet = new (self()->trHeapMemory())
          TR::ARMCallSnippet(codeGen, callNode, label, argSize);
    }

    codeGen->addSnippet(snippet);
    gcPoint = generateImmSymInstruction(
        codeGen, ARMOp_bl, callNode, 0, dependencies,
        new (self()->trHeapMemory())
            TR::SymbolReference(self()->comp()->getSymRefTab(), label),
        snippet);
#else
  ...
  switch (callNode->getOpCodeValue()) {
    ...
    case TR::fcall:
      ...
      if (resType.isFloatingPoint()) {
        ...
        TR::Instruction *cursor = generateTrg1Src1Instruction(
            codeGen, ARMOp_fmsr, callNode, tempReg, returnRegister);
...
#endif
    {
      ...
      if (resType.isDouble()) {
        ...
        TR::Instruction *cursor = generateTrg1Src2Instruction(
            codeGen, ARMOp_fmdrr, callNode, tempReg, lowReg, highReg);
        returnRegister = tempReg;
      }
      break;
    }
