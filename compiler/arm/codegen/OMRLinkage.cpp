  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
    ...
    case TR::Address:
      if (hasToBeOnStack) {
        ...
        cursor = generateMemSrc1Instruction(codeGen, ARMOp_str, firstNode,
          new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset, codeGen), argRegister, cursor);
      }
      numIntArgs++;
      break;
    case TR::Double:
    ...
    case TR::Int64:
      if (hasToBeOnStack) {
        ...
        if (numIntArgs < properties.getNumIntArgRegs() - 1) {
          ...
          cursor = generateMemSrc1Instruction(codeGen, ARMOp_str, firstNode,
            new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset + 4, codeGen), argRegister, cursor);
        }
      }
      numIntArgs += 2;
      break;
  ...
  while (paramCursor != NULL && numIntArgs < properties.getNumIntArgRegs()) {
    ...
    case TR::Int32:
      if (hasToLoadFromStack && numIntArgs < properties.getNumIntArgRegs()) {
        argRegister = machine->getRealRegister(properties.getIntegerArgumentRegister(numIntArgs));
        cursor = generateTrg1MemInstruction(codeGen, ARMOp_ldr, firstNode, argRegister,
          new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset, codeGen), cursor);
      }
      numIntArgs++;
      break;
    case TR::Address:
    ...
    case TR::Int64:
      if (hasToLoadFromStack && numIntArgs < properties.getNumIntArgRegs()) {
        ...
        if (numIntArgs < properties.getNumIntArgRegs() - 1) {
          argRegister = machine->getRealRegister(properties.getIntegerArgumentRegister(numIntArgs + 1));
          cursor = generateTrg1MemInstruction(codeGen, ARMOp_ldr, firstNode, argRegister,
            new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset + 4, codeGen), cursor);
        }
      }
      numIntArgs += 2;
      break;
...
TR::Register* OMR::ARM::Linkage::pushJNIReferenceArg(TR::Node* child)
{
  ...
  if (!clobberable && child->getReferenceCount() > 0) {
    TR::Register* tempReg = pushReg->containsCollectedReference() ? self()->cg()->allocateCollectedReferenceRegister()
      ...
      if (isSpecialArg) {
        ...
        if (numIntegerArgs < numIntArgRegs) {
          ...
          if (numIntegerArgs == 0 /* &&
            ...
            TR::addDependency(
              dependencies, reg, properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR, self()->cg());
          }
        } else {
#ifdef DEBUG_ARM_LINKAGE
      ...
      if (isSpecialArg) {
        ...
        if (numIntegerArgs < numIntArgRegs) {
          if (!self()->cg()->canClobberNodesRegister(child, 0) && (childType != TR::Double)) {
            ...
            if (bigEndian) {
              generateTrg1Src1Instruction(
                self()->cg(), ARMOp_mov, child, tempReg, reg->getRegisterPair()->getHighOrder());
              reg = self()->cg()->allocateRegisterPair(reg->getRegisterPair()->getLowOrder(), tempReg);
            } else {
              generateTrg1Src1Instruction(
                self()->cg(), ARMOp_mov, child, tempReg, reg->getRegisterPair()->getLowOrder());
          ...
          if (numIntegerArgs + 1 < numIntArgRegs) {
            ...
            dependencies->addPostCondition(
              self()->cg()->allocateRegister(), properties.getIntegerArgumentRegister(numIntegerArgs + 1));
          } else {
#ifdef DEBUG_ARM_LINKAGE
            ...
            tempMR = self()->getOutgoingArgumentMemRef(totalSize, argSize,
              bigEndian ? reg->getRegisterPair()->getLowOrder() : reg->getRegisterPair()->getHighOrder(), ARMOp_str,
              pushToMemory[memArg++]);
          }
        } else {
#ifdef DEBUG_ARM_LINKAGE
          ...
          tempMR = self()->getOutgoingArgumentMemRef(totalSize, argSize + 4,
            bigEndian ? reg->getRegisterPair()->getHighOrder() : reg->getRegisterPair()->getLowOrder(), ARMOp_str,
            pushToMemory[memArg++]);
        }
        numIntegerArgs += 2;
        argSize += 8;
...
TR::Register* OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node* callNode, bool isSystem)
{
  ...
  if ((callSymbol->isJITInternalNative()
    ...
    gcPoint = generateImmSymInstruction(codeGen, ARMOp_bl, callNode,
    ...
    gcPoint = generateImmSymInstruction(codeGen, ARMOp_bl, callNode, 0, dependencies,
