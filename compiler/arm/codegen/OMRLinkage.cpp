...
void
OMR::ARM::Linkage::mapStack(TR::ResolvedMethodSymbol* method)
{}

void
OMR::ARM::Linkage::mapSingleAutomatic(TR::AutomaticSymbol* p,
                                      uint32_t& stackIndex)
{}

void
OMR::ARM::Linkage::initARMRealRegisterLinkage()
{}

bool
OMR::ARM::Linkage::hasToBeOnStack(TR::ParameterSymbol* parm)
{
  return false;
}

void
OMR::ARM::Linkage::setParameterLinkageRegisterIndex(
  TR::ResolvedMethodSymbol* method)
{}

TR::Instruction*
OMR::ARM::Linkage::saveArguments(TR::Instruction* cursor)
{
  TR::CodeGenerator* codeGen = self()->cg();
  TR::Machine* machine = self()->machine();
  TR::RealRegister* stackPtr =
    machine->getRealRegister(self()->getProperties().getStackPointerRegister());
...
TR::Instruction*
OMR::ARM::Linkage::loadUpArguments(TR::Instruction* cursor)
{
  TR::CodeGenerator* codeGen = self()->cg();
  TR::Machine* machine = self()->machine();
  TR::RealRegister* stackPtr =
...
TR::Instruction*
OMR::ARM::Linkage::flushArguments(TR::Instruction* cursor)
{
  TR::CodeGenerator* codeGen = self()->cg();
  TR::Machine* machine = self()->machine();
  TR::RealRegister* stackPtr =
...
TR::Register*
OMR::ARM::Linkage::pushJNIReferenceArg(TR::Node* child)
{
    ...
    if (sym) {
      if (sym->isAddressOfClassObject()) {
        ...
        generateTrg1MemInstruction(
          self()->cg(), ARMOp_ldr, child, pushReg, tempMR);
        generateSrc1ImmInstruction(
          self()->cg(), ARMOp_cmp, child, pushReg, 0, 0);
    ...
    } else // must be loadaddr of parm or local
    {
      if (child->pointsToNonNull()) {
        ...
        generateTrg1ImmInstruction(
          self()->cg(), ARMOp_mov, child, pushReg, 0, 0);
        self()->cg()->decReferenceCount(child);
        clobberable = true;
      } else {
        TR::Register* addrReg = self()->cg()->evaluate(child);
        ...
        TR::MemoryReference* tempMR = new (self()->trHeapMemory())
          TR::MemoryReference(addrReg, 0, self()->cg());
        generateTrg1MemInstruction(
          self()->cg(), ARMOp_ldr, child, objReg, tempMR);
        ...
        if (child->getReferenceCount() > 1) {
          ...
          generateTrg1Src1Instruction(
            self()->cg(), ARMOp_mov, child, tempReg, addrReg);
          pushReg = tempReg;
        } else {
          pushReg = addrReg;
        }
        ...
        generateSrc1ImmInstruction(
          self()->cg(), ARMOp_cmp, child, objReg, 0, 0);
        generateConditionalBranchInstruction(
          self()->cg(), child, ARMConditionCodeNE, doneLabel);
        generateTrg1ImmInstruction(
          self()->cg(), ARMOp_mov, child, pushReg, 0, 0);
        ...
        TR::addDependency(
          deps, pushReg, TR::RealRegister::NoReg, TR_GPR, self()->cg());
        TR::addDependency(
          deps, objReg, TR::RealRegister::NoReg, TR_GPR, self()->cg());

        generateLabelInstruction(
          self()->cg(), ARMOp_label, child, doneLabel, deps);
        self()->cg()->decReferenceCount(child);
      }
    }
  } else {
  ...
  if (!clobberable && child->getReferenceCount() > 0) {
    ...
    generateTrg1Src1Instruction(
      self()->cg(), ARMOp_mov, child, tempReg, pushReg);
    pushReg = tempReg;
  }
...
TR::Register*
OMR::ARM::Linkage::pushIntegerWordArg(TR::Node* child)
{
  TR::CodeGenerator* codeGen = self()->cg();
...
TR::Register*
OMR::ARM::Linkage::pushAddressArg(TR::Node* child)
{
  TR::CodeGenerator* codeGen = self()->cg();
  TR::Register* pushRegister = NULL;
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    ...
    if (isClass && self()->cg()->wantToPatchClassPointer(
      ...
      loadAddressConstantInSnippet(
        self()->cg(), child, child->getAddress(), pushRegister);
    } else {
      if (child->isMethodPointerConstant())
        loadAddressConstant(self()->cg(),
                            child,
                            child->getAddress(),
                            pushRegister,
                            NULL,
...
TR::Register*
OMR::ARM::Linkage::pushLongArg(TR::Node* child)
{
  TR::CodeGenerator* codeGen = self()->cg();
  TR::Register* pushRegister = NULL;
...
TR::Register*
OMR::ARM::Linkage::pushFloatArg(TR::Node* child)
{
  // if (isSmall()) return 0;
...
TR::Register*
OMR::ARM::Linkage::pushDoubleArg(TR::Node* child)
{
  // if (isSmall()) return 0;
...
int32_t
OMR::ARM::Linkage::numArgumentRegisters(TR_RegisterKinds kind)
{
  switch (kind) {
    case TR_GPR:
...
int32_t
OMR::ARM::Linkage::buildARMLinkageArgs(
  TR::Node* callNode,
  ...
  bool isVirtualOrJNI)
{
...
#ifndef LOCK_R14
    TR::addDependency(
      dependencies, NULL, TR::RealRegister::gr14, TR_GPR, cg()); // dep 2
#endif
  }
  ...
  const char* sig = "CALL";
  printf(
    "%s: numIntegerArgs %d numMemArgs %d\n", sig, numIntegerArgs, numMemArgs);
  fflush(stdout);
#endif
    ...
    switch (childType) {
      ...
      case TR::Float:
        ...
        if (childType == TR::Address)
          ...
          generateTrg1Src1Instruction(
            self()->cg(), ARMOp_fmrs, callNode, tempReg, reg);
          reg = tempReg;
        }
#endif
        else
        ...
        if (isSpecialArg) {
          if (specialArgReg == properties.getIntegerReturnRegister(0)) {
            ...
            TR::addDependency(
              dependencies, reg, specialArgReg, TR_GPR, self()->cg());
          }
        } else {
          if (numIntegerArgs < numIntArgRegs) {
            if (!self()->cg()->canClobberNodesRegister(child, 0) &&
              ...
              generateTrg1Src1Instruction(
                self()->cg(), ARMOp_mov, child, tempReg, reg);
              reg = tempReg;
            }
            if (numIntegerArgs == 0 /* &&
      ...
      case TR::Double:
        if (childType == TR::Double) {
          ...
          generateTrg2Src1Instruction(
            self()->cg(), ARMOp_fmrrd, callNode, lowReg, highReg, reg);
          reg = tempReg;
        } else
#endif
  ...
  */
  ...
  switch (conventions) {
    case TR_Private:
      if ((!isVirtual) && (specialArgReg != TR::RealRegister::gr11))
        TR::addDependency(
          dependencies, NULL, TR::RealRegister::gr11, TR_GPR, self()->cg());
      // deliberate fall-through
    case TR_Helper:
      TR::addDependency(
        dependencies, NULL, TR::RealRegister::gr4, TR_GPR, self()->cg());
      TR::addDependency(
        dependencies, NULL, TR::RealRegister::gr5, TR_GPR, self()->cg());
      // deliberate fall-through
    case TR_System:
#ifndef LOCK_R14
      if (!isVirtual)
        TR::addDependency(
          dependencies, NULL, TR::RealRegister::gr14, TR_GPR, cg());
#endif
      break;
  }
  ...
  if (numMemArgs > 0) {
    ...
    {
#ifdef DEBUG_ARM_LINKAGE
      printf("pushing mem arg %d of %d (in reg %x) to [sp + %d]... ",
             i,
             numMemArgs,
