        ...
        case TR::Int64:
            if (hasToBeOnStack) {
                ...
                if (numIntArgs < properties.getNumIntArgRegs() - 1) {
                    ...
                    cursor = generateMemSrc1Instruction(codeGen, ARMOp_str, firstNode,
                        new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset + 4, codeGen), argRegister,
                        cursor);
                }
            }
            numIntArgs += 2;
    ...
    if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
        ...
        if (isClass && self()->cg()->wantToPatchClassPointer((TR_OpaqueClassBlock*)child->getAddress(), child)) {
            ...
            if (child->isMethodPointerConstant())
                loadAddressConstant(
                    self()->cg(), child, child->getAddress(), pushRegister, NULL, false, TR_RamMethodSequence);
            else
...
#endif
                    tempMR
                        = self()->getOutgoingArgumentMemRef(totalSize, argSize, reg, ARMOp_str, pushToMemory[memArg++]);
                }
                numIntegerArgs++;
                argSize += 4;
            }
...
TR::Register* OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node* callNode, bool isSystem)
{
    ...
    case TR::fcall:
        ...
        if (resType.isFloatingPoint()) {
            ...
            TR::Instruction* cursor
                = generateTrg1Src1Instruction(codeGen, ARMOp_fmsr, callNode, tempReg, returnRegister);
...
#endif
    {
        ...
        if (resType.isDouble()) {
            ...
            TR::Instruction* cursor
                = generateTrg1Src2Instruction(codeGen, ARMOp_fmdrr, callNode, tempReg, lowReg, highReg);
            returnRegister = tempReg;
        }
        break;
    }
