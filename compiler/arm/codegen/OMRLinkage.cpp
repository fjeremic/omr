  ...
  for (i = from; (isHelper && i > to) || (!isHelper && i < to); i += step) {
    ...
    case TR::Float:
      ...
      if (childType == TR::Address)
        ...
        generateTrg1Src1Instruction(self()->cg(), ARMOp_fmrs, callNode, tempReg,
                                    reg);
        reg = tempReg;
      }
#endif
    ...
    case TR::Double:
      if (childType == TR::Double) {
        ...
        generateTrg2Src1Instruction(self()->cg(), ARMOp_fmrrd, callNode, lowReg,
                                    highReg, reg);
        reg = tempReg;
      } else
#endif
...
TR::Register *
OMR::ARM::Linkage::buildARMLinkageDirectDispatch(TR::Node *callNode,
                                                 bool isSystem) {
  TR::CodeGenerator *codeGen = self()->cg();
