...
public:
  ...
  void genericAnalyser(
    TR::Node* root, TR_ARMOpCodes regToRegOpCode, TR_ARMOpCodes copyOpCode, bool nonClobberingDestination = false);

  void genericLongAnalyser(TR::Node* root, TR_ARMOpCodes lowRegToRegOpCode, TR_ARMOpCodes highRegToRegOpCode,
    TR_ARMOpCodes lowMemToRegOpCode, TR_ARMOpCodes highMemToRegOpCode, TR_ARMOpCodes copyOpCode);

  void integerAddAnalyser(TR::Node* root, TR_ARMOpCodes regToRegOpCode);

  void longAddAnalyser(TR::Node* root);

  bool getReversedOperands()
  {
    return reversedOperands;
  }
  bool setReversedOperands(bool b)
  {
    return (reversedOperands = b);
  }
  bool notReversedOperands()
  {
    return (reversedOperands = ((reversedOperands == false) ? true : false));
  }
