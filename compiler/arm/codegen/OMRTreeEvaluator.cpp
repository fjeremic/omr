  ...
  TR::Node* node, int32_t value, TR::Register* trgReg, TR::CodeGenerator* cg, TR::Instruction* cursor)
{
  ...
  } else if ((base & 0xFFFF0000) == 0) // 16bit Value
  {
    ...
    cursor
      = generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, trgReg, trgReg, base & 0x000000FF, bitTrailing, cursor);
  } else if ((notBase & 0xFFFF0000) == 0) // reversed 16bits
  {
    cursor
      = generateTrg1ImmInstruction(cg, ARMOp_mov, node, trgReg, (notBase >> 8) & 0x000000FF, 8 + notTrailing, cursor);
    cursor
      = generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, trgReg, trgReg, notBase & 0x000000FF, notTrailing, cursor);
    cursor = generateTrg1Src1Instruction(cg, ARMOp_mvn, node, trgReg, trgReg, cursor);
  } else if ((base & 0xFF000000) == 0) // 24bit Value
  {
    cursor
      = generateTrg1ImmInstruction(cg, ARMOp_mov, node, trgReg, (base >> 16) & 0x000000FF, 16 + bitTrailing, cursor);
    if (((base >> 8) & 0x000000FF) != 0)
      cursor = generateTrg1Src1ImmInstruction(
        cg, ARMOp_add, node, trgReg, trgReg, (base >> 8) & 0x000000FF, 8 + bitTrailing, cursor);
    cursor
      = generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, trgReg, trgReg, base & 0x000000FF, bitTrailing, cursor);
  } else {
...
TR::Instruction* loadAddressConstantFixed(TR::CodeGenerator* cg, TR::Node* node, intptrj_t value, TR::Register* trgReg,
      ...
      if (typeAddress == TR_DataAddress) {
        ...
        if (doAOTRelocation) {
          cg->addExternalRelocation(new (cg->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(cursor,
                                      targetAddress, targetAddress2, (TR_ExternalRelocationTargetKind)typeAddress, cg),
            __FILE__, __LINE__, node);
        }
      } else {
        if (doAOTRelocation)
          cg->addExternalRelocation(new (cg->trHeapMemory())
                                      TR::BeforeBinaryEncodingExternalRelocation(cursor, (uint8_t*)value,
  ...
  if (OMR_UNLIKELY(checkFeedingVGDNOP)) {
    ...
    if (!nodeIsNeeded(topNode, node) && (comp->getSymRefTab()->isNonHelper(index, TR::SymbolReferenceTable::vftSymbol))
      ...
      while (tt && (tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
        ...
        if (cg->getSupportsVirtualGuardNOPing() && (checkNode->isNopableInlineGuard() || checkNode->isHCRGuard())
...
TR::Register* OMR::ARM::TreeEvaluator::lloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    ...
    generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)vrlRef->getMethodAddress(), deps, vrlRef);
    tempMR->decNodeReferenceCounts();
...
TR::Register* OMR::ARM::TreeEvaluator::commonLoadEvaluator(
  TR::Node* node, TR_ARMOpCodes memToRegOp, int32_t memSize, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::ARM::TreeEvaluator::lstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    ...
    generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)vwlRef->getMethodAddress(), deps, vwlRef);
    tempMR->decNodeReferenceCounts();
  ...
  TR::Node* node, int32_t byteLen, TR::Register* src, TR::Register* dst, TR::CodeGenerator* cg)
{
  ...
  for (ri = 0; ri<residual>> 2; ri++) {
    ...
    generateTrg1MemInstruction(
      cg, ARMOp_ldr, node, regs[0], new (cg->trHeapMemory()) TR::MemoryReference(src, 4 * ri, cg));
    generateMemSrc1Instruction(
      cg, ARMOp_str, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, 4 * ri, cg), regs[0]);
  }
  if (residual & 2) {
    generateTrg1MemInstruction(
      cg, ARMOp_ldrh, node, regs[0], new (cg->trHeapMemory()) TR::MemoryReference(src, 4 * ri, cg));
    generateMemSrc1Instruction(
      cg, ARMOp_strh, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, 4 * ri, cg), regs[0]);
    ri++;
    ri_x = 2;
  }
  if (residual & 1) {
    generateTrg1MemInstruction(
      cg, ARMOp_ldrb, node, regs[0], new (cg->trHeapMemory()) TR::MemoryReference(src, (4 * ri) + ri_x, cg));
    generateMemSrc1Instruction(
      cg, ARMOp_strb, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, (4 * ri) + ri_x, cg), regs[0]);
  }
...
TR::Register* OMR::ARM::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)arrayCopyHelper->getMethodAddress(), deps, arrayCopyHelper);
...
TR::Register* OMR::ARM::TreeEvaluator::asynccheckEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  gcPoint = generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)asynccheckHelper->getMethodAddress(), NULL,
...
TR::Register* OMR::ARM::TreeEvaluator::conversionAnalyser(
  TR::Node* node, TR_ARMOpCodes memoryToRegisterOp, bool needSignExtend, int dstBits, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* sourceRegister;
