...
uint32_t
findARMLoadConstantLength(int32_t value)
{
                ...
                TR::Instruction* cursor)
{
  ...
  if ((base & 0xFFFFFF00) == 0) // is Immed8r
  {
    ...
    cursor = generateTrg1ImmInstruction(
      cg, ARMOp_mov, node, trgReg, base & 0x000000FF, bitTrailing, cursor);
  ...
  {
    cursor = generateTrg1ImmInstruction(cg,
                                        ARMOp_mov,
                                        node,
                                        trgReg,
                                        (base >> 8) & 0x000000FF,
  ...
  } else if ((notBase & 0xFFFF0000) == 0) // reversed 16bits
  {
    ...
    cursor = generateTrg1Src1ImmInstruction(cg,
                                            ...
                                            cursor);
    cursor =
      generateTrg1Src1Instruction(cg, ARMOp_mvn, node, trgReg, trgReg, cursor);
  } else if ((base & 0xFF000000) == 0) // 24bit Value
  {
    cursor = generateTrg1ImmInstruction(cg,
                                        ARMOp_mov,
    ...
    cursor = generateTrg1Src2Instruction(
      cg, ARMOp_add, node, trgReg, trgReg, op2_2, cursor);
    cursor = generateTrg1Src2Instruction(
      cg, ARMOp_add, node, trgReg, trgReg, op2_1, cursor);
    cursor = generateTrg1Src2Instruction(
      cg, ARMOp_add, node, trgReg, trgReg, op2_0, cursor);
  }
                  ...
                  TR::Instruction* cursor)
{
  ...
  nibbles[0] = cursor =
    generateTrg1Src1Instruction(cg, ARMOp_mov, node, trgReg, op2_3, cursor);
  nibbles[1] = cursor = generateTrg1Src2Instruction(
    cg, ARMOp_add, node, trgReg, trgReg, op2_2, cursor);
  nibbles[2] = cursor = generateTrg1Src2Instruction(
    cg, ARMOp_add, node, trgReg, trgReg, op2_1, cursor);
  nibbles[3] = cursor = generateTrg1Src2Instruction(
    cg, ARMOp_add, node, trgReg, trgReg, op2_0, cursor);
                             ...
                             TR::Instruction* cursor)
{
  ...
  cg->findOrCreateAddressConstant(&address,
                                  TR::Address,
                                  q[0],
                                  q[1],
                                  q[2],
...
TR::Instruction*
loadAddressConstantFixed(TR::CodeGenerator* cg,
                         TR::Node* node,
                         intptrj_t value,
                         TR::Register* trgReg,
                         TR::Instruction* cursor,
    ...
    if (typeAddress == -1) {
      if (doAOTRelocation)
        cg->addExternalRelocation(
          new (cg->trHeapMemory())
            TR::BeforeBinaryEncodingExternalRelocation(cursor,
                                                       (uint8_t*)value,
                                                       (uint8_t*)seqKind,
                    ...
                    uint8_t* targetAddress2)
{
  if (cg->comp()->compileRelocatableCode())
    return loadAddressConstantFixed(cg,
                                    node,
                                    value,
                                    trgReg,
                                    cursor,
...
// also handles iiload
TR::Register*
OMR::ARM::TreeEvaluator::iloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* tempReg;
...
TR::Register*
OMR::ARM::TreeEvaluator::aloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
...
// also handles ilload
TR::Register*
OMR::ARM::TreeEvaluator::lloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    ...
    generateTrg1MemInstruction(
      cg, ARMOp_add, node, bigEndian ? highReg : lowReg, tempMR);
    ...
    generateImmSymInstruction(
      cg, ARMOp_bl, node, (uintptr_t)vrlRef->getMethodAddress(), deps, vrlRef);
    tempMR->decNodeReferenceCounts();
    ...
    generateTrg1MemInstruction(
      cg, ARMOp_ldr, node, bigEndian ? highReg : lowReg, lowMR);
    generateTrg1MemInstruction(
      cg, ARMOp_ldr, node, bigEndian ? lowReg : highReg, highMR);
...
// also handles ibload
TR::Register*
OMR::ARM::TreeEvaluator::bloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
// also handles isload
TR::Register*
OMR::ARM::TreeEvaluator::sloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
// also handles icload
TR::Register*
OMR::ARM::TreeEvaluator::cloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return commonLoadEvaluator(node, ARMOp_ldrh, 2, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::commonLoadEvaluator(TR::Node* node,
...
TR::Register*
OMR::ARM::TreeEvaluator::awrtbarEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::MemoryReference* tempMR =
    new (cg->trHeapMemory()) TR::MemoryReference(node, 4, cg);
  ...
  if (firstChild->getReferenceCount() > 1 &&
    ...
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, sourceRegister, firstChild->getRegister());
    killSource = true;
  } else
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getReferenceCount() > 1 &&
    ...
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, sourceRegister, secondChild->getRegister());
    killSource = true;
  } else
...
TR::Register*
OMR::ARM::TreeEvaluator::lstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* valueChild;
  TR::Compilation* comp = cg->comp();
  ...
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    ...
    TR::addDependency(
      deps, valueReg->getLowOrder(), TR::RealRegister::gr1, TR_GPR, cg);
    TR::addDependency(
      deps, valueReg->getHighOrder(), TR::RealRegister::gr2, TR_GPR, cg);

    generateImmSymInstruction(
      cg, ARMOp_bl, node, (uintptr_t)vwlRef->getMethodAddress(), deps, vwlRef);
    tempMR->decNodeReferenceCounts();
    ...
    if (bigEndian) {
      generateMemSrc1Instruction(
        cg, ARMOp_str, node, lowMR, valueReg->getHighOrder());
      generateMemSrc1Instruction(
        cg, ARMOp_str, node, highMR, valueReg->getLowOrder());
    } else {
      generateMemSrc1Instruction(
        cg, ARMOp_str, node, lowMR, valueReg->getLowOrder());
      generateMemSrc1Instruction(
        cg, ARMOp_str, node, highMR, valueReg->getHighOrder());
    }
    lowMR->decNodeReferenceCounts();
  }
...
TR::Register*
OMR::ARM::TreeEvaluator::bstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::ARM::TreeEvaluator::sstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::ARM::TreeEvaluator::istoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return commonStoreEvaluator(node, ARMOp_str, 4, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::commonStoreEvaluator(TR::Node* node,
                                              TR_ARMOpCodes memToRegOp,
                                              int32_t memSize,
                                              TR::CodeGenerator* cg)
{
  ...
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    generateInstruction(
      cg,
      (TR::Compiler->target.cpu.id() == TR_ARMv6) ? ARMOp_dmb_v6 : ARMOp_dmb_st,
      node);
  }
  generateMemSrc1Instruction(
    cg, memToRegOp, node, tempMR, cg->evaluate(valueChild));
  if (needSync && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
    generateInstruction(
      cg,
      (TR::Compiler->target.cpu.id() == TR_ARMv6) ? ARMOp_dmb_v6 : ARMOp_dmb,
...
#ifdef J9_PROJECT_SPECIFIC
TR::Register*
OMR::ARM::TreeEvaluator::monentEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return VMmonentEvaluator(node, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::monexitEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::ARM::TreeEvaluator::integerBitCount(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_UNIMPLEMENTED();
  return NULL;
}
...
TR::Register*
OMR::ARM::TreeEvaluator::longLowestOneBit(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_UNIMPLEMENTED();
  return NULL;
}
...
TR::Register*
OMR::ARM::TreeEvaluator::longBitCount(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_UNIMPLEMENTED();
  return NULL;
}
...
bool
OMR::ARM::TreeEvaluator::stopUsingCopyReg(TR::Node* node,
                                          TR::Register*& reg,
                                          TR::CodeGenerator* cg)
{
  if (node != NULL) {
...
static void
constLengthArrayCopyEvaluator(TR::Node* node,
                              int32_t byteLen,
                              TR::Register* src,
                              TR::Register* dst,
                              TR::CodeGenerator* cg)
{
  ...
  if (groups != 0) {
    ...
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_sub_r, node, countReg, countReg, 1, 0);
    generateConditionalBranchInstruction(
      cg, node, ARMConditionCodeNE, loopStart);
  }
  ...
  for (ri = 0; ri<residual>> 2; ri++) {
    ...
    generateMemSrc1Instruction(cg,
                               ...
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(dst, 4 * ri, cg),
                               regs[0]);
  }
  if (residual & 2) {
    generateTrg1MemInstruction(cg,
  ...
  if (residual & 1) {
    generateTrg1MemInstruction(cg,
                               ...
                               regs[0],
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(src, (4 * ri) + ri_x, cg));
    generateMemSrc1Instruction(cg,
                               ARMOp_strb,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(dst, (4 * ri) + ri_x, cg),
                               regs[0]);
  }
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  gcPoint =
    generateImmSymInstruction(cg,
                              ARMOp_bl,
                              node,
                              (uintptr_t)asynccheckHelper->getMethodAddress(),
                              NULL,
                              asynccheckHelper,
                              NULL,
                              NULL,
...
TR::Register*
OMR::ARM::TreeEvaluator::performCall(TR::Node* node,
                                     bool isIndirect,
                                     TR::CodeGenerator* cg)
{
  TR::SymbolReference* symRef = node->getSymbolReference();
...
TR::Register*
OMR::ARM::TreeEvaluator::treetopEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (mref->useIndexedForm()) {
      ...
      if (mref->hasDelayedOffset() || offset != 0) {
        ...
        if (resultReg == cg->getMethodMetaDataRegister()) {
          ...
          generateTrg1Src1Instruction(
            cg, ARMOp_mov, node, resultReg, mref->getBaseRegister());
        }
      }
    }
  }
...
TR::Register*
OMR::ARM::TreeEvaluator::BBStartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (node->getLabel() != NULL) {
    node->getLabel()->setInstruction(
      generateLabelInstruction(cg, ARMOp_label, node, node->getLabel(), deps));
    deps =
...
TR::Register*
OMR::ARM::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Block* block = node->getBlock();
  TR::Compilation* comp = cg->comp();
...
TR::Register*
OMR::ARM::TreeEvaluator::NOPEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return NULL;
}
...
TR::Register*
OMR::ARM::TreeEvaluator::unImpOpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return NULL;
}

TR::Register*
OMR::ARM::TreeEvaluator::conversionAnalyser(TR::Node* node,
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getReferenceCount() == 1 && child->getRegister() == NULL &&
    ...
    generateTrg1MemInstruction(
      cg, memoryToRegisterOp, node, targetRegister, tempMR);
    tempMR->decNodeReferenceCounts();
  } else {
    targetRegister = cg->allocateRegister();
    sourceRegister = child->getOpCode().isLong()
                       ...
                       : cg->evaluate(child);
    generateSignOrZeroExtend(
      node, targetRegister, sourceRegister, needSignExtend, dstBits, cg);
  }
  node->setRegister(targetRegister);
  child->decReferenceCount();
...
static void
generateSignOrZeroExtend(TR::Node* node,
                         TR::Register* dst,
                         TR::Register* src,
                         bool needSignExtend,
                         int32_t bitsInDst,
