...
void TR_ARMSubtractAnalyser::integerSubtractAnalyser(
  TR::Node* root, TR_ARMOpCodes regToRegOpCode, TR_ARMOpCodes memToRegOpCode)
{
  TR::Node* firstChild;
  TR::Node* secondChild;
  ...
  if (getCopyReg1()) {
    ...
    if (getSubReg3Reg2()) {
      new (cg()->trHeapMemory())
        TR::ARMTrg1Src2Instruction(ARMOp_sub, root, thirdReg, secondRegister, firstRegister, cg());
...
void TR_ARMSubtractAnalyser::longSubtractAnalyser(TR::Node* root)
{
  ...
  if (getCopyReg1()) {
    ...
    root->setRegister(thirdReg);
    new (cg()->trHeapMemory())
      TR::ARMTrg1Src1Instruction(ARMOp_mov, root, lowThird, firstRegister->getLowOrder(), cg());
    new (cg()->trHeapMemory())
      TR::ARMTrg1Src1Instruction(ARMOp_mov, root, highThird, firstRegister->getHighOrder(), cg());
    if (getSubReg3Reg2()) {
      new (cg()->trHeapMemory())
        TR::ARMTrg1Src1Instruction(ARMOp_sub, root, lowThird, secondRegister->getLowOrder(), cg());
  ...
  } else // assert getSubReg1Mem2() == true
  {
    ...
    //      TR_ARMMemoryReference *highMR = new (cg()->trHeapMemory()) TR_ARMMemoryReference(*lowMR, 4);
    //      new (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SBB4RegMem, firstRegister->getHighOrder(), highMR,
