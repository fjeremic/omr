...
void
TR_ARMSubtractAnalyser::integerSubtractAnalyser(TR::Node* root,
  ...
  if (getCopyReg1()) {
    ...
    if (getSubReg3Reg2()) {
      new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(
        ARMOp_sub, root, thirdReg, secondRegister, firstRegister, cg());
    } else // assert getSubReg3Mem2() == true
    {
      //         TR_ARMMemoryReference *tempMR = new (cg()->trHeapMemory())
      //         TR_ARMMemoryReference(secondChild); new (cg()->trHeapMemory())
      //         TR::ARMRegMemInstruction(memToRegOpCode, thirdReg, tempMR,
      //         cg()); tempMR->decNodeReferenceCounts();
    }
    root->setRegister(thirdReg);
  } else if (getSubReg1Reg2()) {
    new (cg()->trHeapMemory()) TR::ARMTrg1Src1Instruction(
      ARMOp_sub, root, firstRegister, secondRegister, cg());
...
void
TR_ARMSubtractAnalyser::longSubtractAnalyser(TR::Node* root)
{
  TR::Node* firstChild;
  TR::Node* secondChild;
  firstChild = root->getFirstChild();
  ...
  if (getCopyReg1()) {
    ...
    TR::RegisterPair* thirdReg =
      cg()->allocateRegisterPair(lowThird, highThird);
    root->setRegister(thirdReg);
    new (cg()->trHeapMemory()) TR::ARMTrg1Src1Instruction(
      ARMOp_mov, root, lowThird, firstRegister->getLowOrder(), cg());
    new (cg()->trHeapMemory()) TR::ARMTrg1Src1Instruction(
      ARMOp_mov, root, highThird, firstRegister->getHighOrder(), cg());
    if (getSubReg3Reg2()) {
      new (cg()->trHeapMemory()) TR::ARMTrg1Src1Instruction(
        ARMOp_sub, root, lowThird, secondRegister->getLowOrder(), cg());
      //         new (cg()->trHeapMemory())
      //         TR::ARMTrg1Src1Instruction(SBB4RegReg, root, highThird,
      //         secondRegister->getHighOrder(), cg());
    } else // assert getSubReg3Mem2() == true
    {
      //         TR_ARMMemoryReference *lowMR = new (cg()->trHeapMemory())
      //         TR_ARMMemoryReference(secondChild); new (cg()->trHeapMemory())
      //         TR::ARMRegMemInstruction(SUB4RegMem, lowThird, lowMR, cg());
      //         TR_ARMMemoryReference *highMR = new (cg()->trHeapMemory())
      //         TR_ARMMemoryReference(*lowMR, 4); new (cg()->trHeapMemory())
      //         TR::ARMRegMemInstruction(SBB4RegMem, highThird, highMR, cg());
      //         lowMR->decNodeReferenceCounts();
    }
    root->setRegister(thirdReg);
  } else if (getSubReg1Reg2()) {
    //      new (cg()->trHeapMemory()) TR::ARMTrg1Src2Instruction(root,
    //      ARMOp_subf, firstRegister->getLowOrder(),
    //      secondRegister->getLowOrder(), cg()); new (cg()->trHeapMemory())
    //      TR::ARMRegRegInstruction(SBB4RegReg, firstRegister->getHighOrder(),
    //      secondRegister->getHighOrder(), cg());
    root->setRegister(firstRegister);
  } else // assert getSubReg1Mem2() == true
  {
    //      TR_ARMMemoryReference *lowMR = new (cg()->trHeapMemory())
    //      TR_ARMMemoryReference(secondChild); new (cg()->trHeapMemory())
    //      TR::ARMRegMemInstruction(SUB4RegMem, firstRegister->getLowOrder(),
    //      lowMR, cg()); TR_ARMMemoryReference *highMR = new
    //      (cg()->trHeapMemory()) TR_ARMMemoryReference(*lowMR, 4); new
    //      (cg()->trHeapMemory()) TR::ARMRegMemInstruction(SBB4RegMem,
    //      firstRegister->getHighOrder(), highMR, cg());
    //      lowMR->decNodeReferenceCounts();
    //      root->setRegister(firstRegister);
  }
