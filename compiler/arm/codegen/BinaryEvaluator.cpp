...
TR::Register* OMR::ARM::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if ((secondOp == TR::iconst || secondOp == TR::iuconst) && secondChild->getRegister() == NULL) {
  ...
  if ((node->getOpCodeValue() == TR::aiadd || node->getOpCodeValue() == TR::aiuadd) && node->isInternalPointer()) {
...
TR::Register* OMR::ARM::TreeEvaluator::lmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlongMultiply, false, false, false)->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlongMultiply, false, false, false));
...
static TR::Register* idivAndIRemHelper(TR::Node* node, bool isDivide, TR::CodeGenerator* cg)
{
  ...
  TR::SymbolReference* helper
    = cg->symRefTab()->findOrCreateRuntimeHelper(isDivide ? TR_ARMintDivide : TR_ARMintRemainder, false, false, false);

  generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)helper->getMethodAddress(), dependencies, helper);
...
static TR::Register* ldivAndLRemHelper(TR::Node* node, bool isDivide, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlongDivide, false, false, false)->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlongDivide, false, false, false));
  ...
  if (secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL) {
    ...
    if (shiftAmount >= 32) {
      if (isLogical) {
        ...
        new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(ARMOp_mov, node, trgReg->getHighOrder(),
      ...
      if (shiftAmount == 32) {
        ...
        new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(ARMOp_mov, node, trgReg->getLowOrder(),
    ...
    if (srcHighValue == 0) {
      ...
      TR::SymbolReference* longShiftHelper = cg->symRefTab()->findOrCreateRuntimeHelper(
        isLogical ? TR_ARMlongShiftRightLogical : TR_ARMlongShiftRightArithmetic, false, false, false);

      generateImmSymInstruction(
        cg, ARMOp_bl, node, (uintptr_t)longShiftHelper->getMethodAddress(), dependencies, longShiftHelper);
      cg->machine()->setLinkRegisterKilled(true);
    }
  }
  firstChild->decReferenceCount();
  ...
  if (secondChild->getOpCodeValue() == TR::iconst || secondChild->getOpCodeValue() == TR::iuconst) {
    ...
    if (value != 0) {
      ...
      generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, trgReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegRORImmed, srcReg, 32 - value));
    } else {
    ...
    // Use ROR (Rotate Right) Operand2
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, trgReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegRORReg, srcReg, trgReg));
  }
...
TR::Register* OMR::ARM::TreeEvaluator::lrolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::iconst || secondChild->getOpCodeValue() == TR::iuconst) {
    ...
    if (value == 0) {
      ...
      // Not tested
      generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, lowReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSLImmed, srcLowReg, value));
      generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, highReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSLImmed, srcHighReg, value));
      generateTrg1Src2Instruction(cg, ARMOp_orr, node, lowReg, lowReg,
        new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSRImmed, srcHighReg, 32 - value));
      generateTrg1Src2Instruction(cg, ARMOp_orr, node, highReg, highReg,
...
static TR::Register* ibooleanTypeImmediateEvaluator(
  TR::Node* srcNode, TR_ARMOpCodes regOp, TR::Register* srcReg, int32_t imm, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = NULL;
  uint32_t base, rotate;
  ...
  if (srcNode->getReferenceCount() <= 1
    && (regOp == ARMOp_and && imm == -1
      || regOp != ARMOp_and && imm == 0)) { // an operand of a long logical operations can have a half word which is an
                                            // identity value for that operation
    trgReg = srcReg;
    ...
    trgReg = cg->allocateRegister();
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, srcNode, trgReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSLImmed, srcReg, 16));
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, srcNode, trgReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSRImmed, trgReg, 16));
  }
  return trgReg;
}

// Do the work for evaluating lor, lxor, land
static inline TR::Register* lbooleanTypeEvaluator(TR::Node* node, TR_ARMOpCodes regOp, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (!(trgHigh
          = ibooleanTypeImmediateEvaluator(firstChild, regOp, src1Reg->getHighOrder(), (int32_t)(imm64 >> 32), cg))) {
  ...
  if (canCombine) {
    ...
    if (shiftSecondChild->getOpCodeValue() == TR::iconst) {
      ...
      generateTrg1Src2Instruction(
        cg, ARMOp_and, node, trgReg, shiftAmountReg, new (cg->trHeapMemory()) TR_ARMOperand2(0x1f, 0));
...
static inline TR::Register* ibooleanTypeEvaluator(TR::Node* node, TR_ARMOpCodes regOp, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = NULL;
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
  ...
  } else if (regOp == ARMOp_and && secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL
    && secondChild->getInt() == 0xffff) {
    trgReg = cg->allocateRegister();
    TR::Register* src = cg->evaluate(firstChild);
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, trgReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSLImmed, src, 16));
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, trgReg, new (cg->trHeapMemory()) TR_ARMOperand2(ARMOp2RegLSRImmed, trgReg, 16));
  } else {
    TR::Register* src2Reg = cg->evaluate(secondChild);
    trgReg = cg->allocateRegister();
