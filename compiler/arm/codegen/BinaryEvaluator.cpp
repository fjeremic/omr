                     ...
                     TR::CodeGenerator* cg)
{
  ...
  if ((localVal.getValue() < 0) &&
    ...
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_sub, node, trgReg, srcReg, base, rotate);
  } else if (constantIsImmed8r(localVal.getValue(), &base, &rotate)) {
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_add, node, trgReg, srcReg, base, rotate);
  } else {
...
TR::Register*
OMR::ARM::TreeEvaluator::iaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
                  ...
                  TR::CodeGenerator* cg)
{
  ...
  if (constantIsImmed8r(lowValue, &base, &rotate)) {
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_add_r, node, lowReg, srcLowReg, base, rotate);
  } else // constant won't fit
  {
    ...
    generateTrg1Src2Instruction(
      cg, ARMOp_add_r, node, lowReg, srcLowReg, lowValueReg);
  }

  // add high half
  if (constantIsImmed8r(highValue, &base, &rotate)) {
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_adc, node, highReg, srcHighReg, base, rotate);
  } else {
    TR::Register* highValueReg = cg->allocateRegister();
    armLoadConstant(node, highValue, highValueReg, cg);
    generateTrg1Src2Instruction(
      cg, ARMOp_adc, node, highReg, srcHighReg, highValueReg);
  }
  return trgReg;
}

TR::Register*
OMR::ARM::TreeEvaluator::laddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
...
TR::Register*
OMR::ARM::TreeEvaluator::isubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* trgReg = NULL;
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    trgReg = cg->allocateRegister();
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_rsb, node, trgReg, src2Reg, base, rotate);
  } else {
    if ((trgReg = ishiftAnalyser(node, cg, ARMOp_sub)) != NULL) {
      ...
      src1Reg = cg->evaluate(firstChild);
      generateTrg1Src2Instruction(
        cg, ARMOp_sub, node, trgReg, src1Reg, cg->evaluate(secondChild));
    }
...
TR::Register*
OMR::ARM::TreeEvaluator::lsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* trgReg = NULL;
...
TR::Register*
OMR::ARM::TreeEvaluator::imulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  {
    generateTrg1Src2MulInstruction(
      cg, ARMOp_mul, node, trgReg, src1Reg, cg->evaluate(secondChild));
  }
  firstChild->decReferenceCount();
  secondChild->decReferenceCount();
  node->setRegister(trgReg);
...
TR::Register*
OMR::ARM::TreeEvaluator::imulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = cg->allocateRegister();
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    generateTrg2Src2MulInstruction(
      cg, ARMOp_smull, node, trgReg, tempRegLow, magicReg, src1Reg);
    cg->stopUsingRegister(magicReg);
  } else {
    generateTrg2Src2MulInstruction(cg,
                                   ARMOp_smull,
                                   node,
                                   trgReg,
                                   tempRegLow,
...
TR::Register*
OMR::ARM::TreeEvaluator::lmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (firstChild->getReferenceCount() > 1) {
    ...
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dd_lowReg, arg1->getLowOrder());
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dd_highReg, arg1->getHighOrder());
  } else {
  ...
  if (secondChild->getReferenceCount() > 1) {
    ...
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dr_lowReg, arg2->getLowOrder());
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dr_highReg, arg2->getHighOrder());
  } else {
  ...
  generateImmSymInstruction(
    ...
    cg->symRefTab()->findOrCreateRuntimeHelper(
      TR_ARMlongMultiply, false, false, false));
                                         ...
                                         TR::CodeGenerator* cg)
{
  ...
  if (divisor == -1) {
    ...
    if (isDivide) {
      generateTrg1Src1ImmInstruction(
        cg, ARMOp_rsb, node, trgReg, dividendReg, 0, 0);
    } else {
      armLoadConstant(node, 0, trgReg, cg);
    }
    return trgReg;
  ...
  if (isPowerOf2(divisor)) {
    if (!isDivide) {
      ...
      if (constantIsImmed8r(
            divisor - 1, &base, &rotate)) // if divisor - 1 fits in 8 bit imm
                                          // field can do this cheaply
      {
        generateTrg1Src1ImmInstruction(
          cg, ARMOp_and_r, node, trgReg, dividendReg, base, rotate);
      } else {
        armLoadConstant(node, divisor - 1, trgReg, cg);
        generateTrg1Src2Instruction(
          cg, ARMOp_and_r, node, trgReg, dividendReg, trgReg);
      }
      ...
      if (constantIsImmed8r(
            divisor,
            &base,
            &rotate)) // if divisor fits in 8 bit imm field can do this cheaply
      {
        instr2 = generateTrg1Src1ImmInstruction(
          cg, ARMOp_sub, node, trgReg, trgReg, base, rotate);
        instr2->setConditionCode(ARMConditionCodeMI);
      } else {
        TR::Register* temp1Reg = cg->allocateRegister();
        armLoadConstant(node, divisor, temp1Reg, cg);
        instr2 = generateTrg1Src2Instruction(
          cg, ARMOp_sub, node, trgReg, trgReg, temp1Reg);
        instr2->setConditionCode(ARMConditionCodeMI);
      }
      return trgReg;
    } else // isDivide
    {
      ...
      instr = generateTrg1Src1ImmInstruction(
        cg, ARMOp_add, node, temp1Reg, temp1Reg, base, rotate);
      instr->setConditionCode(ARMConditionCodeMI);
      generateShiftRightArithmeticImmediate(
        cg, node, temp1Reg, temp1Reg, trailingZeroes(divisor));

      // (x / -2) is same as (x / 2) * -1
      if (isNegative) {
        generateTrg1Src1ImmInstruction(
          cg, ARMOp_rsb, node, temp1Reg, temp1Reg, 0, 0);
      }
      return temp1Reg;
    }
  } else {
    ...
    // number register
    generateTrg2Src2MulInstruction(
      cg, ARMOp_smull, node, temp1Reg, temp2Reg, magicReg, dividendReg);

    if ((divisor > 0) && (magicNumber < 0)) {
      temp2Reg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, ARMOp_add, node, temp2Reg, dividendReg, temp1Reg);
    } else if ((divisor < 0) && (magicNumber > 0)) {
      temp2Reg = cg->allocateRegister();
      generateTrg1Src2Instruction(
        cg, ARMOp_rsb, node, temp2Reg, dividendReg, temp1Reg);
    } else
      temp2Reg = temp1Reg;
    ...
    if (!isDivide) // is irem
    {
      ...
      generateTrg1Src2Instruction(
        cg, ARMOp_rsb, node, temp6Reg, temp5Reg, dividendReg);
      return temp6Reg;
    } else {
      return temp4Reg;
    }
...
static TR::Register*
idivAndIRemHelper(TR::Node* node, bool isDivide, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  generateImmSymInstruction(cg,
                            ARMOp_bl,
                            node,
                            (uintptr_t)helper->getMethodAddress(),
                            dependencies,
                            helper);
...
static TR::Register*
ldivAndLRemHelper(TR::Node* node, bool isDivide, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* dividend = cg->evaluate(firstChild);
  ...
  if (firstChild->getReferenceCount() > 1) {
    ...
    dd_highReg = cg->allocateRegister();
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dd_lowReg, dividend->getLowOrder());
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dd_highReg, dividend->getHighOrder());
  } else {
  ...
  if (secondChild->getReferenceCount() > 1) {
    ...
    dr_highReg = cg->allocateRegister();
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dr_lowReg, divisor->getLowOrder());
    generateTrg1Src1Instruction(
      cg, ARMOp_mov, node, dr_highReg, divisor->getHighOrder());
  } else {
  ...
  generateImmSymInstruction(
    ...
    dependencies,
    cg->symRefTab()->findOrCreateRuntimeHelper(
      TR_ARMlongDivide, false, false, false));
...
TR::Register*
OMR::ARM::TreeEvaluator::idivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return idivAndIRemHelper(node, true, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::iremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return idivAndIRemHelper(node, false, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::ldivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return ldivAndLRemHelper(node, true, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return ldivAndLRemHelper(node, false, cg);
}

static TR::Register*
ishiftEvaluator(TR::Node* node,
                ...
                TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    generateTrg1Src2Instruction(
      cg, ARMOp_and, node, trgReg, shiftAmountReg, op);
...
TR::Register*
OMR::ARM::TreeEvaluator::ishlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return ishiftEvaluator(node, ARMOp2RegLSLImmed, ARMOp2RegLSLReg, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg =
  ...
  if (/* !isSmall() && */ secondChild->getOpCodeValue() == TR::iconst &&
    ...
    if (shiftAmount < 32) {
      ...
      generateShiftLeftImmediate(
        cg, node, trgReg->getLowOrder(), srcLowReg, shiftAmount);
      generateShiftRightLogicalImmediate(
        cg, node, temp2Reg, srcLowReg, 32 - shiftAmount);
      generateTrg1Src2Instruction(
        cg, ARMOp_orr, node, trgReg->getHighOrder(), temp1Reg, temp2Reg);
    } else // shiftAmount >= 32
    {
      generateShiftLeftImmediate(
        cg, node, trgReg->getHighOrder(), srcLowReg, shiftAmount - 32);
      generateTrg1ImmInstruction(
        cg, ARMOp_mov, node, trgReg->getLowOrder(), 0, 0);
    }
  } else {
    TR::Register* temp1Reg = cg->allocateRegister();
    TR::Register* temp2Reg = cg->allocateRegister();
    ...
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_and, node, shiftAmountReg, cg->evaluate(secondChild), 0x3f, 0);
    generateShiftLeftByRegister(cg, node, temp1Reg, srcHighReg, shiftAmountReg);
    generateShiftLeftByRegister(
      cg, node, trgReg->getLowOrder(), srcLowReg, shiftAmountReg);
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_rsb, node, temp2Reg, shiftAmountReg, 32, 0);
    generateShiftRightLogicalByRegister(
      cg, node, temp3Reg, srcLowReg, temp2Reg);
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_sub, node, temp4Reg, shiftAmountReg, 32, 0);
    ...
    generateTrg1Src2Instruction(
      cg, ARMOp_orr, node, trgReg->getHighOrder(), temp1Reg, temp6Reg);
  }
  firstChild->decReferenceCount();
  secondChild->decReferenceCount();
  node->setRegister(trgReg);
...
TR::Register*
OMR::ARM::TreeEvaluator::ishrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return ishiftEvaluator(node, ARMOp2RegASRImmed, ARMOp2RegASRReg, cg);
}

static TR::Register*
longRightShiftEvaluator(TR::Node* node, bool isLogical, TR::CodeGenerator* cg)
{
  TR::Register* trgReg =
    cg->allocateRegisterPair(cg->allocateRegister(), cg->allocateRegister());
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::iconst &&
    ...
    if (shiftAmount >= 32) {
      if (isLogical) {
        generateTrg1ImmInstruction(
          cg, ARMOp_mov, node, trgReg->getHighOrder(), 0, 0);
      } else {
      ...
      if (shiftAmount == 32) {
        generateTrg1Src1Instruction(
          cg, ARMOp_mov, node, trgReg->getLowOrder(), srcHighReg);
      } else {
        new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(
          ARMOp_mov,
          node,
    ...
    } else // (shiftAmount < 32)
    {
      ...
      generateShiftRightLogicalImmediate(
        cg, node, temp1Reg, srcLowReg, shiftAmount);
      generateShiftLeftImmediate(
        cg, node, temp2Reg, srcHighReg, 32 - shiftAmount);
      generateTrg1Src2Instruction(
        cg, ARMOp_orr, node, trgReg->getLowOrder(), temp1Reg, temp2Reg);
    }
  } else {
    ...
    if (srcHighValue == 0) {
      ...
      if (srcLowValue == 0)
        ...
        generateShiftRightByRegister(cg,
                                     node,
                                     trgReg->getLowOrder(),
                                     srcLowReg,
                                     shiftAmountReg,
      ...
      generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, trgReg->getLowOrder(), srcLowReg);
      generateTrg1Src1Instruction(
        cg, ARMOp_mov, node, trgReg->getHighOrder(), srcHighReg);

      TR::addDependency(
        dependencies, trgReg->getLowOrder(), TR::RealRegister::gr0, TR_GPR, cg);
      TR::addDependency(dependencies,
                        ...
                        cg);
      TR::addDependency(
        dependencies, shiftAmountReg, TR::RealRegister::gr2, TR_GPR, cg);
...
TR::Register*
OMR::ARM::TreeEvaluator::lshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return longRightShiftEvaluator(node, false, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::iushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return ishiftEvaluator(node, ARMOp2RegLSRImmed, ARMOp2RegLSRReg, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return longRightShiftEvaluator(node, true, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::irolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::ARM::TreeEvaluator::lrolEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(node->getNumChildren() == 2,
            "Wrong number of children in inlineIntegerRotateLeft");
  ...
  if (secondChild->getOpCodeValue() == TR::iconst ||
    ...
    if (value == 0) {
      ...
      generateTrg1Src2Instruction(cg,
                                  ...
                                  lowReg,
                                  new (cg->trHeapMemory()) TR_ARMOperand2(
                                    ARMOp2RegLSRImmed, srcHighReg, 32 - value));
      generateTrg1Src2Instruction(cg,
                                  ARMOp_orr,
                                  node,
                                  highReg,
                                  highReg,
                                  new (cg->trHeapMemory()) TR_ARMOperand2(
                                    ARMOp2RegLSRImmed, srcLowReg, 32 - value));
    } else // value > 32
    {
      // Not tested
      generateTrg1Src1Instruction(cg,
                                  ...
                                  lowReg,
                                  new (cg->trHeapMemory()) TR_ARMOperand2(
                                    ARMOp2RegLSLImmed, srcHighReg, value - 32));
      generateTrg1Src1Instruction(cg,
                                  ARMOp_mov,
                                  node,
                                  highReg,
                                  new (cg->trHeapMemory()) TR_ARMOperand2(
                                    ARMOp2RegLSLImmed, srcLowReg, value - 32));
      generateTrg1Src2Instruction(cg,
                                  ARMOp_orr,
                                  node,
                                  lowReg,
                                  lowReg,
                                  new (cg->trHeapMemory()) TR_ARMOperand2(
                                    ARMOp2RegLSRImmed, srcLowReg, 64 - value));
      generateTrg1Src2Instruction(cg,
                                  ARMOp_orr,
                                  node,
                                  highReg,
                                  highReg,
                                  new (cg->trHeapMemory()) TR_ARMOperand2(
                                    ARMOp2RegLSRImmed, srcHighReg, 64 - value));
    }
  } else {
    TR_UNIMPLEMENTED();
  }
...
static TR::Register*
ibooleanTypeImmediateEvaluator(TR::Node* srcNode,
                               TR_ARMOpCodes regOp,
                               TR::Register* srcReg,
                               int32_t imm,
                               TR::CodeGenerator* cg)
{
  ...
  if (srcNode->getReferenceCount() <= 1 &&
      (regOp == ARMOp_and && imm == -1 ||
       regOp != ARMOp_and &&
         imm == 0)) { // an operand of a long logical operations can have a half
                      // word which is an identity value for that operation
    trgReg = srcReg;
  } else if (constantIsImmed8r(imm, &base, &rotate)) {
    trgReg = cg->allocateRegister();
    generateTrg1Src1ImmInstruction(
      cg, regOp, srcNode, trgReg, srcReg, base, rotate);
...
lbooleanTypeEvaluator(TR::Node* node,
                      TR_ARMOpCodes regOp,
                      TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (!(trgHigh = ibooleanTypeImmediateEvaluator(firstChild,
                                                   regOp,
                                                   src1Reg->getHighOrder(),
                                                   (int32_t)(imm64 >> 32),
                                                   cg))) {
    ...
    new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(regOp,
                                                        node,
                                                        trgReg->getLowOrder(),
                                                        src2Reg->getLowOrder(),
                                                        src1Reg->getLowOrder(),
...
TR::Register*
OMR::ARM::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return lbooleanTypeEvaluator(node, ARMOp_and, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return lbooleanTypeEvaluator(node, ARMOp_orr, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
static TR::Register*
ishiftAnalyser(TR::Node* node, TR::CodeGenerator* cg, TR_ARMOpCodes regOp)
{
  ...
  if (canCombine) {
    ...
    if (shiftSecondChild->getOpCodeValue() == TR::iconst) {
      ...
      shiftOp =
        new (cg->trHeapMemory()) TR_ARMOperand2(shiftTypeReg, src1Reg, trgReg);
    }

    if (shiftIsFirstChild)
      generateTrg1Src2Instruction(
        cg, regOp, node, trgReg, cg->evaluate(secondChild), shiftOp);
    else
      generateTrg1Src2Instruction(
        cg, regOp, node, trgReg, cg->evaluate(firstChild), shiftOp);
...
static inline TR::Register*
ibooleanTypeEvaluator(TR::Node* node,
                      TR_ARMOpCodes regOp,
                      TR::CodeGenerator* cg)
{
  TR::Register* trgReg = NULL;
  ...
  if (comp->getOption(TR_TraceCG)) {
    traceMsg(comp,
             "In ibooleanTypeEvaluator for n%dn (%p)\n",
             node->getGlobalIndex(),
             node);
  }
  ...
  if (secondChild->getOpCodeValue() == TR::iconst &&
    ...
    trgReg = cg->allocateRegister();
    generateTrg1Src1ImmInstruction(
      cg, regOp, node, trgReg, cg->evaluate(firstChild), base, rotate);
    ...
    trgReg = cg->allocateRegister();
    generateTrg1Src2Instruction(
      cg, regOp, node, trgReg, cg->evaluate(firstChild), src2Reg);
  }
...
TR::Register*
OMR::ARM::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return ibooleanTypeEvaluator(node, ARMOp_and, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::iorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return ibooleanTypeEvaluator(node, ARMOp_orr, cg);
}

TR::Register*
OMR::ARM::TreeEvaluator::ixorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
            ...
            TR::CodeGenerator* cg)
{
  if (value == 0) {
    ...
    generateTrg1Src1ImmInstruction(
      cg, ARMOp_rsb, node, trgReg, sourceReg, 0, 0);
    ...
    generateTrg1Src2Instruction(
      cg, ARMOp_add, node, trgReg, tempReg, sourceReg);
  } else if (isNonNegativePowerOf2(value + 1)) {
    TR::Register* tempReg = cg->allocateRegister();
    generateShiftLeftImmediate(
      cg, node, tempReg, sourceReg, trailingZeroes(value + 1));
    generateTrg1Src2Instruction(
      cg, ARMOp_sub, node, trgReg, tempReg, sourceReg);
  } else // constant won't fit
  {
    TR::Register* tempReg = cg->allocateRegister();
    armLoadConstant(node, value, tempReg, cg);
    generateTrg1Src2MulInstruction(
      cg, ARMOp_mul, node, trgReg, sourceReg, tempReg);
  }
}
