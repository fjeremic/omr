...
TR::Register* OMR::ARM::TreeEvaluator::isubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else if (firstChild->getOpCode().isLoadConst() && firstChild->getRegister() == NULL && constantIsImmed8r(firstChild->getInt(), &base, &rotate)) {
        TR::Register* src2Reg = cg->evaluate(secondChild);
        trgReg = cg->allocateRegister();
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (srcNode->getReferenceCount() <= 1 && (regOp == ARMOp_and && imm == -1 || regOp != ARMOp_and && imm == 0)) { // an operand of a long logical operations can have a half word which is an identity value for that operation
        trgReg = srcReg;
...
static TR::Register* ishiftAnalyser(TR::Node* node, TR::CodeGenerator* cg, TR_ARMOpCodes regOp)
{
    ...
    if ((secondChild->getOpCode().isLeftShift() || secondChild->getOpCode().isRightShift() || secondChild->getOpCode().isShiftLogical()) && !firstChild->getOpCode().isLoadConst() && secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL) {
        canCombine = true;
        shiftIsFirstChild = false;
    } else if (secondChild->getOpCode().isMul() && secondChild->getOpCode().isInt() && !firstChild->getOpCode().isLoadConst() && secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL) {
    ...
    } else if ((firstChild->getOpCode().isLeftShift() || firstChild->getOpCode().isRightShift() || firstChild->getOpCode().isShiftLogical()) && !secondChild->getOpCode().isLoadConst() && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
        canCombine = true;
        shiftIsFirstChild = true;
        if (regOp == ARMOp_sub)
            regOp = ARMOp_rsb;
    } else if (firstChild->getOpCode().isMul() && firstChild->getOpCode().isInt() && !secondChild->getOpCode().isLoadConst() && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
    ...
    TR_ARMOpCodes regOp, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL && constantIsImmed8r(secondChild->getInt(), &base, &rotate)) {
        TR::Node* firstChild = node->getFirstChild();
        trgReg = cg->allocateRegister();
    ...
    } else if (regOp == ARMOp_and && secondChild->getOpCodeValue() == TR::iconst && secondChild->getRegister() == NULL && secondChild->getInt() == 0xffff) {
        trgReg = cg->allocateRegister();
