  ...
  if (rootLoadOrStore->getOpCode().isIndirect()) {
    if (ref->isUnresolved()) {
      self()->setUnresolvedSnippet(new (cg->trHeapMemory())
          TR::UnresolvedDataSnippet(cg, rootLoadOrStore, rootLoadOrStore->getSymbolReference(), isStore, false));
  ...
  if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
    ...
    if (subTree->getOpCodeValue() == TR::aiadd || subTree->getOpCodeValue() == TR::iadd) {
      ...
      if (symbol->isStatic()) {
        if (ref->isUnresolved()) {
          self()->setUnresolvedSnippet(new (cg->trHeapMemory())
              TR::UnresolvedDataSnippet(cg, subTree, &_symbolReference, subTree->getOpCode().isStore(), false));
          cg->addSnippet(self()->getUnresolvedSnippet());
        } else {
          if (_baseRegister != NULL) {
  ...
  TR::Register* srcReg, TR::Node* srcTree, bool srcModifiable, TR::CodeGenerator* cg)
{
  ...
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    if (_indexRegister != NULL) {
      ...
      new (cg->trHeapMemory())
        TR::ARMTrg1Src2Instruction(ARMOp_add, srcTree, tempTargetRegister, _indexRegister, srcReg, cg);
    ...
    if (_indexRegister != NULL) {
      if (self()->isBaseModifiable())
        ...
        tempTargetRegister = cg->allocateRegister();
      new (cg->trHeapMemory())
        TR::ARMTrg1Src2Instruction(ARMOp_add, srcTree, tempTargetRegister, _baseRegister, _indexRegister, cg);
      ...
      if (self()->isBaseModifiable())
        ...
        tempTargetRegister = cg->allocateRegister();
      new (cg->trHeapMemory())
        TR::ARMTrg1Src2Instruction(ARMOp_add, srcTree, tempTargetRegister, _baseRegister, srcReg, cg);
      if (_baseRegister != tempTargetRegister) {
        self()->decNodeReferenceCounts();
        _baseNode = NULL;
      }
  ...
  if (self()->getUnresolvedSnippet()) {
    ...
    *wcursor = 0xEA000000; // insert a branch to the snippet
    ...
    if (index) {
      ...
      *wcursor = preserve;
      if (op == ARMOp_ldr || op == ARMOp_ldrb || op == ARMOp_str || op == ARMOp_strb || op == ARMOp_ldrsb
    ...
    if (!index && op != ARMOp_add) // common encoding for all true load/store instructions with immediate offsets
    {
      ...
      TR_ASSERT(
        !(self()->isImmediatePreIndexed() && self()->isPostIndexed()), "write back with post-index transfer redundant");
...
uint32_t OMR::ARM::MemoryReference::estimateBinaryLength(TR_ARMOpCodes op)
{
...
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
      if ((op == ARMOp_flds || op == ARMOp_fldd || op == ARMOp_fsts || op == ARMOp_fstd) && _baseRegister != NULL)
  ...
  } else if (op == ARMOp_ldr || op == ARMOp_str || op == ARMOp_ldrb || op == ARMOp_strb || op == ARMOp_ldrex
    || op == ARMOp_strex) {
  ...
  TR::Node* node, TR::SymbolReference* ref, TR::Register* reg, TR::MemoryReference* mr, TR::CodeGenerator* cg)
{
  ...
  if (isPicSite && !cg->comp()->compileRelocatableCode()
    ...
    loadAddressConstantInSnippet(cg, node ? node : cg->getCurrentEvaluationTreeTop()->getNode(), address,
      reg); // isStore ? TR::InstOpCode::Op_st : TR::InstOpCode::Op_load
    return;
  }

  addr = symbol->isStatic() ? (uintptr_t)symbol->getStaticSymbol()->getStaticAddress()
