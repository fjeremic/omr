...
OMR::ARM::MemoryReference::MemoryReference(TR::Register *br, TR::Register *ir,
                                           TR::CodeGenerator *cg)
    : _baseRegister(br), _baseNode(NULL), _indexRegister(ir), _indexNode(NULL),
      _modBase(NULL), _unresolvedSnippet(NULL),
      _symbolReference(cg->comp()->getSymRefTab()), _flag(0), _scale(0) {}

OMR::ARM::MemoryReference::MemoryReference(TR::Register *br, TR::Register *ir,
                                           uint8_t scale, TR::CodeGenerator *cg)
    : _baseRegister(br), _baseNode(NULL), _indexRegister(ir), _indexNode(NULL),
      _modBase(NULL), _unresolvedSnippet(NULL),
      _symbolReference(cg->comp()->getSymRefTab()), _flag(0), _scale(scale) {}

OMR::ARM::MemoryReference::MemoryReference(TR::Register *br, int32_t disp,
                                           TR::CodeGenerator *cg)
...
OMR::ARM::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
    ...
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _unresolvedSnippet(NULL),
      _symbolReference(cg->comp()->getSymRefTab()), _modBase(NULL),
      _length(len), _scale(0), _flag(0) {
  ...
  if (rootLoadOrStore->getOpCode().isIndirect()) {
    if (ref->isUnresolved()) {
      self()->setUnresolvedSnippet(
          new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
              cg, rootLoadOrStore, rootLoadOrStore->getSymbolReference(),
              isStore, false));
    ...
    if (symbol->isStatic()) {
      if (ref->isUnresolved()) {
        ...
        loadRelocatableConstant(rootLoadOrStore, ref, _baseRegister, self(),
                                cg);
      }
    } else {
      if (!symbol
  ...
  self()->addToOffset(rootLoadOrStore,
                      rootLoadOrStore->getSymbolReference()->getOffset(), cg);
  if (self()->getUnresolvedSnippet() != NULL)
    self()->adjustForResolution(cg);
  // TODO: aliasing sets?
}

OMR::ARM::MemoryReference::MemoryReference(TR::Node *node,
...
OMR::ARM::MemoryReference::MemoryReference(TR::MemoryReference &mr,
  ...
  if (mr.getUnresolvedSnippet() != NULL) {
    ...
    _unresolvedSnippet = new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
        cg, 0, &_symbolReference, mr.getUnresolvedSnippet()->resolveForStore(),
        false);
...
void OMR::ARM::MemoryReference::setSymbol(TR::Symbol *symbol,
                                          TR::CodeGenerator *cg) {
...
void OMR::ARM::MemoryReference::fixupVFPOffset(TR::Node *node,
                                               TR::CodeGenerator *cg) {
  if (self()->getUnresolvedSnippet() != NULL) {
    return;
  }
  ...
  if (!constantIsImmed10(displacement)) {
    ...
    int32_t remainingOffset =
        displacement - foldedOffset; // must be within -1024 to 1023
    ...
    if (constantIsImmed8r(foldedOffset, &immValue, &rotate)) {
      if (_baseRegister == NULL)
        generateTrg1ImmInstruction(cg, ARMOp_mov, node, newBase, immValue,
                                   rotate);
      else
        generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, newBase,
                                       _baseRegister, immValue, rotate);
    } else {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (base != 0) {
          if (!isFirstDone) {
            if (_baseRegister == NULL)
              generateTrg1ImmInstruction(cg, ARMOp_mov, node, newBase, base,
                                         shift);
            else
              generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, newBase,
                                             _baseRegister, base, shift);
            isFirstDone = true;
          } else
            generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, newBase,
                                           newBase, base, shift);
...
void OMR::ARM::MemoryReference::addToOffset(TR::Node *node, int32_t amount,
                                            TR::CodeGenerator *cg) {
  if (self()->getUnresolvedSnippet() != NULL) {
  ...
  if ((_length == 4 && !constantIsImmed12(displacement)) ||
    ...
    int32_t remainingOffset =
        displacement - foldedOffset; // must be within -128 to 127
...
void OMR::ARM::MemoryReference::incRegisterTotalUseCounts(
    TR::CodeGenerator *cg) {
  if (_baseRegister != NULL) {
...
void OMR::ARM::MemoryReference::populateMemoryReference(TR::Node *subTree,
                                                        TR::CodeGenerator *cg) {
    ...
    if (subTree->getOpCodeValue() == TR::aiadd ||
      ...
      if (symbol->isStatic()) {
        if (ref->isUnresolved()) {
          self()->setUnresolvedSnippet(
              new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
                  cg, subTree, &_symbolReference,
                  subTree->getOpCode().isStore(), false));
          cg->addSnippet(self()->getUnresolvedSnippet());
        } else {
          if (_baseRegister != NULL) {
            self()->addToOffset(
...
void OMR::ARM::MemoryReference::consolidateRegisters(TR::Register *srcReg,
                                                     TR::Node *srcTree,
                                                     bool srcModifiable,
                                                     TR::CodeGenerator *cg) {
  ...
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    if (_indexRegister != NULL) {
      ...
      new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
          ARMOp_add, srcTree, tempTargetRegister, _indexRegister, srcReg, cg);
    ...
    if (_indexRegister != NULL) {
      if (self()->isBaseModifiable())
        ...
        tempTargetRegister = cg->allocateRegister();
      new (cg->trHeapMemory())
          TR::ARMTrg1Src2Instruction(ARMOp_add, srcTree, tempTargetRegister,
      ...
      self()->setBaseModifiable();
               ...
               (self()->getOffset() != 0 || self()->hasDelayedOffset())) {
      if (self()->isBaseModifiable())
        tempTargetRegister = _baseRegister;
        ...
        tempTargetRegister = cg->allocateRegister();
      new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(
          ARMOp_add, srcTree, tempTargetRegister, _baseRegister, srcReg, cg);
      if (_baseRegister != tempTargetRegister) {
        self()->decNodeReferenceCounts();
        _baseNode = NULL;
      }
...
void OMR::ARM::MemoryReference::assignRegisters(
  ...
  if (_baseRegister != NULL) {
    ...
    if (assignedBaseRegister == NULL) {
      if (_baseRegister->getTotalUseCount() ==
          _baseRegister->getFutureUseCount()) {
        if ((assignedBaseRegister =
                 machine->findBestFreeRegister(TR_GPR, true, true)) == NULL) {
          assignedBaseRegister =
              machine->freeBestRegister(currentInstruction, TR_GPR, NULL, true);
        }
      } else {
        assignedBaseRegister = machine->reverseSpillState(
  ...
  if (_indexRegister != NULL) {
    ...
    if (assignedIndexRegister == NULL) {
      if (_indexRegister->getTotalUseCount() ==
          _indexRegister->getFutureUseCount()) {
        if ((assignedIndexRegister =
                 machine->findBestFreeRegister(TR_GPR, false, true)) == NULL) {
          assignedIndexRegister =
              machine->freeBestRegister(currentInstruction, TR_GPR);
        }
      } else {
        assignedIndexRegister =
            machine->reverseSpillState(currentInstruction, _indexRegister);
      }
  ...
  if (_modBase != NULL) {
    ...
    if (assignedModBaseRegister == NULL) {
      if (_modBase->getTotalUseCount() == _modBase->getFutureUseCount()) {
        if ((assignedModBaseRegister =
                 machine->findBestFreeRegister(TR_GPR, true, true)) == NULL) {
          assignedModBaseRegister =
              machine->freeBestRegister(currentInstruction, TR_GPR, NULL, true);
        }
      } else {
        assignedModBaseRegister = machine->reverseSpillState(
            currentInstruction, _modBase, NULL, true);
      }
...
uint8_t *OMR::ARM::MemoryReference::generateBinaryEncoding(
  ...
  TR::RealRegister *base = self()->getBaseRegister()
                               ? toRealRegister(self()->getBaseRegister())
                               : NULL;
  TR::RealRegister *index = self()->getIndexRegister()
  ...
  if (self()->getUnresolvedSnippet()) {
    ...
    cg->addRelocation(new (cg->trHeapMemory()) TR::LabelRelative24BitRelocation(
        cursor, self()->getUnresolvedSnippet()->getSnippetLabel()));
    *wcursor = 0xEA000000; // insert a branch to the snippet
    ...
    if (index) {
      ...
      *wcursor = preserve;
      if (op == ARMOp_ldr || op == ARMOp_ldrb || op == ARMOp_str ||
          op == ARMOp_strb || op == ARMOp_ldrsb || op == ARMOp_ldrh ||
          op == ARMOp_ldrsh || op == ARMOp_strh) {
        if (base) {
...
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
      else if (op == ARMOp_flds || op == ARMOp_fldd || op == ARMOp_fsts ||
               op == ARMOp_fstd) {
        if (base) {
        ...
        TR_ASSERT(op == ARMOp_mov,
                  "unknown opcode in an unresolved memory reference");
        mBase->setRegisterFieldRM(wcursor);
      }
      wcursor++;
      cursor += ARM_INSTRUCTION_LENGTH;
    ...
    if (!index && op != ARMOp_add) // common encoding for all true load/store
                                   // instructions with immediate offsets
    {
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
      if (!(op == ARMOp_flds || op == ARMOp_fldd || op == ARMOp_fsts ||
            op == ARMOp_fstd)) {
#endif
        base->setRegisterFieldRN(wcursor);
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
    ...
    if (op == ARMOp_ldr || op == ARMOp_str || op == ARMOp_ldrb ||
        op == ARMOp_strb) {
      // addressing mode 2
      if (index) {
        if (0 != _scale) {
        ...
        *wcursor |= 1 << 25; // set I bit for index register (yes, this bit has
                             // different bit positions as well as reverse
                             // semantics in modes 2 and 3!)
        *wcursor |= 1 << 23; // set U bit for +ve
      } else {
        if (constantIsImmed12(displacement)) {
          *wcursor |= displacement < 0 ? -displacement : displacement;
        } else {
          cursor = self()->encodeLargeARMConstant(wcursor, cursor,
                                                  currentInstruction, cg);
          wcursor = (uint32_t *)cursor;
        }
      }
    } else if (op == ARMOp_ldrsb || op == ARMOp_ldrh || op == ARMOp_ldrsh ||
               op == ARMOp_strh) // addressing mode 3
    {
      if (index) {
        base->setRegisterFieldRN(wcursor);
        index->setRegisterFieldRM(wcursor);
    ...
    } else if (op == ARMOp_add) // op is ARMOp_add for delayed offsets (see
                                // loadAddrEvaluator)
    {
      base->setRegisterFieldRN(wcursor);
      ...
      if (constantIsImmed8r(self()->getOffset(), &immBase, &rotate)) {
        ...
        *wcursor |= ((32 - rotate) << 7) &
                    0x00000f00; // mask to deal with rotate == 0 case.
        *wcursor |= immBase;
      } else {
        cursor = self()->encodeLargeARMConstant(wcursor, cursor,
                                                currentInstruction, cg);
    ...
    } else if (op == ARMOp_ldfd || op == ARMOp_stfd || op == ARMOp_ldfs ||
               op == ARMOp_stfs) {
      *wcursor |= (displacement >>
                   2); // offset for these coprocessor instructions is in words
    }
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
    else if (op == ARMOp_flds || op == ARMOp_fldd || op == ARMOp_fsts ||
             op == ARMOp_fstd) {
...
uint32_t OMR::ARM::MemoryReference::estimateBinaryLength(TR_ARMOpCodes op) {
  ...
  } else if (op == ARMOp_ldr || op == ARMOp_str || op == ARMOp_ldrb ||
             op == ARMOp_strb || op == ARMOp_ldrex || op == ARMOp_strex) {
...
uint8_t *OMR::ARM::MemoryReference::encodeLargeARMConstant(
    uint32_t *wcursor, uint8_t *cursor, TR::Instruction *currentInstruction,
    TR::CodeGenerator *cg) {
  bool spillNeeded;
...
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
  vfpInstruction = (op == ARMOp_flds || op == ARMOp_fldd || op == ARMOp_fsts ||
                    op == ARMOp_fstd);
#endif

  TR_ASSERT((op == ARMOp_add || op == ARMOp_ldr || op == ARMOp_ldrb ||
             op == ARMOp_str || op == ARMOp_strb || op == ARMOp_ldrsb ||
             op == ARMOp_ldrh || op == ARMOp_ldrsh || op == ARMOp_strh ||
             vfpInstruction),
            "unrecognized opcode");

  // if doing a ldr then can re-use the target register as the index register
  // (if it is not also the base register), stores must always spill.
  if (op == ARMOp_str || op == ARMOp_strh || op == ARMOp_strb ||
      vfpInstruction ||
      toRealRegister(currentInstruction->getMemoryDataRegister())
              ->getRegisterNumber() == base->getRegisterNumber()) {
    spillNeeded = true;
    rX = cg->machine()->getRealRegister(choose_rX(currentInstruction, base));
    stackPtr = cg->machine()->getRealRegister(
        cg->getLinkage()->getProperties().getStackPointerRegister());
  } else {
    spillNeeded = false;
  ...
  cursor = armLoadConstantInstructions(rX, offset < 0 ? -offset : offset,
                                       cursor, wcursor, cg);
  ...
  if (!vfpInstruction) {
    ...
    *wcursor = 0xE0800000;
    ...
    cursor += ARM_INSTRUCTION_LENGTH;
    *wcursor = preserve;
    rX->setRegisterFieldRN(wcursor); // modify the original instruction to use
                                     // rX as the base register
  }
...
static uint8_t *armLoadConstantInstructions(TR::RealRegister *trgReg,
  ...
  // if these large delayed constants start showing up more frequently it would
  // be worth using similar logic as in armLoadConstant for now just
  // pessimistically generate four instructions
...
static void loadRelocatableConstant(TR::Node *node, TR::SymbolReference *ref,
  ...
  TR_ASSERT(symbol->isStatic() || symbol->isMethod(),
            "loadRelocatableConstant, problem with new symbol hierarchy");
  ...
  if (isPicSite && !cg->comp()->compileRelocatableCode() &&
    ...
    loadAddressConstantInSnippet(
        ...
        reg); // isStore ? TR::InstOpCode::Op_st : TR::InstOpCode::Op_load
    return;
  }

  addr = symbol->isStatic()
             ? (uintptr_t)symbol->getStaticSymbol()->getStaticAddress()
             : (uintptr_t)symbol->getMethodSymbol()->getMethodAddress();

  if (symbol->isStartPC()) {
    generateLoadStartPCInstruction(cg, node, reg, ref);
    /* The constant values do not mean anything here. They will be replaced in
  ...
  if (ref->isUnresolved() || comp->compileRelocatableCode()) {
    ...
    if (symbol->isCountForRecompile()) {
      loadAddressConstant(cg, GCRnode, TR_CountForRecompile, reg, NULL, false,
                          TR_GlobalValue);
    } else if (symbol->isRecompilationCounter()) {
      loadAddressConstant(cg, GCRnode, 1, reg, NULL, false,
      ...
      cg->addSnippet(mr->setUnresolvedSnippet(
          new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
              cg, node, ref, node->getOpCode().isStore(), false)));
    }
  } else {
