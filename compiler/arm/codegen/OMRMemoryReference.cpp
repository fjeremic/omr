...
void OMR::ARM::MemoryReference::populateMemoryReference(TR::Node* subTree, TR::CodeGenerator* cg)
{
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
        ...
        } else if (subTree->getOpCodeValue() == TR::ishl && subTree->getFirstChild()->getOpCode().isLoadConst() && subTree->getSecondChild()->getOpCode().isLoadConst()) {
    ...
    if (self()->getUnresolvedSnippet()) {
        ...
        *wcursor = 0xEA000000; // insert a branch to the snippet
        ...
        if (index) {
            ...
            *wcursor = preserve;
            if (op == ARMOp_ldr || op == ARMOp_ldrb || op == ARMOp_str || op == ARMOp_strb || op == ARMOp_ldrsb || op == ARMOp_ldrh || op == ARMOp_ldrsh || op == ARMOp_strh) {
                if (base) {
...
uint8_t* OMR::ARM::MemoryReference::encodeLargeARMConstant(uint32_t* wcursor, uint8_t* cursor, TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    ...
    TR_ASSERT((op == ARMOp_add || op == ARMOp_ldr || op == ARMOp_ldrb || op == ARMOp_str || op == ARMOp_strb || op == ARMOp_ldrsb || op == ARMOp_ldrh || op == ARMOp_ldrsh || op == ARMOp_strh || vfpInstruction),
        "unrecognized opcode");
    ...
    if (op == ARMOp_str || op == ARMOp_strh || op == ARMOp_strb || vfpInstruction || toRealRegister(currentInstruction->getMemoryDataRegister())->getRegisterNumber() == base->getRegisterNumber()) {
        spillNeeded = true;
...
static TR::RealRegister::RegNum choose_rX(TR::Instruction* currentInstruction, TR::RealRegister* base)
{
    ...
    int32_t exclude = ((TSnum == TR::RealRegister::gr3) ? 1 : 0) | ((TSnum == TR::RealRegister::gr2) ? 2 : 0) | ((basenum == TR::RealRegister::gr3) ? 1 : 0) | ((basenum == TR::RealRegister::gr2) ? 2 : 0);
