...
static void
loadRelocatableConstant(TR::Node* node,
...
static uint8_t*
armLoadConstantInstructions(TR::RealRegister* trgReg,
                            int32_t value,
                            uint8_t* cursor,
                            uint32_t* wcursor,
                            TR::CodeGenerator* cg);
...
OMR::ARM::MemoryReference::MemoryReference(TR::Register* br,
                                           TR::Register* ir,
                                           TR::CodeGenerator* cg)
  : _baseRegister(br)
  , _baseNode(NULL)
  ...
  , _flag(0)
{
  ...
  if (rootLoadOrStore->getOpCode().isIndirect()) {
    ...
    if (symbol->isStatic()) {
      if (ref->isUnresolved()) {
        ...
        loadRelocatableConstant(
          rootLoadOrStore, ref, _baseRegister, self(), cg);
      }
    } else {
      if (!symbol
  ...
  self()->addToOffset(
    rootLoadOrStore, rootLoadOrStore->getSymbolReference()->getOffset(), cg);
  ...
  if (mr.getUnresolvedSnippet() != NULL) {
    ...
    _unresolvedSnippet = new (cg->trHeapMemory())
      TR::UnresolvedDataSnippet(cg,
                                0,
                                &_symbolReference,
                                mr.getUnresolvedSnippet()->resolveForStore(),
                                false);
...
void
OMR::ARM::MemoryReference::setSymbol(TR::Symbol* symbol, TR::CodeGenerator* cg)
{
...
void
OMR::ARM::MemoryReference::fixupVFPOffset(TR::Node* node, TR::CodeGenerator* cg)
{
  if (self()->getUnresolvedSnippet() != NULL) {
    return;
  }
    ...
    if (constantIsImmed8r(foldedOffset, &immValue, &rotate)) {
      if (_baseRegister == NULL)
        generateTrg1ImmInstruction(
          cg, ARMOp_mov, node, newBase, immValue, rotate);
      else
        generateTrg1Src1ImmInstruction(
          cg, ARMOp_add, node, newBase, _baseRegister, immValue, rotate);
    } else {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (base != 0) {
          if (!isFirstDone) {
            if (_baseRegister == NULL)
              generateTrg1ImmInstruction(
                cg, ARMOp_mov, node, newBase, base, shift);
            else
              generateTrg1Src1ImmInstruction(
                cg, ARMOp_add, node, newBase, _baseRegister, base, shift);
            isFirstDone = true;
          } else
            generateTrg1Src1ImmInstruction(
              cg, ARMOp_add, node, newBase, newBase, base, shift);
        }
      }
    }
...
void
OMR::ARM::MemoryReference::addToOffset(TR::Node* node,
                                       int32_t amount,
                                       TR::CodeGenerator* cg)
{
  if (self()->getUnresolvedSnippet() != NULL) {
...
void
OMR::ARM::MemoryReference::adjustForResolution(TR::CodeGenerator* cg)
{
  _modBase = cg->allocateRegister();
}

void
OMR::ARM::MemoryReference::decNodeReferenceCounts()
{
...
void
OMR::ARM::MemoryReference::incRegisterTotalUseCounts(TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
...
void
OMR::ARM::MemoryReference::populateMemoryReference(TR::Node* subTree,
                                                   TR::CodeGenerator* cg)
{
...
void
OMR::ARM::MemoryReference::consolidateRegisters(TR::Register* srcReg,
                                                TR::Node* srcTree,
                                                bool srcModifiable,
                                                TR::CodeGenerator* cg)
{
  ...
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    if (_indexRegister != NULL) {
      if (self()->isBaseModifiable())
        ...
        tempTargetRegister = cg->allocateRegister();
      new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(ARMOp_add,
                                                          srcTree,
                                                          tempTargetRegister,
                                                          _baseRegister,
                                                          _indexRegister,
...
void
OMR::ARM::MemoryReference::assignRegisters(TR::Instruction* currentInstruction,
                                           TR::CodeGenerator* cg)
{
  ...
  if (_modBase != NULL) {
    ...
    if (assignedModBaseRegister == NULL) {
      if (_modBase->getTotalUseCount() == _modBase->getFutureUseCount()) {
        ...
        assignedModBaseRegister =
          machine->reverseSpillState(currentInstruction, _modBase, NULL, true);
      }
...
uint8_t*
OMR::ARM::MemoryReference::generateBinaryEncoding(
        ...
        *wcursor |= 1 << 23; // set U bit for +ve
        ...
        if (constantIsImmed12(displacement)) {
          *wcursor |= displacement < 0 ? -displacement : displacement;
          ...
          cursor = self()->encodeLargeARMConstant(
            wcursor, cursor, currentInstruction, cg);
...
uint32_t
OMR::ARM::MemoryReference::estimateBinaryLength(TR_ARMOpCodes op)
{
  if (self()->getUnresolvedSnippet() != NULL) {
    if (_indexRegister != NULL) {
...
uint8_t*
OMR::ARM::MemoryReference::encodeLargeARMConstant(
  uint32_t* wcursor,
  uint8_t* cursor,
  TR::Instruction* currentInstruction,
  TR::CodeGenerator* cg)
{
  ...
  cursor = armLoadConstantInstructions(
    rX, offset < 0 ? -offset : offset, cursor, wcursor, cg);
                        ...
                        TR::CodeGenerator* cg)
{
  ...
  if (ref->isUnresolved() || comp->compileRelocatableCode()) {
    ...
    if (symbol->isCountForRecompile()) {
      loadAddressConstant(
        cg, GCRnode, TR_CountForRecompile, reg, NULL, false, TR_GlobalValue);
    } else if (symbol->isRecompilationCounter()) {
      loadAddressConstant(
        cg, GCRnode, 1, reg, NULL, false, TR_BodyInfoAddressLoad);
    } else if (symbol->isCompiledMethod()) {
      loadAddressConstant(
        cg, GCRnode, 1, reg, NULL, false, TR_RamMethodSequence);
