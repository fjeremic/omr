...
public:
  ...
  TR_ARMOperand2()
      : _opType(ARMOp2Illegal), _baseRegister(0), _shiftOrImmed(0),
        _immedShift(0), _shiftRegister(0) {}

  // immediates
  TR_ARMOperand2(uint32_t immed8, uint32_t shiftAmount)
      : _opType(ARMOp2Immed8r), _baseRegister(0), _shiftOrImmed(immed8),
        _immedShift(shiftAmount), _shiftRegister(0) {}

  // reg or regRRX
  TR_ARMOperand2(TR_ARMOperand2Type opType, TR::Register *dst)
      : _opType(opType), _baseRegister(dst), _shiftOrImmed(0), _immedShift(0),
        _shiftRegister(0) {}

  // reg SHIFTOP immed
  TR_ARMOperand2(TR_ARMOperand2Type opType, TR::Register *dst, uint32_t immed5)
      : _opType(opType), _baseRegister(dst), _shiftOrImmed(immed5),
        _immedShift(0), _shiftRegister(0) {}

  // reg SHIFTOP reg
  TR_ARMOperand2(TR_ARMOperand2Type opType, TR::Register *dst,
                 TR::Register *shiftReg)
      : _opType(opType), _baseRegister(dst), _shiftOrImmed(0),
        _shiftRegister(shiftReg), _immedShift(0) {}
