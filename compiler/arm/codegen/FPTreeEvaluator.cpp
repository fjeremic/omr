...
static TR::Register* singlePrecisionEvaluator(TR::Node* node, TR_ARMOpCodes opCode, TR::CodeGenerator* cg)
{
  ...
  if (node->getNumChildren() == 2) {
    ...
    generateTrg1Src2Instruction(
      cg, opCode, node, floatTrgReg, moveToFloatRegister(node, src1Reg, cg), moveToFloatRegister(node, src2Reg, cg));
  } else // 1 child
  {
...
static TR::Register* doublePrecisionEvaluator(TR::Node* node, TR_ARMOpCodes opCode, TR::CodeGenerator* cg)
{
  ...
  if (node->getNumChildren() == 2) {
    ...
    generateTrg1Src2Instruction(
      cg, opCode, node, doubleTrgReg, moveToDoubleRegister(node, src1Reg, cg), moveToDoubleRegister(node, src2Reg, cg));
  } else // 1 child
  {
...
static TR::Register* callLong2DoubleHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlong2Double, false, false, false)->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlong2Double, false, false, false));
...
static TR::Register* callLong2FloatHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlong2Float, false, false, false)->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMlong2Float, false, false, false));
...
static TR::Register* callDouble2LongHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMdouble2Long, false, false, false)->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMdouble2Long, false, false, false));
...
static TR::Register* callFloat2LongHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMfloat2Long, false, false, false)->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMfloat2Long, false, false, false));
...
static TR::Register* callDoubleRemainderHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()
      ...
      ->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMdoubleRemainder, false, false, false));
...
static TR::Register* callFloatRemainderHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateImmSymInstruction(cg, ARMOp_bl, node,
    (uintptr_t)cg->symRefTab()
      ...
      ->getMethodAddress(),
    dependencies, cg->symRefTab()->findOrCreateRuntimeHelper(TR_ARMfloatRemainder, false, false, false));
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
    ...
    if (noFPRA) {
      ...
      if (tempMR->getIndexRegister() && tempMR->getBaseRegister()) {
        ...
        generateTrg1Src2Instruction(
          cg, ARMOp_add, node, tempReg, tempMR->getBaseRegister(), tempMR->getIndexRegister());
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
    ...
    if (noFPRA) {
      ...
      highMem = new (cg->trHeapMemory())
        TR::MemoryReference(*tempMR, (TR::Compiler->target.cpu.isBigEndian()) ? 0 : 4, 4, cg);
      generateTrg1MemInstruction(cg, ARMOp_ldr, node, highReg, highMem);
      lowMem = new (cg->trHeapMemory())
        TR::MemoryReference(*tempMR, (TR::Compiler->target.cpu.isBigEndian()) ? 4 : 0, 4, cg);
      ...
      if (highMem->getIndexRegister() && highMem->getBaseRegister()) {
        TR::Register* tempReg = cg->allocateRegister();
        generateTrg1Src2Instruction(
          cg, ARMOp_add, node, tempReg, highMem->getBaseRegister(), highMem->getIndexRegister());
        highMem->decNodeReferenceCounts();
...
TR::Register* OMR::ARM::TreeEvaluator::dbits2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
    ...
    highReg = cg->allocateRegister();
    highMem
      = new (cg->trHeapMemory()) TR::MemoryReference(*tempMR, (TR::Compiler->target.cpu.isBigEndian()) ? 0 : 4, 4, cg);
    generateTrg1MemInstruction(cg, ARMOp_ldr, node, highReg, highMem);
    lowMem
      = new (cg->trHeapMemory()) TR::MemoryReference(*tempMR, (TR::Compiler->target.cpu.isBigEndian()) ? 4 : 0, 4, cg);
    generateTrg1MemInstruction(cg, ARMOp_ldr, node, lowReg, lowMem);
...
//#define STAYINFP
  ...
  if (node->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP()
...
static TR::Register* generateFusedMultiplyAdd(TR::Node* addNode, TR_ARMOpCodes OpCode, TR::CodeGenerator* cg)
{
  ...
  TR_ASSERT(
    mulNode->getReferenceCount() < 2, "Mul node 0x%p reference count %d >= 2\n", mulNode, mulNode->getReferenceCount());
  ...
  if (addNode->getDataType() == TR::Float) {
    if (noFPRA) {
      TR::Register* floatTrgReg = cg->allocateSinglePrecisionRegister();
      generateTrg1Src2Instruction(cg, OpCode, addNode, floatTrgReg, moveToFloatRegister(addNode, source1Register, cg),
    ...
    if (noFPRA) {
      TR::Register* doubleTrgReg = cg->allocateRegister(TR_FPR);
      generateTrg1Src2Instruction(cg, OpCode, addNode, doubleTrgReg, moveToDoubleRegister(addNode, source1Register, cg),
        moveToDoubleRegister(addNode, source2Register, cg));
      generateTrg2Src1Instruction(
        cg, ARMOp_fmrrd, addNode, trgRegister->getLowOrder(), trgRegister->getHighOrder(), doubleTrgReg);
      cg->stopUsingRegister(doubleTrgReg);
    } else {
      generateTrg1Src2Instruction(cg, OpCode, addNode, trgRegister, source1Register, source2Register);
    }
...
TR::Register* OMR::ARM::TreeEvaluator::fnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && isAdd) {
    ...
    if (((isFPStrictMul(firstChild->getFirstChild(), comp) && firstChild->getSecondChild()->getReferenceCount() == 1)
    ...
    if (noFPRA) {
      generateTrg1Src2Instruction(cg, ARMOp_fnmuls, node, floatTrgReg, moveToFloatRegister(node, src1Register, cg),
...
TR::Register* OMR::ARM::TreeEvaluator::dnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && isAdd) {
    ...
    TR::Register* src2Register = cg->evaluate(firstChild->getSecondChild());
    if (noFPRA) {
      generateTrg1Src2Instruction(cg, ARMOp_fnmuld, node, doubleTrgReg, moveToDoubleRegister(node, src1Register, cg),
...
TR::Register* OMR::ARM::TreeEvaluator::i2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR_ARMOpCodes opcode = (node->getOpCodeValue() != TR::iu2d && node->getOpCodeValue() != TR::su2d)
    ? ARMOp_fsitod
...
static void ifFloatEvaluator(
  TR::Node* node, TR_ARMOpCodes opCode, TR_ARMConditionCode cond, TR::CodeGenerator* cg, bool isDouble, bool unordered)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
...
static TR::Register* setboolFloatEvaluator(
  TR::Node* node, TR_ARMOpCodes opCode, TR_ARMConditionCode cond, TR::CodeGenerator* cg, bool isDouble, bool unordered)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::ARM::TreeEvaluator::ifdcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  ifFloatEvaluator(node, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, unordered ? ARMConditionCodeLT : ARMConditionCodeCC, cg,
    isDouble, false);
  return NULL;
}
...
TR::Register* OMR::ARM::TreeEvaluator::ifdcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  ifFloatEvaluator(node, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, unordered ? ARMConditionCodeCS : ARMConditionCodeGE, cg,
    isDouble, false);
  return NULL;
}
...
TR::Register* OMR::ARM::TreeEvaluator::ifdcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  ifFloatEvaluator(node, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, unordered ? ARMConditionCodeHI : ARMConditionCodeGT, cg,
    isDouble, false);
  return NULL;
}
...
TR::Register* OMR::ARM::TreeEvaluator::ifdcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  ifFloatEvaluator(node, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, unordered ? ARMConditionCodeLE : ARMConditionCodeLS, cg,
    isDouble, false);
  return NULL;
}
...
TR::Register* OMR::ARM::TreeEvaluator::dcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (noFPRA) {
    if (isDouble) {
      generateSrc2Instruction(cg, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, node, moveToDoubleRegister(node, src1Reg, cg),
        moveToDoubleRegister(node, src2Reg, cg));
    } else {
      generateSrc2Instruction(cg, isDouble ? ARMOp_fcmpd : ARMOp_fcmps, node, moveToFloatRegister(node, src1Reg, cg),
...
static TR::Register* generateFloatMaxMin(TR::Node* node, TR::CodeGenerator* cg, bool max)
{
  ...
  if (noFPRA && childReg->getKind() == TR_GPR) {
    ...
    if (!max) {
      childReg
        = (isFloat) ? moveFromFloatRegister(node, childFloatReg, cg) : moveFromDoubleRegister(node, childFloatReg, cg);
    }
  }
  ...
  cursor
    = generateTrg1Src1Instruction(cg, (isFloat) ? ARMOp_fcpys : ARMOp_fcpyd, node, trgFloatReg, childFloatReg, cursor);
  cursor->setConditionCode(ARMConditionCodeVS);
  ...
  if (isFloat) {
    ...
    cursor = generateSrc1ImmInstruction(
      cg, ARMOp_cmp, node, max ? trgReg->getHighOrder() : childReg->getHighOrder(), 0x80, 24, cursor); // 0x80000000
    cursor = generateSrc1ImmInstruction(
      cg, ARMOp_cmp, node, max ? trgReg->getLowOrder() : childReg->getLowOrder(), 0, 0, cursor);
   ...
   */
  cursor = generateSrc1ImmInstruction(
    cg, (isFloat) ? ARMOp_fcmpzs : ARMOp_fcmpzd, node, max ? childFloatReg : trgFloatReg, 0, 0, cursor); // Dummy 0
  cursor->setConditionCode(ARMConditionCodeEQ);
  cursor = generateInstruction(cg, ARMOp_fmstat, node, cursor);
  cursor->setConditionCode(ARMConditionCodeEQ);
  cursor
    ...
    if (cg->canClobberNodesRegister(firstChild)) {
      if (isFloat) {
        ...
        cursor = generateTrg2Src1Instruction(
          cg, ARMOp_fmrrd, node, reg->getLowOrder(), reg->getHighOrder(), trgFloatReg, cursor);
      }
      trgReg = reg;
    } else {
