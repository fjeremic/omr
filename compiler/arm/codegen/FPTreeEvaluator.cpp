...
TR::Register* OMR::ARM::TreeEvaluator::faddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (((isFPStrictMul(node->getFirstChild(), comp) && (node->getSecondChild()->getReferenceCount() == 1)) || (isFPStrictMul(node->getSecondChild(), comp) && (node->getFirstChild()->getReferenceCount() == 1))) && performTransformation(comp, "O^O Changing [%p] to fmacs\n", node)) {
...
TR::Register* OMR::ARM::TreeEvaluator::daddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* result = NULL;
    TR::Compilation* comp = cg->comp();
    if (((isFPStrictMul(node->getFirstChild(), comp) && (node->getSecondChild()->getReferenceCount() == 1)) || (isFPStrictMul(node->getSecondChild(), comp) && (node->getFirstChild()->getReferenceCount() == 1))) && performTransformation(comp, "O^O Changing [%p] to fmacd\n", node)) {
...
TR::Register* OMR::ARM::TreeEvaluator::dsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Compilation* comp = cg->comp();
    TR::Register* result = NULL;
    if (isFPStrictMul(node->getFirstChild(), comp) && (node->getSecondChild()->getReferenceCount() == 1) && performTransformation(comp, "O^O Changing [%p] to fmscd\n", node)) {
        result = generateFusedMultiplyAdd(node, ARMOp_fmscd, cg);
    } else if (isFPStrictMul(node->getSecondChild(), comp) && (node->getFirstChild()->getReferenceCount() == 1) && performTransformation(comp, "O^O Changing [%p] to fnmacd\n", node)) {
...
TR::Register* OMR::ARM::TreeEvaluator::fsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Compilation* comp = cg->comp();
    TR::Register* result = NULL;
    if (isFPStrictMul(node->getFirstChild(), comp) && (node->getSecondChild()->getReferenceCount() == 1) && performTransformation(comp, "O^O Changing [%p] to fmscs\n", node)) {
        result = generateFusedMultiplyAdd(node, ARMOp_fmscs, cg);
    } else if (isFPStrictMul(node->getSecondChild(), comp) && (node->getFirstChild()->getReferenceCount() == 1) && performTransformation(comp, "O^O Changing [%p] to fnmacs\n", node)) {
...
TR::Register* OMR::ARM::TreeEvaluator::fnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && isAdd) {
        ...
        if (((isFPStrictMul(firstChild->getFirstChild(), comp) && firstChild->getSecondChild()->getReferenceCount() == 1) || (isFPStrictMul(firstChild->getSecondChild(), comp) && firstChild->getFirstChild()->getReferenceCount() == 1)) && performTransformation(comp, "O^O Changing [%p] to fnmscs\n", node)) {
    ...
    } else if (isFPStrictMul(firstChild, comp) && firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && performTransformation(comp, "O^O Changing [%p] to fnmuls\n", node)) {
...
TR::Register* OMR::ARM::TreeEvaluator::dnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && isAdd) {
        ...
        if (((isFPStrictMul(firstChild->getFirstChild(), comp) && firstChild->getSecondChild()->getReferenceCount() == 1) || (isFPStrictMul(firstChild->getSecondChild(), comp) && firstChild->getFirstChild()->getReferenceCount() == 1)) && performTransformation(comp, "O^O Changing [%p] to fnmscd\n", node)) {
            result = generateFusedMultiplyAdd(node, ARMOp_fnmscd, cg);
            firstChild->unsetRegister(); //unset as the first child isn't the result node
        } else {
            result = doublePrecisionEvaluator(node, ARMOp_fnegd, cg);
        }
    } else if (isFPStrictMul(firstChild, comp) && firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && performTransformation(comp, "O^O Changing [%p] to fnmuld\n", node)) {
        // fneg(node) -> fmul(firstChild)
        TR::Register* doubleTrgReg = cg->allocateRegister(TR_FPR);
        TR::Register* src1Register = cg->evaluate(firstChild->getFirstChild());
        TR::Register* src2Register = cg->evaluate(firstChild->getSecondChild());
...
TR::Register* OMR::ARM::TreeEvaluator::i2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && (firstChild->getOpCodeValue() == TR::iload || firstChild->getOpCodeValue() == TR::iloadi) && (firstChild->getNumChildren() > 0) && (firstChild->getFirstChild()->getNumChildren() == 1) && !(firstChild->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
...
TR::Register* OMR::ARM::TreeEvaluator::f2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && (firstChild->getOpCodeValue() == TR::fload || firstChild->getOpCodeValue() == TR::floadi) && (firstChild->getNumChildren() > 0) && (firstChild->getFirstChild()->getNumChildren() == 1) && !(firstChild->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
        // Coming from memory, last use
        TR::MemoryReference* tempMR = new (cg->trHeapMemory()) TR::MemoryReference(firstChild, 4, cg);
        TR::Register* tempReg = cg->allocateSinglePrecisionRegister();
...
TR::Register* OMR::ARM::TreeEvaluator::d2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && (firstChild->getOpCodeValue() == TR::dload || firstChild->getOpCodeValue() == TR::dloadi) && (firstChild->getNumChildren() > 0) && (firstChild->getFirstChild()->getNumChildren() == 1) && !(firstChild->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
        // Coming from memory, last use
        TR::MemoryReference* tempMR = new (cg->trHeapMemory()) TR::MemoryReference(firstChild, 4, cg);
