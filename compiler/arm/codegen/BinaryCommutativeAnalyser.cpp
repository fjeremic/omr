...
void TR_ARMBinaryCommutativeAnalyser::genericAnalyser(
    TR::Node *root, TR_ARMOpCodes regToRegOpCode, TR_ARMOpCodes copyOpCode,
  ...
  if (getOpReg1Reg2()) {
    //      new TR::ARMRegRegInstruction(regToRegOpCode, firstRegister,
    //      secondRegister); root->setRegister(firstRegister);
  } else if (getOpReg2Reg1()) {
    //      new TR::ARMRegRegInstruction(regToRegOpCode, secondRegister,
    //      firstRegister); root->setRegister(secondRegister);
    //      notReversedOperands();
  } else if (getCopyReg1()) {
    //      TR::Register *tempReg = root->setRegister(cg()->allocateRegister());
    //      new TR::ARMRegRegInstruction(copyOpCode, tempReg, firstRegister);
    //      new TR::ARMRegRegInstruction(regToRegOpCode, tempReg,
    //      secondRegister);
  } else if (getCopyReg2()) {
    //      TR::Register *tempReg = root->setRegister(cg()->allocateRegister());
    //      new TR::ARMRegRegInstruction(copyOpCode, tempReg, secondRegister);
    //      new TR::ARMRegRegInstruction(regToRegOpCode, tempReg,
    //      firstRegister); notReversedOperands();
  } else if (getOpReg1Mem2()) {
    //      TR_ARMMemoryReference *tempMR = new
    //      TR_ARMMemoryReference(secondChild); new
    //      TR::ARMRegMemInstruction(memToRegOpCode, firstRegister, tempMR);
    //      tempMR->decNodeReferenceCounts();
    //      root->setRegister(firstRegister);
  } else {
    //      TR_ARMMemoryReference *tempMR = new
    //      TR_ARMMemoryReference(firstChild); new
    //      TR::ARMRegMemInstruction(memToRegOpCode, secondRegister, tempMR);
    //      tempMR->decNodeReferenceCounts();
...
void TR_ARMBinaryCommutativeAnalyser::genericLongAnalyser(
    TR::Node *root, TR_ARMOpCodes lowRegToRegOpCode,
    TR_ARMOpCodes highRegToRegOpCode, TR_ARMOpCodes lowMemToRegOpCode,
    TR_ARMOpCodes highMemToRegOpCode, TR_ARMOpCodes copyOpCode) {
  TR::Node *firstChild;
  TR::Node *secondChild;
  if (cg()->whichChildToEvaluate(root) == 0) {
    firstChild = root->getFirstChild();
  ...
  if (getOpReg1Reg2()) {
    ...
    //      TR::Register *tempReg =
    //      root->setRegister(cg()->allocateRegisterPair()); new
    //      TR::ARMRegRegInstruction(copyOpCode,
    //                                  tempReg->getLowOrder(),
    //                                  firstRegister->getLowOrder());
    ...
    //      TR_ARMMemoryReference *lowMR  = new
    //      TR_ARMMemoryReference(secondChild); TR_ARMMemoryReference *highMR =
    //      new TR_ARMMemoryReference(*lowMR, 4); new
    //      TR::ARMRegMemInstruction(lowMemToRegOpCode,
    //                                  firstRegister->getLowOrder(),
    //                                  lowMR);
    ...
    //      TR_ARMMemoryReference *lowMR  = new
    //      TR_ARMMemoryReference(firstChild); TR_ARMMemoryReference *highMR =
    //      new TR_ARMMemoryReference(*lowMR, 4); new
    //      TR::ARMRegMemInstruction(lowMemToRegOpCode,
    //                                  secondRegister->getLowOrder(),
    //                                  lowMR);
...
void TR_ARMBinaryCommutativeAnalyser::integerAddAnalyser(
    TR::Node *root, TR_ARMOpCodes regToRegOpCode) {
  TR::Node *firstChild;
  TR::Node *secondChild;
  if (cg()->whichChildToEvaluate(root) == 0) {
    firstChild = root->getFirstChild();
...
void TR_ARMBinaryCommutativeAnalyser::longAddAnalyser(TR::Node *root) {
  ...
  if (getOpReg1Reg2()) {
    //      new TR::ARMRegRegInstruction(ADD4RegReg,
    //      firstRegister->getLowOrder(), secondRegister->getLowOrder()); new
    //      TR::ARMRegRegInstruction(ADC4RegReg, firstRegister->getHighOrder(),
    //      secondRegister->getHighOrder()); root->setRegister(firstRegister);
  } else if (getOpReg2Reg1()) {
    //      new TR::ARMRegRegInstruction(ADD4RegReg,
    //      secondRegister->getLowOrder(), firstRegister->getLowOrder()); new
    //      TR::ARMRegRegInstruction(ADC4RegReg, secondRegister->getHighOrder(),
    //      firstRegister->getHighOrder()); root->setRegister(secondRegister);
    //      notReversedOperands();
  } else if (getCopyRegs()) {
    //      TR::Register     *lowRegister  = cg()->allocateRegister();
    //      TR::Register     *highRegister = cg()->allocateRegister();
    //      TR_RegisterPair *tempReg      =
    //      cg()->allocateRegisterPair(lowRegister, highRegister);
    //      root->setRegister(tempReg);
    //      new TR::ARMRegRegInstruction(MOV4RegReg, lowRegister,
    //      firstRegister->getLowOrder()); new
    //      TR::ARMRegRegInstruction(MOV4RegReg, highRegister,
    //      firstRegister->getHighOrder()); new
    //      TR::ARMRegRegInstruction(ADD4RegReg, tempReg->getLowOrder(),
    //      secondRegister->getLowOrder()); new
    //      TR::ARMRegRegInstruction(ADC4RegReg, tempReg->getHighOrder(),
    //      secondRegister->getHighOrder()); root->setRegister(tempReg);
  } else if (getOpReg1Mem2()) {
    //      TR_ARMMemoryReference *lowMR  = new
    //      TR_ARMMemoryReference(secondChild); TR_ARMMemoryReference *highMR =
    //      new TR_ARMMemoryReference(*lowMR, 4); new
    //      TR::ARMRegMemInstruction(ADD4RegMem, firstRegister->getLowOrder(),
    //      lowMR); new TR::ARMRegMemInstruction(ADC4RegMem,
    //      firstRegister->getHighOrder(), highMR);
    //      lowMR->decNodeReferenceCounts();
    //      root->setRegister(firstRegister);
  } else {
    //      TR_ARMMemoryReference *lowMR  = new
    //      TR_ARMMemoryReference(firstChild); TR_ARMMemoryReference *highMR =
    //      new TR_ARMMemoryReference(*lowMR, 4); new
    //      TR::ARMRegMemInstruction(ADD4RegMem, secondRegister->getLowOrder(),
    //      lowMR); new TR::ARMRegMemInstruction(ADC4RegMem,
    //      secondRegister->getHighOrder(), highMR);
    //      lowMR->decNodeReferenceCounts();
    //      root->setRegister(secondRegister);
    //      notReversedOperands();
  }
