...
void
TR_ARMOperand2::incTotalUseCount()
{
  switch (_opType) {
...
void
TR_ARMOperand2::setBinaryEncoding(uint32_t* instruction)
{
...
bool
TR_ARMOperand2::refsRegister(TR::Register* reg)
{
  switch (_opType) {
    case ARMOp2RegLSLReg:
    case ARMOp2RegLSRReg:
...
void
TR_ARMOperand2::decFutureUseCountAndAdjustRegState()
{
  switch (_opType) {
    case ARMOp2RegLSLReg:
    case ARMOp2RegLSRReg:
    ...
    case ARMOp2RegRORReg:
      if (_shiftRegister->decFutureUseCount() == 0) {
        _shiftRegister->getAssignedRealRegister()->setState(
    ...
    case ARMOp2RegRRX:
      if (_baseRegister->decFutureUseCount() == 0) {
        _baseRegister->getAssignedRealRegister()->setState(
          TR::RealRegister::Unlatched);
...
TR_ARMOperand2*
TR_ARMOperand2::assignRegisters(TR::Instruction* currentInstruction,
                                TR::CodeGenerator* cg)
{
  switch (_opType) {
    case ARMOp2RegLSLReg:
    ...
    case ARMOp2RegRORReg:
      _baseRegister->block();
      _shiftRegister =
        cg->machine()->assignSingleRegister(_shiftRegister, currentInstruction);
      _baseRegister->unblock();

      _shiftRegister->block();
      _baseRegister =
        cg->machine()->assignSingleRegister(_baseRegister, currentInstruction);
      _shiftRegister->unblock();
      break;
