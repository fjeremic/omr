...
uint8_t *storeArgumentItem(TR_ARMOpCodes op, uint8_t *buffer,
                           TR::RealRegister *reg, int32_t offset,
                           TR::CodeGenerator *cg) {
  TR::RealRegister *stackPtr = cg->machine()->getRealRegister(
...
uint8_t *loadArgumentItem(TR_ARMOpCodes op, uint8_t *buffer,
                          TR::RealRegister *reg, int32_t offset,
                          TR::CodeGenerator *cg) {
...
uint8_t *TR::ARMStackCheckFailureSnippet::emitSnippetBody() {
  ...
  TR::SymbolReference *sofRef =
      cg()->comp()->getSymRefTab()->findOrCreateStackOverflowSymbolRef(
          bodySymbol);
  ...
  if (constantIsImmed8r(frameSize, &base, &rotate)) {
    // mov R11, frameSize
    *(int32_t *)buffer =
        0xe3a0b << 12 | (rotate ? 16 - (rotate >> 1) : 0) << 8 | base & 0xFF;
    buffer += 4;
  } else if (!constantIsImmed8r(frameSize - offset, &base, &rotate) &&
             constantIsImmed8r(-offset, &base, &rotate)) {
    // sub R11, R11, -offset
    *(int32_t *)buffer =
        0xe24bb << 12 | ((rotate ? 16 - (rotate >> 1) : 0) << 8) | base & 0xFF;
    buffer += 4;
  } else {
    // R11 <- cg()->getFrameSizeInBytes() by mov, add, add
    // There is no need in another add, since a stack frame will never be that
    // big.
  ...
  *(int32_t *)buffer =
      encodeHelperBranchAndLink(sofRef, buffer, getNode(), cg());
  buffer += 4;
  ...
  *(int32_t *)buffer =
      0xEA000000 |
      encodeBranchDistance((uintptr_t)buffer,
                           (uintptr_t)getReStartLabel()->getCodeLocation());
...
uint32_t TR::ARMStackCheckFailureSnippet::getLength(
  ...
  //   cg()->getLinkage()->getProperties(); uint32_t offset =
  //   linkage.getOffsetToFirstLocal();
  if (!constantIsImmed8r(frameSize, &base, &rotate)) /* &&
  ...
  if (cg()->getSnippetList().size() <= 1 && !bodySymbol->isEHAware() &&
