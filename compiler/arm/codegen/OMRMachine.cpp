    ...
    bool isSinglePrecision)
{
    ...
    for (int i = first; i <= last; i++) {
        if ((_registerFile[i]->getState() == TR::RealRegister::Free || (considerUnlatched && _registerFile[i]->getState() == TR::RealRegister::Unlatched)) && _registerFile[i]->getWeight() < bestWeightSoFar) {
    ...
    bool isSinglePrecision)
{
    ...
    if (forced != NULL) {
        ...
        while (numCandidates > 1 && cursor != NULL && cursor->getOpCodeValue() != ARMOp_label && cursor->getOpCodeValue() != ARMOp_proc) {
...
{
    self()->cg()->_unlatchedRegisterList = (TR::RealRegister**)self()->cg()->trMemory()->allocateHeapMemory(sizeof(TR::RealRegister*) * (TR::RealRegister::NumRegisters + 1));
