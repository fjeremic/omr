...
OMR::ARM::Machine::Machine(TR::CodeGenerator *cg) : OMR::Machine(cg) {
  self()->initializeRegisterFile();
}

TR::RealRegister *OMR::ARM::Machine::findBestFreeRegister(
    TR_RegisterKinds rk, bool excludeGPR0, bool considerUnlatched,
...
TR::RealRegister *OMR::ARM::Machine::freeBestRegister(
  ...
  switch (rk) {
    case TR_GPR:
      if (candidates[0]->getBackingStorage()) {
        ...
        if (!location->isOccupied()) {
          ...
          // TODO in ARMInstruction.cpp). Therefore we need to set the occupied
          // flag for this reuse.
    ...
    case TR_FPR:
      ...
      if (candidates[0]->getBackingStorage()) {
        ...
        location = self()->cg()->allocateSpill(
            8, false, NULL);  // TODO: use 4 when floatSpill is implemented
      }
      break;
  }
  ...
  tmemref = new (self()->cg()->trHeapMemory())
      TR::MemoryReference(currentNode, location->getSymbolReference(),
                          ((rk == TR_FPR) ? 8 : 4), self()->cg());
  ...
  switch (rk) {
    case TR_GPR:
      generateTrg1MemInstruction(self()->cg(), ARMOp_ldr, currentNode, best,
    ...
    case TR_FPR:
      ...
      generateTrg1MemInstruction(
          self()->cg(), (isSinglePrecision ? ARMOp_flds : ARMOp_fldd),
...
TR::RealRegister *OMR::ARM::Machine::reverseSpillState(
  ...
  if (targetRegister == NULL) {
    ...
    if (targetRegister == NULL) {
      targetRegister =
          self()->freeBestRegister(currentInstruction, rk, NULL, excludeGPR0);
    }
  ...
  if (self()->cg()->comp()->getOption(TR_DisableOOL)) {
    switch (rk) {
      case TR_GPR:
        self()->cg()->freeSpill(location,
                                TR::Compiler->om.sizeofReferenceAddress(), 0);
        generateMemSrc1Instruction(self()->cg(), ARMOp_str, currentNode,
                                   tmemref, targetRegister, currentInstruction);
        break;
      case TR_FPR:
        self()->cg()->freeSpill(
            location, 8, 0);  // TODO: use 4 when floatSpill is implemented
        // if targetRegister is FS0-FS15, using ARMOp_fsts.
        generateMemSrc1Instruction(
            self()->cg(), (isSinglePrecision ? ARMOp_fsts : ARMOp_fstd),
    ...
    if (self()->cg()->isOutOfLineColdPath()) {
      ...
      if (currentInstruction->isLabel()) {
        if (((TR::ARMLabelInstruction *)currentInstruction)
                ->getLabelSymbol()
  ...
  if (targetRegister->getState() == TR::RealRegister::Free ||
    ...
    if (self()->cg()->comp()->getOption(TR_TraceCG))
      diagnostic(", which is assigned to %s",
                 currentTargetVirtual->getRegisterName(self()->cg()->comp()));
#endif
    spareReg = self()->findBestFreeRegister(rk, false, false,
                                            isSinglePrecision(registerNumber));
    if (currentAssignedRegister != NULL) {
      TR_ASSERT(
          !isSinglePrecision(registerNumber),
      ...
      if (spareReg == NULL) {
        ...
        TR_ASSERT(!isSinglePrecision(registerNumber),
                  "We want to free all the registers between FS0 and FS15 "
                  "before DirectToJNI call.");
        registerCopy(currentInstruction, rk, targetRegister, spareReg,
                     self()->cg());
      ...
      if (virtualRegister->getTotalUseCount() !=
          virtualRegister->getFutureUseCount()) {
        TR_ASSERT(!isSinglePrecision(registerNumber),
                  "Dummy FS0-FS15 registers must not be used after this "
                  "instruction.");
        self()->reverseSpillState(currentInstruction, virtualRegister,
                                  targetRegister);
      }
...
void OMR::ARM::Machine::initializeRegisterFile() {
  self()->cg()->_unlatchedRegisterList =
      (TR::RealRegister **)self()->cg()->trMemory()->allocateHeapMemory(
  ...
  switch (rk) {
    case TR_GPR:
      new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(
          precedingInstruction, ARMOp_mov, node, targetReg, sourceReg, cg);
      break;
    case TR_FPR:
      bool isTargetSinglePrecision =
          isSinglePrecision(targetReg->getRegisterNumber());
      bool isSourceSinglePrecision =
          isSinglePrecision(sourceReg->getRegisterNumber());
      if (isTargetSinglePrecision) {
        if (isSourceSinglePrecision) {
          new (cg->trHeapMemory())
              TR::ARMTrg1Src1Instruction(precedingInstruction, ARMOp_fcpys,
                                         node, targetReg, sourceReg, cg);
        } else {
          generateTrg1Src1Instruction(cg, ARMOp_fcvtsd, node, targetReg,
                                      sourceReg);
        }
      } else {
        if (isSourceSinglePrecision) {
          generateTrg1Src1Instruction(cg, ARMOp_fcvtds, node, targetReg,
                                      sourceReg);
        } else {
          new (cg->trHeapMemory())
              TR::ARMTrg1Src1Instruction(precedingInstruction, ARMOp_fcpyd,
                                         node, targetReg, sourceReg, cg);
        }
      }
      break;
...
static void registerExchange(TR::Instruction *precedingInstruction,
  ...
  if (rk != TR_GPR) {
    ...
    new (cg->trHeapMemory())
        TR::ARMTrg1Src2Instruction(precedingInstruction, ARMOp_eor, node,
                                   targetReg, targetReg, sourceReg, cg);
    new (cg->trHeapMemory())
        TR::ARMTrg1Src2Instruction(precedingInstruction, ARMOp_eor, node,
                                   sourceReg, targetReg, sourceReg, cg);
    new (cg->trHeapMemory())
        TR::ARMTrg1Src2Instruction(precedingInstruction, ARMOp_eor, node,
                                   targetReg, targetReg, sourceReg, cg);
  }
...
TR::RealRegister *OMR::ARM::Machine::assignSingleRegister(
  ...
  if (assignedRegister == NULL) {
    if (virtualRegister->getTotalUseCount() !=
        virtualRegister->getFutureUseCount()) {
      assignedRegister = self()->reverseSpillState(
          currentInstruction, virtualRegister, NULL, false);
    } else {
      if ((assignedRegister = self()->findBestFreeRegister(
               kindOfRegister, false, true)) == NULL) {
        assignedRegister = self()->freeBestRegister(
            currentInstruction, kindOfRegister, NULL, false);
      }
    }
...
uint32_t OMR::ARM::Machine::_globalRegisterNumberToRealRegisterMap
    [MAX_ARM_GLOBAL_GPRS + MAX_ARM_GLOBAL_FPRS] = {
        TR::RealRegister::gr9,  TR::RealRegister::gr10, TR::RealRegister::gr11,
        TR::RealRegister::gr6,  // BP; cannot offer if debug("hasFramePointer")
        TR::RealRegister::gr4,  TR::RealRegister::gr5,  TR::RealRegister::gr3,
        TR::RealRegister::gr2,  TR::RealRegister::gr1,  TR::RealRegister::gr0,
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
        TR::RealRegister::fp15, TR::RealRegister::fp14, TR::RealRegister::fp13,
        TR::RealRegister::fp12, TR::RealRegister::fp11, TR::RealRegister::fp10,
        TR::RealRegister::fp9,  TR::RealRegister::fp8,
#endif
        TR::RealRegister::fp7,  TR::RealRegister::fp6,  TR::RealRegister::fp5,
        TR::RealRegister::fp4,  TR::RealRegister::fp3,  TR::RealRegister::fp2,
        TR::RealRegister::fp1,  TR::RealRegister::fp0,
};

TR::RegisterDependencyConditions *
OMR::ARM::Machine::createCondForLiveAndSpilledGPRs(
       ...
       i++)  // Skipping the special/reserved register.  Add FPR later
  {
    ...
    if (comp->getOptions()->getRegisterAssignmentTraceOption(
      ...
      traceMsg(comp, "OOL: Taking snap shot %d, %x, %x, %x\n", i,
               _registerStatesSnapShot[i], _assignedRegisterSnapShot[i],
               _registerFlagsSnapShot[i]);
  }
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
  // snapshot is used only for OOL. For now, single precision registers are used
  // only for DirectToJNI.
...
void OMR::ARM::Machine::restoreRegisterStateFromSnapShot() {
       ...
       i++)  // Skipping SpilledReg
  {
    ...
    //   setVirtualAssociatedWithReal((TR::RealRegister::RegNum)(i),
