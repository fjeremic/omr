...
TR::RealRegister *
OMR::ARM::Machine::findBestFreeRegister(TR_RegisterKinds rk, bool excludeGPR0,
...
TR::RealRegister *OMR::ARM::Machine::freeBestRegister(
  ...
  case TR_GPR:
    if (candidates[0]->getBackingStorage()) {
      ...
      if (!location->isOccupied()) {
        ...
        // ARMInstruction.cpp). Therefore we need to set the occupied flag for
        // this reuse.
  ...
  case TR_FPR:
    ...
    generateTrg1MemInstruction(self()->cg(),
                               (isSinglePrecision ? ARMOp_flds : ARMOp_fldd),
...
TR::RealRegister *
OMR::ARM::Machine::reverseSpillState(TR::Instruction *currentInstruction,
                                     TR::Register *spilledRegister,
                                     TR::RealRegister *targetRegister,
                                     bool excludeGPR0, bool isSinglePrecision) {
  TR::MemoryReference *tmemref;
  ...
  if (self()->cg()->comp()->getOption(TR_DisableOOL)) {
    ...
    case TR_GPR:
      ...
      generateMemSrc1Instruction(self()->cg(), ARMOp_str, currentNode, tmemref,
                                 targetRegister, currentInstruction);
      break;
    case TR_FPR:
      self()->cg()->freeSpill(location, 8,
                              0); // TODO: use 4 when floatSpill is implemented
  ...
  case TR_FPR:
    ...
    if (isTargetSinglePrecision) {
      if (isSourceSinglePrecision) {
        new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(
            precedingInstruction, ARMOp_fcpys, node, targetReg, sourceReg, cg);
      } else {
      ...
      if (isSourceSinglePrecision) {
        ...
        new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(
            precedingInstruction, ARMOp_fcpyd, node, targetReg, sourceReg, cg);
      }
    }
    break;
  }
...
TR::RealRegister *
OMR::ARM::Machine::assignSingleRegister(TR::Register *virtualRegister,
