  ...
  TR::RealRegister* forced, bool excludeGPR0, bool isSinglePrecision)
{
  ...
  if (forced != NULL) {
    ...
    while (numCandidates > 1 && cursor != NULL && cursor->getOpCodeValue() != ARMOp_label
  ...
  case TR_FPR:
    ...
    generateTrg1MemInstruction(
      self()->cg(), (isSinglePrecision ? ARMOp_flds : ARMOp_fldd), currentNode, best, tmemref, currentInstruction);
    break;
  }
  ...
  TR::Register* spilledRegister, TR::RealRegister* targetRegister, bool excludeGPR0, bool isSinglePrecision)
{
  ...
  if (self()->cg()->comp()->getOption(TR_DisableOOL)) {
    ...
    case TR_FPR:
      ...
      generateMemSrc1Instruction(self()->cg(), (isSinglePrecision ? ARMOp_fsts : ARMOp_fstd), currentNode, tmemref,
  ...
  TR::Instruction* currentInstruction, TR::Register* virtualRegister, TR::RealRegister::RegNum registerNumber)
{
  ...
  if (targetRegister->getState() == TR::RealRegister::Free
...
#ifdef DEBUG
    if (self()->cg()->comp()->getOption(TR_TraceCG))
      diagnostic(", which is blocked and assigned to %s", currentTargetVirtual->getRegisterName(self()->cg()->comp()));
#endif
    ...
    if (currentAssignedRegister != NULL) {
      TR_ASSERT(
        !isSinglePrecision(registerNumber), "Dummy FS0-FS15 registers must not be used after this instruction.");
  ...
  TR::RealRegister* sourceReg, TR::CodeGenerator* cg)
{
  ...
  case TR_GPR:
    new (cg->trHeapMemory())
      TR::ARMTrg1Src1Instruction(precedingInstruction, ARMOp_mov, node, targetReg, sourceReg, cg);
    break;
  case TR_FPR:
    ...
    if (isTargetSinglePrecision) {
      if (isSourceSinglePrecision) {
        new (cg->trHeapMemory())
          TR::ARMTrg1Src1Instruction(precedingInstruction, ARMOp_fcpys, node, targetReg, sourceReg, cg);
      } else {
      ...
      if (isSourceSinglePrecision) {
        ...
        new (cg->trHeapMemory())
          TR::ARMTrg1Src1Instruction(precedingInstruction, ARMOp_fcpyd, node, targetReg, sourceReg, cg);
      }
    }
    break;
  }
  ...
  TR::RealRegister* sourceReg, TR::RealRegister* middleReg, TR::CodeGenerator* cg)
{
  ...
  if (rk != TR_GPR) {
    ...
    new (cg->trHeapMemory())
      TR::ARMTrg1Src2Instruction(precedingInstruction, ARMOp_eor, node, targetReg, targetReg, sourceReg, cg);
    new (cg->trHeapMemory())
      TR::ARMTrg1Src2Instruction(precedingInstruction, ARMOp_eor, node, sourceReg, targetReg, sourceReg, cg);
    new (cg->trHeapMemory())
      TR::ARMTrg1Src2Instruction(precedingInstruction, ARMOp_eor, node, targetReg, targetReg, sourceReg, cg);
  }
}
