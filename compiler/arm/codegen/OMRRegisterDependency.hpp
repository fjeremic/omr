...
namespace OMR {
typedef OMR::ARM::RegisterDependencyConditions
...
struct TR_ARMRegisterDependency {
  ...
  uint32_t resetRefsRegister() {
    return (_flags &= ~ReferencesDependentRegister);
  }
...
class TR_ARMRegisterDependencyGroup {
  ...
  // Use TR_ARMRegisterDependencyGroup::create to allocate an object of this
  // type
  //
  void *operator new(size_t s, int32_t numDependencies, TR_Memory *m) {
    ...
    if (numDependencies > NUM_DEFAULT_DEPENDENCIES) {
      s += (numDependencies - NUM_DEFAULT_DEPENDENCIES) *
           sizeof(TR_ARMRegisterDependency);
    }
...
public:
  ...
  static TR_ARMRegisterDependencyGroup *create(int32_t numDependencies,
                                               TR_Memory *m) {
...
public:
  ...
  RegisterDependencyConditions(uint8_t numPreConds, uint8_t numPostConds,
                               TR_Memory *m)
  ...
  RegisterDependencyConditions(TR::Node *node, uint32_t extranum,
                               TR::Instruction **cursorPtr,
                               TR::CodeGenerator *cg);

  TR::RegisterDependencyConditions *
  clone(TR::CodeGenerator *, TR::RegisterDependencyConditions *added = NULL);
  TR::RegisterDependencyConditions *
  cloneAndFix(TR::CodeGenerator *,
              TR::RegisterDependencyConditions *added = NULL);

  void unionNoRegPostCondition(TR::Register *reg,
                               TR::CodeGenerator *cg); /* @@@@ */
