...
OMR::X86::I386::CodeGenerator::CodeGenerator() : OMR::X86::CodeGenerator() {
  ...
  if (debug("supportsArrayCmp")) self()->setSupportsArrayCmp();
  ...
  if (TR::Compiler->target.isWindows()) {
    ...
    self()->setRealVMThreadRegister(
        self()->machine()->getRealRegister(TR::RealRegister::ebp));
  ...
  static char *dontConsiderAllAutosForGRA =
      feGetEnv("TR_dontConsiderAllAutosForGRA");
  if (!dontConsiderAllAutosForGRA)
    self()->setConsiderAllAutosAsTacticalGlobalRegisterCandidates();
}

TR::Register *OMR::X86::I386::CodeGenerator::longClobberEvaluate(
    TR::Node *node) {
  ...
  if (node->getReferenceCount() > 1) {
    ...
    generateRegRegInstruction(MOV4RegReg, node, lowReg, temp->getLowOrder(),
                              self());
    generateRegRegInstruction(MOV4RegReg, node, highReg, temp->getHighOrder(),
                              self());
    return longReg;
  } else {
...
TR_GlobalRegisterNumber OMR::X86::I386::CodeGenerator::pickRegister(
  ...
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    return OMR::CodeGenerator::pickRegister(rc, allBlocks, availableRegisters,
                                            highRegisterNumber, candidates);
  }

  if ((rc->getSymbol()->getDataType() == TR::Float) ||
      (rc->getSymbol()->getDataType() == TR::Double)) {
    if (availableRegisters.get(7)) return 7;
    if (availableRegisters.get(8)) return 8;
    if (availableRegisters.get(9)) return 9;
    if (availableRegisters.get(10)) return 10;
    if (availableRegisters.get(11)) return 11;
    if (availableRegisters.get(12)) return 12;
  ...
  if (availableRegisters.get(5)) return 5;  // esi

  if (availableRegisters.get(2)) return 2;  // ecx

  static char *dontUseEBXasGPR = feGetEnv("dontUseEBXasGPR");
  if (!dontUseEBXasGPR && availableRegisters.get(1)) return 1;

#ifdef J9_PROJECT_SPECIFIC
  TR::RecognizedMethod rm =
      self()->comp()->getMethodSymbol()->getRecognizedMethod();
  if (rm == TR::java_util_HashtableHashEnumerator_hasMoreElements) {
    if (availableRegisters.get(4)) return 4;  // edi
    if (availableRegisters.get(3)) return 3;  // edx
  } else
#endif
  {
    ...
    while (bvi.hasMoreElements()) {
      ...
      maxRegisterPressure = self()->estimateRegisterPressure(
          ...
          rc->getSymbolReference(), assigningEDX);
    ...
    if (maxRegisterPressure < self()->getMaximumNumbersOfAssignableGPRs())
      numExtraRegs =
          self()->getMaximumNumbersOfAssignableGPRs() - maxRegisterPressure;

    // dumpOptDetails("For global register candidate %d reg pressure is %d
    // maxRegs %d numExtraRegs %d\n",
...
int32_t OMR::X86::I386::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
    TR::Node *node) {
  // TODO: Currently, lookupEvaluator doesn't deal properly with different
  // glRegDeps on different cases of a lookupswitch.
  //
  static const char *enableLookupswitch =
      feGetEnv("TR_enableGRAAcrossLookupSwitch");
  ...
  if (node->getOpCode().isIf()) {
      ...
      if (node->getOpCode().isBranch()) {
        ...
        if ((firstChild->getReferenceCount() == 1 &&
            ...
            (secondChild->getReferenceCount() == 1 &&
             firstChild->getOpCode().isLoadVarDirect()))
          extraRegsAvailable += 0;  // TODO: put it back to 2 when looking at
                                    // GRA, GRA pushes allocation of 8 registers
    ...
    // we run out of all but one register in these cases....last time I
    // tried....
    //

    if (node->getFirstChild()->getOpCodeValue() == TR:: instanceof) {
      if (!TR::TreeEvaluator::instanceOfOrCheckCastNeedSuperTest(
              node->getFirstChild(), self()) &&
          TR::TreeEvaluator::instanceOfOrCheckCastNeedEqualityTest(
              node->getFirstChild(), self()))
        return self()->getNumberOfGlobalGPRs() -
               4;  // ebp plus three other regs if vft masking is enabled
      else
        return 0;
    }

    // All other conditional branches, we usually need one reg for the compare
    // and possibly one for the vmthread
...
bool OMR::X86::I386::CodeGenerator::codegenMulDecomposition(
    int64_t multiplier) {
  bool iMulDecomposeReport = self()->comp()->getOptions()->getTraceSimplifier(
      TR_TraceMulDecomposition);
  bool answer = false;
  if (iMulDecomposeReport)
    diagnostic("\nCodegen was queried for the value of %d, ",
               (int32_t)multiplier);
