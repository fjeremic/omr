    ...
    if (node->getOpCode().isIf()) {
        ...
        if (node->getFirstChild()->getType().isInt64()) {
            if (node->getOpCode().isBranch()) {
                ...
                if ((firstChild->getReferenceCount() == 1 && firstChild->getOpCode().isLoadVarDirect()) || (secondChild->getReferenceCount() == 1 && firstChild->getOpCode().isLoadVarDirect()))
        ...
        if (node->getFirstChild()->getOpCodeValue() == TR:: instanceof) {
            if (!TR::TreeEvaluator::instanceOfOrCheckCastNeedSuperTest(node->getFirstChild(), self()) && TR::TreeEvaluator::instanceOfOrCheckCastNeedEqualityTest(node->getFirstChild(), self()))
