...
OMR::X86::I386::CodeGenerator::longClobberEvaluate(TR::Node* node)
{
  ...
  if (node->getReferenceCount() > 1) {
    ...
    generateRegRegInstruction(
      MOV4RegReg, node, lowReg, temp->getLowOrder(), self());
    generateRegRegInstruction(
      MOV4RegReg, node, highReg, temp->getHighOrder(), self());
    return longReg;
  } else {
  ...
  TR_LinkHead<TR_RegisterCandidate>* candidates)
{
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    return OMR::CodeGenerator::pickRegister(
      rc, allBlocks, availableRegisters, highRegisterNumber, candidates);
  }
...
int32_t
OMR::X86::I386::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
  TR::Node* node)
{
