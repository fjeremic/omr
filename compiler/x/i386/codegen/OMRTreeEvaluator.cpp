    ...
    TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL && performTransformation(comp, "O^O compareLongAndSetOrderedBoolean: checking that the second child node does not have an assigned register: %d\n", testRegister)) {
    ...
    TR::CodeGenerator* cg)
{
    ...
    TR::Node* secondChild = node->getSecondChild();
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL && performTransformation(comp, "O^O compareLongsForOrder: checking that the second child node does not have an assigned register: %d\n", testRegister)) {
        int32_t lowValue = secondChild->getLongIntLow();
        int32_t highValue = secondChild->getLongIntHigh();
        TR::Node* firstChild = node->getFirstChild();
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL && (isMemOp || firstChild->getReferenceCount() == 1)) {
        if (!isMemOp)
        ...
        if (lowValue == 0 && highValue == 0) {
            ...
            if (absValue == 3 || absValue == 5 || absValue == 9) {
                ...
                if (firstIU2L && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == 0) {
                    firstChild = firstChild->getFirstChild();
                    multiplierRegister = cg->evaluate(firstChild);
                } else if (firstChild->getOpCodeValue() == TR::lushr && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && firstChild->getSecondChild()->getInt() == 32 && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == 0) {
                    firstChild = firstChild->getFirstChild();
                ...
                int32_t dummy = 0;
                TR_X86IntegerMultiplyDecomposer* mulDecomposer = new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer((int64_t)absValue,
                    multiplierRegister,
                    node,
                    cg,
            ...
            if (!firstIU2L && !intMul) {
                ...
                TR_X86IntegerMultiplyDecomposer* mulDecomposer = new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer((int64_t)lowValue,
                    multiplierRegister->getHighOrder(),
                    node,
                    cg,
                    firstChild->getReferenceCount() == 1 ? true : false);
...
TR::Register* OMR::X86::I386::TreeEvaluator::iflcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
        ...
        if ((lowValue | highValue) == 0) {
            ...
            if (firstChild->getOpCodeValue() == TR::land && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && (landConstChild = firstChild->getSecondChild())->getOpCodeValue() == TR::lconst && landConstChild->getLongIntLow() == 0 && landConstChild->getLongIntHigh() == 0xffffffff) {
                TR::Node* landFirstChild = firstChild->getFirstChild();
                if (landFirstChild->getReferenceCount() == 1 && landFirstChild->getRegister() == NULL && landFirstChild->getOpCode().isLoadVar()) {
                    targetRegister = cg->allocateRegister();
...
TR::Register* OMR::X86::I386::TreeEvaluator::lternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!longCompare && condOp.isCompareForEquality() && condition->getFirstChild()->getOpCode().isIntegerOrAddress()) {
        ...
        generateRegRegInstruction((condOp.isCompareTrueIfEqual()) ? ((condOp.isCompareTrueIfGreater()) ? CMOVL4RegReg : CMOVG4RegReg) : ((condOp.isCompareTrueIfGreater()) ? CMOVLE4RegReg : CMOVGE4RegReg), node, trueReg->getRegisterPair()->getLowOrder(), falseReg->getRegisterPair()->getLowOrder(), cg);
        generateRegRegInstruction((condOp.isCompareTrueIfEqual()) ? ((condOp.isCompareTrueIfGreater()) ? CMOVL4RegReg : CMOVG4RegReg) : ((condOp.isCompareTrueIfGreater()) ? CMOVLE4RegReg : CMOVGE4RegReg), node, trueReg->getRegisterPair()->getHighOrder(), falseReg->getRegisterPair()->getHighOrder(), cg);
    } else {
