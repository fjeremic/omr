...
TR::Register *
OMR::X86::I386::TreeEvaluator::longArithmeticCompareRegisterWithImmediate(
    TR::Node *node, TR::Register *cmpRegister, TR::Node *immedChild,
    TR_X86OpCodes firstBranchOpCode, TR_X86OpCodes secondBranchOpCode,
    TR::CodeGenerator *cg) {
  ...
  compareGPRegisterToImmediate(node, cmpRegister->getHighOrder(), highValue,
                               cg);
  ...
  TR::RegisterDependencyConditions *deps =
      generateRegisterDependencyConditions((uint8_t)0, 3, cg);
  deps->addPostCondition(cmpRegister->getLowOrder(), TR::RealRegister::NoReg,
                         cg);
  deps->addPostCondition(cmpRegister->getHighOrder(), TR::RealRegister::NoReg,
                         cg);
  deps->addPostCondition(targetRegister, TR::RealRegister::ByteReg, cg);

  generateLabelInstruction(LABEL, node, doneLabel, deps, cg);

  generateRegRegInstruction(MOVSXReg4Reg1, node, targetRegister, targetRegister,
                            cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::compareLongAndSetOrderedBoolean(
    TR::Node *node, TR_X86OpCodes highSetOpCode, TR_X86OpCodes lowSetOpCode,
    TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL &&
      performTransformation(
          comp,
          "O^O compareLongAndSetOrderedBoolean: checking that the second child "
          "node does not have an assigned register: %d\n",
          testRegister)) {
    ...
    compareGPRegisterToImmediate(node, cmpRegister->getLowOrder(), lowValue,
                                 cg);
    ...
    //
    generateRegRegInstruction(MOVSXReg4Reg1, node, targetRegister,
                              targetRegister, cg);
    ...
    targetRegister =
        temp.longOrderedBooleanAnalyser(node, highSetOpCode, lowSetOpCode);
  }
  return targetRegister;
}

void OMR::X86::I386::TreeEvaluator::compareLongsForOrder(
    TR::Node *node, TR_X86OpCodes highOrderBranchOp,
    TR_X86OpCodes highOrderReversedBranchOp, TR_X86OpCodes lowOrderBranchOp,
    TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *testRegister = secondChild->getRegister();
  if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL &&
      performTransformation(
          comp,
          "O^O compareLongsForOrder: checking that the second child node does "
          "not have an assigned register: %d\n",
          testRegister)) {
    int32_t lowValue = secondChild->getLongIntLow();
    int32_t highValue = secondChild->getLongIntHigh();
    TR::Node *firstChild = node->getFirstChild();
    // KAS:: need delayed evaluation here and use of memimm instructions to
    ...
    if (node->getNumChildren() == 3) {
      ...
      deps->addPostCondition(cmpRegister->getHighOrder(),
                             TR::RealRegister::NoReg, cg);
      deps->addPostCondition(cmpRegister->getLowOrder(),
                             TR::RealRegister::NoReg, cg);
      deps->stopAddingConditions();

      generateLabelInstruction(highOrderBranchOp, node, destinationLabel, deps,
                               cg);
      generateLabelInstruction(JNE4, node, doneLabel, deps, cg);
      compareGPRegisterToImmediate(node, cmpRegister->getLowOrder(), lowValue,
                                   cg);
      generateLabelInstruction(lowOrderBranchOp, node, destinationLabel, deps,
                               cg);
    } else {
    ...
    if (deps) deps->setMayNeedToPopFPRegisters(true);
    ...
    TR_X86CompareAnalyser temp(cg);
    temp.longOrderedCompareAndBranchAnalyser(
        node, lowOrderBranchOp, highOrderBranchOp, highOrderReversedBranchOp);
  }
}
...
//
TR::Register *OMR::X86::I386::TreeEvaluator::aconstEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *reg =
      loadConstant(node, node->getInt(), TR_RematerializableAddress, cg);
  node->setRegister(reg);
  return reg;
}

TR::Register *OMR::X86::I386::TreeEvaluator::lconstEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (abs(lowValue - highValue) <= 128) {
    if (lowValue <= highValue) {
      ...
      if (lowValue == highValue) {
        generateRegRegInstruction(MOV4RegReg, node, lowRegister, highRegister,
                                  cg);
      } else {
        generateRegMemInstruction(
            LEA4RegMem, node, lowRegister,
            generateX86MemoryReference(highRegister, lowValue - highValue, cg),
            cg);
      }
    } else {
      ...
      generateRegMemInstruction(
          LEA4RegMem, node, highRegister,
          generateX86MemoryReference(lowRegister, highValue - lowValue, cg),
          cg);
    }
  } else {
    lowRegister = loadConstant(node, lowValue, TR_RematerializableInt, cg);
  ...
  TR::RegisterPair *longRegister =
      cg->allocateRegisterPair(lowRegister, highRegister);
  node->setRegister(longRegister);
  return longRegister;
}

// also handles ilstore
TR::Register *OMR::X86::I386::TreeEvaluator::lstoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Node *valueChild;
  ...
  //
  if (!isVolatile && valueChild->getRegister() == NULL &&
  ...
  if (!isVolatile && valueChild->getOpCodeValue() == TR::lconst &&
    ...
    if (lowValue == highValue) {
      TR::Register *valueReg =
          loadConstant(node, lowValue, TR_RematerializableInt, cg);
    ...
    if (valueReg) {
      ...
      if (isVolatile) {
        if (cg->useSSEForDoublePrecision() &&
            performTransformation(comp, "O^O Using SSE for volatile store %s\n",
                                  cg->getDebug()->getName(node))) {
          // Get stack piece
          TR::MemoryReference *stackLow =
              cg->machine()->getDummyLocalMR(TR::Int64);
          TR::MemoryReference *stackHigh =
              generateX86MemoryReference(*stackLow, 4, cg);

          // generate: stack1 <- (valueReg->getloworder())    [S4MemReg]
          instr = generateMemRegInstruction(S4MemReg, node, stackLow,
                                            valueReg->getLowOrder(), cg);
          // generate: stack1_plus4 <- (valueReg->gethighorder())
          generateMemRegInstruction(S4MemReg, node, stackHigh,
                                    valueReg->getHighOrder(), cg);

          // generate: xmm <- stack1
          TR::MemoryReference *stack =
              generateX86MemoryReference(*stackLow, 0, cg);
          // Allocate XMM Reg
          TR::Register *reg = cg->allocateRegister(TR_FPR);
          generateRegMemInstruction(cg->getXMMDoubleLoadOpCode(), node, reg,
                                    stack, cg);
          ...
          TR_ASSERT(cg->getX86ProcessorInfo().supportsCMPXCHG8BInstruction(),
                    "Assumption of support of the CMPXCHG8B instruction failed "
                    "in lstoreEvaluator()");
          ...
          instr = generateRegMemInstruction(L4RegMem, node, eaxReg, lowMR,
          ...
          generateRegRegInstruction(MOV4RegReg, node, ebxReg,
                                    valueReg->getLowOrder(),
                                    cg);  // forming the ECX:EBX pair
          generateRegRegInstruction(MOV4RegReg, node, ecxReg,
                                    valueReg->getHighOrder(), cg);

          TR::MemoryReference *cmpxchgMR = generateX86MemoryReference(node, cg);
          generateMemInstruction(
              TR::Compiler->target.isSMP() ? LCMPXCHG8BMem : CMPXCHG8BMem, node,
              cmpxchgMR, deps, cg);
      ...
      } else if (symRef && symRef->isUnresolved() &&
                 symRef->getSymbol()->isVolatile() &&
                 (!comp->getOption(TR_DisableNewX86VolatileSupport) &&
        ...
        instr = generateMemRegInstruction(S4MemReg, node, lowMR,
                                          valueReg->getLowOrder(), cg);
        generateMemRegInstruction(S4MemReg, node, highMR,
                                  valueReg->getHighOrder(), cg);
  ...
  cg->decReferenceCount(valueChild);
  if (lowMR &&
      !(valueChild->isDirectMemoryUpdate() && node->getOpCode().isIndirect()))
  ...
  if (node->getSymbolReference()->getSymbol()->isVolatile()) {
    ...
    if ((lowMR || highMR) && caller) {
      ...
      if (m->getRecognizedMethod() ==
          TR::java_util_concurrent_atomic_AtomicLong_lazySet) {
        if (lowMR) lowMR->setIgnoreVolatile();
        if (highMR) highMR->setIgnoreVolatile();
      }
#endif
    }
  }
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairReturnEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (cg->enableSinglePrecisionMethods() &&
    ...
    generateMemInstruction(
        LDCWMem, node,
        generateX86MemoryReference(cg->findOrCreate2ByteConstant(
                                       node, DOUBLE_PRECISION_ROUND_TO_NEAREST),
                                   cg),
        cg);
  }
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairAddEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (lowValue >= -128 && lowValue <= 127) {
      if (isMemOp)
        instr =
            generateMemImmInstruction(ADD4MemImms, node, lowMR, lowValue, cg);
      else
        instr = generateRegImmInstruction(
            ADD4RegImms, node, targetRegister->getLowOrder(), lowValue, cg);
    } else if (lowValue == 128) {
      if (isMemOp)
        instr = generateMemImmInstruction(SUB4MemImms, node, lowMR, -128, cg);
      else
        instr = generateRegImmInstruction(
            SUB4RegImms, node, targetRegister->getLowOrder(), -128, cg);
      ...
      if (isMemOp)
        instr =
            generateMemImmInstruction(ADD4MemImm4, node, lowMR, lowValue, cg);
      else
        instr = generateRegImmInstruction(
            ADD4RegImm4, node, targetRegister->getLowOrder(), lowValue, cg);
    }
    ...
    if (isMemOp) {
      ...
      generateRegImmInstruction(opCode, node, targetRegister->getHighOrder(),
                                highValue, cg);
    }
  } else if (isMemOp && !needsEflags) {
    TR::Register *tempReg = cg->evaluate(secondChild);
    instr = generateMemRegInstruction(ADD4MemReg, node, lowMR,
                                      tempReg->getLowOrder(), cg);
    generateMemRegInstruction(ADC4MemReg, node, highMR, tempReg->getHighOrder(),
                              cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairSubEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *targetRegister = NULL;
  ...
  if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (lowValue >= -128 && lowValue <= 127) {
      if (isMemOp)
        instr =
            generateMemImmInstruction(SUB4MemImms, node, lowMR, lowValue, cg);
      else
        instr = generateRegImmInstruction(
            SUB4RegImms, node, targetRegister->getLowOrder(), lowValue, cg);
    } else if (lowValue == 128) {
      if (isMemOp)
        instr =
            generateMemImmInstruction(ADD4MemImms, node, lowMR, lowValue, cg);
        ...
        instr = generateRegImmInstruction(
            ADD4RegImms, node, targetRegister->getLowOrder(), -128, cg);
      ...
      if (isMemOp)
        instr =
            generateMemImmInstruction(SUB4MemImm4, node, lowMR, lowValue, cg);
      else
        instr = generateRegImmInstruction(
            SUB4RegImm4, node, targetRegister->getLowOrder(), lowValue, cg);
    }
    ...
    TR::Register *tempReg = cg->evaluate(secondChild);
    instr = generateMemRegInstruction(SUB4MemReg, node, lowMR,
                                      tempReg->getLowOrder(), cg);
    generateMemRegInstruction(SBB4MemReg, node, highMR, tempReg->getHighOrder(),
                              cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairDualMulEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairMulEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (firstOp == TR::i2l && value >= TR::getMinSigned<TR::Int32>() &&
    ...
    if (lowValue == 0 && highValue == 0) {
      ...
      highRegister = cg->allocateRegister();
      generateRegRegInstruction(XOR4RegReg, node, highRegister, highRegister,
                                cg);
    } else if (cg->convertMultiplyToShift(node)) {
      // Don't do a direct memory operation if we have to negate the result
      //
      if (absValue != highValue) node->setDirectMemoryUpdate(false);
      targetRegister = cg->evaluate(node);
      if (absValue != highValue) {
        generateRegInstruction(NEG4Reg, node, targetRegister->getLowOrder(),
                               cg);
        generateRegImmInstruction(ADC4RegImms, node,
                                  targetRegister->getHighOrder(), 0, cg);
        generateRegInstruction(NEG4Reg, node, targetRegister->getHighOrder(),
                               cg);
      }
      return targetRegister;
    } else if (lowValue == 0) {
      ...
      if (absValue == 3 || absValue == 5 || absValue == 9) {
        if (firstIU2L && firstChild->getReferenceCount() == 1 &&
        ...
        if (firstIU2L && firstChild->getReferenceCount() == 1 &&
          ...
          multiplierRegister = cg->evaluate(firstChild);
                   ...
                   firstChild->getSecondChild()->getOpCodeValue() ==
                       TR::iconst &&
        ...
        TR_X86IntegerMultiplyDecomposer *mulDecomposer =
            new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer(
                (int64_t)absValue, multiplierRegister, node, cg,
        ...
        if (highRegister == 0)  // cannot do the decomposition
        {
          if (firstChild->getReferenceCount() > 1 ||
            ...
            generateRegRegImmInstruction(opCode, node, highRegister,
                                         multiplierRegister, highValue, cg);
          } else {
            if (highValue >= -128 && highValue <= 127) {
            ...
            highRegister = cg->allocateRegister();
            generateRegMemImmInstruction(opCode, node, highRegister, nodeMR,
                                         highValue, cg);
          }
        }
      }
      ...
      if ((firstIU2L || intMul) && firstChild->getReferenceCount() == 1 &&
          firstChild->getRegister() == 0) {
        firstChild = firstChild->getFirstChild();
      ...
      generateRegRegInstruction(opCode, node, lowRegister, sourceLow,
                                dependencies, cg);
      if (!firstIU2L && !intMul) {
        ...
        TR::Register
            *tempRegArray[TR_X86IntegerMultiplyDecomposer::MAX_NUM_REGISTERS];
        int32_t tempRegArraySize = 0;
        TR_X86IntegerMultiplyDecomposer *mulDecomposer =
            new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer(
                (int64_t)lowValue, multiplierRegister->getHighOrder(), node, cg,
                firstChild->getReferenceCount() == 1 ? true : false);
        TR::Register *tempRegister = mulDecomposer->decomposeIntegerMultiplier(
            tempRegArraySize, tempRegArray);
        ...
        if (tempRegister == 0)  // decomposition failed
        {
          ...
          if (firstChild->getReferenceCount() == 1) {
            ...
            TR_ASSERT(tempRegArraySize <=
                          TR_X86IntegerMultiplyDecomposer::MAX_NUM_REGISTERS,
                      "Too many temporary registers to handle");
            tempRegArray[tempRegArraySize++] = tempRegister;
          }
          generateRegRegImmInstruction(opCode, node, tempRegister,
                                       multiplierRegister->getHighOrder(),
                                       lowValue, cg);
        }
        generateRegRegInstruction(ADD4RegReg, node, highRegister, tempRegister,
                                  cg);

        // add dependencies for lowRegister, highRegister, mutiplierRegister and
        // temp registers
        uint8_t numDeps = 3 + tempRegArraySize;
        TR::RegisterDependencyConditions *deps =
            generateRegisterDependencyConditions(numDeps, numDeps, cg);
        deps->addPreCondition(lowRegister, TR::RealRegister::eax, cg);
        deps->addPreCondition(highRegister, TR::RealRegister::edx, cg);
        deps->addPreCondition(multiplierRegister->getHighOrder(),
                              TR::RealRegister::NoReg, cg);
        int i;
        ...
        deps->addPostCondition(multiplierRegister->getHighOrder(),
                               TR::RealRegister::NoReg, cg);
        for (i = 0; i < tempRegArraySize; i++)
    ...
    } else  // i.e. if (highValue !=0 && lowValue != 0)
    {
      ...
      if (absValue == 3 || absValue == 5 || absValue == 9) {
        ...
        TR_X86IntegerMultiplyDecomposer *mulDecomposer =
            new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer(
                (int64_t)absValue, sourceLow, node, cg,
                (firstChild->getReferenceCount() == 1 &&
                 highRegister != sourceLow)
                    ? true
                    : false);
        tempRegister = mulDecomposer->decomposeIntegerMultiplier(
            tempRegArraySize, tempRegArray);
        if (tempRegister == 0) {
          tempRegister = cg->allocateRegister();
          TR_ASSERT(tempRegArraySize <=
          ...
          if (firstChild->getReferenceCount() > 1 ||
            ...
            generateRegRegImmInstruction(opCode, node, tempRegister, sourceLow,
                                         highValue, cg);
          } else {
            if (highValue >= -128 && highValue <= 127) {
              opCode = IMUL4RegMemImms;
            ...
            nodeMR = generateX86MemoryReference(firstChild, cg);
            generateRegMemImmInstruction(opCode, node, tempRegister, nodeMR,
                                         highValue, cg);
          }
        }
      }

      TR_ASSERT(tempRegister != NULL,
                "tempRegister shouldn't be NULL at this point");
  ...
  if (lowRegister == NULL) {
    ...
    if (debug("traceInlineLongMultiply"))
      diagnostic("\ninlined long multiply at node [" POINTER_PRINTF_FORMAT
                 "] in method %s",
                 node, comp->signature());
  ...
  if (nodeMR) nodeMR->decNodeReferenceCounts(cg);

  return targetRegister;
}

TR::Register *OMR::X86::I386::TreeEvaluator::integerPairDivEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
#ifdef J9_PROJECT_SPECIFIC
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  TR::RegisterDependencyConditions *idivDependencies =
      generateRegisterDependencyConditions((uint8_t)6, 6, cg);
  ...
  idivDependencies->addPreCondition(firstRegister->getLowOrder(),
                                    TR::RealRegister::NoReg, cg);
  idivDependencies->addPreCondition(secondRegister->getLowOrder(),
                                    TR::RealRegister::NoReg, cg);
  idivDependencies->addPostCondition(firstRegister->getLowOrder(),
                                     TR::RealRegister::NoReg, cg);
  idivDependencies->addPostCondition(secondRegister->getLowOrder(),
                                     TR::RealRegister::NoReg, cg);
  ...
  // generateRegRegInstruction(TEST4RegReg, node, highRegister, highRegister,
  // cg);
  generateLabelInstruction(JNE4, node, callLabel, cg);

  generateRegRegInstruction(MOV4RegReg, node, lowRegister,
                            firstRegister->getLowOrder(), cg);
  divInstr = generateRegRegInstruction(DIV4AccReg, node, lowRegister,
  ...
  TR::RegisterDependencyConditions *xorDependencies1 =
      generateRegisterDependencyConditions((uint8_t)2, 2, cg);
  ...
  TR::RegisterDependencyConditions *dependencies =
      generateRegisterDependencyConditions((uint8_t)0, 2, cg);
  dependencies->addPostCondition(lowRegister, TR::RealRegister::eax, cg);
  dependencies->addPostCondition(highRegister, TR::RealRegister::edx, cg);
  J9::IA32PrivateLinkage *linkage =
      static_cast<J9::IA32PrivateLinkage *>(cg->getLinkage(TR_Private));
  ...
  TR::RegisterDependencyConditions *labelDependencies =
      generateRegisterDependencyConditions((uint8_t)6, 6, cg);
  ...
  labelDependencies->addPreCondition(firstRegister->getLowOrder(),
                                     TR::RealRegister::NoReg, cg);
  labelDependencies->addPreCondition(secondRegister->getLowOrder(),
                                     TR::RealRegister::NoReg, cg);
  labelDependencies->addPostCondition(firstRegister->getLowOrder(),
                                      TR::RealRegister::NoReg, cg);
  labelDependencies->addPostCondition(secondRegister->getLowOrder(),
                                      TR::RealRegister::NoReg, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairRemEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR::RegisterDependencyConditions *dependencies =
      generateRegisterDependencyConditions((uint8_t)4, 6, cg);
  ...
  dependencies->addPreCondition(firstRegister->getLowOrder(),
                                TR::RealRegister::NoReg, cg);
  dependencies->addPreCondition(secondRegister->getLowOrder(),
                                TR::RealRegister::NoReg, cg);
  dependencies->addPostCondition(firstRegister->getLowOrder(),
                                 TR::RealRegister::NoReg, cg);
  dependencies->addPostCondition(secondRegister->getLowOrder(),
                                 TR::RealRegister::NoReg, cg);
  ...
  TR::IA32LinkageUtils::pushLongArg(secondChild, cg);
  TR::IA32LinkageUtils::pushLongArg(firstChild, cg);
  TR::X86ImmSymInstruction *instr = generateHelperCallInstruction(
      node, TR_IA32longRemainder, dependencies, cg);
  if (!linkage->getProperties().getCallerCleanup()) {
    instr->setAdjustsFramePointerBy(-16);  // 2 long args
  }
  ...
  TR::RegisterDependencyConditions *movDependencies =
      generateRegisterDependencyConditions((uint8_t)6, 6, cg);
  ...
  movDependencies->addPreCondition(firstRegister->getLowOrder(),
                                   TR::RealRegister::NoReg, cg);
  movDependencies->addPreCondition(secondRegister->getLowOrder(),
                                   TR::RealRegister::NoReg, cg);
  movDependencies->addPostCondition(firstRegister->getLowOrder(),
                                    TR::RealRegister::NoReg, cg);
  movDependencies->addPostCondition(secondRegister->getLowOrder(),
                                    TR::RealRegister::NoReg, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairNegEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  ...
  generateRegImmInstruction(ADC4RegImms, node, targetRegister->getHighOrder(),
                            0, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairAbsEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *targetRegister = cg->longClobberEvaluate(child);
  node->setRegister(targetRegister);
  TR::Register *signRegister = cg->allocateRegister(TR_GPR);
  generateRegRegInstruction(MOV4RegReg, node, signRegister,
                            targetRegister->getHighOrder(), cg);
  generateRegImmInstruction(SAR4RegImm1, node, signRegister, 31, cg);
  generateRegRegInstruction(ADD4RegReg, node, targetRegister->getLowOrder(),
                            signRegister, cg);
  generateRegRegInstruction(ADC4RegReg, node, targetRegister->getHighOrder(),
                            signRegister, cg);
  generateRegRegInstruction(XOR4RegReg, node, targetRegister->getLowOrder(),
                            signRegister, cg);
  generateRegRegInstruction(XOR4RegReg, node, targetRegister->getHighOrder(),
                            signRegister, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairShlEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *targetRegister;
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (value == 0) {
      ...
      targetRegister = cg->allocateRegisterPair(cg->allocateRegister(),
                                                cg->allocateRegister());
      ...
      generateRegMemInstruction(LEA4RegMem, node, targetRegister->getLowOrder(),
                                tempMR, cg);
      ...
      if (value < 32) {
        ...
        generateRegImmInstruction(SHL4RegImm1, node,
                                  targetRegister->getLowOrder(), value, cg);
      } else {
        if (value != 32) {
          value -= 32;
        ...
        generateRegRegInstruction(XOR4RegReg, node, tempHighReg, tempHighReg,
                                  cg);
    ...
    generateRegRegInstruction(
        SHLD4RegRegCL, node, targetRegister->getHighOrder(),
        targetRegister->getLowOrder(), shiftDependencies, cg);
    generateRegInstruction(SHL4RegCL, node, targetRegister->getLowOrder(),
    ...
    generateRegMemInstruction(
        CMOVNE4RegMem, node, targetRegister->getLowOrder(),
        generateX86MemoryReference(cg->findOrCreate4ByteConstant(node, 0), cg),
        cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairRolEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *targetRegister;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (value != 0) {
      ...
      if (value != 0) {
        ...
        generateRegRegInstruction(MOV4RegReg, node, tempHighReg,
                                  targetRegister->getHighOrder(), cg);
        ...
        generateRegRegImmInstruction(SHLD4RegRegImm1, node,
                                     targetRegister->getLowOrder(), tempHighReg,
                                     value, cg);
    ...
    generateRegRegInstruction(MOV4RegReg, node, scratchReg,
                              targetRegister->getHighOrder(), cg);
    generateRegImmInstruction(TEST1RegImm1, node, shiftAmountReg, 32, cg);
    generateRegRegInstruction(CMOVNE4RegReg, node,
                              targetRegister->getHighOrder(),
                              targetRegister->getLowOrder(), cg);
    generateRegRegInstruction(CMOVNE4RegReg, node,
                              targetRegister->getLowOrder(), scratchReg, cg);
    ...
    generateRegRegInstruction(
        SHLD4RegRegCL, node, targetRegister->getHighOrder(),
        targetRegister->getLowOrder(), shiftDependencies, cg);
    generateRegRegInstruction(SHLD4RegRegCL, node,
                              targetRegister->getLowOrder(), scratchReg,
                              shiftDependencies, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairShrEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *targetRegister;
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (value == 0)
      ...
      generateRegImmInstruction(SAR4RegImm1, node,
                                targetRegister->getHighOrder(), value, cg);
    } else {
      if (value != 32) {
        value -= 32;
      ...
      generateRegRegInstruction(MOV4RegReg, node, targetRegister->getLowOrder(),
                                targetRegister->getHighOrder(), cg);
      generateRegImmInstruction(SAR4RegImm1, node,
                                targetRegister->getHighOrder(), 31, cg);
    }
    node->setRegister(targetRegister);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
    ...
    TR::Register *scratchReg = cg->allocateRegister();
    generateRegRegInstruction(
        SHRD4RegRegCL, node, targetRegister->getLowOrder(),
        targetRegister->getHighOrder(), shiftDependencies, cg);
    generateRegInstruction(SAR4RegCL, node, targetRegister->getHighOrder(),
                           shiftDependencies, cg);
    generateRegRegInstruction(MOV4RegReg, node, scratchReg,
                              targetRegister->getHighOrder(), cg);
    ...
    generateRegRegInstruction(CMOVNE4RegReg, node,
                              ...
                              targetRegister->getHighOrder(), cg);
    generateRegRegInstruction(CMOVNE4RegReg, node,
                              targetRegister->getHighOrder(), scratchReg, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairUshrEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *targetRegister;
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    generateRegRegInstruction(
        SHRD4RegRegCL, node, targetRegister->getLowOrder(),
        targetRegister->getHighOrder(), shiftDependencies, cg);
    generateRegInstruction(SHR4RegCL, node, targetRegister->getHighOrder(),
                           shiftDependencies, cg);
    generateRegImmInstruction(TEST1RegImm1, node, shiftAmountReg, 32, cg);
    generateRegRegInstruction(CMOVNE4RegReg, node,
                              targetRegister->getLowOrder(),
                              targetRegister->getHighOrder(), cg);
    generateRegMemInstruction(
        CMOVNE4RegMem, node, targetRegister->getHighOrder(),
        generateX86MemoryReference(cg->findOrCreate4ByteConstant(node, 0), cg),
        cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::landEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (!isMemOp) {
      ...
      if (firstChild->getReferenceCount() == 1) {
        ...
        if (lowValue != 0)
          generateRegRegInstruction(MOV4RegReg, node, lowReg,
                                    valueReg->getLowOrder(), cg);
        if (highValue != 0)
          generateRegRegInstruction(MOV4RegReg, node, highReg,
                                    valueReg->getHighOrder(), cg);
      }
    }
    ...
    if (lowValue == -1)
      ...
      if (isMemOp) {
        ...
        lowInstr =
            generateMemRegInstruction(S4MemReg, node, lowMR, tempReg, cg);
      } else
        lowInstr =
      ...
      if (isMemOp) {
        opCode =
            (lowValue >= -128 && lowValue <= 127) ? AND4MemImms : AND4MemImm4;
        lowInstr = generateMemImmInstruction(opCode, node, lowMR, lowValue, cg);
      } else {
        opCode =
            (lowValue >= -128 && lowValue <= 127) ? AND4RegImms : AND4RegImm4;
        lowInstr =
            generateRegImmInstruction(opCode, node, lowReg, lowValue, cg);
      }
    }
    ...
    if (highValue == -1)
      ...
      if (isMemOp) {
        ...
        highInstr =
            generateMemRegInstruction(S4MemReg, node, highMR, tempReg, cg);
      } else
        highInstr =
      ...
      if (isMemOp) {
        opCode =
            (highValue >= -128 && highValue <= 127) ? AND4MemImms : AND4MemImm4;
        highInstr =
            generateMemImmInstruction(opCode, node, highMR, highValue, cg);
      } else {
        opCode =
            (highValue >= -128 && highValue <= 127) ? AND4RegImms : AND4RegImm4;
        highInstr =
            generateRegImmInstruction(opCode, node, highReg, highValue, cg);
      }
    }
    ...
    lowInstr = generateMemRegInstruction(AND4MemReg, node, lowMR,
                                         tempReg->getLowOrder(), cg);
    highInstr = generateMemRegInstruction(AND4MemReg, node, highMR,
                                          tempReg->getHighOrder(), cg);
    ...
    temp.genericLongAnalyser(node, AND4RegReg, AND4RegReg, AND4RegMem,
                             AND2RegMem, AND1RegMem, AND4RegMem, MOV4RegReg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::lorEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (lowValue == 0)
      ...
      if (isMemOp) {
        opCode =
            (lowValue >= -128 && lowValue <= 127) ? OR4MemImms : OR4MemImm4;
        lowInstr = generateMemImmInstruction(opCode, node, lowMR, lowValue, cg);
      } else {
        opCode =
            (lowValue >= -128 && lowValue <= 127) ? OR4RegImms : OR4RegImm4;
        lowInstr =
            generateRegImmInstruction(opCode, node, lowReg, lowValue, cg);
      }
    }
    ...
    if (highValue == 0)
      ...
      if (isMemOp) {
        opCode =
            (highValue >= -128 && highValue <= 127) ? OR4MemImms : OR4MemImm4;
        highInstr =
            generateMemImmInstruction(opCode, node, highMR, highValue, cg);
      } else {
        opCode =
            (highValue >= -128 && highValue <= 127) ? OR4RegImms : OR4RegImm4;
        highInstr =
            generateRegImmInstruction(opCode, node, highReg, highValue, cg);
      }
    }
    ...
    lowInstr = generateMemRegInstruction(OR4MemReg, node, lowMR,
                                         tempReg->getLowOrder(), cg);
    highInstr = generateMemRegInstruction(OR4MemReg, node, highMR,
                                          tempReg->getHighOrder(), cg);
    ...
    temp.genericLongAnalyser(node, OR4RegReg, OR4RegReg, OR4RegMem, OR2RegMem,
                             OR1RegMem, OR4RegMem, MOV4RegReg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::lxorEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (lowValue == 0)
      ...
      if (isMemOp) {
        opCode =
            (lowValue >= -128 && lowValue <= 127) ? XOR4MemImms : XOR4MemImm4;
        lowInstr = generateMemImmInstruction(opCode, node, lowMR, lowValue, cg);
      } else {
        opCode =
            (lowValue >= -128 && lowValue <= 127) ? XOR4RegImms : XOR4RegImm4;
        lowInstr =
            generateRegImmInstruction(opCode, node, lowReg, lowValue, cg);
      }
    }
    ...
    if (highValue == 0)
      ...
      if (isMemOp) {
        opCode =
            (highValue >= -128 && highValue <= 127) ? XOR4MemImms : XOR4MemImm4;
        highInstr =
            generateMemImmInstruction(opCode, node, highMR, highValue, cg);
      } else {
        opCode =
            (highValue >= -128 && highValue <= 127) ? XOR4RegImms : XOR4RegImm4;
        highInstr =
            generateRegImmInstruction(opCode, node, highReg, highValue, cg);
      }
    }
    ...
    lowInstr = generateMemRegInstruction(XOR4MemReg, node, lowMR,
                                         tempReg->getLowOrder(), cg);
    highInstr = generateMemRegInstruction(XOR4MemReg, node, highMR,
                                          tempReg->getHighOrder(), cg);
    ...
    temp.genericLongAnalyser(node, XOR4RegReg, XOR4RegReg, XOR4RegMem,
                             XOR2RegMem, XOR1RegMem, XOR4RegMem, MOV4RegReg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::l2iEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *targetRegister;

  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    if (child->getReferenceCount() == 1) {
      ...
      targetRegister = cg->allocateRegister();
      generateRegRegInstruction(MOV4RegReg, node, targetRegister,
                                temp->getLowOrder(), cg);
    }
  }
  ...
  if (cg->enableRegisterInterferences() && node->getOpCode().getSize() == 1)
    cg->getLiveRegisters(TR_GPR)->setByteRegisterAssociation(
        node->getRegister());

  return targetRegister;
}

TR::Register *OMR::X86::I386::TreeEvaluator::ifacmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::acmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::lcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *targetRegister;
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      generateRegRegInstruction(OR4RegReg, node, targetRegister,
      ...
      compareGPRegisterToConstantForEquality(node, lowValue,
                                             cmpRegister->getLowOrder(), cg);
      targetRegister = cg->allocateRegister();
      if (cg->enableRegisterInterferences())
        cg->getLiveRegisters(TR_GPR)->setByteRegisterAssociation(
      ...
      compareGPRegisterToConstantForEquality(node, highValue,
                                             cmpRegister->getHighOrder(), cg);
      ...
      generateRegRegInstruction(AND1RegReg, node, targetRegister,
                                highTargetRegister, cg);
    ...
    TR_X86CompareAnalyser temp(cg);
    targetRegister =
        temp.longEqualityBooleanAnalyser(node, SETE1Reg, AND1RegReg);
  }
  return targetRegister;
}

TR::Register *OMR::X86::I386::TreeEvaluator::lcmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *targetRegister;
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      generateRegRegInstruction(OR1RegReg, node, targetRegister,
                                highTargetRegister, cg);
      cg->stopUsingRegister(highTargetRegister);
    }
    ...
    TR_X86CompareAnalyser temp(cg);
    targetRegister =
        temp.longEqualityBooleanAnalyser(node, SETNE1Reg, OR1RegReg);
  }
  return targetRegister;
}

TR::Register *OMR::X86::I386::TreeEvaluator::lcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETL1Reg, SETB1Reg, cg);
}

TR::Register *OMR::X86::I386::TreeEvaluator::lcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETG1Reg, SETAE1Reg, cg);
}

TR::Register *OMR::X86::I386::TreeEvaluator::lcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETG1Reg, SETA1Reg, cg);
}

TR::Register *OMR::X86::I386::TreeEvaluator::lcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETL1Reg, SETBE1Reg, cg);
}

TR::Register *OMR::X86::I386::TreeEvaluator::lucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETB1Reg, SETB1Reg, cg);
}

TR::Register *OMR::X86::I386::TreeEvaluator::lucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETA1Reg, SETAE1Reg, cg);
}

TR::Register *OMR::X86::I386::TreeEvaluator::lucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETA1Reg, SETA1Reg, cg);
}

TR::Register *OMR::X86::I386::TreeEvaluator::lucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareLongAndSetOrderedBoolean(node, SETB1Reg, SETBE1Reg, cg);
}

// also handles lucmp
TR::Register *OMR::X86::I386::TreeEvaluator::lcmpEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *targetRegister;
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    targetRegister = longArithmeticCompareRegisterWithImmediate(
        node, cg->evaluate(firstChild), secondChild, JAE4, JGE4, cg);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
  } else if (firstChild->getOpCodeValue() == TR::lconst &&
             firstChild->getRegister() == NULL) {
    targetRegister = longArithmeticCompareRegisterWithImmediate(
        node, cg->evaluate(secondChild), firstChild, JBE4, JLE4, cg);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
  } else {
    TR_X86CompareAnalyser temp(cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::lRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::i2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (machine->getVirtualAssociatedWithReal(TR::RealRegister::eax) ==
      childReg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::iu2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *childReg = cg->intClobberEvaluate(child);
  TR::Register *highReg = cg->allocateRegister();
...
TR::Register *OMR::X86::I386::TreeEvaluator::b2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *longReg;
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
      child->getReferenceCount() == 1) {
    TR::MemoryReference *tempMR = generateX86MemoryReference(child, cg);
    longReg = cg->allocateRegisterPair(cg->allocateRegister(),
                                       cg->allocateRegister());
    generateRegMemInstruction(MOVSXReg4Mem1, node, longReg->getLowOrder(),
                              tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    generateRegRegInstruction(MOVSXReg4Reg1, node, longReg->getLowOrder(),
                              longReg->getLowOrder(), cg);
  }

  generateRegRegInstruction(MOV4RegReg, node, longReg->getHighOrder(),
                            longReg->getLowOrder(), cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::bu2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *longReg;
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    longReg = cg->allocateRegisterPair(cg->allocateRegister(),
                                       cg->allocateRegister());
    generateRegMemInstruction(MOVZXReg4Mem1, node, longReg->getLowOrder(),
                              tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    generateRegRegInstruction(MOVZXReg4Reg1, node, longReg->getLowOrder(),
                              longReg->getLowOrder(), cg);
  }

  generateRegRegInstruction(XOR4RegReg, node, longReg->getHighOrder(),
...
TR::Register *OMR::X86::I386::TreeEvaluator::s2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *longReg;
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    longReg = cg->allocateRegisterPair(cg->allocateRegister(),
                                       cg->allocateRegister());
    node->setRegister(longReg);
    generateRegMemInstruction(MOVSXReg4Mem2, node, longReg->getLowOrder(),
                              tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    node->setRegister(longReg);
    generateRegRegInstruction(MOVSXReg4Reg2, node, longReg->getLowOrder(),
                              longReg->getLowOrder(), cg);
  }
...
TR::Register *OMR::X86::I386::TreeEvaluator::su2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *longReg;
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    longReg = cg->allocateRegisterPair(cg->allocateRegister(),
                                       cg->allocateRegister());
    node->setRegister(longReg);
    generateRegMemInstruction(MOVZXReg4Mem2, node, longReg->getLowOrder(),
                              tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    node->setRegister(longReg);
    generateRegRegInstruction(MOVZXReg4Reg2, node, longReg->getLowOrder(),
                              longReg->getLowOrder(), cg);
  }
...
TR::Register *OMR::X86::I386::TreeEvaluator::c2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *longReg;
  ...
  if ((valueChild->getOpCodeValue() == TR::lbits2d) &&
    ...
    if (valueChild->getOpCode().isLoadConst()) {
      instr = generateMemImmInstruction(
          S4MemImm4, node, generateX86MemoryReference(*storeLowMR, 4, cg),
          valueChild->getLongIntHigh(), cg);
      generateMemImmInstruction(S4MemImm4, node, storeLowMR,
                                valueChild->getLongIntLow(), cg);
      TR::Register *valueChildReg = valueChild->getRegister();
      if (valueChildReg && valueChildReg->getKind() == TR_X87 &&
          valueChild->getReferenceCount() == 1)
        instr = generateFPSTiST0RegRegInstruction(
            DSTRegReg, valueChild, valueChildReg, valueChildReg, cg);
      ...
      TR::MemoryReference *loadLowMR =
          generateX86MemoryReference(valueChild, cg);
      generateRegMemInstruction(L4RegMem, node, tempRegister,
                                generateX86MemoryReference(*loadLowMR, 4, cg),
                                cg);
      instr = generateMemRegInstruction(
          S4MemReg, node, generateX86MemoryReference(*storeLowMR, 4, cg),
          tempRegister, cg);
      ...
      if (sourceRegister->getKind() == TR_FPR) {
        TR_ASSERT(!sourceRegister->isSinglePrecision(),
                  "dstore cannot have float operand\n");
        instr = generateMemRegInstruction(MOVSDMemReg, node, storeLowMR,
                                          sourceRegister, cg);
      } else {
        instr = generateFPMemRegInstruction(DSTMemReg, node, storeLowMR,
                                            sourceRegister, cg);
      }
    }
...
TR::Register *OMR::X86::I386::TreeEvaluator::l2fEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    generateMemRegInstruction(S4MemReg, node, lowMR, longReg->getLowOrder(),
                              cg);
    generateMemRegInstruction(S4MemReg, node,
                              generateX86MemoryReference(*lowMR, 4, cg),
                              longReg->getHighOrder(), cg);
    generateFPRegMemInstruction(FLLDRegMem, node, target,
...
TR::Register *OMR::X86::I386::TreeEvaluator::l2dEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    generateMemRegInstruction(S4MemReg, node,
                              generateX86MemoryReference(*lowMR, 4, cg),
                              longReg->getHighOrder(), cg);
    generateFPRegMemInstruction(DLLDRegMem, node, target,
                                generateX86MemoryReference(*lowMR, 0, cg), cg);
    cg->decReferenceCount(child);
  }
...
TR::Register *OMR::X86::I386::TreeEvaluator::performLload(
  ...
  if (isVolatile || (symRef && symRef->isUnresolved())) {
    ...
    if (cg->useSSEForDoublePrecision() &&
        performTransformation(comp, "O^O Using SSE for volatile load %s\n",
                              cg->getDebug()->getName(node))) {
      TR_X86ProcessorInfo &p = cg->getX86ProcessorInfo();

      if (p.isGenuineIntel()) {
        TR::Register *xmmReg = cg->allocateRegister(TR_FPR);
        generateRegMemInstruction(cg->getXMMDoubleLoadOpCode(), node, xmmReg,
                                  sourceMR, cg);
      ...
      TR_ASSERT(cg->getX86ProcessorInfo().supportsCMPXCHG8BInstruction(),
                "Assumption of support of the CMPXCHG8B instruction failed in "
                "performLload()");
    ...
    lowRegister = loadMemory(node, sourceMR, TR_RematerializableInt,
                             node->getOpCode().isIndirect(), cg);
    highRegister =
        loadMemory(node, generateX86MemoryReference(*sourceMR, 4, cg),
                   TR_RematerializableInt, false, cg);
...
// also handles ilload
TR::Register *OMR::X86::I386::TreeEvaluator::lloadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::lbits2dEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadVar() &&
    ...
    generateMemRegInstruction(S4MemReg, node, tempMR, longReg->getLowOrder(),
                              cg);
    generateMemRegInstruction(S4MemReg, node,
                              generateX86MemoryReference(*tempMR, 4, cg),
                              longReg->getHighOrder(), cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::dbits2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadVar() &&
    ...
    generateRegMemInstruction(L4RegMem, node, highReg,
                              generateX86MemoryReference(*tempMR, 4, cg), cg);
    ...
    generateRegMemInstruction(L4RegMem, node, lowReg,
...
TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Node *secondChild = node->getSecondChild();
  TR::LabelSymbol *destinationLabel =
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      if (firstChild->getOpCodeValue() == TR::land &&
          firstChild->getReferenceCount() == 1 &&
          firstChild->getRegister() == NULL &&
          (landConstChild = firstChild->getSecondChild())->getOpCodeValue() ==
              TR::lconst &&
        ...
        if (landFirstChild->getReferenceCount() == 1 &&
          ...
          targetRegister = cg->allocateRegister();
          TR::MemoryReference *tempMR =
              generateX86MemoryReference(landFirstChild, cg);
          tempMR->getSymbolReference().addToOffset(4);
          generateRegMemInstruction(L4RegMem, landFirstChild, targetRegister,
                                    tempMR, cg);
        ...
        generateRegRegInstruction(TEST4RegReg, node, targetRegister,
                                  targetRegister, cg);
      ...
      // Evaluate the global register dependencies and emit the branches by
      // hand; we cannot call generateConditionalJumpInstruction() here because
      // we need to add extra post-conditions, and the conditions need to be
      // present on multiple instructions.
      //
      if (node->getNumChildren() == 3) {
        TR::Node *third = node->getChild(2);
        cg->evaluate(third);
        deps =
            generateRegisterDependencyConditions(third, cg, 2, &popRegisters);
    ...
    TR_X86CompareAnalyser temp(cg);
    temp.longEqualityCompareAndBranchAnalyser(node, NULL, destinationLabel,
                                              JE4);
  }
  return NULL;
}

TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Node *secondChild = node->getSecondChild();
  TR::LabelSymbol *destinationLabel =
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      if (node->getNumChildren() == 3) {
        ...
        cg->evaluate(third);
        deps =
            generateRegisterDependencyConditions(third, cg, 1, &popRegisters);
        deps->setMayNeedToPopFPRegisters(true);
        deps->stopAddingConditions();
    ...
    TR_X86CompareAnalyser temp(cg);
    temp.longEqualityCompareAndBranchAnalyser(node, destinationLabel,
                                              destinationLabel, JNE4);
  }
  return NULL;
}

TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (generateLAddOrSubForOverflowCheck(node, cg)) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR_X86OpCodes compareOp = node->getOpCode().isUnsigned() ? JA4 : JG4;
  TR_X86OpCodes reverseCompareOp = node->getOpCode().isUnsigned() ? JB4 : JL4;
...
TR::Register *OMR::X86::I386::TreeEvaluator::iflcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR_X86OpCodes compareOp = node->getOpCode().isUnsigned() ? JB4 : JL4;
  TR_X86OpCodes reverseCompareOp = node->getOpCode().isUnsigned() ? JA4 : JG4;
...
TR::Register *OMR::X86::I386::TreeEvaluator::lternaryEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (!longCompare && condOp.isCompareForEquality() &&
    ...
    if (condOp.isCompareTrueIfEqual()) {
      ...
      generateRegRegInstruction(
          CMOVNE4RegReg, node, trueReg->getRegisterPair()->getHighOrder(),
      ...
      generateRegRegInstruction(CMOVE4RegReg, node,
                                ...
                                falseReg->getRegisterPair()->getLowOrder(), cg);
      generateRegRegInstruction(
          CMOVE4RegReg, node, trueReg->getRegisterPair()->getHighOrder(),
          falseReg->getRegisterPair()->getHighOrder(), cg);
    }
    ...
    generateRegRegInstruction(
        (condOp.isCompareTrueIfEqual())
            ? ((condOp.isCompareTrueIfGreater()) ? CMOVL4RegReg : CMOVG4RegReg)
            : ((condOp.isCompareTrueIfGreater()) ? CMOVLE4RegReg
                                                 : CMOVGE4RegReg),
        node, trueReg->getRegisterPair()->getLowOrder(),
        falseReg->getRegisterPair()->getLowOrder(), cg);
    generateRegRegInstruction(
        (condOp.isCompareTrueIfEqual())
            ? ((condOp.isCompareTrueIfGreater()) ? CMOVL4RegReg : CMOVG4RegReg)
            : ((condOp.isCompareTrueIfGreater()) ? CMOVLE4RegReg
                                                 : CMOVGE4RegReg),
        node, trueReg->getRegisterPair()->getHighOrder(),
        falseReg->getRegisterPair()->getHighOrder(), cg);
  } else {
    TR::Register *condReg = cg->evaluate(condition);
    generateRegRegInstruction(TEST4RegReg, node, condReg, condReg,
                              cg);  // the condition is an int
    generateRegRegInstruction(CMOVE4RegReg, node,
                              trueReg->getRegisterPair()->getLowOrder(),
                              falseReg->getRegisterPair()->getLowOrder(), cg);
    generateRegRegInstruction(CMOVE4RegReg, node,
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairByteswapEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::integerPairMinMaxEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  generateRegRegInstruction(CMP4RegReg, node, operand0->getLowOrder(),
                            operand1->getLowOrder(), cg);
  generateRegInstruction(SETccLo, node, result->getLowOrder(),
                         cg);  // t1 = (low0 < low1)
  generateRegRegInstruction(CMP4RegReg, node, operand0->getHighOrder(),
                            operand1->getHighOrder(), cg);
  generateRegInstruction(SETccHi, node, result->getHighOrder(),
                         cg);  // t2 = (high0 < high1)
  generateRegRegInstruction(
      CMOVE4RegReg, node, result->getHighOrder(), result->getLowOrder(),
      cg);  // if (high0 == high1) then t2 = t1 = (low0 < low1)

  generateRegRegInstruction(TEST1RegReg, node, result->getHighOrder(),
                            result->getHighOrder(), cg);
  generateRegRegInstruction(MOV4RegReg, node, result->getLowOrder(),
                            operand0->getLowOrder(), cg);
  generateRegRegInstruction(MOV4RegReg, node, result->getHighOrder(),
                            operand0->getHighOrder(), cg);
  generateRegRegInstruction(CMOVE4RegReg, node, result->getLowOrder(),
                            operand1->getLowOrder(), cg);
  generateRegRegInstruction(CMOVE4RegReg, node, result->getHighOrder(),
                            operand1->getHighOrder(), cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::lcmpsetEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR::Register *compareReg = cg->longClobberEvaluate(
      compareValue);  // clobber evaluate because edx:eax may potentially get
                      // clobbered
  ...
  generateMemInstruction(
      TR::Compiler->target.isSMP() ? LCMPXCHG8BMem : CMPXCHG8BMem, node, memRef,
      deps, cg);
...
TR::Register *OMR::X86::I386::TreeEvaluator::awrtbarEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::I386::TreeEvaluator::awrtbariEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // The wrtbar IL op represents a store with side effects.
  // Currently we don't use the side effect node. So just evaluate it and
  // decrement the reference count.
...
TR::Register *OMR::X86::I386::TreeEvaluator::dwrtbarEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // The wrtbar IL op represents a store with side effects.
  // Currently we don't use the side effect node. So just evaluate it and
  // decrement the reference count.
...
TR::Register *OMR::X86::I386::TreeEvaluator::dwrtbariEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // The wrtbar IL op represents a store with side effects.
  // Currently we don't use the side effect node. So just evaluate it and
  // decrement the reference count.
