  ...
  TR::CodeGenerator* cg)
{
  ...
  compareGPRegisterToImmediate(
    node, cmpRegister->getHighOrder(), highValue, cg);
  ...
  deps->addPostCondition(
    cmpRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  deps->addPostCondition(
    cmpRegister->getHighOrder(), TR::RealRegister::NoReg, cg);
  ...
  generateRegRegInstruction(
    MOVSXReg4Reg1, node, targetRegister, targetRegister, cg);

  return targetRegister;
}

TR::Register*
OMR::X86::I386::TreeEvaluator::compareLongAndSetOrderedBoolean(
  TR::Node* node,
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL &&
    ...
    compareGPRegisterToImmediate(
      node, cmpRegister->getLowOrder(), lowValue, cg);
...
void
OMR::X86::I386::TreeEvaluator::compareLongsForOrder(
  TR::Node* node,
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL &&
    ...
    if (node->getNumChildren() == 3) {
      ...
      generateLabelInstruction(
        highOrderBranchOp, node, destinationLabel, deps, cg);
      generateLabelInstruction(JNE4, node, doneLabel, deps, cg);
      compareGPRegisterToImmediate(
        node, cmpRegister->getLowOrder(), lowValue, cg);
      generateLabelInstruction(
        lowOrderBranchOp, node, destinationLabel, deps, cg);
    } else {
                                               ...
                                               TR::CodeGenerator* cg)
{
  ...
  if (abs(lowValue - highValue) <= 128) {
    if (lowValue <= highValue) {
      ...
      if (lowValue == highValue) {
        generateRegRegInstruction(
          MOV4RegReg, node, lowRegister, highRegister, cg);
      } else {
      ...
      if (isVolatile) {
        if (cg->useSSEForDoublePrecision() &&
            performTransformation(comp,
                                  "O^O Using SSE for volatile store %s\n",
          ...
          instr = generateMemRegInstruction(
            S4MemReg, node, stackLow, valueReg->getLowOrder(), cg);
          // generate: stack1_plus4 <- (valueReg->gethighorder())
          generateMemRegInstruction(
            S4MemReg, node, stackHigh, valueReg->getHighOrder(), cg);
          ...
          generateRegMemInstruction(
            cg->getXMMDoubleLoadOpCode(), node, reg, stack, cg);
        ...
        instr = generateMemRegInstruction(
          S4MemReg, node, lowMR, valueReg->getLowOrder(), cg);
        generateMemRegInstruction(
          S4MemReg, node, highMR, valueReg->getHighOrder(), cg);
...
TR::Register*
OMR::X86::I386::TreeEvaluator::integerPairReturnEvaluator(TR::Node* node,
                                                          TR::CodeGenerator* cg)
{
  ...
  if (cg->enableSinglePrecisionMethods() &&
    ...
    generateMemInstruction(
      ...
      generateX86MemoryReference(
        cg->findOrCreate2ByteConstant(node, DOUBLE_PRECISION_ROUND_TO_NEAREST),
        cg),
      cg);
  }
  ...
  if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (isMemOp) {
      ...
      generateRegImmInstruction(
        opCode, node, targetRegister->getHighOrder(), highValue, cg);
    }
  } else if (isMemOp && !needsEflags) {
    TR::Register* tempReg = cg->evaluate(secondChild);
    instr = generateMemRegInstruction(
      ADD4MemReg, node, lowMR, tempReg->getLowOrder(), cg);
    generateMemRegInstruction(
      ADC4MemReg, node, highMR, tempReg->getHighOrder(), cg);
                                                       ...
                                                       TR::CodeGenerator* cg)
{
  ...
  if (!needsEflags && secondChild->getOpCodeValue() == TR::lconst &&
    ...
    TR::Register* tempReg = cg->evaluate(secondChild);
    instr = generateMemRegInstruction(
      SUB4MemReg, node, lowMR, tempReg->getLowOrder(), cg);
    generateMemRegInstruction(
      SBB4MemReg, node, highMR, tempReg->getHighOrder(), cg);
...
TR::Register*
OMR::X86::I386::TreeEvaluator::integerPairDualMulEvaluator(
  TR::Node* node,
  TR::CodeGenerator* cg)
{
                                                       ...
                                                       TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (lowValue == 0 && highValue == 0) {
      ...
      highRegister = cg->allocateRegister();
      generateRegRegInstruction(
        XOR4RegReg, node, highRegister, highRegister, cg);
      ...
      if (absValue != highValue) {
        generateRegInstruction(
          NEG4Reg, node, targetRegister->getLowOrder(), cg);
        generateRegImmInstruction(
          ADC4RegImms, node, targetRegister->getHighOrder(), 0, cg);
        generateRegInstruction(
          NEG4Reg, node, targetRegister->getHighOrder(), cg);
      }
      return targetRegister;
    } else if (lowValue == 0) {
        ...
        if (highRegister == 0) // cannot do the decomposition
        {
          if (firstChild->getReferenceCount() > 1 ||
            ...
            generateRegRegImmInstruction(
              opCode, node, highRegister, multiplierRegister, highValue, cg);
          } else {
            if (highValue >= -128 && highValue <= 127) {
            ...
            generateRegMemImmInstruction(
              opCode, node, highRegister, nodeMR, highValue, cg);
          }
        }
      }
      lowRegister = cg->allocateRegister();
      ...
      generateRegRegInstruction(
        opCode, node, lowRegister, sourceLow, dependencies, cg);
      if (!firstIU2L && !intMul) {
        generateRegRegInstruction(TEST4RegReg,
                                  node,
        ...
        generateRegRegInstruction(
          ADD4RegReg, node, highRegister, tempRegister, cg);
        ...
        deps->addPreCondition(
          multiplierRegister->getHighOrder(), TR::RealRegister::NoReg, cg);
        int i;
      ...
      if (absValue == 3 || absValue == 5 || absValue == 9) {
        ...
        TR_X86IntegerMultiplyDecomposer* mulDecomposer =
          new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer(
            ...
            node,
            cg,
            (firstChild->getReferenceCount() == 1 && highRegister != sourceLow)
              ? true
              : false);
        ...
        if (tempRegister == 0) {
          ...
          if (firstChild->getReferenceCount() > 1 ||
            ...
            generateRegRegImmInstruction(
              opCode, node, tempRegister, sourceLow, highValue, cg);
          } else {
            if (highValue >= -128 && highValue <= 127) {
              opCode = IMUL4RegMemImms;
            } else {
            ...
            nodeMR = generateX86MemoryReference(firstChild, cg);
            generateRegMemImmInstruction(
              opCode, node, tempRegister, nodeMR, highValue, cg);
          }
        }
      }
                                                       ...
                                                       TR::CodeGenerator* cg)
{
  ...
  idivDependencies->addPreCondition(
    firstRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  idivDependencies->addPreCondition(
    secondRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  idivDependencies->addPostCondition(
    firstRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  idivDependencies->addPostCondition(
    secondRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  ...
  generateRegRegInstruction(
    MOV4RegReg, node, lowRegister, firstRegister->getLowOrder(), cg);
  divInstr = generateRegRegInstruction(DIV4AccReg,
                                       node,
                                       lowRegister,
  ...
  generateRegRegInstruction(
    XOR4RegReg, node, highRegister, highRegister, xorDependencies1, cg);
  ...
  labelDependencies->addPreCondition(
    firstRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  labelDependencies->addPreCondition(
    secondRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  labelDependencies->addPostCondition(
    firstRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  labelDependencies->addPostCondition(
    secondRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
                                                       ...
                                                       TR::CodeGenerator* cg)
{
  ...
  movDependencies->addPreCondition(
    firstRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  movDependencies->addPreCondition(
    secondRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  movDependencies->addPostCondition(
    firstRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
  movDependencies->addPostCondition(
    secondRegister->getLowOrder(), TR::RealRegister::NoReg, cg);
                                                       ...
                                                       TR::CodeGenerator* cg)
{
  ...
  generateRegRegInstruction(
    MOV4RegReg, node, signRegister, targetRegister->getHighOrder(), cg);
  generateRegImmInstruction(SAR4RegImm1, node, signRegister, 31, cg);
  generateRegRegInstruction(
    ADD4RegReg, node, targetRegister->getLowOrder(), signRegister, cg);
  generateRegRegInstruction(
    ADC4RegReg, node, targetRegister->getHighOrder(), signRegister, cg);
  generateRegRegInstruction(
    XOR4RegReg, node, targetRegister->getLowOrder(), signRegister, cg);
  generateRegRegInstruction(
    XOR4RegReg, node, targetRegister->getHighOrder(), signRegister, cg);
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (value == 0) {
      ...
      generateRegRegInstruction(MOV4RegReg,
                                ...
                                tempRegister->getHighOrder(),
                                cg);
      TR::MemoryReference* tempMR = generateX86MemoryReference(cg);
      tempMR->setIndexRegister(tempRegister->getLowOrder());
      tempMR->setStride(value);
      generateRegMemInstruction(
        LEA4RegMem, node, targetRegister->getLowOrder(), tempMR, cg);
      generateRegRegImmInstruction(SHLD4RegRegImm1,
                                   node,
                                   targetRegister->getHighOrder(),
                                   tempRegister->getLowOrder(),
                                   value,
                                   cg);
    } else {
      targetRegister = cg->longClobberEvaluate(firstChild);
      if (value < 32) {
        generateRegRegImmInstruction(SHLD4RegRegImm1,
                                     ...
                                     cg);
        generateRegImmInstruction(
          SHL4RegImm1, node, targetRegister->getLowOrder(), value, cg);
      } else {
        if (value != 32) {
          value -= 32;
          generateRegImmInstruction(
        ...
        generateRegRegInstruction(
          XOR4RegReg, node, tempHighReg, tempHighReg, cg);
    ...
    generateRegRegInstruction(SHLD4RegRegCL,
                              node,
                              targetRegister->getHighOrder(),
                              targetRegister->getLowOrder(),
                              shiftDependencies,
                              cg);
    generateRegInstruction(
      SHL4RegCL, node, targetRegister->getLowOrder(), shiftDependencies, cg);
    generateRegImmInstruction(TEST1RegImm1, node, shiftAmountReg, 32, cg);
    generateRegRegInstruction(CMOVNE4RegReg,
                              node,
                              targetRegister->getHighOrder(),
                              targetRegister->getLowOrder(),
                              cg);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (value != 0) {
      ...
      if (value != 0) {
        ...
        generateRegRegInstruction(
          MOV4RegReg, node, tempHighReg, targetRegister->getHighOrder(), cg);
    ...
    generateRegRegInstruction(
      MOV4RegReg, node, scratchReg, targetRegister->getHighOrder(), cg);
    generateRegImmInstruction(TEST1RegImm1, node, shiftAmountReg, 32, cg);
    generateRegRegInstruction(CMOVNE4RegReg,
                              node,
                              targetRegister->getHighOrder(),
                              ...
                              cg);
    generateRegRegInstruction(
      CMOVNE4RegReg, node, targetRegister->getLowOrder(), scratchReg, cg);
                                                       ...
                                                       TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (value == 0)
      ...
      generateRegRegImmInstruction(SHRD4RegRegImm1,
                                   node,
                                   targetRegister->getLowOrder(),
                                   targetRegister->getHighOrder(),
                                   value,
                                   cg);
      generateRegImmInstruction(
        SAR4RegImm1, node, targetRegister->getHighOrder(), value, cg);
    } else {
      if (value != 32) {
        value -= 32;
        generateRegImmInstruction(
      ...
      generateRegRegInstruction(MOV4RegReg,
                                ...
                                cg);
      generateRegImmInstruction(
        SAR4RegImm1, node, targetRegister->getHighOrder(), 31, cg);
    }
    node->setRegister(targetRegister);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
    ...
    TR::Register* scratchReg = cg->allocateRegister();
    generateRegRegInstruction(SHRD4RegRegCL,
                              node,
                              targetRegister->getLowOrder(),
                              targetRegister->getHighOrder(),
                              shiftDependencies,
                              cg);
    generateRegInstruction(
      SAR4RegCL, node, targetRegister->getHighOrder(), shiftDependencies, cg);
    generateRegRegInstruction(
      MOV4RegReg, node, scratchReg, targetRegister->getHighOrder(), cg);
    ...
    generateRegRegInstruction(CMOVNE4RegReg,
                              ...
                              cg);
    generateRegRegInstruction(
      CMOVNE4RegReg, node, targetRegister->getHighOrder(), scratchReg, cg);
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (value < 32) {
      generateRegRegImmInstruction(SHRD4RegRegImm1,
                                   ...
                                   cg);
      generateRegImmInstruction(
        SHR4RegImm1, node, targetRegister->getHighOrder(), value, cg);
    } else {
      if (value != 32) {
        value -= 32;
        generateRegImmInstruction(
    ...
    generateRegRegInstruction(SHRD4RegRegCL,
                              ...
                              cg);
    generateRegInstruction(
      SHR4RegCL, node, targetRegister->getHighOrder(), shiftDependencies, cg);
    generateRegImmInstruction(TEST1RegImm1, node, shiftAmountReg, 32, cg);
    generateRegRegInstruction(CMOVNE4RegReg,
                              node,
                              targetRegister->getLowOrder(),
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if (!isMemOp) {
      ...
      if (firstChild->getReferenceCount() == 1) {
        ...
        if (lowValue != 0)
          generateRegRegInstruction(
            MOV4RegReg, node, lowReg, valueReg->getLowOrder(), cg);
        if (highValue != 0)
          generateRegRegInstruction(
            MOV4RegReg, node, highReg, valueReg->getHighOrder(), cg);
      }
    }
    ...
    lowInstr = generateMemRegInstruction(
      AND4MemReg, node, lowMR, tempReg->getLowOrder(), cg);
    highInstr = generateMemRegInstruction(
      AND4MemReg, node, highMR, tempReg->getHighOrder(), cg);
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    lowInstr = generateMemRegInstruction(
      OR4MemReg, node, lowMR, tempReg->getLowOrder(), cg);
    highInstr = generateMemRegInstruction(
      OR4MemReg, node, highMR, tempReg->getHighOrder(), cg);
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    lowInstr = generateMemRegInstruction(
      XOR4MemReg, node, lowMR, tempReg->getLowOrder(), cg);
    highInstr = generateMemRegInstruction(
      XOR4MemReg, node, highMR, tempReg->getHighOrder(), cg);
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    if (child->getReferenceCount() == 1) {
      ...
      targetRegister = cg->allocateRegister();
      generateRegRegInstruction(
        MOV4RegReg, node, targetRegister, temp->getLowOrder(), cg);
    }
  }
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      if (firstChild->getReferenceCount() != 1) {
        targetRegister = cg->allocateRegister();
        generateRegRegInstruction(
          MOV4RegReg, node, targetRegister, cmpRegister->getLowOrder(), cg);
      }
      generateRegRegInstruction(
        OR4RegReg, node, targetRegister, cmpRegister->getHighOrder(), cg);
      ...
      compareGPRegisterToConstantForEquality(
        node, lowValue, cmpRegister->getLowOrder(), cg);
      targetRegister = cg->allocateRegister();
      if (cg->enableRegisterInterferences())
        cg->getLiveRegisters(TR_GPR)->setByteRegisterAssociation(
          targetRegister);
      ...
      compareGPRegisterToConstantForEquality(
        node, highValue, cmpRegister->getHighOrder(), cg);
      ...
      generateRegRegInstruction(
        AND1RegReg, node, targetRegister, highTargetRegister, cg);
                                               ...
                                               TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      generateRegRegInstruction(
        OR1RegReg, node, targetRegister, highTargetRegister, cg);
      cg->stopUsingRegister(highTargetRegister);
    }
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    longReg =
      cg->allocateRegisterPair(cg->allocateRegister(), cg->allocateRegister());
    generateRegMemInstruction(
      MOVSXReg4Mem1, node, longReg->getLowOrder(), tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    generateRegRegInstruction(
      MOVSXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
  }

  generateRegRegInstruction(
    MOV4RegReg, node, longReg->getHighOrder(), longReg->getLowOrder(), cg);
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    longReg =
      cg->allocateRegisterPair(cg->allocateRegister(), cg->allocateRegister());
    generateRegMemInstruction(
      MOVZXReg4Mem1, node, longReg->getLowOrder(), tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    generateRegRegInstruction(
      MOVZXReg4Reg1, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
  }

  generateRegRegInstruction(
    XOR4RegReg, node, longReg->getHighOrder(), longReg->getHighOrder(), cg);
  node->setRegister(longReg);
  cg->decReferenceCount(child);
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    node->setRegister(longReg);
    generateRegMemInstruction(
      MOVSXReg4Mem2, node, longReg->getLowOrder(), tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    node->setRegister(longReg);
    generateRegRegInstruction(
      MOVSXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
  }
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (child->getOpCode().isLoadVar() && child->getRegister() == NULL &&
    ...
    node->setRegister(longReg);
    generateRegMemInstruction(
      MOVZXReg4Mem2, node, longReg->getLowOrder(), tempMR, cg);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    longReg = cg->allocateRegisterPair(cg->intClobberEvaluate(child),
                                       cg->allocateRegister());
    node->setRegister(longReg);
    generateRegRegInstruction(
      MOVZXReg4Reg2, node, longReg->getLowOrder(), longReg->getLowOrder(), cg);
  }
  ...
  if ((valueChild->getOpCodeValue() == TR::lbits2d) &&
    ...
    if (valueChild->getOpCode().isLoadConst()) {
      instr = generateMemImmInstruction(
        S4MemImm4,
        node,
        generateX86MemoryReference(*storeLowMR, 4, cg),
        valueChild->getLongIntHigh(),
        cg);
      generateMemImmInstruction(
        S4MemImm4, node, storeLowMR, valueChild->getLongIntLow(), cg);
      ...
      if (sourceRegister->getKind() == TR_FPR) {
        ...
        instr = generateMemRegInstruction(
          MOVSDMemReg, node, storeLowMR, sourceRegister, cg);
      } else {
        instr = generateFPMemRegInstruction(
          DSTMemReg, node, storeLowMR, sourceRegister, cg);
      }
    }
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    generateMemRegInstruction(
      S4MemReg, node, lowMR, longReg->getLowOrder(), cg);
    generateMemRegInstruction(S4MemReg,
                              node,
                              generateX86MemoryReference(*lowMR, 4, cg),
                              longReg->getHighOrder(),
                              cg);
    generateFPRegMemInstruction(
      FLLDRegMem, node, target, generateX86MemoryReference(*lowMR, 0, cg), cg);
    cg->decReferenceCount(child);
  }
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    generateMemRegInstruction(S4MemReg,
                              ...
                              cg);
    generateFPRegMemInstruction(
      DLLDRegMem, node, target, generateX86MemoryReference(*lowMR, 0, cg), cg);
    cg->decReferenceCount(child);
  }
...
TR::Register*
OMR::X86::I386::TreeEvaluator::performLload(TR::Node* node,
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (isVolatile || (symRef && symRef->isUnresolved())) {
    ...
    if (cg->useSSEForDoublePrecision() &&
        performTransformation(comp,
                              "O^O Using SSE for volatile load %s\n",
                              cg->getDebug()->getName(node))) {
      ...
      if (p.isGenuineIntel()) {
        ...
        generateRegMemInstruction(
          cg->getXMMDoubleLoadOpCode(), node, xmmReg, sourceMR, cg);
        ...
        generateRegMemInstruction(
          cg->getXMMDoubleLoadOpCode(), node, reg, sourceMR, cg);
        // generate: stack1 <- xmm
        TR::MemoryReference* stack =
          generateX86MemoryReference(*stackLow, 0, cg);
    ...
    lowRegister = loadMemory(node,
                             sourceMR,
                             TR_RematerializableInt,
                             node->getOpCode().isIndirect(),
                             cg);
    highRegister = loadMemory(node,
                              generateX86MemoryReference(*sourceMR, 4, cg),
                              TR_RematerializableInt,
                              false,
                              cg);
                                                ...
                                                TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadVar() &&
    ...
    generateMemRegInstruction(
      S4MemReg, node, tempMR, longReg->getLowOrder(), cg);
    generateMemRegInstruction(S4MemReg,
                              node,
                                                ...
                                                TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadVar() &&
    ...
    generateRegMemInstruction(
      L4RegMem, node, highReg, generateX86MemoryReference(*tempMR, 4, cg), cg);
    ...
    generateRegMemInstruction(
      L4RegMem, node, lowReg, generateX86MemoryReference(*tempMR, 0, cg), cg);
    generateRegMemInstruction(
      L4RegMem, node, highReg, generateX86MemoryReference(*tempMR, 4, cg), cg);
  }
                                                 ...
                                                 TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    if ((lowValue | highValue) == 0) {
      ...
      if (firstChild->getOpCodeValue() == TR::land &&
        ...
        if (landFirstChild->getReferenceCount() == 1 &&
          ...
          generateRegMemInstruction(
            L4RegMem, landFirstChild, targetRegister, tempMR, cg);
        ...
        generateRegRegInstruction(
          TEST4RegReg, node, targetRegister, targetRegister, cg);
    ...
    TR_X86CompareAnalyser temp(cg);
    temp.longEqualityCompareAndBranchAnalyser(
      node, NULL, destinationLabel, JE4);
  }
  return NULL;
}
                                                 ...
                                                 TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst &&
    ...
    TR_X86CompareAnalyser temp(cg);
    temp.longEqualityCompareAndBranchAnalyser(
      node, destinationLabel, destinationLabel, JNE4);
  }
  return NULL;
}
  ...
  if (!longCompare && condOp.isCompareForEquality() &&
    ...
    if (condOp.isCompareTrueIfEqual()) {
      generateRegRegInstruction(CMOVNE4RegReg,
                                ...
                                falseReg->getRegisterPair()->getLowOrder(),
                                cg);
      generateRegRegInstruction(CMOVNE4RegReg,
                                node,
                                trueReg->getRegisterPair()->getHighOrder(),
                                falseReg->getRegisterPair()->getHighOrder(),
                                cg);
    } else {
      generateRegRegInstruction(CMOVE4RegReg,
                                node,
                                trueReg->getRegisterPair()->getLowOrder(),
                                falseReg->getRegisterPair()->getLowOrder(),
                                cg);
    ...
    generateRegRegInstruction(
      (condOp.isCompareTrueIfEqual())
        ...
        : ((condOp.isCompareTrueIfGreater()) ? CMOVLE4RegReg : CMOVGE4RegReg),
      node,
      trueReg->getRegisterPair()->getLowOrder(),
      falseReg->getRegisterPair()->getLowOrder(),
      cg);
    ...
    generateRegRegInstruction(
      TEST4RegReg, node, condReg, condReg, cg); // the condition is an int
    generateRegRegInstruction(CMOVE4RegReg,
                              node,
                              trueReg->getRegisterPair()->getLowOrder(),
                              falseReg->getRegisterPair()->getLowOrder(),
...
TR::Register*
OMR::X86::I386::TreeEvaluator::integerPairByteswapEvaluator(
  TR::Node* node,
  TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::I386::TreeEvaluator::integerPairMinMaxEvaluator(TR::Node* node,
                                                          TR::CodeGenerator* cg)
{
  ...
  generateRegRegInstruction(
    CMP4RegReg, node, operand0->getLowOrder(), operand1->getLowOrder(), cg);
  generateRegInstruction(
    SETccLo, node, result->getLowOrder(), cg); // t1 = (low0 < low1)
  generateRegRegInstruction(
    CMP4RegReg, node, operand0->getHighOrder(), operand1->getHighOrder(), cg);
  generateRegInstruction(
    SETccHi, node, result->getHighOrder(), cg); // t2 = (high0 < high1)
  generateRegRegInstruction(
    CMOVE4RegReg,
    node,
    result->getHighOrder(),
  ...
  generateRegRegInstruction(
    TEST1RegReg, node, result->getHighOrder(), result->getHighOrder(), cg);
  generateRegRegInstruction(
    MOV4RegReg, node, result->getLowOrder(), operand0->getLowOrder(), cg);
  generateRegRegInstruction(
    MOV4RegReg, node, result->getHighOrder(), operand0->getHighOrder(), cg);
  generateRegRegInstruction(
    CMOVE4RegReg, node, result->getLowOrder(), operand1->getLowOrder(), cg);
  generateRegRegInstruction(
    CMOVE4RegReg, node, result->getHighOrder(), operand1->getHighOrder(), cg);
