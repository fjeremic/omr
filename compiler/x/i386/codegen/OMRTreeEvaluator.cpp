...
TR::Register* OMR::X86::I386::TreeEvaluator::longArithmeticCompareRegisterWithImmediate(TR::Node* node,
  TR::Register* cmpRegister, TR::Node* immedChild, TR_X86OpCodes firstBranchOpCode, TR_X86OpCodes secondBranchOpCode,
  TR::CodeGenerator* cg)
{
...
TR::Register* OMR::X86::I386::TreeEvaluator::compareLongAndSetOrderedBoolean(
  TR::Node* node, TR_X86OpCodes highSetOpCode, TR_X86OpCodes lowSetOpCode, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL
    && performTransformation(comp,
      "O^O compareLongAndSetOrderedBoolean: checking that the second child node does not have an assigned register: "
      "%d\n",
      testRegister)) {
...
void OMR::X86::I386::TreeEvaluator::compareLongsForOrder(TR::Node* node, TR_X86OpCodes highOrderBranchOp,
  TR_X86OpCodes highOrderReversedBranchOp, TR_X86OpCodes lowOrderBranchOp, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* testRegister = secondChild->getRegister();
    ...
    if (lowValue <= highValue) {
      ...
      if (lowValue == highValue) {
        ...
        generateRegMemInstruction(
          LEA4RegMem, node, lowRegister, generateX86MemoryReference(highRegister, lowValue - highValue, cg), cg);
      }
    } else {
      lowRegister = loadConstant(node, lowValue, TR_RematerializableInt, cg);
      highRegister = cg->allocateRegister();
      generateRegMemInstruction(
        LEA4RegMem, node, highRegister, generateX86MemoryReference(lowRegister, highValue - lowValue, cg), cg);
    }
  } else {
    lowRegister = loadConstant(node, lowValue, TR_RematerializableInt, cg);
    highRegister = loadConstant(node, highValue, TR_RematerializableInt, cg);
      ...
      if (isVolatile) {
        if (cg->useSSEForDoublePrecision()
          ...
          generateMemInstruction(
            TR::Compiler->target.isSMP() ? LCMPXCHG8BMem : CMPXCHG8BMem, node, cmpxchgMR, deps, cg);
        ...
        && (!comp->getOption(TR_DisableNewX86VolatileSupport) && TR::Compiler->target.is32Bit())) {
        TR_ASSERT(cg->getX86ProcessorInfo().supportsCMPXCHG8BInstruction(),
          "Assumption of support of the CMPXCHG8B instruction failed in lstoreEvaluator()");
        eaxReg = cg->allocateRegister();
        edxReg = cg->allocateRegister();
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    if (lowValue == 0 && highValue == 0) {
      ...
      if (absValue == 3 || absValue == 5 || absValue == 9) {
        ...
        int32_t dummy = 0;
        TR_X86IntegerMultiplyDecomposer* mulDecomposer = new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer(
          (int64_t)absValue, multiplierRegister, node, cg, firstChild->getReferenceCount() == 1 ? true : false);
      ...
      if (!firstIU2L && !intMul) {
        generateRegRegInstruction(
          TEST4RegReg, node, multiplierRegister->getHighOrder(), multiplierRegister->getHighOrder(), cg);
        TR::LabelSymbol* startLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        TR::LabelSymbol* doneLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
        startLabel->setStartInternalControlFlow();
        doneLabel->setEndInternalControlFlow();
        ...
        TR_X86IntegerMultiplyDecomposer* mulDecomposer
          = new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer((int64_t)lowValue,
            multiplierRegister->getHighOrder(), node, cg, firstChild->getReferenceCount() == 1 ? true : false);
        ...
        if (tempRegister == 0) // decomposition failed
        {
          ...
          generateRegRegImmInstruction(opCode, node, tempRegister, multiplierRegister->getHighOrder(), lowValue, cg);
        }
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairDivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  divInstr
    = generateRegRegInstruction(DIV4AccReg, node, lowRegister, secondRegister->getLowOrder(), idivDependencies, cg);
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (value == 0) {
      ...
      generateRegRegImmInstruction(
        SHLD4RegRegImm1, node, targetRegister->getHighOrder(), tempRegister->getLowOrder(), value, cg);
    } else {
      targetRegister = cg->longClobberEvaluate(firstChild);
      if (value < 32) {
        generateRegRegImmInstruction(
          SHLD4RegRegImm1, node, targetRegister->getHighOrder(), targetRegister->getLowOrder(), value, cg);
    ...
    generateRegRegInstruction(
      SHLD4RegRegCL, node, targetRegister->getHighOrder(), targetRegister->getLowOrder(), shiftDependencies, cg);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (value != 0) {
      ...
      if (value != 0) {
        ...
        generateRegRegImmInstruction(SHLD4RegRegImm1, node, targetRegister->getLowOrder(), tempHighReg, value, cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairShrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::iconst) {
    ...
    if (value == 0)
      ...
      generateRegRegImmInstruction(
        SHRD4RegRegImm1, node, targetRegister->getLowOrder(), targetRegister->getHighOrder(), value, cg);
    ...
    TR::Register* scratchReg = cg->allocateRegister();
    generateRegRegInstruction(
      SHRD4RegRegCL, node, targetRegister->getLowOrder(), targetRegister->getHighOrder(), shiftDependencies, cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
    ...
    temp.genericLongAnalyser(node, AND4RegReg, AND4RegReg, AND4RegMem, AND2RegMem, AND1RegMem, AND4RegMem, MOV4RegReg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
    ...
    temp.genericLongAnalyser(node, OR4RegReg, OR4RegReg, OR4RegMem, OR2RegMem, OR1RegMem, OR4RegMem, MOV4RegReg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
    ...
    temp.genericLongAnalyser(node, XOR4RegReg, XOR4RegReg, XOR4RegMem, XOR2RegMem, XOR1RegMem, XOR4RegMem, MOV4RegReg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::lcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst && secondChild->getRegister() == NULL) {
    targetRegister
      = longArithmeticCompareRegisterWithImmediate(node, cg->evaluate(firstChild), secondChild, JAE4, JGE4, cg);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
  } else if (firstChild->getOpCodeValue() == TR::lconst && firstChild->getRegister() == NULL) {
    targetRegister
      = longArithmeticCompareRegisterWithImmediate(node, cg->evaluate(secondChild), firstChild, JBE4, JLE4, cg);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
  } else {
    TR_X86CompareAnalyser temp(cg);
  ...
  if ((valueChild->getOpCodeValue() == TR::lbits2d) && !valueChild->getRegister()) {
    ...
    if (valueChild->getOpCode().isLoadConst()) {
      instr = generateMemImmInstruction(
        S4MemImm4, node, generateX86MemoryReference(*storeLowMR, 4, cg), valueChild->getLongIntHigh(), cg);
      ...
      instr
        = generateMemRegInstruction(S4MemReg, node, generateX86MemoryReference(*storeLowMR, 4, cg), tempRegister, cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::l2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
    ...
    generateMemRegInstruction(S4MemReg, node, generateX86MemoryReference(*lowMR, 4, cg), longReg->getHighOrder(), cg);
    generateFPRegMemInstruction(FLLDRegMem, node, target, generateX86MemoryReference(*lowMR, 0, cg), cg);
    cg->decReferenceCount(child);
  }
...
TR::Register* OMR::X86::I386::TreeEvaluator::l2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
    ...
    generateMemRegInstruction(S4MemReg, node, lowMR, longReg->getLowOrder(), cg);
    generateMemRegInstruction(S4MemReg, node, generateX86MemoryReference(*lowMR, 4, cg), longReg->getHighOrder(), cg);
    generateFPRegMemInstruction(DLLDRegMem, node, target, generateX86MemoryReference(*lowMR, 0, cg), cg);
    cg->decReferenceCount(child);
  }
    ...
    if ((lowValue | highValue) == 0) {
      ...
      if (firstChild->getOpCodeValue() == TR::land && firstChild->getReferenceCount() == 1
        ...
        if (landFirstChild->getReferenceCount() == 1 && landFirstChild->getRegister() == NULL
          ...
          generateRegMemInstruction(L4RegMem, landFirstChild, targetRegister, tempMR, cg);
...
TR::Register* OMR::X86::I386::TreeEvaluator::lternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!longCompare && condOp.isCompareForEquality() && condition->getFirstChild()->getOpCode().isIntegerOrAddress()) {
    ...
    if (condOp.isCompareTrueIfEqual()) {
      generateRegRegInstruction(
        CMOVNE4RegReg, node, trueReg->getRegisterPair()->getLowOrder(), falseReg->getRegisterPair()->getLowOrder(), cg);
      generateRegRegInstruction(CMOVNE4RegReg, node, trueReg->getRegisterPair()->getHighOrder(),
        falseReg->getRegisterPair()->getHighOrder(), cg);
    } else {
      generateRegRegInstruction(
        CMOVE4RegReg, node, trueReg->getRegisterPair()->getLowOrder(), falseReg->getRegisterPair()->getLowOrder(), cg);
      generateRegRegInstruction(CMOVE4RegReg, node, trueReg->getRegisterPair()->getHighOrder(),
        falseReg->getRegisterPair()->getHighOrder(), cg);
    }
    ...
    generateRegRegInstruction(
      CMOVE4RegReg, node, trueReg->getRegisterPair()->getLowOrder(), falseReg->getRegisterPair()->getLowOrder(), cg);
    generateRegRegInstruction(
      CMOVE4RegReg, node, trueReg->getRegisterPair()->getHighOrder(), falseReg->getRegisterPair()->getHighOrder(), cg);
  }
...
TR::Register* OMR::X86::I386::TreeEvaluator::integerPairMinMaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateRegRegInstruction(CMOVE4RegReg, node, result->getHighOrder(), result->getLowOrder(),
