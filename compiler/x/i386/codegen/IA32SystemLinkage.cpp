...
int32_t
TR::IA32SystemLinkage::layoutParm(TR::Node* parmNode,
                                  int32_t& dataCursor,
                                  uint16_t& intReg,
                                  uint16_t& floatReg,
...
int32_t
TR::IA32SystemLinkage::layoutParm(TR::ParameterSymbol* parmSymbol,
                                  int32_t& dataCursor,
                                  uint16_t& intReg,
                                  uint16_t& floatReg,
                                  TR::parmLayoutResult& layoutResult)
  ...
  TR::RegisterDependencyConditions* deps)
{
  ...
  if (longReturnReg) {
    deps->addPostCondition(
      returnReg->getLowOrder(), TR::RealRegister::eax, cg());
    deps->addPostCondition(
      returnReg->getHighOrder(), TR::RealRegister::edx, cg());
  } else if (integerReturnReg) {
    deps->addPostCondition(returnReg, TR::RealRegister::eax, cg());
    deps->addPostCondition(
      cg()->allocateRegister(), TR::RealRegister::edx, cg());
  } else {
    deps->addPostCondition(
      cg()->allocateRegister(), TR::RealRegister::eax, cg());
    deps->addPostCondition(
      cg()->allocateRegister(), TR::RealRegister::edx, cg());
  }
  ...
  if (fpReturnReg) {
    deps->addPostCondition(
      returnReg, _properties.getFloatReturnRegister(), cg());
  } else {
...
TR::Register*
TR::IA32SystemLinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
  ...
  if ((callNode->getDataType() == TR::Float ||
    ...
    generateFPSTiST0RegRegInstruction(
      FSTRegReg, callNode, returnReg, returnReg, cg());
  }
