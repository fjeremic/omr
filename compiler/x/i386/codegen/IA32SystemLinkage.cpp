...
TR::IA32SystemLinkage::IA32SystemLinkage(TR::CodeGenerator *cg)
  ...
  _properties._returnRegisters[0] =
      TR::RealRegister::eax; // IntegerReturnRegiste
  _properties._returnRegisters[1] =
      TR::RealRegister::st0; // Float/Double ReturnReg
  _properties._returnRegisters[2] =
      TR::RealRegister::edx; // LongHighReturnRegister
  ...
  _properties._preservedRegisterMapForGC =
      ((1 << (TR::RealRegister::ebx - 1)) | (1 << (TR::RealRegister::esi - 1)) |
       (1 << (TR::RealRegister::edi - 1)) | (1 << (TR::RealRegister::ebp - 1)) |
...
int32_t TR::IA32SystemLinkage::layoutParm(TR::Node *parmNode,
                                          int32_t &dataCursor, uint16_t &intReg,
                                          uint16_t &floatReg,
...
int32_t TR::IA32SystemLinkage::layoutParm(TR::ParameterSymbol *parmSymbol,
                                          int32_t &dataCursor, uint16_t &intReg,
                                          uint16_t &floatReg,
                                          TR::parmLayoutResult &layoutResult) {
  layoutResult.abstract |= TR::parmLayoutResult::ON_STACK;
...
TR::Register *TR::IA32SystemLinkage::buildIndirectDispatch(TR::Node *callNode) {
  TR_ASSERT(false,
            "TR::IA32SystemLinkage::buildIndirectDispatch is not supported.\n");
  return NULL;
}
...
TR::Register *TR::IA32SystemLinkage::buildVolatileAndReturnDependencies(
  ...
  case TR::Double:
    ...
    break;
  case TR::Int64:
    returnReg = longReturnReg = (TR::Register *)cg()->allocateRegisterPair(
  ...
  TR_ASSERT(_properties.getIntegerReturnRegister() == TR::RealRegister::eax,
            "assertion failure");
  TR_ASSERT(_properties.getLongLowReturnRegister() == TR::RealRegister::eax,
            "assertion failure");
  TR_ASSERT(_properties.getLongHighReturnRegister() == TR::RealRegister::edx,
            "assertion failure");
  TR_ASSERT(_properties.getFloatReturnRegister() == TR::RealRegister::st0,
            "assertion failure");

  if (longReturnReg) {
    deps->addPostCondition(returnReg->getLowOrder(), TR::RealRegister::eax,
                           cg());
    deps->addPostCondition(returnReg->getHighOrder(), TR::RealRegister::edx,
                           cg());
  } else if (integerReturnReg) {
    deps->addPostCondition(returnReg, TR::RealRegister::eax, cg());
    deps->addPostCondition(cg()->allocateRegister(), TR::RealRegister::edx,
                           cg());
  } else {
    deps->addPostCondition(cg()->allocateRegister(), TR::RealRegister::eax,
                           cg());
    deps->addPostCondition(cg()->allocateRegister(), TR::RealRegister::edx,
                           cg());
  }
  ...
  // st0
  if (fpReturnReg) {
    deps->addPostCondition(returnReg, _properties.getFloatReturnRegister(),
                           cg());
  } else {
    // No need for a dummy dep here because FPREGSPILL instruction takes care of
    // it
  }
  ...
  // here, the child class could add NO more register dependencies.

  return returnReg;
}

int32_t
TR::IA32SystemLinkage::buildArgs(TR::Node *callNode,
...
TR::Register *TR::IA32SystemLinkage::buildDirectDispatch(TR::Node *callNode,
                                                         bool spillFPRegs) {
  ...
  TR::RegisterDependencyConditions *dummy =
      generateRegisterDependencyConditions((uint8_t)0, (uint8_t)0, cg());
  ...
  int32_t stackAdjustment =
      cg()->getProperties().getCallerCleanup() ? 0 : -argSize;
  cg()->resetIsLeafMethod();
  TR::X86ImmInstruction *instr = generateImmSymInstruction(
      CALLImm4, callNode, (uintptr_t)methodSymbol->getMethodAddress(),
      methodSymRef, cg());
  ...
  if (cg()->getProperties().getCallerCleanup() && argSize > 0) {
    // Clean up arguments
    //
    generateRegImmInstruction((argSize <= 127) ? ADD4RegImms : ADD4RegImm4,
  ...
  if ((callNode->getDataType() == TR::Float ||
    ...
    generateFPSTiST0RegRegInstruction(FSTRegReg, callNode, returnReg, returnReg,
                                      cg());
  }
