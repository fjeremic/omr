    ...
    : TR::X86SystemLinkage(cg)
{
    ...
    _properties._preservedRegisterMapForGC = ((1 << (TR::RealRegister::ebx - 1)) | (1 << (TR::RealRegister::esi - 1)) | (1 << (TR::RealRegister::edi - 1)) | (1 << (TR::RealRegister::ebp - 1)) | (1 << (TR::RealRegister::esp - 1)));
...
TR::Register* TR::IA32SystemLinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
    ...
    //
    if ((callNode->getDataType() == TR::Float || callNode->getDataType() == TR::Double) && callNode->getReferenceCount() == 1) {
