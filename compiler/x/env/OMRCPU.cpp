  ...
  if (jitConfig && jitConfig->getProcessorInfo() == NULL) {
    ...
    if (!buf) {
      if (jitConfig && jitConfig->getProcessorInfo()) {
        ...
        if (!buf)
          memcpy(buf->_vendorId, "Poughkeepsie",
                 12);  // 12 character dummy string (NIL term not used)
...
uint32_t OMR::X86::CPU::getX86ProcessorSignature() {
...
bool OMR::X86::CPU::getSupportsHardwareSQRT() {
  return true;
}

bool OMR::X86::CPU::testOSForSSESupport() {
  return false;
}

bool OMR::X86::CPU::supportsTransactionalMemoryInstructions() {
