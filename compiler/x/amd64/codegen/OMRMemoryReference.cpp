...
OMR::X86::AMD64::MemoryReference::MemoryReference(
  TR::SymbolReference* symRef, intptrj_t displacement, TR::CodeGenerator* cg, TR_ScratchRegisterManager* srm)
...
void OMR::X86::AMD64::MemoryReference::assignRegisters(TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
  ...
  if (_addressRegister == NULL) {
    ...
    //
    if (_addressRegister->decFutureUseCount() == 0 && assignedAddressRegister->getState() != TR::RealRegister::Locked) {
    ...
    if (self()->getUnresolvedDataSnippet()) {
        ...
        if (cg->needClassAndMethodPointerRelocations()) {
          if (cg->comp()->getOption(TR_UseSymbolValidationManager)) {
            ...
            cg->addExternalRelocation(
              new (cg->trHeapMemory()) TR::ExternalRelocation(displacementLocation, (uint8_t*)srCopy,
                (uint8_t*)(uintptr_t)containingInstruction->getNode()->getInlinedSiteIndex(), TR_ClassAddress, cg),
              __FILE__, __LINE__, containingInstruction->getNode());
          }
        }
      ...
      if (cg->needRelocationsForStatics() || cg->needRelocationsForBodyInfoData())
        cg->addExternalRelocation(new (cg->trHeapMemory())
                                    TR::ExternalRelocation(displacementLocation, 0, TR_BodyInfoAddress, cg),
          __FILE__, __LINE__, containingInstruction->getNode());
    } else if (sr.getSymbol()->isGCRPatchPoint()) {
      if (cg->needRelocationsForStatics()) {
        TR::ExternalRelocation* r
          = new (cg->trHeapMemory()) TR::ExternalRelocation(displacementLocation, 0, TR_AbsoluteMethodAddress, cg);
      ...
      if (cg->needRelocationsForStatics())
        cg->addExternalRelocation(new (cg->trHeapMemory())
                                    TR::ExternalRelocation(displacementLocation, 0, TR_AbsoluteMethodAddress, cg),
          __FILE__, __LINE__, containingInstruction->getNode());
      ...
      TR::DebugCounter::generateRelocation(cg->comp(), displacementLocation, containingInstruction->getNode(), counter);
    }
  } else {
...
void OMR::X86::AMD64::MemoryReference::addMetaDataForCodeAddressDisplacementOnly(
  intptrj_t displacement, uint8_t* cursor, TR::CodeGenerator* cg)
{
  ...
  uint8_t* modRM, TR::Instruction* containingInstruction, TR::CodeGenerator* cg)
{
  ...
  if (self()->getDataSnippet() || self()->getLabel()) {
    ...
    if (_symbolReference.getSymbol()) {
      ...
      addressLoadInstruction = generateRegImm64SymInstruction(containingInstruction->getPrev(), MOV8RegImm64,
        _addressRegister,
        (!self()->getUnresolvedDataSnippet() && sr.getSymbol()->isStatic() && sr.getSymbol()->isClassObject()
          && cg->needClassAndMethodPointerRelocations())
          ? (uint64_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(comp, (TR_OpaqueClassBlock*)displacement)
          : displacement,
        symRef, cg);
      ...
      addressLoadInstruction = generateRegImm64Instruction(
        containingInstruction->getPrev(), MOV8RegImm64, _addressRegister, displacement, cg);
    }
    ...
    if (TR::Compiler->target.isSMP() && self()->getUnresolvedDataSnippet()) {
      ...
      TR_ASSERT((addressLoadInstruction->getPrev()->getKind() == TR::Instruction::IsPatchableCodeAlignment)
        ...
        "Expected TR::X86PatchableCodeAlignmentInstruction or TR::X86BoundaryAvoidance before unresolved memory "
    ...
    if (self()->getBaseRegister() && self()->getIndexRegister()) {
      TR::Instruction* addressAddInstruction = generateRegRegInstruction(
        addressLoadInstruction, ADD8RegReg, self()->getAddressRegister(), self()->getBaseRegister(), cg);
