...
void
OMR::X86::AMD64::MemoryReference::finishInitialization(
  TR::CodeGenerator* cg,
  TR_ScratchRegisterManager* srm)
{
...
void
OMR::X86::AMD64::MemoryReference::decNodeReferenceCounts(TR::CodeGenerator* cg)
{
...
void
OMR::X86::AMD64::MemoryReference::useRegisters(TR::Instruction* instr,
                                               TR::CodeGenerator* cg)
{
...
bool
OMR::X86::AMD64::MemoryReference::needsAddressLoadInstruction(
  intptrj_t rip,
  TR::CodeGenerator* cg)
{
  TR::SymbolReference& sr = self()->getSymbolReference();
...
void
OMR::X86::AMD64::MemoryReference::assignRegisters(
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (_addressRegister == NULL) {
    ...
    if (assignedAddressRegister == NULL) {
      ...
      assignedAddressRegister =
        assignGPRegister(currentInstruction, _addressRegister, TR_WordReg, cg);
  ...
  if (_symbolReference.getSymbol()) {
    ...
    if (self()->getUnresolvedDataSnippet()) {
      ...
      if (cg->needRelocationsForStatics()) {
        ...
        cg->addExternalRelocation(
          r, __FILE__, __LINE__, containingInstruction->getNode());
      }
    } else if (sr.getSymbol()->isCompiledMethod()) {
      if (cg->needRelocationsForStatics())
        cg->addExternalRelocation(
          new (cg->trHeapMemory())
            TR::ExternalRelocation(displacementLocation, 0, TR_RamMethod, cg),
          __FILE__,
          __LINE__,
          containingInstruction->getNode());
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (self()->getDataSnippet() || self()->getLabel()) {
    ...
    if (_symbolReference.getSymbol()) {
      ...
      addressLoadInstruction =
        generateRegImm64Instruction(containingInstruction->getPrev(),
                                    MOV8RegImm64,
                                    _addressRegister,
                                    displacement,
                                    cg);
    ...
    if (self()->getBaseRegister() && self()->getIndexRegister()) {
      TR::Instruction* addressAddInstruction =
        generateRegRegInstruction(addressLoadInstruction,
                                  ADD8RegReg,
                                  self()->getAddressRegister(),
                                  self()->getBaseRegister(),
                                  cg);
    ...
    _flags.reset(
      MemRef_NeedExternalCodeAbsoluteRelocation); // TODO:AMD64: Do I need this?
