...
uint32_t
OMR::X86::AMD64::MemoryReference::estimateBinaryLength(TR::CodeGenerator *cg) {
  uint32_t estimate;
    ...
    if (self()->getUnresolvedDataSnippet()) {
      ...
      if (comp->getOption(TR_EnableHCR) &&
          (!sr.getSymbol()->isStatic() || !sr.getSymbol()->isClassObject() ||
           cg->wantToPatchClassPointer(
               NULL, containingInstruction->getBinaryEncoding()))) // unresolved
      {
      ...
      if (cg->needRelocationsForStatics()) {
        TR::ExternalRelocation *r = new (cg->trHeapMemory())
            TR::ExternalRelocation(displacementLocation, 0,
...
uint8_t *OMR::X86::AMD64::MemoryReference::generateBinaryEncoding(
  ...
  if (self()->getDataSnippet() || self()->getLabel()) {
    ...
    _flags.reset(MemRef_NeedExternalCodeAbsoluteRelocation); // TODO:AMD64: Do I
                                                             // need this?
