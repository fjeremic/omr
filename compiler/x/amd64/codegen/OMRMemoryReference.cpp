...
OMR::X86::AMD64::MemoryReference::MemoryReference(TR::Register *br,
...
OMR::X86::AMD64::MemoryReference::MemoryReference(
    intptrj_t disp, TR::CodeGenerator *cg, TR_ScratchRegisterManager *srm)
...
OMR::X86::AMD64::MemoryReference::MemoryReference(TR::X86DataSnippet *cds,
                                                  TR::CodeGenerator *cg)
    : OMR::X86::MemoryReference(cds, cg) {
  self()->finishInitialization(cg, NULL);
}

OMR::X86::AMD64::MemoryReference::MemoryReference(TR::LabelSymbol *label,
                                                  TR::CodeGenerator *cg)
...
OMR::X86::AMD64::MemoryReference::MemoryReference(
    ...
    : OMR::X86::MemoryReference(rootLoadOrStore, cg,
                                canRematerializeAddressAdds) {
  self()->finishInitialization(cg, srm);
}
...
OMR::X86::AMD64::MemoryReference::MemoryReference(
    TR::SymbolReference *symRef, intptrj_t displacement, TR::CodeGenerator *cg,
    TR_ScratchRegisterManager *srm)
...
OMR::X86::AMD64::MemoryReference::MemoryReference(
    TR::MemoryReference &mr, intptrj_t n, TR::CodeGenerator *cg,
    TR_ScratchRegisterManager *srm)
...
void OMR::X86::AMD64::MemoryReference::finishInitialization(
  ...
  if (self()->getDataSnippet()) {
    ...
    //
    mightNeedAddressRegister = true;
  } else if (comp->getOption(TR_EnableHCR) && sr.getSymbol() &&
  ...
  if (mightNeedAddressRegister) {
    ...
    if (srm) {
      ...
      cg->stopUsingRegister(
          _addressRegister);  // See comments in decNodeReferenceCounts
    }
  } else
    _addressRegister = NULL;
}

void OMR::X86::AMD64::MemoryReference::decNodeReferenceCounts(
    TR::CodeGenerator *cg) {
...
void OMR::X86::AMD64::MemoryReference::useRegisters(TR::Instruction *instr,
                                                    TR::CodeGenerator *cg) {
...
bool OMR::X86::AMD64::MemoryReference::needsAddressLoadInstruction(
    intptrj_t rip, TR::CodeGenerator *cg) {
  TR::SymbolReference &sr = self()->getSymbolReference();
  ...
  if (sr.getSymbol() != NULL && sr.isUnresolved()) {
    ...
    return !IS_32BIT_SIGNED(displacement);
           ...
           cg->needRelocationsForStatics())
    return true;
  else if (sr.getSymbol() && sr.getSymbol()->isRecompilationCounter() &&
           cg->needRelocationsForBodyInfoData())
    return true;
  else if (sr.getSymbol() && sr.getSymbol()->isCountForRecompile() &&
           cg->needRelocationsForPersistentInfoData())
    return true;
  else if (comp->getOption(TR_EnableHCR) && sr.getSymbol() &&
           sr.getSymbol()->isClassObject())
    return true;  // If a class gets replaced, it may no longer fit in an
                  // immediate
...
void OMR::X86::AMD64::MemoryReference::assignRegisters(
  ...
  if (_addressRegister == NULL) {
    ...
    //
    TR::RealRegister *assignedAddressRegister =
        _addressRegister->getAssignedRealRegister();

    if (assignedAddressRegister == NULL) {
      if (_baseRegister != NULL) _baseRegister->block();
      if (_indexRegister != NULL) _indexRegister->block();

      assignedAddressRegister = assignGPRegister(
          currentInstruction, _addressRegister, TR_WordReg, cg);

      if (_indexRegister != NULL) _indexRegister->unblock();
      if (_baseRegister != NULL) _baseRegister->unblock();
    }
...
uint32_t OMR::X86::AMD64::MemoryReference::estimateBinaryLength(
    TR::CodeGenerator *cg) {
  uint32_t estimate;

  if (0 && REGISTERS_CAN_CHANGE_AFTER_INITIALIZATION &&
      self()->getBaseRegister() && self()->getIndexRegister() &&
      _addressRegister) {
  ...
  if (_addressRegister == NULL) {
    ...
    //
    if (_baseRegister == NULL && _indexRegister == NULL) estimate += 1;
  } else {
...
void OMR::X86::AMD64::MemoryReference::addMetaDataForCodeAddressWithLoad(
    uint8_t *displacementLocation, TR::Instruction *containingInstruction,
  ...
  if (_symbolReference.getSymbol()) {
    ...
    if (self()->getUnresolvedDataSnippet()) {
      ...
      if (comp->getOption(TR_EnableHCR) &&
          (!sr.getSymbol()->isStatic() || !sr.getSymbol()->isClassObject() ||
           cg->wantToPatchClassPointer(
               NULL,
               containingInstruction->getBinaryEncoding())))  // unresolved
      {
      ...
      if (sr.getSymbol()->isStatic()) {
          ...
          if (cg->comp()->getOption(TR_UseSymbolValidationManager)) {
            ...
            cg->addExternalRelocation(
                new (cg->trHeapMemory()) TR::ExternalRelocation(
                    displacementLocation, (uint8_t *)srCopy,
                    (uint8_t *)(uintptr_t)containingInstruction->getNode()
                        ->getInlinedSiteIndex(),
                    TR_ClassAddress, cg),
                __FILE__, __LINE__, containingInstruction->getNode());
          }
        ...
        if (cg->wantToPatchClassPointer(
                NULL,
                displacementLocation))  // may not point to beginning of class
        {
          cg->jitAddPicToPatchOnClassRedefinition(((void *)displacement),
      ...
      if (cg->needRelocationsForStatics() ||
        ...
        cg->addExternalRelocation(
            new (cg->trHeapMemory()) TR::ExternalRelocation(
                displacementLocation, (uint8_t *)TR_CountForRecompile,
                TR_GlobalValue, cg),
            __FILE__, __LINE__, containingInstruction->getNode());
      ...
      if (cg->needRelocationsForStatics() ||
        ...
        cg->addExternalRelocation(
            new (cg->trHeapMemory()) TR::ExternalRelocation(
                displacementLocation, 0, TR_BodyInfoAddress, cg),
            __FILE__, __LINE__, containingInstruction->getNode());
    } else if (sr.getSymbol()->isGCRPatchPoint()) {
      if (cg->needRelocationsForStatics()) {
        TR::ExternalRelocation *r =
            new (cg->trHeapMemory()) TR::ExternalRelocation(
                displacementLocation, 0, TR_AbsoluteMethodAddress, cg);
        cg->addExternalRelocation(r, __FILE__, __LINE__,
      ...
      if (cg->needRelocationsForStatics())
        cg->addExternalRelocation(
            new (cg->trHeapMemory()) TR::ExternalRelocation(
                displacementLocation, 0, TR_RamMethod, cg),
            __FILE__, __LINE__, containingInstruction->getNode());
      ...
      TR::DebugCounterBase *counter =
          cg->comp()->getCounterFromStaticAddress(&sr);
      if (counter == NULL) {
        cg->comp()->failCompilation<TR::CompilationException>(
            "Could not generate relocation for debug counter in "
            "OMR::X86::AMD64::MemoryReference::"
            "addMetaDataForCodeAddressWithLoad\n");
      }
      TR::DebugCounter::generateRelocation(cg->comp(), displacementLocation,
                                           containingInstruction->getNode(),
                                           counter);
    }
  } else {
    if (self()->needsCodeAbsoluteExternalRelocation()) {
      cg->addExternalRelocation(
          new (cg->trHeapMemory()) TR::ExternalRelocation(
              displacementLocation, (uint8_t *)0, TR_AbsoluteMethodAddress, cg),
          __FILE__, __LINE__, containingInstruction->getNode());
    }
  }
}

void OMR::X86::AMD64::MemoryReference::
    addMetaDataForCodeAddressDisplacementOnly(intptrj_t displacement,
                                              uint8_t *cursor,
                                              TR::CodeGenerator *cg) {
  if (IS_32BIT_SIGNED(displacement) && !_preferRIPRelative) {
    if (_symbolReference.getSymbol() &&
        _symbolReference.getSymbol()->isClassObject() &&
        cg->wantToPatchClassPointer(
            NULL, cursor))  // may not point to beginning of class
    {
      cg->jitAdd32BitPicToPatchOnClassRedefinition(((void *)displacement),
                                                   cursor);
    }
  }
}
...
uint8_t *OMR::X86::AMD64::MemoryReference::generateBinaryEncoding(
  ...
  TR_ASSERT(containingInstruction != NULL,
            "AMD64: must have containing instruction to insert address load");

  // Compute what RIP will be, based on where our modRM byte is, in case we need
  // it. [RIP+xxx] has 4-byte offset, no SIB, may have an immediate. See x86-64
  // Architecture Programmer's Manual, Volume 3, sections 1.1 and 1.7.1.
  //
  intptrj_t rip = (intptrj_t)(modRM + 5) +
                  containingInstruction->getOpCode().info().ImmediateSize();

  if (self()->getDataSnippet() || self()->getLabel()) {
    // The inherited logic has a special case for RIP-based ConstantDataSnippet
    // and label references.
    //
    return OMR::X86::MemoryReference::generateBinaryEncoding(
        modRM, containingInstruction, cg);
  } else if (self()->needsAddressLoadInstruction(rip, cg)) {
    TR_ASSERT(_addressRegister != NULL,
              "OMR::X86::AMD64::MemoryReference should have allocated an "
              "address register");

    TR::Instruction *addressLoadInstruction;

    uint8_t *displacementLocation =
        containingInstruction->getBinaryEncoding() + 2;
    ...
    if (_symbolReference.getSymbol()) {
      ...
      //
      symRef = new (cg->trHeapMemory())
          TR::SymbolReference(cg->symRefTab(), _symbolReference, 0);

      addressLoadInstruction = generateRegImm64SymInstruction(
          containingInstruction->getPrev(), MOV8RegImm64, _addressRegister,
      ...
      if (self()->getUnresolvedDataSnippet()) {
        self()->getUnresolvedDataSnippet()->setDataReferenceInstruction(
            addressLoadInstruction);
        self()->getUnresolvedDataSnippet()->setDataSymbolReference(symRef);
      }
    } else {
      TR_ASSERT(!self()->getUnresolvedDataSnippet(),
                "Unresolved references should always have a symbol");
    ...
    self()->addMetaDataForCodeAddressWithLoad(
        displacementLocation, containingInstruction, cg, symRef);

    // addressLoadInstruction's node should be that of containingInstruction
    //
    addressLoadInstruction->setNode(
        _baseNode ? _baseNode : containingInstruction->getNode());
    if (TR::Compiler->target.isSMP() && self()->getUnresolvedDataSnippet()) {
      ...
      //
      TR_ASSERT((addressLoadInstruction->getPrev()->getKind() ==
                 TR::Instruction::IsPatchableCodeAlignment) ||
                    (addressLoadInstruction->getPrev()->getKind() ==
                     TR::Instruction::IsBoundaryAvoidance),
                "Expected TR::X86PatchableCodeAlignmentInstruction or "
                "TR::X86BoundaryAvoidance before unresolved memory reference "
                "instruction");
      addressLoadInstruction->getPrev()->setNode(
          containingInstruction->getNode());
    }
    ...
    if (self()->getBaseRegister() && self()->getIndexRegister()) {
      TR::Instruction *addressAddInstruction = generateRegRegInstruction(
          addressLoadInstruction, ADD8RegReg, self()->getAddressRegister(),
          self()->getBaseRegister(), cg);
    ...
    // Transform this memref from [whatever + offset64] to [whatever +
    // _addressRegister] Also transforms [base + index + offset64] to
    // [_addressRegister + index]
    //
    ...
    _flags.reset(MemRef_NeedExternalCodeAbsoluteRelocation);  // TODO:AMD64: Do
                                                              // I need this?
    _symbolReference.setSymbol(NULL);
    _symbolReference.setOffset(0);
    self()->setUnresolvedDataSnippet(
        NULL);  // Otherwise it will get damaged when we re-emit
                // containingInstruction
    ...
    if (IS_32BIT_SIGNED(displacement) && !_preferRIPRelative) {
      ...
      *(uint32_t *)cursor = (uint32_t)displacement;
      ...
      TR_ASSERT(
          !(comp->getOption(TR_EnableHCR) && sr.getSymbol() &&
            sr.getSymbol()->isClassObject()),
    ...
    // Unresolved shadows whose base object is explicitly NULL need to report
    // the offset of the disp32 field.
    //
    // NOTE: this may not work in the highly unlikely case of a field offset >
    // 64k
    ...
    if (self()->getUnresolvedDataSnippet()) {
      ...
      traceMsg(
          comp,
          "found unresolved shadow with NULL base object : data reference "
          "instruction=%p, cursor=%p\n",
          self()->getUnresolvedDataSnippet()->getDataReferenceInstruction(),
          cursor);
    }
