    ...
    TR_ScratchRegisterManager* srm)
{
    ...
    } else if (!self()->getBaseRegister() && !self()->getIndexRegister() && (cg->needRelocationsForStatics() || cg->needClassAndMethodPointerRelocations() || cg->needRelocationsForBodyInfoData() || cg->needRelocationsForPersistentInfoData())) {
    ...
    if (self()->getDataSnippet() || self()->getLabel()) {
        ...
        if (_symbolReference.getSymbol()) {
            ...
            addressLoadInstruction = generateRegImm64SymInstruction(
                ...
                MOV8RegImm64,
                _addressRegister,
                (!self()->getUnresolvedDataSnippet() && sr.getSymbol()->isStatic() && sr.getSymbol()->isClassObject() && cg->needClassAndMethodPointerRelocations()) ? (uint64_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(comp, (TR_OpaqueClassBlock*)displacement) : displacement,
                symRef,
                cg);
        ...
        if (TR::Compiler->target.isSMP() && self()->getUnresolvedDataSnippet()) {
            ...
            //
            TR_ASSERT((addressLoadInstruction->getPrev()->getKind() == TR::Instruction::IsPatchableCodeAlignment) || (addressLoadInstruction->getPrev()->getKind() == TR::Instruction::IsBoundaryAvoidance),
