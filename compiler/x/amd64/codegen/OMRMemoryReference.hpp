...
class OMR_EXTENSIBLE MemoryReference : public OMR::X86::MemoryReference {
  ...
  TR::Register *_addressRegister; // Used when extra loads are required to
                                  // compute the address
  bool _preferRIPRelative; // used to indicate bias to RIP relative addressing
                           // as opposed to SIB
...
public: // Constructors
  ...
  MemoryReference(TR::SymbolReference *symRef, intptrj_t displacement,
                  TR::CodeGenerator *cg, TR_ScratchRegisterManager *srm = NULL);
  ...
  void addMetaDataForCodeAddressWithLoad(uint8_t *displacementLocation,
                                         TR::Instruction *containingInstruction,
                                         TR::CodeGenerator *cg,
                                         TR::SymbolReference *srCopy);
  void addMetaDataForCodeAddressDisplacementOnly(intptrj_t displacement,
                                                 uint8_t *cursor,
                                                 TR::CodeGenerator *cg);
...
protected:
  ...
  void finishInitialization(TR::CodeGenerator *cg,
