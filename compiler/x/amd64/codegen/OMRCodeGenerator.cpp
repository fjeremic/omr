...
OMR::X86::AMD64::CodeGenerator::CodeGenerator() : OMR::X86::CodeGenerator() {
  ...
  if (c) self()->comp()->setOption(TR_DisableValueProfiling);
  ...
  // Interpreter frame shape requires all autos to occupy an 8-byte slot on
  // 64-bit.
  //
  ...
  _globalGPRsPreservedAcrossCalls.init(self()->getNumberOfGlobalRegisters(),
                                       self()->trMemory());
  _globalFPRsPreservedAcrossCalls.init(self()->getNumberOfGlobalRegisters(),
                                       self()->trMemory());
  ...
  // Reduce the maxObjectSizeGuaranteedNotToOverflow value on 64-bit such that
  // the runtime comparison does not sign extend (saves an instruction on array
  // allocations). INT_MAX should be sufficiently large.
  //
...
TR::Register *OMR::X86::AMD64::CodeGenerator::longClobberEvaluate(
    TR::Node *node) {
...
OMR::X86::AMD64::CodeGenerator::getLinkageGlobalRegisterNumber(
  ...
  if (isFloat) {
    if (linkageRegisterIndex >=
        self()->getProperties().getNumFloatArgumentRegisters())
      return -1;
    else
      result = _fprLinkageGlobalRegisterNumbers[linkageRegisterIndex];
  } else {
    if (linkageRegisterIndex >=
        self()->getProperties().getNumIntegerArgumentRegisters())
      return -1;
    else
  ...
  TR::RealRegister::RegNum realReg =
      self()->getProperties().getArgumentRegister(linkageRegisterIndex,
                                                  isFloat);

  TR_ASSERT(self()->getGlobalRegister(result) == realReg,
            "Result must be consistent with globalRegisterTable");
  return result;
}

int32_t OMR::X86::AMD64::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
    TR::Node *node) {
  ...
  if (node->getOpCode().isIf() &&
          node->getFirstChild()->getOpCodeValue() == TR::
          instanceof)
    return self()->getNumberOfGlobalGPRs() - 6;
  else if (node->getOpCode().isSwitch())
    return self()->getNumberOfGlobalGPRs() -
           3;  // A memref in a jump for a MemTable might need a base, index and
               // address register.
...
void OMR::X86::AMD64::CodeGenerator::initLinkageToGlobalRegisterMap() {
  ...
  for (i = 0; i < self()->getProperties().getNumIntegerArgumentRegisters(); i++)
    _gprLinkageGlobalRegisterNumbers[i] =
        globalRegNumbers[self()->getProperties().getIntegerArgumentRegister(i)];

  for (i = 0; i < self()->getProperties().getNumFloatArgumentRegisters(); i++)
    _fprLinkageGlobalRegisterNumbers[i] =
        globalRegNumbers[self()->getProperties().getFloatArgumentRegister(i)];
}

bool OMR::X86::AMD64::CodeGenerator::opCodeIsNoOpOnThisPlatform(
    TR::ILOpCode &opCode) {
