  ...
  : TR::AMD64SystemLinkage(cg)
{
  ...
  if (OMR::X86::AMD64::Machine::enableNewPickRegister()) {
    if (OMR::X86::AMD64::Machine::numGPRRegsWithheld(cg) == 0) {
      ...
      TR_ASSERT(
        OMR::X86::AMD64::Machine::numRegsWithheld(cg) == 2, "numRegsWithheld: only 0 and 2 currently supported");
  }
...
TR::Register* TR::AMD64SystemLinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
  ...
  if (methodSymbol->getMethodAddress()) {
    ...
    auto LoadRegisterInstruction = generateRegImm64SymInstruction(
      MOV8RegImm64, callNode, scratchReg, (uintptr_t)methodSymbol->getMethodAddress(), methodSymRef, cg());
    ...
    instr = generateImmSymInstruction(
      CALLImm4, callNode, (uintptrj_t)methodSymbol->getMethodAddress(), methodSymRef, preDeps, cg());
  }
...
FAIL_TO_LAYOUT_IN_REGS:
  // AMD64 SysV ABI:  If there are no registers available for any eightbyte of an argument, the whole argument is passed
  // on the stack. If registers have already been assigned for some eightbytes of such an argument, the assignments get
  // reverted.
  intArgs = oldIntArgs;
...
void TR::AMD64SystemLinkage::setUpStackSizeForCallNode(TR::Node* node)
{
  ...
  // AMD64 SysV ABI: The end of the input argument area shall be aligned on a 16 (32, if __m256 is passed on stack) byte
  // boundary. In other words, the value (%rsp + 8) is always a multiple of 16 (32) when control is transferred to the
  ...
  if (sizeOfOutGoingArgs > cg()->getLargestOutgoingArgSize()) {
    ...
    if (comp()->getOption(TR_TraceCG))
      traceMsg(
        comp(), "setUpStackSizeForCallNode setLargestOutgoingArgSize %d(for call node %p)\n", sizeOfOutGoingArgs, node);
  }
