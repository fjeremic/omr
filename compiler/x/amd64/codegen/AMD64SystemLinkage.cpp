...
TR::AMD64Win64FastCallLinkage::AMD64Win64FastCallLinkage(TR::CodeGenerator *cg)
  ...
  _properties._properties =
      EightBytePointers | EightByteParmSlots | CallerCleanup |
  ...
  _properties._numFloatArgumentRegisters =
      p - _properties._numIntegerArgumentRegisters;
  ...
  _properties._numberOfPreservedXMMRegisters =
      p - _properties._numberOfPreservedGPRegisters;
  ...
  _properties._numberOfVolatileXMMRegisters =
      p - _properties._numberOfVolatileGPRegisters;
  ...
  _properties._offsetToFirstLocal =
      _properties.getAlwaysDedicateFramePointerRegister() ? -GPR_REG_WIDTH : 0;
  ...
  //
  _properties._registerFlags[TR::RealRegister::xmm0] =
  ...
  TR_ASSERT(p == (machine()->getNumGlobalGPRs() + machine()->_numGlobalFPRs),
            "assertion failure");
}
...
TR::AMD64ABILinkage::AMD64ABILinkage(TR::CodeGenerator *cg)
  ...
  // ReservesOutgoingArgsInPrologue is needed.
  //
  _properties._properties = EightBytePointers | EightByteParmSlots |
                            CallerCleanup | IntegersInRegisters |
                            LongsInRegisters | FloatsInRegisters |
  ...
  for (r = 12; r <= 15; r++) {
    ...
    _properties._preservedRegisterMapForGC |=
        TR::RealRegister::gprMask(TR::RealRegister::rIndex(r));
  }
...
TR::Register *TR::AMD64SystemLinkage::buildVolatileAndReturnDependencies(
  ...
  if (callNode->getOpCode().isIndirect()) {
    ...
    if (vftChild->getRegister() && (vftChild->getReferenceCount() > 1)) {
      ...
      // VFT child dies here; decrement it early so it doesn't interfere with
      // dummy regs.
  ...
  switch (callNode->getDataType()) {
    ...
    case TR::Aggregate:
    default:
      TR_ASSERT(false, "Unrecognized call node data type: #%d",
                (int)callNode->getDataType());
      break;
  }
  ...
  //
  int32_t i;
  TR::RealRegister::RegNum scratchIndex =
      getProperties().getIntegerScratchRegister(1);
  for (i = 0; i < getProperties().getNumVolatileRegisters(); i++) {
    ...
    if (regIndex != returnRegIndex) {
      TR_RegisterKinds rk =
          (i < getProperties()._numberOfVolatileGPRegisters) ? TR_GPR : TR_FPR;
      TR::Register *dummy = cg()->allocateRegister(rk);
      deps->addPostCondition(dummy, regIndex, cg());

      // Note that we don't setPlaceholderReg here.  If this volatile reg is
      // also volatile in the caller's linkage, then that flag doesn't matter
      // much anyway.  If it's preserved in the caller's linkage, then we don't
      // want to set that flag because we want this use of the register to count
      // as a "real" use, thereby motivating the prologue to preserve the
      // register.

      // A scratch register is necessary to call the native without a
      // trampoline.
      //
  ...
  if (callNode->getOpCode().isIndirect()) {
    TR::Node *vftChild = callNode->getFirstChild();
    if (vftChild->getRegister() && (vftChild->getReferenceCount() > 1)) {
      // VFT child survives the call, so we must include it in the
      // postconditions.
      deps->addPostCondition(vftChild->getRegister(), TR::RealRegister::NoReg,
                             cg());
      cg()->recursivelyDecReferenceCount(vftChild);
    }
  }
  ...
  // the caller. The reason is because, child class might call this method,
  // while adding more register dependecies;  if we close the reg dependency
  // here, the child class won't be able to add more register dependencies.
...
int32_t TR::AMD64SystemLinkage::buildArgs(
  ...
  // If the dispatch is indirect we must add the VFT register to the
  // preconditions so that it gets register assigned with the other
  // preconditions to the call.
  //
  if (callNode->getOpCode().isIndirect()) {
    TR::Node *vftChild = callNode->getFirstChild();
    TR_ASSERT(vftChild->getRegister(), "expecting VFT child to be evaluated");
    TR::RealRegister::RegNum scratchRegIndex =
        getProperties().getIntegerScratchRegister(1);
  ...
  for (i = first; i != last; i += direction) {
    ...
    if (layoutResult.abstract & TR::parmLayoutResult::IN_LINKAGE_REG_PAIR) {
      // TODO: AMD64 SysV ABI might put a struct into a pair of linkage
      // registerr
      ...
      TR_ASSERT(regKind == TR_GPR || regKind == TR_FPR,
                "linkage registers includes TR_GPR and TR_FPR\n");
      rregIndex = (regKind == TR_FPR)
                      ? getProperties().getFloatArgumentRegister(regIndex)
    ...
    if (rregIndex != noReg) {
      // For NULL JNI reference parameters, it is possible that the NULL value
      // will be evaluated into a different register than the child.  In that
      // case it is not necessary to copy the temporary scratch register across
      // the call.
      //
      ...
      generateMemRegInstruction(
          TR::Linkage::movOpcodes(MemReg, fullRegisterMovType(vreg)), child,
  ...
  //
  for (i = 0; i < numCopiedRegs; i++) cg()->stopUsingRegister(copiedRegs[i]);
...
TR::Register *TR::AMD64SystemLinkage::buildIndirectDispatch(
    TR::Node *callNode) {
  TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
  ...
  //
  uint32_t pre = getProperties().getNumIntegerArgumentRegisters() +
                 getProperties().getNumFloatArgumentRegisters() + 1;
  uint32_t post = getProperties().getNumVolatileRegisters() + 1 +
                  (callNode->getDataType() == TR::NoType ? 0 : 1);

  TR::RegisterDependencyConditions *callDeps =
      generateRegisterDependencyConditions(pre, 1, cg());
  ...
  TR::RegisterDependencyConditions *postDeps =
      generateRegisterDependencyConditions(0, post, cg());
  TR::Register *returnReg =
      buildVolatileAndReturnDependencies(callNode, postDeps);
...
TR::Register *TR::AMD64SystemLinkage::buildDirectDispatch(TR::Node *callNode,
                                                          bool spillFPRegs) {
  TR::SymbolReference *methodSymRef = callNode->getSymbolReference();
  TR::MethodSymbol *methodSymbol =
      methodSymRef->getSymbol()->castToMethodSymbol();
  ...
  //
  uint32_t pre = getProperties().getNumIntegerArgumentRegisters() +
                 getProperties().getNumFloatArgumentRegisters();
  uint32_t post = getProperties().getNumVolatileRegisters() +
                  (callNode->getDataType() == TR::NoType ? 0 : 1);

  TR::RegisterDependencyConditions *preDeps =
      generateRegisterDependencyConditions(pre, 0, cg());
  TR::RegisterDependencyConditions *postDeps =
      generateRegisterDependencyConditions(0, post, cg());
  ...
  for (int32_t i = 0; i < post; i++) {
    if (postDeps->getPostConditions()
            ->getRegisterDependency(i)
            ->getRealRegister() == scratchRegIndex) {
      scratchReg = postDeps->getPostConditions()
                       ->getRegisterDependency(i)
                       ->getRegister();
      break;
    }
  ...
  if (methodSymbol->getMethodAddress()) {
    ...
    auto LoadRegisterInstruction = generateRegImm64SymInstruction(
        MOV8RegImm64, callNode, scratchReg,
    ...
    instr =
        generateRegInstruction(CALLReg, callNode, scratchReg, preDeps, cg());
  } else {
    instr = generateImmSymInstruction(
        CALLImm4, callNode, (uintptrj_t)methodSymbol->getMethodAddress(),
...
void TR::AMD64ABILinkage::mapIncomingParms(TR::ResolvedMethodSymbol *method,
                                           uint32_t &stackIndex) {
  ...
  for (parmCursor = parameterIterator.getFirst(); parmCursor;
    ...
    if ((parmCursor->getLinkageRegisterIndex() >= 0) &&
        (parmCursor->getAssignedGlobalRegisterIndex() < 0 ||
         hasToBeOnStack(parmCursor))) {
      uint32_t align = getAlignment(parmCursor->getDataType());
      uint32_t alignMinus1 = (align <= AMD64_STACK_SLOT_SIZE)
                                 ? (AMD64_STACK_SLOT_SIZE - 1)
                                 : (align - 1);
      ...
      if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(),
                 "mapIncomingParms setParameterOffset %d for param symbol (reg "
                 "param without home location) %p, hasToBeOnStack() %d\n",
                 parmCursor->getParameterOffset(), parmCursor,
                 hasToBeOnStack(parmCursor));
    } else if (parmCursor->getLinkageRegisterIndex() >= 0 &&
               parmCursor->getAssignedGlobalRegisterIndex() >= 0) {
      // parmCursor->setDontHaveStackSlot(0); // this is a hack , so as we could
      // print stack layout table in createPrologue
      if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(),
                 "mapIncomingParms no need to set parm %p, for it has got "
                 "register %d assigned\n",
...
bool TR::AMD64SystemLinkage::layoutTypeInRegs(
    TR::DataType type, uint16_t &intArgs, uint16_t &floatArgs,
    TR::parmLayoutResult &layoutResult) {
  ...
  switch (type) {
    case TR::Float:
    case TR::Double:
      if (getProperties().getLinkageRegistersAssignedByCardinalPosition() &&
          intArgs < maxIntArgs)
        intArgs++;
...
FAIL_TO_LAYOUT_IN_REGS:
  // AMD64 SysV ABI:  If there are no registers available for any eightbyte of
  // an argument, the whole argument is passed on the stack. If registers have
...
int32_t TR::AMD64SystemLinkage::layoutParm(TR::Node *parmNode,
                                           int32_t &dataCursor,
                                           uint16_t &intReg, uint16_t &floatReg,
                                           TR::parmLayoutResult &layoutResult) {
  // AMD64 SysV ABI:  if the size of an object is larger than four eightbytes,
  // or it contains unaligned fields, it has class MEMORY.
  if (parmNode->getSize() > 4 * AMD64_STACK_SLOT_SIZE) goto LAYOUT_ON_STACK;

  if (layoutTypeInRegs(parmNode->getDataType(), intReg, floatReg,
                       layoutResult)) {
...
LAYOUT_ON_STACK:
  ...
  int32_t align =
      layoutTypeOnStack(parmNode->getDataType(), dataCursor, layoutResult);
  if (comp()->getOption(TR_TraceCG))
...
int32_t TR::AMD64SystemLinkage::layoutParm(TR::ParameterSymbol *parmSymbol,
                                           int32_t &dataCursor,
                                           uint16_t &intReg, uint16_t &floatReg,
                                           TR::parmLayoutResult &layoutResult) {
  // AMD64 SysV ABI:  if the size of an object is larger than four eightbytes,
  ...
  if (layoutTypeInRegs(parmSymbol->getDataType(), intReg, floatReg,
                       layoutResult)) {
    layoutResult.abstract |= TR::parmLayoutResult::IN_LINKAGE_REG;
...
LAYOUT_ON_STACK:
  layoutResult.abstract |= TR::parmLayoutResult::ON_STACK;
  int32_t align =
      layoutTypeOnStack(parmSymbol->getDataType(), dataCursor, layoutResult);
  if (comp()->getOption(TR_TraceCG))
...
void TR::AMD64SystemLinkage::setUpStackSizeForCallNode(TR::Node *node) {
  ...
  // AMD64 SysV ABI: The end of the input argument area shall be aligned on a 16
  // (32, if __m256 is passed on stack) byte boundary. In other words, the value
  ...
  if (sizeOfOutGoingArgs > cg()->getLargestOutgoingArgSize()) {
    ...
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(),
               "setUpStackSizeForCallNode setLargestOutgoingArgSize %d(for "
               "call node %p)\n",
  ...
  if (alignment > _properties.getOutgoingArgAlignment()) {
    ...
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(),
               "setUpStackSizeForCallNode setOutgoingArgAlignment %d(for call "
               "node %p)\n",
               alignment, node);
  }
}
