  ...
  TR::RegisterDependencyConditions* deps)
{
  ...
  switch (callNode->getDataType()) {
    ...
    default:
      TR_ASSERT(false,
                "Unrecognized call node data type: #%d",
  ...
  if (callNode->getOpCode().isIndirect()) {
    ...
    if (vftChild->getRegister() && (vftChild->getReferenceCount() > 1)) {
      ...
      deps->addPostCondition(
        vftChild->getRegister(), TR::RealRegister::NoReg, cg());
      cg()->recursivelyDecReferenceCount(vftChild);
    }
  }
...
TR::Register*
TR::AMD64SystemLinkage::buildDirectDispatch(TR::Node* callNode,
                                            bool spillFPRegs)
{
  TR::SymbolReference* methodSymRef = callNode->getSymbolReference();
  TR::MethodSymbol* methodSymbol =
  ...
  for (int32_t i = 0; i < post; i++) {
    if (postDeps->getPostConditions()
      ...
      scratchReg =
        postDeps->getPostConditions()->getRegisterDependency(i)->getRegister();
      break;
    }
  }
  ...
  if (methodSymbol->getMethodAddress()) {
    ...
    instr =
      generateImmSymInstruction(CALLImm4,
                                callNode,
                                (uintptrj_t)methodSymbol->getMethodAddress(),
                                methodSymRef,
                                preDeps,
                                cg());
  }
...
void
TR::AMD64ABILinkage::mapIncomingParms(TR::ResolvedMethodSymbol* method,
                                      uint32_t& stackIndex)
{
...
bool
TR::AMD64SystemLinkage::layoutTypeInRegs(TR::DataType type,
                                         uint16_t& intArgs,
                                         uint16_t& floatArgs,
                                         TR::parmLayoutResult& layoutResult)
{
...
int32_t
TR::AMD64SystemLinkage::layoutParm(TR::Node* parmNode,
                                   int32_t& dataCursor,
                                   uint16_t& intReg,
                                   uint16_t& floatReg,
                                   TR::parmLayoutResult& layoutResult)
{
  ...
  if (layoutTypeInRegs(
        parmNode->getDataType(), intReg, floatReg, layoutResult)) {
...
int32_t
TR::AMD64SystemLinkage::layoutParm(TR::ParameterSymbol* parmSymbol,
                                   int32_t& dataCursor,
                                   uint16_t& intReg,
                                   uint16_t& floatReg,
                                   TR::parmLayoutResult& layoutResult)
{
  ...
  if (layoutTypeInRegs(
        parmSymbol->getDataType(), intReg, floatReg, layoutResult)) {
    layoutResult.abstract |= TR::parmLayoutResult::IN_LINKAGE_REG;
