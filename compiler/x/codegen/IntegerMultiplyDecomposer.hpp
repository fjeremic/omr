...
public:
  ...
  TR_X86IntegerMultiplyDecomposer(
    int64_t multiplier, TR::Register* sreg, TR::Node* node, TR::CodeGenerator* cg, bool ccs = false, bool sf = false)
  ...
  TR::Register* getSourceRegister() const
  {
    return _sourceRegister;
  }

  TR::Node* getNode() const
  {
    return _node;
  }

  int64_t getConstantMultiplier() const
  {
    return _multiplier;
  }

  bool canClobberSource() const
  {
    return _canClobberSource;
  }
