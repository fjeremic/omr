...
public:
  ...
  TR_X86IntegerMultiplyDecomposer(int64_t multiplier, TR::Register *sreg,
                                  TR::Node *node, TR::CodeGenerator *cg,
                                  bool ccs = false, bool sf = false)
      : _multiplier(multiplier), _sourceRegister(sreg), _node(node), _cg(cg),
        _canClobberSource(ccs), _shiftFollows(sf) {}

  TR::Register *decomposeIntegerMultiplier(int32_t &tempRegArraySize,
...
private:
  ...
  typedef struct {
    int64_t _multiplier;
    bool _sourceDisjointWithFirstRegister; // means that if the source register
                                           // can be clobbered will the register
                                           // assigner be able to use the same
                                           // real register to assign both
                                           // source and register 1
    ...
    bool _subsequentShiftTooExpensive; // used to indicate that
                                       ...
                                       // but not when a subsequent shift is
                                       // added
    uint8_t _numAdditionalRegistersNeeded;
    componentOperation _components[MAX_NUM_COMPONENTS];
  } integerMultiplyComposition;

  static const integerMultiplyComposition
      _integerMultiplySolutions[NUM_CONSTS_DECOMPOSED];
