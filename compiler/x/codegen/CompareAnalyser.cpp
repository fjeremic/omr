...
static bool addressIsTemporarilyInt(TR::Node *node) {
  return (node->getOpCodeValue() == TR::a2i ||
          node->getOpCodeValue() == TR::a2l) &&
         (node->getReferenceCount() == 1);
}

void TR_X86CompareAnalyser::integerCompareAnalyser(
    TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
    bool determineEvaluationOrder, TR_X86OpCodes regRegOpCode,
    TR_X86OpCodes regMemOpCode, TR_X86OpCodes memRegOpCode) {
  ...
  // Do not allow direct memory forms for spine checks.  This ensures the
  // children will be available in registers or as constants.
  //
  ...
  if ((determineEvaluationOrder && (cg()->whichChildToEvaluate(root) == 0)) ||
  ...
  if (getCmpReg1Reg2()) {
    ...
    TR::X86RegMemInstruction *inst = generateRegMemInstruction(
        regMemOpCode, root, firstRegister, tempMR, cg());
  ...
  } else  // Must be CmpMem1Reg2
  {
    ...
    TR::X86MemRegInstruction *inst = generateMemRegInstruction(
        memRegOpCode, root, tempMR, secondRegister, cg());
    if (!cg()->getImplicitExceptionPoint())
      cg()->setImplicitExceptionPoint(inst);
    tempMR->decNodeReferenceCounts(cg());
  }
...
void TR_X86CompareAnalyser::integerCompareAnalyser(TR::Node *root,
                                                   TR_X86OpCodes regRegOpCode,
                                                   TR_X86OpCodes regMemOpCode,
                                                   TR_X86OpCodes memRegOpCode) {
  integerCompareAnalyser(root, root->getFirstChild(), root->getSecondChild(),
                         true, regRegOpCode, regMemOpCode, memRegOpCode);
}
...
//
static TR::Register *optimizeIU2L(TR::Node *child, TR::ILOpCodes origOp,
                                  TR::CodeGenerator *cg) {
...
void TR_X86CompareAnalyser::longOrderedCompareAndBranchAnalyser(
    TR::Node *root, TR_X86OpCodes lowBranchOpCode,
    TR_X86OpCodes highBranchOpCode, TR_X86OpCodes highReversedBranchOpCode) {
  ...
  // can generate better code for compares when one or more children are
  // TR::iu2l but only when we don't need the result of the iu2l for another
  // parent.
      ...
      if (firstOp == TR::su2l || firstOp == TR::bu2l) {
        ...
        if (firstOp == TR::lushr) {
          ...
          /*
           ...
           Apparently, it is possible for the first register (and, probably, the
           second one, also) to be NULL. For java code below if (end != 0
               && logEnd >>> 32)
               < end >>> 32))
               {
               logEnd = end;
          ...
          */
          // TR_ASSERT(firstRegister != NULL,"Can the first register be NULL in
          // this case?");
    ...
    if (secondChild->getReferenceCount() == 1 && secondRegister == 0) {
      if (secondOp == TR::su2l || secondOp == TR::bu2l) {
        ...
        if (secondOp == TR::lushr) {
          ...
          // TR_ASSERT(secondRegister != NULL,"Can the second register be NULL
          // in this case?");
    ...
    if (getEvalChild1()) {
      if (firstChild->getReferenceCount() == 1 &&
        ...
        if (!firstIU2L) {
          ...
          generateRegMemInstruction(L4RegMem, firstChild, delayedFirst,
                                    highFirstMR, cg());
        }
      } else if (firstIU2L) {
    ...
    if (getEvalChild2()) {
      if (secondChild->getReferenceCount() == 1 &&
        ...
        if (!secondIU2L) {
          ...
          generateRegMemInstruction(L4RegMem, secondChild, delayedSecond,
                                    highSecondMR, cg());
        }
      } else if (secondIU2L) {
  ...
  TR::LabelSymbol *destinationLabel =
      root->getBranchDestination()->getNode()->getLabel();
  ...
  if (hasGlobalDeps) {
    ...
    deps = generateRegisterDependencyConditions(
        third, cg(), numAdditionalRegDeps, &popRegisters);
  } else {
    deps = generateRegisterDependencyConditions(
        (uint8_t)0, (uint8_t)numAdditionalRegDeps, cg());
  }
  ...
  if (getCmpReg1Reg2()) {
    TR_ASSERT(delayedFirst == NULL && delayedSecond == NULL,
              "Bad combination in long ordered compare analyser\n");
    TR::Register *firstLow;
    if (firstHighZero == false) {
      deps->addPostCondition(firstRegister->getHighOrder(),
    ...
    if (secondHighZero == false) {
      deps->addPostCondition(secondRegister->getHighOrder(),
                             TR::RealRegister::NoReg, cg());
    ...
    if (firstHighZero) {
      if (secondHighZero == false)  // if both are ui2l, then we just need an
                                    // unsigned low order compare
      {
        generateRegImmInstruction(CMP4RegImms, root,
                                  secondRegister->getHighOrder(), 0, cg());
        highBranchOp = highReversedBranchOpCode;
      }
    } else if (secondHighZero) {
      generateRegImmInstruction(CMP4RegImms, root,
                                firstRegister->getHighOrder(), 0, cg());
    } else {
      generateRegRegInstruction(CMP4RegReg, root, firstRegister->getHighOrder(),
    ...
    if (firstHighZero == false || secondHighZero == false) {
      if (hasGlobalDeps == false) {
        ...
        generateLabelInstruction(highBranchOp, root, destinationLabel, deps,
                                 cg());
    ...
    TR_X86OpCodes highBranchOp = highBranchOpCode;
    if (firstHighZero) {
      if (secondIU2L == false)  // if both are ui2l, then we just need an
                                // unsigned low order compare
      {
    ...
    if (delayedFirst) {
      generateRegMemInstruction(L4RegMem, firstChild, delayedFirst, lowFirstMR,
                                cg());
  ...
  if (lowMR != NULL) lowMR->decNodeReferenceCounts(cg());
  ...
  if (!popRegisters.isEmpty()) {
    ...
    for (TR::Register *popRegister = popRegsIt.getFirst(); popRegister != NULL;
      ...
      generateFPSTiST0RegRegInstruction(FSTRegReg, root, popRegister,
                                        popRegister, cg());
...
void TR_X86CompareAnalyser::longEqualityCompareAndBranchAnalyser(
    TR::Node *root, TR::LabelSymbol *firstBranchLabel,
    TR::LabelSymbol *secondBranchLabel, TR_X86OpCodes secondBranchOp) {
  TR::Node *firstChild = root->getFirstChild();
  TR::Node *secondChild = root->getSecondChild();
  TR::Register *firstRegister = firstChild->getRegister();
  TR::Register *secondRegister = secondChild->getRegister();
  ...
  if (getCmpReg1Reg2()) {
    generateRegRegInstruction(CMP4RegReg, root, firstRegister->getLowOrder(),
                              secondRegister->getLowOrder(), cg());
    if (deps != NULL) {
      deps->addPostCondition(firstRegister->getHighOrder(),
                             TR::RealRegister::NoReg, cg());
      deps->addPostCondition(firstRegister->getLowOrder(),
                             TR::RealRegister::NoReg, cg());
      deps->addPostCondition(secondRegister->getHighOrder(),
                             TR::RealRegister::NoReg, cg());
      deps->addPostCondition(secondRegister->getLowOrder(),
                             TR::RealRegister::NoReg, cg());
      deps->stopAddingConditions();
    }
    if (hasGlobalDeps == false) {
    ...
    generateRegMemInstruction(CMP4RegMem, root, firstRegister->getLowOrder(),
                              lowMR, cg());
    if (hasGlobalDeps == false) {
      generateLabelInstruction(JNE4, root, firstBranchLabel, cg());
    } else {
    ...
    generateRegMemInstruction(CMP4RegMem, root, firstRegister->getHighOrder(),
                              highMR, cg());
  } else  // Must be CmpMem1Reg2
  {
    if (deps != NULL) {
    ...
    generateMemRegInstruction(CMP4MemReg, root, lowMR,
                              secondRegister->getLowOrder(), cg());
    if (hasGlobalDeps == false) {
      generateLabelInstruction(JNE4, root, firstBranchLabel, cg());
    } else {
    ...
    generateMemRegInstruction(CMP4MemReg, root, highMR,
                              secondRegister->getHighOrder(), cg());
  }
...
TR::Register *TR_X86CompareAnalyser::longEqualityBooleanAnalyser(
    TR::Node *root, TR_X86OpCodes setOpCode, TR_X86OpCodes combineOpCode) {
  TR::Node *firstChild = root->getFirstChild();
  TR::Node *secondChild = root->getSecondChild();
  TR::Register *firstRegister = firstChild->getRegister();
  TR::Register *secondRegister = secondChild->getRegister();
  ...
  generateRegRegInstruction(combineOpCode, root, highRegister, lowRegister,
                            cg());

  generateRegRegInstruction(MOVZXReg4Reg1, root, highRegister, highRegister,
                            cg());
...
TR::Register *TR_X86CompareAnalyser::longOrderedBooleanAnalyser(
    TR::Node *root, TR_X86OpCodes highSetOpCode, TR_X86OpCodes lowSetOpCode) {
  TR::Node *firstChild = root->getFirstChild();
  TR::Node *secondChild = root->getSecondChild();
  TR::Register *firstRegister = firstChild->getRegister();
  TR::Register *secondRegister = secondChild->getRegister();
  ...
  generateRegRegInstruction(MOVZXReg4Reg1, root, setRegister, setRegister,
                            cg());
...
TR::Register *TR_X86CompareAnalyser::longCMPAnalyser(TR::Node *root) {
  ...
  generateRegRegInstruction(MOVSXReg4Reg1, root, setRegister, setRegister,
                            cg());
