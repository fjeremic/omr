...
void TR_X86CompareAnalyser::longOrderedCompareAndBranchAnalyser(
  TR::Node* root, TR_X86OpCodes lowBranchOpCode, TR_X86OpCodes highBranchOpCode, TR_X86OpCodes highReversedBranchOpCode)
{
      ...
      if (firstOp == TR::su2l || firstOp == TR::bu2l) {
        ...
        if (firstOp == TR::lushr) {
          ...
          /*
           ...
           Apparently, it is possible for the first register (and, probably, the second one, also) to be NULL. For java
    code below if (end != 0
               && logEnd >>> 32)
               < end >>> 32))
               {
...
void TR_X86CompareAnalyser::longEqualityCompareAndBranchAnalyser(
  TR::Node* root, TR::LabelSymbol* firstBranchLabel, TR::LabelSymbol* secondBranchLabel, TR_X86OpCodes secondBranchOp)
{
  TR::Node* firstChild = root->getFirstChild();
  TR::Node* secondChild = root->getSecondChild();
  TR::Register* firstRegister = firstChild->getRegister();
