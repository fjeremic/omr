    ...
    TR_X86OpCodes highReversedBranchOpCode)
{
    ...
    if (firstChild->isHighWordZero()) {
        ...
        if (firstChild->getReferenceCount() == 1 && firstRegister == 0) {
            if (firstOp == TR::su2l || firstOp == TR::bu2l) {
                firstIU2L = true;
            } else if (firstOp == TR::iu2l || (firstOp == TR::lushr && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && (firstChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true)) == 32)) {
                firstChild = firstChild->getFirstChild();
    ...
    if (secondChild->isHighWordZero()) {
        ...
        if (secondChild->getReferenceCount() == 1 && secondRegister == 0) {
            if (secondOp == TR::su2l || secondOp == TR::bu2l) {
                secondIU2L = true;
            } else if (secondOp == TR::iu2l || (secondOp == TR::lushr && secondChild->getSecondChild()->getOpCodeValue() == TR::iconst && (secondChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true)) == 32)) {
                secondChild = secondChild->getFirstChild();
    ...
    if (cg()->whichChildToEvaluate(root) == 0) {
        if (getEvalChild1()) {
            if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && (firstChild->getOpCodeValue() == TR::lload || (firstChild->getOpCodeValue() == TR::iload && firstIU2L))) {
        ...
        if (getEvalChild2()) {
            if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL && (secondChild->getOpCodeValue() == TR::lload || (secondChild->getOpCodeValue() == TR::iload && secondIU2L))) {
