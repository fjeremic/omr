...
static bool
addressIsTemporarilyInt(TR::Node* node)
{
...
void
TR_X86CompareAnalyser::integerCompareAnalyser(TR::Node* root,
                                              TR::Node* firstChild,
                                              TR::Node* secondChild,
                                              ...
                                              TR_X86OpCodes memRegOpCode)
{
  ...
  if (getCmpReg1Reg2()) {
    generateRegRegInstruction(
      regRegOpCode, root, firstRegister, secondRegister, cg());
...
//
static TR::Register*
optimizeIU2L(TR::Node* child, TR::ILOpCodes origOp, TR::CodeGenerator* cg)
{
...
void
TR_X86CompareAnalyser::longOrderedCompareAndBranchAnalyser(
  TR::Node* root,
  ...
  TR_X86OpCodes highReversedBranchOpCode)
{
      ...
      if (firstChild->getReferenceCount() == 1 &&
        ...
        if (!firstIU2L) {
          ...
          generateRegMemInstruction(
            L4RegMem, firstChild, delayedFirst, highFirstMR, cg());
        }
      } else if (firstIU2L) {
    ...
    if (getEvalChild2()) {
      if (secondChild->getReferenceCount() == 1 &&
        ...
        if (!secondIU2L) {
          ...
          generateRegMemInstruction(
            L4RegMem, secondChild, delayedSecond, highSecondMR, cg());
        }
      } else if (secondIU2L) {
  ...
  if (getCmpReg1Reg2()) {
    ...
    if (firstHighZero == false) {
      deps->addPostCondition(
        firstRegister->getHighOrder(), TR::RealRegister::NoReg, cg());
    ...
    if (secondHighZero == false) {
      deps->addPostCondition(
        secondRegister->getHighOrder(), TR::RealRegister::NoReg, cg());
    ...
    if (firstHighZero) {
                                   ...
                                   // unsigned low order compare
      {
        generateRegImmInstruction(
          CMP4RegImms, root, secondRegister->getHighOrder(), 0, cg());
        highBranchOp = highReversedBranchOpCode;
      }
    } else if (secondHighZero) {
      generateRegImmInstruction(
        CMP4RegImms, root, firstRegister->getHighOrder(), 0, cg());
    } else {
      generateRegRegInstruction(CMP4RegReg,
                                root,
    ...
    if (firstHighZero == false || secondHighZero == false) {
      if (hasGlobalDeps == false) {
        ...
        generateLabelInstruction(
          highBranchOp, root, destinationLabel, deps, cg());
    ...
    if (delayedFirst) {
      generateRegMemInstruction(
        L4RegMem, firstChild, delayedFirst, lowFirstMR, cg());
  ...
  } else // Must be CmpMem1Reg2
  {
    ...
    if (delayedSecond) {
      generateRegMemInstruction(
        L4RegMem, secondChild, delayedSecond, lowSecondMR, cg());
  ...
  if (!popRegisters.isEmpty()) {
    ...
    for (TR::Register* popRegister = popRegsIt.getFirst(); popRegister != NULL;
      ...
      generateFPSTiST0RegRegInstruction(
        FSTRegReg, root, popRegister, popRegister, cg());
...
void
TR_X86CompareAnalyser::longEqualityCompareAndBranchAnalyser(
  TR::Node* root,
  ...
  TR_X86OpCodes secondBranchOp)
{
  ...
  if (getCmpReg1Reg2()) {
    ...
    generateRegMemInstruction(
      CMP4RegMem, root, firstRegister->getLowOrder(), lowMR, cg());
    if (hasGlobalDeps == false) {
      generateLabelInstruction(JNE4, root, firstBranchLabel, cg());
    } else {
      generateLabelInstruction(JNE4, root, firstBranchLabel, deps, cg());
    ...
    generateRegMemInstruction(
      CMP4RegMem, root, firstRegister->getHighOrder(), highMR, cg());
  } else // Must be CmpMem1Reg2
  {
    if (deps != NULL) {
      deps->addPostCondition(
    ...
    generateMemRegInstruction(
      CMP4MemReg, root, lowMR, secondRegister->getLowOrder(), cg());
    if (hasGlobalDeps == false) {
      generateLabelInstruction(JNE4, root, firstBranchLabel, cg());
    } else {
      generateLabelInstruction(JNE4, root, firstBranchLabel, deps, cg());
    ...
    generateMemRegInstruction(
      CMP4MemReg, root, highMR, secondRegister->getHighOrder(), cg());
  }
...
TR::Register*
TR_X86CompareAnalyser::longEqualityBooleanAnalyser(TR::Node* root,
                                                   ...
                                                   TR_X86OpCodes combineOpCode)
{
  ...
  generateRegRegInstruction(
    combineOpCode, root, highRegister, lowRegister, cg());

  generateRegRegInstruction(
    MOVZXReg4Reg1, root, highRegister, highRegister, cg());
...
TR::Register*
TR_X86CompareAnalyser::longOrderedBooleanAnalyser(TR::Node* root,
                                                  ...
                                                  TR_X86OpCodes lowSetOpCode)
{
  ...
  generateRegRegInstruction(
    MOVZXReg4Reg1, root, setRegister, setRegister, cg());
...
TR::Register*
TR_X86CompareAnalyser::longCMPAnalyser(TR::Node* root)
{
  TR::Node* firstChild = root->getFirstChild();
  TR::Node* secondChild = root->getSecondChild();
  TR::Register* firstRegister = firstChild->getRegister();
  ...
  generateRegRegInstruction(
    MOVSXReg4Reg1, root, setRegister, setRegister, cg());
