...
void OMR::X86::MemoryReference::initialize(
...
void OMR::X86::MemoryReference::useRegisters(
...
void OMR::X86::MemoryReference::populateMemoryReference(
    TR::Node* subTree,
    TR::CodeGenerator* cg, TR::Node* parent)
{
    ...
    if ((evalSubTree && subTree->getReferenceCount() > 1)
        ...
        } else if ((subTreeOp == TR::isub || subTreeOp == TR::lsub) && (subTree->getSecondChild()->getOpCodeValue() == TR::iconst || subTree->getSecondChild()->getOpCodeValue() == TR::lconst)) {
...
void OMR::X86::MemoryReference::consolidateRegisters(
    TR::Node* node,
    TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
...
OMR::X86::MemoryReference::estimateBinaryLength(TR::CodeGenerator* cg)
{
    ...
    uint32_t addressTypes = (self()->getBaseRegister() != NULL ? 1 : 0) | (self()->getIndexRegister() != NULL ? 2 : 0) | ((self()->getSymbolReference().getSymbol() != NULL || self()->getSymbolReference().getOffset() != 0 || self()->isForceWideDisplacement()) ? 4 : 0);
    uint32_t length = 0;
...
OMR::X86::MemoryReference::getBinaryLengthLowerBound(TR::CodeGenerator* cg)
{
    ...
    if (self()->getBaseRegister()) {
        ...
        if (registerNumber == TR::RealRegister::vfp) {
            ...
            registerNumber = toRealRegister(cg->machine()->getRealRegister(registerNumber)->getAssignedRealRegister())->getRegisterNumber();
        }
    }
...
OMR::X86::EnlargementResult OMR::X86::MemoryReference::enlarge(TR::CodeGenerator* cg, int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
    ...
    if (!self()->isForceWideDisplacement()) {
        ...
        if (potentialPatchGrowth > 0 && (potentialPatchGrowth >= requestedEnlargementSize || allowPartialEnlargement) && potentialEncodingGrowth <= maxEnlargementSize && performTransformation(cg->comp(), "O^O Enlarging memory reference by %d bytes by forcing wide displacement - allowpartial was %d", potentialPatchGrowth, allowPartialEnlargement)) {
    ...
    TR::CodeGenerator* cg)
{
    ...
    case 5: {
        ...
        if (!(displacement == 0 && !base->needsDisp() && !base->needsDisp() && !self()->isForceWideDisplacement()) && !(displacement >= -128 && displacement <= 127 && !self()->isForceWideDisplacement())) {
            if (self()->getSymbolReference().getSymbol()
                && self()->getSymbolReference().getSymbol()->isClassObject()
...
static void rematerializeAddressAdds(
    TR::Node* rootLoadOrStore,
    TR::CodeGenerator* cg)
{
