...
static void
rematerializeAddressAdds(TR::Node* rootLoadOrStore, TR::CodeGenerator* cg);
      ...
      if (!isUnresolved && base->getOpCodeValue() == TR::loadaddr &&
        ...
        if (isUnresolved) {
          ...
          self()->setUnresolvedDataSnippet(
            TR::UnresolvedDataSnippet::create(cg,
                                              rootLoadOrStore,
                                              &_symbolReference,
                                              isStore,
                                              symRef->canCauseGC()));
...
TR::UnresolvedDataSnippet*
OMR::X86::MemoryReference::setUnresolvedDataSnippet(
...
void
OMR::X86::MemoryReference::initialize(TR::SymbolReference* symRef,
                                      TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Symbol* symbol = symRef->getSymbol();
...
//
void
OMR::X86::MemoryReference::checkAndDecReferenceCount(TR::Node* node,
                                                     rcount_t refCount,
                                                     TR::CodeGenerator* cg)
{
...
void
OMR::X86::MemoryReference::useRegisters(TR::Instruction* instr,
                                        TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
...
int32_t
OMR::X86::MemoryReference::getStrideForNode(TR::Node* node,
                                            TR::CodeGenerator* cg)
{
  int32_t stride = 0;
...
void
OMR::X86::MemoryReference::populateMemoryReference(TR::Node* subTree,
                                                   TR::CodeGenerator* cg,
                                                   TR::Node* parent)
{
  ...
  if ((evalSubTree && subTree->getReferenceCount() > 1) ||
    ...
    if (subTree->getOpCode().isArrayRef() ||
      ...
      if (comp->getOption(TR_TraceCG))
        traceMsg(
          comp, "Entering UpcastingNoOverflow mode at node %x\n", subTree);
      rcount_t refCount = subTree->getFirstChild()->getReferenceCount();
  ...
  if (comp->getOption(TR_TraceRegisterPressureDetails)) {
    traceMsg(
      comp, "   populated memref on %s", cg->getDebug()->getName(subTree));
...
TR::Register*
OMR::X86::MemoryReference::evaluate(TR::Node* node,
                                    TR::CodeGenerator* cg,
                                    TR::Node* parent)
{
...
void
OMR::X86::MemoryReference::consolidateRegisters(TR::Node* node,
                                                TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  if (comp->getOption(TR_TraceRegisterPressureDetails)) {
    traceMsg(
      comp, "  consolidateRegisters on %s", cg->getDebug()->getName(node));
    cg->getDebug()->dumpLiveRegisters();
    traceMsg(comp, "\n");
  }
  ...
  generateRegMemInstruction(
    LEARegMem(), node, tempTargetRegister, interimMemoryReference, cg);
...
void
OMR::X86::MemoryReference::assignRegisters(TR::Instruction* currentInstruction,
                                           TR::CodeGenerator* cg)
{
...
OMR::X86::MemoryReference::estimateBinaryLength(TR::CodeGenerator* cg)
{
  if (self()->getBaseRegister() &&
    ...
    self()->getSymbolReference().setOffset(
      self()->getSymbolReference().getOffset() + cg->vfpState()._displacement);
  }
...
OMR::X86::MemoryReference::enlarge(TR::CodeGenerator* cg,
...
void
OMR::X86::MemoryReference::addMetaDataForCodeAddress(uint32_t addressTypes,
                                                     uint8_t* cursor,
                                                     TR::Node* node,
                                                     TR::CodeGenerator* cg)
{
        ...
        if (staticSym) {
          if (self()->getUnresolvedDataSnippet() == NULL) {
            if (symbol->isConst()) {
              ...
              {
                cg->jitAdd32BitPicToPatchOnClassRedefinition(
                  (void*)(uintptr_t) * (int32_t*)cursor, (void*)cursor, false);
              }
            } else {
  ...
  TR::CodeGenerator* cg)
{
  ...
  self()->addMetaDataForCodeAddress(
    addressTypes, immediateCursor, containingInstruction->getNode(), cg);
...
const uint8_t
  OMR::X86::MemoryReference::_multiplierToStrideMap[HIGHEST_STRIDE_MULTIPLIER +
                                                    1] = {
    0, // 0
    0, // 1
    1, // 2
...
TR::MemoryReference*
generateX86MemoryReference(TR::Register* br,
                           intptrj_t disp,
                           TR::CodeGenerator* cg)
{
...
TR::MemoryReference*
generateX86MemoryReference(TR::Node* node,
                           TR::CodeGenerator* cg,
                           bool canRematerializeAddressAdds)
{
...
TR::MemoryReference*
generateX86MemoryReference(TR::MemoryReference& mr,
                           intptrj_t n,
                           TR::CodeGenerator* cg)
{
...
TR::MemoryReference*
generateX86MemoryReference(TR::SymbolReference* sr,
                           intptrj_t displacement,
                           TR::CodeGenerator* cg)
{
