...
static void rematerializeAddressAdds(TR::Node *rootLoadOrStore,
                                     TR::CodeGenerator *cg);

intptrj_t OMR::X86::MemoryReference::getDisplacement() {
  ...
  if (symbol != NULL) {
    if (symbol->isRegisterMappedSymbol()) {
      TR_ASSERT(!symRef.isUnresolved(),
                "Whoops, a register mapped symbol is unresolved");
      displacement += symbol->castToRegisterMappedSymbol()->getOffset();
    } else if (!symRef.isUnresolved() && symbol->isStatic()) {
      displacement +=
          (uintptrj_t)symbol->castToStaticSymbol()->getStaticAddress();
    }
  }
...
OMR::X86::MemoryReference::MemoryReference(TR::X86DataSnippet *cds,
                                           TR::CodeGenerator *cg)
    : _baseRegister(NULL),
      _baseNode(NULL),
      _indexRegister(NULL),
...
OMR::X86::MemoryReference::MemoryReference(TR::LabelSymbol *label,
                                           TR::CodeGenerator *cg)
    : _baseRegister(NULL),
      _baseNode(NULL),
      _indexRegister(NULL),
...
OMR::X86::MemoryReference::MemoryReference(TR::SymbolReference *symRef,
                                           TR::CodeGenerator *cg)
    : _baseRegister(NULL),
      _baseNode(NULL),
      _indexRegister(NULL),
...
//
OMR::X86::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
                                           TR::CodeGenerator *cg,
  ...
  if (symRef) {
    ...
    if (rootLoadOrStore->getOpCode().isIndirect()) {
      ...
      if (!isUnresolved && base->getOpCodeValue() == TR::loadaddr &&
        ...
        if (isUnresolved) {
          ...
          //
          if (base->getOpCodeValue() == TR::loadaddr &&
              base->getSymbol()->isLocalObject())
            cg->evaluate(base);

          self()->setUnresolvedDataSnippet(TR::UnresolvedDataSnippet::create(
              cg, rootLoadOrStore, &_symbolReference, isStore,
              symRef->canCauseGC()));
    ...
    diagnostic(
        "OMR::X86::MemoryReference::OMR::X86::MemoryReference() ==> no symbol "
        "reference");
  }
...
OMR::X86::MemoryReference::getUnresolvedDataSnippet() {
  return self()->hasUnresolvedDataSnippet()
             ? (TR::UnresolvedDataSnippet *)_dataSnippet
             : NULL;
}

TR::UnresolvedDataSnippet *OMR::X86::MemoryReference::setUnresolvedDataSnippet(
...
TR::X86DataSnippet *OMR::X86::MemoryReference::getDataSnippet() {
  return (self()->hasUnresolvedDataSnippet() ||
          self()->hasUnresolvedVirtualCallSnippet())
             ? NULL
             : (TR::X86DataSnippet *)_dataSnippet;
}

void OMR::X86::MemoryReference::initialize(TR::SymbolReference *symRef,
                                           TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Symbol *symbol = symRef->getSymbol();
  ...
  if (symRef->isUnresolved()) {
    self()->setUnresolvedDataSnippet(TR::UnresolvedDataSnippet::create(
        cg, 0, &_symbolReference, false, symRef->canCauseGC()));
    cg->addSnippet(self()->getUnresolvedDataSnippet());
    self()->setForceWideDisplacement();
  }
  // TODO: aliasing sets?
...
OMR::X86::MemoryReference::MemoryReference(TR::MemoryReference &mr, intptrj_t n,
                                           TR::CodeGenerator *cg,
  ...
  _label = mr._label;
  _symbolReference =
      TR::SymbolReference(comp->getSymRefTab(), mr._symbolReference, n);
  _reloKind = -1;

  if (mr.getUnresolvedDataSnippet() != NULL) {
    _dataSnippet = TR::UnresolvedDataSnippet::create(
        cg, _baseNode, &_symbolReference, false, _symbolReference.canCauseGC());
...
//
void OMR::X86::MemoryReference::checkAndDecReferenceCount(
    TR::Node *node, rcount_t refCount, TR::CodeGenerator *cg) {
...
void OMR::X86::MemoryReference::stopUsingRegisters(TR::CodeGenerator *cg) {
  TR::Register *vmThreadReg = cg->getVMThreadRegister();
  if (_baseRegister != NULL) {
    if (_baseRegister != vmThreadReg) cg->stopUsingRegister(_baseRegister);
  }
  if (_indexRegister != NULL) {
    if (_indexRegister != vmThreadReg) cg->stopUsingRegister(_indexRegister);
  }
}

void OMR::X86::MemoryReference::useRegisters(TR::Instruction *instr,
                                             TR::CodeGenerator *cg) {
  if (_baseRegister != NULL) {
...
int32_t OMR::X86::MemoryReference::getStrideForNode(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  int32_t stride = 0;
  if (node->getOpCodeValue() == TR::imul ||
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
      ...
      if (shiftAmount <= HIGHEST_STRIDE_SHIFT) stride = shiftAmount;
    }
  }
  return stride;
}

void OMR::X86::MemoryReference::populateMemoryReference(TR::Node *subTree,
                                                        TR::CodeGenerator *cg,
                                                        TR::Node *parent) {
  ...
  if (comp->useCompressedPointers()) {
    if ((subTree->getOpCodeValue() == TR::l2a) &&
      ...
      if (subTree->getRegister() == NULL) nodeToBeAdjusted = subTree;
    }
  }
  ...
  if ((evalSubTree && subTree->getReferenceCount() > 1) ||
    ...
    if (subTree->getOpCode().isArrayRef() ||
        (subTreeOp == TR::iadd || subTreeOp == TR::ladd)) {
      ...
      if (integerChild->getOpCode().isLoadConst()) {
        ...
        _symbolReference.addToOffset(
            TR::TreeEvaluator::integerConstNodeValue(integerChild, cg));
      ...
      _symbolReference.addToOffset(
          -TR::TreeEvaluator::integerConstNodeValue(constChild, cg));
      cg->decReferenceCount(constChild);
    } else if (subTreeOp == TR::i2l || subTreeOp == TR::s2l ||
               subTreeOp == TR::s2i) {
    ...
    } else if ((stride = TR::MemoryReference::getStrideForNode(subTree, cg)) !=
               0) {
      if (_indexRegister != NULL) {
      ...
      if (firstChild->getOpCodeValue() == TR::i2l &&
        ...
        if (!sourceReg) self()->evaluate(i2lChild, cg, parent);

        sourceReg = i2lChild->getRegister();

        if (debug("traceMemOp"))
          diagnostic(
              "\nMEMREF ***sourceReg = %p, upperBitsZero = %d, isNonNegative = "
              "%d, skipSignExtension = %d \n",
              sourceReg, sourceReg->areUpperBitsZero(),
              i2lChild->isNonNegative(), i2lChild->skipSignExtension());
        if (sourceReg != NULL &&
      ...
      if (symbol->isRegisterMappedSymbol()) {
        if (_baseRegister != NULL) {
          ...
          if (!symbol->isMethodMetaData()) {
            ...
            // using the frame register as the index register of a memory
            // reference
            //
      ...
      if (symRef->isUnresolved()) {
        self()->setUnresolvedDataSnippet(TR::UnresolvedDataSnippet::create(
            cg, subTree, &_symbolReference, false, symRef->canCauseGC()));
        cg->addSnippet(self()->getUnresolvedDataSnippet());
        self()->setForceWideDisplacement();
      }
      ...
      // Need to decrement ref count because nodes weren't set on
      // memoryreference.
      //
      cg->decReferenceCount(subTree);
    } else if (subTreeOp == TR::aconst) {
      _symbolReference.addToOffset(
          TR::TreeEvaluator::integerConstNodeValue(subTree, cg));
    } else {
      if (_baseRegister != NULL) {
        if (_indexRegister != NULL) {
          self()->consolidateRegisters(subTree, cg);
  ...
  if (comp->getOption(TR_TraceRegisterPressureDetails)) {
    traceMsg(comp, "   populated memref on %s",
             cg->getDebug()->getName(subTree));
    cg->getDebug()->dumpLiveRegisters();
    traceMsg(comp, "\n");
  }
}

TR::Register *OMR::X86::MemoryReference::evaluate(TR::Node *node,
                                                  TR::CodeGenerator *cg,
                                                  TR::Node *parent) {
  ...
  if (self()->inUpcastingMode()) {
    ...
    else if ((node->isNonNegative() || (parent && parent->isNonNegative())) &&
    ...
    // else if (node->isNonNegative() && reg->areUpperBitsZero())
    {
      ...
      if (cg->comp()->getOption(TR_TraceCG))
        traceMsg(
            cg->comp(),
            "Add a sign extension instruction to 64-bit in Upcasting Mode %x\n",
            instr);
    }
      ...
      if (cg->comp()->getOption(TR_TraceCG))
        traceMsg(
            cg->comp(),
            "Add a sign extension instruction to 32-bit in Upcasting Mode %x\n",
            instr);
    }
  }
...
void OMR::X86::MemoryReference::consolidateRegisters(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  if (comp->getOption(TR_TraceRegisterPressureDetails)) {
    traceMsg(comp, "  consolidateRegisters on %s",
  ...
  generateRegMemInstruction(LEARegMem(), node, tempTargetRegister,
                            interimMemoryReference, cg);
...
void OMR::X86::MemoryReference::assignRegisters(
  ...
  if (_baseRegister != NULL) {
    if (_baseRegister ==
        cg->machine()->getRealRegister(TR::RealRegister::vfp)) {
      ...
      if (assignedBaseRegister == NULL) {
        // Note: a MemRef can be used only once -- if you want to reuse make a
        // copy using generateX86MemoryReference(OMR::X86::MemoryReference  &,
        // intptrj_t, TR::CodeGenerator *cg).
        TR_ASSERT(!_baseRegister->getRealRegister(),
                  "_baseRegister is a Real Register already, are you reusing a "
                  "Memory Reference?");
        assignedBaseRegister =
            assignGPRegister(currentInstruction, _baseRegister, TR_WordReg, cg);
      }
  ...
  if (_indexRegister != NULL) {
    ...
    if (assignedIndexRegister == NULL) {
      assignedIndexRegister =
          assignGPRegister(currentInstruction, _indexRegister, TR_WordReg, cg);
    }
...
uint32_t OMR::X86::MemoryReference::estimateBinaryLength(
    TR::CodeGenerator *cg) {
  if (self()->getBaseRegister() &&
      toRealRegister(self()->getBaseRegister())->getRegisterNumber() ==
          TR::RealRegister::vfp) {
    // Rewrite VFP-relative memref in terms of an actual register
    //
    _baseRegister = cg->machine()->getRealRegister(cg->vfpState()._register);
    self()->getSymbolReference().setOffset(
        self()->getSymbolReference().getOffset() +
        cg->vfpState()._displacement);
  }

  TR::RealRegister *base = toRealRegister(self()->getBaseRegister());

  intptrj_t displacement;
  uint32_t addressTypes = (self()->getBaseRegister() != NULL ? 1 : 0) |
  ...
  switch (addressTypes) {
    ...
    case 3:
      length = 1;
      if (base->needsDisp()) {
        // Need a sib byte with 8bit displacement field of zero to get ebp as a
        // base register
        //
        length = 2;
      }
    ...
    case 5:
      ...
      if (displacement == 0 && !base->needsDisp() && !base->needsSIB() &&
          !self()->isForceWideDisplacement()) {
        length = 0;
...
uint32_t OMR::X86::MemoryReference::getBinaryLengthLowerBound(
    TR::CodeGenerator *cg) {
  intptrj_t displacement;
  uint32_t addressTypes = (self()->getBaseRegister() != NULL ? 1 : 0) |
                          (self()->getIndexRegister() != NULL ? 2 : 0) |
  ...
  if (self()->getBaseRegister()) {
    ...
    if (registerNumber == TR::RealRegister::vfp) {
      TR_ASSERT(cg->machine()
                    ->getRealRegister(registerNumber)
                    ->getAssignedRealRegister(),
                "virtual frame pointer must be assigned before estimating "
                "instruction length lower bound!\n");
      registerNumber = toRealRegister(cg->machine()
                                          ->getRealRegister(registerNumber)
                                          ->getAssignedRealRegister())
                           ->getRegisterNumber();
    }
...
OMR::X86::EnlargementResult OMR::X86::MemoryReference::enlarge(
    TR::CodeGenerator *cg, int32_t requestedEnlargementSize,
  ...
  if (!disableMemRefExpansion) return OMR::X86::EnlargementResult(0, 0);
  ...
  if (!self()->isForceWideDisplacement()) {
    ...
    int32_t potentialEncodingGrowth =
        self()->estimateBinaryLength(cg) - currentPatchSize;
    int32_t potentialPatchGrowth =
        self()->getBinaryLengthLowerBound(cg) - currentEncodingAllocation;

    if (potentialPatchGrowth > 0 &&
        (potentialPatchGrowth >= requestedEnlargementSize ||
         allowPartialEnlargement) &&
        potentialEncodingGrowth <= maxEnlargementSize &&
        performTransformation(cg->comp(),
                              "O^O Enlarging memory reference by %d bytes by "
                              "forcing wide displacement - allowpartial was %d",
                              potentialPatchGrowth, allowPartialEnlargement)) {
      return OMR::X86::EnlargementResult(potentialPatchGrowth,
                                         potentialEncodingGrowth);
    } else {
...
void OMR::X86::MemoryReference::addMetaDataForCodeAddress(
    uint32_t addressTypes, uint8_t *cursor, TR::Node *node,
    TR::CodeGenerator *cg) {
  switch (addressTypes) {
    case 6:
    case 2: {
      if (self()->needsCodeAbsoluteExternalRelocation()) {
        cg->addExternalRelocation(
            new (cg->trHeapMemory())
                TR::ExternalRelocation(cursor, 0, TR_AbsoluteMethodAddress, cg),
      ...
      if (self()->getSymbolReference().getSymbol() &&
          ...
          cg->wantToPatchClassPointer(
              NULL, cursor))  // might not point to beginning of class
      {
        cg->jitAdd32BitPicToPatchOnClassRedefinition(
            (void *)(uintptr_t) * (int32_t *)cursor, (void *)cursor,
            self()->getUnresolvedDataSnippet() != NULL);
      }
      ...
      if (symbol) {
          ...
          if (self()->getUnresolvedDataSnippet() == NULL) {
            if (symbol->isConst()) {
              ...
              cg->addExternalRelocation(
                  new (cg->trHeapMemory()) TR::ExternalRelocation(
                      ...
                      (uint8_t *)self()
                          ...
                          ->constantPool(),
                      node ? (uint8_t *)(intptrj_t)node->getInlinedSiteIndex()
                           : (uint8_t *)-1,
                      TR_ConstantPool, cg),
                  __FILE__, __LINE__, node);
            } else if (symbol->isClassObject()) {
              if (cg->needClassAndMethodPointerRelocations()) {
                *(int32_t *)cursor = (int32_t)(
                    TR::Compiler->cls.persistentClassPointerFromClassPointer(
                        cg->comp(),
                        (TR_OpaqueClassBlock
                             *)(self()->getSymbolReference().getOffset() +
                                (intptrj_t)staticSym->getStaticAddress())));
                if (cg->comp()->getOption(TR_UseSymbolValidationManager)) {
                  ...
                  cg->addExternalRelocation(
                      new (cg->trHeapMemory()) TR::ExternalRelocation(
                          cursor, (uint8_t *)&self()->getSymbolReference(),
                          node ? (uint8_t *)(intptrj_t)
              ...
              if (staticSym->isCountForRecompile()) {
                ...
                TR::DebugCounter::generateRelocation(cg->comp(), cursor, node,
                                                     counter);
              } else {
                cg->addExternalRelocation(
                    new (cg->trHeapMemory()) TR::ExternalRelocation(
                        cursor, (uint8_t *)&self()->getSymbolReference(),
                        node ? (uint8_t *)(uintptr_t)node->getInlinedSiteIndex()
                             : (uint8_t *)-1,
                        TR_DataAddress, cg),
                    __FILE__, __LINE__, node);
              }
            ...
            if (symbol->isClassObject() &&
                cg->wantToPatchClassPointer(NULL, cursor))  // unresolved
            {
              cg->jitAdd32BitPicToPatchOnClassRedefinition(
                  (void *)-1, (void *)cursor, true);
            }
          }
        }
      } else {
        ...
        if (label != NULL) {
          if (TR::Compiler->target.is64Bit()) {
            ...
            // TODO:AMD64: Would it be cleaner to have some kind of "isRelative"
            // flag rather than "is64BitTarget"?
            cg->addRelocation(
                new (cg->trHeapMemory())
    ...
    case 5: {
      ...
      if (!(displacement == 0 && !base->needsDisp() && !base->needsDisp() &&
            !self()->isForceWideDisplacement()) &&
        ...
        if (self()->getSymbolReference().getSymbol() &&
            self()->getSymbolReference().getSymbol()->isClassObject() &&
            cg->wantToPatchClassPointer(
                NULL, cursor))  // possibly unresolved, may not point to
                                // beginning of class
        {
          cg->jitAdd32BitPicToPatchOnClassRedefinition(
              (void *)(uintptr_t) * (int32_t *)cursor, (void *)cursor,
...
uint8_t *OMR::X86::MemoryReference::generateBinaryEncoding(
    uint8_t *modRM, TR::Instruction *containingInstruction,
    TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  switch (addressTypes) {
    case 1: {
      ...
      if (baseRegisterNumber == TR::RealRegister::vfp) {
        TR_ASSERT(
            cg->machine()
                ->getRealRegister(baseRegisterNumber)
                ->getAssignedRealRegister(),
    ...
    case 4: {
      ...
      if (symbol) {
        ...
        if (staticSym) {
          if (self()->getUnresolvedDataSnippet() == NULL) {
            *(int32_t *)cursor =
                (int32_t)(self()->getSymbolReference().getOffset() +
                          (intptrj_t)staticSym->getStaticAddress());
          } else {
            *(int32_t *)cursor =
                (int32_t)self()->getSymbolReference().getOffset();
        ...
        if (label != NULL) {
          if (TR::Compiler->target.is64Bit()) {
            // This cast is ok because we only need the low 32 bits of the
            // address
            // *(int32_t *)cursor = -(int32_t)(intptrj_t)(cursor+4);
            // if it is X86MEMIMM instruction, offset need consider immedidate
            // length
            *(int32_t *)cursor = -(int32_t)(intptrj_t)(
                cursor + 4 +
                containingInstruction->getOpCode().info().ImmediateSize());
          } else {
  ...
  self()->addMetaDataForCodeAddress(addressTypes, immediateCursor,
                                    containingInstruction->getNode(), cg);
...
static void rematerializeAddressAdds(TR::Node *rootLoadOrStore,
  ...
  // Only try to rematerialize addresses computation that will cause (avoidable)
  // register pressure. Use the existing register if one has been allocated for
  // a previous use of the sum address. Do nothing if the sum address will not
  // be re-used.
  //
  if (subTree->getOpCode().isArrayRef() && subTree->getRegister() == NULL &&
      subTree->getReferenceCount() > 1) {
    ...
    //
    if (baseNode->getOpCode().isIndirect()) return;
    ...
    if (indexNode->getOpCode().isLoadConst()) {
      ...
      if (debug("traceInstructionSelection")) {
        if (TR::Compiler->target.is64Bit())
          diagnostic("\nRematerializing aladd [" POINTER_PRINTF_FORMAT
                     "] with [" POINTER_PRINTF_FORMAT
                     "] at [" POINTER_PRINTF_FORMAT "]",
                     tempNode, subTree, rootLoadOrStore);
        else
          diagnostic("\nRematerializing aiadd [" POINTER_PRINTF_FORMAT
                     "] with [" POINTER_PRINTF_FORMAT
                     "] at [" POINTER_PRINTF_FORMAT "]",
                     tempNode, subTree, rootLoadOrStore);
      }
...
const uint8_t OMR::X86::MemoryReference::_multiplierToStrideMap
    [HIGHEST_STRIDE_MULTIPLIER + 1] = {
        0,  // 0
        0,  // 1
        1,  // 2
        0,  // 3
...
uint32_t OMR::X86::MemoryReference::getNumMRReferencedGPRegisters() {
  return (self()->getBaseRegister() ? 1 : 0) +
         (self()->getIndexRegister() ? 1 : 0);
}
#endif
...
TR::MemoryReference *generateX86MemoryReference(intptrj_t disp,
                                                TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::MemoryReference(disp, cg);
}

TR::MemoryReference *generateX86MemoryReference(TR::Register *br,
                                                intptrj_t disp,
                                                TR::CodeGenerator *cg) {
...
TR::MemoryReference *generateX86MemoryReference(
    TR::Node *node, TR::CodeGenerator *cg, bool canRematerializeAddressAdds) {
  return new (cg->trHeapMemory())
      TR::MemoryReference(node, cg, canRematerializeAddressAdds);
}

TR::MemoryReference *generateX86MemoryReference(TR::MemoryReference &mr,
                                                intptrj_t n,
                                                TR::CodeGenerator *cg) {
...
TR::MemoryReference *generateX86MemoryReference(TR::SymbolReference *sr,
                                                TR::CodeGenerator *cg) {
...
TR::MemoryReference *generateX86MemoryReference(TR::X86DataSnippet *cds,
                                                TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::MemoryReference(cds, cg);
}

TR::MemoryReference *generateX86MemoryReference(TR::LabelSymbol *label,
                                                TR::CodeGenerator *cg) {
