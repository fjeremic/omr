  ...
  , _reloKind(-1)
{
  ...
  if (symRef) {
    ...
    if (rootLoadOrStore->getOpCode().isIndirect()) {
      ...
      if (!isUnresolved && base->getOpCodeValue() == TR::loadaddr && base->getSymbol()->isLocalObject()) {
        ...
        if (isUnresolved) {
          ...
          self()->setUnresolvedDataSnippet(
            TR::UnresolvedDataSnippet::create(cg, rootLoadOrStore, &_symbolReference, isStore, symRef->canCauseGC()));
...
{
  return (self()->hasUnresolvedDataSnippet() || self()->hasUnresolvedVirtualCallSnippet())
    ? NULL
...
OMR::X86::MemoryReference::MemoryReference(
  TR::MemoryReference& mr, intptrj_t n, TR::CodeGenerator* cg, TR_ScratchRegisterManager* srm)
  : _symbolReference(cg->comp()->getSymRefTab())
{
  TR::Compilation* comp = cg->comp();
  ...
  uint32_t addressTypes, uint8_t* cursor, TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  case 2: {
    if (self()->needsCodeAbsoluteExternalRelocation()) {
      ...
      cg->addExternalRelocation(new (cg->trHeapMemory())
                                  TR::ExternalRelocation(cursor, (uint8_t*)TR_ActiveCardTableBase, TR_GlobalValue, cg),
        __FILE__, __LINE__, node);
    }

    if (self()->getSymbolReference().getSymbol() && self()->getSymbolReference().getSymbol()->isClassObject()
      && cg->wantToPatchClassPointer(NULL, cursor)) // might not point to beginning of class
    {
      cg->jitAdd32BitPicToPatchOnClassRedefinition(
        (void*)(uintptr_t) * (int32_t*)cursor, (void*)cursor, self()->getUnresolvedDataSnippet() != NULL);
    }
          ...
          if (symbol->isConst()) {
            ...
            cg->addExternalRelocation(
              new (cg->trHeapMemory()) TR::ExternalRelocation(cursor,
                ...
                node ? (uint8_t*)(intptrj_t)node->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, cg),
              __FILE__, __LINE__, node);
            ...
            if (cg->needClassAndMethodPointerRelocations()) {
              *(int32_t*)cursor = (int32_t)(TR::Compiler->cls.persistentClassPointerFromClassPointer(cg->comp(),
              ...
              if (cg->comp()->getOption(TR_UseSymbolValidationManager)) {
                ...
                cg->addExternalRelocation(
                  new (cg->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)&self()->getSymbolReference(),
                    node ? (uint8_t*)(intptrj_t)node->getInlinedSiteIndex() : (uint8_t*)-1, TR_ClassAddress, cg),
                  __FILE__, __LINE__, node);
              }
            }
            ...
            if (staticSym->isCountForRecompile()) {
              ...
              TR::ExternalRelocation* r
                = new (cg->trHeapMemory()) TR::ExternalRelocation(cursor, 0, TR_AbsoluteMethodAddress, cg);
              ...
              cg->addExternalRelocation(
                new (cg->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)&self()->getSymbolReference(),
                  node ? (uint8_t*)(uintptr_t)node->getInlinedSiteIndex() : (uint8_t*)-1, TR_DataAddress, cg),
                __FILE__, __LINE__, node);
            }
          }
        } else {
  ...
  case 5: {
    ...
    if (!(displacement == 0 && !base->needsDisp() && !base->needsDisp() && !self()->isForceWideDisplacement())
      && !(displacement >= -128 && displacement <= 127 && !self()->isForceWideDisplacement())) {
      if (self()->getSymbolReference().getSymbol() && self()->getSymbolReference().getSymbol()->isClassObject()
  ...
  case 4: {
    ...
    if (symbol) {
      ...
      if (staticSym) {
        ...
        if (self()->getUnresolvedDataSnippet() == NULL) {
          *(int32_t*)cursor
            = (int32_t)(self()->getSymbolReference().getOffset() + (intptrj_t)staticSym->getStaticAddress());
        } else {
          *(int32_t*)cursor = (int32_t)self()->getSymbolReference().getOffset();
        }
      } else if (methodSym) {
        ...
        *(int32_t*)cursor = 0;
        ...
        displacement = self()->getSymbolReference().getOffset() + symbol->getRegisterMappedSymbol()->getOffset();
        TR_ASSERT(IS_32BIT_SIGNED(displacement),
          "64-bit displacement should have been replaced in TR_AMD64MemoryReference::generateBinaryEncoding");
      ...
      TR::X86DataSnippet* cds = self()->getDataSnippet();
      TR_ASSERT(
        cds == NULL || self()->getLabel() == NULL, "a memRef cannot have both a constant data snippet and a label");
      TR::LabelSymbol* label = NULL;
      if (cds)
        label = cds->getSnippetLabel();
      else
      ...
      if (label != NULL) {
        if (TR::Compiler->target.is64Bit()) {
          ...
          *(int32_t*)cursor
            = -(int32_t)(intptrj_t)(cursor + 4 + containingInstruction->getOpCode().info().ImmediateSize());
        } else {
...
TR::MemoryReference* generateX86MemoryReference(intptrj_t disp, TR::CodeGenerator* cg)
{
...
TR::MemoryReference* generateX86MemoryReference(TR::MemoryReference& mr, intptrj_t n, TR::CodeGenerator* cg)
{
  return new (cg->trHeapMemory()) TR::MemoryReference(mr, n, cg);
}

TR::MemoryReference* generateX86MemoryReference(
  TR::MemoryReference& mr, intptrj_t n, TR_ScratchRegisterManager* srm, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit())
...
TR::MemoryReference* generateX86MemoryReference(TR::SymbolReference* sr, intptrj_t displacement, TR::CodeGenerator* cg)
{
