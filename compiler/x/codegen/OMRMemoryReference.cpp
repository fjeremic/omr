...
OMR::X86::MemoryReference::MemoryReference(TR::X86DataSnippet *cds,
    ...
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _dataSnippet(cds), _label(NULL),
      _symbolReference(cg->comp()->getSymRefTab()), _stride(0), _flags(0),
      _reloKind(-1) {
...
OMR::X86::MemoryReference::MemoryReference(TR::LabelSymbol *label,
                                           TR::CodeGenerator *cg)
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _dataSnippet(NULL), _label(label),
      _symbolReference(cg->comp()->getSymRefTab()), _stride(0), _flags(0),
      _reloKind(-1) {
  self()->setForceWideDisplacement();
}
...
OMR::X86::MemoryReference::MemoryReference(TR::SymbolReference *symRef,
                                           TR::CodeGenerator *cg)
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _dataSnippet(NULL), _label(NULL),
      _symbolReference(cg->comp()->getSymRefTab()), _stride(0), _flags(0),
      _reloKind(-1) {
...
OMR::X86::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
  ...
  if (symRef) {
    ...
    diagnostic("OMR::X86::MemoryReference::OMR::X86::MemoryReference() ==> no "
               "symbol reference");
  }
  ...
  if ((evalSubTree && subTree->getReferenceCount() > 1) ||
    ...
    if (subTree->getOpCode().isArrayRef() ||
      ...
      if (firstChild->getOpCodeValue() == TR::i2l &&
        ...
        if (debug("traceMemOp"))
          diagnostic("\nMEMREF ***sourceReg = %p, upperBitsZero = %d, "
                     "isNonNegative = %d, skipSignExtension = %d \n",
...
void OMR::X86::MemoryReference::addMetaDataForCodeAddress(
    ...
    if (symbol) {
      ...
      if (staticSym) {
        if (self()->getUnresolvedDataSnippet() == NULL) {
          if (symbol->isConst()) {
            ...
            if (staticSym->isCountForRecompile()) {
              ...
              TR::ExternalRelocation *r = new (cg->trHeapMemory())
                  TR::ExternalRelocation(cursor, 0, TR_AbsoluteMethodAddress,
                                         cg);
          ...
          {
            cg->jitAdd32BitPicToPatchOnClassRedefinition((void *)-1,
                                                         (void *)cursor, true);
          }
        }
      }
  ...
  case 5: {
    ...
    if (!(displacement == 0 && !base->needsDisp() && !base->needsDisp() &&
      ...
      if (self()->getSymbolReference().getSymbol() &&
          self()->getSymbolReference().getSymbol()->isClassObject() &&
          cg->wantToPatchClassPointer(NULL,
                                      cursor)) // possibly unresolved, may not
                                               // point to beginning of class
      {
        cg->jitAdd32BitPicToPatchOnClassRedefinition(
            (void *)(uintptr_t) * (int32_t *)cursor, (void *)cursor,
            self()->getUnresolvedDataSnippet() != NULL);
...
TR::MemoryReference *
generateX86MemoryReference(TR::Node *node, TR::CodeGenerator *cg,
                           bool canRematerializeAddressAdds) {
