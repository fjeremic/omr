...
class TR_X86OpCode {
  ...
  struct OpCode_t {
    ...
    inline typename TBuffer::cursor_t encode(typename TBuffer::cursor_t cursor, uint8_t rexbits) const;
  ...
  // TBuffer should only be one of the two: Estimator when calculating length, and Writer when generating binaries.
  template <class TBuffer>
  inline typename TBuffer::cursor_t encode(typename TBuffer::cursor_t cursor, uint8_t rex) const
  {
...
public:
  inline TR_X86OpCode()
    : _opCode(BADIA32Op)
  {}
  inline TR_X86OpCode(TR_X86OpCodes op)
    : _opCode(op)
  {}

  inline const OpCode_t& info() const
  {
    return _binaries[_opCode];
  }
  inline TR_X86OpCodes getOpCodeValue() const
  {
    return _opCode;
  }
  ...
  {
    return (_properties[_opCode] & (IA32OpProp_DoubleFP | IA32OpProp_SingleFP)) == 0;
  }
  ...
  inline uint32_t isCallOp() const
  {
    return isCallOp(_opCode);
  }
  inline uint32_t isCallImmOp() const
  {
    return isCallImmOp(_opCode);
  }
  ...
  inline uint8_t getTestedEFlags() const
  {
    return getTestedEFlags(_opCode);
  }
