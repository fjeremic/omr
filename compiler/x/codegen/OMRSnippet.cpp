...
OMR::X86::Snippet::Snippet(TR::CodeGenerator *cg, TR::Node *node,
...
int32_t TR_Debug::printRestartJump(TR::FILE *pOutFile,
                                   TR::X86RestartSnippet *snippet,
                                   uint8_t *bufferPos) {
  int32_t size = snippet->estimateRestartJumpLength(
      JMP4, bufferPos - (uint8_t *)snippet->cg()->getBinaryBufferStart());
...
int32_t TR_Debug::printRestartJump(TR::FILE *pOutFile,
                                   TR::X86RestartSnippet *snippet,
                                   uint8_t *bufferPos, int32_t branchOp,
                                   const char *branchOpName) {
  int32_t size = snippet->estimateRestartJumpLength(
      (TR_X86OpCodes)branchOp,
      bufferPos - (uint8_t *)snippet->cg()->getBinaryBufferStart());
  printPrefix(pOutFile, NULL, bufferPos, size);
