...
public:
  ...
  MemoryReference(TR::Register *br, TR::SymbolReference *sr, TR::Register *ir,
                  uint8_t s, TR::CodeGenerator *cg)
      : _baseRegister(br), _baseNode(NULL), _indexRegister(ir),
        _indexNode(NULL), _dataSnippet(NULL), _label(NULL),
        _symbolReference(cg->comp()->getSymRefTab()), _stride(s), _flags(0),
        _reloKind(-1) {
    _symbolReference.setSymbol(sr->getSymbol());
    _symbolReference.setOffset(sr->getOffset());
    // TODO: Figure out what to do with the aliasing info here. Should
    // replicate,
    //       but may be able to get away without
  }

  MemoryReference(TR::Register *br, TR::Register *ir, uint8_t s,
                  TR::CodeGenerator *cg)
      : _baseRegister(br), _baseNode(NULL), _indexRegister(ir),
        _indexNode(NULL), _dataSnippet(NULL), _label(NULL),
        _symbolReference(cg->comp()->getSymRefTab()), _stride(s), _flags(0),
  ...
  MemoryReference(intptrj_t disp, TR::CodeGenerator *cg)
      : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
        _indexNode(NULL), _dataSnippet(NULL), _label(NULL),
        _symbolReference(cg->comp()->getSymRefTab()), _stride(0), _flags(0),
        _reloKind(-1) {
  ...
  MemoryReference(TR::Register *br, TR::Register *ir, uint8_t s, intptrj_t disp,
                  TR::CodeGenerator *cg)
      : _baseRegister(br), _baseNode(NULL), _indexRegister(ir),
        _indexNode(NULL), _dataSnippet(NULL), _label(NULL),
        _symbolReference(cg->comp()->getSymRefTab()), _stride(s), _flags(0),
  ...
  MemoryReference(TR::SymbolReference *symRef, intptrj_t displacement,
                  TR::CodeGenerator *cg);

  MemoryReference(TR::MemoryReference &mr, intptrj_t n, TR::CodeGenerator *cg,
                  TR_ScratchRegisterManager *srm = NULL);

  TR::Register *getBaseRegister() { return _baseRegister; }
  TR::Register *setBaseRegister(TR::Register *br) {
  ...
  // An unresolved data snippet, unresolved virtual call snippet, and constant
  // data snippet are mutually exclusive for a given memory reference.  Hence,
  // they share the same pointer.
  //
  TR::UnresolvedDataSnippet *getUnresolvedDataSnippet();

  TR::UnresolvedDataSnippet *
  setUnresolvedDataSnippet(TR::UnresolvedDataSnippet *s);
  ...
  bool isForceWideDisplacement() {
    return _flags.testAny(MemRef_ForceWideDisplacement);
  }
  ...
  bool hasUnresolvedDataSnippet() {
    return _flags.testAny(MemRef_UnresolvedDataSnippet);
  }
  void setHasUnresolvedDataSnippet() {
    _flags.set(MemRef_UnresolvedDataSnippet);
  }

  bool hasUnresolvedVirtualCallSnippet() {
    return _flags.testAny(MemRef_UnresolvedVirtualCallSnippet);
  }
  void setHasUnresolvedVirtualCallSnippet() {
    _flags.set(MemRef_UnresolvedVirtualCallSnippet);
  }

  bool inUpcastingMode() { return _flags.testAny(MemRef_UpcastingMode); }
  void setInUpcastingMode(bool b = true) {
    _flags.set(MemRef_UpcastingMode, b);
  }

  // relevant only for 32 bit
  //
  bool processAsLongVolatileLow() {
    return _flags.testAny(MemRef_ProcessAsLongVolatileLow);
  }
  void setProcessAsLongVolatileLow() {
    _flags.set(MemRef_ProcessAsLongVolatileLow);
  }

  // relevant only for 32 bit
  //
  bool processAsLongVolatileHigh() {
    return _flags.testAny(MemRef_ProcessAsLongVolatileHigh);
  }
  void setProcessAsLongVolatileHigh() {
    _flags.set(MemRef_ProcessAsLongVolatileHigh);
  }

  // relevant only for 64 bit
  //
  bool processAsFPVolatile() {
    return _flags.testAny(MemRef_ProcessAsFPVolatile);
  }
  void setProcessAsFPVolatile() { _flags.set(MemRef_ProcessAsFPVolatile); }

  bool requiresLockPrefix() {
    return _flags.testAny(MemRef_RequiresLockPrefix);
  }
  void setRequiresLockPrefix() { return _flags.set(MemRef_RequiresLockPrefix); }

  bool needsCodeAbsoluteExternalRelocation() {
    return _flags.testAny(MemRef_NeedExternalCodeAbsoluteRelocation);
  }
  void setNeedsCodeAbsoluteExternalRelocation() {
    _flags.set(MemRef_NeedExternalCodeAbsoluteRelocation);
  }
  ...
  // Call this method iteratively to get the registers that this memory
  // reference uses. Use an argument of NULL on the first call, and after that
  ...
  int32_t isValidStrideMultiplier(int32_t constFactor) {
    return ((uint32_t)(constFactor - 1) < HIGHEST_STRIDE_MULTIPLIER)
               ? isNonNegativePowerOf2(constFactor)
               : 0;
  }
  ...
  virtual OMR::X86::EnlargementResult enlarge(TR::CodeGenerator *cg,
  ...
  bool needsSIBByte() {
    return _indexRegister ? true : false;
  } // more needed here for ESP base, etc

  void checkAndDecReferenceCount(TR::Node *node, rcount_t refCount,
                                 TR::CodeGenerator *cg);

  void populateMemoryReference(TR::Node *subTree, TR::CodeGenerator *cg,
                               TR::Node *parent = NULL);

  TR::Register *evaluate(TR::Node *subTree, TR::CodeGenerator *cg,
                         TR::Node *parent = NULL);

  void consolidateRegisters(TR::Node *, TR::CodeGenerator *cg);

  virtual void assignRegisters(TR::Instruction *currentInstruction,
                               TR::CodeGenerator *cg);

  virtual uint8_t *
  generateBinaryEncoding(uint8_t *modRM, TR::Instruction *containingInstruction,
                         TR::CodeGenerator *cg);

  void addMetaDataForCodeAddress(uint32_t addressTypes, uint8_t *cursor,
  ...
  static int32_t convertMultiplierToStride(int32_t multiplier) {
  ...
  uint8_t rexBits() {
    ...
    if (_baseRegister) {
      ...
      if (toRealRegister(_baseRegister)->getRegisterNumber() ==
          TR::RealRegister::vfp)
        baseRegister = toRealRegister(_baseRegister)->getAssignedRealRegister();
      else
        baseRegister = _baseRegister;
      rxbBits |=
          toRealRegister(baseRegister)->rexBits(TR::RealRegister::REX_B, false);
    }
    if (_indexRegister)
      rxbBits |= toRealRegister(_indexRegister)
                     ->rexBits(TR::RealRegister::REX_X, false);
...
TR::MemoryReference *generateX86MemoryReference(intptrj_t,
                                                TR::CodeGenerator *cg);
...
TR::MemoryReference *generateX86MemoryReference(TR::Register *br,
                                                ...
                                                TR::CodeGenerator *cg);
TR::MemoryReference *
generateX86MemoryReference(TR::Node *, TR::CodeGenerator *cg,
                           bool canRematerializeAddressAdds = true);
TR::MemoryReference *generateX86MemoryReference(TR::MemoryReference &,
                                                intptrj_t,
                                                TR::CodeGenerator *cg);
TR::MemoryReference *generateX86MemoryReference(TR::MemoryReference &,
                                                intptrj_t,
                                                TR_ScratchRegisterManager *,
                                                TR::CodeGenerator *cg);
TR::MemoryReference *generateX86MemoryReference(TR::SymbolReference *,
                                                TR::CodeGenerator *cg);
TR::MemoryReference *generateX86MemoryReference(TR::SymbolReference *,
                                                intptrj_t,
                                                TR::CodeGenerator *cg);
TR::MemoryReference *generateX86MemoryReference(TR::X86DataSnippet *,
                                                TR::CodeGenerator *cg);
TR::MemoryReference *generateX86MemoryReference(TR::LabelSymbol *,
                                                TR::CodeGenerator *cg);
