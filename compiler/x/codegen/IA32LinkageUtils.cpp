    ...
    TR::CodeGenerator* cg)
{
    ...
    if (child->getRegister() == NULL) {
        ...
        } else if (child->getOpCodeValue() == TR::fbits2i && !child->normalizeNanValues() && child->getReferenceCount() == 1) {
            pushRegister = pushFloatArg(child->getFirstChild(), cg);
            cg->decReferenceCount(child);
            return pushRegister;
        } else if (child->getOpCode().isMemoryReference() && (child->getReferenceCount() == 1) && (child->getSymbolReference() != cg->comp()->getSymRefTab()->findVftSymbolRef())) {
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (child->getRegister() == NULL) {
        if (child->getOpCode().isLoadConst()) {
            ...
            cg->decReferenceCount(child);
            return NULL;
        } else if (child->getOpCodeValue() == TR::dbits2l && !child->normalizeNanValues() && child->getReferenceCount() == 1) {
