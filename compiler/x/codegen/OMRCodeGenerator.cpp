...
#define CANT_REMATERIALIZE_ADDRESSES                                           \
  (TR::Compiler->target                                                        \
     .is64Bit()) // AMD64 produces a memref with an unassigned addressRegister

TR_X86ProcessorInfo OMR::X86::CodeGenerator::_targetProcessorInfo;

void
TR_X86ProcessorInfo::initialize()
{
...
OMR::X86::CodeGenerator::CodeGenerator()
  ...
  , _dependentDiscardableRegisters(
      getTypedAllocator<TR::Register*>(TR::comp()->allocator()))
  , _clobberingInstructions(
      getTypedAllocator<TR::ClobberingInstruction*>(TR::comp()->allocator()))
  , _outlinedInstructionsList(
      getTypedAllocator<TR_OutlinedInstructions*>(TR::comp()->allocator()))
...
OMR::X86::CodeGenerator::beginInstructionSelection()
{
  ...
  if (self()->enableSinglePrecisionMethods() &&
    ...
    generateMemInstruction(LDCWMem,
                           startNode,
...
OMR::X86::CodeGenerator::endInstructionSelection()
{
  ...
  if (self()->enableSinglePrecisionMethods() &&
    ...
    generateMemInstruction(
      self()->getLastCatchAppendInstruction(),
      LDCWMem,
      generateX86MemoryReference(
        self()->findOrCreate2ByteConstant(
 ...
 */
static bool
willNotInlineCompareAndSwapNative(TR::Node* node,
                                  int8_t size,
                                  TR::Compilation* comp)
{
#ifdef J9_PROJECT_SPECIFIC
 ...
 */
bool
OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(TR::Node* node,
                                                        TR::Compilation* comp)
{
  TR::SymbolReference* callSymRef = node->getSymbolReference();
  TR::MethodSymbol* methodSymbol =
...
void
OMR::X86::CodeGenerator::addLiveDiscardableRegister(TR::Register* reg)
{
...
void
OMR::X86::CodeGenerator::removeLiveDiscardableRegister(TR::Register* reg)
{
...
bool
OMR::X86::CodeGenerator::canNullChkBeImplicit(TR::Node* node)
{
  return self()->canNullChkBeImplicit(node, true);
}

void
OMR::X86::CodeGenerator::clobberLiveDiscardableRegisters(
...
//
void
OMR::X86::CodeGenerator::clobberLiveDependentDiscardableRegisters(
...
//
void
OMR::X86::CodeGenerator::reactivateDependentDiscardableRegisters(
  TR::Register* baseReg)
{
  TR_Stack<TR::Register*> worklist(self()->trMemory());
  worklist.push(baseReg);
...
//
void
OMR::X86::CodeGenerator::deactivateDependentDiscardableRegisters(
  TR::Register* baseReg)
{
  TR_Stack<TR::Register*> worklist(self()->trMemory());
  worklist.push(baseReg);
...
bool
OMR::X86::CodeGenerator::supportsNonHelper(
...
OMR::X86::CodeGenerator::getNanoTimeTemp()
{
  if (_nanoTimeTemp == NULL) {
    ...
    sym = TR::AutomaticSymbol::create(
      self()->trHeapMemory(), TR::Aggregate, sizeof(struct timeval));
#else
...
void
OMR::X86::CodeGenerator::saveBetterSpillPlacements(
  TR::Instruction* branchInstruction)
{
  ...
  for (auto regElement = _spilledIntRegisters.begin();
    ...
    self()->traceRegisterAssignment(
      "Saved better spill placement for %R, mask = %x.",
      *regElement,
      freeRealRegisters);
...
void
OMR::X86::CodeGenerator::removeBetterSpillPlacementCandidate(
  TR::RealRegister* realReg)
{
  ...
  TR_RegisterKinds kindsToBeAssigned)
{
  ...
  TR_ASSERT(
    oi,
    "Could not find OutlinedInstructions from branch label on instr=%p\n",
    branchInstruction);
  ...
  if (deps) {
    TR::Instruction* ins = generateLabelInstruction(oi->getFirstInstruction(),
                                                    LABEL,
...
void
OMR::X86::CodeGenerator::prepareForNonLinearRegisterAssignmentAtMerge(
  TR::X86LabelInstruction* mergeInstruction)
{
  ...
  TR_ASSERT(
    oi,
    "Could not find OutlinedInstructions from merge label on instr=%p\n",
    mergeInstruction);
...
void
OMR::X86::CodeGenerator::processClobberingInstructions(
...
void
OMR::X86::CodeGenerator::doBackwardsRegisterAssignment(
  ...
  TR::Instruction* appendInstruction)
{
  ...
  while (instructionCursor && instructionCursor != appendInstruction) {
    ...
    if (((TR::X86LabelInstruction*)instructionCursor)->getOpCodeValue() ==
          LABEL &&
        (label =
           ((TR::X86LabelInstruction*)instructionCursor)->getLabelSymbol())) {
...
void
OMR::X86::CodeGenerator::doRegisterAssignment(TR_RegisterKinds kindsToAssign)
{
  TR::Instruction* instructionCursor;
  ...
  //
  kindsToAssign =
    TR_RegisterKinds(kindsToAssign & (TR_GPR_Mask | TR_FPR_Mask | TR_VRF_Mask));
  if (kindsToAssign) {
...
bool
OMR::X86::CodeGenerator::isReturnInstruction(TR::Instruction* instr)
{
...
bool
OMR::X86::CodeGenerator::isBranchInstruction(TR::Instruction* instr)
{
...
OMR::X86::CodeGenerator::doBinaryEncoding()
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (TR::Compiler->target.isSMP()) {
      ...
      if (recompilation && !recompilation->useSampling()) {
        ...
        static const TR_AtomicRegion countingAtomicRegions[] = { { 0, 5 },
                                                                 { 0, 0 } };
...
#endif
      {
        ...
        static const TR_AtomicRegion samplingAtomicRegions[] = { { 0, 2 },
                                                                 { 0, 0 } };
  ...
  //
  std::sort(_dataSnippetList.begin(),
            _dataSnippetList.end(),
  ...
  for (TR::Instruction* gcMapCursor = prologueCursor;
    ...
    if (gcMapCursor->needsGCMap())
      gcMapCursor->setGCMap(
        self()->getStackAtlas()->getParameterMap()->clone(self()->trMemory()));
  }
  ...
  while (estimateCursor) {
    ...
    if (estimateCursor == _vfpResetInstruction)
      self()->generateDebugCounter(estimateCursor,
                                   "cg.prologues:#instructionBytes",
                                   estimate - estimatedPrologueStartOffset,
                                            ...
                                            TR_X86OpCodes movRegRegOpCode)
{
  ...
  if (canClobber) {
    ...
    generateRegRegInstruction(
      movRegRegOpCode, node, targetRegister, sourceRegister, self());
    //
...
TR::Register*
OMR::X86::CodeGenerator::intClobberEvaluate(TR::Node* node)
{
  TR_ASSERT(!node->getOpCode().is8Byte() || node->getOpCode().isRef(),
            "Non-ref 8bytes must use longClobberEvaluate");
  TR_ASSERT(
    !node->getOpCode().isRef() || TR::Compiler->target.is32Bit() ||
...
TR::Register*
OMR::X86::CodeGenerator::shortClobberEvaluate(TR::Node* node)
{
...
TR::Register*
OMR::X86::CodeGenerator::floatClobberEvaluate(TR::Node* node)
{
  ...
  if (node->getReferenceCount() > 1) {
    ...
    if (temp->getKind() == TR_FPR)
      generateRegRegInstruction(
        MOVAPSRegReg, node, targetRegister, temp, self());
    else
      generateFPST0STiRegRegInstruction(
        FLDRegReg, node, targetRegister, temp, self());
...
TR::Register*
OMR::X86::CodeGenerator::doubleClobberEvaluate(TR::Node* node)
{
  ...
  if (node->getReferenceCount() > 1) {
    ...
    if (temp->getKind() == TR_FPR)
      generateRegRegInstruction(
        MOVAPDRegReg, node, targetRegister, temp, self());
    else
      generateFPST0STiRegRegInstruction(
        DLDRegReg, node, targetRegister, temp, self());
...
OMR::X86::CodeGenerator::findOrCreateConstantDataSnippet(TR::Node* n,
                                                         void* c,
                                                         size_t s)
{
...
int32_t
OMR::X86::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
...
TR::X86DataSnippet*
OMR::X86::CodeGenerator::create2ByteData(TR::Node* n, int16_t c)
{
  return self()->createDataSnippet(n, &c, 2);
}

TR::X86DataSnippet*
OMR::X86::CodeGenerator::create4ByteData(TR::Node* n, int32_t c)
{
  return self()->createDataSnippet(n, &c, 4);
}

TR::X86DataSnippet*
OMR::X86::CodeGenerator::create8ByteData(TR::Node* n, int64_t c)
{
  return self()->createDataSnippet(n, &c, 8);
}

TR::X86DataSnippet*
OMR::X86::CodeGenerator::create16ByteData(TR::Node* n, void* c)
{
  return self()->createDataSnippet(n, c, 16);
}

static uint32_t
registerBitMask(int32_t reg)
{
  return 1 << (reg - 1); // TODO:AMD64: Use the proper mask value
}

void
OMR::X86::CodeGenerator::buildRegisterMapForInstruction(TR_GCStackMap* map)
{
...
bool
OMR::X86::CodeGenerator::allowGlobalRegisterAcrossBranch(
...
void
OMR::X86::CodeGenerator::apply32BitLoadLabelRelativeRelocation(
  TR::Instruction* movInstruction,
  ...
  int32_t deltaToStartLabel)
{
  ...
  TR_ASSERT(
    movInstructionX86->getOpCodeValue() == MOV4RegImm4,
...
void
OMR::X86::CodeGenerator::apply32BitLabelRelativeRelocation(
  int32_t* cursor,
  TR::LabelSymbol* label)
{
...
//
int32_t
OMR::X86::CodeGenerator::branchDisplacementToHelperOrTrampoline(
...
// for it.
inline bool
getNodeIs64Bit(TR::Node* node, TR::CodeGenerator* cg)
{
...
// for it.
inline intptrj_t
integerConstNodeValue(TR::Node* node, TR::CodeGenerator* cg)
{
...
bool
OMR::X86::CodeGenerator::nodeIsFoldableMemOperand(
  TR::Node* node,
  TR::Node* parent,
...
inline bool
lookForMemrefInChild(TR::Node* node)
{
...
void
OMR::X86::CodeGenerator::simulateNodeEvaluation(
  TR::Node* node,
  TR_RegisterPressureState* state,
  TR_RegisterPressureSummary* summary)
{
  ...
  if (toFold != -1) // toFold child can be folded into a memory operand
  {
    ...
    // Fold memref child
    self()->simulateMemoryReference(
      &memref, node->getChild(toFold), state, summary);
...
uint8_t OMR::X86::CodeGenerator::old32BitPaddingEncoding
  [PADDING_TABLE_MAX_ENCODING_LENGTH][PADDING_TABLE_MAX_ENCODING_LENGTH] = {
    ...
    { 0x8d, 0x84, 0x20, 0x00, 0x00, 0x00, 0x00 }, // 7: lea exx, [exx+00000000]
                                                  // (with SIB)
  };
...
TR_X86PaddingTable OMR::X86::CodeGenerator::_old32BitPaddingTable =
  TR_X86PaddingTable(
    7,
    (uint8_t***)&OMR::X86::CodeGenerator::old32BitPaddingEncoding,
    TR_X86PaddingTable::registerMatters,
    0x8b,
    0x20);
...
paddingTableEncoding(TR_X86PaddingTable* paddingTable, uint8_t length)
{
  ...
  TR_ASSERT(length <= paddingTable->_biggestEncoding,
            "Padding length %d cannot exceed %d",
            length,
            paddingTable->_biggestEncoding);
...
uint8_t*
OMR::X86::CodeGenerator::generatePadding(uint8_t* cursor,
                                         intptrj_t length,
          ...
          for (TR::Instruction* ninst = neighborhood->getNext(); ninst;
            ...
            if (self()->comp()->isPICSite(ninst)) {
              TR::DebugCounter::incStaticDebugCounter(
                ...
                TR::DebugCounter::debugCounterName(
                  self()->comp(),
                  "vgnopNoPatchReason/%d/staticPIC",
                  blockFrequency));
              break;
            }
            if (ninst->isPatchBarrier()) {
              if (ninst->getOpCodeValue() != LABEL)
                TR::DebugCounter::incStaticDebugCounter(
                  ...
                  TR::DebugCounter::debugCounterName(
                    self()->comp(),
                    "vgnopNoPatchReason/%d/patchBarrier",
                    blockFrequency));
              else
                TR::DebugCounter::incStaticDebugCounter(
                  self()->comp(),
...
intptrj_t
OMR::X86::CodeGenerator::alignment(void* cursor,
                                   intptrj_t boundary,
                                   intptrj_t margin)
{
...
bool
OMR::X86::CodeGenerator::patchableRangeNeedsAlignment(void* cursor,
                                                      intptrj_t length,
                                                      intptrj_t boundary,
                                                      intptrj_t margin)
{
...
TR::Instruction*
OMR::X86::CodeGenerator::generateDebugCounterBump(
...
void
OMR::X86::CodeGenerator::removeUnavailableRegisters(
  TR_RegisterCandidate* rc,
  TR::Block** blocks,
...
void
OMR::X86::CodeGenerator::dumpDataSnippets(TR::FILE* outFile)
{
  if (outFile == NULL)
    return;
...
//
void
OMR::X86::CodeGenerator::dumpPreFPRegisterAssignment(
  TR::Instruction* instructionCursor)
{
  ...
  if (instructionCursor->totalReferencedFPRegisters(self()) > 0) {
    trfprintf(self()->comp()->getOutFile(),
              "\n<< Pre-FPR assignment for instruction: %p",
              instructionCursor);
...
//
void
OMR::X86::CodeGenerator::dumpPostFPRegisterAssignment(
  TR::Instruction* instructionCursor,
  TR::Instruction* origPrevInstruction)
{
  if (self()->comp()->getOutFile() == NULL)
...
void
OMR::X86::CodeGenerator::dumpPreGPRegisterAssignment(
  TR::Instruction* instructionCursor)
{
  if (self()->comp()->getOutFile() == NULL)
    return;
...
//
void
OMR::X86::CodeGenerator::dumpPostGPRegisterAssignment(
  TR::Instruction* instructionCursor,
  TR::Instruction* origNextInstruction)
{
  if (self()->comp()->getOutFile() == NULL)
...
int32_t
OMR::X86::CodeGenerator::arrayTranslateMinimumNumberOfElements(
  bool isByteSource,
  bool isByteTarget)
{
...
bool
OMR::X86::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress,
