...
#define CANT_REMATERIALIZE_ADDRESSES                                           \
  (TR::Compiler->target.is64Bit()) // AMD64 produces a memref with an unassigned
                                   // addressRegister
...
void TR_X86ProcessorInfo::initialize() {
  ...
  if (isGenuineIntel()) {
    ...
    case 0x05:
      ...
      break;
    case 0x06: {
      uint32_t extended_model = getCPUModel(_processorSignature) +
...
OMR::X86::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(), _nanoTimeTemp(NULL), _assignmentDirection(Backward),
      _lastCatchAppendInstruction(NULL), _betterSpillPlacements(NULL),
      _dataSnippetList(
      ...
      _outlinedInstructionsList(getTypedAllocator<TR_OutlinedInstructions *>(
          TR::comp()->allocator())),
      _numReservedIPICTrampolines(0), _flags(0) {
...
bool OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(
  ...
  case TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z:
    return willNotInlineCompareAndSwapNative(
        node,
        (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? 8
                                                                           : 4,
        comp);
#endif
  default:
    ...
    while (iterator != self()->getLiveDiscardableRegisters().end()) {
      ...
      if (registerCursor->getRematerializationInfo()
        ...
        if ((rmSymRef->getSymbol() == symbol) ||
          ...
          if (debug("dumpRemat")) {
            diagnostic("---> Clobbering %s discardable register %s at "
                       "instruction %p in %s\n",
        ...
        //
          ...
          if ((IGNodeRematSym != NULL) &&
            ...
            if (debug("dumpRemat")) {
              diagnostic("---> Clobbering %s discardable register %s at "
                         "instruction %p because of shared slot in %s\n",
                         self()->getDebug()->toString(
                             registerCursor->getRematerializationInfo()),
                         self()->getDebug()->getName(registerCursor), instr,
                         self()->comp()->signature());
      ...
      if (info->isIndirect() && info->getBaseRegister() == baseReg) {
        ...
        if (debug("dumpRemat")) {
          diagnostic("---> Clobbering %s discardable register %s at "
                     ...
                     self()->getDebug()->getName(candidate),
...
bool OMR::X86::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode,
  ...
  /*
   * GRA does not work with vector registers on 32 bit due to a bug where xmm
   * registers are not being assigned. This can potentially cause a performance
   * problem in autosimd reductions. This function is where AutoSIMD checks to
   * see if getvelem is suppored for use in reductions. The getvelem case was
   * changed to disable the use of getvelem on 32 bit x86. This code will be
   * reenabled as part of Issue 2035 which tracks the progress of fixing the GRA
   * bug. GRA does not work with vector registers on 64 bit either. getvelem is
   * now being disabled on 64 bit for the same reasons as 32 bit. This code will
   * be reenabled as part of Issue 2280
   */
  case TR::getvelem:
#if 0
...
TR::Instruction *
OMR::X86::CodeGenerator::findBetterSpillPlacement(TR::Register *virtReg,
                                                  int32_t realRegNum) {
  ...
  while (clobInstructionCursor &&
    ...
    while (regIterator !=
      ...
      if (!info->isIndirect() ||
        ...
        if (debug("dumpRemat")) {
          diagnostic("---> Activating %s discardable register %s at "
                     "instruction %p in %s\n",
                     self()->getDebug()->toString(info),
                     self()->getDebug()->getName(*regIterator),
...
#define OVER_ESTIMATION 4
  ...
  if (self()->comp()->getOption(TR_EnableGCRPatching) &&
    ...
    void *addrToPatch = self()
                            ...
                            ->getStaticAddress();
    if (!addrToPatch) {
      TR_ASSERT(false, "Must have updated gcrPatchPointSymbol with the correct "
                       "address by now\n");
...
TR::Register *
OMR::X86::CodeGenerator::gprClobberEvaluate(TR::Node *node,
                                            TR_X86OpCodes movRegRegOpCode) {
...
TR::X86DataSnippet *
OMR::X86::CodeGenerator::createDataSnippet(TR::Node *n, void *c, size_t s) {
  auto snippet =
...
TR::X86ConstantDataSnippet *
OMR::X86::CodeGenerator::findOrCreate2ByteConstant(TR::Node *n, int16_t c) {
  return self()->findOrCreateConstantDataSnippet(n, &c, 2);
}

TR::X86ConstantDataSnippet *
OMR::X86::CodeGenerator::findOrCreate4ByteConstant(TR::Node *n, int32_t c) {
  return self()->findOrCreateConstantDataSnippet(n, &c, 4);
}

TR::X86ConstantDataSnippet *
OMR::X86::CodeGenerator::findOrCreate8ByteConstant(TR::Node *n, int64_t c) {
  return self()->findOrCreateConstantDataSnippet(n, &c, 8);
}

TR::X86ConstantDataSnippet *
OMR::X86::CodeGenerator::findOrCreate16ByteConstant(TR::Node *n, void *c) {
...
OMR::X86::CodeGenerator::pickNOPRegister(TR::Instruction *successor) {
  int32_t j = 1;
...
uint8_t
OMR::X86::CodeGenerator::nodeResultGPRCount(TR::Node *node,
...
uint8_t *OMR::X86::CodeGenerator::generatePadding(
  ...
  const uint8_t sibMask =
      0xb8; // 10111000, where 2^n is set if the NOP with size n uses a SIB byte
  if (length <= _paddingTable->_biggestEncoding) {
    ...
    const intptrj_t jmpThreshold = 100; // Beyond this length, a jmp is faster
                                        // than a sequence of no-op instructions

    if ((properties & TR_AtomicNoOpPadding || length >= jmpThreshold)) {
      TR_ASSERT(0, "Oops -- we should never get here because using JMPs for "
                   "no-ops is very slow");
      if (length >= 5) {
        length -= 5;
