...
void OMR::X86::CodeGenerator::initialize(TR::Compilation* comp)
{
    ...
    // geometry.
    //
    if (((!comp->getOption(TR_DisableTLHPrefetch) && (comp->cg()->getX86ProcessorInfo().isIntelCore2() || comp->cg()->getX86ProcessorInfo().isIntelNehalem())) || (comp->getOption(TR_TLHPrefetch) && self()->targetSupportsSoftwarePrefetches()))) {
...
{
    return TR::CodeGenerator::getX86ProcessorInfo().supportsSSE4_1() && !self()->comp()->getOption(TR_DisableSIMDUTF16LEEncoder);
}

bool OMR::X86::CodeGenerator::getSupportsEncodeUtf16BigWithSurrogateTest()
{
    return TR::CodeGenerator::getX86ProcessorInfo().supportsSSE4_1() && !self()->comp()->getOption(TR_DisableSIMDUTF16BEEncoder);
}
...
{
    return self()->getSupportsVirtualGuardNOPing() && self()->comp()->performVirtualGuardNOPing() && self()->allowGuardMerging();
}
    ...
    TR_RegisterKinds kindsToBeAssigned)
{
    ...
    if (deps) {
        TR::Instruction* ins = generateLabelInstruction(oi->getFirstInstruction(), LABEL, generateLabelSymbol(self()), deps, self());
...
void OMR::X86::CodeGenerator::processClobberingInstructions(TR::ClobberingInstruction* clobInstructionCursor, TR::Instruction* instructionCursor)
{
    ...
    //
    while (clobInstructionCursor && (clobInstructionCursor->getInstruction() == instructionCursor) && self()->enableRematerialisation()) {
...
#define OVER_ESTIMATION 4
    ...
    if (self()->comp()->getOption(TR_EnableGCRPatching) && self()->comp()->getRecompilationInfo() && self()->comp()->getRecompilationInfo()->getJittedBodyInfo()->getUsesGCR()) {
...
bool OMR::X86::CodeGenerator::nodeIsFoldableMemOperand(TR::Node* node, TR::Node* parent, TR_RegisterPressureState* state)
{
    ...
    bool result = (node->getOpCode().isLoadVar() || node->getOpCode().isArrayLength()) && !self()->isCandidateLoad(node, state) && !nodeState.hasRegister();
...
void OMR::X86::CodeGenerator::simulateNodeEvaluation(TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if ((opCode.isMul() || opCode.isDiv() || opCode.isRem()) && !(opCode.isFloat() || opCode.isDouble())) {
        ...
        bool usesMul = true;
        if (secondChild->getOpCode().isLoadConst() && (TR::Compiler->target.is64Bit() || !nodeType.isInt64()) && populationCount(integerConstNodeValue(secondChild, self())) <= 2) {
...
TR_X86PaddingTable OMR::X86::CodeGenerator::_old32BitPaddingTable = TR_X86PaddingTable(7, (uint8_t***)&OMR::X86::CodeGenerator::old32BitPaddingEncoding, TR_X86PaddingTable::registerMatters, 0x8b, 0x20);

TR_X86PaddingTable OMR::X86::CodeGenerator::_intelMultiBytePaddingTable = TR_X86PaddingTable(9, (uint8_t***)&OMR::X86::CodeGenerator::intelMultiBytePaddingEncoding, 0x0, 0x0, 0x0);

TR_X86PaddingTable OMR::X86::CodeGenerator::_K8PaddingTable = TR_X86PaddingTable(9, (uint8_t***)&OMR::X86::CodeGenerator::k8PaddingEncoding, 0x0, 0x0, 0x0);
    ...
    bool recursive)
{
    ...
    if (!recursive && self()->comp()->getOptions()->enableDebugCounters()) {
        if (neighborhood) {
            ...
            if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP && guardNode && (guardNode->isTheVirtualGuardForAGuardedInlinedCall() || guardNode->isHCRGuard() || guardNode->isProfiledGuard() || guardNode->isMethodEnterExitGuard())) {
                ...
                if (length > 0) {
                    ...
                    for (TR::Instruction* ninst = neighborhood->getNext(); ninst; ninst = ninst->getNext()) {
                        if (ninst->isVirtualGuardNOPInstruction()) {
                            if (guardNode->isHCRGuard() && ninst->getNode() && ninst->getNode()->isHCRGuard() && guardNode->getBranchDestination() == ninst->getNode()->getBranchDestination())
                                continue;
...
bool OMR::X86::CodeGenerator::patchableRangeNeedsAlignment(void* cursor, intptrj_t length, intptrj_t boundary, intptrj_t margin)
{
...
bool OMR::X86::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress, intptrj_t sourceAddress)
{
    ...
    //
    return !TR::Compiler->target.cpu.isTargetWithinRIPRange(targetAddress, sourceAddress + 5) || self()->comp()->getOption(TR_StressTrampolines);
}
