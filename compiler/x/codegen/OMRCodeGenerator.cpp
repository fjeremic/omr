...
// Hack markers
#define CANT_REMATERIALIZE_ADDRESSES \
  (TR::Compiler->target.is64Bit())  // AMD64 produces a memref with an
                                    // unassigned addressRegister

TR_X86ProcessorInfo OMR::X86::CodeGenerator::_targetProcessorInfo;

void TR_X86ProcessorInfo::initialize() {
  // For now, we only convert the feature bits into a flags32_t, for easier
  // querying. To retrieve other information, the VM functions can be called
  // directly.
  //
  ...
  uint32_t _processorSignature =
      TR::Compiler->target.cpu.getX86ProcessorSignature();
    ...
    switch (getCPUFamily(_processorSignature)) {
      ...
      case 0x06: {
        uint32_t extended_model =
            getCPUModel(_processorSignature) +
            (getCPUExtendedModel(_processorSignature) << 4);
...
void OMR::X86::CodeGenerator::initialize(TR::Compilation *comp) {
  ...
  else if (_targetProcessorInfo.prefersMultiByteNOP() &&
           !comp->getOption(TR_DisableZealousCodegenOpts))
  ...
  if (_targetProcessorInfo.supportsSSE2() &&
      TR::Compiler->target.cpu.testOSForSSESupport())
    supportsSSE2 = true;
#endif  // defined(TR_TARGET_X86) && !defined(J9HAMMER)

  if (_targetProcessorInfo.supportsTM() && !comp->getOption(TR_DisableTM)) {
    /**
     * Due to many verions of Haswell and a small number of Broadwell have
     * defects for TM and then disabled by Intel, we will return false for any
     * versions before Broadwell.
     *
  ...
  // Choose the best XMM double precision load instruction for the target
  // architecture.
  //
  ...
  //
  if (_targetProcessorInfo.supportsSSE() &&
      TR::Compiler->target.cpu.testOSForSSESupport())
#endif  // defined(TR_TARGET_X86) && !defined(J9HAMMER)
  {
  ...
  if (((!comp->getOption(TR_DisableTLHPrefetch) &&
        (comp->cg()->getX86ProcessorInfo().isIntelCore2() ||
         comp->cg()->getX86ProcessorInfo().isIntelNehalem())) ||
       (comp->getOption(TR_TLHPrefetch) &&
        self()->targetSupportsSoftwarePrefetches()))) {
    self()->setEnableTLHPrefetching();
  }

  self()->setGlobalGPRPartitionLimit(
      self()->machine()->getGlobalGPRPartitionLimit());
  self()->setGlobalFPRPartitionLimit(
      self()->machine()->getGlobalFPRPartitionLimit());
  ...
  /*
   * GRA does not work with vector registers on 32 bit due to a bug where xmm
   * registers are not being assigned. This disables GRA for vector registers on
   * 32 bit. This code will be reenabled as part of Issue 2035 which tracks the
   * progress of fixing the GRA bug. GRA does not work with vector registers on
   * 64 bit either. So GRA is now being disabled vector registers. This code
   * will be reenabled as part of Issue 2280
   */
#if 0
   if (TR::Compiler->target.is64Bit())
      {
  ...
  self()->setGlobalRegisterTable(
      self()->machine()->getGlobalRegisterTable(*_linkageProperties));
  ...
  // Use a virtual frame pointer register.  The real frame pointer register to
  // be used will be substituted based on _vfpState during size estimation.
  //
  _frameRegister = self()->machine()->getRealRegister(TR::RealRegister::vfp);

  TR::Register *vmThreadRegister =
      self()->setVMThreadRegister(self()->allocateRegister());
  TR::RealRegister::RegNum vmThreadIndex =
      _linkageProperties->getMethodMetaDataRegister();
  if (vmThreadIndex != TR::RealRegister::NoReg) {
    TR::RealRegister *vmThreadReal =
        self()->machine()->getRealRegister(vmThreadIndex);
  ...
  if (!debug("disableRefinedAliasSets")) self()->setEnableRefinedAliasSets();
  ...
  self()->setSpillsFPRegistersAcrossCalls();  // TODO:AMD64: Are the preserved
                                              // XMMRs relevant here?

  // Make a conservative estimate of the boundary over which an executable
  // instruction cannot be patched.
  //
  int32_t boundary;
  if (_targetProcessorInfo.isGenuineIntel() ||
      (_targetProcessorInfo.isAuthenticAMD() &&
       _targetProcessorInfo.isAMD15h()))
    boundary = 32;
  else {
    // TR_AuthenticAMD
    // TR_UnknownVendor
  ...
  // Smallest code patching boundary that is known to work on all processors we
  // support.
  //
  ...
  if (!comp->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    TR::RealRegister::RegNum vmThreadRealRegisterIndex =
        _linkageProperties->getMethodMetaDataRegister();
  ...
  if (TR::Compiler->target.cpu.isI386()) self()->setGenerateMasmListingSyntax();
  ...
  if (comp->getOptions()->getRegisterAssignmentTraceOption(
    ...
    self()->setFPRegisterIterator(
        new (self()->trHeapMemory())
            TR::RegisterIterator(self()->machine(), TR::RealRegister::FirstXMMR,
                                 TR::RealRegister::LastXMMR));
  }
...
OMR::X86::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(),
      ...
      _dataSnippetList(
          getTypedAllocator<TR::X86DataSnippet *>(TR::comp()->allocator())),
      _spilledIntRegisters(
          getTypedAllocator<TR::Register *>(TR::comp()->allocator())),
      _liveDiscardableRegisters(
          getTypedAllocator<TR::Register *>(TR::comp()->allocator())),
      _dependentDiscardableRegisters(
          getTypedAllocator<TR::Register *>(TR::comp()->allocator())),
      _clobberingInstructions(getTypedAllocator<TR::ClobberingInstruction *>(
          TR::comp()->allocator())),
      _outlinedInstructionsList(getTypedAllocator<TR_OutlinedInstructions *>(
          TR::comp()->allocator())),
...
#if defined(TR_TARGET_64BIT)
        linkage =
            new (self()->trHeapMemory()) TR::AMD64Win64FastCallLinkage(self());
#else
        linkage = new (self()->trHeapMemory()) TR::IA32SystemLinkage(self());
#endif
      } else {
...
void OMR::X86::CodeGenerator::beginInstructionSelection() {
  ...
  if (recompilation && recompilation->generatePrePrologue() != NULL) {
    ...
    if (methodSymbol->getLinkageConvention() == TR_Private)
      _returnTypeInfoInstruction =
          (TR::X86ImmInstruction *)(self()->getAppendInstruction());
  ...
  if (methodSymbol->getLinkageConvention() == TR_Private &&
    ...
    if (self()->getAppendInstruction())
      _returnTypeInfoInstruction =
          generateImmInstruction(DDImm4, startNode, 0, self());
    else
      _returnTypeInfoInstruction = new (self()->trHeapMemory())
          TR::X86ImmInstruction((TR::Instruction *)NULL, DDImm4, 0, self());
  }
  ...
  if (self()->getAppendInstruction())
    ...
    new (self()->trHeapMemory())
        TR::Instruction(PROCENTRY, (TR::Instruction *)NULL, self());
    ...
    generateMemInstruction(
        ...
        generateX86MemoryReference(
            self()->findOrCreate2ByteConstant(
                startNode, SINGLE_PRECISION_ROUND_TO_NEAREST),
            self()),
        self());
  }
}
...
void OMR::X86::CodeGenerator::endInstructionSelection() {
  ...
  if (self()->enableSinglePrecisionMethods() &&
    ...
    TR_ASSERT(self()->getLastCatchAppendInstruction(),
              "endInstructionSelection() ==> Could not find the dummy finally "
              "block!\n");
    generateMemInstruction(
        self()->getLastCatchAppendInstruction(), LDCWMem,
        generateX86MemoryReference(
            self()->findOrCreate2ByteConstant(
                self()->getLastCatchAppendInstruction()->getNode(),
...
int32_t OMR::X86::CodeGenerator::getMaximumNumbersOfAssignableGPRs() {
  return TR::RealRegister::LastAssignableGPR - TR::RealRegister::FirstGPR +
         1;  // TODO:AMD64: This is including rsp
}

/*
 * This method returns TRUE for all the cases we decide NOT to replace the call
 * to CAS with inline assembly. The GRA and Evaluator should be consistent about
 * whether to inline CAS natives.
 */
static bool willNotInlineCompareAndSwapNative(TR::Node *node, int8_t size,
                                              TR::Compilation *comp) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (disableCASInlining /* || comp->useCompressedPointers() */) return true;

  // In Java9 the sun.misc.Unsafe JNI methods have been moved to jdk.internal,
  // with a set of wrappers remaining in sun.misc to delegate to the new
  // package. We can be called in this function for the wrappers (which we will
  ...
  if (!methodSymbol->isNative()) return true;
...
/** @brief Identify methods which are not transformed into inline assembly.
    ...
    @param node The TR::Node for the method call. NB this function assumes the
   Node is a call.

    @return true if the method will be treated as a normal call, false if the
   method will be converted to inline assembly.
 */
bool OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(
...
// X has a different concept of VR's compared to p/z but since LocalOpts needs
// this to be hoisted within CG, this method is placed here as a dummy until
// platform specific code is removed from LocalOpt
...
void OMR::X86::CodeGenerator::clobberLiveDiscardableRegisters(
    TR::Instruction *instr, TR::MemoryReference *mr) {
  ...
  if (symbol) {
    ...
    // If this instruction clobbers the source of any memory discardable
    // register(s), those registers must be deactivated (marked as
    // non-discardable) after this point. We record which registers are
    // deactivated, so that we can re-activate them when we have assigned
    ...
    while (iterator != self()->getLiveDiscardableRegisters().end()) {
      ...
      if (registerCursor->getRematerializationInfo()
              ->isRematerializableFromMemory()) {
        TR::SymbolReference *rmSymRef =
            registerCursor->getRematerializationInfo()->getSymbolReference();

        if ((rmSymRef->getSymbol() == symbol) ||
            (rmSymRef->getOffset() == mr->getSymbolReference().getOffset())) {
          if (!clob) {
            clob = new (self()->trHeapMemory())
                TR::ClobberingInstruction(instr, self()->trMemory());
          ...
          if (debug("dumpRemat")) {
            diagnostic(
                ...
                "in %s\n",
                self()->getDebug()->toString(
                    registerCursor->getRematerializationInfo()),
                self()->getDebug()->getName(registerCursor), instr,
                self()->comp()->signature());
          }
        }

        // If the symbols are different but will share the same stack slot then
        // a clobber has occurred.
        //
        else if (IGNodeSym != NULL) {
          TR_IGNode *IGNodeRematSym =
              self()->getLocalsIG()->getIGNodeForEntity(rmSymRef->getSymbol());
          if ((IGNodeRematSym != NULL) &&
            ...
            if (debug("dumpRemat")) {
              diagnostic(
                  "---> Clobbering %s discardable register %s at instruction "
                  "%p because of shared slot in %s\n",
                  self()->getDebug()->toString(
                      registerCursor->getRematerializationInfo()),
                  self()->getDebug()->getName(registerCursor), instr,
                  self()->comp()->signature());
    ...
    // If a register-dependent discardable register depends on any of the
    // deactivated registers, it (and those that depend on it if any) must also
    // be deactivated.
    //
  ...
  while (!worklist.isEmpty()) {
    ...
    for (auto iterator = self()->getLiveDiscardableRegisters().begin();
      ...
      if (info->isIndirect() && info->getBaseRegister() == baseReg) {
        ...
        if (debug("dumpRemat")) {
          diagnostic(
              "---> Clobbering %s discardable register %s at instruction %p in "
              "%s\n",
              self()->getDebug()->toString(info),
              self()->getDebug()->getName(candidate), clob->getInstruction(),
              self()->comp()->signature());
        }
      } else
        ++iterator;
...
// program point.
//
void OMR::X86::CodeGenerator::reactivateDependentDiscardableRegisters(
    TR::Register *baseReg) {
  TR_Stack<TR::Register *> worklist(self()->trMemory());
  worklist.push(baseReg);
  ...
  if (debug("dumpRemat"))
    diagnostic(
        "---> Re-activating rematerializable registers dependent on %s: ",
  ...
  while (!worklist.isEmpty()) {
    ...
    for (auto iterator = self()->getDependentDiscardableRegisters().begin();
         iterator != self()->getDependentDiscardableRegisters().end();
         ++iterator) {
      if ((*iterator)->isDiscardable() &&
          (*iterator)->getRematerializationInfo()->getBaseRegister() ==
              baseReg) {
        if (debug("dumpRemat"))
        ...
        //
        if ((*iterator)->getAssignedRegister()) worklist.push(*iterator);
      }
    }
  }
...
//
void OMR::X86::CodeGenerator::deactivateDependentDiscardableRegisters(
    TR::Register *baseReg) {
  TR_Stack<TR::Register *> worklist(self()->trMemory());
  worklist.push(baseReg);
...
#define ALLOWED_TO_REMATERIALIZE(x)   \
  (getRematerializationOptString() && \
   strstr(getRematerializationOptString(), (x)))
...
bool OMR::X86::CodeGenerator::supportsStaticMemoryRematerialization() {
  ...
  return !CANT_REMATERIALIZE_ADDRESSES && b;
}
...
bool OMR::X86::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode,
                                                           TR::DataType dt) {
  /*
   * Most of the vector evaluators for opcodes used in AutoSIMD have been
   * implemented. The cases that return false are placeholders that should be
   * updated as support for more vector evaluators is added.
   */
  ...
  switch (opcode.getOpCodeValue()) {
    ...
    case TR::vmul:
      if (dt == TR::Float || dt == TR::Double ||
          (dt == TR::Int32 && self()->getX86ProcessorInfo().supportsSSE4_1()))
        return true;
      else
        return false;
    case TR::vdiv:
    ...
    /*
     * GRA does not work with vector registers on 32 bit due to a bug where xmm
     * registers are not being assigned. This can potentially cause a
     * performance problem in autosimd reductions. This function is where
     * AutoSIMD checks to see if getvelem is suppored for use in reductions. The
     * getvelem case was changed to disable the use of getvelem on 32 bit x86.
     * This code will be reenabled as part of Issue 2035 which tracks the
     * progress of fixing the GRA bug. GRA does not work with vector registers
     * on 64 bit either. getvelem is now being disabled on 64 bit for the same
     * reasons as 32 bit. This code will be reenabled as part of Issue 2280
     */
    case TR::getvelem:
#if 0
...
TR::SymbolReference *OMR::X86::CodeGenerator::getNanoTimeTemp() {
  if (_nanoTimeTemp == NULL) {
    TR::AutomaticSymbol *sym;
#if defined(LINUX) || defined(OSX)
    sym = TR::AutomaticSymbol::create(self()->trHeapMemory(), TR::Aggregate,
                                      sizeof(struct timeval));
#else
    sym = TR::AutomaticSymbol::create(self()->trHeapMemory(), TR::Aggregate, 8);
#endif
    self()->comp()->getMethodSymbol()->addAutomatic(sym);
    _nanoTimeTemp = new (self()->trHeapMemory())
        TR::SymbolReference(self()->comp()->getSymRefTab(), sym);
  }
...
void OMR::X86::CodeGenerator::saveBetterSpillPlacements(
  ...
  for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR;
       i++) {
    realReg = self()->machine()->getRealRegister((TR::RealRegister::RegNum)i);

    // Skip non-assignable registers
    //
    if (realReg->getState() == TR::RealRegister::Locked) continue;

    if (realReg->getAssignedRegister() == NULL) {
      numFreeRealRegisters++;
      freeRealRegisters |= TR::RealRegister::getRealRegisterMask(
          realReg->getKind(), realReg->getRegisterNumber());
    }
  }

  if (!freeRealRegisters) return;
  ...
  for (auto regElement = _spilledIntRegisters.begin();
       regElement != _spilledIntRegisters.end() && numFreeRealRegisters;
       ++regElement) {
    ...
    //
    if ((*regElement)->containsCollectedReference() ||
        (*regElement)->containsInternalPointer() ||
        (*regElement)->hasBetterSpillPlacement())
      continue;

    self()->traceRegisterAssignment(
        "Saved better spill placement for %R, mask = %x.", *regElement,
        freeRealRegisters);

    TR_BetterSpillPlacement *info =
        new (self()->trHeapMemory()) TR_BetterSpillPlacement;
    ...
    if (info->_next) info->_next->_prev = info;
    _betterSpillPlacements = info;
    (*regElement)->setHasBetterSpillPlacement(true);
  }
}

void OMR::X86::CodeGenerator::removeBetterSpillPlacementCandidate(
    TR::RealRegister *realReg) {
  ...
  if (_betterSpillPlacements)
    self()->traceRegisterAssignment(
        "Removed better spill placement candidate %d.", regNum);
  ...
  for (info = _betterSpillPlacements, next = NULL; info; info = next) {
    ...
    if (info->_freeRealRegs == 0) {
      ...
      if (info->_next) info->_next->_prev = info->_prev;
      ...
      self()->traceRegisterAssignment(
          "%R is no longer a candidate for better spill placement.",
          info->_virtReg);
    }
  }
}

TR::Instruction *OMR::X86::CodeGenerator::findBetterSpillPlacement(
    TR::Register *virtReg, int32_t realRegNum) {
  TR::Instruction *placement;
  TR_BetterSpillPlacement *info;
  for (info = _betterSpillPlacements; info; info = info->_next) {
    if (info->_virtReg == virtReg) break;
  }
  if (info && (info->_freeRealRegs &
               TR::RealRegister::getRealRegisterMask(
                   virtReg->getKind(), (TR::RealRegister::RegNum)realRegNum))) {
    placement = info->_branchInstruction;
    self()->traceRegisterAssignment(
        "Successful better spill placement for %R at [" POINTER_PRINTF_FORMAT
        "].",
        virtReg, placement);
  } else {
    placement = NULL;
    self()->traceRegisterAssignment("Failed better spill placement for %R.",
                                    virtReg);
  }
...
void OMR::X86::CodeGenerator::performNonLinearRegisterAssignmentAtBranch(
  ...
  TR_RegisterAssignerState *branchRAState =
      new (self()->trHeapMemory()) TR_RegisterAssignerState(xm);

  // Take a snapshot of the current register assigner state.
  //
  branchRAState->capture();

  TR_OutlinedInstructions *oi = self()->findOutlinedInstructionsFromLabel(
      branchInstruction->getLabelSymbol());

  TR_ASSERT(
      oi, "Could not find OutlinedInstructions from branch label on instr=%p\n",
      branchInstruction);

  // Restore the register use counts on the registers used in the out-of-line
  // path to accurately reflect their state.
  //
  TR::list<OMR::RegisterUsage *> *outlinedRUL =
      oi->getOutlinedPathRegisterUsageList();
  if (outlinedRUL) {
  ...
  //
  TR::list<OMR::RegisterUsage *> *mainlineRUL =
      oi->getMainlinePathRegisterUsageList();
  if (mainlineRUL) {
  ...
  if (deps) {
    TR::Instruction *ins =
        generateLabelInstruction(oi->getFirstInstruction(), LABEL,
                                 generateLabelSymbol(self()), deps, self());

    if (self()->comp()->getOption(TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(self()->comp(),
               "creating LABEL instruction %p for dependencies\n", ins);
    }
  }
  ...
  // Kill any live register whose future use count has been exhausted.  This can
  // happen when a register is live at the merge point, dies in the mainline
  // path, and is not used in the OOL path.
  //
  ...
  //
  TR_ASSERT(
      !oi->hasBeenRegisterAssigned(),
      "outlined instructions should not have been register assigned already");
  ...
  //
  // TODO: live registers that are not spilled at this point should have their
  // backing storage returned to the free spill list.
  //
  self()->unlockFreeSpillList();

  // Disassociate backing storage that was previously reserved for a spilled
  // virtual if virtual is no longer spilled.  This occurs because the the free
  // spill list was locked.
  //
...
void OMR::X86::CodeGenerator::prepareForNonLinearRegisterAssignmentAtMerge(
  ...
  TR_OutlinedInstructions *oi = self()->findOutlinedInstructionsFromMergeLabel(
      mergeInstruction->getLabelSymbol());

  TR_ASSERT(
      oi, "Could not find OutlinedInstructions from merge label on instr=%p\n",
      mergeInstruction);
  ...
  if (outlinedRUL) {
    // Reset the register use counts on the registers used in the out-of-line
    // path to accurately reflect their use on the mainline path.
    //
...
void OMR::X86::CodeGenerator::processClobberingInstructions(
    TR::ClobberingInstruction *clobInstructionCursor,
    TR::Instruction *instructionCursor) {
  // Activate any discardable registers that this instruction may have
  // clobbered.
  //
  ...
  while (clobInstructionCursor &&
         (clobInstructionCursor->getInstruction() == instructionCursor) &&
         self()->enableRematerialisation()) {
    auto regIterator = clobInstructionCursor->getClobberedRegisters().begin();
    while (regIterator !=
           clobInstructionCursor->getClobberedRegisters().end()) {
      ...
      TR_RematerializationInfo *info =
          (*regIterator)->getRematerializationInfo();

      if (!info->isIndirect() ||
          info->getBaseRegister()->getAssignedRegister()) {
        info->setActive();

        if (debug("dumpRemat")) {
          diagnostic(
              "---> Activating %s discardable register %s at instruction %p in "
              "%s\n",
              self()->getDebug()->toString(info),
              self()->getDebug()->getName(*regIterator), instructionCursor,
              self()->comp()->signature());
        }
      }
...
void OMR::X86::CodeGenerator::doBackwardsRegisterAssignment(
    TR_RegisterKinds kindsToAssign, TR::Instruction *instructionCursor,
  ...
  bool dumpPreGP =
      (debug("dumpGPRA") || debug("dumpGPRA0")) && comp->getOutFile() != NULL;
  bool dumpPostGP =
      (debug("dumpGPRA") || debug("dumpGPRA1")) && comp->getOutFile() != NULL;
#endif

  if (self()->getUseNonLinearRegisterAssigner()) {
    if (!self()->getSpilledRegisterList()) {
      self()->setSpilledRegisterList(
          new (self()->trHeapMemory()) TR::list<TR::Register *>(
              getTypedAllocator<TR::Register *>(comp->allocator())));
    }
  }

  if (self()->getDebug())
    self()->getDebug()->startTracingRegisterAssignment("backward",
                                                       kindsToAssign);
  ...
  while (instructionCursor && instructionCursor != appendInstruction) {
    ...
    // code to increment or decrement counter when a internal control flow end
    // or start label is hit
    TR::LabelSymbol *label;
    if (((TR::X86LabelInstruction *)instructionCursor)->getOpCodeValue() ==
            LABEL &&
        (label = ((TR::X86LabelInstruction *)instructionCursor)
                     ->getLabelSymbol())) {
...
#ifdef DEBUG
    if (dumpPostGP)
      self()->dumpPostGPRegisterAssignment(instructionCursor,
                                           origNextInstruction);
#endif
...
void OMR::X86::CodeGenerator::doRegisterAssignment(
  ...
  if (kindsToAssign & TR_X87_Mask) {
    if (self()->getDebug())
      self()->getDebug()->startTracingRegisterAssignment("forward",
                                                         TR_X87_Mask);
...
#if defined(DEBUG)
      if (dumpPostFP)
        self()->dumpPostFPRegisterAssignment(instructionCursor,
                                             origPrevInstruction);
#endif
      self()->tracePostRAInstruction(instructionCursor);
  ...
  //
  kindsToAssign = TR_RegisterKinds(kindsToAssign &
                                   (TR_GPR_Mask | TR_FPR_Mask | TR_VRF_Mask));
  if (kindsToAssign) {
    self()->getVMThreadRegister()->setFutureUseCount(
        self()->getVMThreadRegister()->getTotalUseCount());
    self()->setAssignmentDirection(Backward);
    self()->getFrameRegister()->setFutureUseCount(
        self()->getFrameRegister()->getTotalUseCount());
    ...
    self()->doBackwardsRegisterAssignment(kindsToAssign,
                                          self()->getAppendInstruction());
  }
}

bool OMR::X86::CodeGenerator::isReturnInstruction(TR::Instruction *instr) {
  if (instr->getOpCodeValue() == RET || instr->getOpCodeValue() == RETImm2 ||
...
bool OMR::X86::CodeGenerator::isBranchInstruction(TR::Instruction *instr) {
  return (instr->getOpCode().isBranchOp() ||
                  instr->getOpCode().getOpCodeValue() == CALLImm4
              ? true
              : false);
}

struct DescendingSortX86DataSnippetByDataSize {
  inline bool operator()(TR::X86DataSnippet *const &a,
                         TR::X86DataSnippet *const &b) {
...
void OMR::X86::CodeGenerator::doBinaryEncoding() {
  ...
  while (procEntryInstruction &&
         procEntryInstruction->getOpCodeValue() != PROCENTRY) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    if (self()->comp()->getMethodSymbol()->getLinkageConvention() != TR_System)
      interpreterEntryInstruction =
          self()->getLinkage()->copyStackParametersToLinkageRegisters(
              procEntryInstruction);
    else
      interpreterEntryInstruction = procEntryInstruction;

    // Patching can occur at the jit entry point, so insert padding if
    // necessary.
    //
...
#endif
      {
        // It's safe to protect just the first 2 bytes because we won't patch
        // anything other than a 2-byte jmp
        static const TR_AtomicRegion samplingAtomicRegions[] = {{0, 2}, {0, 0}};
        atomicRegions = samplingAtomicRegions;
      }

      TR::Instruction *pcai = generatePatchableCodeAlignmentInstruction(
          atomicRegions, procEntryInstruction, self());
  ...
  //
  std::sort(_dataSnippetList.begin(), _dataSnippetList.end(),
            DescendingSortX86DataSnippetByDataSize());
  ...
  for (TR::Instruction *gcMapCursor = prologueCursor;
       gcMapCursor != _vfpResetInstruction;
       gcMapCursor = gcMapCursor->getNext()) {
    if (gcMapCursor->needsGCMap())
      gcMapCursor->setGCMap(self()->getStackAtlas()->getParameterMap()->clone(
          self()->trMemory()));
  }
  ...
  while (estimateCursor) {
      ...
      if (numSlotPushes == 0) {
        ...
        if (_vfpState._register == TR::RealRegister::esp) {
          ...
          estimateCursor->getGCMap()->setInfoBits(
              (_vfpState._displacement -
               _vfpResetInstruction->getSavedState()._displacement)
              << 14);
        } else {
    ...
    if (self()->comp()->getOption(TR_TraceVFPSubstitution))
      self()->getDebug()->dumpInstructionWithVFPState(estimateCursor,
                                                      &prevState);

    if (estimateCursor == _vfpResetInstruction)
      self()->generateDebugCounter(
          estimateCursor, "cg.prologues:#instructionBytes",
...
#define OVER_ESTIMATION 4
  ...
  if (TR::Compiler->target.is64Bit() && self()->hasCodeCacheSwitched() &&
      self()->getPicSlotCount() != 0) {
    int32_t numTrampolinesToReserve =
        self()->getPicSlotCount() - self()->getNumReservedIPICTrampolines();
    TR_ASSERT(numTrampolinesToReserve >= 0,
              "Discrepancy with number of IPIC trampolines to reserve "
              "getPicSlotCount()=%d getNumReservedIPICTrampolines()=%d",
              self()->getPicSlotCount(),
              self()->getNumReservedIPICTrampolines());
  ...
  while (cursorInstruction &&
         cursorInstruction != interpreterEntryInstruction) {
  ...
  while (cursorInstruction) {
    ...
    TR_ASSERT(
        ...
        "Instruction length estimate must be conservatively large (instr=%s, "
        "opcode=%s, estimate=%d, actual=%d",
        self()->getDebug() ? self()->getDebug()->getName(cursorInstruction)
                           : "(unknown)",
        self()->getDebug()
            ? self()->getDebug()->getOpCodeName(&cursorInstruction->getOpCode())
            : "(unknown)",
        cursorInstruction->getEstimatedBinaryLength(),
        self()->getBinaryBufferCursor() - instructionStart);
    ...
    if (TR::Compiler->target.is64Bit() &&
      ...
      //
      TR_ASSERT(_returnTypeInfoInstruction->getOpCodeValue() == DDImm4,
                "assertion failure");
      uint32_t magicWord =
      ...
      if (recomp != NULL && recomp->couldBeCompiledAgain()) {
        recompFlag = (recomp->useSampling()) ? METHOD_SAMPLING_RECOMPILATION
                                             : METHOD_COUNTING_RECOMPILATION;
      }
#endif
      magicWord |= recompFlag;
      _returnTypeInfoInstruction->setSourceImmediate(magicWord);
      *(uint32_t *)(_returnTypeInfoInstruction->getBinaryEncoding()) =
          magicWord;
    }
  ...
  for (auto oiIterator = self()->getOutlinedInstructionsList().begin();
       oiIterator != self()->getOutlinedInstructionsList().end();
       ++oiIterator) {
    ...
    if (block && node && !block->getExceptionSuccessors().empty() &&
        node->canGCandExcept())
  ...
  // with the address of the byte to be patched. Otherwise, fail this
  // compilation and retry without GCR.
  if (self()->comp()->getOption(TR_EnableGCRPatching) &&
      self()->comp()->getRecompilationInfo() &&
      self()
          ->comp()
          ->getRecompilationInfo()
          ->getJittedBodyInfo()
          ->getUsesGCR()) {
    ...
    if (!addrToPatch) {
      TR_ASSERT(false,
                "Must have updated gcrPatchPointSymbol with the correct "
                "address by now\n");
      self()->comp()->failCompilation<TR::GCRPatchFailure>(
          "Must have updated gcrPatchPointSymbol with the correct address by "
          "now");
    }
  }
#endif
...
TR::Register *OMR::X86::CodeGenerator::gprClobberEvaluate(
    TR::Node *node, TR_X86OpCodes movRegRegOpCode) {
  ...
  if (self()->comp()->getOption(TR_TraceCG) &&
      sourceRegister->needsLazyClobbering())
    traceMsg(self()->comp(),
             "LAZY CLOBBERING: node %s register %s refcount=%d canClobber=%s\n",
             self()->getDebug()->getName(node),
             self()->getDebug()->getName(sourceRegister),
             node->getReferenceCount(), canClobber ? "true" : "false");
  ...
  if (canClobber) {
    ...
    if (node->getOpCode().isLoadConst()) {
      if (debug("traceClobberedConstantRegisters") && node->getRegister()) {
        trfprintf(self()->comp()->getOutFile(),
                  "CLOBBERING CONSTANT in %s on " POINTER_PRINTF_FORMAT
                  " in %s\n",
                  self()->getDebug()->getName(node->getRegister()), node,
                  self()->comp()->signature());
    ...
    generateRegRegInstruction(movRegRegOpCode, node, targetRegister,
                              sourceRegister, self());
    //
...
TR::Register *OMR::X86::CodeGenerator::intClobberEvaluate(TR::Node *node) {
  TR_ASSERT(!node->getOpCode().is8Byte() || node->getOpCode().isRef(),
            "Non-ref 8bytes must use longClobberEvaluate");
  TR_ASSERT(!node->getOpCode().isRef() || TR::Compiler->target.is32Bit() ||
                self()->comp()->useCompressedPointers(),
            "64-bit references must use longClobberEvaluate unless under "
            "compression");
...
TR::Register *OMR::X86::CodeGenerator::shortClobberEvaluate(TR::Node *node) {
  ...
  TR_ASSERT(!(node->getOpCode().isRef() && TR::Compiler->target.is64Bit()),
...
TR::Register *OMR::X86::CodeGenerator::floatClobberEvaluate(TR::Node *node) {
  if (node->getReferenceCount() > 1) {
    ...
    TR::Register *targetRegister =
        self()->allocateSinglePrecisionRegister(temp->getKind());
    ...
    if (temp->getKind() == TR_FPR)
      generateRegRegInstruction(MOVAPSRegReg, node, targetRegister, temp,
                                self());
    else
      generateFPST0STiRegRegInstruction(FLDRegReg, node, targetRegister, temp,
                                        self());
...
TR::Register *OMR::X86::CodeGenerator::doubleClobberEvaluate(TR::Node *node) {
  if (node->getReferenceCount() > 1) {
    ...
    if (temp->getKind() == TR_FPR)
      generateRegRegInstruction(MOVAPDRegReg, node, targetRegister, temp,
                                self());
    else
      generateFPST0STiRegRegInstruction(DLDRegReg, node, targetRegister, temp,
                                        self());
...
TR_OutlinedInstructions *
OMR::X86::CodeGenerator::findOutlinedInstructionsFromLabel(
    TR::LabelSymbol *label) {
...
TR_OutlinedInstructions *
OMR::X86::CodeGenerator::findOutlinedInstructionsFromMergeLabel(
    TR::LabelSymbol *label) {
  auto oiIterator = self()->getOutlinedInstructionsList().begin();
  while (oiIterator != self()->getOutlinedInstructionsList().end()) {
    if ((*oiIterator)->getRestartLabel() == label) return (*oiIterator);
    ++oiIterator;
  }
...
TR::X86DataSnippet *OMR::X86::CodeGenerator::createDataSnippet(TR::Node *n,
                                                               void *c,
                                                               size_t s) {
  auto snippet =
      new (self()->trHeapMemory()) TR::X86DataSnippet(self(), n, c, s);
  _dataSnippetList.push_back(snippet);
  return snippet;
}

TR::X86ConstantDataSnippet *
OMR::X86::CodeGenerator::findOrCreateConstantDataSnippet(TR::Node *n, void *c,
                                                         size_t s) {
  // A simple linear search should suffice for now since the number of data
  // constants produced is typically very small.  Eventually, this should be
  // implemented as an ordered list or a hash table.
  for (auto iterator = _dataSnippetList.begin();
       iterator != _dataSnippetList.end(); ++iterator) {
  ...
  //
  auto snippet =
      new (self()->trHeapMemory()) TR::X86ConstantDataSnippet(self(), n, c, s);
  _dataSnippetList.push_back(snippet);
  return snippet;
}

int32_t OMR::X86::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
...
TR::X86ConstantDataSnippet *OMR::X86::CodeGenerator::findOrCreate2ByteConstant(
    TR::Node *n, int16_t c) {
  return self()->findOrCreateConstantDataSnippet(n, &c, 2);
}

TR::X86ConstantDataSnippet *OMR::X86::CodeGenerator::findOrCreate4ByteConstant(
    TR::Node *n, int32_t c) {
  return self()->findOrCreateConstantDataSnippet(n, &c, 4);
}

TR::X86ConstantDataSnippet *OMR::X86::CodeGenerator::findOrCreate8ByteConstant(
    TR::Node *n, int64_t c) {
  return self()->findOrCreateConstantDataSnippet(n, &c, 8);
}

TR::X86ConstantDataSnippet *OMR::X86::CodeGenerator::findOrCreate16ByteConstant(
    TR::Node *n, void *c) {
  return self()->findOrCreateConstantDataSnippet(n, c, 16);
}

TR::X86DataSnippet *OMR::X86::CodeGenerator::create2ByteData(TR::Node *n,
                                                             int16_t c) {
  return self()->createDataSnippet(n, &c, 2);
}

TR::X86DataSnippet *OMR::X86::CodeGenerator::create4ByteData(TR::Node *n,
                                                             int32_t c) {
  return self()->createDataSnippet(n, &c, 4);
}

TR::X86DataSnippet *OMR::X86::CodeGenerator::create8ByteData(TR::Node *n,
                                                             int64_t c) {
  return self()->createDataSnippet(n, &c, 8);
}

TR::X86DataSnippet *OMR::X86::CodeGenerator::create16ByteData(TR::Node *n,
                                                              void *c) {
...
void OMR::X86::CodeGenerator::buildRegisterMapForInstruction(
    TR_GCStackMap *map) {
...
bool OMR::X86::CodeGenerator::allowGlobalRegisterAcrossBranch(
    TR_RegisterCandidate *rc, TR::Node *node) {
  ...
  if (node->getOpCode().isSwitch() &&
    ...
    for (int32_t i = node->getCaseIndexUpperBound() - 1; i > 0; --i)
      if (!rc->getBlocksLiveOnEntry().get(node->getChild(i)
...
uint32_t OMR::X86::CodeGenerator::estimateBinaryLength(
...
// endLabel:
//
// This relocation encodes the distance (endLabel - startLabel) in the
// movInstruction's immediate field deltaToStartLabel is the byte distance from
// the start of the instruction following the mov to the startLabel address
void OMR::X86::CodeGenerator::apply32BitLoadLabelRelativeRelocation(
    TR::Instruction *movInstruction, TR::LabelSymbol *startLabel,
    TR::LabelSymbol *endLabel, int32_t deltaToStartLabel) {
  TR::Instruction *movInstructionX86 = movInstruction;

  TR_ASSERT(movInstructionX86->getOpCodeValue() == MOV4RegImm4,
            "wrong load instruction used for "
            "apply32BitLoadLabelRelativeRelocation\n");

  uint8_t *cursor = movInstructionX86->getBinaryEncoding();
  cursor +=
      movInstructionX86->getOpCode().length(movInstructionX86->rexBits()) +
      movInstructionX86->rexRepeatCount();

  int32_t distance =
      int32_t(endLabel->getCodeLocation() - startLabel->getCodeLocation());
  TR_ASSERT(
      IS_32BIT_SIGNED(distance),
...
void OMR::X86::CodeGenerator::apply32BitLabelRelativeRelocation(
...
int32_t OMR::X86::CodeGenerator::branchDisplacementToHelperOrTrampoline(
    uint8_t *nextInstructionAddress, TR::SymbolReference *helper) {
  intptrj_t helperAddress = (intptrj_t)helper->getMethodAddress();

  if (self()->directCallRequiresTrampoline(helperAddress,
                                           (intptrj_t)nextInstructionAddress)) {
    helperAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
        helper->getReferenceNumber(), (void *)(nextInstructionAddress - 4));

    TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinRIPRange(
                        helperAddress, (intptrj_t)nextInstructionAddress),
...
//
TR::RealRegister::RegNum OMR::X86::CodeGenerator::pickNOPRegister(
    TR::Instruction *successor) {
  int32_t j = 1;
  ...
  TR::RealRegister *ebx =
      self()->machine()->getRealRegister(TR::RealRegister::ebx);
  TR::RealRegister *esi =
      self()->machine()->getRealRegister(TR::RealRegister::esi);
  TR::RealRegister *edi =
      self()->machine()->getRealRegister(TR::RealRegister::edi);
  ...
  if (successor) {
    ...
    while (j <= WINDOW_SIZE && cursor) {
      if (cursor->getOpCodeValue() != FENCE &&
        ...
        if (!ebxLastDef && cursor->defsRegister(ebx)) ebxLastDef = j;
        if (!esiLastDef && cursor->defsRegister(esi)) esiLastDef = j;
        if (!ediLastDef && cursor->defsRegister(edi)) ediLastDef = j;
      }
  ...
  if (ediLastDef < min) reg = TR::RealRegister::edi;
...
// TODO: Don't duplicate this function all over the place.  Find a good header
// for it.
...
bool OMR::X86::CodeGenerator::nodeIsFoldableMemOperand(
  ...
  if (node->getFutureUseCount() > 1) {
    ...
    //
    if (parent->getOpCode().isBndCheck() && node->getOpCode().isArrayLength() &&
        node->getFutureUseCount() == 2) {
      ...
      if (prevTT) {
        ...
        if (nullchk->getOpCode().isNullCheck() &&
            nullchk->getFirstChild() == node)
          result = true;
      }
    }
  }
  ...
  if (self()->traceSimulateTreeEvaluation() && result)
    traceMsg(self()->comp(), " %s foldable into %s",
             self()->getDebug()->getName(node),
...
inline bool lookForMemrefInChild(TR::Node *node) {
  return node->getNumChildren() >= 1 && node->getType().isInt64() &&
         node->isHighWordZero();
}

uint8_t OMR::X86::CodeGenerator::nodeResultGPRCount(
    TR::Node *node, TR_RegisterPressureState *state) {
  TR_ASSERT(!self()->comp()->getOption(TR_DisableRegisterPressureSimulation),
            "assertion failure");

  // 32-bit integer constants practically never need a register on x86
  //  (includes 64-bit integer constants with high word zero needed to maintain
  //  IL correctness)
  //
  if (node->getOpCode().isLoadConst() &&
      (node->getSize() <= 4 ||
       (node->getType().isInt64() && node->isHighWordZero())) &&
      (node->getType().isAddress() || node->getType().isIntegral()) &&
      !(self()->simulatedNodeState(node, state)._keepLiveUntil !=
            NULL  // Check if parent will become a RegStore that keeps this node
                  // live
...
void OMR::X86::CodeGenerator::simulateNodeEvaluation(
  ...
  int32_t toFold =
      -1;  // Child numbers are usually unsigned, but then we couldn't use -1

  if ((node->getNumChildren() == 2 || opCode.isBooleanCompare()) &&
      !opCode.isStore() && !opCode.isIndirect() &&
    ...
    //
    bool clobbersNothing =
        node->getOpCode().isBooleanCompare() || node->getOpCode().isBndCheck();
    bool tryFoldingLeft = node->getOpCode().isCommutative() || clobbersNothing;

    if (TR::Compiler->target.is32Bit()) {
      while (lookForMemrefInChild(left)) left = left->getFirstChild();
      while (lookForMemrefInChild(right)) right = right->getFirstChild();
    }
    ...
    //
    if (tryFoldingLeft &&
        (clobbersNothing || right->getFutureUseCount() == 1) &&
  ...
  if (toFold != -1)  // toFold child can be folded into a memory operand
  {
    ...
    // Fold memref child
    self()->simulateMemoryReference(&memref, node->getChild(toFold), state,
                                    summary);
  ...
  if ((opCode.isMul() || opCode.isDiv() || opCode.isRem()) &&
      !(opCode.isFloat() || opCode.isDouble())) {
    ...
    if (usesMul) {
      ...
      if (self()->isCandidateLoad(firstChild, candidate) &&
          firstChild->getFutureUseCount() == 1)
        candidateDiesHere = true;
      if (self()->isCandidateLoad(secondChild, candidate) &&
          secondChild->getFutureUseCount() == 1)
        candidateDiesHere = true;
  ...
  if (opCode.isByte()) summary->spill(TR_eaxSpill, self());
}

uint8_t OMR::X86::CodeGenerator::old32BitPaddingEncoding
    [PADDING_TABLE_MAX_ENCODING_LENGTH][PADDING_TABLE_MAX_ENCODING_LENGTH] = {
        {0x90},                    // 1: xchg eax,eax
        {0x8b, 0xc0},              // 2: mov eax,eax
        {0x8d, 0x04, 0x20},        // 3: lea exx, [exx]           (with SIB)
        {0x8d, 0x44, 0x20, 0x00},  // 4: lea exx, [exx+00]        (with SIB)
        {0x3e, 0x8d, 0x44, 0x20,
         0x00},  // 5: lea exx, DS:[exx+00]     (with SIB) (with prefix)
        {0x8d, 0x80, 0x00, 0x00, 0x00, 0x00},  // 6: lea exx, [exx+00000000]
        {0x8d, 0x84, 0x20, 0x00, 0x00, 0x00,
         0x00},  // 7: lea exx, [exx+00000000]  (with SIB)
};

uint8_t OMR::X86::CodeGenerator::intelMultiBytePaddingEncoding
    [PADDING_TABLE_MAX_ENCODING_LENGTH][PADDING_TABLE_MAX_ENCODING_LENGTH] = {
        {0x90},
        {0x66, 0x90},
        {0x0f, 0x1f, 0x00},
...
uint8_t OMR::X86::CodeGenerator::k8PaddingEncoding
    [PADDING_TABLE_MAX_ENCODING_LENGTH][PADDING_TABLE_MAX_ENCODING_LENGTH] = {
        {0x90},
        {0x66, 0x90},
        {0x66, 0x66, 0x90},
...
TR_X86PaddingTable OMR::X86::CodeGenerator::_old32BitPaddingTable =
    TR_X86PaddingTable(
        7, (uint8_t ***)&OMR::X86::CodeGenerator::old32BitPaddingEncoding,
        TR_X86PaddingTable::registerMatters, 0x8b, 0x20);

TR_X86PaddingTable OMR::X86::CodeGenerator::_intelMultiBytePaddingTable =
    TR_X86PaddingTable(
        9, (uint8_t ***)&OMR::X86::CodeGenerator::intelMultiBytePaddingEncoding,
        0x0, 0x0, 0x0);

TR_X86PaddingTable OMR::X86::CodeGenerator::_K8PaddingTable =
    TR_X86PaddingTable(9,
                       (uint8_t ***)&OMR::X86::CodeGenerator::k8PaddingEncoding,
                       0x0, 0x0, 0x0);

static const uint8_t *paddingTableEncoding(TR_X86PaddingTable *paddingTable,
                                           uint8_t length) {
  TR_ASSERT(paddingTable->_biggestEncoding <= PADDING_TABLE_MAX_ENCODING_LENGTH,
            "Padding table must have _biggestEncoding <= "
            "PADDING_TABLE_MAX_ENCODING_LENGTH (%d <= %d)",
            paddingTable->_biggestEncoding, PADDING_TABLE_MAX_ENCODING_LENGTH);
  TR_ASSERT(length <= paddingTable->_biggestEncoding,
            "Padding length %d cannot exceed %d", length,
            paddingTable->_biggestEncoding);
  return (uint8_t *)((uintptrj_t)(paddingTable->_encodings) +
                     (length - 1) * PADDING_TABLE_MAX_ENCODING_LENGTH);
}

uint8_t *OMR::X86::CodeGenerator::generatePadding(
    uint8_t *cursor, intptrj_t length, TR::Instruction *neighborhood,
    TR_PaddingProperties properties, bool recursive) {
  const uint8_t *desiredReturnValue = cursor + length;
  const uint8_t sibMask = 0xb8;  // 10111000, where 2^n is set if the NOP with
                                 // size n uses a SIB byte
  if (length <= _paddingTable->_biggestEncoding) {
    ...
    const intptrj_t jmpThreshold =
        100;  // Beyond this length, a jmp is faster than a sequence of no-op
              // instructions

    if ((properties & TR_AtomicNoOpPadding || length >= jmpThreshold)) {
      TR_ASSERT(0,
                "Oops -- we should never get here because using JMPs for "
                "no-ops is very slow");
      if (length >= 5) {
        length -= 5;
      ...
      while (length > _paddingTable->_biggestEncoding) {
        cursor = self()->generatePadding(
            cursor, _paddingTable->_biggestEncoding, neighborhood, properties);
      ...
      //
      cursor =
          self()->generatePadding(cursor, length, neighborhood, properties);
    }
  }
  ...
  if (!recursive && self()->comp()->getOptions()->enableDebugCounters()) {
    if (neighborhood) {
      ...
      for (TR::Instruction *finst = neighborhood; finst;
           finst = finst->getPrev()) {
      ...
      if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP &&
          guardNode &&
          (guardNode->isTheVirtualGuardForAGuardedInlinedCall() ||
           guardNode->isHCRGuard() || guardNode->isProfiledGuard() ||
           guardNode->isMethodEnterExitGuard())) {
        const char *guardKind;
        TR_VirtualGuard *vg =
            self()->comp()->findVirtualGuardInfo(neighborhood->getNode());
        switch (vg->getKind()) {
          case TR_NoGuard:
            guardKind = "NoGuard";
            break;
        ...
        TR::DebugCounter::incStaticDebugCounter(
            ...
            TR::DebugCounter::debugCounterName(
                self()->comp(), "nopCount/%d/%s/%s", blockFrequency,
                neighborhood->description(), guardKind));
        if (length > 0) {
          TR::DebugCounter::incStaticDebugCounter(
              self()->comp(),
              TR::DebugCounter::debugCounterName(
                  self()->comp(), "nopInst/%d/%s/%s", blockFrequency,
                  neighborhood->description(), guardKind));

          for (TR::Instruction *ninst = neighborhood->getNext(); ninst;
               ninst = ninst->getNext()) {
            if (ninst->isVirtualGuardNOPInstruction()) {
              if (guardNode->isHCRGuard() && ninst->getNode() &&
                  ninst->getNode()->isHCRGuard() &&
                  guardNode->getBranchDestination() ==
                      ninst->getNode()->getBranchDestination())
                continue;
              TR::DebugCounter::incStaticDebugCounter(
                  self()->comp(),
                  TR::DebugCounter::debugCounterName(
                      self()->comp(), "vgnopNoPatchReason/%d/vgnop",
                      blockFrequency));
              break;
            }
            if (self()->comp()->isPICSite(ninst)) {
              TR::DebugCounter::incStaticDebugCounter(
                  self()->comp(),
                  TR::DebugCounter::debugCounterName(
                      self()->comp(), "vgnopNoPatchReason/%d/staticPIC",
                      blockFrequency));
              break;
            }
            if (ninst->isPatchBarrier()) {
              if (ninst->getOpCodeValue() != LABEL)
                TR::DebugCounter::incStaticDebugCounter(
                    self()->comp(),
                    TR::DebugCounter::debugCounterName(
                        self()->comp(), "vgnopNoPatchReason/%d/patchBarrier",
                        blockFrequency));
              else
                TR::DebugCounter::incStaticDebugCounter(
                    self()->comp(),
                    TR::DebugCounter::debugCounterName(
                        self()->comp(),
                        "vgnopNoPatchReason/%d/controlFlowMerge",
                        blockFrequency));
              break;
            }
            if (ninst->getEstimatedBinaryLength() > 0) {
              TR::DebugCounter::incStaticDebugCounter(
                  self()->comp(),
                  TR::DebugCounter::debugCounterName(
                      self()->comp(), "vgnopNoPatchReason/%d/%s_%s",
                      blockFrequency,
                      (guardNode->isHCRGuard() ? "hcr" : "nohcr"),
                      ninst->description()));
            }
          }
        }
        ...
        TR::DebugCounter::incStaticDebugCounter(
            self()->comp(), TR::DebugCounter::debugCounterName(
                                self()->comp(), "nopCount/%d/%s",
                                blockFrequency, neighborhood->description()));
        if (length > 0)
          TR::DebugCounter::incStaticDebugCounter(
              self()->comp(), TR::DebugCounter::debugCounterName(
                                  self()->comp(), "nopInst/%d/%s",
                                  blockFrequency, neighborhood->description()));
      }
    } else {
  ...
  TR_ASSERT(cursor == desiredReturnValue,
            "Must produce the correct amount of padding");
  return cursor;
}

intptrj_t OMR::X86::CodeGenerator::alignment(void *cursor, intptrj_t boundary,
                                             intptrj_t margin) {
  return self()->alignment((intptrj_t)cursor, boundary, margin);
}

bool OMR::X86::CodeGenerator::patchableRangeNeedsAlignment(void *cursor,
                                                           intptrj_t length,
                                                           intptrj_t boundary,
                                                           intptrj_t margin) {
...
TR_X86ScratchRegisterManager *
OMR::X86::CodeGenerator::generateScratchRegisterManager(int32_t capacity) {
  return new (self()->trHeapMemory())
      TR_X86ScratchRegisterManager(capacity, self());
}

bool TR_X86ScratchRegisterManager::reclaimAddressRegister(
    TR::MemoryReference *mr) {
  if (TR::Compiler->target.is32Bit()) return false;
...
TR::Instruction *OMR::X86::CodeGenerator::generateDebugCounterBump(
  ...
  if (delta == 1)
    return generateMemInstruction(
        cursor, INC4Mem,
        generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()),
                                   self()),
        self());
  else
    return generateMemImmInstruction(
        cursor, (-128 <= delta && delta <= 127) ? ADD4MemImms : ADD4MemImm4,
        generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()),
                                   self()),
        delta, self());
}
...
TR::Instruction *OMR::X86::CodeGenerator::generateDebugCounterBump(
  ...
  if (deltaReg)
    return generateMemRegInstruction(
        cursor, ADD4MemReg,
        generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()),
                                   self()),
        deltaReg, self());
...
//
void OMR::X86::CodeGenerator::dumpPreFPRegisterAssignment(
    TR::Instruction *instructionCursor) {
  if (self()->comp()->getOutFile() == NULL) return;
  ...
  if (instructionCursor->totalReferencedFPRegisters(self()) > 0) {
    ...
    self()->getDebug()->printReferencedRegisterInfo(
        self()->comp()->getOutFile(), instructionCursor);

    if (debug("dumpFPRegStatus")) {
      self()->machine()->printFPRegisterStatus(self()->fe(),
...
void OMR::X86::CodeGenerator::dumpPostFPRegisterAssignment(
  ...
  if (instructionCursor->totalReferencedFPRegisters(self()) > 0) {
    ...
    trfprintf(self()->comp()->getOutFile(),
              "\n>> Post-FPR assignment for instruction: %p",
              instructionCursor);
...
void OMR::X86::CodeGenerator::dumpPreGPRegisterAssignment(
    TR::Instruction *instructionCursor) {
  if (self()->comp()->getOutFile() == NULL) return;
  ...
  if (instructionCursor->totalReferencedGPRegisters(self()) > 0) {
    ...
    if (debug("dumpGPRegStatus")) {
      self()->machine()->printGPRegisterStatus(
          self()->fe(), self()->machine()->registerFile(),
          self()->comp()->getOutFile());
    }
  }
}
...
bool OMR::X86::CodeGenerator::useSSEFor(TR::DataType type) {
  if (type == TR::Float)
...
bool OMR::X86::CodeGenerator::needToAvoidCommoningInGRA() {
  if (!self()->useSSEForSinglePrecision() &&
      !self()->useSSEForDoublePrecision())
    return true;
  return false;
}

int32_t OMR::X86::CodeGenerator::arrayTranslateMinimumNumberOfElements(
    bool isByteSource, bool isByteTarget) {
  if (TR::CodeGenerator::useOldArrayTranslateMinimumNumberOfIterations())
    return OMR::CodeGenerator::arrayTranslateMinimumNumberOfElements(
        isByteSource, isByteTarget);
  return 8;
}
...
bool OMR::X86::CodeGenerator::directCallRequiresTrampoline(
    intptrj_t targetAddress, intptrj_t sourceAddress) {
  // Adjust the sourceAddress to the start of the following instruction (+5
  // bytes)
  //
  return !TR::Compiler->target.cpu.isTargetWithinRIPRange(targetAddress,
                                                          sourceAddress + 5) ||
