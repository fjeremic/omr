...
#define CANT_REMATERIALIZE_ADDRESSES \
  (TR::Compiler->target.is64Bit()) // AMD64 produces a memref with an unassigned addressRegister
...
void OMR::X86::CodeGenerator::initialize(TR::Compilation* comp)
{
  ...
  if (((!comp->getOption(TR_DisableTLHPrefetch)
         && (comp->cg()->getX86ProcessorInfo().isIntelCore2() || comp->cg()->getX86ProcessorInfo().isIntelNehalem()))
  ...
  if (_targetProcessorInfo.isGenuineIntel()
    || (_targetProcessorInfo.isAuthenticAMD() && _targetProcessorInfo.isAMD15h()))
    boundary = 32;
  else {
    // TR_AuthenticAMD
    // TR_UnknownVendor
...
void OMR::X86::CodeGenerator::beginInstructionSelection()
{
  ...
  if (methodSymbol->getLinkageConvention() == TR_Private && !_returnTypeInfoInstruction) {
    ...
    if (self()->getAppendInstruction())
      ...
      _returnTypeInfoInstruction
        = new (self()->trHeapMemory()) TR::X86ImmInstruction((TR::Instruction*)NULL, DDImm4, 0, self());
  }
  ...
  if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
    generateMemInstruction(LDCWMem, startNode,
...
void OMR::X86::CodeGenerator::endInstructionSelection()
{
  ...
  if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
    ...
    generateMemInstruction(self()->getLastCatchAppendInstruction(), LDCWMem,
      generateX86MemoryReference(self()->findOrCreate2ByteConstant(self()->getLastCatchAppendInstruction()->getNode(),
...
bool OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(TR::Node* node, TR::Compilation* comp)
{
  ...
  case TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z:
    return willNotInlineCompareAndSwapNative(
      node, (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? 8 : 4, comp);
#endif
  default:
    return true;
  }
  ...
  if (symbol) {
    ...
    while (iterator != self()->getLiveDiscardableRegisters().end()) {
      ...
      if (registerCursor->getRematerializationInfo()->isRematerializableFromMemory()) {
        ...
        if ((rmSymRef->getSymbol() == symbol) || (rmSymRef->getOffset() == mr->getSymbolReference().getOffset())) {
          if (!clob) {
...
void OMR::X86::CodeGenerator::reactivateDependentDiscardableRegisters(TR::Register* baseReg)
{
  ...
  if (debug("dumpRemat"))
    diagnostic("---> Re-activating rematerializable registers dependent on %s: ", self()->getDebug()->getName(baseReg));
...
void OMR::X86::CodeGenerator::deactivateDependentDiscardableRegisters(TR::Register* baseReg)
{
  ...
  if (debug("dumpRemat"))
    diagnostic("---> Deactivating rematerialisable registers dependent on %s: ", self()->getDebug()->getName(baseReg));
...
bool OMR::X86::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode, TR::DataType dt)
{
  /*
...
void OMR::X86::CodeGenerator::saveBetterSpillPlacements(TR::Instruction* branchInstruction)
{
  ...
  //
  for (auto regElement = _spilledIntRegisters.begin(); regElement != _spilledIntRegisters.end() && numFreeRealRegisters;
       ++regElement) {
  ...
  TR_RegisterKinds kindsToAssign, TR::Instruction* instructionCursor, TR::Instruction* appendInstruction)
{
  ...
  if (self()->getUseNonLinearRegisterAssigner()) {
    if (!self()->getSpilledRegisterList()) {
      self()->setSpilledRegisterList(
        new (self()->trHeapMemory()) TR::list<TR::Register*>(getTypedAllocator<TR::Register*>(comp->allocator())));
    }
  }
  ...
  while (instructionCursor && instructionCursor != appendInstruction) {
    ...
    if (((TR::X86LabelInstruction*)instructionCursor)->getOpCodeValue() == LABEL
...
{
  if (instr->getOpCodeValue() == RET || instr->getOpCodeValue() == RETImm2 || instr->getOpCodeValue() == ReturnMarker)
    return true;
  else
    return false;
}
...
#define OVER_ESTIMATION 4
  ...
  if (TR::Compiler->target.is64Bit() && self()->hasCodeCacheSwitched() && self()->getPicSlotCount() != 0) {
    ...
    TR_ASSERT(numTrampolinesToReserve >= 0,
      "Discrepancy with number of IPIC trampolines to reserve getPicSlotCount()=%d getNumReservedIPICTrampolines()=%d",
  ...
  while (cursorInstruction) {
    ...
    if (TR::Compiler->target.is64Bit() && (cursorInstruction->getOpCodeValue() == PROCENTRY)) {
  ...
  if (canClobber) {
    ...
    if (node->getOpCode().isLoadConst()) {
      if (debug("traceClobberedConstantRegisters") && node->getRegister()) {
        trfprintf(self()->comp()->getOutFile(), "CLOBBERING CONSTANT in %s on " POINTER_PRINTF_FORMAT " in %s\n",
...
TR::Register* OMR::X86::CodeGenerator::intClobberEvaluate(TR::Node* node)
{
  ...
  TR_ASSERT(!node->getOpCode().isRef() || TR::Compiler->target.is32Bit() || self()->comp()->useCompressedPointers(),
...
TR::Register* OMR::X86::CodeGenerator::shortClobberEvaluate(TR::Node* node)
{
  ...
  TR_ASSERT(
    !(node->getOpCode().isRef() && TR::Compiler->target.is64Bit()), "64-bit references must use longClobberEvaluate");
  ...
  TR::Instruction* movInstruction, TR::LabelSymbol* startLabel, TR::LabelSymbol* endLabel, int32_t deltaToStartLabel)
{
  ...
  TR_ASSERT(IS_32BIT_SIGNED(distance), "relative label does not fit in 4 byte mov immediate instruction\n");
...
TR::RealRegister::RegNum OMR::X86::CodeGenerator::pickNOPRegister(TR::Instruction* successor)
{
  ...
  if (successor) {
    ...
    while (j <= WINDOW_SIZE && cursor) {
      if (cursor->getOpCodeValue() != FENCE && cursor->getOpCodeValue() != LABEL) {
        ++j;
  ...
  TR::Node* node, TR::Node* parent, TR_RegisterPressureState* state)
{
  ...
  if (self()->traceSimulateTreeEvaluation() && result)
    traceMsg(
      self()->comp(), " %s foldable into %s", self()->getDebug()->getName(node), self()->getDebug()->getName(parent));
  return result;
}
...
uint8_t OMR::X86::CodeGenerator::nodeResultGPRCount(TR::Node* node, TR_RegisterPressureState* state)
{
  ...
  //
  if (node->getOpCode().isLoadConst() && (node->getSize() <= 4 || (node->getType().isInt64() && node->isHighWordZero()))
  ...
  TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
  ...
  if ((opCode.isMul() || opCode.isDiv() || opCode.isRem()) && !(opCode.isFloat() || opCode.isDouble())) {
    ...
    bool usesMul = true;
    if (secondChild->getOpCode().isLoadConst() && (TR::Compiler->target.is64Bit() || !nodeType.isInt64())
...
uint8_t* OMR::X86::CodeGenerator::generatePadding(
  uint8_t* cursor, intptrj_t length, TR::Instruction* neighborhood, TR_PaddingProperties properties, bool recursive)
{
    ...
    if (neighborhood) {
      ...
      if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP && guardNode
        ...
        if (length > 0) {
          TR::DebugCounter::incStaticDebugCounter(self()->comp(),
            TR::DebugCounter::debugCounterName(
              self()->comp(), "nopInst/%d/%s/%s", blockFrequency, neighborhood->description(), guardKind));
          ...
          for (TR::Instruction* ninst = neighborhood->getNext(); ninst; ninst = ninst->getNext()) {
            if (ninst->isVirtualGuardNOPInstruction()) {
              if (guardNode->isHCRGuard() && ninst->getNode() && ninst->getNode()->isHCRGuard()
                ...
                continue;
              TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                TR::DebugCounter::debugCounterName(self()->comp(), "vgnopNoPatchReason/%d/vgnop", blockFrequency));
              break;
            }
            if (self()->comp()->isPICSite(ninst)) {
              TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                TR::DebugCounter::debugCounterName(self()->comp(), "vgnopNoPatchReason/%d/staticPIC", blockFrequency));
              break;
            }
            ...
            if (ninst->getEstimatedBinaryLength() > 0) {
              TR::DebugCounter::incStaticDebugCounter(self()->comp(),
                TR::DebugCounter::debugCounterName(self()->comp(), "vgnopNoPatchReason/%d/%s_%s", blockFrequency,
...
intptrj_t OMR::X86::CodeGenerator::alignment(void* cursor, intptrj_t boundary, intptrj_t margin)
{
  return self()->alignment((intptrj_t)cursor, boundary, margin);
}

bool OMR::X86::CodeGenerator::patchableRangeNeedsAlignment(
  void* cursor, intptrj_t length, intptrj_t boundary, intptrj_t margin)
{
  ...
  TR::Instruction* cursor, TR::DebugCounterBase* counter, int32_t delta, TR::RegisterDependencyConditions* cond)
{
  if (delta == 1)
    return generateMemInstruction(
      cursor, INC4Mem, generateX86MemoryReference(counter->getBumpCountSymRef(self()->comp()), self()), self());
  else
    return generateMemImmInstruction(cursor, (-128 <= delta && delta <= 127) ? ADD4MemImms : ADD4MemImm4,
...
{
  if (!self()->useSSEForSinglePrecision() && !self()->useSSEForDoublePrecision())
    return true;
  return false;
}
...
bool OMR::X86::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress, intptrj_t sourceAddress)
{
  // Adjust the sourceAddress to the start of the following instruction (+5 bytes)
  //
  return !TR::Compiler->target.cpu.isTargetWithinRIPRange(targetAddress, sourceAddress + 5)
