...
struct X86LinkageProperties {
  ...
  //
  enum { MaxArgumentRegisters = 30, MaxReturnRegisters = 3, MaxVolatileRegisters = 30, MaxScratchRegisters = 5 };
  ...
  uint32_t getCallerCleanup() const
  {
    return (_properties & CallerCleanup);
  }
  uint32_t passArgsRightToLeft() const
  {
  ...
  uint8_t getGPRWidth() const
  {
    return getPointerSize();
  }
  uint8_t getRetAddressWidth() const
  {
    return getGPRWidth();
  }
  ...
  int32_t getOffsetToFirstParm() const
  {
    return _offsetToFirstParm;
  }

  int32_t getOffsetToFirstLocal() const
  {
  ...
  uint8_t getNumPreservedRegisters() const
  {
    return _numPreservedRegisters;
  }
  uint8_t getNumVolatileRegisters() const
  {
    return _numVolatileRegisters;
  }
  uint8_t getNumScratchRegisters() const
  {
    return _numScratchRegisters;
  }
  ...
  uint32_t getOutgoingArgAlignment() const
  {
    return _OutgoingArgAlignment;
  }
...
public:
  ...
  {
    TR_ASSERT(
      TR::Compiler->target.is64Bit() || dataType != TR_MovDataTypes::Int8, "MOV Int8 should not occur on X86-32");
...
protected:
  ...
  virtual uint8_t* flushArguments(TR::Node* callNode, uint8_t* cursor, bool calculateSizeOnly, int32_t* sizeOfFlushArea,
