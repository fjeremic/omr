...
struct MovStatus  // What kind of RegReg movs must be applied to a given
                  // register
{
...
struct X86LinkageProperties {
  ...
  int8_t _offsetToFirstLocal;  // Points immediately after the local with
                               // highest address
  ...
  uint32_t doesCallerAllocatesBackingStore(
      TR::RealRegister::RegNum regNum) const {
...
class OMR_EXTENSIBLE Linkage : public OMR::Linkage {
 public:
  ...
  virtual TR::Instruction *copyStackParametersToLinkageRegisters(
      TR::Instruction *procEntryInstruction) {
  ...
  static inline TR_X86OpCodes movOpcodes(TR_MovOperandTypes operandType,
    ...
    TR_ASSERT(
        TR::Compiler->target.is64Bit() || dataType != TR_MovDataTypes::Int8,
