...
struct MovStatus // What kind of RegReg movs must be applied to a given register
{
  ...
  TR::RealRegister::RegNum
      sourceReg; // Which reg will be copied over the given reg
  TR::RealRegister::RegNum
      targetReg; // Which reg will receive the given reg's value
  TR_MovDataTypes outgoingDataType; // Type of data originally in the given reg
};

struct X86LinkageProperties {

  // Limits applicable to all linkages.  They must cover both integer and float
  // registers.
  //
  enum {
  ...
  int8_t _offsetToFirstLocal; // Points immediately after the local with highest
                              // address
  ...
  uint32_t getIntegersInRegisters() const {
    return (_properties & IntegersInRegisters);
  }
  uint32_t getLongsInRegisters() const {
    return (_properties & LongsInRegisters);
  }
  uint32_t getFloatsInRegisters() const {
    return (_properties & FloatsInRegisters);
  }
  uint32_t getAlwaysDedicateFramePointerRegister() const {
    return (_properties & AlwaysDedicateFramePointerRegister);
  }
  uint32_t getNeedsThunksForIndirectCalls() const {
    return (_properties & NeedsThunksForIndirectCalls);
  }
  uint32_t getUsesPushesForPreservedRegs() const {
    return (_properties & UsesPushesForPreservedRegs);
  }
  uint32_t getReservesOutgoingArgsInPrologue() const {
    return (_properties & ReservesOutgoingArgsInPrologue);
  }
  uint32_t getUsesRegsForHelperArgs() const {
    return (_properties & UsesRegsForHelperArgs);
  }

  uint8_t getPointerSize() const {
    return (_properties & EightBytePointers) ? 8 : 4;
  }
  uint8_t getPointerShift() const {
    return (_properties & EightBytePointers) ? 3 : 2;
  }

  uint8_t getParmSlotSize() const {
    return (_properties & EightByteParmSlots) ? 8 : 4;
  }
  uint8_t getParmSlotShift() const {
    return (_properties & EightByteParmSlots) ? 3 : 2;
  }
  ...
  uint32_t isPreservedRegister(TR::RealRegister::RegNum regNum) const {
  ...
  uint32_t
  doesCallerAllocatesBackingStore(TR::RealRegister::RegNum regNum) const {
  ...
  uint32_t getKilledAndNonReturn(TR::RealRegister::RegNum regNum) const {
    return ((_registerFlags[regNum] &
             (Preserved | IntegerReturn | FloatReturn)) == 0);
  }
  uint32_t getPreservedRegisterMapForGC() const {
    return _preservedRegisterMapForGC;
  }

  TR::RealRegister::RegNum getPreservedRegister(uint32_t index) const {
  ...
  uint8_t getNumIntegerArgumentRegisters() const {
    return _numIntegerArgumentRegisters;
  }
  uint8_t getNumFloatArgumentRegisters() const {
  ...
  uint32_t setOutgoingArgAlignment(uint32_t s) {
    return (_OutgoingArgAlignment = s);
  }
  ...
  TR::RealRegister::RegNum getArgumentRegister(uint8_t index,
                                               bool isFloat) const {
    return isFloat ? getFloatArgumentRegister(index)
                   : getIntegerArgumentRegister(index);
  }
  ...
  uint8_t getMaxRegistersPreservedInPrologue() const {
    return _maxRegistersPreservedInPrologue;
  }

  uint32_t getNumberOfVolatileGPRegisters() const {
    return _numberOfVolatileGPRegisters;
  }
  uint32_t getNumberOfVolatileXMMRegisters() const {
    return _numberOfVolatileXMMRegisters;
  }

  uint32_t getNumberOfPreservedGPRegisters() const {
    return _numberOfPreservedGPRegisters;
  }
  uint32_t getNumberOfPreservedXMMRegisters() const {
    return _numberOfPreservedXMMRegisters;
  }
};
...
public:
  ...
  virtual TR::Instruction *deallocateFrameIfNeeded(TR::Instruction *cursor,
                                                   int32_t size) {
    return NULL;
  }
  ...
  void mapSingleAutomatic(TR::AutomaticSymbol *p, uint32_t size,
                          uint32_t &stackIndex);

  virtual TR::Instruction *
  copyStackParametersToLinkageRegisters(TR::Instruction *procEntryInstruction) {
  ...
  //
  uint8_t *loadArguments(TR::Node *callNode, uint8_t *cursor,
                         bool calculateSizeOnly, int32_t *sizeOfFlushArea,
                         bool isReturnAddressOnStack);

  uint8_t *storeArguments(TR::Node *callNode, uint8_t *cursor,
                          bool calculateSizeOnly, int32_t *sizeOfFlushArea);

  TR::Instruction *loadArguments(TR::Instruction *prev,
                                 TR::ResolvedMethodSymbol *methodSymbol);

  TR::Instruction *storeArguments(TR::Instruction *prev,
                                  TR::ResolvedMethodSymbol *methodSymbol);
  ...
  virtual void copyGlRegDepsToParameterSymbols(TR::Node *bbStart,
                                               TR::CodeGenerator *cg) {
  ...
  // new instruction.
  //
  uint8_t getMinimumFirstInstructionSize() {
    return _minimumFirstInstructionSize;
  }
  ...
  TR_MovDataTypes movType(TR::DataType type) {
    ...
    case TR::Address:
      return TR::Compiler->target.is64Bit() ? TR_MovDataTypes::Int8
                                            : TR_MovDataTypes::Int4;
    default:
      return Int4;
    }
  }
  ...
  static inline TR_X86OpCodes movOpcodes(TR_MovOperandTypes operandType,
                                         TR_MovDataTypes dataType) {
    TR_ASSERT(TR::Compiler->target.is64Bit() ||
                  dataType != TR_MovDataTypes::Int8,
...
protected:
  ...
  void stopUsingKilledRegisters(TR::RegisterDependencyConditions *deps,
                                TR::Register *returnRegister);
  void associatePreservedRegisters(TR::RegisterDependencyConditions *deps,
                                   TR::Register *returnRegister);
  void coerceFPReturnValueToXMMR(TR::Node *, TR::RegisterDependencyConditions *,
                                 TR::MethodSymbol *, TR::Register *);
  ...
  virtual uint8_t *flushArguments(TR::Node *callNode, uint8_t *cursor,
  ...
  virtual TR::Instruction *
  flushArguments(TR::Instruction *prev, TR::ResolvedMethodSymbol *methodSymbol,
                 bool isReturnAddressOnStack, bool isLoad) {
    // Unless overridden, assume that all parameters are already in the right
    // place, so no extra code needs to be generated to store them.
    return prev;
