...
inline typename TBuffer::cursor_t
TR_X86OpCode::OpCode_t::encode(typename TBuffer::cursor_t cursor,
                               uint8_t rexbits) const
{
  ...
  if (isX87()) {
    ...
    buffer.append(
      (uint8_t)((modrm_opcode << 5) | (modrm_form << 3) | immediate_size));
    return buffer;
  }
  // Prefixes
...
inline void
TR_X86OpCode::OpCode_t::finalize(uint8_t* cursor) const
{
  // Finalize VEX prefix
  switch (*cursor) {
    case 0xC4: {
...
inline void
TR_X86OpCode::CheckAndFinishGroup07(uint8_t* cursor) const
{
  if (info().isGroup07()) {
...
inline uint8_t
TR_X86OpCode::length(uint8_t rex) const
{
  return encode<Estimator>(0, rex);
}
inline uint8_t*
TR_X86OpCode::binary(uint8_t* cursor, uint8_t rex) const
{
...
inline void
TR_X86OpCode::finalize(uint8_t* cursor) const
{
  if (!isPseudoOp())
