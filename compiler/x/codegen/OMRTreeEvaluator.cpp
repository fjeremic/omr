...
TR::Register *OMR::X86::TreeEvaluator::ifxcmpoEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  ...
  bool nodeIs64Bit =
      TR::TreeEvaluator::getNodeIs64Bit(node->getFirstChild(), cg);
  ...
  generateConditionalJumpInstruction(reverseBranch ? JNO4 : JO4, node, cg,
                                     true);
...
TR::Register *OMR::X86::TreeEvaluator::intOrLongClobberEvaluate(
    TR::Node *node, bool nodeIs64Bit, TR::CodeGenerator *cg) {
  // TODO:AMD64: This belongs in CodeGenerator.  In fact, this whole
  // clobberEvaluate interface needs another look.
  if (nodeIs64Bit) {
    TR_ASSERT(TR::TreeEvaluator::getNodeIs64Bit(node, cg),
              "nodeIs64Bit must be consistent with node size");
...
TR::Instruction *OMR::X86::TreeEvaluator::compareGPMemoryToImmediate(
    TR::Node *node, TR::MemoryReference *mr, int32_t value,
    TR::CodeGenerator *cg) {
  ...
  bool is64Bit =
      TR::Compiler->target.is64Bit()
          ? TR::TreeEvaluator::getNodeIs64Bit(node->getFirstChild(), cg)
          : false;
  TR_X86OpCodes cmpOp = (value >= -128 && value <= 127) ? CMPMemImms(is64Bit)
                                                        : CMPMemImm4(is64Bit);
  TR::Instruction *instr =
      generateMemImmInstruction(cmpOp, node, mr, value, cg);
  cg->setImplicitExceptionPoint(instr);
  return instr;
}

void OMR::X86::TreeEvaluator::compareGPRegisterToImmediate(
    TR::Node *node, TR::Register *cmpRegister, int32_t value,
    TR::CodeGenerator *cg) {
  // On IA32, this is called to do half of an 8-byte compare, so even though
  // the node is 64 bit, we should do a 32-bit compare
  bool is64Bit =
      TR::Compiler->target.is64Bit()
          ? TR::TreeEvaluator::getNodeIs64Bit(node->getFirstChild(), cg)
          : false;
  TR_X86OpCodes cmpOp = (value >= -128 && value <= 127) ? CMPRegImms(is64Bit)
                                                        : CMPRegImm4(is64Bit);
...
void OMR::X86::TreeEvaluator::compareGPRegisterToImmediateForEquality(
  ...
  TR_X86OpCodes cmpOp = (value >= -128 && value <= 127) ? CMPRegImms(is64Bit)
                                                        : CMPRegImm4(is64Bit);
  if (value == 0)
    generateRegRegInstruction(TESTRegReg(is64Bit), node, cmpRegister,
                              cmpRegister, cg);
  else
    generateRegImmInstruction(cmpOp, node, cmpRegister, value, cg);
}

TR::Instruction *OMR::X86::TreeEvaluator::insertLoadConstant(
    TR::Node *node, TR::Register *target, intptrj_t value,
    TR_RematerializableTypes type, TR::CodeGenerator *cg,
  ...
  static const TR_X86OpCodes ops[TR_NumRematerializableTypes + 1][3] =
      ...
      {{BADIA32Op, BADIA32Op, BADIA32Op},      // LEA; should not seen here
       ...
       {XOR4RegReg, OR8RegImms,
        BADIA32Op}};  // Long address constant; MOVs handled specially
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    TR_ASSERT(type != TR_RematerializableLong,
              "Longs are rematerialized as pairs of ints on IA32");
  }
  ...
  if (currentInstruction) {
    // Optimized loads inserted arbitrarily into the instruction stream must be
    // checked to ensure they don't modify any eflags needed by surrounding
    // instructions.
    //
    if ((value == 0 || value == -1)) {
      uint8_t EFlags = TR_X86OpCode::getModifiedEFlags(
          ops[opsRow][((value == 0) ? XOR : OR)]);

      if (existsNextInstructionToTestFlags(currentInstruction, EFlags) ||
          cg->requiresCarry()) {
        // Can't alter flags, so must use MOV.  Fall through.
      } else if (value == 0)
        return generateRegRegInstruction(currentInstruction, ops[opsRow][XOR],
                                         target, target, cg);
      else if (value == -1)
        return generateRegImmInstruction(currentInstruction, ops[opsRow][OR],
                                         target, (uint32_t)-1, cg);
    }
    ...
    if (is64Bit) {
      if (cg->constantAddressesCanChangeSize(node) && node &&
          node->getOpCodeValue() == TR::aconst &&
          (node->isClassPointerConstant() || node->isMethodPointerConstant())) {
        movInstruction = generateRegImm64Instruction(MOV8RegImm64, node, target,
                                                     value, cg, reloKind);
      } else if (IS_32BIT_UNSIGNED(value)) {
        // zero-extended 4-byte MOV
        movInstruction = generateRegImmInstruction(
            currentInstruction, MOV4RegImm4, target, value, cg, reloKind);
      } else if (IS_32BIT_SIGNED(value))  // TODO:AMD64: Is there some way we
                                          // could get RIP too?
      {
        movInstruction = generateRegImmInstruction(
            currentInstruction, MOV8RegImm4, target, value, cg, reloKind);
      } else {
        movInstruction = generateRegImm64Instruction(
            currentInstruction, MOV8RegImm64, target, value, cg, reloKind);
      }
    } else {
      movInstruction = generateRegImmInstruction(
          currentInstruction, ops[opsRow][MOV], target, value, cg, reloKind);
    }
        ...
        (cg->fe()->isUnloadAssumptionRequired(
             cg->fe()
                 ->createResolvedMethod(
                     cg->trMemory(), (TR_OpaqueMethodBlock *)node->getAddress(),
                     comp->getCurrentMethod())
                 ->classOfMethod(),
             comp->getCurrentMethod()) ||
         cg->profiledPointersRequireRelocation())) {
    ...
    // constant loads between a compare and a branch cannot clobber the EFLAGS
    // register
    bool canClobberEFLAGS =
        !(cg->getCurrentEvaluationTreeTop()->getNode()->getOpCode().isIf() ||
          cg->requiresCarry());

    if (value == 0 && canClobberEFLAGS) {
      return generateRegRegInstruction(ops[opsRow][XOR], node, target, target,
                                       cg);
    } else if (value == -1 && canClobberEFLAGS) {
      return generateRegImmInstruction(ops[opsRow][OR], node, target,
                                       (uint32_t)-1, cg);
    } else {
      TR::Instruction *movInstruction = NULL;
      if (is64Bit) {
        if (cg->constantAddressesCanChangeSize(node) && node &&
            ...
            (node->isClassPointerConstant() ||
             node->isMethodPointerConstant())) {
          movInstruction = generateRegImm64Instruction(
              MOV8RegImm64, node, target, value, cg, reloKind);
        } else if (IS_32BIT_UNSIGNED(value)) {
          // zero-extended 4-byte MOV
          movInstruction = generateRegImmInstruction(MOV4RegImm4, node, target,
                                                     value, cg, reloKind);
        } else if (IS_32BIT_SIGNED(value))  // TODO:AMD64: Is there some way we
                                            // could get RIP too?
        {
          movInstruction = generateRegImmInstruction(MOV8RegImm4, node, target,
                                                     value, cg, reloKind);
        } else {
          movInstruction = generateRegImm64Instruction(
              MOV8RegImm64, node, target, value, cg, reloKind);
        ...
        movInstruction = generateRegImmInstruction(ops[opsRow][MOV], node,
                                                   target, value, cg, reloKind);
      }
...
TR::Register *OMR::X86::TreeEvaluator::loadConstant(
    TR::Node *node, intptrj_t value, TR_RematerializableTypes type,
    TR::CodeGenerator *cg, TR::Register *targetRegister) {
  if (targetRegister == NULL) {
    targetRegister = cg->allocateRegister();
  }

  TR::Instruction *instr = TR::TreeEvaluator::insertLoadConstant(
      node, targetRegister, value, type, cg);
...
TR::Instruction *OMR::X86::TreeEvaluator::insertLoadMemory(
    TR::Node *node, TR::Register *target, TR::MemoryReference *tempMR,
    TR_RematerializableTypes type, TR::CodeGenerator *cg,
    TR::Instruction *currentInstruction) {
  TR::Compilation *comp = cg->comp();
  ...
  if (type == TR_RematerializableByte && target->getAssignedRealRegister()) {
    TR::RealRegister::RegNum r =
        toRealRegister(target->getAssignedRealRegister())->getRegisterNumber();
    if (r > TR::RealRegister::Last8BitGPR) opCode = MOVZXReg4Mem1;
  }

  TR::Instruction *i;
  if (currentInstruction) {
    i = generateRegMemInstruction(currentInstruction, opCode, target, tempMR,
                                  cg);
  } else {
  ...
  if (node && node->getSymbol()->isStatic() &&
      node->getSymbol()->isClassObject() &&
      cg->wantToPatchClassPointer(NULL, node)) {
...
//
// Also if the unresolved data reference is immediately followed by another
// patchable instruction (such as a virtual guard nop) with a different
// alignment boundary size then there could be an overlap between the two codes
// being patching, unless further padding is inserted.
//
void OMR::X86::TreeEvaluator::padUnresolvedDataReferences(
  ...
  if (TR::Compiler->target.is32Bit())
    padBytes =
        2;  // needs at least 2 bytes as we are patching 8 bytes at a time
  else {
...
TR::Register *OMR::X86::TreeEvaluator::loadMemory(
    TR::Node *node, TR::MemoryReference *sourceMR,
    TR_RematerializableTypes type, bool markImplicitExceptionPoint,
    TR::CodeGenerator *cg) {
  TR::Register *targetRegister = cg->allocateRegister();
  TR::Instruction *instr;
  instr = TR::TreeEvaluator::insertLoadMemory(node, targetRegister, sourceMR,
                                              type, cg);
  ...
  if (markImplicitExceptionPoint) cg->setImplicitExceptionPoint(instr);

  return targetRegister;
}

void OMR::X86::TreeEvaluator::removeLiveDiscardableStatics(
    TR::CodeGenerator *cg) {
  ...
  for (auto iterator = cg->getLiveDiscardableRegisters().begin();
    ...
    if ((*iterator)
            ->getRematerializationInfo()
            ->isRematerializableFromMemory() &&
        (*iterator)
            ->getRematerializationInfo()
            ->getSymbolReference()
            ->getSymbol()
      ...
      if (debug("dumpRemat")) {
        diagnostic(
            "\n---> Deleting static discardable candidate %s at %s "
            "instruction " POINTER_PRINTF_FORMAT,
...
TR::Register *OMR::X86::TreeEvaluator::performIload(
  ...
  TR::Register *reg =
      TR::TreeEvaluator::loadMemory(node, sourceMR, TR_RematerializableInt,
                                    node->getOpCode().isIndirect(), cg);
...
// also handles iaload
TR::Register *OMR::X86::TreeEvaluator::aloadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
  TR::Register *reg =
      TR::TreeEvaluator::loadMemory(node, sourceMR, TR_RematerializableAddress,
                                    node->getOpCode().isIndirect(), cg);
  ...
  if (!node->getSymbolReference()->isUnresolved() &&
    ...
    const char *fieldName =
        node->getSymbolReference()->getOwningMethod(comp)->fieldSignatureChars(
            node->getSymbolReference()->getCPIndex(), len);

    if (fieldName && strstr(fieldName, "Ljava/lang/String;")) {
      generateMemInstruction(PREFETCHT0, node,
                             generateX86MemoryReference(reg, 0, cg), cg);
    }
  }
  ...
  if (node->getSymbolReference()->getSymbol()->isNotCollected()) {
    if (node->getSymbolReference()->getSymbol()->isInternalPointer()) {
      ...
      reg->setPinningArrayPointer(node->getSymbolReference()
                                      ->getSymbol()
...
bool OMR::X86::TreeEvaluator::genNullTestSequence(TR::Node *node,
  ...
  if (comp->useCompressedPointers() && node->containsCompressionSequence()) {
    ...
    if (n->isNonZero()) isNonZero = true;

    if (n->getOpCodeValue() == TR::ladd) {
      if (n->getFirstChild()->isNonZero()) isNonZero = true;

      if (n->getFirstChild()->getOpCodeValue() == TR::iu2l ||
          n->getFirstChild()->getOpCode().isShift()) {
        if (n->getFirstChild()->getFirstChild()->isNonZero()) isNonZero = true;
      }
    }
    ...
    if (!isNonZero) {
      ...
      if (n->getFirstChild()->getOpCode().isShift() &&
          n->getFirstChild()->getFirstChild()->getRegister()) {
...
// also handles iiload
TR::Register *OMR::X86::TreeEvaluator::iloadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
...
// also handles ibload
TR::Register *OMR::X86::TreeEvaluator::bloadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
  TR::Register *reg =
      TR::TreeEvaluator::loadMemory(node, sourceMR, TR_RematerializableByte,
                                    node->getOpCode().isIndirect(), cg);
...
TR::Register *OMR::X86::TreeEvaluator::sloadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *sourceMR = generateX86MemoryReference(node, cg);
  TR::Register *reg =
      TR::TreeEvaluator::loadMemory(node, sourceMR, TR_RematerializableShort,
                                    node->getOpCode().isIndirect(), cg);
...
TR::Register *OMR::X86::TreeEvaluator::fwrtbarEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::fwrtbariEvaluator(
...
TR::Register *OMR::X86::TreeEvaluator::integerStoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *valueChild;
  ...
  if (node->getOpCode().isIndirect()) {
    ...
    if (comp->useCompressedPointers() &&
      ...
      if (isSequence &&
          (translatedNode->getOpCode().isSub() || usingLowMemHeap))
  ...
  if (valueChild->getOpCode().isLoadConst() &&
      valueChild->getRegister() == NULL && !usingCompressedPointers) {
    childIsConstant = true;
  }

  if (childIsConstant &&
      (size <= 4 || IS_32BIT_SIGNED(valueChild->getLongInt()))) {
    ...
    if (!usingCompressedPointers && cg->isMemoryUpdate(node)) {
      if (valueChild->getFirstChild()->getReferenceCount() == 1) {
        ...
        if (comp->getOption(TR_OptimizeForSpace))
          ...
          if (numRegs >= TR::RealRegister::LastAssignableGPR -
                             2)  // -1 for VM thread reg, -1 fudge
            valueChild->setDirectMemoryUpdate(true);
        }
      }
    }
    ...
    if (usingCompressedPointers && !usingLowMemHeap) {
      ...
      //
      generateRegRegInstruction(TESTRegReg(), node, translatedReg,
                                translatedReg, cg);
      generateRegRegInstruction(CMOVERegReg(), node, valueReg, translatedReg,
                                cg);
    } else {
      valueReg = translatedReg;
      if (valueChild->getReferenceCount() == 0) valueRegNeededInFuture = false;
    }
    ...
    if (valueReg) {
      ...
      //
      if (cg->enableRematerialisation() &&
          !valueReg->getRematerializationInfo()) {
        ...
        if (originalValueChild == valueChild) {
          ...
          // This is why we should use TR::DataType in place of
          // TR_RematerializableTypes...
          //
          switch (node->getDataType()) {
            ...
            case TR::Address: {
              if (node && node->getOpCode().hasSymbolReference() &&
                  node->getSymbol() && node->getSymbol()->isClassObject())
                (TR::Compiler->om.generateCompressedObjectHeaders() ||
                 TR::Compiler->target.is32Bit())
                    ? type = TR_RematerializableInt
                    : type = TR_RematerializableLong;
      ...
      TR_ASSERT(valueChild->isDirectMemoryUpdate(),
                "Null valueReg should only occur in direct memory upates");
    }
  }
  ...
  if (tempMR)
    ...
    // Direct Memory Update, we need to dec the first child ref count here,
    // since this was not done by the child evaluator in its direct memory
    // update logic.
  ...
  if (usingCompressedPointers) cg->decReferenceCount(node->getSecondChild());
...
TR::Register *OMR::X86::TreeEvaluator::istoreEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  // TODO:AMD64: Fold this logic into integerStoreEvaluator and share it with
  // AMD64's lstoreEvaluator.
  TR::Node *valueChild;
  TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::X86::TreeEvaluator::bstoreEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerStoreEvaluator(node, cg);
}

// also handles isstore
TR::Register *OMR::X86::TreeEvaluator::sstoreEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerStoreEvaluator(node, cg);
}

// also handles icstore
TR::Register *OMR::X86::TreeEvaluator::cstoreEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerStoreEvaluator(node, cg);
}
...
TR::Register *OMR::X86::TreeEvaluator::arraycmpEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return node->isArrayCmpLen()
             ? TR::TreeEvaluator::SSE2ArraycmpLenEvaluator(node, cg)
             : TR::TreeEvaluator::SSE2ArraycmpEvaluator(node, cg);
}

TR::Register *OMR::X86::TreeEvaluator::SSE2ArraycmpEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  generateRegRegInstruction(SUBRegReg(), node, deltaReg, s2Reg,
                            cg);  // delta = s1 - s2
  ...
  generateRegMemInstruction(MOVUPSRegMem, node, xmm2Reg,
                            generateX86MemoryReference(s2Reg, 0, cg), cg);
  generateRegMemInstruction(MOVUPSRegMem, node, xmm1Reg,
                            generateX86MemoryReference(s2Reg, deltaReg, 0, cg),
                            cg);
  ...
  generateRegMemInstruction(L1RegMem, node, s2ByteVer1Reg,
                            generateX86MemoryReference(s2Reg, 0, cg), cg);
  generateMemRegInstruction(CMP1MemReg, node,
                            generateX86MemoryReference(s2Reg, deltaReg, 0, cg),
                            s2ByteVer1Reg, cg);
  ...
  TR::RegisterDependencyConditions *deps =
      generateRegisterDependencyConditions((uint8_t)0, 8, cg);
...
TR::Register *OMR::X86::TreeEvaluator::SSE2ArraycmpLenEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *s1AddrNode = node->getChild(0);
  TR::Node *s2AddrNode = node->getChild(1);
  TR::Node *lengthNode = node->getChild(2);
  ...
  generateLabelInstruction(LABEL, node, qwordLoop, cg);
  generateRegMemInstruction(MOVUPSRegMem, node, xmm1Reg,
                            generateX86MemoryReference(s1Reg, resultReg, 0, cg),
                            cg);
  generateRegMemInstruction(MOVUPSRegMem, node, xmm2Reg,
                            generateX86MemoryReference(s2Reg, resultReg, 0, cg),
                            cg);
  generateRegRegInstruction(PCMPEQBRegReg, node, xmm1Reg, xmm2Reg, cg);
  generateRegRegInstruction(PMOVMSKB4RegReg, node, equalTestReg, xmm1Reg, cg);
  generateRegImmInstruction(CMPRegImm4(), node, equalTestReg, 0xffff, cg);
  ...
  generateLabelInstruction(LABEL, node, byteLoop, cg);
  generateRegMemInstruction(L1RegMem, node, s2ByteReg,
                            generateX86MemoryReference(s2Reg, resultReg, 0, cg),
                            cg);
  generateMemRegInstruction(CMP1MemReg, node,
                            generateX86MemoryReference(s1Reg, resultReg, 0, cg),
...
bool OMR::X86::TreeEvaluator::stopUsingCopyRegAddr(TR::Node *node,
                                                   TR::Register *&reg,
                                                   TR::CodeGenerator *cg) {
  if (node != NULL) {
...
bool OMR::X86::TreeEvaluator::stopUsingCopyRegInteger(TR::Node *node,
                                                      TR::Register *&reg,
                                                      TR::CodeGenerator *cg) {
  if (node != NULL) {
    reg = cg->evaluate(node);
...
TR::Block *OMR::X86::TreeEvaluator::getOverflowCatchBlock(
    TR::Node *node, TR::CodeGenerator *cg) {
  // make sure the overflowCHK has a catch block first
  TR::Block *overflowCatchBlock = NULL;
  TR::CFGEdgeList &excepSucc = cg->getCurrentEvaluationTreeTop()
                                   ->getEnclosingBlock()
                                   ->getExceptionSuccessors();
  for (auto e = excepSucc.begin(); e != excepSucc.end(); ++e) {
    TR::Block *dest = toBlock((*e)->getTo());
    if (dest->getCatchBlockExtension()->_catchType ==
        TR::Block::CanCatchOverflowCheck)
      overflowCatchBlock = dest;
  }
  TR_ASSERT(overflowCatchBlock != NULL,
            "OverflowChk node %p doesn't have overflow catch block\n", node);

  // the BBStartEvaluator will generate the label but in this case the catch
  // block might not been evaluated yet
  TR::Node *bbstartNode = overflowCatchBlock->getEntry()->getNode();
  TR_ASSERT((bbstartNode->getOpCodeValue() == TR::BBStart),
            "catch block entry %p must be TR::BBStart\n", bbstartNode);
...
void OMR::X86::TreeEvaluator::genArithmeticInstructionsForOverflowCHK(
    TR::Node *node, TR::CodeGenerator *cg) {
  /*
   *overflowCHK
   *   =>child1(Operation)
  ...
  // it is fine that nodeIs64Bit is false for long operand on 32bits platform
  // because the analyzers below don't use *op* in this case anyways
  bool nodeIs64Bit = TR::Compiler->target.is32Bit()
                         ? false
                         : TR::TreeEvaluator::getNodeIs64Bit(operand1, cg);
  switch (node->getOverflowCheckOperation()) {
    ...
    case TR::imul:
      ...
      break;
    case TR::lmul:
      // TODO: leaving lmul overflowCHK on 32bit platform for later since there
      // is no pending demand for this.
      // the 32 bits lmul needs several instructions including to
      // multiplications between the lower and higher parts of the registers and
      // additions of the intermediate results. See
      // TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser Therefore the
      // usual way of only detecting the OF for the last instruction of sequence
      // won't work for this case. The implementation needs to detect OF flags
      // after all the instructions involving higher parts of the registers for
      // both operands and intermediate results.
      TR_ASSERT(TR::Compiler->target.is64Bit(),
                "overflowCHK for lmul on 32 bits is not currently supported\n");
      op = IMULRegReg(nodeIs64Bit);
      break;
    default:
  ...
  bool operationChildEvaluatedAlready =
      operationNode->getRegister() ? true : false;
  ...
  // we need to do the operation again when the Operation node has been
  // evaluated already under a different treetop
  // TODO: there is still a chance that the flags might still be available and
  // we could detect it and avoid repeating the operation
  {
    ...
    switch (node->getOverflowCheckOperation()) {
      ...
      case TR::iadd:
        addMulAnalyser.integerAddAnalyserWithExplicitOperands(
            node, operand1, operand2, op, BADIA32Op, needsEflags);
        break;
      case TR::ladd:
        TR::Compiler->target.is32Bit()
            ? addMulAnalyser.longAddAnalyserWithExplicitOperands(node, operand1,
                                                                 operand2)
            : addMulAnalyser.integerAddAnalyserWithExplicitOperands(
                  node, operand1, operand2, op, BADIA32Op, needsEflags);
        break;
      ...
      case TR::bsub:
        subAnalyser.integerSubtractAnalyserWithExplicitOperands(
            node, operand1, operand2, op, BADIA32Op, MOV1RegReg, needsEflags);
        break;
      case TR::ssub:
      case TR::isub:
        subAnalyser.integerSubtractAnalyserWithExplicitOperands(
            node, operand1, operand2, op, BADIA32Op, MOV4RegReg, needsEflags);
        break;
      case TR::lsub:
        TR::Compiler->target.is32Bit()
            ? subAnalyser.longSubtractAnalyserWithExplicitOperands(
                  node, operand1, operand2)
            : subAnalyser.integerSubtractAnalyserWithExplicitOperands(
                  node, operand1, operand2, op, BADIA32Op, MOV8RegReg,
                  needsEflags);
        break;
      // mul group
      case TR::imul:
        addMulAnalyser.genericAnalyserWithExplicitOperands(
            node, operand1, operand2, op, BADIA32Op, MOV4RegReg);
        break;
      case TR::lmul:
        addMulAnalyser.genericAnalyserWithExplicitOperands(
            node, operand1, operand2, op, BADIA32Op, MOV8RegReg);
...
TR::Register *OMR::X86::TreeEvaluator::overflowCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR_X86OpCodes opcode;
  ...
  TR::Block *overflowCatchBlock =
      TR::TreeEvaluator::getOverflowCatchBlock(node, cg);
  TR::TreeEvaluator::genArithmeticInstructionsForOverflowCHK(node, cg);
  generateLabelInstruction(
      opcode, node, overflowCatchBlock->getEntry()->getNode()->getLabel(), cg);
  cg->decReferenceCount(node->getFirstChild());
  return NULL;
}

static void generateRepMovsInstruction(
    TR_X86OpCodes repmovs, TR::Node *node, TR::Register *sizeRegister,
 ...
 *     The code generator
 */
static void arrayCopy64BitPrimitiveOnIA32(TR::Node *node, TR::Register *dstReg,
                                          TR::Register *srcReg,
                                          TR::Register *sizeReg,
                                          TR::CodeGenerator *cg) {
  ...
  TR::RegisterDependencyConditions *dependencies =
      generateRegisterDependencyConditions((uint8_t)0, (uint8_t)5, cg);
  ...
  generateRegMemInstruction(MOVQRegMem, node, XMM,
                            generateX86MemoryReference(srcReg, 0, cg), cg);
  generateMemRegInstruction(MOVQMemReg, node,
                            generateX86MemoryReference(dstReg, srcReg, 0, cg),
                            XMM, cg);
  generateRegMemInstruction(LEARegMem(), node, srcReg,
                            generateX86MemoryReference(srcReg, scratch, 3, cg),
                            cg);
 ...
 *     The code generator
 */
static void arrayCopy16BitPrimitive(TR::Node *node, TR::Register *dstReg,
                                    TR::Register *srcReg, TR::Register *sizeReg,
                                    TR::CodeGenerator *cg) {
  TR::RegisterDependencyConditions *dependencies =
      generateRegisterDependencyConditions((uint8_t)3, (uint8_t)3, cg);
  ...
  if (node->isForwardArrayCopy()) {
    ...
    generateRegMemInstruction(L2RegMem, node, sizeReg,
                              generateX86MemoryReference(srcReg, 0, cg), cg);
    generateMemRegInstruction(
        S2MemReg, node, generateX86MemoryReference(dstReg, 0, cg), sizeReg, cg);
  } else  // decide direction during runtime
  {
    TR::LabelSymbol *backwardLabel = generateLabelSymbol(cg);

    generateRegRegInstruction(SUBRegReg(), node, dstReg, srcReg,
                              cg);  // dst = dst - src
    generateRegRegInstruction(CMPRegReg(), node, dstReg, sizeReg,
                              cg);  // cmp dst, size
    generateRegMemInstruction(LEARegMem(), node, dstReg,
                              generateX86MemoryReference(dstReg, srcReg, 0, cg),
                              cg);  // dst = dst + src
    generateLabelInstruction(JB4, node, backwardLabel,
                             cg);  // jb, skip backward copy setup
 ...
 *     The code generator
 */
static void arrayCopyDefault(TR::Node *node, uint8_t elementSize,
                             TR::Register *dstReg, TR::Register *srcReg,
  ...
  } else  // decide direction during runtime
  {
...

    {
      ...
      generateRegMemInstruction(
          LEARegMem(), node, srcReg,
          generateX86MemoryReference(srcReg, sizeReg, 0, -(intptr_t)elementSize,
                                     cg),
          cg);
      generateRegMemInstruction(
          LEARegMem(), node, dstReg,
          generateX86MemoryReference(dstReg, sizeReg, 0, -(intptr_t)elementSize,
                                     cg),
          cg);
      generateInstruction(STD, node, cg);
      generateRepMovsInstruction(repmovs, node, sizeReg, NULL, cg);
 ...
 *
 */
static void generateArrayElementStore(TR::Node *node, TR::Register *addressReg,
                                      int32_t index, TR::Register *valueReg,
  ...
  if (valueReg->getKind() == TR_FPR) {
    switch (size) {
      ...
      case 16:
        ...
        break;
      default:
        TR_ASSERT(0, "Unsupported size in generateArrayElementStore, size: %d",
                  size);
        break;
    }
  ...
  generateMemRegInstruction(storeOpcode, node,
                            generateX86MemoryReference(addressReg, index, cg),
                            valueReg, cg);
}
 ...
 *     The code generator
 */
static void generateArrayElementLoad(TR::Node *node, TR::Register *valueReg,
                                     uint8_t size, TR::Register *addressReg,
  ...
  if (valueReg->getKind() == TR_FPR) {
    switch (size) {
      ...
      case 16:
        ...
        break;
      default:
        TR_ASSERT(0, "Unsupported size in generateArrayElementLoad, size: %d",
                  size);
        break;
    }
  } else if (valueReg->getKind() == TR_GPR) {
  ...
  generateRegMemInstruction(loadOpCode, node, valueReg,
                            generateX86MemoryReference(addressReg, index, cg),
                            cg);
}

/** \brief
 *    Generate instructions to do arraycopy for a short constant array. We try
 * to copy as many elements as we can every time.
 *
 *  \param node
 *     The tree node
 *
...
static void arraycopyForShortConstArrayWithDirection(TR::Node *node,
                                                     TR::Register *dstReg,
                                                     TR::Register *srcReg,
                                                     uint32_t size,
                                                     TR::CodeGenerator *cg) {
  ...
  while (size != 0) {
    if (size >= regSize[i]) {
      ...
      generateArrayElementLoad(node, tempReg, regSize[i], srcReg,
                               totalSize - size, cg);
      generateArrayElementStore(node, dstReg, totalSize - size, tempReg,
                                regSize[i], cg);
      size -= regSize[i];
    } else {
      i++;
...
/** \brief
 *    Generate instructions to do arraycopy for a short constant array. We need
 * to copy the source array to registers then store the value back to
 * destination array.
 *
 *  \param node
 *     The tree node
 ...
 *     The code generator
 */
static void arraycopyForShortConstArrayWithoutDirection(TR::Node *node,
                                                        TR::Register *dstReg,
                                                        TR::Register *srcReg,
                                                        uint32_t size,
                                                        TR::CodeGenerator *cg) {
  ...
  {
    generateArrayElementLoad(node, srcReg, residue, srcReg, totalSize - residue,
                             cg);
    reg1 = srcReg;
    residueCase = 1;
  ...
  } else if (residue != 0)  // 1 gpr + 1 xmm
  {
    ...
    generateArrayElementLoad(node, reg2, secondLoadSizeForCase4, srcReg,
                             residue - secondLoadSizeForCase4, cg);
    residueCase = 4;
  }
  ...
  switch (residueCase) {
    case 0:
      break;
    case 1:
      generateArrayElementStore(node, dstReg, totalSize - residue, reg1,
                                residue, cg);
      break;
    case 2:
    ...
    case 3:
      ...
      break;
    case 4:
      generateArrayElementStore(node, dstReg, 0, reg1, firstLoadSizeForCase4,
                                cg);
      generateArrayElementStore(node, dstReg,
                                totalSize - secondLoadSizeForCase4, reg2,
                                secondLoadSizeForCase4, cg);
      cg->stopUsingRegister(reg1);
      break;
  }
}

TR::Register *OMR::X86::TreeEvaluator::arraycopyEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // There are two variants of TR::arraycopy: one has 5 children, the other has
  // 3 children. Details can be found from compiler/il/ILOpCodeProperties.hpp
  //
  // In most, if not all, cases, the 5-child variant requires language specific
  // semantics, which OMR is not aware of. The fact that a 5-child arraycopy is
  // generated indicates at least one of the first two children must be needed
  // when performing the copy; otherwise a 3-child arraycopy should be generated
  // instead. Interpreting the meanings of the first two children definitely
  // requires language specific semantics. For example, the first two children
  // may be for dealing with an arraycopy where the Garbage Collector may need
  // to be notified about the copy or something to that affect.
  //
  // Therefore, this OMR evaluator only handles the 3-child variant, is an
  // operation equivalent to C++'s std::memmove(). Should a downstream project
  // need the 5-child variant evaluator, it needs to override this evaluator in
  // its own TreeEvaluator, and delegates the 3-child variant back to this
  // evaluator by calling OMR::TreeEvaluatorConnector::arraycopyEvaluator.

  TR_ASSERT_FATAL(node->getNumChildren() == 3,
                  "This evaluator is for the 3-child variant of arraycopy.");

  // ALL nodes need be evaluated to comply argument evaluation order;
  // since size node is the last node, its evaluation can be delayed for further
  // optimization
  ...
  TR::Node *sizeNode =
      node->getChild(2);  // the size of memory to copy, in bytes
  ...
  static bool forceByteArrayElementCopy =
      feGetEnv("TR_ForceByteArrayElementCopy");
  ...
  static bool optimizeForConstantLengthArrayCopy =
      feGetEnv("TR_OptimizeForConstantLengthArrayCopy");
  static bool ignoreDirectionForConstantLengthArrayCopy =
      feGetEnv("TR_IgnoreDirectionForConstantLengthArrayCopy");
...
#define shortConstArrayWithoutDirThreshold 16 * 4
  ...
  if (isShortConstArrayWithDirection) {
    ...
    TR::Register *sizeReg =
        cg->gprClobberEvaluate(sizeNode, MOVRegReg(isSize64Bit));
...
TR::Register *OMR::X86::TreeEvaluator::arraytranslateEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  //
  ...
  //    terminal character (dummy when src is byte and dest is word, otherwise,
  //    it's a mask) input length (in elements) stopping char (dummy for X)
  ...
  bool stopUsingCopyReg1 =
      TR::TreeEvaluator::stopUsingCopyRegAddr(node->getChild(0), srcPtrReg, cg);
  bool stopUsingCopyReg2 =
      TR::TreeEvaluator::stopUsingCopyRegAddr(node->getChild(1), dstPtrReg, cg);
  bool stopUsingCopyReg4 = TR::TreeEvaluator::stopUsingCopyRegInteger(
      node->getChild(3), termCharReg, cg);
  bool stopUsingCopyReg5 = TR::TreeEvaluator::stopUsingCopyRegInteger(
      node->getChild(4), lengthReg, cg);
  ...
  if (sourceByte && (node->getChild(3)->getOpCodeValue() == TR::iconst) &&
  ...
  if (sourceByte) {
    ...
    if (arraytranslateOT) {
      helper = TR::Compiler->target.is64Bit() ? TR_AMD64arrayTranslateTROT
                                              : TR_IA32arrayTranslateTROT;
      dependencies->addPostCondition(termCharReg, TR::RealRegister::edx, cg);
    } else
      helper = TR::Compiler->target.is64Bit()
                   ? TR_AMD64arrayTranslateTROTNoBreak
                   : TR_IA32arrayTranslateTROTNoBreak;
  } else {
    TR_ASSERT(node->isTargetByteArrayTranslate(),
              "Both source and target are word for array translate");
    helper = TR::Compiler->target.is64Bit() ? TR_AMD64arrayTranslateTRTO
...
static void packUsingShift(TR::Node *node, TR::Register *tempReg,
                           TR::Register *sourceReg, int32_t size,
                           TR::CodeGenerator *cg) {
...
static void packXMMWithMultipleValues(TR::Node *node, TR::Register *XMMReg,
                                      TR::Register *sourceReg, int8_t size,
                                      TR::CodeGenerator *cg) {
  switch (size) {
    case 1:
    case 2: {
      ...
      auto snippet = generateX86MemoryReference(
          cg->findOrCreate16ByteConstant(
              node, size == 1 ? MASKOFSIZEONE : MASKOFSIZETWO),
          cg);
    ...
    case 8:
      generateRegRegInstruction(MOVQRegReg8, node, XMMReg, sourceReg, cg);
      generateRegRegImmInstruction(PSHUFDRegRegImm1, node, XMMReg, XMMReg,
                                   size == 4 ? 0x00 : 0x44, cg);
      break;
    default:
...
static void arraySetToZeroForShortConstantArrays(TR::Node *node,
  ...
  if (size < 16) {
    ...
    for (int32_t i = 0; i < 4; i++) {
      ...
      for (int32_t j = 0; j < moves; j++) {
        generateArrayElementStore(node, addressReg, index, tempReg, packs[i],
                                  cg);
        index += packs[i];
      }
      size = size % packs[i];
    ...
    if (size % 16 != 0)
      generateArrayElementStore(node, addressReg, size - 16, tempReg, 16, cg);
  }
  cg->stopUsingRegister(tempReg);
}

static void arraySetForShortConstantArrays(TR::Node *node, uint8_t elementSize,
                                           TR::Register *addressReg,
  ...
  //       just a single move.
  // Else we pack an XMM first, and we handle the reminder using a "shifting
  // window" method
  //    e.g. array size is 17 byte, element size is 1 byte, here is the
  //    psudocode:
  ...
  if (totalSize < 16) {
    ...
    for (int32_t i = 1; i < 5; i++) {
      for (int32_t j = 0; j < moves[i]; j++) {
        generateArrayElementStore(node, addressReg, index, currentReg, packs[i],
                                  cg);
        index += packs[i];
      }
    }
  } else {
    // First check if we are able to do it by just doing equal or less than
    // notWorthPacking movs of the valueReg
    if (size < notWorthPacking) {
      for (int32_t i = 0; i < size; i++) {
        generateArrayElementStore(node, addressReg, i * elementSize, valueReg,
                                  elementSize, cg);
      }
    } else {
      TR::Register *XMM = cg->allocateRegister(TR_FPR);
      ...
      if (reminder == elementSize) {
        generateArrayElementStore(node, addressReg, moves[0] * 16, valueReg,
                                  elementSize, cg);
      } else if (reminder != 0) {
        generateArrayElementStore(node, addressReg, totalSize - 16, XMM, 16,
                                  cg);
      }
      cg->stopUsingRegister(XMM);
    }
...
static void arraySet64BitPrimitiveOnIA32(TR::Node *node,
                                         TR::Register *addressReg,
                                         TR::Register *valueReg,
                                         TR::Register *sizeReg,
                                         TR::CodeGenerator *cg) {
  ...
  TR::RegisterDependencyConditions *deps =
      generateRegisterDependencyConditions((uint8_t)0, (uint8_t)2, cg);
  ...
  switch (valueReg->getKind()) {
    case TR_GPR:
      generateRegRegInstruction(MOVDRegReg4, node, XMM,
                                valueReg->getHighOrder(), cg);
      generateRegImmInstruction(PSLLQRegImm1, node, XMM, 32, cg);
      generateRegRegInstruction(MOVDRegReg4, node, XMM, valueReg->getLowOrder(),
                                cg);
      break;
    case TR_FPR:
...
static void arraySetDefault(TR::Node *node, uint8_t elementSize,
                            TR::Register *addressReg, TR::Register *valueReg,
                            TR::Register *sizeReg, TR::CodeGenerator *cg) {
  ...
  if (shiftAmount)
    generateRegImmInstruction(SHRRegImm1(), node, sizeReg, shiftAmount, cg);
  generateInstruction(repOpcode, node, stosDependencies, cg);
  cg->stopUsingRegister(EAX);
}

TR::Register *OMR::X86::TreeEvaluator::arraysetEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // arrayset
  //    address
  //    fill value
  ...
  TR::Register *addressReg = TR::TreeEvaluator::intOrLongClobberEvaluate(
      addressNode, TR::Compiler->target.is64Bit(), cg);
  uintptrj_t size;
  ...
  static bool isConstArraysetEnabled =
      (NULL == feGetEnv("TR_DisableConstArrayset"));
  if (isConstArraysetEnabled && cg->getX86ProcessorInfo().supportsSSSE3() &&
      TR::Compiler->target.is64Bit()) {
    if (valueNode->getOpCode().isLoadConst() &&
        !valueNode->getOpCode().isFloat() &&
    ...
    static char *optimizedArrayLengthStr =
        feGetEnv("TR_ConstArraySetOptLength");
    int32_t optimizedArrayLength =
        optimizedArrayLengthStr ? atoi(optimizedArrayLengthStr) : 256;
  ...
  if (isShortConstantArrayWithZero) {
    ...
    if (valueNode->getOpCode().isFloat() || valueNode->getOpCode().isDouble()) {
      ...
      TR_ASSERT(tempReg->getKind() == TR_FPR,
                "Float and Double must be in an XMM register");
      valueReg = cg->allocateRegister();
      generateRegRegInstruction(elementSize == 8 ? MOVQReg8Reg : MOVDReg4Reg,
    ...
    arraySetForShortConstantArrays(node, elementSize, addressReg, valueReg,
                                   size / elementSize, cg);
    cg->recursivelyDecReferenceCount(sizeNode);
    ...
    if (TR::Compiler->target.is64Bit() &&
        !TR::TreeEvaluator::getNodeIs64Bit(sizeNode, cg)) {
      generateRegRegInstruction(MOVZXReg8Reg4, node, sizeReg, sizeReg, cg);
    }
    if (elementSize == 8 && TR::Compiler->target.is32Bit()) {
...
bool OMR::X86::TreeEvaluator::constNodeValueIs32BitSigned(
    TR::Node *node, intptrj_t *value, TR::CodeGenerator *cg) {
...
bool OMR::X86::TreeEvaluator::getNodeIs64Bit(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  /* This function is intended to allow existing 32-bit instruction selection
   * code to be reused, almost unchanged, to do the corresponding 64-bit logic
   * on AMD64. It compiles away to nothing on IA32, thus preserving performance
   * and code size on IA32, while allowing the logic to be generalized to suit
   * AMD64.
   *
   * Don't use this function for 64-bit logic on IA32; instead, either (1) use
   * separate logic, or (2) use a different test for 64-bitness.  Usually this
   * is not a hinderance, because 64-bit code on IA32 uses register pairs and
   * other things that are totally different from their 32-bit counterparts.
   */
  TR_ASSERT(TR::Compiler->target.is64Bit() || node->getSize() <= 4,
            "64-bit nodes on 32-bit platforms shouldn't use getNodeIs64Bit");
  return TR::Compiler->target.is64Bit() && node->getSize() > 4;
}

intptrj_t OMR::X86::TreeEvaluator::integerConstNodeValue(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::TreeEvaluator::getNodeIs64Bit(node, cg)) {
    return (intptrj_t)
        node->getLongInt();  // Cast to satisfy 32-bit compilers, even though
                             // they never take this path
  } else {
    TR_ASSERT(node->getSize() <= 4,
              "For efficiency on IA32, only call integerConstNodeValue for "
              "32-bit constants");
    return node->getInt();
  }
}

TR::Register *OMR::X86::TreeEvaluator::performCall(TR::Node *node,
                                                   bool isIndirect,
                                                   bool spillFPRegs,
                                                   TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::performHelperCall(
    TR::Node *node, TR::SymbolReference *helperSymRef,
    TR::ILOpCodes helperCallOpCode, bool spillFPRegs, TR::CodeGenerator *cg) {
  TR::ILOpCodes opCode = node->getOpCodeValue();
  TR::Node::recreate(node, helperCallOpCode);
  if (helperSymRef) node->setSymbolReference(helperSymRef);
  TR::Register *targetReg =
      TR::TreeEvaluator::performCall(node, false, spillFPRegs, cg);
 ...
 */
static TR::Register *inlineSinglePrecisionSQRT(TR::Node *node,
                                               TR::CodeGenerator *cg) {
  ...
  if (opRegister->getKind() == TR_FPR) {
    ...
    generateRegRegInstruction(SQRTSSRegReg, node, targetRegister, opRegister,
                              cg);
  } else {
    ...
    if (targetRegister) {
      if (targetRegister->needsPrecisionAdjustment() ||
          targetRegister->mayNeedPrecisionAdjustment()) {
        TR::TreeEvaluator::insertPrecisionAdjustment(targetRegister, operand,
                                                     cg);
      }
  ...
  if (firstChild) cg->recursivelyDecReferenceCount(firstChild);
 ...
 *     The code generator
 */
static TR::Register *inlineAtomicMemoryUpdate(TR::Node *node, TR_X86OpCodes op,
                                              TR::CodeGenerator *cg) {
  TR_ASSERT((!TR_X86OpCode(op).hasLongSource() &&
             !TR_X86OpCode(op).hasLongTarget()) ||
                TR::Compiler->target.is64Bit(),
  ...
  generateMemRegInstruction(
      op, node, generateX86MemoryReference(address, 0, cg), value, cg);
...
/** \brief
 *    Generate instructions to do atomic compare and 64-bit memory update on
 * X86-32.
 *
 *  \param node
 *     The tree node
 *
 *  \param returnValue
 *     Indicate whether the result is the old memory value or the status of
 * memory update
 *
 *  \param cg
 *     The code generator
...
static TR::Register *inline64BitAtomicCompareAndMemoryUpdateOn32Bit(
  ...
  generateMemInstruction(LCMPXCHG8BMem, node,
                         generateX86MemoryReference(address, 0, cg), deps, cg);
  if (!returnValue) {
 ...
 *     The code generator
 */
static TR::Register *inlineAtomicCompareAndMemoryUpdate(TR::Node *node,
                                                        bool returnValue,
                                                        TR::CodeGenerator *cg) {
  bool isNode64Bit = node->getChild(1)->getDataType().isInt64();
  if (TR::Compiler->target.is32Bit() && isNode64Bit) {
    return inline64BitAtomicCompareAndMemoryUpdateOn32Bit(node, returnValue,
                                                          cg);
  }

  TR::Register *address = cg->evaluate(node->getChild(0));
  TR::Register *oldvalue =
      cg->gprClobberEvaluate(node->getChild(1), MOVRegReg());
  TR::Register *newvalue = cg->evaluate(node->getChild(2));
  ...
  generateMemRegInstruction(LCMPXCHGMemReg(isNode64Bit), node,
                            generateX86MemoryReference(address, 0, cg),
                            newvalue, deps, cg);
  if (!returnValue) {
    generateRegInstruction(SETE1Reg, node, oldvalue, cg);
    generateRegRegInstruction(MOVZXReg4Reg1, node, oldvalue, oldvalue, cg);
...
// directCallEvaluator
TR::Register *OMR::X86::TreeEvaluator::directCallEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::SymbolReference *SymRef = node->getSymbolReference();

  if (comp->getSymRefTab()->isNonHelper(
          SymRef, TR::SymbolReferenceTable::singlePrecisionSQRTSymbol)) {
  ...
  if (SymRef && SymRef->getSymbol()->castToMethodSymbol()->isInlinedByCG()) {
    ...
    if (comp->getSymRefTab()->isNonHelper(
      ...
      op = node->getChild(1)->getDataType().isInt32() ? LADD4MemReg
                                                      : LADD8MemReg;
    } else if (comp->getSymRefTab()->isNonHelper(
                   SymRef, TR::SymbolReferenceTable::atomicFetchAndAddSymbol)) {
      op = node->getChild(1)->getDataType().isInt32() ? LXADD4MemReg
                                                      : LXADD8MemReg;
    } else if (comp->getSymRefTab()->isNonHelper(
                   SymRef,
      ...
      op = node->getChild(1)->getDataType().isInt32() ? XCHG4MemReg
                                                      : XCHG8MemReg;
    } else if (comp->getSymRefTab()->isNonHelper(
...
TR::Register *OMR::X86::TreeEvaluator::indirectCallEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // If the method to be called is marked as an inline method, see if it can
  // actually be generated inline.
  //
...
TR::Register *OMR::X86::TreeEvaluator::treetopEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
...
void OMR::X86::TreeEvaluator::compareGPRegisterToConstantForEquality(
    TR::Node *node, int32_t value, TR::Register *cmpRegister,
    TR::CodeGenerator *cg) {
  if (value == 0) {
    generateRegRegInstruction(TEST4RegReg, node, cmpRegister, cmpRegister, cg);
  } else {
    TR::TreeEvaluator::compareGPRegisterToImmediate(node, cmpRegister, value,
                                                    cg);
  }
}

TR::Register *OMR::X86::TreeEvaluator::fenceEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  ...
  if (fenceOp.getOpCodeValue() != BADIA32Op) {
    new (cg->trHeapMemory())
        TR::Instruction(node, fenceOp.getOpCodeValue(), cg);
  } else {
    TR::trap();
  }
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::generateLEAForLoadAddr(
    TR::Node *node, TR::MemoryReference *memRef, TR::SymbolReference *symRef,
    TR::CodeGenerator *cg, bool isInternalPointer) {
  TR::Register *targetRegister;

  TR::Compilation *comp = cg->comp();

  // memRef maybe not directly generated from symRef, for example symRef plus
  // second child in analyzeLea
  ...
  // TODO:AMD64: This often generates mov r1,imm64 followed by lea r1,[r1] which
  // is dumb
  TR_X86OpCodes op = LEARegMem();
  if (TR::Compiler->om.generateCompressedObjectHeaders() && (node->getSymbol()
                                                                 ->isClassObject() /*|| node->getSymbol()->isAddressOfClassObject()*/))
    op = LEA4RegMem;

  TR::Instruction *instr =
      generateRegMemInstruction(op, node, targetRegister, memRef, cg);
  memRef->decNodeReferenceCounts(cg);
  // HCR register PIC site in generateLEAForLoadAddr
  if (node && node->getSymbol()->isClassObject() &&
      cg->wantToPatchClassPointer(NULL, node)) {
...
TR::Register *OMR::X86::TreeEvaluator::loadaddrEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::SymbolReference *symRef = node->getSymbolReference();
  TR::MemoryReference *memRef = generateX86MemoryReference(symRef, cg);

  // for loadaddr, directly allocated register according to its symRef, since
  // memRef only represents symRef
  TR::Register *targetRegister = TR::TreeEvaluator::generateLEAForLoadAddr(
      node, memRef, symRef, cg, false);

  if (symRef->isUnresolved() && TR::Compiler->target.is32Bit()) {
    TR::TreeEvaluator::padUnresolvedDataReferences(
        node, memRef->getSymbolReference(), cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::aRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::integerRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *globalReg = node->getRegister();
  TR::Compilation *comp = cg->comp();
  if (globalReg == NULL) {
  ...
  // Everything that can put a value in a global reg (iRegStore, method
  // parameters) zeroes out the upper bits.
  //
  if (TR::Compiler->target.is64Bit() &&
      node->getOpCodeValue() == TR::iRegLoad &&
...
TR::Register *OMR::X86::TreeEvaluator::iRegStoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *globalReg = cg->evaluate(child);

  if (TR::Compiler->target.is64Bit() && node->getDataType() == TR::Int32) {
    // We disregard needsSignExtension here intentionally.  By always
    // zero-extending at iRegStores, we are free to setUpperBitsAreZero at every
    // iRegLoad, and lots of goodness results.
    //
    // In theory, this also means we must disregard skipSignExtension at i2l
    // nodes. See i2lEvaluator in AMD64TreeEvaluator.cpp for more information.
    //
...
TR::Register *OMR::X86::TreeEvaluator::lRegStoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *globalReg = cg->evaluate(child);
  cg->decReferenceCount(child);
...
TR::Register *OMR::X86::TreeEvaluator::GlRegDepsEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::passThroughEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if ((child->getReferenceCount() > 1 &&
    ...
    if (srcReg->getRegisterPair()) {
      ...
      generateRegRegInstruction(MOVRegReg(), node, copyReg,
                                srcReg->getHighOrder(), cg);
      generateRegRegInstruction(MOVRegReg(), node, copyRegLow,
                                srcReg->getLowOrder(), cg);
...
TR::Register *OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  ...
  if (!block->isExtensionOfPreviousBlock()) {
    ...
    static bool alwaysAlignLoops =
        (feGetEnv("TR_AlwaysAlignLoopEntries") != NULL);
    if (doAlign && !block->isCold() && block->firstBlockInLoop() &&
        (comp->getOptLevel() > warm || alwaysAlignLoops)) {
      generateAlignmentInstruction(
          node, 16, cg);  // TODO: Derive alignment from CPU cache information
    }

    if (node->getNumChildren() > 0)
      inst = generateLabelInstruction(LABEL, node, label, node->getFirstChild(),
                                      &popRegisters, cg);
    else
  ...
  TR::Instruction *fence = generateFenceInstruction(
      FENCE, node,
      TR::Node::createRelative32BitFenceNode(
          node, &block->getInstructionBoundaries()._startPC),
      cg);

  if (!block->getFirstInstruction()) block->setFirstInstruction(fence);

  if (comp->getOption(TR_BreakBBStart)) {
    TR::Machine *machine = cg->machine();
    generateRegImmInstruction(TEST4RegImm4, node,
                              machine->getRealRegister(TR::RealRegister::esp),
                              block->getNumber(), cg);
    generateInstruction(BADIA32Op, node, cg);
  }

  cg->generateDebugCounter((node->getBlock()->isExtensionOfPreviousBlock())
                               ? "cg.blocks/extensions"
                               : "cg.blocks",
                           1, TR::DebugCounter::Exorbitant);

  if (!popRegisters.isEmpty()) {
    ListIterator<TR::Register> popRegsIt(&popRegisters);
    for (TR::Register *popRegister = popRegsIt.getFirst(); popRegister != NULL;
         popRegister = popRegsIt.getNext()) {
      generateFPSTiST0RegRegInstruction(FSTRegReg, node, popRegister,
                                        popRegister, cg);
...
TR::Register *OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  ...
  TR::X86FenceInstruction *instr = generateFenceInstruction(
      FENCE, node,
      TR::Node::createRelative32BitFenceNode(
  ...
  if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
    ...
    // This label is also used by RegisterDependency to detect the end of a
    // block.
    TR::Instruction *labelInst = NULL;
    if (node->getNumChildren() > 0)
      labelInst = generateLabelInstruction(LABEL, node, generateLabelSymbol(cg),
                                           node->getFirstChild(), NULL, cg);
    else
      labelInst =
...
TR::Register *OMR::X86::TreeEvaluator::conversionAnalyser(
    TR::Node *node, TR_X86OpCodes memoryToRegisterOp,
    TR_X86OpCodes registerToRegisterOp, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getReferenceCount() > 1) {
    ...
    if (node->parentSupportsLazyClobber() &&
        ...
        performTransformation(
            comp, "O^O LAZY CLOBBERING: reuse register %s from %s for %s\n",
            cg->getDebug()->getName(sourceRegister),
            cg->getDebug()->getName(child), cg->getDebug()->getName(node))) {
      ...
      // removal of sign/zero-extension in array accesses in JCL methods where
      // we don't emit bound checks.
      //
    ...
    if (child->getRegister() == NULL &&
      ...
      // iu2l
      //   iiload  <- where this node was materialized by lowering an iaload in
      //   compressedPointers mode
      //
      if (node->getOpCodeValue() == TR::iu2l && comp->useCompressedPointers() &&
          child->getOpCode().isLoadIndirect() &&
          child->getSymbolReference()->getSymbol()->getDataType() ==
              TR::Address) {
        targetRegister = cg->evaluate(child);
      } else {
        TR::MemoryReference *tempMR = generateX86MemoryReference(child, cg);
        targetRegister = cg->allocateRegister();
        generateRegMemInstruction(memoryToRegisterOp, node, targetRegister,
                                  tempMR, cg);
        tempMR->decNodeReferenceCounts(cg);
      }
    } else {
  ...
  if (sourceRegister) {
    if (!((sourceRegister == targetRegister) &&
          (registerToRegisterOp == MOVZXReg8Reg4) &&
          (sourceRegister->areUpperBitsZero())))
      generateRegRegInstruction(registerToRegisterOp, node, targetRegister,
                                sourceRegister, cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::NOPEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::sbyteswapEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(node->getNumChildren() == 1,
            "Wrong number of children in byteswapEvaluator");
  TR::Node *child = node->getFirstChild();
...
TR::Register *OMR::X86::TreeEvaluator::icmpsetEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  ...
  // - the current node cannot be used because both icmpset and lcmpset always
  // return an integer sized result
  // - the pointer node cannot be used because on x64 the pointer is 64 bit for
  // both icmpset and lcmpset but
  ...
  TR::Register *compareReg = TR::TreeEvaluator::intOrLongClobberEvaluate(
      compareValue, nodeIs64Bit, cg);
  TR::Register *replaceReg = cg->evaluate(replaceValue);

  // zero the result register to avoid zero extension after the setne
  TR::Register *resultReg = cg->allocateRegister();

  // an XOR4RegReg clears the top half of the register on x64 so it can be used
  // in all cases
  ...
  generateMemRegInstruction(TR::Compiler->target.isSMP()
                                ? LCMPXCHGMemReg(nodeIs64Bit)
                                : CMPXCHGMemReg(nodeIs64Bit),
...
TR::Register *OMR::X86::TreeEvaluator::bztestnsetEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *pointer = node->getChild(0);
...
TR::Register *OMR::X86::TreeEvaluator::PrefetchEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (prefetchOp.getOpCodeValue() != BADIA32Op) {
    ...
    if (secondChild->getOpCode().isLoadConst()) {
      ...
      generateMemInstruction(prefetchOp.getOpCodeValue(), node,
                             generateX86MemoryReference(addrReg, offset, cg),
                             cg);
      ...
      generateMemInstruction(
          prefetchOp.getOpCodeValue(), node,
...
bool TR_X86ComputeCC::setCarryBorrow(TR::Node *flagNode, bool invertValue,
                                     TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::bucmpEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  return node->setRegister(NULL);
}

TR::Register *OMR::X86::TreeEvaluator::scmpEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  return node->setRegister(NULL);
}

TR::Register *OMR::X86::TreeEvaluator::sucmpEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  return node->setRegister(NULL);
}

TR::Register *OMR::X86::TreeEvaluator::icmpEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
  return node->setRegister(NULL);
}

TR::Register *OMR::X86::TreeEvaluator::iucmpEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
  return node->setRegister(NULL);
}

TR::Register *OMR::X86::TreeEvaluator::butestEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return node->setRegister(NULL);
}

TR::Register *OMR::X86::TreeEvaluator::sutestEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return node->setRegister(NULL);
}

// TR::ibatomicor TR::isatomicor TR::iiatomicor TR::ilatomicor
TR::Register *OMR::X86::TreeEvaluator::atomicorEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::tstartEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  /*
  .Lstart:
     xbegin       .Lfallback
  ...
  */
  ...
  TR::LabelSymbol *fallbackLabel =
      TR::LabelSymbol::create(cg->trHeapMemory(), cg);
  TR::LabelSymbol *persistentFailureLabel =
      persistentFailureNode->getBranchDestination()->getNode()->getLabel();
  TR::LabelSymbol *transientFailureLabel =
      transientFailureNode->getBranchDestination()->getNode()->getLabel();
  TR::LabelSymbol *fallThroughLabel =
      fallThroughNode->getBranchDestination()->getNode()->getLabel();

  // if LabelSymbol isn't ready yet, generate a new one and assign to the node.
  if (!fallThroughLabel) {
    fallThroughLabel = generateLabelSymbol(cg);
    fallThroughNode->getBranchDestination()->getNode()->setLabel(
        fallThroughLabel);
  }

  if (!transientFailureLabel) {
    transientFailureLabel = generateLabelSymbol(cg);
    transientFailureNode->getBranchDestination()->getNode()->setLabel(
        transientFailureLabel);
  }

  // in case user will make transientFailure goto persistenFailure, in which
  // case the label will mess up at this point we'd better re-generate the label
  // and set it to persistentFailure node again.
  if (!persistentFailureLabel ||
      persistentFailureLabel != persistentFailureNode->getBranchDestination()
                                    ->getNode()
                                    ->getLabel()) {
    persistentFailureLabel = generateLabelSymbol(cg);
    persistentFailureNode->getBranchDestination()->getNode()->setLabel(
        persistentFailureLabel);
  }
  ...
  if (fallThroughNode->getNumChildren() != 0) {
    ...
    fallThroughConditions =
        generateRegisterDependencyConditions(GRANode, cg, 0, &popRegisters);
  ...
  // startLabel
  // add place holder register so that eax would not contain any useful value
  // before xbegin
  TR::Register *dummyReg = cg->allocateRegister();
  dummyReg->setPlaceholderReg();
  TR::RegisterDependencyConditions *startLabelConditions =
  ...
  if (fallThroughConditions)
    generateLabelInstruction(JMP4, node, fallThroughLabel,
                             fallThroughConditions, cg);
  else
  ...
  if (transientConditions)
    generateLabelInstruction(JNE4, node, transientFailureLabel,
                             transientConditions, cg);
  else
    generateLabelInstruction(JNE4, node, transientFailureLabel, cg);

  // jmp to persistent begin:
  if (persistentConditions)
    generateLabelInstruction(JMP4, node, persistentFailureLabel,
                             persistentConditions, cg);
  else
...
TR::Register *OMR::X86::TreeEvaluator::tfinishEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  generateInstruction(XEND, node, cg);
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::tabortEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
bool OMR::X86::TreeEvaluator::VMinlineCallEvaluator(TR::Node *, bool,
                                                    TR::CodeGenerator *) {
  return false;
}

TR::Register *OMR::X86::TreeEvaluator::ibyteswapEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(node->getNumChildren() == 1,
            "Wrong number of children in byteswapEvaluator");
  TR::Node *child = node->getFirstChild();
  ...
  TR::Register *target =
      TR::TreeEvaluator::intOrLongClobberEvaluate(child, nodeIs64Bit, cg);
...
static const TR_X86OpCodes
    BinaryArithmeticOpCodesForReg[TR::NumOMRTypes][NumBinaryArithmeticOps] = {
        //  Invalid,       Add,         Sub,         Mul,         Div, And, Or,
        //  Xor
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // NoType
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Int8
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Int16
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Int32
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Int64
        {BADIA32Op, ADDSSRegReg, SUBSSRegReg, MULSSRegReg, DIVSSRegReg,
         BADIA32Op, BADIA32Op, BADIA32Op},  // Float
        {BADIA32Op, ADDSDRegReg, SUBSDRegReg, MULSDRegReg, DIVSDRegReg,
         BADIA32Op, BADIA32Op, BADIA32Op},  // Double
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Address
        {BADIA32Op, PADDBRegReg, PSUBBRegReg, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // VectorInt8
        {BADIA32Op, PADDWRegReg, PSUBWRegReg, PMULLWRegReg, BADIA32Op,
         BADIA32Op, BADIA32Op, BADIA32Op},  // VectorInt16
        {BADIA32Op, PADDDRegReg, PSUBDRegReg, PMULLDRegReg, BADIA32Op,
         PANDRegReg, PORRegReg, PXORRegReg},  // VectorInt32
        {BADIA32Op, PADDQRegReg, PSUBQRegReg, BADIA32Op, BADIA32Op, PANDRegReg,
         PORRegReg, PXORRegReg},  // VectorInt64
        {BADIA32Op, ADDPSRegReg, SUBPSRegReg, MULPSRegReg, DIVPSRegReg,
         BADIA32Op, BADIA32Op, BADIA32Op},  // VectorFloat
        {BADIA32Op, ADDPDRegReg, SUBPDRegReg, MULPDRegReg, DIVPDRegReg,
         BADIA32Op, BADIA32Op, BADIA32Op},  // VectorDouble
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Aggregate
};

static const TR_X86OpCodes
    BinaryArithmeticOpCodesForMem[TR::NumOMRTypes][NumBinaryArithmeticOps] = {
        //  Invalid,       Add,         Sub,         Mul,         Div, And, Or,
        //  Xor
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // NoType
        ...
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Int64
        {BADIA32Op, ADDSSRegMem, SUBSSRegMem, MULSSRegMem, DIVSSRegMem,
         BADIA32Op, BADIA32Op, BADIA32Op},  // Float
        {BADIA32Op, ADDSDRegMem, SUBSDRegMem, MULSDRegMem, DIVSDRegMem,
         BADIA32Op, BADIA32Op, BADIA32Op},  // Double
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Address
        {BADIA32Op, PADDBRegMem, PSUBBRegMem, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // VectorInt8
        {BADIA32Op, PADDWRegMem, PSUBWRegMem, PMULLWRegMem, BADIA32Op,
         BADIA32Op, BADIA32Op, BADIA32Op},  // VectorInt16
        {BADIA32Op, PADDDRegMem, PSUBDRegMem, PMULLDRegMem, BADIA32Op,
         PANDRegMem, PORRegMem, PXORRegMem},  // VectorInt32
        {BADIA32Op, PADDQRegMem, PSUBQRegMem, BADIA32Op, BADIA32Op, PANDRegMem,
         PORRegMem, PXORRegMem},  // VectorInt64
        {BADIA32Op, ADDPSRegMem, SUBPSRegMem, MULPSRegMem, DIVPSRegMem,
         BADIA32Op, BADIA32Op, BADIA32Op},  // VectorFloat
        {BADIA32Op, ADDPDRegMem, SUBPDRegMem, MULPDRegMem, DIVPDRegMem,
         BADIA32Op, BADIA32Op, BADIA32Op},  // VectorDouble
        {BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op,
         BADIA32Op, BADIA32Op},  // Aggregate
};
...
TR::Register *
OMR::X86::TreeEvaluator::FloatingPointAndVectorBinaryArithmeticEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR_X86OpCodes opCode = useRegMemForm
                             ? BinaryArithmeticOpCodesForMem[type][arithmetic]
                             : BinaryArithmeticOpCodesForReg[type][arithmetic];
  TR_ASSERT(opCode != BADIA32Op,
            "FloatingPointAndVectorBinaryArithmeticEvaluator: unsupported data "
            "type or arithmetic.");

  if (TR::CodeGenerator::getX86ProcessorInfo().supportsAVX()) {
    if (useRegMemForm)
      generateRegRegMemInstruction(opCode, node, resultReg, operandReg0,
                                   generateX86MemoryReference(operandNode1, cg),
                                   cg);
    else
      generateRegRegRegInstruction(opCode, node, resultReg, operandReg0,
                                   operandReg1, cg);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::bitpermuteEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT(node->getNumChildren() == 3,
            "Wrong number of children in bitpermuteEvaluator");
  ...
  // Zero result reg
  TR::Register *resultReg = cg->allocateRegister(TR_GPR);
  generateRegRegInstruction(XORRegReg(nodeIs64Bit), node, resultReg, resultReg,
                            cg);
  ...
  if (length->getOpCode().isLoadConst()) {
    ...
    for (uintptrj_t x = 0; x < arrayLen; ++x) {
      ...
      if (x >= 8)
        generateRegRegInstruction(XORRegReg(nodeIs64Bit), node, tmpReg, tmpReg,
                                  cg);

      TR::MemoryReference *sourceMR =
          generateX86MemoryReference(addrReg, x, cg);
      generateRegMemInstruction(L1RegMem, node, tmpReg, sourceMR, cg);
      generateRegRegInstruction(BTRegReg(nodeIs64Bit), node, valueReg, tmpReg,
                                cg);
    ...
    TR::RegisterDependencyConditions *shiftDependencies =
        generateRegisterDependencyConditions((uint8_t)1, 1, cg);
    ...
    TR::MemoryReference *sourceMR =
        generateX86MemoryReference(addrReg, indexReg, 0, 0, cg);
    generateRegMemInstruction(L1RegMem, node, tmpReg, sourceMR, cg);
    generateRegRegInstruction(BTRegReg(nodeIs64Bit), node, valueReg, tmpReg,
                              cg);
    generateRegInstruction(SETB1Reg, node, tmpReg, cg);
    ...
    generateRegRegInstruction(SHLRegCL(nodeIs64Bit), node, tmpReg, indexReg,
                              shiftDependencies, cg);
    generateRegRegInstruction(ORRegReg(nodeIs64Bit), node, resultReg, tmpReg,
                              cg);
