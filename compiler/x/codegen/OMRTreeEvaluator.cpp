...
TR::Instruction* OMR::X86::TreeEvaluator::insertLoadConstant(TR::Node* node, TR::Register* target, intptrj_t value,
    ...
    if (target && node && node->getOpCodeValue() == TR::aconst && node->isMethodPointerConstant()
      && (cg->fe()->isUnloadAssumptionRequired(cg->fe()
                                                 ->createResolvedMethod(cg->trMemory(),
                                                   (TR_OpaqueMethodBlock*)node->getAddress(), comp->getCurrentMethod())
                                                 ->classOfMethod(),
...
TR::Register* OMR::X86::TreeEvaluator::loadConstant(
  TR::Node* node, intptrj_t value, TR_RematerializableTypes type, TR::CodeGenerator* cg, TR::Register* targetRegister)
{
  ...
  if (cg->enableRematerialisation()) {
    if (node && node->getOpCode().hasSymbolReference() && node->getSymbol() && node->getSymbol()->isClassObject())
      (TR::Compiler->om.generateCompressedObjectHeaders() || TR::Compiler->target.is32Bit())
  ...
  TR::Instruction* currentInstruction)
{
  ...
  if (node && node->getSymbol()->isStatic() && node->getSymbol()->isClassObject()
...
{
  TR_ASSERT(cg->supportsStaticMemoryRematerialization(), "should not be called without rematerialisation of statics\n");
...
TR::Register* OMR::X86::TreeEvaluator::integerStoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->getOpCode().isIndirect()) {
    ...
    if (comp->useCompressedPointers() && (node->getSymbolReference()->getSymbol()->getDataType() == TR::Address)) {
      ...
      if (usingCompressedPointers) {
        if (!usingLowMemHeap) {
          while ((valueChild->getNumChildren() > 0) && (valueChild->getOpCodeValue() != TR::a2l))
  ...
  if (valueChild->getOpCode().isLoadConst() && valueChild->getRegister() == NULL && !usingCompressedPointers) {
...
{
  return node->isArrayCmpLen() ? TR::TreeEvaluator::SSE2ArraycmpLenEvaluator(node, cg)
...
void OMR::X86::TreeEvaluator::genArithmeticInstructionsForOverflowCHK(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  case TR::lmul:
    ...
    // TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser Therefore the usual way of only detecting the OF for the
    // last instruction of sequence won't work for this case. The implementation needs to detect OF flags after all the
    // instructions involving higher parts of the registers for both operands and intermediate results.
    TR_ASSERT(TR::Compiler->target.is64Bit(), "overflowCHK for lmul on 32 bits is not currently supported\n");
    op = IMULRegReg(nodeIs64Bit);
    break;
  default:
    TR_ASSERT(
      0, "unsupported OverflowCHK opcode %s on node %p\n", cg->comp()->getDebug()->getName(node->getOpCode()), node);
  }
...
static void generateRepMovsInstruction(TR_X86OpCodes repmovs, TR::Node* node, TR::Register* sizeRegister,
  ...
  TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Register* sizeReg, TR::CodeGenerator* cg)
{
  ...
  } else // decide direction during runtime
  {
    ...
    generateRegMemInstruction(
      LEARegMem(), node, dstReg, generateX86MemoryReference(dstReg, srcReg, 0, cg), cg); // dst = dst + src
  ...
  TR::Register* sizeReg, TR::CodeGenerator* cg)
{
  ...
  } else // decide direction during runtime
  {
...

    {
      TR_OutlinedInstructionsGenerator og(backwardLabel, node, cg);
      generateRegMemInstruction(
        LEARegMem(), node, srcReg, generateX86MemoryReference(srcReg, sizeReg, 0, -(intptr_t)elementSize, cg), cg);
      generateRegMemInstruction(
        LEARegMem(), node, dstReg, generateX86MemoryReference(dstReg, sizeReg, 0, -(intptr_t)elementSize, cg), cg);
      generateInstruction(STD, node, cg);
      generateRepMovsInstruction(repmovs, node, sizeReg, NULL, cg);
      generateInstruction(CLD, node, cg);
      generateLabelInstruction(JMP4, node, mainEndLabel, cg);
 ...
 *
 */
static void generateArrayElementStore(
  TR::Node* node, TR::Register* addressReg, int32_t index, TR::Register* valueReg, uint8_t size, TR::CodeGenerator* cg)
{
 ...
 *     The code generator
 */
static void generateArrayElementLoad(
  TR::Node* node, TR::Register* valueReg, uint8_t size, TR::Register* addressReg, int32_t index, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::X86::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  // The fact that a 5-child arraycopy is generated indicates at least one of the first two children must be needed when
  // performing the copy; otherwise a 3-child arraycopy should be generated instead. Interpreting the meanings of the
  // first two children definitely requires language specific semantics. For example, the first two children may be for
  // dealing with an arraycopy where the Garbage Collector may need to be notified about the copy or something to that
  // affect.
  //
  // Therefore, this OMR evaluator only handles the 3-child variant, is an operation equivalent to C++'s std::memmove().
  // Should a downstream project need the 5-child variant evaluator, it needs to override this evaluator in its own
  // TreeEvaluator, and delegates the 3-child variant back to this evaluator by calling
  ...
  TR::Node* node, TR::Register* XMMReg, TR::Register* sourceReg, int8_t size, TR::CodeGenerator* cg)
{
  ...
  case 2: {
    ...
    auto snippet
      = generateX86MemoryReference(cg->findOrCreate16ByteConstant(node, size == 1 ? MASKOFSIZEONE : MASKOFSIZETWO), cg);
...
static void arraySetToZeroForShortConstantArrays(
  TR::Node* node, TR::Register* addressReg, uintptrj_t size, TR::CodeGenerator* cg)
{
...
static void arraySetForShortConstantArrays(TR::Node* node, uint8_t elementSize, TR::Register* addressReg,
  TR::Register* valueReg, uintptrj_t size, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::X86::TreeEvaluator::arraysetEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::Register* addressReg
    = TR::TreeEvaluator::intOrLongClobberEvaluate(addressNode, TR::Compiler->target.is64Bit(), cg);
  uintptrj_t size;
  ...
  if (isShortConstantArrayWithZero) {
    ...
    if (valueNode->getOpCode().isFloat() || valueNode->getOpCode().isDouble()) {
      ...
      valueReg
        = TR::TreeEvaluator::intOrLongClobberEvaluate(valueNode, TR::TreeEvaluator::getNodeIs64Bit(valueNode, cg), cg);
    }
    ...
    TR::Register* sizeReg
      = TR::TreeEvaluator::intOrLongClobberEvaluate(sizeNode, TR::TreeEvaluator::getNodeIs64Bit(sizeNode, cg), cg);
...
{
  TR_ASSERT((!TR_X86OpCode(op).hasLongSource() && !TR_X86OpCode(op).hasLongTarget()) || TR::Compiler->target.is64Bit(),
...
static TR::Register* inlineAtomicCompareAndMemoryUpdate(TR::Node* node, bool returnValue, TR::CodeGenerator* cg)
{
  ...
  generateMemRegInstruction(
    LCMPXCHGMemReg(isNode64Bit), node, generateX86MemoryReference(address, 0, cg), newvalue, deps, cg);
  if (!returnValue) {
    generateRegInstruction(SETE1Reg, node, oldvalue, cg);
    generateRegRegInstruction(MOVZXReg4Reg1, node, oldvalue, oldvalue, cg);
  }
...
TR::Register* OMR::X86::TreeEvaluator::directCallEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  //
  if (returnRegister && returnRegister->needsPrecisionAdjustment() && comp->getCurrentMethod()->isStrictFP()) {
...
TR::Register* OMR::X86::TreeEvaluator::integerRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  //
  if (TR::Compiler->target.is64Bit() && node->getOpCodeValue() == TR::iRegLoad
...
TR::Register* OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!block->isExtensionOfPreviousBlock()) {
    ...
    if (doAlign && !block->isCold() && block->firstBlockInLoop() && (comp->getOptLevel() > warm || alwaysAlignLoops)) {
  ...
  TR::Instruction* fence = generateFenceInstruction(
    FENCE, node, TR::Node::createRelative32BitFenceNode(node, &block->getInstructionBoundaries()._startPC), cg);
  ...
  if (comp->getOption(TR_BreakBBStart)) {
    ...
    generateRegImmInstruction(
      TEST4RegImm4, node, machine->getRealRegister(TR::RealRegister::esp), block->getNumber(), cg);
...
TR::Register* OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::X86FenceInstruction* instr = generateFenceInstruction(FENCE, node,
  ...
  if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
    ...
    if (cg->enableRegisterAssociations() && cg->getAppendInstruction()->getOpCodeValue() != ASSOCREGS) {
...
TR::Register* OMR::X86::TreeEvaluator::conversionAnalyser(
  TR::Node* node, TR_X86OpCodes memoryToRegisterOp, TR_X86OpCodes registerToRegisterOp, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  ...
  if (child->getReferenceCount() > 1) {
    ...
    if (node->parentSupportsLazyClobber() && registerToRegisterOp == MOVZXReg8Reg4 && sourceRegister->areUpperBitsZero()
      && performTransformation(comp, "O^O LAZY CLOBBERING: reuse register %s from %s for %s\n",
        cg->getDebug()->getName(sourceRegister), cg->getDebug()->getName(child), cg->getDebug()->getName(node))) {
    ...
    if (child->getRegister() == NULL && child->getOpCode().isMemoryReference()) {
      ...
      //
      if (node->getOpCodeValue() == TR::iu2l && comp->useCompressedPointers() && child->getOpCode().isLoadIndirect()
  ...
  if (sourceRegister) {
    if (!((sourceRegister == targetRegister) && (registerToRegisterOp == MOVZXReg8Reg4)
...
TR::Register* OMR::X86::TreeEvaluator::icmpsetEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateMemRegInstruction(TR::Compiler->target.isSMP() ? LCMPXCHGMemReg(nodeIs64Bit) : CMPXCHGMemReg(nodeIs64Bit),
...
bool TR_X86ComputeCC::setCarryBorrow(TR::Node* flagNode, bool invertValue, TR::CodeGenerator* cg)
{
...
bool OMR::X86::TreeEvaluator::VMinlineCallEvaluator(TR::Node*, bool, TR::CodeGenerator*)
{
  return false;
}
...
TR::Register* OMR::X86::TreeEvaluator::FloatingPointAndVectorBinaryArithmeticEvaluator(
  TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR_X86OpCodes opCode
    = useRegMemForm ? BinaryArithmeticOpCodesForMem[type][arithmetic] : BinaryArithmeticOpCodesForReg[type][arithmetic];
  TR_ASSERT(
    opCode != BADIA32Op, "FloatingPointAndVectorBinaryArithmeticEvaluator: unsupported data type or arithmetic.");
