...
extern bool
existsNextInstructionToTestFlags(TR::Instruction* startInstr, uint8_t testMask);

TR::Register*
OMR::X86::TreeEvaluator::ifxcmpoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateConditionalJumpInstruction(
    reverseBranch ? JNO4 : JO4, node, cg, true);
...
TR::Register*
OMR::X86::TreeEvaluator::intOrLongClobberEvaluate(TR::Node* node,
                                                  bool nodeIs64Bit,
                                                  TR::CodeGenerator* cg)
{
...
TR::Instruction*
OMR::X86::TreeEvaluator::compareGPMemoryToImmediate(TR::Node* node,
                                                    ...
                                                    TR::CodeGenerator* cg)
{
  ...
  bool is64Bit =
    TR::Compiler->target.is64Bit()
      ...
      : false;
  TR_X86OpCodes cmpOp =
    (value >= -128 && value <= 127) ? CMPMemImms(is64Bit) : CMPMemImm4(is64Bit);
...
void
OMR::X86::TreeEvaluator::compareGPRegisterToImmediate(TR::Node* node,
                                                      ...
                                                      TR::CodeGenerator* cg)
{
  ...
  bool is64Bit =
    TR::Compiler->target.is64Bit()
      ...
      : false;
  TR_X86OpCodes cmpOp =
    (value >= -128 && value <= 127) ? CMPRegImms(is64Bit) : CMPRegImm4(is64Bit);
  generateRegImmInstruction(cmpOp, node, cmpRegister, value, cg);
}

void
OMR::X86::TreeEvaluator::compareGPRegisterToImmediateForEquality(
  TR::Node* node,
  TR::Register* cmpRegister,
  int32_t value,
  TR::CodeGenerator* cg)
{
  ...
  if (value == 0)
    generateRegRegInstruction(
      TESTRegReg(is64Bit), node, cmpRegister, cmpRegister, cg);
  else
    generateRegImmInstruction(cmpOp, node, cmpRegister, value, cg);
}

TR::Instruction*
OMR::X86::TreeEvaluator::insertLoadConstant(TR::Node* node,
                                            TR::Register* target,
                                            intptrj_t value,
                                            ...
                                            TR::Instruction* currentInstruction)
{
  ...
  static const TR_X86OpCodes ops[TR_NumRematerializableTypes + 1][3] =
    //    load 0      load -1     load c
    {
      ...
      { XOR4RegReg, OR8RegImms, BADIA32Op }
  ...
  if (currentInstruction) {
    ...
    if ((value == 0 || value == -1)) {
      uint8_t EFlags =
        TR_X86OpCode::getModifiedEFlags(ops[opsRow][((value == 0) ? XOR : OR)]);

      if (existsNextInstructionToTestFlags(currentInstruction, EFlags) ||
          cg->requiresCarry()) {
        // Can't alter flags, so must use MOV.  Fall through.
      } else if (value == 0)
        return generateRegRegInstruction(
          currentInstruction, ops[opsRow][XOR], target, target, cg);
      else if (value == -1)
        return generateRegImmInstruction(
          currentInstruction, ops[opsRow][OR], target, (uint32_t)-1, cg);
    }
    ...
    bool isPICCandidate =
      symbol ? target && symbol->isStatic() && symbol->isClassObject() : false;
    if (isPICCandidate &&
    ...
    if (target && node && node->getOpCodeValue() == TR::aconst &&
        ...
        (cg->fe()->isUnloadAssumptionRequired(
         ...
         cg->profiledPointersRequireRelocation())) {
      traceMsg(comp,
               "Adding instr %p to MethodPICSites for node %p\n",
               movInstruction,
               node);
    ...
    if (value == 0 && canClobberEFLAGS) {
      return generateRegRegInstruction(
        ops[opsRow][XOR], node, target, target, cg);
    } else if (value == -1 && canClobberEFLAGS) {
      return generateRegImmInstruction(
        ops[opsRow][OR], node, target, (uint32_t)-1, cg);
    } else {
      TR::Instruction* movInstruction = NULL;
      if (is64Bit) {
        if (cg->constantAddressesCanChangeSize(node) && node &&
          ...
          // zero-extended 4-byte MOV
          movInstruction = generateRegImmInstruction(
            MOV4RegImm4, node, target, value, cg, reloKind);
        } else if (IS_32BIT_SIGNED(value)) // TODO:AMD64: Is there some way we
                                           // could get RIP too?
        {
          movInstruction = generateRegImmInstruction(
            MOV8RegImm4, node, target, value, cg, reloKind);
        } else {
          movInstruction = generateRegImm64Instruction(
            MOV8RegImm64, node, target, value, cg, reloKind);
        }
        ...
        movInstruction = generateRegImmInstruction(
          ops[opsRow][MOV], node, target, value, cg, reloKind);
      }
...
TR::Register*
OMR::X86::TreeEvaluator::loadConstant(TR::Node* node,
                                      intptrj_t value,
                                      TR_RematerializableTypes type,
                                      TR::CodeGenerator* cg,
...
TR::Instruction*
OMR::X86::TreeEvaluator::insertLoadMemory(TR::Node* node,
                                          TR::Register* target,
                                          ...
                                          TR::Instruction* currentInstruction)
{
  ...
  if (currentInstruction) {
    i =
      generateRegMemInstruction(currentInstruction, opCode, target, tempMR, cg);
  } else {
...
//
void
OMR::X86::TreeEvaluator::padUnresolvedDataReferences(
  TR::Node* node,
...
TR::Register*
OMR::X86::TreeEvaluator::loadMemory(TR::Node* node,
                                    ...
                                    TR::CodeGenerator* cg)
{
  ...
  instr = TR::TreeEvaluator::insertLoadMemory(
    node, targetRegister, sourceMR, type, cg);
...
void
OMR::X86::TreeEvaluator::removeLiveDiscardableStatics(TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::performIload(TR::Node* node,
                                      TR::MemoryReference* sourceMR,
                                      TR::CodeGenerator* cg)
{
  TR::Register* reg = TR::TreeEvaluator::loadMemory(
    node, sourceMR, TR_RematerializableInt, node->getOpCode().isIndirect(), cg);
...
// also handles iaload
TR::Register*
OMR::X86::TreeEvaluator::aloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::MemoryReference* sourceMR = generateX86MemoryReference(node, cg);
  TR::Register* reg =
    TR::TreeEvaluator::loadMemory(node,
                                  sourceMR,
                                  TR_RematerializableAddress,
  ...
  if (!node->getSymbolReference()->isUnresolved() &&
    ...
    if (fieldName && strstr(fieldName, "Ljava/lang/String;")) {
      generateMemInstruction(
        PREFETCHT0, node, generateX86MemoryReference(reg, 0, cg), cg);
    }
  }
...
//
bool
OMR::X86::TreeEvaluator::genNullTestSequence(TR::Node* node,
                                             TR::Register* opReg,
...
// also handles iiload
TR::Register*
OMR::X86::TreeEvaluator::iloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::MemoryReference* sourceMR = generateX86MemoryReference(node, cg);
...
// also handles ibload
TR::Register*
OMR::X86::TreeEvaluator::bloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::MemoryReference* sourceMR = generateX86MemoryReference(node, cg);
  TR::Register* reg =
    TR::TreeEvaluator::loadMemory(node,
                                  sourceMR,
                                  TR_RematerializableByte,
                                  node->getOpCode().isIndirect(),
                                  cg);
...
TR::Register*
OMR::X86::TreeEvaluator::sloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::MemoryReference* sourceMR = generateX86MemoryReference(node, cg);
  TR::Register* reg =
    TR::TreeEvaluator::loadMemory(node,
                                  sourceMR,
                                  TR_RematerializableShort,
                                  node->getOpCode().isIndirect(),
                                  cg);
                                               ...
                                               TR::CodeGenerator* cg)
{
  ...
  if (childIsConstant &&
    ...
    if (usingCompressedPointers && !usingLowMemHeap) {
      ...
      //
      generateRegRegInstruction(
        TESTRegReg(), node, translatedReg, translatedReg, cg);
      generateRegRegInstruction(
        CMOVERegReg(), node, valueReg, translatedReg, cg);
    } else {
...
TR::Register*
OMR::X86::TreeEvaluator::istoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::bstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::integerStoreEvaluator(node, cg);
}
...
TR::Register*
OMR::X86::TreeEvaluator::sstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::integerStoreEvaluator(node, cg);
}
...
TR::Register*
OMR::X86::TreeEvaluator::cstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::integerStoreEvaluator(node, cg);
}
                                               ...
                                               TR::CodeGenerator* cg)
{
  ...
  generateRegRegInstruction(
    SUBRegReg(), node, deltaReg, s2Reg, cg); // delta = s1 - s2
  ...
  generateRegMemInstruction(
    MOVUPSRegMem, node, xmm2Reg, generateX86MemoryReference(s2Reg, 0, cg), cg);
  generateRegMemInstruction(MOVUPSRegMem,
                            node,
                            xmm1Reg,
  ...
  generateRegMemInstruction(L1RegMem,
                            node,
                            s2ByteVer1Reg,
                            generateX86MemoryReference(s2Reg, 0, cg),
                            cg);
  generateMemRegInstruction(CMP1MemReg,
                            node,
                            generateX86MemoryReference(s2Reg, deltaReg, 0, cg),
                            s2ByteVer1Reg,
                            cg);
  ...
  generateRegMemInstruction(
    ...
    node,
    s2ByteVer2Reg,
    generateX86MemoryReference(s2Reg, equalTestReg, 0, cg),
    cg);
  generateMemRegInstruction(CMP1MemReg,
                            node,
                            generateX86MemoryReference(s2Reg, deltaReg, 0, cg),
...
bool
OMR::X86::TreeEvaluator::stopUsingCopyRegAddr(TR::Node* node,
                                              TR::Register*& reg,
                                              TR::CodeGenerator* cg)
{
  if (node != NULL) {
...
bool
OMR::X86::TreeEvaluator::stopUsingCopyRegInteger(TR::Node* node,
                                                 TR::Register*& reg,
                                                 TR::CodeGenerator* cg)
{
  if (node != NULL) {
...
void
OMR::X86::TreeEvaluator::genArithmeticInstructionsForOverflowCHK(
  TR::Node* node,
  TR::CodeGenerator* cg)
{
  /*
  ...
  switch (node->getOverflowCheckOperation()) {
    ...
    default:
      TR_ASSERT(0,
                "unsupported OverflowCHK opcode %s on node %p\n",
  ...
  // we could detect it and avoid repeating the operation
  {
    ...
    switch (node->getOverflowCheckOperation()) {
      ...
      case TR::ladd:
        TR::Compiler->target.is32Bit()
          ? addMulAnalyser.longAddAnalyserWithExplicitOperands(
      ...
      case TR::lsub:
        TR::Compiler->target.is32Bit()
          ? subAnalyser.longSubtractAnalyserWithExplicitOperands(
              node, operand1, operand2)
          : subAnalyser.integerSubtractAnalyserWithExplicitOperands(
              node, operand1, operand2, op, BADIA32Op, MOV8RegReg, needsEflags);
        break;
      // mul group
      case TR::imul:
...
static void
generateRepMovsInstruction(TR_X86OpCodes repmovs,
                           TR::Node* node,
 ...
 */
static void
arrayCopy64BitPrimitiveOnIA32(TR::Node* node,
                              TR::Register* dstReg,
                              TR::Register* srcReg,
                              TR::Register* sizeReg,
                              TR::CodeGenerator* cg)
{
  ...
  } else // decide direction during runtime
  {
    ...
    generateRegMemInstruction(
      ...
      node,
      scratch,
      generateX86MemoryReference(srcReg, sizeReg, 0, -8, cg),
      cg);
    ...
    generateRegMemInstruction(
      ...
      node,
      scratch,
      generateX86MemoryReference(NULL, scratch, 1, -1, cg),
      cg);
  }
  ...
  generateRegMemInstruction(
    MOVQRegMem, node, XMM, generateX86MemoryReference(srcReg, 0, cg), cg);
  generateMemRegInstruction(MOVQMemReg,
                            node,
 ...
 */
static void
arrayCopy16BitPrimitive(TR::Node* node,
                        TR::Register* dstReg,
                        TR::Register* srcReg,
                        TR::Register* sizeReg,
                        TR::CodeGenerator* cg)
{
  ...
  if (node->isForwardArrayCopy()) {
    ...
    generateRegMemInstruction(
      L2RegMem, node, sizeReg, generateX86MemoryReference(srcReg, 0, cg), cg);
  ...
  } else // decide direction during runtime
  {
    ...
    generateRegRegInstruction(
      SUBRegReg(), node, dstReg, srcReg, cg); // dst = dst - src
    generateRegRegInstruction(
      CMPRegReg(), node, dstReg, sizeReg, cg); // cmp dst, size
    generateRegMemInstruction(LEARegMem(),
                              node,
                              dstReg,
                              generateX86MemoryReference(dstReg, srcReg, 0, cg),
                              cg); // dst = dst + src
    generateLabelInstruction(
      JB4, node, backwardLabel, cg); // jb, skip backward copy setup
...

    {
      ...
      generateRegMemInstruction(
        ...
        node,
        srcReg,
        generateX86MemoryReference(srcReg, sizeReg, 0, -2, cg),
        cg);
      generateRegMemInstruction(
        LEARegMem(),
        node,
        dstReg,
        generateX86MemoryReference(dstReg, sizeReg, 0, -2, cg),
        cg);
...

    {
      ...
      generateRegMemInstruction(
        ...
        generateX86MemoryReference(
          srcReg, sizeReg, 0, -(intptr_t)elementSize, cg),
        cg);
      generateRegMemInstruction(
        LEARegMem(),
        node,
        dstReg,
        generateX86MemoryReference(
          dstReg, sizeReg, 0, -(intptr_t)elementSize, cg),
        cg);
      generateInstruction(STD, node, cg);
      generateRepMovsInstruction(repmovs, node, sizeReg, NULL, cg);
      generateInstruction(CLD, node, cg);
 ...
 */
static void
generateArrayElementStore(TR::Node* node,
                          ...
                          TR::CodeGenerator* cg)
{
  ...
  if (valueReg->getKind() == TR_FPR) {
    switch (size) {
      ...
      default:
        TR_ASSERT(
          0, "Unsupported size in generateArrayElementStore, size: %d", size);
        break;
    }
  ...
  generateMemRegInstruction(storeOpcode,
                            node,
 ...
 */
static void
generateArrayElementLoad(TR::Node* node,
                         TR::Register* valueReg,
                         uint8_t size,
                         TR::Register* addressReg,
                         int32_t index,
                         TR::CodeGenerator* cg)
{
  ...
  if (valueReg->getKind() == TR_FPR) {
    switch (size) {
      ...
      default:
        TR_ASSERT(
          0, "Unsupported size in generateArrayElementLoad, size: %d", size);
        break;
    }
  } else if (valueReg->getKind() == TR_GPR) {
    switch (size) {
  ...
  generateRegMemInstruction(loadOpCode,
                            node,
                            valueReg,
                            generateX86MemoryReference(addressReg, index, cg),
                            cg);
...
static void
arraycopyForShortConstArrayWithDirection(TR::Node* node,
                                         TR::Register* dstReg,
                                         TR::Register* srcReg,
                                         uint32_t size,
                                         TR::CodeGenerator* cg)
{
  ...
  while (size != 0) {
    if (size >= regSize[i]) {
      ...
      generateArrayElementLoad(
        node, tempReg, regSize[i], srcReg, totalSize - size, cg);
      generateArrayElementStore(
        node, dstReg, totalSize - size, tempReg, regSize[i], cg);
      size -= regSize[i];
    } else {
      i++;
    }
 ...
 */
static void
arraycopyForShortConstArrayWithoutDirection(TR::Node* node,
                                            TR::Register* dstReg,
                                            TR::Register* srcReg,
                                            uint32_t size,
                                            TR::CodeGenerator* cg)
{
  ...
  {
    generateArrayElementLoad(
      node, srcReg, residue, srcReg, totalSize - residue, cg);
    reg1 = srcReg;
    residueCase = 1;
  ...
  } else if (residue != 0) // 1 gpr + 1 xmm
  {
    ...
    generateArrayElementLoad(node,
                             reg2,
                             secondLoadSizeForCase4,
                             srcReg,
                             residue - secondLoadSizeForCase4,
                             cg);
    residueCase = 4;
  }
  ...
  switch (residueCase) {
    ...
    case 1:
      generateArrayElementStore(
        node, dstReg, totalSize - residue, reg1, residue, cg);
      break;
    case 2:
    ...
    case 4:
      generateArrayElementStore(
        node, dstReg, 0, reg1, firstLoadSizeForCase4, cg);
      generateArrayElementStore(node,
                                dstReg,
                                totalSize - secondLoadSizeForCase4,
                                reg2,
                                secondLoadSizeForCase4,
                                cg);
      cg->stopUsingRegister(reg1);
...
static void
packXMMWithMultipleValues(TR::Node* node,
                          TR::Register* XMMReg,
                          TR::Register* sourceReg,
                          int8_t size,
                          TR::CodeGenerator* cg)
  ...
  switch (size) {
    ...
    case 8:
      ...
      generateRegRegImmInstruction(
        PSHUFDRegRegImm1, node, XMMReg, XMMReg, size == 4 ? 0x00 : 0x44, cg);
      break;
    default:
...
static void
arraySetToZeroForShortConstantArrays(TR::Node* node,
                                     TR::Register* addressReg,
                                     uintptrj_t size,
                                     TR::CodeGenerator* cg)
{
  ...
  if (size < 16) {
    ...
    for (int32_t i = 0; i < 4; i++) {
      ...
      for (int32_t j = 0; j < moves; j++) {
        generateArrayElementStore(
          node, addressReg, index, tempReg, packs[i], cg);
        index += packs[i];
      }
      size = size % packs[i];
    }
...
static void
arraySetForShortConstantArrays(TR::Node* node,
                               uint8_t elementSize,
                               TR::Register* addressReg,
                               TR::Register* valueReg,
                               uintptrj_t size,
  ...
  if (totalSize < 16) {
    ...
    for (int32_t i = 1; i < 5; i++) {
      for (int32_t j = 0; j < moves[i]; j++) {
        generateArrayElementStore(
          node, addressReg, index, currentReg, packs[i], cg);
        index += packs[i];
      }
    }
  } else {
    ...
    if (size < notWorthPacking) {
      for (int32_t i = 0; i < size; i++) {
        generateArrayElementStore(
          node, addressReg, i * elementSize, valueReg, elementSize, cg);
      }
    } else {
      TR::Register* XMM = cg->allocateRegister(TR_FPR);
      ...
      if (reminder == elementSize) {
        generateArrayElementStore(
          node, addressReg, moves[0] * 16, valueReg, elementSize, cg);
      } else if (reminder != 0) {
        generateArrayElementStore(
          node, addressReg, totalSize - 16, XMM, 16, cg);
      }
      cg->stopUsingRegister(XMM);
    }
  }
...
static void
arraySet64BitPrimitiveOnIA32(TR::Node* node,
                             TR::Register* addressReg,
                             TR::Register* valueReg,
                             TR::Register* sizeReg,
                             TR::CodeGenerator* cg)
{
  ...
  switch (valueReg->getKind()) {
    case TR_GPR:
      generateRegRegInstruction(
        MOVDRegReg4, node, XMM, valueReg->getHighOrder(), cg);
      generateRegImmInstruction(PSLLQRegImm1, node, XMM, 32, cg);
      generateRegRegInstruction(
        MOVDRegReg4, node, XMM, valueReg->getLowOrder(), cg);
      break;
    case TR_FPR:
  ...
  generateMemRegInstruction(
    MOVQMemReg,
    node,
    generateX86MemoryReference(addressReg, sizeReg, 3, -8, cg),
    XMM,
    cg);
  generateLabelInstruction(LOOP1, node, loopLabel, cg);
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (isShortConstantArrayWithZero) {
    ...
    arraySetForShortConstantArrays(
      node, elementSize, addressReg, valueReg, size / elementSize, cg);
    cg->recursivelyDecReferenceCount(sizeNode);
...
bool
OMR::X86::TreeEvaluator::constNodeValueIs32BitSigned(TR::Node* node,
                                                     intptrj_t* value,
                                                     TR::CodeGenerator* cg)
{
...
bool
OMR::X86::TreeEvaluator::getNodeIs64Bit(TR::Node* node, TR::CodeGenerator* cg)
{
                                               ...
                                               TR::CodeGenerator* cg)
{
  if (TR::TreeEvaluator::getNodeIs64Bit(node, cg)) {
    return (intptrj_t)
      node->getLongInt(); // Cast to satisfy 32-bit compilers, even though they
...
TR::Register*
OMR::X86::TreeEvaluator::performCall(TR::Node* node,
                                     bool isIndirect,
                                     bool spillFPRegs,
                                     TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::performHelperCall(TR::Node* node,
 ...
 */
static TR::Register*
inlineSinglePrecisionSQRT(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (opRegister->getKind() == TR_FPR) {
    ...
    generateRegRegInstruction(
      SQRTSSRegReg, node, targetRegister, opRegister, cg);
  } else {
 ...
 */
static TR::Register*
inlineAtomicMemoryUpdate(TR::Node* node,
                         TR_X86OpCodes op,
                         TR::CodeGenerator* cg)
{
  TR_ASSERT(
    (!TR_X86OpCode(op).hasLongSource() && !TR_X86OpCode(op).hasLongTarget()) ||
      TR::Compiler->target.is64Bit(),
 ...
 */
static TR::Register*
inline64BitAtomicCompareAndMemoryUpdateOn32Bit(TR::Node* node,
                                               bool returnValue,
                                               TR::CodeGenerator* cg)
{
  ...
  generateMemInstruction(
    LCMPXCHG8BMem, node, generateX86MemoryReference(address, 0, cg), deps, cg);
  if (!returnValue) {
 ...
 */
static TR::Register*
inlineAtomicCompareAndMemoryUpdate(TR::Node* node,
                                   bool returnValue,
                                   TR::CodeGenerator* cg)
{
  bool isNode64Bit = node->getChild(1)->getDataType().isInt64();
  if (TR::Compiler->target.is32Bit() && isNode64Bit) {
    return inline64BitAtomicCompareAndMemoryUpdateOn32Bit(
      node, returnValue, cg);
  }
  ...
  generateMemRegInstruction(LCMPXCHGMemReg(isNode64Bit),
                            node,
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (SymRef && SymRef->getSymbol()->castToMethodSymbol()->isInlinedByCG()) {
    ...
    if (comp->getSymRefTab()->isNonHelper(
      ...
      op =
        node->getChild(1)->getDataType().isInt32() ? LADD4MemReg : LADD8MemReg;
      ...
      op =
        node->getChild(1)->getDataType().isInt32() ? XCHG4MemReg : XCHG8MemReg;
    } else if (comp->getSymRefTab()->isNonHelper(
...
TR::Register*
OMR::X86::TreeEvaluator::treetopEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
void
OMR::X86::TreeEvaluator::compareGPRegisterToConstantForEquality(
  TR::Node* node,
  int32_t value,
  TR::Register* cmpRegister,
  TR::CodeGenerator* cg)
{
  if (value == 0) {
    ...
    TR::TreeEvaluator::compareGPRegisterToImmediate(
      node, cmpRegister, value, cg);
  }
}

TR::Register*
OMR::X86::TreeEvaluator::fenceEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::generateLEAForLoadAddr(TR::Node* node,
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  TR::Register* targetRegister =
    TR::TreeEvaluator::generateLEAForLoadAddr(node, memRef, symRef, cg, false);
                                                 ...
                                                 TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit() &&
      ...
      performTransformation(
        comp,
        "TREE EVALUATION: setUpperBitsAreZero on iRegLoad %s\n",
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if ((child->getReferenceCount() > 1 &&
    ...
    if (srcReg->getRegisterPair()) {
      ...
      generateRegRegInstruction(
        MOVRegReg(), node, copyReg, srcReg->getHighOrder(), cg);
      generateRegRegInstruction(
        MOVRegReg(), node, copyRegLow, srcReg->getLowOrder(), cg);
...
TR::Register*
OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!block->isExtensionOfPreviousBlock()) {
    ...
    if (node->getNumChildren() > 0)
      inst = generateLabelInstruction(
        LABEL, node, label, node->getFirstChild(), &popRegisters, cg);
    else
  ...
  if (comp->getOption(TR_BreakBBStart)) {
    TR::Machine* machine = cg->machine();
    generateRegImmInstruction(TEST4RegImm4,
                              node,
  ...
  if (!popRegisters.isEmpty()) {
    ...
    for (TR::Register* popRegister = popRegsIt.getFirst(); popRegister != NULL;
      ...
      generateFPSTiST0RegRegInstruction(
        FSTRegReg, node, popRegister, popRegister, cg);
...
TR::Register*
OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
    ...
    if (node->getNumChildren() > 0)
      labelInst = generateLabelInstruction(
        LABEL, node, generateLabelSymbol(cg), node->getFirstChild(), NULL, cg);
    else
      labelInst =
...
TR::Register*
OMR::X86::TreeEvaluator::conversionAnalyser(TR::Node* node,
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getReferenceCount() > 1) {
    ...
    if (node->parentSupportsLazyClobber() &&
        ...
        performTransformation(
          comp,
          "O^O LAZY CLOBBERING: reuse register %s from %s for %s\n",
    ...
    if (child->getRegister() == NULL &&
      ...
      if (node->getOpCodeValue() == TR::iu2l && comp->useCompressedPointers() &&
        ...
        targetRegister = cg->allocateRegister();
        generateRegMemInstruction(
          memoryToRegisterOp, node, targetRegister, tempMR, cg);
        tempMR->decNodeReferenceCounts(cg);
      }
    } else {
  ...
  if (sourceRegister) {
    if (!((sourceRegister == targetRegister) &&
          ...
          (sourceRegister->areUpperBitsZero())))
      generateRegRegInstruction(
        registerToRegisterOp, node, targetRegister, sourceRegister, cg);
  }
...
TR::Register*
OMR::X86::TreeEvaluator::NOPEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return NULL;
}
...
OMR::X86::TreeEvaluator::icmpsetEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::Register* compareReg =
    TR::TreeEvaluator::intOrLongClobberEvaluate(compareValue, nodeIs64Bit, cg);
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (prefetchOp.getOpCodeValue() != BADIA32Op) {
    ...
    if (secondChild->getOpCode().isLoadConst()) {
      ...
      generateMemInstruction(prefetchOp.getOpCodeValue(),
                             node,
      ...
      generateMemInstruction(
        prefetchOp.getOpCodeValue(),
        node,
        generateX86MemoryReference(addrReg, offsetReg, 0, cg),
        cg);
...
bool
TR_X86ComputeCC::setCarryBorrow(TR::Node* flagNode,
                                bool invertValue,
                                TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::bcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::bucmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  return node->setRegister(NULL);
}
...
TR::Register*
OMR::X86::TreeEvaluator::scmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::sucmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  return node->setRegister(NULL);
}
...
TR::Register*
OMR::X86::TreeEvaluator::icmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::iucmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
  return node->setRegister(NULL);
}
...
TR::Register*
OMR::X86::TreeEvaluator::butestEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return node->setRegister(NULL);
}

TR::Register*
OMR::X86::TreeEvaluator::sutestEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return node->setRegister(NULL);
}
...
OMR::X86::TreeEvaluator::tstartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  */
  ...
  if (fallThroughConditions)
    generateLabelInstruction(
      JMP4, node, fallThroughLabel, fallThroughConditions, cg);
  else
  ...
  if (transientConditions)
    generateLabelInstruction(
      JNE4, node, transientFailureLabel, transientConditions, cg);
  else
  ...
  if (persistentConditions)
    generateLabelInstruction(
      JMP4, node, persistentFailureLabel, persistentConditions, cg);
  else
...
bool
OMR::X86::TreeEvaluator::VMinlineCallEvaluator(TR::Node*,
                                               bool,
                                               TR::CodeGenerator*)
{
  return false;
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (TR::CodeGenerator::getX86ProcessorInfo().supportsAVX()) {
    if (useRegMemForm)
      generateRegRegMemInstruction(opCode,
                                   node,
                                   resultReg,
                                   operandReg0,
      ...
      generateRegRegRegInstruction(
        opCode, node, resultReg, operandReg0, operandReg1, cg);
  } else {
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  TR::Register* resultReg = cg->allocateRegister(TR_GPR);
  generateRegRegInstruction(
    XORRegReg(nodeIs64Bit), node, resultReg, resultReg, cg);
  ...
  if (length->getOpCode().isLoadConst()) {
    ...
    generateRegRegInstruction(
      SHLRegCL(nodeIs64Bit), node, tmpReg, indexReg, shiftDependencies, cg);
    generateRegRegInstruction(
      ORRegReg(nodeIs64Bit), node, resultReg, tmpReg, cg);
