...
OMR::X86::Instruction::Instruction(TR::CodeGenerator *cg,
                                   TR::InstOpCode::Mnemonic op, TR::Node *node)
    : OMR::Instruction(cg, op, node), _conditions(0), _rexRepeatCount(0) {}
...
void OMR::X86::Instruction::initialize(TR::CodeGenerator *cg,
                                       TR::RegisterDependencyConditions *cond,
                                       TR_X86OpCodes op, bool flag) {
...
void OMR::X86::Instruction::assumeValidInstruction() {
  TR_ASSERT(
      !(TR::Compiler->target.is64Bit() && self()->getOpCode().isIA32Only()),
...
void OMR::X86::Instruction::assignRegisters(
  ...
  if (self()->getOpCodeValue() != ASSOCREGS) {
    if ((self()->cg()->getAssignmentDirection() == self()->cg()->Backward)) {
      self()->getDependencyConditions()->assignPostConditionRegisters(
          self(), kindsToBeAssigned, self()->cg());
  ...
  } else if ((self()->getOpCodeValue() == ASSOCREGS) &&
             self()->cg()->enableRegisterAssociations()) {
    if (kindsToBeAssigned & TR_GPR_Mask) {
      ...
      for (int i = TR::RealRegister::FirstGPR;
        ...
        //
        if (machine->getRealRegister((TR::RealRegister::RegNum)i)->getState() ==
      ...
      //
      TR_X86RegisterDependencyGroup *depGroup =
          self()->getDependencyConditions()->getPostConditions();
      for (int j = 0;
           j < self()->getDependencyConditions()->getNumPostConditions(); ++j) {
        TR::RegisterDependency *dep = depGroup->getRegisterDependency(j);
        machine->setVirtualAssociatedWithReal(dep->getRealRegister(),
                                              dep->getRegister());
      }
...
bool OMR::X86::Instruction::refsRegister(TR::Register *reg) {
  return self()->getDependencyConditions() ? _conditions->refsRegister(reg)
                                           : false;
}

bool OMR::X86::Instruction::defsRegister(TR::Register *reg) {
  return self()->getDependencyConditions() ? _conditions->defsRegister(reg)
                                           : false;
}

bool OMR::X86::Instruction::usesRegister(TR::Register *reg) {
  return self()->getDependencyConditions() ? _conditions->usesRegister(reg)
                                           : false;
}
...
uint32_t
OMR::X86::Instruction::totalReferencedGPRegisters(TR::CodeGenerator *cg) {
  if (self()->getDependencyConditions()) {
...
uint32_t
OMR::X86::Instruction::totalReferencedFPRegisters(TR::CodeGenerator *cg) {
  if (self()->getDependencyConditions()) {
...
void OMR::X86::Instruction::adjustVFPState(TR_VFPState *state,
                                           TR::CodeGenerator *cg) {
  ...
  if (state->_register == TR::RealRegister::esp) {
    if (self()->getOpCode().isPushOp())
      ...
      state->_displacement -= TR::Compiler->om.sizeofReferenceAddress();
    else if (self()->getOpCodeValue() == RET ||
             self()->getOpCodeValue() == RETImm2 ||
             self()->getOpCodeValue() == ReturnMarker)
      *state = cg->vfpResetInstruction()->getSavedState();
  }
}

void OMR::X86::Instruction::adjustVFPStateForCall(TR_VFPState *state,
                                                  int32_t vfpAdjustmentForCall,
                                                  TR::CodeGenerator *cg) {
  if (state->_register == TR::RealRegister::esp &&
      self()->getOpCode().isCallOp()) {
...
void OMR::X86::Instruction::clobberRegsForRematerialisation() {
  ...
  if (self()->cg()->enableRematerialisation() &&
      ...
      (self()->getOpCodeValue() !=
       ASSOCREGS) // reg associations aren't really instructions, so they don't
                  // modify anything
      &&
      (self()->getOpCodeValue() != LABEL) // labels must already be handled
                                          // properly for a variety of reasons
      && (!self()->getOpCode().isShiftOp()) &&
      (!self()->getOpCode().isRotateOp()) // shifts and rotates often have a
                                          // postcondition on ecx but don't
                                          // clobber it
  ) {
    ...
    TR_X86RegisterDependencyGroup *post =
        self()->getDependencyConditions()->getPostConditions();
    for (uint32_t i = 0;
         i < self()->getDependencyConditions()->getNumPostConditions(); i++) {
      ...
      if (reg->isDiscardable()) {
        ...
        if (debug("dumpRemat"))
          diagnostic("---> Clobbering %s discardable postcondition register %s "
                     "at instruction %s\n",
                     self()->cg()->getDebug()->toString(
                         reg->getRematerializationInfo()),
...
uint8_t OMR::X86::Instruction::operandSizeRexBits() // Rex bits indicating
                                                    // 32/64-bit operand sizes
{
...
uint8_t OMR::X86::Instruction::rexBits() {
  return self()->operandSizeRexBits();
}
#endif

bool OMR::X86::Instruction::isLabel() {
...
void TR_X86OpCode::trackUpperBitsOnReg(TR::Register *reg,
                                       TR::CodeGenerator *cg) {
