...
void OMR::X86::Instruction::initialize(
  TR::CodeGenerator* cg, TR::RegisterDependencyConditions* cond, TR_X86OpCodes op, bool flag)
{
...
{
  TR_ASSERT(
    !(TR::Compiler->target.is64Bit() && self()->getOpCode().isIA32Only()), "Cannot use invalid AMD64 instructions");
}
...
void OMR::X86::Instruction::adjustVFPState(TR_VFPState* state, TR::CodeGenerator* cg)
{
  ...
  if (state->_register == TR::RealRegister::esp) {
    if (self()->getOpCode().isPushOp())
      ...
      state->_displacement -= TR::Compiler->om.sizeofReferenceAddress();
    else if (self()->getOpCodeValue() == RET || self()->getOpCodeValue() == RETImm2
...
void OMR::X86::Instruction::clobberRegsForRematerialisation()
{
  ...
  if (self()->cg()->enableRematerialisation() && self()->getDependencyConditions()
    && (self()->getOpCodeValue()
      != ASSOCREGS) // reg associations aren't really instructions, so they don't modify anything
      ...
      if (reg->isDiscardable()) {
        ...
        if (debug("dumpRemat"))
          diagnostic("---> Clobbering %s discardable postcondition register %s at instruction %s\n",
            self()->cg()->getDebug()->toString(reg->getRematerializationInfo()), self()->cg()->getDebug()->getName(reg),
