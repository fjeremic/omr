...
void
OMR::X86::Instruction::initialize(TR::CodeGenerator* cg,
...
void
OMR::X86::Instruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
  ...
  if (self()->getOpCodeValue() != ASSOCREGS) {
    ...
    if (kindsToBeAssigned & TR_GPR_Mask) {
      ...
      for (int j = 0;
           j < self()->getDependencyConditions()->getNumPostConditions();
           ++j) {
...
bool
OMR::X86::Instruction::refsRegister(TR::Register* reg)
{
...
bool
OMR::X86::Instruction::defsRegister(TR::Register* reg)
{
...
bool
OMR::X86::Instruction::usesRegister(TR::Register* reg)
{
...
bool
OMR::X86::Instruction::dependencyRefsRegister(TR::Register* reg)
{
  return self()->getDependencyConditions() ? _conditions->refsRegister(reg)
                                           : false;
}
...
void
OMR::X86::Instruction::adjustVFPState(TR_VFPState* state, TR::CodeGenerator* cg)
{
...
void
OMR::X86::Instruction::adjustVFPStateForCall(TR_VFPState* state,
...
OMR::X86::Instruction::clobberRegsForRematerialisation()
{
  ...
  if (self()->cg()->enableRematerialisation() &&
    ...
    TR_X86RegisterDependencyGroup* post =
      self()->getDependencyConditions()->getPostConditions();
    for (uint32_t i = 0;
         i < self()->getDependencyConditions()->getNumPostConditions();
         i++) {
      ...
      if (reg->isDiscardable()) {
        ...
        if (debug("dumpRemat"))
          diagnostic(
            "---> Clobbering %s discardable postcondition register %s at "
            "instruction %s\n",
...
uint8_t
OMR::X86::Instruction::operandSizeRexBits() // Rex bits indicating 32/64-bit
                                            // operand sizes
{
...
void
TR_X86OpCode::trackUpperBitsOnReg(TR::Register* reg, TR::CodeGenerator* cg)
{
