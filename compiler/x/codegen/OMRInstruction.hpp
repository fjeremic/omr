...
class EnlargementResult {
  int32_t _patchGrowth; // the minimum guaratneed size of increase - we know we
                        // will grow this much
  int32_t _encodingGrowth; // the maximum size of increase - we have to
                           // accomodate growth of up to this much
public:
...
protected:
  ...
  void initialize(TR::CodeGenerator *cg = NULL,
                  TR::RegisterDependencyConditions *cond = NULL,
                  TR_X86OpCodes op = BADIA32Op, bool flag = false);
...
public:
  ...
  TR_X86OpCodes setOpCodeValue(TR_X86OpCodes op) {
  ...
  virtual EnlargementResult enlarge(int32_t requestedEnlargementSize,
...
protected:
  ...
  // If _opcode->isCallOp(), adds vfpAdjustmentForCall; else, calls
  // TR::Instruction ::adjustVFPState.
  //
  void adjustVFPStateForCall(TR_VFPState *state, int32_t vfpAdjustmentForCall,
                             TR::CodeGenerator *cg);
