...
TR::Register*
TR_X86IntegerMultiplyDecomposer::decomposeIntegerMultiplier(
    ...
    if (decompositionIndex != -1) {
      ...
      if (composition._subsequentShiftTooExpensive == false) {
        ...
        if (shiftAmount < 3 && !cg()->getX86ProcessorInfo().isIntelCore2() &&
            ...
            !cg()
               ->getX86ProcessorInfo()
               .isAMDOpteron()) // TODO:: P3 should go straight to else and use
                                // shift always
        {
          for (; shiftAmount > 0; --shiftAmount) {
            generateRegRegInstruction(
              ADDRegReg(nodeIs64Bit), _node, target, target, _cg);
          }
        } else {
          generateRegImmInstruction(
            SHLRegImm1(nodeIs64Bit), _node, target, shiftAmount, _cg);
        }
        if (_multiplier !=
    ...
    if (onesCount == 2) // sum of 2 powers of 2
    {
      if (_sourceRegister) {
        if (!_canClobberSource && (absMultiplier & 1) == 0) {
          ...
          generateRegRegInstruction(
            MOVRegReg(nodeIs64Bit), _node, temp, _sourceRegister, _cg);
      ...
      generateRegRegInstruction(
        MOVRegReg(nodeIs64Bit), _node, target, _sourceRegister, _cg);
      if (absMultiplier & 1) {
        shiftAmount = trailingZeroes(absMultiplier - 1);
        generateRegImmInstruction(
          SHLRegImm1(nodeIs64Bit), _node, target, shiftAmount, _cg);
        generateRegRegInstruction(
          ADDRegReg(nodeIs64Bit), _node, target, _sourceRegister, _cg);
      } else if (absMultiplier & 2) {
        shiftAmount = trailingZeroes(absMultiplier - 2);
        generateRegImmInstruction(
          SHLRegImm1(nodeIs64Bit), _node, target, shiftAmount, _cg);
        generateRegRegInstruction(
          ADDRegReg(nodeIs64Bit), _node, _sourceRegister, _sourceRegister, _cg);
        generateRegRegInstruction(
          ADDRegReg(nodeIs64Bit), _node, target, _sourceRegister, _cg);
        ...
        generateRegImmInstruction(
          SHLRegImm1(nodeIs64Bit), _node, _sourceRegister, shiftAmount, _cg);
        generateRegRegInstruction(
          ADDRegReg(nodeIs64Bit), _node, target, _sourceRegister, _cg);
      }
      if (_multiplier != absMultiplier) // treats TR::getMinSigned<TR::Int64>()
               ...
               64) // difference of 2 powers of two
    {
      ...
      if (absMultiplier & 1) {
        ...
        generateRegImmInstruction(
          SHLRegImm1(nodeIs64Bit), _node, target, shiftAmount, _cg);
        generateRegRegInstruction(
          SUBRegReg(nodeIs64Bit), _node, target, _sourceRegister, _cg);
      } else if (absMultiplier & 2) {
        shiftAmount = trailingZeroes(absMultiplier + 2);
        generateRegImmInstruction(
          SHLRegImm1(nodeIs64Bit), _node, target, shiftAmount, _cg);
...
int32_t
TR_X86IntegerMultiplyDecomposer::findDecomposition(int64_t multiplier)
{
  ...
  TR::Register** tempRegArray)
{
  ...
  if (composition._mustClobberSource == true) {
    if (_sourceRegister) {
      ...
      _sourceRegister =
        _cg->gprClobberEvaluate(_node->getFirstChild(), MOVRegReg(nodeIs64Bit));
    }
...
bool
TR_X86IntegerMultiplyDecomposer::hasDecomposition(int64_t multiplier)
{
  // linear search for expediency
  // todo: binary search
  int i;
