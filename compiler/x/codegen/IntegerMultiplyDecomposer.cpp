    ...
    if (decompositionIndex != -1) {
        ...
        if (decompositionIndex != -1) {
            ...
            if (composition._subsequentShiftTooExpensive == false) {
                target = generateDecompositionInstructions(decompositionIndex, tempRegArraySize, tempRegArray);
                if (shiftAmount < 3 && !cg()->getX86ProcessorInfo().isIntelCore2() && !cg()->getX86ProcessorInfo().isIntelNehalem() && !cg()->getX86ProcessorInfo().isIntelWestmere() && !cg()->getX86ProcessorInfo().isIntelSandyBridge() && !cg()->getX86ProcessorInfo().isAMD15h() && !cg()->getX86ProcessorInfo().isAMDOpteron()) // TODO:: P3 should go straight to else and use shift always
                {
