...
// decomposeIntegerMultiplier needs to update this array if it allocates new
// temporary registers inside
TR::Register* TR_X86IntegerMultiplyDecomposer::decomposeIntegerMultiplier(
    int32_t& tempRegArraySize,
  ...
  if (disable && absMultiplier != 3 && absMultiplier != 5 &&
      absMultiplier != 9 && !isPowerOf2(absMultiplier)) {
  ...
  if (decompositionIndex != -1) {
    target = generateDecompositionInstructions(decompositionIndex,
                                               tempRegArraySize, tempRegArray);
    if (_multiplier != absMultiplier)  // treats TR::getMinSigned<TR::Int64>()
                                       // and TR::getMinSigned<TR::Int32>()
    ...
    decompositionIndex = findDecomposition(
        (uint64_t)absMultiplier >>
        shiftAmount);  // cast needed so TR::getMinSigned<TR::Int64>() works
    if (decompositionIndex != -1) {
      const integerMultiplyComposition& composition =
          _integerMultiplySolutions[decompositionIndex];
      if (composition._subsequentShiftTooExpensive == false) {
        target = generateDecompositionInstructions(
            decompositionIndex, tempRegArraySize, tempRegArray);
        if (shiftAmount < 3 && !cg()->getX86ProcessorInfo().isIntelCore2() &&
            ...
            !cg()->getX86ProcessorInfo()
                 .isAMDOpteron())  // TODO:: P3 should go straight to else and
                                   // use shift always
        {
          for (; shiftAmount > 0; --shiftAmount) {
            generateRegRegInstruction(ADDRegReg(nodeIs64Bit), _node, target,
                                      target, _cg);
          }
        } else {
          generateRegImmInstruction(SHLRegImm1(nodeIs64Bit), _node, target,
                                    shiftAmount, _cg);
        }
        if (_multiplier !=
            absMultiplier)  // treats TR::getMinSigned<TR::Int64>() properly,
                            // _multiplier < 0 does not
        {
          generateRegInstruction(NEGReg(nodeIs64Bit), _node, target, _cg);
        }
        if (report) {
    ...
    if (onesCount == 2)  // sum of 2 powers of 2
    {
      if (_sourceRegister) {
        if (!_canClobberSource && (absMultiplier & 1) == 0) {
          ...
          if (tempRegArray) {
            TR_ASSERT(tempRegArraySize <= MAX_NUM_REGISTERS,
                      "Too many temporary registers to handle");
            tempRegArray[tempRegArraySize++] = temp;
          }
          generateRegRegInstruction(MOVRegReg(nodeIs64Bit), _node, temp,
                                    _sourceRegister, _cg);
          _sourceRegister = temp;
        }
      } else {
        _sourceRegister = _cg->gprClobberEvaluate(_node->getFirstChild(),
      ...
      generateRegRegInstruction(MOVRegReg(nodeIs64Bit), _node, target,
                                _sourceRegister, _cg);
      if (absMultiplier & 1) {
        ...
        generateRegImmInstruction(SHLRegImm1(nodeIs64Bit), _node, target,
                                  shiftAmount, _cg);
        generateRegRegInstruction(ADDRegReg(nodeIs64Bit), _node,
                                  _sourceRegister, _sourceRegister, _cg);
        generateRegRegInstruction(ADDRegReg(nodeIs64Bit), _node, target,
                                  _sourceRegister, _cg);
        ...
        generateRegImmInstruction(SHLRegImm1(nodeIs64Bit), _node, target,
                                  shiftAmount, _cg);
        shiftAmount =
            trailingZeroes((int64_t)(absMultiplier - (1ll << shiftAmount)));
        generateRegImmInstruction(SHLRegImm1(nodeIs64Bit), _node,
                                  _sourceRegister, shiftAmount, _cg);
        generateRegRegInstruction(ADDRegReg(nodeIs64Bit), _node, target,
                                  _sourceRegister, _cg);
      }
      if (_multiplier != absMultiplier)  // treats TR::getMinSigned<TR::Int64>()
      ...
      return target;
    } else if (onesCount + trailingZeroes(absMultiplier) +
                   leadingZeroes(absMultiplier) ==
               64)  // difference of 2 powers of two
    {
      ...
      if (absMultiplier & 1) {
        ...
        generateRegImmInstruction(SHLRegImm1(nodeIs64Bit), _node, target,
                                  shiftAmount, _cg);
        generateRegRegInstruction(SUBRegReg(nodeIs64Bit), _node, target,
                                  _sourceRegister, _cg);
      } else if (absMultiplier & 2) {
        shiftAmount = trailingZeroes(absMultiplier + 2);
        generateRegImmInstruction(SHLRegImm1(nodeIs64Bit), _node, target,
...
int32_t TR_X86IntegerMultiplyDecomposer::findDecomposition(int64_t multiplier) {
  ...
  // todo: binary search
  int i;
  for (i = 0; (i < NUM_CONSTS_DECOMPOSED) &&
              (_integerMultiplySolutions[i]._multiplier < multiplier);
       ++i) {
    ;
  }

  if ((i < NUM_CONSTS_DECOMPOSED) &&
      (_integerMultiplySolutions[i]._multiplier == multiplier)) {
    const integerMultiplyComposition& composition =
        _integerMultiplySolutions[i];
    ...
    int32_t numLiveRegs =
        _cg->getLiveRegisters(TR_GPR)->getNumberOfLiveRegisters();
    int32_t numGPRRegs = _cg->machine()->getNumberOfGPRs();
    int32_t registerCostOfDecomposition =
        composition._numAdditionalRegistersNeeded -
        correctionIfTakeAdvantageOfClobberableSource;
    if (registerCostOfDecomposition <=
            1 ||  // mul would have a default register cost, too
...
TR::Register*
TR_X86IntegerMultiplyDecomposer::generateDecompositionInstructions(
    int32_t index,
    int32_t& tempRegArraySize,
    TR::Register** tempRegArray) {
  const integerMultiplyComposition& composition =
      _integerMultiplySolutions[index];
  bool nodeIs64Bit = getNodeIs64Bit(_node, _cg);
  ...
  // source register should only be null if integer multiply, so safe to assume
  // first child is evaluated to generate it.
  ...
  for (int32_t cursor = 0; cursor < MAX_NUM_COMPONENTS; ++cursor) {
    ...
    switch (component._operation) {
      ...
      case addRegReg:
        ...
        break;
      case subRegReg:
        generateRegRegInstruction(SUBRegReg(nodeIs64Bit), _node,
                                  registerMap[component._target],
                                  registerMap[component._baseOrImmed], _cg);
        break;
      case movRegReg:
        generateRegRegInstruction(MOVRegReg(nodeIs64Bit), _node,
                                  registerMap[component._target],
                                  registerMap[component._baseOrImmed], _cg);
        break;
      case leaRegReg2:
        generateRegMemInstruction(
            LEARegMem(nodeIs64Bit), _node, registerMap[component._target],
            generateX86MemoryReference(
                0, registerMap[component._index],
      ...
      case done: {
        ...
        for (int32_t i = 1; i <= composition._numAdditionalRegistersNeeded;
             ++i) {
...
bool TR_X86IntegerMultiplyDecomposer::hasDecomposition(int64_t multiplier) {
  ...
  return (i < NUM_CONSTS_DECOMPOSED) &&
         (_integerMultiplySolutions[i]._multiplier == multiplier);
}

const TR_X86IntegerMultiplyDecomposer::integerMultiplyComposition
    TR_X86IntegerMultiplyDecomposer::_integerMultiplySolutions
        [NUM_CONSTS_DECOMPOSED] = {
            {
                1,      // multiplier
                ...
                {{done, 0, 0,
                  0}}  // source * 1, holder used to catch pure shifts
            },
            {3,                          // multiplier
             true,                       // can be assigned with one register
