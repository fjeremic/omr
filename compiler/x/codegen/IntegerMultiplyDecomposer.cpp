  ...
  int32_t& tempRegArraySize, TR::Register** tempRegArray)
{
  ...
  if (decompositionIndex != -1) {
    ...
    decompositionIndex
      = findDecomposition((uint64_t)absMultiplier >> shiftAmount); // cast needed so TR::getMinSigned<TR::Int64>() works
    if (decompositionIndex != -1) {
      ...
      if (composition._subsequentShiftTooExpensive == false) {
        ...
        if (shiftAmount < 3 && !cg()->getX86ProcessorInfo().isIntelCore2()
          && !cg()->getX86ProcessorInfo().isIntelNehalem() && !cg()->getX86ProcessorInfo().isIntelWestmere()
          && !cg()->getX86ProcessorInfo().isIntelSandyBridge() && !cg()->getX86ProcessorInfo().isAMD15h()
  ...
  for (int32_t cursor = 0; cursor < MAX_NUM_COMPONENTS; ++cursor) {
    ...
    case shlRegImm:
      generateRegImmInstruction(
        SHLRegImm1(nodeIs64Bit), _node, registerMap[component._target], component._baseOrImmed, _cg);
      break;
    case addRegReg:
      generateRegRegInstruction(
        ADDRegReg(nodeIs64Bit), _node, registerMap[component._target], registerMap[component._baseOrImmed], _cg);
      break;
    case subRegReg:
      generateRegRegInstruction(
        SUBRegReg(nodeIs64Bit), _node, registerMap[component._target], registerMap[component._baseOrImmed], _cg);
      break;
    case movRegReg:
      generateRegRegInstruction(
        MOVRegReg(nodeIs64Bit), _node, registerMap[component._target], registerMap[component._baseOrImmed], _cg);
      break;
    case leaRegReg2:
      generateRegMemInstruction(LEARegMem(nodeIs64Bit), _node, registerMap[component._target],
