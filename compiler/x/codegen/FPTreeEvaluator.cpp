...
TR::Register* OMR::X86::TreeEvaluator::coerceFPRToXMMR(TR::Node* node, TR::Register* fpRegister, TR::CodeGenerator* cg)
{
  ...
  if (fpRegister->isSinglePrecision()) {
    ...
    generateRegMemInstruction(
      cg->getXMMDoubleLoadOpCode(), node, xmmRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
  }
...
void OMR::X86::TreeEvaluator::insertPrecisionAdjustment(TR::Register* reg, TR::Node* root, TR::CodeGenerator* cg)
{
  TR::DataType dt;
...
#ifdef DEBUG
  else if (!(node->getOpCodeValue() == TR::f2d) && !(node->getOpCode().isFloat())
    && !(node->getOpCodeValue() == TR::f2i) && !(node->getOpCodeValue() == TR::f2l)) {
  ...
  if (cg->useSSEForSinglePrecision()) {
    ...
    if (node->getFloatBits() == 0) {
      ...
      setDiscardableIfPossible(
        TR_RematerializableFloat, targetRegister, node, instr, (intptrj_t)node->getFloatBits(), cg);
    }
  } else {
  ...
  TR::Node* node, TR::MemoryReference* sourceMR, TR::CodeGenerator* cg)
{
  ...
  if (cg->useSSEForSinglePrecision()) {
    if (TR::Compiler->target.is64Bit() && sourceMR->getSymbolReference().isUnresolved()) {
...
TR::Register* OMR::X86::TreeEvaluator::floatingPointStoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (valueChild->getOpCode().isLoadConst()) {
    if (nodeIs64Bit) {
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateMemImmInstruction(
          S4MemImm4, node, generateX86MemoryReference(*tempMR, 4, cg), valueChild->getLongIntHigh(), cg);
      }
    } else {
    ...
    if (sourceRegister->getKind() == TR_FPR) {
      if (TR::Compiler->target.is64Bit() && tempMR->getSymbolReference().isUnresolved()) {
        ...
        if (!tempMR->getSymbolReference().getSymbol()->isShadow()
          ...
          // The 64-bit static case does not require the LEA instruction as we can resolve the address in the MOV reg,
          // imm  instruction preceeding the store.
          //
...
TR::Register* OMR::X86::TreeEvaluator::fpReturnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is32Bit() && !cg->useSSEForDoublePrecision() && returnRegister->getKind() == TR_FPR) {
...
TR::Register* OMR::X86::TreeEvaluator::fpBinaryArithmeticEvaluator(TR::Node* node, bool isFloat, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::X86::TreeEvaluator::fpUnaryMaskEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (value->getKind() != TR_FPR) // Legacy supported for X87, to be deleted
  {
    ...
    generateRegMemInstruction(
      opcode, node, result, generateX86MemoryReference(cg->findOrCreate16ByteConstant(node, mask), cg), cg);
  }
...
TR::Register* OMR::X86::TreeEvaluator::fpRemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (cg->useSSEForDoublePrecision()) // Note: both float and double helpers use SSE2
  {
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      TR::SymbolReference* helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
        nodeIsDouble ? TR_AMD64doubleRemainder : TR_AMD64floatRemainder, false, false, false);
      targetRegister
        = TR::TreeEvaluator::performHelperCall(node, helperSymRef, nodeIsDouble ? TR::dcall : TR::fcall, false, cg);
    } else {
      TR::SymbolReference* helperSymRef = cg->symRefTab()->findOrCreateRuntimeHelper(
        nodeIsDouble ? TR_IA32doubleRemainderSSE : TR_IA32floatRemainderSSE, false, false, false);
      targetRegister
        = TR::TreeEvaluator::performHelperCall(node, helperSymRef, nodeIsDouble ? TR::dcall : TR::fcall, false, cg);
    }
  } else {
  ...
  if ((!nodeIsDouble && cg->useSSEForSinglePrecision()) || (nodeIsDouble && cg->useSSEForDoublePrecision()))
    return targetRegister;

  if (nodeIsDouble && (comp->getCurrentMethod()->isStrictFP() || comp->getOption(TR_StrictFP))) {
    // Strict double op.
    //
...
TR::Register* OMR::X86::TreeEvaluator::commonFPRemEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool isDouble)
{
  TR::Node* divisor = node->getSecondChild();
  TR::Node* dividend = node->getFirstChild();
  TR::Compilation* comp = cg->comp();
  ...
  TR::Node* node, TR::SymbolReference* helperSymRef, TR::CodeGenerator* cg)
{
  ...
  bool optimizeF2IWithSSE = (node->getOpCodeValue() == TR::f2i && cg->getX86ProcessorInfo().supportsSSE());

  bool optimizeD2IWithSSE2 = (node->getOpCodeValue() == TR::d2i && cg->getX86ProcessorInfo().supportsSSE2());
  ...
  if (!optimizeF2IWithSSE && !optimizeD2IWithSSE2) {
    ...
    fpcw = comp->getJittedMethodSymbol()->usesSinglePrecisionMode() ? SINGLE_PRECISION_ROUND_TO_ZERO
    ...
    generateMemInstruction(
      LDCWMem, node, generateX86MemoryReference(cg->findOrCreate2ByteConstant(node, fpcw), cg), cg);
    ...
    if (optimizeF2IWithSSE) {
      if (child->getReferenceCount() == 1 && child->getRegister() == 0 && child->getOpCode().isMemoryReference()) {
        tempMR = generateX86MemoryReference(child, cg);
        ...
        loadInstr = generateRegMemInstruction(
          CVTTSS2SIReg4Mem, node, resultReg, generateX86MemoryReference(*tempMR, 0, cg), cg);
        tempMR->decNodeReferenceCounts(cg);
      } else {
      ...
      if (child->getReferenceCount() == 1 && child->getRegister() == 0 && child->getOpCode().isMemoryReference()) {
        ...
        resultReg = cg->allocateRegister();
        loadInstr = generateRegMemInstruction(
          CVTTSD2SIReg4Mem, node, resultReg, generateX86MemoryReference(*tempMR, 0, cg), cg);
        tempMR->decNodeReferenceCounts(cg);
      } else {
  ...
  //
  cg->addSnippet(
    new (cg->trHeapMemory()) TR::X86FPConvertToIntSnippet(reStartLabel, snippetLabel, helperSymRef, loadInstr, cg));
  ...
  TR::Node* node, TR::SymbolReference* helperSymRef, TR::CodeGenerator* cg)
{
  ...
  if (child->getOpCode().isDouble() && cg->useSSEForDoublePrecision()) {
    ...
    TR::SymbolReference* d2l
      = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_IA32double2LongSSE, false, false, false);
    ...
    TR_OutlinedInstructions* outlinedHelperCall
      = new (cg->trHeapMemory()) TR_OutlinedInstructions(node, TR::lcall, targetRegister, CallLabel, reStartLabel, cg);
    ...
    TR::Register* tempFPR1
      = child->getOpCode().isFloat() ? cg->allocateSinglePrecisionRegister(TR_X87) : cg->allocateRegister(TR_X87);
    ...
    //
    loadHighInstr
      = generateRegMemInstruction(L4RegMem, node, highReg, generateX86MemoryReference(*convertedLongMR, 4, cg), cg);

    loadLowInstr
      = generateRegMemInstruction(L4RegMem, node, lowReg, generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
    ...
    cg->addSnippet(new (cg->trHeapMemory()) TR::X86FPConvertToLongSnippet(
      reStartLabel, snippetLabel, helperSymRef, clobInstruction, loadHighInstr, loadLowInstr, cg));
...
TR::Register* OMR::X86::TreeEvaluator::f2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (cg->useSSEForSinglePrecision()) {
    ...
    if (sourceRegister->getKind() == TR_X87 && child->getReferenceCount() == 1) {
      ...
      generateRegMemInstruction(CVTTSS2SIReg4Mem, node, targetRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
...

    {
      ...
      generateRegMemInstruction(doubleSource ? UCOMISDRegMem : UCOMISSRegMem, node, sourceRegister,
      ...
      // right
      generateRegMemInstruction(CMOVARegMem(longTarget), node, targetRegister,
      ...
      generateRegMemInstruction(CMOVPRegMem(longTarget), node, targetRegister,
        generateX86MemoryReference(
    ...
    if (sourceRegister && sourceRegister->getKind() == TR_X87 && child->getReferenceCount() == 1) {
    ...
    TR_ASSERT(TR::Compiler->target.is32Bit(), "assertion failure");
    return TR::TreeEvaluator::fpConvertToInt(node,
      cg->symRefTab()->findOrCreateRuntimeHelper(
        node->getOpCodeValue() == TR::f2i ? TR_IA32floatToInt : TR_IA32doubleToInt, false, false, false),
      cg);
  }
}
...
TR::Register* OMR::X86::TreeEvaluator::f2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  return TR::TreeEvaluator::fpConvertToLong(
    node, cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32floatToLong, false, false, false), cg);
}
...
TR::Register* OMR::X86::TreeEvaluator::d2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  return TR::TreeEvaluator::fpConvertToLong(
    node, cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32doubleToLong, false, false, false), cg);
}
...
TR::Register* OMR::X86::TreeEvaluator::fRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (globalReg == NULL) {
    if (cg->useSSEForSinglePrecision()) {
      ...
      if (!comp->getJittedMethodSymbol()->usesSinglePrecisionMode() && node->needsPrecisionAdjustment()) {
    ...
    } else if (childGlobalReg && childGlobalReg != globalReg) {
      {
        ...
        for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
          if (i != fpStackSlot) {
            ...
            //   fpStackSlot, i+machine->getNumGlobalGPRs(), reg, getDebug()->getName(childGlobalReg));
            // else
            //   dumpOptDetails("Global reg %d Stack slot %d iglobalreg %d reg %s childreg %s\n", globalRegNum,
            //   fpStackSlot, i+machine->getNumGlobalGPRs(), getDebug()->getName(reg),
...
TR::Register* OMR::X86::TreeEvaluator::generateBranchOrSetOnFPCompare(
  TR::Node* node, TR::Register* accRegister, bool generateBranch, TR::CodeGenerator* cg)
{
  ...
  if (accRegister == NULL
    && (cmpOp == TR::iffcmpneu || cmpOp == TR::fcmpneu || cmpOp == TR::ifdcmpneu || cmpOp == TR::dcmpneu)) {
    if (generateBranch) {
    ...
    && (cmpOp == TR::iffcmpeq || cmpOp == TR::fcmpeq || cmpOp == TR::ifdcmpeq || cmpOp == TR::dcmpeq)) {
    if (generateBranch) {
      TR::LabelSymbol* startLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
...
//
TR::Register* OMR::X86::TreeEvaluator::compareFloatOrDoubleForOrder(TR::Node* node, TR_X86OpCodes fpCmpRegRegOpCode,
  TR_X86OpCodes fpCmpRegMemOpCode, TR_X86OpCodes fpCmpiRegRegOpCode, TR_X86OpCodes xmmCmpRegRegOpCode,
  TR_X86OpCodes xmmCmpRegMemOpCode, bool useFCOMIInstructions, TR::CodeGenerator* cg)
{
  if (((TR_X86OpCode::singleFPOp(fpCmpRegRegOpCode) && cg->useSSEForSinglePrecision())
    ...
    return temp.fpCompareAnalyser(node, fpCmpRegRegOpCode, fpCmpRegMemOpCode, fpCmpiRegRegOpCode, useFCOMIInstructions);
  }
}
...
bool OMR::X86::TreeEvaluator::canUseFCOMIInstructions(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  return (!cg->getX86ProcessorInfo().supportsFCOMIInstructions() || cmpOp == TR::iffcmpneu || cmpOp == TR::iffcmpeq
           || cmpOp == TR::ifdcmpneu || cmpOp == TR::ifdcmpeq || cmpOp == TR::fcmpneu || cmpOp == TR::dcmpneu
           || cmpOp == TR::fcmpeq || cmpOp == TR::dcmpeq)
    ? false
    : true;
}
...
TR::Register* OMR::X86::TreeEvaluator::compareFloatAndBranchEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::Register* accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
    node, FCOMRegReg, FCOMRegMem, FCOMIRegReg, UCOMISSRegReg, UCOMISSRegMem, useFCOMIInstructions, cg);
...
TR::Register* OMR::X86::TreeEvaluator::compareDoubleAndBranchEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  bool useFCOMIInstructions = TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
  TR::Register* accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
    node, DCOMRegReg, DCOMRegMem, DCOMIRegReg, UCOMISDRegReg, UCOMISDRegMem, useFCOMIInstructions, cg);
  return TR::TreeEvaluator::generateBranchOrSetOnFPCompare(node, accRegister, true, cg);
}
