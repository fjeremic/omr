...
TR::Register *OMR::X86::TreeEvaluator::coerceFPRToXMMR(TR::Node *node,
                                                       TR::Register *fpRegister,
                                                       TR::CodeGenerator *cg) {
  TR_ASSERT(fpRegister && fpRegister->getKind() == TR_X87,
            "incorrect register type for XMMR coercion\n");
  ...
  if (fpRegister->isSinglePrecision()) {
    ...
    generateRegMemInstruction(MOVSSRegMem, node, xmmRegister,
    ...
    generateRegMemInstruction(cg->getXMMDoubleLoadOpCode(), node, xmmRegister,
                              generateX86MemoryReference(*tempMR, 0, cg), cg);
  }
...
void OMR::X86::TreeEvaluator::coerceFPOperandsToXMMRs(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  for (int i = 0; i < node->getNumChildren(); i++) {
    ...
    if (reg &&
        reg->getKind() == TR_X87 /* && child->getReferenceCount() > 1 */) {
      TR::TreeEvaluator::coerceFPRToXMMR(child, reg, cg);
    }
  }
}

TR::Register *OMR::X86::TreeEvaluator::coerceXMMRToFPR(
    TR::Node *node, TR::Register *xmmRegister, TR::CodeGenerator *cg) {
  TR_ASSERT(xmmRegister && xmmRegister->getKind() == TR_FPR,
            "incorrect register type for FPR coercion\n");
  ...
  if (xmmRegister->isSinglePrecision()) {
    ...
    generateFPRegMemInstruction(FLDRegMem, node, fpRegister,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
  } else {
    fpRegister = cg->allocateRegister(TR_X87);
    TR::MemoryReference *tempMR = cg->machine()->getDummyLocalMR(TR::Double);
    generateMemRegInstruction(MOVSDMemReg, node, tempMR, xmmRegister, cg);
    generateFPRegMemInstruction(DLDRegMem, node, fpRegister,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
  }
...
void OMR::X86::TreeEvaluator::insertPrecisionAdjustment(TR::Register *reg,
  ...
  if ((node->getOpCode().isDouble() && (node->getOpCodeValue() != TR::f2d)) ||
      (node->getOpCode().isBooleanCompare() &&
       node->getFirstChild()->getDataType() != TR::Float) ||
      node->getOpCodeValue() == TR::d2i || node->getOpCodeValue() == TR::d2l) {
    useFloatSet = false;
  }
#ifdef DEBUG
    ...
    diagnostic(
        "insertPrecisionAdjustment() ==> invalid node type for precision "
        "adjustment!");
  }
#endif
...
TR::Register *OMR::X86::TreeEvaluator::fconstEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  ...
  if (cg->useSSEForSinglePrecision()) {
    ...
    if (node->getFloatBits() == 0) {
      generateRegRegInstruction(XORPSRegReg, node, targetRegister,
                                targetRegister, cg);
    } else {
      TR::MemoryReference *tempMR = generateX86MemoryReference(
          cg->findOrCreate4ByteConstant(node, node->getFloatBits()), cg);
      TR::Instruction *instr = generateRegMemInstruction(
          MOVSSRegMem, node, targetRegister, tempMR, cg);
      setDiscardableIfPossible(TR_RematerializableFloat, targetRegister, node,
                               instr, (intptrj_t)node->getFloatBits(), cg);
    }
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::dconstEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *targetRegister;
  if (cg->useSSEForDoublePrecision()) {
    ...
    if (node->getLongInt() == 0) {
      generateRegRegInstruction(XORPDRegReg, node, targetRegister,
                                targetRegister, cg);
    } else {
      TR::MemoryReference *tempMR = generateX86MemoryReference(
          cg->findOrCreate8ByteConstant(node, node->getLongInt()), cg);
      generateRegMemInstruction(cg->getXMMDoubleLoadOpCode(), node,
    ...
    } else if (node->getLongInt() ==
               IEEE_DOUBLE_1_0)  // hex for ieee double 1.0
    {
...
TR::Register *OMR::X86::TreeEvaluator::performFload(
  ...
  if (cg->useSSEForSinglePrecision()) {
    if (TR::Compiler->target.is64Bit() &&
      ...
      // The 64-bit mode XMM load instructions may be wider than 8-bytes (our
      // patching window) but we won't know that for sure until after register
      // assignment. Hence, the unresolved memory reference must be evaluated
      ...
      targetRegister = cg->allocateSinglePrecisionRegister(TR_FPR);
      instr = generateRegMemInstruction(MOVSSRegMem, node, targetRegister,
                                        sourceMR, cg);
    } else {
      targetRegister = cg->allocateSinglePrecisionRegister(TR_FPR);
      instr = generateRegMemInstruction(MOVSSRegMem, node, targetRegister,
    ...
    targetRegister = cg->allocateSinglePrecisionRegister(TR_X87);
    instr = generateFPRegMemInstruction(FLDRegMem, node, targetRegister,
                                        sourceMR, cg);
  }
  if (node->getOpCode().isIndirect()) cg->setImplicitExceptionPoint(instr);
  node->setRegister(targetRegister);
  return targetRegister;
}

// also handles TR::floadi
TR::Register *OMR::X86::TreeEvaluator::floadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *tempMR = generateX86MemoryReference(node, cg);
  TR::Register *targetRegister =
      TR::TreeEvaluator::performFload(node, tempMR, cg);
...
TR::Register *OMR::X86::TreeEvaluator::performDload(
  ...
  if (cg->useSSEForDoublePrecision()) {
    if (TR::Compiler->target.is64Bit() &&
      ...
      // window) but we won't know that for sure until after register
      // assignment. Hence, the unresolved memory reference must be evaluated
      // into a register first.
      //
      TR::Register *memReg = cg->allocateRegister(TR_GPR);
    ...
    targetRegister = cg->allocateRegister(TR_FPR);
    instr = generateRegMemInstruction(cg->getXMMDoubleLoadOpCode(), node,
                                      targetRegister, sourceMR, cg);
  } else {
    targetRegister = cg->allocateRegister(TR_X87);
    instr = generateFPRegMemInstruction(DLDRegMem, node, targetRegister,
                                        sourceMR, cg);
  }
  if (node->getOpCode().isIndirect()) cg->setImplicitExceptionPoint(instr);
  node->setRegister(targetRegister);
...
TR::Register *OMR::X86::TreeEvaluator::dloadEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::MemoryReference *tempMR = generateX86MemoryReference(node, cg);
  TR::Register *targetRegister =
      TR::TreeEvaluator::performDload(node, tempMR, cg);
  tempMR->decNodeReferenceCounts(cg);
  return targetRegister;
}

TR::Register *OMR::X86::TreeEvaluator::floatingPointStoreEvaluator(
  ...
  //
  if ((valueChild->getOpCodeValue() == TR::ibits2f ||
       valueChild->getOpCodeValue() == TR::lbits2d) &&
    ...
    static TR::ILOpCodes integerOpCodes[2][2] = {{TR::istore, TR::lstore},
                                                 {TR::istorei, TR::lstorei}};
    TR::Node::recreate(
        node, integerOpCodes[nodeIsIndirect][(
                  valueChild->getOpCodeValue() == TR::ibits2f ? 0 : 1)]);
  ...
  if (valueChild->getOpCode().isLoadConst()) {
    if (nodeIs64Bit) {
      if (TR::Compiler->target.is64Bit()) {
        ...
        if (valueChild->getLongInt() == 0) {
          generateRegRegInstruction(XOR8RegReg, node, floatConstReg,
                                    floatConstReg, cg);
        } else {
          generateRegImm64Instruction(MOV8RegImm64, node, floatConstReg,
                                      valueChild->getLongInt(), cg);
        }
        exceptionPoint = generateMemRegInstruction(S8MemReg, node, tempMR,
                                                   floatConstReg, cg);
        cg->stopUsingRegister(floatConstReg);
      } else {
        exceptionPoint = generateMemImmInstruction(
            S4MemImm4, node, tempMR, valueChild->getLongIntLow(), cg);
      ...
      exceptionPoint = generateMemImmInstruction(
          S4MemImm4, node, tempMR, valueChild->getFloatBits(), cg);
    }
    TR::Register *firstChildReg = valueChild->getRegister();
    if (firstChildReg && firstChildReg->getKind() == TR_X87 &&
        valueChild->getReferenceCount() == 1)
      generateFPSTiST0RegRegInstruction(FSTRegReg, valueChild, firstChildReg,
                                        firstChildReg, cg);
    ...
    generateRegMemInstruction(LRegMem(nodeIs64Bit), node, tempRegister, loadMR,
                              cg);
    exceptionPoint = generateMemRegInstruction(SMemReg(nodeIs64Bit), node,
                                               tempMR, tempRegister, cg);
    ...
    if (sourceRegister->getKind() == TR_FPR) {
      if (TR::Compiler->target.is64Bit() &&
        ...
        if (!tempMR->getSymbolReference().getSymbol()->isShadow() &&
          ...
          // The 64-bit static case does not require the LEA instruction as we
          // can resolve the address in the MOV reg, imm  instruction preceeding
          // the store.
          //
          exceptionPoint = generateMemRegInstruction(
              MOVSMemReg(nodeIs64Bit), node, tempMR, sourceRegister, cg);
        } else {
          // The 64-bit store instructions may be wider than 8-bytes (our
          // patching window) but we won't know that for sure until after
          // register assignment. Hence, the unresolved memory reference must be
          // evaluated into a register first.
          //
          TR::Register *memReg = cg->allocateRegister(TR_GPR);
      ...
      exceptionPoint = generateFPMemRegInstruction(FSTMemReg, node, tempMR,
                                                   sourceRegister, cg);
    }
  }
  ...
  tempMR->decNodeReferenceCounts(cg);
  if (nodeIsIndirect) cg->setImplicitExceptionPoint(exceptionPoint);

  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::fpReturnEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *returnRegister = cg->evaluate(node->getFirstChild());
  TR_ASSERT(returnRegister,
            "Return node's child should evaluate to a register");
  ...
  if (TR::Compiler->target.is32Bit() && !cg->useSSEForDoublePrecision() &&
    ...
    returnRegister = TR::TreeEvaluator::coerceXMMRToFPR(node->getFirstChild(),
                                                        returnRegister, cg);
  ...
  TR::RealRegister::RegNum machineReturnRegister =
      (returnRegister->isSinglePrecision())
  ...
  if (comp->getJittedMethodSymbol()->getLinkageConvention() == TR_Private) {
    if (cg->useSSEForDoublePrecision())
      comp->setReturnInfo((returnRegister->isSinglePrecision())
                              ? TR_FloatXMMReturn
                              : TR_DoubleXMMReturn);
    else
      comp->setReturnInfo((returnRegister->isSinglePrecision())
...
TR::Register *OMR::X86::TreeEvaluator::fpBinaryArithmeticEvaluator(
    TR::Node *node, bool isFloat, TR::CodeGenerator *cg) {
  if (cg->useSSEForSinglePrecision() && cg->useSSEForDoublePrecision()) {
    return TR::TreeEvaluator::FloatingPointAndVectorBinaryArithmeticEvaluator(
        node, cg);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::fpUnaryMaskEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  auto result = child->getReferenceCount() == 1
                    ? value
                    : cg->allocateRegister(value->getKind());
  ...
  if (value->getKind() != TR_FPR)  // Legacy supported for X87, to be deleted
  {
    ...
    if (result != value) {
      generateFPST0STiRegRegInstruction(
          value->isSinglePrecision() ? FLDRegReg : DLDRegReg, node, result,
          value, cg);
    }
    ...
    generateRegMemInstruction(
        opcode, node, result,
        generateX86MemoryReference(cg->findOrCreate16ByteConstant(node, mask),
                                   cg),
        cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::fpSqrtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  ...
  if (value->getKind() != TR_FPR)  // Legacy supported for X87, to be deleted
  {
    ...
    if (value->isSinglePrecision()) {
      ...
      generateRegMemInstruction(MOVSSRegMem, node, tmp,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
      generateRegRegInstruction(SQRTSSRegReg, node, tmp, tmp, cg);
      generateMemRegInstruction(MOVSSMemReg, node,
                                generateX86MemoryReference(*tempMR, 0, cg), tmp,
                                cg);
      generateFPRegMemInstruction(FLDRegMem, node, result,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
    } else {
      auto tempMR = cg->machine()->getDummyLocalMR(TR::Double);
      generateFPMemRegInstruction(DSTPMemReg, node, tempMR, value, cg);
      generateRegMemInstruction(MOVSDRegMem, node, tmp,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
      generateRegRegInstruction(SQRTSDRegReg, node, tmp, tmp, cg);
      generateMemRegInstruction(MOVSDMemReg, node,
                                generateX86MemoryReference(*tempMR, 0, cg), tmp,
                                cg);
      generateFPRegMemInstruction(DLDRegMem, node, result,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
    }
    ...
    generateRegRegInstruction(
        value->isSinglePrecision() ? SQRTSSRegReg : SQRTSDRegReg, node, result,
        value, cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::dsqrtEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::faddEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register *OMR::X86::TreeEvaluator::daddEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register *OMR::X86::TreeEvaluator::fsubEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register *OMR::X86::TreeEvaluator::dsubEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register *OMR::X86::TreeEvaluator::fmulEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register *OMR::X86::TreeEvaluator::dmulEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register *OMR::X86::TreeEvaluator::fdivEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register *OMR::X86::TreeEvaluator::ddivEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register *OMR::X86::TreeEvaluator::fpRemEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  bool nodeIsDouble = node->getDataType() == TR::Double;
  TR::Register *targetRegister;
  TR::Compilation *comp = cg->comp();
  const TR::X86LinkageProperties &linkageProperties =
      cg->getLinkage(comp->getJittedMethodSymbol()->getLinkageConvention())
          ->getProperties();

  if (cg->useSSEForDoublePrecision())  // Note: both float and double helpers
                                       // use SSE2
  {
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      TR::SymbolReference *helperSymRef =
          cg->symRefTab()->findOrCreateRuntimeHelper(
              nodeIsDouble ? TR_AMD64doubleRemainder : TR_AMD64floatRemainder,
              false, false, false);
      targetRegister = TR::TreeEvaluator::performHelperCall(
          node, helperSymRef, nodeIsDouble ? TR::dcall : TR::fcall, false, cg);
    } else {
      TR::SymbolReference *helperSymRef =
          cg->symRefTab()->findOrCreateRuntimeHelper(
              nodeIsDouble ? TR_IA32doubleRemainderSSE
                           : TR_IA32floatRemainderSSE,
              false, false, false);
      targetRegister = TR::TreeEvaluator::performHelperCall(
          node, helperSymRef, nodeIsDouble ? TR::dcall : TR::fcall, false, cg);
    }
    ...
    targetRegister =
        TR::TreeEvaluator::commonFPRemEvaluator(node, cg, nodeIsDouble);
  }
...
TR::Register *OMR::X86::TreeEvaluator::commonFPRemEvaluator(
    TR::Node *node, TR::CodeGenerator *cg, bool isDouble) {
  TR::Node *divisor = node->getSecondChild();
  TR::Node *dividend = node->getFirstChild();
  TR::Compilation *comp = cg->comp();
  ...
  generateFPRemainderRegRegInstruction(FPREMRegReg, node, dividendReg,
                                       divisorReg, accReg, deps, cg);
  cg->stopUsingRegister(accReg);

  node->setRegister(dividendReg);
  cg->decReferenceCount(dividend);

  if (divisorReg && divisorReg->getKind() == TR_X87 &&
      divisor->getReferenceCount() == 1)
    generateFPSTiST0RegRegInstruction(FSTRegReg, node, divisorReg, divisorReg,
                                      cg);
...
TR::Register *OMR::X86::TreeEvaluator::i2fEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    if (cg->useSSEForSinglePrecision()) {
      ...
      generateFPRegMemInstruction(FILDRegMem, node, target,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
      target->setMayNeedPrecisionAdjustment();
      target->setNeedsPrecisionAdjustment();
...
TR::Register *OMR::X86::TreeEvaluator::i2dEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *target;
  TR::MemoryReference *tempMR;
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    if (cg->useSSEForDoublePrecision()) {
      ...
      tempMR = generateX86MemoryReference(cg->allocateLocalTemp(), cg);
      generateMemRegInstruction(S4MemReg, node, tempMR, intReg, cg);
      generateFPRegMemInstruction(DILDRegMem, node, target,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
    }
    cg->decReferenceCount(child);
...
//
TR::Register *OMR::X86::TreeEvaluator::fpConvertToInt(
    TR::Node *node, TR::SymbolReference *helperSymRef, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "AMD64 has enableSSE set, so it doesn't use this logic");
  ...
  if (!optimizeF2IWithSSE && !optimizeD2IWithSSE2) {
    ...
    fpcw = comp->getJittedMethodSymbol()->usesSinglePrecisionMode()
               ? SINGLE_PRECISION_ROUND_TO_ZERO
    ...
    generateMemInstruction(LDCWMem, node,
                           generateX86MemoryReference(
                               cg->findOrCreate2ByteConstant(node, fpcw), cg),
                           cg);
    ...
    loadInstr = generateRegMemInstruction(
        L4RegMem, node, resultReg, generateX86MemoryReference(*tempMR, 0, cg),
        cg);
    ...
    if (optimizeF2IWithSSE) {
      if (child->getReferenceCount() == 1 && child->getRegister() == 0 &&
        ...
        resultReg = cg->allocateRegister();
        loadInstr = generateRegMemInstruction(
            CVTTSS2SIReg4Mem, node, resultReg,
            generateX86MemoryReference(*tempMR, 0, cg), cg);
        tempMR->decNodeReferenceCounts(cg);
      } else {
      ...
      if (child->getReferenceCount() == 1 && child->getRegister() == 0 &&
        ...
        resultReg = cg->allocateRegister();
        loadInstr = generateRegMemInstruction(
            CVTTSD2SIReg4Mem, node, resultReg,
            generateX86MemoryReference(*tempMR, 0, cg), cg);
        tempMR->decNodeReferenceCounts(cg);
      } else {
  ...
  //
  cg->addSnippet(new (cg->trHeapMemory()) TR::X86FPConvertToIntSnippet(
      reStartLabel, snippetLabel, helperSymRef, loadInstr, cg));
  ...
  // We want the floating point register to be live through the snippet, so if
  // it is not referenced again we must pop it off the stack here.
  //
...
TR::Register *OMR::X86::TreeEvaluator::fpConvertToLong(
  ...
  if (child->getOpCode().isDouble() && cg->useSSEForDoublePrecision()) {
    ...
    TR::LabelSymbol *reStartLabel =
        TR::LabelSymbol::create(cg->trHeapMemory(), cg);  // exit routine label
    TR::LabelSymbol *CallLabel = TR::LabelSymbol::create(
        cg->trHeapMemory(),
        cg);  // label where long (64-bit) conversion will start
    ...
    // Attempt to convert a double in an XMM register to an integer using
    // CVTTSD2SI. If the conversion succeeds, put the integer in lowReg and
    // sign-extend it to highReg. If the conversion fails (the double is too
    // large), call the helper.
    ...
    TR::SymbolReference *d2l = comp->getSymRefTab()->findOrCreateRuntimeHelper(
        TR_IA32double2LongSSE, false, false, false);
    ...
    TR_OutlinedInstructions *outlinedHelperCall =
        new (cg->trHeapMemory()) TR_OutlinedInstructions(
            node, TR::lcall, targetRegister, CallLabel, reStartLabel, cg);
    ...
    TR::X86FPST0STiRegRegInstruction
        *clobInstruction;  // loads a clobberable copy of the float/double
    TR::X86RegMemInstruction
        *loadHighInstr;  // loads the high dword of the converted long
    TR::X86RegMemInstruction
        *loadLowInstr;  // loads the low dword of the converted long

    TR::Register *tempFPR1 = child->getOpCode().isFloat()
                                 ? cg->allocateSinglePrecisionRegister(TR_X87)
                                 : cg->allocateRegister(TR_X87);

    // WARNING:
    //
    // The following instruction is dissected in the snippet to determine the
    // original double register. If this instruction changes, or if you add any
    // instructions that manipulate the FP stack between here and the call to
    // the snippet, you may need to change the snippet also.
    //
    clobInstruction = generateFPST0STiRegRegInstruction(
        FLDRegReg, node, tempFPR1, doubleReg, cg);

    // For slow conversion only, change the rounding mode on the FPU via its
    ...
    TR::MemoryReference *convertedLongMR =
    ...
    // WARNING:
    //
    // The following load instructions are dissected in the snippet to determine
    // the target registers. If they or their format is changed, you may need to
    // change the snippet also.
    //
    loadHighInstr = generateRegMemInstruction(
        L4RegMem, node, highReg,
        generateX86MemoryReference(*convertedLongMR, 4, cg), cg);

    loadLowInstr = generateRegMemInstruction(
        L4RegMem, node, lowReg,
        generateX86MemoryReference(*convertedLongMR, 0, cg), cg);

    // Jump to the snippet if the converted value is an indefinite integer;
    // otherwise continue.
    //
    ...
    cg->addSnippet(new (cg->trHeapMemory()) TR::X86FPConvertToLongSnippet(
        reStartLabel, snippetLabel, helperSymRef, clobInstruction,
        loadHighInstr, loadLowInstr, cg));
    ...
    //
    if ((cg->decReferenceCount(child) == 0) ||
        (child->getRegister()->getKind() == TR_FPR)) {
      generateFPSTiST0RegRegInstruction(FSTRegReg, node, doubleReg, doubleReg,
                                        cg);
    }
...
TR::Register *OMR::X86::TreeEvaluator::f2iEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  if (cg->useSSEForSinglePrecision()) {
    bool doubleSource;
    bool longTarget;
    ...
    TR_ASSERT(TR::Compiler->target.is64Bit() || !longTarget,
              "Incorrect opcode value in f2iEvaluator");
    ...
    if (sourceRegister->getKind() == TR_X87 &&
      ...
      generateRegMemInstruction(CVTTSS2SIReg4Mem, node, targetRegister,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
      generateRegRegInstruction(cvttOpCode, node, targetRegister,
                                sourceRegister, cg);
    }
    ...
    if (longTarget) {
      TR_ASSERT(TR::Compiler->target.is64Bit(),
                "We should only get here on AMD64");
...

    {
      ...
      // at this point, target is set to -INF and there can only be THREE
      // possible results: -INF, +INF, NaN compare source with ZERO
      generateRegMemInstruction(
          doubleSource ? UCOMISDRegMem : UCOMISSRegMem, node, sourceRegister,
          generateX86MemoryReference(
              doubleSource ? cg->findOrCreate8ByteConstant(node, 0)
                           : cg->findOrCreate4ByteConstant(node, 0),
              cg),
          cg);
      // load max int if source is positive, note that for long case, LLONG_MAX
      // << 1 is loaded as it will be shifted right
      generateRegMemInstruction(
          CMOVARegMem(longTarget), node, targetRegister,
          generateX86MemoryReference(
              longTarget ? cg->findOrCreate8ByteConstant(node, LLONG_MAX << 1)
                         : cg->findOrCreate4ByteConstant(node, INT_MAX),
              cg),
          cg);
      // load zero if source is NaN
      generateRegMemInstruction(
          CMOVPRegMem(longTarget), node, targetRegister,
          generateX86MemoryReference(
              longTarget ? cg->findOrCreate8ByteConstant(node, 0)
                         : cg->findOrCreate4ByteConstant(node, 0),
              cg),
          cg);
    ...
    if (sourceRegister && sourceRegister->getKind() == TR_X87 &&
        child->getReferenceCount() == 1) {
      generateFPSTiST0RegRegInstruction(FSTRegReg, node, sourceRegister,
                                        sourceRegister, cg);
    }
    ...
    TR_ASSERT(TR::Compiler->target.is32Bit(), "assertion failure");
    return TR::TreeEvaluator::fpConvertToInt(
        node,
        cg->symRefTab()->findOrCreateRuntimeHelper(
            node->getOpCodeValue() == TR::f2i ? TR_IA32floatToInt
                                              : TR_IA32doubleToInt,
            false, false, false),
        cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::f2lEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit(), "AMD64 uses f2iEvaluator for this");
  return TR::TreeEvaluator::fpConvertToLong(
      node,
      cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32floatToLong, false,
                                                 false, false),
      cg);
}

TR::Register *OMR::X86::TreeEvaluator::f2dEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *targetRegister;
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    generateRegRegInstruction(CVTSS2SDRegReg, node, targetRegister,
                              targetRegister, cg);
  } else {
    ...
    if (cg->useSSEForSinglePrecision() && sourceRegister->getKind() == TR_FPR) {
      ...
      generateFPRegMemInstruction(FLDRegMem, node, targetRegister,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
    } else {
...
TR::Register *OMR::X86::TreeEvaluator::f2bEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  diagnostic("f2b not expected!");
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::f2sEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  diagnostic("f2s not expected!");
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::f2cEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  diagnostic("f2c not expected!");
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::d2lEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR_ASSERT(TR::Compiler->target.is32Bit(), "AMD64 uses f2iEvaluator for this");

  return TR::TreeEvaluator::fpConvertToLong(
      node,
      cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32doubleToLong, false,
                                                 false, false),
      cg);
}

TR::Register *OMR::X86::TreeEvaluator::d2fEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *targetRegister;
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    generateRegRegInstruction(CVTSD2SSRegReg, node, targetRegister,
                              targetRegister, cg);
  } else {
    TR::Register *sourceRegister = cg->evaluate(child);
    if (cg->useSSEForSinglePrecision()) {
      TR::MemoryReference *tempMR =
          generateX86MemoryReference(cg->allocateLocalTemp(TR::Float), cg);
      targetRegister = cg->allocateSinglePrecisionRegister(TR_FPR);
      generateFPMemRegInstruction(FSTMemReg, node, tempMR, sourceRegister, cg);
      generateRegMemInstruction(MOVSSRegMem, node, targetRegister,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
      targetRegister = cg->doubleClobberEvaluate(child);
      targetRegister->setMayNeedPrecisionAdjustment();
...
TR::Register *OMR::X86::TreeEvaluator::d2bEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  diagnostic("d2b not expected!");
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::d2sEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  diagnostic("d2s not expected!");
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::d2cEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  diagnostic("d2s not expected!");
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::ibits2fEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *target;
  TR::MemoryReference *tempMR;
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadVar()) {
    ...
    //
    // TODO: check the capabilities of the processor rather than whether SSE is
    // enabled.
    //
    tempMR = generateX86MemoryReference(child, cg);
    if (cg->useSSEForDoublePrecision()) {
      ...
      if (child->getReferenceCount() > 1)
        TR::TreeEvaluator::performIload(
            child, generateX86MemoryReference(*tempMR, 0, cg), cg);
    }
    tempMR->decNodeReferenceCounts(cg);
  } else {
    TR::Register *intReg = cg->evaluate(child);
    if (cg->useSSEForDoublePrecision()) {
      ...
      tempMR = cg->machine()->getDummyLocalMR(TR::Int32);
      generateMemRegInstruction(S4MemReg, node, tempMR, intReg, cg);
      generateRegMemInstruction(MOVSSRegMem, node, target,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
      target = cg->allocateSinglePrecisionRegister(TR_X87);
      tempMR = cg->machine()->getDummyLocalMR(TR::Int32);
      generateMemRegInstruction(S4MemReg, node, tempMR, intReg, cg);
      generateFPRegMemInstruction(FLDRegMem, node, target,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
    }
  }
...
TR::Register *OMR::X86::TreeEvaluator::fbits2iEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadVar() &&
    ...
    if (floatReg->getKind() == TR_FPR) {
      ...
      generateRegMemInstruction(L4RegMem, node, target,
                                generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
      tempMR = cg->machine()->getDummyLocalMR(TR::Int32);
      generateFPMemRegInstruction(FSTMemReg, node, tempMR, floatReg, cg);
  ...
  if (node->normalizeNanValues()) {
    static char *disableFastNormalizeNaNs =
        feGetEnv("TR_disableFastNormalizeNaNs");
    ...
    if (disableFastNormalizeNaNs) {
      ...
      TR::RegisterDependencyConditions *internalControlFlowDeps =
          generateRegisterDependencyConditions((uint8_t)0, (uint8_t)1, cg);
      internalControlFlowDeps->addPostCondition(treg, TR::RealRegister::NoReg,
                                                cg);
      ...
      TR::LabelSymbol *startLabel =
          TR::LabelSymbol::create(cg->trHeapMemory(), cg);
      TR::LabelSymbol *slowPathLabel =
          TR::LabelSymbol::create(cg->trHeapMemory(), cg);
      TR::LabelSymbol *normalizeLabel =
          TR::LabelSymbol::create(cg->trHeapMemory(), cg);
      ...
      // Merge point
      //
      generateLabelInstruction(LABEL, node, endLabel, internalControlFlowDeps,
                               cg);
    }
  }
...
TR::Register *OMR::X86::TreeEvaluator::fRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::X86::TreeEvaluator::dRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Register *globalReg = node->getRegister();
  if (globalReg == NULL) {
...
TR::Register *OMR::X86::TreeEvaluator::fRegStoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (cg->useSSEForSinglePrecision()) {
    ...
    machine->setXMMGlobalRegister(globalRegNum - machine->getNumGlobalGPRs(),
                                  globalReg);
    cg->decReferenceCount(child);
    return globalReg;
  } else {
    machine->setFPStackRegister(globalRegNum - machine->getNumGlobalGPRs(),
                                toX86FPStackRegister(globalReg));

    if (cg->decReferenceCount(child) == 0) {
      generateFPSTiST0RegRegInstruction(FSTRegReg, node, globalReg, globalReg,
                                        cg);
      cg->stopUsingRegister(globalReg);
    } else if (childGlobalReg && childGlobalReg != globalReg) {
      {
        int32_t i;
        for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
          if (i != fpStackSlot) {
            ...
            // if (!reg)
            //   dumpOptDetails("Global reg %d Stack slot %d iglobalreg %d reg
            //   address %p childreg %s\n", globalRegNum, fpStackSlot,
            //   i+machine->getNumGlobalGPRs(), reg,
            //   getDebug()->getName(childGlobalReg));
            // else
            //   dumpOptDetails("Global reg %d Stack slot %d iglobalreg %d reg
            //   %s childreg %s\n", globalRegNum, fpStackSlot,
            //   i+machine->getNumGlobalGPRs(), getDebug()->getName(reg),
            //   getDebug()->getName(childGlobalReg));

            if (reg == childGlobalReg) {
              TR::Register *popReg = cg->machine()->getFPStackRegister(i);
              generateFPSTiST0RegRegInstruction(FSTRegReg, node, childGlobalReg,
                                                childGlobalReg, cg, true);
              cg->stopUsingRegister(childGlobalReg);
              cg->machine()
                  ->getFPStackRegisterNode(fpStackSlot)
                  ->setRegister(popReg);
              break;
            }
          }
...
TR::Register *OMR::X86::TreeEvaluator::dRegStoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  int32_t globalRegNum = node->getGlobalRegisterNumber();
  TR::Machine *machine = cg->machine();
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    if (cg->decReferenceCount(child) == 0) {
      generateFPSTiST0RegRegInstruction(DSTRegReg, node, globalReg, globalReg,
                                        cg);
      cg->stopUsingRegister(globalReg);
    } else if (childGlobalReg && childGlobalReg != globalReg) {
      // int32_t fpStackSlot = globalRegNum - machine->getNumGlobalGPRs();
      // TR::Register *fpGlobalReg =
      // cg->machine()->getFPStackRegister(fpStackSlot); if (fpGlobalReg)
      {
        int32_t i;
        for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
          if (i != fpStackSlot) {
            ...
            if (reg == childGlobalReg) {
              TR::Register *popReg = cg->machine()->getFPStackRegister(i);
              generateFPSTiST0RegRegInstruction(DSTRegReg, node, childGlobalReg,
                                                childGlobalReg, cg, true);
              cg->stopUsingRegister(childGlobalReg);
              cg->machine()
                  ->getFPStackRegisterNode(fpStackSlot)
...
TR::Register *OMR::X86::TreeEvaluator::generateBranchOrSetOnFPCompare(
    TR::Node *node, TR::Register *accRegister, bool generateBranch,
    TR::CodeGenerator *cg) {
  ...
  // If not using FCOMI/UCOMISS/UCOMISD, then we must be interpreting the FPSW
  // in AH; generate an FCMPEVAL pseudo-instruction, which will emit the
  // interpretation code at register assignment time (when we have finalized the
  // sense of the comparison).
  //
  if (accRegister != NULL) {
    TR::RegisterDependencyConditions *accRegDep =
    ...
    generateFPCompareEvalInstruction(FCMPEVAL, node, accRegister, accRegDep,
                                     cg);
  ...
  if (accRegister == NULL && (cmpOp == TR::iffcmpneu || cmpOp == TR::fcmpneu ||
    ...
    if (generateBranch) {
      ...
      if (deps && deps->getPreConditions() &&
          deps->getPreConditions()->getMayNeedToPopFPRegisters()) {
        deps1 = deps->clone(cg);
        deps1->setNumPostConditions(0, cg->trMemory());
        deps->setNumPreConditions(0, cg->trMemory());
      }
      generateLabelInstruction(
          JPE4, node, node->getBranchDestination()->getNode()->getLabel(),
          deps1, cg);
      generateLabelInstruction(
          JNE4, node, node->getBranchDestination()->getNode()->getLabel(), deps,
          cg);
    } else {
      ...
      generateRegRegInstruction(OR1RegReg, node, targetRegister, tempRegister,
                                cg);
      generateRegRegInstruction(MOVZXReg4Reg1, node, targetRegister,
                                targetRegister, cg);
      cg->stopUsingRegister(tempRegister);
    }
    ...
    if (generateBranch) {
      ...
      generateLabelInstruction(
          JE4, node, node->getBranchDestination()->getNode()->getLabel(), cg);
      ...
      generateRegRegInstruction(AND1RegReg, node, targetRegister, tempRegister,
                                cg);
      generateRegRegInstruction(MOVZXReg4Reg1, node, targetRegister,
                                targetRegister, cg);
      cg->stopUsingRegister(tempRegister);
    ...
    TR_X86OpCodes op = getBranchOrSetOpCodeForFPComparison(
        node->getOpCodeValue(), (accRegister == NULL));
    if (generateBranch) {
      generateLabelInstruction(
          op, node, node->getBranchDestination()->getNode()->getLabel(), deps,
          cg);
    } else {
      targetRegister = cg->allocateRegister();
      cg->getLiveRegisters(TR_GPR)->setByteRegisterAssociation(targetRegister);
...
//
TR::Register *OMR::X86::TreeEvaluator::compareFloatOrDoubleForOrder(
    TR::Node *node, TR_X86OpCodes fpCmpRegRegOpCode,
    TR_X86OpCodes fpCmpRegMemOpCode, TR_X86OpCodes fpCmpiRegRegOpCode,
    TR_X86OpCodes xmmCmpRegRegOpCode, TR_X86OpCodes xmmCmpRegMemOpCode,
    bool useFCOMIInstructions, TR::CodeGenerator *cg) {
  if (((TR_X86OpCode::singleFPOp(fpCmpRegRegOpCode) &&
        cg->useSSEForSinglePrecision()) ||
       (TR_X86OpCode::doubleFPOp(fpCmpRegRegOpCode) &&
        cg->useSSEForDoublePrecision()))) {
    TR_IA32XMMCompareAnalyser temp(cg);
    return temp.xmmCompareAnalyser(node, xmmCmpRegRegOpCode,
                                   xmmCmpRegMemOpCode);
  } else {
    TR_X86FPCompareAnalyser temp(cg);
    return temp.fpCompareAnalyser(node, fpCmpRegRegOpCode, fpCmpRegMemOpCode,
                                  fpCmpiRegRegOpCode, useFCOMIInstructions);
  }
}

TR::Register *OMR::X86::TreeEvaluator::generateFPCompareResult(
    TR::Node *node, TR::Register *accRegister, TR::CodeGenerator *cg) {
  if (accRegister != NULL) {
    TR::RegisterDependencyConditions *accRegDep =
        generateRegisterDependencyConditions((uint8_t)1, 1, cg);
    accRegDep->addPreCondition(accRegister, TR::RealRegister::eax, cg);
...
bool OMR::X86::TreeEvaluator::canUseFCOMIInstructions(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::ILOpCodes cmpOp = node->getOpCodeValue();

  return (!cg->getX86ProcessorInfo().supportsFCOMIInstructions() ||
          cmpOp == TR::iffcmpneu || cmpOp == TR::iffcmpeq ||
          cmpOp == TR::ifdcmpneu || cmpOp == TR::ifdcmpeq ||
          cmpOp == TR::fcmpneu || cmpOp == TR::dcmpneu || cmpOp == TR::fcmpeq ||
          cmpOp == TR::dcmpeq)
             ? false
             : true;
}

TR::Register *OMR::X86::TreeEvaluator::compareFloatAndBranchEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool useFCOMIInstructions =
      TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
  TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
      node, FCOMRegReg, FCOMRegMem, FCOMIRegReg, UCOMISSRegReg, UCOMISSRegMem,
      useFCOMIInstructions, cg);
  return TR::TreeEvaluator::generateBranchOrSetOnFPCompare(node, accRegister,
                                                           true, cg);
}

TR::Register *OMR::X86::TreeEvaluator::compareDoubleAndBranchEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool useFCOMIInstructions =
      TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
  TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
      node, DCOMRegReg, DCOMRegMem, DCOMIRegReg, UCOMISDRegReg, UCOMISDRegMem,
      useFCOMIInstructions, cg);
  return TR::TreeEvaluator::generateBranchOrSetOnFPCompare(node, accRegister,
                                                           true, cg);
}

TR::Register *OMR::X86::TreeEvaluator::compareFloatAndSetEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool useFCOMIInstructions =
      TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
  TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
...
TR::Register *OMR::X86::TreeEvaluator::compareDoubleAndSetEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool useFCOMIInstructions =
      TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
  TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
...
TR::Register *OMR::X86::TreeEvaluator::compareFloatEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool useFCOMIInstructions =
      TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
  TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
...
TR::Register *OMR::X86::TreeEvaluator::compareDoubleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool useFCOMIInstructions =
      TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
  TR::Register *accRegister = TR::TreeEvaluator::compareFloatOrDoubleForOrder(
