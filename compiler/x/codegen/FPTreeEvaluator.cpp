...
TR::Register*
OMR::X86::TreeEvaluator::coerceFPRToXMMR(TR::Node* node,
                                         ...
                                         TR::CodeGenerator* cg)
{
  ...
  if (fpRegister->isSinglePrecision()) {
    ...
    generateRegMemInstruction(cg->getXMMDoubleLoadOpCode(),
                              node,
                              xmmRegister,
                              generateX86MemoryReference(*tempMR, 0, cg),
                              cg);
...
void
OMR::X86::TreeEvaluator::coerceFPOperandsToXMMRs(TR::Node* node,
                                                 TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::coerceXMMRToFPR(TR::Node* node,
                                         ...
                                         TR::CodeGenerator* cg)
{
  ...
  if (xmmRegister->isSinglePrecision()) {
    ...
    generateFPRegMemInstruction(FLDRegMem,
                                node,
                                fpRegister,
                                generateX86MemoryReference(*tempMR, 0, cg),
                                cg);
    ...
    generateFPRegMemInstruction(DLDRegMem,
                                node,
                                fpRegister,
                                generateX86MemoryReference(*tempMR, 0, cg),
                                cg);
...
void
OMR::X86::TreeEvaluator::insertPrecisionAdjustment(TR::Register* reg,
                                                   TR::Node* root,
                                                   TR::CodeGenerator* cg)
{
  TR::DataType dt;
...
TR::Register*
OMR::X86::TreeEvaluator::fconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (cg->useSSEForSinglePrecision()) {
    ...
    if (node->getFloatBits() == 0) {
      generateRegRegInstruction(
        XORPSRegReg, node, targetRegister, targetRegister, cg);
    } else {
      ...
      setDiscardableIfPossible(TR_RematerializableFloat,
                               targetRegister,
                               node,
                               instr,
                               (intptrj_t)node->getFloatBits(),
                               cg);
    }
  } else {
...
TR::Register*
OMR::X86::TreeEvaluator::dconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister;
  if (cg->useSSEForDoublePrecision()) {
    ...
    if (node->getLongInt() == 0) {
      generateRegRegInstruction(
        XORPDRegReg, node, targetRegister, targetRegister, cg);
    } else {
      TR::MemoryReference* tempMR = generateX86MemoryReference(
        cg->findOrCreate8ByteConstant(node, node->getLongInt()), cg);
      generateRegMemInstruction(
        cg->getXMMDoubleLoadOpCode(), node, targetRegister, tempMR, cg);
    }
  } else {
...
TR::Register*
OMR::X86::TreeEvaluator::performFload(TR::Node* node,
  ...
  if (cg->useSSEForSinglePrecision()) {
    if (TR::Compiler->target.is64Bit() &&
      ...
      instr = generateRegMemInstruction(
        MOVSSRegMem, node, targetRegister, sourceMR, cg);
    } else {
      targetRegister = cg->allocateSinglePrecisionRegister(TR_FPR);
      instr = generateRegMemInstruction(
        MOVSSRegMem, node, targetRegister, sourceMR, cg);
      setDiscardableIfPossible(
        TR_RematerializableFloat, targetRegister, node, instr, sourceMR, cg);
    }
  } else {
    targetRegister = cg->allocateSinglePrecisionRegister(TR_X87);
    instr = generateFPRegMemInstruction(
      FLDRegMem, node, targetRegister, sourceMR, cg);
  }
...
TR::Register*
OMR::X86::TreeEvaluator::floadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::performDload(TR::Node* node,
                                      TR::MemoryReference* sourceMR,
                                      TR::CodeGenerator* cg)
{
  TR::Register* targetRegister;
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    targetRegister = cg->allocateRegister(TR_FPR);
    instr = generateRegMemInstruction(
      cg->getXMMDoubleLoadOpCode(), node, targetRegister, sourceMR, cg);
  } else {
    targetRegister = cg->allocateRegister(TR_X87);
    instr = generateFPRegMemInstruction(
      DLDRegMem, node, targetRegister, sourceMR, cg);
  }
  if (node->getOpCode().isIndirect())
    cg->setImplicitExceptionPoint(instr);
  node->setRegister(targetRegister);
...
TR::Register*
OMR::X86::TreeEvaluator::dloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::MemoryReference* tempMR = generateX86MemoryReference(node, cg);
  TR::Register* targetRegister =
                                                     ...
                                                     TR::CodeGenerator* cg)
{
  ...
  if ((valueChild->getOpCodeValue() == TR::ibits2f ||
    ...
    static TR::ILOpCodes integerOpCodes[2][2] = {
      { TR::istore, TR::lstore }, { TR::istorei, TR::lstorei }
    };
    TR::Node::recreate(node,
                       integerOpCodes[nodeIsIndirect][(
  ...
  if (valueChild->getOpCode().isLoadConst()) {
    if (nodeIs64Bit) {
      if (TR::Compiler->target.is64Bit()) {
        ...
        if (valueChild->getLongInt() == 0) {
          generateRegRegInstruction(
            XOR8RegReg, node, floatConstReg, floatConstReg, cg);
        } else {
          generateRegImm64Instruction(
            MOV8RegImm64, node, floatConstReg, valueChild->getLongInt(), cg);
        }
        exceptionPoint =
          generateMemRegInstruction(S8MemReg, node, tempMR, floatConstReg, cg);
    ...
    if (firstChildReg && firstChildReg->getKind() == TR_X87 &&
      ...
      generateFPSTiST0RegRegInstruction(
        FSTRegReg, valueChild, firstChildReg, firstChildReg, cg);
    ...
    generateRegMemInstruction(
      LRegMem(nodeIs64Bit), node, tempRegister, loadMR, cg);
    exceptionPoint = generateMemRegInstruction(
      SMemReg(nodeIs64Bit), node, tempMR, tempRegister, cg);
    ...
    if (sourceRegister->getKind() == TR_FPR) {
      ...
      exceptionPoint = generateFPMemRegInstruction(
        FSTMemReg, node, tempMR, sourceRegister, cg);
    }
  }
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is32Bit() && !cg->useSSEForDoublePrecision() &&
    ...
    returnRegister = TR::TreeEvaluator::coerceXMMRToFPR(
      node->getFirstChild(), returnRegister, cg);
...
TR::Register*
OMR::X86::TreeEvaluator::fpBinaryArithmeticEvaluator(TR::Node* node,
                                                     bool isFloat,
                                                     TR::CodeGenerator* cg)
{
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (value->getKind() != TR_FPR) // Legacy supported for X87, to be deleted
  {
    ...
    generateRegMemInstruction(opcode,
                              node,
                              result,
                              generateX86MemoryReference(
                                cg->findOrCreate16ByteConstant(node, mask), cg),
                              cg);
  }
...
TR::Register*
OMR::X86::TreeEvaluator::fpSqrtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (value->getKind() != TR_FPR) // Legacy supported for X87, to be deleted
  {
    ...
    if (value->isSinglePrecision()) {
      ...
      generateRegMemInstruction(
        MOVSSRegMem, node, tmp, generateX86MemoryReference(*tempMR, 0, cg), cg);
      generateRegRegInstruction(SQRTSSRegReg, node, tmp, tmp, cg);
      generateMemRegInstruction(
        MOVSSMemReg, node, generateX86MemoryReference(*tempMR, 0, cg), tmp, cg);
      generateFPRegMemInstruction(FLDRegMem,
                                  node,
                                  result,
                                  generateX86MemoryReference(*tempMR, 0, cg),
      ...
      generateRegMemInstruction(
        MOVSDRegMem, node, tmp, generateX86MemoryReference(*tempMR, 0, cg), cg);
      generateRegRegInstruction(SQRTSDRegReg, node, tmp, tmp, cg);
      generateMemRegInstruction(
        MOVSDMemReg, node, generateX86MemoryReference(*tempMR, 0, cg), tmp, cg);
      generateFPRegMemInstruction(DLDRegMem,
                                  node,
                                  result,
                                  generateX86MemoryReference(*tempMR, 0, cg),
...
TR::Register*
OMR::X86::TreeEvaluator::dsqrtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::faddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::daddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::fsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::dsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::fmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::dmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::fdivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, true, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::ddivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::fpBinaryArithmeticEvaluator(node, false, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::fpRemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
                                      ...
                                      // SSE2
  {
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      TR::SymbolReference* helperSymRef =
        cg->symRefTab()->findOrCreateRuntimeHelper(
          nodeIsDouble ? TR_IA32doubleRemainderSSE : TR_IA32floatRemainderSSE,
          false,
          false,
          false);
      targetRegister = TR::TreeEvaluator::performHelperCall(
...
TR::Register*
OMR::X86::TreeEvaluator::commonFPRemEvaluator(TR::Node* node,
                                              TR::CodeGenerator* cg,
                                              bool isDouble)
{
  TR::Node* divisor = node->getSecondChild();
  ...
  generateFPRemainderRegRegInstruction(
    FPREMRegReg, node, dividendReg, divisorReg, accReg, deps, cg);
  ...
  if (divisorReg && divisorReg->getKind() == TR_X87 &&
    ...
    generateFPSTiST0RegRegInstruction(
      FSTRegReg, node, divisorReg, divisorReg, cg);
...
TR::Register*
OMR::X86::TreeEvaluator::i2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    if (cg->useSSEForSinglePrecision()) {
      ...
      generateFPRegMemInstruction(FILDRegMem,
                                  node,
                                  target,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
...
TR::Register*
OMR::X86::TreeEvaluator::i2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* target;
  TR::MemoryReference* tempMR;
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    if (cg->useSSEForDoublePrecision()) {
      ...
      tempMR = generateX86MemoryReference(cg->allocateLocalTemp(), cg);
      generateMemRegInstruction(S4MemReg, node, tempMR, intReg, cg);
      generateFPRegMemInstruction(DILDRegMem,
                                  node,
                                  target,
                                  generateX86MemoryReference(*tempMR, 0, cg),
                                  cg);
...
//
TR::Register*
OMR::X86::TreeEvaluator::fpConvertToInt(TR::Node* node,
                                        TR::SymbolReference* helperSymRef,
                                        TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (!optimizeF2IWithSSE && !optimizeD2IWithSSE2) {
    ...
    generateMemInstruction(
      LDCWMem,
      node,
      generateX86MemoryReference(cg->findOrCreate2ByteConstant(node, fpcw), cg),
      cg);
    ...
    if (optimizeF2IWithSSE) {
      ...
      if (child->getReferenceCount() == 1 && child->getRegister() == 0 &&
        ...
        resultReg = cg->allocateRegister();
        loadInstr =
          generateRegMemInstruction(CVTTSD2SIReg4Mem,
                                    node,
                                    resultReg,
                                    generateX86MemoryReference(*tempMR, 0, cg),
                                    cg);
...
//
TR::Register*
OMR::X86::TreeEvaluator::fpConvertToLong(TR::Node* node,
                                         TR::SymbolReference* helperSymRef,
                                         TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (child->getOpCode().isDouble() && cg->useSSEForDoublePrecision()) {
    ...
    loadHighInstr = generateRegMemInstruction(
      ...
      node,
      highReg,
      generateX86MemoryReference(*convertedLongMR, 4, cg),
      cg);
    ...
    loadLowInstr = generateRegMemInstruction(
      ...
      node,
      lowReg,
      generateX86MemoryReference(*convertedLongMR, 0, cg),
      cg);
    ...
    cg->addSnippet(new (cg->trHeapMemory())
                     TR::X86FPConvertToLongSnippet(reStartLabel,
                                                   snippetLabel,
                                                   helperSymRef,
                                                   clobInstruction,
                                                   loadHighInstr,
    ...
    if ((cg->decReferenceCount(child) == 0) ||
      ...
      generateFPSTiST0RegRegInstruction(
        FSTRegReg, node, doubleReg, doubleReg, cg);
    }
...
TR::Register*
OMR::X86::TreeEvaluator::f2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (cg->useSSEForSinglePrecision()) {
    bool doubleSource;
    bool longTarget;
    ...
    if (sourceRegister->getKind() == TR_X87 &&
      ...
      generateRegRegInstruction(
        cvttOpCode, node, targetRegister, sourceRegister, cg);
    }
...

    {
      ...
      generateRegMemInstruction(
        doubleSource ? UCOMISDRegMem : UCOMISSRegMem,
        node,
        sourceRegister,
        generateX86MemoryReference(doubleSource
                                     ? cg->findOrCreate8ByteConstant(node, 0)
      ...
      generateRegMemInstruction(
        CMOVPRegMem(longTarget),
        node,
        targetRegister,
        generateX86MemoryReference(longTarget
                                     ? cg->findOrCreate8ByteConstant(node, 0)
                                     : cg->findOrCreate4ByteConstant(node, 0),
                                   cg),
        cg);
    ...
    if (sourceRegister && sourceRegister->getKind() == TR_X87 &&
        child->getReferenceCount() == 1) {
      generateFPSTiST0RegRegInstruction(
        FSTRegReg, node, sourceRegister, sourceRegister, cg);
    }
...
TR::Register*
OMR::X86::TreeEvaluator::f2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(TR::Compiler->target.is32Bit(), "AMD64 uses f2iEvaluator for this");
  return TR::TreeEvaluator::fpConvertToLong(
    node,
    cg->symRefTab()->findOrCreateRuntimeHelper(
      TR_IA32floatToLong, false, false, false),
    cg);
}

TR::Register*
OMR::X86::TreeEvaluator::f2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* targetRegister;
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    generateRegRegInstruction(
      CVTSS2SDRegReg, node, targetRegister, targetRegister, cg);
  } else {
...
TR::Register*
OMR::X86::TreeEvaluator::f2bEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::f2sEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::f2cEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::d2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(TR::Compiler->target.is32Bit(), "AMD64 uses f2iEvaluator for this");
  ...
  return TR::TreeEvaluator::fpConvertToLong(
    node,
    cg->symRefTab()->findOrCreateRuntimeHelper(
      TR_IA32doubleToLong, false, false, false),
    cg);
}

TR::Register*
OMR::X86::TreeEvaluator::d2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* targetRegister;
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    generateRegRegInstruction(
      CVTSD2SSRegReg, node, targetRegister, targetRegister, cg);
  } else {
    TR::Register* sourceRegister = cg->evaluate(child);
    if (cg->useSSEForSinglePrecision()) {
      TR::MemoryReference* tempMR =
...
TR::Register*
OMR::X86::TreeEvaluator::d2bEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::d2sEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::d2cEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  diagnostic("d2s not expected!");
  return NULL;
}
...
TR::Register*
OMR::X86::TreeEvaluator::ibits2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* target;
  TR::MemoryReference* tempMR;
...
TR::Register*
OMR::X86::TreeEvaluator::fbits2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Node* child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadVar() &&
    ...
    if (floatReg->getKind() == TR_FPR) {
      ...
      generateRegMemInstruction(
        L4RegMem, node, target, generateX86MemoryReference(*tempMR, 0, cg), cg);
    } else {
      tempMR = cg->machine()->getDummyLocalMR(TR::Int32);
      generateFPMemRegInstruction(FSTMemReg, node, tempMR, floatReg, cg);
      generateRegMemInstruction(
  ...
  if (node->normalizeNanValues()) {
    ...
    if (disableFastNormalizeNaNs) {
      ...
      TR::RegisterDependencyConditions* internalControlFlowDeps =
        generateRegisterDependencyConditions((uint8_t)0, (uint8_t)1, cg);
      internalControlFlowDeps->addPostCondition(
        treg, TR::RealRegister::NoReg, cg);
      ...
      generateLabelInstruction(
        LABEL, node, endLabel, internalControlFlowDeps, cg);
    }
  }
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (cg->useSSEForSinglePrecision()) {
    ...
    if (cg->decReferenceCount(child) == 0) {
      generateFPSTiST0RegRegInstruction(
        FSTRegReg, node, globalReg, globalReg, cg);
        ...
        for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
          if (i != fpStackSlot) {
            ...
            if (reg == childGlobalReg) {
              ...
              generateFPSTiST0RegRegInstruction(
                FSTRegReg, node, childGlobalReg, childGlobalReg, cg, true);
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (cg->useSSEForDoublePrecision()) {
    ...
    if (cg->decReferenceCount(child) == 0) {
      generateFPSTiST0RegRegInstruction(
        DSTRegReg, node, globalReg, globalReg, cg);
      cg->stopUsingRegister(globalReg);
    } else if (childGlobalReg && childGlobalReg != globalReg) {
      // int32_t fpStackSlot = globalRegNum - machine->getNumGlobalGPRs();
        ...
        for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
          if (i != fpStackSlot) {
            ...
            if (reg == childGlobalReg) {
              TR::Register* popReg = cg->machine()->getFPStackRegister(i);
              generateFPSTiST0RegRegInstruction(
                DSTRegReg, node, childGlobalReg, childGlobalReg, cg, true);
              cg->stopUsingRegister(childGlobalReg);
              cg->machine()
                ->getFPStackRegisterNode(fpStackSlot)
                ->setRegister(popReg);
...
TR::Register*
OMR::X86::TreeEvaluator::generateBranchOrSetOnFPCompare(
  TR::Node* node,
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (accRegister != NULL) {
    ...
    generateFPCompareEvalInstruction(
      FCMPEVAL, node, accRegister, accRegDep, cg);
  ...
  if (accRegister == NULL && (cmpOp == TR::iffcmpneu || cmpOp == TR::fcmpneu ||
    ...
    if (generateBranch) {
      ...
      generateRegRegInstruction(
        OR1RegReg, node, targetRegister, tempRegister, cg);
      generateRegRegInstruction(
    ...
    if (generateBranch) {
      ...
      generateRegRegInstruction(
        AND1RegReg, node, targetRegister, tempRegister, cg);
      generateRegRegInstruction(
        MOVZXReg4Reg1, node, targetRegister, targetRegister, cg);
      cg->stopUsingRegister(tempRegister);
    }
...
//
TR::Register*
OMR::X86::TreeEvaluator::compareFloatOrDoubleForOrder(
  TR::Node* node,
  ...
  TR::CodeGenerator* cg)
{
  if (((TR_X86OpCode::singleFPOp(fpCmpRegRegOpCode) &&
    ...
    return temp.xmmCompareAnalyser(
      node, xmmCmpRegRegOpCode, xmmCmpRegMemOpCode);
  } else {
...
TR::Register*
OMR::X86::TreeEvaluator::generateFPCompareResult(TR::Node* node,
                                                 TR::Register* accRegister,
                                                 TR::CodeGenerator* cg)
{
  if (accRegister != NULL) {
...
bool
OMR::X86::TreeEvaluator::canUseFCOMIInstructions(TR::Node* node,
                                                 TR::CodeGenerator* cg)
{
  TR::ILOpCodes cmpOp = node->getOpCodeValue();
                                                        ...
                                                        TR::CodeGenerator* cg)
{
  ...
  TR::Register* accRegister =
    TR::TreeEvaluator::compareFloatOrDoubleForOrder(node,
                                                    FCOMRegReg,
                                                    FCOMRegMem,
                                                    FCOMIRegReg,
                                                    UCOMISSRegReg,
                                                    ...
                                                    useFCOMIInstructions,
                                                    cg);
  return TR::TreeEvaluator::generateBranchOrSetOnFPCompare(
    node, accRegister, true, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::compareDoubleAndBranchEvaluator(TR::Node* node,
                                                         TR::CodeGenerator* cg)
{
  bool useFCOMIInstructions =
    TR::TreeEvaluator::canUseFCOMIInstructions(node, cg);
