    ...
    TR::CodeGenerator* cg)
{
    ...
    if ((node->getOpCode().isDouble() && (node->getOpCodeValue() != TR::f2d)) || (node->getOpCode().isBooleanCompare() && node->getFirstChild()->getDataType() != TR::Float) || node->getOpCodeValue() == TR::d2i || node->getOpCodeValue() == TR::d2l) {
        useFloatSet = false;
    }

#ifdef DEBUG
    else if (!(node->getOpCodeValue() == TR::f2d) && !(node->getOpCode().isFloat()) && !(node->getOpCodeValue() == TR::f2i) && !(node->getOpCodeValue() == TR::f2l)) {
...
TR::Register* OMR::X86::TreeEvaluator::floatingPointStoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    //
    if ((valueChild->getOpCodeValue() == TR::ibits2f || valueChild->getOpCodeValue() == TR::lbits2d) && !valueChild->getRegister()) {
    ...
    } else if (debug("useGPRsForFP") && (cg->getLiveRegisters(TR_GPR)->getNumberOfLiveRegisters() < cg->getMaximumNumbersOfAssignableGPRs() - 1) && valueChild->getOpCode().isLoadVar() && valueChild->getRegister() == NULL && valueChild->getReferenceCount() == 1) {
...
TR::Register* OMR::X86::TreeEvaluator::fpReturnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::RealRegister::RegNum machineReturnRegister = (returnRegister->isSinglePrecision()) ? linkageProperties.getFloatReturnRegister() : linkageProperties.getDoubleReturnRegister();
    ...
    bool isDouble)
{
    ...
    if ((node->getOpCode().isFloat() && !comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) || comp->getCurrentMethod()->isStrictFP() || comp->getOption(TR_StrictFP)) {
...
TR::Register* OMR::X86::TreeEvaluator::fpConvertToInt(TR::Node* node, TR::SymbolReference* helperSymRef, TR::CodeGenerator* cg)
{
    ...
    if (!optimizeF2IWithSSE && !optimizeD2IWithSSE2) {
        ...
        fpcw = comp->getJittedMethodSymbol()->usesSinglePrecisionMode() ? SINGLE_PRECISION_ROUND_TO_ZERO : DOUBLE_PRECISION_ROUND_TO_ZERO;

        fpcw = comp->getJittedMethodSymbol()->usesSinglePrecisionMode() ? SINGLE_PRECISION_ROUND_TO_NEAREST : DOUBLE_PRECISION_ROUND_TO_NEAREST;
...
TR::Register* OMR::X86::TreeEvaluator::fpConvertToLong(TR::Node* node, TR::SymbolReference* helperSymRef, TR::CodeGenerator* cg)
{
    ...
    if (child->getOpCode().isDouble() && cg->useSSEForDoublePrecision()) {
        ...
        //
        int16_t fpcw = comp->getJittedMethodSymbol()->usesSinglePrecisionMode() ? SINGLE_PRECISION_ROUND_TO_ZERO : DOUBLE_PRECISION_ROUND_TO_ZERO;
...
TR::Register* OMR::X86::TreeEvaluator::dRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (globalReg == NULL) {
        if (cg->useSSEForDoublePrecision()) {
            ...
            if ((comp->getCurrentMethod()->isStrictFP() || comp->getOption(TR_StrictFP)) || node->needsPrecisionAdjustment()) {
                globalReg->setMayNeedPrecisionAdjustment();
                globalReg->setNeedsPrecisionAdjustment();
            }
        }
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (accRegister == NULL && (cmpOp == TR::iffcmpneu || cmpOp == TR::fcmpneu || cmpOp == TR::ifdcmpneu || cmpOp == TR::dcmpneu)) {
        if (generateBranch) {
    ...
    } else if (accRegister == NULL && (cmpOp == TR::iffcmpeq || cmpOp == TR::fcmpeq || cmpOp == TR::ifdcmpeq || cmpOp == TR::dcmpeq)) {
        if (generateBranch) {
            TR::LabelSymbol* startLabel = TR::LabelSymbol::create(cg->trHeapMemory(), cg);
...
{
    if (
        ((TR_X86OpCode::singleFPOp(fpCmpRegRegOpCode) && cg->useSSEForSinglePrecision()) || (TR_X86OpCode::doubleFPOp(fpCmpRegRegOpCode) && cg->useSSEForDoublePrecision()))) {
...
bool OMR::X86::TreeEvaluator::canUseFCOMIInstructions(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    return (!cg->getX86ProcessorInfo().supportsFCOMIInstructions() || cmpOp == TR::iffcmpneu || cmpOp == TR::iffcmpeq || cmpOp == TR::ifdcmpneu || cmpOp == TR::ifdcmpeq || cmpOp == TR::fcmpneu || cmpOp == TR::dcmpneu || cmpOp == TR::fcmpeq || cmpOp == TR::dcmpeq) ? false : true;
}
