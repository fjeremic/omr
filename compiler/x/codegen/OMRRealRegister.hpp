...
class OMR_EXTENSIBLE RealRegister : public OMR::RealRegister {
 ...
 public:
  ...
  using OMR::RealRegister::getRealRegisterMask;  // for getting the
                                                 // _registerMask member

 protected:
  // TODO:AMD64: Consider making this back into a plain old byte for consistency
  // with other platforms.
  struct TR_RegisterBinaryEncoding {
    unsigned char
        id : 3;  // 3-bit register identifier used in ModRM and SIB bytes
    unsigned char needsRexPlusRXB : 1;  // Always need a Rex prefix with the R,
                                        // X, or B bit set
    unsigned char
        needsRexForByte : 1;  // Needs a Rex prefix when used as a byte register
    unsigned char needsDisp : 1;  // Needs a displacement field (even if zero)
                                  // when used as a base reg (ie. ebp/r13)
    unsigned char
        needsSIB : 1;  // Needs a SIB byte when used as a base reg (ie. esp/r12)
  };
};
