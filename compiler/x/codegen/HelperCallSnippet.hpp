...
class X86HelperCallSnippet : public TR::X86RestartSnippet {
  ...
  int32_t _stackPointerAdjustment; // For helper calls that need a stack frame
                                   // temporarily deallocated

  // If the displacement of the helper call instruction might be patched
  // dynamically.
  //
  bool _alignCallDisplacementForPatching;

public:
  X86HelperCallSnippet(TR::CodeGenerator *cg, TR::Node *node,
                       TR::LabelSymbol *restartLabel,
                       TR::LabelSymbol *snippetLabel,
                       TR::SymbolReference *helper,
                       int32_t stackPointerAdjustment = 0);

  X86HelperCallSnippet(TR::CodeGenerator *cg, TR::LabelSymbol *restartLabel,
                       TR::LabelSymbol *snippetLabel, TR::Node *callNode,
                       int32_t stackPointerAdjustment = 0);
  ...
  static int32_t branchDisplacementToHelper(uint8_t *callInstructionAddress,
                                            TR::SymbolReference *helper,
                                            TR::CodeGenerator *cg);

  int32_t getOffset() { return _offset; }

  uint8_t *getCallInstructionBufferAddress() {
    return _callInstructionBufferAddress;
  }
  bool getAlignCallDisplacementForPatching() {
    return _alignCallDisplacementForPatching;
  }
  void setAlignCallDisplacementForPatching(bool a) {
    _alignCallDisplacementForPatching = a;
  }
