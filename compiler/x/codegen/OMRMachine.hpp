...
public:
  ...
  TR_RegisterAssignerState(TR::Machine *m)
      : _machine(m), _registerFile(NULL), _registerAssociations(NULL),
        _spilledRegistersList(NULL) {}

  void capture();
  void install();
  TR::RegisterDependencyConditions *
  createDependenciesFromRegisterState(TR_OutlinedInstructions *oi);
...
class OMR_EXTENSIBLE Machine : public OMR::Machine {
  ...
  TR_X86FPStackRegister
      *_copiedFpRegisters[TR_X86FPStackRegister::NumRegisters];
...
public:
  ...
  void adjustRegisterUseCountsUp(TR::list<OMR::RegisterUsage *> *rul,
                                 bool adjustFuture);
  void adjustRegisterUseCountsDown(TR::list<OMR::RegisterUsage *> *rul,
                                   bool adjustFuture);
  void disassociateUnspilledBackingStorage();

  TR::Register **getRegisterAssociations() { return _registerAssociations; }
  void setRegisterAssociations(TR::Register **ra) {
    _registerAssociations = ra;
  }
  ...
  List<TR::Register> *getSpilledRegistersList() {
    return _spilledRegistersList;
  }
  void setSpilledRegistersList(List<TR::Register> *srl) {
    _spilledRegistersList = srl;
  }

  TR::RealRegister *
  findBestFreeGPRegister(TR::Instruction *currentInstruction,
                         TR::Register *virtReg,
                         TR_RegisterSizes requestedRegSize = TR_WordReg,
                         bool considerUnlatched = false);

  TR::RealRegister *freeBestGPRegister(
  ...
  TR::RealRegister *
  reverseGPRSpillState(TR::Instruction *currentInstruction,
  ...
  void
  coerceGPRegisterAssignment(TR::Instruction *currentInstruction,
                             TR::Register *virtualRegister,
                             TR_RegisterSizes requestedRegSize = TR_WordReg);
  ...
  TR_X86FPStackRegister *getFPStackLocationPtr(int32_t location) {
    return (location > TR_X86FPStackRegister::fpStackEmpty &&
  ...
  bool isFPStackFull() {
    return (_fpTopOfStack + 1 == TR_X86FPStackRegister::fpStackFull) ? true
                                                                     : false;
  }
  ...
  TR_X86FPStackRegister *getFPStackRegister(int32_t location) {
    return _fpRegisters[location];
  }
  void setFPStackRegister(int32_t location, TR_X86FPStackRegister *reg) {
    _fpRegisters[location] = reg;
  }
  TR_X86FPStackRegister *getCopiedFPStackRegister(int32_t location) {
    return _copiedFpRegisters[location];
  }
  void setCopiedFPStackRegister(int32_t location, TR_X86FPStackRegister *reg) {
    _copiedFpRegisters[location] = reg;
  }
  TR::Node *getFPStackRegisterNode(int32_t location) {
    return _fpRegisterNodes[location];
  }
  void setFPStackRegisterNode(int32_t location, TR::Node *node) {
    _fpRegisterNodes[location] = node;
  }
  ...
  TR::Instruction *fpStackFXCH(TR::Instruction *currentInstruction,
                               TR::Register *virtReg, bool generateCode = true);

  TR::Instruction *fpStackFXCH(TR::Instruction *currentInstruction,
                               int32_t stackReg);

  void fpCoerceRegistersToTopOfStack(TR::Instruction *currentInstruction,
                                     TR::Register *X, TR::Register *Y,
                                     bool strict);
  ...
  TR::Register *getXMMGlobalRegister(int32_t regNum) {
    return _xmmGlobalRegisters[regNum];
  }
  void setXMMGlobalRegister(int32_t regNum, TR::Register *reg) {
    _xmmGlobalRegisters[regNum] = reg;
  }
...
#if defined(DEBUG)
  void printGPRegisterStatus(TR_FrontEnd *, TR::RealRegister **registerFile,
                             TR::FILE *pOutFile);
  void printFPRegisterStatus(TR_FrontEnd *, TR::FILE *pOutFile);
#endif

protected:
  Machine(uint8_t numIntRegs, uint8_t numFPRegs, TR::CodeGenerator *cg,
          TR::Register **registerAssociations, uint8_t numGlobalGPRs,
          uint8_t numGlobal8BitGPRs, uint8_t numGlobalFPRs,
