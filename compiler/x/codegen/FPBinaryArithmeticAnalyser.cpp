...
void TR_X86FPBinaryArithmeticAnalyser::genericFPAnalyser(TR::Node* root)
{
    ...
    if ((root->getOpCode().isFloat() && !comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) || comp->getCurrentMethod()->isStrictFP() || comp->getOption(TR_StrictFP) || operandNeedsScaling) {
