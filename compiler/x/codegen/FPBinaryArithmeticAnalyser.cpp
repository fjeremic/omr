...
void
TR_X86FPBinaryArithmeticAnalyser::setInputs(TR::Node* firstChild,
...
void
TR_X86FPBinaryArithmeticAnalyser::genericFPAnalyser(TR::Node* root)
{

  TR::Register *targetRegister = NULL, *sourceRegister = NULL, *tempReg = NULL;
  TR::Node *targetChild = NULL, *sourceChild = NULL,
           *opChild[2] = { NULL, NULL };
  ...
  if ((comp->getCurrentMethod()->isStrictFP() ||
    ...
    if (root->getOpCode().isMul() ||
      ...
      generateFPRegMemInstruction(
        DLDRegMem, root, scalingRegister, constMR, _cg);
  ...
  if (isCopyReg()) {
    ...
    generateFPST0STiRegRegInstruction(
      FLDRegReg, root, tempReg, targetRegister, _cg);
  ...
  if (operandNeedsScaling) {
    generateFPST0ST1RegRegInstruction(
      FSCALERegReg, root, targetRegister, scalingRegister, _cg);
  }

  root->setRegister(targetRegister);

  if (isOpRegReg()) {
    generateFPArithmeticRegRegInstruction(
      getRegRegOp(), root, targetRegister, sourceRegister, _cg);
  } else if (isOpRegMem()) {
    TR::MemoryReference* tempMR = generateX86MemoryReference(sourceChild, _cg);
    generateFPRegMemInstruction(
      getRegMemOp(), root, targetRegister, tempMR, _cg);
  ...
  if (operandNeedsScaling) {
    ...
    generateFPST0ST1RegRegInstruction(
      FSCALERegReg, root, root->getRegister(), scalingRegister, _cg);
    generateFPSTiST0RegRegInstruction(
      FSTRegReg, root, scalingRegister, scalingRegister, _cg);
