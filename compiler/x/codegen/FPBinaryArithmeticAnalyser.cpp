...
void TR_X86FPBinaryArithmeticAnalyser::genericFPAnalyser(TR::Node* root)
{
  ...
  do {
    setInputs(opChild[0], opChild[0]->getRegister(), opChild[1], opChild[1]->getRegister());
  ...
  if ((comp->getCurrentMethod()->isStrictFP() || comp->getOption(TR_StrictFP)) && root->getOpCode().isDouble()) {
    ...
    if (root->getOpCode().isMul() || (scaleX87StrictFPDivides && root->getOpCode().isDiv())) {
  ...
  if (isOpRegReg()) {
    ...
    if (sourceChild->getOpCodeValue() == TR::i2f || sourceChild->getOpCodeValue() == TR::i2d) {
  ...
  if ((root->getOpCode().isFloat() && !comp->getJittedMethodSymbol()->usesSinglePrecisionMode())
    || comp->getCurrentMethod()->isStrictFP() || comp->getOption(TR_StrictFP) || operandNeedsScaling) {
...
const TR_X86OpCodes TR_X86FPBinaryArithmeticAnalyser::_opCodePackage[kNumFPPackages][kNumFPArithVariants] = {
  ...
  // Unknown
  { BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op, BADIA32Op },

  // fadd
  { FADDRegReg, FADDRegReg, FADDRegMem, FADDRegMem, FSADDRegMem, FIADDRegMem, FSADDRegMem, FIADDRegMem },

  // dadd
  { DADDRegReg, DADDRegReg, DADDRegMem, DADDRegMem, DSADDRegMem, DIADDRegMem, DSADDRegMem, DIADDRegMem },

  // fmul
  { FMULRegReg, FMULRegReg, FMULRegMem, FMULRegMem, FSMULRegMem, FIMULRegMem, FSMULRegMem, FIMULRegMem },

  // dmul
  { DMULRegReg, DMULRegReg, DMULRegMem, DMULRegMem, DSMULRegMem, DIMULRegMem, DSMULRegMem, DIMULRegMem },

  // fsub
  { FSUBRegReg, FSUBRRegReg, FSUBRegMem, FSUBRRegMem, FSSUBRegMem, FISUBRegMem, FSSUBRRegMem, FISUBRRegMem },

  // dsub
  { DSUBRegReg, DSUBRRegReg, DSUBRegMem, DSUBRRegMem, DSSUBRegMem, DISUBRegMem, DSSUBRRegMem, DISUBRRegMem },

  // fdiv
  { FDIVRegReg, FDIVRRegReg, FDIVRegMem, FDIVRRegMem, FSDIVRegMem, FIDIVRegMem, FSDIVRRegMem, FIDIVRRegMem },

  // ddiv
  { DDIVRegReg, DDIVRRegReg, DDIVRegMem, DDIVRRegMem, DSDIVRegMem, DIDIVRegMem, DSDIVRRegMem, DIDIVRRegMem }
};
