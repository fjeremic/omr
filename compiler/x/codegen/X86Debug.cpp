...
void TR_Debug::printPrefix(TR::FILE *pOutFile, TR::Instruction *instr) {
  ...
  printPrefix(pOutFile, instr, instr->getBinaryEncoding(),
              instr->getBinaryLength());
}

void TR_Debug::printDependencyConditions(
    TR_X86RegisterDependencyGroup *conditions, uint8_t numConditions,
    char *prefix, TR::FILE *pOutFile) {
  char buf[32];
  char *cursor;
  int len, i;
  ...
  for (i = 0; i < numConditions; i++) {
    ...
    *(cursor++) = '(';
    TR::RealRegister::RegNum r =
        (conditions->getRegisterDependency(i))->getRealRegister();
    ...
    *(cursor + 9) = 0x00;
    ...
    if (conditions->getRegisterDependency(i)->getRegister()) {
      printFullRegInfo(pOutFile,
                       conditions->getRegisterDependency(i)->getRegister());
    } else {
...
void TR_Debug::printFullRegisterDependencyInfo(
    TR::FILE *pOutFile, TR::RegisterDependencyConditions *conditions) {
  if (pOutFile == NULL) return;

  if (conditions->getNumPreConditions() > 0) {
    printDependencyConditions(conditions->getPreConditions(),
                              conditions->getNumPreConditions(), "Pre",
                              pOutFile);
  }

  if (conditions->getNumPostConditions() > 0) {
    printDependencyConditions(conditions->getPostConditions(),
                              conditions->getNumPostConditions(), "Post",
                              pOutFile);
  }
}

void TR_Debug::dumpDependencyGroup(TR::FILE *pOutFile,
                                   TR_X86RegisterDependencyGroup *group,
                                   int32_t numConditions, char *prefix,
  ...
  for (i = 0; i < numConditions; ++i) {
    ...
    if (omitNullDependencies)
      if (!virtReg && r != TR::RealRegister::AllFPRegisters) continue;
  ...
  if (!foundDep) trfprintf(pOutFile, " None");
}
...
void TR_Debug::dumpDependencies(TR::FILE *pOutFile, TR::Instruction *instr) {
  ...
  if (pOutFile == NULL ||    // no dump file
      (_cg->getStackAtlas()  // not in instruction selection
       && !(_registerAssignmentTraceFlags &
                TRACERA_IN_PROGRESS &&  // not in register assignment...
            _comp->getOptions()->getRegisterAssignmentTraceOption(
                TR_TraceRADependencies))  // or dependencies are not traced
       ))
    return;

  TR::RegisterDependencyConditions *deps = instr->getDependencyConditions();
  if (!deps) return;  // Nothing to dump

  if (deps->getNumPreConditions() > 0)
    dumpDependencyGroup(pOutFile, deps->getPreConditions(),
                        deps->getNumPreConditions(), " PRE", true);

  if (deps->getNumPostConditions() > 0)
    dumpDependencyGroup(pOutFile, deps->getPostConditions(),
                        deps->getNumPostConditions(), "POST", true);

  trfflush(pOutFile);
}

void TR_Debug::printRegisterInfoHeader(TR::FILE *pOutFile,
                                       TR::Instruction *instr) {
  if (pOutFile == NULL) return;

  trfprintf(pOutFile, "\n\n  FP stack height: %d",
            _cg->machine()->getFPTopOfStack() + 1);
  trfprintf(pOutFile,
            "\n  Referenced Regs:        Register         State        "
            "Assigned      Total Future Flags\n");
  trfflush(pOutFile);
}

void TR_Debug::printReferencedRegisterInfo(TR::FILE *pOutFile,
                                           TR::Instruction *instr) {
  if (pOutFile == NULL) return;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86PaddingInstruction *instr) {
  ...
  if (instr->getBinaryEncoding())
    ...
    trfprintf(pOutFile, "nop\t\t\t%s Padding (%d byte%s)", commentString(),
              instr->getLength(), (instr->getLength() == 1) ? "" : "s");
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86AlignmentInstruction *instr) {
  ...
  if (margin)
    trfprintf(pOutFile, "Alignment (boundary=%d, margin=%d)",
              instr->getBoundary(), instr->getMargin());
  else
...
void TR_Debug::printBoundaryAvoidanceInfo(
    TR::FILE *pOutFile, TR::X86BoundaryAvoidanceInstruction *instr) {
  ...
  const char *sep = "";
  for (const TR_AtomicRegion *region = instr->getAtomicRegions();
       region->getLength(); region++) {
    trfprintf(pOutFile, "%s0x%x:%d", sep, region->getStart(),
              region->getLength());
    sep = ",";
  }
  trfprintf(pOutFile, "]");
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86BoundaryAvoidanceInstruction *instr) {
  if (pOutFile == NULL) return;
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86PatchableCodeAlignmentInstruction *instr) {
  if (pOutFile == NULL) return;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86LabelInstruction *instr) {
  ...
  if (instr->getOpCodeValue() == LABEL) {
    ...
    if (label->isStartInternalControlFlow())
      trfprintf(pOutFile, "\t%s (Start of internal control flow)",
                commentString());
    else if (label->isEndInternalControlFlow())
      trfprintf(pOutFile, "\t%s (End of internal control flow)",
                commentString());
  } else {
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86FenceInstruction *instr) {
  ...
  if (instr->getFenceNode()->getNumRelocations() > 0) {
    ...
    else if (instr->getFenceNode()->getRelocationType() ==
             TR_ExternalAbsoluteAddress)
    ...
    if (!_comp->getOption(TR_MaskAddresses)) {
      for (int32_t i = 0; i < instr->getFenceNode()->getNumRelocations(); ++i)
        trfprintf(pOutFile, " " POINTER_PRINTF_FORMAT,
                  instr->getFenceNode()->getRelocationDestination(i));
    }
    trfprintf(pOutFile, " ]");
  }

  printInstructionComment(
      pOutFile, (instr->getFenceNode()->getNumRelocations() > 0) ? 1 : 3,
      instr);
...
#ifdef J9_PROJECT_SPECIFIC
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86VirtualGuardNOPInstruction *instr) {
  // *this    swipeable for degubbing purposes
  if (pOutFile == NULL) return;

  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s Site:" POINTER_PRINTF_FORMAT ", ",
            getMnemonicName(&instr->getOpCode()), instr->getSite());
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86ImmInstruction *instr) {
  ...
  if (instr->getOpCode().isCallImmOp() &&
    ...
    if (symRef->isUnresolved())
      ...
      trfprintf(pOutFile, " (" POINTER_PRINTF_FORMAT ")",
                instr->getSourceImmediate());  // TODO:AMD64: Target address
                                               // gets truncated
  } else {
    printIntConstant(pOutFile, instr->getSourceImmediate(), 16,
                     getImmediateSizeFromInstruction(instr), true);
    printInstructionComment(pOutFile, 2, instr);
  }
...
void TR_Debug::print(TR::FILE *pOutFile, TR::AMD64Imm64SymInstruction *instr) {
  ...
  if (sym->getMethodSymbol() && name) {
    trfprintf(pOutFile, "%-24s%s %s (" POINTER_PRINTF_FORMAT ")", name,
              commentString(), getOpCodeName(&instr->getOpCode()),
              instr->getSourceImmediate());
  } else if (sym->getLabelSymbol() && name) {
    if (sym->getLabelSymbol()->getSnippet())
      trfprintf(pOutFile, "%-24s%s %s (%s)", name, commentString(),
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86VFPDedicateInstruction *instr) {
  ...
  trfprintf(pOutFile, "%s vfpDedicate %s", commentString(),
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86VFPReleaseInstruction *instr) {
  ...
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "vfpRelease [%s]",
            getName(instr->getDedicateInstruction()));
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86VFPCallCleanupInstruction *instr) {
  if (pOutFile == NULL) return;

  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "vfpCallCleanup (%d bytes)",
            instr->getStackPointerAdjustment());
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86ImmSymInstruction *instr) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    // new code patching might have a call to a snippet label, which is not a
    // method
    if (!sym->isLabel()) {
  ...
  if (sym->getMethodSymbol() && name) {
    ...
    if (sym->getLabelSymbol()->getSnippet())
      trfprintf(pOutFile, "%s %s (%s)", commentString(),
                getOpCodeName(&instr->getOpCode()),
                getName(sym->getLabelSymbol()->getSnippet()));
    else
      trfprintf(pOutFile, "%s %s (" POINTER_PRINTF_FORMAT ")", commentString(),
    ...
    trfprintf(pOutFile, " \t\t%s %s", commentString(),
              getOpCodeName(&instr->getOpCode()));
  }
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86RegInstruction *instr) {
  ...
  if (!(instr->getOpCode().targetRegIsImplicit() != 0))
    print(pOutFile, instr->getTargetRegister(),
          getTargetSizeFromInstruction(instr));
  printInstructionComment(pOutFile, 3, instr);
  dumpDependencies(pOutFile, instr);
  trfflush(pOutFile);
}
...
void TR_Debug::printReferencedRegisterInfo(TR::FILE *pOutFile,
                                           TR::X86RegRegRegInstruction *instr) {
  if (pOutFile == NULL) return;
...
int32_t TR_Debug::printPrefixAndMnemonicWithoutBarrier(TR::FILE *pOutFile,
                                                       TR::Instruction *instr,
                                                       int32_t barrier) {
  int32_t barrierLength =
      ::estimateMemoryBarrierBinaryLength(barrier, _comp->cg());
  ...
  trfprintf(pOutFile, "%s%s\t", (barrier & LockPrefix) ? "lock " : "",
            getMnemonicName(&instr->getOpCode()));

  return nonBarrierLength;
}

void TR_Debug::printPrefixAndMemoryBarrier(TR::FILE *pOutFile,
                                           TR::Instruction *instr,
                                           int32_t barrier,
                                           int32_t barrierOffset) {
  int32_t barrierLength =
      ::estimateMemoryBarrierBinaryLength(barrier, _comp->cg());
  uint8_t *barrierStart = instr->getBinaryEncoding()
                              ? (instr->getBinaryEncoding() + barrierOffset)
                              : NULL;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86MemInstruction *instr) {
  ...
  int32_t barrier = memoryBarrierRequired(
      instr->getOpCode(), instr->getMemoryReference(), _cg, false);
  int32_t barrierOffset =
      printPrefixAndMnemonicWithoutBarrier(pOutFile, instr, barrier);
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86RegMemInstruction *instr) {
  ...
  if (symbol && symbol->isSpillTempAuto()) {
    trfprintf(pOutFile, "%s, spilled for %s", commentString(),
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86RegRegMemInstruction *instr) {
  ...
  if (!(instr->getOpCode().sourceRegIsImplicit() != 0)) {
    print(pOutFile, instr->getSource2ndRegister(),
          getSourceSizeFromInstruction(instr));
    trfprintf(pOutFile, ", ");
  }
  print(pOutFile, instr->getMemoryReference(),
...
void TR_Debug::printReferencedRegisterInfo(TR::FILE *pOutFile,
                                           TR::X86RegRegMemInstruction *instr) {
  if (pOutFile == NULL) return;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86FPRegRegInstruction *instr) {
  ...
  if (!(instr->getOpCode().sourceRegIsImplicit() != 0))
    print(pOutFile, instr->getSourceRegister());
  printInstructionComment(pOutFile, 2, instr);
  printFPRegisterComment(pOutFile, instr->getTargetRegister(),
                         instr->getSourceRegister());
  dumpDependencies(pOutFile, instr);
  trfflush(pOutFile);
}
...
void TR_Debug::print(TR::FILE *pOutFile, TR::MemoryReference *mr,
                     TR_RegisterSizes operandSize) {
  if (pOutFile == NULL) return;

  const char *typeSpecifier[7] = {"byte",    // TR_ByteReg
                                  "word",    // TR_HalfWordReg
                                  "dword",   // TR_WordReg
  ...
  TR_RegisterSizes addressSize =
      (TR::Compiler->target.cpu.isAMD64() ? TR_DoubleWordReg : TR_WordReg);
  bool hasTerm = false;
  ...
  if (sym != NULL || mr->getSymbolReference().getOffset() != 0) {
    ...
    if (!hasPrecedingTerm) {
      ...
      // Treat this as a relative offset and display in base10, or in base16 if
      // the displacement was explicitly forced wider.
      //
  ...
  if (!hasTerm) {
    // This must be an absolute memory reference (a constant data snippet or a
    // label)
    //
    ...
    if (mr->getLabel()) {
      ...
      if (disp) {
        trfprintf(pOutFile, " : ");
        printHexConstant(pOutFile, disp,
                         TR::Compiler->target.is64Bit() ? 16 : 8, false);
      }
    } else if (disp) {
      printHexConstant(pOutFile,
                       TR::Compiler->target.is64Bit() ? disp : (uint32_t)disp,
                       TR::Compiler->target.is64Bit() ? 16 : 8, true);
    } else if (cds) {
...
int32_t TR_Debug::printIntConstant(TR::FILE *pOutFile, int64_t value,
                                   int8_t radix, TR_RegisterSizes size,
                                   bool padWithZeros) {
...
int32_t TR_Debug::printDecimalConstant(TR::FILE *pOutFile, int64_t value,
                                       int8_t width, bool padWithZeros) {
  trfprintf(pOutFile, "%lld", value);
  return 0;
}

int32_t TR_Debug::printHexConstant(TR::FILE *pOutFile, int64_t value,
                                   int8_t width, bool padWithZeros) {
  // we probably need to revisit generateMasmListingSyntax
  const char *prefix = TR::Compiler->target.isLinux()
                           ? "0x"
                           : (_cg->generateMasmListingSyntax() ? "0" : "0x");
  const char *suffix = TR::Compiler->target.isLinux()
                           ? ""
                           : (_cg->generateMasmListingSyntax() ? "h" : "");
...
void TR_Debug::printFPRegisterComment(TR::FILE *pOutFile, TR::Register *source,
                                      TR::Register *target) {
  trfprintf(pOutFile, " using ");
  if (target) print(pOutFile, target);
  if (source && target) trfprintf(pOutFile, " & ");
  if (source) print(pOutFile, source);
}

void TR_Debug::printInstructionComment(TR::FILE *pOutFile, int32_t tabStops,
                                       TR::Instruction *instr) {
  while (tabStops-- > 0) trfprintf(pOutFile, "\t");
...
void TR_Debug::printMemoryReferenceComment(TR::FILE *pOutFile,
                                           TR::MemoryReference *mr) {
  if (pOutFile == NULL) return;

  TR::Symbol *symbol = mr->getSymbolReference().getSymbol();

  if (symbol == NULL && mr->getSymbolReference().getOffset() == 0) return;
...
TR_RegisterSizes TR_Debug::getTargetSizeFromInstruction(
    TR::Instruction *instr) {
  ...
  else if ((instr->getOpCode().hasLongTarget() != 0) ||
           (instr->getOpCode().doubleFPOp() != 0))
...
TR_RegisterSizes TR_Debug::getSourceSizeFromInstruction(
    TR::Instruction *instr) {
  ...
  else if ((instr->getOpCode().hasLongSource() != 0) ||
           (instr->getOpCode().doubleFPOp() != 0))
...
TR_RegisterSizes TR_Debug::getImmediateSizeFromInstruction(
    TR::Instruction *instr) {
...
void TR_Debug::printX86GCRegisterMap(TR::FILE *pOutFile,
                                     TR::GCRegisterMap *map) {
  TR::Machine *machine = _cg->machine();

  trfprintf(pOutFile, "    slot pushes: %d",
            ((map->getMap() & _cg->getRegisterMapInfoBitsMask()) >> 16));
  ...
  for (int i = TR::RealRegister::FirstGPR;
    ...
    if (map->getMap() &
      ...
      trfprintf(pOutFile, "%s ",
                getName(machine->getRealRegister((TR::RealRegister::RegNum)i)));
  }

  trfprintf(pOutFile, "}\n");
}

void TR_Debug::print(TR::FILE *pOutFile, TR::RealRegister *reg,
...
const char *TR_Debug::getName(uint32_t realRegisterIndex,
                              TR_RegisterSizes size) {
...
void TR_Debug::dumpInstructionWithVFPState(TR::Instruction *instr,
                                           const TR_VFPState *prevState) {
  if (_file != NULL) {
...
void TR_Debug::printRegRegInstruction(TR::FILE *pOutFile, const char *opCode,
...
void TR_Debug::printRegMemInstruction(TR::FILE *pOutFile, const char *opCode,
                                      TR::RealRegister *reg,
...
void TR_Debug::printRegImmInstruction(TR::FILE *pOutFile, const char *opCode,
                                      TR::RealRegister *reg, int32_t imm) {
  trfprintf(pOutFile, "%s\t", opCode);
  print(pOutFile, reg);
  if (imm <= 1024)
...
void TR_Debug::printMemRegInstruction(TR::FILE *pOutFile, const char *opCode,
                                      TR::RealRegister *base, int32_t offset,
                                      TR::RealRegister *reg) {
  trfprintf(pOutFile, "%s\t", opCode);
  trfprintf(pOutFile, "[");
...
void TR_Debug::printMemImmInstruction(TR::FILE *pOutFile, const char *opCode,
                                      TR::RealRegister *base, int32_t offset,
                                      int32_t imm) {
  trfprintf(pOutFile, "%s\t", opCode);
  trfprintf(pOutFile, "[");
...
void TR_Debug::printLabelInstruction(TR::FILE *pOutFile, const char *opCode,
                                     TR::LabelSymbol *label) {
  trfprintf(pOutFile, "%s\t", opCode);
  print(pOutFile, label);
}

const char callRegName64[][5] = {"rax", "rsi", "rdx", "rcx"};

const char callRegName32[][5] = {"eax", "esi", "edx", "ecx"};

static const char *getInterpretedMethodNameHelper(
    TR::MethodSymbol *methodSymbol, TR::DataType type) {
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86CallSnippet *snippet) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    for (count = 0; count < callNode->getNumChildren(); count++) {
      ...
      switch (type) {
        ...
        case TR::Int32:
          ...
          trfprintf(pOutFile,
                    "mov \tdword ptr[rsp+%d], %s\t\t#save registers for "
                    "interpreter call snippet",
        ...
        case TR::Int64:
          ...
          trfprintf(pOutFile,
                    "mov \tqword ptr[rsp+%d], %s\t\t#save registers for "
                    "interpreter call snippet",
    ...
    printPrefix(pOutFile, NULL, bufferPos, 5);
    trfprintf(
        pOutFile, "jmp \t%s\t\t# jump out of snippet code",
...
#ifdef TR_TARGET_64BIT
uint8_t *TR_Debug::printArgumentFlush(
    TR::FILE *pOutFile, TR::Node *callNode,
  ...
  for (int i = callNode->getFirstArgumentIndex();
       i < callNode->getNumChildren(); i++) {
      ...
      case TR::Int32:
        if (numGPArgs < linkageProperties.getNumIntegerArgumentRegisters()) {
          ...
          TR::RealRegister::RegNum reg =
              linkageProperties.getIntegerArgumentRegister(numGPArgs);
      ...
      case TR::Int64:
        if (numGPArgs < linkageProperties.getNumIntegerArgumentRegisters()) {
          ...
          TR::RealRegister::RegNum reg =
              linkageProperties.getIntegerArgumentRegister(numGPArgs);
          regName =
              getName(_cg->machine()->getRealRegister(reg), TR_DoubleWordReg);
        }
        numGPArgs++;
        break;
      case TR::Float:
        if (numFPArgs < linkageProperties.getNumFloatArgumentRegisters()) {
          ...
          modrmOffset = 3;
          TR::RealRegister::RegNum reg =
              linkageProperties.getFloatArgumentRegister(numFPArgs);
          regName =
              getName(_cg->machine()->getRealRegister(reg), TR_QuadWordReg);
        }
        numFPArgs++;
        break;
      case TR::Double:
    ...
    if (opCodeName) {
      ...
      switch (bufferPos[modrmOffset] & 0xc0) {
        case 0x40:
          ...
          break;
        default:
          TR_ASSERT(0, "Unexpected Mod/RM value 0x%02x",
                    bufferPos[modrmOffset]);
          // fall through
        case 0x80:
      ...
      if (isFlushToStack)
        trfprintf(pOutFile, "%s\t[rsp +%d], %s", opCodeName, displacement,
                  regName);
      else  // Flush from stack to argument registers.
        trfprintf(pOutFile, "%s\t%s, [rsp +%d]", opCodeName, regName,
                  displacement);
...
const char *TR_Debug::getOpCodeName(TR_X86OpCode *opCode) {
  return opCodeToNameMap[opCode->getOpCodeValue()];
}

const char *TR_Debug::getMnemonicName(TR_X86OpCode *opCode) {
