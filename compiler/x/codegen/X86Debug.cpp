...
void TR_Debug::printDependencyConditions(
  TR_X86RegisterDependencyGroup* conditions, uint8_t numConditions, char* prefix, TR::FILE* pOutFile)
{
  char buf[32];
  char* cursor;
  int len, i;
...
void TR_Debug::dumpDependencyGroup(TR::FILE* pOutFile, TR_X86RegisterDependencyGroup* group, int32_t numConditions,
...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86AlignmentInstruction* instr)
{
  ...
  printPrefix(pOutFile, instr);
  if (instr->getBinaryEncoding())
    trfprintf(pOutFile, "nop (%d byte%s)\t\t%s ", instr->getBinaryLength(), (instr->getBinaryLength() == 1) ? "" : "s",
      commentString());
  else
...
void TR_Debug::print(TR::FILE* pOutFile, TR::MemoryReference* mr, TR_RegisterSizes operandSize)
{
  ...
  if (!hasTerm) {
    ...
    if (mr->getLabel()) {
      ...
      printHexConstant(pOutFile, TR::Compiler->target.is64Bit() ? disp : (uint32_t)disp,
...
static const char* getInterpretedMethodNameHelper(TR::MethodSymbol* methodSymbol, TR::DataType type)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    for (count = 0; count < callNode->getNumChildren(); count++) {
      ...
      case TR::Int32:
        ...
        trfprintf(pOutFile, "mov \tdword ptr[rsp+%d], %s\t\t#save registers for interpreter call snippet", offset,
          callRegName32[count]);
        offset -= 8;
        bufferPos += 4;
        break;
      ...
      case TR::Int64:
        ...
        trfprintf(pOutFile, "mov \tqword ptr[rsp+%d], %s\t\t#save registers for interpreter call snippet", offset,
          callRegName64[count]);
        offset -= 8;
        bufferPos += 5;
        break;
