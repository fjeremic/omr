...
void
TR_Debug::printDependencyConditions(TR_X86RegisterDependencyGroup* conditions,
...
void
TR_Debug::printFullRegisterDependencyInfo(
  TR::FILE* pOutFile,
  TR::RegisterDependencyConditions* conditions)
{
  ...
  if (conditions->getNumPreConditions() > 0) {
    printDependencyConditions(conditions->getPreConditions(),
                              conditions->getNumPreConditions(),
                              "Pre",
                              pOutFile);
  }
  ...
  if (conditions->getNumPostConditions() > 0) {
    printDependencyConditions(conditions->getPostConditions(),
                              conditions->getNumPostConditions(),
                              "Post",
                              pOutFile);
  }
}
...
TR_Debug::dumpDependencies(TR::FILE* pOutFile, TR::Instruction* instr)
{
  ...
  if (deps->getNumPreConditions() > 0)
    dumpDependencyGroup(pOutFile,
                        deps->getPreConditions(),
...
void
TR_Debug::printReferencedRegisterInfo(TR::FILE* pOutFile,
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86PaddingInstruction* instr)
{
  ...
  if (instr->getBinaryEncoding())
    trfprintf(pOutFile,
              "nop (%d byte%s)\t\t%s Padding (%d byte%s)",
...
void
TR_Debug::printBoundaryAvoidanceInfo(TR::FILE* pOutFile,
                                     TR::X86BoundaryAvoidanceInstruction* instr)
{
  ...
  for (const TR_AtomicRegion* region = instr->getAtomicRegions();
       ...
       region++) {
    trfprintf(
      pOutFile, "%s0x%x:%d", sep, region->getStart(), region->getLength());
    sep = ",";
  }
  trfprintf(pOutFile, "]");
}
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86LabelInstruction* instr)
{
  ...
  if (instr->getOpCodeValue() == LABEL) {
    ...
    if (label->isStartInternalControlFlow())
      trfprintf(
        pOutFile, "\t%s (Start of internal control flow)", commentString());
    else if (label->isEndInternalControlFlow())
      trfprintf(
        pOutFile, "\t%s (End of internal control flow)", commentString());
  } else {
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86FenceInstruction* instr)
{
  ...
  printInstructionComment(
    pOutFile, (instr->getFenceNode()->getNumRelocations() > 0) ? 1 : 3, instr);
  ...
  if (instr->getOpCode().isCallImmOp() &&
    ...
    if (symRef->isUnresolved())
      ...
      trfprintf(
        ...
        " (" POINTER_PRINTF_FORMAT ")",
        instr
          ->getSourceImmediate()); // TODO:AMD64: Target address gets truncated
  } else {
    printIntConstant(pOutFile,
                     instr->getSourceImmediate(),
                     16,
                     getImmediateSizeFromInstruction(instr),
                     true);
    printInstructionComment(pOutFile, 2, instr);
  }
...
TR_Debug::print(TR::FILE* pOutFile, TR::AMD64Imm64Instruction* instr)
{
  ...
  if (instr->getOpCode().isCallImmOp() &&
    ...
    if (symRef->isUnresolved())
      ...
      trfprintf(
        pOutFile, " (" POINTER_PRINTF_FORMAT ")", instr->getSourceImmediate());
  } else {
    printIntConstant(pOutFile,
                     instr->getSourceImmediate(),
                     16,
...
void
TR_Debug::print(TR::FILE* pOutFile, TR::X86VFPSaveInstruction* instr)
{
  if (pOutFile == NULL)
    return;
...
void
TR_Debug::print(TR::FILE* pOutFile, TR::X86VFPRestoreInstruction* instr)
{
  if (pOutFile == NULL)
    return;
...
void
TR_Debug::print(TR::FILE* pOutFile, TR::X86VFPDedicateInstruction* instr)
{
  if (pOutFile == NULL)
    return;
...
void
TR_Debug::print(TR::FILE* pOutFile, TR::X86VFPReleaseInstruction* instr)
{
  if (pOutFile == NULL)
    return;
  ...
  printPrefix(pOutFile, instr);
  trfprintf(
    pOutFile, "vfpRelease [%s]", getName(instr->getDedicateInstruction()));
...
void
TR_Debug::print(TR::FILE* pOutFile, TR::X86VFPCallCleanupInstruction* instr)
{
  if (pOutFile == NULL)
    return;
  ...
  printPrefix(pOutFile, instr);
  trfprintf(
    pOutFile, "vfpCallCleanup (%d bytes)", instr->getStackPointerAdjustment());
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86RegRegImmInstruction* instr)
{
  ...
  printIntConstant(pOutFile,
                   ...
                   true);
  trfprintf(
    pOutFile, " \t%s %s", commentString(), getOpCodeName(&instr->getOpCode()));
  dumpDependencies(pOutFile, instr);
  trfflush(pOutFile);
}
...
int32_t
TR_Debug::printPrefixAndMnemonicWithoutBarrier(TR::FILE* pOutFile,
                                               TR::Instruction* instr,
                                               int32_t barrier)
{
...
void
TR_Debug::printPrefixAndMemoryBarrier(TR::FILE* pOutFile,
                                      TR::Instruction* instr,
                                      int32_t barrier,
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86MemInstruction* instr)
{
  ...
  print(
    pOutFile, instr->getMemoryReference(), getTargetSizeFromInstruction(instr));
  printInstructionComment(pOutFile, 2, instr);
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86RegMemInstruction* instr)
{
  ...
  print(
    pOutFile, instr->getMemoryReference(), getSourceSizeFromInstruction(instr));
  printInstructionComment(pOutFile, 2, instr);
  printMemoryReferenceComment(pOutFile, instr->getMemoryReference());
  TR::Symbol* symbol =
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86FPRegRegInstruction* instr)
{
  ...
  printInstructionComment(pOutFile, 2, instr);
  printFPRegisterComment(
    pOutFile, instr->getTargetRegister(), instr->getSourceRegister());
  dumpDependencies(pOutFile, instr);
  trfflush(pOutFile);
}
  ...
  if (!hasTerm) {
    ...
    if (mr->getLabel()) {
      ...
      if (disp) {
        trfprintf(pOutFile, " : ");
        printHexConstant(
          pOutFile, disp, TR::Compiler->target.is64Bit() ? 16 : 8, false);
      }
    } else if (disp) {
...
int32_t
TR_Debug::printIntConstant(TR::FILE* pOutFile,
                           int64_t value,
                           int8_t radix,
...
int32_t
TR_Debug::printDecimalConstant(TR::FILE* pOutFile,
                               int64_t value,
                               int8_t width,
                               bool padWithZeros)
{
...
int32_t
TR_Debug::printHexConstant(TR::FILE* pOutFile,
                           int64_t value,
                           int8_t width,
                           bool padWithZeros)
{
...
void
TR_Debug::printFPRegisterComment(TR::FILE* pOutFile,
                                 TR::Register* source,
                                 TR::Register* target)
{
...
void
TR_Debug::printMemoryReferenceComment(TR::FILE* pOutFile,
                                      TR::MemoryReference* mr)
{
  if (pOutFile == NULL)
    return;
...
void
TR_Debug::dumpInstructionWithVFPState(TR::Instruction* instr,
  ...
  if (_file != NULL) {
    ...
    if (prevState && (vfpState != *prevState)) {
      trfprintf(_file,
                "\n\t%s VFP=%s+%d",
                commentString(),
                getName(vfpState._register),
                vfpState._displacement);
    }
    trfflush(_file);
  }
}
...
void
TR_Debug::printRegRegInstruction(TR::FILE* pOutFile,
                                 const char* opCode,
...
void
TR_Debug::printRegMemInstruction(TR::FILE* pOutFile,
                                 const char* opCode,
                                 TR::RealRegister* reg,
...
void
TR_Debug::printRegImmInstruction(TR::FILE* pOutFile,
                                 const char* opCode,
                                 TR::RealRegister* reg,
                                 int32_t imm)
{
...
void
TR_Debug::printMemRegInstruction(TR::FILE* pOutFile,
                                 const char* opCode,
                                 TR::RealRegister* base,
                                 int32_t offset,
                                 TR::RealRegister* reg)
...
void
TR_Debug::printMemImmInstruction(TR::FILE* pOutFile,
                                 const char* opCode,
                                 TR::RealRegister* base,
                                 int32_t offset,
                                 int32_t imm)
...
void
TR_Debug::printLabelInstruction(TR::FILE* pOutFile,
                                const char* opCode,
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86CallSnippet* snippet)
{
  ...
  printSnippetLabel(
    pOutFile, snippet->getSnippetLabel(), bufferPos, getName(snippet));
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    trfprintf(
      pOutFile,
      "jmp \t%s\t\t# jump out of snippet code",
...
void
TR_Debug::printX86OOLSequences(TR::FILE* pOutFile)
{
  ...
  for (int i = callNode->getFirstArgumentIndex();
    ...
    if (opCodeName) {
      ...
      switch (bufferPos[modrmOffset] & 0xc0) {
        ...
        default:
          TR_ASSERT(
            0, "Unexpected Mod/RM value 0x%02x", bufferPos[modrmOffset]);
          // fall through
        case 0x80:
      ...
      if (isFlushToStack)
        trfprintf(
          pOutFile, "%s\t[rsp +%d], %s", opCodeName, displacement, regName);
      else // Flush from stack to argument registers.
        trfprintf(
          pOutFile, "%s\t%s, [rsp +%d]", opCodeName, regName, displacement);
