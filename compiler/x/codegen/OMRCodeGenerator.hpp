...
struct TR_VFPState {
  ...
  TR_VFPState()
      : _register(TR::RealRegister::NoReg),
        _displacement(0) {}  // Assumes noReg is zero
...
class OMR_EXTENSIBLE CodeGenerator : public OMR::CodeGenerator {
 public:
  ...
  RegisterAssignmentDirection setAssignmentDirection(
      RegisterAssignmentDirection d) {
  ...
  int32_t branchDisplacementToHelperOrTrampoline(
  ...
  void clobberLiveDependentDiscardableRegisters(
      TR::ClobberingInstruction *instr, TR::Register *baseReg);
  ...
  TR_X86ScratchRegisterManager *generateScratchRegisterManager(
      int32_t capacity = 7);
  ...
  bool isObjectOfSizeWorthAligning(uint32_t size) {
    uint32_t lineSize =
        64;  // getX86ProcessorInfo().getL1DataCacheLineSize();
   ...
   */
  TR::X86ConstantDataSnippet *findOrCreateConstantDataSnippet(TR::Node *node,
                                                              void *data,
                                                              size_t size);
  /*
   * \brief find or create a constant data snippet.
   ...
   */
  template <typename T>
  inline TR::X86ConstantDataSnippet *findOrCreateConstantDataSnippet(
      TR::Node *node, T data) {
  ...
  TR_OutlinedInstructions *findOutlinedInstructionsFromLabel(
      TR::LabelSymbol *label);
  TR_OutlinedInstructions *findOutlinedInstructionsFromMergeLabel(
      TR::LabelSymbol *label);
