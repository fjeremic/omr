...
class TR_X86PaddingTable {
 public:
  TR_X86PaddingTable(uint8_t biggestEncoding,
                     uint8_t*** encodings,
                     flags8_t flags,
                     uint16_t sibMask,
                     uint16_t prefixMask)
...
class OMR_EXTENSIBLE CodeGenerator : public OMR::CodeGenerator {
 public:
  ...
  void clobberLiveDependentDiscardableRegisters(
      TR::ClobberingInstruction* instr,
      TR::Register* baseReg);
  ...
  //
  intptrj_t alignment(intptrj_t cursor,
                      intptrj_t boundary,
                      intptrj_t margin = 0) {
  ...
  bool patchableRangeNeedsAlignment(void* cursor,
                                    intptrj_t length,
                                    intptrj_t boundary,
                                    intptrj_t margin = 0);
   ...
   */
  TR::X86DataSnippet* createDataSnippet(TR::Node* node,
                                        void* data,
                                        size_t size);
  /*
   * \brief create a data snippet
 ...
 private:
  bool nodeIsFoldableMemOperand(TR::Node* node,
                                TR::Node* parent,
                                TR_RegisterPressureState* state);
