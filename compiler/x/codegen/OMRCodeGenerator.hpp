...
public:
  ...
  ClobberingInstruction(TR_Memory *m)
      : _instruction(0), _clobberedRegisters(getTypedAllocator<TR::Register *>(
                             TR::comp()->allocator())){};
  ...
  TR::Instruction *setInstruction(TR::Instruction *i) {
    return (_instruction = i);
  }
  TR::list<TR::Register *> &getClobberedRegisters() {
    return _clobberedRegisters;
  }
  void addClobberedRegister(TR::Register *reg) {
    _clobberedRegisters.push_front(reg);
  }
};
...
public:
  ...
  bool supportsVirtualModeExtension() {
    return testFeatureFlags(TR_VirtualModeExtension);
  }
  bool supportsDebuggingExtension() {
    return testFeatureFlags(TR_DebuggingExtension);
  }
  bool supportsPageSizeExtension() {
    return testFeatureFlags(TR_PageSizeExtension);
  }
  bool supportsRDTSCInstruction() {
    return testFeatureFlags(TR_RDTSCInstruction);
  }
  bool hasModelSpecificRegisters() {
    return testFeatureFlags(TR_ModelSpecificRegisters);
  }
  bool supportsPhysicalAddressExtension() {
    return testFeatureFlags(TR_PhysicalAddressExtension);
  }
  bool supportsMachineCheckException() {
    return testFeatureFlags(TR_MachineCheckException);
  }
  bool supportsCMPXCHG8BInstruction() {
    return testFeatureFlags(TR_CMPXCHG8BInstruction);
  }
  bool supportsCMPXCHG16BInstruction() {
    return testFeatureFlags2(TR_CMPXCHG16BInstruction);
  }
  bool hasAPICHardware() { return testFeatureFlags(TR_APICHardware); }
  bool hasMemoryTypeRangeRegisters() {
    return testFeatureFlags(TR_MemoryTypeRangeRegisters);
  }
  bool supportsPageGlobalFlag() { return testFeatureFlags(TR_PageGlobalFlag); }
  bool hasMachineCheckArchitecture() {
    return testFeatureFlags(TR_MachineCheckArchitecture);
  }
  bool supportsCMOVInstructions() {
    return testFeatureFlags(TR_CMOVInstructions);
  }
  bool supportsFCOMIInstructions() {
    return testFeatureFlags(TR_BuiltInFPU | TR_CMOVInstructions);
  }
  bool hasPageAttributeTable() {
    return testFeatureFlags(TR_PageAttributeTable);
  }
  bool has36BitPageSizeExtension() {
    return testFeatureFlags(TR_36BitPageSizeExtension);
  }
  bool hasProcessorSerialNumber() {
    return testFeatureFlags(TR_ProcessorSerialNumber);
  }
  bool supportsCLFLUSHInstruction() {
    return testFeatureFlags(TR_CLFLUSHInstruction);
  }
  bool supportsDebugTraceStore() {
    return testFeatureFlags(TR_DebugTraceStore);
  }
  bool hasACPIRegisters() { return testFeatureFlags(TR_ACPIRegisters); }
  bool supportsMMXInstructions() {
    return testFeatureFlags(TR_MMXInstructions);
  }
  bool supportsFastFPSavesRestores() {
    return testFeatureFlags(TR_FastFPSavesRestores);
  }
  ...
  bool supportsSFence() {
    return testFeatureFlags(TR_SSE | TR_MMXInstructions);
  }
  bool prefersMultiByteNOP() {
    return getX86Architecture() && isGenuineIntel() && !isIntelPentium();
  }

  uint32_t getCPUStepping(uint32_t signature) {
    return (signature & CPUID_SIGNATURE_STEPPING_MASK);
  }
  uint32_t getCPUModel(uint32_t signature) {
    return (signature & CPUID_SIGNATURE_MODEL_MASK) >> 4;
  }
  uint32_t getCPUFamily(uint32_t signature) {
    return (signature & CPUID_SIGNATURE_FAMILY_MASK) >> 8;
  }
  uint32_t getCPUProcessor(uint32_t signature) {
    return (signature & CPUID_SIGNATURE_PROCESSOR_MASK) >> 12;
  }
  uint32_t getCPUExtendedModel(uint32_t signature) {
    return (signature & CPUID_SIGNATURE_EXTENDEDMODEL_MASK) >> 16;
  }
  ...
  bool isIntelPentium() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelPentium;
  }
  bool isIntelP6() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelP6;
  }
  bool isIntelPentium4() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelPentium4;
  }
  bool isIntelCore2() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelCore2;
  }
  bool isIntelTulsa() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelTulsa;
  }
  bool isIntelNehalem() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelNehalem;
  }
  bool isIntelWestmere() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelWestmere;
  }
  bool isIntelSandyBridge() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelSandyBridge;
  }
  bool isIntelIvyBridge() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelIvyBridge;
  }
  bool isIntelHaswell() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelHaswell;
  }
  bool isIntelBroadwell() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelBroadwell;
  }
  bool isIntelSkylake() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorIntelSkylake;
  }

  bool isIntelOldMachine() {
    return (isIntelPentium() || isIntelP6() || isIntelPentium4() ||
            isIntelCore2() || isIntelTulsa() || isIntelNehalem());
  }

  bool isAMDK6() {
    return (_processorDescription & 0x000000fe) == TR_ProcessorAMDK5;
  } // accept either K5 or K6
  bool isAMDAthlonDuron() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorAMDAthlonDuron;
  }
  bool isAMDOpteron() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorAMDOpteron;
  }
  bool isAMD15h() {
    return (_processorDescription & 0x000000ff) == TR_ProcessorAMDFamily15h;
  }
...
private:
  ...
  /**
   * @brief testFlag Ensures that the feature being tested for exists in the
   * mask and then checks whether the feature is set in the flag. The reason for
   * this is to facilitate correctness checks for relocatable compilations. In
   * order for the compiler to use a processor feature, the feature flag should
   * be added to the mask so that the processor validation code also accounts
   * for the use of said feature.
   *
...
public:
  TR_X86PaddingTable(uint8_t biggestEncoding, uint8_t ***encodings,
                     flags8_t flags, uint16_t sibMask, uint16_t prefixMask)
      : _biggestEncoding(biggestEncoding), _encodings(encodings), _flags(flags),
        _sibMask(sibMask), _prefixMask(prefixMask) {}
...
struct TR_VFPState {
  ...
  TR_VFPState()
      : _register(TR::RealRegister::NoReg), _displacement(0) {
...
public:
  ...
  void doBackwardsRegisterAssignment(TR_RegisterKinds kindsToAssign,
                                     TR::Instruction *startInstruction,
  ...
  void prepareForNonLinearRegisterAssignmentAtMerge(
  ...
  const TR::X86LinkageProperties &getProperties() {
    return *_linkageProperties;
  }

  RegisterAssignmentDirection getAssignmentDirection() {
    return _assignmentDirection;
  }
  RegisterAssignmentDirection
  setAssignmentDirection(RegisterAssignmentDirection d) {
  ...
  int32_t
  branchDisplacementToHelperOrTrampoline(uint8_t *nextInstructionAddress,
                                         TR::SymbolReference *helper);

  /**
   * \brief Reserve space in the code cache for a specified number of
   * trampolines.
   *
  ...
  /**
   * \brief Provides the number of trampolines in the current CodeCache that
   * have been reserved for unpopulated interface PIC (IPIC) slots.
   *
   * \return The number of reserved IPIC trampolines.
   */
  int32_t getNumReservedIPICTrampolines() const {
    return _numReservedIPICTrampolines;
  }

  /**
   * \brief Updates the number of reserved IPIC trampolines in the current
   * CodeCache.
   *
   * \param[in] n : number of reserved IPIC trampolines
   */
  void setNumReservedIPICTrampolines(int32_t n) {
    _numReservedIPICTrampolines = n;
  }
  ...
  TR::list<TR::Register *> &getSpilledIntRegisters() {
    return _spilledIntRegisters;
  }

  TR::list<TR::Register *> &getLiveDiscardableRegisters() {
    return _liveDiscardableRegisters;
  }
  void addLiveDiscardableRegister(TR::Register *reg);
  void removeLiveDiscardableRegister(TR::Register *reg);
  void clobberLiveDiscardableRegisters(TR::Instruction *instr,
  ...
  TR::list<TR::Register *> &getDependentDiscardableRegisters() {
    return _dependentDiscardableRegisters;
  }
  void addDependentDiscardableRegister(TR::Register *reg) {
    _dependentDiscardableRegisters.push_front(reg);
  }
  void
  clobberLiveDependentDiscardableRegisters(TR::ClobberingInstruction *instr,
                                           TR::Register *baseReg);
  ...
  TR::list<TR::ClobberingInstruction *> &getClobberingInstructions() {
    return _clobberingInstructions;
  }
  void addClobberingInstruction(TR::ClobberingInstruction *i) {
    _clobberingInstructions.push_front(i);
  }

  TR::list<TR_OutlinedInstructions *> &getOutlinedInstructionsList() {
    return _outlinedInstructionsList;
  }

  TR_X86ScratchRegisterManager *
  generateScratchRegisterManager(int32_t capacity = 7);
  ...
  TR::Instruction *setLastCatchAppendInstruction(TR::Instruction *i) {
    return (_lastCatchAppendInstruction = i);
  }
  TR::Instruction *getLastCatchAppendInstruction() {
    return _lastCatchAppendInstruction;
  }
  ...
  TR::Instruction *findBetterSpillPlacement(TR::Register *virtReg,
                                            int32_t realRegNum);

  int32_t getInstructionPatchAlignmentBoundary() {
    return _instructionPatchAlignmentBoundary;
  }
  void setInstructionPatchAlignmentBoundary(int32_t p) {
    _instructionPatchAlignmentBoundary = p;
  }
  ...
  int32_t getLowestCommonCodePatchingAlignmentBoundary() {
    return _lowestCommonCodePatchingAlignmentBoundary;
  }
  void setLowestCommonCodePatchingAlignmentBoundary(int32_t b) {
    _lowestCommonCodePatchingAlignmentBoundary = b;
  }
  ...
  void apply32BitLoadLabelRelativeRelocation(TR::Instruction *movInstruction,
  ...
  //
  virtual uint8_t *generatePadding(
      uint8_t *cursor, intptrj_t length, TR::Instruction *neighborhood = NULL,
  ...
  //
  intptrj_t alignment(intptrj_t cursor, intptrj_t boundary,
                      intptrj_t margin = 0) {
    TR_ASSERT((boundary & (boundary - 1)) == 0,
              "Can only align to powers of 2");
    return (-cursor - margin) & (boundary - 1);
  }
  intptrj_t alignment(void *cursor, intptrj_t boundary, intptrj_t margin = 0);

  bool patchableRangeNeedsAlignment(void *cursor, intptrj_t length,
                                    intptrj_t boundary, intptrj_t margin = 0);
   ...
   * \param[in] data : a pointer to initial data or NULL for skipping
   * initialization \param[in] size : the size of this data snippet
   *
   * \return : a data snippet with specified size
   */
  TR::X86DataSnippet *createDataSnippet(TR::Node *node, void *data,
                                        size_t size);
  /*
   * \brief create a data snippet
   *
   ...
   */
  template <typename T>
  inline TR::X86DataSnippet *createDataSnippet(TR::Node *node, T data) {
   ...
   * \param[in] data : a pointer to initial data or NULL for skipping
   * initialization \param[in] size : the size of this constant data snippet
   *
   * \return : a constant data snippet with specified size
   */
  TR::X86ConstantDataSnippet *
  findOrCreateConstantDataSnippet(TR::Node *node, void *data, size_t size);
  /*
   * \brief find or create a constant data snippet.
   *
   * \param[in] node : the node which this constant data snippet belongs to
   ...
   */
  template <typename T>
  inline TR::X86ConstantDataSnippet *
  findOrCreateConstantDataSnippet(TR::Node *node, T data) {
  ...
  findOutlinedInstructionsFromLabel(TR::LabelSymbol *label);
  TR_OutlinedInstructions *
  findOutlinedInstructionsFromMergeLabel(TR::LabelSymbol *label);

  const TR_VFPState &vfpState() { return _vfpState; }
  TR::X86VFPSaveInstruction *vfpResetInstruction() {
    return _vfpResetInstruction;
  }
  ...
  virtual uint8_t nodeResultGPRCount(TR::Node *node,
                                     TR_RegisterPressureState *state);

  virtual TR_BitVector *getGlobalRegisters(TR_SpillKinds kind,
                                           TR_LinkageConventions lc) {
    return &_globalRegisterBitVectors[kind];
  }

  virtual void simulateNodeEvaluation(TR::Node *node,
                                      TR_RegisterPressureState *state,
                                      TR_RegisterPressureSummary *summary);

  /**
   * @brief Answers whether a trampoline is required for a direct call
   * instruction to reach a target address.
   *
...
protected:
  ...
  static uint8_t k8PaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
                                  [PADDING_TABLE_MAX_ENCODING_LENGTH];
  static uint8_t
      intelMultiBytePaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
                                   [PADDING_TABLE_MAX_ENCODING_LENGTH];
  static uint8_t old32BitPaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH]
                                        [PADDING_TABLE_MAX_ENCODING_LENGTH];
...
private:
  bool nodeIsFoldableMemOperand(TR::Node *node, TR::Node *parent,
                                TR_RegisterPressureState *state);
  ...
  std::list<TR::ClobberingInstruction *,
            TR::typed_allocator<TR::ClobberingInstruction *,
                                TR::Allocator>>::iterator _clobIterator;
  ...
  enum TR_X86CodeGeneratorFlags {
    ...
    EnableSinglePrecisionMethods =
        0x00000008, ///< support changing FPCW to single precision for
                    ///< individual methods
    EnableRegisterInterferences =
        0x00000010, ///< consider register interferences during register
                    ///< assignment
    ...
    EnableImplicitDivideCheck =
        0x00000100, ///< platform can catch hardware exceptions for divide
    ...
    MapAutosTo8ByteSlots =
        0x00000400, ///< don't round up sizes of autos to an 8-byte slot size
                    ///< when the stack is mapped
    EnableTLHPrefetching =
        0x00000800, ///< enable software prefetches on TLH allocates
    // Available                             = 0x00001000,
    // Available                             = 0x00002000,
    TargetSupportsSoftwarePrefetches =
        0x00004000, ///< target processor and OS both support software prefetch
                    ///< instructions
    MethodEnterExitTracingEnabled = 0x00008000, ///< trace method enter/exits
    // Available                             = 0x00010000,
    PushPreservedRegisters =
        0x00020000 ///< we've chosen to save/restore preserved regs using
                   ///< push/pop instructions instead of movs
  };
...
public:
  ...
  void setEnableBetterSpillPlacements() {
    _flags.set(EnableBetterSpillPlacements);
  }
  void resetEnableBetterSpillPlacements() {
    _flags.reset(EnableBetterSpillPlacements);
  }
  ...
  void setEnableSinglePrecisionMethods() {
    _flags.set(EnableSinglePrecisionMethods);
  }
  ...
  void setEnableRegisterInterferences() {
    _flags.set(EnableRegisterInterferences);
  }

  bool enableRegisterAssociations() {
    return _flags.testAny(EnableRegisterAssociations);
  }
  void setEnableRegisterAssociations() {
    _flags.set(EnableRegisterAssociations);
  }
  ...
  void setTargetSupportsSoftwarePrefetches() {
    _flags.set(TargetSupportsSoftwarePrefetches);
  }
  ...
  bool generateMasmListingSyntax() {
    return _flags.testAny(GenerateMasmListingSyntax);
  }
  ...
  bool enableImplicitDivideCheck() {
    return _flags.testAny(EnableImplicitDivideCheck);
  }
  ...
  TR::X86ImmInstruction *getReturnTypeInfoInstruction() {
  ...
  TR::LabelSymbol *getSwitchToInterpreterLabel() {
    return _switchToInterpreterLabel;
  }
  void setSwitchToInterpreterLabel(TR::LabelSymbol *s) {
    _switchToInterpreterLabel = s;
  }

  bool methodEnterExitTracingEnabled() {
    return _flags.testAny(MethodEnterExitTracingEnabled);
  }
  void setMethodEnterExitTracingEnabled() {
    _flags.set(MethodEnterExitTracingEnabled);
  }
...
#define NEEDS_TRAMPOLINE(target, rip, cg)                                      \
  (cg->directCallRequiresTrampoline((intptrj_t)target, (intptrj_t)rip))
#else
// Give the C++ compiler a hand
#define NEEDS_TRAMPOLINE(target, rip, cg) (0)
#endif

#define IS_32BIT_RIP(x, rip)                                                   \
  ((intptrj_t)(x) ==                                                           \
   (intptrj_t)(rip) + (int32_t)((intptrj_t)(x) - (intptrj_t)(rip)))
