...
void OMR::X86::Linkage::mapCompactedStack(TR::ResolvedMethodSymbol *method) {
  ...
  if (debug("reportCL")) {
...
#ifdef DEBUG
    diagnostic("\n**** Mapped locals size: %d (orig map size=%d, shared "
               "size=%d)  %s\n",
    ...
    diagnostic("\n**** Accumulated totals: mapped size=%d, shared size=%d, "
               "original size=%d after %s\n",
...
TR::Instruction *
OMR::X86::Linkage::loadArguments(TR::Instruction *prev,
                                 TR::ResolvedMethodSymbol *methodSymbol) {
  return self()->flushArguments(prev, methodSymbol, true, true);
}

TR::Instruction *
OMR::X86::Linkage::storeArguments(TR::Instruction *prev,
                                  TR::ResolvedMethodSymbol *methodSymbol) {
