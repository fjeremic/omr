...
void
OMR::X86::Linkage::mapCompactedStack(TR::ResolvedMethodSymbol* method)
{
      ...
      if (localCursor->getGCMapIndex() < 0) {
        ...
        if (igNode) {
          ...
          if (colourToOffsetMap[colour] == -1) {
            self()->mapSingleAutomatic(
              localCursor, colourToSizeMap[colour], stackIndex);
            colourToOffsetMap[colour] = localCursor->getOffset();
#ifdef DEBUG
            isFirst = true;
#endif
...
void
OMR::X86::Linkage::mapStack(TR::ResolvedMethodSymbol* method)
{
...
void
OMR::X86::Linkage::mapIncomingParms(TR::ResolvedMethodSymbol* method)
{
...
void
OMR::X86::Linkage::mapSingleAutomatic(TR::AutomaticSymbol* p,
                                      uint32_t& stackIndex)
{
                                      ...
                                      uint32_t& stackIndex)
{
  ...
  if (self()->cg()->getTraceRAOption(TR_TraceRASpillTemps))
    traceMsg(self()->cg()->comp(),
             ...
             self()->cg()->getDebug()->getName(p),
             size,
             stackIndex);
}

void
OMR::X86::Linkage::stopUsingKilledRegisters(
...
void
OMR::X86::Linkage::associatePreservedRegisters(
  TR::RegisterDependencyConditions* deps,
  TR::Register* returnRegister)
{
  TR::Register* vmThreadReg = self()->cg()->getVMThreadRegister();
  ...
  for (TR_LiveRegisterInfo* liveReg = liveRegisters->getFirstLiveRegister();
    ...
    for (int realReg = TR::RealRegister::LastAssignableGPR;
         realReg >= TR::RealRegister::FirstGPR;
         realReg--) {
...
TR::Register*
OMR::X86::Linkage::findReturnRegisterFromDependencies(
  TR::Node* callNode,
...
//
void
OMR::X86::Linkage::coerceFPReturnValueToXMMR(
  TR::Node* callNode,
  TR::RegisterDependencyConditions* dependencies,
  TR::MethodSymbol* methodSymbol,
  TR::Register* returnReg)
{
  ...
  if (callNode->getOpCode().isFloat()) {
    ...
    generateFPMemRegInstruction(
      FSTPMemReg, callNode, tempMR, fpReg, self()->cg());
    generateRegMemInstruction(
      MOVSSRegMem,
      callNode,
      returnReg,
      generateX86MemoryReference(*tempMR, 0, self()->cg()),
      self()->cg());
  } else {
    ...
    generateFPMemRegInstruction(
      DSTPMemReg, callNode, tempMR, fpReg, self()->cg());
    generateRegMemInstruction(
...
uint8_t*
OMR::X86::Linkage::loadArguments(TR::Node* callNode,
                                 uint8_t* cursor,
                                 bool calculateSizeOnly,
                                 int32_t* sizeOfFlushArea,
                                 bool isReturnAddressOnStack)
{
  return self()->flushArguments(callNode,
                                cursor,
                                calculateSizeOnly,
                                sizeOfFlushArea,
...
uint8_t*
OMR::X86::Linkage::storeArguments(TR::Node* callNode,
                                  uint8_t* cursor,
                                  bool calculateSizeOnly,
                                  int32_t* sizeOfFlushArea)
{
  return self()->flushArguments(
    callNode, cursor, calculateSizeOnly, sizeOfFlushArea, true, false);
}
