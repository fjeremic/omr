      ...
      if (localCursor->getGCMapIndex() >= 0) {
        ...
        if (igNode) {
          ...
          if (localCursor->isInternalPointer() || localCursor->isPinningArrayPointer()
            ...
            performSharing
              = performTransformation(self()->comp(), "O^O COMPACT LOCALS: Sharing slot for local %p\n", localCursor);
      ...
      if (localCursor->getGCMapIndex() >= 0) {
        ...
        if (self()->cg()->getTraceRAOption(TR_TraceRASpillTemps))
          traceMsg(self()->comp(), "\nmapCompactedStack: changing %s (GC index %d) offset from %d to %d",
            self()->comp()->getDebug()->getName(localCursor), localCursor->getGCMapIndex(), localCursor->getOffset(),
            newOffset);
  ...
  TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, TR::MethodSymbol* methodSymbol, bool isDirect)
{
  ...
  case TR::Int64:
    returnRegister = self()->cg()->allocateRegisterPair(
      postConditions->getRegisterDependency(0)->getRegister(), postConditions->getRegisterDependency(1)->getRegister());
    break;
  case TR::Float:
  case TR::Double: {
...
{
  TR_ASSERT(returnReg->getKind() == TR_FPR, "cannot coerce FP return values into non-XMM registers\n");
  ...
  if (callNode->getOpCode().isFloat()) {
    ...
    generateRegMemInstruction(
      MOVSSRegMem, callNode, returnReg, generateX86MemoryReference(*tempMR, 0, self()->cg()), self()->cg());
  } else {
    TR_ASSERT(callNode->getOpCode().isDouble(), "cannot return non-floating-point values in XMM registers\n");
    ...
    generateRegMemInstruction(self()->cg()->getXMMDoubleLoadOpCode(), callNode, returnReg,
