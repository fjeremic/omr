...
void OMR::X86::Linkage::mapCompactedStack(TR::ResolvedMethodSymbol* method)
{
...

    {
        ...
        int32_t* colourToOffsetMap = (int32_t*)self()->trMemory()->allocateStackMemory(self()->cg()->getLocalsIG()->getNumberOfColoursUsedToColour() * sizeof(int32_t));

        uint32_t* colourToSizeMap = (uint32_t*)self()->trMemory()->allocateStackMemory(self()->cg()->getLocalsIG()->getNumberOfColoursUsedToColour() * sizeof(uint32_t));
        ...
        for (i = 0; i < self()->cg()->getLocalsIG()->getNumNodes(); i++) {
            ...
            if ((colour != UNCOLOURED) && ((size = ((TR::Symbol*)(igNode->getEntity()))->castToAutoSymbol()->getRoundedSize()) > colourToSizeMap[colour])) {
