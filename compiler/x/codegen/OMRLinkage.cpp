...
OMR::X86::Linkage::Linkage(TR::CodeGenerator *cg)
    : OMR::Linkage(cg), _minimumFirstInstructionSize(0) {
  // Initialize the movOp table based on preferred load instructions for this
  // target.
  //
  TR_X86OpCodes op =
      cg->getXMMDoubleLoadOpCode() ? cg->getXMMDoubleLoadOpCode() : MOVSDRegMem;
...
void OMR::X86::Linkage::mapCompactedStack(TR::ResolvedMethodSymbol *method) {
...

  {
    ...
    int32_t *colourToOffsetMap =
        (int32_t *)self()->trMemory()->allocateStackMemory(
            self()->cg()->getLocalsIG()->getNumberOfColoursUsedToColour() *
            sizeof(int32_t));

    uint32_t *colourToSizeMap =
        (uint32_t *)self()->trMemory()->allocateStackMemory(
            self()->cg()->getLocalsIG()->getNumberOfColoursUsedToColour() *
            sizeof(uint32_t));

    for (i = 0;
         i < self()->cg()->getLocalsIG()->getNumberOfColoursUsedToColour();
         i++) {
    ...
    for (i = 0; i < self()->cg()->getLocalsIG()->getNumNodes(); i++) {
      ...
      TR_ASSERT(colour != UNCOLOURED,
                "uncoloured local %p (igNode=%p) found in locals IG\n",
                igNode->getEntity(), igNode);

      if ((colour != UNCOLOURED) &&
          ((size = ((TR::Symbol *)(igNode->getEntity()))
                       ->castToAutoSymbol()
                       ->getRoundedSize()) > colourToSizeMap[colour])) {
        colourToSizeMap[colour] = size;
      }
    }

    // Map all garbage collected references together so we can concisely
    // represent stack maps. They must be mapped so that the GC map index in
    // each local symbol is honoured.
    //
    ...
    for (localCursor = automaticIterator.getFirst(); localCursor;
      ...
      if (localCursor->getGCMapIndex() >= 0) {
        TR_IGNode *igNode =
            self()->cg()->getLocalsIG()->getIGNodeForEntity(localCursor);
        ...
        if (igNode) {
          ...
          if (localCursor->isInternalPointer() ||
            ...
            performSharing = performTransformation(
                self()->comp(),
                "O^O COMPACT LOCALS: Sharing slot for local %p\n", localCursor);
    ...
    for (localCursor = automaticIterator.getFirst(); localCursor;
      ...
      if (localCursor->getGCMapIndex() >= 0) {
        int32_t newOffset =
            stackIndex +
            pointerSize * (localCursor->getGCMapIndex() - firstLocalGCIndex);

        if (self()->cg()->getTraceRAOption(TR_TraceRASpillTemps))
          traceMsg(self()->comp(),
                   "\nmapCompactedStack: changing %s (GC index %d) offset from "
                   "%d to %d",
                   self()->comp()->getDebug()->getName(localCursor),
                   localCursor->getGCMapIndex(), localCursor->getOffset(),
                   newOffset);

        localCursor->setOffset(newOffset);

        TR_ASSERT((localCursor->getOffset() <= 0),
                  "Local %p (GC index %d) offset cannot be positive "
                  "(stackIndex = %d)\n",
                  localCursor, localCursor->getGCMapIndex(), stackIndex);

        if (localCursor->getGCMapIndex() ==
            atlas->getIndexOfFirstInternalPointer()) {
      ...
      if (localCursor->getGCMapIndex() < 0) {
        ...
        if (igNode) {
          ...
          if (colourToOffsetMap[colour] == -1) {
            self()->mapSingleAutomatic(localCursor, colourToSizeMap[colour],
                                       stackIndex);
            colourToOffsetMap[colour] = localCursor->getOffset();
#ifdef DEBUG
            isFirst = true;
    ...
    if (self()->comp()->getMethodSymbol()->getLinkageConvention() != TR_System)
      ...
      TR::toX86SystemLinkage(self()->cg()->getLinkage())
          ->mapIncomingParms(method, stackIndex);
  ...
  if (debug("reportCL")) {
...
#ifdef DEBUG
    diagnostic(
        "\n**** Mapped locals size: %d (orig map size=%d, shared size=%d)  "
        "%s\n",
    ...
    diagnostic(
        "\n**** Accumulated totals: mapped size=%d, shared size=%d, original "
        "size=%d after %s\n",
...
void OMR::X86::Linkage::mapStack(TR::ResolvedMethodSymbol *method) {
  if (self()->cg()->getLocalsIG() &&
      self()->cg()->getSupportsCompactedLocals()) {
  ...
  stackIndex -=
      (atlas->getNumberOfSlotsMapped() - atlas->getNumberOfParmSlotsMapped())
      << pointerShift;
  ...
  for (localCursor = automaticIterator.getFirst(); localCursor;
       localCursor = automaticIterator.getNext())
    if (localCursor->getGCMapIndex() >= 0) {
      localCursor->setOffset(
          stackIndex +
          pointerSize * (localCursor->getGCMapIndex() - firstLocalGCIndex));

      TR_ASSERT((localCursor->getOffset() <= 0),
                "Local %p (GC index %d) offset cannot be positive (stackIndex "
                "= %d)\n",
                localCursor, localCursor->getGCMapIndex(), stackIndex);
  ...
  //
  if (!self()->cg()->mapAutosTo8ByteSlots()) {
    if (stackIndex % TR::Compiler->om.sizeofReferenceAddress()) stackIndex -= 4;
...
void OMR::X86::Linkage::mapIncomingParms(TR::ResolvedMethodSymbol *method) {
  ...
  if (self()->getProperties().passArgsRightToLeft()) {
    ...
    // TODO:AMD64: This code deosn't need to support 8-byte slots; put it back
    // the way it was
    uint8_t parmSlotShift = self()->getProperties().getParmSlotShift();
    int32_t topOfParameterArea =
        (method->getNumParameterSlots() << parmSlotShift) + offsetToFirstParm;
    for (; parmCursor; parmCursor = parameterIterator.getNext()) {
      // Incoming parm offsets are zero-based, positive, and based on 4-byte
      // slots
...
void OMR::X86::Linkage::stopUsingKilledRegisters(
  ...
  for (int32_t i = deps->getNumPostConditions() - 1; i >= 0; --i) {
    if (deps->getPostConditions()
            ->getRegisterDependency(i)
            ->getRealRegister() != TR::RealRegister::NoReg) {
      TR::Register *reg =
          deps->getPostConditions()->getRegisterDependency(i)->getRegister();
      if (reg && reg != firstReturnRegister && reg != secondReturnRegister &&
          reg != vmThreadReg)
...
void OMR::X86::Linkage::associatePreservedRegisters(
  ...
  for (TR_LiveRegisterInfo *liveReg = liveRegisters->getFirstLiveRegister();
       liveReg; liveReg = liveReg->getNext()) {
    TR::Register *virtReg = liveReg->getRegister();

    // Skip registers that already have good associations that we don't want to
    // disturb
    //
    if (virtReg == returnRegister) continue;
    if (virtReg == vmThreadReg) continue;

    // Search all registers for the best one to associate with virtReg
    //
    TR::RealRegister::RegNum bestReal = TR::RealRegister::NoReg;
    for (int realReg = TR::RealRegister::LastAssignableGPR;
         realReg >= TR::RealRegister::FirstGPR; realReg--) {
      // Can't use locked regs
      //
      if (self()
              ->machine()
              ->getRealRegister((TR::RealRegister::RegNum)realReg)
              ->getState() == TR::RealRegister::Locked)
        continue;

      // Volatile regs are no good for virtuals that live across a call
      //
      if (!self()->getProperties().isPreservedRegister(
              (TR::RealRegister::RegNum)realReg))
        continue;

      if (self()->machine()->getVirtualAssociatedWithReal(
              (TR::RealRegister::RegNum)realReg) == virtReg) {
        // virtReg is already associated with a preserved realReg, so obviously
        // that's the best choice
        //
        bestReal = (TR::RealRegister::RegNum)realReg;
        break;
      } else if (self()->machine()->getVirtualAssociatedWithReal(
                     (TR::RealRegister::RegNum)realReg) == NULL) {
    ...
    if (bestReal == TR::RealRegister::NoReg) {
      ...
      //
      break;
    } else if (self()->machine()->getVirtualAssociatedWithReal(bestReal) ==
               virtReg) {
...
TR::Register *OMR::X86::Linkage::findReturnRegisterFromDependencies(
    ...
    TR::MethodSymbol *methodSymbol, bool isDirect) {
  TR::Register *returnRegister = NULL;
  TR_X86RegisterDependencyGroup *postConditions =
  ...
  switch (callNode->getDataType()) {
    ...
    case TR::Address:
      ...
      if (isDirect && dependencies->getNumPostConditions() > 1)
        postConditions->getRegisterDependency(1)
            ->getRegister()
            ->setPlaceholderReg();
    ...
    case TR::Double: {
      ...
      //
      int depNum = ((methodSymbol->isVMInternalNative() ||
                     methodSymbol->isJITInternalNative()) &&
                    methodSymbol->preservesAllRegisters())
                       ? 0
                       : 3;
      returnRegister =
          postConditions->getRegisterDependency(depNum)->getRegister();
    }
      // deliberate fall-through
    default:
      if (isDirect && !methodSymbol->preservesAllRegisters()) {
        postConditions->getRegisterDependency(0)
            ->getRegister()
            ->setPlaceholderReg();
        postConditions->getRegisterDependency(1)
            ->getRegister()
...
//
void OMR::X86::Linkage::coerceFPReturnValueToXMMR(
    TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
  ...
  //   int depNum = ((methodSymbol->isVMInternalNative() ||
  //   methodSymbol->isJITInternalNative()) &&
  //   methodSymbol->preservesAllRegisters()) ? 0 : 3; TR::Register *fpReg =
  //   dependencies->getPostConditions()->getRegisterDependency(depNum)->getRegister();

  TR::Register *fpReg =
      callNode->getOpCode().isFloat()
          ? self()->cg()->allocateSinglePrecisionRegister(TR_X87)
  ...
  if (callNode->getOpCode().isFloat()) {
    ...
    generateFPMemRegInstruction(FSTPMemReg, callNode, tempMR, fpReg,
                                self()->cg());
    generateRegMemInstruction(
        MOVSSRegMem, callNode, returnReg,
        generateX86MemoryReference(*tempMR, 0, self()->cg()), self()->cg());
  } else {
    TR_ASSERT(callNode->getOpCode().isDouble(),
              "cannot return non-floating-point values in XMM registers\n");
    TR::MemoryReference *tempMR =
        self()->machine()->getDummyLocalMR(TR::Double);
    generateFPMemRegInstruction(DSTPMemReg, callNode, tempMR, fpReg,
                                self()->cg());
    generateRegMemInstruction(
        self()->cg()->getXMMDoubleLoadOpCode(), callNode, returnReg,
        generateX86MemoryReference(*tempMR, 0, self()->cg()), self()->cg());
  }
...
uint32_t OMR::X86::Linkage::getRightToLeft() {
  return self()->getProperties().passArgsRightToLeft();
}

uint8_t *OMR::X86::Linkage::loadArguments(TR::Node *callNode, uint8_t *cursor,
                                          bool calculateSizeOnly,
                                          int32_t *sizeOfFlushArea,
                                          bool isReturnAddressOnStack) {
  return self()->flushArguments(callNode, cursor, calculateSizeOnly,
                                sizeOfFlushArea, isReturnAddressOnStack, true);
}

uint8_t *OMR::X86::Linkage::storeArguments(TR::Node *callNode, uint8_t *cursor,
                                           bool calculateSizeOnly,
                                           int32_t *sizeOfFlushArea) {
  return self()->flushArguments(callNode, cursor, calculateSizeOnly,
                                sizeOfFlushArea, true, false);
}

TR::Instruction *OMR::X86::Linkage::loadArguments(
    TR::Instruction *prev, TR::ResolvedMethodSymbol *methodSymbol) {
  return self()->flushArguments(prev, methodSymbol, true, true);
}

TR::Instruction *OMR::X86::Linkage::storeArguments(
    TR::Instruction *prev, TR::ResolvedMethodSymbol *methodSymbol) {
...
TR_X86OpCodes
    OMR::X86::Linkage::_movOpcodes[NumMovOperandTypes][NumMovDataTypes] = {
