...
static bool isUnevaluatedZero(TR::Node *child) {
  if (child->getRegister() == NULL) {
    ...
    case TR::fconst:
      return (child->getFloatBits() == 0 ||
              child->getFloatBits() == 0x80000000); // hex for ieee float -0.0
    case TR::dconst:
      return (child->getDoubleBits() == 0 ||
              child->getDoubleBits() == IEEE_DOUBLE_NEGATIVE_0_0);
    case TR::i2f:
    case TR::s2f:
    case TR::b2f:
    case TR::l2f:
...
TR::Register *TR_X86FPCompareAnalyser::fpCompareAnalyser(
    TR::Node *root, TR_X86OpCodes cmpRegRegOpCode,
    TR_X86OpCodes cmpRegMemOpCode, TR_X86OpCodes cmpiRegRegOpCode,
  ...
  TR_X86OpCodes cmpInstr =
      useFCOMIInstructions ? cmpiRegRegOpCode : cmpRegRegOpCode;
  ...
  //
  bool mustSwapOperands = (cmpOp == TR::iffcmple || cmpOp == TR::ifdcmple ||
                           cmpOp == TR::iffcmpgtu || cmpOp == TR::ifdcmpgtu ||
                           cmpOp == TR::fcmple || cmpOp == TR::dcmple ||
                           cmpOp == TR::fcmpgtu || cmpOp == TR::dcmpgtu ||
                           (useFCOMIInstructions &&
                            (cmpOp == TR::iffcmplt || cmpOp == TR::ifdcmplt ||
                             cmpOp == TR::iffcmpgeu || cmpOp == TR::ifdcmpgeu ||
                             cmpOp == TR::fcmplt || cmpOp == TR::dcmplt ||
                             cmpOp == TR::fcmpgeu || cmpOp == TR::dcmpgeu)))
                              ? true
                              : false;

  // Some operators should not have their operands swapped to improve the
  // generated code needed to evaluate the result of the comparison.
  //
  bool preventOperandSwapping =
      (cmpOp == TR::iffcmpltu || cmpOp == TR::ifdcmpltu ||
       cmpOp == TR::iffcmpge || cmpOp == TR::ifdcmpge || cmpOp == TR::fcmpltu ||
       cmpOp == TR::dcmpltu || cmpOp == TR::fcmpge || cmpOp == TR::dcmpge ||
       (useFCOMIInstructions &&
        (cmpOp == TR::iffcmpgt || cmpOp == TR::ifdcmpgt ||
         cmpOp == TR::iffcmpleu || cmpOp == TR::ifdcmpleu ||
         cmpOp == TR::fcmpgt || cmpOp == TR::dcmpgt || cmpOp == TR::fcmpleu ||
         cmpOp == TR::dcmpleu)))
          ? true
          : false;
  ...
  //
  if (cmpOp == TR::fcmpg || cmpOp == TR::fcmpl || cmpOp == TR::dcmpg ||
      cmpOp == TR::dcmpl) {
  ...
  //
  if (preventOperandSwapping ||
      (!mustSwapOperands && _cg->whichChildToEvaluate(root) == 0)) {
  ...
  setInputs(firstChild, firstRegister, secondChild, secondRegister,
            ...
            // If either 'preventOperandSwapping' or 'mustSwapOperands' is set
            // then the initial operand ordering set above must be maintained.
            //
  ...
  if (getEvalChild1() &&
      isUnevaluatedZero(firstChild)) // do we need getEvalChild1() here?
  {
  ...
  } else if (getEvalChild2() &&
             isUnevaluatedZero(secondChild)) // do we need getEvalChild2() here?
  {
    if ((getCmpReg1Reg2() || reverseCmpOp) ||
        (getCmpReg2Reg1() && !reverseCmpOp &&
         !(preventOperandSwapping || mustSwapOperands))) {
  ...
  if (firstRegister &&
      (firstRegister->needsPrecisionAdjustment() ||
       ...
       (firstRegister->mayNeedPrecisionAdjustment() &&
  ...
  if (secondRegister &&
      (secondRegister->needsPrecisionAdjustment() ||
       comp->getOption(TR_StrictFPCompares) ||
       (secondRegister->mayNeedPrecisionAdjustment() &&
  ...
  if (targetRegisterForFTST) {
    ...
    generateFPRegMemInstruction(cmpRegMemOpCode, root, firstRegister, tempMR,
                                _cg);
    ...
    generateFPRegMemInstruction(cmpRegMemOpCode, root, secondRegister, tempMR,
                                _cg);
    notReversedOperands();
    tempMR->decNodeReferenceCounts(_cg);
  } else if (getCmpReg1Reg2() || reverseCmpOp) {
    generateFPCompareRegRegInstruction(cmpInstr, root, firstRegister,
                                       secondRegister, _cg);
  } else if (getCmpReg2Reg1()) {
    generateFPCompareRegRegInstruction(cmpInstr, root, secondRegister,
                                       firstRegister, _cg);
    notReversedOperands();
  }
  ...
  // We must manually move the FP condition flags to the EFLAGS register if we
  // don't use the FCOMI instructions.
  //
  TR::Register *accRegister = _cg->allocateRegister();
  TR::RegisterDependencyConditions *dependencies =
      generateRegisterDependencyConditions((uint8_t)1, 1, _cg);
  ...
  //
  if (targetRegisterForFTST && targetChildForFTST &&
      targetChildForFTST->getReferenceCount() == 0) {
    generateFPSTiST0RegRegInstruction(FSTRegReg, root, targetRegisterForFTST,
                                      targetRegisterForFTST, _cg);
  }
...
TR::Register *
TR_IA32XMMCompareAnalyser::xmmCompareAnalyser(TR::Node *root,
  ...
  bool mustSwapOperands =
      (cmpOp == TR::iffcmple || cmpOp == TR::ifdcmple ||
       cmpOp == TR::iffcmpgtu || cmpOp == TR::ifdcmpgtu ||
       cmpOp == TR::fcmple || cmpOp == TR::dcmple || cmpOp == TR::fcmpgtu ||
       cmpOp == TR::dcmpgtu || cmpOp == TR::iffcmplt || cmpOp == TR::ifdcmplt ||
       cmpOp == TR::iffcmpgeu || cmpOp == TR::ifdcmpgeu ||
       cmpOp == TR::fcmplt || cmpOp == TR::dcmplt || cmpOp == TR::fcmpgeu ||
       cmpOp == TR::dcmpgeu)
          ? true
          : false;
  ...
  if (getCmpReg1Mem2() || reverseMemOp) {
    TR::MemoryReference *tempMR = generateX86MemoryReference(secondChild, _cg);
    generateRegMemInstruction(cmpRegMemOpCode, root, firstRegister, tempMR,
                              _cg);
    tempMR->decNodeReferenceCounts(_cg);
  } else if (getCmpReg2Mem1()) {
    TR::MemoryReference *tempMR = generateX86MemoryReference(firstChild, _cg);
    generateRegMemInstruction(cmpRegMemOpCode, root, secondRegister, tempMR,
                              _cg);
    notReversedOperands();
    tempMR->decNodeReferenceCounts(_cg);
  } else if (getCmpReg1Reg2() || reverseCmpOp) {
    generateRegRegInstruction(cmpRegRegOpCode, root, firstRegister,
                              secondRegister, _cg);
  } else if (getCmpReg2Reg1()) {
    generateRegRegInstruction(cmpRegRegOpCode, root, secondRegister,
                              firstRegister, _cg);
    notReversedOperands();
  }
