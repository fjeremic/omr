...
TR::Register* TR_X86FPCompareAnalyser::fpCompareAnalyser(TR::Node* root, TR_X86OpCodes cmpRegRegOpCode,
  TR_X86OpCodes cmpRegMemOpCode, TR_X86OpCodes cmpiRegRegOpCode, bool useFCOMIInstructions)
{
  ...
  setInputs(firstChild, firstRegister, secondChild, secondRegister, useFCOMIInstructions,
  ...
  //
  if (firstRegister
    && (firstRegister->needsPrecisionAdjustment() || comp->getOption(TR_StrictFPCompares)
  ...
  if (secondRegister
    && (secondRegister->needsPrecisionAdjustment() || comp->getOption(TR_StrictFPCompares)
...
TR::Register* TR_IA32XMMCompareAnalyser::xmmCompareAnalyser(
  TR::Node* root, TR_X86OpCodes cmpRegRegOpCode, TR_X86OpCodes cmpRegMemOpCode)
{
  TR::Node *firstChild, *secondChild;
  TR::ILOpCodes cmpOp = root->getOpCodeValue();
  bool reverseMemOp = false;
