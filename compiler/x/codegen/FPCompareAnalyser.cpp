...
static bool
isUnevaluatedZero(TR::Node* child)
{
...
TR::Register*
TR_X86FPCompareAnalyser::fpCompareAnalyser(TR::Node* root,
                                           ...
                                           bool useFCOMIInstructions)
{
  ...
  bool mustSwapOperands =
    (cmpOp == TR::iffcmple || cmpOp == TR::ifdcmple || cmpOp == TR::iffcmpgtu ||
     cmpOp == TR::ifdcmpgtu || cmpOp == TR::fcmple || cmpOp == TR::dcmple ||
  ...
  if (targetRegisterForFTST) {
    ...
    generateFPRegMemInstruction(
      cmpRegMemOpCode, root, firstRegister, tempMR, _cg);
    ...
    generateFPRegMemInstruction(
      cmpRegMemOpCode, root, secondRegister, tempMR, _cg);
    notReversedOperands();
    tempMR->decNodeReferenceCounts(_cg);
  } else if (getCmpReg1Reg2() || reverseCmpOp) {
    generateFPCompareRegRegInstruction(
      cmpInstr, root, firstRegister, secondRegister, _cg);
  } else if (getCmpReg2Reg1()) {
    generateFPCompareRegRegInstruction(
      cmpInstr, root, secondRegister, firstRegister, _cg);
    notReversedOperands();
  }
  ...
  if (targetRegisterForFTST && targetChildForFTST &&
    ...
    generateFPSTiST0RegRegInstruction(
      FSTRegReg, root, targetRegisterForFTST, targetRegisterForFTST, _cg);
  }
...
void
TR_X86FPCompareAnalyser::setInputs(TR::Node* firstChild,
                                              ...
                                              TR_X86OpCodes cmpRegMemOpCode)
{
  ...
  bool mustSwapOperands =
    (cmpOp == TR::iffcmple || cmpOp == TR::ifdcmple || cmpOp == TR::iffcmpgtu ||
     cmpOp == TR::ifdcmpgtu || cmpOp == TR::fcmple || cmpOp == TR::dcmple ||
     cmpOp == TR::fcmpgtu || cmpOp == TR::dcmpgtu || cmpOp == TR::iffcmplt ||
     cmpOp == TR::ifdcmplt || cmpOp == TR::iffcmpgeu ||
     cmpOp == TR::ifdcmpgeu || cmpOp == TR::fcmplt || cmpOp == TR::dcmplt ||
  ...
  bool preventOperandSwapping =
    (cmpOp == TR::iffcmpltu || cmpOp == TR::ifdcmpltu ||
     cmpOp == TR::iffcmpge || cmpOp == TR::ifdcmpge || cmpOp == TR::fcmpltu ||
     cmpOp == TR::dcmpltu || cmpOp == TR::fcmpge || cmpOp == TR::dcmpge ||
     cmpOp == TR::iffcmpgt || cmpOp == TR::ifdcmpgt || cmpOp == TR::iffcmpleu ||
     cmpOp == TR::ifdcmpleu || cmpOp == TR::fcmpgt || cmpOp == TR::dcmpgt ||
     cmpOp == TR::fcmpleu || cmpOp == TR::dcmpleu)
      ? true
      : false;
  ...
  if (getCmpReg1Mem2() || reverseMemOp) {
    ...
    generateRegRegInstruction(
      cmpRegRegOpCode, root, firstRegister, secondRegister, _cg);
  } else if (getCmpReg2Reg1()) {
    generateRegRegInstruction(
      cmpRegRegOpCode, root, secondRegister, firstRegister, _cg);
    notReversedOperands();
  }
