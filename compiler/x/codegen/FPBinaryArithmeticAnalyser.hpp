...
public:
  ...
  TR_X86OpCodes getRegRegOp() {
    return getOpsReversed() ? _opCodePackage[_package][kOpReg2Reg1]
                            : _opCodePackage[_package][kOpReg1Reg2];
  }

  TR_X86OpCodes getRegMemOp() {
    return getOpsReversed() ? _opCodePackage[_package][kOpReg2Mem1]
                            : _opCodePackage[_package][kOpReg1Mem2];
  }

  TR_X86OpCodes getRegConvSOp() {
    return getOpsReversed() ? _opCodePackage[_package][kOpReg2ConvS1]
                            : _opCodePackage[_package][kOpReg1ConvS2];
  }

  TR_X86OpCodes getRegConvIOp() {
    return getOpsReversed() ? _opCodePackage[_package][kOpReg2ConvI1]
  ...
  bool isEvalTarget() {
    return (_actionMap[_inputs] & kEvalTarget) ? true : false;
  }
  bool isEvalSource() {
    return (_actionMap[_inputs] & kEvalSource) ? true : false;
  }
  bool isCopyReg() {
    return (_actionMap[_inputs] & kCopyTarget) ? true : false;
  }
  ...
  void setInputs(TR::Node *firstChild, TR::Register *firstRegister,
...
private:
  ...
  static const TR_X86OpCodes _opCodePackage[kNumFPPackages]
                                           [kNumFPArithVariants];
