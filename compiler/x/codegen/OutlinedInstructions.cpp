...
TR_OutlinedInstructions::TR_OutlinedInstructions(TR::LabelSymbol *entryLabel,
                                                 TR::CodeGenerator *cg)
    : _entryLabel(entryLabel), _cg(cg), _firstInstruction(NULL),
      _appendInstruction(NULL), _restartLabel(NULL),
      _block(cg->getCurrentEvaluationBlock()), _callNode(NULL),
      _targetReg(NULL), _hasBeenRegisterAssigned(false),
      _postDependencyMergeList(NULL), _outlinedPathRegisterUsageList(NULL),
      _mainlinePathRegisterUsageList(NULL),
      _registerAssignerStateAtMerge(NULL) {
  _entryLabel->setStartOfColdInstructionStream();
}

TR_OutlinedInstructions::TR_OutlinedInstructions(TR::Node *callNode,
                                                 TR::ILOpCodes callOp,
                                                 TR::Register *targetReg,
                                                 TR::LabelSymbol *entryLabel,
                                                 TR::LabelSymbol *restartLabel,
                                                 TR::CodeGenerator *cg)
    : _restartLabel(restartLabel), _block(cg->getCurrentEvaluationBlock()),
      _firstInstruction(NULL), _appendInstruction(NULL), _targetReg(targetReg),
      _entryLabel(entryLabel), _hasBeenRegisterAssigned(false),
      _postDependencyMergeList(NULL), _outlinedPathRegisterUsageList(NULL),
      _mainlinePathRegisterUsageList(NULL), _registerAssignerStateAtMerge(NULL),
      _cg(cg) {
  _entryLabel->setStartOfColdInstructionStream();
...
void TR_OutlinedInstructions::generateOutlinedInstructionsDispatch() {
  ...
  new (_cg->trHeapMemory())
      TR::X86LabelInstruction(NULL, LABEL, _entryLabel, _cg);
  ...
  if (_targetReg) {
    ...
    TR_ASSERT(
        resultReg->getKind() == _targetReg->getKind(),
    ...
    TR_ASSERT((targetRegPair == NULL && resultRegPair == NULL) ||
              ...
              "OutlinedInstructions: targetReg and resultReg must be both "
              "register pairs or neither register pairs.");
    if (targetRegPair) {
      generateRegRegInstruction(MOVRegReg(), _callNode,
                                targetRegPair->getLowOrder(),
                                resultRegPair->getLowOrder(), _cg);
      generateRegRegInstruction(MOVRegReg(), _callNode,
                                targetRegPair->getHighOrder(),
      ...
      default:
        TR_ASSERT(false,
                  "OutlinedInstructions: unsupported result register kind.");
        break;
      }
  ...
  // Dummy label to delimit the end of the helper call dispatch sequence (for
  // exception ranges).
  //
  generateLabelInstruction(
      LABEL, _callNode, TR::LabelSymbol::create(_cg->trHeapMemory(), _cg), _cg);
...
// Create a NoReg dependency for each child of a call that has been evaluated
// into a register. Ignore children that do not have a register since their live
// range should not persist outside of the helper call stream.
//
TR::RegisterDependencyConditions *
TR_OutlinedInstructions::formEvaluatedArgumentDepList() {
  int32_t i, c = 0;

  for (i = _callNode->getFirstArgumentIndex(); i < _callNode->getNumChildren();
       i++) {
  ...
  if (c) {
    ...
    for (i = _callNode->getFirstArgumentIndex();
         i < _callNode->getNumChildren(); i++) {
      TR::Register *reg = _callNode->getChild(i)->getRegister();
      if (reg) {
        TR::RegisterPair *regPair = reg->getRegisterPair();
        if (regPair) {
          depConds->addPostCondition(regPair->getLowOrder(),
                                     TR::RealRegister::NoReg, _cg);
          depConds->addPostCondition(regPair->getHighOrder(),
                                     TR::RealRegister::NoReg, _cg);
        } else {
...
void TR_OutlinedInstructions::assignRegisters(
  ...
  _cg->setInternalControlFlowSafeNestingDepth(
      _cg->internalControlFlowNestingDepth());

  // Create a dependency list on the first instruction in this stream that
  // captures all current real register associations.  This is necessary to get
  // the register assigner back into its original state before the helper stream
  // was processed.
  //
  TR::RegisterDependencyConditions *liveRealRegDeps =
      _cg->machine()->createDepCondForLiveGPRs();
  _firstInstruction->setDependencyConditions(liveRealRegDeps);

  // TODO:AMD64: Fix excessive register assignment exchanges in outlined
  // instruction dispatch.

  // Ensure correct VFP state at the start of the outlined instruction sequence.
  //
  generateVFPRestoreInstruction(cg()->getAppendInstruction(),
                                vfpSaveInstruction, _cg);
...
TR::Node *
TR_OutlinedInstructions::createOutlinedCallNode(TR::Node *callNode,
                                                TR::ILOpCodes callOp) {
  int32_t i;
  TR::Node *child;
  ...
  for (i = 0; i < callNode->getNumChildren(); i++) {
    ...
    if (child->getRegister() != NULL) {
      ...
      if ((child->getOpCodeValue() == TR::loadaddr) &&
        ...
        // Be very conservative at this point, even though it is possible to
        // make it less so.  For example, this will catch the case of an
        // unevaluated argument not persisting outside of the outlined region
        // even though one of its subtrees will.
        //
        (void)_cg->evaluate(child);

        // Do not decrement the reference count here.  It will be decremented
        // when the call node is evaluated again in the helper instruction
        // stream.
        //
        newCallNode->setAndIncChild(i, child);
      }
...
TR_OutlinedInstructionsGenerator::~TR_OutlinedInstructionsGenerator() {
  if (!std::uncaught_exception()) {
    generateLabelInstruction(LABEL, _oi->_callNode,
                             generateLabelSymbol(_oi->_cg), _oi->_cg);
    _oi->swapInstructionListsWithCompilation();
  }
}
