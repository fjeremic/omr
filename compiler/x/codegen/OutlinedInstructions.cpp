...
void
TR_OutlinedInstructions::swapInstructionListsWithCompilation()
{
  TR::Instruction* temp;
...
void
TR_OutlinedInstructions::generateOutlinedInstructionsDispatch()
{
  ...
  if (_targetReg) {
    ...
    if (targetRegPair) {
      generateRegRegInstruction(MOVRegReg(),
                                ...
                                resultRegPair->getLowOrder(),
                                _cg);
      generateRegRegInstruction(MOVRegReg(),
                                _callNode,
                                targetRegPair->getHighOrder(),
                                resultRegPair->getHighOrder(),
                                _cg);
    } else {
...
TR_OutlinedInstructions::formEvaluatedArgumentDepList()
{
  ...
  if (c) {
    ...
    for (i = _callNode->getFirstArgumentIndex();
         i < _callNode->getNumChildren();
         i++) {
      TR::Register* reg = _callNode->getChild(i)->getRegister();
      if (reg) {
        TR::RegisterPair* regPair = reg->getRegisterPair();
        if (regPair) {
          depConds->addPostCondition(
            regPair->getLowOrder(), TR::RealRegister::NoReg, _cg);
          depConds->addPostCondition(
            regPair->getHighOrder(), TR::RealRegister::NoReg, _cg);
        } else {
...
void
TR_OutlinedInstructions::assignRegisters(
  ...
  TR::X86VFPSaveInstruction* vfpSaveInstruction)
{
  ...
  //
  generateVFPRestoreInstruction(
    cg()->getAppendInstruction(), vfpSaveInstruction, _cg);
  ...
  _cg->doBackwardsRegisterAssignment(
    kindsToBeAssigned, _appendInstruction, appendInstruction);
...
void
TR_OutlinedInstructions::assignRegistersOnOutlinedPath(
  TR_RegisterKinds kindsToBeAssigned,
  TR::X86VFPSaveInstruction* vfpSaveInstruction)
{
  if (hasBeenRegisterAssigned()) {
...
OMR::RegisterUsage*
TR_OutlinedInstructions::findInRegisterUsageList(
...
TR_OutlinedInstructionsGenerator::~TR_OutlinedInstructionsGenerator()
{
  if (!std::uncaught_exception()) {
    generateLabelInstruction(
      LABEL, _oi->_callNode, generateLabelSymbol(_oi->_cg), _oi->_cg);
    _oi->swapInstructionListsWithCompilation();
  }
}
