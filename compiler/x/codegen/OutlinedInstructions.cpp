  ...
  if (_targetReg) {
    ...
    if (targetRegPair) {
      generateRegRegInstruction(
        MOVRegReg(), _callNode, targetRegPair->getLowOrder(), resultRegPair->getLowOrder(), _cg);
      generateRegRegInstruction(
        MOVRegReg(), _callNode, targetRegPair->getHighOrder(), resultRegPair->getHighOrder(), _cg);
    } else {
  ...
  for (i = 0; i < callNode->getNumChildren(); i++) {
    ...
    if (child->getRegister() != NULL) {
      ...
      if ((child->getOpCodeValue() == TR::loadaddr) && (child->getSymbolReference()->getSymbol())
        ...
        // Be very conservative at this point, even though it is possible to make it less so.  For example, this will
        // catch the case of an unevaluated argument not persisting outside of the outlined region even though one of
        // its subtrees will.
        //
