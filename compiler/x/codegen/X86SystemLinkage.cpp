...
TR::X86SystemLinkage::copyParametersToHomeLocation(TR::Instruction *cursor) {
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    TR_MovDataTypes movDataType =
        paramMovType(paramCursor); // What sort of MOV instruction does it need?
    ...
    } else // It's in a linkage register
    {
      ...
      if (debug("traceCopyParametersToHomeLocation") && ai == sourceIndex) {
        diagnostic("copyParametersToHomeLocation: Parameter #%d already in "
                   "register %d\n",
                   lri, ai);
      }
    }
...
void TR::X86SystemLinkage::createPrologue(TR::Instruction *cursor) {
  ...
  public:
    ...
    TR_DebugFrameSegmentInfo(TR::Compilation *c, int32_t lowOffset,
        ...
        : _comp(c), _next(next), _description(description), _register(reg),
          _lowOffset(lowOffset), _size(size) {}
...
int32_t
TR::X86SystemLinkage::layoutTypeOnStack(TR::DataType type, int32_t &dataCursor,
                                        TR::parmLayoutResult &layoutResult) {
