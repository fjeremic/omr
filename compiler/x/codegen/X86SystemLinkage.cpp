...
const TR::X86LinkageProperties&
TR::X86SystemLinkage::getProperties()
{
  return _properties;
}

TR::Register*
TR::X86SystemLinkage::buildIndirectDispatch(TR::Node* callNode)
{
...
int32_t
TR::X86SystemLinkage::computeMemoryArgSize(TR::Node* callNode,
                                           int32_t first,
                                           int32_t last,
                                           int8_t direction)
{
  ...
  for (i = first; i != last; i += direction) {
    ...
    layoutParm(
      child, sizeOfOutGoingArgs, numIntArgs, numFloatArgs, layoutResult);
  }
...
TR::X86SystemLinkage::createPrologue(TR::Instruction* cursor)
{
  ...
  if (_properties.getOutgoingArgAlignment() && !cg()->isLeafMethod()) {
    ...
    TR_ASSERT(
      ...
      "AMD64 SysV linkage require outgoingArgAlignment be 16/32 bytes aligned, "
      "while IA32 linkage require 4 bytes aligned.  We currently haven't "
      "support 32 bytes alignment for AMD64 SysV ABI yet.\n");
  ...
  if (properties.getAlwaysDedicateFramePointerRegister()) {
    debugFrameSlotInfo = new (trHeapMemory())
      TR_DebugFrameSegmentInfo(comp(),
                               properties.getOffsetToFirstLocal(),
                               properties.getGPRWidth(),
                               "EBP-save",
                               debugFrameSlotInfo);
  }
  ...
  for (autoCursor = autoIterator.getFirst(); autoCursor != NULL;
    ...
    debugFrameSlotInfo =
      new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(),
                                                    autoCursor->getOffset(),
                                                    autoCursor->getSize(),
                                                    "Local",
                                                    debugFrameSlotInfo);
  }
  ...
  debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
    ...
    adjust,
    "adjust size for stack alignment",
    debugFrameSlotInfo);
  debugFrameSlotInfo = new (trHeapMemory())
    TR_DebugFrameSegmentInfo(comp(),
                             properties.getOffsetToFirstLocal() - localSize -
                               preservedRegsSize - outgoingArgSize - adjust,
                             outgoingArgSize,
                             "Outgoing args",
                             debugFrameSlotInfo);
  if (comp()->getOption(TR_TraceCG)) {
...
TR::X86SystemLinkage::restorePreservedRegisters(TR::Instruction* cursor)
{
  ...
  if (cg()->pushPreservedRegisters()) {
    for (int32_t pindex = 0;
      ...
      if (reg->getHasBeenAssignedInMethod()) {
        cursor =
          new (trHeapMemory()) TR::X86RegInstruction(cursor, POPReg, reg, cg());
      }
    }
  } else {
    int32_t offsetCursor =
    ...
    for (int32_t pindex = _properties.getMaxRegistersPreservedInPrologue() - 1;
      ...
      if (comp()->getOption(TR_TraceCG)) {
        traceMsg(comp(),
                 "reg %d, getHasBeenAssignedInMethod %d\n",
                 idx,
...
TR::X86SystemLinkage::createEpilogue(TR::Instruction* cursor)
{
  ...
  if (_properties.getAlwaysDedicateFramePointerRegister()) {
    ...
    TR::RealRegister* realReg = getSingleWordFrameAllocationRegister();
    cursor =
      new (trHeapMemory()) TR::X86RegInstruction(cursor, POPReg, realReg, cg());
  } else {
...
TR::X86SystemLinkage::copyLinkageInfoToParameterSymbols()
{
  ...
  for (TR::ParameterSymbol* paramCursor = paramIterator.getFirst();
    ...
    TR::parmLayoutResult layoutResult;
    layoutParm(
      paramCursor, sizeOfOutGoingArgs, numIntArgs, numFloatArgs, layoutResult);
...
TR::X86SystemLinkage::mapIncomingParms(TR::ResolvedMethodSymbol* method)
{
  ...
  for (TR::ParameterSymbol* parmCursor = parameterIterator.getFirst();
    ...
    TR::parmLayoutResult layoutResult;
    layoutParm(
      parmCursor, sizeOfOutGoingArgs, numIntArgs, numFloatArgs, layoutResult);
...
void
TR::X86SystemLinkage::copyGlRegDepsToParameterSymbols(TR::Node* bbStart,
                                                      TR::CodeGenerator* cg)
{
...
int32_t
TR::X86SystemLinkage::getParameterStartingPos(int32_t& dataCursor,
                                              uint32_t align)
{
...
int32_t
TR::X86SystemLinkage::layoutTypeOnStack(TR::DataType type,
                                        int32_t& dataCursor,
                                        TR::parmLayoutResult& layoutResult)
{
...
intptrj_t
TR::X86SystemLinkage::entryPointFromCompiledMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}

intptrj_t
TR::X86SystemLinkage::entryPointFromInterpretedMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}
