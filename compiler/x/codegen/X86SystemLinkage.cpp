...
int32_t TR::X86SystemLinkage::computeMemoryArgSize(TR::Node *callNode,
  ...
  for (i = first; i != last; i += direction) {
    ...
    layoutParm(child, sizeOfOutGoingArgs, numIntArgs, numFloatArgs,
               layoutResult);
  }
  return sizeOfOutGoingArgs;
}

static const TR::RealRegister::RegNum NOT_ASSIGNED =
    (TR::RealRegister::RegNum)-1;
...
//
TR::Instruction *TR::X86SystemLinkage::copyParametersToHomeLocation(
    TR::Instruction *cursor) {
  TR::Machine *machine = cg()->machine();
  TR::RealRegister *framePointer =
      machine->getRealRegister(TR::RealRegister::vfp);
  ...
  TR_ASSERT(noReg == 0,
            "noReg must be zero so zero-initializing movStatus will work");

  TR::MovStatus movStatus[TR::RealRegister::NumRegisters] = {
      {(TR::RealRegister::RegNum)0, (TR::RealRegister::RegNum)0,
       (TR_MovDataTypes)0}};
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    int8_t lri =
        paramCursor
            ->getLinkageRegisterIndex();  // How the parameter enters the method
    TR::RealRegister::RegNum ai  // Where method body expects to find it
        = (TR::RealRegister::RegNum)
              paramCursor->getAssignedGlobalRegisterIndex();
    int32_t offset =
        paramCursor
            ->getParameterOffset();  // Location of the parameter's stack slot
    TR_MovDataTypes movDataType = paramMovType(
        paramCursor);  // What sort of MOV instruction does it need?
    ...
    if (lri == NOT_LINKAGE)  // It's on the stack
    {
      ...
      {
        if (loadCursor == NULL) loadCursor = cursor;
        ...
        loadCursor = generateRegMemInstruction(
            loadCursor, TR::Linkage::movOpcodes(RegMem, movDataType),
            machine->getRealRegister(ai),
            generateX86MemoryReference(framePointer, offset, cg()), cg());
      }
    } else  // It's in a linkage register
    {
      TR::RealRegister::RegNum sourceIndex =
          getProperties().getArgumentRegister(lri, isFloat(movDataType));
      ...
      if (ai == NOT_ASSIGNED || hasToBeOnStack(paramCursor)) {
        if (comp()->getOption(TR_TraceCG))
          traceMsg(comp(),
                   "copyToHomeLocation param %p, linkage reg index %d, "
                   "allocated index %d, parameter offset %d, hasToBeOnStack "
                   "%d, parm->isParmHasToBeOnStack() %d.\n",
                   paramCursor, lri, ai, offset, hasToBeOnStack(paramCursor),
                   paramCursor->isParmHasToBeOnStack());
        if (debug("traceCopyParametersToHomeLocation"))
          diagnostic("copyParametersToHomeLocation: Storing %d\n", sourceIndex);
        // stack := lri
        cursor = generateMemRegInstruction(
            cursor, TR::Linkage::movOpcodes(MemReg, movDataType),
            generateX86MemoryReference(framePointer, offset, cg()),
            machine->getRealRegister(sourceIndex), cg());
      }
      ...
      if (ai != NOT_ASSIGNED && ai != sourceIndex) {
        ...
        //
        TR_ASSERT(movStatus[ai].sourceReg == noReg,
                  "Each target reg must have only one source");
        TR_ASSERT(movStatus[sourceIndex].targetReg == noReg,
                  "Each source reg must have only one target");
        if (debug("traceCopyParametersToHomeLocation"))
          diagnostic(
              "copyParametersToHomeLocation: Planning to move %d to %d\n",
              sourceIndex, ai);
      ...
      if (debug("traceCopyParametersToHomeLocation") && ai == sourceIndex) {
        diagnostic(
            "copyParametersToHomeLocation: Parameter #%d already in register "
            "%d\n",
            lri, ai);
      }
    }
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
       paramCursor = paramIterator.getNext()) {
    if (paramCursor->getLinkageRegisterIndex() == NOT_LINKAGE) continue;

    const TR::RealRegister::RegNum paramReg =
        getProperties().getArgumentRegister(
            paramCursor->getLinkageRegisterIndex(),
            isFloat(paramMovType(paramCursor)));
    ...
    if (movStatus[paramReg].targetReg == 0) {
      ...
      TR_ASSERT(
          noReg == 0,
          "noReg must be zero (not %d) for zero-filled initialization to work",
          noReg);
      ...
      while (movStatus[regCursor].targetReg != noReg) {
        ...
        TR_ASSERT(regCursor != paramReg,
                  "Can't yet handle cyclic dependencies");
      ...
      // Work our way backward along the chain, generating all the necessary
      // movs
      //
      while (movStatus[regCursor].sourceReg != noReg) {
        ...
        cursor = generateRegRegInstruction(
            ...
            machine->getRealRegister(source), cg());
...
TR::Instruction *TR::X86SystemLinkage::savePreservedRegisters(
    TR::Instruction *cursor) {
  ...
  const int32_t localSize = getProperties().getOffsetToFirstLocal() -
                            bodySymbol->getLocalMappingCursor();
  const int32_t pointerSize = getProperties().getPointerSize();

  int32_t offsetCursor =
      -localSize + getProperties().getOffsetToFirstLocal() - pointerSize;

  if (_properties.getUsesPushesForPreservedRegs()) {
    for (int32_t pindex = _properties.getMaxRegistersPreservedInPrologue() - 1;
         pindex >= 0; pindex--) {
      TR::RealRegister::RegNum idx =
          _properties.getPreservedRegister((uint32_t)pindex);
      TR::RealRegister *reg = machine()->getRealRegister(idx);
      if (reg->getHasBeenAssignedInMethod() &&
          reg->getState() != TR::RealRegister::Locked) {
        cursor = new (trHeapMemory())
            TR::X86RegInstruction(cursor, PUSHReg, reg, cg());
      }
    }
  } else {
    for (int32_t pindex =
             getProperties().getMaxRegistersPreservedInPrologue() - 1;
         pindex >= 0; pindex--) {
      TR::RealRegister::RegNum idx =
          _properties.getPreservedRegister((uint32_t)pindex);
      TR::RealRegister *reg = machine()->getRealRegister(
          getProperties().getPreservedRegister((uint32_t)pindex));
      if (reg->getHasBeenAssignedInMethod() &&
          reg->getState() != TR::RealRegister::Locked) {
        cursor = generateMemRegInstruction(
            cursor, TR::Linkage::movOpcodes(MemReg, fullRegisterMovType(reg)),
            generateX86MemoryReference(
                machine()->getRealRegister(TR::RealRegister::vfp), offsetCursor,
                cg()),
            reg, cg());
...
void TR::X86SystemLinkage::createPrologue(TR::Instruction *cursor) {
  ...
  class TR_DebugFrameSegmentInfo {
   ...
   public:
    ...
    TR_DebugFrameSegmentInfo(TR::Compilation *c, int32_t lowOffset,
                             uint8_t size, const char *description,
    ...
    TR_DebugFrameSegmentInfo *sort() {
      ...
      for (after = tail; after; before = after, after = after->_next) {
        if (after->_lowOffset > _lowOffset) break;
      }
      _next = after;
      if (before) {
        before->_next = this;
    ...
    void print(TR_Debug *debug) {
      if (_next) _next->print(debug);
      if (_size > 0) {
        diagnostic("        % 4d: % 4d -> % 4d (% 4d) %5.5s %s\n", _lowOffset,
                   _lowOffset, _lowOffset + _size - 1, _size,
  ...
  uint32_t registerSaveDescription =
      0;  // bit N corresponds to _preservedRegisters[N], with 1=preserved

  int32_t pindex;  // Preserved register index
  for (pindex = 0; pindex < properties.getMaxRegistersPreservedInPrologue();
       pindex++) {
    TR::RealRegister *reg = machine()->getRealRegister(
        properties.getPreservedRegister((uint32_t)pindex));
    if (reg->getHasBeenAssignedInMethod() &&
        reg->getState() != TR::RealRegister::Locked) {
  ...
  // Compute frame size
  //
  // allocSize: bytes to be subtracted from the stack pointer when allocating
  // the frame frameSize: total bytes of stack the prologue consumes
  //
  // const int32_t localSize = -properties.getOffsetToFirstLocal() -
  // bodySymbol->getLocalMappingCursor();
  const int32_t localSize =
      properties.getOffsetToFirstLocal() - bodySymbol->getLocalMappingCursor();
  ...
  int32_t frameSize =
      localSize + preservedRegsSize +
      (properties.getReservesOutgoingArgsInPrologue() ? outgoingArgSize : 0) +
      (properties.getAlwaysDedicateFramePointerRegister()
           ? properties.getGPRWidth()
           : 0);

  uint32_t adjust = 0;
  if (_properties.getOutgoingArgAlignment() && !cg()->isLeafMethod()) {
    // AMD64 SysV spec requires: The end of the input argument area shall be
    // aligned on a 16 (32, if __m256 is passed on stack) byte boundary. In
    // other words, the value (%rsp + 8) is always a multiple of 16 (32) when
    // control is transferred to the function entry point.
    TR_ASSERT(
        _properties.getOutgoingArgAlignment() == 16 ||
            _properties.getOutgoingArgAlignment() == 4,
        "AMD64 SysV linkage require outgoingArgAlignment be 16/32 bytes "
        "aligned, while IA32 linkage require 4 bytes aligned.  We currently "
        "haven't support 32 bytes alignment for AMD64 SysV ABI yet.\n");
    const uint32_t stackAlignMinus1 = _properties.getOutgoingArgAlignment() - 1;
    adjust = ((frameSize + properties.getRetAddressWidth() + stackAlignMinus1) &
              ~stackAlignMinus1) -
  ...
  if (properties.getAlwaysDedicateFramePointerRegister()) {
    ...
    TR::RealRegister *stackPointerReg =
    ...
    cg()->initializeVFPState(properties.getFramePointerRegister(),
                             _properties.getPointerSize());
  } else {
  ...
  if (allocSize == 0) {
    ...
    cursor = new (trHeapMemory())
        TR::X86RegInstruction(cursor, PUSHReg, realReg, cg());
  } else {
    const TR_X86OpCodes subOp =
        (allocSize <= 127) ? SUBRegImms() : SUBRegImm4();
    cursor = new (trHeapMemory())
        TR::X86RegImmInstruction(cursor, subOp, espReal, allocSize, cg());
  }

  // Save preserved regs, and tell the frontend how many there are
  //
  bodySymbol->setProloguePushSlots(preservedRegsSize /
  ...
  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(comp(),
             "create prologue using system linkage, after "
             "savePreservedRegisters, cursor is %p.\n",
             cursor);
  }
  ...
  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(comp(),
             "create prologue using system linkage, after "
             "copyParametersToHomeLocation, cursor is %p.\n",
             cursor);
  }
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
        comp(), paramCursor->getOffset(), paramCursor->getSize(), "Parameter",
        debugFrameSlotInfo,
  ...
  if (properties.getAlwaysDedicateFramePointerRegister()) {
    debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
        comp(), properties.getOffsetToFirstLocal(), properties.getGPRWidth(),
        "EBP-save", debugFrameSlotInfo);
  }

  ListIterator<TR::AutomaticSymbol> autoIterator(
      &bodySymbol->getAutomaticList());
  TR::AutomaticSymbol *autoCursor;
  for (autoCursor = autoIterator.getFirst(); autoCursor != NULL;
       autoCursor = autoIterator.getNext()) {
    debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
        comp(), autoCursor->getOffset(), autoCursor->getSize(), "Local",
        debugFrameSlotInfo);
  }

  debugFrameSlotInfo = new (trHeapMemory())
      TR_DebugFrameSegmentInfo(comp(), 0, properties.getRetAddressWidth(),
                               "Return address", debugFrameSlotInfo);
  debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
      comp(),
  ...
  debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
      comp(),
      properties.getOffsetToFirstLocal() - localSize - preservedRegsSize -
          outgoingArgSize - adjust,
      outgoingArgSize, "Outgoing args", debugFrameSlotInfo);
  if (comp()->getOption(TR_TraceCG)) {
    diagnostic("\nFrame layout:\n");
...
TR::Instruction *TR::X86SystemLinkage::restorePreservedRegisters(
    TR::Instruction *cursor) {
  TR::ResolvedMethodSymbol *bodySymbol = comp()->getJittedMethodSymbol();
  const int32_t localSize =
      _properties.getOffsetToFirstLocal() - bodySymbol->getLocalMappingCursor();
  const int32_t pointerSize = _properties.getPointerSize();

  if (cg()->pushPreservedRegisters()) {
    for (int32_t pindex = 0;
         pindex < _properties.getMaxRegistersPreservedInPrologue(); pindex++) {
      TR::RealRegister::RegNum idx =
          _properties.getPreservedRegister((uint32_t)pindex);
      TR::RealRegister *reg = machine()->getRealRegister(idx);
      if (reg->getHasBeenAssignedInMethod()) {
        cursor = new (trHeapMemory())
            TR::X86RegInstruction(cursor, POPReg, reg, cg());
      }
    }
  } else {
    int32_t offsetCursor =
        -localSize + _properties.getOffsetToFirstLocal() - pointerSize;
    for (int32_t pindex = _properties.getMaxRegistersPreservedInPrologue() - 1;
         pindex >= 0; pindex--) {
      TR::RealRegister::RegNum idx =
          _properties.getPreservedRegister((uint32_t)pindex);
      ...
      if (reg->getHasBeenAssignedInMethod()) {
        cursor = generateRegMemInstruction(
            cursor, TR::Linkage::movOpcodes(RegMem, fullRegisterMovType(reg)),
            reg,
            generateX86MemoryReference(
                machine()->getRealRegister(TR::RealRegister::vfp), offsetCursor,
                cg()),
...
void TR::X86SystemLinkage::createEpilogue(TR::Instruction *cursor) {
  ...
  const int32_t localSize =
      _properties.getOffsetToFirstLocal() - bodySymbol->getLocalMappingCursor();
  int32_t allocSize = _properties.getUsesPushesForPreservedRegs()
                          ? localSize
  ...
  if (cg()->pushPreservedRegisters()) {
    ...
    cursor = new (trHeapMemory())
        TR::X86RegImmInstruction(cursor, op, espReal, outgoingArgSize, cg());
  }
  ...
  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(comp(),
             "create epilogue using system linkage, after "
             "restorePreservedRegisters, cursor is %x.\n",
             cursor);
  }
  ...
  if (_properties.getAlwaysDedicateFramePointerRegister()) {
    ...
    //
    cursor = new (trHeapMemory()) TR::X86RegRegInstruction(
        cursor, MOVRegReg(), espReal,
        machine()->getRealRegister(_properties.getFramePointerRegister()),
        cg());
    cursor = new (trHeapMemory()) TR::X86RegInstruction(
        cursor, POPReg,
        machine()->getRealRegister(_properties.getFramePointerRegister()),
    ...
    TR::RealRegister *realReg = getSingleWordFrameAllocationRegister();
    cursor = new (trHeapMemory())
        TR::X86RegInstruction(cursor, POPReg, realReg, cg());
  } else {
    TR_X86OpCodes op = (allocSize <= 127) ? ADDRegImms() : ADDRegImm4();
    cursor = new (trHeapMemory())
        TR::X86RegImmInstruction(cursor, op, espReal, allocSize, cg());
  }

  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(comp(),
             "create epilogue using system linkage, after delocating stack "
             "frame, cursor is %x.\n",
             cursor);
  }
  ...
  if (cursor->getNext()->getOpCodeValue() == RETImm2) {
    toIA32ImmInstruction(cursor->getNext())
        ->setSourceImmediate(bodySymbol->getNumParameterSlots()
                             << getProperties().getParmSlotShift());

    if (comp()->getOption(TR_TraceCG)) {
      traceMsg(comp(),
               "create epilogue using system linkage, ret_IMM set to %d.\n",
               bodySymbol->getNumParameterSlots()
                   << getProperties().getParmSlotShift());
    }
  }
}
...
void TR::X86SystemLinkage::copyLinkageInfoToParameterSymbols() {
  ...
  for (TR::ParameterSymbol *paramCursor = paramIterator.getFirst();
    ...
    TR::parmLayoutResult layoutResult;
    layoutParm(paramCursor, sizeOfOutGoingArgs, numIntArgs, numFloatArgs,
               layoutResult);
    ...
    //
    if (numIntArgs >= maxIntArgs && numFloatArgs >= maxFloatArgs) break;
  }
}

void TR::X86SystemLinkage::mapIncomingParms(TR::ResolvedMethodSymbol *method) {
  TR_ASSERT(!getProperties().passArgsRightToLeft(),
            "Right-to-left not yet implemented on AMD64");
  int32_t sizeOfOutGoingArgs = 0;
  uint16_t numIntArgs = 0, numFloatArgs = 0;
  ...
  for (TR::ParameterSymbol *parmCursor = parameterIterator.getFirst();
       parmCursor; parmCursor = parameterIterator.getNext()) {
    TR::parmLayoutResult layoutResult;
    layoutParm(parmCursor, sizeOfOutGoingArgs, numIntArgs, numFloatArgs,
               layoutResult);
...
void TR::X86SystemLinkage::copyGlRegDepsToParameterSymbols(
    TR::Node *bbStart, TR::CodeGenerator *cg) {
  ...
  if (bbStart->getNumChildren() > 0) {
    ...
    TR_ASSERT(glRegDeps->getOpCodeValue() == TR::GlRegDeps,
              "First child of first Node must be a GlRegDeps");
...
int32_t TR::X86SystemLinkage::getParameterStartingPos(int32_t &dataCursor,
                                                      uint32_t align) {
...
int32_t TR::X86SystemLinkage::layoutTypeOnStack(
    TR::DataType type, int32_t &dataCursor,
    TR::parmLayoutResult &layoutResult) {
