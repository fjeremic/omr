...
uint8_t*
TR::X86HelperCallSnippet::emitSnippetBody()
{
...
void
TR::X86HelperCallSnippet::addMetaDataForLoadAddrArg(uint8_t* buffer,
                                                    TR::Node* child)
{
...
uint8_t*
TR::X86HelperCallSnippet::genHelperCall(uint8_t* buffer)
{
  ...
  *(int32_t*)buffer =
    branchDisplacementToHelper(callInstructionAddress, getDestination(), cg());
...
void
TR_Debug::printBody(TR::FILE* pOutFile,
                    ...
                    uint8_t* bufferPos)
{
  ...
  if (snippet->getStackPointerAdjustment() != 0) {
    ...
    trfprintf(pOutFile,
              ...
              snippet->getStackPointerAdjustment(),
              commentString());
    bufferPos += size;
  }
  ...
  if (snippet->getCallNode()) {
    ...
    for (; i < snippet->getCallNode()->getNumChildren(); i++) {
      ...
      if (child->getOpCodeValue() == TR::loadaddr && !child->getRegister()) {
        ...
        trfprintf(
          pOutFile, "push\t" POINTER_PRINTF_FORMAT, sym->getStaticAddress());
        bufferPos += 5;
      } else if (child->getOpCode().isLoadConst()) {
        int32_t argValue = child->getInt();
  ...
  printPrefix(pOutFile, NULL, bufferPos, 5);
  trfprintf(pOutFile,
            "call\t%s \t%s Helper Address = " POINTER_PRINTF_FORMAT,
            getName(snippet->getDestination()),
            commentString(),
  ...
  if (snippet->getStackPointerAdjustment() != 0) {
    ...
    printPrefix(pOutFile, NULL, bufferPos, size);
    trfprintf(pOutFile,
              "sub \t%s, %d\t\t\t%s Reallocate stack frame",
              TR::Compiler->target.is64Bit() ? "rsp" : "esp",
              snippet->getStackPointerAdjustment(),
              commentString());
    bufferPos += size;
...
uint32_t
TR::X86HelperCallSnippet::getLength(int32_t estimatedSnippetStart)
{
  uint32_t length = 35;
...
int32_t
TR::X86HelperCallSnippet::branchDisplacementToHelper(
