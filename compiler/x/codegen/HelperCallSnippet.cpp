...
void TR::X86HelperCallSnippet::addMetaDataForLoadAddrArg(
    uint8_t* buffer,
    TR::Node* child)
{
    ...
    if (_callNode) {
        ...
        for (; i < _callNode->getNumChildren(); ++i) {
            ...
            if (child->getOpCodeValue() == TR::loadaddr) {
                if (!child->getRegister() || child->getRegister() != deps->getPostConditions()->getRegisterDependency(registerArgs)->getRegister()) {
                    TR::StaticSymbol* sym = child->getSymbol()->getStaticSymbol();
...
uint32_t TR::X86HelperCallSnippet::getLength(int32_t estimatedSnippetStart)
{
    ...
    if (_callNode) {
        ...
        for (; i < _callNode->getNumChildren(); ++i) {
            TR::Node* child = _callNode->getChild(i);
            if (child->getOpCodeValue() == TR::loadaddr && (!child->getRegister() || child->getRegister() != deps->getPostConditions()->getRegisterDependency(registerArgs++)->getRegister())) {
                length += 5;
            } else if (child->getOpCode().isLoadConst()) {
                int32_t argValue = child->getInt();
                if (argValue >= -128 && argValue <= 127)
