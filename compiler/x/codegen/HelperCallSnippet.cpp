...
TR::X86HelperCallSnippet::X86HelperCallSnippet(TR::CodeGenerator *cg,
    ...
    : TR::X86RestartSnippet(cg, node, restartlab, snippetlab,
                            helper->canCauseGC()),
      _destination(helper), _callNode(0), _offset(-1),
  ...
  if (getDestination() ==
      symRefTab->findOrCreateReportMethodEnterSymbolRef(owningMethod)) {
...
TR::X86HelperCallSnippet::X86HelperCallSnippet(TR::CodeGenerator *cg,
                                               ...
                                               TR::Node *callNode,
                                               int32_t stackPointerAdjustment)
    : TR::X86RestartSnippet(cg, callNode, restartlab, snippetlab,
                            callNode->getSymbolReference()->canCauseGC()),
      _destination(callNode->getSymbolReference()), _callNode(callNode),
      _offset(-1), _callInstructionBufferAddress(NULL),
      _stackPointerAdjustment(stackPointerAdjustment),
      _alignCallDisplacementForPatching(false) {
  TR::Compilation *comp = cg->comp();
  TR::SymbolReferenceTable *symRefTab = comp->getSymRefTab();
...
void TR::X86HelperCallSnippet::addMetaDataForLoadAddrArg(uint8_t *buffer,
                                                         TR::Node *child) {
  ...
  if (cg()->comp()->getOption(TR_EnableHCR) &&
      (!child->getSymbol()->isClassObject() ||
       cg()->wantToPatchClassPointer(
           (TR_OpaqueClassBlock *)sym->getStaticAddress(), buffer))) {
    if (TR::Compiler->target.is64Bit())
      cg()->jitAddPicToPatchOnClassRedefinition(
          ((void *)(uintptrj_t)sym->getStaticAddress()), (void *)buffer);
    else
...
uint8_t *TR::X86HelperCallSnippet::genHelperCall(uint8_t *buffer) {
  ...
  if (_callNode) {
    if (!debug("amd64unimplemented"))
      TR_ASSERT(TR::Compiler->target.is32Bit(),
                "AMD64 genHelperCall with _callNode not yet implemented");
    int32_t i = 0;
    ...
    if (_offset != -1) {
      ...
      *buffer++ = 0xFF;
      if (cg()->getLinkage()
              ->getProperties()
              .getAlwaysDedicateFramePointerRegister()) {
    ...
    TR::RegisterDependencyConditions *deps =
        getRestartLabel()->getInstruction()->getDependencyConditions();
    ...
    for (; i < _callNode->getNumChildren(); ++i) {
      ...
      if (child->getOpCodeValue() == TR::loadaddr) {
        if (!child->getRegister() ||
            child->getRegister() != deps->getPostConditions()
                                        ->getRegisterDependency(registerArgs)
                                        ->getRegister()) {
          TR::StaticSymbol *sym = child->getSymbol()->getStaticSymbol();
      ...
      *buffer = 0x50;
      TR_ASSERT(deps, "null dependencies on restart label of helper call "
                      "snippet with register args");
      cg()->machine()
  ...
  if (_alignCallDisplacementForPatching && TR::Compiler->target.isSMP()) {
    uintptrj_t mod =
        (uintptrj_t)(buffer) % cg()->getInstructionPatchAlignmentBoundary();
  ...
  *(int32_t *)buffer = branchDisplacementToHelper(callInstructionAddress,
                                                  getDestination(), cg());

  cg()->addProjectSpecializedRelocation(buffer, (uint8_t *)getDestination(),
                                        NULL, TR_HelperAddress, __FILE__,
                                        __LINE__, _callNode);
...
void TR_Debug::print(TR::FILE *pOutFile, TR::X86HelperCallSnippet *snippet) {
  ...
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    getName(snippet), getName(snippet->getDestination()));
  printBody(pOutFile, snippet, bufferPos);
}

void TR_Debug::printBody(TR::FILE *pOutFile, TR::X86HelperCallSnippet *snippet,
                         uint8_t *bufferPos) {
  TR_ASSERT(pOutFile != NULL, "assertion failure");
  TR::MethodSymbol *sym =
      snippet->getDestination()->getSymbol()->castToMethodSymbol();
  ...
  if (snippet->getStackPointerAdjustment() != 0) {
    ...
    trfprintf(pOutFile,
              "add \t%s, %d\t\t\t%s Temporarily deallocate stack frame",
              TR::Compiler->target.is64Bit() ? "rsp" : "esp",
  ...
  if (snippet->getCallNode()) {
    if (snippet->getOffset() != -1) {
      ...
      bool useDedicatedFrameReg = _comp->cg()
                                      ->getLinkage()
                                      ->getProperties()
                                      .getAlwaysDedicateFramePointerRegister();
      ...
      trfprintf(pOutFile, "push\t[%s +%d]\t%s Address of Receiver",
                useDedicatedFrameReg ? "ebx" : "esp", snippet->getOffset(),
                commentString());
    ...
    for (; i < snippet->getCallNode()->getNumChildren(); i++) {
      ...
      if (child->getOpCodeValue() == TR::loadaddr && !child->getRegister()) {
        ...
        trfprintf(pOutFile, "push\t" POINTER_PRINTF_FORMAT,
                  sym->getStaticAddress());
        bufferPos += 5;
      } else if (child->getOpCode().isLoadConst()) {
        int32_t argValue = child->getInt();
  ...
  trfprintf(pOutFile, "call\t%s \t%s Helper Address = " POINTER_PRINTF_FORMAT,
            getName(snippet->getDestination()), commentString(),
  ...
  if (snippet->getStackPointerAdjustment() != 0) {
    uint8_t size = 5 + (TR::Compiler->target.is64Bit() ? 1 : 0);
    printPrefix(pOutFile, NULL, bufferPos, size);
    trfprintf(pOutFile, "sub \t%s, %d\t\t\t%s Reallocate stack frame",
              TR::Compiler->target.is64Bit() ? "rsp" : "esp",
              snippet->getStackPointerAdjustment(), commentString());
    bufferPos += size;
  }
...
uint32_t TR::X86HelperCallSnippet::getLength(int32_t estimatedSnippetStart) {
  ...
  if (_callNode) {
    ...
    if (_offset != -1) {
      bool useDedicatedFrameReg = cg()->getLinkage()
                                      ->getProperties()
                                      .getAlwaysDedicateFramePointerRegister();
      if (_offset >= -128 && _offset <= 127)
...
int32_t TR::X86HelperCallSnippet::branchDisplacementToHelper(
    uint8_t *callInstructionAddress, TR::SymbolReference *helper,
    TR::CodeGenerator *cg) {
  ...
  if (cg->directCallRequiresTrampoline(helperAddress,
                                       (intptrj_t)callInstructionAddress)) {
    ...
    TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinRIPRange(
                        helperAddress, nextInstructionAddress),
