...
static void zeroExtendTo32BitRegister(TR::Node *node, TR::Register *reg,
                                      int32_t sourceSize,
                                      TR::CodeGenerator *cg) {
  TR_X86OpCodes op;
  ...
  if (op != BADIA32Op) generateRegRegInstruction(op, node, reg, reg, cg);
}

/*
 * \brief
 * this API is for check nodes(like OverflowCHK) with certain operation where
 * the operands are given explicitly by the caller and are not the first and
 * second child of the given root node
 *
 * \param root
 *     the check node
 ...
 */
void TR_X86BinaryCommutativeAnalyser::genericAnalyserWithExplicitOperands(
    TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
    TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
    TR_X86OpCodes copyOpCode,
    bool nonClobberingDestination)  // false by default
{
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            "unsupported  opcode %s for genericAnalyserWithExplicitOperands on "
            "node %p\n",
            _cg->comp()->getDebug()->getName(root->getOpCodeValue()), root);
  TR::Register *tempReg =
      genericAnalyserImpl(root, firstChild, secondChild, regRegOpCode,
                          regMemOpCode, copyOpCode, nonClobberingDestination);
...
/*
 * \brief
 * this API is for regular operation nodes where the first child and second
 * child are the operands by default
 */
void TR_X86BinaryCommutativeAnalyser::genericAnalyser(
    TR::Node *root, TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
    TR_X86OpCodes copyOpCode,
    bool nonClobberingDestination)  // false by default
{
...
/*
 * users should call the genericAnalyser or genericAnalyserWithExplicitOperands
 * APIs instead of calling this one directly
 */
TR::Register *TR_X86BinaryCommutativeAnalyser::genericAnalyserImpl(
    TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
    TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
    TR_X86OpCodes copyOpCode, bool nonClobberingDestination) {
  ...
  if (getOpReg1Reg2()) {
    generateRegRegInstruction(regRegOpCode, root, firstRegister, secondRegister,
                              _cg);
    targetRegister = firstRegister;
  } else if (getOpReg2Reg1()) {
    generateRegRegInstruction(regRegOpCode, root, secondRegister, firstRegister,
                              _cg);
    ...
    if (regMemOpCode == TEST4MemReg || regMemOpCode == TEST8MemReg) {
      generateMemRegInstruction(regMemOpCode, root, tempMR, secondRegister,
                                _cg);
    } else {
      generateRegMemInstruction(regMemOpCode, root, secondRegister, tempMR,
                                _cg);
    }
    targetRegister = secondRegister;
    tempMR->decNodeReferenceCounts(_cg);
...
void TR_X86BinaryCommutativeAnalyser::genericLongAnalyser(
    TR::Node *root, TR_X86OpCodes lowRegRegOpCode,
    TR_X86OpCodes highRegRegOpCode, TR_X86OpCodes lowRegMemOpCode,
    TR_X86OpCodes lowRegMemOpCode2Byte, TR_X86OpCodes lowRegMemOpCode1Byte,
    TR_X86OpCodes highRegMemOpCode, TR_X86OpCodes copyOpCode) {
  TR::Node *firstChild;
  TR::Node *secondChild;
  if (_cg->whichChildToEvaluate(root) == 0) {
    firstChild = root->getFirstChild();
  ...
  // Can generate better code for long operations when one or more children have
  // their high order word as a zero. Examples are xu2l conversion operators,
  // right unsigned shifts by 32 or more bits and just things that value
  // propagation has found are in the range of an unsigned int. we can avoid the
  // evaluation when we don't need the result of such nodes for another parent.
  //
    ...
    if (firstChild->getReferenceCount() == 1 && firstRegister == 0) {
      ...
      if ((firstOp == TR::lushr &&
           ...
           (firstChild->getSecondChild()->getInt() &
            TR::TreeEvaluator::shiftMask(true)) == 32)) {
        bypassedFirstUnsignedConversionChild = firstChild;
        // only decrease RC for bypassed node and the constant node because we
        // bypass the lushr node but keep the reference to its first child
  ...
  //
  const bool dontClobberAnything = bypassedFirstUnsignedConversionChild ||
                                   bypassedSecondUnsignedConversionChild;
  ...
  if (getEvalChild1()) {
    ...
    if (bypassedFirstUnsignedConversionChild)
      zeroExtendTo32BitRegister(bypassedFirstUnsignedConversionChild,
                                firstRegister, firstChild->getSize(), _cg);
  }
  ...
  if (getEvalChild2()) {
    ...
    if (bypassedSecondUnsignedConversionChild)
      zeroExtendTo32BitRegister(bypassedSecondUnsignedConversionChild,
                                secondRegister, secondChild->getSize(), _cg);
  }
  ...
  if (getOpReg1Reg2()) {
    ...
    if (copyHighZero) {
      if (sourceHighZero) {
        ...
        if (root->getOpCodeValue() == TR::land) {
          ...
          generateRegRegInstruction(copyOpCode, root, copyHigh, sourceHigh,
                                    _cg);
        }
      }
      ...
      if (root->getOpCodeValue() == TR::land) {
        ...
        generateRegRegInstruction(copyOpCode, root, copyHigh,
                                  copyReg->getHighOrder(), _cg);
      }
    } else {
      generateRegRegInstruction(copyOpCode, root, copyHigh,
                                copyReg->getHighOrder(), _cg);
      generateRegRegInstruction(highRegRegOpCode, root, copyHigh, sourceHigh,
                                _cg);
    }
    ...
    if (memHighZero) {
      if (targetHighZero) targetHigh = _cg->allocateRegister();
      if (root->getOpCodeValue() == TR::land || targetHighZero) {
        generateRegRegInstruction(XOR4RegReg, root, targetHigh, targetHigh,
                                  _cg);
      }
      ...
      highMR = generateX86MemoryReference(*lowMR, 4, _cg);
      generateRegMemInstruction(highRegMemOpCode, root, targetHigh, highMR,
                                _cg);
    }
...
/*
 * \brief
 * this API is intended for regular add operation nodes where the first child
 * and second child are the operands by default
 */
void TR_X86BinaryCommutativeAnalyser::integerAddAnalyser(
    TR::Node *root, TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
    ...
    TR::Node *carry)   // 0 by default
{
  ...
  targetRegister =
      integerAddAnalyserImpl(root, firstChild, secondChild, regRegOpCode,
                             regMemOpCode, needsEflags, carry);
  root->setRegister(targetRegister);
  _cg->decReferenceCount(firstChild);
  _cg->decReferenceCount(secondChild);
}
...
/*
 * \brief
 * this API is for check nodes(like OverflowCHK) with an add operation where the
 * operands are given explicitly by the caller and are not the first and second
 * child of the given root node
 *
 * \param root
 *     the check node
 * \param firstChild, secondChild
 ...
 */
void TR_X86BinaryCommutativeAnalyser::integerAddAnalyserWithExplicitOperands(
    TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
    TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
    bool needsEflags,  // false by default
    TR::Node *carry)   // 0 by default
{
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            "unsupported opcode %s for integerAddAnalyserWithGivenOperands on "
            "node %p\n",
            _cg->comp()->getDebug()->getName(root->getOpCodeValue()), root);
  TR::Register *tempReg =
      integerAddAnalyserImpl(root, firstChild, secondChild, regRegOpCode,
...
/*
 * users should call the integerAddAnalyser or
 * integerAddAnalyserWithGivenOperands APIs instead of calling this one directly
 */
TR::Register *TR_X86BinaryCommutativeAnalyser::integerAddAnalyserImpl(
    TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
    TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode, bool needsEflags,
    TR::Node *carry) {
  TR::Register *targetRegister;
  ...
  if (root->isInternalPointer()) {
    ...
    //
    if (firstRegister && ((!firstRegister->containsInternalPointer()) ||
                          (firstRegister->getPinningArrayPointer() !=
                           root->getPinningArrayPointer())))
      resetClob1();

    if (secondRegister && ((!secondRegister->containsInternalPointer()) ||
                           (secondRegister->getPinningArrayPointer() !=
                            root->getPinningArrayPointer())))
      resetClob2();
  }

  if (comp->generateArraylets() && root->getOpCodeValue() == TR::aiadd) {
    // result of this add is probably an internal pointer into the arraylet
    // spine so make sure that, if either child is marked as collectible, we
    // won't reuse its register for this internal pointer
  ...
  //
  TR::TreeEvaluator::genNullTestSequence(root, firstRegister, firstRegister,
                                         _cg);

  // maybe a register that did not exist before has just been created,
  // and this new register contains a collected reference
  // arraylets: aiadd is technically internal pointer into spine object, but
  // isn't marked as internal pointer
  if (root->isInternalPointer() ||
      (comp->generateArraylets() && root->getOpCodeValue() == TR::aiadd)) {
    // check whether a newly created register contains collected reference
    if ((getEvalChild1() && ((!firstRegister->containsInternalPointer()) ||
                             (firstRegister->getPinningArrayPointer() !=
                              root->getPinningArrayPointer()))) ||
        (getEvalChild2() && ((!secondRegister->containsInternalPointer()) ||
                             (secondRegister->getPinningArrayPointer() !=
                              root->getPinningArrayPointer())))) {
      ...
      // if we need to copy registers, make sure we have both registers
      // available
      if (getCopyRegs()) {
        if (!firstRegister) firstRegister = _cg->evaluate(firstChild);
        if (!secondRegister) secondRegister = _cg->evaluate(secondChild);
      }
    }
  }

  if (carry != 0) TR_X86ComputeCC::setCarryBorrow(carry, false, _cg);
  ...
  if (getOpReg1Reg2()) {
      ...
      if (root->isInternalPointer()) {
        ...
        if (root->getPinningArrayPointer()) {
          ...
          tempReg->setPinningArrayPointer(root->getPinningArrayPointer());
                 ...
                 root->getOpCodeValue() == TR::aiadd)
        // arraylets: aiadd is technically internal pointer into spine object,
        // but isn't marked as internal pointer
        tempReg = _cg->allocateRegister();
      else
    ...
    // if eflags are required then we cannot use LEA as it doesn't set or use
    // them
    if (needsEflags || (carry != 0)) {
      generateRegRegInstruction(MOVRegReg(is64Bit), root, tempReg,
                                firstRegister, _cg);
      generateRegRegInstruction(regRegOpCode, root, tempReg, secondRegister,
                                _cg);
    } else {
...
/*
 * \brief
 * this API is for check nodes(like OverflowCHK) an ladd operation where the
 * operands are given explicitly by the caller and are not the first and second
 * child of the given root node
 *
 * \param root
...
void TR_X86BinaryCommutativeAnalyser::longAddAnalyserWithExplicitOperands(
    TR::Node *root, TR::Node *firstChild, TR::Node *secondChild) {
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            "unsupported opcode %s for longAddAnalyserWithExplicitOperands on "
            "node %p\n",
            _cg->comp()->getDebug()->getName(root->getOpCodeValue()), root);
...
/*
 * \brief
 * this API is intended for regular ladd operation nodes where the first child
 * and second child are the operands by default
 */
void TR_X86BinaryCommutativeAnalyser::longAddAnalyser(TR::Node *root) {
  ...
  TR::Register *targetRegister =
      longAddAnalyserImpl(root, firstChild, secondChild);
  root->setRegister(targetRegister);
  _cg->decReferenceCount(firstChild);
  _cg->decReferenceCount(secondChild);
}
...
/*
 * users should call the longAddAnalyser or longAddAnalyserWithExplicitOperands
 * APIs instead of calling this one directly
 */
TR::Register *TR_X86BinaryCommutativeAnalyser::longAddAnalyserImpl(
  ...
  // Can generate better code for long adds when one or more children have a
  // high order zero word can avoid the evaluation when we don't need the result
  // of such nodes for another parent.
  //
  TR::Node *bypassedFirstUnsignedConversionChild = NULL;
  TR::Node *bypassedSecondUnsignedConversionChild = NULL;
  ...
  if (isVolatileMemoryOperand(firstChild) ||
      firstChild->getSize() != ILTypeProp::Size_4)
    resetMem1();

  if (isVolatileMemoryOperand(secondChild) ||
      secondChild->getSize() != ILTypeProp::Size_4)
    resetMem2();
  ...
  if (getOpReg1Reg2()) {
    ...
    if (firstHighZero) {
      ...
      generateRegRegInstruction(MOV4RegReg, root, oneHigh,
                                firstRegister->getHighOrder(), _cg);
    }
    if (secondHighZero) {
      generateRegImmInstruction(ADC4RegImms, root, oneHigh, 0, _cg);
    } else {
...
void TR_X86BinaryCommutativeAnalyser::longDualMultiplyAnalyser(TR::Node *root) {
  bool needsHighMulOnly =
      (root->getOpCodeValue() == TR::lumulh) && !root->isDualCyclic();
  ...
  if (getEvalChild1()) firstRegister = _cg->evaluate(firstChild);

  if (getEvalChild2()) secondRegister = _cg->evaluate(secondChild);
  ...
  if (copyOne) {
    ...
    if (memOne) {
      ...
      TR::MemoryReference *oneHighMR =
          generateX86MemoryReference(*oneLowMR, 4, _cg);
  ...
  // requires:
  //   7 registers, five general purpose: R1, R2, R3, R4, R5, two special
  //   registers: EAX, EDX
  // entry:
  //   R5:R1 = a
  //   R3:R4 = b
  //   R1,R3,R4 are overwritten with result; EAX, EDX are overwritten. R5 is
  //   preserved.
  // exit
  //   R4:R3:R2:R1 = r = a * b
  ...
  generateRegRegInstruction(MOV4RegReg, root, eax, r1,
                            _cg);  //   1  MOV   EAX,  R1     // EAX = al
  generateRegRegInstruction(
      MUL4AccReg, root, eax, r4, depsMul[0],
      _cg);  //   2  MUL    R4          // EDX:EAX = al * bl
  generateRegRegInstruction(
      XCHG4RegReg, root, r1, eax,
      _cg);  //   3  XCHG   R1, EAX     // R1 = (al * bl)l; EAX = al
  generateRegRegInstruction(MOV4RegReg, root, r2, edx,
                            _cg);  //   4  MOV    R2, EDX     // R2 = (al * bl)h
  generateRegRegInstruction(
      MUL4AccReg, root, eax, r3, depsMul[1],
      _cg);  //   5  MUL    R3          // EDX:EAX = al * bh
  generateRegRegInstruction(
      XCHG4RegReg, root, r3, edx,
      _cg);  //   6  XCHG   R3, EDX     // R3 = (al * bh)h; EDX = bh
  generateRegRegInstruction(
      ADD4RegReg, root, r2, eax,
      _cg);  //   7  ADD    R2, EAX     // R2 = (al * bl)h + (al * bh)l
  generateRegImmInstruction(
      ADC4RegImms, root, r3, 0,
      _cg);  //   8  ADC    R3, 0       // R3 = (al * bh)h + [carry]
  generateRegRegInstruction(MOV4RegReg, root, eax, r5,
                            _cg);  //   9  MOV   EAX, R5      // EAX = ah
  generateRegRegInstruction(
      MUL4AccReg, root, eax, edx, depsMul[2],
      _cg);  //  10  MUL   EDX          // EDX:EAX = ah * bh
  generateRegRegInstruction(
      XCHG4RegReg, root, r4, edx,
      _cg);  //  11  XCHG   R4, EDX     // R4 = (ah * bh)h; EDX = bl
  generateRegRegInstruction(
      ADD4RegReg, root, r3, eax,
      _cg);  //  12  ADD    R3, EAX     // R3 = (al * bh)l + (ah * bh)l
  generateRegImmInstruction(
      ADC4RegImms, root, r4, 0,
      _cg);  //  13  ADC    R4, 0       // R4 = (ah * bh)h + [carry]
  generateRegRegInstruction(MOV4RegReg, root, eax, r5,
                            _cg);  //  14  MOV   EAX, R5      // EAX = ah
  generateRegRegInstruction(
      MUL4AccReg, root, eax, edx, depsMul[3],
      _cg);  //  15  MUL   EDX          // EDX:EAX = ah * bl
  generateRegRegInstruction(ADD4RegReg, root, r2, eax,
                            _cg);  //  16  ADD    R2, EAX     // R2 = (al * bl)h
                                   //  + (al * bh)l              + (ah * bl)l
  generateRegRegInstruction(ADC4RegReg, root, r3, edx,
                            _cg);  //  17  ADC    R3, EDX     // R3 = (al * bh)l
                                   //  + (ah * bh)l + (ah * bl)h + [carry]
  generateRegImmInstruction(ADC4RegImms, root, r4, 0,
                            _cg);  //  18  ADC    R4, 0       // R4 = (ah * bh)h
                                   //  + [carry]
...
void TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser(TR::Node *root) {
  ...
  // Can generate better code for multiplies when one or more children have a
  // high order zero word can avoid the evaluation when we don't need the result
  // of such nodes for another parent.
  //
  if (firstChild->isHighWordZero()) {
  ...
  if (reportHighWordZero) {
    diagnostic("long multiply analyser: firstChild  [" POINTER_PRINTF_FORMAT
               "] has a zero high word: %d\n",
  ...
  if (reportHighWordZero) {
    diagnostic("long multiply analyser: secondChild [" POINTER_PRINTF_FORMAT
               "] has a zero high word: %d\n",
  ...
  if (getOpReg1Reg2()) {
    ...
    if (!secondHighZero) {
      ...
      if (getOpReg2Reg1() == false) {
        twoHigh = _cg->allocateRegister();
        generateRegRegInstruction(MOV4RegReg, root, twoHigh,
    ...
    if (!firstHighZero) {
      if (!secondHighZero) {
        ...
        generateRegRegInstruction(MOV4RegReg, root, intermediateResultReg,
                                  oneHigh, _cg);
      }
    }

    TR::RegisterDependencyConditions *deps =
        generateRegisterDependencyConditions((uint8_t)3, 3, _cg);
    ...
    if (intermediateResultReg) {
      generateRegRegInstruction(ADD4RegReg, root, oneHigh,
                                intermediateResultReg, _cg);
    }
    ...
    if (intermediateResultReg) {
      generateRegRegInstruction(ADD4RegReg, root, twoHigh,
                                intermediateResultReg, _cg);
    }
    ...
    // Ah:Al=Al*Bl notation means that Ah:Al contains the result of the MUL
    // instruction between Al and Bl
    //
    // both high zero: compute                               Ah:Al=Al*Bl
    // (clobber Ah,Al) Ah zero:        compute Bh=Bh*Al, Ah:Al=Al*Bl, Ah=Ah+Bh
    // (clobber Ah,Al,Bh) Bh zero:        compute Ah=Ah*Bl, Bh:Bl=Bl*Al,
    // Bh=Bh+Ah  (clobber Ah,Bh,Bl) neither zero:   compute Ah=Ah*Bl, Bh=Bh*Al,
    // Bh=Bh+Ah, Ah:Al=Al*Bl, Ah=Ah+Bh  (clobber Ah,Bh,Al)
    ...
    if (firstHighZero) {
      ...
      if (secondHighZero) {
        ...
        generateRegRegInstruction(MOV4RegReg, root, twoLow, secondRegister,
                                  _cg);
      } else {
        generateRegRegInstruction(MOV4RegReg, root, twoHigh,
                                  secondRegister->getHighOrder(), _cg);
        oneLow = _cg->allocateRegister();
        generateRegRegInstruction(MOV4RegReg, root, oneLow,
                                  firstRegister->getLowOrder(), _cg);
        twoLow = secondRegister->getLowOrder();
      }
    ...
    TR::RegisterDependencyConditions *deps =
        generateRegisterDependencyConditions((uint8_t)2, 2, _cg);
    ...
    if (intermediateResultReg)
      generateRegRegInstruction(ADD4RegReg, root, resultHigh,
                                intermediateResultReg, _cg);

    // stop using the registers that we allocated
    //
    if (!firstHighZero && secondHighZero) _cg->stopUsingRegister(oneHigh);

    if (firstHighZero) {
      if (!secondHighZero) _cg->stopUsingRegister(twoHigh);
    }
