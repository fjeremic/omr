    ...
    TR_X86OpCodes copyOpCode)
{
    ...
    if (firstChild->isHighWordZero()) {
        ...
        if (firstChild->getReferenceCount() == 1 && firstRegister == 0) {
            ...
            if ((firstOp == TR::lushr && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && (firstChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true)) == 32)) {
    ...
    if (secondChild->isHighWordZero()) {
        ...
        if (secondChild->getReferenceCount() == 1 && secondRegister == 0) {
            // We cannot use a non-long grandchild that has not been zero-extended.
            if ((secondOp == TR::lushr && secondChild->getSecondChild()->getOpCodeValue() == TR::iconst && (secondChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true)) == 32)) {
    ...
    TR::Node* carry)
{
    ...
    if (root->isInternalPointer()) {
        ...
        //
        if (firstRegister && ((!firstRegister->containsInternalPointer()) || (firstRegister->getPinningArrayPointer() != root->getPinningArrayPointer())))
            resetClob1();

        if (secondRegister && ((!secondRegister->containsInternalPointer()) || (secondRegister->getPinningArrayPointer() != root->getPinningArrayPointer())))
            resetClob2();
    }
    ...
    if (root->isInternalPointer() || (comp->generateArraylets() && root->getOpCodeValue() == TR::aiadd)) {
        ...
        if ((getEvalChild1() && ((!firstRegister->containsInternalPointer()) || (firstRegister->getPinningArrayPointer() != root->getPinningArrayPointer()))) || (getEvalChild2() && ((!secondRegister->containsInternalPointer()) || (secondRegister->getPinningArrayPointer() != root->getPinningArrayPointer())))) {
    ...
    if (getOpReg1Reg2()) {
        ...
        TR::Register* tempReg;
        if (firstRegister->containsCollectedReference() || secondRegister->containsCollectedReference() || firstRegister->containsInternalPointer() || secondRegister->containsInternalPointer()) {
            if (root->isInternalPointer()) {
                tempReg = _cg->allocateRegister();
...
TR::Register* TR_X86BinaryCommutativeAnalyser::longAddAnalyserImpl(TR::Node* root, TR::Node*& firstChild, TR::Node*& secondChild)
{
    ...
    if (firstChild->isHighWordZero() && !needsEflags) {
        firstHighZero = true;
        if (firstChild->getReferenceCount() == 1 && firstRegister == 0) {
            if (firstOp == TR::iu2l || firstOp == TR::su2l || firstOp == TR::bu2l || (firstOp == TR::lushr && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && (firstChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true)) == 32)) {
                bypassedFirstUnsignedConversionChild = firstChild;
                firstChild = firstChild->getFirstChild();
                firstRegister = firstChild->getRegister();
                if (firstOp == TR::lushr) {
    ...
    if (secondChild->isHighWordZero() && !needsEflags) {
        secondHighZero = true;
        if (secondChild->getReferenceCount() == 1 && secondRegister == 0) {
            if (secondOp == TR::iu2l || secondOp == TR::su2l || secondOp == TR::bu2l || (secondOp == TR::lushr && secondChild->getSecondChild()->getOpCodeValue() == TR::iconst && (secondChild->getSecondChild()->getInt() & TR::TreeEvaluator::shiftMask(true)) == 32)) {
                bypassedSecondUnsignedConversionChild = secondChild;
                secondChild = secondChild->getFirstChild();
                secondRegister = secondChild->getRegister();
                if (secondOp == TR::lushr) {
    ...
    if (root->getOpCodeValue() == TR::luaddc && TR_X86ComputeCC::setCarryBorrow(root->getChild(2), false, _cg)) {
