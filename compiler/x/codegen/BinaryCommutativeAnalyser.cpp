...
void TR_X86BinaryCommutativeAnalyser::genericAnalyserWithExplicitOperands(TR::Node* root, TR::Node* firstChild,
  TR::Node* secondChild, TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode, TR_X86OpCodes copyOpCode,
 ...
 */
void TR_X86BinaryCommutativeAnalyser::genericAnalyser(TR::Node* root, TR_X86OpCodes regRegOpCode,
...
void TR_X86BinaryCommutativeAnalyser::genericLongAnalyser(TR::Node* root, TR_X86OpCodes lowRegRegOpCode,
  TR_X86OpCodes highRegRegOpCode, TR_X86OpCodes lowRegMemOpCode, TR_X86OpCodes lowRegMemOpCode2Byte,
  TR_X86OpCodes lowRegMemOpCode1Byte, TR_X86OpCodes highRegMemOpCode, TR_X86OpCodes copyOpCode)
{
  TR::Node* firstChild;
  TR::Node* secondChild;
  if (_cg->whichChildToEvaluate(root) == 0) {
  ...
  if (firstChild->isHighWordZero()) {
    ...
    if (firstChild->getReferenceCount() == 1 && firstRegister == 0) {
      ...
      if ((firstOp == TR::lushr && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst
        ...
        // only decrease RC for bypassed node and the constant node because we bypass the lushr node but keep the
 ...
 * default
 */
void TR_X86BinaryCommutativeAnalyser::integerAddAnalyser(TR::Node* root, TR_X86OpCodes regRegOpCode,
...
void TR_X86BinaryCommutativeAnalyser::integerAddAnalyserWithExplicitOperands(TR::Node* root, TR::Node* firstChild,
  TR::Node* secondChild, TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
  bool needsEflags, // false by default
  TR::Node* carry) // 0 by default
{
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
...
TR::Register* TR_X86BinaryCommutativeAnalyser::integerAddAnalyserImpl(TR::Node* root, TR::Node* firstChild,
  TR::Node* secondChild, TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode, bool needsEflags, TR::Node* carry)
{
  TR::Register* targetRegister;
...
void TR_X86BinaryCommutativeAnalyser::longDualMultiplyAnalyser(TR::Node* root)
{
  ...
  generateRegRegInstruction(
    MUL4AccReg, root, eax, edx, depsMul[2], _cg); //  10  MUL   EDX          // EDX:EAX = ah * bh
  ...
  generateRegRegInstruction(
    MUL4AccReg, root, eax, edx, depsMul[3], _cg); //  15  MUL   EDX          // EDX:EAX = ah * bl
  ...
  generateRegImmInstruction(
    ADC4RegImms, root, r4, 0, _cg); //  18  ADC    R4, 0       // R4 =                           (ah * bh)h + [carry]
