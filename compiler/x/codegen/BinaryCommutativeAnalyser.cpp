 ...
 */
void
TR_X86BinaryCommutativeAnalyser::genericAnalyserWithExplicitOperands(
  TR::Node* root,
  TR::Node* firstChild,
  TR::Node* secondChild,
  ...
  bool nonClobberingDestination) // false by default
{
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            ...
            "node %p\n",
            _cg->comp()->getDebug()->getName(root->getOpCodeValue()),
            root);
  TR::Register* tempReg = genericAnalyserImpl(root,
                                              firstChild,
                                              secondChild,
                                              regRegOpCode,
                                              regMemOpCode,
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::genericAnalyser(
  TR::Node* root,
  TR_X86OpCodes regRegOpCode,
  TR_X86OpCodes regMemOpCode,
  TR_X86OpCodes copyOpCode,
  bool nonClobberingDestination) // false by default
{
  ...
  targetRegister = genericAnalyserImpl(root,
                                       firstChild,
                                       secondChild,
                                       regRegOpCode,
                                       regMemOpCode,
 ...
 */
TR::Register*
TR_X86BinaryCommutativeAnalyser::genericAnalyserImpl(
  TR::Node* root,
  TR::Node* firstChild,
  TR::Node* secondChild,
  TR_X86OpCodes regRegOpCode,
  ...
  bool nonClobberingDestination)
{
  ...
  if (getOpReg1Reg2()) {
    generateRegRegInstruction(
      regRegOpCode, root, firstRegister, secondRegister, _cg);
    targetRegister = firstRegister;
  } else if (getOpReg2Reg1()) {
    generateRegRegInstruction(
      regRegOpCode, root, secondRegister, firstRegister, _cg);
    ...
    if (regMemOpCode == TEST4MemReg || regMemOpCode == TEST8MemReg) {
      generateMemRegInstruction(
        regMemOpCode, root, tempMR, secondRegister, _cg);
    } else {
      generateRegMemInstruction(
        regMemOpCode, root, secondRegister, tempMR, _cg);
    }
    targetRegister = secondRegister;
    tempMR->decNodeReferenceCounts(_cg);
    notReversedOperands();
...
void
TR_X86BinaryCommutativeAnalyser::genericLongAnalyser(
  TR::Node* root,
  ...
  if (getOpReg1Reg2()) {
    ...
    if (copyHighZero) {
      if (sourceHighZero) {
        ...
        if (root->getOpCodeValue() == TR::land) {
          ...
          generateRegRegInstruction(
            copyOpCode, root, copyHigh, sourceHigh, _cg);
        }
      }
      ...
      if (root->getOpCodeValue() == TR::land) {
        ...
        generateRegRegInstruction(
          copyOpCode, root, copyHigh, copyReg->getHighOrder(), _cg);
      }
    } else {
      generateRegRegInstruction(
        copyOpCode, root, copyHigh, copyReg->getHighOrder(), _cg);
      generateRegRegInstruction(
        highRegRegOpCode, root, copyHigh, sourceHigh, _cg);
    }
    ...
    if (memHighZero) {
      ...
      if (root->getOpCodeValue() == TR::land || targetHighZero) {
        generateRegRegInstruction(
          XOR4RegReg, root, targetHigh, targetHigh, _cg);
      }
      ...
      highMR = generateX86MemoryReference(*lowMR, 4, _cg);
      generateRegMemInstruction(
        highRegMemOpCode, root, targetHigh, highMR, _cg);
    }
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::integerAddAnalyser(
  TR::Node* root,
  TR_X86OpCodes regRegOpCode,
  TR_X86OpCodes regMemOpCode,
  ...
  TR::Node* carry)  // 0 by default
{
  ...
  targetRegister = integerAddAnalyserImpl(root,
                                          firstChild,
                                          secondChild,
                                          regRegOpCode,
                                          regMemOpCode,
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::integerAddAnalyserWithExplicitOperands(
  TR::Node* root,
  TR::Node* firstChild,
  TR::Node* secondChild,
  TR_X86OpCodes regRegOpCode,
  ...
  TR::Node* carry)  // 0 by default
{
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            ...
            _cg->comp()->getDebug()->getName(root->getOpCodeValue()),
            root);
  TR::Register* tempReg = integerAddAnalyserImpl(root,
                                                 firstChild,
                                                 secondChild,
                                                 regRegOpCode,
                                                 regMemOpCode,
 ...
 */
TR::Register*
TR_X86BinaryCommutativeAnalyser::integerAddAnalyserImpl(
  TR::Node* root,
  TR::Node* firstChild,
  TR::Node* secondChild,
  TR_X86OpCodes regRegOpCode,
  ...
  TR::Node* carry)
{
  ...
  TR::TreeEvaluator::genNullTestSequence(
    root, firstRegister, firstRegister, _cg);
  ...
  if (root->isInternalPointer() ||
    ...
    if ((getEvalChild1() && ((!firstRegister->containsInternalPointer()) ||
      ...
      setInputs(
        firstChild, firstRegister, secondChild, secondRegister, false, true);
  ...
  if (getOpReg1Reg2()) {
    ...
    if (needsEflags || (carry != 0)) {
      generateRegRegInstruction(
        MOVRegReg(is64Bit), root, tempReg, firstRegister, _cg);
      generateRegRegInstruction(
        regRegOpCode, root, tempReg, secondRegister, _cg);
    } else {
...
//
static bool
isVolatileMemoryOperand(TR::Node* node)
{
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::longAddAnalyserWithExplicitOperands(
  TR::Node* root,
  TR::Node* firstChild,
  TR::Node* secondChild)
{
 ...
 */
void
TR_X86BinaryCommutativeAnalyser::longAddAnalyser(TR::Node* root)
{
  TR::Node* firstChild = NULL;
  TR::Node* secondChild = NULL;
  if (_cg->whichChildToEvaluate(root) == 0) {
 ...
 */
TR::Register*
TR_X86BinaryCommutativeAnalyser::longAddAnalyserImpl(TR::Node* root,
                                                     TR::Node*& firstChild,
                                                     TR::Node*& secondChild)
{
  ...
  if (getOpReg1Reg2()) {
    ...
    if (firstHighZero) {
      ...
      generateRegRegInstruction(
        MOV4RegReg, root, oneHigh, firstRegister->getHighOrder(), _cg);
    }
    if (secondHighZero) {
      generateRegImmInstruction(ADC4RegImms, root, oneHigh, 0, _cg);
    } else {
...
//     ==> b
void
TR_X86BinaryCommutativeAnalyser::longDualMultiplyAnalyser(TR::Node* root)
{
  ...
  generateRegRegInstruction(
    MOV4RegReg, root, eax, r1, _cg); //   1  MOV   EAX,  R1     // EAX = al
  generateRegRegInstruction(
    MUL4AccReg,
    root,
    eax,
  ...
  generateRegRegInstruction(
    MOV4RegReg, root, eax, r5, _cg); //   9  MOV   EAX, R5      // EAX = ah
  generateRegRegInstruction(
    MUL4AccReg,
    root,
    eax,
  ...
  generateRegRegInstruction(
    MOV4RegReg, root, eax, r5, _cg); //  14  MOV   EAX, R5      // EAX = ah
  generateRegRegInstruction(
    MUL4AccReg,
    root,
    eax,
  ...
  generateRegImmInstruction(
    ADC4RegImms, root, r4, 0, _cg); //  18  ADC    R4, 0       // R4 = (ah *
                                    //  bh)h              + [carry]
...
void
TR_X86BinaryCommutativeAnalyser::longMultiplyAnalyser(TR::Node* root)
{
  TR::Node* firstChild = 0;
  TR::Node* secondChild = 0;
  if (_cg->whichChildToEvaluate(root) == 0) {
  ...
  if (getOpReg1Reg2()) {
    ...
    if (!secondHighZero) {
      ...
      if (getOpReg2Reg1() == false) {
        twoHigh = _cg->allocateRegister();
        generateRegRegInstruction(
          MOV4RegReg, root, twoHigh, secondRegister->getHighOrder(), _cg);
      } else {
        twoHigh = secondRegister->getHighOrder();
      }
    } else {
    ...
    if (!firstHighZero) {
      if (!secondHighZero) {
        ...
        generateRegRegInstruction(
          MOV4RegReg, root, intermediateResultReg, oneHigh, _cg);
      }
    }
    ...
    if (intermediateResultReg) {
      generateRegRegInstruction(
        ADD4RegReg, root, oneHigh, intermediateResultReg, _cg);
    }
    ...
    if (!secondHighZero) {
      if (!firstHighZero) {
        ...
        intermediateResultReg = _cg->allocateRegister();
        generateRegRegInstruction(
          MOV4RegReg, root, intermediateResultReg, twoHigh, _cg);
      }
    }
    ...
    if (intermediateResultReg) {
      generateRegRegInstruction(
        ADD4RegReg, root, twoHigh, intermediateResultReg, _cg);
    }
    ...
    if (firstHighZero) {
      ...
      if (secondHighZero) {
        ...
        generateRegRegInstruction(
          MOV4RegReg, root, twoLow, secondRegister, _cg);
      } else {
        generateRegRegInstruction(
          MOV4RegReg, root, twoHigh, secondRegister->getHighOrder(), _cg);
        oneLow = _cg->allocateRegister();
        generateRegRegInstruction(
          MOV4RegReg, root, oneLow, firstRegister->getLowOrder(), _cg);
        twoLow = secondRegister->getLowOrder();
      }
    ...
    if (intermediateResultReg)
      generateRegRegInstruction(
        ADD4RegReg, root, resultHigh, intermediateResultReg, _cg);
