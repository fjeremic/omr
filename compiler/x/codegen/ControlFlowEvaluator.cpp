...
bool isConditionCodeSetForCompareToZero(TR::Node* node, bool justTestZeroFlag)
{
    ...
    for (prevInstr = comp->cg()->getAppendInstruction();
        ...
        // correct set of bits
        if (prevRegInstr && prevRegInstr->getTargetRegister() == node->getRegister() && prevRegInstr->getNode() && prevRegInstr->getNode()->getSize() == node->getSize()) {
...
TR::Register* OMR::X86::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    intptrj_t* branchTable = (intptrj_t*)cg->allocateCodeMemory(numBranchTableEntries * sizeof(branchTable[0]), cg->getCurrentEvaluationBlock()->isCold());
...
void OMR::X86::TreeEvaluator::setupProfiledGuardRelocation(TR::X86RegImmInstruction* cmpInstruction,
    TR::Node* node,
...
void OMR::X86::TreeEvaluator::compareIntegersForEquality(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && (((secondChild->getSize() <= 2) && (!secondChild->isUnsigned())) || (TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg) && !cg->constantAddressesCanChangeSize(secondChild)))) {
        ...
        if (constValue >= -128 && constValue <= 127) {
            ...
            //
            {
                if (firstChild->getOpCode().isAnd() && (firstChild->getRegister() == NULL) && (firstChild->getReferenceCount() == 1)) {
                    // compare  (node)
                    //    and   (firstChild)
                    //       ?  (andFirstChild)
                    //       ?  (andSecondChild)
                    ...
                    uint64_t mask;
                    if (andSecondChild->getOpCode().isLoadConst() && andSecondChild->getRegister() == NULL && ((mask = andSecondChild->get64bitIntegralValueAsUnsigned()) >> 31) == 0) // this needs to be an unsigned value otherwise
                    ...
                    // and may be used in a 4 byte compare
                    {
                        ...
                        //    0
                        //
                        if (andFirstChild->getRegister() == NULL && andFirstChild->getReferenceCount() == 1 && andFirstChild->getOpCode().isLoadVar()) {
                            // memory case
                            ...
                            TR_X86OpCodes testInstr;
                            if (((mask >> 8) == 0 && !andFirstChild->isInvalid8BitGlobalRegister()) || (andSecondChild->getSize() == 1))
                    ...
                    if ((firstChild->getRegister() == NULL) && (firstChild->getOpCode().isAnd() || firstChild->getOpCode().isOr() || firstChild->getOpCode().isXor())) {
                    ...
                    if (isConditionCodeSetForCompareToZero(firstChild, true)) {
                        ...
                        if (compareSize > 1 && (firstChild->getOpCodeValue() == TR::b2i || firstChild->getOpCodeValue() == TR::bu2i || firstChild->getOpCodeValue() == TR::b2s || firstChild->getOpCodeValue() == TR::bu2s) && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
                            compareSize = 1;
                            cg->decReferenceCount(firstChild);
                            firstChild = firstChild->getFirstChild();
                        }
                        if (compareSize > 2 && (firstChild->getOpCodeValue() == TR::su2i || firstChild->getOpCodeValue() == TR::s2i) && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
                            compareSize = 2;
                            cg->decReferenceCount(firstChild);
                            firstChild = firstChild->getFirstChild();
                        }
                        if (firstChild->getOpCode().isMemoryReference() && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg)) {
        ...
        if (constValue != 0 || !isConditionCodeSetForCompareToZero(firstChild, false)) {
            ...
            //
            if (!node->getOpCode().isSpineCheck() && firstChild->getOpCode().isMemoryReference() && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
                TR::MemoryReference* tempMR = generateX86MemoryReference(firstChild, cg);
                TR::TreeEvaluator::compareGPMemoryToImmediate(node, tempMR, constValue, cg);
                tempMR->decNodeReferenceCounts(cg);
            } else {
...
void OMR::X86::TreeEvaluator::compare2BytesForOrder(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        ...
        if ((firstChild->getReferenceCount() == 1) && (firstChild->getRegister() == NULL) && firstChild->getOpCode().isMemoryReference()) {
            TR::MemoryReference* tempMR = generateX86MemoryReference(firstChild, cg);
...
TR::Register* OMR::X86::TreeEvaluator::iternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool longCompareOn32bit = (TR::Compiler->target.is32Bit() && conditionOp.isBooleanCompare() && condition->getFirstChild()->getOpCode().isLong());
    ...
    if (!longCompareOn32bit && conditionOp.isCompareForEquality() && condition->getFirstChild()->getOpCode().isIntegerOrAddress()) {
        ...
        generateRegRegInstruction((conditionOp.isCompareTrueIfEqual()) ? ((conditionOp.isCompareTrueIfGreater()) ? CMOVLRegReg(trueValIs64Bit) : CMOVGRegReg(trueValIs64Bit)) : ((conditionOp.isCompareTrueIfGreater()) ? CMOVLERegReg(trueValIs64Bit) : CMOVGERegReg(trueValIs64Bit)), node, trueReg, falseReg, cg);
    } else {
    ...
    if (node->getOpCodeValue() == TR::bternary && cg->enableRegisterInterferences())
...
TR::Register* OMR::X86::TreeEvaluator::integerIfCmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    //
    if (node->getFirstChild()->getOpCodeValue() == TR::icall && node->getSecondChild()->getOpCodeValue() == TR::iconst) {
        TR::Node* firstChild = node->getFirstChild();
        TR::Node* secondChild = node->getSecondChild();

        TR::MethodSymbol* symbol = firstChild->getSymbol()->castToMethodSymbol();

        if (cg->getSupportsBDLLHardwareOverflowCheck() && (symbol->getRecognizedMethod() == TR::java_math_BigDecimal_noLLOverflowAdd || symbol->getRecognizedMethod() == TR::java_math_BigDecimal_noLLOverflowMul)) {
            cg->evaluate(firstChild);
            cg->evaluate(secondChild);
...
TR::Register* OMR::X86::TreeEvaluator::integerIfCmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (virtualGuardHelper(node, cg)) {
        ...
        if (node->getFirstChild()->getOpCodeValue() == TR::ishr && node->getFirstChild()->getRegister() == NULL && node->getFirstChild()->getReferenceCount() == 1 && (node->getFirstChild()->getFirstChild()->getOpCodeValue() == TR::iloadi || node->getFirstChild()->getFirstChild()->getOpCodeValue() == TR::iload) && node->getFirstChild()->getSecondChild()->getOpCodeValue() == TR::iconst && node->getSecondChild()->getOpCodeValue() == TR::iconst && node->getSecondChild()->getInt() == 0 && performTransformation(comp, "O^O SHIFT PEEPHOLE: detected shift pattern for node %p shifting so mask = %p shift amount = %d \n", node, (((int32_t)-1) << node->getFirstChild()->getSecondChild()->getInt()), node->getFirstChild()->getSecondChild()->getInt())) {
...
TR::Register* OMR::X86::TreeEvaluator::ifbcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        ...
        if ((firstChild->getReferenceCount() == 1) && (firstChild->getRegister() == NULL) && firstChild->getOpCode().isMemoryReference()) {
            ...
            cg->decReferenceCount(firstChild);
            cg->decReferenceCount(secondChild);
        } else if (firstChild->getOpCode().isAnd() && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && // TODO: we can still do better in this case
            firstChild->getSecondChild()->getOpCode().isLoadConst() && (value == 0 || (value == (int32_t)firstChild->getSecondChild()->get64bitIntegralValue() && isPowerOf2(value & 0xff)))) {
            // ifXcmp[eq|ne]
            //   Xand
            //     expr
            //     Xconst k2
            ...
            if (expr->getReferenceCount() == 1 && expr->getRegister() == NULL && expr->getOpCode().isMemoryReference()) {
