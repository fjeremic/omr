...
// Right now, this function is duplicated in TreeEval, ControlFlowEval, Binary
// &UnaryEval.
...
// Right now, this is duplicated in ControlFlowEval, BinaryEval and TreeEval.
inline bool constNodeValueIs32BitSigned(TR::Node *node, intptrj_t *value,
                                        TR::CodeGenerator *cg) {
...
static inline TR::Instruction *generateWiderCompare(TR::Node *node,
                                                    TR::Register *targetReg,
                                                    intptrj_t value,
                                                    TR::CodeGenerator *cg) {
  // using 16bit immediate in a 32-bit compare instruction (0x66 length change
  // prefix) results in a decode penalty. The instruction length decoder of the
  // Intel Core processors cannot decode the length of an LCP instruction in one
  // cycle, therefore it initiates slow decoding, which takes five extra cycles
  // to complete.
  //
...
bool isConditionCodeSetForCompareToZero(TR::Node *node, bool justTestZeroFlag) {
  ...
  for (prevInstr = comp->cg()->getAppendInstruction(); prevInstr;
    ...
    // correct set of bits
    if (prevRegInstr &&
        prevRegInstr->getTargetRegister() == node->getRegister() &&
        prevRegInstr->getNode() &&
      ...
      if (prevRegInstr->getOpCode().modifiesTarget()) {
        ...
        if (justTestZeroFlag) {
          if (!prevInstr->getOpCode().setsCCForTest())
            ...
            // Only constant shift instructions should be considered.  For those
            // instructions, the flags will only be set as required if the count
            // != 0.
            //
            uint32_t count = 0;
            if (prevInstr->getOpCode().hasByteImmediate()) {
              count = ((TR::X86RegImmInstruction *)prevRegInstr)
                          ->getSourceImmediate();
            }
          ...
          if (!prevInstr->getOpCode().setsCCForCompare()) return false;
        }
...
static void binarySearchCaseSpace(TR::Register *selectorReg,
                                  TR::Node *lookupNode, uint32_t lowChild,
                                  uint32_t highChild,
  ...
  if (pivot >= lowChild) {
    ...
    //                                         [--------------------------|------------------------------------)
    //                                         unsigned
    // If all the cases of the lookup are on the same 'half' of the number
    // lines, it doesn't matter if we use JA or JG, so lets use JG. For the
    // signed case, we better use JG if the numbers straddle the 0 boundary; we
    // already do. For the unsigned case, we better use JA if the numbers
    // straddle the TR::getMaxSigned<TR::Int32>() boundary. If that's the case,
    // the highVal will be less than (in the signed sense) than lowVal.
    //
    bool isUnsigned;
    ...
    generateLabelInstruction(isUnsigned ? JA4 : JG4, lookupNode, pivotLabel,
                             cg);

    if (lowChild == pivot) {
      generateJumpInstruction(JE4, lookupNode->getChild(lowChild), cg);
      generateJumpInstruction(JMP4, lookupNode->getChild(1), cg, false,
                              evaluateDefaultGlRegDeps);
      evaluateDefaultGlRegDeps = false;
    } else {
      binarySearchCaseSpace(selectorReg, lookupNode, lowChild, pivot,
                            evaluateDefaultGlRegDeps, cg);
    }
    generateLabelInstruction(LABEL, lookupNode, pivotLabel, cg);
  } else
    TR_ASSERT(pivot == lowChild - 1 && lowChild == highChild,
              "unexpected pivot value in binarySearchCaseSpace");
  ...
  if (highChild == pivot + 1) {
    ...
    binarySearchCaseSpace(selectorReg, lookupNode, pivot + 1, highChild,
                          evaluateDefaultGlRegDeps, cg);
  }
}

TR::Register *OMR::X86::TreeEvaluator::lookupEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  ...
  TR::RegisterDependencyConditions *deps = generateRegisterDependencyConditions(
      (uint8_t)0, TR::RealRegister::MaxAssignableRegisters, cg);
    ...
    if (caseChild->getNumChildren() > 0) {
      ...
      if (firstCaseChild->getOpCodeValue() == TR::GlRegDeps) {
        for (int j = firstCaseChild->getNumChildren() - 1; j >= 0; j--) {
          ...
          TR_GlobalRegisterNumber highGlobalRegNum =
              child->getHighGlobalRegisterNumber();
          ...
          if (globalReg->getKind() == TR_GPR && highGlobalRegNum < 0 &&
            ...
            if (!deps->findPostCondition(registerIndex))
              deps->addPostCondition(
                  globalRegPair ? globalRegPair->getLowOrder() : globalReg,
                  registerIndex, cg);
            if (highGlobalRegNum >= 0) {
              ...
              if (!deps->findPostCondition(highRegisterIndex))
                deps->addPostCondition(globalRegPair->getHighOrder(),
                                       highRegisterIndex, cg);
            }
          }
        }
      }
  ...
  binarySearchCaseSpace(selectorReg, node, 2, node->getNumChildren() - 1,
                        evaluateDefaultGlRegDeps, cg);
...
TR::Register *OMR::X86::TreeEvaluator::tableEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  int32_t i;
  uint32_t numBranchTableEntries = node->getNumChildren() - 2;
  intptrj_t *branchTable = (intptrj_t *)cg->allocateCodeMemory(
      numBranchTableEntries * sizeof(branchTable[0]),
  ...
  if (!canSkipBoundTest) {
    ...
    generateRegImmInstruction(opCode, node, selectorReg, numBranchTableEntries,
                              cg);
  ...
  if (TR::Compiler->target.is64Bit() && cg->comp()->compileRelocatableCode()) {
    ...
    TR::MemoryReference *branchTableLeaMR =
        generateX86MemoryReference(label, cg);
    branchTableReg = cg->allocateRegister();
    generateRegMemInstruction(LEA8RegMem, node, branchTableReg,
                              branchTableLeaMR, cg);
    jumpMR = generateX86MemoryReference(branchTableReg, selectorReg, 3, cg);
  } else {
    jumpMR = generateX86MemoryReference(
        (TR::Register *)NULL, selectorReg,
  ...
  if (secondChild->getNumChildren() > 0) {
    deps = generateRegisterDependencyConditions(secondChild->getFirstChild(),
                                                cg, 0, NULL);
    deps->stopAddingConditions();
  }

  if (cg->getLinkage()->getProperties().getMethodMetaDataRegister() !=
      TR::RealRegister::NoReg) {
    jmpTableInstruction = generateMemTableInstruction(
        JMPMem, node, jumpMR, numBranchTableEntries, deps, cg);
  } else {
  ...
  if (branchTableReg != NULL) cg->stopUsingRegister(branchTableReg);

  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::minmaxEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
void OMR::X86::TreeEvaluator::setupProfiledGuardRelocation(
    TR::X86RegImmInstruction *cmpInstruction, TR::Node *node,
    TR_ExternalRelocationTargetKind reloKind) {
#ifdef J9_PROJECT_SPECIFIC
  // The following makes sure that the TR_ProfiledInlinedMethod relocation for
  // this inlined site will be created later in
  // TR::CodeGenerator::processRelocations()
  TR::Compilation *comp = TR::comp();
  ...
  // If we've generated an instruction, then make sure it is marked to get the
  // right kind of relocation
  if (cmpInstruction) {
...
void OMR::X86::TreeEvaluator::compareIntegersForEquality(
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (constValue >= -128 && constValue <= 127) {
      ...
      //
      {
        if (firstChild->getOpCode().isAnd() &&
          ...
          // This code path cheats a bit and uses a 4-byte instruction to
          // compare a 2-byte value with zero.  Ordinarily, this wouldn't work
          // because the top 2 bytes of the value are garbage. To make this work,
          // we must zero-extend the mask immediate to 4 bytes, thereby ensuring
          // the top two bytes of the masked value will be zero and won't affect
          // the subsequent compare with zero.

          // Therefore, counterintuitively, zero-entending the mask is always
          // the correct thing to do, regardless of whether the compare node
          // claims to be a signed operation!
          uint64_t mask;
          if (andSecondChild->getOpCode().isLoadConst() &&
              andSecondChild->getRegister() == NULL &&
              ((mask = andSecondChild->get64bitIntegralValueAsUnsigned()) >>
               31) == 0)  // this needs to be an unsigned value otherwise
                          ...
                          // and may be used in a 4 byte compare
          {
            ...
            if (andFirstChild->getRegister() == NULL &&
              ...
              TR::MemoryReference *tempMR =
                  generateX86MemoryReference(andFirstChild, cg);
              ...
              if (((mask >> 8) == 0) || (andSecondChild->getSize() == 1))
                ...
                TR::TreeEvaluator::loadConstant(
                    node, mask, TR_RematerializableShort, cg, tempReg);
                generateMemRegInstruction(TEST2MemReg, node, tempMR, tempReg,
                                          cg);
                cg->stopUsingRegister(tempReg);
              } else
                generateMemImmInstruction(TESTMemImm4(is64Bit), node, tempMR,
                                          mask, cg);
              tempMR->decNodeReferenceCounts(cg);
            } else {
              // register case
              // before evaluating the andFirstChild we check to see if it is an
              // integral conversion that is redundant because of the constant
              ...
              if (andFirstChild->getOpCode().isConversion() &&
                  ...
                  andFirstChild->getSize() >
                      andFirstChild->getFirstChild()->getSize() &&
                  ((mask >> (8 * andFirstChild->getFirstChild()->getSize())) ==
                   0)) {
              ...
              TR_X86OpCodes testInstr;
              if (((mask >> 8) == 0 &&
                   !andFirstChild->isInvalid8BitGlobalRegister()) ||
            ...
            temp.genericAnalyser(firstChild, testRRInstr, testMRInstr,
                                 movRRInstr, true);
          }
        } else {
          if ((firstChild->getRegister() == NULL) &&
              (firstChild->getOpCode().isAnd() ||
               firstChild->getOpCode().isOr() ||
          ...
          if (isConditionCodeSetForCompareToZero(firstChild, true)) {
            ...
            if (compareSize > 1 &&
                (firstChild->getOpCodeValue() == TR::b2i ||
                 firstChild->getOpCodeValue() == TR::bu2i ||
                 firstChild->getOpCodeValue() == TR::b2s ||
                 firstChild->getOpCodeValue() == TR::bu2s) &&
                firstChild->getRegister() == NULL &&
                firstChild->getReferenceCount() == 1) {
              compareSize = 1;
            ...
            if (compareSize > 2 &&
                (firstChild->getOpCodeValue() == TR::su2i ||
                 firstChild->getOpCodeValue() == TR::s2i) &&
                firstChild->getRegister() == NULL &&
                firstChild->getReferenceCount() == 1) {
              compareSize = 2;
              cg->decReferenceCount(firstChild);
            ...
            if (firstChild->getOpCode().isMemoryReference() &&
              ...
              if (compareSize == 1)
                ...
                TR::Register *tempReg = cg->allocateRegister();
                TR::TreeEvaluator::loadConstant(
                    node, 0, TR_RematerializableShort, cg, tempReg);
                generateMemRegInstruction(CMP2MemReg, node, tempMR, tempReg,
                                          cg);
                cg->stopUsingRegister(tempReg);
              } else
                TR::TreeEvaluator::compareGPMemoryToImmediate(node, tempMR, 0,
                                                              cg);
              tempMR->decNodeReferenceCounts(cg);
            } else {
              ...
              if (isConditionCodeSetForCompareToZero(firstChild, true)) {
                ...
                if (compareSize == 1)
                  generateRegRegInstruction(TEST1RegReg, node, firstChildReg,
                                            firstChildReg, cg);
                else if (compareSize == 2)
                  generateRegRegInstruction(TEST2RegReg, node, firstChildReg,
                                            firstChildReg, cg);
                else
        ...
        if (firstChild->getOpCode().isMemoryReference() &&
          ...
          TR::MemoryReference *tempMR =
              generateX86MemoryReference(firstChild, cg);
          if (compareSize == 1)
            generateMemImmInstruction(CMP1MemImm1, node, tempMR, constValue,
                                      cg);
          else if (compareSize == 2) {
            // shouldn't use Imm2 instructions
            TR::Register *tempReg = cg->allocateRegister();
            TR::TreeEvaluator::loadConstant(
                node, constValue, TR_RematerializableShort, cg, tempReg);
            generateMemRegInstruction(CMP2MemReg, node, tempMR, tempReg, cg);
            cg->stopUsingRegister(tempReg);
          } else
            TR::TreeEvaluator::compareGPMemoryToImmediate(node, tempMR,
                                                          constValue, cg);
          tempMR->decNodeReferenceCounts(cg);
        } else {
          TR::Register *firstChildReg = cg->evaluate(firstChild);
          if (compareSize == 1)
            generateRegImmInstruction(CMP1RegImm1, node, firstChildReg,
                                      constValue, cg);
          else if (compareSize == 2) {
            /// generateRegImmInstruction(CMP2RegImm2, node, firstChildReg,
            /// constValue, cg);
            generateWiderCompare(node, firstChildReg, constValue, cg);
          } else
            TR::TreeEvaluator::compareGPRegisterToImmediateForEquality(
                node, firstChildReg, constValue, cg);
        }

        if (secondChild->getOpCodeValue() == TR::aconst) {
          TR_ASSERT(!secondChild->isClassPointerConstant(),
                    "Class pointer constant unexpected here\n");
          TR_ASSERT(!secondChild->isMethodPointerConstant(),
                    "Method pointer constant unexpected here\n");
        }
      }
    } else {
      ...
      if (size == 1)
        cmpInstruction = generateRegImmInstruction(
            CMP1RegImm1, node, firstChildReg, constValue, cg);
      else if (size == 2) {
        /// cmpInstruction = generateRegImmInstruction(CMP2RegImm2, node,
        /// firstChildReg, constValue, cg);
        cmpInstruction =
            generateWiderCompare(node, firstChildReg, constValue, cg);
      } else {
        cmpInstruction = generateRegImmInstruction(
            CMPRegImm4(is64Bit), node, firstChildReg, constValue, cg);
      }
      ...
      if (isPICCandidate && cg->wantToPatchClassPointer(
                                (TR_OpaqueClassBlock *)constValue, secondChild))
        ...
        if (secondChild->isClassPointerConstant()) {
          if (cg->profiledPointersRequireRelocation())
            TR::TreeEvaluator::setupProfiledGuardRelocation(
                (TR::X86RegImmInstruction *)cmpInstruction, node,
                TR_ClassPointer);

          if (cg->fe()->isUnloadAssumptionRequired(
                  (TR_OpaqueClassBlock *)secondChild->getAddress(),
    ...
    // If we are comparing class addresses, check whether the header is
    // compressed on 64-bit to do the right size of compare.
    //
    TR::Node *firstChild = node->getFirstChild();
    ...
    if (TR::Compiler->target.is64Bit() &&
      ...
      if ((firstChild->getOpCode().isLoadIndirect() &&
          ...
          (secondChild->getOpCode().isLoadIndirect() &&
           secondChild->getSymbolReference() ==
               comp->getSymRefTab()->findVftSymbolRef())) {
        is64Bit = false;
      }
    }
    ...
    if (node->isProfiledGuard() && cg->profiledPointersRequireRelocation()) {
      ...
      if (secondChild->getOpCodeValue() == TR::aconst) {
        if (secondChild->isClassPointerConstant())
          TR::TreeEvaluator::setupProfiledGuardRelocation(NULL, node,
                                                          TR_ClassPointer);
...
void OMR::X86::TreeEvaluator::compareIntegersForOrder(TR::Node *node,
                                                      TR::Node *firstChild,
                                                      TR::Node *secondChild,
                                                      TR::CodeGenerator *cg) {
  intptrj_t constValue;
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    // condition codes for the first child's register, then we can omit the
    // compare instruction here.
    //
    if (constValue != 0 ||
        !isConditionCodeSetForCompareToZero(firstChild, false)) {
      ...
      if (!node->getOpCode().isSpineCheck() &&
        ...
        TR::MemoryReference *tempMR =
            generateX86MemoryReference(firstChild, cg);
        TR::TreeEvaluator::compareGPMemoryToImmediate(node, tempMR, constValue,
                                                      cg);
        tempMR->decNodeReferenceCounts(cg);
      } else {
        TR::TreeEvaluator::compareGPRegisterToImmediate(
            node, cg->evaluate(firstChild), constValue, cg);
      }
    }
    ...
    bool is64Bit = TR::TreeEvaluator::getNodeIs64Bit(secondChild, cg);
    TR_X86CompareAnalyser temp(cg);
    temp.integerCompareAnalyser(node, firstChild, secondChild, false,
                                CMPRegReg(is64Bit), CMPRegMem(is64Bit),
                                CMPMemReg(is64Bit));
  }
}
...
void OMR::X86::TreeEvaluator::compareIntegersForOrder(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, node->getFirstChild(),
                                             node->getSecondChild(), cg);
}

void OMR::X86::TreeEvaluator::compare2BytesForOrder(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  if (secondChild->getOpCode().isLoadConst() &&
      secondChild->getRegister() == NULL) {
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      if (isByteValue)
        ...
        TR::Register *tempReg = cg->allocateRegister();
        TR::TreeEvaluator::loadConstant(node, value, TR_RematerializableShort,
                                        cg, tempReg);
        generateMemRegInstruction(CMP2MemReg, node, tempMR, tempReg, cg);
        cg->stopUsingRegister(tempReg);
      }
...
void OMR::X86::TreeEvaluator::compareBytesForOrder(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  if (secondChild->getOpCode().isLoadConst() &&
      secondChild->getRegister() == NULL) {
    ...
    if ((firstChild->getReferenceCount() == 1) &&
        ...
        firstChild->getOpCode().isMemoryReference()) {
      TR::MemoryReference *tempMR = generateX86MemoryReference(firstChild, cg);
      generateMemImmInstruction(CMP1MemImm1, firstChild, tempMR,
                                secondChild->getByte(), cg);
      tempMR->decNodeReferenceCounts(cg);
    } else {
      generateRegImmInstruction(CMP1RegImm1, node, cg->evaluate(firstChild),
                                secondChild->getByte(), cg);
    }
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
...
TR::Register *OMR::X86::TreeEvaluator::gotoEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  generateJumpInstruction(JMP4, node, cg, false);
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::igotoEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR_ASSERT(node->getNumChildren() <= 2,
            "igoto cannot have more than 2 children");
  TR::RegisterDependencyConditions *secondChildDeps = NULL;
  if (node->getNumChildren() == 2) {
    TR::Node *secondChild = node->getSecondChild();
    TR_ASSERT(secondChild->getOpCodeValue() == TR::GlRegDeps,
              "second child of a igoto should be a TR::GlRegDeps");
    cg->evaluate(secondChild);
    List<TR::Register> popRegisters(cg->trMemory());

    secondChildDeps =
        generateRegisterDependencyConditions(secondChild, cg, 0, &popRegisters);
    cg->decReferenceCount(secondChild);
...
TR::Register *OMR::X86::TreeEvaluator::integerReturnEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::X86::TreeEvaluator::returnEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::X86::TreeEvaluator::iternaryEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR::Register *trueReg =
      TR::TreeEvaluator::intOrLongClobberEvaluate(trueVal, trueValIs64Bit, cg);
  ...
  bool longCompareOn32bit =
      (TR::Compiler->target.is32Bit() && conditionOp.isBooleanCompare() &&
       condition->getFirstChild()->getOpCode().isLong());
  // if ((conditionOp == TR::icmpeq) || (conditionOp == TR::icmpne) ||
  // (conditionOp == TR::lcmpeq) || (conditionOp == TR::lcmpne))
  if (!longCompareOn32bit && conditionOp.isCompareForEquality() &&
    ...
    if (conditionOp.isCompareTrueIfEqual())
      generateRegRegInstruction(CMOVNERegReg(trueValIs64Bit), node, trueReg,
                                falseReg, cg);
    else
      generateRegRegInstruction(CMOVERegReg(trueValIs64Bit), node, trueReg,
                                falseReg, cg);
    ...
    generateRegRegInstruction((conditionOp.isCompareTrueIfEqual())
                                  ? ((conditionOp.isCompareTrueIfGreater())
                                         ? CMOVLRegReg(trueValIs64Bit)
                                         : CMOVGRegReg(trueValIs64Bit))
                                  : ((conditionOp.isCompareTrueIfGreater())
                                         ? CMOVLERegReg(trueValIs64Bit)
                                         : CMOVGERegReg(trueValIs64Bit)),
...
TR::Register *OMR::X86::TreeEvaluator::integerIfCmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (node->getFirstChild()->getOpCodeValue() == TR::icall &&
    ...
    if (cg->getSupportsBDLLHardwareOverflowCheck() &&
        (symbol->getRecognizedMethod() ==
         ...
         symbol->getRecognizedMethod() ==
             TR::java_math_BigDecimal_noLLOverflowMul)) {
      cg->evaluate(firstChild);
      cg->evaluate(secondChild);
...
static inline void generateMergedGuardCodeIfNeeded(
    TR::Node *node, TR::CodeGenerator *cg, TR::Instruction *runtimeGuard) {
#ifdef J9_PROJECT_SPECIFIC
  if (node->isTheVirtualGuardForAGuardedInlinedCall() &&
    ...
    if (virtualGuard && (virtualGuard->mergedWithOSRGuard() ||
      ...
      if (runtimeGuard->getDependencyConditions())
        vgnopInstr = generateVirtualGuardNOPInstruction(
            runtimeGuard->getPrev(), node, site,
            runtimeGuard->getDependencyConditions()->clone(cg, 0), label, cg);
      else
        vgnopInstr = generateVirtualGuardNOPInstruction(
            runtimeGuard->getPrev(), node, site, NULL, label, cg);
    }
  }
#endif
}

TR::Register *OMR::X86::TreeEvaluator::integerIfCmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  if (virtualGuardHelper(node, cg)) {
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::ishr &&
        ...
        performTransformation(
            comp,
            "O^O SHIFT PEEPHOLE: detected shift pattern for node %p shifting "
            "so mask = %p shift amount = %d \n",
            node,
            (
                ((int32_t)-1)
      ...
      if (loadReg) {
        ...
        TR::MemoryReference *sourceMR =
            generateX86MemoryReference(loadNode, cg);
        generateMemImmInstruction(
            TEST4MemImm4, node, sourceMR,
            (
                ((int32_t)-1)
                << node->getFirstChild()->getSecondChild()->getInt()),
            cg);
      ...
      TR::X86LabelInstruction *instr =
          generateConditionalJumpInstruction(JNE4, node, cg, true);
    ...
    //
    // Disabled code below because this is now being handled in
    // compareIntegersForEquality Below test was risky as the append instruction
    // might not be the instruction we want
    //
    // if (node->isTheVirtualGuardForAGuardedInlinedCall())
    //         {
    //         TR_VirtualGuard *virtualGuard = comp->findVirtualGuardInfo(node);
    ...
    //             !TR::Compiler->cls.sameClassLoaders(comp,
    //             virtualGuard->getThisClass(),
    //             comp->getCurrentMethod()->classOfMethod()))
    //            {
    //            TR::Instruction *guardInstr =
    //            comp->cg()->getAppendInstruction();
...
bool OMR::X86::TreeEvaluator::generateIAddOrSubForOverflowCheck(
    TR::Node *compareNode, TR::CodeGenerator *cg) {
  TR_ArithmeticOverflowCheckNodes u = {0};
  bool matches =
      TR::TreeEvaluator::nodeIsIArithmeticOverflowCheck(compareNode, &u);
  if (matches &&
      (u.operationNode->getOpCode().isAdd() ||
       u.operationNode->getOpCode().isSub()) &&
      (u.leftChild->getReferenceCount() >= 1) &&
      (u.rightChild->getReferenceCount() >= 1) &&
      performTransformation(cg->comp(),
                            "O^O OVERFLOW CHECK RECOGNITION: Recognizing %s\n",
                            cg->getDebug()->getName(compareNode))) {
    TR::Register *rightReg = cg->evaluate(u.rightChild);
    // leftChild might appear twice in this tree, and we need a clobber evaluate
    // only if it also appears elsewhere
    bool leftNeedsCopy = u.leftChild->getReferenceCount() > 2 ||
                         (u.leftChild->getReferenceCount() > 1 &&
                          u.operationNode->getRegister());
    TR::Register *leftReg = leftNeedsCopy ? cg->intClobberEvaluate(u.leftChild)
                                          : cg->evaluate(u.leftChild);
    TR_X86OpCodes opCode =
        u.operationNode->getOpCode().isAdd() ? ADD4RegReg : SUB4RegReg;
    generateRegRegInstruction(opCode, u.operationNode, leftReg, rightReg, cg);
    if (!u.operationNode->getRegister()) {
      // We actually evaluated the operationNode here so we have to do
      // everything its evaluator would have done
...
bool OMR::X86::TreeEvaluator::generateLAddOrSubForOverflowCheck(
    TR::Node *compareNode, TR::CodeGenerator *cg) {
  TR_ArithmeticOverflowCheckNodes u = {0};
  bool matches =
      TR::TreeEvaluator::nodeIsLArithmeticOverflowCheck(compareNode, &u);
  if (matches &&
      (u.operationNode->getOpCode().isAdd() ||
       u.operationNode->getOpCode().isSub()) &&
      (u.leftChild->getReferenceCount() >= 1) &&
    ...
    bool leftNeedsCopy = u.leftChild->getReferenceCount() > 2 ||
                         (u.leftChild->getReferenceCount() > 1 &&
                          u.operationNode->getRegister());
    TR::Register *leftReg = leftNeedsCopy ? cg->longClobberEvaluate(u.leftChild)
                                          : cg->evaluate(u.leftChild);
    if (TR::Compiler->target.is64Bit()) {
      TR_X86OpCodes opCode =
          u.operationNode->getOpCode().isAdd() ? ADD8RegReg : SUB8RegReg;
      generateRegRegInstruction(opCode, u.operationNode, leftReg, rightReg, cg);
    } else if (u.operationNode->getOpCode().isAdd()) {
      generateRegRegInstruction(ADD4RegReg, u.operationNode,
                                leftReg->getLowOrder(), rightReg->getLowOrder(),
                                cg);
      generateRegRegInstruction(ADC4RegReg, u.operationNode,
                                leftReg->getHighOrder(),
                                rightReg->getHighOrder(), cg);
    } else {
      generateRegRegInstruction(SUB4RegReg, u.operationNode,
                                leftReg->getLowOrder(), rightReg->getLowOrder(),
                                cg);
      generateRegRegInstruction(SBB4RegReg, u.operationNode,
                                leftReg->getHighOrder(),
                                rightReg->getHighOrder(), cg);
    }
    if (!u.operationNode->getRegister()) {
...
TR::Register *OMR::X86::TreeEvaluator::integerIfCmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::integerIfCmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::TreeEvaluator::getNodeIs64Bit(node, cg)
          ? TR::TreeEvaluator::generateLAddOrSubForOverflowCheck(node, cg)
          : TR::TreeEvaluator::generateIAddOrSubForOverflowCheck(node, cg)) {
...
TR::Register *OMR::X86::TreeEvaluator::integerIfCmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *OMR::X86::TreeEvaluator::integerIfCmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerIfCmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerIfCmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerIfCmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerIfCmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register *OMR::X86::TreeEvaluator::ifbcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      cg->decReferenceCount(secondChild);
               ...
               firstChild->getRegister() ==
                   NULL &&  // TODO: we can still do better in this case
               firstChild->getSecondChild()->getOpCode().isLoadConst() &&
               (value == 0 || (value == (int32_t)firstChild->getSecondChild()
      ...
      //     Xconst k2
      //   Xconst k1
      // where either k1 is 0, or k1 == k2 and tests only a single bit (need to
      // reverse the branch)
      ...
      if (expr->getReferenceCount() == 1 && expr->getRegister() == NULL &&
        ...
        generateRegImmInstruction(TEST1RegImm1, node, targetRegister, value,
                                  cg);
      ...
      if (value == 0) {
        generateRegRegInstruction(TEST1RegReg, node, targetRegister,
                                  targetRegister, cg);
      } else {
...
TR::Register *OMR::X86::TreeEvaluator::ifbcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::ifbucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  generateConditionalJumpInstruction(JB4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifbcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  generateConditionalJumpInstruction(JGE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifbucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  generateConditionalJumpInstruction(JAE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifbcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  generateConditionalJumpInstruction(JG4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifbucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  generateConditionalJumpInstruction(JA4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifbcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  generateConditionalJumpInstruction(JLE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifbucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  generateConditionalJumpInstruction(JBE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifscmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  if (secondChild->getOpCodeValue() == TR::sconst &&
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      if (value >= -128 && value <= 127) {
        if (value == 0) {
          generateRegRegInstruction(TEST2RegReg, node, targetRegister,
                                    targetRegister, cg);
        } else {
          generateRegImmInstruction(CMP2RegImms, node, targetRegister, value,
                                    cg);
        }
      } else {
        /// generateRegImmInstruction(CMP2RegImm2, node, targetRegister, value,
        /// cg);
  ...
  generateConditionalJumpInstruction(
      node->getOpCodeValue() == TR::ifscmpeq ? JE4 : JNE4, node, cg, true);
  return NULL;
}

// ifscmpneEvaluator handled by ifscmpeqEvaluator

TR::Register *OMR::X86::TreeEvaluator::ifscmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::ifscmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateConditionalJumpInstruction(JGE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifscmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateConditionalJumpInstruction(JG4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifscmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateConditionalJumpInstruction(JLE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifsucmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForEquality(node, cg);
  generateConditionalJumpInstruction(
      node->getOpCodeValue() == TR::ifsucmpeq ? JE4 : JNE4, node, cg, true);
  return NULL;
}

TR::Register *OMR::X86::TreeEvaluator::ifsucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateConditionalJumpInstruction(JB4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifsucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateConditionalJumpInstruction(JAE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifsucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateConditionalJumpInstruction(JA4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::ifsucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateConditionalJumpInstruction(JBE4, node, cg, true);
  return NULL;
...
TR::Register *OMR::X86::TreeEvaluator::integerEqualityHelper(
    TR::Node *node, TR_X86OpCodes setOp, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareIntegersForEquality(node, cg);
  TR::Register *targetRegister = cg->allocateRegister();
  generateRegInstruction(setOp, node, targetRegister, cg);

  generateRegRegInstruction(MOVZXReg4Reg1, node, targetRegister, targetRegister,
                            cg);
...
TR::Register *OMR::X86::TreeEvaluator::integerCmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerEqualityHelper(node, SETE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::integerCmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::X86::TreeEvaluator::integerCmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETL1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::integerCmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETGE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::integerCmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETG1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::integerCmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETLE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerCmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETB1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerCmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETAE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerCmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETA1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::unsignedIntegerCmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::integerOrderHelper(node, SETBE1Reg, cg);
}

// also handles bcmpne
TR::Register *OMR::X86::TreeEvaluator::bcmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *targetRegister = cg->allocateRegister();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      generateRegRegInstruction(TEST1RegReg, node, testRegister, testRegister,
                                cg);
    } else {
  ...
  generateRegInstruction(isEq ? SETE1Reg : SETNE1Reg, node, targetRegister, cg);
  generateRegRegInstruction(MOVZXReg4Reg1, node, targetRegister, targetRegister,
                            cg);
  node->setRegister(targetRegister);
  return targetRegister;
...
TR::Register *OMR::X86::TreeEvaluator::bcmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::bcmpEvaluator(node, SETL1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::bcmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::bcmpEvaluator(node, SETGE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::bcmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::bcmpEvaluator(node, SETG1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::bcmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::bcmpEvaluator(node, SETLE1Reg, cg);
}

// also handles scmpne
TR::Register *OMR::X86::TreeEvaluator::scmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *targetRegister = cg->allocateRegister();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value >= -128 && value <= 127) {
      if (value == 0) {
        generateRegRegInstruction(TEST2RegReg, node, testRegister, testRegister,
                                  cg);
      } else {
  ...
  generateRegInstruction(
      node->getOpCodeValue() == TR::scmpeq ? SETE1Reg : SETNE1Reg, node,
      targetRegister, cg);
  generateRegRegInstruction(MOVZXReg4Reg1, node, targetRegister, targetRegister,
                            cg);
  return targetRegister;
...
TR::Register *OMR::X86::TreeEvaluator::scmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETL1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::scmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETGE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::scmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETG1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::scmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETLE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::sucmpeqEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Register *targetRegister = cg->allocateRegister();
  TR::Node *secondChild = node->getSecondChild();
  ...
  generateRegInstruction(
      node->getOpCodeValue() == TR::sucmpeq ? SETE1Reg : SETNE1Reg, node,
      targetRegister, cg);
  generateRegRegInstruction(MOVZXReg4Reg1, node, targetRegister, targetRegister,
                            cg);
  return targetRegister;
...
TR::Register *OMR::X86::TreeEvaluator::sucmpltEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETB1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::sucmpgeEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETAE1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::sucmpgtEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETA1Reg, cg);
}

TR::Register *OMR::X86::TreeEvaluator::sucmpleEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
...
static bool virtualGuardHelper(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (cg->needClassAndMethodPointerRelocations()) {
    ...
    switch (reloKind) {
      ...
      default:
        TR_ASSERT_FATAL(0,
                        "AOT guard in node but not one of known guards "
                        "supported for AOT. Guard: %d",
                        reloKind);
        break;
    }
  ...
  TR::Instruction *vgnopInstr =
      generateVirtualGuardNOPInstruction(node, site, deps, label, cg);
  TR::Instruction *patchPoint = cg->getVirtualGuardForPatching(vgnopInstr);

  // Guards patched when the threads are stopped have no issues with
  // multithreaded patching. therefore alignment is not required
  if (TR::Compiler->target.isSMP() && !node->isStopTheWorldGuard()) {
    // the compiler is now capable of generating a train of vgnops all looking
    // to patch the same point with different constraints. alignment is required
    // before the delegated patch instruction if this collaborative merging is
    // to work we only want one alignment instruction because the instructions
    // always try to move next to their target instruction and we can end up
    // with an infinite loop if two or more dual. for now we know that all patch
    // points have the same alignment requirement so just delegate the alignment
    // generation
...
void OMR::X86::CodeGenerator::addMetaDataForBranchTableAddress(
    uint8_t *target, TR::Node *caseNode,
    TR::X86MemTableInstruction *jmpTableInstruction) {
  self()->addExternalRelocation(
      new (self()->trHeapMemory())
          TR::ExternalRelocation(target, 0, TR_AbsoluteMethodAddress, self()),
      __FILE__, __LINE__, caseNode->getBranchDestination()->getNode());

  TR::LabelSymbol *label =
      caseNode->getBranchDestination()->getNode()->getLabel();
  TR::LabelRelocation *relocation =
      new (self()->trHeapMemory()) TR::LabelAbsoluteRelocation(target, label);
  self()->addRelocation(relocation);

  if (jmpTableInstruction) jmpTableInstruction->addRelocation(relocation);
}
