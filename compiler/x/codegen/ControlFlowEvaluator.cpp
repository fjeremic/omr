...
static inline TR::Instruction* generateWiderCompare(
  TR::Node* node, TR::Register* targetReg, intptrj_t value, TR::CodeGenerator* cg)
{
  ...
  uint32_t highChild, bool& evaluateDefaultGlRegDeps, TR::CodeGenerator* cg)
{
  ...
  if (pivot >= lowChild) {
    ...
    // For the unsigned case, we better use JA if the numbers straddle the TR::getMaxSigned<TR::Int32>() boundary. If
    // that's the case, the highVal will be less than (in the signed sense) than lowVal.
    //
    bool isUnsigned;
...
TR::Register* OMR::X86::TreeEvaluator::lookupEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::RegisterDependencyConditions* deps
    = generateRegisterDependencyConditions((uint8_t)0, TR::RealRegister::MaxAssignableRegisters, cg);
...
TR::Register* OMR::X86::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit() && cg->comp()->compileRelocatableCode()) {
    ...
    jumpMR = generateX86MemoryReference(
      (TR::Register*)NULL, selectorReg, (uint8_t)(TR::Compiler->target.is64Bit() ? 3 : 2), (intptrj_t)branchTable, cg);
      ...
      //
      {
        if (firstChild->getOpCode().isAnd() && (firstChild->getRegister() == NULL)
                    ...
                    // and may be used in a 4 byte compare
          {
            ...
            if (andFirstChild->getRegister() == NULL && andFirstChild->getReferenceCount() == 1
              ...
              if (andFirstChild->getOpCode().isConversion() && andFirstChild->getType().isIntegral()
                && andFirstChild->getFirstChild()->getType().isIntegral() && !andFirstChild->getRegister()
          ...
          if (isConditionCodeSetForCompareToZero(firstChild, true)) {
            ...
            if (compareSize > 2 && (firstChild->getOpCodeValue() == TR::su2i || firstChild->getOpCodeValue() == TR::s2i)
              && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
              compareSize = 2;
              cg->decReferenceCount(firstChild);
              firstChild = firstChild->getFirstChild();
          ...
          if (cg->fe()->isUnloadAssumptionRequired(
                cg->fe()
                  ->createResolvedMethod(
                    cg->trMemory(), (TR_OpaqueMethodBlock*)secondChild->getAddress(), comp->getCurrentMethod())
                  ->classOfMethod(),
  ...
  TR::Node* node, TR::Node* firstChild, TR::Node* secondChild, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
    ...
    temp.integerCompareAnalyser(
      node, firstChild, secondChild, false, CMPRegReg(is64Bit), CMPRegMem(is64Bit), CMPMemReg(is64Bit));
  }
}
...
TR::Register* OMR::X86::TreeEvaluator::iternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!longCompareOn32bit && conditionOp.isCompareForEquality()
    ...
    generateRegRegInstruction((conditionOp.isCompareTrueIfEqual())
        ...
        : ((conditionOp.isCompareTrueIfGreater()) ? CMOVLERegReg(trueValIs64Bit) : CMOVGERegReg(trueValIs64Bit)),
    ...
    if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
      ...
      if (runtimeGuard->getDependencyConditions())
        vgnopInstr = generateVirtualGuardNOPInstruction(
          runtimeGuard->getPrev(), node, site, runtimeGuard->getDependencyConditions()->clone(cg, 0), label, cg);
      else
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::ishr && node->getFirstChild()->getRegister() == NULL
      ...
      if (loadReg) {
        generateRegImmInstruction(
          TEST4RegImm4, node, loadReg, (((int32_t)-1) << node->getFirstChild()->getSecondChild()->getInt()), cg);
      } else {
        TR::MemoryReference* sourceMR = generateX86MemoryReference(loadNode, cg);
        generateMemImmInstruction(
          TEST4MemImm4, node, sourceMR, (((int32_t)-1) << node->getFirstChild()->getSecondChild()->getInt()), cg);
      }
...
TR::Register* OMR::X86::TreeEvaluator::ifbcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
    ...
    if ((firstChild->getReferenceCount() == 1) && (firstChild->getRegister() == NULL)
      ...
      && (value == 0
        || (value == (int32_t)firstChild->getSecondChild()->get64bitIntegralValue() && isPowerOf2(value & 0xff)))) {
      // ifXcmp[eq|ne]
      //   Xand
      //     expr
      //     Xconst k2
...
TR::Register* OMR::X86::TreeEvaluator::ifscmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateConditionalJumpInstruction(node->getOpCodeValue() == TR::ifscmpeq ? JE4 : JNE4, node, cg, true);
  return NULL;
}
...
{
  TR::TreeEvaluator::compareIntegersForEquality(node, cg);
  generateConditionalJumpInstruction(node->getOpCodeValue() == TR::ifsucmpeq ? JE4 : JNE4, node, cg, true);
  return NULL;
}
...
TR::Register* OMR::X86::TreeEvaluator::integerOrderHelper(TR::Node* node, TR_X86OpCodes setOp, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
  node->setRegister(targetRegister);
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register* OMR::X86::TreeEvaluator::bcmpEvaluator(TR::Node* node, TR_X86OpCodes setOp, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
  TR::TreeEvaluator::compareBytesForOrder(node, cg);
  node->setRegister(targetRegister);
...
TR::Register* OMR::X86::TreeEvaluator::scmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateRegInstruction(node->getOpCodeValue() == TR::scmpeq ? SETE1Reg : SETNE1Reg, node, targetRegister, cg);
  generateRegRegInstruction(MOVZXReg4Reg1, node, targetRegister, targetRegister, cg);
  return targetRegister;
}
...
TR::Register* OMR::X86::TreeEvaluator::cmp2BytesEvaluator(TR::Node* node, TR_X86OpCodes setOp, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
  TR::TreeEvaluator::compare2BytesForOrder(node, cg);
  generateRegInstruction(setOp, node, targetRegister, cg);
