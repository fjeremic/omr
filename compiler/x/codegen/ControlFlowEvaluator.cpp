...
// Right now, this is duplicated in ControlFlowEval, BinaryEval and TreeEval.
inline bool
constNodeValueIs32BitSigned(TR::Node* node,
                            intptrj_t* value,
                            TR::CodeGenerator* cg)
{
...
generateWiderCompare(TR::Node* node,
...
bool
isConditionCodeSetForCompareToZero(TR::Node* node, bool justTestZeroFlag)
{
        ...
        if (justTestZeroFlag) {
          if (!prevInstr->getOpCode().setsCCForTest())
            ...
            if (prevInstr->getOpCode().hasByteImmediate()) {
              count =
                ((TR::X86RegImmInstruction*)prevRegInstr)->getSourceImmediate();
            }
...
static uint32_t
sumOf2ConsecutivePowersOf2(uint32_t numberOfCases)
{
...
static void
binarySearchCaseSpace(TR::Register* selectorReg,
                      TR::Node* lookupNode,
                      uint32_t lowChild,
                      uint32_t highChild,
                      ...
                      TR::CodeGenerator* cg)
{
  ...
  if (pivot >= lowChild) {
    ...
    generateLabelInstruction(
      isUnsigned ? JA4 : JG4, lookupNode, pivotLabel, cg);

    if (lowChild == pivot) {
      generateJumpInstruction(JE4, lookupNode->getChild(lowChild), cg);
      generateJumpInstruction(
        JMP4, lookupNode->getChild(1), cg, false, evaluateDefaultGlRegDeps);
      evaluateDefaultGlRegDeps = false;
    } else {
      binarySearchCaseSpace(
        selectorReg, lookupNode, lowChild, pivot, evaluateDefaultGlRegDeps, cg);
    }
  ...
  if (highChild == pivot + 1) {
    ...
    binarySearchCaseSpace(selectorReg,
                          lookupNode,
                          pivot + 1,
                          highChild,
...
TR::Register*
OMR::X86::TreeEvaluator::lookupEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
          ...
          if (globalReg->getKind() == TR_GPR && highGlobalRegNum < 0 &&
            ...
            if (highGlobalRegNum >= 0) {
              ...
              if (!deps->findPostCondition(highRegisterIndex))
                deps->addPostCondition(
                  globalRegPair->getHighOrder(), highRegisterIndex, cg);
            }
          }
        }
      }
...
TR::Register*
OMR::X86::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  int32_t i;
  ...
  if (!canSkipBoundTest) {
    ...
    generateRegImmInstruction(
      opCode, node, selectorReg, numBranchTableEntries, cg);
  ...
  if (TR::Compiler->target.is64Bit() && cg->comp()->compileRelocatableCode()) {
    ...
    generateRegMemInstruction(
      LEA8RegMem, node, branchTableReg, branchTableLeaMR, cg);
  ...
  if (secondChild->getNumChildren() > 0) {
    deps = generateRegisterDependencyConditions(
      secondChild->getFirstChild(), cg, 0, NULL);
    deps->stopAddingConditions();
  }
...
TR::Register*
OMR::X86::TreeEvaluator::minmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
void
OMR::X86::TreeEvaluator::setupProfiledGuardRelocation(
  ...
  TR_ExternalRelocationTargetKind reloKind)
{
  ...
  traceMsg(comp,
           "setupProfiledGuardRelocation: site %p type %d node %p\n",
           site,
           site->getType(),
           node);
#endif
...
void
OMR::X86::TreeEvaluator::compareIntegersForEquality(TR::Node* node,
                                                    TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (constValue >= -128 && constValue <= 127) {
      ...
      //
      {
        if (firstChild->getOpCode().isAnd() &&
                         ...
                         // and may be used in a 4 byte compare
          {
            ...
            if (andFirstChild->getRegister() == NULL &&
              ...
              if (((mask >> 8) == 0) || (andSecondChild->getSize() == 1))
                ...
                generateMemRegInstruction(
                  TEST2MemReg, node, tempMR, tempReg, cg);
                cg->stopUsingRegister(tempReg);
              } else
                generateMemImmInstruction(
                  TESTMemImm4(is64Bit), node, tempMR, mask, cg);
            ...
            temp.genericAnalyser(
              firstChild, testRRInstr, testMRInstr, movRRInstr, true);
          }
        } else {
          ...
          if (isConditionCodeSetForCompareToZero(firstChild, true)) {
            ...
            if (firstChild->getOpCode().isMemoryReference() &&
              ...
              if (compareSize == 1)
                ...
                generateMemRegInstruction(
                  CMP2MemReg, node, tempMR, tempReg, cg);
                cg->stopUsingRegister(tempReg);
              } else
              ...
              if (isConditionCodeSetForCompareToZero(firstChild, true)) {
                ...
                if (compareSize == 1)
                  generateRegRegInstruction(
                    TEST1RegReg, node, firstChildReg, firstChildReg, cg);
                else if (compareSize == 2)
                  generateRegRegInstruction(
                    TEST2RegReg, node, firstChildReg, firstChildReg, cg);
                else
        ...
        if (firstChild->getOpCode().isMemoryReference() &&
          ...
          if (compareSize == 1)
            generateMemImmInstruction(
              CMP1MemImm1, node, tempMR, constValue, cg);
          else if (compareSize == 2) {
            // shouldn't use Imm2 instructions
            TR::Register* tempReg = cg->allocateRegister();
            TR::TreeEvaluator::loadConstant(
    ...
    if (node->isProfiledGuard() && cg->profiledPointersRequireRelocation()) {
      ...
      if (secondChild->getOpCodeValue() == TR::aconst) {
        if (secondChild->isClassPointerConstant())
          TR::TreeEvaluator::setupProfiledGuardRelocation(
            NULL, node, TR_ClassPointer);
        else if (secondChild->isMethodPointerConstant())
          TR::TreeEvaluator::setupProfiledGuardRelocation(
            NULL, node, TR_MethodPointer);
      }
    }
  }
}
...
void
OMR::X86::TreeEvaluator::compareIntegersForOrder(TR::Node* node,
                                                 TR::Node* firstChild,
                                                 TR::Node* secondChild,
                                                 TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    bool is64Bit = TR::TreeEvaluator::getNodeIs64Bit(secondChild, cg);
    TR_X86CompareAnalyser temp(cg);
    temp.integerCompareAnalyser(node,
                                firstChild,
                                secondChild,
                                false,
                                CMPRegReg(is64Bit),
...
OMR::X86::TreeEvaluator::compareIntegersForOrder(TR::Node* node,
                                                 TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareIntegersForOrder(
    node, node->getFirstChild(), node->getSecondChild(), cg);
}

void
OMR::X86::TreeEvaluator::compare2BytesForOrder(TR::Node* node,
                                               TR::CodeGenerator* cg)
{
  TR::Node* secondChild = node->getSecondChild();
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      if (isByteValue)
        ...
        TR::Register* tempReg = cg->allocateRegister();
        TR::TreeEvaluator::loadConstant(
          node, value, TR_RematerializableShort, cg, tempReg);
        generateMemRegInstruction(CMP2MemReg, node, tempMR, tempReg, cg);
        cg->stopUsingRegister(tempReg);
      }
      tempMR->decNodeReferenceCounts(cg);
...
void
OMR::X86::TreeEvaluator::compareBytesForOrder(TR::Node* node,
                                              TR::CodeGenerator* cg)
{
  TR::Node* secondChild = node->getSecondChild();
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      TR::MemoryReference* tempMR = generateX86MemoryReference(firstChild, cg);
      generateMemImmInstruction(
        CMP1MemImm1, firstChild, tempMR, secondChild->getByte(), cg);
      tempMR->decNodeReferenceCounts(cg);
    } else {
      generateRegImmInstruction(CMP1RegImm1,
                                node,
                                cg->evaluate(firstChild),
                                secondChild->getByte(),
                                cg);
...
TR::Register*
OMR::X86::TreeEvaluator::gotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::igotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::returnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (!longCompareOn32bit && conditionOp.isCompareForEquality() &&
    ...
    if (conditionOp.isCompareTrueIfEqual())
      generateRegRegInstruction(
        CMOVNERegReg(trueValIs64Bit), node, trueReg, falseReg, cg);
    else
      generateRegRegInstruction(
        CMOVERegReg(trueValIs64Bit), node, trueReg, falseReg, cg);
    ...
    generateRegRegInstruction(
      TEST4RegReg, node, condReg, condReg, cg); // condition is always an int
    generateRegRegInstruction(
      CMOVERegReg(trueValIs64Bit), node, trueReg, falseReg, cg);
  }
...
static inline void
generateMergedGuardCodeIfNeeded(TR::Node* node,
                                TR::CodeGenerator* cg,
    ...
    if (virtualGuard && (virtualGuard->mergedWithOSRGuard() ||
      ...
      if (runtimeGuard->getDependencyConditions())
        vgnopInstr = generateVirtualGuardNOPInstruction(
          runtimeGuard->getPrev(),
          node,
          site,
          runtimeGuard->getDependencyConditions()->clone(cg, 0),
          label,
          cg);
      else
        vgnopInstr = generateVirtualGuardNOPInstruction(
  ...
  if (virtualGuardHelper(node, cg)) {
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::ishr &&
        ...
        performTransformation(
          comp,
          "O^O SHIFT PEEPHOLE: detected shift pattern for node %p shifting so "
          "mask = %p shift amount = %d \n",
          node,
          (((int32_t)-1) << node->getFirstChild()->getSecondChild()->getInt()),
...
bool
OMR::X86::TreeEvaluator::generateIAddOrSubForOverflowCheck(
  TR::Node* compareNode,
  TR::CodeGenerator* cg)
{
  ...
  if (matches &&
    ...
    bool leftNeedsCopy =
      u.leftChild->getReferenceCount() > 2 ||
...
bool
OMR::X86::TreeEvaluator::generateLAddOrSubForOverflowCheck(
  TR::Node* compareNode,
  TR::CodeGenerator* cg)
{
  TR_ArithmeticOverflowCheckNodes u = { 0 };
  ...
  if (matches &&
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      generateRegRegInstruction(ADD4RegReg,
                                u.operationNode,
                                leftReg->getLowOrder(),
                                rightReg->getLowOrder(),
                                cg);
      generateRegRegInstruction(ADC4RegReg,
                                u.operationNode,
                                leftReg->getHighOrder(),
                                rightReg->getHighOrder(),
                                cg);
    } else {
      generateRegRegInstruction(SUB4RegReg,
                                u.operationNode,
                                leftReg->getLowOrder(),
                                rightReg->getLowOrder(),
                                cg);
      generateRegRegInstruction(SBB4RegReg,
                                u.operationNode,
                                leftReg->getHighOrder(),
                                rightReg->getHighOrder(),
                                cg);
...
TR::Register*
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpltEvaluator(TR::Node* node,
                                                         TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register*
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpgeEvaluator(TR::Node* node,
                                                         TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register*
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpgtEvaluator(TR::Node* node,
                                                         TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
...
TR::Register*
OMR::X86::TreeEvaluator::unsignedIntegerIfCmpleEvaluator(TR::Node* node,
                                                         TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareIntegersForOrder(node, cg);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      if (expr->getReferenceCount() == 1 && expr->getRegister() == NULL &&
        ...
        generateRegImmInstruction(
          TEST1RegImm1, node, targetRegister, value, cg);
      ...
      if (value == 0) {
        generateRegRegInstruction(
          TEST1RegReg, node, targetRegister, targetRegister, cg);
      } else {
  ...
  if (secondChild->getOpCodeValue() == TR::sconst &&
    ...
    if ((firstChild->getReferenceCount() == 1) &&
      ...
      if (value >= -128 && value <= 127) {
        if (value == 0) {
          generateRegRegInstruction(
            TEST2RegReg, node, targetRegister, targetRegister, cg);
        } else {
          generateRegImmInstruction(
            CMP2RegImms, node, targetRegister, value, cg);
        }
      } else {
...
TR::Register*
OMR::X86::TreeEvaluator::integerEqualityHelper(TR::Node* node,
                                               TR_X86OpCodes setOp,
                                               TR::CodeGenerator* cg)
{
  TR::TreeEvaluator::compareIntegersForEquality(node, cg);
  ...
  generateRegRegInstruction(
    MOVZXReg4Reg1, node, targetRegister, targetRegister, cg);
...
TR::Register*
OMR::X86::TreeEvaluator::integerOrderHelper(TR::Node* node,
                                            TR_X86OpCodes setOp,
                                            TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
...
// also handles bcmpne
TR::Register*
OMR::X86::TreeEvaluator::bcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      generateRegRegInstruction(
        TEST1RegReg, node, testRegister, testRegister, cg);
    } else {
...
TR::Register*
OMR::X86::TreeEvaluator::bcmpEvaluator(TR::Node* node,
                                       TR_X86OpCodes setOp,
                                       TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
...
TR::Register*
OMR::X86::TreeEvaluator::bcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::bcmpEvaluator(node, SETL1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::bcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::bcmpEvaluator(node, SETGE1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::bcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::bcmpEvaluator(node, SETG1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::bcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
// also handles scmpne
TR::Register*
OMR::X86::TreeEvaluator::scmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value >= -128 && value <= 127) {
      if (value == 0) {
        generateRegRegInstruction(
          TEST2RegReg, node, testRegister, testRegister, cg);
      } else {
...
TR::Register*
OMR::X86::TreeEvaluator::cmp2BytesEvaluator(TR::Node* node,
                                            TR_X86OpCodes setOp,
                                            TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
...
TR::Register*
OMR::X86::TreeEvaluator::scmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETL1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::scmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETGE1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::scmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETG1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::scmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETLE1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::sucmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
  TR::Node* secondChild = node->getSecondChild();
...
TR::Register*
OMR::X86::TreeEvaluator::sucmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETB1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::sucmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETAE1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::sucmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::cmp2BytesEvaluator(node, SETA1Reg, cg);
}

TR::Register*
OMR::X86::TreeEvaluator::sucmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
