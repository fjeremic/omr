...
public:
  X86FPConversionSnippet(TR::CodeGenerator* codeGen, TR::Node* node, TR::LabelSymbol* restartlab,
  ...
  TR::SymbolReference* getHelperSymRef()
  {
    return _helperSymRef;
  }
  TR::SymbolReference* setHelperSymRef(TR::SymbolReference* s)
  {
    return (_helperSymRef = s);
  }
...
public:
  X86FPConvertToIntSnippet(TR::LabelSymbol* restartlab, TR::LabelSymbol* snippetlab, TR::SymbolReference* helperSymRef,
    TR::X86RegInstruction* convertInstr, TR::CodeGenerator* codeGen)
  ...
  TR::X86RegInstruction* getConvertInstruction()
  {
    return _convertInstruction;
  }
  uint8_t* genFPConversion(uint8_t* buffer);
  virtual uint32_t getLength(int32_t estimatedSnippetStart);
  virtual Kind getKind()
  {
    return IsFPConvertToInt;
  }
};
...
public:
  ...
  X86FPConvertToLongSnippet(TR::LabelSymbol* restartlab, TR::LabelSymbol* snippetlab, TR::SymbolReference* helperSymRef,
    TR::X86FPST0STiRegRegInstruction* clobInstr, TR::X86RegMemInstruction* loadHighInstr,
    TR::X86RegMemInstruction* loadLowInstr, TR::CodeGenerator* codeGen)
  ...
  TR::RealRegister* getLowRegister()
  {
    return _lowRegister;
  }
  TR::RealRegister* getHighRegister()
  {
    return _highRegister;
  }
  TR::RealRegister* getDoubleRegister()
  {
    return _doubleRegister;
  }
