...
extern int32_t memoryBarrierRequired(TR_X86OpCode &op, TR::MemoryReference *mr,
                                     TR::CodeGenerator *cg,
                                     bool onlyAskingAboutFences);
extern int32_t estimateMemoryBarrierBinaryLength(int32_t barrier,
                                                 TR::CodeGenerator *cg);
extern void padUnresolvedReferenceInstruction(TR::Instruction *instr,
                                              TR::MemoryReference *mr,
                                              TR::CodeGenerator *cg);
extern void insertUnresolvedReferenceInstructionMemoryBarrier(
    TR::CodeGenerator *cg, int32_t barrier, TR::Instruction *inst,
    TR::MemoryReference *mr, TR::Register *srcReg = NULL,
    TR::MemoryReference *anotherMr = NULL);
...
public:
  X86PaddingInstruction(uint8_t length, TR::Node *node, TR::CodeGenerator *cg)
      : TR::Instruction(node, BADIA32Op, cg), _length(length),
        _properties(TR_NoOpPadding) {}

  X86PaddingInstruction(uint8_t length, TR_PaddingProperties properties,
                        TR::Node *node, TR::CodeGenerator *cg)
      : TR::Instruction(node, BADIA32Op, cg), _length(length),
        _properties(properties) {}

  X86PaddingInstruction(TR::Instruction *precedingInstruction, uint8_t length,
                        TR::CodeGenerator *cg)
      : TR::Instruction(BADIA32Op, precedingInstruction, cg), _length(length),
        _properties(TR_NoOpPadding) {}
...
public:
  X86PaddingSnippetInstruction(uint8_t length, TR::Node *node,
                               TR::CodeGenerator *cg)
      : TR::X86PaddingInstruction(length, node, cg), _unresolvedSnippet(NULL) {}

  X86PaddingSnippetInstruction(uint8_t length, TR_PaddingProperties properties,
                               TR::Node *node, TR::CodeGenerator *cg)
      : TR::X86PaddingInstruction(length, properties, node, cg),
        _unresolvedSnippet(NULL) {}

  X86PaddingSnippetInstruction(TR::Instruction *precedingInstruction,
                               uint8_t length, TR::CodeGenerator *cg)
      : TR::X86PaddingInstruction(precedingInstruction, length, cg),
        _unresolvedSnippet(NULL) {}

  X86PaddingSnippetInstruction(TR::Instruction *precedingInstruction,
                               uint8_t length, TR_PaddingProperties properties,
                               TR::CodeGenerator *cg)
...
public:
  X86BoundaryAvoidanceInstruction(const TR_AtomicRegion *atomicRegions,
                                  uint8_t boundarySpacing, uint8_t maxPadding,
                                  TR::Instruction *targetCode,
                                  TR::CodeGenerator *cg)
      : TR::Instruction(BADIA32Op, targetCode->getPrev(), cg),
        _sizeOfProtectiveNop(0), _atomicRegions(atomicRegions),
        _boundarySpacing(boundarySpacing), _maxPadding(maxPadding),
        _targetCode(targetCode), _minPaddingLength(0) {
  ...
  X86BoundaryAvoidanceInstruction(int32_t sizeOfProtectiveNop,
      ...
      : TR::Instruction(BADIA32Op, targetCode->getPrev(), cg),
        ...
        _atomicRegions(atomicRegions), _boundarySpacing(boundarySpacing),
        _maxPadding(maxPadding), _targetCode(targetCode), _minPaddingLength(0) {
    setNode(targetCode->getNode());
  }
  ...
  X86BoundaryAvoidanceInstruction(TR::Instruction *precedingInstruction,
      ...
      : TR::Instruction(BADIA32Op, precedingInstruction, cg),
        _atomicRegions(atomicRegions), _boundarySpacing(boundarySpacing),
        _maxPadding(maxPadding), _targetCode(NULL), _sizeOfProtectiveNop(0),
        _minPaddingLength(0) {
  ...
  virtual int32_t estimateBinaryLength(int32_t currentEstimate);
  virtual uint8_t *generateBinaryEncoding();
  virtual OMR::X86::EnlargementResult enlarge(int32_t requestedEnlargementSize,
  ...
  static TR_AtomicRegion
      unresolvedAtomicRegions[]; // For patching unresolved references

protected:
  virtual int32_t betterPadLength(int32_t oldPadLength,
...
class X86PatchableCodeAlignmentInstruction
    : public TR::X86BoundaryAvoidanceInstruction {

public:
  // Note: we use cg->getInstructionPatchAlignmentBoundary() as the max padding,
  // even though that is 1 more byte than we should ever actually need.  The
  // reason is that we don't want the inherited logic to stop silently when it
  // reaches this limit; rather, we want it to try to add more padding, and trip
  // on an assertion inside the betterPadLength function so that we are alerted
  // to the problem.
  ...
  X86PatchableCodeAlignmentInstruction(const TR_AtomicRegion *atomicRegions,
      ...
      : TR::X86BoundaryAvoidanceInstruction(
            atomicRegions, cg->getInstructionPatchAlignmentBoundary(),
  ...
  X86PatchableCodeAlignmentInstruction(const TR_AtomicRegion *atomicRegions,
                                       ...
                                       TR::CodeGenerator *cg)
      : TR::X86BoundaryAvoidanceInstruction(
            sizeOfProtectiveNop, atomicRegions,
            cg->getInstructionPatchAlignmentBoundary(),
            cg->getInstructionPatchAlignmentBoundary(), patchableCode, cg) {}
  ...
  X86PatchableCodeAlignmentInstruction(TR::Instruction *precedingInstruction,
                                       ...
                                       TR::CodeGenerator *cg)
      : TR::X86BoundaryAvoidanceInstruction(
            precedingInstruction, atomicRegions,
            cg->getInstructionPatchAlignmentBoundary(),
            cg->getInstructionPatchAlignmentBoundary(), cg) {
  ...
  static TR_AtomicRegion
      spinLoopAtomicRegions[]; // For any patching done using a 2-byte self-loop
  static TR_AtomicRegion
      CALLImm4AtomicRegions[]; // For patching the displacement of a 5-byte call
                               // instruction
};
...
public:
  X86LabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
                      TR::CodeGenerator *cg, bool b = false);
  X86LabelInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
  ...
  virtual bool isPatchBarrier() {
    return getOpCodeValue() == LABEL && _symbol &&
           _symbol->isTargeted() != TR_no;
  }
  ...
  void assignOutlinedInstructions(TR_RegisterKinds kindsToBeAssigned,
...
public:
  ...
  X86AlignmentInstruction(TR::Node *node, uint8_t boundary,
                          TR::CodeGenerator *cg)
      : TR::Instruction(node, BADIA32Op, cg), _boundary(boundary), _margin(0),
        _minPaddingLength(0) {}

  X86AlignmentInstruction(TR::Node *node, uint8_t boundary,
                          TR::RegisterDependencyConditions *cond,
                          TR::CodeGenerator *cg)
      : TR::Instruction(cond, node, BADIA32Op, cg), _boundary(boundary),
        _margin(0), _minPaddingLength(0) {}

  X86AlignmentInstruction(TR::Node *node, uint8_t boundary, uint8_t margin,
                          TR::CodeGenerator *cg)
      : TR::Instruction(node, BADIA32Op, cg), _boundary(boundary),
        _margin(margin), _minPaddingLength(0) {}
  ...
  X86AlignmentInstruction(TR::Instruction *precedingInstruction,
                          uint8_t boundary, TR::CodeGenerator *cg)
      : TR::Instruction(BADIA32Op, precedingInstruction, cg),
        _boundary(boundary), _margin(0), _minPaddingLength(0) {}
  ...
  X86AlignmentInstruction(TR::Instruction *precedingInstruction,
                          ...
                          TR::CodeGenerator *cg)
      : TR::Instruction(BADIA32Op, precedingInstruction, cg),
        _boundary(boundary), _margin(margin), _minPaddingLength(0) {}
...
class X86FenceInstruction : public TR::Instruction {
  TR::Node
      *_fenceNode; // todo: replace uses of this with TR::Instruction::_node

public:
  X86FenceInstruction(TR_X86OpCodes op, TR::Node *, TR::Node *n,
                      TR::CodeGenerator *cg);

  X86FenceInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                      TR::Node *n, TR::CodeGenerator *cg);
...
public:
  X86VirtualGuardNOPInstruction(TR_X86OpCodes op, TR::Node *node,
                                TR_VirtualGuardSite *site,
                                TR::RegisterDependencyConditions *cond,
                                TR::CodeGenerator *cg,
                                TR::LabelSymbol *label = 0)
      : TR::X86LabelInstruction(op, node, label, cond, cg), _site(site),
        _nopSize(0), _register(TR::RealRegister::NoReg) {}

  X86VirtualGuardNOPInstruction(TR::Instruction *precedingInstruction,
                                TR_X86OpCodes op, TR::Node *node,
                                TR_VirtualGuardSite *site,
                                TR::RegisterDependencyConditions *cond,
                                TR::CodeGenerator *cg,
                                TR::LabelSymbol *label = 0)
      : TR::X86LabelInstruction(precedingInstruction, op, label, cond, cg),
        _site(site), _nopSize(0), _register(TR::RealRegister::NoReg) {
    setNode(node);
  }
...
class X86ImmInstruction : public TR::Instruction {
  ...
  int32_t _adjustsFramePointerBy; // TODO: Rename me.  Calls don't adjust the
                                  // VFP per se; they adjust the stack pointer
  int32_t _reloKind;

public:
  X86ImmInstruction(TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg,
                    int32_t reloKind = TR_NoRelocation)
      : TR::Instruction(node, op, cg), _sourceImmediate(0),
        _adjustsFramePointerBy(0), _reloKind(reloKind) {}

  X86ImmInstruction(int32_t imm, TR::Node *node, TR_X86OpCodes op,
                    TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation)
      : TR::Instruction(node, op, cg), _sourceImmediate(imm),
        _adjustsFramePointerBy(0), _reloKind(reloKind) {}

  X86ImmInstruction(int32_t imm, TR_X86OpCodes op,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation)
      : TR::Instruction(op, precedingInstruction, cg), _sourceImmediate(imm),
        _adjustsFramePointerBy(0), _reloKind(reloKind) {}

  X86ImmInstruction(TR::RegisterDependencyConditions *cond, int32_t imm,
  ...
  X86ImmInstruction(TR::RegisterDependencyConditions *cond, int32_t imm,
                    TR_X86OpCodes op, TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation)
  ...
  X86ImmInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm,
                    TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation);

  X86ImmInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
...
public:
  X86ImmSnippetInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm,
...
public:
  X86ImmSymInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm,
                       TR::SymbolReference *sr, TR::CodeGenerator *cg);

  X86ImmSymInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                       int32_t imm, TR::SymbolReference *sr,
                       TR::CodeGenerator *cg);
...
public:
  X86RegInstruction(TR::Register *reg, TR::Node *node, TR_X86OpCodes op,
                    TR::CodeGenerator *cg)
    ...
    if (cg->enableRematerialisation() && reg->isDiscardable() &&
      ...
      if (debug("dumpRemat")) {
        diagnostic(
            ...
            reg->getRematerializationInfo()->toString(comp),
            reg->getRegisterName(comp), this);
      }
    }
  }

  X86RegInstruction(TR::Register *reg, TR_X86OpCodes op,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
  ...
  X86RegInstruction(TR::RegisterDependencyConditions *cond, TR::Register *reg,
                    TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg)
  ...
  X86RegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg,
                    TR::CodeGenerator *cg);

  X86RegInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
  ...
  void applyTargetRegisterToModRMByte(uint8_t *modRM) {
    ...
    if (getOpCode().hasTargetRegisterInModRM()) {
      ...
      // If not in RM field and not in opcode, then must have a reg field in
      // ModRM byte
      //
  ...
  uint8_t targetRegisterRexBits() {
    ...
    if (getOpCode().hasTargetRegisterInModRM())
      ...
      rxbBitmask = TR::RealRegister::REX_B;
    else // If not in RM field and not in opcode, then must have a reg field in
         // ModRM byte
      rxbBitmask = TR::RealRegister::REX_R;
    // Add the appropriate bits to the Rex prefix byte
    //
    TR::RealRegister *target = toRealRegister(_targetRegister);
    return target->rexBits(rxbBitmask,
                           getOpCode().hasByteTarget() ? true : false);
  }
#endif
  virtual uint8_t *generateOperand(uint8_t *cursor);
  virtual int32_t estimateBinaryLength(int32_t currentEstimate);
...
public:
  X86RegRegInstruction(TR::Register *sreg, TR::Register *treg, TR::Node *node,
                       TR_X86OpCodes op, TR::CodeGenerator *cg)
      : TR::X86RegInstruction(treg, node, op, cg), _sourceRegister(sreg) {
    useRegister(sreg);
  }

  X86RegRegInstruction(TR::Register *sreg, TR::Register *treg, TR_X86OpCodes op,
                       TR::Instruction *precedingInstruction,
                       TR::CodeGenerator *cg)
  ...
  X86RegRegInstruction(TR::RegisterDependencyConditions *cond,
                       TR::Register *sreg, TR::Register *treg, TR::Node *node,
                       TR_X86OpCodes op, TR::CodeGenerator *cg)
  ...
  X86RegRegInstruction(TR::RegisterDependencyConditions *cond,
                       TR::Register *sreg, TR::Register *treg, TR_X86OpCodes op,
                       TR::Instruction *precedingInstruction,
                       TR::CodeGenerator *cg)
  ...
  X86RegRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                       TR::Register *sreg, TR::CodeGenerator *cg);

  X86RegRegInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
  ...
  uint8_t sourceRegisterRexBits() {
    ...
    //
    TR::RealRegister *source = toRealRegister(_sourceRegister);
    return source->rexBits(rxbBitmask,
                           getOpCode().hasByteSource() ? true : false);
  }
#endif
...
public:
  X86RegImmInstruction(int32_t imm, TR::Register *treg, TR::Node *node,
                       TR_X86OpCodes op, TR::CodeGenerator *cg,
                       int32_t reloKind = TR_NoRelocation)
      : TR::X86RegInstruction(treg, node, op, cg), _sourceImmediate(imm),
        _reloKind(reloKind) {}

  X86RegImmInstruction(int32_t imm, TR::Register *treg, TR_X86OpCodes op,
                       TR::Instruction *precedingInstruction,
                       TR::CodeGenerator *cg,
                       int32_t reloKind = TR_NoRelocation)
      : TR::X86RegInstruction(treg, op, precedingInstruction, cg),
  ...
  X86RegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                       int32_t imm, TR::CodeGenerator *cg,
                       int32_t reloKind = TR_NoRelocation);

  X86RegImmInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
...
public:
  X86RegImmSymInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg,
                          int32_t imm, TR::SymbolReference *sr,
                          TR::CodeGenerator *cg);

  X86RegImmSymInstruction(TR::Instruction *precedingInstruction,
                          TR_X86OpCodes op, TR::Register *reg, int32_t imm,
                          TR::SymbolReference *sr, TR::CodeGenerator *cg);
...
public:
  X86RegRegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                          TR::Register *sreg, int32_t imm,
                          TR::CodeGenerator *cg);

  X86RegRegImmInstruction(TR::Instruction *precedingInstruction,
                          TR_X86OpCodes op, TR::Register *treg,
                          TR::Register *sreg, int32_t imm,
                          TR::CodeGenerator *cg);
...
public:
  X86RegRegRegInstruction(TR::RegisterDependencyConditions *cond,
                          TR::Register *srreg, TR::Register *slreg,
                          TR::Register *treg, TR::Node *node, TR_X86OpCodes op,
                          TR::CodeGenerator *cg)
  ...
  X86RegRegRegInstruction(TR::RegisterDependencyConditions *cond,
                          TR::Register *srreg, TR::Register *slreg,
                          TR::Register *treg, TR_X86OpCodes op,
                          TR::Instruction *precedingInstruction,
                          TR::CodeGenerator *cg)
      : TR::X86RegRegInstruction(cond, srreg, treg, op, precedingInstruction,
                                 cg),
        _source2ndRegister(slreg) {
    useRegister(slreg);
  }

  X86RegRegRegInstruction(TR::Register *srreg, TR::Register *slreg,
                          TR::Register *treg, TR::Node *node, TR_X86OpCodes op,
                          TR::CodeGenerator *cg)
  ...
  X86RegRegRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
...
public:
  X86MemInstruction(TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg,
                    TR::Register *srcReg = NULL)
      : TR::Instruction(node, op, cg), _memoryReference(NULL) {}

  X86MemInstruction(TR::MemoryReference *mr, TR::Node *node, TR_X86OpCodes op,
    ...
    // Find out if this instruction clobbers the memory reference associated
    // with a live discardable register.
    //
  ...
  X86MemInstruction(TR::MemoryReference *mr, TR_X86OpCodes op,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg, TR::Register *srcReg = NULL)
  ...
  X86MemInstruction(TR::RegisterDependencyConditions *cond,
                    TR::MemoryReference *mr, TR::Node *node, TR_X86OpCodes op,
                    TR::CodeGenerator *cg, TR::Register *srcReg = NULL)
    ...
    if (!cg->comp()->getOption(TR_DisableNewX86VolatileSupport)) {
      ...
      if (barrier)
        insertUnresolvedReferenceInstructionMemoryBarrier(cg, barrier, this,
                                                          mr);
    }
  ...
  X86MemInstruction(TR::RegisterDependencyConditions *cond,
                    TR::MemoryReference *mr, TR_X86OpCodes op,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg, TR::Register *srcReg = NULL)
      : TR::Instruction(cond, op, precedingInstruction, cg),
        _memoryReference(mr) {
  ...
  X86MemInstruction(TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
                    TR::CodeGenerator *cg, TR::Register *sreg = NULL);

  X86MemInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
...
public:
  X86MemTableInstruction(TR_X86OpCodes op, TR::Node *node,
                         TR::MemoryReference *mr, ncount_t numEntries,
                         TR::CodeGenerator *cg)
      : TR::X86MemInstruction(op, node, mr, cg), _numRelocations(0),
        _capacity(numEntries) {
    _relocations = (TR::LabelRelocation **)cg->trMemory()->allocateHeapMemory(
        numEntries * sizeof(_relocations[0]));
  }
  ...
  X86MemTableInstruction(TR_X86OpCodes op, TR::Node *node,
                         ...
                         TR::CodeGenerator *cg)
      : TR::X86MemInstruction(op, node, mr, deps, cg), _numRelocations(0),
        _capacity(numEntries) {
    _relocations = (TR::LabelRelocation **)cg->trMemory()->allocateHeapMemory(
        numEntries * sizeof(_relocations[0]));
  }
  ...
  void addRelocation(TR::LabelRelocation *r) {
    TR_ASSERT(_numRelocations < _capacity,
              "Can't add another relocation to a X86MemTableInstruction that "
              "is already full");
  ...
  TR::LabelRelocation *getRelocation(ncount_t index) {
    TR_ASSERT(0 <= index && index < _numRelocations,
              "X86MemTableInstruction::getRelocation - index %d is out of "
              "range (0-%d)",
              index, _numRelocations);
...
class X86CallMemInstruction : public TR::X86MemInstruction {
  int32_t _adjustsFramePointerBy; // TODO: Rename me.  Calls don't affect the
                                  // VFP per se; they affect the stack pointer

public:
  // TODO: roll this entire class into the TR::X86MemInstruction  class.
  //
  X86CallMemInstruction(TR_X86OpCodes op, TR::Node *node,
                        TR::MemoryReference *mr,
                        TR::RegisterDependencyConditions *cond,
                        TR::CodeGenerator *cg);

  X86CallMemInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                        TR::MemoryReference *mr,
                        TR::RegisterDependencyConditions *cond,
                        TR::CodeGenerator *cg);
...
public:
  X86MemImmInstruction(int32_t imm, TR::MemoryReference *mr, TR::Node *node,
                       TR_X86OpCodes op, TR::CodeGenerator *cg,
                       int32_t reloKind = TR_NoRelocation)
      : TR::X86MemInstruction(mr, node, op, cg), _sourceImmediate(imm),
        _reloKind(reloKind) {}

  X86MemImmInstruction(int32_t imm, TR::MemoryReference *mr, TR_X86OpCodes op,
                       TR::Instruction *precedingInstruction,
                       TR::CodeGenerator *cg,
                       int32_t reloKind = TR_NoRelocation)
  ...
  X86MemImmInstruction(TR_X86OpCodes op, TR::Node *node,
                       TR::MemoryReference *mr, int32_t imm,
                       TR::CodeGenerator *cg,
                       int32_t reloKind = TR_NoRelocation);

  X86MemImmInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                       TR::MemoryReference *mr, int32_t imm,
                       TR::CodeGenerator *cg,
                       int32_t reloKind = TR_NoRelocation);
...
public:
  X86MemImmSymInstruction(TR_X86OpCodes op, TR::Node *node,
                          TR::MemoryReference *mr, int32_t imm,
                          TR::SymbolReference *sr, TR::CodeGenerator *cg);
...
public:
  X86MemRegInstruction(TR::Register *sreg, TR::MemoryReference *mr,
                       TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg)
  ...
  X86MemRegInstruction(TR::RegisterDependencyConditions *cond,
                       TR::Register *sreg, TR::MemoryReference *mr,
                       TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg)
  ...
  X86MemRegInstruction(TR_X86OpCodes op, TR::Node *node,
                       TR::MemoryReference *mr, TR::Register *sreg,
                       TR::CodeGenerator *cg);

  X86MemRegInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                       TR::MemoryReference *mr, TR::Register *sreg,
                       TR::CodeGenerator *cg);
...
public:
  X86MemRegImmInstruction(TR_X86OpCodes op, TR::Node *node,
                          TR::MemoryReference *mr, TR::Register *sreg,
                          int32_t imm, TR::CodeGenerator *cg);
...
public:
  X86RegMemInstruction(TR::MemoryReference *mr, TR::Register *treg,
                       TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg)
  ...
  X86RegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                       TR::MemoryReference *mr, TR::CodeGenerator *cg);

  X86RegMemInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                       TR::Register *treg, TR::MemoryReference *mr,
                       TR::CodeGenerator *cg);
...
public:
  X86RegMemImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                          TR::MemoryReference *mr, int32_t imm,
                          TR::CodeGenerator *cg);
...
public:
  X86RegRegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *slreg,
                          TR::Register *srreg, TR::MemoryReference *mr,
                          TR::CodeGenerator *cg);
  X86RegRegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *slreg,
                          TR::Register *srreg, TR::MemoryReference *mr,
                          TR::RegisterDependencyConditions *cond,
...
public:
  X86FPRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *reg,
                      TR::CodeGenerator *cg);

  X86FPRegInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                      TR::Register *reg, TR::CodeGenerator *cg);
...
public:
  AMD64RegImm64Instruction(uint64_t imm, TR::Register *treg, TR::Node *node,
                           TR_X86OpCodes op, TR::CodeGenerator *cg,
                           int32_t reloKind = TR_NoRelocation)
      : TR::X86RegInstruction(treg, node, op, cg), _sourceImmediate(imm),
        _reloKind(reloKind) {}

  AMD64RegImm64Instruction(uint64_t imm, TR::Register *treg, TR_X86OpCodes op,
                           TR::Instruction *precedingInstruction,
                           TR::CodeGenerator *cg,
                           int32_t reloKind = TR_NoRelocation)
      : TR::X86RegInstruction(treg, op, precedingInstruction, cg),
  ...
  AMD64RegImm64Instruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
  ...
  AMD64RegImm64Instruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                           uint64_t imm, TR::RegisterDependencyConditions *cond,
                           TR::CodeGenerator *cg,
                           int32_t reloKind = TR_NoRelocation)
...
public:
  AMD64RegImm64SymInstruction(TR_X86OpCodes op, TR::Node *node,
                              TR::Register *reg, uint64_t imm,
                              TR::SymbolReference *sr, TR::CodeGenerator *cg);

  AMD64RegImm64SymInstruction(TR::Instruction *precedingInstruction,
                              TR_X86OpCodes op, TR::Register *reg, uint64_t imm,
                              TR::SymbolReference *sr, TR::CodeGenerator *cg);
...
public:
  AMD64Imm64Instruction(uint64_t imm, TR::Node *node, TR_X86OpCodes op,
                        TR::CodeGenerator *cg)
      : TR::Instruction(node, op, cg), _sourceImmediate(imm) {}

  AMD64Imm64Instruction(uint64_t imm, TR_X86OpCodes op,
                        TR::Instruction *precedingInstruction,
                        TR::CodeGenerator *cg)
      : TR::Instruction(op, precedingInstruction, cg), _sourceImmediate(imm) {}

  AMD64Imm64Instruction(TR::RegisterDependencyConditions *cond, uint64_t imm,
                        TR::Node *node, TR_X86OpCodes op, TR::CodeGenerator *cg)
      : TR::Instruction(cond, node, op, cg), _sourceImmediate(imm) {}
  ...
  AMD64Imm64Instruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm,
                        TR::CodeGenerator *cg)
      : TR::Instruction(node, op, cg), _sourceImmediate(imm) {}

  AMD64Imm64Instruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                        uint64_t imm, TR::CodeGenerator *cg)
      : TR::Instruction(op, precedingInstruction, cg), _sourceImmediate(imm) {}
...
public:
  AMD64Imm64SymInstruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm,
                           TR::SymbolReference *sr, TR::CodeGenerator *cg)
...
public:
  X86FPRegRegInstruction(TR::Register *sreg, TR::Register *treg, TR::Node *node,
                         TR_X86OpCodes op, TR::CodeGenerator *cg)
      : TR::X86RegRegInstruction(sreg, treg, node, op, cg) {}

  X86FPRegRegInstruction(TR::Register *sreg, TR::Register *treg,
                         TR_X86OpCodes op,
                         TR::Instruction *precedingInstruction,
                         TR::CodeGenerator *cg)
      : TR::X86RegRegInstruction(sreg, treg, op, precedingInstruction, cg) {}

  X86FPRegRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                         TR::Register *sreg, TR::CodeGenerator *cg);
  ...
  enum EassignmentResults {
    kSourceCanBePopped =
        0x01, // source operand can be popped by this instruction
    kTargetCanBePopped =
        0x02,                // target operand can be popped by this instruction
...
public:
  X86FPST0ST1RegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                               TR::Register *treg, TR::Register *sreg,
                               TR::CodeGenerator *cg);
...
public:
  X86FPST0STiRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                               TR::Register *treg, TR::Register *sreg,
                               TR::CodeGenerator *cg);
...
public:
  X86FPSTiST0RegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                               TR::Register *treg, TR::Register *sreg,
...
public:
  X86FPArithmeticRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                   TR::Register *treg, TR::Register *sreg,
                                   TR::CodeGenerator *cg);
...
public:
  X86FPCompareRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                TR::Register *treg, TR::Register *sreg,
                                TR::CodeGenerator *cg);
...
public:
  X86FPCompareEvalInstruction(TR_X86OpCodes op, TR::Node *node,
                              TR::Register *accRegister, TR::CodeGenerator *cg);
...
class X86FPRemainderRegRegInstruction
    : public TR::X86FPST0ST1RegRegInstruction {
  TR::Register *_accRegister;

public:
  X86FPRemainderRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                  TR::Register *treg, TR::Register *sreg,
                                  TR::CodeGenerator *cg);
...
public:
  X86FPMemRegInstruction(TR_X86OpCodes op, TR::Node *node,
                         TR::MemoryReference *mr, TR::Register *sreg,
                         TR::CodeGenerator *cg);
...
public:
  X86FPRegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                         TR::MemoryReference *mr, TR::CodeGenerator *cg);
...
public:
  X86VFPSaveInstruction(TR::Instruction *precedingInstruction,
                        TR::CodeGenerator *cg)
...
public:
  X86VFPRestoreInstruction(TR::Instruction *precedingInstruction,
                           TR::X86VFPSaveInstruction *saveInstruction,
                           TR::CodeGenerator *cg)
      : _saveInstruction(saveInstruction), TR::Instruction(AdjustFramePtr,
                                                           precedingInstruction,
                                                           cg) {}

  X86VFPRestoreInstruction(TR::X86VFPSaveInstruction *saveInstruction,
                           TR::Node *node, TR::CodeGenerator *cg)
      : _saveInstruction(saveInstruction), TR::Instruction(node, AdjustFramePtr,
                                                           cg) {}
...
class X86VFPDedicateInstruction : public TR::X86RegMemInstruction {
  ...
  TR::MemoryReference *memref(TR::CodeGenerator *cg) {
    ...
    return generateX86MemoryReference(
        machine->getRealRegister(TR::RealRegister::vfp), 0, cg);
  }

public:
  X86VFPDedicateInstruction(TR::Instruction *precedingInstruction,
                            TR::RealRegister *framePointerReg,
                            TR::CodeGenerator *cg)
      : TR::X86RegMemInstruction(precedingInstruction, LEARegMem(),
                                 framePointerReg, memref(cg), cg) {}

  X86VFPDedicateInstruction(TR::RealRegister *framePointerReg, TR::Node *node,
                            TR::CodeGenerator *cg)
      : TR::X86RegMemInstruction(LEARegMem(), node, framePointerReg, memref(cg),
                                 cg) {}
  ...
  X86VFPDedicateInstruction(TR::Instruction *precedingInstruction,
                            ...
                            TR::CodeGenerator *cg)
      : TR::X86RegMemInstruction(precedingInstruction, LEARegMem(),
                                 framePointerReg, memref(cg), cond, cg) {}
  ...
  virtual void adjustVFPState(TR_VFPState *state, TR::CodeGenerator *cg) {
    _savedState = cg->vfpState();
    cg->initializeVFPState(
        toRealRegister(getTargetRegister())->getRegisterNumber(), 0);
  }
};

class X86VFPReleaseInstruction : public TR::Instruction {
  // Ideally, TR::X86VFPDedicateInstruction  would inherit
  // TR::X86VFPSaveInstruction, and then this could inherit
  // TR::X86VFPRestoreInstruction, but instead we must duplicate the latter's
  // functionality here.

  TR::X86VFPDedicateInstruction *_dedicateInstruction;

public:
  X86VFPReleaseInstruction(TR::Instruction *precedingInstruction,
                           TR::X86VFPDedicateInstruction *dedicateInstruction,
                           TR::CodeGenerator *cg)
      : _dedicateInstruction(dedicateInstruction), TR::Instruction(
                                                       AdjustFramePtr,
                                                       precedingInstruction,
                                                       cg) {}

  X86VFPReleaseInstruction(TR::X86VFPDedicateInstruction *dedicateInstruction,
                           TR::Node *node, TR::CodeGenerator *cg)
      : _dedicateInstruction(dedicateInstruction), TR::Instruction(
                                                       node, AdjustFramePtr,
                                                       cg) {}
...
public:
  X86VFPCallCleanupInstruction(TR::Instruction *precedingInstruction,
                               int32_t adjustment, TR::CodeGenerator *cg)
      : _stackPointerAdjustment(adjustment), TR::Instruction(
                                                 AdjustFramePtr,
                                                 precedingInstruction, cg) {}

  X86VFPCallCleanupInstruction(int32_t adjustment, TR::Node *node,
                               TR::CodeGenerator *cg)
      : _stackPointerAdjustment(adjustment), TR::Instruction(
                                                 node, AdjustFramePtr, cg) {}
...
TR::X86MemInstruction *generateMemInstruction(TR::Instruction *,
                                              TR_X86OpCodes op,
                                              TR::MemoryReference *mr,
                                              TR::CodeGenerator *cg);
TR::X86RegInstruction *
generateRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                       TR::RegisterDependencyConditions *cond,
                       TR::CodeGenerator *cg);
TR::X86RegInstruction *generateRegInstruction(TR_X86OpCodes op, TR::Node *,
                                              TR::Register *reg1,
                                              TR::CodeGenerator *cg);
...
TR::X86MemInstruction *generateMemInstruction(TR_X86OpCodes op, TR::Node *,
                                              TR::MemoryReference *mr,
                                              TR::CodeGenerator *cg);
...
TR::X86RegImmInstruction *generateRegImmInstruction(
    TR::Instruction *, TR_X86OpCodes op, TR::Register *reg1, int32_t imm,
    TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation);
TR::X86RegMemInstruction *generateRegMemInstruction(TR::Instruction *,
                                                    TR_X86OpCodes op,
                                                    TR::Register *reg1,
                                                    TR::MemoryReference *mr,
                                                    TR::CodeGenerator *cg);
TR::X86RegRegInstruction *generateRegRegInstruction(TR::Instruction *,
                                                    TR_X86OpCodes op,
                                                    TR::Register *reg1,
                                                    TR::Register *reg2,
                                                    TR::CodeGenerator *cg);

TR::Instruction *generateInstruction(TR_X86OpCodes, TR::Node *,
                                     TR::RegisterDependencyConditions *cond,
                                     TR::CodeGenerator *cg);
...
generateMemImmSymInstruction(TR_X86OpCodes op, TR::Node *,
                             TR::MemoryReference *mr, int32_t imm,
                             TR::SymbolReference *sr, TR::CodeGenerator *cg);
...
generateLabelInstruction(TR_X86OpCodes op, TR::Node *, TR::LabelSymbol *sym,
                         TR::RegisterDependencyConditions *cond,
                         TR::CodeGenerator *cg);
...
TR::X86LabelInstruction *generateLabelInstruction(TR_X86OpCodes op,
                                                  ...
                                                  TR::LabelSymbol *sym,
                                                  TR::CodeGenerator *cg);
TR::X86LabelInstruction *generateLabelInstruction(TR::Instruction *i,
                                                  TR_X86OpCodes op,
                                                  TR::LabelSymbol *sym,
                                                  TR::CodeGenerator *cg);
TR::X86LabelInstruction *
generateLabelInstruction(TR_X86OpCodes op, TR::Node *, TR::LabelSymbol *sym,
                         TR::Node *glRegDep, List<TR::Register> *popRegs,
                         bool evaluateGlRegDeps, TR::CodeGenerator *cg);
inline TR::X86LabelInstruction *
...
inline TR::X86LabelInstruction *
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
                         TR::Node *glRegDep, TR::CodeGenerator *cg) {
  return generateLabelInstruction(op, node, sym, glRegDep, 0, true, cg);
}
TR::X86LabelInstruction *
generateLongLabelInstruction(TR_X86OpCodes op, TR::Node *, TR::LabelSymbol *sym,
                             TR::RegisterDependencyConditions *cond,
                             TR::CodeGenerator *cg);
...
TR::X86LabelInstruction *generateJumpInstruction(
    TR_X86OpCodes op, TR::Node *jumpNode, TR::CodeGenerator *cg,
    bool needsVMThreadRegister = false, bool evaluateGlRegDeps = true);

TR::X86LabelInstruction *
generateConditionalJumpInstruction(TR_X86OpCodes op, TR::Node *ifNode,
                                   TR::CodeGenerator *cg,
                                   bool needsVMThreadRegister = false);

TR::X86FenceInstruction *generateFenceInstruction(TR_X86OpCodes op, TR::Node *,
                                                  TR::Node *,
                                                  TR::CodeGenerator *cg);

#ifdef J9_PROJECT_SPECIFIC
TR::X86VirtualGuardNOPInstruction *
generateVirtualGuardNOPInstruction(TR::Node *, TR_VirtualGuardSite *site,
                                   TR::RegisterDependencyConditions *deps,
                                   TR::CodeGenerator *cg);
TR::X86VirtualGuardNOPInstruction *
generateVirtualGuardNOPInstruction(TR::Node *, TR_VirtualGuardSite *site,
                                   TR::RegisterDependencyConditions *deps,
                                   TR::LabelSymbol *symbol,
                                   TR::CodeGenerator *cg);
TR::X86VirtualGuardNOPInstruction *generateVirtualGuardNOPInstruction(
    TR::Instruction *i, TR::Node *, TR_VirtualGuardSite *site,
    TR::RegisterDependencyConditions *deps, TR::LabelSymbol *symbol,
    TR::CodeGenerator *cg);
#endif

TR::X86RegImmInstruction *
generateRegImmInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                          int32_t imm, TR::RegisterDependencyConditions *cond,
                          TR::CodeGenerator *cg);
TR::X86RegImmInstruction *
generateRegImmInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
...
generateRegImmSymInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                             int32_t imm, TR::SymbolReference *,
                             TR::CodeGenerator *cg);

TR::X86RegMemInstruction *generateRegMemInstruction(
    TR_X86OpCodes op, TR::Node *, TR::Register *reg1, TR::MemoryReference *mr,
    TR::RegisterDependencyConditions *deps, TR::CodeGenerator *cg);
TR::X86RegMemInstruction *
generateRegMemInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                          TR::MemoryReference *mr, TR::CodeGenerator *cg);
TR::X86RegRegInstruction *generateRegRegInstruction(
    TR_X86OpCodes op, TR::Node *, TR::Register *reg1, TR::Register *reg2,
    TR::RegisterDependencyConditions *deps, TR::CodeGenerator *cg);
TR::X86RegRegInstruction *
generateRegRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                          TR::Register *reg2, TR::CodeGenerator *cg);
TR::X86MemImmInstruction *
generateMemImmInstruction(TR_X86OpCodes op, TR::Node *, TR::MemoryReference *mr,
                          int32_t imm, TR::CodeGenerator *cg,
                          int32_t reloKind = TR_NoRelocation);
TR::X86MemImmInstruction *generateMemImmInstruction(
    TR::Instruction *, TR_X86OpCodes op, TR::MemoryReference *mr, int32_t imm,
    TR::CodeGenerator *cg, int32_t reloKind = TR_NoRelocation);
...
TR::X86MemRegInstruction *generateMemRegInstruction(
    TR_X86OpCodes op, TR::Node *, TR::MemoryReference *mr, TR::Register *reg1,
    TR::RegisterDependencyConditions *deps, TR::CodeGenerator *cg);
TR::X86MemRegInstruction *
generateMemRegInstruction(TR_X86OpCodes op, TR::Node *, TR::MemoryReference *mr,
                          TR::Register *reg1, TR::CodeGenerator *cg);
TR::X86ImmSymInstruction *generateImmSymInstruction(
    TR_X86OpCodes op, TR::Node *, int32_t imm, TR::SymbolReference *,
...
generateRegRegRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                             TR::Register *reg2, TR::Register *reg3,
                             TR::RegisterDependencyConditions *deps,
                             TR::CodeGenerator *cg);
TR::X86RegRegRegInstruction *
generateRegRegRegInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
...
generateRegRegMemInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                             TR::Register *reg2, TR::MemoryReference *mr,
                             TR::RegisterDependencyConditions *deps,
                             TR::CodeGenerator *cg);
TR::X86RegRegMemInstruction *
generateRegRegMemInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
...
generateImmSnippetInstruction(TR_X86OpCodes op, TR::Node *, int32_t imm,
                              TR::UnresolvedDataSnippet *,
                              TR::CodeGenerator *cg);

TR::X86RegMemImmInstruction *
generateRegMemImmInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                             TR::MemoryReference *mr, int32_t imm,
                             TR::CodeGenerator *cg);
TR::X86RegRegImmInstruction *
generateRegRegImmInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                             TR::Register *reg2, int32_t imm,
                             TR::CodeGenerator *cg);

TR::X86CallMemInstruction *generateCallMemInstruction(
    TR_X86OpCodes op, TR::Node *, TR::MemoryReference *mr,
    TR::RegisterDependencyConditions *, TR::CodeGenerator *cg);
...
generateHelperCallInstruction(TR::Node *, TR_RuntimeHelper,
                              TR::RegisterDependencyConditions *,
                              TR::CodeGenerator *cg);
TR::X86ImmSymInstruction *generateHelperCallInstruction(TR::Instruction *,
                                                        TR_RuntimeHelper,
                                                        TR::CodeGenerator *cg);
...
generateImm64Instruction(TR::Instruction *precedingInstruction,
                         TR_X86OpCodes op, uint64_t imm, TR::CodeGenerator *cg);
...
TR::X86FPRegInstruction *generateFPRegInstruction(TR_X86OpCodes op, TR::Node *,
                                                  TR::Register *reg1,
                                                  TR::CodeGenerator *cg);

TR::X86FPST0ST1RegRegInstruction *
generateFPST0ST1RegRegInstruction(TR_X86OpCodes op, TR::Node *,
                                  TR::Register *reg1, TR::Register *reg2,
                                  TR::CodeGenerator *cg);
TR::X86FPST0STiRegRegInstruction *
generateFPST0STiRegRegInstruction(TR_X86OpCodes op, TR::Node *,
                                  TR::Register *reg1, TR::Register *reg2,
                                  TR::CodeGenerator *cg);
TR::X86FPSTiST0RegRegInstruction *
generateFPSTiST0RegRegInstruction(TR_X86OpCodes op, TR::Node *,
                                  TR::Register *reg1, TR::Register *reg2,
                                  TR::CodeGenerator *cg, bool forcePop = false);

TR::X86FPArithmeticRegRegInstruction *
generateFPArithmeticRegRegInstruction(TR_X86OpCodes op, TR::Node *,
                                      TR::Register *reg1, TR::Register *reg2,
                                      TR::CodeGenerator *cg);

TR::X86FPCompareRegRegInstruction *
generateFPCompareRegRegInstruction(TR_X86OpCodes op, TR::Node *,
                                   TR::Register *reg1, TR::Register *reg2,
                                   TR::CodeGenerator *cg);

TR::X86FPCompareEvalInstruction *
generateFPCompareEvalInstruction(TR_X86OpCodes op, TR::Node *,
                                 TR::Register *accRegister,
                                 TR::CodeGenerator *cg);

TR::X86FPCompareEvalInstruction *generateFPCompareEvalInstruction(
    TR_X86OpCodes op, TR::Node *, TR::Register *accRegister,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg);

TR::X86FPRemainderRegRegInstruction *
generateFPRemainderRegRegInstruction(TR_X86OpCodes op, TR::Node *,
                                     TR::Register *reg1, TR::Register *reg2,
                                     TR::CodeGenerator *cg);
...
generateFPRegMemInstruction(TR_X86OpCodes op, TR::Node *, TR::Register *reg1,
                            TR::MemoryReference *mr, TR::CodeGenerator *cg);

TR::X86FPReturnInstruction *
generateFPReturnInstruction(TR_X86OpCodes op, TR::Node *,
                            TR::RegisterDependencyConditions *,
                            TR::CodeGenerator *cg);

TR::X86FPReturnImmInstruction *
generateFPReturnImmInstruction(TR_X86OpCodes cp, TR::Node *, int32_t imm,
                               TR::RegisterDependencyConditions *,
                               TR::CodeGenerator *cg);
...
generatePatchableCodeAlignmentInstructionWithProtectiveNop(
    ...
    int32_t protectiveNopSize, TR::CodeGenerator *cg);
