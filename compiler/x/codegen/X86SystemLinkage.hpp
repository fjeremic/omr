...
typedef struct parmLayoutResult {
  enum statusEnum {
    IN_LINKAGE_REG = 0x1,
    IN_LINKAGE_REG_PAIR =
        0x2, // Only on AMD64 SysV ABI: a struct might take 2 linkage registers.
    ON_STACK = 0x4,
  };
  uint8_t abstract;
  uint32_t offset;
...
protected:
  ...
  int32_t computeMemoryArgSize(TR::Node *callNode, int32_t first, int32_t last,
                               int8_t direction);
  ...
  virtual int32_t layoutParm(TR::Node *parmNode, int32_t &dataCursor,
                             ...
                             TR::parmLayoutResult &layoutResult) = 0;
  virtual int32_t layoutParm(TR::ParameterSymbol *paramSymbol,
                             int32_t &dataCursor, uint16_t &intReg,
                             uint16_t &floatRrgs, TR::parmLayoutResult &) = 0;

  virtual TR::Register *
  buildVolatileAndReturnDependencies(TR::Node *,
                                     TR::RegisterDependencyConditions *) = 0;

  /**
   * @brief Returns a register appropriate for allocating/de-allocating small
   * stack frames
   *
...
inline TR::X86SystemLinkage *toX86SystemLinkage(TR::Linkage *l) {
  return (TR::X86SystemLinkage *)l;
}
