...
extern TR::Register*
intOrLongClobberEvaluate(TR::Node* node,
                         bool nodeIs64Bit,
                         TR::CodeGenerator* cg);
...
inline TR::Register*
evaluateAndForceSize(TR::Node* node, bool is64Bit, TR::CodeGenerator* cg)
{
...
bool
OMR::X86::TreeEvaluator::analyseSubForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (firstChild->getRegister() == NULL &&
    ...
    if (stride) {
      ...
      leaMR = generateX86MemoryReference(
        NULL, indexRegister, stride, displacement, cg);
      targetRegister = cg->allocateRegister();
      generateRegMemInstruction(
        LEARegMem(nodeIs64Bit), node, targetRegister, leaMR, cg);
    ...
    if (firstOp.isAdd() && TR::TreeEvaluator::constNodeValueIs32BitSigned(
      ...
      if (llChild->getRegister() == NULL && llChild->getReferenceCount() == 1 &&
        ...
        //
        leaMR =
          generateX86MemoryReference(cg->evaluate(lrChild),
        ...
        //
        leaMR =
          generateX86MemoryReference(cg->evaluate(llChild),
...
bool
OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if ((secondOp.isAdd() || secondOp.isSub()) &&
    ...
    if (stride && addFirstChild->getReferenceCount() == 1 &&
      ...
      leaMR = generateX86MemoryReference(
        baseRegister, indexRegister, stride, offset, cg);
      targetRegister = cg->allocateRegister();
      generateRegMemInstruction(
        LEARegMem(nodeIs64Bit), node, targetRegister, leaMR, cg);
  ...
  if (indexNode) {
    ...
    if (constNode) {
      ...
      if (constNode->getOpCodeValue() == TR::lconst)
        leaMR = generateX86MemoryReference(
          NULL, indexRegister, stride, constNode->getLongInt(), cg);
      else
        leaMR = generateX86MemoryReference(
          NULL, indexRegister, stride, constNode->getInt(), cg);
      tempNode = NULL;
      ...
      leaMR = generateX86MemoryReference(
        cg->evaluate(baseNode->getFirstChild()),
        indexRegister,
        stride,
      ...
      leaMR = generateX86MemoryReference(
        cg->evaluate(baseNode), indexRegister, stride, 0, cg);
      tempNode = baseNode;
    }
    targetRegister = cg->allocateRegister();
    generateRegMemInstruction(
...
static void
evaluateIfNotConst(TR::Node* node, TR::CodeGenerator* cg)
{
  if (!node->getOpCode().isLoadConst()) {
    diagnostic(
      "Evaluating Non Constant %p (%s)\n", node, node->getOpCode().getName());
    cg->evaluate(node);
  } else {
    diagnostic(
      "Not evaluating Constant %p (%s)\n", node, node->getOpCode().getName());
  }
}
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (NEED_CC(node) || (opCode == TR::luaddc) || (opCode == TR::iuaddc)) {
    TR_ASSERT((nodeIs64Bit && (opCode == TR::ladd || opCode == TR::luadd) ||
               opCode == TR::luaddc) ||
    ...
    TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(
      node,
      AddRegReg(nodeIs64Bit, isWithCarry),
  ...
  if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
    ...
    if (!targetRegister && secondChild->getOpCode().isLoadConst() &&
      ...
      if (targetRegister &&
        ...
        if (TR::TreeEvaluator::genNullTestSequence(
              node, firstOperandReg, targetRegister, cg))
          tempReg = targetRegister;

        if (computesCarry) {
          generateRegRegInstruction(
            MOVRegReg(nodeIs64Bit), node, targetRegister, tempReg, cg);
          ...
          generateRegMemInstruction(
            LEARegMem(nodeIs64Bit), node, targetRegister, tempMR, cg);
        }
      } else {
        if (constValue >= -128 && constValue <= 127) {
          if (computesCarry) {
            if (isMemOp)
              instr =
                generateMemImmInstruction(AddMemImms(nodeIs64Bit, isWithCarry),
                                          node,
                                          tempMR,
                                          constValue,
                                          cg);
              ...
              instr =
                generateRegImmInstruction(AddRegImms(nodeIs64Bit, isWithCarry),
                                          node,
                                          targetRegister,
                                          constValue,
                                          cg);
            ...
            if (isMemOp)
              ...
              instr = generateRegImmInstruction(
                ADDRegImms(nodeIs64Bit), node, targetRegister, 1, cg);
          } else if (constValue == -1) {
            if (isMemOp)
              instr =
              ...
              instr = generateRegImmInstruction(
                SUBRegImms(nodeIs64Bit), node, targetRegister, 1, cg);
          } else {
            if (isMemOp)
              instr = generateMemImmInstruction(
                ADDMemImms(nodeIs64Bit), node, tempMR, constValue, cg);
            else
              instr = generateRegImmInstruction(
                ADDRegImms(nodeIs64Bit), node, targetRegister, constValue, cg);
          }
        } else if ((constValue == 128) && !computesCarry) {
          if (isMemOp)
            instr = generateMemImmInstruction(
              SUBMemImms(nodeIs64Bit), node, tempMR, (uint32_t)-128, cg);
          else
            instr = generateRegImmInstruction(SUBRegImms(nodeIs64Bit),
                                              node,
                                              targetRegister,
                                              (uint32_t)-128,
                                              cg);
          ...
          //
          TR::TreeEvaluator::genNullTestSequence(
            node, targetRegister, targetRegister, cg);
      ...
      instr = generateMemRegInstruction(AddMemReg(nodeIs64Bit, isWithCarry),
                                        node,
                                        tempMR,
                                        cg->evaluate(secondChild),
                                        cg);
...
TR::Register*
OMR::X86::TreeEvaluator::baddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL &&
    ...
    if (targetRegister && firstChild->getReferenceCount() > 1) {
      ...
      if (value == 1) {
        ...
        if (isMemOp)
          ...
          instr = generateRegImmInstruction(
            ADD1RegImm1, node, targetRegister, value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d",
                   node,
    ...
    instr = generateMemRegInstruction(
      ADD1MemReg, node, tempMR, cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by var", node);
  } else {
    TR_X86BinaryCommutativeAnalyser temp(cg);
...
TR::Register*
OMR::X86::TreeEvaluator::saddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL &&
      ...
      if (value >= -128 && value <= 127) {
        if (value == 1) {
          ...
          if (isMemOp)
            ...
            instr = generateRegImmInstruction(
              ADD4RegImms, node, targetRegister, value, cg);
        }
      } else if (value == 128) {
        if (isMemOp)
          instr = generateMemImmInstruction(
            SUB2MemImms, node, tempMR, (uint32_t)-128, cg);
        else
          instr = generateRegImmInstruction(
            SUB4RegImms, node, targetRegister, (uint32_t)-128, cg);
      } else {
        if (isMemOp)
          instr =
          ...
          instr = generateRegImmInstruction(
            ADD2RegImm2, node, targetRegister, value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d",
                   node,
    ...
    instr = generateMemRegInstruction(
      ADD2MemReg, node, tempMR, cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by var", node);
  } else {
    TR_X86BinaryCommutativeAnalyser temp(cg);
...
TR::Register*
OMR::X86::TreeEvaluator::caddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb) ||
    ...
    TR_ASSERT((nodeIs64Bit && (opCode == TR::lsub || opCode == TR::lusub) ||
               opCode == TR::lusubb) ||
    ...
    TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(
      node,
      SubRegReg(nodeIs64Bit, isWithBorrow),
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
    ...
    if (targetRegister && !computesCarry &&
      ...
      if (constValue >= -128 && constValue <= 127) {
        if (computesCarry) {
          ...
          if (isMemOp)
            instr = generateMemImmInstruction(
              SUBMemImms(nodeIs64Bit), node, tempMR, constValue, cg);
          else
            instr = generateRegImmInstruction(
              SUBRegImms(nodeIs64Bit), node, targetRegister, constValue, cg);
        }
      } else if ((constValue == 128) && !computesCarry) {
        if (isMemOp)
          instr = generateMemImmInstruction(
            ADDMemImms(nodeIs64Bit), node, tempMR, (uint32_t)-128, cg);
        else
          instr = generateRegImmInstruction(
            ADDRegImms(nodeIs64Bit), node, targetRegister, (uint32_t)-128, cg);
      } else {
        if (isMemOp)
          instr =
    ...
    instr = generateMemRegInstruction(SubMemReg(nodeIs64Bit, isWithBorrow),
                                      node,
                                      tempMR,
                                      cg->evaluate(secondChild),
                                      cg);
...
TR::Register*
OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL &&
    ...
    if (targetRegister && firstChild->getReferenceCount() > 1) {
      ...
      if (value == 1) {
        ...
        if (isMemOp)
          ...
          instr = generateRegImmInstruction(
            SUB1RegImm1, node, targetRegister, value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by const %d",
                   node,
    ...
    instr = generateMemRegInstruction(
      SUB1MemReg, node, tempMR, cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
  } else {
    TR_X86SubtractAnalyser temp(cg);
...
TR::Register*
OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL &&
      ...
      if (value >= -128 && value <= 127) {
        if (value == 1) {
          ...
          if (isMemOp)
            ...
            instr = generateRegImmInstruction(
              SUB4RegImms, node, targetRegister, value, cg);
        }
      } else {
        if (isMemOp)
          instr =
          ...
          instr = generateRegImmInstruction(
            SUB2RegImm2, node, targetRegister, value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by const %d",
                   node,
    ...
    instr = generateMemRegInstruction(
      SUB2MemReg, node, tempMR, cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
  } else {
    TR_X86SubtractAnalyser temp(cg);
...
TR::Register*
OMR::X86::TreeEvaluator::csubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
                                                 ...
                                                 TR::CodeGenerator* cg)
{
  ...
  if (node->isDualCyclic() || needsHighMulOnly) {
    ...
    multDependencies->addPreCondition(
      lmulTargetRegister, TR::RealRegister::eax, cg);
    multDependencies->addPreCondition(
      lumulhTargetRegister, TR::RealRegister::edx, cg);
    multDependencies->addPostCondition(
      lmulTargetRegister, TR::RealRegister::eax, cg);
    multDependencies->addPostCondition(
      lumulhTargetRegister, TR::RealRegister::edx, cg);

    // TODO: sometimes acc-mem may be more efficient
    generateRegRegInstruction(MUL8AccReg,
                              node,
                              lmulTargetRegister,
                              lumulhTargetRegister,
                              multDependencies,
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      ...
      generateRegRegInstruction(
        XOR4RegReg, node, targetRegister, targetRegister, cg);
    } else {
      ...
      if (targetRegister == 0 && IS_32BIT_SIGNED(value)) // decomposition failed
      {
        ...
        if (firstChild->getReferenceCount() > 1 ||
          ...
          generateRegRegImmInstruction(
            opCode, node, targetRegister, cg->evaluate(firstChild), value, cg);
        } else {
          if (firstChild->getOpCode().isMemoryReference()) {
            ...
            generateRegMemImmInstruction(
              opCode, node, targetRegister, tempMR, value, cg);
            tempMR->decNodeReferenceCounts(cg);
          } else {
            if (value >= -128 && value <= 127) {
              opCode = IMULRegRegImms(nodeIs64Bit);
            ...
            targetRegister = cg->evaluate(firstChild);
            generateRegRegImmInstruction(
              opCode, node, targetRegister, targetRegister, value, cg);
          }
        }
      }
    }
  ...
  if (targetRegister == 0) {
    ...
    temp.genericAnalyser(node,
                         IMULRegReg(nodeIs64Bit),
                         IMULRegMem(nodeIs64Bit),
                         MOVRegReg(nodeIs64Bit));
    targetRegister = node->getRegister();
  }
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  generateRegRegInstruction(IMULAccReg(nodeIs64Bit),
                            node,
                            eaxRegister,
                            targetRegister,
                            multDependencies,
  ...
  if (isPowerOf2(dvalue)) {
    ...
    if (rootOpCode.isRem()) {
      ...
      if (isMinInt) {
        ...
        //
        generateRegRegInstruction(
          TESTRegReg(nodeIs64Bit), node, tempRegister, tempRegister, cg);
        ...
        if (1 <= dvalue && dvalue <= 0x40000000) {
          ...
          generateRegImmInstruction(
            AND8RegImm4, node, tempRegister, (uint32_t)(2 * dvalue - 1), cg);
          ...
          generateRegImmInstruction(ROL8RegImm1, node, tempRegister, 1, cg);
          generateRegRegInstruction(
            MOVZXReg8Reg4, node, tempRegister, tempRegister, cg);
          generateRegImmInstruction(ROR8RegImm1, node, tempRegister, 1, cg);
        } else {
          ...
          generateRegImmInstruction(AND8RegImm4,
                                    node,
                                    tempRegister,
                                    SIGN32 |
                                      (uint32_t)((int64_t)(dvalue - 1) >> 32),
                                    cg);
        ...
        generateRegImmInstruction(
          AND4RegImm4, node, tempRegister, (uint32_t)SIGN32 + dvalue - 1, cg);
        generateLabelInstruction(JNS4, node, doneLabel, cg);
      }
      ...
      if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
        ...
        generateRegImmInstruction(
          ORRegImm4(nodeIs64Bit), node, tempRegister, (uint32_t)-dvalue, cg);
        ...
        if (dvalue == CONSTANT64(0x100000000)) {
          ...
          generateRegImmInstruction(
            XOR4RegImms, node, tempRegister, (unsigned)-1, cg);
          // Flip all 64 bits
          generateRegImmInstruction(
            XOR8RegImms, node, tempRegister, (unsigned)-1, cg);
          ...
          generateRegImmInstruction(OR8RegImm4,
                                    node,
                                    tempRegister,
                                    (~SIGN32) &
                                      (uint32_t)((int64_t)(-dvalue) >> 33),
                                    cg);
          // Un-rotate
    ...
    if ((dividend->isNonNegative() == false) && (dvalue > 0 || isMinInt)) {
      ...
      cdqDependencies->addPostCondition(
        tempRegister, TR::RealRegister::eax, cg);
      ...
      {
        generateRegRegInstruction(
          SUBRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
      } else if (!nodeIs64Bit ||
        ...
        generateRegImmInstruction(
          ANDRegImm4(nodeIs64Bit), node, edxRegister, (uint32_t)dvalue - 1, cg);
        generateRegRegInstruction(
          ADDRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
      } else {
        // dvalue-1 is too big for an Imm4, so shift off the bits instead.
        //
        int32_t shiftAmount = leadingZeroes(dvalue) + 1;
        generateRegImmInstruction(
          SHL8RegImm1, node, edxRegister, shiftAmount, cg);
        generateRegImmInstruction(
          SHR8RegImm1, node, edxRegister, shiftAmount, cg);
        generateRegRegInstruction(
          ADDRegReg(nodeIs64Bit), node, tempRegister, edxRegister, cg);
      }
    }

    generateRegImmInstruction(SARRegImm1(nodeIs64Bit),
                              node,
                              tempRegister,
                              trailingZeroes(dvalue),
                              cdqDependencies,
                              cg);

    if (isNegative) {
      generateRegInstruction(
        NEGReg(nodeIs64Bit), node, tempRegister, cdqDependencies, cg);
    }
    ...
    if (!rootOpCode.isRem() && !(dvalue > 0 && m < 0) &&
      ...
      multDependencies->addPreCondition(eaxRegister, TR::RealRegister::eax, cg);
      multDependencies->addPreCondition(
        dividendRegister, TR::RealRegister::edx, cg);
      multDependencies->addPostCondition(
        eaxRegister, TR::RealRegister::eax, cg);
      multDependencies->addPostCondition(
        dividendRegister, TR::RealRegister::edx, cg);
    } else {
      ...
      multDependencies->addPostCondition(
        eaxRegister, TR::RealRegister::eax, cg);
      multDependencies->addPostCondition(
        edxRegister, TR::RealRegister::edx, cg);
    }
    ...
    if (!nodeIs64Bit || IS_32BIT_SIGNED(m))
      generateRegImmInstruction(
        MOVRegImm4(nodeIs64Bit), node, eaxRegister, m, cg);
    else
      generateRegMemInstruction(
        LEA8RegMem, node, eaxRegister, generateX86MemoryReference(m, cg), cg);
    generateRegRegInstruction(IMULAccReg(nodeIs64Bit),
                              node,
                              eaxRegister,
                              dividendRegister,
    ...
    if ((dvalue > 0) && (m < 0)) {
      generateRegRegInstruction(
        ADDRegReg(nodeIs64Bit), node, edxRegister, dividendRegister, cg);
    } else if ((dvalue < 0) && (m > 0)) {
      generateRegRegInstruction(
        SUBRegReg(nodeIs64Bit), node, edxRegister, dividendRegister, cg);
    }

    // Do the magic shift
    //
    generateRegImmInstruction(
      SARRegImm1(nodeIs64Bit), node, edxRegister, s, cg);
    ...
    if (!(dividend->isNonNegative() && (dvalue > 0))) {
      ...
      generateRegRegInstruction(
        MOVRegReg(nodeIs64Bit), node, eaxRegister, edxRegister, cg);
      generateRegImmInstruction(
        SHRRegImm1(nodeIs64Bit), node, eaxRegister, nodeIs64Bit ? 63 : 31, cg);
      generateRegRegInstruction(
        ADDRegReg(nodeIs64Bit), node, edxRegister, eaxRegister, cg);
    ...
    if (rootOpCode.isRem()) {
      if (nodeIs64Bit && !IS_32BIT_SIGNED(dvalue)) {
        ...
        generateRegRegInstruction(
          IMULRegReg(nodeIs64Bit), node, edxRegister, scratchReg, cg);
        ...
        generateRegRegImmInstruction(
          opCode, node, edxRegister, edxRegister, dvalue, cg);
      }
                                                  ...
                                                  TR::CodeGenerator* cg)
{
  ...
  if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst() &&
    ...
    TR::LabelSymbol *startLabel,
      *divisionLabel = NULL, *overflowSnippetLabel = NULL, *restartLabel = NULL;
    ...
    if (needsExplicitOverflowCheck) {
      ...
      generateLabelInstruction(LABEL, node, startLabel, cg);
      generateRegImmInstruction(
        CMPRegImms(nodeIs64Bit), node, eaxRegister, 1, cg);
    ...
    if (!nodeIs64Bit && node->isUnsigned()) {
      generateRegRegInstruction(
        XORRegReg(nodeIs64Bit), node, edxRegister, edxRegister, edxDeps, cg);

      if (divisorRegister) {
        divideInstr = generateRegRegInstruction(DIVAccReg(nodeIs64Bit),
                                                node,
                                                eaxRegister,
                                                divisorRegister,
                                                eaxEdxDeps,
      ...
      if (divisorRegister) {
        if (node->getFirstChild()->isNonNegative() ||
          ...
          generateRegRegInstruction(
            XOR4RegReg, node, edxRegister, edxRegister, edxDeps, cg);
        else
        ...
        if (node->getOpCode().isUnsigned() ||
          ...
          divideInstr = generateRegRegInstruction(IDIVAccReg(nodeIs64Bit),
                                                  node,
                                                  eaxRegister,
                                                  divisorRegister,
                                                  eaxEdxDeps,
        ...
        if (node->getOpCode().isUnsigned() ||
          ...
          divideInstr = generateRegMemInstruction(
            IDIVAccMem(nodeIs64Bit), node, eaxRegister, tempMR, eaxEdxDeps, cg);
        tempMR->decNodeReferenceCounts(cg);
      }
    }
...
OMR::X86::TreeEvaluator::generateRegisterShift(TR::Node* node,
                                               ...
                                               TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (shiftAmount == 0) {
      ...
      if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift() &&
        ...
        instr = generateRegRegInstruction(
          MOVRegReg(nodeIs64Bit), node, targetRegister, origTargetRegister, cg);
      } else {
      ...
      instr = generateRegImmInstruction(
        immShiftOpCode, node, targetRegister, shiftAmount, cg);
    }
  } else {
    ...
    TR::RegisterDependencyConditions* shiftDependencies =
      generateRegisterDependencyConditions((uint8_t)1, 1, cg);
    shiftDependencies->addPreCondition(
      shiftAmountReg, TR::RealRegister::ecx, cg);
    shiftDependencies->addPostCondition(
      shiftAmountReg, TR::RealRegister::ecx, cg);
    targetRegister =
      TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
    ...
    instr = generateRegRegInstruction(regShiftOpCode,
                                      node,
                                      targetRegister,
                                      shiftAmountReg,
                                      shiftDependencies,
...
OMR::X86::TreeEvaluator::generateMemoryShift(TR::Node* node,
                                             TR_X86OpCodes immShiftOpCode,
                                             TR_X86OpCodes regShiftOpCode,
                                             TR::CodeGenerator* cg)
{
  ...
  if (loadConstant && performTransformation(
                        comp,
                        "O^O GenerateMemoryShift: load is not constant %d\n",
                        loadConstant)) {
    intptrj_t shiftAmount =
      TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) &
      TR::TreeEvaluator::shiftMask(nodeIs64Bit);
    if (shiftAmount != 0) {
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shift by const %d",
                   ...
                   shiftAmount);
      instr = generateMemImmInstruction(
        immShiftOpCode, node, memRef, shiftAmount, cg);
    }
  } else {
    TR::ILOpCodes op = secondChild->getOpCodeValue();
    TR::Register* shiftAmountReg = NULL;
    ...
    shiftDependencies->addPostCondition(
      shiftAmountReg, TR::RealRegister::ecx, cg);
    instr = generateMemRegInstruction(
      regShiftOpCode, node, memRef, shiftAmountReg, shiftDependencies, cg);
    if (debug("traceMemOp"))
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (node->isDirectMemoryUpdate()) {
    TR::TreeEvaluator::generateMemoryShift(
      node, SHLMemImm1(nodeIs64Bit), SHLMemCL(nodeIs64Bit), cg);
  } else if (shiftAmountNode->getOpCode().isLoadConst() &&
             (shiftAmount =
                TR::TreeEvaluator::integerConstNodeValue(shiftAmountNode, cg) &
                TR::TreeEvaluator::shiftMask(nodeIs64Bit),
    ...
    TR::Register* targetRegister = cg->allocateRegister();
    generateRegMemInstruction(
      LEARegMem(nodeIs64Bit), node, targetRegister, memRef, cg);
    node->setRegister(targetRegister);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
  } else {
    TR::TreeEvaluator::generateRegisterShift(
      node, SHLRegImm1(nodeIs64Bit), SHLRegCL(nodeIs64Bit), cg);
  }
  return node->getRegister();
}
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (rotateAmount == 0) {
      ...
      generateRegImmInstruction(
        ROLRegImm1(nodeIs64Bit), node, targetRegister, rotateAmount, cg);
    }
  } else {
    targetRegister =
      TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
    ...
    rotateDependencies->addPreCondition(
      rotateAmountReg, TR::RealRegister::ecx, cg);
    rotateDependencies->addPostCondition(
      rotateAmountReg, TR::RealRegister::ecx, cg);

    generateRegRegInstruction(ROLRegCL(nodeIs64Bit),
                              node,
                              targetRegister,
                              rotateAmountReg,
                              rotateDependencies,
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (node->isDirectMemoryUpdate()) {
    TR::TreeEvaluator::generateMemoryShift(
      node, SARMemImm1(nodeIs64Bit), SARMemCL(nodeIs64Bit), cg);
  } else {
    TR::TreeEvaluator::generateRegisterShift(
      node, SARRegImm1(nodeIs64Bit), SARRegCL(nodeIs64Bit), cg);
  }
  return node->getRegister();
}
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (node->isDirectMemoryUpdate()) {
    ...
    TR::TreeEvaluator::generateRegisterShift(
      node, SHRRegImm1(nodeIs64Bit), SHRRegCL(nodeIs64Bit), cg);
  }
...
TR::Register*
OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (testOpCode == TR::bconst &&
    ...
    if (isMemOp) {
      ...
      if (value == 0) {
        ...
        instr = generateRegMemInstruction(
          LEA4RegMem, node, targetRegister, tempMR, cg);
      } else {
        targetRegister = cg->intClobberEvaluate(firstChild);
        instr = generateRegImmInstruction(
          SHL1RegImm1, node, targetRegister, value, cg);
      }
    }
  } else {
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(
        SHL1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
    } else {
      targetRegister = cg->intClobberEvaluate(firstChild);
      instr = generateRegRegInstruction(
        SHL1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
  }
...
TR::Register*
OMR::X86::TreeEvaluator::sshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* secondChild = node->getSecondChild();
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (testOpcode == TR::iconst &&
    ...
    if (isMemOp) {
      ...
      if (value == 0) {
        ...
        targetRegister = cg->intClobberEvaluate(firstChild);
        instr = generateRegImmInstruction(
          SHL4RegImm1, node, targetRegister, value, cg);
      }
    }
  } else {
    TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(
        SHL2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
    } else {
      targetRegister = cg->intClobberEvaluate(firstChild);
      instr = generateRegRegInstruction(
        SHL4RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
    }
  }
...
TR::Register*
OMR::X86::TreeEvaluator::bshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (testOpcode == TR::bconst &&
    ...
    if (value != 0) {
      if (isMemOp) {
        ...
        instr = generateRegImmInstruction(
          SAR1RegImm1, node, targetRegister, value, cg);
    }
  } else {
    TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(
        SAR1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
    } else
      instr = generateRegRegInstruction(
        SAR1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
  }
...
TR::Register*
OMR::X86::TreeEvaluator::sshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (testOpcode == TR::iconst &&
    ...
    if (value != 0) {
      if (isMemOp) {
        ...
        instr = generateRegImmInstruction(
          SAR2RegImm1, node, targetRegister, value, cg);
    }
  } else {
    TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(
        SAR2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
    } else
      instr = generateRegRegInstruction(
        SAR2RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
  }
...
TR::Register*
OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if ((testOpcode2 == TR::bconst || testOpcode2 == TR::buconst) &&
    ...
    if (isMemOp) {
      ...
      instr =
        generateRegImmInstruction(SHR1RegImm1, node, targetRegister, value, cg);
  } else {
    TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(
        SHR1MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
    } else
      instr = generateRegRegInstruction(
        SHR1RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
  }
...
TR::Register*
OMR::X86::TreeEvaluator::sushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (testOpcode == TR::iconst &&
    ...
    if (isMemOp) {
      ...
      instr =
        generateRegImmInstruction(SHR2RegImm1, node, targetRegister, value, cg);
  } else {
    TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(
        SHR2MemCL, node, tempMR, shiftAmountReg, shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
    } else
      instr = generateRegRegInstruction(
        SHR2RegCL, node, targetRegister, shiftAmountReg, shiftDependencies, cg);
  }
...
TR_X86OpCodes OMR::X86::TreeEvaluator::_logicalOpPackage[numLogicalOpPackages]
                                                        [numLogicalOpForms] = {
                                                          // band
                                                          { AND1RegReg,
                                                            AND1RegMem,
...
TR::Register*
OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node* node,
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
    ...
    if (!isMemOp && node->getOpCode().isAnd() && !firstChild->getRegister() &&
      ...
      if (firstChild->getFirstChild()->getReferenceCount() > 1 ||
        ...
        generateRegRegInstruction(
          MOVRegReg(nodeIs64Bit), node, temp, targetRegister, cg);
        targetRegister = temp;
      }
    ...
    if (node->getOpCode().isXor() && constValue == -1) {
      if (isMemOp) {
        instr = generateMemInstruction(
          package[TR::TreeEvaluator::logicalNotOp], node, tempMR, cg);
        if (debug("traceMemOp"))
        ...
        instr =
          generateRegInstruction(NOTReg(nodeIs64Bit), node, targetRegister, cg);
      }
    } else {
      if (constValue >= -128 && constValue <= 127) {
        if (isMemOp)
          instr = generateMemImmInstruction(
            package[TR::TreeEvaluator::logicalByteMemImmOp],
            node,
            tempMR,
            constValue,
            cg);
          ...
          instr = generateRegImmInstruction(
            package[TR::TreeEvaluator::logicalByteImmOp],
            node,
            targetRegister,
            constValue,
            cg);
        ...
        if (isMemOp)
          instr = generateMemImmInstruction(
            package[TR::TreeEvaluator::logicalMemImmOp],
            node,
            tempMR,
            constValue,
            cg);
          ...
          instr =
            generateRegImmInstruction(package[TR::TreeEvaluator::logicalImmOp],
                                      node,
                                      targetRegister,
                                      constValue,
                                      cg);
...
TR::Register*
OMR::X86::TreeEvaluator::bandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::sandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::X86::TreeEvaluator::candEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::borEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::sorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::corEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::iorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::bxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::sxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::cxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
...
TR::Register*
OMR::X86::TreeEvaluator::ixorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::logicalEvaluator(
