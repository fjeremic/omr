...
bool OMR::X86::TreeEvaluator::analyseSubForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
        ...
        if (firstOp.isAdd() && TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &dummyConstValue, cg)) {
            ...
            if (llChild->getRegister() == NULL && llChild->getReferenceCount() == 1 && (stride = TR::MemoryReference::getStrideForNode(llChild, cg))) {
                // sub
                //    add         (firstChild)
                //       mul/shl  (llChild)
                //          ?
            ...
            } else if (lrChild->getRegister() == NULL && lrChild->getReferenceCount() == 1 && (stride = TR::MemoryReference::getStrideForNode(lrChild, cg))) {
                // sub
                //    add         (firstChild)
                //       ?        (llchild)
                //       mul/shl  (lrChild)
...
bool OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((secondOp.isAdd() || secondOp.isSub()) && secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL) {
        ...
        if (stride && addFirstChild->getReferenceCount() == 1 && addFirstChild->getRegister() == NULL && addSecondOp.isLoadConst()) {
            // add
            ...
            if (firstChild->getOpCodeValue() == TR::loadaddr && firstChild->getSymbolReference()->getSymbol()->isMethodMetaData()) {
    ...
    if (indexNode) {
        ...
        if (constNode) {
            ...
            if (constNode->getOpCodeValue() == TR::lconst)
                ...
                leaMR = generateX86MemoryReference(NULL,
                    ...
                    constNode->getInt(), cg);
            tempNode = NULL;
        } else if (baseNode->getRegister() == NULL && baseNode->getReferenceCount() == 1 && baseNode->getOpCode().isAdd() && baseNode->getSecondChild()->getOpCode().isLoadConst() && TR::TreeEvaluator::constNodeValueIs32BitSigned(baseNode->getSecondChild(), &dummyConstValue, cg)) {
            // add            (** children may be reversed **)
            //    add            (baseNode)
            //       ?
            //       const
        ...
        if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && firstOp.isAdd()) {
            ...
            if (firstChild->getSecondChild()->getReferenceCount() == 1 && firstChild->getSecondChild()->getRegister() == NULL) {
            ...
            if (stride1 | stride2) {
                ...
                if (indexNode->getOpCode().isLoadConst()) {
                    ...
                    leaMR = generateX86MemoryReference(cg->evaluate(baseNode),
                        TR::TreeEvaluator::integerConstNodeValue(constNode, cg) + TR::TreeEvaluator::integerConstNodeValue(indexNode, cg), cg);
                } else {
                    leaMR = generateX86MemoryReference(cg->evaluate(baseNode),
                        cg->evaluate(indexNode),
                        0,
    ...
    } else if (firstChild->getOpCode().getOpCodeValue() == TR::loadaddr && (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1)) {
        // add
        //    loadaddr    (firstChild)
        //    ?           (secondChild)
        ...
        node->setRegister(targetRegister);
        return true;
    } else if ((firstChild->getOpCode().isSub() || firstChild->getOpCode().isAdd()) && (firstChild->getOpCode().getSize() == node->getOpCode().getSize()) && (firstChild->getRegister() == NULL) && (firstChild->getSecondChild()->getOpCode().isLoadConst()) && TR::TreeEvaluator::constNodeValueIs32BitSigned(firstChild->getSecondChild(), &dummyConstValue, cg) && (firstChild->getReferenceCount() == 1)) {
        // add
        //    sub/add    (firstChild)
        //        child 0
        //        loadconstant
...
TR::Register* OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (NEED_CC(node) || (opCode == TR::luaddc) || (opCode == TR::iuaddc)) {
        TR_ASSERT((nodeIs64Bit && (opCode == TR::ladd || opCode == TR::luadd) || opCode == TR::luaddc) || (!nodeIs64Bit && (opCode == TR::iadd || opCode == TR::iuadd) || opCode == TR::iuaddc),
    ...
    if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
        ...
        if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL) && ((TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg)) || (comp->useCompressedPointers() && (constValue == TR::Compiler->vm.heapBaseAddress()) && firstChild->getReferenceCount() > 1 && !isMemOp)) && performTransformation(comp, "O^O analyseAddForLEA: checking that second node is a memory reference %x\n", constValue)) {
            if (!isMemOp)
            ...
            //
            if (targetRegister && (firstChild->getReferenceCount() > 1 || (node->getOpCode().isArrayRef() && (internalPointerMismatch || targetRegister->containsCollectedReference())))) {
                ...
                // still
                //
                if (targetRegister->containsCollectedReference() && (node->getOpCode().isArrayRef()) && !node->isInternalPointer()) {
    ...
    if (targetRegister && node->getOpCode().isArrayRef() && node->isInternalPointer()) {
        if (node->getPinningArrayPointer()) {
            ...
            TR::Node* firstChild = node->getFirstChild();
            if ((firstChild->getOpCodeValue() == TR::aload) && firstChild->getSymbolReference()->getSymbol()->isAuto() && firstChild->getSymbolReference()->getSymbol()->isPinningArrayPointer()) {
                targetRegister->setContainsInternalPointer();
...
TR::Register* OMR::X86::TreeEvaluator::baddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL && performTransformation(comp, "O^O BaddEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::saddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL && performTransformation(comp, "O^O SaddEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::caddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL && performTransformation(comp, "O^O CaddEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb) || (node->getOpCodeValue() == TR::iusubb)) {
        TR_ASSERT((nodeIs64Bit && (opCode == TR::lsub || opCode == TR::lusub) || opCode == TR::lusubb) || (!nodeIs64Bit && (opCode == TR::isub || opCode == TR::iusub) || opCode == TR::iusubb),
            "CC computation not supported for this node %p with opcode %s\n", node, cg->comp()->getDebug()->getName(opCode));
    ...
    intptrj_t constValue;
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg) && performTransformation(comp, "O^O IntegerSubEvaluator: register is not NULL, or second operand is not a 32 bit constant. Register value: %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL && performTransformation(comp, "O^O BSUBEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
        int32_t value = secondChild->getByte();
        if (!isMemOp)
            targetRegister = cg->evaluate(firstChild);
        if (targetRegister && firstChild->getReferenceCount() > 1) {
...
TR::Register* OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL && performTransformation(comp, "O^O SSUBEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
        int32_t value = secondChild->getShortInt();
        if (!isMemOp)
            targetRegister = cg->evaluate(firstChild);
        if (firstChild->getReferenceCount() > 1) {
...
TR::Register* OMR::X86::TreeEvaluator::csubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL && performTransformation(comp, "O^O CSubEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
        int32_t value = secondChild->getConst<uint16_t>();
        if (!isMemOp)
            targetRegister = cg->evaluate(firstChild);
        if (targetRegister && firstChild->getReferenceCount() > 1) {
...
TR::Register* OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && (value = TR::TreeEvaluator::integerConstNodeValue(secondChild, cg))) {
        ...
        if (value == 0) {
            ...
            TR_X86IntegerMultiplyDecomposer* mulDecomposer = new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer(value,
...
TR::Register* OMR::X86::TreeEvaluator::signedIntegerDivOrRemAnalyser(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool isMinInt = ((!nodeIs64Bit && dvalue == TR::getMinSigned<TR::Int32>()) || (nodeIs64Bit && dvalue == TR::getMinSigned<TR::Int64>()));
...
TR::Register* OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst() && TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) != 0) {
        ...
        } else if (secondChild->getReferenceCount() == 1 && !secondChild->getRegister() && secondChild->getOpCode().isMemoryReference()) {
        ...
        if (!nodeIs64Bit && node->isUnsigned()) {
            ...
            if (divisorRegister) {
                ...
                if (node->getOpCode().isUnsigned() || (node->getFirstChild()->isNonNegative() && node->getSecondChild()->isNonNegative()))
                    divideInstr = generateRegRegInstruction(DIVAccReg(nodeIs64Bit), node, eaxRegister, divisorRegister, eaxEdxDeps, cg);
                else
...
TR::X86RegInstruction* OMR::X86::TreeEvaluator::generateRegisterShift(TR::Node* node, TR_X86OpCodes immShiftOpCode, TR_X86OpCodes regShiftOpCode, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (shiftAmount == 0) {
            ...
            if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift() && (targetRegister->containsCollectedReference() || node->containsCompressionSequence())) {
        ...
        if (op == TR::su2i || op == TR::s2i || op == TR::b2i || op == TR::bu2i || op == TR::l2i) {
            if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == 0) {
                ...
                if (secondChild->getOpCode().isLoadIndirect() && grandChild->getReferenceCount() == 1 && grandChild->getRegister() == 0) {
                    ...
                    if (reportShiftAmount)
                        diagnostic("shift: removed sign or zero extend to shift amount in memory in method %s\n",
                            comp->signature());
                } else if (secondChild->getOpCode().isLoadVarDirect() && grandChild->getReferenceCount() == 1 && grandChild->getRegister() == 0) {
        ...
        if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift() && targetRegister->containsCollectedReference()) {
            TR::Register* origTargetRegister = targetRegister;
            targetRegister = cg->allocateRegister();
            instr = generateRegRegInstruction(MOVRegReg(nodeIs64Bit), node, targetRegister, origTargetRegister, cg);
        }
    ...
    if (loadConstant && performTransformation(comp, "O^O GenerateMemoryShift: load is not constant %d\n", loadConstant)) {
        ...
        if (op == TR::su2i || op == TR::s2i || op == TR::b2i || op == TR::bu2i || op == TR::l2i) {
            if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == 0) {
                ...
                if (grandChild->getOpCode().isLoadIndirect() && grandChild->getReferenceCount() == 1 && grandChild->getRegister() == 0) {
                    TR::Node::recreate(grandChild, TR::bloadi);
                    secondChild->decReferenceCount();
                    secondChild = grandChild;
                    if (reportShiftAmount)
                        diagnostic("shift: removed sign or zero extend to shift amount in memory in method %s\n", comp->signature());
                } else if (grandChild->getOpCode().isLoadVarDirect() && grandChild->getReferenceCount() == 1 && grandChild->getRegister() == 0) {
                    TR::Node::recreate(grandChild, TR::bload);
                    secondChild->decReferenceCount();
                    secondChild = grandChild;
                    if (reportShiftAmount)
...
TR::Register* OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else if (shiftAmountNode->getOpCode().isLoadConst() && (shiftAmount = TR::TreeEvaluator::integerConstNodeValue(shiftAmountNode, cg) & TR::TreeEvaluator::shiftMask(nodeIs64Bit), shiftAmount > 0 && shiftAmount <= 3) && performTransformation(comp, "O^O IntegerShlEvaluator: replace shift with lea\n")) {
...
TR::Register* OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (comp->useCompressedPointers() && nodeIs64Bit && (TR::Compiler->vm.heapBaseAddress() == 0) && (node->getFirstChild()->getOpCodeValue() == TR::a2l)) {
...
TR::Register* OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else if ((testOpcode1 == TR::bconst || testOpcode1 == TR::buconst) && performTransformation(comp, "O^O BUSHREvaluator: first child is not an 8-bit signed two's complement, or an 8 bit unsigned %x\n", testOpcode1)) {
        targetRegister = cg->allocateRegister();
    ...
    if ((testOpcode2 == TR::bconst || testOpcode2 == TR::buconst) && performTransformation(comp, "O^O BUSHREvaluator: first child is not an 8-bit signed two's complement, or an 8 bit unsigned %x\n", testOpcode2)) {
    ...
    TR::CodeGenerator* cg)
{
    ...
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && performTransformation(comp, "O^O LogicalEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && IS_32BIT_SIGNED(constValue) && performTransformation(comp, "O^O checking that the store has not happened yet. Target register: %x\n", testRegister)) {
