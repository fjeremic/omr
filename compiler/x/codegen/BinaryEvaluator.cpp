...
extern TR::Register *intOrLongClobberEvaluate(TR::Node *node, bool nodeIs64Bit,
                                              TR::CodeGenerator *cg);
...
//
static void forceSize(TR::Node *node, TR::Register *reg, bool is64Bit,
                      TR::CodeGenerator *cg) {
...
inline TR::Register *evaluateAndForceSize(TR::Node *node, bool is64Bit,
                                          TR::CodeGenerator *cg) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    TR_ASSERT(!is64Bit,
              "64-bit expressions should be using register pairs on IA32");
  }
  return reg;
}

bool OMR::X86::TreeEvaluator::analyseSubForLEA(TR::Node *node,
                                               TR::CodeGenerator *cg) {
  ...
  intptrj_t displacement =
      -TR::TreeEvaluator::integerConstNodeValue(secondChild, cg);
  ...
  if (firstChild->getRegister() == NULL &&
    ...
    if (stride) {
      ...
      leaMR = generateX86MemoryReference(NULL, indexRegister, stride,
                                         displacement, cg);
      targetRegister = cg->allocateRegister();
      generateRegMemInstruction(LEARegMem(nodeIs64Bit), node, targetRegister,
                                leaMR, cg);
    ...
    if (firstOp.isAdd() && TR::TreeEvaluator::constNodeValueIs32BitSigned(
                               secondChild, &dummyConstValue, cg)) {
      // sub
      //    add         (firstChild)
      //       ?        (llChild)
      //       ?        (lrchild)
      ...
      if (llChild->getRegister() == NULL && llChild->getReferenceCount() == 1 &&
        ...
        leaMR = generateX86MemoryReference(
            cg->evaluate(lrChild), cg->evaluate(llChild->getFirstChild()),
        ...
        cg->decReferenceCount(llChild->getSecondChild());
                 ...
                 (stride =
                      TR::MemoryReference::getStrideForNode(lrChild, cg))) {
        // sub
        //    add         (firstChild)
        //       ?        (llchild)
        //       mul/shl  (lrChild)
        ...
        //
        leaMR = generateX86MemoryReference(
            cg->evaluate(llChild), cg->evaluate(lrChild->getFirstChild()),
            stride, displacement, cg);
        cg->decReferenceCount(lrChild->getFirstChild());
        cg->decReferenceCount(lrChild->getSecondChild());
      } else {
        leaMR = generateX86MemoryReference(
            cg->evaluate(llChild), cg->evaluate(lrChild), 0, displacement, cg);
      }
      targetRegister = cg->allocateRegister();
      generateRegMemInstruction(LEARegMem(nodeIs64Bit), node, targetRegister,
                                leaMR, cg);
...
bool OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node *node,
                                               TR::CodeGenerator *cg) {
  ...
  // TR_ASSERT(node->getSize() == firstChild->getSize(), "evaluateAndForceSize
  // never needed for firstChild");
  ...
  if ((secondOp.isAdd() || secondOp.isSub()) &&
    ...
    if (stride && addFirstChild->getReferenceCount() == 1 &&
        addFirstChild->getRegister() == NULL && addSecondOp.isLoadConst()) {
      // add
      ...
      intptrj_t offset =
          TR::TreeEvaluator::integerConstNodeValue(addSecondChild, cg);
      if (secondOp.isSub()) offset = -offset;
      ...
      leaMR = generateX86MemoryReference(baseRegister, indexRegister, stride,
                                         offset, cg);
      targetRegister = cg->allocateRegister();
      generateRegMemInstruction(LEARegMem(nodeIs64Bit), node, targetRegister,
                                leaMR, cg);
  ...
  if (indexNode) {
    ...
    if (constNode) {
      ...
      tempNode = NULL;
               ...
               TR::TreeEvaluator::constNodeValueIs32BitSigned(
                   baseNode->getSecondChild(), &dummyConstValue, cg)) {
      // add            (** children may be reversed **)
      //    add            (baseNode)
      //       ?
      //       const
      ...
      leaMR = generateX86MemoryReference(
          cg->evaluate(baseNode->getFirstChild()), indexRegister, stride,
          TR::TreeEvaluator::integerConstNodeValue(constNode, cg), cg);
      tempNode = baseNode->getFirstChild();
    } else {
      leaMR = generateX86MemoryReference(cg->evaluate(baseNode), indexRegister,
                                         stride, 0, cg);
      tempNode = baseNode;
    }
    targetRegister = cg->allocateRegister();
    ...
    node->setRegister(targetRegister);
    return true;
  } else if (constNode && TR::TreeEvaluator::constNodeValueIs32BitSigned(
                              constNode, &dummyConstValue, cg)) {
    // add
    //    firstChild
    //    const
    //
    if (firstChild->getRegister() == NULL &&
        firstChild->getReferenceCount() == 1 && firstOp.isAdd()) {
      // add
      //    add   (firstChild)
      //       ?
      //       ?
      //    const
      //
      // We should only traverse more deeply if firstChild's children do not
      // have associated registers. Otherwise, we risk extending the lifetimes
      // of firstChild's grandchildren's registers, and hence adding register
      // pressure.
      //
      if (firstChild->getFirstChild()->getReferenceCount() == 1 &&
          firstChild->getFirstChild()->getRegister() == NULL) {
        stride1 = TR::MemoryReference::getStrideForNode(
            firstChild->getFirstChild(), cg);
      }
      if (firstChild->getSecondChild()->getReferenceCount() == 1 &&
          firstChild->getSecondChild()->getRegister() == NULL) {
        stride2 = TR::MemoryReference::getStrideForNode(
            firstChild->getSecondChild(), cg);
      }
      if (stride1 | stride2) {
        if (stride1) {
          // add
          ...
          leaMR = generateX86MemoryReference(
              cg->evaluate(baseNode), cg->evaluate(indexNode), stride1,
              TR::TreeEvaluator::integerConstNodeValue(constNode, cg), cg);
          ...
          TR_ASSERT(node->getSize() == baseNode->getSize(),
                    "evaluateAndForceSize never needed for baseNode");
          leaMR = generateX86MemoryReference(
              cg->evaluate(baseNode), cg->evaluate(indexNode), stride2,
              TR::TreeEvaluator::integerConstNodeValue(constNode, cg), cg);
    ...
    //    loadaddr    (firstChild)
    //    ?           (secondChild)
    // fold parent add of 'loadaddr + secondChild' into lea generated for the
    // loadaddr
    ...
    //
    // leaMR is not first chid's memory reference, it also plus second second
    // child, so can't allocate register directly according to first kid's
    // symbol
    ...
    return true;
             ...
             (firstChild->getOpCode().getSize() ==
              node->getOpCode().getSize()) &&
             (firstChild->getRegister() == NULL) &&
             (firstChild->getSecondChild()->getOpCode().isLoadConst()) &&
             TR::TreeEvaluator::constNodeValueIs32BitSigned(
                 firstChild->getSecondChild(), &dummyConstValue, cg) &&
             (firstChild->getReferenceCount() == 1)) {
    // add
    //    sub/add    (firstChild)
    //        child 0
    //        loadconstant
    //    ?        (second)
    intptrj_t val = TR::TreeEvaluator::integerConstNodeValue(
        firstChild->getSecondChild(), cg);
    if (firstChild->getOpCode().isSub()) val = -val;
    leaMR =
        generateX86MemoryReference(cg->evaluate(firstChild->getFirstChild()),
                                   cg->evaluate(secondChild), 0, val, cg);
    targetRegister = cg->allocateRegister();
    generateRegMemInstruction(LEARegMem(nodeIs64Bit), node, targetRegister,
                              leaMR, cg);
    node->setRegister(targetRegister);
...
TR::Register *OMR::X86::TreeEvaluator::integerDualAddOrSubEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR_ASSERT((node->getOpCodeValue() == TR::luaddh) ||
                (node->getOpCodeValue() == TR::luadd) ||
                (node->getOpCodeValue() == TR::lusubh) ||
                (node->getOpCodeValue() == TR::lusub),
            "Unexpected dual operator. Expected luadd, luaddh, lusub, or "
            "lusubh as part of cyclic dual.");
  ...
  if (pair->getReferenceCount() == 1) {
    diagnostic("Found %p (%s) with only reference to %p (%s)\n", node,
               node->getOpCode().getName(), pair, pair->getOpCode().getName());
    if (node->isDualHigh()) {
      // for high part carry is still required, so evaluate the children for low
      // part for this first
      diagnostic("Evaluating Children of %p (%s) for %p (%s)\n", pair,
                 pair->getOpCode().getName(), node,
                 node->getOpCode().getName());
      evaluateIfNotConst(pair->getFirstChild(), cg);
      evaluateIfNotConst(pair->getSecondChild(), cg);
    } else {
      // for low part, high part is not required, but children must be
      // evaluated.
      diagnostic(
          "Evaluating Children of %p (%s) for %p (%s) (not required for quad "
          "calc)\n",
    ...
    if (node->isDualHigh()) {
      // for high part carry is still required, so evaluate the low part for
      // this first
    ...
    diagnostic("Evaluating Children of %p (%s) for %p (%s)\n", lowNode,
               lowNode->getOpCode().getName(), highNode,
    ...
    cg->evaluate(lowNode);
    diagnostic("Evaluating %p (%s) after %p (%s)\n", highNode,
               highNode->getOpCode().getName(), lowNode,
               lowNode->getOpCode().getName());
    cg->setRequiresCarry(true);
    cg->setComputesCarry(true);
...
TR::Register *OMR::X86::TreeEvaluator::integerAddEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  computesCarry =
      computesCarry || isWithCarry || node->nodeRequiresConditionCodes();

  if (NEED_CC(node) || (opCode == TR::luaddc) || (opCode == TR::iuaddc)) {
    TR_ASSERT(
        (nodeIs64Bit && (opCode == TR::ladd || opCode == TR::luadd) ||
         opCode == TR::luaddc) ||
            (!nodeIs64Bit && (opCode == TR::iadd || opCode == TR::iuadd) ||
             opCode == TR::iuaddc),
    ...
    TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(
        node, AddRegReg(nodeIs64Bit, isWithCarry),
        AddRegMem(nodeIs64Bit, isWithCarry), true, /* produce eflags */
  ...
  if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
    ...
    if (!targetRegister && secondChild->getOpCode().isLoadConst() &&
        (secondChild->getRegister() == NULL) &&
        ((TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild,
                                                         &constValue, cg)) ||
         (comp->useCompressedPointers() &&
          (constValue == TR::Compiler->vm.heapBaseAddress()) &&
          firstChild->getReferenceCount() > 1 && !isMemOp)) &&
        performTransformation(comp,
                              "O^O analyseAddForLEA: checking that second node "
                              "is a memory reference %x\n",
                              constValue)) {
      if (!isMemOp) targetRegister = cg->evaluate(firstChild);

      bool internalPointerMismatch = false;

      if (targetRegister && node->getOpCode().isArrayRef() &&
          node->isInternalPointer()) {
        if (targetRegister->containsInternalPointer()) {
          if (node->getPinningArrayPointer() !=
              targetRegister->getPinningArrayPointer()) {
      ...
      if (targetRegister &&
          (firstChild->getReferenceCount() > 1 ||
           (node->getOpCode().isArrayRef() &&
            (internalPointerMismatch ||
             targetRegister->containsCollectedReference())))) {
        TR::Register *firstOperandReg = targetRegister;

        // For a non-internal pointer TR::aiadd created when merging news (for
        // example) commoning is permissible across a GC point; however the
        // register needs to be marked as a collected reference for GC to behave
        // correctly; note that support for internal pointer TR::aiadd (e.g.
        // array access) is not present still
        //
        if (targetRegister->containsCollectedReference() &&
            (node->getOpCode().isArrayRef()) && !node->isInternalPointer()) {
        ...
        // ladd
        //    ==>iu2l
        // the firstChild is commoned, so the add & iu2l will get different
        // registers
        //
        TR::Register *tempReg = firstOperandReg;
        if (TR::TreeEvaluator::genNullTestSequence(node, firstOperandReg,
                                                   targetRegister, cg))
          tempReg = targetRegister;

        if (computesCarry) {
          generateRegRegInstruction(MOVRegReg(nodeIs64Bit), node,
                                    targetRegister, tempReg, cg);
          generateRegImmInstruction(AddRegImm4(nodeIs64Bit, isWithCarry), node,
                                    targetRegister, constValue, cg);
        } else {
          tempMR = generateX86MemoryReference(tempReg, constValue, cg);
          generateRegMemInstruction(LEARegMem(nodeIs64Bit), node,
                                    targetRegister, tempMR, cg);
        }
      } else {
        if (constValue >= -128 && constValue <= 127) {
          if (computesCarry) {
            if (isMemOp)
              instr = generateMemImmInstruction(
                  AddMemImms(nodeIs64Bit, isWithCarry), node, tempMR,
                  constValue, cg);
            else
              instr = generateRegImmInstruction(
                  AddRegImms(nodeIs64Bit, isWithCarry), node, targetRegister,
                  constValue, cg);
          } else if (constValue == 1) {
            if (isMemOp)
              instr =
                  generateMemInstruction(INCMem(nodeIs64Bit), node, tempMR, cg);
            else
              instr = generateRegImmInstruction(ADDRegImms(nodeIs64Bit), node,
                                                targetRegister, 1, cg);
          } else if (constValue == -1) {
            if (isMemOp)
              instr =
                  generateMemInstruction(DECMem(nodeIs64Bit), node, tempMR, cg);
            else
              instr = generateRegImmInstruction(SUBRegImms(nodeIs64Bit), node,
                                                targetRegister, 1, cg);
          } else {
            if (isMemOp)
              instr = generateMemImmInstruction(ADDMemImms(nodeIs64Bit), node,
                                                tempMR, constValue, cg);
            else
              instr = generateRegImmInstruction(ADDRegImms(nodeIs64Bit), node,
                                                targetRegister, constValue, cg);
          }
          ...
          if (isMemOp)
            instr = generateMemImmInstruction(SUBMemImms(nodeIs64Bit), node,
                                              tempMR, (uint32_t)-128, cg);
          else
            instr =
                generateRegImmInstruction(SUBRegImms(nodeIs64Bit), node,
                                          targetRegister, (uint32_t)-128, cg);
        } else {
          // comp()->useCompressedPointers
          // ladd
          //    iu2l
          ...
          //
          TR::TreeEvaluator::genNullTestSequence(node, targetRegister,
                                                 targetRegister, cg);

          if (isMemOp)
            instr =
                generateMemImmInstruction(AddMemImm4(nodeIs64Bit, isWithCarry),
                                          node, tempMR, constValue, cg);
          else
            instr =
                generateRegImmInstruction(AddRegImm4(nodeIs64Bit, isWithCarry),
                                          node, targetRegister, constValue, cg);
        }
        if (debug("traceMemOp"))
          diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d",
                     node, constValue);
      }
    } else if (isMemOp) {
      instr =
          generateMemRegInstruction(AddMemReg(nodeIs64Bit, isWithCarry), node,
                                    tempMR, cg->evaluate(secondChild), cg);
      if (debug("traceMemOp"))
    ...
    if (isMemOp) {
      ...
      temp.integerAddAnalyser(node, AddRegReg(nodeIs64Bit, isWithCarry),
...
TR::Register *OMR::X86::TreeEvaluator::baddEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  // This comment has been tested on X86_32 and X86_64 Linux only, and has not
  // been tested on any Windows operating systems.
  // TR_ASSERT(TR::Compiler->target.is32Bit(), "AMD64 baddEvaluator not yet
  // supported");
  ...
  if (NEED_CC(node)) {
    ...
    // we need eflags from integerAddAnalyser for the CC sequence
    TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(
        node, ADD1RegReg, ADD1RegMem, true /* produce eflags */);
    targetRegister = node->getRegister();
    return targetRegister;
  }
  ...
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL &&
      performTransformation(comp,
                            "O^O BaddEvaluator: checking that the store has "
                            "not happened yet. Target register: %x\n",
                            testRegister)) {
    ...
    if (targetRegister && firstChild->getReferenceCount() > 1) {
      ...
      if (value == 1) {
        ...
        if (isMemOp)
          instr =
              generateMemImmInstruction(ADD1MemImm1, node, tempMR, value, cg);
        else
          instr = generateRegImmInstruction(ADD1RegImm1, node, targetRegister,
                                            value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d",
    ...
    instr = generateMemRegInstruction(ADD1MemReg, node, tempMR,
...
TR::Register *OMR::X86::TreeEvaluator::saddEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (NEED_CC(node)) {
    ...
    // we need eflags from integerAddAnalyser for the CC sequence
    TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(
        node, ADD2RegReg, ADD2RegMem, true /* produce eflags */);
    targetRegister = node->getRegister();
    return targetRegister;
  }
  ...
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL &&
      performTransformation(comp,
                            "O^O SaddEvaluator: checking that the store has "
                            "not happened yet. Target register: %x\n",
                            testRegister)) {
    ...
    if (targetRegister && firstChild->getReferenceCount() > 1) {
      ...
      if (value >= -128 && value <= 127) {
        if (value == 1) {
          ...
          if (isMemOp)
            instr =
                generateMemImmInstruction(ADD2MemImms, node, tempMR, value, cg);
          else
            instr = generateRegImmInstruction(ADD4RegImms, node, targetRegister,
                                              value, cg);
        }
      } else if (value == 128) {
        if (isMemOp)
          instr = generateMemImmInstruction(SUB2MemImms, node, tempMR,
                                            (uint32_t)-128, cg);
        else
          instr = generateRegImmInstruction(SUB4RegImms, node, targetRegister,
                                            (uint32_t)-128, cg);
      } else {
        if (isMemOp)
          instr =
              generateMemImmInstruction(ADD2MemImm2, node, tempMR, value, cg);
        else
          instr = generateRegImmInstruction(ADD2RegImm2, node, targetRegister,
                                            value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by const %d",
    ...
    instr = generateMemRegInstruction(ADD2MemReg, node, tempMR,
                                      cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] inc by var", node);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::caddEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  TR_ASSERT(TR::Compiler->target.is32Bit(),
            "AMD64 baddEvaluator not yet supported");
  ...
  TR::Register *testRegister = secondChild->getRegister();
  if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL &&
      performTransformation(comp,
                            "O^O CaddEvaluator: checking that the store has "
                            "not happened yet. Target register:  %x\n",
                            testRegister)) {
    ...
    if (targetRegister && firstChild->getReferenceCount() > 1) {
      TR::MemoryReference *tempMR =
          generateX86MemoryReference(targetRegister, value, cg);
      targetRegister = cg->allocateRegister();
      generateRegMemInstruction(LEA4RegMem, node, targetRegister, tempMR, cg);
    } else {
...
TR::Register *OMR::X86::TreeEvaluator::integerSubEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  const TR::ILOpCodes opCode = node->getOpCodeValue();
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Node *firstChild = node->getFirstChild();
  ...
  computesCarry =
      computesCarry || isWithBorrow || node->nodeRequiresConditionCodes();

  if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb) ||
      (node->getOpCodeValue() == TR::iusubb)) {
    TR_ASSERT(
        (nodeIs64Bit && (opCode == TR::lsub || opCode == TR::lusub) ||
         opCode == TR::lusubb) ||
            (!nodeIs64Bit && (opCode == TR::isub || opCode == TR::iusub) ||
             opCode == TR::iusubb),
        "CC computation not supported for this node %p with opcode %s\n", node,
        cg->comp()->getDebug()->getName(opCode));

    const bool isWithBorrow = (opCode == TR::lusubb) || (opCode == TR::iusubb);
    TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(
        node, SubRegReg(nodeIs64Bit, isWithBorrow),
        SubRegMem(nodeIs64Bit, isWithBorrow), MOVRegReg(nodeIs64Bit), true,
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
      TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue,
                                                     cg) &&
      performTransformation(
          comp,
          "O^O IntegerSubEvaluator: register is not NULL, or second operand is "
          "not a 32 bit constant. Register value: %x\n",
          testRegister)) {
    if (!computesCarry && TR::TreeEvaluator::analyseSubForLEA(node, cg)) {
      return node->getRegister();
    }
    if (!isMemOp) targetRegister = cg->evaluate(firstChild);
    if (targetRegister && !computesCarry &&
        firstChild->getReferenceCount() > 1) {
      ...
      if (constValue >= -128 && constValue <= 127) {
        if (computesCarry) {
          if (isMemOp)
            instr =
                generateMemImmInstruction(SubMemImms(nodeIs64Bit, isWithBorrow),
                                          node, tempMR, constValue, cg);
          else
            instr =
                generateRegImmInstruction(SubRegImms(nodeIs64Bit, isWithBorrow),
                                          node, targetRegister, constValue, cg);
        } else if (constValue == 1) {
          if (isMemOp)
            instr =
        ...
        if (isMemOp)
          instr =
              generateMemImmInstruction(SubMemImm4(nodeIs64Bit, isWithBorrow),
                                        node, tempMR, constValue, cg);
        else
          instr =
              generateRegImmInstruction(SubRegImm4(nodeIs64Bit, isWithBorrow),
                                        node, targetRegister, constValue, cg);
      }

      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by const %d",
                   node, constValue);
    }
  } else if (isMemOp) {
    instr =
        generateMemRegInstruction(SubMemReg(nodeIs64Bit, isWithBorrow), node,
                                  tempMR, cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
  } else {
    TR_X86SubtractAnalyser temp(cg);
    temp.integerSubtractAnalyser(node, SubRegReg(nodeIs64Bit, isWithBorrow),
                                 SubRegMem(nodeIs64Bit, isWithBorrow),
                                 MOVRegReg(nodeIs64Bit), computesCarry, 0);
...
TR::Register *OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (NEED_CC(node)) {
    TR_ASSERT(node->getOpCodeValue() == TR::bsub ||
                  node->getOpCodeValue() == TR::busub,
              "CC computation not supported for this node %p with opcode %s\n",
              node, cg->comp()->getDebug()->getName(node->getOpCode()));

    // we need eflags from integerAddAnalyser for the CC sequence
    TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(
        node, SUB1RegReg, SUB1RegMem, MOV1RegReg, true /* produce eflags */);
    targetRegister = node->getRegister();
    return targetRegister;
  }
  ...
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL &&
      performTransformation(comp,
                            "O^O BSUBEvaluator: checking that the store has "
                            "not happened yet. Target register:  %x\n",
                            testRegister)) {
    int32_t value = secondChild->getByte();
    if (!isMemOp) targetRegister = cg->evaluate(firstChild);
    if (targetRegister && firstChild->getReferenceCount() > 1) {
      ...
      if (value == 1) {
        ...
        if (isMemOp)
          instr =
              generateMemImmInstruction(SUB1MemImm1, node, tempMR, value, cg);
        else
          instr = generateRegImmInstruction(SUB1RegImm1, node, targetRegister,
                                            value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by const %d",
    ...
    instr = generateMemRegInstruction(SUB1MemReg, node, tempMR,
                                      cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (NEED_CC(node)) {
    ...
    // we need eflags from integerAddAnalyser for the CC sequence
    TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(
        node, SUB2RegReg, SUB2RegMem, MOV4RegReg, true /* produce eflags */);
    targetRegister = node->getRegister();
    return targetRegister;
  }
  ...
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL &&
      performTransformation(comp,
                            "O^O SSUBEvaluator: checking that the store has "
                            "not happened yet. Target register:  %x\n",
                            testRegister)) {
    int32_t value = secondChild->getShortInt();
    if (!isMemOp) targetRegister = cg->evaluate(firstChild);
    if (firstChild->getReferenceCount() > 1) {
      ...
      if (value >= -128 && value <= 127) {
        if (value == 1) {
          ...
          if (isMemOp)
            instr =
                generateMemImmInstruction(SUB2MemImms, node, tempMR, value, cg);
          else
            instr = generateRegImmInstruction(SUB4RegImms, node, targetRegister,
                                              value, cg);
        }
        ...
        if (isMemOp)
          instr =
              generateMemImmInstruction(SUB2MemImm2, node, tempMR, value, cg);
        else
          instr = generateRegImmInstruction(SUB2RegImm2, node, targetRegister,
                                            value, cg);
      }
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by const %d",
    ...
    instr = generateMemRegInstruction(SUB2MemReg, node, tempMR,
                                      cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
  } else {
    ...
    temp.integerSubtractAnalyser(
        node, SUB4RegReg, SUB2RegMem,
        MOV4RegReg);  // Use MOV4 to avoid unnecessary size prefix
    return node->getRegister();
  }
...
TR::Register *OMR::X86::TreeEvaluator::csubEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL &&
      performTransformation(comp,
                            "O^O CSubEvaluator: checking that the store has "
                            "not happened yet. Target register: %x\n",
                            testRegister)) {
    int32_t value = secondChild->getConst<uint16_t>();
    if (!isMemOp) targetRegister = cg->evaluate(firstChild);
...
TR::Register *OMR::X86::TreeEvaluator::integerDualMulEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (node->isDualCyclic() || needsHighMulOnly) {
    ...
    // result <-- RAX, but by-product is RDX will be required at later
    // evaluation of lumulh (or result/by-product may be reversed, if lumulh
    // encountered first, and RAX may not be required if lumulh is on its own)
    TR::RegisterDependencyConditions *multDependencies =
    ...
    lumulhTargetRegister =
        cg->longClobberEvaluate(lumulhNode->getSecondChild());

    multDependencies->addPreCondition(lmulTargetRegister, TR::RealRegister::eax,
                                      cg);
    multDependencies->addPreCondition(lumulhTargetRegister,
                                      TR::RealRegister::edx, cg);
    multDependencies->addPostCondition(lmulTargetRegister,
                                       TR::RealRegister::eax, cg);
    multDependencies->addPostCondition(lumulhTargetRegister,
                                       TR::RealRegister::edx, cg);

    // TODO: sometimes acc-mem may be more efficient
    generateRegRegInstruction(MUL8AccReg, node, lmulTargetRegister,
                              lumulhTargetRegister, multDependencies, cg);
...
TR::Register *OMR::X86::TreeEvaluator::integerMulEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      ...
      generateRegRegInstruction(XOR4RegReg, node, targetRegister,
                                targetRegister, cg);
    } else {
      bool canClobberSource;

      if (SHIFT_MAY_HAVE_ADDRESS_CHILD &&
          firstChild->getDataType() == TR::Address) {
      ...
      TR_X86IntegerMultiplyDecomposer *mulDecomposer =
          new (cg->trHeapMemory()) TR_X86IntegerMultiplyDecomposer(
              value, firstChild->getRegister(), node, cg, canClobberSource);
      int32_t dummy;
      ...
      if (targetRegister == 0 &&
          IS_32BIT_SIGNED(value))  // decomposition failed
      {
        ...
        if (firstChild->getReferenceCount() > 1 ||
          ...
          targetRegister = cg->allocateRegister();
          generateRegRegImmInstruction(opCode, node, targetRegister,
                                       cg->evaluate(firstChild), value, cg);
        } else {
          if (firstChild->getOpCode().isMemoryReference()) {
            ...
            TR::MemoryReference *tempMR =
                generateX86MemoryReference(firstChild, cg);
            targetRegister = cg->allocateRegister();
            generateRegMemImmInstruction(opCode, node, targetRegister, tempMR,
                                         value, cg);
            tempMR->decNodeReferenceCounts(cg);
          } else {
            if (value >= -128 && value <= 127) {
    ...
    if (targetRegister) {
      node->setRegister(targetRegister);
      if (!firstChildRefCountIsZero) cg->decReferenceCount(firstChild);
      cg->decReferenceCount(secondChild);
    }
  }
  ...
  if (targetRegister == 0) {
    TR_X86BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, IMULRegReg(nodeIs64Bit), IMULRegMem(nodeIs64Bit),
...
TR::Register *OMR::X86::TreeEvaluator::integerMulhEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (firstChild->getReferenceCount() > 1)
      cg->evaluate(firstChild);  // Respect the evaluation point of the child --
                                 // TODO: this could be sub-optimal and is
                                 // likely best handled elsewhere
    cg->recursivelyDecReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);

    TR::Register *targetRegister = cg->allocateRegister();
    generateRegRegInstruction(XOR4RegReg, node, targetRegister, targetRegister,
                              cg);
    node->setRegister(targetRegister);
    return targetRegister;
  }
  ...
  //
  TR::Register *targetRegister =
      TR::TreeEvaluator::intOrLongClobberEvaluate(secondChild, nodeIs64Bit, cg);
  TR::Register *eaxRegister =
      TR::TreeEvaluator::intOrLongClobberEvaluate(firstChild, nodeIs64Bit, cg);
  ...
  TR_ASSERT(node->getOpCodeValue() == TR::imulh,
            "IMULAccReg only works for imulh");
  generateRegRegInstruction(IMULAccReg(nodeIs64Bit), node, eaxRegister,
                            targetRegister, multDependencies, cg);
...
TR::Register *OMR::X86::TreeEvaluator::signedIntegerDivOrRemAnalyser(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Node *dividend = node->getFirstChild();
  TR::ILOpCode &rootOpCode = node->getOpCode();
  intptrj_t dvalue =
      TR::TreeEvaluator::integerConstNodeValue(node->getSecondChild(), cg);
  ...
  TR::Register *dividendRegister =
      TR::TreeEvaluator::intOrLongClobberEvaluate(dividend, nodeIs64Bit, cg);
  ...
  if (isPowerOf2(dvalue)) {
    ...
    if (rootOpCode.isRem()) {
      ...
      if (isMinInt) {
        ...
        // is negative.
        //
        generateRegRegInstruction(TESTRegReg(nodeIs64Bit), node, tempRegister,
                                  tempRegister, cg);
        ...
        if (1 <= dvalue && dvalue <= 0x40000000) {
          ...
          generateRegImmInstruction(AND8RegImm4, node, tempRegister,
          ...
          //
          generateRegImmInstruction(ROL8RegImm1, node, tempRegister, 1, cg);
          generateRegRegInstruction(MOVZXReg8Reg4, node, tempRegister,
                                    tempRegister, cg);
          generateRegImmInstruction(ROR8RegImm1, node, tempRegister, 1, cg);
        } else {
          TR_ASSERT(dvalue > CONSTANT64(0x80000000),
                    "dvalue should not be negative at this point; "
                    "TR::getMinSigned<TR::Int64>() is handled elsewhere");
        ...
        generateRegImmInstruction(AND4RegImm4, node, tempRegister,
                                  (uint32_t)SIGN32 + dvalue - 1, cg);
        generateLabelInstruction(JNS4, node, doneLabel, cg);
      }
      ...
      if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
        ...
        generateRegImmInstruction(ORRegImm4(nodeIs64Bit), node, tempRegister,
                                  (uint32_t)-dvalue, cg);
        ...
        if (dvalue == CONSTANT64(0x100000000)) {
          // Can't set exactly 32 bits with one OR operation, so we need to be
          // sneaky.
          //
          // Clear top 32 bits & flip bottom 32 bits
          generateRegImmInstruction(XOR4RegImms, node, tempRegister,
                                    (unsigned)-1, cg);
          // Flip all 64 bits
          generateRegImmInstruction(XOR8RegImms, node, tempRegister,
                                    (unsigned)-1, cg);
          // Now the top 32 bits are set, and bottom 32 bits have their original
          // values
        } else {
          // Rotate so sign bit is at bit 31
          generateRegImmInstruction(ROR8RegImm1, node, tempRegister, 33, cg);
          // Set the appropriate bits, making sure the Imm4 has its sign bit
          // clear to leave the top 33 bits alone
          generateRegImmInstruction(
              OR8RegImm4, node, tempRegister,
    ...
    // If dividend is negative, we must add dvalue-1 to it in order to get the
    // right result from the SAR. We use CDQ (or CQO) to allow us to do this
    // without introducing control flow.
    //
    if ((dividend->isNonNegative() == false) && (dvalue > 0 || isMinInt)) {
      ...
      cdqDependencies->addPostCondition(tempRegister, TR::RealRegister::eax,
                                        cg);
      ...
      {
        generateRegRegInstruction(SUBRegReg(nodeIs64Bit), node, tempRegister,
                                  edxRegister, cg);
      } else if (!nodeIs64Bit ||
                 (dvalue > 0 && dvalue <= CONSTANT64(0x80000000))) {
        int32_t mask = dvalue - 1;
        TR_ASSERT(mask >= 0,
                  "AMD64: mask must have sign bit clear so that, when "
                  "sign-extended, it clears the upper 32 bits of tempRegister");
        generateRegImmInstruction(ANDRegImm4(nodeIs64Bit), node, edxRegister,
                                  (uint32_t)dvalue - 1, cg);
        generateRegRegInstruction(ADDRegReg(nodeIs64Bit), node, tempRegister,
                                  edxRegister, cg);
      } else {
        // dvalue-1 is too big for an Imm4, so shift off the bits instead.
        //
        int32_t shiftAmount = leadingZeroes(dvalue) + 1;
        generateRegImmInstruction(SHL8RegImm1, node, edxRegister, shiftAmount,
                                  cg);
        generateRegImmInstruction(SHR8RegImm1, node, edxRegister, shiftAmount,
                                  cg);
        generateRegRegInstruction(ADDRegReg(nodeIs64Bit), node, tempRegister,
                                  edxRegister, cg);
      }
    ...
    generateRegImmInstruction(SARRegImm1(nodeIs64Bit), node, tempRegister,
                              trailingZeroes(dvalue), cdqDependencies, cg);

    if (isNegative) {
      generateRegInstruction(NEGReg(nodeIs64Bit), node, tempRegister,
                             cdqDependencies, cg);
    }
    ...
    if (nodeIs64Bit) {
      // This prevents a compile error on IA32, where intptrj_t can't be cast to
      // int64_t.
      //
      int64_t tm, ts;
    ...
    TR::RegisterDependencyConditions *multDependencies =
        generateRegisterDependencyConditions((uint8_t)2, 2, cg);
    if (!rootOpCode.isRem() && !(dvalue > 0 && m < 0) &&
        !(dvalue < 0 && m > 0)) {
      multDependencies->addPreCondition(eaxRegister, TR::RealRegister::eax, cg);
      multDependencies->addPreCondition(dividendRegister, TR::RealRegister::edx,
                                        cg);
      multDependencies->addPostCondition(eaxRegister, TR::RealRegister::eax,
                                         cg);
      multDependencies->addPostCondition(dividendRegister,
                                         TR::RealRegister::edx, cg);
    } else {
      ...
      multDependencies->addPostCondition(eaxRegister, TR::RealRegister::eax,
                                         cg);
      multDependencies->addPostCondition(edxRegister, TR::RealRegister::edx,
                                         cg);
    }
    ...
    if (!nodeIs64Bit || IS_32BIT_SIGNED(m))
      generateRegImmInstruction(MOVRegImm4(nodeIs64Bit), node, eaxRegister, m,
                                cg);
    else
      generateRegMemInstruction(LEA8RegMem, node, eaxRegister,
                                generateX86MemoryReference(m, cg), cg);
    generateRegRegInstruction(IMULAccReg(nodeIs64Bit), node, eaxRegister,
                              dividendRegister, multDependencies, cg);

    cg->stopUsingRegister(eaxRegister);

    // Some special adjustment (?) when multiplier and divisor have opposite
    // signs.
    //
    if ((dvalue > 0) && (m < 0)) {
      generateRegRegInstruction(ADDRegReg(nodeIs64Bit), node, edxRegister,
                                dividendRegister, cg);
    } else if ((dvalue < 0) && (m > 0)) {
      generateRegRegInstruction(SUBRegReg(nodeIs64Bit), node, edxRegister,
                                dividendRegister, cg);
    }

    // Do the magic shift
    //
    generateRegImmInstruction(SARRegImm1(nodeIs64Bit), node, edxRegister, s,
                              cg);
    ...
    if (!(dividend->isNonNegative() && (dvalue > 0))) {
      ...
      generateRegRegInstruction(MOVRegReg(nodeIs64Bit), node, eaxRegister,
                                edxRegister, cg);
      generateRegImmInstruction(SHRRegImm1(nodeIs64Bit), node, eaxRegister,
                                nodeIs64Bit ? 63 : 31, cg);
      generateRegRegInstruction(ADDRegReg(nodeIs64Bit), node, edxRegister,
                                eaxRegister, cg);
    ...
    if (rootOpCode.isRem()) {
      if (nodeIs64Bit && !IS_32BIT_SIGNED(dvalue)) {
        ...
        generateRegRegInstruction(IMULRegReg(nodeIs64Bit), node, edxRegister,
                                  scratchReg, cg);
        ...
        generateRegRegImmInstruction(opCode, node, edxRegister, edxRegister,
                                     dvalue, cg);
      }

      generateRegRegInstruction(SUBRegReg(nodeIs64Bit), node, dividendRegister,
                                edxRegister, multDependencies, cg);
      cg->stopUsingRegister(edxRegister);
...
TR::Register *OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  ...
  if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst() &&
      TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) != 0) {
    TR::Register *reg =
        TR::TreeEvaluator::signedIntegerDivOrRemAnalyser(node, cg);
    node->setRegister(reg);
    cg->decReferenceCount(firstChild);
    cg->decReferenceCount(secondChild);
    return reg;
  } else {
    bool needsExplicitOverflowCheck =
        !cg->enableImplicitDivideCheck() && !node->divisionCannotOverflow();

    TR::LabelSymbol *startLabel, *divisionLabel = NULL,
                                 *overflowSnippetLabel = NULL,
                                 *restartLabel = NULL;

    TR::Instruction *divideInstr = NULL;

    TR::Register *eaxRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(
        firstChild, nodeIs64Bit, cg);
    ...
    if (needsExplicitOverflowCheck) {
      ...
      TR_ASSERT(divisorRegister == NULL,
                "No source register because source is in memory");
    } else {
      divisorRegister = cg->evaluate(secondChild);
    }

    TR::RegisterDependencyConditions *edxDeps =
    ...
    if (needsExplicitOverflowCheck) {
      ...
      // Only TR::getMinSigned<TR::Int32>() (or TR::getMinSigned<TR::Int64>())
      // will overflow when compared with 1
      //
      generateLabelInstruction(LABEL, node, startLabel, cg);
      generateRegImmInstruction(CMPRegImms(nodeIs64Bit), node, eaxRegister, 1,
                                cg);
    ...
    if (!nodeIs64Bit && node->isUnsigned()) {
      generateRegRegInstruction(XORRegReg(nodeIs64Bit), node, edxRegister,
                                edxRegister, edxDeps, cg);

      if (divisorRegister) {
        divideInstr =
            generateRegRegInstruction(DIVAccReg(nodeIs64Bit), node, eaxRegister,
                                      divisorRegister, eaxEdxDeps, cg);
      } else {
        TR::MemoryReference *tempMR =
            generateX86MemoryReference(secondChild, cg);
        divideInstr = generateRegMemInstruction(
            DIVAccMem(nodeIs64Bit), node, eaxRegister, tempMR, eaxEdxDeps, cg);
        tempMR->decNodeReferenceCounts(cg);
      }
    } else {
      if (divisorRegister) {
        if (node->getFirstChild()->isNonNegative() ||
          ...
          generateRegRegInstruction(XOR4RegReg, node, edxRegister, edxRegister,
                                    edxDeps, cg);
        else
          generateInstruction(CXXAcc(nodeIs64Bit), node, eaxEdxDeps, cg);
        if (node->getOpCode().isUnsigned() ||
            (node->getFirstChild()->isNonNegative() &&
             node->getSecondChild()->isNonNegative()))
          divideInstr = generateRegRegInstruction(DIVAccReg(nodeIs64Bit), node,
                                                  eaxRegister, divisorRegister,
                                                  eaxEdxDeps, cg);
        else
          divideInstr = generateRegRegInstruction(IDIVAccReg(nodeIs64Bit), node,
                                                  eaxRegister, divisorRegister,
                                                  eaxEdxDeps, cg);
      } else {
        TR::MemoryReference *tempMR =
        ...
        if (node->getOpCode().isUnsigned() ||
            (node->getFirstChild()->isNonNegative() &&
             node->getSecondChild()->isNonNegative()))
          divideInstr =
              generateRegMemInstruction(DIVAccMem(nodeIs64Bit), node,
                                        eaxRegister, tempMR, eaxEdxDeps, cg);
        else
          divideInstr =
              generateRegMemInstruction(IDIVAccMem(nodeIs64Bit), node,
                                        eaxRegister, tempMR, eaxEdxDeps, cg);
        tempMR->decNodeReferenceCounts(cg);
      }
    }
    ...
    if (needsExplicitOverflowCheck) {
      ...
      TR_ASSERT(divisorRegister != NULL,
                "Divide check snippet needs divisor in a register");
      cg->addSnippet(new (cg->trHeapMemory()) TR::X86DivideCheckSnippet(
          restartLabel, overflowSnippetLabel, divisionLabel, node->getOpCode(),
          node->getType(),
          divideInstr->getX86RegInstruction()->getIA32RegRegInstruction(), cg));
    }
...
TR::X86RegInstruction *OMR::X86::TreeEvaluator::generateRegisterShift(
    TR::Node *node, TR_X86OpCodes immShiftOpCode, TR_X86OpCodes regShiftOpCode,
    TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    intptrj_t shiftAmount =
        TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) &
        TR::TreeEvaluator::shiftMask(nodeIs64Bit);
    if (shiftAmount == 0) {
      targetRegister = cg->evaluate(firstChild);
    } else {
      targetRegister = cg->evaluate(firstChild);
      ...
      if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift() &&
        ...
        instr =
            generateRegRegInstruction(MOVRegReg(nodeIs64Bit), node,
                                      targetRegister, origTargetRegister, cg);
      } else {
        targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(
            firstChild,
            (SHIFT_MAY_HAVE_ADDRESS_CHILD)
                ? TR::TreeEvaluator::getNodeIs64Bit(firstChild, cg)
                : nodeIs64Bit,
            cg);
      }

      instr = generateRegImmInstruction(immShiftOpCode, node, targetRegister,
                                        shiftAmount, cg);
    }
  } else {
    TR::ILOpCodes op = secondChild->getOpCodeValue();
    TR::Register *shiftAmountReg = NULL;
    if (op == TR::su2i || op == TR::s2i || op == TR::b2i || op == TR::bu2i ||
        op == TR::l2i) {
      if (secondChild->getReferenceCount() == 1 &&
          secondChild->getRegister() == 0) {
        ...
        if (secondChild->getOpCode().isLoadIndirect() &&
          ...
          if (reportShiftAmount)
            diagnostic(
                "shift: removed sign or zero extend to shift amount in memory "
                "in method %s\n",
                comp->signature());
          ...
          if (reportShiftAmount)
            diagnostic(
                ...
                comp->signature());
                   ...
                   TR::Compiler->target.is64Bit())  // cannot use a reigster
                                                    // pair as the shift amount
        {
          secondChild->decReferenceCount();
          secondChild = grandChild;
          if (reportShiftAmount)
            diagnostic(
                "shift: removed sign or zero extend to shift amount in method "
                "%s\n",
                comp->signature());
    ...
    if (!shiftAmountReg) shiftAmountReg = cg->evaluate(secondChild);

    TR::RegisterDependencyConditions *shiftDependencies =
        generateRegisterDependencyConditions((uint8_t)1, 1, cg);
    shiftDependencies->addPreCondition(shiftAmountReg, TR::RealRegister::ecx,
                                       cg);
    shiftDependencies->addPostCondition(shiftAmountReg, TR::RealRegister::ecx,
                                        cg);
    targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(
        firstChild, nodeIs64Bit, cg);
    ...
    if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift() &&
      ...
      TR::Register *origTargetRegister = targetRegister;
      targetRegister = cg->allocateRegister();
      instr = generateRegRegInstruction(MOVRegReg(nodeIs64Bit), node,
                                        targetRegister, origTargetRegister, cg);
    }

    instr = generateRegRegInstruction(regShiftOpCode, node, targetRegister,
                                      shiftAmountReg, shiftDependencies, cg);
  }
...
TR::X86MemInstruction *OMR::X86::TreeEvaluator::generateMemoryShift(
    TR::Node *node, TR_X86OpCodes immShiftOpCode, TR_X86OpCodes regShiftOpCode,
    TR::CodeGenerator *cg) {
  ...
  if (loadConstant &&
      performTransformation(
          comp, "O^O GenerateMemoryShift: load is not constant %d\n",
          loadConstant)) {
    intptrj_t shiftAmount =
        TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) &
        TR::TreeEvaluator::shiftMask(nodeIs64Bit);
    if (shiftAmount != 0) {
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shift by const %d",
                   node, shiftAmount);
      instr = generateMemImmInstruction(immShiftOpCode, node, memRef,
                                        shiftAmount, cg);
    }
  } else {
    TR::ILOpCodes op = secondChild->getOpCodeValue();
    ...
    shiftDependencies->addPostCondition(shiftAmountReg, TR::RealRegister::ecx,
                                        cg);
    instr = generateMemRegInstruction(regShiftOpCode, node, memRef,
                                      shiftAmountReg, shiftDependencies, cg);
    if (debug("traceMemOp"))
...
TR::Register *OMR::X86::TreeEvaluator::integerShlEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  ...
  if (node->isDirectMemoryUpdate()) {
    TR::TreeEvaluator::generateMemoryShift(node, SHLMemImm1(nodeIs64Bit),
                                           SHLMemCL(nodeIs64Bit), cg);
  } else if (shiftAmountNode->getOpCode().isLoadConst() &&
             (shiftAmount = TR::TreeEvaluator::integerConstNodeValue(
                                shiftAmountNode, cg) &
                            TR::TreeEvaluator::shiftMask(nodeIs64Bit),
              shiftAmount > 0 && shiftAmount <= 3) &&
             performTransformation(
                 comp, "O^O IntegerShlEvaluator: replace shift with lea\n")) {
    ...
    TR::TreeEvaluator::generateRegisterShift(node, SHLRegImm1(nodeIs64Bit),
                                             SHLRegCL(nodeIs64Bit), cg);
  }
  return node->getRegister();
}

TR::Register *OMR::X86::TreeEvaluator::integerRolEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    intptrj_t rotateAmount =
        TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) &
        TR::TreeEvaluator::rotateMask(nodeIs64Bit);
    if (rotateAmount == 0) {
      targetRegister = cg->evaluate(firstChild);
    } else {
      targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(
          firstChild, nodeIs64Bit, cg);
      generateRegImmInstruction(ROLRegImm1(nodeIs64Bit), node, targetRegister,
                                rotateAmount, cg);
    }
  } else {
    targetRegister = TR::TreeEvaluator::intOrLongClobberEvaluate(
    ...
    TR::RegisterDependencyConditions *rotateDependencies =
        generateRegisterDependencyConditions((uint8_t)1, 1, cg);

    rotateDependencies->addPreCondition(rotateAmountReg, TR::RealRegister::ecx,
                                        cg);
    rotateDependencies->addPostCondition(rotateAmountReg, TR::RealRegister::ecx,
                                         cg);

    generateRegRegInstruction(ROLRegCL(nodeIs64Bit), node, targetRegister,
                              rotateAmountReg, rotateDependencies, cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::integerShrEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  if (node->isDirectMemoryUpdate()) {
    TR::TreeEvaluator::generateMemoryShift(node, SARMemImm1(nodeIs64Bit),
                                           SARMemCL(nodeIs64Bit), cg);
  } else {
    TR::TreeEvaluator::generateRegisterShift(node, SARRegImm1(nodeIs64Bit),
                                             SARRegCL(nodeIs64Bit), cg);
  }
  return node->getRegister();
}

TR::Register *OMR::X86::TreeEvaluator::integerUshrEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Compilation *comp = cg->comp();
  ...
  if (node->isDirectMemoryUpdate()) {
    TR::Instruction *shiftInstruction = TR::TreeEvaluator::generateMemoryShift(
        node, SHRMemImm1(nodeIs64Bit), SHRMemCL(nodeIs64Bit), cg);
    if (shiftInstruction) faultingInstruction = shiftInstruction;
  } else {
    TR::TreeEvaluator::generateRegisterShift(node, SHRRegImm1(nodeIs64Bit),
                                             SHRRegCL(nodeIs64Bit), cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (testOpCode == TR::bconst &&
      performTransformation(comp,
                            "O^O BSHLEvaluator: second child is not an 8-bit "
                            "signed, two's complement number: %x\n",
                            testOpCode)) {
    int32_t value = secondChild->getByte();
    if (isMemOp) {
      if (value != 0) {
        ...
        if (debug("traceMemOp"))
          diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by const %d",
                     node, value);
      }
    } else {
      if (value == 0) {
        ...
        targetRegister = cg->allocateRegister();
        instr = generateRegMemInstruction(LEA4RegMem, node, targetRegister,
                                          tempMR, cg);
      } else {
        targetRegister = cg->intClobberEvaluate(firstChild);
        instr = generateRegImmInstruction(SHL1RegImm1, node, targetRegister,
                                          value, cg);
      }
    }
  } else {
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(SHL1MemCL, node, tempMR, shiftAmountReg,
                                        shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
    } else {
      targetRegister = cg->intClobberEvaluate(firstChild);
      instr = generateRegRegInstruction(SHL1RegCL, node, targetRegister,
                                        shiftAmountReg, shiftDependencies, cg);
    }
  }
  ...
  if (isMemOp) {
    if (oursIsTheOnlyMemRef)
      ...
      tempMR->stopUsingRegisters(cg);
    if (instr) cg->setImplicitExceptionPoint(instr);
  } else {
...
TR::Register *OMR::X86::TreeEvaluator::sshlEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (testOpcode == TR::iconst &&
      performTransformation(comp,
                            "O^O SSHLEvaluator: second child is not a 16-bit "
                            "integer constant: %x\n",
                            testOpcode)) {
    ...
    if (isMemOp) {
      ...
      if (value == 0) {
        ...
        targetRegister = cg->intClobberEvaluate(firstChild);
        instr = generateRegImmInstruction(SHL4RegImm1, node, targetRegister,
                                          value, cg);
      }
    }
  } else {
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(SHL2MemCL, node, tempMR, shiftAmountReg,
                                        shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shl by var", node);
    } else {
      targetRegister = cg->intClobberEvaluate(firstChild);
      instr = generateRegRegInstruction(SHL4RegCL, node, targetRegister,
                                        shiftAmountReg, shiftDependencies, cg);
    }
  }
...
TR::Register *OMR::X86::TreeEvaluator::bshrEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (testOpcode == TR::bconst &&
      performTransformation(comp,
                            "O^O BSHREvaluator: second child is not an 8-bit "
                            "signed Two's complement opcode %x\n",
                            testOpcode)) {
    int32_t value = secondChild->getByte();
    if (value != 0) {
      if (isMemOp) {
        ...
        if (debug("traceMemOp"))
          diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by const %d",
                     node, value);
      } else
        instr = generateRegImmInstruction(SAR1RegImm1, node, targetRegister,
                                          value, cg);
    }
  } else {
    TR::Register *shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(SAR1MemCL, node, tempMR, shiftAmountReg,
                                        shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
    } else
      instr = generateRegRegInstruction(SAR1RegCL, node, targetRegister,
                                        shiftAmountReg, shiftDependencies, cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::sshrEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (testOpcode == TR::iconst &&
      performTransformation(comp,
                            "O^O SSHREvaluator: second child is not a 16-bit "
                            "signed two's complement number %x\n",
                            testOpcode)) {
    int32_t value = secondChild->getInt();
    if (value != 0) {
      if (isMemOp) {
        ...
        instr = generateRegImmInstruction(SAR2RegImm1, node, targetRegister,
                                          value, cg);
    }
  } else {
    TR::Register *shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(SAR2MemCL, node, tempMR, shiftAmountReg,
                                        shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] shr by var", node);
    } else
      instr = generateRegRegInstruction(SAR2RegCL, node, targetRegister,
                                        shiftAmountReg, shiftDependencies, cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::bushrEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (isMemOp) {
    ...
    if (firstChild->getReferenceCount() > 1) {
      ...
      tempMR = generateX86MemoryReference(firstChild, cg, false);
             ...
             performTransformation(
                 comp,
                 "O^O BUSHREvaluator: first child is not an 8-bit signed two's "
                 "complement, or an 8 bit unsigned %x\n",
                 testOpcode1)) {
    targetRegister = cg->allocateRegister();
  ...
  if ((testOpcode2 == TR::bconst || testOpcode2 == TR::buconst) &&
    ...
    if (isMemOp) {
      ...
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by const %d",
                   node, value);
    } else
      instr = generateRegImmInstruction(SHR1RegImm1, node, targetRegister,
                                        value, cg);
  } else {
    TR::Register *shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(SHR1MemCL, node, tempMR, shiftAmountReg,
                                        shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
    } else
      instr = generateRegRegInstruction(SHR1RegCL, node, targetRegister,
                                        shiftAmountReg, shiftDependencies, cg);
  }
...
TR::Register *OMR::X86::TreeEvaluator::sushrEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (testOpcode == TR::iconst &&
      performTransformation(comp,
                            "O^O SUSHREvaluator: opcode is not a 16-bit signed "
                            "two's complement %x\n",
                            testOpcode)) {
    int32_t value = secondChild->getInt();
    if (isMemOp) {
      ...
      instr = generateRegImmInstruction(SHR2RegImm1, node, targetRegister,
                                        value, cg);
  } else {
    TR::Register *shiftAmountReg = cg->evaluate(secondChild);
    ...
    if (isMemOp) {
      instr = generateMemRegInstruction(SHR2MemCL, node, tempMR, shiftAmountReg,
                                        shiftDependencies, cg);
      if (debug("traceMemOp"))
        diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] ushr by var", node);
    } else
      instr = generateRegRegInstruction(SHR2RegCL, node, targetRegister,
                                        shiftAmountReg, shiftDependencies, cg);
  }
...
TR_X86OpCodes OMR::X86::TreeEvaluator::_logicalOpPackage
    [numLogicalOpPackages][numLogicalOpForms] = {
        // band
        ...
        // bor
        {OR1RegReg, OR1RegMem, MOV1RegReg, OR1RegImm1, BADIA32Op, OR1MemImm1,
         OR1MemImm1, OR1MemReg, NOT1Mem},
        // cor, sor
        {OR2RegReg, OR2RegMem, MOV4RegReg, OR2RegImms, OR2RegImm2, OR2MemImms,
         OR2MemImm2, OR2MemReg, NOT2Mem},
        // ior
        {OR4RegReg, OR4RegMem, MOV4RegReg, OR4RegImms, OR4RegImm4, OR4MemImms,
         OR4MemImm4, OR4MemReg, NOT4Mem},
        // lor
        {OR8RegReg, OR8RegMem, MOV8RegReg, OR8RegImms, OR8RegImm4, OR8MemImms,
         OR8MemImm4, OR8MemReg, NOT8Mem},
        // bxor
...
TR::Register *OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
      performTransformation(comp,
                            "O^O LogicalEvaluator: checking that the store has "
                            "not happened yet. Target register: %x\n",
                            testRegister)) {
    switch (secondChild->getDataType()) {
      ...
      default:
        TR_ASSERT(
            0,
            "Unexpected data type for logical node [" POINTER_PRINTF_FORMAT
            "]\n",
            secondChild);
    }
  }
  ...
  if (isMemOp) {
    TR_ASSERT(
        !node->nodeRequiresConditionCodes(),
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL &&
      ...
      performTransformation(comp,
                            "O^O checking that the store has not happened yet. "
                            "Target register: %x\n",
                            testRegister)) {
    if (!isMemOp && node->getOpCode().isAnd() && !firstChild->getRegister() &&
      ...
      if (firstChild->getFirstChild()->getReferenceCount() > 1 ||
        ...
        generateRegRegInstruction(MOVRegReg(nodeIs64Bit), node, temp,
                                  targetRegister, cg);
        targetRegister = temp;
      }
    ...
    if (node->getOpCode().isXor() && constValue == -1) {
      if (isMemOp) {
        instr = generateMemInstruction(package[TR::TreeEvaluator::logicalNotOp],
                                       node, tempMR, cg);
        if (debug("traceMemOp"))
          diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] logical not",
                     node);
      } else {
        // No harm doing the whole register
        instr = generateRegInstruction(NOTReg(nodeIs64Bit), node,
                                       targetRegister, cg);
      }
    } else {
      if (constValue >= -128 && constValue <= 127) {
        if (isMemOp)
          instr = generateMemImmInstruction(
              package[TR::TreeEvaluator::logicalByteMemImmOp], node, tempMR,
              constValue, cg);
        else
          instr = generateRegImmInstruction(
              package[TR::TreeEvaluator::logicalByteImmOp], node,
              targetRegister, constValue, cg);
      } else {
        if (isMemOp)
          instr = generateMemImmInstruction(
              package[TR::TreeEvaluator::logicalMemImmOp], node, tempMR,
              constValue, cg);
        else
          instr = generateRegImmInstruction(
              package[TR::TreeEvaluator::logicalImmOp], node, targetRegister,
              constValue, cg);
      }
    }
    if (debug("traceMemOp") && isMemOp)
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT
                 "] logical op by const %d",
                 node, constValue);
  } else if (isMemOp) {
    instr =
        generateMemRegInstruction(package[TR::TreeEvaluator::logicalMemRegOp],
                                  node, tempMR, cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] logical op by var",
                 node);
  } else {
    TR_X86BinaryCommutativeAnalyser temp(cg);
...
TR::Register *OMR::X86::TreeEvaluator::bandEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[bandOpPackage], cg);
...
TR::Register *OMR::X86::TreeEvaluator::sandEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[sandOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::candEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[candOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::iandEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[iandOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::borEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Register *targetRegister = TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[borOpPackage], cg);
...
TR::Register *OMR::X86::TreeEvaluator::sorEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[sorOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::corEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[corOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::iorEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[iorOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::bxorEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *targetRegister = TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[bxorOpPackage], cg);
...
TR::Register *OMR::X86::TreeEvaluator::sxorEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[sxorOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::cxorEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[cxorOpPackage], cg);
}

TR::Register *OMR::X86::TreeEvaluator::ixorEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::logicalEvaluator(
      node, _logicalOpPackage[ixorOpPackage], cg);
}
