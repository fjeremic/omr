...
bool OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (indexNode) {
    ...
    if (constNode) {
      ...
      tempNode = NULL;
    } else if (baseNode->getRegister() == NULL && baseNode->getReferenceCount() == 1 && baseNode->getOpCode().isAdd()
    ...
    if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && firstOp.isAdd()) {
      ...
      //
      if (firstChild->getFirstChild()->getReferenceCount() == 1 && firstChild->getFirstChild()->getRegister() == NULL) {
    ...
    targetRegister
      = TR::TreeEvaluator::generateLEAForLoadAddr(firstChild, leaMR, firstChild->getSymbolReference(), cg, isInternal);
    cg->decReferenceCount(firstChild);
    node->setRegister(targetRegister);
    return true;
    ...
    if (firstChild->getOpCode().isSub())
      val = -val;
    leaMR
      = generateX86MemoryReference(cg->evaluate(firstChild->getFirstChild()), cg->evaluate(secondChild), 0, val, cg);
    targetRegister = cg->allocateRegister();
    generateRegMemInstruction(LEARegMem(nodeIs64Bit), node, targetRegister, leaMR, cg);
    node->setRegister(targetRegister);
    cg->decReferenceCount(firstChild->getFirstChild());
  ...
  if (!computesCarry && TR::TreeEvaluator::analyseAddForLEA(node, cg)) {
    ...
    if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL)
      && ((TR::TreeEvaluator::constNodeValueIs32BitSigned(secondChild, &constValue, cg))
        || (comp->useCompressedPointers() && (constValue == TR::Compiler->vm.heapBaseAddress())
        ...
        if (constValue >= -128 && constValue <= 127) {
          if (computesCarry) {
            if (isMemOp)
              ...
              instr
                = generateRegImmInstruction(AddRegImms(nodeIs64Bit, isWithCarry), node, targetRegister, constValue, cg);
      ...
      instr
        = generateMemRegInstruction(AddMemReg(nodeIs64Bit, isWithCarry), node, tempMR, cg->evaluate(secondChild), cg);
      if (debug("traceMemOp"))
    ...
    if (isMemOp) {
      ...
      temp.integerAddAnalyser(
        node, AddRegReg(nodeIs64Bit, isWithCarry), AddRegMem(nodeIs64Bit, isWithCarry), computesCarry, 0);
      targetRegister = node->getRegister();
    }
  }
...
TR::Register* OMR::X86::TreeEvaluator::baddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (NEED_CC(node)) {
    ...
    // we need eflags from integerAddAnalyser for the CC sequence
    TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, ADD1RegReg, ADD1RegMem, true /* produce eflags */);
    targetRegister = node->getRegister();
    return targetRegister;
  }
  ...
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
    && performTransformation(
      comp, "O^O BaddEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::saddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (NEED_CC(node)) {
    ...
    // we need eflags from integerAddAnalyser for the CC sequence
    TR_X86BinaryCommutativeAnalyser(cg).integerAddAnalyser(node, ADD2RegReg, ADD2RegMem, true /* produce eflags */);
    targetRegister = node->getRegister();
    return targetRegister;
  }
  ...
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL
    && performTransformation(
      comp, "O^O SaddEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::caddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL
    && performTransformation(
      comp, "O^O CaddEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb) || (node->getOpCodeValue() == TR::iusubb)) {
    ...
    TR_X86SubtractAnalyser(cg).integerSubtractAnalyser(node, SubRegReg(nodeIs64Bit, isWithBorrow),
      SubRegMem(nodeIs64Bit, isWithBorrow), MOVRegReg(nodeIs64Bit), true, isWithBorrow ? node->getChild(2) : 0);
    targetRegister = node->getRegister();
    return targetRegister;
  }
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
    ...
    && performTransformation(comp,
      "O^O IntegerSubEvaluator: register is not NULL, or second operand is not a 32 bit constant. Register value: %x\n",
      testRegister)) {
      ...
      if (constValue >= -128 && constValue <= 127) {
        if (computesCarry) {
          if (isMemOp)
            ...
            instr
              = generateRegImmInstruction(SubRegImms(nodeIs64Bit, isWithBorrow), node, targetRegister, constValue, cg);
        } else if (constValue == 1) {
          if (isMemOp)
            instr = generateMemInstruction(DECMem(nodeIs64Bit), node, tempMR, cg);
          else
        ...
        if (isMemOp)
          ...
          instr
            = generateRegImmInstruction(SubRegImm4(nodeIs64Bit, isWithBorrow), node, targetRegister, constValue, cg);
      }
    ...
    instr
      = generateMemRegInstruction(SubMemReg(nodeIs64Bit, isWithBorrow), node, tempMR, cg->evaluate(secondChild), cg);
    if (debug("traceMemOp"))
      diagnostic("\n*** Node [" POINTER_PRINTF_FORMAT "] dec by var", node);
  } else {
    TR_X86SubtractAnalyser temp(cg);
    temp.integerSubtractAnalyser(node, SubRegReg(nodeIs64Bit, isWithBorrow), SubRegMem(nodeIs64Bit, isWithBorrow),
...
TR::Register* OMR::X86::TreeEvaluator::bsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::bconst && testRegister == NULL
    && performTransformation(
      comp, "O^O BSUBEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
    int32_t value = secondChild->getByte();
    if (!isMemOp)
      targetRegister = cg->evaluate(firstChild);
    if (targetRegister && firstChild->getReferenceCount() > 1) {
...
TR::Register* OMR::X86::TreeEvaluator::ssubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::sconst && testRegister == NULL
    && performTransformation(
      comp, "O^O SSUBEvaluator: checking that the store has not happened yet. Target register:  %x\n", testRegister)) {
    int32_t value = secondChild->getShortInt();
    if (!isMemOp)
      targetRegister = cg->evaluate(firstChild);
    if (firstChild->getReferenceCount() > 1) {
...
TR::Register* OMR::X86::TreeEvaluator::csubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (NEED_CC(node)) {
    TR_ASSERT(node->getOpCodeValue() == TR::csub, "CC computation not supported for this node %p with opcode %s\n",
      node, cg->comp()->getDebug()->getName(node->getOpCode()));
  ...
  if (secondChild->getOpCodeValue() == TR::cconst && testRegister == NULL
    && performTransformation(
      comp, "O^O CSubEvaluator: checking that the store has not happened yet. Target register: %x\n", testRegister)) {
    int32_t value = secondChild->getConst<uint16_t>();
    if (!isMemOp)
      targetRegister = cg->evaluate(firstChild);
    if (targetRegister && firstChild->getReferenceCount() > 1) {
...
TR::Register* OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->isDualCyclic() || needsHighMulOnly) {
    ...
    // (or result/by-product may be reversed, if lumulh encountered first, and RAX may not be required if lumulh is on
    // its own)
...
TR::Register* OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() && TR::TreeEvaluator::integerConstNodeValue(secondChild, cg) == 0) {
    ...
    if (firstChild->getReferenceCount() > 1)
      cg->evaluate(firstChild); // Respect the evaluation point of the child -- TODO: this could be sub-optimal and is
  ...
  if (isPowerOf2(dvalue)) {
    ...
    if (rootOpCode.isRem()) {
      ...
      if (isMinInt) {
        ...
        if (1 <= dvalue && dvalue <= 0x40000000) {
          ...
          TR_ASSERT(dvalue > CONSTANT64(0x80000000),
            "dvalue should not be negative at this point; TR::getMinSigned<TR::Int64>() is handled elsewhere");
          ...
          generateRegImmInstruction(
            AND8RegImm4, node, tempRegister, SIGN32 | (uint32_t)((int64_t)(dvalue - 1) >> 32), cg);
      ...
      if (!nodeIs64Bit || dvalue <= CONSTANT64(0x80000000)) {
        ...
        if (dvalue == CONSTANT64(0x100000000)) {
          ...
          generateRegImmInstruction(
            OR8RegImm4, node, tempRegister, (~SIGN32) & (uint32_t)((int64_t)(-dvalue) >> 33), cg);
          // Un-rotate
    ...
    if ((dividend->isNonNegative() == false) && (dvalue > 0 || isMinInt)) {
      ...
      if (dvalue == 2) // special case when working with 2
      {
        ...
        TR_ASSERT(mask >= 0,
          "AMD64: mask must have sign bit clear so that, when sign-extended, it clears the upper 32 bits of "
          "tempRegister");
...
TR::Register* OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!opCode.isUnsigned() && secondChild->getOpCode().isLoadConst()
    ...
    if (!nodeIs64Bit && node->isUnsigned()) {
      ...
      if (divisorRegister) {
        divideInstr
          = generateRegRegInstruction(DIVAccReg(nodeIs64Bit), node, eaxRegister, divisorRegister, eaxEdxDeps, cg);
      } else {
      ...
      if (divisorRegister) {
        ...
        if (node->getOpCode().isUnsigned()
          ...
          divideInstr
            = generateRegRegInstruction(IDIVAccReg(nodeIs64Bit), node, eaxRegister, divisorRegister, eaxEdxDeps, cg);
      } else {
        TR::MemoryReference* tempMR = generateX86MemoryReference(secondChild, cg);
        if (node->getFirstChild()->isNonNegative() || node->getOpCode().isUnsigned())
          generateRegRegInstruction(XOR4RegReg, node, edxRegister, edxRegister, edxDeps, cg);
    ...
    if (needsExplicitOverflowCheck) {
      ...
      cg->addSnippet(
        new (cg->trHeapMemory()) TR::X86DivideCheckSnippet(restartLabel, overflowSnippetLabel, divisionLabel,
          node->getOpCode(), node->getType(), divideInstr->getX86RegInstruction()->getIA32RegRegInstruction(), cg));
    }
...
TR::X86RegInstruction* OMR::X86::TreeEvaluator::generateRegisterShift(
  TR::Node* node, TR_X86OpCodes immShiftOpCode, TR_X86OpCodes regShiftOpCode, TR::CodeGenerator* cg)
{
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
      ...
      if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == 0) {
        ...
        if (secondChild->getOpCode().isLoadIndirect() && grandChild->getReferenceCount() == 1
          ...
          if (reportShiftAmount)
            diagnostic(
              "shift: removed sign or zero extend to shift amount in memory in method %s\n", comp->signature());
...
TR::Register* OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->isDirectMemoryUpdate()) {
    TR::Instruction* shiftInstruction
      = TR::TreeEvaluator::generateMemoryShift(node, SHRMemImm1(nodeIs64Bit), SHRMemCL(nodeIs64Bit), cg);
    if (shiftInstruction)
  ...
  if (comp->useCompressedPointers() && nodeIs64Bit && (TR::Compiler->vm.heapBaseAddress() == 0)
...
TR::Register* OMR::X86::TreeEvaluator::bshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (testOpCode == TR::bconst
    && performTransformation(
      comp, "O^O BSHLEvaluator: second child is not an 8-bit signed, two's complement number: %x\n", testOpCode)) {
    int32_t value = secondChild->getByte();
    if (isMemOp) {
      if (value != 0) {
...
TR_X86OpCodes OMR::X86::TreeEvaluator::_logicalOpPackage[numLogicalOpPackages][numLogicalOpForms] = {
  // band
  { AND1RegReg, AND1RegMem, MOV1RegReg, AND1RegImm1, BADIA32Op, AND1MemImm1, AND1MemImm1, AND1MemReg, NOT1Mem },
  // cand/sand
  { AND2RegReg, AND2RegMem, MOV4RegReg, AND2RegImms, AND2RegImm2, AND2MemImms, AND2MemImm2, AND2MemReg, NOT2Mem },
  // iand
  { AND4RegReg, AND4RegMem, MOV4RegReg, AND4RegImms, AND4RegImm4, AND4MemImms, AND4MemImm4, AND4MemReg, NOT4Mem },
  // land
  { AND8RegReg, AND8RegMem, MOV8RegReg, AND8RegImms, AND8RegImm4, AND8MemImms, AND8MemImm4, AND8MemReg, NOT8Mem },
  // bor
  { OR1RegReg, OR1RegMem, MOV1RegReg, OR1RegImm1, BADIA32Op, OR1MemImm1, OR1MemImm1, OR1MemReg, NOT1Mem },
  // cor, sor
  { OR2RegReg, OR2RegMem, MOV4RegReg, OR2RegImms, OR2RegImm2, OR2MemImms, OR2MemImm2, OR2MemReg, NOT2Mem },
  // ior
  { OR4RegReg, OR4RegMem, MOV4RegReg, OR4RegImms, OR4RegImm4, OR4MemImms, OR4MemImm4, OR4MemReg, NOT4Mem },
  // lor
  { OR8RegReg, OR8RegMem, MOV8RegReg, OR8RegImms, OR8RegImm4, OR8MemImms, OR8MemImm4, OR8MemReg, NOT8Mem },
  // bxor
  { XOR1RegReg, XOR1RegMem, MOV1RegReg, XOR1RegImm1, BADIA32Op, XOR1MemImm1, XOR1MemImm1, XOR1MemReg, NOT1Mem },
  // cxor, sxor
  { XOR2RegReg, XOR2RegMem, MOV4RegReg, XOR2RegImms, XOR2RegImm2, XOR2MemImms, XOR2MemImm2, XOR2MemReg, NOT2Mem },
  // ixor
  { XOR4RegReg, XOR4RegMem, MOV4RegReg, XOR4RegImms, XOR4RegImm4, XOR4MemImms, XOR4MemImm4, XOR4MemReg, NOT4Mem },
  // lxor
  { XOR8RegReg, XOR8RegMem, MOV8RegReg, XOR8RegImms, XOR8RegImm4, XOR8MemImms, XOR8MemImm4, XOR8MemReg, NOT8Mem },
};

TR::Register* OMR::X86::TreeEvaluator::logicalEvaluator(TR::Node* node, TR_X86OpCodes package[], TR::CodeGenerator* cg)
{
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Register* targetRegister = NULL;
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (isMemOp) {
    TR_ASSERT(!node->nodeRequiresConditionCodes(), "a node requiring CC evaluation should not be a direct mem update");
  ...
  if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && IS_32BIT_SIGNED(constValue)
    ...
    if (!isMemOp && node->getOpCode().isAnd() && !firstChild->getRegister() && firstChild->getOpCode().isConversion()
      && firstChild->getType().isIntegral() && firstChild->getFirstChild()->getType().isIntegral()
    ...
    if (node->getOpCode().isXor() && constValue == -1) {
      ...
      if (constValue >= -128 && constValue <= 127) {
        if (isMemOp)
          instr
            = generateMemImmInstruction(package[TR::TreeEvaluator::logicalByteMemImmOp], node, tempMR, constValue, cg);
        else
          instr = generateRegImmInstruction(
            package[TR::TreeEvaluator::logicalByteImmOp], node, targetRegister, constValue, cg);
      } else {
        if (isMemOp)
          instr = generateMemImmInstruction(package[TR::TreeEvaluator::logicalMemImmOp], node, tempMR, constValue, cg);
        else
          instr
            = generateRegImmInstruction(package[TR::TreeEvaluator::logicalImmOp], node, targetRegister, constValue, cg);
      }
    }
    ...
    TR_X86BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, package[TR::TreeEvaluator::logicalRegRegOp], package[TR::TreeEvaluator::logicalRegMemOp],
