...
TR::Register *
OMR::X86::TreeEvaluator::integerDualAddOrSubEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  ...
  if (pair->getReferenceCount() == 1) {
    ...
    if (node->isDualHigh()) {
      ...
      // evaluated.
      diagnostic("Evaluating Children of %p (%s) for %p (%s) (not required for "
                 "quad calc)\n",
...
TR::Register *
OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
...
TR::Register *
OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  const TR::ILOpCodes opCode = node->getOpCodeValue();
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Node *firstChild = node->getFirstChild();
...
TR::Register *
OMR::X86::TreeEvaluator::integerDualMulEvaluator(TR::Node *node,
                                                 TR::CodeGenerator *cg) {
...
TR::Register *
OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      ...
      TR_X86IntegerMultiplyDecomposer *mulDecomposer = new (cg->trHeapMemory())
          TR_X86IntegerMultiplyDecomposer(value, firstChild->getRegister(),
                                          node, cg, canClobberSource);
      int32_t dummy;
...
TR::Register *
OMR::X86::TreeEvaluator::integerMulhEvaluator(TR::Node *node,
                                              TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (firstChild->getReferenceCount() > 1)
      cg->evaluate(firstChild); // Respect the evaluation point of the child --
                                // TODO: this could be sub-optimal and is likely
...
TR::Register *
OMR::X86::TreeEvaluator::signedIntegerDivOrRemAnalyser(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
      ...
      if (secondChild->getReferenceCount() == 1 &&
        ...
        if (secondChild->getOpCode().isLoadIndirect() &&
          ...
          if (reportShiftAmount)
            diagnostic("shift: removed sign or zero extend to shift amount in "
                       "memory in method %s\n",
                       comp->signature());
          ...
          if (reportShiftAmount)
            diagnostic("shift: removed sign or zero extend to shift amount in "
                   ...
                   TR::Compiler->target.is64Bit()) // cannot use a reigster pair
                                                   // as the shift amount
        {
          secondChild->decReferenceCount();
          secondChild = grandChild;
...
TR::Register *
OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
...
TR::Register *
OMR::X86::TreeEvaluator::integerRolEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
...
TR::Register *
OMR::X86::TreeEvaluator::integerShrEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  if (node->isDirectMemoryUpdate()) {
...
TR::Register *
OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node *node,
                                              TR::CodeGenerator *cg) {
  bool nodeIs64Bit = TR::TreeEvaluator::getNodeIs64Bit(node, cg);
  TR::Compilation *comp = cg->comp();
