 ...
 */
void
TR_X86SubtractAnalyser::integerSubtractAnalyserWithExplicitOperands(
  TR::Node* root,
  TR::Node* firstChild,
  TR::Node* secondChild,
  ...
  TR::Node* borrow) // 0 by default
{
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            ...
            root);
  TR::Register* tempReg = integerSubtractAnalyserImpl(root,
                                                      firstChild,
                                                      secondChild,
                                                      regRegOpCode,
                                                      regMemOpCode,
 ...
 */
void
TR_X86SubtractAnalyser::integerSubtractAnalyser(
  TR::Node* root,
  TR_X86OpCodes regRegOpCode,
  TR_X86OpCodes regMemOpCode,
  TR_X86OpCodes copyOpCode,
  ...
  TR::Node* borrow) // 0 by default
{
  ...
  targetRegister = integerSubtractAnalyserImpl(root,
                                               firstChild,
                                               secondChild,
                                               regRegOpCode,
                                               regMemOpCode,
 ...
 */
TR::Register*
TR_X86SubtractAnalyser::integerSubtractAnalyserImpl(TR::Node* root,
                                                    TR::Node* firstChild,
                                                    TR::Node* secondChild,
                                                    TR_X86OpCodes regRegOpCode,
                                                    TR_X86OpCodes regMemOpCode,
    ...
    if (firstChild->getReferenceCount() > 1) {
      ...
      if (firstChild->getOpCodeValue() == TR::iconst && loadedConst) {
        ...
        if (secondChild->getReferenceCount() == 1 && secondRegister != 0 &&
          ...
          generateRegInstruction(
            NEGReg(nodeIs64Bit), secondChild, secondRegister, _cg);
          ...
          generateRegRegInstruction(
            copyOpCode, root, thirdReg, firstRegister, _cg);
        }
      }
      targetRegister = thirdReg;
      if (getSubReg3Reg2()) {
        generateRegRegInstruction(
          regRegOpCode, root, thirdReg, secondRegister, _cg);
      ...
      } else // assert getSubReg3Mem2() == true
      {
        ...
        generateRegMemInstruction(
          regMemOpCode, root, firstRegister, tempMR, _cg);
        tempMR->decNodeReferenceCounts(_cg);
      }
 ...
 */
void
TR_X86SubtractAnalyser::longSubtractAnalyserWithExplicitOperands(
  TR::Node* root,
  TR::Node* firstChild,
  TR::Node* secondChild)
{
 ...
 */
void
TR_X86SubtractAnalyser::longSubtractAnalyser(TR::Node* root)
{
  TR::Node* firstChild = root->getFirstChild();
  TR::Node* secondChild = root->getSecondChild();
  TR::Register* targetRegister = NULL;
 ...
 */
TR::Register*
TR_X86SubtractAnalyser::longSubtractAnalyserImpl(TR::Node* root,
                                                 TR::Node*& firstChild,
                                                 TR::Node*& secondChild)
{
  TR::Register* firstRegister = firstChild->getRegister();
  ...
  if (getCopyReg1()) {
    ...
    targetRegister = thirdReg;
    generateRegRegInstruction(
      MOV4RegReg, root, lowThird, firstRegister->getLowOrder(), _cg);

    if (firstHighZero) {
      generateRegRegInstruction(XOR4RegReg, root, highThird, highThird, _cg);
    } else {
      generateRegRegInstruction(
        MOV4RegReg, root, highThird, firstRegister->getHighOrder(), _cg);
    }

    if (getSubReg3Reg2()) {
      if (secondHighZero) {
        generateRegRegInstruction(
          regRegOpCode, root, lowThird, secondRegister, _cg);
        generateRegImmInstruction(SBB4RegImms, root, highThird, 0, _cg);
      } else {
        generateRegRegInstruction(
          regRegOpCode, root, lowThird, secondRegister->getLowOrder(), _cg);
        generateRegRegInstruction(
          SBB4RegReg, root, highThird, secondRegister->getHighOrder(), _cg);
      }
    } else // assert getSubReg3Mem2() == true
    {
    ...
    if (secondHighZero) {
      generateRegRegInstruction(
        regRegOpCode, root, firstRegister->getLowOrder(), secondRegister, _cg);
      generateRegImmInstruction(
        SBB4RegImms, root, firstRegister->getHighOrder(), 0, _cg);
    } else {
      generateRegRegInstruction(regRegOpCode,
                                root,
  ...
  } else // assert getSubReg1Mem2() == true
  {
    ...
    generateRegMemInstruction(
      regMemOpCode, root, firstRegister->getLowOrder(), lowMR, _cg);
    ...
    if (secondHighZero) {
      ...
      generateRegMemInstruction(
        SBB4RegMem, root, firstRegister->getHighOrder(), highMR, _cg);
    }
