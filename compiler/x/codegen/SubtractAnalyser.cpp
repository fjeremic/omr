 ...
 */
void TR_X86SubtractAnalyser::integerSubtractAnalyser(TR::Node* root, TR_X86OpCodes regRegOpCode,
  ...
  TR::Node* root, TR::Node*& firstChild, TR::Node*& secondChild)
{
  ...
  if (root->getOpCodeValue() == TR::lusubb && TR_X86ComputeCC::setCarryBorrow(root->getChild(2), true, _cg)) {
  ...
  if (getCopyReg1()) {
    ...
    } else // assert getSubReg3Mem2() == true
    {
       ...
       * that shifts by 32 bits. This is the only case where the useSecondHighOrder bit is set. When the first child of
       * the lushr is in a register, code above handles the shift. When the first child of the lushr is in memory, the
       * below ensures that the upper part of the first child of the lushr is used as lowMR.
       */
      if (useSecondHighOrder) {
        TR_ASSERT(secondHighZero,
          "useSecondHighOrder should be consistent with secondHighZero. useSecondHighOrder subsumes secondHighZero");
  ...
  } else // assert getSubReg1Mem2() == true
  {
    TR::MemoryReference* lowMR = generateX86MemoryReference(secondChild, _cg);
    /**
     * The below code is needed to ensure correct behaviour when the subtract analyser encounters a lushr bytecode that
     * shifts by 32 bits. This is the only case where the useSecondHighOrder bit is set. When the first child of the
     * lushr is in a register, code above handles the shift. When the first child of the lushr is in memory, the below
     * ensures that the upper part of the first child of the lushr is used as lowMR.
     */
    if (useSecondHighOrder)
      lowMR = generateX86MemoryReference(*lowMR, 4, _cg);
