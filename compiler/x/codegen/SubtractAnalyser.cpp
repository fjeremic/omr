...
/*
 * \brief
 * this API is for check nodes(like OverflowCHK) with sub operation where the
 ...
 */
void TR_X86SubtractAnalyser::integerSubtractAnalyserWithExplicitOperands(
    ...
    TR::Node *borrow)  // 0 by default
{
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            "unsupported opcode %s for "
            "integerSubtractAnalyserWithExplicitOperands on node %p\n",
            _cg->comp()->getDebug()->getName(root->getOpCodeValue()), root);
  TR::Register *tempReg = integerSubtractAnalyserImpl(
      root, firstChild, secondChild, regRegOpCode, regMemOpCode, copyOpCode,
      needsEflags, borrow);
...
/*
 * \brief
 * this API is for regular sub operation nodes where the first child and second
 * child are the operands by default
 */
void TR_X86SubtractAnalyser::integerSubtractAnalyser(
    ...
    TR::Node *borrow)  // 0 by default
{
  ...
  targetRegister = integerSubtractAnalyserImpl(root, firstChild, secondChild,
                                               regRegOpCode, regMemOpCode,
...
/*
 * users should call the integerSubtractAnalyser or
 * integerSubtractAnalyserWithExplicitOperands APIs instead of calling this one
 * directly
 */
TR::Register *TR_X86SubtractAnalyser::integerSubtractAnalyserImpl(
    TR::Node *root, TR::Node *firstChild, TR::Node *secondChild,
    TR_X86OpCodes regRegOpCode, TR_X86OpCodes regMemOpCode,
    TR_X86OpCodes copyOpCode, bool needsEflags, TR::Node *borrow) {
  TR::Register *targetRegister = NULL;
  ...
  if (getEvalChild1()) {
    ...
    if (firstRegister == 0 && firstChild->getOpCodeValue() == TR::iconst &&
        (firstChild != secondChild)) {
      // An iconst may have to be generated.  The iconst will be generated after
      // the
  ...
  if (loadedConst) {
    if (firstRegister == 0) {
      ...
      TR::TreeEvaluator::insertLoadConstant(firstChild, firstRegister,
                                            firstChild->getInt(),
                                            TR_RematerializableInt, _cg);
    } else {
      // firstchild was evaluated.  The code for an iconst does not need to be
      // generated.
  ...
  if (borrow != 0) TR_X86ComputeCC::setCarryBorrow(borrow, true, _cg);
    ...
    if (firstChild->getReferenceCount() > 1) {
      ...
      if (firstChild->getOpCodeValue() == TR::iconst && loadedConst) {
        ...
        if (secondChild->getReferenceCount() == 1 && secondRegister != 0 &&
            !needsEflags && (borrow == 0)) {
          // use one fewer registers if we negate the clobberable secondRegister
          // and add Don't do this though if condition codes are needed.  The
          // sequence depends on the carry flag being valid as if a sub was
          // done.
          //
          bool nodeIs64Bit = TR_X86OpCode(regRegOpCode).hasLongSource();
          generateRegInstruction(NEGReg(nodeIs64Bit), secondChild,
                                 secondRegister, _cg);
          ...
          generateRegRegInstruction(copyOpCode, root, thirdReg, firstRegister,
                                    _cg);
        }
      }
      targetRegister = thirdReg;
      if (getSubReg3Reg2()) {
        generateRegRegInstruction(regRegOpCode, root, thirdReg, secondRegister,
                                  _cg);
      } else  // assert getSubReg3Mem2() == true
      {
        TR::MemoryReference *tempMR =
            generateX86MemoryReference(secondChild, _cg);
        generateRegMemInstruction(regMemOpCode, root, thirdReg, tempMR, _cg);
        tempMR->decNodeReferenceCounts(_cg);
      }
    } else {
      if (getSubReg3Reg2()) {
        generateRegRegInstruction(regRegOpCode, root, firstRegister,
                                  secondRegister, _cg);
      } else  // assert getSubReg3Mem2() == true
      {
        TR::MemoryReference *tempMR =
            generateX86MemoryReference(secondChild, _cg);
        generateRegMemInstruction(regMemOpCode, root, firstRegister, tempMR,
                                  _cg);
        tempMR->decNodeReferenceCounts(_cg);
      }
...
/*
 * \brief
 * this API is for check nodes(like OverflowCHK) with an lsub operation where
...
void TR_X86SubtractAnalyser::longSubtractAnalyserWithExplicitOperands(
  ...
  TR_ASSERT(root->getOpCodeValue() == TR::OverflowCHK,
            "unsupported opcode %s for "
            "longSubtractAnalyserWithExplicitOperands on node %p\n",
            _cg->comp()->getDebug()->getName(root->getOpCodeValue()), root);
  TR::Register *tempReg =
      longSubtractAnalyserImpl(root, firstChild, secondChild);
  _cg->decReferenceCount(firstChild);
  _cg->decReferenceCount(secondChild);
  _cg->stopUsingRegister(tempReg);
}
...
/*
 * \brief
 * this API is intended for regular lsub operation nodes where the first child
...
/*
 * users should call the longSubtractAnalyser or
 * longSubtractAnalyserWithExplicitOperands APIs instead of calling this one
 * directly
 */
TR::Register *TR_X86SubtractAnalyser::longSubtractAnalyserImpl(
  ...
  if (isVolatileMemoryOperand(firstChild)) resetMem1();

  if (isVolatileMemoryOperand(secondChild)) resetMem2();
  ...
  if (getCopyReg1()) {
    ...
    targetRegister = thirdReg;
    generateRegRegInstruction(MOV4RegReg, root, lowThird,
                              firstRegister->getLowOrder(), _cg);

    if (firstHighZero) {
      generateRegRegInstruction(XOR4RegReg, root, highThird, highThird, _cg);
    } else {
      generateRegRegInstruction(MOV4RegReg, root, highThird,
                                firstRegister->getHighOrder(), _cg);
    }

    if (getSubReg3Reg2()) {
      if (secondHighZero) {
        generateRegRegInstruction(regRegOpCode, root, lowThird, secondRegister,
                                  _cg);
        generateRegImmInstruction(SBB4RegImms, root, highThird, 0, _cg);
      } else {
        generateRegRegInstruction(regRegOpCode, root, lowThird,
                                  secondRegister->getLowOrder(), _cg);
        generateRegRegInstruction(SBB4RegReg, root, highThird,
    ...
    } else  // assert getSubReg3Mem2() == true
    {
      ...
      /**
       * The below code is needed to ensure correct behaviour when the subtract
       * analyser encounters a lushr bytecode that shifts by 32 bits. This is
       * the only case where the useSecondHighOrder bit is set. When the first
       * child of the lushr is in a register, code above handles the shift. When
       * the first child of the lushr is in memory, the below ensures that the
       * upper part of the first child of the lushr is used as lowMR.
       */
      if (useSecondHighOrder) {
        TR_ASSERT(secondHighZero,
                  "useSecondHighOrder should be consistent with "
                  "secondHighZero. useSecondHighOrder subsumes secondHighZero");
      ...
      if (secondHighZero) {
        ...
        TR::MemoryReference *highMR =
            generateX86MemoryReference(*lowMR, 4, _cg);
    ...
    if (secondHighZero) {
      generateRegRegInstruction(regRegOpCode, root,
                                firstRegister->getLowOrder(), secondRegister,
                                _cg);
      generateRegImmInstruction(SBB4RegImms, root,
                                firstRegister->getHighOrder(), 0, _cg);
    } else {
      generateRegRegInstruction(regRegOpCode, root,
                                firstRegister->getLowOrder(),
                                secondRegister->getLowOrder(), _cg);
      generateRegRegInstruction(SBB4RegReg, root, firstRegister->getHighOrder(),
                                secondRegister->getHighOrder(), _cg);
    }
    targetRegister = firstRegister;
  } else  // assert getSubReg1Mem2() == true
  {
    ...
    /**
     * The below code is needed to ensure correct behaviour when the subtract
     * analyser encounters a lushr bytecode that shifts by 32 bits. This is the
     * only case where the useSecondHighOrder bit is set. When the first child
     * of the lushr is in a register, code above handles the shift. When the
     * first child of the lushr is in memory, the below ensures that the upper
     * part of the first child of the lushr is used as lowMR.
     */
    if (useSecondHighOrder) lowMR = generateX86MemoryReference(*lowMR, 4, _cg);

    generateRegMemInstruction(regMemOpCode, root, firstRegister->getLowOrder(),
                              lowMR, _cg);
    ...
    if (secondHighZero) {
      ...
      generateRegMemInstruction(SBB4RegMem, root, firstRegister->getHighOrder(),
                                highMR, _cg);
    }
