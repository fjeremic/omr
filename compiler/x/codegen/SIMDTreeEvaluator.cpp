...
ConvertToPatchableMemoryReference(TR::MemoryReference *mr, TR::Node *node,
                                  TR::CodeGenerator *cg) {
...
OMR::X86::TreeEvaluator::SIMDsplatsEvaluator(TR::Node *node,
  ...
  case TR::VectorInt32:
    ...
    generateRegRegImmInstruction(PSHUFDRegRegImm1, node, resultReg, resultReg,
                                 0x00, cg); // 00 00 00 00 shuffle xxxA to AAAA
    break;
  case TR::VectorInt64:
    ...
    generateRegRegImmInstruction(PSHUFDRegRegImm1, node, resultReg, resultReg,
                                 0x44, cg); // 01 00 01 00 shuffle xxBA to BABA
    break;
  case TR::VectorFloat:
...
OMR::X86::TreeEvaluator::SIMDgetvelemEvaluator(TR::Node *node,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    TR_ASSERT(false, "non-const second child not currently supported in "
