...
TR::Register* OMR::X86::TreeEvaluator::SIMDsplatsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  case TR::VectorInt32:
    ...
    generateRegRegImmInstruction(
      PSHUFDRegRegImm1, node, resultReg, resultReg, 0x00, cg); // 00 00 00 00 shuffle xxxA to AAAA
    break;
  case TR::VectorInt64:
    ...
    generateRegRegImmInstruction(
      PSHUFDRegRegImm1, node, resultReg, resultReg, 0x44, cg); // 01 00 01 00 shuffle xxBA to BABA
    break;
  case TR::VectorFloat:
    generateRegRegImmInstruction(
      PSHUFDRegRegImm1, node, resultReg, childReg, 0x00, cg); // 00 00 00 00 shuffle xxxA to AAAA
    break;
  case TR::VectorDouble:
    generateRegRegImmInstruction(
      PSHUFDRegRegImm1, node, resultReg, childReg, 0x44, cg); // 01 00 01 00 shuffle xxBA to BABA
    break;
  default:
    if (cg->comp()->getOption(TR_TraceCG))
