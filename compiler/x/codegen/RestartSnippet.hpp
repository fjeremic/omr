...
class X86RestartSnippet : public TR::Snippet {
 ...
 public:
  X86RestartSnippet(TR::CodeGenerator *cg, TR::Node *n,
                    TR::LabelSymbol *restartlab, TR::LabelSymbol *snippetlab,
                    bool isGCSafePoint)
  ...
  uint8_t *genRestartJump(TR_X86OpCodes branchOp, uint8_t *bufferCursor,
    ...
    TR_ASSERT((branchOp >= JA4) && (branchOp <= JMP4),
              "opcode must be a long branch for conditional restart in a "
              "restart snippet\n");
  ...
  uint32_t estimateRestartJumpLength(TR_X86OpCodes branchOp,
    ...
    intptrj_t distance =
        location - (estimatedSnippetLocation + 2);  // 2 is size of short branch
  ...
  uint32_t estimateRestartJumpLength(TR_X86OpCodes branchOp,
                                     int32_t estimatedSnippetLocation) {
    return estimateRestartJumpLength(branchOp, estimatedSnippetLocation,
                                     _restartLabel);
  }
};
