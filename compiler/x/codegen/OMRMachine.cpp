    ...
    uint8_t testMask)
{
    ...
    } while (testMask && cursor->getOpCodeValue() != LABEL && cursor->getOpCodeValue() != RET && cursor->getOpCodeValue() != RETImm2 && cursor->getOpCodeValue() != ReturnMarker && !cursor->getOpCode().isBranchOp());
...
{
    if (reg1->getStartOfRange() && reg2->getEndOfRange() && reg1->getStartOfRange()->getIndex() >= reg2->getEndOfRange()->getIndex())
        return false;
    if (reg1->getEndOfRange() && reg2->getStartOfRange() && reg1->getEndOfRange()->getIndex() <= reg2->getStartOfRange()->getIndex())
        return false;
    return true;
}
    ...
    bool considerUnlatched)
{
    ...
    if (useRegisterAssociations && virtReg->getAssociation() != TR::RealRegister::NoReg && self()->getVirtualAssociatedWithReal((TR::RealRegister::RegNum)virtReg->getAssociation()) == virtReg && (_registerFile[virtReg->getAssociation()]->getAssignedRegister() == NULL || (considerUnlatched && _registerFile[virtReg->getAssociation()]->getState() == TR::RealRegister::Unlatched))) {
    ...
    for (i = first; i <= last; i++) {
        ...
        if (useRegisterWeights) {
            if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i) || linkageProperties.isCalleeVolatileRegister((TR::RealRegister::RegNum)i)) && _registerFile[i]->getWeight() == IA32_REGISTER_INITIAL_PRESERVED_WEIGHT) {
        ...
        if (((_registerFile[i]->getAssignedRegister() == NULL && _registerFile[i]->getState() != TR::RealRegister::Blocked) || (considerUnlatched && _registerFile[i]->getState() == TR::RealRegister::Unlatched))) {
            ...
            if (weight < bestWeightSoFar) {
                ...
                numCandidates = 0;
            } else if (weight == bestWeightSoFar && useRegisterInterferences && (weight & IA32_REGISTER_INTERFERENCE_WEIGHT)) {
                TR::Register* r = self()->getVirtualAssociatedWithReal((TR::RealRegister::RegNum)i);
                if (r && (r->getAssociation() != TR::RealRegister::NoReg) && _registerFile[i]->getState() == TR::RealRegister::Free) {
    ...
    bool considerVirtAsSpillCandidate)
{
    ...
    for (i = numCandidates - 1; i >= 0; i--) {
        ...
        if (enableRematerialisation && info && info->isActive()) {
            if (candidates[i]->getRematerializationInfo()->isRematerializableFromMemory() || candidates[i]->getRematerializationInfo()->isRematerializableFromAddress()) {
                if (interferes)
                    j = BestInterferingDiscardableMemory;
                else if (byteRegisterInterference && registerNumber <= TR::RealRegister::Last8BitGPR)
                    j = BestMayInterfereDiscardableMemory;
    ...
    if (bestType >= 0) {
        ...
        for (i = 0; i < bestType; i++) {
            if (bestDistances[i] && (bestDistance - bestDistances[i]) < freeBestRegisterSignificantDistances[i][bestType] && (bestDistance - bestDistances[i]) < bestDistances[i]) {
    ...
    //
    TR::Register* bestDiscardableRegister = (bestRegister->isDiscardable() && bestRegister->getRematerializationInfo()->isActive()) ? bestRegister : NULL;
    ...
    TR_RegisterSizes requestedRegSize)
{
    ...
    if (self()->cg()->enableRematerialisation()) {
        ...
        //
        if (spilledRegister->getRematerializationInfo() && spilledRegister->getRematerializationInfo()->isRematerialized()) {
            if (debug("dumpRemat")) {
    ...
    bool coerceToSatisfyRegDeps)
{
    ...
    if (targetRegister->getState() == TR::RealRegister::Free) {
        ...
        if (instr)
            self()->cg()->traceRAInstruction(instr);
    } else if (targetRegister->getState() == TR::RealRegister::Blocked || targetRegister->getState() == TR::RealRegister::Assigned) {
...
void OMR::X86::Machine::setGPRWeightsFromAssociations()
{
    ...
    for (int i = TR::RealRegister::FirstGPR;
        ...
        if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i) || linkageProperties.isCalleeVolatileRegister((TR::RealRegister::RegNum)i)) && _registerFile[i]->getHasBeenAssignedInMethod() == false) {
            if (assocReg) {
...
{
    TR::Compilation* comp = self()->cg()->comp();
    TR::RegisterDependencyConditions* associations = generateRegisterDependencyConditions((uint8_t)0, TR::RealRegister::LastAssignableGPR, self()->cg());
...
TR::RegisterDependencyConditions* OMR::X86::Machine::createDepCondForLiveGPRs()
{
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastXMMR; i = ((i == TR::RealRegister::LastAssignableGPR) ? TR::RealRegister::FirstXMMR : i + 1)) {
        ...
        if (realReg->getState() == TR::RealRegister::Assigned || realReg->getState() == TR::RealRegister::Free || realReg->getState() == TR::RealRegister::Blocked)
            c++;
    }
    ...
    if (c) {
        ...
        for (i = TR::RealRegister::FirstGPR; i <= endReg; i = ((i == TR::RealRegister::LastAssignableGPR) ? TR::RealRegister::FirstXMMR : i + 1)) {
            ...
            if (realReg->getState() == TR::RealRegister::Assigned) {
                ...
                TR_ASSERT(!spilledRegisterList || !(std::find(spilledRegisterList->begin(), spilledRegisterList->end(), virtReg) != spilledRegisterList->end()), "a register should not be in both an assigned state and in the spilled list\n");
                deps->addPostCondition(virtReg, realReg->getRegisterNumber(), self()->cg());
                if (calledForDeps) {
                    virtReg->incTotalUseCount();
                    virtReg->incFutureUseCount();
...
TR::RealRegister** OMR::X86::Machine::captureRegisterFile()
{
    ...
    TR::RealRegister** registerFileClone = (TR::RealRegister**)self()->cg()->trMemory()->allocateMemory(arraySize, heapAlloc);
    ...
    for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg; i = ((i == TR::RealRegister::LastAssignableGPR) ? TR::RealRegister::FirstXMMR : i + 1)) {
        registerFileClone[i] = (TR::RealRegister*)self()->cg()->trMemory()->allocateMemory(sizeof(TR::RealRegister), heapAlloc);
        *registerFileClone[i] = *_registerFile[i];
    }

    registerFileClone[TR::RealRegister::vfp] = (TR::RealRegister*)self()->cg()->trMemory()->allocateMemory(sizeof(TR::RealRegister), heapAlloc);
...
void OMR::X86::Machine::installRegisterFile(TR::RealRegister** registerFileCopy)
{
    ...
    for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg; i = ((i == TR::RealRegister::LastAssignableGPR) ? TR::RealRegister::FirstXMMR : i + 1)) {
        ...
        if (assignedVirtReg != NULL) {
            TR_ASSERT(_registerFile[i]->getState() == TR::RealRegister::Assigned || _registerFile[i]->getState() == TR::RealRegister::Locked,
...
TR::Register** OMR::X86::Machine::captureRegisterAssociations()
{
    ...
    TR::Register** registerAssociationsClone = (TR::Register**)self()->cg()->trMemory()->allocateMemory(arraySize, heapAlloc);

    int32_t endReg = TR::RealRegister::LastXMMR;
    for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg; i = ((i == TR::RealRegister::LastAssignableGPR) ? TR::RealRegister::FirstXMMR : i + 1)) {
        if (_registerAssociations[i] != NULL) {
            registerAssociationsClone[i] = (TR::Register*)self()->cg()->trMemory()->allocateMemory(sizeof(TR::Register), heapAlloc);
    ...
    if (_registerAssociations[TR::RealRegister::vfp] != NULL) {
        registerAssociationsClone[TR::RealRegister::vfp] = (TR::Register*)self()->cg()->trMemory()->allocateMemory(sizeof(TR::Register), heapAlloc);
    ...
    bool generateCode)
{
    ...
    //
    TR_ASSERT(((_fpTopOfStack > TR_X86FPStackRegister::fpStackEmpty) && (_fpTopOfStack < TR_X86FPStackRegister::fpStackFull)),
...
TR::Instruction* OMR::X86::Machine::freeBestFPRegister(TR::Instruction* prevInstruction)
{
    ...
    while (numCandidates > 1 && cursor != NULL && cursor->getOpCodeValue() != LABEL && cursor->getOpCodeValue() != RET && cursor->getOpCodeValue() != RETImm2 && cursor->getOpCodeValue() != ReturnMarker && !cursor->getOpCode().isBranchOp()) {
