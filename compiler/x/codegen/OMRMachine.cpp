...
static bool
registersMayOverlap(TR::Register* reg1, TR::Register* reg2)
{
...
int32_t
OMR::X86::Machine::getGlobalReg(TR::RealRegister::RegNum reg)
{
                                          ...
                                          bool considerUnlatched)
{
  ...
  TR_ASSERT(
    (virtReg && (virtReg->getKind() == TR_GPR || virtReg->getKind() == TR_FPR ||
                 virtReg->getKind() == TR_VRF)),
    "OMR::X86::Machine::findBestFreeGPRegister() ==> Unexpected register "
    "kind!");
  ...
  for (i = first; i <= last; i++) {
    ...
    if (useRegisterWeights) {
      ...
      if (associatedRegister &&
        ...
        weight &=
          ~(IA32_REGISTER_ASSOCIATED_WEIGHT | IA32_REGISTER_PLACEHOLDER_WEIGHT);
    ...
    for (cursor = currentInstruction->getPrev(); cursor && numCandidates > 1;
      ...
      for (i = 0; i < numCandidates; i++) {
        if (cursor->refsRegister(candidates[i]._virtual)) {
          self()->cg()->traceRegInterference(
            virtReg, candidates[i]._virtual, distance);
...
TR::RealRegister*
OMR::X86::Machine::freeBestGPRegister(TR::Instruction* currentInstruction,
                                      TR::Register* virtReg,
                                      TR_RegisterSizes requestedRegSize,
                                      ...
                                      bool considerVirtAsSpillCandidate)
{
  ...
  if (targetRegister)
    self()->cg()->traceRegisterAssignment(
      "Spill candidates for %R (target %R):",
      virtReg,
  ...
  for (i = numCandidates - 1; i >= 0; i--) {
    registerNumber =
      toRealRegister(candidates[i]->getAssignedRegister())->getRegisterNumber();
    interferes =
      interference &
      (1 << (registerNumber - 1)); // TODO:AMD64: Use the proper mask value

    TR_RematerializationInfo* info =
      candidates[i]->isDiscardable() ? candidates[i]->getRematerializationInfo()
                                     : NULL;
  ...
  if (bestType >= 0) {
    for (i = 0; i < NumBestRegisters; i++) {
      if (bestRegisters[i])
        self()->cg()->traceRegisterAssignment("   (best %s %R at distance %d)",
                                              bestRegisters[i],
                                              bestRegisterTypes[i],
                                              bestDistances[i]);
    }
  ...
  if (enableRematerialisation && bestDiscardableRegister) {
    ...
    if (info->isRematerializableFromMemory()) {
      ...
      if (info->getDataType() == TR_RematerializableFloat) {
        instr = generateRegMemInstruction(
          currentInstruction, MOVSSRegMem, best, tempMR, self()->cg());
      } else {
        instr = TR::TreeEvaluator::insertLoadMemory(0,
                                                    best,
                                                    tempMR,
                                                    info->getDataType(),
                                                    self()->cg(),
      ...
      TR::MemoryReference* tempMR =
        generateX86MemoryReference(info->getSymbolReference(), self()->cg());
      instr = generateRegMemInstruction(
        currentInstruction, LEARegMem(), best, tempMR, self()->cg());
    } else {
      if (info->getDataType() == TR_RematerializableFloat) {
        TR::MemoryReference* tempMR = generateX86MemoryReference(
          self()->cg()->findOrCreate4ByteConstant(currentInstruction->getNode(),
                                                  info->getConstant()),
          self()->cg());
        instr = generateRegMemInstruction(
          currentInstruction, MOVSSRegMem, best, tempMR, self()->cg());
      } else {
        instr = TR::TreeEvaluator::insertLoadConstant(0,
                                                      best,
                                                      info->getConstant(),
                                                      info->getDataType(),
                                                      self()->cg(),
    ...
    if (debug("dumpRemat")) {
      if (info->isIndirect())
        diagnostic(
          "---> Spilling %s rematerialisable register %s (type %d, base %s)\n",
          self()->getDebug()->toString(info),
          self()->getDebug()->getName(bestDiscardableRegister),
          info->getDataType(),
          self()->getDebug()->getName(info->getBaseRegister()));
    ...
    if ((bestRegister->getKind() == TR_FPR)) {
      ...
      if (containsInternalPointer) {
        ...
        if (bestRegister->getBackingStorage()) {
          ...
          if (self()->cg()->getDebug())
            self()->cg()->traceRegisterAssignment(
              "find or create free backing store (%p) for %s with initial max "
              "spill depth of %d and adding to list\n",
              location,
              self()->cg()->getDebug()->getName(bestRegister),
              self()->cg()->getCurrentPathDepth());
        } else {
          location = self()->cg()->allocateSpill(
            TR::Compiler->om.sizeofReferenceAddress(),
            bestRegister->containsCollectedReference(),
            &offset);
          location->setMaxSpillDepth(self()->cg()->getCurrentPathDepth());
          if (self()->cg()->getDebug())
            self()->cg()->traceRegisterAssignment(
...
TR::RealRegister*
OMR::X86::Machine::reverseGPRSpillState(TR::Instruction* currentInstruction,
                                        ...
                                        TR_RegisterSizes requestedRegSize)
{
  ...
  TR::MemoryReference* tempMR =
    generateX86MemoryReference(location->getSymbolReference(),
...
void
OMR::X86::Machine::coerceGPRegisterAssignment(
  ...
  if (targetRegister->getState() == TR::RealRegister::Free) {
    ...
    if (currentAssignedRegister != NULL) {
      ...
      if (candidate) {
        ...
        candidate = self()->freeBestGPRegister(currentInstruction,
...
void
OMR::X86::Machine::coerceXMMRegisterAssignment(
  TR::Instruction* currentInstruction,
  TR::Register* virtualRegister,
...
void
OMR::X86::Machine::swapGPRegisters(TR::Instruction* currentInstruction,
  ...
  TR_RegisterSizes requestedRegSize)
{
  ...
  if ((candidate = self()->findBestFreeGPRegister(
    ...
    candidate = self()->freeBestGPRegister(
      currentInstruction, virtualRegister, requestedRegSize);
  }
  ...
  if (virtualRegister->getTotalUseCount() !=
    ...
    self()->cg()->setRegisterAssignmentFlag(TR_RegisterReloaded);
    self()->reverseGPRSpillState(
      currentInstruction, virtualRegister, candidate);
  }
    ...
    if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i) ||
      ...
      if (assocReg
            ->isPlaceholderReg()) { // placeholder register and is only needed
                                    // at the specific dependency site (usually
                                    // a killed register on a call) so defer
                                    // this register's weight to that of
                                    // registers where the associated register
                                    // has a longer life
...
void
OMR::X86::Machine::initializeRegisterFile(
  const struct TR::X86LinkageProperties& properties)
{
  ...
  for (reg = TR::RealRegister::r8; reg <= TR::RealRegister::LastAssignableGPR;
    ...
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
      ...
      TR::RealRegister::gprMask((TR::RealRegister::RegNum)reg),
      self()->cg());
  }
#endif
  ...
  for (reg = TR::RealRegister::FirstFPR; reg <= TR::RealRegister::LastFPR;
    ...
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
      ...
      TR::RealRegister::Free,
      (TR::RealRegister::RegNum)reg,
      TR::RealRegister::fprMask((TR::RealRegister::RegNum)reg),
      self()->cg());
  }
  ...
  for (reg = TR::RealRegister::FirstMMXR; reg <= TR::RealRegister::LastMMXR;
    ...
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
      ...
      TR::RealRegister::Free,
      (TR::RealRegister::RegNum)reg,
      TR::RealRegister::mmrMask((TR::RealRegister::RegNum)reg),
      self()->cg());
  }
...
uint32_t*
OMR::X86::Machine::getGlobalRegisterTable(
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastXMMR;
      ...
      if (realReg->getState() == TR::RealRegister::Assigned ||
        ...
        deps->addPostCondition(
          virtReg, realReg->getRegisterNumber(), self()->cg());
...
void
OMR::X86::Machine::installRegisterFile(TR::RealRegister** registerFileCopy)
{
  int32_t endReg = TR::RealRegister::LastXMMR;
...
OMR::X86::Machine::captureSpilledRegistersList()
{
  ...
  for (auto iter = self()->cg()->getSpilledRegisterList()->begin();
       iter != self()->cg()->getSpilledRegisterList()->end();
       ++iter) {
    srl->push_front(*iter);
  }
  ...
  TR_OutlinedInstructions* oi)
{
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (realReg->getState() == TR::RealRegister::Assigned) {
      TR::Register* virtReg = realReg->getAssignedRegister();
      deps->addPostCondition(
        virtReg, realReg->getRegisterNumber(), _machine->cg());
      ...
      if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
        traceMsg(comp,
                 "   create ASSIGNED dependency: virtual %p -> %s\n",
                 virtReg,
  ...
  for (auto iter = _spilledRegistersList->begin();
       iter != _spilledRegistersList->end();
       ++iter) {
...
//
void
TR_RegisterAssignerState::capture()
{
...
//
void
TR_RegisterAssignerState::install()
{
...
//
bool
TR_RegisterAssignerState::isLive(TR::Register* virtReg)
{
...
void
OMR::X86::Machine::adjustRegisterUseCountsUp(TR::list<OMR::RegisterUsage*>* rul,
                                             bool adjustFuture)
{
  if (!rul)
    return;
...
void
OMR::X86::Machine::adjustRegisterUseCountsDown(
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned) {
      ...
      if (location) {
        ...
        self()->cg()->freeSpill(
          location, size, virtReg->isSpilledToSecondHalf() ? 4 : 0);
...
TR::MemoryReference*
OMR::X86::Machine::getDummyLocalMR(TR::DataType dt)
{
...
void
OMR::X86::Machine::initializeFPStackRegisterFile()
{
...
//
bool
OMR::X86::Machine::isFPRTopOfStack(TR::Register* virtReg)
{
  TR_X86FPStackRegister* fpReg =
...
//
void
OMR::X86::Machine::fpStackPush(TR::Register* virtReg)
{
...
//
void
OMR::X86::Machine::fpStackCoerce(TR::Register* virtReg, int32_t stackLocation)
{
  //
  // Check for stack overflow.
  //
...
//
void
OMR::X86::Machine::fpCoerceRegistersToTopOfStack(
...
//
TR_X86OpCodes
OMR::X86::Machine::fpDetermineReverseOpCode(TR_X86OpCodes op)
{
...
//
TR_X86OpCodes
OMR::X86::Machine::fpDeterminePopOpCode(TR_X86OpCodes op)
{
...
OMR::X86::Machine::freeBestFPRegister(TR::Instruction* prevInstruction)
{
  ...
  for (int i = TR_X86FPStackRegister::fpFirstStackReg;
       i < TR_X86FPStackRegister::NumRegisters;
       i++) {
...
//
TR::Instruction*
OMR::X86::Machine::fpSpillFPR(TR::Instruction* prevInstruction,
                              TR::Register* vreg)
{
  ...
  if (fpReg != NULL) {
    ...
    cursor = new (self()->cg()->trHeapMemory())
      TR::X86FPMemRegInstruction(cursor,
                                 isFloat ? FSTPMemReg : DSTPMemReg,
                                 tempMR,
                                 self()->fpMapToStackRelativeRegister(vreg),
                                 self()->cg());
  }
...
#if defined(DEBUG)
static void
printOneRegisterStatus(TR_FrontEnd* fe,
                       TR::FILE* pOutFile,
                       TR_Debug* debug,
                       TR::RealRegister* reg)
{
...
void
OMR::X86::Machine::printGPRegisterStatus(TR_FrontEnd* fe,
...
//
void
OMR::X86::Machine::printFPRegisterStatus(TR_FrontEnd* fe, TR::FILE* pOutFile)
{
  char buf[32];
  char* cursor;
  int32_t i, j;
