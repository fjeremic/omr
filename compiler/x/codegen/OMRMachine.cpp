...
bool existsNextInstructionToTestFlags(TR::Instruction* startInstr, uint8_t testMask)
{
  ...
  } while (testMask && cursor->getOpCodeValue() != LABEL && cursor->getOpCodeValue() != RET
    && cursor->getOpCodeValue() != RETImm2 && cursor->getOpCodeValue() != ReturnMarker
...
TR::RealRegister* OMR::X86::Machine::findBestFreeGPRegister(
  TR::Instruction* currentInstruction, TR::Register* virtReg, TR_RegisterSizes requestedRegSize, bool considerUnlatched)
{
  int first, last, i;
  ...
  for (i = first; i <= last; i++) {
    ...
    if (((_registerFile[i]->getAssignedRegister() == NULL && _registerFile[i]->getState() != TR::RealRegister::Blocked)
  ...
  TR_RegisterSizes requestedRegSize, TR::RealRegister::RegNum targetRegister, bool considerVirtAsSpillCandidate)
{
  ...
  char* bestRegisterTypes[NumBestRegisters] = { "non-interfering discardable (from constant) register",
    ...
    "interfering discardable (from memory) register", "non-interfering target register", "interfering target register",
    "non-interfering register", "may interfere byte register", "interfering register" };
  ...
  if (numCandidates == 0) {
    ...
    //
    TR_ASSERT(considerVirtAsSpillCandidate, "freeBestGPRegister(): could not find any GPR spill candidates for %s\n",
  ...
  for (cursor = currentInstruction->getPrev(); cursor; cursor = cursor->getPrev()) {
    ...
    if (cursor->getOpCodeValue() == FENCE || cursor->getOpCodeValue() == FPREGSPILL)
      continue;
  ...
  if (bestType >= 0) {
    ...
    for (i = 0; i < bestType; i++) {
      if (bestDistances[i] && (bestDistance - bestDistances[i]) < freeBestRegisterSignificantDistances[i][bestType]
  ...
  if (enableRematerialisation && bestDiscardableRegister) {
    ...
    if (info->isRematerializableFromMemory()) {
      ...
      if (info->isIndirect()) {
        TR_ASSERT(info->getBaseRegister()->getAssignedRegister(),
          ...
          self()->getDebug()->getName(info->getBaseRegister()), self()->getDebug()->getName(bestDiscardableRegister));
      ...
      if (info->getDataType() == TR_RematerializableFloat) {
        ...
        instr
          = TR::TreeEvaluator::insertLoadMemory(0, best, tempMR, info->getDataType(), self()->cg(), currentInstruction);
      }
      ...
      if (info->getDataType() == TR_RematerializableFloat) {
        TR::MemoryReference* tempMR = generateX86MemoryReference(
          self()->cg()->findOrCreate4ByteConstant(currentInstruction->getNode(), info->getConstant()), self()->cg());
        instr = generateRegMemInstruction(currentInstruction, MOVSSRegMem, best, tempMR, self()->cg());
      } else {
    ...
    if (debug("dumpRemat")) {
      if (info->isIndirect())
        diagnostic("---> Spilling %s rematerialisable register %s (type %d, base %s)\n",
          self()->getDebug()->toString(info), self()->getDebug()->getName(bestDiscardableRegister), info->getDataType(),
        ...
        diagnostic("---> Spilling %s rematerialisable register %s (type %d)\n", self()->getDebug()->toString(info),
          self()->getDebug()->getName(bestDiscardableRegister), info->getDataType());
    }
  } else {
    ...
    if ((bestRegister->getKind() == TR_FPR)) {
      ...
      if (containsInternalPointer) {
        ...
        if (bestRegister->getBackingStorage()) {
          ...
          if (self()->cg()->getDebug())
            self()->cg()->traceRegisterAssignment(
              "find or create free backing store (%p) for %s with initial max spill depth of %d and adding to list\n",
              location, self()->cg()->getDebug()->getName(bestRegister), self()->cg()->getCurrentPathDepth());
        } else {
          location = self()->cg()->allocateSpill(
            TR::Compiler->om.sizeofReferenceAddress(), bestRegister->containsCollectedReference(), &offset);
          location->setMaxSpillDepth(self()->cg()->getCurrentPathDepth());
          if (self()->cg()->getDebug())
            self()->cg()->traceRegisterAssignment(
              "find or create free backing store (%p) for %s with initial max spill depth of %d and adding to list\n",
    ...
    instr
      = new (self()->cg()->trHeapMemory()) TR::X86RegMemInstruction(currentInstruction, op, best, tempMR, self()->cg());
  ...
  TR::Register* spilledRegister, TR::RealRegister* targetRegister, TR_RegisterSizes requestedRegSize)
{
  ...
  if (spilledRegister->getKind() == TR_FPR) {
    instr = new (self()->cg()->trHeapMemory()) TR::X86MemRegInstruction(currentInstruction,
      spilledRegister->isSinglePrecision() ? MOVSSMemReg : MOVSDMemReg, tempMR, targetRegister, self()->cg());
    ...
    instr = new (self()->cg()->trHeapMemory())
      TR::X86MemRegInstruction(currentInstruction, MOVDQUMemReg, tempMR, targetRegister, self()->cg());
    ...
    instr = new (self()->cg()->trHeapMemory())
      TR::X86MemRegInstruction(currentInstruction, SMemReg(), tempMR, targetRegister, self()->cg());
    // Do not add a freed spill slot back onto the free list if the list is locked.
    // This is to enforce re-use of the same spill slot for a virtual register
    // while assigning non-linear control flow regions.
    //
    self()->cg()->freeSpill(
      location, TR::Compiler->om.sizeofReferenceAddress(), spilledRegister->isSpilledToSecondHalf() ? 4 : 0);
    if (!self()->cg()->isFreeSpillListLocked()) {
      spilledRegister->setBackingStorage(NULL);
    }
  }
  ...
  if (targetRegister->getState() == TR::RealRegister::Free) {
    if (currentAssignedRegister == NULL) {
      ...
      instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
        currentInstruction, MOVRegReg(), currentAssignedRegister, targetRegister, self()->cg());
    ...
    if (currentAssignedRegister != NULL) {
      instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
        currentInstruction, XCHGRegReg(), currentAssignedRegister, targetRegister, self()->cg());
      ...
      if ((targetRegister != candidate) && (candidate != currentTargetVirtual)) {
        instr = new (self()->cg()->trHeapMemory())
          TR::X86RegRegInstruction(currentInstruction, MOVRegReg(), targetRegister, candidate, self()->cg());
  ...
  if (targetRegister->getState() == TR::RealRegister::Free) {
    if (currentAssignedRegister == NULL) {
      ...
      if (virtualRegister->getKind() == TR_VRF) {
        instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
          currentInstruction, MOVDQURegReg, currentAssignedRegister, targetRegister, self()->cg());
      } else if (virtualRegister->isSinglePrecision()) {
        instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
          currentInstruction, MOVAPSRegReg, currentAssignedRegister, targetRegister, self()->cg());
      } else {
        instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
          currentInstruction, MOVAPDRegReg, currentAssignedRegister, targetRegister, self()->cg());
      }
      currentAssignedRegister->setState(TR::RealRegister::Free);
      currentAssignedRegister->setAssignedRegister(NULL);
    }
    ...
    if (currentAssignedRegister != NULL) {
      ...
      if (virtualRegister->getKind() == TR_FPR && virtualRegister->isSinglePrecision()) {
        xchgOp = XORPSRegReg;
      } else // virtualRegister->getKind() == TR_VRF || (virtualRegister->getKind() == TR_FPR &&
      ...
      self()->cg()->traceRegAssigned(currentTargetVirtual, currentAssignedRegister);
      instr = new (self()->cg()->trHeapMemory())
        TR::X86RegRegInstruction(currentInstruction, xchgOp, currentAssignedRegister, targetRegister, self()->cg());
      self()->cg()->traceRAInstruction(instr);
      instr = new (self()->cg()->trHeapMemory())
        TR::X86RegRegInstruction(currentInstruction, xchgOp, targetRegister, currentAssignedRegister, self()->cg());
      self()->cg()->traceRAInstruction(instr);
      instr = new (self()->cg()->trHeapMemory())
        TR::X86RegRegInstruction(currentInstruction, xchgOp, currentAssignedRegister, targetRegister, self()->cg());
      self()->cg()->traceRAInstruction(instr);
      ...
      if (targetRegister != candidate) {
        if (virtualRegister->getKind() == TR_VRF) {
          instr = new (self()->cg()->trHeapMemory())
            TR::X86RegRegInstruction(currentInstruction, MOVDQURegReg, targetRegister, candidate, self()->cg());
        } else if (currentTargetVirtual->isSinglePrecision()) {
          instr = new (self()->cg()->trHeapMemory())
            TR::X86RegRegInstruction(currentInstruction, MOVAPSRegReg, targetRegister, candidate, self()->cg());
        } else {
          instr = new (self()->cg()->trHeapMemory())
            TR::X86RegRegInstruction(currentInstruction, MOVAPDRegReg, targetRegister, candidate, self()->cg());
        }
  ...
  TR::Instruction* currentInstruction, TR::RealRegister::RegNum regNum1, TR::RealRegister::RegNum regNum2)
{
  ...
  TR::Instruction* instr = new (self()->cg()->trHeapMemory())
    TR::X86RegRegInstruction(currentInstruction, XCHGRegReg(), realReg1, realReg2, self()->cg());
...
void OMR::X86::Machine::initializeRegisterFile(const struct TR::X86LinkageProperties& properties)
{
  ...
  if (!dontUseEBXasGPR) {
    ...
    _registerFile[TR::RealRegister::ebx] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
      TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::ebx, TR::RealRegister::ebxMask, self()->cg());
  ...
  _registerFile[TR::RealRegister::ebp] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
    TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::ebp, TR::RealRegister::ebpMask, self()->cg());
  _registerFile[TR::RealRegister::ebp]->setAssignedRegister(_registerFile[TR::RealRegister::ebp]);

  _registerFile[TR::RealRegister::esp] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
    TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::esp, TR::RealRegister::espMask, self()->cg());
  _registerFile[TR::RealRegister::esp]->setAssignedRegister(_registerFile[TR::RealRegister::esp]);

  _registerFile[TR::RealRegister::vfp] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
    TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked, TR::RealRegister::vfp, TR::RealRegister::noRegMask, self()->cg());
  ...
  for (reg = TR::RealRegister::r8; reg <= TR::RealRegister::LastAssignableGPR; reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(TR_GPR,
      ...
      TR::RealRegister::Free, (TR::RealRegister::RegNum)reg, TR::RealRegister::gprMask((TR::RealRegister::RegNum)reg),
      self()->cg());
  }
#endif
  ...
  for (reg = TR::RealRegister::FirstFPR; reg <= TR::RealRegister::LastFPR; reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(TR_X87,
      properties.isPreservedRegister((TR::RealRegister::RegNum)reg) ? PRESERVED_WEIGHT : NONPRESERVED_WEIGHT,
      TR::RealRegister::Free, (TR::RealRegister::RegNum)reg, TR::RealRegister::fprMask((TR::RealRegister::RegNum)reg),
      self()->cg());
  }
  ...
  for (reg = TR::RealRegister::FirstMMXR; reg <= TR::RealRegister::LastMMXR; reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(TR_GPR,
      properties.isPreservedRegister((TR::RealRegister::RegNum)reg) ? PRESERVED_WEIGHT : NONPRESERVED_WEIGHT,
      TR::RealRegister::Free, (TR::RealRegister::RegNum)reg, TR::RealRegister::mmrMask((TR::RealRegister::RegNum)reg),
      self()->cg());
  }
  ...
  for (reg = TR::RealRegister::FirstXMMR; reg <= TR::RealRegister::LastXMMR; reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(TR_FPR,
      properties.isPreservedRegister((TR::RealRegister::RegNum)reg) ? PRESERVED_WEIGHT : NONPRESERVED_WEIGHT,
      TR::RealRegister::Free, (TR::RealRegister::RegNum)reg, TR::RealRegister::xmmrMask((TR::RealRegister::RegNum)reg),
      self()->cg());
  }
}
...
TR::RealRegister** OMR::X86::Machine::cloneRegisterFile(TR::RealRegister** registerFile, TR_AllocationKind allocKind)
{
  ...
  // the vfp entry is a real register and it must always point to the same _frameRegister pointer so the memRef
  // assignRegisters check if (_baseRegister == cg()->machine()->getRealRegister(TR::RealRegister::vfp) works correctly
  ...
  TR_OutlinedInstructions* oi)
{
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (realReg->getState() == TR::RealRegister::Assigned) {
      ...
      if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
        traceMsg(
          comp, "   create ASSIGNED dependency: virtual %p -> %s\n", virtReg, _machine->getDebug()->getName(realReg));
      }
  ...
  for (auto iter = _spilledRegistersList->begin(); iter != _spilledRegistersList->end(); ++iter) {
    ...
    if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(
        comp, "   create SPILLED dependency: virtual %p -> backing storage %p\n", *iter, (*iter)->getBackingStorage());
    }
  }
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned) {
      ...
      if (location) {
        ...
        traceMsg(
          self()->cg()->comp(), "disassociating backing storage %p from assigned virtual %p\n", location, virtReg);
      }
    }
  }
}
...
void OMR::X86::Machine::fpStackCoerce(TR::Register* virtReg, int32_t stackLocation)
{
  ...
  // Check for stack overflow.
  //
  TR_ASSERT((stackLocation < TR_X86FPStackRegister::fpStackFull), "fpStackCoerce() ==> Floating point stack overflow!");
...
TR::Register* OMR::X86::Machine::fpStackPop()
{
  ...
  // Check for stack underflow.
  //
  TR_ASSERT((_fpTopOfStack > TR_X86FPStackRegister::fpStackEmpty), "fpStackPop() ==> Floating point stack underflow!");
...
TR::Instruction* OMR::X86::Machine::freeBestFPRegister(TR::Instruction* prevInstruction)
{
  ...
  while (numCandidates > 1 && cursor != NULL && cursor->getOpCodeValue() != LABEL && cursor->getOpCodeValue() != RET
    && cursor->getOpCodeValue() != RETImm2 && cursor->getOpCodeValue() != ReturnMarker
    && !cursor->getOpCode().isBranchOp()) {
...
TR::Instruction* OMR::X86::Machine::fpSpillFPR(TR::Instruction* prevInstruction, TR::Register* vreg)
{
  ...
  if (fpReg != NULL) {
    ...
    cursor = new (self()->cg()->trHeapMemory()) TR::X86FPMemRegInstruction(
      cursor, isFloat ? FSTPMemReg : DSTPMemReg, tempMR, self()->fpMapToStackRelativeRegister(vreg), self()->cg());
  }
  ...
  TR::Instruction* prevInstruction, TR::Register* spilledRegister)
{
  ...
  cursor = new (self()->cg()->trHeapMemory())
    TR::X86FPRegMemInstruction(cursor, isFloat ? FLDRegMem : DLDRegMem, realFPReg, tempMR, self()->cg());
