...
static const int16_t
    freeBestRegisterSignificantDistances[NumBestRegisters][NumBestRegisters] = {
        {0, 5, 10, 20, 20, 20, 40, 40, 60, 60,
         60},  // BestNonInterferingDiscardableConstant
        {0, 0, 10, 20, 20, 20, 40, 40, 60, 60,
         60},  // BestMayInterfereDiscardableConstant
        {0, 0, 0, 10, 20, 20, 30, 30, 50, 50,
         50},  // BestInterferingDiscardableConstant
        {0, 0, 0, 0, 5, 10, 40, 40, 60, 60,
         60},  // BestNonInterferingDiscardableMemory
        {0, 0, 0, 0, 0, 10, 40, 40, 60, 60,
         60},  // BestMayInterfereDiscardableMemory
        {0, 0, 0, 0, 0, 0, 30, 30, 50, 50,
         50},  // BestInterferingDiscardableMemory
...
bool existsNextInstructionToTestFlags(TR::Instruction *startInstr,
  ...
  do {
    ...
    // Cursor instruction tests an eflag that is set exclusively by our
    // testMask.
    //
  ...
  } while (testMask && cursor->getOpCodeValue() != LABEL &&
...
OMR::X86::Machine::Machine(uint8_t numIntRegs, uint8_t numFPRegs,
                           TR::CodeGenerator *cg,
                           TR::Register **registerAssociations,
                           uint8_t numGlobalGPRs, uint8_t numGlobal8BitGPRs,
    ...
    : OMR::Machine(cg),
      ...
      _globalRegisterNumberToRealRegisterMap(
          globalRegisterNumberToRealRegisterMap),
...
TR::RealRegister *OMR::X86::Machine::findBestFreeGPRegister(
  ...
  TR_ASSERT((virtReg &&
             (virtReg->getKind() == TR_GPR || virtReg->getKind() == TR_FPR ||
              virtReg->getKind() == TR_VRF)),
            "OMR::X86::Machine::findBestFreeGPRegister() ==> Unexpected "
            "register kind!");

  bool useRegisterAssociations =
      self()->cg()->enableRegisterAssociations() ? true : false;
  bool useRegisterInterferences =
      self()->cg()->enableRegisterInterferences() ? true : false;
  bool useRegisterWeights =
      self()->cg()->enableRegisterWeights() ? true : false;

  if (useRegisterAssociations &&
      virtReg->getAssociation() != TR::RealRegister::NoReg &&
      self()->getVirtualAssociatedWithReal(
          (TR::RealRegister::RegNum)virtReg->getAssociation()) == virtReg &&
      (_registerFile[virtReg->getAssociation()]->getAssignedRegister() ==
           NULL ||
       (considerUnlatched &&
        _registerFile[virtReg->getAssociation()]->getState() ==
            TR::RealRegister::Unlatched))) {
    if (_registerFile[virtReg->getAssociation()]->getState() !=
        TR::RealRegister::Locked) {
      if (requestedRegSize != TR_ByteReg ||
          virtReg->getAssociation() <= TR::RealRegister::Last8BitGPR) {
  ...
  for (i = first; i <= last; i++) {
    ...
    if (useRegisterWeights) {
      if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i) ||
           linkageProperties.isCalleeVolatileRegister(
               (TR::RealRegister::RegNum)i)) &&
          _registerFile[i]->getWeight() ==
              IA32_REGISTER_INITIAL_PRESERVED_WEIGHT) {
      ...
      //
      // Increase the weight if the register interferes with the virtual
      // register.
      //
      if (associatedRegister &&
          !registersMayOverlap(virtReg, associatedRegister))
        weight &= ~(IA32_REGISTER_ASSOCIATED_WEIGHT |
                    IA32_REGISTER_PLACEHOLDER_WEIGHT);
  ...
  if (useRegisterInterferences && (numCandidates > 1)) {
    ...
    int32_t distance = 0;
    for (cursor = currentInstruction->getPrev(); cursor && numCandidates > 1;
         cursor = cursor->getPrev()) {
      if (cursor->getOpCodeValue() == PROCENTRY) break;

      if (cursor->getOpCodeValue() == ASSOCREGS) continue;

      for (i = 0; i < numCandidates; i++) {
        if (cursor->refsRegister(candidates[i]._virtual)) {
          self()->cg()->traceRegInterference(virtReg, candidates[i]._virtual,
                                             distance);
...
TR::RealRegister *OMR::X86::Machine::freeBestGPRegister(
    TR::Instruction *currentInstruction, TR::Register *virtReg,
    TR_RegisterSizes requestedRegSize, TR::RealRegister::RegNum targetRegister,
    bool considerVirtAsSpillCandidate) {
  TR::Register *candidates[16];  // TODO:AMD64: Should be max number of regs of
                                 // any one kind
  int numCandidates = 0;
  int first, last;
  TR::Instruction *cursor;
  ...
  bool enableRematerialisation =
      self()->cg()->enableRematerialisation() ? true : false;

  TR_ASSERT(
      virtReg && (virtReg->getKind() == TR_GPR ||
                  virtReg->getKind() == TR_FPR || virtReg->getKind() == TR_VRF),
      "OMR::X86::Machine::freeBestGPRegister() ==> expecting to free GPRs or "
      "XMMRs only!");
  ...
  if (targetRegister)
    self()->cg()->traceRegisterAssignment(
        "Spill candidates for %R (target %R):", virtReg,
    ...
    if (realReg->getState() == TR::RealRegister::Assigned) {
      ...
      if (targetRegister == i) {
        if (useRegisterInterferences &&
            (interference &
             (1 << (i - 1))))  // TODO:AMD64: Use the proper mask value
  ...
  if (numCandidates == 0) {
    // If we are trying to find a suitable spill register for a virtual that
    // currently occupies a register required in a register dependency, there
    // may not be any suitable spill candidates if all available real registers
    // are requested in the register dependency.  In such a case, where our
    // virtual need not be in a register across the dependency, choose the
    // virtual itself as a spill candidate.
    //
    TR_ASSERT(considerVirtAsSpillCandidate,
              "freeBestGPRegister(): could not find any GPR spill candidates "
              "for %s\n",
  ...
  // From all the spillable candidates identified, choose the most appropriate
  // based on its rematerialisation value and its proximity to the current
  // instruction.
  //
  TR::RealRegister::RegNum registerNumber;
  for (cursor = currentInstruction->getPrev(); cursor;
       cursor = cursor->getPrev()) {
    if (cursor->getOpCodeValue() == PROCENTRY) break;

    if (numCandidates == 0) break;

    if (distance > FREE_BEST_REGISTER_SEARCH_DISTANCE) break;
    ...
    for (i = 0; i < numCandidates; i++) {
      registerNumber = toRealRegister(candidates[i]->getAssignedRegister())
                           ->getRegisterNumber();
      ...
      interferes =
          interference &
          (1 << (registerNumber - 1));  // TODO:AMD64: Use the proper mask value
      ...
      if (cursor->refsRegister(candidates[i])) {
        if (distance > FREE_BEST_REGISTER_MINIMUM_CANDIDATE_DISTANCE) {
          TR_RematerializationInfo *info =
              candidates[i]->isDiscardable()
                  ? candidates[i]->getRematerializationInfo()
                  : NULL;

          if (enableRematerialisation && info && info->isActive()) {
            if (info->isRematerializableFromMemory() ||
                info->isRematerializableFromAddress()) {
              if (interferes)
                j = BestInterferingDiscardableMemory;
              else if (byteRegisterInterference &&
                       registerNumber <= TR::RealRegister::Last8BitGPR)
                j = BestMayInterfereDiscardableMemory;
              else
                j = BestNonInterferingDiscardableMemory;
            } else {
              TR_ASSERT(candidates[i]
                            ->getRematerializationInfo()
                            ->isRematerializableFromConstant(),
                        "freeBestGPRegister => unknown rematerialisable "
                        "register type!");
              if (interferes)
            ...
            if (debug("dumpRemat")) {
              diagnostic(
                  "---> Identified %s rematerialisation spill "
                  "candidate %s at instruction %s in %s\n",
                  self()->getDebug()->toString(info),
                  self()->getDebug()->getName(candidates[i]),
                  self()->getDebug()->getName(currentInstruction),
                  comp->signature());
            }
          } else {
            if (useRegisterInterferences) {
          ...
          bestType = j;
          if (useRegisterInterferences &&
              (bestRegisters[j] == bestRegisters[InterferingTarget]))
  ...
  if (bestType >= 0) {
    for (i = 0; i < NumBestRegisters; i++) {
      if (bestRegisters[i])
        self()->cg()->traceRegisterAssignment(
            "   (best %s %R at distance %d)", bestRegisters[i],
    ...
    for (i = 0; i < bestType; i++) {
      if (bestDistances[i] &&
          (bestDistance - bestDistances[i]) <
              freeBestRegisterSignificantDistances[i][bestType] &&
  ...
  TR::Register *bestDiscardableRegister =
      (bestRegister->isDiscardable() &&
       bestRegister->getRematerializationInfo()->isActive())
          ? bestRegister
          : NULL;
  ...
  if (enableRematerialisation && bestDiscardableRegister) {
    TR_RematerializationInfo *info =
        bestDiscardableRegister->getRematerializationInfo();

    if (info->isRematerializableFromMemory()) {
      TR::MemoryReference *tempMR =
          generateX86MemoryReference(info->getSymbolReference(), self()->cg());

      if (info->isIndirect()) {
        TR_ASSERT(info->getBaseRegister()->getAssignedRegister(),
                  "base register %s of dependent rematerialisable register %s "
                  "must be assigned\n",
      ...
      if (info->getDataType() == TR_RematerializableFloat) {
        instr = generateRegMemInstruction(currentInstruction, MOVSSRegMem, best,
                                          tempMR, self()->cg());
      } else {
        instr = TR::TreeEvaluator::insertLoadMemory(
            0, best, tempMR, info->getDataType(), self()->cg(),
            currentInstruction);
      }
    } else if (info->isRematerializableFromAddress()) {
      TR::MemoryReference *tempMR =
          generateX86MemoryReference(info->getSymbolReference(), self()->cg());
      instr = generateRegMemInstruction(currentInstruction, LEARegMem(), best,
                                        tempMR, self()->cg());
    } else {
      if (info->getDataType() == TR_RematerializableFloat) {
        TR::MemoryReference *tempMR = generateX86MemoryReference(
            self()->cg()->findOrCreate4ByteConstant(
                currentInstruction->getNode(), info->getConstant()),
            self()->cg());
        instr = generateRegMemInstruction(currentInstruction, MOVSSRegMem, best,
                                          tempMR, self()->cg());
      } else {
        instr = TR::TreeEvaluator::insertLoadConstant(
            0, best, info->getConstant(), info->getDataType(), self()->cg(),
            currentInstruction);
      }
    }
    ...
    if (debug("dumpRemat")) {
      if (info->isIndirect())
        diagnostic(
            "---> Spilling %s rematerialisable register %s (type %d, base "
            "%s)\n",
            self()->getDebug()->toString(info),
            self()->getDebug()->getName(bestDiscardableRegister),
            info->getDataType(),
    ...
    if (bestRegister->containsInternalPointer()) containsInternalPointer = true;
    ...
    if ((bestRegister->getKind() == TR_FPR)) {
      if (bestRegister->getBackingStorage()) {
        ...
        location = self()->cg()->allocateSpill(
            bestRegister->isSinglePrecision() ? 4 : 8, false, &offset);
      }
      ...
      if (containsInternalPointer) {
        if (bestRegister->getBackingStorage()) {
          ...
          location = self()->cg()->allocateInternalPointerSpill(
              bestRegister->getPinningArrayPointer());
        }
      } else {
        if (bestRegister->getBackingStorage()) {
          // If there is backing storage associated with a register, it means
          ...
          if (self()->cg()->getDebug())
            self()->cg()->traceRegisterAssignment(
                "find or create free backing store (%p) for %s with initial "
                "max spill depth of %d and adding to list\n",
    ...
    if (self()->cg()->getUseNonLinearRegisterAssigner()) {
      ...
      TR_ASSERT(self()->cg()->getSpilledRegisterList(),
                "no spilled reg list allocated");
    ...
    TR::MemoryReference *tempMR = generateX86MemoryReference(
        location->getSymbolReference(), offset, self()->cg());
    ...
    TR_X86OpCodes op;
    if (bestRegister->getKind() == TR_FPR) {
      op = (bestRegister->isSinglePrecision())
               ? MOVSSRegMem
    ...
    instr = new (self()->cg()->trHeapMemory()) TR::X86RegMemInstruction(
        currentInstruction, op, best, tempMR, self()->cg());
...
TR::RealRegister *OMR::X86::Machine::reverseGPRSpillState(
    TR::Instruction *currentInstruction, TR::Register *spilledRegister,
    TR::RealRegister *targetRegister, TR_RegisterSizes requestedRegSize) {
  TR::Compilation *comp = self()->cg()->comp();
  if (targetRegister == NULL) {
    targetRegister = self()->findBestFreeGPRegister(
        currentInstruction, spilledRegister, requestedRegSize);
    if (targetRegister == NULL) {
      targetRegister = self()->freeBestGPRegister(
          currentInstruction, spilledRegister, requestedRegSize);
    }
  }
  ...
  if (self()->cg()->enableBetterSpillPlacements()) {
    if (spilledRegister->hasBetterSpillPlacement()) {
      TR::Instruction *betterInstruction =
          self()->cg()->findBetterSpillPlacement(
              spilledRegister, targetRegister->getRegisterNumber());
  ...
  if (self()->cg()->enableRematerialisation()) {
    ...
    if (spilledRegister->getRematerializationInfo() &&
      ...
      if (debug("dumpRemat")) {
        diagnostic(
            "---> Avoiding storing %s rematerializable register %s to memory "
            "in %s\n",
            self()->getDebug()->toString(
                spilledRegister->getRematerializationInfo()),
  ...
  if (spilledRegister->getKind() == TR_FPR) {
    ...
    //
    self()->cg()->freeSpill(location,
                            spilledRegister->isSinglePrecision() ? 4 : 8,
    ...
    instr = new (self()->cg()->trHeapMemory()) TR::X86MemRegInstruction(
        currentInstruction, MOVDQUMemReg, tempMR, targetRegister, self()->cg());
    ...
    instr = new (self()->cg()->trHeapMemory()) TR::X86MemRegInstruction(
        currentInstruction, SMemReg(), tempMR, targetRegister, self()->cg());
    // Do not add a freed spill slot back onto the free list if the list is
    // locked. This is to enforce re-use of the same spill slot for a virtual
    // register while assigning non-linear control flow regions.
    //
    self()->cg()->freeSpill(location, TR::Compiler->om.sizeofReferenceAddress(),
                            spilledRegister->isSpilledToSecondHalf() ? 4 : 0);
    if (!self()->cg()->isFreeSpillListLocked()) {
      spilledRegister->setBackingStorage(NULL);
    }
...
void OMR::X86::Machine::coerceGPRegisterAssignment(
  ...
  TR::RealRegister *currentAssignedRegister =
      virtualRegister->getAssignedRealRegister();
  TR::Instruction *instr = NULL;
  ...
  if (targetRegister->getState() == TR::RealRegister::Free) {
    if (currentAssignedRegister == NULL) {
      if (virtualRegister->getTotalUseCount() !=
        ...
        self()->reverseGPRSpillState(currentInstruction, virtualRegister,
                                     targetRegister);
      }
    } else {
      instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
          currentInstruction, MOVRegReg(), currentAssignedRegister,
    ...
    if (instr) self()->cg()->traceRAInstruction(instr);
    ...
    if (currentAssignedRegister != NULL) {
      ...
      TR::RealRegister *candidate = self()->findBestFreeGPRegister(
      ...
      if (candidate) {
        ...
        // A spill is needed, either to satisfy the coercion, or to free up a
        // second register whence the virtual currently occupying the target may
        // be moved, i.e. an indirect coercion.
        self()->cg()->setRegisterAssignmentFlag(TR_RegisterSpilled);
        candidate = self()->freeBestGPRegister(
            currentInstruction, currentTargetVirtual, TR_WordReg,
            registerNumber, coerceToSatisfyRegDeps);
      }

      if ((targetRegister != candidate) &&
          (candidate != currentTargetVirtual)) {
        instr = new (self()->cg()->trHeapMemory())
            TR::X86RegRegInstruction(currentInstruction, MOVRegReg(),
        ...
        self()->cg()->traceRAInstruction(instr);
        self()->cg()->resetRegisterAssignmentFlag(
            TR_RegisterSpilled);  // the spill (if any) has been traced
      }
  ...
  targetRegister->setState(TR::RealRegister::Assigned,
                           virtualRegister->isPlaceholderReg());
...
void OMR::X86::Machine::coerceXMMRegisterAssignment(
    TR::Instruction *currentInstruction, TR::Register *virtualRegister,
  ...
  if (targetRegister->getState() == TR::RealRegister::Free) {
    if (currentAssignedRegister == NULL) {
      ...
      if (virtualRegister->getKind() == TR_VRF) {
        instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
            currentInstruction, MOVDQURegReg, currentAssignedRegister,
            targetRegister, self()->cg());
      } else if (virtualRegister->isSinglePrecision()) {
        instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
            currentInstruction, MOVAPSRegReg, currentAssignedRegister,
            targetRegister, self()->cg());
      } else {
        instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
            currentInstruction, MOVAPDRegReg, currentAssignedRegister,
            targetRegister, self()->cg());
      }
      currentAssignedRegister->setState(TR::RealRegister::Free);
      currentAssignedRegister->setAssignedRegister(NULL);
    ...
    if (currentAssignedRegister != NULL) {
      ...
      if (virtualRegister->getKind() == TR_FPR &&
          virtualRegister->isSinglePrecision()) {
        xchgOp = XORPSRegReg;
      } else  // virtualRegister->getKind() == TR_VRF ||
              // (virtualRegister->getKind() == TR_FPR &&
              // !virtualRegister->isSinglePrecision())
      {
        xchgOp = XORPDRegReg;
      }
      ...
      self()->cg()->traceRegAssigned(currentTargetVirtual,
                                     currentAssignedRegister);
      instr = new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
          currentInstruction, xchgOp, currentAssignedRegister, targetRegister,
          self()->cg());
      self()->cg()->traceRAInstruction(instr);
      instr = new (self()->cg()->trHeapMemory())
          TR::X86RegRegInstruction(currentInstruction, xchgOp, targetRegister,
      ...
      TR::RealRegister *candidate = self()->findBestFreeGPRegister(
          currentInstruction, currentTargetVirtual, TR_QuadWordReg);
      ...
      if (candidate) {
        ...
        self()->cg()->setRegisterAssignmentFlag(TR_RegisterSpilled);
        candidate = self()->freeBestGPRegister(
            currentInstruction, currentTargetVirtual, TR_QuadWordReg, regNum,
      ...
      if (targetRegister != candidate) {
        if (virtualRegister->getKind() == TR_VRF) {
          instr = new (self()->cg()->trHeapMemory())
              TR::X86RegRegInstruction(currentInstruction, MOVDQURegReg,
                                       targetRegister, candidate, self()->cg());
        } else if (currentTargetVirtual->isSinglePrecision()) {
          instr = new (self()->cg()->trHeapMemory())
              TR::X86RegRegInstruction(currentInstruction, MOVAPSRegReg,
                                       targetRegister, candidate, self()->cg());
        } else {
          instr = new (self()->cg()->trHeapMemory())
              TR::X86RegRegInstruction(currentInstruction, MOVAPDRegReg,
                                       targetRegister, candidate, self()->cg());
        }
    ...
    if (currentAssignedRegister != NULL) {
      ...
      self()->cg()->traceRAInstruction(instr);
      currentAssignedRegister->setState(
          TR::RealRegister::Assigned, currentTargetVirtual->isPlaceholderReg());
      currentAssignedRegister->setAssignedRegister(currentTargetVirtual);
      currentTargetVirtual->setAssignedRegister(currentAssignedRegister);
    } else {
      TR::RealRegister *candidate = self()->findBestFreeGPRegister(
      ...
      if (targetRegister != candidate) {
        ...
        candidate->setState(TR::RealRegister::Assigned,
                            currentTargetVirtual->isPlaceholderReg());
        candidate->setAssignedRegister(currentTargetVirtual);
        currentTargetVirtual->setAssignedRegister(candidate);
        self()->cg()->traceRegAssigned(currentTargetVirtual, candidate);
        self()->cg()->traceRAInstruction(instr);
        self()->cg()->setRegisterAssignmentFlag(
            TR_RegisterSpilled);  // the spill (if any) has been traced
      }
...
void OMR::X86::Machine::swapGPRegisters(TR::Instruction *currentInstruction,
  ...
  TR::Instruction *instr =
      new (self()->cg()->trHeapMemory()) TR::X86RegRegInstruction(
          currentInstruction, XCHGRegReg(), realReg1, realReg2, self()->cg());
...
void OMR::X86::Machine::coerceGPRegisterAssignment(
  ...
  if ((candidate = self()->findBestFreeGPRegister(
           currentInstruction, virtualRegister, requestedRegSize)) == NULL) {
    self()->cg()->setRegisterAssignmentFlag(TR_RegisterSpilled);
    candidate = self()->freeBestGPRegister(currentInstruction, virtualRegister,
                                           requestedRegSize);
  }
...
void OMR::X86::Machine::setGPRWeightsFromAssociations() {
  ...
  for (int i = TR::RealRegister::FirstGPR;
    ...
    //
    if (self()->getRealRegister((TR::RealRegister::RegNum)i)->getState() ==
        TR::RealRegister::Locked)
      continue;
    ...
    if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i) ||
      ...
      assocReg->setAssociation(i);
      if (assocReg
              ->isPlaceholderReg()) {  // placeholder register and is only
                                       // needed at the specific dependency site
                                       // (usually a killed register on a call)
                                       // so defer this register's weight to
                                       // that of registers where the associated
                                       // register has a longer life
...
void OMR::X86::Machine::createRegisterAssociationDirective(
    TR::Instruction *cursor) {
  TR::Compilation *comp = self()->cg()->comp();
  TR::RegisterDependencyConditions *associations =
      generateRegisterDependencyConditions(
          (uint8_t)0, TR::RealRegister::LastAssignableGPR, self()->cg());
  ...
  for (int i = 0; i < TR::RealRegister::LastAssignableGPR; i++) {
    ...
    associations->addPostCondition(self()->getVirtualAssociatedWithReal(regNum),
                                   regNum, self()->cg(), 0, true);
  }

  associations->stopAddingPostConditions();

  new (self()->cg()->trHeapMemory())
      TR::Instruction(associations, ASSOCREGS, cursor, self()->cg());
...
void OMR::X86::Machine::initializeRegisterFile(
    const struct TR::X86LinkageProperties &properties) {
  int reg;
  ...
  // GPRs
  //
  _registerFile[TR::RealRegister::eax] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR,
                       properties.isPreservedRegister(TR::RealRegister::eax)
                           ? PRESERVED_WEIGHT
                           : NONPRESERVED_WEIGHT,
                       TR::RealRegister::Free, TR::RealRegister::eax,
                       TR::RealRegister::eaxMask, self()->cg());

  static char *dontUseEBXasGPR = feGetEnv("dontUseEBXasGPR");

  if (!dontUseEBXasGPR) {
    _registerFile[TR::RealRegister::ebx] = new (self()->cg()->trHeapMemory())
        TR::RealRegister(TR_GPR,
                         properties.isPreservedRegister(TR::RealRegister::ebx)
                             ? PRESERVED_WEIGHT
                             : NONPRESERVED_WEIGHT,
                         TR::RealRegister::Free, TR::RealRegister::ebx,
                         TR::RealRegister::ebxMask, self()->cg());
  } else {
    _registerFile[TR::RealRegister::ebx] = new (self()->cg()->trHeapMemory())
        TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked,
                         TR::RealRegister::ebx, TR::RealRegister::ebxMask,
                         self()->cg());
    _registerFile[TR::RealRegister::ebx]->setAssignedRegister(
        _registerFile[TR::RealRegister::ebx]);
  }

  _registerFile[TR::RealRegister::ecx] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR,
                       properties.isPreservedRegister(TR::RealRegister::ecx)
                           ? PRESERVED_WEIGHT
                           : NONPRESERVED_WEIGHT,
                       TR::RealRegister::Free, TR::RealRegister::ecx,
                       TR::RealRegister::ecxMask, self()->cg());

  _registerFile[TR::RealRegister::edx] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR,
                       properties.isPreservedRegister(TR::RealRegister::edx)
                           ? PRESERVED_WEIGHT
                           : NONPRESERVED_WEIGHT,
                       TR::RealRegister::Free, TR::RealRegister::edx,
                       TR::RealRegister::edxMask, self()->cg());

  _registerFile[TR::RealRegister::edi] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR,
                       properties.isPreservedRegister(TR::RealRegister::edi)
                           ? PRESERVED_WEIGHT
                           : NONPRESERVED_WEIGHT,
                       TR::RealRegister::Free, TR::RealRegister::edi,
                       TR::RealRegister::ediMask, self()->cg());

  _registerFile[TR::RealRegister::esi] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR,
                       properties.isPreservedRegister(TR::RealRegister::esi)
                           ? PRESERVED_WEIGHT
                           : NONPRESERVED_WEIGHT,
                       TR::RealRegister::Free, TR::RealRegister::esi,
  ...
  _registerFile[TR::RealRegister::ebp] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked,
                       TR::RealRegister::ebp, TR::RealRegister::ebpMask,
                       self()->cg());
  _registerFile[TR::RealRegister::ebp]->setAssignedRegister(
      _registerFile[TR::RealRegister::ebp]);

  _registerFile[TR::RealRegister::esp] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked,
                       TR::RealRegister::esp, TR::RealRegister::espMask,
                       self()->cg());
  _registerFile[TR::RealRegister::esp]->setAssignedRegister(
      _registerFile[TR::RealRegister::esp]);

  _registerFile[TR::RealRegister::vfp] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, UNUSED_WEIGHT, TR::RealRegister::Locked,
                       TR::RealRegister::vfp, TR::RealRegister::noRegMask,
                       self()->cg());
  _registerFile[TR::RealRegister::vfp]->setAssignedRegister(
      _registerFile[TR::RealRegister::NoReg]);

#ifdef TR_TARGET_64BIT
  for (reg = TR::RealRegister::r8; reg <= TR::RealRegister::LastAssignableGPR;
       reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR,
        properties.isPreservedRegister((TR::RealRegister::RegNum)reg)
            ? PRESERVED_WEIGHT
            : NONPRESERVED_WEIGHT,
  ...
  //
  for (reg = TR::RealRegister::FirstFPR; reg <= TR::RealRegister::LastFPR;
       reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_X87,
        properties.isPreservedRegister((TR::RealRegister::RegNum)reg)
  ...
  for (reg = TR::RealRegister::FirstMMXR; reg <= TR::RealRegister::LastMMXR;
       reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_GPR,
        properties.isPreservedRegister((TR::RealRegister::RegNum)reg)
  ...
  for (reg = TR::RealRegister::FirstXMMR; reg <= TR::RealRegister::LastXMMR;
       reg++) {
    _registerFile[reg] = new (self()->cg()->trHeapMemory()) TR::RealRegister(
        TR_FPR,
        properties.isPreservedRegister((TR::RealRegister::RegNum)reg)
            ...
            : NONPRESERVED_WEIGHT,
        TR::RealRegister::Free, (TR::RealRegister::RegNum)reg,
        TR::RealRegister::xmmrMask((TR::RealRegister::RegNum)reg),
        self()->cg());
  }
}

uint32_t *OMR::X86::Machine::getGlobalRegisterTable(
    const struct TR::X86LinkageProperties &property) {
...
TR::RealRegister **OMR::X86::Machine::cloneRegisterFile(
  ...
  // the vfp entry is a real register and it must always point to the same
  // _frameRegister pointer so the memRef assignRegisters check if
  // (_baseRegister == cg()->machine()->getRealRegister(TR::RealRegister::vfp)
  // works correctly
  registerFileClone[TR::RealRegister::vfp] = self()->cg()->getFrameRegister();

  // if there are no live FPRs then initialize the clone with the incoming
  // registerFile values for the XMMRegs so these pointers are not garbage
  // values
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastXMMR;
      ...
      if (realReg->getState() == TR::RealRegister::Assigned ||
        ...
        if (realReg->getState() == TR::RealRegister::Free) {
          virtReg = self()->cg()->allocateRegister(
              i <= TR::RealRegister::LastAssignableGPR ? TR_GPR : TR_FPR);
...
// if calledForDeps == false then this is being called to generate conditions
// for use in regAssocs
TR::RegisterDependencyConditions *
OMR::X86::Machine::createCondForLiveAndSpilledGPRs(
    bool calledForDeps, TR::list<TR::Register *> *spilledRegisterList) {
  int32_t i, c = 0;
...
TR::RealRegister **OMR::X86::Machine::captureRegisterFile() {
  ...
  for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    registerFileClone[i] =
        (TR::RealRegister *)self()->cg()->trMemory()->allocateMemory(
            sizeof(TR::RealRegister), heapAlloc);
  ...
  registerFileClone[TR::RealRegister::vfp] =
      (TR::RealRegister *)self()->cg()->trMemory()->allocateMemory(
          sizeof(TR::RealRegister), heapAlloc);
  *registerFileClone[TR::RealRegister::vfp] =
      *_registerFile[TR::RealRegister::vfp];

  return registerFileClone;
}

void OMR::X86::Machine::installRegisterFile(
    TR::RealRegister **registerFileCopy) {
  int32_t endReg = TR::RealRegister::LastXMMR;
  ...
  for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (assignedVirtReg != NULL) {
      TR_ASSERT(
          _registerFile[i]->getState() == TR::RealRegister::Assigned ||
  ...
  for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg;
       i = ((i == TR::RealRegister::LastAssignableGPR)
                ? TR::RealRegister::FirstXMMR
                : i + 1)) {
    // Some real register fields are sticky and their contents must be
    // preserved.
    //
  ...
  *_registerFile[TR::RealRegister::vfp] =
      *registerFileCopy[TR::RealRegister::vfp];
}
...
TR::Register **OMR::X86::Machine::captureRegisterAssociations() {
  ...
  TR::Register **registerAssociationsClone =
      (TR::Register **)self()->cg()->trMemory()->allocateMemory(arraySize,
                                                                heapAlloc);
  ...
  for (int32_t i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (_registerAssociations[i] != NULL) {
      registerAssociationsClone[i] =
          (TR::Register *)self()->cg()->trMemory()->allocateMemory(
              sizeof(TR::Register), heapAlloc);
  ...
  if (_registerAssociations[TR::RealRegister::vfp] != NULL) {
    registerAssociationsClone[TR::RealRegister::vfp] =
        (TR::Register *)self()->cg()->trMemory()->allocateMemory(
            sizeof(TR::Register), heapAlloc);
    *registerAssociationsClone[TR::RealRegister::vfp] =
        *_registerAssociations[TR::RealRegister::vfp];
  } else {
...
TR::list<TR::Register *> *OMR::X86::Machine::captureSpilledRegistersList() {
  ...
  TR_ASSERT(self()->cg()->getSpilledRegisterList(),
            "expecting an existing spilledRegistersList before capture");

  TR::list<TR::Register *> *srl =
      new (self()->cg()->trHeapMemory()) TR::list<TR::Register *>(
          getTypedAllocator<TR::Register *>(self()->cg()->comp()->allocator()));
...
TR::RegisterDependencyConditions *
TR_RegisterAssignerState::createDependenciesFromRegisterState(
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
       i = ((i == TR::RealRegister::LastAssignableGPR)
                ? TR::RealRegister::FirstXMMR
                : i + 1)) {
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned) numDeps++;
  }
  ...
  if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
    traceMsg(comp,
             "createDependenciesFromRegisterState : %d live registers: %d "
             "assigned, %d spilled\n",
             numDeps, numDeps - _spilledRegistersList->size(),
             _spilledRegistersList->size());
  }

  if (numDeps == 0) return NULL;

  TR::RegisterDependencyConditions *deps =
      generateRegisterDependencyConditions(0, numDeps, _machine->cg());
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (realReg->getState() == TR::RealRegister::Assigned) {
      ...
      // Because this happens during register assignment the future use needs to
      // be adjusted to account for this new use.  The total use count will be
      // adjusted when this dependency is hung off an instruction.
      //
      virtReg->incFutureUseCount();
      ...
      bool found = (std::find(_spilledRegistersList->begin(),
                              _spilledRegistersList->end(),
                              virtReg) != _spilledRegistersList->end());
  ...
  //
  for (auto iter = _spilledRegistersList->begin();
       iter != _spilledRegistersList->end(); ++iter) {
    ...
    if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(
          comp,
          "   create SPILLED dependency: virtual %p -> backing storage %p\n",
...
void OMR::X86::Machine::adjustRegisterUseCountsUp(
    TR::list<OMR::RegisterUsage *> *rul, bool adjustFuture) {
  if (!rul) return;
  TR::Compilation *comp = self()->cg()->comp();
  for (auto iter = rul->begin(); iter != rul->end(); ++iter) {
    if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(comp,
               "Adjusting UP register use counts of reg %p "
               "(fuc=%d:tuc=%d:adjustFuture=%d) by %d -> ",
               (*iter)->virtReg, (*iter)->virtReg->getFutureUseCount(),
               (*iter)->virtReg->getTotalUseCount(), adjustFuture,
               (*iter)->useCount);
    }

    (*iter)->virtReg->incTotalUseCount((*iter)->useCount);

    if (adjustFuture) (*iter)->virtReg->incFutureUseCount((*iter)->useCount);
...
void OMR::X86::Machine::adjustRegisterUseCountsDown(
    TR::list<OMR::RegisterUsage *> *rul, bool adjustFuture) {
  if (!rul) return;
  TR::Compilation *comp = self()->cg()->comp();
  ...
  for (auto iter = rul->begin(); iter != rul->end(); ++iter) {
    if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
      traceMsg(comp,
               "Adjusting DOWN register use counts of reg %p "
               "(fuc=%d:tuc=%d:adjustFuture=%d) by %d -> ",
               (*iter)->virtReg, (*iter)->virtReg->getFutureUseCount(),
               (*iter)->virtReg->getTotalUseCount(), adjustFuture,
               (*iter)->useCount);
    ...
    if (adjustFuture) (*iter)->virtReg->decFutureUseCount((*iter)->useCount);
  ...
  for (i = TR::RealRegister::FirstGPR; i <= endReg;
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned) {
      ...
      if (location) {
        ...
        self()->cg()->freeSpill(location, size,
                                virtReg->isSpilledToSecondHalf() ? 4 : 0);
...
// Retrieve a memory reference handle to a dummy local variable of the specified
// type. If the dummy has not been allocated, allocate it and return the handle.
//
// These reusable temporaries are intended for use whenever a temporary store to
// a memory location is required.
...
TR::RealRegister *OMR::X86::Machine::fpMapToStackRelativeRegister(
    TR::Register *vreg) {
  TR_X86FPStackRegister *fpReg =
      toX86FPStackRegister(vreg->getAssignedRealRegister());
  return _registerFile[self()->getFPTopOfStack() -
                       fpReg->getFPStackRegisterNumber() +
...
void OMR::X86::Machine::initializeFPStackRegisterFile() {
  _fpStack[TR_X86FPStackRegister::fp0] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp0, TR::RealRegister::NoReg,
                            self()->cg());
  _fpStack[TR_X86FPStackRegister::fp1] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp1, TR::RealRegister::NoReg,
                            self()->cg());
  _fpStack[TR_X86FPStackRegister::fp2] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp2, TR::RealRegister::NoReg,
                            self()->cg());
  _fpStack[TR_X86FPStackRegister::fp3] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp3, TR::RealRegister::NoReg,
                            self()->cg());
  _fpStack[TR_X86FPStackRegister::fp4] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp4, TR::RealRegister::NoReg,
                            self()->cg());
  _fpStack[TR_X86FPStackRegister::fp5] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp5, TR::RealRegister::NoReg,
                            self()->cg());
  _fpStack[TR_X86FPStackRegister::fp6] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp6, TR::RealRegister::NoReg,
                            self()->cg());
  _fpStack[TR_X86FPStackRegister::fp7] = new (self()->cg()->trHeapMemory())
      TR_X86FPStackRegister(TR_X86FPStackRegister::Free,
                            TR_X86FPStackRegister::fp7, TR::RealRegister::NoReg,
                            self()->cg());
}
...
bool OMR::X86::Machine::isFPRTopOfStack(TR::Register *virtReg) {
  TR_X86FPStackRegister *fpReg =
      toX86FPStackRegister(virtReg->getAssignedRegister());
...
//
void OMR::X86::Machine::fpStackCoerce(TR::Register *virtReg,
...
void OMR::X86::Machine::popEntireStack() {
  ...
  for (int32_t i = 0; i < topOfStack + 1; i++) self()->fpStackPop();
}
...
//
// If 'generateCode' is true then a corresponding FXCH instruction will be
// generated in the instruction stream.
//
TR::Instruction *OMR::X86::Machine::fpStackFXCH(
    TR::Instruction *prevInstruction, TR::Register *vreg, bool generateCode) {
  TR_X86FPStackRegister *fpReg =
      toX86FPStackRegister(vreg->getAssignedRegister());
  ...
  if (generateCode) {
    ...
    cursor = new (self()->cg()->trHeapMemory())
        TR::X86FPRegInstruction(cursor, FXCHReg, realFPReg, self()->cg());
  }

  _fpStack[_fpTopOfStack] = _fpStack[vregNum];
  _fpStack[vregNum] = pTop;

  _fpStack[_fpTopOfStack]->setFPStackRegisterNumber(
      (TR_X86FPStackRegister::TR_X86FPStackRegisters)_fpTopOfStack);
  _fpStack[vregNum]->setFPStackRegisterNumber(
      (TR_X86FPStackRegister::TR_X86FPStackRegisters)vregNum);

  return cursor;
}

// Exchange a real FP stack relative register with the top register on the FP
// stack and generate code.  It is assumed that both registers of interest are
// on the FP stack.
//
TR::Instruction *OMR::X86::Machine::fpStackFXCH(
    TR::Instruction *prevInstruction, int32_t stackReg) {
  TR_X86FPStackRegister *pTop = _fpStack[_fpTopOfStack];
...
// Coerce registers X and Y into FP stack positions ST0 and ST1 in as few stack
// exchanges as possible.
//
// If 'strict' is true, X and Y will be exchanged into positions ST0 and ST1,
// respectively.
//
// If 'strict' is false, either X or Y will occupy ST0 and the other register
// will occupy ST1. The orientation will be chosen based on whichever results in
// the fewest possible register FXCHs to achieve.
//
// It is assumed that both X and Y are on the stack already.
//
void OMR::X86::Machine::fpCoerceRegistersToTopOfStack(
    TR::Instruction *prevInstruction, TR::Register *X, TR::Register *Y,
    bool strict) {
  ...
  switch (n) {
    ...
    default:
      diagnostic(
          "fpCoerceRegistersToTopOfStack() ==> Invalid stack configuration!");
      break;
  }
}
...
//
TR::Instruction *OMR::X86::Machine::freeBestFPRegister(
    TR::Instruction *prevInstruction) {
  TR::Register *candidates[TR_X86FPStackRegister::NumRegisters];
  int numCandidates = 0;

  TR_ASSERT(
      (_fpTopOfStack + 1 == TR_X86FPStackRegister::fpStackFull),
  ...
  //
  for (int i = TR_X86FPStackRegister::fpFirstStackReg;
       i < TR_X86FPStackRegister::NumRegisters; i++) {
  ...
  // Of the candidates identified, choose the one that is used in the most
  // distant future. Start with the first instruction following the current
  // instruction.
  //
  //                                             Previous   ->    Current ->
  //                                             Next
  ...
  while (numCandidates > 1 && cursor != NULL &&
         cursor->getOpCodeValue() != LABEL && cursor->getOpCodeValue() != RET &&
         cursor->getOpCodeValue() != RETImm2 &&
         cursor->getOpCodeValue() != ReturnMarker &&
         !cursor->getOpCode().isBranchOp()) {
...
TR::Instruction *OMR::X86::Machine::fpSpillFPR(TR::Instruction *prevInstruction,
  ...
  TR_ASSERT((fpReg != NULL),
            "OMR::X86::Machine::fpSpillFPR ==> Attempting to spill a "
  ...
  if (fpReg != NULL) {
    ...
    TR_BackingStore *location =
        self()->cg()->allocateSpill(isFloat ? 4 : 8, false, &offset);
    TR::MemoryReference *tempMR = generateX86MemoryReference(
        location->getSymbolReference(), offset, self()->cg());
    vreg->setBackingStorage(location);
    TR_ASSERT(offset == 0 || offset == 4, "assertion failure");
    vreg->setIsSpilledToSecondHalf(offset > 0);
    cursor = new (self()->cg()->trHeapMemory()) TR::X86FPMemRegInstruction(
        cursor, isFloat ? FSTPMemReg : DSTPMemReg, tempMR,
...
TR::Instruction *OMR::X86::Machine::reverseFPRSpillState(
  ...
  bool isFloat =
      spilledRegister->isSinglePrecision();  // && debug("floatSpills");

  TR::RealRegister *realFPReg =
      self()->fpMapToStackRelativeRegister(spilledRegister);
  cursor = new (self()->cg()->trHeapMemory()) TR::X86FPRegMemInstruction(
      cursor, isFloat ? FLDRegMem : DLDRegMem, realFPReg, tempMR, self()->cg());

  self()->cg()->freeSpill(location, isFloat ? 4 : 8,
                          spilledRegister->isSpilledToSecondHalf() ? 4 : 0);
...
//
TR::Instruction *OMR::X86::Machine::fpSpillStack(
    TR::Instruction *prevInstruction) {
  TR::Instruction *cursor = prevInstruction;
...
#if defined(DEBUG)
static void printOneRegisterStatus(TR_FrontEnd *fe, TR::FILE *pOutFile,
                                   TR_Debug *debug, TR::RealRegister *reg) {
  trfprintf(pOutFile, "                      ");
  debug->printFullRegInfo(pOutFile, reg);
}

void OMR::X86::Machine::printGPRegisterStatus(TR_FrontEnd *fe,
...
//
void OMR::X86::Machine::printFPRegisterStatus(TR_FrontEnd *fe,
                                              TR::FILE *pOutFile) {
  char buf[32];
  char *cursor;
  int32_t i, j;
  ...
  trfprintf(pOutFile,
            "\n  FP Reg Status:          Register         State        "
            "Assigned      Total Future\n");
  for (i = _fpTopOfStack, j = 0; i != TR_X86FPStackRegister::fpStackEmpty;
       i--, j++) {
    memset(buf, ' ', 25);
    cursor = buf + 17;
    sprintf(cursor, "st%d: ", j);
    trfprintf(pOutFile, buf);
    self()->getDebug()->printFullRegInfo(pOutFile,
                                         _fpStack[i]->getAssignedRegister());
  }
