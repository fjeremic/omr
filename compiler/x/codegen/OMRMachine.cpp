...
OMR::X86::Machine::Machine(uint8_t numIntRegs, uint8_t numFPRegs,
    ...
    : OMR::Machine(cg), _registerAssociations(registerAssociations),
      _numGlobalGPRs(numGlobalGPRs), _numGlobal8BitGPRs(numGlobal8BitGPRs),
      _numGlobalFPRs(numGlobalFPRs), _xmmGlobalRegisters(xmmGlobalRegisters),
      _globalRegisterNumberToRealRegisterMap(
          globalRegisterNumberToRealRegisterMap),
      _spilledRegistersList(NULL), _numGPRs(numIntRegs) {
...
TR::RealRegister *OMR::X86::Machine::freeBestGPRegister(
  ...
  TR_ASSERT(virtReg &&
                (virtReg->getKind() == TR_GPR || virtReg->getKind() == TR_FPR ||
                 virtReg->getKind() == TR_VRF),
            "OMR::X86::Machine::freeBestGPRegister() ==> expecting to free "
            "GPRs or XMMRs only!");
...
TR::RealRegister *OMR::X86::Machine::reverseGPRSpillState(
  ...
  if (self()->cg()->enableRematerialisation()) {
    ...
    if (spilledRegister->getRematerializationInfo() &&
      ...
      if (debug("dumpRemat")) {
        diagnostic("---> Avoiding storing %s rematerializable register %s to "
                   "memory in %s\n",
...
void OMR::X86::Machine::swapGPRegisters(TR::Instruction *currentInstruction,
  ...
  TR::Instruction *instr = new (self()->cg()->trHeapMemory())
      TR::X86RegRegInstruction(currentInstruction, XCHGRegReg(), realReg1,
                               realReg2, self()->cg());
...
void OMR::X86::Machine::setGPRWeightsFromAssociations() {
  ...
  for (int i = TR::RealRegister::FirstGPR;
    ...
    if ((linkageProperties.isPreservedRegister((TR::RealRegister::RegNum)i) ||
      ...
      assocReg->setAssociation(i);
      if (assocReg
              ->isPlaceholderReg()) { // placeholder register and is only needed
                                      // at the specific dependency site
                                      // (usually a killed register on a call)
                                      // so defer this register's weight to that
                                      // of registers where the associated
...
TR::RealRegister **
OMR::X86::Machine::cloneRegisterFile(TR::RealRegister **registerFile,
...
TR::RealRegister *
OMR::X86::Machine::fpMapToStackRelativeRegister(TR::Register *vreg) {
...
//
TR::Instruction *
OMR::X86::Machine::fpStackFXCH(TR::Instruction *prevInstruction,
                               TR::Register *vreg, bool generateCode) {
...
//
TR::Instruction *
OMR::X86::Machine::freeBestFPRegister(TR::Instruction *prevInstruction) {
...
TR::Instruction *OMR::X86::Machine::fpSpillFPR(TR::Instruction *prevInstruction,
  ...
  TR_ASSERT((fpReg != NULL), "OMR::X86::Machine::fpSpillFPR ==> Attempting to "
                             "spill a non-existant register!");
...
//
TR::Instruction *
OMR::X86::Machine::reverseFPRSpillState(TR::Instruction *prevInstruction,
...
//
TR::Instruction *
OMR::X86::Machine::fpSpillStack(TR::Instruction *prevInstruction) {
  TR::Instruction *cursor = prevInstruction;
