...
generateRematerializationInfo(TR::Node* node,
...
//
void
setDiscardableIfPossible(TR_RematerializableTypes type,
    ...
    if (info->isIndirect()) {
      ...
      if (debug("dumpRemat")) {
        diagnostic(
          ...
          info->toString(comp),
          candidate->getRegisterName(comp),
          type,
                              ...
                              TR::CodeGenerator* cg)
{
  ...
  if (cg->supportsLocalMemoryRematerialization() && sym->isAutoOrParm()) {
    ...
    TR_ASSERT(
      ...
      baseNode,
      baseNodeReg ? baseNodeReg->getRegisterName(comp) : "null",
