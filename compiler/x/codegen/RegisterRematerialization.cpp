...
static TR_RematerializationInfo *generateRematerializationInfo(
    TR::Node *node, TR::MemoryReference *mr, TR_RematerializableTypes type,
    TR::Instruction *instr, TR::CodeGenerator *cg);

static TR_RematerializationInfo *generateRematerializationInfo(
    TR::Node *node, intptrj_t constant, TR_RematerializableTypes type,
    TR::Instruction *instr, TR::CodeGenerator *cg);

static TR_RematerializationInfo *generateRematerializationInfo(
    TR::Node *node, TR::SymbolReference *symRef, TR_RematerializableTypes type,
    TR::Instruction *instr, TR::CodeGenerator *cg);
...
void setDiscardableIfPossible(TR_RematerializableTypes type,
                              TR::Register *candidate, TR::Node *node,
                              TR::Instruction *instr, TR::MemoryReference *mr,
                              TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR_RematerializationInfo *info =
      generateRematerializationInfo(node, mr, type, instr, cg);
  ...
  if (info) {
    ...
    if (info->isIndirect()) {
      ...
      if (debug("dumpRemat")) {
        diagnostic(
            "---> Creating %s discardable register %s (type %d, base %s) "
            "for node [%p] at instruction %p in %s\n",
            info->toString(comp), candidate->getRegisterName(comp), type,
            mr->getBaseRegister()->getRegisterName(comp), node, instr,
            comp->signature());
      }
    } else {
      if (debug("dumpRemat")) {
        diagnostic(
            "---> Creating %s discardable register %s (type %d) "
            "for node [%p] at instruction %p in %s\n",
            info->toString(comp), candidate->getRegisterName(comp), type, node,
...
void setDiscardableIfPossible(TR_RematerializableTypes type,
                              TR::Register *candidate, TR::Node *node,
                              TR::Instruction *instr, intptrj_t constant,
                              TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR_RematerializationInfo *info =
      generateRematerializationInfo(node, constant, type, instr, cg);
...
void setDiscardableIfPossible(TR_RematerializableTypes type,
  ...
  TR::Compilation *comp = cg->comp();
  TR_RematerializationInfo *info =
      generateRematerializationInfo(node, symRef, type, instr, cg);
...
static TR_RematerializationInfo *generateRematerializationInfo(
  ...
  if (symRef->isUnresolved() || sym->isVolatile()) return NULL;
  ...
  if (cg->supportsLocalMemoryRematerialization() && sym->isAutoOrParm()) {
    ...
    // of local objects.
    if (node->getOpCode().isIndirect()) return NULL;
    // A dependent discardable register can only depend on a base register.
    if (mr->getIndexRegister()) return NULL;
    info = new (cg->trHeapMemory())
        TR_RematerializationInfo(instr, type, symRef, NULL);
    ...
    TR_ASSERT(!baseReg || baseNode,
              "base register for memory rematerialization should be associated "
              "with a base node\n");

    TR::Register *baseNodeReg = baseNode->getRegister();

    TR_ASSERT(
        !baseReg || sym->isLocalObject() || baseNodeReg == baseReg,
        "node register of non-local object base node [%p] is %s; %s expected\n",
        baseNode, baseNodeReg ? baseNodeReg->getRegisterName(comp) : "null",
        baseReg->getRegisterName(comp));
#endif
    ...
    // The base register must still be live if we are to rematerialize a
    // candidate with it.
    if (baseReg && baseNode->getReferenceCount() <= 1) return NULL;

    info = new (cg->trHeapMemory())
        TR_RematerializationInfo(instr, type, symRef, baseReg);
  }

  if (info && isStore) info->setStore();
...
static TR_RematerializationInfo *generateRematerializationInfo(
    TR::Node *node, intptrj_t constant, TR_RematerializableTypes type,
    TR::Instruction *instr, TR::CodeGenerator *cg) {
  if (node->getReferenceCount() <= REMATERIALIZATION_THRESHOLD) return NULL;

  if (!cg->supportsConstantRematerialization()) return NULL;
  ...
  return new (cg->trHeapMemory())
      TR_RematerializationInfo(instr, type, constant);
}
...
static TR_RematerializationInfo *generateRematerializationInfo(
  ...
  if (!cg->supportsAddressRematerialization()) return NULL;

  if (sym->isLocalObject() || (sym->isStatic() && !symRef->isUnresolved()))
    return new (cg->trHeapMemory())
        TR_RematerializationInfo(instr, type, symRef);
