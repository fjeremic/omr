...
static TR_RematerializationInfo* generateRematerializationInfo(
  TR::Node* node, intptrj_t constant, TR_RematerializableTypes type, TR::Instruction* instr, TR::CodeGenerator* cg);
    ...
    if (info->isIndirect()) {
      ...
      if (debug("dumpRemat")) {
        diagnostic("---> Creating %s discardable register %s (type %d, base %s) "
          ...
          info->toString(comp), candidate->getRegisterName(comp), type, mr->getBaseRegister()->getRegisterName(comp),
...
//
void setDiscardableIfPossible(TR_RematerializableTypes type, TR::Register* candidate, TR::Node* node,
  TR::Instruction* instr, intptrj_t constant, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
...
static TR_RematerializationInfo* generateRematerializationInfo(
  TR::Node* node, TR::MemoryReference* mr, TR_RematerializableTypes type, TR::Instruction* instr, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
