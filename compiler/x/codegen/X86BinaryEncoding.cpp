    ...
    bool onlyAskingAboutFences)
{
    ...
    if (barrier) {
        if ((!cg->getX86ProcessorInfo().supportsLFence() || !cg->getX86ProcessorInfo().supportsMFence()) || disableExplicitFences) {
...
uint8_t* TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding()
{
    ...
    if (label->getCodeLocation() == NULL) {
        ...
        //
        offset = cg()->getBinaryBufferStart() + label->getEstimatedCodeLocation() - (patchCursor + IA32LengthOfShortBranch + cg()->getAccumulatedInstructionLengthError());
    ...
    uint8_t next = requiresAtomicPatching ? cg()->sizeOfInstructionToBePatched(this) : cg()->sizeOfInstructionToBePatchedHCRGuard(this);
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (getReloKind() != -1) // TODO: need to change Body info one to use this
        {
            ...
            case TR_MethodPointer:
                if (getNode() && getNode()->getInlinedSiteIndex() == -1 && (void*)(uintptr_t)getSourceImmediate() == cg()->comp()->getCurrentMethod()->resolvedMethodAddress())
...
void TR::AMD64RegImm64Instruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (cg()->needRelocationsForHelpers()) {
        if (getNode()->getOpCode().hasSymbolReference() && methodSymRef && (methodSymRef->getReferenceNumber() == TR_referenceArrayCopy || methodSymRef->getReferenceNumber() == TR_prepareForOSR)) { // the reference number is set in j9x86evaluator.cpp/VMarrayStoreCheckArrayCopyEvaluator
            cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
    ...
    if (comp->fej9()->needClassAndMethodPointerRelocations()) {
        if (((getNode()->getOpCodeValue() == TR::aconst) && getNode()->isMethodPointerConstant() && needsAOTRelocation()) || staticHCRPIC) {
            ...
            case TR_MethodPointer:
                if (getNode() && getNode()->getInlinedSiteIndex() == -1 && (void*)getSourceImmediate() == cg()->comp()->getCurrentMethod()->resolvedMethodAddress())
                    setReloKind(TR_RamMethod);
                // intentional fall-through
            case TR_RamMethod:
                symbolKind = TR::SymbolType::typeMethod;
