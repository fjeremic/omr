            ...
            if (isTrampolineRequired) {
              ...
              targetAddress =
                  TR::CodeCacheManager::instance()->findHelperTrampoline(
                     ...
                     getNode()->isPreparedForDirectJNI()) {
            if (isTrampolineRequired) {
              TR_ASSERT(0, "We seem to never generate CALLIMM instructions for "
                           "JNI on 64Bit!!!");
            }
          } else {
        ...
        if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
          ...
          cg()->addExternalRelocation(
              new (cg()->trHeapMemory()) TR::ExternalRelocation(
                  ...
                  (uint8_t *)getNode()->getInlinedSiteIndex(), TR_ClassAddress,
                  cg()),
              __FILE__, __LINE__, getNode());
        }
      } break;
    ...
    case TR_NativeMethodAbsolute: {
      if (cg()->comp()->getOption(TR_EmitRelocatableELFFile)) {
        ...
        cg()->addStaticRelocation(
            TR::StaticRelocation(cursor, target->externalName(cg()->trMemory()),
