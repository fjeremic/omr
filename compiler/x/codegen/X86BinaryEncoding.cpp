...
int32_t
memoryBarrierRequired(TR_X86OpCode& op,
                      ...
                      bool onlyAskingAboutFences)
{
  ...
  if (!onlyAskingAboutFences && mr->requiresLockPrefix()) {
    TR_ASSERT(op.supportsLockPrefix(),
              "cannot emit a lock prefix with this type of instruction");
    return LockPrefix;
  }
  ...
  // Unresolved references are assumed to be volatile until they can be proven
  // otherwise. The memory barrier will be removed by PicBuilder when the
  // reference is resolved and proven to be non-volatile.
  //
  ...
  if (barrier) {
    if ((!cg->getX86ProcessorInfo().supportsLFence() ||
         !cg->getX86ProcessorInfo().supportsMFence()) ||
...
int32_t
estimateMemoryBarrierBinaryLength(int32_t barrier, TR::CodeGenerator* cg)
{
  int32_t length = 0;
...
uint8_t
getMemoryBarrierBinaryLengthLowerBound(int32_t barrier, TR::CodeGenerator* cg)
{
...
OMR::X86::Instruction::generateBinaryEncoding()
{
  ...
  // it is needed to avoid recursive calls when generateOperand() requests to
  // regenerate the binary code by returning NULL;
  for (;;) {
    ...
    // cursor is normal not NULL, and hence we can finish binary encoding and
    // exit the loop cursor is NULL when generateOperand() requests to
    // regenerate the binary code, which may happen during encoding of memref
    // with unresolved symbols on 64-bit
    if (cursor) {
...
int32_t
OMR::X86::Instruction::estimateBinaryLength(int32_t currentEstimate)
{
  self()->setEstimatedBinaryLength(
    self()->getOpCode().length(self()->rexBits()) +
...
uint8_t*
TR::X86PaddingInstruction::generateBinaryEncoding()
{
...
int32_t
TR::X86PaddingInstruction::estimateBinaryLength(int32_t currentEstimate)
{
...
TR::Snippet*
TR::X86PaddingSnippetInstruction::getSnippetForGC()
{
...
TR::X86BoundaryAvoidanceInstruction::enlarge(int32_t requestedEnlargementSize,
                                             int32_t maxEnlargementSize,
                                             bool allowPartialEnlargement)
{
  static char* disableMinPadding =
    feGetEnv("TR_DisableBoundaryAvoidanceMinPadding");
  if (disableMinPadding)
    return OMR::X86::EnlargementResult(0, 0);

  if ((maxEnlargementSize < requestedEnlargementSize &&
       !allowPartialEnlargement) ||
      requestedEnlargementSize < 1)
    return OMR::X86::EnlargementResult(0, 0);

  int32_t enlargementSize =
    std::min(requestedEnlargementSize, maxEnlargementSize);
...
int32_t
TR::X86BoundaryAvoidanceInstruction::estimateBinaryLength(
  int32_t currentEstimate)
{
  ...
  //
  for (const TR_AtomicRegion* cur = _atomicRegions; cur->getLength() > 0;
       cur++) {
...
uint8_t*
TR::X86BoundaryAvoidanceInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();

  int32_t modulo =
    ((uintptrj_t)(instructionStart + _minPaddingLength)) % _boundarySpacing;
  int32_t padLength = 0;

  const TR_AtomicRegion* cur = _atomicRegions;
  while (cur->getLength() > 0) {
    TR_ASSERT(cur->getLength() <= _boundarySpacing,
              "Atomic regions of %d bytes can't fit between %d-byte boundaries",
              cur->getLength(),
              _boundarySpacing);
  ...
  cg()->addAccumulatedInstructionLengthError(getEstimatedBinaryLength() -
                                             getBinaryLength());
  uint8_t* afterPadding = cg()->generatePadding(
    instructionStart, _minPaddingLength + padLength, this);
  ...
  int32_t unaccommodatedRegionStart)
{
  ...
  TR_ASSERT(result < getBoundarySpacing(),
            "Patchable code has no viable padding that will let it be updated "
            "atomically");
  return result;
}
...
OMR::X86::EnlargementResult
TR::X86AlignmentInstruction::enlarge(int32_t requestedEnlargementSize,
                                     int32_t maxEnlargementSize,
                                     bool allowPartialEnlargement)
{
...
int32_t
TR::X86AlignmentInstruction::estimateBinaryLength(int32_t currentEstimate)
{
...
uint8_t*
TR::X86AlignmentInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  uint8_t* cursor = instructionStart;

  intptrj_t paddingLength =
    cg()->alignment(cursor + _minPaddingLength, _boundary, _margin);
  cursor =
    cg()->generatePadding(cursor, _minPaddingLength + paddingLength, this);
...
uint8_t*
TR::X86LabelInstruction::generateBinaryEncoding()
{
    ...
    if (label == NULL) {
      ...
      *(uint32_t*)cursor = 0;
      ...
      if (label->getCodeLocation() != NULL) {
        ...
        //
        distance =
          label->getCodeLocation() - (cursor + IA32LengthOfShortBranch);
      } else {
        // Conservative estimate of distance if target address is not known
        // exactly. (e.g., a forward relative branch)
        //
        distance =
      ...
      TR_ASSERT(getOpCodeValue() != XBEGIN4 || !_permitShortening,
                "XBEGIN4 cannot be shortened and can only be used with a label "
                "instruction that cannot shorten - use generateLongLabel!\n");

      if (distance >= -128 && distance <= 127 && getOpCode().isBranchOp() &&
          _permitShortening) {
        ...
        if (label->getCodeLocation() != NULL) {
          *(int8_t*)cursor = (int8_t)distance;
          ...
          cg()->addRelocation(new (cg()->trHeapMemory())
                                TR::LabelRelative8BitRelocation(cursor, label));
        ...
        if (getOpCode().isShortBranchOp()) {
          ...
          TR_ASSERT(
            0,
            "short form branch displacement too large: instr=%p, distance=%d\n",
            this,
            distance);
          comp->failCompilation<TR::CompilationException>(
            "short form branch displacement too large");
        }
        ...
        if (label->getCodeLocation() != NULL) {
          *(int32_t*)cursor = distance + IA32LengthOfShortBranch -
                              getOpCode().length(self()->rexBits()) - 4;
        } else {
          cg()->addRelocation(
            new (cg()->trHeapMemory())
...
uint8_t
TR::X86LabelInstruction::getBinaryLengthLowerBound()
{
  ...
  return getOpCode().length(self()->rexBits()) +
         4; // assume absolute code reference
}

OMR::X86::EnlargementResult
TR::X86LabelInstruction::enlarge(int32_t requestedEnlargementSize,
                                 int32_t maxEnlargementSize,
                                 bool allowPartialEnlargement)
{
  ...
  // in the X86 codegen a branch instruction has a size eg JMP4 and that size is
  // the maximum requested size - we never make a JMP1 a JMP4 for example - we
  // trust people choosing a lower maximum size know what they are doing the
  // binary encoding will, usually try to use the smallest instruction it can
  // and so if a JMP4 can be encoded as a JMP1 it will be
  // this function prevents this instruction shrinking by calling
  // prohibitShortening() this increases the size lowerbound of the instruction,
  // thus "enlarging" the instruction and, hopefully, making it a suitable
  // candidate for patching

  if (!getOpCode().isBranchOp())
    return OMR::X86::EnlargementResult(0, 0);

  if (!getOpCode().hasIntImmediate() || !_permitShortening ||
      getOpCodeValue() == VirtualGuardNOP)
    return OMR::X86::EnlargementResult(0, 0);
...
int32_t
TR::X86LabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  TR::Compilation* comp = cg()->comp();
  if (getOpCode().isBranchOp()) {
    ...
    if (getOpCode().hasIntImmediate()) {
      ...
      if (getLabelSymbol() && getLabelSymbol()->getEstimatedCodeLocation()) {
        int32_t distance = getLabelSymbol()->getEstimatedCodeLocation() -
                           (currentEstimate + IA32LengthOfShortBranch);
        if (distance >= -128 && distance < 0 && _permitShortening) {
          immediateLength =
            0; // really 1, but for conditional branches (all excep JMP4) the
               // opcode entry will be 1 too big for short branch because the
               // short branch op is 1 byte, but the long branch op for
               // conditionals is 2
    ...
    setEstimatedBinaryLength(getOpCode().length(self()->rexBits()) +
                             immediateLength);
  } else if (getOpCodeValue() == LABEL) {
...
TR::X86FenceInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  cg()->addProjectSpecializedRelocation(
    cursor, 0, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__, _fenceNode);
}

uint8_t*
TR::X86FenceInstruction::generateBinaryEncoding()
{
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  int i;
...
int32_t
TR::X86VirtualGuardNOPInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(TR::Compiler->target.is64Bit()
                             ? 5
                             : 6); // TODO:AMD64: What if patched instruction
                                   // needs a Rex?  Is 6 enough?
  return currentEstimate + getEstimatedBinaryLength();
}

uint8_t*
TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding()
{
  TR::Compilation* comp = cg()->comp();
  uint8_t* instructionStart = cg()->getBinaryBufferCursor();
  ...
  // guards that do not require atomic patching have a more relaxed sizing
  // constraing since they are only patched while all threads are stopped
  ...
  // If the current instruction is already a patchable instruction, we cannot
  // use it as a vgnop guard site. We will need to insert nops in this case.
  //
  ...
  // in the case of an HCR guard merged with a Profiled Guard, we know the
  // instruction to patch is the Profield Guard conditional jump. Because the
  // HCR Guard jumps to the same destination we are guaranteed that the patch
  // site will be big enough for the required jump and so we force the patch
  // size to longJumpWidth to suppress NOP emission
  else if (getNode()->isProfiledGuard()) {
    _nopSize = 0;
  }

  // HCR guards are special because they occur in stop the world so we don't
  // require atomic nop sequences - all other guards require a single
  // instruction to prevent possible inconsistent state seen by the processor
  // while decoding when patching occurs
  if (instToBePatched && _nopSize > 0 &&
      !cg()->nopsAlsoProcessedByRelocations()) {
    TR_ASSERT(
      _nopSize > next,
      "Emitting nops when the instruction is big enough is always wrong!");
    OMR::X86::EnlargementResult enlargement = instToBePatched->enlarge(
      _nopSize - next, getEstimatedBinaryLength(), !requiresAtomicPatching);
    if (enlargement.getPatchGrowth() + next >= _nopSize) {
      ...
      TR_ASSERT(
        enlargement.getPatchGrowth() == 0,
        "Instructions should only partially enlarge when dealing with an HCR "
        "guard - any other partial enlargement is a waste of code cache!");
    }
    // donate some of our estimated size to the newly englarged instruction
    // the instruction to be patched will have already taken the extra size when
    // computing enlarge
    if (enlargement.getEncodingGrowth() > 0)
      setEstimatedBinaryLength(getEstimatedBinaryLength() -
                               enlargement.getEncodingGrowth());
  }

  cursor = cg()->generatePadding(cursor,
                                 _nopSize,
                                 this,
                                 requiresAtomicPatching ? TR_AtomicNoOpPadding
                                                        : TR_NoOpPadding);
  ...
  if (getOpCode().hasIntImmediate()) {
    if (needsAOTRelocation()) {
      cg()->addExternalRelocation(
        new (cg()->trHeapMemory())
          TR::ExternalRelocation(cursor, 0, TR_BodyInfoAddress, cg()),
        __FILE__,
        __LINE__,
        getNode());
    }
    ...
    if (getReloKind() != -1) // TODO: need to change Body info one to use this
    {
      ...
      switch (getReloKind()) {
        ...
        default:
          cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(
              cursor, 0, (TR_ExternalRelocationTargetKind)getReloKind(), cg()),
            __FILE__,
            __LINE__,
            getNode());
      }
    ...
    if (std::find(cg()->comp()->getStaticHCRPICSites()->begin(),
                  cg()->comp()->getStaticHCRPICSites()->end(),
                  this) != cg()->comp()->getStaticHCRPICSites()->end()) {
...
uint8_t*
TR::X86ImmInstruction::generateOperand(uint8_t* cursor)
{
  ...
  if (getOpCode().hasIntImmediate()) {
    *(int32_t*)cursor = (int32_t)getSourceImmediate();
    ...
    cursor += 4;
  } else if (getOpCode().hasByteImmediate() ||
             getOpCode().hasSignExtendImmediate()) {
...
uint8_t
TR::X86ImmInstruction::getBinaryLengthLowerBound()
{
...
int32_t
TR::X86ImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  uint32_t immediateLength = 1;
  if (getOpCode().hasIntImmediate()) {
    immediateLength = 4;
...
uint8_t*
TR::X86ImmSnippetInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* immediateCursor = cursor;
...
int32_t
TR::X86ImmSymInstruction::estimateBinaryLength(int32_t currentEstimate)
{

  currentEstimate =
    TR::X86ImmInstruction::estimateBinaryLength(currentEstimate);
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
        ...
        if (labelSym) {
          ...
          if (methodSym && methodSym->isHelper()) {
            cg()->addProjectSpecializedRelocation(
              ...
              __LINE__,
              getNode());
          } else if (methodSym && methodSym->isJNI() && getNode() &&
                     getNode()->isPreparedForDirectJNI()) {
            static const int methodJNITypes = 4;
            static const int reloTypes[methodJNITypes] = {
              TR_JNIVirtualTargetAddress,
              0 /*Interfaces*/,
              TR_JNIStaticTargetAddress,
              TR_JNISpecialTargetAddress
            };
            int rType =
              methodSym->getMethodKind() - 1; // method kinds are 1-based
            TR_ASSERT(reloTypes[rType],
                      "There shouldn't be direct JNI interface calls!");

            cg()->addExternalRelocation(
              new (cg()->trHeapMemory()) TR::ExternalRelocation(
                cursor,
                (uint8_t*)getNode()->getSymbolReference(),
                getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex()
                          : (uint8_t*)-1,
                (TR_ExternalRelocationTargetKind)reloTypes[rType],
                cg()),
              __FILE__,
              __LINE__,
              getNode());
      ...
      cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(
          ...
          getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex()
                    : (uint8_t*)-1,
          TR_ConstantPool,
          cg()),
        __FILE__,
      ...
      if (symbol->isConst()) {
        ...
        if (staticSym && staticSym->isCompiledMethod())
          cg()->addExternalRelocation(
            new (cg()->trHeapMemory())
              TR::ExternalRelocation(cursor, 0, TR_RamMethod, cg()),
            __FILE__,
            __LINE__,
            getNode());
          ...
          TR::DebugCounterBase* counter =
            comp->getCounterFromStaticAddress(getSymbolReference());
          if (counter == NULL) {
            comp->failCompilation<TR::CompilationException>(
...
uint8_t*
TR::X86ImmSymInstruction::generateOperand(uint8_t* cursor)
{
  TR::Compilation* comp = cg()->comp();
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (sym->isStatic())
      *(intptrj_t*)cursor =
        (intptr_t)(sym->getStaticSymbol()->getStaticAddress());
    else
      *(int32_t*)cursor = (int32_t)getSourceImmediate();
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
      ...
      if (TR::Compiler->target.is64Bit() && cg()->hasCodeCacheSwitched() &&
          getOpCodeValue() == CALLImm4) {
      ...
      if (comp->isRecursiveMethodTarget(sym)) {
        ...
        if (TR::Compiler->target.is64Bit()) {
          ...
          TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinRIPRange(
                            (intptrj_t)start, nextInstructionAddress),
                          "Method start must be within RIP range");
          cg()->fe()->reserveTrampolineIfNecessary(
            comp, getSymbolReference(), true);
        }
        ...
        if (!labelSym) {
          // TODO:
          //
          // The following is only necessary on 64-bit because of the present
          // way in which CALL instructions are created.  On 32-bit the target
          // is always reachable from the call site, so the actual 32-bit target
          // address is encoded in the immediate field of the instruction. There
          // is no need to ask the symbol reference for the actual address
          // because all the information is already present in the instruction.
          //
          // On 64-bit, the target address may not fit in a 32-bit address.  The
          // target symbol must be queried to determine the actual target
          // address and then whether it is with the 32-bit displacement range.
          // Hence, 64-bit relies on a valid target symbol to find the actual
          // target address.
          //
          // Not all CALL instructions have symbol references that describe the
          // actual target. Profiled targets in static IPICs are an example. The
          // handshake between instruction selection and binary encoding is
          // dubious, and the safe thing to do at this stage is to revert back
          // to the original logic in this method on 32-bit.  Such CALL
          // instructions do not appear on the 64-bit path.
          //
          ...
          if (TR::Compiler->target.is64Bit()) {
            ...
            //
            // Symbols created for JNI methods have their method address
            // pointing to the RAM method. Otherwise, the symbol method address
            // points at the actual target address.
            //
            TR::Node* symNode = getNode();
            if (methodSym && methodSym->isJNI() && symNode &&
                symNode->isPreparedForDirectJNI()) {
              TR::ResolvedMethodSymbol* resolvedMethodSym =
                sym->getResolvedMethodSymbol();
              TR_ResolvedMethod* resolvedMethod =
                resolvedMethodSym ? resolvedMethodSym->getResolvedMethod() : 0;
              targetAddress =
                (uintptrj_t)resolvedMethod->startAddressForJNIMethod(comp);
            } else
              targetAddress =
                (intptrj_t)getSymbolReference()->getMethodAddress();
          }

          bool isTrampolineRequired = cg()->directCallRequiresTrampoline(
            targetAddress, currentInstructionAddress);

          if (methodSym && methodSym->isHelper()) {
            if (isTrampolineRequired) {
              // TODO:AMD64: Consider AOT ramifications
              targetAddress =
                TR::CodeCacheManager::instance()->findHelperTrampoline(
            ...
            if (isTrampolineRequired) {
              TR_ASSERT(0,
                        "We seem to never generate CALLIMM instructions for "
                        "JNI on 64Bit!!!");
            }
          } else {
            // we need to reserve trampoline in all cases, since methods can get
            // recompiled and we might need one in the future.
            if (TR::Compiler->target.is64Bit())
              cg()->fe()->reserveTrampolineIfNecessary(
                comp, getSymbolReference(), true);

            if (isTrampolineRequired) {
              targetAddress = cg()->fe()->methodTrampolineLookup(
                comp, getSymbolReference(), (void*)cursor);
            }
          }

          TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinRIPRange(
                            targetAddress, nextInstructionAddress),
      ...
      if (!symbol->isConst() && symbol->isClassObject()) {
        if (cg()->needClassAndMethodPointerRelocations()) {
          if (sym->isStatic()) {
            *(intptrj_t*)cursor =
              (intptrj_t)
                TR::Compiler->cls.persistentClassPointerFromClassPointer(
                  cg()->comp(),
                  (TR_OpaqueClassBlock*)sym->getStaticSymbol()
                    ->getStaticAddress());
          } else {
            *(int32_t*)cursor =
...
uint8_t*
TR::X86RegInstruction::generateOperand(uint8_t* cursor)
{
...
uint8_t
TR::X86RegInstruction::getBinaryLengthLowerBound()
{
...
int32_t
TR::X86RegInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  TR_X86OpCode& opCode = getOpCode();
...
OMR::X86::EnlargementResult
TR::X86RegInstruction::enlarge(int32_t requestedEnlargementSize,
                               int32_t maxEnlargementSize,
                               bool allowPartialEnlargement)
{
  static char* disableRexExpansion =
    feGetEnv("TR_DisableREXInstructionExpansion");
  if (disableRexExpansion ||
      cg()->comp()->getOption(TR_DisableZealousCodegenOpts))
    return OMR::X86::EnlargementResult(0, 0);

  if (getOpCode().info().supportsAVX() &&
      cg()->getX86ProcessorInfo().supportsAVX())
    return OMR::X86::EnlargementResult(
      0, 0); // REX expansion isn't allowed for AVX instructions
  ...
  if (TR::Compiler->target.is64Bit() &&
      ...
      performTransformation(comp,
                            "O^O Enlarging instruction %p by %d bytes by "
                            "repeating the REX prefix\n",
                            this,
                            enlargementSize)) {
...
uint8_t*
TR::X86RegRegInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* modRM = cursor - 1;
  if (getOpCode().hasTargetRegisterIgnored() == 0) {
    applyTargetRegisterToModRMByte(modRM);
...
uint8_t*
TR::X86RegRegRegInstruction::generateOperand(uint8_t* cursor)
{
  TR_ASSERT(getOpCode().info().supportsAVX(),
            "TR::X86RegRegRegInstruction must be an AVX instruction.");
  uint8_t* modRM = cursor - 1;
  if (getOpCode().hasTargetRegisterIgnored() == 0) {
    applyTargetRegisterToModRMByte(modRM);
  }
...
TR::X86RegImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (getOpCode().hasIntImmediate()) {
    // TODO: PIC registration code only works for 32-bit platforms as static PIC
    // address is a 32 bit quantity
    //
    bool staticPIC = false;
    if (std::find(comp->getStaticPICSites()->begin(),
                  comp->getStaticPICSites()->end(),
                  this) != comp->getStaticPICSites()->end()) {
      TR_ASSERT(getOpCode().hasIntImmediate(),
                "StaticPIC: class pointer cannot be smaller than 4 bytes");
      staticPIC = true;
    }

    // HCR register HCR PIC sites in
    // TR::X86RegImmInstruction::generateBinaryEncoding
    bool staticHCRPIC = false;
    if (std::find(comp->getStaticHCRPICSites()->begin(),
                  comp->getStaticHCRPICSites()->end(),
                  this) != comp->getStaticHCRPICSites()->end()) {
      TR_ASSERT(getOpCode().hasIntImmediate(),
                "StaticHCRPIC: class pointer cannot be smaller than 4 bytes");
      staticHCRPIC = true;
    }

    bool staticMethodPIC = false;
    if (std::find(comp->getStaticMethodPICSites()->begin(),
    ...
    if (staticHCRPIC) {
      cg()->addExternalRelocation(
        new (cg()->trHeapMemory())
          TR::ExternalRelocation((uint8_t*)cursor,
                                 (uint8_t*)(uintptr_t)getSourceImmediate(),
                                 TR_HCR,
                                 cg()),
        __FILE__,
      ...
      void* classPointer =
        (void*)cg()
          ...
          ->createResolvedMethod(
            cg()->trMemory(),
            (TR_OpaqueMethodBlock*)(uintptr_t)getSourceImmediateAsAddress(),
    ...
    switch (_reloKind) {
      case TR_HEAP_BASE:
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
            cursor, (uint8_t*)TR_HeapBase, TR_GlobalValue, cg()),
          __FILE__,
          __LINE__,
          getNode());
        break;
      ...
      case TR_HEAP_TOP:
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
            cursor, (uint8_t*)TR_HeapTop, TR_GlobalValue, cg()),
          __FILE__,
          __LINE__,
          getNode());
        break;
      ...
      case TR_ACTIVE_CARD_TABLE_BASE:
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
            cursor, (uint8_t*)TR_ActiveCardTableBase, TR_GlobalValue, cg()),
          __FILE__,
          __LINE__,
          getNode());
        break;
...
uint8_t*
TR::X86RegImmInstruction::generateOperand(uint8_t* cursor)
{
  TR::Compilation* comp = cg()->comp();
...
uint8_t
TR::X86RegImmInstruction::getBinaryLengthLowerBound()
{
  uint8_t len = getOpCode().length(self()->rexBits());
...
int32_t
TR::X86RegImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  uint32_t immediateLength = 1;
  if (getOpCode().hasIntImmediate()) {
    immediateLength = 4;
  ...
  setEstimatedBinaryLength(getOpCode().length(self()->rexBits()) +
                           rexRepeatCount() + immediateLength);
  return currentEstimate + getEstimatedBinaryLength();
}
  ...
  switch (getReloKind()) {
    ...
    case TR_ClassObject: {
      if (cg()->needClassAndMethodPointerRelocations()) {
        ...
        TR_ASSERT(
          !(getSymbolReference()->isUnresolved() && !symbol->isClassObject()),
    ...
    case TR_DebugCounter: {
      ...
      TR::DebugCounter::generateRelocation(
        cg()->comp(), cursor, getNode(), counter);
    } break;
...
uint8_t*
TR::X86RegImmSymInstruction::generateOperand(uint8_t* cursor)
{
  TR::Compilation* comp = cg()->comp();
  ...
  TR_ASSERT(getOpCode().hasIntImmediate(),
            "expecting a 4 byte address for this instruction class!\n");
  TR_ASSERT(getSymbolReference(),
            "expecting a symbol reference for this instruction class!\n");
...
void
TR::X86RegRegImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
...
uint8_t*
TR::X86RegRegImmInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* modRM = cursor - 1;
  if (getOpCode().hasTargetRegisterIgnored() == 0) {
    applyTargetRegisterToModRMByte(modRM);
...
uint8_t
TR::X86RegRegImmInstruction::getBinaryLengthLowerBound()
{
  uint8_t len = getOpCode().length(self()->rexBits());
...
int32_t
TR::X86RegRegImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  uint32_t immediateLength = 1;
  if (getOpCode().hasIntImmediate()) {
    immediateLength = 4;
...
bool
TR::X86MemInstruction::needsLockPrefix()
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
  return getOpCode().needsLockPrefix() || (barrier & LockPrefix);
}

uint8_t*
TR::X86MemInstruction::generateOperand(uint8_t* cursor)
{
  return getMemoryReference()->generateBinaryEncoding(cursor - 1, this, cg());
}

uint8_t
TR::X86MemInstruction::getBinaryLengthLowerBound()
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
...
int32_t
TR::X86MemInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
  int32_t length = 0;
  ...
  int32_t patchBoundaryPadding =
    (TR::Compiler->target.isSMP() &&
     getMemoryReference()->getSymbolReference().isUnresolved())
      ? 1
      : 0;

  setEstimatedBinaryLength(getOpCode().length(self()->rexBits()) + length +
...
OMR::X86::EnlargementResult
TR::X86MemInstruction::enlarge(int32_t requestedEnlargementSize,
                               int32_t maxEnlargementSize,
                               bool allowPartialEnlargement)
{
  TR::Compilation* comp = cg()->comp();
  ...
  OMR::X86::EnlargementResult result =
    getMemoryReference()->enlarge(cg(),
                                  requestedEnlargementSize,
                                  maxEnlargementSize,
                                  allowPartialEnlargement);
  if (result.getEncodingGrowth() > 0)
    setEstimatedBinaryLength(getEstimatedBinaryLength() +
                             result.getEncodingGrowth());
...
TR::X86MemImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    // HCR register HCR pic sites in
    // TR::X86MemImmInstruction::generateBinaryEncoding
    bool staticHCRPIC = false;
    if (std::find(comp->getStaticHCRPICSites()->begin(),
                  comp->getStaticHCRPICSites()->end(),
                  this) != comp->getStaticHCRPICSites()->end()) {
...
uint8_t*
TR::X86MemImmInstruction::generateOperand(uint8_t* cursor)
{
  TR::Compilation* comp = cg()->comp();
    ...
    if (getOpCode().hasIntImmediate()) {
      ...
      *(int8_t*)cursor = (int8_t)getSourceImmediate();
      ...
      if (comp->getOption(TR_EnableGCRPatching)) {
        ...
        if (node && (node->getOpCodeValue() == TR::ificmpeq ||
                     node->getOpCodeValue() ==
                       TR::ificmpne) /*node->getNumChildren() > 0*/) {
...
uint8_t
TR::X86MemImmInstruction::getBinaryLengthLowerBound()
{
...
int32_t
TR::X86MemImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
...
uint8_t*
TR::X86MemImmSymInstruction::generateOperand(uint8_t* cursor)
{
  cursor = getMemoryReference()->generateBinaryEncoding(cursor - 1, this, cg());
  if (cursor) {
    TR_ASSERT(getOpCode().hasIntImmediate(),
...
uint8_t*
TR::X86MemRegInstruction::generateOperand(uint8_t* cursor)
{
  if (getOpCode().hasSourceRegisterIgnored() == 0) {
...
void
TR::X86MemRegImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  if (getOpCode().hasIntImmediate()) {
    if (std::find(cg()->comp()->getStaticHCRPICSites()->begin(),
                  cg()->comp()->getStaticHCRPICSites()->end(),
...
uint8_t*
TR::X86MemRegImmInstruction::generateOperand(uint8_t* cursor)
{
  toRealRegister(getSourceRegister())->setRegisterFieldInModRM(cursor - 1);
  cursor = getMemoryReference()->generateBinaryEncoding(cursor - 1, this, cg());
  if (cursor) {
...
uint8_t
TR::X86MemRegImmInstruction::getBinaryLengthLowerBound()
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
...
int32_t
TR::X86MemRegImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  int32_t length = getMemoryReference()->estimateBinaryLength(cg());
...
bool
TR::X86RegMemInstruction::needsLockPrefix()
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
  return getOpCode().needsLockPrefix() || (barrier & LockPrefix);
...
uint8_t*
TR::X86RegMemInstruction::generateOperand(uint8_t* cursor)
{
  if (getOpCode().hasTargetRegisterIgnored() == 0) {
...
uint8_t
TR::X86RegMemInstruction::getBinaryLengthLowerBound()
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
...
int32_t
TR::X86RegMemInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
  ...
  setEstimatedBinaryLength(getOpCode().length(self()->rexBits()) +
                           rexRepeatCount() + length + patchBoundaryPadding);
  return currentEstimate + getEstimatedBinaryLength();
}

OMR::X86::EnlargementResult
TR::X86RegMemInstruction::enlarge(int32_t requestedEnlargementSize,
                                  int32_t maxEnlargementSize,
                                  bool allowPartialEnlargement)
{
  TR::Compilation* comp = cg()->comp();
...
uint8_t*
TR::X86RegRegMemInstruction::generateOperand(uint8_t* cursor)
{
  TR_ASSERT(getOpCode().info().supportsAVX(),
            "TR::X86RegRegRegInstruction must be an AVX instruction.");
  uint8_t* modRM = cursor - 1;
...
void
TR::X86RegMemImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  if (getOpCode().hasIntImmediate()) {
    if (std::find(cg()->comp()->getStaticHCRPICSites()->begin(),
                  cg()->comp()->getStaticHCRPICSites()->end(),
...
uint8_t*
TR::X86RegMemImmInstruction::generateOperand(uint8_t* cursor)
{
  if (getOpCode().hasTargetRegisterIgnored() == 0) {
    toRealRegister(getTargetRegister())->setRegisterFieldInModRM(cursor - 1);
  }
...
uint8_t
TR::X86RegMemImmInstruction::getBinaryLengthLowerBound()
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
...
int32_t
TR::X86RegMemImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  int32_t barrier =
    memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
...
uint8_t*
TR::X86FPRegInstruction::generateOperand(uint8_t* cursor)
{
...
uint8_t*
TR::X86FPRegRegInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* opCode = cursor - 1;
...
uint8_t*
TR::X86FPST0ST1RegRegInstruction::generateOperand(uint8_t* cursor)
{
  return cursor;
}
...
uint8_t*
TR::X86FPArithmeticRegRegInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* opCode = cursor - 1;
...
uint8_t*
TR::X86FPST0STiRegRegInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* opCode = cursor - 1;
...
uint8_t*
TR::X86FPSTiST0RegRegInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* opCode = cursor - 1;
...
uint8_t*
TR::X86FPCompareRegRegInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* opCode = cursor - 1;
  applyRegistersToOpCode(opCode, cg()->machine());
  return cursor;
...
uint8_t*
TR::X86FPRegMemInstruction::generateOperand(uint8_t* cursor)
{
  return getMemoryReference()->generateBinaryEncoding(cursor - 1, this, cg());
}

uint8_t
TR::X86FPRegMemInstruction::getBinaryLengthLowerBound()
{
  return getOpCode().length(self()->rexBits());
}
...
uint8_t*
TR::X86FPMemRegInstruction::generateOperand(uint8_t* cursor)
{
  cursor = getMemoryReference()->generateBinaryEncoding(cursor - 1, this, cg());
  if (cursor) {
...
TR::AMD64RegImm64Instruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  TR::SymbolReference* methodSymRef =
    getNode()->getOpCode().hasSymbolReference()
      ? getNode()->getSymbolReference()
      : NULL;

#ifdef J9_PROJECT_SPECIFIC
  if (cg()->needRelocationsForHelpers()) {
    if (
      getNode()->getOpCode().hasSymbolReference() && methodSymRef &&
      ...
      cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(
          cursor, (uint8_t*)methodSymRef, TR_AbsoluteHelperAddress, cg()),
        __FILE__,
        __LINE__,
        getNode());
    }
  ...
  if (comp->fej9()->needClassAndMethodPointerRelocations()) {
    if (((getNode()->getOpCodeValue() == TR::aconst) &&
         getNode()->isMethodPointerConstant() && needsAOTRelocation()) ||
        staticHCRPIC) {
      cg()->addExternalRelocation(
        new (cg()->trHeapMemory())
          TR::ExternalRelocation(cursor, NULL, TR_RamMethod, cg()),
        __FILE__,
        __LINE__,
        getNode());
    } else {
  ...
  if (staticPIC) {
    cg()->jitAddPicToPatchOnClassUnload(((void*)getSourceImmediate()),
                                        (void*)cursor);
  }
  ...
  if (staticHCRPIC) {
    cg()->addExternalRelocation(
      new (cg()->trHeapMemory()) TR::ExternalRelocation(
        (uint8_t*)cursor, (uint8_t*)getSourceImmediate(), TR_HCR, cg()),
      __FILE__,
      __LINE__,
      getNode());
    cg()->jitAddPicToPatchOnClassRedefinition(((void*)getSourceImmediate()),
                                              (void*)cursor);
  }
    ...
    void* classPointer =
      (void*)cg()
        ...
        ->createResolvedMethod(cg()->trMemory(),
                               (TR_OpaqueMethodBlock*)getSourceImmediate(),
                               comp->getCurrentMethod())
        ->classOfMethod();
...
uint8_t*
TR::AMD64RegImm64Instruction::generateOperand(uint8_t* cursor)
{
  uint8_t* modRM = cursor - 1;
  if (getOpCode().hasTargetRegisterIgnored() == 0) {
    applyTargetRegisterToModRMByte(modRM);
  }
  TR_ASSERT(getOpCode().hasLongImmediate(),
            "Imm64 instructions must have long immediates");
...
uint8_t
TR::AMD64RegImm64Instruction::getBinaryLengthLowerBound()
{
  return getOpCode().length(self()->rexBits()) + rexRepeatCount() + 8;
}

int32_t
TR::AMD64RegImm64Instruction::estimateBinaryLength(int32_t currentEstimate)
{
...
TR::AMD64RegImm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (getSymbolReference()->getSymbol()->isLabel()) {
    ...
    //
    cg()->addRelocation(new (cg()->trHeapMemory()) TR::LabelAbsoluteRelocation(
      cursor, getSymbolReference()->getSymbol()->castToLabelSymbol()));

    switch (getReloKind()) {
      case TR_AbsoluteMethodAddress:
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
            cursor, NULL, TR_AbsoluteMethodAddress, cg()),
          __FILE__,
          __LINE__,
          getNode());
        break;
    ...
    switch (getReloKind()) {
      ...
      case TR_NativeMethodAbsolute: {
        if (cg()->comp()->getOption(TR_EmitRelocatableELFFile)) {
          TR_ResolvedMethod* target = getSymbolReference()
                                        ->getSymbol()
                                        ->castToResolvedMethodSymbol()
                                        ->getResolvedMethod();
          cg()->addStaticRelocation(
            TR::StaticRelocation(cursor,
...
uint8_t*
TR::AMD64RegImm64SymInstruction::generateOperand(uint8_t* cursor)
{
  uint8_t* modRM = cursor - 1;
...
TR::AMD64Imm64Instruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  if (needsAOTRelocation()) {
    cg()->addProjectSpecializedRelocation(
      cursor, 0, NULL, TR_BodyInfoAddress, __FILE__, __LINE__, getNode());
  }
}

uint8_t*
TR::AMD64Imm64Instruction::generateOperand(uint8_t* cursor)
{
...
uint8_t
TR::AMD64Imm64Instruction::getBinaryLengthLowerBound()
{
...
int32_t
TR::AMD64Imm64Instruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(getOpCode().length(self()->rexBits()) + 8);
  return currentEstimate + getEstimatedBinaryLength();
}
...
//
int32_t
TR::AMD64Imm64SymInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  currentEstimate =
    TR::AMD64Imm64Instruction::estimateBinaryLength(currentEstimate);
  return currentEstimate;
}
...
TR::AMD64Imm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  if (getOpCodeValue() == DQImm64) {
    cg()->addProjectSpecializedRelocation(
      cursor,
      (uint8_t*)(uint64_t)getSourceImmediate(),
      getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex()
                : (uint8_t*)-1,
      TR_Thunks,
      __FILE__,
...
uint8_t*
TR::AMD64Imm64SymInstruction::generateOperand(uint8_t* cursor)
{
  TR_ASSERT(getOpCodeValue() == DQImm64,
            "TR::AMD64Imm64SymInstruction expected to be DQImm64 only");
