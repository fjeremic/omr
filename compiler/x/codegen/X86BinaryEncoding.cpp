  ...
  TR_X86OpCode& op, TR::MemoryReference* mr, TR::CodeGenerator* cg, bool onlyAskingAboutFences)
{
  ...
  if (barrier) {
    if ((!cg->getX86ProcessorInfo().supportsLFence() || !cg->getX86ProcessorInfo().supportsMFence())
...
uint8_t* OMR::X86::Instruction::generateBinaryEncoding()
{
  ...
  for (;;) {
    ...
    // cursor is NULL when generateOperand() requests to regenerate the binary code, which may happen during encoding of
    // memref with unresolved symbols on 64-bit
    if (cursor) {
...
uint8_t* TR::X86BoundaryAvoidanceInstruction::generateBinaryEncoding()
{
  ...
  while (cur->getLength() > 0) {
    TR_ASSERT(cur->getLength() <= _boundarySpacing, "Atomic regions of %d bytes can't fit between %d-byte boundaries",
...
void TR::X86LabelInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (!getOpCode().hasRelativeBranchDisplacement() && !(getOpCodeValue() == LABEL)) {
    ...
    if (label == NULL) {
      ...
      *(uint32_t*)cursor = 0;
      ...
      TR_ASSERT(getOpCodeValue() != XBEGIN4 || !_permitShortening,
        "XBEGIN4 cannot be shortened and can only be used with a label instruction that cannot shorten - use "
        "generateLongLabel!\n");
    ...
    if (getOpCode().hasIntImmediate()) {
      ...
      if (getLabelSymbol() && getLabelSymbol()->getEstimatedCodeLocation()) {
        ...
        if (distance >= -128 && distance < 0 && _permitShortening) {
          immediateLength
            = 0; // really 1, but for conditional branches (all excep JMP4) the opcode entry will be 1 too big for short
                 // branch because the short branch op is 1 byte, but the long branch op for conditionals is 2
...
int32_t TR::X86VirtualGuardNOPInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  setEstimatedBinaryLength(
    TR::Compiler->target.is64Bit() ? 5 : 6); // TODO:AMD64: What if patched instruction needs a Rex?  Is 6 enough?
  return currentEstimate + getEstimatedBinaryLength();
}
...
uint8_t* TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding()
{
  ...
  // guards that do not require atomic patching have a more relaxed sizing constraing since they are only patched while
  ...
  if (instToBePatched && _nopSize > 0 && !cg()->nopsAlsoProcessedByRelocations()) {
    ...
    if (enlargement.getPatchGrowth() + next >= _nopSize) {
      ...
      TR_ASSERT(enlargement.getPatchGrowth() == 0,
        "Instructions should only partially enlarge when dealing with an HCR guard - any other partial enlargement is "
        "a waste of code cache!");
    }
    ...
    if (getReloKind() != -1) // TODO: need to change Body info one to use this
    {
      ...
      case TR_SpecialRamMethodConst:
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getNode()->getSymbolReference(),
      ...
      case TR_ClassPointer:
        if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
          cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                        TR::ExternalRelocation(cursor, (uint8_t*)(uintptr_t)getSourceImmediate(),
                                          (uint8_t*)symbolKind, TR_SymbolFromManager, cg()),
            __FILE__, __LINE__, getNode());
        } else {
          cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        cursor, (uint8_t*)getNode(), (TR_ExternalRelocationTargetKind)_reloKind, cg()),
            __FILE__, __LINE__, getNode());
        }
        break;
      default:
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
      if (!comp->isRecursiveMethodTarget(sym)) {
        ...
        if (labelSym) {
          ...
          if (methodSym && methodSym->isHelper()) {
            cg()->addProjectSpecializedRelocation(
              cursor, (uint8_t*)getSymbolReference(), NULL, TR_HelperAddress, __FILE__, __LINE__, getNode());
            ...
            static const int reloTypes[methodJNITypes]
              = { TR_JNIVirtualTargetAddress, 0 /*Interfaces*/, TR_JNIStaticTargetAddress, TR_JNISpecialTargetAddress };
            ...
            cg()->addProjectSpecializedRelocation(
              cursor, (uint8_t*)getSymbolReference(), NULL, TR_RelativeMethodAddress, __FILE__, __LINE__, getNode());
          }
        }
      }
    } else if (getOpCodeValue() == DDImm4) {
      cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)(uintptrj_t)getSourceImmediate(),
          getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, cg()),
        __FILE__, __LINE__, getNode());
      ...
      if (symbol->isConst()) {
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory())
            TR::ExternalRelocation(cursor, (uint8_t*)getSymbolReference()->getOwningMethod(comp)->constantPool(),
              getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, cg()),
          __FILE__, __LINE__, getNode());
        ...
        if (cg()->needClassAndMethodPointerRelocations()) {
          if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            ...
            cg()->addExternalRelocation(
              new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
                getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ClassAddress,
                cg()),
              __FILE__, __LINE__, getNode());
          }
        }
        ...
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
            getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_MethodObject, cg()),
          __FILE__, __LINE__, getNode());
      } else {
        ...
        if (staticSym && staticSym->isCompiledMethod())
          ...
          cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
              getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_DataAddress, cg()),
            __FILE__, __LINE__, getNode());
      }
    }
  }
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
      ...
      *(int32_t*)cursor = (int32_t)(targetAddress - nextInstructionAddress);
      ...
      if (!symbol->isConst() && symbol->isClassObject()) {
        if (cg()->needClassAndMethodPointerRelocations()) {
          if (sym->isStatic()) {
            *(intptrj_t*)cursor = (intptrj_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(
  ...
  int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
  ...
  TR::Compilation* comp = cg()->comp();
  if (TR::Compiler->target.is64Bit() && !getOpCode().info().hasMandatoryPrefix()
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (staticMethodPIC) {
      void* classPointer
        = (void*)cg()
            ->fe()
            ->createResolvedMethod(cg()->trMemory(), (TR_OpaqueMethodBlock*)(uintptr_t)getSourceImmediateAsAddress(),
    ...
    case TR_HEAP_BASE:
      cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor, (uint8_t*)TR_HeapBase, TR_GlobalValue, cg()),
        __FILE__, __LINE__, getNode());
      break;

    case TR_HEAP_TOP:
      cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor, (uint8_t*)TR_HeapTop, TR_GlobalValue, cg()),
        __FILE__, __LINE__, getNode());
      break;
    case TR_HEAP_BASE_FOR_BARRIER_RANGE:
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                    cursor, (uint8_t*)TR_HeapBaseForBarrierRange0, TR_GlobalValue, cg()),
        __FILE__, __LINE__, getNode());
      break;
    case TR_HEAP_SIZE_FOR_BARRIER_RANGE:
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                    cursor, (uint8_t*)TR_HeapSizeForBarrierRange0, TR_GlobalValue, cg()),
        __FILE__, __LINE__, getNode());
      break;
...
void TR::X86RegImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  case TR_ClassPointer:
    if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
      ...
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                    cursor, (uint8_t*)getNode(), (TR_ExternalRelocationTargetKind)getReloKind(), cg()),
        __FILE__, __LINE__, getNode());
    }
    break;
  case TR_DebugCounter: {
    ...
    if (counter == NULL) {
      cg()->comp()->failCompilation<TR::CompilationException>(
        "Could not generate relocation for debug counter in TR::X86RegImmSymInstruction::addMetaDataForCodeAddress\n");
    }
...
int32_t TR::X86MemInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  ...
  int32_t patchBoundaryPadding
    = (TR::Compiler->target.isSMP() && getMemoryReference()->getSymbolReference().isUnresolved()) ? 1 : 0;
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (_reloKind == TR_ClassAddress && cg()->needClassAndMethodPointerRelocations()) {
      ...
      if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
        ...
        cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                      TR::ExternalRelocation(cursor, (uint8_t*)getNode()->getSymbolReference(),
                                        (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex(), TR_ClassAddress, cg()),
          __FILE__, __LINE__, getNode());
      }
    }
  }
  ...
  if (symbol->isConst()) {
    ...
    if (cg()->needClassAndMethodPointerRelocations()) {
      *(int32_t*)cursor = (int32_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(
      ...
      if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
        ...
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
            getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ClassAddress, cg()),
          __FILE__, __LINE__, getNode());
      }
    }
  } else if (symbol->isMethod()) {
    ...
    if (counter == NULL) {
      comp->failCompilation<TR::CompilationException>(
        "Could not generate relocation for debug counter in TR::X86MemImmSymInstruction::addMetaDataForCodeAddress\n");
    }
    TR::DebugCounter::generateRelocation(comp, cursor, getNode(), counter);
  } else {
    cg()->addExternalRelocation(
...
void TR::AMD64RegImm64Instruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (comp->fej9()->needClassAndMethodPointerRelocations()) {
    if (((getNode()->getOpCodeValue() == TR::aconst) && getNode()->isMethodPointerConstant() && needsAOTRelocation())
      || staticHCRPIC) {
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, NULL, TR_RamMethod, cg()),
        __FILE__, __LINE__, getNode());
    } else {
      TR::SymbolType symbolKind = TR::SymbolType::typeClass;
      switch (_reloKind) {
      case TR_ClassAddress: {
        ...
        if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
          cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                        TR::ExternalRelocation(cursor, (uint8_t*)getSourceImmediate(),
                                          (uint8_t*)TR::SymbolType::typeClass, TR_SymbolFromManager, cg()),
            __FILE__, __LINE__, getNode());
        } else {
          cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)methodSymRef,
                                        (uint8_t*)getNode()->getInlinedSiteIndex(), TR_ClassAddress, cg()),
            __FILE__, __LINE__, getNode());
        }
      } break;
  ...
  if (staticHCRPIC) {
    cg()->addExternalRelocation(
      new (cg()->trHeapMemory()) TR::ExternalRelocation((uint8_t*)cursor, (uint8_t*)getSourceImmediate(), TR_HCR, cg()),
      __FILE__, __LINE__, getNode());
  ...
  if (staticMethodPIC) {
    void* classPointer = (void*)cg()
                           ...
                           ->createResolvedMethod(
                             cg()->trMemory(), (TR_OpaqueMethodBlock*)getSourceImmediate(), comp->getCurrentMethod())
                           ->classOfMethod();
...
void TR::AMD64RegImm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  if (getSymbolReference()->getSymbol()->isLabel()) {
    ...
    case TR_AbsoluteMethodAddress:
      cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor, NULL, TR_AbsoluteMethodAddress, cg()),
        __FILE__, __LINE__, getNode());
      break;
    default:
      break;
...
void TR::AMD64Imm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
  if (getOpCodeValue() == DQImm64) {
    cg()->addProjectSpecializedRelocation(cursor, (uint8_t*)(uint64_t)getSourceImmediate(),
      getNode() ? (uint8_t*)(uintptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_Thunks, __FILE__, __LINE__,
      getNode());
  }
}
