...
int32_t memoryBarrierRequired(TR_X86OpCode &op, TR::MemoryReference *mr,
                              TR::CodeGenerator *cg,
                              bool onlyAskingAboutFences) {
  TR::Compilation *comp = cg->comp();
  if (!TR::Compiler->target.isSMP()) return NoFence;

  // LOCKed instructions are serializing instructions and implicitly handle
  // any required memory barriers.
  //
  if (op.needsLockPrefix()) return NoFence;
...
int32_t estimateMemoryBarrierBinaryLength(int32_t barrier,
                                          TR::CodeGenerator *cg) {
  int32_t length = 0;

  if (!cg->comp()->getOption(TR_DisableNewX86VolatileSupport)) return 0;
...
uint8_t getMemoryBarrierBinaryLengthLowerBound(int32_t barrier,
                                               TR::CodeGenerator *cg) {
...
int32_t TR::X86PaddingInstruction::estimateBinaryLength(
...
OMR::X86::EnlargementResult TR::X86BoundaryAvoidanceInstruction::enlarge(
    int32_t requestedEnlargementSize, int32_t maxEnlargementSize,
    bool allowPartialEnlargement) {
  static char *disableMinPadding =
      feGetEnv("TR_DisableBoundaryAvoidanceMinPadding");
  if (disableMinPadding) return OMR::X86::EnlargementResult(0, 0);
...
int32_t TR::X86BoundaryAvoidanceInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
uint8_t *TR::X86BoundaryAvoidanceInstruction::generateBinaryEncoding() {
  ...
  while (cur->getLength() > 0) {
    TR_ASSERT(cur->getLength() <= _boundarySpacing,
              ...
              cur->getLength(), _boundarySpacing);
...
int32_t TR::X86AlignmentInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
uint8_t *TR::X86LabelInstruction::generateBinaryEncoding() {
  ...
  if (getOpCode().hasRelativeBranchDisplacement()) {
      ...
      *(uint32_t *)cursor = 0;
      ...
      if (label->getCodeLocation() != NULL) {
        ...
        distance = cg()->getBinaryBufferStart()
                   ...
                   // +4 == Temporary and possibly incomplete fix for WebSphere
                   // problem,
      ...
      if (distance >= -128 && distance <= 127 && getOpCode().isBranchOp() &&
        ...
        if (getOpCode().isShortBranchOp()) {
          ...
          //
          TR_ASSERT(0,
                    "short form branch displacement too large: instr=%p, "
                    "distance=%d\n",
                    this, distance);
    ...
    immediateCursor = cursor;
  } else  // assume absolute code address referencing instruction like push
          // label
  {
    cursor = getOpCode().binary(instructionStart, self()->rexBits());
    immediateCursor = cursor;
...
uint8_t TR::X86LabelInstruction::getBinaryLengthLowerBound() {
  if (getOpCodeValue() == LABEL) return 0;
...
OMR::X86::EnlargementResult TR::X86LabelInstruction::enlarge(
  ...
  if (disableJMPExpansion) return OMR::X86::EnlargementResult(0, 0);
  ...
  if (!getOpCode().isBranchOp()) return OMR::X86::EnlargementResult(0, 0);
...
void TR::X86FenceInstruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  cg()->addProjectSpecializedRelocation(cursor, 0, NULL,
                                        TR_AbsoluteMethodAddress, __FILE__,
                                        __LINE__, _fenceNode);
}
...
int32_t TR::X86VirtualGuardNOPInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
uint8_t *TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding() {
  ...
  if (offset >= -128 && offset <= 127) {
    if (next < shortJumpWidth || !instToBePatched) _nopSize = shortJumpWidth;
  } else {
    if (next < longJumpWidth || !instToBePatched) _nopSize = longJumpWidth;
  }
  ...
  cursor = cg()->generatePadding(
      cursor, _nopSize, this,
      ...
      switch (getReloKind()) {
        ...
        case TR_SpecialRamMethodConst:
          cg()->addExternalRelocation(
              new (cg()->trHeapMemory()) TR::ExternalRelocation(
                  cursor, (uint8_t *)getNode()->getSymbolReference(),
        ...
        case TR_ClassPointer:
          if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            cg()->addExternalRelocation(
                new (cg()->trHeapMemory()) TR::ExternalRelocation(
                    cursor, (uint8_t *)(uintptr_t)getSourceImmediate(),
                    (uint8_t *)symbolKind, TR_SymbolFromManager, cg()),
                __FILE__, __LINE__, getNode());
          } else {
            cg()->addExternalRelocation(
                new (cg()->trHeapMemory()) TR::ExternalRelocation(
                    cursor, (uint8_t *)getNode(),
        ...
        default:
          cg()->addExternalRelocation(
              new (cg()->trHeapMemory()) TR::ExternalRelocation(
                  cursor, 0, (TR_ExternalRelocationTargetKind)getReloKind(),
                  cg()),
              __FILE__, __LINE__, getNode());
      }
    }
...
int32_t TR::X86ImmSymInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  currentEstimate =
...
void TR::X86ImmSymInstruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
        ...
        if (labelSym) {
          ...
          if (methodSym && methodSym->isHelper()) {
            cg()->addProjectSpecializedRelocation(
                cursor, (uint8_t *)getSymbolReference(), NULL, TR_HelperAddress,
                __FILE__, __LINE__, getNode());
            ...
            static const int reloTypes[methodJNITypes] = {
                TR_JNIVirtualTargetAddress, 0 /*Interfaces*/,
            ...
            cg()->addProjectSpecializedRelocation(
                cursor, (uint8_t *)getSymbolReference(), NULL,
      ...
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
              cursor, (uint8_t *)(uintptrj_t)getSourceImmediate(),
      ...
      if (symbol->isConst()) {
        ...
        if (cg()->needClassAndMethodPointerRelocations()) {
          if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            cg()->addExternalRelocation(
                new (cg()->trHeapMemory()) TR::ExternalRelocation(
                    cursor, (uint8_t *)(uintptr_t)getSourceImmediate(),
                    (uint8_t *)TR::SymbolType::typeClass, TR_SymbolFromManager,
                    cg()),
                __FILE__, __LINE__, getNode());
          } else {
            cg()->addExternalRelocation(
                new (cg()->trHeapMemory()) TR::ExternalRelocation(
                    cursor, (uint8_t *)getSymbolReference(),
                    getNode()
                        ? (uint8_t *)(uintptr_t)getNode()->getInlinedSiteIndex()
                        : (uint8_t *)-1,
                    TR_ClassAddress, cg()),
        ...
        if (staticSym && staticSym->isCompiledMethod())
          ...
          cg()->addExternalRelocation(
              new (cg()->trHeapMemory()) TR::ExternalRelocation(
                  cursor, (uint8_t *)(staticSym->getStaticAddress()),
          ...
          TR::DebugCounter::generateRelocation(comp, cursor, getNode(),
                                               counter);
        } else
          cg()->addExternalRelocation(
              new (cg()->trHeapMemory()) TR::ExternalRelocation(
    ...
    if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
      ...
      if (comp->isRecursiveMethodTarget(sym)) {
        ...
        if (TR::Compiler->target.is64Bit()) {
          ...
          cg()->fe()->reserveTrampolineIfNecessary(comp, getSymbolReference(),
                                                   true);
        }
          ...
          if (TR::Compiler->target.is64Bit()) {
            ...
            if (methodSym && methodSym->isJNI() && symNode &&
              ...
              TR::ResolvedMethodSymbol *resolvedMethodSym =
                  sym->getResolvedMethodSymbol();
              TR_ResolvedMethod *resolvedMethod =
                  resolvedMethodSym ? resolvedMethodSym->getResolvedMethod()
                                    : 0;
              targetAddress =
...
void TR::X86RegImmInstruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  if (getOpCode().hasIntImmediate()) {
    ...
    if (staticHCRPIC) {
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
              (uint8_t *)cursor, (uint8_t *)(uintptr_t)getSourceImmediate(),
              TR_HCR, cg()),
          __FILE__, __LINE__, getNode());
      cg()->jitAdd32BitPicToPatchOnClassRedefinition(
          ((void *)(uintptr_t)getSourceImmediateAsAddress()), (void *)cursor);
      ...
      void *classPointer =
          (void *)cg()
              ...
              ->createResolvedMethod(cg()->trMemory(),
                                     (TR_OpaqueMethodBlock *)(uintptr_t)
                                         getSourceImmediateAsAddress(),
    ...
    switch (_reloKind) {
      ...
      case TR_HEAP_BASE_FOR_BARRIER_RANGE:
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(
                cursor, (uint8_t *)TR_HeapBaseForBarrierRange0, TR_GlobalValue,
                cg()),
            __FILE__, __LINE__, getNode());
        break;
      case TR_HEAP_SIZE_FOR_BARRIER_RANGE:
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(
                cursor, (uint8_t *)TR_HeapSizeForBarrierRange0, TR_GlobalValue,
                cg()),
            __FILE__, __LINE__, getNode());
        break;
      case TR_ACTIVE_CARD_TABLE_BASE:
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(
                cursor, (uint8_t *)TR_ActiveCardTableBase, TR_GlobalValue,
                cg()),
            __FILE__, __LINE__, getNode());
        break;
...
int32_t TR::X86RegImmInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  uint32_t immediateLength = 1;
  if (getOpCode().hasIntImmediate()) {
    immediateLength = 4;
...
void TR::X86RegImmSymInstruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  switch (getReloKind()) {
    ...
    case TR_DebugCounter: {
      ...
      TR::DebugCounter::generateRelocation(cg()->comp(), cursor, getNode(),
                                           counter);
    } break;
...
int32_t TR::X86RegRegImmInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  uint32_t immediateLength = 1;
  if (getOpCode().hasIntImmediate()) {
    immediateLength = 4;
...
uint8_t TR::X86MemInstruction::getBinaryLengthLowerBound() {
  ...
  uint8_t length = 0;
  if (getOpCode().needsLockPrefix() || (barrier & LockPrefix)) length++;
...
OMR::X86::EnlargementResult TR::X86MemInstruction::enlarge(
  ...
  OMR::X86::EnlargementResult result = getMemoryReference()->enlarge(
      cg(), requestedEnlargementSize, maxEnlargementSize,
      allowPartialEnlargement);
...
int32_t TR::X86MemImmInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
int32_t TR::X86MemRegImmInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  int32_t length = getMemoryReference()->estimateBinaryLength(cg());
...
int32_t TR::X86RegMemInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  int32_t barrier =
      memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
...
int32_t TR::X86RegMemImmInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  int32_t barrier =
      memoryBarrierRequired(getOpCode(), getMemoryReference(), cg(), false);
...
uint8_t *TR::X86FPArithmeticRegRegInstruction::generateOperand(
    uint8_t *cursor) {
  uint8_t *opCode = cursor - 1;
      ...
      switch (_reloKind) {
        case TR_ClassAddress: {
          ...
          if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
            cg()->addExternalRelocation(
                new (cg()->trHeapMemory()) TR::ExternalRelocation(
                    cursor, (uint8_t *)getSourceImmediate(),
                    (uint8_t *)TR::SymbolType::typeClass, TR_SymbolFromManager,
                    cg()),
                __FILE__, __LINE__, getNode());
          } else {
...
int32_t TR::AMD64RegImm64Instruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
void TR::AMD64RegImm64SymInstruction::addMetaDataForCodeAddress(
    uint8_t *cursor) {
  TR::Compilation *comp = cg()->comp();
      ...
      case TR_NativeMethodAbsolute: {
        if (cg()->comp()->getOption(TR_EmitRelocatableELFFile)) {
          TR_ResolvedMethod *target = getSymbolReference()
                                          ...
                                          ->getResolvedMethod();
          cg()->addStaticRelocation(TR::StaticRelocation(
              cursor, target->externalName(cg()->trMemory()),
...
int32_t TR::AMD64Imm64Instruction::estimateBinaryLength(
    int32_t currentEstimate) {
  setEstimatedBinaryLength(getOpCode().length(self()->rexBits()) + 8);
  return currentEstimate + getEstimatedBinaryLength();
}
...
//
int32_t TR::AMD64Imm64SymInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  currentEstimate =
...
void TR::AMD64Imm64SymInstruction::addMetaDataForCodeAddress(uint8_t *cursor) {
  if (getOpCodeValue() == DQImm64) {
    cg()->addProjectSpecializedRelocation(
        cursor, (uint8_t *)(uint64_t)getSourceImmediate(),
        getNode() ? (uint8_t *)(uintptr_t)getNode()->getInlinedSiteIndex()
                  : (uint8_t *)-1,
        TR_Thunks, __FILE__, __LINE__, getNode());
  }
}
