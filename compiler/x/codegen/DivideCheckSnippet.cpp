...
uint8_t *TR::X86DivideCheckSnippet::emitSnippetBody() {
  ...
  TR::RealRegister *realDivisorReg =
      toRealRegister(_divideInstruction->getSourceRegister());
  TR::RealRegister *realDividendReg =
      toRealRegister(_divideInstruction->getTargetRegister());

  // CMP realDivisorReg, -1
  //
  uint8_t rexPrefix =
      TR::Compiler->target.is64Bit()
          ? realDivisorReg->rexBits(TR::RealRegister::REX_B, false)
          : 0;
  ...
  *buffer++ = -1;
  ...
  if (_divOp.isDiv() &&
      realDividendReg->getRegisterNumber() != TR::RealRegister::eax) {
    // MOV eax, realDividendReg
    //
    if (TR::Compiler->target.is64Bit()) {
      uint8_t rexPrefix =
          realDividendReg->rexBits(TR::RealRegister::REX_R, false);
      if (_divOp.isLong())
        rexPrefix |= TR::RealRegister::REX | TR::RealRegister::REX_W;
      if (rexPrefix) *buffer++ = rexPrefix;
    }
    *buffer++ = 0x89;
    *buffer = 0xC0;
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86DivideCheckSnippet *snippet)  // TODO:FIX THIS!!!
{
  ...
  TR::RealRegister *realDivisorReg =
      toRealRegister(snippet->getDivideInstruction()->getSourceRegister());
  TR::RealRegister *realDividendReg =
      toRealRegister(snippet->getDivideInstruction()->getTargetRegister());
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (isLong) rexPrefix |= TR::RealRegister::REX | TR::RealRegister::REX_W;
    if (rexPrefix) cmpSize++;
  }
#endif
  printPrefix(pOutFile, NULL, bufferPos, cmpSize);
  trfprintf(pOutFile, "cmp\t%s, -1",
            getName(realDivisorReg));  // TODO: Might be 64-bit register
  bufferPos += cmpSize;

  int32_t size = snippet->estimateRestartJumpLength(
      JNE4, bufferPos - (uint8_t *)0, snippet->getDivideLabel());
  printPrefix(pOutFile, NULL, bufferPos, size);
  printLabelInstruction(pOutFile, "jne", snippet->getDivideLabel());
  bufferPos += size;

  if (snippet->getOpCode().isDiv() &&
      realDividendReg->getRegisterNumber() != TR::RealRegister::eax) {
    int32_t movSize = 2;
#if defined(TR_TARGET_64BIT)
    if (TR::Compiler->target.is64Bit()) {
      uint8_t rexPrefix =
          realDividendReg->rexBits(TR::RealRegister::REX_R, false);
      if (isLong) rexPrefix |= TR::RealRegister::REX | TR::RealRegister::REX_W;
      if (rexPrefix) movSize++;
    }
#endif
    printPrefix(pOutFile, NULL, bufferPos, movSize);
    trfprintf(pOutFile, "mov\teax, %s",
              getName(realDividendReg));  // TODO: Might be rax
    bufferPos += movSize;
  }
...
uint32_t TR::X86DivideCheckSnippet::getLength(int32_t estimatedSnippetStart) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (_divOp.isLong())
      rexPrefix |= TR::RealRegister::REX | TR::RealRegister::REX_W;
    if (rexPrefix) fixedLength++;
  }
  uint32_t jumpLength = estimateRestartJumpLength(
      JNE4, estimatedSnippetStart + fixedLength + 2, _divideLabel);
  ...
  jumpLength += estimateRestartJumpLength(estimatedSnippetStart + jumpLength +
                                          fixedLength + 2);
