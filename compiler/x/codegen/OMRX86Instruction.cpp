    ...
    switch (getOpCodeValue()) {
      ...
      default:
        if (cg->getDebug())
          TR_ASSERT(
              0,
              "Can't compute VFP adjustment due to instruction %s: %s %s, %d",
              cg->getDebug()->getName(this),
              cg->getDebug()->getMnemonicName(&getOpCode()),
              cg->getDebug()->getName(getTargetRegister()),
              getSourceImmediate());
        else
...
void insertUnresolvedReferenceInstructionMemoryBarrier(
  ...
  if (TR::Compiler->target.is64Bit()) addressReg = mr->getAddressRegister();
  ...
  if (deps) generateLabelInstruction(fenceInst, LABEL, doneLabel, deps, cg);
}
  ...
  if ((cg()->getAssignmentDirection() == cg()->Backward)) {
    ...
    if (kindsToBeAssigned & getSourceRegister()->getKindAsMask()) {
      ...
      if (cg()->enableRematerialisation() &&
        ...
        if (debug("dumpRemat")) {
          diagnostic(
              "---> Deactivating %s discardable register %s at instruction "
              "%p\n",
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    if (result & (kSourceCanBePopped | kTargetCanBePopped)) {
      ...
      if ((result & (kSourceCanBePopped | kTargetCanBePopped)) ==
        ...
        //
        TR_ASSERT(0,
                  "TR::X86FPCompareRegRegInstruction::assignRegisters() ==> "
...
bool TR::X86FPCompareRegRegInstruction::swapOperands() {
  ...
  if (debug("dumpFPRA")) diagnostic("Register operands out of order: ");
  ...
  if (  // !debug("saveFXCHs")       ||
    ...
    if (debug("dumpFPRA")) diagnostic("fxch inserted\n");

    (void)cg()->machine()->fpStackFXCH(this->getPrev(), getTargetRegister());
    return false;
  }

  if (debug("dumpFPRA")) diagnostic("%s -> ", node->getOpCode().getName());
  ...
  while (cursor != NULL) {
    ...
    if (cursorOp.isBranchOp() || cursorOp.isSetRegInstruction()) break;
  ...
  if (cursor != NULL) {
    ...
    TR_ASSERT(0,
              "\nTR::X86FPCompareRegRegInstruction::swapOperands() ==> branch "
              "or set instruction not found for FP comparison!\n");
  }
...
TR_X86OpCodes getBranchOrSetOpCodeForFPComparison(TR::ILOpCodes cmpOp,
  ...
  switch (cmpOp) {
    ...
    case TR::dcmpg:
      TR_ASSERT(0,
                "getBranchOrSetOpCodeForFPComparison() ==> comparison op is "
    ...
    case TR::dcmpgtu:
      TR_ASSERT(0,
                "getBranchOrSetOpCodeForFPComparison() ==> comparison op "
    ...
    default:
      TR_ASSERT(0,
                "getBranchOrSetOpCodeForFPComparison() ==> invalid comparison "
                "op: %d\n",
                cmpOp);
  }
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    if (assignedRegister == NULL) {
      ...
      if (sourceRegister->getTotalUseCount() !=
        ...
        diagnostic(
            "TR::X86FPMemRegInstruction::assignRegisters ==> first reference "
            "of source operand!");
      }
    } else {
      if (!machine->isFPRTopOfStack(sourceRegister)) {
...
TR::X86MemTableInstruction *generateMemTableInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
    ncount_t numEntries, TR::RegisterDependencyConditions *deps,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemTableInstruction(op, node, mr, numEntries, deps, cg);
}

TR::X86MemImmSymInstruction *generateMemImmSymInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, int32_t imm,
    TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86ImmInstruction *generateImmInstruction(
    TR::Instruction *prev, TR_X86OpCodes op, int32_t imm,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86PaddingInstruction *generatePaddingInstruction(
    TR::Instruction *precedingInstruction, uint8_t length,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86AlignmentInstruction *generateAlignmentInstruction(
    TR::Node *node, uint8_t boundary, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86AlignmentInstruction *generateAlignmentInstruction(
    TR::Instruction *precedingInstruction, uint8_t boundary,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86AlignmentInstruction *generateAlignmentInstruction(
    TR::Instruction *precedingInstruction, uint8_t boundary, uint8_t margin,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86LabelInstruction *generateLabelInstruction(
    TR_X86OpCodes opCode, TR::Node *node, TR::LabelSymbol *label,
    TR::Node *glRegDep, List<TR::Register> *popRegisters,
...
TR::X86LabelInstruction *generateConditionalJumpInstruction(
    TR_X86OpCodes opCode, TR::Node *ifNode, TR::CodeGenerator *cg,
    bool needsVMThreadRegister) {
  TR::Compilation *comp = cg->comp();
  TR::X86LabelInstruction *inst;
  TR::LabelSymbol *destinationLabel =
...
TR::X86VirtualGuardNOPInstruction *generateVirtualGuardNOPInstruction(
    TR::Node *node, TR_VirtualGuardSite *site,
...
TR::X86RegImmInstruction *generateRegImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, int32_t imm,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86RegImmInstruction *generateRegImmInstruction(
    TR::Instruction *instr, TR_X86OpCodes op, TR::Register *treg, int32_t imm,
...
TR::X86RegImmSymInstruction *generateRegImmSymInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, int32_t imm,
    TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86RegMemInstruction *generateRegMemInstruction(TR_X86OpCodes op,
                                                    TR::Node *node,
                                                    TR::Register *treg,
                                                    TR::MemoryReference *mr,
                                                    TR::CodeGenerator *cg) {
...
TR::X86RegMemInstruction *generateRegMemInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
    TR::MemoryReference *mr, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86RegRegInstruction *generateRegRegInstruction(TR_X86OpCodes op,
                                                    TR::Node *node,
                                                    TR::Register *treg,
                                                    TR::Register *sreg,
                                                    TR::CodeGenerator *cg) {
...
TR::X86MemImmInstruction *generateMemImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, int32_t imm,
    TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory())
...
TR::X86MemRegInstruction *generateMemRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, TR::Register *sreg, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86RegMemImmInstruction *generateRegMemImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *reg,
    TR::MemoryReference *mr, int32_t imm, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegMemImmInstruction(op, node, reg, mr, imm, cg);
}

TR::X86RegRegImmInstruction *generateRegRegImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    int32_t imm, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86ImmSymInstruction *generateHelperCallInstruction(
    TR::Node *node, TR_RuntimeHelper index,
...
TR::X86VFPSaveInstruction *generateVFPSaveInstruction(
...
TR::X86VFPRestoreInstruction *generateVFPRestoreInstruction(
    TR::X86VFPSaveInstruction *saveInstruction, TR::Node *node,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86VFPCallCleanupInstruction *generateVFPCallCleanupInstruction(
    TR::Instruction *precedingInstruction, int32_t adjustment,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86FPSTiST0RegRegInstruction *generateFPSTiST0RegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    TR::CodeGenerator *cg, bool forcePop) {
  return new (cg->trHeapMemory())
...
TR::X86FPCompareRegRegInstruction *generateFPCompareRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::X86FPReturnInstruction *generateFPReturnInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
...
TR::AMD64RegImm64Instruction *generateRegImm64Instruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, uint64_t imm,
    TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory())
...
TR::AMD64Imm64Instruction *generateImm64Instruction(
    TR_X86OpCodes op, TR::Node *node, uint64_t imm,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
