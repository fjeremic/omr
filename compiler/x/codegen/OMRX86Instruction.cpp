        ...
        if (getDependencyConditions()) {
            ...
            if (snippet && (devirtSnippet = snippet->getGuardedDevirtualSnippet())) {
                ...
                if (classObjectRegister != NULL && !classObjectRegister->getRealRegister()) {
                    ...
                    if (assignedClassObjectRegister == NULL && classObjectRegister->getTotalUseCount() == classObjectRegister->getFutureUseCount()) {
                        TR::Machine* machine = cg()->machine();
...
{
    if ((reg == getTargetRegister() && getOpCode().modifiesTarget()) || (reg == getSourceRegister() && getOpCode().modifiesSource())) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->defsRegister(reg);
    }
...
void TR::X86RegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    ...
    if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
        ...
        TR_RegisterSizes firstRequestedRegSize = getOpCode().hasByteTarget() ? TR_ByteReg : getOpCode().hasXMMTarget() ? TR_QuadWordReg : TR_WordReg;

        TR_RegisterSizes secondRequestedRegSize = getOpCode().hasByteSource() ? TR_ByteReg : getOpCode().hasXMMSource() ? TR_QuadWordReg : TR_WordReg;
...
{
    if ((reg == getTargetRegister() && getOpCode().usesTarget()) || reg == getSourceRegister() || reg == getSource2ndRegister()) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->usesRegister(reg);
    }
...
void TR::X86RegRegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    ...
    if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
        ...
        if (firstRegister->decFutureUseCount() == 0 && assignedFirstRegister->getState() != TR::RealRegister::Locked && firstRegister == getTargetRegister() && getOpCodeValue() != WRTBAR) {
            cg()->traceRegFreed(firstRegister, assignedFirstRegister);
            firstRegister->setAssignedRegister(NULL);
            assignedFirstRegister->setState(TR::RealRegister::Unlatched);
        }
        ...
        if (firstRegister->getFutureUseCount() == 0 && assignedFirstRegister->getState() != TR::RealRegister::Locked && getOpCodeValue() == WRTBAR) {
            cg()->traceRegFreed(firstRegister, assignedFirstRegister);
            firstRegister->setAssignedRegister(NULL);
...
void TR::X86MemRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    if ((cg()->getAssignmentDirection() == cg()->Backward)) {
        ...
        if (kindsToBeAssigned & getSourceRegister()->getKindAsMask()) {
            ...
            if ((getMemoryReference()->getBaseRegister() == cg()->getVMThreadRegister()) || (getMemoryReference()->getIndexRegister() == cg()->getVMThreadRegister())) {
                blockedEbp = true;
            ...
            //
            if (cg()->enableRematerialisation() && getSourceRegister()->isDiscardable() && getSourceRegister()->getRematerializationInfo()->getDefinition() == this) {
                if (debug("dumpRemat")) {
    ...
    , _memoryReference(mr)
{
    ...
    // a live discardable register.
    //
    if (cg->enableRematerialisation() && (getOpCodeValue() == LEA2RegMem || getOpCodeValue() == LEA4RegMem || getOpCodeValue() == LEA8RegMem) && !cg->getLiveDiscardableRegisters().empty()) {
        cg->clobberLiveDiscardableRegisters(this, mr);
    }
}
...
void TR::X86RegRegMemInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    if ((cg()->getAssignmentDirection() == cg()->Backward)) {
        ...
        TR_RegisterSizes requestedRegSize = getOpCode().hasByteTarget() ? TR_ByteReg : getOpCode().hasXMMTarget() ? TR_QuadWordReg : TR_WordReg;
        ...
        if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
            ...
            if (assignedTargetRegister != NULL) {
                ...
                if (getTargetRegister()->decFutureUseCount() == 0 && assignedTargetRegister->getState() != TR::RealRegister::Locked) {
                    cg()->traceRegFreed(getTargetRegister(), assignedTargetRegister);
                    getTargetRegister()->setAssignedRegister(NULL);
                    assignedTargetRegister->setState(TR::RealRegister::Unlatched);
                }

                if (getSource2ndRegister()->decFutureUseCount() == 0 && assignedSource2ndRegister->getState() != TR::RealRegister::Locked) {
...
bool TR::X86FPCompareRegRegInstruction::swapOperands()
{
    ...
    if ( // !debug("saveFXCHs")       ||
        ...
        swappedOp == TR::iffcmple || swappedOp == TR::ifdcmple || swappedOp == TR::fcmple || swappedOp == TR::dcmple || swappedOp == TR::iffcmpgtu || swappedOp == TR::ifdcmpgtu || swappedOp == TR::fcmpgtu || swappedOp == TR::dcmpgtu ||
        // the following cases lead to complicated tests if using FCOMI,
        // or lead to code growth (extra CMP) if using FCOM.
        swappedOp == TR::iffcmplt || swappedOp == TR::ifdcmplt || swappedOp == TR::fcmplt || swappedOp == TR::dcmplt || swappedOp == TR::iffcmpgeu || swappedOp == TR::ifdcmpgeu || swappedOp == TR::fcmpgeu || swappedOp == TR::dcmpgeu) {
        if (debug("dumpFPRA"))
    ...
    TR::CodeGenerator* cg)
{
    ...
    TR::X86LabelInstruction* instr = generateLabelInstruction(opCode,
        node,
        label,
    ...
    bool evaluateGlRegDeps)
{
    ...
    (jumpNode->getNumChildren() > 0) ? inst = generateLabelInstruction(opCode,
                                           jumpNode,
                                           destinationLabel,
