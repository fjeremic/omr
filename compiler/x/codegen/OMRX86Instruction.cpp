...
//
#define BACKWARD_BRANCH_BOUNDARY_SPACING                                       \
  (16) // K8 opt guide section 6.1; no indication that this helps on Intel
...
TR::RealRegister *assign8BitGPRegister(TR::Instruction *instr,
  ...
  if (candidate->getRegisterNumber() > TR::RealRegister::Last8BitGPR) {
    if ((candidate = machine->findBestFreeGPRegister(instr, virtReg,
                                                     TR_ByteReg)) == NULL) {
      cg->setRegisterAssignmentFlag(TR_RegisterSpilled);
      candidate = machine->freeBestGPRegister(instr, virtReg, TR_ByteReg);
    }

    machine->coerceGPRegisterAssignment(instr, virtReg,
                                        candidate->getRegisterNumber());
...
TR::RealRegister *assignGPRegister(TR::Instruction *instr,
  ...
  if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
    ...
    assignedRegister =
        machine->reverseGPRSpillState(instr, virtReg, NULL, requestedRegSize);
  }

  // If first use, totaluse and futureuse will be the same
  //
  else {
    if ((assignedRegister = machine->findBestFreeGPRegister(
             instr, virtReg, requestedRegSize, true))) {
      if (cg->enableBetterSpillPlacements())
        cg->removeBetterSpillPlacementCandidate(assignedRegister);
    } else {
      cg->setRegisterAssignmentFlag(TR_RegisterSpilled);
      assignedRegister =
          machine->freeBestGPRegister(instr, virtReg, requestedRegSize);
    }
  }
...
TR::X86LabelInstruction::X86LabelInstruction(TR_X86OpCodes op, TR::Node *node,
                                             TR::LabelSymbol *sym,
                                             TR::CodeGenerator *cg, bool b)
    : TR::Instruction(node, op, cg) {
  initialize(sym, b);
}

TR::X86LabelInstruction::X86LabelInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::LabelSymbol *sym, TR::CodeGenerator *cg, bool b)
    : TR::Instruction(op, precedingInstruction, cg) {
  initialize(sym, b);
}

TR::X86LabelInstruction::X86LabelInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg, bool b)
...
void TR::X86LabelInstruction::assignOutlinedInstructions(
  ...
  //
  TR_OutlinedInstructions *oi = cg()->findOutlinedInstructionsFromLabel(
      labelInstruction->getLabelSymbol());
  TR_ASSERT(oi,
            "Could not find OutlinedInstructions stream from label.  instr=%p, "
            "label=%p\n",
            labelInstruction, labelInstruction->getLabelSymbol());

  if (!oi->hasBeenRegisterAssigned())
    oi->assignRegisters(kindsToBeAssigned,
                        generateVFPSaveInstruction(getPrev(), cg()));
}

// Take the now-assigned register dependencies from this instruction and
// replicate them on the outlined instruction branch instruction.
//
void TR::X86LabelInstruction::addPostDepsToOutlinedInstructionsBranch() {
  TR::RegisterDependencyConditions *mergeDeps =
      getDependencyConditions()->clone(cg());
  ...
  for (int32_t i = 0; i < mergeDeps->getNumPostConditions(); i++) {
    ...
    //
    TR_ASSERT(dependency->getRealRegister() != TR::RealRegister::NoReg,
              "unassigned merge dep register");
#endif
  }
}

void TR::X86LabelInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  ...
  if (kindsToBeAssigned & TR_GPR_Mask) {
    if (getDependencyConditions()) {
      ...
      // ----------------------
      //
      getDependencyConditions()->assignPostConditionRegisters(
          this, kindsToBeAssigned, cg());

      if (getOpCodeValue() == LABEL &&
          getLabelSymbol()->isInternalControlFlowMerge()) {
        // This is the merge label of a non-linear internal control flow region.
        // Record the register assigner state before continuing on the main
        // path.
        //
        ...
        if (getLabelSymbol()->isNonLinear() &&
            cg()->getUseNonLinearRegisterAssigner()) {
          // This is a branch to an outlined instruction sequence.  Now perform
          // register assignment over it.
          //
          cg()->performNonLinearRegisterAssignmentAtBranch(this,
                                                           kindsToBeAssigned);
        } else {
        ...
        assignOutlinedInstructions(kindsToBeAssigned,
                                   _outlinedInstructionBranch);
      }
      ...
      // ---------------------
      //
      getDependencyConditions()->assignPreConditionRegisters(
          this, kindsToBeAssigned, cg());
    } else if (getOpCodeValue() != LABEL &&
               getLabelSymbol()->isStartOfColdInstructionStream()) {
      if (getLabelSymbol()->isNonLinear() &&
      ...
      // If this is a GuardedDevirtualSnippet, then must find out what real
      // register contains the class object reference.  This must be used by the
      // snippet to generate the full virtual call sequence.
      //
      if (snippet && (devirtSnippet = snippet->getGuardedDevirtualSnippet())) {
        TR::Register *classObjectRegister =
            devirtSnippet->getClassObjectRegister();
        if (classObjectRegister != NULL &&
            !classObjectRegister->getRealRegister()) {
          ...
          if (assignedClassObjectRegister == NULL &&
              classObjectRegister->getTotalUseCount() ==
                  classObjectRegister->getFutureUseCount()) {
            TR::Machine *machine = cg()->machine();

            cg()->clearRegisterAssignmentFlags();
            cg()->setRegisterAssignmentFlag(TR_NormalAssignment);

            if ((assignedClassObjectRegister = machine->findBestFreeGPRegister(
                     this, classObjectRegister, TR_WordReg)) != NULL) {
              machine->coerceGPRegisterAssignment(
                  this, classObjectRegister,
                  assignedClassObjectRegister->getRegisterNumber());

              // No use of this register here in the original stream.  So, need
              // to bump its total use count without bumping its future use
              // count so that it doesn't look like the first time a register is
              // seen.  That is make it fail the getTotalUseCount() ==
              // getFutureUseCount() test used to detect the first reference to
              // a register in the assignment direction
              //
    ...
    //
    if (getOpCode().isConditionalBranchOp() &&
        cg()->internalControlFlowNestingDepth() == 0) {
      if (cg()->enableBetterSpillPlacements())
    ...
    if (getDependencyConditions()) {
      ...
      getDependencyConditions()->assignPreConditionRegisters(
          this->getPrev(), kindsToBeAssigned, cg());
      getDependencyConditions()->assignPostConditionRegisters(
          this, kindsToBeAssigned, cg());
    }
  }
...
TR::X86FenceInstruction::X86FenceInstruction(TR_X86OpCodes op, TR::Node *node,
                                             TR::Node *fenceNode,
                                             TR::CodeGenerator *cg)
    : TR::Instruction(node, op, cg), _fenceNode(fenceNode) {}

TR::X86FenceInstruction::X86FenceInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Node *node,
    TR::CodeGenerator *cg)
...
TR::X86ImmInstruction::X86ImmInstruction(TR::Instruction *precedingInstruction,
                                         ...
                                         int32_t reloKind)
    : TR::Instruction(cond, op, precedingInstruction, cg),
      _sourceImmediate(imm), _adjustsFramePointerBy(0), _reloKind(reloKind) {
...
TR::X86ImmSnippetInstruction::X86ImmSnippetInstruction(
    TR_X86OpCodes op, TR::Node *node, int32_t imm,
    TR::UnresolvedDataSnippet *us, TR::CodeGenerator *cg)
    : TR::X86ImmInstruction(imm, node, op, cg), _unresolvedSnippet(us) {}

TR::X86ImmSnippetInstruction::X86ImmSnippetInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, int32_t imm,
    TR::UnresolvedDataSnippet *us, TR::CodeGenerator *cg)
...
TR::X86ImmSymInstruction::X86ImmSymInstruction(TR_X86OpCodes op, TR::Node *node,
                                               int32_t imm,
                                               TR::SymbolReference *sr,
                                               TR::CodeGenerator *cg)
    : TR::X86ImmInstruction(imm, node, op, cg), _symbolReference(sr) {}
...
TR::X86ImmSymInstruction::X86ImmSymInstruction(
    TR_X86OpCodes op, TR::Node *node, int32_t imm, TR::SymbolReference *sr,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
    : TR::X86ImmInstruction(cond, imm, node, op, cg), _symbolReference(sr) {}

TR::X86ImmSymInstruction::X86ImmSymInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, int32_t imm,
    TR::SymbolReference *sr, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg)
...
TR::X86RegInstruction::X86RegInstruction(TR_X86OpCodes op, TR::Node *node,
                                         TR::Register *reg,
                                         TR::CodeGenerator *cg)
  ...
  //
  if (cg->enableRematerialisation() && reg->isDiscardable() &&
      getOpCode().modifiesTarget()) {
    TR::ClobberingInstruction *clob = new (cg->trHeapMemory())
        TR::ClobberingInstruction(this, cg->trMemory());
...
void TR::X86RegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (getDependencyConditions()) {
  ...
  if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
    TR::RealRegister *assignedRegister =
        getTargetRegister()->getAssignedRealRegister();
    TR_RegisterSizes requestedRegSize =
        getOpCode().hasByteTarget() ? TR_ByteReg : TR_WordReg;

    if (assignedRegister == NULL) {
      assignedRegister =
          assignGPRegister(this, getTargetRegister(), requestedRegSize, cg());
...
TR::X86RegRegInstruction::X86RegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                               TR::Register *treg,
                                               TR::Register *sreg,
                                               TR::CodeGenerator *cg)
...
TR::X86RegRegInstruction::X86RegRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
...
TR::X86RegRegInstruction::X86RegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
...
bool TR::X86RegRegInstruction::usesRegister(TR::Register *reg) {
  if ((reg == getTargetRegister() && getOpCode().usesTarget()) ||
...
void TR::X86RegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (getDependencyConditions()) {
    if ((cg()->getAssignmentDirection() == cg()->Backward)) {
      getTargetRegister()->block();
  ...
  if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
    ...
    TR_RegisterSizes firstRequestedRegSize =
        getOpCode().hasByteTarget()
            ? TR_ByteReg
            : getOpCode().hasXMMTarget() ? TR_QuadWordReg : TR_WordReg;

    TR_RegisterSizes secondRequestedRegSize =
        getOpCode().hasByteSource()
            ? TR_ByteReg
    ...
    TR::RealRegister *assignedFirstRegister =
        firstRegister->getAssignedRealRegister();

    if (assignedFirstRegister == NULL) {
      assignedFirstRegister =
          assignGPRegister(this, firstRegister, firstRequestedRegSize, cg());
    ...
    TR::RealRegister *assignedSecondRegister =
        secondRegister->getAssignedRealRegister();
    ...
    if (assignedSecondRegister == NULL) {
      ...
      // If first use, totaluse and futureuse will be the same
      //
      if (secondRegister->getTotalUseCount() ==
          secondRegister->getFutureUseCount()) {
        if (regRegCopy &&
            assignedFirstRegister->getState() == TR::RealRegister::Unlatched) {
          assignedSecondRegister = assignedFirstRegister;
        } else if ((assignedSecondRegister = machine->findBestFreeGPRegister(
                        this, secondRegister, secondRequestedRegSize, true))) {
          if (cg()->enableBetterSpillPlacements())
            cg()->removeBetterSpillPlacementCandidate(
                toRealRegister(assignedSecondRegister));
        } else {
          cg()->setRegisterAssignmentFlag(TR_RegisterSpilled);
          assignedSecondRegister = machine->freeBestGPRegister(
              this, secondRegister, secondRequestedRegSize);
        }
      }
      ...
      //
        ...
        cg()->setRegisterAssignmentFlag(TR_RegisterReloaded);
        assignedSecondRegister = machine->reverseGPRSpillState(
            this, secondRegister, NULL, secondRequestedRegSize);
      }

      secondRegister->setAssignedRegister(assignedSecondRegister);
      secondRegister->setAssignedAsByteRegister(secondRequestedRegSize ==
                                                TR_ByteReg);
      assignedSecondRegister->setAssignedRegister(secondRegister);
      assignedSecondRegister->setState(TR::RealRegister::Assigned,
                                       secondRegister->isPlaceholderReg());
...
TR::X86RegImmInstruction::X86RegImmInstruction(TR_X86OpCodes op, TR::Node *node,
                                               TR::Register *treg, int32_t imm,
                                               TR::CodeGenerator *cg,
                                               int32_t reloKind)
    : TR::X86RegInstruction(treg, node, op, cg), _sourceImmediate(imm),
      _reloKind(reloKind) {}

TR::X86RegImmInstruction::X86RegImmInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
    int32_t imm, TR::CodeGenerator *cg, int32_t reloKind)
    : TR::X86RegInstruction(treg, op, precedingInstruction, cg),
      _sourceImmediate(imm), _reloKind(reloKind) {}

TR::X86RegImmInstruction::X86RegImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, int32_t imm,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg,
    int32_t reloKind)
    : TR::X86RegInstruction(cond, treg, node, op, cg), _sourceImmediate(imm),
      _reloKind(reloKind) {}
...
void TR::X86RegImmInstruction::adjustVFPState(TR_VFPState *state,
                                              TR::CodeGenerator *cg) {
  TR::RealRegister::RegNum targetReal =
      toRealRegister(toRealRegister(getTargetRegister()))->getRegisterNumber();
  if (state->_register == targetReal) {
    ...
    default:
      if (cg->getDebug())
        TR_ASSERT(
            0, "Can't compute VFP adjustment due to instruction %s: %s %s, %d",
            cg->getDebug()->getName(this),
            cg->getDebug()->getMnemonicName(&getOpCode()),
...
TR::X86RegImmSymInstruction::X86RegImmSymInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *reg, int32_t imm,
    TR::SymbolReference *sr, TR::CodeGenerator *cg)
...
TR::X86RegImmSymInstruction::X86RegImmSymInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *reg,
...
TR::X86RegRegImmInstruction::X86RegRegImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    int32_t imm, TR::CodeGenerator *cg)
...
TR::X86RegRegRegInstruction::X86RegRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *slreg,
    TR::Register *srreg, TR::CodeGenerator *cg)
    : TR::X86RegRegInstruction(srreg, treg, node, op, cg),
...
TR::X86RegRegRegInstruction::X86RegRegRegInstruction(
    ...
    : TR::X86RegRegInstruction(srreg, treg, op, precedingInstruction, cg),
      _source2ndRegister(slreg) {
  useRegister(slreg);
}
...
TR::X86RegRegRegInstruction::X86RegRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *slreg,
    TR::Register *srreg, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg)
    : TR::X86RegRegInstruction(cond, srreg, treg, node, op, cg),
      _source2ndRegister(slreg) {
  useRegister(slreg);
}
...
TR::X86RegRegRegInstruction::X86RegRegRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
    TR::Register *slreg, TR::Register *srreg,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
    : TR::X86RegRegInstruction(cond, srreg, treg, op, precedingInstruction, cg),
      _source2ndRegister(slreg) {
  useRegister(slreg);
}
...
bool TR::X86RegRegRegInstruction::usesRegister(TR::Register *reg) {
  if ((reg == getTargetRegister() && getOpCode().usesTarget()) ||
      reg == getSourceRegister() || reg == getSource2ndRegister()) {
    return true;
  } else if (getDependencyConditions()) {
    return getDependencyConditions()->usesRegister(reg);
  }
...
void TR::X86RegRegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if ((cg()->getAssignmentDirection() == cg()->Backward)) {
    if (getDependencyConditions()) {
      getTargetRegister()->block();
  ...
  if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
    ...
    if (firstRegister->decFutureUseCount() == 0 &&
        assignedFirstRegister->getState() != TR::RealRegister::Locked &&
        firstRegister == getTargetRegister() && getOpCodeValue() != WRTBAR) {
      cg()->traceRegFreed(firstRegister, assignedFirstRegister);
      firstRegister->setAssignedRegister(NULL);
      assignedFirstRegister->setState(TR::RealRegister::Unlatched);
    }
    ...
    if (assignedSecondRegister == NULL) {
      assignedSecondRegister =
          assignGPRegister(this, secondRegister, secondRequestedRegSize, cg());
    } else if (secondRequestedRegSize == TR_ByteReg) {
      assignedSecondRegister = assign8BitGPRegister(this, secondRegister, cg());
    }
    ...
    TR::RealRegister *assignedThirdRegister =
        thirdRegister->getAssignedRealRegister();

    if (assignedThirdRegister == NULL) {
      assignedThirdRegister =
          assignGPRegister(this, thirdRegister, thirdRequestedRegSize, cg());
...
void padUnresolvedReferenceInstruction(TR::Instruction *instr,
                                       TR::MemoryReference *mr,
                                       TR::CodeGenerator *cg) {
  mr->getUnresolvedDataSnippet()->setDataReferenceInstruction(instr);
  generateBoundaryAvoidanceInstruction(
      TR::X86BoundaryAvoidanceInstruction::unresolvedAtomicRegions, 8, 8, instr,
      cg);
}

void insertUnresolvedReferenceInstructionMemoryBarrier(
    TR::CodeGenerator *cg, int32_t barrier, TR::Instruction *inst,
    TR::MemoryReference *mr, TR::Register *srcReg,
    TR::MemoryReference *anotherMr) {
  ...
  if (is5ByteFence) {
    ...
    TR::RealRegister *espReal =
        cg->machine()->getRealRegister(TR::RealRegister::esp);
    TR::MemoryReference *espMemRef = generateX86MemoryReference(espReal, 0, cg);
    fenceInst = new (cg->trHeapMemory()) TR::X86MemImmInstruction(
        padInst, fenceOp.getOpCodeValue(), espMemRef, 0, cg);
  } else {
    // generate memory fence
    padInst = generateAlignmentInstruction(inst, 4, cg);
    fenceInst = new (cg->trHeapMemory())
        TR::Instruction(fenceOp.getOpCodeValue(), padInst, cg);
  }
...
TR::X86MemInstruction::X86MemInstruction(TR_X86OpCodes op, TR::Node *node,
                                         TR::MemoryReference *mr,
                                         TR::CodeGenerator *cg,
                                         TR::Register *sreg)
...
void TR::X86MemInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (getDependencyConditions()) {
  ...
  if (kindsToBeAssigned & (TR_X87_Mask | TR_FPR_Mask | TR_VRF_Mask)) {
    TR::UnresolvedDataSnippet *snippet =
        getMemoryReference()->getUnresolvedDataSnippet();
    if (snippet) {
      if (kindsToBeAssigned & TR_X87_Mask)
        snippet->setNumLiveX87Registers(
            cg()->machine()->fpGetNumberOfLiveFPRs());

      if (kindsToBeAssigned & (TR_FPR_Mask | TR_VRF_Mask))
        snippet->setHasLiveXMMRegisters(
            (cg()->machine()->fpGetNumberOfLiveXMMRs() > 0) ? true : false);
    }
  }
#endif
...
void TR::X86MemTableInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
...
TR::X86CallMemInstruction::X86CallMemInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
...
TR::X86CallMemInstruction::X86CallMemInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg)
...
void TR::X86CallMemInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if ((cg()->getAssignmentDirection() == cg()->Backward)) {
    if (getDependencyConditions()) {
      getMemoryReference()->blockRegisters();
      getDependencyConditions()->assignPostConditionRegisters(
          this, kindsToBeAssigned, cg());
      getMemoryReference()->unblockRegisters();
      getDependencyConditions()->blockPostConditionRealDependencyRegisters(
          cg());
    }
    getMemoryReference()->assignRegisters(this, cg());
    if (getDependencyConditions()) {
      getDependencyConditions()->unblockPostConditionRealDependencyRegisters(
          cg());
      getMemoryReference()->blockRegisters();
      getDependencyConditions()->assignPreConditionRegisters(
          this, kindsToBeAssigned, cg());
...
TR::X86MemImmInstruction::X86MemImmInstruction(TR_X86OpCodes op, TR::Node *node,
                                               TR::MemoryReference *mr,
                                               int32_t imm,
                                               TR::CodeGenerator *cg,
                                               int32_t reloKind)
    : TR::X86MemInstruction(mr, node, op, cg), _sourceImmediate(imm),
      _reloKind(reloKind) {}

TR::X86MemImmInstruction::X86MemImmInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, int32_t imm, TR::CodeGenerator *cg,
    int32_t reloKind)
...
TR::X86MemImmSymInstruction::X86MemImmSymInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr, int32_t imm,
    TR::SymbolReference *sr, TR::CodeGenerator *cg)
    : TR::X86MemImmInstruction(imm, mr, node, op, cg), _symbolReference(sr) {}

TR::X86MemImmSymInstruction::X86MemImmSymInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, int32_t imm, TR::SymbolReference *sr,
    TR::CodeGenerator *cg)
...
TR::X86MemRegInstruction::X86MemRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                               TR::MemoryReference *mr,
                                               TR::Register *sreg,
                                               TR::CodeGenerator *cg)
...
TR::X86MemRegInstruction::X86MemRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, TR::Register *sreg, TR::CodeGenerator *cg)
    : TR::X86MemInstruction(mr, op, precedingInstruction, cg, sreg),
      _sourceRegister(sreg) {
  useRegister(sreg);
}
...
TR::X86MemRegInstruction::X86MemRegInstruction(
    ...
    TR::Register *sreg, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg)
    : TR::X86MemInstruction(cond, mr, node, op, cg, sreg),
      _sourceRegister(sreg) {
  useRegister(sreg);
}
...
TR::X86MemRegInstruction::X86MemRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, TR::Register *sreg,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg)
    : TR::X86MemInstruction(cond, mr, op, precedingInstruction, cg, sreg),
      _sourceRegister(sreg) {
  useRegister(sreg);
}
...
void TR::X86MemRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if ((cg()->getAssignmentDirection() == cg()->Backward)) {
    if (getDependencyConditions()) {
      getSourceRegister()->block();
    ...
    if (kindsToBeAssigned & getSourceRegister()->getKindAsMask()) {
      ...
      if ((getMemoryReference()->getBaseRegister() ==
           cg()->getVMThreadRegister()) ||
          (getMemoryReference()->getIndexRegister() ==
           cg()->getVMThreadRegister())) {
        blockedEbp = true;
        oldState =
            cg()->machine()->getRealRegister(TR::RealRegister::ebp)->getState();
        cg()->machine()
      ...
      if (assignedRegister == NULL) {
        assignedRegister =
            assignGPRegister(this, getSourceRegister(), requestedRegSize, cg());
      } else if (requestedRegSize == TR_ByteReg) {
        assignedRegister =
            assign8BitGPRegister(this, getSourceRegister(), cg());
      }

      // If the source register became discardable because of this instruction,
      // reset its rematerializability before we allocate registers for this
      // instruction.
      //
      if (cg()->enableRematerialisation() &&
          getSourceRegister()->isDiscardable() &&
          getSourceRegister()->getRematerializationInfo()->getDefinition() ==
              this) {
        if (debug("dumpRemat")) {
          diagnostic("---> Deactivating %s discardable register %s at "
                     "instruction %p\n",
                     cg()->getDebug()->toString(
                         getSourceRegister()->getRematerializationInfo()),
      ...
      if (blockedEbp) {
        ...
        case TR::RealRegister::Free:
          cg()->machine()
              ->getRealRegister(TR::RealRegister::ebp)
              ->resetState(TR::RealRegister::Free);
          break;
        ...
        case TR::RealRegister::Assigned:
          cg()->machine()
              ->getRealRegister(TR::RealRegister::ebp)
              ->resetState(TR::RealRegister::Assigned);
          break;
        case TR::RealRegister::Blocked:
          cg()->machine()
              ->getRealRegister(TR::RealRegister::ebp)
              ->resetState(TR::RealRegister::Blocked);
          break;
        case TR::RealRegister::Locked:
          cg()->machine()
              ->getRealRegister(TR::RealRegister::ebp)
              ->resetState(TR::RealRegister::Locked);
          break;
        }
      }
...
#ifdef J9_PROJECT_SPECIFIC
    TR_ASSERT(kindsToBeAssigned & TR_X87_Mask,
              "not assigning FPRs in forward RA pass");
    TR::UnresolvedDataSnippet *snippet =
        getMemoryReference()->getUnresolvedDataSnippet();
    if (snippet)
...
TR::X86MemRegImmInstruction::X86MemRegImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::MemoryReference *mr,
    TR::Register *sreg, int32_t imm, TR::CodeGenerator *cg)
...
TR::X86MemRegImmInstruction::X86MemRegImmInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference *mr, TR::Register *sreg, int32_t imm,
    TR::CodeGenerator *cg)
...
TR::X86RegMemInstruction::X86RegMemInstruction(TR_X86OpCodes op, TR::Node *node,
                                               TR::Register *treg,
                                               TR::MemoryReference *mr,
                                               TR::CodeGenerator *cg)
  ...
  //
  if (cg->enableRematerialisation() &&
      (getOpCodeValue() == LEA2RegMem || getOpCodeValue() == LEA4RegMem ||
       getOpCodeValue() == LEA8RegMem) &&
      !cg->getLiveDiscardableRegisters().empty()) {
    cg->clobberLiveDiscardableRegisters(this, mr);
  }
}

TR::X86RegMemInstruction::X86RegMemInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
...
TR::X86RegMemInstruction::X86RegMemInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
    TR::MemoryReference *mr, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg)
...
void TR::X86RegMemInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (getDependencyConditions()) {
    if (cg()->getAssignmentDirection() == cg()->Backward) {
      getTargetRegister()->block();
...
TR::X86RegMemImmInstruction::X86RegMemImmInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
    TR::MemoryReference *mr, int32_t imm, TR::CodeGenerator *cg)
...
TR::X86RegRegMemInstruction::X86RegRegMemInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *slreg, TR::Register *srreg,
    TR::MemoryReference *mr, TR::CodeGenerator *cg)
    : TR::X86RegMemInstruction(op, node, slreg, mr, cg),
...
TR::X86RegRegMemInstruction::X86RegRegMemInstruction(
    ...
    TR::MemoryReference *mr, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg)
    : TR::X86RegMemInstruction(op, node, slreg, mr, cond, cg),
      _source2ndRegister(srreg) {
  useRegister(srreg);
}

bool TR::X86RegRegMemInstruction::refsRegister(TR::Register *reg) {
  if (getMemoryReference()->refsRegister(reg) || reg == getTargetRegister() ||
      reg == getSource2ndRegister()) {
    return true;
  } else if (getDependencyConditions()) {
    return getDependencyConditions()->refsRegister(reg);
...
void TR::X86RegRegMemInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if ((cg()->getAssignmentDirection() == cg()->Backward)) {
    if (getDependencyConditions()) {
      getMemoryReference()->blockRegisters();
    ...
    if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
      ...
      if (assignedTargetRegister == NULL) {
        assignedTargetRegister =
            assignGPRegister(this, getTargetRegister(), requestedRegSize, cg());
      } else if (requestedRegSize == TR_ByteReg) {
        assignedTargetRegister =
            assign8BitGPRegister(this, getTargetRegister(), cg());
      }

      getSource2ndRegister()->unblock();
      getTargetRegister()->block();

      assignedSource2ndRegister =
          getSource2ndRegister()->getAssignedRealRegister();
      if (assignedSource2ndRegister == NULL) {
        assignedSource2ndRegister = assignGPRegister(
            this, getSource2ndRegister(), requestedRegSize, cg());
      }
      ...
      if (assignedTargetRegister != NULL) {
        ...
        if (getSource2ndRegister()->decFutureUseCount() == 0 &&
          ...
          cg()->traceRegFreed(getSource2ndRegister(),
                              assignedSource2ndRegister);
...
TR::X86FPRegInstruction::X86FPRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                             TR::Register *reg,
                                             TR::CodeGenerator *cg)
...
void TR::X86FPRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (kindsToBeAssigned & TR_X87_Mask) {
    TR::Register *targetRegister = getTargetRegister();
    TR_X86FPStackRegister *assignedRegister =
        toX86FPStackRegister(targetRegister->getAssignedRealRegister());
    TR::Machine *machine = cg()->machine();
    ...
    TR::RealRegister *fpReg =
        machine->fpMapToStackRelativeRegister(targetRegister);
...
// General method for coercing the source and target operands onto the FP stack
// for an instruction.
//
uint32_t TR::X86FPRegRegInstruction::assignTargetSourceRegisters() {
  ...
  if (toX86FPStackRegister(sourceRegister->getAssignedRealRegister()) == NULL) {
    // The FP register is not on the FP stack
    //
    if (sourceRegister->getTotalUseCount() !=
        sourceRegister->getFutureUseCount()) {
      // FP register has been spilled from the stack
      //
...
void TR::X86FPST0ST1RegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack,
              "TR::X86FPST0STiRegRegInstruction::assignRegisters ==> source "
              "not on FP stack!");

    TR_ASSERT(result & kTargetOnFPStack,
              "TR::X86FPST0STiRegRegInstruction::assignRegisters ==> target "
              "not on FP stack!");

    machine->fpCoerceRegistersToTopOfStack(this->getPrev(), targetRegister,
                                           sourceRegister, true);
...
TR::X86FPSTiST0RegRegInstruction::X86FPSTiST0RegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
...
TR::X86FPSTiST0RegRegInstruction::X86FPSTiST0RegRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
    TR::Register *sreg, TR::CodeGenerator *cg, bool forcePop)
    : TR::X86FPRegRegInstruction(sreg, treg, op, precedingInstruction, cg) {
  _forcePop = forcePop;
}

void TR::X86FPSTiST0RegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack,
              "TR::X86FPSTiST0RegRegInstruction::assignRegisters ==> source "
              "not on FP stack!");

    TR_ASSERT(result & kTargetOnFPStack,
              "TR::X86FPSTiST0RegRegInstruction::assignRegisters ==> target "
              "not on FP stack!");

    TR_ASSERT((result & (kSourceCanBePopped | kTargetCanBePopped)) !=
                  (kSourceCanBePopped | kTargetCanBePopped),
              "TR::X86FPSTiST0RegRegInstruction::assignRegisters ==> both "
              "source and target cannot be popped!");

    // Safety valve: we currently don't support stores into an arbitrary FP
    // stack register without a pop.
    //
    // TR_ASSERT( result & (kSourceCanBePopped | kTargetCanBePopped),
    //        "TR::X86FPSTiST0RegRegInstruction::assignRegisters ==> at least
    //        one of source and target must be popped!");
    ...
    if (_forcePop || ((result & (kSourceCanBePopped | kTargetCanBePopped)))) {
      // If the target can be popped then the source and target must be the same
      // virtual register; above not necessarily true with new global FP
      // register assignment
      //
      if ((_forcePop || (result & kTargetCanBePopped)) &&
        ...
        TR::RealRegister *fpReg =
            machine->fpMapToStackRelativeRegister(targetRegister);
        new (cg()->trHeapMemory())
            TR::X86FPRegInstruction(cursor, FSTPReg, fpReg, cg());
      } else
      // if (result & kSourceCanBePopped)
      {
        TR_X86OpCodes popOpCode =
            machine->fpDeterminePopOpCode(getOpCodeValue());
...
void TR::X86FPST0STiRegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    if (!(result & kTargetOnFPStack)) {
      // First def of target.  Note that the register is not pushed onto the
      // stack here because it only appears on the stack after the instruction
      // executes.
      //
      lateTargetPush = true;
      sourceRegister->block();
    ...
    if (result & kSourceCanBePopped) {
      ...
      if (sourceRegister != targetRegister) {
        ...
        TR::RealRegister *fpReg =
            machine->fpMapToStackRelativeRegister(sourceRegister);
        new (cg()->trHeapMemory())
            TR::X86FPRegInstruction(cursor, FSTPReg, fpReg, cg());
        machine->fpStackPop();
      }
...
void TR::X86FPArithmeticRegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack,
              "TR::X86FPArithmeticRegRegInstruction::assignRegisters ==> "
              "source not on FP stack!");

    TR_ASSERT(result & kTargetOnFPStack,
              "TR::X86FPArithmeticRegRegInstruction::assignRegisters ==> "
              "target not on FP stack!");

    // Dead tree elimination doesn't always eliminate dead trees and we can
    // eventually pop the target as well, TODO: ensure proper stack balance.
    // TR_ASSERT( !(result & kTargetCanBePopped),
    //        "TR::X86FPArithmeticRegRegInstruction::assignRegisters ==> target
    //        register cannot be popped!" );
    ...
    if (result & kSourceCanBePopped) {
      ...
      if (!machine->isFPRTopOfStack(sourceRegister) &&
        ...
        // The target operand is already at TOS: generate a reverse and pop
        // instruction.
        //
        TR_X86OpCodes reverseOpCode =
            machine->fpDetermineReverseOpCode(getOpCodeValue());
        popOpCode = machine->fpDeterminePopOpCode(reverseOpCode);

        (void)machine->fpStackFXCH(this->getPrev(), sourceRegister, false);

        // don't exchange operands - they're virtual registers; the StackFXCH
        // does the exchange
      } else {
...
TR::X86FPCompareRegRegInstruction::X86FPCompareRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    TR::CodeGenerator *cg)
    : TR::X86FPRegRegInstruction(sreg, treg, node, op, cg) {}
...
void TR::X86FPCompareRegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    TR_ASSERT(result & kSourceOnFPStack,
              "TR::X86FPCompareRegRegInstruction::assignRegisters ==> source "
              "not on FP stack!");

    TR_ASSERT(result & kTargetOnFPStack,
              "TR::X86FPCompareRegRegInstruction::assignRegisters ==> target "
              "not on FP stack!");
    ...
    if (result & (kSourceCanBePopped | kTargetCanBePopped)) {
      ...
      if ((result & (kSourceCanBePopped | kTargetCanBePopped)) ==
        ...
        if (getOpCodeValue() == FCOMIRegReg ||
            getOpCodeValue() == DCOMIRegReg) {
          popOpCode = FCOMIPReg;
        ...
        //
        TR_ASSERT(0, "TR::X86FPCompareRegRegInstruction::assignRegisters() ==> "
                     "source operand should not be dead after compare!");
      }

      setOpCodeValue(popOpCode);
    } else {
      // Both operands are live after the compare.  This should ONLY occur in
      // the presence of a global register allocator.
      //
      if (machine->isFPRTopOfStack(sourceRegister) &&
          (sourceRegister != targetRegister)) {
        swapOperands();
    ...
    if (result & kSourceCanBePopped) {
      if (needLateSourcePop) {
        ...
        TR::RealRegister *realFPReg =
            machine->fpMapToStackRelativeRegister(sourceRegister);
        new (cg()->trHeapMemory())
            TR::X86FPRegInstruction(cursor, FSTPReg, realFPReg, cg());
      }
...
bool TR::X86FPCompareRegRegInstruction::swapOperands() {
  ...
  if ( // !debug("saveFXCHs")       ||
      ...
      swappedOp == TR::iffcmple || swappedOp == TR::ifdcmple ||
      swappedOp == TR::fcmple || swappedOp == TR::dcmple ||
      swappedOp == TR::iffcmpgtu || swappedOp == TR::ifdcmpgtu ||
      swappedOp == TR::fcmpgtu || swappedOp == TR::dcmpgtu ||
      // the following cases lead to complicated tests if using FCOMI,
      // or lead to code growth (extra CMP) if using FCOM.
      swappedOp == TR::iffcmplt || swappedOp == TR::ifdcmplt ||
      swappedOp == TR::fcmplt || swappedOp == TR::dcmplt ||
      swappedOp == TR::iffcmpgeu || swappedOp == TR::ifdcmpgeu ||
      swappedOp == TR::fcmpgeu || swappedOp == TR::dcmpgeu) {
    if (debug("dumpFPRA"))
  ...
  if (cursor != NULL) {
    ...
    instr = getBranchOrSetOpCodeForFPComparison(
        swappedOp, (instr == FCOMIRegReg || instr == DCOMIRegReg));
    cursor->setOpCodeValue(instr);

    if (debug("dumpFPRA"))
      diagnostic("%s\n", cursor->getOpCode().getOpCodeName(cg()));
  } else {
    TR_ASSERT(0, "\nTR::X86FPCompareRegRegInstruction::swapOperands() ==> "
                 "branch or set instruction not found for FP comparison!\n");
  }
...
TR::X86FPCompareEvalInstruction::X86FPCompareEvalInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *accRegister,
    TR::CodeGenerator *cg)
...
void TR::X86FPCompareEvalInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  TR::Instruction *cursor = this;
  TR::Node *node = getNode();
  ...
  if (kindsToBeAssigned & TR_GPR_Mask) {
    TR_ASSERT(_accRegister != NULL,
              "TR::X86FPCompareEvalInstruction::assignRegisters() ==> "
              "_accRegister is NULL\n");

    // eax should have been assigned to _accRegister at this point.
    //
    TR::Register *accRegister = _accRegister->getAssignedRegister();
    TR_ASSERT(accRegister != NULL,
              "TR::X86FPCompareEvalInstruction::assignRegisters() ==> "
              "_accRegister has not been assigned!\n");
    ...
    case TR::dcmpgt:
    case TR::fcmpgt:
      cursor = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(
          cursor, AND2RegImm2, accRegister, 0x4500, cg());
      break;
    ...
    case TR::dcmpgtu:
    case TR::fcmpgtu:
      TR_ASSERT(0, "TR::X86FPCompareEvalRegInstruction::assignRegisters() ==> "
                   "shouldn't be able to generate this condition!\n");
      break;
    ...
    case TR::fcmpge:
      cursor = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(
          cursor, AND2RegImm2, accRegister, 0x0500, cg());
      break;
    ...
    case TR::fcmpgeu:
      cursor = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(
          cursor, AND2RegImm2, accRegister, 0x4500, cg());
      cursor = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(
          cursor, CMP2RegImm2, accRegister, 0x0100, cg());
      break;
    ...
    case TR::fcmpeq:
      cursor = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(
          cursor, AND2RegImm2, accRegister, 0x4500, cg());
      cursor = new (cg()->trHeapMemory()) TR::X86RegImmInstruction(
          cursor, CMP2RegImm2, accRegister, 0x4000, cg());
      break;
    ...
    default:
      TR_ASSERT(0,
                "TR::X86FPCompareEvalRegInstruction::assignRegisters() ==> "
                "invalid comparison op: %d\n",
                cmpOp);
      break;
    }
...
TR_X86OpCodes getBranchOrSetOpCodeForFPComparison(TR::ILOpCodes cmpOp,
  ...
  case TR::fcmpg:
  case TR::dcmpg:
    TR_ASSERT(0, "getBranchOrSetOpCodeForFPComparison() ==> comparison op is "
                 "neither branch nor set\n");
    break;
  ...
  case TR::fcmpgtu:
  case TR::dcmpgtu:
    TR_ASSERT(0, "getBranchOrSetOpCodeForFPComparison() ==> comparison op "
  ...
  default:
    TR_ASSERT(
        0,
        "getBranchOrSetOpCodeForFPComparison() ==> invalid comparison op: %d\n",
        cmpOp);
  }
...
TR::X86FPRemainderRegRegInstruction::X86FPRemainderRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    TR::Register *accReg, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg)
...
TR::X86FPRemainderRegRegInstruction::X86FPRemainderRegRegInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *treg,
    TR::Register *sreg, TR::CodeGenerator *cg)
    : TR::X86FPST0ST1RegRegInstruction(precedingInstruction, op, treg, sreg,
                                       cg) {}

void TR::X86FPRemainderRegRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {

  if (kindsToBeAssigned &
      TR_GPR_Mask) // TODO: Move this code generation in FPTreeEvaluator.cpp
                   // rather than doing it here
  {
    OMR::X86::Instruction::assignRegisters(kindsToBeAssigned);

    TR::RealRegister *accReg =
        toRealRegister(_accRegister->getAssignedRegister());
    TR::LabelSymbol *loopLabel =
        TR::LabelSymbol::create(cg()->trHeapMemory(), cg());
    TR::RegisterDependencyConditions *deps = getDependencyConditions();

    new (cg()->trHeapMemory())
        TR::X86LabelInstruction(getPrev(), LABEL, loopLabel, cg());
    TR::Instruction *cursor = new (cg()->trHeapMemory())
        TR::X86RegInstruction(this, STSWAcc, accReg, cg());
    cursor = new (cg()->trHeapMemory())
        TR::X86RegImmInstruction(cursor, TEST2RegImm2, accReg, 0x0400, cg());
    new (cg()->trHeapMemory())
        TR::X86LabelInstruction(cursor, JNE4, loopLabel, deps, cg());
...
void TR::X86FPMemRegInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (kindsToBeAssigned & TR_GPR_Mask) {
    getMemoryReference()->assignRegisters(this, cg());
  }
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    TR::Register *sourceRegister = getSourceRegister();
    TR_X86FPStackRegister *assignedRegister =
        toX86FPStackRegister(sourceRegister->getAssignedRealRegister());
    TR::Machine *machine = cg()->machine();
    ...
    if (assignedRegister == NULL) {
      ...
      if (sourceRegister->getTotalUseCount() !=
        ...
        diagnostic("TR::X86FPMemRegInstruction::assignRegisters ==> first "
                   "reference of source operand!");
      }
    } else {
      if (!machine->isFPRTopOfStack(sourceRegister)) {
        (void)machine->fpStackFXCH(this->getPrev(), sourceRegister);
    ...
    // If last use, use pop form of instruction and pop the register from the FP
    // stack.
    //
    if (sourceRegister->decFutureUseCount() == 0) {
...
void TR::X86FPRegMemInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (kindsToBeAssigned & TR_GPR_Mask) {
    getMemoryReference()->assignRegisters(this, cg());
  }
  ...
  if (kindsToBeAssigned & TR_X87_Mask) {
    ...
    if (targetRegister->decFutureUseCount() == 0) {
      if (getOpCodeValue() == FLDRegMem || getOpCodeValue() == DLDRegMem) {
        // Temporary patch
        //
        // If the target register is not used, pop it off the FP stack.  This
        // can happen after the optimizer runs and eliminates unnecessary stores
        // of unresolved data.
        //
        new (cg()->trHeapMemory())
            TR::X86FPRegInstruction(this, FSTPReg, fpReg, cg());

        // Only two uses were added after it was determined the future use count
        // was zero.
        //
...
void TR::X86VFPDedicateInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
    TR_ASSERT(cg()->getAssignmentDirection() == TR::CodeGenerator::Backward,
              "Can't handle frame pointer registers that are assigned in the "
              "forward direction");
...
void TR::X86VFPReleaseInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (kindsToBeAssigned &
      _dedicateInstruction->getTargetRegister()->getKindAsMask()) {
    TR_ASSERT(cg()->getAssignmentDirection() == TR::CodeGenerator::Backward,
              "Can't handle frame pointer registers that are assigned in the "
              "forward direction");
    ...
    //
    toRealRegister(_dedicateInstruction->getTargetRegister())
        ->setState(TR::RealRegister::Locked);
  }
}

TR_AtomicRegion
    TR::X86PatchableCodeAlignmentInstruction::spinLoopAtomicRegions[] = {
        {0x00, 2}, // Spin loops just need two atomically-patchable bytes
        {0, 0}};

TR_AtomicRegion
    TR::X86PatchableCodeAlignmentInstruction::CALLImm4AtomicRegions[] = {
        // We could use 4 here to align just the displacement, but instead we
        // align
...
void TR::X86BoundaryAvoidanceInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
...
int32_t TR::X86BoundaryAvoidanceInstruction::betterPadLength(
    int32_t oldPadLength, const TR_AtomicRegion *unaccommodatedRegion,
    int32_t unaccommodatedRegionStart) {
  return oldPadLength +
         (int32_t)cg()->alignment(unaccommodatedRegionStart,
                                  _boundarySpacing); // cast explicitly
}
...
TR::AMD64RegImm64SymInstruction::AMD64RegImm64SymInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *reg, uint64_t imm,
    TR::SymbolReference *sr, TR::CodeGenerator *cg)
...
void TR::AMD64RegImm64SymInstruction::autoSetReloKind() {
  ...
  if (symbol->isDebugCounter())
    ...
    setReloKind(TR_ConstantPool);
  else if (symbol->isStatic() && !getSymbolReference()->isUnresolved() &&
           !symbol->isClassObject() && !symbol->isNotDataAddress())
    setReloKind(TR_DataAddress);
  else
    setReloKind(-1);
}
...
TR::Instruction *generateInstruction(TR::Instruction *prev, TR_X86OpCodes op,
                                     TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::Instruction(op, prev, cg);
}

TR::Instruction *generateInstruction(TR_X86OpCodes op, TR::Node *node,
                                     TR::CodeGenerator *cg) {
...
TR::X86MemInstruction *generateMemInstruction(TR_X86OpCodes op, TR::Node *node,
                                              TR::MemoryReference *mr,
                                              TR::CodeGenerator *cg) {
...
generateMemInstruction(TR::Instruction *precedingInstruction, TR_X86OpCodes op,
                       TR::MemoryReference *mr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemInstruction(precedingInstruction, op, mr, cg);
}
...
TR::X86MemTableInstruction *generateMemTableInstruction(TR_X86OpCodes op,
                                                        TR::Node *node,
                                                        TR::MemoryReference *mr,
                                                        ncount_t numEntries,
                                                        TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemTableInstruction(op, node, mr, numEntries, cg);
}

TR::X86MemTableInstruction *
generateMemTableInstruction(TR_X86OpCodes op, TR::Node *node,
                            TR::MemoryReference *mr, ncount_t numEntries,
                            TR::RegisterDependencyConditions *deps,
                            TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemTableInstruction(op, node, mr, numEntries, deps, cg);
}

TR::X86MemImmSymInstruction *
generateMemImmSymInstruction(TR_X86OpCodes op, TR::Node *node,
                             TR::MemoryReference *mr, int32_t imm,
                             TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemImmSymInstruction(op, node, mr, imm, sr, cg);
}

TR::X86ImmInstruction *generateImmInstruction(TR_X86OpCodes op, TR::Node *node,
                                              int32_t imm,
                                              TR::CodeGenerator *cg) {
...
TR::X86ImmInstruction *generateImmInstruction(TR_X86OpCodes op, TR::Node *node,
                                              ...
                                              int32_t reloKind) {
  return new (cg->trHeapMemory())
      TR::X86ImmInstruction(op, node, imm, cg, reloKind);
}

TR::X86ImmInstruction *
generateImmInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm,
                       TR::RegisterDependencyConditions *cond,
                       TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86ImmInstruction(op, node, imm, cond, cg);
}

TR::X86ImmInstruction *generateImmInstruction(TR::Instruction *prev,
                                              TR_X86OpCodes op, int32_t imm,
                                              TR::CodeGenerator *cg) {
...
TR::X86ImmInstruction *
generateImmInstruction(TR::Instruction *prev, TR_X86OpCodes op, int32_t imm,
                       TR::RegisterDependencyConditions *cond,
                       TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86ImmInstruction(prev, op, imm, cond, cg);
}

TR::X86RegInstruction *generateRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                              TR::Register *treg,
                                              TR::CodeGenerator *cg) {
...
generateRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                       TR::RegisterDependencyConditions *cond,
                       TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegInstruction(op, node, treg, cond, cg);
}

TR::X86RegInstruction *generateRegInstruction(TR::Instruction *prev,
                                              TR_X86OpCodes op,
                                              TR::Register *reg1,
                                              TR::CodeGenerator *cg) {
...
TR::X86BoundaryAvoidanceInstruction *generateBoundaryAvoidanceInstruction(
    const TR_AtomicRegion *atomicRegions, uint8_t boundarySpacing,
    uint8_t maxPadding, TR::Instruction *targetCode, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86BoundaryAvoidanceInstruction(
      atomicRegions, boundarySpacing, maxPadding, targetCode, cg);
}
...
generatePatchableCodeAlignmentInstruction(const TR_AtomicRegion *atomicRegions,
                                          ...
                                          TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86PatchableCodeAlignmentInstruction(
      atomicRegions, patchableCode, cg);
}

TR::X86PatchableCodeAlignmentInstruction *
generatePatchableCodeAlignmentInstructionWithProtectiveNop(
    const TR_AtomicRegion *atomicRegions, TR::Instruction *patchableCode,
    int32_t sizeOfProtectiveNop, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86PatchableCodeAlignmentInstruction(
      atomicRegions, patchableCode, sizeOfProtectiveNop, cg);
}

TR::X86PatchableCodeAlignmentInstruction *
generatePatchableCodeAlignmentInstruction(TR::Instruction *prev,
                                          const TR_AtomicRegion *atomicRegions,
                                          TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86PatchableCodeAlignmentInstruction(prev, atomicRegions, cg);
}

TR::X86PaddingInstruction *generatePaddingInstruction(uint8_t length,
                                                      TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86PaddingInstruction(length, node, cg);
}

TR::X86PaddingInstruction *
generatePaddingInstruction(TR::Instruction *precedingInstruction,
                           uint8_t length, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86PaddingInstruction(precedingInstruction, length, cg);
}

TR::X86PaddingSnippetInstruction *
generatePaddingSnippetInstruction(uint8_t length, TR::Node *node,
                                  TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86PaddingSnippetInstruction(length, node, cg);
}

TR::X86AlignmentInstruction *
generateAlignmentInstruction(TR::Node *node, uint8_t boundary,
                             TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86AlignmentInstruction(node, boundary, cg);
}

TR::X86AlignmentInstruction *
generateAlignmentInstruction(TR::Node *node, uint8_t boundary, uint8_t margin,
                             TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86AlignmentInstruction(node, boundary, margin, cg);
}

TR::X86AlignmentInstruction *
generateAlignmentInstruction(TR::Instruction *precedingInstruction,
                             uint8_t boundary, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86AlignmentInstruction(precedingInstruction, boundary, cg);
}

TR::X86AlignmentInstruction *
generateAlignmentInstruction(TR::Instruction *precedingInstruction,
                             uint8_t boundary, uint8_t margin,
                             TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86AlignmentInstruction(precedingInstruction, boundary, margin, cg);
}

TR::X86LabelInstruction *generateLabelInstruction(TR_X86OpCodes op,
                                                  TR::Node *node,
                                                  TR::LabelSymbol *sym,
                                                  TR::CodeGenerator *cg) {
...
TR::X86LabelInstruction *generateLabelInstruction(TR::Instruction *pred,
                                                  TR_X86OpCodes op,
                                                  TR::LabelSymbol *sym,
                                                  TR::CodeGenerator *cg) {
...
generateLabelInstruction(TR_X86OpCodes op, TR::Node *node, TR::LabelSymbol *sym,
                         TR::RegisterDependencyConditions *cond,
                         TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86LabelInstruction(op, node, sym, cond, cg);
}

TR::X86LabelInstruction *generateLabelInstruction(
    TR::Instruction *prev, TR_X86OpCodes op, TR::LabelSymbol *sym,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86LabelInstruction(prev, op, sym, cond, cg);
}

TR::X86LabelInstruction *generateLongLabelInstruction(TR_X86OpCodes op,
                                                      TR::Node *node,
                                                      TR::LabelSymbol *sym,
                                                      TR::CodeGenerator *cg) {
  TR::X86LabelInstruction *toReturn =
      new (cg->trHeapMemory()) TR::X86LabelInstruction(op, node, sym, cg);
...
TR::X86LabelInstruction *generateLongLabelInstruction(
    ...
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  TR::X86LabelInstruction *toReturn =
      new (cg->trHeapMemory()) TR::X86LabelInstruction(op, node, sym, cond, cg);
  toReturn->prohibitShortening();
  return toReturn;
}

TR::X86LabelInstruction *
generateLabelInstruction(TR_X86OpCodes opCode, TR::Node *node,
                         TR::LabelSymbol *label, TR::Node *glRegDep,
  ...
  TR::X86LabelInstruction *instr = generateLabelInstruction(
      opCode, node, label,
      generateRegisterDependencyConditions(glRegDep, cg, 0, popRegisters), cg);

  return instr;
}

static TR_AtomicRegion longBranchAtomicRegions[] = {
    {0x0, 6}, // The entire long branch instruction; TODO: JMP instructions are
              // only 5 bytes
    {0, 0}};

TR::X86LabelInstruction *generateJumpInstruction(TR_X86OpCodes opCode,
                                                 TR::Node *jumpNode,
                                                 TR::CodeGenerator *cg,
                                                 bool needsVMThreadRegister,
                                                 bool evaluateGlRegDeps) {
  TR::LabelSymbol *destinationLabel =
      jumpNode->getBranchDestination()->getNode()->getLabel();
  TR::X86LabelInstruction *inst;

  (jumpNode->getNumChildren() > 0)
      ? inst = generateLabelInstruction(opCode, jumpNode, destinationLabel,
                                        jumpNode->getFirstChild(), 0,
                                        evaluateGlRegDeps, cg)
...
TR::X86LabelInstruction *
generateConditionalJumpInstruction(TR_X86OpCodes opCode, TR::Node *ifNode,
                                   TR::CodeGenerator *cg,
                                   bool needsVMThreadRegister) {
  TR::Compilation *comp = cg->comp();
  TR::X86LabelInstruction *inst;
  TR::LabelSymbol *destinationLabel =
      ifNode->getBranchDestination()->getNode()->getLabel();

  if (ifNode->getNumChildren() == 3) {
    List<TR::Register> popRegisters(cg->trMemory());
    TR::Node *glRegDep = ifNode->getChild(2);
    inst = generateLabelInstruction(opCode, ifNode, destinationLabel, glRegDep,
                                    &popRegisters, cg);
    ...
    if (!popRegisters.isEmpty()) {
      ...
      for (TR::Register *popRegister = popRegsIt.getFirst();
           popRegister != NULL; popRegister = popRegsIt.getNext()) {
        generateFPSTiST0RegRegInstruction(FSTRegReg, ifNode, popRegister,
                                          popRegister, cg);
...
TR::X86FenceInstruction *generateFenceInstruction(TR_X86OpCodes op,
                                                  TR::Node *node,
                                                  TR::Node *fenceNode,
                                                  TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FenceInstruction(op, node, fenceNode, cg);
}

#ifdef J9_PROJECT_SPECIFIC
TR::X86VirtualGuardNOPInstruction *
generateVirtualGuardNOPInstruction(TR::Node *node, TR_VirtualGuardSite *site,
                                   TR::RegisterDependencyConditions *deps,
                                   TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VirtualGuardNOPInstruction(VirtualGuardNOP, node, site, deps, cg);
}
...
generateVirtualGuardNOPInstruction(TR::Node *node, TR_VirtualGuardSite *site,
                                   ...
                                   TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86VirtualGuardNOPInstruction(
      VirtualGuardNOP, node, site, deps, cg, label);
}

TR::X86VirtualGuardNOPInstruction *generateVirtualGuardNOPInstruction(
    TR::Instruction *i, TR::Node *node, TR_VirtualGuardSite *site,
    TR::RegisterDependencyConditions *deps, TR::LabelSymbol *label,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86VirtualGuardNOPInstruction(
      i, VirtualGuardNOP, node, site, deps, cg, label);
}
...
bool TR::X86VirtualGuardNOPInstruction::defsRegister(TR::Register *reg) {
  return usesRegister(reg);
}
bool TR::X86VirtualGuardNOPInstruction::refsRegister(TR::Register *reg) {
  return usesRegister(reg);
}
#endif

TR::X86RegImmInstruction *
generateRegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                          int32_t imm, TR::RegisterDependencyConditions *cond,
                          TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegImmInstruction(op, node, treg, imm, cond, cg);
}

TR::X86RegImmInstruction *
generateRegImmInstruction(TR::Instruction *instr, TR_X86OpCodes op,
                          TR::Register *treg, int32_t imm,
                          TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory())
      TR::X86RegImmInstruction(instr, op, treg, imm, cg, reloKind);
}
...
generateRegImmInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                          ...
                          int32_t reloKind) {
  return new (cg->trHeapMemory())
      TR::X86RegImmInstruction(op, node, treg, imm, cg, reloKind);
}

TR::X86RegImmSymInstruction *
generateRegImmSymInstruction(TR_X86OpCodes op, TR::Node *node,
                             TR::Register *treg, int32_t imm,
                             TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegImmSymInstruction(op, node, treg, imm, sr, cg);
}

TR::X86RegMemInstruction *
generateRegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                          TR::MemoryReference *mr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegMemInstruction(op, node, treg, mr, cg);
}
...
generateRegMemInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                          ...
                          TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegMemInstruction(op, node, treg, mr, cond, cg);
}

TR::X86RegMemInstruction *generateRegMemInstruction(TR::Instruction *instr,
                                                    TR_X86OpCodes op,
                                                    TR::Register *treg,
                                                    TR::MemoryReference *mr,
                                                    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegMemInstruction(instr, op, treg, mr, cg);
}

TR::X86RegRegInstruction *
generateRegRegInstruction(TR_X86OpCodes op, TR::Node *node, TR::Register *treg,
                          TR::Register *sreg, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegInstruction(op, node, treg, sreg, cg);
}

TR::X86RegRegInstruction *generateRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *treg, TR::Register *sreg,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegInstruction(op, node, treg, sreg, cond, cg);
...
TR::X86RegRegInstruction *generateRegRegInstruction(TR::Instruction *instr,
                                                    TR_X86OpCodes op,
                                                    TR::Register *treg,
                                                    TR::Register *sreg,
                                                    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegInstruction(instr, op, treg, sreg, cg);
}

TR::X86RegRegRegInstruction *
generateRegRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                             TR::Register *reg1, TR::Register *reg2,
                             TR::Register *reg3, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegRegInstruction(op, node, reg1, reg2, reg3, cg);
}

TR::X86RegRegRegInstruction *generateRegRegRegInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *reg1, TR::Register *reg2,
    TR::Register *reg3, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegRegInstruction(op, node, reg1, reg2, reg3, cond, cg);
}

TR::X86RegRegMemInstruction *
generateRegRegMemInstruction(TR_X86OpCodes op, TR::Node *node,
                             TR::Register *reg1, TR::Register *reg2,
                             TR::MemoryReference *mr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegMemInstruction(op, node, reg1, reg2, mr, cg);
}

TR::X86RegRegMemInstruction *generateRegRegMemInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *reg1, TR::Register *reg2,
    TR::MemoryReference *mr, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegMemInstruction(op, node, reg1, reg2, mr, cond, cg);
}

TR::X86MemImmInstruction *
generateMemImmInstruction(TR_X86OpCodes op, TR::Node *node,
                          TR::MemoryReference *mr, int32_t imm,
                          TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory())
      TR::X86MemImmInstruction(op, node, mr, imm, cg, reloKind);
}

TR::X86MemImmInstruction *
generateMemImmInstruction(TR::Instruction *precedingInstruction,
                          TR_X86OpCodes op, TR::MemoryReference *mr,
                          int32_t imm, TR::CodeGenerator *cg,
                          int32_t reloKind) {
  return new (cg->trHeapMemory())
      TR::X86MemImmInstruction(precedingInstruction, op, mr, imm, cg, reloKind);
}

TR::X86MemRegInstruction *
generateMemRegInstruction(TR::Instruction *precedingInstruction,
                          TR_X86OpCodes op, TR::MemoryReference *mr,
                          TR::Register *sreg, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemRegInstruction(precedingInstruction, op, mr, sreg, cg);
}

TR::X86MemRegInstruction *generateMemRegInstruction(TR_X86OpCodes op,
                                                    TR::Node *node,
                                                    TR::MemoryReference *mr,
                                                    TR::Register *sreg,
                                                    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemRegInstruction(op, node, mr, sreg, cg);
}

TR::X86MemRegInstruction *
generateMemRegInstruction(TR_X86OpCodes op, TR::Node *node,
                          TR::MemoryReference *mr, TR::Register *sreg,
                          TR::RegisterDependencyConditions *cond,
                          TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86MemRegInstruction(op, node, mr, sreg, cond, cg);
}

TR::X86ImmSymInstruction *generateImmSymInstruction(TR_X86OpCodes op,
                                                    TR::Node *node, int32_t imm,
                                                    TR::SymbolReference *sr,
                                                    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86ImmSymInstruction(op, node, imm, sr, cg);
}
...
TR::X86ImmSymInstruction *generateImmSymInstruction(
    ...
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86ImmSymInstruction(op, node, imm, sr, cond, cg);
}

TR::X86ImmSnippetInstruction *
generateImmSnippetInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm,
                              TR::UnresolvedDataSnippet *snippet,
                              TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86ImmSnippetInstruction(op, node, imm, snippet, cg);
}

TR::X86RegMemImmInstruction *
generateRegMemImmInstruction(TR_X86OpCodes op, TR::Node *node,
                             TR::Register *reg, TR::MemoryReference *mr,
                             int32_t imm, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegMemImmInstruction(op, node, reg, mr, imm, cg);
}

TR::X86RegRegImmInstruction *
generateRegRegImmInstruction(TR_X86OpCodes op, TR::Node *node,
                             TR::Register *treg, TR::Register *sreg,
                             int32_t imm, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86RegRegImmInstruction(op, node, treg, sreg, imm, cg);
}
...
TR::X86CallMemInstruction *generateCallMemInstruction(
    ...
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86CallMemInstruction(op, node, mr, cond, cg);
}

TR::X86CallMemInstruction *generateCallMemInstruction(TR_X86OpCodes op,
                                                      TR::Node *node,
                                                      TR::MemoryReference *mr,
                                                      TR::CodeGenerator *cg) {
...
TR::X86ImmSymInstruction *generateHelperCallInstruction(TR::Instruction *cursor,
                                                        TR_RuntimeHelper index,
                                                        TR::CodeGenerator *cg) {
  TR::SymbolReference *helperSymRef =
      cg->symRefTab()->findOrCreateRuntimeHelper(index, false, false, false);
  cg->resetIsLeafMethod();
  return new (cg->trHeapMemory()) TR::X86ImmSymInstruction(
      cursor, CALLImm4, (uintptrj_t)helperSymRef->getMethodAddress(),
      helperSymRef, cg);
}

TR::X86ImmSymInstruction *
generateHelperCallInstruction(TR::Node *node, TR_RuntimeHelper index,
                              TR::RegisterDependencyConditions *dependencies,
                              TR::CodeGenerator *cg) {
  TR::SymbolReference *helperSymRef =
      cg->symRefTab()->findOrCreateRuntimeHelper(index, false, false, false);
  cg->resetIsLeafMethod();
  return generateImmSymInstruction(CALLImm4, node,
                                   (uintptrj_t)helperSymRef->getMethodAddress(),
                                   helperSymRef, dependencies, cg);
}

void TR::X86VFPCallCleanupInstruction::assignRegisters(
    TR_RegisterKinds kindsToBeAssigned) {
  if (kindsToBeAssigned & TR_GPR_Mask) {
    TR_ASSERT(cg()->internalControlFlowNestingDepth() >= 1,
              "TR::X86VFPCallCleanupInstruction %s must only be used inside "
...
TR::X86VFPSaveInstruction *
generateVFPSaveInstruction(TR::Instruction *precedingInstruction,
                           TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPSaveInstruction(precedingInstruction, cg);
}

TR::X86VFPSaveInstruction *generateVFPSaveInstruction(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86VFPSaveInstruction(node, cg);
}

TR::X86VFPRestoreInstruction *
generateVFPRestoreInstruction(TR::Instruction *precedingInstruction,
                              TR::X86VFPSaveInstruction *saveInstruction,
                              TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPRestoreInstruction(precedingInstruction, saveInstruction, cg);
}

TR::X86VFPRestoreInstruction *
generateVFPRestoreInstruction(TR::X86VFPSaveInstruction *saveInstruction,
                              TR::Node *node, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPRestoreInstruction(saveInstruction, node, cg);
}

TR::X86VFPDedicateInstruction *
generateVFPDedicateInstruction(TR::Instruction *precedingInstruction,
                               TR::RealRegister *framePointerReg,
                               TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPDedicateInstruction(precedingInstruction, framePointerReg, cg);
}

TR::X86VFPDedicateInstruction *
generateVFPDedicateInstruction(TR::RealRegister *framePointerReg,
                               TR::Node *node, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPDedicateInstruction(framePointerReg, node, cg);
}

TR::X86VFPDedicateInstruction *generateVFPDedicateInstruction(
    TR::Instruction *precedingInstruction, TR::RealRegister *framePointerReg,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86VFPDedicateInstruction(
      precedingInstruction, framePointerReg, cond, cg);
}

TR::X86VFPDedicateInstruction *generateVFPDedicateInstruction(
    TR::RealRegister *framePointerReg, TR::Node *node,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPDedicateInstruction(framePointerReg, node, cond, cg);
}
...
TR::X86VFPReleaseInstruction *generateVFPReleaseInstruction(
  ...
  return new (cg->trHeapMemory()) TR::X86VFPReleaseInstruction(
      precedingInstruction, dedicateInstruction, cg);
}

TR::X86VFPReleaseInstruction *generateVFPReleaseInstruction(
    TR::X86VFPDedicateInstruction *dedicateInstruction, TR::Node *node,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPReleaseInstruction(dedicateInstruction, node, cg);
}

TR::X86VFPCallCleanupInstruction *
generateVFPCallCleanupInstruction(TR::Instruction *precedingInstruction,
                                  int32_t adjustment, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPCallCleanupInstruction(precedingInstruction, adjustment, cg);
}

TR::X86VFPCallCleanupInstruction *
generateVFPCallCleanupInstruction(int32_t adjustment, TR::Node *node,
                                  TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86VFPCallCleanupInstruction(adjustment, node, cg);
}

TR::X86FPRegInstruction *generateFPRegInstruction(TR_X86OpCodes op,
                                                  TR::Node *node,
                                                  TR::Register *reg,
                                                  TR::CodeGenerator *cg) {
...
generateFPST0ST1RegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                  TR::Register *treg, TR::Register *sreg,
                                  TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPST0ST1RegRegInstruction(op, node, treg, sreg, cg);
...
generateFPST0STiRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                  TR::Register *treg, TR::Register *sreg,
                                  TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPST0STiRegRegInstruction(op, node, treg, sreg, cg);
}

TR::X86FPSTiST0RegRegInstruction *
generateFPSTiST0RegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                  TR::Register *treg, TR::Register *sreg,
                                  TR::CodeGenerator *cg, bool forcePop) {
  return new (cg->trHeapMemory())
      TR::X86FPSTiST0RegRegInstruction(op, node, treg, sreg, cg, forcePop);
}

TR::X86FPArithmeticRegRegInstruction *
generateFPArithmeticRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                      TR::Register *treg, TR::Register *sreg,
                                      TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPArithmeticRegRegInstruction(op, node, treg, sreg, cg);
}

TR::X86FPCompareRegRegInstruction *
generateFPCompareRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                   TR::Register *treg, TR::Register *sreg,
                                   TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPCompareRegRegInstruction(op, node, treg, sreg, cg);
}

TR::X86FPCompareEvalInstruction *
generateFPCompareEvalInstruction(TR_X86OpCodes op, TR::Node *node,
                                 TR::Register *accRegister,
                                 TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPCompareEvalInstruction(op, node, accRegister, cg);
}

TR::X86FPCompareEvalInstruction *generateFPCompareEvalInstruction(
    TR_X86OpCodes op, TR::Node *node, TR::Register *accRegister,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPCompareEvalInstruction(op, node, accRegister, cond, cg);
}

TR::X86FPRemainderRegRegInstruction *
generateFPRemainderRegRegInstruction(TR_X86OpCodes op, TR::Node *node,
                                     TR::Register *treg, TR::Register *sreg,
                                     TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPRemainderRegRegInstruction(op, node, treg, sreg, cg);
}
...
TR::X86FPRemainderRegRegInstruction *generateFPRemainderRegRegInstruction(
    ...
    TR::Register *accReg, TR::RegisterDependencyConditions *cond,
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::X86FPRemainderRegRegInstruction(
      op, node, treg, sreg, accReg, cond, cg);
}

TR::X86FPMemRegInstruction *generateFPMemRegInstruction(TR_X86OpCodes op,
                                                        TR::Node *node,
                                                        TR::MemoryReference *mr,
                                                        TR::Register *sreg,
                                                        TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPMemRegInstruction(op, node, mr, sreg, cg);
}

TR::X86FPRegMemInstruction *generateFPRegMemInstruction(TR_X86OpCodes op,
                                                        TR::Node *node,
                                                        TR::Register *treg,
                                                        TR::MemoryReference *mr,
                                                        TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPRegMemInstruction(op, node, treg, mr, cg);
}

TR::X86FPReturnInstruction *
generateFPReturnInstruction(TR_X86OpCodes op, TR::Node *node,
                            TR::RegisterDependencyConditions *cond,
                            TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPReturnInstruction(cond, node, op, cg);
}

TR::X86FPReturnImmInstruction *
generateFPReturnImmInstruction(TR_X86OpCodes op, TR::Node *node, int32_t imm,
                               TR::RegisterDependencyConditions *cond,
                               TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::X86FPReturnImmInstruction(op, node, imm, cond, cg);
}

TR::AMD64RegImm64Instruction *
generateRegImm64Instruction(TR_X86OpCodes op, TR::Node *node,
                            TR::Register *treg, uint64_t imm,
                            TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory())
      TR::AMD64RegImm64Instruction(op, node, treg, imm, cg, reloKind);
}

TR::AMD64RegImm64Instruction *
generateRegImm64Instruction(TR::Instruction *precedingInstruction,
                            TR_X86OpCodes op, TR::Register *treg, uint64_t imm,
                            TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory()) TR::AMD64RegImm64Instruction(
      precedingInstruction, op, treg, imm, cg, reloKind);
}
...
generateRegImm64Instruction(TR_X86OpCodes op, TR::Node *node,
                            ...
                            TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory())
      TR::AMD64RegImm64Instruction(op, node, treg, imm, cond, cg, reloKind);
}
...
generateRegImm64Instruction(TR::Instruction *precedingInstruction,
                            ...
                            TR::CodeGenerator *cg, int32_t reloKind) {
  return new (cg->trHeapMemory()) TR::AMD64RegImm64Instruction(
      precedingInstruction, op, treg, imm, cond, cg, reloKind);
}

TR::AMD64Imm64Instruction *generateImm64Instruction(TR_X86OpCodes op,
                                                    TR::Node *node,
                                                    uint64_t imm,
                                                    TR::CodeGenerator *cg) {
...
generateImm64Instruction(TR::Instruction *precedingInstruction,
                         TR_X86OpCodes op, uint64_t imm,
                         TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64Instruction(precedingInstruction, op, imm, cg);
}

TR::AMD64Imm64Instruction *
generateImm64Instruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm,
                         TR::RegisterDependencyConditions *cond,
                         TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64Instruction(op, node, imm, cond, cg);
}

TR::AMD64Imm64Instruction *generateImm64Instruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, uint64_t imm,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64Instruction(precedingInstruction, op, imm, cond, cg);
}

// TR::AMD64Imm64SymInstruction
//
TR::AMD64RegImm64SymInstruction *
generateRegImm64SymInstruction(TR_X86OpCodes op, TR::Node *node,
                               TR::Register *reg, uint64_t imm,
                               TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64RegImm64SymInstruction(op, node, reg, imm, sr, cg);
}

TR::AMD64RegImm64SymInstruction *generateRegImm64SymInstruction(
    TR::Instruction *precedingInstruction, TR_X86OpCodes op, TR::Register *reg,
    uint64_t imm, TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::AMD64RegImm64SymInstruction(
      precedingInstruction, op, reg, imm, sr, cg);
}

TR::AMD64Imm64SymInstruction *
generateImm64SymInstruction(TR_X86OpCodes op, TR::Node *node, uint64_t imm,
                            TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64SymInstruction(op, node, imm, sr, cg);
}
...
generateImm64SymInstruction(TR::Instruction *precedingInstruction,
                            ...
                            TR::SymbolReference *sr, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64SymInstruction(precedingInstruction, op, imm, sr, cg);
}

TR::AMD64Imm64SymInstruction *generateImm64SymInstruction(
    TR_X86OpCodes op, TR::Node *node, uint64_t imm, TR::SymbolReference *sr,
    TR::RegisterDependencyConditions *cond, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64SymInstruction(op, node, imm, sr, cond, cg);
}
...
TR::AMD64Imm64SymInstruction *generateImm64SymInstruction(
    ...
    TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory())
      TR::AMD64Imm64SymInstruction(precedingInstruction, op, imm, sr, cond, cg);
}
