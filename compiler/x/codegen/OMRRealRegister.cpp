...
TR::RealRegister* OMR::X86::RealRegister::regMaskToRealRegister(
  ...
  if (rk == TR_GPR)
    ...
    TR_ASSERT(false,
              "Invalid TR_RegisterKinds value passed to "
              "OMR::X86::RealRegister::regMaskToRealRegister()");

  return cg->machine()->getRealRegister(TR::RealRegister::RegNum(rr + bitPos));
}

TR_RegisterMask OMR::X86::RealRegister::getAvailableRegistersMask(
    TR_RegisterKinds rk) {
  if (rk == TR_GPR)
...
TR::RealRegister::RegMask OMR::X86::RealRegister::getRealRegisterMask(
    TR_RegisterKinds rk,
