...
public:
  ...
  void addPreCondition(TR::Register* vr,
                       ...
                       bool isAssocRegDependency = false)
  {
    TR_X86RegisterDependencyIndex newCursor = unionRealDependencies(
      _preConditions, _addCursorForPre, vr, rr, cg, flag, isAssocRegDependency);
  ...
  void unionPreCondition(TR::Register* vr,
                         ...
                         bool isAssocRegDependency = false)
  {
    TR_X86RegisterDependencyIndex newCursor = unionDependencies(
      _preConditions, _addCursorForPre, vr, rr, cg, flag, isAssocRegDependency);
    TR_ASSERT(newCursor <= _numPreConditions, "Too many dependencies");
    if (newCursor == _addCursorForPre)
                        ...
                        bool isAssocRegDependency = false)
  {
    TR_X86RegisterDependencyIndex newCursor =
      unionRealDependencies(_postConditions,
                            _addCursorForPost,
                            vr,
                            rr,
                            cg,
  ...
  {
    TR_X86RegisterDependencyIndex newCursor =
      unionDependencies(_postConditions,
                        _addCursorForPost,
                        vr,
                        rr,
                        cg,
                                   ...
                                   TR::CodeGenerator* cg)
  {
    if (_preConditions != NULL) {
      if ((kindsToBeAssigned & TR_X87_Mask)) {
        _preConditions->assignFPRegisters(
          currentInstruction, kindsToBeAssigned, _numPreConditions, cg);
      } else {
...
generateRegisterDependencyConditions(TR::Node*,
                                     TR::CodeGenerator*,
