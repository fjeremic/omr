...
namespace OMR {
typedef OMR::X86::RegisterDependencyConditions
...
#define UsesGlobalDependentFPRegister \
  (UsesDependentRegister | GlobalRegisterFPDependency)
...
class TR_X86RegisterDependencyGroup {
  ...
  // Use TR_X86RegisterDependencyGroup::create to allocate an object of this
  // type
  //
 ...
 public:
  ...
  static TR_X86RegisterDependencyGroup *create(int32_t numDependencies,
                                               TR_Memory *m) {
  ...
  TR::RegisterDependency *getRegisterDependency(
      TR_X86RegisterDependencyIndex index) {
    return &_dependencies[index];
  }

  void setDependencyInfo(TR_X86RegisterDependencyIndex index, TR::Register *vr,
                         TR::RealRegister::RegNum rr, TR::CodeGenerator *cg,
                         uint8_t flag = UsesDependentRegister,
                         bool isAssocRegDependency = false);

  TR::RegisterDependency *findDependency(TR::Register *vr,
                                         TR_X86RegisterDependencyIndex stop) {
    TR::RegisterDependency *result = NULL;
    for (TR_X86RegisterDependencyIndex i = 0; !result && (i < stop); i++)
      if (_dependencies[i].getRegister() == vr) result = _dependencies + i;
    return result;
  }

  TR::RegisterDependency *findDependency(TR::RealRegister::RegNum rr,
                                         TR_X86RegisterDependencyIndex stop) {
    TR::RegisterDependency *result = NULL;
    for (TR_X86RegisterDependencyIndex i = 0; !result && (i < stop); i++)
      if (_dependencies[i].getRealRegister() == rr) result = _dependencies + i;
    return result;
  }
  ...
  void blockRealDependencyRegisters(
      TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg);
  void unblockRealDependencyRegisters(
      TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg);
};
...
class RegisterDependencyConditions : public OMR::RegisterDependencyConditions {
  ...
  TR_X86RegisterDependencyIndex unionDependencies(
  ...
  TR_X86RegisterDependencyIndex unionRealDependencies(
      TR_X86RegisterDependencyGroup *deps, TR_X86RegisterDependencyIndex cursor,
      TR::Register *vr, TR::RealRegister::RegNum rr, TR::CodeGenerator *cg,
      uint8_t flag, bool isAssocRegDependency);
 ...
 public:
  ...
  TR::RegisterDependencyConditions *clone(
      TR::CodeGenerator *cg,
      TR_X86RegisterDependencyIndex additionalRegDeps = 0);
  ...
  void setMayNeedToPopFPRegisters(bool b) {
    if (_preConditions) _preConditions->setMayNeedToPopFPRegisters(b);
    if (_postConditions) _postConditions->setMayNeedToPopFPRegisters(b);
  }
  ...
  void setNeedToClearFPStack(bool b) {
    if (_preConditions)
      ...
      if (_postConditions) _postConditions->setNeedToClearFPStack(b);
    }
  }

  TR_X86RegisterDependencyIndex getNumPreConditions() {
    return _numPreConditions;
  }

  TR_X86RegisterDependencyIndex setNumPreConditions(
      TR_X86RegisterDependencyIndex n, TR_Memory *m) {
  ...
  TR_X86RegisterDependencyIndex getNumPostConditions() {
    return _numPostConditions;
  }
  TR_X86RegisterDependencyIndex setNumPostConditions(
      TR_X86RegisterDependencyIndex n, TR_Memory *m) {
  ...
  TR_X86RegisterDependencyIndex getAddCursorForPre() {
    return _addCursorForPre;
  }
  TR_X86RegisterDependencyIndex setAddCursorForPre(
      TR_X86RegisterDependencyIndex a) {
    return (_addCursorForPre = a);
  }

  TR_X86RegisterDependencyIndex getAddCursorForPost() {
    return _addCursorForPost;
  }
  TR_X86RegisterDependencyIndex setAddCursorForPost(
      TR_X86RegisterDependencyIndex a) {
    return (_addCursorForPost = a);
  }
  ...
  void addPreCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
                       ...
                       uint8_t flag = UsesDependentRegister,
                       bool isAssocRegDependency = false) {
    TR_X86RegisterDependencyIndex newCursor =
        unionRealDependencies(_preConditions, _addCursorForPre, vr, rr, cg,
                              flag, isAssocRegDependency);
    TR_ASSERT(newCursor <= _numPreConditions, "Too many dependencies");
    if (newCursor == _addCursorForPre)
      _numPreConditions--;  // A vmThread/ebp dependency was displaced, so there
                            // is now one less condition.
    else
      _addCursorForPre = newCursor;
  }

  void unionPreCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
                         TR::CodeGenerator *cg,
                         uint8_t flag = UsesDependentRegister,
                         bool isAssocRegDependency = false) {
    TR_X86RegisterDependencyIndex newCursor =
        unionDependencies(_preConditions, _addCursorForPre, vr, rr, cg, flag,
                          isAssocRegDependency);
    TR_ASSERT(newCursor <= _numPreConditions, "Too many dependencies");
    if (newCursor == _addCursorForPre)
      _numPreConditions--;  // A union occurred, so there is now one less
                            // condition
    else
      _addCursorForPre = newCursor;
  }
  ...
  void addPostCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
                        ...
                        bool isAssocRegDependency = false) {
    TR_X86RegisterDependencyIndex newCursor =
        unionRealDependencies(_postConditions, _addCursorForPost, vr, rr, cg,
                              flag, isAssocRegDependency);
    TR_ASSERT(newCursor <= _numPostConditions, "Too many dependencies");
    if (newCursor == _addCursorForPost)
      _numPostConditions--;  // A vmThread/ebp dependency was displaced, so
                             // there is now one less condition.
    else
      _addCursorForPost = newCursor;
  }

  void unionPostCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
                          TR::CodeGenerator *cg,
                          uint8_t flag = UsesDependentRegister,
                          bool isAssocRegDependency = false) {
    TR_X86RegisterDependencyIndex newCursor =
        unionDependencies(_postConditions, _addCursorForPost, vr, rr, cg, flag,
                          isAssocRegDependency);
    TR_ASSERT(newCursor <= _numPostConditions, "Too many dependencies");
    if (newCursor == _addCursorForPost)
      _numPostConditions--;  // A union occurred, so there is now one less
                             // condition
    else
      _addCursorForPost = newCursor;
  }
  ...
  void assignPreConditionRegisters(TR::Instruction *currentInstruction,
    ...
    if (_preConditions != NULL) {
      if ((kindsToBeAssigned & TR_X87_Mask)) {
        _preConditions->assignFPRegisters(currentInstruction, kindsToBeAssigned,
                                          _numPreConditions, cg);
      } else {
        cg->clearRegisterAssignmentFlags();
        cg->setRegisterAssignmentFlag(TR_PreDependencyCoercion);
        _preConditions->assignRegisters(currentInstruction, kindsToBeAssigned,
                                        _numPreConditions, cg);
      }
    }
  }
  ...
  void assignPostConditionRegisters(TR::Instruction *currentInstruction,
    ...
    if (_postConditions != NULL) {
      if ((kindsToBeAssigned & TR_X87_Mask)) {
        _postConditions->assignFPRegisters(
            currentInstruction, kindsToBeAssigned, _numPostConditions, cg);
      } else {
        cg->clearRegisterAssignmentFlags();
        cg->setRegisterAssignmentFlag(TR_PostDependencyCoercion);
        _postConditions->assignRegisters(currentInstruction, kindsToBeAssigned,
                                         _numPostConditions, cg);
      }
    }
  }
  ...
  TR::RealRegister *getRealRegisterFromVirtual(TR::Register *virtReg,
                                               TR::CodeGenerator *cg);
...
TR::RegisterDependencyConditions *generateRegisterDependencyConditions(
    TR::Node *, TR::CodeGenerator *, TR_X86RegisterDependencyIndex = 0,
    List<TR::Register> * = 0);
TR::RegisterDependencyConditions *generateRegisterDependencyConditions(
    TR_X86RegisterDependencyIndex, TR_X86RegisterDependencyIndex,
    TR::CodeGenerator *);
