...
public:
  ...
  TR::MemoryReference* getMemRef()
  {
    return _memRef;
  }
  void setMemRef(TR::MemoryReference* memRef)
  {
    _memRef = memRef;
  }

  uint32_t getInterference()
  {
    return _liveRegisterInfo._interference;
  }
  ...
  bool isDiscardable()
  {
    return _flags.testAny(IsDiscardable);
  }
  void setIsDiscardable()
  {
    _flags.set(IsDiscardable);
  }
  void resetIsDiscardable()
  {
    _flags.reset(IsDiscardable);
  }

  bool needsLazyClobbering()
  {
    return _flags.testAny(NeedsLazyClobbering);
  }
  ...
  bool areUpperBitsZero()
  {
    return _flags.testAny(UpperBitsAreZero);
  }
  void setUpperBitsAreZero(bool b = true)
  {
    _flags.set(UpperBitsAreZero, b);
  }

  bool isSpilledToSecondHalf()
  {
    return _flags.testAny(SpilledToSecondHalf);
  }
...
private:
  enum {
    ...
    NeedsLazyClobbering = 0x0800, // X86 service releases only (well, originally the intent): node refcount = 1 is not
                                  // enough to determine that this can be clobbered; must also check register node count
