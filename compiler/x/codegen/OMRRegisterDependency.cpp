...
OMR::X86::RegisterDependencyConditions::RegisterDependencyConditions(
    ...
    : _numPreConditions(-1), _numPostConditions(-1), _addCursorForPre(0),
      _addCursorForPost(0) {
      ...
      if (child->getOpCodeValue() == TR::PassThrough &&
        ...
        if ((fpChild->getDataType() == TR::Float) ||
          ...
          if (fpGlobalReg != child->getRegister()) {
            ...
            machine->setFPStackRegister(child->getGlobalRegisterNumber() -
                                            machine->getNumGlobalGPRs(),
      ...
      if (cg->getUseNonLinearRegisterAssigner()) {
        if (virtReg->getAssignedRegister()) {
          ...
          TR::X86RegMemInstruction *inst = new (cg->trHeapMemory())
              TR::X86RegMemInstruction(currentInstruction, op, assignedReg,
                                       tempMR, cg);
          ...
          if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
            cg->traceRegisterAssignment("Generate reload of virt %s due to "
                                        "spillRegIndex dep at inst %p\n",
                                        comp->getDebug()->getName(virtReg),
                                        currentInstruction);
