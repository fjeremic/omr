...
static void
generateRegcopyDebugCounter(TR::CodeGenerator* cg, const char* category)
{
  ...
  , _addCursorForPost(0)
{
  ...
  for (i = node->getNumChildren() - 1; i >= 0; i--) {
    ...
    if (registers.find(globalReg)) {
      if (globalReg->getKind() == TR_GPR) {
        ...
        copyReg =
          (globalReg->containsCollectedReference() && !containsInternalPointer)
        ...
        generateRegRegInstruction(
          placeToAdd, MOVRegReg(), copyReg, globalReg, cg);
        ...
        if (highGlobalReg) {
          ...
          generateRegRegInstruction(
            MOV4RegReg, node, highCopyReg, highGlobalReg, cg);
        } else {
        ...
        if (globalReg->isSinglePrecision()) {
          ...
          generateRegRegInstruction(
            MOVAPSRegReg, node, copyReg, child->getRegister(), cg);
        } else {
          copyReg = cg->allocateRegister(TR_FPR);
          generateRegRegInstruction(
            MOVAPDRegReg, node, copyReg, child->getRegister(), cg);
        }
      } else if (globalReg->getKind() == TR_VRF) {
        generateRegcopyDebugCounter(cg, "vrf");
        copyReg = cg->allocateRegister(TR_VRF);
        generateRegRegInstruction(
          MOVDQURegReg, node, copyReg, child->getRegister(), cg);
      } else {
        ...
        generateFPST0STiRegRegInstruction(
          FLDRegReg, node, copyReg, child->getRegister(), cg);
    ...
    if (globalReg->getKind() == TR_GPR) {
      ...
      addPostCondition(
        globalReg, realRegNum, cg, UsesGlobalDependentFPRegister);
      int32_t fpRegNum =
        child->getGlobalRegisterNumber() - machine->getNumGlobalGPRs();
  ...
  if (numLongs != numLongsAdded)
    TR_ASSERT(
      0, "Mismatch numLongs %d numLongsAdded %d\n", numLongs, numLongsAdded);
}
  ...
  if (vr) {
    if (vr->getRealRegister()) {
      ...
      switch (vrIndex) {
        ...
        default:
          TR_ASSERT(0,
                    "Unexpected real register %s in regdep",
    ...
    for (TR_X86RegisterDependencyIndex candidate = 0; candidate < cursor;
      ...
      if (dep->getRegister() == vr) {
        ...
        if (min == TR::RealRegister::NoReg) {
          ...
          deps->setDependencyInfo(
            candidate, vr, max, cg, flag, isAssocRegDependency);
          ...
          deps->setDependencyInfo(
            candidate, vr, min, cg, flag, isAssocRegDependency);
        } else {
...
void
OMR::X86::RegisterDependencyConditions::unionNoRegPostCondition(
  TR::Register* reg,
  TR::CodeGenerator* cg)
{
    ...
    for (candidate = 0; candidate < cursor; candidate++) {
      ...
      if (dep->getRealRegister() == vmThreadRealRegisterIndex) {
        ...
        if (dep->getRegister() == vmThreadRegister) {
          ...
          deps->setDependencyInfo(
            candidate, vr, rr, cg, flag, isAssocRegDependency);
        } else {
...
OMR::X86::RegisterDependencyConditions::clone(
  TR::CodeGenerator* cg,
  TR_X86RegisterDependencyIndex additionalRegDeps)
{
...
bool
OMR::X86::RegisterDependencyConditions::refsRegister(TR::Register* r)
{
...
bool
OMR::X86::RegisterDependencyConditions::defsRegister(TR::Register* r)
{
...
bool
OMR::X86::RegisterDependencyConditions::usesRegister(TR::Register* r)
{
  for (int i = 0; i < _numPreConditions; i++) {
    if (_preConditions->getRegisterDependency(i)->getRegister() == r &&
...
void
OMR::X86::RegisterDependencyConditions::useRegisters(TR::Instruction* instr,
                                                     TR::CodeGenerator* cg)
{
  int32_t i;
...
void
TR_X86RegisterDependencyGroup::blockRealDependencyRegisters(
...
void
TR_X86RegisterDependencyGroup::unblockRealDependencyRegisters(
  TR_X86RegisterDependencyIndex numberOfRegisters,
  TR::CodeGenerator* cg)
{
  TR::Machine* machine = cg->machine();
...
void
TR_X86RegisterDependencyGroup::assignRegisters(
        ...
        if (dependentRealReg->getState() == TR::RealRegister::Blocked &&
          ...
          for (j = i + 1; j < numRealAssocRegisters; j++) {
            ...
            if (assignedRegNum2 == dependentRegNum &&
              ...
              machine->swapGPRegisters(
                currentInstruction, assignedRegNum, assignedRegNum2);
              break;
            }
          }
        }
    ...
    for (i = 0; i < numRealAssocRegisters; i++) {
      ...
      if (dependentRealReg->getState() == TR::RealRegister::Free) {
        if (virtReg->getKind() == TR_FPR || virtReg->getKind() == TR_VRF)
          machine->coerceXMMRegisterAssignment(
    ...
    do {
      ...
      for (i = firstByteRegAssoc; i <= lastByteRegAssoc; i++) {
        ...
        if (toRealRegister(virtReg->getAssignedRealRegister()) == NULL) {
          machine->coerceGPRegisterAssignment(
            currentInstruction, virtReg, TR_ByteReg);
          virtReg->block();
          changed = true;
        }
      }
    ...
    do {
      ...
      for (i = firstNoRegAssoc; i <= lastNoRegAssoc; i++) {
        ...
        if (toRealRegister(virtReg->getAssignedRealRegister()) == NULL) {
          if (virtReg->getKind() == TR_FPR || virtReg->getKind() == TR_VRF)
            machine->coerceGPRegisterAssignment(
              currentInstruction, virtReg, TR_QuadWordReg);
          else
...
void
TR_X86RegisterDependencyGroup::setDependencyInfo(
...
void
OMR::X86::RegisterDependencyConditions::createRegisterAssociationDirective(
...
void
TR_X86RegisterDependencyGroup::assignFPRegisters(
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (numberOfRegisters > 0) {
    ...
    orderGlobalRegsOnFPStack(
      cursor, kindsToBeAssigned, numberOfRegisters, &popRegisters, cg);
...
//
void
TR_X86RegisterDependencyGroup::orderGlobalRegsOnFPStack(
  ...
  TR::CodeGenerator* cg)
{
  ...
  memset(
    stackShape, 0xff, TR_X86FPStackRegister::NumRegisters * sizeof(int32_t));
      ...
      if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
        ...
        if (_dependencies[i].getGlobalFPRegister()) {
          ...
          while ((reqdStackHeight != currentStackHeight) &&
            ...
            virtReg =
              machine->getFPStackLocationPtr(topOfStack)->getAssignedRegister();
          }
...
uint32_t
OMR::X86::RegisterDependencyConditions::numReferencedFPRegisters(
  TR::CodeGenerator* cg)
{
  TR::Machine* machine = cg->machine();
  uint32_t i, total = 0;
...
uint32_t
OMR::X86::RegisterDependencyConditions::numReferencedGPRegisters(
  TR::CodeGenerator* cg)
{
  uint32_t i, total = 0;
  TR::Register* reg;
                                     ...
                                     TR::CodeGenerator* cg)
{
  return new (cg->trHeapMemory())
    TR::RegisterDependencyConditions(numPreConds, numPostConds, cg->trMemory());
}
...
{
  return new (cg->trHeapMemory())
    TR::RegisterDependencyConditions(node, cg, additionalRegDeps, popRegisters);
}
