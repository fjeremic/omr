    ...
    , _addCursorForPost(0)
{
    ...
    for (i = node->getNumChildren() - 1; i >= 0; i--) {
        ...
        if (globalReg->getKind() == TR_GPR || globalReg->getKind() == TR_VRF || XMM_GPRS_USE_DISTINCT_NUMBERS) {
            ...
            //
            realRegNum = (TR::RealRegister::RegNum)(TR::RealRegister::FirstFPR + (totalNumFPGlobalRegs - numFPGlobalRegs - 1));
        ...
        if (registers.find(globalReg)) {
            if (globalReg->getKind() == TR_GPR) {
                ...
                copyReg = (globalReg->containsCollectedReference() && !containsInternalPointer) ? cg->allocateCollectedReferenceRegister() : cg->allocateRegister();
                ...
                if (highGlobalReg) {
                    ...
                    highCopyReg = (highGlobalReg->containsCollectedReference() && !containsInternalPointer) ? cg->allocateCollectedReferenceRegister() : cg->allocateRegister();
        ...
        if (popRegisters && globalReg->getKind() == TR_X87 && child->getReferenceCount() == 0 && (child->getDataType() == TR::Float || child->getDataType() == TR::Double)) {
...
{
    for (int i = 0; i < _numPreConditions; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister() == r && _preConditions->getRegisterDependency(i)->getUsesRegister()) {
            return true;
        }
    }

    for (int j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister() == r && _postConditions->getRegisterDependency(j)->getUsesRegister()) {
            return true;
        }
    }
    ...
    TR::CodeGenerator* cg)
{
    ...
    // NoReg associations.
    //
    TR::RegisterDependency** dependencies = (TR::RegisterDependency**)cg->trMemory()->allocateStackMemory(numberOfRegisters * sizeof(TR::RegisterDependency*));
        ...
        if (dependentRegNum == TR::RealRegister::SpilledReg) {
            if (cg->getUseNonLinearRegisterAssigner()) {
                if (virtReg->getAssignedRegister()) {
                    ...
                    TR::X86RegMemInstruction* inst = new (cg->trHeapMemory()) TR::X86RegMemInstruction(currentInstruction, op, assignedReg, tempMR, cg);
    ...
    if (hasByteDeps) {
        ...
        for (i = 0; i < numberOfRegisters; i++) {
            virtReg = _dependencies[i].getRegister();
            dependentRegNum = _dependencies[i].getRealRegister();
            if (virtReg && (kindsToBeAssigned & virtReg->getKindAsMask()) && dependentRegNum == TR::RealRegister::ByteReg) {
                dependencies[numDependencyRegisters++] = getRegisterDependency(i);
            }
        }
    ...
    if (hasNoRegDeps) {
        ...
        for (i = 0; i < numberOfRegisters; i++) {
            virtReg = _dependencies[i].getRegister();
            dependentRegNum = _dependencies[i].getRealRegister();
            if (virtReg && (kindsToBeAssigned & virtReg->getKindAsMask()) && dependentRegNum == TR::RealRegister::NoReg) {
                dependencies[numDependencyRegisters++] = getRegisterDependency(i);
            }
        }
    ...
    if (hasBestFreeRegDeps) {
        ...
        for (i = 0; i < numberOfRegisters; i++) {
            virtReg = _dependencies[i].getRegister();
            dependentRegNum = _dependencies[i].getRealRegister();
            if (virtReg && (kindsToBeAssigned & virtReg->getKindAsMask()) && dependentRegNum == TR::RealRegister::BestFreeReg) {
                dependencies[numDependencyRegisters++] = getRegisterDependency(i);
            }
        }
    ...
    if (numberOfRegisters > 0) {
        ...
        if (labelInstruction && labelInstruction->getNeedToClearFPStack()) {
            ...
            for (i = 0; i < numberOfRegisters; i++) {
                ...
                if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
                    if (_dependencies[i].getGlobalFPRegister()) {
                        if ((virtReg->getFutureUseCount() == 0) || (virtReg->getTotalUseCount() == virtReg->getFutureUseCount())) {
        ...
        for (i = 0; i < numberOfRegisters; i++) {
            TR::Register* virtReg = _dependencies[i].getRegister();
            if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
                if (((virtReg->getFutureUseCount() != 0) && (virtReg->getTotalUseCount() != virtReg->getFutureUseCount())) && !virtReg->getAssignedRealRegister()) {
        ...
        if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
            if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
                ...
                if (_dependencies[i].getGlobalFPRegister()) {
                    ...
                    while ((reqdStackHeight != currentStackHeight) && (reqdStackHeight >= 0)) {
                        if (!machine->isFPRTopOfStack(virtReg)) {
                            cursor = machine->fpStackFXCH(cursor, virtReg);
                        }
...
#if DEBUG
                        if ((currentStackHeight < 0) || (currentStackHeight >= TR_X86FPStackRegister::NumRegisters))
                            TR_ASSERT(0, "Array out of bounds exception in array stackShape\n");
                        if ((reqdStackHeight < 0) || (reqdStackHeight >= TR_X86FPStackRegister::NumRegisters))
                            TR_ASSERT(0, "Array out of bounds exception in array stackShape\n");
#endif
...
uint32_t OMR::X86::RegisterDependencyConditions::numReferencedFPRegisters(TR::CodeGenerator* cg)
{
    ...
    for (i = 0; i < _numPreConditions; i++) {
        ...
        if ((reg && reg->getKind() == TR_X87) || (!reg && _preConditions->getRegisterDependency(i)->getRealRegister() == TR::RealRegister::AllFPRegisters)) {
            total++;
        }
    }
    ...
    for (i = 0; i < _numPostConditions; i++) {
        ...
        if ((reg && reg->getKind() == TR_X87) || (!reg && _postConditions->getRegisterDependency(i)->getRealRegister() == TR::RealRegister::AllFPRegisters)) {
            total++;
        }
    }
