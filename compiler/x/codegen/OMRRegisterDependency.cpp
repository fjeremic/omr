...
OMR::X86::RegisterDependencyConditions::RegisterDependencyConditions(TR::Node* node, TR::CodeGenerator* cg,
  TR_X86RegisterDependencyIndex additionalRegDeps, List<TR::Register>* popRegisters)
  : _numPreConditions(-1)
  ...
  , _addCursorForPost(0)
{
  ...
  _preConditions
    = TR_X86RegisterDependencyGroup::create(node->getNumChildren() + additionalRegDeps + numLongs, cg->trMemory());
  _postConditions
    = TR_X86RegisterDependencyGroup::create(node->getNumChildren() + additionalRegDeps + numLongs, cg->trMemory());
  ...
  for (i = node->getNumChildren() - 1; i >= 0; i--) {
    ...
    if (globalReg->getKind() == TR_GPR || globalReg->getKind() == TR_VRF || XMM_GPRS_USE_DISTINCT_NUMBERS) {
      ...
      //
      realRegNum
        = (TR::RealRegister::RegNum)(TR::RealRegister::FirstFPR + (totalNumFPGlobalRegs - numFPGlobalRegs - 1));
        ...
        if ((fpChild->getDataType() == TR::Float) || (fpChild->getDataType() == TR::Double)) {
          ...
          if (fpGlobalReg != child->getRegister()) {
            ...
            machine->setFPStackRegister(
              child->getGlobalRegisterNumber() - machine->getNumGlobalGPRs(), toX86FPStackRegister(globalReg));
          }
        }
      }
    ...
    if (popRegisters && globalReg->getKind() == TR_X87 && child->getReferenceCount() == 0
...
TR_X86RegisterDependencyIndex OMR::X86::RegisterDependencyConditions::unionDependencies(
  TR_X86RegisterDependencyGroup* deps, TR_X86RegisterDependencyIndex cursor, TR::Register* vr,
    ...
    if (vr->getRealRegister()) {
      ...
      case TR::RealRegister::vfp:
        ...
        TR_ASSERT(rr == vrIndex || rr == TR::RealRegister::NoReg,
          "esp and vfp can be assigned only to themselves or NoReg, not %s", cg->getDebug()->getRealRegisterName(rr));
        break;
      default:
...
void TR_X86RegisterDependencyGroup::assignRegisters(TR::Instruction* currentInstruction,
  TR_RegisterKinds kindsToBeAssigned, TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator* cg)
{
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (virtReg && (kindsToBeAssigned & virtReg->getKindAsMask()) && dependentRegNum != TR::RealRegister::NoReg
      && dependentRegNum != TR::RealRegister::ByteReg && dependentRegNum != TR::RealRegister::SpilledReg
  ...
  if (hasBestFreeRegDeps) {
    ...
    TR_ASSERT(
      (bestFreeRegAssoc == numDependencyRegisters - 1), "there can only be one bestFreeRegDep in a dependency list");
  }
...
void TR_X86RegisterDependencyGroup::setDependencyInfo(TR_X86RegisterDependencyIndex index, TR::Register* vr,
  TR::RealRegister::RegNum rr, TR::CodeGenerator* cg, uint8_t flag, bool isAssocRegDependency)
{
  ...
  TR_RegisterKinds kindsToBeAssigned, TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator* cg)
{
  ...
  if (numberOfRegisters > 0) {
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
        if (_dependencies[i].getGlobalFPRegister()) {
          ...
          if (assignedRegister) {
            ...
            TR_ASSERT(reqdStackHeight == currentStackHeight, "Stack height for global FP register is NOT correct\n");
          }
        }
      } else {
      ...
      TR_ASSERT(
        (!globalFPRA || ((machine->getFPTopOfStack() + 1) == numberOfFPRegisters)), "Stack height is NOT correct\n");
...
TR::RegisterDependencyConditions* generateRegisterDependencyConditions(
  TR_X86RegisterDependencyIndex numPreConds, TR_X86RegisterDependencyIndex numPostConds, TR::CodeGenerator* cg)
{
