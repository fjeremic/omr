...
static void generateRegcopyDebugCounter(TR::CodeGenerator *cg,
                                        const char *category) {
  if (!cg->comp()->getOptions()->enableDebugCounters()) return;
  TR::TreeTop *tt = cg->getCurrentEvaluationTreeTop();
  const char *fullName = TR::DebugCounter::debugCounterName(
      cg->comp(), "regcopy/cg.genRegDepConditions/%s/(%s)/%s/block_%d",
      category, cg->comp()->signature(),
...
OMR::X86::RegisterDependencyConditions::RegisterDependencyConditions(
  ...
  TR_X86RegisterDependencyIndex numFPGlobalRegs = 0, totalNumFPGlobalRegs = 0;
  int32_t i;
  ...
  // Pre-compute how many x87 FP stack slots we need to accommodate all the FP
  // global registers.
  //
  for (i = 0; i < node->getNumChildren(); ++i) {
    ...
    if (reg->getKind() == TR_GPR) {
      if (child->getHighGlobalRegisterNumber() > -1) numLongs++;
    } else if (reg->getKind() == TR_X87) {
      totalNumFPGlobalRegs++;
    }
  }

  _preConditions = TR_X86RegisterDependencyGroup::create(
      node->getNumChildren() + additionalRegDeps + numLongs, cg->trMemory());
  _postConditions = TR_X86RegisterDependencyGroup::create(
      node->getNumChildren() + additionalRegDeps + numLongs, cg->trMemory());
  ...
  for (i = node->getNumChildren() - 1; i >= 0; i--) {
    ...
    TR::RealRegister::RegNum realRegNum = TR::RealRegister::NoReg,
                             realHighRegNum = TR::RealRegister::NoReg;
    if (globalReg->getKind() == TR_GPR || globalReg->getKind() == TR_VRF ||
        XMM_GPRS_USE_DISTINCT_NUMBERS) {
      realRegNum =
      ...
      // Convert the global register number from an st0-based value to an
      // xmm0-based one.
      //
      realRegNum = (TR::RealRegister::RegNum)(
          cg->getGlobalRegister(globalRegNum) - TR::RealRegister::FirstFPR +
          TR::RealRegister::FirstXMMR);

      // Find the global register that has been allocated in this XMMR, if any.
      //
      TR::Register *xmmGlobalReg = cg->machine()->getXMMGlobalRegister(
          realRegNum - TR::RealRegister::FirstXMMR);
      if (xmmGlobalReg) globalReg = xmmGlobalReg;
    } else {
      TR_ASSERT(globalReg->getKind() == TR_X87,
                "invalid global register kind\n");

      // Compute the real global register number based on the number of global
      // FPRs allocated so far.
      //
      realRegNum = (TR::RealRegister::RegNum)(
          TR::RealRegister::FirstFPR +
          (totalNumFPGlobalRegs - numFPGlobalRegs - 1));

      // Find the global register that has been allocated in this FP stack slot,
      // if any.
      //
      int32_t fpStackSlot =
          child->getGlobalRegisterNumber() - machine->getNumGlobalGPRs();
      TR::Register *fpGlobalReg =
          cg->machine()->getFPStackRegister(fpStackSlot);
      if (fpGlobalReg) globalReg = fpGlobalReg;
    ...
    if (registers.find(globalReg)) {
      if (globalReg->getKind() == TR_GPR) {
        ...
        if (globalReg->getPinningArrayPointer()) containsInternalPointer = true;

        copyReg = (globalReg->containsCollectedReference() &&
        ...
        generateRegRegInstruction(placeToAdd, MOVRegReg(), copyReg, globalReg,
                                  cg);
        ...
        if (highGlobalReg) {
          ...
          highCopyReg = (highGlobalReg->containsCollectedReference() &&
                         !containsInternalPointer)
                            ? cg->allocateCollectedReferenceRegister()
                            : cg->allocateRegister();
          ...
          if (containsInternalPointer) {
            ...
            highCopyReg->setPinningArrayPointer(
                highGlobalReg->getPinningArrayPointer());
          }

          generateRegRegInstruction(MOV4RegReg, node, highCopyReg,
                                    highGlobalReg, cg);
        } else {
        ...
        if (globalReg->isSinglePrecision()) {
          ...
          generateRegRegInstruction(MOVAPSRegReg, node, copyReg,
                                    child->getRegister(), cg);
        } else {
          copyReg = cg->allocateRegister(TR_FPR);
          generateRegRegInstruction(MOVAPDRegReg, node, copyReg,
                                    child->getRegister(), cg);
        }
      } else if (globalReg->getKind() == TR_VRF) {
        generateRegcopyDebugCounter(cg, "vrf");
        copyReg = cg->allocateRegister(TR_VRF);
        generateRegRegInstruction(MOVDQURegReg, node, copyReg,
                                  child->getRegister(), cg);
      } else {
        ...
        generateFPST0STiRegRegInstruction(FLDRegReg, node, copyReg,
                                          child->getRegister(), cg);

        int32_t fpRegNum =
            child->getGlobalRegisterNumber() - machine->getNumGlobalGPRs();
        // dumpOptDetails("FP reg num %d global reg num %d global reg %s\n",
        // fpRegNum, child->getGlobalRegisterNumber(),
        // getDebug()->getName(globalReg));
        cg->machine()->setCopiedFPStackRegister(
            fpRegNum, toX86FPStackRegister(globalReg));
        ...
        for (i = 0; i < TR_X86FPStackRegister::NumRegisters; i++) {
          ...
          if ((reg == globalReg) && (i != fpRegNum)) {
            // dumpOptDetails("FP reg num %d global reg num %d global reg %s\n",
            // i, child->getGlobalRegisterNumber(),
            // getDebug()->getName(copyReg));
            cg->machine()->setCopiedFPStackRegister(
                i, toX86FPStackRegister(copyReg));
            if (child->getOpCodeValue() == TR::PassThrough) {
              TR::Node *fpChild = child->getFirstChild();
      ...
      if (child->getOpCodeValue() == TR::PassThrough &&
        ...
        if ((fpChild->getDataType() == TR::Float) ||
            (fpChild->getDataType() == TR::Double)) {
          // if the child's register is live on another stack slot, dont change
          // the register.
          TR::Register *fpGlobalReg = NULL;
          for (int j = 0; j < TR_X86FPStackRegister::NumRegisters &&
                          fpGlobalReg != child->getRegister();
               j++) {
    ...
    if (globalReg->getKind() == TR_GPR) {
      ...
      addPreCondition(globalReg, realRegNum, cg);
      addPostCondition(globalReg, realRegNum, cg);
      cg->machine()->setXMMGlobalRegister(
          realRegNum - TR::RealRegister::FirstXMMR, globalReg);
    } else if (globalReg->getKind() == TR_X87) {
      addPreCondition(globalReg, realRegNum, cg, UsesGlobalDependentFPRegister);
      addPostCondition(globalReg, realRegNum, cg,
                       UsesGlobalDependentFPRegister);
      int32_t fpRegNum =
          child->getGlobalRegisterNumber() - machine->getNumGlobalGPRs();
      cg->machine()->setFPStackRegister(fpRegNum,
                                        toX86FPStackRegister(globalReg));
    }
    ...
    if (popRegisters && globalReg->getKind() == TR_X87 &&
        child->getReferenceCount() == 0 &&
        (child->getDataType() == TR::Float ||
         child->getDataType() == TR::Double)) {
      popRegisters->add(globalReg);
    } else if (copyReg) {
      cg->stopUsingRegister(copyReg);
      if (highCopyReg) cg->stopUsingRegister(highCopyReg);
    }
  }
...
OMR::X86::RegisterDependencyConditions::unionDependencies(
    TR_X86RegisterDependencyGroup *deps, TR_X86RegisterDependencyIndex cursor,
    TR::Register *vr, TR::RealRegister::RegNum rr, TR::CodeGenerator *cg,
    uint8_t flag, bool isAssocRegDependency) {
  TR::Machine *machine = cg->machine();
  ...
  if (vr) {
    if (vr->getRealRegister()) {
      TR::RealRegister::RegNum vrIndex =
          toRealRegister(vr)->getRegisterNumber();
      switch (vrIndex) {
        ...
        case TR::RealRegister::vfp:
          ...
          //
          TR_ASSERT(
              rr == vrIndex || rr == TR::RealRegister::NoReg,
    ...
    for (TR_X86RegisterDependencyIndex candidate = 0; candidate < cursor;
         candidate++) {
      ...
      if (dep->getRegister() == vr) {
        ...
        if (min == TR::RealRegister::NoReg) {
          ...
          deps->setDependencyInfo(candidate, vr, max, cg, flag,
                                  isAssocRegDependency);
        } else if (max == TR::RealRegister::ByteReg) {
          // Specific regs are stronger than ByteReg
          TR_ASSERT(
              min <= TR::RealRegister::Last8BitGPR,
              "Only byte registers are compatible with ByteReg dependency");
          deps->setDependencyInfo(candidate, vr, min, cg, flag,
                                  isAssocRegDependency);
        } else {
          // This assume fires for some assocRegs instructions
          // TR_ASSERT(min == max, "Specific register dependency is only
...
void OMR::X86::RegisterDependencyConditions::unionNoRegPostCondition(
    TR::Register *reg, TR::CodeGenerator *cg) {
  unionPostCondition(reg, TR::RealRegister::NoReg, cg);
}

TR_X86RegisterDependencyIndex
OMR::X86::RegisterDependencyConditions::unionRealDependencies(
    TR_X86RegisterDependencyGroup *deps, TR_X86RegisterDependencyIndex cursor,
    TR::Register *vr, TR::RealRegister::RegNum rr, TR::CodeGenerator *cg,
    uint8_t flag, bool isAssocRegDependency) {
  TR::Machine *machine = cg->machine();
  ...
  static TR::RealRegister::RegNum vmThreadRealRegisterIndex =
      TR::RealRegister::ebp;
    ...
    for (candidate = 0; candidate < cursor; candidate++) {
      ...
      if (dep->getRealRegister() == vmThreadRealRegisterIndex) {
        ...
        if (dep->getRegister() == vmThreadRegister) {
          // diagnostic("\nEnvicting virt reg %s dep for %s replaced with virt
          // reg %s\n      {\"%s\"}",
          ...
          deps->setDependencyInfo(candidate, vr, rr, cg, flag,
                                  isAssocRegDependency);
        } else {
          ...
          //   cg->comp()->getCurrentMethod()->signature());
          TR_ASSERT(vr == vmThreadRegister,
                    "Conflicting EBP register dependencies.\n");
        }
        return cursor;
      }
    }
...
TR::RegisterDependencyConditions *OMR::X86::RegisterDependencyConditions::clone(
    TR::CodeGenerator *cg, TR_X86RegisterDependencyIndex additionalRegDeps) {
...
void OMR::X86::RegisterDependencyConditions::useRegisters(
    TR::Instruction *instr, TR::CodeGenerator *cg) {
  int32_t i;

  for (i = 0; i < _numPreConditions; i++) {
    TR::Register *virtReg =
        _preConditions->getRegisterDependency(i)->getRegister();
    if (virtReg) {
  ...
  for (i = 0; i < _numPostConditions; i++) {
    TR::Register *virtReg =
        _postConditions->getRegisterDependency(i)->getRegister();
    if (virtReg) {
      instr->useRegister(virtReg);
    }
  }
...
void TR_X86RegisterDependencyGroup::blockRealDependencyRegisters(
    TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg) {
  TR::Machine *machine = cg->machine();
...
void TR_X86RegisterDependencyGroup::unblockRealDependencyRegisters(
    TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg) {
  TR::Machine *machine = cg->machine();
  for (TR_X86RegisterDependencyIndex i = 0; i < numberOfRegisters; i++) {
    if (_dependencies[i].getRealRegister() != TR::RealRegister::NoReg) {
...
void TR_X86RegisterDependencyGroup::assignRegisters(
    TR::Instruction *currentInstruction, TR_RegisterKinds kindsToBeAssigned,
    TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg) {
  ...
  TR::RegisterDependency **dependencies =
      (TR::RegisterDependency **)cg->trMemory()->allocateStackMemory(
    ...
    if (dependentRegNum == TR::RealRegister::SpilledReg) {
      if (cg->getUseNonLinearRegisterAssigner()) {
        if (virtReg->getAssignedRegister()) {
          TR_ASSERT(virtReg->getBackingStorage(),
                    "should have a backing store for spilled reg virtuals");
          assignedReg = toRealRegister(virtReg->getAssignedRegister());

          TR::MemoryReference *tempMR = generateX86MemoryReference(
              virtReg->getBackingStorage()->getSymbolReference(), cg);

          TR_X86OpCodes op;
          if (assignedReg->getKind() == TR_FPR) {
            op = (assignedReg->isSinglePrecision())
                     ? MOVSSRegMem
          ...
          TR::X86RegMemInstruction *inst =
              new (cg->trHeapMemory()) TR::X86RegMemInstruction(
                  currentInstruction, op, assignedReg, tempMR, cg);
          ...
          if (comp->getOption(TR_TraceNonLinearRegisterAssigner)) {
            cg->traceRegisterAssignment(
                "Generate reload of virt %s due to spillRegIndex dep at inst "
                "%p\n",
        ...
        if (!(std::find(cg->getSpilledRegisterList()->begin(),
                        cg->getSpilledRegisterList()->end(),
                        virtReg) != cg->getSpilledRegisterList()->end())) {
  ...
  // proper real registers in one shot (seems to happen more often than one
  // would think)
  //
      ...
      if (virtReg->getKind() == TR_GPR) {
        ...
        if (dependentRealReg->getState() == TR::RealRegister::Blocked &&
          ...
          for (j = i + 1; j < numRealAssocRegisters; j++) {
            ...
            TR::RealRegister::RegNum dependentRegNum2 =
                dependencies[j]->getRealRegister();
            ...
            if (assignedRegNum2 == dependentRegNum &&
              ...
              machine->swapGPRegisters(currentInstruction, assignedRegNum,
                                       assignedRegNum2);
              break;
            }
          }
  ...
  // registers out of their existing registers and possibly freeing those
  // registers up to also be filled by the proper candidates as simply as
  // possible
  //
  do {
    changed = false;
    for (i = 0; i < numRealAssocRegisters; i++) {
      ...
      if (dependentRealReg->getState() == TR::RealRegister::Free) {
        if (virtReg->getKind() == TR_FPR || virtReg->getKind() == TR_VRF)
          machine->coerceXMMRegisterAssignment(currentInstruction, virtReg,
                                               dependentRegNum);
        else
  ...
  // register is not yet assigned.  We could do this at the same time as the
  // previous loop, but that could lead to some naive changes that would have
  // been easier once all the opportunities found by the first loop have been
  // found.
  //
  do {
    changed = false;
  ...
  //
  // Assign byte register dependencies first.  Unblock any NoReg dependencies to
  // not constrain the byte register choices.
  //
  if (hasByteDeps) {
    if (hasNoRegDeps) {
  ...
  if (hasBestFreeRegDeps) {
    ...
    bestFreeRealReg =
        machine->findBestFreeGPRegister(currentInstruction, virtReg);
    bestFreeRealRegIndex = bestFreeRealReg
                               ? bestFreeRealReg->getRegisterNumber()
...
void TR_X86RegisterDependencyGroup::setDependencyInfo(
    TR_X86RegisterDependencyIndex index, TR::Register *vr,
    TR::RealRegister::RegNum rr, TR::CodeGenerator *cg, uint8_t flag,
  ...
  if (vr && vr->isLive() && rr != TR::RealRegister::NoReg &&
...
TR::RealRegister *
OMR::X86::RegisterDependencyConditions::getRealRegisterFromVirtual(
...
void TR_X86RegisterDependencyGroup::assignFPRegisters(
    TR::Instruction *prevInstruction, TR_RegisterKinds kindsToBeAssigned,
    TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator *cg) {
  TR::Machine *machine = cg->machine();
  ...
  if (numberOfRegisters > 0) {
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
        if (_dependencies[i].getGlobalFPRegister()) numberOfFPRegisters++;
      }
    }
#endif
    ...
    if (labelInstruction && labelInstruction->getNeedToClearFPStack()) {
    ...
    orderGlobalRegsOnFPStack(cursor, kindsToBeAssigned, numberOfRegisters,
                             &popRegisters, cg);
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
        if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
          ...
          // If this is the first reference of a register, then this must be the
          // caller side return value.  Assume it already exists on the FP stack
          // if not a global FP register; else coerce it to the right stack
          // location. The required stack must be available at this point.
          //
          if (_dependencies[i].getGlobalFPRegister()) {
            int32_t reqdStackHeight =
                _dependencies[i].getRealRegister() - TR::RealRegister::FirstFPR;
            machine->fpStackCoerce(
                virtReg, (machine->getFPTopOfStack() - reqdStackHeight));
            virtReg->decFutureUseCount();
          } else {
            if (virtReg->decFutureUseCount() != 0) {
              machine->fpStackPush(virtReg);
      ...
      } else if (_dependencies[i].getRealRegister() ==
                 TR::RealRegister::AllFPRegisters) {
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
        if (_dependencies[i].getGlobalFPRegister()) {
          ...
          TR_X86FPStackRegister *assignedRegister =
              toX86FPStackRegister(virtReg->getAssignedRealRegister());
          if (assignedRegister) {
            int32_t reqdStackHeight =
                _dependencies[i].getRealRegister() - TR::RealRegister::FirstFPR;
            int32_t currentStackHeight =
                machine->getFPTopOfStack() -
                assignedRegister->getFPStackRegisterNumber();
      ...
      TR_ASSERT((!globalFPRA ||
                 ((machine->getFPTopOfStack() + 1) == numberOfFPRegisters)),
    ...
    // We may need to pop registers off the FP stack at this late stage in
    // certain cases; in particular if there are 2 global registers having the
    // same value (same child C in the GLRegDeps on a branch) and one of them is
    // never used in the extended block after the branch; then only one value
    // will be popped off the stack when the last reference to child C is seen
    // in the extended basic block. This case cannot be detected during
    // instruction selection as the reference count on the child C is non-zero
    // at the dependency but in fact one of the registers must be popped off
    // (late) in this case.
    //
    if (getMayNeedToPopFPRegisters() && !popRegisters.isEmpty()) {
      ...
      for (TR::Register *popRegister = popRegsIt.getFirst();
        ...
        TR::RealRegister *popRealRegister =
            machine->fpMapToStackRelativeRegister(popRegister);
        cursor = new (cg->trHeapMemory())
            TR::X86FPRegInstruction(cursor, FSTPReg, popRealRegister, cg);
        machine->fpStackPop();
      }
    }
  }
...
// is 3N/2 exchanges where N is number of incorrect stack locations (this could
// happen if there are N/2 pairwise incorrect global registers. This could
// possibly be done in N+1 approx maybe (?)
//
void TR_X86RegisterDependencyGroup::orderGlobalRegsOnFPStack(
    TR::Instruction *cursor, TR_RegisterKinds kindsToBeAssigned,
    TR_X86RegisterDependencyIndex numberOfRegisters,
    List<TR::Register> *poppedRegisters, TR::CodeGenerator *cg) {
  //
  TR::Machine *machine = cg->machine();
  int32_t *stackShape = machine->getFPStackShape();
  memset(stackShape, 0xff,
         TR_X86FPStackRegister::NumRegisters * sizeof(int32_t));
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
      if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
        TR_X86FPStackRegister *assignedRegister =
            toX86FPStackRegister(virtReg->getAssignedRealRegister());
        TR_ASSERT(assignedRegister != NULL,
                  "All spilled registers should have been reloaded by now\n");

        if (_dependencies[i].getGlobalFPRegister()) {
          int32_t reqdStackHeight =
              _dependencies[i].getRealRegister() - TR::RealRegister::FirstFPR;
          int32_t currentStackHeight =
              topOfStack - assignedRegister->getFPStackRegisterNumber();
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (virtReg && kindsToBeAssigned & virtReg->getKindAsMask()) {
      if (virtReg->getTotalUseCount() != virtReg->getFutureUseCount()) {
        // If this is not the first reference of a register, then coerce it to
        // the top of stack.
        //
        if (_dependencies[i].getGlobalFPRegister()) {
          int32_t reqdStackHeight =
          ...
          while ((reqdStackHeight != currentStackHeight) &&
            ...
            assignedRegister =
                toX86FPStackRegister(virtReg->getAssignedRealRegister());
            int32_t tempCurrentStackHeight =
                topOfStack - assignedRegister->getFPStackRegisterNumber();
            ...
            if ((currentStackHeight < 0) ||
              ...
              TR_ASSERT(0,
                        "Array out of bounds exception in array stackShape\n");
            if ((reqdStackHeight < 0) ||
            ...
            virtReg = machine->getFPStackLocationPtr(topOfStack)
                          ->getAssignedRegister();
          }

          if (origRegister->decFutureUseCount() == 0) {
            // It is possible for a global register to go dead at a dependency;
            // add it to the list of registers to be popped in this case
...
uint32_t OMR::X86::RegisterDependencyConditions::numReferencedFPRegisters(
    TR::CodeGenerator *cg) {
  TR::Machine *machine = cg->machine();
  uint32_t i, total = 0;
  TR::Register *reg;
  ...
  for (i = 0; i < _numPreConditions; i++) {
    ...
    if ((reg && reg->getKind() == TR_X87) ||
        (!reg && _preConditions->getRegisterDependency(i)->getRealRegister() ==
                     TR::RealRegister::AllFPRegisters)) {
      total++;
    }
  }
  ...
  for (i = 0; i < _numPostConditions; i++) {
    ...
    if ((reg && reg->getKind() == TR_X87) ||
        (!reg && _postConditions->getRegisterDependency(i)->getRealRegister() ==
                     TR::RealRegister::AllFPRegisters)) {
      total++;
    }
  }
...
uint32_t OMR::X86::RegisterDependencyConditions::numReferencedGPRegisters(
    TR::CodeGenerator *cg) {
  uint32_t i, total = 0;
  TR::Register *reg;
...
TR::RegisterDependencyConditions *generateRegisterDependencyConditions(
    TR_X86RegisterDependencyIndex numPreConds,
    TR_X86RegisterDependencyIndex numPostConds, TR::CodeGenerator *cg) {
  return new (cg->trHeapMemory()) TR::RegisterDependencyConditions(
      numPreConds, numPostConds, cg->trMemory());
}
...
TR::RegisterDependencyConditions *generateRegisterDependencyConditions(
    ...
    List<TR::Register> *popRegisters) {
  return new (cg->trHeapMemory()) TR::RegisterDependencyConditions(
      node, cg, additionalRegDeps, popRegisters);
}
