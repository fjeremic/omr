...
uint8_t *TR::X86FPConversionSnippet::emitCallToConversionHelper(
    uint8_t *buffer) {
...
uint8_t *TR::X86FPConvertToIntSnippet::genFPConversion(uint8_t *buffer) {
  ...
  TR_ASSERT(
      cg()->getProperties().getIntegerReturnRegister() == TR::RealRegister::eax,
...
uint32_t TR::X86FPConvertToIntSnippet::getLength(
...
uint32_t TR::X86FPConvertToLongSnippet::getLength(
    int32_t estimatedSnippetStart) {
  uint32_t length = 5;
