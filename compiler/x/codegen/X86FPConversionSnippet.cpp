...
uint8_t*
TR::X86FPConversionSnippet::emitSnippetBody()
{
...
uint8_t*
TR::X86FPConversionSnippet::emitCallToConversionHelper(uint8_t* buffer)
{
  ...
  *(int32_t*)buffer = (int32_t)(helperAddress - nextInstructionAddress);
  cg()->addProjectSpecializedRelocation(buffer,
                                        (uint8_t*)getHelperSymRef(),
                                        NULL,
                                        TR_HelperAddress,
                                        __FILE__,
                                        __LINE__,
  ...
  if (_convertInstruction->getIA32RegMemInstruction()) {
    ...
    *buffer++ = 0x24;
    ...
    TR::X86RegRegInstruction* instr =
      _convertInstruction->getIA32RegRegInstruction();
    TR_ASSERT(instr != NULL,
              "f2i conversion instruction must be either L4RegMem or "
              "CVTTSS2SIRegReg\n");

    TR::RealRegister* sourceRegister =
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86FPConvertToIntSnippet* snippet)
{
  ...
  TR::RealRegister* targetRegister =
    toRealRegister(snippet->getConvertInstruction()->getTargetRegister());
  ...
  if (reg != TR::RealRegister::eax) {
    ...
    trfprintf(
      pOutFile, ", eax\t\t%s preserve helper return reg", commentString());
    bufferPos += 2;
  }
  ...
  TR::X86RegRegInstruction* instr =
    snippet->getConvertInstruction()->getIA32RegRegInstruction();
...
uint32_t
TR::X86FPConvertToIntSnippet::getLength(int32_t estimatedSnippetStart)
{
  uint32_t length = 11;
  TR::Machine* machine = cg()->machine();

  if (toRealRegister(_convertInstruction->getTargetRegister())
        ->getRegisterNumber() != TR::RealRegister::eax) {
    // MOV   R, eax
    // XCHG  R, eax
    //
    length += 3;
...
uint8_t*
TR::X86FPConvertToLongSnippet::genFPConversion(uint8_t* buffer)
{
...
TR_Debug::print(TR::FILE* pOutFile, TR::X86FPConvertToLongSnippet* snippet)
{
  ...
  uint8_t action =
    TR::X86FPConvertToLongSnippet::_registerActions[snippet->getAction() &
                                                    0x7f];
...
void
TR::X86FPConvertToLongSnippet::analyseLongConversion()
{
  // The current assumption is that register assignment will occur prior
  // to the snippets being sized and emitted.
  //
  TR_ASSERT(
    ((_loadHighInstruction && _loadHighInstruction->getTargetRegister()) &&
  ...
  _action |=
    ((_doubleRegister->getRegisterNumber() != TR::RealRegister::st0) << 7);
  _action |=
    ((_lowRegister->getRegisterNumber() == TR::RealRegister::eax) << 3);
  _action |=
    ((_lowRegister->getRegisterNumber() == TR::RealRegister::edx) << 2);
  _action |=
    ((_highRegister->getRegisterNumber() == TR::RealRegister::eax) << 1);
  _action |= (_highRegister->getRegisterNumber() == TR::RealRegister::edx);
}

uint32_t
TR::X86FPConvertToLongSnippet::getLength(int32_t estimatedSnippetStart)
{
  uint32_t length = 5;
