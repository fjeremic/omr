...
TR::X86FPConversionSnippet::emitCallToConversionHelper(uint8_t *buffer) {
  ...
  *(int32_t *)buffer = (int32_t)(helperAddress - nextInstructionAddress);
  cg()->addProjectSpecializedRelocation(buffer, (uint8_t *)getHelperSymRef(),
  ...
  *buffer++ = 0xec;
  ...
  if (_convertInstruction->getIA32RegMemInstruction()) {
    ...
    *buffer++ = 0x24;
    ...
    TR_ASSERT(instr != NULL, "f2i conversion instruction must be either "
                             "L4RegMem or CVTTSS2SIRegReg\n");
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86FPConvertToIntSnippet *snippet) {
  if (pOutFile == NULL)
    return;
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::X86FPConvertToLongSnippet *snippet) {
  if (pOutFile == NULL)
    return;
