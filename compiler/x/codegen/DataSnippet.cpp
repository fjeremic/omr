...
TR::X86DataSnippet::X86DataSnippet(TR::CodeGenerator *cg, TR::Node *n, void *c,
                                   size_t size)
    : TR::Snippet(cg, n, TR::LabelSymbol::create(cg->trHeapMemory(), cg),
                  false),
...
void TR::X86DataSnippet::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  if (_isClassAddress) {
    bool needRelocation =
        TR::Compiler->cls.classUnloadAssumptionNeedsRelocation(cg()->comp());
    if (needRelocation && !cg()->comp()->compileRelocatableCode()) {
      cg()->addExternalRelocation(
          new (TR::comp()->trHeapMemory()) TR::ExternalRelocation(
              cursor, NULL, TR_ClassUnloadAssumption, cg()),
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      {
        cg()->jitAddPicToPatchOnClassRedefinition(((void *)-1), (void *)cursor,
                                                  true);
      }
    } else {
      if (!needRelocation)
    ...
    if (clazz && cg()->comp()->compileRelocatableCode() &&
        cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
              cursor, (uint8_t *)clazz, (uint8_t *)TR::SymbolType::typeClass,
...
void TR::X86DataSnippet::printValue(TR::FILE *pOutFile, TR_Debug *debug) {
  ...
  switch (getDataSize()) {
    case 2:
      trfprintf(pOutFile, "0x%04x | %d", 0xffff & (int32_t)getData<int16_t>(),
                (int32_t)getData<int16_t>());
      break;
    case 4:
      trfprintf(pOutFile, "0x%08x | %d | float %g", getData<int32_t>(),
                getData<int32_t>(), getData<float>());
      break;
    case 8:
      trfprintf(pOutFile, "0x%016llx | %lld | double %g", getData<int64_t>(),
                getData<int64_t>(), getData<double>());
      break;
    default:
