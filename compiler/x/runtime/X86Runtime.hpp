...
#include <cpuid.h>
#include <emmintrin.h>
#define cpuid(CPUInfo, EAXValue)                                               \
  __cpuid(EAXValue, CPUInfo[0], CPUInfo[1], CPUInfo[2], CPUInfo[3])
#define cpuidex(CPUInfo, EAXValue, ECXValue)                                   \
  __cpuid_count(                                                               \
    EAXValue, ECXValue, CPUInfo[0], CPUInfo[1], CPUInfo[2], CPUInfo[3])
inline unsigned long long
_xgetbv(unsigned int ecx)
{
 ...
 */
inline void
maskProcessorFlags(TR_X86CPUIDBuffer* pBuffer)
{
...
feGetEnv(const char*);
inline bool
jitGetCPUID(TR_X86CPUIDBuffer* pBuffer)
{
  enum
  {
    EAX = 0,
  ...
  if (CPUInfo[EAX] > 0) {
    ...
    if (pBuffer->_featureFlags2 & TR_OSXSAVE) {
      if (((6 & _xgetbv(0)) != 6) ||
          feGetEnv("TR_DisableAVX")) // '6' = mask for XCR0[2:1]='11b' (XMM
                                     // state and YMM state are enabled)
      {
...
inline bool
AtomicCompareAndSwap(volatile uint32_t* ptr, uint32_t old_val, uint32_t new_val)
{
#if defined(OMR_OS_WINDOWS)
  return old_val == (uint32_t)_InterlockedCompareExchange(
                      (volatile long*)ptr, (long)new_val, (long)old_val);
#else
...
inline bool
AtomicCompareAndSwap(volatile uint16_t* ptr, uint16_t old_val, uint16_t new_val)
{
#if defined(OMR_OS_WINDOWS)
  return old_val == (uint16_t)_InterlockedCompareExchange16(
                      (volatile short*)ptr, (short)new_val, (short)old_val);
#else
  return __sync_bool_compare_and_swap(ptr, old_val, new_val);
#endif /* defined(OMR_OS_WINDOWS) */
}
