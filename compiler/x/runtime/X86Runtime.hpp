...
#define cpuidex(CPUInfo, EAXValue, ECXValue)                                   \
  __cpuid_count(EAXValue, ECXValue, CPUInfo[0], CPUInfo[1], CPUInfo[2],        \
                CPUInfo[3])
...
inline bool AtomicCompareAndSwap(volatile uint32_t *ptr, uint32_t old_val,
                                 uint32_t new_val) {
#if defined(OMR_OS_WINDOWS)
...
inline bool AtomicCompareAndSwap(volatile uint16_t *ptr, uint16_t old_val,
                                 uint16_t new_val) {
#if defined(OMR_OS_WINDOWS)
