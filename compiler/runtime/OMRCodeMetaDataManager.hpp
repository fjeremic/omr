...
public:
  ...
  static TR::CodeMetaDataManager *codeMetaDataManager() {
    return _codeMetaDataManager;
  }
   ...
   * Note, insertMetaData does not check to verify that an metadata's given
   range
  ...
  */
  ...
  /**
   * @brief Determines if the metadata manager contain a reference to a
   particular
   * MethodMetaDataPOD.
   ...
   * reclamation could occur.  This will result in the metadata manager
   containing a
   * different MethodMetaDataPOD for a given startPC.

   * @param  metadata The metadata for which to search for.
   * @return Returns true if the same metadata is found for the metadata 's
   startPC
   *         and false otherwise.
  */
  ...
  /**
   * @brief Remove a given metadata from the metadata manager if it is found
   * therein.
   *
  ...
  /**
   * @brief Attempts to find a registered metadata for a given metadata's
   * startPC.
   *
   * Note: findMetaDataForPC does not locally acquire the JIT metadata monitor.
   * Users must first manually acquire the monitor or go through another
   * function that does.
   *
...
protected:
  ...
  /**
   * @brief Inserts an metadata into the metadata manager causing it to
   ...
   */
  bool insertRange(TR::MethodMetaDataPOD *metaData, uintptr_t startPC,
                   uintptr_t endPC);

  /**
   * @brief Removes an metadata from the metadata manager  causing it to no
   * longer represent a given memory range.
   *
   * Note this method expects to be called via another method in the metadata
   * manager and thus does not acquire the metadata manager's monitor.
   *
   ...
   * representing the given range, false otherwise.
   */
  bool removeRange(const TR::MethodMetaDataPOD *metaData, uintptr_t startPC,
                   uintptr_t endPC);
  ...
  TR::MethodMetaDataPOD *findMetaDataInHash(TR::MetaDataHashTable *table,
                                            uintptr_t searchValue);

  uintptr_t insertMetaDataRangeInHash(TR::MetaDataHashTable *table,
  ...
  allocateMethodStoreInHash(TR::MetaDataHashTable *table);

  uintptr_t
  removeMetaDataRangeFromHash(TR::MetaDataHashTable *table,
                              const TR::MethodMetaDataPOD *dataToRemove,
                              uintptr_t startPC, uintptr_t endPC);

  TR::MethodMetaDataPOD **
  removeMetaDataArrayFromHash(TR::MethodMetaDataPOD **array,
                              const TR::MethodMetaDataPOD *dataToRemove);

  TR::MetaDataHashTable *allocateCodeMetaDataHash(uintptr_t start,
                                                  uintptr_t end);
