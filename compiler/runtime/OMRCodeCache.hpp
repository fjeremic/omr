...
/*
 * The following #defines and typedefs must appear before any #includes in this
 * file
 */
...
class OMR_EXTENSIBLE CodeCache {
 ...
 public:
  ...
  void alignWarmCodeAlloc(uint32_t round) {
    _warmCodeAlloc = align(_warmCodeAlloc, round);
  }
  void alignColdCodeAlloc(uint32_t round) {
    _coldCodeAlloc = align(_coldCodeAlloc, round);
  }

  TR::CodeCache *getNextCodeCache() { return _next; }

  static TR::CodeCache *allocate(TR::CodeCacheManager *manager,
                                 size_t segmentSize,
                                 int32_t reservingCompThreadID);
  void destroy(TR::CodeCacheManager *manager);

  uint8_t *allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize,
                              uint8_t **coldCode, bool needsToBeContiguous,
                              bool isMethodHeaderNeeded = true);

  /**
   * @brief Trims the size of the previously allocated code memory in this
   *        CodeCache to the specified length.  Any leftover bytes are
   * reclaimed.
   *
   ...
   */
  bool trimCodeMemoryAllocation(void *codeMemoryStart,
   ...
   *        This function simply calls reserveSpaceForTrampoline and will exist
   * only while API uses in downstream projects are cleaned up.
   */
  CodeCacheErrorCode::ErrorCode reserveSpaceForTrampoline_bridge(
      int32_t numTrampolines = 1);
   ...
   *    Upon a reservation failure, the code cache is marked as nearing
   * capacity.
   *
   * @param[in] numTrampolines : optional parameter for the number of
   * trampolines to request at once.  Default value is 1.
   *
   * @return : ERRORCODE_SUCCESS on success; ERRORCODE_INSUFFICIENTSPACE on
   * failure
   */
  CodeCacheErrorCode::ErrorCode reserveSpaceForTrampoline(
      int32_t numTrampolines = 1);
  ...
  size_t getFreeContiguousSpace() const {
    return _coldCodeAlloc - _warmCodeAlloc;
  }
  ...
  size_t getSizeOfLargestFreeWarmBlock() const {
    return _sizeOfLargestFreeWarmBlock;
  }
  size_t getSizeOfLargestFreeColdBlock() const {
    return _sizeOfLargestFreeColdBlock;
  }
  ...
  int32_t reserveResolvedTrampoline(TR_OpaqueMethodBlock *method,
                                    bool inBinaryEncoding);
  void createTrampoline(CodeCacheTrampolineCode *trampoline,
                        void *targetStartPC, TR_OpaqueMethodBlock *method);
  bool allocateTempTrampolineSyncBlock();

  void patchCallPoint(TR_OpaqueMethodBlock *method, void *callSite,
  ...
  void setCCPreLoadedCodeAddress(TR_CCPreLoadedCode h, void *a) {
    _CCPreLoadedCode[h] = a;
  }
   ...
   * @param[in] codeCacheSegment : the code cache memory segment that has been
   * allocated
   * @param[in] allocatedCodeCacheSizeInBytes : the size (in bytes) of the
   * allocated code cache
   *
 ...
 private:
  void updateMaxSizeOfFreeBlocks(CodeCacheFreeCacheBlock *blockPtr,
                                 size_t blockSize);
 ...
 public:
  bool addFreeBlock2WithCallSite(uint8_t *start, uint8_t *end, char *file,
                                 uint32_t lineNumber);
   ...
   */
  CodeCacheTempTrampolineSyncBlock *trampolineSyncList() {
    return _trampolineSyncList;
  }

  /**
   * @brief Setter for trampolineSyncList
   *
   * @param[in] sl : A CodeCacheTempTrampolineSyncBlock to be the new head of
   * the trampolineSyncList
   */
  void setTrampolineSyncList(CodeCacheTempTrampolineSyncBlock *sl) {
    _trampolineSyncList = sl;
  }

  /**
   * @brief Getter for the current hashEntrySlab
   *
   * @returns The head of the hashEntrySlab list (i.e., the current
   * hashEntrySlab)
   */
   ...
   *
   * @param[in] hes : A CodeCacheHashEntrySlab to be the new head of the
   * hashEntrySlab list
   */
   ...
   */
  void setTempTrampolineBase(uint8_t *address) {
    _tempTrampolineBase = address;
  }
   ...
   *
   * @param[in] address : The new next allocation address of temporary
   * trampolines
   */
  void setTempTrampolineNext(uint8_t *address) {
    _tempTrampolineNext = address;
  }
   ...
   */
  void setTrampolineAllocationMark(uint8_t *address) {
    _trampolineAllocationMark = address;
  }

  /**
   * @brief Adjusts the trampoline allocation mark by the specified amount
   *
   * @param[in] numBytes : The number of bytes to adjust the allocation mark by.
   * Negative values are permitted
   */
  void adjustTrampolineAllocationMark(int32_t numBytes) {
    _trampolineAllocationMark += numBytes;
  }
   ...
   */
  void setTrampolineReservationMark(uint8_t *address) {
    _trampolineReservationMark = address;
  }

  /**
   * @brief Adjusts the trampoline reservation mark by the specified amount
   *
   * @param[in] numBytes : The number of bytes to adjust the reservation mark
   * by.  Negative values are permitted
   */
  void adjustTrampolineReservationMark(int32_t numBytes) {
    _trampolineReservationMark += numBytes;
  }
  ...
  // flag accessed under mutex <== This is deceiving! There are two different
  // monitors we may hold (not at the same time!) when we write to this. We can
  // either be holding the code cache monitor *OR* the manager's code cache list
  // monitor.
  // TODO: When we move to C++11 replace volatile with something that actually
  // enforces ordering semantics.
