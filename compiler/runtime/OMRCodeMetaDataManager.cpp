...
void CodeMetaDataManager::updateCache(uintptr_t currentPC)
{
  ...
  if (currentPC != _cachedPC) {
    ...
    TR_ASSERT(_cachedHashTable,
      "Either we lost a code cache or we attempted to find a hash table for a non-code cache startPC: Searched for %p",
      currentPC);
  }
}
...
#define DETERMINE_BUCKET(value, start, buckets) \
  (((((uintptr_t)(value) - (uintptr_t)(start)) >> (uintptr_t)9) * (uintptr_t)sizeof(uintptr_t)) + (uintptr_t)(buckets))
  ...
  TR::MetaDataHashTable* table, const TR::MethodMetaDataPOD* dataToRemove, uintptr_t startPC, uintptr_t endPC)
{
  ...
  do {
    if (LOW_BIT_SET(*index)) {
      ...
      temp
        = (TR::MethodMetaDataPOD*)(self()->removeMetaDataArrayFromHash((TR::MethodMetaDataPOD**)*index, dataToRemove));
      if (!temp)
        return (uintptr_t)1;
...
TR::MetaDataHashTable* CodeMetaDataManager::allocateCodeMetaDataHash(uintptr_t start, uintptr_t end)
{
  ...
  table
    = (TR::MetaDataHashTable*)TR_Memory::jitPersistentAlloc(sizeof(TR::MetaDataHashTable), TR_Memory::CodeMetaDataAVL);
