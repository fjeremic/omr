...
void
ia32CodeCacheParameters(int32_t*, void**, int32_t*, int32_t*);
void
amd64CodeCacheParameters(int32_t*, void**, int32_t*, int32_t*);
void
ppcCodeCacheParameters(int32_t*, void**, int32_t*, int32_t*);
void
armCodeCacheParameters(int32_t*, void**, int32_t*, int32_t*);
void
s390zLinux64CodeCacheParameters(int32_t*, void**, int32_t*, int32_t*);

uint32_t
getPPCCacheLineSize();

extern void
setDllSlip(char* codeStart, char* codeEnd, char* dllName, TR::Compilation*);

void
initializeJitRuntimeHelperTable(char jvmpi);
...
{
public:
  static TR_LinkageInfo* get(void* startPC)
  {
    return (TR_LinkageInfo*)(((uint32_t*)startPC) - 1);
  }
  ...
  bool hasFailedRecompilation()
  {
    return (_word & HasFailedRecompilation) != 0;
  }
  bool recompilationAttempted()
  {
    return hasBeenRecompiled() || hasFailedRecompilation();
  }
  bool isBeingCompiled() { return (_word & IsBeingRecompiled) != 0; }

  inline uint16_t getReservedWord() { return (_word & ReservedMask) >> 16; }
  inline void setReservedWord(uint16_t w)
  {
    _word |= ((w << 16) & ReservedMask);
  }
  ...
  enum
  {
    ...
    // NOTE: flags have to be set under the compilation monitor or during
    // compilation process
...
public:
   ...
   * \brief
   *
   * Looks up the given helper table and returns the function pointer if it's a
   * function index. For constant number entries, return
   ...
   * \brief
   *
   * For helper functions, this API returns the raw entry address. If the
   * runtime helper's corresponding linkage is not a function, return the helper
   * as data.
   *
   */
   ...
   * \brief
   *
   * Returns the linkage type of the given helper index. The linkage types are
   * set during the JIT initilization phase.
   */
   ...
   * For example, on X86, there are private linkage, System V ABI, fastcall,
   * cdecl, etc.
   */
  void setAddress(TR_RuntimeHelper h,
                  void* a,
                  TR_LinkageConventions lc = TR_Helper)
  {
    _helpers[h] = a;
    _linkage[h] = lc;
  }
 ...
 * The values stored in the helper table are either entry point addresses or
 * constant values, depending on the linkage type.
 */
inline void*
runtimeHelperValue(TR_RuntimeHelper h)
{
  return runtimeHelpers.getFunctionEntryPointOrConst(h);
}
inline TR_LinkageConventions
runtimeHelperLinkage(TR_RuntimeHelper h)
{
  return runtimeHelpers.getLinkage(h);
}
...
#define RELOCATION_CROSS_PLATFORM_FLAGS_MASK                                   \
  (RELOCATION_TYPE_EIP_OFFSET | RELOCATION_TYPE_WIDE_OFFSET)
...
// These macros are intended for use when HI_VALUE and LO_VALUE will be
// recombined after LO_VALUE is sign-extended (e.g. when LO_VALUE is used with
// an instruction that takes a signed 16-bit operand). In this case we have to
// adjust HI_VALUE now so that the original value will be obtained once the two
// are recombined.
...
/*
 * Define some internal temporary types for use during compilation which will
 * get converted to actual relocation types later on (purposely all capital to
 * differentiate from actual relo types and larger than actual relocation type
 * so we catch any incorrect use that treats the temp type as actual relocation
 * type)
 */
enum
{
...
// Multi Code Cache Routine for S390 for checking whether an entry point is
// within reach of a RIL instruction. In RIL instruction, the relative address
// is specified in number of half words.
#define CHECK_32BIT_TRAMPOLINE_RANGE(x, rip)                                   \
  (((intptrj_t)(x) ==                                                          \
    (intptrj_t)(rip) +                                                         \
      ((intptrj_t)((int32_t)(((intptrj_t)(x) - (intptrj_t)(rip)) / 2))) *      \
        2) &&                                                                  \
   (x % 2 == 0))

// Routine to check trampoline range for x86-64
#define IS_32BIT_RIP_JUMP(x, rip)                                              \
  ((intptrj_t)(x) ==                                                           \
