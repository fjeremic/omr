...
public:
  ...
  RuntimeAssumption* getNextAssumptionForSameJittedBody() const
  {
    ...
    while (toReturn && toReturn->isMarkedForDetach() && toReturn != this)
      toReturn =
        (RuntimeAssumption*)(((uintptr_t)
                                toReturn->_nextAssumptionForSameJittedBody) &
                             ~MARK_FOR_DELETE);
  ...
  bool isAssumingRange(uintptrj_t rangeStartPC,
...
{
protected:
  PatchNOPedGuardSite(TR_PersistentMemory* pm,
                      uintptrj_t key,
                      TR_RuntimeAssumptionKind kind,
                      uint8_t* location,
                      uint8_t* destination)
...
{
protected:
  PatchMultipleNOPedGuardSites(TR_PersistentMemory* pm,
                               uintptrj_t key,
                               TR_RuntimeAssumptionKind kind,
                               PatchSites* sites)
    : OMR::LocationRedirectRuntimeAssumption(pm, key)
...
public:
  virtual void compensate(TR_FrontEnd* vm, bool isSMP, void*)
  {
    for (size_t i = 0; i < _patchSites->getSize(); ++i)
      TR::PatchNOPedGuardSite::compensate(
        isSMP, _patchSites->getLocation(i), _patchSites->getDestination(i));
  }
