...
class RuntimeAssumption {
 ...
 protected:
  ...
  void addToRAT(TR_PersistentMemory *persistentMemory,
                TR_RuntimeAssumptionKind kind, TR_FrontEnd *fe,
                RuntimeAssumption **sentinel);

  /**
   * Directly return the next runtime assumption regardless of whether it is
   * marked for detach or not
   *
   * Note this API is only intended for use by utility methods of the
   * RuntimeAssumptionTable and must be run under the assumptionTableMutex
   */
   ...
   *
   * Note this API is only intended for use by utility methods of teh
   * RuntimeAssumptionTable and must be run under the assumptionTableMutex
   */
  ...
  RuntimeAssumption *getNextAssumptionForSameJittedBodyEvenIfDead() const {
    return (RuntimeAssumption *)(((uintptr_t)_nextAssumptionForSameJittedBody) &
                                 ~MARK_FOR_DELETE);
  }

  /**
   * Update the next pointer for the same jitted body to the specified
   * RuntimeAssumption
   *
   * This update preserves the MARK_FOR_DELETE bit.
   * Note this API only intended for use by utility methods of the
   * RuntimeAssumptionTable and must be run under the assumptionTableMutex
   */
  void setNextAssumptionForSameJittedBody(RuntimeAssumption *link) {
    _nextAssumptionForSameJittedBody =
        (RuntimeAssumption *)((uintptr_t)link |
                              (((uintptr_t)_nextAssumptionForSameJittedBody) &
                               MARK_FOR_DELETE));
  }
  ...
  /**
   * This utility destroys the data in the RuntimeAssumption to facilitate
   * debugging
   */
  void paint() {
    _key = 0xDEADF00D;
  ...
  void markForDetach() {
    _nextAssumptionForSameJittedBody =
        (RuntimeAssumption *)(((uintptr_t)_nextAssumptionForSameJittedBody) |
                              MARK_FOR_DELETE);
  }

  /**
   * Returns true when the assumption has previously been marked using the
   * markForDetach routine
   */
  bool isMarkedForDetach() const {
    return (((uintptr_t)(_nextAssumptionForSameJittedBody)&MARK_FOR_DELETE) ==
            MARK_FOR_DELETE);
  }
   ...
   *
   *  \note
   *     This function should be called when we attempt to reclaim the runtime
   * assumption right before deallocation. Calling this function too early can
   * result in potentially accessing already deallocated memory, as the runtime
   * assumption may still be examined/processed even after calling `compensate`
   * and all the way up until the runtime assumption is deallocated.
   */
 ...
 public:
  ...
  RuntimeAssumption *getNextAssumptionForSameJittedBody() const {
    ...
    while (toReturn && toReturn->isMarkedForDetach() && toReturn != this)
      toReturn =
          (RuntimeAssumption *)(((uintptr_t)toReturn
                                     ->_nextAssumptionForSameJittedBody) &
                                ~MARK_FOR_DELETE);
    // handle the case where you have gone full circle
    return toReturn != this || !toReturn->isMarkedForDetach()
               ? toReturn
               : const_cast<RuntimeAssumption *>(this);
  }
  ...
  /*
   * These functions are used to determine whether the runtime assumption falls
   * within a given range. Both bounds are inclusive.
   */
  virtual uint8_t *getFirstAssumingPC() = 0;
  virtual uint8_t *getLastAssumingPC() = 0;
  bool assumptionInRange(uintptrj_t start, uintptrj_t end) {
    return ((uintptrj_t)getFirstAssumingPC()) <= end &&
           start <= ((uintptrj_t)getLastAssumingPC());
  }

  virtual uintptrj_t getKey() { return _key; }

  virtual uintptrj_t hashCode() {
    return TR_RuntimeAssumptionTable::hashCode(getKey());
  }
  ...
  bool isAssumingMethod(void *metaData,
                        bool reclaimPrePrologueAssumptions = false);
  bool isAssumingRange(uintptrj_t rangeStartPC, uintptrj_t rangeEndPC,
                       uintptrj_t rangeColdStartPC, uintptrj_t rangeColdEndPC,
                       uintptrj_t rangeStartMD, uintptrj_t rangeEndMD);

 private:
  RuntimeAssumption *_next;
  RuntimeAssumption
      *_nextAssumptionForSameJittedBody;  // links assumptions that pertain to
                                          // the same method body These should
                                          // form a circular linked list with a
                                          // sentinel Lower bit is used to mark
                                          // assumptions that will be lazily
                                          // removed
 protected:
...
class LocationRedirectRuntimeAssumption : public RuntimeAssumption {
 protected:
  ...
  virtual RuntimeAssumptionCategory getAssumptionCategory() {
...
class SentinelRuntimeAssumption : public OMR::RuntimeAssumption {
 public:
  SentinelRuntimeAssumption() : RuntimeAssumption(NULL, 0) {
    setNextAssumptionForSameJittedBody(
        this);  // pointing to itself means that the list is empty
  }
  virtual TR_RuntimeAssumptionKind getAssumptionKind() {
    return RuntimeAssumptionSentinel;
  }
  virtual OMR::RuntimeAssumptionCategory getAssumptionCategory() {
    return OMR::SentinelCategory;
  }
...
class PatchNOPedGuardSite : public OMR::LocationRedirectRuntimeAssumption {
 protected:
  PatchNOPedGuardSite(TR_PersistentMemory *pm, uintptrj_t key,
                      TR_RuntimeAssumptionKind kind, uint8_t *location,
                      uint8_t *destination)
 ...
 public:
  ...
  virtual void compensate(TR_FrontEnd *vm, bool isSMP, void *) {
    compensate(isSMP, _location, _destination);
  }
...
class PatchMultipleNOPedGuardSites
    : public OMR::LocationRedirectRuntimeAssumption {
 protected:
  PatchMultipleNOPedGuardSites(TR_PersistentMemory *pm, uintptrj_t key,
                               TR_RuntimeAssumptionKind kind, PatchSites *sites)
      : OMR::LocationRedirectRuntimeAssumption(pm, key), _patchSites(sites) {}

 public:
  virtual void compensate(TR_FrontEnd *vm, bool isSMP, void *) {
    for (size_t i = 0; i < _patchSites->getSize(); ++i)
      TR::PatchNOPedGuardSite::compensate(isSMP, _patchSites->getLocation(i),
  ...
  virtual uint8_t *getFirstAssumingPC() {
    return _patchSites->getFirstLocation();
  }
  virtual uint8_t *getLastAssumingPC() {
    return _patchSites->getLastLocation();
  }
