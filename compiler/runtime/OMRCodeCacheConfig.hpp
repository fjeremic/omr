...
struct CodeCacheCodeGenCallbacks {
  void (*codeCacheConfig)(int32_t codeCacheSizeInBytes,
                          uint32_t *numTempTrampolines);

  void (*createHelperTrampolines)(uint8_t *helperBase, int32_t helperCount);

  void (*createMethodTrampoline)(void *trampoline, void *targetStartPC,
                                 TR_OpaqueMethodBlock *method);

  int (*patchTrampoline)(void *method, void *callingPoint, void *currentStartPC,
                         void *currentTrampoline, void *newStartPC,
                         void *extraArg);

  void (*createCCPreLoadedCode)(uint8_t *CCPreLoadedCodeBase,
                                uint8_t *CCPreLoadedCodeTop,
                                void **CCPreLoadedCode, TR::CodeGenerator *cg);
};

class OMR_EXTENSIBLE CodeCacheConfig {
public:
  CodeCacheConfig()
      : _trampolineCodeSize(0), _CCPreLoadedCodeSize(0),
        _numOfRuntimeHelpers(0), _codeCacheKB(0), _codeCacheTotalKB(0),
        _codeCachePadKB(0), _codeCacheAlignment(0),
        ...
        _codeCacheHashEntryAllocatorSlabSize(4096), _largeCodePageSize(0),
        _largeCodePageFlags(0), _allowedToGrowCache(false),
        _needsMethodTrampolines(false), _trampolineSpacePercentage(0),
        _lowCodeCacheThreshold(0), _maxNumberOfCodeCaches(0),
        _canChangeNumCodeCaches(false), _verbosePerformance(false),
        _verboseCodeCache(false), _verboseReclamation(false),
        _doSanityChecks(false), _codeCacheFreeBlockRecylingEnabled(false),
        _emitExecutableELF(false), _emitRelocatableELF(false) {
  ...
  int32_t _CCPreLoadedCodeSize; /*!< size of the pre-Loaded CodeCache Helpers
                                   code in bytes */
  ...
  bool _verbosePerformance; /*!< should performance related events be reported
                               to the verbose log */
  bool _verboseCodeCache;   /*!< should code cache events be reported to the
                               verbose log */
  bool _verboseReclamation; /*!< should reclamation events be reported to the
                               verbose log */
