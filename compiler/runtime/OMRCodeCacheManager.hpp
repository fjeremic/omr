...
typedef struct CodeCacheSymbolContainer {
  ...
  uint32_t
      _totalSymbolNameLength; /**< Sum of the symbol names in the linked list */
...
typedef struct CodeCacheRelocationInfo {
  ...
  uint32_t _symbol; /**< index of the symbol to be relocated in the linked list
                       of CodeCacheSymbol, with head symbol being 0 */
...
typedef struct CodeCacheRelocationInfoContainer {
  CodeCacheRelocationInfo
      *_head; /**< First CodeCacheRelocationInfo structure */
  CodeCacheRelocationInfo
      *_tail; /**< Latest CodeCacheRelocationInfo structure added */
  uint32_t _numRelocations; /**< Total number of CodeCacheRelocationInfor in the
                               linked list */
...
public:
  ...
  TR::CodeCache *initialize(bool useConsolidatedCache,
                            uint32_t numberOfCodeCachesToCreateAtStartup);

  void destroy();

  // These two functions are for allocating Native backing memory for code cache
  // structures In future these facilities should probably be implemented via
  // cs2 allocators
  void *getMemory(size_t sizeInBytes);
  void freeMemory(void *memoryToFree);

  TR::CodeCache *
  allocateCodeCacheObject(TR::CodeCacheMemorySegment *codeCacheSegment,
  ...
  getNewCodeCacheMemorySegment(size_t segmentSize,
                               size_t &codeCacheSizeAllocated);

  void unreserveCodeCache(TR::CodeCache *codeCache);
  TR::CodeCache *
  reserveCodeCache(bool compilationCodeAllocationsMustBeContiguous,
                   size_t sizeEstimate, int32_t compThreadID,
                   int32_t *numReserved);
  ...
  uint8_t *allocateCodeMemory(size_t warmCodeSize, size_t coldCodeSize,
                              TR::CodeCache **codeCache_pp, uint8_t **coldCode,
   ...
   *                  == -1 : the application thread is requesting the
   * allocation; new code cache will be reserved
   *                  >=  0 : a compilation thread is requesting the allocation;
   * new code cache will be reserved
   *
   * @return if allocation is successful, the allocated TR::CodeCache object
   * from the new segment; NULL otherwise.
   */
  TR::CodeCache *
  allocateCodeCacheFromNewSegment(size_t segmentSizeInBytes,
  ...
  CodeCacheTrampolineCode *findMethodTrampoline(TR_OpaqueMethodBlock *method,
                                                void *callingPC);
  ...
  void performSizeAdjustments(size_t &warmCodeSize, size_t &coldCodeSize,
                              bool needsToBeContiguous,
  ...
  TR::CodeCacheMemorySegment *
  allocateCodeCacheRepository(size_t repositorySize);
  TR::CodeCacheMemorySegment *
  carveCodeCacheSpaceFromRepository(size_t segmentSize,
                                    size_t &codeCacheSizeToAllocate);
  void undoCarvingFromRepository(TR::CodeCacheMemorySegment *segment);
  TR::CodeCacheMemorySegment *
  setupMemorySegmentFromRepository(uint8_t *start, uint8_t *end,
                                   size_t &codeCacheSizeToAllocate);
  void freeMemorySegment(TR::CodeCacheMemorySegment *segment);

  // sneaky accounting to provide the right external perception of how much
  // space is used
  ...
  TR::CodeCache *getRepositoryCodeCacheAddress() {
    return _repositoryCodeCache;
  }
  TR::Monitor *getCodeCacheRepositoryMonitor() {
    return _codeCacheRepositoryMonitor;
  }

  uint8_t *allocateCodeMemoryWithRetries(
  ...
  bool lowCodeCacheSpaceThresholdReached() {
    return _lowCodeCacheSpaceThresholdReached;
  }

  void repositoryCodeCacheCreated();
  void registerCompiledMethod(const char *sig, uint8_t *startPC,
                              uint32_t codeSize);
...
protected:
  ...
  bool _initialized; /*!< flag to indicate if code cache manager has been
                        initialized or not */
...
protected:
  TR::ELFExecutableGenerator
      *_elfExecutableGenerator; /**< Executable ELF generator */
  TR::ELFRelocatableGenerator
      *_elfRelocatableGenerator; /**< Relocatable ELF generator */

  // collect information on code cache symbols here, will be post processed into
  // the elf trailer structure
  static TR::CodeCacheSymbolContainer
      *_symbolContainer; /**< Symbol container used for tracking
                            CodeCacheSymbols. Note: This static member is not
                            thread-safe. Synchronization is needed if multiple
                            compilation threads are active */
  TR::CodeCacheSymbolContainer
      *_relocatableSymbolContainer; /**< Symbol container used for tracking
                                       CodeCacheSymbols, for the purpose of
                                       writing to relocatable ELF object file */
  TR::CodeCacheRelocationInfoContainer
      *_relocations; /**< for tracking relocation info */
  TR::ELFRelocationResolver
      _resolver; /**< this translates between a TR::StaticRelocation and the ELF
                    relocation type required for the platform */
  const char *_objectFileName; /**< filename of the object file to generate,
                                  obtained from cmd line options */
