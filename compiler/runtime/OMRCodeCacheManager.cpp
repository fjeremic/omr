...
TR::CodeCache*
OMR::CodeCacheManager::initialize(bool allocateMonolithicCodeCache,
...
TR::CodeCache*
OMR::CodeCacheManager::allocateCodeCacheObject(
...
// affinity.
TR::CodeCache*
OMR::CodeCacheManager::reserveCodeCache(
                                         ...
                                         bool needSync)
{
  ...
  TR::CodeCache* codeCache = self()->findCodeCacheFromPC(callSite);
  return codeCache->replaceTrampoline(
    method, oldTrampoline, oldTargetPC, newTargetPC, needSync);
}
...
//
uint8_t*
OMR::CodeCacheManager::allocateCodeMemoryWithRetries(
  size_t warmCodeSize,
  size_t coldCodeSize,
  TR::CodeCache** codeCache_pp,
  ...
  bool isMethodHeaderNeeded)
{
  ...
  TR_ASSERT(codeCache->isReserved(),
            "Code cache must be reserved retries=%d",
            allocationRetries);
  int32_t compThreadID =
      ...
      *codeCache_pp = codeCache; // code cache has been switched // MCT
      ...
      if (config.verboseCodeCache()) {
        TR_VerboseLog::writeLineLocked(
          ...
          codeCache,
          codeCache->getCodeBase(),
          codeCache->getCodeTop());
      }
      ...
      warmCodeAddress =
        self()->allocateCodeMemoryWithRetries(warmCodeSize,
                                              coldCodeSize,
                                              codeCache_pp,
                                              allocationRetries,
                                              coldCode,
...
#ifdef MCT_DEBUG
  fprintf(stderr,
          "Cache %p has %u bytes free. We requested %u bytes\n",
          *codeCache_pp,
          (*codeCache_pp)->getFreeContiguousSpace(),
          warmCodeSize + coldCodeSize);
#endif
...
void
OMR::CodeCacheManager::registerCompiledMethod(const char* sig,
                                              uint8_t* startPC,
                                              uint32_t codeSize)
{
#if (HOST_OS == OMR_LINUX)
...
void
OMR::CodeCacheManager::registerStaticRelocation(
  ...
  size_t& codeCacheSizeToAllocate)
{
  ...
  if (config.verboseCodeCache()) {
    if (start)
      ...
      TR_VerboseLog::writeLineLocked(
        TR_Vlog_FAILURE,
        "failed to carve size=%lu. Free space = %u",
        codeCacheSizeToAllocate,
        freeSpace);
  }
  ...
  if (start) {
    TR::CodeCacheMemorySegment* memorySegment =
      self()->setupMemorySegmentFromRepository(
        start, end, codeCacheSizeToAllocate);
...
//----------------------------
void
OMR::CodeCacheManager::undoCarvingFromRepository(
...
OMR::CodeCacheManager::initializeRelocatableELFGenerator(void)
{
  ...
  _elfRelocatableGenerator = new (_rawAllocator)
    TR::ELFRelocatableGenerator(_rawAllocator,
                                _codeCacheRepositorySegment->segmentBase(),
                                _codeCacheRepositorySegment->segmentTop() -
                                  _codeCacheRepositorySegment->segmentBase());
}
...
OMR::CodeCacheManager::initializeExecutableELFGenerator(void)
{
  ...
  _elfExecutableGenerator = new (_rawAllocator)
    TR::ELFExecutableGenerator(_rawAllocator,
                               _codeCacheRepositorySegment->segmentBase(),
                               _codeCacheRepositorySegment->segmentTop() -
                                 _codeCacheRepositorySegment->segmentBase());
}
