...
TR::CodeCache* OMR::CodeCacheManager::findCodeCacheFromPC(void* inCacheAddress)
{
  ...
  while (codeCache) {
    ...
    if ((uint8_t*)inCacheAddress >= codeCache->getCodeBase() && (uint8_t*)inCacheAddress <= codeCache->getHelperTop())
      return codeCache;
...
intptrj_t OMR::CodeCacheManager::findHelperTrampoline(int32_t helperIndex, void* callSite)
{
  ...
  bool isMethodHeaderNeeded)
{
  ...
  warmCodeAddress
    = codeCache->allocateCodeMemory(warmCodeSize, coldCodeSize, coldCode, needsToBeContiguous, isMethodHeaderNeeded);
  ...
  if (allocationRetries > 0) {
    ...
    if (codeCache) // found an appropriate code cache
    {
      ...
      *codeCache_pp = codeCache; // code cache has been switched // MCT
      ...
      if (needsToBeContiguous) // We should abort the compilation if we change the cache during a compilation that needs
  ...
  *codeCache_pp = codeCache;
  ...
  warmCodeAddress = self()->allocateCodeMemoryWithRetries(
    warmCodeSize, coldCodeSize, codeCache_pp, allocationRetries, coldCode, needsToBeContiguous, isMethodHeaderNeeded);
...
TR::CodeCacheMemorySegment* OMR::CodeCacheManager::allocateCodeCacheRepository(size_t repositorySize)
{
  ...
  _codeCacheRepositorySegment = self()->allocateCodeCacheSegment(repositorySize, codeCacheSizeAllocated, startAddress);
  ...
  size_t segmentSize, size_t& codeCacheSizeToAllocate)
{
  ...
  if (config.verboseCodeCache()) {
    if (start)
      TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
        "carved size=%u range: " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT, codeCacheSizeToAllocate, start, end);
    else
      TR_VerboseLog::writeLineLocked(
        TR_Vlog_FAILURE, "failed to carve size=%lu. Free space = %u", codeCacheSizeToAllocate, freeSpace);
  }
...
void OMR::CodeCacheManager::freeMemorySegment(TR::CodeCacheMemorySegment* segment)
{
  segment->free(self());
}
