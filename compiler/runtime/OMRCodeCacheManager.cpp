...
#if !defined(TR_HOST_POWER)
        || (!TR::Options::getCmdLineOptions()->getOption(TR_StressTrampolines) && self()->usingRepository() && config.codeCacheTotalKB() <= REACHEABLE_RANGE_KB)
#endif
    );
            ...
            if (!codeCache->isReserved()) // we cannot touch the reserved ones
            {
                ...
                if (almostFull == TR_no || (almostFull == TR_maybe && !compilationCodeAllocationsMustBeContiguous)) {
                    ...
                    if (sizeEstimate == 0 || // If size estimate is not given we'll blindly pick anything
                        codeCache->getFreeContiguousSpace() >= sizeEstimate || codeCache->getSizeOfLargestFreeWarmBlock() >= sizeEstimate // we don't know yet the warm/cold requirements
    ...
    bool isMethodHeaderNeeded)
{
    ...
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    size_t allocateSize = 2 * (warmCodeSize + coldCodeSize + config.numRuntimeHelpers() * config.trampolineCodeSize() + config.ccPreLoadedCodeSize());
