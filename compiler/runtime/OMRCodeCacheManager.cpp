...
TR::CodeCache *OMR::CodeCacheManager::initialize(
  ...
  if (_symbolContainer == NULL) {
    ...
    symbolContainer->_totalSymbolNameLength = 1; // precount for the UNDEF
                                                 // symbol
...
void OMR::CodeCacheManager::destroy() {
  ...
  if (_elfRelocatableGenerator) {
    TR_ASSERT(_elfRelocatableGenerator->emitELF(
...
OMR::CodeCacheManager::findMethodTrampoline(TR_OpaqueMethodBlock *method,
                                            void *callingPC) {
  TR::CodeCacheConfig &config = self()->codeCacheConfig();
