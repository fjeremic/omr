...
uint8_t* OMR::CodeCache::getCodeAlloc() {
  return _segment->segmentAlloc();
}

uint8_t* OMR::CodeCache::getCodeBase() {
  return _segment->segmentBase();
}

uint8_t* OMR::CodeCache::getCodeTop() {
...
void OMR::CodeCache::writeMethodHeader(void* freeBlock,
                                       size_t size,
                                       bool isCold) {
...
bool OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t* start,
                                               uint8_t* end,
                                               char* file,
                                               uint32_t lineNumber) {
  TR::CodeCacheConfig& config = _manager->codeCacheConfig();
...
void OMR::CodeCache::updateMaxSizeOfFreeBlocks(
    CodeCacheFreeCacheBlock* blockPtr,
    size_t blockSize) {
  TR::CodeCacheConfig& config = _manager->codeCacheConfig();
...
//
uint8_t* OMR::CodeCache::findFreeBlock(size_t size,
                                       bool isCold,
...
void OMR::CodeCache::checkForErrors() {
  if (_freeBlockList) {
    ...
    // scope for cache walk
    {
      ...
      TR::CodeCacheConfig& config = _manager->codeCacheConfig();
      uint8_t* start = align(((uint8_t*)_segment->segmentTop()) + sizeof(char*),
