    ...
    size_t allocatedCodeCacheSizeInBytes)
{
    ...
    *((TR::CodeCache**)(_segment->segmentBase())) = self(); // Write a pointer to this cache at the beginning of the segment
    ...
    if (!config.needsMethodTrampolines()) {
        ...
        _trampolineBase = _helperBase - ((_helperBase - _segment->segmentBase()) * config.trampolineSpacePercentage() / 100);
    ...
    uint32_t lineNumber)
{
    ...
    if (_freeBlockList) {
        ...
        } else if (curr->_next && ((uint8_t*)curr->_next - end < sizeof(CodeCacheFreeCacheBlock)) && !(start < _warmCodeAlloc && (uint8_t*)curr->_next >= _coldCodeAlloc)) {
            // merge with the next block, but don't merge warm blocks with cold blocks
            if ((start - ((uint8_t*)curr + curr->_size) < sizeof(CodeCacheFreeCacheBlock)) && !((uint8_t*)curr < _warmCodeAlloc && start >= _coldCodeAlloc)) {
...
OMR::CodeCache::findFreeBlock(size_t size, bool isCold, bool isMethodHeaderNeeded)
{
    ...
    TR::CodeCacheConfig& config = _manager->codeCacheConfig();
    if (!isCold) {
        TR_ASSERT(!config.codeCacheFreeBlockRecylingEnabled() || _sizeOfLargestFreeWarmBlock == biggestLink->_size, "_sizeOfLargestFreeWarmBlock=%d  biggestLink->_size=%d",
            _sizeOfLargestFreeWarmBlock, (int32_t)biggestLink->_size);
    } else {
        TR_ASSERT(!config.codeCacheFreeBlockRecylingEnabled() || _sizeOfLargestFreeColdBlock == biggestLink->_size, "assertion failure");
    }
    ...
    bool isMethodHeaderNeeded)
{
    ...
    if (coldSize != 0 && !coldIsFreeBlock && coldSize + (warmIsFreeBlock ? 0 : warmSize) > self()->getFreeContiguousSpace()) {
        return NULL;
    }
