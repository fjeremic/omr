...
OMR::CodeCache::writeMethodHeader(void* freeBlock, size_t size, bool isCold)
{
  ...
  if (!isCold)
    memcpy(
      block->_eyeCatcher, config.warmEyeCatcher(), sizeof(block->_eyeCatcher));
  else
    memcpy(
      block->_eyeCatcher, config.coldEyeCatcher(), sizeof(block->_eyeCatcher));
  block->_metaData = NULL;
}

bool
OMR::CodeCache::trimCodeMemoryAllocation(void* codeMemoryStart,
                           ...
                           size_t allocatedCodeCacheSizeInBytes)
{
  ...
  *((TR::CodeCache**)(_segment->segmentBase())) =
  ...
  _CCPreLoadedCodeBase = _CCPreLoadedCodeTop - config.ccPreLoadedCodeSize();
  TR_ASSERT(
    (((size_t)_CCPreLoadedCodeBase) & config.codeCacheHelperAlignmentMask()) ==
      0,
    "Per-code cache helper sizes do not account for alignment requirements.");
  _coldCodeAlloc = _CCPreLoadedCodeBase;
  ...
  if (size) {
    ...
    if (_trampolineReservationMark >= _trampolineBase + size) {
      ...
      if (config.verboseCodeCache()) {
        TR_VerboseLog::writeLineLocked(
          TR_Vlog_CODECACHE,
          "CodeCache %p marked as full in reserveSpaceForTrampoline",
          self());
      }
    }
  }
      ...
      for (CodeCacheHashEntry* entry = _resolvedMethodHT->_buckets[entryIdx];
        ...
        if (trampoline && entry->_info._resolved._currentStartPC != newPC) {
          self()->createTrampoline(
            trampoline, newPC, entry->_info._resolved._method);
          entry->_info._resolved._currentStartPC = newPC;
        }
      }
    }
  ...
  // scope to patch trampoline
  {
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(
      ...
      TR_VerboseLog::writeLineLocked(
        ...
        "j9method=0x%p,resolvedTramp=0x%p,methodRunAddress=0x%p,newStartPC=0x%"
        "p,extraArg=0x%p",
        callSite,
        method,
        resolvedTramp,
...
{
  TR::CodeCacheConfig& config = _manager->codeCacheConfig();
  config.mccCallbacks().createMethodTrampoline(
    trampoline, targetStartPC, method);
}
                                          ...
                                          uint32_t lineNumber)
{
  ...
  if (end <= (start + sizeof(CodeCacheFreeCacheBlock))) {
    if (config.verboseReclamation()) {
      TR_VerboseLog::writeLineLocked(
        ...
        "0x%016x alignment = 0x%04x sizeof(CodeCacheFreeCacheBlock) = 0x%08x",
        file,
        lineNumber,
        start_o,
        end,
        config.codeCacheAlignment(),
  ...
  if (config.verboseReclamation()) {
    TR_VerboseLog::writeLineLocked(
      TR_Vlog_CODECACHE,
      "--ccr-- addFreeBlock2WithCallSite CC=%p start=%p end=%p mergedBlock=%p "
      "link=%p link->_size=%u, _sizeOfLargestFreeWarmBlock=%d "
...
void
OMR::CodeCache::updateMaxSizeOfFreeBlocks(CodeCacheFreeCacheBlock* blockPtr,
                                          size_t blockSize)
{
  TR::CodeCacheConfig& config = _manager->codeCacheConfig();
...
//
uint8_t*
OMR::CodeCache::findFreeBlock(size_t size,
                              bool isCold,
...
{
  fprintf(stderr,
          "Code Cache @%p flags=0x%x almostFull=%d\n",
          this,
          _flags,
          _almostFull);
  fprintf(stderr,
          "   cold-warm hole size        = %8" OMR_PRIuSIZE " bytes\n",
          self()->getFreeContiguousSpace());
  fprintf(stderr,
          "   warmCodeAlloc=%p coldCodeAlloc=%p\n",
          (void*)_warmCodeAlloc,
          (void*)_coldCodeAlloc);
  if (_freeBlockList) {
    fprintf(stderr,
  ...
  if (config.trampolineCodeSize()) {
    fprintf(stderr,
            "   trampoline free space = %d (temp=%d)\n",
