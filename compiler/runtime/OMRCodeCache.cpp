...
bool OMR::CodeCache::trimCodeMemoryAllocation(void* codeMemoryStart, size_t actualSizeInBytes)
{
  ...
  TR_ASSERT(
    cacheHeader->_eyeCatcher[0] == config.warmEyeCatcher()[0], "Missing eyecatcher during trimCodeMemoryAllocation");
  ...
  if (_flags & CODECACHE_FULL_SYNC_REQUIRED) {
    ...
    for (syncBlock = _trampolineSyncList; syncBlock; syncBlock = syncBlock->_next) {
      ...
      for (uint32_t entryIdx = 0; entryIdx < syncBlock->_entryCount; entryIdx++) {
        ...
        self()->createTrampoline(entry->_info._resolved._currentTrampoline, newPC, entry->_info._resolved._method);
        entry->_info._resolved._currentStartPC = newPC;
      }
...
bool OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t* start, uint8_t* end, char* file, uint32_t lineNumber)
{
  TR::CodeCacheConfig& config = _manager->codeCacheConfig();
...
uint8_t* OMR::CodeCache::findFreeBlock(size_t size, bool isCold, bool isMethodHeaderNeeded)
{
  ...
  if (bestFitLink) {
    ...
    if (config.verboseReclamation()) {
      TR_VerboseLog::writeLineLocked(TR_Vlog_CODECACHE,
        "--ccr- findFreeBlock: CodeCache=%p size=%u isCold=%d bestFitLink=%p bestFitLink->size=%u leftBlock=%p", this,
        size, isCold, bestFitLink, bestFitLink->_size, leftBlock);
    }
  }
...
void OMR::CodeCache::printOccupancyStats()
{
  ...
  if (config.trampolineCodeSize()) {
    fprintf(stderr, "   trampoline free space = %d (temp=%d)\n",
      (int32_t)(_trampolineReservationMark - _trampolineBase), (int32_t)(_tempTrampolineNext - _tempTrampolineBase));
  }
}
    ...
    // scope for cache walk
    {
      ...
      for (CodeCacheFreeCacheBlock* currLink = _freeBlockList; currLink; currLink = currLink->_next) {
        ...
        if (currLink->_size > cacheSize) {
          fprintf(stderr,
            "checkForErrors cache %p: Error: Size of the free block %u is bigger than the size of the cache %u\n", this,
            (uint32_t)currLink->_size, (uint32_t)cacheSize);
          doCrash = true;
        }
        ...
        if (currLink->_next) {
          if ((uint8_t*)currLink->_next == endBlock) {
            ...
            if (!((uint8_t*)currLink < _warmCodeAlloc && endBlock >= _coldCodeAlloc)) {
              fprintf(stderr,
                "checkForErrors cache %p: Error: missed freed block coalescing opportunity. Next block (%p) is "
                "adjacent to current one %p-%p\n",
            ...
            if ((uint8_t*)currLink->_next <= endBlock) {
              fprintf(stderr,
                "checkForErrors cache %p: Error: next block (%p) should come after end of current one %p-%p\n", this,
                currLink->_next, currLink, endBlock);
              doCrash = true;
              ;
            }
            // A free block is always followed by a used block except when this is the last free block in the warm
            // region
            if (endBlock != _warmCodeAlloc) {
              ...
              if (*eyeCatcherPosition != *(_manager->codeCacheConfig().warmEyeCatcher())) {
                fprintf(stderr,
                  "checkForErrors cache %p: Error: block coming after this free one (%p-%p) does not have the eye "
                  "catcher but %u\n",
      ...
      // 3. All used blocks must have a CodeCacheMethodHeader with size as first parameter, folowed by eyeCatcher and
      // metaData
      TR::CodeCacheConfig& config = _manager->codeCacheConfig();
      ...
      while (start < this->_trampolineBase) {
        ...
        } else // used seg
        {
          ...
          if (*eyeCatcherPosition != *(_manager->codeCacheConfig().warmEyeCatcher())) {
            ...
            fprintf(stderr,
              "checkForErrors cache %p: block %p is not in the free list and does not have eye-catcher; prevBlock=%p\n",
