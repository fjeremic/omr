 ...
 *                                      |      TEMPTRAMPOLINEBASE   |
 *                                      |               |           |
 *          warmCodeAlloc coldCodeAlloc |               |       HELPERBASE
 * HELPERTOP
 *             |-->           <--|      |               |           |         |
 ...
 *
 *
 * * There are a fixed number of helper trampolines. Each helper has an index
 * into these trampolines.
 * * There are a fixed number of temporary trampoline slots.
 * `_tempTrampolineNext` goes from `_tempTrampolineBase`, allocating to
 * `tempTrampolineMax.`
 * * There are a fixed number of permanent trampoline slots.
 * **  `_trampolineReservationMark` goes from `_tempTrampolineBase` to
 * `_trampolineBase` backwards, reserving trampoline slots.
...
OMR::CodeCache::CacheCriticalSection::CacheCriticalSection(
    TR::CodeCache *codeCache)
    : CriticalSection(codeCache->_mutex) {}

TR::CodeCache *OMR::CodeCache::self() {
...
void OMR::CodeCache::writeMethodHeader(void *freeBlock, size_t size,
                                       bool isCold) {
  ...
  if (!isCold)
    memcpy(block->_eyeCatcher, config.warmEyeCatcher(),
           sizeof(block->_eyeCatcher));
  else
    memcpy(block->_eyeCatcher, config.coldEyeCatcher(),
           sizeof(block->_eyeCatcher));
  block->_metaData = NULL;
}

bool OMR::CodeCache::trimCodeMemoryAllocation(void *codeMemoryStart,
  ...
  codeMemoryStart =
      (uint8_t *)codeMemoryStart -
      sizeof(CodeCacheMethodHeader);  // Do we always have a header?
  actualSizeInBytes =
      (actualSizeInBytes + sizeof(CodeCacheMethodHeader) + round) & ~round;
  ...
  if (actualSizeInBytes >= oldSize) return false;
  ...
  if (config.verboseReclamation()) {
    TR_VerboseLog::writeLineLocked(
        TR_Vlog_CODECACHE,
        "--trimCodeMemoryAllocation-- CC=%p cacheHeader=%p oldSize=%u "
        "actualSizeInBytes=%d shrinkage=%u",
  ...
  if (expectedHeapAlloc == _warmCodeAlloc) {
    ...
    return true;
  } else  // the allocation could have been from a free block or from the cold
          // portion
  {
    if (shrinkage >= MIN_SIZE_BLOCK) {
      ...
      if (self()->addFreeBlock2((uint8_t *)codeMemoryStart + actualSizeInBytes,
                                (uint8_t *)expectedHeapAlloc)) {
...
//
bool OMR::CodeCache::initialize(TR::CodeCacheManager *manager,
                                TR::CodeCacheMemorySegment *codeCacheSegment,
                                size_t allocatedCodeCacheSizeInBytes) {
  _manager = manager;

  // heapSize can be calculated as (codeCache->helperTop -
  // codeCacheSegment->heapBase), which is equal to segmentSize If
  // codeCachePadKB is set, this will make the system believe that we allocated
  // segmentSize bytes, instead of _jitConfig->codeCachePadKB * 1024 bytes If
  // codeCachePadKB is not set, heapSize is segmentSize anyway
  _segment = codeCacheSegment;

  // helperTop is heapTop, usually
  // When codeCachePadKB > segmentSize, the helperTop is not at the very end of
  // the segemnt
  ...
  //
  _hashEntrySlab = CodeCacheHashEntrySlab::allocate(
      manager, config.codeCacheHashEntryAllocatorSlabSize());
  if (!_hashEntrySlab) {
    return false;
  }
  ...
  *((TR::CodeCache **)(_segment->segmentBase())) =
      self();  // Write a pointer to this cache at the beginning of the segment
  ...
  if (!config.trampolineCodeSize()) {
    ...
    _CCPreLoadedCodeTop = (uint8_t *)(((size_t)_trampolineBase) &
                                      (~config.codeCacheHelperAlignmentMask()));
    _CCPreLoadedCodeBase = _CCPreLoadedCodeTop - config.ccPreLoadedCodeSize();
    TR_ASSERT((((size_t)_CCPreLoadedCodeBase) &
               config.codeCacheHelperAlignmentMask()) == 0,
              "Per-code cache helper sizes do not account for alignment "
              "requirements.");
  ...
  // Helpers are located at the top of the code cache (offset N), growing down
  // towards the base (offset 0)
  size_t trampolineSpaceSize =
      config.trampolineCodeSize() * config.numRuntimeHelpers();
  // _helperTop is heapTop
  _helperBase = _helperTop - trampolineSpaceSize;
  _helperBase = (uint8_t *)(((size_t)_helperBase) &
                            (~config.codeCacheTrampolineAlignmentBytes()));
  ...
  if (!config.needsMethodTrampolines()) {
    ...
    _trampolineBase = _helperBase - ((_helperBase - _segment->segmentBase()) *
                                     config.trampolineSpacePercentage() / 100);

    // Grab the configuration details from the JIT platform code
    //
    // (_helperTop - segment->heapBase) is heapSize
    config.mccCallbacks().codeCacheConfig(
        (_helperTop - _segment->segmentBase()), &_tempTrampolinesMax);
  }

  mcc_printf("mcc_initialize: trampoline base %p\n", _trampolineBase);

  // set the temporary trampoline slab right under the helper trampolines,
  // should be already aligned
  _tempTrampolineTop = _helperBase;
  _tempTrampolineBase =
      _tempTrampolineTop - (config.trampolineCodeSize() * _tempTrampolinesMax);
  _tempTrampolineNext = _tempTrampolineBase;

  // Check if we have enough space in the code cache to contain the trampolines
  if (_trampolineBase >= _tempTrampolineNext &&
      config.needsMethodTrampolines()) {
    _hashEntrySlab->free(manager);
    return false;
  }
  ...
  //
  config.mccCallbacks().createHelperTrampolines((uint8_t *)_helperBase,
  ...
  if (config.needsMethodTrampolines()) {
    // Initialize hashtables to hold trampolines for resolved and unresolved
    // methods
  ...
  // Usable space is between _warmCodeAlloc and _trampolineBase. Everything else
  // is overhead Only relevant if code cache repository is used
  size_t spaceLost = (_warmCodeAlloc - _segment->segmentBase()) +
                     (_segment->segmentTop() - _trampolineBase);
...
OMR::CodeCacheTrampolineCode *OMR::CodeCache::allocateTempTrampoline() {
  ...
  if (_tempTrampolineNext >= _tempTrampolineTop) return NULL;
...
OMR::CodeCacheErrorCode::ErrorCode OMR::CodeCache::reserveSpaceForTrampoline(
    int32_t numTrampolines) {
...
//---------------------------------------------------------------------------
int32_t OMR::CodeCache::reserveResolvedTrampoline(TR_OpaqueMethodBlock *method,
                                                  bool inBinaryEncoding) {
  int32_t retValue = CodeCacheErrorCode::ERRORCODE_SUCCESS;  // assume success
  TR_ASSERT(
      _reserved,
      "CodeCache %p is not reserved when calling reservedResolvedTrampoline\n",
      this);

  // does the platform need trampolines at all?
  TR::CodeCacheConfig &config = _manager->codeCacheConfig();
  if (!config.needsMethodTrampolines()) return retValue;
  ...
  // scope for cache critical section
  {
    ...
    if (!entry) {
      // Reserve a new trampoline since there is not an active reservation for
      // given method
      //
      ...
      if (retValue == OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS) {
        ...
        if (!self()->addResolvedMethod(method))
          retValue =
              CodeCacheErrorCode::ERRORCODE_FATALERROR;  // couldn't allocate
                                                         // memory from VM
      }
    }
  }
...
//
OMR::CodeCacheTrampolineCode *OMR::CodeCache::findTrampoline(
...
OMR::CodeCacheTrampolineCode *OMR::CodeCache::findTrampoline(
  ...
  TR::CodeCacheConfig &config = _manager->codeCacheConfig();
  trampoline =
      (CodeCacheTrampolineCode *)(_helperBase +
                                  (config.trampolineCodeSize() * helperIndex));
...
OMR::CodeCacheTrampolineCode *OMR::CodeCache::replaceTrampoline(
    TR_OpaqueMethodBlock *method, void *oldTrampoline, void *oldTargetPC,
  ...
  if (oldTrampoline == NULL) {
    ...
    if (needSync) {
      ...
      // Save the temporary trampoline entry for future temp->parm
      // synchronization
...
void OMR::CodeCache::syncTempTrampolines() {
  if (_flags & CODECACHE_FULL_SYNC_REQUIRED) {
    for (uint32_t entryIdx = 0; entryIdx < _resolvedMethodHT->_size;
         entryIdx++) {
      for (CodeCacheHashEntry *entry = _resolvedMethodHT->_buckets[entryIdx];
           entry; entry = entry->_next) {
        void *newPC =
            (void *)TR::Compiler->mtd.startPC(entry->_info._resolved._method);
        void *trampoline = entry->_info._resolved._currentTrampoline;
        if (trampoline && entry->_info._resolved._currentStartPC != newPC) {
          self()->createTrampoline(trampoline, newPC,
    ...
    for (CodeCacheTempTrampolineSyncBlock *syncBlock = _trampolineSyncList;
         syncBlock; syncBlock = syncBlock->_next)
    ...
    for (syncBlock = _trampolineSyncList; syncBlock;
         syncBlock = syncBlock->_next) {
      // TR_ASSERT(syncBlock->_entryCount <= syncBlock->_entryListSize);
      // Synchronize all stored sync requests
      for (uint32_t entryIdx = 0; entryIdx < syncBlock->_entryCount;
           entryIdx++) {
        ...
        self()->createTrampoline(entry->_info._resolved._currentTrampoline,
                                 newPC, entry->_info._resolved._method);
        entry->_info._resolved._currentStartPC = newPC;
      }
...
//
void OMR::CodeCache::patchCallPoint(TR_OpaqueMethodBlock *method,
                                    void *callSite, void *newStartPC,
                                    void *extraArg) {
  TR::CodeCacheConfig &config = _manager->codeCacheConfig();
  ...
  //   grab its current trampoline plus its current pointed-at address (if no
  //   trampoline, both are NULL). Then, call code_patching with these things as
  //   argument.
  ...
  // scope to patch trampoline
  {
      ...
      if (entry) {
        ...
        if (resolvedTramp)
          methodRunAddress = entry->_info._resolved._currentStartPC;
                   ...
                   TR_UseGlueIfMethodTrampolinesAreNotNeeded)) {
      // Safety measure, return to the old behaviour of always going through the
      // glue
      return;
    }
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(
      ...
      TR_VerboseLog::writeLineLocked(
          ...
          "j9method=0x%p,resolvedTramp=0x%p,methodRunAddress=0x%p,newStartPC="
          "0x%p,extraArg=0x%p",
          callSite, method, resolvedTramp, methodRunAddress, newStartPC,
          extraArg);
    }

    // Patch the code for a method trampoline
    int32_t rc = _manager->codeCacheConfig().mccCallbacks().patchTrampoline(
        method, callSite, methodRunAddress, resolvedTramp, newStartPC,
        extraArg);
  }
}
...
//
void OMR::CodeCache::createTrampoline(CodeCacheTrampolineCode *trampoline,
                                      void *targetStartPC,
                                      TR_OpaqueMethodBlock *method) {
  TR::CodeCacheConfig &config = _manager->codeCacheConfig();
  config.mccCallbacks().createMethodTrampoline(trampoline, targetStartPC,
                                               method);
}
...
bool OMR::CodeCache::saveTempTrampoline(CodeCacheHashEntry *entry) {
  ...
  for (syncBlock = _trampolineSyncList; syncBlock;
    ...
    for (int32_t i = 0; i < syncBlock->_entryCount; i++) {
      if (syncBlock->_hashEntryArray[i] == entry) return true;
    }
...
bool OMR::CodeCache::allocateTempTrampolineSyncBlock() {
  CodeCacheTempTrampolineSyncBlock *block =
      static_cast<CodeCacheTempTrampolineSyncBlock *>(
  ...
  block->_hashEntryArray = static_cast<CodeCacheHashEntry **>(
      _manager->getMemory(sizeof(CodeCacheHashEntry *) *
                          config.codeCacheTempTrampolineSyncArraySize()));
...
OMR::CodeCacheHashEntry *OMR::CodeCache::allocateHashEntry() {
  ...
  if ((slab->_heapAlloc + sizeof(CodeCacheHashEntry)) > slab->_heapTop) {
    TR::CodeCacheConfig &config = _manager->codeCacheConfig();
    slab = CodeCacheHashEntrySlab::allocate(
        _manager, config.codeCacheHashEntryAllocatorSlabSize());
...
// empty by default
OMR::CodeCacheMethodHeader *OMR::CodeCache::addFreeBlock(void *metaData) {
  return NULL;
}
...
bool OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t *start, uint8_t *end,
                                               char *file,
                                               uint32_t lineNumber) {
  TR::CodeCacheConfig &config = _manager->codeCacheConfig();
  ...
  if (end <= (start + sizeof(CodeCacheFreeCacheBlock))) {
    if (config.verboseReclamation()) {
      TR_VerboseLog::writeLineLocked(
          TR_Vlog_FAILURE,
          "addFreeBlock2[%s.%d]: failed to add free block. start = 0x%016x end "
          "= 0x%016x alignment = 0x%04x sizeof(CodeCacheFreeCacheBlock) = "
          "0x%08x",
          file, lineNumber, start_o, end, config.codeCacheAlignment(),
          sizeof(CodeCacheFreeCacheBlock));
    }
    return false;
  }
  ...
  if (_freeBlockList) {
    ...
    for (curr = _freeBlockList; curr->_next && (uint8_t *)(curr->_next) < start;
         curr = curr->_next) {
    }

    if (start < (uint8_t *)curr &&
        (uint8_t *)curr - end < sizeof(CodeCacheFreeCacheBlock)) {
      // merge with the curr block ahead, which is also the first block
      TR_ASSERT(end <= (uint8_t *)curr,
                "assertion failure");  // check for no overlap of blocks
      ...
      if (!(start < _warmCodeAlloc && (uint8_t *)curr >= _coldCodeAlloc)) {
        ...
        mergedBlock = curr;
        // fprintf(stderr, "--ccr-- merging new free block of the size %d with a
        // block of the size %d at %p\n", size, curr->size, link);
        ...
        // link->size);
               ...
               ((uint8_t *)curr->_next - end <
                sizeof(CodeCacheFreeCacheBlock)) &&
               !(start < _warmCodeAlloc &&
                 (uint8_t *)curr->_next >= _coldCodeAlloc)) {
      // merge with the next block, but don't merge warm blocks with cold blocks
      if ((start - ((uint8_t *)curr + curr->_size) <
           sizeof(CodeCacheFreeCacheBlock)) &&
          !((uint8_t *)curr < _warmCodeAlloc && start >= _coldCodeAlloc)) {
        // merge with the previous and the next blocks
        mergedBlock = curr;
        // fprintf(stderr, "--ccr-- merging new free block of the size %d with
        // blocks of the size %d and %d at %p\n", size, curr->_size,
        // curr->_next->_size, curr);
        curr->_size =
            (uint8_t *)curr->_next + curr->_next->_size - (uint8_t *)curr;
  ...
  if (config.verboseReclamation()) {
    TR_VerboseLog::writeLineLocked(
        TR_Vlog_CODECACHE,
        "--ccr-- addFreeBlock2WithCallSite CC=%p start=%p end=%p "
        "mergedBlock=%p link=%p link->_size=%u, _sizeOfLargestFreeWarmBlock=%d "
        "_sizeOfLargestFreeColdBlock=%d warmCodeAlloc=%p coldBlockAlloc=%p",
        this, (void *)start, (void *)end, mergedBlock, link,
        (uint32_t)link->_size, _sizeOfLargestFreeWarmBlock,
        _sizeOfLargestFreeColdBlock, _warmCodeAlloc, _coldCodeAlloc);
  }
#ifdef DEBUG
  uint8_t *paintStart = start + sizeof(CodeCacheFreeCacheBlock);
  memset((void *)paintStart, 0xcc,
         ((CodeCacheFreeCacheBlock *)start)->_size -
             sizeof(CodeCacheFreeCacheBlock));
#endif

  if (config.doSanityChecks()) self()->checkForErrors();

  return true;
}

void OMR::CodeCache::updateMaxSizeOfFreeBlocks(
    CodeCacheFreeCacheBlock *blockPtr, size_t blockSize) {
  TR::CodeCacheConfig &config = _manager->codeCacheConfig();
  if (config.codeCacheFreeBlockRecylingEnabled()) {
    if ((uint8_t *)blockPtr < _warmCodeAlloc) {
      if (blockSize > _sizeOfLargestFreeWarmBlock) {
        // fprintf(stderr, "_sizeOfLargestFreeBlock for cache %p increased to
        // %d\n", this, blockSize);
...
//
uint8_t *OMR::CodeCache::findFreeBlock(size_t size, bool isCold,
  ...
  TR_ASSERT(_freeBlockList,
            "Because we first checked that a freeBlockExists, freeBlockList "
            "cannot be null");

  // Find the smallest free link to fit the requested blockSize
  for (currLink = _freeBlockList, prevLink = NULL; currLink;
       prevLink = currLink, currLink = currLink->_next) {
    if (isCold) {
      if ((void *)currLink < (void *)_coldCodeAlloc) continue;
    } else {
      if ((void *)currLink >= (void *)_warmCodeAlloc) continue;
    ...
    if (!biggestLink) {
      ...
      if (currLink->_size >=
          biggestLink->_size)  // >= is important; do not use > only as
                               // secondBiggestLink may be wrong
      {
  ...
  if (!isCold) {
    TR_ASSERT(!config.codeCacheFreeBlockRecylingEnabled() ||
              ...
              "_sizeOfLargestFreeWarmBlock=%d  biggestLink->_size=%d",
              _sizeOfLargestFreeWarmBlock, (int32_t)biggestLink->_size);
  } else {
    TR_ASSERT(!config.codeCacheFreeBlockRecylingEnabled() ||
                  _sizeOfLargestFreeColdBlock == biggestLink->_size,
              "assertion failure");
  }

  if (bestFitLink) {
    // Fix the linked list by removing the allocated block AND if there is any
    // unused space left in the currLink chunk, reclaim it and put back on the
    // freeList
    CodeCacheFreeCacheBlock *leftBlock =
        self()->removeFreeBlock(size, bestFitLinkPrev, bestFitLink);

    if (bestFitLink == biggestLink)  // Size of biggest might have changed
    {
      uint64_t leftBlockSize = leftBlock ? leftBlock->_size : 0;
      uint64_t secondBiggestSize =
          secondBiggestLink ? secondBiggestLink->_size : 0;
      uint64_t biggestSize =
          leftBlockSize > secondBiggestSize ? leftBlockSize : secondBiggestSize;
      if (!isCold) {
    ...
    if (config.verboseReclamation()) {
      TR_VerboseLog::writeLineLocked(
          TR_Vlog_CODECACHE,
          "--ccr- findFreeBlock: CodeCache=%p size=%u isCold=%d bestFitLink=%p "
          "bestFitLink->size=%u leftBlock=%p",
...
OMR::CodeCacheFreeCacheBlock *OMR::CodeCache::removeFreeBlock(
    size_t blockSize, CodeCacheFreeCacheBlock *prev,
    CodeCacheFreeCacheBlock *curr) {
...
void OMR::CodeCache::printOccupancyStats() {
  ...
  if (_freeBlockList) {
    fprintf(stderr,
            "   sizeOfLargestFreeColdBlock = %8" OMR_PRIuSIZE " bytes\n",
            _sizeOfLargestFreeColdBlock);
    fprintf(stderr,
            "   sizeOfLargestFreeWarmBlock = %8" OMR_PRIuSIZE " bytes\n",
            _sizeOfLargestFreeWarmBlock);
    fprintf(stderr, "   reclaimed sizes:");
    // scope for critical section
    {
      CacheCriticalSection resolveAndCreateTrampoline(self());
      for (CodeCacheFreeCacheBlock *currLink = _freeBlockList; currLink;
           currLink = currLink->_next)
    ...
    // scope for cache walk
    {
      ...
      for (CodeCacheFreeCacheBlock *currLink = _freeBlockList; currLink;
        ...
        if (currLink->_size > cacheSize) {
          fprintf(stderr,
                  "checkForErrors cache %p: Error: Size of the free block %u "
                  "is bigger than the size of the cache %u\n",
        ...
        if ((uint8_t *)currLink < _segment->segmentBase() + sizeof(void *) ||
          ...
          fprintf(stderr,
                  "checkForErrors cache %p: Error: curLink %p is outside cache "
                  "boundaries\n",
                  this, currLink);
          doCrash = true;
        }
        uint8_t *endBlock = (uint8_t *)currLink + currLink->_size;
        if (endBlock < _segment->segmentBase() + sizeof(void *) ||
            endBlock > _segment->segmentTop()) {
          fprintf(stderr,
                  "checkForErrors cache %p: Error: End of block %p residing at "
                  "%p is outside cache boundaries\n",
        ...
        if (currLink->_next) {
          if ((uint8_t *)currLink->_next == endBlock) {
            // Two freed blocks can be adjacent if one belongs to the warm
            // region and the other one belongs to the cold region
            if (!((uint8_t *)currLink < _warmCodeAlloc &&
                  endBlock >= _coldCodeAlloc)) {
              fprintf(stderr,
                      "checkForErrors cache %p: Error: missed freed block "
                      "coalescing opportunity. Next block (%p) is adjacent to "
                      "current one %p-%p\n",
            ...
            if ((uint8_t *)currLink->_next <= endBlock) {
              fprintf(stderr,
                      "checkForErrors cache %p: Error: next block (%p) should "
                      "come after end of current one %p-%p\n",
                      this, currLink->_next, currLink, endBlock);
              doCrash = true;
              ;
            }
            // A free block is always followed by a used block except when this
            // is the last free block in the warm region
            if (endBlock != _warmCodeAlloc) {
              // There should be valid code between this block and the next
              uint8_t *eyeCatcherPosition =
                  endBlock + offsetof(CodeCacheMethodHeader, _eyeCatcher);
              if (*eyeCatcherPosition !=
                  *(_manager->codeCacheConfig().warmEyeCatcher())) {
                fprintf(
                    stderr,
                    "checkForErrors cache %p: Error: block coming after this "
                    "free one (%p-%p) does not have the eye catcher but %u\n",
      ...
      if (_sizeOfLargestFreeWarmBlock != maxFreeWarmSize) {
        fprintf(stderr,
                "checkForErrors cache %p: Error: "
                "_sizeOfLargestFreeWarmBlock(%" OMR_PRIuSIZE
      ...
      if (_sizeOfLargestFreeColdBlock != maxFreeColdSize) {
        fprintf(stderr,
                "checkForErrors cache %p: Error: "
                "_sizeOfLargestFreeColdBlock(%" OMR_PRIuSIZE
      ...
      //    The only exception is when we make transition from warm to cold
      //    section
      // 2. A used block must be followed by another used block or by a free
      // block
      // 3. All used blocks must have a CodeCacheMethodHeader with size as first
      // parameter, folowed by eyeCatcher and metaData
      TR::CodeCacheConfig &config = _manager->codeCacheConfig();
      uint8_t *start =
          align(((uint8_t *)_segment->segmentTop()) + sizeof(char *),
                config.codeCacheAlignment() - 1);
      ...
      while (start < this->_trampolineBase) {
        ...
        } else  // used seg
        {
          // There should be valid code between this block and the next
          uint8_t *eyeCatcherPosition =
              start + offsetof(CodeCacheMethodHeader, _eyeCatcher);
          if (*eyeCatcherPosition !=
              *(_manager->codeCacheConfig().warmEyeCatcher())) {
            prevBlock = start;
            start = start + ((CodeCacheMethodHeader *)start)->_size;
            if (start >= _warmCodeAlloc) start = _coldCodeAlloc;
            continue;
          } else {
            fprintf(stderr,
                    "checkForErrors cache %p: block %p is not in the free list "
                    "and does not have eye-catcher; prevBlock=%p\n",
...
OMR::CodeCacheHashEntry *OMR::CodeCache::findResolvedMethod(
    TR_OpaqueMethodBlock *method) {
...
void OMR::CodeCache::findOrAddResolvedMethod(TR_OpaqueMethodBlock *method) {
  ...
  if (!entry) self()->addResolvedMethod(method);
}

// Allocate code memory within a code cache. Some of the allocation may come
// from the free list
//

uint8_t *OMR::CodeCache::allocateCodeMemory(size_t warmCodeSize,
                                            size_t coldCodeSize,
                                            uint8_t **coldCode,
  ...
  _manager->performSizeAdjustments(
      warmSize, coldSize, needsToBeContiguous,
  ...
  if (!needsToBeContiguous) {
    if (warmSize) warmIsFreeBlock = _sizeOfLargestFreeWarmBlock >= warmSize;
    if (coldSize) coldIsFreeBlock = _sizeOfLargestFreeColdBlock >= coldSize;
  }
  ...
  if (coldSize != 0 && !coldIsFreeBlock &&
      coldSize + (warmIsFreeBlock ? 0 : warmSize) >
          self()->getFreeContiguousSpace()) {
    return NULL;
  }
  ...
  if (!warmIsFreeBlock) {
    if (warmSize) {
      ...
      // Thus the free code cache space decreases by
      // (cacheHeapAlloc-_warmCodeAlloc)
      _manager->decreaseFreeSpaceInCodeCacheRepository(cacheHeapAlloc -
                                                       _warmCodeAlloc);
    ...
    warmCodeAddress = self()->findFreeBlock(
        warmSize, false,
        isMethodHeaderNeeded);  // side effect: free block is unlinked
    // If isMeathodHeaderNeeded is true, warmCodeAddress will look like a
    // pointer to CodeCacheMethodHeader Otherwise, it look like a pointer to
    // CodeCacheFreeCacheBlock

    // Note that findFreeBlock may return a block which is slightly bigger than
    // what we wanted Change the warmSize to the higher size so that the size of
    // the block is correctly maintained
  ...
  if (!coldIsFreeBlock) {
    if (coldSize) {
      ...
      if (cacheHeapAlloc < _warmCodeAlloc) {
        // This case should never happen now because we checked above that both
        // warm and cold can be allocated
      ...
      _manager->decreaseFreeSpaceInCodeCacheRepository(_coldCodeAlloc -
                                                       cacheHeapAlloc);
    ...
    coldCodeAddress = self()->findFreeBlock(
        coldSize, true,
        isMethodHeaderNeeded);  // side effect: free block is unlinked
    // Note that findFreeBlock may return a block which is slightly bigger than
    // what we wanted Change the warmSize to the higher size so that the size of
...
void *OMR::CodeCache::getCCPreLoadedCodeAddress(
    TR_CCPreLoadedCode h, TR::CodeGenerator *codeGenerator) {
  if (!_CCPreLoadedCodeInitialized) {
    if (TR_numCCPreLoadedCode) {
      TR::CodeCacheConfig &config = _manager->codeCacheConfig();
      config.mccCallbacks().createCCPreLoadedCode(
          _CCPreLoadedCodeBase, _CCPreLoadedCodeTop, _CCPreLoadedCode,
          codeGenerator);
      _CCPreLoadedCodeInitialized = true;
    }
  }
  return (h >= 0 && h < TR_numCCPreLoadedCode) ? _CCPreLoadedCode[h]
...
// A value of -1 for this parameter means that an application thread is
// requesting the reservation A positive value means that a compilation thread
// is requesting the reservation A value of -2 (or less) means that no
// reservation is requested
//
TR::CodeCache *OMR::CodeCache::allocate(TR::CodeCacheManager *manager,
                                        size_t segmentSize,
                                        int32_t reservingCompThreadID) {
  return manager->allocateCodeCacheFromNewSegment(segmentSize,
                                                  reservingCompThreadID);
}
