...
class OMR_EXTENSIBLE CodeCacheMemorySegment {
 public:
  ...
  CodeCacheMemorySegment(uint8_t* memory, size_t size)
      : _base(memory), _alloc(memory), _top(memory + size) {}
  CodeCacheMemorySegment(uint8_t* memory, uint8_t* top)
      : _base(memory), _alloc(memory), _top(top) {}

  TR::CodeCacheMemorySegment* self();

  void* operator new(size_t size, TR::CodeCacheMemorySegment* segment) {
    return segment;
  }

  void operator delete(void* pmem,
                       TR::CodeCacheMemorySegment* segment) { /* do nothing */
  }
