...
enum CodeCacheFlags {
  CODECACHE_FULL_SYNC_REQUIRED =
      0x00000001,  // Force synchronization of all permanent trampolines
  CODECACHE_CACHE_IS_FULL = 0x00000002,  // Code cache is marked/considered full
  CODECACHE_TRAMP_REPORTED =
      0x00000004,  // Code cache tramp region has been reported.
  CODECACHE_CCPRELOADED_REPORTED =
      0x00000008,  // Code cache pre loaded code region has been reported.
};
...
class CodeCacheHashEntrySlab {
 public:
  ...
  void operator delete(void *ptr,
                       CodeCacheHashEntrySlab *slab) { /* do nothing */
  }

  static CodeCacheHashEntrySlab *allocate(TR::CodeCacheManager *manager,
                                          size_t slabSize);
...
class CodeCacheHashTable {
 public:
  ...
  CodeCacheHashEntry *findUnresolvedMethod(void *constPool,
  ...
  void reloHashUnresolvedMethod(void *constPoolOld, void *constPoolNew,
                                int32_t cpIndex);
...
struct CodeCacheTempTrampolineSyncBlock {
  CodeCacheHashEntry **_hashEntryArray; /*!< a list of hash entries representing
                                           trampolines to be synchronized */
...
CodeCacheMethodHeader *getCodeCacheMethodHeader(char *p, int searchLimit,
                                                MethodExceptionData *metaData);

struct CodeCacheFreeCacheBlock {
  size_t _size;
  CodeCacheFreeCacheBlock *_next;
};
#define MIN_SIZE_BLOCK \
  (sizeof(CodeCacheFreeCacheBlock) > 96 ? sizeof(CodeCacheFreeCacheBlock) : 96)
...
#define addFreeBlock2(start, end) \
  addFreeBlock2WithCallSite((start), (end), __FILE__, __LINE__)
