...
void TR::PatchNOPedGuardSite::compensate(bool isSMP, uint8_t *location,
                                         uint8_t *destination) {
...
void TR::PatchSites::add(uint8_t *location, uint8_t *destination) {
  TR_ASSERT_FATAL(_size < _maxSize,
                  "Cannot add more patch sites, max size is %d", _maxSize);
...
bool TR::PatchSites::equals(PatchSites *other) {
  ...
  for (size_t i = 0; i < _size; ++i) {
    if (getLocation(i) != other->getLocation(i)) return false;
  }
  return true;
}
...
bool TR::PatchSites::containsLocation(uint8_t *location) {
  if (_firstLocation > location || _lastLocation < location) return false;
...
uint8_t *TR::PatchSites::getLocation(size_t index) {
  TR_ASSERT(index < _size, "Invalid index for patch site %d but size %d", index,
            _size);
