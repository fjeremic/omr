...
_patchVirtualGuard(uint8_t* locationAddr,
                   ...
                   int32_t smpFlag);
#else
extern "C" void ASM_CALL
_patchVirtualGuard(uint8_t*, uint8_t*, uint32_t);
#endif

void
TR::PatchNOPedGuardSite::compensate(bool isSMP,
                                    uint8_t* location,
                                    uint8_t* destination)
{
  ...
  , _lastLocation(NULL)
{
  ...
  TR_ASSERT(maxSize == bytes / (sizeof(uint8_t*) * 2),
            "Requested number of patch sites %d exceeds supported size\n",
            maxSize);
 ...
 */
void
TR::PatchSites::add(uint8_t* location, uint8_t* destination)
{
  TR_ASSERT_FATAL(
    _size < _maxSize, "Cannot add more patch sites, max size is %d", _maxSize);
 ...
 */
bool
TR::PatchSites::equals(PatchSites* other)
{
  if (other->getSize() != _size)
    return false;

  if (_firstLocation != other->getFirstLocation() ||
      _lastLocation != other->getLastLocation())
    return false;
 ...
 */
bool
TR::PatchSites::containsLocation(uint8_t* location)
{
...
bool
TR::PatchSites::internalContainsLocation(uint8_t* location)
{
  for (size_t i = 0; i < _size; ++i) {
...
uint8_t*
TR::PatchSites::getLocation(size_t index)
{
  TR_ASSERT(
    index < _size, "Invalid index for patch site %d but size %d", index, _size);
  return _patchPoints[index * 2];
}
uint8_t*
TR::PatchSites::getDestination(size_t index)
{
  TR_ASSERT(
    index < _size, "Invalid index for patch site %d but size %d", index, _size);
...
void
TR::PatchSites::reclaim(PatchSites* sites)
{
  TR_ASSERT(sites->_refCount > 0,
            "Attempt to reclaim patch sites without a reference");
  sites->_refCount--;
