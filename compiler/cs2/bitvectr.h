...
class ABitVector : private Allocator {
 ...
 public:
  ...
  void GrowTo(BitIndex index, bool geometric = true,
  ...
  BitIndex ClearLastOneIfThereIsOneInRange(BitIndex low, BitIndex high,
                                           bool &foundone);
    ...
    bool SetToNextOne() {
      ...
      if (fWord == 0) {
        ...
        for (wordIndex = (fIndex + kBitWordSize - 1) / kBitWordSize;
             wordIndex < fWordCount; wordIndex += 1) {
 ...
 private:
  ...
  class BitRef {
   public:
    ...
    BitRef(const BitRef &b) : fIndex(b.fIndex), fVector(b.fVector) {}
    ...
    BitRef &operator=(bool bitValue) {
      ...
      if (bitValue == 0 && fVector.SizeInBits() < fIndex) return *this;
...
inline ABitVector<Allocator>::ABitVector(
    ...
    : Allocator(adoptVector), fNumBits(0), fBitWords(NULL) {
  *this = adoptVector;
}

template <class Allocator>
inline ABitVector<Allocator>::ABitVector(
    const ABitVector<Allocator> &adoptVector, const Allocator &a)
    : Allocator(a), fNumBits(0), fBitWords(NULL) {
  *this = adoptVector;
}
...
template <class Allocator>
inline BitWord ABitVector<Allocator>::WordValueAt(BitIndex bitIndex,
                                                  BitIndex wordIndex) const {
...
template <class Allocator>
inline void ABitVector<Allocator>::GrowTo(BitIndex newBitSize, bool geometric,
                                          bool forceGeometric) {
...
template <class Allocator>
inline void ABitVector<Allocator>::GrowTo(const ABitVector &vector) {
...
inline void ABitVector<Allocator>::And(const B2 &v) {
  ...
  if (!v.hasBitWordRepresentation() || (v.wordSize() != kBitWordSize)) {
    ...
    for (vc.SetToNextOne(); vc.Valid(); vc.SetToNextOne()) {
      if (vc / kBitWordSize != wordIndex) {
        if (wordIndex < lastWordIndex) {
          for (i = lastWord; i < wordIndex; i++) WordAt(i) = 0;
          WordAt(wordIndex) &= word;
    ...
    for (i = lastWord; i < lastWordIndex; i++) WordAt(i) = 0;
  } else /* (hasBitWordRepresentation() && (v.wordSize() == kBitWordSize)) */ {
    ...
    if (low < thisLow)
      ...
      for (i = thisLow; i < low; i++) WordAt(i) = kZeroBits;
    }
    if (high > thisHigh) {
      high = thisHigh;
    } else {
      for (i = thisHigh; i > high; i--) WordAt(i) = kZeroBits;
    }

    // AND in all of the words from the 2nd vector
    for (i = low; i <= high; i++) WordAt(i) &= v.WordAt(i);
...
inline void ABitVector<Allocator>::Andc(const B2 &v) {
  ...
  if (!v.hasBitWordRepresentation() || (v.wordSize() != kBitWordSize)) {
    ...
    int32_t thisLow = FirstOneWordIndex();
    int32_t thisHigh = LastOneWordIndex();
    if (high < thisLow || low > thisHigh) return;  // No intersection

    // AND in the logical NOT of all of the words from the 2nd vector
    if (low < thisLow) low = thisLow;
    if (high > thisHigh) high = thisHigh;
...
template <class Allocator>
inline void Swap(ABitVector<Allocator> &vectorA,
                 ABitVector<Allocator> &vectorB) {
...
template <class Allocator>
inline ABitVector<Allocator> &ABitVector<Allocator>::operator=(
    const ABitVector<Allocator> &adoptVector) {
...
template <class Allocator>
inline bool ABitVector<Allocator>::operator==(
    const ABitVector<Allocator> &v) const {
...
template <class Allocator>
inline unsigned long ABitVector<Allocator>::MemoryUsage() const {
...
inline void ABitVector<Allocator>::SetAll(BitIndex numBits) {
  ...
  for (uint32_t i = 0; i < lastWordIndex; i++) WordAt(i) = (BitWord)-1;

  uint32_t lastIndex = lastWordIndex * kBitWordSize;
  for (uint32_t i = lastIndex; i < numBits; i++) (*this)[i] = true;
}
...
inline BitIndex ABitVector<Allocator>::LastOne() const {
  ...
  for (wordIndex = 0; (wordIndex < vectorWordSize); ++wordIndex)
    if (WordAt(wordIndex) != kZeroBits) lastIndex = wordIndex;
...
inline int32_t ABitVector<Allocator>::LastOneWordIndex() const {
  ...
  for (wordIndex = SizeInWords() - 1;
       (wordIndex >= 0) && (WordAt(wordIndex) == kZeroBits); wordIndex--)
    ;
...
template <class Allocator>
inline BitIndex ABitVector<Allocator>::ClearLastOneIfThereIsOneInRange(
    BitIndex low, BitIndex high, bool &foundone) {
...
template <class Allocator>
inline bool ABitVector<Allocator>::Intersects(
    const ABitVector<Allocator> &vector) const {
...
#ifndef USE_POPCNTB
template <class Allocator>
inline uint32_t ABitVector<Allocator>::PopulationCount(uint32_t numBits) const {
  uint32_t popCount = 0;
    ...
    for (wordIndex = 0; wordIndex < streamWordSize;
      ...
      for (BitIndex i2 = wordIndex; i2 < wordIndex + WORDS_IN_16_BIT_ACCUM;
        ...
        for (BitIndex i1 = i2; i1 < i2 + WORDS_IN_8_BIT_ACCUM; i1++) {
          c1 +=
              __popcntb(fBitWords[i1]);  // count the number of 1's in each byte
                                         // of the word at wordIndex index
          c1a += __popcntb(
              fBitWords[streamWordSize +
                        i1]);  // count the number of 1's in each byte of the
                               // word at wordIndex+streamWordSize index
          c1b += __popcntb(
              fBitWords[streamWordSize * 2 +
                        i1]);  // count the number of 1's in each byte of the
                               // word at wordIndex+2*streamWordSize index
          c1c += __popcntb(
              fBitWords[streamWordSize * 3 +
                        i1]);  // count the number of 1's in each byte of the
                               // word at wordIndex+3*streamWordSize index
        }
#ifdef BITVECTOR_64BIT
    ...
    for (; wordIndex < wordIndex + streamWordSize;
      ...
      for (BitIndex i1 = wordIndex; i1 < wordIndex + WORDS_IN_8_BIT_ACCUM;
        ...
        c1a += __popcntb(fBitWords[streamWordSize +
                                   i1]);  // count the number of 1's in each
...
template <class Allocator>
inline uint32_t ABitVector<Allocator>::PopulationCount(
    const ABitVector<Allocator> &mask) const {
  ...
  if (thisWordSize > maskWordSize) thisWordSize = maskWordSize;
...
template <class Allocator>
inline void ABitVector<Allocator>::CopyToMemory(void *pMemory,
                                                BitIndex numBits) const {
  ...
  for (; shortwordIndex < memoryWordSize; ++shortwordIndex) *pBitWord++ = 0;
}
...
template <class Allocator>
inline void ABitVector<Allocator>::CopyFromMemory(void *pMemory,
                                                  BitIndex numBits) {
  ShortWord *pBitWord = (ShortWord *)pMemory;
...
inline bool ABitVector<Allocator>::And(
    const ABitVector<Allocator> &inputVector,
