...
public:
  ...
  BitIndex ClearLastOneIfThereIsOneInRange(BitIndex low,
                                           BitIndex high,
                                           bool& foundone);
  ...
  void Or(const B2& v);
  template<class B2>
  void Andc(const B2& v);
  template<class B2>
  void And(const B2& v);
  template<class B2>
  ABitVector& operator=(const B2& v);
  ...
  public:
    ...
    // Dereference operator - identical to cast to index operator - note it
    // returns the index, not a reference to it.
    BitIndex operator*() const { return Cursor::fIndex; }

    // Member access operator - not applicable to bit vectors, there is no
    // element only an index BitIndex *operator->() const { return &
    // (this->operator*()); }
    ...
    {
      return this == &other || (Cursor::fVector == other.fVector &&
  ...
  friend ostr& operator<<(ostr& out, const ABitVector& vector)
  {
    ...
    out << "( ";
    for (vectorCursor.SetToFirstOne(), i = 0; vectorCursor.Valid();
...
private:
  ...
  public:
    ...
    BitRef& operator=(bool bitValue)
    {
      ...
      CS2Assert(fIndex < fVector.SizeInBits(),
                ("Bit vector index out of range"));
  ...
  template<class A2>
  friend inline void Swap(ABitVector<A2>& vectorA, ABitVector<A2>& vectorB);
};

template<class Allocator>
inline uint32_t
ABitVector<Allocator>::SizeInBits() const
{
  return fNumBits;
}
...
template<class Allocator>
inline BitWord&
ABitVector<Allocator>::WordAt(uint32_t wordIndex) const
{
  CS2Assert(wordIndex < SizeInWords(fNumBits),
            ("Accessing ABitVector word %d, over current size %d\n",
             wordIndex,
             SizeInWords(fNumBits)));
  return fBitWords[wordIndex];
}

template<class Allocator>
inline ABitVector<Allocator>::ABitVector(
  const ABitVector<Allocator>& adoptVector)
...
template<class Allocator>
inline bool
ABitVector<Allocator>::ValueAt(BitIndex bitIndex) const
{
  BitIndex wordIndex;
...
template<class Allocator>
inline BitWord
ABitVector<Allocator>::WordValueAt(BitIndex bitIndex, BitIndex wordIndex) const
{
  if (bitIndex >= SizeInBits())
    return BitWord(0);
...
template<class Allocator>
inline void
ABitVector<Allocator>::GrowTo(BitIndex newBitSize,
                              bool geometric,
                              bool forceGeometric)
{
  ...
  if (oldBitSize == 0) {
    ...
    fBitWords = (BitWord*)Allocator::reallocate(
      SizeInBytes(numBits), fBitWords, SizeInBytes(oldBitSize));
    memset(((char*)fBitWords) + SizeInBytes(oldBitSize),
           0,
           SizeInBytes(numBits) - SizeInBytes(oldBitSize));
  }
  fNumBits = numBits;
}
...
template<class Allocator>
inline void
ABitVector<Allocator>::GrowTo(const ABitVector& vector)
{
...
template<class Allocator>
inline bool
ABitVector<Allocator>::operator!=(const ABitVector& v) const
{
  return !operator==(v);
}
...
template<class Allocator>
inline uint32_t
ABitVector<Allocator>::SizeInWords(BitIndex numBits)
{
#ifdef DEV_VERSION
...
template<class Allocator>
inline uint32_t
ABitVector<Allocator>::SizeInWords() const
{
...
template<class Allocator>
inline uint32_t
ABitVector<Allocator>::SizeInShortWords(BitIndex numBits)
{
#ifdef DEV_VERSION
  CS2Assert(numBits < 0xFFFFFFFFul - kBitWordSize, ("Illegal vector length"));
#endif
...
template<class Allocator>
inline uint32_t
ABitVector<Allocator>::SizeInBytes(BitIndex numBits)
{
...
template<class Allocator>
inline bool
ABitVector<Allocator>::And(const ABitVector& inputVector)
{
...
template<class Allocator>
inline bool
ABitVector<Allocator>::Andc(const ABitVector& inputVector)
{
...
template<class Allocator>
inline bool
ABitVector<Allocator>::Or(const ABitVector& inputVector)
{
  GrowTo(inputVector);
...
template<class B2>
inline void
ABitVector<Allocator>::Or(const B2& v)
{
  ...
  if (!v.hasBitWordRepresentation() || (v.wordSize() != kBitWordSize)) {
    ...
    // can use more efficient implementation by ORed corresponding words from
    // the two bit vectors together as their underlying bit representations are
    // equivalent
...
template<class B2>
inline void
ABitVector<Allocator>::And(const B2& v)
{
  ...
  if (!v.hasBitWordRepresentation() || (v.wordSize() != kBitWordSize)) {
    ...
    // can use more efficient implementation by ANDed corresponding words from
    // the two bit vectors together as their underlying bit representations are
    // equivalent
...
template<class B2>
inline void
ABitVector<Allocator>::Andc(const B2& v)
{
  if (v.IsZero())
    return;
  if (!v.hasBitWordRepresentation() || (v.wordSize() != kBitWordSize)) {
    ...
    // can use more efficient implementation by ANDC corresponding words from
    // the two bit vectors together as their underlying bit representations are
    // equivalent
    int32_t low = v.FirstOneWordIndex();
    int32_t high = v.LastOneWordIndex();
...
template<class B2>
inline void
ABitVector<Allocator>::Or(B2** vs, uint32_t nvs)
{
  uint32_t i;
  for (i = 0; i < nvs; i++) {
    Or(*(vs[i]));
...
template<class Allocator>
inline bool
ABitVector<Allocator>::Xor(const ABitVector& inputVector)
{
  GrowTo(inputVector);
...
template<class Allocator>
inline void
Swap(ABitVector<Allocator>& vectorA, ABitVector<Allocator>& vectorB)
{
...
template<class Allocator>
inline ABitVector<Allocator>&
ABitVector<Allocator>::operator=(const ABitVector<Allocator>& adoptVector)
{
...
template<class Allocator>
inline bool
ABitVector<Allocator>::operator==(const ABitVector<Allocator>& v) const
{
...
ABitVector<Allocator>::MemoryUsage() const
{
...
//
// Clear all bits in the vector (ie. clear and discard memory), clear to null
// set.
...
template<class Allocator>
inline void
ABitVector<Allocator>::SetAll(BitIndex numBits)
{
  if (numBits == 0)
    return;
...
template<class Allocator>
inline BitIndex
ABitVector<Allocator>::LowestZero() const
{
...
template<class Allocator>
inline BitIndex
ABitVector<Allocator>::FirstOne() const
{
  uint32_t wordIndex, vectorWordSize;
...
template<class Allocator>
inline BitIndex
ABitVector<Allocator>::LastOne() const
{
  ...
  if (lastIndex >= vectorWordSize || WordAt(lastIndex) == kZeroBits)
    return 0;
  return lastIndex * kBitWordSize + kBitWordSize -
         BitManipulator::TrailingZeroes(WordAt(lastIndex)) - 1;
}

// ABitVector<Allocator>::FirstOneWordIndex ()
//
// Return the index of the first word with a one bit in the vector, or
// SizeInWords() if not found
template<class Allocator>
inline int32_t
ABitVector<Allocator>::FirstOneWordIndex() const
{
  int32_t wordIndex;
...
//
// Return the index of the last word with a one bit in the vector, or -1 if
// vector isZero
template<class Allocator>
inline int32_t
ABitVector<Allocator>::LastOneWordIndex() const
{
  int32_t wordIndex;
...
template<class Allocator>
inline BitIndex
ABitVector<Allocator>::ClearLastOneIfThereIsOneInRange(BitIndex low,
                                                       BitIndex high,
                                                       bool& foundone)
{
  ...
  for (BitIndex wordIndex = highWordIndex; wordIndex >= lowWordIndex;) {
    if (WordAt(wordIndex) != kZeroBits) {
      int32_t rc = wordIndex * kBitWordSize + kBitWordSize -
...
template<class Allocator>
inline bool
ABitVector<Allocator>::IsZero() const
{
  uint32_t wordIndex, vectorWordSize;
...
template<class Allocator>
inline bool
ABitVector<Allocator>::IsNull() const
{
...
template<class Allocator>
inline bool
ABitVector<Allocator>::Intersects(const ABitVector<Allocator>& vector) const
{
...
template<class B2>
inline bool
ABitVector<Allocator>::Intersects(const B2& vector) const
{
...
template<class Allocator>
inline bool
ABitVector<Allocator>::operator<=(const ABitVector<Allocator>& vector) const
{
  uint32_t wordIndex, thisWordSize, vectorWordSize, smallerWordSize;
...
template<class Allocator>
inline uint32_t
ABitVector<Allocator>::PopulationCount(uint32_t numBits) const
{
  uint32_t popCount = 0;
...
ABitVector<Allocator>::PopulationCount(uint32_t numBits) const
{
  ...
  // WORDS_IN_8_BIT_ACCUM - worst-case maximum number of words to accumulate
  // popcntb results in 8 bits sections WORDS_IN_16_BIT_ACCUM - worst-case
  // maximum number of words to accumulate popcntb results in 16 bits sections
  ...
  streamWordSize =
    (lastWord - lastWord % (WORDS_IN_16_BIT_ACCUM * 4)) / 4; // for 4 streams

  if (lastWord >= WORDS_IN_16_BIT_ACCUM * 4) {
    // version 1 - more than 32,736 words (4 streams)

    for (wordIndex = 0; wordIndex < streamWordSize;
         wordIndex += WORDS_IN_16_BIT_ACCUM) {
      BitWord c2 = 0, c2_1 = 0, c2_2 = 0, c2a = 0, c2a_1 = 0, c2a_2 = 0;
      BitWord c2b = 0, c2b_1 = 0, c2b_2 = 0, c2c = 0, c2c_1 = 0, c2c_2 = 0;

      for (BitIndex i2 = wordIndex; i2 < wordIndex + WORDS_IN_16_BIT_ACCUM;
           i2 += WORDS_IN_8_BIT_ACCUM) {
        BitWord c1 = 0, c1a = 0, c1b = 0, c1c = 0;
        for (BitIndex i1 = i2; i1 < i2 + WORDS_IN_8_BIT_ACCUM; i1++) {
          c1 += __popcntb(fBitWords[i1]); // count the number of 1's in each
                                          // byte of the word at wordIndex index
          c1a += __popcntb(
            fBitWords[streamWordSize +
                      i1]); // count the number of 1's in each byte of the word
                            // at wordIndex+streamWordSize index
          c1b += __popcntb(
            fBitWords[streamWordSize * 2 +
                      i1]); // count the number of 1's in each byte of the word
                            // at wordIndex+2*streamWordSize index
          c1c += __popcntb(
            fBitWords[streamWordSize * 3 +
                      i1]); // count the number of 1's in each byte of the word
                            // at wordIndex+3*streamWordSize index
        }
#ifdef BITVECTOR_64BIT
      ...
      tempCount +=
        ((c2)&0x00000000FFFFFFFFul) + ((c2 >> 32) & 0x00000000FFFFFFFFul);
      c2a = ((c2a)&0x0000FFFF0000FFFFul) + ((c2a >> 16) & 0x0000FFFF0000FFFFul);
      tempCount2 +=
        ((c2a)&0x00000000FFFFFFFFul) + ((c2a >> 32) & 0x00000000FFFFFFFFul);
      c2b = ((c2b)&0x0000FFFF0000FFFFul) + ((c2b >> 16) & 0x0000FFFF0000FFFFul);
      tempCount3 +=
        ((c2b)&0x00000000FFFFFFFFul) + ((c2b >> 32) & 0x00000000FFFFFFFFul);
      c2c = ((c2c)&0x0000FFFF0000FFFFul) + ((c2c >> 16) & 0x0000FFFF0000FFFFul);
      tempCount4 +=
        ((c2c)&0x00000000FFFFFFFFul) + ((c2c >> 32) & 0x00000000FFFFFFFFul);
#else  /* !BITVECTOR_64BIT */
    ...
    wordIndex =
      wordIndex +
      streamWordSize * 3; // advance the wordIndex to catch up with streams
  }

  streamWordSize = ((lastWord - wordIndex) -
                    ((lastWord - wordIndex) % (WORDS_IN_8_BIT_ACCUM * 2))) /
                   2; // for 2 streams
  ...
  if (lastWord - wordIndex >= WORDS_IN_8_BIT_ACCUM * 2) {
    ...
    for (; wordIndex < wordIndex + streamWordSize;
         wordIndex += WORDS_IN_8_BIT_ACCUM) {
      BitWord c1 = 0, c1a = 0, c1b = 0, c1c = 0;
      for (BitIndex i1 = wordIndex; i1 < wordIndex + WORDS_IN_8_BIT_ACCUM;
           i1++) {
        c1 += __popcntb(fBitWords[i1]); // count the number of 1's in each byte
                                        // of the word at wordIndex index
        c1a += __popcntb(fBitWords[streamWordSize +
                                   i1]); // count the number of 1's in each byte
                                         // of the word at wordIndex index
      }
#ifdef BITVECTOR_64BIT
      c2_1 += (c1 & 0x00FF00FF00FF00FFul);
    ...
    wordIndex =
      wordIndex +
      streamWordSize; // advance the wordIndex to catch up with 2 streams
  }
  ...
  // deal with the residue of bits of the last word (the word which the bit is
  // selected from)
  if (lastWord < thisWordSize) {
    uint32_t bitIndex;
    BitWord testWord;
...
#ifdef BITVECTOR_64BIT
  tempCount0 = ((tempCount0)&0x00FF00FF00FF00FFul) +
               ((tempCount0 >> 8) & 0x00FF00FF00FF00FFul);
  tempCount0a = ((tempCount0a)&0x00FF00FF00FF00FFul) +
                ((tempCount0a >> 8) & 0x00FF00FF00FF00FFul);
  tempCount0 += tempCount0a;
  tempCount0 = ((tempCount0)&0x0000FFFF0000FFFFul) +
               ((tempCount0 >> 16) & 0x0000FFFF0000FFFFul);
  tempCount0 = ((tempCount0)&0x00000000FFFFFFFFul) +
               ((tempCount0 >> 32) & 0x00000000FFFFFFFFul);
#else  /* !BITVECTOR_64BIT */
  tempCount0 = ((tempCount0)&0x00FF00FFul) + ((tempCount0 >> 8) & 0x00FF00FFul);
  tempCount0a =
    ((tempCount0a)&0x00FF00FFul) + ((tempCount0a >> 8) & 0x00FF00FFul);
  tempCount0 += tempCount0a;
  tempCount0 =
    ((tempCount0)&0x0000FFFFul) + ((tempCount0 >> 16) & 0x0000FFFFul);
#endif /* BITVECTOR_64BIT */
  ...
  CS2Assert(checkCount ==
              tempCount + tempCount2 + tempCount3 + tempCount4 + tempCount0,
...
template<class Allocator>
inline uint32_t
ABitVector<Allocator>::PopulationCount(const ABitVector<Allocator>& mask) const
{
...
template<class Allocator>
inline void
ABitVector<Allocator>::CopyToMemory(void* pMemory, BitIndex numBits) const
{
  ShortWord* pBitWord = (ShortWord*)pMemory;
  uint32_t shortwordIndex, wordIndex, smallerWordSize, vectorWordSize,
    memoryWordSize;
  ...
  for (shortwordIndex = wordIndex = 0; shortwordIndex < smallerWordSize;
       ++shortwordIndex) {
#ifdef BITVECTOR_64BIT
    union
    {
...
template<class Allocator>
inline void
ABitVector<Allocator>::CopyFromMemory(void* pMemory, BitIndex numBits)
{
  ShortWord* pBitWord = (ShortWord*)pMemory;
  uint32_t shortwordIndex, wordIndex, vectorWordSize, memoryWordSize,
    smallerWordSize;
...
// ABitVector<Allocator>::And (const ABitVector<Allocator> &,
// ABitVector<Allocator> &) const
//
// Compute the bitwise AND of this vector and the input vector.
...
template<class Allocator>
inline bool
ABitVector<Allocator>::And(const ABitVector<Allocator>& inputVector,
...
// ABitVector<Allocator>::Andc (const ABitVector<Allocator> &,
// ABitVector<Allocator> &) const
//
...
template<class Allocator>
inline bool
ABitVector<Allocator>::Andc(const ABitVector<Allocator>& inputVector,
                            ABitVector<Allocator>& outputVector) const
{
  uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize,
    largerWordSize, outputWordSize;
...
// ABitVector<Allocator>::Or (const ABitVector<Allocator> &,
// ABitVector<Allocator> &) const
//
// Compute the bitwise OR of this vector and the input vector.
// Place the result in the output vector.
...
template<class Allocator>
inline bool
ABitVector<Allocator>::Or(const ABitVector<Allocator>& inputVector,
                          ABitVector<Allocator>& outputVector) const
{
  uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize,
    largerWordSize, outputWordSize;
...
// ABitVector<Allocator>::Xor (const ABitVector<Allocator> &,
// ABitVector<Allocator> &) const
//
// Compute the bitwise XOR of this vector and the input vector.
// Place the result in the output vector.
...
template<class Allocator>
inline bool
ABitVector<Allocator>::Xor(const ABitVector<Allocator>& inputVector,
                           ABitVector<Allocator>& outputVector) const
{
  uint32_t wordIndex, thisWordSize, inputWordSize, smallerWordSize,
    largerWordSize, outputWordSize;
