...
public:
  ...
  public:
    Cursor(const ABitVector &adoptVector)
        : fVector(adoptVector), fWord(0), fIndex(0),
          fWordCount(fVector.SizeInWords()) {}

    Cursor(const Cursor &adoptCursor)
        : fVector(adoptCursor.fVector), fWord(adoptCursor.fVector),
          fIndex(adoptCursor.fIndex), fWordCount(adoptCursor.fWordCount) {}
...
template <class Allocator>
inline ABitVector<Allocator> &ABitVector<Allocator>::
operator=(const ABitVector<Allocator> &adoptVector) {
...
template <class Allocator>
inline bool ABitVector<Allocator>::
operator==(const ABitVector<Allocator> &v) const {
...
template <class Allocator>
inline bool ABitVector<Allocator>::
operator<=(const ABitVector<Allocator> &vector) const {
  uint32_t wordIndex, thisWordSize, vectorWordSize, smallerWordSize;
...
template <class Allocator>
inline ABitVector<Allocator> &ABitVector<Allocator>::
operator-=(const ABitVector<Allocator> &v) {
  Andc(v);
  return *this;
}
