...
class PhaseMeasuringNode : private Allocator {
 public:
  ...
  ListIndex FindChild(const char *name, HashValue hv = 0) const {
    ...
    if (fChildMap.Locate((char *)name, hi, hv)) return (fChildMap)[hi];
    return 0;
  }
...
inline void PhaseMeasuringNode<Meter, Allocator>::Dump(
    ostream &out, uint32_t indent, Metric total, bool running,
  ...
  if (offset < 72) offset += sprintf(line + offset, "%*s", 72 - offset, "");
  offset += sprintf(line + offset, "|%d", int(fCount));
  if (running) offset += sprintf(line + offset, "*");
  out << line << "\n";
}
...
class PhaseProfiler {
 ...
 public:
  ...
  void SetName(const char *name, HashValue hv = 0) {
    if (fSummaryMeter.Collect()) fIndex = fSummaryMeter.Find(name, hv);
  }
