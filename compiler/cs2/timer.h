...
typedef AIXTimer PlatformTimer;
    ...
    (defined(__MVS__) && defined(_XOPEN_SOURCE_EXTENDED))
/**
    ...
    if (csv)
      ...
      if (usecs / 1000) {
        ...
        if (alternativeFormat) {
          ...
          if (h)
            offset +=
                sprintf(line + offset, "%0d:%02d:%02d.%03d ", h, m, s, ms);
          else
...
public:
  RunnableMeter(void) : Meter(), fIsRunning(false) {}
   ...
   * to be running also.
   */
  RunnableMeter(const RunnableMeter &t) : Meter(), fIsRunning(t.fIsRunning) {}
...
public:
  ...
  PhaseMeasuringNode(const char *name = NULL, const Allocator &a = Allocator())
      : Allocator(a), fName(NULL), fParent(0), fChildMap(a), fTotalMetric(0),
        fCount(0), fRunning(false) {
    if (name) {
  ...
  PhaseMeasuringNode(const PhaseMeasuringNode &node)
      : Allocator(node), fName(NULL), fParent(node.fParent),
        fChildMap(node.fChildMap), fTotalMetric(node.fTotalMetric),
        fCount(node.fCount), fRunning(node.fRunning) {
    if (node.fName) {
  ...
  uint32_t sprintfMetric(char *line, Metric total,
                         bool alternativeFormat = false,
                         bool csv = false) const {
    CS2Assert(!fRunning, ("Cannot read running meter: %s", Name()));
    return Meter::sprintfMetric(line, fTotalMetric, total, alternativeFormat,
                                csv);
  }
  ...
  void Dump(ostream &out, uint32_t indent = 0, Metric total = 0,
            bool running = false, bool alternativeFormat = false,
            bool csv = false);
...
public:
   ...
   */
  PhaseMeasuringSummary(const char *name = "Total elapsed metric",
                        const Allocator &a = Allocator(), bool collect = true)
      : fNodes(a), fNode(0), fCollect(collect) {
   ...
   */
  template <class ostream>
  void DumpSummary(ostream &s, bool alternativeFormat = false,
                   bool csv = false);
  ...
  void Start(ListIndex child) {
    CS2Assert(fNodes[child].Parent() == GetCurrentMeter(),
              ("Incorrect meter nesting for meters %s %s",
               fNodes[GetCurrentMeter()].Name(), fNodes[child].Name()));
    fNodes[child].Start();
...
private:
  template <class ostream>
  void DumpSummaryNode(ostream &s, ListIndex root, uint32_t indent,
                       Metric total, bool running = false,
...
template <class ostream>
inline void
PhaseMeasuringNode<Meter, Allocator>::Dump(ostream &out, uint32_t indent,
                                           Metric total, bool running,
...
inline void PhaseMeasuringSummary<Meter, Allocator>::DumpSummaryNode(
    ostream &out, ListIndex root, uint32_t indent, Metric total, bool running,
    bool alternativeFormat, bool csv) {
...
template <class Meter, class Allocator>
template <class ostream>
inline void PhaseMeasuringSummary<Meter, Allocator>::DumpSummary(
    ostream &out, bool alternativeFormat, bool csv) {
  ...
  if (csv) {
    ...
    offset =
        sprintf(line, "Phase                           %s  |count *=active",
...
/////////////////////////
/**
 * \brief A convenience class for metering the metric spent in an optimization
 * phase. Metering starts when Start() is called. When Stop() is called, the
 * meter is stopped and the metric us submitted to gPhaseMeasuringSummary.
 */
template <class Meter, class Allocator,
          class PhaseMeasuringSummary =
              CS2::PhaseMeasuringSummary<Meter, Allocator>>
class PhaseProfiler {
private:
  ListIndex fIndex;
...
/**
 * \brief A convenience class for metering the metric spent in a lexical block.
 * Metering starts at construction and ends at destruction. The metric is
 * automatically reported to gPhaseMeasuringSummary.
 */
template <class Meter, class Allocator,
          class PhaseMeasuringSummary =
              CS2::PhaseMeasuringSummary<Meter, Allocator>>
...
public:
  /**
   * \brief Stores the phase name and starts a meter. Phase name must not be
   * null.
   */
  template <size_t N>
  ...
  LexicalBlockProfiler(const char *phase, PhaseMeasuringSummary &s =
                                              PhaseMeasuringSummary::Global())
      : PhaseProfiler<Meter, Allocator>(phase, s) {
    if (s.Collect()) {
      PhaseProfiler<Meter, Allocator>::Start();
    }
  ...
  LexicalBlockProfiler(
      const char *phase, uint32_t count,
      PhaseMeasuringSummary &s = PhaseMeasuringSummary::Global())
  ...
  LexicalBlockProfiler(
      const char *phase, const char *name,
      PhaseMeasuringSummary &s = PhaseMeasuringSummary::Global())
      : PhaseProfiler<Meter, Allocator>(NULL, s) {
    if (s.Collect()) {
      char pname[1024];
...
// following is equivalent is the closest equivalent to "template aliasing"
// PhaseMeasuringSummary in C++11:
//
...
class PhaseTimingSummary
    : public PhaseMeasuringSummary<RunnableTimer, Allocator> {
private:
  // Should not be copy constructed or copied.
...
public:
  PhaseTimingSummary(const char *name = "Total elapsed time",
                     const Allocator &a = Allocator(), bool collect = true)
      : PhaseMeasuringSummary<RunnableTimer, Allocator>(name, a, collect) {}
};

// following is equivalent is the closest equivalent to "template aliasing"
// LexicalBlockProfiler in C++11:
//
// template <class Allocator = CS2::allocator>
...
template <class Allocator,
          class PhaseTimingSummary =
              CS2::PhaseMeasuringSummary<RunnableTimer, Allocator>>
class LexicalBlockTimer : public LexicalBlockProfiler<RunnableTimer, Allocator,
                                                      PhaseTimingSummary> {
private:
  // Should not be copy constructed or copied.
...
public:
  ...
  LexicalBlockTimer(const char (&phase)[N],
      ...
      : LexicalBlockProfiler<RunnableTimer, Allocator, PhaseTimingSummary>(
            phase, s) {}
