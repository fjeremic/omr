...
public:
  ...
  {
    CS2Assert(!fRunning, ("Cannot read running meter: %s", Name()));
    return Meter::sprintfMetric(
      line, fTotalMetric, total, alternativeFormat, csv);
  }
...
template<size_t N>
inline HashValue
HashFunction(char const (&strVal)[N])
{
  int i;
...
template<class ostream>
inline void
PhaseMeasuringNode<Meter, Allocator>::Dump(ostream& out,
                                           uint32_t indent,
                                           Metric total,
                                           bool running,
...
template<class ostream>
inline void
PhaseMeasuringSummary<Meter, Allocator>::DumpSummaryNode(ostream& out,
                                                         ListIndex root,
                                                         uint32_t indent,
                                                         Metric total,
                                                         bool running,
...
template<class ostream>
inline void
PhaseMeasuringSummary<Meter, Allocator>::DumpSummary(ostream& out,
                                                     bool alternativeFormat,
                                                     bool csv)
{
...
class LexicalBlockTimer
  : public LexicalBlockProfiler<RunnableTimer, Allocator, PhaseTimingSummary>
{
private:
  // Should not be copy constructed or copied.
...
public:
  ...
  LexicalBlockTimer(const char (&phase)[N],
    ...
    : LexicalBlockProfiler<RunnableTimer, Allocator, PhaseTimingSummary>(phase,
                                                                         s)
  {}
