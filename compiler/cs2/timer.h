...
public:
  ...
  void Stop(void)
  {
    read_real_time(&stop, TIMEBASE_SZ);
  }
  void Reset(void)
  {
...
public:
  ...
  void SetParent(ListIndex parent)
  {
    fParent = parent;
  }
  ListIndex Parent() const
  {
    return fParent;
  }
...
public:
  ...
  LexicalBlockProfiler(const char* phase, const char* name, PhaseMeasuringSummary& s = PhaseMeasuringSummary::Global())
    : PhaseProfiler<Meter, Allocator>(NULL, s)
  {
    if (s.Collect()) {
      char pname[1024];
