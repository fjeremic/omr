...
public:
  ...
  void GrowTo(SparseBitIndex index,
              bool geometric = true,
...
private:
  ...
  public:
    ...
    void append(S2& s, uint16_t fromIndex, uint16_t toIndex)
    {
      ...
      memcpy(
        &fSegment[fNumValues], &s.Indices()[fromIndex], n * sizeof(uint16_t));
      fNumValues += n;
    }
    ...
    {
      fSegment = (uint16_t*)a.reallocate(
        size * sizeof(uint16_t), fSegment, fSize * sizeof(uint16_t));
      fSize = size;
    }
    ...
    bool operator==(const struct Segment& s1) const
    {
      ...
      return !memcmp(
        fSegment, s1.fSegment, sizeof(uint16_t) * PopulationCount());
    }
  ...
  Segment* AddSegment(SparseBitIndex index,
                      SparseBitIndex count,
                      uint16_t* bits);
  Segment* OrSegment(SparseBitIndex index,
                     SparseBitIndex count,
                     uint16_t* bits);
  void RemoveSegment(SparseBitIndex index);
  void RemoveSegmentAt(SparseBitIndex segmentindex);

  SparseBitIndex FindIndex(const Segment&,
                           uint16_t,
                           SparseBitIndex l = 0,
                           SparseBitIndex h = 0) const;
  SparseBitIndex AdvanceIndex(const Segment&,
                              uint16_t,
                              SparseBitIndex l = 0,
                              SparseBitIndex h = 0) const;
...
ASparseBitVector<Allocator>::SparseBitRef::Set()
{
  ...
  if (n) {
    ...
    base = (Segment*)fVector.reallocate(
      (n + 1) * sizeof(Segment), base, n * sizeof(Segment));
...
template<class Allocator>
inline void
Swap(ASparseBitVector<Allocator>& vectorA, ASparseBitVector<Allocator>& vectorB)
{
...
ASparseBitVector<Allocator>::ValueAt(size_t elementIndex) const
{
  ...
  if (s)
    return GetSegment(*s, elementIndex);
  return false;
}
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::GrowTo(SparseBitIndex index,
                                    bool geometric,
                                    bool forceGeometric)
{
  if (IsZero()) {
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::Clear()
{
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::ClearToNull()
{
  Clear();
  fBase = NULL;
}
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::Cursor::Valid() const
{
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::Cursor::SetToFirstOne()
{
  fSegmentIndex = 0;
  fSegmentOffset = 0;
  if (Valid()) {
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::Cursor::SetToNextOne()
{
  ...
  CS2Assert(fSegmentPopCount ==
              fVector.fBase[fSegmentIndex].PopulationCount() - 1,
            ("ASparseBitVector updated durign traversal %d %d",
             fSegmentPopCount,
             fVector.fBase[fSegmentIndex].PopulationCount() - 1));
  fSegmentIndex += 1;
  if (!Valid())
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::Cursor::SetToPreviousOne()
{
  CS2Assert(Valid(), ("Cannot advance an invalid cursor"));
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::Cursor::SetToNextOneAfter(SparseBitIndex next)
{
  CS2Assert(Valid(), ("Cannot advance an invalid cursor"));
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::Cursor::SetToLastOne()
{
...
template<class Allocator>
inline SparseBitIndex
ASparseBitVector<Allocator>::PopulationCount(SparseBitIndex numBits) const
{
...
template<class Allocator>
inline SparseBitIndex
ASparseBitVector<Allocator>::PopulationCount(
...
template<class Allocator>
inline SparseBitIndex
ASparseBitVector<Allocator>::SizeInBits() const
{
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::IsZero() const
{
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::IsNull() const
{
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::Intersects(
...
template<class Allocator>
inline SparseBitIndex
ASparseBitVector<Allocator>::FindIndex(
...
template<class Allocator>
inline SparseBitIndex
ASparseBitVector<Allocator>::AdvanceIndex(
  const typename ASparseBitVector<Allocator>::Segment& thisSegment,
  uint16_t s,
  SparseBitIndex l,
  SparseBitIndex h) const
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::GrowSegment(
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::SetSegment(
  typename ASparseBitVector<Allocator>::Segment& thisSegment,
  SparseBitIndex bit)
{
  ...
  if (thisSegment.AllocatedSize() == thisSegment.fNumValues)
    GrowSegment(
      thisSegment, thisSegment.AllocatedSize(), thisSegment.fNumValues + 1);
  ...
  CS2Assert(thisSegment.fNumValues < thisSegment.AllocatedSize(),
            ("Segment Overflow %d(%d %d)>=%d",
             (thisSegment.fNumValues - 1 - t) + (t + 1),
             thisSegment.fNumValues,
             t,
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::ClearSegment(
  typename ASparseBitVector<Allocator>::Segment& thisSegment,
  SparseBitIndex bit)
{
  SparseBitIndex t = FindIndex(thisSegment, bit);
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::GetSegment(
  typename ASparseBitVector<Allocator>::Segment& thisSegment,
...
template<class Allocator>
inline SparseBitIndex
ASparseBitVector<Allocator>::ValueAtSegment(
  const typename ASparseBitVector<Allocator>::Segment& thisSegment,
                                        ...
                                        SparseBitIndex count)
{
  ...
  if (n) {
    ...
    base = (Segment*)Allocator::reallocate(
      (n + 1) * sizeof(Segment), base, n * sizeof(Segment));
    memmove(&base[i + 1], &base[i], (n - i) * sizeof(Segment));
  } else {
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::OrSegment(
  typename ASparseBitVector<Allocator>::Segment& thisSegment,
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::OrSegmentMask(
  typename ASparseBitVector<Allocator>::Segment& thisSegment,
  const typename ASparseBitVector<Allocator>::Segment& inputSegment,
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::AndSegment(
  typename ASparseBitVector<Allocator>::Segment& thisSegment,
  const typename ASparseBitVector<Allocator>::Segment& inputSegment)
{
  if (thisSegment.IsZero())
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::AndcSegment(
  typename ASparseBitVector<Allocator>::Segment& thisSegment,
  const typename ASparseBitVector<Allocator>::Segment& inputSegment)
{
  if (thisSegment.IsZero())
  ...
  while (i < myPopCount - 1) { // if we're at last element, nothing to copy
    ...
    memmove(&thisSegment.Indices()[oi],
            &thisSegment.Indices()[ci],
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::IsSubset(
  const typename ASparseBitVector<Allocator>::Segment& thisSegment,
...
template<class str>
inline void
ASparseBitVector<Allocator>::DumpSegment(
  str& out,
  const typename ASparseBitVector<Allocator>::Segment& s) const
{
  SparseBitIndex i;
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::TruncateSegment(
  typename ASparseBitVector<Allocator>::Segment& thisSegment)
{
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::RemoveSegmentAt(SparseBitIndex segmentIndex)
{
  ...
  if (fNumberOfSegments > 1) {
    ...
    fBase =
      (Segment*)Allocator::reallocate(sizeof(Segment) * (fNumberOfSegments - 1),
                                      fBase,
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::RemoveSegment(SparseBitIndex index)
{
  SparseBitIndex i;
...
ASparseBitVector<Allocator>::MemoryUsage() const
{
  unsigned long ret = 0;
  SparseBitIndex i;
  for (i = 0; i < fNumberOfSegments; i++)
...
inline str&
operator<<(str& out, const ASparseBitVector<Allocator>& vector)
{
  vector.Dump(out);
  return out;
}
...
template<class str>
inline void
ASparseBitVector<Allocator>::Dump(str& out) const
{
  if (IsNull()) {
    out << "0";
    return;
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::Andc(
...
template<class B2>
inline bool
ASparseBitVector<Allocator>::Andc(const B2& inputVector)
{
  ...
  ASparseBitVector<Allocator>& outputVector) const
{
  ...
  for (i = 0; i < fNumberOfSegments; i++) {
    ...
    if (!is) {
      outputVector.AddSegment(
        s->HighBits(), s->PopulationCount(), s->Indices());
    } else {
...
template<class B2>
inline bool
ASparseBitVector<Allocator>::And(const B2& vector)
{
  if (IsZero())
    return false;
...
template<class B2>
inline void
ASparseBitVector<Allocator>::Or(const B2& abv)
{
...
template<class ACursor>
inline void
ASparseBitVector<Allocator>::OrCursor(ACursor& abvc)
{
...
template<class Allocator>
inline void
ASparseBitVector<Allocator>::Or(ASparseBitVector<Allocator>** vs,
                                SparseBitIndex nvs)
{
  if (nvs == 0)
    return;
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::Or(const ASparseBitVector<Allocator>& inputVector)
{
  if (inputVector.IsZero())
    return false;
  if (IsZero()) {
                                ...
                                ASparseBitVector<Allocator>& outputVector) const
{
  ...
  for (i = 0; i < inputVector.fNumberOfSegments; i++) {
    ...
    if (!s) {
      typename ASparseBitVector<Allocator>::Segment* os =
        outputVector.AddSegment(
          is->HighBits(), is->PopulationCount(), is->Indices());
      changed = true;
    }
  }
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::OrMask(
  const ASparseBitVector<Allocator>& inputVector,
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::And(const ASparseBitVector<Allocator>& inputVector)
{
  if (IsZero())
    return false;
  if (inputVector.IsZero()) {
...
template<class Allocator>
inline bool
ASparseBitVector<Allocator>::And(
  const ASparseBitVector<Allocator>& inputVector,
  ASparseBitVector<Allocator>& outputVector) const
{
  if (IsZero()) {
...
template<class B2>
inline bool
ASparseBitVector<Allocator>::Intersects(const B2& inputVector) const
{
  if (IsZero())
    return false;
