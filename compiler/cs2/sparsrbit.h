...
class ASparseBitVector : private Allocator {
 ...
 public:
  ...
  ASparseBitVector(const Allocator &a = Allocator())
      : Allocator(a), fBase(NULL), fNumberOfSegments(0) {}
  ...
  ASparseBitVector(const ASparseBitVector &v)
    ...
    if (n) {
      ...
      for (size_t i = 0; i < n; i++) fBase[i].copy(v.fBase[i], *this);
    } else {
  ...
  SparseBitIndex ClearLastOneIfThereIsOneInRange(SparseBitIndex low,
                                                 SparseBitIndex high,
                                                 bool &foundone);
  ...
  bool hasBitWordRepresentation() const {
    return false;
  }  // returns whether this bit vector is based on an array of bitWords (which
     // it is not)
  ...
  void GrowTo(SparseBitIndex index, bool geometric = true,
              bool forceGeometric = false);
  ...
  template <class B2>
  bool Andc(const B2 &inputVector);
  ...
  // Clear all bits in the vector (ie. clear and discard memory), and make
  // empty.
  void Clear();
  ...
  // Determine if the vector is cleared to null, ie is the zero measure set
  // "null set" which is also the empty set.
  bool IsNull() const;
 ...
 private:
  class Segment {
   public:
    ...
    void append(S2 &s, uint16_t fromIndex, uint16_t toIndex) {
      ...
      memcpy(&fSegment[fNumValues], &s.Indices()[fromIndex],
             n * sizeof(uint16_t));
      fNumValues += n;
    }
    ...
    void adopt(uint16_t highbits, size_t numValues, size_t size,
               uint16_t *bits) {
      fHighBits = highbits;
      adopt(numValues, size, bits);
    }

    // shallow swap of this with s (cannot override Swap method in cs2.h as
    // Segment is private)
    void swap(Segment &s) {
    ...
    void reallocate(size_t size, Allocator &a) {
      fSegment = (uint16_t *)a.reallocate(size * sizeof(uint16_t), fSegment,
                                          fSize * sizeof(uint16_t));
      fSize = size;
    }

    void deallocate(Allocator &a) {
      a.deallocate(fSegment, fSize * sizeof(uint16_t));
    }
    ...
    SparseBitIndex LastOne() const {
      return HighBits() + fSegment[fNumValues - 1];
    }
    ...
    bool operator==(const struct Segment &s1) const {
      ...
      return !memcmp(fSegment, s1.fSegment,
                     sizeof(uint16_t) * PopulationCount());
    }

    size_t MemoryUsage() { return sizeof(Segment) + fSize * sizeof(uint16_t); }
    //  TODO: need to add destructor, then change everywhere that deallocates to
    //  rely on this destructor
  ...
  Segment *AddSegment(SparseBitIndex index, SparseBitIndex count,
                      uint16_t *bits);
  Segment *OrSegment(SparseBitIndex index, SparseBitIndex count,
                     uint16_t *bits);
  void RemoveSegment(SparseBitIndex index);
  void RemoveSegmentAt(SparseBitIndex segmentindex);

  SparseBitIndex FindIndex(const Segment &, uint16_t, SparseBitIndex l = 0,
                           SparseBitIndex h = 0) const;
  SparseBitIndex AdvanceIndex(const Segment &, uint16_t, SparseBitIndex l = 0,
                              SparseBitIndex h = 0) const;

  // Grow thisSegment to at least the given size.
  //
  // if numValues is given, then it must accommodate numValues. Thus if
  // numValues exceeds given size then grow size by some factor more. The new
  // size is then guaranteed to exceed numValues.
  ...
  template <class str>
  void DumpSegment(str &out, const Segment &vector) const;
  ...
  class SparseBitRef {
   public:
    ...
    SparseBitRef(const SparseBitRef &r)
        : fIndex(r.fIndex), fVector(r.fVector) {}
 ...
 public:
  ...
  class IntersectionCursor {
   public:
    ...
    bool SetToFirstOne() {
      ...
      if (Valid()) return FindNextCommonItem();
      return false;
    }
    bool SetToNextOne() {
      if (c1 <= c2) {
    ...
    bool FindNextMaskedItem() {
      if (m.Valid()) {
        while (c >= m) {
          ...
          if (!m.SetToNextOneAfter(c - 1)) break;
        }
      }
      return true;
    }
  ...
  template <class A2>
  friend inline void Swap(ASparseBitVector<A2> &vectorA,
                          ASparseBitVector<A2> &vectorB);
};
...
template <class Allocator>
inline typename ASparseBitVector<Allocator>::SparseBitRef &
ASparseBitVector<Allocator>::SparseBitRef::operator=(bool bitValue) {
...
template <class Allocator>
inline typename ASparseBitVector<Allocator>::SparseBitRef &
ASparseBitVector<Allocator>::SparseBitRef::Set() {
  ...
  if (n) {
    ...
    base = (Segment *)fVector.reallocate((n + 1) * sizeof(Segment), base,
                                         n * sizeof(Segment));
...
template <class Allocator>
inline typename ASparseBitVector<Allocator>::SparseBitRef &
ASparseBitVector<Allocator>::SparseBitRef::Clear() {
  typename ASparseBitVector<Allocator>::Segment *segment =
      fVector.FindSegment(fIndex);
  if (segment) {
...
template <class Allocator>
inline ASparseBitVector<Allocator> &ASparseBitVector<Allocator>::operator=(
    const ASparseBitVector<Allocator> &vector) {
  if (vector.IsNull())
    ClearToNull();
  else
    Clear();
...
template <class Allocator>
inline void Swap(ASparseBitVector<Allocator> &vectorA,
                 ASparseBitVector<Allocator> &vectorB) {
...
template <class A2>
inline ASparseBitVector<Allocator> &ASparseBitVector<Allocator>::operator=(
    const ASparseBitVector<A2> &vector) {
  if (vector.IsNull())
    ClearToNull();
  else
    Clear();
...
inline ASparseBitVector<Allocator> &ASparseBitVector<Allocator>::operator=(
  ...
  while (c1.Valid()) {
    ...
    for (c1.SetToNextOne(); c1.Valid() && ((c1 & ~kSegmentMask) == highbits);
         c1.SetToNextOne()) {
      count++;
    }
...
inline void ASparseBitVector<Allocator>::Compact() {
  size_t i;
  for (i = 0; i < fNumberOfSegments; i++) CompactSegment(fBase[i]);
}
...
template <class Allocator>
inline typename ASparseBitVector<Allocator>::SparseBitRef
    ASparseBitVector<Allocator>::operator[](size_t index) {
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::ValueAt(size_t elementIndex) const {
...
template <class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::FirstOne() const {
...
template <class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::LastOne() const {
...
ASparseBitVector<Allocator>::ClearLastOneIfThereIsOneInRange(
  ...
  if (IsZero()) {
    ...
    CS2Assert(ret <= high,
              ("sparsebitvector and tableof disagree on highbounds"));
    if (s.fNumValues <= 1)
...
template <class Allocator>
inline void ASparseBitVector<Allocator>::GrowTo(SparseBitIndex index,
                                                bool geometric,
                                                bool forceGeometric) {
  if (IsZero()) {
    Clear();
...
template <class Allocator>
inline ASparseBitVector<Allocator>::Cursor::Cursor(
    const ASparseBitVector<Allocator> &adoptVector)
    : fVector(adoptVector) {}

template <class Allocator>
inline ASparseBitVector<Allocator>::Cursor::Cursor(
    const typename ASparseBitVector<Allocator>::Cursor &adoptCursor)
...
inline bool ASparseBitVector<Allocator>::Cursor::SetToNextOne() {
  ...
  CS2Assert(
      fSegmentPopCount == fVector.fBase[fSegmentIndex].PopulationCount() - 1,
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::Cursor::SetToNextOneAfter(
    SparseBitIndex next) {
  CS2Assert(Valid(), ("Cannot advance an invalid cursor"));
...
// number of leading bits to examine.
template <class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::PopulationCount(
    SparseBitIndex numBits) const {
  CS2Assert(numBits == 0xEFFFFFFFul,
            ("Population count subset not implemented"));
...
template <class Allocator>
inline SparseBitIndex ASparseBitVector<Allocator>::PopulationCount(
    const ASparseBitVector<Allocator> &mask) const {
  SparseBitIndex ret = 0;

  typename ASparseBitVector<Allocator>::IntersectionCursor c(*this, mask);
  for (c.SetToFirstOne(); c.Valid(); c.SetToNextOne()) ret += 1;
  return ret;
}
...
template <class Allocator>
inline typename ASparseBitVector<Allocator>::Segment *
ASparseBitVector<Allocator>::FindSegment(SparseBitIndex index) const {
  size_t i, n = fNumberOfSegments;
  for (i = 0; i < n; i++) {
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::Intersects(
    const ASparseBitVector<Allocator> &inputVector) const {
  size_t i1 = 0, i2 = 0, n1 = fNumberOfSegments,
         n2 = inputVector.fNumberOfSegments;
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::operator!=(
    const ASparseBitVector &inputVector) const {
...
inline SparseBitIndex ASparseBitVector<Allocator>::FindIndex(
    const typename ASparseBitVector<Allocator>::Segment &thisSegment,
    uint16_t search, SparseBitIndex l, SparseBitIndex h) const {
  size_t low = l, high = h;
...
inline SparseBitIndex ASparseBitVector<Allocator>::AdvanceIndex(
    const typename ASparseBitVector<Allocator>::Segment &thisSegment,
    uint16_t s, SparseBitIndex l, SparseBitIndex h) const {
  ...
  if (long(search - value) + low < high) high = long(search - value) + low;
  ...
  while (high - low > residue) {
    ...
    if (value == search)
      ...
      low = mid;
      if (long(search - value) + low <
          high) /* trim range based on search value */
  ...
  if (low > high || thisSegment.Indices()[low] >= search) return low;
...
inline void ASparseBitVector<Allocator>::GrowSegment(
    typename ASparseBitVector<Allocator>::Segment &thisSegment, uint32_t size,
    uint32_t numValues) {
  if (numValues > 0) {
    if (numValues <= 1024) {
...
inline void ASparseBitVector<Allocator>::SetSegment(
    typename ASparseBitVector<Allocator>::Segment &thisSegment,
    SparseBitIndex bit) {
  ...
  if (thisSegment.AllocatedSize() == thisSegment.fNumValues)
    GrowSegment(thisSegment, thisSegment.AllocatedSize(),
                thisSegment.fNumValues + 1);

  memmove(&thisSegment.Indices()[t + 1], &thisSegment.Indices()[t],
          (thisSegment.fNumValues - t) * sizeof(uint16_t));
  CS2Assert(thisSegment.fNumValues < thisSegment.AllocatedSize(),
            ("Segment Overflow %d(%d %d)>=%d",
             (thisSegment.fNumValues - 1 - t) + (t + 1), thisSegment.fNumValues,
             t, thisSegment.AllocatedSize()));
...
inline void ASparseBitVector<Allocator>::ClearSegment(
  ...
  if (t < thisSegment.fNumValues && bit == ValueAtSegment(thisSegment, t)) {
    thisSegment.fNumValues -= 1;
    memmove(&thisSegment.Indices()[t], &thisSegment.Indices()[t + 1],
            (thisSegment.fNumValues - t) * sizeof(uint16_t));
  }
}
...
template <class Allocator>
inline typename ASparseBitVector<Allocator>::Segment *
ASparseBitVector<Allocator>::AddSegment(SparseBitIndex index,
  ...
  if (n) {
    ...
    base = (Segment *)Allocator::reallocate((n + 1) * sizeof(Segment), base,
                                            n * sizeof(Segment));
    memmove(&base[i + 1], &base[i], (n - i) * sizeof(Segment));
  } else {
...
inline typename ASparseBitVector<Allocator>::Segment *
ASparseBitVector<Allocator>::AddSegment(SparseBitIndex index,
                                        SparseBitIndex count, uint16_t *bits) {
...
template <class Allocator>
inline typename ASparseBitVector<Allocator>::Segment *
ASparseBitVector<Allocator>::OrSegment(SparseBitIndex index,
                                       SparseBitIndex count, uint16_t *bits) {
...
inline void ASparseBitVector<Allocator>::CopySegment(
  ...
  CS2Assert(thisSegment.HighBits() == inputSegment.HighBits(),
            ("Copying segments with diff hb %d!=%d", thisSegment.HighBits(),
             inputSegment.HighBits()));

  thisSegment.reallocate(inputSegment.PopulationCount(), *this);
  memcpy(thisSegment.Indices(), inputSegment.Indices(),
         inputSegment.PopulationCount());
}

template <class Allocator>
inline void ASparseBitVector<Allocator>::CompactSegment(
    typename ASparseBitVector<Allocator>::Segment &thisSegment) {
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::OrSegment(
    typename ASparseBitVector<Allocator>::Segment &thisSegment,
...
inline bool ASparseBitVector<Allocator>::OrSegment(
  ...
  SparseBitIndex toAllocate =
      (thisSegment.PopulationCount() + inputSegment.PopulationCount());
  ...
  if (isInplace && (thisSegment.LastOne() < inputSegment.FirstOne())) {
    ...
    while (ii < inPopCount) {
      ...
      if ((i < myPopCount) &&
          thisSegment.Indices()[i] == inputSegment.Indices()[ii]) {
        // Ignore common bits
        ii += 1;
      } else {
...
// Segment methods
template <class Allocator>
inline bool ASparseBitVector<Allocator>::OrSegmentMask(
    typename ASparseBitVector<Allocator>::Segment &thisSegment,
    const typename ASparseBitVector<Allocator>::Segment &inputSegment,
    const typename ASparseBitVector<Allocator>::Segment &maskSegment) {
  if (inputSegment.IsZero() || maskSegment.IsZero()) return false;
  ...
  while (ii < inPopCount && mi < maskPopCount) {
    ...
    if (i == myPopCount ||
        thisSegment.Indices()[i] > maskSegment.Indices()[mi]) {
      if (tmpSegment.IsZero()) {
        GrowSegment(tmpSegment, thisSegment.AllocatedSize(), toAllocate);
        tmpSegment.fHighBits = thisSegment.fHighBits;
      }
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::AndSegment(
    typename ASparseBitVector<Allocator>::Segment &thisSegment,
    const typename ASparseBitVector<Allocator>::Segment &inputSegment) {
  ...
  SparseBitIndex myPopCount = thisSegment.PopulationCount();
  SparseBitIndex inPopCount = inputSegment.PopulationCount();
  SparseBitIndex resPopCount =
      (myPopCount > inPopCount) ? myPopCount : inPopCount;
  ...
  for (i = 0; i < thisSegment.PopulationCount(); i++) {
    ...
    if (inputSegment.Indices()[ii] == thisSegment.Indices()[i]) {
      CS2Assert(oi < outputSegment.AllocatedSize(),
                ("Writing past the end of segment %d >= %d (is=%d)", oi,
                 outputSegment.AllocatedSize(), PopulationCount()));
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::AndcSegment(
    typename ASparseBitVector<Allocator>::Segment &thisSegment,
    const typename ASparseBitVector<Allocator>::Segment &inputSegment) {
  if (thisSegment.IsZero()) return false;
  ...
  while (i < myPopCount - 1) {  // if we're at last element, nothing to copy
    ...
    memmove(&thisSegment.Indices()[oi], &thisSegment.Indices()[ci],
            (i - ci) * sizeof(uint16_t));
    oi += i - ci;
  }
  if (oi == 0)
    TruncateSegment(thisSegment);
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::IsSubset(
    const typename ASparseBitVector<Allocator>::Segment &thisSegment,
...
inline void ASparseBitVector<Allocator>::DumpSegment(
  ...
  for (i = 0; i < s.PopulationCount(); i++) out << ValueAtSegment(s, i) << " ";
}

template <class Allocator>
inline void ASparseBitVector<Allocator>::TruncateSegment(
    typename ASparseBitVector<Allocator>::Segment &thisSegment) {
  thisSegment.fNumValues = 0;
}

template <class Allocator>
inline void ASparseBitVector<Allocator>::RemoveSegmentAt(
    SparseBitIndex segmentIndex) {
  ...
  if (fNumberOfSegments > 1) {
    ...
    fBase = (Segment *)Allocator::reallocate(
        sizeof(Segment) * (fNumberOfSegments - 1), fBase,
...
template <class Allocator>
inline void ASparseBitVector<Allocator>::RemoveSegment(SparseBitIndex index) {
  SparseBitIndex i;

  for (i = 0; i < fNumberOfSegments; i++) {
    if (fBase[i].fHighBits >= index >> 16) {
      if (fBase[i].fHighBits == index >> 16) RemoveSegmentAt(i);
      return;
    }
  }
}
...
template <class Allocator>
inline unsigned long ASparseBitVector<Allocator>::MemoryUsage() const {
  unsigned long ret = 0;
  SparseBitIndex i;
  for (i = 0; i < fNumberOfSegments; i++) ret += fBase[i].MemoryUsage();
...
// Return the number of '1' bits in the vector.  Optionally specify the
// number of leading bits to examine.
// SparseBitIndex ASparseBitVector<Allocator>::PopulationCount (SparseBitIndex
// numBits) const{
//  SparseBitIndex i,ret=0;
//  CS2Assert(numBits==0xEFFFFFFFul,("Population count subset not
//  implemented")); for (i=0; i< fNumberOfSegments; i++)
...
inline str &operator<<(str &out, const ASparseBitVector<Allocator> &vector) {
  vector.Dump(out);
  return out;
}
...
inline bool ASparseBitVector<Allocator>::Andc(
    const ASparseBitVector<Allocator> &inputVector) {
  ...
  for (i = 0; i < fNumberOfSegments; i++) {
    ...
    const typename ASparseBitVector<Allocator>::Segment *is =
        inputVector.FindSegment(s->HighBits());
...
template <class Allocator>
template <class B2>
inline bool ASparseBitVector<Allocator>::Andc(const B2 &inputVector) {
  if (IsZero()) return false;
...
inline bool ASparseBitVector<Allocator>::Andc(
  ...
  for (i = 0; i < fNumberOfSegments; i++) {
    typename ASparseBitVector<Allocator>::Segment *s = &fBase[i];
    typename ASparseBitVector<Allocator>::Segment *is =
        inputVector.FindSegment(s->HighBits());
...
inline bool ASparseBitVector<Allocator>::And(const B2 &vector) {
  ...
  while (c1.Valid() && (i < fNumberOfSegments)) {
    ...
    for (; i < fNumberOfSegments; /* do not i++ */) {
      ...
      if (sHighBits >= highBits) {
        if (sHighBits > highBits) s = NULL;
        break;
      }
      s = NULL;
      // this Segment does not exist in vector; AND is zero - so delete it, do
      // not i++ after
      RemoveSegmentAt(i);
      changed = true;
    }
    ...
    if (s == NULL) { /* did not find segment, need to advance c2 for next bit */
      ...
      uint16_t *bits =
          (uint16_t *)Allocator::allocate(count * sizeof(uint16_t));
      for (SparseBitIndex bi = 0; bi < count; bi++) {
  ...
  for (; i < fNumberOfSegments; i++) {
    // we are beyond highest Segment for vector; AND is zero - so delete any
    // higher segments of *this
    changed = true;
    RemoveSegmentAt(i);
    i--;
  }
...
inline void ASparseBitVector<Allocator>::OrCursor(ACursor &abvc) {
  ...
  while (abvc.Valid()) {
    uint16_t *bits =
        (uint16_t *)Allocator::allocate(BUFFERSIZE * sizeof(uint16_t));
    SparseBitIndex bc = 0;
    ...
    do {
      ...
      abvc.SetToNextOne();
    } while (bc < BUFFERSIZE && abvc.Valid() &&
...
template <class Allocator>
inline void ASparseBitVector<Allocator>::Or(ASparseBitVector<Allocator> **vs,
                                            SparseBitIndex nvs) {
  if (nvs == 0) return;
  if (nvs == 1) {
    Or(*vs[0]);
  ...
  typename ASparseBitVector<Allocator>::Segment **ls =
      (typename ASparseBitVector<Allocator>::Segment **)Allocator::allocate(
          sizeof(void *) * nvs);
  SparseBitIndex *cs =
      (SparseBitIndex *)Allocator::allocate(sizeof(SparseBitIndex) * nvs);
...
inline void ASparseBitVector<Allocator>::Or(
    ASparseBitVector<Allocator> **vs, SparseBitIndex nvs,
    const ASparseBitVector<Allocator> &mask) {
  ...
  for (SparseBitIndex i = 0; i < nvs; i++) {
    for (SparseBitIndex s = 0; s < vs[i]->fNumberOfSegments; s++) {
      SparseBitIndex hbits = vs[i]->fBase[s].HighBits();
      if (mask.FindSegment(hbits)) segments[hbits >> 16] = 1;
    }
  }
  ...
  for (sc.SetToFirstOne(); sc.Valid(); sc.SetToNextOne()) {
    ...
    for (SparseBitIndex tile = 0; mc < mlast && tile < TILE_COUNT; tile++) {
      ...
      while (i < count) {
        ...
        while (c < last && sp[c] < high) {
          if (bits[sp[c] - low] == 2) bits[sp[c] - low] = 1;
          c += 1;
        }
        if (c < last) {
          cs[i2] = c;
...
inline bool ASparseBitVector<Allocator>::Or(
  ...
  for (i = 0; i < inputVector.fNumberOfSegments; i++) {
    ...
    if (!s) {
      typename ASparseBitVector<Allocator>::Segment *os =
          outputVector.AddSegment(is->HighBits(), is->PopulationCount(),
                                  is->Indices());
      changed = true;
    }
  }
...
inline bool ASparseBitVector<Allocator>::OrMask(
  ...
  for (i = 0; i < inputVector.fNumberOfSegments; i++) {
    ...
    typename ASparseBitVector<Allocator>::Segment *ms =
        maskVector.FindSegment(is->HighBits());
    if (ms) {
      typename ASparseBitVector<Allocator>::Segment *s =
          AddSegment(is->HighBits(), is->PopulationCount());
