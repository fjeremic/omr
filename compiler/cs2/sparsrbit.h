...
private:
  ...
  public:
    ...
    SparseBitIndex AllocatedSize() const
    {
      return SparseBitIndex(fSize);
    }
    SparseBitIndex HighBits() const
    {
      return SparseBitIndex(fHighBits) << 16;
    }
    SparseBitIndex IsZero() const
    {
      return fNumValues == 0;
    }
    SparseBitIndex PopulationCount() const
    {
      return fNumValues;
    }
...
public:
  ...
  template <class A2>
  friend inline void Swap(ASparseBitVector<A2>& vectorA, ASparseBitVector<A2>& vectorB);
};
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::operator!=(const ASparseBitVector& inputVector) const
{
  ...
  typename ASparseBitVector<Allocator>::Segment& thisSegment, SparseBitIndex bit)
{
  ...
  if (t < thisSegment.fNumValues && bit == ValueAtSegment(thisSegment, t)) {
    thisSegment.fNumValues -= 1;
    memmove(&thisSegment.Indices()[t], &thisSegment.Indices()[t + 1], (thisSegment.fNumValues - t) * sizeof(uint16_t));
  }
}
  ...
  for (i = 0; i < thisSegment.PopulationCount(); i++) {
    ...
    if (inputSegment.Indices()[ii] == thisSegment.Indices()[i]) {
      CS2Assert(oi < outputSegment.AllocatedSize(),
        ("Writing past the end of segment %d >= %d (is=%d)", oi, outputSegment.AllocatedSize(), PopulationCount()));
...
template <class Allocator>
inline void ASparseBitVector<Allocator>::RemoveSegmentAt(SparseBitIndex segmentIndex)
{
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::Andc(const ASparseBitVector<Allocator>& inputVector)
{
  if (IsZero())
    return false;
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::Or(const ASparseBitVector<Allocator>& inputVector)
{
  if (inputVector.IsZero())
    return false;
  if (IsZero()) {
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::And(const ASparseBitVector<Allocator>& inputVector)
{
  if (IsZero())
    return false;
  if (inputVector.IsZero()) {
