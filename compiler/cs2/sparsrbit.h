...
private:
  ...
  public:
    ...
    template <class S2> void append(S2 &s) {
...
template <class Allocator>
inline ASparseBitVector<Allocator> &ASparseBitVector<Allocator>::
operator=(const ASparseBitVector<Allocator> &vector) {
  if (vector.IsNull())
    ClearToNull();
  else
    Clear();
...
template <class A2>
inline ASparseBitVector<Allocator> &ASparseBitVector<Allocator>::
operator=(const ASparseBitVector<A2> &vector) {
  if (vector.IsNull())
    ClearToNull();
  else
    Clear();
...
inline ASparseBitVector<Allocator>::Cursor::Cursor(
    ...
    : fVector(adoptCursor.fVector), fSegment(adoptCursor.fSegment),
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::
operator!=(const ASparseBitVector &inputVector) const {
...
template <class Allocator>
inline bool ASparseBitVector<Allocator>::
operator==(const ASparseBitVector<Allocator> &inputVector) const {
  if (IsZero() && inputVector.IsZero())
    return true;
  if (IsZero() || inputVector.IsZero())
    return false;
