...
public:
  void *allocate(size_t size, const char *name = NULL) { return malloc(size); }
...
template <class base_allocator> class stat_allocator : private base_allocator {
public:
  ...
  stat_allocator(const base_allocator &a = base_allocator(),
                 bool _stats = false)
      : base_allocator(a), collect_stats(_stats), alloc_cnt(0), dealloc_cnt(0),
        realloc_cnt(0), alloc_size(0), realloc_size(0), dealloc_size(0),
        watermark(0), high_watermark(0) {}

  ~stat_allocator() {
    if (collect_stats && alloc_cnt != 0) {
      printf("  ALLOC= %llu SIZE=%llu AVG=%llu\n",
             (long long unsigned int)alloc_cnt,
             (long long unsigned int)alloc_size,
             (long long unsigned int)(alloc_cnt == 0 ? 0
                                                     : alloc_size / alloc_cnt));
      printf("DEALLOC= %llu SIZE=%llu AVG=%llu\n",
             (long long unsigned int)dealloc_cnt,
             (long long unsigned int)dealloc_size,
             (long long unsigned int)(dealloc_cnt == 0
                                          ? 0
                                          : dealloc_size / alloc_cnt));
      printf("REALLOC= %llu SIZE=%llu AVG=%llu\n",
             (long long unsigned int)realloc_cnt,
             (long long unsigned int)realloc_size,
             (long long unsigned int)(realloc_cnt == 0
                                          ? 0
                                          : realloc_size / alloc_cnt));
...
template <size_t segmentsize = 65536, uint32_t segmentcount = 10,
          class base_allocator = ::CS2::malloc_allocator>
...
private:
  ...
  public:
    Segment(Segment *n)
        : next(n), prev(NULL), freelist(NULL), alloc(0), freed(0) {
      if (n)
        n->prev = this;
    }
    ...
    void *allocate(uint32_t index) {
      ...
      if (!is_full(index)) {
        void *ret =
            (char *)this + sizeof(Segment) + element_size(index) * alloc;
        alloc += 1;
        return ret;
      }
      return NULL;
    ...
    uint32_t get_free(uint32_t index) {
      return (element_count(index) - alloc) * element_size(index);
    }
    ...
    for (uint32_t i = 1; i < segmentcount; i++) {
      ...
      if (totalsize)
        o << " segment[" << i << "](" << s->element_size(i) << ")"
          << " count=" << numsegs << " size(alloc,free)=(" << allocsize << ","
          << freesize << ":" << (allocsize * 100 / totalsize) << "%) "
          << " pad=(" << totalsize - allocsize - freesize << ":"
          << (totalsize - allocsize - freesize) * 100 / totalsize << "%)\n";
    }
    return base_allocator::stats(o, a);
  }
...
public:
  ...
  friend bool operator==(const shared_allocator &left,
                         const shared_allocator &right) {
...
template <size_t segmentsize = 65536,
          class base_allocator = ::CS2::malloc_allocator>
...
public:
  arena_allocator(base_allocator b = base_allocator())
      : base_allocator(b), segment(NULL), allocated(0) {}
  ~arena_allocator() {
    Segment *s = segment;
  ...
  void *allocate(size_t size, const char *name = NULL) {
    ...
    if (segment && size >= arena_size()) {
      Segment *new_segment =
          (Segment *)base_allocator::allocate(sizeof(Segment) + size, name);
      ...
      Segment *new_segment =
          (Segment *)base_allocator::allocate(segmentsize, name);
  ...
  template <class ostr, class allocator> ostr &stats(ostr &o, allocator &a) {
    uint32_t c = 0;
    for (Segment *s = segment; s; s = s->next)
      c += 1;
