...
class heap_allocator : private base_allocator {
 ...
 public:
  heap_allocator(const base_allocator &a = base_allocator())
      : base_allocator(a) {
    for (uint32_t i = 0; i < segmentcount; i++) segments[i] = NULL;
  }
  ...
  void *allocate(size_t size, const char *name = NULL) {
    ...
    for (Segment *s = segments[ix]; s; s = s->next_segment()) {
      ...
      if (ret) {
        if (s != segments[ix]) segments[ix] = s->move_to_head(segments[ix]);
        return ret;
      }
    }
...
class shared_allocator {
 ...
 public:
  ...
  // library containers
  template <typename T>
  operator TR::typed_allocator<T, shared_allocator>() {
