...
CS2_ARTEMP inline AElementType &CS2_BASEARDECL::ElementAt(
...
CS2_ARTEMP inline AElementType &CS2_BASEARDECL::ElementAt(size_t elementIndex) {
  if (elementIndex >= NumberOfElements()) GrowTo(elementIndex + 1);
...
CS2_ARTEMP inline AElementType &CS2_BASEARDECL::operator[](
    size_t elementIndex) {
  return ElementAt(elementIndex);
}
...
CS2_ARTEMP inline CS2_BASEARDECL::BaseArrayOf(const Allocator &a)
  ...
  CS2Assert(sizeof(AElementType) <= SegmentSize(),
            ("Segment size %d must be larger than size of element %d.",
             (int)SegmentSize(), (int)sizeof(AElementType)));
...
CS2_ARTEMP inline CS2_BASEARDECL &CS2_BASEARDECL::operator=(
    const CS2_BASEARDECL &inputArray) {
...
CS2_ARTEMP inline void CS2_BASEARDECL::GrowTo(size_t newSize) {
  ...
  if (segmentMapIndex >= fMaxSegments) {
    if (fSegmentMap == NULL) {
      ...
      void *newSegmentMapAllocation = Allocator::reallocate(
          maxSegments * sizeof(DerivedElement *), fSegmentMap,
          fMaxSegments * sizeof(DerivedElement *));
  ...
  for (newSegmentIndex = fNumberOfSegments;
       newSegmentIndex < segmentMapIndex + 1; ++newSegmentIndex) {
  ...
  CS2Assert(
      ...
      ("Number of current segments %d is smaller than expected size of %d.",
       (int)fNumberOfSegments, (int)(newSegmentIndex + 1)));
}
...
class ArrayOf : public CS2_BASEARDECL {
 ...
 public:
  ArrayOf(size_t ignore, const Allocator &a = Allocator(),
  ...
  class Cursor {
   public:
    Cursor(const ArrayOf<AElementType, Allocator, segmentBits, Initializer> &a)
        : fArray(a), fSegmentIndex(0), fElementIndex(0), fSegment(NULL) {}
    ...
    void SetTo(size_t index) {
      ...
      if (fSegmentIndex < fNumSegments) {
        ...
        if (fNumElements > fArray.ElementsPerSegment()) {
          ...
          if (fElementIndex >= fNumElements) fSegmentIndex += 1;
        }
      } else
        fNumElements = 0;
    }
    ...
    typename ArrayOf<AElementType, Allocator, segmentBits,
                     Initializer>::DerivedElement *
...
class StaticArrayOf
 ...
 public:
  StaticArrayOf(size_t size, const Allocator &a = Allocator(),
                const Initializer i = Initializer())
  ...
  AElementType &operator[](size_t elementIndex) const {
    CS2Assert(elementIndex < CS2_ARDECL::NumberOfElements(),
              ("Index %lu does not exist (>%lu)", elementIndex,
