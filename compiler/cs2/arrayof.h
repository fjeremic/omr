...
template<>
struct is_pod<uint32_t>
{
  operator bool() { return true; }
};
template<>
struct is_pod<int32_t>
{
  operator bool() { return true; }
};
template<>
struct is_pod<uint16_t>
{
  operator bool() { return true; }
};
template<>
struct is_pod<int16_t>
{
  operator bool() { return true; }
};
template<>
...
#define CS2_ARTEMP                                                             \
  template<class AElementType, class Allocator, size_t segmentBits>
...
/// ArrayOf
///
/// \brief Extensible array data structure.  This data structure will
/// automatically initialize array elements if a default constructor exists for
/// the element type.  In addition, storage will be grown preserving existing
/// array values if an index outside the existing range is used.
///
...
public:
  ...
  template<class ostr>
  friend ostr& operator<<(ostr& out, const CS2_BASEARDECL& table)
  {
...
CS2_ARTEMP inline size_t
CS2_BASEARDECL::ElementsPerSegment() const
{
  return size_t(1) << segmentBits;
}

CS2_ARTEMP inline size_t
CS2_BASEARDECL::SegmentSize() const
{
  return ElementsPerSegment() * sizeof(AElementType);
}

CS2_ARTEMP inline size_t
CS2_BASEARDECL::SegmentMask() const
{
  return ~((~size_t(0)) << segmentBits);
}

CS2_ARTEMP inline AElementType&
CS2_BASEARDECL::ElementAt(size_t elementIndex) const
{
  return DerivedElementAt(elementIndex)->Element();
}

CS2_ARTEMP inline typename CS2_BASEARDECL::DerivedElement*
CS2_BASEARDECL::Segment(size_t elementIndex) const
{
...
CS2_ARTEMP inline AElementType&
CS2_BASEARDECL::ElementAt(size_t elementIndex)
{
...
CS2_ARTEMP inline const AElementType& CS2_BASEARDECL::operator[](
...
CS2_ARTEMP inline size_t
CS2_BASEARDECL::NumberOfElements() const
{
  ...
  : Allocator(a)
{
  CS2Assert(sizeof(AElementType) <= SegmentSize(),
            ("Segment size %d must be larger than size of element %d.",
             (int)SegmentSize(),
             (int)sizeof(AElementType)));
...
CS2_ARTEMP inline CS2_BASEARDECL::BaseArrayOf(const CS2_BASEARDECL& inputArray,
                                              const Allocator& a)
  : Allocator(a)
  ...
  , fNumberOfSegments(inputArray.fNumberOfSegments)
{
  ...
  if (fNumberOfSegments == 0) {
    ...
    fSegmentMap = (DerivedElement**)Allocator::allocate(
      fMaxSegments * sizeof(DerivedElement*));

    for (segmentIndex = 0; segmentIndex < fNumberOfSegments; ++segmentIndex) {
      fSegmentMap[segmentIndex] =
        (DerivedElement*)Allocator::allocate(SegmentSize());
    }
  }
}

CS2_ARTEMP inline CS2_BASEARDECL&
CS2_BASEARDECL::operator=(const CS2_BASEARDECL& inputArray)
{
  ...
  if (fMaxSegments < inputArray.fMaxSegments) {
    ...
    DerivedElement** newSegmentMap = (DerivedElement**)Allocator::allocate(
      inputArray.fMaxSegments * sizeof(DerivedElement*));
  ...
  {
    firstSegment = (fNumberOfSegments < inputArray.fNumberOfSegments)
                     ? fNumberOfSegments
                     : inputArray.fNumberOfSegments;
    ...
    for (segmentIndex = 0; segmentIndex < firstSegment; ++segmentIndex) {
      {
        ...
        {
          for (elementIndex = 0; elementIndex < ElementsPerSegment();
               ++elementIndex) {
  ...
  for (segmentIndex = firstSegment; segmentIndex < fNumberOfSegments;
       ++segmentIndex) {
    fSegmentMap[segmentIndex] =
      (DerivedElement*)Allocator::allocate(SegmentSize());
  }
...
CS2_ARTEMP inline void
CS2_BASEARDECL::GrowTo(size_t newSize)
{
  ...
  if (segmentMapIndex >= fMaxSegments) {
    if (fSegmentMap == NULL) {
      ...
      fSegmentMap = (DerivedElement**)Allocator::allocate(
        updatedMaxSegments * sizeof(DerivedElement*));
      fMaxSegments = updatedMaxSegments;
    } else {
      size_t maxSegments = segmentMapIndex + (fMaxSegments >> 1) + 1;
      void* newSegmentMapAllocation =
        Allocator::reallocate(maxSegments * sizeof(DerivedElement*),
                              fSegmentMap,
                              fMaxSegments * sizeof(DerivedElement*));
  ...
  for (newSegmentIndex = fNumberOfSegments;
       ...
       ++newSegmentIndex) {
    fSegmentMap[newSegmentIndex] =
      (DerivedElement*)Allocator::allocate(SegmentSize());
    fNumberOfSegments = newSegmentIndex + 1;
  }

  CS2Assert(
    segmentMapIndex + 1 == fNumberOfSegments,
    ("Number of current segments %d is smaller than expected size of %d.",
     (int)fNumberOfSegments,
     (int)(newSegmentIndex + 1)));
}

CS2_ARTEMP inline void
CS2_BASEARDECL::ShrinkTo(size_t newSize)
{
  ...
  for (segmentIndex = firstDeadSegment; segmentIndex < fNumberOfSegments;
       ++segmentIndex) {
    Allocator::deallocate(fSegmentMap[segmentIndex], SegmentSize());
  }
...
CS2_ARTEMP inline unsigned long
CS2_BASEARDECL::DynamicMemoryUsage() const
{
...
CS2_ARTEMP inline unsigned long
CS2_BASEARDECL::MemoryUsage() const
{
  return DynamicMemoryUsage() + sizeof(CS2_BASEARDECL);
}

template<class AElementType,
         class Allocator,
         size_t segmentBits = 8,
         class Initializer = AElementType>
...
public:
  ...
  /// \brief Copy constructor.
  ArrayOf(ArrayOf<AElementType, Allocator, segmentBits>& inputArray,
          const Allocator& a = Allocator())
    : CS2_BASEARDECL(a)
    ...
    , fNumInitialized(0)
  {
    ...
    for (c.SetToFirst(), ic.SetToFirst(); c.Valid();
         c.SetToNext(), ic.SetToNext())
  ...
  bool operator==(
    const ArrayOf<AElementType, Allocator, segmentBits>& inputArray) const
  {
  ...
  void GrowTo(size_t newSize)
  {
    ...
    size_t elementIndex;
    for (elementIndex = fNumInitialized; elementIndex < newSize;
         elementIndex++) {
      typename CS2_BASEARDECL::DerivedElement* currentElement =
        CS2_BASEARDECL::DerivedElementAt(elementIndex);
      new (currentElement)
        typename CS2_BASEARDECL::DerivedElement(fInitializer);
    }
  ...
  void Sort(size_t from, size_t to)
  {
    ...
    if (to == from + 2) {
      if (CS2_BASEARDECL::ElementAt(from + 1) < CS2_BASEARDECL::ElementAt(from))
        Swap(CS2_BASEARDECL::ElementAt(from),
             CS2_BASEARDECL::ElementAt(from + 1));
      return;
    }
  ...
  template<class ostr>
  friend ostr& operator<<(ostr& out, const CS2_ARDECL& table)
  {
    Cursor c(table);
  ...
  public:
    ...
    void SetTo(size_t index)
    {
      ...
      if (fSegmentIndex < fNumSegments) {
        ...
        fNumElements = fArray.NumberOfElements() -
                       (fSegmentIndex)*fArray.ElementsPerSegment();
    ...
    typename ArrayOf<AElementType, Allocator, segmentBits, Initializer>::
      DerivedElement*
      DerivedElement() const
    {
  ...
  public:
    BackwardsCursor(
      const ArrayOf<AElementType, Allocator, segmentBits, Initializer>& a)
      : fArray(a)
      , fSegmentIndex(0)
      , fElementIndex(0)
      , fSegment(NULL)
  ...
  public:
    ...
    {
      return this == &other ||
             (fArray == other.fArray && fCursor == other.fCursor);
    }
...
template<class AElementType,
         ...
         class Initializer = AElementType>
class StaticArrayOf
  : public ArrayOf<AElementType, Allocator, segmentBits, Initializer>
{
public:
  ...
  AElementType& operator[](size_t elementIndex) const
  {
    CS2Assert(elementIndex < CS2_ARDECL::NumberOfElements(),
              ("Index %lu does not exist (>%lu)",
               elementIndex,
               CS2_ARDECL::NumberOfElements()));
