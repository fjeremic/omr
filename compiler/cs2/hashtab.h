...
inline HashValue Hash_FNV(const unsigned char *s, uint32_t len,
...
public:
  ...
  HashTable(uint32_t ignored, const Allocator &a = Allocator())
      : Allocator(a), fTable(NULL), fMask(0), fNextFree(0), fTableSize(0),
        fHighestIndex(0) {}
  ...
  bool Add(const AKeyType &, const ADataType &, HashIndex &,
           HashValue hashValue = 0, bool noLocate = false);
...
CS2_HT_TEMP inline CS2_HTE_DECL::HashTableEntry(
    ...
    : fKey(entry.fKey), fData(entry.fData), fHashCode(entry.fHashCode),
      fChain(entry.fChain) {}
...
CS2_HT_TEMP inline typename CS2_HTE_DECL &CS2_HTE_DECL::
operator=(const typename CS2_HTE_DECL &entry) {
  fKey = entry.fKey;
  fData = entry.fData;
...
CS2_HT_TEMP inline const ADataType &CS2_HT_DECL::
operator[](HashIndex index) const {
  return DataAt(index);
}
...
inline CS2_HT_DECL::HashTable(const CS2_HT_DECL &table)
    : Allocator(table),
      fTable(table.fTableSize > 0
                 ...
                 : NULL),
      fTableSize(table.fTableSize), fMask(table.fMask),
      fNextFree(table.fNextFree), fHighestIndex(table.fHighestIndex) {
...
CS2_HT_TEMP bool
    ...
    inline CS2_HT_DECL::Add(const AKeyType &key, const ADataType &data,
                            HashIndex &hashIndex, HashValue hashValue,
                            bool noLocate) {
