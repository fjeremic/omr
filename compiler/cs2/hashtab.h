...
inline HashValue
Hash_FNV(const unsigned char* s,
         uint32_t len,
...
CS2_HT_TEMP inline void*
CS2_HTE_DECL::operator new(size_t, void* ptr)
{
  return ptr;
}
...
CS2_HT_TEMP inline typename CS2_HTE_DECL&
CS2_HTE_DECL::operator=(const typename CS2_HTE_DECL& entry)
{
  fKey = entry.fKey;
  fData = entry.fData;
  fHashCode = entry.fHashCode;
...
CS2_HT_TEMP inline const AKeyType&
CS2_HTE_DECL::Key() const
{
  return fKey;
}
...
CS2_HT_TEMP inline void
CS2_HTE_DECL::SetKey(const AKeyType& key)
{
  fKey = key;
}
...
CS2_HT_TEMP inline const ADataType&
CS2_HTE_DECL::Data() const
{
  return fData;
}

CS2_HT_TEMP inline ADataType&
CS2_HTE_DECL::Data()
{
  return fData;
}
...
CS2_HT_TEMP inline void
CS2_HTE_DECL::SetData(const ADataType& data)
{
  fData = data;
}
...
CS2_HT_TEMP inline HashValue
CS2_HTE_DECL::HashCode() const
{
  return fHashCode;
}
...
CS2_HT_TEMP inline void
CS2_HTE_DECL::SetHashCode(HashValue value)
{
  fHashCode = value;
}
...
CS2_HT_TEMP inline bool
CS2_HTE_DECL::Valid() const
{
  return (fHashCode != 0);
}
...
CS2_HT_TEMP inline HashIndex
CS2_HTE_DECL::CollisionChain() const
{
  return fChain;
}
...
CS2_HT_TEMP inline void
CS2_HTE_DECL::SetCollisionChain(HashIndex chainIndex)
{
  fChain = chainIndex;
}
...
CS2_HT_TEMP inline const ADataType&
CS2_HT_DECL::DataAt(HashIndex index) const
{
...
CS2_HT_TEMP inline ADataType&
CS2_HT_DECL::DataAt(HashIndex index)
{
  CS2Assert(index < fTableSize, ("Hash index %d out of range", index));
  CS2Assert(fTable[index].Valid(), ("Invalid hash table entry"));
...
CS2_HT_TEMP inline void
CS2_HT_DECL::SetDataAt(HashIndex index, const ADataType& data)
{
  CS2Assert(index < fTableSize, ("Hash index %d out of range", index));
  CS2Assert(fTable[index].Valid(), ("Invalid hash table entry"));
...
CS2_HT_TEMP inline const AKeyType&
CS2_HT_DECL::KeyAt(HashIndex index) const
{
  CS2Assert(index < fTableSize, ("Hash index %d out of range", index));
  CS2Assert(fTable[index].Valid(), ("Invalid hash table entry"));
...
CS2_HT_TEMP inline bool
CS2_HT_DECL::Locate(const AKeyType& key) const
{
  HashIndex tempIdx = 0;
  HashValue tempVal = 0;
...
CS2_HT_TEMP inline bool
CS2_HT_DECL::Locate(const AKeyType& key, HashIndex& index) const
{
  HashValue tempVal = 0;
...
CS2_HT_TEMP inline bool
CS2_HT_DECL::Add(const AKeyType& key, const ADataType& data)
{
  HashIndex hashIndex;
...
CS2_HT_TEMP inline const ADataType&
CS2_HT_DECL::Get(const AKeyType& key) const
{
  HashIndex hashIndex;
...
CS2_HT_TEMP inline HashIndex
CS2_HT_DECL::HighestIndex() const
{
...
inline CS2_HT_DECL::HashTable(const CS2_HT_DECL& table)
  : Allocator(table)
  , fTable(table.fTableSize > 0 ? (HashTableEntry*)Allocator::allocate(
...
CS2_HT_TEMP
inline CS2_HT_DECL&
CS2_HT_DECL::operator=(const CS2_HT_DECL& table)
{
                                                   ...
                                                   HashIndex openAreaSize)
{
  ...
  HashTableEntry* newTable =
    (HashTableEntry*)Allocator::allocate(newTableSize * sizeof(HashTableEntry));
  if (newTable == NULL)
    SystemResourceError::Memory();
...
CS2_HT_TEMP void inline CS2_HT_DECL::Remove(HashIndex hashIndex)
{
  ...
  if (hashIndex > (fMask + 1)) {
    ...
    if (collisionChain) {
      ...
      memcpy(
        fTable + hashIndex, fTable + firstCollision, sizeof(HashTableEntry));
