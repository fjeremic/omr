...
CS2_HT_TEMP void inline CS2_HT_DECL::init(HashIndex numElements) {
  ...
  if (newTable == NULL) SystemResourceError::Memory();
...
CS2_HT_TEMP void inline CS2_HT_DECL::GrowAndRehash(HashIndex oldSize,
  ...
  for (oldIndex = 0; oldIndex < oldSize; ++oldIndex) {
    if (oldBase[oldIndex].Valid()) {  // this is a valid entry
      ...
      if (hashIndex > fHighestIndex) fHighestIndex = hashIndex;
  ...
  if (oldBase) Allocator::deallocate(oldBase, oldSize * sizeof(HashTableEntry));
}
...
CS2_HT_TEMP void inline CS2_HT_DECL::MakeEmpty() {
  ...
  for (hashIndex = 0; hashIndex < fTableSize; ++hashIndex) {
    if (fTable[hashIndex].Valid()) fTable[hashIndex].~HashTableEntry();
  }
