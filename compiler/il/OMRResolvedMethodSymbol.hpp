...
class OMR_EXTENSIBLE ResolvedMethodSymbol : public TR::MethodSymbol {
 ...
 public:
  ...
  static TR::ResolvedMethodSymbol *create(AllocatorType, TR_ResolvedMethod *,
                                          TR::Compilation *);
  ...
  /*
   * \brief Get known object index of a parameter if there is any known object
   * information available
   *
   * \parm ordinal
   *     the ordinal of a parameter including the receiver if the method is a
   * virtual method
   */
  ...
  bool genIL(TR_FrontEnd *fe, TR::Compilation *comp,
             TR::SymbolReferenceTable *symRefTab,
             TR::IlGenRequest &customRequest);
  void cleanupUnreachableOSRBlocks(int32_t inlinedSiteIndex,
                                   TR::Compilation *comp);
  void insertRematableStoresFromCallSites(TR::Compilation *comp,
                                          int32_t siteIndex,
  ...
  void insertStoresForDeadStackSlots(TR::Compilation *comp,
                                     TR_ByteCodeInfo &byteCodeInfo,
                                     TR::TreeTop *insertTree,
                                     bool keepStashedArgsLive = true);
  bool sharesStackSlots(TR::Compilation *comp);
  void resetLiveLocalIndices();

  bool genPartialIL(TR_FrontEnd *, TR::Compilation *,
                    TR::SymbolReferenceTable *,
                    bool forceClassLookahead = false,
                    TR_InlineBlocks *blocksToInline = 0);

  void genOSRHelperCall(int32_t inlinedSiteIndex,
                        TR::SymbolReferenceTable *symRefTab,
                        TR::CFG *cfg = NULL);
  void genAndAttachOSRCodeBlocks(int32_t inlinedSiteIndex);
  int matchInduceOSRCall(TR::TreeTop *insertionPoint, int16_t callerIndex,
                         int16_t byteCodeIndex, const char *childPath);

  TR::TreeTop *genInduceOSRCallNode(TR::TreeTop *insertionPoint,
                                    int32_t numChildren, bool copyChildren,
                                    bool shouldSplitBlock = true,
                                    TR::CFG *cfg = NULL);
  TR::TreeTop *genInduceOSRCall(TR::TreeTop *insertionPoint,
                                int32_t inlinedSiteIndex, int32_t numChildren,
                                bool copyChildren, bool shouldSplitBlock,
                                TR::CFG *cfg = NULL);
  TR::TreeTop *genInduceOSRCall(TR::TreeTop *insertionPoint,
                                int32_t inlinedSiteIndex,
                                TR_OSRMethodData *osrMethodData,
                                int32_t numChildren, bool copyChildren,
                                bool shouldSplitBlock, TR::CFG *cfg = NULL);
  ...
  bool induceOSRAfter(TR::TreeTop *insertionPoint, TR_ByteCodeInfo induceBCI,
                      TR::TreeTop *branch, bool extendRemainder, int32_t offset,
                      TR::TreeTop **lastTreeTop = NULL);
  bool induceOSRAfterAndRecompile(TR::TreeTop *insertionPoint,
                                  TR_ByteCodeInfo induceBCI,
                                  TR::TreeTop *branch, bool extendRemainder,
                                  int32_t offset, TR::TreeTop **lastTreeTop);
  TR::TreeTop *induceImmediateOSRWithoutChecksBefore(
  ...
  bool cannotAttemptOSRAt(TR_ByteCodeInfo &bci, TR::Block *blockToOSRAt,
                          TR::Compilation *comp);
  bool cannotAttemptOSRDuring(int32_t callSite, TR::Compilation *comp,
                              bool runCleanup = true);
  bool supportsInduceOSR(TR_ByteCodeInfo &bci, TR::Block *blockToOSRAt,
                         TR::Compilation *comp, bool runCleanup = true);
 ...
 private:
  ...
  // todo: implement a way to avoid the memory usage for when genIL is not
  // called
  //
  TR::CFG *_flowGraph;
  ...
  TR::list<std::pair<int32_t, std::pair<int32_t, int32_t> > >
      _bytecodeProfilingOffsets;
  ...
  TR::TreeTop *induceOSRAfterImpl(TR::TreeTop *insertionPoint,
                                  TR_ByteCodeInfo induceBCI,
                                  TR::TreeTop *branch, bool extendRemainder,
                                  int32_t offset,
                                  TR::TreeTop **lastTreeTop = NULL);
 ...
 public:
   ...
   */
  template <typename AllocatorType>
  static TR::ResolvedMethodSymbol *createJittedMethodSymbol(AllocatorType m,
                                                            TR_ResolvedMethod *,
                                                            TR::Compilation *);
