...
FirstOMROp,
  ...
  ZEROCHK, // Zero-check an int.  Symref indicates call to perform when first child is zero.  Other children are
  ...
  compressedRefs, // no-op anchor providing optimizable subexpressions used for compression/decompression.  First child
                  // is address load/store, second child is heap base displacement
  ...
  instanceof, // instanceof - symref is the class object, cp index is in the
            ...
            variableNewArray, // new array - type not known at compile time, type must be a j9class, do not use type
                              // enums
            ...
            lusubh, // subtract 2 unsigned long integers (the high parts of prior lusub) as high part of 128bit
                    // subtraction.
            ...
            lookup, // lookupswitch (child1 is selector expression, child2 the default destination, subsequent children
                    // are case nodes
            trtLookup, // special lookupswitch (child1 must be trt, child2 the default destination, subsequent children
                       // are case nodes) The internal control flow is similar to lookup, but each CASE represents a
            ...
            table, // tableswitch (child1 is the selector, child2 the default destination, subsequent children are the
                   // branch targets (the last child may be a branch table address, use getCaseIndexUpperBound() when
                   // iterating over branch targets)
            exceptionRangeFence, // (J9) SymbolReference is the aliasing effect, initializer is where the code address
                                 // gets put when binary is generated used for delimiting function, try blocks, catch
                                 // clauses, finally clauses, etc.
            dbgFence, // used to delimit code (stmts) for debug info.  Has no symbol reference.
            NULLCHK, // Null check a pointer.  child 1 is indirect reference. Symbolref indicates failure
                     // action/destination
            ResolveCHK, // Resolve check a static, field or method. child 1 is reference to be resolved. Symbolref
                        // indicates failure action/destination
            ResolveAndNULLCHK, // Resolve check a static, field or method and Null check the underlying pointer.  child
                               // 1 is reference to be resolved. Symbolref indicates failure action/destination
            DIVCHK, // Divide by zero check. child 1 is the divide. Symbolref indicates failure action/destination
            OverflowCHK, // Overflow check. child 1 is the operation node(add, mul, sub). Child 2 and child 3 are the
                         // operands of the operation of the operation. Symbolref indicates failure action/destination
            UnsignedOverflowCHK, // UnsignedOverflow check. child 1 is the operation node(add, mul, sub). Child 2 and
                                 // child 3 are the operands of the operation of the operation. Symbolref indicates
                                 // failure action/destination
            BNDCHK, // Array bounds check, checks that child 1 > child 2 >= 0 (child 1 is bound, 2 is index). Symbolref
                    // indicates failure action/destination
            ArrayCopyBNDCHK, // Array copy bounds check, checks that child 1 >= child 2. Symbolref indicates failure
                             // action/destination
            BNDCHKwithSpineCHK, // Array bounds check and spine check
            SpineCHK, // Check if the base array has a spine
            ArrayStoreCHK, // Array store check. child 1 is object, 2 is array. Symbolref indicates failure
                           // action/destination
            ArrayCHK, // Array compatibility check. child 1 is object1, 2 is object2. Symbolref indicates failure
                      // action/destination
            ...
            bucmp, // Currently only valid for zEmulator. Based on the ordering of the two children set the return
                   // value:
            ...
            icmpset, // icmpset(pointer,c,r): compare *pointer with c, if it matches, replace with r.  Returns 0 on
                     // match, 1 otherwise
            lcmpset, // the operation is done atomically - return type is int for both [il]cmpset
            bztestnset, // bztestnset(pointer,c): atomically sets *pointer to c and returns the original value of *p
                        // (represents Test And Set on Z)
