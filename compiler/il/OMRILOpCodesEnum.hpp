...
FirstOMROp,
    ...
    // Read barrier is used to represent loads with side effects like check for
    // GC, debugging etc. It is the same as the corresponding load except that it
    // needs to be anchored under a treetop. The children and symbol of a read
    // barrier are the same as the corresponding load.
    ...
    astorei,  // store indirect address                (child1 a dest, child2 a
              // value)
    ...
    // be the same as the first child (when the offset is on the symbol
    // reference. But for array references, children 1 and 3 will be quite
    // different although child 1's subtree will contain a reference to child
    // 3's subtree
    ...
    ishr,        // shift integer right arithmetically               (child1 >>
                 // child2)
    lshr,        // shift long integer right arithmetically          (child1 >>
                 // child2)
    bshr,        // shift byte right arithmetically                  (child1 >>
                 // child2)
    sshr,        // shift short integer arithmetically               (child1 >>
                 // child2)
    iushr,       // shift integer right logically                   (child1 >>
                 // child2)
    lushr,       // shift long integer right logically              (child1 >>
                 // child2)
    bushr,       // shift byte right logically                      (child1 >>
                 // child2)
    sushr,       // shift short integer right logically             (child1 >>
                 // child2)
    ...
    lcmp,   // long compare (1 if child1 > child2, 0 if child1 == child2, -1 if
            // child1 < child2)
    fcmpl,  // float compare l (1 if child1 > child2, 0 if child1 == child2, -1
            // if child1 < child2 or unordered)
    fcmpg,  // float compare g (1 if child1 > child2 or unordered, 0 if child1
            // == child2, -1 if child1 < child2)
    dcmpl,  // double compare l (1 if child1 > child2, 0 if child1 == child2, -1
            // if child1 < child2 or unordered)
    dcmpg,  // double compare g (1 if child1 > child2 or unordered, 0 if child1
            // == child2, -1 if child1 < child2)
    ...
    iffcmpgeu,  // float compare and branch if greater than or equal or
                // unordered
    ...
    ifdcmpgeu,  // double compare and branch if greater than or equal or
                // unordered
    ...
    loadaddr,   // load address of non-heap storage item (Auto, Parm, Static or
                // Method)
    ZEROCHK,  // Zero-check an int.  Symref indicates call to perform when first
              // child is zero.  Other children are arguments to the call.
    callIf,   // Call symref if first child evaluates to true.  Other childrem
    ...
    iternary,   // Ternary Operator:  Based on the result of the first child,
                // take the value of the
    lternary,   //   second (first child evaluates to true) or third(first child
    ...
    MethodEnterHook,  // called after a frame is built, temps initialized, and
                      // monitor acquired (if necessary)
    MethodExitHook,   // called immediately before returning, frame not yet
                      // collapsed, monitor released (if necessary)
    PassThrough,      // Dummy node that represents its single child.
    compressedRefs,   // no-op anchor providing optimizable subexpressions used
    ...
    v2v,    // vector to vector conversion. preserves bit pattern (noop), only
            // changes datatype
    ...
    checkcast,  // checkcast
    checkcastAndNULLCHK,  // checkcast and NULL check the underlying object
                          // reference
    ...
    variableNewArray,   // new array - type not known at compile time, type must
                        // be a j9class, do not use type enums
    ...
    iucalli,  // indirect call returning unsigned integer (child1 is addr of
              // function)
    lcalli,   // indirect call returning long integer (child1 is addr of
              // function)
    lucalli,  // indirect call returning unsigned long integer (child1 is addr
              // of function)
    ...
    luaddh,   // add 2 unsigned long integers (the high parts of prior luadd) as
    ...
    aiuadd,   // add unsigned integer to address with address result (child1 a,
              // child2 i)
    aladd,  // add long integer to address with address result (child1 a, child2
            // i) (64-bit only)
    aluadd,  // add unsigned long integer to address with address result (child1
             // a, child2 i) (64-bit only)
    lusubh,  // subtract 2 unsigned long integers (the high parts of prior
    ...
    iumulh,  // multiply 2 unsigned integers, and return the high word of the
             // product
    lmulh,  // multiply 2 long integers, and return the high word of the product
    lumulh,   // multiply 2 unsigned long integers, and return the high word of
              // the product
    ...
    lookup,   // lookupswitch (child1 is selector expression, child2 the default
              // destination, subsequent children are case nodes
    trtLookup,  // special lookupswitch (child1 must be trt, child2 the default
    ...
    Case,   // case nodes that are children of TR_switch.  Uses the
            // branchdestination and the int const field
    table,  // tableswitch (child1 is the selector, child2 the default
            // destination, subsequent children are the branch targets (the last
            // child may be a branch table address, use getCaseIndexUpperBound()
            // when iterating over branch targets)
    exceptionRangeFence,  // (J9) SymbolReference is the aliasing effect,
                          // initializer is where the code address gets put when
                          // binary is generated used for delimiting function,
                          // try blocks, catch clauses, finally clauses, etc.
    dbgFence,  // used to delimit code (stmts) for debug info.  Has no symbol
               // reference.
    NULLCHK,  // Null check a pointer.  child 1 is indirect reference. Symbolref
              // indicates failure action/destination
    ResolveCHK,         // Resolve check a static, field or method. child 1 is
                        // reference to be resolved. Symbolref indicates failure
                        // action/destination
    ResolveAndNULLCHK,  // Resolve check a static, field or method and Null
                        // check the underlying pointer.  child 1 is reference
                        // to be resolved. Symbolref indicates failure
                        // action/destination
    DIVCHK,  // Divide by zero check. child 1 is the divide. Symbolref indicates
             // failure action/destination
    OverflowCHK,  // Overflow check. child 1 is the operation node(add, mul,
                  // sub). Child 2 and child 3 are the operands of the operation
                  // of the operation. Symbolref indicates failure
                  // action/destination
    UnsignedOverflowCHK,  // UnsignedOverflow check. child 1 is the operation
                          // node(add, mul, sub). Child 2 and child 3 are the
                          // operands of the operation of the operation.
                          // Symbolref indicates failure action/destination
    BNDCHK,  // Array bounds check, checks that child 1 > child 2 >= 0 (child 1
             // is bound, 2 is index). Symbolref indicates failure
             // action/destination
    ArrayCopyBNDCHK,  // Array copy bounds check, checks that child 1 >=
                      // child 2. Symbolref indicates failure action/destination
    BNDCHKwithSpineCHK,  // Array bounds check and spine check
    SpineCHK,            // Check if the base array has a spine
    ArrayStoreCHK,       // Array store check. child 1 is object, 2 is array.
                         // Symbolref indicates failure action/destination
    ArrayCHK,   // Array compatibility check. child 1 is object1, 2 is object2.
                // Symbolref indicates failure action/destination
    ...
    arraytranslate,  // Inline code for translation of part of an array to
                     // another form via lookup
    arraytranslateAndTest,  // Inline code for scanning of part of an array for
    ...
    arraycmpWithPad,  // memory comparison when src1 length != src2 length and
                      // padding is needed
    allocationFence,  // Internal fence guarding escape of newObject & final
                      // fields - eliminatable
    loadFence,        // JEP171: prohibits loadLoad and loadStore reordering (on
                      // globals)
    storeFence,  // JEP171: prohibits loadStore and storeStore reordering (on
                 // globals)
    fullFence,   // JEP171: prohibits loadLoad, loadStore, storeLoad, and
    ...
    bucmp,  // Currently only valid for zEmulator. Based on the ordering of the
            // two children set the return value:
    ...
    iuaddc,  // Currently only valid for zEmulator.  Add two unsigned ints with
             // carry
    luaddc,  // Add two longs with carry
    iusubb,  // Subtract two ints with borrow
    lusubb,  // Subtract two longs with borrow

    icmpset,  // icmpset(pointer,c,r): compare *pointer with c, if it matches,
              // replace with r.  Returns 0 on match, 1 otherwise
    lcmpset,  // the operation is done atomically - return type is int for both
              // [il]cmpset
    bztestnset,  // bztestnset(pointer,c): atomically sets *pointer to c and
                 // returns the original value of *p (represents Test And Set on
                 // Z)

    // the atomic ops.. atomically update the symref.  first child is address,
    // second child is the RHS interestingly, these ops act like loads and
    ...
    ibranch,  // generic indirct branch --> first child is a constant indicating
              // the mask
    ...
    trtSimple,  // same as TRT but ignoring the returned source byte address and
                // table entry value

    ihbit, ilbit, inolz, inotz, ipopcnt,

    lhbit, llbit, lnolz, lnotz, lpopcnt,

    ibyteswap,  // swap bytes in an integer

    bbitpermute, sbitpermute, ibitpermute, lbitpermute,
