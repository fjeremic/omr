...
FirstOMROp,
  ...
  lcmp,    // long compare (1 if child1 > child2, 0 if child1 == child2, -1 if
           // child1 < child2)
  fcmpl, // float compare l (1 if child1 > child2, 0 if child1 == child2, -1 if
         // child1 < child2 or unordered)
  fcmpg, // float compare g (1 if child1 > child2 or unordered, 0 if child1 ==
         // child2, -1 if child1 < child2)
  dcmpl, // double compare l (1 if child1 > child2, 0 if child1 == child2, -1 if
         // child1 < child2 or unordered)
  ...
  compressedRefs,  // no-op anchor providing optimizable subexpressions used for
                   // compression/decompression.  First child is address
  ...
  monexitfence, // denotes the end of a monitored region solely for live monitor
                // meta data
  ...
  variableNewArray,  // new array - type not known at compile time, type must be
  ...
  aiuadd,  // add unsigned integer to address with address result (child1 a,
           // child2 i)
  aladd, // add long integer to address with address result (child1 a, child2 i)
         // (64-bit only)
  aluadd, // add unsigned long integer to address with address result (child1 a,
  ...
  lumulh, // multiply 2 unsigned long integers, and return the high word of the
          // product
  ...
  table, // tableswitch (child1 is the selector, child2 the default destination,
  ...
  OverflowCHK, // Overflow check. child 1 is the operation node(add, mul, sub).
  ...
  UnsignedOverflowCHK, // UnsignedOverflow check. child 1 is the operation
                       ...
                       // indicates failure action/destination
  BNDCHK, // Array bounds check, checks that child 1 > child 2 >= 0 (child 1 is
  ...
  ArrayStoreCHK, // Array store check. child 1 is object, 2 is array. Symbolref
                 // indicates failure action/destination
  ...
  arraytranslate, // Inline code for translation of part of an array to another
                  // form via lookup
  ...
  arraycmpWithPad, // memory comparison when src1 length != src2 length and
                   // padding is needed
  allocationFence, // Internal fence guarding escape of newObject & final fields
                   // - eliminatable
  ...
  storeFence, // JEP171: prohibits loadStore and storeStore reordering (on
              // globals)
  fullFence, // JEP171: prohibits loadLoad, loadStore, storeLoad, and storeStore
  ...
  bucmp, // Currently only valid for zEmulator. Based on the ordering of the two
