...
uint16_t OMR::Node::addChildrenAndSymRef(
    uint16_t childIndex, TR::Node *child,
...
bool OMR::Node::isSingleRef() { return self()->getReferenceCount() == 1; }
...
inline void OMR::Node::ChildIterator::stepForward() {
  if (_index < _node->getNumChildren()) ++_index;
}
...
uint16_t OMR::Node::getUseDefIndex() { return _unionA._useDefIndex; }
...
uint64_t OMR::Node::setAddress(uint64_t a) {
  ...
  if (TR::Compiler->target.is32Bit()) a = a & 0x00000000ffffffff;
...
TR::DataType OMR::Node::getDataType() {
  if (_opCode.hasNoDataType()) return self()->computeDataType();
