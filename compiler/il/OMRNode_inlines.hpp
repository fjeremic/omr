...
TR::Node* OMR::Node::recreateWithSymRefWithoutProperties(
  ...
  uint16_t numChildArgsActual =
      node->addChildrenAndSymRef(0, first, childrenAndSymRef...);
  TR_ASSERT(numChildArgs == numChildArgsActual,
            "numChildArgs = %d does not match actual number of child args = %d",
...
TR::Node* OMR::Node::recreateWithoutSymRef(TR::Node* originalNode,
                                           ...
                                           TR::Node* first,
                                           Children... children) {
  return recreateWithSymRefWithoutProperties(originalNode, op, numChildren,
                                             numChildArgs, first, children...);
}

template <class... ChildrenAndSymRefType>
TR::Node* OMR::Node::createWithSymRefInternal(
    TR::ILOpCodes op,
    uint16_t numChildren,
    uint16_t numChildArgs,
    TR::Node* first,
    ChildrenAndSymRefType... childrenAndSymRef) {
  return recreateWithSymRefWithoutProperties(0, op, numChildren, numChildArgs,
                                             first, childrenAndSymRef...);
}
template <class... Children>
TR::Node* OMR::Node::createWithoutSymRef(TR::ILOpCodes op,
                                         ...
                                         TR::Node* first,
                                         Children... children) {
  return createWithSymRefInternal(op, numChildren, numChildArgs, first,
                                  children...);
}
...
inline TR::Node* OMR::Node::getChild(int32_t c) {
  if (!self()->hasNodeExtension()) {
    TR_ASSERT(c < NUM_DEFAULT_CHILDREN,
              "getChild(%d) called on node " POINTER_PRINTF_FORMAT
              " with %d children.",
...
inline TR::Node* OMR::Node::getFirstChild() {
  TR_ASSERT(self()->getNumChildren() >= 1,
            "getFirstChild() called on node " POINTER_PRINTF_FORMAT
            " with no children",
            self());
...
inline TR::Node* OMR::Node::getLastChild() {
  TR_ASSERT(self()->getNumChildren() >= 1,
            "getLastChild() called on node " POINTER_PRINTF_FORMAT
            " with no children",
            self());
...
inline TR::Node* OMR::Node::getSecondChild() {
  TR_ASSERT(self()->getNumChildren() >= 2,
            "getSecondChild() called on node " POINTER_PRINTF_FORMAT
...
inline TR::Node* OMR::Node::getThirdChild() {
  TR_ASSERT(self()->getNumChildren() >= 3,
            "getThirdChild() called on node " POINTER_PRINTF_FORMAT
...
vcount_t OMR::Node::getVisitCount() {
  return _visitCount;
}

vcount_t OMR::Node::setVisitCount(vcount_t vc) {
  return (_visitCount = vc);
}

vcount_t OMR::Node::incVisitCount() {
  ++_visitCount;
  TR_ASSERT(_visitCount > 0, "Assertion failure : %s (0x%p)",
            self()->getOpCode().getName(), this);
  return _visitCount;
}

rcount_t OMR::Node::getReferenceCount() {
  return _referenceCount;
}

rcount_t OMR::Node::setReferenceCount(rcount_t rc) {
  return (_referenceCount = rc);
}

rcount_t OMR::Node::incReferenceCount() {
  ++(_referenceCount);
  TR_ASSERT(_referenceCount > 0, "Assertion failure : %s (0x%p)",
            self()->getOpCode().getName(), this);
  return _referenceCount;
}

rcount_t OMR::Node::decReferenceCount() {
  TR_ASSERT(_referenceCount > 0 || self()->getOpCode().isTreeTop(),
            "Assertion failure : %s (0x%p)", self()->getOpCode().getName(),
            this);
  return --(_referenceCount);
}

scount_t OMR::Node::getLocalIndex() {
  return _localIndex;
}

scount_t OMR::Node::setLocalIndex(scount_t li) {
...
scount_t OMR::Node::getFutureUseCount() {
  return _localIndex;
}

scount_t OMR::Node::setFutureUseCount(scount_t li) {
...
scount_t OMR::Node::decFutureUseCount() {
  return --_localIndex;
}
...
int32_t OMR::Node::getOSRInductionOffset() {
  TR_ASSERT(self()->isPotentialOSRPointHelperCall(),
            "TR::Node::getOSRInductionOffset: used for a non "
            "potentialOSRPointHelper call node n%dn %p",
...
int32_t OMR::Node::setOSRInductionOffset(int32_t offset) {
  TR_ASSERT(self()->isPotentialOSRPointHelperCall(),
            "TR::Node::setOSRInductionOffset: used for a non "
            "potentialOSRPointHelper call node n%dn %p",
            self()->getGlobalIndex(), self());
  return (int32_t)(_unionBase._osrInductionOffset = (int64_t)offset);
}

int64_t OMR::Node::getConstValue() {
  return _unionBase._constValue;
}

uint64_t OMR::Node::getUnsignedConstValue() {
...
int32_t OMR::Node::getLongIntLow() {
  return (int32_t)self()->getLongInt();
}

int32_t OMR::Node::getLongIntHigh() {
  return (int32_t)(self()->getLongInt() >> 32);
}

uint64_t OMR::Node::getUnsignedLongInt() {
  return (uint64_t)_unionBase._constValue;
}
...
uint32_t OMR::Node::getUnsignedLongIntLow() {
  return (uint32_t)self()->getUnsignedLongInt();
}

uint32_t OMR::Node::getUnsignedLongIntHigh() {
  return (uint32_t)(self()->getUnsignedLongInt() >> 32);
}

int32_t OMR::Node::getInt() {
  TR_ASSERT(self()->getOpCodeValue() != TR::fconst,
            "TR::Node::getInt: used for an fconst node");
  return (int32_t)_unionBase._constValue;
}

int32_t OMR::Node::setInt(int32_t i) {
  TR_ASSERT(self()->getOpCodeValue() != TR::fconst,
            "TR::Node::setInt: used for an fconst node");
  self()->freeExtensionIfExists();
  return _unionBase._constValue = (int64_t)i;
}

uint32_t OMR::Node::getUnsignedInt() {
...
uint16_t OMR::Node::getUnsignedShortInt() {
...
uint8_t OMR::Node::getUnsignedByte() {
...
namespace OMR {
template <>
inline uint8_t Node::getConst<uint8_t>() {
  return self()->getUnsignedByte();
}
template <>
inline int8_t Node::getConst<int8_t>() {
  return self()->getByte();
}
template <>
inline uint16_t Node::getConst<uint16_t>() {
  return self()->getUnsignedShortInt();
}
template <>
inline int16_t Node::getConst<int16_t>() {
  return self()->getShortInt();
}
template <>
inline uint32_t Node::getConst<uint32_t>() {
  return self()->getUnsignedInt();
}
template <>
inline int32_t Node::getConst<int32_t>() {
  return self()->getInt();
}
template <>
inline uint64_t Node::getConst<uint64_t>() {
  return self()->getUnsignedLongInt();
}
template <>
inline int64_t Node::getConst<int64_t>() {
  return self()->getLongInt();
}
template <>
inline float Node::getConst<float>() {
  return self()->getFloat();
}
template <>
inline double Node::getConst<double>() {
  return self()->getDouble();
}

template <>
inline uint8_t Node::setConst<uint8_t>(uint8_t b) {
  return self()->setUnsignedByte(b);
}
template <>
inline int8_t Node::setConst<int8_t>(int8_t b) {
  return self()->setByte(b);
}
template <>
inline uint16_t Node::setConst<uint16_t>(uint16_t c) {
  return self()->setUnsignedShortInt(c);
}
template <>
inline int16_t Node::setConst<int16_t>(int16_t s) {
  return self()->setShortInt(s);
}
template <>
inline uint32_t Node::setConst<uint32_t>(uint32_t i) {
  return self()->setUnsignedInt(i);
}
template <>
inline int32_t Node::setConst<int32_t>(int32_t i) {
  return self()->setInt(i);
}
template <>
inline uint64_t Node::setConst<uint64_t>(uint64_t l) {
  return self()->setUnsignedLongInt(l);
}
template <>
inline int64_t Node::setConst<int64_t>(int64_t l) {
  return self()->setLongInt(l);
}
template <>
inline float Node::setConst<float>(float f) {
  return self()->setFloat(f);
}
template <>
inline double Node::setConst<double>(double d) {
  return self()->setDouble(d);
}
}  // namespace OMR

template <class T>
T OMR::Node::getIntegerNodeValue() {
  ...
  switch (opcode) {
    ...
    case TR::bconst:
      ...
      break;
    case TR::cconst:
      length = (T)self()->getConst<uint16_t>();
      break;
    case TR::sconst:
...
TR::TreeTop* OMR::Node::getBranchDestination() {
  TR_ASSERT(self()->hasBranchDestinationNode(),
            "attempting to access _branchDestinationNode field for node %s %p "
...
inline bool OMR::Node::hasNodeExtension() {
  return _flags.testAny(nodeHasExtension);
}

inline void OMR::Node::setHasNodeExtension(bool v) {
  _flags.set(nodeHasExtension, v);
}

inline bool OMR::Node::isNegative() {
  return self()->isNonPositive() && self()->isNonZero();
}

inline bool OMR::Node::isPositive() {
...
inline bool OMR::Node::isReferenceNonNull() {
  TR_ASSERT(self()->canCheckReferenceIsNonNull(),
            "Unexpected opcode %s for isReferenceNonNull",
...
inline const char* OMR::Node::printIsReferenceNonNull() {
