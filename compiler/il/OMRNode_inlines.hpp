...
inline TR::Node* OMR::Node::getChild(int32_t c)
{
  if (!self()->hasNodeExtension()) {
    TR_ASSERT(c < NUM_DEFAULT_CHILDREN, "getChild(%d) called on node " POINTER_PRINTF_FORMAT " with %d children.", c,
...
{
  TR_ASSERT(
    self()->getNumChildren() >= 1, "getFirstChild() called on node " POINTER_PRINTF_FORMAT " with no children", self());
...
{
  TR_ASSERT(
    self()->getNumChildren() >= 1, "getLastChild() called on node " POINTER_PRINTF_FORMAT " with no children", self());
...
{
  TR_ASSERT(
    self()->canCheckReferenceIsNonNull(), "Unexpected opcode %s for isReferenceNonNull", self()->getOpCode().getName());
