...
class OMR_EXTENSIBLE Block : public TR::CFGNode {
 public:
  ...
  ~Block() {
    TR_ASSERT(0,
              "blocks should not be freed until the destructor is updated to "
              "also free the catch block extension");
 ...
 private:
  ...
  void uncommonNodesBetweenBlocks(
      TR::Compilation *, TR::Block *,
      TR::ResolvedMethodSymbol *methodSymbol = NULL);
