...
#if DEBUG
  if (node != NULL) {
    TR_ASSERT(node->asBlock() != NULL,
              "Bad downcast from TR::CFGNode to TR::Block");
  }
#endif
 ...
 * Textual order is just a linear order of blocks. It does not represent the
 * control flow.
 * --> At any given point during compilation, the code compiled is represented
 * by a linked list of treetops, which are broken up into blocks.
...
public:
  ...
  ~Block() {
    TR_ASSERT(0, "blocks should not be freed until the destructor is updated "
                 "to also free the catch block extension");
  ...
  static TR::Block *createBlock(TR::TreeTop *entry, TR::TreeTop *exit,
                                TR::CFG &cfg);
  static TR::Block *createEmptyBlock(TR::Node *, TR::Compilation *,
                                     int32_t frequency = -1,
                                     TR::Block *block = NULL);
  static TR::Block *createEmptyBlock(TR::Compilation *comp,
                                     int32_t frequency = -1,
                                     TR::Block *block = NULL);

  TR::Block *breakFallThrough(TR::Compilation *comp, TR::Block *faller,
                              TR::Block *fallee);
  static void insertBlockAsFallThrough(TR::Compilation *comp, TR::Block *block,
                                       TR::Block *newFallThroughBlock);
  static void redirectFlowToNewDestination(TR::Compilation *comp,
                                           TR::CFGEdge *origEdge,
                                           TR::Block *newTo,
  ...
  /// getNextBlock and getPrevBlock return the next/previous block in the
  /// "textual" orders of blocks
  ...
  TR::Block *split(TR::TreeTop *startOfNewBlock, TR::CFG *cfg,
                   bool fixupCommoning = false,
                   bool copyExceptionSuccessors = true,
                   TR::ResolvedMethodSymbol *methodSymbol = NULL);

  TR::Block *splitPostGRA(TR::TreeTop *startOfNewBlock, TR::CFG *cfg,
                          bool copyExceptionSuccessors = true,
                          TR::ResolvedMethodSymbol *methodSymbol = NULL);

  TR::Block *splitWithGivenMethodSymbol(TR::ResolvedMethodSymbol *methodSymbol,
                                        TR::TreeTop *startOfNewBlock,
                                        TR::CFG *cfg,
                                        bool fixupCommoning = false,
                                        bool copyExceptionSuccessors = true);

  TR::Block *createConditionalSideExitBeforeTree(
      TR::TreeTop *tree, TR::TreeTop *compareTree, TR::TreeTop *exitTree,
      TR::TreeTop *returnTree, TR::CFG *cfg, bool markCold = true);

  TR::Block *createConditionalBlocksBeforeTree(
      TR::TreeTop *tree, TR::TreeTop *compareTree, TR::TreeTop *ifTree,
      TR::TreeTop *elseTree, TR::CFG *cfg, bool changeBlockExtensions = true,
      bool markCold = true);

  TR::Block *splitEdge(TR::Block *, TR::Block *, TR::Compilation *,
                       TR::TreeTop **lastTreeTop = NULL,
                       bool findOptimalInsertionPoint = true);

  virtual void removeFromCFG(TR::Compilation *);

  // remove the branch treetop from the block as well as the associated branch
  // edge. removeBranch assumes the last nonfence node is a branch.
  //
  ...
  // remove the edge from the previous destination block and update any GlRegDep
  // Nodes
  //
  void changeBranchDestination(TR::TreeTop *newDestination, TR::CFG *cfg,
                               bool callerFixesRegdeps = false);

  TR::Node *findFirstReference(TR::Symbol *sym, vcount_t visitCount);
  void collectReferencedAutoSymRefsIn(TR::Compilation *comp, TR_BitVector *,
                                      vcount_t);
  void collectReferencedAutoSymRefsIn(TR::Compilation *comp, TR::Node *,
                                      TR_BitVector *, vcount_t);
  ...
  TR_BlockStructure *setStructureOf(TR_BlockStructure *p) {
    return (_pStructureOf = p);
  }
  ...
  InstructionBoundaries &getInstructionBoundaries() {
    return _instructionBoundaries;
  }

  void addExceptionRangeForSnippet(uint32_t startPC, uint32_t endPC);
  InstructionBoundaries *getFirstSnippetBoundaries() {
    return _snippetBoundaries.getFirst();
  }

  TR::Instruction *getFirstInstruction() { return _firstInstruction; }
  TR::Instruction *setFirstInstruction(TR::Instruction *i) {
    return (_firstInstruction = i);
  }

  TR::Instruction *getLastInstruction() { return _lastInstruction; }
  TR::Instruction *setLastInstruction(TR::Instruction *i) {
    return (_lastInstruction = i);
  }
  ...
  public:
    ...
    TR_CatchBlockExtension()
        : _exceptionClass(NULL), _exceptionClassNameChars(NULL),
          _exceptionClassNameLength(0), _catchType(0), _exceptionsCaught(0),
          _handlerIndex(0), _inlineDepth(0), _owningMethod(NULL) {}
    ...
    TR_CatchBlockExtension(TR_CatchBlockExtension &other)
        : _exceptionClass(other._exceptionClass),
          ...
          _handlerIndex(other._handlerIndex), _inlineDepth(other._inlineDepth),
  ...
  TR_CatchBlockExtension *getCatchBlockExtension() {
    return _catchBlockExtension;
  }
  void setCatchBlockExtension(TR_CatchBlockExtension *extension) {
    _catchBlockExtension = extension;
  }

  void setHandlerInfo(uint32_t c, uint8_t d, uint16_t i, TR_ResolvedMethod *m,
                      TR::Compilation *comp);
  void setHandlerInfoWithOutBCInfo(
      uint32_t c, uint8_t d, uint16_t i, TR_ResolvedMethod *m,
  ...
  void setJ9EstimateCodeSizeMethod(TR_ResolvedMethod *m) {
    _j9EstimateSizeMethod = m;
  }
  TR_ResolvedMethod *getJ9EstimateCodeSizeMethod() {
    return _j9EstimateSizeMethod;
  }

  TR::DebugCounterAggregation *getDebugCounters() { return _debugCounters; }
  void setDebugCounters(TR::DebugCounterAggregation *debugCounters) {
    _debugCounters = debugCounters;
  }

  // getNormalizedFrequency returns a value between 0 and 100.
  // getGlobalNormalizedFrequency multiplies the normalizedfrequency by 10 for
  // hot methods and by 100 for scorching methods.
  //
  ...
  void setSpecializedDesyncCatchBlock() {
    _flags.set(_specializedDesyncCatchBlock);
  }
  bool specializedDesyncCatchBlock() {
    return _flags.testAny(_specializedDesyncCatchBlock);
  }
  ...
  bool isSynchronizedHandler() {
    return _flags.testAny(_isSynchronizedHandler);
  }
  ...
  void setIsCreatedAtCodeGen(bool b = true) {
    _flags.set(_createdAtCodeGen, b);
  }
  ...
  bool isCreatedByVersioning() {
    return _flags.testAny(_isCreatedByVersioning);
  }
  void setCreatedByVersioning(bool b) { _flags.set(_isCreatedByVersioning, b); }

  bool isEntryOfShortRunningLoop() {
    return _flags.testAny(_isEntryOfShortRunningLoop);
  }
  void setIsEntryOfShortRunningLoop() {
    _flags.set(_isEntryOfShortRunningLoop, true);
  }

  bool wasHeaderOfCanonicalizedLoop() {
    return _flags.testAny(_wasHeaderOfCanonicalizedLoop);
  }
  void setWasHeaderOfCanonicalizedLoop(bool b) {
    _flags.set(_wasHeaderOfCanonicalizedLoop, b);
  }
  ...
  flags16_t getPartialFlags() {
    return flags16_t(_moreflags.getValue(0xFF));
  } // J9

  void setIsUnsanitizeable(bool b = true) {
    _moreflags.set(_unsanitizeable, b);
  } // J9
  ...
  void setRestartBlock(bool b = true) {
    _moreflags.set(_restartBlock, b);
  } // J9
  bool isRestartBlock() { return _moreflags.testAny(_restartBlock); }

  void setPartialInlineBlock(bool b = true) {
    _moreflags.set(_partialInlineBlock, b);
  }
  bool isPartialInlineBlock() {
    return _moreflags.testAny(_partialInlineBlock);
  }

  void setIsEndBlock(bool b = true) { _moreflags.set(_endBlock, b); }
  bool isEndBlock() { return _moreflags.testAny(_endBlock); }

  void setIsDifficultBlock(bool b = true) {
    _moreflags.set(_difficultBlock, b);
  }
...
private:
  ...
  void
  uncommonNodesBetweenBlocks(TR::Compilation *, TR::Block *,
                             TR::ResolvedMethodSymbol *methodSymbol = NULL);
  ...
  { _isExtensionOfPreviousBlock = 0x00000001,
    _isCold = 0x00000002,
    _isSuperCold =
        0x00040000, // User specified cold/hotness by pragma or @ASSERT for PLX
    // AVAILABLE                          = 0x00000080,
    ...
    _isSpecialized =
        0x00800000, // Block has been specialized by loop specializer
  ...
  // TODO: These members are only used in J9EstimateCodeSize and should be moved
  // out.
  int32_t _blockSize;
...
public:
  ...
  TR_BlockCloner(TR::CFG *cfg, bool cloneBranchesExactly = false,
                 bool cloneSuccessorsOfLastBlock = false)
      : _cfg(cfg), _cloneBranchesExactly(cloneBranchesExactly),
