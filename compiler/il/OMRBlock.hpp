...
//
static inline TR::Block*
toBlock(TR::CFGNode* node)
{
#if DEBUG
  if (node != NULL) {
...
public:
  ...
  static TR::Block* createBlock(TR::TreeTop* entry,
                                TR::TreeTop* exit,
                                TR::CFG& cfg);
  static TR::Block* createEmptyBlock(TR::Node*,
                                     TR::Compilation*,
                                     int32_t frequency = -1,
                                     TR::Block* block = NULL);
  ...
  TR::Block* breakFallThrough(TR::Compilation* comp,
                              TR::Block* faller,
                              TR::Block* fallee);
  static void insertBlockAsFallThrough(TR::Compilation* comp,
                                       TR::Block* block,
  ...
  // Nodes
  //
  void changeBranchDestination(TR::TreeTop* newDestination,
                               TR::CFG* cfg,
                               bool callerFixesRegdeps = false);

  TR::Node* findFirstReference(TR::Symbol* sym, vcount_t visitCount);
  void collectReferencedAutoSymRefsIn(TR::Compilation* comp,
                                      TR_BitVector*,
                                      vcount_t);
  void collectReferencedAutoSymRefsIn(TR::Compilation* comp,
                                      TR::Node*,
  ...
  void setHandlerInfo(uint32_t c,
                      uint8_t d,
                      uint16_t i,
                      TR_ResolvedMethod* m,
                      TR::Compilation* comp);
...
public:
  ...
  TR_BlockCloner(TR::CFG* cfg,
                 bool cloneBranchesExactly = false,
