...
void OMR::ILOpCode::checkILOpArrayLengths() {
  for (int i = TR::FirstOMROp; i < TR::NumIlOps; i++) {
    ...
    TR_ASSERT(props.opcode == opCode,
              "_opCodeProperties table out of sync at index %d, has %s\n", i,
              op.getName());
  }
}
...
void OMR::ILOpCode::setTarget() {
  if (TR::Compiler->target.is64Bit()) {
    for (int32_t i = 0; i < TR::NumIlOps; ++i) {
      flags32_t *tp =
          (flags32_t *)(&_opCodeProperties[i].typeProperties);  // so ugly
...
TR::ILOpCodes OMR::ILOpCode::compareOpCode(TR::DataType dt,
