...
public:
    ...
    TR::ILOpCodes getOpCodeForSwapChildren() const { return _opCodeProperties[_opCode].swapChildrenOpCode; }

    /// Get the opcode to be used if the sense of this (compare) opcode is reversed.
    /// e.g. ificmplt --> ificmpge
    TR::ILOpCodes getOpCodeForReverseBranch() const { return _opCodeProperties[_opCode].reverseBranchOpCode; }

    /// Get the boolean compare opcode that corresponds to this compare-and-branch
    /// opcode.
    /// e.g. ificmplt --> icmplt
    TR::ILOpCodes convertIfCmpToCmp() { return _opCodeProperties[_opCode].booleanCompareOpCode; }

    /// Get the compare-and-branch opcode that corresponds to this boolean compare
    /// e.g. icmplt --> ificmplt
    TR::ILOpCodes convertCmpToIfCmp() { return _opCodeProperties[_opCode].ifCompareOpCode; }
    ...
    bool isArithmetic() const
    {
        return isAdd() || isSub() || isMul() || isDiv() || isRem() || isLeftShift() || isRightShift()
            || isShiftLogical() || isAnd() || isXor() || isOr() || isNeg() || isTernaryAdd() || isTernarySub();
    }
    ...
    bool isFunctionCall() const
    {
        return isCall() && getOpCodeValue() != TR::arraycopy && getOpCodeValue() != TR::arrayset
            && getOpCodeValue() != TR::bitOpMem && getOpCodeValue() != TR::bitOpMemND
            && getOpCodeValue() != TR::arraycmp && getOpCodeValue() != TR::arraycmpWithPad;
    }
