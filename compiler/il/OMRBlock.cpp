...
OMR::Block::getLastNonControlFlowTreeTop()
{
    ...
    while (tt->getNode()->getOpCode().isBranch() || tt->getNode()->getOpCode().isReturn() || tt->getNode()->getOpCode().isJumpWithMultipleTargets()) {
        tt = tt->getPrevTreeTop();
    }
    return tt;
}
...
bool OMR::Block::canFallThroughToNextBlock()
{
    ...
    if (lastNode->getOpCode().isGoto() || lastNode->getOpCode().isReturn() || lastNode->getOpCodeValue() == TR::athrow || lastNode->getOpCodeValue() == TR::igoto)
        return false;
    return true;
}
...
bool OMR::Block::doesNotNeedLabelAtStart()
{
    ...
    for (auto thisEdge = self()->getPredecessors().begin(); thisEdge != self()->getPredecessors().end(); ++thisEdge) {
        ...
        if (from == prevBlock && from->canFallThroughToNextBlock()) {
            ...
            if (lastNode->getOpCodeValue() == TR::treetop)
                lastNode = lastNode->getFirstChild();
            if (lastNode->getOpCodeValue() != TR::lookup && lastNode->getOpCodeValue() != TR::table && lastNode->getOpCodeValue() != TR::tstart && lastNode->getOpCodeValue() != TR::trtLookup)
                continue;
        }
        return false;
    }
...
OMR::Block::breakFallThrough(TR::Compilation* comp, TR::Block* faller, TR::Block* fallee)
{
    ...
    if (lastNode->getOpCode().isReturn() || lastNode->getOpCode().isGoto() || lastNode->getOpCode().isSwitch() || lastNode->getOpCode().isJumpWithMultipleTargets() || lastNode->getOpCodeValue() == TR::athrow)
 ...
 */
void OMR::Block::redirectFlowToNewDestination(TR::Compilation* comp, TR::CFGEdge* origEdge, TR::Block* newTo, bool useGotoForFallThrough)
{
...
void OMR::Block::uncommonNodesBetweenBlocks(TR::Compilation* comp, TR::Block* newBlock, TR::ResolvedMethodSymbol* methodSymbol)
{
...
bool OMR::Block::isTargetOfJumpWhoseTargetCanBeChanged(TR::Compilation* comp)
{
    ...
    for (auto predEdge = self()->getPredecessors().begin(); predEdge != self()->getPredecessors().end(); ++predEdge) {
        ...
        if (predBlock != startBlock && predBlock->getLastRealTreeTop()->getNode()->getOpCode().isJumpWithMultipleTargets() && predBlock->getLastRealTreeTop()->getNode()->getOpCode().hasBranchChildren())
            return true;
    }
    return false;
}
...
static bool checkStoreRegNodeListForNode(TR::Node* passThroughNode, List<TR::Node>* storeNodeList)
{
    ...
    for (TR::Node* storeNode = nodeListIter.getCurrent(); storeNode; storeNode = nodeListIter.getNext()) {
        if (storeNode->getFirstChild() == value && storeNode->getLowGlobalRegisterNumber() == passThroughNode->getLowGlobalRegisterNumber() && storeNode->getHighGlobalRegisterNumber() == passThroughNode->getHighGlobalRegisterNumber())
            return true;
    }
    return false;
}
    ...
    for (int i = 0; i < regDeps->getNumChildren(); i++) {
        ...
        if (dep->getOpCodeValue() == TR::PassThrough) {
            ...
            if (nodeInfoEntry != nodeInfo->end()) {
                ...
                if (nodeInfoEntry->second.second != NULL && nodeInfoEntry->second.second->getOpCode().isLoadReg() && dep->getLowGlobalRegisterNumber() == nodeInfoEntry->second.second->getLowGlobalRegisterNumber() && dep->getHighGlobalRegisterNumber() == nodeInfoEntry->second.second->getHighGlobalRegisterNumber()) {
        ...
        if (isInternalPointer) {
            ...
            if (value->getOpCode().isArrayRef()) {
                ...
                if (valueChild->isInternalPointer() && valueChild->getPinningArrayPointer()) {
                    ...
                    if (valueChildSymRef != NULL && (valueChild->getOpCode().isLoadVarDirect() && valueChildSymRef->getSymbol()->isAuto()) || (valueChild->getOpCode().isLoadReg() && valueChildSymRef->getSymbol()->castToAutoSymbol()->isInternalPointer())) {
...
#define MIN_EXCEPTION_NAME_SIZE 5
    ...
    { 9, "Exception", CanCatchNullCheck | CanCatchDivCheck | CanCatchBoundCheck | CanCatchArrayStoreCheck | CanCatchArrayNew | CanCatchCheckCast | CanCatchMonitorExit },
    ...
    { 16, "RuntimeException", CanCatchNullCheck | CanCatchDivCheck | CanCatchBoundCheck | CanCatchArrayStoreCheck | CanCatchArrayNew | CanCatchCheckCast | CanCatchMonitorExit },
...
void OMR::Block::collectReferencedAutoSymRefsIn(TR::Compilation* comp, TR_BitVector* referencedAutoSymRefs, vcount_t visitCount)
{
...
void OMR::Block::collectReferencedAutoSymRefsIn(TR::Compilation* comp, TR::Node* node, TR_BitVector* referencedAutoSymRefs, vcount_t visitCount)
{
...
{
    return self()->isExtensionOfPreviousBlock() ? self()->getEntry()->getPrevTreeTop()->getNode()->getBlock()->startOfExtendedBlock()
                                                : self();
}
...
void OMR::Block::setHandlerInfoWithOutBCInfo(uint32_t c, uint8_t d, uint16_t i, TR_ResolvedMethod* m, TR::Compilation* comp)
{
    self()->ensureCatchBlockExtensionExists(comp);
    TR_CatchBlockExtension* cbe = _catchBlockExtension;
    cbe->_catchType = c;
...
void OMR::Block::setExceptionClassName(char* name, int32_t length, TR::Compilation* comp)
{
    ...
    _catchBlockExtension->_exceptionClass = self()->getOwningMethod()->fe()->getClassFromSignature(name, length, self()->getOwningMethod());
...
TR_BlockCloner::cloneBlocks(TR::Block* from, TR::Block* lastBlock)
{
    ...
    for (; from; from = from->getNextBlock()) {
        ...
        TR::Block* to = new (comp->trHeapMemory()) TR::Block(*from, TR::TreeTop::create(comp, NULL), TR::TreeTop::create(comp, NULL));
...
TR_BlockCloner::cloneBlocks(TR_LinkHeadAndTail<BlockMapper>* bMap)
{
    ...
    for (BlockMapper* itr = bMap->getFirst(); itr->getNext(); itr = itr->getNext()) {
        ...
        //
        if (
            lastRealNode->getOpCode().isIf() && lastRealNode->getBranchDestination()->getNode()->getBlock()->getNumber() == itr->_to->getNextBlock()->getNumber()) {
