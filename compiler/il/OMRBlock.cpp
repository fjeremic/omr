...
TR::Block*
OMR::Block::createEmptyBlock(TR::Compilation* comp,
                             int32_t frequency,
                             TR::Block* block)
{
  ...
  , _catchBlockExtension(NULL)
{
  ...
  if (other._globalRegisters)
    _globalRegisters =
      new (other._region) TR_Array<TR_GlobalRegister>(*other._globalRegisters);
...
TR::Block*
OMR::Block::createEmptyBlock(TR::Node* n,
                             TR::Compilation* comp,
                             int32_t frequency,
                             TR::Block* block)
{
...
OMR::Block::doesNotNeedLabelAtStart()
{
  ...
  for (auto thisEdge = self()->getPredecessors().begin();
       thisEdge != self()->getPredecessors().end();
       ++thisEdge) {
...
TR::Block*
OMR::Block::breakFallThrough(TR::Compilation* comp,
                             TR::Block* faller,
                             TR::Block* fallee)
{
 ...
 */
void
OMR::Block::insertBlockAsFallThrough(TR::Compilation* comp,
                                     TR::Block* block,
                                     TR::Block* newFallThroughBlock)
{
  ...
  TR::TreeTop::insertTreeTops(comp,
                              block->getExit(),
 ...
 */
void
OMR::Block::redirectFlowToNewDestination(TR::Compilation* comp,
                                         TR::CFGEdge* origEdge,
                                         TR::Block* newTo,
  ...
  if (from->getEntry() == NULL) {
    ...
    if (lastNode->getOpCode().isBranch() &&
      ...
      if (lastNode->getOpCode().hasBranchChildren()) {
        ...
        if (origToEntry)
          traceMsg(comp,
                   "jumpwithmultipletargets: origToEntry->getNode = %p\n",
                   origToEntry->getNode());
        if (newToEntry)
          traceMsg(comp,
                   "jumpwithmultipletargets: newToEntry->getNode = %p\n",
                   newToEntry->getNode());
        // Get the right branch target
        for (int32_t i = 0; i < lastNode->getNumChildren() - 1; i++) {
          TR::Node* child = lastNode->getChild(i);
          traceMsg(comp,
                   "considering node %p with branch destination %p \n",
                   child,
...
OMR::Block::findVirtualGuardBlock(TR::CFG* cfg)
{
  ...
  for (auto nextEdge = self()->getPredecessors().begin();
       nextEdge != self()->getPredecessors().end();
       ++nextEdge) {
...
void
OMR::Block::changeBranchDestination(TR::TreeTop* newDestination,
                                    TR::CFG* cfg,
...
void
OMR::Block::uncommonNodesBetweenBlocks(TR::Compilation* comp,
                                       TR::Block* newBlock,
                                       TR::ResolvedMethodSymbol* methodSymbol)
{
  ...
  TR_HandleInjectedBasicBlock ibb(comp,
                                  NULL,
...
OMR::Block::isTargetOfJumpWhoseTargetCanBeChanged(TR::Compilation* comp)
{
  ...
  for (auto predEdge = self()->getPredecessors().begin();
       predEdge != self()->getPredecessors().end();
       ++predEdge) {
...
TR::Block*
OMR::Block::splitWithGivenMethodSymbol(TR::ResolvedMethodSymbol* methodSymbol,
                                       TR::TreeTop* startOfNewBlock,
                                       TR::CFG* cfg,
                                       bool fixupCommoning,
                                       bool copyExceptionSuccessors)
{
  return self()->split(startOfNewBlock,
                       cfg,
                       fixupCommoning,
                       copyExceptionSuccessors,
                       methodSymbol);
}
 ...
 */
static void
replaceNodesInSubtree(TR::Node* node,
                      NodeTable* nodeInfo,
 ...
 */
static void
replaceNodesInTrees(TR::Compilation* comp,
                    TR::TreeTop* start,
                    NodeTable* nodeInfo)
{
 ...
 */
static bool
checkIfRegisterIsAvailable(TR::Compilation* comp,
                           TR::Node* node,
                           TR_BitVector& unavailableRegisters)
{
 ...
 */
static bool
checkStoreRegNodeListForNode(TR::Node* passThroughNode,
 ...
 */
static void
gatherUnavailableRegisters(TR::Compilation* comp,
                           TR::Node* regDeps,
      ...
      if (nodeInfoEntry != nodeInfo->end()) {
        ...
        if (nodeInfoEntry->second.second == NULL) {
          ...
          if (storeRegNodeInfoEntry == storeNodeInfo->end() &&
            ...
            if (checkIfRegisterIsAvailable(
                  comp, storeNode, unavailableRegisters)) {
        ...
        if (nodeInfoEntry->second.second != NULL &&
          ...
          TR::Node* regStore = TR::Node::create(
            value,
            comp->il.opCodeForRegisterStore(value->getDataType()),
            1,
            nodeToBeStored);
          ...
          if (nodeToBeStored->getOpCode().isLoadConst() ||
            ...
            TR_ASSERT_FATAL(
              storeRegNodeInfoEntry != storeNodeInfo->end(),
              "We have a node n%dn under PassThrough node n%dn but we did not "
              "find a regStore that is using the info.",
                    ...
                    TR::TreeTop* insertBefore)
{
  ...
  if (!symRef) {
    symRef =
      comp->getSymRefTab()->createTemporary(methodSymbol, value->getDataType());
 ...
 */
TR::Block*
OMR::Block::splitPostGRA(TR::TreeTop* startOfNewBlock,
                         TR::CFG* cfg,
                         bool copyExceptionSuccessors,
                         TR::ResolvedMethodSymbol* methodSymbol)
{
  TR::Compilation* comp = cfg->comp();
  TR::Block* newBlock = self()->split(
    startOfNewBlock, cfg, false, copyExceptionSuccessors, methodSymbol);
...

  {
    ...
    for (TR::TreeTop* iter = newBlock->getEntry();
                   ...
                   0) // Case - 2 : TreeTop Node is Branch
      {
        ...
        if (regDeps->getOpCodeValue() == TR::GlRegDeps) {
          gatherUnavailableRegisters(comp,
                                     regDeps,
                                     iter,
                                     nodeInfo,
                                     storeNodeInfo,
                                     &storeRegNodePostSplitPoint,
        ...
        if (regInfo.first > -1) {
          TR::SymbolReference* ref =
            value->getOpCode().isLoadReg()
              ...
              : createSymRefForNode(
                  comp, methodSymbol, value, self()->getExit());
          TR::Node* regLoad = TR::Node::create(
            value, comp->il.opCodeForRegisterLoad(value->getDataType()));
          TR::Node* regStore = TR::Node::create(
            value,
    ...
    if (depCount > 0) {
      ...
      TR::Node* exitGlRegDeps =
        TR::Node::create(self()->getExit()->getNode(), TR::GlRegDeps, depCount);
      childIdx = 0;
...
TR::Block*
OMR::Block::split(TR::TreeTop* startOfNewBlock,
                  TR::CFG* cfg,
                  bool fixupCommoning,
                  bool copyExceptionSuccessors,
                  TR::ResolvedMethodSymbol* methodSymbol)
{
  ...
  self()->setExit(TR::TreeTop::create(comp,
                                      startOfNewBlock->getPrevTreeTop(),
  ...
  if (structure) {
    ...
    if (thisBlockStructure) {
      ...
      TR_StructureSubGraphNode* blockStructureNode2 =
        new (cfg->structureRegion()) TR_StructureSubGraphNode(blockStructure2);
      ...
      for (auto nextSucc = subNode->getSuccessors().begin();
           nextSucc != subNode->getSuccessors().end();
           ++nextSucc)
      ...
      for (auto nextSucc = subNode->getExceptionSuccessors().begin();
           nextSucc != subNode->getExceptionSuccessors().end();
           ++nextSucc) {
        ...
        for (auto e1 = block2->getExceptionSuccessors().begin();
             e1 != block2->getExceptionSuccessors().end();
             ++e1) {
...
void
OMR::Block::collectReferencedAutoSymRefsIn(TR::Compilation* comp,
                                           ...
                                           vcount_t visitCount)
{
  ...
  for (TR::TreeTop* tt = self()->getFirstRealTreeTop(); tt != self()->getExit();
       tt = tt->getNextTreeTop())
    self()->collectReferencedAutoSymRefsIn(
      comp, tt->getNode(), referencedAutoSymRefs, visitCount);
}
                                           ...
                                           vcount_t visitCount)
{
  ...
  for (int32_t i = 0; i < node->getNumChildren(); ++i)
    self()->collectReferencedAutoSymRefsIn(
      comp, node->getChild(i), referencedAutoSymRefs, visitCount);
}

TR::Block*
OMR::Block::splitEdge(TR::Block* from,
                      TR::Block* to,
                      TR::Compilation* c,
                      ...
                      bool findOptimalInsertionPoint)
{
  ...
  TR_ASSERT(
    !to->isOSRCatchBlock(),
    "Splitting edge to OSRCatchBlock (block_%d -> block_%d) is not supported\n",
    from->getNumber(),
    to->getNumber());
  ...
  if (entryTree) {
    ...
    if (isExceptionEdge) {
      newBlock->setHandlerInfo(to->getCatchType(),
                               to->getInlineDepth(),
                               to->getHandlerIndex(),
                               to->getOwningMethod(),
                               c);
      ...
      TR::TreeTop* gotoTreeTop = TR::TreeTop::create(
        c,
        TR::Node::create(
          from->getExit()->getNode(), TR::Goto, 0, to->getEntry()));
...
void
OMR::Block::setHandlerInfo(uint32_t c,
                           uint8_t d,
                           uint16_t i,
                           TR_ResolvedMethod* m,
                           TR::Compilation* comp)
...
void
OMR::Block::setHandlerInfoWithOutBCInfo(uint32_t c,
                                        uint8_t d,
                                        uint16_t i,
                                        TR_ResolvedMethod* m,
                                        TR::Compilation* comp)
...
void
OMR::Block::setExceptionClassName(char* name,
                                  int32_t length,
                                  TR::Compilation* comp)
{
  self()->ensureCatchBlockExtensionExists(comp);
...
TR_BlockCloner::cloneBlocks(TR::Block* from, TR::Block* lastBlock)
{
  ...
  for (; from; from = from->getNextBlock()) {
    ...
    TR::Block* to = new (comp->trHeapMemory()) TR::Block(
      *from, TR::TreeTop::create(comp, NULL), TR::TreeTop::create(comp, NULL));
