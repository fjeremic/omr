...
{
  bool isGoToBlock = (self()->getEntry() != NULL && self()->getLastRealTreeTop()->getPrevTreeTop() == self()->getEntry()
  ...
  for (auto thisEdge = self()->getPredecessors().begin(); thisEdge != self()->getPredecessors().end(); ++thisEdge) {
    ...
    if (from == prevBlock && from->canFallThroughToNextBlock()) {
      ...
      if (lastNode->getOpCodeValue() != TR::lookup && lastNode->getOpCodeValue() != TR::table
        && lastNode->getOpCodeValue() != TR::tstart && lastNode->getOpCodeValue() != TR::trtLookup)
        continue;
    }
    return false;
  }
...
void OMR::Block::changeBranchDestination(TR::TreeTop* newDestination, TR::CFG* cfg, bool callerFixesRegdeps)
{
  ...
  if (numChildren > 0 && branchNode->getChild(numChildren - 1)->getOpCodeValue() == TR::GlRegDeps) {
    ...
    if (newDestination->getNode()->getNumChildren() == 0) {
      ...
      TR_ASSERT(
        toGlRegDep->getOpCodeValue() == TR::GlRegDeps, "expected the bbstart child to be a GlRegDep.  It's not!");
  ...
  TR::Compilation* comp, TR::Node* node, TR_BitVector& unavailableRegisters)
{
  ...
  if (dt.isIntegral() || dt.isAddress()) {
    ...
    TR_ASSERT_FATAL(false,
      "Unknown data type %s encountered when trying to pick a register for postGRA block splitting!", dt.toString());
...
{
  // Following assert is to make sure that this function is called with node which have Global Registers associated with
  // it.
      ...
      if (nodeInfoEntry != nodeInfo->end()) {
        ...
        if (nodeInfoEntry->second.second == NULL) {
          ...
          if (storeRegNodeInfoEntry == storeNodeInfo->end()
            ...
            // Node under the PassThrough is not stored into register before the split point which means we must have
            // stored it into the register after split point. In this case we do not have to do anything as when we
            // replace the node post split point, it will be replaced under corresponding regstore as well.
            needToCreateRegStore = false;
          } else {
            // Node under passthrough is stored into some set of registers (Not necessarily same set of registers as we
            // only record last regStore for the node) and if those set of registers are available, then use them to
            // create a replacement.
            TR_ASSERT_FATAL(storeRegNodeInfoEntry != storeNodeInfo->end(),
              "We should have a regStore pre split point that can be used to allocate register for replacement node");
            TR::Node* storeNode = storeRegNodeInfoEntry->second;
            if (checkIfRegisterIsAvailable(comp, storeNode, unavailableRegisters)) {
              // Whether the PassThrough uses same register or not, use the last recorded regStore to assign register to
              // the node.
              ...
              // Course of action to analyze reg store for block splitter is to check for the registers used for the
              // node post split point before we check the regStore information pre split point. As regStore post split
              // point is most likely to be used later. This will reduce number of shuffles. So if we get the candidate
              // from before split point, we do not need to check list of RegStores post split point.
        ...
        if (nodeInfoEntry->second.second != NULL && nodeInfoEntry->second.second->getOpCode().isLoadReg()
          ...
          && (!needToCheckStoreRegPostSplitPoint || !checkStoreRegNodeListForNode(dep, storeRegNodePostSplitPoint))) {
          // We have either used different register for replacement to value node after split point or Passthrough has
          // corresponding regStore before spilt point, but registers were unavailable. In these cases we need to create
          // a regStore for the PassThrough. If we have replacement then need to store the replacement to register,
          // else, value needs to be stored.
          TR::Node* nodeToBeStored = nodeInfoEntry->second.second != NULL ? nodeInfoEntry->second.second : value;
          TR::Node* regStore
            = TR::Node::create(value, comp->il.opCodeForRegisterStore(value->getDataType()), 1, nodeToBeStored);
          ...
          if (nodeToBeStored->getOpCode().isLoadConst() || nodeToBeStored->getOpCodeValue() == TR::loadaddr
            ...
            auto storeRegNodeInfoEntry = storeNodeInfo->find(value);
            TR_ASSERT_FATAL(storeRegNodeInfoEntry != storeNodeInfo->end(),
              "We have a node n%dn under PassThrough node n%dn but we did not find a regStore that is using the info.",
  ...
  TR::Compilation* comp, TR::ResolvedMethodSymbol* methodSymbol, TR::Node* value, TR::TreeTop* insertBefore)
{
  ...
  if (value->isInternalPointer() && value->getPinningArrayPointer()) {
    ...
    TR::Symbol* symbol
      = symRef->getSymbol()->castToInternalPointerAutoSymbol()->setPinningArrayPointer(value->getPinningArrayPointer());
  } else {
  ...
  TR::TreeTop* startOfNewBlock, TR::CFG* cfg, bool copyExceptionSuccessors, TR::ResolvedMethodSymbol* methodSymbol)
{
...

  {
    ...
    StoreRegNodeTable* storeNodeInfo
      = new (stackMemoryRegion) StoreRegNodeTable(NodeTableComparator(), StoreRegNodeTableAllocator(stackMemoryRegion));
     ...
     */
    for (TR::PostorderNodeOccurrenceIterator iter(start, comp, "FIX_POSTGRA_SPLIT_COMMONING"); iter != end; ++iter) {
     ...
     *       1. If node under PassThrough node is stored into the same register after split point, we do not have to do
     * anything.
    ...
    for (TR::TreeTop* iter = newBlock->getEntry(); iter != nextExtendedBlockEntry;) {
      ...
      if (node->getOpCode().isStoreReg()) // Case - 1 : TreeTop Node is RegStore
      {
        ...
        // If storedNode is needed to be uncommoned and we do not have found replacement node yet and it is stored in
        // register which is still available then use the register to store the node.
      ...
      } else if (node->getOpCode().isBranch() && node->getNumChildren() > 0) // Case - 2 : TreeTop Node is Branch
      {
        ...
        if (regDeps->getOpCodeValue() == TR::GlRegDeps) {
          gatherUnavailableRegisters(
            comp, regDeps, iter, nodeInfo, storeNodeInfo, &storeRegNodePostSplitPoint, unavailableRegisters);
        }
      ...
      if (nextTreeTop == nextExtendedBlockEntry) // TreeTop Node is BBExit and next tree top is nextExtendedBlockEntry
      {
        ...
        if (regDeps && regDeps->getOpCodeValue() == TR::GlRegDeps) {
          // If the previous tree top of BBEnd contains branch, we need to make sure any regStore we add are added
          // before that.
    ...
    for (auto iter = nodeInfo->begin(), end = nodeInfo->end(); iter != end; ++iter) {
      ...
      // For all the nodes in the nodeTable for which we can not find replacement yet, create replacement node using
      // available registers.
      if (!iter->second.second) {
        ...
        // If we can not find a register, store it into temp slot.
          ...
          // Also we need to make sure that replacement node is a node that is converted back to original type of the
          // node.
          ...
          self()->getExit()->insertBefore(TR::TreeTop::create(comp,
            TR::Node::createWithSymRef(
              value, comp->il.opCodeForDirectStore(convertedValDataType), 1, convertedValueNode, storedValueSymRef)));
      ...
      // Now as we have created a node to replace all the references of value node after split point, add all nodes
      // which are not stored into the temp slot to the list for entry and exit dependency which will be used to create
      // GlRegDeps for exit node of the original block and entry node of new block.
  ...
  bool copyExceptionSuccessors, TR::ResolvedMethodSymbol* methodSymbol)
{
  ...
  TR::Block* block2 = new (comp->trHeapMemory())
    TR::Block(TR::TreeTop::create(comp, TR::Node::create(startNode, TR::BBStart)), self()->getExit(), comp->trMemory());
  ...
  if (structure) {
    ...
    if (thisBlockStructure) {
      TR_BlockStructure* blockStructure2
        = new (cfg->structureRegion()) TR_BlockStructure(comp, block2->getNumber(), block2);
...
#define MIN_EXCEPTION_NAME_SIZE 5
  ...
  { 9, "Throwable", CanCatchEverything }, { 12, "UnknownError", CanCatchNew | CanCatchArrayNew },
  { 13, "InternalError", CanCatchNew | CanCatchArrayNew }, { 16, "OutOfMemoryError", CanCatchNew | CanCatchArrayNew },
  { 16, "RuntimeException",
    CanCatchNullCheck | CanCatchDivCheck | CanCatchBoundCheck | CanCatchArrayStoreCheck | CanCatchArrayNew
      | CanCatchCheckCast | CanCatchMonitorExit },
  { 18, "ClassCastException", CanCatchCheckCast }, { 18, "IllegalAccessError", CanCatchArrayNew },
  { 18, "InstantiationError", CanCatchNew }, { 18, "StackOverflowError", CanCatchNew | CanCatchArrayNew },
  { 19, "ArithmeticException", CanCatchDivCheck }, { 19, "ArrayStoreException", CanCatchArrayStoreCheck },
  { 19, "VirtualMachineError", CanCatchNew | CanCatchArrayNew }, { 20, "NullPointerException", CanCatchNullCheck },
  { 25, "IndexOutOfBoundsException", CanCatchBoundCheck }, { 26, "NegativeArraySizeException", CanCatchArrayNew },
...
TR_Array<TR_GlobalRegister>& OMR::Block::getGlobalRegisters(TR::Compilation* c)
{
  if (!_globalRegisters)
    _globalRegisters = new (c->trStackMemory())
      TR_Array<TR_GlobalRegister>(c->trMemory(), c->cg()->getNumberOfGlobalRegisters(), true, stackAlloc);
...
void OMR::Block::setHandlerInfo(uint32_t c, uint8_t d, uint16_t i, TR_ResolvedMethod* m, TR::Compilation* comp)
{
...
void OMR::Block::setHandlerInfoWithOutBCInfo(
  uint32_t c, uint8_t d, uint16_t i, TR_ResolvedMethod* m, TR::Compilation* comp)
{
  self()->ensureCatchBlockExtensionExists(comp);
  TR_CatchBlockExtension* cbe = _catchBlockExtension;
  cbe->_catchType = c;
...
void OMR::Block::setExceptionClassName(char* name, int32_t length, TR::Compilation* comp)
{
  ...
  if (length < 10 + MIN_EXCEPTION_NAME_SIZE || length > 10 + MAX_EXCEPTION_NAME_SIZE || strncmp(name, "java/lang/", 10))
    return;
...
bool OMR::Block::verifyOSRInduceBlock(TR::Compilation* comp)
{
  ...
  if (self()->getSuccessors().size() != 1 || self()->getSuccessors().front()->getTo() != comp->getFlowGraph()->getEnd())
    return false;
...
TR::Block* TR_BlockCloner::cloneBlocks(TR_LinkHeadAndTail<BlockMapper>* bMap)
{
  ...
  // through node (incorrectly) by cloneNode.  This code patches up the branches so they point to the right target (the
  // old fall through node) we don't mess with the last node as the connection is handled by whoever called us
  //
...
TR::Block* TR_BlockCloner::doBlockClone(TR_LinkHeadAndTail<BlockMapper>* bMap)
{
  ...
  // cloning along a fall through path, and you are trying to preserve branches exactly (ie _cloneBranchesExactly is set
  // to true Luckily, so far no code has ever not done that.  Most code that clonesBranchesExactly clones a single
