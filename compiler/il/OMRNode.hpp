...
#define NULL_USEDEF_SYMBOL_INDEX \
  0xFFFF  // TODO: should be 0xFFFF until we change the date type of _localIndex
          // in Symbol.hpp
...
/// \def HIGH_VISIT_COUNT: recommended max # visit counts between reset
/// operations (ie. soft limit) \def VCOUNT_HEADROOM:  max # visit counts that
...
class OMR_EXTENSIBLE Node {
 ...
 private:
  ...
  /// this is a C++ convention to avoid accidental assignment between instances
  /// of this class Once we have universal C++11 support, should be changed to
  /// use "= delete"
 ...
 protected:
  ...
  static TR::Node *createInternal(TR::Node *originatingByteCodeNode,
 ...
 public:
  ...
  static TR::Node *recreateWithSymRef(TR::Node *originalNode, TR::ILOpCodes op,
                                      TR::SymbolReference *newSymRef);

  // create methods from everywhere other than the ilGenerator need
  // to pass in a TR::Node pointer from which the byte code information will be
  // copied
  //
  static TR::Node *create(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
  ...
  static TR::Node *create(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
                          ...
                          TR::TreeTop *dest = 0);
  static TR::Node *createWithSymRef(TR::Node *originatingByteCodeNode,
                                    TR::ILOpCodes op, uint16_t numChildren,
  ...
  static TR::Node *createWithSymRef(TR::Node *originatingByteCodeNode,
                                    ...
                                    TR::Node *first,
                                    TR::SymbolReference *symRef);
  static TR::Node *createOnStack(TR::Node *originatingByteCodeNode,
                                 TR::ILOpCodes op, uint16_t numChildren = 0);
  static TR::Node *recreateWithoutProperties(TR::Node *originalNode,
                                             TR::ILOpCodes op,
                                             uint16_t numChildren = 0,
                                             TR::SymbolReference *symRef = 0);

  static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren = 0);
  static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren,
                          TR::TreeTop *dest);
  static TR::Node *create(TR::ILOpCodes op, uint16_t numChildren,
                          int32_t intValue, TR::TreeTop *dest = 0);
  static TR::Node *createWithSymRef(TR::ILOpCodes op, uint16_t numChildren,
 ...
 private:
  static TR::Node *recreateWithoutSymRef_va_args(TR::Node *originalNode,
                                                 TR::ILOpCodes op,
                                                 uint16_t numChildren,
                                                 uint16_t numChildArgs,
                                                 va_list &args);
  static TR::Node *createWithoutSymRef(TR::ILOpCodes op, uint16_t numChildren,
                                       uint16_t numChildArgs, ...);
  static TR::Node *recreateWithoutSymRef(TR::Node *originalNode,
                                         TR::ILOpCodes op, uint16_t numChildren,
                                         uint16_t numChildArgs, ...);
  static TR::Node *recreateWithSymRefWithoutProperties(TR::Node *originalNode,
                                                       TR::ILOpCodes op,
                                                       uint16_t numChildren,
                                                       uint16_t numChildArgs,
                                                       ...);
 ...
 private:
  ...
  template <class... ChildrenAndSymRefType>
  static TR::Node *createWithSymRefInternal(
      TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs,
 ...
 public:
  ...
  static TR::Node *createWithRoomForOneMore(
      TR::ILOpCodes op, uint16_t numChildren, void *symbolRefOrBranchTarget = 0,
      TR::Node *first = 0, TR::Node *second = 0, TR::Node *third = 0,
      TR::Node *fourth = 0, TR::Node *fifth = 0);
  static TR::Node *createWithRoomForThree(TR::ILOpCodes op, TR::Node *first,
                                          TR::Node *second,
                                          void *symbolRefOrBranchTarget = 0);
  static TR::Node *createWithRoomForFive(TR::ILOpCodes op, TR::Node *first,
                                         TR::Node *second, TR::Node *third,
                                         TR::Node *fourth,
                                         void *symbolRefOrBranchTarget = 0);

  static TR::Node *createif(TR::ILOpCodes op, TR::Node *first, TR::Node *second,
                            TR::TreeTop *branchTarget = 0);
  static TR::Node *createbranch(TR::ILOpCodes op, TR::Node *first,
                                TR::TreeTop *branchTarget = 0);
  static TR::Node *createCase(TR::Node *originatingByteCodeNode, TR::TreeTop *,
                              CASECONST_TYPE = 0);

  static TR::Node *createArrayOperation(TR::ILOpCodes arrayOp, TR::Node *first,
                                        TR::Node *second, TR::Node *third,
                                        TR::Node *fourth = NULL,
                                        TR::Node *fifth = NULL);
  static TR::Node *createArraycopy();
  static TR::Node *createArraycopy(TR::Node *first, TR::Node *second,
                                   TR::Node *third);
  static TR::Node *createArraycopy(TR::Node *first, TR::Node *second,
                                   TR::Node *third, TR::Node *fourth,
                                   TR::Node *fifth);
  ...
  /**
   * \brief
   *    Create a call to potentialOSRPointHelperSymbol, only to be used during
   * ILGen
   *
   ...
   *    The offset to be added to the originatingByteCodeNode's bytecode index
   * to get the target bytecode index. This is to be stored in
   * _unionBase._osrInductionOffset
   *
   * \note
   *    One can not create a potentialOSRPointHelper call and stick it to
   * anywhere in the trees without the right exception setup and without the
   * right bookkeeping being in place for the call. Preventative checks are
   * required to avoid potential misuses. Due the fact that during ILGen the OSR
   * infrastructure may be incomplete, the checks can be done in ILGen are
   *    different than checks needed after ILGen.
   *
   *    This API does checks required to create the helper in ILGen, thus it is
   * only to be used in ILGen.
   */
  static TR::Node *createPotentialOSRPointHelperCallInILGen(
      TR::Node *originatingByteCodeNode, int32_t offset);
  /**
   * \brief
   *    Create a call to osrFearPointHelperSymbol
   *
   * \parm originatingByteCodeNode  The node whose bytecode info is used to
   * create the call
   *
   */
  static TR::Node *createOSRFearPointHelperCall(
      TR::Node *originatingByteCodeNode);

  static TR::Node *createLoad(TR::SymbolReference *symRef);
  static TR::Node *createLoad(TR::Node *originatingByteCodeNode,
                              TR::SymbolReference *);
  ...
  static TR::Node *createStore(TR::Node *originatingByteCodeNode,
                               TR::SymbolReference *symRef, TR::Node *value);

  static TR::Node *createRelative32BitFenceNode(void *relocationAddress);
  static TR::Node *createRelative32BitFenceNode(
      TR::Node *originatingByteCodeNode, void *);

  static TR::Node *createAddressNode(TR::Node *originatingByteCodeNode,
                                     TR::ILOpCodes op, uintptrj_t address);
  static TR::Node *createAddressNode(TR::Node *originatingByteCodeNode,
                                     TR::ILOpCodes op, uintptrj_t address,
                                     uint8_t precision);

  static TR::Node *createAllocationFence(TR::Node *originatingByteCodeNode,
                                         TR::Node *fenceNode);
  ...
  static TR::Node *createConstZeroValue(TR::Node *originatingByteCodeNode,
                                        TR::DataType dt);
  static TR::Node *createConstOne(TR::Node *originatingByteCodeNode,
                                  TR::DataType dt);
  static TR::Node *createConstDead(TR::Node *originatingByteCodeNode,
                                   TR::DataType dt, intptrj_t extraData = 0);

  static TR::Node *createCompressedRefsAnchor(TR::Node *firstChild);

  static TR::Node *createAddConstantToAddress(TR::Node *addr, intptr_t value,
                                              TR::Node *parent = NULL);
  static TR::Node *createLiteralPoolAddress(TR::Node *node, size_t offset);

  static TR::Node *createVectorConst(TR::Node *originatingByteCodeNode,
                                     TR::DataType dt);
   ...
   */
 private:
  void copyChildren(TR::Node *from, uint16_t numChildren = 0,
                    bool forNodeExtensionOnly = false);
  ...
  static bool isLegalCallToCreate(TR::ILOpCodes opvalue) {
    ...
    TR_ASSERT(opvalue != TR::arraycopy,
              "use createArraycopy to create this node");
    TR_ASSERT(opvalue != TR::v2v,
              "use createVectorConversion to create node: %s",
              opcode.getName());
    TR_ASSERT(opvalue != TR::vconst, "use createVectorConst to create node: %s",
              opcode.getName());
    return true;
  }
  ...
  static bool isNotDeprecatedUnsigned(TR::ILOpCodes opvalue) {
    switch (opvalue) {
        // More cases is going to be added to this function for
        // eclipse/omr#2657. return false;
   ...
   */
 public:
  /** @return true if n is one of the children of this node, return false
   * otherwise */
  ...
  /// creates clone, and adjusts reference counts of clone, node and its
  /// children as though the node had been uncommoned from its parent.
  ///
  ...
  bool containsNode(TR::Node *searchNode,
                    vcount_t visitCount);  // Careful how you use this: it
                                           // doesn't account for aliasing
  ...
  /// is a composite operator, made from a high order part and its adjunct
  /// operator which is its third child. It returns true if the node has the
  /// form:
  ///
  ///     highOp
  ///       firstChild
  ...
  /// Whether this node is high part of a ternary subtract or addition, like a
  /// dual this is a composite operator, made from a high order part and its
  /// adjunct operator which is the first child of its third child. It returns
  /// true if the node has the form:
  ///
  ///     highOp
  ///       firstChild
  ///       secondChild
  ...
  /// Given a monenter node, return the persistent class identifer that's being
  /// synchronized
  ...
  // Given that this is a NULLCHK node, find the reference that is being
  // checked.
  TR::Node *getNullCheckReference();
  void setNullCheckReference(TR::Node *refNode);

  /// getFirstArgumentIndex returns the child index where the arguments start
  /// for a call node. For an indirect call the first child may be the vft.
  ...
  void gatherAllNodesWhichMightKillCondCode(
  ...
  TR::TreeTop *createStoresForVar(TR::SymbolReference *&nodeRef,
  ...
  /** \brief
   *     Determines whether this node should be sign/zero extended to 32-bit at
   * the point of evaluation (source) by checking for the
   * signExtendTo32BitAtSource or zeroExtendTo32BitAtSource flags.
   */
  bool isExtendedTo32BitAtSource();

  /** \brief
   *     Determines whether this node should be sign/zero extended to 64-bit at
   * the point of evaluation (source) by checking for the
   * signExtendTo64BitAtSource or zeroExtendTo64BitAtSource flags.
   */
  bool isExtendedTo64BitAtSource();

  /** \brief
   *     Determines whether this node should be sign extended at the point of
   * evaluation (source) by checking for the signExtendTo32BitAtSource or
   * signExtendTo64BitAtSource flags.
   */
  bool isSignExtendedAtSource();

  /** \brief
   *     Determines whether this node should be zero extended at the point of
   * evaluation (source) by checking for the zeroExtendTo32BitAtSource or
   * zeroExtendTo64BitAtSource flags.
   */
  bool isZeroExtendedAtSource();

  /** \brief
   *     Determines whether this node should be sign extended to 32-bits at the
   * point of evaluation (source).
   */
  bool isSignExtendedTo32BitAtSource();

  /** \brief
   *     Determines whether this node should be sign extended to 64-bits at the
   * point of evaluation (source).
   */
  ...
  /** \brief
   *     Determines whether this node should be zero extended to 32-bits at the
   * point of evaluation (source).
   */
  ...
  void rotateChildren(int32_t first,
                      int32_t last);  ///< @note when finsihed, last child ends
                                      ///< up where first child used to be
  ...
  void resetVisitCounts(
      vcount_t t);  ///< reset visit counts on this node and all its children
  ...
  // Decrement the reference count and if it reaches zero decrement the counts
  // of all the children
  ...
  // These three methods should be used only if you're sure you can't use one of
  // the other ones.
  ...
  inline T getConst();
  template <typename T>
  inline T setConst(T t);

  template <class T>
  inline T getIntegerNodeValue();
  ...
  TR_GlobalRegisterNumber setHighGlobalRegisterNumber(
  ...
  // Result of this node is being stored into the same location as its left
  // child
  ...
  // Flag used by TR::ladd and TR::lsub or by TR::lshl and TR::lshr for
  // compressedPointers
  ...
  // Flag used by TR_if or TR::istore/TR::iRegStore or TR::iadd, TR::ladd,
  // TR::isub, TR::lsub
  ...
  // Flags used by aconst and iaload (on s390 iaload can be used after dynamic
  // lit pool)
 ...
 public:
  class ChildIterator {
   ...
   public:
    ...
    void stepForward();
    bool isAt(const ChildIterator &other) {
   ...
   public:  // operators
    ...
    TR::Node *operator->() {
      return currentChild();
    }  // Make a ChildIterator act like a Node pointer
    TR::Node *operator*() {
      return currentChild();
 ...
 protected:
  ...
  // Misc helpers
  TR::Node *duplicateTreeWithCommoningImpl(
      CS2::HashTable<TR::Node *, TR::Node *, TR::Allocator> &nodeMapping);

  bool collectSymbolReferencesInNode(TR_BitVector &symbolReferencesInNode,
                                     vcount_t visitCount);
  ...
  void copyNodeExtension(TR::NodeExtension *other, uint16_t numElems,
                         size_t size);
 ...
 protected:
  ...
  union UnionBase {
    ...
    UnionBase() {
      memset(this, 0,
             sizeof(UnionBase));  // in C++11 notation: _unionBase = {0};
    }
  };

  union UnionPropertyA {
    // these first two TR::SymbolReference fields are aliases, and are not
    // disjoint
    TR::SymbolReference *_symbolReference;  ///< hasSymbolReference()
    TR::SymbolReference
        *_regLoadStoreSymbolReference;  ///< hasRegLoadStoreSymbolReference()
    ...
    UnionPropertyA() {
      memset(
          this, 0,
          sizeof(
              UnionPropertyA));  ///< in C++11 notation: _unionPropertyA = {0};
    }
  };
 ...
 protected:
  enum {
    ...
    invalid8BitGlobalRegister =
        0x00000010,  ///< value is in a global register which cannot be used on
                     ///< an 8 bit instruction
    nodeHasExtension = 0x00000020,
    directMemoryUpdate =
        0x00000040,  ///< Result of this node is being stored into the same
    ...
    arraysetLengthMultipleOfPointerSize =
        0x00020000,  ///< flag used by TR::arrayset to guarantee that checking
                     ...
                     ///< (leading to more efficient code being generated, e.g.
                     ///< REP STOSQ on X86

    // Flags used by TR::bitOpMem
    bitOpMemOPMASK =
        0x00003000,  ///< currently, XOR, AND, OR operations are suppoerted
    ...
    nodeUsedForBranchOnCount =
        0x00000400,  ///< Flag used by TR_if or TR::istore/TR::iRegStore or
                     ///< TR::iadd, TR::ladd, TR::isub, TR::lsub

    dontMoveUnderBranch = 0x00002000,  ///< Flag used by TR::xload/TR::xRegLoad
    nodeCreatedByPRE = 0x00040000,     ///< Flag used by TR_xload

    /** \brief
     *     Represents that a load must be sign/zero extended to a particular
     * width at the point of evaluation.
     *
     *  \details
     *     This flag is used by load nodes to signal the code generator to emit
     * a load and sign/zero extend instructions for the evaluation of this
     * particular load. These flags are used in conjunction with the
     *     unneededConv flag to avoid sign/zero extension conversions which the
     * respective load feeds into.
     */
    ...
    // Flags used by TR_if
    maxLoopIterationGuard =
        0x00000800,  ///< allows redundant async check removal to remove ac's
    ...
    // that the operation should be executed by unsigned instruction in the
    // codegen
    // currently used for ldiv folding to idiv, when both children are with
    // highWordZero
    Unsigned = 0x00004000,

    // Flags used by TR::aconst and TR::iaload (on s390 iaload can be used after
    // dynamic lit pool)
    //
     ...
     *
     *  \details
     *     This flag is used by conversion nodes to signal the code generator
     * that the respective conversion is not needed because the value the
     * conversion is acting on has been cleansed prior to reaching this
     * evaluation point.
     */
    ...
    // TR::instanceof
    referenceIsNonNull =
        0x00008000,  ///< Sometimes we can't mark the child as non-null (because
                     ///< it's not provably non-null everywhere) but we know
                     ///< it's non-null by the time we run this parent
