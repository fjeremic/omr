...
public:
  ...
  static TR::Node* recreateWithSymRef(TR::Node* originalNode,
                                      TR::ILOpCodes op,
  ...
  // copied
  //
  static TR::Node* create(TR::Node* originatingByteCodeNode,
                          TR::ILOpCodes op,
  ...
  static TR::Node* create(TR::ILOpCodes op,
                          uint16_t numChildren,
                          TR::TreeTop* dest);
  static TR::Node* create(TR::ILOpCodes op,
                          uint16_t numChildren,
                          int32_t intValue,
                          TR::TreeTop* dest = 0);
  static TR::Node* createWithSymRef(TR::ILOpCodes op,
                                    uint16_t numChildren,
                                    TR::Node* first,
...
private:
  static TR::Node* recreateWithoutSymRef_va_args(TR::Node* originalNode,
                                                 ...
                                                 va_list& args);
  static TR::Node* createWithoutSymRef(TR::ILOpCodes op,
                                       uint16_t numChildren,
                                       uint16_t numChildArgs,
                                       ...);
...
private:
  ...
  uint16_t addChildrenAndSymRef(uint16_t childIndex,
                                TR::Node* child,
...
public:
  ...
  static TR::Node* createWithRoomForOneMore(TR::ILOpCodes op,
                                            uint16_t numChildren,
                                            ...
                                            TR::Node* fourth = 0,
                                            TR::Node* fifth = 0);
  static TR::Node* createWithRoomForThree(TR::ILOpCodes op,
                                          TR::Node* first,
                                          TR::Node* second,
                                          void* symbolRefOrBranchTarget = 0);
  static TR::Node* createWithRoomForFive(TR::ILOpCodes op,
                                         TR::Node* first,
                                         TR::Node* second,
                                         TR::Node* third,
                                         TR::Node* fourth,
  ...
  static TR::Node* createif(TR::ILOpCodes op,
                            TR::Node* first,
                            TR::Node* second,
                            TR::TreeTop* branchTarget = 0);
  static TR::Node* createbranch(TR::ILOpCodes op,
                                TR::Node* first,
                                TR::TreeTop* branchTarget = 0);
  static TR::Node* createCase(TR::Node* originatingByteCodeNode,
                              TR::TreeTop*,
                              CASECONST_TYPE = 0);

  static TR::Node* createArrayOperation(TR::ILOpCodes arrayOp,
                                        TR::Node* first,
                                        TR::Node* second,
                                        TR::Node* third,
                                        TR::Node* fourth = NULL,
  ...
  static TR::Node* createArraycopy(TR::Node* first,
                                   TR::Node* second,
                                   TR::Node* third);
  static TR::Node* createArraycopy(TR::Node* first,
                                   TR::Node* second,
  ...
  static TR::Node* createStore(TR::SymbolReference* symRef,
                               TR::Node* value,
                               TR::ILOpCodes op);
  static TR::Node* createStore(TR::SymbolReference* symRef,
                               TR::Node* value,
  ...
  static TR::Node* aconst(TR::Node* originatingByteCodeNode,
                          uintptrj_t val,
                          uint8_t precision);
  ...
  static TR::Node* createConstDead(TR::Node* originatingByteCodeNode,
                                   TR::DataType dt,
                                   intptrj_t extraData = 0);

  static TR::Node* createCompressedRefsAnchor(TR::Node* firstChild);

  static TR::Node* createAddConstantToAddress(TR::Node* addr,
                                              intptr_t value,
                                              TR::Node* parent = NULL);
   ...
   */
private:
  void copyChildren(TR::Node* from,
                    uint16_t numChildren = 0,
  ...
  static bool isLegalCallToCreate(TR::ILOpCodes opvalue)
  {
    ...
    TR_ASSERT(opvalue != TR::v2v,
              ...
              opcode.getName());
    TR_ASSERT(opvalue != TR::vconst,
              "use createVectorConst to create node: %s",
              opcode.getName());
    return true;
  }
...
protected:
  ...
  void copyNodeExtension(TR::NodeExtension* other,
                         uint16_t numElems,
                         size_t size);
...
protected:
  ...
  union UnionBase
  {
    ...
    {
      memset(
        this, 0, sizeof(UnionBase)); // in C++11 notation: _unionBase = {0};
    }
  };
  ...
  union UnionPropertyA
  {
    ...
    UnionPropertyA()
    {
      memset(
        this,
        0,
        sizeof(UnionPropertyA)); ///< in C++11 notation: _unionPropertyA = {0};
    }
  };
...
protected:
  enum
  {
    ...
    invalid8BitGlobalRegister =
      0x00000010, ///< value is in a global register which cannot be used on an
                  ///< 8 bit instruction
    ...
    arraysetLengthMultipleOfPointerSize =
      0x00020000, ///< flag used by TR::arrayset to guarantee that checking for
                  ///< lengths smaller than pointer size is not needed (leading
                  ///< to more efficient code being generated, e.g. REP STOSQ on
                  ///< X86
