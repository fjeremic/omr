...
public:
  ...
  static TR::Node* create(
    TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, int32_t intValue, TR::TreeTop* dest = 0);
...
public:
  ...
  TR::ILOpCode& getOpCode()
  {
    return _opCode;
  }
  TR::ILOpCodes getOpCodeValue()
  {
  ...
  uint16_t setNumChildren(uint16_t num)
  {
    return (_numChildren = num);
  }

  ncount_t getGlobalIndex()
  {
    return _globalIndex;
  }
  void setGlobalIndex(ncount_t i)
  {
    _globalIndex = i;
  }
  ...
  bool isPowerOfTwo()
  {
    return _flags.testAny(IsPowerOfTwo);
  }
  void setIsPowerOfTwo(bool b)
  {
    _flags.set(IsPowerOfTwo, b);
  }
  ...
  bool isLoadAndTest()
  {
    return _flags.testAny(loadAndTest);
  }
  void setIsLoadAndTest(bool v)
  {
    _flags.set(loadAndTest, v);
  }
...
protected:
  struct NodeExtensionStore {
    void setExtensionPtr(TR::NodeExtension* ptr)
    {
      _data = ptr;
    }
    TR::NodeExtension* getExtensionPtr()
    {
      return _data;
    }

    void setNumElems(uint16_t numElems)
    {
      _numElems = numElems;
    }
...
protected:
  enum {
    ...
    = 0x00020000, ///< flag used by TR::arrayset to guarantee that checking for lengths smaller than pointer size is not
                  ///< needed (leading to more efficient code being generated, e.g. REP STOSQ on X86
    ...
    referenceIsNonNull = 0x00008000, ///< Sometimes we can't mark the child as non-null (because it's not provably
                                     ///< non-null everywhere) but we know it's non-null by the time we run this parent
