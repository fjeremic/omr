...
TR::Symbol*
OMR::Symbol::create(AllocatorType m)
{
...
template<typename AllocatorType>
TR::Symbol*
OMR::Symbol::create(AllocatorType m, TR::DataType d)
{
...
template<typename AllocatorType>
TR::Symbol*
OMR::Symbol::create(AllocatorType m, TR::DataType d, uint32_t s)
{
...
{
  return self()->isVariableSizeSymbol() &&
         self()->castToVariableSizeSymbol()->isReferenced();
}

bool
OMR::Symbol::dontEliminateStores(TR::Compilation* comp,
                                 bool isForLocalDeadStore)
{
  return (self()->isAuto() && _flags.testAny(PinningArrayPointer)) ||
         ...
         (comp->getSymRefTab()->findThisRangeExtensionSymRef() &&
          (self() ==
           comp->getSymRefTab()->findThisRangeExtensionSymRef()->getSymbol()));
}
...
{
  uint32_t numSlots =
    self()->getRoundedSize() / self()->convertTypeToSize(TR::Address);
...
{
  int32_t roundedSize =
    (int32_t)((self()->getSize() + 3) & (~3)); // cast explicitly
...
template<typename AllocatorType>
TR::Symbol*
OMR::Symbol::createShadow(AllocatorType m)
{
...
template<typename AllocatorType>
TR::Symbol*
OMR::Symbol::createShadow(AllocatorType m, TR::DataType d)
{
...
template<typename AllocatorType>
TR::Symbol*
OMR::Symbol::createShadow(AllocatorType m, TR::DataType d, uint32_t s)
{
...
template<typename AllocatorType>
TR::Symbol*
OMR::Symbol::createNamedShadow(AllocatorType m,
                               TR::DataType d,
                               uint32_t s,
                               char* name)
{
...
template TR::Symbol*
OMR::Symbol::createNamedShadow(TR_StackMemory, TR::DataType, uint32_t, char*);
template TR::Symbol*
OMR::Symbol::createNamedShadow(TR_HeapMemory, TR::DataType, uint32_t, char*);
template TR::Symbol*
OMR::Symbol::createNamedShadow(PERSISTENT_NEW_DECLARE,
                               TR::DataType,
                               uint32_t,
                               char*);
