...
TR::RegisterMappedSymbol *OMR::Symbol::castToRegisterMappedSymbol() {
  TR_ASSERT(self()->isRegisterMappedSymbol(),
            "OMR::Symbol::castToRegisterMappedSymbol, symbol is not a register "
            "mapped symbol");
  return (TR::RegisterMappedSymbol *)this;
}

TR::AutomaticSymbol *OMR::Symbol::castToAutoSymbol() {
  TR_ASSERT(self()->isAuto(),
...
TR::ParameterSymbol *OMR::Symbol::castToParmSymbol() {
  TR_ASSERT(self()->isParm(),
            "OMR::Symbol::castToParmSymbol, symbol is not a parameter symbol");
...
TR::AutomaticSymbol *OMR::Symbol::castToInternalPointerAutoSymbol() {
  TR_ASSERT(self()->isInternalPointerAuto(),
            "OMR::Symbol::castToInternalAutoSymbol, symbol is not an internal "
            "pointer automatic symbol");
  return (TR::AutomaticSymbol *)this;
}

TR::AutomaticSymbol *OMR::Symbol::castToLocalObjectSymbol() {
  TR_ASSERT(self()->isLocalObject(),
            "OMR::Symbol::castToLocalObjectSymbol, symbol is not an internal "
            "pointer automatic symbol");
  return (TR::AutomaticSymbol *)this;
}

TR::StaticSymbol *OMR::Symbol::castToStaticSymbol() {
  TR_ASSERT(self()->isStatic(),
            "OMR::Symbol::castToStaticSymbol, symbol is not a static symbol");
  return (TR::StaticSymbol *)this;
}

TR::StaticSymbol *OMR::Symbol::castToNamedStaticSymbol() {
  TR_ASSERT(self()->isNamed() && self()->isStatic(),
            "OMR::Symbol::castToNamedStaticSymbol, symbol is not a named "
            "static symbol");
  return (TR::StaticSymbol *)this;
}
...
TR::MethodSymbol *OMR::Symbol::castToMethodSymbol() {
  TR_ASSERT(
      self()->isMethod(),
      "OMR::Symbol::castToMethodSymbol, symbol[%p] is not a method symbol",
      this);
...
TR::ResolvedMethodSymbol *OMR::Symbol::castToResolvedMethodSymbol() {
  TR_ASSERT(self()->isResolvedMethod(),
            "OMR::Symbol::castToResolvedMethodSymbol, symbol is not a resolved "
            "method symbol");
  return (TR::ResolvedMethodSymbol *)this;
}

TR::Symbol *OMR::Symbol::castToShadowSymbol() {
  TR_ASSERT(self()->isShadow(),
            "OMR::Symbol::castToShadowSymbol, symbol is not a shadow symbol");
  return (TR::Symbol *)this;
}

TR::RegisterMappedSymbol *OMR::Symbol::castToMethodMetaDataSymbol() {
  TR_ASSERT(self()->isMethodMetaData(),
            "OMR::Symbol::castToMethodMetaDataSymbol, symbol is not a meta "
            "data symbol");
  return (TR::RegisterMappedSymbol *)this;
}

TR::LabelSymbol *OMR::Symbol::castToLabelSymbol() {
  TR_ASSERT(self()->isLabel(),
            "OMR::Symbol::castToLabelSymbol, symbol is not a label symbol");
  return (TR::LabelSymbol *)this;
}

TR::ResolvedMethodSymbol *OMR::Symbol::castToJittedMethodSymbol() {
  TR_ASSERT(self()->isJittedMethod(), "OMR::Symbol::castToJittedMethodSymbol, "
                                      "symbol is not a resolved method symbol");
  return (TR::ResolvedMethodSymbol *)this;
}

TR::StaticSymbol *OMR::Symbol::castToCallSiteTableEntrySymbol() {
  TR_ASSERT(self()->isCallSiteTableEntry(),
            "OMR::Symbol::castToCallSiteTableEntrySymbol expected a call site "
            "table entry symbol");
  return (TR::StaticSymbol *)this;
}
...
TR::StaticSymbol *OMR::Symbol::castToMethodTypeTableEntrySymbol() {
  TR_ASSERT(self()->isMethodTypeTableEntry(),
            "OMR::Symbol::castToMethodTypeTableEntrySymbol expected a method "
            "type table entry symbol");
  return (TR::StaticSymbol *)this;
}

TR::AutomaticSymbol *OMR::Symbol::castToVariableSizeSymbol() {
  TR_ASSERT(self()->isVariableSizeSymbol(),
            "OMR::Symbol::castToVariableSizeSymbol, symbol is not a "
            "VariableSizeSymbol symbol");
  return (TR::AutomaticSymbol *)this;
}
...
bool OMR::Symbol::isCollectedReference() {
  return (self()->getDataType() == TR::Address || self()->isLocalObject()) &&
         !self()->isNotCollected();
}
...
void OMR::Symbol::setSpillTempLoaded() {
  if (self()->isAuto()) // For non auto spills (ie. to original location) we
                        // can't optimize removing spills
...
bool OMR::Symbol::isConstantDynamic() {
...
bool OMR::Symbol::isRecognizedKnownObjectShadow() {
  return self()->isRecognizedShadow() &&
         _flags.testAny(RecognizedKnownObjectShadow);
}
...
bool OMR::Symbol::isNonLinear() {
  return (
      self()->isLabel() &&
      _flags.testValue(OOLMask, (StartOfColdInstructionStream | NonLinear)));
}
...
bool OMR::Symbol::isConstantPoolAddress() {
...
bool OMR::Symbol::isConstObjectRef() {
  return self()->isStatic() &&
         (_flags.testAny(ConstString) ||
          _flags2.testAny(ConstMethodType | ConstMethodHandle |
                          ConstantDynamic));
}

bool OMR::Symbol::isStaticField() {
  return self()->isStatic() &&
         !(self()->isConstObjectRef() || self()->isClassObject() ||
           self()->isAddressOfClassObject() || self()->isConst());
}

bool OMR::Symbol::isFixedObjectRef() {
  return self()->isConstObjectRef() || self()->isCallSiteTableEntry() ||
         self()->isMethodTypeTableEntry();
}
...
TR::RegisterMappedSymbol *OMR::Symbol::getRegisterMappedSymbol() {
  return self()->isRegisterMappedSymbol() ? (TR::RegisterMappedSymbol *)this
                                          : 0;
}
...
TR::StaticSymbol *OMR::Symbol::getCallSiteTableEntrySymbol() {
  return self()->isCallSiteTableEntry()
             ? self()->castToCallSiteTableEntrySymbol()
             : NULL;
}

TR::StaticSymbol *OMR::Symbol::getMethodTypeTableEntrySymbol() {
  return self()->isMethodTypeTableEntry()
             ? self()->castToMethodTypeTableEntrySymbol()
             : NULL;
}
