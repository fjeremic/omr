...
OMR::TreeTop::duplicateTree()
{
  ...
  return self()->create(comp,
                        self()->getNode()->duplicateTree(),
                        self()->getPrevTreeTop(),
                        self()->getNextTreeTop());
}

TR::TreeTop*
OMR::TreeTop::createIncTree(TR::Compilation* comp,
                            TR::Node* node,
...
TR::TreeTop*
OMR::TreeTop::createResetTree(TR::Compilation* comp,
                              TR::Node* node,
                              TR::SymbolReference* symRef,
                              int32_t resetAmount,
                              TR::TreeTop* precedingTreeTop,
                              bool isRecompCounter)
{
  ...
  if (comp->cg()->getAccessStaticsIndirectly() && !symRef->isUnresolved() &&
    ...
    storeNode = TR::Node::createWithSymRef(
      ...
      2,
      loadaddrNode,
      TR::Node::create(node, TR::iconst, 0, resetAmount),
      symRef);
  } else {
    storeNode = TR::Node::createWithSymRef(
      TR::istore,
...
void
OMR::TreeTop::insertTreeTops(TR::Compilation* comp,
...
void
OMR::TreeTop::insertTreeTopsBeforeMe(TR::TreeTop* firstTree,
                                     TR::TreeTop* lastTree)
{
  if (!lastTree)
    lastTree = firstTree;
...
void
OMR::TreeTop::insertTreeTopsAfterMe(TR::TreeTop* firstTree,
                                    TR::TreeTop* lastTree)
{
  if (!lastTree)
    lastTree = firstTree;
...
void
OMR::TreeTop::removeDeadTrees(TR::Compilation* comp,
                              TR::TreeTop* first,
                              TR::TreeTop* last)
{
...
bool
OMR::TreeTop::adjustBranchOrSwitchTreeTop(TR::Compilation* comp,
