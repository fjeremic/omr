...
TR::TreeTop *OMR::TreeTop::create(TR::Compilation *comp, TR::Node *node,
                                  TR::TreeTop *next, TR::TreeTop *prev) {
  bool trace = comp->getOption(TR_TraceCG) || debug("traceGRA");
  return new (trace, comp->trMemory()) TR::TreeTop(node, next, prev);
}

TR::TreeTop *OMR::TreeTop::create(TR::Compilation *comp,
                                  TR::TreeTop *precedingTreeTop,
                                  TR::Node *node) {
  bool trace = comp->getOption(TR_TraceCG) || debug("traceGRA");
  return new (trace, comp->trMemory())
      TR::TreeTop(precedingTreeTop, node, comp);
}

TR::TreeTop *OMR::TreeTop::duplicateTree() {
  TR::Compilation *comp = TR::comp();
  return self()->create(comp, self()->getNode()->duplicateTree(),
                        self()->getPrevTreeTop(), self()->getNextTreeTop());
}

TR::TreeTop *OMR::TreeTop::createIncTree(TR::Compilation *comp, TR::Node *node,
                                         TR::SymbolReference *symRef,
                                         int32_t incAmount,
                                         TR::TreeTop *precedingTreeTop,
                                         bool isRecompCounter) {
  TR::StaticSymbol *symbol = symRef->getSymbol()->castToStaticSymbol();
  TR::DataType type = symbol->getDataType();
  TR::Node *storeNode;
  if (comp->cg()->getAccessStaticsIndirectly() && !symRef->isUnresolved() &&
      type != TR::Address) {
    TR::SymbolReference *symRefShadow;
    if (isRecompCounter)
      symRefShadow =
          comp->getSymRefTab()->findOrCreateCounterAddressSymbolRef();
    else
      symRefShadow = comp->getSymRefTab()->createKnownStaticDataSymbolRef(
          symbol->getStaticAddress(), TR::Address);
    TR::Node *loadaddrNode =
        TR::Node::createWithSymRef(node, TR::loadaddr, 0, symRefShadow);
    storeNode = TR::Node::createWithSymRef(
        TR::istorei, 2, 2, loadaddrNode,
        TR::Node::create(
            TR::iadd, 2,
  ...
  return precedingTreeTop == NULL
             ? TR::TreeTop::create(comp, storeNode)
             : TR::TreeTop::create(comp, precedingTreeTop, storeNode);
}

TR::TreeTop *OMR::TreeTop::createResetTree(
    TR::Compilation *comp, TR::Node *node, TR::SymbolReference *symRef,
  ...
  if (comp->cg()->getAccessStaticsIndirectly() && !symRef->isUnresolved() &&
    ...
    storeNode = TR::Node::createWithSymRef(
        TR::istore, 1, 1, TR::Node::create(node, TR::iconst, 0, resetAmount),
        symRef);
  }
  return precedingTreeTop == NULL
             ? TR::TreeTop::create(comp, storeNode)
...
void OMR::TreeTop::insertTreeTops(TR::Compilation *comp,
                                  TR::TreeTop *beforeInsertionPoint,
                                  TR::TreeTop *firstTree,
                                  TR::TreeTop *lastTree = NULL) {
  if (!lastTree) lastTree = firstTree;
...
void OMR::TreeTop::insertTreeTopsBeforeMe(TR::TreeTop *firstTree,
                                          TR::TreeTop *lastTree) {
  if (!lastTree) lastTree = firstTree;
...
void OMR::TreeTop::insertTreeTopsAfterMe(TR::TreeTop *firstTree,
                                         TR::TreeTop *lastTree) {
  if (!lastTree) lastTree = firstTree;
...
// removeDeadTrees will take a list of trees and 'kill' them by converting the
// children of the tree to be tree-tops themselves. This is handy if you need to
// convert a set of trees to a new set of trees.
...
void OMR::TreeTop::removeDeadTrees(TR::Compilation *comp, TR::TreeTop *first,
                                   TR::TreeTop *last) {
...
void *OMR::TreeTop::operator new(size_t s, bool trace, TR_Memory *m) {
  if (!trace) return m->allocateHeapMemory(s);
...
OMR::TreeTop::TreeTop(TR::TreeTop *precedingTreeTop, TR::Node *node,
                      TR::Compilation *c)
    : _pNode(node) {
...
TR::TreeTop *OMR::TreeTop::getExtendedBlockExitTreeTop() {
  TR_ASSERT(self()->getNode()->getOpCodeValue() == TR::BBStart,
            "getExitTreeTop, is only valid for a bbStart");
  TR::Block *b;
...
bool OMR::TreeTop::adjustBranchOrSwitchTreeTop(TR::Compilation *comp,
                                               TR::TreeTop *oldTargetTree,
  ...
  if (opCode.isBranch()) {
    ...
    for (int i = self()->getNode()->getCaseIndexUpperBound() - 1; i > 0; i--) {
      TR::TreeTop *branchTreeTop =
          self()->getNode()->getChild(i)->getBranchDestination();
      if (branchTreeTop == oldTargetTree) {
        flag = true;
    ...
    // TR_ASSERT(false, "Attempting to adjustBranchOrSwitchTreeTop for multi-way
    // branch (isJumpWithMultipleTargets(node) == true): %p\n", node);
...
// A naive no-aliasing-needed check to see if a treetop has any chance of
// killing anything Used by no and low opt CodeGenPrep phase passes
bool OMR::TreeTop::isPossibleDef() {
  TR::Node *defNode = self()->getNode()->getOpCodeValue() == TR::treetop
                          ? self()->getNode()->getFirstChild()
                          : self()->getNode();
