  ...
  int32_t incAmount, TR::TreeTop* precedingTreeTop, bool isRecompCounter)
{
  ...
  if (comp->cg()->getAccessStaticsIndirectly() && !symRef->isUnresolved() && type != TR::Address) {
    ...
    storeNode = TR::Node::createWithSymRef(TR::istorei, 2, 2, loadaddrNode,
      TR::Node::create(TR::iadd, 2, TR::Node::createWithSymRef(TR::iloadi, 1, 1, loadaddrNode, symRef),
  ...
  int32_t resetAmount, TR::TreeTop* precedingTreeTop, bool isRecompCounter)
{
  ...
  if (comp->cg()->getAccessStaticsIndirectly() && !symRef->isUnresolved() && type != TR::Address) {
    ...
    storeNode = TR::Node::createWithSymRef(
      TR::istorei, 2, 2, loadaddrNode, TR::Node::create(node, TR::iconst, 0, resetAmount), symRef);
  } else {
    storeNode
  ...
  TR::Compilation* comp, TR::TreeTop* oldTargetTree, TR::TreeTop* newTargetTree)
{
  ...
  if (opCode.isBranch()) {
    ...
    // TR_ASSERT(false, "Attempting to adjustBranchOrSwitchTreeTop for multi-way branch (isJumpWithMultipleTargets(node)
    // == true): %p\n", node);
