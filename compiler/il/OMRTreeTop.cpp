...
void OMR::TreeTop::insertTreeTops(TR::Compilation* comp, TR::TreeTop* beforeInsertionPoint, TR::TreeTop* firstTree, TR::TreeTop* lastTree = NULL)
{
    if (!lastTree)
        lastTree = firstTree;
...
bool OMR::TreeTop::adjustBranchOrSwitchTreeTop(TR::Compilation* comp, TR::TreeTop* oldTargetTree, TR::TreeTop* newTargetTree)
{
    TR::Node* node = self()->getNode();
    TR::ILOpCode opCode = self()->getNode()->getOpCode();
    bool flag = false;
