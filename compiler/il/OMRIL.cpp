...
OMR::IL::opCodeForCorrespondingIndirectLoad(TR::ILOpCodes loadOpCode) {
  ...
  case TR::drdbari:
  case TR::ardbari:
    // There is not necessarily a guaranteed symmetry about whether an indirect
    // rdbar should be mapped to an indirect wrtbar or a normal indirect store.
    // The mapping of rdbar/ wrtbar totally depends on the actual use in
    // subprojects and should be undefined in OMR level.
    TR_ASSERT_FATAL(
        0,
...
OMR::IL::opCodeForCorrespondingIndirectStore(TR::ILOpCodes storeOpCode) {
  ...
  case TR::dwrtbari:
    // There is not necessarily a guaranteed symmetry about whether an indirect
    // wrtbar should be mapped to an indirect rdbar or a normal indirect load.
    // The mapping of rdbar/wrtbar totally depends on the actual use in
    // subprojects and should be undefined in OMR level.
    TR_ASSERT_FATAL(
        0,
