...
class NodeExtension {
public:
  NodeExtension(TR_NodeExtAllocator &alloc) : _allocator(alloc){};
  ...
  void operator delete(void *ptr, uint16_t arrayElems, TR_NodeExtAllocator &m) {
    size_t size = sizeof(NodeExtension) +
                  (arrayElems - NUM_DEFAULT_ELEMS) * sizeof(uintptr_t);
  ...
  template <class T> T setElem(uint16_t index, T elem) {
