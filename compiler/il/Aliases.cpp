...
#ifdef J9_PROJECT_SPECIFIC
static TR_BitVector *addVeryRefinedCallAliasSets(TR::ResolvedMethodSymbol *,
                                                 TR_BitVector *, List<void> *);
#endif

OMR::SymbolReference::SymbolReference(
    TR::SymbolReferenceTable *symRefTab, TR::SymbolReference &sr, intptrj_t o,
...
void OMR::SymbolReference::copyAliasSets(TR::SymbolReference *fromSymRef,
                                         TR::SymbolReferenceTable *symRefTab) {
  _useDefAliases = fromSymRef->_useDefAliases;
}

TR_BitVector *OMR::SymbolReference::getUseonlyAliasesBV(
    TR::SymbolReferenceTable *symRefTab) {
  ...
  switch (kind) {
    case TR::Symbol::IsMethod: {
      ...
      // A potentialOSRPointHelper call is an exception point that may go to OSR
      // catch block ( see Node API exceptionsRaised), the control flow
      // constraint imposed by the exception edge will apply to all the global
      // optimizations that may move things around. Local optimizations also ask
      // exceptionsRaised to determine if a code motion across certain point is
      // safe. So aliasing is not necessary. However, we'd like to add aliasing
      // here to cause the compiler to be more conservative about reordering
      // this helper with other operations. The aliasing can always be relaxed
      // when necessary.
      //
      if (symRefTab->isNonHelper(
              self(),
      ...
      // Preventing the reordering of fear point helper w.r.t. OSR points and
      // yield/invalidation points is the minimum requirement of aliasing for
      // OSR fear point helper. These reorderings would in almost all cases be
      // naturally disallowed simply due to the fact that the fear point is
      // represented as a call, which even without aliasing could e.g. perform
      // I/O. Thus the following is a highly conservative aliasing and can be
      // relaxed later when necessary
      //
      if (symRefTab->isNonHelper(
              self(), TR::SymbolReferenceTable::osrFearPointHelperSymbol)) {
        return &symRefTab->aliasBuilder.defaultMethodUseAliases();
      }
    ...
    case TR::Symbol::IsResolvedMethod: {
      TR::ResolvedMethodSymbol *resolvedMethodSymbol =
          _symbol->castToResolvedMethodSymbol();
    ...
    case TR::Symbol::IsParameter:
      ...
      if (symRefTab->aliasBuilder.catchLocalUseSymRefs().isSet(
...
TR_BitVector *OMR::SymbolReference::getUseDefAliasesBV(
  ...
  if (!self()->reallySharesSymbol(comp)) {
    switch (kind) {
      ...
      case TR::Symbol::IsStatic: {
        // For unresolved constant dynamic, we need to invoke a Java bootstrap
        // method, which can have arbitrary side effects, so the aliasing should
        // be conservative here. isConstObjectRef now returns true for condy, so
        // we add an explicit condition, more like a short-circuit, to say if we
        // are unresolved and not isConstObjectRef (this is the same as before),
        // or if we are unresolved and condy (this is the extra condition
        // added), we would return conservative aliases.
        if ((self()->isUnresolved() &&
             (_symbol->isConstantDynamic() || !_symbol->isConstObjectRef())) ||
        ...
        } else if (!symRefTab->aliasBuilder
                        .mutableGenericIntShadowHasBeenCreated()) {
          // getUseDefAliases must return NULL
          return NULL;
        } else if (kind == TR::Symbol::IsStatic &&
                   !symRefTab->aliasBuilder
                        .litPoolGenericIntShadowHasBeenCreated()) {
          // getUseDefAliases must return NULL
          return NULL;
        }
        break;
  ...
  switch (kind) {
    case TR::Symbol::IsMethod: {
      ...
      if (symRefTab->isNonHelper(self(),
                                 TR::SymbolReferenceTable::arraySetSymbol) ||
          symRefTab->isNonHelper(
              self(), TR::SymbolReferenceTable::osrFearPointHelperSymbol) ||
          symRefTab->isNonHelper(
              self(),
              TR::SymbolReferenceTable::potentialOSRPointHelperSymbol)) {
      ...
      if (symRefTab->isNonHelper(self(),
                                 TR::SymbolReferenceTable::arrayCmpSymbol))
        return 0;

      switch (self()->getReferenceNumber()) {
        case TR_methodTypeCheck:
        case TR_nullCheck:
          return &symRefTab->aliasBuilder
                      .defaultMethodDefAliasesWithoutImmutable();
        ...
        case TR_newArray:
        case TR_multiANewArray:
          if ((comp->generateArraylets() || comp->isDLT()) &&
              includeGCSafePoint)
    ...
    case TR::Symbol::IsResolvedMethod: {
      ...
      if (!comp->getOption(TR_EnableHCR)) {
        switch (resolvedMethodSymbol->getRecognizedMethod()) {
          ...
          case TR::java_lang_System_arraycopy: {
            TR_BitVector *aliases = new (aliasRegion)
                TR_BitVector(bvInitialSize, aliasRegion, growability);
            *aliases |= symRefTab->aliasBuilder.arrayElementSymRefs();
            ...
            if (resolvedMethodSymbol->isPureFunction()) return NULL;
          ...
          case TR::com_ibm_dataaccess_DecimalData_setflags:
            if (!(
                    ...
                    TR::Compiler->target.cpu
                        .getSupportsDecimalFloatingPointFacility() ||
#endif
      ...
      TR_PersistentMethodInfo *methodInfo =
          comp->getRecompilationInfo() ? TR_PersistentMethodInfo::get(method)
                                       : NULL;
      if (methodInfo &&
          (methodInfo->hasRefinedAliasSets() ||
        ...
        if (methodInfo->doesntKillAnything() &&
        ...
        if ((resolvedMethodSymbol->hasVeryRefinedAliasSets() ||
          ...
          if (resolvedMethodSymbol->hasVeryRefinedAliasSets())
            exactAliases = symRefTab->aliasBuilder.getVeryRefinedCallAliasSets(
            ...
            exactAliases = addVeryRefinedCallAliasSets(resolvedMethodSymbol,
                                                       aliases, &methodsPeeked);
            symRefTab->aliasBuilder.setVeryRefinedCallAliasSets(
                resolvedMethodSymbol, exactAliases);
          }
          if (exactAliases) {
            return exactAliases;
          }
    ...
    case TR::Symbol::IsShadow: {
      if ((self()->isUnresolved() && !_symbol->isConstObjectRef()) ||
          _symbol->isVolatile() || self()->isLiteralPoolAddress() ||
          self()->isFromLiteralPool() ||
          (_symbol->isUnsafeShadowSymbol() && !self()->reallySharesSymbol())) {
        if (symRefTab->aliasBuilder.unsafeArrayElementSymRefs().get(
                self()->getReferenceNumber())) {
          TR_BitVector *aliases = new (aliasRegion)
              TR_BitVector(bvInitialSize, aliasRegion, growability);
          *aliases |=
              comp->getSymRefTab()
                  ->aliasBuilder.defaultMethodDefAliasesWithoutImmutable();
          *aliases -= symRefTab->aliasBuilder.cpSymRefs();
          return aliases;
        } else
          return &comp->getSymRefTab()
                      ->aliasBuilder.defaultMethodDefAliasesWithoutImmutable();
      }

      TR_BitVector *aliases = NULL;
      if (_symbol == symRefTab->findGenericIntShadowSymbol()) {
        aliases = new (aliasRegion)
            TR_BitVector(bvInitialSize, aliasRegion, growability);
        *aliases |= symRefTab->aliasBuilder.arrayElementSymRefs();
        if (comp->generateArraylets())
          *aliases |= symRefTab->aliasBuilder.arrayletElementSymRefs();
      ...
      if (_symbol->isArrayShadowSymbol() &&
          _symbol->getDataType() == TR::PackedDecimal) {
        if (!aliases)
          aliases = new (aliasRegion)
              TR_BitVector(bvInitialSize, aliasRegion, growability);
      ...
      if (_symbol->isArrayShadowSymbol() && _symbol->getDataType().isVector()) {
        if (!aliases)
          aliases = new (aliasRegion)
              TR_BitVector(bvInitialSize, aliasRegion, growability);
        aliases->set(symRefTab->getArrayShadowIndex(
            _symbol->getDataType().vectorToScalar()));
      }
      // the other way around
      if (_symbol->isArrayShadowSymbol() &&
          !_symbol->getDataType().isVector()) {
        if (!aliases)
          aliases = new (aliasRegion)
              TR_BitVector(bvInitialSize, aliasRegion, growability);
        aliases->set(symRefTab->getArrayShadowIndex(
            _symbol->getDataType().scalarToVector()));
      }
      ...
      if (_symbol->isArrayShadowSymbol() &&
        ...
        while (bvi.hasMoreElements()) {
          ...
          if (symRefTab->getSymRef(symRefNum)->getSymbol()->getDataType() ==
              type)
      ...
      if (aliases) aliases->set(self()->getReferenceNumber());

      if (symRefTab->aliasBuilder.unsafeArrayElementSymRefs().get(
              self()->getReferenceNumber()))
        *aliases -= symRefTab->aliasBuilder.cpSymRefs();
      else if (symRefTab->aliasBuilder.cpSymRefs().get(
                   self()->getReferenceNumber()))
    ...
    case TR::Symbol::IsStatic: {
      ...
      // we add an explicit condition, more like a short-circuit, to say if we
      // are unresolved and not isConstObjectRef (this is the same as before),
      // or if we are unresolved and condy (this is the extra condition added),
      // we would return conservative aliases.
      if ((self()->isUnresolved() &&
           (_symbol->isConstantDynamic() || !_symbol->isConstObjectRef())) ||
    ...
    default:
      ...
      if (comp->generateArraylets() &&
          comp->getSymRefTab()->aliasBuilder.gcSafePointSymRefNumbers().get(
              self()->getReferenceNumber()) &&
          includeGCSafePoint)
...
bool OMR::SymbolReference::sharesSymbol(bool includingGCSafePoint) {
  ...
  if (self()->reallySharesSymbol(c)) return true;
  ...
  switch (kind) {
    ...
    case TR::Symbol::IsStatic: {
      ...
      if ((self()->isUnresolved() &&
          ...
          self()->isFromLiteralPool() || _symbol->isUnsafeShadowSymbol() ||
          (_symbol->isArrayShadowSymbol() &&
           c->getMethodSymbol()->hasVeryRefinedAliasSets())) {
        // getUseDefAliases might not return NULL
      } else if (!symRefTab->aliasBuilder
                      .mutableGenericIntShadowHasBeenCreated()) {
        // getUseDefAliases must return NULL
...
void OMR::SymbolReference::setLiteralPoolAliases(
    TR_BitVector *aliases, TR::SymbolReferenceTable *symRefTab) {
  if (!symRefTab->findGenericIntShadowSymbol()) return;

  TR_SymRefIterator i(symRefTab->aliasBuilder.genericIntShadowSymRefs(),
                      symRefTab);
...
void OMR::SymbolReference::setSharedShadowAliases(
    TR_BitVector *aliases, TR::SymbolReferenceTable *symRefTab) {
  if (self()->reallySharesSymbol() && !_symbol->isUnsafeShadowSymbol()) {
    ...
    TR_SymRefIterator i(
        type.isAddress()
            ...
            : (type.isInt32()
                   ? symRefTab->aliasBuilder.intShadowSymRefs()
...
void OMR::SymbolReference::setSharedStaticAliases(
    TR_BitVector *aliases, TR::SymbolReferenceTable *symRefTab) {
  if (self()->reallySharesSymbol()) {
    ...
    TR_SymRefIterator i(
        type.isAddress()
            ...
            : (type.isInt32()
                   ? symRefTab->aliasBuilder.intStaticSymRefs()
...
TR_BitVector *addVeryRefinedCallAliasSets(
    TR::ResolvedMethodSymbol *methodSymbol, TR_BitVector *aliases,
  ...
  if (methodsPeeked->find(methodId)) {
    // This can't be allocated into the alias region as it must be accessed
    // across optimizations
    TR_BitVector *heapAliases = new (comp->trHeapMemory()) TR_BitVector(
        comp->getSymRefCount(), comp->trMemory(), heapAlloc, growable);
    *heapAliases |= *aliases;
    return heapAliases;
  }
  ...
  //
  if (methodsPeeked->getSize() >= PEEK_THRESHOLD) return 0;

  methodsPeeked->add(methodId);

  dumpOptDetails(
      comp, "O^O REFINING ALIASES: Peeking into the IL to refine aliases \n");

  if (!methodSymbol->getResolvedMethod()->genMethodILForPeeking(methodSymbol,
                                                                comp, true))
    return 0;
  ...
  for (TR::TreeTop *tt = methodSymbol->getFirstTreeTop(); tt;
    ...
    if (node->getOpCode().isResolveCheck()) return 0;
    ...
    if (node->getOpCode().isStore()) {
      TR::SymbolReference *symRefInCallee = node->getSymbolReference(),
                          *symRefInCaller;
      ...
      if (symInCallee->isShadow()) {
        if (symInCallee->isArrayShadowSymbol())
          symRefInCaller =
              symRefTab->getSymRef(symRefTab->getArrayShadowIndex(type));

        else if (symInCallee->isArrayletShadowSymbol())
          symRefInCaller =
              symRefTab->getSymRef(symRefTab->getArrayletShadowIndex(type));

        else
          symRefInCaller =
              symRefTab->findShadowSymbol(symRefInCallee->getOwningMethod(comp),
        ...
        symRefInCaller =
            symRefTab->findStaticSymbol(symRefInCallee->getOwningMethod(comp),
                                        symRefInCallee->getCPIndex(), type);
        if (symRefInCaller) {
          if (symRefInCaller->reallySharesSymbol(comp))
  ...
  // This can't be allocated into the alias region as it must be accessed across
  // optimizations
  TR_BitVector *heapAliases = new (comp->trHeapMemory()) TR_BitVector(
      comp->getSymRefCount(), comp->trMemory(), heapAlloc, growable);
  *heapAliases |= *aliases;
...
bool OMR::SymbolReference::willUse(TR::SymbolReference *sr2,
                                   TR::SymbolReferenceTable *symRefTab) {
  if (self()->getSymbol() == sr2->getSymbol()) return true;
...
bool OMR::SymbolReference::canKill(TR::SymbolReference *sr2) {
  TR::Compilation *comp = TR::comp();
  if (self()->getSymbol() == sr2->getSymbol()) return true;
  if (!self()->sharesSymbol()) return false;
...
bool OMR::SymbolReference::storeCanBeRemoved() {
  ...
  return !s->isVolatile() &&
         (((s->getDataType() != TR::Double) &&
           (s->getDataType() != TR::Float)) ||
          comp->cg()->getSupportsJavaFloatSemantics() ||
          (self()->isTemporary(comp) && !s->behaveLikeNonTemp()));
}

char *classNameToSignature(const char *name, int32_t &len,
                           TR::Compilation *comp, TR_AllocationKind allocKind) {
  char *sig;
...
const char *OMR::SymbolReference::getTypeSignature(int32_t &len,
  ...
  switch (_symbol->getKind()) {
    ...
    case TR::Symbol::IsShadow:
      return (_cpIndex > 0 ? self()->getOwningMethod(comp)->fieldSignatureChars(
                                 _cpIndex, len)
                           : 0);
...
/**
 * Copies the input symref reference number into this symref if such a copy is
 * possible
 *
 ...
 */
void OMR::SymbolReference::copyRefNumIfPossible(
    TR::SymbolReference *symRef, TR::SymbolReferenceTable *symRefTab) {
  int32_t index = symRef->getReferenceNumber();
  if ((index < symRefTab->getNonhelperIndex(
                   symRefTab->getLastCommonNonhelperSymbol())) ||
 ...
 */
void OMR::SymbolReference::makeIndependent(TR::SymbolReferenceTable *symRefTab,
                                           TR::SymbolReference *symRef) {
  TR::Compilation *comp = symRefTab->comp();
  TR_ASSERT(self()->getSymbol()->isArrayShadowSymbol(),
            "symref #%d is not an array shadow\n",
            self()->getReferenceNumber());
  TR_ASSERT(symRef->getSymbol()->isArrayShadowSymbol(),
            "symref #%d is not an array shadow\n",
  ...
  if (NULL == symRef->getIndependentSymRefs())
    symRef->setIndependentSymRefs(new (comp->trHeapMemory()) TR_BitVector(
        symRefTab->getNumSymRefs(), comp->trMemory(), heapAlloc, growable));
...
void OMR::SymbolReference::setAliasedTo(TR_BitVector &bv,
                                        TR::SymbolReferenceTable *symRefTab,
                                        bool symmetric) {
  TR::Compilation *comp = symRefTab->comp();

  TR_ASSERT(_useDefAliases,
            "this symref doesn't have its own aliasing bitvector");
  if (!symmetric) {
    *_useDefAliases |= bv;
  } else {
    // we must process one by one to ensure symmetric aliasing
    TR_SymRefIterator sit(bv, symRefTab);
    for (TR::SymbolReference *symRef = sit.getNext(); symRef;
         symRef = sit.getNext()) {
