...
OMR::SymbolReference::getUseDefAliasesBV(bool isDirectCall, bool includeGCSafePoint)
{
    ...
    if (!self()->reallySharesSymbol(comp)) {
        ...
        case TR::Symbol::IsStatic: {
            ...
            if ((self()->isUnresolved() && (_symbol->isConstantDynamic() || !_symbol->isConstObjectRef())) || _symbol->isVolatile() || self()->isLiteralPoolAddress() || self()->isFromLiteralPool() || _symbol->isUnsafeShadowSymbol() || (_symbol->isArrayShadowSymbol() && comp->getMethodSymbol()->hasVeryRefinedAliasSets())) {
    ...
    case TR::Symbol::IsMethod: {
        ...
        if (symRefTab->isNonHelper(self(), TR::SymbolReferenceTable::arraySetSymbol) || symRefTab->isNonHelper(self(), TR::SymbolReferenceTable::osrFearPointHelperSymbol) || symRefTab->isNonHelper(self(), TR::SymbolReferenceTable::potentialOSRPointHelperSymbol)) {
    ...
    case TR::Symbol::IsResolvedMethod: {
        ...
        if (methodInfo && (methodInfo->hasRefinedAliasSets() || comp->getMethodHotness() >= veryHot || resolvedMethodSymbol->hasVeryRefinedAliasSets()) && (method->isStatic() || method->isFinal() || isDirectCall)) {
            TR_BitVector* aliases = new (aliasRegion) TR_BitVector(bvInitialSize, aliasRegion, growability);
            if ((comp->generateArraylets() || comp->isDLT()) && includeGCSafePoint)
            ...
            if ((resolvedMethodSymbol->hasVeryRefinedAliasSets() || comp->getMethodHotness() >= hot) && !debug("disableVeryRefinedCallAliasSets")) {
    ...
    case TR::Symbol::IsShadow: {
        if ((self()->isUnresolved() && !_symbol->isConstObjectRef()) || _symbol->isVolatile() || self()->isLiteralPoolAddress() || self()->isFromLiteralPool() || (_symbol->isUnsafeShadowSymbol() && !self()->reallySharesSymbol())) {
        ...
        if (_symbol->isArrayShadowSymbol() && supportArrayRefinement && comp->getMethodSymbol()->hasVeryRefinedAliasSets()) {
            if (!aliases)
                aliases = new (aliasRegion) TR_BitVector(bvInitialSize, aliasRegion, growability);
    ...
    case TR::Symbol::IsStatic: {
        ...
        // (this is the same as before), or if we are unresolved and condy
        // (this is the extra condition added), we would return conservative aliases.
        if ((self()->isUnresolved() && (_symbol->isConstantDynamic() || !_symbol->isConstObjectRef())) || self()->isLiteralPoolAddress() || self()->isFromLiteralPool() || _symbol->isVolatile()) {
...
bool OMR::SymbolReference::sharesSymbol(bool includingGCSafePoint)
{
    ...
    case TR::Symbol::IsStatic: {
        ...
        // (this is the same as before), or if we are unresolved and condy
        // (this is the extra condition added), we would return conservative aliases.
        if ((self()->isUnresolved() && (_symbol->isConstantDynamic() || !_symbol->isConstObjectRef())) || _symbol->isVolatile() || self()->isLiteralPoolAddress() || self()->isFromLiteralPool() || _symbol->isUnsafeShadowSymbol() || (_symbol->isArrayShadowSymbol() && c->getMethodSymbol()->hasVeryRefinedAliasSets())) {
            // getUseDefAliases might not return NULL
        } else if (!symRefTab->aliasBuilder.mutableGenericIntShadowHasBeenCreated()) {
            // getUseDefAliases must return NULL
            return false;
...
addVeryRefinedCallAliasSets(TR::ResolvedMethodSymbol* methodSymbol, TR_BitVector* aliases, List<void>* methodsPeeked)
{
    ...
    for (TR::TreeTop* tt = methodSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if ((node->getOpCodeValue() == TR::treetop) || (node->getOpCodeValue() == TR::compressedRefs) || node->getOpCode().isCheck())
...
bool OMR::SymbolReference::storeCanBeRemoved()
{
    ...
    return !s->isVolatile() && (((s->getDataType() != TR::Double) && (s->getDataType() != TR::Float)) || comp->cg()->getSupportsJavaFloatSemantics() || (self()->isTemporary(comp) && !s->behaveLikeNonTemp()));
}

char* classNameToSignature(const char* name, int32_t& len, TR::Compilation* comp, TR_AllocationKind allocKind)
{
    char* sig;
...
void OMR::SymbolReference::copyRefNumIfPossible(TR::SymbolReference* symRef, TR::SymbolReferenceTable* symRefTab)
{
    ...
    if ((index < symRefTab->getNonhelperIndex(symRefTab->getLastCommonNonhelperSymbol())) || (self()->getSymbol() == symRefTab->findGenericIntShadowSymbol()) || symRef->getSymbol()->isUnsafeShadowSymbol()) {
