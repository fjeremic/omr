...
OMR::SymbolReference::SymbolReference(TR::SymbolReferenceTable* symRefTab, TR::SymbolReference& sr, intptrj_t o,
    ...
    if (node->getOpCode().isStore()) {
      ...
      if (symInCallee->isShadow()) {
        if (symInCallee->isArrayShadowSymbol())
          ...
          symRefInCaller
            = symRefTab->findShadowSymbol(symRefInCallee->getOwningMethod(comp), symRefInCallee->getCPIndex(), type);
        ...
        symRefInCaller
          = symRefTab->findStaticSymbol(symRefInCallee->getOwningMethod(comp), symRefInCallee->getCPIndex(), type);
        if (symRefInCaller) {
          if (symRefInCaller->reallySharesSymbol(comp))
...
void OMR::SymbolReference::makeIndependent(TR::SymbolReferenceTable* symRefTab, TR::SymbolReference* symRef)
{
  ...
  TR_ASSERT(
    self()->getSymbol()->isArrayShadowSymbol(), "symref #%d is not an array shadow\n", self()->getReferenceNumber());
  TR_ASSERT(
    symRef->getSymbol()->isArrayShadowSymbol(), "symref #%d is not an array shadow\n", symRef->getReferenceNumber());
