...
OMR::SymbolReference::getUseonlyAliasesBV(TR::SymbolReferenceTable *symRefTab) {
  ...
  case TR::Symbol::IsMethod: {
    ...
    // catch block ( see Node API exceptionsRaised), the control flow constraint
    // imposed by the exception edge will apply to all the global optimizations
    // that may move things around. Local optimizations also ask
    ...
    // here to cause the compiler to be more conservative about reordering this
    // helper with other operations. The aliasing can always be relaxed when
    // necessary.
    //
    if (symRefTab->isNonHelper(
            self(), TR::SymbolReferenceTable::potentialOSRPointHelperSymbol)) {
    ...
    // yield/invalidation points is the minimum requirement of aliasing for OSR
    // fear point helper. These reorderings would in almost all cases be
...
TR_BitVector *
OMR::SymbolReference::getUseDefAliasesBV(bool isDirectCall,
                                         bool includeGCSafePoint) {
  ...
  case TR::Symbol::IsStatic: {
    // For unresolved constant dynamic, we need to invoke a Java bootstrap
    // method, which can have arbitrary side effects, so the aliasing should be
    // conservative here. isConstObjectRef now returns true for condy, so we add
    // an explicit condition, more like a short-circuit, to say if we are
    // unresolved and not isConstObjectRef (this is the same as before), or if
    // we are unresolved and condy (this is the extra condition added), we would
    // return conservative aliases.
    if ((self()->isUnresolved() &&
         (_symbol->isConstantDynamic() || !_symbol->isConstObjectRef())) ||
...
#ifdef J9_PROJECT_SPECIFIC
TR_BitVector *
addVeryRefinedCallAliasSets(TR::ResolvedMethodSymbol *methodSymbol,
                            TR_BitVector *aliases, List<void> *methodsPeeked) {
  TR::Compilation *comp = TR::comp();
