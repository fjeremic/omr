...
#ifdef J9_PROJECT_SPECIFIC
static TR_BitVector*
addVeryRefinedCallAliasSets(TR::ResolvedMethodSymbol*,
                            TR_BitVector*,
                            List<void>*);
#endif
...
void
OMR::SymbolReference::copyAliasSets(TR::SymbolReference* fromSymRef,
                                    TR::SymbolReferenceTable* symRefTab)
{
      ...
      if (methodInfo &&
        ...
        if ((resolvedMethodSymbol->hasVeryRefinedAliasSets() ||
          ...
          if (resolvedMethodSymbol->hasVeryRefinedAliasSets())
            ...
            exactAliases = addVeryRefinedCallAliasSets(
              resolvedMethodSymbol, aliases, &methodsPeeked);
...
void
OMR::SymbolReference::setLiteralPoolAliases(TR_BitVector* aliases,
                                            TR::SymbolReferenceTable* symRefTab)
{
  if (!symRefTab->findGenericIntShadowSymbol())
    return;
...
void
OMR::SymbolReference::setSharedShadowAliases(
  TR_BitVector* aliases,
  TR::SymbolReferenceTable* symRefTab)
{
...
void
OMR::SymbolReference::setSharedStaticAliases(
  TR_BitVector* aliases,
  TR::SymbolReferenceTable* symRefTab)
{
                            ...
                            List<void>* methodsPeeked)
{
  ...
  if (!methodSymbol->getResolvedMethod()->genMethodILForPeeking(
        methodSymbol, comp, true))
    return 0;
  ...
  // optimizations
  TR_BitVector* heapAliases = new (comp->trHeapMemory())
    TR_BitVector(comp->getSymRefCount(), comp->trMemory(), heapAlloc, growable);
  *heapAliases |= *aliases;
  return heapAliases;
}
#endif
...
bool
OMR::SymbolReference::willUse(TR::SymbolReference* sr2,
                              TR::SymbolReferenceTable* symRefTab)
{
...
char*
classNameToSignature(const char* name,
                     int32_t& len,
                     TR::Compilation* comp,
...
const char*
OMR::SymbolReference::getTypeSignature(int32_t& len,
                                       TR_AllocationKind allocKind,
                                       bool* isFixed)
{
  TR::Compilation* comp = TR::comp();
 ...
 */
void
OMR::SymbolReference::copyRefNumIfPossible(TR::SymbolReference* symRef,
                                           TR::SymbolReferenceTable* symRefTab)
{
...
void
OMR::SymbolReference::setAliasedTo(TR_BitVector& bv,
                                   TR::SymbolReferenceTable* symRefTab,
                                   bool symmetric)
{
  TR::Compilation* comp = symRefTab->comp();
