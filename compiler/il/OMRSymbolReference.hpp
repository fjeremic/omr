...
public:
  ...
  void init(TR::SymbolReferenceTable *symRefTab, uint32_t refNumber,
            TR::Symbol *sym = 0, intptrj_t offset = 0,
            mcount_t owningMethodIndex = JITTED_METHOD_INDEX,
            int32_t cpIndex = -1, int32_t unresolvedIndex = 0,
  ...
  SymbolReference(TR::SymbolReferenceTable *symRefTab, TR::Symbol *symbol,
                  intptrj_t offset = 0);

  SymbolReference(TR::SymbolReferenceTable *symRefTab, int32_t refNumber,
  ...
  SymbolReference(TR::SymbolReferenceTable *symRefTab, TR::Symbol *sym,
  ...
  SymbolReference(TR::SymbolReferenceTable *symRefTab, TR::SymbolReference &sr,
                  intptrj_t offset,
                  TR::KnownObjectTable::Index knownObjectIndex =
                      TR::KnownObjectTable::UNKNOWN);
  ...
  static TR::SymbolReference *create(TR::SymbolReferenceTable *symRefTab,
                                     TR::Symbol *sym,
  ...
  getUseDefAliasesIncludingGCSafePoint(bool isDirectCall = false);
  ...
  void copyAliasSets(TR::SymbolReference *,
                     TR::SymbolReferenceTable *symRefTab);

  void setLiteralPoolAliases(TR_BitVector *,
                             TR::SymbolReferenceTable *symRefTab);
  void setSharedShadowAliases(TR_BitVector *,
                              TR::SymbolReferenceTable *symRefTab);
  void setSharedStaticAliases(TR_BitVector *,
                              TR::SymbolReferenceTable *symRefTab);
  ...
  const char *getTypeSignature(int32_t &len, TR_AllocationKind = stackAlloc,
                               bool *isFixed = NULL);

  void copyRefNumIfPossible(TR::SymbolReference *sr,
                            TR::SymbolReferenceTable *symRefTab);
  ...
  virtual void setAliasedTo(TR_BitVector &bv,
                            TR::SymbolReferenceTable *symRefTab,
                            bool symmetric);
...
protected:
   ...
   */
  SymbolReference(TR::SymbolReferenceTable *symRefTab, TR::Symbol *symbol,
                  intptrj_t offset, const char *name);

  friend class ::TR_Debug;

  template <uint32_t> friend class ::TR_NodeAliasSetInterface;

  template <uint32_t> friend class ::TR_SymAliasSetInterface;

  TR_BitVector *getUseonlyAliasesBV(TR::SymbolReferenceTable *symRefTab);
  TR_BitVector *getUseDefAliasesBV(bool isDirectCall = false,
                                   bool gcSafe = false);
  ...
  TR_ExtraInfoForNew
      *_extraInfo; ///< Extra info pointer used for some SymbolReferences
...
char *classNameToSignature(const char *, int32_t &len, TR::Compilation *,
                           TR_AllocationKind = stackAlloc);
