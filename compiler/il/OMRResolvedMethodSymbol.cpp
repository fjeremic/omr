  ...
  , _bytecodeProfilingOffsets(comp->allocator())
{
  ...
  if ((_methodIndex > JITTED_METHOD_INDEX
    ...
    || comp->isDLT() || (comp->getOption(TR_UseSymbolValidationManager) && comp->compileRelocatableCode())) {
  ...
  while (p) {
    ...
    *q = '\0';
    ...
    traceMsg(comp, "signature: %s, callSiteInsertionPoint: %d, bcIndexInsertionPoint: %x\n", mSignature,
      (callSiteInsertionPoint ? *callSiteInsertionPoint : -1), (bcIndexInsertionPoint ? *bcIndexInsertionPoint : -1));
    break;
  }
}
  ...
  if (stashedArgs) {
    for (int32_t i = 0; i < numChildren; ++i)
      induceOSRCallNode->setAndIncChild(
        i, TR::Node::createLoad(induceOSRCallNode, self()->comp()->getSymRefTab()->getSymRef((*stashedArgs)[i])));
  ...
  TR::TreeTop* branch, bool extendRemainder, int32_t offset, TR::TreeTop** lastTreeTop)
{
  ...
  TR_ASSERT_FATAL(induceOSRCallNode->getOpCode().isCall() && symRef->getReferenceNumber() == TR_induceOSRAtCurrentPC,
    "induceOSRCallNode %p (n%dn) under induceOSRCallTree %p should be a call node with TR_induceOSRAtCurrentPC helper "
    "call",
  ...
  TR::TreeTop* branch, bool extendRemainder, int32_t offset, TR::TreeTop** lastTreeTop)
{
  ...
  if (self()->supportsInduceOSR(induceBCI, block, self()->comp())) {
    ...
    if (self()->comp()->getOption(TR_TraceOSR))
      traceMsg(
        self()->comp(), "  Created OSR block_%d and inserting it at the end of the method\n", osrBlock->getNumber());
  ...
  TR_OSRMethodData* osrMethodData, int32_t numChildren, bool copyChildren, bool shouldSplitBlock, TR::CFG* callerCFG)
{
  ...
  if (self()->comp()->getOption(TR_TraceOSR))
    traceMsg(self()->comp(), "Osr point added for %p, callerIndex=%d, bcindex=%d\n", insertionPointNode,
      insertionPointNode->getByteCodeInfo().getCallerIndex(), insertionPointNode->getByteCodeInfo().getByteCodeIndex());
  ...
  // shouldn't have been able to induce without an OSR point in the first place. This should be removed, as well as the
  // genOSRHelperCall's cfg argument.
  if (firstOSRPoint) {
  ...
  for (TR::TreeTop* tt = self()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
      ...
      if (matchCode > 0) {
        ...
        if (trace) {
          ...
          traceMsg(self()->comp(), "fake induce %p generated for %s at callsite %d bytecode %x\n", ttnode, mSignature,
            callSiteInsertionPoint, bcIndexInsertionPoint);
        }
        skipNextTT = true;
      }
    } else
    ...
    if (self()->comp()->isPotentialOSRPoint(tt->getNode())) {
      ...
      if (self()->comp()->isOSRTransitionTarget(TR::postExecutionOSR)) {
        ...
        osrPoint
          = new (self()->comp()->trHeapMemory()) TR_OSRPoint(offsetBCI, osrMethodData, self()->comp()->trMemory());
        osrPoint->setOSRIndex(self()->addOSRPoint(osrPoint));
        if (self()->comp()->getOption(TR_TraceOSR))
  ...
  int32_t currentInlinedSiteIndex, TR::SymbolReferenceTable* symRefTab, TR::CFG* cfg)
{
  ...
  TR::Node* vmThread = TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0,
    new (self()->comp()->trHeapMemory()) TR::SymbolReference(
      symRefTab, TR::RegisterMappedSymbol::createMethodMetaDataSymbol(self()->comp()->trHeapMemory(), "vmThread")));
  ...
  if (self()->comp()->getCurrentInlinedSiteIndex() == -1) {
    ...
    if (!voidReturn) {
      TR::Node* retValNode
        = TR::Node::create(firstNode, self()->comp()->il.opCodeForConst(self()->getResolvedMethod()->returnType()), 0);
  ...
  try {
    if (!_firstTreeTop || !comp->isPeekingMethod()) {
      ...
      _flowGraph->setStartAndEnd(
        new (comp->trHeapMemory()) TR::Block(comp->trMemory()), new (comp->trHeapMemory()) TR::Block(comp->trMemory()));
      ...
      //methodSymbol's auto sym refs and pending push sym refs list; this only confuses analyses into thinking these
      ...
      if (_methodFlags.testAny(IlGenSuccess)) {
        ...
        // parms, and locals alive. Only do this in non HCR mode since under HCR we will be more selective about where
        // to add OSR points.
        //
        const int32_t siteIndex = comp->getCurrentInlinedSiteIndex();
        bool doOSR = comp->getOption(TR_EnableOSR) && !comp->isPeekingMethod()
          && !(comp->getOption(TR_DisableNextGenHCR) && comp->getOption(TR_EnableHCR)) && comp->supportsInduceOSR()
...
bool OMR::ResolvedMethodSymbol::cannotAttemptOSRAt(TR_ByteCodeInfo& bci, TR::Block* blockToOSRAt, TR::Compilation* comp)
{
  ...
  if (bci.doNotProfile()) {
    if (comp->getOption(TR_TraceOSR))
      traceMsg(
        comp, "Cannot attempt OSR at bytecode index %d:%d as it did not exist at ilgen\n", callSite, byteCodeIndex);
    return true;
  }
  ...
  TR::Compilation* comp, TR_ByteCodeInfo& byteCodeInfo, TR::TreeTop* insertTree, bool keepStashedArgsLive)
{
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (!nextSymRef->getSymbol()->isParm()
      ...
      TR::Node* storeNode
        = TR::Node::createWithSymRef(comp->il.opCodeForDirectStore(nextSymRef->getSymbol()->getDataType()), 1, 1,
  ...
  TR::Compilation* comp, int32_t inlinedSiteIndex, TR_ByteCodeInfo& byteCodeInfo, TR::TreeTop* induceOSRTree)
{
  if (!comp->osrStateIsReliable()) {
    traceMsg(comp,
      "OSR state may not be reliable enough to trust liveness info computed at IL gen time; so avoiding dead stack "
      "slot store insertion\n");
    return;
  }
...
bool OMR::ResolvedMethodSymbol::detectInternalCycles(TR::CFG* cfg, TR::Compilation* comp)
{
  if (cfg) {
          ...
          for (auto e = excepSucc.begin(); e != excepSucc.end(); ++e) {
            ...
            if (dest == node) {
              ...
              TR::Node* asyncNode = TR::Node::createWithSymRef(dest->getEntry()->getNextTreeTop()->getNode(),
                TR::asynccheck, 0, comp->getSymRefTab()->findOrCreateAsyncCheckSymbolRef(comp->getMethodSymbol()));
              ...
              TR::Node* gotoNode
                = TR::Node::create(dest->getEntry()->getNextTreeTop()->getNode(), TR::Goto, 0, gotoBlock->getEntry());
...
bool OMR::ResolvedMethodSymbol::catchBlocksHaveRealPredecessors(TR::CFG* cfg, TR::Compilation* comp)
{
  for (TR::CFGNode* node = cfg->getFirstNode(); node; node = node->getNext()) {
    if (!node->getExceptionPredecessors().empty()) {
      // catch block
...
void OMR::ResolvedMethodSymbol::removeUnusedLocals()
{
  ...
  if (debug("traceUnusedLocals")) {
    ...
    diagnostic("%08d(%08d) %08d(%08d)   %s\n", beforeSize - removedSize, afterTotalSize, removedSize, removedTotalSize,
      self()->comp()->signature());
  }
#endif
}
