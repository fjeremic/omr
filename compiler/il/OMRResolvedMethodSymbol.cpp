...
OMR::ResolvedMethodSymbol::create(AllocatorType m, TR_ResolvedMethod *rm,
                                  TR::Compilation *comp) {
...
OMR::ResolvedMethodSymbol::ResolvedMethodSymbol(TR_ResolvedMethod *method,
    ...
    : TR::MethodSymbol(TR_Private, method->convertToMethod()),
      _resolvedMethod(method), _flowGraph(0), _unimplementedOpcode(0),
      _firstTreeTop(0), _autoSymRefs(0), _pendingPushSymRefs(0),
      _parmSymRefs(0), _osrPoints(comp->trMemory()), _tempIndex(-1),
      _arrayCopyTempSlot(-1), _syncObjectTemp(0), _thisTempForObjectCtor(0),
      _atcDeferredCountTemp(0), _automaticList(comp->trMemory()),
      _parameterList(comp->trMemory()),
      _variableSizeSymbolList(comp->trMemory()), _comp(comp),
      _firstJitTempIndex(-1), _cannotAttemptOSR(NULL), _properties(0),
...
static void
getNeighboringSymRefLists(int32_t index,
                          TR_Array<List<TR::SymbolReference>> *listArray,
...
void OMR::ResolvedMethodSymbol::setParmSymRef(int32_t slot,
  ...
  if (!_parmSymRefs)
    _parmSymRefs = new (self()->comp()->trHeapMemory())
        TR_Array<TR::SymbolReference *>(self()->comp()->trMemory(),
                                        self()->getNumParameterSlots());
 ...
 */
int32_t
OMR::ResolvedMethodSymbol::getProfilingByteCodeIndex(int32_t bytecodeIndex) {
...
static bool
compareProfilingOffsetInfo(const std::pair<int32_t, int32_t> &first,
...
OMR::ResolvedMethodSymbol::create(TR_StackMemory m, TR_ResolvedMethod *rm,
                                  TR::Compilation *comp);
template TR::ResolvedMethodSymbol *
OMR::ResolvedMethodSymbol::create(TR_HeapMemory m, TR_ResolvedMethod *rm,
                                  TR::Compilation *comp);
template TR::ResolvedMethodSymbol *
OMR::ResolvedMethodSymbol::create(PERSISTENT_NEW_DECLARE m,
                                  TR_ResolvedMethod *rm, TR::Compilation *comp);
