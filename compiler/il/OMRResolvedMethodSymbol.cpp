...
TR::ResolvedMethodSymbol*
OMR::ResolvedMethodSymbol::create(AllocatorType m,
...
static TR::TreeTop*
findEndTreeTop(TR::ResolvedMethodSymbol* rms)
{
  TR::TreeTop* end = NULL;
...
template<typename AllocatorType>
TR::ResolvedMethodSymbol*
OMR::ResolvedMethodSymbol::createJittedMethodSymbol(AllocatorType m,
...
void
bcIndexForFakeInduce(TR::Compilation* comp,
                     ...
                     char* childPath)
{
  ...
  while (p) {
    ...
    *q = '\0';
    ...
    traceMsg(
      comp,
      "signature: %s, callSiteInsertionPoint: %d, bcIndexInsertionPoint: %x\n",
      mSignature,
                                                ...
                                                TR::CFG* cfg)
{
  ...
  TR_ASSERT(point,
            "Could not find osr point for bytecode %d:%d!",
  ...
  if (self()->comp()->getOptions()->getVerboseOption(TR_VerboseOSRDetails))
    TR_VerboseLog::writeLineLocked(TR_Vlog_OSRD,
                                   "Injected induceOSR call at %3d:%d in %s",
                                   refNode->getInlinedSiteIndex(),
                                   refNode->getByteCodeIndex(),
                                   self()->comp()->signature());
 ...
 */
bool
OMR::ResolvedMethodSymbol::induceOSRAfterAndRecompile(
  ...
  TR::TreeTop** lastTreeTop)
{
  ...
  TR_ASSERT_FATAL(
    ...
    induceOSRCallNode,
    induceOSRCallNode->getGlobalIndex(),
 ...
 */
bool
OMR::ResolvedMethodSymbol::induceOSRAfter(TR::TreeTop* insertionPoint,
                                          TR_ByteCodeInfo induceBCI,
                                          TR::TreeTop* branch,
                                          bool extendRemainder,
                                          int32_t offset,
                                          TR::TreeTop** lastTreeTop)
{
  return self()->induceOSRAfterImpl(insertionPoint,
                                    induceBCI,
                                    branch,
                                    extendRemainder,
                                    offset,
 ...
 */
TR::TreeTop*
OMR::ResolvedMethodSymbol::induceOSRAfterImpl(TR::TreeTop* insertionPoint,
                                              TR_ByteCodeInfo induceBCI,
                                              TR::TreeTop* branch,
                                              bool extendRemainder,
                                              int32_t offset,
  ...
  if (self()->supportsInduceOSR(induceBCI, block, self()->comp())) {
    ...
    if (remainderTree->getNode()->getOpCodeValue() != TR::BBEnd) {
      if (extendRemainder) {
        ...
        if (self()->comp()->getOption(TR_TraceOSR))
          traceMsg(self()->comp(),
                   "  Split of block_%d at n%dn produced block_%d\n",
                   block->getNumber(),
                   remainderTree->getNode()->getGlobalIndex(),
                   remainderBlock->getNumber());
      }
...
TR::TreeTop*
OMR::ResolvedMethodSymbol::induceImmediateOSRWithoutChecksBefore(
  TR::TreeTop* insertionPoint)
{
  if (self()->supportsInduceOSR(insertionPoint->getNode()->getByteCodeInfo(),
    ...
    return self()->genInduceOSRCallAndCleanUpFollowingTreesImmediately(
      insertionPoint,
      insertionPoint->getNode()->getByteCodeInfo(),
      false,
      self()->comp());
  if (self()->comp()->getOption(TR_TraceOSR))
    traceMsg(self()->comp(),
                                            ...
                                            TR::CFG* cfg)
{
  ...
  return self()->genInduceOSRCall(insertionPoint,
                                  inlinedSiteIndex,
                                  osrMethodData,
                                  numChildren,
                                  copyChildren,
  ...
  if (firstOSRPoint) {
    ...
    if (self()->comp()->getOption(TR_TraceOSR))
      traceMsg(self()->comp(),
               "code %p %d catch %p %d\n",
               OSRCodeBlock,
               OSRCodeBlock->getNumber(),
               OSRCatchBlock,
               OSRCatchBlock->getNumber());

    self()->getLastTreeTop()->insertTreeTopsAfterMe(OSRCatchBlock->getEntry(),
                                                    OSRCodeBlock->getExit());
    self()->genOSRHelperCall(
      inlinedSiteIndex, self()->comp()->getSymRefTab(), callerCFG);
  }

  self()->insertRematableStoresFromCallSites(
    self()->comp(), inlinedSiteIndex, induceOSRCallTree);
  ...
  int32_t currentInlinedSiteIndex)
{
  ...
  char childPath[10];
  childPath[0] = '\0';
  bcIndexForFakeInduce(
    self()->comp(), &callSiteInsertionPoint, &bcIndexInsertionPoint, childPath);
  ...
  for (TR::TreeTop* tt = self()->getFirstTreeTop(); tt;
    ...
    if (!skipNextTT) {
      ...
      if (matchCode > 0) {
        ...
        self()->genInduceOSRCallNode(
          tt, (numChildren - firstArgIndex), matchCode == 1);
    ...
    if (self()->comp()->isPotentialOSRPoint(tt->getNode())) {
      ...
      if (self()->comp()->isOSRTransitionTarget(TR::preExecutionOSR) ||
        ...
        osrPoint = new (self()->comp()->trHeapMemory()) TR_OSRPoint(
          ttnode->getByteCodeInfo(), osrMethodData, self()->comp()->trMemory());
        osrPoint->setOSRIndex(self()->addOSRPoint(osrPoint));
        if (self()->comp()->getOption(TR_TraceOSR))
          traceMsg(self()->comp(),
      ...
      if (edge == block->getExceptionSuccessors().end())
        self()->comp()->getFlowGraph()->addEdge(
          TR::CFGEdge::createExceptionEdge(
            block, OSRCatchBlock, self()->comp()->trMemory()));
    }
...
static bool
isTwoSlotSymRefIn(List<TR::SymbolReference>* symRefList)
{
  if (symRefList) {
...
OMR::ResolvedMethodSymbol::sharesStackSlot(TR::SymbolReference* symRef)
{
  ...
  if (slot < 0)
    getNeighboringSymRefLists(-slot - 1,
                              self()->getPendingPushSymRefs(),
                              &listForPrevSlot,
                              &list,
                              &listForNextSlot);
  else
    getNeighboringSymRefLists(slot,
                              self()->getAutoSymRefs(),
                              &listForPrevSlot,
                              &list,
                              &listForNextSlot);
...
void
OMR::ResolvedMethodSymbol::genOSRHelperCall(int32_t currentInlinedSiteIndex,
                                            ...
                                            TR::CFG* cfg)
{
  ...
  TR_ASSERT(self()->getFirstTreeTop(),
            "first tree top is NULL in %s",
            self()->signature(self()->comp()->trMemory()));
  ...
  TR::Node* vmThread = TR::Node::createWithSymRef(
    ...
    TR::loadaddr,
    0,
    new (self()->comp()->trHeapMemory())
      TR::SymbolReference(symRefTab,
                          TR::RegisterMappedSymbol::createMethodMetaDataSymbol(
                            self()->comp()->trHeapMemory(), "vmThread")));
  TR::SymbolReference* osrHelper =
    symRefTab->findOrCreateRuntimeHelper(TR_prepareForOSR, false, false, true);
  ...
  TR::Node* osrCall = TR::Node::createWithSymRef(
    firstNode, TR::call, loadNodes.size(), osrHelper);
  ...
  if (self()->comp()->cg()->getSupportsTM() &&
    ...
    if (trace)
      traceMsg(self()->comp(),
               "osrNode->getPrevTreeTop()->getNode() %p\n",
  ...
  if (self()->comp()->getCurrentInlinedSiteIndex() == -1) {
    ...
    TR::Node* osrReturnAddress = TR::Node::createLoad(
      firstNode,
      self()->comp()->getSymRefTab()->findOrCreateOSRReturnAddressSymbolRef());
    ...
    TR::Node* retNode =
      TR::Node::create(firstNode,
                       self()->getResolvedMethod()->returnOpCode(),
                       voidReturn ? 0 : 1);
    if (!voidReturn) {
      TR::Node* retValNode =
        TR::Node::create(firstNode,
...
bool
OMR::ResolvedMethodSymbol::genIL(TR_FrontEnd* fe,
                                 TR::Compilation* comp,
                                 TR::SymbolReferenceTable* symRefTab,
 ...
 */
bool
OMR::ResolvedMethodSymbol::cannotAttemptOSRDuring(int32_t callSite,
                                                  TR::Compilation* comp,
                                                  bool runCleanup)
{
  if (comp->getOption(TR_TraceOSR))
    traceMsg(
      comp, "Checking if OSR can be attempted during call site %d\n", callSite);

  int32_t origCallSite = callSite;
  TR_OSRMethodData* osrMethodData =
    comp->getOSRCompilationData()->findOrCreateOSRMethodData(callSite, self());
                                              ...
                                              TR::Compilation* comp)
{
  ...
  if (comp->getOption(TR_TraceOSR))
    traceMsg(comp,
             "Checking if OSR can be attempted at bytecode index %d:%d\n",
             callSite,
             byteCodeIndex);
  ...
  if (self()->_cannotAttemptOSR->get(byteCodeIndex)) {
    if (comp->getOption(TR_TraceOSR))
      traceMsg(comp,
               "Cannot attempt OSR at bytecode index %d:%d\n",
               callSite,
               byteCodeIndex);
    return true;
  }
...
void
OMR::ResolvedMethodSymbol::cleanupUnreachableOSRBlocks(int32_t inlinedSiteIndex,
                                                       TR::Compilation* comp)
{
  TR_OSRMethodData* osrMethodData =
    inlinedSiteIndex > -1
  ...
  while (osrMethodData) {
    ...
    if (!CallerOSRCodeBlock || CallerOSRCodeBlock->isUnreachable()) {
      if (comp->getOption(TR_TraceOSR))
        traceMsg(comp,
                 "Osr catch block at inlined site index %d is absent\n",
                 osrMethodData->getInlinedSiteIndex());
      ...
      traceMsg(comp,
               "Osr catch block at inlined site index %d is present\n",
               osrMethodData->getInlinedSiteIndex());
  ...
  if (!allCallersOSRCodeBlocksAreStillInCFG) {
    TR_OSRMethodData* cursorOsrMethodData =
      comp->getOSRCompilationData()->findOrCreateOSRMethodData(inlinedSiteIndex,
                                                               self());
...
void
OMR::ResolvedMethodSymbol::insertRematableStoresFromCallSites(
  TR::Compilation* comp,
  int32_t siteIndex,
 ...
 */
void
OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlots(
  TR::Compilation* comp,
 ...
 */
void
OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlotsBeforeInducingOSR(
  TR::Compilation* comp,
  int32_t inlinedSiteIndex,
  TR_ByteCodeInfo& byteCodeInfo,
  TR::TreeTop* induceOSRTree)
{
  if (!comp->osrStateIsReliable()) {
    traceMsg(
      comp,
      "OSR state may not be reliable enough to trust liveness info computed at "
      "IL gen time; so avoiding dead stack slot store insertion\n");
    return;
  }
...
void
OMR::ResolvedMethodSymbol::setParmSymRef(int32_t slot,
...
OMR::ResolvedMethodSymbol::getPendingPushSymRefs(int32_t slot)
{
  ...
  if (!_pendingPushSymRefs) {
    _pendingPushSymRefs =
      new (m->trHeapMemory()) TR_Array<List<TR::SymbolReference>>(m, 10, true);
  }
...
bool
OMR::ResolvedMethodSymbol::detectInternalCycles(TR::CFG* cfg,
                                                TR::Compilation* comp)
{
  if (cfg) {
        ...
        if (!excepSucc.empty()) {
          for (auto e = excepSucc.begin(); e != excepSucc.end(); ++e) {
            ...
            if (dest == node) {
              ...
              dumpOptDetails(
                comp,
                "Detected catch block with exception successor as itself %d\n",
                node->getNumber());
              ...
              if (containsMonexit) {
                TR::TreeTop* storeTT = TR::TreeTop::create(
                  comp,
                  (TR::Node::create(
                    clonedCatch->getEntry()->getNode(), TR::monexitfence, 0)));
                dumpOptDetails(
                  comp,
              ...
              TR::Node* asyncNode = TR::Node::createWithSymRef(
                dest->getEntry()->getNextTreeTop()->getNode(),
                TR::asynccheck,
                0,
              ...
              TR::Node* gotoNode =
                TR::Node::create(dest->getEntry()->getNextTreeTop()->getNode(),
                                 TR::Goto,
                                 0,
                                 gotoBlock->getEntry());
  ...
  for (TR::CFGNode* node = cfg->getFirstNode(); node; node = node->getNext()) {
    if (!node->getExceptionPredecessors().empty()) {
      ...
      if (!node->getPredecessors().empty()) {
        dumpOptDetails(comp,
                       "detected catch block_%d with real predecessors\n",
                       node->getNumber());
        return true;
      }
    }
 ...
 */
void
OMR::ResolvedMethodSymbol::setProfilerFrequency(int32_t bytecodeIndex,
                                                int32_t frequency)
{
  for (auto itr = _bytecodeProfilingOffsets.begin(),
            end = _bytecodeProfilingOffsets.end();
 ...
 */
int32_t
OMR::ResolvedMethodSymbol::getProfilerFrequency(int32_t bytecodeIndex)
{
  for (auto itr = _bytecodeProfilingOffsets.begin(),
            end = _bytecodeProfilingOffsets.end();
       itr != end;
 ...
 */
void
OMR::ResolvedMethodSymbol::addProfilingOffsetInfo(int32_t startBCI,
                                                  int32_t endBCI)
{
  for (auto itr = _bytecodeProfilingOffsets.begin(),
            end = _bytecodeProfilingOffsets.end();
 ...
 */
void
OMR::ResolvedMethodSymbol::dumpProfilingOffsetInfo(TR::Compilation* comp)
{
  for (auto itr = _bytecodeProfilingOffsets.begin(),
            end = _bytecodeProfilingOffsets.end();
       itr != end;
...
OMR::ResolvedMethodSymbol::create(TR_StackMemory m,
                                  TR_ResolvedMethod* rm,
                                  TR::Compilation* comp);
template TR::ResolvedMethodSymbol*
OMR::ResolvedMethodSymbol::create(TR_HeapMemory m,
                                  TR_ResolvedMethod* rm,
                                  TR::Compilation* comp);
template TR::ResolvedMethodSymbol*
