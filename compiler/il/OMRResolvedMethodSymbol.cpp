...
void bcIndexForFakeInduce(TR::Compilation* comp, int16_t* callSiteInsertionPoint,
...
bool OMR::ResolvedMethodSymbol::canInjectInduceOSR(TR::Node* node)
{
    ...
    if (sym->isMethod()) {
        ...
        if (methSym->getMethodKind() == TR::MethodSymbol::Helper || methSym->isNative() || methSym->getMethodKind() == TR::MethodSymbol::Special) {
            if (trace)
 ...
 */
bool OMR::ResolvedMethodSymbol::induceOSRAfterAndRecompile(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI, TR::TreeTop* branch,
 ...
 *          is generated successfully
 */
bool OMR::ResolvedMethodSymbol::induceOSRAfter(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI, TR::TreeTop* branch,
    bool extendRemainder, int32_t offset, TR::TreeTop** lastTreeTop)
{
...
OMR::ResolvedMethodSymbol::genInduceOSRCallAndCleanUpFollowingTreesImmediately(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI, bool shouldSplitBlock, TR::Compilation* comp)
{
    ...
    if (induceOSRCallTree) {
        ...
        while (treeAfterInduceOSRCall) {
            if (((treeAfterInduceOSRCall->getNode()->getOpCodeValue() == TR::athrow) && treeAfterInduceOSRCall->getNode()->throwInsertedByOSR()) || (treeAfterInduceOSRCall->getNode()->getOpCodeValue() == TR::BBEnd))
                break;
    ...
    const char* childPath)
{
    ...
    if (childPath[0] == 'b' || childPath[0] == 'a') {
        ...
        if ((callerIndex != -3 && refNode->getInlinedSiteIndex() != callerIndex) || (byteCodeIndex != -3 && refNode->getByteCodeIndex() != byteCodeIndex))
            return 0;
...
void OMR::ResolvedMethodSymbol::genOSRHelperCall(int32_t currentInlinedSiteIndex, TR::SymbolReferenceTable* symRefTab, TR::CFG* cfg)
{
    // If not specified use the outermost CFG
    if (!cfg)
        cfg = self()->comp()->getFlowGraph();
...
bool OMR::ResolvedMethodSymbol::genIL(TR_FrontEnd* fe, TR::Compilation* comp, TR::SymbolReferenceTable* symRefTab, TR::IlGenRequest& customRequest)
{
...
bool OMR::ResolvedMethodSymbol::sharesStackSlots(TR::Compilation* comp)
{
    ...
    for (i = 0; !isRequired && autosListArray && i < autosListArray->size(); ++i) {
        ...
        if ((symRef->getCPIndex() < methodSymbol->getFirstJitTempIndex()) && ((autosList.getSize() > 1) || (prevTakesTwoSlots && !autosList.isEmpty()))) {
            if (comp->getOption(TR_TraceOSR)) {
...
void OMR::ResolvedMethodSymbol::insertRematableStoresFromCallSites(TR::Compilation* comp, int32_t siteIndex, TR::TreeTop* induceOSRTree)
{
 ...
 */
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlots(TR::Compilation* comp, TR_ByteCodeInfo& byteCodeInfo, TR::TreeTop* insertTree, bool keepStashedArgsLive)
{
 ...
 */
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlotsBeforeInducingOSR(TR::Compilation* comp, int32_t inlinedSiteIndex, TR_ByteCodeInfo& byteCodeInfo, TR::TreeTop* induceOSRTree)
{
...
OMR::ResolvedMethodSymbol::findOSRPoint(TR_ByteCodeInfo& bcInfo)
{
    for (intptrj_t i = 0; i < _osrPoints.size(); ++i) {
        ...
        if (pointBCInfo.getByteCodeIndex() == bcInfo.getByteCodeIndex() && pointBCInfo.getCallerIndex() == bcInfo.getCallerIndex())
            return _osrPoints[i];
    }
...
void OMR::ResolvedMethodSymbol::removeUnusedLocals()
{
    ...
    while (cursor) {
        ...
        if (local->getReferenceCount() == 0 && !(liveButMaybeUnreferencedLocals && !local->isLiveLocalIndexUninitialized() && liveButMaybeUnreferencedLocals->isSet(local->getLiveLocalIndex()))) {
#ifdef DEBUG
