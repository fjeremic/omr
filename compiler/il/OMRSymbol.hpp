 ...
 * dozens of subclasses, we have chosen to create [special-case symbol
 * factories.][design]
 *
...
class OMR_EXTENSIBLE Symbol {
 ...
 protected:
   ...
   *        size is set to 0.
   */
  Symbol() : _size(0), _name(0), _flags(0), _flags2(0), _localIndex(0) {}
 ...
 public:
  ...
  // These methods perform an explicit (debug) assume that the symbol is a
  // correct type and then return the symbol explicitly cast to the type.
  //
  ...
  bool dontEliminateStores(TR::Compilation *comp,
                           bool isForLocalDeadStore = false);
  ...
  enum {
    ...
    SlotSharedByRefAndNonRef =
        0x00400000,  ///< used in fsd to indicate that an reference symbol
                     ///< shares a slot with a nonreference symbol
    ...
    SpillTempLoaded =
        0x04000000,  ///< share bit with loadaddr because spill temps will never
                     ///< have their address taken. Used to remove store to
                     ///< spill if never loaded
    // Available              = 0x02000000,
    VariableSizeSymbol =
        0x01000000,  ///< non-java only?: specially managed automatic symbols
                     ///< that contain both an activeSize and a size
    ...
    //
    ParmHasToBeOnStack =
        0x80000000,  ///< parameter is both loadAddr-ed and assigned a global
                     ///< register, or parameter has been stored (store opcode)
    ...
    //
    StartOfColdInstructionStream =
        0x80000000,  // label at the start of an out-of-line instruction stream
    ...
    InternalControlFlowMerge =
        0x02000000,  // mainline merge label for OOL instructions
    EndOfColdInstructionStream = 0x01000000,
    NonLinear = 0x01000000,  // TAROK and temporary.  This bit is used in
                             // conjunction with StartOfColdInstructionStream
                             //    to distinguish "classic" OOL instructions and
                             //    the new form for Tarok.
  ...
  { RelativeLabel = 0x00000001,       // Label Symbols only *+N
    ...
    HasAddrTaken =
        0x00000010,  // used to denote that we have a loadaddr of this symbol
 ...
 public:
   ...
   *
   * \deprecated These factories are deprecated and should be removed when
   * possible.
   *
   * @{
   */
 public:
  template <typename AllocatorType>
  static TR::Symbol *createNamedShadow(AllocatorType m, TR::DataType d,
                                       uint32_t s, char *name = NULL);
