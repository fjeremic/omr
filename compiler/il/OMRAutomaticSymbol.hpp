...
public:
  ...
  createLocalObject(AllocatorType m, int32_t arrayType, TR::DataType d,
  ...
  template <typename AllocatorType>
  static TR::AutomaticSymbol *
  createLocalObject(AllocatorType m, TR::ILOpCodes kind,
                    TR::SymbolReference *classSymRef, TR::DataType d,
                    uint32_t s, TR_FrontEnd *fe);
...
public:
  template <typename AllocatorType>
  static TR::AutomaticSymbol *
  createInternalPointer(AllocatorType m,
                        TR::AutomaticSymbol *pinningArrayPointer = 0);

  template <typename AllocatorType>
  static TR::AutomaticSymbol *
  createInternalPointer(AllocatorType m, TR::DataType d,
                        TR::AutomaticSymbol *pinningArrayPointer = 0);

  template <typename AllocatorType>
  static TR::AutomaticSymbol *createInternalPointer(AllocatorType m,
...
#define TR_VSS_NAME "VTS"
  ...
  enum {
    IsReferenced =
        0x01, ///< was this symbol ever actually used in any instruction? -- if
              ///< not then it does not have to be mapped
    IsAddressTaken =
        0x02, ///< address taken symbols have to be kept alive longer
    IsSingleUse =
        0x04, ///< a temp ref created for a one off privatization in a single
              ///< evaluator (doesn't use refCounts so automatically freed)
  };
