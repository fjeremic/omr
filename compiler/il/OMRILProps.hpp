 ...
 * specify the expected number of children. The remaining bits, from high to
 * low, specify the expected type of each child, from last to first. Types are
...
static const TR::ILChildPropType IndirectCallType =
  0xffffffffffffff00ULL | TR::Address;
...
static_assert(TR::NumTypes < 254,
              "There are too many data types to fit in one byte.");
...
#define TWO_CHILD(type1, type2)                                                \
  CHILD_COUNT(2ULL) | FIRST_CHILD(type1) | SECOND_CHILD(type2)
#define THREE_CHILD(type1, type2, type3)                                       \
  CHILD_COUNT(3ULL) | FIRST_CHILD(type1) | SECOND_CHILD(type2) |               \
    THIRD_CHILD(type3)
#define FOUR_CHILD(type1, type2, type3, type4)                                 \
  CHILD_COUNT(4ULL) | FIRST_CHILD(type1) | SECOND_CHILD(type2) |               \
    THIRD_CHILD(type3) | FOURTH_CHILD(type4)
#define FIVE_CHILD(type1, type2, type3, type4, type5)                          \
  CHILD_COUNT(5ULL) | FIRST_CHILD(type1) | SECOND_CHILD(type2) |               \
    THIRD_CHILD(type3) | FOURTH_CHILD(type4) | FIFTH_CHILD(type5)
#define SIX_CHILD(type1, type2, type3, type4, type5, type6)                    \
  CHILD_COUNT(6ULL) | FIRST_CHILD(type1) | SECOND_CHILD(type2) |               \
    THIRD_CHILD(type3) | FOURTH_CHILD(type4) | FIFTH_CHILD(type5) |            \
    SIXTH_CHILD(type6)
#define SEVEN_CHILD(type1, type2, type3, type4, type5, type6, type7)           \
  CHILD_COUNT(7ULL) | FIRST_CHILD(type1) | SECOND_CHILD(type2) |               \
    THIRD_CHILD(type3) | FOURTH_CHILD(type4) | FIFTH_CHILD(type5) |            \
    SIXTH_CHILD(type6) | SEVENTH_CHILD(type7)
...
#define GET_SECOND_CHILD_TYPE(childProp)                                       \
  TR::DataTypes(((childProp) >> 8) & 0xff)
#define GET_THIRD_CHILD_TYPE(childProp)                                        \
  TR::DataTypes(((childProp) >> 16) & 0xff)
#define GET_FOURTH_CHILD_TYPE(childProp)                                       \
  TR::DataTypes(((childProp) >> 24) & 0xff)
#define GET_FIFTH_CHILD_TYPE(childProp)                                        \
  TR::DataTypes(((childProp) >> 32) & 0xff)
#define GET_SIXTH_CHILD_TYPE(childProp)                                        \
  TR::DataTypes(((childProp) >> 40) & 0xff)
#define GET_SEVENTH_CHILD_TYPE(childProp)                                      \
  TR::DataTypes(((childProp) >> 48) & 0xff)
#define GET_CHILD_TYPE(index, childProp)                                       \
  TR::DataTypes(((childProp) >> ((index)*8)) & 0xff)
...
enum
{
  ...
  SignedExponentiation =
    0x00000040, ///< signed/unsigned base with signed exponent
  UnsignedExponentiation =
    0x00000080, ///< signed/unsigned base with unsigned exponent
  CompareTrueIfLess =
    0x00000100, ///< Result is true if left <  right.  Set for < <= !=
  CompareTrueIfGreater =
    0x00000200, ///< Result is true if left >  right.  Set for > >= !=
  CompareTrueIfEqual =
    0x00000400, ///< Result is true if left == right.  Set for <= >= ==
  CompareTrueIfUnordered =
    0x00000800, ///< Result is true if left and right are incomparable.  (NaN)
  CanUseStoreAsAnAccumulator = 0x00001000,
  HasBranchChild =
    0x00002000, ///< used in conjunction with jumpwithmultipletargets
  SkipDynamicLitPoolOnInts =
    0x00004000, ///< do not perform dynamicLiteralPool on any integral constant
                ///< children of this node (used for lengths of array ops)
  Abs = 0x00008000,
