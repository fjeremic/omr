...
OMR::Node::Node()
    : _opCode(TR::BadILOp), _numChildren(0),
      //_globalIndex(0),
      _flags(0), _visitCount(0), _localIndex(0), _referenceCount(0),
      _byteCodeInfo(), _unionBase(), _unionPropertyA() {}
...
OMR::Node::Node(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
    ...
    : _opCode(op), _numChildren(numChildren),
      //_globalIndex(0),
      _flags(0), _visitCount(0), _localIndex(0), _referenceCount(0),
      _byteCodeInfo(), _unionBase(), _unionPropertyA() {
...
OMR::Node::Node(TR::Node *from, uint16_t numChildren)
    : _opCode(TR::BadILOp), _numChildren(0), _globalIndex(0), _flags(0),
      _visitCount(0), _localIndex(0), _referenceCount(0), _byteCodeInfo(),
      _unionBase(), _unionPropertyA() {
  TR::Compilation *comp = TR::comp();
...
void OMR::Node::copyValidProperties(TR::Node *fromNode, TR::Node *toNode) {
  ...
  if ((fromOpCode == TR::bitOpMemND) && (toOpCode == TR::bitOpMem)) {
    if (fromNode->isOrBitOpMem())
      toNode->setOrBitOpMem(true);
    if (fromNode->isAndBitOpMem())
      toNode->setAndBitOpMem(true);
    if (fromNode->isXorBitOpMem())
      toNode->setXorBitOpMem(true);
  }
...
TR::Node *
OMR::Node::recreateWithoutProperties(TR::Node *originalNode, TR::ILOpCodes op,
                                     uint16_t numChildren, TR::Node *first,
...
TR::Node *
OMR::Node::createRelative32BitFenceNode(TR::Node *originatingByteCodeNode,
...
TR::Node *OMR::Node::createConstZeroValue(TR::Node *originatingByteCodeNode,
  ...
  default:
    TR_ASSERT_SAFE_FATAL(false,
                         "datatype %s not supported for createConstZeroValue\n",
                         dt.toString());
  }
  return constZero;
}
...
TR::Node *OMR::Node::createConstOne(TR::Node *originatingByteCodeNode,
  ...
  default:
    TR_ASSERT_SAFE_FATAL(
        false, "datatype %s not supported for createConstOne\n", dt.toString());
  }
  return constOne;
}
...
recordProcessedNodeResult(TR::Node *node, TR_YesNoMaybe collectedness,
...
TR_YesNoMaybe OMR::Node::computeIsCollectedReferenceImpl(
  ...
  while (curNode) {
    ...
    case TR::aconst:
      ...
      // aladd/aiadd on a temp whose value is null as an internal pointer, thus
      // creating a temp, pinning array and internal pointer map for it.
...
int32_t OMR::Node::setEvaluationPriority(int32_t p) {
  ...
  {
    TR_ASSERT(0, "setEvaluationPriority cannot be called after the node has "
...
bool OMR::Node::hasFoldedImplicitNULLCHK() {
  TR_ASSERT(self()->getOpCode().isBndCheck() ||
                self()->getOpCode().isSpineCheck(),
            "assertion failure");
...
void OMR::Node::setRarePathForwardArrayCopy(bool v) {
  ...
  if (v != self()->isRarePathForwardArrayCopy() &&
      performNodeTransformation2(
          ...
          "%d\n",
          self(), v))
    _flags.setValue(arraycopyDirectionMask,
                    v ? arraycopyDirectionForwardRarePath
                      : arraycopyDirectionUnknown);
  if (v)
...
void OMR::Node::setIsDontMoveUnderBranch(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isLoadReg() ||
                self()->getOpCode().isLoadVarDirect(),
            "assertion failure");
...
bool OMR::Node::isNormalizedShift() {
  TR_ASSERT((self()->getOpCode().isLeftShift() ||
             self()->getOpCode().isRightShift() ||
             self()->getOpCode().isRotate()),
