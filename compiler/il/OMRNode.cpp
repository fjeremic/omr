  ...
  , _unionPropertyA()
{
  ...
  TR_ASSERT(self()->hasSymbolReference() + self()->hasRegLoadStoreSymbolReference() + self()->hasBranchDestinationNode()
        + self()->hasBlock() + self()->hasArrayStride() + self()->hasPinningArrayPointer() + self()->hasDataType()
      <= 1,
    ...
    self()->hasBranchDestinationNode(), self()->hasBlock(), self()->hasArrayStride(), self()->hasPinningArrayPointer(),
    self()->hasDataType());
}
  ...
  TR::Node* originalNode, TR::ILOpCodes op, TR::SymbolReference* newSymRef)
{
  ...
  // to deallocate any objects it has created. This is one of them. It will be recreated if necessary on createInternal.
  ...
  TR::Node* first, TR::Node* second, TR::Node* third, TR::Node* fourth, TR::Node* fifth)
{
  ...
  if (symbolRefOrBranchTarget != NULL || node->hasSymbolReference() || node->hasBranchDestinationNode()) {
    ...
    if (node->hasSymbolReference())
      ...
      TR_ASSERT(false,
        "attempting to access _branchDestinationNode or _symbolReference field for node %s %p that does not have one "
        "of them",
...
{
  // So far extra 3rd child could be added to the following nodes: if, variableNewArray, arraystorechk or a node with a
  // padding address these nodes are created with room for three children
  TR::Node* node;
...
TR::Node* OMR::Node::createWithRoomForFive(
  TR::ILOpCodes op, TR::Node* first, TR::Node* second, TR::Node* third, TR::Node* fourth, void* symbolRefOrBranchTarget)
{
  TR_ASSERT(
  ...
  TR::Node* originatingByteCodeNode, int32_t osrInductionOffset)
{
  ...
  TR::Node* callNode = TR::Node::createWithSymRef(
    originatingByteCodeNode, TR::call, 0, TR::comp()->getSymRefTab()->findOrCreatePotentialOSRPointHelperSymbolRef());
...
TR::Node* OMR::Node::createAddressNode(
  TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uintptrj_t address, uint8_t precision)
{
...
TR::Node* OMR::Node::createAddressNode(TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uintptrj_t address)
{
  TR::Node* node = TR::Node::create(originatingByteCodeNode, op, 0, 0);
  node->setAddress(address);
  return node;
...
TR::Node* OMR::Node::aconst(TR::Node* originatingByteCodeNode, uintptrj_t val, uint8_t precision)
{
...
TR::Node* OMR::Node::createConstDead(TR::Node* originatingByteCodeNode, TR::DataType dt, intptrj_t extraData)
{
  TR::Node* result = NULL;
...
TR::Node* OMR::Node::createVectorConst(TR::Node* originatingByteCodeNode, TR::DataType dt)
{
...
TR::Node* OMR::Node::createVectorConversion(TR::Node* src, TR::DataType trgType)
{
...
{
  TR_ASSERT(self()->getOpCode().hasPinningArrayPointer(), "Opcode %s is not supported, node is " POINTER_PRINTF_FORMAT,
      ...
      if (storeSymbol->isStatic()) {
        ...
        if (symRef->isUnresolved() || storeRef->isUnresolved())
          return TR::Compiler->cls.jitStaticsAreSame(
            c, storeRef->getOwningMethod(c), storeRef->getCPIndex(), symRef->getOwningMethod(c), symRef->getCPIndex());
...
TR::TreeTop* OMR::Node::getVirtualCallTreeForGuard()
{
  ...
  while (1) {
    ...
    while (callTree->getNode()->getOpCodeValue() != TR::BBEnd) {
      ...
      if ((callNode && callNode->getOpCode().isCallIndirect()) || (callTree->getNode()->getOpCodeValue() == TR::Goto))
        break;
  ...
  if (storesNeedToBeCreated) {
    ...
    if (isInternalPointer) {
      ...
      if (self()->getOpCode().isArrayRef()) {
        ...
        if (child->isInternalPointer())
          ...
          if (child->getOpCode().isLoadVarDirect() && child->getSymbolReference()->getSymbol()->isAuto()) {
            if (child->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer())
              pinningArray
                = child->getSymbolReference()->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer();
            else {
        ...
        pinningArray
          = self()->getSymbolReference()->getSymbol()->castToInternalPointerAutoSymbol()->getPinningArrayPointer();
...
TR::Node* OMR::Node::setChild(int32_t c, TR::Node* p)
{
  ...
  if (self()->hasNodeExtension()) {
    TR_ASSERT(
      c < _unionBase._extension.getNumElems(), "Out of bounds errror when setting element %d in node %p\n", c, this);
...
TR::Register* OMR::Node::setRegister(TR::Register* reg)
{
  if (reg) {
    if (reg->isLive()) {
      ...
      if (pair) {
        TR_ASSERT(
          pair->getLowOrder()->isLive() && pair->getHighOrder()->isLive(), "Register pair components are not live");
...
int64_t OMR::Node::get64bitIntegralValue()
{
  ...
  if (type.isInt8())
    ...
    TR_ASSERT(
      false, "Must be an integral or address but it is type %s on node %p\n", self()->getDataType().toString(), self());
    return 0;
  }
...
TR::DataType OMR::Node::getArrayCopyElementType()
{
  ...
  if (_numChildren == 3) {
    ...
    TR_ASSERT(
      _numChildren == 5, "getArrayCopyElementType called on an arraycopy node with an invalid number of children");
...
TR::SymbolReference* OMR::Node::getSymbolReference()
{
  ...
  TR_ASSERT(self()->hasSymbolReference(),
    "attempting to access _symbolReference field for node %s %p that does not have it", self()->getOpCode().getName(),
    this);
...
TR::AutomaticSymbol* OMR::Node::getPinningArrayPointer()
{
  TR_ASSERT(self()->hasPinningArrayPointer(),
    "attempting to access _pinningArrayPointer field for node %s %p that does not have it",
    self()->getOpCode().getName(), this);
...
TR::DataType OMR::Node::computeDataType()
{
  ...
  if (_opCode.hasSymbolReference() || self()->hasRegLoadStoreSymbolReference()) {
    TR::SymbolReference* symRef = (_opCode.hasSymbolReference())
...
{
  return !self()->getOpCode().isIf() && (self()->getOpCodeValue() != TR::PassThrough) && _flags.testAny(cannotOverFlow);
}
...
void OMR::Node::setStoredValueIsIrrelevant(bool v)
{
  ...
  TR_ASSERT(c->getOption(TR_EnableOSR),
    "Currently this OMR::Node flag means different things on different front ends. If we wanted to reuse this query "
    "across front ends other than Java (which uses these kinds of stores for OSR) then we probably need to map it to a "
    "different bit in the OMR::Node flags");
  TR_ASSERT((self()->getOpCode().isStore() && self()->getSymbolReference()->getSymbol()->isAutoOrParm()),
    "Opcode must be store to a stack slot");
...
bool OMR::Node::isInternalPointer()
{
  return _flags.testAny(internalPointer)
    && (self()->getOpCode().hasPinningArrayPointer() || self()->getOpCode().isArrayRef());
}
...
void OMR::Node::setBackwardArrayCopy(bool v)
{
  ...
  if (v) {
    ...
    TR_ASSERT(self()->isBackwardArrayCopy() && !self()->isForwardArrayCopy() && !self()->isRarePathForwardArrayCopy(),
      "assertion failure");
  } else {
    if (self()->isBackwardArrayCopy()
...
void OMR::Node::setRarePathForwardArrayCopy(bool v)
{
  ...
  if (v)
    TR_ASSERT(self()->isRarePathForwardArrayCopy() && self()->isForwardArrayCopy() && !self()->isBackwardArrayCopy(),
      "assertion failure");
  else
...
{
  TR_ASSERT(
    self()->getOpCodeValue() == TR::bitOpMem || self()->getOpCodeValue() == TR::bitOpMemND, "Opcode must be bitOpMem");
...
{
  TR_ASSERT(self()->getOpCodeValue() == TR::fRegLoad || self()->getOpCodeValue() == TR::dRegLoad,
...
void OMR::Node::setNeedsPrecisionAdjustment(bool v)
{
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::fRegLoad || self()->getOpCodeValue() == TR::dRegLoad,
    "Opcode must be FP global reg load");
  if (performNodeTransformation2(
        c, "O^O NODE FLAGS: Setting needsPrecisionAdjustment flag on node %p to %d\n", self(), v))
    _flags.set(precisionAdjustment, v);
}

bool OMR::Node::chkNeedsPrecisionAdjustment()
{
  return (self()->getOpCodeValue() == TR::fRegLoad || self()->getOpCodeValue() == TR::dRegLoad)
...
void OMR::Node::setIsNodeCreatedByPRE()
{
  TR::Compilation* c = TR::comp();
  TR_ASSERT(
    self()->canChkNodeCreatedByPRE(), "Unexpected opcode %s in setIsNodeCreatedByPRE()", self()->getOpCode().getName());
...
{
  TR_ASSERT(
    self()->canChkNodeCreatedByPRE(), "Unexpected opcode %s in chkNodeCreatedByPRE()", self()->getOpCode().getName());
...
void OMR::Node::resetIsTheVirtualGuardForAGuardedInlinedCall()
{
  ...
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
        c, "O^O NODE FLAGS: Resetting isTheVirtualGuardForAGuardedInlinedCall flag on node %p\n", self()))
...
void OMR::Node::setIsMethodEnterExitGuard(bool b)
{
  ...
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (b
    && performNodeTransformation2(c, "O^O NODE FLAGS: Setting methodEnterExitGuard flag to %d on node %p\n", b, self()))
...
void OMR::Node::setIsDirectMethodGuard(bool b)
{
  ...
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (b
    && performNodeTransformation2(c, "O^O NODE FLAGS: Setting directMethodGuard flag to %d on node %p\n", b, self()))
...
{
  TR_ASSERT(
    self()->getType().isInt64() || self()->getType().isAddress(), "Can only call this for a long or address opcode\n");
...
{
  TR_ASSERT(
    self()->getOpCodeValue() == TR::monexit || self()->getOpCodeValue() == TR::monent, "Opcode must be monexit/monent");
...
{
  return (self()->getOpCodeValue() == TR::lmul || self()->getOpCodeValue() == TR::luadd
           || self()->getOpCodeValue() == TR::lusub || self()->getOpCodeValue() == TR::ladd
...
{
  TR_ASSERT(self()->getOpCodeValue() == TR::lmul || self()->getOpCodeValue() == TR::luadd
      || self()->getOpCodeValue() == TR::lusub || self()->getOpCodeValue() == TR::ladd
      || self()->getOpCodeValue() == TR::lsub,
...
{
  TR_ASSERT((self()->getOpCode().isLeftShift() || self()->getOpCode().isRightShift() || self()->getOpCode().isRotate()),
...
static void resetFlagsForCodeMotionHelper(TR::Node* node, TR::NodeChecklist& visited)
{
