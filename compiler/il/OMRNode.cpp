...
void *OMR::Node::operator new(size_t s, TR::NodePool &nodes) {
  return (void *)nodes.allocate();
}

void OMR::Node::operator delete(void *node, TR::NodePool &nodes) {
  nodes.deallocate((TR::Node *)node);
}

void *OMR::Node::operator new(size_t s, void *ptr) throw() {
  return ::operator new(s, ptr);
}

void OMR::Node::operator delete(void *node, void *ptr) throw() {
...
OMR::Node::Node(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
  ...
  if (self()->getGlobalIndex() == MAX_NODE_COUNT) {
    ...
    comp->failCompilation<TR::ExcessiveComplexity>(
        "Global index equal to max node count");
  }
  ...
  if (comp->getDebug()) comp->getDebug()->newNode(self());

  // check that _unionPropertyA union is disjoint
  TR_ASSERT(self()->hasSymbolReference() +
                    self()->hasRegLoadStoreSymbolReference() +
                    self()->hasBranchDestinationNode() + self()->hasBlock() +
                    self()->hasArrayStride() +
                    self()->hasPinningArrayPointer() + self()->hasDataType() <=
                1,
            "_unionPropertyA union is not disjoint for this node %s (%p):\n"
            "  has({SymbolReference, ...}, ..., DataType) = "
            "({%1d,%1d},%1d,%1d,%1d,%1d,%1d)\n",
            self()->getOpCode().getName(), this, self()->hasSymbolReference(),
            self()->hasRegLoadStoreSymbolReference(),
            self()->hasBranchDestinationNode(), self()->hasBlock(),
            self()->hasArrayStride(), self()->hasPinningArrayPointer(),
            self()->hasDataType());
}
...
OMR::Node::Node(TR::Node *from, uint16_t numChildren)
  ...
  if (self()->hasDataType()) self()->setDataType(TR::NoType);
  ...
  // a memcpy is used above to copy fields from the argument "node" to "this",
  // but opt attributes are separate, and need separate initialization.
...
TR::Node *OMR::Node::copy(TR::Node *from) {
  ...
  if (opCode.isIf() || opCodeValue == TR::anewarray ||
      opCodeValue == TR::newarray || opCodeValue == TR::arraycopy ||
      opCodeValue == TR::tstart || opCodeValue == TR::arrayset)
    ++numChildren;

  // note arraystorechk node has 1 or 2 children but it allocates spaces for 3
  // children we need to make sure we have room for 3
  if (opCodeValue == TR::ArrayStoreCHK) numChildren = 3;
...
TR::Node *OMR::Node::copy(TR::Node *from, int32_t numChildren) {
  TR::Compilation *comp = TR::comp();
  TR_ASSERT(numChildren >= from->getNumChildren(),
            "copy must have at least as many children as original");
 ...
 *
 * If !forNodeExtensionOnly then clone the default children as well, including
 * the _numChildren[Room] fields
 */
void OMR::Node::copyChildren(
    TR::Node *from, uint16_t numChildren /* ignored if !forNodeExtensionOnly */,
  ...
  if (from->hasNodeExtension()) {
    ...
    if (numChildren > from->_unionBase._extension.getNumElems()) {
      ...
      for (uint16_t childNum = 0;
           childNum < from->_unionBase._extension.getNumElems(); childNum++)
      ...
      self()->copyNodeExtension(from->_unionBase._extension.getExtensionPtr(),
                                from->_unionBase._extension.getNumElems(),
                                size);
    }
  } else {
...
void OMR::Node::copyValidProperties(TR::Node *fromNode, TR::Node *toNode) {
  ...
  UnionPropertyA_Type fromUnionPropertyA_Type =
      fromNode->getUnionPropertyA_Type();
  ...
  if (fromUnionPropertyA_Type == toUnionPropertyA_Type) {
    switch (fromUnionPropertyA_Type) {
      case HasSymbolReference:
        ...
        toNode->_unionPropertyA._symbolReference =
            fromNode->_unionPropertyA._symbolReference;
#else
...
#else
        toNode->setRegLoadStoreSymbolReference(
            fromNode->getRegLoadStoreSymbolReference());
#endif
        break;
  ...
  bool fromConst =
      fromNode->getOpCode().isLoadConst() &&
      (fromNode->getType().isIntegral() || fromNode->getType().isAddress());
  bool toConst =
      toNode->getOpCode().isLoadConst() &&
      (toNode->getType().isIntegral() || toNode->getType().isAddress());
  ...
  if (toNode->_numChildren == 0 || !toNode->hasNodeExtension()) {
    /* if (fromNode->getOpCode().getOpCodeValue() == TR::allocationFence)
       self()->setAllocation(NULL); */
    {
      // may be using other property that is unioned with _unionBase._children
      // but do not know which one so have to copy it forward irregardless of
      // whether it is valid or not
      toNode->_unionBase = fromNode->_unionBase;
    }
  }

  // nameSymRef & callNodeData, if set on formNode, is already set for toNode
  // (its looked up via globalIndex which is unchanged)
  // TODO: check whether properties are valid for the replacement node and
  // remove if invalid
  ...
  // TODO:
  // OMR::Base
  // // this union still needs to be completed on consts, and
  // _unionBase._unionBase._unionedWithChildren
  //  _unionBase:
  //    _children[NUM_DEFAULT_CHILDREN]  - cstr body
  //    _constValue        - init list, need to fix users of recreate to NOT set
  //    const, THEN do recreate _fpConstValue _dpConstValue _fpConstValueBits
  //    _extension         - done
  //       _data
...
TR::Node *OMR::Node::recreateWithSymRef(TR::Node *originalNode,
                                        TR::ILOpCodes op,
                                        TR::SymbolReference *newSymRef) {
  return TR::Node::recreateAndCopyValidPropertiesImpl(originalNode, op,
                                                      newSymRef);
}
 ...
 * OMR::Node::recreate
 *
 * creates a new node with a new op, based on the originalNode, reusing the
 * memory occupied by originalNode and returning the same address as it.
 *
 * It will have the same globalIndex, poolIndex, byteCodeInfo, and optAttributes
 * as the originalNode, but properties from the originalNode that are valid for
 * the new node only will be copied. All other properties will be reset to their
 * default values.
 */
TR::Node *OMR::Node::recreateAndCopyValidPropertiesImpl(
    TR::Node *originalNode, TR::ILOpCodes op, TR::SymbolReference *newSymRef) {
  TR_ASSERT_FATAL(TR::Node::isNotDeprecatedUnsigned(op),
                  "Trying to use a deprecated unsigned opcode: #%d \n", op);
  TR_ASSERT(originalNode != NULL,
            "trying to recreate node from a NULL originalNode.");
  if (originalNode->getOpCodeValue() == op) {
    if (!originalNode->hasSymbolReference() ||
        newSymRef != originalNode->getSymbolReference())
      originalNode->_byteCodeInfo.setDoNotProfile(1);

    // need to at least set the new symbol reference on the node before
    // returning
    if (newSymRef) originalNode->setSymbolReference(newSymRef);

    return originalNode;
  }

  TR::Compilation *comp = TR::comp();
  //   // one may consider copy forwarding as a transformation of the old way of
  //   doing things to the new way
  //   // "untransforming" means reverting to setOpCodeValue only. However
  //   reverting may in future be a
  //   // bad thing to do, so disabling this.
  //   if (!performTransformation(comp, "O^O RECREATE: recreating node %s %p;
  //   original opcode %s\n",
  //         TR::ILOpCode(op).getName(), originalNode,
  //         originalNode->getOpCode().getName()))
  //      {
  //      originalNode->setOpCodeValue(op);
  //      return originalNode;
  //      }

  // need to copy the original node so the properties are available later to set
  // on the recreated node
  ...
  // we can't delete the node, otherwise we will potentially lose the poolIndex
  // (and other properties), but we do need to deallocate any objects it has
  // created. This is one of them. It will be recreated if necessary on
  // createInternal. other properties such as nameSymRef & callNodeData, are
  // associated with the _globalIndex and are not freed and realloced.
  originalNode->freeExtensionIfExists();

  // this clears most properties, so copyValidProperties must be complete
  TR::Node *node = TR::Node::createInternal(
      0, op, originalNode->getNumChildren(), originalNode);
#else
  // this does not clear properties, but some properties may be invalid for the
  // replacement node, but it does not matter if copyValidProperties is
  // complete, though it must be accurate
  TR::Node *node = originalNode;
  node->setOpCodeValue(op);
#endif

  if (newSymRef) {
    if (originalNodeCopy->hasSymbolReference() ||
        originalNodeCopy->hasRegLoadStoreSymbolReference())
      originalNodeCopy->setSymbolReference(newSymRef);
    else if (node->hasSymbolReference() ||
             node->hasRegLoadStoreSymbolReference())
...
TR::Node *OMR::Node::createInternal(TR::Node *originatingByteCodeNode,
                                    TR::ILOpCodes op, uint16_t numChildren,
                                    TR::Node *originalNode) {
  TR_ASSERT_FATAL(TR::Node::isNotDeprecatedUnsigned(op),
                  "The unsigned opcode is deprecated: %d \n", op);
  if (!originalNode)
    return new (TR::comp()->getNodePool())
        TR::Node(originatingByteCodeNode, op, numChildren);
  else {
    ...
    const TR_ByteCodeInfo byteCodeInfo =
        originalNode
            ->getByteCodeInfo();  // copy bytecode info into temporary variable
    // TR::Node * node = new (TR::comp()->getNodePool(), poolIndex) TR::Node(0,
    // op, numChildren);
...
TR::Node *OMR::Node::create(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
                            uint16_t numChildren) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  TR::Node *node =
      TR::Node::createInternal(originatingByteCodeNode, op, numChildren);
  return node;
}

TR::Node *OMR::Node::create(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
                            uint16_t numChildren, TR::TreeTop *dest) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  TR::Node *node =
      TR::Node::createInternal(originatingByteCodeNode, op, numChildren);
  if (dest) node->setBranchDestination(dest);
  return node;
}

TR::Node *OMR::Node::create(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
                            uint16_t numChildren, int32_t intValue,
                            TR::TreeTop *dest) {
  TR_ASSERT(op != TR::bconst && op != TR::cconst && op != TR::sconst,
            "Invalid constructor for 8/16-bit constants");
  TR::Node *node =
      TR::Node::create(originatingByteCodeNode, op, numChildren, dest);
  if (op == TR::lconst) {
...
TR::Node *OMR::Node::create(TR::Node *originatingByteCodeNode, TR::ILOpCodes op,
                            uint16_t numChildren, TR::Node *first) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  TR::Node *node =
      TR::Node::createInternal(originatingByteCodeNode, op, numChildren);
...
TR::Node *OMR::Node::createWithSymRef(TR::Node *originatingByteCodeNode,
                                      TR::ILOpCodes op, uint16_t numChildren,
...
TR::Node *OMR::Node::createWithSymRef(TR::Node *originatingByteCodeNode,
                                      ...
                                      TR::SymbolReference *symRef) {
  TR::Node *node =
      TR::Node::create(originatingByteCodeNode, op, numChildren, first);
  node->setSymbolReference(symRef);
  return node;
}

TR::Node *OMR::Node::createOnStack(TR::Node *originatingByteCodeNode,
                                   TR::ILOpCodes op, uint16_t numChildren) {
  return TR::Node::createInternal(originatingByteCodeNode, op, numChildren);
}

TR::Node *OMR::Node::recreateWithoutProperties(TR::Node *originalNode,
                                               TR::ILOpCodes op,
                                               uint16_t numChildren,
                                               TR::SymbolReference *symRef) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  TR::Node *node = TR::Node::createInternal(0, op, numChildren, originalNode);
  if (symRef != NULL || node->hasSymbolReference() ||
      node->hasRegLoadStoreSymbolReference()) {
    // don't attempt to set this if the symRef is NULL and the node isn't a
    // symRef type; otherwise attempt to do so
    node->setSymbolReference(symRef);
  }
  return node;
}

TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren) {
...
TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren,
                            int32_t intValue, TR::TreeTop *dest) {
  return TR::Node::create(0, op, numChildren, intValue, dest);
}

TR::Node *OMR::Node::createWithSymRef(TR::ILOpCodes op, uint16_t numChildren,
                                      TR::Node *first, uintptr_t extraChildren,
                                      TR::SymbolReference *symRef) {
  uint16_t numChildrenRoom =
      numChildren + (uint16_t)extraChildren;  // explictly cast to avoid warning
  TR::Node *node =
      TR::Node::createWithSymRef(op, numChildrenRoom, 1, first, symRef);
...
TR::Node *OMR::Node::createWithSymRef(TR::ILOpCodes op, uint16_t numChildren,
                                      ...
                                      uintptr_t extraChildren,
                                      TR::SymbolReference *symRef) {
  TR::Node *node =
      TR::Node::createWithSymRef(op, numChildren, first, extraChildren,
                                 symRef);  // explictly cast to avoid warning
...
TR::Node *OMR::Node::createWithSymRef(TR::ILOpCodes op, uint16_t numChildren,
  ...
  uint16_t numChildrenRoom =
      numChildren +
      (uint16_t)extraChildrenForFixup;  // explictly cast to avoid warning
...
TR::Node *OMR::Node::recreateWithoutSymRef_va_args(TR::Node *originalNode,
                                                   TR::ILOpCodes op,
                                                   uint16_t numChildren,
                                                   uint16_t numChildArgs,
                                                   va_list &args) {
  TR_ASSERT(numChildArgs > 0,
            "must be called with at least one child, but numChildArgs = %d",
            numChildArgs);
  TR::Node *first = va_arg(args, TR::Node *);
  TR_ASSERT(first != NULL, "child %d must be non NULL", 0);
  TR::Node *node =
      TR::Node::createInternal(first, op, numChildren, originalNode);
  node->setAndIncChild(0, first);
...
TR::Node *OMR::Node::createWithoutSymRef(TR::ILOpCodes op, uint16_t numChildren,
                                         uint16_t numChildArgs, ...) {
  va_list args;
  va_start(args, numChildArgs);
  TR::Node *node = TR::Node::recreateWithoutSymRef_va_args(0, op, numChildren,
                                                           numChildArgs, args);
  va_end(args);
  return node;
}

TR::Node *OMR::Node::recreateWithoutSymRef(TR::Node *originalNode,
                                           TR::ILOpCodes op,
                                           uint16_t numChildren,
                                           uint16_t numChildArgs, ...) {
  va_list args;
  va_start(args, numChildArgs);
  TR::Node *node = TR::Node::recreateWithoutSymRef_va_args(
      originalNode, op, numChildren, numChildArgs, args);
  va_end(args);
  return node;
}

TR::Node *OMR::Node::recreateWithSymRefWithoutProperties(TR::Node *originalNode,
                                                         TR::ILOpCodes op,
                                                         uint16_t numChildren,
                                                         uint16_t numChildArgs,
                                                         ...) {
...
uint16_t OMR::Node::addChildrenAndSymRef(uint16_t lastIndex,
                                         TR::SymbolReference *symRef) {
...
TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren,
                            ...
                            TR::Node *third) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::createWithoutSymRef(op, numChildren, 3, first, second,
                                       third);
}
...
TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren,
                            ...
                            TR::Node *fourth) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::createWithoutSymRef(op, numChildren, 4, first, second, third,
                                       fourth);
}
...
TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren,
  ...
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::createWithoutSymRef(op, numChildren, 5, first, second, third,
                                       fourth, fifth);
}
...
TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren,
                            ...
                            TR::Node *sixth) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::createWithoutSymRef(op, numChildren, 6, first, second, third,
                                       fourth, fifth, sixth);
}
...
TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren,
                            ...
                            TR::Node *seventh) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::createWithoutSymRef(op, numChildren, 7, first, second, third,
...
TR::Node *OMR::Node::create(TR::ILOpCodes op, uint16_t numChildren,
  ...
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::createWithoutSymRef(op, numChildren, 8, first, second, third,
...
TR::Node *OMR::Node::recreateWithoutProperties(TR::Node *originalNode,
                                               ...
                                               TR::Node *first) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 1,
                                         first);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(TR::Node *originalNode,
                                               ...
                                               TR::Node *second) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 2,
                                         first, second);
}

TR::Node *OMR::Node::recreateWithoutProperties(
    TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren,
    TR::Node *first, TR::Node *second, TR::Node *third) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 3,
                                         first, second, third);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
  ...
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 4,
...
TR::Node *OMR::Node::recreateWithoutProperties(
    ...
    TR::Node *fifth) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 5,
...
TR::Node *OMR::Node::recreateWithoutProperties(
    TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren,
    TR::Node *first, TR::Node *second, TR::Node *third, TR::Node *fourth,
    TR::Node *fifth, TR::Node *sixth) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 6,
                                         first, second, third, fourth, fifth,
                                         sixth);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
    TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren,
    TR::Node *first, TR::Node *second, TR::Node *third, TR::Node *fourth,
    TR::Node *fifth, TR::Node *sixth, TR::Node *seventh) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 7,
                                         first, second, third, fourth, fifth,
                                         sixth, seventh);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
    TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren,
    TR::Node *first, TR::Node *second, TR::Node *third, TR::Node *fourth,
    TR::Node *fifth, TR::Node *sixth, TR::Node *seventh, TR::Node *eighth) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithoutSymRef(originalNode, op, numChildren, 8,
                                         first, second, third, fourth, fifth,
...
TR::Node *OMR::Node::recreateWithoutProperties(TR::Node *originalNode,
  ...
  return TR::Node::recreateWithSymRefWithoutProperties(
      originalNode, op, numChildren, 1, first, symRef);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
  ...
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithSymRefWithoutProperties(
      originalNode, op, numChildren, 2, first, second, symRef);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
    TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren,
    TR::Node *first, TR::Node *second, TR::Node *third, TR::Node *fourth,
    TR::Node *fifth, TR::SymbolReference *symRef) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithSymRefWithoutProperties(
      originalNode, op, numChildren, 5, first, second, third, fourth, fifth,
      symRef);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
    TR::Node *originalNode, TR::ILOpCodes op, uint16_t numChildren,
    TR::Node *first, TR::Node *second, TR::Node *third, TR::Node *fourth,
    TR::Node *fifth, TR::Node *sixth, TR::SymbolReference *symRef) {
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithSymRefWithoutProperties(
      originalNode, op, numChildren, 6, first, second, third, fourth, fifth,
      sixth, symRef);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
  ...
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithSymRefWithoutProperties(
      originalNode, op, numChildren, 7, first, second, third, fourth, fifth,
      sixth, seventh, symRef);
}
...
TR::Node *OMR::Node::recreateWithoutProperties(
  ...
  TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
  return TR::Node::recreateWithSymRefWithoutProperties(
      originalNode, op, numChildren, 8, first, second, third, fourth, fifth,
      sixth, seventh, eighth, symRef);
}

TR::Node *OMR::Node::createWithRoomForOneMore(TR::ILOpCodes op,
                                              uint16_t numChildren,
                                              void *symbolRefOrBranchTarget,
                                              TR::Node *first, TR::Node *second,
                                              TR::Node *third, TR::Node *fourth,
                                              TR::Node *fifth) {
  TR::Node *node =
      TR::Node::createWithoutSymRef(op, numChildren, 2, first, second);
  node->addExtensionElements(1);
  if (symbolRefOrBranchTarget != NULL || node->hasSymbolReference() ||
      node->hasBranchDestinationNode()) {
    // don't attempt to set this if the symbolRefOrBranchTarget is NULL and the
    // node isn't one of the valid types
    if (node->hasSymbolReference())
      ...
      TR_ASSERT(
          false,
          "attempting to access _branchDestinationNode or _symbolReference "
          "field for node %s %p that does not have one of them",
          node->getOpCode().getName(), node);
  }

  if (third) node->setAndIncChild(2, third);
  if (fourth) node->setAndIncChild(3, fourth);
  if (fifth) node->setAndIncChild(4, fifth);
  node->setChild(numChildren, 0);  // clear (numChildren+1)th child
  return node;
}

TR::Node *OMR::Node::createWithRoomForThree(TR::ILOpCodes op, TR::Node *first,
                                            TR::Node *second,
                                            void *symbolRefOrBranchTarget) {
  // So far extra 3rd child could be added to the following nodes: if,
  // variableNewArray, arraystorechk or a node with a padding address these
  // nodes are created with room for three children
  TR::Node *node;
  ...
  TR_ASSERT(
      node->getOpCode().isIf() ||
          (node->getOpCodeValue() == TR::variableNewArray) ||
          node->getOpCode().canHavePaddingAddress() ||
          node->getOpCodeValue() == TR::ArrayStoreCHK,
      "createWithRoomForThree should only be used to create an ifcmp, "
      "arraystorechk, variableNewArray node or a node with a padding address");
#else
  TR_ASSERT(
      node->getOpCode().isIf() ||
          (node->getOpCodeValue() == TR::variableNewArray),
...
TR::Node *OMR::Node::createWithRoomForFive(TR::ILOpCodes op, TR::Node *first,
                                           TR::Node *second, TR::Node *third,
                                           TR::Node *fourth,
                                           void *symbolRefOrBranchTarget) {
  TR_ASSERT((first != NULL) && (second != NULL) && (third != NULL) &&
                (fourth != NULL),
            "must supply all four children");
  return TR::Node::createWithRoomForOneMore(op, 4, symbolRefOrBranchTarget,
                                            first, second, third, fourth);
}

// An 'if' node is created with room for 3 children.
TR::Node *OMR::Node::createif(TR::ILOpCodes op, TR::Node *first,
                              TR::Node *second, TR::TreeTop *branchTarget) {
  // An 'if' node is created with room for 3 children.
  // The third child may be set to point at a GlRegDep node
  //
  return TR::Node::createWithRoomForThree(op, first, second,
                                          (void *)branchTarget);
}

TR::Node *OMR::Node::createbranch(TR::ILOpCodes op, TR::Node *first,
                                  TR::TreeTop *branchTarget) {
  // So far extra 3rd child could be added to the following nodes: if, newarray,
  // anewarray these nodes are created with room for three children
...
TR::Node *OMR::Node::createCase(TR::Node *originatingByteCodeNode,
                                TR::TreeTop *destination,
                                CASECONST_TYPE caseConstant) {
  TR::Node *caseNode =
      TR::Node::createInternal(originatingByteCodeNode, TR::Case, 0);
...
/**
 * May be called with first and/or second NULL, but if 3rd and higher children
 * are specified they must be non-NULL
 */
TR::Node *OMR::Node::createArrayOperation(TR::ILOpCodes arrayOp,
                                          TR::Node *first, TR::Node *second,
                                          TR::Node *third, TR::Node *fourth,
                                          TR::Node *fifth) {
  uint16_t numChildren = 3;
  if (fourth) numChildren = 4;
  if (fifth) numChildren = 5;

  return TR::Node::createWithRoomForOneMore(arrayOp, numChildren, 0, first,
...
TR::Node *OMR::Node::createArraycopy() {
  ...
  node->addExtensionElements(1);
  node->setArrayCopyElementType(
      TR::Int8);  // can be reset by consumer but don't leave this uninitialized
  node->setSymbolReference(
      TR::comp()->getSymRefTab()->findOrCreateArrayCopySymbol());

  return node;
}

TR::Node *OMR::Node::createArraycopy(TR::Node *first, TR::Node *second,
                                     TR::Node *third) {
  TR::Node *node =
      TR::Node::createArrayOperation(TR::arraycopy, first, second, third);
  node->setArrayCopyElementType(
      TR::Int8);  // can be reset by consumer but don't leave this uninitialized
  node->setSymbolReference(
      TR::comp()->getSymRefTab()->findOrCreateArrayCopySymbol());
...
TR::Node *OMR::Node::createArraycopy(TR::Node *first, TR::Node *second,
                                     TR::Node *third, TR::Node *fourth,
                                     TR::Node *fifth) {
  TR::Node *node = TR::Node::createArrayOperation(TR::arraycopy, first, second,
                                                  third, fourth, fifth);
  node->setSymbolReference(
      TR::comp()->getSymRefTab()->findOrCreateArrayCopySymbol());
  return node;
...
TR::Node *OMR::Node::createPotentialOSRPointHelperCallInILGen(
  ...
  //
  TR_ASSERT(comp->getCurrentIlGenerator(),
            "This API must be called during ILGen");
  TR_ASSERT(!comp->isPeekingMethod(),
            "Can not generate the helper call during peeking");
  TR_ASSERT(comp->supportsInduceOSR(),
            "Can not create the helper without OSR support");

  TR::Node *callNode = TR::Node::createWithSymRef(
      originatingByteCodeNode, TR::call, 0,
      TR::comp()
          ->getSymRefTab()
          ->findOrCreatePotentialOSRPointHelperSymbolRef());
  callNode->setOSRInductionOffset(osrInductionOffset);

  // Node created outside of ilgen will have doNotProfile set, this results in
  // OSR infrastructure believing it can not OSR at this node. Reset this flag
  // since a potentialOSRPointHelper is a safe OSR transition point
  //
  callNode->getByteCodeInfo().setDoNotProfile(0);
  return callNode;
}

TR::Node *OMR::Node::createOSRFearPointHelperCall(
    TR::Node *originatingByteCodeNode) {
  TR::Compilation *comp = TR::comp();
...
TR::Node *OMR::Node::createLoad(TR::SymbolReference *symRef) {
  return TR::Node::createLoad(0, symRef);
}

TR::Node *OMR::Node::createLoad(TR::Node *originatingByteCodeNode,
                                TR::SymbolReference *symRef) {
  TR::Node *load = TR::Node::createWithSymRef(
      originatingByteCodeNode,
      TR::comp()->il.opCodeForDirectLoad(symRef->getSymbol()->getDataType()), 0,
      symRef);
...
TR::Node *OMR::Node::createStore(TR::SymbolReference *symRef, TR::Node *value,
                                 TR::ILOpCodes op) {
  return TR::Node::createStore(symRef, value, op, 0);
}

TR::Node *OMR::Node::createStore(TR::SymbolReference *symRef, TR::Node *value,
                                 TR::ILOpCodes op, size_t size) {
  TR::Node *store = TR::Node::createWithSymRef(op, 1, 1, value, symRef);
  return store;
}

TR::Node *OMR::Node::createStore(TR::Node *originatingByteCodeNode,
                                 TR::SymbolReference *symRef, TR::Node *value) {
  TR::DataType type = symRef->getSymbol()->getDataType();
  TR::ILOpCodes op = TR::comp()->il.opCodeForDirectStore(type);
  return TR::Node::createWithSymRef(originatingByteCodeNode, op, 1, value,
                                    symRef);
}
...
TR::Node *OMR::Node::createRelative32BitFenceNode(
    TR::Node *originatingByteCodeNode, void *relocationAddress) {
  TR::Node *node = TR::Node::createInternal(originatingByteCodeNode,
                                            TR::exceptionRangeFence, 0);
...
TR::Node *OMR::Node::createAddressNode(TR::Node *originatingByteCodeNode,
                                       TR::ILOpCodes op, uintptrj_t address,
                                       uint8_t precision) {
...
TR::Node *OMR::Node::createAllocationFence(TR::Node *originatingByteCodeNode,
                                           TR::Node *fenceNode) {
  TR::Node *node =
      TR::Node::create(originatingByteCodeNode, TR::allocationFence, 0, 0);
...
TR::Node *OMR::Node::buconst(TR::Node *originatingByteCodeNode, uint8_t val) {
  TR::Node *r = TR::Node::create(originatingByteCodeNode, TR::buconst);
  r->setUnsignedByte(val);
  return r;
}

TR::Node *OMR::Node::buconst(uint8_t val) { return TR::Node::buconst(0, val); }
...
TR::Node *OMR::Node::cconst(TR::Node *originatingByteCodeNode, uint16_t val) {
  TR::Node *r = TR::Node::create(originatingByteCodeNode, TR::cconst);
  r->setUnsignedShortInt(val);
  return r;
}

TR::Node *OMR::Node::cconst(uint16_t val) { return TR::Node::cconst(0, val); }
...
TR::Node *OMR::Node::iuconst(TR::Node *originatingByteCodeNode, uint32_t val) {
  return TR::Node::create(originatingByteCodeNode, TR::iuconst, 0,
                          (int32_t)val);
}

TR::Node *OMR::Node::iuconst(uint32_t val) { return TR::Node::iuconst(0, val); }
...
TR::Node *OMR::Node::luconst(TR::Node *originatingByteCodeNode, uint64_t val) {
  TR::Node *r = TR::Node::create(originatingByteCodeNode, TR::luconst);
  r->setUnsignedLongInt(val);
  return r;
}

TR::Node *OMR::Node::luconst(uint64_t val) { return TR::Node::luconst(0, val); }

TR::Node *OMR::Node::aconst(TR::Node *originatingByteCodeNode, uintptrj_t val) {
...
TR::Node *OMR::Node::aconst(uintptrj_t val) { return TR::Node::aconst(0, val); }

TR::Node *OMR::Node::createConstZeroValue(TR::Node *originatingByteCodeNode,
                                          TR::DataType dt) {
  ...
  switch (dt) {
    ...
    default:
      TR_ASSERT_SAFE_FATAL(
          false, "datatype %s not supported for createConstZeroValue\n",
          dt.toString());
  }
  return constZero;
}

TR::Node *OMR::Node::createConstOne(TR::Node *originatingByteCodeNode,
                                    TR::DataType dt) {
  ...
  switch (dt) {
    ...
    case TR::Double:
      ...
      break;
    default:
      TR_ASSERT_SAFE_FATAL(false,
                           "datatype %s not supported for createConstOne\n",
                           dt.toString());
  }
  return constOne;
}

TR::Node *OMR::Node::createConstDead(TR::Node *originatingByteCodeNode,
                                     TR::DataType dt, intptrj_t extraData) {
  TR::Node *result = NULL;
  ...
  switch (dt) {
    ...
    case TR::Int64:
      ...
      break;
    case TR::Address:
      // Just because a reference is dead doesn't mean we can fill it with
      // garbage
    ...
    default:
      TR_ASSERT_SAFE_FATAL(false,
                           "datatype %s not supported for createConstDead\n",
                           dt.toString());
  }
  return result;
}
...
TR::Node *OMR::Node::createAddConstantToAddress(TR::Node *addr, intptr_t value,
                                                TR::Node *parent) {
  TR::Node *ret = NULL;
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    ret->setAndIncChild(0, addr);
    ret->setAndIncChild(
        1, TR::Node::create(parentOfNewNode, TR::iconst, 0, value));
  }
...
TR::Node *OMR::Node::createVectorConst(TR::Node *originatingByteCodeNode,
                                       TR::DataType dt) {
  TR::Node *node =
      TR::Node::createInternal(originatingByteCodeNode, TR::vconst, 0, NULL);
  node->setDataType(dt);
  return node;
}

// v2v is vector conversion that preserves bit-pattern, effectively a noop that
// only changes datatype.
TR::Node *OMR::Node::createVectorConversion(TR::Node *src,
                                            TR::DataType trgType) {
...
/**
 * @return true if searchNode is one of the children of this node, return false
 * otherwise
 */
bool OMR::Node::hasChild(TR::Node *searchNode) {
  ...
  for (uint32_t i = 0; i < self()->getNumChildren(); i++) {
    if (searchNode == self()->getChild(i)) return true;
  }
  return false;
}
...
void OMR::Node::addChildren(TR::Node **extraChildren, uint16_t num) {
  ...
  if (self()->hasNodeExtension()) {
    ...
    self()->copyNodeExtension(_unionBase._extension.getExtensionPtr(), numElems,
                              self()->sizeOfExtension());
  } else {
...
TR::Node *OMR::Node::setAndIncChild(int32_t c, TR::Node *p) {
  TR_ASSERT(self()->getOpCodeValue() != TR::BBStart || c != 1,
            "Can't use second child of TR::BBStart %p", this);
...
TR::Node *OMR::Node::duplicateTreeWithCommoningImpl(
    CS2::HashTable<TR::Node *, TR::Node *, TR::Allocator> &nodeMapping) {
  ...
  for (int32_t i = 0; i < self()->getNumChildren(); ++i) {
    to->setAndIncChild(
        i, self()->getChild(i)->duplicateTreeWithCommoningImpl(nodeMapping));
  }
...
TR::Node *OMR::Node::duplicateTree(bool duplicateChildren) {
  ...
  // Would like to enable this, but some optimizations (e,g. Field
  // Privatization) seem to rely on the old behaviour of duplicateTree TR::Node
  // *safeNode = safeDuplicateTree(duplicateChildren);

  // traceMsg(TR::comp(), "--------------\n duplicateTree duplicating %p
  // unsafeNode %p safeNode %p duplicateChildren = %d\n
...
/**
 * Method to duplicate an entire subtree. Tries to do so 'safely' by maintaining
 * a mapping of visited (and duplicated) nodes
 */
...
TR::Node *OMR::Node::duplicateTree_DEPRECATED(bool duplicateChildren) {
  ...
  if (newRoot->getOpCode().isStoreReg() || newRoot->getOpCode().isLoadReg()) {
    if (newRoot->requiresRegisterPair(comp)) {
      ...
      newRoot->setHighGlobalRegisterNumber(
          self()->getHighGlobalRegisterNumber());
    } else {
  ...
  for (int32_t i = 0; i < self()->getNumChildren(); i++) {
    ...
    if (child) {
      TR::Node *newChild =
          duplicateChildren ? child->duplicateTree_DEPRECATED() : child;
...
bool OMR::Node::isUnsafeToDuplicateAndExecuteAgain(int32_t *nodeVisitBudget) {
  ...
  if ((*nodeVisitBudget) <= 0)
    return true;  // Conservative answer because we can't affort to check all
                  // children
  ...
  if (self()->getOpCode().hasSymbolReference()) {
    if (self()->getSymbolReference()->isUnresolved()) {
      ...
      // Despite the symref, loadaddr is really like a constant, and is safe to
      // re-execute.
    } else if (self()->getOpCode().isLoadVarDirect()) {
      // Multiple loads of the same variable can return different values, which
      // is particularly problematic if the variable is an address to be
      // dereferenced by the containing tree. We'll aggressively call this
      // "safe" here, but the caller must ensure that loads within the
      // duplicated tree can't return something unsafe.
    } else if (self()->getOpCode().isLoadIndirect()) {
      switch (self()->getSymbolReference()->getReferenceNumber() -
      ...
      // Things with symrefs generally can have side-effects, so it is not safe
      // to run them twice.
      //
      return true;
    }
  } else {
    // Things without symrefs generally have no side-effects, so it is safe to
    // run them twice.
  }
  ...
  for (int32_t i = 0; !unsafeChildFound && i < self()->getNumChildren(); i++)
    unsafeChildFound = self()->getChild(i)->isUnsafeToDuplicateAndExecuteAgain(
        nodeVisitBudget);
 ...
 *     1) If the visit count is the same as the given visit count, we must not
 * look into the subtree. 2) We must maintain the visit counts of the nodes
 * visited; to do this we use the next available visit count, i.e.
 * (visitCount+1)
 */
bool OMR::Node::containsNode(TR::Node *searchNode, vcount_t visitCount) {
  if (self() == searchNode) return true;

  vcount_t oldVisitCount = self()->getVisitCount();
  if (oldVisitCount == visitCount) return false;
  self()->setVisitCount(visitCount);

  for (int i = 0; i < self()->getNumChildren(); i++) {
    if (self()->getChild(i)->containsNode(searchNode, visitCount)) return true;
  }
...
bool OMR::Node::hasUnresolvedSymbolReference() {
  return self()->getOpCode().hasSymbolReference() &&
         self()->getSymbolReference()->isUnresolved();
}
...
bool OMR::Node::isThisPointer() {
  return self()->getOpCode().hasSymbolReference() &&
         self()->getSymbolReference()->isThisPointer();
}
 ...
 *        pairSecondChild
 *
 * and the opcodes for highOp/adjunctOp are lumulh/lmul, luaddh/luadd, or
 * lusubh/lusub.
 */
bool OMR::Node::isDualHigh() {
  if ((self()->getNumChildren() == 3) && self()->getChild(2)) {
    TR::ILOpCodes pairOpValue = self()->getChild(2)->getOpCodeValue();
    if (((self()->getOpCodeValue() == TR::lumulh) &&
         (pairOpValue == TR::lmul)) ||
        ((self()->getOpCodeValue() == TR::luaddh) &&
         (pairOpValue == TR::luadd)) ||
        ((self()->getOpCodeValue() == TR::lusubh) &&
         (pairOpValue == TR::lusub)) ||
        ((self()->getOpCodeValue() == TR::luaddh) &&
         (pairOpValue == TR::ladd)) ||
...
/**
 * Whether this node is high part of a ternary subtract or addition, like a dual
 * this is a composite operator, made from a high order part and its adjunct
 * operator which is the first child of its third child. It returns true if the
 * node has the form:
 *
 *    highOp
 *      firstChild
 ...
 */
bool OMR::Node::isTernaryHigh() {
  if (((self()->getOpCodeValue() == TR::luaddc) ||
       (self()->getOpCodeValue() == TR::lusubb)) &&
      (self()->getNumChildren() == 3) && self()->getChild(2) &&
      (self()->getChild(2)->getNumChildren() == 1) &&
      self()->getChild(2)->getFirstChild()) {
    TR::ILOpCodes ccOpValue = self()->getChild(2)->getOpCodeValue();
    TR::ILOpCodes pairOpValue =
        self()->getChild(2)->getFirstChild()->getOpCodeValue();
    if ((ccOpValue == TR::computeCC) &&
        (((self()->getOpCodeValue() == TR::luaddc) &&
          (pairOpValue == TR::luadd)) ||
         ((self()->getOpCodeValue() == TR::lusubb) &&
          (pairOpValue == TR::lusub)) ||
         ((self()->getOpCodeValue() == TR::luaddc) &&
...
/**
 * Whether this node is the high or low part of a "dual", in cyclic
 * representation. ie it represents a composite operator, together with its
 * pair. The node and its pair have each other as its third child, completing
 * the cycle. It returns true if the node has the form:
 *
 *    node
 *      firstChild
 *      secondChild
...
bool OMR::Node::isConstZeroBytes() {
  if (!self()->getOpCode().isLoadConst()) return false;
  ...
  switch (self()->getDataType()) {
    ...
    default:
      TR_ASSERT(false,
                "Unrecognized const node %s can't be checked for zero bytes");
      return false;
  }
}
...
/**
 * The typical use is to allow a simple recursive decrement to be done vs a full
 * evaluation (or anchoring) of an address tree that turns out not to be
 * required at tree evaluation (or tree simplifiation) time
 *
...
bool OMR::Node::canGCandReturn() {
  ...
  if (self()->getOpCode().isResolveCheck()) {
    ...
    if (node->getOpCode().isIndirect() &&
      ...
      return false;
    if (node->getOpCodeValue() == TR::arraycopy) return false;
    return true;
  }
...
bool OMR::Node::canGCandExcept() {
  ...
  //
  TR::Node *node = (self()->getOpCodeValue() == TR::treetop)
                       ? self()->getFirstChild()
                       : self();
...
bool OMR::Node::dontEliminateStores(bool isForLocalDeadStore) {
  TR::Compilation *comp = TR::comp();
  if (self()->getSymbolReference()->getSymbol()->dontEliminateStores(
          comp, isForLocalDeadStore) ||
 ...
 *
 * \note
 *    This query currently only works on opcodes that can have a pinning array
 * pointer, i.e. aiadd, aiuadd, aladd, aluadd. Suppport for other opcodes can be
 * added if necessary.
 */
bool OMR::Node::computeIsInternalPointer() {
  TR_ASSERT(self()->getOpCode().hasPinningArrayPointer(),
            "Opcode %s is not supported, node is " POINTER_PRINTF_FORMAT,
 ...
 *
 * \note
 *    This query traverses down the children of the node and try to find if the
 * node originates from something collected, i.e. anything that is collected
 * reference or internal pointer.
 */
bool OMR::Node::computeIsCollectedReference() {
  ...
  return (self()->computeIsCollectedReferenceImpl(
              processedNodesCollected, processedNodesNotCollected) != TR_no);
}

static TR_YesNoMaybe recordProcessedNodeResult(
    TR::Node *node, TR_YesNoMaybe collectedness,
    TR::NodeChecklist &processedNodesCollected,
...
TR_YesNoMaybe OMR::Node::computeIsCollectedReferenceImpl(
  ...
  if (curNode->getOpCode().isTreeTop()) return TR_no;
  ...
  // -- The node is not contained in either collected or uncollected checklists
  // - first time encounter, process the node
  //    and add it to the appropriate checklist(s) based on the result.
  // -- Node is seen in both checklitsts - previously processed with result of
  // TR_maybe
  // -- Node is seen in processedNodesCollected - previously processed with
  // result of TR_yes
  // -- Node is seen in processedNodesNotCollected - previously processed with
  // result of TR_no
  bool ternarySeenCollected = processedNodesCollected.contains(receiverNode);
  bool ternarySeenNotCollected =
      processedNodesNotCollected.contains(receiverNode);
  ...
  while (curNode) {
    if (curNode->isInternalPointer())
      return recordProcessedNodeResult(receiverNode, TR_yes,
                                       processedNodesCollected,
                                       processedNodesNotCollected);

    TR::ILOpCode op = curNode->getOpCode();
    TR::ILOpCodes opValue = curNode->getOpCodeValue();

    // If a language can handle a collected reference going via a non-address
    // type, then the logic would need to be augmented to handle that
    if (op.isConversion())
      return recordProcessedNodeResult(receiverNode, TR_no,
                                       processedNodesCollected,
                                       processedNodesNotCollected);
    ...
    if (op.getDataType() != TR::Address)
      return recordProcessedNodeResult(receiverNode, TR_no,
                                       processedNodesCollected,
                                       processedNodesNotCollected);
    // The following are all opcodes that are address type, non-TreeTop and
    // non-conversion
    ...
    if (op.isTernary()) {
      TR_YesNoMaybe secondChildResult =
          curNode->getSecondChild()->computeIsCollectedReferenceImpl(
              processedNodesCollected, processedNodesNotCollected);
      if (TR_maybe == secondChildResult) {
        TR_YesNoMaybe thirdChildResult =
            curNode->getThirdChild()->computeIsCollectedReferenceImpl(
                processedNodesCollected, processedNodesNotCollected);
        return recordProcessedNodeResult(receiverNode, thirdChildResult,
                                         processedNodesCollected,
                                         processedNodesNotCollected);
      } else
        return recordProcessedNodeResult(receiverNode, secondChildResult,
                                         processedNodesCollected,
                                         processedNodesNotCollected);
    }
    ...
    if (op.isLoadVar() || op.isLoadAddr() || op.isLoadReg()) {
      TR_ASSERT(curNode->hasSymbolReference() ||
                    curNode->hasRegLoadStoreSymbolReference(),
                "node " POINTER_PRINTF_FORMAT
                "(%s) should have symbol reference",
                curNode, op.getName());
      TR::Symbol *symbol = curNode->getSymbolReference()->getSymbol();
      // isCollectedReference() responds false to generic int shadows because
      // their type is int. However, address type generic int shadows refer to
      // collected slots.
      if (opValue == TR::aloadi &&
          symbol == TR::comp()->getSymRefTab()->findGenericIntShadowSymbol())
        return recordProcessedNodeResult(receiverNode, TR_yes,
                                         processedNodesCollected,
                                         processedNodesNotCollected);
      else
        return recordProcessedNodeResult(
            receiverNode, (symbol->isCollectedReference() ? TR_yes : TR_no),
            processedNodesCollected, processedNodesNotCollected);
    }
    ...
    if (op.isNew() || op.isCall() || opValue == TR::variableNew ||
        opValue == TR::variableNewArray)
      return recordProcessedNodeResult(receiverNode, TR_yes,
                                       processedNodesCollected,
                                       processedNodesNotCollected);
    ...
    switch (opValue) {
      case TR::aconst:
        // aconst null can either be collected or uncollected. The collectedness
        // of aconst null can be determined by usedef analysis, which is too
        // expensive for a very basic query. Treating aconst null as a collected
        // reference is always safe. The only problem is: we might mark an
        // aladd/aiadd on a temp whose value is null as an internal pointer,
        // thus creating a temp, pinning array and internal pointer map for it.
        ...
        if (curNode->getAddress() != 0)
          ...
          // Null constant under ternary (i.e. we reached here via recursive
          // calls), return maybe to indicate need to check the other child.
          if (self() != curNode)
            return recordProcessedNodeResult(receiverNode, TR_no,
                                             processedNodesCollected,
                                             processedNodesNotCollected);
          else
            return recordProcessedNodeResult(receiverNode, TR_maybe,
                                             processedNodesCollected,
                                             processedNodesNotCollected);
        }
      case TR::getstack:
        return recordProcessedNodeResult(receiverNode, TR_no,
                                         ...
                                         processedNodesNotCollected);
      default:
        TR_ASSERT(false, "Unsupported opcode %s on node " POINTER_PRINTF_FORMAT,
                  op.getName(), curNode);
        return TR_no;
    }
  }
...
bool OMR::Node::addressPointsAtObject() {
  ...
  TR_ASSERT(self()->getDataType() == TR::Address,
            "addressPointsAtObject should only be called for nodes whose data "
            "type is TR::Address; node is " POINTER_PRINTF_FORMAT,
            self());

  if (self()->getOpCodeValue() == TR::aconst) return false;

  if (self()->getOpCode().hasSymbolReference() &&
      (comp->getSymRefTab()->isVtableEntrySymbolRef(
...
bool OMR::Node::collectSymbolReferencesInNode(
    TR_BitVector &symbolReferencesInNode, vcount_t visitCount) {
  int32_t i;

  // The visit count in the node must be maintained by this method.
  //
  vcount_t oldVisitCount = self()->getVisitCount();
  if (oldVisitCount == visitCount) return true;
  self()->setVisitCount(visitCount);
  ...
  //
  if (self()->getOpCode().hasSymbolReference() &&
      self()->getOpCode().isLoadVar())
    symbolReferencesInNode.set(
 ...
 * The current visit count must be maintained by this method, i.e. all nodes
 * that have the current visit count must retain it and all nodes that do not
 * must not get it.
 */
bool OMR::Node::isSafeToReplaceNode(TR::TreeTop *curTreeTop) {
  TR::Compilation *c = TR::comp();
  // Collect all symbols that could be killed between here and the next
  // reference
  //
  TR_BitVector symbolReferencesInNode(c->getSymRefCount(), c->trMemory(),
                                      stackAlloc);
  c->incVisitCount();
  ...
  c->incVisitCount();
  for (TR::TreeTop *treeTop = curTreeTop->getNextTreeTop(); treeTop;
...

  {
    ...
    if (node->containsNode(self(), c->getVisitCount())) return true;
    ...
    //
    if (definingSymRef && definingSymRef->getUseDefAliases().containsAny(
                              symbolReferencesInNode, c))
      return false;
  }
...
bool OMR::Node::isl2aForCompressedArrayletLeafLoad() {
  if (self()->getOpCodeValue() != TR::l2a) return false;

  TR::Node *node = self()->getFirstChild();
  if (node->getOpCodeValue() == TR::lshl) node = node->getFirstChild();

  if (node->getOpCodeValue() != TR::iu2l) return false;

  node = node->getFirstChild();
  if (node->getOpCodeValue() != TR::iloadi) return false;
...
bool OMR::Node::mayModifyValue(TR::SymbolReference *symRef) {
  ...
  // aliasing information is generated.  It should be commoned with the code in
  // isSafeToReplace so that if the aliasing information is available it's used
  // otherwise, it uses the existing algorithm.
  //

  bool symbolIsUnresolved = false;
  TR::Node *node = self();
  if (self()->getOpCodeValue() == TR::treetop ||
      self()->getOpCode().isResolveOrNullCheck()) {
    if (self()->getOpCode().isResolveCheck()) symbolIsUnresolved = true;
    node = self()->getFirstChild();
  }
  ...
  if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monexit ||
    ...
    //
    return !symbol->isAutoOrParm() &&
           !(symbol->isStatic() && symbol->isConst()) &&
           !symbol->isMethodMetaData();
  }
  ...
  if (node->getOpCode().isStore()) {
    ...
    if (symbol->isAuto())
      return storeSymbol->isAuto() &&
             storeRef->getCPIndex() == symRef->getCPIndex();
    ...
    if (symbol->isStatic()) {
      if (symbol->isConst()) return false;

      if (storeSymbol->isStatic()) {
        if (symbol->getDataType() != storeSymbol->getDataType()) return false;
        TR::Compilation *c = TR::comp();
        if (symRef->isUnresolved() || storeRef->isUnresolved())
          return TR::Compiler->cls.jitStaticsAreSame(
              c, storeRef->getOwningMethod(c), storeRef->getCPIndex(),
    ...
    if (symbol->isShadow() && storeSymbol->isShadow()) {
      ...
      if (storeRef->getCPIndex() == -1) return false;  // its an array

      TR::Compilation *c = TR::comp();
      return TR::Compiler->cls.jitFieldsAreSame(
          c, storeRef->getOwningMethod(c), storeRef->getCPIndex(),
...
bool OMR::Node::performsVolatileAccess(vcount_t visitCount) {
  ...
  if (self()->getOpCode().hasSymbolReference()) {
    ...
    if (sym != NULL && sym->isVolatile()) foundVolatile = true;
  }
...
bool OMR::Node::isRematerializable(TR::Node *parent, bool onlyConsiderOpCode) {
...
#ifdef J9_PROJECT_SPECIFIC
  if (opCode.getDataType() == TR::PackedDecimal) return false;

  if (opCode.getOpCodeValue() == TR::pd2i ||
      opCode.getOpCodeValue() == TR::pd2iOverflow ||
      opCode.getOpCodeValue() == TR::pd2l ||
      opCode.getOpCodeValue() == TR::pd2lOverflow)
    return false;
#endif

  if ((opCodeValue == TR::lloadi) && self()->isBigDecimalLoad()) return true;
  ...
  //
  if (!comp->cg()->doRematerialization()) return false;
  ...
  if (opCode.isAdd() || opCode.isSub() || opCode.isMul() ||
      opCode.isLeftShift() || opCode.isRightShift() || opCode.isAnd() ||
      opCode.isOr() || opCode.isXor()) {
    ...
    if (!onlyConsiderOpCode) {
      ...
      if (firstChildLive && secondChildLive) return true;
    } else
      return true;
  }
  ...
  if (opCode.isLoadVarDirect()) {
    ...
    if (sym->isAutoOrParm()) return true;
  }
...
bool OMR::Node::isDoNotPropagateNode() {
  ...
  // 1) are impure (where their evaluation does not yield the same value at
  // different tree points)
...
bool OMR::Node::anchorConstChildren() {
  TR_ASSERT(self()->getOpCode().isLoadConst(),
            "constNeedsAnchor only valid for const nodes\n");
  if (self()->getNumChildren() >
      0)  // aggregate and BCD const nodes have an address child
  {
    TR_ASSERT(self()->getNumChildren() == 1,
              "expecting BCD/Aggr const node %p to have 1 child and not %d "
              "children\n",
...
bool OMR::Node::isZeroExtension() {
  if (self()->getOpCode().isZeroExtension()) return true;
...
bool OMR::Node::isPureCall() {
  ...
  return ((TR::MethodSymbol *)self()->getSymbolReference()->getSymbol())
      ->isPureFunction();
}
...
bool OMR::Node::isClassUnloadingConst() {
  ...
  if ((self()->getOpCodeValue() == TR::aloadi) &&
      (self()->getSymbolReference()->isLiteralPoolAddress()))
  ...
  if ((((constNode->isClassPointerConstant()) &&
        !TR::Compiler->cls.sameClassLoaders(
            c, (TR_OpaqueClassBlock *)constNode->getAddress(),
            c->getCurrentMethod()->classOfMethod())) ||
       ((constNode->isMethodPointerConstant()) &&
...
TR_OpaqueClassBlock *OMR::Node::getMonitorClass(TR_ResolvedMethod *vmMethod) {
#ifdef J9_PROJECT_SPECIFIC
  TR::Compilation *c = TR::comp();
  TR::Node *object = self()->getOpCodeValue() != TR::tstart
                         ? self()->getFirstChild()
                         : self()->getThirdChild();
  ...
  if (self()->hasMonitorClassInNode()) return self()->getMonitorClassInNode();

  if (object->getOpCode().hasSymbolReference()) {
    TR::SymbolReference *symRef = object->getSymbolReference();
    if (symRef->isThisPointer()) {
      TR_OpaqueClassBlock *clazz =
          (TR_OpaqueClassBlock *)vmMethod->classOfMethod();
      if (TR::Compiler->cls.classDepthOf(clazz) == 0) return 0;
      return clazz;
    }
    if (object->getOpCodeValue() == TR::loadaddr && !symRef->isUnresolved() &&
...
TR_OpaqueClassBlock *OMR::Node::getMonitorClassInNode() {
  TR_ASSERT(self()->hasMonitorClassInNode(),
            "Node %p does not have monitor class in Node");
  TR::Node *classz = (self()->getOpCodeValue() != TR::tstart)
                         ? self()->getChild(1)
                         : self()->getChild(4);
...
TR::Node *OMR::Node::getNullCheckReference() {
  TR_ASSERT((self()->getOpCode().isNullCheck() ||
             (self()->getOpCodeValue() == TR::checkcastAndNULLCHK) ||
             (self()->getOpCodeValue() == TR::ZEROCHK)),
            "Node::getNullCheckReference, expected NULLCHK node instead "
            "of " POINTER_PRINTF_FORMAT,
            self());
  ...
  if (firstChild->getNumChildren() == 0) return 0;
...
void OMR::Node::setNullCheckReference(TR::Node *refNode) {
  ...
  if (firstChild->getNumChildren() == 0) {
    TR_ASSERT(0,
              "Null check node " POINTER_PRINTF_FORMAT
              " does not have null check reference",
              self());
  }
...
int32_t OMR::Node::getFirstArgumentIndex() {
  TR_ASSERT(
      ...
      "getFirstArgumentIndex called for non-call node " POINTER_PRINTF_FORMAT,
      self());

  if (self()->getOpCode().isIndirect()) return 1;

  if (self()->getOpCodeValue() == TR::ZEROCHK)
    return 1;  // First argument is the int to compare with zero, not an arg to
               // the helper

  // commented out because a fair amount of code has assumptions on this
  // function returning 0 for static JNI
  //
...
uint32_t OMR::Node::getSize() {
  if (self()->getType().isAggregate()) {
    ...
    // opcodes without inherent datatypes will be 0 sized; get actual size from
    // their derived datatypes
...
int32_t OMR::Node::getMaxIntegerPrecision() {
  ...
  TR_ASSERT(
      false,
      "getMaxIntegerPrecision must only be called for an integral value\n");
  return -1;
}

uint32_t OMR::Node::getNumberOfSlots() {
...
uint16_t OMR::Node::getCaseIndexUpperBound() {
  ...
  while (n > 2 && !self()->getChild(n - 1)->getOpCode().isCase()) n--;
  return n;
}

TR::Node *OMR::Node::getStoreNode() {
  if (self()->getOpCode().isStore()) return self();
  if (self()->getNumChildren() > 0 &&
      self()->getFirstChild()->getOpCode().isStore())
    return self()->getFirstChild();
  return NULL;
}
...
TR_OpaqueMethodBlock *OMR::Node::getOwningMethod() {
  ...
  if (self()->getInlinedSiteIndex() >= 0) {
    TR_InlinedCallSite &ics =
        comp->getInlinedCallSite(self()->getInlinedSiteIndex());
    method = comp->fe()->getInlinedCallSiteMethod(&ics);
  } else {
    method = (TR_OpaqueMethodBlock *)(comp->getCurrentMethod()
...
void *OMR::Node::getAOTMethod() {
  ...
  if (index == -1) {
    ...
    TR_AOTMethodInfo *aotMethodInfo =
        (TR_AOTMethodInfo *)c->getInlinedCallSite(index)._methodInfo;
    return (void *)aotMethodInfo->resolvedMethod;
  }
}

const char *OMR::Node::getTypeSignature(int32_t &len,
...
/**
 * 'Value' of the node can change, affecting analysis results in the optimizer
 * such as value number This gives affected node (self) a new unique value
 * number.
 */
...
void OMR::Node::devirtualizeCall(TR::TreeTop *treeTop) {
  ...
  TR::ResolvedMethodSymbol *methodSymbol =
      self()->getSymbol()->castToResolvedMethodSymbol();
 ...
 * assumption is that we are calling it recursively on all the nodes. If the
 * children are evaluated in a register, this function will return true if the
 * evaluation of this node might kill the condition code.
 */
bool OMR::Node::nodeMightKillCondCode() {
  ...
  if (opcode.getOpCodeValue() == TR::BBStart ||
      opcode.getOpCodeValue() == TR::BBEnd)
    return false;

  // treetops don't do anything (we already checked the children in the for
  // loop)
  if (self()->getOpCodeValue() == TR::treetop) return false;

  // simple loads/stores won't kill the CC
  if ((opcode.isLoadReg() || opcode.isStoreReg() || opcode.isLoadDirect() ||
       opcode.isStoreDirect()) &&
      (self()->getSize() == 4 || self()->getSize() == 8))
    return false;

  // this conversion will be a no-op -> CC is not killed unless the child kills
  // it
  ...
  if (((opcode.isLoad() || opcode.isStore() || opcode.isLoadReg() ||
        opcode.isStoreDirectOrReg() || opcode.isLoadAddr()) &&
       (self()->getSize() == 4 || self()->getSize() == 8 ||
        self()->getSize() == 2 || self()->getSize() == 1 ||
        self()->getSize() == 0)))
    return false;

  if (opcode.isArrayRef() &&
...
void OMR::Node::gatherAllNodesWhichMightKillCondCode(
    vcount_t vc, TR::list<TR::Node *> &nodesWhichKillCondCode) {
  if (self()->getVisitCount() == vc) return;
  ...
  for (uint16_t i = 0; i < self()->getNumChildren(); i++) {
    self()->getChild(i)->gatherAllNodesWhichMightKillCondCode(
        vc, nodesWhichKillCondCode);
  }
}

TR::TreeTop *OMR::Node::extractTheNullCheck(TR::TreeTop *prevTreeTop) {
  TR::Compilation *comp = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::NULLCHK ||
                self()->getOpCodeValue() == TR::ResolveAndNULLCHK,
            "extractTheNullCheck can only be called for a null check node; "
            "node is " POINTER_PRINTF_FORMAT,
            self());

  TR::Node *passThru =
      TR::Node::create(TR::PassThrough, 1, self()->getNullCheckReference());
  TR::Node *newNullCheckNode = TR::Node::createWithSymRef(
      TR::NULLCHK, 1, 1, passThru, self()->getSymbolReference());
  ...
  if (self()->getOpCodeValue() == TR::NULLCHK) {
    ...
    self()->setSymbolReference(
        comp->getSymRefTab()->findOrCreateResolveCheckSymbolRef(0));
  }
...
int32_t OMR::Node::countNumberOfNodesInSubtree(vcount_t visitCount) {
  if (self()->getVisitCount() == visitCount) return 0;
...
uint32_t OMR::Node::exceptionsRaised() {
  ...
  //
  if (!node->getOpCode().canRaiseException()) return possibleExceptions;
...
TR::Node *OMR::Node::skipConversions() {
  TR::Node *node = self();
  if (self()->getNumChildren() != 1) return node;

  while (node->getOpCode().isConversion() &&
         node->getOpCode().isStrictWidenConversion())
    node = node->getFirstChild();
 ...
 */
TR::TreeTop *OMR::Node::createStoresForVar(TR::SymbolReference *&nodeRef,
  ...
  if ((self()->getReferenceCount() == 1) &&
    ...
    nodeRef = comp->getSymRefTab()->createTemporary(
        comp->getMethodSymbol(), self()->getDataType(), false);

    if (self()->isNotCollected()) nodeRef->getSymbol()->setNotCollected();
  ...
  if (self()->isNotCollected()) {
    nodeRef = comp->getSymRefTab()->createTemporary(comp->getMethodSymbol(),
                                                    TR::Address);
  ...
  if (isInternalPointer && self()->getOpCode().isArrayRef() &&
      (comp->getSymRefTab()->getNumInternalPointers() >=
           (comp->maxInternalPointers() / 2) ||
    ...
    if (!firstChild->getOpCode().isArrayRef() &&
         ...
         {
      TR::SymbolReference *newArrayRef = comp->getSymRefTab()->createTemporary(
    ...
    if (!storesNeedToBeCreated) {
      if (!secondChild->getOpCode().isLoadConst()  /* &&
        ...
        TR::Node *newStoreIntOrLong =
            TR::Node::createStore(newIntOrLongRef, secondChild);
        TR::TreeTop *newStoreIntOrLongTree =
            TR::TreeTop::create(comp, newStoreIntOrLong);
        ...
        if (child->isInternalPointer())
          ...
          if (child->getOpCode().isLoadVarDirect() &&
            ...
            if (child->getSymbolReference()
              ...
              pinningArray =
                  child->getSymbolReference()->getSymbol()->castToAutoSymbol();
              pinningArray->setPinningArrayPointer();
            }
          } else {
            newArrayRef = comp->getSymRefTab()->createTemporary(
                comp->getMethodSymbol(), TR::Address);
...
// Note that for node with refCount > 1, printed node may NOT be the first
// referenced node despite not being marked with "==>"
void OMR::Node::printFullSubtree() {
  TR::Compilation *comp = TR::comp();
  TR_BitVector nodeChecklistBeforeDump(comp->getNodeCount(), comp->trMemory(),
                                       stackAlloc, notGrowable);
...
const char *OMR::Node::getName(TR_Debug *debug) {
...
bool OMR::Node::isReferenceArrayCopy() {
  ...
  return ((self()->getNumChildren() == NUM_CHILDREN_REFERENCE_ARRAYCOPY)
              ? true
              : false);
}

bool OMR::Node::chkReferenceArrayCopy() {
  return self()->getOpCodeValue() == TR::arraycopy &&
         self()->isReferenceArrayCopy();
}

const char *OMR::Node::printIsReferenceArrayCopy() {
  return self()->chkReferenceArrayCopy() ? "referenceArrayCopy " : "";
}

// OMR::Node::setOpCodeValue is now private, and deprecated. Use
// OMR::Node::recreate instead.
...
bool OMR::Node::isExtendedTo32BitAtSource() {
  return self()->isSignExtendedTo32BitAtSource() ||
         self()->isZeroExtendedTo32BitAtSource();
}

bool OMR::Node::isExtendedTo64BitAtSource() {
  return self()->isSignExtendedTo64BitAtSource() ||
         self()->isZeroExtendedTo64BitAtSource();
}

bool OMR::Node::isSignExtendedAtSource() {
  return self()->isSignExtendedTo32BitAtSource() ||
         self()->isSignExtendedTo64BitAtSource();
}

bool OMR::Node::isZeroExtendedAtSource() {
  return self()->isZeroExtendedTo32BitAtSource() ||
         self()->isZeroExtendedTo64BitAtSource();
}

bool OMR::Node::isSignExtendedTo32BitAtSource() {
  return self()->getOpCode().isLoadVar() &&
         _flags.testAny(signExtendTo32BitAtSource);
}

bool OMR::Node::isSignExtendedTo64BitAtSource() {
  return self()->getOpCode().isLoadVar() &&
         _flags.testAny(signExtendTo64BitAtSource);
}

void OMR::Node::setSignExtendTo32BitAtSource(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isLoadVar(),
            "Can only call this for variable loads\n");
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting signExtendedTo32BitAtSource flag on node %p "
          "to %d\n",
          self(), v)) {
...
void OMR::Node::setSignExtendTo64BitAtSource(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting signExtendedTo64BitAtSource flag on node %p "
          "to %d\n",
          self(), v)) {
...
const char *OMR::Node::printIsSignExtendedTo32BitAtSource() {
  return self()->isSignExtendedTo32BitAtSource()
             ? "signExtendedTo32BitAtSource "
             : "";
}

const char *OMR::Node::printIsSignExtendedTo64BitAtSource() {
  return self()->isSignExtendedTo64BitAtSource()
             ? "signExtendedTo64BitAtSource "
             : "";
}

bool OMR::Node::isZeroExtendedTo32BitAtSource() {
  return self()->getOpCode().isLoadVar() &&
         _flags.testAny(zeroExtendTo32BitAtSource);
}

bool OMR::Node::isZeroExtendedTo64BitAtSource() {
  return self()->getOpCode().isLoadVar() &&
         _flags.testAny(zeroExtendTo64BitAtSource);
}
...
void OMR::Node::setZeroExtendTo32BitAtSource(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting zeroExtendTo32BitAtSource flag on node %p "
          "to %d\n",
          self(), v)) {
...
void OMR::Node::setZeroExtendTo64BitAtSource(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting zeroExtendTo64BitAtSource flag on node %p "
          "to %d\n",
          self(), v)) {
...
const char *OMR::Node::printIsZeroExtendedTo32BitAtSource() {
  return self()->isZeroExtendedTo32BitAtSource() ? "zeroExtendTo32BitAtSource "
                                                 : "";
}

const char *OMR::Node::printIsZeroExtendedTo64BitAtSource() {
...
#ifdef J9_PROJECT_SPECIFIC
  if (self()->getType().isBCD() && f.isClear()) self()->resetDecimalSignFlags();
#endif
  _flags = f;
...
void OMR::Node::setByteCodeInfo(const TR_ByteCodeInfo &bcInfo) {
  ...
  if (!TR::comp()->getCurrentIlGenerator()) _byteCodeInfo.setDoNotProfile(1);
}
...
uint32_t OMR::Node::getByteCodeIndex() {
  return _byteCodeInfo.getByteCodeIndex();
}

void OMR::Node::setByteCodeIndex(uint32_t i) {
  _byteCodeInfo.setByteCodeIndex(i);
}

int16_t OMR::Node::getInlinedSiteIndex() {
  return _byteCodeInfo.getCallerIndex();
}

void OMR::Node::setInlinedSiteIndex(int16_t i) {
  _byteCodeInfo.setCallerIndex(i);
}

TR::Node *OMR::Node::setChild(int32_t c, TR::Node *p) {
  TR_ASSERT(self()->getOpCodeValue() != TR::BBStart || c != 1,
            "Can't use second child of TR::BBStart");
  if (self()->hasNodeExtension()) {
...
TR::Node *OMR::Node::getExtendedChild(int32_t c) {
  TR_ASSERT(
      self()->hasNodeExtension() && c < _unionBase._extension.getNumElems(),
...
TR::Node *OMR::Node::setSecond(TR::Node *p) {
  TR_ASSERT(self()->getOpCodeValue() != TR::BBStart,
            "Can't use second child of TR::BBStart");
...
void OMR::Node::swapChildren() {
  ...
  // assume no longer valid since if opcodes have three children, but still just
  // want to swap the first two
  TR::Node *firstChild = self()->getFirstChild();
...
TR::Node *OMR::Node::removeLastChild() {
...
TR::Node *OMR::Node::findChild(TR::ILOpCodes opcode, bool isReversed) {
  if (isReversed) {
    for (int32_t i = self()->getNumChildren() - 1; i >= 0; i--) {
      ...
      if (currNode->getOpCodeValue() == opcode) return currNode;
    }
  } else {
    for (uint16_t i = 0; i < self()->getNumChildren(); i++) {
      TR::Node *currNode = self()->getChild(i);
...
int32_t OMR::Node::findChildIndex(TR::Node *child) {
  for (uint16_t i = 0; i < self()->getNumChildren(); i++) {
    if (self()->getChild(i) == child) return i;
  }
  return -1;
}
...
int32_t OMR::Node::countChildren(TR::ILOpCodes opcode) {
  ...
  for (uint16_t i = 0; i < self()->getNumChildren(); i++) {
    TR::Node *currNode = self()->getChild(i);
    if (currNode->getOpCodeValue() == opcode) count++;
  }
  return count;
}

bool OMR::Node::requiresRegisterPair(TR::Compilation *comp) {
  return (self()->getType().isInt64() && TR::Compiler->target.is32Bit() &&
          !comp->cg()->use64BitRegsOn32Bit());
}
...
rcount_t OMR::Node::recursivelyDecReferenceCount() {
  ...
  if (self()->getReferenceCount() > 0)
    ...
    TR_ASSERT(self()->getOpCode().isTreeTop(),
              "OMR::Node::recursivelyDecReferenceCount() invoked for "
              "nontreetop node %s " POINTER_PRINTF_FORMAT " with count == 0",
              self()->getOpCode().getName(), self());
    count = 0;
  }

  if (count == 0) {
    for (int32_t childCount = self()->getNumChildren() - 1; childCount >= 0;
         childCount--) {
      TR_ASSERT(self()->getChild(childCount) != NULL,
                "parent %s " POINTER_PRINTF_FORMAT
                " trying to decrement reference of NULL child\n",
                self()->getOpCode().getName(), self());
...
void OMR::Node::initializeFutureUseCounts(vcount_t visitCount) {
  if (visitCount == self()->getVisitCount()) return;
...
void OMR::Node::setIsNotRematerializeable() {
  ...
  if (performNodeTransformation1(
          c, "Setting notRematerializeable flag on node %p\n", self()))
    _localIndex = (_localIndex | SCOUNT_HIGH_BIT);
}

bool OMR::Node::isRematerializeable() {
  if (_localIndex & SCOUNT_HIGH_BIT) return false;
  return true;
}

TR::Register *OMR::Node::getRegister() {
  TR_ASSERT(self()->getOpCodeValue() != TR::BBStart,
            "don't call getRegister for a BBStart");
  if ((uintptr_t)_unionA._register & 1)
    return 0;  // tagged pointer means the field is actually an evaluation
               // priority
...
TR::Register *OMR::Node::setRegister(TR::Register *reg) {
  if (reg) {
    if (reg->isLive()) {
      ...
      if (pair) {
        TR_ASSERT(
            pair->getLowOrder()->isLive() && pair->getHighOrder()->isLive(),
...
int32_t OMR::Node::getEvaluationPriority(TR::CodeGenerator *codeGen) {
  if (_unionA._register == 0)  // not evaluated into register & priority unknown
  {
    ...
    return self()->setEvaluationPriority(
        codeGen->getEvaluationPriority(self()));
  }
...
int32_t OMR::Node::setEvaluationPriority(int32_t p) {
  ...
  {
    TR_ASSERT(0,
              "setEvaluationPriority cannot be called after the node has "
...
bool OMR::Node::canGet32bitIntegralValue() {
  ...
  return self()->getOpCode().isLoadConst() &&
         (type.isInt32() || type.isInt16() || type.isInt8());
}

int32_t OMR::Node::get32bitIntegralValue() {
  TR_ASSERT(self()->getOpCode().isLoadConst(),
            "get32bitIntegralValue called on node which is not loadConst");
  TR_ASSERT(self()->canGet32bitIntegralValue(),
            "get32bitIntegralValue called on a node which returns false from "
            "canGet32bitIntegralValue()");
  TR::DataType type = self()->getType();
  ...
  if (type.isInt32())
    ...
    TR_ASSERT(false, "Must be an <= size 4 integral but it is type %s",
...
int64_t OMR::Node::get64bitIntegralValue() {
  TR_ASSERT(self()->getOpCode().isLoadConst(),
            "get64bitIntegralValue called on node which is not loadConst");
  TR_ASSERT(self()->canGet64bitIntegralValue(),
            "get64bitIntegralValue called on a node which returns false from "
            "canGet64bitIntegralValue()");
  if (!self()->getOpCode().isLoadConst()) return 0;
  ...
  if (type.isInt8())
    ...
    return self()->getAddress();  // TR::Compiler->target.is64Bit() ?
                                  // getUnsignedLongInt() : getUnsignedInt();
  else {
    TR_ASSERT(false,
              "Must be an integral or address but it is type %s on node %p\n",
...
void OMR::Node::set64bitIntegralValue(int64_t value) {
  ...
  if (type.isInt8())
    ...
    TR_ASSERT(false, "Must be an integral value but it is type %s",
              self()->getDataType().toString());
  }
}
...
uint64_t OMR::Node::get64bitIntegralValueAsUnsigned() {
  ...
  if (type.isInt8())
    ...
    return TR::Compiler->target.is64Bit() ? self()->getUnsignedLongInt()
                                          : self()->getUnsignedInt();
  else {
    TR_ASSERT(false, "Must be an integral or address but it is type %s",
              self()->getDataType().toString());
    return 0;
  }
...
TR::Node *OMR::Node::getAllocation() {
  TR_ASSERT((self()->getOpCodeValue() == TR::allocationFence),
            "Opcode must be TR::allocationFence in this method");
...
void OMR::Node::copyNodeExtension(TR::NodeExtension *other, uint16_t numElems,
                                  size_t size) {
  TR::Compilation *comp = TR::comp();
...
void OMR::Node::addExtensionElements(uint16_t num) {
  ...
  if (numElems > NUM_DEFAULT_CHILDREN) {
    if (self()->hasNodeExtension())
      self()->copyNodeExtension(_unionBase._extension.getExtensionPtr(),
                                numElems, self()->sizeOfExtension());
    else
      self()->createNodeExtension(numElems);
  }
...
void OMR::Node::freeExtensionIfExists() {
  if (self()->hasNodeExtension()) {
    ...
    for (uint16_t childNum = 0;
         (childNum < NUM_DEFAULT_CHILDREN && childNum < numElems); childNum++) {
...
TR::DataType OMR::Node::getArrayCopyElementType() {
  TR_ASSERT(self()->getOpCodeValue() == TR::arraycopy ||
                self()->getOpCodeValue() == TR::arrayset,
            "getArrayCopyElementType called on an invalid node");

  if (_numChildren == 3) {
    return static_cast<TR::DataTypes>(
        _unionBase._extension.getExtensionPtr()->getElem<uint32_t>(
            _numChildren));
  } else {
    TR_ASSERT(_numChildren == 5,
              "getArrayCopyElementType called on an arraycopy node with an "
...
void OMR::Node::setArrayCopyElementType(TR::DataType type) {
  ...
  if (_numChildren == 3) {
    TR_ASSERT(
        _unionBase._extension.getNumElems() > _numChildren,
        "Need an extra extension element slot in setArrayCopyElementType");

    _unionBase._extension.getExtensionPtr()->setElem<uint32_t>(_numChildren,
                                                               type);
  }
}

TR_OpaqueClassBlock *OMR::Node::getArrayStoreClassInNode() {
  TR_ASSERT(self()->hasNodeExtension(),
            "hasArrayStoreCheckInfo node needs extension");
  if (self()->hasArrayStoreCheckInfo()) {
    ArrayStoreCheckInfo *info =
        _unionBase._extension.getExtensionPtr()->getElem<ArrayStoreCheckInfo *>(
            2);
...
void OMR::Node::setArrayStoreClassInNode(TR_OpaqueClassBlock *o) {
  if (!self()->hasArrayStoreCheckInfo()) self()->createArrayStoreCheckInfo();
...
void OMR::Node::createArrayStoreCheckInfo() {
  TR::Compilation *comp = TR::comp();
  ArrayStoreCheckInfo *info =
      (ArrayStoreCheckInfo *)comp->trMemory()->allocateMemory(
          sizeof(ArrayStoreCheckInfo), heapAlloc);
  memset(info, 0, sizeof(ArrayStoreCheckInfo));
  TR_ASSERT(self()->hasNodeExtension(),
            "hasArrayStoreCheckInfo node needs extension");

  _unionBase._extension.getExtensionPtr()->setElem<ArrayStoreCheckInfo *>(2,
                                                                          info);
}

TR_OpaqueMethodBlock *OMR::Node::getMethod() {
  TR_ASSERT(self()->getInlinedSiteIndex() < -1,
            "Call getMethod only for dummy nodes in estimate code size");
  TR_ASSERT(!self()->hasNodeExtension(),
            "Cannot Call getMethod on node with extension");
...
TR::LabelSymbol *OMR::Node::getLabel() {
  ...
  // TR_ASSERT(self()->getOpCodeValue() == TR::BBStart, "getLabel called on a
  // node that is not a BBStart");
...
TR_GlobalRegisterNumber OMR::Node::setGlobalRegisterNumber(
    TR_GlobalRegisterNumber i) {
...
TR_GlobalRegisterNumber OMR::Node::setLowGlobalRegisterNumber(
    TR_GlobalRegisterNumber i) {
  _unionBase._unionedWithChildren._globalRegisterInfo.setLow(i, self());
  return _unionBase._unionedWithChildren._globalRegisterInfo.getLow();
}
...
TR_GlobalRegisterNumber OMR::Node::setHighGlobalRegisterNumber(
    TR_GlobalRegisterNumber i) {
  _unionBase._unionedWithChildren._globalRegisterInfo.setHigh(i, self());
  return _unionBase._unionedWithChildren._globalRegisterInfo.getHigh();
}

size_t OMR::Node::getLiteralPoolOffset() {
...
void *OMR::Node::getRelocationDestination(uint32_t n) {
  return _unionBase._unionedWithChildren._relocationInfo
      ._relocationDestination[n];
}

void *OMR::Node::setRelocationDestination(uint32_t n, void *p) {
  self()->freeExtensionIfExists();
  return (_unionBase._unionedWithChildren._relocationInfo
              ._relocationDestination[n] = p);
}

uint32_t OMR::Node::getRelocationType() {
...
uint32_t OMR::Node::getNumRelocations() {
...
void *OMR::Node::getMonitorInfo() {
  TR_ASSERT(self()->getOpCodeValue() == TR::monent ||
...
bool OMR::Node::hasSymbolReference() {
  return self()->getOpCode().hasSymbolReference();
}
...
bool OMR::Node::hasSymbolReferenceOfAnyType() {
  return self()->hasSymbolReference() ||
         self()->hasRegLoadStoreSymbolReference();
}

bool OMR::Node::hasBranchDestinationNode() {
  return self()->getOpCode().isBranch();
}

bool OMR::Node::hasBlock() {
  return self()->getOpCodeValue() == TR::BBStart ||
         self()->getOpCodeValue() == TR::BBEnd;
}
...
bool OMR::Node::hasDataType() {
  ...
  // Some opcodes do not have explicitly encoded datatype. Nodes with these
  // opcodes will automatically deduce datatype and cache it in its _dataType
  // field.
  return _opCode.hasNoDataType() && !_opCode.hasSymbolReference() &&
         !self()->hasRegLoadStoreSymbolReference();
}
...
OMR::Node::UnionPropertyA_Type OMR::Node::getUnionPropertyA_Type() {
  if (self()->hasSymbolReference())
    ...
    // This code is to be removed, once all illegal accesses of
    // _regLoadStoreSymbolReference are removed
    return OMR::Node::HasSymbolReference;
#else
...
TR::SymbolReference *OMR::Node::getSymbolReference() {
  ...
  TR_ASSERT(self()->hasSymbolReference(),
            "attempting to access _symbolReference field for node %s %p that "
            "does not have it",
...
TR::SymbolReference *OMR::Node::getRegLoadStoreSymbolReference() {
  TR_ASSERT(self()->hasRegLoadStoreSymbolReference(),
            "attempting to access _regLoadStoreSymbolReference field for node "
            "%s %p that does not have it",
            self()->getOpCode().getName(), this);
  return _unionPropertyA._regLoadStoreSymbolReference;
}

TR::SymbolReference *OMR::Node::setRegLoadStoreSymbolReference(
    TR::SymbolReference *p) {
  TR_ASSERT(self()->hasRegLoadStoreSymbolReference(),
            "attempting to access _regLoadStoreSymbolReference field for node "
            "%s %p that does not have it",
            self()->getOpCode().getName(), this);
...
TR::SymbolReference *OMR::Node::getSymbolReferenceOfAnyType() {
  TR_ASSERT(self()->hasSymbolReferenceOfAnyType(),
            "attempting to access a regLoad/regStore or normal symbolReference "
            "for node %s %p that does not have it",
            self()->getOpCode().getName(), this);
  if (self()->hasRegLoadStoreSymbolReference())
    return self()->getRegLoadStoreSymbolReference();
...
TR::Symbol *OMR::Node::getSymbol() {
  ...
  if (self()->hasRegLoadStoreSymbolReference())
    return self()->getRegLoadStoreSymbolReference()
               ? self()->getRegLoadStoreSymbolReference()->getSymbol()
               : NULL;
#else
  TR_ASSERT(hasSymbolReference(),
            "attempting to access _symbolReference field for node %s %p that "
            "does not have it",
            self()->getOpCode().getName(), this);
#endif
  return (_unionPropertyA._symbolReference != NULL)
             ? _unionPropertyA._symbolReference->getSymbol()
             : NULL;
}
...
TR::Block *OMR::Node::getBlock(bool ignored) {
  TR_ASSERT(
      self()->hasBlock(),
      "attempting to access _block field for node %s %p that does not have it",
      self()->getOpCode().getName(), this);
...
int32_t OMR::Node::getArrayStride() {
  TR_ASSERT(self()->hasArrayStride(),
            "attempting to access _arrayStride field for node %s %p that does "
            "not have it",
            self()->getOpCode().getName(), this);
...
TR::AutomaticSymbol *OMR::Node::getPinningArrayPointer() {
  TR_ASSERT(self()->hasPinningArrayPointer(),
            "attempting to access _pinningArrayPointer field for node %s %p "
...
TR::DataType OMR::Node::computeDataType() {
  // If opcode has SymRef, must get DataType from SymRef, since _dataType member
  // is union-aliased with SymRef.
  if (_opCode.hasSymbolReference() ||
      self()->hasRegLoadStoreSymbolReference()) {
    TR::SymbolReference *symRef =
        (_opCode.hasSymbolReference())
            ? self()->getSymbolReference()
            : (self()->hasRegLoadStoreSymbolReference())
                  ? self()->getRegLoadStoreSymbolReference()
                  : NULL;
  ...
  TR_ASSERT(self()->hasDataType(),
            "attempting to access _dataType field for node %s %p that does not "
            "have it",
            self()->getOpCode().getName(), self());
  if (_unionPropertyA._dataType != TR::NoType) return _unionPropertyA._dataType;

  if (self()->getNumChildren() > 0) {
    // Type erasure for some vector opcodes. They deduce type from children
    // nodes
    if (_opCode.isVector()) {
      // Vector comparison returning resultant vector should return vector bool
      // int type, WCode ACR 265
      if (_opCode.isBooleanCompare())
        _unionPropertyA._dataType = self()
                                        ->getFirstChild()
                                        ->getDataType()
                                        .getVectorIntegralType()
                                        .getDataType();
        ...
        _unionPropertyA._dataType =
            self()->getFirstChild()->getDataType().getDataType();

      return _unionPropertyA._dataType;
    }

    if (_opCode.getOpCodeValue() == TR::getvelem)
      return _unionPropertyA._dataType = self()
                                             ->getFirstChild()
                                             ->getDataType()
                                             .vectorToScalar()
                                             .getDataType();
...
void OMR::Node::setIsZero(bool v) {
  TR::Compilation *c = TR::comp();
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting nodeIsZero flag on node %p to %d\n",
          self(), v))
    _flags.set(nodeIsZero, v);
}

const char *OMR::Node::printIsZero() { return self()->isZero() ? "X==0 " : ""; }
...
void OMR::Node::setIsNonZero(bool v) {
  TR::Compilation *c = TR::comp();
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting nodeIsNonZero flag on node %p to %d\n",
          self(), v))
    _flags.set(nodeIsNonZero, v);
}

const char *OMR::Node::printIsNonZero() {
  return self()->isNonZero() ? "X!=0 " : "";
}

bool OMR::Node::isNull() {
  return (self()->getOpCodeValue() == TR::loadaddr
              ? false
              : _flags.testAny(nodeIsNull));
}

void OMR::Node::setIsNull(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() != TR::loadaddr,
            "Can't call this for loadaddr");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting null flag on node %p to %d\n", self(), v))
...
bool OMR::Node::isNonNull() {
  return (self()->getOpCodeValue() == TR::loadaddr
              ...
              : (_flags.testAny(nodeIsNonNull) || self()->isInternalPointer() ||
                 (self()->getOpCode().hasSymbolReference() &&
                  self()->getSymbol()->isInternalPointer())));
}

void OMR::Node::setIsNonNull(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(v || self()->getOpCodeValue() != TR::loadaddr,
            "Can't call this for loadaddr");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting nonNull flag on node %p to %d\n", self(),
          v))
...
bool OMR::Node::pointsToNull() {
  return (self()->getOpCodeValue() == TR::loadaddr
              ? _flags.testAny(nodePointsToNull)
              : false);
}
...
void OMR::Node::setPointsToNull(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::loadaddr,
            "Can only call this for loadaddr");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting nodePointsToNull flag on node %p to %d\n",
          self(), v))
...
const char *OMR::Node::printPointsToNull() {
  return self()->chkPointsToNull() ? "*X==null " : "";
}

bool OMR::Node::pointsToNonNull() {
  return (self()->getOpCodeValue() == TR::loadaddr
              ? _flags.testAny(nodePointsToNonNull)
              : false);
}
...
void OMR::Node::setIsInvalid8BitGlobalRegister(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting invalid8BitGlobalRegister flag on node %p "
          "to %d\n",
          self(), v))
...
const char *OMR::Node::printIsInvalid8BitGlobalRegister() {
  return self()->isInvalid8BitGlobalRegister() ? "invalid8BitGlobalRegister "
                                               : "";
}

bool OMR::Node::isDirectMemoryUpdate() {
...
void OMR::Node::setDirectMemoryUpdate(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting directMemoryUpdate flag on node %p to %d\n",
          self(), v))
    _flags.set(directMemoryUpdate, v);
}

const char *OMR::Node::printIsDirectMemoryUpdate() {
...
void OMR::Node::setIsProfilingCode() {
  TR::Compilation *c = TR::comp();
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting profilingCode flag on node %p\n", self()))
    _flags.set(profilingCode);
}

const char *OMR::Node::printIsProfilingCode() {
  return self()->isProfilingCode() ? "profilingCode " : "";
}

bool OMR::Node::hasBeenVisitedForHints() {
  return _flags.testAny(visitedForHints);
}

void OMR::Node::setHasBeenVisitedForHints(bool v) {
...
void OMR::Node::setIsNonNegative(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting nodeIsNonNegative flag on node %p to %d\n",
          self(), v))
    _flags.set(nodeIsNonNegative, v);
}

const char *OMR::Node::printIsNonNegative() {
...
void OMR::Node::setIsNonPositive(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting nodeIsNonPositive flag on node %p to %d\n",
          self(), v))
    _flags.set(nodeIsNonPositive, v);
}

const char *OMR::Node::printIsNonPositive() {
...
bool OMR::Node::isNonDegenerateArrayCopy() {
  TR_ASSERT(self()->getOpCodeValue() == TR::arraycopy,
            "Opcode must be arraycopy");
  TR::Node *len =
      self()->getNumChildren() == 5 ? self()->getChild(4) : self()->getChild(2);
...
void OMR::Node::setCannotOverflow(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() != TR::PassThrough,
            "can't set cannotOverflow on PassThrough\n");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting cannotOverflow flag on node %p to %d\n",
          self(), v))
...
const char *OMR::Node::printCannotOverflow() {
  return self()->chkCannotOverflow() ? "cannotOverflow " : "";
}

bool OMR::Node::isSafeToSkipTableBoundCheck() {
...
void OMR::Node::setIsSafeToSkipTableBoundCheck(bool b) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting canSkipTableBoundCheck flag on node %p to "
          "%d\n",
          self(), b))
...
const char *OMR::Node::printIsSafeToSkipTableBoundCheck() {
  return self()->chkSafeToSkipTableBoundCheck() ? "safeToSkipTblBndChk " : "";
}

bool OMR::Node::isNOPLongStore() {
  TR_ASSERT(self()->getOpCode().isStore() && self()->getType().isInt64(),
            "Opcode must be long store");
...
void OMR::Node::setNOPLongStore(bool v) {
  ...
  TR_ASSERT(self()->getOpCode().isStore() && self()->getType().isInt64(),
            "Opcode must be long store");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting NOPLongStore flag on node %p to %d\n",
          self(), v))
    _flags.set(NOPLongStore, v);
}

bool OMR::Node::chkNOPLongStore() {
  return self()->getOpCode().isStore() && self()->getType().isInt64() &&
         _flags.testAny(NOPLongStore);
}

const char *OMR::Node::printIsNOPLongStore() {
  return self()->chkNOPLongStore() ? "NOPLongStore " : "";
}

bool OMR::Node::storedValueIsIrrelevant() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT((self()->getOpCode().isStore() &&
...
void OMR::Node::setStoredValueIsIrrelevant(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(
      c->getOption(TR_EnableOSR),
      "Currently this OMR::Node flag means different things on different front "
      "ends. If we wanted to reuse this query across front ends other than "
      "Java (which uses these kinds of stores for OSR) then we probably need "
      "to map it to a different bit in the OMR::Node flags");
  TR_ASSERT((self()->getOpCode().isStore() &&
             self()->getSymbolReference()->getSymbol()->isAutoOrParm()),
            "Opcode must be store to a stack slot");
  if (self()->getOpCode().isStore() &&
    ...
    if (performNodeTransformation2(
            c,
            "O^O NODE FLAGS: Setting StoredValueIsIrrelevant flag on node %p "
            "to %d\n",
            self(), v))
...
bool OMR::Node::chkStoredValueIsIrrelevant() {
  ...
  return c->getOption(TR_EnableOSR) && self()->getOpCode().isStore() &&
         self()->getSymbolReference()->getSymbol()->isAutoOrParm() &&
         _flags.testAny(StoredValueIsIrrelevant);
}

const char *OMR::Node::printIsStoredValueIsIrrelevant() {
...
void OMR::Node::setHeapificationStore(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting HeapificationStore flag on node %p to %d\n",
          self(), v))
    _flags.set(HeapificationStore, v);
}

bool OMR::Node::chkHeapificationStore() {
  return (self()->getOpCodeValue() == TR::astore) &&
         _flags.testAny(HeapificationStore);
}

const char *OMR::Node::printIsHeapificationStore() {
...
void OMR::Node::setHeapificationAlloc(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting HeapificationAlloc flag on node %p to %d\n",
          self(), v))
    _flags.set(HeapificationAlloc, v);
}

bool OMR::Node::chkHeapificationAlloc() {
  return self()->getOpCode().isNew() && _flags.testAny(HeapificationAlloc);
}

const char *OMR::Node::printIsHeapificationAlloc() {
...
bool OMR::Node::isLiveMonitorInitStore() {
  TR_ASSERT(self()->getOpCodeValue() == TR::astore, "Opcode must be astore");
  return (self()->getOpCode().hasSymbolReference() &&
          self()->getSymbol()->holdsMonitoredObject() &&
...
void OMR::Node::setLiveMonitorInitStore(bool v) {
  ...
  if (self()->getOpCode().hasSymbolReference() &&
      ...
      performNodeTransformation2(c,
                                 "O^O NODE FLAGS: Setting liveMonitorInitStore "
                                 "flag on node %p to %d\n",
                                 self(), v))
...
bool OMR::Node::chkLiveMonitorInitStore() {
  return (self()->getOpCodeValue() == TR::astore) &&
         self()->isLiveMonitorInitStore();
}

const char *OMR::Node::printIsLiveMonitorInitStore() {
  return self()->chkLiveMonitorInitStore() ? "liveMonitorInitStore " : "";
}

bool OMR::Node::useSignExtensionMode() {
  return _flags.testAny(SignExtensionMode) && self()->getOpCode().isLoadVar() &&
         self()->getType().isInt32();
}
...
void OMR::Node::setUseSignExtensionMode(bool b) {
  ...
  if (self()->getOpCode().isLoadVar() && self()->getType().isInt32() &&
      performNodeTransformation2(c,
                                 "O^O NODE FLAGS: Setting useSignExtensionMode "
                                 "flag on node %p to %d\n",
                                 self(), b)) {
...
const char *OMR::Node::printSetUseSignExtensionMode() {
  return self()->useSignExtensionMode() ? "SignExtMode " : "";
}

bool OMR::Node::hasFoldedImplicitNULLCHK() {
  TR_ASSERT(
      self()->getOpCode().isBndCheck() || self()->getOpCode().isSpineCheck(),
      "assertion failure");
...
void OMR::Node::setHasFoldedImplicitNULLCHK(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting foldedImplicitNULLCHK flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkFoldedImplicitNULLCHK() {
  return self()->getOpCode().isBndCheck() &&
         _flags.testAny(foldedImplicitNULLCHK);
}

const char *OMR::Node::printHasFoldedImplicitNULLCHK() {
...
void OMR::Node::setThrowInsertedByOSR(bool v) {
  ...
  if (c->getOption(TR_EnableOSR) && (self()->getOpCodeValue() == TR::athrow)) {
    if (performNodeTransformation2(c,
                                   "O^O NODE FLAGS: Setting ThrowInsertedByOSR "
                                   "flag on node %p to %d\n",
                                   self(), v))
...
bool OMR::Node::chkThrowInsertedByOSR() {
  return (self()->getOpCodeValue() == TR::athrow) &&
         _flags.testAny(ThrowInsertedByOSR);
}

const char *OMR::Node::printIsThrowInsertedByOSR() {
...
void OMR::Node::setIsTheVirtualCallNodeForAGuardedInlinedCall() {
  ...
  if (performNodeTransformation1(
          c,
          "O^O NODE FLAGS: Setting virtualCallNodeForAGuardedInlinedCall flag "
          "on node %p\n",
          self()))
...
void OMR::Node::resetIsTheVirtualCallNodeForAGuardedInlinedCall() {
  ...
  if (performNodeTransformation1(
          c,
          "ReO^O NODE FLAGS: Setting virtualCallNodeForAGuardedInlinedCall "
          "flag on node %p\n",
          self()))
...
bool OMR::Node::chkTheVirtualCallNodeForAGuardedInlinedCall() {
  return self()->getOpCode().isCall() &&
         _flags.testAny(virtualCallNodeForAGuardedInlinedCall);
}

const char *OMR::Node::printIsTheVirtualCallNodeForAGuardedInlinedCall() {
  return self()->chkTheVirtualCallNodeForAGuardedInlinedCall()
             ? "virtualCallNodeForAGuardedInlinedCall "
             : "";
}
...
bool OMR::Node::isDontTransformArrayCopyCall() {
  TR_ASSERT(self()->isArrayCopyCall(),
            "attempt to set transformArrayCopyCall flag not an arraycopy call");
...
void OMR::Node::setDontTransformArrayCopyCall() {
  ...
  if (self()->isArrayCopyCall()) {
    if (performNodeTransformation1(
            c,
            "O^O NODE FLAGS: Setting dontTransformArrayCopyCall flag on node "
            "%p\n",
            self()))
...
bool OMR::Node::chkDontTransformArrayCopyCall() {
  return self()->isArrayCopyCall() &&
         _flags.testAny(dontTransformArrayCopyCall);
}

const char *OMR::Node::printIsDontTransformArrayCopyCall() {
  return self()->chkDontTransformArrayCopyCall() ? "dontTransformArrayCopyCall "
                                                 : "";
}

bool OMR::Node::isNodeRecognizedArrayCopyCall() {
  TR_ASSERT(self()->getOpCodeValue() == TR::call,
            "Opcode must be vcall to arraycopy");
...
void OMR::Node::setNodeIsRecognizedArrayCopyCall(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting nodeIsRecognizedArrayCopyCall flag on node "
          "%p to %d\n",
          self(), v))
...
bool OMR::Node::chkNodeRecognizedArrayCopyCall() {
  return self()->getOpCodeValue() == TR::call &&
         _flags.testAny(nodeIsRecognizedArrayCopyCall);
}

const char *OMR::Node::printIsNodeRecognizedArrayCopyCall() {
  return self()->chkNodeRecognizedArrayCopyCall()
             ? "nodeRecognizedArrayCopyCall "
             : "";
}
...
bool OMR::Node::containsCompressionSequence() {
  return _flags.testAny(isCompressionSequence);
}

void OMR::Node::setContainsCompressionSequence(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isAdd() || self()->getOpCode().isSub() ||
                self()->getOpCode().isShift(),
            "Opcode must be an add/sub/shift");
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting isCompressionSequence flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkCompressionSequence() {
  return (self()->getOpCode().isAdd() || self()->getOpCode().isSub() ||
          self()->getOpCode().isShift()) &&
         _flags.testAny(isCompressionSequence);
}

const char *OMR::Node::printContainsCompressionSequence() {
  return self()->chkCompressionSequence() ? "compressionSequence " : "";
}

bool OMR::Node::isInternalPointer() {
  return _flags.testAny(internalPointer) &&
         (self()->getOpCode().hasPinningArrayPointer() ||
...
void OMR::Node::setIsInternalPointer(bool v) {
  ...
  TR_ASSERT(self()->getOpCode().hasPinningArrayPointer() ||
                self()->getOpCode().isArrayRef(),
            "Opcode must be one that can have a pinningArrayPointer or must be "
            "an array reference");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting internalPointer flag on node %p to %d\n",
          self(), v))
    _flags.set(internalPointer, v);
}

const char *OMR::Node::printIsInternalPointer() {
  return self()->isInternalPointer() ? "internalPtr " : "";
}

bool OMR::Node::isArrayTRT() {
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslateAndTest,
            "Opcode must be arraytranslateAndTest");
...
void OMR::Node::setArrayTRT(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslateAndTest,
            "Opcode must be arraytranslateAndTest");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting arrayTRT flag on node %p to %d\n", self(),
          v))
    _flags.set(arrayTRT, v);
}

bool OMR::Node::chkArrayTRT() {
  return self()->getOpCodeValue() == TR::arraytranslateAndTest &&
         _flags.testAny(arrayTRT);
}

const char *OMR::Node::printArrayTRT() {
...
void OMR::Node::setCharArrayTRT(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslateAndTest,
            "Opcode must be arraytranslateAndTest");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting charArrayTRT flag on node %p to %d\n",
          self(), v))
...
bool OMR::Node::chkCharArrayTRT() {
  return self()->getOpCodeValue() == TR::arraytranslateAndTest &&
         _flags.testAny(charArrayTRT);
}

const char *OMR::Node::printCharArrayTRT() {
  return self()->chkCharArrayTRT() ? "charArrayTRT " : "";
}

bool OMR::Node::isSourceByteArrayTranslate() {
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate,
            "Opcode must be arraytranslate");
...
void OMR::Node::setSourceIsByteArrayTranslate(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting sourceIsByteArrayTranslate flag on node %p "
          "to %d\n",
          self(), v))
...
const char *OMR::Node::printSetSourceIsByteArrayTranslate() {
...
void OMR::Node::setTargetIsByteArrayTranslate(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting targetIsByteArrayTranslate flag on node %p "
          "to %d\n",
          self(), v))
...
const char *OMR::Node::printSetTargetIsByteArrayTranslate() {
...
const char *OMR::Node::printIsByteToByteTranslate() {
  return self()->chkByteToByteTranslate() ? "byte2byteXlate " : "";
}

const char *OMR::Node::printIsByteToCharTranslate() {
  return self()->chkByteToCharTranslate() ? "byte2charXlate " : "";
}

const char *OMR::Node::printIsCharToByteTranslate() {
  return self()->chkCharToByteTranslate() ? "char2byteXlate " : "";
}

const char *OMR::Node::printIsCharToCharTranslate() {
...
void OMR::Node::setTermCharNodeIsHint(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting termCharNodeIsHint flag on node %p to %d\n",
          self(), v))
...
bool OMR::Node::chkTermCharNodeIsHint() {
  return self()->getOpCodeValue() == TR::arraytranslate &&
         _flags.testAny(termCharNodeIsHint);
}

const char *OMR::Node::printSetTermCharNodeIsHint() {
...
void OMR::Node::setSourceCellIsTermChar(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraytranslate,
            "Opcode must be arraytranslate");
  if (performNodeTransformation2(c,
                                 "O^O NODE FLAGS: Setting sourceCellIsTermChar "
                                 "flag on node %p to %d\n",
                                 self(), v))
...
bool OMR::Node::chkSourceCellIsTermChar() {
  return self()->getOpCodeValue() == TR::arraytranslate &&
         _flags.testAny(sourceCellIsTermChar);
}

const char *OMR::Node::printSourceCellIsTermChar() {
...
void OMR::Node::setTableBackedByRawStorage(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting tableBackedByRawStorage flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkTableBackedByRawStorage() {
  return self()->getOpCodeValue() == TR::arraytranslate &&
         _flags.testAny(tableBackedByRawStorage);
}

const char *OMR::Node::printSetTableBackedByRawStorage() {
  return self()->chkTableBackedByRawStorage() ? "tableBackedByRawStorage " : "";
}

bool OMR::Node::isArrayCmpLen() {
  TR_ASSERT(self()->getOpCodeValue() == TR::arraycmp,
            "Opcode must be arraycmp");
  return _flags.testAny(arrayCmpLen);
}

void OMR::Node::setArrayCmpLen(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::arraycmp,
            "Opcode must be arraycmp");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting arrayCmpLen flag on node %p to %d\n",
          self(), v))
    _flags.set(arrayCmpLen, v);
}

bool OMR::Node::chkArrayCmpLen() {
  return self()->getOpCodeValue() == TR::arraycmp &&
         _flags.testAny(arrayCmpLen);
}

const char *OMR::Node::printArrayCmpLen() {
...
void OMR::Node::setArrayCmpSign(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::arraycmp,
            "Opcode must be arraycmp");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting arrayCmpSign flag on node %p to %d\n",
          self(), v))
    _flags.set(arrayCmpSign, v);
}

bool OMR::Node::chkArrayCmpSign() {
  return self()->getOpCodeValue() == TR::arraycmp &&
         _flags.testAny(arrayCmpSign);
}

const char *OMR::Node::printArrayCmpSign() {
...
void OMR::Node::setHalfWordElementArrayCopy(bool v) {
  ...
  if (v) {
    if (performNodeTransformation2(
            c,
            "O^O NODE FLAGS: Setting halfWordElementArrayCopy flag on node %p "
            "to %d\n",
            self(), v))
...
bool OMR::Node::chkHalfWordElementArrayCopy() {
  return self()->getOpCodeValue() == TR::arraycopy &&
         self()->isHalfWordElementArrayCopy();
}

const char *OMR::Node::printIsHalfWordElementArrayCopy() {
...
void OMR::Node::setWordElementArrayCopy(bool v) {
  ...
  if (v) {
    if (performNodeTransformation2(
            c,
            "O^O NODE FLAGS: Setting wordElementArrayCopy flag on node %p to "
            "%d\n",
            self(), v))
...
bool OMR::Node::chkWordElementArrayCopy() {
  return self()->getOpCodeValue() == TR::arraycopy &&
         self()->isWordElementArrayCopy();
}

const char *OMR::Node::printIsWordElementArrayCopy() {
...
void OMR::Node::setForwardArrayCopy(bool v) {
  ...
  if (v) {
    if (!wasRarePathForwardArrayCopy &&
        performNodeTransformation2(
            c,
            "O^O NODE FLAGS: Setting forwardArrayCopy flag on node %p to %d\n",
            self(), v))
      _flags.setValue(arraycopyDirectionMask, arraycopyDirectionForward);
    TR_ASSERT(self()->isForwardArrayCopy() && !self()->isBackwardArrayCopy(),
              "assertion failure");
    TR_ASSERT(
        wasRarePathForwardArrayCopy == self()->isRarePathForwardArrayCopy(),
...
bool OMR::Node::chkForwardArrayCopy() {
  return self()->getOpCodeValue() == TR::arraycopy &&
         self()->isForwardArrayCopy();
}

const char *OMR::Node::printIsForwardArrayCopy() {
...
void OMR::Node::setBackwardArrayCopy(bool v) {
  ...
  if (v) {
    if (performNodeTransformation2(
            c,
            "O^O NODE FLAGS: Setting backwardArrayCopy flag on node %p to %d\n",
            self(), v))
...
const char *OMR::Node::printIsBackwardArrayCopy() {
...
bool OMR::Node::isRarePathForwardArrayCopy() {
  TR_ASSERT(self()->getOpCodeValue() == TR::arraycopy,
            "Opcode must be arraycopy");
  return _flags.testValue(arraycopyDirectionMask,
                          arraycopyDirectionForwardRarePath);
}
...
void OMR::Node::setRarePathForwardArrayCopy(bool v) {
  ...
  if (v != self()->isRarePathForwardArrayCopy() &&
      performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting rarePathForwardArrayCopy flag on node %p to "
          "%d\n",
          self(), v))
    _flags.setValue(
        arraycopyDirectionMask,
        v ? arraycopyDirectionForwardRarePath : arraycopyDirectionUnknown);
  if (v)
    TR_ASSERT(self()->isRarePathForwardArrayCopy() &&
                  self()->isForwardArrayCopy() &&
                  !self()->isBackwardArrayCopy(),
              "assertion failure");
  else
    TR_ASSERT(!self()->isRarePathForwardArrayCopy() &&
                  (wasForwardArrayCopy == self()->isForwardArrayCopy()),
              "assertion failure");
}
...
const char *OMR::Node::printIsRarePathForwardArrayCopy() {
...
void OMR::Node::setNoArrayStoreCheckArrayCopy(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting noArrayStoreCheckArrayCopy flag on node %p "
          "to %d\n",
          self(), v))
...
bool OMR::Node::chkNoArrayStoreCheckArrayCopy() {
  return self()->getOpCodeValue() == TR::arraycopy &&
         self()->isNoArrayStoreCheckArrayCopy();
}

const char *OMR::Node::printIsNoArrayStoreCheckArrayCopy() {
  return self()->chkNoArrayStoreCheckArrayCopy() ? "noArrayStoreCheckArrayCopy "
                                                 : "";
}

bool OMR::Node::isArraysetLengthMultipleOfPointerSize() {
  TR_ASSERT(self()->getOpCodeValue() == TR::arrayset,
            "Opcode must be arrayset");
...
bool OMR::Node::isXorBitOpMem() {
  TR_ASSERT(self()->getOpCodeValue() == TR::bitOpMem ||
                self()->getOpCodeValue() == TR::bitOpMemND,
            "Opcode must be bitOpMem");
...
void OMR::Node::setXorBitOpMem(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::bitOpMem ||
            ...
            "Opcode must be bitOpMem");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting XOR flag on node %p to %d\n", self(), v))
...
const char *OMR::Node::printXorBitOpMem() {
...
void OMR::Node::setOrBitOpMem(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::bitOpMem ||
            ...
            "Opcode must be bitOpMem");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting OR flag on node %p to %d\n", self(), v))
...
const char *OMR::Node::printOrBitOpMem() {
...
void OMR::Node::setAndBitOpMem(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::bitOpMem ||
            ...
            "Opcode must be bitOpMem");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting AND flag on node %p to %d\n", self(), v))
...
const char *OMR::Node::printAndBitOpMem() {
  return self()->chkAndBitOpMem() ? "SubOp=AND " : "";
}

bool OMR::Node::isArrayChkPrimitiveArray1() {
  TR_ASSERT(self()->getOpCodeValue() == TR::ArrayCHK,
            "Opcode must be ArrayCHK");
...
void OMR::Node::setArrayChkPrimitiveArray1(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting arrayChkPrimitiveArray1 flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkArrayChkPrimitiveArray1() {
  return self()->getOpCodeValue() == TR::ArrayCHK &&
         _flags.testAny(arrayChkPrimitiveArray1);
}

const char *OMR::Node::printIsArrayChkPrimitiveArray1() {
...
void OMR::Node::setArrayChkReferenceArray1(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting arrayChkReferenceArray1 flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkArrayChkReferenceArray1() {
  return self()->getOpCodeValue() == TR::ArrayCHK &&
         _flags.testAny(arrayChkReferenceArray1);
}

const char *OMR::Node::printIsArrayChkReferenceArray1() {
...
void OMR::Node::setArrayChkPrimitiveArray2(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting arrayChkPrimitiveArray2 flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkArrayChkPrimitiveArray2() {
  return self()->getOpCodeValue() == TR::ArrayCHK &&
         _flags.testAny(arrayChkPrimitiveArray2);
}

const char *OMR::Node::printIsArrayChkPrimitiveArray2() {
...
void OMR::Node::setArrayChkReferenceArray2(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting arrayChkReferenceArray2 flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkArrayChkReferenceArray2() {
  return self()->getOpCodeValue() == TR::ArrayCHK &&
         _flags.testAny(arrayChkReferenceArray2);
}

const char *OMR::Node::printIsArrayChkReferenceArray2() {
...
bool OMR::Node::chkSkipWrtBar() {
  return self()->getOpCode().isWrtBar() && _flags.testAny(SkipWrtBar);
}

const char *OMR::Node::printIsSkipWrtBar() {
...
void OMR::Node::setIsHeapObjectWrtBar(bool v) {
  ...
  TR_ASSERT(self()->getOpCode().isWrtBar(), "Opcode must be wrtbar");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting heapObjectWrtBar flag on node %p to %d\n",
          self(), v))
    _flags.set(heapObjectWrtBar, v);
}

bool OMR::Node::chkHeapObjectWrtBar() {
  return self()->getOpCode().isWrtBar() && debug("useHeapObjectFlags") &&
         _flags.testAny(heapObjectWrtBar);
}

const char *OMR::Node::printIsHeapObjectWrtBar() {
...
void OMR::Node::setIsNonHeapObjectWrtBar(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting nonHeapObjectWrtBar flag on node %p to %d\n",
          self(), v))
...
bool OMR::Node::chkNonHeapObjectWrtBar() {
  return self()->getOpCode().isWrtBar() && debug("useHeapObjectFlags") &&
         _flags.testAny(nonHeapObjectWrtBar);
}

const char *OMR::Node::printIsNonHeapObjectWrtBar() {
...
bool OMR::Node::needsSignExtension() {
  return _flags.testAny(NeedsSignExtension) &&
         (self()->getOpCodeValue() == TR::iRegStore ||
          self()->getOpCodeValue() == TR::istore);
}

void OMR::Node::setNeedsSignExtension(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::iRegStore ||
                self()->getOpCode().isStore(),
            "assertion failure");
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting needsSignExtension flag on node %p to %d\n",
          self(), b))
    _flags.set(NeedsSignExtension, b);
}

const char *OMR::Node::printNeedsSignExtension() {
  return self()->needsSignExtension() ? "NeedsSignExt " : "";
}

bool OMR::Node::skipSignExtension() {
  return _flags.testAny(SkipSignExtension) &&
         (self()->getOpCodeValue() == TR::iRegLoad ||
          self()->getOpCodeValue() == TR::iadd ||
          self()->getOpCodeValue() == TR::isub);
}
void OMR::Node::setSkipSignExtension(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::iRegLoad ||
                self()->getOpCodeValue() == TR::iadd ||
                self()->getOpCodeValue() == TR::isub ||
                self()->getOpCode().isLoad(),
            "assertion failure");
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting skipSignExtension flag on node %p to %d\n",
          self(), b))
    _flags.set(SkipSignExtension, b);
}

const char *OMR::Node::printSkipSignExtension() {
  return self()->skipSignExtension() ? "SkipSignExt " : "";
}

bool OMR::Node::needsPrecisionAdjustment() {
  TR_ASSERT(self()->getOpCodeValue() == TR::fRegLoad ||
                self()->getOpCodeValue() == TR::dRegLoad,
            "Opcode must be FP global reg load");
  return _flags.testAny(precisionAdjustment);
}

void OMR::Node::setNeedsPrecisionAdjustment(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::fRegLoad ||
                self()->getOpCodeValue() == TR::dRegLoad,
            "Opcode must be FP global reg load");
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting needsPrecisionAdjustment flag on node %p to "
          "%d\n",
          self(), v))
    _flags.set(precisionAdjustment, v);
}

bool OMR::Node::chkNeedsPrecisionAdjustment() {
  return (self()->getOpCodeValue() == TR::fRegLoad ||
          self()->getOpCodeValue() == TR::dRegLoad) &&
         _flags.testAny(precisionAdjustment);
}

const char *OMR::Node::printNeedsPrecisionAdjustment() {
...
void OMR::Node::setIsDontMoveUnderBranch(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(
      self()->getOpCode().isLoadReg() || self()->getOpCode().isLoadVarDirect(),
      "assertion failure");
  if (self()->getOpCode().isLoadVarDirect() &&
      performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting dontMoveUnderBranch flag on node %p to %d\n",
          self(), v)) {
...
bool OMR::Node::chkDontMoveUnderBranch() {
  return (self()->getOpCode().isLoadReg() ||
          self()->getOpCode().isLoadVarDirect()) &&
         _flags.testAny(dontMoveUnderBranch);
}

const char *OMR::Node::printIsDontMoveUnderBranch() {
  return self()->chkDontMoveUnderBranch() ? "dontMoveUnderBranch " : "";
}

bool OMR::Node::canChkNodeCreatedByPRE() {
...
void OMR::Node::setIsNodeCreatedByPRE() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->canChkNodeCreatedByPRE(),
            "Unexpected opcode %s in setIsNodeCreatedByPRE()",
            self()->getOpCode().getName());
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting nodeCreatedByPRE flag on node %p\n",
          self()))
...
void OMR::Node::resetIsNodeCreatedByPRE() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->canChkNodeCreatedByPRE(),
            "Unexpected opcode %s in resetIsNodeCreatedByPRE()",
            self()->getOpCode().getName());
  if (performNodeTransformation1(
          c, "ReO^O NODE FLAGS: Setting nodeCreatedByPRE flag on node %p\n",
          self()))
...
bool OMR::Node::chkNodeCreatedByPRE() {
  TR_ASSERT(self()->canChkNodeCreatedByPRE(),
            "Unexpected opcode %s in chkNodeCreatedByPRE()",
            self()->getOpCode().getName());
  return _flags.testAny(nodeCreatedByPRE) &&
         self()->getOpCode().isLoadVarDirect();
}

const char *OMR::Node::printIsNodeCreatedByPRE() {
...
bool OMR::Node::chkIsPrivatizedInlinerArg() {
  return self()->getOpCode().isStoreDirectOrReg() &&
         self()->isPrivatizedInlinerArg();
}

const char *OMR::Node::printIsPrivatizedInlinerArg() {
...
void OMR::Node::setIsMaxLoopIterationGuard(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting maxLoopIterationGuard flag on node %p to "
          "%d\n",
          self(), v))
    _flags.set(maxLoopIterationGuard, v);
}

const char *OMR::Node::printIsMaxLoopIterationGuard() {
  return self()->isMaxLoopIterationGuard() ? "maxLoopIternGuard " : "";
}

bool OMR::Node::isStopTheWorldGuard() {
  return self()->isHCRGuard() || self()->isOSRGuard() ||
         self()->isBreakpointGuard();
}

bool OMR::Node::isProfiledGuard() {
  return _flags.testValue(inlineGuardMask, inlineProfiledGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsProfiledGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting inlineProfiledGuard flag on node %p\n",
          self()))
    _flags.set(inlineProfiledGuard);
}

const char *OMR::Node::printIsProfiledGuard() {
  return self()->isProfiledGuard() ? "inlineProfiledGuard " : "";
}

bool OMR::Node::isInterfaceGuard() {
  return _flags.testValue(inlineGuardMask, inlineInterfaceGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsInterfaceGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting inlineInterfaceGuard flag on node %p\n",
          self()))
    _flags.set(inlineInterfaceGuard);
}

const char *OMR::Node::printIsInterfaceGuard() {
  return self()->isInterfaceGuard() ? "inlineInterfaceGuard " : "";
}

bool OMR::Node::isAbstractGuard() {
  return _flags.testValue(inlineGuardMask, inlineAbstractGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsAbstractGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting inlineAbstractGuard flag on node %p\n",
          self()))
    _flags.set(inlineAbstractGuard);
}

const char *OMR::Node::printIsAbstractGuard() {
  return self()->isAbstractGuard() ? "inlineAbstractGuard " : "";
}

bool OMR::Node::isHierarchyGuard() {
  return _flags.testValue(inlineGuardMask, inlineHierarchyGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsHierarchyGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting inlineHierarchyGuard flag on node %p\n",
          self()))
    _flags.set(inlineHierarchyGuard);
}

const char *OMR::Node::printIsHierarchyGuard() {
  return self()->isHierarchyGuard() ? "inlineHierarchyGuard " : "";
}

bool OMR::Node::isNonoverriddenGuard() {
  return _flags.testValue(inlineGuardMask, inlineNonoverriddenGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsNonoverriddenGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c,
          "O^O NODE FLAGS: Setting inlineNonoverriddenGuard flag on node %p\n",
          self()))
    _flags.set(inlineNonoverriddenGuard);
}

const char *OMR::Node::printIsNonoverriddenGuard() {
  return self()->isNonoverriddenGuard() ? "inlineNonoverriddenGuard " : "";
}

bool OMR::Node::isSideEffectGuard() {
  return _flags.testValue(inlineGuardMask, sideEffectGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsSideEffectGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting sideEffectGuard flag on node %p\n",
          self()))
    _flags.set(sideEffectGuard);
}

const char *OMR::Node::printIsSideEffectGuard() {
  return self()->isSideEffectGuard() ? "sideEffectGuard " : "";
}

bool OMR::Node::isDummyGuard() {
  return _flags.testValue(inlineGuardMask, dummyGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsDummyGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting dummyGuard flag on node %p\n", self()))
    _flags.set(dummyGuard);
}

const char *OMR::Node::printIsDummyGuard() {
  return self()->isDummyGuard() ? "dummyGuard " : "";
}

bool OMR::Node::isHCRGuard() {
  return _flags.testValue(inlineGuardMask, inlineHCRGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsHCRGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting inlineHCRGuard flag on node %p\n",
          self()))
    _flags.set(inlineHCRGuard);
}

const char *OMR::Node::printIsHCRGuard() {
  return self()->isHCRGuard() ? "inlineHCRGuard " : "";
}

bool OMR::Node::isTheVirtualGuardForAGuardedInlinedCall() {
  return (_flags.testAny(inlineGuardMask) || self()->isHCRGuard()) &&
         self()->getOpCode().isIf();
}

void OMR::Node::resetIsTheVirtualGuardForAGuardedInlinedCall() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c,
          "O^O NODE FLAGS: Resetting isTheVirtualGuardForAGuardedInlinedCall "
          "flag on node %p\n",
          self()))
...
bool OMR::Node::isNopableInlineGuard() {
  TR::Compilation *c = TR::comp();
  return self()->isTheVirtualGuardForAGuardedInlinedCall() &&
         !self()->isProfiledGuard() &&
         !(self()->isBreakpointGuard() &&
           c->getOption(TR_DisableNopBreakpointGuard));
}

bool OMR::Node::isMutableCallSiteTargetGuard() {
  return _flags.testValue(inlineGuardMask, mutableCallSiteTargetGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsMutableCallSiteTargetGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(c,
                                 "O^O NODE FLAGS: Setting "
                                 "mutableCallSiteTargetGuard flag on node %p\n",
                                 self()))
    _flags.set(mutableCallSiteTargetGuard);
}

const char *OMR::Node::printIsMutableCallSiteTargetGuard() {
  return self()->isMutableCallSiteTargetGuard() ? "mutableCallSiteTargetGuard "
                                                : "";
}

bool OMR::Node::isMethodEnterExitGuard() {
  return _flags.testValue(inlineGuardMask, methodEnterExitGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsMethodEnterExitGuard(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (b &&
      performNodeTransformation2(c,
                                 "O^O NODE FLAGS: Setting methodEnterExitGuard "
                                 "flag to %d on node %p\n",
                                 b, self()))
    _flags.set(methodEnterExitGuard);
}

const char *OMR::Node::printIsMethodEnterExitGuard() {
  return self()->isMethodEnterExitGuard() ? "methodEnterExit " : "";
}

bool OMR::Node::isDirectMethodGuard() {
  return _flags.testValue(inlineGuardMask, directMethodGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsDirectMethodGuard(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (b &&
      performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting directMethodGuard flag to %d on node %p\n",
          b, self()))
    _flags.set(directMethodGuard);
}

const char *OMR::Node::printIsDirectMethodGuard() {
  return self()->isDirectMethodGuard() ? "directMethodGuard " : "";
}

bool OMR::Node::isOSRGuard() {
  return _flags.testValue(inlineGuardMask, osrGuard) &&
         self()->getOpCode().isIf();
}

void OMR::Node::setIsOSRGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting osrGuard flag on node %p\n", self()))
    _flags.set(osrGuard);
}

void OMR::Node::setIsBreakpointGuard() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting breakpoint guard flag on node %p\n",
          self()))
    _flags.set(breakpointGuard);
}

bool OMR::Node::isBreakpointGuard() {
  return _flags.testValue(inlineGuardMask, breakpointGuard) &&
         self()->getOpCode().isIf();
}

const char *OMR::Node::printIsBreakpointGuard() {
  return self()->isBreakpointGuard() ? "breakpointGuard " : "";
}

const char *OMR::Node::printIsOSRGuard() {
  return self()->isOSRGuard() ? "osrGuard " : "";
}

bool OMR::Node::childrenWereSwapped() {
...
void OMR::Node::setSwappedChildren(bool v) {
  ...
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting swappedChildren flag on node %p to %d\n",
          self(), v))
...
void OMR::Node::setIsVersionableIfWithMaxExpr(TR::Compilation *c) {
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting versionIfWithMaxExpr flag on node %p\n",
          self()))
...
void OMR::Node::setIsVersionableIfWithMinExpr(TR::Compilation *c) {
  TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
  if (performNodeTransformation1(
          c, "O^O NODE FLAGS: Setting versionIfWithMinExpr flag on node %p\n",
          self()))
...
void OMR::Node::setStoreAlreadyEvaluated(bool b) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting storeAlreadyEvaluated flag on node %p to "
          "%d\n",
          self(), b))
...
const char *OMR::Node::printStoreAlreadyEvaluated() {
...
void OMR::Node::setSeenRealReference(bool b) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting seenRealReference flag on node %p to %d\n",
          self(), b))
    _flags.set(SeenRealReference, b);
}

bool OMR::Node::chkSeenRealReference() {
  return self()->getOpCode().isLoadReg() && _flags.testAny(SeenRealReference);
}

const char *OMR::Node::printIsSeenRealReference() {
  return self()->chkSeenRealReference() ? "SeenRealReference " : "";
}

bool OMR::Node::normalizeNanValues() {
  TR_ASSERT(self()->getOpCodeValue() == TR::fbits2i ||
                self()->getOpCodeValue() == TR::dbits2l,
...
void OMR::Node::setNormalizeNanValues(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting mustNormalizeNanValues flag on node %p to "
          "%d\n",
          self(), v))
...
bool OMR::Node::chkNormalizeNanValues() {
  return (self()->getOpCodeValue() == TR::fbits2i ||
          self()->getOpCodeValue() == TR::dbits2l) &&
         _flags.testAny(mustNormalizeNanValues);
}

const char *OMR::Node::printNormalizeNanValues() {
...
void OMR::Node::setIsHighWordZero(bool b) {
  ...
  TR_ASSERT(self()->getType().isInt64() || self()->getType().isAddress(),
            "Can only call this for a long or address opcode\n");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting highWordZero flag on node %p to %d\n",
          self(), b))
    _flags.set(highWordZero, b);
}

bool OMR::Node::chkHighWordZero() {
  return (self()->getType().isInt64() || self()->getType().isAddress()) &&
         _flags.testAny(highWordZero);
}

const char *OMR::Node::printIsHighWordZero() {
  return self()->chkHighWordZero() ? "highWordZero " : "";
}

bool OMR::Node::isUnsigned() {
  TR_ASSERT(!self()->getType().isInt64(),
            "can only be used for nonlong opcodes");
...
void OMR::Node::setUnsigned(bool b) {
  ...
  TR_ASSERT(!self()->getType().isInt64(),
            "can only be used for nonlong opcodes");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting unsigned flag on node %p to %d\n", self(),
          b))
    _flags.set(Unsigned, b);
}

bool OMR::Node::chkUnsigned() {
  return !self()->getType().isInt64() && !self()->getOpCode().isIf() &&
         !self()->getOpCode().isReturn() && _flags.testAny(Unsigned);
}

const char *OMR::Node::printIsUnsigned() {
  return self()->chkUnsigned() ? "Unsigned " : "";
}

bool OMR::Node::isClassPointerConstant() {
  TR_ASSERT((self()->getOpCodeValue() == TR::aconst) ||
...
void OMR::Node::setIsClassPointerConstant(bool b) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT((self()->getOpCodeValue() == TR::aconst),
            "Can only call this for aconst\n");
  if (performNodeTransformation2(c,
                                 "O^O NODE FLAGS: Setting classPointerConstant "
                                 "flag on node %p to %d\n",
                                 self(), b))
...
bool OMR::Node::chkClassPointerConstant() {
  return (self()->getOpCodeValue() == TR::aconst ||
          self()->getOpCodeValue() == TR::aloadi) &&
         _flags.testAny(classPointerConstant);
}

const char *OMR::Node::printIsClassPointerConstant() {
...
void OMR::Node::setIsMethodPointerConstant(bool b) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting methodPointerConstant flag on node %p to "
          "%d\n",
          self(), b))
...
const char *OMR::Node::printIsMethodPointerConstant() {
...
bool OMR::Node::isUnneededIALoad() {
  return (self()->getOpCodeValue() == TR::aloadi &&
          _flags.testAny(unneededIALoad));
}

void OMR::Node::setUnneededIALoad(bool v) {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::aloadi,
            "Can only call this for iaload");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting unneededIALoad flag on node %p to %d\n",
          self(), v))
    _flags.set(unneededIALoad, v);
}

bool OMR::Node::canSkipSync() {
  TR_ASSERT(self()->getOpCodeValue() == TR::monexit ||
                self()->getOpCodeValue() == TR::monent,
...
void OMR::Node::setSkipSync(bool v) {
  ...
  if (performTransformation(
          c, "O^O NODE FLAGS: Setting skipSync flag on node %p to %d\n", self(),
          v))
    _flags.set(skipSync, v);
}

bool OMR::Node::chkSkipSync() {
  return (self()->getOpCodeValue() == TR::monexit ||
          self()->getOpCodeValue() == TR::monent) &&
         _flags.testAny(skipSync);
}

const char *OMR::Node::printIsSkipSync() {
...
void OMR::Node::setStaticMonitor(bool v) {
  ...
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting staticMonitor flag on node %p to %d\n",
          self(), v))
    _flags.set(staticMonitor, v);
}

bool OMR::Node::chkStaticMonitor() {
  return (self()->getOpCodeValue() == TR::monent ||
          self()->getOpCodeValue() == TR::monexit) &&
         _flags.testAny(staticMonitor);
}

const char *OMR::Node::printIsStaticMonitor() {
...
void OMR::Node::setSyncMethodMonitor(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting syncMethodMonitor flag on node %p to %d\n",
          self(), v))
...
const char *OMR::Node::printIsSyncMethodMonitor() {
...
void OMR::Node::setReadMonitor(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::monent ||
            ...
            "Opcode must be monent or monexit");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting readMonitor flag on node %p to %d\n",
          self(), v))
...
const char *OMR::Node::printIsReadMonitor() {
...
void OMR::Node::setLocalObjectMonitor(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting localObjectMonitor flag on node %p to %d\n",
          self(), v))
...
const char *OMR::Node::printIsLocalObjectMonitor() {
...
void OMR::Node::setAllocationCanBeRemoved(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting allocationCanBeRemoved flag on node %p to "
          "%d\n",
          self(), v))
...
const char *OMR::Node::printAllocationCanBeRemoved() {
  return self()->chkAllocationCanBeRemoved() ? "allocationCanBeRemoved " : "";
}

bool OMR::Node::canSkipZeroInitialization() {
  TR_ASSERT(self()->getOpCodeValue() == TR::New ||
                self()->getOpCodeValue() == TR::variableNew ||
                self()->getOpCodeValue() == TR::variableNewArray ||
                self()->getOpCodeValue() == TR::newarray ||
                self()->getOpCodeValue() == TR::anewarray ||
...
void OMR::Node::setCanSkipZeroInitialization(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::New ||
            ...
            "Opcode must be newarray or anewarray");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting skipZeroInit flag on node %p to %d\n",
          self(), v))
...
bool OMR::Node::chkSkipZeroInitialization() {
  return _flags.testAny(skipZeroInit) &&
         (self()->getOpCodeValue() == TR::New ||
          self()->getOpCodeValue() == TR::newarray ||
          self()->getOpCodeValue() == TR::anewarray ||
          self()->getOpCodeValue() == TR::multianewarray);
}

const char *OMR::Node::printCanSkipZeroInitialization() {
  return self()->chkSkipZeroInitialization() ? "skipZeroInit " : "";
}

bool OMR::Node::isAdjunct() {
  return (self()->getOpCodeValue() == TR::lmul ||
          self()->getOpCodeValue() == TR::luadd ||
          self()->getOpCodeValue() == TR::lusub ||
          self()->getOpCodeValue() == TR::ladd ||
...
void OMR::Node::setIsAdjunct(bool v) {
  TR_ASSERT(self()->getOpCodeValue() == TR::lmul ||
                ...
                self()->getOpCodeValue() == TR::ladd ||
                self()->getOpCodeValue() == TR::lsub,
            "Opcode must be lmul, luadd, lusub, ladd or lsub");
  _flags.set(adjunct, v);
}

bool OMR::Node::cannotTrackLocalUses() {
  TR_ASSERT(self()->getOpCodeValue() == TR::loadaddr,
            "Opcode must be loadaddr");
...
void OMR::Node::setCannotTrackLocalUses(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::loadaddr,
            "Opcode must be loadaddr");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting cannotTrack flag on node %p to %d\n",
          self(), v))
...
bool OMR::Node::chkCannotTrackLocalUses() {
  return self()->getOpCodeValue() == TR::loadaddr &&
         _flags.testAny(cantTrackLocalUses);
}

const char *OMR::Node::printCannotTrackLocalUses() {
...
void OMR::Node::setEscapesInColdBlock(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting escapesInColdBlock flag on node %p to %d\n",
          self(), v))
...
bool OMR::Node::chkEscapesInColdBlock() {
  return self()->getOpCodeValue() == TR::loadaddr &&
         _flags.testAny(coldBlockEscape);
}

const char *OMR::Node::printEscapesInColdBlock() {
...
bool OMR::Node::cannotTrackLocalStringUses() {
  TR::Compilation *c = TR::comp();
  TR_ASSERT(self()->getOpCodeValue() == TR::loadaddr,
            "Opcode must be loadaddr (flag shared on PLX)");
...
void OMR::Node::setCannotTrackLocalStringUses(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting cannotTrackString flag on node %p to %d\n",
          self(), v))
...
bool OMR::Node::chkCannotTrackLocalStringUses() {
  return self()->getOpCodeValue() == TR::loadaddr &&
         _flags.testAny(cantTrackLocalStringUses);
}

const char *OMR::Node::printCannotTrackLocalStringUses() {
...
bool OMR::Node::canOmitSync() {
  TR_ASSERT((self()->getOpCodeValue() == TR::allocationFence) ||
                (self()->getOpCodeValue() == TR::fullFence),
...
void OMR::Node::setOmitSync(bool v) {
  ...
  TR_ASSERT((self()->getOpCodeValue() == TR::allocationFence) ||
            ...
            "Opcode must be TR::allocationFence or TR::fullFence");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting omitSync flag on node %p to %d\n", self(),
          v))
    _flags.set(omitSync, v);
}

bool OMR::Node::chkOmitSync() {
  return (self()->getOpCodeValue() == TR::allocationFence) &&
         _flags.testAny(omitSync);
}

const char *OMR::Node::printIsOmitSync() {
  return self()->chkOmitSync() ? "omitSync " : "";
}

bool OMR::Node::isSimpleDivCheck() {
  TR_ASSERT((self()->getOpCode().isDiv() || self()->getOpCode().isRem()),
            "Opcode must be div/rem");
...
void OMR::Node::setSimpleDivCheck(bool v) {
  ...
  TR_ASSERT((self()->getOpCode().isDiv() || self()->getOpCode().isRem()),
            "Opcode must be div/rem");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting simpleDivCheck flag on node %p to %d\n",
          self(), v))
    _flags.set(simpleDivCheck, v);
}

bool OMR::Node::chkSimpleDivCheck() {
  return (self()->getOpCode().isDiv() || self()->getOpCode().isRem()) &&
         _flags.testAny(simpleDivCheck);
}

const char *OMR::Node::printIsSimpleDivCheck() {
  return self()->chkSimpleDivCheck() ? "simpleDivCheck " : "";
}

bool OMR::Node::isNormalizedShift() {
  TR_ASSERT(
      (self()->getOpCode().isLeftShift() ||
       self()->getOpCode().isRightShift() || self()->getOpCode().isRotate()),
...
void OMR::Node::setNormalizedShift(bool v) {
  ...
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting normalizedShift flag on node %p to %d\n",
          self(), v))
    _flags.set(normalizedShift, v);
}

bool OMR::Node::chkNormalizedShift() {
  return (self()->getOpCode().isLeftShift() ||
          self()->getOpCode().isRightShift()) &&
         _flags.testAny(normalizedShift);
}

const char *OMR::Node::printIsNormalizedShift() {
  return self()->chkNormalizedShift() ? "normalizedShift " : "";
}

bool OMR::Node::isFPStrictCompliant() {
...
void OMR::Node::setIsFPStrictCompliant(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting resultFPStrictCompliant flag on node %p to "
          "%d\n",
          self(), v))
    _flags.set(resultFPStrictCompliant, v);
}

const char *OMR::Node::printIsFPStrictCompliant() {
  return self()->isFPStrictCompliant() ? "FPPrecise " : "";
}

bool OMR::Node::isUnneededConversion() {
...
void OMR::Node::setUnneededConversion(bool v) {
  ...
  if (performNodeTransformation2(
          c,
          "O^O NODE FLAGS: Setting unneededConversion flag on node %p to %d\n",
          self(), v))
    _flags.set(unneededConv, v);
}

const char *OMR::Node::printIsUnneededConversion() {
  return self()->isUnneededConversion() ? "unneededConv " : "";
}

bool OMR::Node::parentSupportsLazyClobber() {
  return self()->getOpCode().isConversion() &&
         _flags.testAny(ParentSupportsLazyClobber);
}
...
void OMR::Node::setParentSupportsLazyClobber(bool v) {
  ...
  TR_ASSERT(
      self()->getReferenceCount() <= 1,
      "Lazy clobber requires node reference count of 1 (actual value is %d)",
      self()->getReferenceCount());
  _flags.set(ParentSupportsLazyClobber, v);
}

const char *OMR::Node::printParentSupportsLazyClobber() {
...
bool OMR::Node::useCallForFloatToFixedConversion() {
  TR_ASSERT(self()->isFloatToFixedConversion(),
            "Opcode must be a float to fixed conversion");
  return self()->isFloatToFixedConversion() &&
         _flags.testAny(callForFloatToFixedConversion);
}
...
const char *OMR::Node::printUseCallForFloatToFixedConversion() {
  return self()->chkUseCallForFloatToFixedConversion()
             ? "useCallForFloatToFixedConv "
             : "";
}
...
bool OMR::Node::chkReturnIsDummy() {
  return (self()->getOpCodeValue() == TR::Return) &&
         _flags.testAny(returnIsDummy);
}

const char *OMR::Node::printReturnIsDummy() {
...
void OMR::Node::setAlignTLHAlloc(bool v) {
  ...
  TR_ASSERT(self()->getOpCodeValue() == TR::New,
            "Opcode value must be TR::New");
  if (performNodeTransformation2(
          c, "O^O NODE FLAGS: Setting align on TLH flag on node %p to %d\n",
          self(), v))
...
const char *OMR::Node::printCopyToNewVirtualRegister() {
...
bool OMR::Node::nodeRequiresConditionCodes() {
  ...
  return self()->chkOpsNodeRequiresConditionCodes()
             ? _flags.testAny(requiresConditionCodes)
             : false;
}
...
const char *OMR::Node::printRequiresConditionCodes() {
  return self()->nodeRequiresConditionCodes() ? "requiresConditionCodes " : "";
}

static void resetFlagsForCodeMotionHelper(TR::Node *node,
  ...
  if (node->chkCannotOverflow()) node->setCannotOverflow(false);
  if (node->chkHighWordZero()) node->setIsHighWordZero(false);
  if (node->getOpCode().canUseBranchOnCount() && node->isUseBranchOnCount())
    node->setIsUseBranchOnCount(false);

  if (node->chkIsReferenceNonNull()) node->setReferenceIsNonNull(false);
}
...
// a node where these properties are not true. Can be used to reset other
// properties as well (over the ones that are already reset).
//
void OMR::Node::resetFlagsForCodeMotion() {
...
bool OMR::Node::isLoadOfStaticFinalField() {
  if (self()->hasSymbolReference()) {
    TR::Symbol *sym = self()->getSymbol();
    if (sym->isFinal() && sym->isStaticField()) return true;
  }
  return false;
}
