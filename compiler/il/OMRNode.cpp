...
OMR::Node::copy(TR::Node* from)
{
    ...
    if (opCode.isIf() || opCodeValue == TR::anewarray || opCodeValue == TR::newarray || opCodeValue == TR::arraycopy || opCodeValue == TR::tstart || opCodeValue == TR::arrayset)
        ++numChildren;
 ...
 */
void OMR::Node::copyChildren(TR::Node* from, uint16_t numChildren /* ignored if !forNodeExtensionOnly */, bool forNodeExtensionOnly)
{
...
bool OMR::Node::isTernaryHigh()
{
    if (((self()->getOpCodeValue() == TR::luaddc) || (self()->getOpCodeValue() == TR::lusubb))
        ...
        if ((ccOpValue == TR::computeCC) && (((self()->getOpCodeValue() == TR::luaddc) && (pairOpValue == TR::luadd)) || ((self()->getOpCodeValue() == TR::lusubb) && (pairOpValue == TR::lusub)) || ((self()->getOpCodeValue() == TR::luaddc) && (pairOpValue == TR::ladd)) || ((self()->getOpCodeValue() == TR::lusubb) && (pairOpValue == TR::lsub))))
            return true;
    }
    return false;
}
...
bool OMR::Node::safeToDoRecursiveDecrement()
{
    ...
    if (!refCanBeKilled(self(), comp)) {
        return true;
    } else if (self()->getOpCode().isAdd() && !refCanBeKilled(self()->getFirstChild(), comp) && !refCanBeKilled(self()->getSecondChild(), comp)) {
        return true;
    } else if (self()->getOpCode().isAdd() && self()->getFirstChild()->getOpCode().isAdd() && !refCanBeKilled(self()->getFirstChild()->getFirstChild(), comp) && !refCanBeKilled(self()->getFirstChild()->getSecondChild(), comp) && !refCanBeKilled(self()->getSecondChild(), comp)) {
...
bool OMR::Node::canGCandReturn()
{
    ...
    if (self()->getOpCode().isResolveCheck()) {
        ...
        if (node->getOpCode().isIndirect() && node->getOpCode().isLoadVarOrStore() && node->getSymbolReference()->getSymbol()->isShadow() && !node->getSymbolReference()->isFromLiteralPool())
            return false;
...
bool OMR::Node::canGCandExcept()
{
    ...
    return (node->getOpCode().isCheck() || (node->getOpCode().hasSymbolReference() && node->getSymbolReference()->canGCandExcept()));
}
...
{
    TR::Compilation* comp = TR::comp();
    if (self()->getSymbolReference()->getSymbol()->dontEliminateStores(comp, isForLocalDeadStore) || (self()->getSymbol()->isAutoOrParm() && self()->storedValueIsIrrelevant()))
        return true;
...
bool OMR::Node::addressPointsAtObject()
{
    ...
    if (self()->getOpCode().hasSymbolReference() && (comp->getSymRefTab()->isVtableEntrySymbolRef(self()->getSymbolReference())))
        return false;
...
bool OMR::Node::mayModifyValue(TR::SymbolReference* symRef)
{
    ...
    if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::monexit || (node->getOpCode().hasSymbolReference() && node->getSymbol()->isVolatile()) || symbolIsUnresolved) {
    ...
    if (node->getOpCode().isStore()) {
        ...
        if (symbol->isParm())
            return storeSymbol->isParm() && ((TR::ParameterSymbol*)symbol)->getParameterOffset() == ((TR::ParameterSymbol*)storeSymbol)->getParameterOffset();
...
bool OMR::Node::isRematerializable(TR::Node* parent, bool onlyConsiderOpCode)
{
    ...
    if (opCode.getOpCodeValue() == TR::pd2i || opCode.getOpCodeValue() == TR::pd2iOverflow || opCode.getOpCodeValue() == TR::pd2l || opCode.getOpCodeValue() == TR::pd2lOverflow)
        return false;
#endif
    ...
    if (opCode.isArrayLength() || opCode.isConversion()) {
        if (onlyConsiderOpCode || (((self()->getFirstChild()->getFutureUseCount() & 0x7fff) > 0) && (self()->getFirstChild()->getReferenceCount() > 1)))
            return true;
    }
    ...
    if (opCode.isAdd() || opCode.isSub() || opCode.isMul() || opCode.isLeftShift() || opCode.isRightShift() || opCode.isAnd() || opCode.isOr() || opCode.isXor()) {
        ...
        if (!onlyConsiderOpCode) {
            if (((self()->getSecondChild()->getFutureUseCount() & 0x7fff) > 0) && (self()->getSecondChild()->getReferenceCount() > 1))
                secondChildLive = true;

            if (((self()->getFirstChild()->getFutureUseCount() & 0x7fff) > 0) && (self()->getFirstChild()->getReferenceCount() > 1))
                firstChildLive = true;
...
bool OMR::Node::isZeroExtension()
{
    ...
    //a2x extensions
    if (self()->getOpCode().isConversion() && self()->getType().isIntegral() && self()->getFirstChild()->getType().isAddress() && self()->getSize() > self()->getFirstChild()->getSize())
        return true;

    //x2a extensions
    if (self()->getOpCode().isConversion() && self()->getType().isAddress() && self()->getSize() > self()->getFirstChild()->getSize())
        return true;
...
bool OMR::Node::isClassUnloadingConst()
{
    ...
    if ((((constNode->isClassPointerConstant()) && !TR::Compiler->cls.sameClassLoaders(c, (TR_OpaqueClassBlock*)constNode->getAddress(), c->getCurrentMethod()->classOfMethod())) || ((constNode->isMethodPointerConstant()) && !c->compileRelocatableCode() && !TR::Compiler->cls.sameClassLoaders(c, c->fe()->createResolvedMethod(c->trMemory(), (TR_OpaqueMethodBlock*)constNode->getAddress(), c->getCurrentMethod())->classOfMethod(), c->getCurrentMethod()->classOfMethod()))))
...
bool OMR::Node::nodeMightKillCondCode()
{
    ...
    if (((opcode.isLoad() || opcode.isStore() || opcode.isLoadReg() || opcode.isStoreDirectOrReg() || opcode.isLoadAddr())
            && (self()->getSize() == 4 || self()->getSize() == 8 || self()->getSize() == 2 || self()->getSize() == 1 || self()->getSize() == 0)))
        return false;

    if (opcode.isArrayRef() && self()->getSecondChild()->getOpCode().isLoadConst() && self()->getSecondChild()->get64bitIntegralValue() > 0 && self()->getSecondChild()->get64bitIntegralValue() < 4096) // address increment can use LA or fold into MemoryReference
        return false;
#endif
...
OMR::Node::createStoresForVar(TR::SymbolReference*& nodeRef, TR::TreeTop* insertBefore, bool simpleRef)
{
    ...
    if ((self()->getReferenceCount() == 1) && self()->getOpCode().hasSymbolReference() && self()->getSymbolReference()->getSymbol()->isAuto()) {
    ...
    if ((self()->hasPinningArrayPointer() && self()->computeIsInternalPointer()) || (self()->getOpCode().isLoadVarDirect() && self()->getSymbolReference()->getSymbol()->isAuto() && self()->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer()))
    ...
    if (isInternalPointer && self()->getOpCode().isArrayRef() && (comp->getSymRefTab()->getNumInternalPointers() >= (comp->maxInternalPointers() / 2) || comp->cg()->supportsComplexAddressing()) && (self()->getReferenceCount() == 1)) {
        storesNeedToBeCreated = false;
        TR::Node* firstChild = self()->getFirstChild();
        ...
        if (!firstChild->getOpCode().isArrayRef() && !firstChild->isInternalPointer() /* &&
...
OMR::Node::computeDataType()
{
    ...
    if (_opCode.hasSymbolReference() || self()->hasRegLoadStoreSymbolReference()) {
        TR::SymbolReference* symRef = (_opCode.hasSymbolReference()) ? self()->getSymbolReference() : (self()->hasRegLoadStoreSymbolReference()) ? self()->getRegLoadStoreSymbolReference() : NULL;
...
{
    return (self()->getOpCodeValue() == TR::loadaddr ? true : (_flags.testAny(nodeIsNonNull) || self()->isInternalPointer() || (self()->getOpCode().hasSymbolReference() && self()->getSymbol()->isInternalPointer())));
}
...
void OMR::Node::setLiveMonitorInitStore(bool v)
{
    TR::Compilation* c = TR::comp();
    TR_ASSERT((self()->getOpCodeValue() == TR::astore), "Opcode must be astore");
    if (self()->getOpCode().hasSymbolReference() && self()->getSymbol()->holdsMonitoredObject() && performNodeTransformation2(c, "O^O NODE FLAGS: Setting liveMonitorInitStore flag on node %p to %d\n", self(), v))
...
{
    TR::Compilation* c = TR::comp();
    if (self()->getOpCode().isLoadVar() && self()->getType().isInt32() && performNodeTransformation2(c, "O^O NODE FLAGS: Setting useSignExtensionMode flag on node %p to %d\n", self(), b)) {
...
{
    return _flags.testAny(SkipSignExtension) && (self()->getOpCodeValue() == TR::iRegLoad || self()->getOpCodeValue() == TR::iadd || self()->getOpCodeValue() == TR::isub);
}
void OMR::Node::setSkipSignExtension(bool b)
{
    TR::Compilation* c = TR::comp();
    TR_ASSERT(self()->getOpCodeValue() == TR::iRegLoad || self()->getOpCodeValue() == TR::iadd || self()->getOpCodeValue() == TR::isub || self()->getOpCode().isLoad(), "assertion failure");
...
void OMR::Node::setIsDontMoveUnderBranch(bool v)
{
    ...
    if (self()->getOpCode().isLoadVarDirect() && performNodeTransformation2(c, "O^O NODE FLAGS: Setting dontMoveUnderBranch flag on node %p to %d\n", self(), v)) {
...
{
    TR::Compilation* c = TR::comp();
    return self()->isTheVirtualGuardForAGuardedInlinedCall() && !self()->isProfiledGuard() && !(self()->isBreakpointGuard() && c->getOption(TR_DisableNopBreakpointGuard));
}
...
{
    TR_ASSERT(self()->getOpCodeValue() == TR::New || self()->getOpCodeValue() == TR::variableNew || self()->getOpCodeValue() == TR::variableNewArray || self()->getOpCodeValue() == TR::newarray || self()->getOpCodeValue() == TR::anewarray || self()->getOpCodeValue() == TR::multianewarray,
        "Opcode must be newarray or anewarray");
...
{
    TR::Compilation* c = TR::comp();
    TR_ASSERT(self()->getOpCodeValue() == TR::New || self()->getOpCodeValue() == TR::newarray || self()->getOpCodeValue() == TR::anewarray || self()->getOpCodeValue() == TR::variableNewArray || self()->getOpCodeValue() == TR::variableNew || self()->getOpCodeValue() == TR::multianewarray,
        "Opcode must be newarray or anewarray");
...
{
    return _flags.testAny(skipZeroInit) && (self()->getOpCodeValue() == TR::New || self()->getOpCodeValue() == TR::newarray || self()->getOpCodeValue() == TR::anewarray || self()->getOpCodeValue() == TR::multianewarray);
}
...
bool OMR::Node::isAdjunct()
{
    return (self()->getOpCodeValue() == TR::lmul || self()->getOpCodeValue() == TR::luadd || self()->getOpCodeValue() == TR::lusub
               || self()->getOpCodeValue() == TR::ladd || self()->getOpCodeValue() == TR::lsub)
