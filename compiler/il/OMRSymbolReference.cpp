...
void OMR::SymbolReference::init(TR::SymbolReferenceTable* symRefTab, uint32_t refNumber, TR::Symbol* sym,
  intptrj_t offset, mcount_t owningMethodIndex, int32_t cpIndex, int32_t unresolvedIndex, bool checkNoNamedShadow)
{
...
OMR::SymbolReference::SymbolReference(TR::SymbolReferenceTable* symRefTab, TR::Symbol* symbol, intptrj_t offset)
{
  self()->init(symRefTab, symRefTab->assignSymRefNumber(self()), symbol, offset);
}

OMR::SymbolReference::SymbolReference(
  TR::SymbolReferenceTable* symRefTab, int32_t refNumber, TR::Symbol* ps, intptrj_t offset)
{
  self()->init(symRefTab, refNumber, ps, offset);
}

OMR::SymbolReference::SymbolReference(TR::SymbolReferenceTable* symRefTab,
  TR::SymbolReferenceTable::CommonNonhelperSymbol number, TR::Symbol* ps, intptrj_t offset)
{
 ...
 */
OMR::SymbolReference::SymbolReference(
  TR::SymbolReferenceTable* symRefTab, TR::Symbol* symbol, intptrj_t offset, const char* name)
{
  self()->init(symRefTab, symRefTab->assignSymRefNumber(self()), symbol, offset,
...
{
  return (_flags.testAny(ReallySharesSymbol) || (c->hasUnsafeSymbol() && (_symbol->isStatic() || _symbol->isShadow())));
}
...
bool OMR::SymbolReference::isTemporary(TR::Compilation* c)
{
  return self()->getSymbol()->isAuto()
    && (self()->getCPIndex() >= self()->getOwningMethodSymbol(c)->getFirstJitTempIndex() || self()->getCPIndex() < 0);
}
...
uintptrj_t* OMR::SymbolReference::getKnownObjectReferenceLocation(TR::Compilation* comp)
{
  return self()->hasKnownObjectIndex() ? comp->getKnownObjectTable()->getPointerLocation(self()->getKnownObjectIndex())
                                       : NULL;
}
