...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64ImmSymInstruction* instr)
{
  ...
  if (name)
    trfprintf(pOutFile,
              "%s \t" POINTER_PRINTF_FORMAT "\t\t; Direct Call \"%s\"",
              getOpCodeName(&instr->getOpCode()),
                ...
                TR::ARM64ConditionalBranchInstruction* instr)
{
  ...
  trfprintf(
    pOutFile, "b.%s \t", ARM64ConditionNames[instr->getConditionCode()]);
  print(pOutFile, label);
  if (snippet) {
    trfprintf(pOutFile, " (%s)", getName(snippet));
  }
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64AdminInstruction* instr)
{
  ...
  if (node) {
    if (node->getOpCodeValue() == TR::BBStart) {
      trfprintf(
        pOutFile, " (BBStart (block_%d))", node->getBlock()->getNumber());
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64Trg1Src1ImmInstruction* instr)
{
  ...
  if (op == TR::InstOpCode::subsimmx || op == TR::InstOpCode::subsimmw) {
    ...
    if (r && r->getRealRegister() &&
      ...
      isAlias = true;
      trfprintf(
        pOutFile, "cmpimm%c \t", (op == TR::InstOpCode::subsimmx) ? 'x' : 'w');
      print(pOutFile, instr->getSource1Register(), TR_WordReg);
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64Trg1Src2Instruction* instr)
{
  ...
  if (op == TR::InstOpCode::subsx || op == TR::InstOpCode::subsw) {
    ...
    if (r && r->getRealRegister() &&
      ...
      isCmp = true;
      trfprintf(
        pOutFile, "cmp%c \t", (op == TR::InstOpCode::subsx) ? 'x' : 'w');
    }
  }
  if (!isCmp) {
    trfprintf(pOutFile, "%s \t", getOpCodeName(&instr->getOpCode()));
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64Trg1Src2ShiftedInstruction* instr)
{
  ...
  trfprintf(pOutFile,
            " %s %d",
            ARM64ShiftCodeNames[instr->getShiftType()],
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64Trg1Src2ExtendedInstruction* instr)
{
  ...
  trfprintf(pOutFile,
            " %s %d",
            ARM64ExtendCodeNames[instr->getExtendType()],
            instr->getShiftAmount());
  trfflush(_comp->getOutFile());
}
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64Trg1MemInstruction* instr)
{
  ...
  if (symbol && symbol->isSpillTempAuto()) {
    trfprintf(
      pOutFile, "\t\t; spilled for %s", getName(instr->getNode()->getOpCode()));
  }
  if (instr->getSnippetForGC() != NULL) {
    trfprintf(pOutFile,
              "\t\t; Backpatched branch to Unresolved Data %s",
...
TR_Debug::printARM64GCRegisterMap(TR::FILE* pOutFile, TR::GCRegisterMap* map)
{
  ...
  for (int i = 0; i < 32; i++) {
    if (map->getMap() & (1 << i))
      trfprintf(pOutFile,
                "%s ",
                getName(machine->getRealRegister(
                  (TR::RealRegister::RegNum)(i + TR::RealRegister::FirstGPR))));
  }
...
void
TR_Debug::printARM64OOLSequences(TR::FILE* pOutFile)
{
  TR_UNIMPLEMENTED();
}
...
bool
TR_Debug::isBranchToTrampoline(TR::SymbolReference* symRef,
                               uint8_t* cursor,
                               int32_t& distance)
{
