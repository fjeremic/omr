 ...
 */
extern TR::Instruction *loadConstant32(TR::CodeGenerator *cg, TR::Node *node,
 ...
 * @param[in] cursor : instruction cursor
 */
extern TR::Instruction *loadConstant64(TR::CodeGenerator *cg, TR::Node *node,
                                       int64_t value, TR::Register *trgReg,
                                       TR::Instruction *cursor = NULL);

extern TR::Instruction *
loadAddressConstant(TR::CodeGenerator *cg, TR::Node *node, intptr_t value,
...
public:
   ...
   */
  const TR::ARM64LinkageProperties &getProperties() {
   ...
   */
  void apply24BitLabelRelativeRelocation(int32_t *cursor,
   ...
   * @param[in] label : label
   */
  void apply32BitLabelRelativeRelocation(int32_t *cursor,
                                         TR::LabelSymbol *label);
  ...
  pickRegister(TR_RegisterCandidate *regCan, TR::Block **barr,
   ...
   */
  bool allowGlobalRegisterAcrossBranch(TR_RegisterCandidate *regCan,
                                       TR::Node *branchNode);
  /**
   ...
   * @return register number for specified index, -1 if index is too large for
   * datat type
   */
  TR_GlobalRegisterNumber
  ...
  TR_GlobalRegisterNumber _gprLinkageGlobalRegisterNumbers
      [TR::RealRegister::NumRegisters]; // could be smaller
  TR_GlobalRegisterNumber _fprLinkageGlobalRegisterNumbers
      [TR::RealRegister::NumRegisters]; // could be smaller
  ...
  TR::list<TR_ARM64OutOfLineCodeSection *> &getARM64OutOfLineCodeSectionList() {
    return _outOfLineCodeSectionList;
  }

  // We need to provide an implementation to avoid an unimplemented assert. See
  // issue #4446.
   ...
   */
  TR::ARM64ImmInstruction *getReturnTypeInfoInstruction() {
   ...
   */
  void setReturnTypeInfoInstruction(TR::ARM64ImmInstruction *rtii) {
