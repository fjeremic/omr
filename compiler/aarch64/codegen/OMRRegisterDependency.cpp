...
void
OMR::ARM64::RegisterDependencyConditions::unionNoRegPostCondition(
  TR::Register* reg,
  TR::CodeGenerator* cg)
{
...
bool
OMR::ARM64::RegisterDependencyConditions::refsRegister(TR::Register* r)
{
...
bool
OMR::ARM64::RegisterDependencyConditions::defsRegister(TR::Register* r)
{
  for (uint16_t i = 0; i < _addCursorForPre; i++) {
    if (_preConditions->getRegisterDependency(i)->getRegister() == r &&
...
bool
OMR::ARM64::RegisterDependencyConditions::defsRealRegister(TR::Register* r)
{
  for (uint16_t i = 0; i < _addCursorForPre; i++) {
...
bool
OMR::ARM64::RegisterDependencyConditions::usesRegister(TR::Register* r)
{
  for (uint16_t i = 0; i < _addCursorForPre; i++) {
    if (_preConditions->getRegisterDependency(i)->getRegister() == r &&
...
void
OMR::ARM64::RegisterDependencyConditions::incRegisterTotalUseCounts(
  TR::CodeGenerator* cg)
{
...
void
TR_ARM64RegisterDependencyGroup::assignRegisters(
  ...
  if (!comp->getOption(TR_DisableOOL)) {
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      // the OOL during backwards RA assignment
        ...
        TR::ARM64LabelInstruction* labelInstr =
          (TR::ARM64LabelInstruction*)currentInstruction;
      ...
      if (realOne == NULL) {
        if (virtReg->getTotalUseCount() == virtReg->getFutureUseCount()) {
          if ((assignedRegister = machine->findBestFreeRegister(
            ...
            assignedRegister =
              machine->freeBestRegister(currentInstruction, virtReg, NULL);
          }
        } else {
          assignedRegister =
            machine->reverseSpillState(currentInstruction, virtReg, NULL);
        }
