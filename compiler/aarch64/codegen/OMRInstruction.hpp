...
public:
  ...
  Instruction(TR::CodeGenerator *cg, TR::Instruction *precedingInstruction,
              TR::InstOpCode::Mnemonic op, TR::Node *node = NULL);
  /**
   * @brief Constructor
   * @param[in] cg : CodeGenerator
   * @param[in] op : opcode
  ...
  virtual bool isLabel() {
    return _opcode.getMnemonic() == TR::InstOpCode::label;
  }
  ...
  /**
   * @brief Answers whether a dependency condition associated with this
   * instruction references the given virtual register
   * @param[in] reg : virtual register
  ...
  /*
   * Maps to TIndex in Instruction. Here we set values specific to ARM64
   * CodeGen.
   *
