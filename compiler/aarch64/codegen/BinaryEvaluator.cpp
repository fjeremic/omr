  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
    ...
    if (constantIsUnsignedImm12(value)) {
      generateTrg1Src1ImmInstruction(
        cg, isAdd ? TR::InstOpCode::addimmw : TR::InstOpCode::subimmw, node, trgReg, src1Reg, value);
    } else {
      TR::Register* tmpReg = cg->allocateRegister();
      loadConstant32(cg, node, value, tmpReg);
      generateTrg1Src2Instruction(
        cg, isAdd ? TR::InstOpCode::addw : TR::InstOpCode::subw, node, trgReg, src1Reg, tmpReg);
      cg->stopUsingRegister(tmpReg);
    }
  } else {
    TR::Register* src2Reg = cg->evaluate(secondChild);
    generateTrg1Src2Instruction(
      cg, isAdd ? TR::InstOpCode::addw : TR::InstOpCode::subw, node, trgReg, src1Reg, src2Reg);
  }
...
static TR::Register* iremHelper(TR::Node* node, bool is64bit, TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src2Instruction(
    cg, is64bit ? TR::InstOpCode::sdivx : TR::InstOpCode::sdivw, node, tmpReg, src1Reg, src2Reg);
 ...
 */
static inline bool logicImmediateHelper(uint64_t value, bool is64Bit, bool& n, uint32_t& immEncoded)
{
