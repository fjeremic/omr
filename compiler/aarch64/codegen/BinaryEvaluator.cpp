...
genericBinaryEvaluator(TR::Node* node,
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (constantIsUnsignedImm12(value) && regOp != regOpImm) {
      generateTrg1Src1ImmInstruction(
        cg, regOpImm, node, trgReg, src1Reg, value);
    } else {
...
static TR::Register*
addOrSubInteger(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (constantIsUnsignedImm12(value)) {
      generateTrg1Src1ImmInstruction(cg,
                                     isAdd ? TR::InstOpCode::addimmw
                                           : TR::InstOpCode::subimmw,
                                     node,
                                     trgReg,
                                     src1Reg,
                                     value);
...
{
  return genericBinaryEvaluator(
    node, TR::InstOpCode::addw, TR::InstOpCode::addimmw, false, cg);
}
...
{
  return genericBinaryEvaluator(
    node, TR::InstOpCode::addx, TR::InstOpCode::addimmx, true, cg);
}
...
{
  return genericBinaryEvaluator(
    node, TR::InstOpCode::subw, TR::InstOpCode::subimmw, false, cg);
}
...
{
  return genericBinaryEvaluator(
    node, TR::InstOpCode::subx, TR::InstOpCode::subimmx, true, cg);
}

// Multiply a register by a 32-bit constant
static void
mulConstant32(TR::Node* node,
              TR::Register* treg,
              TR::Register* sreg,
              int32_t value,
              TR::CodeGenerator* cg)
...
OMR::ARM64::TreeEvaluator::imulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src3Instruction(
    cg, TR::InstOpCode::smaddl, node, trgReg, src1Reg, src2Reg, zeroReg, cond);
  cg->stopUsingRegister(zeroReg);
  /* logical shift right by 32 bits */
  uint32_t imm = 0x183F; // N=1, immr=32, imms=63
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::ubfmx, node, trgReg, trgReg, imm);
...
static TR::Register*
idivHelper(TR::Node* node, bool is64bit, TR::CodeGenerator* cg)
{
...
static TR::Register*
iremHelper(TR::Node* node, bool is64bit, TR::CodeGenerator* cg)
{
  // TODO: Add checks for special cases
...
OMR::ARM64::TreeEvaluator::lmulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::smulh, node, trgReg, src1Reg, src2Reg);
...
{
  return genericBinaryEvaluator(
    node, TR::InstOpCode::sdivw, TR::InstOpCode::sdivw, false, cg);
}
...
{
  return genericBinaryEvaluator(
    node, TR::InstOpCode::sdivx, TR::InstOpCode::sdivx, true, cg);
}
...
static TR::Register*
shiftHelper(TR::Node* node, TR::ARM64ShiftCode shiftType, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (shift != 0) {
      ...
      generateTrg1Src2ShiftedInstruction(
        cg, op, node, trgReg, trgReg, trgReg, TR::SH_LSL, shift);
    }
  } else {
    TR::Register* shiftAmountReg = cg->evaluate(secondChild);
    generateNegInstruction(
      cg, node, shiftAmountReg, shiftAmountReg); // change ROL to ROR
    if (is64bit) {
      // 32->64 bit sign extension: SXTW is alias of SBFM
      uint32_t imm = 0x101F; // N=1, immr=0, imms=31
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::sbfmx, node, shiftAmountReg, shiftAmountReg, imm);
    }
...
inline bool
findLogicImmediateBitPattern(T value,
                             int elementSize,
                             int32_t& rotateCount,
                             int32_t& onesCount)
{
 ...
 */
static inline bool
logicImmediateHelper(uint64_t value,
                     bool is64Bit,
                     bool& n,
                     uint32_t& immEncoded)
{
 ...
 */
static inline TR::Register*
logicBinaryEvaluator(TR::Node* node,
                     TR::InstOpCode::Mnemonic regOp,
                     TR::InstOpCode::Mnemonic regOpImm,
                     bool is64Bit,
                     TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    bool N;
    uint32_t immEncoded;
    if (logicImmediateHelper(
          is64Bit ? value : (uint32_t)value, is64Bit, N, immEncoded)) {
      generateLogicalImmInstruction(
        cg, regOpImm, node, trgReg, src1Reg, N, immEncoded);
    } else {
      src2Reg = cg->allocateRegister();
      if (is64Bit) {
        loadConstant64(cg, node, value, src2Reg);
...
OMR::ARM64::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  return logicBinaryEvaluator(
    node, TR::InstOpCode::andw, TR::InstOpCode::andimmw, false, cg);
}
...
OMR::ARM64::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  // boolean and of 2 integers
  return logicBinaryEvaluator(
    node, TR::InstOpCode::andx, TR::InstOpCode::andimmx, true, cg);
}
...
OMR::ARM64::TreeEvaluator::iorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  return logicBinaryEvaluator(
    node, TR::InstOpCode::orrw, TR::InstOpCode::orrimmw, false, cg);
}
...
OMR::ARM64::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  // boolean or of 2 integers
  return logicBinaryEvaluator(
    node, TR::InstOpCode::orrx, TR::InstOpCode::orrimmx, true, cg);
}
...
OMR::ARM64::TreeEvaluator::ixorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  return logicBinaryEvaluator(
    node, TR::InstOpCode::eorw, TR::InstOpCode::eorimmw, false, cg);
}
...
OMR::ARM64::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  // boolean xor of 2 integers
  return logicBinaryEvaluator(
    node, TR::InstOpCode::eorx, TR::InstOpCode::eorimmx, true, cg);
}
