...
static void loadRelocatableConstant(TR::Node *node, TR::SymbolReference *ref,
  ...
  if (ref->isUnresolved() || comp->compileRelocatableCode()) {
    ...
    if (symbol->isCountForRecompile()) {
      ...
      loadAddressConstant(cg, GCRnode, (intptr_t)ref, reg, NULL, false,
                          TR_ClassAddress);
    } else {
...
OMR::ARM64::MemoryReference::MemoryReference(TR::CodeGenerator *cg)
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _unresolvedSnippet(NULL), _flag(0), _length(0),
      _scale(0) {
  _symbolReference =
...
OMR::ARM64::MemoryReference::MemoryReference(TR::Register *br, TR::Register *ir,
                                             TR::CodeGenerator *cg)
    : _baseRegister(br), _baseNode(NULL), _indexRegister(ir), _indexNode(NULL),
      _unresolvedSnippet(NULL), _flag(0), _length(0), _scale(0) {
  _symbolReference =
      new (cg->trHeapMemory()) TR::SymbolReference(cg->comp()->getSymRefTab());
  _offset = _symbolReference->getOffset();
}

OMR::ARM64::MemoryReference::MemoryReference(TR::Register *br, int32_t disp,
                                             TR::CodeGenerator *cg)
    : _baseRegister(br), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _unresolvedSnippet(NULL), _flag(0), _length(0),
      _scale(0), _offset(disp) {
  _symbolReference =
      new (cg->trHeapMemory()) TR::SymbolReference(cg->comp()->getSymRefTab());
}

OMR::ARM64::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
                                             uint32_t len,
                                             TR::CodeGenerator *cg)
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _unresolvedSnippet(NULL), _flag(0), _length(len),
      _scale(0), _offset(0),
...
OMR::ARM64::MemoryReference::MemoryReference(TR::Node *node,
                                             TR::SymbolReference *symRef,
                                             uint32_t len,
                                             TR::CodeGenerator *cg)
    : _baseRegister(NULL), _baseNode(NULL), _indexRegister(NULL),
      _indexNode(NULL), _unresolvedSnippet(NULL), _flag(0), _length(len),
      _scale(0), _offset(0), _symbolReference(symRef) {
...
void OMR::ARM64::MemoryReference::setSymbol(TR::Symbol *symbol,
                                            TR::CodeGenerator *cg) {
...
void OMR::ARM64::MemoryReference::addToOffset(TR::Node *node, intptrj_t amount,
                                              TR::CodeGenerator *cg) {
  ...
  if (!constantIsImm9(displacement)) {
    ...
    if (_baseRegister != NULL) {
      if (node->getOpCode().isLoadConst() && node->getRegister()) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::addx, node, newBase,
        ...
        if (constantIsUnsignedImm12(displacement)) {
          generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addimmx, node,
                                         newBase, _baseRegister, displacement);
        } else {
...
void OMR::ARM64::MemoryReference::decNodeReferenceCounts(
    TR::CodeGenerator *cg) {
  if (_baseRegister != NULL) {
    if (_baseNode != NULL)
...
void OMR::ARM64::MemoryReference::populateMemoryReference(
    ...
    if (subTree->getOpCodeValue() == TR::aiadd ||
      ...
      if (symbol->isStatic()) {
        if (ref->isUnresolved()) {
          self()->setUnresolvedSnippet(
              new (cg->trHeapMemory()) TR::UnresolvedDataSnippet(
                  cg, subTree, ref, subTree->getOpCode().isStore(), false));
          cg->addSnippet(self()->getUnresolvedSnippet());
        } else {
          _baseRegister = cg->allocateRegister();
          _baseNode = NULL;
...
void OMR::ARM64::MemoryReference::consolidateRegisters(TR::Register *srcReg,
                                                       TR::Node *srcTree,
                                                       bool srcModifiable,
                                                       TR::CodeGenerator *cg) {
  ...
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    if (_indexRegister != NULL) {
      ...
      generateTrg1Src2Instruction(cg, TR::InstOpCode::addx, srcTree,
                                  tempTargetRegister, _baseRegister,
                                  _indexRegister);
      ...
      self()->setBaseModifiable();
               ...
               (self()->getOffset(true) != 0 || self()->hasDelayedOffset())) {
      if (self()->isBaseModifiable())
        tempTargetRegister = _baseRegister;
...
void OMR::ARM64::MemoryReference::incRegisterTotalUseCounts(
    TR::CodeGenerator *cg) {
  if (_baseRegister != NULL) {
...
void OMR::ARM64::MemoryReference::assignRegisters(
    ...
    if (assignedBaseRegister == NULL) {
      if (_baseRegister->getTotalUseCount() ==
        ...
        if ((assignedBaseRegister =
                 machine->findBestFreeRegister(TR_GPR, true)) == NULL) {
          assignedBaseRegister =
              machine->freeBestRegister(currentInstruction, _baseRegister);
        }
      } else {
        assignedBaseRegister =
            machine->reverseSpillState(currentInstruction, _baseRegister);
      }
    ...
    if (assignedIndexRegister == NULL) {
      if (_indexRegister->getTotalUseCount() ==
        ...
        if ((assignedIndexRegister =
                 machine->findBestFreeRegister(TR_GPR, false)) == NULL) {
          assignedIndexRegister =
              machine->freeBestRegister(currentInstruction, _indexRegister);
        }
      } else {
        assignedIndexRegister =
    ...
    if (op.getMnemonic() != TR::InstOpCode::addimmx) {
      ...
      if (index) {
        ...
        if (isImm9OffsetInstruction(enc)) {
          ...
          TR_ASSERT(displacement == 0,
                    "Offset must be zero for specified instruction.");
          cursor += ARM64_INSTRUCTION_LENGTH;
        } else {
...
uint32_t OMR::ARM64::MemoryReference::estimateBinaryLength(TR::InstOpCode op) {
  if (self()->getUnresolvedSnippet() != NULL) {
    ...
    if (op.getMnemonic() != TR::InstOpCode::addimmx) {
      ...
      // addimmx instruction
      TR_ASSERT(self()->getIndexRegister() == NULL,
                "MemoryReference with unexpected indexed form");
