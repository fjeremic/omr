                        ...
                        TR::CodeGenerator* cg)
{
  ...
  uintptr_t addr = symbol->isStatic()
  ...
  if (ref->isUnresolved() || comp->compileRelocatableCode()) {
    ...
    if (symbol->isCountForRecompile()) {
      ...
      loadAddressConstant(
        cg, GCRnode, (intptr_t)ref, reg, NULL, false, TR_ClassAddress);
    } else {
...
OMR::ARM64::MemoryReference::MemoryReference(TR::Register* br,
                                             TR::Register* ir,
                                             TR::CodeGenerator* cg)
  : _baseRegister(br)
  , _baseNode(NULL)
...
void
OMR::ARM64::MemoryReference::setSymbol(TR::Symbol* symbol,
                                       TR::CodeGenerator* cg)
{
...
void
OMR::ARM64::MemoryReference::addToOffset(TR::Node* node,
                                         intptrj_t amount,
                                         TR::CodeGenerator* cg)
{
      ...
      if (node->getOpCode().isLoadConst() && node->getRegister()) {
        ...
        if (constantIsUnsignedImm12(displacement)) {
          ...
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::addx, node, newBase, _baseRegister, tempReg);
...
void
OMR::ARM64::MemoryReference::decNodeReferenceCounts(TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
    if (_baseNode != NULL)
...
void
OMR::ARM64::MemoryReference::populateMemoryReference(TR::Node* subTree,
                                                     TR::CodeGenerator* cg)
{
...
void
OMR::ARM64::MemoryReference::consolidateRegisters(TR::Register* srcReg,
                                                  TR::Node* srcTree,
                                                  bool srcModifiable,
                                                  TR::CodeGenerator* cg)
{
...
void
OMR::ARM64::MemoryReference::incRegisterTotalUseCounts(TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
...
void
OMR::ARM64::MemoryReference::assignRegisters(
...
/* register offset */
static bool
isRegisterOffsetInstruction(uint32_t enc)
{
...
static bool
isImm9OffsetInstruction(uint32_t enc)
{
...
static bool
isImm12OffsetInstruction(uint32_t enc)
{
...
static bool
isExclusiveMemAccessInstruction(TR::InstOpCode::Mnemonic op)
{
...
uint8_t*
OMR::ARM64::MemoryReference::generateBinaryEncoding(
  TR::Instruction* currentInstruction,
  uint8_t* cursor,
  TR::CodeGenerator* cg)
{
...
uint32_t
OMR::ARM64::MemoryReference::estimateBinaryLength(TR::InstOpCode op)
{
  if (self()->getUnresolvedSnippet() != NULL) {
    TR_UNIMPLEMENTED();
  } else {
