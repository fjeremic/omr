...
class OMR_EXTENSIBLE MemoryReference : public OMR::MemoryReference {
 ...
 public:
  ...
  bool refsRegister(TR::Register *reg) {
    return (reg == _baseRegister || reg == _indexRegister);
  }
  ...
  void setBaseModifiable() {
    _flag |= TR_ARM64MemoryReferenceControl_Base_Modifiable;
  }
  /**
  ...
  void setIndexModifiable() {
    _flag |= TR_ARM64MemoryReferenceControl_Index_Modifiable;
  }
  /**
   ...
   * @param[in] cg : CodeGenerator
   */
  void consolidateRegisters(TR::Register *srcReg, TR::Node *srcTree,
                            bool srcModifiable, TR::CodeGenerator *cg);
   ...
   * @param[in] op : opcode of the instruction to attach this memory reference
   * to
   ...
   */
  uint8_t *generateBinaryEncoding(TR::Instruction *ci, uint8_t *cursor,
                                  TR::CodeGenerator *cg);
};
