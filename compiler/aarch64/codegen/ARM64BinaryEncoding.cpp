...
uint8_t *TR::ARM64ImmSymInstruction::generateBinaryEncoding() {
  ...
  if (getOpCodeValue() == TR::InstOpCode::bl) {
    ...
    if (cg()->comp()->isRecursiveMethodTarget(sym)) {
      ...
      TR_ASSERT_FATAL(jitToJitStart, "Unknown compiled method entry point.  "
                                     "Entry point should be available by now.");
...
int32_t TR::ARM64ConditionalBranchInstruction::estimateBinaryLength(
...
int32_t TR::ARM64VirtualGuardNOPInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
