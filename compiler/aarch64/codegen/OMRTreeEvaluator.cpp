...
TR::Instruction*
loadConstant32(TR::CodeGenerator* cg,
               TR::Node* node,
               int32_t value,
               TR::Register* trgReg,
               TR::Instruction* cursor)
{
  ...
  if (op != TR::InstOpCode::bad) {
    ...
    cursor = generateTrg1ImmInstruction(
      cg, TR::InstOpCode::movzw, node, trgReg, (value & 0xFFFF), cursor);
    cursor =
      generateTrg1ImmInstruction(cg,
                                 TR::InstOpCode::movkw,
                                 node,
                                 trgReg,
                                 (((value >> 16) & 0xFFFF) | TR::MOV_LSL16),
                                 cursor);
  }
...
TR::Instruction*
loadConstant64(TR::CodeGenerator* cg,
               TR::Node* node,
               int64_t value,
               TR::Register* trgReg,
               TR::Instruction* cursor)
{
  ...
  if (value == 0LL) {
    // 0
    cursor = generateTrg1ImmInstruction(
      cg, TR::InstOpCode::movzx, node, trgReg, 0, cursor);
  } else if (~value == 0LL) {
    // -1
    cursor = generateTrg1ImmInstruction(
      cg, TR::InstOpCode::movnx, node, trgReg, 0, cursor);
  } else {
    uint16_t h[4];
 ...
 */
static void
addMetaDataForLoadAddressConstantFixed(TR::CodeGenerator* cg,
                                       TR::Node* node,
  ...
  switch (typeAddress) {
    ...
    case TR_DebugCounter: {
      ...
      TR::DebugCounter::generateRelocation(
        comp, firstInstruction, node, counter);
      return;
    }
...
loadAddressConstantRelocatable(TR::CodeGenerator* cg,
                               TR::Node* node,
                               intptrj_t value,
                               TR::Register* trgReg,
                               ...
                               int16_t typeAddress = -1)
{
  ...
  cursor =
    generateTrg1ImmInstruction(cg,
                               ...
                               node,
                               trgReg,
                               ((value >> 16) & 0x0000ffff) | TR::MOV_LSL16,
                               cursor);
  ...
  cursor = generateTrg1ImmInstruction(cg,
                                      ...
                                      node,
                                      trgReg,
                                      (value >> 48) | (TR::MOV_LSL16 * 3),
                                      cursor);

  addMetaDataForLoadAddressConstantFixed(
    cg, node, firstInstruction, typeAddress, value);
                    ...
                    int16_t typeAddress)
{
  if (cg->comp()->compileRelocatableCode()) {
    return loadAddressConstantRelocatable(
      cg, node, value, trgReg, cursor, typeAddress);
  }
                    ...
                    TR::CodeGenerator* cg)
{
  ...
  if (needSync) {
    generateSynchronizationInstruction(
      cg, TR::InstOpCode::dmb, node, 0xF); // dmb SY
  }
...
OMR::ARM64::TreeEvaluator::aloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::ldrimmx, node, tempReg, tempMR);
                     ...
                     TR::CodeGenerator* cg)
{
  ...
  if (needSync) {
    generateSynchronizationInstruction(
      cg, TR::InstOpCode::dmb, node, 0xE); // dmb ST
  }
...
TR::Register*
OMR::ARM64::TreeEvaluator::arraytranslateAndTestEvaluator(TR::Node* node,
                                                          TR::CodeGenerator* cg)
{
...
TR::Instruction*
OMR::ARM64::TreeEvaluator::generateVFTMaskInstruction(TR::CodeGenerator* cg,
                                                      TR::Node* node,
                                                      TR::Register* dstReg,
                                                      TR::Register* srcReg,
