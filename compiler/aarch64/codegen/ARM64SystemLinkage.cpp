...
static void
mapSingleParameter(TR::ParameterSymbol* parameter,
                   uint32_t& stackIndex,
                   bool inCalleeFrame)
{
  if (inCalleeFrame) {
...
void
TR::ARM64SystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p,
                                           uint32_t& stackIndex)
{
                                       ...
                                       List<TR::ParameterSymbol>& parmList)
{
  ...
  if (machine->getLinkRegisterKilled()) {
    ...
    cursor = generateMemSrc1Instruction(
      ...
      firstNode,
      stackSlot,
      machine->getRealRegister(TR::RealRegister::x30),
      cursor);
  }
        ...
        if (nextIntArgReg < getProperties().getNumIntArgRegs()) {
          ...
          cursor = generateMemSrc1Instruction(
            ...
            machine->getRealRegister(
              (TR::RealRegister::RegNum)(TR::RealRegister::x0 + nextIntArgReg)),
            cursor);
          nextIntArgReg++;
        } else {
          nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
      ...
      case TR::Double:
        if (nextFltArgReg < getProperties().getNumFloatArgRegs()) {
          ...
          cursor = generateMemSrc1Instruction(
            ...
            stackSlot,
            machine->getRealRegister(
              (TR::RealRegister::RegNum)(TR::RealRegister::v0 + nextFltArgReg)),
            cursor);
          nextFltArgReg++;
        } else {
          nextFltArgReg = getProperties().getNumFloatArgRegs() + 1;
  ...
  for (int r = TR::RealRegister::x19; r <= TR::RealRegister::x28; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      ...
      cursor = generateMemSrc1Instruction(
        cg(), TR::InstOpCode::strimmx, firstNode, stackSlot, rr, cursor);
      offset += 8;
    }
  }
  for (int r = TR::RealRegister::v8; r <= TR::RealRegister::v15; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      TR::MemoryReference* stackSlot =
        new (trHeapMemory()) TR::MemoryReference(sp, offset, codeGen);
      cursor = generateMemSrc1Instruction(
        cg(), TR::InstOpCode::vstrimmd, firstNode, stackSlot, rr, cursor);
      offset += 8;
    }
  }
}
...
TR::ARM64SystemLinkage::createEpilogue(TR::Instruction* cursor)
{
  ...
  for (int r = TR::RealRegister::x19; r <= TR::RealRegister::x28; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      ...
      cursor = generateTrg1MemInstruction(
        cg(), TR::InstOpCode::ldrimmx, lastNode, rr, stackSlot, cursor);
      offset += 8;
    }
  }
  for (int r = TR::RealRegister::v8; r <= TR::RealRegister::v15; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      TR::MemoryReference* stackSlot =
        new (trHeapMemory()) TR::MemoryReference(sp, offset, codeGen);
      cursor = generateTrg1MemInstruction(
        cg(), TR::InstOpCode::vldrimmd, lastNode, rr, stackSlot, cursor);
      offset += 8;
    }
  }
  ...
  if (machine->getLinkRegisterKilled()) {
    TR::MemoryReference* stackSlot =
      new (trHeapMemory()) TR::MemoryReference(sp, 0, codeGen);
    cursor = generateTrg1MemInstruction(
      cg(), TR::InstOpCode::ldrimmx, lastNode, lr, stackSlot, cursor);
  }
  ...
  // return
  cursor = generateRegBranchInstruction(
    codeGen, TR::InstOpCode::ret, lastNode, lr, cursor);
}

int32_t
TR::ARM64SystemLinkage::buildArgs(
  TR::Node* callNode,
...

{
  ...
  for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
    ...
    switch (childType) {
      ...
      default:
        TR_ASSERT(
          false, "Argument type %s is not supported\n", childType.toString());
    }
  }
      ...
      case TR::Address:
        ...
        if (numIntegerArgs < properties.getNumIntArgRegs()) {
          ...
          mref = getOutgoingArgumentMemRef(
            argMemReg, argRegister, op, pushToMemory[argIndex++]);
        ...
        if (numFloatArgs < properties.getNumFloatArgRegs()) {
          if (!cg()->canClobberNodesRegister(child, 0)) {
            ...
            generateTrg1Src1Instruction(
              cg(), op, callNode, tempReg, argRegister);
            argRegister = tempReg;
          }
  ...
  if (numMemArgs > 0) {
    ...
    generateTrg1Src1ImmInstruction(
      cg(), TR::InstOpCode::subimmx, callNode, argMemReg, sp, totalSize);
...
TR::Register*
TR::ARM64SystemLinkage::buildDirectDispatch(TR::Node* callNode)
{
  ...
  if (totalSize > 0) {
    if (constantIsUnsignedImm12(totalSize)) {
      generateTrg1Src1ImmInstruction(
        cg(), TR::InstOpCode::subimmx, callNode, sp, sp, totalSize);
    } else {
  ...
  if (totalSize > 0) {
    if (constantIsUnsignedImm12(totalSize)) {
      generateTrg1Src1ImmInstruction(
        cg(), TR::InstOpCode::addimmx, callNode, sp, sp, totalSize);
    } else {
      TR_ASSERT_FATAL(false, "Too many arguments.");
    }
  }
...
intptrj_t
TR::ARM64SystemLinkage::entryPointFromCompiledMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}

intptrj_t
TR::ARM64SystemLinkage::entryPointFromInterpretedMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}
