...
TR::ARM64SystemLinkage::ARM64SystemLinkage(TR::CodeGenerator *cg)
  ...
  _properties._registerFlags[TR::RealRegister::x0] =
  ...
  _properties._registerFlags[TR::RealRegister::v0] =
  ...
  _properties._preservedRegisterMapForGC =
      0x00000000;  // ToDo: Determine the value
...
const TR::ARM64LinkageProperties &TR::ARM64SystemLinkage::getProperties() {
  return _properties;
}
...
void TR::ARM64SystemLinkage::initARM64RealRegisterLinkage() {
  ...
  for (icount = TR::RealRegister::x18; icount <= TR::RealRegister::x28;
       icount++)
    machine->getRealRegister((TR::RealRegister::RegNum)icount)
        ->setWeight(0xf000);
...
uint32_t TR::ARM64SystemLinkage::getRightToLeft() {
  return getProperties().getRightToLeft();
}

static void mapSingleParameter(TR::ParameterSymbol *parameter,
                               uint32_t &stackIndex, bool inCalleeFrame) {
  if (inCalleeFrame) {
...
void TR::ARM64SystemLinkage::mapStack(TR::ResolvedMethodSymbol *method) {
  ...
  for (int r = TR::RealRegister::x19; r <= TR::RealRegister::x28; r++) {
    TR::RealRegister *rr =
        machine->getRealRegister((TR::RealRegister::RegNum)r);
  ...
  for (TR::ParameterSymbol *parameter = parameterIterator.getFirst();
    ...
    switch (parameter->getDataType()) {
      ...
      case TR::Aggregate:
        TR_ASSERT(false,
                  "Function parameters of aggregate types are not currently "
...
void TR::ARM64SystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                                uint32_t &stackIndex) {
  int32_t roundedSize = (p->getSize() + 3) & (~3);
  if (roundedSize == 0) roundedSize = 4;
...
void TR::ARM64SystemLinkage::createPrologue(
    TR::Instruction *cursor, List<TR::ParameterSymbol> &parmList) {
  TR::CodeGenerator *codeGen = cg();
  TR::Machine *machine = codeGen->machine();
  TR::ResolvedMethodSymbol *bodySymbol = comp()->getJittedMethodSymbol();
  const TR::ARM64LinkageProperties &properties = getProperties();
  TR::RealRegister *sp =
      machine->getRealRegister(properties.getStackPointerRegister());
  ...
  if (constantIsUnsignedImm12(frameSize)) {
    cursor = generateTrg1Src1ImmInstruction(
        codeGen, TR::InstOpCode::subimmx, firstNode, sp, sp, frameSize, cursor);
  } else {
    TR_UNIMPLEMENTED();
  }
  ...
  if (machine->getLinkRegisterKilled()) {
    TR::MemoryReference *stackSlot =
        new (trHeapMemory()) TR::MemoryReference(sp, 0, codeGen);
    cursor = generateMemSrc1Instruction(
        cg(), TR::InstOpCode::strimmx, firstNode, stackSlot,
  ...
  for (TR::ParameterSymbol *parameter = parameterIterator.getFirst();
       ...
       (nextIntArgReg < getProperties().getNumIntArgRegs() ||
        nextFltArgReg < getProperties().getNumFloatArgRegs());
       parameter = parameterIterator.getNext()) {
    TR::MemoryReference *stackSlot = new (trHeapMemory())
        TR::MemoryReference(sp, parameter->getParameterOffset(), codeGen);
    ...
    switch (parameter->getDataType()) {
      ...
      case TR::Address:
        if (nextIntArgReg < getProperties().getNumIntArgRegs()) {
          op = (parameter->getSize() == 8) ? TR::InstOpCode::strimmx
                                           : TR::InstOpCode::strimmw;
          cursor = generateMemSrc1Instruction(
              cg(), op, firstNode, stackSlot,
              machine->getRealRegister((TR::RealRegister::RegNum)(
                  TR::RealRegister::x0 + nextIntArgReg)),
              cursor);
          nextIntArgReg++;
        } else {
          nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
      ...
      case TR::Double:
        if (nextFltArgReg < getProperties().getNumFloatArgRegs()) {
          op = (parameter->getSize() == 8) ? TR::InstOpCode::vstrimmd
                                           : TR::InstOpCode::vstrimms;
          cursor = generateMemSrc1Instruction(
              cg(), op, firstNode, stackSlot,
              machine->getRealRegister((TR::RealRegister::RegNum)(
                  TR::RealRegister::v0 + nextFltArgReg)),
              cursor);
          nextFltArgReg++;
        } else {
          nextFltArgReg = getProperties().getNumFloatArgRegs() + 1;
  ...
  for (int r = TR::RealRegister::x19; r <= TR::RealRegister::x28; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      TR::MemoryReference *stackSlot =
          new (trHeapMemory()) TR::MemoryReference(sp, offset, codeGen);
      cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::strimmx,
                                          firstNode, stackSlot, rr, cursor);
      offset += 8;
    }
  }
  for (int r = TR::RealRegister::v8; r <= TR::RealRegister::v15; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      TR::MemoryReference *stackSlot =
          new (trHeapMemory()) TR::MemoryReference(sp, offset, codeGen);
      cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::vstrimmd,
                                          firstNode, stackSlot, rr, cursor);
      offset += 8;
    }
  }
...
void TR::ARM64SystemLinkage::createEpilogue(TR::Instruction *cursor) {
  ...
  for (int r = TR::RealRegister::x19; r <= TR::RealRegister::x28; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      TR::MemoryReference *stackSlot =
          new (trHeapMemory()) TR::MemoryReference(sp, offset, codeGen);
      cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::ldrimmx,
                                          lastNode, rr, stackSlot, cursor);
      offset += 8;
    }
  }
  for (int r = TR::RealRegister::v8; r <= TR::RealRegister::v15; r++) {
    ...
    if (rr->getHasBeenAssignedInMethod()) {
      TR::MemoryReference *stackSlot =
          new (trHeapMemory()) TR::MemoryReference(sp, offset, codeGen);
      cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::vldrimmd,
                                          lastNode, rr, stackSlot, cursor);
      offset += 8;
    }
  }
  ...
  if (machine->getLinkRegisterKilled()) {
    TR::MemoryReference *stackSlot =
        new (trHeapMemory()) TR::MemoryReference(sp, 0, codeGen);
    cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::ldrimmx, lastNode,
                                        lr, stackSlot, cursor);
  }
  ...
  if (constantIsUnsignedImm12(frameSize)) {
    cursor = generateTrg1Src1ImmInstruction(
        codeGen, TR::InstOpCode::addimmx, lastNode, sp, sp, frameSize, cursor);
  } else {
    TR_UNIMPLEMENTED();
  }

  // return
  cursor = generateRegBranchInstruction(codeGen, TR::InstOpCode::ret, lastNode,
                                        lr, cursor);
}

int32_t TR::ARM64SystemLinkage::buildArgs(
...

{
  ...
  for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
    ...
    switch (childType) {
      ...
      case TR::Int64:
      case TR::Address:
        if (numIntegerArgs >= properties.getNumIntArgRegs()) numMemArgs++;
        numIntegerArgs++;
        break;

      case TR::Float:
      case TR::Double:
        if (numFloatArgs >= properties.getNumFloatArgRegs()) numMemArgs++;
        numFloatArgs++;
        break;
    ...
    switch (childType) {
      ...
      case TR::Address:
        ...
        if (numIntegerArgs < properties.getNumIntArgRegs()) {
          ...
          mref = getOutgoingArgumentMemRef(argMemReg, argRegister, op,
      ...
      case TR::Double:
        ...
        if (numFloatArgs < properties.getNumFloatArgRegs()) {
          if (!cg()->canClobberNodesRegister(child, 0)) {
            ...
            op = (childType == TR::Float) ? TR::InstOpCode::fmovs
                                          : TR::InstOpCode::fmovd;
            generateTrg1Src1Instruction(cg(), op, callNode, tempReg,
                                        argRegister);
            argRegister = tempReg;
          }
  ...
  int32_t floatRegsUsed = (numFloatArgs > properties.getNumFloatArgRegs())
                              ...
                              : numFloatArgs;
  for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::v0 +
                                      floatRegsUsed);
  ...
  if (numMemArgs > 0) {
    TR::RealRegister *sp =
        cg()->machine()->getRealRegister(properties.getStackPointerRegister());
    generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::subimmx, callNode,
                                   argMemReg, sp, totalSize);
...
TR::Register *TR::ARM64SystemLinkage::buildDirectDispatch(TR::Node *callNode) {
  ...
  TR::RealRegister *sp =
      cg()->machine()->getRealRegister(pp.getStackPointerRegister());
  ...
  generateImmSymInstruction(
      ...
      (uintptr_t)callSymbol->getMethodAddress(), dependencies,
  ...
  if (totalSize > 0) {
    if (constantIsUnsignedImm12(totalSize)) {
      generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addimmx, callNode,
                                     sp, sp, totalSize);
    } else {
      TR_ASSERT_FATAL(false, "Too many arguments.");
    }
...
TR::Register *TR::ARM64SystemLinkage::buildIndirectDispatch(
    TR::Node *callNode) {
  TR_UNIMPLEMENTED();
  return NULL;
}
