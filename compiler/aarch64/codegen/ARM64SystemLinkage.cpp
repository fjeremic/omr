...
void TR::ARM64SystemLinkage::createPrologue(TR::Instruction* cursor, List<TR::ParameterSymbol>& parmList)
{
  ...
  if (machine->getLinkRegisterKilled()) {
    ...
    cursor = generateMemSrc1Instruction(
      cg(), TR::InstOpCode::strimmx, firstNode, stackSlot, machine->getRealRegister(TR::RealRegister::x30), cursor);
  }
  ...
  for (TR::ParameterSymbol* parameter = parameterIterator.getFirst(); parameter != NULL
    ...
    TR::MemoryReference* stackSlot
      = new (trHeapMemory()) TR::MemoryReference(sp, parameter->getParameterOffset(), codeGen);
    ...
    case TR::Address:
      if (nextIntArgReg < getProperties().getNumIntArgRegs()) {
        ...
        cursor = generateMemSrc1Instruction(cg(), op, firstNode, stackSlot,
          machine->getRealRegister((TR::RealRegister::RegNum)(TR::RealRegister::x0 + nextIntArgReg)), cursor);
        nextIntArgReg++;
      } else {
        nextIntArgReg = getProperties().getNumIntArgRegs() + 1;
      }
    ...
    case TR::Double:
      if (nextFltArgReg < getProperties().getNumFloatArgRegs()) {
        ...
        cursor = generateMemSrc1Instruction(cg(), op, firstNode, stackSlot,
          machine->getRealRegister((TR::RealRegister::RegNum)(TR::RealRegister::v0 + nextFltArgReg)), cursor);
        nextFltArgReg++;
      } else {
        nextFltArgReg = getProperties().getNumFloatArgRegs() + 1;
      }
...
TR::Register* TR::ARM64SystemLinkage::buildDirectDispatch(TR::Node* callNode)
{
  ...
  generateImmSymInstruction(cg(), TR::InstOpCode::bl, callNode, (uintptr_t)callSymbol->getMethodAddress(), dependencies,
