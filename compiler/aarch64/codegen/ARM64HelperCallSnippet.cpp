...
TR::ARM64HelperCallSnippet::emitSnippetBody()
{
  ...
  intptr_t distance =
    (intptr_t)(
      getDestination()->getSymbol()->castToMethodSymbol()->getMethodAddress()) -
    (intptr_t)cursor;
  ...
  *(int32_t*)cursor =
    TR::InstOpCode::getOpCodeBinaryEncoding(TR::InstOpCode::bl) |
    ((distance >> 2) & 0x3ffffff); // imm26

  cg()->addExternalRelocation(
    new (cg()->trHeapMemory()) TR::ExternalRelocation(
      cursor, (uint8_t*)getDestination(), TR_HelperAddress, cg()),
    __FILE__,
    __LINE__,
    getNode());
  ...
  if (_restartLabel != NULL) {
    ...
    if (constantIsSignedImm28(distance)) {
      // b distance
      *(int32_t*)cursor =
        TR::InstOpCode::getOpCodeBinaryEncoding(TR::InstOpCode::b) |
        ((distance >> 2) & 0x3ffffff); // imm26
      cursor += ARM64_INSTRUCTION_LENGTH;
    } else {
...
TR_Debug::print(TR::FILE* pOutFile, TR::ARM64HelperCallSnippet* snippet)
{
  ...
  char* info = "";
  intptr_t target = (intptr_t)(snippet->getDestination()
                                 ->getSymbol()
                                 ->castToMethodSymbol()
                                 ->getMethodAddress());
  int32_t distance;
  ...
  if (restartLabel != NULL) {
    ...
    if (TR::Compiler->target.cpu
          .isTargetWithinUnconditionalBranchImmediateRange(
            (intptrj_t)restartLocation, (intptrj_t)bufferPos)) {
      printPrefix(pOutFile, NULL, bufferPos, 4);
      trfprintf(pOutFile,
                "b \t" POINTER_PRINTF_FORMAT "\t\t; Back to ",
                restartLocation);
