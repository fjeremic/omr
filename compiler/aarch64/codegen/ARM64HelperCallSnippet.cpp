...
uint8_t* TR::ARM64HelperCallSnippet::emitSnippetBody()
{
  ...
  if (!constantIsSignedImm28(distance)) {
    distance
      = TR::CodeCacheManager::instance()->findHelperTrampoline(getDestination()->getReferenceNumber(), (void*)cursor)
      - (intptr_t)cursor;
  ...
  *(int32_t*)cursor
  ...
  cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                TR::ExternalRelocation(cursor, (uint8_t*)getDestination(), TR_HelperAddress, cg()),
...
void TR_Debug::print(TR::FILE* pOutFile, TR::ARM64HelperCallSnippet* snippet)
{
  ...
  if (restartLabel != NULL) {
    ...
    if (TR::Compiler->target.cpu.isTargetWithinUnconditionalBranchImmediateRange(
