...
public:
   ...
   *        with a relative unconditional branch with immediate (B or BL)
   * instruction.
   *
   ...
   *        is within the reachable displacement range for a relative
   * unconditional branch with immediate (B or BL) instruction.
   *
   * @param[in] : targetAddress : the address of the target
   *
   * @param[in] : sourceAddress : the address of the relative unconditional
   * branch with immediate (B or BL) instruction from which the displacement
   * range is measured.
   *
   * @return true if the target is within range; false otherwise.
   */
  bool isTargetWithinUnconditionalBranchImmediateRange(intptrj_t targetAddress,
