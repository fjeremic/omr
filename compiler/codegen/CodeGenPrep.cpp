  ...
  if (node->getOpCodeValue() == TR::ldiv && self()->codegenSupportsUnsignedIntegerDivide()) {
    ...
    if (firstChildOp == TR::iu2l && secondChildOp == TR::iu2l
      && performTransformation(
        self()->comp(), "%sReduced ldiv with highWordZero children in node [%p] to idiv\n", OPT_DETAILS, node)) {
  ...
  if (self()->getLocalsIG() && self()->getSupportsCompactedLocals()) {
    ...
    if (node->getOpCode().isStoreDirect() && node->getSymbolReference()->getSymbol()->isAuto()
      ...
      if (!storeLocal->holdsMonitoredObject() && !storeLocal->isInternalPointer()
        && !storeLocal->isPinningArrayPointer() && !loadLocal->holdsMonitoredObject() && !loadLocal->isInternalPointer()
  ...
  TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount, TR::DataType storeType)
{
  ...
  if (opCode.isConversion()
    || (opCode.isAnd() && parent->getSecondChild()->getOpCode().isLoadConst()
      && parent->getSecondChild()->getType().isIntegral() && parent->getSecondChild()->get64bitIntegralValue() >= 0
      ...
      if (child->getVisitCount() != visitCount) {
        ...
        if ((isOneByte && (childOp == TR::i2b || childOp == TR::b2i || childOp == TR::bu2i))
          ...
          if (child->getReferenceCount() > 1
            && (!treeTop->getNode()->getOpCode().isBooleanCompare() || (childOp != TR::bu2i && childOp != TR::su2i))
    ...
    if (node->getOpCode().isNew()) {
      ...
      if (node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::anewarray) {
        ...
        if (classChild->getOpCodeValue() != TR::loadaddr) {
          ...
          int32_t length;
          char* className = TR::Compiler->cls.classNameChars(self()->comp(),
            (TR_OpaqueClassBlock*)classChild->getSymbol()->castToStaticSymbol()->getStaticAddress(), length);
          TR::DebugCounter::prependDebugCounter(self()->comp(),
            TR::DebugCounter::debugCounterName(self()->comp(), "allocations/%s/(%.*s)", opName, length, className), tt);
        }
      } else {
        // newarray
