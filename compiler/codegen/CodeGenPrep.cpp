...
void
OMR::CodeGenerator::lowerTreesPreChildrenVisit(TR::Node* parent,
                                               TR::TreeTop* treeTop,
                                               vcount_t visitCount)
{
  ...
  if (!disableILMulPwr2Opt &&
      ...
      performTransformation(
        self()->comp(), "%sPwr of 2 mult opt node %p\n", OPT_DETAILS, parent)) {
    ...
    if (secondChild->getOpCode().isLoadConst()) {
      ...
      int32_t shftAmnt = -1;
      if ((shftAmnt =
             TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(value)) > 0) {
        if (value > 0) {
...
void
OMR::CodeGenerator::lowerTreeIfNeeded(TR::Node* node,
  ...
  if (node->getOpCodeValue() == TR::ldiv &&
    ...
    if (firstChildOp == TR::iu2l && secondChildOp == TR::iu2l &&
        performTransformation(
          ...
          OPT_DETAILS,
          node)) {
      TR::Node* idivChild = TR::Node::create(
        ...
        if ((storeIGNode != NULL) && (loadIGNode != NULL)) {
          ...
          if (colourLoad == colourStore && colourLoad != UNCOLOURED &&
            ...
            if (!liveButMaybeUnreferencedLocals) {
              int32_t numLocals =
                self()->comp()->getMethodSymbol()->getAutomaticList().getSize();
 ...
 */
void
OMR::CodeGenerator::identifyUnneededByteConvNodes(TR::Node* parent,
                                                  TR::TreeTop* treeTop,
                                                  vcount_t visitCount,
  ...
  if (opCode.isConversion() ||
    ...
    for (int32_t childCount = parent->getNumChildren() - 1; childCount >= 0;
      ...
      if (child->getVisitCount() != visitCount) {
        ...
        if (child->getReferenceCount() == 1)
          self()->identifyUnneededByteConvNodes(
            child, treeTop, visitCount, storeType);
      }
    }
  }
}
...
OMR::CodeGenerator::identifyUnneededByteConvNodes()
{
  ...
  for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    if ((
      ...
      self()->identifyUnneededByteConvNodes(
        node, tt, visitCount, node->getDataType());
    }
...
bool
shouldResetRequiresConditionCodes(TR::Node* node)
{
...
void
OMR::CodeGenerator::rematerializeCmpUnderTernary(TR::Node* node)
{
