...
void OMR::CodeGenerator::lowerTreesPreChildrenVisit(TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount)
{
    ...
    if (!disableILMulPwr2Opt && ((parent->getOpCodeValue() == TR::imul) || (parent->getOpCodeValue() == TR::lmul)) && performTransformation(self()->comp(), "%sPwr of 2 mult opt node %p\n", OPT_DETAILS, parent)) {
...
void OMR::CodeGenerator::lowerTreeIfNeeded(
    TR::Node* node,
    ...
    TR::TreeTop* tt)
{
    ...
    if (node->getOpCodeValue() == TR::ldiv && self()->codegenSupportsUnsignedIntegerDivide()) {
        ...
        if (firstChildOp == TR::iu2l && secondChildOp == TR::iu2l && performTransformation(self()->comp(), "%sReduced ldiv with highWordZero children in node [%p] to idiv\n", OPT_DETAILS, node)) {
    ...
    if (self()->getLocalsIG() && self()->getSupportsCompactedLocals()) {
        ...
        //  load auto2 (colour x)
        if (node->getOpCode().isStoreDirect() && node->getSymbolReference()->getSymbol()->isAuto() && node->getFirstChild()->getReferenceCount() == 1 && node->getFirstChild()->getOpCode().isLoadVarDirect() && node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto()) {
            ...
            if (!storeLocal->holdsMonitoredObject() && !storeLocal->isInternalPointer() && !storeLocal->isPinningArrayPointer() && !loadLocal->holdsMonitoredObject() && !loadLocal->isInternalPointer() && !loadLocal->isPinningArrayPointer()) {
                ...
                if ((storeIGNode != NULL) && (loadIGNode != NULL)) {
                    ...
                    if (colourLoad == colourStore && colourLoad != UNCOLOURED && performTransformation(self()->comp(), "%sCoalescing locals by removing store tree %p, load = %d, store = %d \n", OPT_DETAILS, node, colourLoad, colourStore)) {
...
void OMR::CodeGenerator::identifyUnneededByteConvNodes(TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount, TR::DataType storeType)
{
    ...
    // if the b2i is noped.
    if (opCode.isConversion() || (opCode.isAnd() && parent->getSecondChild()->getOpCode().isLoadConst() && parent->getSecondChild()->getType().isIntegral() && parent->getSecondChild()->get64bitIntegralValue() >= 0 && parent->getSecondChild()->get64bitIntegralValue() < 128)
        ...
        for (int32_t childCount = parent->getNumChildren() - 1; childCount >= 0; childCount--) {
            ...
            if (child->getVisitCount() != visitCount) {
                ...
                bool isTwoByte = (storeType == TR::Int16
                    ...
                    && child->getOpCode().isConversion();
                if ((isOneByte && (childOp == TR::i2b || childOp == TR::b2i || childOp == TR::bu2i)) || (isTwoByte && (childOp == TR::i2s || childOp == TR::s2i || childOp == TR::su2i))) {
                    if (child->getReferenceCount() > 1 && (!treeTop->getNode()->getOpCode().isBooleanCompare() || (childOp != TR::bu2i && childOp != TR::su2i)) && performTransformation(self()->comp(), "%sReplacing shared i2b/b2i node %p\n", OPT_DETAILS, child)) {
                        TR::Node* newChild = TR::Node::create(childOp, 1, child->getFirstChild());
                        child->decReferenceCount();
                        parent->setAndIncChild(childCount, newChild);
                        child = newChild;
                    }
                    if ((childOp == TR::b2i || childOp == TR::s2i) && treeTop->getNode()->getOpCode().isBooleanCompare() && (child->getFirstChild()->getOpCodeValue() == TR::s2b || child->getFirstChild()->getOpCodeValue() == TR::f2b || child->getFirstChild()->getOpCodeValue() == TR::d2b || child->getFirstChild()->getOpCodeValue() == TR::i2b || child->getFirstChild()->getOpCodeValue() == TR::l2b || child->getFirstChild()->getOpCodeValue() == TR::f2s || child->getFirstChild()->getOpCodeValue() == TR::d2s || child->getFirstChild()->getOpCodeValue() == TR::i2s || child->getFirstChild()->getOpCodeValue() == TR::l2s || child->getFirstChild()->getOpCodeValue() == TR::iRegLoad || child->getFirstChild()->getOpCodeValue() == TR::iuRegLoad) && performTransformation(self()->comp(), "%sChanging b2i node %p to unsigned conversion\n", OPT_DETAILS, child)) {
...
void OMR::CodeGenerator::identifyUnneededByteConvNodes()
{
    ...
    for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        else if (node->getOpCode().isBooleanCompare() && (TR::ILOpCode::isEqualCmp(node->getOpCode().getOpCodeValue()) || TR::ILOpCode::isNotEqualCmp(node->getOpCode().getOpCodeValue())) && node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getType().isIntegral()) {
...
void OMR::CodeGenerator::insertDebugCounters()
{
    ...
    for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
            ...
            if (block->getPredecessors().size() == 1) {
                ...
                if (predBlock->getEntry() && predBlock->getExit()) {
                    ...
                    if (predNode->getOpCode().isSwitch()) {
                        ...
                        TR::DebugCounter::prependDebugCounter(self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "branchtargets/%s/(%s)/%s/%d/%d", opName, self()->comp()->signature(), self()->comp()->getHotnessName(self()->comp()->getMethodHotness()), predNode->getByteCodeIndex(), tt->getNode()->getByteCodeIndex()),
                            tt->getNextTreeTop());
                    }
                }
            }
        ...
        if (node->getOpCode().isBranch() && !node->getOpCode().isSwitch()) {
            ...
            TR::DebugCounter::prependDebugCounter(self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "branches/%s/(%s)/%s/%d", opName, self()->comp()->signature(), self()->comp()->getHotnessName(self()->comp()->getMethodHotness()), tt->getNode()->getByteCodeIndex()),
                tt);
        }
        ...
        if (node->getOpCode().isNew()) {
            ...
            if (node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::anewarray) {
                ...
                if (classChild->getOpCodeValue() != TR::loadaddr) {
                    TR::DebugCounter::prependDebugCounter(self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "allocations/%s/child-%s", opName, classChild->getOpCode().getName()), tt);
                } else if (classChild->getSymbolReference()->isUnresolved()) {
                    TR::DebugCounter::prependDebugCounter(self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "allocations/%s/unresolved", opName, classChild->getOpCode().getName()), tt);
                } else {
                    int32_t length;
                    ...
                    TR::DebugCounter::prependDebugCounter(self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "allocations/%s/(%.*s)", opName, length, className), tt);
                }
            } else {
                //newarray
