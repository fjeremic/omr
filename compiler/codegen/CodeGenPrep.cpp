...
void OMR::CodeGenerator::lowerTreesPreChildrenVisit(TR::Node *parent,
                                                    TR::TreeTop *treeTop,
                                                    vcount_t visitCount) {
  ...
  if (!disableILMulPwr2Opt &&
    ...
    if (secondChild->getOpCode().isLoadConst()) {
      ...
      int32_t shftAmnt = -1;
      if ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(
               value)) > 0) {
        if (value > 0) {
        ...
        } else  // negative value of the multiply constant
        {
          if (secondChild->getReferenceCount() == 1) {
            ...
            TR::Node *newChild =
                TR::Node::create(parent, TR::iconst, 0, shftAmnt);
            TR::Node *newChild2;
            isInteger ? newChild2 = TR::Node::create(parent, TR::ishl, 2)
                      : newChild2 = TR::Node::create(parent, TR::lshl, 2);
            ...
            parent->getSecondChild()->decReferenceCount();
            parent->setNumChildren(1);
            isInteger ? TR::Node::recreate(parent, TR::ineg)
                      : TR::Node::recreate(parent, TR::lneg);
...
void OMR::CodeGenerator::lowerTreeIfNeeded(TR::Node *node,
                                           int32_t childNumberOfNode,
                                           TR::Node *parent, TR::TreeTop *tt) {
  if (node->getOpCodeValue() == TR::loadaddr &&
      node->getOpCode().hasSymbolReference() && node->getSymbol()->isLabel()) {
  ...
  if (node->getOpCodeValue() == TR::ldiv &&
    ...
    if (firstChildOp == TR::iu2l && secondChildOp == TR::iu2l &&
        performTransformation(
            self()->comp(),
            "%sReduced ldiv with highWordZero children in node [%p] to idiv\n",
            OPT_DETAILS, node)) {
      TR::Node *idivChild =
  ...
  if (self()->getLocalsIG() && self()->getSupportsCompactedLocals()) {
    ...
    if (node->getOpCode().isStoreDirect() &&
      ...
      TR::AutomaticSymbol *storeLocal =
          node->getSymbolReference()->getSymbol()->castToAutoSymbol();
      TR::AutomaticSymbol *loadLocal = node->getFirstChild()
                                           ->getSymbolReference()
                                           ->getSymbol()
                                           ->castToAutoSymbol();
      ...
      if (!storeLocal->holdsMonitoredObject() &&
        ...
        TR_IGNode *storeIGNode =
            self()->getLocalsIG()->getIGNodeForEntity(storeLocal);
        TR_IGNode *loadIGNode =
            self()->getLocalsIG()->getIGNodeForEntity(loadLocal);
        ...
        if ((storeIGNode != NULL) && (loadIGNode != NULL)) {
          ...
          if (colourLoad == colourStore && colourLoad != UNCOLOURED &&
              performTransformation(self()->comp(),
                                    "%sCoalescing locals by removing store "
                                    "tree %p, load = %d, store = %d \n",
                                    OPT_DETAILS, node, colourLoad,
                                    colourStore)) {
            // if we are removing the initialization of the store local, it will
            // become uninitialized
            if (storeLocal->isInitializedReference() &&
                !loadLocal->isInitializedReference()) {
            ...
            TR_BitVector *liveButMaybeUnreferencedLocals =
                self()->getLiveButMaybeUnreferencedLocals();
            if (!liveButMaybeUnreferencedLocals) {
              int32_t numLocals = self()
                                      ->comp()
                                      ->getMethodSymbol()
                                      ->getAutomaticList()
                                      .getSize();
              liveButMaybeUnreferencedLocals = new (self()->trHeapMemory())
                  TR_BitVector(numLocals, self()->trMemory());
              self()->setLiveButMaybeUnreferencedLocals(
                  liveButMaybeUnreferencedLocals);
            }
            if (!storeLocal->isLiveLocalIndexUninitialized())
              liveButMaybeUnreferencedLocals->set(
                  storeLocal->getLiveLocalIndex());
            if (!loadLocal->isLiveLocalIndexUninitialized())
              liveButMaybeUnreferencedLocals->set(
                  loadLocal->getLiveLocalIndex());
  ...
  if (node->getOpCode().isBranch()) {
    ...
    if (destinationNode) {
      TR::Block *destinationBlock =
          node->getBranchDestination()->getNode()->getBlock();

      if (destinationBlock && (destinationBlock->getVisitCount() ==
                               self()->comp()->getVisitCount()))
        self()->getCurrentBlock()->setBranchesBackwards();
    }
  }

  if (node->getOpCodeValue() == TR::BBStart &&
      self()->comp()->getFlowGraph()->getStructure() != NULL) {
  ...
  if (node->getOpCodeValue() == TR::loadaddr ||
    ...
    if (node->getSymbol()->isParm()) node->getSymbol()->setParmHasToBeOnStack();
  } else if (node->getOpCode().isStore() && node->getSymbol()->isParm() &&
...
/**
 * Scan down the current subtree looking for any b2i or i2b (we know the parent
 * is a [i]bstore).  If the operations between the store and the convert cannot
 * trigger an overflow or underflow of the byte value ie consist only of bitwise
 * and, or or negate, then we can consider the upper bits to be garbage as they
 * will be thrownaway as part of the store.  Thus, we can mark the convert an
 * unneeded.  Codegen can then avoid generating the associated code.
 */
void OMR::CodeGenerator::identifyUnneededByteConvNodes(TR::Node *parent,
                                                       TR::TreeTop *treeTop,
                                                       vcount_t visitCount,
                                                       TR::DataType storeType) {
  parent->setVisitCount(visitCount);

  TR::ILOpCode &opCode = parent->getOpCode();

  // any other opcode could potentially cause over/underflow and change the
  // lowest byte if the b2i is noped.
  if (opCode.isConversion() ||
      ...
      opCode.isStore() || opCode.isLoad() ||
      ((parent == treeTop->getNode()) && opCode.isBooleanCompare())) {
    bool skipFirstChild = opCode.isIndirect();
    for (int32_t childCount = parent->getNumChildren() - 1; childCount >= 0;
         childCount--) {
      ...
      if (child->getVisitCount() != visitCount) {
        ...
        // o-type is evaluated as passThrough... making this optimization
        // invalid
        bool isOneByte = (storeType == TR::Int8
#ifdef J9_PROJECT_SPECIFIC
                          || (treeTop->getNode()->getOpCode().isBCDStore() &&
                              treeTop->getNode()->getSize() == 1)
#endif
                              ) &&
...
#ifdef J9_PROJECT_SPECIFIC
                          || (treeTop->getNode()->getOpCode().isBCDStore() &&
                              treeTop->getNode()->getSize() == 2)
#endif
                              ) &&
                         child->getOpCode().isConversion();
        if ((isOneByte && (childOp == TR::i2b || childOp == TR::b2i ||
                           childOp == TR::bu2i)) ||
            (isTwoByte && (childOp == TR::i2s || childOp == TR::s2i ||
                           childOp == TR::su2i))) {
          if (child->getReferenceCount() > 1 &&
              (!treeTop->getNode()->getOpCode().isBooleanCompare() ||
               (childOp != TR::bu2i && childOp != TR::su2i)) &&
              performTransformation(self()->comp(),
                                    "%sReplacing shared i2b/b2i node %p\n",
                                    OPT_DETAILS, child)) {
            TR::Node *newChild =
                TR::Node::create(childOp, 1, child->getFirstChild());
            child->decReferenceCount();
            parent->setAndIncChild(childCount, newChild);
            child = newChild;
          }
          if ((childOp == TR::b2i || childOp == TR::s2i) &&
              treeTop->getNode()->getOpCode().isBooleanCompare() &&
              ...
              performTransformation(
                  self()->comp(),
                  "%sChanging b2i node %p to unsigned conversion\n",
                  OPT_DETAILS, child)) {
            TR::Node::recreate(child,
                               (storeType == TR::Int8 ? TR::bu2i : TR::su2i));
          } else if (performTransformation(
                         self()->comp(),
                         "%sMarking i2b/b2i node %p as unneeded\n", OPT_DETAILS,
                         child))
            child->setUnneededConversion(true);
        }

        if (child->getReferenceCount() == 1)
          self()->identifyUnneededByteConvNodes(child, treeTop, visitCount,
                                                storeType);
      }
    }
  }
...
void OMR::CodeGenerator::identifyUnneededByteConvNodes() {
  ...
  if (!performTransformation(
          self()->comp(),
          "%s ===>   Identify and mark Unneeded b2i/i2b conversions  <===\n",
          OPT_DETAILS))
    return;
  ...
  for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    TR_ASSERT(node->getVisitCount() != visitCount,
              "Code Gen: error in marking b2i nodes");
    ...
    if ((
      ...
      self()->identifyUnneededByteConvNodes(node, tt, visitCount,
                                            node->getDataType());
    }
...
bool shouldResetRequiresConditionCodes(TR::Node *node) {
  if (!node->chkOpsNodeRequiresConditionCodes()) return false;
  if (!node->nodeRequiresConditionCodes()) return false;
  if (node->getOpCode().isArithmetic()) return true;
...
void OMR::CodeGenerator::cleanupFlags(TR::Node *node) {
  ...
  if (node->isAdjunct()) node->setIsAdjunct(false);
  ...
  if (node->getOpCodeValue() == TR::computeCC) {
    // implicitly uncommon child by increasing the reference count of its
    // children as though their parent had been uncommoned. This has the effect
    // of keeping their registers alive until the actual point of uncommoning
    ...
    // do not set requires condition code flag, this will only be set when is
    // about to be eval for CC node
  }
  ...
  if (node->isDualHigh()) {
    TR_ASSERT(!node->getChild(2)->isAdjunct(),
              "Code Limitation: Cannot cope with common adjunct between dual "
              "highs. node = %p.",
              node);
...
void OMR::CodeGenerator::addCountersToEdges(TR::Block *b) {
  ...
  bool found = (std::find(_counterBlocks.begin(), _counterBlocks.end(), b) !=
                _counterBlocks.end());
  if (lastNode->getOpCode().isBranch() && !found) {
    ...
    const char *c = TR::DebugCounter::debugCounterName(
        self()->comp(), "block_%d TAKEN", b->getNumber());
    ...
    if (c && self()->comp()->getOptions()->dynamicDebugCounterIsEnabled(c) && !(dest->getPredecessors().size() == 1) /* && !toBlock(relevantEdge->getFrom())->getSuccessors().isSingleton() */)
      ...
      traceMsg(self()->comp(),
               "\nSplitting edge, create new intermediate block_%d to add edge "
               "counters",
               newBlock->getNumber());
      if (glRegDeps) newBlock->takeGlRegDeps(self()->comp(), glRegDeps);
    ...
    TR::DebugCounter::prependDebugCounter(
        self()->comp(),
        TR::DebugCounter::debugCounterName(self()->comp(), "block_%d TAKEN",
                                           b->getNumber()),
    ...
    if (lastNode->getOpCode().isIf()) {
      TR::DebugCounter::prependDebugCounter(
          ...
          TR::DebugCounter::debugCounterName(
              self()->comp(), "block_%d NOT TAKEN", b->getNumber()),
  ...
  for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    if (node->getOpCodeValue() == TR::BBStart) {
      ...
      if (block->isCold()) {
        TR::DebugCounter::prependDebugCounter(
            self()->comp(),
            TR::DebugCounter::debugCounterName(
                self()->comp(), "blocks/%sCompiles/coldBlocks/=%d",
                self()->comp()->getHotnessName(
                    self()->comp()->getMethodHotness()),
                block->getNumber()),
            tt->getNextTreeTop(), 1, TR::DebugCounter::Free);
        TR::DebugCounter::prependDebugCounter(
            self()->comp(),
            TR::DebugCounter::debugCounterName(
                self()->comp(), "coldBlocks/byJittedBody/(%s)/%s/=%d",
                self()->comp()->signature(),
                self()->comp()->getHotnessName(
                    self()->comp()->getMethodHotness()),
                block->getNumber()),
        ...
        TR::DebugCounter::prependDebugCounter(
            self()->comp(),
            TR::DebugCounter::debugCounterName(
                self()->comp(), "blocks/%sCompiles/warmBlocks/=%d",
                self()->comp()->getHotnessName(
                    self()->comp()->getMethodHotness()),
                block->getNumber()),
            tt->getNextTreeTop());
        TR::DebugCounter::prependDebugCounter(
            self()->comp(),
            TR::DebugCounter::debugCounterName(
                self()->comp(), "warmBlocks/byJittedBody/(%s)/%s/=%d",
                self()->comp()->signature(),
                self()->comp()->getHotnessName(
                    self()->comp()->getMethodHotness()),
                block->getNumber()),
        ...
        if (predBlock->getEntry() && predBlock->getExit()) {
          ...
          if (predNode->getOpCode().isSwitch()) {
            ...
            TR::DebugCounter::prependDebugCounter(
                self()->comp(),
                TR::DebugCounter::debugCounterName(
                    self()->comp(), "branchtargets/%s/(%s)/%s/%d/%d", opName,
                    self()->comp()->signature(),
                    self()->comp()->getHotnessName(
                        self()->comp()->getMethodHotness()),
    ...
    if (node->getOpCode().isBranch() && !node->getOpCode().isSwitch()) {
      ...
      TR::DebugCounter::prependDebugCounter(
          self()->comp(),
          TR::DebugCounter::debugCounterName(
              self()->comp(), "branches/%s/(%s)/%s/%d", opName,
              self()->comp()->signature(),
              self()->comp()->getHotnessName(
                  self()->comp()->getMethodHotness()),
              tt->getNode()->getByteCodeIndex()),
      ...
      if (node->getOpCodeValue() == TR::New ||
        ...
        if (classChild->getOpCodeValue() != TR::loadaddr) {
          TR::DebugCounter::prependDebugCounter(
              self()->comp(),
              TR::DebugCounter::debugCounterName(
                  self()->comp(), "allocations/%s/child-%s", opName,
          ...
          TR::DebugCounter::prependDebugCounter(
              self()->comp(),
              TR::DebugCounter::debugCounterName(
                  self()->comp(), "allocations/%s/unresolved", opName,
                  classChild->getOpCode().getName()),
              tt);
        } else {
          int32_t length;
          char *className = TR::Compiler->cls.classNameChars(
              self()->comp(),
              (TR_OpaqueClassBlock *)classChild->getSymbol()
                  ->castToStaticSymbol()
                  ->getStaticAddress(),
              length);
          TR::DebugCounter::prependDebugCounter(
        ...
        TR::DebugCounter::prependDebugCounter(
            self()->comp(),
            TR::DebugCounter::debugCounterName(
                self()->comp(), "allocations/%s/%s", opName, typeName),
            tt);
      }
    }
  }
