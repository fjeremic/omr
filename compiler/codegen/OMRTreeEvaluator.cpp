...
bool
OMR::TreeEvaluator::isStaticClassSymRef(TR::SymbolReference* symRef)
{
...
bool
OMR::TreeEvaluator::instanceOfOrCheckCastNeedEqualityTest(TR::Node* node,
                                                          TR::CodeGenerator* cg)
{
...
// test
bool
OMR::TreeEvaluator::instanceOfOrCheckCastNeedSuperTest(TR::Node* node,
                                                       TR::CodeGenerator* cg)
{
  TR::Node* castClassNode = node->getSecondChild();
                                                     ...
                                                     TR::CodeGenerator* cg)
{
  ...
  while (node->getType() == TR::NoType) {
    ...
    TR_ASSERT(
      node,
      "The node should always be valid while looking for a Child with a type");
  }
  TR_ASSERT(node->getType() != TR::NoType,
            "Expecting node %p, to have a specific type here",
            node);
...
int32_t
OMR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(int32_t value)
{
  if (isNonNegativePowerOf2(value)) {
    int32_t shiftAmount = 0;
    while ((value = ((uint32_t)value) >> 1)) {
...
int32_t
OMR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(int64_t value)
{
  if (isNonNegativePowerOf2(value)) {
    int32_t shiftAmount = 0;
    while ((value = ((uint64_t)value) >> 1)) {
...
TR::Register*
OMR::TreeEvaluator::unImpOpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT_FATAL(
    false, "Opcode %s is not implemented\n", node->getOpCode().getName());
  return NULL;
}

TR::Register*
OMR::TreeEvaluator::badILOpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT_FATAL(
    false, "badILOp %s cannot be evaluated\n", node->getOpCode().getName());
  return NULL;
}

bool
OMR::TreeEvaluator::nodeIsIArithmeticOverflowCheck(
  TR::Node* node,
...
bool
OMR::TreeEvaluator::nodeIsIAddOverflowCheck(TR::Node* node,
                                            TR_ArithmeticOverflowCheckNodes* u)
{
...
bool
OMR::TreeEvaluator::nodeIsISubOverflowCheck(TR::Node* node,
                                            TR_ArithmeticOverflowCheckNodes* u)
{
  TR::Compilation* comp = TR::comp();
  if (comp->getOption(TR_DisableTreePatternMatching))
...
bool
OMR::TreeEvaluator::nodeIsLArithmeticOverflowCheck(
  TR::Node* node,
  TR_ArithmeticOverflowCheckNodes* u)
{
...
bool
OMR::TreeEvaluator::nodeIsLAddOverflowCheck(TR::Node* node,
                                            TR_ArithmeticOverflowCheckNodes* u)
{
  TR::Compilation* comp = TR::comp();
  if (comp->getOption(TR_DisableTreePatternMatching))
...
bool
OMR::TreeEvaluator::nodeIsLSubOverflowCheck(TR::Node* node,
                                            TR_ArithmeticOverflowCheckNodes* u)
{
  TR::Compilation* comp = TR::comp();
  if (comp->getOption(TR_DisableTreePatternMatching))
...
//
void
OMR::TreeEvaluator::evaluateNodesWithFutureUses(TR::Node* node,
                                                TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (node->getFutureUseCount() > 0) {
    ...
    if (actualLoadOrStoreChild->getOpCode().isStore() ||
      ...
      if (comp->getOption(TR_TraceCG))
        traceMsg(
          comp, "O^O pre-evaluating escaping commoned subtree %p\n", node);
...
//
void
OMR::TreeEvaluator::initializeStrictlyFutureUseCounts(TR::Node* node,
                                                      vcount_t visitCount,
                                                      TR::CodeGenerator* cg)
{
  if (node->getRegister() != NULL) {
  ...
  if (node->getVisitCount() != visitCount) {
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++)
      TR::TreeEvaluator::initializeStrictlyFutureUseCounts(
        node->getChild(i), visitCount, cg);
  }
