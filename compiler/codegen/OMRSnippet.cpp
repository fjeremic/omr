...
OMR::Snippet::Snippet(TR::CodeGenerator* cg,
                      TR::Node* node,
...
uint8_t* OMR::Snippet::emitSnippet() {
...
void OMR::Snippet::print(TR::FILE* f, TR_Debug* debug) {
  ...
  debug->printSnippetLabel(f, self()->getSnippetLabel(), cursor,
                           "<Unknown Snippet>");
  ...
  if (self()->getLength(0) % sizeof(uint64_t) != 0) {
    debug->printPrefix(f, NULL, cursor,
                       self()->getLength(0) % sizeof(uint64_t));
  }
}
