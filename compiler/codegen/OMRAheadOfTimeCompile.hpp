...
public:
  ...
  AheadOfTimeCompile(uint32_t *headerSizeMap, TR::Compilation *c)
      : _comp(c), _sizeOfAOTRelocations(0), _relocationData(NULL),
  ...
  TR_LinkHead<TR::IteratedExternalRelocation> &getAOTRelocationTargets() {
    return _aotRelocationTargets;
  }

  uint32_t getSizeOfAOTRelocations() { return _sizeOfAOTRelocations; }
  uint32_t setSizeOfAOTRelocations(uint32_t s) {
    return (_sizeOfAOTRelocations = s);
  }
  uint32_t addToSizeOfAOTRelocations(uint32_t n) {
    return (_sizeOfAOTRelocations += n);
  }
  ...
  virtual uint8_t *
  initializeAOTRelocationHeader(TR::IteratedExternalRelocation *relocation) = 0;

  // virtual void dumpRelocationData() = 0;
  void dumpRelocationData() {}

  void traceRelocationOffsets(uint8_t *&cursor, int32_t offsetSize,
