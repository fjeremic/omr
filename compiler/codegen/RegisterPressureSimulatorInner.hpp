...
void computeSimulatedSpilledRegs(TR_BitVector *spilledRegisters,
                                 int32_t blockNum,
...
struct TR_RegisterPressureState {
  ...
  int32_t _gprPressure; // (int32_t so increments will match decrements even
                        // with enormous register pressure)
  int32_t _fprPressure; // (ditto)
  int32_t _vrfPressure; // (ditto)
  int32_t _gprPreviousPressure; // Pressure immediately before the latest call
                                // to simulateNodeEvaluation
  ...
  bool _candidateIsLiveOnEntry; // TODO: Why do we need this?  Why doesn't it
                                // work when I check the candidate's
                                // getBlocksLiveOnEntry?
  bool _pressureRiskFromStart;  // True if candidate was live on entry and would
                                // not be in a register without GRA
  uint32_t _pressureRiskUntilEnd; // Nonzero if a value has been stored to the
                                  // candidate, the value would not be in a
                                  // register without GRA, and the candidate is
                                  // live on an exit we haven't reached yet
  uint32_t
      _numLiveCandidateLoads; // Number of isCandidateLoad nodes that have been
                              // evaluated, but whose refcount is not yet zero
  uint32_t _rematNestDepth;   // How many nodes between current node and current
                              // treetop will be duplicated by rematerialization
  vcount_t _visitCountForInit; // Used to indicate that a node's entry in the
                               // node state array has been initialized

  uint32_t _memrefNestDepth; // Recursion depth of simulateMemoryReference; used
                             // for diagnostics
  TR_SimulatedMemoryReference *_currentMemref;

  TR_RegisterPressureState(
      TR_RegisterCandidate *candidate, bool candidateIsLiveOnEntry,
      ...
      uint32_t gprLimit, uint32_t fprLimit, uint32_t vrfLimit,
      vcount_t visitCountForInit)
      : _currentBlock(NULL), _currentTreeTop(NULL), _candidate(candidate),
        _alreadyAssignedOnEntry(alreadyAssignedOnEntry),
        _alreadyAssignedOnExit(alreadyAssignedOnExit),
        _candidatesAlreadyAssigned(candidatesAlreadyAssigned), _gprPressure(0),
        _fprPressure(0), _vrfPressure(0), _gprLimit(gprLimit),
        _fprLimit(fprLimit), _vrfLimit(vrfLimit),
        _candidateIsLiveOnEntry(candidateIsLiveOnEntry),
        _pressureRiskFromStart(candidateIsLiveOnEntry),
        _pressureRiskUntilEnd(0), _numLiveCandidateLoads(0),
        _visitCountForInit(visitCountForInit), _memrefNestDepth(0),
        _rematNestDepth(0), _currentMemref(0) {}
  TR_RegisterPressureState(const TR_RegisterPressureState &other,
                           TR_RegisterCandidate *candidate,
                           bool candidateIsLiveOnEntry,
                           vcount_t visitCountForInit)
      : _currentBlock(other._currentBlock),
        _currentTreeTop(other._currentTreeTop), _candidate(candidate),
        ...
        _gprPressure(other._gprPressure), _fprPressure(other._fprPressure),
        _vrfPressure(other._vrfPressure),
        _candidateIsLiveOnEntry(candidateIsLiveOnEntry),
        _pressureRiskFromStart(candidateIsLiveOnEntry),
        _pressureRiskUntilEnd(0), _gprLimit(other._gprLimit),
        _fprLimit(other._fprLimit), _vrfLimit(other._vrfLimit),
  ...
  // This indicates that we have already prepared this node for the simulation
  // process. It's an internal bookkeeping thing.
  //
  ...
  bool pressureIsAtRisk() {
    return _pressureRiskFromStart || (_pressureRiskUntilEnd > 0);
  }
  bool candidateIsLiveAfterGRA() {
    return pressureIsAtRisk() || (_numLiveCandidateLoads > 0);
  }
...
struct TR_RegisterPressureSummary {
  ...
  bool isLinkagePresent(TR_LinkageConventions lc) {
    return 0 != (_linkageConventionMask & (1 << lc));
  }
  ...
  bool gprIsStale(uint32_t threshold) {
    return !isSpilled(TR_gprSpill) && _gprPressure > threshold;
  }
  bool fprIsStale(uint32_t threshold) {
    return !isSpilled(TR_fprSpill) && _fprPressure > threshold;
  }
  bool vrfIsStale(uint32_t threshold) {
    return !isSpilled(TR_vrfSpill) && _vrfPressure > threshold;
  }

  void accumulate(TR_RegisterPressureSummary other) {
    accumulate(other._gprPressure, other._fprPressure, other._vrfPressure,
               other._linkageConventionMask, other._spillMask);
  }

  void accumulate(TR_RegisterPressureState *state, TR::CodeGenerator *cg,
                  uint32_t gprTemps = 0, uint32_t fprTemps = 0,
                  uint32_t vrfTemps = 0);
  ...
  TR_RegisterPressureSummary() // Initially stale
      : _gprPressure(PRESSURE_LIMIT), _fprPressure(PRESSURE_LIMIT),
        _vrfPressure(PRESSURE_LIMIT), _linkageConventionMask(0), _spillMask(0) {
  }
  TR_RegisterPressureSummary(uint8_t gprPressure, uint8_t fprPressure,
                             uint8_t vrfPressure)
      : _gprPressure(gprPressure), _fprPressure(fprPressure),
        _vrfPressure(vrfPressure), _linkageConventionMask(0), _spillMask(0) {}

private:
  void accumulate(uint32_t gprPressure, uint32_t fprPressure = 0,
                  uint32_t vrfPressure = 0, uint32_t linkageConventionMask = 0,
    ...
    //
    _gprPressure =
        std::min<uint32_t>(std::max(_gprPressure, gprPressure), PRESSURE_LIMIT);
    _fprPressure =
        std::min<uint32_t>(std::max(_fprPressure, fprPressure), PRESSURE_LIMIT);
    _vrfPressure =
        std::min<uint32_t>(std::max(_vrfPressure, vrfPressure), PRESSURE_LIMIT);
    /*
             TR_ASSERT(_gprPressure <= INT_MAX/2 && gprPressure <= INT_MAX/2,
       "gprPressure is getting too high!"); TR_ASSERT(_fprPressure <= INT_MAX/2
       && fprPressure <= INT_MAX/2, "fprPressure is getting too high!");
             TR_ASSERT(_vrfPressure <= INT_MAX/2 && vrfPressure <= INT_MAX/2,
       "vrfPressure is getting too high!");
...
void computeSpilledRegsForAllPresentLinkages(
    TR_BitVector *spilledRegisters, TR_RegisterPressureSummary &summary);
void setSpilledRegsForAllPresentLinkages(TR_BitVector *spilledRegisters,
...
bool isLoadAlreadyAssignedOnEntry(TR::Node *node,
                                  TR_RegisterPressureState *state);

struct TR_SimulatedNodeState {
  TR_ALLOC(TR_Memory::CodeGenerator)

  TR::TreeTop *_keepLiveUntil; // Register pressure contribution should not be
                               // subtracted until this tree has been simulated
  uint16_t _isCausingPressureRiskUntilEnd : 1; // For nodes included in
                                               // _pressureRiskUntilEnd
  ...
  void setHeight(uint32_t newHeight) {
    _height = std::min<uint32_t>(newHeight, 255);
  }
};

struct TR_SimulatedMemoryReference {
  TR_SimulatedMemoryReference(TR_Memory *m)
      : _undecrementedNodes(m), _numRegisters(0), _numConsolidatedRegisters(0) {
  }

  List<TR::Node>
      _undecrementedNodes; // Nodes that need a simulateDecReferenceCount call
  int32_t _numRegisters;
  int32_t _numConsolidatedRegisters;

  enum { MAX_NUM_REGISTERS = 2, MAX_STRIDE = 8 }; // X86

  void add(TR::Node *node, TR_RegisterPressureState *state,
           TR::CodeGenerator *cg);
  void simulateDecNodeReferenceCounts(TR_RegisterPressureState *state,
                                      TR::CodeGenerator *cg);
};
...
void simulateBlockEvaluation(TR::Block *block, TR_RegisterPressureState *state,
                             TR_RegisterPressureSummary *summary);
void simulateTreeEvaluation(TR::Node *node, TR_RegisterPressureState *state,
                            TR_RegisterPressureSummary *summary);
void simulateSkippedTreeEvaluation(TR::Node *node,
                                   TR_RegisterPressureState *state,
                                   TR_RegisterPressureSummary *summary,
                                   char tagChar = 's'); // 's' for "skipped"
void simulateNodeInitialization(
    TR::Node *node, TR_RegisterPressureState
                        *state); // ok to call if node is already initialized
...
bool nodeResultConsumesNoRegisters(TR::Node *node,
                                   TR_RegisterPressureState *state);
void simulateMemoryReference(TR_SimulatedMemoryReference *memref,
                             TR::Node *node, TR_RegisterPressureState *state,
                             TR_RegisterPressureSummary *summary);
void simulationPrePass(TR::TreeTop *tt, TR::Node *node,
                       TR_RegisterPressureState *state,
                       TR_RegisterPressureSummary *summary);
...
// them, and their implementations can call these ones to handle the mundane
// cases.
//
...
virtual uint8_t nodeResultGPRCount(TR::Node *node,
                                   TR_RegisterPressureState *state);
virtual uint8_t nodeResultFPRCount(TR::Node *node,
                                   TR_RegisterPressureState *state);
virtual uint8_t nodeResultSSRCount(TR::Node *node,
                                   TR_RegisterPressureState *state);
virtual uint8_t nodeResultARCount(TR::Node *node,
                                  TR_RegisterPressureState *state);
virtual uint8_t nodeResultVRFCount(TR::Node *node,
                                   TR_RegisterPressureState *state);
virtual bool nodeCanBeFoldedIntoMemref(TR::Node *node,
                                       TR_RegisterPressureState *state);
virtual bool nodeWillBeRematerialized(TR::Node *node,
                                      TR_RegisterPressureState *state);
virtual void simulateNodeEvaluation(TR::Node *node,
                                    TR_RegisterPressureState *state,
                                    TR_RegisterPressureSummary *summary);
virtual void getNumberOfTemporaryRegistersUsedByCall(
    TR::Node *node, TR_RegisterPressureState *state,
...
// Note state array: one per node, stack allocated (so invalid after GRA is
// finished)
//
