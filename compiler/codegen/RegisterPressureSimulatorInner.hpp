...
struct TR_RegisterPressureState {
  ...
  uint32_t _numLiveCandidateLoads; // Number of isCandidateLoad nodes that have been evaluated, but whose refcount is
                                   // not yet zero
  uint32_t
    _rematNestDepth; // How many nodes between current node and current treetop will be duplicated by rematerialization
  ...
  TR_RegisterPressureState(TR_RegisterCandidate* candidate, bool candidateIsLiveOnEntry,
    TR_BitVector& alreadyAssignedOnEntry, TR_BitVector& alreadyAssignedOnExit,
    TR_LinkHead<TR_RegisterCandidate>* candidatesAlreadyAssigned, uint32_t gprLimit, uint32_t fprLimit,
    ...
    , _currentMemref(0)
  {}
  TR_RegisterPressureState(const TR_RegisterPressureState& other, TR_RegisterCandidate* candidate,
  ...
  {
    return _candidateIsLiveOnEntry;
  }
...
struct TR_RegisterPressureSummary {
  ...
  bool isSpilled(TR_SpillKinds kind)
  {
    return (_spillMask & (1 << kind)) != 0;
  }
  ...
  void accumulate(TR_RegisterPressureState* state, TR::CodeGenerator* cg, uint32_t gprTemps = 0, uint32_t fprTemps = 0,
    uint32_t vrfTemps = 0);
