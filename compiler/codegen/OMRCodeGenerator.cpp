  ...
  , _lmmdFailed(false)
{
  ...
  _disableInternalPointers = self()->comp()->getOption(TR_MimicInterpreterFrameShape)
    || self()->comp()->getOptions()->realTimeGC() || self()->comp()->getOption(TR_DisableInternalPointers);
...
OMR::CodeGenerator::TR_RegisterPressureSummary* OMR::CodeGenerator::calculateRegisterPressure()
{
  return NULL;
}
...
bool OMR::CodeGenerator::addressesMatch(TR::Node* addr1, TR::Node* addr2, bool addressesUnderSameTreeTop)
{
  ...
  if (self()->nodeMatches(addr1, addr2, addressesUnderSameTreeTop)) {
    ...
    if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)) {
      if (self()->additionsMatch(addr1, addr2, addressesUnderSameTreeTop)) {
        foundMatch = true;
      } else if (addr1->getFirstChild() && addr2->getFirstChild() && self()->isSupportedAdd(addr1->getFirstChild())
        ...
        if (firstChildrenMatch && addr1->getSecondChild() && addr2->getSecondChild()
          ...
          secondChildrenMatch
            = self()->additionsMatch(addr1->getSecondChild(), addr2->getSecondChild(), addressesUnderSameTreeTop);
        }
  ...
  if (!foundMatch && addressesUnderSameTreeTop) {
    ...
    // 1) before evaluation : refCounts of both nodes are 1 (a more refined test, that requires scanning ahead in the
    // block, is if both are first references) 2) during evaluation : registers are both NULL (i.e. both first ref)
    ...
    if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)
      ...
      if (traceInspect())
        traceMsg(comp(),
          "\t\tfound possibly matching additions : update addr1 to %s (%p), addr2 to %s (%p) and continue matching\n",
...
uint8_t* OMR::CodeGenerator::alignBinaryBufferCursor()
{
  ...
  if (self()->supportsJitMethodEntryAlignment() && boundary > 1) {
    ...
    TR_ASSERT_FATAL(threshold <= boundary,
      "JIT method entry alignment threshold (%d) definition is violated as it is larger than the boundary (%d)",
      threshold, boundary);
...
uint8_t* OMR::CodeGenerator::emitSnippets()
{
  ...
  for (auto iterator = _snippetList.begin(); iterator != _snippetList.end(); ++iterator) {
    ...
    if (codeOffset != NULL) {
      TR_ASSERT((*iterator)->getLength(self()->getBinaryBufferCursor() - self()->getBinaryBufferStart())
...
TR::Instruction* OMR::CodeGenerator::getVirtualGuardForPatching(TR::Instruction* vgdnop)
{
  ...
  for (TR::Instruction* prevI = vgdnop->getPrev(); prevI; prevI = prevI->getPrev()) {
    if (prevI->isVirtualGuardNOPInstruction()) {
      ...
      } else if ((prevI->getNode()->getOpCodeValue() == TR::BBStart || prevI->getNode()->getOpCodeValue() == TR::BBEnd)
  ...
  if (toReturn != vgdnop) {
    ...
    if (self()->comp()->getOption(TR_TraceCG))
      traceMsg(self()->comp(),
        "vgdnop instruction [%p] begins scanning for patch instructions for mergeable guard [%p]\n", vgdnop, toReturn);
  }
  return toReturn;
}
...
int32_t OMR::CodeGenerator::sizeOfInstructionToBePatched(TR::Instruction* vgdnop)
{
...
int32_t OMR::CodeGenerator::sizeOfInstructionToBePatchedHCRGuard(TR::Instruction* vgdnop)
{
  TR::Instruction* nextI;
  TR::Node* firstBBEnd = NULL;
...
intptrj_t OMR::CodeGenerator::hiValue(intptrj_t address)
{
  if (self()
        ->comp()
        ->compileRelocatableCode()) // We don't want to store values using HI_VALUE at compile time, otherwise, we do
                                    // this a 2nd time when we relocate (and new value is based on old one)
...
bool OMR::CodeGenerator::isInMemoryInstructionCandidate(TR::Node* node)
{
  ...
  if (node->getFirstChild()->isSingleRefUnevaluated() && valueChild->isSingleRefUnevaluated()
