...
TR_TreeEvaluatorFunctionPointer OMR::CodeGenerator::_nodeToInstrEvaluators[] = {
#include "codegen/TreeEvaluatorTable.hpp"
};

static_assert(TR::NumIlOps ==
                  (sizeof(OMR::CodeGenerator::_nodeToInstrEvaluators) /
                   sizeof(OMR::CodeGenerator::_nodeToInstrEvaluators[0])),
              "NodeToInstrEvaluators is not the correct size");

#define OPT_DETAILS "O^O CODE GENERATION: "

TR::Instruction *OMR::CodeGenerator::generateNop(TR::Node *node,
...
OMR::CodeGenerator::CodeGenerator()
    : _compilation(TR::comp()),
      ...
      _availableSpillTemps(
          getTypedAllocator<TR::SymbolReference *>(TR::comp()->allocator())),
      _counterBlocks(getTypedAllocator<TR::Block *>(TR::comp()->allocator())),
      _liveReferenceList(
          getTypedAllocator<TR_LiveReference *>(TR::comp()->allocator())),
      _snippetList(getTypedAllocator<TR::Snippet *>(TR::comp()->allocator())),
      _registerArray(self()->comp()->trMemory()),
      _spill4FreeList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _spill8FreeList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _spill16FreeList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _internalPointerSpillFreeList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      ...
      _allSpillList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _relocationList(
          getTypedAllocator<TR::Relocation *>(TR::comp()->allocator())),
      _externalRelocationList(
          getTypedAllocator<TR::Relocation *>(TR::comp()->allocator())),
      ...
      _stackOfArtificiallyInflatedNodes(
          self()->comp() ? self()->comp()->trMemory() : 0, 16),
  ...
  uintptrj_t maxSize =
      TR::Compiler->vm.getOverflowSafeAllocSize(self()->comp());
  int32_t i;
  ...
  for (i = 0; i < TR_NumLinkages; ++i) _linkages[i] = NULL;

  _maxObjectSizeGuaranteedNotToOverflow =
      (maxSize > UINT_MAX) ? UINT_MAX : maxSize;

  if (self()->comp()->getDebug()) self()->comp()->getDebug()->resetDebugData();

  self()->setIsLeafMethod();
}

TR_StackMemory OMR::CodeGenerator::trStackMemory() {
...
void OMR::CodeGenerator::lowerTrees() {
  TR::Delimiter d(self()->comp(), self()->comp()->getOption(TR_TraceCG),
                  "LowerTrees");
  ...
  //
  // NOTE: when changing, please be aware that during the following tree walk
  // some trees might be removed (look at lowerTreeIfNeeded)
  //
  ...
  for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
    ...
    TR_ASSERT(node->getVisitCount() != visitCount,
              "Code Gen: error in lowering trees");
    TR_ASSERT(node->getReferenceCount() == 0,
              "Code Gen: error in lowering trees");
...
void OMR::CodeGenerator::lowerTreesWalk(TR::Node *parent, TR::TreeTop *treeTop,
                                        vcount_t visitCount) {
...
void OMR::CodeGenerator::preLowerTrees() {
  ...
  _localsThatAreStored = new (self()->comp()->trHeapMemory())
      TR_BitVector(symRefCount, self()->comp()->trMemory(), heapAlloc);
  _numLocalsWhenStoreAnalysisWasDone = symRefCount;
}

void OMR::CodeGenerator::lowerTreesPreTreeTopVisit(TR::TreeTop *tt,
                                                   vcount_t visitCount) {
...
void OMR::CodeGenerator::lowerTreesPostTreeTopVisit(TR::TreeTop *tt,
                                                    vcount_t visitCount) {}

void OMR::CodeGenerator::lowerTreesPostChildrenVisit(TR::Node *parent,
                                                     TR::TreeTop *treeTop,
                                                     vcount_t visitCount) {}
...
void OMR::CodeGenerator::setUpForInstructionSelection() {
  ...
  // prepareNodeForInstructionSelection is called during a separate walk of the
  // treetops because the _register and _label fields are unioned members of a
  // node.  prepareNodeForInstructionSelection zeros the _register field while
  // the second for loop sets label fields on destination nodes.
  //
  ...
  for (tt = self()->comp()->getStartTree(); tt;
       prev = tt, tt = tt->getNextTreeTop()) {
    TR::Node *node = tt->getNode();

    if ((node->getOpCodeValue() == TR::treetop) ||
        node->getOpCode().isAnchor() || node->getOpCode().isCheck()) {
    ...
    if (opcode.getOpCodeValue() == TR::BBStart) {
      ...
      for (int i = 1; i < upperBound; ++i) {
        if (node->getChild(i)->getBranchDestination()->getNode()->getLabel() ==
            NULL) {
 ...
 * a constant integer is made alive across calls and, thereby, lower the
 * preserved register pressure caused due to this.
 *
 * \details
 *
 * \verbatim
 * Take z/Architecture as an example, we could run into the following situation
 * where constant integer 16 is loaded to a preserved register GPR6, which is
 * then loaded to GPR2 as call parameter. The GPR6 will be preserved by the
 * callee and can incur extra register shuffling.
 *
 ...
 * LGHI    GPR0,0xff28
 * BASR    GPR14,GPR14         # call virtual function, which will unnecessarily
 * preserve GPR6. LGR     GPR2,GPR6           # load parameter from preserved
 * register again for the next use
 * ...
 *
 * \endverbatim
 *
 * Loading integer constants as immediate values [i.e. LHI GPR2, 0x10] is
 * preferred. The ranges for immediate value loading on different platforms are
 * governed by the following platform-specific query:
 *
 * \verbatim
      ...
      for (uint32_t i = 0; i < callNode->getNumChildren(); ++i) {
        ...
        if (paramNode->getReferenceCount() > 1 &&
          ...
          TR::Node *newConstNode =
              TR::Node::create(paramNode->getOpCodeValue(), 0);
...
void OMR::CodeGenerator::doInstructionSelection() {
  ...
  self()->setNextAvailableBlockIndex(comp->getFlowGraph()->getNextNodeNumber() +
                                     1);
  ...
  if (comp->getOption(TR_TraceCG) || debug("traceGRA")) {
    self()->getDebug()->setupToDumpTreesAndInstructions(
...

  {
    ...
    TR_BitVector nodeChecklistBeforeDump(
        comp->getNodeCount(), self()->trMemory(), stackAlloc, growable);

    for (TR::TreeTop *tt = comp->getStartTree(); tt;
         tt = self()->getCurrentEvaluationTreeTop()->getNextTreeTop()) {
      TR::Instruction *prevInstr = self()->getAppendInstruction();
      TR::Node *node = tt->getNode();
      TR::ILOpCodes opCode = node->getOpCodeValue();

      TR::Node *firstChild =
          node->getNumChildren() > 0 ? node->getFirstChild() : 0;
        ...
        if (!block->isExtensionOfPreviousBlock()) {
          ...
          if (liveLocals) {
            if (block->getLiveLocals()) {
              liveLocals = new (self()->trHeapMemory())
                  TR_BitVector(*block->getLiveLocals());
            } else {
              liveLocals =
                  new (self()->trHeapMemory()) TR_BitVector(*liveLocals);
              liveLocals->empty();
            }
          }
        }
...
#if DEBUG
        // Verify that we are only being more conservative by inheriting the
        // live local information from the previous block.
        //
        else if (liveLocals && debug("checkBlockEntryLiveLocals")) {
          TR_BitVector *extendedBlockLocals = new (self()->trStackMemory())
              TR_BitVector(*(block->getLiveLocals()));
      ...
      if (comp->getOption(TR_TraceCG) || debug("traceGRA")) {
        ...
        self()->getDebug()->dumpSingleTreeWithInstrs(tt, NULL, true, false,
                                                     true, true);
        traceMsg(comp, "\n------------------------------\n");
      }
      ...
      if (liveLocals) {
        ...
        if (debug("checkBlockEntryLiveLocals")) {
          ...
          // Check for a loadaddr of a local object containing collected
          // references. If so, this local object becomes live at this point.
          //
          else if (opCode == TR::treetop) {
            if (firstChild->getOpCodeValue() == TR::loadaddr) {
              liveSym = firstChild->getSymbol()->getLocalObjectSymbol();
      ...
      if (comp->getOption(TR_TraceCG) || debug("traceGRA")) {
        ...
        if (tt == self()->getCurrentEvaluationTreeTop()) {
          traceMsg(comp, "------------------------------\n");
          self()->getDebug()->dumpSingleTreeWithInstrs(tt, prevInstr->getNext(),
                                                       true, true, true, false);
        } else {
          // dump all the trees that the evaluator handled
          traceMsg(comp, "------------------------------");
          for (TR::TreeTop *dumptt = tt;
               dumptt !=
               self()->getCurrentEvaluationTreeTop()->getNextTreeTop();
               dumptt = dumptt->getNextTreeTop()) {
            traceMsg(comp, "\n");
            self()->getDebug()->dumpSingleTreeWithInstrs(dumptt, NULL, true,
                                                         false, true, false);
          }

          // all instructions are on the tt tree
          self()->getDebug()->dumpSingleTreeWithInstrs(
              tt, prevInstr->getNext(), false, true, false, false);
        }
    ...
    // Virtual function insertInstructionPrefetches is implemented only for s390
    // platform, for all other platforms the function is empty
    //
...
TR_PersistentMemory *OMR::CodeGenerator::trPersistentMemory() {
  return self()->trMemory()->trPersistentMemory();
}

void OMR::CodeGenerator::addSymbolAndDataTypeToMap(TR::Symbol *symbol,
                                                   TR::DataType dt) {
...
bool OMR::CodeGenerator::isRegisterClobberable(TR::Register *reg,
                                               uint16_t count) {
...
bool OMR::CodeGenerator::canClobberNodesRegister(TR::Node *node, uint16_t count,
 ...
 */
bool OMR::CodeGenerator::getSupportsIbyteswap() { return false; }
...
void OMR::CodeGenerator::toggleIsInOOLSection() {
  _flags4.set(IsInOOLSection, !self()->getIsInOOLSection());
}

bool OMR::CodeGenerator::traceBCDCodeGen() {
  return self()->comp()->getOptions()->getTraceCGOption(
...
bool OMR::CodeGenerator::traceSimulateTreeEvaluation() {
  return self()->comp()->getOptions()->trace(
...
TR::SymbolReference *OMR::CodeGenerator::getSymRef(TR_RuntimeHelper h) {
  return _symRefTab->getSymRef(h);
}
...
bool OMR::CodeGenerator::mulDecompositionCostIsJustified(int numOfOperations,
                                                         char bitPosition[],
                                                         char operationType[],
                                                         int64_t value) {
  if (self()->comp()->getOptions()->getTraceSimplifier(
...
uint8_t *OMR::CodeGenerator::getCodeStart() {
  return _binaryBufferStart + self()->getPrePrologueSize() +
...
bool OMR::CodeGenerator::needRelocationsForLookupEvaluationData() {
  return self()->comp()->compileRelocatableCode();
}

bool OMR::CodeGenerator::isGlobalVRF(TR_GlobalRegisterNumber n) {
  return self()->hasGlobalVRF() && n >= self()->getFirstGlobalVRF() &&
         n <= self()->getLastGlobalVRF();
}

bool OMR::CodeGenerator::isGlobalFPR(TR_GlobalRegisterNumber n) {
  return !self()->isGlobalGPR(n);
}

TR_BitVector *OMR::CodeGenerator::getBlocksWithCalls() {
  if (!_blocksWithCalls) self()->computeBlocksWithCalls();
  return _blocksWithCalls;
}

bool OMR::CodeGenerator::profiledPointersRequireRelocation() {
  return self()->comp()->compileRelocatableCode();
}

bool OMR::CodeGenerator::needGuardSitesEvenWhenGuardRemoved() {
  return self()->comp()->compileRelocatableCode();
}

bool OMR::CodeGenerator::supportVMInternalNatives() {
  return !self()->comp()->compileRelocatableCode();
}

bool OMR::CodeGenerator::supportsInternalPointers() {
  if (_disableInternalPointers) return false;
...
int32_t OMR::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
    TR::Block *block) {
...
TR::Register *OMR::CodeGenerator::allocateSinglePrecisionRegister(
    TR_RegisterKinds rk) {
...
void OMR::CodeGenerator::apply8BitLabelRelativeRelocation(
    int32_t *cursor, TR::LabelSymbol *label) {
  *(int8_t *)cursor += (int8_t)(intptrj_t)label->getCodeLocation();
}
void OMR::CodeGenerator::apply12BitLabelRelativeRelocation(
    int32_t *cursor, TR::LabelSymbol *label, bool isCheckDisp) {
  TR_ASSERT(0,
            "unexpected call to "
            "OMR::CodeGenerator::apply12BitLabelRelativeRelocation");
}
...
void OMR::CodeGenerator::apply24BitLabelRelativeRelocation(int32_t *cursor,
                                                           TR::LabelSymbol *) {
  TR_ASSERT(0,
            "unexpected call to "
            "OMR::CodeGenerator::apply24BitLabelRelativeRelocation");
}
void OMR::CodeGenerator::apply16BitLoadLabelRelativeRelocation(
    TR::Instruction *, TR::LabelSymbol *, TR::LabelSymbol *, int32_t) {
  TR_ASSERT(0,
            "unexpected call to "
            "OMR::CodeGenerator::apply16BitLoadLabelRelativeRelocation");
}
void OMR::CodeGenerator::apply32BitLoadLabelRelativeRelocation(
    TR::Instruction *, TR::LabelSymbol *, TR::LabelSymbol *, int32_t) {
  TR_ASSERT(0,
            "unexpected call to "
            "OMR::CodeGenerator::apply32BitLoadLabelRelativeRelocation");
}
void OMR::CodeGenerator::apply64BitLoadLabelRelativeRelocation(
    TR::Instruction *, TR::LabelSymbol *) {
  TR_ASSERT(0,
            "unexpected call to "
            "OMR::CodeGenerator::apply64BitLoadLabelRelativeRelocation");
}
void OMR::CodeGenerator::apply32BitLabelRelativeRelocation(int32_t *cursor,
                                                           TR::LabelSymbol *) {
  TR_ASSERT(0,
            "unexpected call to "
            "OMR::CodeGenerator::apply32BitLabelRelativeRelocation");
}
void OMR::CodeGenerator::apply32BitLabelTableRelocation(int32_t *cursor,
                                                        TR::LabelSymbol *) {
  TR_ASSERT(
      0,
      "unexpected call to OMR::CodeGenerator::apply32BitLabelTableRelocation");
}
...
bool OMR::CodeGenerator::useClobberEvaluate() {
  ...
  if (!self()->comp()->getOption(TR_DisableEnhancedClobberEval)) return false;
#endif
  return true;
}

bool OMR::CodeGenerator::opCodeIsNoOp(TR::ILOpCode &opCode) {
  if (TR::ILOpCode::isOpCodeAnImplicitNoOp(opCode)) return true;

  return self()->opCodeIsNoOpOnThisPlatform(opCode);
}

bool OMR::CodeGenerator::getTraceRAOption(uint32_t mask) {
  return self()->comp()->getOptions()->getTraceRAOption(mask);
}

void OMR::CodeGenerator::traceRAInstruction(TR::Instruction *instr) {
  const static char *traceEveryInstruction =
      feGetEnv("TR_traceEveryInstructionDuringRA");
  if (self()->getDebug())
    self()->getDebug()->traceRegisterAssignment(
        instr, true, traceEveryInstruction ? true : false);
}
...
void OMR::CodeGenerator::traceRegAssigned(TR::Register *virtReg,
                                          TR::Register *realReg) {
  if (self()->getDebug())
    self()->getDebug()->traceRegisterAssigned(_regAssignFlags, virtReg,
                                              realReg);
}
...
void OMR::CodeGenerator::traceRegFreed(TR::Register *virtReg,
                                       TR::Register *realReg) {
  if (self()->getDebug())
    self()->getDebug()->traceRegisterFreed(virtReg, realReg);
}

void OMR::CodeGenerator::traceRegInterference(TR::Register *virtReg,
                                              TR::Register *interferingVirtual,
                                              int32_t distance) {
  if (self()->getDebug())
    self()->getDebug()->traceRegisterInterference(virtReg, interferingVirtual,
                                                  distance);
}

void OMR::CodeGenerator::traceRegWeight(TR::Register *realReg,
                                        uint32_t weight) {
  if (self()->getDebug())
...
bool OMR::CodeGenerator::enableRefinedAliasSets() {
  ...
  return _enabledFlags.testAny(EnableRefinedAliasSets) &&
...
void OMR::CodeGenerator::generateCode() {
  ...
  TR::LexicalMemProfiler mp("Code Generation",
                            self()->comp()->phaseMemProfiler());
...
void OMR::CodeGenerator::removeUnusedLocals() {
  if (self()->comp()->getOption(TR_MimicInterpreterFrameShape)) return;
...
bool OMR::CodeGenerator::areAssignableGPRsScarce() {
  ...
  if (c1) threshold = atoi(c1);
  return (self()->getMaximumNumbersOfAssignableGPRs() <= threshold);
}

// returns true iff, based on opcodes and offsets if two store/load/address
// nodes are definitely disjoint (i.e. guaranteed not to overlap) any
// combination of stores,loads and address nodes are allowed as node1 and node2
// The return tells how node1 and node2 is overlapped (TR_StorageOverlapKind)
TR_StorageOverlapKind OMR::CodeGenerator::storageMayOverlap(TR::Node *node1,
                                                            size_t length1,
                                                            TR::Node *node2,
                                                            size_t length2) {
  if ((node2->getOpCode().isLoadVarOrStore() ||
       node2->getType().isAddress()) &&  // node1 is usually an always valid
                                         // store so check node2 first
    ...
    if (self()->traceBCDCodeGen())
      traceMsg(self()->comp(),
               "overlap=true : node1 %s (%p) and/or node2 %s (%p) are not "
               "valid load/store/address nodes\n",
...
int32_t OMR::CodeGenerator::defaultArrayTranslateMinimumNumberOfIterations(
    const char *methodName) {
  if (TR::CodeGenerator::useOldArrayTranslateMinimumNumberOfIterations())
    return INT_MAX;
  TR_ASSERT(
      false,
      "%s not implemented, platform codegen should choose an appropriate value",
      methodName);
  return 10001;
}
...
bool OMR::CodeGenerator::isAliasedGRN(TR_GlobalRegisterNumber n) {
  TR_ASSERT(
      self()->isGlobalVRF(n) || self()->isGlobalFPR(n),
  ...
  return n >= _firstOverlappedGlobalFPR &&
         n <= _lastOverlappedGlobalFPR;  // _lastOverlappedFPR includes
                                         // _lastOverlappingVector
}

TR_GlobalRegisterNumber OMR::CodeGenerator::getOverlappedAliasForGRN(
    TR_GlobalRegisterNumber n) {
  TR_ASSERT(self()->isAliasedGRN(n),
            "Can only get alias from supported types. Current grn : %d (%s)", n,
            self()->getDebug()->getGlobalRegisterName(n));
  ...
  if (n >= _firstOverlappedGlobalFPR &&
      n < _firstOverlappedGlobalVRF)  // n is a a float grn but not a vector grn
  {
    aliasedGrn = n + self()->getOverlapOffsetBetweenAliasedGRNs();
    TR_ASSERT(self()->isGlobalVRF(aliasedGrn),
              "Aliased FPR %d did not give an a valid VRF %d", n, aliasedGrn);
  } else if (n >= _firstOverlappedGlobalVRF &&
             n <=
                 _lastOverlappedGlobalVRF)  // n is a pure overlapped vector grn
  {
    aliasedGrn = n - self()->getOverlapOffsetBetweenAliasedGRNs();
    TR_ASSERT(self()->isGlobalVRF(aliasedGrn),
              "Aliased VRF %d did not give an a valid FPR %d", n, aliasedGrn);
  } else {
...
bool OMR::CodeGenerator::isSupportedAdd(TR::Node *addr) {
  if (addr->getOpCode().isAdd() &&
      (addr->getType().isAddress() || addr->getType().isInt32() ||
       addr->getType().isIntegral()))
    return true;
  else
    return false;
}

bool OMR::CodeGenerator::uniqueAddressOccurrence(
    TR::Node *addr1, TR::Node *addr2, bool addressesUnderSameTreeTop) {
  bool duringEvaluation = self()->getCodeGeneratorPhase() >
  ...
  if (duringEvaluation) {
    ...
    if (addr1->getReferenceCount() <=
            1 &&  // refCounts may be zero when called during ilgen and the
                  // nodes are freshly popped
...
bool OMR::CodeGenerator::nodeMatches(TR::Node *addr1, TR::Node *addr2,
                                     bool addressesUnderSameTreeTop) {
  bool foundMatch = false;
  if (addr1 == addr2) {
    foundMatch = true;
  } else if (addr1->getOpCodeValue() == TR::loadaddr &&
             addr2->getOpCodeValue() == TR::loadaddr &&
             addr1->getSymbolReference() == addr2->getSymbolReference()) {
    foundMatch = true;
  } else if (addr1->getType().isIntegral() &&
             addr1->getOpCode().isLoadConst() &&
             addr2->getType().isIntegral() &&
             addr2->getOpCode().isLoadConst() &&
             addr1->get64bitIntegralValue() == addr2->get64bitIntegralValue()) {
    foundMatch = true;
  } else if (addressesUnderSameTreeTop && addr1->getOpCodeValue() == TR::i2a &&
             addr2->getOpCodeValue() == TR::i2a &&
             addr1->getFirstChild()->getOpCode().isLoadVarDirect() &&
             addr1->getFirstChild()
                 ->getSymbolReference()
                 ->getSymbol()
                 ->isMethodMetaData() &&
             addr1->getFirstChild()->getOpCodeValue() ==
                 addr2->getFirstChild()->getOpCodeValue() &&
             addr1->getFirstChild()->getSize() ==
                 addr2->getFirstChild()->getSize() &&
             addr1->getFirstChild()->getSymbolReference() ==
                 addr2->getFirstChild()->getSymbolReference()) {
    // i2a
    //    iload _GPR13
    foundMatch = true;
  } else if (self()->uniqueAddressOccurrence(addr1, addr2,
                                             addressesUnderSameTreeTop)) {
    TR::ILOpCode op1 = addr1->getOpCode();
    TR::ILOpCode op2 = addr2->getOpCode();
    if (op1.getOpCodeValue() == op2.getOpCodeValue() && op1.isLoadVar() &&
        op1.getDataType() == TR::Address &&
        addr1->getSymbolReference() == addr2->getSymbolReference()) {
      // aload, ardbar etc
      if (op1.isLoadDirect()) foundMatch = true;
      // aloadi, ardbari etc
      else if (op1.isLoadIndirect() &&
               self()->nodeMatches(addr1->getFirstChild(),
                                   addr2->getFirstChild(),
                                   addressesUnderSameTreeTop))
        foundMatch = true;
    }
...
bool OMR::CodeGenerator::additionsMatch(TR::Node *addr1, TR::Node *addr2,
                                        bool addressesUnderSameTreeTop) {
  bool foundMatch = false;
  ...
  if (self()->nodeMatches(addr1ChildOne, addr2ChildOne,
                          addressesUnderSameTreeTop)) {
    if (self()->nodeMatches(addr1ChildTwo, addr2ChildTwo,
                            addressesUnderSameTreeTop)) {
      foundMatch = true;
    } else if (self()->isSupportedAdd(addr1ChildTwo) &&
               self()->isSupportedAdd(addr2ChildTwo) &&
               self()->additionsMatch(addr1ChildTwo, addr2ChildTwo,
                                      addressesUnderSameTreeTop)) {
      // aladd
      //    loadaddr
      //    ladd
  ...
  } else if (self()->nodeMatches(addr1ChildTwo, addr2ChildTwo,
                                 addressesUnderSameTreeTop) &&
             self()->isSupportedAdd(addr1ChildOne) &&
             self()->isSupportedAdd(addr2ChildOne) &&
             self()->additionsMatch(addr1ChildOne, addr2ChildOne,
                                    addressesUnderSameTreeTop)) {
    // aiadd
    //    aiadd                   // addr1ChildOne
    //       aload cached_static
    ...
    int64_t offsetOne =
        addr1ChildOne->getSecondChild()->get64bitIntegralValue() +
        addr1ChildTwo->get64bitIntegralValue();
    int64_t offsetTwo = addr2ChildTwo->get64bitIntegralValue();
    /*
          traceMsg(comp(),"adding offsets from %p and %p (%lld + %lld) and %p
       (%lld) (found base match %s %p)\n",
...
bool OMR::CodeGenerator::addressesMatch(TR::Node *addr1, TR::Node *addr2,
                                        bool addressesUnderSameTreeTop) {
  bool foundMatch = false;
    ...
    if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)) {
      if (self()->additionsMatch(addr1, addr2, addressesUnderSameTreeTop)) {
        foundMatch = true;
                 ...
                 self()->isSupportedAdd(addr1->getFirstChild()) &&
                 self()->isSupportedAdd(addr2->getFirstChild())) {
        bool firstChildrenMatch = self()->additionsMatch(
            addr1->getFirstChild(), addr2->getFirstChild(),
            addressesUnderSameTreeTop);
        bool secondChildrenMatch = false;

        if (firstChildrenMatch && addr1->getSecondChild() &&
            addr2->getSecondChild() &&
  ...
  if (!foundMatch && addressesUnderSameTreeTop) {
    bool duringEvaluation = self()->getCodeGeneratorPhase() >
                            TR::CodeGenPhase::SetupForInstructionSelectionPhase;
    // if the caller asserts the addresses are under the same treetop (so no
    // intervening kills are possible)
    //  and one of the mutually exclusive tests below are true then addresses
    //  will also match
    // 1) before evaluation : refCounts of both nodes are 1 (a more refined
    // test, that requires scanning ahead in the block, is if both are first
    // references) 2) during evaluation : registers are both NULL (i.e. both
    // first ref)

    if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2) &&
        self()->nodeMatches(addr1->getSecondChild(), addr2->getSecondChild(),
                            addressesUnderSameTreeTop)) {
      // for the case where the iaload itself is below an addition, match the
      // additions first aiadd
      //    iaload
      //       aload
      //    iconst
      ...
      if (traceInspect())
        traceMsg(comp(),
                 "\t\tfound possibly matching additions : update addr1 to %s "
                 "(%p), addr2 to %s (%p) and continue matching\n",
    ...
    if (addr1->getOpCodeValue() == TR::aloadi &&
        ...
        addr1->getSymbolReference() == addr2->getSymbolReference() &&
        self()->addressesMatch(addr1->getFirstChild(),
                               addr2->getFirstChild()) &&
        self()->uniqueAddressOccurrence(addr1, addr2,
                                        addressesUnderSameTreeTop)) {
      foundMatch = true;
    }
  }
...
void OMR::CodeGenerator::reserveCodeCache() {
  ...
  _codeCache = TR::CodeCacheManager::instance()->reserveCodeCache(
      false, 0, compThreadID, &numReserved);
  ...
  if (!_codeCache)  // Cannot reserve a cache; all are used
  {
    ...
    // If some code caches have some space but cannot be used because they are
    // reserved we will throw an exception in the call to
    // TR::CodeCacheManager::reserveCodeCache
    //
    if (comp->compileRelocatableCode()) {
      comp->failCompilation<TR::RecoverableCodeCacheError>(
          "Cannot reserve code cache");
    }
...
uint8_t *OMR::CodeGenerator::allocateCodeMemoryInner(
    uint32_t warmCodeSizeInBytes, uint32_t coldCodeSizeInBytes,
  ...
  uint8_t *warmCode = TR::CodeCacheManager::instance()->allocateCodeMemory(
      warmCodeSizeInBytes, coldCodeSizeInBytes, &codeCache, coldCode, false,
      isMethodHeaderNeeded);

  if (codeCache != self()->getCodeCache()) {
    // Either we didn't get a code cache, or the one we got should be reserved
    TR_ASSERT(!codeCache || codeCache->isReserved(),
              "Substitute code cache isn't marked as reserved");
  ...
  TR_ASSERT(!((warmCodeSizeInBytes && !warmCode) ||
              (coldCodeSizeInBytes && !coldCode)),
            "Allocation failed but didn't throw an exception");

  return warmCode;
}

uint8_t *OMR::CodeGenerator::allocateCodeMemory(uint32_t warmCodeSizeInBytes,
                                                uint32_t coldCodeSizeInBytes,
                                                uint8_t **coldCode,
                                                bool isMethodHeaderNeeded) {
  uint8_t *warmCode;
  warmCode = self()->allocateCodeMemoryInner(
      warmCodeSizeInBytes, coldCodeSizeInBytes, coldCode, isMethodHeaderNeeded);

  if (self()->getCodeGeneratorPhase() ==
      TR::CodeGenPhase::BinaryEncodingPhase) {
...
uint8_t *OMR::CodeGenerator::allocateCodeMemory(uint32_t codeSizeInBytes,
                                                bool isCold,
                                                bool isMethodHeaderNeeded) {
  uint8_t *coldCode;
  if (isCold) {
    self()->allocateCodeMemory(0, codeSizeInBytes, &coldCode,
                               isMethodHeaderNeeded);
    return coldCode;
  }
  return self()->allocateCodeMemory(codeSizeInBytes, 0, &coldCode,
                                    isMethodHeaderNeeded);
}
...
void OMR::CodeGenerator::trimCodeMemoryToActualSize() {
  ...
  self()->getCodeCache()->trimCodeMemoryAllocation(bufferStart,
                                                   actualCodeLengthInBytes);
}
...
bool OMR::CodeGenerator::convertMultiplyToShift(TR::Node *node) {
  ...
  //
  if (!secondChild->getOpCode().isLoadConst()) return false;
  ...
  if (secondChild->getOpCodeValue() == TR::lconst ||
    ...
    if (longMultiplier == 0) return false;  // Can't handle this case
    if (longMultiplier < 0) longMultiplier = -longMultiplier;
    ...
    if (multiplier == 0) return false;  // Can't handle this case
    if (multiplier < 0) multiplier = -multiplier;
  }

  if (!isNonNegativePowerOf2(multiplier) &&
      multiplier != TR::getMinSigned<TR::Int32>())
    return false;
...
bool OMR::CodeGenerator::isMemoryUpdate(TR::Node *node) {
  if (self()->comp()->getOption(TR_DisableDirectMemoryOps)) return false;
  ...
  if (valueChild->getNumChildren() != 2) return false;

  // Multiplies on x86 do not have direct memory forms.  This check is done here
  // rather than in the caller because this function can have the unexpected
  // side effect of swapping children of commutative operations in order to
  ...
  for (loadIndex = 0; loadIndex <= 1; loadIndex++) {
    ...
    if (!node->getOpCode().isIndirect())
      ...
      if (node->getFirstChild() == loadNode->getFirstChild()) break;
      // make sure the address nodes matches (aladd, etc.)
      if (self()->addressesMatch(node->getFirstChild(),
                                 loadNode->getFirstChild(), true))
        break;
    }
  }
  ...
  //
  if (loadIndex > 0) {
    if (!valueChild->getOpCode().isCommutative()) return false;
...
void OMR::CodeGenerator::compute32BitMagicValues(int32_t d, int32_t *m,
                                                 int32_t *s) {
  int p, first, last, mid;
  unsigned int ad, anc, delta, q1, r1, q2, r2, t;
  const unsigned int two31 = 2147483648u;

// Cache some common denominators and their magic values.  The key values in
// this array MUST be in numerically increasing order for the binary search to
// work.
#define NUM_32BIT_MAGIC_VALUES 11
  static uint32_t div32BitMagicValues[NUM_32BIT_MAGIC_VALUES][3] =
      ...
      {{3, 0x55555556, 0},    {5, 0x66666667, 1},    {7, 0x92492493, 2},
       {9, 0x38e38e39, 1},    {10, 0x66666667, 2},   {12, 0x2aaaaaab, 1},
       {24, 0x2aaaaaab, 2},   {60, 0x88888889, 5},   {100, 0x51eb851f, 5},
...
void OMR::CodeGenerator::compute64BitMagicValues(int64_t d, int64_t *m,
                                                 int64_t *s) {
  int p, first, last, mid;
...
uint64_t OMR::CodeGenerator::computeUnsigned64BitMagicValues(uint64_t d,
                                                             int32_t *s,
                                                             int32_t *a) {
  int p;
...
uint8_t *OMR::CodeGenerator::alignBinaryBufferCursor() {
  ...
  TR_ASSERT_FATAL(
      boundary > 0,
      "JIT method entry alignment boundary (%d) definition is violated",
      boundary);
  ...
  if (self()->supportsJitMethodEntryAlignment() && boundary > 1) {
    ...
    alignedBinaryBufferCursor = reinterpret_cast<uint8_t *>(OMR::align(
        reinterpret_cast<size_t>(alignedBinaryBufferCursor), boundary));

    TR_ASSERT_FATAL(
        OMR::aligned(reinterpret_cast<size_t>(alignedBinaryBufferCursor),
                     boundary),
        "alignedBinaryBufferCursor [%p] is not aligned to the specified "
        "boundary (%d)",
        alignedBinaryBufferCursor, boundary);

    alignedBinaryBufferCursor -= offset;

    uint32_t threshold = self()->getJitMethodEntryAlignmentThreshold();

    TR_ASSERT_FATAL(threshold <= boundary,
                    "JIT method entry alignment threshold (%d) definition is "
                    "violated as it is larger than the boundary (%d)",
                    threshold, boundary);

    if (alignedBinaryBufferCursor - _binaryBufferCursor <= threshold) {
      _binaryBufferCursor = alignedBinaryBufferCursor;
      self()->setJitMethodEntryPaddingSize(_binaryBufferCursor -
                                           _binaryBufferStart);
...
uint32_t OMR::CodeGenerator::getJitMethodEntryAlignmentThreshold() {
  return self()->getJitMethodEntryAlignmentBoundary();
}

int32_t OMR::CodeGenerator::setEstimatedLocationsForSnippetLabels(
  ...
  if (self()->hasDataSnippets()) {
    estimatedSnippetStart = self()->setEstimatedLocationsForDataSnippetLabels(
...
uint8_t *OMR::CodeGenerator::emitSnippets() {
  ...
  for (auto iterator = _snippetList.begin(); iterator != _snippetList.end();
    ...
    if (codeOffset != NULL) {
      TR_ASSERT((*iterator)->getLength(self()->getBinaryBufferCursor() -
                                       self()->getBinaryBufferStart()) +
                        self()->getBinaryBufferCursor() >=
                    codeOffset,
                "%s length estimate must be conservatively large (snippet "
                "@ " POINTER_PRINTF_FORMAT ", estimate=%d, actual=%d)",
                self()->getDebug()->getName(*iterator), *iterator,
                (*iterator)->getLength(self()->getBinaryBufferCursor() -
                                       self()->getBinaryBufferStart()),
...
TR::LabelSymbol *OMR::CodeGenerator::lookUpSnippet(
    int32_t snippetKind, TR::SymbolReference *symRef) {
  for (auto iterator = _snippetList.begin(); iterator != _snippetList.end();
       ++iterator) {
...
TR::SymbolReference *OMR::CodeGenerator::allocateLocalTemp(
    TR::DataType dt, bool isInternalPointer) {
  //
  TR::AutomaticSymbol *temp;
  if (isInternalPointer) {
    temp = TR::AutomaticSymbol::createInternalPointer(
        self()->trHeapMemory(), dt, TR::Symbol::convertTypeToSize(dt),
        self()->fe());
  } else {
    temp = TR::AutomaticSymbol::create(self()->trHeapMemory(), dt,
                                       TR::Symbol::convertTypeToSize(dt));
  }
  self()->comp()->getMethodSymbol()->addAutomatic(temp);
  return new (self()->trHeapMemory())
      TR::SymbolReference(self()->comp()->getSymRefTab(), temp);
}
...
int32_t OMR::CodeGenerator::getFirstBit(TR_BitVector &bv) {
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (self()->getGlobalRegister((TR_GlobalRegisterNumber)nextElmnt) !=
        (uint32_t)(-1)) {
      return nextElmnt;
    }
  }
...
void OMR::CodeGenerator::computeBlocksWithCalls() {
  ...
  _blocksWithCalls =
      new (self()->trHeapMemory()) TR_BitVector(bcount, self()->trMemory());
  ...
  for (pTree = self()->comp()->getStartTree(); pTree != NULL;
    ...
    if (bvec.isSet(btemp)) _blocksWithCalls->set(bnum);
  }
}

/**
From codegen/CodeGenerator.cpp to make the size of instructions to patch
smarter.
**/

bool OMR::CodeGenerator::areMergeableGuards(TR::Instruction *earlierGuard,
  ...
  return earlierGuard->isMergeableGuard() && laterGuard->isMergeableGuard() &&
         ...
         (!earlierGuard->getNode()->isStopTheWorldGuard() ||
          laterGuard->getNode()->isStopTheWorldGuard());
}

TR::Instruction *OMR::CodeGenerator::getVirtualGuardForPatching(
    TR::Instruction *vgdnop) {
  TR_ASSERT(vgdnop->isVirtualGuardNOPInstruction(),
            "getGuardForPatching called with non VirtualGuardNOPInstruction "
            "[%p] - this only works for guards!",
            vgdnop);

  if (!vgdnop->isMergeableGuard()) return vgdnop;

  // If there are no previous instructions the instruction must be the patch
  // point as there is nothing to merge with
  if (!vgdnop->getPrev()) return vgdnop;

  // Guard merging is only done when the guard trees are consecutive in treetop
  // order only separated by BBStart and BBEnd trees Skip back to the BBStart
  // since we need to get the block for vgdnop
  if (vgdnop->getPrev()->getNode()->getOpCodeValue() != TR::BBStart)
    return vgdnop;

  // there could be local RA generated reg-reg movs between guards so we resort
  // to checking the trees and making sure that the guards are in treetop order
  // we only merge blocks in the same extended blocks - virtual guard head
  // merger will arrange for this to happen when possible for guards
  TR::Instruction *toReturn = vgdnop;
  TR::Block *extendedBlockStart =
      vgdnop->getPrev()->getNode()->getBlock()->startOfExtendedBlock();
  for (TR::Instruction *prevI = vgdnop->getPrev(); prevI;
       prevI = prevI->getPrev()) {
    if (prevI->isVirtualGuardNOPInstruction()) {
      ...
      if (prevI->isMergeableGuard() &&
          prevI->getNode()->getBranchDestination() ==
              vgdnop->getNode()->getBranchDestination()) {
        // instruction tied to an acceptable guard so do nothing and continue
                 ...
                 prevI->getNode()->getBlock()->startOfExtendedBlock() ==
                     extendedBlockStart) {
        // we are in the same extension and haven't found any problematic
        // instructions yet so keep scanning
      } else {
        break;
      }
    }
  ...
  if (toReturn != vgdnop) {
    TR::DebugCounter::incStaticDebugCounter(
        self()->comp(),
        TR::DebugCounter::debugCounterName(self()->comp(), "guardMerge/(%s)",
                                           self()->comp()->signature()));
    if (self()->comp()->getOption(TR_TraceCG))
      traceMsg(self()->comp(),
               "vgdnop instruction [%p] begins scanning for patch instructions "
               "for mergeable guard [%p]\n",
               vgdnop, toReturn);
  }
  return toReturn;
}

TR::Instruction *OMR::CodeGenerator::getInstructionToBePatched(
    TR::Instruction *vgdnop) {
  TR::Instruction *nextI;
  TR::Node *firstBBEnd = NULL;

  for (nextI = self()->getVirtualGuardForPatching(vgdnop)->getNext();
       nextI != NULL; nextI = nextI->getNext()) {
    if (nextI->isVirtualGuardNOPInstruction()) {
      if (!self()->areMergeableGuards(vgdnop, nextI)) return NULL;
      continue;
    }
    ...
    if (nextI->getEstimatedBinaryLength() > 0) return nextI;
...
int32_t OMR::CodeGenerator::sizeOfInstructionToBePatched(
    TR::Instruction *vgdnop) {
...
int32_t OMR::CodeGenerator::sizeOfInstructionToBePatchedHCRGuard(
    TR::Instruction *vgdnop) {
  TR::Instruction *nextI;
  TR::Node *firstBBEnd = NULL;
  int32_t accumulatedSize = 0;

  for (nextI = self()->getInstructionToBePatched(vgdnop); nextI != NULL;
       nextI = nextI->getNext()) {
    if (nextI->isVirtualGuardNOPInstruction()) {
      if (!self()->areMergeableGuards(vgdnop, nextI)) break;
      continue;
    }

    if (nextI->isPatchBarrier() || self()->comp()->isPICSite(nextI)) break;

    accumulatedSize += nextI->getBinaryLengthLowerBound();

    if (accumulatedSize > nextI->getMaxPatchableInstructionLength()) break;
...
//
void OMR::CodeGenerator::addMonClass(TR::Node *monNode,
                                     TR_OpaqueClassBlock *clazz) {
  _monitorMapping.add(monNode);
  _monitorMapping.add(clazz);
}

TR_OpaqueClassBlock *OMR::CodeGenerator::getMonClass(TR::Node *monNode) {
  ...
  return 0;
      ...
      (defined(__IBMC__) || defined(__IBMCPP__) || defined(__ibmxl__)))
...
bool OMR::CodeGenerator::canNullChkBeImplicit(TR::Node *node) {
  return self()->canNullChkBeImplicit(node, false);
}

bool OMR::CodeGenerator::canNullChkBeImplicit(TR::Node *node, bool doChecks) {
  if (self()->comp()->getOption(TR_DisableTraps)) return false;

  if (!doChecks) return true;
  ...
  if (opCode.isLoadVar() ||
    ...
    if (symRef && symRef->getSymbol()->getOffset() + symRef->getOffset() <
                      self()->getNumberBytesReadInaccessible())
      return true;
  } else if (opCode.isStore()) {
    TR::SymbolReference *symRef = firstChild->getSymbolReference();
    if (symRef && symRef->getSymbol()->getOffset() + symRef->getOffset() <
                      self()->getNumberBytesWriteInaccessible())
      return true;
    ...
    return true;
             ...
             self()->getNumberBytesReadInaccessible() >
                 self()->fe()->getOffsetOfContiguousArraySizeField()) {
    return true;
  }
  return false;
}
...
TR::Instruction *OMR::CodeGenerator::generateDebugCounter(const char *name,
                                                          int32_t delta,
                                                          int8_t fidelity) {
...
TR::Instruction *OMR::CodeGenerator::generateDebugCounter(
    const char *name, TR::Register *deltaReg, int8_t fidelity) {
  return self()->generateDebugCounter(NULL, name, deltaReg, fidelity);
}

TR::Instruction *OMR::CodeGenerator::generateDebugCounter(
    TR::Instruction *cursor, const char *name, int32_t delta, int8_t fidelity,
    int32_t staticDelta) {
  if (!cursor) cursor = self()->getAppendInstruction();
  if (!self()->comp()->getOptions()->enableDebugCounters()) return cursor;
  ...
  TR::DebugCounterAggregation *aggregatedCounters =
      self()
          ...
          ->getPersistentInfo()
          ->getDynamicCounters()
          ->createAggregation(self()->comp(), name);
  aggregatedCounters->aggregateStandardCounters(
      self()->comp(), cursor->getNode(), name, delta, fidelity, staticDelta);
  if (!aggregatedCounters->hasAnyCounters()) return cursor;

  if (TR::DebugCounter::relocatableDebugCounter(self()->comp()) &&
      !aggregatedCounters->initializeReloData(self()->comp(), delta, fidelity,
                                              staticDelta))
    return cursor;

  TR::SymbolReference *symref =
      aggregatedCounters->getBumpCountSymRef(self()->comp());
...
TR::Instruction *OMR::CodeGenerator::generateDebugCounter(
  ...
  if (!cursor) cursor = self()->getAppendInstruction();
  if (!self()->comp()->getOptions()->enableDebugCounters()) return cursor;
  // We won't do any aggregation (histogram buckets, bytecode breakdown, etc.)
  // if we're getting the delta from a register
  TR::DebugCounter *counter = TR::DebugCounter::getDebugCounter(
      self()->comp(), name, fidelity, staticDelta);
  if (!counter) return cursor;
...
TR::Instruction *OMR::CodeGenerator::generateDebugCounter(
    const char *name, TR::RegisterDependencyConditions &cond, int32_t delta,
    int8_t fidelity, int32_t staticDelta, TR::Instruction *cursor) {
  if (!cursor) cursor = self()->getAppendInstruction();
  if (!self()->comp()->getOptions()->enableDebugCounters()) return cursor;
  if (delta == 0) return cursor;
  TR::DebugCounterAggregation *aggregatedCounters =
...
TR::Instruction *OMR::CodeGenerator::generateDebugCounter(
    ...
    TR::RegisterDependencyConditions &cond, int8_t fidelity,
    int32_t staticDelta, TR::Instruction *cursor) {
  if (!cursor) cursor = self()->getAppendInstruction();
  if (!self()->comp()->getOptions()->enableDebugCounters()) return cursor;
  // We won't do any aggregation (histogram buckets, bytecode breakdown, etc.)
  // if we're getting the delta from a register
...
TR::Instruction *OMR::CodeGenerator::generateDebugCounter(
    const char *name, TR_ScratchRegisterManager &srm, int32_t delta,
    int8_t fidelity, int32_t staticDelta, TR::Instruction *cursor) {
  if (!cursor) cursor = self()->getAppendInstruction();
  if (!self()->comp()->getOptions()->enableDebugCounters()) return cursor;
  if (delta == 0) return cursor;
...
void OMR::CodeGenerator::recordSingleRegisterUse(TR::Register *reg) {
  for (auto iter = self()->getReferencedRegisterList()->begin(),
            end = self()->getReferencedRegisterList()->end();
       iter != end; ++iter) {
    if ((*iter)->virtReg == reg) {
      (*iter)->useCount++;
      return;
  ...
  OMR::RegisterUsage *ru =
      new (self()->trHeapMemory()) OMR::RegisterUsage(reg, 1);
  self()->getReferencedRegisterList()->push_front(ru);
}

void OMR::CodeGenerator::startRecordingRegisterUsage() {
  self()->setReferencedRegisterList(
      new (self()->trHeapMemory()) TR::list<OMR::RegisterUsage *>(
          getTypedAllocator<OMR::RegisterUsage *>(
              self()->comp()->allocator())));
...
void OMR::CodeGenerator::addRelocation(TR::Relocation *r) {
  if (r->isExternalRelocation()) {
    TR_ASSERT(false,
              "Cannot use addRelocation to add an AOT relocation. Please use "
              "addExternalRelocation here");
  } else {
...
void OMR::CodeGenerator::addExternalRelocation(
    TR::Relocation *r, const char *generatingFileName,
    uintptr_t generatingLineNumber, TR::Node *node,
    TR::ExternalRelocationPositionRequest where) {
  TR_ASSERT(
      generatingFileName,
      "External relocation location has improper NULL filename specified");
  if (self()->comp()->compileRelocatableCode()) {
    TR::RelocationDebugInfo *genData =
        new (self()->trHeapMemory()) TR::RelocationDebugInfo;
...
void OMR::CodeGenerator::addExternalRelocation(
    TR::Relocation *r, TR::RelocationDebugInfo *info,
    TR::ExternalRelocationPositionRequest where) {
  if (self()->comp()->compileRelocatableCode()) {
    TR_ASSERT(info,
              "External relocation location does not have associated debug "
              "information");
    r->setDebugInfo(info);
    switch (where) {
      ...
      default:
        TR_ASSERT_FATAL(
            false, "invalid TR::ExternalRelocationPositionRequest %d", where);
        break;
    }
  }
}

void OMR::CodeGenerator::addStaticRelocation(
...
intptrj_t OMR::CodeGenerator::hiValue(intptrj_t address) {
  if (self()
          ->comp()
          ->compileRelocatableCode())  // We don't want to store values using
                                       // HI_VALUE at compile time, otherwise,
                                       // we do this a 2nd time when we relocate
                                       // (and new value is based on old one)
...
void OMR::CodeGenerator::addToOSRTable(int32_t instructionPC,
                                       TR_ByteCodeInfo &bcInfo) {
  self()->comp()->getOSRCompilationData()->addInstruction(instructionPC,
                                                          bcInfo);
}
...
void OMR::CodeGenerator::addAllocatedRegisterPair(TR::RegisterPair *temp) {
  ...
  } else  // for AR:GPR pairs and beyond
  {
    ...
    if (!temp->getLowOrder()->isLive())
      _liveRegisters[temp->getLowOrder()->getKind()]->addRegister(
          temp->getLowOrder());
    if (!temp->getHighOrder()->isLive())
      _liveRegisters[temp->getHighOrder()->getKind()]->addRegister(
          temp->getHighOrder());
  }
}
...
TR::RegisterPair *OMR::CodeGenerator::allocateRegisterPair(TR::Register *lo,
                                                           TR::Register *ho) {
  TR::RegisterPair *temp =
      new (self()->trHeapMemory()) TR::RegisterPair(lo, ho);
  self()->addAllocatedRegisterPair(temp);
  return temp;
}

TR::RegisterPair *OMR::CodeGenerator::allocateSinglePrecisionRegisterPair(
    TR::Register *lo, TR::Register *ho) {
  TR::RegisterPair *temp =
      new (self()->trHeapMemory()) TR::RegisterPair(TR_FPR);
...
TR::RegisterPair *OMR::CodeGenerator::allocateFloatingPointRegisterPair(
    TR::Register *lo, TR::Register *ho) {
  TR::RegisterPair *temp =
      new (self()->trHeapMemory()) TR::RegisterPair(lo, ho);
  temp->setKind(TR_FPR);
...
bool OMR::CodeGenerator::constantAddressesCanChangeSize(TR::Node *node) {
  if (!self()->comp()->compileRelocatableCode() ||
      TR::Compiler->target.is32Bit() || node == NULL)
    return false;
...
bool OMR::CodeGenerator::isInMemoryInstructionCandidate(TR::Node *node) {
  ...
  //     bconst
  //
  // On Z systems, this can use an OI instruction, which is an SI form
  // instruction.
  ...
  // callers of this function should make sure that the opcode on valueChild
  // corresponds to a valid in-memory instruction on the target instruction set

  if (valueChild->getNumChildren() == 0) {
    return false;
  }

  // whatever is under the valueChild has to be indirect too, and its address
  // must match the store's address

  if (valueChild->getFirstChild()->getOpCode().isLoadVar() &&
      valueChild->getFirstChild()->getOpCode().isIndirect() &&
      self()->addressesMatch(node->getChild(0),
                             valueChild->getFirstChild()->getFirstChild(),
                             true)) {
  } else {
    return false;
  }

  // in-memory instruction use only applies if ref count == 1 and the value
  // exists only in memory (not loaded into reg). this restriction applies to:
  //
...
TR::Linkage *OMR::CodeGenerator::createLinkageForCompilation() {
  return self()->getLinkage(TR_System);
}

TR::TreeTop *OMR::CodeGenerator::lowerTree(TR::Node *root, TR::TreeTop *tt) {
  TR_ASSERT(0,
            "Unimplemented lowerTree() called for an opcode that needs to be "
            "lowered");
  return NULL;
}
 ...
 *
 * This method can be used to emit a prefetch instruction. By default it does
 * nothing. It can be overwritten to set the conditions for the generation of
 * the prefetch instruction.
 */
void OMR::CodeGenerator::insertPrefetchIfNecessary(
...
void OMR::CodeGenerator::switchCodeCacheTo(TR::CodeCache *newCodeCache) {
  ...
  TR_ASSERT(oldCodeCache != newCodeCache,
            "Attempting to switch to the currently held code cache");
  ...
  if (self()->committedToCodeCache() || !newCodeCache) {
    ...
    if (newCodeCache) {
      comp->failCompilation<TR::RecoverableCodeCacheError>(
          "Already committed to current code cache");
    }

    comp->failCompilation<TR::CodeCacheError>(
        "Already committed to current code cache");
  }
...
void OMR::CodeGenerator::redoTrampolineReservationIfNecessary(
  ...
  TR_ASSERT_FATAL(instructionSymRef,
                  "Expecting instruction to have a SymbolReference");

  /**
   * Determine the callee symbol reference based on the target of the call
   * instruction
   */
  TR::LabelSymbol *labelSymbol =
      instructionSymRef->getSymbol()->getLabelSymbol();
  ...
  TR_ASSERT_FATAL(calleeSymRef != NULL,
                  "Missing possible re-reservation for trampolines");

  if (calleeSymRef->getReferenceNumber() >= TR_numRuntimeHelpers) {
    self()->fe()->reserveTrampolineIfNecessary(self()->comp(), calleeSymRef,
                                               true);
  }
}
