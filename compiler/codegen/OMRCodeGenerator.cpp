...
TR::Instruction*
OMR::CodeGenerator::generateNop(TR::Node* node,
...
OMR::CodeGenerator::lowerTrees()
{
  ...
  TR::Delimiter d(
    self()->comp(), self()->comp()->getOption(TR_TraceCG), "LowerTrees");
...
void
OMR::CodeGenerator::lowerTreesWalk(TR::Node* parent,
                                   TR::TreeTop* treeTop,
                                   vcount_t visitCount)
{
...
void
OMR::CodeGenerator::lowerTreesPreTreeTopVisit(TR::TreeTop* tt,
                                              vcount_t visitCount)
{
...
void
OMR::CodeGenerator::lowerTreesPostTreeTopVisit(TR::TreeTop* tt,
                                               vcount_t visitCount)
{}

void
OMR::CodeGenerator::lowerTreesPostChildrenVisit(TR::Node* parent,
                                                TR::TreeTop* treeTop,
                                                vcount_t visitCount)
{}
  ...
  for (TR::TreeTop* tt = self()->comp()->getStartTree(); tt;
    ...
    if (node->getNumChildren() >= 1 &&
      ...
      if (checklist.contains(callNode)) {
        if (comp->getOption(TR_TraceCG)) {
          traceMsg(comp,
                   "Skipping previously visited call node %d\n",
    ...
    for (TR::TreeTop* tt = comp->getStartTree(); tt;
      ...
      if (comp->getOption(TR_TraceCG) || debug("traceGRA")) {
        ...
        if (tt == self()->getCurrentEvaluationTreeTop()) {
          traceMsg(comp, "------------------------------\n");
          self()->getDebug()->dumpSingleTreeWithInstrs(
            tt, prevInstr->getNext(), true, true, true, false);
        } else {
          ...
          for (TR::TreeTop* dumptt = tt;
            ...
            traceMsg(comp, "\n");
            self()->getDebug()->dumpSingleTreeWithInstrs(
              dumptt, NULL, true, false, true, false);
          }
...
void
OMR::CodeGenerator::addSymbolAndDataTypeToMap(TR::Symbol* symbol,
                                              TR::DataType dt)
{
...
void
OMR::CodeGenerator::startUsingRegister(TR::Register* reg)
{
...
void
OMR::CodeGenerator::stopUsingRegister(TR::Register* reg)
{
  if (reg != NULL && _liveRegisters[reg->getKind()])
...
bool
OMR::CodeGenerator::canClobberNodesRegister(TR::Node* node,
                                            uint16_t count,
...
void
OMR::CodeGenerator::traceBCDEntry(char* str, TR::Node* node)
{
...
void
OMR::CodeGenerator::traceBCDExit(char* str, TR::Node* node)
{
  if (self()->traceBCDCodeGen())
...
TR::SymbolReference*
OMR::CodeGenerator::getSymRef(TR_RuntimeHelper h)
{
...
TR::Linkage*
OMR::CodeGenerator::createLinkage(TR_LinkageConventions lc)
{
  TR_UNIMPLEMENTED();
  return NULL;
}
...
bool
OMR::CodeGenerator::mulDecompositionCostIsJustified(int numOfOperations,
                                                    char bitPosition[],
                                                    char operationType[],
                                                    int64_t value)
{
...
void
OMR::CodeGenerator::apply8BitLabelRelativeRelocation(int32_t* cursor,
                                                     TR::LabelSymbol* label)
{
  *(int8_t*)cursor += (int8_t)(intptrj_t)label->getCodeLocation();
}
void
OMR::CodeGenerator::apply12BitLabelRelativeRelocation(int32_t* cursor,
                                                      TR::LabelSymbol* label,
                                                      bool isCheckDisp)
{
  TR_ASSERT(
    0,
    "unexpected call to OMR::CodeGenerator::apply12BitLabelRelativeRelocation");
}
void
OMR::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t* cursor,
                                                      TR::LabelSymbol* label)
{
...
void
OMR::CodeGenerator::apply24BitLabelRelativeRelocation(int32_t* cursor,
                                                      TR::LabelSymbol*)
{
  TR_ASSERT(
    0,
    "unexpected call to OMR::CodeGenerator::apply24BitLabelRelativeRelocation");
}
void
OMR::CodeGenerator::apply16BitLoadLabelRelativeRelocation(TR::Instruction*,
                                                          TR::LabelSymbol*,
                                                          TR::LabelSymbol*,
                                                          int32_t)
{
...
void
OMR::CodeGenerator::apply32BitLoadLabelRelativeRelocation(TR::Instruction*,
                                                          TR::LabelSymbol*,
                                                          TR::LabelSymbol*,
                                                          int32_t)
{
...
void
OMR::CodeGenerator::apply64BitLoadLabelRelativeRelocation(TR::Instruction*,
                                                          TR::LabelSymbol*)
{
  TR_ASSERT(0,
            "unexpected call to "
...
void
OMR::CodeGenerator::apply32BitLabelRelativeRelocation(int32_t* cursor,
                                                      TR::LabelSymbol*)
{
  TR_ASSERT(
    0,
    "unexpected call to OMR::CodeGenerator::apply32BitLabelRelativeRelocation");
}
void
OMR::CodeGenerator::apply32BitLabelTableRelocation(int32_t* cursor,
                                                   TR::LabelSymbol*)
{
  TR_ASSERT(
    0, "unexpected call to OMR::CodeGenerator::apply32BitLabelTableRelocation");
}

void
OMR::CodeGenerator::addSnippet(TR::Snippet* s)
{
  _snippetList.push_back(s);
}

void
OMR::CodeGenerator::setCurrentBlock(TR::Block* b)
{
  _currentBlock = b;
...
bool
OMR::CodeGenerator::getTraceRAOption(uint32_t mask)
{
  return self()->comp()->getOptions()->getTraceRAOption(mask);
}

void
OMR::CodeGenerator::traceRAInstruction(TR::Instruction* instr)
{
...
void
OMR::CodeGenerator::tracePreRAInstruction(TR::Instruction* instr)
{
  if (self()->getDebug())
...
void
OMR::CodeGenerator::tracePostRAInstruction(TR::Instruction* instr)
{
  if (self()->getDebug())
...
void
OMR::CodeGenerator::traceRegAssigned(TR::Register* virtReg,
                                     TR::Register* realReg)
{
  if (self()->getDebug())
    self()->getDebug()->traceRegisterAssigned(
      _regAssignFlags, virtReg, realReg);
}
...
void
OMR::CodeGenerator::traceRegFreed(TR::Register* virtReg, TR::Register* realReg)
{
  if (self()->getDebug())
...
void
OMR::CodeGenerator::traceRegInterference(TR::Register* virtReg,
                                         TR::Register* interferingVirtual,
                                         int32_t distance)
{
  if (self()->getDebug())
    self()->getDebug()->traceRegisterInterference(
      virtReg, interferingVirtual, distance);
}

void
OMR::CodeGenerator::traceRegWeight(TR::Register* realReg, uint32_t weight)
{
  if (self()->getDebug())
...
void
OMR::CodeGenerator::traceRegisterAssignment(const char* format, ...)
{
  if (self()->getDebug()) {
    va_list args;
...
OMR::CodeGenerator::storageMayOverlap(TR::Node* node1,
                                      size_t length1,
                                      TR::Node* node2,
                                      size_t length2)
{
...
int32_t
OMR::CodeGenerator::defaultArrayTranslateMinimumNumberOfIterations(
...
OMR::CodeGenerator::isAliasedGRN(TR_GlobalRegisterNumber n)
{
  TR_ASSERT(
    ...
    "Global register number is of incorrect type. Current type: %d (%s)",
    n,
...
OMR::CodeGenerator::getOverlappedAliasForGRN(TR_GlobalRegisterNumber n)
{
  TR_ASSERT(self()->isAliasedGRN(n),
            "Can only get alias from supported types. Current grn : %d (%s)",
            n,
            self()->getDebug()->getGlobalRegisterName(n));
      ...
      n < _firstOverlappedGlobalVRF) // n is a a float grn but not a vector grn
  {
    ...
    TR_ASSERT(self()->isGlobalVRF(aliasedGrn),
              "Aliased FPR %d did not give an a valid VRF %d",
              n,
              aliasedGrn);
             ...
             n <= _lastOverlappedGlobalVRF) // n is a pure overlapped vector grn
  {
    aliasedGrn = n - self()->getOverlapOffsetBetweenAliasedGRNs();
    TR_ASSERT(self()->isGlobalVRF(aliasedGrn),
              "Aliased VRF %d did not give an a valid FPR %d",
              n,
              aliasedGrn);
  } else {
...
bool
OMR::CodeGenerator::uniqueAddressOccurrence(TR::Node* addr1,
                                            TR::Node* addr2,
...
bool
OMR::CodeGenerator::nodeMatches(TR::Node* addr1,
                                TR::Node* addr2,
                                bool addressesUnderSameTreeTop)
{
  ...
  if (addr1 == addr2) {
    ...
    //    iload _GPR13
    foundMatch = true;
  } else if (self()->uniqueAddressOccurrence(
               addr1, addr2, addressesUnderSameTreeTop)) {
...
bool
OMR::CodeGenerator::additionsMatch(TR::Node* addr1,
                                   TR::Node* addr2,
                                   bool addressesUnderSameTreeTop)
{
  bool foundMatch = false;
  ...
  if (self()->nodeMatches(
        addr1ChildOne, addr2ChildOne, addressesUnderSameTreeTop)) {
    if (self()->nodeMatches(
          addr1ChildTwo, addr2ChildTwo, addressesUnderSameTreeTop)) {
      foundMatch = true;
...
bool
OMR::CodeGenerator::addressesMatch(TR::Node* addr1,
                                   TR::Node* addr2,
                                   bool addressesUnderSameTreeTop)
{
  bool foundMatch = false;
    ...
    if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)) {
      if (self()->additionsMatch(addr1, addr2, addressesUnderSameTreeTop)) {
        ...
        bool firstChildrenMatch =
          self()->additionsMatch(addr1->getFirstChild(),
                                 addr2->getFirstChild(),
                                 addressesUnderSameTreeTop);
        ...
        if (firstChildrenMatch && addr1->getSecondChild() &&
          ...
          secondChildrenMatch =
            self()->additionsMatch(addr1->getSecondChild(),
                                   addr2->getSecondChild(),
                                   addressesUnderSameTreeTop);
        }
  ...
  if (!foundMatch && addressesUnderSameTreeTop) {
    ...
    if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2) &&
        self()->nodeMatches(addr1->getSecondChild(),
                            addr2->getSecondChild(),
                            addressesUnderSameTreeTop)) {
...
uint8_t*
OMR::CodeGenerator::allocateCodeMemoryInner(uint32_t warmCodeSizeInBytes,
                                            ...
                                            bool isMethodHeaderNeeded)
{
  ...
  uint8_t* warmCode =
    TR::CodeCacheManager::instance()->allocateCodeMemory(warmCodeSizeInBytes,
                                                         coldCodeSizeInBytes,
                                                         &codeCache,
                                                         coldCode,
                                                         false,
  ...
  TR_ASSERT(
    !((warmCodeSizeInBytes && !warmCode) || (coldCodeSizeInBytes && !coldCode)),
...
uint8_t*
OMR::CodeGenerator::allocateCodeMemory(uint32_t warmCodeSizeInBytes,
                                       uint32_t coldCodeSizeInBytes,
                                       uint8_t** coldCode,
                                       bool isMethodHeaderNeeded)
{
...
uint8_t*
OMR::CodeGenerator::allocateCodeMemory(uint32_t codeSizeInBytes,
                                       bool isCold,
                                       bool isMethodHeaderNeeded)
{
  uint8_t* coldCode;
  if (isCold) {
    self()->allocateCodeMemory(
      0, codeSizeInBytes, &coldCode, isMethodHeaderNeeded);
    return coldCode;
  }
  return self()->allocateCodeMemory(
    codeSizeInBytes, 0, &coldCode, isMethodHeaderNeeded);
}
...
OMR::CodeGenerator::isMemoryUpdate(TR::Node* node)
{
  ...
  for (loadIndex = 0; loadIndex <= 1; loadIndex++) {
    ...
    if (!node->getOpCode().isIndirect())
      ...
      if (self()->addressesMatch(
            node->getFirstChild(), loadNode->getFirstChild(), true))
        break;
    }
  }
...
void
armCodeSync(uint8_t* start, uint32_t size);
#elif defined(TR_HOST_ARM64)
void
arm64CodeSync(uint8_t* start, uint32_t size);
#elif defined(TR_HOST_POWER)
void
ppcCodeSync(uint8_t* start, uint32_t size);
#else
...
void
OMR::CodeGenerator::compute32BitMagicValues(int32_t d, int32_t* m, int32_t* s)
{
...
void
OMR::CodeGenerator::compute64BitMagicValues(int64_t d, int64_t* m, int64_t* s)
{
  int p, first, last, mid;
...
uint64_t
OMR::CodeGenerator::computeUnsigned64BitMagicValues(uint64_t d,
                                                    int32_t* s,
                                                    int32_t* a)
{
  int p;
  ...
  if (self()->supportsJitMethodEntryAlignment() && boundary > 1) {
    ...
    TR_ASSERT_FATAL(
      OMR::aligned(reinterpret_cast<size_t>(alignedBinaryBufferCursor),
                   boundary),
      "alignedBinaryBufferCursor [%p] is not aligned to the specified boundary "
      "(%d)",
...
int32_t
OMR::CodeGenerator::setEstimatedLocationsForSnippetLabels(
  int32_t estimatedSnippetStart)
{
  ...
  if (self()->hasDataSnippets()) {
    estimatedSnippetStart =
      self()->setEstimatedLocationsForDataSnippetLabels(estimatedSnippetStart);
  }
  ...
  for (auto iterator = _snippetList.begin(); iterator != _snippetList.end();
    ...
    if (codeOffset != NULL) {
      TR_ASSERT((*iterator)->getLength(self()->getBinaryBufferCursor() -
                ...
                self()->getDebug()->getName(*iterator),
                *iterator,
...
OMR::CodeGenerator::allocateLocalTemp(TR::DataType dt, bool isInternalPointer)
{
  ...
  if (isInternalPointer) {
    ...
    temp = TR::AutomaticSymbol::create(
...
bool
OMR::CodeGenerator::areMergeableGuards(TR::Instruction* earlierGuard,
  ...
  if (toReturn != vgdnop) {
    TR::DebugCounter::incStaticDebugCounter(
      ...
      TR::DebugCounter::debugCounterName(
        self()->comp(), "guardMerge/(%s)", self()->comp()->signature()));
    if (self()->comp()->getOption(TR_TraceCG))
      traceMsg(self()->comp(),
...
int32_t
OMR::CodeGenerator::sizeOfInstructionToBePatchedHCRGuard(
...
int32_t
leadingZeroes(int32_t inputWord)
{
...
int32_t
leadingZeroes(int64_t inputWord)
{
  uint64_t byteMask;
  uint32_t bitCount;
...
bool
OMR::CodeGenerator::isILOpCodeSupported(TR::ILOpCodes o)
{
...
TR::Instruction*
OMR::CodeGenerator::generateDebugCounter(const char* name,
                                         int32_t delta,
                                         int8_t fidelity)
{
                                         ...
                                         int32_t staticDelta)
{
  ...
  if (TR::DebugCounter::relocatableDebugCounter(self()->comp()) &&
      !aggregatedCounters->initializeReloData(
        self()->comp(), delta, fidelity, staticDelta))
    return cursor;
...
//
void
OMR::CodeGenerator::recordSingleRegisterUse(TR::Register* reg)
{
...
void
OMR::CodeGenerator::addRelocation(TR::Relocation* r)
{
...
void
OMR::CodeGenerator::addExternalRelocation(
  TR::Relocation* r,
...
intptrj_t
OMR::CodeGenerator::hiValue(intptrj_t address)
{
  if (self()
        ->comp()
        ->compileRelocatableCode()) // We don't want to store values using
                                    // HI_VALUE at compile time, otherwise, we
                                    // do this a 2nd time when we relocate (and
                                    // new value is based on old one)
...
void
OMR::CodeGenerator::addToOSRTable(TR::Instruction* instr)
{
  self()->comp()->getOSRCompilationData()->addInstruction(instr);
}

void
OMR::CodeGenerator::addToOSRTable(int32_t instructionPC,
...
void
OMR::CodeGenerator::addAllocatedRegisterPair(TR::RegisterPair* temp)
{
...
void
OMR::CodeGenerator::addAllocatedRegister(TR::Register* temp)
{
  uint32_t idx = _registerArray.add(temp);
  temp->setIndex(idx);
...
TR::RegisterPair*
OMR::CodeGenerator::allocateRegisterPair(TR::Register* lo, TR::Register* ho)
{
...
// Z
bool
OMR::CodeGenerator::AddArtificiallyInflatedNodeToStack(TR::Node* n)
{
...
OMR::CodeGenerator::lowerTree(TR::Node* root, TR::TreeTop* tt)
{
  TR_ASSERT(
    0,
    "Unimplemented lowerTree() called for an opcode that needs to be lowered");
  return NULL;
}
 ...
 */
void
OMR::CodeGenerator::insertPrefetchIfNecessary(TR::Node* node,
...
void
OMR::CodeGenerator::redoTrampolineReservationIfNecessary(
  ...
  TR::SymbolReference* instructionSymRef)
{
  ...
  if (calleeSymRef->getReferenceNumber() >= TR_numRuntimeHelpers) {
    self()->fe()->reserveTrampolineIfNecessary(
      self()->comp(), calleeSymRef, true);
  }
}
