...
OMR::CodeGenerator::CodeGenerator()
    : _compilation(TR::comp()), _trMemory(_compilation->trMemory()),
      _liveLocals(0), _currentEvaluationTreeTop(0), _currentEvaluationBlock(0),
      _prePrologueSize(0), _implicitExceptionPoint(0),
      _localsThatAreStored(NULL), _numLocalsWhenStoreAnalysisWasDone(-1),
      _ialoadUnneeded(self()->comp()->trMemory()),
      _symRefTab(self()->comp()->getSymRefTab()), _vmThreadRegister(NULL),
      _stackAtlas(NULL), _methodStackMap(NULL), _binaryBufferStart(NULL),
      _binaryBufferCursor(NULL), _largestOutgoingArgSize(0),
      _estimatedCodeLength(0), _estimatedSnippetStart(0),
      _accumulatedInstructionLengthError(0), _registerSaveDescription(0),
      _extendedToInt64GlobalRegisters(self()->comp()->allocator()),
      _liveButMaybeUnreferencedLocals(NULL), _assignedGlobalRegisters(NULL),
      _aheadOfTimeCompile(NULL), _globalRegisterTable(NULL),
      _currentGRABlockLiveOutSet(NULL), _localsIG(NULL), _lastGlobalGPR(0),
      _firstGlobalFPR(0), _lastGlobalFPR(0), _firstOverlappedGlobalFPR(0),
      _lastOverlappedGlobalFPR(0), _last8BitGlobalGPR(0),
      _globalGPRPartitionLimit(0), _globalFPRPartitionLimit(0),
      _firstInstruction(NULL), _appendInstruction(NULL), _firstGlobalVRF(-1),
      _lastGlobalVRF(-1), _firstOverlappedGlobalVRF(-1),
      _lastOverlappedGlobalVRF(-1), _overlapOffsetBetweenFPRandVRFgrns(0),
      _supportedLiveRegisterKinds(0),
      _monitorMapping(self()->comp()->trMemory(), 256), _blocksWithCalls(NULL),
      _codeCache(0), _committedToCodeCache(false), _codeCacheSwitched(false),
      _blockRegisterPressureCache(NULL), _simulatedNodeStates(NULL),
      ...
      _internalPointerSpillFreeList(
          getTypedAllocator<TR_BackingStore *>(TR::comp()->allocator())),
      _spilledRegisterList(NULL), _referencedRegistersList(NULL),
      ...
      _preJitMethodEntrySize(0), _jitMethodEntryPaddingSize(0),
      _lastInstructionBeforeCurrentEvaluationTreeTop(NULL),
      _unlatchedRegisterList(NULL), _indentation(2), _currentBlock(NULL),
      _realVMThreadRegister(NULL), _internalControlFlowNestingDepth(0),
      ...
      _stackOfMemoryReferencesCreatedDuringEvaluation(
          self()->comp() ? self()->comp()->trMemory() : 0, 16),
      randomizer(self()->comp()), _outOfLineColdPathNestedDepth(0),
      _codeGenPhase(self()), _symbolDataTypeMap(self()->comp()->allocator()),
      _lmmdFailed(false) {
...
int32_t
OMR::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Block *block) {
...
TR::Register *
OMR::CodeGenerator::allocateSinglePrecisionRegister(TR_RegisterKinds rk) {
...
void OMR::CodeGenerator::apply12BitLabelRelativeRelocation(
  ...
  TR_ASSERT(0, "unexpected call to "
...
int32_t
OMR::CodeGenerator::arrayTranslateMinimumNumberOfElements(bool isByteSource,
                                                          bool isByteTarget) {
...
OMR::CodeGenerator::getOverlappedAliasForGRN(TR_GlobalRegisterNumber n) {
      ...
      n < _firstOverlappedGlobalVRF) // n is a a float grn but not a vector grn
  {
    ...
    TR_ASSERT(self()->isGlobalVRF(aliasedGrn),
             ...
             n <= _lastOverlappedGlobalVRF) // n is a pure overlapped vector grn
  {
    aliasedGrn = n - self()->getOverlapOffsetBetweenAliasedGRNs();
    TR_ASSERT(self()->isGlobalVRF(aliasedGrn),
...
bool OMR::CodeGenerator::uniqueAddressOccurrence(
  ...
  if (duringEvaluation) {
    ...
    if (addr1->getReferenceCount() <=
            1 && // refCounts may be zero when called during ilgen and the nodes
                 // are freshly popped
...
TR::LabelSymbol *
OMR::CodeGenerator::lookUpSnippet(int32_t snippetKind,
...
TR::Instruction *
OMR::CodeGenerator::getVirtualGuardForPatching(TR::Instruction *vgdnop) {
...
TR::Instruction *
OMR::CodeGenerator::generateDebugCounter(TR::Instruction *cursor,
...
void OMR::CodeGenerator::addRelocation(TR::Relocation *r) {
  if (r->isExternalRelocation()) {
    TR_ASSERT(false, "Cannot use addRelocation to add an AOT relocation. "
                     "Please use addExternalRelocation here");
  } else {
...
void OMR::CodeGenerator::addExternalRelocation(
    ...
    TR::ExternalRelocationPositionRequest where) {
  if (self()->comp()->compileRelocatableCode()) {
    TR_ASSERT(info, "External relocation location does not have associated "
                    "debug information");
    r->setDebugInfo(info);
    switch (where) {
    ...
    default:
      TR_ASSERT_FATAL(false, "invalid TR::ExternalRelocationPositionRequest %d",
                      where);
      break;
    }
  }
...
intptrj_t OMR::CodeGenerator::hiValue(intptrj_t address) {
  if (self()
          ...
          ->compileRelocatableCode()) // We don't want to store values using
                                      // HI_VALUE at compile time, otherwise, we
                                      // do this a 2nd time when we relocate
...
TR::RegisterPair *
OMR::CodeGenerator::allocateSinglePrecisionRegisterPair(TR::Register *lo,
                                                        TR::Register *ho) {
  TR::RegisterPair *temp =
...
TR::RegisterPair *
OMR::CodeGenerator::allocateFloatingPointRegisterPair(TR::Register *lo,
                                                      TR::Register *ho) {
  TR::RegisterPair *temp =
      new (self()->trHeapMemory()) TR::RegisterPair(lo, ho);
  temp->setKind(TR_FPR);
...
TR::TreeTop *OMR::CodeGenerator::lowerTree(TR::Node *root, TR::TreeTop *tt) {
  TR_ASSERT(0, "Unimplemented lowerTree() called for an opcode that needs to "
               "be lowered");
  return NULL;
}
