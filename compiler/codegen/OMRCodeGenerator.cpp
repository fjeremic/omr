...
static_assert(TR::NumIlOps == (sizeof(OMR::CodeGenerator::_nodeToInstrEvaluators) / sizeof(OMR::CodeGenerator::_nodeToInstrEvaluators[0])),
    ...
    , _lmmdFailed(false)
{
    ...
    _disableInternalPointers = self()->comp()->getOption(TR_MimicInterpreterFrameShape) || self()->comp()->getOptions()->realTimeGC() || self()->comp()->getOption(TR_DisableInternalPointers);
    ...
    for (tt = self()->comp()->getStartTree(); tt; prev = tt, tt = tt->getNextTreeTop()) {
        ...
        if (opcode.getOpCodeValue() == TR::BBStart) {
            ...
            if (node->getBranchDestination()->getNode()->getLabel() == NULL) {
                ...
                TR::LabelSymbol* label = TR::LabelSymbol::create(self()->trHeapMemory(), self(), node->getBranchDestination()->getNode()->getBlock());
    ...
    for (TR::TreeTop* tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (node->getNumChildren() >= 1 && node->getFirstChild()->getOpCode().isFunctionCall()) {
            ...
            for (uint32_t i = 0; i < callNode->getNumChildren(); ++i) {
                ...
                if (paramNode->getReferenceCount() > 1 && paramNode->getOpCode().isLoadConst() && !self()->isMaterialized(paramNode)) {
...
bool OMR::CodeGenerator::mulDecompositionCostIsJustified(int numOfOperations, char bitPosition[], char operationType[], int64_t value)
{
...
bool OMR::CodeGenerator::nodeMatches(TR::Node* addr1, TR::Node* addr2, bool addressesUnderSameTreeTop)
{
    ...
    if (addr1 == addr2) {
        foundMatch = true;
    } else if (addr1->getOpCodeValue() == TR::loadaddr && addr2->getOpCodeValue() == TR::loadaddr && addr1->getSymbolReference() == addr2->getSymbolReference()) {
        foundMatch = true;
    } else if (addr1->getType().isIntegral() && addr1->getOpCode().isLoadConst() && addr2->getType().isIntegral() && addr2->getOpCode().isLoadConst() && addr1->get64bitIntegralValue() == addr2->get64bitIntegralValue()) {
        foundMatch = true;
    } else if (addressesUnderSameTreeTop && addr1->getOpCodeValue() == TR::i2a && addr2->getOpCodeValue() == TR::i2a && addr1->getFirstChild()->getOpCode().isLoadVarDirect() && addr1->getFirstChild()->getSymbolReference()->getSymbol()->isMethodMetaData() && addr1->getFirstChild()->getOpCodeValue() == addr2->getFirstChild()->getOpCodeValue() && addr1->getFirstChild()->getSize() == addr2->getFirstChild()->getSize() && addr1->getFirstChild()->getSymbolReference() == addr2->getFirstChild()->getSymbolReference()) {
        // i2a
        //    iload _GPR13
        foundMatch = true;
        ...
        if (op1.getOpCodeValue() == op2.getOpCodeValue() && op1.isLoadVar() && op1.getDataType() == TR::Address && addr1->getSymbolReference() == addr2->getSymbolReference()) {
            // aload, ardbar etc
            if (op1.isLoadDirect())
                foundMatch = true;
            // aloadi, ardbari etc
...
bool OMR::CodeGenerator::additionsMatch(TR::Node* addr1, TR::Node* addr2, bool addressesUnderSameTreeTop)
{
    ...
    if (self()->nodeMatches(addr1ChildOne, addr2ChildOne, addressesUnderSameTreeTop)) {
        if (self()->nodeMatches(addr1ChildTwo, addr2ChildTwo, addressesUnderSameTreeTop)) {
            foundMatch = true;
        } else if (self()->isSupportedAdd(addr1ChildTwo) && self()->isSupportedAdd(addr2ChildTwo) && self()->additionsMatch(addr1ChildTwo, addr2ChildTwo, addressesUnderSameTreeTop)) {
            // aladd
            //    loadaddr
            //    ladd
            //       x
    ...
    } else if (self()->nodeMatches(addr1ChildTwo, addr2ChildTwo, addressesUnderSameTreeTop) && self()->isSupportedAdd(addr1ChildOne) && self()->isSupportedAdd(addr2ChildOne) && self()->additionsMatch(addr1ChildOne, addr2ChildOne, addressesUnderSameTreeTop)) {
        // aiadd
        //    aiadd                   // addr1ChildOne
        //       aload cached_static
        //       iconst x
        ...
        foundMatch = true;
    } else if (self()->isSupportedAdd(addr1ChildOne) && addr1ChildOne->getFirstChild() == addr2ChildOne && addr1ChildOne->getSecondChild()->getOpCode().isLoadConst() && addr1ChildTwo->getOpCode().isLoadConst() && addr2ChildTwo->getOpCode().isLoadConst()) {
        // aladd
        //    aladd       addr1ChildOne
        //       aload
        //       lconst x
...
bool OMR::CodeGenerator::addressesMatch(TR::Node* addr1, TR::Node* addr2, bool addressesUnderSameTreeTop)
{
    ...
    if (self()->nodeMatches(addr1, addr2, addressesUnderSameTreeTop)) {
        ...
        if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)) {
            if (self()->additionsMatch(addr1, addr2, addressesUnderSameTreeTop)) {
                foundMatch = true;
            } else if (addr1->getFirstChild() && addr2->getFirstChild() && self()->isSupportedAdd(addr1->getFirstChild()) && self()->isSupportedAdd(addr2->getFirstChild())) {
                ...
                if (firstChildrenMatch && addr1->getSecondChild() && addr2->getSecondChild() && self()->isSupportedAdd(addr1->getSecondChild()) && self()->isSupportedAdd(addr2->getSecondChild())) {
    ...
    if (!foundMatch && addressesUnderSameTreeTop) {
        ...
        if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2) && self()->nodeMatches(addr1->getSecondChild(), addr2->getSecondChild(), addressesUnderSameTreeTop)) {
        ...
        if (addr1->getOpCodeValue() == TR::aloadi && addr2->getOpCodeValue() == TR::aloadi && addr1->getSymbolReference() == addr2->getSymbolReference() && self()->addressesMatch(addr1->getFirstChild(), addr2->getFirstChild()) && self()->uniqueAddressOccurrence(addr1, addr2, addressesUnderSameTreeTop)) {
            foundMatch = true;
        }
    }
...
bool OMR::CodeGenerator::isMemoryUpdate(TR::Node* node)
{
    ...
    for (loadIndex = 0; loadIndex <= 1; loadIndex++) {
        ...
        //
        if (!loadNode->getOpCode().isLoadVar() || loadNode->getSymbol() != node->getSymbol() || loadNode->getSymbolReference()->getOffset() != node->getSymbolReference()->getOffset()) {
            continue;
        }
...
void OMR::CodeGenerator::compute32BitMagicValues(
    int32_t d,
    int32_t* m,
    int32_t* s)
{
...
void OMR::CodeGenerator::compute64BitMagicValues(
    int64_t d,
    int64_t* m,
    int64_t* s)
{
...
TR::Instruction* OMR::CodeGenerator::getVirtualGuardForPatching(TR::Instruction* vgdnop)
{
    ...
    for (TR::Instruction* prevI = vgdnop->getPrev(); prevI; prevI = prevI->getPrev()) {
        if (prevI->isVirtualGuardNOPInstruction()) {
            ...
            if (prevI->isMergeableGuard() && prevI->getNode()->getBranchDestination() == vgdnop->getNode()->getBranchDestination()) {
                // instruction tied to an acceptable guard so do nothing and continue
            } else if ((prevI->getNode()->getOpCodeValue() == TR::BBStart || prevI->getNode()->getOpCodeValue() == TR::BBEnd) && prevI->getNode()->getBlock()->startOfExtendedBlock() == extendedBlockStart) {
...
TR::Instruction* OMR::CodeGenerator::getInstructionToBePatched(TR::Instruction* vgdnop)
{
    ...
    for (nextI = self()->getVirtualGuardForPatching(vgdnop)->getNext(); nextI != NULL; nextI = nextI->getNext()) {
        ...
        if (node->getOpCodeValue() == TR::BBEnd) {
            if (firstBBEnd == NULL)
                firstBBEnd = node;
            else if (firstBBEnd != node && (node->getBlock()->getNextBlock() == NULL || !node->getBlock()->getNextBlock()->isExtensionOfPreviousBlock()))
                return NULL;
        }
...
//
void OMR::CodeGenerator::addMonClass(TR::Node* monNode, TR_OpaqueClassBlock* clazz)
{
...
bool OMR::CodeGenerator::canNullChkBeImplicit(TR::Node* node, bool doChecks)
{
    ...
    if (opCode.isLoadVar() || (TR::Compiler->target.is64Bit() && opCode.getOpCodeValue() == TR::l2i)) {
        ...
        if (symRef && symRef->getSymbol()->getOffset() + symRef->getOffset() < self()->getNumberBytesWriteInaccessible())
            return true;
    } else if (opCode.isCall() && opCode.isIndirect() && self()->getNumberBytesReadInaccessible() > TR::Compiler->om.offsetOfObjectVftField()) {
        return true;
    } else if (opCode.getOpCodeValue() == TR::iushr && self()->getNumberBytesReadInaccessible() > self()->fe()->getOffsetOfContiguousArraySizeField()) {
        return true;
    }
    return false;
}
...
{
    return (_nodeToInstrEvaluators[o] != TR::TreeEvaluator::unImpOpEvaluator) && (_nodeToInstrEvaluators[o] != TR::TreeEvaluator::badILOpEvaluator);
}
...
bool OMR::CodeGenerator::isInMemoryInstructionCandidate(TR::Node* node)
{
    ...
    if (valueChild->getFirstChild()->getOpCode().isLoadVar() && valueChild->getFirstChild()->getOpCode().isIndirect() && self()->addressesMatch(node->getChild(0), valueChild->getFirstChild()->getFirstChild(), true)) {
    } else {
        return false;
    }
    ...
    if (node->getFirstChild()->isSingleRefUnevaluated() && valueChild->isSingleRefUnevaluated() && valueChild->getFirstChild()->isSingleRefUnevaluated()) {
    } else {
        return false;
    }
 ...
 */
void OMR::CodeGenerator::insertPrefetchIfNecessary(TR::Node* node, TR::Register* targetRegister)
{
    return;
}
...
void OMR::CodeGenerator::redoTrampolineReservationIfNecessary(TR::Instruction* callInstr, TR::SymbolReference* instructionSymRef)
{
