...
class TR_LiveRegisterInfo {
 public:
...
#if defined(TR_TARGET_POWER)
  uint64_t removeAssociation(TR::Compilation *compilation,
                             TR_RegisterMask realRegMask)
#else
  uint32_t removeAssociation(TR::Compilation *compilation,
                             TR_RegisterMask realRegMask)
#endif
  {
    if (realRegMask && (_association & realRegMask)) {
  ...
  void addByteRegisterInterference() {
    if (!(_association & 0x80000000)) _interference |= 0x80000000;
  }
...
class TR_LiveRegisters {
 public:
  ...
  TR_LiveRegisters(TR::Compilation *comp)
      : _compilation(comp), _head(NULL), _pool(NULL), _numLiveRegisters(0) {}

  TR_LiveRegisterInfo *addRegister(TR::Register *reg,
                                   bool updateInterferences = true);
  ...
  static void moveRegToList(TR_LiveRegisters *from, TR_LiveRegisters *to,
                            TR::Register *reg);
