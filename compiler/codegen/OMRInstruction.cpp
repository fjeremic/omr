...
Instruction::Instruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                         TR::Node *node)
    : _opcode(op), _next(0), _prev(cg->getAppendInstruction()), _node(node),
      _binaryEncodingBuffer(0), _binaryLength(0), _estimatedBinaryLength(0),
      _cg(cg), _liveLocals(0), _liveMonitors(0), _gc() {
...
TR::Instruction *Instruction::move(TR::Instruction *newLocation) {
  ...
  // Update the append instruction if we are moving this instruction to the
...
bool Instruction::requiresAtomicPatching() {
...
bool Instruction::isMergeableGuard() {
  ...
  return mergeOnlyHCRGuards ? self()->getNode()->isStopTheWorldGuard()
                            : self()->getNode()->isNopableInlineGuard();
}
} // namespace OMR
