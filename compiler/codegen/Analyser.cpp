...
void TR_Analyser::setInputs(TR::Node *firstChild, TR::Register *firstRegister,
                            TR::Node *secondChild, TR::Register *secondRegister,
                            bool nonClobberingDestination,
                            bool dontClobberAnything, TR::Compilation *comp,
  ...
  TR::SymbolReference *vftPointerSymRef =
      TR::comp()->getSymRefTab()->element(TR::SymbolReferenceTable::vftSymbol);
  ...
  if (!dontClobberAnything) {
    if (!nonClobberingDestination) {
      ...
      // set both operands to be clobberable because know that instruction
      // doesn't actually clobber destination operand so want to make either one
      // a possible no-cost destination operand.
      setClob1();
      setClob2();
    }
  }
