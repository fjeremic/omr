...
class OMR_EXTENSIBLE CodeGenerator {
 ...
 public:
  ...
  void lowerTrees();
  void lowerTreesWalk(TR::Node* parent,
                      TR::TreeTop* treeTop,
                      vcount_t visitCount);

  void lowerTreeIfNeeded(TR::Node* node,
                         int32_t childNumber,
                         TR::Node* parent,
                         TR::TreeTop* tt);
  ...
  void lowerTreesPreChildrenVisit(TR::Node* parent,
                                  TR::TreeTop* treeTop,
                                  vcount_t visitCount);
  void lowerTreesPostChildrenVisit(TR::Node* parent,
                                   TR::TreeTop* treeTop,
                                   vcount_t visitCount);
  ...
  void identifyUnneededByteConvNodes(TR::Node*,
                                     TR::TreeTop*,
                                     vcount_t,
                                     TR::DataType);
  ...
  void processReference(TR::Node* reference,
                        TR::Node* parent,
                        TR::TreeTop* treeTop);
  ...
  void needSpillTemp(TR_LiveReference* cursor,
                     TR::Node* parent,
                     TR::TreeTop* treeTop);
  ...
  bool canClobberNodesRegister(TR::Node* node,
                               uint16_t count = 1,
  ...
  bool mulDecompositionCostIsJustified(int numOfOperations,
                                       char bitPosition[],
                                       char operationType[],
                                       int64_t value);
   ...
   */
  uint8_t* allocateCodeMemory(uint32_t codeSizeInBytes,
                              bool isCold,
  ...
  bool allowGlobalRegisterAcrossBranch(TR_RegisterCandidate*,
                                       TR::Node* branchNode);
  void removeUnavailableRegisters(TR_RegisterCandidate* rc,
                                  TR::Block** blocks,
                                  TR_BitVector& availableRegisters) {}
  ...
  void traceRegAssigned(TR::Register* virtReg,
                        TR::Register* realReg,
  ...
  void traceRegInterference(TR::Register* virtReg,
                            TR::Register* interferingVirtual,
                            int32_t distance);
  ...
  TR::list<TR::Register*>* setSpilledRegisterList(TR::list<TR::Register*>* r) {
  ...
  TR_BackingStore* allocateSpill(bool containsCollectedReference,
                                 int32_t* offset,
                                 bool reuse = true);
  TR_BackingStore* allocateSpill(int32_t size,
  ...
  void addExternalRelocation(TR::Relocation* r,
  ...
  bool isSnippetMatched(TR::Snippet* snippet,
                        int32_t snippetKind,
                        TR::SymbolReference* symRef) {
    return false;
  }
  ...
  // Register pressure
  //
  void estimateRegisterPressure(TR::Node* node,
                                int32_t& registerPressure,
                                int32_t& maxRegisterPressure,
                                int32_t maxRegisters,
                                TR_BitVector* valuesInGlobalRegs,
                                bool isCold,
                                vcount_t visitCount,
                                TR::SymbolReference* symRef,
                                bool& symRefIsLive,
                                bool checkForIMuls,
                                bool& vmThreadUsed);
  int32_t estimateRegisterPressure(TR::Block* block,
                                   vcount_t visitCount,
...
#include "codegen/RegisterPressureSimulatorInner.hpp"
 ...
 public:
  ...
  TR::Instruction* generateNop(TR::Node* node,
                               TR::Instruction* instruction = 0,
  ...
  bool nodeMatches(TR::Node* addr1,
                   TR::Node* addr2,
                   bool addressesUnderSameTreeTop = false);
  bool additionsMatch(TR::Node* addr1,
                      TR::Node* addr2,
                      bool addressesUnderSameTreeTop = false);
  bool addressesMatch(TR::Node* addr1,
                      TR::Node* addr2,
                      bool addressesUnderSameTreeTop = false);
  // Z codegen
  bool uniqueAddressOccurrence(TR::Node* addr1,
                               TR::Node* addr2,
                               bool addressesUnderSameTreeTop);

  TR_StorageOverlapKind storageMayOverlap(TR::Node* node1,
                                          size_t length1,
                                          TR::Node* node2,
                                          size_t length2);
