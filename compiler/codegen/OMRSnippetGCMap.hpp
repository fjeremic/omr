...
public:
  ...
  TR_GCStackMap* getStackMap()
  {
    return _stackMap;
  }
  void setStackMap(TR_GCStackMap* m)
  {
    _stackMap = m;
  }

  uint32_t getGCRegisterMask()
  {
    return _GCRegisterMask;
  }
  void setGCRegisterMask(uint32_t regMask)
  {
    _GCRegisterMask = regMask;
  }

  bool isGCSafePoint()
  {
    return _flags.testAll(TO_MASK8(GCSafePoint));
  }
  void setGCSafePoint()
  {
    _flags.set(TO_MASK8(GCSafePoint));
  }
  void resetGCSafePoint()
  {
    _flags.reset(TO_MASK8(GCSafePoint));
  }
