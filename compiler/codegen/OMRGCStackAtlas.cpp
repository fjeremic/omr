...
TR::GCStackAtlas *OMR::GCStackAtlas::self() {
      ...
      while (mapEntry) {
        ...
        if (m->getLowestCodeOffset() == map->getLowestCodeOffset() &&
            ...
            ((m->getLiveMonitorBits() != 0) ==
             ...
             (m->getLiveMonitorBits() == 0 ||
              !memcmp(m->getLiveMonitorBits(), map->getLiveMonitorBits(),
                      mapBytes))) &&
...
uint32_t OMR::GCStackAtlas::getNumberOfDistinctPinningArrays() {
  ...
  if (self()->getInternalPointerMap()) {
    ...
    ListIterator<TR_InternalPointerPair> internalPtrIt(
        &self()->getInternalPointerMap()->getInternalPointerPairs());

    for (TR_InternalPointerPair *internalPtrPair = internalPtrIt.getFirst();
         internalPtrPair; internalPtrPair = internalPtrIt.getNext()) {
      bool seenPinningArrayBefore = false;
      ListIterator<TR_InternalPointerPair> seenInternalPtrIt(
          &seenInternalPtrPairs);
      for (TR_InternalPointerPair *seenInternalPtrPair =
               seenInternalPtrIt.getFirst();
    ...
    //
    ListIterator<TR::AutomaticSymbol> autoIt(
        &self()->getPinningArrayPtrsForInternalPtrRegs());
    TR::AutomaticSymbol *autoSymbol;
    for (autoSymbol = autoIt.getFirst(); autoSymbol;
         autoSymbol = autoIt.getNext()) {
