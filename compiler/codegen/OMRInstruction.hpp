...
public:
  ...
  {
    return _opcode.getMnemonic();
  }
  void setOpCodeValue(TR::InstOpCode::Mnemonic op)
  {
    _opcode.setMnemonic(op);
  }

  /*
   * Instruction stream links.
   */
  TR::Instruction* getNext()
  {
    return _next;
  }
  void setNext(TR::Instruction* n)
  {
    _next = n;
  }

  TR::Instruction* getPrev()
  {
    return _prev;
  }
  void setPrev(TR::Instruction* p)
  {
    _prev = p;
  }
  ...
  uint8_t getBinaryLength()
  {
    return _binaryLength;
  }
  void setBinaryLength(uint8_t length)
  {
   ...
   */
  uint8_t getEstimatedBinaryLength()
  {
    return _estimatedBinaryLength;
  }
  void setEstimatedBinaryLength(uint8_t e)
  {
    _estimatedBinaryLength = e;
  }
...
#include "codegen/InstructionKindEnum.hpp"
  ...
  bool needsGCMap()
  {
    return (_index & TO_MASK(NeedsGCMapBit)) != 0;
  }
  ...
  bool needsAOTRelocation()
  {
    return (_index & TO_MASK(NeedsAOTRelocation)) != 0;
  }
  ...
  TR_GCStackMap* getGCMap()
  {
    return _gc._GCMap;
  }
  TR_GCStackMap* setGCMap(TR_GCStackMap* map)
  {
    return (_gc._GCMap = map);
  }
  TCollectableReferenceMask getGCRegisterMask()
  {
    return _gc._GCRegisterMask;
  }
  ...
  TR_BitVector* getLiveMonitors()
  {
    return _liveMonitors;
  }
  TR_BitVector* setLiveMonitors(TR_BitVector* v)
  {
    return (_liveMonitors = v);
  }

  bool requiresAtomicPatching();

  int32_t getMaxPatchableInstructionLength()
  {
    return 0;
  }
