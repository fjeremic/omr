...
class OMR_EXTENSIBLE Instruction {
 ...
 public:
  ...
  /*
   * Address of binary buffer where this instruction was encoded.  The binary
   * buffer is NULL if this instruction has not been encoded yet.
   */
   ...
   *
   * @returns the last instruction in the expansion or this instruction if no
   * expansion was performed.
   */
  virtual TR::Instruction *expandInstruction() { return self(); }

  /*
   * Assign the specified register kind(s) on this instruction to real
   * registers.
   */
  ...
  /*
   * A 24-bit value indicating an approximate ordering of instructions.  Note
   * that for an instruction 'I1' followed by an instruction 'I2' it is not
   * guaranteed that I2.index > I1.index, though it is usually true.
   */
...
#include "codegen/InstructionKindEnum.hpp"
  ...
  void setNeedsAOTRelocation(bool v = true) {
    v ? _index |= TO_MASK(NeedsAOTRelocation)
      : _index &= ~TO_MASK(NeedsAOTRelocation);
  }
...
#include "codegen/InstructionFlagEnum.hpp"
  ...
  static_assert(LastBaseFlag <= MaxBaseFlag + 1,
                "OMR::Instruction too many flag bits for flag width");
