...
void
TR_LiveRegisters::moveRegToList(TR_LiveRegisters* from,
                                TR_LiveRegisters* to,
                                TR::Register* reg)
{
...
TR_LiveRegisters::addRegister(TR::Register* reg, bool updateInterferences)
{
  ...
  if (debug("traceLiveRegisters")) {
    diagnostic(
      ...
      entry,
      reg,
      cg()->getCurrentEvaluationTreeTop()->getNode(),
      reg->getKind());
  }
#endif
  ...
  if (regPair) {
    ...
    if (regPair->getLowOrder()->isLive() &&
      ...
      if (cg()->getLiveRegisters(regPair->getLowOrder()->getKind()))
        cg()
          ->getLiveRegisters(regPair->getLowOrder()->getKind())
    ...
    if (regPair->getHighOrder()->isLive() &&
      ...
      if (cg()->getLiveRegisters(regPair->getHighOrder()->getKind()))
        cg()
          ->getLiveRegisters(regPair->getHighOrder()->getKind())
