...
void TR_LiveRegisters::moveRegToList(TR_LiveRegisters *from,
                                     TR_LiveRegisters *to, TR::Register *reg) {
  from->removeRegisterFromLiveList(reg);
  reg->getLiveRegisterInfo()->addToList(to->_head);
  to->_numLiveRegisters++;
}

TR_LiveRegisterInfo *TR_LiveRegisters::addRegister(TR::Register *reg,
                                                   bool updateInterferences) {
  ...
  if (debug("traceLiveRegisters")) {
    diagnostic(
        "Add live register %p for %p while processing node %p, kind %d\n",
        entry, reg, cg()->getCurrentEvaluationTreeTop()->getNode(),
        reg->getKind());
  }
#endif
...
TR_LiveRegisterInfo *TR_LiveRegisters::addRegisterPair(TR::RegisterPair *reg) {
  ...
  if (!reg->getLowOrder()->isLive()) addRegister(reg->getLowOrder());
  if (!reg->getHighOrder()->isLive()) addRegister(reg->getHighOrder());
...
void TR_LiveRegisters::setAssociation(TR::Register *reg,
                                      TR::RealRegister *realReg) {
  if (!reg->isLive()) return;
  ...
  for (TR_LiveRegisterInfo *p = _head; p; p = p->getNext()) {
    if (p != liveReg) p->addInterference(liveReg->getAssociation());
  }
}
...
void TR_LiveRegisters::setByteRegisterAssociation(TR::Register *reg) {
  ...
  if (reg->isLive()) liveReg->addByteRegisterAssociation();

  // Mark all current live virtual registers as interfering with a byte
  // register.
  //
  for (TR_LiveRegisterInfo *p = _head; p; p = p->getNext()) {
    if (p != liveReg) p->addByteRegisterInterference();
  }
}

void TR_LiveRegisters::stopUsingRegister(TR::Register *reg) {
  if (reg->isLive()) {
    if (reg->getLiveRegisterInfo()->getNodeCount() == 0) registerIsDead(reg);
  }
}

void TR_LiveRegisters::registerIsDead(TR::Register *reg,
                                      bool updateInterferences) {
  ...
  if (regPair) {
    // For a register pair, it may be the case that we've killed one of its
    // components after its last reference to reduce interferences.  Hence, we
    // can relax the liveness constraint for register pair components.
    //
    if (regPair->getLowOrder()->isLive() &&
        (regPair->getLowOrder()->getLiveRegisterInfo()->getNodeCount() == 0)) {
  ...
  if (debug("traceLiveRegisters")) {
    ...
    diagnostic(
        "Remove live register %p, interference = %08x%08x, real reg = %08x, "
        "kind %d\n",
        reg, highBits, lowerBits, liveReg->getAssociation(), reg->getKind());
#else
    diagnostic(
        "Remove live register %p, interference = %08x, real reg = %08x\n", reg,
        reg->getInterference(), liveReg->getAssociation());
#endif
  }
