...
public:
  ...
  virtual bool isExternalRelocation()
  {
    return true;
  }
...
public:
  ...
  TR::LabelSymbol* setLabel(TR::LabelSymbol* l)
  {
    return (_label = l);
  }
...
class LabelRelative16BitRelocation : public TR::LabelRelocation {
  ...
  //
  // The regular TR::LabelRelative16BitRelocation(uint8_t *p, TR::LabelSymbol *l) only supports instructions with 16-bit
  // relocations, where the relocation is 2 bytes (hardcoded value) form the start of the instruction
...
public:
  ...
  bool isInstructionOffset()
  {
    return _instructionOffset;
  }
  bool setInstructionOffset(bool b)
  {
    return (_instructionOffset = b);
  }
  int8_t getAddressDifferenceDivisor()
  {
    return _addressDifferenceDivisor;
  }
...
public:
  InstructionAbsoluteRelocation(
    uint8_t* updateLocation, TR::Instruction* i, bool useEndAddr) /* specified if the start or
...
public:
  ...
  TR::LabelSymbol* getEndLabel()
  {
    return _endLabel;
  }
  TR::LabelSymbol* setEndLabel(TR::LabelSymbol* l)
  {
    return (_endLabel = l);
  }

  int32_t setDeltaToStartLabel(int32_t d)
  {
    return (_deltaToStartLabel = d);
  }
  int32_t getDeltaToStartLabel()
  {
    return _deltaToStartLabel;
  }
...
public:
  ...
  IteratedExternalRelocation(
    uint8_t* target, TR_ExternalRelocationTargetKind k, flags8_t modifier, TR::CodeGenerator* codeGen);
  IteratedExternalRelocation(uint8_t* target, uint8_t* target2, TR_ExternalRelocationTargetKind k, flags8_t modifier,
    TR::CodeGenerator* codeGen);

  uint32_t getNumberOfRelocationSites()
  {
    return _numberOfRelocationSites;
  }
  ...
  uint8_t* setTargetAddress(uint8_t* p)
  {
    return (_targetAddress = p);
  }

  uint8_t* getTargetAddress2()
  {
    return _targetAddress2;
  }

  uint8_t* getRelocationData()
  {
    return _relocationData;
  }
  uint8_t* setRelocationData(uint8_t* p)
  {
    return (_relocationData = p);
  }

  uint8_t* getRelocationDataCursor()
  {
    return _relocationDataCursor;
  }
  ...
  uint16_t getSizeOfRelocationData()
  {
    return _sizeOfRelocationData;
  }
  ...
  void setOrderedPair()
  {
    _recordModifier.set(RELOCATION_TYPE_ORDERED_PAIR);
  }

  uint8_t getModifierValue()
  {
    return _recordModifier.getValue();
  }
  void setModifierValue(uint8_t v)
  {
    _recordModifier.setValue(0x00, v);
  }
  ...
  TR_ExternalRelocationTargetKind getTargetKind()
  {
    return _kind;
  }
  TR_ExternalRelocationTargetKind setTargetKind(TR_ExternalRelocationTargetKind k)
  {
    return (_kind = k);
  }
...
public:
  ...
  ExternalRelocation(uint8_t* p, uint8_t* target, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* codeGen)
    : TR::Relocation(p)
  ...
  ExternalRelocation(
    uint8_t* p, uint8_t* target, uint8_t* target2, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* codeGen)
    : TR::Relocation(p)
    , _targetAddress(target)
  ...
  virtual uint32_t getNarrowSize()
  {
    return 2;
  }
  virtual uint32_t getWideSize()
  {
    return 4;
  }
...
public:
  ...
  uint8_t* getLocation2()
  {
    return _update2Location;
  }
  void setLocation2(uint8_t* l)
  {
    _update2Location = l;
  }
...
public:
  ...
  BeforeBinaryEncodingExternalRelocation(
    TR::Instruction* instr, uint8_t* target, TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* codeGen)
  ...
  BeforeBinaryEncodingExternalRelocation(TR::Instruction* instr, uint8_t* target, uint8_t* target2,
    TR_ExternalRelocationTargetKind kind, TR::CodeGenerator* codeGen)
  ...
  TR::Instruction* getUpdateInstruction()
  {
    return _instruction;
  }
};
