...
#define AOTcgDiag0(comp, s)                                                    \
  if (AOTcgDiagOn)                                                             \
  dumpOptDetails(comp, s)
#define AOTcgDiag1(comp, s, p1)                                                \
  if (AOTcgDiagOn)                                                             \
  dumpOptDetails(comp, s, p1)
#define AOTcgDiag2(comp, s, p1, p2)                                            \
  if (AOTcgDiagOn)                                                             \
  dumpOptDetails(comp, s, p1, p2)
#define AOTcgDiag3(comp, s, p1, p2, p3)                                        \
  if (AOTcgDiagOn)                                                             \
  dumpOptDetails(comp, s, p1, p2, p3)
#define AOTcgDiag4(comp, s, p1, p2, p3, p4)                                    \
  if (AOTcgDiagOn)                                                             \
  dumpOptDetails(comp, s, p1, p2, p3, p4)
#define AOTcgDiag5(comp, s, p1, p2, p3, p4, p5)                                \
  if (AOTcgDiagOn)                                                             \
  dumpOptDetails(comp, s, p1, p2, p3, p4, p5)
#define AOTcgDiag6(comp, s, p1, p2, p3, p4, p5, p6)                            \
  if (AOTcgDiagOn)                                                             \
  dumpOptDetails(comp, s, p1, p2, p3, p4, p5, p6)
#else
...
typedef enum {
  ...
  // start with 1, as there is a check in PPC preventing 0 valued immediates
...
public:
  ...
  LabelRelative12BitRelocation(uint8_t *p, TR::LabelSymbol *l,
...
class LabelRelative16BitRelocation : public TR::LabelRelocation {
  ...
  // instruction.
  // Eg.: BranchPreload instruction BPP (48-bit instruction), where 16-bit
  // relocation is at 32-47 bits, TR::LabelRelative16BitRelocation(*p,*l, 4,
  // true)
  //
  // The regular TR::LabelRelative16BitRelocation(uint8_t *p, TR::LabelSymbol
  // *l) only supports instructions with 16-bit relocations, where the
  // relocation is 2 bytes (hardcoded value) form the start of the instruction
...
public:
  ...
  LabelRelative16BitRelocation(uint8_t *p, TR::LabelSymbol *l, int8_t divisor,
                               bool isInstOffset = false)
      : TR::LabelRelocation(p, l), _addressDifferenceDivisor(divisor),
  ...
  int8_t setAddressDifferenceDivisor(int8_t d) {
    return (_addressDifferenceDivisor = d);
  }
...
/** \brief
 *
 *  Represents a 16-bit relocation from an offset into the binary encoding
 * location of an instruction to a label by a specified width (bytes,
 * half-words, words, double-words, etc.).
 */
class InstructionLabelRelative16BitRelocation : public TR::LabelRelocation {
public:
  /** \brief
   *     Initializes the InstructionLabelRelative16BitRelocation relocation
   * using a \c NULL base target pointer.
   *
   *  \param cursor
   *     The instruction whose binary encoding location will be used for the
   * relocation.
   *
   *  \param offset
   *     The offset from the binary encoding of \p cursor where the relocation
   * will be encoded
   *
   *  \param l
   ...
   *
   *  \param divisor
   *     The width of each unit of the encoding. For example if \p divisor is 2
   * then the value which will be encoded at \p cursor + \p offset will be the
   * number of half-words between \p cursor and \p l
   */
...
/** \brief
 *
 *  Represents a 32-bit relocation from an offset into the binary encoding
 * location of an instruction to a label by a specified width (bytes,
 * half-words, words, double-words, etc.).
 */
...
public:
  InstructionAbsoluteRelocation(uint8_t *updateLocation, TR::Instruction *i,
                                bool useEndAddr) /* specified if the start or
                                                    the end address of the
                                                    instruction is required */
      : TR::Relocation(updateLocation), _instruction(i),
        _useEndAddr(useEndAddr) {}
  virtual void apply(TR::CodeGenerator *cg);
...
// FIXME: these label absolute relocations should really be a subclass of
// instruction
// absolute.
...
class LoadLabelRelative16BitRelocation : public TR::Relocation {
  ...
  int32_t _deltaToStartLabel; // used to catch potentially nasty register dep
                              // related bugs

public:
  LoadLabelRelative16BitRelocation() : TR::Relocation() {}
  LoadLabelRelative16BitRelocation(TR::Instruction *i, TR::LabelSymbol *start,
                                   TR::LabelSymbol *end, int32_t delta)
      : TR::Relocation(NULL), _lastInstruction(i), _startLabel(start),
        _endLabel(end), _deltaToStartLabel(delta) {}
  ...
  TR::LabelSymbol *setStartLabel(TR::LabelSymbol *l) {
    return (_startLabel = l);
  }
...
public:
  ...
  LoadLabelRelative32BitRelocation(TR::Instruction *i, TR::LabelSymbol *start,
                                   TR::LabelSymbol *end, int32_t delta)
      : TR::Relocation(NULL), _lastInstruction(i), _startLabel(start),
        _endLabel(end), _deltaToStartLabel(delta) {}
  TR::Instruction *getLastInstruction() { return _lastInstruction; }
...
class IteratedExternalRelocation
    : public TR_Link<TR::IteratedExternalRelocation> {
public:
  IteratedExternalRelocation()
      : TR_Link<TR::IteratedExternalRelocation>(), _numberOfRelocationSites(0),
        _targetAddress(NULL), _targetAddress2(NULL), _relocationData(NULL),
        _relocationDataCursor(NULL), _sizeOfRelocationData(0),
        _recordModifier(0), _full(false), _kind(TR_ConstantPool) {}

  IteratedExternalRelocation(uint8_t *target, TR_ExternalRelocationTargetKind k,
                             flags8_t modifier, TR::CodeGenerator *codeGen);
  IteratedExternalRelocation(uint8_t *target, uint8_t *target2,
  ...
  uint8_t *setRelocationDataCursor(uint8_t *p) {
    return (_relocationDataCursor = p);
  }
  ...
  uint16_t setSizeOfRelocationData(uint16_t s) {
    return (_sizeOfRelocationData = s);
  }
  uint16_t addToSizeOfRelocationData(uint16_t s) {
    return (_sizeOfRelocationData += s);
  }
  ...
  void setNeedsWideOffsets() {
    _recordModifier.set(RELOCATION_TYPE_WIDE_OFFSET);
  }

  bool isOrderedPair() {
    return _recordModifier.testAny(RELOCATION_TYPE_ORDERED_PAIR);
  }
  ...
  setTargetKind(TR_ExternalRelocationTargetKind k) {
    return (_kind = k);
  }
...
public:
  ExternalRelocation()
      : TR::Relocation(), _targetAddress(NULL), _targetAddress2(NULL),
        _kind(TR_ConstantPool), _relocationRecord(NULL) {}

  ExternalRelocation(uint8_t *p, uint8_t *target,
                     TR_ExternalRelocationTargetKind kind,
                     TR::CodeGenerator *codeGen)
      : TR::Relocation(p), _targetAddress(target), _targetAddress2(NULL),
        _kind(kind), _relocationRecord(NULL) {}

  ExternalRelocation(uint8_t *p, uint8_t *target, uint8_t *target2,
                     TR_ExternalRelocationTargetKind kind,
                     TR::CodeGenerator *codeGen)
      : TR::Relocation(p), _targetAddress(target), _targetAddress2(target2),
        _kind(kind), _relocationRecord(NULL) {}
  ...
  static const char *getName(TR_ExternalRelocationTargetKind k) {
    return _externalRelocationTargetKindNames[k];
  }
  static uintptr_t getGlobalValue(uint32_t g) {
    TR_ASSERT(g >= 0 && g < TR_NumGlobalValueItems,
              "invalid index for global item");
...
public:
  ExternalOrderedPair32BitRelocation(uint8_t *location1, uint8_t *location2,
                                     uint8_t *target,
                                     TR_ExternalRelocationTargetKind k,
                                     TR::CodeGenerator *codeGen);
...
public:
  BeforeBinaryEncodingExternalRelocation() : TR::ExternalRelocation() {}
  ...
  BeforeBinaryEncodingExternalRelocation(TR::Instruction *instr,
                                         uint8_t *target, uint8_t *target2,
                                         TR_ExternalRelocationTargetKind kind,
                                         TR::CodeGenerator *codeGen)
...
typedef TR::LoadLabelRelative16BitRelocation
    TR_16BitLoadLabelRelativeRelocation;
typedef TR::LoadLabelRelative32BitRelocation
    TR_32BitLoadLabelRelativeRelocation;
typedef TR::LoadLabelRelative64BitRelocation
    TR_64BitLoadLabelRelativeRelocation;
typedef TR::IteratedExternalRelocation TR_IteratedExternalRelocation;
typedef TR::ExternalRelocation TR_ExternalRelocation;
typedef TR::ExternalOrderedPair32BitRelocation
    TR_32BitExternalOrderedPairRelocation;
typedef TR::BeforeBinaryEncodingExternalRelocation
