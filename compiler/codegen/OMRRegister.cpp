...
OMR::Register::Register(uint32_t f)
    : _flags(f), _backingStorage(NULL), _pinningArrayPointer(NULL),
      _assignedRegister(NULL), _siblingRegister(NULL), _startOfRange(NULL),
      _endOfRange(NULL), _startOfRangeNode(NULL), _totalUseCount(0),
      _futureUseCount(0), _outOfLineUseCount(0), _association(0), _kind(TR_GPR),
      _index(0) {}

OMR::Register::Register(TR_RegisterKinds rk)
    : _flags(0), _backingStorage(NULL), _pinningArrayPointer(NULL),
      _assignedRegister(NULL), _siblingRegister(NULL), _startOfRange(NULL),
      _endOfRange(NULL), _startOfRangeNode(NULL), _totalUseCount(0),
      _futureUseCount(0), _outOfLineUseCount(0), _association(0), _kind(rk),
      _index(0) {}
...
OMR::Register::Register(TR_RegisterKinds rk, uint16_t ar)
    : _flags(0), _backingStorage(NULL), _pinningArrayPointer(NULL),
      _assignedRegister(NULL), _siblingRegister(NULL), _startOfRange(NULL),
      _endOfRange(NULL), _startOfRangeNode(NULL), _totalUseCount(0),
      _futureUseCount(0), _outOfLineUseCount(0), _association(ar), _kind(rk),
      _index(0) {}

TR::Register *OMR::Register::self() {
...
TR::RealRegister *OMR::Register::getAssignedRealRegister() {
  return self()->getAssignedRegister()
             ? self()->getAssignedRegister()->getRealRegister()
             : NULL;
}
    ...
    if (assignedReg != NULL) {
      if (assignedReg->getState() == TR::RealRegister::Blocked) {
        assignedReg->setState(TR::RealRegister::Assigned,
                              self()->isPlaceholderReg());
      }
    }
  }
}

const char *OMR::Register::getRegisterName(TR::Compilation *comp,
...
const char *OMR::Register::getRegisterKindName(TR::Compilation *comp,
                                               TR_RegisterKinds rk) {
  if (comp->getDebug())
...
#if defined(DEBUG)
void OMR::Register::print(TR::Compilation *comp, TR::FILE *pOutFile,
                          TR_RegisterSizes size) {
...
#if defined(DEBUG)
const char *TR_RematerializationInfo::toString(TR::Compilation *comp) {
