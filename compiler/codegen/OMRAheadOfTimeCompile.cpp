...
extern bool
isOrderedPair(uint8_t reloType);
...
void
OMR::AheadOfTimeCompile::traceRelocationOffsets(
  uint8_t*& cursor,
  int32_t offsetSize,
  ...
  if (comp->cg()->needGuardSitesEvenWhenGuardRemoved() &&
    ...
    if (comp->getOption(TR_TraceAll))
      traceMsg(comp,
               ...
               removedGuardKind,
               virtualGuard,
               virtualGuard->getCallNode(),
               virtualGuard->getGuardNode(),
               virtualGuard->getThisClass(),
               virtualGuard->getCalleeIndex(),
               virtualGuard->getByteCodeIndex(),
               site);
  }
#endif
}
