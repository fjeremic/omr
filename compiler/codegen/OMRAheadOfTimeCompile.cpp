...
void createGuardSiteForRemovedGuard(TR::Compilation* comp, TR::Node* ifNode)
{
  ...
  if (comp->cg()->needGuardSitesEvenWhenGuardRemoved() && ifNode->isTheVirtualGuardForAGuardedInlinedCall()) {
    ...
    if (comp->getOption(TR_TraceAll))
      traceMsg(comp,
        "createGuardSiteForRemovedGuard: removedGuardKind %d, removedGurad %p, _callNode %p, _guardNode %p, _thisClass "
        "%p, _calleeIndex %d, _byteCodeIndex %d, addedAOTNopSite %p\n",
        removedGuardKind, virtualGuard, virtualGuard->getCallNode(), virtualGuard->getGuardNode(),
        virtualGuard->getThisClass(), virtualGuard->getCalleeIndex(), virtualGuard->getByteCodeIndex(), site);
  }
#endif
}
