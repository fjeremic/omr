...
TR::AheadOfTimeCompile *OMR::AheadOfTimeCompile::self() {
  return static_cast<TR::AheadOfTimeCompile *>(this);
}

TR_Debug *OMR::AheadOfTimeCompile::getDebug() {
  return self()->comp()->getDebug();
}

TR_Memory *OMR::AheadOfTimeCompile::trMemory() {
  return self()->comp()->trMemory();
}

void OMR::AheadOfTimeCompile::traceRelocationOffsets(
    uint8_t *&cursor, int32_t offsetSize, const uint8_t *endOfCurrentRecord,
    bool isOrderedPair) {
  // Location Offsets
...
#ifdef J9_PROJECT_SPECIFIC
  if (comp->cg()->needGuardSitesEvenWhenGuardRemoved() &&
      ifNode->isTheVirtualGuardForAGuardedInlinedCall()) {
    ...
    if (virtualGuard->getKind() == TR_HCRGuard) {
      if (comp->getOption(TR_TraceReloCG))
        traceMsg(comp,
                 "createGuardSiteForRemovedGuard: removing HCRGuard, no need "
                 "to add AOTNOPsite, node %p\n",
                 ifNode);
      return;
    }
    ...
    if (virtualGuard->getKind() == TR_BreakpointGuard) {
      if (comp->getOption(TR_TraceReloCG))
        traceMsg(comp,
                 "createGuardSiteForRemovedGuard: removing BreakpointGuard, no "
                 "need to add AOTNOPsite, node %p\n",
                 ifNode);
      return;
    }
    ...
    case TR_ProfiledGuard:
      ...
      TR_ASSERT(ifNode->isProfiledGuard(),
                "guard for profiled guard has unexpected kind");
      break;
    case TR_InterfaceGuard:
      removedGuardKind = TR_RemovedInterfaceGuard;
      TR_ASSERT(ifNode->isInterfaceGuard(),
                "guard for interface guard has unexpected kind");
      break;
    case TR_NonoverriddenGuard:
    case TR_DirectMethodGuard:
      removedGuardKind = TR_RemovedNonoverriddenGuard;
      TR_ASSERT(ifNode->isNonoverriddenGuard(),
                "guard for nonoverridden guard has unexpected kind");
      break;
    default:
      TR_ASSERT(false,
                "removed virtual guard type %d on node %p not supported yet.",
                virtualGuard->getKind(), ifNode);
      break;
    };
    ...
    if (comp->getOption(TR_TraceAll))
      traceMsg(comp,
               "createGuardSiteForRemovedGuard: removedGuardKind %d, "
               "removedGurad %p, _callNode %p, _guardNode %p, _thisClass %p, "
               "_calleeIndex %d, _byteCodeIndex %d, addedAOTNopSite %p\n",
               removedGuardKind, virtualGuard, virtualGuard->getCallNode(),
               virtualGuard->getGuardNode(), virtualGuard->getThisClass(),
               virtualGuard->getCalleeIndex(), virtualGuard->getByteCodeIndex(),
               site);
  }
#endif
}
