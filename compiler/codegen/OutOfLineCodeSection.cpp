...
TR_OutOfLineCodeSection::TR_OutOfLineCodeSection(TR::Node *callNode,
                                                 ...
                                                 TR::CodeGenerator *cg)
    : _restartLabel(restartLabel), _firstInstruction(NULL),
      _appendInstruction(NULL), _targetReg(targetReg), _entryLabel(entryLabel),
      _hasBeenRegisterAssigned(false), _cg(cg) {
  _entryLabel->setStartOfColdInstructionStream();
  _restartLabel->setEndOfColdInstructionStream();
  _block = callNode->getSymbolReference()->canCauseGC()
...
TR_OutOfLineCodeSection::TR_OutOfLineCodeSection(TR::LabelSymbol *entryLabel,
                                                 TR::CodeGenerator *cg)
    : _entryLabel(entryLabel), _cg(cg), _firstInstruction(NULL),
      _appendInstruction(NULL), _restartLabel(NULL), _block(NULL),
      _callNode(NULL), _targetReg(NULL), _hasBeenRegisterAssigned(false) {
  _entryLabel->setStartOfColdInstructionStream();
}
...
TR_OutOfLineCodeSection::TR_OutOfLineCodeSection(TR::LabelSymbol *entryLabel,
                                                 ...
                                                 TR::CodeGenerator *cg)
    : _entryLabel(entryLabel), _cg(cg), _firstInstruction(NULL),
      _appendInstruction(NULL), _restartLabel(restartLabel), _block(NULL),
      _callNode(NULL), _targetReg(NULL), _hasBeenRegisterAssigned(false) {
  _entryLabel->setStartOfColdInstructionStream();
  _restartLabel->setEndOfColdInstructionStream();
...
void TR_OutOfLineCodeSection::preEvaluatePersistentHelperArguments() {
  TR_ASSERT(_callNode,
            "preEvaluatePersistentHelperArguments can only be called for "
            "TR_OutOfLineCodeSection which are helper calls");
  TR::TreeEvaluator::initializeStrictlyFutureUseCounts(
      _callNode, _cg->comp()->incVisitCount(), _cg);
...
TR::Node *
TR_OutOfLineCodeSection::createOutOfLineCallNode(TR::Node *callNode,
                                                 TR::ILOpCodes callOp) {
  int32_t i;
  ...
  for (i = 0; i < callNode->getNumChildren(); i++) {
    ...
    TR::TreeEvaluator::initializeStrictlyFutureUseCounts(child, visitCount,
                                                         _cg);
  }
  ...
  for (i = 0; i < callNode->getNumChildren(); i++) {
    ...
    if (child->getRegister() != NULL) {
      ...
      if ((child->getOpCodeValue() == TR::loadaddr) &&
          (callNode->getOpCodeValue() == TR:: instanceof
           || callNode->getOpCodeValue() == TR::checkcast ||
