...
TR_StorageOverlapKind TR_StorageInfo::mayOverlapWith(TR_StorageInfo *info) {
  ...
  if (symbolMismatch && _class == TR_DirectMappedAuto &&
    ...
    return TR_NoOverlap;  // directly mapped autos from loadaddrs or direct
                          ...
                          // that aload from autos and static addresses
                          // certainly can as the aload may contain a static
                          // address
  }
  ...
  if (compareRanges) {
    ...
    if (overlapStart >=
        overlapEnd)  // i.e. is this an impossible (>) or trivial range (==)
                     // with start >= end? If so then there is no overlap
    {
