...
TR_StorageInfo::TR_StorageInfo(TR::Node *node, size_t length,
                               TR::Compilation *c)
    : _node(node), _length(length), _address(NULL), _symRef(NULL), _offset(0),
      _class(TR_UnknownClass), _comp(c) {
...
void TR_StorageInfo::print() {
  traceMsg(
      comp(),
      "\t\t\t%s (%p) len %d: addr %s (%p) symRef #%d, offset %d, class %s\n",
      _node->getOpCode().getName(), _node, _length,
...
void TR_StorageInfo::populateAddress(TR::Node *address) {
  ...
  if (comp()->cg()->isSupportedAdd(_address) &&
    ...
    _address = _address->getFirstChild();
             ...
             _address->getFirstChild()->getFirstChild()->getNumChildren() ==
                 0 &&
             _address->getFirstChild()
                 ->getSecondChild()
                 ->getOpCode()
                 .isLoadConst() &&
             _address->getSecondChild()->getOpCode().isLoadConst()) {
    ...
    _offset += _address->getSecondChild()->get64bitIntegralValue();
    _offset +=
        _address->getFirstChild()->getSecondChild()->get64bitIntegralValue();
...
TR_StorageOverlapKind TR_StorageInfo::mayOverlapWith(TR_StorageInfo *info) {
  ...
  // lengths and offset do not matter in these first two cases as disjointness
  // is known from the storage classes alone
  bool symbolMismatch = _symRef && info->_symRef &&
                        _symRef->getSymbol() != info->_symRef->getSymbol();
  if (symbolMismatch && _class == TR_DirectMappedAuto &&
      info->_class == TR_DirectMappedAuto) {
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               "\t\toverlap=false : autoDirectMapped and diff symbols (#%d "
               "(%p) and #%d (%p))\n",
               _symRef->getReferenceNumber(), _symRef->getSymbol(),
               info->_symRef->getReferenceNumber(), info->_symRef->getSymbol());

    return TR_NoOverlap; // separately directly mapped autos from loadaddrs or
                         // direct stores/loads
  } else if (symbolMismatch && _class == TR_DirectMappedStatic &&
             info->_class == TR_DirectMappedStatic) {
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               "\t\toverlap=false : staticDirectMapped and diff symbols (#%d "
               "(%p) and #%d (%p))\n",
               _symRef->getReferenceNumber(), _symRef->getSymbol(),
               info->_symRef->getReferenceNumber(), info->_symRef->getSymbol());

    return TR_NoOverlap; // separately directly mapped autos from loadaddrs or
                         // cached static aloads
  } else if ((_class == TR_DirectMappedAuto &&
              info->_class == TR_DirectMappedStatic) ||
             (_class == TR_DirectMappedAuto &&
              info->_class == TR_StaticBaseAddress) ||
             (_class == TR_DirectMappedAuto &&
              info->_class == TR_PrivateStaticBaseAddress) ||
    ...
    return TR_NoOverlap; // directly mapped autos from loadaddrs or direct
                         // stores cannot interfere with static addresses note
                         // that aload from autos and static addresses certainly
                         // can as the aload may contain a static address
  }

  // TR_StaticBaseAddress and TR_DirectMappedStatic are unlikely to be found in
  // the same compile unit (different fundamental ways of accessing statics) but
  // if they are both present then they may overlap as the TR_StaticBaseAddress
  // points to the start of the the static area and TR_DirectMappedStatic points
  // to a particular static symbol
  ...
  // next only consider cases where classes are the same but the lengths and
  // offsets may differ
  bool compareRanges = false;
  if (_symRef && info->_symRef && _symRef == info->_symRef) {
    if (_class == TR_StaticBaseAddress &&
        info->_class == TR_StaticBaseAddress) {
      if (comp()->cg()->traceBCDCodeGen())
        traceMsg(comp(), "\t\t\tcompareRanges : staticBaseAddress case\n");
      compareRanges = true;
    } else if (_class == TR_PrivateStaticBaseAddress &&
               info->_class == TR_PrivateStaticBaseAddress) {
      if (comp()->cg()->traceBCDCodeGen())
        traceMsg(comp(),
                 "\t\t\tcompareRanges : privateStaticBaseAddress case\n");
      compareRanges = true;
    } else if (_class == TR_DirectMappedStatic &&
               info->_class == TR_DirectMappedStatic) {
      if (comp()->cg()->traceBCDCodeGen())
  ...
  if (!compareRanges && _address && info->_address &&
      comp()->cg()->nodeMatches(_address, info->_address)) {
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               "\t\t\tcompareRanges : nodes match case (%s (%p) and %s (%p))\n",
               _address->getOpCode().getName(), _address,
               info->_address->getOpCode().getName(), info->_address);
    compareRanges = true;
  }
  ...
  if (compareRanges) {
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               "\t\t\tcompareRanges : range1 %d->%d vs range2 %d->%d --> "
               "overlap range %d->%d\n",
    ...
    if (overlapStart >=
        overlapEnd) // i.e. is this an impossible (>) or trivial range (==) with
                    // start >= end? If so then there is no overlap
    {
      overlap = TR_NoOverlap;
    } else {
      // If overlapStart < overlapEnd then this is a valid range and there is
      // overlap (overlapStart->overlapEnd is the co-incident)
      if (startOne == startTwo) {
        ...
        if (startOne + std::min(_length, info->_length) - 1 >= overlapStart) {
          ...
          //    :   abababab  (i.e. result not the same as if a temp was used in
          //    which case answer would be ababcd)
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               "\t\toverlap=%s (%s) : overlap range %d->%d is %spossible\n",
               overlap ? "true" : "false", getName(overlap), overlapStart,
               overlapEnd, overlap ? "" : "im");
