...
void TR_StorageInfo::print()
{
  traceMsg(comp(), "\t\t\t%s (%p) len %d: addr %s (%p) symRef #%d, offset %d, class %s\n", _node->getOpCode().getName(),
    _node, _length, _address ? _address->getOpCode().getName() : "NULL", _address,
...
TR_StorageOverlapKind TR_StorageInfo::mayOverlapWith(TR_StorageInfo* info)
{
  ...
  if (symbolMismatch && _class == TR_DirectMappedAuto && info->_class == TR_DirectMappedAuto) {
    ...
    return TR_NoOverlap; // directly mapped autos from loadaddrs or direct stores cannot interfere with static addresses
                         // note that aload from autos and static addresses certainly can as the aload may contain a
                         // static address
  }
  ...
  if (compareRanges) {
                                    ...
                                    // then there is no overlap
    {
      ...
      // If overlapStart < overlapEnd then this is a valid range and there is overlap (overlapStart->overlapEnd is the
      // co-incident)
