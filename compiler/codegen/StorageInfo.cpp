...
{
    _address = address;
    if (comp()->cg()->isSupportedAdd(_address) && _address->getFirstChild()->getNumChildren() == 0 && _address->getSecondChild()->getOpCode().isLoadConst()) {
        _offset += _address->getSecondChild()->get64bitIntegralValue();
        _address = _address->getFirstChild();
    } else if (comp()->cg()->isSupportedAdd(_address) && comp()->cg()->isSupportedAdd(_address->getFirstChild()) && _address->getFirstChild()->getFirstChild()->getNumChildren() == 0 && _address->getFirstChild()->getSecondChild()->getOpCode().isLoadConst() && _address->getSecondChild()->getOpCode().isLoadConst()) {
        // aiadd
        //    aiadd
        //       aload
        //       iconst
...
TR_StorageInfo::mayOverlapWith(TR_StorageInfo* info)
{
    ...
    } else if ((_class == TR_DirectMappedAuto && info->_class == TR_DirectMappedStatic) || (_class == TR_DirectMappedAuto && info->_class == TR_StaticBaseAddress) || (_class == TR_DirectMappedAuto && info->_class == TR_PrivateStaticBaseAddress) ||

        (_class == TR_DirectMappedStatic && info->_class == TR_DirectMappedAuto) ||

        (_class == TR_StaticBaseAddress && info->_class == TR_DirectMappedAuto) || (_class == TR_StaticBaseAddress && info->_class == TR_PrivateStaticBaseAddress) ||

        (_class == TR_PrivateStaticBaseAddress && info->_class == TR_DirectMappedAuto) || (_class == TR_PrivateStaticBaseAddress && info->_class == TR_StaticBaseAddress)) {
        if (comp()->cg()->traceBCDCodeGen())
