...
TR_StorageInfo::TR_StorageInfo(TR::Node* node,
                               size_t length,
                               TR::Compilation* c)
  : _node(node)
  , _length(length)
  ...
  , _comp(c)
{
  if (node->getOpCode().isLoadVarOrStore())
    ...
    TR_ASSERT(false,
              "node %s (%p) should be a load or store or address\n",
...
TR_StorageInfo::print()
{
  traceMsg(
    ...
    _node->getOpCode().getName(),
    _node,
    _length,
    _address ? _address->getOpCode().getName() : "NULL",
    _address,
    _symRef ? _symRef->getReferenceNumber() : -1,
    _offset,
    getName());
}
...
TR_StorageInfo::mayOverlapWith(TR_StorageInfo* info)
{
  ...
  if (symbolMismatch && _class == TR_DirectMappedAuto &&
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               ...
               _symRef->getSymbol(),
               info->_symRef->getReferenceNumber(),
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               "\t\toverlap=false : diff storage classes (%s and %s)\n",
               getName(),
               info->getName());
  ...
  if (_length == 0 || info->_length == 0) // unknown lengths, can do no more
  {
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               "\t\toverlap=true : unknown lengths (%d and %d)\n",
               _length,
               info->_length);
  ...
  if (!compareRanges && _address && info->_address &&
    ...
    if (comp()->cg()->traceBCDCodeGen())
      traceMsg(comp(),
               ...
               _address->getOpCode().getName(),
               _address,
               info->_address->getOpCode().getName(),
               info->_address);
    compareRanges = true;
  }
