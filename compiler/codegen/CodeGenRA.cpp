  ...
  vcount_t visitCount, TR::SymbolReference* symRef, bool& symRefIsLive, bool checkForIMuls, bool& vmThreadUsed)
{
  ...
  for (childCount = node->getNumChildren() - 1; childCount >= 0; childCount--)
    self()->estimateRegisterPressure(node->getChild(childCount), registerPressure, maxRegisterPressure, maxRegisters,
      valuesInGlobalRegs, isCold, visitCount, symRef, symRefIsLive, checkForIMuls, vmThreadUsed);
  ...
  if (node->getOpCode().isResolveCheck() || (node->getOpCode().isCall()) || node->getOpCodeValue() == TR::New
        ...
        || node->getOpCodeValue() == TR::multianewarray || node->getOpCodeValue() == TR::MergeNew
        || node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit
        || node->getOpCodeValue() == TR::checkcast || node->getOpCodeValue() == TR::
    ...
    if (node->getOpCode().isLoadVar() && node->getSymbol()->isAutoOrParm() && valuesInGlobalRegs
      ...
      if (!node->getOpCode().isLoadConst()) {
        ...
        if (dtype == TR::Float || dtype == TR::Double
          ...
          || dtype == TR::DecimalFloat || dtype == TR::DecimalDouble || dtype == TR::DecimalLongDouble
#endif
        )
          gprCandidate = false;
        if (node->getDataType() == TR::Float || node->getDataType() == TR::Double
#ifdef J9_PROJECT_SPECIFIC
          || node->getDataType() == TR::DecimalFloat || node->getDataType() == TR::DecimalDouble
#endif
        ) {
          if (!gprCandidate)
            registerPressure++;
  ...
  TR::SymbolReference* symRef, bool checkForIMuls)
{
  ...
  while (treeTop) {
    ...
    if (vmThreadUsed && (maxRegisterPressure >= self()->getMaximumNumbersOfAssignableGPRs())) {
      // dumpOptDetails("For global register candidate %d reg pressure reached limit %d at block_%d isCold %d maxFreq
...
TR_BackingStore* OMR::CodeGenerator::allocateInternalPointerSpill(TR::AutomaticSymbol* pinningArrayPointer)
{
  ...
  for (auto i = self()->getInternalPointerSpillFreeList().begin(); i != self()->getInternalPointerSpillFreeList().end();
       ++i) {
    if ((*i)
          ->getSymbolReference()
          ->getSymbol()
  ...
  if (!spill) {
    int32_t slot;
    TR::AutomaticSymbol* spillSymbol = TR::AutomaticSymbol::createInternalPointer(
      self()->trHeapMemory(), TR::Address, TR::Compiler->om.sizeofReferenceAddress(), self()->fe());
...
{
  return self()->allocateSpill((int32_t)(TR::Compiler->om.sizeofReferenceAddress()), containsCollectedReference, offset,
  ...
  while (referenceIterator != _liveReferenceList.end()) {
    ...
    if (tempSymRef) {
      ...
      if (storeSpillTemp) {
        ...
        TR::TreeTop::create(
          self()->comp(), insertionTree, TR::Node::createWithSymRef(TR::astore, 1, 1, newReference, tempSymRef));
      }
...
bool OMR::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate*, TR::Node* branchNode)
{
...
TR_GlobalRegisterNumber OMR::CodeGenerator::pickRegister(TR_RegisterCandidate* rc, TR::Block** allBlocks,
  ...
  TR_LinkHead<TR_RegisterCandidate>* candidatesAlreadyAssigned)
{
  ...
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    while (blockIterator.hasMoreElements()) {
      ...
      if (!block->isExtensionOfPreviousBlock() && !blockIsIgnorablyCold(block, self())) {
        ...
        TR_RegisterPressureState blockEntryState(NULL, false, alreadyAssignedOnEntry, alreadyAssignedOnExit,
          ...
          self()->getNumberOfGlobalFPRs() + meniscus - fprDelta, self()->getNumberOfGlobalVRFs() + meniscus - vrfDelta,
          0);
        for (TR_RegisterCandidate* candidate = candidatesAlreadyAssigned->getFirst(); candidate;
        ...
        if (cacheIsInconclusive) {
          if (canAffordFullSimulation) {
            TR_RegisterPressureState state(blockEntryState, rc, rc->getBlocksLiveOnEntry().isSet(block->getNumber()),
              self()->comp()->incVisitCount());
          ...
          //
          if (self()->traceSimulateTreeEvaluation()) {
            traceMsg(self()->comp(), "            using cache for block_%d -- g=%d, f=%d, v=%d", block->getNumber(),
    ...
    while (blockIterator.hasMoreElements()) {
      ...
      if (block != self()->comp()->getFlowGraph()->getEnd() && !blockIsIgnorablyCold(block, self())) {
        ...
        //
        int32_t blockWeight
          = 1; // Note that we use a static loop-nesting-depth heuristic here for consistency with the rest of GRA
                ...
                if (remainingRegisters.isSet(preferredRegister)) {
                  ...
                  if (self()->traceSimulateTreeEvaluation()) {
                    traceMsg(self()->comp(), "            found copy to/from #%d, register %d (%s), weight %d, at %s\n",
                      ...
                      self()->getDebug()->getGlobalRegisterName(hottestCopiedRegister), hottestCopiedRegisterWeight,
              ...
              } else // is Unpreferred
              {
                if (self()->traceSimulateTreeEvaluation()) {
                  traceMsg(self()->comp(),
                    ...
                    candidate->getSymbolReference()->getReferenceNumber(), candidate->getGlobalRegisterNumber(),
                    self()->getDebug()->getGlobalRegisterName(candidate->getGlobalRegisterNumber()), blockWeight,
                    self()->getDebug()->getName(node));
                }
                ...
                if (candidateRegister != -1) {
                  if (!isUnpreferred) {
                    if (remainingRegisters.isSet(candidateRegister)) {
                      ...
                      if (self()->traceSimulateTreeEvaluation()) {
                        traceMsg(self()->comp(),
                          "            found call parameter index %i, register %d (%s), weight %d, at %s\n", i,
                  ...
                  } else // candidate is unpreferred
                  {
                    if (self()->traceSimulateTreeEvaluation()) {
                      traceMsg(self()->comp(),
                        "            unprefer call parameter index %i, register %d (%s), weight %d, at %s\n", i,
                        candidateRegister, self()->getDebug()->getGlobalRegisterName(candidateRegister), blockWeight,
    ...
    if (linkageRegister == -1) {
      ...
      unpreferredRegisters
        = *self()->getGlobalRegisters(TR_linkageSpill, self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
  ...
  if (1) {
    ...
    if (debug("tracePickRegister") || self()->comp()->getOptions()->trace(OMR::tacticalGlobalRegisterAllocator)) {
      traceMsg(
        self()->comp(), "pickRegister:\tPreserved register info is %spresent\n", preservedRegisters ? "" : "NOT ");
    }
...
uint8_t OMR::CodeGenerator::gprCount(TR::DataType type, int32_t size)
{
  ...
  return (type.isInt64() && TR::Compiler->target.is32Bit() && !self()->use64BitRegsOn32Bit())
    ? 2
...
{
  if (state->_candidate)
    traceMsg(comp, " %c%c", state->_pressureRiskFromStart ? '+' : state->_numLiveCandidateLoads ? '|' : ' ',
...
static TR_RegisterCandidate* findCandidate(TR::SymbolReference* symRef, TR_LinkHead<TR_RegisterCandidate>* candidates,
...
static void keepMostRecentValueAliveIfLiveOnEntryToSuccessor(TR_RegisterCandidate* candidate, TR::TreeTop* exitPoint,
...
static void killMostRecentValueIfKeptAliveUntilCurrentTreeTop(
  TR_RegisterCandidate* candidate, OMR::CodeGenerator::TR_RegisterPressureState* state, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  TR::Block* block, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
  ...
  if (1) {
    blockIsLoopHeader
      = (b = block->getStructureOf()) && (loop = b->getContainingLoop()) && (loop->getNumber() == block->getNumber());
  }
  ...
  if (node->getOpCode().isStoreDirect()) {
    ...
    for (auto e = node->getBlock()->getSuccessors().begin(); e != node->getBlock()->getSuccessors().end(); ++e) {
      ...
      if (state->_candidatesAlreadyAssigned)
        for (TR_RegisterCandidate* candidate = state->_candidatesAlreadyAssigned->getFirst(); candidate;
             candidate = candidate->getNext())
        ...
        if ((numGPRTemporaries >= 1 || numFPRTemporaries >= 1 || numVRFTemporaries >= 1)
          && (self()->comp()->getOption(TR_TraceRegisterPressureDetails)
            && !performTransformation(self()->comp(),
              "O^O GLOBAL REGISTER ALLOCATION: Call %p has %d GPR and %d FPR and VRF %d dummy registers\n", node,
              numGPRTemporaries, numFPRTemporaries, numVRFTemporaries))) {
          numGPRTemporaries = 0;
          numFPRTemporaries = 0;
          numVRFTemporaries = 0;
        }
  ...
  TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
      ...
      if (callNodeLinkage) {
        if (childIndex < maxChildrenInRegisters
          ...
          if (callNodeLinkage && self()->comp()->getOption(TR_TraceRegisterPressureDetails))
            traceMsg(self()->comp(), " (%s arg %d in %s, %d left)", self()->getDebug()->getName(node), childIndex,
  ...
  if (summary->_gprPressure < summary->PRESSURE_LIMIT)
    TR_ASSERT(state->_gprPressure <= summary->_gprPressure,
      "Children of %s must record max register gprPressure in summary; %d > %d", self()->getDebug()->getName(node),
  ...
  if (summary->_fprPressure < summary->PRESSURE_LIMIT)
    TR_ASSERT(state->_fprPressure <= summary->_fprPressure,
      "Children of %s must record max register fprPressure in summary; %d > %d", self()->getDebug()->getName(node),
  ...
  if (summary->_vrfPressure <= summary->PRESSURE_LIMIT)
    TR_ASSERT(state->_vrfPressure <= summary->_vrfPressure,
      "Children of %s must record max register vrfPressure in summary; %d > %d", self()->getDebug()->getName(node),
  ...
  if (nodeGotFoldedIntoMemref(node, state, self())
    ...
    if (callNodeLinkage) {
      ...
      for (int32_t childIndex = std::min<int32_t>(maxChildrenInRegisters, node->getNumChildren()) - 1; childIndex >= 0;
           childIndex--)
  ...
  uint32_t& numGPRTemporaries, uint32_t& numFPRTemporaries, uint32_t& numVRFTemporaries)
{
...

  {
    ...
    bv |= *self()->getGlobalRegisters(TR_vmThreadSpill,
      self()->comp()->getJittedMethodSymbol()->getLinkageConvention()); // vmthread, when present, is also a virtual reg
                                                                        // not represented in trees
...
void OMR::CodeGenerator::simulateNodeGoingLive(TR::Node* node, TR_RegisterPressureState* state)
{
  ...
  if (self()->traceSimulateTreeEvaluation()) {
    ...
    traceMsg(self()->comp(), "%2d(%d) g%+d=%-2d f%+d=%-2d v%+d=%-2d", self()->simulatedNodeState(node, state)._height,
  ...
  TR::Node* node, TR_RegisterPressureState* state, TR::CodeGenerator* cg)
{
  ...
  _numRegisters += 1;
  //_numRegisters += cg->simulatedNodeState(node)._liveGPRs; // Fix this once we find a platform that can put other reg
  //kinds in a memref
}
