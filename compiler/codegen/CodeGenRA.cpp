...
void OMR::CodeGenerator::checkForLiveRegisters(
    TR_LiveRegisters *liveRegisters) {
  TR_Debug *debug = self()->comp()->getDebug();
  bool regsAreLive = false;

  if (liveRegisters && liveRegisters->getFirstLiveRegister()) {
    traceMsg(self()->comp(), "\n\n");
    for (TR_LiveRegisterInfo *p = liveRegisters->getFirstLiveRegister(); p;
         p = p->getNext()) {
      if (p->getNode())
        traceMsg(self()->comp(),
                 "Virtual register %s (for node [%s], ref count=%d) is live at "
                 "end of method\n",
                 debug->getName(p->getRegister()), debug->getName(p->getNode()),
                 p->getNode()->getReferenceCount());
      else
        traceMsg(self()->comp(),
                 "Virtual register %s %p is live at end of method with node "
                 "count %d\n",
                 debug->getName(p->getRegister()), p->getRegister(),
                 p->getNodeCount());
      regsAreLive = true;
    }
  }

  TR_ASSERT(!regsAreLive,
            "Virtual or real registers are live at end of method");
}
#endif

void OMR::CodeGenerator::estimateRegisterPressure(
    TR::Node *node, int32_t &registerPressure, int32_t &maxRegisterPressure,
    int32_t maxRegisters, TR_BitVector *valuesInGlobalRegs, bool isCold,
    vcount_t visitCount, TR::SymbolReference *symRef, bool &symRefIsLive,
    bool checkForIMuls, bool &vmThreadUsed) {
  if (node->getVisitCount() == visitCount) {
    node->setLocalIndex(node->getLocalIndex() - 1);
    if (node->getLocalIndex() == 0) {
      if (node->getOpCode().isLoadVar() && node->getSymbol()->isAutoOrParm() &&
          valuesInGlobalRegs &&
          valuesInGlobalRegs->get(
        ...
        if (!node->getOpCode().isLoadConst()) {
          registerPressure--;
          if ((node->getType().isInt64()) && TR::Compiler->target.is32Bit())
            registerPressure--;
        }
  ...
  for (childCount = node->getNumChildren() - 1; childCount >= 0; childCount--)
    self()->estimateRegisterPressure(
        node->getChild(childCount), registerPressure, maxRegisterPressure,
        maxRegisters, valuesInGlobalRegs, isCold, visitCount, symRef,
        symRefIsLive, checkForIMuls, vmThreadUsed);
    ...
    if (node->getOpCode().isLoadVar() && node->getSymbol()->isAutoOrParm() &&
      ...
      if (!node->getOpCode().isLoadConst()) {
        ...
        if (dtype == TR::Float || dtype == TR::Double
            ...
            || dtype == TR::DecimalFloat || dtype == TR::DecimalDouble ||
        ...
        if (node->getDataType() == TR::Float ||
          ...
          if (!gprCandidate) registerPressure++;
        }
#ifdef J9_PROJECT_SPECIFIC
        else if (node->getDataType() == TR::DecimalLongDouble) {
          if (!gprCandidate) registerPressure += 2;
        }
#endif
        else {
          if (gprCandidate) {
      ...
      if (!symRefIsLive) {
        ...
        if (highRegisterPressureOpCode ||
            ((node->getType().isInt64()) && TR::Compiler->target.is32Bit() &&
             (node->getOpCode().isMul() || node->getOpCode().isDiv() ||
              node->getOpCode().isRem() || node->getOpCode().isLeftShift() ||
              node->getOpCode().isRightShift() ||
              node->getOpCode().isBooleanCompare()))) {
          if (!isCold) {
            if (registerPressure >=
                (maxRegisters - NUM_REGS_USED_BY_COMPLEX_OPCODES -
          ...
          if (registerPressure > maxRegisterPressure) {
            if (!isCold) maxRegisterPressure = registerPressure;
          }
        }
      }
    }
...
int32_t OMR::CodeGenerator::estimateRegisterPressure(
    TR::Block *block, vcount_t visitCount, int32_t maxStaticFrequency,
    int32_t maxFrequency, bool &vmThreadUsed, int32_t numGlobalRegs,
    TR_BitVector *valuesInGlobalRegs, TR::SymbolReference *symRef,
    bool checkForIMuls) {
  ...
  if (maxFrequency < 0) {
    maxFrequency = 0;
    for (TR::CFGNode *node = self()->comp()->getFlowGraph()->getFirstNode();
  ...
  //
  if (block->isCold() ||
      ((maxFrequency > 0) && (block->getFrequency() * 100 / maxFrequency < 20)))
    isCold = true;
  ...
  while (treeTop) {
    ...
    self()->estimateRegisterPressure(
        node, currRegisterPressure, maxRegisterPressure,
        self()->getMaximumNumbersOfAssignableGPRs(), valuesInGlobalRegs, isCold,
        visitCount, symRef, symRefIsLive, checkForIMuls, vmThreadUsed);

    if (vmThreadUsed &&
        (maxRegisterPressure >= self()->getMaximumNumbersOfAssignableGPRs())) {
      // dumpOptDetails("For global register candidate %d reg pressure reached
      // limit %d at block_%d isCold %d maxFreq %d\n",
...
// Set the future use count on all registers, and keep track of what kind of
...
TR_RegisterKinds OMR::CodeGenerator::prepareRegistersForAssignment() {
  ...
  for (i = 0; i < regArray.size(); i++) {
    ...
    if (!(foundKindsMask & km)) foundKindsMask |= km;
  }

  return TR_RegisterKinds(foundKindsMask);
}

TR_BackingStore *OMR::CodeGenerator::allocateInternalPointerSpill(
  ...
  for (auto i = self()->getInternalPointerSpillFreeList().begin();
    ...
    if ((*i)->getSymbolReference()
            ...
            ->getPinningArrayPointer() == pinningArrayPointer) {
      spill = *i;
  ...
  if (!spill) {
    ...
    TR::AutomaticSymbol *spillSymbol =
        TR::AutomaticSymbol::createInternalPointer(
            self()->trHeapMemory(), TR::Address,
            TR::Compiler->om.sizeofReferenceAddress(), self()->fe());
...
TR_BackingStore *OMR::CodeGenerator::allocateSpill(
    bool containsCollectedReference, int32_t *offset, bool reuse) {
  return self()->allocateSpill(
      (int32_t)(TR::Compiler->om.sizeofReferenceAddress()),
      containsCollectedReference, offset, reuse);  // cast parameter explicitly
}

TR_BackingStore *OMR::CodeGenerator::allocateSpill(
    int32_t dataSize, bool containsCollectedReference, int32_t *offset,
    bool reuse) {
  TR_ASSERT_FATAL(dataSize <= 16, "assertion failure");
  TR_ASSERT_FATAL(!containsCollectedReference ||
                      (dataSize == TR::Compiler->om.sizeofReferenceAddress()),
                  "assertion failure");

  if (self()->getTraceRAOption(TR_TraceRASpillTemps))
    traceMsg(self()->comp(), "\nallocateSpill(%d, %s, %s)", dataSize,
             containsCollectedReference ? "collected" : "uncollected",
             offset ? "offset" : "NULL");
  ...
  // multiplied by the size of a Java pointer, and that will only give it 4
  // bytes.
  //
  // TODO: Make mapStack smarter and support this in the GC maps.
  //
  bool try8ByteSpills =
      (dataSize < 16) && ((TR::Compiler->om.sizeofReferenceAddress() == 8) ||
                          !containsCollectedReference);
  ...
  if (reuse) {
    ...
    if ((spill && self()->getTraceRAOption(TR_TraceRASpillTemps) &&
         !performTransformation(
             self()->comp(), "O^O SPILL TEMPS: Reuse spill temp %s\n",
      ...
      //
      // This approach was chosen because it tends to make
      // lastOptTransformationIndex produce a smaller delta than if we had left
      // the spill in the lists.
      // TODO: Use the same size for the new spill temp so that it will be freed
      // to the same list, thereby further reducing the delta.
      //
      spill = NULL;
    }
  }
  ...
  if (spill) {
    ...
    //
    int spillSize = std::max(
        dataSize,
        static_cast<int32_t>(TR::Compiler->om.sizeofReferenceAddress()));

    TR_ASSERT(4 <= spillSize && spillSize <= 16,
              "Spill temps should be between 4 and 16 bytes");
    spillSymbol = TR::AutomaticSymbol::create(self()->trHeapMemory(),
                                              TR::NoType, spillSize);
    spillSymbol->setSpillTempAuto();
    self()->comp()->getMethodSymbol()->addAutomatic(spillSymbol);
    spill = new (self()->trHeapMemory())
  ...
  if (dataSize <= 4 && spillSymbol->getSize() == 8) {
    ...
    if (offset == NULL || spill->secondHalfIsOccupied() ||
        !performTransformation(
            self()->comp(), "O^O HALF-SLOT SPILLS: Use second half of %s\n",
            self()->getDebug()->getName(spill->getSymbolReference()))) {
...
void OMR::CodeGenerator::freeSpill(TR_BackingStore *spill, int32_t dataSize,
                                   int32_t offset) {
  ...
  if (self()->getTraceRAOption(TR_TraceRASpillTemps)) {
    traceMsg(
        ...
        self()->getDebug()->getName(spill->getSymbolReference()->getSymbol()),
        spill->firstHalfIsOccupied() ? 1 : 0,
        spill->secondHalfIsOccupied() ? 1 : 0, dataSize, offset,
        self()->isFreeSpillListLocked());
  }

  // Do not add a freed spill slot back onto the free list if the list is
  // locked. This is to enforce re-use of the same spill slot for a virtual
  // register while assigning non-linear control flow regions.
  //
  // However, we must still clear the occupied status of the temp.
  //
  bool updateFreeList = self()->isFreeSpillListLocked() ? false : true;

  if (spill->getSymbolReference()
          ->getSymbol()
          ->getAutoSymbol()
          ->isInternalPointer()) {
    spill->setIsEmpty();
    ...
    if (updateFreeList) {
      ...
      if (self()->getTraceRAOption(TR_TraceRASpillTemps))
        traceMsg(self()->comp(), "\n -> Added to internalPointerSpillFreeList");
             ...
             spill->getSymbolReference()->getSymbol()->getSize() == 8) {
    if (offset == 0) {
    ...
    if (spill->isEmpty()) {
      ...
      // TODO: Once every caller can cope with nonzero offsets, we should add
      // first-half-occupied symbols into _spill4FreeList.
      if (self()->getTraceRAOption(TR_TraceRASpillTemps))
        traceMsg(self()->comp(),
                 "\n -> first half is still occupied; conservatively keeping "
                 "out of spill4FreeList");
    } else {
...
TR::Register *OMR::CodeGenerator::allocateRegister(TR_RegisterKinds rk) {
  ...
  if (self()->getDebug()) self()->getDebug()->newRegister(temp);
...
void OMR::CodeGenerator::findAndFixCommonedReferences() {
  ...
  for (TR::TreeTop *tt = self()->comp()->getStartTree(); tt;
    ...
    if (isSafePoint) {
      ...
      if (safePointNode->getVisitCount() != self()->comp()->getVisitCount()) {
        ...
        if (!_liveReferenceList.empty()) {
          self()->spillLiveReferencesToTemps(tt->getPrevTreeTop(),
  ...
  if (debug("traceSpill")) diagnostic("End spilling at GC safe points\n");
}

void OMR::CodeGenerator::findCommonedReferences(TR::Node *node,
                                                TR::TreeTop *treeTop) {
  if (node->getVisitCount() == self()->comp()->getVisitCount()) return;
  ...
  for (int32_t childCount = node->getNumChildren() - 1; childCount >= 0;
    ...
    if (child->getDataType() == TR::Address &&
      ...
      TR::Symbol *sym =
          child->getOpCode().hasSymbolReference() ? child->getSymbol() : 0;
...
void OMR::CodeGenerator::processReference(TR::Node *reference, TR::Node *parent,
                                          TR::TreeTop *treeTop) {
  // The live reference list appender is used to add and remove elements from
  // the live reference list, so that the list is maintained in the order in
  ...
  while (iterator != _liveReferenceList.end()) {
    if ((*iterator)->getReferenceNode() == reference) {
      ...
      // reference and must remove this reference from the list because it can
      // not be alive across any future calls that may be encountered.
      if (reference->getReferenceCount() ==
          (*iterator)->getNumberOfParents() + 1) {
      ...
      // if it turns out to be alive across a GC safe point and we have to
      // introduce a temp and fix up all the parents.
      else {
  ...
  cursor = new (self()->trHeapMemory())
      TR_LiveReference(reference, parent, self()->trMemory());
  _liveReferenceList.push_back(cursor);
  self()->needSpillTemp(cursor, parent, treeTop);
}

void OMR::CodeGenerator::spillLiveReferencesToTemps(
  ...
  while (referenceIterator != _liveReferenceList.end()) {
    ...
    if (originalReference->getOpCode().hasSymbolReference() &&
      ...
      if (!_availableSpillTemps.empty()) {
        ...
        firstAvailableSpillTemp =
            _availableSpillTemps.erase(firstAvailableSpillTemp);
        if (debug("traceSpill"))
          diagnostic("   Re-use available spill temp [%p] for reference [%p]\n",
                     tempSymRef, originalReference);
      } else {
        int32_t slot;
        TR::AutomaticSymbol *tempSym =
            TR::AutomaticSymbol::create(self()->trHeapMemory());
        tempSym->setSize(TR::DataType::getSize(TR::Address));
        tempSym->setSpillTempAuto();
        tempSymRef = new (self()->trHeapMemory())
            TR::SymbolReference(self()->comp()->getSymRefTab(), tempSym);
    ...
    while (nodeCursor) {
      for (int32_t childCount = 0; childCount < nodeCursor->getNumChildren();
           ++childCount) {
      ...
      if (storeSpillTemp) {
        ...
        TR::TreeTop::create(self()->comp(), insertionTree,
                            TR::Node::createWithSymRef(
                                TR::astore, 1, 1, newReference, tempSymRef));
      }
    ...
    originalReference->setReferenceCount(
        originalReference->getReferenceCount() -
        (*referenceIterator)->getNumberOfParents());
...
void OMR::CodeGenerator::needSpillTemp(TR_LiveReference *cursor,
                                       TR::Node *parent, TR::TreeTop *treeTop) {
  // If the reference is a simple load and the load is not killed before the
  // first use after this GC point, there is no need for a spill temp, the
  // original load can just be duplicated at the first use after this point.
  //
  if (cursor->needSpillTemp()) return;  // Reference already needs a spill temp
...
bool OMR::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate *,
                                                         TR::Node *branchNode) {
  return !(branchNode->getOpCode().isJumpWithMultipleTargets()) ||
         debug("enableSwitch");
}
...
void OMR::CodeGenerator::initializeRegisterPressureSimulator() {
  ...
  _simulatedNodeStates = new (self()->trStackMemory())
      TR_SimulatedNodeState[self()->comp()->getNodeCount()];

  _blockRegisterPressureCache =
      new (self()->trStackMemory()) TR_RegisterPressureSummary[numBlocks];

#if defined(CACHE_CANDIDATE_SPECIFIC_RESULTS)
  int32_t tagsSize =
      numBlocks * sizeof(_blockAndCandidateRegisterPressureCacheTags[0]);
  _blockAndCandidateRegisterPressureCache =
      new (trStackMemory()) TR_RegisterPressureSummary[numBlocks];
  _blockAndCandidateRegisterPressureCacheTags =
      (int32_t *)trMemory()->allocateStackMemory(tagsSize);
...
bool OMR::CodeGenerator::prepareForGRA() {
  ...
  if (self()->getDebug() && self()->comp()->getOptions()->trace(
...
static bool blockIsMuchColderThanContainingLoop(TR::Block *block,
                                                TR::CodeGenerator *cg) {
  if (cg->comp()->getMethodHotness() <= warm) {
    ...
    if (block->getFrequency() < regSimBlockFreqCutoff) {
      if (cg->traceSimulateTreeEvaluation())
        traceMsg(cg->comp(),
                 "            Block %d is not hot enough for simulation (%d)\n",
  ...
  if (result && cg->traceSimulateTreeEvaluation())
    traceMsg(
        cg->comp(),
        "            Block %d is much colder than containing loop (%d << %d)\n",
...
void OMR::CodeGenerator::TR_RegisterPressureState::updateRegisterPressure(
    TR::Symbol *symbol) {
  ...
  if (dt == TR::NoType) dt = symbol->getDataType();
...
TR_GlobalRegisterNumber OMR::CodeGenerator::pickRegister(
    TR_RegisterCandidate *rc, TR::Block **allBlocks,
  ...
  static volatile uint8_t gprsWithheldFromPickRegister = 0,
                          fprsWithheldFromPickRegister = 0,
                          vrfWithheldFromPickRegister = 0,
                          gprsWithheldFromPickRegisterWhenWarm = 0;
  int32_t currentCandidateWeight = -1;
  int32_t maxCandidateWeight = -1;
  if (!isInitialized) {
    static char *rwgpr = feGetEnv("TR_gprsWithheldFromPickRegister");
    static char *rwgprWarm =
        feGetEnv("TR_gprsWithheldFromPickRegisterWhenWarm");
  ...
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    const bool usesFPR =
        (dtype == TR::Float || dtype == TR::Double
#ifdef J9_PROJECT_SPECIFIC
         || dtype == TR::DecimalFloat || dtype == TR::DecimalDouble ||
         dtype == TR::DecimalLongDouble
#endif
    ...
    if (self()->terseSimulateTreeEvaluation()) {
      traceMsg(self()->comp(),
               "         { Picking register for %s%s candidate #%d %s\n",
               highRegisterNumber ? "high word of " : "",
               usesFPR ? "FPR" : (usesVRF ? "VRF" : "GPR"),
      ...
      if (debug("dumpAllSpillKinds")) {
        for (int32_t i = 0; i < TR_numSpillKinds; i++) {
          ...
          traceMsg(self()->comp(), "            %s regs: ",
                   self()->getDebug()->getSpillKindName(sk));
          self()->getDebug()->print(self()->comp()->getOptions()->getLogFile(),
    ...
    if (!_blockRegisterPressureCache) {
      // Note that this assumes no new blocks or nodes are added between calls
      // to pickRegister
      self()->initializeRegisterPressureSimulator();
      TR_ASSERT(_blockRegisterPressureCache && _simulatedNodeStates,
                "assertion failure");
    }

    TR_BitVector remainingRegisters = availableRegisters;
    TR_BitVector *spilledRegisters = self()->getGlobalRegisters(
        TR_vmThreadSpill,
    ...
    if (spilledRegisters) {
      if (self()->traceSimulateTreeEvaluation()) {
        ...
        traceMsg(self()->comp(),
                 "            vmThread register not enabled; rejected: ");
        self()->getDebug()->print(self()->comp()->getOptions()->getLogFile(),
                                  &regsToPrint);
        traceMsg(self()->comp(), "\n");
      }
    ...
    TR_BitVector alreadyAssignedOnEntry(self()->comp()->getSymRefCount(),
                                        self()->trMemory(), stackAlloc,
                                        growable);
    TR_BitVector alreadyAssignedOnExit(self()->comp()->getSymRefCount(),
                                       self()->trMemory(), stackAlloc,
                                       growable);
    ...
    while (blockIterator.hasMoreElements()) {
      if (canExitEarly && remainingRegisters.isEmpty()) break;

      TR::Block *block = allBlocks[blockIterator.getNextElement()];

      if (!block->isExtensionOfPreviousBlock() &&
          !blockIsIgnorablyCold(block, self())) {
        ...
        static int32_t meniscus =
            feGetEnv("TR_registerPressureMeniscus")
                ? atoi(feGetEnv("TR_registerPressureMeniscus"))
                : 0;
        ...
        TR_RegisterPressureState blockEntryState(
            NULL, false, alreadyAssignedOnEntry, alreadyAssignedOnExit,
            candidatesAlreadyAssigned,
        ...
        for (TR_RegisterCandidate *candidate =
                 candidatesAlreadyAssigned->getFirst();
             candidate; candidate = candidate->getNext()) {
          if (candidate->getBlocksLiveOnEntry().get(block->getNumber())) {
            ...
            TR::Symbol *candidateSymbol =
                candidate->getSymbolReference()->getSymbol();
        ...
        //
        TR_RegisterPressureSummary *cachedSummary =
            &_blockRegisterPressureCache[block->getNumber()];

        // Live on exit info is not useful for cacheable simulations, so don't
        // do any for now.  In reality, we'll need to consider exactly when we
        // want to consider the cache unusable.  Presumably we could attempt a
        // cacheable simulation of each block, and if the block is "uncacheable"
        // we could just not use the cache for that one.
        //
        ...
        if (cacheIsInconclusive) {
          if (canAffordFullSimulation) {
            TR_RegisterPressureState state(
                blockEntryState, rc,
                rc->getBlocksLiveOnEntry().isSet(block->getNumber()),
                self()->comp()->incVisitCount());
            if (rc->getBlocksLiveOnEntry().isSet(block->getNumber())) {
              state.updateRegisterPressure(rcSymbol);
            }

            // Perform the simulation for current block and accumulate into
            // highWaterMark
            //
            ...
            //
            _blockAndCandidateRegisterPressureCache[block->getNumber()] =
                summary;
            ...
            if (self()->traceSimulateTreeEvaluation())
              traceMsg(self()->comp(),
                       "            saved summary for block_%d\n",
                       block->getNumber());
          } else {
            // Can't afford a full simulation; assume GRA is no longer
            // profitable.
            //
            if (self()->terseSimulateTreeEvaluation())
              traceMsg(self()->comp(),
          ...
          if (self()->traceSimulateTreeEvaluation()) {
            traceMsg(self()->comp(),
                     "            using cache for block_%d -- g=%d, f=%d, v=%d",
                     block->getNumber(), cachedSummary->_gprPressure,
                     cachedSummary->_fprPressure, cachedSummary->_vrfPressure);
        ...
        // 1d. Eliminate from consideration any regs whose usage may cause a
        // spill. If early exits are allowed, do this on each iteration.
        //
        if (canExitEarly) {
          TR_BitVector spilledRegs(self()->getNumberOfGlobalRegisters(),
                                   self()->trMemory());
          self()->computeSpilledRegsForAllPresentLinkages(&spilledRegs,
                                                          highWaterMark);
          remainingRegisters -= spilledRegs;
          if (self()->traceSimulateTreeEvaluation()) {
            traceMsg(self()->comp(), "            rejected registers: ");
            self()->getDebug()->print(
                self()->comp()->getOptions()->getLogFile(), &spilledRegs);
            highWaterMark.dumpSpillMask(self());
            traceMsg(self()->comp(), "\n            remaining registers: ");
            self()->getDebug()->print(
                self()->comp()->getOptions()->getLogFile(),
                &remainingRegisters);
            traceMsg(self()->comp(), "\n");
          }
        }
    ...
    TR_BitVector unpreferredRegisters(self()->getNumberOfGlobalRegisters(),
                                      self()->trMemory());

    TR_BitVector visitedNodesForCandidateUse(0, self()->trMemory(), stackAlloc,
                                             growable);
    ...
    while (blockIterator.hasMoreElements()) {
      TR::Block *block = allBlocks[blockIterator.getNextElement()];
      if (block != self()->comp()->getFlowGraph()->getEnd() &&
          !blockIsIgnorablyCold(block, self())) {
        // Only scan for copies if this block is hotter than the hottest known
        // copy
        //
        int32_t blockWeight =
            1;  // Note that we use a static loop-nesting-depth heuristic here
                // for consistency with the rest of GRA
        ...
        if (blockWeight > hottestCopiedRegisterWeight) {
          ...
          for (TR::TreeTop *tt = block->getEntry(); tt != stopTree;
               tt = tt->getNextTreeTop()) {
            TR::Node *node = tt->getNode();
            bool isUnpreferred;
            TR_RegisterCandidate *candidate =
                self()->findCoalescenceForRegisterCopy(node, rc,
                                                       &isUnpreferred);
            ...
            if (candidate) {
              if (!isUnpreferred) {
                ...
                if (remainingRegisters.isSet(preferredRegister)) {
                  ...
                  if (self()->traceSimulateTreeEvaluation()) {
                    traceMsg(
                        self()->comp(),
                        "            found copy to/from #%d, register %d (%s), "
                        "weight %d, at %s\n",
                        candidate->getSymbolReference()->getReferenceNumber(),
                        hottestCopiedRegister,
                        self()->getDebug()->getGlobalRegisterName(
                            hottestCopiedRegister),
                        hottestCopiedRegisterWeight,
              ...
              } else  // is Unpreferred
              {
                if (self()->traceSimulateTreeEvaluation()) {
                  traceMsg(
                      self()->comp(),
                      "            unprefer copy to/from #%d, register %d "
                      "(%s), weight %d, at %s\n",
                      candidate->getSymbolReference()->getReferenceNumber(),
                ...
                if (candidate->getHighGlobalRegisterNumber() != -1)
                  unpreferredRegisters.set(
                      candidate->getHighGlobalRegisterNumber());
              }
            }
            ...
            if ((node->getOpCodeValue() == TR::treetop) &&
              ...
              for (uint32_t i = 0; i < callNode->getNumChildren(); i++) {
                ...
                TR_GlobalRegisterNumber candidateRegister =
                    self()->findCoalescenceRegisterForParameter(callNode, rc, i,
                                                                &isUnpreferred);
                if (candidateRegister != -1) {
                  if (!isUnpreferred) {
                    if (remainingRegisters.isSet(candidateRegister)) {
                      ...
                      if (self()->traceSimulateTreeEvaluation()) {
                        traceMsg(self()->comp(),
                                 "            found call parameter index %i, "
                                 "register %d (%s), weight %d, at %s\n",
                  ...
                  } else  // candidate is unpreferred
                  {
                    if (self()->traceSimulateTreeEvaluation()) {
                      traceMsg(self()->comp(),
                               "            unprefer call parameter index %i, "
                               "register %d (%s), weight %d, at %s\n",
                               i, candidateRegister,
                               self()->getDebug()->getGlobalRegisterName(
                                   candidateRegister),
            ...
            if (!candidate) {
              TR_RegisterCandidate *candidate = self()->findUsedCandidate(
                  node, rc, &visitedNodesForCandidateUse);
              if (candidate) {
                if (self()->traceSimulateTreeEvaluation()) {
                  traceMsg(
                      self()->comp(),
                      "            unprefer used candidate #%d, register %d "
                      "(%s), weight %d, at %s\n",
                      candidate->getSymbolReference()->getReferenceNumber(),
                      candidate->getGlobalRegisterNumber(),
                      self()->getDebug()->getGlobalRegisterName(
    ...
    if (hottestCopiedRegister == -1) {
      // Found no preferred register.  If we can avoid the unpreferred
      // registers, do so.
      //
      unpreferredRegisters &= remainingRegisters;
      if (unpreferredRegisters != remainingRegisters) {
        if (self()->traceSimulateTreeEvaluation() &&
            !unpreferredRegisters.isEmpty()) {
      ...
      TR_ASSERT(remainingRegisters.isSet(hottestCopiedRegister),
                "assertion failure");
      if (self()->traceSimulateTreeEvaluation())
    ...
    if (remainingRegisters.isEmpty()) {
      if (self()->terseSimulateTreeEvaluation())
        traceMsg(self()->comp(),
                 "         } No good registers for candidate #%d after reg "
                 "pressure simulation\n",
                 rc->getSymbolReference()->getReferenceNumber());
      else
        dumpOptDetails(self()->comp(),
                       "No good registers for candidate #%d after reg pressure "
                       "simulation\n",
                       rc->getSymbolReference()->getReferenceNumber());
      ...
      TR_GlobalRegisterNumber result =
          TR_BitVectorIterator(remainingRegisters).getFirstElement();

      if (self()->terseSimulateTreeEvaluation())
        traceMsg(self()->comp(),
                 "         } Picked register %d (%s) for candidate #%d %s\n",
      ...
      while (blockIterator.hasMoreElements() &&
             !availableRegisters.isEmpty())  // TODO: Can availableRegisters
                                             // possibly be empty here?
      {
        TR::Block *block = allBlocks[blockIterator.getNextElement()];
        if (!block->isExtensionOfPreviousBlock()) {
          TR_RegisterPressureSummary *cachedSummary =
  ...
  if (1) {
    ...
    uint8_t regsWithheld =
        (dtype == TR::Float || dtype == TR::Double
         ...
         dtype == TR::DecimalLongDouble
            ...
            ? fprsWithheldFromPickRegister
            : (self()->comp()->getMethodHotness() == warm)
                  ? gprsWithheldFromPickRegisterWhenWarm
                  : gprsWithheldFromPickRegister;

    if (availableRegisters.elementCount() <= regsWithheld) {
      if (debug("tracePickRegister") ||
          self()->comp()->getOptions()->trace(
              OMR::tacticalGlobalRegisterAllocator)) {
        traceMsg(self()->comp(),
                 "pickRegister: Withholding %d registers from candidate #%d\n",
                 availableRegisters.elementCount(),
                 rc->getSymbolReference()->getReferenceNumber());
      }
      return -1;
    } else {
      if (debug("tracePickRegister") ||
        ...
        traceMsg(
            self()->comp(),
            "pickRegister: %d registers still available for candidate #%d\n",
            availableRegisters.elementCount(),
            rc->getSymbolReference()->getReferenceNumber());
      }
    }
    ...
    if (rcSymbol->isParm()) {
      int8_t lri = rcSymbol->getParmSymbol()->getLinkageRegisterIndex();
      if (lri >= 0) {
        linkageRegister = self()->getLinkageGlobalRegisterNumber(
    ...
    if (debug("tracePickRegister") ||
        self()->comp()->getOptions()->trace(
            OMR::tacticalGlobalRegisterAllocator)) {
      traceMsg(self()->comp(),
               "pickRegister:\tPreserved register info is %spresent\n",
    ...
    if (preservedRegisters) {
      ...
      if (preservedRegisters) {
        ...
        while (!isLiveAcrossNonColdCall && bvi.hasMoreElements()) {
          isLiveAcrossNonColdCall =
              !blockIsIgnorablyCold(allBlocks[bvi.getNextElement()], self());
        }
      }
      ...
      if (debug("tracePickRegister") ||
          self()->comp()->getOptions()->trace(
              OMR::tacticalGlobalRegisterAllocator)) {
        traceMsg(self()->comp(),
                 "pickRegister:\tRegister is %slive across non-cold call\n",
    ...
    TR_ASSERT(preferences,
              "A register selection preference order must be chosen");
    if (debug("tracePickRegister") ||
        self()->comp()->getOptions()->trace(
            OMR::tacticalGlobalRegisterAllocator)) {
    ...
    for (bestLowRegisterNumber = -1;
         bestLowRegisterNumber == -1 && preferences[0]; preferences++) {
      switch (preferences[0]) {
        case 'L':  // The linkage register carrying the candidate into the
                   // method
          TR_ASSERT(linkageRegister != -1,
                    "Candidate must have a linkage register");
        ...
        case 'P':  // Any preserved register
          TR_ASSERT(preservedRegisters,
                    "Must have preservedRegisters info to select a preserved "
                    "register");
        ...
        case 'N':  // Any non-preserved register
          TR_ASSERT(preservedRegisters,
                    "Must have preservedRegisters info to select a "
                    "non-preserved register");
          TR_ASSERT(linkageRegister == -1,
                    "Candidates arriving in linkage regs can't be given other "
                    "linkage regs");
          //
        ...
        case 'O':  // Any non-preserved non-linkage register ('O'ther volatile
                   // registers)
          // Not yet implemented
          // Continue with the next preference
          break;
    ...
    if (debug("tracePickRegister") ||
      ...
      if (bestLowRegisterNumber == -1) {
        traceMsg(self()->comp(),
                 "pickRegister:\tNOT assigned a global register\n");
      } else {
...
TR_RegisterCandidate *OMR::CodeGenerator::findCoalescenceForRegisterCopy(
    TR::Node *node, TR_RegisterCandidate *rc, bool *isUnpreferred) {
  ...
  if (node->getOpCode().isStoreDirect() &&
    ...
    TR_RegisterCandidate *storedCand =
        self()->comp()->getGlobalRegisterCandidates()->find(
            node->getSymbolReference());

    if (storedCand) {
      int32_t loadedSymRefNum =
          node->getFirstChild()->getSymbolReference()->getReferenceNumber();
      // unpreferred if the candidate for coalescing does not match the
      // candidate rc, currently being evaluated
      *isUnpreferred =
          !(loadedSymRefNum == rc->getSymbolReference()->getReferenceNumber());
      candidate = storedCand;
    }

    TR_RegisterCandidate *loadedCand =
        self()->comp()->getGlobalRegisterCandidates()->find(
            node->getFirstChild()->getSymbolReference());
    if (loadedCand) {
      int32_t storedSymRefNum =
          node->getSymbolReference()->getReferenceNumber();
      // unpreferred if the candidate for coalescing does not match the
      // candidate rc, currently being evaluated
      *isUnpreferred =
          !(storedSymRefNum == rc->getSymbolReference()->getReferenceNumber());
...
TR_GlobalRegisterNumber OMR::CodeGenerator::findCoalescenceRegisterForParameter(
  ...
  if (paramNode->getOpCode().isLoadVarDirect()) {
    int32_t paramSymRefNum =
        paramNode->getSymbolReference()->getReferenceNumber();
    *isUnpreferred =
        !(paramSymRefNum == rc->getSymbolReference()->getReferenceNumber());
  }
  return -1;
}

TR_RegisterCandidate *OMR::CodeGenerator::findUsedCandidate(
    TR::Node *node, TR_RegisterCandidate *rc, TR_BitVector *visitedNodes) {
  ...
  if (node->getOpCode().isLoadVarDirect() || node->getOpCode().isStoreDirect())
    gprToCoalesce = self()->comp()->getGlobalRegisterCandidates()->find(
        node->getSymbolReference());

  for (int32_t i = 0; (gprToCoalesce == NULL) && (i < node->getNumChildren());
       i++)
    gprToCoalesce =
        self()->findUsedCandidate(node->getChild(i), rc, visitedNodes);
...
// all callees.
//
#define LINKAGE_THAT_IS_ALWAYS_PRESENT \
  (self()->comp()->getMethodSymbol()->getLinkageConvention())

void OMR::CodeGenerator::computeSimulatedSpilledRegs(
    TR_BitVector *spilledRegisters, int32_t blockNum,
  ...
  if (symRefNum == _blockAndCandidateRegisterPressureCacheTags[blockNum]) {
    if (traceSimulateTreeEvaluation())
      traceMsg(comp(),
               "Using candidate-specific regsiter pressure simulation cache "
               "for block_%d, sym #%d\n",
               blockNum, symRefNum);
    computeSpilledRegsForAllPresentLinkages(
        spilledRegisters, _blockAndCandidateRegisterPressureCache[blockNum]);
  } else
#endif
  {
    self()->computeSpilledRegsForAllPresentLinkages(
        spilledRegisters, _blockRegisterPressureCache[blockNum]);
  }
}
...
void OMR::CodeGenerator::computeSpilledRegsForAllPresentLinkages(
  ...
  for (int32_t i = 0; i < TR_numProbableSpillKinds; i++) {
    ...
    if (summary.isSpilled(spillKind))
      self()->setSpilledRegsForAllPresentLinkages(spilledRegisters, summary,
                                                  spillKind);
  }
}
...
void OMR::CodeGenerator::setSpilledRegsForAllPresentLinkages(
  ...
  for (int32_t i = 0; i < TR_NumLinkages; i++) {
    ...
    if (summary.isLinkagePresent(lc) || lc == LINKAGE_THAT_IS_ALWAYS_PRESENT) {
      ...
      if (regs) *spilledRegisters |= *regs;
    }
  }
}

TR::SymbolReference *
OMR::CodeGenerator::TR_RegisterPressureState::getCandidateSymRef() {
  return _candidate ? _candidate->getSymbolReference() : NULL;
}

void OMR::CodeGenerator::TR_RegisterPressureState::addVirtualRegister(
    TR::Register *reg) {
  if (reg == NULL) return;
...
void OMR::CodeGenerator::TR_RegisterPressureState::removeVirtualRegister(
    TR::Register *reg) {
  if (reg == NULL) return;
  if (reg->getRealRegister()) return;
...
bool OMR::CodeGenerator::TR_RegisterPressureState::isInitialized(
    TR::Node *node) {
  return node->getVisitCount() == _visitCountForInit;
}

void OMR::CodeGenerator::TR_RegisterPressureSummary::setLinkagePresent(
    TR_LinkageConventions lc, TR::CodeGenerator *cg) {
...
void OMR::CodeGenerator::TR_RegisterPressureSummary::spill(
    TR_SpillKinds kind, TR::CodeGenerator *cg) {
  _spillMask |= (1 << kind);
  if (cg->traceSimulateTreeEvaluation())
    traceMsg(cg->comp(), " !%s", cg->getDebug()->getSpillKindName(kind));
}

void OMR::CodeGenerator::TR_RegisterPressureSummary::dumpSpillMask(
    TR::CodeGenerator *cg) {
  if (cg->traceSimulateTreeEvaluation()) {
    for (int32_t i = 0; i < TR_numSpillKinds; i++) {
      TR_SpillKinds spillKind = (TR_SpillKinds)i;
      if (isSpilled(spillKind))
        traceMsg(cg->comp(), " %s",
                 cg->getDebug()->getSpillKindName(spillKind));
    }
  }
}

void OMR::CodeGenerator::TR_RegisterPressureSummary::dumpLinkageConventionMask(
    TR::CodeGenerator *cg) {
  if (cg->traceSimulateTreeEvaluation()) {
    for (int32_t i = 0; i < TR_NumLinkages; i++) {
      TR_LinkageConventions lc = (TR_LinkageConventions)i;
      if (isLinkagePresent(lc))
        traceMsg(cg->comp(), " %s",
                 cg->getDebug()->getLinkageConventionName(lc));
    }
  }
}

bool OMR::CodeGenerator::isCandidateLoad(TR::Node *node,
                                         TR::SymbolReference *candidate) {
  return node->getOpCode().isLoadVarDirect() &&
         node->getSymbolReference() == candidate;
}

bool OMR::CodeGenerator::isCandidateLoad(TR::Node *node,
                                         TR_RegisterPressureState *state) {
  return state->_candidate ? self()->isCandidateLoad(
                                 node, state->_candidate->getSymbolReference())
                           : false;
}

bool OMR::CodeGenerator::isLoadAlreadyAssignedOnEntry(
    TR::Node *node, TR_RegisterPressureState *state) {
  return node->getOpCode().isLoadVarDirect() &&
         (state->_alreadyAssignedOnEntry.isSet(
             node->getSymbolReference()->getReferenceNumber()));
}

uint8_t OMR::CodeGenerator::gprCount(TR::DataType type, int32_t size) {
  if (type.isAggregate())
    return (TR::Compiler->target.is32Bit() && !self()->use64BitRegsOn32Bit() &&
            size > 4 && size <= 8)
               ? 2
               : 1;

  return (type.isInt64() && TR::Compiler->target.is32Bit() &&
          !self()->use64BitRegsOn32Bit())
             ? 2
             : (type.isIntegral() || type.isAddress()) ? 1 : 0;
}
...
void OMR::CodeGenerator::addToUnlatchedRegisterList(TR::RealRegister *reg) {
  ...
  for (i = 0; _unlatchedRegisterList[i] != 0; i++)
    if (reg == _unlatchedRegisterList[i]) break;  // found my register
...
inline void leaveSpaceForRegisterPressureState(
    OMR::CodeGenerator::TR_RegisterPressureState *state,
    TR::Compilation *comp) {
  // Prints spaces to match the amount of space consumed by the g=, f=, c=, etc
  // columns. Call after dumpSimulatedNode for nodes where those columns won't
  // be printed.  This keeps any subsequent annotations lined up with those on
  // other rows.
  //
  traceMsg(comp, "%*s", 26, "");
...
static TR_RegisterCandidate *findCandidate(
    TR::SymbolReference *symRef, TR_LinkHead<TR_RegisterCandidate> *candidates,
    TR_RegisterCandidate *anotherCandidate = NULL) {
  if (anotherCandidate && anotherCandidate->getSymbolReference() == symRef)
    return anotherCandidate;

  TR_RegisterCandidate *result;
  for (result = candidates->getFirst();
       result && result->getSymbolReference() != symRef;
       result = result->getNext())
    ;
  return result;
}

static void rememberMostRecentValue(
    TR::SymbolReference *symRef, TR::Node *valueNode,
    OMR::CodeGenerator::TR_RegisterPressureState *state,
    TR::CodeGenerator *cg) {
  if (state->_alreadyAssignedOnExit.isSet(symRef->getReferenceNumber()) ||
      (state->_candidate && (state->getCandidateSymRef() == symRef))) {
    TR_RegisterCandidate *candidate = findCandidate(
        symRef, state->_candidatesAlreadyAssigned, state->_candidate);
    TR_ASSERT(candidate,
              "rememberMostRecentValue: there should be a matching candidate "
              "for #%d %s",
              symRef->getReferenceNumber(), cg->getDebug()->getName(symRef));
    if (candidate) candidate->setMostRecentValue(valueNode);
  }
}

static void keepMostRecentValueAliveIfLiveOnEntryToSuccessor(
    TR_RegisterCandidate *candidate, TR::TreeTop *exitPoint,
    TR::CFGNode *successor, OMR::CodeGenerator::TR_RegisterPressureState *state,
    TR::CodeGenerator *cg) {
...
static void killMostRecentValueIfKeptAliveUntilCurrentTreeTop(
  ...
  if (candidate && candidate->getMostRecentValue()) {
    TR::Node *mrv = candidate->getMostRecentValue();
    TR::CodeGenerator::TR_SimulatedNodeState &nodeState =
        cg->simulatedNodeState(mrv);
...
void OMR::CodeGenerator::simulateBlockEvaluation(
    TR::Block *block, TR_RegisterPressureState *state,
    TR_RegisterPressureSummary *summary) {
  TR_ASSERT(!block->isExtensionOfPreviousBlock(),
            "simulateBlockEvaluation operates on extended basic blocks");
  state->_currentBlock = block;

  if (self()->traceSimulateTreeEvaluation()) {
    traceMsg(self()->comp(), "            { simulating block_%d",
             block->getNumber());
    if (state->_candidate)
      traceMsg(self()->comp(), ", candidate %d",
               state->getCandidateSymRef()->getReferenceNumber());
    else
      traceMsg(self()->comp(), ", candidate ignored");
    traceMsg(self()->comp(), "\n               Already assigned on entry: ");
    self()->getDebug()->print(self()->comp()->getOptions()->getLogFile(),
                              &state->_alreadyAssignedOnEntry);
    traceMsg(self()->comp(), "\n               Already assigned on exit: ");
    self()->getDebug()->print(self()->comp()->getOptions()->getLogFile(),
                              &state->_alreadyAssignedOnExit);
  }
  ...
  if (state->_candidate || state->_candidatesAlreadyAssigned) {
    ...
    // considered live until the end of the last block where they are live on
    // exit
    //
    for (tt = block->getEntry()->getNextTreeTop(); tt;
         tt = tt->getNextTreeTop()) {
      TR::Node *node = tt->getNode();
      if (node->getOpCodeValue() == TR::BBStart &&
  ...
  while (tt && !state->mustAbort()) {
    ...
    if (tt->getNode()->getOpCode().isStoreDirect())
      rememberMostRecentValue(tt->getNode()->getSymbolReference(),
                              tt->getNode()->getFirstChild(), state, self());
    else if (tt->getNode()->getOpCode().isLoadVarDirect())
      rememberMostRecentValue(tt->getNode()->getSymbolReference(),
                              tt->getNode(), state, self());

    // Stop when we hit the next extended basic block
    //
    tt = tt->getNextTreeTop();
    if (!tt || (tt->getNode()->getOpCodeValue() == TR::BBStart &&
                !tt->getNode()->getBlock()->isExtensionOfPreviousBlock()))
      break;

    if (tt->getNode()->getOpCodeValue() == TR::BBEnd) {
      _blockRegisterPressureCache[tt->getNode()->getBlock()->getNumber()] =
          perBlockSummary;
      summary->accumulate(perBlockSummary);
      perBlockSummary.reset(state->_gprPressure, state->_fprPressure,
                            state->_vrfPressure);
    }
  }
  ...
  if (1) {
    blockIsLoopHeader = (b = block->getStructureOf()) &&
  ...
  if (self()->traceSimulateTreeEvaluation()) {
    traceMsg(self()->comp(),
             ...
             block->getNumber(), summary->_gprPressure, summary->_fprPressure,
             summary->_vrfPressure);
    summary->dumpSpillMask(self());
    traceMsg(self()->comp(), "\n");
  }
}
...
void OMR::CodeGenerator::simulationPrePass(
    TR::TreeTop *tt, TR::Node *node, TR_RegisterPressureState *state,
    TR_RegisterPressureSummary *summary) {
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    ...
    //
    uint8_t childHeight =
        self()->simulatedNodeState(node->getChild(i), state)._height;
  ...
  //
  if (node->getOpCode().isStoreDirect()) {
    rememberMostRecentValue(node->getSymbolReference(), node->getFirstChild(),
                            state, self());
    ...
    TR_RegisterCandidate *rc =
        findCandidate(node->getSymbolReference(),
                      state->_candidatesAlreadyAssigned, state->_candidate);
    if (rc) rc->setLastLoad(node);
  } else if (node->getOpCodeValue() == TR::BBEnd) {
    for (auto e = node->getBlock()->getSuccessors().begin();
         e != node->getBlock()->getSuccessors().end(); ++e) {
      if (state->_candidate)
        keepMostRecentValueAliveIfLiveOnEntryToSuccessor(
            state->_candidate, tt, (*e)->getTo(), state, self());
      if (state->_candidatesAlreadyAssigned)
        for (TR_RegisterCandidate *candidate =
                 state->_candidatesAlreadyAssigned->getFirst();
             candidate; candidate = candidate->getNext())
          keepMostRecentValueAliveIfLiveOnEntryToSuccessor(
              candidate, tt, (*e)->getTo(), state, self());
    }
  }
}

static bool nodeGotFoldedIntoMemref(
    TR::Node *node, OMR::CodeGenerator::TR_RegisterPressureState *state,
    TR::CodeGenerator *cg) {
  return (state->_currentMemref && !cg->isCandidateLoad(node, state) &&
...
void OMR::CodeGenerator::simulateTreeEvaluation(
  ...
  if (self()->getHasResumableTrapHandler() &&
    ...
    bool isResolvedLoadVar =
        (op.isLoadVar() && child->getFutureUseCount() == 1 &&
         !child->getSymbolReference()->isUnresolved());
    bool canFoldIntoBNDCHK =
        (nextTreeTop &&
         nextTreeTop->getNode()->getOpCodeValue() == TR::BNDCHK &&
  ...
  //
  if (self()->getSupportsVirtualGuardNOPing() &&
      self()->comp()->performVirtualGuardNOPing() &&
      node->isNopableInlineGuard()) {
    // Guard will be no-oped.
    self()->simulateSkippedTreeEvaluation(node, state, summary,
                                          'g');  // 'g' for "guard"
  ...
  if (nodeState.hasRegister()) {
    ...
    if (self()->traceSimulateTreeEvaluation()) {
      ...
      if (self()->isCandidateLoad(node, state))
        ...
        if (findCandidate(node->getSymbolReference(),
                          state->_candidatesAlreadyAssigned)
                ->getLastLoad() == node)
          tagChar = 'A';
        else
          tagChar = 'a';
    ...
    if (node->getOpCodeValue() == TR::BBEnd) {
      ...
      //
      killMostRecentValueIfKeptAliveUntilCurrentTreeTop(state->_candidate,
                                                        state, self());
      for (TR_RegisterCandidate *candidate =
               state->_candidatesAlreadyAssigned->getFirst();
           candidate; candidate = candidate->getNext())
        killMostRecentValueIfKeptAliveUntilCurrentTreeTop(candidate, state,
                                                          self());
    }

    uint32_t numGPRTemporaries = 0, numFPRTemporaries = 0,
             numVRFTemporaries = 0;
    ...
    if (self()->nodeWillBeRematerialized(node, state)) {
      // TR_ASSERT(node->getFutureUseCount() >= 2, "Node %s must have refcount
      // >= 2 to be rematerialized", getDebug()->getName(node));
      TR_SimulatedNodeState &nodeState =
          self()->simulatedNodeState(node, state);
      ...
      if (isCall) {
        self()->getNumberOfTemporaryRegistersUsedByCall(
            node, state, numGPRTemporaries, numFPRTemporaries,
            numVRFTemporaries);
        if ((numGPRTemporaries >= 1 || numFPRTemporaries >= 1 ||
             numVRFTemporaries >= 1) &&
            (self()->comp()->getOption(TR_TraceRegisterPressureDetails) &&
             !performTransformation(
                 self()->comp(),
                 "O^O GLOBAL REGISTER ALLOCATION: Call %p has %d GPR and %d "
                 "FPR and VRF %d dummy registers\n",
                 node, numGPRTemporaries, numFPRTemporaries,
                 numVRFTemporaries))) {
          numGPRTemporaries = 0;
          numFPRTemporaries = 0;
          numVRFTemporaries = 0;
    ...
    //
    summary->accumulate(state, self(), numGPRTemporaries, numFPRTemporaries,
                        numVRFTemporaries);
    ...
    if (state->candidateIsLiveAfterGRA()) {
      ...
      if (isCall) {
        ...
        TR::Symbol *rcSymbol =
            state->_candidate->getSymbolReference()->getSymbol();
        TR::Linkage *linkage = self()->getLinkage();

        // In Xplink, 31-bit mode, each call has a special argument passed in
        // GPR12 Incoming special argument can be  passed to the call,
        // otherwise, TR_linkageSpill has to be set so that other candidates
        // don't get assigned GPR12 across this call
        summary->spill(TR_linkageSpill, self());
        summary->setLinkagePresent(
      ...
      if (isCall) {
        summary->spill(TR_vmThreadSpill, self());
                 ...
                 node->getOpCodeValue() == TR::monent ||
                 node->getOpCodeValue() ==
                     TR::monexit)  // TODO: Is this the best test to use?
      {
        summary->spill(TR_vmThreadSpill, self());
      }
    }
...
void OMR::CodeGenerator::simulateSkippedTreeEvaluation(
  ...
  // early as possible, to expose more opportunities for efficient use of
  // registers.

  static char *disableSimulateSkippedTreeEvaluation =
      feGetEnv("TR_disableSimulateSkippedTreeEvaluation");
...
void OMR::CodeGenerator::simulateNodeEvaluation(
  ...
  TR_ASSERT(!self()->simulatedNodeState(node, state).hasRegister(),
            "simulateNodeEvaluation called on live node " POINTER_PRINTF_FORMAT,
            node);
  ...
  if (node->getNumChildren() == 2 &&
    ...
    evaluateSecondChildFirst =
        (self()->simulatedNodeState(node->getSecondChild(), state)._height >
         self()->simulatedNodeState(node->getFirstChild(), state)._height);
  }

  static char *neverEvaluateSecondChildFirst =
      feGetEnv("TR_neverEvaluateSecondChildFirst");
  if (neverEvaluateSecondChildFirst) evaluateSecondChildFirst = false;

  // For calls, most arguments are passed in memory, and don't add to register
  // pressure, so those children can be decremented immediately.
  // childrenInRegisters indicates which children have NOT been decremented.
  //
  flags32_t childrenInRegisters(
      0);  // (Don't bother with a bitvector on the slight chance that some
           // child beyond the first 32 will get in a register)
  ...
  if (node->getOpCode().isFunctionCall())
    callNodeLinkage = self()->getLinkage(
        node->getSymbol()->castToMethodSymbol()->getLinkageConvention());

  if (callNodeLinkage) {
    if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
      traceMsg(
          self()->comp(), " (%s %s linkage)", self()->getDebug()->getName(node),
          self()->getDebug()->getLinkageConventionName(
              node->getSymbol()->castToMethodSymbol()->getLinkageConvention()));
    for (int32_t k = 0; k < NumRegisterKinds; k++)
      numArgumentRegisters[k] =
          callNodeLinkage->numArgumentRegisters((TR_RegisterKinds)k);
  }
  ...
  if (evaluateSecondChildFirst) {
    ...
    if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
      traceMsg(self()->comp(), " (%s before %s)",
               self()->getDebug()->getName(node->getSecondChild()),
    ...
    for (int32_t childIndex = (hasMemRef ? 1 : 0);
         childIndex < node->getNumChildren(); childIndex++) {
      self()->simulateTreeEvaluation(node->getChild(childIndex), state,
                                     summary);
      if (callNodeLinkage) {
        if (childIndex < maxChildrenInRegisters &&
            --numArgumentRegisters[callNodeLinkage->argumentRegisterKind(
                node->getChild(childIndex))] >= 0) {
          ...
          if (callNodeLinkage &&
            ...
            traceMsg(self()->comp(), " (%s arg %d in %s, %d left)",
                     ...
                     self()->getDebug()->getRegisterKindName(
                         callNodeLinkage->argumentRegisterKind(
                             node->getChild(childIndex))),
                     numArgumentRegisters[callNodeLinkage->argumentRegisterKind(
                         node->getChild(childIndex))]);
        } else {
          // Child is in memory, so its register dies immediately after the
          // store
    ...
    if (hasMemRef)  // Codegen usually evaluates address child last
      self()->simulateMemoryReference(&memref, node->getChild(0), state,
                                      summary);
  }

  if (self()->comp()->getOption(TR_TraceRegisterPressureDetails)) {
    traceMsg(self()->comp(),
             "state->_gprPressure = %d summary->_gprPressure = %d "
             "summary->PRESSURE_LIMIT = %d\n",
             state->_gprPressure, summary->_gprPressure,
             summary->PRESSURE_LIMIT);
  }

  if (summary->_gprPressure < summary->PRESSURE_LIMIT)
    TR_ASSERT(state->_gprPressure <= summary->_gprPressure,
              "Children of %s must record max register gprPressure in summary; "
              "%d > %d",
              self()->getDebug()->getName(node), state->_gprPressure,
              summary->_gprPressure);
  if (summary->_fprPressure < summary->PRESSURE_LIMIT)
    TR_ASSERT(state->_fprPressure <= summary->_fprPressure,
              "Children of %s must record max register fprPressure in summary; "
              "%d > %d",
              self()->getDebug()->getName(node), state->_fprPressure,
              summary->_fprPressure);
  if (summary->_vrfPressure <= summary->PRESSURE_LIMIT)
    TR_ASSERT(state->_vrfPressure <= summary->_vrfPressure,
              "Children of %s must record max register vrfPressure in summary; "
              "%d > %d",
              self()->getDebug()->getName(node), state->_vrfPressure,
              summary->_vrfPressure);
  ...
  if (nodeGotFoldedIntoMemref(node, state, self()) &&
      self()->simulatedNodeState(node)._willBeRematerialized ==
          0  // remat nodes have already had their children incremented, so we
             // must decrement them
  ) {
    ...
    if (callNodeLinkage) {
      if (node->getNumChildren() >= 1) tag = " decRegArgs";

      if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
        traceMsg(self()->comp(),
                 " childrenInRegisters=" UINT64_PRINTF_FORMAT_HEX,
                 childrenInRegisters.getValue());

      for (int32_t childIndex = std::min<int32_t>(maxChildrenInRegisters,
                                                  node->getNumChildren()) -
                                1;
      ...
      if (node->getNumChildren() >= 1) tag = " decChildren";

      for (int32_t childIndex = 0; childIndex < node->getNumChildren();
           childIndex++)
        self()->simulateDecReferenceCount(node->getChild(childIndex), state);
    }
...
bool OMR::CodeGenerator::nodeWillBeRematerialized(
    TR::Node *node, TR_RegisterPressureState *state) {
  ...
  // is what we want.
  //
  if (!state->_candidate) return false;

  // Nodes that aren't commoned can't possibly be rematerialized
  //
  if (node->getFutureUseCount() <= 1) return false;
  ...
  if (state->_currentMemref) {
    if (op.isArrayRef())
      ...
      bool isByConst = (node->getNumChildren() >= 2)
                           ? (node->getSecondChild()->getOpCode().isLoadConst())
                           : false;
      if (self()->getSupportsConstantOffsetInAddressing() &&
          (op.isAdd() || op.isSub()) && isByConst)
        result = true;
      else if (self()->getSupportsScaledIndexAddressing() &&
               (op.isMul() || op.isLeftShift()) && isByConst)
        result = true;
    }
  }
...
void OMR::CodeGenerator::simulateMemoryReference(
    TR_SimulatedMemoryReference *memref, TR::Node *node,
...
static int64_t powerOfTwoScaleFactor(
    TR::Node *node)  // INT_MAX means the node doesn't represent a fixed-point
                     // mul by a power of 2
{
  TR::ILOpCode &op = node->getOpCode();
  if (!op.isInteger() && !op.isLong()) return INT_MAX;

  if (node->getNumChildren() < 2) return INT_MAX;

  TR::Node *constChild = node->getSecondChild();
  if (!constChild->getOpCode().isLoadConst()) return INT_MAX;

  int64_t value = (constChild->getOpCode().isLong()) ? constChild->getLongInt()
                                                     : constChild->getInt();

  if (op.isMul() && !(value & (value - 1))) return value;

  if (op.isLeftShift() &&
      value <= 31)  // Assuming platforms can't do 4GB strides anyway
    return ((int64_t)1 << value);

  return INT_MAX;
}

bool OMR::CodeGenerator::nodeCanBeFoldedIntoMemref(
    TR::Node *node, TR_RegisterPressureState *state) {
  // Based on populateMemoryReference, but no recursion because that's done at a
  // higher level

  TR::ILOpCode &op = node->getOpCode();
  TR::Node *firstChild =
      (node->getNumChildren() >= 1) ? node->getFirstChild() : NULL;
  TR::Node *secondChild =
      (node->getNumChildren() >= 2) ? node->getSecondChild() : NULL;
  bool isIntOrAddress = (op.isInteger() || node->getType().isAddress());
  bool isByConst =
      secondChild ? (secondChild->getOpCode().isLoadConst()) : false;
  bool isByIntOrLong = secondChild ? (secondChild->getType().isInt32() ||
                                      secondChild->getType().isInt64())
                                   : false;

  bool result = false;

  if ((self()->simulatedNodeState(node).hasRegister() ||
       node->getFutureUseCount() > 1) &&
    ...
    result = true;
  } else if (powerOfTwoScaleFactor(node) <=
             (TR_SimulatedMemoryReference::MAX_STRIDE)) {
    ...
    result = true;
  } else if ((op.getOpCodeValue() == TR::loadaddr) ||
             (op.isLoadConst() && isIntOrAddress)) {
...
bool OMR::CodeGenerator::nodeResultConsumesNoRegisters(
    TR::Node *node, TR_RegisterPressureState *state) {
  TR::ILOpCode &op = node->getOpCode();
  if (op.isTreeTop()) {
    // Some treetop nodes lie about their return type
    return true;
  } else if (state->_currentMemref &&
             self()->nodeCanBeFoldedIntoMemref(node, state)) {
    return true;
  }
...
uint8_t OMR::CodeGenerator::nodeResultGPRCount(
    TR::Node *node, TR_RegisterPressureState *state) {
  if (self()->nodeResultConsumesNoRegisters(node, state))
    ...
    if (result == 2 && !node->getType().isAggregate() &&
        node->isHighWordZero() && node->getFutureUseCount() <= 1)
      result = 1;
    return result;
  }
}

uint8_t OMR::CodeGenerator::nodeResultFPRCount(
    TR::Node *node, TR_RegisterPressureState *state) {
  if (self()->nodeResultConsumesNoRegisters(node, state))
    return 0;
  else if (node->getOpCodeValue() == TR::PassThrough)
...
uint8_t OMR::CodeGenerator::nodeResultVRFCount(
    TR::Node *node, TR_RegisterPressureState *state) {
  if (self()->nodeResultConsumesNoRegisters(node, state))
    return 0;
  else if (node->getOpCodeValue() == TR::PassThrough)
...
uint8_t OMR::CodeGenerator::nodeResultSSRCount(
    TR::Node *node, TR_RegisterPressureState *state) {
#ifdef J9_PROJECT_SPECIFIC
...
uint8_t OMR::CodeGenerator::nodeResultARCount(TR::Node *node,
                                              TR_RegisterPressureState *state) {
  if (node->getOpCodeValue() == TR::PassThrough)
...
void OMR::CodeGenerator::getNumberOfTemporaryRegistersUsedByCall(
    TR::Node *node, TR_RegisterPressureState *state,
    uint32_t &numGPRTemporaries, uint32_t &numFPRTemporaries,
    uint32_t &numVRFTemporaries) {
  int32_t numGPRVolatileRegs = 0, numGPRLinkageRegs = 0, numFPRVolatileRegs = 0,
          numFPRLinkageRegs = 0, numVRFVolatileRegs = 0, numVRFLinkageRegs = 0;

  {
    TR::StackMemoryRegion stackMemoryRegion(*self()->trMemory());
    TR_BitVector bv(self()->getNumberOfGlobalRegisters(), self()->trMemory(),
                    stackAlloc, growable);
    ...
    bv |= *self()->getGlobalRegisters(
        ...
        self()
            ...
            ->getLinkageConvention());  // vmthread, when present, is also a
  ...
  for (int32_t i = node->getFirstArgumentIndex(); i < node->getNumChildren();
       i++) {
    TR::Node *child = node->getChild(i);
    numGPRArgs = std::min(
        numGPRLinkageRegs,
        numGPRArgs + self()->simulatedNodeState(child, state)._liveGPRs);
    numFPRArgs = std::min(
        numFPRLinkageRegs,
        numFPRArgs + self()->simulatedNodeState(child, state)._liveFPRs);
    numVRFArgs = std::min(
        numVRFLinkageRegs,
        numVRFArgs + self()->simulatedNodeState(child, state)._liveVRFs);
  }
...
void OMR::CodeGenerator::simulateNodeInitialization(
    TR::Node *node, TR_RegisterPressureState *state) {
...
void OMR::CodeGenerator::simulateNodeGoingLive(
    TR::Node *node, TR_RegisterPressureState *state) {
  TR_ASSERT(state->isInitialized(node),
            "Node %s should have been initialized before going live",
            self()->getDebug()->getName(node));
  ...
  for (uint16_t i = 0; i < node->getNumChildren(); i++) {
    TR::Node *child = node->getChild(i);
    TR_SimulatedNodeState &childNodeState =
        self()->simulatedNodeState(child, state);
    if (childNodeState._willBeRematerialized &&
        childNodeState._childRefcountsHaveBeenDecremented) {
      if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
  ...
  //
  if (self()->traceSimulateTreeEvaluation()) {
    self()->getDebug()->dumpSimulatedNode(
        node, self()->isCandidateLoad(node, state) ? 'C' : ' ');
    traceMsg(self()->comp(), "%2d(%d) g%+d=%-2d f%+d=%-2d v%+d=%-2d",
             self()->simulatedNodeState(node, state)._height,
             node->getNumChildren(), self()->nodeResultGPRCount(node, state),
             state->_gprPressure, self()->nodeResultFPRCount(node, state),
             state->_fprPressure, self()->nodeResultVRFCount(node, state),
             state->_vrfPressure);
    standardNodeSimulationAnnotations(state, self()->comp());
  }
}

void OMR::CodeGenerator::simulateNodeGoingDead(
    TR::Node *node, TR_RegisterPressureState *state) {
  TR_SimulatedNodeState &nodeState = self()->simulatedNodeState(node);
  ...
  if (self()->isCandidateLoad(node, state) && state->candidateIsLiveOnEntry() &&
    ...
    // There's another candidate load coming up, and that one will be live on
    // entry too
    state->_pressureRiskFromStart = true;
    if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
      traceMsg(self()->comp(), " *%s", self()->getDebug()->getName(node));
  } else if (self()->isLoadAlreadyAssignedOnEntry(node, state) &&
             (node != findCandidate(node->getSymbolReference(),
                                    state->_candidatesAlreadyAssigned)
                          ->getLastLoad())) {
  ...
  if (self()->isCandidateLoad(node, state) && nodeState._liveCandidateLoad) {
    state->_numLiveCandidateLoads--;
    TR_ASSERT(state->_numLiveCandidateLoads >= 0,
              "numLiveCandidateNodes must never be negative");
    nodeState._liveCandidateLoad = 0;
  } else {
    nodeState._liveGPRs = nodeState._liveFPRs = nodeState._liveVRFs =
        nodeState._liveSSRs = nodeState._liveARs = 0;
    TR_ASSERT(!nodeState.hasRegister(), "assertion failure");
  }
}

void OMR::CodeGenerator::simulateDecReferenceCount(
    TR::Node *node, TR_RegisterPressureState *state) {
  self()->simulateNodeInitialization(node, state);
  ...
  TR_ASSERT(node->getFutureUseCount() > 0,
            "Too many simulated refcount decrements on node %s, refcount %d",
  ...
  if (node->decFutureUseCount() == 0) {
    ...
    if (nodeState._childRefcountsHaveBeenDecremented) {
      if (nodeState._keepLiveUntil)  // && !simulatedNodeState(node,
                                     // state)._willBeRematerialized)
      {
        // GRA will cause this node to stay live until the end of a block, so
        // don't remove it from the pressure estimates yet.
        //
        // TODO: If the node being kept alive is a RegLoad, this logic will
        // cause it to be counted both as a "liveCandidateLoad" as well as
        // pressure risk, which makes the logs look confusing because you get
        // both a "|" and a "+", making it look like there are two values of the
        // candidate live at the same time.  This should be counted ONLY as
        // pressure risk, not as a live candidate load, because the latter
        // should include only virtual regs that would be live even in the
        // absence of GRA.  I'm thinking I should be calling
        // simulateNodeGoingDead here, but I'm not sure of all the ramifications
        // of that.
        //
        if (state->_candidate &&
            node == state->_candidate->getMostRecentValue()) {
      ...
      // node was never evaluated, so all its refs to its children were not
      // actually needed. This is kind of like a lazy
...
void OMR::CodeGenerator::TR_SimulatedMemoryReference::
    simulateDecNodeReferenceCounts(TR_RegisterPressureState *state,
                                   TR::CodeGenerator *cg) {
...
void OMR::CodeGenerator::TR_RegisterPressureSummary::accumulate(
    TR_RegisterPressureState *state, TR::CodeGenerator *cg, uint32_t gprTemps,
    uint32_t fprTemps, uint32_t vrfTemps) {
