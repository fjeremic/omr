...
void OMR::CodeGenerator::estimateRegisterPressure(TR::Node* node, int32_t& registerPressure, int32_t& maxRegisterPressure, int32_t maxRegisters, TR_BitVector* valuesInGlobalRegs, bool isCold, vcount_t visitCount, TR::SymbolReference* symRef, bool& symRefIsLive, bool checkForIMuls, bool& vmThreadUsed)
{
    if (node->getVisitCount() == visitCount) {
        node->setLocalIndex(node->getLocalIndex() - 1);
        if (node->getLocalIndex() == 0) {

            if (node->getOpCode().isLoadVar() && node->getSymbol()->isAutoOrParm() && valuesInGlobalRegs && valuesInGlobalRegs->get(node->getSymbolReference()->getReferenceNumber())) {
                //
    ...
    if (node->getOpCode().isResolveCheck() || (node->getOpCode().isCall()) || node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::multianewarray || node->getOpCodeValue() == TR::MergeNew || node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit || node->getOpCodeValue() == TR::checkcast || node->getOpCodeValue() == TR:: instanceof || node->getOpCodeValue() == TR::arraycopy)
    ...
    if (node->getLocalIndex() > 0) {
        if (node->getOpCode().isLoadVar() && node->getSymbol()->isAutoOrParm() && valuesInGlobalRegs && valuesInGlobalRegs->get(node->getSymbolReference()->getReferenceNumber())) {
            ...
            if (!symRefIsLive) {
                ...
                //
                if (highRegisterPressureOpCode || ((node->getType().isInt64()) && TR::Compiler->target.is32Bit() && (node->getOpCode().isMul() || node->getOpCode().isDiv() || node->getOpCode().isRem() || node->getOpCode().isLeftShift() || node->getOpCode().isRightShift() || node->getOpCode().isBooleanCompare()))) {
                    if (!isCold) {
...
OMR::CodeGenerator::estimateRegisterPressure(TR::Block* block, vcount_t visitCount, int32_t maxStaticFrequency, int32_t maxFrequency, bool& vmThreadUsed, int32_t numGlobalRegs, TR_BitVector* valuesInGlobalRegs, TR::SymbolReference* symRef, bool checkForIMuls)
{
    ...
    while (treeTop) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            // Use some measure for coldness
            //
            if (node->getBlock()->isCold() || ((maxFrequency > 0) && (node->getBlock()->getFrequency() * 100 / maxFrequency < 20)))
                isCold = true;
            else {
                TR_BlockStructure* blockStructure = block->getStructureOf();
                int32_t blockWeight = 1;
...
void OMR::CodeGenerator::findCommonedReferences(TR::Node* node, TR::TreeTop* treeTop)
{
    ...
    for (int32_t childCount = node->getNumChildren() - 1; childCount >= 0; childCount--) {
        ...
        if (child->getDataType() == TR::Address && !child->getOpCode().isLoadConst() && child->getOpCodeValue() != TR::loadaddr) {
...
void OMR::CodeGenerator::spillLiveReferencesToTemps(TR::TreeTop* insertionTree, std::list<TR::SymbolReference*, TR::typed_allocator<TR::SymbolReference*, TR::Allocator>>::iterator firstAvailableSpillTemp)
{
    ...
    TR_LinkHead<TR_RegisterCandidate>* candidatesAlreadyAssigned)
{
    ...
    if (1) {
        TR::DataType dtype = rc->getDataType();
        uint8_t regsWithheld = (dtype == TR::Float
                                   || dtype == TR::Double
#ifdef J9_PROJECT_SPECIFIC
                                   || dtype == TR::DecimalFloat
                                   || dtype == TR::DecimalDouble
            ...
            : (self()->comp()->getMethodHotness() == warm) ? gprsWithheldFromPickRegisterWhenWarm : gprsWithheldFromPickRegister;
...
void OMR::CodeGenerator::computeSimulatedSpilledRegs(TR_BitVector* spilledRegisters, int32_t blockNum, TR::SymbolReference* candidate)
{
...
void OMR::CodeGenerator::computeSpilledRegsForAllPresentLinkages(TR_BitVector* spilledRegisters, TR_RegisterPressureSummary& summary)
{
...
void OMR::CodeGenerator::setSpilledRegsForAllPresentLinkages(TR_BitVector* spilledRegisters, TR_RegisterPressureSummary& summary, TR_SpillKinds spillKind)
{
...
{
    return node->getOpCode().isLoadVarDirect() && (state->_alreadyAssignedOnEntry.isSet(node->getSymbolReference()->getReferenceNumber()));
}
    ...
    TR_RegisterPressureSummary* summary)
{
    ...
    if (1) {
        blockIsLoopHeader = (b = block->getStructureOf()) && (loop = b->getContainingLoop()) && (loop->getNumber() == block->getNumber());
    }
...
nodeGotFoldedIntoMemref(
    ...
    TR::CodeGenerator* cg)
{
    return (state->_currentMemref
        && !cg->isCandidateLoad(node, state)
        && cg->nodeResultGPRCount(node, state) == 0
        && cg->nodeResultFPRCount(node, state) == 0);
}

void OMR::CodeGenerator::simulateTreeEvaluation(TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
...
void OMR::CodeGenerator::simulateSkippedTreeEvaluation(TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary, char tagChar)
{
...
void OMR::CodeGenerator::simulateNodeEvaluation(TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
...
void OMR::CodeGenerator::simulateMemoryReference(TR_SimulatedMemoryReference* memref, TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
...
void OMR::CodeGenerator::getNumberOfTemporaryRegistersUsedByCall(TR::Node* node, TR_RegisterPressureState* state, uint32_t& numGPRTemporaries, uint32_t& numFPRTemporaries, uint32_t& numVRFTemporaries)
{
