...
TR_BackingStore *
OMR::CodeGenerator::allocateSpill(bool containsCollectedReference,
                                  int32_t *offset, bool reuse) {
...
TR_BackingStore *
OMR::CodeGenerator::allocateSpill(int32_t dataSize,
  ...
  if (1) {
    ...
    for (bestLowRegisterNumber = -1;
      ...
      case 'P': // Any preserved register
        TR_ASSERT(
            preservedRegisters,
            "Must have preservedRegisters info to select a preserved register");
      ...
      case 'N': // Any non-preserved register
        TR_ASSERT(preservedRegisters, "Must have preservedRegisters info to "
                                      "select a non-preserved register");
        TR_ASSERT(linkageRegister == -1, "Candidates arriving in linkage regs "
                                         "can't be given other linkage regs");
        //
...
OMR::CodeGenerator::findUsedCandidate(TR::Node *node, TR_RegisterCandidate *rc,
...
static TR_RegisterCandidate *
findCandidate(TR::SymbolReference *symRef,
...
static void
rememberMostRecentValue(TR::SymbolReference *symRef, TR::Node *valueNode,
                        OMR::CodeGenerator::TR_RegisterPressureState *state,
                        TR::CodeGenerator *cg) {
...
static bool
nodeGotFoldedIntoMemref(TR::Node *node,
                        OMR::CodeGenerator::TR_RegisterPressureState *state,
                        TR::CodeGenerator *cg) {
...
void OMR::CodeGenerator::simulateNodeEvaluation(
  ...
  flags32_t childrenInRegisters(
      0); // (Don't bother with a bitvector on the slight chance that some child
          // beyond the first 32 will get in a register)
...
uint8_t
OMR::CodeGenerator::nodeResultGPRCount(TR::Node *node,
                                       TR_RegisterPressureState *state) {
...
uint8_t
OMR::CodeGenerator::nodeResultFPRCount(TR::Node *node,
                                       TR_RegisterPressureState *state) {
  if (self()->nodeResultConsumesNoRegisters(node, state))
    return 0;
  else if (node->getOpCodeValue() == TR::PassThrough)
...
uint8_t
OMR::CodeGenerator::nodeResultVRFCount(TR::Node *node,
                                       TR_RegisterPressureState *state) {
  if (self()->nodeResultConsumesNoRegisters(node, state))
    return 0;
  else if (node->getOpCodeValue() == TR::PassThrough)
...
uint8_t
OMR::CodeGenerator::nodeResultSSRCount(TR::Node *node,
                                       TR_RegisterPressureState *state) {
#ifdef J9_PROJECT_SPECIFIC
