    ...
    for (TR_LiveRegisterInfo* p = liveRegisters->getFirstLiveRegister(); p;
      ...
      if (p->getNode())
        traceMsg(self()->comp(),
                 ...
                 "end of method\n",
                 debug->getName(p->getRegister()),
                 debug->getName(p->getNode()),
...
void
OMR::CodeGenerator::estimateRegisterPressure(TR::Node* node,
                                             ...
                                             bool& vmThreadUsed)
{
  ...
  for (childCount = node->getNumChildren() - 1; childCount >= 0; childCount--)
    self()->estimateRegisterPressure(node->getChild(childCount),
                                     registerPressure,
                                     maxRegisterPressure,
                                     maxRegisters,
                                     valuesInGlobalRegs,
...
int32_t
OMR::CodeGenerator::estimateRegisterPressure(TR::Block* block,
                                             vcount_t visitCount,
...
TR_BackingStore*
OMR::CodeGenerator::allocateInternalPointerSpill(
  TR::AutomaticSymbol* pinningArrayPointer)
{
  ...
  for (auto i = self()->getInternalPointerSpillFreeList().begin();
       i != self()->getInternalPointerSpillFreeList().end();
       ++i) {
    if ((*i)
          ->getSymbolReference()
          ->getSymbol()
                                  ...
                                  bool reuse)
{
  return self()->allocateSpill(
    ...
    offset,
    reuse); // cast parameter explicitly
}
                                  ...
                                  bool reuse)
{
  ...
  if (reuse) {
    ...
    if ((spill && self()->getTraceRAOption(TR_TraceRASpillTemps) &&
         !performTransformation(
           self()->comp(),
           "O^O SPILL TEMPS: Reuse spill temp %s\n",
  ...
  if (spill) {
    ...
    spillSymbol = TR::AutomaticSymbol::create(
      self()->trHeapMemory(), TR::NoType, spillSize);
    spillSymbol->setSpillTempAuto();
    self()->comp()->getMethodSymbol()->addAutomatic(spillSymbol);
    spill = new (self()->trHeapMemory())
      TR_BackingStore(self()->comp()->getSymRefTab(), spillSymbol, 0);
  ...
  if (dataSize <= 4 && spillSymbol->getSize() == 8) {
    ...
    if (offset == NULL || spill->secondHalfIsOccupied() ||
        !performTransformation(
          self()->comp(),
          "O^O HALF-SLOT SPILLS: Use second half of %s\n",
          self()->getDebug()->getName(spill->getSymbolReference()))) {
...
void
OMR::CodeGenerator::freeSpill(TR_BackingStore* spill,
                              int32_t dataSize,
                              int32_t offset)
{
...
TR::Register*
OMR::CodeGenerator::allocateRegister(TR_RegisterKinds rk)
{
...
void
OMR::CodeGenerator::processReference(TR::Node* reference,
                                     TR::Node* parent,
                                     TR::TreeTop* treeTop)
{
      ...
      if (reference->getReferenceCount() ==
        ...
        if (reference->getOpCode().hasSymbolReference()) {
          ...
          if (symRef->getSymbol()->isSpillTempAuto()) {
            ...
            if (debug("traceSpill"))
              diagnostic(
                "Release spill temp [%p] at node [%p]\n", symRef, reference);
          }
        }
      }
...
void
OMR::CodeGenerator::spillLiveReferencesToTemps(
  TR::TreeTop* insertionTree,
  std::list<TR::SymbolReference*,
            TR::typed_allocator<TR::SymbolReference*, TR::Allocator>>::iterator
    firstAvailableSpillTemp)
{
...
void
OMR::CodeGenerator::needSpillTemp(TR_LiveReference* cursor,
                                  TR::Node* parent,
                                  TR::TreeTop* treeTop)
{
  // If the reference is a simple load and the load is not killed before the
...
bool
OMR::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate*,
                                                    TR::Node* branchNode)
{
...
static bool
blockIsMuchColderThanContainingLoop(TR::Block* block, TR::CodeGenerator* cg)
{
...
static bool
blockIsIgnorablyCold(TR::Block* block, TR::CodeGenerator* cg)
{
...
void
OMR::CodeGenerator::TR_RegisterPressureState::updateRegisterPressure(
  TR::Symbol* symbol)
{
...
OMR::CodeGenerator::pickRegister(
  ...
  TR_LinkHead<TR_RegisterCandidate>* candidatesAlreadyAssigned)
{
  ...
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    while (blockIterator.hasMoreElements()) {
      ...
      if (!block->isExtensionOfPreviousBlock() &&
        ...
        TR_RegisterPressureState blockEntryState(
          NULL,
          false,
          alreadyAssignedOnEntry,
          alreadyAssignedOnExit,
        ...
        if (canExitEarly) {
          ...
          if (self()->traceSimulateTreeEvaluation()) {
            ...
            traceMsg(self()->comp(), "\n            remaining registers: ");
            self()->getDebug()->print(
              self()->comp()->getOptions()->getLogFile(), &remainingRegisters);
            traceMsg(self()->comp(), "\n");
          }
        }
      }
    ...
    TR_BitVector visitedNodesForCandidateUse(
      0, self()->trMemory(), stackAlloc, growable);
    ...
    while (blockIterator.hasMoreElements()) {
      ...
      if (block != self()->comp()->getFlowGraph()->getEnd() &&
        ...
        //
        int32_t blockWeight =
          1; // Note that we use a static loop-nesting-depth heuristic here for
             // consistency with the rest of GRA
            ...
            if (candidate) {
              ...
              } else // is Unpreferred
              {
                if (self()->traceSimulateTreeEvaluation()) {
                  traceMsg(
                    self()->comp(),
                    "            unprefer copy to/from #%d, register %d (%s), "
                    "weight %d, at %s\n",
                    candidate->getSymbolReference()->getReferenceNumber(),
            ...
            if ((node->getOpCodeValue() == TR::treetop) &&
              ...
              for (uint32_t i = 0; i < callNode->getNumChildren(); i++) {
                ...
                TR_GlobalRegisterNumber candidateRegister =
                  self()->findCoalescenceRegisterForParameter(
                    callNode, rc, i, &isUnpreferred);
    ...
    if (remainingRegisters.isEmpty()) {
      ...
      if (self()->terseSimulateTreeEvaluation())
        traceMsg(self()->comp(),
                 ...
                 result,
                 self()->getDebug()->getGlobalRegisterName(result),
                 rc->getSymbolReference()->getReferenceNumber(),
                 self()->getDebug()->getName(rc->getSymbolReference()));
      ...
      while (
        blockIterator.hasMoreElements() &&
        !availableRegisters
           .isEmpty()) // TODO: Can availableRegisters possibly be empty here?
      {
        TR::Block* block = allBlocks[blockIterator.getNextElement()];
        if (!block->isExtensionOfPreviousBlock()) {
          TR_RegisterPressureSummary* cachedSummary =
  ...
  if (1) {
    ...
    if (debug("tracePickRegister") || self()->comp()->getOptions()->trace(
                                        OMR::tacticalGlobalRegisterAllocator)) {
      traceMsg(self()->comp(),
               "pickRegister: Candidate #%d {\n",
               rc->getSymbolReference()->getReferenceNumber());
    }
    ...
    if (rcSymbol->isParm()) {
      ...
      if (lri >= 0) {
        linkageRegister =
          self()->getLinkageGlobalRegisterNumber(lri, rcSymbol->getDataType());
      }
    }
    if (debug("tracePickRegister") || self()->comp()->getOptions()->trace(
                                        OMR::tacticalGlobalRegisterAllocator)) {
      if (linkageRegister == -1) {
        ...
        traceMsg(self()->comp(),
                 "pickRegister:\tLinkage register #%d\n",
                 linkageRegister);
      }
    }
    ...
    if (debug("tracePickRegister") || self()->comp()->getOptions()->trace(
                                        OMR::tacticalGlobalRegisterAllocator)) {
      traceMsg(
        self()->comp(), "pickRegister:\tPreference order: %s\n", preferences);
    }
    ...
    for (bestLowRegisterNumber = -1;
         bestLowRegisterNumber == -1 && preferences[0];
         preferences++) {
    ...
    if (debug("tracePickRegister") || self()->comp()->getOptions()->trace(
      ...
      if (bestLowRegisterNumber == -1) {
        ...
        traceMsg(self()->comp(),
                 "pickRegister:\tPicked global register #%d\n",
...
OMR::CodeGenerator::findCoalescenceForRegisterCopy(TR::Node* node,
                                                   TR_RegisterCandidate* rc,
                                                   bool* isUnpreferred)
{
...
TR_GlobalRegisterNumber
OMR::CodeGenerator::findCoalescenceRegisterForParameter(
  TR::Node* callNode,
  TR_RegisterCandidate* rc,
  uint32_t childIndex,
  bool* isUnpreferred)
...
TR_RegisterCandidate*
OMR::CodeGenerator::findUsedCandidate(TR::Node* node,
                                      TR_RegisterCandidate* rc,
...
void
OMR::CodeGenerator::computeSimulatedSpilledRegs(TR_BitVector* spilledRegisters,
                                                int32_t blockNum,
...
void
OMR::CodeGenerator::computeSpilledRegsForAllPresentLinkages(
  ...
  TR_RegisterPressureSummary& summary)
{
  for (int32_t i = 0; i < TR_numProbableSpillKinds; i++) {
    ...
    if (summary.isSpilled(spillKind))
      self()->setSpilledRegsForAllPresentLinkages(
        spilledRegisters, summary, spillKind);
  }
}

void
OMR::CodeGenerator::setSpilledRegsForAllPresentLinkages(
  TR_BitVector* spilledRegisters,
  TR_RegisterPressureSummary& summary,
...
void
OMR::CodeGenerator::TR_RegisterPressureState::addVirtualRegister(
  TR::Register* reg)
{
  if (reg == NULL)
    return;
...
void
OMR::CodeGenerator::TR_RegisterPressureState::removeVirtualRegister(
  TR::Register* reg)
{
  if (reg == NULL)
    return;
...
bool
OMR::CodeGenerator::TR_RegisterPressureState::isInitialized(TR::Node* node)
{
  return node->getVisitCount() == _visitCountForInit;
}

void
OMR::CodeGenerator::TR_RegisterPressureSummary::setLinkagePresent(
...
void
OMR::CodeGenerator::TR_RegisterPressureSummary::spill(TR_SpillKinds kind,
                                                      TR::CodeGenerator* cg)
{
  _spillMask |= (1 << kind);
  if (cg->traceSimulateTreeEvaluation())
...
void
OMR::CodeGenerator::TR_RegisterPressureSummary::dumpSpillMask(
  TR::CodeGenerator* cg)
{
  if (cg->traceSimulateTreeEvaluation()) {
    for (int32_t i = 0; i < TR_numSpillKinds; i++) {
      ...
      if (isSpilled(spillKind))
        traceMsg(
          cg->comp(), " %s", cg->getDebug()->getSpillKindName(spillKind));
    }
  }
}

void
OMR::CodeGenerator::TR_RegisterPressureSummary::dumpLinkageConventionMask(
  TR::CodeGenerator* cg)
{
  if (cg->traceSimulateTreeEvaluation()) {
    for (int32_t i = 0; i < TR_NumLinkages; i++) {
      ...
      if (isLinkagePresent(lc))
        traceMsg(
          cg->comp(), " %s", cg->getDebug()->getLinkageConventionName(lc));
    }
  }
}

bool
OMR::CodeGenerator::isCandidateLoad(TR::Node* node,
                                    TR::SymbolReference* candidate)
{
...
bool
OMR::CodeGenerator::isLoadAlreadyAssignedOnEntry(
  TR::Node* node,
  TR_RegisterPressureState* state)
{
  return node->getOpCode().isLoadVarDirect() &&
...
uint8_t
OMR::CodeGenerator::gprCount(TR::DataType type, int32_t size)
{
  if (type.isAggregate())
...
void
OMR::CodeGenerator::addToUnlatchedRegisterList(TR::RealRegister* reg)
{
...
inline void
standardNodeSimulationAnnotations(
...
inline void
leaveSpaceForRegisterPressureState(
  OMR::CodeGenerator::TR_RegisterPressureState* state,
  TR::Compilation* comp)
{
...
static void
rememberMostRecentValue(TR::SymbolReference* symRef,
                        TR::Node* valueNode,
                        OMR::CodeGenerator::TR_RegisterPressureState* state,
                        TR::CodeGenerator* cg)
{
...
void
OMR::CodeGenerator::simulateBlockEvaluation(TR::Block* block,
                                            TR_RegisterPressureState* state,
                                            TR_RegisterPressureSummary* summary)
{
  ...
  if (self()->traceSimulateTreeEvaluation()) {
    traceMsg(
      self()->comp(), "            { simulating block_%d", block->getNumber());
    if (state->_candidate)
      traceMsg(self()->comp(),
               ", candidate %d",
  ...
  while (tt && !state->mustAbort()) {
    ...
    if (tt->getNode()->getOpCode().isStoreDirect())
      rememberMostRecentValue(tt->getNode()->getSymbolReference(),
                              tt->getNode()->getFirstChild(),
                              state,
                              self());
    else if (tt->getNode()->getOpCode().isLoadVarDirect())
      rememberMostRecentValue(
        tt->getNode()->getSymbolReference(), tt->getNode(), state, self());
...
void
OMR::CodeGenerator::simulationPrePass(TR::TreeTop* tt,
                                      TR::Node* node,
                                      TR_RegisterPressureState* state,
                                      TR_RegisterPressureSummary* summary)
{
  ...
  if (node->getOpCode().isStoreDirect()) {
    rememberMostRecentValue(
      node->getSymbolReference(), node->getFirstChild(), state, self());
  } else if (node->getOpCode().isLoadVarDirect()) {
    rememberMostRecentValue(node->getSymbolReference(), node, state, self());
    TR_RegisterCandidate* rc = findCandidate(node->getSymbolReference(),
    ...
    for (auto e = node->getBlock()->getSuccessors().begin();
         e != node->getBlock()->getSuccessors().end();
         ++e) {
      if (state->_candidate)
        keepMostRecentValueAliveIfLiveOnEntryToSuccessor(
...
void
OMR::CodeGenerator::simulateTreeEvaluation(TR::Node* node,
                                           TR_RegisterPressureState* state,
                                           TR_RegisterPressureSummary* summary)
{
  ...
  if (self()->getHasResumableTrapHandler() &&
    ...
    bool canFoldIntoBNDCHK =
      (nextTreeTop && nextTreeTop->getNode()->getOpCodeValue() == TR::BNDCHK &&
  ...
  if (self()->getSupportsVirtualGuardNOPing() &&
    ...
    self()->simulateSkippedTreeEvaluation(
      node, state, summary, 'g'); // 'g' for "guard"
  ...
  if (nodeState.hasRegister()) {
    ...
    if (node->getOpCodeValue() == TR::BBEnd) {
      ...
      //
      killMostRecentValueIfKeptAliveUntilCurrentTreeTop(
        state->_candidate, state, self());
      for (TR_RegisterCandidate* candidate =
             state->_candidatesAlreadyAssigned->getFirst();
           candidate;
           candidate = candidate->getNext())
        killMostRecentValueIfKeptAliveUntilCurrentTreeTop(
          candidate, state, self());
    }
      ...
      if (isCall) {
        ...
        if ((numGPRTemporaries >= 1 || numFPRTemporaries >= 1 ||
            ...
            (self()->comp()->getOption(TR_TraceRegisterPressureDetails) &&
             !performTransformation(
               self()->comp(),
               "O^O GLOBAL REGISTER ALLOCATION: Call %p has %d GPR and %d FPR "
               "and VRF %d dummy registers\n",
               node,
               numGPRTemporaries,
               numFPRTemporaries,
               numVRFTemporaries))) {
    ...
    //
    summary->accumulate(
      state, self(), numGPRTemporaries, numFPRTemporaries, numVRFTemporaries);
...
void
OMR::CodeGenerator::simulateSkippedTreeEvaluation(
  TR::Node* node,
  TR_RegisterPressureState* state,
  TR_RegisterPressureSummary* summary,
  char tagChar)
...
void
OMR::CodeGenerator::simulateNodeEvaluation(TR::Node* node,
                                           TR_RegisterPressureState* state,
                                           TR_RegisterPressureSummary* summary)
{
  ...
  if (evaluateSecondChildFirst) {
    ...
    for (int32_t childIndex = (hasMemRef ? 1 : 0);
         ...
         childIndex++) {
      self()->simulateTreeEvaluation(
        node->getChild(childIndex), state, summary);
      if (callNodeLinkage) {
    ...
    if (hasMemRef) // Codegen usually evaluates address child last
      self()->simulateMemoryReference(
        &memref, node->getChild(0), state, summary);
  }
  ...
  if (summary->_fprPressure < summary->PRESSURE_LIMIT)
    TR_ASSERT(
      state->_fprPressure <= summary->_fprPressure,
  ...
  if (summary->_vrfPressure <= summary->PRESSURE_LIMIT)
    TR_ASSERT(
      state->_vrfPressure <= summary->_vrfPressure,
  ...
  if (nodeGotFoldedIntoMemref(node, state, self()) &&
      self()->simulatedNodeState(node)._willBeRematerialized ==
        0 // remat nodes have already had their children incremented, so we must
          // decrement them
  ) {
    ...
    if (callNodeLinkage) {
      ...
      for (int32_t childIndex =
             std::min<int32_t>(maxChildrenInRegisters, node->getNumChildren()) -
             1;
           childIndex >= 0;
           childIndex--)
...
bool
OMR::CodeGenerator::nodeWillBeRematerialized(TR::Node* node,
                                             TR_RegisterPressureState* state)
{
...
void
OMR::CodeGenerator::simulateMemoryReference(TR_SimulatedMemoryReference* memref,
                                            TR::Node* node,
                                            TR_RegisterPressureState* state,
                                            TR_RegisterPressureSummary* summary)
{
...
static int64_t
powerOfTwoScaleFactor(
  TR::Node* node) // INT_MAX means the node doesn't represent a fixed-point mul
                  // by a power of 2
{
  TR::ILOpCode& op = node->getOpCode();
...
bool
OMR::CodeGenerator::nodeCanBeFoldedIntoMemref(TR::Node* node,
                                              TR_RegisterPressureState* state)
{
...
bool
OMR::CodeGenerator::nodeResultConsumesNoRegisters(
  TR::Node* node,
  TR_RegisterPressureState* state)
{
  TR::ILOpCode& op = node->getOpCode();
...
uint8_t
OMR::CodeGenerator::nodeResultARCount(TR::Node* node,
                                      TR_RegisterPressureState* state)
{
  if (node->getOpCodeValue() == TR::PassThrough)
...
void
OMR::CodeGenerator::getNumberOfTemporaryRegistersUsedByCall(
  TR::Node* node,
  TR_RegisterPressureState* state,
  ...
  uint32_t& numVRFTemporaries)
{
...

  {
    ...
    TR_BitVector bv(self()->getNumberOfGlobalRegisters(),
                    self()->trMemory(),
                    stackAlloc,
                    growable);
    ...
    bv |= *self()->getGlobalRegisters(
      ...
      self()
        ...
        ->getLinkageConvention()); // vmthread, when present, is also a virtual
                                   // reg not represented in trees
    bv &= *self()->getGlobalRegisters(
      TR_gprSpill,
      self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
...
void
OMR::CodeGenerator::simulateNodeInitialization(TR::Node* node,
                                               TR_RegisterPressureState* state)
{
...
void
OMR::CodeGenerator::simulateNodeGoingLive(TR::Node* node,
                                          TR_RegisterPressureState* state)
{
  ...
  if (self()->traceSimulateTreeEvaluation()) {
    ...
    traceMsg(self()->comp(),
             "%2d(%d) g%+d=%-2d f%+d=%-2d v%+d=%-2d",
             self()->simulatedNodeState(node, state)._height,
             node->getNumChildren(),
             self()->nodeResultGPRCount(node, state),
             state->_gprPressure,
             self()->nodeResultFPRCount(node, state),
             state->_fprPressure,
             self()->nodeResultVRFCount(node, state),
             state->_vrfPressure);
...
void
OMR::CodeGenerator::simulateNodeGoingDead(TR::Node* node,
                                          TR_RegisterPressureState* state)
{
  TR_SimulatedNodeState& nodeState = self()->simulatedNodeState(node);
...
void
OMR::CodeGenerator::simulateDecReferenceCount(TR::Node* node,
                                              TR_RegisterPressureState* state)
{
  self()->simulateNodeInitialization(node, state);
                                    ...
                                    // state)._willBeRematerialized)
      {
        ...
        if (state->_candidate &&
          ...
          if (!nodeState._isCausingPressureRiskUntilEnd) {
            ...
            if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
              traceMsg(
                self()->comp(), " keep:%s", self()->getDebug()->getName(node));
          }
        } else if (self()->comp()->getOption(TR_TraceRegisterPressureDetails))
          traceMsg(self()->comp(),
                   " keeping:%s",
                   self()->getDebug()->getName(
                     node)); // Some node other than the candidate's most recent
                             // value is being kept alive
      } else {
        if (nodeState._isCausingPressureRiskUntilEnd) {
          state->_pressureRiskUntilEnd--;
...
void
OMR::CodeGenerator::TR_SimulatedMemoryReference::simulateDecNodeReferenceCounts(
  TR_RegisterPressureState* state,
  TR::CodeGenerator* cg)
{
