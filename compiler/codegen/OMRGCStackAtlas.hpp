...
public:
  ...
  GCStackAtlas(uint32_t numParms, uint32_t numSlots, TR_Memory *m)
      : _atlasBits(NULL), _numberOfParmSlotsMapped(numParms),
        _numberOfSlotsMapped(numSlots),
#ifdef J9_PROJECT_SPECIFIC
        _numberOfPaddingSlots(0),
#endif
        _numberOfSlotsToBeInitialized(0), _numberOfPendingPushSlots(0),
        _parmBaseOffset(0), _trMemory(m), _localBaseOffset(0), _numberOfMaps(0),
        _parameterMap(NULL), _localMap(NULL), _indexOfFirstInternalPtr(0),
        _offsetOfFirstInternalPtr(0), _internalPointerMap(NULL),
        _pinningArrayPtrsForInternalPtrRegs(m), _stackAllocMap(NULL),
        _mapList(m), _hasUninitializedPinningArrayPointer(false) {
  }
...
#ifdef J9_PROJECT_SPECIFIC
  uint32_t _numberOfPaddingSlots; // number of padding slots used to align local
                                  // objects
#endif
