...
OMR::CodeGenerator::evaluate(TR::Node* node)
{
  ...
  if (opcode != TR::BBStart && node->getRegister()) {
    ...
    if (self()->comp()->getOption(TR_TraceRegisterPressureDetails)) {
      traceMsg(
        self()->comp(), "  evaluated %s", self()->getDebug()->getName(node));
    ...
    while (_stackOfArtificiallyInflatedNodes.topIndex() >
      ...
      if (self()->comp()->getOption(TR_TraceCG)) {
        self()->comp()->getDebug()->trace(
          " _stackOfArtificiallyInflatedNodes.pop() %p, decReferenceCount(...) "
          "called. reg=%s\n",
          artificiallyInflatedNode,
...
int32_t
OMR::CodeGenerator::whichNodeToEvaluate(TR::Node* first, TR::Node* second)
{
  ...
  if ((node->getReferenceCount() == 1) && reg &&
    ...
    TR_ASSERT(reg->isLive() ||
                (diagnostic("\n*** Error: Register %s for node "
                            ...
                            node->getName(self()->comp()->getDebug())),
                 0),
              "Node %s register should be live",
