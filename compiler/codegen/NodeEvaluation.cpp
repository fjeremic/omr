...
TR::Register* OMR::CodeGenerator::evaluate(TR::Node* node)
{
  ...
  if (opcode != TR::BBStart && node->getRegister()) {
      ...
      if (artificiallyInflatedNode->getReferenceCount() == 1) {
        ...
        if (self()->comp()->getOption(TR_TraceCG)) {
          self()->comp()->getDebug()->trace(
            " _stackOfArtificiallyInflatedNodes.pop(): node %p part of commoned case, might have avoided a bug!\n",
    ...
    TR_ASSERT(
      checkRefCount, "evaluate: the node's register wasn't set (node [%s])", node->getName(self()->comp()->getDebug()));
  }
  ...
  if (reg && reg->getOpaquePseudoRegister()) {
    ...
    if (node->getReferenceCount() == 1) {
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
          "\tdecrement owningRegisterCount %d->%d on ref #%d (%s) for reg %s as %s (%p) refCount == 1 (going to 0)\n",
