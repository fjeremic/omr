...
OMR::CodeGenerator::evaluate(TR::Node* node)
{
    ...
    if (opcode != TR::BBStart && node->getRegister()) {
        ...
        if (self()->comp()->useAnchors()) {
            if (((node->getOpCode().isStoreIndirect() && (self()->comp()->useCompressedPointers() && (node->getSymbolReference()->getSymbol()->getDataType() == TR::Address))) || opcode == TR::awrtbari) && node->getReferenceCount() <= 2 && !checkRefCount)
                checkRefCount = true;
        }
...
void OMR::CodeGenerator::evaluateChildrenWithMultipleRefCount(TR::Node* node)
{
    for (int i = 0; i < node->getNumChildren(); i++) {
        ...
        if (child->getRegister() == NULL) // not already evaluated
        {
            ...
            //
            if (child->getReferenceCount() > 1 && (child->getOpCode().hasSymbolReference() || (child->getOpCodeValue() == TR::l2a && child->getChild(0)->containsCompressionSequence()))) {
                ...
                if (node->isNopableInlineGuard()
                    ...
                    if (!child->getChild(0)->getChild(0)->getRegister() && child->getChild(0)->getChild(0)->getReferenceCount() > 1)
