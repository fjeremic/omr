...
TR::Register *OMR::CodeGenerator::evaluate(TR::Node *node) {
  ...
  bool trace =
      self()->comp()->getOptions()->getTraceCGOption(TR_TraceCGEvaluation);
  TR::ILOpCodes opcode = node->getOpCodeValue();

  TR_ASSERT(!self()->comp()->getOption(TR_EnableParanoidRefCountChecks) ||
                node->getOpCode().isTreeTop() || node->getReferenceCount() > 0,
            "OMR::CodeGenerator::evaluate invoked for nontreetop node [%s] "
            "with count == 0",
  ...
  if (opcode != TR::BBStart && node->getRegister()) {
    ...
    // can perform a destructive evaluation of their subtree, a bug can occur
    // where the results of a partial evalutation are destructively overwritten
    // before being completely evalutated.
    //
    ...
    // b) calling populateMemoryReference on node_B reduces the reference count
    // of one
    //    of the base or index nodes to 1, creating the "opportunity" for a
    //    destructive evaluation.
    //
    // The following chain of events occurs:
    //
    // 1) evaluate is called on node_A. This evaluator can produce instructions
    // of RX form,
    //    and chooses to do so for this node.
    //
    // 2) populateMemoryReference is called on node_B, and evaluates the
    // subtree, returning
    //    a TR_MemoryReference to node_A's evaluator. This memory reference has
    //    not been dereferenced yet, and the base (and optionally index) nodes
    //    may have registers assigned to them.
    //
    // 3) evaluate is called on node_C, which chooses to destructively evaluate
    // the commoned
    //    base node. The memory reference's base register now contains a garbage
    //    value.
    //
    // 4) control passes to node_A's evaluator, which emits an RX instruction
    // using node_B's
    //    memory reference and node_C's register.
    //
    // In the past, the fix for this was to switch the order of evaluation: call
    // evaluate on node_C and then call populateMemoryReference on node_B. This
    // fixes this scenario, but the capability of another tree and evaluation
    // pattern to create this bug still exists.
    //
    ...
    //       evaluate
    //
    // If the commoned node's reference count is 2 coming into ificmpeq's
    // evaluator, then the second sub-evaluate call could be destructive to the
    // first populateMemoryReference.
    //
    // Even worse, if either subtree could be destructive, then there would be
    // no correct order to perform the function calls:
    //
    // ificmpeq
    //    iiload
    //       ishl
    ...
    //
    // This code implements changes to eliminate the second condition for this
    // bug by performing the following check and fixup:
    //
    // If in a function which partially evaluates its subtree, note all
    // non-restricted nodes that have a reference count > 1. If any of those
    // node's reference counts reach 1, then artificially inflate those
    // reference counts by 1 for the lifetime of the parent evaluation.
    //
    int32_t topOfNodeStackBeforeEvaluation =
        _stackOfArtificiallyInflatedNodes.topIndex();

    // Memory references are not like registers, and should not be allowed to
    // escape their evaluator. Explicitly note memory references that are not
    // loaded into registers and automatically call stopUsingMemRefRegister on
    // all memory references that have "escaped".
    //
    // Only the s390 memory references are tracked in this way.
    //
    int32_t topOfMemRefStackBeforeEvaluation =
        _stackOfMemoryReferencesCreatedDuringEvaluation.topIndex();
    ...
    if (self()->comp()->getOption(TR_TraceRegisterPressureDetails)) {
      traceMsg(self()->comp(), "  evaluated %s",
    ...
    //
    while (_stackOfArtificiallyInflatedNodes.topIndex() >
           topOfNodeStackBeforeEvaluation) {
      TR::Node *artificiallyInflatedNode =
          _stackOfArtificiallyInflatedNodes.pop();

      if (artificiallyInflatedNode->getReferenceCount() == 1) {
        // When inflating reference counts, two cases exist:
        //
        // 1) N's ref count reaches 1 in a populate* call, which is then inc'ed
        // to 2.
        //
        // 1a) N is never evaluated, so the ref count never goes down to 1.
        // (node was not commoned in another subtree)
        //
        //     - no tree difference should be seen in this case.
        //
        // 1b) N is evaluated, so the ref count then goes down to 1. (node was
        // commoned in another subtree)
        //
        //     - register shuffling _could_ be seen in this case.
        //     - but a bug might have been avoided: partial and complete
        //     evaluation of a commoned node occurred.
        //
        if (self()->comp()->getOption(TR_TraceCG)) {
          self()->comp()->getDebug()->trace(
              " _stackOfArtificiallyInflatedNodes.pop(): node %p part of "
      ...
      if (artificiallyInflatedNode->getOpaquePseudoRegister()) {
        TR_OpaquePseudoRegister *reg =
            artificiallyInflatedNode->getOpaquePseudoRegister();
      ...
      if (self()->comp()->getOption(TR_TraceCG)) {
        self()->comp()->getDebug()->trace(
            " _stackOfArtificiallyInflatedNodes.pop() %p, "
            "decReferenceCount(...) called. reg=%s\n",
            artificiallyInflatedNode,
            artificiallyInflatedNode->getRegister()
                ? artificiallyInflatedNode->getRegister()->getRegisterName(
                      self()->comp())
                : "null");
      }
    }
    ...
    bool checkRefCount =
        (node->getReferenceCount() <= 1 || (reg && reg == node->getRegister()));
    ...
    TR_ASSERT(checkRefCount,
              "evaluate: the node's register wasn't set (node [%s])",
              node->getName(self()->comp()->getDebug()));
  }
...
int32_t OMR::CodeGenerator::whichNodeToEvaluate(TR::Node *first,
                                                TR::Node *second) {
...
int32_t OMR::CodeGenerator::whichChildToEvaluate(TR::Node *node) {
  ...
  for (int32_t childCount = 0; childCount < node->getNumChildren();
       ++childCount) {
...
int32_t OMR::CodeGenerator::getEvaluationPriority(TR::Node *node) {
  ...
  for (int32_t childCount = node->getNumChildren() - 1; childCount >= 0;
    ...
    if (childPriority >= nodePriority) nodePriority = childPriority + 1;
  }
...
rcount_t OMR::CodeGenerator::incReferenceCount(TR::Node *node) {
  ...
  if (reg && reg->getOpaquePseudoRegister()) {
    ...
    TR_StorageReference *pseudoRegStorageReference =
        pseudoReg->getStorageReference();

    TR_ASSERT(pseudoRegStorageReference,
...
rcount_t OMR::CodeGenerator::decReferenceCount(TR::Node *node) {
  ...
  if ((node->getReferenceCount() == 1) && reg &&
      self()->getLiveRegisters(reg->getKind())) {
    TR_ASSERT(
        reg->isLive() || (diagnostic("\n*** Error: Register %s for node "
                                     "[%s] died prematurely\n",
  ...
  if (reg && reg->getOpaquePseudoRegister()) {
    ...
    if (node->getReferenceCount() == 1) {
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
                 "\tdecrement owningRegisterCount %d->%d on ref #%d (%s) for "
                 "reg %s as %s (%p) refCount == 1 (going to 0)\n",
                 ...
                 self()->getDebug()->getName(reg), node->getOpCode().getName(),
                 node);
    }
    ...
    TR_ASSERT(false,
              "tempMemSlots should only be attached to pseudoRegisters and not "
              "node %p\n",
              node);
  }
#endif
...
rcount_t OMR::CodeGenerator::recursivelyDecReferenceCount(TR::Node *node) {
  TR_ASSERT(!self()->comp()->getOption(TR_EnableParanoidRefCountChecks) ||
                node->getOpCode().isTreeTop() || node->getReferenceCount() > 0,
            "OMR::CodeGenerator::recursivelyDecReferenceCount invoked for "
            "nontreetop node [%s] with count == 0",
            node->getName(self()->comp()->getDebug()));
    ...
    if (child->getRegister() == NULL)  // not already evaluated
    {
      ...
      if (child->getReferenceCount() > 1 &&
          (child->getOpCode().hasSymbolReference() ||
           (child->getOpCodeValue() == TR::l2a &&
            child->getChild(0)->containsCompressionSequence()))) {
        TR::SymbolReference *vftPointerSymRef =
            self()->comp()->getSymRefTab()->element(
                TR::SymbolReferenceTable::vftSymbol);
        if (node->isNopableInlineGuard() &&
          ...
          if (!child->getChild(0)->getChild(0)->getRegister() &&
            ...
            self()->evaluateChildrenWithMultipleRefCount(
