...
OMR::CodeGenPhase::performAll()
{
  ...
  for (; i < TR::CodeGenPhase::getListSize(); i++) {
    ...
    TR::RegionProfiler rp(
      _cg->comp()->trMemory()->heapMemoryRegion(),
      *_cg->comp(),
      "codegen/%s/%s",
...
void
OMR::CodeGenPhase::performProcessRelocationsPhase(TR::CodeGenerator* cg,
  ...
  if (regex && TR::SimpleRegex::match(regex, comp->getCurrentMethod())) {
    if (TR::Compiler->target.is64Bit()) {
      setDllSlip((char*)cg->getCodeStart(),
                 (char*)cg->getCodeStart() + cg->getCodeLength(),
                 "SLIPDLL64",
                 comp);
    } else {
      setDllSlip((char*)cg->getCodeStart(),
...
void
OMR::CodeGenPhase::performEmitSnippetsPhase(TR::CodeGenerator* cg,
                                            TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...
void
OMR::CodeGenPhase::performBinaryEncodingPhase(TR::CodeGenerator* cg,
                                              TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...
void
OMR::CodeGenPhase::performPeepholePhase(TR::CodeGenerator* cg,
                                        TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(
      comp->getOutFile(), "Post Peephole Instructions", false);
}

void
OMR::CodeGenPhase::performMapStackPhase(TR::CodeGenerator* cg,
                                        TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
  cg->remapGCIndicesInInternalPtrFormat();
  {
    ...
    if (comp->getOption(TR_TraceCG) ||
      ...
      comp->getDebug()->dumpMethodInstrs(
        comp->getOutFile(), "Post Stack Map", false);
  }
  cg->setMappingAutomatics();
}

void
OMR::CodeGenPhase::performRegisterAssigningPhase(TR::CodeGenerator* cg,
                                                 TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...

  {
    ...
    if (cg->getTraceRAOption(TR_TraceRAListing))
      if (cg->getDebug())
        cg->getDebug()->dumpMethodInstrs(
          comp->getOutFile(), "Before Local RA", false);
  ...
  if (comp->getOption(TR_TraceCG) ||
    ...
    comp->getDebug()->dumpMethodInstrs(
      comp->getOutFile(), "Post Register Assignment Instructions", false, true);
}

void
OMR::CodeGenPhase::performCreateStackAtlasPhase(TR::CodeGenerator* cg,
                                                TR::CodeGenPhase* phase)
{
  cg->createStackAtlas();
}
...
void
OMR::CodeGenPhase::performInstructionSelectionPhase(TR::CodeGenerator* cg,
                                                    TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...
void
OMR::CodeGenPhase::performSetupForInstructionSelectionPhase(
  TR::CodeGenerator* cg,
  TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceRegisterPressureDetails) &&
    ...
    for (TR::Block* block = comp->getStartBlock(); block;
      ...
      TR::CodeGenerator::TR_RegisterPressureState state(
        ...
        emptyBitVector,
        &emptyCandidateList,
        cg->getNumberOfGlobalGPRs(),
        cg->getNumberOfGlobalFPRs(),
        cg->getNumberOfGlobalVRFs(),
        vc);
...
void
OMR::CodeGenPhase::performLowerTreesPhase(TR::CodeGenerator* cg,
                                          TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...
void
OMR::CodeGenPhase::performUncommonCallConstNodesPhase(TR::CodeGenerator* cg,
                                                      TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...
void
OMR::CodeGenPhase::performReserveCodeCachePhase(TR::CodeGenerator* cg,
                                                TR::CodeGenPhase* phase)
{
  cg->reserveCodeCache();
}
...
void
OMR::CodeGenPhase::performInliningReportPhase(TR::CodeGenerator* cg,
                                              TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...
void
OMR::CodeGenPhase::performFindAndFixCommonedReferencesPhase(
  TR::CodeGenerator* cg,
  TR::CodeGenPhase* phase)
{
...
void
OMR::CodeGenPhase::performRemoveUnusedLocalsPhase(TR::CodeGenerator* cg,
                                                  TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
...
void
OMR::CodeGenPhase::performInsertDebugCountersPhase(TR::CodeGenerator* cg,
                                                   TR::CodeGenPhase* phase)
{
...
void
OMR::CodeGenPhase::performExpandInstructionsPhase(TR::CodeGenerator* cg,
                                                  TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(
      comp->getOutFile(),
      "Post Instruction Expansion Instructions",
      false,
      true);
}
...
OMR::CodeGenPhase::getName(PhaseValue phase)
{
  switch (phase) {
    ...
    default:
      TR_ASSERT(
        false, "TR::CodeGenPhase %d doesn't have a corresponding name.", phase);
      return NULL;
  };
}
