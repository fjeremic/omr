...
/*
 * We must initialize this static array first before the getListSize()
 * definition
 */
...
TR::CodeGenPhase *OMR::CodeGenPhase::self() {
  return static_cast<TR::CodeGenPhase *>(this);
}

/*
 * This getListSize definition must be placed after the static array PhaseList
 * has been initialized
 */
int OMR::CodeGenPhase::getListSize() {
  return sizeof(OMR::CodeGenPhase::PhaseList) /
         sizeof(OMR::CodeGenPhase::PhaseList[0]);
}
...
void OMR::CodeGenPhase::performAll() {
  ...
  for (; i < TR::CodeGenPhase::getListSize(); i++) {
    ...
    TR::RegionProfiler rp(
        _cg->comp()->trMemory()->heapMemoryRegion(), *_cg->comp(),
        "codegen/%s/%s",
        _cg->comp()->getHotnessName(_cg->comp()->getMethodHotness()),
        self()->getName(phaseToDo));
...
int OMR::CodeGenPhase::getNumPhases() {
  return static_cast<int>(TR::CodeGenPhase::LastOMRPhase);
}

void OMR::CodeGenPhase::performProcessRelocationsPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();

  if (comp->getPersistentInfo()->isRuntimeInstrumentationEnabled()) {
    // This must be called before relocations to generate the relocation data
    // for the profiled instructions.
    cg->createHWPRecords();
  }
  ...
  cg->syncCode(cg->getBinaryBufferStart(),
               cg->getBinaryBufferCursor() - cg->getBinaryBufferStart());

  if (comp->getOption(TR_EnableOSR)) {
    if (comp->getOption(TR_TraceOSR) &&
        !comp->getOption(TR_DisableOSRSharedSlots)) {
  ...
  if (cg->getAheadOfTimeCompile() &&
      (comp->getOption(TR_TraceRelocatableDataCG) ||
       comp->getOption(TR_TraceRelocatableDataDetailsCG) ||
       comp->getOption(TR_TraceReloCG))) {
    traceMsg(comp, "\n<relocatableDataCG>\n");
    if (comp->getOption(TR_TraceRelocatableDataDetailsCG))  // verbose output
    {
      ...
      traceMsg(comp,
               "Code start = %8x, Method start pc = %x, Method start pc offset "
               "= 0x%x\n",
               relocatableMethodCodeStart, cg->getCodeStart(),
               cg->getCodeStart() - relocatableMethodCodeStart);
    }
  ...
  if (regex && TR::SimpleRegex::match(regex, comp->getCurrentMethod())) {
    if (TR::Compiler->target.is64Bit()) {
      setDllSlip((char *)cg->getCodeStart(),
                 (char *)cg->getCodeStart() + cg->getCodeLength(), "SLIPDLL64",
                 comp);
    } else {
      setDllSlip((char *)cg->getCodeStart(),
                 (char *)cg->getCodeStart() + cg->getCodeLength(), "SLIPDLL31",
                 comp);
    }
  }
  if (comp->getOption(TR_TraceCG) ||
      comp->getOptions()->getTraceCGOption(TR_TraceCGPostBinaryEncoding)) {
...
void OMR::CodeGenPhase::performEmitSnippetsPhase(TR::CodeGenerator *cg,
  ...
  if (comp->getOption(TR_TraceCG) ||
    ...
    diagnostic("\nbuffer start = %8x, code start = %8x, buffer length = %d",
               cg->getBinaryBufferStart(), cg->getCodeStart(),
...
void OMR::CodeGenPhase::performBinaryEncodingPhase(TR::CodeGenerator *cg,
                                                   TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  phase->reportPhase(BinaryEncodingPhase);

  if (cg->getDebug()) cg->getDebug()->roundAddressEnumerationCounters();
...
void OMR::CodeGenPhase::performPeepholePhase(TR::CodeGenerator *cg,
                                             TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(comp->getOutFile(),
                                       "Post Peephole Instructions", false);
}

void OMR::CodeGenPhase::performMapStackPhase(TR::CodeGenerator *cg,
                                             TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  cg->remapGCIndicesInInternalPtrFormat();
  {
    ...
    if (comp->getOption(TR_TraceCG) ||
        comp->getOptions()->getTraceCGOption(TR_TraceEarlyStackMap))
      comp->getDebug()->dumpMethodInstrs(comp->getOutFile(), "Post Stack Map",
                                         false);
  }
  cg->setMappingAutomatics();
}

void OMR::CodeGenPhase::performRegisterAssigningPhase(TR::CodeGenerator *cg,
                                                      TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
...

  {
    ...
    TR_RegisterKinds nonColourableKindsToAssign =
        cg->prepareRegistersForAssignment();

    cg->jettisonAllSpills();  // Spill temps used before now may lead to
                              // conflicts if also used by register assignment

    // Do local register assignment for non-colourable registers.
    //
    if (cg->getTraceRAOption(TR_TraceRAListing))
      if (cg->getDebug())
        cg->getDebug()->dumpMethodInstrs(comp->getOutFile(), "Before Local RA",
                                         false);

    cg->doRegisterAssignment(nonColourableKindsToAssign);

    if (comp->compilationShouldBeInterrupted(
  ...
  if (comp->getOption(TR_TraceCG) ||
      comp->getOptions()->getTraceCGOption(TR_TraceCGPostRegisterAssignment))
    comp->getDebug()->dumpMethodInstrs(comp->getOutFile(),
                                       "Post Register Assignment Instructions",
                                       false, true);
}

void OMR::CodeGenPhase::performCreateStackAtlasPhase(TR::CodeGenerator *cg,
                                                     TR::CodeGenPhase *phase) {
  cg->createStackAtlas();
}

void OMR::CodeGenPhase::performInstructionSelectionPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  phase->reportPhase(InstructionSelectionPhase);

  if (comp->getOption(TR_TraceCG) || comp->getOption(TR_TraceTrees) ||
      comp->getOptions()->getTraceCGOption(TR_TraceCGPreInstructionSelection))
  ...
  if (comp->getOption(TR_TraceCG) ||
      comp->getOptions()->getTraceCGOption(TR_TraceCGPostInstructionSelection))
    comp->getDebug()->dumpMethodInstrs(
        comp->getOutFile(), "Post Instruction Selection Instructions", false,
        true);
  ...
  if (comp->compilationShouldBeInterrupted(
    ...
    comp->failCompilation<TR::CompilationInterrupted>(
        "interrupted after instruction selection");
  }
}

void OMR::CodeGenPhase::performSetupForInstructionSelectionPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceRegisterPressureDetails) &&
    ...
    traceMsg(comp,
             "         { Post optimization register pressure simulation\n");
    TR_BitVector emptyBitVector;
    vcount_t vc = comp->incVisitCount();
    cg->initializeRegisterPressureSimulator();
    for (TR::Block *block = comp->getStartBlock(); block;
         block = block->getNextExtendedBlock()) {
      TR_LinkHead<TR_RegisterCandidate> emptyCandidateList;
      TR::CodeGenerator::TR_RegisterPressureState state(
          NULL, 0, emptyBitVector, emptyBitVector, &emptyCandidateList,
          cg->getNumberOfGlobalGPRs(), cg->getNumberOfGlobalFPRs(),
...
void OMR::CodeGenPhase::performLowerTreesPhase(TR::CodeGenerator *cg,
                                               TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG)) comp->dumpMethodTrees("Post Lower Trees");
}

void OMR::CodeGenPhase::performUncommonCallConstNodesPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
...
void OMR::CodeGenPhase::performReserveCodeCachePhase(TR::CodeGenerator *cg,
                                                     TR::CodeGenPhase *phase) {
  cg->reserveCodeCache();
}

void OMR::CodeGenPhase::performInliningReportPhase(TR::CodeGenerator *cg,
                                                   TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
...
void OMR::CodeGenPhase::performCleanUpFlagsPhase(TR::CodeGenerator *cg,
                                                 TR::CodeGenPhase *phase) {
  TR::TreeTop *tt;
...
void OMR::CodeGenPhase::performFindAndFixCommonedReferencesPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  if (!cg->comp()->useRegisterMaps()) cg->findAndFixCommonedReferences();
}

void OMR::CodeGenPhase::performRemoveUnusedLocalsPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
...
void OMR::CodeGenPhase::performInsertDebugCountersPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  cg->insertDebugCounters();
}

void OMR::CodeGenPhase::performExpandInstructionsPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(
        comp->getOutFile(), "Post Instruction Expansion Instructions", false,
        true);
}

const char *OMR::CodeGenPhase::getName() {
...
const char *OMR::CodeGenPhase::getName(PhaseValue phase) {
  switch (phase) {
    ...
    default:
      TR_ASSERT(false, "TR::CodeGenPhase %d doesn't have a corresponding name.",
                phase);
      return NULL;
  };
}
