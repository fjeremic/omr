...
void TR::LabelRelative12BitRelocation::apply(TR::CodeGenerator *codeGen) {
  AOTcgDiag2(
      ...
      " label=" POINTER_PRINTF_FORMAT "\n",
      getUpdateLocation(), getLabel());
  codeGen->apply12BitLabelRelativeRelocation((int32_t *)getUpdateLocation(),
                                             getLabel(), isCheckDisp());
}
...
void TR::LabelRelative16BitRelocation::apply(TR::CodeGenerator *codeGen) {
  ...
  if (getAddressDifferenceDivisor() == 1)
    ...
    codeGen->apply16BitLabelRelativeRelocation(
        (int32_t *)getUpdateLocation(), getLabel(),
        getAddressDifferenceDivisor(), isInstructionOffset());
}
...
TR::InstructionLabelRelative16BitRelocation::
    InstructionLabelRelative16BitRelocation(TR::Instruction *cursor,
                                            int32_t offset, TR::LabelSymbol *l,
                                            int32_t divisor)
    : TR::LabelRelocation(NULL, l), _cursor(cursor), _offset(offset),
      _divisor(divisor) {}
...
void TR::ExternalRelocation::addExternalRelocation(TR::CodeGenerator *codeGen) {
  ...
  for (r = aot.getFirst(); r != 0; r = r->getNext()) {
    if (r->getTargetAddress2())
      AOTcgDiag6(comp,
                 ...
                 r, r->full(), r->getTargetAddress(), r->getTargetAddress2(),
                 r->getTargetKind(), r->getModifierValue());
    else
      AOTcgDiag5(comp,
                 "r=" POINTER_PRINTF_FORMAT
                 " full=%x target=" POINTER_PRINTF_FORMAT
                 " kind=%x modifier=%x\n",
                 r, r->full(), r->getTargetAddress(), r->getTargetKind(),
                 r->getModifierValue());
  ...
  if (_targetAddress2)
    AOTcgDiag6(
        comp,
        "temp=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT
        " target2=" POINTER_PRINTF_FORMAT " kind=%x modifier=%x\n",
        temp, temp->full(), temp->getTargetAddress(), temp->getTargetAddress2(),
        temp->getTargetKind(), temp->getModifierValue());
  else
    AOTcgDiag5(comp,
               "temp=" POINTER_PRINTF_FORMAT
               " full=%x target=" POINTER_PRINTF_FORMAT
               " kind=%x modifier=%x\n",
               temp, temp->full(), temp->getTargetAddress(),
               temp->getTargetKind(), temp->getModifierValue());
...
void TR::ExternalRelocation::trace(TR::Compilation *comp) {
  ...
  if (data) {
    traceMsg(comp, "%-35s %-32s %5d      %04x       %04x %8p\n",
             getName(this->getTargetKind()), data->file, data->line,
             methodOffset, programOffset, data->node);
    traceMsg(comp,
...
TR::ExternalOrderedPair32BitRelocation::ExternalOrderedPair32BitRelocation(
    uint8_t *location1, uint8_t *location2, uint8_t *target,
    TR_ExternalRelocationTargetKind k, TR::CodeGenerator *codeGen)
    : TR::ExternalRelocation(), _update2Location(location2) {
  AOTcgDiag0(codeGen->comp(), "TR::ExternalOrderedPair32BitRelocation::"
...
TR::IteratedExternalRelocation::IteratedExternalRelocation(
    uint8_t *target, TR_ExternalRelocationTargetKind k, flags8_t modifier,
    TR::CodeGenerator *codeGen)
    : TR_Link<TR::IteratedExternalRelocation>(), _numberOfRelocationSites(0),
      _targetAddress(target), _targetAddress2(NULL), _relocationData(NULL),
      ...
      _recordModifier(modifier.getValue()), _full(false), _kind(k) {
  AOTcgDiag0(TR::comp(),
             "TR::IteratedExternalRelocation::IteratedExternalRelocation\n");
}

TR::IteratedExternalRelocation::IteratedExternalRelocation(
    uint8_t *target, uint8_t *target2, TR_ExternalRelocationTargetKind k,
    flags8_t modifier, TR::CodeGenerator *codeGen)
    : TR_Link<TR::IteratedExternalRelocation>(), _numberOfRelocationSites(0),
      _targetAddress(target), _targetAddress2(target2), _relocationData(NULL),
      _relocationDataCursor(NULL),
      // initial size is size of header for this type
      _sizeOfRelocationData(
          codeGen->getAheadOfTimeCompile()->getSizeOfAOTRelocationHeader(k)),
...
void TR::IteratedExternalRelocation::initializeRelocation(
    TR::CodeGenerator *codeGen) {
  AOTcgDiag0(TR::comp(),
...
void TR::IteratedExternalRelocation::addRelocationEntry(
