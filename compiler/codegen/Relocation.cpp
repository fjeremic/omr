...
void TR::LabelRelative8BitRelocation::apply(TR::CodeGenerator* codeGen)
{
  AOTcgDiag2(codeGen->comp(),
    "TR::LabelRelative8BitRelocation::apply cursor=" POINTER_PRINTF_FORMAT " label=" POINTER_PRINTF_FORMAT "\n",
...
{
  AOTcgDiag2(codeGen->comp(),
    "TR::LabelRelative12BitRelocation::apply cursor=" POINTER_PRINTF_FORMAT " label=" POINTER_PRINTF_FORMAT "\n",
    getUpdateLocation(), getLabel());
...
{
  AOTcgDiag2(codeGen->comp(),
    "TR::LabelRelative16BitRelocation::apply cursor=" POINTER_PRINTF_FORMAT " label=" POINTER_PRINTF_FORMAT "\n",
    getUpdateLocation(), getLabel());
...
{
  AOTcgDiag2(codeGen->comp(),
    "TR::LabelRelative24BitRelocation::apply cursor=" POINTER_PRINTF_FORMAT " label=" POINTER_PRINTF_FORMAT "\n",
    getUpdateLocation(), getLabel());
...
{
  AOTcgDiag2(codeGen->comp(),
    "TR::LabelRelative32BitRelocation::apply cursor=" POINTER_PRINTF_FORMAT " label=" POINTER_PRINTF_FORMAT "\n",
    getUpdateLocation(), getLabel());
...
void TR::LabelAbsoluteRelocation::apply(TR::CodeGenerator* codeGen)
{
  ...
  AOTcgDiag2(codeGen->comp(),
    "TR::LabelAbsoluteRelocation::apply cursor=" POINTER_PRINTF_FORMAT " label=" POINTER_PRINTF_FORMAT "\n", cursor,
    getLabel());
...
void TR::InstructionAbsoluteRelocation::apply(TR::CodeGenerator* codeGen)
{
  ...
  AOTcgDiag2(codeGen->comp(),
    "TR::InstructionAbsoluteRelocation::apply cursor=" POINTER_PRINTF_FORMAT " instruction=" POINTER_PRINTF_FORMAT "\n",
    cursor, address);
  *cursor = address;
}
...
void TR::LoadLabelRelative16BitRelocation::apply(TR::CodeGenerator* codeGen)
{
  AOTcgDiag3(codeGen->comp(),
    "TR::LoadLabelRelative16BitRelocation::apply lastInstruction=" POINTER_PRINTF_FORMAT
    " startLabel=" POINTER_PRINTF_FORMAT " endLabel=" POINTER_PRINTF_FORMAT "\n",
    getLastInstruction(), getStartLabel(), getEndLabel());
  codeGen->apply16BitLoadLabelRelativeRelocation(
    getLastInstruction(), getStartLabel(), getEndLabel(), getDeltaToStartLabel());
}

void TR::LoadLabelRelative32BitRelocation::apply(TR::CodeGenerator* codeGen)
{
  AOTcgDiag3(codeGen->comp(),
    "TR::LoadLabelRelative32BitRelocation::apply lastInstruction=" POINTER_PRINTF_FORMAT
    " startLabel=" POINTER_PRINTF_FORMAT " endLabel=" POINTER_PRINTF_FORMAT "\n",
    getLastInstruction(), getStartLabel(), getEndLabel());
  codeGen->apply32BitLoadLabelRelativeRelocation(
    getLastInstruction(), getStartLabel(), getEndLabel(), getDeltaToStartLabel());
}
...
{
  AOTcgDiag2(codeGen->comp(),
    "TR::LoadLabelRelative64BitRelocation::apply lastInstruction=" POINTER_PRINTF_FORMAT " label=" POINTER_PRINTF_FORMAT
    "\n",
...
uint8_t TR::ExternalRelocation::collectModifier()
{
  ...
  AOTcgDiag2(comp,
    "TR::ExternalRelocation::collectModifier distance from start of buffer=%x, from start of method=%x\n",
...
void TR::ExternalRelocation::addExternalRelocation(TR::CodeGenerator* codeGen)
{
  ...
  for (r = aot.getFirst(); r != 0; r = r->getNext()) {
    if (r->getTargetAddress2())
      AOTcgDiag6(comp,
        "r=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT " target2=" POINTER_PRINTF_FORMAT
        ", kind=%x modifier=%x\n",
      ...
      AOTcgDiag5(comp, "r=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT " kind=%x modifier=%x\n", r,
        r->full(), r->getTargetAddress(), r->getTargetKind(), r->getModifierValue());
  ...
  TR::IteratedExternalRelocation* temp = _targetAddress2
    ? new (codeGen->trHeapMemory())
      TR::IteratedExternalRelocation(_targetAddress, _targetAddress2, _kind, modifier, codeGen)
    : new (codeGen->trHeapMemory()) TR::IteratedExternalRelocation(_targetAddress, _kind, modifier, codeGen);

  aot.add(temp);
  if (_targetAddress2)
    AOTcgDiag6(comp,
      "temp=" POINTER_PRINTF_FORMAT " full=%x target=" POINTER_PRINTF_FORMAT " target2=" POINTER_PRINTF_FORMAT
      " kind=%x modifier=%x\n",
      temp, temp->full(), temp->getTargetAddress(), temp->getTargetAddress2(), temp->getTargetKind(),
...
TR::ExternalOrderedPair32BitRelocation::ExternalOrderedPair32BitRelocation(uint8_t* location1, uint8_t* location2,
  uint8_t* target, TR_ExternalRelocationTargetKind k, TR::CodeGenerator* codeGen)
...
void TR::ExternalOrderedPair32BitRelocation::apply(TR::CodeGenerator* codeGen)
{
  ...
  TR_ExternalRelocationTargetKind kind = getRelocationRecord()->getTargetKind();
  if (TR::Compiler->target.cpu.isPower()
    && (kind == TR_ArrayCopyHelper || kind == TR_ArrayCopyToc || kind == TR_RamMethodSequence || kind == TR_GlobalValue
      || kind == TR_BodyInfoAddressLoad || kind == TR_DataAddress || kind == TR_DebugCounter)) {
    TR::Instruction* instr = (TR::Instruction*)getUpdateLocation();
    TR::Instruction* instr2 = (TR::Instruction*)getLocation2();
...
TR::IteratedExternalRelocation::IteratedExternalRelocation(
  uint8_t* target, uint8_t* target2, TR_ExternalRelocationTargetKind k, flags8_t modifier, TR::CodeGenerator* codeGen)
  : TR_Link<TR::IteratedExternalRelocation>()
  , _numberOfRelocationSites(0)
  , _targetAddress(target)
...
void TR::IteratedExternalRelocation::addRelocationEntry(uint32_t locationOffset)
{
  TR::Compilation* comp = TR::comp();
  AOTcgDiag2(comp,
    "TR::IteratedExternalRelocation::addRelocationEntry _relocationDataCursor=" POINTER_PRINTF_FORMAT
    ", locationOffset=%x\n",
