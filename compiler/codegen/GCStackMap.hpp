...
public:
  ...
  TR_InternalPointerPair(TR::AutomaticSymbol *pinningArrayPtr,
                         TR::AutomaticSymbol *internalPtr)
      : _pinningArrayPtr(pinningArrayPtr), _internalPtrAuto(internalPtr) {
    TR_ASSERT(!internalPtr || internalPtr->isInternalPointerAuto(),
              "should be internal pointerauto");
  }

  TR_InternalPointerPair(TR::AutomaticSymbol *pinningArrayPtr,
                         int32_t internalPtrRegNum)
      : _pinningArrayPtr(pinningArrayPtr),
        _internalPtrRegNum(internalPtrRegNum) {}

  TR_InternalPointerPair *clone(TR_Memory *m) {
    return new (m->trHeapMemory())
        TR_InternalPointerPair(_pinningArrayPtr, _internalPtrAuto);
  }

  TR::AutomaticSymbol *getPinningArrayPointer() { return _pinningArrayPtr; }
  void setPinningArrayPointer(TR::AutomaticSymbol *pinningArrayPointer) {
    _pinningArrayPtr = pinningArrayPointer;
  }
  ...
  void setInternalPointerAuto(TR::AutomaticSymbol *internalPointerAuto) {
    TR_ASSERT(!internalPointerAuto ||
                  internalPointerAuto->isInternalPointerAuto(),
              "should be internal pointerauto");
    _internalPtrAuto = internalPointerAuto;
  }
  int32_t getInternalPtrRegNum() { return _internalPtrRegNum; }
  void setInternalPtrRegNum(int32_t internalPtrRegNum) {
    _internalPtrRegNum = internalPtrRegNum;
  }
};
...
public:
  ...
  TR_InternalPointerMap(TR_Memory *m)
      : _trMemory(m), _numInternalPtrs(0), _numDistinctPinningArrays(0),
        _internalPtrPairs(m), _size(0) {}

  TR_Memory *trMemory() { return _trMemory; }
  TR_HeapMemory trHeapMemory() { return trMemory(); }

  TR_InternalPointerMap *clone() {
    TR_InternalPointerMap *newIPtrMap =
        new (trHeapMemory()) TR_InternalPointerMap(trMemory());
    ListIterator<TR_InternalPointerPair> pairIt(&_internalPtrPairs);

    for (TR_InternalPointerPair *pair = pairIt.getFirst(); pair;
         pair = pairIt.getNext())
  ...
  List<TR_InternalPointerPair> &getInternalPointerPairs() {
    return _internalPtrPairs;
  }
  ...
  void addInternalPointerPair(TR::AutomaticSymbol *pinningArrayPtr,
                              int32_t internalPtrRegNum) {
    addInternalPointerPair(new (trHeapMemory()) TR_InternalPointerPair(
        pinningArrayPtr, internalPtrRegNum));
  }

  void addInternalPointerPair(TR::AutomaticSymbol *pinningArrayPtr,
                              TR::AutomaticSymbol *internalPtr) {
    addInternalPointerPair(new (trHeapMemory()) TR_InternalPointerPair(
        pinningArrayPtr, internalPtr));
  }
...
public:
  ...
  TR_GCStackMap(uint32_t slots)
      : _lowestCodeOffset(0), _lowestOffsetInstruction(0),
        _internalPointerMap(0), _liveMonitorBits(0),
  ...
  void operator delete(void *gcStackMap, TR_HeapMemory m,
                       uint32_t numberOfSlotsToMap) {
  ...
  TR::Instruction *getLowestOffsetInstruction() {
    return _lowestOffsetInstruction;
  }
  TR::Instruction *setLowestOffsetInstruction(TR::Instruction *i) {
    return (_lowestOffsetInstruction = i);
  }

  TR_InternalPointerMap *getInternalPointerMap() { return _internalPointerMap; }
  TR_InternalPointerMap *setInternalPointerMap(TR_InternalPointerMap *map) {
    return (_internalPointerMap = map);
  }

  uint32_t getNumberOfSlotsMapped() { return _numberOfSlotsMapped; }

  bool isInternalPointerMapIdenticalTo(TR_GCStackMap *map) {
    return _internalPointerMap->isInternalPointerMapIdenticalTo(
        map->getInternalPointerMap());
  }

  bool isByteCodeInfoIdenticalTo(TR_GCStackMap *map) {
    return ((_byteCodeInfo.getCallerIndex() ==
             map->getByteCodeInfo().getCallerIndex()) &&
            (_byteCodeInfo.getByteCodeIndex() ==
             map->getByteCodeInfo().getByteCodeIndex()) &&
            (_byteCodeInfo.doNotProfile() ==
             map->getByteCodeInfo().doNotProfile()));
  }

  void setRegisterBits(uint32_t bits) { _registerMap.setRegisterBits(bits); }
  void resetRegistersBits(uint32_t bits) {
    _registerMap.resetRegisterBits(bits);
  }
  ...
  void maskRegistersWithInfoBits(uint32_t mask, uint32_t info) {
    _registerMap.maskRegistersWithInfoBits(mask, info);
  }
  void setInfoBits(uint32_t info) { _registerMap.setInfoBits(info); }

  uint32_t getRegisterSaveDescription() {
    return _registerMap.getRegisterSaveDescription();
  }
  void setRegisterSaveDescription(uint32_t bits) {
    _registerMap.setRegisterSaveDescription(bits);
  }

  void setBit(int32_t bitNumber) {
    _mapBits[bitNumber >> 3] |= 1 << (bitNumber & 7);
  }
  bool isSet(int32_t bitNumber) {
    return (_mapBits[bitNumber >> 3] & (1 << (bitNumber & 7))) != 0;
  }
  void resetBit(int32_t bitNumber) {
    _mapBits[bitNumber >> 3] &= ~(1 << (bitNumber & 7));
  }

  void setLiveMonitorBit(int32_t bitNumber) {
    _liveMonitorBits[bitNumber >> 3] |= 1 << (bitNumber & 7);
  }
  ...
  TR_GCStackMap *clone(TR_Memory *m) {
    TR_GCStackMap *newMap = new (m->trHeapMemory(), _numberOfSlotsMapped)
        TR_GCStackMap(_numberOfSlotsMapped);
    ...
    if (_liveMonitorBits) {
      ...
      memcpy(newMap->_liveMonitorBits, getLiveMonitorBits(),
             getMapSizeInBytes());
    }
...
public:
  ...
  TR_GCStackAllocMap(uint32_t slots) : _numberOfSlotsMapped(slots) {
    clearBits();
  }
