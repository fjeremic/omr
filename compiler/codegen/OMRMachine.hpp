...
class OMR_EXTENSIBLE Machine {
 ...
 public:
  ...
  Machine(TR::CodeGenerator* cg)
      : _cg(cg), numLockedGPRs(-1), numLockedFPRs(-1), numLockedVRFs(-1) {}
  ...
  /** \brief
   *     Sets the number of locked registers of a specific register kind for use
   * as a cache lookup.
   *
   *  \param kind
   ...
   *
   *  \param numLocked
   *     The number of registers of the respective kind that are in the locked
   * state.
   *
   *  \return
   *     The number of registers of the respective kind that are in the locked
  ...
  int16_t setNumberOfLockedRegisters(TR_RegisterKinds kind, int16_t numLocked) {
    TR_ASSERT(numLocked >= 0,
              "Expecting number of locked registers to be >= 0");
    switch (kind) {
      case TR_GPR:
  ...
  int16_t getNumberOfLockedRegisters(TR_RegisterKinds kind) {
    switch (kind) {
      case TR_GPR:
        TR_ASSERT(numLockedGPRs >= 0,
                  "Expecting number of locked registers to be >= 0");
        return numLockedGPRs;
      case TR_FPR:
        TR_ASSERT(numLockedFPRs >= 0,
                  "Expecting number of locked registers to be >= 0");
        return numLockedFPRs;
      case TR_VRF:
        TR_ASSERT(numLockedVRFs >= 0,
                  "Expecting number of locked registers to be >= 0");
        return numLockedVRFs;
      default:
        TR_ASSERT_FATAL(false, "Unknown register kind");
   ...
   */
  TR::RealRegister* realRegister(TR::RealRegister::RegNum rn) {
    return _registerFile[rn];
  }
