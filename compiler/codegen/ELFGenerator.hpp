...
public:
   ...
   */
  ELFGenerator(TR::RawAllocator rawAllocator, uint8_t const *codeStart,
               size_t codeSize)
      : _rawAllocator(rawAllocator), _codeStart(codeStart), _codeSize(codeSize),
        _header(NULL), _programHeader(NULL), _zeroSection(NULL),
        _textSection(NULL), _relaSection(NULL), _dynSymSection(NULL),
        _shStrTabSection(NULL), _dynStrSection(NULL) {}
...
#define ELFClass ELFCLASS32;
  ...
  /**
   * Pure virtual function that should be implemented to handle how the header
   * is set up for different types of ELF objects
   */
  virtual void initialize(void) = 0;

  /**
   * Pure virtual function that should be implemented to initialize header
   * members appropriately
   */
  virtual void initializeELFHeader(void) = 0;

  /**
   * Sets up arch and OS specific information, along with ELFEHeader's e_ident
   * member
   */
   ...
   */
  void initializeTextSection(uint32_t shName, ELFAddress shAddress,
   ...
   */
  void initializeDynSymSection(uint32_t shName, ELFOffset shOffset,
                               uint32_t shSize, uint32_t shLink);
   ...
   * @param[in] shSize the section header size
   */
  void initializeStrTabSection(uint32_t shName, ELFOffset shOffset,
                               uint32_t shSize);
   ...
   * @param[in] shSize the section header size
   */
  void initializeDynStrSection(uint32_t shName, ELFOffset shOffset,
                               uint32_t shSize);
   ...
   * @param[in] shSize the section header size
   */
  void initializeRelaSection(uint32_t shName, ELFOffset shOffset,
                             uint32_t shSize);
   ...
   * @param[in] size the size of the char array, which includes the null
   * terminator
   */
  ...
  TR::RawAllocator
      _rawAllocator; /**< the RawAllocator passed to the constructor */

  ELFEHeader *_header; /**< The ELFEHeader, required for all ELF files */
  ELFProgramHeader
      *_programHeader; /**< The ELFProgramHeader, required for executable ELF */
  ...
  uint32_t _elfTrailerSize; /**< Size of the region of the ELF file that would
                               be occupied by the trailer segment */
  uint32_t _elfHeaderSize; /**< Size of the region of the ELF file that would be
                              occupied by the header segment */
  uint32_t _totalELFSymbolNamesLength; /**< Total length of the symbol names
                                          that would be written to file  */
  struct TR::CodeCacheSymbol
      *_symbols; /**< Chain of CodeCacheSymbol structures to be written */
  ...
  struct TR::CodeCacheRelocationInfo
      *_relocations; /**< Struct containing relocation info to be written */
...
public:
   ...
   * @param[in] totalELFSymbolNamesLength the sum of symbol name lengths + 1 for
   * UNDEF
   * @return bool whether emitting ELF file succeeded
   */
  bool emitELF(const char *filename, CodeCacheSymbol *symbols,
               uint32_t numSymbols, uint32_t totalELFSymbolNamesLength);
...
public:
   ...
   * @return bool whether emitting ELF file succeeded
   */
  bool emitELF(const char *filename, TR::CodeCacheSymbol *symbols,
               uint32_t numSymbols, uint32_t totalELFSymbolNamesLength,
