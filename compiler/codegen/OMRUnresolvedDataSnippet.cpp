...
OMR::UnresolvedDataSnippet::UnresolvedDataSnippet(TR::CodeGenerator *cg,
                                                  ...
                                                  bool isStore,
                                                  bool isGCSafePoint)
    : Snippet(cg, node, generateLabelSymbol(cg)), _dataSymbolReference(symRef),
      _dataReferenceInstruction(NULL), _addressOfDataReference(0) {
  if (isStore) {
    setUnresolvedStore();
  }
...
OMR::UnresolvedDataSnippet::create(TR::CodeGenerator *cg, TR::Node *node,
                                   TR::SymbolReference *s, bool isStore,
                                   bool canCauseGC) {
  return new (cg->trHeapMemory())
      TR::UnresolvedDataSnippet(cg, node, s, isStore, canCauseGC);
}
