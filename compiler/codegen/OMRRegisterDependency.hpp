...
public:
  ...
  TR::RegisterDependency* getDependencyWithTarget(TR::RealRegister::RegNum regNum)
  {
    ...
    TR_ASSERT(
      regNum != TR::RealRegister::SpilledReg, "Multiple dependencies can map to 'SpilledReg', can't return just one");
...
private:
  void addDependency(TR::RealRegister::RegNum regNum, TR::RealRegister* assignedReg, uint32_t index)
  {
    ...
    if (assignedReg) {
      ...
      TR_ASSERT(
        arr >= 0 && arr < TR::RealRegister::NumRegisters, "Register number (%d) used as index but out of range!", arr);
