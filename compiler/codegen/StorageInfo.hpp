...
enum TR_StorageOverlapKind {
  TR_NoOverlap =
      0,  // s: the source (address) of data move; t: the destination (address)
          // of the move. With regard to their lengths, they don't overlap
  ...
  TR_DestructiveOverlap,  // Overlap and s's s < t and s + min (s_l, t_l) > t
                          // where s_l is the source length while t_l is the
                          // destination length.
  TR_PriorPosOverlap,  // Overlap and s's address is prior to t's but they do
                       // not destructively overlap
  TR_NumOverlapTypes
};
