...
OMR::CodeGenerator::buildGCMapForInstruction(TR::Instruction* instr)
{
  ...
  if (liveLocals || liveMonitors) {
    ...
    if (debug("traceLiveMonitors")) {
      if (liveMonitors)
        traceMsg(comp,
                 "building monitor map for instr %p node %p\n",
                 instr,
                 instr->getNode());
      else
        traceMsg(comp,
                 "no monitor map for instr %p node %p\n",
                 instr,
                 instr->getNode());
    }
    ...
    for (TR::AutomaticSymbol* localCursor = automaticIterator.getFirst();
      ...
      if (mapIndex >= 0 && mapIndex < atlas->getIndexOfFirstSpillTemp()) {
        ...
        if (liveMonitors &&
          ...
          if (debug("traceLiveMonitors"))
            traceMsg(comp,
                     "setting map bit for local %p (%d) mapIndex %d\n",
                     localCursor,
                     localCursor->getLiveLocalIndex(),
                     mapIndex);
  ...
  for (auto location = self()->getCollectedSpillList().begin();
       location != self()->getCollectedSpillList().end();
       ++location) {
    if ((*location)->containsCollectedReference() &&
      ...
      if ((TR::Compiler->target.cpu.isPower() ||
        ...
        if (self()->getDebug())
          traceMsg(
            comp,
            "\nSkipping GC map [%p] index %d (%s) for instruction [%p] in OOL "
            "hot path because it has already been reverse spilled.\n",
            map,
            s->getGCMapIndex(),
...
bool
TR_InternalPointerMap::isInternalPointerMapIdenticalTo(
...
void
TR_GCStackMap::addToAtlas(TR::Instruction* instruction,
