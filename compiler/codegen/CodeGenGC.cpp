...
TR_GCStackMap* OMR::CodeGenerator::buildGCMapForInstruction(TR::Instruction* instr)
{
  ...
  for (auto location = self()->getCollectedSpillList().begin(); location != self()->getCollectedSpillList().end();
    ...
    if ((*location)->containsCollectedReference()
      ...
      //
      if ((TR::Compiler->target.cpu.isPower() || TR::Compiler->target.cpu.isZ()) && (*location)->getMaxSpillDepth() == 0
        && comp->cg()->isOutOfLineHotPath()) {
        if (self()->getDebug())
          traceMsg(comp,
            "\nSkipping GC map [%p] index %d (%s) for instruction [%p] in OOL hot path because it has already been "
            "reverse spilled.\n",
