...
void OMR::CodeGenerator::createStackAtlas() {
  ...
  // Stack mapping will have to map the stack in a way that honours these
  // indices
  //
  ...
  //
  int32_t firstMappedParmOffset =
      methodSymbol->getNumParameterSlots() * stackSlotSize;

  // Now assign GC map indices to parameters depending on the linkage mapping.
  // At the same time build the parameter map.
  //
  TR_GCStackMap *parameterMap = new (self()->trHeapMemory(), numberOfParmSlots)
      TR_GCStackMap(numberOfParmSlots);
  ...
  // this index.
  //
  // Locals that need initialization during the method prologue are mapped
  // first, then the ones that do not need initialization.
  ...
  for (localCursor = automaticIterator.getFirst(); localCursor;
    ...
    if (localCursor->getGCMapIndex() < 0 &&
        localCursor->isCollectedReference() && !localCursor->isLocalObject() &&
  ...
  //
  TR_GCStackMap *localMap =
      new (self()->trHeapMemory(), numberOfSlots) TR_GCStackMap(numberOfSlots);
  ...
  //
  TR::GCStackAtlas *atlas = new (self()->trHeapMemory())
      TR::GCStackAtlas(numberOfParmSlots, numberOfSlots, self()->trMemory());
...
void OMR::CodeGenerator::buildGCMapsForInstructionAndSnippet(
    TR::Instruction *instr) {
  if (instr->needsGCMap()) {
    TR_GCStackMap *map = self()->buildGCMapForInstruction(instr);

    // The info bits are encoded in the register mask on the instruction (if
    // there are any).
    //
    map->maskRegistersWithInfoBits(instr->getGCRegisterMask(),
  ...
  if (instr->getSnippetForGC()) {
    ...
    if (snippet && snippet->gcMap().isGCSafePoint() &&
        !snippet->gcMap().getStackMap()) {
      TR_GCStackMap *map = self()->buildGCMapForInstruction(instr);
...
TR_GCStackMap *OMR::CodeGenerator::buildGCMapForInstruction(
    TR::Instruction *instr) {
  ...
  if (liveLocals || liveMonitors) {
    ...
    for (TR::AutomaticSymbol *localCursor = automaticIterator.getFirst();
         localCursor; localCursor = automaticIterator.getNext()) {
      ...
      if (mapIndex >= 0 && mapIndex < atlas->getIndexOfFirstSpillTemp()) {
        ...
        if (liveMonitors &&
            liveMonitors->get(localCursor->getLiveLocalIndex())) {
          if (debug("traceLiveMonitors"))
  ...
  //
  for (auto location = self()->getCollectedSpillList().begin();
       location != self()->getCollectedSpillList().end(); ++location) {
    if ((*location)->containsCollectedReference() &&
      ...
      TR::AutomaticSymbol *s =
          (*location)->getSymbolReference()->getSymbol()->getAutoSymbol();
      ...
      if ((TR::Compiler->target.cpu.isPower() ||
          ...
          (*location)->getMaxSpillDepth() == 0 &&
          comp->cg()->isOutOfLineHotPath()) {
        if (self()->getDebug())
          traceMsg(
              comp,
              "\nSkipping GC map [%p] index %d (%s) for instruction [%p] in "
              "OOL hot path because it has already been reverse spilled.\n",
              map, s->getGCMapIndex(),
              self()->getDebug()->getName((*location)->getSymbolReference()),
              instr);
        continue;
      }

      TR_ASSERT(s->getGCMapIndex() >= atlas->getIndexOfFirstSpillTemp(),
...
void OMR::CodeGenerator::remapGCIndicesInInternalPtrFormat() {
  ...
  // Also note that as per the proposal we have to mark the base array temp AND
  // the internal pointer itself as scalar temps; so GC ignores them in its
  // normal course.
  //
  TR::Compilation *comp = self()->comp();
  ...
  for (localCursor = automaticIterator.getFirst(); localCursor;
    ...
    if (localCursor->isInternalPointer()) {
      ...
      TR::AutomaticSymbol *internalPtrCursor =
          localCursor->castToInternalPointerAutoSymbol();
      internalPtrMap->addInternalPointerPair(
          internalPtrCursor->getPinningArrayPointer(), internalPtrCursor);
      stackAtlas->removePinningArrayPtrForInternalPtrReg(
          internalPtrCursor->getPinningArrayPointer());
    }
  }
...
bool TR_InternalPointerMap::isInternalPointerMapIdenticalTo(
    TR_InternalPointerMap *map) {
  if (getNumInternalPointers() != map->getNumInternalPointers()) return false;
  ...
  for (TR_InternalPointerPair *pair = listIt.getFirst(); pair;
       pair = listIt.getNext()) {
    bool found = false;
    list2It.reset();
    for (TR_InternalPointerPair *pair2 = list2It.getFirst(); pair2;
         pair2 = list2It.getNext()) {
    ...
    map = instr->getGCMap();
             ...
             (instr->getNode()->getByteCodeInfo().getCallerIndex() !=
                  instr->getPrev()
                      ...
                      .getCallerIndex() ||
              0)) {
    // find the previous instruction that contains a map and copy it with the
    // current instruction's byte code info
    //
    for (TR::Instruction *prev = instr->getPrev(); prev;
         prev = prev->getPrev()) {
...
void TR_GCStackMap::addToAtlas(TR::Instruction *instruction,
  ...
  if (osrEnabled) codeGen->addToOSRTable(instruction);
}

void TR_GCStackMap::addToAtlas(uint8_t *callSiteAddress,
                               TR::CodeGenerator *codeGen) {
  // Fill in the code range and add this map to the atlas.
  //
  ...
  codeGen->getStackAtlas()->addStackMap(this);
  bool osrEnabled = codeGen->comp()->getOption(TR_EnableOSR);
  if (osrEnabled) codeGen->addToOSRTable(callSiteOffset, getByteCodeInfo());
}
