...
void OMR::CodeGenerator::createStackAtlas()
{
    ...
    for (localCursor = automaticIterator.getFirst(); localCursor; localCursor = automaticIterator.getNext()) {
        if (localCursor->getGCMapIndex() < 0 && localCursor->isCollectedReference() && !localCursor->isLocalObject() && !localCursor->isInitializedReference() && !localCursor->isInternalPointer() && !localCursor->isPinningArrayPointer()) {
...
OMR::CodeGenerator::buildGCMapForInstruction(TR::Instruction* instr)
{
        ...
        for (TR::AutomaticSymbol* localCursor = automaticIterator.getFirst();
            ...
            if (mapIndex >= 0 && mapIndex < atlas->getIndexOfFirstSpillTemp()) {
                if (liveLocals && liveLocals->get(localCursor->getLiveLocalIndex())) {
                    ...
                    } else if (localCursor->isCollectedReference() && !localCursor->isInternalPointer() && !localCursor->isPinningArrayPointer()) {
                        map->setBit(mapIndex);
                    }
                }
    ...
    for (auto location = self()->getCollectedSpillList().begin(); location != self()->getCollectedSpillList().end(); ++location) {
        if ((*location)->containsCollectedReference() && !(*location)->getSymbolReference()->getSymbol()->isInternalPointer() && !(*location)->getSymbolReference()->getSymbol()->isPinningArrayPointer() && (*location)->isOccupied()) {
...
bool TR_InternalPointerMap::isInternalPointerMapIdenticalTo(TR_InternalPointerMap* map)
{
    ...
    for (TR_InternalPointerPair* pair = listIt.getFirst(); pair; pair = listIt.getNext()) {
        ...
        for (TR_InternalPointerPair* pair2 = list2It.getFirst(); pair2; pair2 = list2It.getNext()) {
            if ((pair->getPinningArrayPointer() == pair2->getPinningArrayPointer()) && (pair->getInternalPtrRegNum() == pair2->getInternalPtrRegNum())) {
                found = true;
                break;
            }
        }
...
void OMR::CodeGenerator::addToAtlas(TR::Instruction* instr)
{
    ...
    if (instr->needsGCMap()) {
        map = instr->getGCMap();
    } else if (comp->getOption(TR_GenerateCompleteInlineRanges) && instr->getNode() && instr->getPrev() && instr->getPrev()->getNode() && instr->getBinaryLength() > 0 && (instr->getNode()->getByteCodeInfo().getCallerIndex() != instr->getPrev()->getNode()->getByteCodeInfo().getCallerIndex() || 0)) {
...
void TR_GCStackMap::addToAtlas(TR::Instruction* instruction, TR::CodeGenerator* codeGen)
{
...
void TR_GCStackMap::addToAtlas(uint8_t* callSiteAddress, TR::CodeGenerator* codeGen)
{
    // Fill in the code range and add this map to the atlas.
    //
