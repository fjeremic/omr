...
#define TR_SSR_ASSERT()                                                        \
  TR_ASSERT(self()->getKind() != TR_SSR,                                       \
            "only non-TR_SSR registers use _totalUseCount : an aggregate or "  \
            "BCD type node has been evaluated as a non-aggregate or BCD node " \
            "(missing o2x cast?)\n")
#define TR_MAX_USECOUNT_ASSERT(a)                                              \
  TR_ASSERT(_totalUseCount <= MAX_NODE_COUNT - a,                              \
            "TR::Register::_totalUseCount == MAX_NODE_COUNT")
...
public:
  ...
  TR_BackingStore *setBackingStorage(TR_BackingStore *bs) {
    return (_backingStorage = bs);
  }
  ...
  TR::Register *setAssignedRegister(TR::Register *r) {
    return (_assignedRegister = r);
  }

  TR::Register *getSiblingRegister() { return _siblingRegister; }
  TR::Register *setSiblingRegister(TR::Register *r) {
    return (_siblingRegister = r);
  }

  TR::Instruction *getStartOfRange() { return _startOfRange; }
  TR::Instruction *setStartOfRange(TR::Instruction *i) {
    return _startOfRange = i;
  }
  ...
  TR::Node *setStartOfRangeNode(TR::Node *n) { /* _startOfRange=NULL; */
  ...
  ncount_t incFutureUseCount(ncount_t fuc = 1) {
    TR_ASSERT(_futureUseCount <= MAX_NODE_COUNT - fuc,
              "TR::Register::_futureUseCount == MAX_NODE_COUNT");
  ...
  ncount_t setOutOfLineUseCount(ncount_t uc) {
    return (_outOfLineUseCount = uc);
  }
  ncount_t incOutOfLineUseCount(ncount_t uc = 1) {
    return (_outOfLineUseCount += uc);
  }
  ncount_t decOutOfLineUseCount(ncount_t uc = 1) {
    return (_outOfLineUseCount -= uc);
  }
  ...
  bool containsCollectedReference() {
    return _flags.testAny(ContainsCollectedReference);
  }
  ...
  bool containsInternalPointer() {
    return _flags.testAny(ContainsInternalPointer);
  }
  ...
  virtual const char *getRegisterName(TR::Compilation *comp,
                                      TR_RegisterSizes size = TR_WordReg);
  static const char *getRegisterKindName(TR::Compilation *comp,
                                         TR_RegisterKinds rk);
...
#if defined(DEBUG)
  virtual void print(TR::Compilation *comp, TR::FILE *pOutFile,
                     TR_RegisterSizes size = TR_WordReg);
#endif
...
private:
  TR_BackingStore
      *_backingStorage; // location where register is spilled if spilled
  TR::AutomaticSymbol
      *_pinningArrayPointer; // pinning array object if containing internal ptr

  TR::Register
      *_assignedRegister;         // register to which this register is assigned
  ...
  TR::Node *_startOfRangeNode;    // Node of _startOfRange. Needed if start of
                                  // range is Register load

  ncount_t _totalUseCount;     // holds the number of references to the register
  ncount_t _futureUseCount;    // used by register assigner to keep track of how
                               // many uses are left to assign
  ncount_t _outOfLineUseCount; // holds the number of references to the register
                               // in out of line code sections

  // used by register assigner to predispose virtuals to a particular real
  // register so that they tend to end up being in the right register for
  // instructions that care (like div or call)
