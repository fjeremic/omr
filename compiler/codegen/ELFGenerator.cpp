...
void
TR::ELFGenerator::initializeTextSection(uint32_t shName,
...
void
TR::ELFGenerator::initializeDynSymSection(uint32_t shName,
                                          ELFOffset shOffset,
                                          uint32_t shSize,
                                          uint32_t shLink)
{
...
void
TR::ELFGenerator::initializeStrTabSection(uint32_t shName,
                                          ELFOffset shOffset,
                                          uint32_t shSize)
{
  ELFSectionHeader* shdr = static_cast<ELFSectionHeader*>(
...
void
TR::ELFGenerator::initializeDynStrSection(uint32_t shName,
                                          ELFOffset shOffset,
                                          uint32_t shSize)
{
  ELFSectionHeader* shdr = static_cast<ELFSectionHeader*>(
...
void
TR::ELFGenerator::initializeRelaSection(uint32_t shName,
                                        ELFOffset shOffset,
                                        uint32_t shSize)
{
  ELFSectionHeader* shdr = static_cast<ELFSectionHeader*>(
...
TR::ELFGenerator::emitELFFile(const char* filename)
{
  ...
  writeSectionNameToFile(
    elfFile, _dynSymSectionName, sizeof(_dynSymSectionName));

  writeSectionNameToFile(
    elfFile, _shStrTabSectionName, sizeof(_shStrTabSectionName));

  writeSectionNameToFile(
    elfFile, _dynStrSectionName, sizeof(_dynStrSectionName));
...
TR::ELFGenerator::writeELFSymbolsToFile(::FILE* fp)
{
  ...
  ELFSymbolNames[0] =
    0; // first bit needs to be 0, corresponding to the name of the UNDEF symbol
  char* names =
...
TR::ELFExecutableGenerator::initializePHdr(void)
{
  ...
  _programHeader->p_offset =
    sizeof(ELFEHeader); // offset of program header from the first byte of file
                        // to be loaded
...
TR::ELFExecutableGenerator::buildSectionHeaders(void)
{
  ...
  initializeStrTabSection(
    shNameOffset, symbolsStartOffset - shStrTabNameLength, shStrTabNameLength);
  shNameOffset += sizeof(_shStrTabSectionName);

  initializeDynStrSection(
    shNameOffset, symbolNamesStartOffset, _totalELFSymbolNamesLength);
...
TR::ELFRelocatableGenerator::initializeELFHeader(void)
{
  ...
  _header->e_shoff =
    sizeof(ELFEHeader) + _codeSize; // start of the section header table in
                                    // bytes from the first byte of the ELF file
...
TR::ELFRelocatableGenerator::buildSectionHeaders(void)
{
  ...
  initializeRelaSection(
    shNameOffset, relaStartOffset, _numRelocations * sizeof(ELFRela));
