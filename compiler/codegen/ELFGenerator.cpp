...
void TR::ELFGenerator::initializeELFHeaderForPlatform(void) {
  ...
  _header->e_ident[EI_OSABI] =
      ELFOSABI_LINUX; // Current support for Linux only. AIX would use the macro
                      // ELFOSABI_AIX.
...
void TR::ELFGenerator::initializeRelaSection(uint32_t shName,
  ...
  shdr->sh_offset = shOffset;
  shdr->sh_size = shSize;
  shdr->sh_link = 3; // dynsymSection index in the elf file. Kept this hardcoded
                     // for now as this shdr applies to relocatable only
  shdr->sh_info = 1;
  shdr->sh_addralign = TR::Compiler->target.is64Bit() ? 8 : 4;
...
void TR::ELFExecutableGenerator::initializeELFHeader(void) {
  ...
  _header->e_entry = (ELFAddress)
      _codeStart; // virtual address to which the system first transfers control
