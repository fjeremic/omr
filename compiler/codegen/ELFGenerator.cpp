...
void TR::ELFGenerator::initializeRelaSection(uint32_t shName, ELFOffset shOffset, uint32_t shSize)
{
  ...
  shdr->sh_size = shSize;
  shdr->sh_link
    = 3; // dynsymSection index in the elf file. Kept this hardcoded for now as this shdr applies to relocatable only
  shdr->sh_info = 1;
  shdr->sh_addralign = TR::Compiler->target.is64Bit() ? 8 : 4;
...
void TR::ELFExecutableGenerator::buildSectionHeaders(void)
{
  ...
  initializeTextSection(shNameOffset, (ELFAddress)_codeStart, sizeof(ELFEHeader) + sizeof(ELFProgramHeader), _codeSize);
...
void TR::ELFRelocatableGenerator::initializeELFHeader(void)
{
  ...
  _header->e_shoff
    = sizeof(ELFEHeader) + _codeSize; // start of the section header table in bytes from the first byte of the ELF file
...
{
  uint32_t shStrTabNameLength = sizeof(_zeroSectionName) + sizeof(_shStrTabSectionName) + sizeof(_textSectionName)
    + sizeof(_relaSectionName) + sizeof(_dynSymSectionName) + sizeof(_dynStrSectionName);
