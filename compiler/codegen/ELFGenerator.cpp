...
void TR::ELFGenerator::initializeELFHeaderForPlatform(void) {
  ...
  _header->e_ident[EI_DATA] =
      TR::Compiler->target.cpu.isLittleEndian() ? ELFDATA2LSB : ELFDATA2MSB;

  for (auto b = EI_PAD; b < EI_NIDENT; b++) _header->e_ident[b] = 0;
  _header->e_ident[EI_OSABI] =
      ELFOSABI_LINUX;  // Current support for Linux only. AIX would use the
                       // macro ELFOSABI_AIX.
...
void TR::ELFGenerator::initializeZeroSection() {
  ELFSectionHeader *shdr = static_cast<ELFSectionHeader *>(
      _rawAllocator.allocate(sizeof(ELFSectionHeader)));
...
void TR::ELFGenerator::initializeTextSection(uint32_t shName,
...
void TR::ELFGenerator::initializeDynSymSection(uint32_t shName,
                                               ELFOffset shOffset,
                                               uint32_t shSize,
                                               uint32_t shLink) {
  ELFSectionHeader *shdr = static_cast<ELFSectionHeader *>(
  ...
  shdr->sh_info =
      1;  // index of first non-local symbol: for now all symbols are global
...
void TR::ELFGenerator::initializeStrTabSection(uint32_t shName,
                                               ELFOffset shOffset,
                                               uint32_t shSize) {
  ELFSectionHeader *shdr = static_cast<ELFSectionHeader *>(
      _rawAllocator.allocate(sizeof(ELFSectionHeader)));
...
void TR::ELFGenerator::initializeDynStrSection(uint32_t shName,
                                               ELFOffset shOffset,
                                               uint32_t shSize) {
  ELFSectionHeader *shdr = static_cast<ELFSectionHeader *>(
      _rawAllocator.allocate(sizeof(ELFSectionHeader)));
...
void TR::ELFGenerator::initializeRelaSection(uint32_t shName,
                                             ELFOffset shOffset,
                                             uint32_t shSize) {
  ELFSectionHeader *shdr = static_cast<ELFSectionHeader *>(
      _rawAllocator.allocate(sizeof(ELFSectionHeader)));
  ...
  shdr->sh_size = shSize;
  shdr->sh_link =
      3;  // dynsymSection index in the elf file. Kept this hardcoded for now as
          // this shdr applies to relocatable only
  shdr->sh_info = 1;
  shdr->sh_addralign = TR::Compiler->target.is64Bit() ? 8 : 4;
...
bool TR::ELFGenerator::emitELFFile(const char *filename) {
  ...
  writeSectionNameToFile(elfFile, _dynSymSectionName,
                         sizeof(_dynSymSectionName));

  writeSectionNameToFile(elfFile, _shStrTabSectionName,
                         sizeof(_shStrTabSectionName));

  writeSectionNameToFile(elfFile, _dynStrSectionName,
...
void TR::ELFGenerator::writeHeaderToFile(::FILE *fp) {
...
void TR::ELFGenerator::writeSectionHeaderToFile(::FILE *fp,
                                                ELFSectionHeader *shdr) {
  fwrite(shdr, sizeof(uint8_t), sizeof(ELFSectionHeader), fp);
}

void TR::ELFGenerator::writeSectionNameToFile(::FILE *fp, char *name,
                                              uint32_t size) {
...
void TR::ELFGenerator::writeELFSymbolsToFile(::FILE *fp) {
  ELFSymbol *elfSym =
      static_cast<ELFSymbol *>(_rawAllocator.allocate(sizeof(ELFSymbol)));
  ...
  ELFSymbolNames[0] = 0;  // first bit needs to be 0, corresponding to the name
                          // of the UNDEF symbol
  char *names =
      ELFSymbolNames + 1;  // the rest of the array will contain method names
  TR::CodeCacheSymbol *sym = _symbols;

  const uint8_t *
      rangeStart;  // relocatable elf files need symbol offset from segment base
  if (_relaSection) {
...
void TR::ELFGenerator::writeRelaEntriesToFile(::FILE *fp) {
  if (_numRelocations > 0) {
    ...
    elfRelas->r_addend =
        0;  // addends are always 0, so it is kept out of the while loop
...
void TR::ELFExecutableGenerator::initialize(void) {
  ELFEHeader *hdr = static_cast<ELFEHeader *>(
      _rawAllocator.allocate(sizeof(ELFEHeader), std::nothrow));
  _header = hdr;

  ELFProgramHeader *phdr = static_cast<ELFProgramHeader *>(
      _rawAllocator.allocate(sizeof(ELFProgramHeader), std::nothrow));
...
void TR::ELFExecutableGenerator::initializeELFHeader(void) {
  ...
  _header->e_entry = (ELFAddress)_codeStart;  // virtual address to which the
                                              // system first transfers control
  _header->e_phoff = sizeof(ELFEHeader);  // program header offset
  _header->e_shoff = sizeof(ELFEHeader) + sizeof(ELFProgramHeader) +
                     _codeSize;  // section header offset
...
void TR::ELFExecutableGenerator::initializePHdr(void) {
  ...
  _programHeader->p_offset =
      sizeof(ELFEHeader);  // offset of program header from the first byte of
                           // file to be loaded
  _programHeader->p_vaddr =
      (ELFAddress)_codeStart;  // virtual address to load into
  _programHeader->p_paddr =
      (ELFAddress)_codeStart;            // physical address to load into
  _programHeader->p_filesz = _codeSize;  // in-file size
  _programHeader->p_memsz = _codeSize;   // in-memory size
  _programHeader->p_flags =
      PF_X |
      PF_R;  // should add PF_W if we get around to loading patchable code
  _programHeader->p_align = 0x1000;
}

void TR::ELFExecutableGenerator::buildSectionHeaders(void) {
  uint32_t shStrTabNameLength =
      sizeof(_zeroSectionName) + sizeof(_shStrTabSectionName) +
      sizeof(_textSectionName) + sizeof(_dynSymSectionName) +
      sizeof(_dynStrSectionName);

  /* offset calculations */
  uint32_t trailerStartOffset =
      sizeof(ELFEHeader) + sizeof(ELFProgramHeader) + _codeSize;
  uint32_t symbolsStartOffset = trailerStartOffset +
                                (sizeof(ELFSectionHeader) * /* #shdr */ 5) +
                                shStrTabNameLength;
  uint32_t symbolNamesStartOffset =
      symbolsStartOffset + ((_numSymbols + /* UNDEF */ 1) * sizeof(ELFSymbol));
  uint32_t shNameOffset = 0;

  initializeZeroSection();
  shNameOffset += sizeof(_zeroSectionName);

  initializeTextSection(shNameOffset, (ELFAddress)_codeStart,
                        sizeof(ELFEHeader) + sizeof(ELFProgramHeader),
                        _codeSize);
  shNameOffset += sizeof(_textSectionName);

  initializeDynSymSection(shNameOffset, symbolsStartOffset,
  ...
  initializeDynStrSection(shNameOffset, symbolNamesStartOffset,
                          _totalELFSymbolNamesLength);
  shNameOffset += sizeof(_dynStrSectionName);
}

bool TR::ELFExecutableGenerator::emitELF(const char *filename,
                                         CodeCacheSymbol *symbols,
                                         uint32_t numSymbols,
...
void TR::ELFRelocatableGenerator::initializeELFHeader(void) {
  ...
  _header->e_shoff = sizeof(ELFEHeader) +
                     _codeSize;  // start of the section header table in bytes
                                 // from the first byte of the ELF file
...
void TR::ELFRelocatableGenerator::buildSectionHeaders(void) {
  uint32_t shStrTabNameLength =
      sizeof(_zeroSectionName) + sizeof(_shStrTabSectionName) +
      sizeof(_textSectionName) + sizeof(_relaSectionName) +
  ...
  initializeRelaSection(shNameOffset, relaStartOffset,
...
bool TR::ELFRelocatableGenerator::emitELF(const char *filename,
                                          CodeCacheSymbol *symbols,
                                          uint32_t numSymbols,
                                          uint32_t totalELFSymbolNamesLength,
