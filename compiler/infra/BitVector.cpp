...
int32_t
TR_BitVector::commonElementCount(TR_BitVector& v2)
{
  if (v2._lastChunkWithNonZero < _firstChunkWithNonZero ||
      v2._firstChunkWithNonZero > _lastChunkWithNonZero)
    return false; // No intersection
  int32_t low = _firstChunkWithNonZero >= v2._firstChunkWithNonZero
                  ? _firstChunkWithNonZero
                  : v2._firstChunkWithNonZero;
  int32_t high = _lastChunkWithNonZero <= v2._lastChunkWithNonZero
                   ? _lastChunkWithNonZero
                   : v2._lastChunkWithNonZero;
  int32_t count = 0;
...
bool
TR_BitVector::hasMoreThanOneElement()
{
...
void
TR_BitVector::setChunkSize(int32_t chunkSize)
{
  ...
  if (_numChunks > chunkSize) {
    ...
    if (_lastChunkWithNonZero < 0)
      ...
      for (_lastChunkWithNonZero = chunkSize - 1;
           _chunks[_lastChunkWithNonZero] == 0;
           _lastChunkWithNonZero--)
        ;
    }
  } else {
  ...
  chunk_t* newChunks =
    _region != NULL ? (chunk_t*)_region->allocate(chunkSize * sizeof(chunk_t))
                    : (chunk_t*)TR_Memory::jitPersistentAlloc(
                        chunkSize * sizeof(chunk_t), TR_Memory::BitVector);
...
const char*
TR_BitVector::getHexString()
{
  int32_t chunk_hexlen = (BITS_IN_CHUNK / 4);
  char* buf =
    _region
      ? (char*)_region->allocate(_numChunks * chunk_hexlen + 1)
      : (char*)TR_Memory::jitPersistentAlloc(_numChunks * chunk_hexlen + 1);
  char* pos = buf;
#ifdef TRACK_TRBITVECTOR_MEMORY
...
void
TR_BitVector::print(TR::Compilation* comp, TR::FILE* file)
{
  if (comp->getDebug()) {
    if (file == NULL)
...
void
TR_SingleBitContainer::print(TR::Compilation* comp, TR::FILE* file)
{
  if (comp->getDebug()) {
    if (file == NULL)
      file = comp->getOutFile();
...
//
inline bool
TR_BitContainer::operator==(TR_BitVector& v2)
{
  if (_type == bitvector)
...
bool
TR_BitContainer::hasMoreThanOneElement()
{
...
bool
TR_BitContainer::intersects(TR_BitVector& v2)
{
  if (_type == bitvector && _bitVector)
...
bool
TR_BitContainer::intersects(TR_BitContainer& v2)
{
