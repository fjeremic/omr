...
bool TR_BitVector::hasMoreThanOneElement() {
  if (_firstChunkWithNonZero < _lastChunkWithNonZero) return true;
  if (_lastChunkWithNonZero < 0) return false;
  int32_t count = 0;
...
void TR_BitVector::setChunkSize(int32_t chunkSize) {
  if (chunkSize == _numChunks) return;
  if (chunkSize == 0) {
  ...
  if (_numChunks > chunkSize) {
    ...
    if (_lastChunkWithNonZero < 0)
      ...
      for (_lastChunkWithNonZero = chunkSize - 1;
           _chunks[_lastChunkWithNonZero] == 0; _lastChunkWithNonZero--)
        ;
    }
  } else {
    // Growing the bit vector
    if (_lastChunkWithNonZero < 0) _firstChunkWithNonZero = chunkSize;
  }
  ...
  if (_chunks) {
    ...
    if (_region == NULL) jitPersistentFree(_chunks);
  }
...
const char *TR_BitVector::getHexString() {
  ...
  char *buf = _region ? (char *)_region->allocate(_numChunks * chunk_hexlen + 1)
                      : (char *)TR_Memory::jitPersistentAlloc(
                            _numChunks * chunk_hexlen + 1);
  char *pos = buf;
#ifdef TRACK_TRBITVECTOR_MEMORY
...
void TR_BitVector::print(TR::Compilation *comp, TR::FILE *file) {
  if (comp->getDebug()) {
    if (file == NULL) file = comp->getOutFile();
