 ...
 *
 * \def TR::trap                a convenience function for taking down the VM
 ...
 *
 * \def TR_ASSERT               a macro that defines an assertion which is
 * compiled out (including format strings) during production builds
 *
 * \def TR_UNIMPLEMENTED        a macro that expands into an unconditional
 * assertion failure. Used to indicate that a function has not been implemented
 * and may not be called.
 *
 * We also provide Expect and Ensure, based on the developing C++ Core
 * guidelines [1].
 *
...
void OMR_NORETURN fatal_assertion(const char* file,
                                  int line,
...
// no-return.
void assertion(const char* file,
               int line,
               const char* condition,
               const char* format,
               ...);
...
#define TR_ASSERT_FATAL(condition, format, ...)                               \
  do {                                                                        \
    (condition) ? (void)0                                                     \
                : TR::fatal_assertion(__FILE__, __LINE__, #condition, format, \
                                      ##__VA_ARGS__);                         \
  } while (0)

#define TR_UNIMPLEMENTED()                        \
  ::TR::fatal_assertion(__FILE__, __LINE__, NULL, \
                        "Unimplemented function: %s", __FUNCTION__)
...
#define TR_ASSERT(condition, format, ...)                               \
  do {                                                                  \
    (condition) ? (void)0                                               \
                : TR::assertion(__FILE__, __LINE__, #condition, format, \
                                ##__VA_ARGS__);                         \
  } while (0)
