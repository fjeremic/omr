...
void OMR_NORETURN
fatal_assertion(const char* file,
                int line,
...
#define TR_ASSERT_FATAL(condition, format, ...)                                \
  do {                                                                         \
    (condition) ? (void)0                                                      \
                : TR::fatal_assertion(                                         \
                    __FILE__, __LINE__, #condition, format, ##__VA_ARGS__);    \
  } while (0)

#define TR_UNIMPLEMENTED()                                                     \
  ::TR::fatal_assertion(                                                       \
    __FILE__, __LINE__, NULL, "Unimplemented function: %s", __FUNCTION__)
...
#define TR_ASSERT(condition, format, ...)                                      \
  do {                                                                         \
    (condition)                                                                \
      ? (void)0                                                                \
      : TR::assertion(__FILE__, __LINE__, #condition, format, ##__VA_ARGS__);  \
  } while (0)
