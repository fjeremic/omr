...
private:
  ...
  // The _interferenceMatrix is a lower triangular bit matrix that summarizes
  // interferences between nodes.  For a node pair (L,H), the corresponding bit
  // should be set if the two nodes interfere.
  //
  ...
  //                  0  1  2  3  4  5 ...
  //                +---------------------
  //               0| X                         The bit position for a node pair
  //               (L,H) 1| 0  X                      can be computed with:
  //   High Node   2| 1  2  X
...
public:
  ...
  TR_IGBase()
      : _numNodes(0), _numColours(0), _numberOfColoursUsedToColour(0),
  ...
  IGNodeColour getNumberOfColoursUsedToColour() {
    return _numberOfColoursUsedToColour;
  }
  IGNodeColour setNumberOfColoursUsedToColour(IGNodeColour c) {
    return (_numberOfColoursUsedToColour = c);
  }
};
#endif
