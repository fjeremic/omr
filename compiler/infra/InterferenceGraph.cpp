...
TR_InterferenceGraph::TR_InterferenceGraph(TR::Compilation *comp,
    ...
    : _compilation(comp), _nodeTable(NULL), _nodeStack(NULL),
      _trMemory(comp->trMemory()), TR_IGBase() {
  ...
  setInterferenceMatrix(new (trHeapMemory()) TR_BitVector(numBits, trMemory(),
                                                          heapAlloc, growable));
...
void TR_InterferenceGraph::removeInterferenceBetween(TR_IGNode *igNode1,
                                                     TR_IGNode *igNode2) {
...
void TR_InterferenceGraph::partitionNodesIntoDegreeSets(
  ...
  if (debug("traceIG")) {
    ...
    diagnostic("\n[%4d] degree < %-10d : ",
               colourableDegreeSet.PopulationCount(), getNumColours());
...
bool TR_InterferenceGraph::simplify() {
  ...
  while (!workingSet.IsZero()) {
    partitionNodesIntoDegreeSets(workingSet, colourableDegreeSet,
      ...
      for (colourableCursor.SetToFirstOne(); colourableCursor.Valid();
        ...
        if (debug("traceIG")) {
          diagnostic("SIMPLIFY: Selected colourable IG node #%d with degree "
                     "%d: (igNode=%p, entity=%p)\n",
                     igNode->getIndex(), igNode->getWorkingDegree(), igNode,
                     igNode->getEntity());
        }
