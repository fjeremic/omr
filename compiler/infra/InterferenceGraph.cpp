...
TR_InterferenceGraph::TR_InterferenceGraph(TR::Compilation* comp,
                                           int32_t estimatedNodes)
  : _compilation(comp)
  , _nodeTable(NULL)
  , _nodeStack(NULL)
  ...
  , TR_IGBase()
{
  TR_ASSERT(estimatedNodes > 0,
            "interference graph must be created with a node estimate!");

  int32_t numBits = (estimatedNodes * (estimatedNodes - 1)) >> 1;
  setInterferenceMatrix(new (trHeapMemory()) TR_BitVector(
    numBits, trMemory(), heapAlloc, growable));
  _nodeTable = new (trHeapMemory())
    TR_Array<TR_IGNode*>(trMemory(), estimatedNodes, false, heapAlloc);
  _nodeStack = new (trHeapMemory())
    TR_Stack<TR_IGNode*>(trMemory(), estimatedNodes, false, heapAlloc);

  // TODO: allocate from stack memory
  //
  _entityHash._numBuckets = 73;
  _entityHash._buckets = (HashTableEntry**)trMemory()->allocateHeapMemory(
    _entityHash._numBuckets * sizeof(HashTableEntry*));
  memset(
    _entityHash._buckets, 0, _entityHash._numBuckets * sizeof(HashTableEntry*));
}

// Add a new, unique entity to the interference graph.
//
TR_IGNode*
TR_InterferenceGraph::add(void* entity, bool ignoreDuplicates)
{
  ...
  TR_ASSERT(
    !igNode, "entity %p already exists in this interference graph\n", entity);
...
//
void
TR_InterferenceGraph::addInterferenceBetween(TR_IGNode* igNode1,
                                             TR_IGNode* igNode2)
{
  IMIndex bitNum =
    getNodePairToBVIndex(igNode1->getIndex(), igNode2->getIndex());
...
bool
TR_InterferenceGraph::hasInterference(void* entity1, void* entity2)
{
  ...
  TR_ASSERT(
    igNode1,
    "hasInterference: entity1 %p does not exist in this interference graph\n",
    entity1);
  TR_ASSERT(
    igNode2,
    "hasInterference: entity2 %p does not exist in this interference graph\n",
    entity2);
...
void
TR_InterferenceGraph::removeInterferenceBetween(TR_IGNode* igNode1,
                                                TR_IGNode* igNode2)
{
...
void
TR_InterferenceGraph::removeAllInterferences(void* entity)
{
  TR_IGNode* igNode = getIGNodeForEntity(entity);

  TR_ASSERT(
    igNode,
    "removeAllInterferences: entity %p is not in the interference graph\n",
    entity);
...
void
TR_InterferenceGraph::addIGNodeToEntityHash(TR_IGNode* igNode)
{
  void* entity = igNode->getEntity();

  TR_ASSERT(entity,
            "addIGNodeToEntityHash: IGNode %p does not reference its data\n",
            igNode);
  ...
  HashTableEntry* entry =
    (HashTableEntry*)trMemory()->allocateHeapMemory(sizeof(HashTableEntry));
...
TR_IGNode*
TR_InterferenceGraph::getIGNodeForEntity(void* entity)
{
  int32_t hashBucket = entityHashBucket(entity);
...
// Doesn't actually remove a node from the IG, but adjusts the working degrees
// of its neighbours and itself to appear that way.
//
void
TR_InterferenceGraph::virtualRemoveEntityFromIG(void* entity)
{
  TR_IGNode* igNode = getIGNodeForEntity(entity);
...
//
void
TR_InterferenceGraph::virtualRemoveNodeFromIG(TR_IGNode* igNode)
{
  igNode->decWorkingDegreeOfNeighbours();
  igNode->setIsRemovedFromIG();
  igNode->setWorkingDegree(0);
...
//
void
TR_InterferenceGraph::partitionNodesIntoDegreeSets(
  CS2::ABitVector<TR::Allocator>& workingSet,
  ...
  CS2::ABitVector<TR::Allocator>& notColourableDegreeSet)
{
  ...
  if (debug("traceIG")) {
    ...
    diagnostic("\n[%4d] degree < %-10d : ",
               colourableDegreeSet.PopulationCount(),
               getNumColours());
    (*comp()) << colourableDegreeSet;

    diagnostic("\n[%4d] degree < MAX_DEGREE : ",
               notColourableDegreeSet.PopulationCount());
...
IGNodeDegree
TR_InterferenceGraph::findMaxDegree()
{
...
//
bool
TR_InterferenceGraph::doColouring(IGNodeColour numColours)
{
  bool success;
  ...
  if (debug("traceIG")) {
    if (!success) {
      diagnostic(
        "COLOUR IG: Could not find a colouring for IG %p with %d colours.\n",
        this,
        numColours);
    } else {
      diagnostic("COLOUR IG: IG %p coloured with %d colours.\n",
                 this,
                 getNumberOfColoursUsedToColour());
    }
  }
...
// Find the minimum number of colours required to colour this interference
// graph.
//
IGNodeColour
TR_InterferenceGraph::findMinimumChromaticNumber()
{
  return 0;
}
...
TR_InterferenceGraph::simplify()
{
  ...
  while (!workingSet.IsZero()) {
    partitionNodesIntoDegreeSets(
      workingSet, colourableDegreeSet, notColourableDegreeSet);

    // Push nodes from the colourable set onto the stack and adjust the degrees
    // of their neighbours until there are no nodes remaining in the colourable
    // set.
    //
    if (!colourableDegreeSet.IsZero()) {
      CS2::ABitVector<TR::Allocator>::Cursor colourableCursor(
        colourableDegreeSet);

      for (colourableCursor.SetToFirstOne(); colourableCursor.Valid();
           colourableCursor.SetToNextOne()) {
        igNode = getNodeTable(colourableCursor);

        if (debug("traceIG")) {
          diagnostic("SIMPLIFY: Selected colourable IG node #%d with degree "
                     "%d: (igNode=%p, entity=%p)\n",
                     igNode->getIndex(),
                     igNode->getWorkingDegree(),
                     igNode,
                     igNode->getEntity());
        }
    ...
    // There are no nodes left in the colourable set.  Choose a spill candidate
    // among nodes in the non-colourable degree set.
    //
    TR_ASSERT(!notColourableDegreeSet.IsZero(),
              "not colourable set must contain at least one member\n");

    CS2::ABitVector<TR::Allocator>::Cursor notColourableCursor(
      notColourableDegreeSet);
    if (!notColourableDegreeSet.IsZero()) {
      ...
      bestSpillNode = NULL;
      for (notColourableCursor.SetToFirstOne(); notColourableCursor.Valid();
           notColourableCursor.SetToNextOne()) {
...
TR_InterferenceGraph::select()
{
  ...
  while (!getNodeStack()->isEmpty()) {
    ...
    if (debug("traceIG")) {
      diagnostic("SELECT:  For IG node #%d (%p), available colours = ",
                 igNode->getIndex(),
                 igNode);
...
#ifdef DEBUG
void
TR_InterferenceGraph::dumpIG(char* msg)
{
  if (msg) {
