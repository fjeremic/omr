 ...
 public:
  ...
  void report(FILE *file) const {
    if (samples() >= 2)
      ...
      fprintf(file, "Statistics on: %s   Num samples = %u SUM=%f\n", _name,
              samples(), sum());
    if (_intervalerr) fprintf(file, "Invalid statistics; interval error\n");
  }
  double stddev() const {
    double s = 0.0;
    if (_sumwt != 0.0 && _sumwt != 1.0) {
      double variance = ((_sumsq - (_sum / _sumwt) * (_sum / _sumwt) * _sumwt) /
                         (_sumwt - 1.0));
      if (variance > 0.0) s = sqrt(variance);
    }
    return s;
  }
...
// integer. The number of events to monitor is given by the template parameter
// N. The ID of the first event is given as parameter to the constructor
// (minEventId). The constructor also needs an array of strings which designate
// the names of the events (binNames). Of course, 'binNames' should have at
// least N entries. There is no MIN/MAX/AVG/STDDEV defined
//--------------------------------------------------------------------------
template <int N>
