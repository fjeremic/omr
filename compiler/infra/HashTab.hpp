...
public:
  ...
  virtual TR_HashId calculateHash(const void *key) const {
    return (TR_HashId((uintptrj_t)key) >> 2) % _closedAreaSize;
  }
  virtual bool isEqual(const void *key1, const void *key2) const {
    return key1 == key2;
  }

  TR_HashTab(TR_Memory *mem, TR_AllocationKind a = heapAlloc,
             uint32_t initialSize = kDefaultSize, bool g = true)
      : _tableSize(0), _allocType(a), _table(NULL), _closedAreaSize(0),
        _allowGrowth(g), _trMemory(mem) {
    init(initialSize);
    _trace = false;
  }
  ...
  bool add(void *key, TR_HashId hashIndex, void *data) {
    TR_HashTableEntry *entry =
        new (trMemory(), _allocType) TR_HashTableEntry(key, data, 0);
...
private:
  ...
  public:
    ...
    TR_HashTableEntry() : _chain(0), _data(NULL) {}
    ...
    TR_HashTableEntry(const TR_HashTableEntry &entry)
        : _key(entry._key), _chain(entry._chain), _data(entry._data) {}
...
public:
  TR_HashTabInt(TR_Memory *mem, TR_AllocationKind allocType = heapAlloc,
  ...
  TR_HashId calculateHash(const void *key) const {
    return (TR_HashId((intptrj_t)key) % _closedAreaSize);
  }

  bool locate(int32_t key, TR_HashId &hashIndex) {
    return TR_HashTab::locate((const void *)(uintptrj_t)key, hashIndex);
  }
  bool add(int32_t key, TR_HashId hashIndex, void *data) {
    return TR_HashTab::add((void *)(uintptrj_t)key, hashIndex, data);
  }
};

class TR_HashTabString : public TR_HashTab {
public:
  TR_HashTabString(TR_Memory *mem, TR_AllocationKind allocType = heapAlloc,
                   uint32_t initialSize = kDefaultSize, bool allowGrowth = true)
      : TR_HashTab(mem, allocType, initialSize, allowGrowth) {}
  ...
  bool locate(const char *key, TR_HashId &hashIndex) {
    return TR_HashTab::locate((const void *)key, hashIndex);
  }
  bool add(const char *key, TR_HashId hashIndex, void *data) {
    return TR_HashTab::add((void *)key, hashIndex, data);
  }
};
class TR_HashTabDouble : public TR_HashTab {
public:
  TR_HashTabDouble(TR_Memory *mem, TR_AllocationKind allocType = heapAlloc,
                   uint32_t initialSize = kDefaultSize, bool allowGrowth = true)
      : TR_HashTab(mem, allocType, initialSize, allowGrowth) {}

  TR_HashId calculateHash(
      const void *key) const // TODO: research better hash fns for double
  {
    union {
      double dbl;
      struct {
  ...
  bool locate(const double *key, TR_HashId &hashIndex) {
    return TR_HashTab::locate((const void *)key, hashIndex);
  }
  bool add(double *key, TR_HashId hashIndex, void *data) {
    return TR_HashTab::add((void *)key, hashIndex, data);
  }
};
...
public:
  TR_HashTabLong(TR_Memory *mem, TR_AllocationKind allocType = heapAlloc,
                 uint32_t initialSize = kDefaultSize, bool allowGrowth = true)
      : TR_HashTab(mem, allocType, initialSize, allowGrowth) {}
  ...
  bool locate(const int64_t *key, TR_HashId &hashIndex) {
    return TR_HashTab::locate((const void *)key, hashIndex);
  }
  bool add(int64_t *key, TR_HashId hashIndex, void *data) {
    return TR_HashTab::add((void *)key, hashIndex, data);
  }
};
...
public:
  ...
  bool locate(const float *key, TR_HashId &hashIndex) {
    return TR_HashTab::locate((const void *)key, hashIndex);
  }
  bool add(float *key, TR_HashId hashIndex, void *data) {
    return TR_HashTab::add((void *)key, hashIndex, data);
  }
};
