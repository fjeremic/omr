...
operator=(const reference_wrapper &other) throw() {
  m_ref = other.m_ref;
  return *this;
}
...
template <typename T> reference_wrapper<T>::operator reference_type() throw() {
  return *m_ref;
}

template <typename T> reference_wrapper<T> inline ref(T &reference) {
