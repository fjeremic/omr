...
template <typename T>
reference_wrapper<T>& reference_wrapper<T>::operator=(const reference_wrapper& other) throw()
{
  m_ref = other.m_ref;
  return *this;
}
...
template <typename T>
typename reference_wrapper<T>::reference_type reference_wrapper<T>::get() throw()
{
  return *m_ref;
}
