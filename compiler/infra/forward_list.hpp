 ...
 * @class TR::forward_list
 *
 * @brief A partial re-implementation of std::forward_list for C++03
 * supplemented by a linear-time size method.
 *
 * std::forward_list is a container introduced in the 2011 revision of the C++
 * standard for which the objective is to provide the performance of a
 * singly-linked list. Because the continer is singly-linked, the interfaces
 * common to other containers, where manipulations are performed on the
 * half-closed interval [begin, end), are problematic. To provide a work-around,
 * std::forward_list provides similar operations that operate on the open
 * interval (before_begin, end), and provide [operation]_after variants that
 * perform the requested operation on the position immediately following the
 * provided iterator.
 *
 * A further result of the strict requirement of being equally performant to a
 * manually maintained singly-linked list, is that std::forward_list does not
 * provide a size method, as it cannot provide one that operates in O(1) time.
 * However, there are a number of places in our code base that depend on being
 * able to request the size of a container, where the container is one targetted
 * for re-implementation as a forward_list. To reconcile these requirements, we
 * provide an extension to std::forward_list that provides a size() operation
 * that runs in time linear to the number of objects stored in the container.
 *
 */
...
public:
  ...
  //   explicit forward_list(size_type size, const allocator_type &allocator =
  //   allocator_type()); forward_list(size_type size, const value_type&
  //   initialValue, const allocator_type &allocator = allocator_type());
  ...
  //   InputIterator last); void assign(size_type n, const value_type &value);
  ...
  //   iterator insert_after(const_iterator position, size_type count, const
  //   value_type& value);
  template <typename InputIterator>
  iterator insert_after(iterator position, InputIterator first,
                        InputIterator last);
  ...
  //   void splice_after(const_iterator position, forward_list &donor,
  //   const_iterator breakPoint); void splice_after(const_iterator position,
  //   forward_list &donor, const_iterator nearBound, const_iterator farBound);
...
private:
  ...
  typedef typename allocator_type::template rebind<ListElement>::other
      ListElementAllocator;
...
TR::forward_list<T, Alloc>::forward_list(const allocator_type &allocator)
    : _head(NULL), _allocator(allocator) {}

// template < typename T, typename Alloc >
// TR::forward_list< T, Alloc >::forward_list(size_type size, const
...
// template < typename T, typename Alloc >
// TR::forward_list< T, Alloc >::forward_list(size_type size, const value_type
// &initialValue, const allocator_type &allocator) :
//   _head(NULL),
//   _allocator(allocator)
//   {
//   }
...
// template < typename T, typename Alloc >
// template < typename InputIterator >
// TR::forward_list< T, Alloc >::forward_list(InputIterator first, InputIterator
// last, const allocator_type &allocator) :
//   _head(NULL),
//   _allocator(allocator)
//   {
//   }
...
TR::forward_list<T, Alloc>::forward_list(const forward_list &prototype)
    : _head(NULL), _allocator(prototype._allocator) {
...
template <typename T, typename Alloc>
bool TR::forward_list<T, Alloc>::empty() const throw() {
...
template <typename T, typename Alloc>
void TR::forward_list<T, Alloc>::push_front(const value_type &value) {
...
template <typename T, typename Alloc>
void TR::forward_list<T, Alloc>::pop_front() {
...
template <typename T, typename Alloc>
typename TR::forward_list<T, Alloc>::iterator
TR::forward_list<T, Alloc>::insert_after(iterator position,
                                         const value_type &value) {
  ListElement *newElement = _allocator.allocate(1);
  _allocator.construct(newElement,
                       ListElement(position._element->_next, value));
  position._element->_next = newElement;
}

// template < typename T, typename Alloc >
// typename TR::forward_list< T, Alloc >::iterator
// TR::forward_list< T, Alloc >::insert_after(const_iterator position, size_type
...
template <typename InputIterator>
typename TR::forward_list<T, Alloc>::iterator
TR::forward_list<T, Alloc>::insert_after(iterator position, InputIterator first,
                                         InputIterator last) {
  Link *const farBound = position._element->_next;

  try {
    Link *prev = position._element;
    for (auto currentPosition = first; currentPosition != last;
         ++currentPosition) {
      ListElement *newElement = _allocator.allocate(1);
      try {
        _allocator.construct(newElement,
                             ListElement(farBound, *currentPosition));
      } catch (...) {
...
// template < typename T, typename Alloc >
// typename TR::forward_list< T, Alloc >::iterator
// TR::forward_list< T, Alloc >::erase_after(const_iterator position, iterator
// last)
//   {
//   }
...
// template < typename T, typename Alloc >
// void
// TR::forward_list< T, Alloc >::resize(size_type newSize, const value_type
// &value)
//   {
//   }

template <typename T, typename Alloc>
void TR::forward_list<T, Alloc>::clear() throw() {
...
// template < typename T, typename Alloc >
// void
// TR::forward_list< T, Alloc >::splice_after(const_iterator position,
// forward_list &donor)
//   {
//   }
...
// void
// TR::forward_list< T, Alloc >::splice_after(const_iterator position,
// forward_list &donor, const_iterator breakPoint)
//   {
//   }
...
template <typename T, typename Alloc>
void TR::forward_list<T, Alloc>::remove(const value_type &value) {
  for (auto preceding = before_begin(), current = begin(); current != end();
       ++preceding, ++current) {
    while (*current == value) {
...
template <typename T, typename Alloc>
typename TR::forward_list<T, Alloc>::const_iterator &
TR::forward_list<T, Alloc>::const_iterator::
...
template <typename T, typename Alloc>
TR::forward_list<T, Alloc>::iterator::iterator() : _element(degenerate()) {}
