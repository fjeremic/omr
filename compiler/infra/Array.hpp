...
public:
  ...
  TR_Array()
      : _nextIndex(0), _internalSize(0), _allocationKind(heapAlloc),
        _zeroInit(false), _trMemory(0), _trPersistentMemory(0), _array(NULL) {}
...
protected:
  void copy(const TR_Array<T> &other) {
    ...
    if (_trMemory)
      ...
      _array = (T *)_trPersistentMemory->allocatePersistentMemory(
          _internalSize * sizeof(T));
  ...
  void growTo(uint32_t size) {
    ...
    if (_trMemory)
      ...
      TR_ASSERT(false, "Cannot allocate memory for array with no TR_Memory or "
                       "TR_PersistentMemory.");
