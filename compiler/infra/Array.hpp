...
public:
  ...
  TR_Array(const TR_Array<T>& other)
  {
    copy(other);
  }
  ...
  {
    TR_ASSERT(
      index < _nextIndex, "TR_Array::operator[] const, Invalid array index %d while size is %d", index, _nextIndex);
    return _array[index];
  }
...
public:
  ...
  bool atEnd()
  {
    return (_cursor >= _array->size() - 1);
  }
  bool pastEnd()
  {
    return (_cursor >= _array->size());
  }
};
