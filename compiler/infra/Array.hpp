...
class TR_Array {
 public:
  ...
  TR_Array(TR_Memory* m, uint32_t initialSize = 8, bool zeroInit = true,
           TR_AllocationKind allocKind = heapAlloc) {
    init(m, m->trPersistentMemory(), initialSize, zeroInit, allocKind);
  }

  TR_Array(TR_Memory* m, TR_AllocationKind allocKind, uint32_t initialSize = 8,
           bool zeroInit = true) {
    init(m, m->trPersistentMemory(), initialSize, zeroInit, allocKind);
  }

  TR_Array(TR_PersistentMemory* pm, uint32_t initialSize = 8,
           bool zeroInit = true) {
    init(0, pm, initialSize, zeroInit, persistentAlloc);
  }

  void init(TR_Memory* m, uint32_t initialSize = 8, bool zeroInit = true,
            TR_AllocationKind allocKind = heapAlloc) {
    init(m, m->trPersistentMemory(), initialSize, zeroInit, allocKind);
  }

  void init(TR_Memory* m, TR_PersistentMemory* pm, uint32_t initialSize = 8,
            bool zeroInit = true, TR_AllocationKind allocKind = heapAlloc) {
    _nextIndex = 0;
    ...
    if (m)
      _array = (T*)m->allocateMemory(initialSize * sizeof(T), _allocationKind,
                                     TR_MemoryBase::Array);
    else if (pm)
      _array = (T*)pm->allocatePersistentMemory(initialSize * sizeof(T));
    else
      TR_ASSERT(false,
                "Attempting to allocate an array without a memory object");
    if (zeroInit) memset(_array, 0, initialSize * sizeof(T));
  }
  ...
  /**
   * @brief Copies the provided object into the first unused element at the end
   * of the array and returns the index to the newly created element.  This may
   * require growing the backing array.
   * @param t The object to be copied into the array.
   * @return The index of the newly created element.
   */
  uint32_t add(T t) {
    if (_nextIndex == _internalSize) {
      TR_ASSERT(_internalSize < UINT_MAX / 2,
                "Array trying to grow bigger than UINT_MAX\n");
  ...
  void insert(T t, uint32_t index) {
    if (_nextIndex <= index)
      ...
      for (uint32_t i = _nextIndex; i > index; --i) _array[i] = _array[i - 1];
      _array[index] = t;
      _nextIndex++;
    }
  }
  ...
  void append(const TR_Array<T>& other) {
    for (uint32_t i = 0; i < other._nextIndex; i++) add(other[i]);
  }

  void remove(uint32_t index) {
    TR_ASSERT(index < _nextIndex, "TR_Array::remove, index >= _nextIndex");
    for (uint32_t i = index + 1; i < _nextIndex; ++i) _array[i - 1] = _array[i];
    --_nextIndex;
  }
  ...
  T& element(uint32_t index) {
    TR_ASSERT(index < _nextIndex,
              "TR_Array::element, Invalid array index %d while size is %d",
              index, _nextIndex);
    return _array[index];
  }
  ...
  T& operator[](uint32_t index) {
    if (index >= _nextIndex) {
      if (index >= _internalSize) growTo(index + _internalSize);
      _nextIndex = index + 1;
    }
  ...
  const T& operator[](uint32_t index) const {
    TR_ASSERT(
        index < _nextIndex,
        "TR_Array::operator[] const, Invalid array index %d while size is %d",
        index, _nextIndex);
    return _array[index];
  }
  ...
  void freeMemory() {
    if (_allocationKind == persistentAlloc) jitPersistentFree(_array);
  }
 ...
 protected:
  void copy(const TR_Array<T>& other) {
    ...
    if (_trMemory)
      _array = (T*)_trMemory->allocateMemory(
          _internalSize * sizeof(T), _allocationKind, TR_MemoryBase::Array);
    else if (_trPersistentMemory)
      _array = (T*)_trPersistentMemory->allocatePersistentMemory(_internalSize *
                                                                 sizeof(T));
  ...
  void growTo(uint32_t size) {
    ...
    char* tmpArray;
    if (_trMemory)
      tmpArray = (char*)_trMemory->allocateMemory(newSize, _allocationKind,
                                                  TR_MemoryBase::Array);
    else if (_trPersistentMemory)
      tmpArray = (char*)_trPersistentMemory->allocatePersistentMemory(newSize);
    else
      TR_ASSERT(false,
                "Cannot allocate memory for array with no TR_Memory or "
                "TR_PersistentMemory.");
...
template <class T>
class TR_PersistentArray : public TR_Array<T> {
 public:
  TR_ALLOC(TR_Memory::Array);
  TR_PersistentArray(TR_Memory* m, uint32_t initialSize = 8,
                     bool zeroInit = true)
      : TR_Array<T>(m, initialSize, zeroInit, persistentAlloc) {}
  TR_PersistentArray(TR_PersistentMemory* m, uint32_t initialSize = 8,
                     bool zeroInit = true)
      : TR_Array<T>(m, initialSize, zeroInit) {}
};

// Use to traverse non-null entries of a TR_Array<T *>
// Note the difference between the the template types for the iter, vs. the
// array
//
template <class T>
class TR_ArrayIterator {
 private:
  uint32_t _cursor;
  TR_Array<T*>* _array;
