...
public:
  ...
  TR_Array(TR_Memory* m,
           uint32_t initialSize = 8,
           bool zeroInit = true,
  ...
  TR_Array(TR_PersistentMemory* pm,
           uint32_t initialSize = 8,
           bool zeroInit = true)
  {
            ...
            TR_AllocationKind allocKind = heapAlloc)
  {
    ...
    if (m)
      _array = (T*)m->allocateMemory(
        initialSize * sizeof(T), _allocationKind, TR_MemoryBase::Array);
    else if (pm)
...
protected:
  ...
  void growTo(uint32_t size)
  {
    ...
    char* tmpArray;
    if (_trMemory)
      tmpArray = (char*)_trMemory->allocateMemory(
        newSize, _allocationKind, TR_MemoryBase::Array);
    else if (_trPersistentMemory)
...
public:
  TR_ALLOC(TR_Memory::Array);
  TR_PersistentArray(TR_Memory* m,
                     uint32_t initialSize = 8,
                     bool zeroInit = true)
    : TR_Array<T>(m, initialSize, zeroInit, persistentAlloc)
  {}
  TR_PersistentArray(TR_PersistentMemory* m,
                     uint32_t initialSize = 8,
                     bool zeroInit = true)
