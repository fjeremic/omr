...
void
TR::TrilDumper::visitingMethod(TR::ResolvedMethodSymbol* method)
{
  ...
  fprintf(_outputFile,
          "(method return=%s ",
          TR::DataType::getName(resolvedMethod->returnType()));
  ...
  // print the argument types, skipping the `arg` field entirely if there are
  // none
  ...
  if (!paramIter.atEnd()) {
    ...
    fprintf(
      _outputFile, "args=[%s", TR::DataType::getName(param->getDataType()));
...
void
TR::TrilDumper::returnedToMethod(TR::ResolvedMethodSymbol* method)
{
  fprintf(_outputFile, ") ");
}

void
TR::TrilDumper::visitingNode(TR::Node* node)
{
  ...
  if (opcodeValue == TR::BBStart) {
    ...
    if (opcode.isLoadConst()) {
      ...
      fprintf(
        _outputFile, "offset=%ld ", node->getSymbolReference()->getOffset());
    } else if (opcode.isBranch()) {
      // if the node is a branch, print the name of the target block
      auto target =
        node->getBranchDestination()->getEnclosingBlock(true)->getNumber();
      if (target == 1) {
...
void
TR::TrilDumper::visitingCommonedChildNode(TR::Node* node)
{
...
void
TR::TrilDumper::visitedAllChildrenOfNode(TR::Node* node)
{
  auto opcode = node->getOpCode();
  auto opcodeValue = opcode.getOpCodeValue();
