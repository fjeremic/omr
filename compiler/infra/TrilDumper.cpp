...
void TR::TrilDumper::visitingMethod(TR::ResolvedMethodSymbol* method) {
  ...
  fprintf(_outputFile, "(method return=%s ",
  ...
  if (!paramIter.atEnd()) {
    ...
    fprintf(_outputFile, "args=[%s",
            TR::DataType::getName(param->getDataType()));
    ...
    if (opcode.isLoadConst()) {
      ...
      if (opcode.isInteger()) {
        fprintf(_outputFile, "%ld ", node->get64bitIntegralValue());
                     ...
                     .isIntegerOrAddress())  // since there is no `isAddress()`
      {
      ...
      if (sym->isParm()) {
        ...
        fprintf(_outputFile, "temp=\"symref_%d\" ",
