 ...
 *
 * This class uses the Observer pattern to separate the actual traversal
 * mechanism from the code that needs to do the traversal. An observer can
 * register itself with an instance of this class to be notified of the various
 * transitions during the traversal.
 *
 * This class expects the input IL to be "sound"; properly structured for
 * traversal.
 */
class ILTraverser {
 public:
   ...
   * ILTraverser to be notified of the various transitions done during
   * traversal.
   *
 ...
 private:
  std::vector<Observer*, TR::typed_allocator<Observer*, TR::Allocator>>
      _observers;
