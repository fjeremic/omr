...
public:
  ...
  TR::CFGNode *addNode(TR::CFGNode *n, TR_RegionStructure *parent = NULL,
                       bool isEntryInParent = false);
  ...
  void
  copyExceptionSuccessors(TR::CFGNode *from, TR::CFGNode *to,
  ...
  void removeEdge(TR::CFGEdgeList &succList, int32_t selfNumber,
                  int32_t destNumber);
  ...
  int32_t createTraversalOrder(bool forward, TR_AllocationKind allocationKind,
                               TR_BitVector *backEdges = NULL);
  ...
  void getBranchCounters(TR::Node *node, TR::Block *block, int32_t *taken,
                         int32_t *notTaken, TR::Compilation *comp);

  void normalizeFrequencies(TR_BitVector *);

  void setEdgeFrequenciesOnNode(TR::CFGNode *node, int32_t branchToCount,
                                int32_t fallThroughCount,
                                TR::Compilation *comp);
  void setUniformEdgeFrequenciesOnNode(TR::CFGNode *node, int32_t branchToCount,
                                       bool addFrequency,
                                       TR::Compilation *comp);
  ...
  //
  void getBranchCountersFromProfilingData(TR::Node *node, TR::Block *block,
...
public: // FIXME: These public members should eventually be wrtapped in an
        // interface.
...
public:
  TR_CFGIterator(TR::CFGEdgeList &regularEdges, TR::CFGEdgeList &exceptionEdges)
      : _regularEdges(regularEdges), _exceptionEdges(exceptionEdges),
  ...
  TR::CFGEdge *getCurrent() {
    return _regularEdgeIterator == _regularEdges.end()
               ? (_exceptionEdgeIterator == _exceptionEdges.end()
                      ? NULL
...
public:
  TR_PredecessorIterator(TR::CFGNode *node)
      : TR_CFGIterator(node->getPredecessors(),
                       node->getExceptionPredecessors()) {}
};
