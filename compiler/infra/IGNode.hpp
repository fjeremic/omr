...
public:
  ...
  TR_IGNode(TR_Memory *m)
      : _pEntity(NULL), _index(0), _degree(0), _workingDegree(0),
        _colour(UNCOLOURED), _adjList(m), _flags(0) {}

  TR_IGNode(void *p, TR_Memory *m)
      : _pEntity(p), _index(0), _degree(0), _workingDegree(0),
        _colour(UNCOLOURED), _adjList(m), _flags(0) {}
  ...
  IGNodeDegree incWorkingDegree() {
    TR_ASSERT(_workingDegree < MAX_DEGREE,
              "incWorkingDegree: maximum node degree exceeded!");
    return ++_workingDegree;
  }

  IGNodeDegree incWorkingDegreeBy(IGNodeDegree d) {
    TR_ASSERT((MAX_DEGREE - _workingDegree) >= d,
              "incWorkingDegreeBy: maximum node degree exceeded!");
    return (_workingDegree += d);
  }

  IGNodeDegree decWorkingDegree() {
    TR_ASSERT(_workingDegree >= 1,
              "decWorkingDegree: working degree is less than 0! "
              "(_workingDegree=%d nodeIndex=%d)",
              _workingDegree, getIndex());
    return --_workingDegree;
  }

  IGNodeDegree decWorkingDegreeBy(IGNodeDegree d) {
    TR_ASSERT(_workingDegree >= d,
              "decWorkingDegreeBy: degree is less than 0!");
    return (_workingDegree -= d);
  }
  ...
  IGNodeDegree incDegreeBy(IGNodeDegree d) {
    TR_ASSERT((MAX_DEGREE - _degree) >= d,
              "incDegreeBy: maximum node degree exceeded!");
    return (_degree += d);
  }
