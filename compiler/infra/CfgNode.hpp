...
class CFGNode : public ::TR_Link1<CFGNode> {
 ...
 public:
  ...
  // use getSuccessorEdge or getExceptionEdge if a search in a particular list
  // is required
  ...
  void setFrequency(int32_t f) {
    if (f >= SHRT_MAX) f = SHRT_MAX - 1;

    _frequency = f;
  }

  static int32_t normalizedFrequency(int32_t rawFrequency,
                                     int32_t maxFrequency);
  static int32_t denormalizedFrequency(int32_t normalizedFrequency,
                                       int32_t maxFrequency);
 ...
 private:
  ...
  CFGEdge *getEdgeMatchingNodeInAList(CFGNode *n, TR::CFGEdgeList &list,
                                      FUNC blockGetter);
