...
public:
  TR::CFGEdgeList& getSuccessors()
  {
    return _successors;
  }
  TR::CFGEdgeList& getPredecessors()
  {
    return _predecessors;
  }
  TR::CFGEdgeList& getExceptionSuccessors()
  {
    return _exceptionSuccessors;
  }
  TR::CFGEdgeList& getExceptionPredecessors()
  {
    return _exceptionPredecessors;
  }
  ...
  void addSuccessor(CFGEdge* p)
  {
    _successors.push_front(p);
  }
  void addSuccessor(CFGEdge* p, TR_AllocationKind allocKind)
  {
    _successors.push_front(p);
  }

  void addPredecessor(CFGEdge* p)
  {
    _predecessors.push_front(p);
  }
  void addPredecessor(CFGEdge* p, TR_AllocationKind allocKind)
  {
    _predecessors.push_front(p);
  }

  void addExceptionSuccessor(CFGEdge* p)
  {
    _exceptionSuccessors.push_front(p);
  }
  void addExceptionSuccessor(CFGEdge* p, TR_AllocationKind allocKind)
  {
    _exceptionSuccessors.push_front(p);
  }

  void addExceptionPredecessor(CFGEdge* p)
  {
    _exceptionPredecessors.push_front(p);
  }
  void addExceptionPredecessor(CFGEdge* p, TR_AllocationKind allocKind)
  {
    _exceptionPredecessors.push_front(p);
  }
  ...
  bool isUnreachable()
  {
    return _predecessors.empty() && _exceptionPredecessors.empty();
  }
  ...
  int32_t getForwardTraversalIndex()
  {
    return _forwardTraversalIndex;
  }
  void setForwardTraversalIndex(int32_t idx)
  {
    _forwardTraversalIndex = idx;
  }
  int32_t getBackwardTraversalIndex()
  {
    return _backwardTraversalIndex;
  }
  void setBackwardTraversalIndex(int32_t idx)
  {
    _backwardTraversalIndex = idx;
  }
...
private:
  ...
  static CFGNode* fromBlockGetter(CFGEdge* e)
  {
    return e->getFrom();
  };
  static CFGNode* toBlockGetter(CFGEdge* e)
  {
    return e->getTo();
  };
