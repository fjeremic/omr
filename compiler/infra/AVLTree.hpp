...
class TR_AVLNode {
 public:
  ...
  bool verify(T *parent) {
    ...
    if (!_left->verify((T *)this)) return false;
    if (!_right->verify((T *)this)) return false;
    return true;
  }
...
#if DEBUG
  void printTree(TR::FILE *outFile, int32_t indent, char *prefix,
                 bool isLeftChild) {
...
template <class T>
class TR_AVLTreeIterator {
 public:
  TR_ALLOC(TR_Memory::AVLTree)

  TR_AVLTreeIterator() : _tree(NULL), _next(NULL) {}
  TR_AVLTreeIterator(TR_AVLTree<T> &tree) { reset(tree); }

  TR_AVLTree<T> *getBase() { return _tree; }
  void reset(TR_AVLTree<T> &tree) {
    _tree = &tree;
 ...
 private:
  ...
  T *getNextHigher(T *node) {
    ...
    for (node = node->getParent(); node && node->getKey() <= key;
         node = node->getParent()) {
    }
    return node;
  }
...
template <class T>
class TR_AVLTreeHandler {
 public:
  TR_ALLOC(TR_Memory::AVLTree)

  T *find(int32_t key, TR_AVLTree<T> &tree) { return tree.find(key); }
  T *findOrCreate(int32_t key, TR_AVLTree<T> &tree) {
    int32_t heightChanged;
    T *result;
    if (debug("traceAVL")) diagnostic("\nCall findOrCreate with key %d\n", key);
    if (tree.getRoot()) {
  ...
  T *insert(T *newNode, TR_AVLTree<T> &tree) {
    ...
    if (tree.getRoot()) {
      ...
      _treeChanged = false;
      result =
          findOrCreate(newNode->getKey(), tree.rootReference(), heightChanged);
    } else {
      result = newNode;
  ...
  T *copyAll(TR_AVLTree<T> &tree) { return copySubtree(tree.getRoot()); }
  ...
  void empty(TR_AVLTree<T> &tree) { emptySubtree(tree.rootReference()); }
...
template <class T>
int32_t TR_AVLTreeHandler<T>::balanceLeft(T *&node) {
...
template <class T>
int32_t TR_AVLTreeHandler<T>::balanceRight(T *&node) {
...
template <class T>
T *TR_AVLTreeHandler<T>::findOrCreate(int32_t key, T *&node,
                                      int32_t &heightChanged) {
  ...
  if (key < node->getKey()) {
    ...
    if (heightChanged) {
      ...
      if (heightChanged < -1) heightChanged = balanceLeft(node);
    }
  }
    ...
    if (heightChanged) {
      heightChanged = node->balance() + 1;
      node->setBalance(heightChanged);
      if (heightChanged > 1) heightChanged = balanceRight(node);
    }
  }
...
template <class T>
T *TR_AVLTreeHandler<T>::remove(int32_t key, T *&node, int32_t &heightChanged) {
  ...
  if (key == node->getKey()) {
    ...
    _treeChanged = true;
    if (node->right() == NULL) {
      if (node->left()) node->left()->setParent(node->getParent());
      heightChanged = 1;
      node = node->left();
      return result;
    }
  ...
  if (takeLeftBranch || key < node->getKey()) {
    ...
    if (heightChanged) {
      ...
      if (heightChanged > 0) heightChanged = !balanceRight(node);
    }
  }
    ...
    if (heightChanged) {
      heightChanged = node->balance();
      node->setBalance(heightChanged - 1);
      if (heightChanged < 0) heightChanged = !balanceLeft(node);
    }
  }
...
template <class T>
void TR_AVLTreeHandler<T>::swapRightmost(T *&node, T *&swap) {
  ...
  if (node->right()) {
    ...
    node->setBalance(swap->balance());
    swap->setBalance(balance);
