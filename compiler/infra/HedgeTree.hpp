...
public:
  ...
  void setRightSibling(bool b)
  {
    _rightSibling = b;
  }
  void setLeftSibling(bool b)
  {
    _leftSibling = b;
  }
  T* getParent()
...
public:
  ...
  TR_HedgeTreeIterator()
    ...
    , _next(NULL)
  {}
  TR_HedgeTreeIterator(TR_HedgeTree<T>& tree)
  {
    reset(tree);
  }

  TR_HedgeTree<T>* getBase()
  {
    return _tree;
  }
...
public:
  ...
  T* getRoot(TR_HedgeTree<T>& tree)
  {
    return tree.getRoot();
  }
  void setRoot(TR_HedgeTree<T>& tree, T* node)
  {
    tree.setRoot(node);
  }
...
template <class T>
T* TR_HedgeTreeHandler<T>::findOrCreate(int32_t key, T*& node, int32_t& rebalance)
{
  ...
  if (takeLeftBranch || key < node->getKey()) {
    ...
    if (rebalance) {
      ...
      if (node->isLeftSibling()) {
        ...
        p1 = node->right();
        p2 = p1->left();
        if ((node->isRightSibling() && p2->isRightSibling()) || (!node->isRightSibling() && p1->isLeftSibling())) {
          // Right subtree's left subtree is lifted to a new level above
    ...
    if (rebalance) {
      ...
      if (node->isRightSibling()) {
        ...
        p1 = node->left();
        p2 = p1->right();
        if ((node->isLeftSibling() && p2->isLeftSibling()) || (!node->isLeftSibling() && p1->isRightSibling())) {
          // Left subtree's right subtree is lifted to a new level above
          // this node. The original height is then restored.
          //
          //              |                |
