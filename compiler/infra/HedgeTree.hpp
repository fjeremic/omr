...
class TR_HedgeNode {
 public:
  ...
  bool verify(T *parent) {
    if (isLeftSibling() &&
        (_left->isLeftSibling() || _left->isRightSibling())) {
      TR_ASSERT(0, "Double sibling chain for key %d", getKey());
      return false;
    }
    if (isRightSibling() &&
        (_right->isLeftSibling() || _right->isRightSibling())) {
      TR_ASSERT(0, "Double sibling chain for key %d", getKey());
      return false;
    }
    if (_parent != parent) {
...
#if DEBUG
  void printTree(TR_FrontEnd *fe, TR::FILE *outFile, int32_t indent,
                 char *prefix, bool isLeftChild, bool isSibling) {
  ...
  void printTree(TR::FILE *outFile, int32_t indent, char *prefix,
                 bool isLeftChild, bool isSibling) {}
#endif
...
template <class T>
class TR_HedgeTreeIterator {
 public:
  TR_ALLOC(TR_Memory::HedgeTree)

  TR_HedgeTreeIterator() : _tree(NULL), _next(NULL) {}
...
class TR_HedgeTreeHandler {
 public:
  ...
  T *remove(int32_t key, TR_HedgeTree<T> &tree) {
    int32_t rebalance;
    _treeChanged = false;
    if (debug("traceHedge")) diagnostic("\nCall remove with key %d\n", key);
...
template <class T>
T *TR_HedgeTreeHandler<T>::findOrCreate(int32_t key, T *&node,
                                        int32_t &rebalance) {
  ...
  if (key < node->getKey()) {
    ...
    if (rebalance) {
      if (node->isLeftSibling()) {
        ...
        //
        if (--rebalance) node->setLeftSibling(true);
      }
    }
  }
    ...
    if (rebalance) {
      if (node->isRightSibling()) {
        ...
        // to become a sibling. Otherwise rebalancing is complete.
        //
        if (--rebalance) node->setRightSibling(true);
      }
    }
  }
...
template <class T>
T *TR_HedgeTreeHandler<T>::remove(int32_t key, T *&node, int32_t &rebalance) {
...
template <class T>
void TR_HedgeTreeHandler<T>::swapRightmost(T *&node, T *&swap) {
