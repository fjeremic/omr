...
public:
  ...
  TR_SingleBitContainer()
    : _value(false)
  {}
  TR_SingleBitContainer(int64_t initBits,
                        TR_Memory* m,
               ...
               TR_MemoryBase::ObjectType ot = TR_MemoryBase::BitVector)
  {
    ...
    if (_numChunks) {
      if (_region) {
        ...
        TR_ASSERT(
          allocKind == persistentAlloc,
          "Should not allocate through trMemory except for persistent memory");
...
private:
  ...
  void sanityCheck(const char* source)
  {
    ...
    if (low != _firstChunkWithNonZero || high != _lastChunkWithNonZero) {
      ...
      printf(
        "TR_BitVector Low is %d, should be %d\n", _firstChunkWithNonZero, low);
      printf("TR_BitVector High is %d, should be %d, num chunks = %d\n",
             _lastChunkWithNonZero,
             high,
             _numChunks);
...
template<class BitVector>
inline void
Assign(BitVector& cs2bv, TR_BitVector& trbv, bool clear = true)
{
  uint32_t count = 0;
...
template<class BitVector>
inline void
Assign(BitVector& cs2bv, TR_BitContainer& trbc, bool clear = true)
{
