...
public:
  ...
  bool isSingleValue()
  {
    return (_type != bitvector);
  }
  int32_t getSingleValue()
  {
    return _singleBit;
  }
...
public:
  ...
  uint32_t ValueAt(uint32_t index) const
  {
    return bv.get(index);
  }
  uint32_t LastOne() const
  {
    return bv.getHighestBitPosition();
  }
  ...
  {
    return bv._firstChunkWithNonZero;
  }
  int32_t LastOneWordIndex() const
  {
    return bv._lastChunkWithNonZero;
  }
