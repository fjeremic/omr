...
public:
  ...
  //
  TR_BitVector()
      : _numChunks(0), _chunks(NULL), _firstChunkWithNonZero(0),
        _lastChunkWithNonZero(-1), _growable(growable), _region(0) {}
  TR_BitVector(TR::Region &region)
      : _numChunks(0), _chunks(NULL), _firstChunkWithNonZero(0),
        _lastChunkWithNonZero(-1), _growable(growable), _region(&region) {}
  ...
  TR_BitVector(const TR_BitVector &v2)
      : _numChunks(0), _firstChunkWithNonZero(0), _lastChunkWithNonZero(-1),
        _chunks(NULL), _growable(growable) {
    _region = v2._region;
    *this = v2;
...
#ifdef DEBUG
    TR_ASSERT((((bitIndex >> SHIFT) & ~((int64_t)0x7fffffff)) == 0) ||
                  (bitIndex < 0),
...
public:
  ...
  TR_BitMatrix(int32_t rows, int32_t bitsPerRow, TR_Memory *m,
               TR_AllocationKind allocKind = heapAlloc)
      : _rows(rows), _bitsPerRow(bitsPerRow),
