...
class TR_BitContainer {
 public:
  ...
  operator TR_BitVector *() {
    TR_ASSERT(_type == bitvector,
              "BitContainer cannot be converted to BitVector\n");
    return _bitVector;
  }

  TR_BitVector *getBitVector() {
    return (_type == bitvector ? _bitVector : NULL);
  }
...
class TR_SingleBitContainer {
 public:
  ...
  TR_SingleBitContainer(int64_t initBits, TR_Memory *m,
  ...
  bool intersects(TR_SingleBitContainer &other) {
    return _value && other._value;
  }
  bool operator==(TR_SingleBitContainer &other) {
    return _value == other._value;
  }
...
class TR_BitVector {
 public:
  ...
  //
  TR_BitVector(int64_t initBits, TR_Memory *m,
               TR_AllocationKind allocKind = heapAlloc,
               TR_BitVectorGrowable growableOrNot = growable,
    ...
    if (_numChunks) {
      if (_region) {
        ...
        TR_ASSERT(allocKind == persistentAlloc,
                  "Should not allocate through trMemory except for persistent "
                  "memory");
        _chunks = (chunk_t *)TR_Memory::jitPersistentAlloc(
            _numChunks * sizeof(chunk_t), TR_Memory::BitVector);
      }
  ...
  TR_BitVector(int64_t initBits, TR::Region &region,
               TR_BitVectorGrowable growableOrNot = growable,
               TR_MemoryBase::ObjectType ot = TR_MemoryBase::BitVector) {
    _chunks = NULL;
    _numChunks = getChunkIndex(initBits - 1) + 1;
  ...
  // to be empty.
  //
  void init(int64_t initBits, TR_Memory *m,
            TR_AllocationKind allocKind = heapAlloc,
            TR_BitVectorGrowable growableOrNot = notGrowable) {
    if (_chunks && _region == NULL) jitPersistentFree(_chunks);
    _region = NULL;
    switch (allocKind) {
      case heapAlloc:
        _region = &(m->heapMemoryRegion());
  ...
  void init(int64_t initBits, TR::Region &region,
            TR_BitVectorGrowable growableOrNot = notGrowable) {
    if (_chunks && _region == NULL) jitPersistentFree(_chunks);
    _region = &region;
    _growable = growable;
  ...
  void ensureBits(int64_t n) {
    ...
    if (chunkIndex >= _numChunks) setChunkSize(chunkIndex + 1);
#if BV_SANITY_CHECK
  ...
  int32_t get(int64_t n) {
    int32_t chunkIndex = getChunkIndex(n);
    if (chunkIndex > _lastChunkWithNonZero) return 0;
  ...
  void set(int64_t n) {
    ...
    if (chunkIndex > _lastChunkWithNonZero) _lastChunkWithNonZero = chunkIndex;
  ...
  int64_t getHighestBitPosition() {
    if (_lastChunkWithNonZero < 0) return 0;
  ...
  void reset(int64_t n, bool updateLowHigh = true) {
    ...
    if (chunkIndex > _lastChunkWithNonZero ||
        chunkIndex < _firstChunkWithNonZero)
      return;
  ...
  void operator=(const TR_BitVector &v2) {
    ...
    if (high < 0) {
      ...
      for (i = _firstChunkWithNonZero; i < low; i++) _chunks[i] = 0;
      for (i = low; i <= high; i++) _chunks[i] = v2._chunks[i];
      for (i = high + 1; i <= _lastChunkWithNonZero; i++) _chunks[i] = 0;
  ...
  void operator|=(TR_BitVector &v2) {
    if (v2._lastChunkWithNonZero < 0) return;  // other is empty

    // Grow the this vector if smaller than the 2nd vector
    int32_t v2Used = v2._numChunks;
    if (_numChunks < v2Used) setChunkSize(v2Used);

    // OR in all of the words from the 2nd vector
    for (int32_t i = v2._firstChunkWithNonZero; i <= v2._lastChunkWithNonZero;
         i++)
  ...
  //
  void operator&=(TR_BitVector &v2) {
    if (_lastChunkWithNonZero < 0) return;  // Already empty
    int32_t low = v2._firstChunkWithNonZero;
    int32_t high = v2._lastChunkWithNonZero;
    ...
    if (high > _lastChunkWithNonZero)
      ...
      for (i = _lastChunkWithNonZero; i > high; i--) _chunks[i] = 0;
    }
  ...
  //
  bool intersects(TR_BitVector &v2) {
    if (_lastChunkWithNonZero < 0) return false;  // empty - no intersection
    int32_t low = v2._firstChunkWithNonZero;
    int32_t high = v2._lastChunkWithNonZero;
    if (high < _firstChunkWithNonZero || low > _lastChunkWithNonZero)
    ...
    // AND in all of the words from the 2nd vector
    if (low < _firstChunkWithNonZero) low = _firstChunkWithNonZero;
    if (high > _lastChunkWithNonZero) high = _lastChunkWithNonZero;
    for (int32_t i = low; i <= high; i++)
      if (_chunks[i] & v2._chunks[i]) return true;
    return false;
  }

  // Perform a bitwise negation (AND-NOT) between this vector and a second
  // vector Results are in the this vector
  //
  void operator-=(TR_BitVector &v2) {
    if (_lastChunkWithNonZero < 0) return;
    int32_t low = v2._firstChunkWithNonZero;
    int32_t high = v2._lastChunkWithNonZero;
    if (high < _firstChunkWithNonZero || low > _lastChunkWithNonZero)
    ...
    if (low < _firstChunkWithNonZero) low = _firstChunkWithNonZero;
    if (high > _lastChunkWithNonZero) high = _lastChunkWithNonZero;
    for (int32_t i = low; i <= high; i++) _chunks[i] &= ~v2._chunks[i];
  ...
  //
  bool operator==(TR_BitVector &v2) {
    if (_lastChunkWithNonZero != v2._lastChunkWithNonZero) return false;
    if (_lastChunkWithNonZero < 0) return true;  // Both empty
    // Now both are non-empty
    if (_firstChunkWithNonZero != v2._firstChunkWithNonZero) return false;
    if (_lastChunkWithNonZero != v2._lastChunkWithNonZero) return false;
    for (int32_t i = _firstChunkWithNonZero; i <= _lastChunkWithNonZero; i++)
      if (_chunks[i] != v2._chunks[i]) return false;
    return true;
  }
  ...
  void setAll(int64_t n) {
    ...
    int32_t chunkIndex = getChunkIndex(n - 1);
    if (chunkIndex >= _numChunks) setChunkSize(chunkIndex + 1);
    for (i = chunkIndex - 1; i >= 0; i--) _chunks[i] = (chunk_t)-1;
    for (i = getBitIndex(chunkIndex); i < n; i++)
      _chunks[chunkIndex] |= getBitMask(i);
    _firstChunkWithNonZero = 0;
    if (_lastChunkWithNonZero < chunkIndex) _lastChunkWithNonZero = chunkIndex;
#if BV_SANITY_CHECK
  ...
  void setAll(int64_t m, int64_t n) {
    ...
    if (_lastChunkWithNonZero < lastChunk) _lastChunkWithNonZero = lastChunk;
    ...
    if (firstChunk == lastChunk) {
      ...
      if (i > 0) {
        for (; i < BITS_IN_CHUNK; i++) _chunks[firstChunk] |= getBitMask(i);
      } else {
      ...
      if (i < (BITS_IN_CHUNK - 1)) {
        for (; i >= 0; i--) _chunks[lastChunk] |= getBitMask(i);
      } else {
      ...
      //
      for (i = firstChunk + 1; i < lastChunk; i++) _chunks[i] = (chunk_t)-1;
    }
#if BV_SANITY_CHECK
    sanityCheck("setAll(m,n)");
#endif
  ...
  void resetAll(int64_t m, int64_t n) {
    ...
    int32_t firstChunk = getChunkIndex(m);
    int32_t lastChunk = getChunkIndex(n);
    if (_lastChunkWithNonZero < firstChunk ||
        _firstChunkWithNonZero > lastChunk)
    ...
    if (firstChunk == lastChunk) {
      ...
      // Set the complete chunks
      //
      for (i = firstChunk + 1; i < lastChunk; i++) _chunks[i] = 0;
    }
    // Reset first and last chunks with non-zero
    resetLowAndHighChunks(_firstChunkWithNonZero, _lastChunkWithNonZero);
#if BV_SANITY_CHECK
 ...
 private:
...
#endif
  int32_t _numChunks;
  // _firstChunkWithNonZero and _lastChunkWithNonero must be maintained
  // precisely
  ...
  void sanityCheck(const char *source) {
    // Calculate the correct values for _firstChunkWithNonZero and
    // _lastChunkWithNonZero and check them against the stored values
    int32_t low, high;
    int32_t i;
    if (!_chunks) {
      low = 0;
...
#ifdef DEBUG
    TR_ASSERT(
        (((bitIndex >> SHIFT) & ~((int64_t)0x7fffffff)) == 0) || (bitIndex < 0),
  ...
  //
  static int32_t getIndexInChunk(int64_t bitIndex) {
    return bitIndex & (BITS_IN_CHUNK - 1);
  }
  ...
  static inline chunk_t getBitMask(int32_t bitIndex) {
    return (chunk_t)1 << ((BITS_IN_CHUNK - 1) -
                          (bitIndex & (BITS_IN_CHUNK - 1)));
  }

  // Given a bit index range, calculate the bit mask within chunk for that range
  // of bits from m to n
  //
  static inline chunk_t getBitMask(int32_t m, int32_t n) {
    return ((getBitMask(m) - 1) << 1) - getBitMask(n) + 2;
  }
  ...
  //
  static inline chunk_t getBitMask(int32_t bitIndex) {
    return (chunk_t)1 << (bitIndex & (BITS_IN_CHUNK - 1));
  }
  ...
  //
  static inline chunk_t getBitMask(int32_t m, int32_t n) {
    return ((getBitMask(n) - 1) << 1) - getBitMask(m) + 2;
  }
  ...
  //
  static int64_t getBitIndex(int32_t chunkIndex) {
    return ((int64_t)chunkIndex) << SHIFT;
  }
 ...
 protected:
  void getNextBit() {
    ...
    if (!tmpChunk) {
      ...
      // curChunk is less than the last chunk with non-zero bits, so the do loop
      // will terminate
      do {
        curChunk++;
...
class TR_BitMatrix {
 public:
  ...
  TR_BitMatrix(int32_t rows, int32_t bitsPerRow, TR_Memory *m,
               TR_AllocationKind allocKind = heapAlloc)
      : _rows(rows),
...
inline TR_BitVector &TR_BitVector::operator=(const BitVector &sparse) {
  ...
  empty();
  for (bi.SetToFirstOne(); bi.Valid(); bi.SetToNextOne()) set(bi);
#if BV_SANITY_CHECK
...
class CS2_TR_BitVector {
 public:
  ...
  chunk_t WordAt(uint32_t wordIndex) const {
    if (wordIndex >= bv._numChunks) return 0;
  ...
  struct Cursor {
    ...
    bool SetToNextOneAfter(uint32_t v) {
      while (Valid() && (*this) <= v) SetToNextOne();
      return Valid();
    }
...
inline void Assign(BitVector &cs2bv, TR_BitContainer &trbc, bool clear = true) {
  ...
  if (trbv == NULL) {
    ...
    // so its a bit vector, for now just copies the elements of the TR_BitVector
    // into cs2bv
    // cs2bv = *trbv;
...
// Perform inplace bitwise negation (AND-NOT) between this vector and a second
// vector.
//
template <class BitVector>
inline TR_BitVector &TR_BitVector::operator-=(const BitVector &sparse) {
...
inline TR_BitVector &TR_BitVector::operator&=(const BitVector &sparse) {
  ...
  for (chunk = low; chunk <= high; chunk++) {
    while (bi.Valid() && chunk > getChunkIndex(bi)) bi.SetToNextOne();
    if (!bi.Valid()) break;
  ...
  for (; chunk < high; chunk++) _chunks[chunk] = 0;
