...
//
template <class T>
class TR_Stack : public TR_Array<T> {
public:
  ...
  int32_t topIndex()
  {
    return TR_Array<T>::lastIndex();
  }
  T& top()
  {
    return TR_Array<T>::element(topIndex());
  }
  void push(T t)
  {
    TR_Array<T>::add(t);
  ...
  bool isEmpty()
  {
    return TR_Array<T>::size() == 0;
  }
  void clear()
  {
    TR_Array<T>::setSize(0);
  }

  // FIFO behaviour
  T& bottom()
  {
    return TR_Array<T>::element(0);
  }
  T drop()
  {
    T t = bottom();
