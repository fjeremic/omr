...
{
    if (node->getOpCodeValue() != TR::iload && node->getOpCodeValue() != TR::lload && node->getOpCodeValue() != TR::iloadi && node->getOpCodeValue() != TR::lloadi) {
        return false;
    } else {
        return true;
    }
...
bool TR_AddressTree::process(TR::Node* aiaddNode, bool onlyConsiderConstAiaddSecondChild)
{
    ...
    if (opCodeAiaddFirstChild == TR::aload || opCodeAiaddFirstChild == TR::aloadi) {
        ...
        if ((opCodeAiaddSecondChild == TR::isub || opCodeAiaddSecondChild == TR::lsub || opCodeAiaddSecondChild == TR::iadd || opCodeAiaddSecondChild == TR::ladd) && !onlyConsiderConstAiaddSecondChild) {
                ...
                if (opCodeIsubFirstChild == TR::imul || opCodeIsubFirstChild == TR::lmul) {
                    ...
                    if (indSymFromSubTree->getOpCodeValue() == TR::iload || indSymFromSubTree->getOpCodeValue() == TR::lload) {
                        ...
                        if (
                            ...
                            && (indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::iload || indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::lload || indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::iconst || indSymFromSubTree->getSecondChild()->getOpCodeValue() == TR::lconst)) {
                            validAiaddSubTree = true;
        ...
        } else if ((opCodeAiaddSecondChild == TR::imul || opCodeAiaddSecondChild == TR::lmul) && !onlyConsiderConstAiaddSecondChild) {
            validAiaddSubTree = true;
