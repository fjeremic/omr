...
bool TR_AddressTree::findComplexAddressGenerationTree(TR::Node *node,
                                                      vcount_t visitCount,
                                                      TR::Node *parent) {
  ...
  if (isILLoad(node)) {
    ...
    for (childNumber = 0; childNumber < parent->getNumChildren(); childNumber++)
      if (parent->getChild(childNumber) == node) break;
...
bool TR_AddressTree::processBaseAndIndex(TR::Node *parent) {
  ...
  if (isValid)
    ...
    return findComplexAddressGenerationTree(parent, comp()->incVisitCount(),
                                            parent);
  }
}
...
bool TR_AddressTree::processMultiplyNode(TR::Node *multiplyNode) {
  ...
  if (opCodeSecondMulChild != TR::iconst &&
      opCodeSecondMulChild != TR::lconst) {
    dumpOptDetails(comp(),
                   "AddressTree: second node of multiply is not iconst\n");
    return false;
  }
  ...
  if (opCodeFirstMulChild == TR::iadd || opCodeFirstMulChild == TR::ladd) {
    if (!processBaseAndIndex(firstMulChild)) {
      dumpOptDetails(comp(),
                     "AddressTree: first node of multiply is iadd/ladd but "
                     "children are not ok\n");
      return false;
    }
    ...
    dumpOptDetails(
        comp(),
        "AddressTree: first node of multiply is not iadd/ladd/iload/lload\n");
    return false;
  }
...
//
// process looks for an address generation sub-tree. The tree typically is
// headed up with an aiadd but can also be an iadd or ladd tree in the case of
// unsafe get/put being used.
//
...
//    ilload <DirectByteBuffer>
//       aload <Buffer>
bool TR_AddressTree::process(TR::Node *aiaddNode,
                             bool onlyConsiderConstAiaddSecondChild) {
  ...
  if (opCodeaiAdd != TR::aiadd && opCodeaiAdd != TR::aladd) {
    dumpOptDetails(comp(),
                   "AddressTree: Can not construct an address tree without an "
                   "address node\n");
    return false;
  }

  // Use 'UnconvertedChild' instead of 'Child' below to skip over potential
  // l2i's that can be strewn about for 64-bit sign extension work.
  ...
  if (opCodeAiaddFirstChild == TR::aload ||
      opCodeAiaddFirstChild == TR::aloadi) {
    _baseVarNode.setParentAndChildNumber(aiaddNode, 0);
    if ((opCodeAiaddSecondChild == TR::isub ||
         opCodeAiaddSecondChild == TR::lsub ||
         opCodeAiaddSecondChild == TR::iadd ||
         opCodeAiaddSecondChild == TR::ladd) &&
        !onlyConsiderConstAiaddSecondChild) {
      TR::Node *isubFirstChild =
          aiaddSecondChild->getFirstChild()->skipConversions();
      TR::ILOpCodes opCodeIsubFirstChild = isubFirstChild->getOpCodeValue();
      TR::Node *isubSecondChild =
          aiaddSecondChild->getSecondChild()->skipConversions();
      TR::ILOpCodes opCodeIsubSecondChild = isubSecondChild->getOpCodeValue();

      // (64-bit, for 32-bit, change the l to i in the opcodes below and remove
      // the i2l... aladd
      //   aload
      //   lsub
      //     lmul
      ...
      if (opCodeIsubSecondChild != TR::iconst &&
          opCodeIsubSecondChild != TR::lconst) {
        dumpOptDetails(comp(),
                       "AddressTree: i(l)sub second child is not constant\n");
      } else {
        if (opCodeIsubFirstChild == TR::imul ||
            opCodeIsubFirstChild == TR::lmul) {
          multiplySubTree = isubFirstChild;
          TR::Node *indSymFromSubTree =
              isubFirstChild->getFirstChild()->skipConversions();
          ...
          if (indSymFromSubTree->getOpCodeValue() == TR::iload ||
            ...
            // the following check is safe because there should only be one
            // induction variable active for loops we consider, but it does
            // however require that the induction variable be the first child,
            // so we will catch a case of:
            //   a[indVar + j]
            // but not:
            //   a[j + indVar]
            if ((indSymFromSubTree->getFirstChild()->getOpCodeValue() ==
                     TR::iload ||
                 indSymFromSubTree->getFirstChild()->getOpCodeValue() ==
                     TR::lload) &&
                (indSymFromSubTree->getSecondChild()->getOpCodeValue() ==
                     TR::iload ||
                 indSymFromSubTree->getSecondChild()->getOpCodeValue() ==
                     TR::lload ||
                 indSymFromSubTree->getSecondChild()->getOpCodeValue() ==
              ...
              _indexBaseNode.setParentAndChildNumber(
                  indSymFromSubTree->getFirstChild(), 0);
            }
          }
          ...
          validAiaddSubTree = true;
        } else if (opCodeIsubFirstChild == TR::iadd ||
                   opCodeIsubFirstChild == TR::ladd) {
          _multiplyNode.setParentAndChildNumber(aiaddSecondChild, 0);
          validAiaddSubTree = processBaseAndIndex(isubFirstChild);
        } else {
          validAiaddSubTree = findComplexAddressGenerationTree(
              isubFirstChild, comp()->incVisitCount(), aiaddSecondChild);
          if (validAiaddSubTree)
            _multiplyNode.setParentAndChildNumber(aiaddSecondChild, 0);
          else
            dumpOptDetails(
                comp(),
                "AddressTree: i(l)sub children are not i(l)mul or i(l)const\n");
        }
    ...
    } else if (opCodeAiaddSecondChild == TR::iconst ||
               opCodeAiaddSecondChild == TR::lconst) {
      validAiaddSubTree = true;
    ...
    } else if ((opCodeAiaddSecondChild == TR::imul ||
                opCodeAiaddSecondChild == TR::lmul) &&
               !onlyConsiderConstAiaddSecondChild) {
      validAiaddSubTree = true;
      ...
      dumpOptDetails(comp(),
                     "AddressTree: second child of aiadd/aladd is not "
                     "iload/i(l)sub/i(l)mul\n");
    }
  } else {
    dumpOptDetails(comp(),
                   "AddressTree: first child of aiadd/aladd is not aiload\n");
  }
...
void TR_OpCodePattern::tracePattern(TR::Node *node) {
  traceMsg(TR::comp(), "{ Trying %s [%s] pattern on %s n%dn\n", getName(),
           TR::ILOpCode(_opCode).getName(), node->getOpCode().getName(),
           node->getGlobalIndex());
}

bool TR_Pattern::matches(TR::Node *node, TR_Unification &uni,
                         TR::Compilation *comp) {
  ...
  // guard their pattern builder code with a test of
  // TR_DisableTreePatternMatching. However, just in case they forgot, the
  // following test at least ensures that the patterns have no effect on
  // generated code.
  //
  ...
  if (comp->getOption(TR_TraceTreePatternMatching)) tracePattern(node);
...
bool TR_UnifyPattern::thisMatches(TR::Node *node, TR_Unification &uni,
                                  TR::Compilation *comp) {
  if (comp->getOption(TR_TraceTreePatternMatching)) {
    traceMsg(comp, "Unify %d with %s in state ", _index,
...
bool TR_CommutativePattern::thisMatches(TR::Node *node, TR_Unification &uni,
                                        TR::Compilation *comp) {
  ...
  if (_leftPattern->matches(node->getFirstChild(), uni, comp) &&
    ...
    // If the left one matched and the right one failed, then we need to undo
    // the unifications from the left one
    uni.undoTo(mark);
