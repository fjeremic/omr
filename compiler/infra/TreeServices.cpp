...
bool
TR_AddressTree::findComplexAddressGenerationTree(TR::Node* node,
                                                 vcount_t visitCount,
                                                 TR::Node* parent)
{
...
TR_AddressTree::processBaseAndIndex(TR::Node* parent)
{
  ...
  if (isValid)
    ...
    return findComplexAddressGenerationTree(
      parent, comp()->incVisitCount(), parent);
  }
}
...
//       aload <Buffer>
bool
TR_AddressTree::process(TR::Node* aiaddNode,
...
void
TR_Pattern::tracePattern(TR::Node* node)
{
  traceMsg(TR::comp(),
           "{ Trying %s pattern on %s n%dn\n",
           getName(),
...
void
TR_OpCodePattern::tracePattern(TR::Node* node)
{
  traceMsg(TR::comp(),
           "{ Trying %s [%s] pattern on %s n%dn\n",
           getName(),
           TR::ILOpCode(_opCode).getName(),
           node->getOpCode().getName(),
           node->getGlobalIndex());
}

bool
TR_Pattern::matches(TR::Node* node, TR_Unification& uni, TR::Compilation* comp)
{
...
bool
TR_UnifyPattern::thisMatches(TR::Node* node,
                             TR_Unification& uni,
                             TR::Compilation* comp)
{
  if (comp->getOption(TR_TraceTreePatternMatching)) {
    traceMsg(comp,
             "Unify %d with %s in state ",
             _index,
...
bool
TR_CommutativePattern::thisMatches(TR::Node* node,
                                   TR_Unification& uni,
                                   TR::Compilation* comp)
{
