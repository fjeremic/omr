...
public:
  ...
  bool testAny(T mask) const
  {
    return (_flags & mask) != 0;
  }
  bool testAll(T mask) const
  {
  ...
  void set(T mask, bool b)
  {
    b ? set(mask) : reset(mask);
  }
  void toggle(T mask)
  {
    _flags ^= mask;
  ...
  //
  bool testValue(T mask, T value) const
  {
    return (_flags & mask) == value;
  }
  void setValue(T mask, T value)
  {
    _flags = (_flags & ~mask) | value;
  }
  T getValue(T mask) const
  {
    return _flags & mask;
  }
  T getValue() const
  ...
  bool operator==(FlagsTemplate<T>& other)
  {
