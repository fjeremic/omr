...
TR::TreeTopIteratorImpl::TreeTopIteratorImpl(TR::TreeTop* start,
                                             TR::Compilation* comp,
                                             const char* name)
  : _current(start)
  , _comp(comp)
...
TR::Node*
TR::TreeTopIteratorImpl::currentNode()
{
  return currentTree()->getNode();
}

bool
TR::TreeTopIteratorImpl::isAt(PreorderNodeIterator& other)
{
  return other.isAt(currentTree());
}

void
TR::TreeTopIteratorImpl::logCurrentLocation()
{
...
void
TR::TreeTopIteratorImpl::stepForward()
{
  TR_ASSERT(_current,
            "Cannot stepForward a TreeTopIterator that has already moved "
            "beyond the first or last tree of the method");
  _current = _current->getNextTreeTop();
}

void
TR::TreeTopIteratorImpl::stepBackward()
{
  TR_ASSERT(_current,
            "Cannot stepBackward a TreeTopIterator that has already moved "
            "beyond the first or last tree of the method");
...
TR::NodeIterator::NodeIterator(TR::TreeTop* start,
                               TR::Compilation* comp,
                               const char* name)
...
bool
TR::NodeIterator::isAt(PreorderNodeIterator& other)
{
...
void
TR::NodeIterator::logCurrentLocation()
{
...
TR::PreorderNodeIterator::PreorderNodeIterator(TR::TreeTop* start,
                                               TR::Compilation* comp,
                                               const char* name)
...
bool
TR::PreorderNodeIterator::alreadyBeenPushed(TR::Node* node)
{
  return _checklist.contains(node);
}

void
TR::PreorderNodeIterator::push(TR::Node* node)
{
  TR_ASSERT(!alreadyBeenPushed(node),
            "Cannot push node n%dn that was already pushed",
            node->getGlobalIndex());
...
void
TR::PreorderNodeIterator::stepForward()
{
  // Move down into the next un-visited child
  //
  for (int32_t i = _stack.top()._child;
       i < _stack.top()._node->getNumChildren();
       i++) {
...
TR::PostorderNodeIterator::PostorderNodeIterator(TR::TreeTop* start,
                                                 TR::Compilation* comp,
                                                 const char* name)
  : NodeIterator(start, comp, name)
{
...
bool
TR::PostorderNodeIterator::alreadyBeenPushed(TR::Node* node)
{
  return _checklist.contains(node);
}

void
TR::PostorderNodeIterator::push(TR::Node* node)
{
  TR_ASSERT(!alreadyBeenPushed(node),
            "Cannot push node n%dn that was already pushed",
            node->getGlobalIndex());
...
void
TR::PostorderNodeIterator::descend()
{
...
void
TR::PostorderNodeIterator::stepForward()
{
...
TR::Node*
TR::NodeOccurrenceIterator::currentNode()
{
  if (stackDepth() == 0)
    return currentTree()->getNode();
  else
...
void
TR::NodeOccurrenceIterator::logCurrentLocation()
{
  if (_name && comp() && comp()->getOption(TR_TraceILWalks)) {
    if (currentTree()) {
      TR::Node* node = currentNode();
...
bool
TR::PreorderNodeOccurrenceIterator::alreadyPushedChildren(TR::Node* node)
{
  return _checklist.contains(node);
}

void
TR::PreorderNodeOccurrenceIterator::push(TR::Node* node)
{
  _stack.push(WalkState(node));
  _stack.top()._isBetweenChildren = (node->getNumChildren() >= 2);
  _checklist.add(node);
...
void
TR::PreorderNodeOccurrenceIterator::stepForward()
{
  if (currentNode()->getNumChildren() >= 1 &&
      !alreadyPushedChildren(currentNode())) {
...
bool
TR::PostorderNodeOccurrenceIterator::alreadyPushedChildren(TR::Node* node)
{
  return _checklist.contains(node);
}

void
TR::PostorderNodeOccurrenceIterator::pushLeftmost(TR::Node* nodeArg)
{
  for (TR::Node* node = nodeArg;
       node->getNumChildren() >= 1 && !alreadyPushedChildren(node);
...
void
TR::PostorderNodeOccurrenceIterator::stepForward()
{
  if (_stack.isEmpty()) {
    // Nullary treetop node; move on to the next treetop
    //
...
bool
TR::BlockIterator::isLoggingEnabled()
{
  return (_name && _comp && _comp->getOption(TR_TraceILWalks));
}

TR::ReversePostorderSnapshotBlockIterator::
  ReversePostorderSnapshotBlockIterator(TR::Block* start,
                                        TR::Compilation* comp,
                                        const char* name)
...
TR::ReversePostorderSnapshotBlockIterator::
  ReversePostorderSnapshotBlockIterator(TR::CFG* cfg,
                                        TR::Compilation* comp,
                                        const char* name)
  : BlockIterator(comp, name)
...
void
TR::ReversePostorderSnapshotBlockIterator::takeSnapshot(TR::Block* start)
{
  // TODO: This should use an iterative algorithm with a worklist, but the
  // recursive algo is so much easier to write...
...
void
TR::ReversePostorderSnapshotBlockIterator::visit(
  TR::Block* block,
  TR::BlockChecklist& alreadyVisited)
{
  ...
  if (block->getEntry()) // Don't add the special blocks with no trees to the
                         // snapshot
    _postorder.add(block);
}

TR::Block*
TR::ReversePostorderSnapshotBlockIterator::currentBlock()
{
...
void
TR::ReversePostorderSnapshotBlockIterator::stepForward()
{
  TR_ASSERT(currentBlock(),
            "cannot stepForward a ReversePostorderSnapshotBlockIterator that "
            "has already finished its walk");
  do
    --_currentIndex;
...
void
TR::ReversePostorderSnapshotBlockIterator::stepBackward()
{
  TR_ASSERT(currentBlock(),
            "cannot stepForward a ReversePostorderSnapshotBlockIterator that "
            "has already finished its walk");
...
bool
TR::ReversePostorderSnapshotBlockIterator::isStepOperationFinished()
{
  if (!currentBlock())
  ...
  if (isLoggingEnabled())
    traceMsg(comp(),
             "BLOCK  %s Skip block_%d removed during walk\n",
             _name,
...
void
TR::ReversePostorderSnapshotBlockIterator::logCurrentLocation()
{
  if (isLoggingEnabled()) {
    if (currentBlock())
      traceMsg(
        comp(), "BLOCK  %s @ block_%d\n", _name, currentBlock()->getNumber());
    else
      traceMsg(comp(), "BLOCK  %s finished\n", _name);
  }
}

TR::AllBlockIterator::AllBlockIterator(TR::CFG* cfg,
                                       TR::Compilation* comp,
                                       const char* name)
  : BlockIterator(comp, name)
  , _cfg(cfg)
...
TR::Block*
TR::AllBlockIterator::currentBlock()
{
  return _currentBlock;
}

void
TR::AllBlockIterator::stepForward()
{
  TR_ASSERT(_currentBlock,
            "Cannot stepForward an AllBlockIterator that has already finished "
            "iterating");
  ...
  // Note: it's crucially important that we use
  // _currentBlock->getNext()->asBlock() and definitely NOT
  // _currentBlock->getNextBlock().  The latter returns blocks in treetop order,
  // which will not provide the "all blocks" guarantee if new blocks are
  // inserted physically before the current one.

  // Move forward, stopping when we hit the end of the list, or a block we've
  // already seen
  //
  TR::CFGNode* next = _nextBlock;
  if (next == NULL || _alreadyVisited.contains(next->asBlock())) {
    // Start another lap of the list to catch newly added blocks we need to
    // visit
    //
    next = _cfg->getFirstNode();
    TR_ASSERT(next != NULL,
              "AllBlockIterator not expecting a CFG with no blocks at all");
  ...
  if (REMOVED_BLOCKS_CAN_BE_REINSERTED && !next) {
    ...
    for (next = _cfg->getFirstNode(); next; next = next->getNext()) {
      if (!_alreadyVisited.contains(next->asBlock())) {
        ...
        if (isLoggingEnabled())
          traceMsg(comp(),
                   "BLOCK  %s REMOVED_BLOCKS_CAN_BE_REINSERTED: block_%d found "
                   "via extra scan\n",
                   _name,
  ...
  if (next) {
    ...
    _nextBlock =
      _currentBlock->getNext() ? _currentBlock->getNext()->asBlock() : NULL;
    _alreadyVisited.add(_currentBlock);
...
void
TR::AllBlockIterator::logCurrentLocation()
{
  if (isLoggingEnabled()) {
    if (currentBlock())
      traceMsg(
...
TR::TreeTopOrderExtendedBlockIterator::TreeTopOrderExtendedBlockIterator(
  TR::Compilation* comp,
  const char* name)
  : BlockIterator(comp, name)
...
TR::Block*
TR::TreeTopOrderExtendedBlockIterator::getFirst()
{
  return _currBlock;
}

TR::Block*
TR::TreeTopOrderExtendedBlockIterator::getLast()
{
  if (_nextBlock != NULL) {
...
void
TR::TreeTopOrderExtendedBlockIterator::operator++()
{
  TR_ASSERT(_currBlock != NULL,
            "Cannot increment an TreeTopOrderExtendedBlockIterator that has "
...
void
TR::TreeTopOrderExtendedBlockIterator::logCurrentLocation()
{
  if (isLoggingEnabled()) {
    if (getFirst() != NULL) {
