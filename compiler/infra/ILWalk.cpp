...
bool TR::NodeIterator::isAt(PreorderNodeIterator &other) {
  ...
  //
  if (currentTree() != other.currentTree()) return false;
  if (stackDepth() != other.stackDepth()) return false;
...
void TR::PreorderNodeIterator::stepForward() {
  ...
  for (int32_t i = _stack.top()._child;
       i < _stack.top()._node->getNumChildren(); i++) {
...
void TR::PostorderNodeIterator::descend() {
  ...
  for (;;) {
    ...
    _stack.top()._child = i;
    if (i == node->getNumChildren()) break;
...
bool TR::PostorderNodeOccurrenceIterator::alreadyPushedChildren(
    TR::Node *node) {
  return _checklist.contains(node);
}
...
void TR::PostorderNodeOccurrenceIterator::stepForward() {
  if (_stack.isEmpty()) {
    ...
    //
    TreeTopIteratorImpl::stepForward();
    if (currentTree()) pushLeftmost(currentTree()->getNode());
  } else if (++_stack.top()._child < _stack.top()._node->getNumChildren()) {
    // Our attempt to move to the next sibling was successful
    //
    _stack.top()._isBetweenChildren = true;
...
TR::ReversePostorderSnapshotBlockIterator::
    ReversePostorderSnapshotBlockIterator(TR::CFG *cfg, TR::Compilation *comp,
                                          const char *name)
    : BlockIterator(comp, name),
...
void TR::ReversePostorderSnapshotBlockIterator::visit(
    TR::Block *block, TR::BlockChecklist &alreadyVisited) {
  if (alreadyVisited.contains(block)) return;
...
TR::AllBlockIterator::AllBlockIterator(TR::CFG *cfg, TR::Compilation *comp,
                                       const char *name)
    : BlockIterator(comp, name),
      _cfg(cfg),
