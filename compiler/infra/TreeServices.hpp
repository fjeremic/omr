...
class TR_Pattern {
 public:  // TODO: should be protected.  Not sure why that doesn't work.
  ...
  virtual bool thisMatches(TR::Node* node,
                           TR_Unification& uni,
...
class TR_ChoicePattern : public TR_Pattern {
 ...
 public:
  TR_ChoicePattern(TR_Pattern* firstChoice,
                   TR_Pattern* secondChoice,
                   TR_Pattern* next)
      : TR_Pattern(next),
...
class TR_ChildPattern : public TR_Pattern {
 ...
 public:
  TR_ChildPattern(int32_t childIndex,
                  TR_Pattern* childPattern,
...
class TR_ChildrenPattern : public TR_Pattern {
 ...
 public:
  TR_ChildrenPattern(TR_Pattern* leftPattern,
...
class TR_CommutativePattern : public TR_ChildrenPattern {
 ...
 public:
  TR_CommutativePattern(TR_Pattern* leftPattern,
                        TR_Pattern* rightPattern,
                        TR_Pattern* next = NULL)
...
class TR_PatternBuilder {
 ...
 public:
  ...
  TR_ChoicePattern* choice(TR_Pattern* firstChoice,
                           TR_Pattern* secondChoice,
                           TR_Pattern* next) {
  ...
  TR_ChildPattern* child(int32_t childIndex,
                         TR_Pattern* childPattern,
                         TR_Pattern* next = NULL) {
    return new (comp()->trPersistentMemory())
  ...
  // semantics
  TR_Pattern* unify(TR::ILOpCodes op,
                    TR_Unification::TR_Index index,
                    TR_Pattern* next = NULL) {
  ...
  TR_Pattern* child(TR::ILOpCodes op,
                    int32_t childIndex,
                    TR_Pattern* childPattern,
                    TR_Pattern* next = NULL) {
    return opCode(op, child(childIndex, childPattern, next));
  }
  TR_Pattern* children(TR::ILOpCodes op,
                       TR_Pattern* leftPattern,
                       TR_Pattern* rightPattern,
                       TR_Pattern* next = NULL) {
    return opCode(op, children(leftPattern, rightPattern));
  }
  TR_Pattern* commutative(TR::ILOpCodes op,
                          TR_Pattern* leftPattern,
                          TR_Pattern* rightPattern,
                          TR_Pattern* next = NULL) {
  ...
  TR_Pattern* commutativeOrNot(TR::ILOpCodes op,
                               TR_Pattern* leftPattern,
                               TR_Pattern* rightPattern,
                               TR_Pattern* next = NULL) {
  ...
  TR_Pattern* operator()(TR::ILOpCodes op,
                         void* structure,
                         TR::Node*& element) {
