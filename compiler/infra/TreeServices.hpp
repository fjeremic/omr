...
public:
  ...
  bool process(TR::Node *aiaddNode,
               bool onlyConsiderConstAiaddSecondChild = false);
...
protected:
  ...
  virtual bool findComplexAddressGenerationTree(TR::Node *node,
                                                vcount_t visitCount,
                                                TR::Node *parent);
...
public:
  ...
  // (For efficiency, should be chosen to make this struct a multiple of word
  // size)
  //
...
protected:
  TR::Node **
      _unifiedNodes; // Array of pointers.  Entries not yet unified must be NULL

  // Undo stack
  //
  TR_Mark _undoCount;                     // How many unifications so far
  TR_Index _undoStack[UNIFICATION_LIMIT]; // the first _undoCount entries in
                                          // this are indices into _unifiedNodes
                                          // that have been unified so far

public:
  TR_Unification(TR::Node **unifiedNodes)
      : _unifiedNodes(unifiedNodes), _undoCount(0) {
  ...
  void add(TR_Index unifiedNodeIndex, TR::Node *node) {
    TR_ASSERT(_unifiedNodes[unifiedNodeIndex] == NULL,
              "Can't add a unification at index %d; node is already non-null",
              unifiedNodeIndex);
  ...
  void dump(TR::Compilation *comp) {
    ...
    for (TR_Mark i = 0; i < _undoCount; i++) {
      traceMsg(comp, "%s%d:%s", sep, _undoStack[i],
               comp->getDebug()->getName(_unifiedNodes[_undoStack[i]]));
      sep = " ";
    }
    traceMsg(comp, "}");
  }
...
public: // TODO: should be protected.  Not sure why that doesn't work.
  TR_Pattern *_next; // Every pattern is assumed to be part of a conjunction
                     // because that's so common
  virtual const char *getName() = 0;
  virtual bool thisMatches(TR::Node *node, TR_Unification &uni,
...
public:
  TR_OpCodePattern(TR::ILOpCodes opCode, TR_Pattern *next = NULL)
      : TR_Pattern(next), _opCode(opCode) {}
};
...
protected:
  ...
  virtual bool thisMatches(TR::Node *node, TR_Unification &uni,
                           TR::Compilation *comp) {
    return _firstChoice->matches(node, uni, comp) ||
           _secondChoice->matches(node, uni, comp);
  }

public:
  TR_ChoicePattern(TR_Pattern *firstChoice, TR_Pattern *secondChoice,
                   TR_Pattern *next)
      : TR_Pattern(next), _firstChoice(firstChoice),
...
protected:
  ...
  virtual bool thisMatches(TR::Node *node, TR_Unification &uni,
                           TR::Compilation *comp) {
    return TR_OpCodePattern::thisMatches(node, uni, comp) &&
           node->getInt() == _value;
  }

public:
  TR_IConstPattern(int32_t value, TR_Pattern *next = NULL)
      : TR_OpCodePattern(TR::iconst, next), _value(value) {}
};
...
protected:
  ...
  virtual bool thisMatches(TR::Node *node, TR_Unification &uni,
                           TR::Compilation *comp) {
    return TR_OpCodePattern::thisMatches(node, uni, comp) &&
           node->getLongInt() == _value;
  }

public:
  TR_LConstPattern(int64_t value, TR_Pattern *next = NULL)
      : TR_OpCodePattern(TR::lconst, next), _value(value) {}
};
...
public:
  TR_ChildPattern(int32_t childIndex, TR_Pattern *childPattern,
                  TR_Pattern *next = NULL)
      : TR_Pattern(next), _childIndex(childIndex), _childPattern(childPattern) {
  }
};
...
public:
  TR_ChildrenPattern(TR_Pattern *leftPattern, TR_Pattern *rightPattern,
                     TR_Pattern *next = NULL)
      : TR_Pattern(next), _leftPattern(leftPattern),
...
public:
  TR_CommutativePattern(TR_Pattern *leftPattern, TR_Pattern *rightPattern,
                        TR_Pattern *next = NULL)
...
class TR_PatternBuilder {
  ...
  // Many patterns can be created just by using the various overloaded
  // operator() methods.
  //
...
public:
  ...
  TR_UnifyPattern *unify(TR_Unification::TR_Index index,
                         TR_Pattern *next = NULL) {
    return new (comp()->trPersistentMemory()) TR_UnifyPattern(index, next);
  }
  TR_ChoicePattern *choice(TR_Pattern *firstChoice, TR_Pattern *secondChoice,
                           TR_Pattern *next) {
    return new (comp()->trPersistentMemory())
        TR_ChoicePattern(firstChoice, secondChoice, next);
  }
  ...
  TR_ChildPattern *child(int32_t childIndex, TR_Pattern *childPattern,
                         TR_Pattern *next = NULL) {
    return new (comp()->trPersistentMemory())
        TR_ChildPattern(childIndex, childPattern, next);
  }
  TR_ChildrenPattern *children(TR_Pattern *leftPattern,
                               TR_Pattern *rightPattern,
                               TR_Pattern *next = NULL) {
    return new (comp()->trPersistentMemory())
        TR_ChildrenPattern(leftPattern, rightPattern, next);
  }
  TR_CommutativePattern *commutative(TR_Pattern *leftPattern,
                                     TR_Pattern *rightPattern,
                                     TR_Pattern *next = NULL) {
    return new (comp()->trPersistentMemory())
        TR_CommutativePattern(leftPattern, rightPattern, next);
  }

  // Patterns that also look for particular opcodes, in addition to their usual
  // semantics
  TR_Pattern *unify(TR::ILOpCodes op, TR_Unification::TR_Index index,
                    TR_Pattern *next = NULL) {
    return opCode(op, unify(index, next));
  }
  TR_Pattern *noRegister(TR::ILOpCodes op, TR_Pattern *next = NULL) {
    return opCode(op, noRegister(next));
  }
  TR_Pattern *child(TR::ILOpCodes op, int32_t childIndex,
                    TR_Pattern *childPattern, TR_Pattern *next = NULL) {
    return opCode(op, child(childIndex, childPattern, next));
  }
  TR_Pattern *children(TR::ILOpCodes op, TR_Pattern *leftPattern,
                       TR_Pattern *rightPattern, TR_Pattern *next = NULL) {
    return opCode(op, children(leftPattern, rightPattern));
  }
  TR_Pattern *commutative(TR::ILOpCodes op, TR_Pattern *leftPattern,
                          TR_Pattern *rightPattern, TR_Pattern *next = NULL) {
    return opCode(op, commutative(leftPattern, rightPattern));
  }

  TR_Pattern *commutativeOrNot(TR::ILOpCodes op, TR_Pattern *leftPattern,
                               TR_Pattern *rightPattern,
                               TR_Pattern *next = NULL) {
    // Create a TR_CommutativePattern or TR_ChildrenPattern based on whether the
  ...
  TR_Unification::TR_Index unificationIndex(void *structure,
                                            TR::Node *&element) {
  ...
  TR_Pattern *operator()(TR::ILOpCodes op, void *structure,
                         TR::Node *&element) {
    return unify(op, unificationIndex(structure, element));
  }
  TR_Pattern *operator()(TR::ILOpCodes op, TR_Pattern *leftPattern,
                         TR_Pattern *rightPattern) {
    return commutativeOrNot(op, leftPattern, rightPattern);
  }
  TR_Pattern *operator()(TR::ILOpCodes op, void *structure, TR::Node *&element,
                         TR_Pattern *leftPattern, TR_Pattern *rightPattern) {
    return commutativeOrNot(op, leftPattern, rightPattern,
                            unify(unificationIndex(structure, element)));
  }
};
