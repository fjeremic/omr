...
// within the pattern to nodes within the target tree.
{
 ...
 protected:
  TR::Node **_unifiedNodes;  // Array of pointers.  Entries not yet unified must
                             // be NULL
  ...
  TR_Index
      _undoStack[UNIFICATION_LIMIT];  // the first _undoCount entries in this
                                      // are indices into _unifiedNodes that
                                      // have been unified so far

 public:
  TR_Unification(TR::Node **unifiedNodes)
      : _unifiedNodes(unifiedNodes),
        _undoCount(0) {}  // Caller is responsible for zeroing out unifiedNodes
