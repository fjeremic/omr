...
protected:
  ...
  TreeTopIteratorImpl(TR::TreeTop *start, TR::Compilation *comp,
...
public:
  TreeTopIterator(TR::TreeTop *start, TR::Compilation *comp,
                  const char *name = NULL)
...
protected:
  struct WalkState {
    ...
    WalkState(TR::Node *node)
        : _node(node), _child(0), _isBetweenChildren(false) {}

    bool equals(struct WalkState &other) {
      return _node == other._node && _child == other._child;
    }
...
protected:
  NodeOccurrenceIterator(TR::TreeTop *start, TR::Compilation *comp,
                         const char *name = NULL)
...
public:
  PreorderNodeOccurrenceIterator(TR::TreeTop *start, TR::Compilation *comp,
                                 const char *name = NULL);
...
public:
  PostorderNodeOccurrenceIterator(TR::TreeTop *start, TR::Compilation *comp,
                                  const char *name = NULL);
...
class ReversePostorderSnapshotBlockIterator : protected BlockIterator {
  TR_Array<TR::Block *> _postorder; // Note that this is opposite to the order
                                    // we want, so we walk this array in reverse
  int32_t _currentIndex; // can be 1 past the valid index range of _postorder:
                         // -1 or _postorder.lastIndex()+1
...
public:
  ReversePostorderSnapshotBlockIterator(TR::Block *start, TR::Compilation *comp,
                                        const char *name = NULL);
...
public:
  AllBlockIterator(TR::CFG *cfg, TR::Compilation *comp,
                   const char *name = NULL);
...
/** \brief
 *     Iterates through extended basic block sequences in a treetop order, where
 * treetop order means the order in which the extended basic blocks appear in a
 * trace log file.
 */
class TreeTopOrderExtendedBlockIterator : protected BlockIterator {

public:
  TreeTopOrderExtendedBlockIterator(TR::Compilation *comp,
                                    const char *name = NULL);

  /** \brief
   *     Returns the first basic block in the current iteration of the extended
   * basic block sequence.
   */
  TR::Block *getFirst();

  /** \brief
   *     Returns the last basic block in the current iteration of the extended
   * basic block sequence.
   */
  TR::Block *getLast();

  /** \brief
   *     Advances the iterator to the next extended basic block in TreeTop
   * order.
   */
  void operator++();
