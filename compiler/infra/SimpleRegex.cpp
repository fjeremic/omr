...
SimpleRegex::Simple* SimpleRegex::processSimple(char*& s, TR_YesNoMaybe allowAlternates, bool& foundError)
{
    ...
    if (m->remainder == NULL)
        ...
        if ((m->remainder->component->type == wildcards && (m->remainder->component->data.counts & 1) != 0) || (m->remainder->fixed_chars_right == 0 && m->remainder->remainder != NULL)) {
            m->fixed_chars_right = 0;
        } else {
            i = 0;
...
bool SimpleRegex::Simple::match(const char* s, bool isCaseSensitive, bool useLocale)
{
    ...
    case wildcards:
        ...
        if (fixed_chars_right != 0 || remainder == NULL) {
            return (strlen(s) >= fixed_chars_right) && matchesRemainder(s + strlen(s) - fixed_chars_right, isCaseSensitive, useLocale);
        }
