...
SimpleRegex *SimpleRegex::create(char *&s) {
  ...
  if (!regex || s[0] != '}' || foundError) return NULL;
  ++s;
...
void *SimpleRegex::Component::operator new(size_t size, PERSISTENT_NEW_DECLARE,
                                           size_t numChars) {
  if (numChars > sizeof(Data)) size += numChars - sizeof(Data);
  return jitPersistentAlloc(size);
}

// Process a simple_pattern: a sequence of components
//
SimpleRegex::Simple *SimpleRegex::processSimple(char *&s,
                                                TR_YesNoMaybe allowAlternates,
                                                bool &foundError) {
  int32_t i, lo, hi;
  char *startSimple = s;

  if (s[0] == '\0' || s[0] == ',' || s[0] == '|' || s[0] == '}') return NULL;
  ...
  if (s[0] == '[' && allowAlternates == TR_yes) {
    ...
    for (i = 0; i < sizeof(m->component->data.bit_map) /
                        sizeof(m->component->data.bit_map[0]);
    ...
    while (s[i] != '\0' && s[i] != '*' && s[i] != '?' && s[i] != ',' &&
           s[i] != '|' && s[i] != '}') {
      if (s[i] == '[' && allowAlternates != TR_no) break;
      if (s[i] == '\\' && s[i + 1] != '\0') ++i;
      ++i;
    }
    ...
    while (s[0] != '\0' && s[0] != '*' && s[0] != '?' && s[0] != ',' &&
           s[0] != '|' && s[0] != '}') {
      if (s[0] == '[' && allowAlternates != TR_no) break;
      if (s[0] == '\\' && s[1] != '\0') ++s;
...
bool SimpleRegex::Simple::matchesRemainder(const char *s, bool isCaseSensitive,
                                           bool useLocale) {
  if (remainder == NULL) return s[0] == '\0';

  return remainder->match(s, isCaseSensitive, useLocale);
}

bool SimpleRegex::Simple::match(const char *s, bool isCaseSensitive,
                                bool useLocale) {
  int i;
  ...
  switch (component->type) {
    case simple_string:
      for (i = 0; component->data.str[i]; ++i, ++s) {
        if (isCaseSensitive) {
          if (s[0] != component->data.str[i]) return false;
        } else {
          char lower_s0 =
              useLocale ? (tolower(s[0])) : (::tolower_ignore_locale(s[0]));
          char lower_stri =
              useLocale ? (tolower(component->data.str[i]))
      ...
      break;
    case wildcards:
      if (strlen(s) < component->data.counts >> 1) return false;
      ...
      if (fixed_chars_right != 0 || remainder == NULL) {
        return (strlen(s) >= fixed_chars_right) &&
               matchesRemainder(s + strlen(s) - fixed_chars_right,
                                isCaseSensitive, useLocale);
      }

      // tricky case abc*def*ghi
      //
      do {
        if (matchesRemainder(s, isCaseSensitive, useLocale)) return true;
      } while (*++s);
      return false;
      break;
    case char_alternatives: {
      i = s[0];
      bool charIsInBitMap = ((component->data.bit_map[BWORD(i)] &
                              (CONSTANT64(1) << BBIT(i))) != 0);
      if (!isCaseSensitive && !charIsInBitMap) {
        ...
        charIsInBitMap = ((component->data.bit_map[BWORD(lower_i)] &
                           (CONSTANT64(1) << BBIT(lower_i))) != 0) ||
                         ((component->data.bit_map[BWORD(upper_i)] &
                           (CONSTANT64(1) << BBIT(upper_i))) != 0);
      }
      return charIsInBitMap &&
             matchesRemainder(++s, isCaseSensitive, useLocale);
      break;
    }
    default:
      /*** ???? ***/
...
bool SimpleRegex::Regex::match(const char *s, bool isCaseSensitive,
                               bool useLocale) {
  int32_t rc = false;
...
void SimpleRegex::print(bool negate) {
  TR_VerboseLog::write("{");
  if (negate ^ _negate) TR_VerboseLog::write("^");
  _regex->print();
  TR_VerboseLog::write("}");
}

void SimpleRegex::Regex::print() {
  if (simple) simple->print();
  if (remainder) {
    TR_VerboseLog::write("|");
    remainder->print();
  }
...
void SimpleRegex::Simple::print() {
  ...
  switch (component->type) {
    case wildcards:
      for (i = 2; i <= component->data.counts; i += 2)
        TR_VerboseLog::write("?");
      if (component->data.counts & 1) TR_VerboseLog::write("*");
      break;
    case char_alternatives: {
      TR_VerboseLog::write("[");
      if ((component->data.bit_map[BWORD(0)] & (1 << BBIT(0))) != 0) {
        TR_VerboseLog::write("^");
        for (i = 1; i < 256; i++) {
          if ((component->data.bit_map[BWORD(i)] &
               (CONSTANT64(1) << BBIT(i))) == 0)
  ...
  if (remainder) remainder->print();
}

bool SimpleRegex::match(TR::SimpleRegex *regex, const char *s,
                        bool isCaseSensitive) {
  return regex->match(s, isCaseSensitive, true);
}

bool SimpleRegex::match(TR::SimpleRegex *regex, int32_t i,
                        bool isCaseSensitive) {
  char s[20];
  sprintf(s, "%d", i);
  return regex->match(s, isCaseSensitive, true);
...
bool SimpleRegex::match(TR::SimpleRegex *regex, TR_ResolvedMethod *feMethod,
                        bool isCaseSensitive) {
  TR::Compilation *comp = TR::comp();
  TR::StackMemoryRegion stackMemoryRegion(*comp->trMemory());

  bool result = regex->match(feMethod->signature(comp->trMemory(), stackAlloc),
                             isCaseSensitive, true);

  return result;
}

bool SimpleRegex::matchIgnoringLocale(TR::SimpleRegex *regex, const char *s,
                                      bool isCaseSensitive) {
