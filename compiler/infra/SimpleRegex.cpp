...
SimpleRegex::Regex*
SimpleRegex::processRegex(char*& s, bool& foundError)
{
...
void*
SimpleRegex::Component::operator new(size_t size,
                                     PERSISTENT_NEW_DECLARE,
                                     size_t numChars)
{
...
//
SimpleRegex::Simple*
SimpleRegex::processSimple(char*& s,
...
bool
SimpleRegex::Simple::matchesRemainder(const char* s,
...
bool
SimpleRegex::Simple::match(const char* s, bool isCaseSensitive, bool useLocale)
{
  int i;
  ...
  switch (component->type) {
    ...
    case wildcards:
      ...
      if (fixed_chars_right != 0 || remainder == NULL) {
        return (strlen(s) >= fixed_chars_right) &&
               matchesRemainder(
                 s + strlen(s) - fixed_chars_right, isCaseSensitive, useLocale);
      }
    ...
    case char_alternatives: {
      i = s[0];
      bool charIsInBitMap =
        ((component->data.bit_map[BWORD(i)] & (CONSTANT64(1) << BBIT(i))) != 0);
...
bool
SimpleRegex::Regex::match(const char* s, bool isCaseSensitive, bool useLocale)
{
  int32_t rc = false;
...
bool
SimpleRegex::match(const char* s, bool isCaseSensitive, bool useLocale)
{
...
bool
SimpleRegex::match(TR::SimpleRegex* regex, const char* s, bool isCaseSensitive)
{
  return regex->match(s, isCaseSensitive, true);
}

bool
SimpleRegex::match(TR::SimpleRegex* regex, int32_t i, bool isCaseSensitive)
{
  char s[20];
  sprintf(s, "%d", i);
  return regex->match(s, isCaseSensitive, true);
...
bool
SimpleRegex::match(TR::SimpleRegex* regex,
                   ...
                   bool isCaseSensitive)
{
  ...
  bool result = regex->match(
    feMethod->signature(comp->trMemory(), stackAlloc), isCaseSensitive, true);

  return result;
}

bool
SimpleRegex::matchIgnoringLocale(TR::SimpleRegex* regex,
                                 const char* s,
                                 bool isCaseSensitive)
{
