...
class ListBase {
 ...
 public:
  ...
  bool isDoubleton() const {
    return _pHead && _pHead->getNextElement() &&
           !_pHead->getNextElement()->getNextElement();
  }
  ...
  bool find(T *elem) const {
    for (ListElement<T> *le = _pHead; le; le = le->getNextElement())
      if (elem == le->getData()) return true;
    return false;
  }

  int32_t getSize() const {
    int32_t size = 0;
    for (ListElement<T> *p = _pHead; p != NULL; p = p->getNextElement()) size++;
    return size;
  }

  ListElement<T> *getLastElement() const {
    if (!_pHead) return NULL;
    ListElement<T> *p;
...
class List : public ListBase<T> {
 ...
 public:
  ...
  // probably don't need this unless going to also create smart iterator pointer
  // class to ensure destruction of iterator created by this entry.
  //
  // ListIterator<class T> *createIterator() {return new ListIterator<class
  // T>(this);};

  ListElement<T> *add(T *p) {
    TR_ASSERT(
        this->_region != NULL,
        "Should not be using heap allocation to add to a persistent list. The "
        "'add' routine in TR_PersistentList should be called\n");
    return this->_pHead =
               new (this->getRegion()) ListElement<T>(p, this->_pHead);
  }
  ...
  T *remove(T *elem) {
    if (this->_pHead == NULL) return NULL;
  ...
  void removeNext(ListElement<T> *previous) {
    if (previous == NULL) {
      ...
      TR_ASSERT(previous->getNextElement(),
                "List::removeNext(previous) on last element in list");
  ...
  ListElement<T> *addAfter(T *elem, ListElement<T> *previous) {
    ...
    if (previous == NULL)
      ...
      TR_ASSERT(
          this->_region != NULL,
          "Should not be using heap allocation to add to a persistent list. "
          "The 'addAfter' routine in TR_PersistentList should be called\n");
      p = new (this->getRegion())
          ListElement<T>(elem, previous->getNextElement());
  ...
  void add(List<T> &other) {
    for (ListElement<T> *cursor = other._pHead; cursor;
...
class ListIterator {
 ...
 public:
  ...
  ListIterator(ListBase<T> *p)
      : _pHead(p->getListHead()), _pCursor(p->getListHead()) {}
  ...
  bool atLastElement() {
    TR_ASSERT(
        !atEnd(),
        "atLastElement() not valid when already at the end of the iterator\n");
...
class ListAppender {
 ...
 public:
  ...
  ListAppender(List<T> *p) : _pList(p) {
    // Get _pCursor to point at the last element of the list
    //
    _pCursor = p->getListHead();
    if (_pCursor != NULL) {
      ListElement<T> *pNext;
      for (pNext = _pCursor->getNextElement(); pNext != NULL;
           _pCursor = pNext, pNext = pNext->getNextElement())
        ;
    }
  }
  ...
  ListElement<T> *add(T *p) {
    TR_ASSERT(_pList->_region != NULL,
              "Should not be using heap allocation to append to a persistent "
              "list. Should use 'addPersistent' instead\n");
  ...
  ListElement<T> *addPersistent(T *p) {
    TR_ASSERT(_pList->_region == NULL,
              "Should not be using persistent allocation to append to a "
              "non-persistent list.\n");
...
template <class T>
class TR_ScratchList : public List<T> {
 public:
  TR_ScratchList(TR_Memory *m) : List<T>(m) {
    List<T>::_region = &(m->currentStackRegion());
  }

  TR_ScratchList(T *p, TR_Memory *m) : List<T>(m) {
    List<T>::_region = &(m->currentStackRegion());
  ...
  ListElement<T> *add(T *p) {
    return List<T>::_pHead =
               new (List<T>::getRegion()) ListElement<T>(p, List<T>::_pHead);
  }
...
template <class T>
class TR_PersistentList : public List<T> {
 public:
  TR_PersistentList() : List<T>() { List<T>::_pHead = 0; }
  TR_PersistentList(T *p) : List<T>() {
    List<T>::_pHead = new (PERSISTENT_NEW) ListElement<T>(p);
  }
  ListElement<T> *add(T *p) {
    return List<T>::_pHead =
               new (PERSISTENT_NEW) ListElement<T>(p, List<T>::_pHead);
  }
 ...
 */
template <class T>
class TR_Queue : public TR_ScratchList<T> {
 public:
  ...
  void enqueue(T *elem) {
    if (_pTail == NULL) {
      List<T>::_pHead = _pTail =
          _useFreeList ? new (_freeList) ListElement<T>(elem, List<T>::_pHead)
                       : new (List<T>::getRegion())
                             ListElement<T>(elem, List<T>::_pHead);
    } else {
      ListElement<T> *p =
          _useFreeList ? new (_freeList) ListElement<T>(elem, NULL)
  ...
  void releaseFreeList() {
    // if you use a queue with freelist in a context where the queue object
    // itself survives beyond the stack scope of allocation of items and thus
    // need to reset the free list for the current stack allocation scope
...
template <class T>
class ListHeadAndTail : public List<T> {
 protected:
  ListElement<T> *_pTail;
 ...
 public:
  ...
  ListHeadAndTail(TR_Memory *m, T *p) : List<T>(m->heapMemoryRegion()) {
    init();
    add(p);
  }
  ListHeadAndTail(TR::Region &region) : List<T>(region) { init(); }
  ListHeadAndTail(TR::Region &region, T *p) : List<T>(region) {
    init();
    add(p);
  }
