...
public:
  ...
  ListElement<T>* setNextElement(ListElement<T>* p)
  {
    return (_pNext = p);
  }
};
...
public:
  ...
  void setRegion(TR::Region& region)
  {
    _region = &region;
  }
...
public:
  ...
  ListElement<T>* setListHead(ListElement<T>* p)
  {
    return (this->_pHead = p);
  }
  ...
  ListElement<T>* add(T* p)
  {
    TR_ASSERT(this->_region != NULL,
      "Should not be using heap allocation to add to a persistent list. The 'add' routine in TR_PersistentList should "
      "be called\n");
  ...
  ListElement<T>* addAfter(T* elem, ListElement<T>* previous)
  {
    ...
    if (previous == NULL)
      ...
      TR_ASSERT(this->_region != NULL,
        "Should not be using heap allocation to add to a persistent list. The 'addAfter' routine in TR_PersistentList "
        "should be called\n");
  ...
  T* getHeadData()
  {
    return this->_pHead ? this->_pHead->getData() : NULL;
  }
...
public:
  ...
  void set(ListBase<T>* p)
  {
    _pHead = _pCursor = p->getListHead();
  }
  void set(ListElement<T>* p)
  {
    _pHead = _pCursor = p;
  }
  ...
  ListElement<T>* getCurrentElement()
  {
    return _pCursor;
  }
...
public:
  ...
  bool inFirstList()
  {
    return !_iteratingThruSecondList;
  }
  bool inSecondList()
  {
    return _iteratingThruSecondList;
...
public:
  ...
  ListElement<T>* add(T* p)
  {
    return List<T>::_pHead = new (PERSISTENT_NEW) ListElement<T>(p, List<T>::_pHead);
  }
...
public:
  ...
  void init()
  {
    this->_pHead = _pTail = 0;
  }

  ListElement<T>* getListTail()
  {
    return _pTail;
  }
  ListElement<T>* setListTail(ListElement<T>* p)
  {
    return (_pTail = p);
  }
  T* getHeadData()
  {
    return this->_pHead->getData();
  }
