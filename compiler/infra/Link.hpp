...
class TR_LinkHead0 {
 public:
  ...
  bool isDoubleton() {
    return _head && _head->getNext() && !_head->getNext()->getNext();
  }
  ...
  int32_t getSize() {
    int32_t size = 0;
    for (T *c = _head; c; c = c->getNext()) ++size;
    return size;
  }
...
class TR_Link1 {
 ...
 public:
  ...
  T *getNext() {
    while (_next && !(_next->isValid())) _next = _next->_next;
    return _next;
  }
  void setNext(T *n) {
    setValid(true);
...
template <class T>
class TR_Link : public TR_Link0<T> {
 public:
...
template <class T>
class TR_LinkHead : public TR_LinkHead0<T> {
 public:
...
template <class T>
class TR_TwoLinkListIterator {
 public:
  TR_TwoLinkListIterator() {}
  TR_TwoLinkListIterator(T *l1, T *l2)
      : _firstListHead(l1), _secondListHead(l2) {}
...
// Template specialization to deal with (ptr,val) pairs that the top class can't
// deal with unless you do ugly casting
template <class Key, class Value>
class TR_Pair<Key *, Value> {
 public:
  TR_ALLOC(TR_Memory::Pair);  // FIXME
  TR_Pair(Key *key, Value value) : _key(key), _value(value) {}
  Key *getKey() { return _key; }
