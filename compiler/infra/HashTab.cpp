...
void*
TR_HashTab::getData(TR_HashId id)
{
...
void
TR_HashTab::init(uint32_t newSize, bool growth)
{
  ...
  _closedAreaSize =
    _closedAreaSize < kMinimumSize ? kMinimumSize : _closedAreaSize;
  ...
  _table = (TR_HashTableEntry**)trMemory()->allocateMemory(
    _tableSize * sizeof(TR_HashTableEntry*),
    _allocType,
    TR_Memory::HashTableEntry);
 ...
 */
bool
TR_HashTab::locate(const void* key, TR_HashId& hashIndex)
{
  TR_HashId hashValue;
 ...
 */
void
TR_HashTab::growAndRehash(uint32_t newSize)
{
...
bool
TR_HashTab::growTo(uint32_t newSize)
{
  if (NULL == _table) {
    init(newSize);
  } else {
 ...
 */
bool
TR_HashTab::addElement(void* key,
                       TR_HashId& hashIndex,
                       TR_HashTableEntry* entry)
{
  ...
  TR_ASSERT(
    hashIndex < _tableSize, "index too big %d >=%d\n", hashIndex, _tableSize);
  ...
  do {
    ...
    if (0 == nextIndex) {
      nextIndex = _nextFree++;
      TR_ASSERT(_nextFree < _tableSize,
                "out of free space: index %d (%d)\n",
                nextIndex,
                _tableSize);
...
bool
TR_HashTabIterator::atEnd()
{
  return _curHashIndex >= _baseHashTab->_tableSize;
}
