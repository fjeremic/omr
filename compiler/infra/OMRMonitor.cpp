...
void* OMR::Monitor::operator new(size_t size) {
  return TR::Compiler->persistentAllocator().allocate(size);
}

void OMR::Monitor::operator delete(void* p) {
  TR::Compiler->persistentAllocator().deallocate(p);
}

TR::Monitor* OMR::Monitor::create(char* name) {
...
TR::Monitor* OMR::Monitor::self() {
  return static_cast<TR::Monitor*>(this);
}

bool OMR::Monitor::init(char* name) {
  _name = name;
...
void OMR::Monitor::destroy(TR::Monitor* monitor) {
  delete monitor;
}
...
char const* OMR::Monitor::getName() {
  return _name;
}
