...
static inline bool
contiguousBits(uint32_t lmask)
{
  return contiguousBits((int32_t)lmask);
}

static inline bool
contiguousBits(int64_t lmask)
{
...
static inline int32_t
leadingZeroes(uint64_t input)
{
  return (leadingZeroes((int64_t)input));
}

static inline int32_t
leadingZeroes(uint32_t inputWord)
{
...
static inline int32_t
trailingZeroes(int32_t inputWord)
{
  int32_t work;
  work = inputWord;
  work = ~work & (work - 1);
...
static inline int32_t
trailingZeroes(uint32_t inputWord)
{
...
static inline int32_t
trailingZeroes(uint64_t input)
{
  return trailingZeroes((int64_t)input);
}

static inline int32_t
ceilingPowerOfTwo(int32_t inputWord)
{
  return 1 << (32 - leadingZeroes(inputWord - 1));
}

static inline int32_t
floorPowerOfTwo(int32_t inputWord)
{
  return 1 << (31 - leadingZeroes(inputWord));
}

static inline int64_t
floorPowerOfTwo64(int64_t inputWord)
{
  return 1LL << (63 - leadingZeroes(inputWord));
}

static inline int32_t
leadingOnes(int32_t inputWord)
{
  return leadingZeroes(~inputWord);
}

static inline int32_t
leadingOnes(uint32_t inputWord)
{
  return leadingZeroes(~inputWord);
}

static inline int32_t
leadingOnes(int64_t input)
{
  return leadingZeroes(~input);
}

static inline int32_t
leadingOnes(uint64_t input)
{
  return leadingZeroes(~input);
}
...
static inline int32_t
populationCount(int32_t inputWord)
{
  uint32_t work, temp;
...
static inline int32_t
populationCount(uint32_t inputWord)
{
...
// return the number of 1-bits in the argument
static inline int32_t
populationCount(int64_t inputWord)
{
  uint64_t work, temp;
...
static inline int32_t
populationCount(uint64_t inputWord)
{
...
computePositivePowerOfTen(int32_t exponent)
{
  // TODO: there is a better algorithm to reduce the number of multiplies -- see
  // Simplifier.cpp reduceExpTwoAndGreaterToMultiplication
  TR_ASSERT(exponent >= 0 && exponent <= TR::getMaxSignedPrecision<TR::Int64>(),
            "exponent %d should be in the inclusive range 0->%d\n",
            exponent,
            TR::getMaxSignedPrecision<TR::Int64>());
  uint64_t base = 1;
...
static inline bool
isPositivePowerOfTen(int64_t val)
{
  int32_t exponent = trailingZeroes((uint64_t)val);
  if (exponent <= TR::getMaxSignedPrecision<TR::Int64>() &&
      (uint64_t)val == computePositivePowerOfTen(exponent))
    return true;
  else
    return false;
}
...
static inline int64_t
getMaxAbsValueForPrecision(int32_t precision)
{
...
static int32_t
getPrecisionFromValue(int64_t value)
{
...
static inline bool
isEven(uint32_t input)
{
  return isEven((int32_t)input);
}

static inline bool
isOdd(uint32_t input)
{
  return isOdd((int32_t)input);
}

static inline bool
isEven(int64_t input)
{
  return (input & 0x1) == 0;
}
...
static inline bool
isEven(uint64_t input)
{
  return isEven((int64_t)input);
}

static inline bool
isOdd(uint64_t input)
{
  return isOdd((int64_t)input);
}

static inline bool
isNonNegativePowerOf2(int32_t input)
{
  if (input == INT_MIN) {
    return false;
  } else {
...
static inline bool
isNonPositivePowerOf2(int32_t input)
{
  return (input & -input) == -input;
}

static inline bool
isPowerOf2(int32_t input)
{
...
static inline bool
isNonNegativePowerOf2(int64_t input)
{
  if (input == LONG_MIN) {
    return false;
  } else {
...
static inline bool
isNonPositivePowerOf2(int64_t input)
{
  return (input & -input) == -input;
}

static inline bool
isPowerOf2(int64_t input)
{
  input = input < 0 ? -input : input;
  return (input & -input) == input;
}
...
static inline int32_t
trailingZeroes(intptrj_t input)
{
#ifdef TR_HOST_64BIT
  return trailingZeroes((int64_t)input);
#else
...
static inline int32_t
populationCount(intptrj_t input)
{
#ifdef TR_HOST_64BIT
