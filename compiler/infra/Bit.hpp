...
public:
  ...
  int32_t getHighBits()
  {
    return (uint32_t)intVal >> 16;
  }
  int32_t getLowBits()
  {
    return (uint32_t)intVal & 0xffff;
  }
  int32_t getLowSign()
  {
    return ((uint32_t)intVal >> 15) & 0x1;
  }
  int32_t getHighSign()
  {
    return (uint32_t)intVal >> 31;
  }
  int32_t getByte3()
  {
    return (uint32_t)intVal >> 24;
  }
  int32_t getByte2()
  {
    return ((uint32_t)intVal >> 16) & 0xff;
  }
  int32_t getByte1()
  {
    return ((uint32_t)intVal >> 8) & 0xff;
  }
  int32_t getByte0()
  {
    return (uint32_t)intVal & 0xff;
  }
