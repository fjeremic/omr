...
#if defined(OMR_OS_WINDOWS) || (defined(LINUX) && !defined(OMRZTPF)) || \
    defined(OSX) || defined(AIXPPC)
...
#define tlsAlloc(variable)                \
  if ((variable) == TLS_OUT_OF_INDEXES) { \
    (variable) = TlsAlloc();              \
  }
...
#endif
#endif /* defined(OMR_OS_WINDOWS) || (defined(LINUX) && !defined(OMRZTPF)) || \
          defined(OSX) || defined(AIXPPC) */
