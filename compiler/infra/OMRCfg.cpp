...
TR_OrderedExceptionHandlerIterator::TR_OrderedExceptionHandlerIterator(TR::Block* tryBlock, TR::Region& workingRegion)
{
  if (tryBlock->getExceptionSuccessors().empty())
    ...
    for (auto e = tryBlock->getExceptionSuccessors().begin(); e != tryBlock->getExceptionSuccessors().end(); ++e) {
      TR::Block* b = toBlock((*e)->getTo());
      TR_ASSERT((_handlers[((inlineDim - b->getInlineDepth() - 1) * handlerDim) + b->getHandlerIndex()] == NULL),
...
int32_t OMR::CFG::createTraversalOrder(bool forward, TR_AllocationKind allocationKind, TR_BitVector* backEdges)
{
  ...
  /*
   * This is allocated before entering the new stack memory region so that the caller can allocate it on its stack scope
   * if desired.
   */
        ...
        for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
          if (comp()->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp(), "\t\tsuccessor %d coldness %d very coldness %d\n", edge->getTo()->getNumber(),
              (int)edge->getTo()->asBlock()->isCold(), (int)edge->getTo()->asBlock()->isSuperCold());
...
bool OMR::CFG::setEdgeFrequenciesFrom()
{
  ...
  for (node = getFirstNode(); node; node = node->getNext()) {
    ...
    for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
      ...
      if (pred->getEntry() != NULL && (pred->getSuccessors().size() == 1) && pred->getSuccessors().front()->getTo()
    ...
    if (regionStr != NULL) {
      ...
      if (sumEdgeFrequency > 0) {
        for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
          ...
          setFrequencyOnSuccessor(region, succ, start, succNum, sumEdgeFrequency, parentFrequency, _regionFrequencies,
            i, _compilation, _max_edge_freq);
        }
      }
...
void OMR::CFG::setBlockAndEdgeFrequenciesBasedOnStructure()
{
  ...
  if (setBlockFrequencies) {
...

    {
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(comp(),
          "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Computing region weight factors based on CFG structure, num "
          "regions=%d...\n",
...
int32_t TR::CFGNode::denormalizedFrequency(int32_t normalizedFrequency, int32_t maxFrequency)
{
  ...
  // so cast to int64_t
  int32_t frequency
    = (int32_t)((int64_t)((maxFrequency * (int64_t)(normalizedFrequency - MAX_COLD_BLOCK_COUNT))) / MAX_BLOCK_COUNT);
  return frequency;
}
...
void OMR::CFG::findReachableBlocks(TR_BitVector* result)
{
  ...
  if (edgesToBeRemoved.isEmpty())
    return;
  // the below code is just a second pass to verify that removing edges from start to the alwayskeepblocks doesn't make
  // the alwayskeepblocks unreachable this is possible when there is a cycle in the CFG
