...
void OMR::CFG::addExceptionEdge(TR::CFGNode *f, TR::CFGNode *t) {
  ...
  for (auto e = f->getExceptionSuccessors().begin();
    ...
    if (existingCatchBlock->getCatchType() == 0 ||
      ...
      if (comp()->getOption(TR_TraceAddAndRemoveEdge)) {
        traceMsg(comp(), "\nAddition of exception edge aborted - existing "
                         "catch alredy handles this case!");
      }
      return;
    }
  }
...
TR::CFGNode::CFGNode(TR_Memory *m)
    : _nodeNumber(-1), _visitCount(0), _frequency(-1),
      _forwardTraversalIndex(-1), _backwardTraversalIndex(-1),
      _region(m->heapMemoryRegion()), _successors(m->heapMemoryRegion()),
...
TR::CFGNode::CFGNode(int32_t n, TR_Memory *m)
    : _nodeNumber(n), _visitCount(0), _frequency(-1),
      _forwardTraversalIndex(-1), _backwardTraversalIndex(-1),
      _region(m->heapMemoryRegion()), _successors(m->heapMemoryRegion()),
      _predecessors(m->heapMemoryRegion()),
      _exceptionSuccessors(m->heapMemoryRegion()),
...
TR::CFGNode::CFGNode(TR::Region &region)
    : _nodeNumber(-1), _visitCount(0), _frequency(-1),
      _forwardTraversalIndex(-1), _backwardTraversalIndex(-1), _region(region),
      _successors(region), _predecessors(region), _exceptionSuccessors(region),
...
static OrphanType unreachableOrphan(TR::CFG *cfg, TR::CFGNode *from,
  ...
  if (cfg->getStructure()) {
    ...
    TR_ASSERT(toSubNode, "cannot have cfg edge a->b in region p where b is not "
                         "a direct subnode of p");
...
void OMR::CFG::setBlockAndEdgeFrequenciesBasedOnStructure() {
  if (_compilation->getOption(TR_TraceBFGeneration))
    dumpOptDetails(comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: "
                           "Setting edge frequencies...\n");
  ...
  if (setBlockFrequencies) {
    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(
          comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating "
                  "block and edge frequencies within regions...\n");
...

    {
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(
            comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating "
                    "weight factors based on CFG structure...\n");
...
void OMR::CFG::getBranchCounters(TR::Node *node, TR::Block *block,
  ...
  TR_ASSERT(0, "Branch counters can only be set if we have either an external "
