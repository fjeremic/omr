...
void
OMR::CFG::addExceptionEdge(TR::CFGNode* f, TR::CFGNode* t)
{
  if (comp()->getOption(TR_TraceAddAndRemoveEdge)) {
    traceMsg(comp(),
  ...
  for (auto e = f->getExceptionSuccessors().begin();
       e != f->getExceptionSuccessors().end();
       ++e) {
...
void
OMR::CFG::copyExceptionSuccessors(TR::CFGNode* from,
                                  TR::CFGNode* to,
                                  bool (*predicate)(TR::CFGEdge*))
{
  for (auto e1 = from->getExceptionSuccessors().begin();
       e1 != from->getExceptionSuccessors().end();
       ++e1) {
    if (predicate(*e1)) {
  ...
  TR::Region& workingRegion)
{
  if (tryBlock->getExceptionSuccessors().empty())
    ...
    for (auto e = tryBlock->getExceptionSuccessors().begin();
         e != tryBlock->getExceptionSuccessors().end();
         ++e) {
...
TR::CFGEdge*
TR::CFGEdge::createEdge(TR::CFGNode* pF,
                        TR::CFGNode* pT,
                        TR_Memory* trMemory,
...
TR::CFGEdge*
TR::CFGEdge::createEdge(TR::CFGNode* pF, TR::CFGNode* pT, TR::Region& region)
{
...
TR::CFGEdge*
TR::CFGEdge::createExceptionEdge(TR::CFGNode* pF,
                                 TR::CFGNode* pT,
                                 TR_Memory* trMemory,
                                 TR_AllocationKind allocKind)
{
...
void
TR::CFGEdge::setFrom(TR::CFGNode* pF)
{
  _pFrom = pF;
...
void
TR::CFGEdge::setTo(TR::CFGNode* pT)
{
  _pTo = pT;
...
void
TR::CFGEdge::setExceptionFrom(TR::CFGNode* pF)
{
  _pFrom = pF;
...
void
TR::CFGEdge::setExceptionTo(TR::CFGNode* pT)
{
  _pTo = pT;
...
void
TR::CFGEdge::setFromTo(TR::CFGNode* pF, TR::CFGNode* pT)
{
  setFrom(pF);
  setTo(pT);
}
...
void
TR::CFGEdge::setExceptionFromTo(TR::CFGNode* pF, TR::CFGNode* pT)
{
  _pFrom = pF;
  _pTo = pT;
  pF->addExceptionSuccessor(this);
...
static int32_t
getTotalEdgesFrequency(TR::CFGEdgeList& edgesList)
{
  int32_t totalFreq = 0;
...
bool
OMR::CFG::updateBlockFrequency(TR::Block* block, int32_t newFreq)
{
  int16_t oldFreq = block->getFrequency();
  if (newFreq != oldFreq && newFreq >= 0) {
    if (comp()->getOption(TR_TraceBFGeneration))
      traceMsg(comp(),
               "updated block %d freq from %d to %d\n",
               block->getNumber(),
               oldFreq,
               newFreq);
...
void
OMR::CFG::updateBlockFrequencyFromEdges(TR::Block* block)
{
...
//
void
TR::CFGNode::removeFromCFG(TR::Compilation* c)
{}

TR::TreeTop*
OMR::CFG::findLastTreeTop()
{
...
TR::CFGEdge*
TR::CFGNode::getEdge(TR::CFGNode* n)
{
...
TR::CFGEdge*
TR::CFGNode::getEdgeMatchingNodeInAList(TR::CFGNode* n,
                                        TR::CFGEdgeList& list,
                                        FUNC blockGetter)
{
...
TR::CFGEdge*
TR::CFGNode::getSuccessorEdge(TR::CFGNode* n)
{
  return getEdgeMatchingNodeInAList(n, getSuccessors(), toBlockGetter);
}

TR::CFGEdge*
TR::CFGNode::getExceptionSuccessorEdge(TR::CFGNode* n)
{
  return getEdgeMatchingNodeInAList(n, getExceptionSuccessors(), toBlockGetter);
}

TR::CFGEdge*
TR::CFGNode::getPredecessorEdge(TR::CFGNode* n)
{
  return getEdgeMatchingNodeInAList(n, getPredecessors(), fromBlockGetter);
}

TR::CFGEdge*
TR::CFGNode::getExceptionPredecessorEdge(TR::CFGNode* n)
{
  return getEdgeMatchingNodeInAList(
    n, getExceptionPredecessors(), fromBlockGetter);
}

bool
TR::CFGNode::hasSuccessor(TR::CFGNode* n)
{
  return (bool)getSuccessorEdge(n);
}

bool
TR::CFGNode::hasExceptionSuccessor(TR::CFGNode* n)
{
  return (bool)getExceptionSuccessorEdge(n);
}

bool
TR::CFGNode::hasPredecessor(TR::CFGNode* n)
{
  return (bool)getPredecessorEdge(n);
}

bool
TR::CFGNode::hasExceptionPredecessor(TR::CFGNode* n)
{
  return (bool)getExceptionPredecessorEdge(n);
}

TR::CFGNode*
OMR::CFG::removeNode(TR::CFGNode* node)
{
...
//
static OrphanType
unreachableOrphan(TR::CFG* cfg, TR::CFGNode* from, TR::CFGNode* to)
{
...
*/
bool
OMR::CFG::removeEdge(TR::CFGEdge* edge)
{
...
bool
OMR::CFG::removeEdge(TR::CFGEdge* edge, bool recursiveImpl)
{
  TR::CFGNode* from = edge->getFrom();
  TR::CFGNode* to = edge->getTo();
...
bool
OMR::CFG::removeEdge(TR::CFGNode* from, TR::CFGNode* to)
{
...
void
OMR::CFG::removeEdge(TR::CFGEdgeList& succList,
                     int32_t selfNumber,
                     int32_t destNumber)
{
...
void
OMR::CFG::resetVisitCounts(vcount_t count)
{
...
bool
OMR::CFG::consumePseudoRandomFrequencies()
{
  TR::CFGNode* nextNode = getFirstNode();
...
int32_t
OMR::CFG::compareExceptionSuccessors(TR::CFGNode* node1, TR::CFGNode* node2)
{
...
bool
alwaysTrue(TR::CFGEdge* e)
{
  return true;
}

void
TR::CFGNode::moveSuccessors(TR::CFGNode* to)
{
...
void
TR::CFGNode::movePredecessors(TR::CFGNode* to)
{
...
void
OMR::CFG::findLoopingBlocks(TR_BitVector& loopingBlocks)
{
  ...
  typedef TR::typed_allocator<std::pair<int32_t const, LoopInfo*>, TR::Region&>
    LoopMapAllocator;
        ...
        do {
          ...
          for (auto itr = loops.begin(), end = loops.end(); itr != end; ++itr) {
            ...
            if (itr->second->top >= index) {
              if (trace)
                dumpOptDetails(comp(),
                               "Block %d in a loop based at block %d\n",
                               number,
                               stack[itr->second->bottom]->getNumber());
...
int32_t
OMR::CFG::createTraversalOrder(bool forward,
                               ...
                               TR_BitVector* backEdges)
{
  ...
  TR::CFGNode** traversalOrder = (TR::CFGNode**)trMemory()->allocateMemory(
    numberOfBlocks * sizeof(TR::CFGNode*),
    allocationKind,
  ...
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  TR_Stack<TR::CFGNode*> blockStack(
    trMemory(), numberOfBlocks, false, stackAlloc);
  TR_Stack<TR_CFGIterator*> iteratorStack(
    trMemory(), numberOfBlocks, false, stackAlloc);
  TR_BitVector* blocksBeingVisited =
    new (trStackMemory()) TR_BitVector(numberOfBlocks, trMemory(), stackAlloc);
...
OMR::CFG::propagateColdInfo(bool generateFrequencies)
{
  ...
  for (int b = 0; b < _forwardTraversalLength; b++) {
    ...
    if (!block->asBlock()->isCold() ||
      ...
      if (allPredsCold)
        for (TR::CFGEdge* edge = pit.getFirst(); edge; edge = pit.getNext()) {
          ...
          if (haveSuperColdBlocks &&
            ...
            if (comp()->getOption(TR_TraceBFGeneration))
              dumpOptDetails(comp(),
                             "\t\tignoring backedge from %d\n",
                             edge->getFrom()->getNumber());
            continue; // don't bother looking at loop backedges
          }
          if (comp()->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp(),
                           "\t\tpredecessor %d coldness %d\n",
                           edge->getFrom()->getNumber(),
      ...
      if (allPredsCold && performTransformation(
                            comp(),
                            "%smarked block_%d cold (all preds were cold)\n",
                            OPT_DETAILS,
                            block->getNumber())) {
  ...
  for (int b = 0; b < _backwardTraversalLength; b++) {
    ...
    if (!block->asBlock()->isCold() ||
      ...
      if (allSuccsCold && (!comp()->ilGenTrace() ||
                           performTransformation(
                             comp(),
                             "%smarked block_%d cold (all succs were cold)\n",
                             OPT_DETAILS,
                             block->getNumber()))) {
        block->asBlock()->setIsCold();
...
void
OMR::CFG::setUniformEdgeFrequenciesOnNode(TR::CFGNode* node,
                                          ...
                                          TR::Compilation* comp)
{
  ...
  for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end();
    ...
    if (comp->getOption(TR_TraceBFGeneration))
      dumpOptDetails(comp,
                     "Edge %p between %d and %d has freq %d (Uniform)\n",
                     *e,
                     (*e)->getFrom()->getNumber(),
                     (*e)->getTo()->getNumber(),
                     (*e)->getFrequency());
...
void
OMR::CFG::setEdgeFrequenciesOnNode(TR::CFGNode* node,
                                   int32_t branchToCount,
                                   ...
                                   TR::Compilation* comp)
{
  ...
  for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end();
    ...
    if (comp->getOption(TR_TraceBFGeneration))
      dumpOptDetails(comp,
                     "Edge %p between %d and %d has freq %d\n",
                     *e,
                     (*e)->getFrom()->getNumber(),
                     (*e)->getTo()->getNumber(),
                     (*e)->getFrequency());
...
OMR::CFG::setEdgeFrequenciesFrom()
{
  ...
  for (node = getFirstNode(); node; node = node->getNext()) {
    ...
    if (block->getEntry() != NULL && (node->getSuccessors().size() == 2) &&
      ...
      self()->getBranchCounters(
        ifNode, block, &branchToCount, &fallThroughCount, comp());

      if (fallThroughCount || branchToCount) {
        self()->setEdgeFrequenciesOnNode(
          node, branchToCount, fallThroughCount, _compilation);
        ...
        self()->setEdgeFrequenciesOnNode(
          node, _max_edge_freq, _max_edge_freq, _compilation);
      }
    } else if (block->getEntry() != NULL &&
               !((node->getSuccessors().size() == 1) ||
                 (node->getSuccessors().size() == 2))) {
      self()->setUniformEdgeFrequenciesOnNode(
        node, _max_edge_freq, false, _compilation);
    }
  }
...
OMR::CFG::propagateEntryFactorsFrom(TR_Structure* str, float factor)
{
  ...
  if (region == NULL) {
    ...
    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(
        comp(),
        "Set block frequency on block_%d to %d, current factor %lf\n",
        block->getNumber(),
        block->getFrequency(),
        factor);
    return;
...
OMR::CFG::computeEntryFactorsAcyclic(TR_RegionStructure* region)
{
  ...
  if (_compilation->getOption(TR_TraceBFGeneration))
    dumpOptDetails(comp(),
                   "Setting factor of %lf on region %d \n",
                   factor,
                   region->getNumber());
}
...
OMR::CFG::computeEntryFactorsLoop(TR_RegionStructure* region)
{
  ...
  for (TR::CFGEdge* edge = pit.getFirst(); edge; edge = pit.getNext()) {
    ...
    isBackEdge =
      region->contains(pred->asBlock()->getStructureOf(), this->getStructure());

    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(comp(),
                     "For loop %d pred %d isBackEdge %d\n",
                     region->getNumber(),
                     pred->getNumber(),
                     isBackEdge);
  ...
  if (toStructureSubGraphNode(succ)->getStructure() &&
    ...
    if ((thisEdgeFrequency == max_edge_freq) &&
      ...
      if (succBlock) {
        ...
        if (_compilation->getOption(TR_TraceBFGeneration))
          dumpOptDetails(_compilation,
                         "Setting frequency of %d on region %d (to block)\n",
...
static void
summarizeEdgeFrequencies(ListIterator<TR::CFGEdge> i,
                         int32_t* sumEdgeFreq,
                         int32_t* edgeCount)
{
  for (TR::CFGEdge* e = i.getFirst(); e; e = i.getNext()) {
                        ...
                        TR::Compilation* _compilation)
{
  for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
    ...
    if (!_seenNodes->isSet(succ->getNumber()) &&
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(_compilation,
                       "Added block(or region) %d to the walk\n",
                       succ->getNumber());
    }
  }
}
                            ...
                            TR::Compilation* _compilation)
{
  for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
    ...
    if (!_seenNodes->isSet(succ->getNumber()) &&
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(
          _compilation, "Added block_%d to the walk\n", succ->getNumber());
    }
  }
}
...
OMR::CFG::walkStructure(TR_RegionStructure* region)
{
  ...
  if (startBlock) {
    ...
    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(comp(),
                     "Setting frequency of %d on block_%d (entry)\n",
                     INITIAL_BLOCK_FREQUENCY_FACTOR,
                     startBlock->getBlock()->getNumber());
  }
    ...
    if (regionStr != NULL) {
      ...
      if (sumEdgeFrequency > 0) {
        for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
          ...
          setFrequencyOnSuccessor(region,
                                  succ,
                                  start,
                                  succNum,
                                  sumEdgeFrequency,
      ...
      if ((block->getSuccessors().size() == 2) &&
        ...
        if (toStructureSubGraphNode(succ)->getStructure() &&
          ...
          if (!_seenNodes->isSet(succ->getNumber())) {
            if (_compilation->getOption(TR_TraceBFGeneration))
              dumpOptDetails(
                comp(), "Added block_%d to the walk\n", succ->getNumber());
            stack.add(succ);
          }
          ...
          if (branchToBlock) {
            ...
            if (_compilation->getOption(TR_TraceBFGeneration))
              dumpOptDetails(
                comp(),
                "Setting frequency of %d on region %d (GOTO block)\n",
                _regionFrequencies[succ->getNumber()],
                succ->getNumber());
          }
        }
        ...
        generalFrequencyPropagation(region,
                                    sit,
                                    block,
                                    _seenNodes,
                                    _regionFrequencies,
...
void
OMR::CFG::getBranchCounters(TR::Node* node,
                            TR::Block* block,
                            int32_t* taken,
                            int32_t* notTaken,
                            TR::Compilation* comp)
{
  ...
  if (((branchToBlock->getFrequency() >= 0) &&
    ...
    *notTaken = fallThroughFrequency; //(fallThroughFrequency*blockFreq)/(branchToFrequency
    ...
    if (comp->getOption(TR_TraceBFGeneration))
      traceMsg(comp,
               "raw scaling %d max edge %d old max edge %d\n",
               rawScalingFactor,
               _maxEdgeFrequency,
    ...
    *notTaken =
      fallThroughBlock->getFrequency(); //(fallThroughBlock->getFrequency()*blockFreq)/(branchToBlock->getFrequency()
                                        //+ fallThroughBlock->getFrequency());
...
int32_t
TR::CFGNode::normalizedFrequency(int32_t rawFrequency, int32_t maxFrequency)
{
  int32_t delta;
  if (maxFrequency > 0) {
...
void
TR::CFGNode::normalizeFrequency(int32_t frequency, int32_t maxFrequency)
{
...
void
TR::CFGNode::normalizeFrequency(int32_t maxFrequency)
{
...
int32_t
TR::CFGNode::denormalizedFrequency(int32_t normalizedFrequency,
...
int32_t
TR::CFGNode::denormalizeFrequency(int32_t maxFrequency)
{
...
void
TR::CFGEdge::normalizeFrequency(int32_t maxEdgeFrequency)
{
  int32_t frequency = getFrequency();
...
void
OMR::CFG::normalizeNodeFrequencies(TR_BitVector* nodesToBeNormalized,
                                   TR_Array<TR::CFGEdge*>* edgeArray)
{
  ...
  if (_maxFrequency < 0) {
    for (node = getFirstNode(); node; node = node->getNext()) {
      ...
      if (comp()->getOption(TR_TraceBFGeneration))
        traceMsg(comp(),
                 "11maxFrequency old %d new %d node %d\n",
                 _maxFrequency,
                 frequency,
                 node->getNumber());
      if (frequency > _maxFrequency) {
        if (comp()->getOption(TR_TraceBFGeneration))
          traceMsg(comp(),
                   "22maxFrequency old %d new %d node %d\n",
                   _maxFrequency,
                   frequency,
                   node->getNumber());
  ...
  for (node = getFirstNode(); node; node = node->getNext()) {
    if (nodesToBeNormalized->get(node->getNumber())) {
      ...
      if (comp()->getOption(TR_TraceBFGeneration))
        traceMsg(comp(),
                 "normalize : max frequency %d freq %d node %d\n",
                 _maxFrequency,
                 frequency,
                 node->getNumber());
...
void
OMR::CFG::normalizeEdgeFrequencies(TR_Array<TR::CFGEdge*>* edgesArray)
{
  TR::CFGEdge* edge;
  int32_t edgeIndex = 0;
...
void
OMR::CFG::normalizeFrequencies(TR_BitVector* nodesToBeNormalized)
{
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
