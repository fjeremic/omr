...
TR::CFGNode *OMR::CFG::addNode(TR::CFGNode *n, TR_RegionStructure *parent,
                               bool isEntryInParent) {
  _nodes.add(n);
  ...
  if (parent && getStructure()) {
    ...
    if (block) {
      ...
      if (!blockStructure)
        blockStructure = new (structureRegion())
            TR_BlockStructure(comp(), block->getNumber(), block);
      else {
      ...
      if (!blockNode) {
        blockNode =
            new (structureRegion()) TR_StructureSubGraphNode(blockStructure);
        if (!isEntryInParent) {
...
TR::CFGEdge *OMR::CFG::addEdge(TR::CFGNode *f, TR::CFGNode *t) {
  ...
  TR_ASSERT(
      !f->hasExceptionSuccessor(t),
      "adding a non exception edge when there's already an exception edge");
...
void OMR::CFG::addExceptionEdge(TR::CFGNode *f, TR::CFGNode *t) {
  ...
  for (auto e = f->getExceptionSuccessors().begin();
       e != f->getExceptionSuccessors().end(); ++e) {
    TR::Block *existingCatchBlock = toBlock((*e)->getTo());
    if (newCatchBlock == existingCatchBlock) return;

    // OSR exception edges are special and we do not want any 'optimization'
    // done to them from the following special checks
    if (newCatchBlock->isOSRCatchBlock() ||
        existingCatchBlock->isOSRCatchBlock())
      continue;

    // If the existing catch block is going to be considered first and it
    // catches everything that the new catch block does then the new catch block
    // isn't reaching from the 'f' block
    //
    // Catch block 'A' is considered before catch block 'B' if 'A' is from a
    // greater inline depth or 'A' and 'B' are from the same inline depth and
    // 'A' handler index is less than 'B's.
    //
    ...
    if (existingDepth < newDepth ||
        (existingDepth == newDepth && existingCatchBlock->getHandlerIndex() >
                                          newCatchBlock->getHandlerIndex()))
      continue;

    // The existing catch block is going to be considered first.  Don't add an
    // edge to the new catch block if the existing one catches everything that
    // the new one catches.
    //
    ...
    if (existingCatchBlock->getCatchType() == 0 ||
        ...
        (existingDepth == newDepth &&
         existingCatchBlock->getCatchType() == newCatchBlock->getCatchType())) {
      if (comp()->getOption(TR_TraceAddAndRemoveEdge)) {
        traceMsg(comp(),
                 "\nAddition of exception edge aborted - existing catch alredy "
                 "handles this case!");
      }
      return;
    }
...
void OMR::CFG::addSuccessorEdges(TR::Block *block) {
  ...
  switch (node->getOpCode().getOpCodeValue()) {
    ...
    case TR::ifdcmpleu: {
      ...
      if (branchBlock != nextBlock) addEdge(block, nextBlock);
      break;
    }
    case TR::Goto:
...
void OMR::CFG::copyExceptionSuccessors(TR::CFGNode *from, TR::CFGNode *to,
                                       bool (*predicate)(TR::CFGEdge *)) {
  for (auto e1 = from->getExceptionSuccessors().begin();
       e1 != from->getExceptionSuccessors().end(); ++e1) {
    if (predicate(*e1)) {
...
TR_OrderedExceptionHandlerIterator::TR_OrderedExceptionHandlerIterator(
    TR::Block *tryBlock, TR::Region &workingRegion) {
  if (tryBlock->getExceptionSuccessors().empty())
    _dim = 0;
  else {
    int32_t handlerDim = 1, inlineDim = 1;
    for (auto e = tryBlock->getExceptionSuccessors().begin();
         e != tryBlock->getExceptionSuccessors().end(); ++e) {
      TR::Block *b = toBlock((*e)->getTo());
      if (b->getHandlerIndex() >= handlerDim)
        handlerDim = b->getHandlerIndex() + 1;
      if (b->getInlineDepth() >= inlineDim) inlineDim = b->getInlineDepth() + 1;
    }

    _dim = handlerDim * inlineDim;
    _handlers =
        (TR::Block **)workingRegion.allocate(_dim * sizeof(TR::Block *));
    ...
    for (auto e = tryBlock->getExceptionSuccessors().begin();
      ...
      TR::Block *b = toBlock((*e)->getTo());
      TR_ASSERT(
          (_handlers[((inlineDim - b->getInlineDepth() - 1) * handlerDim) +
                     b->getHandlerIndex()] == NULL),
...
TR::Block *TR_OrderedExceptionHandlerIterator::getCurrent() {
  TR::Block *handler = 0;
  while (_index < _dim && (handler = _handlers[_index]) == 0) ++_index;
...
TR::CFGEdge *TR::CFGEdge::createEdge(TR::CFGNode *pF, TR::CFGNode *pT,
                                     TR_Memory *trMemory,
  ...
  if (pT->getFrequency() >= 0) newEdge->setFrequency(pT->getFrequency());
  if ((pF->getFrequency() >= 0) &&
      (pF->getFrequency() < newEdge->getFrequency()))
...
TR::CFGEdge *TR::CFGEdge::createExceptionEdge(TR::CFGNode *pF, TR::CFGNode *pT,
                                              TR_Memory *trMemory,
                                              TR_AllocationKind allocKind) {
  TR::CFGEdge *newEdge = new (trMemory, allocKind) TR::CFGEdge(pF, pT);
...
void OMR::CFG::updateBlockFrequencyFromEdges(TR::Block *block) {
  int32_t totalPredFreq =
      getTotalEdgesFrequency(block->getPredecessors()) +
      getTotalEdgesFrequency(block->getExceptionPredecessors());
  int32_t totalSuccFreq =
      getTotalEdgesFrequency(block->getSuccessors()) +
      getTotalEdgesFrequency(block->getExceptionSuccessors());
...
TR::TreeTop *OMR::CFG::findLastTreeTop() {
  TR::Block *cursorBlock =
      getStart()->getSuccessors().front()->getTo()->asBlock();
  ...
  if (prevCursorBlock) return prevCursorBlock->getExit();
...
TR::CFGEdge *TR::CFGNode::getEdge(TR::CFGNode *n) {
  ...
  for (TR::CFGEdge *edge = ei.getFirst(); edge != NULL; edge = ei.getNext()) {
    if (edge->getTo() == n) return edge;
  }
  return NULL;
}

template <typename FUNC>
TR::CFGEdge *TR::CFGNode::getEdgeMatchingNodeInAList(TR::CFGNode *n,
                                                     TR::CFGEdgeList &list,
                                                     FUNC blockGetter) {
  for (auto edge = list.begin(); edge != list.end(); ++edge) {
    if (blockGetter(*edge) == n) return *edge;
  }
  return NULL;
}
...
TR::CFGEdge *TR::CFGNode::getExceptionPredecessorEdge(TR::CFGNode *n) {
  return getEdgeMatchingNodeInAList(n, getExceptionPredecessors(),
                                    fromBlockGetter);
}
...
TR::CFGNode *OMR::CFG::removeNode(TR::CFGNode *node) {
  if (node->nodeIsRemoved()) return 0;
  ...
  // All predecessors must have been removed otherwise the CFG will be
  // inconsistent.
  //
...
//
static OrphanType unreachableOrphan(TR::CFG *cfg, TR::CFGNode *from,
                                    TR::CFGNode *to) {
  ...
  // to be unreachable.
  //
  if ((to->getPredecessors().empty() &&
       to->getExceptionPredecessors().empty() && to != cfg->getEnd()) ||
      (to->getExceptionPredecessors().empty() &&
       (to->getPredecessors().size() == 1) &&
       to->getPredecessors().front()->getFrom() == to) ||
      (to->getPredecessors().empty() &&
       (to->getExceptionPredecessors().size() == 1) &&
  ...
  // from backedges originating from within 'to' then recognize 'to' to be
  // orphaned
  //
  if (cfg->getStructure()) {
    TR_RegionStructure *parent =
        from->asBlock()->getStructureOf()->findCommonParent(
            to->asBlock()->getStructureOf(), cfg);
    TR_StructureSubGraphNode *toSubNode =
        parent->findSubNodeInRegion(to->getNumber());
    TR_ASSERT(toSubNode,
              "cannot have cfg edge a->b in region p where b is not a direct "
              "subnode of p");
    ...
    if (toRegion) {
      ...
      if (onlyBackEdges) return IsOrphanedRegion;
    }
  }
...
bool OMR::CFG::removeEdge(TR::CFGEdge *edge) {
  ...
  if (std::find(from->getSuccessors().begin(), from->getSuccessors().end(),
                edge) != from->getSuccessors().end()) {
    found = true;
    from->getSuccessors().remove(edge);
  } else if (std::find(from->getExceptionSuccessors().begin(),
                       from->getExceptionSuccessors().end(),
                       edge) != from->getExceptionSuccessors().end()) {
    found = true;
    from->getExceptionSuccessors().remove(edge);
  }

  if (std::find(to->getPredecessors().begin(), to->getPredecessors().end(),
                edge) != to->getPredecessors().end()) {
    found = true;
    to->getPredecessors().remove(edge);
  } else if (std::find(to->getExceptionPredecessors().begin(),
                       to->getExceptionPredecessors().end(),
                       edge) != to->getExceptionPredecessors().end()) {
    found = true;
  ...
  if (comp()->getOption(TR_TraceAddAndRemoveEdge))
    traceMsg(comp(), "\nRemoving edge %d-->%d (depth %d):\n", from->getNumber(),
             to->getNumber(), _removeEdgeNestingDepth);
  ...
  if (orphan != IsParented) {
...

    {
      ...
      blocksVisited = new (trStackMemory())
          TR_BitVector(getNextNodeNumber(), trMemory(), stackAlloc);
  ...
  if (doWalk) {
    ...
    do {
      ...
      if (blocksVisited->get(from->getNumber())) continue;

      // if the node is already removed, donot process
      //
      if (from->nodeIsRemoved()) continue;
      ...
      for (TR::CFGEdge *e = edgesIt.getFirst(); e; e = edgesIt.getNext()) {
        ...
        if (std::find(from->getSuccessors().begin(),
                      from->getSuccessors().end(),
                      e) != from->getSuccessors().end())
          from->getSuccessors().remove(e);
        else
          from->getExceptionSuccessors().remove(e);
        if (std::find(to->getPredecessors().begin(),
                      to->getPredecessors().end(),
                      e) != to->getPredecessors().end())
        ...
        if (orphan != IsParented && !blocksVisited->get(to->getNumber()) &&
            !to->nodeIsRemoved()) {
          {
            if (comp()->getOption(TR_TraceAddAndRemoveEdge))
              traceMsg(comp(), "\nAdding node %d to nodesToBeRemoved from %d\n",
            ...
            if (orphan == IsOrphanedNode) nodesToBeRemoved.add(to);
        ...
        if (getStructure()) {
          ...
          if (comp()->getOption(TR_TraceAddAndRemoveEdge)) {
            traceMsg(comp(),
                     ...
                     from->getNumber(), to->getNumber());
            /// comp()->getDebug()->print(comp()->getOutFile(), getStructure(),
            /// 6);
          }
        }
      }
...
bool OMR::CFG::removeEdge(TR::CFGEdge *edge, bool recursiveImpl) {
  ...
  //
  if (_removeEdgeNestingDepth > (comp()->getOption(TR_ProcessHugeMethods)
                                     ? (MAX_REMOVE_EDGE_NESTING_DEPTH * 2)
                                     : MAX_REMOVE_EDGE_NESTING_DEPTH)) {
    comp()->failCompilation<TR::ExcessiveComplexity>(
        "Exceeded removeEdge nesting depth");
  }
  ...
  // If we have structure, and 'to' is the entry node of a cyclic region
  // and as a result of removing this edge we have made 'to' unreachable, except
  // from backedges originating from within 'to' then remove the backedges so
  // that we can remove the 'to' node by correctly recognize 'to' to be orphaned
  //
  if (getStructure()) {
    TR_RegionStructure *parent =
        from->asBlock()->getStructureOf()->findCommonParent(
            to->asBlock()->getStructureOf(), self());
    TR_StructureSubGraphNode *toSubNode =
        parent->findSubNodeInRegion(to->getNumber());
    TR_ASSERT(toSubNode,
              "cannot have cfg edge a->b in region p where b is not a direct "
  ...
  //
  if ((to->getPredecessors().empty() &&
       to->getExceptionPredecessors().empty() && to != getEnd()) ||
      (to->getPredecessors().empty() &&
       (to->getExceptionPredecessors().size() == 1) &&
       to->getExceptionPredecessors().front()->getFrom() == to) ||
      (to->getExceptionPredecessors().empty() &&
...
bool OMR::CFG::removeEdge(TR::CFGNode *from, TR::CFGNode *to) {
  ...
  for (TR::CFGEdge *edge = ei.getFirst(); edge != NULL; edge = ei.getNext())
    if (edge->getTo() == to) return removeEdge(edge);
  return false;
}

void OMR::CFG::removeEdge(TR::CFGEdgeList &succList, int32_t selfNumber,
                          int32_t destNumber) {
...
void OMR::CFG::removeUnreachableBlocks() {
  // Prevent recursion
  //
  if (_removingUnreachableBlocks) return;
  ...
  //
  TR_BitVector reachableBlocks(getNumberOfNodes(), comp()->trMemory(),
                               stackAlloc, growable);
  ...
  for (; node; node = node->getNext()) {
    if (!reachableBlocks.get(node->getNumber()) && node->asBlock() &&
        node != getEnd())
...
void OMR::CFG::resetVisitCounts(vcount_t count) {
  ...
  if (getStructure()) getStructure()->resetVisitCounts(count);
}
...
bool OMR::CFG::consumePseudoRandomFrequencies() {
  ...
  TR_PseudoRandomNumbersListElement *origNumbers =
      comp()->getPersistentInfo()->getCurPseudoRandomNumbersListElem();
  ...
  for (; nextNode != NULL; nextNode = nextNode->getNext()) {
    ...
    int32_t f = comp()->convertNonDeterministicInput(
        nextNode->getFrequency(), MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, NULL,
        0, false);
    ...
    TR_SuccessorIterator sit(nextNode);
    for (TR::CFGEdge *nextEdge = sit.getFirst(); nextEdge != NULL;
         nextEdge = sit.getNext()) {
      count++;
      nextEdge->setFrequency(comp()->convertNonDeterministicInput(
          nextEdge->getFrequency(), MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT,
          NULL, 0, false));
      edgeIndex++;
    }
  }
  ...
  int32_t f2 = comp()->convertNonDeterministicInput(
      maxFrequency, MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, NULL, 0, false);
  count++;
  if (f2 > getMaxFrequency()) setMaxFrequency(f2);

  int32_t curIndex = comp()->getPersistentInfo()->getCurIndex();
  TR_PseudoRandomNumbersListElement *curNumbers =
      comp()->getPersistentInfo()->getCurPseudoRandomNumbersListElem();
    ...
    if (!self()->consumePseudoRandomFrequencies()) {
      ...
      if (comp()->getOption(TR_TraceBFGeneration))
        comp()->dumpMethodTrees(
            "Trees after setting frequencies from structures",
...
int32_t OMR::CFG::compareExceptionSuccessors(TR::CFGNode *node1,
                                             TR::CFGNode *node2) {
  ...
  if ((node1->getExceptionSuccessors().size() == 1) &&
    ...
    if (node1->getExceptionSuccessors().front() ==
        node2->getExceptionSuccessors().front())
      return BOTH_SAME;
  }
  ...
  TR_BitVector *firstSucc =
      new (trStackMemory()) TR_BitVector(numberOfNodes, trMemory(), stackAlloc);
  TR_BitVector *secondSucc =
      new (trStackMemory()) TR_BitVector(numberOfNodes, trMemory(), stackAlloc);
  TR_BitVector *comparison =
      new (trStackMemory()) TR_BitVector(numberOfNodes, trMemory(), stackAlloc);
  int32_t result = -1;
...
TR::Block **OMR::CFG::createArrayOfBlocks(TR_AllocationKind allocKind) {
  ...
  TR::Block **a = (TR::Block **)trMemory()->allocateMemory(
      numberOfBlocks * sizeof(TR::Block *), allocKind);
...
void OMR::CFG::findLoopingBlocks(TR_BitVector &loopingBlocks) {
  ...
  typedef TR::typed_allocator<std::pair<int32_t const, LoopInfo *>,
                              TR::Region &>
      LoopMapAllocator;
  typedef std::less<int32_t> LoopMapComparator;
  typedef std::map<int32_t, LoopInfo *, LoopMapComparator, LoopMapAllocator>
      LoopMap;
  ...
  for (; block; block = block->getNextBlock()) {
    ...
    if (block->getVisitCount() == blockVisitCount) continue;

    int32_t lowestLoop = 0;  // lowest loop in the table of loops
    int32_t top = 0;
    int32_t loopID = 0;
    stack[top] = block;
    for (i = numberOfBlocks - 1; i >= 0; i--) seenIndex[i] = -1;
    while (top >= 0) {
      ...
      for (TR::CFGEdge *e = pi.getFirst(); e; e = pi.getNext()) {
        ...
        if (trace) dumpOptDetails(comp(), "Predecessor %d\n", n);
        if (seenIndex[n] >= 0) {
          ...
          if (lowestLoop > 0) {
            ...
            // This is done iteratively to avoid removing entries while using a
            // cursor.
            do {
              i = 0;
              for (auto itr = loops.begin(), end = loops.end(); itr != end;
                   ++itr) {
                j = itr->first;
                if (lowestLoop != j) {
            ...
            LoopInfo *info = new (workingSpace.allocate(sizeof(LoopInfo)))
                LoopInfo(workingSpace);
          ...
          // If it is a member of an existing loop whose bottom is still on the
          // stack, that loop can be extended to the current block
...
// generate either a forward or backward reverse post-order traversal of the
// blocks in the CFG The traversal order array is always allocated anew since we
// don't know if the number of blocks in the CFG has changed since the last time
// we allocated the array. This function uses two parallel stacks to accomplish
// a post-order traversal: blockStack and iteratorStack. blockStack holds blocks
// whose out edges (either successors or predecessors depending on the traversal
// direction) haven't yet all been visited.  Blocks are pushed on the stack to
// be visited.  The iteratorStack holds the iterators for the out edges for the
// blocks on the blockStack.
int32_t OMR::CFG::createTraversalOrder(bool forward,
                                       TR_AllocationKind allocationKind,
                                       TR_BitVector *backEdges) {
  static const bool DERIVE_BACKWARD_FROM_FORWARD = true;
  vcount_t traversalVisitCount = comp()->incVisitCount();
  int32_t numberOfBlocks = getNextNodeNumber();

  /*
   * This is allocated before entering the new stack memory region so that the
   * caller can allocate it on its stack scope if desired.
   */
  TR::CFGNode **traversalOrder = (TR::CFGNode **)trMemory()->allocateMemory(
      numberOfBlocks * sizeof(TR::CFGNode *), allocationKind,
  ...
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  TR_Stack<TR::CFGNode *> blockStack(trMemory(), numberOfBlocks, false,
                                     stackAlloc);
  TR_Stack<TR_CFGIterator *> iteratorStack(trMemory(), numberOfBlocks, false,
                                           stackAlloc);
  TR_BitVector *blocksBeingVisited = new (trStackMemory())
      TR_BitVector(numberOfBlocks, trMemory(), stackAlloc);
  ...
  while (!blockStack.isEmpty()) {
    ...
    while (edge) {
      ...
      if (nextBlock->getVisitCount() != traversalVisitCount) {
        ...
        // create the right kind of iterator for it and push it onto
        // iteratorStack
        TR_CFGIterator *blockIter;
        if (DERIVE_BACKWARD_FROM_FORWARD || forward)
        ...
        if (backEdges) backEdges->set(edge->getId());
      }

      edge = iter->getNext();
    }

    /* Push fall thru block to stack last for forward traversal if we want
     * proper order for ebb */
    if ((DERIVE_BACKWARD_FROM_FORWARD && !forward) && !foundUnvisitedBlock &&
        fallThroughEdge) {
      ...
      if (fallThroughBlock->getVisitCount() != traversalVisitCount) {
        ...
        TR_CFGIterator *blockIter =
            new (trStackMemory()) TR_SuccessorIterator(fallThroughBlock);
        blockIter->getFirst();
        iteratorStack.push(blockIter);
        foundUnvisitedBlock = true;
    ...
    if (!foundUnvisitedBlock) {
      // we've visited all of the top block's out edges, so add to the traversal
      // order and remove from the block stack
      TR_ASSERT(traversalInsertPosition > 0,
                "traversed more blocks than CFG claims to have");
  ...
  // TR_ASSERT(traversalInsertPosition == 0, "not all blocks in CFG found by
  // traversal, unreachable blocks should be removed first");
  if (traversalInsertPosition > 0) {
    // there are unreachable blocks so the traversal doesn't start at the
    // beginning of the traversalOrder array shift everything down by the number
    // of empty positions
  ...
  if (forward) {
    ...
    if (DERIVE_BACKWARD_FROM_FORWARD) {
      for (int32_t idx = 0; idx < numberOfBlocks; idx++) {
        if (idx > (numberOfBlocks - 1) - idx) break;
...
TR_BlockCloner *OMR::CFG::clone() {
  ...
  TR_BlockCloner *cloner =
      new (structureRegion()) TR_BlockCloner(self(), false, true);
  TR::Block *clonedBlock =
      cloner->cloneBlocks(comp()->getStartTree()->getNode()->getBlock(),
                          lastTreeTop->getNode()->getBlock());
...
void OMR::CFG::propagateColdInfo(bool generateFrequencies) {
  ...
  if (optName) noPropagateSuperCold = true;
  ...
  for (int b = 0; b < _forwardTraversalLength; b++) {
    ...
    if (!block->asBlock()->isCold() ||
        (haveSuperColdBlocks && !block->asBlock()->isSuperCold() &&
         !block->asBlock()->hasCallToSuperCold())) {
      ...
      if (allPredsCold)
        for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
          // Note : Disabled code below because we do not want to make a loop
          // cold just because there is a dominating block outside the loop that
          // is marked cold
          //
          if (haveSuperColdBlocks &&
              (block->getForwardTraversalIndex() <=
               edge->getFrom()->getForwardTraversalIndex())) {
            if (comp()->getOption(TR_TraceBFGeneration))
    ...
    if (!block->asBlock()->isCold() ||
      ...
      if (allSuccsCold)
        for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
          if (comp()->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp(),
                           "\t\tsuccessor %d coldness %d very coldness %d\n",
                           edge->getTo()->getNumber(),
                           (int)edge->getTo()->asBlock()->isCold(),
          ...
          if (!toBlock->isCold()) {
            ...
            if (curColdSucc) {
              ...
              if (coldFreq == toBlock->getFrequency()) curColdSucc = toBlock;
            } else {
              curColdSucc = toBlock;
...
void OMR::CFG::setUniformEdgeFrequenciesOnNode(TR::CFGNode *node,
                                               int32_t branchToCount,
                                               bool addFrequency,
                                               TR::Compilation *comp) {
  TR::CFG *cfg = self();
  ...
  if (frequencySet) {
    if (!frequencySet->get(block->getNumber())) addFrequency = false;
  }

  int32_t size = node->getSuccessors().size();

  for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end();
       ++e) {
    if (addFrequency)
...
void OMR::CFG::setEdgeFrequenciesOnNode(TR::CFGNode *node,
                                        int32_t branchToCount,
                                        int32_t fallThroughCount,
                                        TR::Compilation *comp) {
  TR::Block *block = node->asBlock();
  TR::Block *branchToBlock = block->getLastRealTreeTop()
                                 ->getNode()
...
bool OMR::CFG::setEdgeFrequenciesFrom() {
  ...
  for (node = getFirstNode(); node; node = node->getNext()) {
    ...
    if (block->getEntry() != NULL && (node->getSuccessors().size() == 2) &&
      ...
      self()->getBranchCounters(ifNode, block, &branchToCount,
                                &fallThroughCount, comp());

      if (fallThroughCount || branchToCount) {
        self()->setEdgeFrequenciesOnNode(node, branchToCount, fallThroughCount,
                                         _compilation);
        ...
        self()->setEdgeFrequenciesOnNode(node, _max_edge_freq, _max_edge_freq,
                                         _compilation);
      }
    } else if (block->getEntry() != NULL &&
               !((node->getSuccessors().size() == 1) ||
                 (node->getSuccessors().size() == 2))) {
      self()->setUniformEdgeFrequenciesOnNode(node, _max_edge_freq, false,
                                              _compilation);
    }
  }
  ...
  for (node = getFirstNode(); node; node = node->getNext()) {
    ...
    if (block->getEntry() != NULL && (node->getSuccessors().size() == 1) &&
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(
            comp(),
            "Edge %p between %d and %d has freq %d (GOTO forward pass)\n", es,
            es->getFrom()->getNumber(), es->getTo()->getNumber(),
            es->getFrequency());
    }
  }
  ...
  for (node = getFirstNode(); node; node = node->getNext()) {
    ...
    for (auto e = block->getPredecessors().begin();
      ...
      if (pred->getEntry() != NULL && (pred->getSuccessors().size() == 1) &&
        ...
        int32_t edgeFrequency = 0;
        for (auto es = block->getSuccessors().begin();
             es != block->getSuccessors().end(); ++es)
        ...
        if (_compilation->getOption(TR_TraceBFGeneration))
          dumpOptDetails(
              comp(),
              "Edge %p between %d and %d has freq %d (GOTO backward pass)\n",
...
void OMR::CFG::computeEntryFactorsFrom(TR_Structure *str, float &factor) {
  ...
  for (node = it.getFirst(); node != NULL; node = it.getNext()) {
    ...
    if (factor > maxSubFactor) maxSubFactor = factor;
  }
...
void OMR::CFG::propagateEntryFactorsFrom(TR_Structure *str, float factor) {
  ...
  if (region == NULL) {
    ...
    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(
          comp(), "Set block frequency on block_%d to %d, current factor %lf\n",
...
float OMR::CFG::computeOutsideEdgeFactor(TR::CFGEdge *outEdge,
                                         TR::CFGNode *pred) {
  float factor = (float)pred->asBlock()->getFrequency() /
                 (float)INITIAL_BLOCK_FREQUENCY_FACTOR;
...
float OMR::CFG::computeInsideEdgeFactor(TR::CFGEdge *inEdge,
                                        TR::CFGNode *pred) {
  float factor = (float)pred->asBlock()->getFrequency() /
                 (float)INITIAL_BLOCK_FREQUENCY_FACTOR;

  // dumpOptDetails(comp(),"For pred %d inEdgeFrequency %d block freq %d factor
  // %f\n", pred->getNumber(), inEdge->getFrequency(),
  // pred->asBlock()->getFrequency(), factor);

  int32_t sumEdgeFreq = 0;
  TR_SuccessorIterator sit(pred);
  for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
    if (edge != inEdge) sumEdgeFreq += edge->getFrequency();
  }
...
void OMR::CFG::computeEntryFactorsAcyclic(TR_RegionStructure *region) {
  ...
  for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
    ...
    if (edge->getFrequency() <= 0) continue;
...
void OMR::CFG::computeEntryFactorsLoop(TR_RegionStructure *region) {
  ...
  for (TR::CFGEdge *edge = pit.getFirst(); edge; edge = pit.getNext()) {
    ...
    isBackEdge = region->contains(pred->asBlock()->getStructureOf(),
                                  this->getStructure());
...
void OMR::CFG::processAcyclicRegion(TR_RegionStructure *region) {
  walkStructure(region);
}

void OMR::CFG::processNaturalLoop(TR_RegionStructure *region) {
  walkStructure(region);
}

static void setFrequencyOnSuccessor(
    TR_RegionStructure *region, TR::CFGNode *succ, TR::CFGNode *start,
    int32_t succNum, int32_t sumEdgeFrequency, int32_t parentFrequency,
    int32_t *_regionFrequencies, ListIterator<TR::CFGEdge> i,
  ...
  if (toStructureSubGraphNode(succ)->getStructure() &&
    ...
    if (succ->getNumber() == start->getNumber()) return;
    ...
    int32_t factoredFrequency =
        (int32_t)((float)parentFrequency * (float)thisEdgeFrequency /
                  (float)sumEdgeFrequency);

    TR::Block *succBlock = NULL;

    if (toStructureSubGraphNode(succ)->getStructure()->asBlock())
      succBlock =
          toStructureSubGraphNode(succ)->getStructure()->asBlock()->getBlock();

    // now set the block frequencies, in reality this should set another field
    // on the block frequency info something like a temporal frequency
    if ((thisEdgeFrequency == max_edge_freq) &&
      ...
      if (succBlock) {
        if (!succBlock->isCold()) {
          ...
          if (_compilation->getOption(TR_TraceBFGeneration))
            dumpOptDetails(_compilation,
                           "Setting frequency of %d on block_%d (to block)\n",
        ...
        if (_compilation->getOption(TR_TraceBFGeneration))
          dumpOptDetails(
              _compilation, "Setting frequency of %d on region %d (to block)\n",
              _regionFrequencies[succ->getNumber()], succ->getNumber());
      }
    } else {
      if (succBlock) {
        if (!succBlock->isCold()) {
          if ((succBlock->getFrequency() + factoredFrequency) >
              INITIAL_BLOCK_FREQUENCY_FACTOR)
            succBlock->setFrequency(INITIAL_BLOCK_FREQUENCY_FACTOR);
          else
            succBlock->setFrequency(succBlock->getFrequency() +
                                    factoredFrequency);
        ...
        if ((_regionFrequencies[succ->getNumber()] + factoredFrequency) >
            INITIAL_BLOCK_FREQUENCY_FACTOR)
          _regionFrequencies[succ->getNumber()] =
              INITIAL_BLOCK_FREQUENCY_FACTOR;
        else
...
static void summarizeEdgeFrequencies(ListIterator<TR::CFGEdge> i,
                                     int32_t *sumEdgeFreq, int32_t *edgeCount) {
  for (TR::CFGEdge *e = i.getFirst(); e; e = i.getNext()) {
...
/* This will replace the function above, once the other callees are using STL
Lists as Argument 1 static void summarizeEdgeFrequencies(TR::CFGEdgeList list,
int32_t *sumEdgeFreq, int32_t *edgeCount)
   {
...
*/
static void addAllInnerBlocksToLoop(TR_RegionStructure *region,
...
static void generalFrequencyPropagation(
    TR_RegionStructure *region, TR_SuccessorIterator sit, TR::Block *block,
    TR_BitVector *_seenNodes, int32_t *_regionFrequencies,
  ...
  for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
    ...
    if (!_seenNodes->isSet(succ->getNumber()) &&
        ...
        toStructureSubGraphNode(succ)->getStructure()->getParent() == region) {
      if (toStructureSubGraphNode(succ)->getStructure()->asBlock()) {
        TR::Block *succBlock = toStructureSubGraphNode(succ)
                                   ->getStructure()
                                   ->asBlock()
                                   ->getBlock();
        if (!succBlock->isCold()) {
          ...
          if (_compilation->getOption(TR_TraceBFGeneration))
            dumpOptDetails(
                _compilation,
                "Setting frequency of %d on block_%d (switch or lookup)\n",
                succBlock->getFrequency(), succBlock->getNumber());
        }
      } else {
        ...
        if (_compilation->getOption(TR_TraceBFGeneration))
          dumpOptDetails(
              _compilation,
              "Setting frequency of %d on region %d (switch or lookup)\n",
              _regionFrequencies[succ->getNumber()], succ->getNumber());
      }
      stack->add(succ);
      if (_compilation->getOption(TR_TraceBFGeneration))
...
void OMR::CFG::walkStructure(TR_RegionStructure *region) {
  ...
  TR_BitVector *_seenNodes = new (trStackMemory())
      TR_BitVector(numBlocks, trMemory(), stackAlloc, notGrowable);

  TR_StructureSubGraphNode *start = region->getEntry();

  TR_BlockStructure *startBlock =
      toStructureSubGraphNode(start)->getStructure()->asBlock();
  ...
  int32_t *_regionFrequencies =
      (int32_t *)trMemory()->allocateStackMemory(numRegions * sizeof(int32_t));
  ...
  while (!stack.isEmpty() && !_foundIllegalStructure) {
    ...
    TR_RegionStructure *regionStr =
        toStructureSubGraphNode(node)->getStructure()->asRegion();
    if (regionStr != NULL) {
      ...
      if (sumEdgeFrequency > 0) {
        for (TR::CFGEdge *edge = sit.getFirst(); edge; edge = sit.getNext()) {
          ...
          setFrequencyOnSuccessor(
              region, succ, start, succNum, sumEdgeFrequency, parentFrequency,
    ...
    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(comp(), "\tVisiting block region %d\n",
                     toStructureSubGraphNode(node)
                         ->getStructure()
                         ->asBlock()
                         ->getNumber());

    TR::Block *block =
        toStructureSubGraphNode(node)->getStructure()->asBlock()->getBlock();
    ...
    if (block->getEntry()) {
      ...
      if ((block->getSuccessors().size() == 2) &&
        ...
        int32_t sumEdgeFrequency = 0;
        int32_t succNum = 0;
        // Need a temporary List to pass into setFrequencyOnSuccessor, and
        // summarizeEdgeFrequencies will be removed as part of the STL List
        // replacement process
        List<TR::CFGEdge> temp(comp()->trMemory());
        for (auto iter = block->getSuccessors().begin();
             iter != block->getSuccessors().end(); ++iter)
          temp.add(*iter);
        ...
        // for GOTO blocks just propagate the block frequency info, edge
        ...
        if (toStructureSubGraphNode(succ)->getStructure() &&
          ...
          if (toStructureSubGraphNode(succ)->getStructure()->asBlock())
            branchToBlock = toStructureSubGraphNode(succ)
                                ->getStructure()
                                ->asBlock()
                                ->getBlock();
          ...
          if (branchToBlock) {
            if (!branchToBlock->isCold()) {
              if ((branchToBlock->getFrequency() + blockFreq) >
                  INITIAL_BLOCK_FREQUENCY_FACTOR) {
                if (_compilation->getOption(TR_TraceBFGeneration))
                  dumpOptDetails(comp(),
                                 "Setting frequency of %d on block_%d (GOTO "
                                 "block) - MAX block frequency\n",
                                 INITIAL_BLOCK_FREQUENCY_FACTOR,
                                 branchToBlock->getNumber());
                branchToBlock->setFrequency(INITIAL_BLOCK_FREQUENCY_FACTOR);
              } else {
                branchToBlock->setFrequency(branchToBlock->getFrequency() +
                                            blockFreq);
                if (_compilation->getOption(TR_TraceBFGeneration))
                  dumpOptDetails(comp(),
                                 "Adding frequency of %d on block_%d (GOTO "
                                 "block), prev frequency was %d\n",
                                 blockFreq, branchToBlock->getNumber(),
                                 branchToBlock->getFrequency() - blockFreq);
              }
            }
          } else {
            if ((_regionFrequencies[succ->getNumber()] + blockFreq) >
                INITIAL_BLOCK_FREQUENCY_FACTOR)
              _regionFrequencies[succ->getNumber()] =
                  INITIAL_BLOCK_FREQUENCY_FACTOR;
            else
            ...
            if (_compilation->getOption(TR_TraceBFGeneration))
              dumpOptDetails(
                  comp(), "Setting frequency of %d on region %d (GOTO block)\n",
                  _regionFrequencies[succ->getNumber()], succ->getNumber());
          }
        }
        ...
        if (_compilation->getOption(TR_TraceBFGeneration))
          dumpOptDetails(_compilation,
                         "Edge %p between %d and %d has freq %d (GOTO)\n", es,
                         es->getFrom()->getNumber(), es->getTo()->getNumber(),
                         es->getFrequency());
      } else {
        generalFrequencyPropagation(region, sit, block, _seenNodes,
                                    _regionFrequencies, &stack, _compilation);
      }
    } else {
      generalFrequencyPropagation(region, sit, block, _seenNodes,
                                  _regionFrequencies, &stack, _compilation);
...
void OMR::CFG::setBlockAndEdgeFrequenciesBasedOnStructure() {
  if (_compilation->getOption(TR_TraceBFGeneration))
    dumpOptDetails(comp(),
                   "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Setting edge "
                   "frequencies...\n");

  bool setBlockFrequencies =
      self()->setEdgeFrequenciesFrom();  // also sets edge probabilities
  ...
  if (setBlockFrequencies) {
    if (_compilation->getOption(TR_TraceBFGeneration))
      dumpOptDetails(
          comp(),
          "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating block and "
          "edge frequencies within regions...\n");
...

    {
      ...
      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(
            comp(),
            "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Computing region "
            "weight factors based on CFG structure, num regions=%d...\n",
            comp()->getFlowGraph()->getNextNodeNumber());

      float maxFactor = 1.0f;
      computeEntryFactorsFrom(getStructure(), maxFactor);
      if (maxFactor > MAX_REGION_FACTOR) maxFactor = MAX_REGION_FACTOR;

      if (_compilation->getOption(TR_TraceBFGeneration))
        dumpOptDetails(
            comp(),
            "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating weight "
            "factors based on CFG structure...\n");

      _maxFrequency =
          (int32_t)((float)maxFactor * INITIAL_BLOCK_FREQUENCY_FACTOR);
...
void OMR::CFG::getBranchCounters(TR::Node *node, TR::Block *block,
                                 int32_t *taken, int32_t *notTaken,
                                 TR::Compilation *comp) {
  TR::Block *branchToBlock =
      node->getBranchDestination()->getNode()->getBlock();
  TR::Block *fallThroughBlock = block->getNextBlock();
  int32_t branchToFrequency = block->getEdge(branchToBlock)->getFrequency();
  int32_t fallThroughFrequency =
      block->getEdge(fallThroughBlock)->getFrequency();

  if (((branchToBlock->getFrequency() >= 0) &&
       (fallThroughBlock->getFrequency() >= 0)) &&
      (((branchToFrequency > 0) && (fallThroughFrequency >= 0)) ||
       ((fallThroughFrequency > 0) && (branchToFrequency >= 0)))) {
    int32_t blockFreq = block->getFrequency();
    if (block->getFrequency() <= 0) blockFreq = 1;

    *taken = branchToFrequency;  //(branchToFrequency*blockFreq)/(branchToFrequency
                                 //+ fallThroughFrequency);
    *notTaken = fallThroughFrequency;  //(fallThroughFrequency*blockFreq)/(branchToFrequency
                                       //+ fallThroughFrequency);

    if (comp->getOption(TR_TraceBFGeneration))
      traceMsg(
          comp,
          "taken %d NOT taken %d branch %d fall through %d  block freq %d\n",
          *taken, *notTaken, branchToFrequency, fallThroughFrequency,
          blockFreq);

    if (*taken > _max_edge_freq) *taken = _max_edge_freq;
    if (*notTaken > _max_edge_freq) *notTaken = _max_edge_freq;

    int32_t rawScalingFactor = _oldMaxEdgeFrequency;
    if (rawScalingFactor < 0) rawScalingFactor = _maxEdgeFrequency;
    ...
    if (rawScalingFactor > 0) {
      if (*taken > MAX_COLD_BLOCK_COUNT) {
        *taken = (rawScalingFactor * (*taken)) /
                 (MAX_COLD_BLOCK_COUNT + MAX_BLOCK_COUNT);
      }

      if (*notTaken > MAX_COLD_BLOCK_COUNT) {
        *notTaken = (rawScalingFactor * (*notTaken)) /
                    (MAX_COLD_BLOCK_COUNT + MAX_BLOCK_COUNT);
      }
    }
    return;
    ...
    *notTaken = fallThroughBlock
    ...
    if (rawScalingFactor < 0) rawScalingFactor = _maxFrequency;

    if (comp->getOption(TR_TraceBFGeneration))
      traceMsg(comp, "raw scaling %d max %d old max %d\n", rawScalingFactor,
               _maxFrequency, _oldMaxFrequency);
      ...
      if (parent->isNaturalLoop()) {
        if (node->getOpCode().isIf()) {
          TR::Block *branchToBlock =
              node->getBranchDestination()->getNode()->getBlock();
          TR::Block *fallThroughBlock = block->getNextBlock();
          bool loopContainsBranchTo =
              parent->contains(branchToBlock->getStructureOf(), getStructure());
          bool loopContainsFallThrough = parent->contains(
              fallThroughBlock->getStructureOf(), getStructure());

          // dumpOptDetails(comp(), "block_%d branchToBlock %d fallThroughBlock
          // %d\n", block->getNumber(), branchToBlock->getNumber(),
  ...
  TR_ASSERT(0,
            "Branch counters can only be set if we have either an external "
            "profiler or structure\n");
}
...
void OMR::CFG::scaleEdgeFrequencies() {
  ...
  for (node = getFirstNode(); node; node = node->getNext()) {
    ...
    if (frequency >= 0) {
      ...
      for (auto e = node->getSuccessors().begin();
           e != node->getSuccessors().end(); ++e) {
      ...
      if (successorFrequency > 0) {
        ...
        for (auto e = node->getSuccessors().begin();
          ...
          // dumpOptDetails("1Edge %p between %d and %d has freq %d
          // (_maxEdgeFrequency %d)\n", e, e->getFrom()->getNumber(),
          // e->getTo()->getNumber(), e->getFrequency(), _maxEdgeFrequency);
        }

        if (hottestEdge->getFrequency() == 0) {
          hottestEdge->setFrequency(frequency);
          if (frequency > _maxEdgeFrequency) _maxEdgeFrequency = frequency;
        }
      }
    }
  }
...
int32_t TR::CFGNode::normalizedFrequency(int32_t rawFrequency,
...
void TR::CFGNode::normalizeFrequency(int32_t frequency, int32_t maxFrequency) {
  if (asBlock()->isCold() && (frequency <= MAX_COLD_BLOCK_COUNT)) return;
...
int32_t TR::CFGNode::denormalizedFrequency(int32_t normalizedFrequency,
                                           int32_t maxFrequency) {
  if (normalizedFrequency <= MAX_COLD_BLOCK_COUNT) return normalizedFrequency;

  // the multiply below can overflow
  // so cast to int64_t
  int32_t frequency =
      (int32_t)((int64_t)((maxFrequency * (int64_t)(normalizedFrequency -
...
void TR::CFGEdge::normalizeFrequency(int32_t maxEdgeFrequency) {
  int32_t frequency = getFrequency();
  // dumpOptDetails("Edge %p between %d and %d has freq %d (_maxEdgeFrequency
  // %d)\n", this, getFrom()->getNumber(), getTo()->getNumber(), getFrequency(),
  // maxEdgeFrequency);
  ...
  setFrequency((frequency * (MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT)) /
               maxEdgeFrequency);
}

void OMR::CFG::normalizeNodeFrequencies(TR_BitVector *nodesToBeNormalized,
                                        TR_Array<TR::CFGEdge *> *edgeArray) {
  if (!nodesToBeNormalized) return;
  int32_t edgeIndex = 0;
  TR::CFGNode *node;
  if (_maxFrequency < 0) {
    for (node = getFirstNode(); node; node = node->getNext()) {
...
void OMR::CFG::normalizeEdgeFrequencies(TR_Array<TR::CFGEdge *> *edgesArray) {
  ...
  if (_maxEdgeFrequency < 0) {
    for (edgeIndex = 0; edgeIndex < _numEdges; edgeIndex++) {
      ...
      if (comp()->getOption(TR_TraceBFGeneration))
        traceMsg(comp(),
                 "11maxEdgeFrequency old %d new %d edge (%d -> %d) %p\n",
                 _maxEdgeFrequency, frequency, edge->getFrom()->getNumber(),
                 edge->getTo()->getNumber(), edge);

      if (frequency > _maxEdgeFrequency) {
        if (comp()->getOption(TR_TraceBFGeneration))
          traceMsg(comp(),
                   "22maxEdgeFrequency old %d new %d edge (%d -> %d) %p\n",
                   _maxEdgeFrequency, frequency, edge->getFrom()->getNumber(),
                   edge->getTo()->getNumber(), edge);
  ...
  if (_maxEdgeFrequency <= 0) return;

  if (_maxFrequency > _maxEdgeFrequency) _maxEdgeFrequency = _maxFrequency;

  // TR_ASSERT((_maxEdgeFrequency > 0), "Max edge frequency must be set by
  // now\n");
  for (edgeIndex = 0; edgeIndex < _numEdges; edgeIndex++) {
    edge = edgesArray->element(edgeIndex);
...
void OMR::CFG::resetFrequencies() {
  ...
  TR_BitVector *nodesToBeReset = new (trStackMemory())
      TR_BitVector(numBlocks, trMemory(), stackAlloc, notGrowable);
...
void OMR::CFG::normalizeFrequencies(TR_BitVector *nodesToBeNormalized) {
  TR::StackMemoryRegion stackMemoryRegion(*trMemory());
  TR_Array<TR::CFGEdge *> *edgesArray = new (trStackMemory())
      TR_Array<TR::CFGEdge *>(trMemory(), _numEdges, true, stackAlloc);
...
void OMR::CFG::findReachableBlocks(TR_BitVector *result) {
  ...
  while (!stack.isEmpty()) {
    ...
    if (!result->get(node->getNumber())) {
...

      {
        ...
        for (TR::CFGEdge *edge = edges.getFirst(); edge;
             edge = edges.getNext()) {
  ...
  // the below code is just a second pass to verify that removing edges from
  // start to the alwayskeepblocks doesn't make the alwayskeepblocks unreachable
  // this is possible when there is a cycle in the CFG
  TR_BitVector reachableBlocksAfterEdgesRemoved(
      getNumberOfNodes(), comp()->trMemory(), stackAlloc, growable);
  ListIterator<TR::CFGEdge> edgesToRemove(&edgesToBeRemoved);
  for (TR::CFGEdge *edge = edgesToRemove.getFirst(); edge;
       edge = edgesToRemove.getNext()) {
    removeEdge(edge);
  }
  stack.push(getStart());
  while (!stack.isEmpty()) {
...
TR::Region &OMR::CFG::getInternalRegion() { return self()->_internalRegion; }
