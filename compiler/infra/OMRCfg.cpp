    ...
    TR::CFGNode* t)
{
    ...
    for (auto e = f->getExceptionSuccessors().begin(); e != f->getExceptionSuccessors().end(); ++e) {
        ...
        if (existingDepth < newDepth || (existingDepth == newDepth && existingCatchBlock->getHandlerIndex() > newCatchBlock->getHandlerIndex()))
            continue;
...
void OMR::CFG::copyExceptionSuccessors(
    TR::CFGNode* from,
    TR::CFGNode* to,
...
static OrphanType unreachableOrphan(TR::CFG* cfg, TR::CFGNode* from, TR::CFGNode* to)
{
    ...
    // to be unreachable.
    //
    if ((to->getPredecessors().empty() && to->getExceptionPredecessors().empty() && to != cfg->getEnd()) || (to->getExceptionPredecessors().empty() && (to->getPredecessors().size() == 1) && to->getPredecessors().front()->getFrom() == to) || (to->getPredecessors().empty() && (to->getExceptionPredecessors().size() == 1) && to->getExceptionPredecessors().front()->getFrom() == to)) {
...
bool OMR::CFG::removeEdge(TR::CFGEdge* edge, bool recursiveImpl)
{
    ...
    //
    if (_removeEdgeNestingDepth > (comp()->getOption(TR_ProcessHugeMethods) ? (MAX_REMOVE_EDGE_NESTING_DEPTH * 2) : MAX_REMOVE_EDGE_NESTING_DEPTH)) {
    ...
    // to be unreachable.
    //
    if ((to->getPredecessors().empty() && to->getExceptionPredecessors().empty() && to != getEnd()) || (to->getPredecessors().empty() && (to->getExceptionPredecessors().size() == 1) && to->getExceptionPredecessors().front()->getFrom() == to) || (to->getExceptionPredecessors().empty() && (to->getPredecessors().size() == 1) && to->getPredecessors().front()->getFrom() == to)) {
        blocksWereRemoved = true;
        removeNode(to);
    }
...
void OMR::CFG::propagateColdInfo(bool generateFrequencies)
{
    ...
    for (int b = 0; b < _forwardTraversalLength; b++) {
        ...
        if (!block->asBlock()->isCold() || (haveSuperColdBlocks && !block->asBlock()->isSuperCold() && !block->asBlock()->hasCallToSuperCold())) {
            ...
            if (allPredsCold && performTransformation(comp(), "%smarked block_%d cold (all preds were cold)\n", OPT_DETAILS, block->getNumber())) {
    ...
    for (int b = 0; b < _backwardTraversalLength; b++) {
        ...
        if (!block->asBlock()->isCold() || (haveSuperColdBlocks && !block->asBlock()->isSuperCold() && !block->asBlock()->hasCallToSuperCold())) {
            ...
            if (allSuccsCold && (!comp()->ilGenTrace() || performTransformation(comp(), "%smarked block_%d cold (all succs were cold)\n", OPT_DETAILS, block->getNumber()))) {
                block->asBlock()->setIsCold();
...
void OMR::CFG::setUniformEdgeFrequenciesOnNode(TR::CFGNode* node, int32_t branchToCount, bool addFrequency, TR::Compilation* comp)
{
    TR::CFG* cfg = self();
...
void OMR::CFG::setEdgeFrequenciesOnNode(TR::CFGNode* node, int32_t branchToCount, int32_t fallThroughCount, TR::Compilation* comp)
{
    TR::Block* block = node->asBlock();
...
bool OMR::CFG::setEdgeFrequenciesFrom()
{
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        if (block->getEntry() != NULL && (node->getSuccessors().size() == 2) && block->getLastRealTreeTop()->getNode()->getOpCode().isBranch()) {
        ...
        } else if (block->getEntry() != NULL && !((node->getSuccessors().size() == 1) || (node->getSuccessors().size() == 2))) {
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        if (block->getEntry() != NULL && (node->getSuccessors().size() == 1) && node->getSuccessors().front()->getTo() && block->hasSuccessor(node->getSuccessors().front()->getTo())) {
    ...
    for (node = getFirstNode(); node; node = node->getNext()) {
        ...
        for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
            ...
            if (pred->getEntry() != NULL && (pred->getSuccessors().size() == 1) && pred->getSuccessors().front()->getTo() && pred->hasSuccessor(pred->getSuccessors().front()->getTo())) {
                int32_t edgeFrequency = 0;
...
{
    if (toStructureSubGraphNode(succ)->getStructure() && toStructureSubGraphNode(succ)->getStructure()->getParent() && toStructureSubGraphNode(succ)->getStructure()->getParent() == region) {
    ...
    TR::Compilation* _compilation)
{
    for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
        ...
        if (!_seenNodes->isSet(succ->getNumber()) && toStructureSubGraphNode(succ)->getStructure() && toStructureSubGraphNode(succ)->getStructure()->getParent() && toStructureSubGraphNode(succ)->getStructure()->getParent() == region) {
            stack->add(succ);
            if (_compilation->getOption(TR_TraceBFGeneration))
...
void OMR::CFG::walkStructure(TR_RegionStructure* region)
{
    ...
    while (!stack.isEmpty() && !_foundIllegalStructure) {
        ...
        if (block->getEntry()) {
            ...
            if ((block->getSuccessors().size() == 2) && block->getLastRealTreeTop()->getNode()->getOpCode().isBranch()) {
                int32_t sumEdgeFrequency = 0;
                int32_t succNum = 0;
                ...
                addAllInnerBlocksToLoop(region, sit, _seenNodes, &stack, _compilation);
            } else if ((block->getSuccessors().size() == 1) && block->getSuccessors().front()->getTo() && block->hasSuccessor(block->getSuccessors().front()->getTo())) {
...
void OMR::CFG::getBranchCounters(TR::Node* node, TR::Block* block, int32_t* taken, int32_t* notTaken, TR::Compilation* comp)
{
    ...
    if (((branchToBlock->getFrequency() >= 0) && (fallThroughBlock->getFrequency() >= 0)) && (((branchToFrequency > 0) && (fallThroughFrequency >= 0)) || ((fallThroughFrequency > 0) && (branchToFrequency >= 0)))) {
        int32_t blockFreq = block->getFrequency();
        ...
        *notTaken = fallThroughFrequency; //(fallThroughFrequency*blockFreq)/(branchToFrequency + fallThroughFrequency);
        ...
        return;
    } else if ((branchToBlock->getPredecessors().size() == 1) && (fallThroughBlock->getPredecessors().size() == 1) && (((branchToBlock->getFrequency() > 0) && (fallThroughBlock->getFrequency() >= 0)) || ((fallThroughBlock->getFrequency() > 0) && (branchToBlock->getFrequency() >= 0)))) {
        int32_t blockFreq = block->getFrequency();
        if (block->getFrequency() <= 0)
            blockFreq = 1;
