...
static void traceAssertionFailure(const char *file, int32_t line,
                                  const char *condition, const char *s,
                                  va_list ap) {
  ...
  // this diagnostic call adds useful info to log file and flushes it, if we're
  // tracing the current method
  if (comp) {
...
namespace TR {
static void OMR_NORETURN va_fatal_assertion(const char *file, int line,
...
void assertion(const char *file, int line, const char *condition,
  ...
  if (comp) {
    ...
    if (comp->getOption(TR_IgnoreAssert)) return;
    // TR_SoftFailOnAssume: on nonfatal assertion failure, cancel the
    // compilation without crashing the process.
