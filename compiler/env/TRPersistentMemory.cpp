...
void *TR_MemoryBase::jitPersistentAlloc(size_t size, ObjectType ot) {
  return ::trPersistentMemory
             ? ::trPersistentMemory->allocatePersistentMemory(size, ot)
             : 0;
}
void TR_MemoryBase::jitPersistentFree(void *mem) {
  ::trPersistentMemory->freePersistentMemory(mem);
}
...
TR_PersistentMemory::TR_PersistentMemory(
    ...
    : TR_MemoryBase(), _signature(MEMINFO_SIGNATURE), _persistentInfo(this),
      _persistentAllocator(TR::ref(persistentAllocator)),
      _totalPersistentAllocations() {}

TR_PersistentMemory::TR_PersistentMemory(
    void *jitConfig, TR::PersistentAllocator &persistentAllocator)
    : TR_MemoryBase(), _signature(MEMINFO_SIGNATURE), _persistentInfo(this),
      _persistentAllocator(TR::ref(persistentAllocator)),
      _totalPersistentAllocations() {}
...
void TR_PersistentMemory::printMemStatsToVlog() {
  ...
  for (uint32_t i = 0; i < TR_MemoryBase::NumObjectTypes; i++) {
    TR_VerboseLog::writeLine(
        TR_Vlog_MEMORY, "\t_totalPersistentAllocations[%s]=%lu", objectName[i],
        (unsigned long)_totalPersistentAllocations[i]);
  }
