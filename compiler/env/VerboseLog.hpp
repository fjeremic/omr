...
/*
 * TR_VerboseLog stores the config of the jit early on, and can use it for
 * printing to the verbose log Each verbose write should have a Tag (see
 * TR_VlogTag for list of options).  The tag is of the form: '\n#TAG:  '
 */
class TR_VerboseLog {
 public:
  // writeLine and write provide multi line write capabilities, and are to be
  // used with vlogAcquire() and vlogRelease(), this ensures nice formatted
  // verbose logs
  static void write(const char* format, ...);
  static void writeLine(TR_VlogTag tag, const char* format, ...);
  // writeLineLocked is a single line print function, it provides the locks for
  // you
 ...
 private:
  static void vwrite(const char* format,
                     va_list args);  // defined in each front end
  static void writeTimeStamp();
  static const char* _vlogTable[];
  static void* _config;  // we will use void * as the different implementations
                         // have different config types
};
