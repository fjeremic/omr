...
OMR::KnownObjectTable::addArrayWithConstantElements(Index index)
{
  ...
  if (!_arrayWithConstantElements)
    _arrayWithConstantElements =
      new (self()->comp()->trHeapMemory()) TR_BitVector(
        self()->getEndIndex(), self()->comp()->trMemory(), heapAlloc, growable);
...
OMR::KnownObjectTable::getIndexAt(uintptrj_t* objectReferenceLocation)
{
  ...
  uintptrj_t objectPointer =
    *objectReferenceLocation; // Note: object references held as uintptrj_t must
                              // never be compressed refs
...
uintptrj_t*
OMR::KnownObjectTable::getfPointerLocationAt(
