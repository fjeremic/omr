...
#define va_copy_end(x) va_end(x)
    ...
    !defined(__C99)  // zOS defines it only if __C99 is defined
...
class TR_FrontEnd : public TR_Uncopyable {
 public:
  ...
  virtual TR_ResolvedMethod *createResolvedMethod(TR_Memory *,
                                                  TR_OpaqueMethodBlock *,
                                                  TR_ResolvedMethod * = 0,
                                                  TR_OpaqueClassBlock * = 0);
  virtual OMR::MethodMetaDataPOD *createMethodMetaData(TR::Compilation *comp) {
    return NULL;
  }

  virtual TR_OpaqueMethodBlock *getMethodFromName(char *className,
                                                  char *methodName,
                                                  char *signature);
  ...
  virtual const char *sampleSignature(TR_OpaqueMethodBlock *aMethod,
                                      char *bug = 0, int32_t bufLen = 0,
                                      TR_Memory *memory = NULL);

  virtual int32_t getLineNumberForMethodAndByteCodeIndex(TR_OpaqueMethodBlock *,
                                                         int32_t);
  ...
  virtual TR_OpaqueMethodBlock *getInlinedCallSiteMethod(
  ...
  virtual TR_YesNoMaybe isInstanceOf(TR_OpaqueClassBlock *instanceClass,
                                     TR_OpaqueClassBlock *castClass,
                                     bool instanceIsFixed,
                                     bool castIsFixed = true,
                                     bool callSiteVettedForAOT = false);
  virtual bool isUnloadAssumptionRequired(TR_OpaqueClassBlock *,
                                          TR_ResolvedMethod *);
  ...
  virtual TR_OpaqueClassBlock *getClassClassPointer(
      TR_OpaqueClassBlock *objectClassPointer);
  virtual TR_OpaqueClassBlock *getClassFromMethodBlock(
      TR_OpaqueMethodBlock *mb);
  virtual int32_t getNewArrayTypeFromClass(TR_OpaqueClassBlock *clazz);

  // VM+Shared
  virtual TR_OpaqueClassBlock *getArrayClassFromComponentClass(
  ...
  virtual TR_OpaqueClassBlock *getComponentClassFromArrayClass(
      TR_OpaqueClassBlock *arrayClass);
  virtual TR_OpaqueClassBlock *getLeafComponentClassFromArrayClass(
      TR_OpaqueClassBlock *arrayClass);

  // Null-terminated.  bufferSize >= 1+getStringUTF8Length(objectPointer).
  // Returns buffer just for convenience.
  virtual char *getStringUTF8(uintptrj_t objectPointer, char *buffer,
  ...
  virtual void reserveTrampolineIfNecessary(TR::Compilation *,
                                            TR::SymbolReference *symRef,
                                            bool inBinaryEncoding);
  virtual intptrj_t methodTrampolineLookup(TR::Compilation *,
                                           TR::SymbolReference *symRef,
                                           void *callSite);
