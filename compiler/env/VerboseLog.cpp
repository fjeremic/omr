...
void
TR_VerboseLog::writeLine(TR_VlogTag tag, const char* format, ...)
{
...
void
TR_VerboseLog::writeLineLocked(TR_VlogTag tag, const char* format, ...)
{
  TR_ASSERT(tag != TR_Vlog_null, "TR_Vlog_null is not a valid Vlog tag");
  vlogAcquire();
  va_list args;
...
void
TR_VerboseLog::write(const char* format, ...)
{
  va_list args;
  va_start(args, format);
  vwrite(format, args);
...
void
TR_VerboseLog::initialize(void* config)
{
  _config = config;
}
