...
#if (TR_TARGET_64BIT && TR_HOST_64BIT) || \
    (TR_HOST_32BIT && TR_TARGET_32BIT) || defined(XCOMPILE)
...
typedef struct TR_ByteCodeInfo {
 ...
 private:
  ...
  uint32_t _isSameReceiver : 1; /* check whether callee has the same receiver as
                                   the caller */
  int32_t _callerIndex : 13; /* index into TR_InlinedCallSite array.  -1 implies
                                outermost function */
  int32_t
      _byteCodeIndex : 17; /* should not be less than jitConfig->bcSizeLimit,
                              currently is 64k (16 bits) */
...
/* Useful between initializing a data structure and making it visible to other
 * threads */
#if !defined(TR_HOST_POWER)
#if defined(TR_HOST_ARM)
// dmb
#define FLUSH_MEMORY(smp) \
  if (smp) __asm__(".word 0xf57ff05f");
#else
#define FLUSH_MEMORY(smp)
#endif
#else
