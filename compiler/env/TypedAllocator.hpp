...
class typed_allocator : public typed_allocator<void, Alloc> {
 public:
  ...
  pointer allocate(
      size_type n,
      typename typed_allocator<void, untyped_allocator>::const_pointer hint =
          0) {
    return static_cast<pointer>(
        typed_allocator<void, untyped_allocator>::allocate(
            n * sizeof(value_type), hint));
  }

  typename typed_allocator<void, untyped_allocator>::value_type deallocate(
      pointer p, size_type n) throw() {
    typed_allocator<void, untyped_allocator>::deallocate(
        p, n * sizeof(value_type));
  }

  static void construct(pointer p, const_reference prototype) {
    ::new (p) value_type(prototype);
  }
