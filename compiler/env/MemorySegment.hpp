...
public:
  ...
  MemorySegment(void *const segment, size_t const size)
      : _segment(segment), _size(size), _allocated(0), _next(this) {}

  MemorySegment(const MemorySegment &other)
      : _segment(other._segment), _size(other._size),
        _allocated(other._allocated), _next(this) {
    TR_ASSERT(_allocated == 0 && other._next == &other,
              "Copying segment descriptor that's in use");
  }
  ...
  void *allocate(size_t bytes) {
    TR_ASSERT(!(_allocated + bytes > _size),
              "Requested allocation would overflow");
  ...
  friend bool operator==(const MemorySegment &left,
                         const MemorySegment &right) {
    return &left == &right;
  }
