...
public:
  ...
  // trees.  This does not imply that it will for this compilation unit: you
  // must ask Compilation::generateArraylets() to answer that.
  //
  ...
  intptrj_t getArrayLengthInElements(TR::Compilation* comp,
                                     uintptrj_t objectPointer);
  uintptrj_t getArrayLengthInBytes(TR::Compilation* comp,
                                   uintptrj_t objectPointer);
  uintptrj_t getArrayElementWidthInBytes(TR::DataType type);
  uintptrj_t getArrayElementWidthInBytes(TR::Compilation* comp,
                                         uintptrj_t objectPointer);
  uintptrj_t decompressReference(TR::Compilation* comp,
  ...
  int64_t maxArraySizeInElementsForAllocation(TR::Node* newArray,
                                              TR::Compilation* comp);
  int64_t maxArraySizeInElements(int32_t knownMinElementSize,
                                 TR::Compilation* comp);
