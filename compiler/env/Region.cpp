...
Region::Region(TR::SegmentProvider &segmentProvider,
               TR::RawAllocator rawAllocator)
    : _bytesAllocated(0), _segmentProvider(segmentProvider),
      _rawAllocator(rawAllocator),
      _initialSegment(_initialSegmentArea.data, INITIAL_SEGMENT_SIZE),
      _currentSegment(TR::ref(_initialSegment)), _lastDestructable(NULL) {}

Region::Region(const Region &prototype)
    : _bytesAllocated(0), _segmentProvider(prototype._segmentProvider),
...
Region::~Region() throw() {
  ...
  TR_ASSERT(_currentSegment.get() == _initialSegment,
            "self-referencial link was broken");
}
...
void *Region::allocate(size_t const size, void *hint) {
  ...
  TR_ASSERT(newSegment.remaining() >= roundedSize,
            "Allocated segment is too small");
