...
class Region {
   ...
   * interface or invoking the destructor of allocated objects of arbitrary
   * type.
   */
  class Destructable {
  public:
    Destructable(Destructable *const prev) : _prev(prev) {}
  ...
  /**
   * @brief A class template allowing the creation of allocation instances of
   * arbitrary type, all of which have a predictable means of accessing their
   * destructor.
   */
  template <typename T> class Instance : public Destructable {
  public:
    Instance(Destructable *prev, const T &value)
        : Destructable(prev), _instance(value) {}
...
public:
   ...
   *
   * @param[in] prototype A copy-constructible instance used to initialize the
   * Region-managed instance.
   *
   * Object instances created in this manner will be destroyed in LIFO order
   * when the owning Region is destroyed.\n NOTE: If, using a region R0, a
   * second Region R1 is instantiated (directly or indirectly) via this
   * mechanism, any objects created in R0 after the instantiation of R1 will
   * have a shorter life span than all the objects created in R1.\n \n Given the
   * context of R0 and R1 above:\n Objects created in R0 before R1 > Objects
   * created in R1 > Objects created in R0 after R1\n \n This is most likely
   * going to be invoked using a temporary:\n
   * ```region.create( MyClass(arg0, ...argN) );```\n
   * Or, if using the default constructor, use extra parentheses to avoid the
   * 'most vexing parse':\n
...
inline void *operator new(size_t size, TR::Region &region) {
  return region.allocate(size);
}
inline void operator delete(void *p, TR::Region &region) {
  region.deallocate(p);
}
...
template <typename T> T &TR::Region::create(const T &value) {
