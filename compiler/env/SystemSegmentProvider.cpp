...
OMR::SystemSegmentProvider::SystemSegmentProvider(size_t segmentSize,
                                                  TR::RawAllocator rawAllocator)
    : TR::SegmentAllocator(segmentSize), _rawAllocator(rawAllocator),
      _currentBytesAllocated(0), _highWaterMark(0),
...
TR::MemorySegment &OMR::SystemSegmentProvider::request(size_t requiredSize) {
  ...
  try {
    ...
    _highWaterMark = _currentBytesAllocated > _highWaterMark
                         ? _currentBytesAllocated
                         : _highWaterMark;
...
size_t OMR::SystemSegmentProvider::bytesAllocated() const throw() {
  return _highWaterMark;
}

size_t OMR::SystemSegmentProvider::regionBytesAllocated() const throw() {
  return _highWaterMark;
}

size_t OMR::SystemSegmentProvider::systemBytesAllocated() const throw() {
  return _highWaterMark;
}

size_t OMR::SystemSegmentProvider::allocationLimit() const throw() {
