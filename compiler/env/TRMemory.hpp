...
T
round_t(T currentValue, size_t remainder, bool roundDown)
{
  if (roundDown) {
...
size_t
alignmentRemainder(uintptr_t allocationSize)
{
...
template<size_t alignment>
size_t
alignmentRemainder(void* allocation)
{
...
template<size_t alignment>
size_t
alignAllocationSize(size_t allocationSize, bool alignDown = false)
{
...
template<size_t alignment>
void*
alignAllocation(void* p, bool alignDown = false)
{
...
inline void*
operator new(size_t size, PERSISTENT_NEW_DECLARE) throw()
{
  return TR_Memory::jitPersistentAlloc(size, TR_MemoryBase::UnknownType);
}

inline void*
operator new(size_t size, TR_HeapMemory heapMemory)
{
  return heapMemory.allocate(size);
}

inline void*
operator new(size_t size, TR_StackMemory stackMemory)
{
...
inline void*
operator new[](size_t size, PERSISTENT_NEW_DECLARE)
{
  return TR_Memory::jitPersistentAlloc(size, TR_MemoryBase::UnknownType);
}
...
#define TR_ALLOC_IMPL(a)                                                       \
  ...
  void* operator new(                                                          \
    size_t s, TR_HeapMemory m, TR_MemoryBase::ObjectType ot = a)               \
  {                                                                            \
    return m.allocate(s, ot);                                                  \
  }                                                                            \
  void operator delete(void* p, TR_HeapMemory m, TR_MemoryBase::ObjectType ot) \
  {                                                                            \
  ...
  void operator delete[](                                                      \
    void* p, TR_HeapMemory m, TR_MemoryBase::ObjectType ot)                    \
  {                                                                            \
    return m.deallocate(p);                                                    \
  }                                                                            \
  void* operator new(                                                          \
    size_t s, TR_StackMemory m, TR_MemoryBase::ObjectType ot = a)              \
  {                                                                            \
    return m.allocate(s, ot);                                                  \
  }                                                                            \
  void operator delete(                                                        \
    void* p, TR_StackMemory m, TR_MemoryBase::ObjectType ot)                   \
  {                                                                            \
    return m.deallocate(p);                                                    \
  }                                                                            \
  void* operator new[](                                                        \
...
template<TR_AllocationKind kind,
         uint32_t minbits,
...
globalAllocator(const char* name = NULL)
{
...
public:
  ...
  public:
    ...
    void _printf()
    {
      printf("%11llu %11llu %11llu %11llu",
             (long long unsigned int)_allocated,
             (long long unsigned int)_freed,
             (long long unsigned int)_maxLive,
...
public:
  ...
  static uint32_t sprintfMetric(char* line,
                                Metric value,
                                Metric total,
                                ...
                                bool csv = false)
  {
    ...
    if (csv) {
      ...
      if (isWithFreed)
        offset +=
          sprintf(line + offset, "%llu", (long long unsigned int)value._freed);
      if (isWithHWM)
        offset += sprintf(
          line + offset, "%llu", (long long unsigned int)value._maxLive);
...
static inline TR::typed_allocator<T, Alloc>
getTypedAllocator(Alloc al)
{
