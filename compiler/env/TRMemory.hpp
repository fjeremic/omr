...
public:
  ...
  {
    return _trPersistentMemory;
  }

  TR_Memory(TR_PersistentMemory&, TR::Region& heapMemoryRegion);

  void setCompilation(TR::Compilation* compilation)
  {
    _comp = compilation;
  }
...
#define TR_ALLOC_IMPL(a)                                                             \
  ...
  void* operator new(size_t s, TR_HeapMemory m, TR_MemoryBase::ObjectType ot = a)    \
  {                                                                                  \
  ...
  void* operator new(size_t s, TR_StackMemory m, TR_MemoryBase::ObjectType ot = a)   \
  {                                                                                  \
    return m.allocate(s, ot);                                                        \
  }                                                                                  \
  void operator delete(void* p, TR_StackMemory m, TR_MemoryBase::ObjectType ot)      \
  {                                                                                  \
    return m.deallocate(p);                                                          \
  }                                                                                  \
  void* operator new[](size_t s, TR_StackMemory m, TR_MemoryBase::ObjectType ot = a) \
  ...
  ostr& stats(ostr& o, allocator& a)
  {
    ...
    for (int i = minbits; i < maxbits; i++) {
      if (statsArray[i - minbits]) {
        ...
        o << "bucket[" << i - minbits << "] (" << bucketsize(i) << ") : allocated = " << statsArray[i - minbits]
