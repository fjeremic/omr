...
public:
  ...
  TRMemoryAllocator<kind, minbits, maxbits> &
  operator=(const TRMemoryAllocator &a) {
    stats_largeAlloc = 0;
    scavenge = a.scavenge;
    memset(&freelist, 0, sizeof(freelist));
    memset(&statsArray, 0, sizeof(freelist));
...
public:
  ...
  static Metric
      _currentMemUsage; // current statistics across all alloc types and memory,
                        // persistent, stack, heap and CS2 allocators
  static uint8_t
...
template <typename T>
static inline TR::typed_allocator<T, TR::Allocator>
getTypedAllocator(TR::Allocator al) {
