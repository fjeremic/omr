...
protected:
  VMAccessCriticalSection(VMAccessAcquireProtocol protocol,
                          TR::Compilation *comp)
      : _protocol(protocol), _acquiredVMAccess(false), _hasVMAccess(false),
        _comp(comp), _omrVMThread(NULL), _vmAccessReleased(false),
  ...
  VMAccessCriticalSection(VMAccessAcquireProtocol protocol,
      ...
      : _protocol(protocol), _acquiredVMAccess(false), _hasVMAccess(false),
        _comp(NULL), _omrVMThread(omrVMThread), _vmAccessReleased(false),
        _initializedBySubClass(false) {
    // Implementation provided by subclass
  }

public:
  VMAccessCriticalSection(TR::Compilation *comp,
                          VMAccessAcquireProtocol protocol)
      : _protocol(protocol), _acquiredVMAccess(false), _hasVMAccess(false),
        _omrVMThread(NULL), _comp(comp), _vmAccessReleased(false),
        _initializedBySubClass(false) {
    switch (protocol) {
    ...
    case tryToAcquireVMAccess:
      _hasVMAccess =
          TR::Compiler->vm.tryToAcquireAccess(comp, &_acquiredVMAccess);
    }
  }

  VMAccessCriticalSection(OMR_VMThread *omrVMThread,
                          VMAccessAcquireProtocol protocol)
      : _protocol(protocol), _acquiredVMAccess(false), _hasVMAccess(false),
        _omrVMThread(omrVMThread), _comp(NULL), _vmAccessReleased(false),
        _initializedBySubClass(false) {
    switch (protocol) {
    case acquireVMAccessIfNeeded:
    ...
    case tryToAcquireVMAccess:
      _hasVMAccess =
          TR::Compiler->vm.tryToAcquireAccess(omrVMThread, &_acquiredVMAccess);
      break;
    }
  }
  ...
  ~VMAccessCriticalSection() {
    ...
    // Do not run this destructor unless it was the OMR class that acquired
    // access
    //
    if (!_initializedBySubClass) {

      // Do not run this destructor if a subclass destructor has already
      // released access
      //
      if (!_vmAccessReleased) {
        if (_comp) {
          ...
          switch (_protocol) {
          case acquireVMAccessIfNeeded:
            TR::Compiler->vm.releaseVMAccessIfNeeded(_omrVMThread,
                                                     _acquiredVMAccess);
            break;
