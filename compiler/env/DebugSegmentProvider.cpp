...
TR::DebugSegmentProvider::DebugSegmentProvider(size_t segmentSize,
                                               TR::RawAllocator rawAllocator)
    : TR::SegmentAllocator(segmentSize), _rawAllocator(rawAllocator),
      _segments(std::less<TR::MemorySegment>(),
                SegmentSetAllocator(rawAllocator)) {}
...
#endif /* (defined(LINUX) && !defined(OMRZTPF)) || defined(__APPLE__) ||       \
          defined(_AIX) */
    _segments.erase(it);
  }
}
...
TR::MemorySegment &TR::DebugSegmentProvider::request(size_t requiredSize) {
  size_t adjustedSize =
      ((requiredSize + (defaultSegmentSize() - 1)) / defaultSegmentSize()) *
      defaultSegmentSize();
#if (defined(LINUX) && !defined(OMRZTPF)) || defined(__APPLE__) || defined(_AIX)
  void *newSegmentArea = mmap(NULL, adjustedSize, PROT_READ | PROT_WRITE,
                              MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
  if (newSegmentArea == MAP_FAILED)
    throw std::bad_alloc();
#elif defined(OMR_OS_WINDOWS)
  void *newSegmentArea = VirtualAlloc(NULL, adjustedSize,
  ...
  try {
    auto result =
        _segments.insert(TR::MemorySegment(newSegmentArea, adjustedSize));
...
#if (defined(LINUX) && !defined(OMRZTPF)) || defined(__APPLE__) || defined(_AIX)
  void *remap = mmap(segment.base(), segment.size(), PROT_NONE,
...
size_t TR::DebugSegmentProvider::bytesAllocated() const throw() {
  return _bytesAllocated;
}

size_t TR::DebugSegmentProvider::regionBytesAllocated() const throw() {
  return _bytesAllocated;
}

size_t TR::DebugSegmentProvider::systemBytesAllocated() const throw() {
  return _bytesAllocated;
}

size_t TR::DebugSegmentProvider::allocationLimit() const throw() {
  return static_cast<size_t>(-1);
}

void TR::DebugSegmentProvider::setAllocationLimit(size_t allocationLimit) {
  return;
}
