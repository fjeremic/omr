  ...
  : _compilation(comp)
{
  ...
  _tableEntries =
    (TR_Array<List<TR_ExceptionTableEntry>>*)comp->trMemory()
      ->allocateHeapMemory(sizeof(TR_Array<List<TR_ExceptionTableEntry>>) * i);
  ...
  for (; tt; tt = catchBlock->getExit()->getNextTreeTop()) {
    ...
    if (!catchBlock->getExceptionPredecessors().empty()) {
      ...
      while (!exceptionPredecessors.empty()) {
        ...
        addSnippetRanges(
          tableEntries, first, catchBlock, catchType, method, comp);
        ...
        for (;;) {
          ...
          first = prevBlock;
          addSnippetRanges(
            tableEntries, prevBlock, catchBlock, catchType, method, comp);
        }
        ...
        for (;;) {
          ...
          last = nextBlock;
          addSnippetRanges(
            tableEntries, nextBlock, catchBlock, catchType, method, comp);
        }
