...
TR_ExceptionTableEntryIterator::TR_ExceptionTableEntryIterator(
    TR::Compilation *comp)
    : _compilation(comp) {
  int32_t i = comp->getMaxInlineDepth() + 1;
  _tableEntries = (TR_Array<List<TR_ExceptionTableEntry> > *)comp->trMemory()
                      ->allocateHeapMemory(
                          sizeof(TR_Array<List<TR_ExceptionTableEntry> >) * i);
  for (int32_t j = 0; j < i; ++j) _tableEntries[j].init(comp->trMemory());
  ...
  for (; tt; tt = catchBlock->getExit()->getNextTreeTop()) {
    ...
    if (!catchBlock->getExceptionPredecessors().empty()) {
      List<TR_ExceptionTableEntry> &tableEntries =
          _tableEntries[catchBlock->getInlineDepth()]
                       [catchBlock->getHandlerIndex()];
      ...
      //
      TR::CFGEdgeList &exceptionPredecessors =
          catchBlock->getExceptionPredecessors();
      while (!exceptionPredecessors.empty()) {
        ...
        addSnippetRanges(tableEntries, first, catchBlock, catchType, method,
                         comp);
        ...
        for (;;) {
          ...
          if (!found) break;
          first = prevBlock;
          addSnippetRanges(tableEntries, prevBlock, catchBlock, catchType,
                           method, comp);
        }
        ...
        for (;;) {
          ...
          if (!found) break;
          last = nextBlock;
          addSnippetRanges(tableEntries, nextBlock, catchBlock, catchType,
                           method, comp);
        }

        TR_ExceptionTableEntry *ete =
            new (comp->trHeapMemory()) TR_ExceptionTableEntry;
        ...
        ete->_instructionHandlerPC =
            catchBlock->getInstructionBoundaries()._startPC;
...
void TR_ExceptionTableEntryIterator::addSnippetRanges(
    List<TR_ExceptionTableEntry> &tableEntries, TR::Block *snippetBlock,
    TR::Block *catchBlock, uint32_t catchType, TR_ResolvedMethod *method,
    TR::Compilation *comp) {
  TR::Block::InstructionBoundaries *ib =
      snippetBlock->getFirstSnippetBoundaries();
...
uint32_t TR_ExceptionTableEntryIterator::size() {
  uint32_t i = 0;
  for (TR_ExceptionTableEntry *e = getFirst(); e; e = getNext()) ++i;
  return i;
}
