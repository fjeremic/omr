...
class OMR_EXTENSIBLE VMEnv {
 public:
  ...
  //
  void releaseVMAccessIfNeeded(TR::Compilation* comp,
                               bool haveAcquiredVMAccess) {}
  void releaseVMAccessIfNeeded(OMR_VMThread* omrVMThread,
                               bool haveAcquiredVMAccess) {}
  ...
  uintptrj_t OSRFrameSizeInBytes(TR::Compilation* comp,
                                 TR_OpaqueMethodBlock* method) {
    return 0;
  }
  bool ensureOSRBufferSize(TR::Compilation* comp,
                           uintptrj_t osrFrameSizeInBytes,
                           uintptrj_t osrScratchBufferSizeInBytes,
                           uintptrj_t osrStackFrameSizeInBytes) {
    return false;
  }
  ...
  /**
   * @brief Returns offset from the current thread to the intermediate result
   * field. The field contains intermediate result from the latest guarded load
   * during concurrent scavenge.
   */
  uintptrj_t thisThreadGetGSIntermediateResultOffset(TR::Compilation* comp) {
    return 0;
  }
  /**
   * @brief Returns offset from the current thread to the flags to check if
   * concurrent scavenge is active
   */
  uintptrj_t thisThreadGetConcurrentScavengeActiveByteAddressOffset(
      TR::Compilation* comp) {
    return 0;
  }
  /**
   * @brief Returns offset from the current thread to the field with the base
   * address of the evacuate memory region
   */
  uintptrj_t thisThreadGetEvacuateBaseAddressOffset(TR::Compilation* comp) {
    return 0;
  }
  /**
   * @brief Returns offset from the current thread to the field with the top
   * address of the evacuate memory region
   */
   ...
   * It contains data from the most recent guarded load during concurrent
   * scavenge
   */
  ...
  /**
   * @brief Returns offset from the current thread to the filed with the read
   * barrier handler address
   */
