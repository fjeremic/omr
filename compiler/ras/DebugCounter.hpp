...
public:
  ...
  bool initializeReloData(TR::Compilation *comp, int32_t delta, int8_t fidelity,
                          int32_t staticDelta);
  void finalizeReloData(TR::Compilation *comp, TR::Node *node,
                        uint8_t seqKind = 0);
...
class DebugCounter : public DebugCounterBase {
  ...
  uint64_t _bumpCountBase; // The last value of bumpCount that was accumulated
                           // into totalCount
  int8_t _fidelity;        // (See the Fidelities enumeration)
  flags8_t _flags;

  enum Flags {
    ContributesToDenominator = 0x01, // Increments to _bumpCount are also added
                                     // to _denominator->_totalCount
    IsDenominator = 0x02,
  };

  DebugCounter(const char *name, int8_t fidelity,
               DebugCounter *denominator = NULL, int8_t flags = 0)
      : DebugCounterBase(name), _fidelity(fidelity), _denominator(denominator),
        _flags(flags), _totalCount(0), _bumpCount(0), _bumpCountBase(0) {
...
public:
  ...
  // Counter names must be either static strings or be allocated in persistent
  // memory; these functions will scan existing counter names and reuse them
  // where possible to avoid unnecessary allocation.
  //
  static const char *debugCounterName(TR::Compilation *comp, const char *format,
                                      ...);
  static const char *debugCounterBucketName(TR::Compilation *comp,
  ...
  //
  static DebugCounter *getDebugCounter(TR::Compilation *comp, const char *name,
                                       int8_t fidelity, int32_t staticDelta);

  static void incStaticDebugCounter(TR::Compilation *comp, const char *name,
                                    int32_t staticDelta = 1) {
    getDebugCounter(comp, name, DebugCounter::Free, staticDelta);
  }

  // Most users of debug counters will want to call this first function, or
  // TR::CodeGenerator::generateDebugCounter.
  //
  static void
  prependDebugCounter(TR::Compilation *comp, const char *name,
                      TR::TreeTop *nextTreeTop, int32_t delta = 1,
                      int8_t fidelity = DebugCounter::Undetermined) {
    prependDebugCounter(comp, name, nextTreeTop, delta, fidelity, delta);
  }
  static void prependDebugCounter(TR::Compilation *comp, const char *name,
                                  TR::TreeTop *nextTreeTop, int32_t delta,
                                  int8_t fidelity, int32_t staticDelta);

  static void
  prependDebugCounter(TR::Compilation *comp, const char *name,
                      TR::TreeTop *nextTreeTop, TR::Node *deltaNode,
                      int8_t fidelity = DebugCounter::Undetermined) {
  ...
  static void prependDebugCounterBump(TR::Compilation *comp,
                                      TR::TreeTop *nextTreeTop,
                                      DebugCounterBase *counter, int32_t delta);
  static void prependDebugCounterBump(TR::Compilation *comp,
                                      TR::TreeTop *nextTreeTop,
                                      DebugCounterBase *counter,
                                      TR::Node *deltaNode);

  static void generateRelocation(TR::Compilation *comp, uint8_t *location,
                                 TR::Node *node, TR::DebugCounterBase *counter);
  static void generateRelocation(TR::Compilation *comp,
  ...
  { Free = 30,        // Time spent maintaining counters is undetectable
    ...
    Undetermined = 0, // The default.  Use another value only if you have reason
                      // to believe it's justified.
  ...
  /** On 32-bit platforms, dereference the address as uint32 pointer, since we
   * use 32-bit operations*/
  int64_t getCount() {
    return TR::Compiler->target.is64Bit()
               ? _totalCount
               : *(reinterpret_cast<uint32_t *>(&_totalCount));
  }
  DebugCounter *getDenominator() { return _denominator; }
  bool isDenominator() { return _flags.testAny(IsDenominator); }
  bool contributesToDenominator() {
    return _flags.testAny(ContributesToDenominator);
  }

  void getInsertionCounterNames(TR::Compilation *comp,
...
class DebugCounterAggregation : public DebugCounterBase {
  ...
  void aggregateDebugCounterInsertions(TR::Compilation *comp,
                                       TR_OpaqueMethodBlock *method,
                                       int32_t bytecodeIndex,
                                       TR::DebugCounter *counter, int32_t delta,
                                       int8_t fidelity, int32_t staticDelta);
  void aggregateDebugCounterHistogram(TR::Compilation *comp,
                                      DebugCounter *counter, int32_t delta,
                                      int8_t fidelity, int32_t staticDelta);

protected:
  DebugCounterAggregation(const char *name, TR_PersistentMemory *mem)
      : DebugCounterBase(name), _mem(mem),
...
public:
  ...
  void aggregateStandardCounters(TR::Compilation *comp, TR::Node *node,
                                 const char *counterName, int32_t delta,
                                 int8_t fidelity, int32_t staticDelta);
  void aggregateStandardCounters(TR::Compilation *comp,
                                 TR_OpaqueMethodBlock *method,
                                 int32_t bytecodeIndex, const char *counterName,
...
class DebugCounterGroup {
  CS2::HashTable<const char *, DebugCounter *, TRPersistentMemoryAllocator>
      _countersHashTable;
  CS2::HashTable<const char *, DebugCounterAggregation *,
                 TRPersistentMemoryAllocator>
      _aggregateCountersHashTable;
  TR_PersistentList<DebugCounter> _counters;
  TR_PersistentList<DebugCounterAggregation> _aggregations;
  DebugCounter *createCounter(const char *name, int8_t fidelity,
                              TR_PersistentMemory *mem);
  DebugCounter *findCounter(const char *name, int32_t nameLength);
  TR::Monitor
      *_countersMutex; /**< Monitor used to synchronize read/write actions to
                          _countersHashTable, otherwise we may have a race */
...
public:
  ...
  const char *counterName(TR::Compilation *comp, const char *format,
                          va_list args);

  DebugCounter *getCounter(
      TR::Compilation *comp, const char *name,
      int8_t fidelity =
          DebugCounter::Undetermined); // Returns NULL if counter is disabled

  DebugCounterAggregation *createAggregation(TR::Compilation *comp,
                                             const char *name);
  DebugCounterAggregation *findAggregation(const char *nameChars,
                                           int32_t nameLength);
  ...
  enum SpecialCharacters {
    ...
    FRACTION_SEPARATOR =
        '/', // Counter has a denominator counter and contributes to it
    NUMERIC_SEPARATOR = '=',   // Remainder of string sorted numerically rather
                               // than alphabetically
    HISTOGRAM_INDICATOR = '#', // Counter is suitable for producing a histogram
    VERBATIM_START = '(', // Start of a 'verbatim' region, within which special
                          // characters will be ignored
    VERBATIM_STOP =
        ')', // End of a 'verbatim' region; verbatim regions can be nested
    LOCATION_POINT =
        '~', // Preferred place to insert code coordinate information
  };
};
