...
// characters are compared e.g. option names are ascii strings that should not
// be compared differently in different locales
...
int
strnicmp_ignore_locale(const char* s1, const char* s2, size_t n);
...
inline int
STRNICMP(char* p1, char* p2, int len)
{
  while ((len--) > 0) {
    char c1 = *p1++, c2 = *p2++;
    int diff = tolower(c1) - tolower(c2);
