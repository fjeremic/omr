...
TR_Debug::TR_Debug(TR::Compilation *c)
  ...
  } else  // still need to initialize other fields that are being initialized
          // inside resetDebugData()
  {
    _nextLabelNumber = 1;
...
void TR_Debug::newNode(TR::Node *node) {
  ...
  if (regex && TR::SimpleRegex::match(regex, buf, false)) breakOn();

  regex = _comp->getOptions()->getDebugOnCreate();
  if (regex && TR::SimpleRegex::match(regex, buf, false)) debugOnCreate();
}
...
void TR_Debug::newInstruction(TR::Instruction *instr) {
  ...
  if (_comp->getAddressEnumerationOption(TR_EnumerateInstruction) ||
    ...
    _comp->getToNumberMap().Add((void *)instr,
                                (intptr_t)_nextInstructionNumber);
...
void TR_Debug::newVariableSizeSymbol(TR::AutomaticSymbol *sym) {
  TR_ASSERT(_comp, "Required compilation object is NULL.\n");
  int32_t strLength =
      strlen(TR_VSS_NAME) + TR::getMaxSignedPrecision<TR::Int32>() + 7;
...
void TR_Debug::addInstructionComment(TR::Instruction *instr, char *comment,
  ...
  if (comment == NULL || _comp->getOutFile() == NULL) return;

  TR::SimpleRegex *regex = _comp->getOptions()->getTraceForCodeMining();
  if (regex && !TR::SimpleRegex::match(regex, comment)) return;
  ...
  if (_comp->getToCommentMap().Locate(instr, hashIndex)) {
    ...
    List<char> *comments =
        new (_comp->trHeapMemory()) List<char>(_comp->trMemory());
    comments->add(comment);
    _comp->getToCommentMap().Add(instr, comments);
  }
}

const char *TR_Debug::getDiagnosticFormat(const char *format, char *buffer,
                                          int32_t length) {
  if (!_comp->getOption(TR_MaskAddresses)) return format;
  ...
  const char *c = format;
  for (; *c; c++, j++) {
    if (j >= length) allowedToWrite = false;
    if (allowedToWrite) buffer[j] = *c;
    ...
    if (*c == '%') {
      ...
      if (*c == 'p') {
        ...
        if (j + slen >= length) allowedToWrite = false;
        if (allowedToWrite) memcpy(&buffer[j], s, slen);
        j += slen - 2;
        sawPercentP = true;
      } else {
        if (j + c - base + 1 >= length) allowedToWrite = false;
        if (allowedToWrite) memcpy(&buffer[j], base, c - base + 1);
        j += c - base;
      }
    }
  }
  ...
  if (j >= length) allowedToWrite = false;
  if (allowedToWrite) buffer[j] = 0;  // null terminate
  j++;

  if (!sawPercentP) return format;
  if (allowedToWrite) return buffer;
...
bool TR_Debug::performTransformationImpl(bool canOmitTransformation,
                                         const char *format, ...) {
  if (_comp->getOptimizer()) _comp->getOptimizer()->incOptMessageIndex();

  const char *string = format;  // without formattedString, we do our best with
                                // the format string itself
  bool alreadyFormatted = false;
  char formatBuffer[200];
  char messageBuffer[300];

  // We try to avoid doing the work of formatting the string if we don't need
  // to.
  //
  if ((_comp->getOption(TR_CountOptTransformations) &&
       _comp->getOptions()->getVerboseOptTransformationsRegex()) ||
      (_file != NULL && canOmitTransformation &&
       _comp->getOptions()->getDisabledOptTransformations())) {
    va_list args;
    va_start(args, format);
    string =
        formattedString(messageBuffer, sizeof(messageBuffer), format, args);
    va_end(args);
  ...
  if (canOmitTransformation) {
    ...
    TR::SimpleRegex *regex =
        _comp->getOptions()->getDisabledOptTransformations();
    if (regex && (TR::SimpleRegex::match(regex, curTransformationIndex) ||
                  TR::SimpleRegex::match(regex, string)))
      return false;

    if (curTransformationIndex <
            _comp->getOptions()->getFirstOptTransformationIndex() ||
        curTransformationIndex >
            _comp->getOptions()->getLastOptTransformationIndex())
      return false;
  ...
  if (canOmitTransformation) {
    if (_registerAssignmentTraceFlags & TRACERA_IN_PROGRESS)
      trfprintf(_file, "\n");  // traceRA convention is a newline at the start
                               // rather than the end

    trfprintf(_file, "[%6d] ", curTransformationIndex);
    if (_comp->getOptIndex() == _comp->getOptions()->getLastOptIndex()) {
      trfprintf(_file, "%3d.%-4d ", _comp->getOptIndex(),
                _comp->getOptSubIndex());
    }

    if ((format[0] == '%' && format[1] == 's') ||
        (format[0] == 'O' && format[1] == '^' && format[2] == 'O')) {
      ;
    } else {
      // prefix the string with O^O so that tooling can easily count
      // transformations this should already be in most transformations but it
      // could get accidentally dropped
  ...
  TR::IO::vfprintf(
      _file, getDiagnosticFormat(format, buffer, sizeof(buffer) / sizeof(char)),
      args);
  va_end(args);
  trfflush(_file);
...
void TR_Debug::vtrace(const char *format, va_list args) {
  if (_file != NULL) {
    char buffer[256];
    TR::IO::vfprintf(_file, getDiagnosticFormat(format, buffer, sizeof(buffer)),
                     args);
    trfflush(_file);
  }
}
...
void TR_Debug::printMethodHotness() {
  ...
  trfprintf(_file, "\nThis method is %s",
            _comp->getHotnessName(_comp->getMethodHotness()));
...
void TR_Debug::printInstrDumpHeader(const char *title) {
  ...
  int prefixWidth =
      addressFieldWidth * 2 + codeByteColumnWidth +
  ...
  if (strcmp(title, "Post Instruction Selection Instructions") == 0 ||
    ...
    trfprintf(_file,
              "\n%*s|       +------------------------------------------ %s",
              addressFieldWidth - 2, " ", lineNumber);
    trfprintf(_file,
              "\n%*s|       |       +----------------------------------------- "
              "instruction",
              addressFieldWidth - 2, " ");
    ...
    trfprintf(
        _file,
        "\n%*s|        | %*s|  |%*s+-------------------------------------- "
        "%sopcode and operands",
        addressFieldWidth - 1, " ", addressFieldWidth, " ",
        codeByteColumnWidth - 2, " ", lineNumber);
    trfprintf(_file,
              "\n%*s|        | %*s|  |%*s|\t\t\t\t+----------- additional "
              "information",
              addressFieldWidth - 1, " ", addressFieldWidth, " ",
...
//
// If no TR_Instruction object is given (instr == NULL), the column is left
// blank.
//
...
//
uint8_t *TR_Debug::printPrefix(TR::FILE *pOutFile, TR::Instruction *instr,
                               uint8_t *cursor, uint8_t size) {
  if (cursor != NULL) {
    ...
    if (_comp->getOption(TR_MaskAddresses)) {
      if (instr)
        sprintf(prefix, "%*s %08x [%s]", addressFieldWidth, "*Masked*", offset,
                getName(instr));
      else
        sprintf(prefix, "%*s %08x %*s", addressFieldWidth, "*Masked*", offset,
                addressFieldWidth + 2, " ");
      ...
      if (instr)
        sprintf(prefix, POINTER_PRINTF_FORMAT " %08x [%s]", cursor, offset,
                getName(instr));
      else
        sprintf(prefix, POINTER_PRINTF_FORMAT " %08x %*s", cursor, offset,
                addressFieldWidth + 2, " ");
    ...
    if (TR::Compiler->target.cpu.isX86()) {
      ...
      for (int i = 0; i < size && p1 - p0 + 9 < prefixWidth;
           i += 4, p1 += 9, cursor += 4)
  ...
  if (regex) {
    ...
    if (TR::SimpleRegex::match(regex, p)) breakOn();
  }

  return cursor;
}

void TR_Debug::printSnippetLabel(TR::FILE *pOutFile, TR::LabelSymbol *label,
                                 uint8_t *cursor, const char *comment1,
                                 const char *comment2) {
  int addressFieldWidth = TR::Compiler->debug.hexAddressFieldWidthInChars();
  int codeByteColumnWidth = TR::Compiler->debug.codeByteColumnWidth();
  int prefixWidth =
  ...
  if (_comp->getOption(TR_MaskAddresses)) {
    trfprintf(pOutFile, "\n\n%*s %08x %*s",
              TR::Compiler->debug.hexAddressFieldWidthInChars(), "*Masked*",
              offset, addressFieldWidth + codeByteColumnWidth + 2, " ");
  } else {
    trfprintf(pOutFile, "\n\n" POINTER_PRINTF_FORMAT " %08x %*s", cursor,
              offset, addressFieldWidth + codeByteColumnWidth + 2, " ");
  }
  ...
  if (comment1) {
    trfprintf(
        ...
        (TR::Compiler->target.cpu.isX86() && TR::Compiler->target.isLinux())
            ...
            : ';',
        comment1);
    if (comment2) trfprintf(pOutFile, " (%s)", comment2);
  }
}
...
const char *TR_Debug::signature(TR::ResolvedMethodSymbol *s) {
  ...
  if (s->getRecognizedMethod() == TR::java_lang_Class_newInstancePrototype)
    return s->getResolvedMethod()->newInstancePrototypeSignature(
        _comp->trMemory());
#endif
...
TR_OpaqueClassBlock *TR_Debug::containingClass(TR::SymbolReference *symRef) {
  ...
  if (method) {
    ...
    return fe()->getClassFromSignature(className, classNameLen,
...
void TR_Debug::nodePrintAllFlags(TR::Node *node,
                                 TR_PrettyPrinterString &output) {
  char *format = "%s";
  ...
  output.append(format,
                node->printIsTheVirtualCallNodeForAGuardedInlinedCall());
  if (!inDebugExtension())
...
void TR_Debug::print(TR::SymbolReference *symRef,
                     TR_PrettyPrinterString &output, bool hideHelperMethodInfo,
                     bool verbose) {
  int32_t displacement = 0;
  uint32_t numSpaces;
  TR_PrettyPrinterString symRefNum(this), symRefOffset(this),
      symRefAddress(this), symRefName(this), symRefKind(this), otherInfo(this),
      symRefWCodeId(this), symRefObjIndex(this), labelSymbol(this);
  ...
  if (sym) {
    if (!inDebugExtension() && _comp->cg()->getMappingAutomatics() &&
  ...
  else if (sym && sym->isFixedObjectRef() && comp()->getKnownObjectTable() &&
           !symRef->isUnresolved()) {
    TR::KnownObjectTable::Index i =
        comp()->getKnownObjectTable()->getExistingIndexAt(
            (uintptrj_t *)sym->castToStaticSymbol()->getStaticAddress());
    if (i != TR::KnownObjectTable::UNKNOWN)
      symRefObjIndex.append(" (==obj%d)", (int)i);
  }

  if (sym) {
    if (symRef->isUnresolved()) symRefKind.append(" unresolved");
    switch (symRef->hasBeenAccessedAtRuntime()) {
      case TR_yes:
        symRefKind.append(" accessed");
        break;
      case TR_no:
        symRefKind.append(" notAccessed");
        break;
      default:
        break;
    }
    if (symRef->getSymbol()->isFinal()) symRefKind.append(" final");
    if (symRef->getSymbol()->isVolatile()) symRefKind.append(" volatile");
    switch (sym->getKind()) {
      ...
      case TR::Symbol::IsMethod: {
        ...
        if (!inDebugExtension()) {
          if (methodSym->isNative()) symRefKind.append(" native");
      ...
      case TR::Symbol::IsShadow:
        if (sym->isNamedShadowSymbol() &&
            sym->getNamedShadowSymbol()->getName() != NULL) {
      ...
      case TR::Symbol::IsMethodMetaData:
        ...
        symRefName.append(
            " %s", symRef->getSymbol()->getMethodMetaDataSymbol()->getName());
        break;
      case TR::Symbol::IsLabel:
        print(sym->castToLabelSymbol(), labelSymbol);
        if (!labelSymbol.isEmpty()) labelSymbol.append(" ");
        break;
      default:
  ...
  numSpaces = getNumSpacesAfterIndex(
      symRef->getReferenceNumber(),
      getIntLength(_comp->getSymRefTab()->baseArray.size()));
  ...
  if (verbose) {
    ...
    if (hideHelperMethodInfo)
      output.append(" %s%s[%s]%s", labelSymbol.getStr(), symRefWCodeId.getStr(),
                    symRefOffset.getStr(), symRefObjIndex.getStr());
    else
      output.append(" %s%s%s[%s%s%s]%s%s", symRefName.getStr(),
                    labelSymbol.getStr(), symRefWCodeId.getStr(),
                    symRefKind.getStr(), symRefOffset.isEmpty() ? "" : " ",
                    symRefOffset.getStr(), symRefObjIndex.getStr(),
                    otherInfo.getStr());
    ...
    if (hideHelperMethodInfo)
      output.append(" %s%s[%s%s%s]%s", labelSymbol.getStr(),
                    symRefWCodeId.getStr(), symRefNum.getStr(),
                    symRefOffset.isEmpty() ? "" : " ", symRefOffset.getStr(),
                    symRefObjIndex.getStr());
    else
      output.append(" %s%s%s[%s%s%s%s%s]%s%s", symRefName.getStr(),
                    labelSymbol.getStr(), symRefWCodeId.getStr(),
                    symRefNum.getStr(), symRefKind.isEmpty() ? "" : " ",
                    symRefKind.getStr(), symRefOffset.isEmpty() ? "" : " ",
                    symRefOffset.getStr(), symRefObjIndex.getStr(),
                    otherInfo.getStr());
  }
...
void TR_Debug::print(TR::LabelSymbol *labelSymbol,
                     TR_PrettyPrinterString &output) {
  output.append("%s", getName(labelSymbol));
}

const char *TR_Debug::getName(void *address, const char *prefix,
                              uint32_t nextNumber, bool enumerate) {
  TR_ASSERT(_comp, "Required compilation object is NULL.\n");
  ...
  if (enumerate) {
    if (!address) {
      char *buf = (char *)_comp->trMemory()->allocateHeapMemory(
          20 + TR::Compiler->debug.pointerPrintfMaxLenInChars());
    ...
    if (_comp->getToStringMap().Locate((void *)address, hashIndex)) {
      ...
      uint8_t indentation =
          TR::Compiler->debug.hexAddressFieldWidthInChars() - 4;
  ...
  char *buf = (char *)_comp->trMemory()->allocateHeapMemory(
      20 + TR::Compiler->debug.pointerPrintfMaxLenInChars());
  if (_comp->getOption(TR_MaskAddresses)) {
    sprintf(buf, "%*s", TR::Compiler->debug.hexAddressFieldWidthInChars(),
            "*Masked*");
    return buf;
  } else {
    if (address)
...
const char *TR_Debug::getVSSName(TR::AutomaticSymbol *sym) {
  ...
  TR_ASSERT(
      false,
      "could not find variable size symbol in the _comp->getToStringMap()");
...
const char *TR_Debug::getName(TR::Symbol *sym) {
  ...
  //
  return getName((void *)sym, "SYM_", _nextSymbolNumber++,
                 _comp->getAddressEnumerationOption(TR_EnumerateSymbol));
}
...
const char *TR_Debug::getName(TR::Instruction *instr) {
  ...
  return getName((void *)instr, "IN1_", 0,
                 _comp->getAddressEnumerationOption(TR_EnumerateInstruction));
}
...
const char *TR_Debug::getName(TR::CFGNode *node) {
  ...
  if (_comp->getAddressEnumerationOption(TR_EnumerateBlock)) {
    ...
    sprintf(buf, "%*s", TR::Compiler->debug.hexAddressWidthInChars(),
            "*Masked*");
  } else {
...
const char *TR_Debug::getName(TR::SymbolReference *symRef) {
  ...
  nonhelperIndex = comp()->getSymRefTab()->getNonhelperIndex(
      comp()->getSymRefTab()->getLastCommonNonhelperSymbol());
  ...
  if (index < nonhelperIndex) {
    if (index >= numHelperSymbols +
                     TR::SymbolReferenceTable::firstArrayShadowSymbol &&
        index < numHelperSymbols +
                    TR::SymbolReferenceTable::firstArrayShadowSymbol +
                    TR::NumTypes)
      return "<array-shadow>";
    if (index >= numHelperSymbols +
      ...
      return getPerCodeCacheHelperName((TR_CCPreLoadedCode)(
          index - numHelperSymbols -
          TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol));
...
void TR_Debug::print(TR::FILE *pOutFile, TR::SymbolReferenceTable *symRefTab) {
  if (pOutFile != NULL && symRefTab->baseArray.size() > 0 &&
      _comp->getOption(TR_TraceAliases)) {
    ...
    for (int32_t i = 0; i < symRefTab->baseArray.size(); i++)
      if (symRefTab->getSymRef(i))
        trfprintf(pOutFile, "  %d[" POINTER_PRINTF_FORMAT "]\n", i,
                  symRefTab->getSymRef(i));
  }
}

void TR_Debug::printAliasInfo(TR::FILE *pOutFile,
                              TR::SymbolReferenceTable *symRefTab) {
  if (pOutFile == NULL) return;

  trfprintf(pOutFile, "\nSymbol References with Aliases:\n\n");
  for (int32_t i = symRefTab->getIndexOfFirstSymRef();
       i < symRefTab->getNumSymRefs(); ++i)
    if (symRefTab->getSymRef(i))
...
void TR_Debug::printAliasInfo(TR::FILE *pOutFile, TR::SymbolReference *symRef) {
  ...
  TR_BitVector *useOnlyAliases =
      symRef->getUseonlyAliasesBV(_comp->getSymRefTab());
...
TR::ResolvedMethodSymbol *TR_Debug::getOwningMethodSymbol(
...
const char *TR_Debug::getAutoName(TR::SymbolReference *symRef) {
  ...
  char *name = (char *)_comp->trMemory()->allocateHeapMemory(
      50 + TR::Compiler->debug.pointerPrintfMaxLenInChars());
  ...
  if (symRef->getSymbol()->isSpillTempAuto()) {
    ...
    if (symRef->getSymbol()->getDataType() == TR::Float ||
      ...
      sprintf(symName, "#FPSPILL%zu_%d", symRef->getSymbol()->getSize(),
              symRef->getReferenceNumber());
    else
      sprintf(symName, "#SPILL%zu_%d", symRef->getSymbol()->getSize(),
              symRef->getReferenceNumber());

    if (_comp->getOption(TR_MaskAddresses))
      sprintf(name, "<%s *Masked*>", symName);
    else
      sprintf(name, "<%s " POINTER_PRINTF_FORMAT ">", symName,
              symRef->getSymbol());
  } else if (symRef->isTempVariableSizeSymRef()) {
    TR_ASSERT(symRef->getSymbol()->isVariableSizeSymbol(),
    ...
    int debugNameLen;
    char *debugName = getOwningMethod(symRef)->localName(
        slot, 0, debugNameLen,
        comp()
            ->trMemory());  // TODO: Proper bcIndex somehow; TODO: proper length
    if (!debugName) {
      debugName = "";
      debugNameLen = 0;
    }
    ...
    if (symRef->getSymbol()->castToAutoSymbol()->isPinningArrayPointer())
      sprintf(name, "%.*s<pinning array auto slot %d>", debugNameLen, debugName,
              slot);
    else if (symRef->getSymbol()->castToAutoSymbol()->holdsMonitoredObject()) {
      if (symRef->holdsMonitoredObjectForSyncMethod())
        sprintf(name, "%.*s<auto slot %d holds monitoredObject syncMethod>",
                debugNameLen, debugName, slot);
      else
        sprintf(name, "%.*s<auto slot %d holds monitoredObject>", debugNameLen,
                debugName, slot);
    } else
    ...
    if (symRef->getSymbol()->castToAutoSymbol()->isInternalPointer())
      ...
      else if (symRef->getSymbol()
                   ->castToAutoSymbol()
                   ->holdsMonitoredObject()) {
        if (symRef->holdsMonitoredObjectForSyncMethod())
          sprintf(name, "<temp slot %d holds monitoredObject syncMethod>",
                  slot);
        else
...
const char *TR_Debug::getParmName(TR::SymbolReference *symRef) {
  ...
  int32_t slot = symRef->getCPIndex();
  const char *s =
      symRef->getSymbol()->castToParmSymbol()->getTypeSignature(signatureLen);
  char *debugName = getOwningMethod(symRef)->localName(
      slot, 0, debugNameLen,
  ...
  if (slot == 0 && !getOwningMethodSymbol(symRef)->isStatic()) {
    buf = (char *)_comp->trMemory()->allocateHeapMemory(debugNameLen +
                                                        signatureLen + 17);
    sprintf(buf, "%.*s<'this' parm %.*s>", debugNameLen, debugName,
            signatureLen, s);
  } else {
    buf = (char *)_comp->trMemory()->allocateHeapMemory(debugNameLen +
                                                        signatureLen + 15);
    sprintf(buf, "%.*s<parm %d %.*s>", debugNameLen, debugName,
            symRef->getCPIndex(), signatureLen, s);
  }
  return buf;
}
...
const char *TR_Debug::getStaticName_ForListing(TR::SymbolReference *symRef) {
  ...
  if (_comp->getSymRefTab()->isConstantAreaSymbol(sym) && sym->isStatic() &&
      sym->isNamed()) {
    return sym->getName();
  }
...
const char *TR_Debug::getStaticName(TR::SymbolReference *symRef) {
  ...
  if (sym->isClassObject()) {
    ...
    if (sym->isAddressOfClassObject()) return "<address of class object>";
    ...
    if (sym->isConstString()) {
      ...
      if (!symRef->isUnresolved() &&
          getStaticNameCriticalSection.acquiredVMAccess()) {
        uintptrj_t stringLocation =
            (uintptrj_t)sym->castToStaticSymbol()->getStaticAddress();
        if (stringLocation) {
          uintptrj_t string =
              comp()->fej9()->getStaticReferenceFieldAtAddress(stringLocation);
          length = comp()->fej9()->getStringUTF8Length(string);
          contents = (char *)comp()->trMemory()->allocateMemory(
          ...
          // Stop before any non-printable characters (like newlines or UTF8
          // weirdness)
          //
          intptrj_t i;
        ...
        char *result =
            (char *)_comp->trMemory()->allocateHeapMemory(length + 20);
        sprintf(result, "<string \"%.*s%s%s\">", prefixLength, contents, etc,
                contents + suffixOffset);
        return result;
      }
#endif
      return "<string>";
    ...
    if (sym->isConstObjectRef()) return "<constant object ref>";

    if (sym->isConst()) return "<constant>";

    return getOwningMethod(symRef)->staticName(symRef->getCPIndex(),
                                               comp()->trMemory());
  }
  ...
  if (sym->isCallSiteTableEntry()) {
    ...
    sprintf(s, "<callSite entry @%d " POINTER_PRINTF_FORMAT ">",
            sym->castToCallSiteTableEntrySymbol()->getCallSiteIndex(),
            staticAddress);
    return s;
  }
  ...
  if (sym->isMethodTypeTableEntry()) {
    ...
    sprintf(s, "<methodType entry @%d " POINTER_PRINTF_FORMAT ">",
            sym->castToMethodTypeTableEntrySymbol()->getMethodTypeIndex(),
            staticAddress);
    return s;
  }
#endif
  ...
  if (staticAddress) {
    char *name = (char *)_comp->trMemory()->allocateHeapMemory(
        TR::Compiler->debug.pointerPrintfMaxLenInChars() + 5);
    if (_comp->getOption(TR_MaskAddresses))
      sprintf(name, "*Masked*");
    else
...
// Note: This array needs to match up with what is in
// compile/SymbolReferenceTable.hpp
...
const char *TR_Debug::getShadowName(TR::SymbolReference *symRef) {
  if (symRef->getCPIndex() >= 0 && !symRef->getSymbol()->isArrayShadowSymbol())
    return getOwningMethod(symRef)->fieldName(symRef->getCPIndex(),
                                              comp()->trMemory());

  if (symRef->getSymbol() ==
      _comp->getSymRefTab()->findGenericIntShadowSymbol()) {
  ...
  if (symRef->getSymbol()->isUnsafeShadowSymbol()) return "<unsafe shadow sym>";
  ...
  if (symRef->getSymbol()) {
    ...
    if (symRef->getSymbol()->isMemoryTypeShadowSymbol()) return "<memory-type>";
  ...
  const int32_t numCommonNonhelperSymbols =
      TR::SymbolReferenceTable::lastCommonNonhelperSymbol -
      TR::SymbolReferenceTable::firstCommonNonhelperNonArrayShadowSymbol -
      TR_numCCPreLoadedCode;
  static_assert(sizeof(commonNonhelperSymbolNames) /
                        sizeof(commonNonhelperSymbolNames[0]) ==
                    numCommonNonhelperSymbols,
                "commonNonhelperSymbolNames array must match "
                "CommonNonhelperSymbol enumeration");

  for (int32_t i =
           TR::SymbolReferenceTable::firstCommonNonhelperNonArrayShadowSymbol;
       i < _comp->getSymRefTab()->getLastCommonNonhelperSymbol(); i++) {
    TR::SymbolReference *other = _comp->getSymRefTab()->element(
        (TR::SymbolReferenceTable::CommonNonhelperSymbol)i);
    if (other && other->getSymbol() == symRef->getSymbol())
      return commonNonhelperSymbolNames
          [i -
           TR::SymbolReferenceTable::firstCommonNonhelperNonArrayShadowSymbol];
  }
    ...
    if (node->getOpCodeValue() == TR::BBStart) {
      ...
      if (block->isCatchBlock()) {
        ...
        if (classNameChars) {
          ...
          trfprintf(pOutFile, " (catches %.*s)", length,
                    getName(classNameChars, length));
        } else {
          classNameChars = "...";
          length = 3;
...
void TR_Debug::saveNodeChecklist(TR_BitVector &saveArea) {
  saveArea = _nodeChecklist;
}

void TR_Debug::restoreNodeChecklist(TR_BitVector &saveArea) {
...
int32_t TR_Debug::dumpLiveRegisters() {
  ...
  for (i = 0; i < NumRegisterKinds; i++) {
    ...
    if (liveRegisters)
      trfprintf(pOutFile, " %s=%d", getRegisterKindName(kind),
                liveRegisters->getNumberOfLiveRegisters());
  }
  ...
  for (i = 0; i < NumRegisterKinds; i++) {
    ...
    TR_LiveRegisters *liveRegisters = _comp->cg()->getLiveRegisters(kind);
    if (liveRegisters) {
      for (TR_LiveRegisterInfo *info = liveRegisters->getFirstLiveRegister();
...
int32_t TR_Debug::dumpLiveRegisters(TR::FILE *pOutFile, TR_RegisterKinds rk) {
  ...
  for (TR_LiveRegisterInfo *p = liveReg->getFirstLiveRegister(); p;
       p = p->getNext()) {
    ...
    if (regPair)
      trfprintf(pOutFile,
                "\t[" POINTER_PRINTF_FORMAT "] %d:  " POINTER_PRINTF_FORMAT
                " pair (" POINTER_PRINTF_FORMAT ", " POINTER_PRINTF_FORMAT
                ")  ",
  ...
  if (n == 0) trfprintf(pOutFile, "\tNo live %s.\n", getRegisterKindName(rk));
...
void TR_Debug::dumpSingleTreeWithInstrs(TR::TreeTop *treeTop,
                                        TR::Instruction *instr, bool dumpTrees,
                                        bool dumpInstrs, bool dumpRefCounts,
                                        bool dumpLiveRegs) {
  TR::FILE *pOutFile = _comp->getOutFile();
  if (pOutFile == NULL) return;
  ...
  if (dumpInstrs) {
    ...
    if (treeTop->getLastInstruction()) {
      ...
      while (cursor) {
        ...
        if (cursor == treeTop->getLastInstruction()) break;
      ...
      while (cursor) {
        ...
        if (outlinedCursor) {
          ...
          if (outlinedCursor != lastOutlinedTarget) {
            // Avoiding dumping the same outlined code sequence more than once
            // by keeping track of the last sequence we dumped Not fool-proof,
            // but cheap
...
void TR_Debug::dumpMethodInstrs(TR::FILE *pOutFile, const char *title,
                                bool dumpTrees, bool header) {
  ...
  if (header) printInstrDumpHeader(title);
  ...
  if (dumpTrees) {
    ...
    trfprintf(
        pOutFile,
        "\n\n============================================================\n");
    for (TR::TreeTop *treeTop = _comp->getStartTree(); treeTop;
         treeTop = treeTop->getNextTreeTop()) {
      printWithFixedPrefix(_comp->getOutFile(), treeTop->getNode(), 1, true,
                           false, " ");
      if (treeTop->getLastInstruction()) {
        trfprintf(pOutFile, "\n------------------------------\n");
        while (instr) {
          print(pOutFile, instr);
          if (instr == treeTop->getLastInstruction()) break;
          instr = instr->getNext();
        }
        instr = instr->getNext();
        trfprintf(pOutFile,
  ...
  while (instr) {
    ...
    if (printLinenos) {
      ...
      // Emit .line directive if there was change in line number, or source file
      // or inlined method state since last time
      if ((crtLineNo != -1) &&
          ((crtLineNo != lastLineNoPrinted) ||
           (inlinedIndex != lastInlinedIndex) ||
           (sourceFileIndex != lastSourceFileIndex) || printEveryLine)) {
        // Following is somewhat PPC specific - line numbers for "primary"
        // source are relative to the first line number of the method. Line
        // numbrers for includes are absoluted . Since we treat Inlined methods
        // (like include files - see previous), such line numbers are absolute
        // as well.
        ...
        if (TR::Compiler->target.cpu.isZ() && TR::Compiler->target.isLinux()) {
          ...
          lno = ((sourceFileIndex == 0) && (inlinedIndex == -1))
                    ? (crtLineNo - bfLineNo + 1)
                    : crtLineNo;
        }

        if (lno > 0)  // this check is workaround for front end problem with
                      // line numbers
        {
          trfprintf(pOutFile, "\n\t.line \t %d", lno);
  ...
  //
#if defined(TR_TARGET_POWER) || defined(TR_TARGET_S390)
  // After RA outlined code is linked to the regular instruction stream and will
  // automatically be printed
  if (_comp->cg()->getCodeGeneratorPhase() <
      TR::CodeGenPhase::RegisterAssigningPhase)
#if defined(TR_TARGET_POWER)
  ...
  if (TR::Compiler->target.cpu.isX86()) printX86OOLSequences(pOutFile);
#endif
...
//
void TR_Debug::printRegisterKilled(TR::Register *reg) {
  TR::FILE *pOutFile = _comp->getOutFile();
  TR::Node *node;
  // many nodes can reference one register, so it is misleading to report just
  // the last one set better to report none. node = reg->isLive() ?
  // reg->getLiveRegisterInfo()->getNode() : NULL;
  node = NULL;

  if (node) {
    trfprintf(pOutFile, " [%s] (%3d)%*s%s   ", getName(node),
              node->getReferenceCount(), _comp->cg()->_indentation, " ",
              getName(node->getOpCode()));
  } else {
    trfprintf(pOutFile, "  %*s       %*s", addressWidth, " ",
...
//
void TR_Debug::printNodeEvaluation(TR::Node *node, const char *relationship,
                                   TR::Register *reg, bool printOpCode) {
  if (!node) return;
  TR::FILE *pOutFile = _comp->getOutFile();
  trfprintf(pOutFile, " [%s] (%3d)%*s%s%s%s%s\n", getName(node),
            node->getReferenceCount(), _comp->cg()->_indentation, " ",
            (printOpCode) ? getName(node->getOpCode()) : "", relationship,
...
void TR_Debug::dumpMixedModeDisassembly() {
  ...
  for (; inst; inst = inst->getNext()) {
    if (inst->getNode() != NULL &&
        (!n || ((inst->getNode()->getByteCodeInfo().getCallerIndex() !=
                     n->getByteCodeInfo().getCallerIndex() ||
      ...
      trfprintf(pOutFile, "\n\n");
      char *indent = (char *)_comp->trMemory()->allocateHeapMemory(
          6 + 3 * (_comp->getMaxInlineDepth() + 1));
      printByteCodeStack(n->getInlinedSiteIndex(), n->getByteCodeIndex(),
                         indent);
    }
...
void TR_Debug::dumpInstructionComments(TR::FILE *pOutFile,
  ...
  if (_comp->getToCommentMap().Locate((void *)instr, hashIndex)) {
    char *data;
    List<char> *comments =
        (List<char> *)_comp->getToCommentMap().DataAt(hashIndex);
    ...
    // Dump out all comments added to the instruction _comp->getToCommentMap()
    // hashtable
...
void TR_Debug::printCommonDataMiningAnnotations(TR::FILE *pOutFile,
                                                TR::Instruction *inst,
                                                bool needsStartComment) {
  if (inst != NULL && inst->getNode()) {
    ...
    TR::SimpleRegex *regex = _comp->getOptions()->getTraceForCodeMining();
    if (regex && (TR::SimpleRegex::match(regex, "ALL") ||
                  TR::SimpleRegex::match(regex, IL_KEY) ||
                  TR::SimpleRegex::match(regex, FRQ_KEY) ||
                  TR::SimpleRegex::match(regex, CLD_KEY))) {
    ...
    !defined(TR_TARGET_ARM64)
void TR_Debug::print(TR::FILE *pOutFile, TR::Instruction *inst) {
...
void TR_Debug::print(TR::FILE *pOutFile, TR::list<TR::Snippet *> &snippetList,
  ...
  for (auto snippets = snippetList.begin(); snippets != snippetList.end();
       ++snippets) {
    print(pOutFile, *snippets);
  }

  if (_comp->cg()->hasDataSnippets()) _comp->cg()->dumpDataSnippets(pOutFile);
}
...
void TR_Debug::print(TR::FILE *pOutFile, List<TR::Snippet> &snippetList,
  ...
  for (TR::Snippet *snippet = snippets.getFirst(); snippet;
       snippet = snippets.getNext()) {
...
#if defined(TR_TARGET_X86)
  if (TR::Compiler->target.cpu.isX86()) return getNamex(snippet);
#endif
#if defined(TR_TARGET_ARM)
  if (TR::Compiler->target.cpu.isARM()) return getNamea(snippet);
#endif
#if defined(TR_TARGET_ARM64)
  if (TR::Compiler->target.cpu.isARM64()) return getNamea64(snippet);
#endif
...
// need to add 1 to regNum because the enum has 0 = NoReg, and we want 0 to =
// GPR0
#if defined(TR_TARGET_X86)
...
const char *TR_Debug::getName(TR::Register *reg, TR_RegisterSizes size) {
...
#endif
    TR_ASSERT(0,
              "TR_Debug::getName() ==> unknown target platform for given real "
              "register\n");
  }

  if (_comp->getAddressEnumerationOption(TR_EnumerateRegister) &&
      reg == _comp->cg()->getVMThreadRegister())
    return "GPR_0000";

  const int32_t maxPrefixSize = 4;
  char prefix[maxPrefixSize + 1];
  if (reg->getRegisterPair()) {
    // Prefix checking for register pairs is a bit complex, and likely
    // irrelevant since they can't contain collected refs or internal pointers
  ...
  if (_comp->getToStringMap().Locate((void *)reg, hashIndex)) {
    ...
    if (!strncmp(name, prefix, strlen(prefix))) return name;
    //
  ...
  if (reg->getRegisterPair()) {
    ...
    char *buf = (char *)_comp->trMemory()->allocateHeapMemory(strlen(high) +
                                                              strlen(low) + 2);
    ...
    char *buf = (char *)_comp->trMemory()->allocateHeapMemory(
        maxPrefixSize + 6 + 11);  // max register kind name size plus underscore
                                  // plus 10-digit reg num plus null terminator
    uint32_t regNum =
        (uint32_t)(intptrj_t)_comp->getToNumberMap().DataAt(hashIndex);
    sprintf(buf, "%s%s_%04d", prefix, getRegisterKindName(reg->getKind()),
            regNum);
    _comp->getToStringMap().Add((void *)reg, buf);
    return buf;
  } else {
    char *buf = (char *)_comp->trMemory()->allocateHeapMemory(
        maxPrefixSize + 6 + TR::Compiler->debug.pointerPrintfMaxLenInChars() +
        1);
    if (_comp->getOption(TR_MaskAddresses))
      sprintf(buf, "%s%s_*Masked*", prefix,
              getRegisterKindName(reg->getKind()));
    else
      sprintf(buf, "%s%s_" POINTER_PRINTF_FORMAT, prefix,
              getRegisterKindName(reg->getKind()), reg);
    _comp->getToStringMap().Add((void *)reg, buf);
    return buf;
  }
}

const char *TR_Debug::getGlobalRegisterName(TR_GlobalRegisterNumber regNum,
                                            TR_RegisterSizes size) {
  uint32_t realRegNum = _comp->cg()->getGlobalRegister(regNum);
#if defined(TR_TARGET_X86)
  if (TR::Compiler->target.cpu.isX86()) return getName(realRegNum, size);
#endif
#if defined(TR_TARGET_POWER)
  if (TR::Compiler->target.cpu.isPower()) return getPPCRegisterName(realRegNum);
#endif
#if defined(TR_TARGET_S390)
  if (TR::Compiler->target.cpu.isZ()) return getS390RegisterName(realRegNum);
#endif
#if defined(TR_TARGET_ARM)
  if (TR::Compiler->target.cpu.isARM()) return getName(realRegNum, size);
#endif
  return "???";
}
...
void TR_Debug::printRegisterMask(TR::FILE *pOutFile, TR_RegisterMask mask,
                                 TR_RegisterKinds rk) {
  if (pOutFile == NULL) return;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::Register *reg,
                     TR_RegisterSizes size) {
  if (pOutFile == NULL) return;
#if defined(TR_TARGET_S390)
...
void TR_Debug::printFullRegInfo(TR::FILE *pOutFile, TR::Register *reg) {
  ...
  static const char *ignoreFreeRegs =
      feGetEnv("TR_ignoreFreeRegsDuringTraceRA");
  static const char *ignoreFreeAndLockedRegs =
      feGetEnv("TR_ignoreFreeAndLockedRegsDuringTraceRA");
  ...
  if (reg->getRealRegister()) {
    ...
    static const char *stateNames[5] = {"Free", "Unlatched", "Assigned",
                                        "Blocked", "Locked"};
    ...
    if (reg->getRealRegister()->getState() == TR::RealRegister::Assigned) {
      ...
      TR_ASSERT(virtReg,
                "real register is assigned to an unknown virtual register");
      if (virtReg) {
        TR_ASSERT(reg->getRealRegister()->getHasBeenAssignedInMethod(),
                  "Register state is assigned but register has never been "
                  "assigned in method");
      }
      trfprintf(pOutFile, "[ %-10s ]", getName(virtReg));
      trfprintf(pOutFile, "[%5d/%5d]", virtReg->getFutureUseCount(),
      ...
      trfprintf(pOutFile, "[ %-10s ]",
                stateNames[reg->getRealRegister()->getState()]);
    ...
    else if (reg->getFutureUseCount() != 0 &&
    ...
    trfprintf(pOutFile, " ][%5d][%5d]\n", reg->getTotalUseCount(),
              reg->getFutureUseCount());
  }
}

void TR_Debug::printGPRegisterStatus(TR::FILE *pOutFile,
                                     OMR::MachineConnector *machine) {
  if (pOutFile == NULL) return;
#if defined(TR_TARGET_S390)
  if (TR::Compiler->target.cpu.isZ()) {
...
void TR_Debug::printFPRegisterStatus(TR::FILE *pOutFile,
                                     OMR::MachineConnector *machine) {
  if (pOutFile == NULL) return;
#if defined(TR_TARGET_S390)
  if (TR::Compiler->target.cpu.isZ()) {
...
const char *TR_Debug::toString(TR_RematerializationInfo *info) {
  if (info->isRematerializableFromConstant()) return "constant load";
...
void TR_Debug::print(TR::FILE *pOutFile, TR::RegisterMappedSymbol *localCursor,
                     bool isSpillTemp) {
  trfprintf(pOutFile,
            "  Local [%s] (GC map index : %3d, Offset : %3d, Size : %d) is an ",
            getName(localCursor), localCursor->getGCMapIndex(),
            localCursor->getOffset(), localCursor->getSize());
  ...
  if (isSpillTemp) trfprintf(pOutFile, "spill ");

  if (localCursor->isLocalObject()) trfprintf(pOutFile, "local object ");
...
void TR_Debug::print(TR::FILE *pOutFile, uint8_t *instrStart,
                     uint8_t *instrEnd) {}
...
void TR_Debug::print(TR::FILE *pOutFile, TR::GCStackAtlas *atlas) {
  ...
  for (parmCursor = parameterIterator.getFirst(); parmCursor;
       parmCursor = parameterIterator.getNext())
    print(pOutFile, parmCursor, false);

  ListIterator<TR::AutomaticSymbol> automaticIterator(
      &methodSymbol->getAutomaticList());
  TR::AutomaticSymbol *localCursor;
  for (localCursor = automaticIterator.getFirst(); localCursor;
       localCursor = automaticIterator.getNext())
    print(pOutFile, localCursor, false);

  // Print the map for spill temps
  //
  for (auto temps = _comp->cg()->getCollectedSpillList().begin();
       temps != _comp->cg()->getCollectedSpillList().end(); ++temps) {
    TR::AutomaticSymbol *s =
        (*temps)->getSymbolReference()->getSymbol()->getAutoSymbol();
  ...
  if (internalPtrMap) {
    ...
    ListElement<TR_InternalPointerPair> *currElement =
        internalPtrMap->getInternalPointerPairs().getListHead();
    for (; currElement; currElement = currElement->getNextElement()) {
      ...
      trfprintf(pOutFile,
                "    Base array index : %d Internal pointer index : %d\n", a,
                b);
    }
  }
  ...
  if (!atlas->getPinningArrayPtrsForInternalPtrRegs().isEmpty()) {
    int32_t indexOfFirstInternalPtr = atlas->getIndexOfFirstInternalPointer();
    ListElement<TR::AutomaticSymbol> *currElement =
        atlas->getPinningArrayPtrsForInternalPtrRegs().getListHead();
    for (; currElement; currElement = currElement->getNextElement())
      trfprintf(
          pOutFile,
          "    Base array index : %d pins internal pointers only in regs\n",
  ...
  if (atlas->getStackAllocMap()) {
    trfprintf(pOutFile, "Stack alloc map size : %d ",
              atlas->getStackAllocMap()->getMapSizeInBytes());
...
void TR_Debug::print(TR::FILE *pOutFile, TR_GCStackMap *map,
                     TR::GCStackAtlas *atlas) {
  if (pOutFile == NULL) return;

  trfprintf(pOutFile, "\n  Code offset range starts at [%08x]",
            map->_lowestCodeOffset);
  trfprintf(pOutFile, "\n  GC stack map information : ");
  trfprintf(pOutFile, "\n    number of stack slots mapped = %d",
            map->_numberOfSlotsMapped);
  ...
  if (internalPtrMap) {
    ...
    for (; currElement; currElement = currElement->getNextElement())
      trfprintf(
          pOutFile,
          "      pinning array GC stack map index = %d Internal pointer regnum "
          "= %d\n",
...
void TR_Debug::dump(TR::FILE *pOutFile, TR_CHTable *chTable) {
  ...
  if (!chTable->_preXMethods && !chTable->_classes && vguards.empty()) return;

  trfprintf(pOutFile,
            "                       Class Hierarchy Assumption Table\n");
  trfprintf(pOutFile,
            "------------------------------------------------------------------"
            "----------------------\n");

  if (!inDebugExtension() && !vguards.empty()) {
    ...
    for (auto info = vguards.begin(); info != vguards.end(); ++info, ++i) {
      ...
      sprintf(guardKindName, "%s %s%s",
              getVirtualGuardKindName((*info)->getKind()),
              (*info)->mergedWithHCRGuard() ? "+ HCRGuard " : "",
              (*info)->mergedWithOSRGuard() ? "+ OSRGuard " : "");

      if (!(*info)->getSymbolReference())
        trfprintf(pOutFile, "[%4d] %-49s %s%s\n", i, guardKindName,
                  (*info)->isInlineGuard() ? "inlined " : "", guardKindName);
      else
        trfprintf(pOutFile,
                  "[%4d] %-49s %scalleeSymbol=" POINTER_PRINTF_FORMAT "\n", i,
                  guardKindName, (*info)->isInlineGuard() ? "inlined " : "",
                  (*info)->getSymbolReference()->getSymbol());
      ListIterator<TR_VirtualGuardSite> siteIt(&(*info)->getNOPSites());
      for (TR_VirtualGuardSite *site = siteIt.getFirst(); site;
           site = siteIt.getNext()) {
        uint8_t *loc = site->getLocation();
        uint8_t *&dest = site->getDestination();

        trfprintf(pOutFile,
                  "\tSite: location=" POINTER_PRINTF_FORMAT
                  " (e+%5x) branch-dest=" POINTER_PRINTF_FORMAT " (e+%5x)\n",
                  loc, loc - startPC, dest, dest - startPC);
      }
      ListIterator<TR_InnerAssumption> innerIt(&(*info)->getInnerAssumptions());
      for (TR_InnerAssumption *inner = innerIt.getFirst(); inner;
           inner = innerIt.getNext()) {
        trfprintf(pOutFile,
                  "\tInner Assumption: calleeSymbol=" POINTER_PRINTF_FORMAT
                  " for parm ordinal=%d\n",
                  inner->_guard->getSymbolReference()->getSymbol(),
                  inner->_ordinal);
      }
    }
  }
...
const char *TR_Debug::getRuntimeHelperName(int32_t index) {
  ...
  if (index < TR_FSRH) {
    switch (index) {
      ...
      case TR_referenceArrayCopy:
        return "jitReferenceArrayCopy";
            ...
            TR::Compiler->target.cpu.isAMD64()) &&
           !inDebugExtension()) {
    if (index < TR_LXRH) {
      switch (index) {
        ...
        case TR_X86updateInterpreterDispatchGlueSite:
          return "updateInterpreterDispatchGlueSite";
        ...
        case TR_X86PatchSingleComparePIC_mov:
          return "jitX86PatchSingleComparePIC_mov";
        case TR_X86PatchSingleComparePIC_je:
          return "jitX86PatchSingleComparePIC_je";
        case TR_X86PatchMultipleComparePIC_mov:
          return "jitX86PatchMultipleComparePIC_mov";
        case TR_X86PatchMultipleComparePIC_je:
          return "jitX86PatchMultipleComparePIC_je";
        case TR_X86CodeCachePrefetchHelper:
          return "per code cache TLH prefetch helper";

        case TR_outlinedPrologue_0preserved:
          return "outlinedPrologue_0preserved";
        case TR_outlinedPrologue_1preserved:
          return "outlinedPrologue_1preserved";
        case TR_outlinedPrologue_2preserved:
          return "outlinedPrologue_2preserved";
        case TR_outlinedPrologue_3preserved:
          return "outlinedPrologue_3preserved";
        case TR_outlinedPrologue_4preserved:
          return "outlinedPrologue_4preserved";
        case TR_outlinedPrologue_5preserved:
          return "outlinedPrologue_5preserved";
        case TR_outlinedPrologue_6preserved:
          return "outlinedPrologue_6preserved";
        case TR_outlinedPrologue_7preserved:
          return "outlinedPrologue_7preserved";
        case TR_outlinedPrologue_8preserved:
          return "outlinedPrologue_8preserved";
      }
    } else if (TR::Compiler->target.cpu.isI386()) {
      switch (index) {
    ...
    switch (index) {
      ...
      case TR_PPCforwardLEArrayCopy:
        return "__forwardLEArrayCopy";
      case TR_PPCLEArrayCopy:
        return "__leArrayCopy";
      ...
      case TR_PPCarrayTranslateTRTOSimpleVMX:
        return "__arrayTranslateTRTOSimpleVMX";
      case TR_PPCarrayCmpVMX:
        return "__arrayCmpVMX";
      case TR_PPCarrayCmpLenVMX:
        return "__arrayCmpLenVMX";
      case TR_PPCarrayCmpScalar:
        return "__arrayCmpScalar";
      case TR_PPCarrayCmpLenScalar:
        return "__arrayCmpLenScalar";
    ...
    switch (index) {
      ...
      case TR_S390interpreterUnresolvedSpecialGlue:
        return "_interpreterUnresolvedSpecialGlue";
      case TR_S390interpreterUnresolvedDirectVirtualGlue:
        return "_interpreterUnresolvedDirectVirtualGlue";
      ...
      case TR_S390icallVMprJavaSendVirtualD:
        return "icallVMprJavaSendVirtualD";
      case TR_S390interpreterVoidStaticGlue:
        return "_interpreterVoidStaticGlue";
      case TR_S390interpreterIntStaticGlue:
        return "_interpreterIntStaticGlue";
      case TR_S390interpreterLongStaticGlue:
        return "_interpreterLongStaticGlue";
      case TR_S390interpreterFloatStaticGlue:
        return "_interpreterFloatStaticGlue";
      case TR_S390interpreterDoubleStaticGlue:
        return "_interpreterDoubleStaticGlue";
      case TR_S390interpreterSyncVoidStaticGlue:
        return "_interpreterSyncVoidStaticGlue";
      case TR_S390interpreterSyncIntStaticGlue:
        return "_interpreterSyncIntStaticGlue";
      case TR_S390interpreterSyncLongStaticGlue:
        return "_interpreterSyncLongStaticGlue";
      case TR_S390interpreterSyncFloatStaticGlue:
        return "_interpreterSyncFloatStaticGlue";
      case TR_S390interpreterSyncDoubleStaticGlue:
  ...
  if (inDebugExtension()) return "platform specific - not implemented";
...
const char *TR_Debug::getVirtualGuardTestTypeName(
...
void TR_Debug::dumpSimulatedNode(TR::Node *node, char tagChar) {
  ...
  if (_comp->cg()->simulatedNodeState(node)._willBeRematerialized) {
    ...
    trfprintf(_file, "%2d/%-2d", node->getLocalIndex(),
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (node->getType().isIntegral()) {
      ...
      //                   break;
      //                default:
      //                   TR_ASSERT(0, "Unsigned integral type should be one of
      //                   the above"); value = limit+1; break;
      //                }
      //             if (value <= limit)
      //                {
      //                trfprintf(_file, unsignedFormatString,
      //                getName(node->getOpCode()), value);
      //                }
      //             else
      //                {
      //                trfprintf(_file, opCodeOnlyFormatString,
      //                getName(node->getOpCode()));
      //                }
      //             }
      //          else
      ...
      trfprintf(_file, "%s %-8g", getName(node->getOpCode()),
                node->getDouble());
      padding -= 9;
...
void TR_Debug::dumpGlobalRegisterTable() {
  ...
  for (int32_t i = 0; i < _comp->cg()->getNumberOfGlobalRegisters(); i++) {
    trfprintf(_file, "   %d: %s\n", i,
              getGlobalRegisterName((TR_GlobalRegisterNumber)i));
  }
}

void TR_Debug::startTracingRegisterAssignment(const char *direction,
  ...
  trfprintf(_file,
            "<legend>\n"
            "  V(F/T)   virtual register V with future use count F and total "
            "use count T\n"
            ...
            "  V{I,D}?  considering V with association index I and "
            "interference distance D\n"
            "</legend>\n");
  trfflush(_file);
...
void TR_Debug::stopTracingRegisterAssignment() {
  ...
  if (!_comp->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRABasic))
    return;
  if (_registerAssignmentTraceCursor) trfprintf(_file, "\n");
  ...
  for (const char *c = format; *c; c++, j++) {
    if (*c == '%' && *(c + 1) == 'R') {
      ...
      if (j + slen >= length)  // re-allocate buffer if too small
      {
        length += 40;
        char *newBuffer =
            (char *)_comp->trMemory()->allocateHeapMemory(length + 1);
...
void TR_Debug::traceRegisterAssignment(TR::Instruction *instr,
                                       bool insertedByRA, bool postRA) {
  ...
  if (insertedByRA)
    ...
    _registerAssignmentTraceFlags &= ~TRACERA_INSTRUCTION_INSERTED;
  else if (!_comp->getOptions()->getRegisterAssignmentTraceOption(
               TR_TraceRAPreAssignmentInstruction))
    return;
  print(_file, instr);
  if (_registerAssignmentTraceCursor) {
    trfprintf(_file, "\n");
    _registerAssignmentTraceCursor = 0;
    if (postRA) {
      if (_comp->getOptions()->getRegisterAssignmentTraceOption(
...
/* Work in progress side-by side traceRA={*} */
        ...
        if (_registerKindsToAssign & TR_GPR_Mask) {
          ...
          for (TR::Register *gpr = iter->getFirst(); gpr;
               gpr = iter->getNext()) {
...
#if defined(TR_TARGET_S390)
        if (_registerKindsToAssign & TR_VRF_Mask &&
            _cg->getSupportsVectorRegisters()) {
          trfprintf(_file, "<vrfs>\n");
          TR::RegisterIterator *iter = _cg->getVRFRegisterIterator();
          for (TR::Register *vrf = iter->getFirst(); vrf;
               vrf = iter->getNext()) {
        ...
        if (_registerKindsToAssign & TR_FPR_Mask) {
          ...
          for (TR::Register *fpr = iter->getFirst(); fpr;
               fpr = iter->getNext()) {
...
void TR_Debug::traceRegisterAssigned(TR_RegisterAssignmentFlags flags,
                                     TR::Register *virtReg,
                                     TR::Register *realReg) {
  ...
  if (virtReg->isPlaceholderReg() &&
      !_comp->getOptions()->getRegisterAssignmentTraceOption(
          TR_TraceRADependencies))
    return;
  char buf[30];
  const char *preCoercionSymbol =
      flags.testAny(TR_PreDependencyCoercion) ? "!" : "";
  const char *postCoercionSymbol =
      flags.testAny(TR_PostDependencyCoercion) ? "!" : "";
  const char *regSpillSymbol = flags.testAny(TR_RegisterSpilled) ? "$" : "";
  const char *regReloadSymbol = flags.testAny(TR_RegisterReloaded) ? "$" : "";
  const char *openParen = flags.testAny(TR_ColouringCoercion)
                              ? "{"
                              : (flags.testAny(TR_IndirectCoercion) ? "(" : "");
  const char *closeParen =
      flags.testAny(TR_ColouringCoercion)
          ? "}"
          : (flags.testAny(TR_IndirectCoercion) ? ")" : "");
  const char *equalSign = flags.testAny(TR_ByColouring)
                              ? "#"
                              : (flags.testAny(TR_ByAssociation) ? ":" : "=");
  sprintf(buf, "%s%s%s%s(%d/%d)%s%s%s%s%s ", preCoercionSymbol, openParen,
          regReloadSymbol, getName(virtReg), virtReg->getFutureUseCount(),
          virtReg->getTotalUseCount(), equalSign, regSpillSymbol,
...
void TR_Debug::traceRegisterFreed(TR::Register *virtReg,
                                  TR::Register *realReg) {
  TR_ASSERT(realReg, "Cannot trace assignment of NULL Real Register\n");
  TR_ASSERT(virtReg, "Cannot trace assignment of NULL Virtual Register\n");
  ...
  if (virtReg->isPlaceholderReg() &&
    ...
    return;
  char buf[30];
  sprintf(buf, "%s(%d/%d)~%s ", getName(virtReg), virtReg->getFutureUseCount(),
          virtReg->getTotalUseCount(), getName(realReg));
  if ((_registerAssignmentTraceCursor += strlen(buf)) > 80) {
    _registerAssignmentTraceCursor = strlen(buf);
    trfprintf(_file, "\n%s", buf);
  } else {
...
void TR_Debug::traceRegisterInterference(TR::Register *virtReg,
                                         TR::Register *interferingVirtual,
                                         int32_t distance) {
  TR_ASSERT(virtReg && interferingVirtual,
            "cannot trace assignment of NULL registers\n");
  if (_file == NULL) return;
  if (!_comp->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRADetails))
    return;
  char buf[40];
  sprintf(buf, "%s{%d,%d}? ", getName(interferingVirtual),
          interferingVirtual->getAssociation(), distance);
  if ((_registerAssignmentTraceCursor += strlen(buf)) > 80) {
    _registerAssignmentTraceCursor = strlen(buf);
    trfprintf(_file, "\n%s", buf);
  } else {
...
#define MAX_NO_ARGUMENTS 6
  ...
  if (!started) {
    ...
    // in the case of the break_after commands file was already created in
    // final.c
    if ((Argv[0] = ::feGetEnv("TR_DEBUGGER")) == NULL)
...
#define environ (*(__Envn()))
      const char **env =
          (const char **)environ;  // pass TR environemtn to debugger
      fclose(cf);
      Argv[1] = "-c";
      Argv[2] = cfname;
      Argv[3] = "-a";
      ...
      for (x = 0; x == 0;)
        ;
      if (remove(cfname)) printf("Could not delete %s .\n", cfname);
      started = true;
    } else
...
extern "C" {
long _stdcall GetCurrentProcessId();  // msf - including Windows.h causes
                                      // problems with UDATA redefinition
long _stdcall CreateProcessA(const char *appName, const char *cmdLine,
                             void *processAttributes, void *threadAttributes,
                             int inheritsHandles, long creationFlags,
                             void *envP, const char *currentDirectory,
...
#define BUFFER_SIZE 1024
  ...
  if (!started) {
    ...
    int *startupInfo = (int *)calloc(
        20, 4);  // msf - yuck. should get system headers and include here
    *startupInfo = 17 * 4;             // ditto...
    ...
    size = sprintf(appBuffer,
                   "\"C:\\Program Files\\Debugging Tools For Windows\\windbg\" "
                   "-p %d -srcpath %s -c \"%s\"",
                   ppid, srcDir, cmdBuffer);
    TR_ASSERT(size < BUFFER_SIZE, "assertion failure");

#ifdef WINDOWS_API_INCLUDED
    CreateProcessA(NULL, appBuffer, NULL, NULL, 0, 0, NULL, NULL,
                   (LPSTARTUPINFOA)startupInfo,
                   (LPPROCESS_INFORMATION)processInfo);
#else
    CreateProcessA(NULL, appBuffer, NULL, NULL, 0, 0, NULL, NULL, startupInfo,
                   processInfo);
#endif
...
void TR_Debug::printStackBacktraceToTraceLog(TR::Compilation *comp) {
  TR_ASSERT(
      comp,
      "ERROR: Must provide a compliation object in order to print to trace");
  TR_CallStackIteratorImpl cs;
  cs.printStackBacktrace(comp);
}

static void printDenominators(TR::DebugCounter *c, int64_t rawCount,
                              FILE *counterFile) {
  if (!c) return;
...
static int counterCompare(const char *left, const char *right) {
  // Like strcmp but it looks for NUMERIC_SEPARATOR and does numeric ordering
  // where indicated Note: '.' doesn't work like an actual decimal point, ".12"
  // > ".2" because 12 > 2

  // Works by looking at the names as numeric and non-numeric sections, and
  // comparing one section at a time

  static char numericStart[] = {TR::DebugCounterGroup::NUMERIC_SEPARATOR, 0};
  static char numericEnd[] = {TR::DebugCounterGroup::FRACTION_SEPARATOR,
                              TR::DebugCounterGroup::RATIO_SEPARATOR, '.', 0};
  ...
  while (*left != 0 && *right != 0) {
    ...
    if (leftSectionLength != rightSectionLength) {
      return numericComparisonMode
                 ? leftSectionLength -
                       rightSectionLength  // Assume that numbers are not
                                           // 0-padded, so any longer number
                                           // must be bigger
                 : strcmp(left, right);
    }
    ...
    // Except for '.' in numeric mode, a terminator changes the
    // numeric/non-numeric mode
    bool switchMode = (left[0] != '.');
    // Terminators are the same, so skip to the next character, which starts the
    // next section
    left++;
    right++;

    if (switchMode) numericComparisonMode = !numericComparisonMode;
  }
...
static FILE *openDebugCounterFile(char *counterFileName) {
  FILE *result = NULL;
  if (counterFileName) result = fopen(counterFileName, "a");
...
static int32_t partition(void *array, int32_t first, int32_t last,
                         int32_t elemSize, int (*cmpFunc)(void *, void *)) {
  int32_t i = first;
  int32_t j = last;
  while (1) {
    while (cmpFunc((char *)array + i * elemSize,
                   (char *)array + first * elemSize) < 0)
      i++;
    while (cmpFunc((char *)array + j * elemSize,
                   (char *)array + first * elemSize) > 0)
      j--;
    if (i >= j) return j;
...
static void quicksort(void *array, int32_t first, int32_t last,
                      int32_t elemSize, int (*cmpFunc)(void *, void *)) {
  int32_t mid;
  if (first < last) {
...
static void qsort(void *array, int32_t nitems, int32_t elemSize,
                  int (*cmpFunc)(void *, void *)) {
  quicksort(array, 0, nitems - 1, elemSize, cmpFunc);
}

void TR_Debug::printDebugCounters(TR::DebugCounterGroup *counterGroup,
                                  const char *name) {
  ListBase<TR::DebugCounter> &counters = counterGroup->_counters;
  if (counters.isEmpty()) return;

  TR::DebugCounter **counterArray =
      (TR::DebugCounter **)(TR::Compiler->regionAllocator.allocate(
  ...
  static FILE *counterFile =
      openDebugCounterFile(::feGetEnv("TR_DebugCounterFileName"));
  fprintf(counterFile, "\n== %s ==\n", name);
  ...
  // Avoid displaying redundant counters.  If a nonzero counter C has a
  // denominator D, no other nonzero counter has denominator D, and C's count
  // equals D's count, then D is redundant.  This is actually quite a common
  // occurrence.
  //
  ...
  for (i = 0; i < count; i++) {
    ...
    if (numNumerators == 0) keep = true;
    if (!keep) counterArray[i] = NULL;
  }

  for (i = 0; i < count; i++) {
    c = counterArray[i];
    if (c && c->getCount() != 0) {
      fprintf(counterFile, "%3d: %-*s | %12.0f | ", i, longestName,
              c->getName(), (double)(c->getCount()));
