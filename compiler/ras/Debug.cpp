...
const char*
jitdCurrentMethodSignature(TR::Compilation* comp)
{
  return comp->getCurrentMethod()->signature(comp->trMemory(), heapAlloc);
}

void*
TR_Debug::operator new(size_t s, TR_HeapMemory m)
{
  return m.allocate(s, TR_Memory::Debug);
}

void*
TR_Debug::operator new(size_t s, TR::PersistentAllocator& allocator)
{
  return allocator.allocate(s);
}

TR_Debug*
createDebugObject(TR::Compilation* comp)
{
  TR_Debug* dbg = NULL;
  if (comp)
...
inline void
roundToBoundary(uint32_t& value, uint32_t boundary)
{
...
bool
TR_Debug::performTransformationImpl(bool canOmitTransformation,
                                    const char* format,
                                    ...)
{
  ...
  if (canOmitTransformation) {
    ...
    if (_comp->getOptIndex() == _comp->getOptions()->getLastOptIndex()) {
      trfprintf(
        _file, "%3d.%-4d ", _comp->getOptIndex(), _comp->getOptSubIndex());
    }
  ...
  TR::IO::vfprintf(
    _file,
    getDiagnosticFormat(format, buffer, sizeof(buffer) / sizeof(char)),
    args);
  va_end(args);
  trfflush(_file);
...
TR_Debug::vtrace(const char* format, va_list args)
{
  if (_file != NULL) {
    char buffer[256];
    TR::IO::vfprintf(
      _file, getDiagnosticFormat(format, buffer, sizeof(buffer)), args);
    trfflush(_file);
  }
}
...
//
uint8_t*
TR_Debug::printPrefix(TR::FILE* pOutFile,
...
void
TR_Debug::printSnippetLabel(TR::FILE* pOutFile,
                            ...
                            const char* comment2)
{
  ...
  if (_comp->getOption(TR_MaskAddresses)) {
    trfprintf(pOutFile,
              "\n\n%*s %08x %*s",
              TR::Compiler->debug.hexAddressFieldWidthInChars(),
              "*Masked*",
              offset,
...
TR_Debug::containingClass(TR::SymbolReference* symRef)
{
  ...
  if (method) {
    ...
    return fe()->getClassFromSignature(
      className, classNameLen, symRef->getOwningMethod(comp()));
  }
...
void
TR_Debug::print(TR::SymbolReference* symRef,
                ...
                bool verbose)
{
  ...
  if (verbose) {
    ...
    if (hideHelperMethodInfo)
      ...
      output.append(" %s%s%s[%s%s%s%s%s]%s%s",
                    symRefName.getStr(),
                    labelSymbol.getStr(),
                    symRefWCodeId.getStr(),
                    symRefNum.getStr(),
...
TR_Debug::getName(TR::CFGNode* node)
{
  ...
  if (_comp->getAddressEnumerationOption(TR_EnumerateBlock)) {
    ...
    sprintf(
      buf, "%*s", TR::Compiler->debug.hexAddressWidthInChars(), "*Masked*");
  } else {
...
TR_Debug::getName(TR::SymbolReference* symRef)
{
  ...
  if (index < nonhelperIndex) {
    if (index >=
          numHelperSymbols + TR::SymbolReferenceTable::firstArrayShadowSymbol &&
...
void
TR_Debug::printAliasInfo(TR::FILE* pOutFile,
...
TR_Debug::getAutoName(TR::SymbolReference* symRef)
{
  ...
  if (symRef->getSymbol()->isSpillTempAuto()) {
    ...
    if (_comp->getOption(TR_MaskAddresses))
      ...
      sprintf(
        name, "<%s " POINTER_PRINTF_FORMAT ">", symName, symRef->getSymbol());
    ...
    if (symRef->getSymbol()->castToAutoSymbol()->isPinningArrayPointer())
      ...
      if (symRef->holdsMonitoredObjectForSyncMethod())
        sprintf(name,
                "%.*s<auto slot %d holds monitoredObject syncMethod>",
                debugNameLen,
                debugName,
                slot);
      else
        sprintf(name,
                "%.*s<auto slot %d holds monitoredObject>",
                debugNameLen,
                debugName,
                slot);
    } else
    ...
    if (symRef->getSymbol()->castToAutoSymbol()->isInternalPointer())
      ...
      if (symRef->getSymbol()->castToAutoSymbol()->isPinningArrayPointer())
        ...
        if (symRef->holdsMonitoredObjectForSyncMethod())
          sprintf(
            name, "<temp slot %d holds monitoredObject syncMethod>", slot);
        else
...
TR_Debug::getParmName(TR::SymbolReference* symRef)
{
  ...
  if (slot == 0 && !getOwningMethodSymbol(symRef)->isStatic()) {
    buf = (char*)_comp->trMemory()->allocateHeapMemory(debugNameLen +
                                                       signatureLen + 17);
    sprintf(
      buf, "%.*s<'this' parm %.*s>", debugNameLen, debugName, signatureLen, s);
  } else {
    buf = (char*)_comp->trMemory()->allocateHeapMemory(debugNameLen +
                                                       signatureLen + 15);
    sprintf(buf,
...
TR_Debug::getStaticName(TR::SymbolReference* symRef)
{
  ...
  if (sym->isCallSiteTableEntry()) {
    ...
    sprintf(s,
            "<callSite entry @%d " POINTER_PRINTF_FORMAT ">",
  ...
  if (sym->isMethodTypeTableEntry()) {
    ...
    sprintf(s,
            "<methodType entry @%d " POINTER_PRINTF_FORMAT ">",
...
TR_Debug::getShadowName(TR::SymbolReference* symRef)
{
  ...
  for (int32_t i =
         TR::SymbolReferenceTable::firstCommonNonhelperNonArrayShadowSymbol;
       i < _comp->getSymRefTab()->getLastCommonNonhelperSymbol();
       i++) {
                                   ...
                                   bool dumpLiveRegs)
{
  ...
  if (dumpTrees)
    printWithFixedPrefix(
      pOutFile, treeTop->getNode(), 1, true, dumpRefCounts, " ");
...
void
TR_Debug::dumpMethodInstrs(TR::FILE* pOutFile,
                           const char* title,
                           bool dumpTrees,
                           bool header)
{
  ...
  if (dumpTrees) {
    ...
    for (TR::TreeTop* treeTop = _comp->getStartTree(); treeTop;
      ...
      printWithFixedPrefix(
        _comp->getOutFile(), treeTop->getNode(), 1, true, false, " ");
      if (treeTop->getLastInstruction()) {
        trfprintf(pOutFile, "\n------------------------------\n");
        while (instr) {
...
//
void
TR_Debug::printNodeEvaluation(TR::Node* node,
...
TR_Debug::dumpMixedModeDisassembly()
{
  ...
  for (; inst; inst = inst->getNext()) {
    if (inst->getNode() != NULL &&
      ...
      printByteCodeStack(
        n->getInlinedSiteIndex(), n->getByteCodeIndex(), indent);
    }
...
void
TR_Debug::dumpInstructionComments(TR::FILE* pOutFile,
                                  TR::Instruction* instr,
...
void
TR_Debug::printCommonDataMiningAnnotations(TR::FILE* pOutFile,
                                           TR::Instruction* inst,
                                           bool needsStartComment)
{
...
void
TR_Debug::print(TR::FILE* pOutFile,
...
TR_Debug::getName(TR::Register* reg, TR_RegisterSizes size)
{
  ...
  if (reg->getRegisterPair()) {
    ...
    sprintf(
      buf, "%s%s_%04d", prefix, getRegisterKindName(reg->getKind()), regNum);
    _comp->getToStringMap().Add((void*)reg, buf);
    return buf;
  } else {
    char* buf = (char*)_comp->trMemory()->allocateHeapMemory(
    ...
    if (_comp->getOption(TR_MaskAddresses))
      sprintf(
        buf, "%s%s_*Masked*", prefix, getRegisterKindName(reg->getKind()));
    else
      sprintf(buf,
              "%s%s_" POINTER_PRINTF_FORMAT,
              prefix,
...
const char*
TR_Debug::getGlobalRegisterName(TR_GlobalRegisterNumber regNum,
...
void
TR_Debug::printRegisterMask(TR::FILE* pOutFile,
...
TR_Debug::printFullRegInfo(TR::FILE* pOutFile, TR::Register* reg)
{
  ...
  if (reg->getRealRegister()) {
    ...
    if (reg->getRealRegister()->getState() == TR::RealRegister::Assigned) {
      ...
      trfprintf(
        pOutFile, "[ %-10s ]", stateNames[reg->getRealRegister()->getState()]);
...
void
TR_Debug::printGPRegisterStatus(TR::FILE* pOutFile,
...
void
TR_Debug::printFPRegisterStatus(TR::FILE* pOutFile,
                                OMR::MachineConnector* machine)
{
  if (pOutFile == NULL)
    return;
...
TR_Debug::print(TR::FILE* pOutFile, TR::GCStackAtlas* atlas)
{
  ...
  for (auto temps = _comp->cg()->getCollectedSpillList().begin();
       temps != _comp->cg()->getCollectedSpillList().end();
       ++temps) {
...
void
TR_Debug::dumpSimulatedNode(TR::Node* node, char tagChar)
{
  trfprintf(_file, "\n               [%s]", getName(node));
  if (_comp->cg()->simulatedNodeState(node)._willBeRematerialized) {
    ...
    trfprintf(
      _file, "%2d/%-2d", node->getLocalIndex(), node->getReferenceCount());
  } else {
    trfprintf(_file, "     ");
  }
  ...
  if (node->getOpCode().hasSymbolReference()) {
    ...
    if (node->getType().isIntegral()) {
      ...
      trfprintf(
        _file, "%s %-8g", getName(node->getOpCode()), node->getDouble());
      padding -= 9;
...
void
TR_Debug::startTracingRegisterAssignment(const char* direction,
...
void
TR_Debug::traceRegisterAssignment(TR::Instruction* instr,
                                  bool insertedByRA,
                                  bool postRA)
{
...
void
TR_Debug::traceRegisterAssigned(TR_RegisterAssignmentFlags flags,
                                TR::Register* virtReg,
                                TR::Register* realReg)
{
  ...
  const char* closeParen = flags.testAny(TR_ColouringCoercion)
                             ? "}"
                             : (flags.testAny(TR_IndirectCoercion) ? ")" : "");
...
void
TR_Debug::traceRegisterInterference(TR::Register* virtReg,
                                    ...
                                    int32_t distance)
{
  ...
  sprintf(buf,
          "%s{%d,%d}? ",
          getName(interferingVirtual),
          interferingVirtual->getAssociation(),
          distance);
  if ((_registerAssignmentTraceCursor += strlen(buf)) > 80) {
    _registerAssignmentTraceCursor = strlen(buf);
    trfprintf(_file, "\n%s", buf);
...
yield(void);
void
TR_Debug::setupDebugger(void* addr)
{
...
#elif defined(LINUX)
void
TR_Debug::setupDebugger(void* startaddr, void* endaddr, bool before)
{
  static bool started = false;
  char cfname[256];
  FILE* cf;
  ...
  if (!started) {
    ...
    if (cf = fopen(cfname, "r")) {
      struct inheritance inh = {
        0
      };               /* use all the default inheritance stuff */
...
#else
    CreateProcessA(
      NULL, appBuffer, NULL, NULL, 0, 0, NULL, NULL, startupInfo, processInfo);
#endif
...
void
TR_Debug::setSingleAllocMetaData(bool usesSingleAllocMetaData)
{
...
void
TR_Debug::printStackBacktraceToTraceLog(TR::Compilation* comp)
{
  TR_ASSERT(
    comp,
...
static void
printDenominators(TR::DebugCounter* c, int64_t rawCount, FILE* counterFile)
{
  if (!c)
    return;
...
static int
counterCompare(const char* left, const char* right)
{
  ...
  static char numericEnd[] = { TR::DebugCounterGroup::FRACTION_SEPARATOR,
                               TR::DebugCounterGroup::RATIO_SEPARATOR,
                               '.',
                               0 };
  ...
  while (*left != 0 && *right != 0) {
    ...
    if (leftSectionLength != rightSectionLength) {
      return numericComparisonMode
               ? leftSectionLength -
                   rightSectionLength // Assume that numbers are not 0-padded,
                                      // so any longer number must be bigger
               : strcmp(left, right);
    }
...
static FILE*
openDebugCounterFile(char* counterFileName)
{
  FILE* result = NULL;
  if (counterFileName)
...
static int
compareDebugCounter(const void* a, const void* b)
{
...
static void
swap(void* elemA, void* elemB, int32_t elemSize)
{
  if ((elemSize % 4) == 0) {
          ...
          int (*cmpFunc)(void*, void*))
{
  ...
  while (1) {
    while (
      cmpFunc((char*)array + i * elemSize, (char*)array + first * elemSize) < 0)
      i++;
    while (
      cmpFunc((char*)array + j * elemSize, (char*)array + first * elemSize) > 0)
      j--;
    if (i >= j)
      return j;
...
void
TR_Debug::printDebugCounters(TR::DebugCounterGroup* counterGroup,
                             const char* name)
{
