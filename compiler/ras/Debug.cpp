  ...
  TR::FILE* pOutFile, TR::LabelSymbol* label, uint8_t* cursor, const char* comment1, const char* comment2)
{
  ...
  trfprintf(pOutFile, ":");
  if (comment1) {
    trfprintf(pOutFile, "\t\t%c %s", (TR::Compiler->target.cpu.isX86() && TR::Compiler->target.isLinux()) ? '#' : ';',
      comment1);
    if (comment2)
  ...
  TR::SymbolReference* symRef, TR_PrettyPrinterString& output, bool hideHelperMethodInfo, bool verbose)
{
  ...
  TR_PrettyPrinterString symRefNum(this), symRefOffset(this), symRefAddress(this), symRefName(this), symRefKind(this),
    otherInfo(this), symRefWCodeId(this), symRefObjIndex(this), labelSymbol(this);
  ...
  if (sym) {
    if (!inDebugExtension() && _comp->cg()->getMappingAutomatics() && sym->isRegisterMappedSymbol()
  ...
  if (symRef->getKnownObjectIndex() != TR::KnownObjectTable::UNKNOWN)
    ...
    TR::KnownObjectTable::Index i
      = comp()->getKnownObjectTable()->getExistingIndexAt((uintptrj_t*)sym->castToStaticSymbol()->getStaticAddress());
  ...
  if (verbose) {
    ...
    if (hideHelperMethodInfo)
      output.append(
        " %s%s[%s]%s", labelSymbol.getStr(), symRefWCodeId.getStr(), symRefOffset.getStr(), symRefObjIndex.getStr());
    else
      output.append(" %s%s%s[%s%s%s]%s%s", symRefName.getStr(), labelSymbol.getStr(), symRefWCodeId.getStr(),
        symRefKind.getStr(), symRefOffset.isEmpty() ? "" : " ", symRefOffset.getStr(), symRefObjIndex.getStr(),
        otherInfo.getStr());
    ...
    if (hideHelperMethodInfo)
      output.append(" %s%s[%s%s%s]%s", labelSymbol.getStr(), symRefWCodeId.getStr(), symRefNum.getStr(),
        symRefOffset.isEmpty() ? "" : " ", symRefOffset.getStr(), symRefObjIndex.getStr());
    else
      output.append(" %s%s%s[%s%s%s%s%s]%s%s", symRefName.getStr(), labelSymbol.getStr(), symRefWCodeId.getStr(),
        symRefNum.getStr(), symRefKind.isEmpty() ? "" : " ", symRefKind.getStr(), symRefOffset.isEmpty() ? "" : " ",
...
const char* TR_Debug::getName(TR::SymbolReference* symRef)
{
  ...
  if (index < nonhelperIndex) {
    ...
    if (index >= numHelperSymbols + TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol
      ...
      return getPerCodeCacheHelperName(
        (TR_CCPreLoadedCode)(index - numHelperSymbols - TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol));
...
void TR_Debug::dumpSingleTreeWithInstrs(
  TR::TreeTop* treeTop, TR::Instruction* instr, bool dumpTrees, bool dumpInstrs, bool dumpRefCounts, bool dumpLiveRegs)
{
  TR::FILE* pOutFile = _comp->getOutFile();
  if (pOutFile == NULL)
    return;
      ...
      while (cursor) {
        ...
        if (outlinedCursor) {
          ...
          if (outlinedCursor != lastOutlinedTarget) {
            // Avoiding dumping the same outlined code sequence more than once by keeping track of the last sequence we
            // dumped Not fool-proof, but cheap
...
void TR_Debug::printRegisterKilled(TR::Register* reg)
{
  ...
  if (node) {
    trfprintf(pOutFile, " [%s] (%3d)%*s%s   ", getName(node), node->getReferenceCount(), _comp->cg()->_indentation, " ",
      getName(node->getOpCode()));
  } else {
    trfprintf(pOutFile, "  %*s       %*s", addressWidth, " ", _comp->cg()->_indentation, " ");
  }
  trfprintf(pOutFile, "%s%s\n", reg->getRegisterName(_comp), reg->isLive() ? " (killed)" : " (killed, already dead)");
}

// Prints info on node and register under evaluation
//
void TR_Debug::printNodeEvaluation(TR::Node* node, const char* relationship, TR::Register* reg, bool printOpCode)
{
  if (!node)
    return;
  TR::FILE* pOutFile = _comp->getOutFile();
  trfprintf(pOutFile, " [%s] (%3d)%*s%s%s%s%s\n", getName(node), node->getReferenceCount(), _comp->cg()->_indentation,
    " ", (printOpCode) ? getName(node->getOpCode()) : "", relationship, (reg) ? reg->getRegisterName(_comp) : "",
    ...
    if (inst->getNode() != NULL
      && (!n
        || ((inst->getNode()->getByteCodeInfo().getCallerIndex() != n->getByteCodeInfo().getCallerIndex()
              || inst->getNode()->getByteCodeInfo().getByteCodeIndex() != n->getByteCodeInfo().getByteCodeIndex())
...
void TR_Debug::printCommonDataMiningAnnotations(TR::FILE* pOutFile, TR::Instruction* inst, bool needsStartComment)
{
  if (inst != NULL && inst->getNode()) {
    ...
    TR::SimpleRegex* regex = _comp->getOptions()->getTraceForCodeMining();
    if (regex
      && (TR::SimpleRegex::match(regex, "ALL") || TR::SimpleRegex::match(regex, IL_KEY)
        || TR::SimpleRegex::match(regex, FRQ_KEY) || TR::SimpleRegex::match(regex, CLD_KEY))) {
...
void TR_Debug::print(TR::FILE* pOutFile, TR::list<TR::Snippet*>& snippetList, bool isWarm)
{
  if (pOutFile == NULL)
    return;
...
void TR_Debug::print(TR::FILE* pOutFile, List<TR::Snippet>& snippetList, bool isWarm)
{
  if (pOutFile == NULL)
    return;
...
const char* TR_Debug::getName(TR::Register* reg, TR_RegisterSizes size)
{
  ...
  if (reg->getRegisterPair()) {
    ...
    char* buf = (char*)_comp->trMemory()->allocateHeapMemory(
      maxPrefixSize + 6 + 11); // max register kind name size plus underscore plus 10-digit reg num plus null terminator
  ...
  if (!inDebugExtension() && !vguards.empty()) {
    ...
    for (auto info = vguards.begin(); info != vguards.end(); ++info, ++i) {
      ...
      for (TR_VirtualGuardSite* site = siteIt.getFirst(); site; site = siteIt.getNext()) {
        ...
        trfprintf(pOutFile,
          "\tSite: location=" POINTER_PRINTF_FORMAT " (e+%5x) branch-dest=" POINTER_PRINTF_FORMAT " (e+%5x)\n", loc,
          loc - startPC, dest, dest - startPC);
      }
...
void TR_Debug::traceRegisterAssigned(TR_RegisterAssignmentFlags flags, TR::Register* virtReg, TR::Register* realReg)
{
  ...
  if (_file == NULL || !_comp->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRABasic))
    return;
  // Avoid superfluous traces, unless the user asks for them.
  if (virtReg->isPlaceholderReg() && !_comp->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRADependencies))
    return;
  char buf[30];
  ...
  sprintf(buf, "%s%s%s%s(%d/%d)%s%s%s%s%s ", preCoercionSymbol, openParen, regReloadSymbol, getName(virtReg),
    virtReg->getFutureUseCount(), virtReg->getTotalUseCount(), equalSign, regSpillSymbol, getName(realReg), closeParen,
    postCoercionSymbol);
...
#define BUFFER_SIZE 1024
  ...
  if (!started) {
    ...
    *startupInfo = 17 * 4; // ditto...
    ...
    size = sprintf(appBuffer, "\"C:\\Program Files\\Debugging Tools For Windows\\windbg\" -p %d -srcpath %s -c \"%s\"",
      ppid, srcDir, cmdBuffer);
    TR_ASSERT(size < BUFFER_SIZE, "assertion failure");

#ifdef WINDOWS_API_INCLUDED
    CreateProcessA(
      NULL, appBuffer, NULL, NULL, 0, 0, NULL, NULL, (LPSTARTUPINFOA)startupInfo, (LPPROCESS_INFORMATION)processInfo);
#else
...
static int counterCompare(const char* left, const char* right)
{
  ...
  while (*left != 0 && *right != 0) {
    ...
    if (leftSectionLength != rightSectionLength) {
      return numericComparisonMode ? leftSectionLength
