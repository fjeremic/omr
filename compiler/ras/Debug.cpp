...
TR_Debug::TR_Debug(TR::Compilation *c)
    : _comp(c), _compilationFilters(0), _relocationFilters(0),
      _inlineFilters(0), _lastFrequency(-1), _isCold(false),
      _currentParent(NULL), _currentChildIndex(0), _mainEntrySeen(false) {
  if (c) {
    _cg = c->cg();
    _fe = c->fe();
    _file = c->getOutFile();
  ...
  while (instr) {
    ...
    if (printLinenos) {
      ...
      if ((crtLineNo != -1) &&
        ...
        if (lno > 0) // this check is workaround for front end problem with line
                     // numbers
        {
          trfprintf(pOutFile, "\n\t.line \t %d", lno);
...
const char *TR_Debug::getName(TR::Register *reg, TR_RegisterSizes size) {
  ...
  if (reg->getRealRegister()) {
    ...
    if (TR::Compiler->target.cpu.isARM64())
      return getName((TR::RealRegister *)reg, size);
#endif
    TR_ASSERT(0, "TR_Debug::getName() ==> unknown target platform for given "
                 "real register\n");
  }
...
void TR_Debug::startTracingRegisterAssignment(const char *direction,
  ...
  trfprintf(_file, "<legend>\n"
                   "  V(F/T)   virtual register V with future use count F and "
                   "total use count T\n"
      ...
      return numericComparisonMode
                 ? leftSectionLength -
                       rightSectionLength // Assume that numbers are not
                                          // 0-padded, so any longer number must
                                          // be bigger
                 : strcmp(left, right);
    }
