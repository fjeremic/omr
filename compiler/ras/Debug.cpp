...
bool TR_Debug::performTransformationImpl(bool canOmitTransformation, const char* format, ...)
{
    ...
    if (canOmitTransformation) {
        ...
        if (curTransformationIndex < _comp->getOptions()->getFirstOptTransformationIndex() || curTransformationIndex > _comp->getOptions()->getLastOptTransformationIndex())
            return false;
...
void TR_Debug::printSnippetLabel(TR::FILE* pOutFile, TR::LabelSymbol* label, uint8_t* cursor, const char* comment1, const char* comment2)
{
    int addressFieldWidth = TR::Compiler->debug.hexAddressFieldWidthInChars();
    int codeByteColumnWidth = TR::Compiler->debug.codeByteColumnWidth();
    int prefixWidth = addressFieldWidth * 2 + codeByteColumnWidth + 12; // 8 bytes of offsets, 2 spaces, and opening and closing brackets
...
void TR_Debug::print(TR::SymbolReference* symRef, TR_PrettyPrinterString& output, bool hideHelperMethodInfo, bool verbose)
{
    ...
    if (sym) {
        if (!inDebugExtension() && _comp->cg()->getMappingAutomatics() && sym->isRegisterMappedSymbol() && sym->getRegisterMappedSymbol()->getOffset() != 0) {
...
TR_Debug::getName(TR::SymbolReference* symRef)
{
    ...
    if (index < nonhelperIndex) {
        if (index >= numHelperSymbols + TR::SymbolReferenceTable::firstArrayShadowSymbol && index < numHelperSymbols + TR::SymbolReferenceTable::firstArrayShadowSymbol + TR::NumTypes)
            return "<array-shadow>";
        if (index >= numHelperSymbols + TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol && index <= numHelperSymbols + TR::SymbolReferenceTable::lastPerCodeCacheHelperSymbol)
...
void TR_Debug::dumpMethodInstrs(TR::FILE* pOutFile, const char* title, bool dumpTrees, bool header)
{
    ...
    while (instr) {
        ...
        if (printLinenos) {
            ...
            if ((crtLineNo != -1) && ((crtLineNo != lastLineNoPrinted) || (inlinedIndex != lastInlinedIndex) || (sourceFileIndex != lastSourceFileIndex) || printEveryLine)) {
...
void TR_Debug::dumpMixedModeDisassembly()
{
    ...
    for (; inst; inst = inst->getNext()) {
        if (inst->getNode() != NULL && (!n || ((inst->getNode()->getByteCodeInfo().getCallerIndex() != n->getByteCodeInfo().getCallerIndex() || inst->getNode()->getByteCodeInfo().getByteCodeIndex() != n->getByteCodeInfo().getByteCodeIndex()) && inst->getBinaryLength() > 0))) {
            n = inst->getNode();
...
void TR_Debug::printCommonDataMiningAnnotations(TR::FILE* pOutFile, TR::Instruction* inst, bool needsStartComment)
{
    if (inst != NULL && inst->getNode()) {
        ...
        TR::SimpleRegex* regex = _comp->getOptions()->getTraceForCodeMining();
        if (regex && (TR::SimpleRegex::match(regex, "ALL") || TR::SimpleRegex::match(regex, IL_KEY) || TR::SimpleRegex::match(regex, FRQ_KEY) || TR::SimpleRegex::match(regex, CLD_KEY))) {
...
void TR_Debug::printFullRegInfo(TR::FILE* pOutFile, TR::Register* reg)
{
    ...
    if (reg->getRealRegister()) {
        ...
        if (ignoreFreeAndLockedRegs && (reg->getRealRegister()->getState() == TR::RealRegister::Locked || reg->getRealRegister()->getState() == TR::RealRegister::Free))
            return;
        ...
        if (reg->getRealRegister()->getState() == TR::RealRegister::Assigned) {
            ...
            if (reg->getRealRegister()->getState() == TR::RealRegister::Locked && reg->getAssignedRegister() != NULL && reg->getAssignedRegister() != reg) {
                TR::Register* virtReg = reg->getAssignedRegister();
                trfprintf(pOutFile, "[%5d/%5d]",
                    virtReg->getFutureUseCount(),
                    virtReg->getTotalUseCount());
