  ...
  while (_trace_size < MAX_STACK_SIZE) {
    ...
    PSYMBOL_INFO pSymbol
      = (PSYMBOL_INFO)TR::globalAllocator("CallStackIterator")
          .allocate((sizeof(SYMBOL_INFO) + MAX_SYM_NAME * sizeof(TCHAR) + sizeof(ULONG64) - 1) / sizeof(ULONG64));
    ...
    if (SymFromAddr(hProcess, StackFrame.AddrPC.Offset, &dwDisplacement, pSymbol)) {
      _function_names[_trace_size]
        = (char*)TR::globalAllocator("CallStackIterator").allocate(strlen(pSymbol->Name) * sizeof(char) + 1);
