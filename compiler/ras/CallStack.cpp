...
#define GET_CURR_PC(dst)                                                \
  do {                                                                  \
    asm("bl $+4;"  /*branch and link to next instruction*/              \
        "mflr %0;" /*copy contents of link register (which now contains \
                      address of this instruction) to dst*/             \
        : "=r"(dst)                                                     \
        : /*no inputs*/                                                 \
        : "lr");                                                        \
  } while (0)
  ...
  if (*pc != 0) {
    ...
    if (_tb_table->tb.has_tboff) {
      ...
      if (((unsigned int *)(uintptr_t)_tb_table - *((unsigned int *)x)) > pc) {
        ...
        _offset_in_proc =
            ((uintptr_t)pc) - ((uintptr_t)_tb_table - *((unsigned int *)x));
      }
    } else {
      _offset_in_proc = 0;
    }
...
TR_PPCCallStackIterator::TR_PPCCallStackIterator()
    : TR_CallStackIterator(), _num_next(0) {
  GET_CURR_PC(_pc);
  GET_CURR_TOS(_tos);
  _set_tb_table();
  ...
  if (_done) {
    ...
    if (_tb_table->tb.name_present) {
      ...
      if (_tb_table->tb.has_ctl) {
        int y = *((int *)x);
        x += sizeof(_tb_table->tb_ext.ctl_info) +
             (y * sizeof(_tb_table->tb_ext.ctl_info_disp[0]));
      }
...
void TR_LinuxCallStackIterator::printSymbol(int32_t frame, char *sig,
                                            TR::Compilation *comp) {
  char lib[256];
  char func[256];
  intptr_t offset;
  ...
  int rc =
      sscanf(sig, "%255[^(](%255[^+]+%lx) [%lx]", lib, func, &offset, &address);
  if (rc == 4) {
...
TR_MvsCallStackIterator::TR_MvsCallStackIterator() : TR_CallStackIterator() {
...
bool TR_MvsCallStackIterator::callTraceback() {
  ...
  _offset =
      (char *)_parms.__tf_call_instruction - (char *)_parms.__tf_entry_addr;
  return true;
}
...
TR_WinCallStackIterator::TR_WinCallStackIterator()
    : TR_CallStackIterator(), _num_next(0), _trace_size(0), _done(true) {
  // Load dbghelp.dll
  ...
  SymGetOptionsType SymGetOptions =
      (SymGetOptionsType)GetProcAddress((HMODULE)hLib, "SymGetOptions");
  SymSetOptionsType SymSetOptions =
      (SymSetOptionsType)GetProcAddress((HMODULE)hLib, "SymSetOptions");
  SymInitializeType SymInitialize =
      (SymInitializeType)GetProcAddress((HMODULE)hLib, "SymInitialize");
  StackWalk64Type StackWalk64 =
      (StackWalk64Type)GetProcAddress((HMODULE)hLib, "StackWalk64");
  SymFromAddrType SymFromAddr =
      (SymFromAddrType)GetProcAddress((HMODULE)hLib, "SymFromAddr");
  SymCleanupType SymCleanup =
      (SymCleanupType)GetProcAddress((HMODULE)hLib, "SymCleanup");

  PFUNCTION_TABLE_ACCESS_ROUTINE64 SymFunctionTableAccess64 =
      (PFUNCTION_TABLE_ACCESS_ROUTINE64)GetProcAddress(
          (HMODULE)hLib, "SymFunctionTableAccess64");
  PGET_MODULE_BASE_ROUTINE64 SymGetModuleBase64 =
      (PGET_MODULE_BASE_ROUTINE64)GetProcAddress((HMODULE)hLib,
                                                 "SymGetModuleBase64");

  // Ensure all the function pointers are loaded correctly.
  char *loadFailed = NULL;
  if (SymGetOptions == NULL) loadFailed = "SymGetOptions";
  if (SymSetOptions == NULL) loadFailed = "SymSetOptions";
  if (SymInitialize == NULL) loadFailed = "SymInitialize";
  if (StackWalk64 == NULL) loadFailed = "StackWalk64";
  if (SymFromAddr == NULL) loadFailed = "SymFromAddr";
  if (SymFunctionTableAccess64 == NULL) loadFailed = "SymFunctionTableAccess64";
  if (SymGetModuleBase64 == NULL) loadFailed = "SymGetModuleBase64";

  if (loadFailed) {
    fprintf(stderr,
            "Unable to print trace stack. GetProcAddress failed for %s\n",
            loadFailed);
    return;
  }
...
#define SYMOPT_DEBUG 0x80000000
  ...
  DWORD searchPathLength =
      GetModuleFileName(NULL, userSearchPathBuffer, MAX_SEARCHPATH);
  if (searchPathLength == 0 || searchPathLength >= MAX_SEARCHPATH) {
    fprintf(stderr,
            "unable to find module path for current executing - symbol names "
            "might be incorrect\n");
    userSearchPath = NULL;
  } else
  ...
  if (!(*SymInitialize)(hProcess, userSearchPath, TRUE)) {
    ...
    fprintf(stderr,
            "Unable to retrieve symbol informaton. SymInitialize returned "
            "error : %d\n",
            GetLastError());
  }
  ...
  while (_trace_size < MAX_STACK_SIZE) {
    if (!StackWalk64(
            ...
            SymFunctionTableAccess64,  // Function table access function (FPO
                                       // access on x86)
            SymGetModuleBase64,  // Function that can determine module base for
                                 // the given address
    ...
    PSYMBOL_INFO pSymbol =
        (PSYMBOL_INFO)TR::globalAllocator("CallStackIterator")
            .allocate((sizeof(SYMBOL_INFO) + MAX_SYM_NAME * sizeof(TCHAR) +
                       sizeof(ULONG64) - 1) /
                      sizeof(ULONG64));
    ...
    if (SymFromAddr(hProcess, StackFrame.AddrPC.Offset, &dwDisplacement,
                    pSymbol)) {
      _function_names[_trace_size] =
          (char *)TR::globalAllocator("CallStackIterator")
              .allocate(strlen(pSymbol->Name) * sizeof(char) + 1);
...
#elif !(defined(OMR_OS_WINDOWS) && defined(TR_HOST_X86) && \
        defined(TR_HOST_32BIT))
