...
void
TR_CallStackIterator::printStackBacktrace(TR::Compilation* comp)
{
  while (!isDone()) {
    if (comp)
...
void
TR_PPCCallStackIterator::_set_tb_table()
{
  _tb_table = NULL;
  uintptr_t x = 0;
...
bool
TR_PPCCallStackIterator::getNext()
{
  if (++_num_next > 200)
...
const char*
TR_PPCCallStackIterator::getProcedureName()
{
  if (_done) {
    return NULL;
  } else {
...
void
TR_LinuxCallStackIterator::printSymbol(int32_t frame,
                                       char* sig,
                                       TR::Compilation* comp)
{
  char lib[256];
...
void
TR_LinuxCallStackIterator::printStackBacktrace(TR::Compilation* comp)
{
...
bool
TR_MvsCallStackIterator::callTraceback()
{
...
bool
TR_MvsCallStackIterator::getNext()
{
  if (isDone())
    return false;
...
uintptr_t
TR_WinCallStackIterator::getOffsetInProcedure()
{
  return (_done ? 0 : _offset_in_proc[_num_next]);
}

const char*
TR_WinCallStackIterator::getProcedureName()
{
  return (_done ? NULL : _function_names[_num_next]);
}

unsigned long
TR_WinCallStackIterator::getEip()
{
  unsigned long Eip;
  ...
  while (_trace_size < MAX_STACK_SIZE) {
    if (!StackWalk64(IMAGE_FILE_MACHINE_I386, // Machine architecture type (x86)
                     ...
                     SymFunctionTableAccess64, // Function table access function
                                               // (FPO access on x86)
                     SymGetModuleBase64, // Function that can determine module
                                         // base for the given address
...
bool
TR_WinCallStackIterator::getNext()
{
  _done = (++_num_next >= _trace_size);
  return !_done;
}
