...
public:
  ...
  {
    int32_t index = _sideTable.indexOf(node);
    TR_ASSERT(index >= _basis,
              "Cannot mark node n%dn before basis %d live",
              node->getGlobalIndex(),
              _basis);
...
bool
isILValidationLoggingEnabled(TR::Compilation* comp);

void
checkILCondition(TR::Node* node,
                 bool condition,
                 TR::Compilation* comp,
...
void
printILDiagnostic(TR::Compilation* comp, const char* formatStr, ...);
void
vprintILDiagnostic(TR::Compilation* comp, const char* formatStr, va_list ap);
}
