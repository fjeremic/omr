...
class TR_Debug {
 public:
  ...
  virtual char *formattedString(char *buf, uint32_t bufLen, const char *format,
                                va_list args, TR_AllocationKind = heapAlloc);
  ...
  // Options processing
  //
  virtual TR::FILE *findLogFile(TR::Options *aotCmdLineOptions,
                                TR::Options *jitCmdLineOptions,
                                TR::OptionSet *optSet, char *logFileName);
  virtual int32_t findLogFile(const char *logFileName,
                              TR::Options *aotCmdLineOptions,
                              TR::Options *jitCmdLineOptions,
                              TR::Options **optionsArray, int32_t arraySize);
  virtual void dumpOptionHelp(TR::OptionTable *jitOptions,
                              TR::OptionTable *feOptions,
                              TR::SimpleRegex *nameFilter);

  static void dumpOptions(char *optionsType, char *options, char *envOptions,
                          TR::Options *cmdLineOptions,
                          TR::OptionTable *jitOptions,
                          TR::OptionTable *feOptions, void *, TR_FrontEnd *);
  virtual char *limitfileOption(
      char *, void *, TR::OptionTable *, TR::Options *, bool loadLimit,
      TR_PseudoRandomNumbersListElement **pseudoRandomListHeadPtr = 0);
  virtual char *inlinefileOption(char *, void *, TR::OptionTable *,
                                 TR::Options *);
  virtual char *limitOption(char *, void *, TR::OptionTable *, TR::Options *,
                            bool loadLimit);
  virtual int32_t *loadCustomStrategy(char *optFileName);
  virtual bool methodCanBeCompiled(TR_Memory *mem, TR_ResolvedMethod *,
                                   TR_FilterBST *&);
  virtual bool methodCanBeRelocated(TR_Memory *mem, TR_ResolvedMethod *,
                                    TR_FilterBST *&);
  virtual bool methodSigCanBeCompiled(const char *, TR_FilterBST *&,
                                      TR::Method::Type methodType);
  virtual bool methodSigCanBeRelocated(const char *, TR_FilterBST *&);
  virtual bool methodSigCanBeCompiledOrRelocated(const char *, TR_FilterBST *&,
                                                 bool isRelocation,
                                                 TR::Method::Type methodType);
  virtual bool methodCanBeFound(TR_Memory *, TR_ResolvedMethod *,
                                TR::CompilationFilters *, TR_FilterBST *&);
  virtual bool methodSigCanBeFound(const char *, TR::CompilationFilters *,
                                   TR_FilterBST *&,
                                   TR::Method::Type methodType);
  virtual TR::CompilationFilters *getCompilationFilters() {
    return _compilationFilters;
  }
  virtual TR::CompilationFilters *getRelocationFilters() {
    return _relocationFilters;
  }
  ...
  virtual TR_FilterBST *addFilter(char *&, int32_t, int32_t, int32_t,
  ...
  virtual void printAnnotationInfoEntry(J9AnnotationInfo *,
                                        J9AnnotationInfoEntry *, int32_t);

  virtual void printOptimizationHeader(const char *, const char *, int32_t,
                                       bool mustBeDone);
  ...
  virtual const char *getGlobalRegisterName(TR_GlobalRegisterNumber regNum,
  ...
  virtual const char *getVirtualGuardTestTypeName(
  ...
  virtual void printIRTrees(TR::FILE *, const char *,
                            TR::ResolvedMethodSymbol *);
  virtual void printBlockOrders(TR::FILE *, const char *,
                                TR::ResolvedMethodSymbol *);
  virtual void print(TR::FILE *, TR::CFG *);
  virtual void print(TR::FILE *, TR_Structure *structure, uint32_t indentation);
  virtual void print(TR::FILE *, TR_RegionAnalysis *structure,
                     uint32_t indentation);
  virtual int32_t print(TR::FILE *, TR::TreeTop *);
  virtual int32_t print(TR::FILE *, TR::Node *, uint32_t indentation = 0,
                        bool printSubtree = true);
  virtual void print(TR::FILE *, TR::SymbolReference *);
  virtual void print(TR::SymbolReference *, TR_PrettyPrinterString &,
                     bool hideHelperMethodInfo = false, bool verbose = false);
  ...
  virtual int32_t printWithFixedPrefix(TR::FILE *, TR::Node *,
  ...
  virtual void print(J9JITExceptionTable *data, TR_ResolvedMethod *feMethod,
  ...
  virtual void dumpSingleTreeWithInstrs(TR::TreeTop *, TR::Instruction *, bool,
                                        bool, bool, bool);
  virtual void dumpMethodInstrs(TR::FILE *, const char *, bool,
                                bool header = false);
  virtual void printRegisterKilled(TR::Register *reg);
  virtual void printNodeEvaluation(TR::Node *node,
                                   const char *relationship = "",
                                   TR::Register *reg = NULL,
                                   bool printOpCode = true);
  virtual void dumpMixedModeDisassembly();
  virtual void dumpInstructionComments(TR::FILE *, TR::Instruction *,
                                       bool needsStartComment = true);
  virtual void printCommonDataMiningAnnotations(TR::FILE *pOutFile,
                                                TR::Instruction *inst,
                                                bool needsStartComment = true);
  ...
  virtual void print(TR::FILE *, TR_GCStackMap *,
                     TR::GCStackAtlas *atlas = NULL);

  virtual void printRegisterMask(TR::FILE *pOutFile, TR_RegisterMask mask,
                                 TR_RegisterKinds rk);
  virtual void print(TR::FILE *, TR::Register *,
                     TR_RegisterSizes size = TR_WordReg);
  ...
  virtual void startTracingRegisterAssignment() {
    startTracingRegisterAssignment("backward");
  }
  virtual void startTracingRegisterAssignment(
      const char *direction,
      TR_RegisterKinds kindsToAssign = TR_RegisterKinds(TR_GPR_Mask |
                                                        TR_FPR_Mask));
  ...
  virtual void traceRegisterAssignment(TR::Instruction *instr,
                                       bool insertedByRA = true,
                                       bool postRA = false);
  virtual void traceRegisterAssigned(TR_RegisterAssignmentFlags flags,
                                     TR::Register *virtReg,
                                     TR::Register *realReg);
  virtual void traceRegisterFreed(TR::Register *virtReg, TR::Register *realReg);
  virtual void traceRegisterInterference(TR::Register *virtReg,
                                         TR::Register *interferingVirtual,
                                         int32_t distance);
  virtual void traceRegisterWeight(TR::Register *realReg, uint32_t weight);

  virtual void printGPRegisterStatus(TR::FILE *pOutFile,
                                     OMR::MachineConnector *machine);
  virtual void printFPRegisterStatus(TR::FILE *pOutFile,
                                     OMR::MachineConnector *machine);
  ...
  virtual void dumpInstructionWithVFPState(TR::Instruction *instr,
                                           const TR_VFPState *prevState);

  void print(TR::FILE *, TR::RealRegister *,
             TR_RegisterSizes size = TR_WordReg);
  const char *getNamex(TR::Snippet *);
  void printRegMemInstruction(TR::FILE *, const char *, TR::RealRegister *reg,
                              TR::RealRegister *base = 0, int32_t = 0);
  void printRegRegInstruction(TR::FILE *, const char *, TR::RealRegister *reg1,
                              TR::RealRegister *reg2 = 0);
  void printRegImmInstruction(TR::FILE *, const char *, TR::RealRegister *reg,
                              int32_t imm);
  void printMemRegInstruction(TR::FILE *, const char *, TR::RealRegister *base,
                              int32_t offset, TR::RealRegister * = 0);
  void printMemImmInstruction(TR::FILE *, const char *, TR::RealRegister *base,
  ...
  virtual void printARMDelayedOffsetInstructions(TR::FILE *pOutFile,
                                                 TR::ARMMemInstruction *instr);
  virtual void printARMHelperBranch(TR::SymbolReference *symRef,
                                    uint8_t *cursor, TR::FILE *outFile,
                                    const char *opcodeName = "bl");
  virtual const char *getOpCodeName(TR_ARMOpCode *);
  const char *getName(TR::RealRegister *, TR_RegisterSizes size = TR_WordReg);
  const char *getName(uint32_t realRegisterIndex,
                      TR_RegisterSizes = (TR_RegisterSizes)-1);
  void print(TR::FILE *, TR::ARMHelperCallSnippet *);
#endif
#if defined(TR_TARGET_S390)
  virtual void printRegisterDependencies(TR::FILE *pOutFile,
  ...
  virtual void printNodesInEdgeListIterator(TR::FILE *, TR::CFGEdgeList &li,
                                            bool fromNode);
  virtual void print(TR::FILE *, TR::Block *block, uint32_t indentation);
  virtual void print(TR::FILE *, TR_RegionStructure *regionStructure,
                     uint32_t indentation);
  virtual void printSubGraph(TR::FILE *, TR_RegionStructure *regionStructure,
                             uint32_t indentation);
  virtual void print(TR::FILE *, TR_InductionVariable *inductionVariable,
                     uint32_t indentation);
  virtual bool inDebugExtension() { return false; }
  virtual void *dxMallocAndRead(uintptrj_t size, void *remotePtr,
                                bool dontAddToMap = false) {
    return remotePtr;
  }
  virtual void *dxMallocAndReadString(void *remotePtr,
                                      bool dontAddToMap = false) {
    return remotePtr;
  }
  ...
  void printBasicPreNodeInfoAndIndent(TR::FILE *, TR::Node *,
                                      uint32_t indentation);
  ...
  int32_t *printStackAtlas(uintptrj_t startPC, uint8_t *mapBits,
                           int32_t numberOfSlotsMapped, bool fourByteOffsets,
                           int32_t *sizeOfStackAtlas, int32_t frameSize);
  uint16_t printStackAtlasDetails(uintptrj_t startPC, uint8_t *mapBits,
                                  int numberOfSlotsMapped, bool fourByteOffsets,
                                  int32_t *sizeOfStackAtlas, int32_t frameSize,
                                  int32_t *offsetInfo);
  uint8_t *printMapInfo(uintptrj_t startPC, uint8_t *mapBits,
                        int32_t numberOfSlotsMapped, bool fourByteOffsets,
                        int32_t *sizeOfStackAtlas,
                        TR_ByteCodeInfo *byteCodeInfo,
                        uint16_t indexOfFirstInternalPtr, int32_t offsetInfo[],
                        bool nummaps = false);
  void printStackMapInfo(uint8_t *&mapBits, int32_t numberOfSlotsMapped,
                         int32_t *sizeOfStackAtlas, int32_t *offsetInfo,
                         bool nummaps = false);
  void printJ9JITExceptionTableDetails(
      J9JITExceptionTable *data, J9JITExceptionTable *dbgextRemotePtr = NULL);
  void printSnippetLabel(TR::FILE *, TR::LabelSymbol *label, uint8_t *cursor,
                         const char *comment1, const char *comment2 = 0);
  uint8_t *printPrefix(TR::FILE *, TR::Instruction *, uint8_t *cursor,
                       uint8_t size);
  ...
  char *printSymbolName(TR::FILE *, TR::Symbol *, TR::SymbolReference *,
                        TR::MemoryReference *mr = NULL);
  bool isBranchToTrampoline(TR::SymbolReference *, uint8_t *, int32_t &);

  virtual void printDebugCounters(TR::DebugCounterGroup *counterGroup,
                                  const char *name);
  ...
  void findLogFile(const char *logFileName, TR::Options *cmdOptions,
                   TR::Options **optionArray, int32_t arraySize,
                   int32_t &index);

  void printPreds(TR::FILE *, TR::CFGNode *);
  void printBaseInfo(TR::FILE *, TR_Structure *structure, uint32_t indentation);
  void print(TR::FILE *, TR_BlockStructure *blockStructure,
             uint32_t indentation);
  ...
  void printVCG(TR::FILE *, TR::Block *block, int32_t vorder = -1,
                int32_t horder = -1);

  void printByteCodeStack(int32_t parentStackIndex, uint16_t byteCodeIndex,
                          char *indent);
  ...
  // Number of spaces that must be inserted after index when index's length <
  // maxIndexLength so the information following it will be aligned
  uint32_t getNumSpacesAfterIndex(uint32_t index,
                                  uint32_t maxIndexLength) const;

#if defined(TR_TARGET_X86)
  void printPrefix(TR::FILE *, TR::Instruction *instr);
  int32_t printPrefixAndMnemonicWithoutBarrier(TR::FILE *,
                                               TR::Instruction *instr,
                                               int32_t barrier);
  void printPrefixAndMemoryBarrier(TR::FILE *, TR::Instruction *instr,
                                   int32_t barrier, int32_t barrierOffset);
  void dumpDependencyGroup(TR::FILE *pOutFile,
  ...
  void printBoundaryAvoidanceInfo(TR::FILE *,
  ...
  void printFullRegisterDependencyInfo(
      TR::FILE *, TR::RegisterDependencyConditions *conditions);
  void printDependencyConditions(TR_X86RegisterDependencyGroup *, uint8_t,
                                 char *, TR::FILE *);

  void print(TR::FILE *, TR::MemoryReference *, TR_RegisterSizes);
  void printReferencedRegisterInfo(TR::FILE *, TR::MemoryReference *);

  int32_t printIntConstant(TR::FILE *pOutFile, int64_t value, int8_t radix,
                           TR_RegisterSizes size = TR_WordReg,
                           bool padWithZeros = false);
  int32_t printDecimalConstant(TR::FILE *pOutFile, int64_t value, int8_t width,
                               bool padWithZeros);
  int32_t printHexConstant(TR::FILE *pOutFile, int64_t value, int8_t width,
                           bool padWithZeros);
  void printInstructionComment(TR::FILE *pOutFile, int32_t tabStops,
                               TR::Instruction *instr);
  void printFPRegisterComment(TR::FILE *pOutFile, TR::Register *target,
                              TR::Register *source);
  ...
  uint8_t *printEmitLoadPPCHelperAddrToCtr(TR::FILE *, uint8_t *, int32_t,
                                           TR::RealRegister *);
  uint8_t *printEmitLoadIndirectPPCHelperAddrToCtr(TR::FILE *, uint8_t *,
                                                   TR::RealRegister *,
  ...
  uint8_t *print(TR::FILE *pOutFile, TR::PPCArrayCopyCallSnippet *snippet,
                 uint8_t *cursor);
  ...
  void print(TR::FILE *, TR_ARMOperand2 *op,
             TR_RegisterSizes size = TR_WordReg);
  ...
  void printS390RegisterDependency(TR::FILE *pOutFile, TR::Register *virtReg,
                                   int realReg, bool uses, bool defs);
  ...
  uint8_t *printRuntimeInstrumentationOnOffInstruction(
      TR::FILE *pOutFile, uint8_t *cursor, bool isRION,
...
#define TRACERA_INSTRUCTION_INSERTED (0x0002)
  ...
  const static uint32_t DEFAULT_NODE_LENGTH =
      82;  // Tree information printed to the right of node OPCode will be
           // aligned if node OPCode's length <= DEFAULT_NODE_LENGTH, or will
           // follow the node OPCode immediately (non-aligned) otherwise.
