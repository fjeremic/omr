...
static T* begin(T (&reqArray)[N])
{
  return &reqArray[0];
}
template<typename T, size_t N>
static T* end(T (&reqArray)[N])
{
  return &reqArray[0] + N;
}
  ...
  : _comp(comp)
{
  ...
  TR::BlockValidationRule* temp_block_rules[] = { new (
    comp->trHeapMemory()) TR::ValidateNodeRefCountWithinBlock(_comp) };
  ...
  _methodValidationRules.assign(begin(temp_method_rules),
                                end(temp_method_rules));
...
TR::ILValidator::getRequiredMethodValidationRules(
  const OMR::ILValidationStrategy* strategy)
{
  ...
  while (strategy->id != OMR::endRules) {
    for (auto it = _methodValidationRules.begin();
         it != _methodValidationRules.end();
         ++it) {
      /**
       * Each *ValidationRule has a unique `id`. These ids are defined in
       * ILValidationStrategies.hpp and they are assigned in
       * ILValidationRules.cpp.
       */
...
TR::ILValidator::getRequiredBlockValidationRules(
  const OMR::ILValidationStrategy* strategy)
{
  std::vector<TR::BlockValidationRule*> reqBlockValidationRules;
  while (strategy->id != OMR::endRules) {
    for (auto it = _blockValidationRules.begin();
         it != _blockValidationRules.end();
         ++it) {
      if (strategy->id == (*it)->id()) {
...
TR::ILValidator::getRequiredNodeValidationRules(
  const OMR::ILValidationStrategy* strategy)
{
  std::vector<TR::NodeValidationRule*> reqNodeValidationRules;
  while (strategy->id != OMR::endRules) {
    for (auto it = _nodeValidationRules.begin();
         it != _nodeValidationRules.end();
         ++it) {
      if (strategy->id == (*it)->id()) {
...
void
TR::ILValidator::validate(const OMR::ILValidationStrategy* strategy)
{
  /**
   * Selection Phase:
  ...
  for (auto it = reqMethodValidationRules.begin();
       it != reqMethodValidationRules.end();
       ++it) {
    (*it)->validate(methodSymbol);
  }

  /* Checks performed across an extended blocks. */
  for (auto it = reqBlockValidationRules.begin();
       it != reqBlockValidationRules.end();
       ++it) {
  ...
  for (auto it = reqNodeValidationRules.begin();
       it != reqNodeValidationRules.end();
       ++it) {
    for (TR::PreorderNodeIterator nodeIter(
           methodSymbol->getFirstTreeTop(), comp(), "NODE_VALIDATOR");
         nodeIter.currentTree();
         ++nodeIter) {
...
TR::ILValidator*
TR::createILValidatorObject(TR::Compilation* comp)
{
