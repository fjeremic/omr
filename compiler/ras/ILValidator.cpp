...
const OMR::ILValidationStrategy *TR::omrValidationStrategies[] = {
    OMR::emptyStrategy, OMR::preCodegenValidationStrategy,
 ...
 *
 **************************************************************************/
template <typename T, size_t N> static T *begin(T (&reqArray)[N]) {
  return &reqArray[0];
}
template <typename T, size_t N> static T *end(T (&reqArray)[N]) {
  return &reqArray[0] + N;
}

TR::ILValidator::ILValidator(TR::Compilation *comp) : _comp(comp) {
  /**
...
TR::ILValidator::getRequiredMethodValidationRules(
  ...
  while (strategy->id != OMR::endRules) {
    for (auto it = _methodValidationRules.begin();
         it != _methodValidationRules.end(); ++it) {
      /**
...
TR::ILValidator::getRequiredBlockValidationRules(
  ...
  while (strategy->id != OMR::endRules) {
    for (auto it = _blockValidationRules.begin();
         it != _blockValidationRules.end(); ++it) {
      if (strategy->id == (*it)->id()) {
...
TR::ILValidator::getRequiredNodeValidationRules(
  ...
  while (strategy->id != OMR::endRules) {
    for (auto it = _nodeValidationRules.begin();
         it != _nodeValidationRules.end(); ++it) {
      if (strategy->id == (*it)->id()) {
...
void TR::ILValidator::validate(const OMR::ILValidationStrategy *strategy) {
  ...
  for (auto it = reqNodeValidationRules.begin();
       it != reqNodeValidationRules.end(); ++it) {
    for (TR::PreorderNodeIterator nodeIter(methodSymbol->getFirstTreeTop(),
                                           comp(), "NODE_VALIDATOR");
