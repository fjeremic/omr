...
static char optionCategories[] = {
    ' ', 'C', 'O', 'L', 'D',
    'R', 'I', 'M', 0};  // Must match categories[] in codegen.dev/Options.cpp
...
TR::FILE *TR_Debug::findLogFile(TR::Options *cmdLineOptions,
  ...
  if (fileName && !STRICMP(logFileName, fileName))
    ...
    for (TR::OptionSet *prev = cmdLineOptions->getFirstOptionSet();
         prev && prev != optSet; prev = prev->getNext()) {
...
TR::FILE *TR_Debug::findLogFile(TR::Options *aotCmdLineOptions,
...
// Search all options sets, jitCmdLineOptions and aotCmdLineOptions to see if
// they have speciafied a log file with the given name. All options that match
// are returned in optionsArray. if the size of the array is insufficient to
// hold them all, the required size is returned so that the caller can provide a
// bigger array
int32_t TR_Debug::findLogFile(const char *logFileName,
...
void TR_Debug::findLogFile(const char *logFileName, TR::Options *cmdOptions,
                           TR::Options **optionsArray, int32_t arraySize,
                           int32_t &index) {
  if (!cmdOptions) return;
  if (cmdOptions->getLogFileName() &&
      !STRICMP(logFileName, cmdOptions->getLogFileName())) {
    if (index < arraySize) optionsArray[index] = cmdOptions;
    index++;
  }
  for (TR::OptionSet *optSet = cmdOptions->getFirstOptionSet(); optSet;
       optSet = optSet->getNext()) {
    if (optSet->getOptions()->getLogFileName() &&
        !STRICMP(logFileName, optSet->getOptions()->getLogFileName())) {
      if (index < arraySize) optionsArray[index] = optSet->getOptions();
      index++;
    }
  }
}
...
void TR_Debug::dumpOptionHelp(TR::OptionTable *firstOjit,
                              TR::OptionTable *firstOfe,
  ...
  for (int32_t cat = 0; optionCategories[cat]; cat++) {
    ...
    //
    if (optionCategories[cat] == 'I') continue;

    TR::OptionTable *ojit = firstOjit;
    TR::OptionTable *ofe = firstOfe;
    bool entryFound = false;
    while (ojit->name || ofe->name) {
      if (ojit->name && (ojit->helpText == NULL ||
                         ojit->helpText[0] != optionCategories[cat] ||
                         (regexExcludes(nameFilter, ojit->name) &&
                          regexExcludes(nameFilter, ojit->helpText)))) {
        ojit++;
        continue;
      }
      if (ofe->name &&
          (ofe->helpText == NULL || ofe->helpText[0] != optionCategories[cat] ||
           (regexExcludes(nameFilter, ofe->name) &&
            regexExcludes(nameFilter, ofe->helpText)))) {
        ofe++;
        continue;
      }
      if (!ojit->name)
      ...
      //
      if (!entry->length) entry->length = strlen(entry->name);
      ...
      if (i > 1) TR_VerboseLog::write("%.*s", i - 1, entry->helpText + 1);
      currentColumn += i - 1;
      ...
      while (entry->helpText[i]) {
        ...
        if ((i - start) >= (optionLineWidth - DESCRIPTION_START_COLUMN)) {
          if (lastWordBreak == start) lastWordBreak = i;
          TR_VerboseLog::write("%.*s", lastWordBreak - start,
                               entry->helpText + start);
        ...
        if (entry->helpText[i] == ' ') lastWordBreak = i;
        i++;
      }
...
void TR_Debug::dumpOptions(char *optionsType, char *options, char *envOptions,
                           TR::Options *cmdLineOptions, TR::OptionTable *ojit,
                           TR::OptionTable *ofe, void *feBase,
                           TR_FrontEnd *fe) {
  TR::OptionTable *entry;
  char *base;
  ...
  if (cmdLineOptions->getVerboseOption(TR_VerboseExtended)) {
    extendCheck = true;
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "%s type: Testarossa (Full) ",
                             optionsType);
  ...
  if (envOptions) {
    if (*options) TR_VerboseLog::write(",");
  ...
  while (ojit->name || ofe->name) {
    if (ojit->name &&
        (!ojit->msg ||
         (ojit->msg[0] == 'F' && !(ojit->msgInfo & OPTION_FOUND))) &&
        !ojit->enabled) {
      ojit++;
      continue;
    }
    if (ofe->name &&
        (!ofe->msg || (ofe->msg[0] == 'F' && !(ofe->msgInfo & OPTION_FOUND))) &&
        !ofe->enabled) {
      ofe++;
      continue;
    }
    ...
    if (entry->fcn == TR::Options::setBit) {
      ...
      if (entry->msg[0] == 'P') printIt = (value == entry->parm2);
      ...
      if (entry->msg[0] == 'P') printIt = (value != 0);
    } else if (entry->fcn == TR::Options::set32BitNumeric ||
               entry->fcn == TR::Options::set32BitSignedNumeric ||
      ...
      value =
          ((TR::Options *)base)->isDisabled((OMR::Optimizations)entry->parm1);
      printIt = (value != 0);
    ...
    if (printIt && (entry->msg[0] == 'F' || extendCheck)) {
      ...
      if (regex)
        ...
        // since java uses different function, we need to check for that to get
        // our verbose options printed
        TR_VerboseLog::write("{");
        base = (char *)cmdLineOptions;
        char *sep = "";
        for (int i = 0; i < TR_NumVerboseOptions; i++) {
          ...
          if (cmdLineOptions->getVerboseOption(flag)) {
            TR_VerboseLog::write("%s%s", sep,
                                 cmdLineOptions->getVerboseOptionName(flag));
            sep = "|";
          }
        }
        TR_VerboseLog::write("}");
  ...
  if (fej9->generateCompressedPointers()) {
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     ");
    TR_VerboseLog::write("compressedRefs shiftAmount=%d",
                         TR::Compiler->om.compressedReferenceShift());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     ");
    TR_VerboseLog::write("compressedRefs isLowMemHeap=%d",
