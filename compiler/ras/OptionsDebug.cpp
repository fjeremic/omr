...
static char optionCategories[] = {
  ' ', 'C', 'O', 'L', 'D', 'R', 'I', 'M', 0
}; // Must match categories[] in codegen.dev/Options.cpp
...
TR::FILE*
TR_Debug::findLogFile(TR::Options* cmdLineOptions,
                      TR::OptionSet* optSet,
                      char* logFileName)
{
...
TR::FILE*
TR_Debug::findLogFile(TR::Options* aotCmdLineOptions,
...
// bigger array
int32_t
TR_Debug::findLogFile(const char* logFileName,
...
static bool
regexExcludes(TR::SimpleRegex* regex, const char* string)
{
  if (regex)
...
void
TR_Debug::dumpOptionHelp(TR::OptionTable* firstOjit,
  ...
  for (int32_t cat = 0; optionCategories[cat]; cat++) {
    ...
    while (ojit->name || ofe->name) {
      ...
      if (currentColumn < DESCRIPTION_START_COLUMN)
        TR_VerboseLog::write(
          "%*s", DESCRIPTION_START_COLUMN - currentColumn, " ");
      else {
        TR_VerboseLog::writeLine(
          TR_Vlog_INFO, "%*s", DESCRIPTION_START_COLUMN, " ");
      }
      ...
      while (entry->helpText[i]) {
        ...
        if ((i - start) >= (optionLineWidth - DESCRIPTION_START_COLUMN)) {
          if (lastWordBreak == start)
            lastWordBreak = i;
          TR_VerboseLog::write(
            "%.*s", lastWordBreak - start, entry->helpText + start);
...
void
TR_Debug::dumpOptions(char* optionsType,
                      char* options,
                      char* envOptions,
                      ...
                      TR_FrontEnd* fe)
{
  ...
  if (cmdLineOptions->getVerboseOption(TR_VerboseExtended)) {
    extendCheck = true;
    TR_VerboseLog::writeLine(
      TR_Vlog_INFO, "%s type: Testarossa (Full) ", optionsType);
      ...
      if (regex)
        ...
        for (int i = 0; i < TR_NumVerboseOptions; i++) {
          ...
          if (cmdLineOptions->getVerboseOption(flag)) {
            TR_VerboseLog::write(
              "%s%s", sep, cmdLineOptions->getVerboseOptionName(flag));
            sep = "|";
          }
        }
        TR_VerboseLog::write("}");
