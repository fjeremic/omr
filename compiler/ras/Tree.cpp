...
// Prints the symbol reference table showing only symbols that were added during
// the last optimization and also sets TR::Compilationl::_prevSymRefTabSize to
// the current size of symbol reference table as it is the end of this
// optimization.
//
void TR_Debug::printSymRefTable(TR::FILE *pOutFile, bool printFullTable) {
  ...
  if (printFullTable) _comp->setPrevSymRefTabSize(0);

  if (currSymRefTabSize > 0 &&
      _comp->getPrevSymRefTabSize() <
          currSymRefTabSize)  // entry(ies) have been added to symbol refetence
                              // table since last optimization was performed
  {
    if (printFullTable)
      trfprintf(pOutFile, "\nSymbol References:\n------------------\n");
    else
    ...
    for (int i = _comp->getPrevSymRefTabSize(); i < currSymRefTabSize; i++) {
      if ((symRefIterator = symRefTab->getSymRef(i))) {
        output.reset();
        print(symRefIterator, output, false /*hideHelperMethodInfo*/,
              true /*verbose*/);
...
void TR_Debug::printOptimizationHeader(const char *funcName,
                                       const char *optName, int32_t optIndex,
                                       bool mustBeDone) {
  if (_file == NULL) return;

  trfprintf(_file, "<optimization id=%d name=%s method=%s>\n", optIndex,
            optName ? optName : "???", funcName);
  trfprintf(_file, "Performing %d: %s%s\n", optIndex, optName ? optName : "???",
            mustBeDone ? " mustBeDone" : "");
}
...
static bool valueIsProbablyHex(TR::Node *node) {
  switch (node->getDataType()) {
    case TR::Int16:
      ...
      return false;
    case TR::Int32:
      if (node->getInt() > 16384 || node->getInt() < -16384) return true;
      return false;
    case TR::Int64:
...
void TR_Debug::printLoadConst(TR::Node *node, TR_PrettyPrinterString &output) {
  ...
  switch (node->getDataType()) {
    ...
    case TR::Int64:
      if (isUnsigned) {
        if (valueIsProbablyHex(node))
          output.append(" " UINT64_PRINTF_FORMAT_HEX,
                        node->getUnsignedLongInt());
        else
    ...
    case TR::Double: {
      output.append(" %g [" UINT64_PRINTF_FORMAT_HEX "]", node->getDouble(),
                    node->getDoubleBits());
    } break;
    ...
    case TR::DecimalDouble: {
      output.append(" %g [" UINT64_PRINTF_FORMAT_HEX "]", node->getDouble(),
                    node->getDouble(), node->getDoubleBits());
    } break;
#ifdef SUPPORT_DFP
    ...
    case TR::Address:
      if (node->getAddress() == 0)
        output.append(" NULL");
      else if (!inDebugExtension() && _comp->getOption(TR_MaskAddresses))
        output.append(" *Masked*");
      else
        output.append(" " UINT64_PRINTF_FORMAT_HEX, node->getAddress());
      if (!inDebugExtension() && node->isClassPointerConstant()) {
        TR_OpaqueClassBlock *clazz = (TR_OpaqueClassBlock *)node->getAddress();
        int32_t len;
        char *sig = TR::Compiler->cls.classNameChars(_comp, clazz, len);
        if (clazz) {
...
void TR_Debug::print(TR::FILE *pOutFile, TR::CFG *cfg) {
  ...
  array = (TR::CFGNode **)cfg->comp()->trMemory()->allocateStackMemory(
      numNodes * sizeof(TR::CFGNode *));
  ...
  for (index = 0; index < numNodes; index++)
    if (array[index] != NULL) print(pOutFile, array[index], 6);
...
void TR_Debug::print(TR::FILE *outFile, TR_RegionAnalysis *regionAnalysis,
                     uint32_t indentation) {
  if (outFile == NULL) return;

  for (int32_t index = 0; index < regionAnalysis->_totalNumberOfNodes;
       index++) {
    TR_RegionAnalysis::StructInfo &node = regionAnalysis->getInfo(index);
    if (node._structure == NULL) continue;
...
void TR_Debug::print(TR::FILE *pOutFile, TR_Structure *structure,
                     uint32_t indentation) {
...
void TR_Debug::print(TR::FILE *pOutFile, TR_RegionStructure *regionStructure,
                     uint32_t indentation) {
  if (pOutFile == NULL) return;

  TR_RegionStructure *versionedLoop = NULL;
  if (!inDebugExtension() && debug("fullStructure"))
    ...
    if (regionStructure->containsInternalCycles())
      ...
      if (inDebugExtension())
        ...
        if (entryBlock->isCold())
          type =
              "Natural loop is the slow version of the fast versioned Natural "
              "loop ";
        // else if (entryBlock->isRare())
        //  type = "Natural loop is the slow version of the fast specialized
        //  Natural loop ";
        else
    ...
    if (versionedLoop)
      trfprintf(pOutFile, "%*s%d [%s] %s %d\n", indentation, " ",
                regionStructure->getNumber(), getName(regionStructure), type,
                versionedLoop->getNumber());
    else
  ...
  //
  if (!inDebugExtension())
    for (TR_InductionVariable *v = regionStructure->getFirstInductionVariable();
         v; v = v->getNext())
...
void TR_Debug::printSubGraph(TR::FILE *pOutFile,
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    ...
    if (node->getNumber() != node->getStructure()->getNumber()) {
      ...
      //
      trfprintf(pOutFile, "%*s%d(%d) -->", indentation + offset * 2, " ",
                node->getNumber(), node->getStructure()->getNumber());
    } else {
      trfprintf(pOutFile, "%*s(%s:%s)%d -->", indentation + offset * 2, " ",
                getName(node), getName(node->getStructure()),
                node->getNumber());
    }
    ...
    for (auto edge = node->getSuccessors().begin();
      ...
      if (regionStructure->isExitEdge(*edge)) trfprintf(pOutFile, "*");
    ...
    if (node->getStructure()->getParent() != regionStructure)
      trfprintf(
          pOutFile,
          "******* Structure %d does not refer back to its parent structure\n",
          node->getStructure()->getNumber());
  }

  ListElement<TR::CFGEdge> *firstExitEdge =
      regionStructure->getExitEdges().getListHead();
  ...
  for (exitEdge = firstExitEdge; exitEdge != NULL;
       exitEdge = exitEdge->getNextElement()) {
  ...
  if (!inDebugExtension()) {
    ...
    if (verbose) {
      ...
      si.reset();
      for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
        trfprintf(pOutFile, "%*s%d:", indentation + offset * 2, " ",
                  node->getNumber());
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR_InductionVariable *inductionVariable,
                     uint32_t indentation) {
  if (pOutFile == NULL) return;
  ...
  trfprintf(pOutFile, "%*sInduction variable [%s]\n", indentation, " ",
            getName(inductionVariable->getLocal()));
...
void TR_Debug::printBaseInfo(TR::FILE *pOutFile, TR_Structure *structure,
                             uint32_t indentation) {
  if (pOutFile == NULL) return;

  // Do indentation and dump the address of this block
  trfprintf(pOutFile, "%*s%d [%s] %s", indentation, " ", structure->getNumber(),
            getName(structure), structNames[structure->getKind()]);

  trfprintf(pOutFile, "\n");
}

void TR_Debug::print(TR::FILE *pOutFile, TR_BlockStructure *blockStructure,
                     uint32_t indentation) {
  if (pOutFile == NULL) return;
  ...
  if (blockStructure->getBlock()->getStructureOf() != blockStructure)
    trfprintf(pOutFile,
              "******* Block %d does not refer back to block structure\n",
              blockStructure->getBlock()->getNumber());
}

void TR_Debug::print(TR::FILE *pOutFile, TR::CFGNode *cfgNode,
                     uint32_t indentation) {
  if (cfgNode->asBlock())
...
void TR_Debug::print(TR::FILE *pOutFile, TR_StructureSubGraphNode *node,
                     uint32_t indentation) {
  print(pOutFile, node->getStructure(), indentation);
}

void TR_Debug::printNodesInEdgeListIterator(TR::FILE *pOutFile,
                                            TR::CFGEdgeList &li,
                                            bool fromNode) {
  TR::CFGEdge *edge;
  TR::Block *b;
...
void TR_Debug::print(TR::FILE *pOutFile, TR::Block *block,
                     uint32_t indentation) {
  if (pOutFile == NULL) return;

  // Do indentation and dump the address of this block
  trfprintf(pOutFile, "%*s", indentation, " ");
  if (block->getNumber() >= 0) trfprintf(pOutFile, "%4d ", block->getNumber());
  trfprintf(pOutFile, "[%s] ", getName(block));

  // If there are no nodes associated with this block, it must be the entry or
  // exit node.
  if (!block->getEntry()) {
    TR_ASSERT(!block->getExit(),
              "both entry and exit must be specified for TR_Block");
    ...
    if (_comp->getOption(TR_TracePartialInlining)) {
      ...
      if (block->isUnsanitizeable()) trfprintf(pOutFile, "U, ");
      if (block->containsCall()) trfprintf(pOutFile, "C, ");
      if (block->isRestartBlock()) trfprintf(pOutFile, "R, ");
      if (block->isPartialInlineBlock()) trfprintf(pOutFile, "P, ");
    }
  ...
  printNodesInEdgeListIterator(pOutFile, block->getPredecessors(),
                               fromNode = true);
  trfprintf(pOutFile, "]\n");

  // Dump successors
  trfprintf(pOutFile, "%*sout       = [", indentation + 11, " ");
  printNodesInEdgeListIterator(pOutFile, block->getSuccessors(),
                               fromNode = false);
  trfprintf(pOutFile, "]\n");

  // Dump exception predecessors
  trfprintf(pOutFile, "%*sexception in  = [", indentation + 11, " ");
  printNodesInEdgeListIterator(pOutFile, block->getExceptionPredecessors(),
                               fromNode = true);
  trfprintf(pOutFile, "]\n");

  // Dump exception successors
  trfprintf(pOutFile, "%*sexception out = [", indentation + 11, " ");
  printNodesInEdgeListIterator(pOutFile, block->getExceptionSuccessors(),
                               fromNode = false);
  trfprintf(pOutFile, "]\n");
}

static void printInlinePath(TR::FILE *pOutFile, TR_InlinedCallSite *site,
                            TR_Debug *debug) {
...
void TR_Debug::printIRTrees(TR::FILE *pOutFile, const char *title,
                            TR::ResolvedMethodSymbol *methodSymbol) {
  if (pOutFile == NULL) return;

  if (!methodSymbol) methodSymbol = _comp->getMethodSymbol();
  ...
  if (_comp->getMethodSymbol() == methodSymbol &&
      _comp->getNumInlinedCallSites() > 0) {
    trfprintf(pOutFile, "\nCall Stack Info\n", title, sig);
    ...
    for (int32_t i = 0; i < _comp->getNumInlinedCallSites(); ++i) {
      ...
      trfprintf(pOutFile, "    %4d       %4d       %5d       ", i,
                ics._byteCodeInfo.getCallerIndex(),
                ics._byteCodeInfo.getByteCodeIndex());

      TR::KnownObjectTable *knot = _comp->getKnownObjectTable();
      TR::KnownObjectTable::Index targetMethodHandleIndex =
          TR::KnownObjectTable::UNKNOWN;
      if (knot && meth && meth->convertToMethod()->isArchetypeSpecimen() &&
          meth->getMethodHandleLocation())
        targetMethodHandleIndex =
            knot->getExistingIndexAt(meth->getMethodHandleLocation());
      ...
      trfprintf(
          ...
          _comp->compileRelocatableCode()
              ? ((TR_AOTMethodInfo *)ics._methodInfo)
                    ->resolvedMethod->signature(comp()->trMemory(), heapAlloc)
              : fe()->sampleSignature(ics._methodInfo, 0, 0,
                                      _comp->trMemory()));
  ...
  for (; tt; tt = tt->getNextTreeTop()) {
    ...
    if (_comp->getOption(TR_TraceLiveness) && methodSymbol->getAutoSymRefs() &&
      ...
      for (int32_t i = _comp->getSymRefTab()->getIndexOfFirstSymRef();
           i < _comp->getSymRefTab()->getNumSymRefs(); i++) {
        TR::SymbolReference *symRef = _comp->getSymRefTab()->getSymRef(i);
        if (symRef && symRef->getSymbol()->isAutoOrParm() &&
            liveLocals->isSet(symRef->getSymbol()
...
void TR_Debug::printBlockOrders(TR::FILE *pOutFile, const char *title,
                                TR::ResolvedMethodSymbol *methodSymbol) {
  TR::TreeTop *tt = methodSymbol->getFirstTreeTop();
...
char *TR_Debug::formattedString(char *buf, uint32_t bufLen, const char *format,
                                va_list args,
...
int32_t TR_Debug::print(TR::FILE *pOutFile, TR::Node *node,
                        uint32_t indentation, bool printChildren) {
  ...
  if (_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (node->getOpCode().isLoadConst()) printLoadConst(pOutFile, node);
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (!debug("disableDumpNodeFlags")) printNodeFlags(pOutFile, node);
  ...
  if (printChildren) {
    ...
    if (node->getOpCode().isSwitch()) {
      ...
      printBasicPreNodeInfoAndIndent(pOutFile, node->getSecondChild(),
                                     indentation);
      nodeCount++;
      ...
      printDestination(pOutFile,
                       node->getSecondChild()->getBranchDestination());
      ...
      if (node->getOpCodeValue() == TR::lookup ||
          node->getOpCodeValue() == TR::trtLookup) {
        bool unsigned_case = node->getFirstChild()->getOpCode().isUnsigned();

        TR::Node *oldParent = getCurrentParent();
        int32_t oldChildIndex = getCurrentChildIndex();
        for (i = 2; i < upperBound; i++) {
          printBasicPreNodeInfoAndIndent(pOutFile, node->getChild(i),
                                         indentation);
          nodeCount++;
          ...
          if (node->getOpCodeValue() == TR::trtLookup) {
            ...
            printDestination(pOutFile,
                             node->getChild(i)->getBranchDestination());
          } else {
            if (sizeof(CASECONST_TYPE) == 8) {
              if (unsigned_case)
                output.append(INT64_PRINTF_FORMAT_HEX ":\t",
                              node->getChild(i)->getCaseConstant());
              else
                output.append(INT64_PRINTF_FORMAT ":\t",
                              node->getChild(i)->getCaseConstant());
            } else {
              if (unsigned_case)
        ...
        if (upperBound < node->getNumChildren() &&
          ...
          nodeCount +=
              print(pOutFile, node->getChild(upperBound), indentation, true);
        }
      }
    } else {
      TR::Node *oldParent = getCurrentParent();
...
uint32_t TR_Debug::getNumSpacesAfterIndex(uint32_t index,
                                          uint32_t maxIndexLength) const {
...
// Return the number of nodes encountered.
//
int32_t TR_Debug::printWithFixedPrefix(TR::FILE *pOutFile, TR::Node *node,
                                       uint32_t indentation, bool printChildren,
                                       bool printRefCounts,
                                       const char *prefix) {
  uint32_t numSpaces, globalIndex;

  TR_PrettyPrinterString globalIndexPrefix(this), output(this);
  ...
  if (_nodeChecklist.isSet(node->getGlobalIndex())) {
    if (printRefCounts) {
      trfprintf(pOutFile, "%s%s%dn%*s  (%3d) %*s==>%s", prefix,
                globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
      ...
      trfprintf(pOutFile, "%s%s%dn%*s  %*s==>%s", prefix,
                globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
    ...
    if (_comp->cg()->getAppendInstruction() != NULL &&
        node->getDataType() != TR::NoType && node->getRegister() != NULL) {
  ...
  if (printRefCounts)
    trfprintf(pOutFile, "%s%s%dn%*s  (%3d) %*s", prefix,
              globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
              node->getReferenceCount(), indentation, " ");
  else
    trfprintf(pOutFile, "%s%s%dn%*s  %*s", prefix, globalIndexPrefix.getStr(),
              globalIndex, numSpaces, "", indentation, " ");
  ...
  if (printChildren) {
    ...
    if (node->getOpCode().isSwitch()) {
      trfprintf(pOutFile, "\n");
      nodeCount +=
          printWithFixedPrefix(pOutFile, node->getFirstChild(), indentation,
                               true, printRefCounts, prefix);
      ...
      if (node->getSecondChild()->getNumChildren() == 1)  // a GlRegDep
        nodeCount += printWithFixedPrefix(
            pOutFile, node->getSecondChild()->getFirstChild(),
            indentation + DEFAULT_INDENT_INCREMENT, true, printRefCounts,
            prefix);
      ...
      if (node->getOpCodeValue() == TR::lookup ||
        ...
        for (i = 2; i < upperBound; i++) {
          ...
          numSpaces =
              getNumSpacesAfterIndex(globalIndex, MAX_GLOBAL_INDEX_LENGTH);
          ...
          if (node->getChild(i)->getNumChildren() == 1)  // a GlRegDep
            nodeCount += printWithFixedPrefix(
                pOutFile, node->getChild(i)->getFirstChild(),
                indentation + DEFAULT_INDENT_INCREMENT, true, printRefCounts,
                prefix);
        }
      } else {
        ...
        if (upperBound < node->getNumChildren() &&
            node->getChild(upperBound)->getOpCodeValue() != TR::GlRegDeps) {
          nodeCount +=
              printWithFixedPrefix(pOutFile, node->getChild(upperBound),
                                   indentation, true, printRefCounts, prefix);
        }
      }
    } else {
      TR::Node *oldParent = getCurrentParent();
      ...
      for (i = 0; i < node->getNumChildren(); i++) {
        ...
        nodeCount +=
            printWithFixedPrefix(pOutFile, node->getChild(i), indentation, true,
...
void TR_Debug::printDestination(TR::TreeTop *treeTop,
  ...
  TR::Block *block = node->getBlock();
  output.append(" --> ");
  if (block->getNumber() >= 0) output.append("block_%d", block->getNumber());
  output.append(" BBStart at n%dn", node->getGlobalIndex());
}

void TR_Debug::printBasicPreNodeInfoAndIndent(TR::FILE *pOutFile,
                                              TR::Node *node,
                                              uint32_t indentation) {
  uint32_t numSpaces;
  ...
  if (node->getOpCodeValue() == TR::BBStart &&
      ...
      node->getBlock()
          ->getPrevBlock())  // a block that is not an extension of previous
                             // block and is not the first block
  {
    trfprintf(pOutFile, "\n");
  }
  ...
  numSpaces =
      getNumSpacesAfterIndex(node->getGlobalIndex(), MAX_GLOBAL_INDEX_LENGTH);
  trfprintf(pOutFile, "%s%dn%*s %*s", globalIndexPrefix.getStr(),
            node->getGlobalIndex(), numSpaces, "", indentation, " ");

  _comp->setNodeOpCodeLength(0);
}

void TR_Debug::printBasicPostNodeInfo(TR::FILE *pOutFile, TR::Node *node,
                                      uint32_t indentation) {
  TR_PrettyPrinterString output(this);
  if (pOutFile == NULL) return;

  output.append("  ");
  if ((_comp->getNodeOpCodeLength() + indentation) <
      DEFAULT_NODE_LENGTH + DEFAULT_INDENT_INCREMENT)
    output.append("%*s",
                  DEFAULT_NODE_LENGTH + DEFAULT_INDENT_INCREMENT -
                      (_comp->getNodeOpCodeLength() + indentation),
                  "");

  int32_t lineNumber = -1;
  if (!inDebugExtension()) lineNumber = _comp->getLineNumber(node);
  ...
  if (!inDebugExtension() && _comp->getOptimizer() &&
      _comp->getOptimizer()->getValueNumberInfo())
    output.append(
        " vn=%d",
        _comp->getOptimizer()->getValueNumberInfo()->getValueNumber(node));
  else
    output.append(" vn=-");
...
void TR_Debug::printNodeInfo(TR::Node *node, TR_PrettyPrinterString &output,
                             bool prettyPrint) {
  TR_PrettyPrinterString globalIndexPrefix(this);
  ...
  if (!prettyPrint || (node->getOpCodeValue() != TR::BBStart &&
  ...
  if (node->getOpCode().isNullCheck()) {
    if (node->getNullCheckReference())
      output.append(" on %s%dn", globalIndexPrefix.getStr(),
                    node->getNullCheckReference()->getGlobalIndex());
    else
      output.append(" on null NullCheckReference ----- INVALID tree!!");
  } else if (node->getOpCodeValue() == TR::allocationFence) {
    if (node->getAllocation())
      output.append(" on %s%dn", globalIndexPrefix.getStr(),
                    node->getAllocation()->getGlobalIndex());
    else
  ...
  if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
    ...
    if (node->getOpCode()
            .isCheck())  // Omit helper method info for OpCodes that are checked
    ...
    if (node->getNumRelocations() > 0) {
      ...
      if (inDebugExtension())
        ...
        for (i = 0; i < node->getNumRelocations(); ++i)
          output.append(" " POINTER_PRINTF_FORMAT,
                        node->getRelocationDestination(i));
      }
      output.append(" ]");
    }
    ...
    if (block->isCatchBlock() && !inDebugExtension()) {
      ...
      if (classNameChars) {
        ...
        output.append(" (catches %.*s)", length,
    ...
    if (block->isLoopInvariantBlock()) output.append(" (loop pre-header)");
    TR_BlockStructure *blockStructure = block->getStructureOf();
    if (blockStructure) {
      if (!inDebugExtension() && _comp->getFlowGraph()->getStructure()) {
    ...
    if ((nextBlock = block->getNextBlock()) &&
        !nextBlock
             ->isExtensionOfPreviousBlock())  // end of a block that is not the
                                              // last block and the next block is
                                              // not an extension of it
      output.append(" =====");
  } else if (node->getOpCode().isArrayLength()) {
    int32_t stride = node->getArrayStride();

    if (stride > 0) output.append(" (stride %d)", stride);
  } else if (!inDebugExtension() && (node->getOpCode().isLoadReg() ||
                                     node->getOpCode().isStoreReg())) {
    if ((node->getType().isInt64() && TR::Compiler->target.is32Bit() &&
         !_comp->cg()->use64BitRegsOn32Bit()))
      output.append(" %s:%s ",
                    getGlobalRegisterName(node->getHighGlobalRegisterNumber()),
                    getGlobalRegisterName(node->getLowGlobalRegisterNumber()));
    else
      output.append(" %s ",
                    getGlobalRegisterName(node->getGlobalRegisterNumber()));

    if (node->getOpCode().isLoadReg())
      print(node->getRegLoadStoreSymbolReference(), output);
  } else if (!inDebugExtension() && node->getOpCodeValue() == TR::PassThrough) {
    // print only if under a GlRegDep
    bool isParentGlRegDep =
        getCurrentParent()
            ? (getCurrentParent()->getOpCodeValue() == TR::GlRegDeps)
            : false;
    if (isParentGlRegDep) {
      ...
      if ((node->getFirstChild()->getType().isInt64() &&
        ...
        output.append(" %s ", getGlobalRegisterName(
                                  node->getGlobalRegisterNumber(), size));
    }
  ...
  if (node->getOpCode().isLoadConst()) {
    ...
    if (getCurrentParent() &&
        getCurrentParent()->getOpCodeValue() == TR::newarray &&
...
void TR_Debug::printBCDNodeInfo(TR::Node *node,
                                TR_PrettyPrinterString &output) {
  if (node->getType().isBCD()) {
    if (node->getOpCode().isStore() || node->getOpCode().isCall() ||
        node->getOpCode().isLoadConst() ||
        (node->getOpCode().isConversion() &&
         !node->getOpCode().isConversionWithFraction())) {
      if (node->hasSourcePrecision()) {
        output.append(" <prec=%d (len=%d) srcprec=%d> ",
                      node->getDecimalPrecision(), node->getSize(),
                      node->getSourcePrecision());
      } else {
        output.append(" <prec=%d (len=%d)> ", node->getDecimalPrecision(),
                      node->getSize());
      }
    } else if (node->getOpCode().isLoad()) {
      output.append(" <prec=%d (len=%d) adj=%d> ", node->getDecimalPrecision(),
                    node->getSize(), node->getDecimalAdjust());
    } else if (node->canHaveSourcePrecision()) {
      output.append(
          " <prec=%d (len=%d) srcprec=%d %s=%d round=%d> ",
          node->getDecimalPrecision(), node->getSize(),
          node->getSourcePrecision(),
      ...
      output.append(
          " <prec=%d (len=%d) %s=%d round=%d> ", node->getDecimalPrecision(),
          node->getSize(),
          node->getOpCode().isConversionWithFraction() ? "frac" : "adj",
          node->getOpCode().isConversionWithFraction()
              ? node->getDecimalFraction()
    ...
    if (!node->getOpCode().isStore()) {
      ...
      if (node->hasKnownOrAssumedCleanSign() ||
        ...
        if (node->hasKnownOrAssumedCleanSign()) output.append("clean");
        if (node->hasKnownOrAssumedPreferredSign())
          output.append("%spreferred",
        ...
        if (node->hasKnownOrAssumedSignCode())
          output.append(
              ...
              node->hasKnownOrAssumedCleanSign() ||
                  ...
                  ? "/"
                  : "",
              getName(node->hasKnownSignCode() ? node->getKnownSignCode()
                                               : node->getAssumedSignCode()));
        output.append(") ");
      } else if (node->getOpCode().isLoad()) {
        output.append("%s ",
                      node->hasSignStateOnLoad() ? "hasState" : "noState");
      } else {
        output.append("? ");
      }
    }
...
void TR_Debug::printDFPNodeInfo(TR::FILE *pOutFile, TR::Node *node) {
  TR_PrettyPrinterString output(this);
  printDFPNodeInfo(node, output);
  trfprintf(pOutFile, "%s", output.getStr());
  _comp->incrNodeOpCodeLength(output.getLength());
...
void TR_Debug::printDFPNodeInfo(TR::Node *node,
                                TR_PrettyPrinterString &output) {
...
void TR_Debug::printVCG(TR::FILE *pOutFile, TR::CFG *cfg, const char *sig) {
  ...
  if (debug("nestedVCG")) {
    ...
    trfprintf(pOutFile,
              "node: {title: \"Top1\" label: \"%s\" vertical_order: 0 "
              "horizontal_order: 0 textcolor: blue borderwidth: 1}\n",
              sig);

    if (cfg->getStructure()) printVCG(pOutFile, cfg->getStructure());
    ...
    for (order = 1, treeTop = _comp->getStartTree(); treeTop;
...
void TR_Debug::printVCG(TR::FILE *pOutFile, TR_Structure *structure) {
  if (structure->asRegion()) printVCG(pOutFile, structure->asRegion());
}

void TR_Debug::printVCG(TR::FILE *pOutFile,
                        TR_RegionStructure *regionStructure) {
  trfprintf(pOutFile, "graph: {\n");
...
void TR_Debug::printVCG(TR::FILE *pOutFile, TR_StructureSubGraphNode *node,
                        bool isEntry) {
  if (_structureChecklist.isSet(node->getNumber())) return;
  ...
  if (isEntry) trfprintf(pOutFile, "vertical_order: 1 ");
...
void TR_Debug::printVCGEdges(TR::FILE *pOutFile,
                             TR_StructureSubGraphNode *node) {
  for (auto edge = node->getSuccessors().begin();
       edge != node->getSuccessors().end(); ++edge) {
  ...
  for (auto edge = node->getExceptionSuccessors().begin();
    ...
    printVCG(pOutFile, to, false);  // print it out if it is an exit destination
    trfprintf(pOutFile,
              "edge: { sourcename: \"%s\" targetname: \"%s\" color: pink}\n",
              getName(node), getName(to));
  }
}
...
//                                                      no-opt   cold    warm
//                                                      hot            veryhot
//                                                      scorching reducedWarm
//                                                      unknown
static const char *blockColours[numHotnessLevels] = {
    "white", "blue", "lightblue", "lightyellow",
    "gold",  "red",  "orange",    "white"};
static const char *edgeColours[numHotnessLevels] = {
    "black", "blue", "lightblue", "lightyellow",
    "gold",  "red",  "orange",    "black"};

void TR_Debug::printVCG(TR::FILE *pOutFile, TR::Block *block, int32_t vorder,
                        int32_t horder) {
  if (pOutFile == NULL) return;
  ...
  if (!block->getEntry()) {
    ...
    if (block->getPredecessors().empty())
      trfprintf(pOutFile,
                "vertical_order: 0 label: \"Entry\" shape: ellipse color: "
                "lightgreen ");
    else
    ...
    if (vorder != -1) trfprintf(pOutFile, "vertical_order: %d ", vorder);
    if (horder != -1) trfprintf(pOutFile, "horizontal_order: %d ", horder);
  }
  trfprintf(pOutFile, "}\n");
  ...
  for (auto edge = block->getSuccessors().begin();
    ...
    if (b->getNumber() >= 0) {
      trfprintf(pOutFile,
                "edge: { sourcename: \"%d\" targetname: \"%d\" color: %s}\n",
                block->getNumber(), b->getNumber(),
                edgeColours[unknownHotness]);
    }
  }
  ...
  for (auto edge = block->getExceptionSuccessors().begin();
    ...
    if (b->getNumber() >= 0) {
      trfprintf(pOutFile,
                "edge: { sourcename: \"%d\" targetname: \"%d\" linestyle: "
                "dotted label: \"exception\" color: %s }\n",
                block->getNumber(), b->getNumber(),
                edgeColours[unknownHotness]);
    }
  }
...
//
void TR_Debug::printVCG(TR::FILE *pOutFile, TR::Node *node,
                        uint32_t indentation) {
  if (pOutFile == NULL) return;
  ...
  if (_nodeChecklist.isSet(node->getGlobalIndex())) {
    trfprintf(pOutFile, "%*s==>%s at %s\\n", 12 + indentation, " ",
              getName(node->getOpCode()), getName(node));
    return;
  }
...
const char *TR_Debug::getName(TR::DataType type) {
...
int32_t childTypes[] = {
    ...
    TR::Address,  // TR::compressedRefs   FIXME:IL: shouldn't second arg be
                  // TR::Int64?  Type info should be like TR::aladd
    ...
    TR::ZonedDecimalSignTrailingSeparate |
        (TR::Address << 8),  // TR::zdstsLoadi
    ...
    TR::ZonedDecimalSignLeadingEmbedded |
        (TR::Address << 8),  // TR::zdsleStorei
    TR::ZonedDecimalSignLeadingSeparate |
        (TR::Address << 8),  // TR::zdslsStorei
    TR::ZonedDecimalSignTrailingSeparate |
        (TR::Address << 8),  // TR::zdstsStorei
...
const char *TR_Debug::getName(TR::ILOpCodes opCode) {
  return TR::ILOpCode(opCode).getName();
}

const char *TR_Debug::getName(TR::ILOpCode opCode) { return opCode.getName(); }

void TR_Debug::verifyGlobalIndices(TR::Node *node,
                                   TR::Node **nodesByGlobalIndex) {
  TR::Node *nodeFromArray = nodesByGlobalIndex[node->getGlobalIndex()];
  if (nodeFromArray == node) return;
  TR_ASSERT(nodeFromArray == NULL,
            "Found two nodes with the same global index: %p and %p",
            nodeFromArray, node);
...
void TR_Debug::verifyTrees(TR::ResolvedMethodSymbol *methodSymbol) {
  ...
  TR::Node **nodesByGlobalIndex =
      (TR::Node **)_comp->trMemory()->allocateStackMemory(size);
...
// accumulated in the node's localIndex. In pass 2 the reference count is
// checked.
//
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
      ...
      if (node->getOpCodeValue() == TR::multianewarray &&
        ...
        if (checkType & mask) expectedType = checkType >> (8 * (i + 1));
      }
      expectedType &= 0xFF;
      if (debug("checkTypes") && expectedType != TR::NoType) {
        ...
        if (childType != expectedType && childType != TR::NoType &&
          ...
          if (getFile() != NULL) {
            trfprintf(getFile(),
                      "TREE VERIFICATION ERROR -- node [%s] has wrong type for "
                      "child [%s] (%s), expected %s\n",
                      getName(node), getName(child), getName(childType),
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (isTreeTop) {
      if (node->getReferenceCount() != 0) {
        if (getFile() != NULL)
          trfprintf(getFile(),
                    "TREE VERIFICATION ERROR -- treetop node [%s] with ref "
                    "count %d\n",
    ...
    if (node->getReferenceCount() > 1 &&
        (node->getOpCodeValue() == TR::call ||
         node->getOpCodeValue() == TR::calli)) {
      if (getFile() != NULL)
        trfprintf(getFile(),
                  "TREE VERIFICATION ERROR -- void call node [%s] with ref "
                  "count %d\n",
                  getName(node), node->getReferenceCount());
      TR_ASSERT(debug("fixTrees"), "Tree verification error");
    }
    ...
    if (node->getReferenceCount() != node->getLocalIndex()) {
      if (getFile() != NULL)
        trfprintf(getFile(),
                  "TREE VERIFICATION ERROR -- node [%s] ref count is %d and "
                  "should be %d\n",
                  getName(node), node->getReferenceCount(),
                  node->getLocalIndex());
      TR_ASSERT(debug("fixTrees"), "Tree verification error");
      // if there is logging, don't fix the ref count!
      if (getFile() == NULL) node->setReferenceCount(node->getLocalIndex());
    }
  }
}

TR::Node *TR_Debug::verifyFinalNodeReferenceCounts(
    TR::ResolvedMethodSymbol *methodSymbol) {
  _nodeChecklist.empty();

  TR::Node *firstBadNode = NULL;
  for (TR::TreeTop *tt = methodSymbol->getFirstTreeTop(); tt;
       tt = tt->getNextTreeTop()) {
    TR::Node *badNode = verifyFinalNodeReferenceCounts(tt->getNode());
    if (!firstBadNode) firstBadNode = badNode;
  }

  if (getFile() != NULL) trfflush(getFile());

  if (debug("enforceFinalNodeReferenceCounts")) {
    TR_ASSERT(!firstBadNode,
              "Node [%s] final ref count is %d and should be zero\n",
              getName(firstBadNode), firstBadNode->getReferenceCount());
  }
...
void TR_Debug::verifyBlocks(TR::ResolvedMethodSymbol *methodSymbol) {
  ...
  for (tt = methodSymbol->getFirstTreeTop(); tt;
       tt = exitTreeTop->getNextTreeTop()) {
    ...
    // go back to the start of the block, and check the localIndex to make sure
    // it is 0 do not walk the tree backwards as this causes huge stack usage in
    // verifyBlocksPass2
    _nodeChecklist.empty();
    for (tt = firstTreeTop; tt != exitTreeTop->getNextTreeTop();
         tt = tt->getNextTreeTop())
      verifyBlocksPass2(tt->getNode());
  }
}

// Child access within a Block verification. In pass 1, the verifyRefCount for
// each child is decremented for each visit.  The second pass is to make sure
// that the verifyRefCount is zero by the end of the block.
//
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
      ...
      if (_nodeChecklist.isSet(child->getGlobalIndex())) {
        ...
        // If the child has not yet been visited, set its localIndex and visit
        // it
...
void TR_Debug::verifyBlocksPass2(TR::Node *node) {
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (node->getLocalIndex() != 0) {
      char buffer[100];
      sprintf(buffer,
              "BLOCK VERIFICATION ERROR -- node [%s] accessed outside of its "
              "(extended) basic block: %d time(s)",
              getName(node), node->getLocalIndex());
      if (getFile() != NULL) trfprintf(getFile(), buffer);
