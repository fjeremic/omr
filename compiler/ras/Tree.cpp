...
void TR_Debug::printIRTrees(TR::FILE* pOutFile, const char* title, TR::ResolvedMethodSymbol* methodSymbol)
{
    ...
    if (_comp->getMethodSymbol() == methodSymbol && _comp->getNumInlinedCallSites() > 0) {
        ...
        for (int32_t i = 0; i < _comp->getNumInlinedCallSites(); ++i) {
            ...
            trfprintf(pOutFile, "%s\n",
                _comp->compileRelocatableCode() ? ((TR_AOTMethodInfo*)ics._methodInfo)->resolvedMethod->signature(comp()->trMemory(), heapAlloc) : fe()->sampleSignature(ics._methodInfo, 0, 0, _comp->trMemory()));
...
char* TR_Debug::formattedString(char* buf, uint32_t bufLen, const char* format, va_list args, TR_AllocationKind allocationKind)
{
    va_list args_copy;
...
void TR_Debug::printBasicPreNodeInfoAndIndent(TR::FILE* pOutFile, TR::Node* node, uint32_t indentation)
{
    ...
    if (node->getOpCodeValue() == TR::BBStart && !node->getBlock()->isExtensionOfPreviousBlock() && node->getBlock()->getPrevBlock()) //a block that is not an extension of previous block and is not the first block
    {
        trfprintf(pOutFile, "\n");
    }
...
void TR_Debug::printBCDNodeInfo(TR::Node* node, TR_PrettyPrinterString& output)
{
    if (node->getType().isBCD()) {
        if (node->getOpCode().isStore() || node->getOpCode().isCall() || node->getOpCode().isLoadConst() || (node->getOpCode().isConversion() && !node->getOpCode().isConversionWithFraction())) {
    ...
    if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
        ...
        for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) {
            ...
            if (debug("checkTypes") && expectedType != TR::NoType) {
                ...
                if (childType != expectedType && childType != TR::NoType && !((node->getOpCodeValue() == TR::imul || node->getOpCodeValue() == TR::ishl) && child->getOpCodeValue() == TR::loadaddr)) {
                    if (getFile() != NULL) {
                        trfprintf(getFile(),
