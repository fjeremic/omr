...
void TR_Debug::printSymRefTable(TR::FILE* pOutFile, bool printFullTable)
{
  ...
  if (currSymRefTabSize > 0
    && _comp->getPrevSymRefTabSize()
      < currSymRefTabSize) // entry(ies) have been added to symbol refetence table since last optimization was performed
  {
    if (printFullTable)
      trfprintf(pOutFile, "\nSymbol References:\n------------------\n");
    else
...
void TR_Debug::print(TR::FILE* pOutFile, TR_RegionStructure* regionStructure, uint32_t indentation)
{
  ...
  if (!inDebugExtension() && debug("fullStructure"))
    ...
    if (versionedLoop)
      ...
      trfprintf(
        pOutFile, "%*s%d [%s] %s\n", indentation, " ", regionStructure->getNumber(), getName(regionStructure), type);
  }
...
void TR_Debug::printSubGraph(TR::FILE* pOutFile, TR_RegionStructure* regionStructure, uint32_t indentation)
{
  ...
  for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
    ...
    if (node->getNumber() != node->getStructure()->getNumber()) {
      ...
      //
      trfprintf(
        pOutFile, "%*s%d(%d) -->", indentation + offset * 2, " ", node->getNumber(), node->getStructure()->getNumber());
    } else {
...
void TR_Debug::print(TR::FILE* pOutFile, TR_BlockStructure* blockStructure, uint32_t indentation)
{
  ...
  if (blockStructure->getBlock()->getStructureOf() != blockStructure)
    trfprintf(
      pOutFile, "******* Block %d does not refer back to block structure\n", blockStructure->getBlock()->getNumber());
}
...
void TR_Debug::printIRTrees(TR::FILE* pOutFile, const char* title, TR::ResolvedMethodSymbol* methodSymbol)
{
  ...
  for (; tt; tt = tt->getNextTreeTop()) {
    ...
    if (_comp->getOption(TR_TraceLiveness) && methodSymbol->getAutoSymRefs() && ttNode->getOpCodeValue() == TR::BBStart
  ...
  if (printChildren) {
    ...
    if (node->getOpCode().isSwitch()) {
      ...
      if (node->getSecondChild()->getNumChildren() == 1) // a GlRegDep
      {
        ...
        nodeCount
          += print(pOutFile, node->getSecondChild()->getFirstChild(), indentation + DEFAULT_INDENT_INCREMENT, true);
      }
      ...
      if (node->getOpCodeValue() == TR::lookup || node->getOpCodeValue() == TR::trtLookup) {
        ...
        for (i = 2; i < upperBound; i++) {
          ...
          if (node->getChild(i)->getNumChildren() == 1) // a GlRegDep
          {
            ...
            nodeCount
              += print(pOutFile, node->getChild(i)->getFirstChild(), indentation + DEFAULT_INDENT_INCREMENT, true);
          }
        }
        setCurrentParentAndChildIndex(oldParent, oldChildIndex);
      } else {
...
// Return the number of nodes encountered.
//
int32_t TR_Debug::printWithFixedPrefix(
  TR::FILE* pOutFile, TR::Node* node, uint32_t indentation, bool printChildren, bool printRefCounts, const char* prefix)
{
  ...
  if (_nodeChecklist.isSet(node->getGlobalIndex())) {
    if (printRefCounts) {
      trfprintf(pOutFile, "%s%s%dn%*s  (%3d) %*s==>%s", prefix, globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
      ...
      trfprintf(pOutFile, "%s%s%dn%*s  %*s==>%s", prefix, globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
  ...
  if (printRefCounts)
    trfprintf(pOutFile, "%s%s%dn%*s  (%3d) %*s", prefix, globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
      node->getReferenceCount(), indentation, " ");
  else
    trfprintf(
      pOutFile, "%s%s%dn%*s  %*s", prefix, globalIndexPrefix.getStr(), globalIndex, numSpaces, "", indentation, " ");
  ...
  if (printChildren) {
    ...
    if (node->getOpCode().isSwitch()) {
      ...
      trfprintf(pOutFile, "\n%s%s%dn%*s  %*s", prefix, globalIndexPrefix.getStr(), globalIndex, numSpaces, "",
        indentation, " ");
      nodeCount++;
      output.append("default ");
      _comp->incrNodeOpCodeLength(output.getLength());
      ...
      if (node->getOpCodeValue() == TR::lookup || node->getOpCodeValue() == TR::trtLookup) {
        ...
        if (upperBound < node->getNumChildren() && node->getChild(upperBound)->getOpCodeValue() != TR::GlRegDeps) {
          nodeCount
            += printWithFixedPrefix(pOutFile, node->getChild(upperBound), indentation, true, printRefCounts, prefix);
        }
      }
    } else {
      TR::Node* oldParent = getCurrentParent();
...
void TR_Debug::printBasicPreNodeInfoAndIndent(TR::FILE* pOutFile, TR::Node* node, uint32_t indentation)
{
  ...
  trfprintf(
    pOutFile, "%s%dn%*s %*s", globalIndexPrefix.getStr(), node->getGlobalIndex(), numSpaces, "", indentation, " ");
...
void TR_Debug::printBasicPostNodeInfo(TR::FILE* pOutFile, TR::Node* node, uint32_t indentation)
{
  ...
  if (lineNumber < 0) {
    output.append("%s=[%d,%d,-] rc=", bciOrLoc, node->getByteCodeInfo().getCallerIndex(),
      node->getByteCodeInfo().getByteCodeIndex());
    output.append("%d", node->getReferenceCount());
  } else {
    output.append("%s=[%d,%d,%d] rc=", bciOrLoc, node->getByteCodeInfo().getCallerIndex(),
  ...
  if (!inDebugExtension() && _comp->getOptimizer() && _comp->getOptimizer()->getValueNumberInfo())
...
void TR_Debug::printNodeInfo(TR::Node* node, TR_PrettyPrinterString& output, bool prettyPrint)
{
  ...
  if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
    ...
    if ((nextBlock = block->getNextBlock())
      && !nextBlock->isExtensionOfPreviousBlock()) // end of a block that is not the last block and the next block is
  ...
  } else if (!inDebugExtension() && (node->getOpCode().isLoadReg() || node->getOpCode().isStoreReg())) {
    if ((node->getType().isInt64() && TR::Compiler->target.is32Bit() && !_comp->cg()->use64BitRegsOn32Bit()))
      output.append(" %s:%s ", getGlobalRegisterName(node->getHighGlobalRegisterNumber()),
    ...
    if (isParentGlRegDep) {
      ...
      if ((node->getFirstChild()->getType().isInt64() && TR::Compiler->target.is32Bit()))
        output.append(" %s:%s ", getGlobalRegisterName(node->getHighGlobalRegisterNumber(), size),
          getGlobalRegisterName(node->getLowGlobalRegisterNumber(), size));
      else
        output.append(" %s ", getGlobalRegisterName(node->getGlobalRegisterNumber(), size));
    }
  ...
  if (node->getType().isBCD()) {
    if (node->getOpCode().isStore() || node->getOpCode().isCall() || node->getOpCode().isLoadConst()
      ...
      if (node->hasSourcePrecision()) {
        output.append(
          " <prec=%d (len=%d) srcprec=%d> ", node->getDecimalPrecision(), node->getSize(), node->getSourcePrecision());
      } else {
        output.append(" <prec=%d (len=%d)> ", node->getDecimalPrecision(), node->getSize());
      }
      ...
      output.append(" <prec=%d (len=%d) srcprec=%d %s=%d round=%d> ", node->getDecimalPrecision(), node->getSize(),
        node->getSourcePrecision(), node->getOpCode().isConversionWithFraction() ? "frac" : "adj",
        node->getOpCode().isConversionWithFraction() ? node->getDecimalFraction() : node->getDecimalAdjust(),
        node->getDecimalRound());
    } else {
      output.append(" <prec=%d (len=%d) %s=%d round=%d> ", node->getDecimalPrecision(), node->getSize(),
    ...
    if (!node->getOpCode().isStore()) {
      ...
      if (node->hasKnownOrAssumedCleanSign() || node->hasKnownOrAssumedPreferredSign()
        ...
        if (node->hasKnownOrAssumedSignCode())
          output.append("%s%s", node->hasKnownOrAssumedCleanSign() || node->hasKnownOrAssumedPreferredSign() ? "/" : "",
...
void TR_Debug::printDFPNodeInfo(TR::Node* node, TR_PrettyPrinterString& output)
{
...
void TR_Debug::printVCG(TR::FILE* pOutFile, TR::CFG* cfg, const char* sig)
{
  ...
  if (debug("nestedVCG")) {
    ...
    trfprintf(pOutFile,
      "node: {title: \"Top1\" label: \"%s\" vertical_order: 0 horizontal_order: 0 textcolor: blue borderwidth: 1}\n",
      sig);
        ...
        if (childType != expectedType && childType != TR::NoType
          ...
          if (getFile() != NULL) {
            trfprintf(getFile(),
              ...
              getName(child), getName(childType), getName((TR::DataTypes)expectedType));
          }
...
void TR_Debug::verifyTreesPass2(TR::Node* node, bool isTreeTop)
{
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (node->getReferenceCount() > 1 && (node->getOpCodeValue() == TR::call || node->getOpCodeValue() == TR::calli)) {
      if (getFile() != NULL)
    ...
    if (node->getReferenceCount() != node->getLocalIndex()) {
      if (getFile() != NULL)
        trfprintf(getFile(), "TREE VERIFICATION ERROR -- node [%s] ref count is %d and should be %d\n", getName(node),
          node->getReferenceCount(), node->getLocalIndex());
      TR_ASSERT(debug("fixTrees"), "Tree verification error");
