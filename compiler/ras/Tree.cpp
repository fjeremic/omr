...
TR_Debug::printBottomLegend(TR::FILE* pOutFile)
{
  ...
  trfprintf(pOutFile,
            "%s=[x,y,z]: byte-code-info [callee-index, %s, %s]\n",
            bciOrLoc,
            bciOrLocVerbose,
            lineOrStatement);
                           ...
                           // table since last optimization was performed
  {
    ...
    for (int i = _comp->getPrevSymRefTabSize(); i < currSymRefTabSize; i++) {
      if ((symRefIterator = symRefTab->getSymRef(i))) {
        output.reset();
        print(symRefIterator,
              output,
              false /*hideHelperMethodInfo*/,
              true /*verbose*/);
...
void
TR_Debug::printOptimizationHeader(const char* funcName,
                                  const char* optName,
                                  int32_t optIndex,
                                  bool mustBeDone)
{
  ...
  trfprintf(_file,
            "<optimization id=%d name=%s method=%s>\n",
            optIndex,
            optName ? optName : "???",
            funcName);
  trfprintf(_file,
...
static bool
valueIsProbablyHex(TR::Node* node)
{
...
TR_Debug::printLoadConst(TR::Node* node, TR_PrettyPrinterString& output)
{
  ...
  switch (node->getDataType()) {
    ...
    case TR::Double: {
      output.append(" %g [" UINT64_PRINTF_FORMAT_HEX "]",
                    node->getDouble(),
                    node->getDoubleBits());
    } break;
...
void
TR_Debug::print(TR::FILE* outFile,
...
void
TR_Debug::printSubGraph(TR::FILE* pOutFile,
                        TR_RegionStructure* regionStructure,
                        uint32_t indentation)
{
  if (pOutFile == NULL)
  ...
  if (!inDebugExtension()) {
    ...
    if (verbose) {
      ...
      for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
        trfprintf(
          pOutFile, "%*s%d:", indentation + offset * 2, " ", node->getNumber());
...
void
TR_Debug::printBaseInfo(TR::FILE* pOutFile,
                        TR_Structure* structure,
                        uint32_t indentation)
{
  if (pOutFile == NULL)
...
void
TR_Debug::printNodesInEdgeListIterator(TR::FILE* pOutFile,
                                       TR::CFGEdgeList& li,
                                       bool fromNode)
{
  TR::CFGEdge* edge;
...
TR_Debug::print(TR::FILE* pOutFile, TR::Block* block, uint32_t indentation)
{
  ...
  printNodesInEdgeListIterator(
    pOutFile, block->getPredecessors(), fromNode = true);
  trfprintf(pOutFile, "]\n");

  // Dump successors
  trfprintf(pOutFile, "%*sout       = [", indentation + 11, " ");
  printNodesInEdgeListIterator(
    pOutFile, block->getSuccessors(), fromNode = false);
  trfprintf(pOutFile, "]\n");

  // Dump exception predecessors
  trfprintf(pOutFile, "%*sexception in  = [", indentation + 11, " ");
  printNodesInEdgeListIterator(
    pOutFile, block->getExceptionPredecessors(), fromNode = true);
  trfprintf(pOutFile, "]\n");

  // Dump exception successors
  trfprintf(pOutFile, "%*sexception out = [", indentation + 11, " ");
  printNodesInEdgeListIterator(
    pOutFile, block->getExceptionSuccessors(), fromNode = false);
  trfprintf(pOutFile, "]\n");
}

static void
printInlinePath(TR::FILE* pOutFile, TR_InlinedCallSite* site, TR_Debug* debug)
{
...
void
TR_Debug::printIRTrees(TR::FILE* pOutFile,
                       const char* title,
  ...
  for (; tt; tt = tt->getNextTreeTop()) {
    ...
    if (_comp->getOption(TR_TraceLiveness) && methodSymbol->getAutoSymRefs() &&
      ...
      for (int32_t i = _comp->getSymRefTab()->getIndexOfFirstSymRef();
           i < _comp->getSymRefTab()->getNumSymRefs();
           i++) {
...
void
TR_Debug::printBlockOrders(TR::FILE* pOutFile,
                           const char* title,
                           TR::ResolvedMethodSymbol* methodSymbol)
{
...
char*
TR_Debug::formattedString(char* buf,
                          uint32_t bufLen,
                          const char* format,
                          va_list args,
...
void
TR_PrettyPrinterString::append(const char* format, ...)
{
  va_list args;
                ...
                bool printChildren)
{
  ...
  if (printChildren) {
    ...
    if (node->getOpCode().isSwitch()) {
      ...
      printBasicPreNodeInfoAndIndent(
        pOutFile, node->getSecondChild(), indentation);
      nodeCount++;
      ...
      if (node->getSecondChild()->getNumChildren() == 1) // a GlRegDep
      {
        ...
        nodeCount += print(pOutFile,
                           node->getSecondChild()->getFirstChild(),
                           indentation + DEFAULT_INDENT_INCREMENT,
                           true);
      }
      ...
      if (node->getOpCodeValue() == TR::lookup ||
        ...
        for (i = 2; i < upperBound; i++) {
          printBasicPreNodeInfoAndIndent(
            pOutFile, node->getChild(i), indentation);
          nodeCount++;
...
//
int32_t
TR_Debug::printWithFixedPrefix(TR::FILE* pOutFile,
                               TR::Node* node,
                               uint32_t indentation,
                               bool printChildren,
                               bool printRefCounts,
                               const char* prefix)
{
  ...
  if (printChildren) {
    ...
    if (node->getOpCode().isSwitch()) {
      trfprintf(pOutFile, "\n");
      nodeCount += printWithFixedPrefix(pOutFile,
                                        node->getFirstChild(),
                                        indentation,
                                        true,
                                        printRefCounts,
...
void
TR_Debug::printBasicPreNodeInfoAndIndent(TR::FILE* pOutFile,
                                         TR::Node* node,
                                         uint32_t indentation)
{
  uint32_t numSpaces;
  ...
  if (node->getOpCodeValue() == TR::BBStart &&
      ...
      node->getBlock()
        ->getPrevBlock()) // a block that is not an extension of previous block
...
void
TR_Debug::printBasicPostNodeInfo(TR::FILE* pOutFile,
                                 TR::Node* node,
                                 uint32_t indentation)
{
  TR_PrettyPrinterString output(this);
...
void
TR_Debug::printNodeInfo(TR::Node* node,
  ...
  if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
    ...
    if (block->isCatchBlock() && !inDebugExtension()) {
      ...
      if (classNameChars) {
        ...
        output.append(
          " (catches %.*s)", length, getName(classNameChars, length));
      } else {
        classNameChars = "...";
        length = 3;
        output.append(
    ...
    if (isParentGlRegDep) {
      ...
      if ((node->getFirstChild()->getType().isInt64() &&
        ...
        output.append(
          " %s ", getGlobalRegisterName(node->getGlobalRegisterNumber(), size));
    }
  ...
  if (node->getType().isBCD()) {
    if (node->getOpCode().isStore() || node->getOpCode().isCall() ||
      ...
      if (node->hasSourcePrecision()) {
        ...
        output.append(
          " <prec=%d (len=%d)> ", node->getDecimalPrecision(), node->getSize());
      }
...
void
TR_Debug::printVCG(TR::FILE* pOutFile,
                   TR_StructureSubGraphNode* node,
                   bool isEntry)
{
                   ...
                   int32_t horder)
{
  ...
  if (!block->getEntry()) {
    ...
    if (block->getPredecessors().empty())
      trfprintf(
        pOutFile,
        "vertical_order: 0 label: \"Entry\" shape: ellipse color: lightgreen ");
    else
...
TR_Debug::printVCG(TR::FILE* pOutFile, TR::Node* node, uint32_t indentation)
{
  ...
  if (node->getOpCode().isSwitch()) {
    trfprintf(
      pOutFile, "%*s ***can't print switches yet***\\n", indentation + 10, " ");
  } else {
    for (i = 0; i < node->getNumChildren(); i++)
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    if (isTreeTop) {
      if (node->getReferenceCount() != 0) {
        if (getFile() != NULL)
          trfprintf(
            getFile(),
            "TREE VERIFICATION ERROR -- treetop node [%s] with ref count %d\n",
            getName(node),
            node->getReferenceCount());
        TR_ASSERT(debug("fixTrees"), "Tree verification error");
    ...
    if (node->getReferenceCount() > 1 &&
      ...
      if (getFile() != NULL)
        trfprintf(
          getFile(),
          "TREE VERIFICATION ERROR -- void call node [%s] with ref count %d\n",
          getName(node),
          node->getReferenceCount());
      TR_ASSERT(debug("fixTrees"), "Tree verification error");
    }
