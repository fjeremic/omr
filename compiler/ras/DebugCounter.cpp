  ...
  TR::Compilation* comp, uint8_t* location, TR::Node* node, TR::DebugCounterBase* counter)
{
  ...
  TR::ExternalRelocation* r
    = new (comp->trHeapMemory()) TR::ExternalRelocation(location, (uint8_t*)counter, TR_DebugCounter, comp->cg());
...
TR::Node* TR::DebugCounterBase::createBumpCounterNode(TR::Compilation* comp, TR::Node* deltaNode)
{
  ...
  TR::Node* load
    = TR::Node::createWithSymRef(deltaNode, TR::Compiler->target.is64Bit() ? TR::lload : TR::iload, 0, symref);
  TR::Node* add = TR::Node::create(TR::Compiler->target.is64Bit() ? TR::ladd : TR::iadd, 2, load, deltaNode);
  TR::Node* store
    = TR::Node::createWithSymRef(TR::Compiler->target.is64Bit() ? TR::lstore : TR::istore, 1, 1, add, symref);
...
TR::SymbolReference* TR::DebugCounter::getBumpCountSymRef(TR::Compilation* comp)
{
  TR::SymbolReference* symRef = comp->getSymRefTab()->findOrCreateCounterSymRef(
    const_cast<char*>(_name), TR::Compiler->target.is64Bit() ? TR::Int64 : TR::Int32, &_bumpCount);
  ...
  TR::Compilation* comp, TR_OpaqueMethodBlock* method, int32_t bytecodeIndex, const char* (&counterNames)[3])
{
  ...
  if (insertByteCode) {
    counterNames[0] = TR::DebugCounter::debugCounterName(comp,
      comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byByteCode.(%s)=%d"), methodName,
      bytecodeIndex);
  }
  if (insertJittedBody) {
    counterNames[1] = TR::DebugCounter::debugCounterName(comp,
      comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byJittedBody.(%s).%s"), jittedBody,
      comp->getHotnessName());
  }
  if (insertMethod) {
    counterNames[2] = TR::DebugCounter::debugCounterName(
      comp, comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byMethod.(%s)"), methodName);
  }
}
