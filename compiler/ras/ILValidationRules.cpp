  ...
  for (TR::TreeTop *currentTree = start; currentTree != stop;
    ...
    if (next) {
      ...
      checkSoundnessCondition(
          currentTree, next->getPrevTreeTop() == currentTree,
      ...
      checkNodeSoundness(currentTree, currentTree->getNode(), ancestorNodes,
                         visitedNodes);
    }
  }
}

void TR::SoundnessRule::checkNodeSoundness(TR::TreeTop *location,
                                           TR::Node *node,
  ...
  for (int32_t i = 0; i < node->getNumChildren(); i++) {
    ...
    checkSoundnessCondition(location, child != NULL,
...
void TR::SoundnessRule::checkSoundnessCondition(TR::TreeTop *location,
                                                bool condition,
  ...
  if (!condition) {
    if (location && location->getNode()) {
      ...
      TR::printILDiagnostic(
          comp(), "*** VALIDATION ERROR: IL is unsound ***\nMethod: %s\n",
          comp()->signature());
    }
    va_list args;
...
void TR::ValidateLivenessBoundaries::validate(
  ...
  TR::TreeTop *start = methodSymbol->getFirstTreeTop();
  TR::TreeTop *stop = methodSymbol->getLastTreeTop();
  for (TR::PostorderNodeOccurrenceIterator iter(start, comp(),
                                                "VALIDATE_LIVENESS_BOUNDARIES");
       iter != stop; ++iter) {
    ...
    if (node->getOpCodeValue() == TR::BBEnd) {
      ...
      if (nextTree) {
        TR::checkILCondition(
            node, nextTree->getNode()->getOpCodeValue() == TR::BBStart, comp(),
...
void TR::ValidateLivenessBoundaries::validateEndOfExtendedBlockBoundary(
    TR::Node *node, TR::LiveNodeWindow &liveNodes) {
...
void TR::ValidateLivenessBoundaries::updateNodeState(
    TR::Node *node, TR::NodeSideTable<TR::NodeState> &nodeStates,
  ...
  if (node->getReferenceCount() == state._futureReferenceCount) {
    ...
    if (node->getReferenceCount() == 0) {
      TR::checkILCondition(
          node, node->getOpCode().isTreeTop(), comp(),
 ...
 */
void TR::ValidateNodeRefCountWithinBlock::validateRefCountPass1(
    TR::Node *node) {
...
void TR::ValidateNodeRefCountWithinBlock::validateRefCountPass2(
    TR::Node *node) {
  ...
  if (!_nodeChecklist.isSet(node->getGlobalIndex())) {
    ...
    TR::checkILCondition(
        node, node->getLocalIndex() == 0, comp(),
    ...
    for (auto i = 0; i < actChildCount; ++i) {
      ...
      if (childOpcode.getOpCodeValue() != TR::GlRegDeps) {
        ...
        TR::checkILCondition(
            ...
            comp(), "Child %d has unexpected type %s (expected %s)", i,
            actChildTypeName, expChildTypeName);
      } else {
        /**
         ...
         */
        TR::checkILCondition(
            node, opcode.canHaveGlRegDeps() && (i == actChildCount - 1), comp(),
