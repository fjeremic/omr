  ...
  for (TR::TreeTop* currentTree = start; currentTree != stop; currentTree = currentTree->getNextTreeTop()) {
    ...
    if (next) {
      ...
      checkSoundnessCondition(currentTree, next->getPrevTreeTop() == currentTree,
        "Doubly-linked treetop list must be consistent: n%dn->n%dn<-n%dn", currentTree->getNode()->getGlobalIndex(),
        next->getNode()->getGlobalIndex(), next->getPrevTreeTop()->getNode()->getGlobalIndex());
    } else {
...
void TR::ValidateLivenessBoundaries::validate(TR::ResolvedMethodSymbol* methodSymbol)
{
  ...
  TR::TreeTop* start = methodSymbol->getFirstTreeTop();
  TR::TreeTop* stop = methodSymbol->getLastTreeTop();
  for (TR::PostorderNodeOccurrenceIterator iter(start, comp(), "VALIDATE_LIVENESS_BOUNDARIES"); iter != stop; ++iter) {
...
void TR::ValidateLivenessBoundaries::validateEndOfExtendedBlockBoundary(TR::Node* node, TR::LiveNodeWindow& liveNodes)
{
  for (LiveNodeWindow::Iterator lnwi(liveNodes); lnwi.currentNode(); ++lnwi) {
    TR::checkILCondition(
      node, false, comp(), "Node cannot live across block boundary at n%dn", lnwi.currentNode()->getGlobalIndex());
  }
  /**
  ...
  TR::Node* node, TR::NodeSideTable<TR::NodeState>& nodeStates, TR::LiveNodeWindow& liveNodes)
{
  ...
  if (node->getReferenceCount() == state._futureReferenceCount) {
    ...
    if (node->getReferenceCount() == 0) {
      TR::checkILCondition(
        node, node->getOpCode().isTreeTop(), comp(), "Only nodes with isTreeTop opcodes can have refcount == 0");
    } else {
      liveNodes.add(node);
    }
  }
    ...
    for (auto i = 0; i < actChildCount; ++i) {
      ...
      if (childOpcode.getOpCodeValue() != TR::GlRegDeps) {
        ...
        const auto expChildTypeName
          = (expChildType >= TR::NumTypes) ? "UnspecifiedChildType" : TR::DataType::getName(expChildType);
        const auto actChildTypeName = TR::DataType::getName(actChildType);
        TR::checkILCondition(node, (expChildType >= TR::NumTypes || actChildType == expChildType), comp(),
         ...
         */
        TR::checkILCondition(
          node, opcode.canHaveGlRegDeps() && (i == actChildCount - 1), comp(), "Unexpected GlRegDeps child %d", i);
      }
    }
  }
}
...
void TR::Validate_ireturnReturnType::validate(TR::Node* node)
{
  ...
  if (opcode.getOpCodeValue() == TR::ireturn) {
    ...
    for (auto i = 0; i < childCount; ++i) {
      ...
      TR::checkILCondition(node, (actChildType == TR::Int32 || actChildType == TR::Int16 || actChildType == TR::Int8),
        comp(), "ireturn has an invalid child type %s (expected Int{8,16,32})", childTypeName);
    }
  }
}
