 ...
 *
 * NOTE: For adding a new ILValidationRule, please update
 * ILValidationStrategies.hpp. The `id` assigned in the new ILValidationRule
 * must then match the newly defined one in ILValidationStrategies.hpp (which is
 * a `OMR::ILValidationRule` value).
 */
...
void
TR::SoundnessRule::validate(TR::ResolvedMethodSymbol* methodSymbol)
{
  ...
  checkSoundnessCondition(
    stop, !stop || stop->getNode() != NULL, "Stop tree must have a node");

  TR::NodeChecklist treetopNodes(comp()), ancestorNodes(comp()),
    visitedNodes(comp());

  /* NOTE: Can't use iterators here, because iterators presuppose that the IL is
   * sound. */
  for (TR::TreeTop* currentTree = start; currentTree != stop;
       currentTree = currentTree->getNextTreeTop()) {
    checkSoundnessCondition(
      currentTree, currentTree->getNode() != NULL, "Tree must have a node");
    checkSoundnessCondition(currentTree,
                            !treetopNodes.contains(currentTree->getNode()),
    ...
    if (next) {
      ...
      checkSoundnessCondition(
        currentTree,
        next->getPrevTreeTop() == currentTree,
      ...
      checkSoundnessCondition(currentTree,
                              stop == NULL,
                              "Reached the end of the trees after n%dn without "
                              "encountering the stop tree n%dn",
                              currentTree->getNode()->getGlobalIndex(),
                              stop ? stop->getNode()->getGlobalIndex() : 0);
      checkNodeSoundness(
        currentTree, currentTree->getNode(), ancestorNodes, visitedNodes);
    }
  }
}

void
TR::SoundnessRule::checkNodeSoundness(TR::TreeTop* location,
                                      TR::Node* node,
                                      ...
                                      TR::NodeChecklist& visitedNodes)
{
  ...
  checkSoundnessCondition(location,
...
void
TR::SoundnessRule::checkSoundnessCondition(TR::TreeTop* location,
                                           bool condition,
                                           ...
                                           ...)
{
  if (!condition) {
    if (location && location->getNode()) {
      TR::printILDiagnostic(
        comp(),
        "*** VALIDATION ERROR: IL is unsound at n%dn ***\nMethod: %s\n",
        location->getNode()->getGlobalIndex(),
        comp()->signature());
    } else {
      TR::printILDiagnostic(
        comp(),
        "*** VALIDATION ERROR: IL is unsound ***\nMethod: %s\n",
        comp()->signature());
    }
    va_list args;
...
TR::ValidateLivenessBoundaries::ValidateLivenessBoundaries(
  TR::Compilation* comp)
  : TR::MethodValidationRule(comp, OMR::validateLivenessBoundaries)
{}

void
TR::ValidateLivenessBoundaries::validate(TR::ResolvedMethodSymbol* methodSymbol)
{
  /**
  ...
  for (TR::PostorderNodeOccurrenceIterator iter(
         start, comp(), "VALIDATE_LIVENESS_BOUNDARIES");
       iter != stop;
       ++iter) {
    ...
    if (node->getOpCodeValue() == TR::BBEnd) {
      ...
      if (nextTree) {
        TR::checkILCondition(node,
                             nextTree->getNode()->getOpCodeValue() ==
                               TR::BBStart,
                             comp(),
                             "Expected BBStart after BBEnd");
        isEndOfExtendedBlock =
          !nextTree->getNode()->getBlock()->isExtensionOfPreviousBlock();
      } else {
...
void
TR::ValidateLivenessBoundaries::validateEndOfExtendedBlockBoundary(
  TR::Node* node,
  TR::LiveNodeWindow& liveNodes)
{
  ...
  /**
   * At the end of an extended block, no node we've already seen could ever be
   * seen again. Slide the live node window to keep its bitvector compact.
   */
...
void
TR::ValidateLivenessBoundaries::updateNodeState(
  TR::Node* node,
  ...
  TR::LiveNodeWindow& liveNodes)
{
  ...
  if (TR::isILValidationLoggingEnabled(comp())) {
    if (!liveNodes.isEmpty()) {
      ...
      char* separator = "";
      for (TR::LiveNodeWindow::Iterator lnwi(liveNodes); lnwi.currentNode();
           ++lnwi) {
        traceMsg(
          comp(), "%sn%dn", separator, lnwi.currentNode()->getGlobalIndex());
        separator = ", ";
      }
      traceMsg(comp(), "}\n");
    }
 ...
 *
 */
TR::ValidateNodeRefCountWithinBlock::ValidateNodeRefCountWithinBlock(
  TR::Compilation* comp)
  : TR::BlockValidationRule(comp, OMR::validateNodeRefCountWithinBlock)
{}

void
TR::ValidateNodeRefCountWithinBlock::validate(TR::TreeTop* firstTreeTop,
                                              TR::TreeTop* exitTreeTop)
{
   ...
   * make sure it is 0.
   *
   * NOTE: Walking the tree backwards causes huge stack usage in
   * validateRefCountPass2.
   */
  _nodeChecklist.empty();
  for (TR::TreeTop* tt = firstTreeTop; tt != exitTreeTop->getNextTreeTop();
       tt = tt->getNextTreeTop()) {
 ...
 */
void
TR::ValidateNodeRefCountWithinBlock::validateRefCountPass1(TR::Node* node)
{
...
void
TR::ValidateNodeRefCountWithinBlock::validateRefCountPass2(TR::Node* node)
{
...
void
TR::ValidateChildCount::validate(TR::Node* node)
{
  ...
  if (opcode.expectedChildCount() != ILChildProp::UnspecifiedChildCount) {
    ...
    if (!opcode.canHaveGlRegDeps()) {
      ...
      TR::checkILCondition(
        node,
        node->getChild(actChildCount - 1)->getOpCodeValue() == TR::GlRegDeps,
        comp(),
        "Child count %d does not match expected value of %d (%d without "
        "GlRegDeps) and last child is not a GlRegDeps",
        actChildCount,
        expChildCount + 1,
        expChildCount);
    } else {
      ...
      TR::checkILCondition(node,
                           actChildCount == expChildCount,
                           comp(),
                           "Child count %d matches neither expected values of "
                           "%d (without GlRegDeps) nor %d (with GlRegDeps)",
                           actChildCount,
                           expChildCount,
                           expChildCount + 1);
    }
...
void
TR::ValidateChildTypes::validate(TR::Node* node)
{
  auto opcode = node->getOpCode();
  if (opcode.expectedChildCount() != ILChildProp::UnspecifiedChildCount) {
    const auto expChildCount = opcode.expectedChildCount();
    ...
    for (auto i = 0; i < actChildCount; ++i) {
      ...
      if (childOpcode.getOpCodeValue() != TR::GlRegDeps) {
        ...
        TR::checkILCondition(
          node,
          (expChildType >= TR::NumTypes || actChildType == expChildType),
          comp(),
          "Child %d has unexpected type %s (expected %s)",
          i,
          actChildTypeName,
          expChildTypeName);
      } else {
         ...
         */
        TR::checkILCondition(node,
                             opcode.canHaveGlRegDeps() &&
                               (i == actChildCount - 1),
                             comp(),
                             "Unexpected GlRegDeps child %d",
                             i);
      }
    }
  }
 ...
 */
TR::Validate_ireturnReturnType::Validate_ireturnReturnType(
  TR::Compilation* comp)
  : TR::NodeValidationRule(comp, OMR::validate_ireturnReturnType)
{}

void
TR::Validate_ireturnReturnType::validate(TR::Node* node)
{
  auto opcode = node->getOpCode();
  if (opcode.getOpCodeValue() == TR::ireturn) {
    ...
    for (auto i = 0; i < childCount; ++i) {
      ...
      TR::checkILCondition(
        node,
        (actChildType == TR::Int32 || actChildType == TR::Int16 ||
