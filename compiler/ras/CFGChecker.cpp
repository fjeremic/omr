  ...
  */
  ...
  for (auto edge = block->getExceptionSuccessors().begin();
    ...
    for (auto edge2 = block->getExceptionSuccessors().begin();
         edge2 != block->getExceptionSuccessors().end(); ++edge2)
  ...
  if (opCode.isBranch()) {
    ...
    for (auto edge = block->getSuccessors().begin();
      ...
      if (!((next == fallThroughBlock) || (next == branchBlock))) {
        if (_outFile)
          trfprintf(_outFile,
                    "Successor block [%d] of block [%d] containing a branch "
                    "does not match the destination(s) specified in the IL "
                    "branch instruction\n",
                    next->getNumber(), block->getNumber());
        return false;
      }
...
int32_t TR_CFGChecker::getNumUniqueCases(TR::Node *switchNode) {
  ...
  uniqueTreeTops =
      (TR::TreeTop **)_cfg->comp()->trMemory()->allocateStackMemory(
  ...
  for (int i = 2; i <= numCases; i++) {
    ...
    if (unique) uniqueTreeTops[numUniqueCases++] = branchTreeTop;
  }
...
void TR_CFGChecker::performConsistencyCheck() {
  ...
  node = _cfg->getEnd();
  if (!isConsistent(toBlock(node))) _isCFGConsistent = false;

  for (int32_t i = 0; i < _numRealBlocks; i++) {
    if (!isConsistent(_blocksInProgramOrder[i])) _isCFGConsistent = false;
  }

  if (checkForUnreachableCycles()) _isCFGConsistent = false;
...
bool TR_CFGChecker::isConsistent(TR::Block *block) {
  ...
  if (block->getPredecessors().empty() &&
    ...
    if (block == _cfg->getEnd()) return true;
    if (_outFile)
  ...
  for (auto nextEdge = block->getExceptionPredecessors().begin();
    ...
    if (!_blockChecklist.isSet(nextBlock->getNumber())) {
      if (_outFile)
        trfprintf(_outFile,
                  "Exception predecessor block [%d] of block [%d] is not in "
                  "the CFG\n",
                  nextBlock->getNumber(), block->getNumber());
      return false;
    }
...
bool TR_CFGChecker::checkForUnreachableCycles() {
  ...
  reachableBlocks.init(_cfg->getNumberOfNodes(), _cfg->comp()->trMemory(),
                       stackAlloc, growable);
