...
void TR_CFGChecker::performCorrectnessCheck()
{
    ...
    if (_cfg->getStart()->getSuccessors().size() != 1) {
        ...
        if (_cfg->getStart()->getSuccessors().front()->getTo() != _blocksInProgramOrder[0]) {
            if (_outFile)
                trfprintf(_outFile, "The successor block for the (dummy) start block in the CFG is NOT the start block in the actual program\n");
            _successorsCorrect = false;
        }
    }
...
bool TR_CFGChecker::arrangeBlocksInProgramOrder()
{
    ...
    while (cursorTreeTop) {
        ...
        //
        if ((nextNodeNumber < 0 && b->getNumber() != nextNodeNumber) || (nextNodeNumber >= 0 && b->getNumber() >= nextNodeNumber)) {
            if (_outFile)
                trfprintf(_outFile, "Block %d [%p]  at tree node [%p] has a bad node number [%d]\n", b, node, b->getNumber());
            return false;
        }
    ...
    if (i != _numRealBlocks) {
        if (_outFile)
            trfprintf(_outFile, "Number of blocks in trees [%d] does not match number in CFG [%d]\n", i, _numRealBlocks);
        return false;
    }
    return true;
}
   ...
   */
    ...
    if (!(opCode.isBranch() || opCode.isSwitch() || opCode.isReturn() || opCode.isJumpWithMultipleTargets() || opCode.isStoreReg() || opCode.getOpCodeValue() == TR::athrow)) {
        if (!(block->getSuccessors().size() == 1)) {
            ...
            if (_outFile)
                trfprintf(_outFile, "Last non-fence opcode in block [%d] is not a branch, switch, or a return and it does not have exactly one successor\n", block->getNumber());
            return false;
        }

        if (block->getSuccessors().front()->getTo() != _blocksInProgramOrder[i + 1]) {
            if (_outFile)
                trfprintf(_outFile, "Successor block [%d] of block [%d] (with no branch, switch, or return at the end) is not the fall through block\n", block->getSuccessors().front()->getTo()->getNumber(), block->getNumber());
            return false;
        }
    }
    ...
    if (opCode.isBranch()) {
        ...
        if (opCode.getOpCodeValue() == TR::Goto) {
            if (!(block->getSuccessors().size() == 1)) {
                if (_outFile)
                    trfprintf(_outFile, "Number of successors of block [%d] having a goto at the exit is not equal to one\n", block->getNumber());
                return false;
            }
        } else {
            ...
            if (!(numUniqueChildren == block->getSuccessors().size())) {
                if (_outFile)
                    trfprintf(_outFile, "Number of successors of block [%d] having an if at the exit is not equal to the number of unique targets of the if\n", block->getNumber());
                return false;
            }
        }
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (!((next == fallThroughBlock) || (next == branchBlock))) {
                if (_outFile)
                    trfprintf(_outFile, "Successor block [%d] of block [%d] containing a branch does not match the destination(s) specified in the IL branch instruction\n",
                        next->getNumber(), block->getNumber());
                return false;
            }
        }
        ...
        if (!(getNumUniqueCases(lastNode) == block->getSuccessors().size())) {
            if (_outFile)
                trfprintf(_outFile, "Number of successors of block [%d] having a switch at the exit is not equal to the number of destinations in the IL switch instruction\n", block->getNumber());
            return false;
        }
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (!equalsAnyChildOf(next->getEntry(), lastNode)) {
                if (_outFile)
                    trfprintf(_outFile, "Successor block [%d] of block [%d] containing a switch does not match any of the destinations specified in the IL switch instruction\n", next->getNumber(), block->getNumber());
                return false;
            }
        }
        ...
        if (!(block->getSuccessors().size() == 1)) {
            if (_outFile)
                trfprintf(_outFile, "Number of successors of block [%d] having a return at the exit is not equal to one\n", block->getNumber());
            return false;
        }
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (next != cfgEnd) {
                if (_outFile)
                    trfprintf(_outFile, "Successor block [%d] of block [%d] containing a return is NOT the exit block\n", next->getNumber(), block->getNumber());
                return false;
            }
        }
    }
...
bool TR_CFGChecker::isConsistent(TR::Block* block)
{
    ...
    for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
        ...
        if (matchEdge == nextBlock->getSuccessors().end()) {
            if (_outFile)
                trfprintf(_outFile, "Predecessor block [%d] of block [%d] does not contain block [%d] in its successors list\n", nextBlock->getNumber(), block->getNumber(), block->getNumber());
            return false;
        }
    }
    ...
    for (auto nextEdge = block->getExceptionPredecessors().begin(); nextEdge != block->getExceptionPredecessors().end(); ++nextEdge) {
        ...
        if (matchEdge == nextBlock->getExceptionSuccessors().end()) {
            if (_outFile)
                trfprintf(_outFile, "Exception Predecessor block [%d] of block [%d] does not contain block [%d] in its exception successors list\n", nextBlock->getNumber(), block->getNumber(), block->getNumber());
            return false;
        }
    }
...
bool TR_CFGChecker::checkForUnreachableCycles()
{
    ...
    for (TR::CFGNode* node = _cfg->getFirstNode(); node; node = node->getNext())
        if (!reachableBlocks.isSet(node->getNumber()) && node->asBlock() && node != _cfg->getEnd()) {
            unreachable = true;
            if (_outFile)
                trfprintf(_outFile, "Block %d [%p] is unreachable or is in an unreachable cycle\n", node->getNumber(), node);
        }
