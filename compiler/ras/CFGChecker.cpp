...
TR_CFGChecker::TR_CFGChecker(TR::ResolvedMethodSymbol* methodSymbol,
                             TR_Debug* debug)
...
TR_CFGChecker::check()
{
...

  {
    ...
    if (debug("traceCFGCHK")) {
      _cfg->comp()->dumpMethodTrees(
        "Printing out the TreeTops from CFGChecker");
      if (_outFile)
...
#if defined(DEBUG) ||                                                          \
  (defined(PROD_WITH_ASSUMES) && !defined(DISABLE_CFG_CHECK))
...
//
void
TR_CFGChecker::performCorrectnessCheck()
{
  _successorsCorrect = true;
  ...
  if (_cfg->getStart()->getSuccessors().size() != 1) {
    if (_outFile)
      trfprintf(_outFile,
                "There is more than one successor block for the start block\n");
    ...
    //
    if (_cfg->getStart()->getSuccessors().front()->getTo() !=
        _blocksInProgramOrder[0]) {
      if (_outFile)
        trfprintf(_outFile,
                  "The successor block for the (dummy) start block in the CFG "
                  "is NOT the start block in the actual program\n");
      _successorsCorrect = false;
    }
  }
...
//
bool
TR_CFGChecker::arrangeBlocksInProgramOrder()
{
  ...
  _blocksInProgramOrder =
    (TR::Block**)_cfg->comp()->trMemory()->allocateStackMemory(
      (_numRealBlocks + 1) * sizeof(TR::Block*));
  ...
  if (nextNodeNumber < -1 ||
    ...
    if (_outFile)
      trfprintf(
        _outFile, "CFG has a bad nextNodeNumber [%d]\n", nextNodeNumber);
    return false;
  }
  ...
  while (cursorTreeTop) {
    ...
    if (!_blockChecklist.isSet(b->getNumber())) {
      if (_outFile)
        trfprintf(_outFile,
                  "Block %d [%p]  at tree node [%p] is in the trees but not in "
                  "the CFG\n",
                  b->getNumber(),
                  b,
                  node);
    ...
    if ((nextNodeNumber < 0 && b->getNumber() != nextNodeNumber) ||
      ...
      if (_outFile)
        trfprintf(
          _outFile,
          "Block %d [%p]  at tree node [%p] has a bad node number [%d]\n",
          b,
          node,
          b->getNumber());
      return false;
  ...
  if (i != _numRealBlocks) {
    if (_outFile)
      trfprintf(
        _outFile,
        "Number of blocks in trees [%d] does not match number in CFG [%d]\n",
        i,
        _numRealBlocks);
    return false;
  }
...
//
bool
TR_CFGChecker::areSuccessorsCorrect(int32_t i)
{
  ...
  if ((_cfg->getMaxFrequency() >= 0) &&
     ...
     {
     if (_outFile) trfprintf(_outFile, "Block [%d] does not have frequency
  set\n", block->getNumber()); return false;
     }
  */
  ...
  for (auto edge = block->getSuccessors().begin();
    ...
    if (!_blockChecklist.isSet(next->getNumber())) {
      if (_outFile)
        trfprintf(_outFile,
                  "Successor block [%d] of block [%d] is not in the CFG\n",
                  next->getNumber(),
                  block->getNumber());
      return false;
    }
  ...
  for (auto edge = block->getExceptionSuccessors().begin();
    ...
    if (!_blockChecklist.isSet(next->getNumber())) {
      if (_outFile)
        trfprintf(
          _outFile,
          "Exception successor block [%d] of block [%d] is not in the CFG\n",
          next->getNumber(),
          block->getNumber());
      return false;
    }
    for (auto edge2 = block->getExceptionSuccessors().begin();
         edge2 != block->getExceptionSuccessors().end();
         ++edge2)
      if (((*edge) != (*edge2)) && (next == (*edge2)->getTo())) {
        if (_outFile)
          trfprintf(_outFile,
                    "Exception successor block [%d] of block [%d] is listed "
                    "more than once\n",
                    next->getNumber(),
                    block->getNumber());
        return false;
  ...
  if ((lastNode->getOpCodeValue() == TR::NULLCHK) ||
      (lastNode->getOpCodeValue() == TR::treetop))
    lastNode =
      lastNode->getFirstChild(); // athrow might be a child of a null check
  TR::ILOpCode& opCode = lastNode->getOpCode();

  if (!(opCode.isBranch() || opCode.isSwitch() || opCode.isReturn() ||
        opCode.isJumpWithMultipleTargets() || opCode.isStoreReg() ||
        opCode.getOpCodeValue() == TR::athrow)) {
    if (!(block->getSuccessors().size() == 1)) {
      ...
      if (_outFile)
        trfprintf(
          _outFile,
          "Last non-fence opcode in block [%d] is not a branch, switch, or a "
          "return and it does not have exactly one successor\n",
          block->getNumber());
      return false;
    }

    if (block->getSuccessors().front()->getTo() !=
        _blocksInProgramOrder[i + 1]) {
      if (_outFile)
        trfprintf(_outFile,
                  "Successor block [%d] of block [%d] (with no branch, switch, "
                  "or return at the end) is not the fall through block\n",
                  block->getSuccessors().front()->getTo()->getNumber(),
                  block->getNumber());
      return false;
    }
  }
  ...
  if (opCode.isBranch()) {
    ...
    if (opCode.getOpCodeValue() == TR::Goto) {
      if (!(block->getSuccessors().size() == 1)) {
        if (_outFile)
          trfprintf(_outFile,
                    "Number of successors of block [%d] having a goto at the "
                    "exit is not equal to one\n",
                    block->getNumber());
        return false;
      }
    } else {
      ...
      if (!(numUniqueChildren == block->getSuccessors().size())) {
        if (_outFile)
          trfprintf(
            _outFile,
            "Number of successors of block [%d] having an if at the exit is "
            "not equal to the number of unique targets of the if\n",
            block->getNumber());
        return false;
      }
    }
    ...
    for (auto edge = block->getSuccessors().begin();
      ...
      if (!((next == fallThroughBlock) || (next == branchBlock))) {
        if (_outFile)
          trfprintf(
            _outFile,
            "Successor block [%d] of block [%d] containing a branch does not "
            "match the destination(s) specified in the IL branch instruction\n",
            next->getNumber(),
            block->getNumber());
        return false;
      }
    ...
    //
    // if (!((lastNode->getCaseIndexUpperBound()-1) ==
    // block->getSuccessors().size()))

    if (!(getNumUniqueCases(lastNode) == block->getSuccessors().size())) {
      if (_outFile)
        trfprintf(_outFile,
                  "Number of successors of block [%d] having a switch at the "
                  "exit is not equal to the number of destinations in the IL "
                  "switch instruction\n",
                  block->getNumber());
      return false;
    }
    ...
    for (auto edge = block->getSuccessors().begin();
      ...
      if (!equalsAnyChildOf(next->getEntry(), lastNode)) {
        if (_outFile)
          trfprintf(_outFile,
                    "Successor block [%d] of block [%d] containing a switch "
                    "does not match any of the destinations specified in the "
    ...
    if (!(block->getSuccessors().size() == 1)) {
      if (_outFile)
        trfprintf(_outFile,
                  "Number of successors of block [%d] having a return at the "
                  "exit is not equal to one\n",
                  block->getNumber());
      return false;
    }
    ...
    for (auto edge = block->getSuccessors().begin();
      ...
      if (next != cfgEnd) {
        if (_outFile)
          trfprintf(_outFile,
                    "Successor block [%d] of block [%d] containing a return is "
                    "NOT the exit block\n",
                    next->getNumber(),
                    block->getNumber());
        return false;
...
//
bool
TR_CFGChecker::equalsAnyChildOf(TR::TreeTop* treeTop, TR::Node* switchNode)
{
  for (int i = switchNode->getCaseIndexUpperBound() - 1; i > 0; i--) {
    TR::TreeTop* branchTreeTop =
      switchNode->getChild(i)->getBranchDestination();
...
int32_t
TR_CFGChecker::getNumUniqueCases(TR::Node* switchNode)
{
  int32_t numCases = switchNode->getCaseIndexUpperBound() - 1;
  int32_t numUniqueCases = 0;

  TR::TreeTop** uniqueTreeTops;
  uniqueTreeTops = (TR::TreeTop**)_cfg->comp()->trMemory()->allocateStackMemory(
    numCases * sizeof(TR::TreeTop*));

  memset(uniqueTreeTops, 0, numCases * sizeof(TR::TreeTop*));

  TR::TreeTop* defaultBranchTreeTop =
    switchNode->getSecondChild()->getBranchDestination();
  ...
  // if (_outFile) trfprintf(_outFile, "numUniqueChildren =
  // %d\n",numUniqueChildren);
...
//
void
TR_CFGChecker::performConsistencyCheck()
{
  _isCFGConsistent = true;
  /*
    TR::CFGEdge *edge;
    for (edge = _cfg->getFirstEdge(); edge; edge = edge->getNext())
       ...
       if (!_cfg->getNodes().find(from))
          {
          if (_outFile) trfprintf(_outFile, "Block [%p] numbered %d is not in
    the CFG nodes list\n", from, from->getNumber()); _isCFGConsistent = false;
          break;
          }
       if (!from->getSuccessors().find(edge) &&
    !from->getExceptionSuccessors().find(edge))
          {
          if (_outFile) trfprintf(_outFile, "Edge between block [%p] numbered %d
    and block [%p] numbered %d is in the CFG edge list but not in successors
    list\n", from, from->getNumber(), edge->getTo(),
    edge->getTo()->getNumber()); _isCFGConsistent = false; break;
          }
       }
    */
...
//
bool
TR_CFGChecker::isConsistent(TR::Block* block)
{
  // If the block is null, there was something wrong with the block counts
  // earlier.
  //
  ...
  if (block->getPredecessors().empty() &&
    ...
    if (_outFile)
      trfprintf(
        _outFile, "Block %d [%p] is an orphan\n", block->getNumber(), block);
    return false;
  }
  ...
  for (auto nextEdge = block->getPredecessors().begin();
    ...
    if (!_blockChecklist.isSet(nextBlock->getNumber())) {
      if (_outFile)
        trfprintf(_outFile,
                  "Predecessor block [%d] of block [%d] is not in the CFG\n",
                  nextBlock->getNumber(),
                  block->getNumber());
      return false;
    }
    ...
    if (matchEdge == nextBlock->getSuccessors().end()) {
      if (_outFile)
        trfprintf(_outFile,
                  "Predecessor block [%d] of block [%d] does not contain block "
                  "[%d] in its successors list\n",
                  nextBlock->getNumber(),
                  block->getNumber(),
                  block->getNumber());
      return false;
  ...
  while (basicP != block->getPredecessors().end()) {
    ...
    if (e->getTo() != block) {
      if (_outFile) {
        trfprintf(_outFile,
                  "ERROR: edge from %d to %d does not point to block_%d\n",
                  e->getFrom()->getNumber(),
                  e->getTo()->getNumber(),
                  block->getNumber());
      }
    ...
    if (!foundMe) {
      if (_outFile) {
        trfprintf(_outFile,
                  "ERROR: block_%d is a predecessor of block_%d but the "
                  "reverse is not true\n",
                  e->getFrom()->getNumber(),
                  block->getNumber());
      }
  ...
  while (basicS != block->getSuccessors().end()) {
    ...
    if (e->getFrom() != block) {
      if (_outFile) {
        trfprintf(_outFile,
                  "ERROR: edge from %d to %d does not come from block_%d\n",
                  e->getFrom()->getNumber(),
                  e->getTo()->getNumber(),
                  block->getNumber());
      }
    ...
    if (!foundMe) {
      if (_outFile) {
        trfprintf(_outFile,
                  "ERROR: block_%d is a successor of block_%d but the reverse "
                  "is not true\n",
                  e->getTo()->getNumber(),
                  block->getNumber());
      }
  ...
  for (auto nextEdge = block->getExceptionPredecessors().begin();
    ...
    if (!_blockChecklist.isSet(nextBlock->getNumber())) {
      if (_outFile)
        trfprintf(
          _outFile,
          "Exception predecessor block [%d] of block [%d] is not in the CFG\n",
          nextBlock->getNumber(),
          block->getNumber());
      return false;
    }
    auto matchEdge = nextBlock->getExceptionSuccessors().begin();
    for (; matchEdge != nextBlock->getExceptionSuccessors().end();
         ++matchEdge) {
      if ((*matchEdge) == (*nextEdge))
        break;
    }
    if (matchEdge == nextBlock->getExceptionSuccessors().end()) {
      if (_outFile)
        trfprintf(_outFile,
                  "Exception Predecessor block [%d] of block [%d] does not "
                  "contain block [%d] in its exception successors list\n",
                  nextBlock->getNumber(),
                  block->getNumber(),
                  block->getNumber());
      return false;
  ...
  // One would expect that we could check here for a single block exception
  // loop, however, because of the strange nature of monexit, we need to
  // accomodate the possibility of a single block exception loop.
...
TR_CFGChecker::checkForUnreachableCycles()
{
  ...
  reachableBlocks.init(
    _cfg->getNumberOfNodes(), _cfg->comp()->trMemory(), stackAlloc, growable);
  _cfg->comp()->getFlowGraph()->findReachableBlocks(&reachableBlocks);

  bool unreachable = false;
  for (TR::CFGNode* node = _cfg->getFirstNode(); node; node = node->getNext())
    if (!reachableBlocks.isSet(node->getNumber()) && node->asBlock() &&
        node != _cfg->getEnd()) {
      unreachable = true;
      if (_outFile)
        trfprintf(
          _outFile,
          "Block %d [%p] is unreachable or is in an unreachable cycle\n",
          node->getNumber(),
          node);
    }
