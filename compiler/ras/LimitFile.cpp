...
TR_FilterBST* TR_Debug::addFilter(char*& filterString, int32_t scanningExclude, int32_t optionSetIndex, int32_t lineNum,
...
TR_FilterBST* TR_Debug::addFilter(
  char*& filterString, int32_t scanningExclude, int32_t optionSetIndex, int32_t lineNum, bool loadLimit)
{
  if (loadLimit) {
    // initializing filters using the new interface
...
TR_FilterBST* TR_Debug::addExcludedMethodFilter(bool loadLimit)
{
  TR_FilterBST* filterBST
    = new (TR::Compiler->regionAllocator) TR_FilterBST(TR_FILTER_EXCLUDE_SPECIFIC_METHOD, TR_EXCLUDED_OPTIONSET_INDEX);
  if (loadLimit) {
    _relocationFilters = findOrCreateFilters(_relocationFilters);
...
bool TR_Debug::addSamplingPoint(char* filterString, TR_FilterBST*& lastSamplingPoint, bool loadLimit)
{
  char* p;
  int32_t tickCount;
 ...
 */
char* TR_Debug::inlinefileOption(char* option, void* base, TR::OptionTable* entry, TR::Options* cmdLineOptions)
{
  char* endOpt = option;
  char* name = option;
  char* fail = option;
  ...
  if (!success) {
    ...
    return fail; // We want to fail if we can't read the file because it is too easy to miss that the file wasn't picked
                 // up
  }
  return endOpt;
}
 ...
 *     Otherwise a pointer to the next comma or NULL.
 */
char* TR_Debug::limitfileOption(char* option, void* base, TR::OptionTable* entry, TR::Options* cmdLineOptions,
  bool loadLimit, TR_PseudoRandomNumbersListElement** pseudoRandomListHeadPtr)
{
  char* endOpt = option;
  char* name = option;
  char* fail = option;
...
char* TR_Debug::limitOption(
  char* option, void* base, TR::OptionTable* entry, TR::Options* cmdLineOptions, bool loadLimit)
{
  char* p = option;
  ...
  for (TR_FilterBST* filter = _compilationFilters->samplingPoints; filter; filter = filter->getNext()) {
    if (filter->getFilterType() == TR_FILTER_SAMPLE_INTERPRETED) {
      TR_VerboseLog::write("(%d)\tInterpreted %s.%s%s\tcount=%d\n", filter->getTickCount(), filter->getClass(),
        filter->getName(), filter->getSignature(), filter->getSampleCount());
    } else {
  ...
  const char* methodSig, TR::CompilationFilters* filters, TR_FilterBST*& filter, TR::Method::Type methodType)
{
  ...
  if (filters->hasClassNameSigFilter() || filters->hasNameSigFilter()) {
    if (filters->hasClassNameSigFilter()) {
      ...
      if (filter)
        filter
          = filter->find(methodName, methodNameLen, methodClass, methodClassLen, methodSignature, methodSignatureLen);
    }
