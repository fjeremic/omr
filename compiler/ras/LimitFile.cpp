...
#define FILTER_SIZE \
  (sizeof(TR::CompilationFilters) + sizeof(TR_FilterBST *) * FILTER_HASH_SIZE)
...
TR::CompilationFilters *TR_Debug::findOrCreateFilters(
    TR::CompilationFilters *filters) {
  if (filters)
    return filters;
  else {
    int32_t size = sizeof(TR::CompilationFilters) +
                   sizeof(TR_FilterBST *) * FILTER_HASH_SIZE;
...
TR_FilterBST *TR_Debug::addFilter(char *&filterString, int32_t scanningExclude,
                                  int32_t optionSetIndex, int32_t lineNum,
                                  TR::CompilationFilters *anyFilters) {
  uint32_t filterType =
      scanningExclude ? TR_FILTER_EXCLUDE_NAME_ONLY : TR_FILTER_NAME_ONLY;
  ...
  TR_FilterBST *filterBST = new (TR::Compiler->regionAllocator)
      TR_FilterBST(filterType, optionSetIndex, lineNum);
  ...
  if (*filterString == '{') {
    ...
    filterBST->setNext(filters->hasRegexFilter() ? filters->filterRegexList
                                                 : NULL);
    ...
    if (filterType == TR_FILTER_EXCLUDE_NAME_ONLY ||
      ...
      TR_FilterBST **bucket =
          &(filters->filterHash[nameLength % FILTER_HASH_SIZE]);
...
TR_FilterBST *TR_Debug::addFilter(char *&filterString, int32_t scanningExclude,
  ...
  if (loadLimit) {
    ...
    return addFilter(filterString, scanningExclude, optionSetIndex, lineNum,
                     _relocationFilters);
  } else {
    // initializing filters using the new interface
...
TR_FilterBST *TR_Debug::addExcludedMethodFilter(bool loadLimit) {
  TR_FilterBST *filterBST = new (TR::Compiler->regionAllocator) TR_FilterBST(
      TR_FILTER_EXCLUDE_SPECIFIC_METHOD, TR_EXCLUDED_OPTIONSET_INDEX);
  if (loadLimit) {
    _relocationFilters = findOrCreateFilters(_relocationFilters);
...
bool TR_Debug::addSamplingPoint(char *filterString,
                                TR_FilterBST *&lastSamplingPoint,
                                bool loadLimit) {
  char *p;
  int32_t tickCount;
  if (1 != sscanf(filterString, "(%d) ", &tickCount)) return false;
  uint32_t type;
  ...
  TR_FilterBST *filterBST =
      new (TR::Compiler->regionAllocator) TR_FilterBST(type, tickCount);
  if (!scanFilterName(methodName, filterBST)) return false;
  ...
  //
  if (filterBST->getFilterType() != TR_FILTER_SPECIFIC_METHOD) return false;
...
bool TR_Debug::scanInlineFilters(FILE *inlineFile, int32_t &lineNumber,
                                 TR::CompilationFilters *filters) {
  ...
  while (fgets(limitReadBuffer, sizeof(limitReadBuffer), inlineFile)) {
    ...
    if (includeFlag == '[') {
      ...
      if (filter) {
        ...
        inlineFileError =
            !scanInlineFilters(inlineFile, lineNumber, filter->subGroup);
      }
      ...
      filter = addFilter(p, (('+' == includeFlag) ? 0 : 1), optionSet,
                         lineNumber, filters);
...
/** \brief
 *     This function opens the inlinefile and adds its entries to a
 * TR::CompilationFilters.
 *
 *     An inlinefile is a file containing a list of methods, one per line, which
 * the inliner will limit itself to when trying to perform inlining. In other
 * words, only methods from the file can be inlined, but there is no guarantee
 * that any of them will be inlined. The format for entry is:
 *
 *     + signature
 *
 *  \param option
 ...
 *
 *  \return
 *     The unmodified parameter option if there is a problem with the file,
 * aborting JIT initialization. Otherwise a pointer to the next comma or NULL.
 */
char *TR_Debug::inlinefileOption(char *option, void *base,
  ...
  if (!len) return option;

  char *inlineFileName =
      (char *)(TR::Compiler->regionAllocator.allocate(len + 1));
  ...
  if (!success) {
    TR_VerboseLog::write("<JIT: fatal: unable to read inline file --> '%s'>\n",
                         inlineFileName);
    return fail;  // We want to fail if we can't read the file because it is too
                  // easy to miss that the file wasn't picked up
  }
  return endOpt;
}

/** \brief
 *     Processes a limitfile= option, parses and applies the limitfile to
 * compilation control.
 *
 *     A limitfile is a compiler verbose log, produced by the option
 * -Xjit:verbose,vlog=filename When a verbose log is used as a limitfile, only
 * the methods contained within the file will be compiled if they are queued for
 * compilation. The format of the method entries in the file must match that of
 * a verbose log.
 *
 ...
 *
 *     The when the latter is used, xx-yy denotes a line number range (starting
 * at zero and ignoring # comments) to restrict the limiting to. This is
 * commonly used in debugging to narrow down a problem to a specific method by
 * doing a binary search on the limitfile.
 *
 *  \param option
 *     Points to the first char after inlinefile=
 *
 ...
 * aborting JIT initialization. Otherwise a pointer to the next comma or NULL.
 */
char *TR_Debug::limitfileOption(
    char *option, void *base, TR::OptionTable *entry,
    TR::Options *cmdLineOptions, bool loadLimit,
    TR_PseudoRandomNumbersListElement **pseudoRandomListHeadPtr) {
  char *endOpt = option;
  char *name = option;
  char *fail = option;
  ...
  if (range) {
    if (!*endOpt) return option;
    firstLine = TR::Options::getNumericValue(++endOpt);
    if (*endOpt == ',') lastLine = TR::Options::getNumericValue(++endOpt);
    if (*endOpt != ')') return option;
    ++endOpt;
  }
  ...
  if (limitFile) {
    ...
    TR_PseudoRandomNumbersListElement *curPseudoRandomListElem =
        pseudoRandomListHead;
    int32_t curIndex = 0;

    while (fgets(limitReadBuffer, sizeof(limitReadBuffer), limitFile)) {
      ++lineNumber;
      if (lineNumber < firstLine || lineNumber > lastLine) continue;
      ...
      if (strncmp(limitReadBuffer, "-precompileMethod", 17) == 0) {
        ...
        if (!addFilter(p, (('+' == includeFlag) ? 0 : 1), optionSet, lineNumber,
                       loadLimit)) {
          limitFileError = true;
          break;
        }
      } else if (strncmp(limitReadBuffer, PSEUDO_RANDOM_NUMBER_PREFIX,
                         PSEUDO_RANDOM_NUMBER_PREFIX_LEN) == 0) {
        ...
        while (isdigit(p[0])) {
          ...
          if (isNegative) randNum = -1 * randNum;
          ...
          if ((curPseudoRandomListElem == NULL) ||
            ...
            TR_PseudoRandomNumbersListElement *newPseudoRandomListElem =
                (TR_PseudoRandomNumbersListElement
                     *)(TR::Compiler->regionAllocator.allocate(sz));
          ...
          if (*p == PSEUDO_RANDOM_SUFFIX) break;
      ...
      } else if (limitReadBuffer[0] == '(' &&
                 cmdLineOptions->getOption(TR_OrderCompiles)) {
...
char *TR_Debug::limitOption(char *option, void *base, TR::OptionTable *entry,
                            TR::Options *cmdLineOptions, bool loadLimit) {
  char *p = option;
...
void *TR_FilterBST::operator new(size_t size,
                                 TR::PersistentAllocator &allocator) {
  return allocator.allocate(size);
}

TR_FilterBST *TR_FilterBST::find(const char *methodName,
...
TR_FilterBST *TR_FilterBST::find(const char *methodName, int32_t methodNameLen,
                                 ...
                                 int32_t methodClassLen,
                                 const char *methodSignature,
                                 int32_t methodSignatureLen) {
  ...
  for (node = this; node; node = node->getChild(rc)) {
    rc = strncmp(methodName, node->getName(), methodNameLen);
    if (rc == 0) rc = methodNameLen - node->getNameLen();
    if (rc == 0) {
      rc = strncmp(methodClass, node->getClass(), methodClassLen);
      if (rc == 0) rc = methodClassLen - strlen(node->getClass());
      if (rc == 0) {
        rc = strncmp(methodSignature, node->getSignature(), methodSignatureLen);
        if (rc == 0) rc = methodSignatureLen - strlen(node->getSignature());
        if (rc == 0) break;
      }
    }
    rc = (rc < 0) ? 0 : 1;
  ...
  if (filters) {
    if (filters->filterHash) {
      for (i = 0; i < FILTER_HASH_SIZE; i++)
        if (filters->filterHash[i]) printFilterTree(filters->filterHash[i]);
    }
    ...
    if (filters->filterRegexList) {
      for (TR_FilterBST *filter = filters->filterRegexList; filter;
...
void TR_Debug::printFilterTree(TR_FilterBST *root) {
  if (root->getChild(0)) printFilterTree(root->getChild(0));
  print(root);
  if (root->getChild(1)) printFilterTree(root->getChild(1));
}

void TR_Debug::printSamplingPoints() {
  for (TR_FilterBST *filter = _compilationFilters->samplingPoints; filter;
       filter = filter->getNext()) {
    if (filter->getFilterType() == TR_FILTER_SAMPLE_INTERPRETED) {
      TR_VerboseLog::write("(%d)\tInterpreted %s.%s%s\tcount=%d\n",
                           filter->getTickCount(), filter->getClass(),
                           filter->getName(), filter->getSignature(),
...
int32_t TR_Debug::scanFilterName(char *string, TR_FilterBST *filter) {
  ...
  while (*string && *string != '\t' && *string != ',' && *string != '\n') {
    if (omrPattern) {
      ...
      if (*string == '.') {
        ...
        signatureChars = string + 1;
        signatureLen = 0;
        filterType = filter->isExclude() ? TR_FILTER_EXCLUDE_SPECIFIC_METHOD
                                         : TR_FILTER_SPECIFIC_METHOD;
      }
        ...
        if (filterType == TR_FILTER_EXCLUDE_NAME_ONLY ||
            filterType == TR_FILTER_NAME_ONLY) {
          filterType = filter->isExclude() ? TR_FILTER_EXCLUDE_NAME_AND_SIG
                                           : TR_FILTER_NAME_AND_SIG;
        }
      } else
        signatureLen++;
    }
  ...
  char *buf = (char *)(TR::Compiler->regionAllocator.allocate(
      length + 3));  // NULL terminated strings
...
bool TR_Debug::methodSigCanBeCompiled(const char *methodSig,
                                      TR_FilterBST *&filter,
                                      TR::Method::Type methodType) {
  return methodSigCanBeCompiledOrRelocated(methodSig, filter, false,
                                           methodType);
}

bool TR_Debug::methodSigCanBeRelocated(const char *methodSig,
                                       TR_FilterBST *&filter) {
  return methodSigCanBeCompiledOrRelocated(methodSig, filter, true,
                                           TR::Method::J9);
}

bool TR_Debug::methodSigCanBeFound(const char *methodSig,
                                   TR::CompilationFilters *filters,
                                   TR_FilterBST *&filter,
                                   TR::Method::Type methodType) {
  ...
  if (methodType != TR::Method::J9) {
    ...
    if (methodSig[0] == '/')  // omr method pattern
    {
      ...
      TR_ASSERT(methodSignature,
                "unable to pattern match java method signature");
  ...
  if (filters->hasClassNameSigFilter() || filters->hasNameSigFilter()) {
    if (filters->hasClassNameSigFilter()) {
      ...
      //
      filter =
          filters->filterHash[(length + methodClassLen) % FILTER_HASH_SIZE];
      if (filter)
        filter =
            filter->find(methodName, methodNameLen, methodClass, methodClassLen,
                         methodSignature, methodSignatureLen);
    }
    ...
    if (!filter && filters->hasNameSigFilter()) {
      ...
      if (filter)
        filter = filter->find(methodName, methodNameLen, "", 0, methodSignature,
                              methodSignatureLen);
    }
  }
  ...
  if (!filter && filters->hasNameFilter()) {
    ...
    if (filter) filter = filter->find(methodName, methodNameLen);
  }
  ...
  if (!filter && filters->hasRegexFilter()) {
    ...
    if (filter) filter = filter->findRegex(methodSig);
  }
...
bool TR_Debug::methodCanBeFound(TR_Memory *trMemory, TR_ResolvedMethod *method,
                                TR::CompilationFilters *filters,
                                TR_FilterBST *&filter) {
  const char *methodSig = method->signature(trMemory);
  return methodSigCanBeFound(methodSig, filters, filter,
                             method->convertToMethod()->methodType());
}

bool TR_Debug::methodSigCanBeCompiledOrRelocated(const char *methodSig,
                                                 TR_FilterBST *&filter,
                                                 bool loadLimit,
                                                 TR::Method::Type methodType) {
  ...
  bool found =
      methodSigCanBeFound(methodSig, compOrReloFilter, filter, methodType);
  if (!found) {
...
bool TR_Debug::methodCanBeCompiled(TR_Memory *trMemory,
                                   TR_ResolvedMethod *method,
                                   TR_FilterBST *&filter) {
  const char *methodSig = method->signature(trMemory);
  return methodSigCanBeCompiled(methodSig, filter,
                                method->convertToMethod()->methodType());
}

bool TR_Debug::methodCanBeRelocated(TR_Memory *trMemory,
                                    TR_ResolvedMethod *method,
                                    TR_FilterBST *&filter) {
  const char *methodSig = method->signature(trMemory);
  ...
  if (optFile) {
    ...
    while (fgets(lineBuffer, sizeof(lineBuffer), optFile)) {
      if (optCount >= (sizeof(optNumBuffer) / sizeof(optNumBuffer[0]))) {
        TR_VerboseLog::write(
            "<JIT: WARNING: reached limit of %d optFile lines; ignoring "
            "subsequent lines\n",
            optCount);
        break;
      }
      ...
      int optIndex;
      if (!sscanf(lineBuffer, "Performing %d: ", &optIndex)) continue;

      char *name =
          strchr(lineBuffer, ':') + 2;  // 2 moves past the colon and the space
      int32_t nameLen = strcspn(name, " \n");

      int32_t optNum;
      for (optNum = 0; optNum < OMR::numOpts; optNum++) {
        const char *actualName =
            OMR::Optimizer::getOptimizationName((OMR::Optimizations)optNum);
      ...
      if (optNum == OMR::numOpts)
        TR_VerboseLog::write(
            "<JIT: WARNING: ignoring optFile line; no matching opt name for "
            "'%s'\n",
            name);
    }
    ...
    if (optCount > 0) {
      ...
      customStrategy = (int32_t *)(TR::Compiler->regionAllocator.allocate(
          srcSize +
          sizeof(optNumBuffer[0])));  // One extra for the endOpts entry
      memcpy(customStrategy, optNumBuffer, srcSize);
      customStrategy[optCount] = OMR::endOpts;
    } else {
      TR_VerboseLog::write(
          "<JIT: WARNING: ignoring optFile; contains no suitable opt names\n");
    }
  } else {
