...
TR_FilterBST*
TR_Debug::addFilter(char*& filterString,
...
bool
TR_Debug::addSamplingPoint(char* filterString,
...
bool
TR_Debug::scanInlineFilters(FILE* inlineFile,
                            int32_t& lineNumber,
                            TR::CompilationFilters* filters)
{
  ...
  while (fgets(limitReadBuffer, sizeof(limitReadBuffer), inlineFile)) {
    ...
    if (includeFlag == '[') {
      ...
      filter = addFilter(
        p, (('+' == includeFlag) ? 0 : 1), optionSet, lineNumber, filters);
 ...
 */
char*
TR_Debug::inlinefileOption(char* option,
                           void* base,
  ...
  TR_PseudoRandomNumbersListElement** pseudoRandomListHeadPtr)
{
  ...
  if (limitFile) {
    ...
    while (fgets(limitReadBuffer, sizeof(limitReadBuffer), limitFile)) {
      ...
      } else if (strncmp(limitReadBuffer,
                         PSEUDO_RANDOM_NUMBER_PREFIX,
        ...
        while (isdigit(p[0])) {
          ...
          if ((curPseudoRandomListElem == NULL) ||
            ...
            TR_PseudoRandomNumbersListElement* newPseudoRandomListElem =
              (TR_PseudoRandomNumbersListElement*)(TR::Compiler->regionAllocator
                                                     .allocate(sz));
...
char*
TR_Debug::limitOption(char* option,
                      void* base,
                      TR::OptionTable* entry,
                      TR::Options* cmdLineOptions,
                      bool loadLimit)
...
void*
TR_FilterBST::operator new(size_t size, TR::PersistentAllocator& allocator)
{
...
TR_FilterBST*
TR_FilterBST::find(const char* methodName,
...
void
TR_FilterBST::insert(TR_FilterBST* node)
{
...
bool
TR_Debug::methodSigCanBeCompiled(const char* methodSig,
                                 TR_FilterBST*& filter,
                                 TR::Method::Type methodType)
{
  return methodSigCanBeCompiledOrRelocated(
    methodSig, filter, false, methodType);
}
...
{
  return methodSigCanBeCompiledOrRelocated(
    methodSig, filter, true, TR::Method::J9);
}

bool
TR_Debug::methodSigCanBeFound(const char* methodSig,
                              TR::CompilationFilters* filters,
                              TR_FilterBST*& filter,
                              TR::Method::Type methodType)
{
...
bool
TR_Debug::methodCanBeFound(TR_Memory* trMemory,
                           TR_ResolvedMethod* method,
                           TR::CompilationFilters* filters,
                           TR_FilterBST*& filter)
{
  const char* methodSig = method->signature(trMemory);
  return methodSigCanBeFound(
    methodSig, filters, filter, method->convertToMethod()->methodType());
}

bool
TR_Debug::methodSigCanBeCompiledOrRelocated(const char* methodSig,
                                            TR_FilterBST*& filter,
                                            bool loadLimit,
                                            TR::Method::Type methodType)
{
...
bool
TR_Debug::methodCanBeCompiled(TR_Memory* trMemory,
                              TR_ResolvedMethod* method,
                              TR_FilterBST*& filter)
{
  const char* methodSig = method->signature(trMemory);
  return methodSigCanBeCompiled(
    methodSig, filter, method->convertToMethod()->methodType());
}

bool
TR_Debug::methodCanBeRelocated(TR_Memory* trMemory,
                               TR_ResolvedMethod* method,
                               TR_FilterBST*& filter)
{
  const char* methodSig = method->signature(trMemory);
