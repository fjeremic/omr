  ...
  if (optFile) {
    ...
    while (fgets(lineBuffer, sizeof(lineBuffer), optFile)) {
      if (optCount >= (sizeof(optNumBuffer) / sizeof(optNumBuffer[0]))) {
        TR_VerboseLog::write("<JIT: WARNING: reached limit of %d optFile "
                             "lines; ignoring subsequent lines\n",
                             optCount);
        break;
      }
      ...
      if (optNum == OMR::numOpts)
        TR_VerboseLog::write("<JIT: WARNING: ignoring optFile line; no "
                             "matching opt name for '%s'\n",
                             name);
    }
