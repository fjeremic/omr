...
#define LOCALE_DEF \
  static const char *ignoreLocaleOption = feGetEnv(LOCALE_ENV_VAR)
...
int tolower_ignore_locale(int c) {
  LOCALE_DEF;
  if (USE_LOCALE) return tolower(c);
...
int toupper_ignore_locale(int c) {
  LOCALE_DEF;
  if (USE_LOCALE) return toupper(c);
...
int stricmp_ignore_locale(const char *s1, const char *s2) {
  LOCALE_DEF;
  if (USE_LOCALE) return STRICMP(s1, s2);
...
int strnicmp_ignore_locale(const char *s1, const char *s2, size_t length) {
  LOCALE_DEF;
  if (USE_LOCALE) return STRNICMP(s1, s2, length);
