...
void TR::S390BranchInstruction::assignRegistersAndDependencies(
  ...
  if (getOpCode().isBranchOp() &&
    ...
    // Reusing the OOL Section merge label for other branches might be unsafe.
                 ...
                 TR_TraceRARegisterStates))
      traceMsg(comp, "\nOOL: Reusing the OOL Section merge label for other "
                     "branches might be unsafe.\n");
  }
}
...
uint8_t *TR::S390PseudoInstruction::generateBinaryEncoding() {
  ...
  if (_callDescLabel !=
      NULL) // We have to emit a branch around a 8-byte aligned call descriptor.
  {
...
uint8_t *TR::S390RRFInstruction::generateBinaryEncoding() {
  ...
  switch (getKind()) {
  // note that RRD and RRF have swapped R1 and R3 positions but in both cases R1
  // is the target and R3 is a source (src2 in particular)
  case TR::Instruction::IsRRD: // nnnn 1x32 e.g. MSDBR/MSEBR/MSER/MSDR
    ...
    if (!isSrcPair)
      toRealRegister(srcReg)->setRegisterField((uint32_t *)cursor,
                                               0); // encode src in R2 position
    else
 ...
 */
int32_t
TR::S390RILInstruction::adjustCallOffsetWithTrampoline(int32_t offset,
                                                       uint8_t *currentInst) {
