...
TR::S390LabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
    ...
    if (comp->getOption(TR_EnableLabelTargetNOPs)) {
        ...
        TR::Node* node = getNode();
        if (node != NULL && node->getOpCodeValue() == TR::BBStart && node->getLabel() == getLabelSymbol() && node->getBlock()->firstBlockInLoop() && !node->getBlock()->isCold() && node->getBlock()->getFrequency() > 1000) {
...
TR::S390BranchInstruction::generateBinaryEncoding()
{
    ...
    if (label->getCodeLocation() != NULL) {
        ...
        distance = (cg()->getBinaryBufferStart() + label->getEstimatedCodeLocation()) - (cursor + cg()->getAccumulatedInstructionLengthError());
        distance /= 2;
        doRelocation = true;
    }
...
{
    if (matchesAnyRegister(reg, getRegisterOperand(1)) || matchesAnyRegister(reg, getTableRegister()) || matchesAnyRegister(reg, getTermCharRegister()) || matchesAnyRegister(reg, getRegisterOperand(2))) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->refsRegister(reg);
    }
...
{
    if (matchesTargetRegister(reg) || matchesAnyRegister(reg, getRegisterOperand(2)) || (_isSourceReg2Present && matchesAnyRegister(reg, getRegisterOperand(3)))) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->refsRegister(reg);
    }
...
{
    if (matchesTargetRegister(reg) || matchesAnyRegister(reg, getRegisterOperand(2)) || (matchesAnyRegister(reg, getRegisterOperand(3)))) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->refsRegister(reg);
    }
...
TR::S390RILInstruction::estimateBinaryLength(int32_t currentEstimate)
{
    ...
    if (TR::Compiler->target.is64Bit() && (getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LGFRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LLGFRL)) {
        ...
        //
        delta = 24;
    } else if (TR::Compiler->target.is64Bit() && (getOpCode().getOpCodeValue() == TR::InstOpCode::STRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LRL)) {
        //  We could end up generating
        //  STG   Rscrtch, SPILLSLOT(,GPR5)
        //  LLIHF Rscrtch, (addr&0xFFFFFFFF00000000)>>32
        //  IILF  Rscrtch, (addr&0xFFFFFFFF)
...
TR::S390RILInstruction::generateBinaryEncoding()
{
    ...
    if (getTargetSnippet() && getTargetSnippet()->getKind() == TR::Snippet::IsConstantData) {
        ...
        cg()->addRelocation(new (cg()->trHeapMemory()) TR::LabelRelative32BitRelocation(cursor, getTargetSnippet()->getSnippetLabel()));
        cursor += getOpCode().getInstructionLength();
    } else if (getOpCode().getOpCodeValue() == TR::InstOpCode::LRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LGFRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LLGFRL || getOpCode().getOpCodeValue() == TR::InstOpCode::STRL || getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL) {
        {
            ...
            } else // We need to do things the old fashioned way
            {
                ...
                if (getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LGFRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LLGFRL) {
                    sourceReg = (TR::RealRegister*)((TR::S390RegInstruction*)this)->getRegisterOperand(1);
                    if (!scratchReg)
                        scratchReg = sourceReg;
                } else if (getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL || getOpCode().getOpCodeValue() == TR::InstOpCode::STRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LRL) {
                    sourceReg = (TR::RealRegister*)((TR::S390RegInstruction*)this)->getRegisterOperand(1);
                    if (!scratchReg)
                        scratchReg = assignBestSpillRegister();
        ...
        if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this) != comp->getStaticPICSites()->end()) {
            ...
            unloadableClass = (TR_OpaqueClassBlock*)cg()->fe()->createResolvedMethod(cg()->trMemory(),
                                                                  (TR_OpaqueMethodBlock*)getTargetPtr(), comp->getCurrentMethod())
                                  ->classOfMethod();
            doRegisterPIC = true;
        }

        if (doRegisterPIC && !TR::Compiler->cls.sameClassLoaders(comp, unloadableClass, comp->getCurrentMethod()->classOfMethod())) {
        ...
        if (getTargetLabel() != NULL) {
            ...
            if (getTargetSnippet() != NULL) {
                ...
                bool isHelper = getSymbolReference() && getSymbolReference()->getSymbol()->isMethod() && getSymbolReference()->getSymbol()->castToMethodSymbol()->isHelper();
...
TR::S390RIEInstruction::generateBinaryEncoding()
{
    ...
    if (getBranchDestinationLabel()) {
        ...
        if (label->getCodeLocation() != NULL) {
            ...
            // Label location is unknown
            // estimate the relative branch distance
            distance = (cg()->getBinaryBufferStart() + label->getEstimatedCodeLocation()) - (instructionStart + cg()->getAccumulatedInstructionLengthError());
            doRelocation = true;
        }
    ...
    // if we don't have a branch destination (and not type RIE5), we have a 16-bit immediate value to compare
        ...
        *(int16_t*)(cursor) |= bos((int16_t)getSourceImmediate16());
        TR_ASSERT(((getSourceImmediate16() & 0xB900) != 0xB900) || ((getOpCodeValue() != 0xEC71) && (getOpCodeValue() != 0xEC73)),
...
TR::S390MIIInstruction::generateBinaryEncoding()
{
    ...
    int distance = (cg()->getBinaryBufferStart() + getLabelSymbol()->getEstimatedCodeLocation()) - (cursor + cg()->getAccumulatedInstructionLengthError());
    ...
    if (distance >= MIN_12_RELOCATION_VAL && distance <= MAX_12_RELOCATION_VAL && !bpp && offset >= MIN_24_RELOCATION_VAL && offset <= MAX_24_RELOCATION_VAL) {
        // clear the number of bytes we intend to use in the buffer.
        memset((void*)cursor, 0, getEstimatedBinaryLength());
    ...
    */
    ...
    if (performEmptyPatch) {
        ...
        for (TR::Instruction* nextI = getNext(); nextI != NULL; nextI = nextI->getNext()) {
            ...
            if (nextI->getOpCodeValue() == TR::InstOpCode::LABEL || nextI->getOpCodeValue() == TR::InstOpCode::BRCL || nextI->getOpCodeValue() == TR::InstOpCode::BRASL || nextI->getOpCodeValue() == TR::InstOpCode::DC || nextI->getOpCodeValue() == TR::InstOpCode::DC2 || nextI->getOpCodeValue() == TR::InstOpCode::LARL)
                break;
    ...
    if (performEmptyPatch && (sumEstimatedBinaryLength > 0) && performTransformation(comp, "O^O CODE GENERATION: Generating empty patch for guard node %p\n", getNode())) {
        doRelocation = false;
