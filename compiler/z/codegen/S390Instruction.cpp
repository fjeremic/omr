...
void
TR::S390RSInstruction::generateAdditionalSourceRegisters(TR::Register* fReg,
                                                         TR::Register* lReg)
{
...
bool
isLoopEntryAlignmentEnabled(TR::Compilation* comp)
{
 ...
 */
bool
TR::S390LabeledInstruction::isNopCandidate()
{
...
TR::S390LabelInstruction::generateBinaryEncoding()
{
    ...
    if (tryToUseLabelTargetNOPs) {
      ...
      if (traceLabelTargetNOPs) {
        ...
        traceMsg(comp,
                 ...
                 this,
                 thisInst->getOpCode().getMnemonicName(),
                 instructionStart,
  ...
  if (tryToUseLabelTargetNOPs) {
    ...
    if (isOdd(labelTargetNOPLimit)) {
      if (traceLabelTargetNOPs)
        traceMsg(comp,
                 "\tlabelTargetNOPLimit %d is odd reduce by 1 to %d\n",
                 labelTargetNOPLimit,
                 labelTargetNOPLimit - 1);
      labelTargetNOPLimit--;
    }
  ...
  if (offset && (doUseLabelTargetNOPs || isNopCandidate())) {
    ...
    if (offset < 238) {
      ...
      instr = generateS390BranchInstruction(cg(),
                                            TR::InstOpCode::BRC,
      ...
      if (doUseLabelTargetNOPs) {
        ...
        if (traceLabelTargetNOPs)
          traceMsg(comp,
                   "\tbytesOfJump=%d : update offset %lld -> %lld\n",
                   bytesOfJump,
                   offset,
                   offset + bytesOfJump);
        offset += bytesOfJump;
    ...
    for (; offset;) {
      ...
      if (doUseLabelTargetNOPs) {
        ...
        if (traceLabelTargetNOPs)
          traceMsg(comp,
                   "\tbytesOfNOPs=%d : end update offset %lld -> %lld\n",
                   bytesOfNOPs,
                   offset,
                   offset + bytesOfNOPs);
        offset += bytesOfNOPs;
...
TR::S390LabelInstruction::considerForLabelTargetNOPs(bool inEncodingPhase)
{
  ...
  if (traceLabelTargetNOPs)
    traceMsg(comp,
             ...
             " %p (%s)\n",
             inEncodingPhase ? "encoding" : "estimating",
             this,
             thisInst->getOpCode().getMnemonicName(),
             prevInst,
             prevInst->getOpCode().getMnemonicName());
  ...
  if (isSkipForLabelTargetNOPs()) {
    ...
    if (traceLabelTargetNOPs)
      traceMsg(comp,
               "\t%p (%s) may need to be aligned : set doConsider=true\n",
               this,
               thisInst->getOpCode().getMnemonicName());
    doConsider = true;
  }
...
TR::S390LabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  ...
  if (comp->getOption(TR_EnableLabelTargetNOPs)) {
          ...
          false)) // inEncodingPhase=false (in estimating phase now)
    {
      ...
      if (traceLabelTargetNOPs)
        traceMsg(comp,
                 "\tupdate estimatedSize %d by maxNOPBump %d -> %d\n",
                 estimatedSize,
                 maxNOPBump,
...
void
TR::S390LabelInstruction::assignRegistersAndDependencies(
...
void
TR::S390BranchInstruction::assignRegistersAndDependencies(
  TR_RegisterKinds kindToBeAssigned)
{
  //
  // If there are any dependency conditions on this instruction, apply them.
...
TR::S390BranchInstruction::generateBinaryEncoding()
{
  ...
  if (doRelocation) {
    if (shortRelocation) {
      ...
      cg()->addProjectSpecializedRelocation(relocationPoint,
                                            NULL,
                                            NULL,
                                            TR_AbsoluteMethodAddress,
                                            __FILE__,
...
TR::S390BranchOnIndexInstruction::generateBinaryEncoding()
{
  ...
  if (doRelocation) {
    if (shortRelocation) {
      ...
      AOTcgDiag1(cg()->comp(),
                 "add TR_AbsoluteMethodAddress cursor=%x\n",
                 relocationPoint);
...
TR::S390DebugCounterBumpInstruction::generateBinaryEncoding()
{
  ...
  traceMsg(comp,
           "[%p] DCB using %s as scratch reg with spill=%s\n",
           this,
  ...
  if (std::find(comp->getStaticHCRPICSites()->begin(),
    ...
    cg()->addExternalRelocation(
      new (cg()->trHeapMemory()) TR::ExternalRelocation(
        (uint8_t*)locationToPatch, (uint8_t*)*locationToPatch, TR_HCR, cg()),
      __FILE__,
      __LINE__,
      getNode());
  }
...
void
TR::S390RegInstruction::assignRegistersNoDependencies(
  TR_RegisterKinds kindToBeAssigned)
{
  TR::Machine* machine = cg()->machine();
  setRegisterOperand(
  ...
  if (getTargetSnippet() &&
             ...
             getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL) {
    {
      ...
      } else // We need to do things the old fashioned way
      {
        ...
        if (((uint64_t)addr) >> 32) {
          ...
          //
          *(int32_t*)cursor =
            boi(0xC00E0000 | (uint32_t)(0x0000FFFF & (((uint64_t)addr) >> 48)));
          scratchReg->setRegisterField((uint32_t*)cursor);
          cursor += 4;
    ...
    if (getTargetLabel() != NULL) {
      ...
      if (getTargetSnippet() != NULL) {
        ...
        if (isHelper) {
          AOTcgDiag2(
            comp, "add TR_HelperAddress cursor=%p i2=%p\n", cursor, i2);
    ...
    if (getTargetSnippet() != NULL) {
       ...
       */
      TR_ASSERT(
        getTargetPtr() || (getTargetSymbol() && getTargetSymbol()->isMethod()),
        ...
        *(int32_t*)(cursor + 2) =
        ...
        if (sym && sym->castToMethodSymbol()->isHelper()) {
          ...
          cg()->addProjectSpecializedRelocation(
            cursor + 2,
            (uint8_t*)getSymbolReference()->getMethodAddress(),
            NULL,
    ...
    TR_ASSERT(0,
              "OpCode:%d is not handled in RILInstruction yet",
...
TR::S390RSInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  if (getMemoryReference() != NULL) {
    return getMemoryReference()->estimateBinaryLength(
      currentEstimate, cg(), this);
  } else {
  ...
  // immediate value to compare
    ...
    *(int16_t*)(cursor) |= bos((int16_t)getSourceImmediate16());
    TR_ASSERT(((getSourceImmediate16() & 0xB900) != 0xB900) ||
              ...
              "CLFIT has Immediate value 0xB9XX, signalHandler might confuse "
              "DIVCHK with BNDCHK");
  }
...
TR::S390RIEInstruction::splitIntoCompareAndLongBranch(void)
{
  ...
  if (isRRE || isRR)
    currInstr = generateRRInstruction(cg(),
                                      cmpOp,
                                      getNode(),
                                      getRegisterOperand(1),
    ...
    currInstr = generateRIInstruction(cg(),
                                      cmpOp,
                                      getNode(),
                                      getRegisterOperand(1),
                                      (int16_t)getSourceImmediate8(),
                                      prevInstr);
  else if (isRIL)
    currInstr = generateRILInstruction(cg(),
                                       cmpOp,
                                       getNode(),
                                       getRegisterOperand(1),
                                       (uint8_t)getSourceImmediate8(),
                                       prevInstr);
  ...
  currInstr = generateS390BranchInstruction(cg(),
                                            TR::InstOpCode::BRC,
                                            getBranchCondition(),
                                            getNode(),
                                            getBranchDestinationLabel(),
                                            prevInstr);
 ...
 */
void
TR::S390RIEInstruction::splitIntoCompareAndBranch(
...
void
appendElementSizeMnemonic(char* opCodeBuffer, int8_t elementSize)
{
...
TR::S390SMIInstruction::generateBinaryEncoding()
{
  ...
  if (distance >= MIN_RELOCATION_VAL && distance <= MAX_RELOCATION_VAL &&
    ...
    *(cursor) |= ((uint8_t)getMask() << 4);
    ...
    cg()->addRelocation(
      new (cg()->trHeapMemory())
        TR::LabelRelative16BitRelocation(cursor, getLabelSymbol(), 4, true));
...
TR::MemoryReference*
getFirstReadWriteMemoryReference(TR::Instruction* i)
{
