...
void TR::S390RSInstruction::generateAdditionalSourceRegisters(TR::Register* fReg, TR::Register* lReg)
{
  ...
  if (firstRegNum != lastRegNum && lastRegNum - firstRegNum > 1) {
    ...
    // _additionalRegisters = new (cg()->trHeapMemory(),TR_MemoryBase::Array) TR_Array<TR::Register *>(cg()->trMemory(),
    // numRegsToAdd, false);
...
uint8_t* TR::S390LabelInstruction::generateBinaryEncoding()
{
  ...
  if (tryToUseLabelTargetNOPs) {
    ...
    if (tryToUseLabelTargetNOPs) {
      ...
      // the offset that matters is the offset relative to the start of the object (the object start is 256 byte aligned
      // but programs in this object may only be 8 byte aligned) instructionStart-cg()->getCodeStart() is the offset
      // just for this one routine or nested routine so have to add in the object size for any already compiled separate
      // programs (batchObjCodeSize) and any offset for any already compiled parent programs for the nested case
      // (getCodeSize)
      ...
      if (traceLabelTargetNOPs)
        traceMsg(comp,
          "\toffsetForLabelTargetNOPs = absOffset 0x%p + batchObjCodeSize 0x%p + objCodeSize 0x%p = 0x%p\n",
      ...
      if (traceLabelTargetNOPs) {
        ...
        traceMsg(comp,
          "\tTR::S390LabeledInstruction %p (%s) at cursor %p (skipThisOne=%s, offset = %p, codeStart %p)\n", this,
          thisInst->getOpCode().getMnemonicName(), instructionStart, isSkipForLabelTargetNOPs() ? "true" : "false",
  ...
  if (tryToUseLabelTargetNOPs) {
    ...
    if (isOdd(labelTargetNOPLimit)) {
      if (traceLabelTargetNOPs)
        traceMsg(
          comp, "\tlabelTargetNOPLimit %d is odd reduce by 1 to %d\n", labelTargetNOPLimit, labelTargetNOPLimit - 1);
      labelTargetNOPLimit--;
    }
    ...
    if (traceLabelTargetNOPs)
      traceMsg(comp, "\tcomparing offset %lld >? labelTargetNOPLimit %d (offsetForLabelTargetNOPs = %p)\n", offset,
        labelTargetNOPLimit, offsetForLabelTargetNOPs);
    if (offset > labelTargetNOPLimit) {
      ...
      if (traceLabelTargetNOPs)
        traceMsg(comp,
          "\tset useLabelTargetNOPs = true : offsetForLabelTargetNOPs=%p > labelTargetNOPLimit %d, offset=%lld, cursor "
          "%p -> %p, labelTargetBytesInserted=%d\n",
  ...
  if (offset && (doUseLabelTargetNOPs || isNopCandidate())) {
    ...
    for (; offset;) {
      ...
      if (doUseLabelTargetNOPs) {
        ...
        if (traceLabelTargetNOPs)
          traceMsg(
            comp, "\tbytesOfNOPs=%d : end update offset %lld -> %lld\n", bytesOfNOPs, offset, offset + bytesOfNOPs);
        offset += bytesOfNOPs;
        offset = (uint64_t)offset & 0xff;
      } else {
        offset = (uint64_t)cursor & 0xff;
    ...
    if (doUseLabelTargetNOPs) {
      if (traceLabelTargetNOPs)
        traceMsg(comp,
          "\tfinished NOP insertion : setBinaryLength to 0, addAccumError = estBinLen %d - bytesInserted %d = %d, "
          "setBinaryEncoding to 0x%x\n",
          getEstimatedBinaryLength(), labelTargetBytesInserted, getEstimatedBinaryLength() - labelTargetBytesInserted,
          instructionStart);
      setBinaryLength(0);
      TR_ASSERT(getEstimatedBinaryLength() >= labelTargetBytesInserted,
        "label inst %p : estimatedBinaryLength %d must be >= labelTargetBytesInserted %d inserted\n", this,
...
bool TR::S390LabelInstruction::considerForLabelTargetNOPs(bool inEncodingPhase)
{
  TR::Compilation* comp = cg()->comp();
  TR::Instruction* thisInst = this;
  bool traceLabelTargetNOPs = comp->getOption(TR_TraceLabelTargetNOPs);
  ...
  if (traceLabelTargetNOPs)
    traceMsg(comp, "considerForLabelTargetNOPs check during %s phase : %p (%s), prev  %p (%s)\n",
      inEncodingPhase ? "encoding" : "estimating", this, thisInst->getOpCode().getMnemonicName(), prevInst,
  ...
  if (isSkipForLabelTargetNOPs()) {
    ...
    // this is the case where some instructions (e.g. SCHEDON/SCHEDOFF) have been inserted between the estimate and
    // encoding phases and therefore this routine would return false during estimation and true during encoding and the
    // estimate bump for NOPs would have been missed
    if (traceLabelTargetNOPs)
      ...
      if (getLabelSymbol() != NULL) {
        if (traceLabelTargetNOPs)
          traceMsg(comp, "\tupdate label %s (%p) estimatedCodeLocation with estimate %d + maxNOPBump %d = %d\n",
            cg()->getDebug()->getName(getLabelSymbol()), getLabelSymbol(), currentEstimate, maxNOPBump,
...
void TR::S390BranchInstruction::assignRegistersAndDependencies(TR_RegisterKinds kindToBeAssigned)
{
  ...
  if (getOpCode().isBranchOp() && getLabelSymbol()->isStartOfColdInstructionStream()) {
    ...
    TR_ASSERT(
      oi, "Could not find S390OutOfLineCodeSection stream from label.  instr=%p, label=%p\n", this, getLabelSymbol());
...
uint8_t* TR::S390BranchInstruction::generateBinaryEncoding()
{
  ...
  // msf - commented out since it does not work in cross-compile mode - TR_ASSERT(((binOpCode & 0xFF0F)== 0xA704),"Only
  // TR::InstOpCode::BRC is handled here\n");
...
void TR::S390RegInstruction::assignRegistersNoDependencies(TR_RegisterKinds kindToBeAssigned)
{
  TR::Machine* machine = cg()->machine();
...
uint8_t* TR::S390RRFInstruction::generateBinaryEncoding()
{
  ...
  switch (getKind()) {
  // note that RRD and RRF have swapped R1 and R3 positions but in both cases R1 is the target and R3 is a source (src2
  // in particular)
...
int32_t TR::S390RILInstruction::estimateBinaryLength(int32_t currentEstimate)
{
  ...
  if (TR::Compiler->target.is64Bit()
    ...
    //
    delta = 12;
  } else if (TR::Compiler->target.is64Bit() && getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL) {
    //  We could end up generating
    //  LLIHF Rscrtch, (addr&0xFFFFFFFF00000000)>>32
    //  IILF  Rscrtch, (addr&0xFFFFFFFF)
    ...
    && (getOpCode().getOpCodeValue() == TR::InstOpCode::STRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LRL)) {
    //  We could end up generating
    //  STG   Rscrtch, SPILLSLOT(,GPR5)
    //  LLIHF Rscrtch, (addr&0xFFFFFFFF00000000)>>32
    //  IILF  Rscrtch, (addr&0xFFFFFFFF)
...
uint8_t* TR::S390RILInstruction::generateBinaryEncoding()
{
  ...
  if (getTargetSnippet() && getTargetSnippet()->getKind() == TR::Snippet::IsConstantData) {
    ...
    cg()->addRelocation(
      new (cg()->trHeapMemory()) TR::LabelRelative32BitRelocation(cursor, getTargetSnippet()->getSnippetLabel()));
    cursor += getOpCode().getInstructionLength();
  } else if (getOpCode().getOpCodeValue() == TR::InstOpCode::LRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL
    || getOpCode().getOpCodeValue() == TR::InstOpCode::LGFRL || getOpCode().getOpCodeValue() == TR::InstOpCode::LLGFRL
    || getOpCode().getOpCodeValue() == TR::InstOpCode::STRL || getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL) {
    {
    ...
    if (std::find(comp->getStaticPICSites()->begin(), comp->getStaticPICSites()->end(), this)
      ...
      unloadableClass
        = (TR_OpaqueClassBlock*)cg()
            ->fe()
            ->createResolvedMethod(cg()->trMemory(), (TR_OpaqueMethodBlock*)getTargetPtr(), comp->getCurrentMethod())
            ->classOfMethod();
      doRegisterPIC = true;
    }
    ...
    if (getTargetLabel() != NULL) {
      ...
      if (getTargetSnippet() != NULL) {
        ...
        if (isHelper) {
          ...
          cg()->addProjectSpecializedRelocation(
            cursor + 2, (uint8_t*)getSymbolReference(), NULL, TR_HelperAddress, __FILE__, __LINE__, getNode());
        }
      }
    }
    cursor += getOpCode().getInstructionLength();
      ...
      if (comp->isRecursiveMethodTarget(getTargetSymbol())) {
        ...
        *(int32_t*)(cursor + 2) = boi(((intptrj_t)jitTojitStart - (intptrj_t)cursor) / 2);
        ...
        if (sym && sym->castToMethodSymbol()->isHelper()) {
          ...
          cg()->addProjectSpecializedRelocation(cursor + 2, (uint8_t*)getSymbolReference()->getMethodAddress(), NULL,
...
uint8_t* TR::S390RIEInstruction::generateBinaryEncoding()
{
  ...
  // if we don't have a branch destination (and not type RIE5), we have a 16-bit immediate value to compare
    ...
    *(int16_t*)(cursor) |= bos((int16_t)getSourceImmediate16());
    TR_ASSERT(
      ((getSourceImmediate16() & 0xB900) != 0xB900) || ((getOpCodeValue() != 0xEC71) && (getOpCodeValue() != 0xEC73)),
...
uint8_t* TR::S390RIEInstruction::splitIntoCompareAndLongBranch(void)
{
  ...
  if (isRRE || isRR)
    currInstr = generateRRInstruction(cg(), cmpOp, getNode(), getRegisterOperand(1), getRegisterOperand(2), prevInstr);
  else if (isRI)
    currInstr
  ...
  // set 'this' - the compare and branch instruction being removed, 'next' pointer to the instruction after the inserted
  // BRC/BRCL so the main cg generateBinaryEncoding loop does not encode the new CHI and BRC/BRCL instruction a second
  // time
...
uint8_t* TR::S390SS2Instruction::generateBinaryEncoding()
{
  ...
  // generate binary for the length field
  TR_ASSERT(
    getLen() >= 0 && getLen() <= 15, "SS2 L1 must be in the range 0->15 and not %d from [%p]\n", getLen(), getNode());
...
uint8_t* TR::S390MIIInstruction::generateBinaryEncoding()
{
  ...
  if (distance >= MIN_12_RELOCATION_VAL && distance <= MAX_12_RELOCATION_VAL && !bpp && offset >= MIN_24_RELOCATION_VAL
...
uint8_t* TR::S390VirtualGuardNOPInstruction::generateBinaryEncoding()
{
  ...
  if (performEmptyPatch) {
    ...
    for (TR::Instruction* nextI = getNext(); nextI != NULL; nextI = nextI->getNext()) {
      ...
      if (nextI->getOpCodeValue() == TR::InstOpCode::LABEL || nextI->getOpCodeValue() == TR::InstOpCode::BRCL
        || nextI->getOpCodeValue() == TR::InstOpCode::BRASL || nextI->getOpCodeValue() == TR::InstOpCode::DC
  ...
  // If we scanned ahead and see that the estimated binary length is zero (i.e. return point of slow path is right after
  // the guard - methodEnter/Exit is an example of such) we'll just generate the NOP branch.
