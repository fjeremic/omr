...
void TR::S390RSInstruction::generateAdditionalSourceRegisters(
  ...
  if (firstRegNum != lastRegNum && lastRegNum - firstRegNum > 1) {
    ...
    // _additionalRegisters = new (cg()->trHeapMemory(),TR_MemoryBase::Array)
    // TR_Array<TR::Register *>(cg()->trMemory(), numRegsToAdd, false);
    int8_t curReg = firstRegNum + 1;
    for (int8_t i = 0; i < numRegsToAdd; i++) {
      // (*_additionalRegisters)[i] =
      // machine->getRealRegister(((TR::RealRegister::RegNum)curReg));
      TR::Register *temp =
          machine->getRealRegister(((TR::RealRegister::RegNum)curReg));
...
bool isLoopEntryAlignmentEnabled(TR::Compilation *comp) {
  if (comp->getOption(TR_DisableLoopEntryAlignment)) return false;
  if (comp->getOption(TR_EnableLoopEntryAlignment)) return true;

  // Loop alignment not worth the extra bytes for <= warm compilations because
  // if a loop at warm would have exhibited benefit from alignment then the loop
  // must have been hot enough for us to recompile the method to > warm in the
  // first place.

  if (comp->getOptLevel() <= warm) return false;

  if (comp->getOption(
          TR_EnableLabelTargetNOPs))  // only one type of NOP insertion is
                                      // currently supported
    return false;
...
bool TR::S390LabeledInstruction::isNopCandidate() {
  ...
  if (!isLoopEntryAlignmentEnabled(comp)) return false;
  ...
  if (node != NULL && node->getOpCodeValue() == TR::BBStart &&
    ...
    if (block->firstBlockInLoop() && !block->isCold() &&
        block->getFrequency() > 1000) {
      TR_BlockStructure *blockStructure = block->getStructureOf();
      if (blockStructure) {
        TR_RegionStructure *region =
            (TR_RegionStructure *)blockStructure->getContainingLoop();
        if (region) {
          ...
          for (auto e = entry->getPredecessors().begin();
               e != entry->getPredecessors().end(); ++e) {
            TR::Block *predBlock = toBlock((*e)->getFrom());
            if (!region->contains(predBlock->getStructureOf(),
                                  region->getParent()))
              continue;
            // Backedge found.
            ...
            for (currInstr = firstInstr;; currInstr = currInstr->getNext()) {
              if (currInstr == NULL) {
                // We might be here if the loop structure isn't consecutive in
                // memory
                return false;
              }

              loopLength += currInstr->getOpCode().getInstructionLength();
              if (loopLength > 256) return false;
              if (currInstr == lastInstr) break;
            }

            // Determine if the loop will cross cache line boundry
            uint8_t *cursor = cg()->getBinaryBufferCursor();
            if (loopLength <= 256 &&
                (((uint64_t)cursor + loopLength) & 0xffffff00) >
                    ((uint64_t)cursor & 0xffffff00)) {
              isNopCandidate = true;
...
/**
 * LabelInstruction is a pseudo instruction that generates a label or constant
 * (with a label)
 *
...
uint8_t *TR::S390LabelInstruction::generateBinaryEncoding() {
  ...
  if (tryToUseLabelTargetNOPs) {
    tryToUseLabelTargetNOPs =
        considerForLabelTargetNOPs(true);  // inEncodingPhase=true
    if (tryToUseLabelTargetNOPs) {
      offsetForLabelTargetNOPs = instructionStart - cg()->getCodeStart();
      // the offset that matters is the offset relative to the start of the
      // object (the object start is 256 byte aligned but programs in this
      // object may only be 8 byte aligned)
      // instructionStart-cg()->getCodeStart() is the offset just for this one
      // routine or nested routine so have to add in the object size for any
      // already compiled separate programs (batchObjCodeSize) and any offset
      // for any already compiled parent programs for the nested case
      // (getCodeSize)
      ...
      if (traceLabelTargetNOPs)
        traceMsg(comp,
                 "\toffsetForLabelTargetNOPs = absOffset 0x%p + "
                 "batchObjCodeSize 0x%p + objCodeSize 0x%p = 0x%p\n",
                 offsetForLabelTargetNOPs, batchObjCodeSize, objCodeSize,
                 offsetForLabelTargetNOPs + batchObjCodeSize + objCodeSize);

      offsetForLabelTargetNOPs =
          offsetForLabelTargetNOPs + batchObjCodeSize + objCodeSize;
      ...
      if (traceLabelTargetNOPs) {
        ...
        traceMsg(comp,
                 "\tTR::S390LabeledInstruction %p (%s) at cursor %p "
                 "(skipThisOne=%s, offset = %p, codeStart %p)\n",
                 this, thisInst->getOpCode().getMnemonicName(),
                 instructionStart,
                 isSkipForLabelTargetNOPs() ? "true" : "false",
                 offsetForLabelTargetNOPs, cg()->getCodeStart());
        traceMsg(comp, "\tprev %p (%s)\n", prevInst,
                 prevInst->getOpCode().getMnemonicName());
      }
    }
  }
  ...
  memset((void *)cursor, 0, getEstimatedBinaryLength());
  uint16_t binOpCode =
      *(uint16_t *)(getOpCode().getOpCodeBinaryRepresentation());
  ...
  if (getOpCode().getOpCodeValue() == TR::InstOpCode::DC) {
    ...
    cg()->addRelocation(new (cg()->trHeapMemory())
                            TR::LabelAbsoluteRelocation(cursor, label));
    cg()->addProjectSpecializedRelocation(cursor, NULL, NULL,
                                          TR_AbsoluteMethodAddress, __FILE__,
                                          __LINE__, getNode());
  ...
  } else  // must be real LABEL instruction
  {
    ...
    if (_alignment != 0) {
      if (tryToUseLabelTargetNOPs && traceLabelTargetNOPs)
        traceMsg(comp,
                 "force tryToUseLabelTargetNOPs to false because _alignment "
                 "already needed on inst %p\n",
                 this);
      tryToUseLabelTargetNOPs = false;

      int32_t padding =
          _alignment - (((uintptrj_t)instructionStart) % _alignment);
    ...
    TR_ASSERT(getOpCode().getOpCodeValue() == TR::InstOpCode::LABEL,
              "LabelInstr not DC or LABEL, what is it??");
  }
  ...
  if (tryToUseLabelTargetNOPs) {
    ...
    if (traceLabelTargetNOPs)
      traceMsg(comp,
               "\tcomparing offset %lld >? labelTargetNOPLimit %d "
               "(offsetForLabelTargetNOPs = %p)\n",
    ...
    if (offset > labelTargetNOPLimit) {
      ...
      if (traceLabelTargetNOPs)
        traceMsg(comp,
                 "\tset useLabelTargetNOPs = true : "
                 "offsetForLabelTargetNOPs=%p > labelTargetNOPLimit %d, "
                 "offset=%lld, cursor %p -> %p, labelTargetBytesInserted=%d\n",
                 offsetForLabelTargetNOPs, labelTargetNOPLimit, offset, cursor,
                 newInstructionStart, labelTargetBytesInserted);
    }
  } else {
  ...
  if (offset && (doUseLabelTargetNOPs || isNopCandidate())) {
    ...
    getLabelSymbol()->setCodeLocation(
        newInstructionStart);  // Label location need to be updated
    assert(offset % 2 == 0);
    TR::Instruction *prevInstr = getPrev();
    TR::Instruction *instr;
    TR_Debug *debugObj = cg()->getDebug();
    // Insert a BRC instruction to skip NOP instructions if there're more than 3
    // NOPs to insert
    if (offset < 238) {
      uint8_t *curBeforeJump = cursor;
      instr = generateS390BranchInstruction(
          cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK15, getNode(),
      ...
      if (debugObj)
        debugObj->addInstructionComment(
            instr, "Skip NOP instructions for loop alignment");
      if (doUseLabelTargetNOPs) {
    ...
    if (doUseLabelTargetNOPs) {
      if (traceLabelTargetNOPs)
        traceMsg(
            comp,
            "\tfinished NOP insertion : setBinaryLength to 0, addAccumError = "
            "estBinLen %d - bytesInserted %d = %d, setBinaryEncoding to 0x%x\n",
            getEstimatedBinaryLength(), labelTargetBytesInserted,
            getEstimatedBinaryLength() - labelTargetBytesInserted,
            instructionStart);
      setBinaryLength(0);
      TR_ASSERT(getEstimatedBinaryLength() >= labelTargetBytesInserted,
                "label inst %p : estimatedBinaryLength %d must be >= "
                "labelTargetBytesInserted %d inserted\n",
                this, getEstimatedBinaryLength(), labelTargetBytesInserted);
      cg()->addAccumulatedInstructionLengthError(getEstimatedBinaryLength() -
                                                 labelTargetBytesInserted);
      setBinaryEncoding(instructionStart);
      return cursor;
    }
  }
...
bool TR::S390LabelInstruction::considerForLabelTargetNOPs(
    bool inEncodingPhase) {
  TR::Compilation *comp = cg()->comp();
  TR::Instruction *thisInst = this;
  bool traceLabelTargetNOPs = comp->getOption(TR_TraceLabelTargetNOPs);
  TR::Instruction *prevInst = getPrev();
  while (prevInst &&
         prevInst->getOpCode().getOpCodeValue() == TR::InstOpCode::ASSOCREGS)
    prevInst = prevInst->getPrev();

  bool doConsider = true;
  if (traceLabelTargetNOPs)
    traceMsg(comp,
             "considerForLabelTargetNOPs check during %s phase : %p (%s), prev "
             " %p (%s)\n",
             inEncodingPhase ? "encoding" : "estimating", this,
             thisInst->getOpCode().getMnemonicName(), prevInst,
             prevInst->getOpCode().getMnemonicName());

  if (isSkipForLabelTargetNOPs()) {
    if (traceLabelTargetNOPs)
      traceMsg(comp,
               "\t%p (%s) marked with isSkipForLabelTargetNOPs = true : set "
               "doConsider=false\n",
               this, thisInst->getOpCode().getMnemonicName());
    doConsider = false;
  } else if (prevInst &&
             prevInst->getOpCode().getOpCodeValue() == TR::InstOpCode::BASR) {
    if (traceLabelTargetNOPs)
      traceMsg(comp, "\tprev %p is a BASR : set doConsider=false\n", prevInst);
    doConsider = false;
  } else if (inEncodingPhase && !wasEstimateDoneForLabelTargetNOPs()) {
    // this is the case where some instructions (e.g. SCHEDON/SCHEDOFF) have
    // been inserted between the estimate and encoding phases and therefore this
    // routine would return false during estimation and true during encoding and
    // the estimate bump for NOPs would have been missed
    if (traceLabelTargetNOPs)
      traceMsg(comp,
               "\t%p (%s) marked with wasEstimateDoneForLabelTargetNOPs = "
               "false : set doConsider=false\n",
               this, thisInst->getOpCode().getMnemonicName());
    doConsider = false;
  } else {
    if (traceLabelTargetNOPs)
...
int32_t TR::S390LabelInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  ...
  if (comp->getOption(TR_EnableLabelTargetNOPs)) {
    if (considerForLabelTargetNOPs(
            false))  // inEncodingPhase=false (in estimating phase now)
    {
      bool traceLabelTargetNOPs = comp->getOption(TR_TraceLabelTargetNOPs);
      ...
      if (getLabelSymbol() != NULL) {
        if (traceLabelTargetNOPs)
          traceMsg(comp,
                   "\tupdate label %s (%p) estimatedCodeLocation with estimate "
                   "%d + maxNOPBump %d = %d\n",
                   cg()->getDebug()->getName(getLabelSymbol()),
                   getLabelSymbol(), currentEstimate, maxNOPBump,
                   currentEstimate + maxNOPBump);
        getLabelSymbol()->setEstimatedCodeLocation(currentEstimate +
                                                   maxNOPBump);
      }
      if (traceLabelTargetNOPs)
...
void TR::S390LabelInstruction::assignRegistersAndDependencies(
  ...
  if (getDependencyConditions()) {
    block(_sourceReg, _sourceRegSize, _targetReg, _targetRegSize, _targetMem,
          _sourceMem);
    getDependencyConditions()->assignPostConditionRegisters(
        this, kindToBeAssigned, cg());
    unblock(_sourceReg, _sourceRegSize, _targetReg, _targetRegSize, _targetMem,
            _sourceMem);
  }
...
void TR::S390BranchInstruction::assignRegistersAndDependencies(
    TR_RegisterKinds kindToBeAssigned) {
  //
  // If there are any dependency conditions on this instruction, apply them.
  // Any register or memory references must be blocked before the condition
  ...
  if (getOpCode().isBranchOp() &&
    ...
    TR_S390OutOfLineCodeSection *oi =
        cg()->findS390OutOfLineCodeSectionFromLabel(getLabelSymbol());
    TR_ASSERT(oi,
              "Could not find S390OutOfLineCodeSection stream from label.  "
              "instr=%p, label=%p\n",
              this, getLabelSymbol());
    if (!oi->hasBeenRegisterAssigned()) oi->assignRegisters(kindToBeAssigned);
  }
  if (getOpCode().getOpCodeValue() ==
      TR::InstOpCode::AP /*&&     toS390SS2Instruction(this)->getLabel()*/) {
    TR_S390OutOfLineCodeSection *oi =
        cg()->findS390OutOfLineCodeSectionFromLabel(
  ...
  if (getOpCode().isBranchOp() &&
      getLabelSymbol()->isEndOfColdInstructionStream()) {
    // This if statement prevents RA to restore register snapshot on regular
    // branches to the OOL section merging point. Register snapshot is a
    // snapshot of register states taken at OOL merge label. Using this snapshot
    // RA can enforce the similarity of register states at the end of
    // main-stream code and OOL path. Generally the safer option is to not reuse
    // OOL merge label for any other purpose. This can be done by creating an
    // extra label right after merge point label.
    if (cg()->getIsInOOLSection()) {
      // Branches from inside an OOL section to the merge-points are not
      // allowed. Branches in the OOL section can jump to the end of section and
      // then only one branch (the last instruction of an OOL section) jumps to
      // the merge-point. In other words, OOL section must contain exactly one
      // exit point.
    ...
    else if (comp->getOptions()->getRegisterAssignmentTraceOption(
                 TR_TraceRARegisterStates))
      traceMsg(comp,
               "\nOOL: Reusing the OOL Section merge label for other branches "
               "might be unsafe.\n");
  }
}
...
/**
 *
 * BranchInstruction is a pseudo instruction that generates instructions to
 * branch to a symbol.
 *
 * Currently, this pseudo instruction handles the following 2 scenarios:
 *
 ...
 *
 *     On G5, the instruction will be a branch relative or branch on condition
 * instruction. If the branch distance is within +/- 2**16, it will be done with
 * a relative branch. Otherwise, it will be done with the following (horrible)
 * sequence: BRAS r7, 4 DC   branching target address L    r7,0(,r7) BCR  r7
 *     Subtle note: since 64-bit is running on a Freeway or above (by
 * definition), slow muck will not be generated. Note this is different than
 * BranchOnCount and BranchOnIndex since they don't have long branch equivalents
 * for themselves.
 */
uint8_t *TR::S390BranchInstruction::generateBinaryEncoding() {
  ...
  // msf - commented out since it does not work in cross-compile mode -
  // TR_ASSERT(((binOpCode & 0xFF0F)== 0xA704),"Only TR::InstOpCode::BRC is
  // handled here\n");
  ...
  if (label->getCodeLocation() != NULL) {
    ...
    distance =
        (cg()->getBinaryBufferStart() + label->getEstimatedCodeLocation()) -
  ...
  if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
    ...
    if (getOpCode().isBranchOp() &&
        (getBranchCondition() != TR::InstOpCode::COND_NOP)) {
  ...
  if (doRelocation) {
    if (shortRelocation) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR::LabelRelative16BitRelocation(relocationPoint, label));
    } else if (longRelocation) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR::LabelRelative32BitRelocation(relocationPoint, label));
    } else {
      AOTcgDiag1(comp, "add TR_AbsoluteMethodAddress cursor=%x\n", cursor);
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR::LabelAbsoluteRelocation(relocationPoint, label));
      cg()->addProjectSpecializedRelocation(relocationPoint, NULL, NULL,
                                            TR_AbsoluteMethodAddress, __FILE__,
                                            __LINE__, getNode());
    }
  }
...
int32_t TR::S390BranchInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  int32_t length = 6;
  TR::Compilation *comp = cg()->comp();
...
/**
 * BranchOnCountInstruction is a pseudo instruction that generates instructions
 * to branch on count to a symbol.
 *
 ...
 * instruction. If the branch distance is within +/- 2**16, it will be done with
 * a relative branch. Otherwise, it will be done with the following (horrible)
 * sequence: BRAS r7, 4 DC   branching target address L    r7,0(,r7) BCT  Rx,R7
...
uint8_t *TR::S390BranchOnCountInstruction::generateBinaryEncoding() {
  ...
  if ((binOpCode & 0xFF0F) == 0xCC06)  // BRCTH
  {
    ...
    *(int32_t *)(cursor + 2) |= boi(distance);
    relocationPoint = cursor + 2;
    cursor += getOpCode().getInstructionLength();
    toRealRegister(getRegisterOperand(1))
        ->setRegisterField((uint32_t *)instructionStart);
    if (doRelocation) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR::LabelRelative32BitRelocation(relocationPoint, label));
    ...
    if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
      ...
      *(int16_t *)(cursor + 2) |= bos(distance);
      ...
      TR_ASSERT(((binOpCode & 0xFF0F) == 0xA706),
                "Only TR::InstOpCode::BRCT is handled here\n");

      TR::LabelSymbol *relLabel =
          TR::LabelSymbol::createRelativeLabel(cg()->trHeapMemory(), cg(), +4);
      setLabelSymbol(relLabel);

      relLabel =
          TR::LabelSymbol::createRelativeLabel(cg()->trHeapMemory(), cg(), +5);

      TR::Instruction *instr;
      instr = generateS390BranchInstruction(cg(), TR::InstOpCode::BRC,
                                            TR::InstOpCode::COND_MASK15,
                                            getNode(), relLabel, this);
      instr->setEstimatedBinaryLength(4);
      instr = generateS390BranchInstruction(cg(), TR::InstOpCode::BRC,
                                            TR::InstOpCode::COND_MASK15,
                                            getNode(), label, instr);
...
int32_t TR::S390BranchOnCountInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
/**
 * BranchOnIndexInstruction is a pseudo instruction that generates instructions
 * to branch to a symbol.
 *
 * Currently, this pseudo instruction handles the following scenario:
 *
 * 1.  emit branch instruction to a label.
 ...
 * instruction. If the branch distance is within +/- 2**16, it will be done with
 * a relative branch. Otherwise, it will be done with the following (horrible)
 * sequence: BRAS r7, 4 DC   branching target address L    r7,0(,r7) BXL/BXH
 * rx,ry,r7 MSF:: NB::: 64-bit support needs to be added -- Done: LD LD: could
 * do better for long displacement on N3 and 64bit--TODO
 */
  ...
  if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
    ...
    *(int16_t *)(cursor + 2) |= bos(distance);
    ...
    TR_ASSERT_FATAL(false,
                    "Cannot encode branch on index instruction because "
                    "distance (%d) is out of range",
                    distance);
  }
...
int32_t TR::S390BranchOnIndexInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  setEstimatedBinaryLength(getOpCode().getInstructionLength());
  ...
  if (_fenceNode != NULL)  // must be fence node
  {
    if (_fenceNode->getRelocationType() == TR_AbsoluteAddress) {
      for (int32_t i = 0; i < _fenceNode->getNumRelocations(); ++i) {
        *(uint8_t **)(_fenceNode->getRelocationDestination(i)) =
            instructionStart;
      }
    } else if (_fenceNode->getRelocationType() == TR_EntryRelative32Bit) {
      for (int32_t i = 0; i < _fenceNode->getNumRelocations(); ++i) {
        *(uint32_t *)(_fenceNode->getRelocationDestination(i)) =
            boi(cg()->getCodeLength());
      }
    } else  // entryrelative16bit
    {
      for (int32_t i = 0; i < _fenceNode->getNumRelocations(); ++i) {
        *(uint16_t *)(_fenceNode->getRelocationDestination(i)) =
            bos((uint16_t)cg()->getCodeLength());
      }
    }
  }
  setBinaryLength(0);

  if (_callDescLabel != NULL)  // We have to emit a branch around a 8-byte
                               // aligned call descriptor.
  {
    // For zOS-31 XPLINK, if the call descriptor is too far away from the native
    // call NOP, we have to manually emit a branch around the call descriptor in
    // the main line code.
    //    BRC  4 + Padding
    //    <Padding>
...
int32_t TR::S390PseudoInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
...
/**
 * This instruction is bumps the value at the snippet corresponding to the
 * address of the debug counter count. The valid opcode is TR::InstOpCode::DCB
 *
 * Either 2 or 4 instructions are encoded matching the following conditions:
 *
 * If a free register was saved:                      Otherwise if a spill is
 * required: STG     Rscrtch,   offToLongDispSlot(,GPR5) LGRL    Rscrtch,
 * counterRelocation               LGRL    Rscrtch,   counterRelocation AGSI
 * 0(Rscrtch),   delta                           AGSI 0(Rscrtch),   delta LG
 * Rscrtch,   offToLongDispSlot(,GPR5)
 *
...
uint8_t *TR::S390DebugCounterBumpInstruction::generateBinaryEncoding() {
  ...
  int32_t offsetToLongDispSlot =
      (cg()->getLinkage())->getOffsetToLongDispSlot();
  ...
  TR_ASSERT(scratchReg != NULL,
            "TR_S390DebugCounterBumpInstruction::generateBinaryEncoding -- A "
            "scratch reg should always be found.");
  ...
  if (spillNeeded) {
    *(int32_t *)cursor =
        boi(0xE3005000 | (offsetToLongDispSlot &
                          0xFFF));  // STG Rscrtch,offToLongDispSlot(,GPR5)
  ...
  *(int16_t *)cursor = bos(0xC408);  // LGRL Rscrtch, counterRelocation
  scratchReg->setRegisterField((uint32_t *)cursor);  //
  cg()->addRelocation(new (cg()->trHeapMemory())     //
                      TR::LabelRelative32BitRelocation(
                          cursor, getCounterSnippet()->getSnippetLabel()));  //
  cursor += 6;                                                               //

  *(int32_t *)cursor =
      boi(0xEB000000 | (((int8_t)getDelta()) << 16));  // On 64-bit platforms
  scratchReg->setBaseRegisterField(
      (uint32_t *)cursor);  // AGSI 0(Rscrtch), delta
  cursor += 4;              //
  *(int16_t *)cursor = TR::Compiler->target.is64Bit()
                           ? bos(0x007A)
                           : bos(0x006A);  // On 32-bit platforms
  cursor += 2;                             //  ASI 0(Rscrtch), delta

  if (spillNeeded) {
    *(int32_t *)cursor =
        boi(0xE3005000 | (offsetToLongDispSlot &
                          0xFFF));  // LG Rscrtch,offToLongDispSlot(,GPR5)
    scratchReg->setRegisterField((uint32_t *)cursor);  //
    cursor += 4;                                       //
...
uint8_t *TR::S390ImmInstruction::generateBinaryEncoding() {
  ...
  uint8_t *cursor = instructionStart;
  memset((void *)cursor, 0, getEstimatedBinaryLength());
  TR_ASSERT(getOpCode().getOpCodeValue() == TR::InstOpCode::DC,
            "ImmInstruction is for TR::InstOpCode::DC only!");
  TR::Compilation *comp = cg()->comp();
  ...
  */
  ...
  if (std::find(comp->getStaticPICSites()->begin(),
                comp->getStaticPICSites()->end(),
                this) != comp->getStaticPICSites()->end()) {
    // On 64-bit, the second word of the pointer is the one in
    // getStaticPICSites. We can't register the first word because the second
    // word wouldn't yet be binary-encoded by the time we need to compute the
    // proper hash key.
    //
    void **locationToPatch =
        (void **)(cursor - (TR::Compiler->target.is64Bit() ? 4 : 0));
    cg()->jitAddPicToPatchOnClassUnload(*locationToPatch, locationToPatch);
  }
  if (std::find(comp->getStaticHCRPICSites()->begin(),
                comp->getStaticHCRPICSites()->end(),
                this) != comp->getStaticHCRPICSites()->end()) {
    // On 64-bit, the second word of the pointer is the one in
    // getStaticHCRPICSites. We can't register the first word because the second
    // word wouldn't yet be binary-encoded by the time we need to compute the
    // proper hash key.
    //
    void **locationToPatch =
        (void **)(cursor - (TR::Compiler->target.is64Bit() ? 4 : 0));
    cg()->jitAddPicToPatchOnClassRedefinition(*locationToPatch,
                                              locationToPatch);
    cg()->addExternalRelocation(
        new (cg()->trHeapMemory())
            TR::ExternalRelocation((uint8_t *)locationToPatch,
                                   (uint8_t *)*locationToPatch, TR_HCR, cg()),
        __FILE__, __LINE__, getNode());
  }
 ...
 */
TR::S390ImmInstruction *TR::S390ImmInstruction::getS390ImmInstruction() {
  return this;
}
#endif
...
uint8_t *TR::S390Imm2Instruction::generateBinaryEncoding() {
  ...
  uint8_t *cursor = instructionStart;
  memset((void *)cursor, 0, getEstimatedBinaryLength());
  TR_ASSERT(getOpCode().getOpCodeValue() == TR::InstOpCode::DC2,
            "ImmInstruction is for TR::InstOpCode::DC2 only!");
  TR::Compilation *comp = cg()->comp();
...
void TR::S390RegInstruction::assignRegistersNoDependencies(
    TR_RegisterKinds kindToBeAssigned) {
  TR::Machine *machine = cg()->machine();
  setRegisterOperand(
      1, machine->assignBestRegister(getRegisterOperand(1), this, BOOKKEEPING));
...
uint8_t *TR::S390RRInstruction::generateBinaryEncoding() {
  ...
  } else  // RR
  {
    ...
    if (getFirstConstant() >= 0) {
      ...
      toRealRegister(getRegForBinaryEncoding(getRegisterOperand(i++)))
          ->setRegister1Field((uint32_t *)cursor);
    if (getSecondConstant() >= 0) {
      (*(uint32_t *)cursor) &= boi(0xFFF0FFFF);
      (*(uint32_t *)cursor) |= boi((getSecondConstant() & 0xF) << 16);
    } else
      toRealRegister(getRegForBinaryEncoding(getRegisterOperand(i)))
          ->setRegister2Field((uint32_t *)cursor);
  }
...
uint8_t *TR::S390TranslateInstruction::generateBinaryEncoding() {
  ...
  toRealRegister(getRegisterOperand(2))
      ->setRegisterField((uint32_t *)cursor, 0);
  toRealRegister(getRegForBinaryEncoding(getRegisterOperand(1)))
      ->setRegisterField((uint32_t *)cursor, 1);
...
uint8_t *TR::S390RRFInstruction::generateBinaryEncoding() {
  ...
  TR::RegisterPair *srcRegPair =
      srcReg != NULL ? srcReg->getRegisterPair() : NULL;
  if (srcRegPair != NULL) isSrcPair = true;
  ...
  TR::RegisterPair *tgtRegPair =
      tgtReg != NULL ? tgtReg->getRegisterPair() : NULL;
  if (tgtRegPair != NULL) isTgtPair = true;

  switch (getKind()) {
    // note that RRD and RRF have swapped R1 and R3 positions but in both cases
    // R1 is the target and R3 is a source (src2 in particular)
    case TR::Instruction::IsRRD:  // nnnn 1x32 e.g. MSDBR/MSEBR/MSER/MSDR
      if (!isTgtPair)
        toRealRegister(tgtReg)->setRegisterField(
            (uint32_t *)cursor, 3);  // encode target in R1 position
      else
        toRealRegister(tgtRegPair->getHighOrder())
            ->setRegisterField((uint32_t *)cursor, 3);

      if (!isSrcPair)
        toRealRegister(srcReg)->setRegisterField(
            (uint32_t *)cursor, 0);  // encode src in R2 position
      else
        toRealRegister(srcRegPair->getHighOrder())
            ->setRegisterField((uint32_t *)cursor, 0);

      if (!isSrc2Pair)
        toRealRegister(getRegisterOperand(3))
            ->setRegisterField((uint32_t *)cursor,
                               1);  // encode src2 in R3 position
      else
        toRealRegister(src2RegPair->getHighOrder())
            ->setRegisterField((uint32_t *)cursor, 1);
    ...
    case TR::Instruction::IsRRF:  // nnnn 3x12 e.g. IEDTR/IEXTR
      if (!isTgtPair)
        toRealRegister(tgtReg)->setRegisterField(
            (uint32_t *)cursor, 1);  // encode target in R1 position
      else
        toRealRegister(tgtRegPair->getHighOrder())
            ->setRegisterField((uint32_t *)cursor, 1);
    ...
    case TR::Instruction::IsRRF2:  // M3,  ., R1, R2
      if (tgtReg != NULL) {
        if (!isTgtPair)
          toRealRegister(getRegForBinaryEncoding(tgtReg))
              ->setRegisterField((uint32_t *)cursor, 1);
        else
          toRealRegister(tgtRegPair->getHighOrder())
              ->setRegisterField((uint32_t *)cursor, 1);
      ...
      if (srcReg != NULL) {
        if (!isSrcPair)
          toRealRegister(getRegForBinaryEncoding(srcReg))
              ->setRegisterField((uint32_t *)cursor, 0);
        else
          toRealRegister(srcRegPair->getHighOrder())
              ->setRegisterField((uint32_t *)cursor, 0);
...
int32_t TR::S390RILInstruction::estimateBinaryLength(int32_t currentEstimate) {
  ...
  if (TR::Compiler->target.is64Bit() &&
    ...
    // LARL could also have 8 more bytes for patching if target becomes
    // unaddressable.
    delta = 10;
  } else {
    delta = 2;
 ...
 * This method calculates the target address by computing the offset in
 * halfwords. This value will be used by binary encoding to set the correct
 * target. Example of instruction that uses this are BRCL and BRASL instruction.
 * Usually its used for getting helper method's target addresses.
 *
 ...
 */
int32_t TR::S390RILInstruction::adjustCallOffsetWithTrampoline(
    int32_t offset, uint8_t *currentInst) {
  int32_t offsetHalfWords = offset;

  // Check to make sure that we can reach our target!  Otherwise, we need to
  // look up appropriate trampoline and branch through the trampoline.

  if (cg()->directCallRequiresTrampoline(getTargetPtr(),
                                         (intptrj_t)currentInst)) {
    intptrj_t targetAddr;

#if defined(CODE_CACHE_TRAMPOLINE_DEBUG)
    printf("Target: %p,  Cursor: %p, Our Reference # is: %d\n", getTargetPtr(),
           (uintptrj_t)currentInst, getSymbolReference()->getReferenceNumber());
#endif
    if (getSymbolReference()->getReferenceNumber() < TR_S390numRuntimeHelpers)
      ...
      targetAddr = cg()->fe()->methodTrampolineLookup(
          cg()->comp(), getSymbolReference(), (void *)currentInst);

    TR_ASSERT_FATAL(
        TR::Compiler->target.cpu.isTargetWithinBranchRelativeRILRange(
            targetAddr, (intptrj_t)currentInst),
...
uint8_t *TR::S390RILInstruction::generateBinaryEncoding() {
  ...
  if (getTargetSnippet() &&
    ...
    cg()->addRelocation(new (cg()->trHeapMemory())
                            TR::LabelRelative32BitRelocation(
                                cursor, getTargetSnippet()->getSnippetLabel()));
    cursor += getOpCode().getInstructionLength();
             ...
             getOpCode().getOpCodeValue() == TR::InstOpCode::STGRL) {
    {
      ...
      if (TR::Compiler->target.cpu.isTargetWithinBranchRelativeRILRange(
              (intptrj_t)getTargetPtr(), (intptrj_t)cursor)) {
        getOpCode().copyBinaryToBuffer(instructionStart);
        toRealRegister(getRegisterOperand(1))
            ->setRegister1Field((uint32_t *)cursor);
      ...
      } else  // We need to do things the old fashioned way
      {
        TR_ASSERT(TR::Compiler->target.is64Bit(),
                  "We should only be here on 64-bit platforms\n");
        ...
        if (getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL ||
            getOpCode().getOpCodeValue() == TR::InstOpCode::LGFRL ||
            getOpCode().getOpCodeValue() == TR::InstOpCode::LLGFRL) {
          sourceReg = (TR::RealRegister *)((TR::S390RegInstruction *)this)
          ...
          *(int32_t *)cursor =
              boi(0xE3005000 |
                  (offsetToLongDispSlot &
                   0xFFF));  // STG scratchReg, offToLongDispSlot(,GPR5)
          scratchReg->setRegisterField((uint32_t *)cursor);
          cursor += 4;
          *(int16_t *)cursor = bos(0x0024);
          cursor += 2;
        ...
        if (((uint64_t)addr) >> 32) {
          ...
          //
          *(int32_t *)cursor = boi(
              0xC00E0000 | (uint32_t)(0x0000FFFF & (((uint64_t)addr) >> 48)));
          scratchReg->setRegisterField((uint32_t *)cursor);
          cursor += 4;
          *(int16_t *)cursor = bos(0xFFFF & (uint16_t)(((uint64_t)addr) >> 32));
          cursor += 2;

          if (addr & 0xFFFFFFFF)  //  LLIHF does zero out low word... so only do
                                  //  IILF if non-NULL
          {
            // IILF scratchReg, <addr&0x00000000FFFFFFFF>
            //
            *(int32_t *)cursor =
                boi(0xC0090000 | (0xFFFF & ((uint32_t)addr >> 16)));
            scratchReg->setRegisterField((uint32_t *)cursor);
            cursor += 4;
        ...
        } else  // Note, we must handle NULL correctly
        {
          ...
          //
          *(int32_t *)cursor =
              boi(0xC00F0000 | (0xFFFF & ((uint32_t)addr >> 16)));
          scratchReg->setRegisterField((uint32_t *)cursor);
          cursor += 4;
          *(int16_t *)cursor = bos(addr & 0xFFFF);
          cursor += 2;
        ...
        if (getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL) {
          ...
          //
          *(int32_t *)cursor =
              boi(0xE3000000);  // LG  sourceReg, 0(,scratchReg)
          ...
          *(int16_t *)cursor = bos(0x0004);
          ...
          //
          *(int32_t *)cursor =
              boi(0xE3000000);  // LGF sourceReg, 0(,scratchReg)
          sourceReg->setRegisterField((uint32_t *)cursor);
          ...
          *(int16_t *)cursor = bos(0x0014);
          ...
          //
          *(int32_t *)cursor =
              boi(0xE3000000);  // LLGF sourceReg, 0(,scratchReg)
          sourceReg->setRegisterField((uint32_t *)cursor);
          scratchReg->setRegister2Field((uint32_t *)cursor);
          cursor += 4;
          ...
          *(int32_t *)cursor = boi(0x58000000);  // L sourceReg, 0(,scratchReg)
          ...
          //
          *(int32_t *)cursor =
              boi(0xE3000000);  // STG sourceReg, 0(,scratchReg)
          sourceReg->setRegisterField((uint32_t *)cursor);
          scratchReg->setRegister2Field((uint32_t *)cursor);
          cursor += 4;
          *(int16_t *)cursor = bos(0x0024);
        ...
        if (spillNeeded) {
          *(int32_t *)cursor = boi(
              0xE3005000 | (offsetToLongDispSlot &
                            0xFFF));  // LG scratchReg, offToLongDispSlot(,GPR5)
          scratchReg->setRegisterField((uint32_t *)cursor);
          cursor += 4;
          *(int16_t *)cursor = bos(0x0004);
          cursor += 2;
    ...
    if (getTargetSnippet() != NULL) {
      cg()->addRelocation(
          new (cg()->trHeapMemory()) TR::LabelRelative32BitRelocation(
              cursor, getTargetSnippet()->getSnippetLabel()));
    } else if (getTargetLabel() != NULL) {
      cg()->addRelocation(
          new (cg()->trHeapMemory())
              TR::LabelRelative32BitRelocation(cursor, getTargetLabel()));
    }
    ...
    if (std::find(comp->getStaticPICSites()->begin(),
      ...
      doRegisterPIC = true;
    } else if (std::find(comp->getStaticMethodPICSites()->begin(),
                         comp->getStaticMethodPICSites()->end(),
                         this) != comp->getStaticMethodPICSites()->end()) {
      unloadableClass =
          (TR_OpaqueClassBlock *)cg()
              ->fe()
              ->createResolvedMethod(cg()->trMemory(),
                                     (TR_OpaqueMethodBlock *)getTargetPtr(),
    ...
    if (doRegisterPIC &&
      ...
      cg()->jitAdd32BitPicToPatchOnClassUnload(
          (void *)unloadableClass, (void *)(uintptrj_t *)(cursor + 2));

      // register 32 bit patchable immediate part of a LARL instruction
    }

    TR::Symbol *sym = (getNode() && getNode()->getOpCode().hasSymbolReference())
                          ? getNode()->getSymbol()
                          : NULL;
    if (!(sym && sym->isStartPC()) && getTargetSymbol() &&
        getTargetSymbol()->isMethod()) {
      setImmediateOffsetInBytes(
          (uint8_t *)getSymbolReference()->getMethodAddress() - cursor);
    }

    if (sym && sym->isStartPC())
      setImmediateOffsetInBytes(
          (uint8_t *)sym->getStaticSymbol()->getStaticAddress() - cursor);
    ...
    if (getTargetLabel() != NULL) {
      ...
      if (getTargetSnippet() != NULL) {
        ...
        bool isHelper =
            getSymbolReference() &&
            getSymbolReference()->getSymbol()->isMethod() &&
        ...
        if (!isImmediateOffsetInBytes() &&
          ...
          TR_ASSERT((int32_t)(targetAddr) == (int32_t)(getTargetPtr()),
                    "LARL lower 32-bits is incorrect!\n");
          (*(int32_t *)(cursor + 2)) |= boi(i2);
          cursor += getOpCode().getInstructionLength();
          ...
          if ((targetAddr << 16) >> 48 != (getTargetPtr() << 16) >> 48) {
            ...
            toRealRegister(getRegisterOperand(1))
                ->setRegister1Field((uint32_t *)cursor);
            (*(int16_t *)(cursor + 2)) |=
                bos((int16_t)((getTargetPtr() << 16) >> 48));
            cursor += 4;
          }
          cursor -= getOpCode().getInstructionLength();
        } else {
        ...
        if (isHelper) {
          ...
          cg()->addProjectSpecializedRelocation(
              cursor + 2, (uint8_t *)getSymbolReference(), NULL,
    ...
    // If it is a branch to unresolvedData snippet, we need to align the
    // immediate field so that it can be patched atomically using ST
    if (getTargetSnippet() != NULL &&
        (getTargetSnippet()->getKind() == TR::Snippet::IsUnresolvedData)) {
    ...
    // When a method is recompiled the entry point of the old implementation
    // gets patched with a branch to the preprologue. The callee is then
    // responsible for patching the address of the callers BRASL to point to the
    // new implementation, and subsequently branch to the new location. Because
    // this patching happens at runtime we must ensure that the store to the
    // relative immediate offset in the BRASL is done atomically. On zOS we
    // allow atomic patching on both a 4 and an 8 byte boundary. On zLinux 64
    // bit we only allow atomic patching on a 4 byte boundary as we use
    // Multi-Code Caches and require trampolines.
    ...
    if (getTargetSnippet() != NULL) {
       ...
       * 1- Recursive call: Address is calculated by getCodeStart. MethodAddress
       * has not set yet 2- 'targetAddress' is provided: We use provided address
       * to generate the brasl address 3- 'targetAddress' is not provided: In
       * this case the address is extracted from TargetSymbol. If targetAddress
       * is not provided, TargetSymbol should be a method symbol.
       */
      TR_ASSERT(
          getTargetPtr() ||
              (getTargetSymbol() && getTargetSymbol()->isMethod()),
      ...
      if (comp->isRecursiveMethodTarget(getTargetSymbol())) {
        ...
        *(int32_t *)(cursor + 2) =
            boi(((intptrj_t)jitTojitStart - (intptrj_t)cursor) / 2);
      } else {
        TR::MethodSymbol *callSymbol = NULL;
        if (getTargetSymbol()) {
          callSymbol = getTargetSymbol()->isMethod()
                           ? getTargetSymbol()->castToMethodSymbol()
                           : NULL;
        }
        ...
        if (getTargetPtr() == 0 && callSymbol) {
          i2 = (int32_t)(((uintptrj_t)(callSymbol->getMethodAddress()) -
                          (uintptrj_t)cursor) /
                         2);
        }
#if defined(TR_TARGET_64BIT)
        ...
        (*(int32_t *)(cursor + 2)) = boi(i2);
        TR::Symbol *sym =
            getSymbolReference() ? getSymbolReference()->getSymbol() : NULL;
        TR::ResolvedMethodSymbol *resolvedMethodSym =
            sym ? sym->getResolvedMethodSymbol() : NULL;
        TR_ResolvedMethod *resolvedMethod =
            resolvedMethodSym ? resolvedMethodSym->getResolvedMethod() : NULL;
        if (sym && sym->castToMethodSymbol()->isHelper()) {
          ...
          cg()->addProjectSpecializedRelocation(
              cursor + 2, (uint8_t *)getSymbolReference()->getMethodAddress(),
              NULL, TR_MethodCallAddress, __FILE__, __LINE__, getNode());
        }
      }
    }
    cursor += getOpCode().getInstructionLength();
    ...
    TR_ASSERT(getOpCode().getInstructionLength() == 6,
              "Extended immediate instruction must be length of 6\n");
    (*(int32_t *)(cursor + 2)) |= boi((int32_t)getSourceImmediate());

    cursor += getOpCode().getInstructionLength();
  } else {
    TR_ASSERT(0, "OpCode:%d is not handled in RILInstruction yet",
              getOpCode().getOpCodeValue());
  }
 ...
 *
 *    2. targetReg => R1,      MemRef    => (B2,D2),                 MaskImm =>
 * M3    ... (ICM)
 *
 *    3. targetReg => R1,      MemRef    => (B2,D2), secondReg => R3 ...
 * (SLLG,...) targetReg => R1,                          , secondReg => R3
 * sourceImm => D2  ... (SLLG,...) targetReg => R1,      MemRef    => (B2,D2)
 * ... (SLL,...) targetReg => R1, sourceImm => D2  ... (SLL,...)
 */
int32_t TR::S390RSInstruction::estimateBinaryLength(int32_t currentEstimate) {
  if (getMemoryReference() != NULL) {
    return getMemoryReference()->estimateBinaryLength(currentEstimate, cg(),
                                                      this);
  } else {
...
uint8_t *TR::S390RSInstruction::generateBinaryEncoding() {
  ...
  if (op == TR::InstOpCode::CDS || op == TR::InstOpCode::CDSG ||
      op == TR::InstOpCode::MVCLE || op == TR::InstOpCode::MVCLU ||
      op == TR::InstOpCode::CLCLE || op == TR::InstOpCode::CLCLU) {
    toRealRegister(getSecondRegister()->getHighOrder())
        ->setRegister2Field((uint32_t *)cursor);
  ...
  if (getMemoryReference() != NULL) {
    longDispTouchUpPadding =
        getMemoryReference()->generateBinaryEncodingTouchUpForLongDisp(
            cursor, cg(), this);
  }
...
uint8_t *TR::S390RRSInstruction::generateBinaryEncoding() {
  ...
  // write the reg evaluation into the right-most nibble of a byte which starts
  // just after the bytecode.
  ...
  int32_t padding =
      getBranchDestinationLabel()->generateBinaryEncoding(cursor, cg(), this);
  cursor += padding;
...
uint8_t *TR::S390RIEInstruction::generateBinaryEncoding() {
  ...
  // add the register number for the first register to the buffer after the op
  // code.
  TR::Register *tgtReg = getRegForBinaryEncoding(getRegisterOperand(1));
  ...
  if (RIE1 || RIE5 || RIE4) {
    // add the register number for the second register to the buffer after the
    // first register.
    TR::Register *srcReg = getRegForBinaryEncoding(getRegisterOperand(2));
  ...
  if (getBranchDestinationLabel()) {
    ...
    if ((distance / 2) >= MIN_IMMEDIATE_VAL &&
        (distance / 2) <= MAX_IMMEDIATE_VAL) {
      if (doRelocation) {
        cg()->addRelocation(
            new (cg()->trHeapMemory())
                TR::LabelRelative16BitRelocation(cursor, label));
      } else {
      ...
      if (isRRE) {
        // position cursor forward so that the registers encoding lines up with
        // isRR
        cursor += 2;
      }

      // add the register number for the first register to the buffer after the
      // op code.
      TR::Register *tgtReg = getRegForBinaryEncoding(getRegisterOperand(1));
      toRealRegister(tgtReg)->setRegister1Field((uint32_t *)cursor);
      ...
      if (isRR || isRRE) {
        // add the register number for the second register to the buffer after
        // the first register.
        TR::Register *srcReg = getRegForBinaryEncoding(getRegisterOperand(2));
        toRealRegister(srcReg)->setRegister2Field((uint32_t *)cursor);
        cursor += 2;
      } else if (isRI) {
        // TR::InstOpCode::CHI/TR::InstOpCode::CGHI are signed so sign extend
        // the 8 bit immediate value to 16 bits
        (*(int16_t *)(cursor)) = bos((int16_t)getSourceImmediate8());
        cursor += 2;
      } else if (isRIL) {
        // TR::InstOpCode::CLGFI/TR::InstOpCode::CLFI are unsigned so zero
        // extend the 8 bit immediate value to 32 bits
      ...
      *(cursor + 1) |= (uint8_t)getMask();
      ...
      if (doRelocation) {
        cg()->addRelocation(
            new (cg()->trHeapMemory())
                TR::LabelRelative32BitRelocation(cursor, label));
      } else {
  ...
  // if we don't have a branch destination (and not type RIE5), we have a 16-bit
  // immediate value to compare
  else if (!RIE5) {
    *(int16_t *)(cursor) |= bos((int16_t)getSourceImmediate16());
    TR_ASSERT(
        ((getSourceImmediate16() & 0xB900) != 0xB900) ||
            ((getOpCodeValue() != 0xEC71) && (getOpCodeValue() != 0xEC73)),
        "CLFIT has Immediate value 0xB9XX, signalHandler might confuse DIVCHK "
        "with BNDCHK");
  }
  // if we are type RIE5 place the two immediate values in
  else {
    if (getOpCodeValue() == TR::InstOpCode::RISBLG ||
        getOpCodeValue() == TR::InstOpCode::RISBHG)
      // mask the I3 field as bits 1-3 must be 0
      *(int8_t *)(cursor) |= (int8_t)getSourceImmediate8One() & 0x1F;
    else if (getOpCodeValue() == TR::InstOpCode::RISBG ||
             getOpCodeValue() == TR::InstOpCode::RISBGN) {
      // mask the I3 field as bits 0-1 must be 0
      *(int8_t *)(cursor) |= (int8_t)getSourceImmediate8One() & 0x3F;
      TR_ASSERT(((int8_t)getSourceImmediate8One() & 0xC0) == 0,
                "Bits 0-1 in the I3 field for %s must be 0",
    ...
    if (getOpCodeValue() == TR::InstOpCode::RISBLG ||
      ...
      *(int8_t *)(cursor) |= (int8_t)getSourceImmediate8Two() & 0xBF;
      TR_ASSERT(((int8_t)getSourceImmediate8Two() & 0x40) == 0,
                "Bit 1 in the I4 field for %s must be 0",
                getOpCodeValue() == TR::InstOpCode::RISBG ? "RISBG" : "RISBGN");
    } else
...
uint8_t *TR::S390RIEInstruction::splitIntoCompareAndLongBranch(void) {
  ...
  if (isRRE || isRR)
    currInstr =
        generateRRInstruction(cg(), cmpOp, getNode(), getRegisterOperand(1),
                              getRegisterOperand(2), prevInstr);
  else if (isRI)
    currInstr =
        generateRIInstruction(cg(), cmpOp, getNode(), getRegisterOperand(1),
                              (int16_t)getSourceImmediate8(), prevInstr);
  else if (isRIL)
    currInstr =
        generateRILInstruction(cg(), cmpOp, getNode(), getRegisterOperand(1),
  ...
  currInstr = generateS390BranchInstruction(
      cg(), TR::InstOpCode::BRC, getBranchCondition(), getNode(),
  ...
  // set 'this' - the compare and branch instruction being removed, 'next'
  // pointer to the instruction after the inserted BRC/BRCL so the main cg
  // generateBinaryEncoding loop does not encode the new CHI and BRC/BRCL
 ...
 * branch is provided as an argument. The compare instruction is inserted right
 * after the combined compare and branch instruction.
 */
void TR::S390RIEInstruction::splitIntoCompareAndBranch(
    TR::Instruction *insertBranchAfterThis) {
  // Generate equivalent Compare instruction
  TR::InstOpCode::Mnemonic cmpOp;
  bool isRR = false;
  bool isRRE = false;
  ...
  switch (getOpCodeValue()) {
    ...
    default:
      TR_ASSERT(false,
                "Don't know how to split this compare and branch opcode %s\n",
  ...
  // generate a BRC
  generateS390BranchInstruction(cg(), TR::InstOpCode::BRC, getBranchCondition(),
                                getNode(), getBranchDestinationLabel(),
                                insertBranchAfterThis);
...
uint8_t *TR::S390RISInstruction::generateBinaryEncoding() {
  ...
  // add the register number for the first register to the buffer right after
  // the op code.
  toRealRegister(getRegisterOperand(1))->setRegister1Field((uint32_t *)cursor);
...
bool TR::S390RXInstruction::refsRegister(TR::Register *reg) {
  // 64bit mem refs clobber high word regs
  if (reg->getKind() != TR_FPR && reg->getKind() != TR_VRF &&
      getMemoryReference() && reg->getRealRegister()) {
    TR::RealRegister *realReg = (TR::RealRegister *)reg;
    TR::Register *regMem = reg;
  ...
  if (matchesTargetRegister(reg)) {
    return true;
  } else if ((getMemoryReference() != NULL) &&
...
uint8_t *TR::S390RXInstruction::generateBinaryEncoding() {
  ...
  if (getMemoryReference()->isLongDisplacementRequired()) {
    auto longDisplacementMnemonic =
        TR::InstOpCode::getEquivalentLongDisplacementMnemonic(getOpCodeValue());
...
uint8_t *TR::S390RXFInstruction::generateBinaryEncoding() {
  ...
  if (isTgtFPPair) {
    toRealRegister(srcRegPair->getHighOrder())
        ->setRegisterField((uint32_t *)cursor);
    toRealRegister(tgtRegPair->getHighOrder())
        ->setRegisterField((uint32_t *)cursor + 1, 7);
  } else {
...
const char *getOpCodeName(TR::InstOpCode *opCode) {
  return TR::InstOpCode::metadata[opCode->getOpCodeValue()].name;
}

TR::S390VInstruction::~S390VInstruction() {
  if (_opCodeBuffer) TR::Instruction::jitPersistentFree((char *)_opCodeBuffer);
}
...
char *TR::S390VInstruction::setOpCodeBuffer(char *c) {
  if (_opCodeBuffer)
    TR_ASSERT(false,
              "trying to set OpCodeBuffer after it has already been set!\n");
...
const char *TR::S390VRIInstruction::getExtendedMnemonicName() {
  if (getOpCodeBuffer()) return getOpCodeBuffer();
  ...
  if (getOpCode().isVectorFPOp()) {
    ...
    if (getM5() & 0x8) tmpOpCodeBuffer[0] = 'W';
    printM5 = false;
  } else
...
uint8_t *TR::S390VRIaInstruction::generateBinaryEncoding() {
  ...
  (*reinterpret_cast<uint16_t *>(cursor + 2)) |=
      bos(static_cast<uint16_t>(getImmediateField16()));

  // Operands
  toRealRegister(getRegisterOperand(1))
      ->setRegister1Field(reinterpret_cast<uint32_t *>(cursor));
...
uint8_t *TR::S390VRIcInstruction::generateBinaryEncoding() {
  ...
  toRealRegister(getRegisterOperand(1))
      ->setRegister1Field(reinterpret_cast<uint32_t *>(cursor));
  toRealRegister(getRegisterOperand(2))
      ->setRegister2Field(reinterpret_cast<uint32_t *>(cursor));
...
uint8_t *TR::S390VRIdInstruction::generateBinaryEncoding() {
  ...
  toRealRegister(getRegisterOperand(2))
      ->setRegister2Field(reinterpret_cast<uint32_t *>(cursor));
  toRealRegister(getRegisterOperand(3))
      ->setRegister3Field(reinterpret_cast<uint32_t *>(cursor));
...
uint8_t *TR::S390VRIfInstruction::generateBinaryEncoding() {
  ...
  (*reinterpret_cast<uint16_t *>(cursor + 3)) |=
      bos(static_cast<uint16_t>((getImmediateField16() & 0xff) << 4));
...
uint8_t *TR::S390VRIiInstruction::generateBinaryEncoding() {
  ...
  // The Immediate field
  (*reinterpret_cast<uint16_t *>(cursor + 3)) |=
      bos(static_cast<uint16_t>((getImmediateField8() & 0xff) << 4));
...
/** \details
 *
 * VRR instruction format get extended mnemonic name returns a charactor array
 * that contains the extended mnemonic names
 */
const char *TR::S390VRRInstruction::getExtendedMnemonicName() {
  ...
  if (getOpCode().isVectorFPOp()) {
    ...
    if (singleSelector & 0x8) tmpOpCodeBuffer[0] = 'W';
    ...
    if ((getOpCodeValue() == TR::InstOpCode::VUPL ||
         getOpCodeValue() == TR::InstOpCode::VMAL ||
  ...
  if (ccMask & 0x1) strcat(tmpOpCodeBuffer, "S");
...
/** \details
 *
 *  VRR Generate Binary Encoding for most sub-types of the VRR instruction
 * format
 *
...
uint8_t *TR::S390VRRInstruction::generateBinaryEncoding() {
  ...
  if (getRegisterOperand(4) != NULL) {
    ...
    toRealRegister(getRegisterOperand(4))
        ->setRegister4Field(reinterpret_cast<uint32_t *>(cursor));
  } else {
 ...
 * Performs error checking on the operands and then deleagte the encoding work
 * to its parent class
 */
...
uint8_t *TR::S390VRRgInstruction::generateBinaryEncoding() {
  ...
  toRealRegister(v1Reg)->setRegister2Field(
...
uint8_t *TR::S390VRRhInstruction::generateBinaryEncoding() {
  ...
  toRealRegister(v1Reg)->setRegister2Field(
      reinterpret_cast<uint32_t *>(cursor));
  toRealRegister(v2Reg)->setRegister3Field(
      reinterpret_cast<uint32_t *>(cursor));
...
uint8_t *TR::S390VRRiInstruction::generateBinaryEncoding() {
  ...
  // Operands
  toRealRegister(r1Reg)->setRegister1Field(
      reinterpret_cast<uint32_t *>(cursor));
  toRealRegister(v2Reg)->setRegister2Field(
      reinterpret_cast<uint32_t *>(cursor));
 ...
 *
 * Generate binary encoding for most vector register-storage formats: VRS, VRV,
 * and VRX VSI, VRV, and VRS-d have their own implementations and are not
 * handled here
 */
 ...
 */
int32_t TR::S390VStorageInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  if (getMemoryReference() != NULL) {
    return getMemoryReference()->estimateBinaryLength(currentEstimate, cg(),
                                                      this);
...
uint8_t *TR::S390VRSdInstruction::generateBinaryEncoding() {
  ...
  TR::Register *v1Reg =
      getOpCode().isStore() ? getRegisterOperand(2) : getRegisterOperand(1);
  TR::Register *r3Reg =
      getOpCode().isStore() ? getRegisterOperand(1) : getRegisterOperand(2);
  ...
  toRealRegister(r3Reg)->setRegister2Field(
      reinterpret_cast<uint32_t *>(cursor));
  toRealRegister(v1Reg)->setRegister4Field(
      reinterpret_cast<uint32_t *>(cursor));
  ...
  // Finish patching up if long disp was needed
  longDispTouchUpPadding =
      memRef->generateBinaryEncodingTouchUpForLongDisp(cursor, cg(), this);
...
////////////////////////////////////////////////////////////////////////////////
int32_t TR::S390MemMemInstruction::estimateBinaryLength(
    int32_t currentEstimate) {
  int32_t length = 0;
  if (getMemoryReference() != NULL) {
    return getMemoryReference()->estimateBinaryLength(currentEstimate, cg(),
...
uint8_t *TR::S390MemMemInstruction::generateBinaryEncoding() {
  ...
  if (getMemoryReference2())
    longDispTouchUpPadding +=
        getMemoryReference2()->generateBinaryEncodingTouchUpForLongDisp(
            cursor, cg(), this);
...
uint8_t *TR::S390SS1Instruction::generateBinaryEncoding() {
  ...
  // If we spilled the 2nd memref, than we need to ensure the base register
  // field and the displacement for the first memref is re-written to the now
  // changed cursor
  if (getLocalLocalSpillReg2()) {
    toRealRegister(getMemoryReference()->getBaseRegister())
        ->setBaseRegisterField((uint32_t *)cursor);
  ...
  TR_ASSERT(getLen() >= 0 && getLen() <= 255,
            "SS1 L must be in the range 0->255 and not %d on inst %p\n",
            getLen(), this);
...
uint8_t *TR::S390SS2Instruction::generateBinaryEncoding() {
  ...
  // generate binary for the length field
  TR_ASSERT(getLen() >= 0 && getLen() <= 15,
            "SS2 L1 must be in the range 0->15 and not %d from [%p]\n",
            getLen(), getNode());
  TR_ASSERT(getLen2() >= 0 && getLen2() <= 15,
            "SS2 L2 must be in the range 0->15 and not %d\n", getLen2());
  (*(uint32_t *)cursor) &= boi(0xFF00FFFF);
...
uint8_t *TR::S390SS4Instruction::generateBinaryEncoding() {
  ...
  if (getLengthReg())
    toRealRegister(getRegForBinaryEncoding(getLengthReg()))
        ->setRegisterField((uint32_t *)cursor, 5);
  if (getSourceKeyReg())
    toRealRegister(getRegForBinaryEncoding(getSourceKeyReg()))
        ->setRegisterField((uint32_t *)cursor, 4);
...
bool TR::S390SSFInstruction::refsRegister(TR::Register *reg) {
  ...
  if (matchesTargetRegister(reg)) {
    return true;
             ...
             (getMemoryReference()->refsRegister(reg) == true)) {
    return true;
  } else if ((getMemoryReference2() != NULL) &&
             (getMemoryReference2()->refsRegister(reg) == true)) {
    return true;
  } else if (getDependencyConditions()) {
    return getDependencyConditions()->refsRegister(reg);
  }
...
uint8_t *TR::S390RSLInstruction::generateBinaryEncoding() {
  ...
  // generate binary for the length field
  TR_ASSERT(getLen() <= 0xf,
            "length %d is too large to encode in RSL instruction\n", getLen());
  (*(uint32_t *)cursor) &= boi(0xFF0FFFFF);
...
uint8_t *TR::S390RSLbInstruction::generateBinaryEncoding() {
  ...
  // generate binary for the length field
  TR_ASSERT(getLen() <= UCHAR_MAX,
            "length %d is too large to encode in RSLb instruction\n", getLen());
  (*(uint32_t *)cursor) &= boi(0xFF00FFFF);
  (*(uint8_t *)(cursor + 1)) |= (getLen() & UCHAR_MAX);

  cursor += 2;

  TR_ASSERT(getRegisterOperand(1),
            "expecting a reg for RSLb instruction op %d\n", getOpCodeValue());
  ...
  TR_ASSERT(getMask() <= 0xf,
            "mask %d is too large to encode in RSLb instruction\n", getMask());
...
int32_t TR::S390MIIInstruction::estimateBinaryLength(int32_t currentEstimate) {
  // TODO:for now use instruction length (for case 0(R14), later need to add
  // support for longer disposition potentially reuse
  // Memoryreference->estimateBinaryLength
...
uint8_t *TR::S390MIIInstruction::generateBinaryEncoding() {
  ...
  int distance = (cg()->getBinaryBufferStart() +
                  getLabelSymbol()->getEstimatedCodeLocation()) -
                 (cursor + cg()->getAccumulatedInstructionLengthError());

  static bool bpp = (feGetEnv("TR_BPPNOBINARY") != NULL);
  intptrj_t offset =
      (intptrj_t)getSymRef()->getMethodAddress() - (intptrj_t)instructionStart;

  if (distance >= MIN_12_RELOCATION_VAL && distance <= MAX_12_RELOCATION_VAL &&
      !bpp && offset >= MIN_24_RELOCATION_VAL &&
      offset <= MAX_24_RELOCATION_VAL) {
    // clear the number of bytes we intend to use in the buffer.
    memset((void *)cursor, 0, getEstimatedBinaryLength());
    ...
    cg()->addRelocation(
        new (cg()->trHeapMemory())
            TR::LabelRelative12BitRelocation(cursor, getLabelSymbol(), false));

    // add the comparison mask for the instruction to the buffer.
    *(int16_t *)cursor |= ((uint16_t)getMask() << 12);

    // advance the cursor past the mask encoding and  part of RI2 (relocation)
    // (cursor at 16th bit)
    cursor += 1;
    ...
    intptrj_t offset = (intptrj_t)getSymRef()->getMethodAddress() -
                       (intptrj_t)instructionStart;
...
uint8_t *TR::S390SMIInstruction::generateBinaryEncoding() {
  ...
  if (distance >= MIN_RELOCATION_VAL && distance <= MAX_RELOCATION_VAL &&
      !bpp) {
    // clear the number of bytes we intend to use in the buffer.
    memset((void *)cursor, 0, getEstimatedBinaryLength());
    ...
    *(cursor) |= ((uint8_t)getMask() << 4);
    ...
    // add the memory reference for the target to the buffer, and advance the
    // cursor past what gets written.
    // TODO: make sure MemoryReference()->generateBinaryEncoding handles it
    // correctly
    int32_t padding = getMemoryReference()->generateBinaryEncoding(
        instructionStart, cg(), this);
    // TODO: fix for when displacement is too big and need an extra instruction
    // cursor += padding;
    cursor += 2;
    // add relocation
...
#ifdef J9_PROJECT_SPECIFIC
/**
 * VirtualGuardNOPInstruction is a pseudo instruction that generates a BRC or
 * BRCL This branch will be to a label.  It is assumed it is within range of a
 * BRC on a G5-- previous checking will ensure this.  If on a freeway, we are
 * free to choose either BRC or BRCL depending on the range.
 */
uint8_t *TR::S390VirtualGuardNOPInstruction::generateBinaryEncoding() {
  ...
  // a previous guard is patching to the same destination and we can recycle the
  // patch point so setup the patching location to use this previous guard and
  // generate no instructions ourselves
  if (guardForPatching != this) {
    ...
    if (label->getCodeLocation() == NULL) {
      cg()->addRelocation(
          new (cg()->trHeapMemory()) TR::LabelAbsoluteRelocation(
  ...
  if (label->getCodeLocation() != NULL) {
    // Label location is known--doubt this will ever occur, but for
    // completeness.... calculate the relative branch distance
    distance = (label->getCodeLocation() - cursor);
    doRelocation = false;
    _site->setDestination(label->getCodeLocation());
#ifdef DEBUG
    ...
    // bool brcRangeExceeded = (distance<MIN_IMMEDIATE_VAL ||
    // distance>MAX_IMMEDIATE_VAL);
  ...
  // We can only do the empty patch (i.e. no instrs emitted, and a full taken
  // patch is written in) if the patching occurs during GC pause times.  The
  // patching of up to 6-bytes is potentially not atomic.

  bool performEmptyPatch =
      getNode()->isStopTheWorldGuard() || getNode()->isProfiledGuard();

  // Stop the world guards that are merged with profiled guards never need to
  // generate NOPs for patching because the profiled guard will generate the NOP
  // branch to the same location the stop the world guard needs to branch to, so
  // we can always use the NOP branch form the profiled guard as our stop the
  // world patch point.

  int32_t sumEstimatedBinaryLength = getNode()->isProfiledGuard() ? 6 : 0;

  if (performEmptyPatch) {
    // so at this point we know we are an stop the world guard and that there
    // may be other stop the world guards after us that will use us as their
    // patch point. We now calculate how much space we are sure to have to
    // overwrite to inform us about what to do next
    TR::Node *firstBBEnd = NULL;
    for (TR::Instruction *nextI = getNext(); nextI != NULL;
         nextI = nextI->getNext()) {
      if (nextI->isVirtualGuardNOPInstruction()) {
        // if we encounter a guard that will not use us as a patch point we need
        // to stop counting and get out
        if (cg()->getVirtualGuardForPatching(nextI) != this) break;
        // otherwise they are going to use us as patch point so skip them and
        // keep going
        continue;
      }

      if (comp->isPICSite(nextI)) break;

      // the following instructions can be patched at runtime to set
      // destinations etc and so we cannot include them in the region for an HCR
      // guard to overwrite because the runtime patching could take place after
      // the HCR guard has been patched
      // TODO: BRCL and BRASL are only patchable under specific conditions -
      // this check can be made more specific to let some cases through
      ...
      // we shouldn't need to check for PSEUDO instructions, but VGNOP has a
      // non-zero length so skip PSEUDO instructions from length consideration
      // because in practice they will generally be zero
      if (nextI->getOpCode().getInstructionFormat() != PSEUDO)
        sumEstimatedBinaryLength += nextI->getOpCode().getInstructionLength();

      // we never need more than 6 bytes so save time and get out when we have
      // found what we want
      if (sumEstimatedBinaryLength >= 6) break;
  ...
  // If we scanned ahead and see that the estimated binary length is zero (i.e.
  // return point of slow path is right after the guard - methodEnter/Exit is an
  // example of such) we'll just generate the NOP branch.
  if (performEmptyPatch && (sumEstimatedBinaryLength > 0) &&
      performTransformation(
          comp,
          "O^O CODE GENERATION: Generating empty patch for guard node %p\n",
          getNode())) {
    doRelocation = false;
    ...
    // if distance is within 32K limit, generate short instruction
    binOpCode = *(uint16_t *)(TR::InstOpCode::getOpCodeBinaryRepresentation(
        TR::InstOpCode::BRC));
...
TR::MemoryReference *getFirstReadWriteMemoryReference(TR::Instruction *i) {
  ...
  switch (i->getKind()) {
    ...
    default:
      TR_ASSERT(0,
                "This type of instruction needs to be told how to get memory "
                "reference if any. opcode=%s kind=%d\n",
                i->getOpCode().getMnemonicName(), i->getKind());
      break;
  }
  return mr;
}
