...
void
TR_S390BinaryAnalyser::remapInputs(TR::Node* firstChild,
                                       ...
                                       TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    if (!done) {
      ...
      if (getBinaryReg3Reg2() || (secondRegister != NULL)) {
        generateRRInstruction(
          cg(), regToRegOpCode, root, thirdReg, secondRegister);
      } else {
        ...
        if (instructionFormat == RXE_FORMAT) {
          generateRXEInstruction(
            cg(), memToRegOpCode, root, thirdReg, tempMR, 0);
        } else {
  ...
  } else // assert getBinaryReg1Mem2() == true
  {
    ...
    if (instructionFormat == RXE_FORMAT) {
      generateRXEInstruction(
        cg(), memToRegOpCode, root, firstRegister, tempMR, 0);
    } else {
...
TR_S390BinaryAnalyser::longSubtractAnalyser(TR::Node* root)
{
  ...
  if (getCopyReg1()) {
    ...
    root->setRegister(thirdReg);
    generateRRInstruction(
      cg(), TR::InstOpCode::LGR, root, thirdReg, firstRegister);
