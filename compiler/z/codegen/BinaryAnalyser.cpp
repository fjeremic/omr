...
void TR_S390BinaryAnalyser::remapInputs(TR::Node *firstChild,
...
void TR_S390BinaryAnalyser::genericAnalyser(
  ...
  setInputs(firstChild, firstRegister, secondChild, secondRegister, false,
            false, comp, false, false);

  /*
   * Check if SH or CH can be used to evaluate this integer subtract/compare
   * node. The second operand of SH/CH is a 16-bit number from memory. And using
  ...
  if (getCopyReg1()) {
    ...
    default: {
      TR_ASSERT_FATAL(
          false,
          "Generic binary analyser for register kind (%d) is unimplemented",
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      if (getBinaryReg3Reg2() || secondRegister != NULL) {
        if (regToRegOpCode == TR::InstOpCode::SR) {
          generateRRRInstruction(cg(), TR::InstOpCode::SRK, root, thirdReg,
                                 firstRegister, secondRegister);
          done = true;
        } else if (regToRegOpCode == TR::InstOpCode::SLR) {
          generateRRRInstruction(cg(), TR::InstOpCode::SLRK, root, thirdReg,
                                 firstRegister, secondRegister);
          done = true;
        } else if (regToRegOpCode == TR::InstOpCode::SGR) {
          generateRRRInstruction(cg(), TR::InstOpCode::SGRK, root, thirdReg,
                                 firstRegister, secondRegister);
          done = true;
        } else if (regToRegOpCode == TR::InstOpCode::SLGR) {
          generateRRRInstruction(cg(), TR::InstOpCode::SLGRK, root, thirdReg,
                                 firstRegister, secondRegister);
          done = true;
        }
      }
    ...
    if (!done) {
      ...
      if (getBinaryReg3Reg2() || (secondRegister != NULL)) {
        generateRRInstruction(cg(), regToRegOpCode, root, thirdReg,
                              secondRegister);
      } else {
        TR::Node *loadBaseAddr =
            is16BitMemory2Operand ? secondChild->getFirstChild() : secondChild;
        TR::MemoryReference *tempMR =
            generateS390MemoryReference(loadBaseAddr, cg());

        // floating-point arithmatics don't have RXY format instructions, so no
        // long displacement
        ...
        if (instructionFormat == RXE_FORMAT) {
          generateRXEInstruction(cg(), memToRegOpCode, root, thirdReg, tempMR,
                                 0);
        } else {
  ...
  } else // assert getBinaryReg1Mem2() == true
  {
    ...
    if (instructionFormat == RXE_FORMAT) {
      generateRXEInstruction(cg(), memToRegOpCode, root, firstRegister, tempMR,
                             0);
    } else {
...
void TR_S390BinaryAnalyser::longSubtractAnalyser(TR::Node *root) {
  ...
  if (getCopyReg1()) {
    ...
    root->setRegister(thirdReg);
    generateRRInstruction(cg(), TR::InstOpCode::LGR, root, thirdReg,
                          firstRegister);
    ...
    {
      TR::MemoryReference *longMR =
          generateS390MemoryReference(secondChild, cg());
  ...
  } else // assert getBinaryReg1Mem2() == true
  {
    ...
    TR::Node *baseAddrNode =
        is16BitMemory2Operand ? secondChild->getFirstChild() : secondChild;
    TR::MemoryReference *longMR =
        generateS390MemoryReference(baseAddrNode, cg());
