...
inline TR::Register* convertToFixed(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (isUnsignedConversion) {
    if (floatToIntLogicalConverted) {
       ...
       * 2^31. If value < 2^31, MSB will be 1 due to borrow and lower bits will NOT be touched. The XOR MSB test will
       * put the sign back if original exceeded max integral value. xor will take care of -inf<a<min case (result is min
       * signed value as above) by unsetting MSB, which gives 0 as required by XLC lang ref.
       */
      ...
      // 1) fp round to 0
      //   todo: the only reason this is placed here is because we can specify rounding mode explicitly to "round to
      //   zero".
      //         convertOp for hex doesn't allow setting rounding mode mask, but IEEE does, so we may not need this here
      //         for IEEE
...
inline TR::Register* convertFromFixed(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (isUint32Src) {
    ...
    case TR::Double:
      ...
      deps->addPostCondition(targetRegister, TR::RealRegister::AssignAny);
      generateS390ImmOp(
        cg, TR::InstOpCode::ADB, node, targetRegister, targetRegister, (int64_t)CONSTANT64(0x41f0000000000000), deps);
      break;
    default:
...
TR::Register* OMR::Z::TreeEvaluator::floatRemHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->getDataType() == TR::Float) {
    ...
    TR::SymbolReference* helperCallSymRef
      = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitMathHelperFREM, false, false, false);
    ...
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, labelOK); // it is not < +0
    TR::SymbolReference* helperCallSymRef
      = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitMathHelperDREM, false, false, false);
  ...
  // Artificially setting reference count of callNode to 1 and decreasing it makes sure that this register is no longer
  // live in the method.
