...
inline void
genLogicalConversionForInt(TR::Node* node,
                           TR::CodeGenerator* cg,
                           TR::Register* targetRegister,
                           int8_t shift_amount)
{
...
inline void
genArithmeticConversionForInt(TR::Node* node,
                              TR::CodeGenerator* cg,
                              TR::Register* targetRegister,
                              int8_t shift_amount)
{
...
static void
generateValueConversionToIntType(TR::CodeGenerator* cg,
                                 TR::Node* node,
                                 TR::Register* targetRegister)
{
  int32_t nshift = 0;
 ...
 */
TR::Register*
generateExtendedFloatConstantReg(TR::Node* node,
                                 TR::CodeGenerator* cg,
                                 int64_t constHi,
                                 int64_t constLo,
                                 TR::RegisterDependencyConditions** depsPtr)
{
  ...
  generateRXInstruction(
    cg, TR::InstOpCode::LD, node, trgReg->getHighOrder(), mrHi);
  generateRXInstruction(
    cg, TR::InstOpCode::LD, node, trgReg->getLowOrder(), mrLo);
...
convertToFixed(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (isUnsignedConversion) {
    if (floatToIntLogicalConverted) {
      ...
      switch (firstChild->getDataType()) {
        ...
        case TR::Double:
          generateS390ImmOp(
            cg, subFPOp, node, srcRegCpy, srcRegCpy, MSBIntConstInFP);
          break;
        default:
      ...
      generateRRFInstruction(
        cg, convertOp, node, targetRegister, srcRegCpy, (int8_t)0x5, true);

      // 4) Handle the sign bit by XOR
      generateRILInstruction(
        cg, TR::InstOpCode::XILF, node, targetRegister, 0x80000000);
  ...
  } else // Signed Target Path
  {
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::XGR, node, targetRegister, targetRegister);
    ...
    // 3) Convert to Fixed
    generateRRFInstruction(
      cg, convertOp, node, targetRegister, srcRegister, (int8_t)0x5, true);
...
convertFromFixed(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (isUint32Src) {
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::LTR, node, srcRegister, srcRegister);
    ...
    switch (node->getDataType()) {
      case TR::Float: // fall-through
        generateRRInstruction(
          cg, TR::InstOpCode::LDEBR, node, targetRegister, targetRegister);
      case TR::Double:
        ...
        generateS390ImmOp(cg,
                          ...
                          targetRegister,
                          targetRegister,
                          (int64_t)CONSTANT64(0x41f0000000000000),
                          deps);
        break;
      default:
    ...
    if (isFloatTgt)
      generateRRInstruction(
        cg, TR::InstOpCode::LEDBR, node, targetRegister, targetRegister);
    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, skipAddLabel, deps);
...
OMR::Z::TreeEvaluator::faddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if ((canUseNodeForFusedMultiply(node->getFirstChild()) ||
    ...
    temp.floatBinaryCommutativeAnalyser(
      node, TR::InstOpCode::AEBR, TR::InstOpCode::AEB);
...
OMR::Z::TreeEvaluator::daddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if ((canUseNodeForFusedMultiply(node->getFirstChild()) ||
    ...
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.doubleBinaryCommutativeAnalyser(
      node, TR::InstOpCode::ADBR, TR::InstOpCode::ADB);
    cg->decReferenceCount(node->getFirstChild());
    cg->decReferenceCount(node->getSecondChild());
  }
...
OMR::Z::TreeEvaluator::fmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_S390BinaryCommutativeAnalyser temp(cg);
  temp.floatBinaryCommutativeAnalyser(
    node, TR::InstOpCode::MEEBR, TR::InstOpCode::MEEB);
      ...
      traceMsg(
        ...
        "Successfully changed fsub to fused multiply and sub operation\n");
             ...
             generateFusedMultiplyAddIfPossible(
               cg, node, TR::InstOpCode::MAEBR, TR::InstOpCode::LCEBR)) {
    if (comp->getOption(TR_TraceCG))
      traceMsg(comp,
...
OMR::Z::TreeEvaluator::dsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (canUseNodeForFusedMultiply(node->getFirstChild()) &&
    ...
    if (comp->getOption(TR_TraceCG))
      traceMsg(
        comp,
        "Successfully changed dsub to fused multiply and subtract operation\n");
  } else if (canUseNodeForFusedMultiply(node->getSecondChild()) &&
             generateFusedMultiplyAddIfPossible(
               cg, node, TR::InstOpCode::MADBR, TR::InstOpCode::LCDBR)) {
    if (comp->getOption(TR_TraceCG))
      traceMsg(comp,
...
OMR::Z::TreeEvaluator::floatRemHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->getDataType() == TR::Float) {
    generateRRInstruction(
      cg, TR::InstOpCode::LER, node, targetRegister, firstRegister);
    generateRRFInstruction(cg,
                           TR::InstOpCode::DIEBR,
                           node,
                           targetRegister,
    ...
    TR::SymbolReference* helperCallSymRef =
      cg->symRefTab()->findOrCreateRuntimeHelper(
        TR_S390jitMathHelperFREM, false, false, false);
    callNode = TR::Node::createWithSymRef(node, TR::fcall, 2, helperCallSymRef);
  } else {
    generateRRInstruction(
      cg, TR::InstOpCode::LDR, node, targetRegister, firstRegister);
    generateRRFInstruction(cg,
                           TR::InstOpCode::DIDBR,
                           node,
                           targetRegister,
    ...
    TR::SymbolReference* helperCallSymRef =
      cg->symRefTab()->findOrCreateRuntimeHelper(
        TR_S390jitMathHelperDREM, false, false, false);
  ...
  generateS390BranchInstruction(
    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, labelOK);
  outlinedSlowPath->swapInstructionListsWithCompilation();
  ...
  generateS390LabelInstruction(
    cg, TR::InstOpCode::LABEL, node, labelOK, postDeps);
...
FPtoIntBitsTypeCoercionHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->getFirstChild()->isSingleRefUnevaluated() &&
    ...
    if (nodeType == TR::Int64)
      targetReg = genericLoadHelper<64, 64, MemReg>(
        node, cg, tempmemref, NULL, false, true);
    else
      targetReg = genericLoadHelper<32, 32, MemReg>(
        node, cg, tempmemref, NULL, false, true);
  } else {
    ...
    if (nodeType == TR::Int64)
      targetReg =
        genericLoadHelper<64, 64, MemReg>(node, cg, tempMR2, NULL, false, true);
    else
      targetReg =
        genericLoadHelper<32, 32, MemReg>(node, cg, tempMR2, NULL, false, true);
  }
  ...
  if (node->normalizeNanValues()) {
    ...
    TR::MemoryReference* nanMemRef =
      nodeType == TR::Int64
        ? generateS390MemoryReference(
            static_cast<int64_t>(0x7ff8000000000000), nodeType, cg, litBase)
        : generateS390MemoryReference(
            static_cast<int32_t>(0x7fc00000), nodeType, cg, litBase);
    ...
    generateRXEInstruction(cg,
                           ...
                           node,
                           targetFPR,
                           generateS390MemoryReference(0x00F, cg),
                           0);
    ...
    if (nodeType == TR::Int64)
      targetReg = genericLoadHelper<64, 64, MemReg>(
        node, cg, nanMemRef, NULL, false, true);
    else
      targetReg = genericLoadHelper<32, 32, MemReg>(
        node, cg, nanMemRef, NULL, false, true);

    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, cleansedNumber, deps);
...
OMR::Z::TreeEvaluator::ibits2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.cpu.getSupportsFloatingPointExtensionFacility() &&
    ...
    generateRSInstruction(
      cg, TR::InstOpCode::SLLG, node, tempreg, sourceReg, 32);
...
l2dHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateRRInstruction(
    cg, TR::InstOpCode::CDGBR, node, targetFloatRegister, longRegister);
  ...
  if (node->getOpCodeValue() == TR::lu2d) {
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::LTGR, node, longRegister, longRegister);
    // If positive or zero, done BNL
    ...
    generateS390ImmOp(cg,
                      ...
                      targetFloatRegister,
                      targetFloatRegister,
                      (int64_t)CONSTANT64(0x43f0000000000000),
                      deps);
    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, skipAddLabel, deps);
    skipAddLabel->setEndInternalControlFlow();
...
l2fHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (IntToFloatLogicalConverted)
    ...
    generateRRInstruction(
      cg, convertOpCode, node, targetFloatRegister, longRegister);
...
f2lHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateRRInstruction(
    cg, TR::InstOpCode::CEBR, node, floatRegister, floatRegister);
  // if NaN, then done
  ...
  cFlowRegionStart->setStartInternalControlFlow();
  generateS390BranchInstruction(
    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BO, node, cFlowRegionEnd);
  dependencies =
...
d2lHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (checkNaN) {
    ...
    cFlowRegionStart->setStartInternalControlFlow();
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNZ, node, label1);
    dependencies =
      new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 2, cg);
    dependencies->addPostCondition(targetRegister, TR::RealRegister::AssignAny);
    dependencies->addPostCondition(floatRegister, TR::RealRegister::AssignAny);
  ...
  if (checkNaN) { // done label
    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, label1, dependencies);
...
{
  return TR::TreeEvaluator::floorCeilEvaluator(
    node, cg, TR::InstOpCode::FIDBR, (int8_t)0x6);
}
...
{
  return TR::TreeEvaluator::floorCeilEvaluator(
    node, cg, TR::InstOpCode::FIDBR, (int8_t)0x7);
}
...
{
  return TR::TreeEvaluator::floorCeilEvaluator(
    node, cg, TR::InstOpCode::FIEBR, (int8_t)0x6);
}
...
{
  return TR::TreeEvaluator::floorCeilEvaluator(
    node, cg, TR::InstOpCode::FIEBR, (int8_t)0x7);
}

TR::Register*
OMR::Z::TreeEvaluator::floorCeilEvaluator(
  TR::Node* node,
  TR::CodeGenerator* cg,
  TR::InstOpCode::Mnemonic regToRegOpCode,
  int8_t mask)
{
  ...
  generateRRFInstruction(
    cg, regToRegOpCode, node, targetRegister, srcRegister, mask, true);
