 ...
 * this happens during doInstructionSelection when we don't have stack frame, so we need to update the memory reference
...
public:
  ...
  MemoryReference(
    TR::Node* node, TR::SymbolReference* symRef, TR::CodeGenerator* cg, TR_StorageReference* storageReference = NULL);
  ...
  TR::Node* setBaseNode(TR::Node* bn)
  {
    return _baseNode = bn;
  }

  TR::Register* getIndexRegister()
  {
  ...
  TR::Node* getIndexNode()
  {
    return _indexNode;
  }
  TR::Node* setIndexNode(TR::Node* in)
  {
    return _indexNode = in;
  }
   ...
   */
  void setOffset(intptrj_t amount)
  {
    _offset = amount;
  }

  void addToOffset(intptrj_t amount)
  {
    _offset += amount;
  }
  ...
  TR_StorageReference* getStorageReference()
  {
   ...
   * underlying loadaddr symRef. This loadaddr symRef does not reflect the actual size of the symbol being loaded so the
  ...
  int32_t getFixedSizeForAlignment()
  {
    return _fixedSizeForAlignment;
  }

  int32_t getLeftMostByte()
  {
    return _leftMostByte;
  }
  int32_t setLeftMostByte(int32_t byte)
  {
    return _leftMostByte = byte;
  }

  bool getMemRefUsedBefore()
  {
    return _flags.testAny(MemRefUsedBefore);
  }
  void setMemRefUsedBefore()
  {
    _flags.set(MemRefUsedBefore);
  }
  void resetMemRefUsedBefore()
  {
    _flags.reset(MemRefUsedBefore);
  }

  bool getDispAdjusted()
  {
    return _flags.testAny(DisplacementAdjusted);
  }
  void setDispAdjusted()
  {
    _flags.set(DisplacementAdjusted);
  }

  bool getPseudoLive()
  {
    return _flags.testAny(PseudoLive);
  }
  void setPseudoLive()
  {
    _flags.set(PseudoLive);
  }
  ...
  void setCheckForLongDispSlot()
  {
    _flags.set(CheckForLongDispSlot);
  }
  ...
  bool is2ndMemRef()
  {
    return _flags.testAny(TR_S390MemRef_Is2ndMemRef);
  }
  void setIs2ndMemRef()
  {
    _flags.set(TR_S390MemRef_Is2ndMemRef);
  }
  void resetIs2ndMemRef()
  {
    _flags.reset(TR_S390MemRef_Is2ndMemRef);
  }
  ...
  bool forceFolding()
  {
    return _flags.testAny(TR_S390MemRef_ForceFolding);
  }
  ...
  void setConstantDataSnippet()
  {
    _flags.set(MemRef_ConstantDataSnippet);
  }

  bool isMemRefMustNotSpill()
  {
    return _flags.testAny(MemRefMustNotSpill);
  }
  void setMemRefMustNotSpill()
  {
    _flags.set(MemRefMustNotSpill);
  }

  bool ZeroBasePtr_EvaluateSubtree(TR::Node* subTree, TR::CodeGenerator* cg, MemoryReference* mr);

  void setSymbolReference(TR::SymbolReference* ref)
  {
