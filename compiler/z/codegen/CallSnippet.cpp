...
uint8_t *TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::Mnemonic op,
                                                uint8_t *buffer,
                                                TR::RealRegister *reg,
                                                int32_t offset,
                                                TR::CodeGenerator *cg) {
  ...
  TR_ASSERT((offset & 0xfffff000) == 0,
            "CallSnippet: Unexpected displacement %d for storeArgumentItem",
            offset);
  *toS390Cursor(buffer) |= offset & 0x00000fff;
  return buffer + opCode.getInstructionLength();
}

uint8_t *TR::S390CallSnippet::S390flushArgumentsToStack(uint8_t *buffer,
                                                        TR::Node *callNode,
                                                        int32_t argSize,
                                                        TR::CodeGenerator *cg) {
  ...
  TR::Linkage *linkage = cg->getLinkage(
      callNode->getSymbol()->castToMethodSymbol()->getLinkageConvention());
    ...
    switch (child->getDataType()) {
      ...
      case TR::Int32:
        ...
        if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
          buffer = storeArgumentItem(
              TR::InstOpCode::ST, buffer,
              machine->getRealRegister(
                  linkage->getIntegerArgumentRegister(intArgNum)),
              offset, cg);
        }
        intArgNum++;
      ...
      case TR::Address:
        ...
        if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
          buffer = storeArgumentItem(
              TR::InstOpCode::getStoreOpCode(), buffer,
              machine->getRealRegister(
                  linkage->getIntegerArgumentRegister(intArgNum)),
              offset, cg);
        }
          ...
          if (TR::Compiler->target.is64Bit()) {
            ...
            if (intArgNum < linkage->getNumIntegerArgumentRegisters() - 1) {
              buffer = storeArgumentItem(
                  TR::InstOpCode::ST, buffer,
                  machine->getRealRegister(
                      linkage->getIntegerArgumentRegister(intArgNum + 1)),
                  offset + 4, cg);
            }
          }
        }
      ...
      case TR::Float:
        ...
        if (floatArgNum < linkage->getNumFloatArgumentRegisters()) {
          buffer = storeArgumentItem(
              TR::InstOpCode::STE, buffer,
              machine->getRealRegister(
                  linkage->getFloatArgumentRegister(floatArgNum)),
              offset, cg);
        }
        floatArgNum++;
        if (rightToLeft) {
 ...
 */
int32_t TR::S390CallSnippet::instructionCountForArguments(
    TR::Node *callNode, TR::CodeGenerator *cg) {
  ...
  for (int32_t i = argStart; i < callNode->getNumChildren(); i++) {
    ...
    switch (child->getDataType()) {
      ...
      case TR::Address:
        if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
          count += TR::InstOpCode::getInstructionLength(
      ...
      case TR::Int64:
        if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
          ...
          if ((TR::Compiler->target.is32Bit()) &&
              intArgNum < linkage->getNumIntegerArgumentRegisters() - 1) {
            count += TR::InstOpCode::getInstructionLength(
                TR::InstOpCode::getLoadOpCode());
          }
        }
...
uint8_t *TR::S390CallSnippet::getCallRA() {
  ...
  TR_ASSERT(getBranchInstruction() != NULL,
            "CallSnippet: branchInstruction is NULL");
  return (uint8_t *)(getBranchInstruction()->getNext()->getBinaryEncoding());
}

TR_RuntimeHelper TR::S390CallSnippet::getHelper(TR::MethodSymbol *methodSymbol,
                                                TR::DataType type,
                                                TR::CodeGenerator *cg) {
...
TR_RuntimeHelper TR::S390CallSnippet::getInterpretedDispatchHelper(
    TR::SymbolReference *methodSymRef, TR::DataType type) {
...
uint32_t TR::S390CallSnippet::getLength(int32_t estimatedSnippetStart) {
  // *this   swipeable for debugger
  // length = instructionCountForArgsInBytes + (BASR + L(or LG) + BASR
  // +3*sizeof(uintptrj_t)) + NOPs number of pad bytes has not been set when
  // this method is called to estimate codebuffer size, so -- i'll put an
  // conservative number here...
  return (instructionCountForArguments(getNode(), cg()) +
          getPICBinaryLength(cg()) + 3 * sizeof(uintptrj_t) +
...
void TR_Debug::print(TR::FILE *pOutFile, TR::S390CallSnippet *snippet) {
  ...
  if (!methodSymRef) methodSymRef = callNode->getSymbolReference();
  ...
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    methodSymRef->isUnresolved() ? "Unresolved Call Snippet"
                                                 : "Call Snippet");

  bufferPos = printS390ArgumentsFlush(pOutFile, callNode, bufferPos,
                                      snippet->getSizeOfArguments());

  if (methodSymRef->isUnresolved() ||
      (_comp->compileRelocatableCode() &&
       !_comp->getOption(TR_UseSymbolValidationManager))) {
    if (methodSymbol->isSpecial()) {
    ...
    if ((methodSymbol->isVMInternalNative() ||
      ...
      switch (callNode->getDataType()) {
        ...
        default:
          TR_ASSERT(0,
                    "Bad return data type for a call node.  DataType was %s\n",
                    getName(callNode->getDataType()));
      }
    }
  }
  bufferPos = printRuntimeInstrumentationOnOffInstruction(pOutFile, bufferPos,
                                                          false);  // RIOFF
  ...
  if (padbytes == 2) {
    ...
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(pOutFile,
              "DC   \t0x00000000 \t\t# 4-bytes padding for alignment");
    bufferPos += 4;
  } else if (padbytes == 6) {
    printPrefix(pOutFile, NULL, bufferPos, 6);
    trfprintf(pOutFile,
              "DC   \t0x000000000000 \t\t# 6-bytes padding for alignment");
    bufferPos += 6;
  }

  printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# Method Address",
            glueRef->getMethodAddress());
  ...
  trfprintf(
      pOutFile, "DC   \t%p \t\t# Method Pointer",
      methodSymRef->isUnresolved() ? 0 : methodSymbol->getMethodAddress());
}
