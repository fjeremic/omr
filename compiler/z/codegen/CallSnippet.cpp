...
TR_RuntimeHelper TR::S390CallSnippet::getHelper(
  TR::MethodSymbol* methodSymbol, TR::DataType type, TR::CodeGenerator* cg)
{
...
TR_RuntimeHelper TR::S390CallSnippet::getInterpretedDispatchHelper(TR::SymbolReference* methodSymRef, TR::DataType type)
{
  ...
  if (methodSymRef->isUnresolved()
    ...
    TR_ASSERT(
      !isJitInduceOSRCall || !comp->compileRelocatableCode(), "calling jitInduceOSR is not supported yet under AOT\n");
...
uint32_t TR::S390CallSnippet::getLength(int32_t estimatedSnippetStart)
{
  ...
  return (instructionCountForArguments(getNode(), cg()) + getPICBinaryLength(cg()) + 3 * sizeof(uintptrj_t)
    + getRuntimeInstrumentationOnOffInstructionLength(cg()) + sizeof(uintptrj_t)); // the last item is for padding
}
...
void TR_Debug::print(TR::FILE* pOutFile, TR::S390CallSnippet* snippet)
{
  ...
  if (methodSymRef->isUnresolved()
    || (_comp->compileRelocatableCode() && !_comp->getOption(TR_UseSymbolValidationManager))) {
    if (methodSymbol->isSpecial()) {
  ...
  if (snippet->getKind() == TR::Snippet::IsUnresolvedCall) {
    ...
    trfprintf(pOutFile, "LARL \tGPR14, *+%d <%p>\t# Start of Data Const.", 8 + lengthOfLoad + padbytes,
  ...
  trfprintf(
    pOutFile, "DC   \t%p \t\t# Method Pointer", methodSymRef->isUnresolved() ? 0 : methodSymbol->getMethodAddress());
}
