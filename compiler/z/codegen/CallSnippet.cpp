...
uint8_t*
TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::Mnemonic op,
                                       uint8_t* buffer,
                                       TR::RealRegister* reg,
                                       int32_t offset,
                                       TR::CodeGenerator* cg)
...
uint8_t*
TR::S390CallSnippet::S390flushArgumentsToStack(uint8_t* buffer,
                                               TR::Node* callNode,
                                               int32_t argSize,
                                               TR::CodeGenerator* cg)
{
    ...
    switch (child->getDataType()) {
      ...
      case TR::Int32:
        ...
        if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
          buffer =
            storeArgumentItem(TR::InstOpCode::ST,
                              buffer,
      ...
      case TR::Address:
        ...
        if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
          buffer =
            storeArgumentItem(TR::InstOpCode::getStoreOpCode(),
                              buffer,
                              machine->getRealRegister(
                                linkage->getIntegerArgumentRegister(intArgNum)),
                              offset,
      ...
      case TR::Float:
        ...
        if (floatArgNum < linkage->getNumFloatArgumentRegisters()) {
          buffer =
            storeArgumentItem(TR::InstOpCode::STE,
                              buffer,
                              machine->getRealRegister(
      ...
      case TR::Double:
        ...
        if (floatArgNum < linkage->getNumFloatArgumentRegisters()) {
          buffer =
            storeArgumentItem(TR::InstOpCode::STD,
                              buffer,
                              machine->getRealRegister(
                                linkage->getFloatArgumentRegister(floatArgNum)),
                              offset,
 ...
 */
int32_t
TR::S390CallSnippet::instructionCountForArguments(TR::Node* callNode,
                                                  TR::CodeGenerator* cg)
{
...
TR_RuntimeHelper
TR::S390CallSnippet::getHelper(TR::MethodSymbol* methodSymbol,
                               TR::DataType type,
                               TR::CodeGenerator* cg)
{
...
TR_RuntimeHelper
TR::S390CallSnippet::getInterpretedDispatchHelper(
...
TR_Debug::print(TR::FILE* pOutFile, TR::S390CallSnippet* snippet)
{
  ...
  bufferPos = printS390ArgumentsFlush(
    pOutFile, callNode, bufferPos, snippet->getSizeOfArguments());
  ...
  bufferPos = printRuntimeInstrumentationOnOffInstruction(
    pOutFile, bufferPos, false); // RIOFF
  ...
  if (snippet->getKind() == TR::Snippet::IsUnresolvedCall) {
    ...
    trfprintf(pOutFile,
              "LARL \tGPR14, *+%d <%p>\t# Start of Data Const.",
    ...
    printPrefix(pOutFile, NULL, bufferPos, 6);
    trfprintf(pOutFile,
              "BRASL \tGPR14, <%p>\t# Branch to Helper Method %s",
  ...
  trfprintf(
    pOutFile, "DC   \t%p \t\t# Method Address", glueRef->getMethodAddress());
  ...
  trfprintf(pOutFile,
            "DC   \t%p \t\t# Method Pointer",
            methodSymRef->isUnresolved() ? 0
                                         : methodSymbol->getMethodAddress());
}
