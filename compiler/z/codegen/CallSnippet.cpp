...
TR::S390CallSnippet::getLength(int32_t estimatedSnippetStart)
{
    ...
    return (instructionCountForArguments(getNode(), cg()) + getPICBinaryLength(cg()) + 3 * sizeof(uintptrj_t) + getRuntimeInstrumentationOnOffInstructionLength(cg()) + sizeof(uintptrj_t)); // the last item is for padding
}
