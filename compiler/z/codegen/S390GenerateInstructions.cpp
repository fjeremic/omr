    ...
    bool targetIsFarAndCold)
{
    ...
    if (!cg->comp()->getOption(TR_DisableCompareAndBranchInstruction) && !needsCC && replacementOpCode != TR::InstOpCode::BAD && TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)) {
    ...
    // otherwise we'll generate with the compare opcode pased and an TR::InstOpCode::BRC.
        ...
        if (!needsCC && comp->getOption(TR_EnableEBBCCInfo) && cg->isActiveCompareCC(compareOpCode, first, second) && performTransformation(comp, "O^O generateS390CompareAndBranchInstruction case 1 remove RR Compare[%s\t %s, %s]: reuse CC from ccInst [%p].", opcTmp.getMnemonicName(), debugObj->getName(first), debugObj->getName(second), ccInst))
            returnInstruction = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, bc, node, branchDestination);
        else if (!needsCC && comp->getOption(TR_EnableEBBCCInfo) && cg->isActiveCompareCC(compareOpCode, second, first) && performTransformation(comp, "O^O generateS390CompareAndBranchInstruction case 2 remove RR Compare[%s\t %s, %s]: reuse CC from ccInst [%p].", opcTmp.getMnemonicName(), debugObj->getName(first), debugObj->getName(second), ccInst))
    ...
    TR::RealRegister::RegNum checkForThisRealRegister = TR::RealRegister::NoReg)
{
    for (int i = 0; i < conds1_addCursor; i++) {
        ...
        for (int j = 0; j < conds2_addCursor; j++) {
            ...
            if ((conds1_real == conds2_real) && (conds1->getRegisterDependency(i)->getRegister() != conds2->getRegisterDependency(j)->getRegister())) {
                // Conflict found
    ...
    TR::Instruction* preced)
{
    ...
    if (instructionFormat == RXa_FORMAT) {
        result = preced != NULL ? new (INSN_HEAP) TR::S390RXInstruction(op, n, treg, mf, preced, cg) : new (INSN_HEAP) TR::S390RXInstruction(op, n, treg, mf, cg);
    } else {
        ...
        result = preced != NULL ? new (INSN_HEAP) TR::S390RXYInstruction(op, n, treg, mf, preced, cg) : new (INSN_HEAP) TR::S390RXYInstruction(op, n, treg, mf, cg);
    }
...
generateRXInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, uint8_t mask, TR::MemoryReference* mf, TR::Instruction* preced)
{
    ...
    if (instructionFormat == RXb_FORMAT) {
        ...
        result = preced != NULL ? new (INSN_HEAP) TR::S390RXYbInstruction(op, n, mask, mf, preced, cg) : new (INSN_HEAP) TR::S390RXYbInstruction(op, n, mask, mf, cg);
    }
    ...
    TR::Instruction* preced)
{
    ...
    if (instructionFormat == RSa_FORMAT) {
        result = preced != NULL ? new (INSN_HEAP) TR::S390RSInstruction(op, n, treg, mf, preced, cg) : new (INSN_HEAP) TR::S390RSInstruction(op, n, treg, mf, cg);
    } else {
        ...
        result = preced != NULL ? new (INSN_HEAP) TR::S390RSYInstruction(op, n, treg, static_cast<uint32_t>(0), mf, preced, cg) : new (INSN_HEAP) TR::S390RSYInstruction(op, n, treg, static_cast<uint32_t>(0), mf, cg);
    }
    ...
    TR::MemoryReference* mf, TR::Instruction* preced)
{
    ...
    if (instructionFormat == RSb_FORMAT) {
        result = preced != NULL ? new (INSN_HEAP) TR::S390RSInstruction(op, n, treg, mask, mf, preced, cg) : new (INSN_HEAP) TR::S390RSInstruction(op, n, treg, mask, mf, cg);
    } else {
        ...
        result = preced != NULL ? new (INSN_HEAP) TR::S390RSYInstruction(op, n, treg, mask, mf, preced, cg) : new (INSN_HEAP) TR::S390RSYInstruction(op, n, treg, mask, mf, cg);
    }
    ...
    TR::MemoryReference* mf, TR::Instruction* preced)
{
    ...
    if (instructionFormat == RSa_FORMAT) {
        result = preced != NULL ? new (INSN_HEAP) TR::S390RSInstruction(op, n, treg, sreg, mf, preced, cg) : new (INSN_HEAP) TR::S390RSInstruction(op, n, treg, sreg, mf, cg);
    } else {
        TR_ASSERT_FATAL(instructionFormat == RSYa_FORMAT, "Mnemonic (%s) is incorrectly used as an RSY instruction", TR::InstOpCode::metadata[op].name);

        result = preced != NULL ? new (INSN_HEAP) TR::S390RSYInstruction(op, n, treg, sreg, mf, preced, cg) : new (INSN_HEAP) TR::S390RSYInstruction(op, n, treg, sreg, mf, cg);
    }
    ...
    TR::MemoryReference* mf, TR::Instruction* preced)
{
    ...
    if (instructionFormat == RSa_FORMAT) {
        result = preced != NULL ? new (INSN_HEAP) TR::S390RSInstruction(op, n, freg, lreg, mf, preced, cg) : new (INSN_HEAP) TR::S390RSInstruction(op, n, freg, lreg, mf, cg);
    } else {
        TR_ASSERT_FATAL(instructionFormat == RSYa_FORMAT, "Mnemonic (%s) is incorrectly used as an RSY instruction", TR::InstOpCode::metadata[op].name);

        result = preced != NULL ? new (INSN_HEAP) TR::S390RSYInstruction(op, n, freg, lreg, mf, preced, cg) : new (INSN_HEAP) TR::S390RSYInstruction(op, n, freg, lreg, mf, cg);
    }
    ...
    TR::Instruction* preced)
{
    ...
    if (instructionFormat == SI_FORMAT) {
        result = preced != NULL ? new (INSN_HEAP) TR::S390SIInstruction(op, n, mf, imm, preced, cg) : new (INSN_HEAP) TR::S390SIInstruction(op, n, mf, imm, cg);
    } else {
        ...
        result = preced != NULL ? new (INSN_HEAP) TR::S390SIYInstruction(op, n, mf, imm, preced, cg) : new (INSN_HEAP) TR::S390SIYInstruction(op, n, mf, imm, cg);
    }
...
#ifdef J9_PROJECT_SPECIFIC
    if (op == TR::InstOpCode::VAP || op == TR::InstOpCode::VSP || op == TR::InstOpCode::VDP || op == TR::InstOpCode::VMP || op == TR::InstOpCode::VMSP || op == TR::InstOpCode::VSDP || op == TR::InstOpCode::VRP) {
        generateS390DAAExceptionRestoreSnippet(cg, n, instr, op, false);
    }
#endif
    ...
    TR::RegisterDependencyConditions* cond, TR::Instruction* preced, TR::Register* base)
{
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
        if (op == TR::InstOpCode::LG || op == TR::InstOpCode::L) {
            TR::S390ConstantDataSnippet* targetSnippet = op == TR::InstOpCode::LG ? targetSnippet = cg->findOrCreate8ByteConstant(node, (int64_t)imm) : targetSnippet = cg->findOrCreate4ByteConstant(node, (int32_t)imm);
    ...
    TR::RegisterDependencyConditions* cond, TR::Instruction* preced, TR::Register* base, bool isPICCandidate)
{
    ...
    if (!LGRLinst) {
        cursor = instructionFormat == RXE_FORMAT ? generateRXEInstruction(cg, op, node, treg, dataref, 0) : generateRXInstruction(cg, op, node, treg, dataref);
    }
    if (alloc) {
        cg->stopUsingRegister(base);
    }
...
generateEXDispatch(TR::Node* node, TR::CodeGenerator* cg, TR::Register* maskReg, TR::Register* useThisLitPoolReg, TR::Instruction* instr, TR::Instruction* preced, TR::RegisterDependencyConditions* deps)
{
    ...
    while (instr->getSourceRegister(i)) {
        if (instr->getSourceRegister(i)->getRealRegister() == NULL && instr->getSourceRegister(i)->getAssignedRegister() == NULL) {
            depsNeeded++;
        }
        i++;
    }
    ...
    while (instr->getTargetRegister(j)) {
        if (instr->getTargetRegister(j)->getRealRegister() == NULL && instr->getTargetRegister(j)->getAssignedRegister() == NULL) {
            depsNeeded++;
        }
        j++;
    }
    ...
    if (instrMR) {
        if (instrMR->getBaseRegister() && instrMR->getBaseRegister()->getRealRegister() == NULL && instrMR->getBaseRegister()->getAssignedRegister() == NULL) {
            depsNeeded++;
        }
        if (instrMR->getIndexRegister() && instrMR->getIndexRegister()->getRealRegister() == NULL && instrMR->getIndexRegister()->getAssignedRegister() == NULL) {
            depsNeeded++;
        }
    }
    ...
    bool hasNOP)
{
    ...
    if (BCDCHKNode && BCDCHKNode->getOpCodeValue() == TR::BCDCHK) {
        ...
        TR::S390RestoreGPR7Snippet* restoreSnippet = new (INSN_HEAP) TR::S390RestoreGPR7Snippet(cg, n, restoreGPR7SnippetHandler, handlerLabel);
