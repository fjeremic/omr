...
TR::Instruction* generateS390BranchInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op,
  TR::InstOpCode::S390BranchCondition brCond, TR::Node* n, TR::LabelSymbol* sym, TR::RegisterDependencyConditions* cond,
  ...
  TR::LabelSymbol* branchDestination, bool needsCC, bool targetIsFarAndCold)
{
  ...
  if (!cg->comp()->getOption(TR_DisableCompareAndBranchInstruction) && !needsCC
    && replacementOpCode != TR::InstOpCode::BAD && TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)) {
  ...
  // otherwise we'll generate with the compare opcode pased and an TR::InstOpCode::BRC.
    ...
    if (!needsCC && comp->getOption(TR_EnableEBBCCInfo) && cg->isActiveCompareCC(compareOpCode, first, second)
      && performTransformation(comp,
        "O^O generateS390CompareAndBranchInstruction case 1 remove RR Compare[%s\t %s, %s]: reuse CC from ccInst [%p].",
      ...
      && performTransformation(comp,
        "O^O generateS390CompareAndBranchInstruction case 2 remove RR Compare[%s\t %s, %s]: reuse CC from ccInst [%p].",
        opcTmp.getMnemonicName(), debugObj->getName(first), debugObj->getName(second), ccInst))
...
TR::Instruction* generateRRFInstruction(
  TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, uint8_t mask, bool isMask3, TR::Instruction* preced)
{
...
bool CheckForRegisterDependencyConditionsRealRegisterMergeConflict(TR_S390RegisterDependencyGroup* conds1,
  int conds1_addCursor, TR_S390RegisterDependencyGroup* conds2, int conds2_addCursor,
  TR::RegisterDependency** conflict1, TR::RegisterDependency** conflict2, TR::CodeGenerator* cg,
  ...
  TR::Register* implicitRegTrg1, TR::Instruction* preced)
{
  ...
  if (preced)
    instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
      op, n, op1Reg, mf2, op3Reg, mf4, preced, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
  else
    instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
  ...
  TR::Register* implicitRegTrg0, TR::Register* implicitRegTrg1, TR::Instruction* preced)
{
  ...
  if (preced)
    instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
      op, n, op1Reg, mf2, NULL, NULL, preced, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
  else
    instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
...
TR::Instruction* generateSS5WithImplicitGPRsInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
  ...
  TR::Register* implicitRegSrc1, TR::Register* implicitRegTrg0, TR::Register* implicitRegTrg1, TR::Instruction* preced)
{
  if (mf2)
    preced = mf2->enforceSSFormatLimits(n, cg, preced);
  if (mf4)
  ...
  if (preced)
    instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
      op, n, op1Reg, mf2, NULL, mf4, preced, implicitRegSrc0, implicitRegSrc1, implicitRegTrg0, implicitRegTrg1, cg);
  else
    instr = new (INSN_HEAP) TR::S390SS5WithImplicitGPRsInstruction(
  ...
  TR::Register* targetReg, TR::Register* sourceReg2, TR::Instruction* preced)
{
  if (preced)
    return new (INSN_HEAP)
      TR::S390VRRaInstruction(cg, op, n, targetReg, sourceReg2, 0 /* mask5 */, 0 /* mask4 */, 0 /* mask3 */, preced);
  else
    return new (INSN_HEAP)
  ...
  TR::RegisterDependencyConditions* cond, TR::SymbolReference* callSymRef, TR::Instruction* preced)
{
  ...
  if (s->getKind() == TR::Snippet::IsVirtualUnresolved) {
    ...
    callInstr
      = new (INSN_HEAP) TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, killRegRA, s, postDeps, callSymRef, cg);
...
TR::Instruction* generateRegLitRefInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node,
  TR::Register* treg, int32_t imm, TR::RegisterDependencyConditions* cond, TR::Instruction* preced, TR::Register* base,
  bool isPICCandidate)
{
  TR::Compilation* comp = cg->comp();
  bool alloc = false;
      ...
      if (isMethod) {
        unloadableClass
          = (TR_OpaqueClassBlock*)cg->fe()
              ->createResolvedMethod(cg->trMemory(), (TR_OpaqueMethodBlock*)(intptr_t)imm, comp->getCurrentMethod())
              ->classOfMethod();
 ...
 */
TR::Instruction* generateRegLitRefInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node,
  TR::Register* treg, uintptrj_t imm, int32_t reloType, TR::RegisterDependencyConditions* cond, TR::Instruction* preced,
  TR::Register* base)
{
  bool alloc = false;
  TR::Instruction* cursor;
  ...
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    if (op == TR::InstOpCode::LG || op == TR::InstOpCode::L) {
      ...
      cursor = (TR::S390RILInstruction*)generateRILInstruction(
        cg, (op == TR::InstOpCode::LG) ? TR::InstOpCode::LGRL : TR::InstOpCode::LRL, node, treg, targetSnippet, preced);
      return cursor;
    }
  }
  ...
  AOTcgDiag5(comp,
    "generateRegLitRefInstruction dataref=%x constantDataSnippet=%x symbolReference=%x symbol=%x reloType=%x\n",
 ...
 */
TR::Instruction* generateRegLitRefInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node,
  TR::Register* treg, int64_t imm, TR::RegisterDependencyConditions* cond, TR::Instruction* preced, TR::Register* base,
  bool isPICCandidate)
{
  bool alloc = false;
  TR::Instruction* cursor;
  ...
  if (instructionFormat == RILa_FORMAT || instructionFormat == RILb_FORMAT || instructionFormat == RILc_FORMAT) {
    ...
    if (comp->getOption(TR_EnableHCR) && isPICCandidate) {
      ...
      if (node->isClassUnloadingConst()) {
        ...
        if (isMethod) {
          unloadableClass
            = (TR_OpaqueClassBlock*)cg->fe()
                ->createResolvedMethod(cg->trMemory(), (TR_OpaqueMethodBlock*)imm, comp->getCurrentMethod())
                ->classOfMethod();
          if (!TR::Compiler->cls.sameClassLoaders(comp, unloadableClass, comp->getCurrentMethod()->classOfMethod()))
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10) && op == TR::InstOpCode::LG) {
      ...
      LGRLinst
        = (TR::S390RILInstruction*)generateRILInstruction(cg, TR::InstOpCode::LGRL, node, treg, targetsnippet, 0);
      cursor = LGRLinst;
    } else {
      base = cg->allocateRegister();
      if (cond != 0) {
 ...
 */
TR::Instruction* generateRegLitRefInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node,
  TR::Register* treg, uintptrj_t imm, TR::RegisterDependencyConditions* cond, TR::Instruction* preced,
  ...
  TR::RegisterDependencyConditions* deps)
{
  ...
  // We need to grab all registers from the EX itself and the EX target instruction and attach them as post dependencies
  // to the EX inst This is to make sure that RA assigns all these registers when we assign the EX instruction Otherwise
  // RA may insert register movement instructions attached to the out of line (lit pool) EX target instruction.
...
TR::Instruction* generateS390EInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
  TR::Register* tgt, TR::Register* tgt2, TR::Register* src, TR::Register* src2, TR::RegisterDependencyConditions* cond,
  TR::Instruction* preced)
{
  if (preced) {
...
TR::Instruction* generateSerializationInstruction(TR::CodeGenerator* cg, TR::Node* node, TR::Instruction* preced)
{
  ...
  TR::InstOpCode::S390BranchCondition cond = TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)
  ...
  TR::Node* srcElementNode, int elementSize, TR::Register* zeroReg, TR::Instruction* preced)
{
  ...
  if (srcElementNode->getOpCode().isLoadConst()) {
      ...
      <= (uint64_t)(uint16_t)(-1)) // if source is integral loadconst with value <= max uint16_t
    {
      ...
      TR::MemoryReference* constMR
        = generateS390MemoryReference(srcElementNode->getIntegerNodeValue<int64_t>(), TR::Int64, cg, NULL, node);
    ...
    if (srcElementNode->getDataType() == TR::Double) {
      ...
      if (srcReg->getRegisterPair() != NULL) {
        ...
        generateVRSbInstruction(
          cg, TR::InstOpCode::VLVG, node, targetVRF, srcReg, generateS390MemoryReference(zeroReg, 0, cg), mask);
    }
 ...
 */
void generateShiftAndKeepSelected64Bit(TR::Node* node, TR::CodeGenerator* cg, TR::Register* aFirstRegister,
  TR::Register* aSecondRegister, int aFromBit, int aToBit, int aShiftAmount, bool aClearOtherBits,
  bool aSetConditionCode)
{
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) && !aSetConditionCode) {
    generateRIEInstruction(cg, TR::InstOpCode::RISBGN, node, aFirstRegister, aSecondRegister, aFromBit,
      aToBit | (aClearOtherBits ? 0x80 : 0x00), aShiftAmount);
  } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    generateRIEInstruction(cg, TR::InstOpCode::RISBG, node, aFirstRegister, aSecondRegister, aFromBit,
      aToBit | (aClearOtherBits ? 0x80 : 0x00), aShiftAmount);
  } else {
    generateRSInstruction(
      cg, TR::InstOpCode::SRLG, node, aFirstRegister, aSecondRegister, (63 - aToBit) + aShiftAmount);
    generateRSInstruction(
      cg, TR::InstOpCode::SLLG, node, aFirstRegister, aFirstRegister, (63 - aToBit) + aShiftAmount + aFromBit);
 ...
 * with an option to clear the rest of first register's bits.
 */
void generateShiftAndKeepSelected31Bit(TR::Node* node, TR::CodeGenerator* cg, TR::Register* aFirstRegister,
  TR::Register* aSecondRegister, int aFromBit, int aToBit, int aShiftAmount, bool aClearOtherBits,
  bool aSetConditionCode)
{
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
    generateRIEInstruction(cg, TR::InstOpCode::RISBLG, node, aFirstRegister, aSecondRegister, aFromBit,
      aToBit | (aClearOtherBits ? 0x80 : 0x00), aShiftAmount);
  } else {
