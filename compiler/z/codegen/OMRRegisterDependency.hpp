...
namespace OMR {
typedef OMR::Z::RegisterDependencyConditions
    RegisterDependencyConditionsConnector;
}
#else
...
#define RefsAndDefsDependentRegister                                           \
  (ReferencesDependentRegister | DefinesDependentRegister)
...
public:
  ...
  // use TR_S390RegisterDependencyGroup::create to allocate an object of this
  // type

  void *operator new(size_t s, int32_t numDependencies, TR_Memory *m) {
    TR_ASSERT(numDependencies > 0,
              "operator new called with numDependencies == 0");
    if (numDependencies > NUM_DEFAULT_DEPENDENCIES) {
      s += (numDependencies - NUM_DEFAULT_DEPENDENCIES) *
           sizeof(TR::RegisterDependency);
    }
    return m->allocateHeapMemory(s, TR_MemoryBase::RegisterDependencyGroup);
  }

  static TR_S390RegisterDependencyGroup *create(int32_t numDependencies,
                                                TR_Memory *m) {
    return numDependencies ? new (numDependencies, m)
                                 TR_S390RegisterDependencyGroup
                           : 0;
  }
  ...
  void setDependencyInfo(uint32_t index, TR::Register *vr,
                         TR::RealRegister::RegDep rr, uint8_t flag) {
    setDependencyInfo(index, vr, static_cast<TR::RealRegister::RegNum>(rr),
                      flag);
  }
  ...
  void setDependencyInfo(uint32_t index, TR::Register *vr,
    ...
    if (vr)
      vr->setDependencySet(true);
  }

  TR::Register *searchForRegister(TR::Register *vr, uint8_t flag,
                                  uint32_t numberOfRegisters,
                                  TR::CodeGenerator *cg) {
    for (int32_t i = 0; i < numberOfRegisters; i++) {
      if (_dependencies[i].getRegister() == vr &&
          (_dependencies[i].getFlags() & flag))
        return _dependencies[i].getRegister();
    }
    return NULL;
  }

  TR::Register *searchForRegister(TR::RealRegister::RegNum rr, uint8_t flag,
                                  uint32_t numberOfRegisters,
                                  TR::CodeGenerator *cg) {
    for (int32_t i = 0; i < numberOfRegisters; i++) {
  ...
  int32_t searchForRegisterPos(TR::Register *vr, uint8_t flag,
                               uint32_t numberOfRegisters,
                               TR::CodeGenerator *cg) {
    for (int32_t i = 0; i < numberOfRegisters; i++) {
      if (_dependencies[i].getRegister() == vr &&
  ...
  uint32_t genBitMapOfAssignableGPRs(TR::CodeGenerator *cg,
                                     uint32_t numberOfRegisters);

  void setRealRegisterForDependency(int32_t index,
  ...
  void decFutureUseCounts(uint32_t numberOfRegisters, TR::CodeGenerator *cg) {
  ...
  void stopUsingDepRegs(uint32_t numberOfRegisters, TR::Register *ret1,
                        TR::Register *ret2, TR::CodeGenerator *cg) {
    for (uint32_t i = 0; i < numberOfRegisters; i++) {
...
public:
  ...
  RegisterDependencyConditions(TR::RegisterDependencyConditions *iConds,
  ...
  RegisterDependencyConditions(TR_S390RegisterDependencyGroup *_preConditions,
                               ...
                               TR::CodeGenerator *cg)
      : _preConditions(_preConditions), _postConditions(_postConditions),
        _numPreConditions(numPreConds), _addCursorForPre(numPreConds),
        _numPostConditions(numPostConds), _addCursorForPost(numPostConds),
        _isHint(false), _isUsed(false), _conflictsResolved(false), _cg(cg) {}

  RegisterDependencyConditions()
      : _preConditions(NULL), _postConditions(NULL), _numPreConditions(0),
        _addCursorForPre(0), _numPostConditions(0), _addCursorForPost(0),
        _isHint(false), _isUsed(false), _conflictsResolved(false), _cg(NULL) {}

  // VMThread work: implicitly add an extra post condition for a possible vm
  // thread register post dependency.  Do not need for pre because they are so
  // infrequent.
  RegisterDependencyConditions(uint16_t numPreConds, uint16_t numPostConds,
                               TR::CodeGenerator *cg)
      : _preConditions(TR_S390RegisterDependencyGroup::create(numPreConds,
                                                              cg->trMemory())),
        _postConditions(TR_S390RegisterDependencyGroup::create(
            numPostConds + NUM_VM_THREAD_REG_DEPS, cg->trMemory())),
        _numPreConditions(numPreConds), _addCursorForPre(0),
  ...
  void resolveSplitDependencies(TR::CodeGenerator *cg, TR::Node *Noce,
                                TR::Node *child, TR::RegisterPair *regPair,
                                List<TR::Register> &regList, TR::Register *&reg,
                                TR::Register *&highReg, TR::Register *&copyReg,
  ...
  void addPreCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
                       uint8_t flag = ReferencesDependentRegister) {
    addPreCondition(vr, static_cast<TR::RealRegister::RegDep>(rr), flag);
  }

  void addPreCondition(TR::Register *vr, TR::RealRegister::RegDep rr,
                       uint8_t flag = ReferencesDependentRegister) {
    TR_ASSERT(!getIsUsed(), "ERROR: cannot add pre conditions to an used "
                            "dependency, create a copy first\n");
    ...
    if (_addCursorForPre >= _numPreConditions) {
      ...
      _cg->comp()->failCompilation<TR::CompilationException>(
          "addPreCondition list overflow, abort compilation\n");
    }
    _preConditions->setDependencyInfo(_addCursorForPre++, vr, rr, flag);
  }

  TR_S390RegisterDependencyGroup *getPostConditions() {
  ...
  void addAssignAnyPostCondOnMemRef(TR::MemoryReference *memRef) {
    ...
    if (base != NULL) {
      addPostConditionIfNotAlreadyInserted(base, TR::RealRegister::AssignAny,
                                           ReferencesDependentRegister);
    }
    if (indx != NULL) {
      addPostConditionIfNotAlreadyInserted(indx, TR::RealRegister::AssignAny,
                                           ReferencesDependentRegister);
    }
  }

  void addPostCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
                        uint8_t flag = ReferencesDependentRegister) {
    addPostCondition(vr, static_cast<TR::RealRegister::RegDep>(rr), flag);
  }

  void addPostCondition(TR::Register *vr, TR::RealRegister::RegDep rr,
                        uint8_t flag = ReferencesDependentRegister) {
    TR_ASSERT(!getIsUsed(), "ERROR: cannot add post conditions to an used "
                            "dependency, create a copy first\n");
    ...
    if (_addCursorForPost >= _numPostConditions) {
      ...
      _cg->comp()->failCompilation<TR::CompilationException>(
          "addPostCondition list overflow, abort compilation\n");
    }
    _postConditions->setDependencyInfo(_addCursorForPost++, vr, rr, flag);
  }

  void assignPreConditionRegisters(TR::Instruction *currentInstruction,
                                   TR_RegisterKinds kindToBeAssigned,
                                   TR::CodeGenerator *cg) {
    if (_preConditions != NULL) {
      cg->clearRegisterAssignmentFlags();
      cg->setRegisterAssignmentFlag(TR_PreDependencyCoercion);
      _preConditions->assignRegisters(currentInstruction, kindToBeAssigned,
                                      _addCursorForPre, cg);
    }
  }

  void assignPostConditionRegisters(TR::Instruction *currentInstruction,
                                    TR_RegisterKinds kindToBeAssigned,
                                    TR::CodeGenerator *cg) {
    if (_postConditions != NULL) {
      cg->clearRegisterAssignmentFlags();
      ...
      if (getIsHint())
        ...
        _postConditions->assignRegisters(currentInstruction, kindToBeAssigned,
                                         _addCursorForPost, cg);
    }
  }

  TR::Register *
  searchPreConditionRegister(TR::RealRegister::RegNum rr,
                             uint8_t flag = RefsAndDefsDependentRegister) {
    return _preConditions == NULL ? NULL
                                  : _preConditions->searchForRegister(
                                        rr, flag, _addCursorForPre, _cg);
  }

  TR::Register *
  searchPostConditionRegister(TR::RealRegister::RegNum rr,
                              uint8_t flag = RefsAndDefsDependentRegister) {
    return _postConditions == NULL ? NULL
                                   : _postConditions->searchForRegister(
                                         rr, flag, _addCursorForPost, _cg);
  }

  TR::Register *
  searchPreConditionRegister(TR::Register *vr,
                             uint8_t flag = RefsAndDefsDependentRegister) {
    return _preConditions == NULL ? NULL
                                  : _preConditions->searchForRegister(
                                        vr, flag, _addCursorForPre, _cg);
  }

  TR::Register *
  searchPostConditionRegister(TR::Register *vr,
                              uint8_t flag = RefsAndDefsDependentRegister) {
    return _postConditions == NULL ? NULL
                                   : _postConditions->searchForRegister(
                                         vr, flag, _addCursorForPost, _cg);
  }

  int32_t
  searchPostConditionRegisterPos(TR::Register *vr,
                                 uint8_t flag = RefsAndDefsDependentRegister) {
    return _postConditions == NULL ? -1
                                   : _postConditions->searchForRegisterPos(
                                         vr, flag, _addCursorForPost, _cg);
  }

  void stopUsingDepRegs(TR::CodeGenerator *cg, TR::Register *ret1 = NULL,
                        TR::Register *ret2 = NULL) {
    if (_preConditions != NULL)
  ...
  // These methods are temporary until dependencies are re-engineered down the
  // road We only add in the post condition if the desired virt reg is not
  // already spoken for
  bool addPreConditionIfNotAlreadyInserted(
      TR::Register *vr, TR::RealRegister::RegNum rr,
      uint8_t flag = ReferencesDependentRegister);
  bool addPreConditionIfNotAlreadyInserted(
      TR::Register *vr, TR::RealRegister::RegDep rr,
      uint8_t flag = ReferencesDependentRegister);
  ...
  TR::RegisterDependencyConditions *clone(TR::CodeGenerator *cg,
  ...
  void bookKeepingRegisterUses(TR::Instruction *instr, TR::CodeGenerator *cg,
                               int32_t oldPreCursor = 0,
                               int32_t oldPostCursor = 0);
  void createRegisterAssociationDirective(TR::Instruction *instruction,
                                          TR::CodeGenerator *cg);

  bool doesConditionExist(TR_S390RegisterDependencyGroup *regDepArr,
                          TR::Register *vr, TR::RealRegister::RegNum rr,
                          uint32_t flag, uint32_t numberOfRegisters,
                          bool overwriteAssignAny = false);
  bool doesPreConditionExist(TR::Register *vr, TR::RealRegister::RegNum rr,
                             uint32_t flag, bool overwriteAssignAny = false);
  bool doesPostConditionExist(TR::Register *vr, TR::RealRegister::RegNum rr,
                              uint32_t flag, bool overwriteAssignAny = false);
