...
public:
  ...
  {
    setDependencyInfo(
      index, vr, static_cast<TR::RealRegister::RegNum>(rr), flag);
  }
...
public:
  ...
  RegisterDependencyConditions(uint16_t numPreConds,
                               ...
                               TR::CodeGenerator* cg)
    : _preConditions(
        TR_S390RegisterDependencyGroup::create(numPreConds, cg->trMemory()))
  ...
  void addAssignAnyPostCondOnMemRef(TR::MemoryReference* memRef)
  {
    ...
    if (base != NULL) {
      addPostConditionIfNotAlreadyInserted(
        base, TR::RealRegister::AssignAny, ReferencesDependentRegister);
    }
    if (indx != NULL) {
      addPostConditionIfNotAlreadyInserted(
        indx, TR::RealRegister::AssignAny, ReferencesDependentRegister);
    }
  }
                                   ...
                                   TR::CodeGenerator* cg)
  {
    if (_preConditions != NULL) {
      ...
      _preConditions->assignRegisters(
        currentInstruction, kindToBeAssigned, _addCursorForPre, cg);
    }
  }
                                    ...
                                    TR::CodeGenerator* cg)
  {
    if (_postConditions != NULL) {
      ...
      if (getIsHint())
        ...
        _postConditions->assignRegisters(
          currentInstruction, kindToBeAssigned, _addCursorForPost, cg);
    }
  }
