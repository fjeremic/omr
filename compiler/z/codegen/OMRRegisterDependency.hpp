...
public:
  ...
  RegisterDependencyConditions(TR_S390RegisterDependencyGroup* _preConditions,
    TR_S390RegisterDependencyGroup* _postConditions, uint16_t numPreConds, uint16_t numPostConds, TR::CodeGenerator* cg)
  ...
  bool getConflictsResolved()
  {
    return _conflictsResolved;
  }
  void setConflictsResolved()
  {
    _conflictsResolved = true;
  }
  void resetConflictsResolved()
  {
    _conflictsResolved = false;
  }

  TR_S390RegisterDependencyGroup* getPreConditions()
  {
  ...
  uint32_t getAddCursorForPre()
  {
    return _addCursorForPre;
  }
  uint32_t setAddCursorForPre(uint16_t a)
  {
    return _addCursorForPre = a;
  }

  uint32_t getAddCursorForPost()
  {
    return _addCursorForPost;
  }
  uint32_t setAddCursorForPost(uint16_t a)
  {
  ...
  bool doesConditionExist(TR_S390RegisterDependencyGroup* regDepArr, TR::Register* vr, TR::RealRegister::RegNum rr,
    uint32_t flag, uint32_t numberOfRegisters, bool overwriteAssignAny = false);
  bool doesPreConditionExist(
    TR::Register* vr, TR::RealRegister::RegNum rr, uint32_t flag, bool overwriteAssignAny = false);
  bool doesPostConditionExist(
    TR::Register* vr, TR::RealRegister::RegNum rr, uint32_t flag, bool overwriteAssignAny = false);
