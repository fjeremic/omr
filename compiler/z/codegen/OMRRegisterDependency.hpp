 ...
 public:
  ...
  void stopUsingDepRegs(uint32_t numberOfRegisters, TR::Register *ret1,
    ...
    for (uint32_t i = 0; i < numberOfRegisters; i++) {
      ...
      if (depReg != ret1 && depReg != ret2) cg->stopUsingRegister(depReg);
    }
  }
...
class RegisterDependencyConditions : public OMR::RegisterDependencyConditions {
 ...
 public:
  ...
  void addPreCondition(TR::Register *vr, TR::RealRegister::RegDep rr,
                       uint8_t flag = ReferencesDependentRegister) {
    TR_ASSERT(!getIsUsed(),
              "ERROR: cannot add pre conditions to an used dependency, create "
              "a copy first\n");
  ...
  void addPostCondition(TR::Register *vr, TR::RealRegister::RegDep rr,
                        uint8_t flag = ReferencesDependentRegister) {
    TR_ASSERT(!getIsUsed(),
              "ERROR: cannot add post conditions to an used dependency, create "
              "a copy first\n");
  ...
  TR::Register *searchPreConditionRegister(
  ...
  TR::Register *searchPostConditionRegister(
      TR::RealRegister::RegNum rr,
      uint8_t flag = RefsAndDefsDependentRegister) {
  ...
  TR::Register *searchPreConditionRegister(
      TR::Register *vr, uint8_t flag = RefsAndDefsDependentRegister) {
    return _preConditions == NULL ? NULL
                                  : _preConditions->searchForRegister(
  ...
  int32_t searchPostConditionRegisterPos(
      TR::Register *vr, uint8_t flag = RefsAndDefsDependentRegister) {
