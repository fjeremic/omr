  ...
  void printInfo(const char *info) {
    if (_outFile) {
      if (!(!comp()->getOption(TR_TraceCG) &&
            comp()->getOptions()->getTraceCGOption(
                TR_TraceCGPostBinaryEncoding) &&
            comp()->getOptions()->getTraceCGOption(
...
class TR_S390PreRAPeephole : private TR_S390Peephole {
 ...
 private:
  /** \brief
   *     Attempts to reduce L[' '|FH|G] R,MR1  ST[' '|FH|G] R,MR2 sequences to
   * MVC MR2, MR1 to save a register and instruction.
   *
   *  \param storeOpCode
   ...
   */
  bool attemptLoadStoreReduction(TR::InstOpCode::Mnemonic storeOpCode,
                                 uint16_t size);
};
...
class TR_S390PostRAPeephole : private TR_S390Peephole {
 ...
 private:
  ...
  /** \brief
   *     Attempts to reduce LHI R,0 instructions to XR R,R instruction to save 2
   * bytes of icache.
   *
   *  \return
   *     true if the reduction was successful; false otherwise.
