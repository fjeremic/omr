    ...
    TR::RealRegister::RegNum& regNum)
{
    ...
    if (foundLow || foundHigh) {
        ...
        copyReg = (reg->containsCollectedReference() && !containsInternalPointer) ? cg->allocateCollectedReferenceRegister() : cg->allocateRegister(kind);
        ...
        iCursor = isVector ? generateVRRaInstruction(cg, opCode, node, copyReg, reg, iCursor) : generateRRInstruction(cg, opCode, node, copyReg, reg, iCursor);
        ...
        if (highReg) {
            if (kind == TR_GPR) {
                ...
                highCopyReg = (highReg->containsCollectedReference() && !containsInternalPointer) ? cg->allocateCollectedReferenceRegister() : cg->allocateRegister(kind);
...
{
    TR::RegisterDependencyConditions* other = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(_numPreConditions + additionalRegDeps,
...
bool OMR::Z::RegisterDependencyConditions::refsRegister(TR::Register* r)
{
    for (int32_t i = 0; i < _numPreConditions; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister() == r && _preConditions->getRegisterDependency(i)->getRefsRegister()) {
            return true;
        }
    }
    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister() == r && _postConditions->getRegisterDependency(j)->getRefsRegister()) {
            return true;
        }
    }
    return false;
...
{
    for (int32_t i = 0; i < _numPreConditions; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister() == r && _preConditions->getRegisterDependency(i)->getDefsRegister()) {
            return true;
        }
    }
    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister() == r && _postConditions->getRegisterDependency(j)->getDefsRegister()) {
            return true;
        }
    }
    return false;
...
{
    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister() == r && _postConditions->getRegisterDependency(j)->getRealRegister() == TR::RealRegister::MayDefine) {
            return true;
        }
    }
    return false;
...
{
    for (int32_t i = 0; i < _numPreConditions; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister() == r && (_preConditions->getRegisterDependency(i)->getRefsRegister() || _preConditions->getRegisterDependency(i)->getDefsRegister())) {
            return true;
        }
    }

    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister() == r && (_postConditions->getRegisterDependency(j)->getRefsRegister() || _postConditions->getRegisterDependency(j)->getDefsRegister())) {
            return true;
        }
    }
    return false;
 ...
 */
void OMR::Z::RegisterDependencyConditions::bookKeepingRegisterUses(TR::Instruction* instr, TR::CodeGenerator* cg, int32_t oldPreCursor, int32_t oldPostCursor)
{
...
void TR_S390RegisterDependencyGroup::assignRegisters(TR::Instruction* currentInstruction,
    ...
    TR::CodeGenerator* cg)
{
        ...
        for (i = 0; i < numOfDependencies; i++) {
            ...
            if (dependentRegNum == TR::RealRegister::SpilledReg && !_dependencies[i].getRegister()->getRealRegister()) {
                ...
                if (virtReg->getAssignedRealRegister()) {
                    ...
                    TR::Instruction* inst = isVector ? generateVRXInstruction(cg, opCode, currentNode, assignedReg, tempMR, 0, currentInstruction) : generateRXInstruction(cg, opCode, currentNode, assignedReg, tempMR, currentInstruction);
            ...
            // we also need to free up all locked backing storage if we are exiting the OOL during backwards RA assignment
                ...
                TR_RegisterKinds rk = virtReg->getKind();
                int32_t dataSize;
                if (toS390LabelInstruction(currentInstruction)->getLabelSymbol()->isStartOfColdInstructionStream() && location) {
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (dependentRegNum != TR::RealRegister::SpilledReg && dependentRegNum != TR::RealRegister::KillVolHighRegs && dependentRegNum != TR::RealRegister::NoReg) {
            virtReg = _dependencies[i].getRegister();
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (virtReg->getAssignedRealRegister() != NULL) {
            if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
                ...
                if (_dependencies[i].getRealRegister() == assignedRegNum || (map.getDependencyWithTarget(assignedRegNum) && ((virtReg->getKind() != TR_GPR || haveSpareGPRs) && (virtReg->getKind() != TR_FPR || haveSpareFPRs) && (virtReg->getKind() != TR_VRF || haveSpareVRFs)))) {
                    virtReg->block();
                }
            }
        }
    ...
    do {
        ...
        for (i = 0; i < numOfDependencies; i++) {
            ...
            if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::AssignAny && dependentRegNum != TR::RealRegister::SpilledReg && dependentRealReg->getState() == TR::RealRegister::Free) {
#if 0
    ...
    do {
        ...
        for (i = 0; i < numOfDependencies; i++) {
            ...
            if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::AssignAny && dependentRegNum != TR::RealRegister::SpilledReg) {
...
// Coerce the assignment
               ...
               fprintf(stdout, "COERCE2 %i (%s, %d)\n", i, cg->getDebug()->getName(virtReg), dependentRegNum-1);
                    ...
                    if (assignedRegister != NULL && assignedRegister->getState() == TR::RealRegister::Free) {
                        ...
                        for (int32_t lcount = 0; lcount < numOfDependencies; lcount++) {
                            ...
                            if (aRealNum != TR::RealRegister::NoReg && aRealNum != TR::RealRegister::AssignAny && aRealNum != TR::RealRegister::SpilledReg && assignedRegister == machine->getRealRegister(aRealNum)) {
#if 0
    ...
    for (i = 0; i < numOfDependencies; i++) {
        ...
        if (dependentRegister->getRegisterPair() == NULL && dependentRegister->getAssignedRegister() != NULL && getRegisterDependency(i)->getRealRegister() != TR::RealRegister::SpilledReg) {
            ...
            if (highAssignedRegister && highAssignedRegister->getRealRegister()->getState() != TR::RealRegister::Locked) {
 ...
 */
void OMR::Z::RegisterDependencyConditions::createRegisterAssociationDirective(TR::Instruction* instruction, TR::CodeGenerator* cg)
{
    TR::Machine* machine = cg->machine();
