...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
  ...
  : _preConditions(TR_S390RegisterDependencyGroup::create(
    (iConds ? iConds->getNumPreConditions() : 0) + numNewPreConds, cg->trMemory()))
  , _postConditions(TR_S390RegisterDependencyGroup::create(
      (iConds ? iConds->getNumPostConditions() : 0) + numNewPostConds, cg->trMemory()))
...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
  ...
  : _preConditions(TR_S390RegisterDependencyGroup::create(
    conds_1->getNumPreConditions() + conds_2->getNumPreConditions(), cg->trMemory()))
  , _postConditions(TR_S390RegisterDependencyGroup::create(
      conds_1->getNumPostConditions() + conds_2->getNumPostConditions(), cg->trMemory()))
  ...
  TR::Register*& copyReg, TR::Register*& highCopyReg, TR::Instruction*& iCursor, TR::RealRegister::RegNum& regNum)
{
  ...
  if (foundLow || foundHigh) {
    ...
    copyReg = (reg->containsCollectedReference() && !containsInternalPointer) ? cg->allocateCollectedReferenceRegister()
  ...
  TR::CodeGenerator* cg, int32_t additionalRegDeps)
{
  TR::RegisterDependencyConditions* other = new (cg->trHeapMemory())
    TR::RegisterDependencyConditions(_numPreConditions + additionalRegDeps, _numPostConditions + additionalRegDeps, cg);
  int32_t i = 0;
...
uint32_t TR_S390RegisterDependencyGroup::genBitMapOfAssignableGPRs(TR::CodeGenerator* cg, uint32_t numberOfRegisters)
{
  ...
  TR_RegisterKinds kindToBeAssigned, uint32_t numOfDependencies, TR::CodeGenerator* cg)
{
  ...
  for (i = 0; i < numOfDependencies; i++) {
    ...
    if (dependentRegNum != TR::RealRegister::SpilledReg && dependentRegNum != TR::RealRegister::KillVolHighRegs
      ...
      if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
        ...
        if (_dependencies[i].getRealRegister() == assignedRegNum
          || (map.getDependencyWithTarget(assignedRegNum)
            && ((virtReg->getKind() != TR_GPR || haveSpareGPRs) && (virtReg->getKind() != TR_FPR || haveSpareFPRs)
  ...
  for (i = 0; i < numOfDependencies; i++) {
    ...
    //
    if (dependentRegNum == TR::RealRegister::EvenOddPair || dependentRegNum == TR::RealRegister::FPPair) {
  ...
  do {
    ...
    for (i = 0; i < numOfDependencies; i++) {
      ...
      if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::AssignAny
        && dependentRegNum != TR::RealRegister::SpilledReg && dependentRealReg->getState() == TR::RealRegister::Free) {
#if 0
...
// Coerce the assignment
          ...
          if (assignedRegister != NULL && assignedRegister->getState() == TR::RealRegister::Free) {
            ...
            for (int32_t lcount = 0; lcount < numOfDependencies; lcount++) {
              ...
              if (aRealNum != TR::RealRegister::NoReg && aRealNum != TR::RealRegister::AssignAny
                && aRealNum != TR::RealRegister::SpilledReg && assignedRegister == machine->getRealRegister(aRealNum)) {
#if 0
...
bool OMR::Z::RegisterDependencyConditions::doesConditionExist(TR_S390RegisterDependencyGroup* regDepArr,
  TR::Register* vr, TR::RealRegister::RegNum rr, uint32_t flag, uint32_t numberOfRegisters, bool overwriteAssignAny)
{
...
bool OMR::Z::RegisterDependencyConditions::doesPreConditionExist(
  TR::Register* vr, TR::RealRegister::RegNum rr, uint32_t flag, bool overwriteAssignAny)
{
  ...
  TR::Register* vr, TR::RealRegister::RegDep rr, uint8_t flag)
{
  int32_t pos = -1;
  if (_postConditions != NULL
    && (pos = _postConditions->searchForRegisterPos(vr, RefsAndDefsDependentRegister, _addCursorForPost, _cg)) < 0) {
    if (!(vr->getRealRegister() && vr->getRealRegister()->getState() == TR::RealRegister::Locked))
