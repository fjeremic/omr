...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
  TR::RegisterDependencyConditions* iConds,
  uint16_t numNewPreConds,
  uint16_t numNewPostConds,
  TR::CodeGenerator* cg)
...
void
OMR::Z::RegisterDependencyConditions::resolveSplitDependencies(
  TR::CodeGenerator* cg,
  TR::Node* node,
  TR::Node* child,
  TR::RegisterPair* regPair,
  List<TR::Register>& regList,
  TR::Register*& reg,
  TR::Register*& highReg,
  TR::Register*& copyReg,
  TR::Register*& highCopyReg,
  ...
  if (foundLow || foundHigh) {
    ...
    if (highReg) {
      if (kind == TR_GPR) {
        ...
        iCursor = generateRRInstruction(
          cg, opCode, node, highCopyReg, highReg, iCursor);
        highReg = highCopyReg;
      } else // kind == FPR
      {
 ...
 */
void
OMR::Z::RegisterDependencyConditions::bookKeepingRegisterUses(
  TR::Instruction* instr,
  TR::CodeGenerator* cg,
  int32_t oldPreCursor,
...
uint32_t
TR_S390RegisterDependencyGroup::genBitMapOfAssignableGPRs(
  TR::CodeGenerator* cg,
...
void
TR_S390RegisterDependencyGroup::assignRegisters(
  ...
  TR::CodeGenerator* cg)
{
      ...
      if (dependentRegNum == TR::RealRegister::SpilledReg &&
        ...
        if (virtReg->getAssignedRealRegister()) {
          ...
          TR::MemoryReference* tempMR = generateS390MemoryReference(
            currentNode,
            virtReg->getBackingStorage()->getSymbolReference(),
            cg);
          TR::InstOpCode::Mnemonic opCode;
  ...
  for (i = 0; i < numOfDependencies; i++) {
    ...
    if (dependentRegNum == TR::RealRegister::LegalEvenOfPair) {
      ...
      //
      if (assRealReg == NULL || (!machine->isLegalEvenRegister(
                                  toRealRegister(assRealReg), ALLOWBLOCKED))) {
...
#ifdef DEBUG_MMIT
        fprintf(stderr,
                "DEP ASS Best Legal Even %p (NOT pair)\n",
    ...
    if (dependentRegNum == TR::RealRegister::LegalOddOfPair) {
      // if not assigned, or assigned but not legal.
      //
      if (assRealReg == NULL || (!machine->isLegalOddRegister(
                                  toRealRegister(assRealReg), ALLOWBLOCKED))) {
...
#ifdef DEBUG_MMIT
        fprintf(stderr,
                "DEP ASS Best Legal Odd %p (NOT pair)\n",
    ...
    if (dependentRegNum == TR::RealRegister::LegalFirstOfFPPair) {
      // if not assigned, or assigned but not legal.
      //
      if (assRealReg == NULL || (!machine->isLegalFirstOfFPRegister(
                                  toRealRegister(assRealReg), ALLOWBLOCKED))) {
...
#ifdef DEBUG_FPPAIR
        fprintf(stderr,
                "DEP ASS Best Legal FirstOfFPPair %p (NOT pair)\n",
    ...
    if (dependentRegNum == TR::RealRegister::LegalSecondOfFPPair) {
      // if not assigned, or assigned but not legal.
      //
      if (assRealReg == NULL || (!machine->isLegalSecondOfFPRegister(
                                  toRealRegister(assRealReg), ALLOWBLOCKED))) {
...
#ifdef DEBUG_FPPAIR
        fprintf(stderr,
                "DEP ASS Best Legal SecondOfFPPair  %p (NOT pair)\n",
    ...
    for (i = 0; i < numOfDependencies; i++) {
      ...
      if (dependentRegNum != TR::RealRegister::NoReg &&
        ...
        machine->coerceRegisterAssignment(
          currentInstruction, virtReg, dependentRegNum);
        virtReg->block();
        changed = true;
      }
    }
          ...
          if (assignedRegister != NULL &&
            ...
            for (int32_t lcount = 0; lcount < numOfDependencies; lcount++) {
              ...
              if (aRealNum != TR::RealRegister::NoReg &&
                ...
                machine->coerceRegisterAssignment(
                  currentInstruction, depReg, aRealNum);
 ...
 */
void
OMR::Z::RegisterDependencyConditions::createRegisterAssociationDirective(
...
bool
OMR::Z::RegisterDependencyConditions::doesConditionExist(
  TR_S390RegisterDependencyGroup* regDepArr,
  TR::Register* vr,
  TR::RealRegister::RegNum rr,
  uint32_t flag,
  uint32_t numberOfRegisters,
...
bool
OMR::Z::RegisterDependencyConditions::doesPreConditionExist(
  TR::Register* vr,
  TR::RealRegister::RegNum rr,
  uint32_t flag,
  bool overwriteAssignAny)
{
  return doesConditionExist(
    _preConditions, vr, rr, flag, _addCursorForPre, overwriteAssignAny);
}

bool
OMR::Z::RegisterDependencyConditions::doesPostConditionExist(
  TR::Register* vr,
  TR::RealRegister::RegNum rr,
  uint32_t flag,
  bool overwriteAssignAny)
{
  return doesConditionExist(
    _postConditions, vr, rr, flag, _addCursorForPost, overwriteAssignAny);
}

bool
OMR::Z::RegisterDependencyConditions::addPreConditionIfNotAlreadyInserted(
  TR::Register* vr,
  TR::RealRegister::RegNum rr,
  uint8_t flag)
{
...
bool
OMR::Z::RegisterDependencyConditions::addPostConditionIfNotAlreadyInserted(
  TR::Register* vr,
  TR::RealRegister::RegNum rr,
  uint8_t flag)
{
