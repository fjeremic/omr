...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
  ...
  _preConditions =
      new (totalNum, cg->trMemory()) TR_S390RegisterDependencyGroup;
  _postConditions =
      new (totalNum, cg->trMemory()) TR_S390RegisterDependencyGroup;
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (child->getHighGlobalRegisterNumber() > -1) {
      highRegNum = (TR::RealRegister::RegNum)cg->getGlobalRegister(
          child->getHighGlobalRegisterNumber());
    }

    TR::RegisterPair *regPair = reg->getRegisterPair();
    resolveSplitDependencies(cg, node, child, regPair, regList, reg, highReg,
                             copyReg, highCopyReg, iCursor, regNum);
...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
    TR::RegisterDependencyConditions *iConds, uint16_t numNewPreConds,
    uint16_t numNewPostConds, TR::CodeGenerator *cg)
    : _preConditions(TR_S390RegisterDependencyGroup::create(
          (iConds ? iConds->getNumPreConditions() : 0) + numNewPreConds,
          cg->trMemory())),
      _postConditions(TR_S390RegisterDependencyGroup::create(
          (iConds ? iConds->getNumPostConditions() : 0) + numNewPostConds,
          cg->trMemory())),
      _numPreConditions((iConds ? iConds->getNumPreConditions() : 0) +
                        numNewPreConds),
      _addCursorForPre(0),
      _numPostConditions((iConds ? iConds->getNumPostConditions() : 0) +
                         numNewPostConds),
      _addCursorForPost(0), _isHint(false), _isUsed(false),
      _conflictsResolved(false), _cg(cg) {
  int32_t i;

  TR_S390RegisterDependencyGroup *depGroup;
  uint16_t numPreConds =
      (iConds ? iConds->getNumPreConditions() : 0) + numNewPreConds;
  uint16_t numPostConds =
      (iConds ? iConds->getNumPostConditions() : 0) + numNewPostConds;
  uint32_t flag;
...
OMR::Z::RegisterDependencyConditions::RegisterDependencyConditions(
    TR::RegisterDependencyConditions *conds_1,
    TR::RegisterDependencyConditions *conds_2, TR::CodeGenerator *cg)
    : _preConditions(TR_S390RegisterDependencyGroup::create(
          conds_1->getNumPreConditions() + conds_2->getNumPreConditions(),
          cg->trMemory())),
      _postConditions(TR_S390RegisterDependencyGroup::create(
          conds_1->getNumPostConditions() + conds_2->getNumPostConditions(),
          cg->trMemory())),
      _numPreConditions(conds_1->getNumPreConditions() +
                        conds_2->getNumPreConditions()),
      _addCursorForPre(0), _numPostConditions(conds_1->getNumPostConditions() +
                                              conds_2->getNumPostConditions()),
      _addCursorForPost(0), _isUsed(false), _isHint(false),
      _conflictsResolved(false), _cg(cg) {
  int32_t i;
  TR_S390RegisterDependencyGroup *depGroup;
  uint32_t flag;
...
void OMR::Z::RegisterDependencyConditions::resolveSplitDependencies(
    TR::CodeGenerator *cg, TR::Node *node, TR::Node *child,
    TR::RegisterPair *regPair, List<TR::Register> &regList, TR::Register *&reg,
    TR::Register *&highReg, TR::Register *&copyReg, TR::Register *&highCopyReg,
    TR::Instruction *&iCursor, TR::RealRegister::RegNum &regNum) {
  TR::Compilation *comp = cg->comp();
  ...
  if (foundLow || foundHigh) {
    ...
    iCursor =
        isVector
            ? generateVRRaInstruction(cg, opCode, node, copyReg, reg, iCursor)
    ...
    if (highReg) {
      if (kind == TR_GPR) {
        ...
        highCopyReg =
            (highReg->containsCollectedReference() && !containsInternalPointer)
                ? cg->allocateCollectedReferenceRegister()
                : cg->allocateRegister(kind);

        if (containsInternalPointer) {
          highCopyReg->setContainsInternalPointer();
          highCopyReg->setPinningArrayPointer(
              highReg->getPinningArrayPointer());
        }

        iCursor = generateRRInstruction(cg, opCode, node, highCopyReg, highReg,
                                        iCursor);
        highReg = highCopyReg;
      } else // kind == FPR
      {
...
OMR::Z::RegisterDependencyConditions::clone(TR::CodeGenerator *cg,
                                            int32_t additionalRegDeps) {
  TR::RegisterDependencyConditions *other = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(_numPreConditions + additionalRegDeps,
                                       _numPostConditions + additionalRegDeps,
                                       cg);
  int32_t i = 0;
  ...
  for (i = _numPreConditions - 1; i >= 0; --i) {
    ...
    other->getPreConditions()->setDependencyInfo(
        i, dep->getRegister(), dep->getRealRegister(), dep->getFlags());
  }
...
bool OMR::Z::RegisterDependencyConditions::mayDefineRegister(TR::Register *r) {
  for (int32_t j = 0; j < _numPostConditions; j++) {
    if (_postConditions->getRegisterDependency(j)->getRegister() == r &&
        _postConditions->getRegisterDependency(j)->getRealRegister() ==
            TR::RealRegister::MayDefine) {
      return true;
    }
  }
  return false;
 ...
 * When the dependencies have been created by merging new registers into an
 * existing list then only call useRegister for the new registers in the list.
 * Calling useRegister again on the existing registers will cause the
 * totalUseCount to be too high and the virtual register will remain set to the
 * real register for the entire duration of register assignment.
 */
void OMR::Z::RegisterDependencyConditions::bookKeepingRegisterUses(
    TR::Instruction *instr, TR::CodeGenerator *cg, int32_t oldPreCursor,
    int32_t oldPostCursor) {
  if (instr->getOpCodeValue() != TR::InstOpCode::ASSOCREGS) {
    ...
    if (_preConditions != NULL) {
      for (int32_t i = oldPreCursor; i < _addCursorForPre; i++) {
        instr->useRegister(
            _preConditions->getRegisterDependency(i)->getRegister(), true);
      }
    }
    ...
    if (_postConditions != NULL) {
      for (int32_t j = oldPostCursor; j < _addCursorForPost; j++) {
        instr->useRegister(
            _postConditions->getRegisterDependency(j)->getRegister(), true);
      }
    }
  }
}

uint32_t TR_S390RegisterDependencyGroup::genBitMapOfAssignableGPRs(
    TR::CodeGenerator *cg, uint32_t numberOfRegisters) {
  TR::Machine *machine = cg->machine();

  // TODO: This looks like it can be cached as it should be the same value every
  // time in a single compilation. Should also investigate whether we even need
  // this? Don't we already block registers which are part of a dependency? I
  // suppose with a mask we can avoid shuffling things into GPRs which are part
  // of the dependency and may need to get other virtual registers shuffled into
  // such GPRs. Need to think about this. Are other codegens doing this?
...
void TR_S390RegisterDependencyGroup::assignRegisters(
    TR::Instruction *currentInstruction, TR_RegisterKinds kindToBeAssigned,
    uint32_t numOfDependencies, TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Machine *machine = cg->machine();
  TR::Register *virtReg;
  ...
  if (!comp->getOption(TR_DisableOOL)) {
    for (i = 0; i < numOfDependencies; i++) {
      ...
      if (dependentRegNum == TR::RealRegister::SpilledReg &&
          !_dependencies[i].getRegister()->getRealRegister()) {
        TR_ASSERT_FATAL(virtReg->getBackingStorage() != NULL,
                        "Spilled virtual register on dependency list does not "
                        "have backing storage");

        if (virtReg->getAssignedRealRegister()) {
          // this happens when the register was first spilled in main line path
          // then was reverse spilled and assigned to a real register in OOL
          // path. We protected the backing store when doing the reverse spill
          // so we could re-spill to the same slot now
          TR::Node *currentNode = currentInstruction->getNode();
          TR::RealRegister *assignedReg =
              toRealRegister(virtReg->getAssignedRegister());
          cg->traceRegisterAssignment(
              "\nOOL: Found %R spilled in main line and reused inside OOL",
              virtReg);
          TR::MemoryReference *tempMR = generateS390MemoryReference(
              currentNode, virtReg->getBackingStorage()->getSymbolReference(),
              cg);
          TR::InstOpCode::Mnemonic opCode;
          ...
          TR::Instruction *inst =
              isVector
                  ? generateVRXInstruction(cg, opCode, currentNode, assignedReg,
                                           tempMR, 0, currentInstruction)
                  : generateRXInstruction(cg, opCode, currentNode, assignedReg,
        ...
        // now we are leaving the OOL sequence, anything that was previously
        // spilled in OOL hot path or main line should be considered as mainline
        // spill for the next OOL sequence.
        virtReg->getBackingStorage()->setMaxSpillDepth(1);
      }
      // we also need to free up all locked backing storage if we are exiting
      // the OOL during backwards RA assignment
      else if (currentInstruction->isLabel() &&
  ...
  TR_ASSERT(lockedGPRs == machine->getNumberOfLockedRegisters(TR_GPR),
            "Inconsistent number of locked GPRs");
  TR_ASSERT(lockedFPRs == machine->getNumberOfLockedRegisters(TR_FPR),
            "Inconsistent number of locked FPRs");
  TR_ASSERT(lockedVRFs == machine->getNumberOfLockedRegisters(TR_VRF),
            "Inconsistent number of locked VRFs");
#endif

  // To handle circular dependencies, we block a real register if (1) it is
  // already assigned to a correct virtual register and (2) if it is assigned to
  // one register in the list but is required by another. However, if all
  // available registers are requested, we do not block in case (2) to avoid all
  // registers being blocked.

  bool haveSpareGPRs = true;
  bool haveSpareFPRs = true;
  bool haveSpareVRFs = true;

  TR_ASSERT(numGPRs <= (TR::RealRegister::LastGPR - TR::RealRegister::FirstGPR +
                        1 - machine->getNumberOfLockedRegisters(TR_GPR)),
            "Too many GPR dependencies, unable to assign");
  TR_ASSERT(numFPRs <= (TR::RealRegister::LastFPR - TR::RealRegister::FirstFPR +
                        1 - machine->getNumberOfLockedRegisters(TR_FPR)),
            "Too many FPR dependencies, unable to assign");
  TR_ASSERT(numVRFs <= (TR::RealRegister::LastVRF - TR::RealRegister::FirstVRF +
                        1 - machine->getNumberOfLockedRegisters(TR_VRF)),
            "Too many VRF dependencies, unable to assign");

  if (numGPRs == (TR::RealRegister::LastGPR - TR::RealRegister::FirstGPR + 1 -
                  machine->getNumberOfLockedRegisters(TR_GPR)))
    haveSpareGPRs = false;
  if (numFPRs == (TR::RealRegister::LastFPR - TR::RealRegister::FirstFPR + 1 -
                  machine->getNumberOfLockedRegisters(TR_FPR)))
    haveSpareFPRs = false;
  if (numVRFs == (TR::RealRegister::LastVRF - TR::RealRegister::FirstVRF + 1 -
  ...
  for (i = 0; i < numOfDependencies; i++) {
    ...
    if (virtReg->getAssignedRealRegister() != NULL) {
      if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg) {
        ...
        TR::RealRegister::RegNum assignedRegNum =
            toRealRegister(virtReg->getAssignedRealRegister())
                ->getRegisterNumber();

        // Always block if the required register and assigned register match or
        // if the assigned register is required by another dependency but only
        // if there are any spare registers left so as to avoid blocking all
        // existing registers
    ...
    if (_dependencies[i].getRealRegister() ==
        TR::RealRegister::KillVolHighRegs) {
      machine->spillAllVolatileHighRegisters(currentInstruction);
      _dependencies[i].setRealRegister(
          TR::RealRegister::NoReg); // ignore from now on
    }
  }

  /////    REGISTER PAIRS

  // We handle register pairs by passing regpair dependencies stubs into the
  // register dependency list.  The following pieces of code are responsible for
  // replacing these stubs with actual real register values, using register
  // allocation routines in the S390Machine, and hence, allow the dependency
  // routines to complete the allocation.
  //
  // We need to run through the list of dependencies looking for register pairs.
  // We assign a register pair to any such pairs that are found and then look
  // for the low and high order regs in the list of deps.  We update the low and
  // high order pair deps with the real reg assigned by to the pair, and then
  // let the these deps be handled as any other single reg dep.
  //
  ...
  // and low regs as the real high and low will not match the dep virt reg
  // entries.
  //
  for (i = 0; i < numOfDependencies; i++) {
    // Get reg pair pointer
    //
    TR::RegisterPair *virtRegPair =
        _dependencies[i].getRegister()->getRegisterPair();
    ...
    if (dependentRegNum == TR::RealRegister::EvenOddPair ||
      ...
      // This call returns a pair placeholder that points to the assigned real
      // registers
      //     retPair:
      //                 high -> realRegHigh -> virtRegHigh
      //                 low  -> realRegLow  -> virtRegLow
      //
      // No bookkeeping on assignment call as we do bookkeeping at end of this
      // method
      //
      retPair = (TR::RegisterPair *)machine->assignBestRegister(
          (TR::Register *)virtRegPair, currentInstruction, NOBOOKKEEPING,
          availRegMask);
      ...
      // See if there are any LegalEvenOfPair/LegalOddOfPair deps in list that
      // correspond to the assigned pair
      //
      for (j = 0; j < numOfDependencies; j++) {

        // Check to ensure correct assignment of even/odd pair.
        //
        if (((_dependencies[j].getRealRegister() ==
                  TR::RealRegister::LegalEvenOfPair ||
              _dependencies[j].getRealRegister() ==
  ...
  for (i = 0; i < numOfDependencies; i++) {
    ...
    if (dependentRegNum == TR::RealRegister::LegalEvenOfPair) {
      ...
      //
      if (assRealReg == NULL ||
          (!machine->isLegalEvenRegister(toRealRegister(assRealReg),
                                         ALLOWBLOCKED))) {
    ...
    if (dependentRegNum == TR::RealRegister::LegalOddOfPair) {
      ...
      //
      if (assRealReg == NULL ||
          (!machine->isLegalOddRegister(toRealRegister(assRealReg),
                                        ALLOWBLOCKED))) {
    ...
    if (dependentRegNum == TR::RealRegister::LegalFirstOfFPPair) {
      ...
      //
      if (assRealReg == NULL ||
          (!machine->isLegalFirstOfFPRegister(toRealRegister(assRealReg),
                                              ALLOWBLOCKED))) {
        TR::RealRegister *bestFirstFP =
            machine->findBestLegalSiblingFPRegister(true);
        _dependencies[i].setRealRegister(TR::RealRegister::NoReg);
    ...
    if (dependentRegNum == TR::RealRegister::LegalSecondOfFPPair) {
      ...
      //
      if (assRealReg == NULL ||
          (!machine->isLegalSecondOfFPRegister(toRealRegister(assRealReg),
                                               ALLOWBLOCKED))) {
        TR::RealRegister *bestSecondFP =
            machine->findBestLegalSiblingFPRegister(false);
        _dependencies[i].setRealRegister(TR::RealRegister::NoReg);
  ...
  // ops to ensure that any defined dependency [virt reg, real reg] (where real
  // reg is not NoReg), is assigned such that virtreg->realreg.
  //
  do {
    changed = false;
...
// Coerce the assignment
          ...
          if (assignedRegister != NULL &&
              assignedRegister->getState() == TR::RealRegister::Free) {
            // For each dependency
            for (int32_t lcount = 0; lcount < numOfDependencies; lcount++) {
              // get the dependencies required real reg
              TR::RealRegister::RegNum aRealNum =
                  _dependencies[lcount].getRealRegister();
              ...
              if (aRealNum != TR::RealRegister::NoReg &&
                ...
                machine->coerceRegisterAssignment(currentInstruction, depReg,
                                                  aRealNum);
  ...
  // This is generally used when we wish to force an early assignment to a virt
  // reg (before the first inst that actually uses the reg).
  //
  ...
  for (i = 0; i < numOfDependencies; i++) {
    if (_dependencies[i].getRealRegister() == TR::RealRegister::AssignAny) {
      ...
      if (virtReg->isUsedInMemRef()) {
        ...
        // No bookkeeping on assignment call as we do bookkeeping at end of this
        // method
        TR::Register *targetReg = machine->assignBestRegister(
            virtReg, currentInstruction, NOBOOKKEEPING, availRegMask);

        // Disable this dependency
        _dependencies[i].setRealRegister(
            toRealRegister(targetReg)->getRegisterNumber());
        virtReg->block();
      }
    }
  }
  ...
  for (i = 0; i < numOfDependencies; i++) {
    if (_dependencies[i].getRealRegister() == TR::RealRegister::AssignAny) {
      ...
      if (!virtReg->isUsedInMemRef()) {
        ...
        // method
        TR::Register *targetReg = machine->assignBestRegister(
            virtReg, currentInstruction, NOBOOKKEEPING, 0xffffffff);
  ...
  // Check to see if we can release any registers that are not used in the
  // future This is why we omit BOOKKEEPING in the assign calls above
  //
  for (i = 0; i < numOfDependencies; i++) {
    ...
    if (dependentRegister->getRegisterPair() == NULL &&
        ...
        getRegisterDependency(i)->getRealRegister() !=
            TR::RealRegister::SpilledReg) {
      ...
      if (assignedRegister &&
          assignedRealRegister->getState() != TR::RealRegister::Locked) {
        TR_ASSERT(
            dependentRegister->getFutureUseCount() >= 0,
      ...
      TR::Register *highAssignedRegister =
          dependentRegisterHigh->getAssignedRegister();

      if (highAssignedRegister &&
          highAssignedRegister->getRealRegister()->getState() !=
      ...
      if (highAssignedRegister &&
        ...
        TR::RealRegister *assignedRegister =
            highAssignedRegister->getRealRegister();
      ...
      TR::Register *lowAssignedRegister =
          dependentRegisterLow->getAssignedRegister();

      if (lowAssignedRegister &&
          lowAssignedRegister->getRealRegister()->getState() !=
              TR::RealRegister::Locked) {
        TR_ASSERT(
      ...
      if (lowAssignedRegister &&
          dependentRegisterLow->getFutureUseCount() == 0) {
        TR::RealRegister *assignedRegister =
            lowAssignedRegister->getRealRegister();
  ...
  if (!comp->getOption(TR_DisableOOL)) {
    ...
    for (i = 0; i < numOfDependencies; i++) {
      if (_dependencies[i].getRegister()->getRealRegister()) {
        TR::RealRegister *realReg =
            toRealRegister(_dependencies[i].getRegister());
        dependentRegNum = _dependencies[i].getRealRegister();
        if (dependentRegNum == TR::RealRegister::SpilledReg) {
          virtReg = realReg->getAssignedRegister();

          traceMsg(comp, "\nOOL HPR Spill: %s",
                   cg->getDebug()->getName(realReg));
...
void OMR::Z::RegisterDependencyConditions::createRegisterAssociationDirective(
  ...
  for (int32_t j = 0; j < getNumPreConditions(); j++) {
    ...
    if (dependency->getRegister()) {
      machine->setVirtualAssociatedWithReal(dependency->getRealRegister(),
                                            dependency->getRegister());
    }
  }
...
bool OMR::Z::RegisterDependencyConditions::doesConditionExist(
    TR_S390RegisterDependencyGroup *regDepArr, TR::Register *vr,
    TR::RealRegister::RegNum rr, uint32_t flag, uint32_t numberOfRegisters,
    bool overwriteAssignAny) {
  for (int32_t i = 0; i < numberOfRegisters; i++) {
    ...
    if ((regDep->getRegister() == vr) && (regDep->getFlags() == flag)) {
      // If all properties of these two dependencies are the same, then it
      // exists already in the dependency group.
      if (regDep->getRealRegister() == rr) {
        return true;
      }

      // Post-condition number: Virtual register pointer, RegNum number,
      // condition flags 0: 4AE60068, 71(AssignAny), 00000003 1: 4AE5F04C,
      // 71(AssignAny), 00000003 2: 4AE60158, 15(GPR14), 00000003
      //
      // merged with:
      //
      // 0: 4AE60068, 1(GPR0), 00000003
      //
      // Produces an assertion. The fix for this is to overwrite the AssignAny
      // dependency.
      //
      else if (regDep->getRealRegister() == TR::RealRegister::AssignAny) {
        // AssignAny exists already, and the argument real register is not
        // AssignAny
        // - overwrite the existing real register with the argument real
        // register
        regDep->setRealRegister(rr);
        return true;
      } else if (rr == TR::RealRegister::AssignAny) {
        // The existing register dependency is not AssignAny but the argument
        // real register is
        // - simply return true
        return true;
      }
...
bool OMR::Z::RegisterDependencyConditions::doesPreConditionExist(
    TR::Register *vr, TR::RealRegister::RegNum rr, uint32_t flag,
    bool overwriteAssignAny) {
  return doesConditionExist(_preConditions, vr, rr, flag, _addCursorForPre,
                            overwriteAssignAny);
}

bool OMR::Z::RegisterDependencyConditions::doesPostConditionExist(
    TR::Register *vr, TR::RealRegister::RegNum rr, uint32_t flag,
    bool overwriteAssignAny) {
  return doesConditionExist(_postConditions, vr, rr, flag, _addCursorForPost,
                            overwriteAssignAny);
}
...
bool OMR::Z::RegisterDependencyConditions::addPreConditionIfNotAlreadyInserted(
  ...
  return addPreConditionIfNotAlreadyInserted(
      vr, static_cast<TR::RealRegister::RegDep>(rr), flag);
}
 ...
 * @param rr    The real register or real register property of the
 * pre-condition.
...
bool OMR::Z::RegisterDependencyConditions::addPreConditionIfNotAlreadyInserted(
  ...
  int32_t pos = -1;
  if (_preConditions != NULL &&
      (pos = _preConditions->searchForRegisterPos(
           vr, RefsAndDefsDependentRegister, _addCursorForPre, _cg)) < 0) {
    if (!(vr->getRealRegister() &&
          vr->getRealRegister()->getState() == TR::RealRegister::Locked))
      addPreCondition(vr, rr, flag);
    return true;
  } else if (pos >= 0 &&
             _preConditions->getRegisterDependency(pos)->getFlags() != flag) {
...
bool OMR::Z::RegisterDependencyConditions::addPostConditionIfNotAlreadyInserted(
    TR::Register *vr, TR::RealRegister::RegNum rr, uint8_t flag) {
  return addPostConditionIfNotAlreadyInserted(
      vr, static_cast<TR::RealRegister::RegDep>(rr), flag);
}
...
bool OMR::Z::RegisterDependencyConditions::addPostConditionIfNotAlreadyInserted(
  ...
  int32_t pos = -1;
  if (_postConditions != NULL &&
      (pos = _postConditions->searchForRegisterPos(
           vr, RefsAndDefsDependentRegister, _addCursorForPost, _cg)) < 0) {
    if (!(vr->getRealRegister() &&
          vr->getRealRegister()->getState() == TR::RealRegister::Locked))
      addPostCondition(vr, rr, flag);
    return true;
  } else if (pos >= 0 &&
             _postConditions->getRegisterDependency(pos)->getFlags() != flag) {
