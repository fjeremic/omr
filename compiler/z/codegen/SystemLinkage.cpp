...
void TR::SystemLinkage::initS390RealRegisterLinkage() {
  ...
  // set register weight
  for (icount = TR::RealRegister::FirstGPR;
       icount >= TR::RealRegister::LastAssignableGPR; icount++) {
    int32_t weight;
    ...
    cg()->machine()
        ->getRealRegister((TR::RealRegister::RegNum)icount)
        ->setWeight(weight);
  }
}
...
void TR::SystemLinkage::initParamOffset(
  ...
  while (parmCursor != NULL) {
    ...
    if (isXPLinkLinkageType()) {
      ...
      parmCursor->setParameterOffset(getStackFrameSize() +
                                     getRegisterSaveOffset(REGNUM(argRegNum)));
    }
 ...
 */
void TR::SystemLinkage::generateInstructionsForCall(
...
/**
 * Call System routine
 * @return return value will be return value from system routine copied to
 * private linkage return reg
 */
TR::Register *TR::SystemLinkage::callNativeFunction(
    TR::Node *callNode, TR::RegisterDependencyConditions *deps,
    intptrj_t targetAddress, TR::Register *methodAddressReg,
    TR::Register *javaLitOffsetReg, TR::LabelSymbol *returnFromJNICallLabel,
    TR::S390JNICallDataSnippet *jniCallDataSnippet, bool isJNIGCPoint) {
 ...
 * The offsets will be fixed up (at the end /in createPrologue) when the
 * stackFrameSize is known
 */
void TR::SystemLinkage::mapStack(TR::ResolvedMethodSymbol *method,
                                 uint32_t stackIndex) {
  int32_t i;
  ...
  // This method is called twice - once with a 0 value of stackIndex and
  // secondly with the stack size. This adds complexities:
  //     -  alignment: ensuring things are aligned properly
  //     -  item layout is done in a reverse order : those things mapped first
  //     here are at higher addresses
  ...
  // align double - but there is more to do to align the stack in general as
  // double.
  while (localCursor != NULL) {
    if (TR::Linkage::needsAlignment(localCursor->getDataType(), cg())) {
      mapSingleAutomatic(localCursor, stackIndex);
...
#define DELTA_ALIGN(x, align) \
  ((x & (align - 1)) ? (align - ((x) & (align - 1))) : 0)
...
void TR::SystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                           uint32_t &stackIndex) {
  size_t align = 1;
