...
TR::S390zLinuxSystemLinkage::S390zLinuxSystemLinkage(TR::CodeGenerator *cg)
  ...
  setRegisterFlag(TR::RealRegister::GPR6,
                  IntegerArgumentAddToPost); // I only need to do this for r6
                                             // because r6 is the only preserved
                                             // linkage register on zlinux.
...
void TR::S390zLinuxSystemLinkage::initParamOffset(
  ...
  while (parmCursor != NULL) {
    ...
    if (parmCursor->getDataType() == TR::Aggregate) {
      TR_ASSERT(0, "should not reach here due to folding of "
    ...
    case TR::Int64:
      if (TR::Compiler->target.is32Bit()) {
        // make sure that we can fit the entire value in registers, not half in
        // registers half on the stack
...
TR::Instruction *
TR::S390zLinuxSystemLinkage::fillGPRsInEpilogue(TR::Node *node,
...
TR::Instruction *
TR::S390zLinuxSystemLinkage::fillFPRsInEpilogue(TR::Node *node,
                                                TR::Instruction *cursor) {
  TR::RealRegister *spReg = getNormalStackPointerRealRegister();
...
TR::Instruction *
TR::S390zLinuxSystemLinkage::spillGPRsInPrologue(TR::Node *node,
                                                 TR::Instruction *cursor) {
  int16_t GPRSaveMask = 0;
...
TR::Instruction *
TR::S390zLinuxSystemLinkage::spillFPRsInPrologue(TR::Node *node,
                                                 TR::Instruction *cursor) {
  TR::RealRegister *spReg = getNormalStackPointerRealRegister();
  int32_t offset = getFPRSaveAreaEndOffset();
  int16_t FPRSaveMask = getFPRSaveMask();
