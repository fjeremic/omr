  ...
  : TR::SystemLinkage(cg, TR_SystemLinux)
{
  ...
  setRegisterFlag(
    ...
    Preserved); // Setting the return address register as preserved so that way
                // we do not always have to emit stores in prologues / epilogues
                // This possibly assumes that we don't shuffle the return
                // address register around elsewhere
...
void
TR::S390zLinuxSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
  ...
  cursor = generateS390BranchInstruction(cg(),
                                         TR::InstOpCode::BCR,
                                         node,
                                         TR::InstOpCode::COND_BCR,
                                         getReturnAddressRealRegister(),
                                         cursor);
}

void
TR::S390zLinuxSystemLinkage::createPrologue(TR::Instruction* cursor)
{
  ...
  if (cg()->isLiteralPoolOnDemandOn() != true && firstSnippet != NULL)
    cursor =
      new (trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::LARL,
                                                  node,
                                                  getLitPoolRealRegister(),
                                                  firstSnippet,
                                                  cursor,
                                                  cg());
#endif
  ...
  if (largeStack) {
    ...
    cursor = generateS390ImmToRegister(
      cg(), node, tempReg, (intptr_t)(stackFrameSize * -1), cursor);
    cursor = generateRRInstruction(
      cg(), TR::InstOpCode::getAddRegOpCode(), node, spReg, tempReg, cursor);
  } else {
...
void
TR::S390zLinuxSystemLinkage::generateInstructionsForCall(
  TR::Node* callNode,
...
#if defined(JITTEST)
  {
    ...
    if (callNode->getOpCode().isIndirect()) {
      ...
      TR::Instruction* callInstr =
        new (trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::BRASL,
                                                    callNode,
                                                    fpReg,
                                                    callSymbol,
                                                    callSymRef,
 ...
 */
TR::Register*
TR::S390zLinuxSystemLinkage::callNativeFunction(
  TR::Node* callNode,
  TR::RegisterDependencyConditions* deps,
  intptrj_t targetAddress,
  TR::Register* methodAddressReg,
    ...
    case TR::lcalli: {
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateRSInstruction(
          cg(), TR::InstOpCode::SLLG, callNode, highReg, highReg, 32);
        generateRRInstruction(
          cg(), TR::InstOpCode::LR, callNode, highReg, lowReg);
 ...
 */
void
TR::S390zLinuxSystemLinkage::initParamOffset(
  ...
  List<TR::ParameterSymbol>* parameterList)
{
  ...
  while (parmCursor != NULL) {
    ...
    if (argRegNum == TR::RealRegister::NoReg) {
      ...
      stackIndex +=
        std::max<int32_t>(parmCursor->getSize(), cg()->machine()->getGPRSize());
    } else {
...
int32_t
TR::S390zLinuxSystemLinkage::getRegisterSaveOffset(
  ...
  for (i = lastReg = firstReg = TR::RealRegister::FirstGPR;
    ...
    if (getPreserved(REGNUM(i))) {
      ...
      if (!findStartReg &&
        ...
        if (comp()->getOption(TR_TraceCG))
          traceMsg(comp(),
                   "\tGenerating preserve stores from %d  to %d \n",
