  ...
  : TR::SystemLinkage(cg, TR_SystemLinux)
{
  ...
  setRegisterFlag(TR::RealRegister::GPR6, IntegerArgumentAddToPost); // I only need to do this for r6 because r6 is the
                                                                     // only preserved linkage register on zlinux.
  ...
  setRegisterFlag(
    TR::RealRegister::GPR14, Preserved); // Setting the return address register as preserved so that way we do not
                                         // always have to emit stores in prologues / epilogues This possibly assumes
                                         // that we don't shuffle the return address register around elsewhere
...
void TR::S390zLinuxSystemLinkage::createPrologue(TR::Instruction* cursor)
{
  ...
  if (largeStack) {
    ...
    cursor = generateRXInstruction(
      cg(), TR::InstOpCode::LA, node, spReg, generateS390MemoryReference(spReg, (stackFrameSize) * -1, cg()), cursor);
  }
...
void TR::S390zLinuxSystemLinkage::generateInstructionsForCall(TR::Node* callNode,
  TR::RegisterDependencyConditions* deps, intptrj_t targetAddress, TR::Register* methodAddressReg,
...
#if defined(JITTEST)
  {
    ...
    if (callNode->getOpCode().isIndirect()) {
      ...
      TR::Instruction* callInstr = new (trHeapMemory())
        TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, fpReg, callSymbol, callSymRef, codeGen);
    ...
    if (getPreserved(REGNUM(i))) {
      ...
      if (!findStartReg
        ...
        if (lastReg - firstReg == 0)
          cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getLoadOpCode(), node, getRealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
        else
  ...
  for (i = lastReg = firstReg = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR; ++i) {
    ...
    if (getPreserved(REGNUM(i))) {
      ...
      if (!findStartReg
        ...
        if (lastReg - firstReg == 0)
          cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getStoreOpCode(), node, getRealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
        else
