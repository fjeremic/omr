...
TR::S390zLinuxSystemLinkage::S390zLinuxSystemLinkage(TR::CodeGenerator* cg)
  ...
  setRegisterFlag(
      ...
      IntegerArgumentAddToPost);  // I only need to do this for r6 because r6 is
                                  // the only preserved linkage register on
                                  // zlinux.
  ...
  setRegisterFlag(
      ...
      Preserved);  // Setting the return address register as preserved so that
                   // way we do not always have to emit stores in prologues /
                   // epilogues This possibly assumes that we don't shuffle the
                   // return address register around elsewhere
  ...
  if (!comp()->getCurrentMethod()->isJ9()) {
    ...
    // For JIT to native transition, We will set up param4 and parm4 in GPR8 and
    // GPR9 during param evaluation and will move them to GPR5 and GPR6 just
    // before going into native code. This is needed for correct evaluation of
    // parms as GPR5 is java stack pointer and GPR6 may be Literal pool pointer
  ...
  if (cg->getSupportsVectorRegisters()) {
    ...
    // The vector registers used for parameter passing are allocated in the
    // following order to enable future exension which might operate on vector
    // register pairs
...
void TR::S390zLinuxSystemLinkage::createEpilogue(TR::Instruction* cursor) {
  ...
  cursor = generateS390BranchInstruction(
      cg(), TR::InstOpCode::BCR, node, TR::InstOpCode::COND_BCR,
      getReturnAddressRealRegister(), cursor);
}
...
void TR::S390zLinuxSystemLinkage::createPrologue(TR::Instruction* cursor) {
  ...
  size_t localSize =
      ((-1 * static_cast<int32_t>(bodySymbol->getLocalMappingCursor())) +
       (8 - 1)) &
      ~(8 - 1);
  setStackFrameSize(((getOffsetToFirstParm() + argSize + localSize) + (8 - 1)) &
                    ~(8 - 1));

  int32_t stackFrameSize = getStackFrameSize();

  TR_ASSERT_FATAL((stackFrameSize & 7) == 0,
  ...
  if (cg()->isLiteralPoolOnDemandOn() != true && firstSnippet != NULL)
    cursor = new (trHeapMemory()) TR::S390RILInstruction(
        TR::InstOpCode::LARL, node, getLitPoolRealRegister(), firstSnippet,
        cursor, cg());
#endif
  ...
  const bool largeStack =
      stackFrameSize < MIN_IMMEDIATE_VAL || stackFrameSize > MAX_IMMEDIATE_VAL;

  if (largeStack) {
    TR::RealRegister* tempReg = getRealRegister(TR::RealRegister::GPR0);

    cursor = generateS390ImmToRegister(cg(), node, tempReg,
                                       (intptr_t)(stackFrameSize * -1), cursor);
    cursor = generateRRInstruction(cg(), TR::InstOpCode::getAddRegOpCode(),
                                   node, spReg, tempReg, cursor);
  } else {
    cursor = generateRXInstruction(
        cg(), TR::InstOpCode::LA, node, spReg,
        generateS390MemoryReference(spReg, (stackFrameSize) * -1, cg()),
        cursor);
  }
...
void TR::S390zLinuxSystemLinkage::generateInstructionsForCall(
...
#if defined(JITTEST)
  {
    if (callNode->getOpCode().isIndirect()) {
      ...
      generateRRInstruction(codeGen, TR::InstOpCode::BASR, callNode,
                            systemReturnAddressRegister, targetReg, deps);
    } else {
      ...
      TR::Instruction* callInstr = new (trHeapMemory())
          TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, fpReg,
                                 callSymbol, callSymRef, codeGen);
 ...
 */
TR::Register* TR::S390zLinuxSystemLinkage::callNativeFunction(
    TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg,
    TR::Register* javaLitOffsetReg, TR::LabelSymbol* returnFromJNICallLabel,
    TR::S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint) {
  ...
  switch (callNode->getOpCodeValue()) {
    ...
    case TR::lcalli: {
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateRSInstruction(cg(), TR::InstOpCode::SLLG, callNode, highReg,
                              highReg, 32);
        generateRRInstruction(cg(), TR::InstOpCode::LR, callNode, highReg,
                              lowReg);
...
#endif
      returnRegister =
          deps->searchPostConditionRegister(getFloatReturnRegister());
      break;
#if defined(SUPPORT_DFP) && defined(J9_PROJECT_SPECIFIC)
    case TR::decall:
    case TR::decalli:
    ...
    case TR::vcalli:
      returnRegister =
          deps->searchPostConditionRegister(getVectorReturnRegister());
      break;
    default:
      returnRegister = NULL;
...
void TR::S390zLinuxSystemLinkage::initParamOffset(
    TR::ResolvedMethodSymbol* method, int32_t stackIndex,
  ...
  while (parmCursor != NULL) {
    ...
    if (parmCursor->getDataType() == TR::Aggregate) {
      TR_ASSERT(0,
                "should not reach here due to folding of "
                "getDataTypeOfAggregateMember()");
    ...
    switch (parmCursor->getDataType()) {
      case TR::Int64:
        if (TR::Compiler->target.is32Bit()) {
          // make sure that we can fit the entire value in registers, not half
          // in registers half on the stack
    ...
    if (argRegNum == TR::RealRegister::NoReg) {
      if ((isPadFloatParms() && parmCursor->getType().isFloatingPoint()) &&
          (gprSize > parmCursor->getSize()))
        parmCursor->setParameterOffset(stackIndex + gprSize -
                                       parmCursor->getSize());
      else
        parmCursor->setParameterOffset(stackIndex);
      stackIndex += std::max<int32_t>(parmCursor->getSize(),
                                      cg()->machine()->getGPRSize());
    } else {
      // Check where to locate the linkage register 6 which is preserved on
      // zLiux
      if ((getIntegerArgumentAddToPost(argRegNum) != 0 &&
           parmCursor->isParmHasToBeOnStack()) ||
        ...
        parmCursor->setParameterOffset(
            getStackFrameSize() + getRegisterSaveOffset(REGNUM(argRegNum)));
    ...
    {
      // linkage related
      parmCursor->setParameterOffset(parmCursor->getParameterOffset() +
                                     gprSize - parmCursor->getSize());
    }

    if (TR::Compiler->target.is64Bit() && parmCursor->getType().isAddress() &&
        (parmCursor->getSize() == 4)) {
      // This is a 31-bit pointer parameter. It's real location is +4 bytes from
      // the start of the 8-byte parm slot. Since ptr31 parms are passed as
      // 64-bit values, the prologue code has the spill the full 8-bytes into
      // the parm slot. However, the rest of the code will load/store this parm
      // as a 4-byte value, hence we have to add +4 here.
...
int32_t TR::S390zLinuxSystemLinkage::getRegisterSaveOffset(
  ...
  switch (srcReg) {
    ...
    case TR::RealRegister::GPR1:
      TR_ASSERT(false,
                "ERROR: TR::zLinuxSystemLinkage::getRegisterSaveOffset called "
                "for volatile reg: %d",
                srcReg);
      return -1;
    ...
    default:
      return gpr2Offset +
             (srcReg - TR::RealRegister::GPR2) * cg()->machine()->getGPRSize();
  }
}

TR::Instruction* TR::S390zLinuxSystemLinkage::fillGPRsInEpilogue(
  ...
  for (i = lastReg = firstReg = TR::RealRegister::FirstGPR;
    ...
    if (getPreserved(REGNUM(i))) {
      ...
      if (findStartReg) firstReg = static_cast<TR::RealRegister::RegNum>(i);

      if ((getRealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()) {
        if (comp()->getOption(TR_TraceCG)) traceMsg(comp(), "\tand Assigned. ");
        findStartReg = false;
      }
      ...
      if (!findStartReg &&
          (!(getRealRegister(REGNUM(i)))->getHasBeenAssignedInMethod() ||
           (i == TR::RealRegister::LastGPR &&
            firstReg != TR::RealRegister::LastGPR))) {
        // LastGPR is the stack pointer on zLinux which always be saved and
        // restored
        ...
        // traceMsg(comp(),"\tstackFrameSize = %d offset =
        // %d\n",stackFrameSize,offset);

        TR::MemoryReference* retAddrMemRef =
            generateS390MemoryReference(spReg, offset + stackFrameSize, cg());

        if (lastReg - firstReg == 0)
          cursor = generateRXInstruction(
              cg(), TR::InstOpCode::getLoadOpCode(), node,
              getRealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
        else
          cursor = generateRSInstruction(
              cg(), TR::InstOpCode::getLoadMultipleOpCode(), node,
              getRealRegister(REGNUM(firstReg)),
              getRealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);

        findStartReg = true;
      }

      if (comp()->getOption(TR_TraceCG)) traceMsg(comp(), "\n");
    }
  }
  ...
  if (!hasRestoreSP) {
    ...
    cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), node,
                                   getRealRegister(REGNUM(lastReg)),
                                   retAddrMemRef, cursor);
  }
  return cursor;
}

TR::Instruction* TR::S390zLinuxSystemLinkage::fillFPRsInEpilogue(
    TR::Node* node, TR::Instruction* cursor) {
  TR::RealRegister* spReg = getNormalStackPointerRealRegister();
...
TR::Instruction* TR::S390zLinuxSystemLinkage::spillGPRsInPrologue(
    TR::Node* node, TR::Instruction* cursor) {
  int16_t GPRSaveMask = 0;
      ...
      if (!findStartReg &&
        ...
        if (comp()->getOption(TR_TraceCG))
          traceMsg(comp(), "\tGenerating preserve stores from %d  to %d \n",
                   (lastReg - TR::RealRegister::FirstGPR),
                   firstReg - TR::RealRegister::FirstGPR);
        ...
        if (lastReg - firstReg == 0)
          cursor = generateRXInstruction(
              cg(), TR::InstOpCode::getStoreOpCode(), node,
              getRealRegister(REGNUM(firstReg)), retAddrMemRef, cursor);
        else
          cursor = generateRSInstruction(
...
TR::Instruction* TR::S390zLinuxSystemLinkage::spillFPRsInPrologue(
    TR::Node* node, TR::Instruction* cursor) {
  TR::RealRegister* spReg = getNormalStackPointerRealRegister();
  int32_t offset = getFPRSaveAreaEndOffset();
  int16_t FPRSaveMask = getFPRSaveMask();
