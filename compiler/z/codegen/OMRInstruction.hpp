...
#define printInstr(comp, instr)                                                \
  (comp->getDebug() ? comp->getDebug()->printInstruction((instr)) : (void)0)
...
protected:
  ...
  Instruction(TR::CodeGenerator *cg, TR::Instruction *precedingInstruction,
              TR::InstOpCode::Mnemonic op, TR::Node *n = 0);

  void initialize(TR::RegisterDependencyConditions *cond) {
    _conditions = cond;
  }
  void initialize(TR::Instruction *precedingInstruction = NULL,
                  bool instFlag = false,
                  TR::RegisterDependencyConditions *cond = NULL,
                  bool condFlag = false);
...
public:
  ...
  TR::RegisterDependencyConditions *getDependencyConditions() {
    return _conditions;
  }
  TR::RegisterDependencyConditions *
  setDependencyConditions(TR::RegisterDependencyConditions *cond);
  TR::RegisterDependencyConditions *
  setDependencyConditionsNoBookKeeping(TR::RegisterDependencyConditions *cond);
  TR::RegisterDependencyConditions *
  updateDependencyConditions(TR::RegisterDependencyConditions *cond);
  void
  resetDependencyConditions(TR::RegisterDependencyConditions *cond = NULL) {
    _conditions = cond;
  }
  ...
  void setupThrowsImplicitNullPointerException(TR::Node *n,
                                               TR::MemoryReference *mr);

  bool throwsImplicitNullPointerException() {
    return _flags.testAny(ThrowsImplicitNullPointerException);
  }
  void setThrowsImplicitNullPointerException();

  // TODO - set this flag whenever an instruction can cause an implicit
  // exception currently this is only implemented for null pointer exceptions,
  // but it should include others - such as divide by 0, etc.
  bool throwsImplicitException() {
    return _flags.testAny(ThrowsImplicitException);
  }
  ...
  bool matchesAnyRegister(TR::Register *reg, TR::Register *instReg1,
  ...
  virtual bool getTwoLocalLocalAllocFreeReg(TR::RealRegister **reg1,
                                            TR::RealRegister **reg2);

  virtual TR::RealRegister *getLocalLocalSpillReg1() {
    return _longDispSpillReg1;
  }
  virtual TR::RealRegister *setLocalLocalSpillReg1(TR::RealRegister *reg) {
    return _longDispSpillReg1 = reg;
  }

  virtual TR::RealRegister *getLocalLocalSpillReg2() {
    return _longDispSpillReg2;
  }
  virtual TR::RealRegister *setLocalLocalSpillReg2(TR::RealRegister *reg) {
    return _longDispSpillReg2 = reg;
  }

  virtual uint32_t getBinLocalFreeRegs() { return _binFreeRegs; }
  virtual uint32_t setBinLocalFreeRegs(uint32_t bin) {
    return _binFreeRegs = bin;
  }
  ...
  virtual bool isFloat() {
    return _opcode.singleFPOp() > 0 || _opcode.doubleFPOp() > 0;
  }
  ...
  virtual void setMaskField(uint32_t *instruction, int8_t mask,
                            int8_t nibbleIndex);
  ...
  /** @return true if reg matches any uses operand registers or implicitly used
   * real registers */
  virtual bool refsRegister(TR::Register *reg);
  /** @return ture if reg matches and result register */
  virtual bool defsRegister(TR::Register *reg);
  virtual bool
  defsAnyRegister(TR::Register *reg); // Includes looking into register pairs
  ...
  virtual TR::Register *getRegisterOperand(uint32_t i) {
    return (i > 0 && i <= _targetRegSize + _sourceRegSize)
               ? (TR::Register *)_operands[i - 1]
               : NULL;
  }
  virtual void setRegisterOperand(uint32_t i, TR::Register *newReg) {
    TR_ASSERT((i > 0 && i <= _targetRegSize + _sourceRegSize),
              "Register operand number %d does not exist.", i);
  ...
  /** The following safe virtual downcast method is used under debug only for
   * assertion checking */
...
private:
  ...
  enum // _flags
  {
    ...
    CCused = 0x0200, ///< CC set by current instruction is used by subsequent
                     ///< instructions.
    OutOfLineEX =
        0x0400, ///< TR::InstOpCode::EX instruction references a
                ///< ConstantInstructionSnippet object or an SSx instruction is
                ///< the target of an TR::InstOpCode::EX instruction
...
protected:
   ...
   *   Operands are TR::Register * or any derrived type, TR_MemoryReference * in
   * the order target registers, source registers, source mem, target mem
   *   Register operands fall into target or source depending how they are used.
   * If a register operand is set it will be in target category, if register is
   * set and used it will also be in target category but not in source. The
   * opcode property UsesTarget indicates that all target registers are used for
   * input. 390 Instructions have 0,1,2, or 3 register operands often refered in
   * manual as R1,R2,R3. These operands can be retreived using
   *     getRegisterOperand(i). Note that a compare or store instruction that
   * does not set operand 1 will put R1 into the sourceReg section of operands.
   *     This is not what it was like before. getTargetRegister used to fetch R1
   * and getSourceRegister used to fetch R2. These were not always meaning
   *     source and target so now these methods are disallowed. Register
   * operands can be register pairs for instructions that use an even-odd pair.
   */
  void **_operands;
  ...
  int8_t numOperands(void) {
    return _sourceRegSize + _targetRegSize + _sourceMemSize + _targetMemSize;
  }
  ...
  TR::Register **targetRegBase() {
    return (_targetRegSize != 0) ? (TR::Register **)_operands + _targetStart
                                 : NULL;
  }
  TR::Register **sourceRegBase() {
    return (_sourceRegSize != 0) ? ((TR::Register **)_operands) + _sourceStart
                                 : NULL;
  }
  TR::MemoryReference **sourceMemBase() {
    return (_sourceMemSize != 0) ? ((TR::MemoryReference **)_operands) +
                                       _targetRegSize + _sourceRegSize
                                 : NULL;
  }
  TR::MemoryReference **targetMemBase() {
    return (_targetMemSize != 0)
               ? ((TR::MemoryReference **)_operands) + _targetRegSize +
                     _sourceRegSize + _sourceMemSize
               : NULL;
  }
  ...
  private:
    // template <class AElementType, class Allocator = CS2::allocator, size_t
    // segmentBits = 8>
    typename CS2::ArrayOf<T, TR::Allocator, 3> _impl;

  public:
    explicit RegisterArray(TR::Compilation *c) : _impl(c->allocator()) {}
  ...
  public:
    explicit RegisterBitVector(TR::Compilation *c) : _impl(c->allocator()) {}
  ...
  int32_t gatherRegPairsAtFrontOfArray(TR::Register **regArr,
                                       int32_t sourceRegSize);
  TR::Register *assignRegisterNoDependencies(TR::Register *reg);
  void assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned);
  virtual void
  assignRegistersAndDependencies(TR_RegisterKinds kindToBeAssigned);

  void block(TR::Register **sourceReg, int32_t _sourceRegSize,
             TR::Register **targetReg, int targetRegSize,
             TR::MemoryReference **sourceMem, TR::MemoryReference **targetMem);
  void unblock(TR::Register **sourceReg, int32_t sourceRegSize,
               TR::Register **targetReg, int targetRegSize,
  ...
  uint32_t useTargetMemoryReference(TR::MemoryReference *memRef,
