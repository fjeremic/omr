...
public:
  ...
  int32_t getBlockIndex()
  {
    return _blockIndex;
  }
  void setBlockIndex(int32_t i)
  {
    _blockIndex = i;
  }
  ...
  bool isOutOfLineEX()
  {
    return _flags.testAny(OutOfLineEX);
  }
  void setOutOfLineEX()
  {
    _flags.set(OutOfLineEX);
  }
  ...
  void setThrowsImplicitException()
  {
    _flags.set(ThrowsImplicitException);
  }

  bool isExtDisp()
  {
    return _flags.testAny(ExtDisp);
  }
  void setExtDisp()
  {
    _flags.set(ExtDisp);
  ...
  bool isExtDisp2()
  {
    return _flags.testAny(ExtDisp2);
  }
  void setExtDisp2()
  {
    _flags.set(ExtDisp2);
  ...
  {
    return _flags.testAny(ExceptBranchOp);
  }
  void setExceptBranchOp()
  {
    _flags.set(ExceptBranchOp);
  }

  bool isCCuseKnown()
  {
    return _flags.testAny(CCuseKnown);
  }
  void setCCuseKnown()
  {
    _flags.set(CCuseKnown);
  }
  ...
  bool isDebugHookOp()
  {
    return _flags.testAny(DebugHookOp);
  }
  void setDebugHookOp()
  {
    _flags.set(DebugHookOp);
  }
  ...
  virtual uint32_t getBinLocalFreeRegs()
  {
    return _binFreeRegs;
  }
  ...
  virtual bool isLoad()
  {
    return _opcode.isLoad() > 0;
  }
  virtual bool isStore()
  {
    return _opcode.isStore() > 0;
  }
  virtual bool isBranchOp()
  {
    return _opcode.isBranchOp() > 0;
  }
  virtual bool isTrap()
  {
    return _opcode.isTrap() > 0;
  }
  virtual bool isLabel()
  {
    return _opcode.isLabel() > 0;
  }
  ...
  {
    return _opcode.isAdmin() > 0;
  }
  virtual bool isDebugFence()
  {
    return false;
  }
  ...
  virtual bool implicitlyUsesGPR0()
  {
    return _opcode.implicitlyUsesGPR0() > 0;
  }
  virtual bool implicitlyUsesGPR1()
  {
    return _opcode.implicitlyUsesGPR1() > 0;
  }
  virtual bool implicitlyUsesGPR2()
  {
    return _opcode.implicitlyUsesGPR2() > 0;
  }

  virtual bool implicitlySetsGPR0()
  {
    return _opcode.implicitlySetsGPR0() > 0;
  }
  virtual bool implicitlySetsGPR1()
  {
    return _opcode.implicitlySetsGPR1() > 0;
  }
  virtual bool implicitlySetsGPR2()
  {
    return _opcode.implicitlySetsGPR2() > 0;
  }

  bool isCompare()
  {
    return _opcode.isCompare() > 0;
  }
  bool fprOp()
  {
    return _opcode.fprOp() > 0;
  }
  ...
  uint32_t getNumRegisterOperands()
  {
    return _targetRegSize + _sourceRegSize;
  }
  ...
  bool isWillBePatched()
  {
    return (_index & WillBePatched) != 0;
  }
  void setWillBePatched()
  {
    _index |= WillBePatched;
  }
...
protected:
   ...
   * category but not in source. The opcode property UsesTarget indicates that all target registers are used for input.
   * 390 Instructions have 0,1,2, or 3 register operands often refered in manual as R1,R2,R3. These operands can be
   * retreived using getRegisterOperand(i). Note that a compare or store instruction that does not set operand 1 will
   * put R1 into the sourceReg section of operands. This is not what it was like before. getTargetRegister used to fetch
   * R1 and getSourceRegister used to fetch R2. These were not always meaning source and target so now these methods are
   * disallowed. Register operands can be register pairs for instructions that use an even-odd pair.
   */
  void** _operands;
  ...
  {
    return (_targetMemSize != 0) ? ((TR::MemoryReference**)_operands) + _targetRegSize + _sourceRegSize + _sourceMemSize
                                 : NULL;
  }
