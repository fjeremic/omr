...
public:
  ...
  static TR::Register *directCallEvaluator(TR::Node *node,
                                           TR::CodeGenerator *cg);
  static TR::Register *indirectCallEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg);
  ...
  static TR::Register *dualMulHelper64(TR::Node *node, TR::Node *lmulNode,
                                       TR::Node *lumulhNode,
                                       TR::CodeGenerator *cg);
  ...
  static TR::Register *integerRolEvaluator(TR::Node *node,
                                           TR::CodeGenerator *cg);
   ...
   *     lconst value is a sequence of contiguous 1's surrounded by 0's or vice
   * versa. For example, the lconst value can be: 00011111111000,
   * 1111000000011111, 0100000
   *
   ...
   *
   *     We can either materialize the constant in a register or use the
   * NIHF+NILF instructions to AND the most and least significant 32 bit
   * portions in the first operand. These are both more expensive than
   * generating a RISBG instruction.
   *
   *     This is because in the above case we can perform the AND operation with
   * the following single RISBG instruction:
   *
   ...
   *
   *    \param shiftAmount
   *      The number of bits we should shift the result of the logical AND. A
   * positive value represents a left shift, a negative value represents a right
   * shift, and a value of zero represents no shift. Note that if the absolute
   * value of number is greater than 6 bits, then this optimization will not
   * work since the RISBG instruction will only hold 6 bits for the shift
   * operand.
   *
   *   \param isSignedShift
   *      If replacing a signed shift+and combination, this variable evaluates
   * to true. Else it will evaluate to false. Note: If we are using this
   * function to replace a standalone 'and', then this variable should be false.
   *
   *   \return
  ...
  static TR::Register *iffcmpequEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *iffcmpneuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *iffcmpltuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *iffcmpgeuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *iffcmpgtuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *iffcmpleuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  ...
  static TR::Register *ifdcmpequEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifdcmpltuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifdcmpgeuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifdcmpgtuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifdcmpleuEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  ...
  static TR::Register *ifsucmpeqEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifsucmpltEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifsucmpgeEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifsucmpgtEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *ifsucmpleEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  ...
  static TR::InstOpCode::S390BranchCondition
  getBranchConditionFromCompareOpCode(TR::ILOpCodes opCode);
  static TR::InstOpCode::S390BranchCondition mapBranchConditionToLOCRCondition(
      TR::InstOpCode::S390BranchCondition incomingBc);
  static TR::InstOpCode::Mnemonic getCompareOpFromNode(TR::CodeGenerator *cg,
                                                       TR::Node *node);
  ...
  static bool treeContainsAllOtherUsesForNode(TR::Node *condition,
                                              TR::Node *trueVal);
  ...
  static TR::Register *barrierFenceEvaluator(TR::Node *node,
                                             TR::CodeGenerator *cg);
  static TR::Register *lookupEvaluator(TR::Node *node, TR::CodeGenerator *cg);
  static void tableEvaluatorCaseLabelHelper(
  ...
  static TR::Register *resolveAndNULLCHKEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg);
  ...
  static TR::Register *vRegStoreEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  ...
  static TR::Register *vDivOrRemHelper(TR::Node *node, TR::CodeGenerator *cg,
                                       bool isDivision);
  ...
  static TR::Register *vectorElementShiftHelper(TR::Node *node,
                                                TR::CodeGenerator *cg);
  ...
  static TR::Register *inlineVectorUnaryOp(TR::Node *node,
                                           TR::CodeGenerator *cg,
                                           TR::InstOpCode::Mnemonic op);
  static TR::Register *inlineVectorBinaryOp(TR::Node *node,
                                            TR::CodeGenerator *cg,
                                            TR::InstOpCode::Mnemonic op);
  static TR::Register *inlineVectorTernaryOp(TR::Node *node,
                                             TR::CodeGenerator *cg,
                                             TR::InstOpCode::Mnemonic op);

  static TR::Register *bitpermuteEvaluator(TR::Node *node,
                                           TR::CodeGenerator *cg);

  static TR::Register *tryToReuseInputVectorRegs(TR::Node *node,
                                                 TR::CodeGenerator *cg);
  ...
  static void createDAACondDeps(TR::Node *node,
                                TR::RegisterDependencyConditions *daaDeps,
                                TR::Instruction *daaInstr,
                                TR::CodeGenerator *cg);
  static void addToRegDep(TR::RegisterDependencyConditions *daaDeps,
                          TR::Register *reg, bool mightUseRegPair);

  static TR::Register *passThroughEvaluator(TR::Node *node,
                                            TR::CodeGenerator *cg);

  /** \brief
   *     Evaluates a primitive or reference arraycopy node. A primitive
   * arraycopy has 3 children and a reference arraycopy has 5 children.
   *
   *     The z Systems code generator requires that <c>isForwardArrayCopy()</c>
   * really means that a forward array copy is safe and that if it is not set it
   * really means that a backward array copy is safe. The optimizer should have
   * generated the correct tests around the arraycopy to enforce this
   * assumption.
   *
   *     Furthermore note that the z Systems code generator does not need to
   * check for a particular frequency on the length, again because the optimizer
   * will have generated a test for a particular length, if one is quite common,
   * and there will be two versions of the arraycopy call; one with a constant
   * length and one with a variable length.
   *
   *  \param node
   *     The arraycopy node.
   *
   ...
   *     <c>NULL</c>
   */
  static TR::Register *arraycopyEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);

  /** \brief
   *     Evaluates a primitive arraycopy node by generating an MVC memory-memory
   copy for a forward arraycopy and a
   ...
   *     The number of bytes to copy.
   */
  static void primitiveArraycopyEvaluator(TR::Node *node, TR::CodeGenerator *cg,
                                          TR::Node *byteSrcNode,
                                          TR::Node *byteDstNode,
                                          TR::Node *byteLenNode);

  /** \brief
   *  Generates Load/Store sequence for array copy as per type of array copy
   * element
   *
   * \param node
   *     The arraycopy node.
   ...
   *
   *  \param deps
   *     Register dependency conditions of the external ICF. Load-and-store for
   * array copy itself is usually a load-store instruction pair without internal
   * control flows. But project specific implementations could extend this and
   * construct an ICF. Should an ICF be constructed, the inner ICF will use the
   * outer ICF's dependencies and build upon it.
   *
   */
  static void generateLoadAndStoreForArrayCopy(
  ...
  /** \brief
   *  Generate sequence for forward array copy using MVC memory-memory copy
   * instruction
   *
   *  \param node
   *     The arraycopy node.
   ...
   */
  static void forwardArrayCopySequenceGenerator(
  ...
  static void genLoopForwardArrayCopy(TR::Node *node, TR::CodeGenerator *cg,
                                      TR::Register *byteSrcReg,
                                      TR::Register *byteDstReg,
                                      TR::Register *loopIterReg,
                                      bool isConstantLength = false);

  /** \brief
   *  Generates Element wise Memory To Memory copy sequence in forward/backward
   * direction
   *
   * \param node
   *     The arraycopy node.
   ...
   *
   *  \param deps
   *     Register dependency conditions of the external ICF this mem-to-mem copy
   * resides. Mem-to-Mem element copy itself is a loop of load-store
   * instructions that has its own ICF. If this ICF resides in another ICF, the
   * inner ICF will use the outer ICF's dependencies.
   *
   *  \return
   *     Register depdendecy conditions containg registers allocated within
   * Internal Control Flow
   *
   */
  static TR::RegisterDependencyConditions *generateMemToMemElementCopy(
      TR::Node *node, TR::CodeGenerator *cg, TR::Register *byteSrcReg,
      TR::Register *byteDstReg, TR::Register *byteLenReg,
      TR_S390ScratchRegisterManager *srm, bool isForward, bool needsGuardedLoad,
      bool genStartICFLabel = false,
      TR::RegisterDependencyConditions *deps = NULL);
   ...
   * Internal Control Flow
   */
  static TR::RegisterDependencyConditions *backwardArrayCopySequenceGenerator(
      TR::Node *node, TR::CodeGenerator *cg, TR::Register *byteSrcReg,
      TR::Register *byteDstReg, TR::Register *byteLenReg, TR::Node *byteLenNode,
      TR_S390ScratchRegisterManager *srm, TR::LabelSymbol *mergeLabel);
  ...
  static TR::Register *evaluateLengthMinusOneForMemoryCmp(TR::Node *node,
                                                          TR::CodeGenerator *cg,
                                                          bool clobberEvaluate,
                                                          bool &lenMinusOne);

  static TR::Register *inlineIfArraycmpEvaluator(TR::Node *node,
                                                 TR::CodeGenerator *cg,
                                                 bool &inlined);

  static TR::Register *
  arraycmpHelper(TR::Node *node, TR::CodeGenerator *cg, bool isWideChar,
                 bool isEqualCmp = false, int32_t cmpValue = 0,
                 TR::LabelSymbol *compareTarget = NULL,
                 TR::Node *ificmpNode = NULL, bool needResultReg = true,
  ...
  static TR::Register *arraytranslateEvaluator(TR::Node *node,
                                               TR::CodeGenerator *cg);
  static TR::Register *
  arraytranslateEncodeSIMDEvaluator(TR::Node *node, TR::CodeGenerator *cg,
                                    ArrayTranslateFlavor convType);
  static TR::Register *
  arraytranslateDecodeSIMDEvaluator(TR::Node *node, TR::CodeGenerator *cg,
                                    ArrayTranslateFlavor convType);
  static TR::Register *arraytranslateAndTestEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg);
  static TR::Register *long2StringEvaluator(TR::Node *node,
                                            TR::CodeGenerator *cg);
  ...
  static TR::Register *arraycmpEvaluatorWithPad(TR::Node *node,
                                                TR::CodeGenerator *cg);
  ...
  static TR::Register *iRegStoreEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *fRegStoreEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *dRegStoreEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *lRegStoreEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  static TR::Register *GlRegDepsEvaluator(TR::Node *node,
                                          TR::CodeGenerator *cg);
  ...
  static TR::Register *
  evaluateNULLCHKWithPossibleResolve(TR::Node *node, bool needsResolve,
                                     TR::CodeGenerator *cg);

  static TR::Register *PrefetchEvaluator(TR::Node *node, TR::CodeGenerator *cg);

  static TR::Register *loadAutoOffsetEvaluator(TR::Node *node,
                                               TR::CodeGenerator *cg);

  static TR::Register *trtEvaluator(TR::Node *node, TR::CodeGenerator *cg);

  static TR::Register *integerHighestOneBit(TR::Node *node,
                                            TR::CodeGenerator *cg);
  static TR::Register *integerNumberOfLeadingZeros(TR::Node *node,
                                                   TR::CodeGenerator *cg);
  static TR::Register *integerNumberOfTrailingZeros(TR::Node *node,
                                                    TR::CodeGenerator *cg);
  ...
  static TR::Register *longNumberOfLeadingZeros(TR::Node *node,
                                                TR::CodeGenerator *cg);
  static TR::Register *longNumberOfTrailingZeros(TR::Node *node,
                                                 TR::CodeGenerator *cg);
  ...
  static TR::Register *inlineIfButestEvaluator(TR::Node *node,
                                               TR::CodeGenerator *cg,
                                               TR::Node *cmpOpNode,
                                               TR::Node *cmpValNode);
  static TR::Register *inlineIfBifEvaluator(TR::Node *ifNode,
                                            TR::CodeGenerator *cg,
                                            TR::Node *cmpOpNode,
                                            TR::Node *cmpValNode);
  ...
  static TR::Register *performCall(TR::Node *node, bool isIndirect,
                                   TR::CodeGenerator *cg);

  static TR::Instruction *genLoadForObjectHeaders(TR::CodeGenerator *cg,
                                                  TR::Node *node,
                                                  TR::Register *reg,
                                                  TR::MemoryReference *tempMR,
                                                  TR::Instruction *iCursor);
  static TR::Instruction *
  genLoadForObjectHeadersMasked(TR::CodeGenerator *cg, TR::Node *node,
                                TR::Register *reg, TR::MemoryReference *tempMR,
                                TR::Instruction *iCursor);

  static TR::Register *inlineIfTestDataClassHelper(TR::Node *node,
                                                   TR::CodeGenerator *cg,
                                                   bool &inlined);
  static TR::Register *vsplatsEvaluator(TR::Node *node, TR::CodeGenerator *cg);

  static TR::Register *z990PopCountHelper(TR::Node *node, TR::CodeGenerator *cg,
                                          TR::Register *inReg,
                                          TR::Register *outReg);

  static TR::Register *getConditionCodeOrFold(TR::Node *node,
                                              TR::CodeGenerator *cg,
                                              TR_FoldType foldType,
                                              TR::Node *parent);
  static TR::Register *getConditionCodeOrFoldIf(TR::Node *node,
                                                TR::CodeGenerator *cg,
                                                bool isFoldedIf,
                                                TR::Node *ificmpNode);
...
private:
  /** \brief
   *     Inlines an intrinsic for calls to atomicAddSymbol which are represented
   * by a call node of the form for 32-bit (64-bit similar):
   *
   *     \code
   ...
   */
  static TR::Register *intrinsicAtomicAdd(TR::Node *node,
                                          TR::CodeGenerator *cg);

  /** \brief
   *     Inlines an intrinsic for calls to atomicFetchAndAddSymbol which are
   * represented by a call node of the form for 32-bit (64-bit similar):
   *
   *     \code
   ...
   *
   *  \return
   *     A register holding the original value in memory (before the addition)
   * at the <address> location.
   */
  static TR::Register *intrinsicAtomicFetchAndAdd(TR::Node *node,
                                                  TR::CodeGenerator *cg);

  /** \brief
   *     Inlines an intrinsic for calls to atomicSwapSymbol which are
   * represented by a call node of the form for 32-bit (64-bit similar):
   *
   *     \code
   ...
   *
   *  \return
   *     A register holding the original value in memory (before the swap) at
   * the <address> location.
   */
  static TR::Register *intrinsicAtomicSwap(TR::Node *node,
                                           TR::CodeGenerator *cg);

  static void commonButestEvaluator(TR::Node *node, TR::CodeGenerator *cg);
  static TR::Register *ifFoldingHelper(TR::Node *node, TR::CodeGenerator *cg,
                                       bool &handledBIF);
};
