...
OMR::Z::RealRegister::RealRegister(TR::CodeGenerator* cg)
    : OMR::RealRegister(cg, NoReg), _modified(0) {}

OMR::Z::RealRegister::RealRegister(TR_RegisterKinds rk,
                                   uint16_t w,
                                   RegState s,
                                   RegNum rn,
                                   RegMask m,
                                   TR::CodeGenerator* cg)
    : OMR::RealRegister(rk, w, s, (uint16_t)rn, rn, m, cg), _modified(0) {}
...
void OMR::Z::RealRegister::setRegister1Field(uint32_t* instruction) {
  self()->setRegisterField(instruction);
}

void OMR::Z::RealRegister::setRegister2Field(uint32_t* instruction) {
  self()->setIndexRegisterField(instruction);
}

void OMR::Z::RealRegister::setRegister3Field(uint32_t* instruction) {
 ...
 *
 * Only VRF Register can set this RXB field. When a vector register acts as
 * operands, its register binary encoding's most significant bit is stored in 1
 * of 4 RXB bits.
 *
 * The RXB field is in the lower 4 bits of fifth byte (i.e. bit 36-39 of an
 * instructions)
 *
 */
void OMR::Z::RealRegister::setRegisterRXBField(uint32_t* instruction,
                                               RegNum reg,
                                               int index) {
  ...
  uint32_t RXB =
      boi((_fullRegBinaryEncodings[reg] & 0x10) >> 4 << (4 - index) << 24);
  *(instruction + 1) |= RXB;
}
 ...
 * The 'base register field' corresponds to bit 16-19 of an instruction, which
 * is where the 3rd field for register operands
 *
...
void OMR::Z::RealRegister::setBaseRegisterField(uint32_t* instruction,
                                                RegNum reg) {
  *instruction &=
      boi(0xFFFF0FFF);  // clear out bit 16-19 of the instruction memory
  *instruction |= boi((_fullRegBinaryEncodings[reg] & 0x0f) << 12);
  if (TR::RealRegister::isVRF(reg))
    TR::RealRegister::setRegisterRXBField(
        instruction, reg, 3);  // base reg is in the 3rd register field
}
...
void OMR::Z::RealRegister::setIndexRegisterField(uint32_t* instruction,
                                                 RegNum reg) {
  *instruction &=
      boi(0xFFF0FFFF);  // clear out bit 12-15 of the instruction memory
  *instruction |= boi((_fullRegBinaryEncodings[reg] & 0x0f) << 16);
  if (TR::RealRegister::isVRF(reg))
    TR::RealRegister::setRegisterRXBField(
        instruction, reg, 2);  // index reg is in the 2nd register field
}

/** \details
 *
 * Static method that sets register field
 * The register field is bit 8-11 of an instruction, which is where the 1st
 * field for register operands
 *
...
void OMR::Z::RealRegister::setRegisterField(uint32_t* instruction, RegNum reg) {
  *instruction &=
      boi(0xFF0FFFFF);  // clear out bit 8-11 of the instruction memory
  *instruction |= boi((_fullRegBinaryEncodings[reg] & 0x0f) << 20);
  if (TR::RealRegister::isVRF(reg))
    TR::RealRegister::setRegisterRXBField(instruction, reg,
                                          1);  // 1st register field
}
...
/** \details
 *
 * Static method that sets register2 field (also refered to as the index
 * register)
 */
void OMR::Z::RealRegister::setRegister2Field(uint32_t* instruction,
                                             RegNum reg) {
...
/** \details
 *
 * Static method that sets register3 field, which is also refered to as the base
 * register at bit 16-19
 */
void OMR::Z::RealRegister::setRegister3Field(uint32_t* instruction,
                                             RegNum reg) {
 ...
 * The 4th reg field is in the 1st 4-bit of the 3rd halfword (bit 32-35) of the
 * instruction
 *
 */
void OMR::Z::RealRegister::setRegister4Field(uint32_t* instruction,
                                             RegNum reg) {
  TR_ASSERT(TR::RealRegister::isVRF(reg),
            "setRegister4Field is only for vector registers");
  TR::RealRegister::setRegisterField(
      instruction + 1, 7, reg);  // the 4th reg field is in the 1st 4-bit of the
                                 // 3rd halfword of the instuction
  TR::RealRegister::setRegisterRXBField(instruction, reg, 4);
}

// static method
bool OMR::Z::RealRegister::isPseudoRealReg(RegNum reg) {
  return (reg < FirstGPR || reg > LastVRF);
}

// static method
bool OMR::Z::RealRegister::isRealReg(RegNum reg) {
  return (reg >= FirstGPR && reg <= LastVRF);
}
...
const uint8_t OMR::Z::RealRegister::_fullRegBinaryEncodings
