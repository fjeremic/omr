...
OMR::Z::TreeEvaluator::returnEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getOpCode().isJumpWithMultipleTargets() && (node->getNumChildren() > 1) && node->getChild(node->getNumChildren() - 1)->getOpCodeValue() == TR::GlRegDeps) {
...
{
    return node->getOpCode().isIf() && node->getFirstChild()->getOpCodeValue() == TR::butest && node->getFirstChild()->isSingleRefUnevaluated() && node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getType().isInt32();
}

bool OMR::Z::TreeEvaluator::isCandidateForCompareEvaluation(TR::Node* node)
{
    return node->getOpCode().isIf() && TR::TreeEvaluator::isSingleRefUnevalAndCompareOrBu2iOverCompare(node->getFirstChild()) && node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getType().isInt32();
}
...
static inline void generateMergedGuardCodeIfNeeded(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
        ...
        if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
            ...
            if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifacmpeq || node->getOpCodeValue() == TR::iflcmpeq)
...
#ifdef J9_PROJECT_SPECIFIC
    if ((firstChild->getOpCodeValue() == TR:: instanceof) && !(comp->getOption(TR_OptimizeForSpace) || debug("noInlineIfInstanceOf")) && (firstChild->getRegister() == NULL) && (node->getReferenceCount() <= 1) && secondChild->getOpCode().isLoadConst() && (((secondChild->getInt() == 0 || secondChild->getInt() == 1)))) {
...
#ifdef J9_PROJECT_SPECIFIC
    if (cg->profiledPointersRequireRelocation() && node->isProfiledGuard() && secondChild->getOpCodeValue() == TR::aconst && (secondChild->isClassPointerConstant() || secondChild->isMethodPointerConstant())) {
        TR_VirtualGuard* virtualGuard = comp->findVirtualGuardInfo(node);
...
OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (node->getOpCodeValue() == TR::icmpeq || node->getOpCodeValue() == TR::acmpeq) {
        ...
        if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
            TR::Node* firstChild = node->getFirstChild();
            TR::Node* secondChild = node->getSecondChild();
            if (node->getOpCodeValue() != TR::acmpeq && TR::Compiler->target.is64Bit() && firstChild->getOpCodeValue() == TR::iand && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && secondChild->getOpCodeValue() == TR::iconst && secondChild->getInt() == 0 && performTransformation(cg->comp(), "O^O CODE GENERATION:  ===>   Use RXSBG to perform icmpeq  <==\n")) {
...
OMR::Z::TreeEvaluator::icmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->getOpCode().isUnsignedCompare() && firstChild->getReferenceCount() == 1 && firstChild->isNonNegative() && secondChild->isNonNegative() && performTransformation(cg->comp(), "O^O CODE GENERATION:  ===>   Use SR/SRL to perform icmplt  <==\n")) {
...
OMR::Z::TreeEvaluator::icmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->getOpCode().isUnsignedCompare() && secondChild->getReferenceCount() == 1 && firstChild->isNonNegative() && secondChild->isNonNegative() && performTransformation(cg->comp(), "O^O CODE GENERATION:  ===>   Use SR/SRL to perform icmpgt  <==\n")) {
...
OMR::Z::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (upperBound < node->getNumChildren() && (node->getChild(upperBound)->getOpCodeValue() == TR::loadaddr || node->getChild(upperBound)->getOpCodeValue() == TR::aload || node->getChild(upperBound)->getOpCodeValue() == TR::aRegLoad)) {
...
TR::Register* OMR::Z::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(TR::Node* node, bool needsResolve, TR::CodeGenerator* cg)
{
    ...
    if (cg->getSupportsImplicitNullChecks() && !firstChild->isUnneededIALoad()) {
        if (opCode.isLoadVar() || (TR::Compiler->target.is64Bit() && opCode.getOpCodeValue() == TR::l2i) || (hasCompressedPointers && firstChild->getFirstChild()->getOpCode().getOpCodeValue() == TR::i2l)) {
            ...
            else if (symRef && (symRef->getSymbol()->getOffset() + symRef->getOffset()) < cg->getNumberBytesReadInaccessible()) {
                ...
                if (n->getReferenceCount() == 2 && nextTreeTop) {
                    ...
                    if (nextTopNode) {
                        if (nextTopNode->getOpCode().isBndCheck()) {
                            if ((nextTopNode->getOpCode().isSpineCheck() && (nextTopNode->getChild(2) == n)) || (!nextTopNode->getOpCode().isSpineCheck() && (nextTopNode->getFirstChild() == n))) {
                                needLateEvaluation = false;
                                nextTopNode->setHasFoldedImplicitNULLCHK(true);
                                traceMsg(comp, "\nMerging NULLCHK [%p] and BNDCHK [%p] of load child [%p]", node, nextTopNode, n);
                            }
                        } else if (nextTopNode->getOpCode().isIf() && nextTopNode->isNonoverriddenGuard() && nextTopNode->getFirstChild() == firstChild) {
                            needLateEvaluation = false;
            ...
            if (n->getOpCode().hasSymbolReference() && symRef->getSymbol()->getOffset() + symRef->getOffset() < cg->getNumberBytesWriteInaccessible()) {
                needExplicitCheck = false;
            }
        } else if (opCode.isCall() && opCode.isIndirect() && cg->getNumberBytesReadInaccessible() > TR::Compiler->om.offsetOfObjectVftField()) {
            needExplicitCheck = false;
        } else if (opCode.getOpCodeValue() == TR::iushr && cg->getNumberBytesReadInaccessible() > cg->fe()->getOffsetOfContiguousArraySizeField()) {
            ...
            if (cg->getImplicitExceptionPoint() && cg->getNumberBytesReadInaccessible() > cg->fe()->getOffsetOfContiguousArraySizeField()) {
                needExplicitCheck = false;
    ...
    if (needExplicitCheck) {
        ...
        if (cg->getHasResumableTrapHandler()) {
            ...
            // i.e.  o.f == NULL
            if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) && reference->getOpCode().isLoadVar() && (reference->getOpCodeValue() != TR::ardbari) && reference->getRegister() == NULL) {
            ...
            } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) && reference->getRegister() == NULL && comp->useCompressedPointers() && reference->getOpCodeValue() == TR::l2a && reference->getFirstChild()->getOpCodeValue() == TR::iu2l && reference->getFirstChild()->getFirstChild()->getOpCode().isLoadVar() && TR::Compiler->om.compressedReferenceShiftOffset() == 0) {
            ...
            if (!firstChild->getOpCode().isCall() && reference->getOpCode().isLoadVar() && (reference->getOpCodeValue() != TR::ardbari) && // ardbari needs to be evaluated before being NULLCHK'ed because of its side effect.
...
OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::TreeEvaluator::treeContainsAllOtherUsesForNode(condition, trueVal) && (!trueVal->isUnneededConversion() || TR::TreeEvaluator::treeContainsAllOtherUsesForNode(condition, trueVal->getFirstChild()))) {
        if (comp->getOption(TR_TraceCG))
    ...
    if (condition->getOpCode().isBooleanCompare() && (condition->getFirstChild()->getType().isIntegral() || condition->getFirstChild()->getType().isAddress())) {
        if (comp->getOption(TR_TraceCG))
