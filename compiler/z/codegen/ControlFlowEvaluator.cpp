...
extern TR::Register *
iDivRemGenericEvaluator(TR::Node *node, TR::CodeGenerator *cg, bool isDivision,
...
generateS390Compare(TR::Node *node, TR::CodeGenerator *cg,
                    TR::InstOpCode::Mnemonic branchOp,
                    TR::InstOpCode::S390BranchCondition fBranchOpCond,
                    TR::InstOpCode::S390BranchCondition rBranchOpCond) {
  // Generate compare code, find out if ops were reversed
...
TR::Register *OMR::Z::TreeEvaluator::returnEvaluator(TR::Node *node,
  ...
  case TR::ireturn:
    ...
    if (linkage->isNeedsWidening())
      new (cg->trHeapMemory()) TR::S390RRInstruction(
          TR::InstOpCode::LGFR, node, returnValRegister, returnValRegister, cg);
    break;
  case TR::lreturn:
    ...
    if (virtualGuard && (virtualGuard->mergedWithOSRGuard() ||
      ...
      if (instr && instr->getNode() == node && instr->getDependencyConditions())
        mergedGuardDeps = new (cg->trHeapMemory())
            TR::RegisterDependencyConditions(instr->getDependencyConditions(),
                                             1, 0, cg);
  ...
  if (node->getOpCodeValue() == TR::icmpeq ||
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
      ...
      if (node->getOpCodeValue() != TR::acmpeq &&
          ...
          performTransformation(cg->comp(), "O^O CODE GENERATION:  ===>   Use "
                                            "RXSBG to perform icmpeq  <==\n")) {
  ...
  case AddressTable32bit: {
    if (!node->chkCannotOverflow())
      generateS390CompareAndBranchInstruction(
          ...
          node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
          false, false); // make sure the case selector is within range of our
                         // case constants
...
TR::Register *OMR::Z::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node,
  ...
  TR_S390OutOfLineCodeSection *outlinedHelperCall = new (cg->trHeapMemory())
      TR_S390OutOfLineCodeSection(node, TR::call, NULL, slowPathOOLLabel,
                                  doneLabel, cg);
 ...
 */
TR::Register *
OMR::Z::TreeEvaluator::resolveAndNULLCHKEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
...
void OMR::Z::TreeEvaluator::createDAACondDeps(
  ...
  case TR::Instruction::IsRX: // CVB, CVD
  {
    ...
    if (mr) {
      ...
      TR::TreeEvaluator::addToRegDep(
          daaDeps, static_cast<TR::Register *>(mr->getIndexRegister()), false);
    }
    break;
  }
  ...
  case TR::Instruction::IsSS2: // ZAP, Arithmetics
  {
    ...
    if (mr1) {
      TR::TreeEvaluator::addToRegDep(
          daaDeps, static_cast<TR::Register *>(mr1->getBaseRegister()), false);
      TR::TreeEvaluator::addToRegDep(
          daaDeps, static_cast<TR::Register *>(mr1->getIndexRegister()), false);
    }

    if (mr2) {
      TR::TreeEvaluator::addToRegDep(
          daaDeps, static_cast<TR::Register *>(mr2->getBaseRegister()), false);
      TR::TreeEvaluator::addToRegDep(
          daaDeps, static_cast<TR::Register *>(mr2->getIndexRegister()), false);
    }
    break;
  }
...
TR::Register *OMR::Z::TreeEvaluator::inlineIfButestEvaluator(
    ...
    TR::Node *cmpValNode) {
  TR_ASSERT(cmpValNode->getOpCode().isLoadConst() &&
                cmpValNode->getType().isInt32(),
...
OMR::Z::TreeEvaluator::getCompareOpFromNode(TR::CodeGenerator *cg,
                                            TR::Node *node) {
  TR_ASSERT(
