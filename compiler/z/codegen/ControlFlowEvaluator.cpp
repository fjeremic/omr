...
static bool virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (comp->compileRelocatableCode()) {
    ...
    default:
      TR_ASSERT_FATAL(0, "got AOT guard in node but virtual guard not one of known guards supported for AOT. Guard: %d",
...
static TR::Register* maxMinHelper(TR::Node* node, TR::CodeGenerator* cg, bool isMax)
{
  ...
  if (node->getOpCodeValue() == TR::imax || node->getOpCodeValue() == TR::imin) {
    ...
    TR_ASSERT_FATAL(
      node->getOpCodeValue(), "Opcode %s cannot be evaluated by maxMinHelper\n", node->getOpCode().getName());
  }
...
TR::Register* OMR::Z::TreeEvaluator::maxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return maxMinHelper(node, cg, true);
}

TR::Register* OMR::Z::TreeEvaluator::minEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::branchEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return NULL;
}
TR::Register* OMR::Z::TreeEvaluator::ibranchEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return NULL;
}
TR::Register* OMR::Z::TreeEvaluator::mbranchEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return NULL;
}
...
{
  return node->getOpCode().isIf() && node->getFirstChild()->getOpCodeValue() == TR::butest
    && node->getFirstChild()->isSingleRefUnevaluated() && node->getSecondChild()->getOpCode().isLoadConst()
  ...
  if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
    ...
    if (virtualGuard && (virtualGuard->mergedWithOSRGuard() || virtualGuard->mergedWithHCRGuard())) {
      ...
      if (instr && instr->getNode() == node && instr->getDependencyConditions())
        mergedGuardDeps
          = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(instr->getDependencyConditions(), 1, 0, cg);
...

      {
        ...
        TR::Instruction* vgnopInstr
          = generateVirtualGuardNOPInstruction(cg, node, site, deps, fallThroughLabel, instr ? instr->getPrev() : NULL);
        ...
        TR::Instruction* vgnopInstr
          = generateVirtualGuardNOPInstruction(cg, node, site, mergedGuardDeps, label, instr ? instr->getPrev() : NULL);
        vgnopInstr->setNext(instr);
        cg->setAppendInstruction(instr);
      }
...
TR::Register* OMR::Z::TreeEvaluator::ificmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if ((firstChild->getOpCodeValue() == TR:: instanceof)
    && !(comp->getOption(TR_OptimizeForSpace) || debug("noInlineIfInstanceOf")) && (firstChild->getRegister() == NULL)
    && (node->getReferenceCount() <= 1) && secondChild->getOpCode().isLoadConst()
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::ifsucmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (node->getOpCodeValue() == TR::icmpeq || node->getOpCodeValue() == TR::acmpeq) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
      TR::Node* firstChild = node->getFirstChild();
      TR::Node* secondChild = node->getSecondChild();
      if (node->getOpCodeValue() != TR::acmpeq && TR::Compiler->target.is64Bit()
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::sucmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return generateS390CompareBool(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE);
}
  ...
  TR::Register* branchTableReg, TR::Register* reg1)
{
  TR::Compilation* comp = cg->comp();
  {
    ...
    new (cg->trHeapMemory()) TR::S390RIInstruction(
      TR::InstOpCode::BRAS, node, reg1, (4 + (sizeof(uintptrj_t) * numBranchTableEntries)) / 2, cg);
...
TR::Register* OMR::Z::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  /*
   * High level idea -- there are two modes, a relative table, where we load a relative offset from the table and branch
   * to it, and an address table, where we load an address out of the table and branch to it since cases are indexed by
   * 1, we need to shift by either 4 or 8 to get the next value in memory.
   */
  ...
  TR::Node* node, bool needsResolve, TR::CodeGenerator* cg)
{
  ...
  if (needExplicitCheck) {
    ...
    if (cg->getHasResumableTrapHandler()) {
      ...
      // i.e.  o.f == NULL
      if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) && reference->getOpCode().isLoadVar()
      ...
      } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) && reference->getRegister() == NULL
        ...
        if (appendTo->getOpCodeValue() == TR::InstOpCode::LLGF) {
          ...
          appendTo = generateRIEInstruction(cg, TR::InstOpCode::getCmpImmTrapOpCode(), node, targetRegister, (int16_t)0,
            TR::InstOpCode::COND_BE, appendTo);
        }
      } else {
      ...
      if (!firstChild->getOpCode().isCall() && reference->getOpCode().isLoadVar()
        ...
        && // ardbari needs to be evaluated before being NULLCHK'ed because of its side effect.
        ...
        appendTo
          = generateRXInstruction(cg, TR::InstOpCode::getLoadTestOpCode(), reference, targetRegister, tempMR, appendTo);
        tempMR->stopUsingMemRefRegister(cg);

        appendTo = generateS390BranchInstruction(
          cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, snippetLabel, appendTo);
        ...
        appendTo = generateS390CompareAndBranchInstruction(
          cg, cmpOpCode, node, targetRegister, NULLVALUE, branchOpCond, snippetLabel, false, true, appendTo);
        TR::Instruction* cursor = appendTo;
        cursor->setExceptBranchOp();
      }
    }
  ...
  TR::Node* ifNode, TR::CodeGenerator* cg, TR::Node* cmpOpNode, TR::Node* cmpValNode)
{
  ...
  if (ifNode->getFirstChild()->getOpCodeValue() == TR::bu2i) {
    ...
    //      cmpOpNode
    //    cmpValNode
    // We have previously verified that cmpOpNode isSingleRefUnevaluated(), so therefore the bu2i must also be
    // isSingleRefEvaluated() and we should dec ref count here
...
bool OMR::Z::TreeEvaluator::treeContainsAllOtherUsesForNode(TR::Node* treeNode, TR::Node* node)
{
...
TR::Register* OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    traceMsg(comp,
      "Starting evaluation of ternary node %p condition %p (in reg %p) trueVal %p (in reg %p) falseVal %p (in reg "
      "%p)\n",
  ...
  if (TR::TreeEvaluator::treeContainsAllOtherUsesForNode(condition, trueVal)
    && (!trueVal->isUnneededConversion()
      || TR::TreeEvaluator::treeContainsAllOtherUsesForNode(condition, trueVal->getFirstChild()))) {
    if (comp->getOption(TR_TraceCG))
      traceMsg(comp,
        "Calling evaluate (instead of clobber evaluate for node %p because all other uses are in the compare tree)\n",
        trueVal);
  ...
  if (condition->getOpCode().isBooleanCompare()
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
      ...
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp,
          "firstReg = %p secondReg = %p trueReg = %p falseReg = %p trueReg->is64BitReg() = %d falseReg->is64BitReg() = "
          "%d isLongCompare = %d\n",
    ...
    TR::Instruction* compareInst
      = generateRILInstruction(cg, condition->getOpCode().isLongCompare() ? TR::InstOpCode::CGFI : TR::InstOpCode::CFI,
...
TR::Register* OMR::Z::TreeEvaluator::dternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (conditionNode->getOpCode().isCompareDouble()) {
    ...
    // To handle unordered compares, we switch the compare operation, since the vector compare instructions return false
    // for unordered. For example, if the original compare is dcmpgeu:
    ...
    // The above operation returns 0's for not equals, so we must swap the true and false values to select the correct
    // "true value" for cmpne operations Swap the values for unordered comparisons also (see above)
