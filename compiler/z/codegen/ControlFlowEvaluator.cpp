...
void
killRegisterIfNotLocked(TR::CodeGenerator* cg,
...
virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateVirtualGuardNOPInstruction(
    cg, node, site, deps, node->getBranchDestination()->getNode()->getLabel());

  cg->recursivelyDecReferenceCount(node->getFirstChild());
  cg->recursivelyDecReferenceCount(node->getSecondChild());

  traceMsg(comp,
           "virtualGuardHelper for %s %s\n",
...
static TR::Register*
generateS390lcmpEvaluator64(TR::Node* node,
                            TR::CodeGenerator* cg,
                            ...
                            bool isBoolean)
{
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    generateS390ImmOp(cg,
                      isUnsigned ? TR::InstOpCode::CLG : TR::InstOpCode::CG,
                      node,
                      cmpRegister,
                      cmpRegister,
                      long_value);
...
OMR::Z::TreeEvaluator::fcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  brCond = generateS390CompareOps(
    node, cg, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL);
...
generateS390Compare(TR::Node* node,
                    TR::CodeGenerator* cg,
                    TR::InstOpCode::Mnemonic branchOp,
                    TR::InstOpCode::S390BranchCondition fBranchOpCond,
                    TR::InstOpCode::S390BranchCondition rBranchOpCond)
 ...
 */
static TR::Register*
maxMinHelper(TR::Node* node, TR::CodeGenerator* cg, bool isMax)
{
  ...
  if (node->getOpCodeValue() == TR::imax ||
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
      ...
      generateRRFInstruction(
        cg, TR::InstOpCode::SELR, node, lhsReg, rhsReg, tmpRegister, mask);
      ...
      generateRRFInstruction(
        cg, TR::InstOpCode::LOCR, node, lhsReg, rhsReg, mask, true);
    } else {
      lhsReg = cg->gprClobberEvaluate(lhsNode);
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
      ...
      generateRREInstruction(
        cg, TR::InstOpCode::CGR, node, tmpRegister, rhsReg);
      generateRRFInstruction(
        cg, TR::InstOpCode::SELGR, node, lhsReg, rhsReg, tmpRegister, mask);
    } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      lhsReg = cg->gprClobberEvaluate(lhsNode);

      generateRREInstruction(cg, TR::InstOpCode::CGR, node, lhsReg, rhsReg);
      generateRRFInstruction(
        cg, TR::InstOpCode::LOCGR, node, lhsReg, rhsReg, mask, true);
    } else {
      lhsReg = cg->gprClobberEvaluate(lhsNode);
...
lcmpHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::LCGR, node, targetRegister, src1Reg);
    generateRSInstruction(cg, TR::InstOpCode::SRAG, node, src1Reg, src1Reg, 63);
    generateRSInstruction(
      cg, TR::InstOpCode::SRLG, node, targetRegister, targetRegister, 63);
    generateRRInstruction(
      cg, TR::InstOpCode::OGR, node, targetRegister, src1Reg);
  } else {
    // Assume LT
    ...
    cFlowRegionStart->setStartInternalControlFlow();
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, node, labelLT);

    // If GT, we invert the result register
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, node, labelGT);
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::LCGR, node, targetRegister, targetRegister);
    ...
    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, labelLT, dependencies);
  ...
  switch (node->getOpCodeValue()) {
    ...
    case TR::lreturn:
      ...
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateRSInstruction(
          cg, TR::InstOpCode::SRLG, node, highRegister, returnValRegister, 32);
...
bool
OMR::Z::TreeEvaluator::isCandidateForButestEvaluation(TR::Node* node)
{
...
bool
OMR::Z::TreeEvaluator::isCandidateForCompareEvaluation(TR::Node* node)
{
  return node->getOpCode().isIf() &&
...
bool
OMR::Z::TreeEvaluator::isSingleRefUnevalAndCompareOrBu2iOverCompare(
  TR::Node* node)
{
 ...
 */
static inline void
generateMergedGuardCodeIfNeeded(TR::Node* node, TR::CodeGenerator* cg)
{
#ifdef J9_PROJECT_SPECIFIC
  TR::Compilation* comp = cg->comp();
    ...
    if (virtualGuard && (virtualGuard->mergedWithOSRGuard() ||
...

      {
        ...
        TR::Instruction* vgnopInstr =
          generateVirtualGuardNOPInstruction(cg,
                                             node,
                                             site,
                                             deps,
                                             fallThroughLabel,
        ...
        generateS390LabelInstruction(
          cg, TR::InstOpCode::LABEL, node, fallThroughLabel, mergedGuardDeps);
      } else {
      ...
      traceMsg(
        comp,
        "generateMergedGuardCodeIfNeeded for %s %s\n",
        comp->getDebug()
          ? comp->getDebug()->getVirtualGuardKindName(virtualGuard->getKind())
          : "???Guard",
        virtualGuard->mergedWithHCRGuard()
...
OMR::Z::TreeEvaluator::ificmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::TreeEvaluator::isCandidateForButestEvaluation(node)) {
    ...
    return TR::TreeEvaluator::inlineIfButestEvaluator(
      butestNode, cg, cmpNode, valueNode);
  }
  ...
  if (TR::TreeEvaluator::isCandidateForCompareEvaluation(node)) {
    ...
    return TR::TreeEvaluator::inlineIfBifEvaluator(
      ifNode, cg, cmpNode, valueNode);
  }
  ...
  if (node->getOpCodeValue() == TR::ificmpeq ||
      node->getOpCodeValue() == TR::ifacmpeq) {
    reg = generateS390CompareBranch(node,
                                    cg,
                                    TR::InstOpCode::BRC,
 ...
 */
TR::Register*
OMR::Z::TreeEvaluator::ifFoldingHelper(TR::Node* node,
                                       TR::CodeGenerator* cg,
                                       bool& handledBIF)
{
  handledBIF = true;
...
OMR::Z::TreeEvaluator::ificmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  return generateS390CompareBranch(node,
                                   cg,
                                   TR::InstOpCode::BRC,
...
{
  generateS390CompareBranch(node,
                            cg,
                            TR::InstOpCode::BRC,
                            TR::InstOpCode::COND_BE,
                            TR::InstOpCode::COND_BE);
...
OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  if (node->getOpCodeValue() == TR::icmpeq ||
    ...
    return generateS390CompareBool(node,
                                   cg,
                                   TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BE,
                                   TR::InstOpCode::COND_BE);
...
{
  return generateS390lcmpEvaluator64(
    node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, CMP4BOOLEAN);
}
...
{
  return generateS390lcmpEvaluator64(
    node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, CMP4BOOLEAN);
}
...
{
  return generateS390lcmpEvaluator64(
    node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, CMP4BOOLEAN);
}
...
{
  return generateS390lcmpEvaluator64(
    node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, CMP4BOOLEAN);
}
...
{
  return generateS390lcmpEvaluator64(
    node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, CMP4BOOLEAN);
}
...
{
  return generateS390lcmpEvaluator64(
    node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, CMP4BOOLEAN);
}
...
void
OMR::Z::TreeEvaluator::tableEvaluatorCaseLabelHelper(
  TR::Node* node,
  TR::CodeGenerator* cg,
  ...
  TR::Register* reg1)
{
  TR::Compilation* comp = cg->comp();
  {
    ...
    new (cg->trHeapMemory()) TR::S390RIInstruction(
      ...
      node,
      reg1,
      (4 + (sizeof(uintptrj_t) * numBranchTableEntries)) / 2,
      cg);
    ...
    generateRXInstruction(
      cg, TR::InstOpCode::getLoadOpCode(), node, selectorReg, tempMR);
...
OMR::Z::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getNumChildren() > 0) {
    ...
    deps =
      generateRegisterDependencyConditions(cg, secondChild->getFirstChild(), i);
    if (reg1 != NULL)
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (selectorType.isInt8() || selectorType.isInt16() ||
      ...
      generateRRInstruction(
        cg, TR::InstOpCode::LGFR, node, selectorReg, selectorReg);
  } else {
  ...
  switch (tableKindToBeEvaluated) {
    case AddressTable32bit: {
      if (!node->chkCannotOverflow())
        generateS390CompareAndBranchInstruction(
          ...
          false,
          false); // make sure the case selector is within range of our case
                  // constants
    ...
    case AddressTable64bitIntLookup: {
      ...
      generateRSInstruction(
        cg, TR::InstOpCode::SLLG, node, selectorReg, selectorReg, 3);
    }
    ...
    case RelativeTable64bitLongLookup: {
      if (!node->chkCannotOverflow())
        generateS390CompareAndBranchInstruction(
          ...
          false);
      generateRSInstruction(
        cg, TR::InstOpCode::SLLG, node, selectorReg, selectorReg, 2);
    } break;
  }

  TR::TreeEvaluator::tableEvaluatorCaseLabelHelper(node,
                                                   cg,
                                                   tableKindToBeEvaluated,
                                                   numBranchTableEntries,
                                                   selectorReg,
                                                   branchTableReg,
                                                   reg1);

  TR::Instruction* cursor = generateS390RegInstruction(
    cg, TR::InstOpCode::BCR, node, selectorReg, deps);
  ((TR::S390RegInstruction*)cursor)
    ->setBranchCondition(TR::InstOpCode::COND_BCR);
...
bool
isSwitchFoldableNoncall(TR::Node* node)
{
 ...
 */
TR::Register*
OMR::Z::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(TR::Node* node,
                                                          bool needsResolve,
                                                          TR::CodeGenerator* cg)
{
  // NOTE:
  ...
  if (needExplicitCheck) {
    ...
    if (cg->getHasResumableTrapHandler()) {
      ...
      if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) &&
        ...
        appendTo =
          generateRXInstruction(cg,
                                TR::InstOpCode::getLoadAndTrapOpCode(),
                                node,
                                targetRegister,
        ...
        if (appendTo->getOpCodeValue() == TR::InstOpCode::LLGF) {
          ...
          appendTo =
            generateRIEInstruction(cg,
                                   TR::InstOpCode::getCmpImmTrapOpCode(),
                                   node,
                                   targetRegister,
                                   (int16_t)0,
                                   TR::InstOpCode::COND_BE,
        ...
        appendTo = generateRIEInstruction(cg,
                                          TR::InstOpCode::getCmpImmTrapOpCode(),
                                          node,
                                          targetRegister,
                                          (int16_t)0,
      ...
      if (!firstChild->getOpCode().isCall() &&
        ...
        TR::MemoryReference* tempMR =
          generateS390MemoryReference(reference, cg);
        appendTo = generateRXInstruction(cg,
                                         TR::InstOpCode::getLoadTestOpCode(),
                                         reference,
                                         targetRegister,
                                         tempMR,
                                         appendTo);
        ...
        appendTo = generateS390BranchInstruction(cg,
                                                 TR::InstOpCode::BRC,
                                                 TR::InstOpCode::COND_BE,
                                                 node,
                                                 snippetLabel,
        ...
        appendTo = generateS390CompareAndBranchInstruction(cg,
                                                           cmpOpCode,
                                                           node,
                                                           targetRegister,
                                                           NULLVALUE,
...
OMR::Z::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (valueToCheck->getOpCode().getOpCodeValue() == TR::acmpeq) {
    ...
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, opBranchCond, node, slowPathOOLLabel);
  } else {
    TR::Register* valueReg = cg->evaluate(valueToCheck);
    generateRIInstruction(cg, TR::InstOpCode::CHI, node, valueReg, 0);
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, slowPathOOLLabel);
  }
...
void
OMR::Z::TreeEvaluator::addToRegDep(TR::RegisterDependencyConditions* daaDeps,
                                   TR::Register* reg,
...
void
OMR::Z::TreeEvaluator::createDAACondDeps(
  TR::Node* node,
  ...
  switch (daaInstr->getKind()) {
    case TR::Instruction::IsRX: // CVB, CVD
    {
      ...
      TR::TreeEvaluator::addToRegDep(
        daaDeps, instr->getRegisterOperand(1), false);
      if (mr) {
        TR::TreeEvaluator::addToRegDep(
    ...
    case TR::Instruction::IsRXY: // CVBY, CVBG
    {
      ...
      TR::TreeEvaluator::addToRegDep(
        daaDeps, instr->getRegisterOperand(1), true);
      if (mr) {
        TR::TreeEvaluator::addToRegDep(
    ...
    {
      TR::TreeEvaluator::addToRegDep(
        daaDeps, daaInstr->getRegisterOperand(1), false);
      TR::TreeEvaluator::addToRegDep(
        daaDeps, daaInstr->getRegisterOperand(2), false);
      TR::TreeEvaluator::addToRegDep(
        daaDeps, daaInstr->getRegisterOperand(3), false);
      break;
    }
...
TR::Node*
OMR::Z::TreeEvaluator::DAAAddressPointer(TR::Node* callNode,
                                         TR::CodeGenerator* cg)
{
...
void
OMR::Z::TreeEvaluator::commonButestEvaluator(TR::Node* node,
                                             TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
...
TR::Register*
OMR::Z::TreeEvaluator::inlineIfButestEvaluator(TR::Node* node,
                                               TR::CodeGenerator* cg,
                                               TR::Node* cmpOpNode,
                                               TR::Node* cmpValNode)
{
  ...
  TR::Instruction* instr = generateS390BranchInstruction(
    ...
    cond,
    cmpOpNode,
    cmpOpNode->getBranchDestination()->getNode()->getLabel(),
    deps);
...
TR::Register*
OMR::Z::TreeEvaluator::inlineIfBifEvaluator(TR::Node* ifNode,
                                            TR::CodeGenerator* cg,
                                            TR::Node* cmpOpNode,
                                            TR::Node* cmpValNode)
{
 ...
 */
TR::InstOpCode::S390BranchCondition
OMR::Z::TreeEvaluator::getBranchConditionFromCompareOpCode(TR::ILOpCodes opCode)
{
  switch (opCode) {
    case TR::icmpeq:
    case TR::acmpeq:
...
int32_t
OMR::Z::TreeEvaluator::countReferencesInTree(TR::Node* treeNode, TR::Node* node)
{
...
bool
OMR::Z::TreeEvaluator::treeContainsAllOtherUsesForNode(TR::Node* treeNode,
                                                       TR::Node* node)
{
...
OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (comp->getOption(TR_TraceCG))
    traceMsg(comp,
             "Done evaluating child %p in reg %p and %p in reg %p\n",
             trueVal,
             trueReg,
             falseVal,
             falseReg);
  ...
  if (condition->getOpCode().isBooleanCompare() &&
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
      ...
      generateS390LabelInstruction(
        cg, TR::InstOpCode::LABEL, node, cFlowRegionEnd, conditions);
      cFlowRegionEnd->setEndInternalControlFlow();
  ...
  if (conditionNode->getOpCode().isCompareDouble()) {
    ...
    if (ifOp == TR::dcmpgeu || ifOp == TR::dcmpgtu || ifOp == TR::dcmplt ||
      ...
      generateVRRcInstruction(
        cg, compareOp, node, vectorSelReg, secondCmpReg, firstCmpReg, 1, 0, 3);
    else
      generateVRRcInstruction(
        cg, compareOp, node, vectorSelReg, firstCmpReg, secondCmpReg, 1, 0, 3);
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::LDGR, node, tempReg, conditionReg);

    // generate compare with zero
    generateVRIaInstruction(cg, TR::InstOpCode::VGBM, node, vzeroReg, 0, 0);
    generateVRRcInstruction(
      cg, TR::InstOpCode::VFCE, node, vectorSelReg, tempReg, vzeroReg, 1, 0, 3);
