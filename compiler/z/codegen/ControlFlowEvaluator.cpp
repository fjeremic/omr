...
extern TR::Instruction *generateS390PackedCompareAndBranchOps(
...
extern TR::Register *iDivRemGenericEvaluator(
    TR::Node *node, TR::CodeGenerator *cg, bool isDivision,
...
void killRegisterIfNotLocked(TR::CodeGenerator *cg,
                             TR::RealRegister::RegNum reg,
                             TR::Instruction *instr,
                             TR::RegisterDependencyConditions *deps = NULL) {
  TR::Register *dummy = NULL;
  if (cg->machine()->getRealRegister(reg)->getState() !=
...
/**
 * Helper to generate virtual guard if node is so flagged.  In 32 bit mode the
 * NOP instruction, a BRC, has only a +ve displacement of 2^12 (4k), and so in
 * some cases it is not safe to use it on a G5 platform (no problems with BRCL).
 * At this point, since instructions have not yet been generated, there is no
 * good way to guage how far away the label will be, so we cannot even determine
 * if it is safe.  Will never be enabled for G5 hardware Note that some
 * instructions *must* have a virtual guard generated for them.
 */
static bool virtualGuardHelper(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (comp->compileRelocatableCode()) {
    ...
    switch (virtualGuard->getKind()) {
      ...
      default:
        TR_ASSERT_FATAL(0,
                        "got AOT guard in node but virtual guard not one of "
                        "known guards supported for AOT. Guard: %d",
  ...
  if (node->getNumChildren() == 3) {
    ...
    deps = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions((uint16_t)0, (uint16_t)0, cg);
  }

  if (virtualGuard->shouldGenerateChildrenCode())
    cg->evaluateChildrenWithMultipleRefCount(node);

  generateVirtualGuardNOPInstruction(
      cg, node, site, deps,
  ...
  traceMsg(comp, "virtualGuardHelper for %s %s\n",
           comp->getDebug() ? comp->getDebug()->getVirtualGuardKindName(
                                  virtualGuard->getKind())
                            : "???Guard",
...
static TR::Register *generateS390lcmpEvaluator64(
    TR::Node *node, TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic brOp,
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    generateS390ImmOp(cg, isUnsigned ? TR::InstOpCode::CLG : TR::InstOpCode::CG,
                      node, cmpRegister, cmpRegister, long_value);
    ...
    deps->addPostConditionIfNotAlreadyInserted(cmpRegister,
    ...
    generateS390CompareAndBranchInstruction(
        cg, isUnsigned ? TR::InstOpCode::CLGR : TR::InstOpCode::CGR, node,
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::fcmplEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  ...
  brCond = generateS390CompareOps(node, cg, TR::InstOpCode::COND_BH,
  ...
  if (node->getOpCodeValue() == TR::fcmpg ||
      node->getOpCodeValue() == TR::dcmpg) {
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_MASK1, node,
                                  cFlowRegionEnd);
  }

  // Got here? means either A<B or (TR::fcmpl instruction and (A==NaN ||
  // B==NaN)) set targetRegister value to -1
...
inline TR::InstOpCode::S390BranchCondition generateS390Compare(
 ...
 */
static TR::Register *maxMinHelper(TR::Node *node, TR::CodeGenerator *cg,
                                  bool isMax) {
  ...
  // Mask is 4 to pick rhs when lhs is less for max, 2 to pick rhs when lhs is
  // greater for min
  const uint8_t mask = isMax ? 0x4 : 0x2;

  if (node->getOpCodeValue() == TR::imax ||
      node->getOpCodeValue() == TR::imin) {
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
      lhsReg = cg->allocateRegister();

      // Load into a tmp instead of clobberEvaluating into lhsReg to avoid an
      ...
      generateRRFInstruction(cg, TR::InstOpCode::SELR, node, lhsReg, rhsReg,
                             tmpRegister, mask);
      ...
      auto bc = isMax ? TR::InstOpCode::COND_BHR : TR::InstOpCode::COND_BLR;

      generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CR, node,
                                              lhsReg, rhsReg, bc,
                                              cFlowRegionEnd, false);
      ...
      deps->addPostConditionIfNotAlreadyInserted(lhsReg,
                                                 TR::RealRegister::AssignAny);
      deps->addPostConditionIfNotAlreadyInserted(rhsReg,
                                                 TR::RealRegister::AssignAny);
  ...
  } else if (node->getOpCodeValue() == TR::lmax ||
             node->getOpCodeValue() == TR::lmin) {
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
      lhsReg = cg->allocateRegister();
      ...
      generateRREInstruction(cg, TR::InstOpCode::CGR, node, tmpRegister,
                             rhsReg);
      generateRRFInstruction(cg, TR::InstOpCode::SELGR, node, lhsReg, rhsReg,
                             tmpRegister, mask);
    } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      lhsReg = cg->gprClobberEvaluate(lhsNode);
      ...
      generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CGR, node,
                                              lhsReg, rhsReg, bc,
                                              cFlowRegionEnd, false);
    ...
    TR_ASSERT_FATAL(node->getOpCodeValue(),
                    "Opcode %s cannot be evaluated by maxMinHelper\n",
...
TR::Register *OMR::Z::TreeEvaluator::maxEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  return maxMinHelper(node, cg, true);
}

TR::Register *OMR::Z::TreeEvaluator::minEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  return maxMinHelper(node, cg, false);
}

/**
 * 64bit version lcmpEvaluator Helper: long compare (1 if child1 > child2, 0 if
 * child1 == child2, -1 if child1 < child2 or unordered)
 */
TR::Register *lcmpHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  // TODO:There is probably a better way to implement this.  Should re-visit if
  // we get the chance. As things stand now, we will to LCR R1,R1 when R1=0
  // which is useless...  But still probably cheaper than an extra branch.
  TR::LabelSymbol *cFlowRegionStart = generateLabelSymbol(cg);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    generateRSInstruction(cg, TR::InstOpCode::SRLG, node, targetRegister,
                          targetRegister, 63);
    generateRRInstruction(cg, TR::InstOpCode::OGR, node, targetRegister,
                          src1Reg);
  } else {
    // Assume LT
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      generateS390ImmOp(cg, TR::InstOpCode::CG, node, src1Reg, src1Reg,
    ...
    cFlowRegionStart->setStartInternalControlFlow();
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BL, node, labelLT);

    // If GT, we invert the result register
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BH, node, labelGT);
    ...
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, labelLT,
                                 dependencies);
  ...
  if (!cg->canClobberNodesRegister(firstChild)) cg->stopUsingRegister(src1Reg);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
...
TR::Register *OMR::Z::TreeEvaluator::branchEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return NULL;
}
TR::Register *OMR::Z::TreeEvaluator::ibranchEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return NULL;
}
TR::Register *OMR::Z::TreeEvaluator::mbranchEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return NULL;
}
 ...
 * goto label address
 */
TR::Register *OMR::Z::TreeEvaluator::gotoEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  ...
  if (node->getNumChildren() > 0) {
    ...
    generateS390BranchInstruction(
        ...
        temp->getLabel(), generateRegisterDependencyConditions(cg, child, 0));
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::igotoEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR_ASSERT(node->getNumChildren() >= 1,
            "at least one child expected for igoto");
  ...
  if (node->getNumChildren() > 1) {
    TR_ASSERT(node->getNumChildren() == 2 &&
                  node->getChild(1)->getOpCodeValue() == TR::GlRegDeps,
              "igoto has maximum of two children and second one must be global "
              "register dependency");
  ...
  if (deps)
    cursor =
        generateS390RegInstruction(cg, TR::InstOpCode::BCR, node, dest, deps);
  else
    cursor = generateS390RegInstruction(cg, TR::InstOpCode::BCR, node, dest);

  ((TR::S390RegInstruction *)cursor)
      ->setBranchCondition(TR::InstOpCode::COND_BCR);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::returnEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  // add register dependency for the return value register and the return
  // register
  ...
  if (node->getOpCode().isJumpWithMultipleTargets() &&
      ...
      node->getChild(node->getNumChildren() - 1)->getOpCodeValue() ==
          TR::GlRegDeps) {
  ...
  switch (node->getOpCodeValue()) {
    case TR::areturn:
      ...
      dependencies->addPostCondition(returnValRegister,
                                     linkage->getIntegerReturnRegister());
      break;
    case TR::ireturn:
      dependencies->addPostCondition(returnValRegister,
                                     linkage->getIntegerReturnRegister());
      ...
      if (linkage->isNeedsWidening())
        new (cg->trHeapMemory())
            TR::S390RRInstruction(TR::InstOpCode::LGFR, node, returnValRegister,
    ...
    case TR::lreturn:
      ...
      if (TR::Compiler->target.is64Bit()) {
        dependencies->addPostCondition(returnValRegister,
                                       linkage->getLongReturnRegister());
      } else {
        TR::Register *highRegister = cg->allocateRegister();

        generateRSInstruction(cg, TR::InstOpCode::SRLG, node, highRegister,
                              returnValRegister, 32);

        dependencies->addPostCondition(returnValRegister,
                                       linkage->getLongLowReturnRegister());
        dependencies->addPostCondition(highRegister,
                                       linkage->getLongHighReturnRegister());
    ...
    case TR::dfreturn:
      ...
      dependencies->addPostCondition(returnValRegister,
                                     linkage->getFloatReturnRegister());
      break;
    case TR::dreturn:
#ifdef J9_PROJECT_SPECIFIC
    case TR::ddreturn:
      ...
      dependencies->addPostCondition(returnValRegister,
                                     linkage->getDoubleReturnRegister());
      break;
#ifdef J9_PROJECT_SPECIFIC
    case TR::dereturn:
      comp->setReturnInfo(TR_DoubleReturn);
      dependencies->addPostCondition(returnValRegister->getHighOrder(),
                                     linkage->getLongDoubleReturnRegister0());
      dependencies->addPostCondition(returnValRegister->getLowOrder(),
                                     linkage->getLongDoubleReturnRegister2());
      break;
#endif
    case TR::Return:
...
bool OMR::Z::TreeEvaluator::isCandidateForCompareEvaluation(TR::Node *node) {
  return node->getOpCode().isIf() &&
         TR::TreeEvaluator::isSingleRefUnevalAndCompareOrBu2iOverCompare(
             node->getFirstChild()) &&
         node->getSecondChild()->getOpCode().isLoadConst() &&
         node->getSecondChild()->getType().isInt32();
}

bool OMR::Z::TreeEvaluator::isSingleRefUnevalAndCompareOrBu2iOverCompare(
    TR::Node *node) {
  if (!node->isSingleRefUnevaluated()) return false;

  if (node->getOpCodeValue() == TR::bu2i)
    return TR::TreeEvaluator::isSingleRefUnevalAndCompareOrBu2iOverCompare(
        node->getFirstChild());
...
/**
 * This helper is for generating a VGNOP for HCR or OSR guard when the guard it
 * was merged with cannot be NOPed.
 * 1. HCR or OSR guard is merged with ProfiledGuard
 * 2. Any NOPable guards when node is switched from icmpne to icmpeq (can
 * potentially happen during block reordering)
 *
 * When the condition is switched on node from eq to ne, HCR or OSR guard has to
 * be patch to go to the fall through path
 */
static inline void generateMergedGuardCodeIfNeeded(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
#ifdef J9_PROJECT_SPECIFIC
  TR::Compilation *comp = cg->comp();
  if (node->isTheVirtualGuardForAGuardedInlinedCall() &&
    ...
    if (virtualGuard && (virtualGuard->mergedWithOSRGuard() ||
                         virtualGuard->mergedWithHCRGuard())) {
      ...
      if (instr && instr->getNode() == node && instr->getDependencyConditions())
        mergedGuardDeps =
            new (cg->trHeapMemory()) TR::RegisterDependencyConditions(
                instr->getDependencyConditions(), 1, 0, cg);
...

      {
        ...
        TR::Instruction *vgnopInstr = generateVirtualGuardNOPInstruction(
            cg, node, site, deps, fallThroughLabel,
            instr ? instr->getPrev() : NULL);
        vgnopInstr->setNext(instr);
        cg->setAppendInstruction(instr);
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                     fallThroughLabel, mergedGuardDeps);
      } else {
        TR::LabelSymbol *label =
            node->getBranchDestination()->getNode()->getLabel();

        TR::Instruction *vgnopInstr = generateVirtualGuardNOPInstruction(
            cg, node, site, mergedGuardDeps, label,
            instr ? instr->getPrev() : NULL);
        vgnopInstr->setNext(instr);
        cg->setAppendInstruction(instr);
      }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ificmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  if (TR::TreeEvaluator::isCandidateForButestEvaluation(node)) {
    ...
    return TR::TreeEvaluator::inlineIfButestEvaluator(butestNode, cg, cmpNode,
                                                      valueNode);
  }
  ...
  if (TR::TreeEvaluator::isCandidateForCompareEvaluation(node)) {
    ...
    return TR::TreeEvaluator::inlineIfBifEvaluator(ifNode, cg, cmpNode,
                                                   valueNode);
  }
  ...
  if (node->getOpCodeValue() == TR::ificmpeq ||
      node->getOpCodeValue() == TR::ifacmpeq) {
    reg = generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifFoldingHelper(TR::Node *node,
                                                     TR::CodeGenerator *cg,
                                                     bool &handledBIF) {
  handledBIF = true;
  TR::Node *firstChild = node->getFirstChild(),
  ...
  bool inlined = false;
  TR::Register *reg =
      TR::TreeEvaluator::inlineIfTestDataClassHelper(node, cg, inlined);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ificmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  bool inlined = false;
  TR::Register *reg;
  ...
  TR::Register *result =
      TR::TreeEvaluator::ifFoldingHelper(node, cg, handledBIF);
  if (handledBIF) return result;

  return generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BL,
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ificmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  bool inlined = false;
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ificmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  bool inlined = false;
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ificmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  bool inlined = false;
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iflcmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                            TR::InstOpCode::COND_BE, TR::InstOpCode::COND_BE);
  generateMergedGuardCodeIfNeeded(node, cg);
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iflcmpneEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  if (virtualGuardHelper(node, cg)) {
    return NULL;
  }
  ...
  generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                            TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE);
  generateMergedGuardCodeIfNeeded(node, cg);
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iflcmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                            TR::InstOpCode::COND_BNH, TR::InstOpCode::COND_BNL);
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iflcmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                            TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH);
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iflcmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                            TR::InstOpCode::COND_BNL, TR::InstOpCode::COND_BNH);
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iflcmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  generateS390CompareBranch(node, cg, TR::InstOpCode::BRC,
                            TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL);
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifacmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifacmpneEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpeqEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifbcmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
///////////////////////////////////////////////////////////////////////////////////////
// ifbcmpneEvaluator (byte compare and branch if not equal) handled by
// ifbcmpeqEvaluator
///////////////////////////////////////////////////////////////////////////////////////
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifbcmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifbcmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifbcmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifbcmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifscmpeqEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifscmpltEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpltEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifscmpgeEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifscmpgtEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifscmpleEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpleEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifsucmpeqEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifsucmpltEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpltEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifsucmpgeEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifsucmpgtEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ifsucmpleEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ificmpleEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::icmpeqEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  if (node->getOpCodeValue() == TR::icmpeq ||
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
      ...
      if (node->getOpCodeValue() != TR::acmpeq &&
          ...
          performTransformation(cg->comp(),
                                "O^O CODE GENERATION:  ===>   Use RXSBG to "
                                "perform icmpeq  <==\n")) {
        ...
        if ((val1 & (val1 - 1)) == 0) {
          ...
          generateRIEInstruction(cg, TR::InstOpCode::RXSBG, node, target,
                                 source, 63, 128 + 63, rotBy);
    ...
    return generateS390CompareBool(node, cg, TR::InstOpCode::BRC,
                                   TR::InstOpCode::COND_BE,
                                   TR::InstOpCode::COND_BE);
  } else {
    return generateS390CompareBool(node, cg, TR::InstOpCode::BRC,
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::icmpltEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();

  if (!node->getOpCode().isUnsignedCompare() &&
      firstChild->getReferenceCount() == 1 && firstChild->isNonNegative() &&
      secondChild->isNonNegative() &&
      performTransformation(
          cg->comp(),
          "O^O CODE GENERATION:  ===>   Use SR/SRL to perform icmplt  <==\n")) {
    // Even if 1st child refcount is 1, the register may be shared with other
    // nodes and still be live. gprClobberEvaluate will do the right thing.
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::icmpgeEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390CompareBool(node, cg, TR::InstOpCode::BRC,
                                 TR::InstOpCode::COND_BNL,
                                 TR::InstOpCode::COND_BNH);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::icmpgtEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();

  if (!node->getOpCode().isUnsignedCompare() &&
      secondChild->getReferenceCount() == 1 && firstChild->isNonNegative() &&
      secondChild->isNonNegative() &&
      performTransformation(
          cg->comp(),
          "O^O CODE GENERATION:  ===>   Use SR/SRL to perform icmpgt  <==\n")) {
    TR::Register *firstReg = cg->evaluate(firstChild);
    // Even if 2nd child refcount is 1, the register may be shared with other
    // nodes and still be live. gprClobberEvaluate will do the right thing.
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::icmpleEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390CompareBool(node, cg, TR::InstOpCode::BRC,
                                 TR::InstOpCode::COND_BNH,
                                 TR::InstOpCode::COND_BNL);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lcmpeqEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                     TR::InstOpCode::COND_BE, CMP4BOOLEAN);
}

/**
 * Long compare if not equal
 */
TR::Register *OMR::Z::TreeEvaluator::lcmpneEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                     TR::InstOpCode::COND_BNE, CMP4BOOLEAN);
}

/**
 * Long compare if less than or equal
 */
TR::Register *OMR::Z::TreeEvaluator::lcmpleEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                     TR::InstOpCode::COND_BNH, CMP4BOOLEAN);
}

/**
 * Long compare if less than
 */
TR::Register *OMR::Z::TreeEvaluator::lcmpltEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                     TR::InstOpCode::COND_BL, CMP4BOOLEAN);
}

/**
 * Long compare if greater than or equal
 */
TR::Register *OMR::Z::TreeEvaluator::lcmpgeEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                     TR::InstOpCode::COND_BNL, CMP4BOOLEAN);
}

/**
 * Long compare if greater than
 */
TR::Register *OMR::Z::TreeEvaluator::lcmpgtEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return generateS390lcmpEvaluator64(node, cg, TR::InstOpCode::BRC,
                                     TR::InstOpCode::COND_BH, CMP4BOOLEAN);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::acmpeqEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bcmpeqEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bcmpltEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bcmpgeEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bcmpgtEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bcmpleEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::scmpeqEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::scmpltEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpltEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::scmpgeEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::scmpgtEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::scmpleEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpleEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sucmpeqEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return generateS390CompareBool(node, cg, TR::InstOpCode::BRC,
                                 TR::InstOpCode::COND_BNE,
                                 TR::InstOpCode::COND_BNE);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sucmpltEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpltEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sucmpgeEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpgeEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sucmpgtEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpgtEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sucmpleEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::icmpleEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lcmpEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return lcmpHelper64(node, cg);
}

void OMR::Z::TreeEvaluator::tableEvaluatorCaseLabelHelper(
    TR::Node *node, TR::CodeGenerator *cg, tableKind tableKindToBeEvaluated,
    int32_t numBranchTableEntries, TR::Register *selectorReg,
    TR::Register *branchTableReg, TR::Register *reg1) {
  TR::Compilation *comp = cg->comp();
  {
    TR_ASSERT(reg1, "Java must have allocated a temp reg");
    TR_ASSERT(tableKindToBeEvaluated == AddressTable32bit ||
    ...
    new (cg->trHeapMemory()) TR::S390RIInstruction(
        TR::InstOpCode::BRAS, node, reg1,
        (4 + (sizeof(uintptrj_t) * numBranchTableEntries)) / 2, cg);
    ...
    for (int32_t i = 0; i < numBranchTableEntries; ++i) {
      ...
      new (cg->trHeapMemory())
          TR::S390LabelInstruction(TR::InstOpCode::DC, node, label, cg);
    }

    TR::MemoryReference *tempMR =
        generateS390MemoryReference(reg1, selectorReg, 0, cg);
 ...
 * There may me other children after the branch targets, so use
 * getCaseIndexUpperBound() instead of getNumChildren() to determine the number
 * of branch targets Unlike lookuptable, this table is contiguous from 0 up to
 * maxentry.
 */
TR::Register *OMR::Z::TreeEvaluator::tableEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  int32_t i;
  uint32_t upperBound = node->getCaseIndexUpperBound();
  uint32_t numBranchTableEntries =
      upperBound - 2;  // First two children are not branch cases
  ...
  bool isUnsigned = node->getFirstChild()->getOpCode().isUnsigned() ||
                    node->getFirstChild()->getOpCodeValue() ==
                        TR::bu2i;  // TODO: check in  switchAnalyzer
  ...
  if (secondChild->getNumChildren() > 0) {
    ...
    deps = generateRegisterDependencyConditions(
        cg, secondChild->getFirstChild(), i);
    if (reg1 != NULL)
      deps->addPostConditionIfNotAlreadyInserted(reg1,
                                                 TR::RealRegister::AssignAny);
    deps->addPostConditionIfNotAlreadyInserted(selectorReg,
                                               TR::RealRegister::AssignAny);
  } else {
    deps = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, i, cg);
    if (reg1 != NULL) deps->addPostCondition(reg1, TR::RealRegister::AssignAny);

    deps->addPostCondition(selectorReg, TR::RealRegister::AssignAny);
  }

  // Determine if the selector register needs to be sign-extended on a 64-bit
  // system
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (selectorType.isInt8() || selectorType.isInt16() ||
        selectorType.isInt32())
      generateRRInstruction(cg, TR::InstOpCode::LGFR, node, selectorReg,
                            selectorReg);
  } else {
    TR_ASSERT(!node->getFirstChild()->getType().isInt64(),
              "Unhandled long selector type for table node %p\n", node);
    tableKindToBeEvaluated = AddressTable32bit;
  }

  /*
   * High level idea -- there are two modes, a relative table, where we load a
   * relative offset from the table and branch to it, and an address table,
   * where we load an address out of the table and branch to it since cases are
   * indexed by 1, we need to shift by either 4 or 8 to get the next value in
   * memory.
   */
  ...
  switch (tableKindToBeEvaluated) {
    case AddressTable32bit: {
      if (!node->chkCannotOverflow())
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CL, node, selectorReg,
            static_cast<int32_t>(numBranchTableEntries),
            TR::InstOpCode::COND_BNL,
            node->getSecondChild()
                ->getBranchDestination()
                ->getNode()
                ->getLabel(),
            false, false);  // make sure the case selector is within range of
                            // our case constants
    ...
    case AddressTable64bitIntLookup: {
      ...
      generateRSInstruction(cg, TR::InstOpCode::SLLG, node, selectorReg,
                            selectorReg, 3);
    }
    // generateRRInstruction(cg, TR::InstOpCode::LGFR, node, selectorReg,
    // selectorReg);
    ...
    case AddressTable64bitLongLookup: {
      if (!node->chkCannotOverflow())
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CLG, node, selectorReg,
            static_cast<int64_t>(numBranchTableEntries),
            TR::InstOpCode::COND_BNL,
            node->getSecondChild()
                ->getBranchDestination()
                ->getNode()
  ...
  TR::TreeEvaluator::tableEvaluatorCaseLabelHelper(
      node, cg, tableKindToBeEvaluated, numBranchTableEntries, selectorReg,
      branchTableReg, reg1);

  TR::Instruction *cursor = generateS390RegInstruction(cg, TR::InstOpCode::BCR,
                                                       node, selectorReg, deps);
  ((TR::S390RegInstruction *)cursor)
      ->setBranchCondition(TR::InstOpCode::COND_BCR);
  ...
  if (reg1 != NULL) cg->stopUsingRegister(reg1);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lookupEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  ...
  if (node->getOpCodeValue() == TR::trtLookup) {
    ...
    // No need to set up regDep for tempReg as no need to take care of its spill
    // for any branch which dest is out of trtLookup block.
  }
  ...
  for (int32_t ii = 2; ii < numChildren; ii++) {
    ...
    if (node->getOpCodeValue() == TR::trtLookup) {
      ...
      if (type.isInt64()) {
        generateS390ImmOp(cg, TR::InstOpCode::CG, node, selectorReg,
                          selectorReg, child->getCaseConstant());
      } else {
        generateS390ImmOp(cg, TR::InstOpCode::C, node, selectorReg, selectorReg,
                          child->getCaseConstant());
      }
    ...
    if (child->getNumChildren() > 0) {
      ...
      if (type.isInt64() && TR::Compiler->target.is32Bit()) {
        ...
        generateS390LabelInstruction(
            cg, TR::InstOpCode::LABEL, node, skipLoCompareLabel,
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, brCond, node,
            child->getBranchDestination()->getNode()->getLabel(),
            generateRegisterDependencyConditions(cg, child->getFirstChild(),
                                                 0));
    } else {
      cursor = generateS390BranchInstruction(
          cg, TR::InstOpCode::BRC, brCond, node,
  ...
  // Peek if the default branch is to the immediately following block, only for
  // trtlookup
  if (node->getOpCodeValue() == TR::trtLookup) {
    ...
    TR::Block *fallThroughBlock =
        cg->getCurrentEvaluationBlock()->getNextBlock();
  ...
  if (defaultChild->getNumChildren() > 0) {
    ...
    if (needDefaultBranch)
      ...
      cursor->setDependencyConditions(generateRegisterDependencyConditions(
          cg, defaultChild->getFirstChild(), 0));
  } else {
    if (needDefaultBranch)
      generateS390BranchInstruction(
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(
    TR::Node *node, bool needsResolve, TR::CodeGenerator *cg) {
  // NOTE:
  ...
  if (cg->getSupportsImplicitNullChecks() && !firstChild->isUnneededIALoad()) {
    if (opCode.isLoadVar() ||
        ...
        (hasCompressedPointers &&
         firstChild->getFirstChild()->getOpCode().getOpCodeValue() ==
             TR::i2l)) {
      ...
      if (n->getReferenceCount() == 1 &&
        ...
        // and node->getFirstChild's refCount will be at least 2 (one under the
        // nullchk and the other under the translate treetop)
        //
      ...
      // Check if offset from a NULL reference will fall into the inaccessible
      // bytes, resulting in an implicit trap being raised.
      else if (symRef &&
        ...
        // and is only going to be used immediately in a BNDCHK, combine the
        // checks.
        //
        ...
        if (n->getReferenceCount() == 2 && nextTreeTop) {
          ...
          if (nextTopNode) {
            if (nextTopNode->getOpCode().isBndCheck()) {
              if ((nextTopNode->getOpCode().isSpineCheck() &&
                   (nextTopNode->getChild(2) == n)) ||
                  (!nextTopNode->getOpCode().isSpineCheck() &&
                   (nextTopNode->getFirstChild() == n))) {
                needLateEvaluation = false;
              ...
              reference
                  ->incReferenceCount();  // will be decremented again later
            }
          }
        }
      }
      ...
      if (n->getOpCode().hasSymbolReference() &&
          symRef->getSymbol()->getOffset() + symRef->getOffset() <
              cg->getNumberBytesWriteInaccessible()) {
        needExplicitCheck = false;
      }
      ...
      needExplicitCheck = false;
               ...
               cg->getNumberBytesReadInaccessible() >
                   cg->fe()->getOffsetOfContiguousArraySizeField()) {
      ...
      //
      // This can occur when the array length is represented in bytes, not
      // elements. The optimizer must intervene for this to happen.
      //
  ...
  if (needExplicitCheck) {
    ...
    if (cg->getHasResumableTrapHandler()) {
      ...
      if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) &&
        ...
        appendTo = generateRXInstruction(
            cg, TR::InstOpCode::getLoadAndTrapOpCode(), node, targetRegister,
            generateS390MemoryReference(reference, cg), appendTo);
        ...
        if (appendTo->getOpCodeValue() == TR::InstOpCode::LLGF) {
          ...
          appendTo = generateRIEInstruction(
              cg, TR::InstOpCode::getCmpImmTrapOpCode(), node, targetRegister,
              (int16_t)0, TR::InstOpCode::COND_BE, appendTo);
        }
      } else {
        targetRegister = reference->getRegister();

        if (targetRegister == NULL) targetRegister = cg->evaluate(reference);
      ...
      if (!firstChild->getOpCode().isCall() &&
        ...
        reference->setRegister(targetRegister);
        TR::MemoryReference *tempMR =
            generateS390MemoryReference(reference, cg);
        appendTo =
            generateRXInstruction(cg, TR::InstOpCode::getLoadTestOpCode(),
                                  reference, targetRegister, tempMR, appendTo);
        tempMR->stopUsingMemRefRegister(cg);

        appendTo = generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
        ...
        if (comp->useCompressedPointers() &&
          ...
          TR::ILOpCodes loadOp = comp->il.opCodeForIndirectLoad(TR::Int32);
          TR::ILOpCodes rdbarOp =
              comp->il.opCodeForIndirectReadBarrier(TR::Int32);
          while (n->getOpCodeValue() != loadOp &&
                 n->getOpCodeValue() != rdbarOp) {
            n->setIsNonZero(true);
            n = n->getFirstChild();
          }
          n->setIsNonZero(true);
        ...
        if (n != NULL && n->getRegister() != NULL) {
          ...
          // For concurrent scavenge the source is loaded and shifted by the
          // guarded load, thus we need to use CG here for a non-zero
          // compressedrefs shift value
          if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none) {
            cmpOpCode = TR::InstOpCode::getCmpOpCode();
          } else {
            cmpOpCode = (n->getOpCode().getSize() > 4) ? TR::InstOpCode::CG
                                                       : TR::InstOpCode::C;
          }
        } else {
          targetRegister = cg->evaluate(reference);
          cmpOpCode = TR::InstOpCode::getCmpOpCode();  // reference is always an
                                                       // address type
        }

        appendTo = generateS390CompareAndBranchInstruction(
            cg, cmpOpCode, node, targetRegister, NULLVALUE, branchOpCond,
            snippetLabel, false, true, appendTo);
        TR::Instruction *cursor = appendTo;
        cursor->setExceptBranchOp();
      }
    }
  ...
  if (needLateEvaluation) {
    ...
    if (opCode.isLoad() && firstChild->getReferenceCount() == 1 &&
      ...
      if (needExplicitCheck && firstChild->getRegister() == NULL) {
        // load with reference count 1 and no register, this means load is not
        // evaluated this load is only meaningful for explicit NULL CHK it
        // should not be evaluated yet. Its reference counter will be decrease
        // later.
        cg->decReferenceCount(reference);
      } else {
        cg->decReferenceCount(firstChild);
      ...
      cg->evaluate(firstChild);
      if (fixRefCount) firstChild->incReferenceCount();
    }
    ...
    if (faultingInstruction) {
      ...
      if (debugObj) {
        debugObj->addInstructionComment(
            faultingInstruction, "Throws Implicit Null Pointer Exception");
      }
    }
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::NULLCHKEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
 ...
 * Zero check an int.
 */
TR::Register *OMR::Z::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  ...
  if (valueToCheck->getOpCode().getOpCodeValue() == TR::acmpeq) {
    TR::InstOpCode::S390BranchCondition opBranchCond = generateS390CompareOps(
        valueToCheck, cg, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE);
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC, opBranchCond, node,
                                  slowPathOOLLabel);
  } else {
  ...
  TR::Instruction *cursor = generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, slowPathOOLLabel);
  ...
  TR_S390OutOfLineCodeSection *outlinedHelperCall =
      new (cg->trHeapMemory()) TR_S390OutOfLineCodeSection(
          node, TR::call, NULL, slowPathOOLLabel, doneLabel, cg);
  ...
  // Children other than the first are only for the outlined path; we don't need
  // them here
...
/**
 * resolveAndNULLCHKEvaluator - Resolve check a static, field or method and Null
 * check the underlying pointer.  child 1 is reference to be resolved. Symbolref
 * indicates failure action/destination
 */
TR::Register *OMR::Z::TreeEvaluator::resolveAndNULLCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(node, true, cg);
}

void OMR::Z::TreeEvaluator::addToRegDep(
    TR::RegisterDependencyConditions *daaDeps, TR::Register *reg,
    bool mightUseRegPair) {
  if (!reg) return;

  // if it is GPR5 then we don't need to create deps
  if (reg->getRealRegister()) {
    if (((TR::RealRegister *)reg->getRealRegister())->getRegisterNumber() ==
  ...
  if (mightUseRegPair) {
    ...
    if (reg->getRegisterPair()) {
      ...
      daaDeps->addPostCondition(regPair->getHighOrder(),
                                TR::RealRegister::LegalEvenOfPair);
      daaDeps->addPostCondition(regPair->getLowOrder(),
                                TR::RealRegister::LegalOddOfPair);
      return;
    }
  }

  daaDeps->addPostConditionIfNotAlreadyInserted(reg,
                                                TR::RealRegister::AssignAny);
}

void OMR::Z::TreeEvaluator::createDAACondDeps(
    TR::Node *node, TR::RegisterDependencyConditions *daaDeps,
  ...
  switch (daaInstr->getKind()) {
    ...
    {
      TR::S390RXInstruction *instr =
          static_cast<TR::S390RXInstruction *>(daaInstr);
      TR::MemoryReference *mr = instr->getMemoryReference();

      TR::TreeEvaluator::addToRegDep(daaDeps, instr->getRegisterOperand(1),
                                     false);
      if (mr) {
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr->getBaseRegister()), false);
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr->getIndexRegister()),
            false);
      }
      break;
    }
    ...
    {
      TR::S390RXYInstruction *instr =
          static_cast<TR::S390RXYInstruction *>(daaInstr);
      TR::MemoryReference *mr = instr->getMemoryReference();
      ...
      if (mr) {
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr->getBaseRegister()), true);
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr->getIndexRegister()), true);
      }
      break;
    }
    case TR::Instruction::IsSS2:  // ZAP, Arithmetics
    {
      TR::S390SS2Instruction *instr =
          static_cast<TR::S390SS2Instruction *>(daaInstr);
      TR::MemoryReference *mr1 = instr->getMemoryReference();
      TR::MemoryReference *mr2 = instr->getMemoryReference2();

      if (mr1) {
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr1->getBaseRegister()),
            false);
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr1->getIndexRegister()),
            false);
      }

      if (mr2) {
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr2->getBaseRegister()),
            false);
        TR::TreeEvaluator::addToRegDep(
            daaDeps, static_cast<TR::Register *>(mr2->getIndexRegister()),
            false);
      }
      break;
    }
    case TR::Instruction::IsVRIf:  // VAP,VDP, VMP, VSP, VRP, [VMSP, VSDP
                                   // unused]
    {
      TR::TreeEvaluator::addToRegDep(daaDeps, daaInstr->getRegisterOperand(1),
                                     false);
      TR::TreeEvaluator::addToRegDep(daaDeps, daaInstr->getRegisterOperand(2),
                                     false);
      TR::TreeEvaluator::addToRegDep(daaDeps, daaInstr->getRegisterOperand(3),
                                     false);
      break;
    }
    ...
    {
      TR::S390VSIInstruction *instr =
          static_cast<TR::S390VSIInstruction *>(daaInstr);
      TR::MemoryReference *mr1 = instr->getMemoryReference();
...
TR::Node *OMR::Z::TreeEvaluator::DAAAddressPointer(TR::Node *callNode,
                                                   TR::CodeGenerator *cg) {
    ...
    if (callNode->getSymbol()
      ...
      if ((callNode->getSymbol()
        ...
        return TR::Node::create(
            TR::l2a, 1,
            TR::Node::create(
                TR::ladd, 2, pdBufAddressNode,
                TR::Node::create(
                    TR::i2l, 1,
                    TR::Node::create(TR::iadd, 2, pdBufPositionNode, index))));
      }
    }
  }
#endif
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    TR::Node *addressOffset =
        TR::Node::create(TR::aladd, 2, addressHeader, addressIndex);

    // Update the address header size
    addressHeader->setLongInt(
    ...
    TR::Node *addressOffset =
        TR::Node::create(TR::aiadd, 2, addressHeader, addressIndex);
...
TR::Register *OMR::Z::TreeEvaluator::butestEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
...
void OMR::Z::TreeEvaluator::commonButestEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR_ASSERT(
      secondChild->getOpCode().isLoadConst() && secondChild->getType().isInt8(),
...
TR::Register *OMR::Z::TreeEvaluator::inlineIfButestEvaluator(
    TR::Node *node, TR::CodeGenerator *cg, TR::Node *cmpOpNode,
    TR::Node *cmpValNode) {
  TR_ASSERT(
      cmpValNode->getOpCode().isLoadConst() && cmpValNode->getType().isInt32(),
      "Expecting constant second child for if node, found node %s (%p)\n",
      cmpValNode->getOpCode().getName(), cmpValNode);

  TR::RegisterDependencyConditions *deps =
      getGLRegDepsDependenciesFromIfNode(cg, cmpOpNode);

  TR::TreeEvaluator::commonButestEvaluator(node, cg);

  TR::InstOpCode::S390BranchCondition cond = getButestBranchCondition(
      cmpOpNode->getOpCodeValue(), cmpValNode->getInt());
  TR::Instruction *instr = generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, cond, cmpOpNode,
...
TR::Register *OMR::Z::TreeEvaluator::inlineIfBifEvaluator(
    TR::Node *ifNode, TR::CodeGenerator *cg, TR::Node *cmpOpNode,
    TR::Node *cmpValNode) {
  TR_ASSERT(
      cmpValNode->getOpCode().isLoadConst() && cmpValNode->getType().isInt32(),
      "Expecting constant second child for if node, found node %s (%p)\n",
  ...
  if (ifNode->getFirstChild()->getOpCodeValue() == TR::bu2i) {
    ...
    //      cmpOpNode
    //    cmpValNode
    // We have previously verified that cmpOpNode isSingleRefUnevaluated(), so
    // therefore the bu2i must also be isSingleRefEvaluated() and we should dec
    // ref count here
...
OMR::Z::TreeEvaluator::getBranchConditionFromCompareOpCode(
    TR::ILOpCodes opCode) {
  switch (opCode) {
    case TR::icmpeq:
    case TR::acmpeq:
    ...
    default: {
      ...
      return TR::InstOpCode::COND_BE;  // not a valid return value.  We should
                                       // never ever get here.
    } break;
  }
}
...
/**
 * since load on condition loads the conditional value when one of the following
 * is true, we must do the opposite to mimic branch behaviour, which would
 * branch around the load when one of the following is true.
 */
TR::InstOpCode::S390BranchCondition
OMR::Z::TreeEvaluator::mapBranchConditionToLOCRCondition(
...
TR::InstOpCode::Mnemonic OMR::Z::TreeEvaluator::getCompareOpFromNode(
    TR::CodeGenerator *cg, TR::Node *node) {
  TR_ASSERT(
      node->getOpCode().isBooleanCompare() && node->getNumChildren() > 0,
      "node %p is not a compare opcode, or compare opcode has 0 children!\n",
      node);
  TR_ASSERT(node->getFirstChild()->getType().isIntegral() ||
...
int32_t OMR::Z::TreeEvaluator::countReferencesInTree(TR::Node *treeNode,
                                                     TR::Node *node) {
  if (treeNode->getVisitCount() >= TR::comp()->getVisitCount()) return 0;

  treeNode->setVisitCount(TR::comp()->getVisitCount());

  if (treeNode->getRegister()) return 0;

  if (treeNode == node) return 1;

  int32_t sum = 0;
  for (int32_t i = 0; i < treeNode->getNumChildren(); i++) {
    sum +=
        TR::TreeEvaluator::countReferencesInTree(treeNode->getChild(i), node);
  }
  return sum;
}

bool OMR::Z::TreeEvaluator::treeContainsAllOtherUsesForNode(TR::Node *treeNode,
                                                            TR::Node *node) {
  ...
  if (numberOfInstancesToFind == 0) return true;

  int32_t instancesFound =
      TR::TreeEvaluator::countReferencesInTree(treeNode, node);
...
TR::Register *OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    traceMsg(comp,
             "Starting evaluation of ternary node %p condition %p (in reg %p) "
             "trueVal %p (in reg %p) falseVal %p (in reg %p)\n",
             node, condition, condition->getRegister(), trueVal,
             trueVal->getRegister(), falseVal, falseVal->getRegister());

  TR::Register *trueReg = 0;
  if (TR::TreeEvaluator::treeContainsAllOtherUsesForNode(condition, trueVal) &&
      (!trueVal->isUnneededConversion() ||
       TR::TreeEvaluator::treeContainsAllOtherUsesForNode(
           condition, trueVal->getFirstChild()))) {
    if (comp->getOption(TR_TraceCG))
      traceMsg(comp,
               "Calling evaluate (instead of clobber evaluate for node %p "
               "because all other uses are in the compare tree)\n",
               trueVal);
  ...
  TR::Register *falseReg =
      cg->evaluate(falseVal);  // cg->gprClobberEvaluate(falseVal);

  if (comp->getOption(TR_TraceCG))
    traceMsg(comp, "Done evaluating child %p in reg %p and %p in reg %p\n",
             trueVal, trueReg, falseVal, falseReg);

  // I don't want to evaluate the first child as icmp evaluator places value in
  // an integer, which we can short circuit.
  if (condition->getOpCode().isBooleanCompare() &&
    ...
    TR::InstOpCode::Mnemonic compareOp =
        TR::TreeEvaluator::getCompareOpFromNode(cg, condition);
    ...
    auto bc = TR::TreeEvaluator::getBranchConditionFromCompareOpCode(
        condition->getOpCodeValue());

    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z15)) {
      generateRRInstruction(cg, compareOp, node, firstReg, secondReg);

      auto mnemonic = trueVal->getOpCode().is8Byte() ? TR::InstOpCode::SELGR
                                                     : TR::InstOpCode::SELR;
      generateRRFInstruction(
          cg, mnemonic, node, trueReg, falseReg, trueReg,
          getMaskForBranchCondition(
              TR::TreeEvaluator::mapBranchConditionToLOCRCondition(bc)));
    } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      generateRRInstruction(cg, compareOp, node, firstReg, secondReg);

      auto mnemonic = trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR
                                                     : TR::InstOpCode::LOCR;
      generateRRFInstruction(
          cg, mnemonic, node, trueReg, falseReg,
          getMaskForBranchCondition(
              TR::TreeEvaluator::mapBranchConditionToLOCRCondition(bc)),
          true);
    } else {
      ...
      generateRRInstruction(cg,
                            trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LGR
                                                           : TR::InstOpCode::LR,
                            node, trueReg, falseReg);

      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                   cFlowRegionEnd, conditions);
      cFlowRegionEnd->setEndInternalControlFlow();

      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp,
                 "firstReg = %p secondReg = %p trueReg = %p falseReg = %p "
                 "trueReg->is64BitReg() = %d falseReg->is64BitReg() = %d "
                 "isLongCompare = %d\n",
    ...
    TR::Instruction *compareInst = generateRILInstruction(
        cg,
        condition->getOpCode().isLongCompare() ? TR::InstOpCode::CGFI
                                               : TR::InstOpCode::CFI,
        condition, condition->getRegister(), 0);
  ...
  if (!node->isNotCollected()) trueReg->setContainsCollectedReference();
...
// Take advantage of VRF/FPR overlap and use vector compare and select
// instructions to implement dternary
TR::Register *OMR::Z::TreeEvaluator::dternaryEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  ...
  if (conditionNode->getOpCode().isCompareDouble()) {
    ...
    // To handle unordered compares, we switch the compare operation, since the
    // vector compare instructions return false for unordered. For example, if
    // the original compare is dcmpgeu:
    //    (x >= y [true if unordered]) ? a : b
    //    switch to (x < y) ? b : a;
    // So that only if x < y, do we select the "false value"

    if (ifOp == TR::dcmpgeu || ifOp == TR::dcmpgtu || ifOp == TR::dcmplt ||
        ifOp == TR::dcmple)
      generateVRRcInstruction(cg, compareOp, node, vectorSelReg, secondCmpReg,
                              firstCmpReg, 1, 0, 3);
    else
      generateVRRcInstruction(cg, compareOp, node, vectorSelReg, firstCmpReg,
                              secondCmpReg, 1, 0, 3);

    // The above operation returns 0's for not equals, so we must swap the true
    // and false values to select the correct "true value" for cmpne operations
    // Swap the values for unordered comparisons also (see above)
    if (ifOp == TR::dcmpne ||
        conditionNode->getOpCode().isCompareTrueIfUnordered())
      generateVRReInstruction(cg, TR::InstOpCode::VSEL, node, returnReg,
                              falseValueReg, trueValueReg, vectorSelReg);
    else
      generateVRReInstruction(cg, TR::InstOpCode::VSEL, node, returnReg,
                              trueValueReg, falseValueReg, vectorSelReg);
    ...
    generateRRInstruction(cg, TR::InstOpCode::LDGR, node, tempReg,
                          conditionReg);

    // generate compare with zero
    generateVRIaInstruction(cg, TR::InstOpCode::VGBM, node, vzeroReg, 0, 0);
    generateVRRcInstruction(cg, TR::InstOpCode::VFCE, node, vectorSelReg,
                            tempReg, vzeroReg, 1, 0, 3);

    // generate select - if condition == 0, vectorSelReg will contain all 1s, so
    // false and true are swapped
    generateVRReInstruction(cg, TR::InstOpCode::VSEL, node, returnReg,
                            falseValueReg, trueValueReg, vectorSelReg);
