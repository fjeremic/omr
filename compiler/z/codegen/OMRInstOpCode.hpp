...
public:
  ...
  uint64_t gprOp()
  {
    return (metadata[_mnemonic].properties & (S390OpProp_DoubleFP | S390OpProp_SingleFP)) == 0;
  }
  uint64_t fprOp()
  {
    return metadata[_mnemonic].properties & (S390OpProp_DoubleFP | S390OpProp_SingleFP);
  }
  ...
  uint64_t shouldUseRegPairForTarget()
  {
    return usesRegPairForTarget();
  }
  ...
  static uint8_t getInstructionLength(
    Mnemonic i_opCode); /* Using a table that maps values of bit 0-1 to instruction length from POP chapter 5,  topic:
  ...
  uint8_t getInstructionLength()
  {
    return getInstructionLength(_mnemonic);
  }
  uint8_t getInstructionFormat()
  {
    return getInstructionFormat(_mnemonic);
  }
