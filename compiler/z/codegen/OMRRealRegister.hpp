...
protected:
  ...
  RealRegister(TR_RegisterKinds, uint16_t, RegState, RegNum, RegMask,
               TR::CodeGenerator *);

public:
  // methods for manipulating flags
  bool getModified() { return _modified.testAny(isModified); }
  bool setModified(bool b) {
    (b) ? _modified.set(isModified) : _modified.reset(isModified);
    return b;
  }
  ...
  void setRegisterField(uint32_t *instruction,
                        int32_t nibbleIndex); // nibbleIndex-> rightmost
                                              // nibble=0, leftmost nibble=7
  ...
  static void setRegisterField(uint32_t *instruction, int32_t nibbleIndex,
                               RegNum reg);
