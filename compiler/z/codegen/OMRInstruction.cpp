...
#define CTOR_INITIALIZER_LIST                                                  \
  _conditions(NULL), _operands(NULL), _useRegs(NULL), _defRegs(NULL),          \
    ...
    _sourceRegSize(0), _sourceMemSize(0), _targetMemSize(0), _sourceStart(-1), \
    _targetStart(-1)
...
TR::RegisterDependencyConditions*
OMR::Z::Instruction::setDependencyConditions(
  TR::RegisterDependencyConditions* cond)
{
  ...
  cond->bookKeepingRegisterUses(
    self(), self()->cg(), oldPreAddCursor, oldPostAddCursor);
  cond->setIsUsed();
  ...
  TR::RegisterDependencyConditions* cond)
{
  ...
  cond->bookKeepingRegisterUses(
    self(), self()->cg(), currentPreCursor, currentPostCursor);
  cond->setIsUsed();
  return _conditions = cond;
}

void
OMR::Z::Instruction::initialize(TR::Instruction* precedingInstruction,
                                bool instFlag,
                                TR::RegisterDependencyConditions* cond,
                                bool condFlag)
{
...
bool
OMR::Z::Instruction::matchesAnyRegister(TR::Register* reg,
                                        TR::Register* instReg)
{
  if (!instReg)
    return false;
...
bool
OMR::Z::Instruction::usesOnlyRegister(TR::Register* reg)
{
  int32_t i;
...
static bool
isInternalControlFlowOneEntryOneExit(TR::Instruction* regionEnd,
                                     TR::Compilation* comp)
{
...
void
OMR::Z::Instruction::setupThrowsImplicitNullPointerException(
  TR::Node* n,
  TR::MemoryReference* mr)
{
  ...
  if (self()->cg()->getSupportsImplicitNullChecks()) {
    ...
    if (n != NULL & mr != NULL && mr->getCausesImplicitNullPointerException() &&
      ...
      TR::Node* firstChild =
        self()->cg()->getCurrentEvaluationTreeTop()->getNode()->getFirstChild();
  ...
  if (self()->getOpCodeValue() != TR::InstOpCode::ASSOCREGS) {
    ...
    // if (kindToBeAssigned == TR_GPR)
    {
      ...
      for (int32_t i = first; i <= last; ++i) {
        TR::Register* virtReg =
          machine->getVirtualAssociatedWithReal((TR::RealRegister::RegNum)(i));
        if (virtReg) {
    ...
    for (int32_t i = TR::RealRegister::GPR0; i != TR::RealRegister::GPR15 + 1;
      ...
      if (0 != self()->cg()->getS390Linkage()->getPreserved(
        ...
        linkageBasedSaveMask |=
          (1 << (7 - ((i - 1) >> 1))); // bit 0 = GPR0/1, GPR0=1, GPR15=16. 'Or'
                                       // with bit [(i-1)>>1]
      }
    }
 ...
 */
bool
OMR::Z::Instruction::containsRegister(TR::Register* reg)
{
  TR::Register** _sourceReg = self()->sourceRegBase();
  TR::Register** _targetReg = self()->targetRegBase();
  TR::MemoryReference** _sourceMem = self()->sourceMemBase();
...
uint32_t
OMR::Z::Instruction::useTargetMemoryReference(TR::MemoryReference* memRef,
...
//  elements
int
OMR::Z::Instruction::gatherRegPairsAtFrontOfArray(TR::Register** regArr,
                                                  int32_t regArrSize)
{
  int32_t pairCnt = 0;
...
//
bool
OMR::Z::Instruction::anySpilledRegisters(TR::Register** listReg,
                                         int32_t listRegSize)
{
  int32_t i;
...
bool
OMR::Z::Instruction::anySpilledRegisters(TR::MemoryReference** listMem,
                                         int32_t listMemSize)
{
  int32_t i;
...
void
OMR::Z::Instruction::assignRegistersAndDependencies(
...
void
OMR::Z::Instruction::block(TR::Register** sourceReg,
                           int sourceRegSize,
...
void
OMR::Z::Instruction::unblock(TR::Register** sourceReg,
                             int sourceRegSize,
                             TR::Register** targetReg,
                             int targetRegSize,
                             TR::MemoryReference** sourceMem,
...
bool
OMR::Z::Instruction::checkRegForGPR0Disable(TR::InstOpCode::Mnemonic op,
                                            TR::Register* reg)
{
...
bool
OMR::Z::Instruction::getTwoLocalLocalAllocFreeReg(TR::RealRegister** reg1,
...
void
OMR::Z::Instruction::setMaskField(uint32_t* instruction,
                                  int8_t mask,
                                  int8_t nibbleIndex)
{
