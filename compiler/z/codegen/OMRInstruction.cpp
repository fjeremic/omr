...
{
  TR_ASSERT_FATAL(TR::Compiler->target.cpu.getSupportsArch(_opcode.getMinimumALS()),
...
void OMR::Z::Instruction::setCCInfo()
{
  ...
  if (_opcode.setsCC()) {
    ...
    if (!self()->cg()->comp()->getOption(TR_EnableEBBCCInfo)) {
      ...
      if (_opcode.setsOverflowFlag() || _opcode.setsZeroFlag() || _opcode.setsSignFlag() || _opcode.setsCompareFlag()
...
void OMR::Z::Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
  ...
  if (outOfLineEXInstr) {
    ...
    outOfLineEXInstr->setPrev(self()->getPrev()); // Temporarily set Prev() instruction of snippet to Prev() of EX just
                                                  // in case we insert LR_move on assignRegisters()
  ...
  if (debugObj && comp->getOption(TR_TraceRegisterState)) {
    ...
    char* comment
      = (char*)self()->cg()->trMemory()->allocateHeapMemory(strlen(commentTemplate) + strlen(strFreeRegs) - 1);
    if (comment != NULL) {
    ...
    && (_opcode.is64bit() || _opcode.is32to64bit()
      || (TR::Compiler->target.is64Bit()
        && (self()->getOpCodeValue() == TR::InstOpCode::LA || self()->getOpCodeValue() == TR::InstOpCode::LAY
          || self()->getOpCodeValue() == TR::InstOpCode::LARL || self()->getOpCodeValue() == TR::InstOpCode::BASR
...
void OMR::Z::Instruction::assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned)
{
  ...
  //
  if (numTrgtPairs == 0 && _targetReg && _targetRegSize == 1 && _targetMem == NULL && !self()->getOpCode().usesTarget()
  ...
  if (_targetReg) {
    for (i = 0; i < _targetRegSize; ++i) {
      ...
      if (_targetReg[i]->is64BitReg() && _targetReg[i]->getRealRegister() == NULL && _targetReg[i]->getKind() != TR_FPR
  ...
  // srcReg assignment)." "We only unblock those target registers that are not alive anymore. Block them again after pre
  // conds are done."
  ...
  if (_sourceReg) {
    for (i = 0; i < _sourceRegSize; ++i) {
      ...
      if (regPair != NULL) {
        ...
        } else if (self()->getOpCodeValue() != TR::InstOpCode::LM && self()->getOpCodeValue() != TR::InstOpCode::LMG
    ...
    for (i = 0; i < _targetRegSize; ++i) {
      ...
      if (regPair != NULL) {
        ...
        if ((self()->isStore() || self()->isCompare() || self()->isTrap() || self()->getOpCode().usesTarget()
          ...
          && (self()->getOpCodeValue() != TR::InstOpCode::EDMK) && (self()->getOpCodeValue() != TR::InstOpCode::TRTR)) {
