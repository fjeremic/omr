...
void OMR::Z::Instruction::initialize(TR::Instruction* precedingInstruction, bool instFlag, TR::RegisterDependencyConditions* cond, bool condFlag)
{
...
void OMR::Z::Instruction::setupThrowsImplicitNullPointerException(TR::Node* n, TR::MemoryReference* mr)
{
    ...
    if (self()->cg()->getSupportsImplicitNullChecks()) {
        ...
        if (n != NULL & mr != NULL && mr->getCausesImplicitNullPointerException() && self()->cg()->getCurrentEvaluationTreeTop()->getNode()->getOpCode().isNullCheck() && self()->cg()->getImplicitExceptionPoint() == NULL) {
            ...
            if ((self()->getNode()->getOpCode().hasSymbolReference() && self()->getNode()->getSymbolReference() == comp->getSymRefTab()->findVftSymbolRef()) || (n->hasChild(nullCheckReference) && mr->usesRegister(nullCheckReference->getRegister()))) {
...
OMR::Z::Instruction::useTargetRegister(TR::Register* reg)
{
    ...
    if (reg->getKind() == TR_GPR && (_opcode.is64bit() || _opcode.is32to64bit() || (TR::Compiler->target.is64Bit() && (self()->getOpCodeValue() == TR::InstOpCode::LA || self()->getOpCodeValue() == TR::InstOpCode::LAY || self()->getOpCodeValue() == TR::InstOpCode::LARL || self()->getOpCodeValue() == TR::InstOpCode::BASR || self()->getOpCodeValue() == TR::InstOpCode::BRASL)))) {
        reg->setIs64BitReg(true);
...
OMR::Z::Instruction::assignRegisterNoDependencies(TR::Register* reg)
{
    ...
    if (reg->getRealRegister() != NULL) {
        ...
        if (self()->getOpCodeValue() == TR::InstOpCode::BCR && toRealRegister(reg)->getRegisterNumber() == TR::RealRegister::GPR0 && (((TR::S390RegInstruction*)self())->getBranchCondition() == TR::InstOpCode::COND_MASK14 || ((TR::S390RegInstruction*)self())->getBranchCondition() == TR::InstOpCode::COND_MASK15))
            skipBookkeeping = true;

        if (!skipBookkeeping && realReg->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtReg && virtReg->decFutureUseCount() == 0) {
    ...
    if ((reg->getRegisterPair() && (reg->getRegisterPair()->getHighOrder()->getRealRegister() != NULL || reg->getRegisterPair()->getLowOrder()->getRealRegister() != NULL))) {
        ...
        if (realRegHigh->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtRegHigh && virtRegHigh->decFutureUseCount() == 0) {
        ...
        if (realRegLow->getState() != TR::RealRegister::Locked && BOOKKEEPING && virtRegLow && virtRegLow->decFutureUseCount() == 0) {
...
bool OMR::Z::Instruction::anySpilledRegisters(TR::MemoryReference** listMem, int32_t listMemSize)
{
    ...
    for (i = 0; i < listMemSize; ++i) {
        ...
        TR::Register* reg;
        if ((reg = memRef->getBaseRegister()) && reg->getAssignedRealRegister() == NULL && (reg->getTotalUseCount() != reg->getFutureUseCount())) {
            return true;
        }
        if ((reg = memRef->getIndexRegister()) && reg->getAssignedRealRegister() == NULL && (reg->getTotalUseCount() != reg->getFutureUseCount())) {
            return true;
        }
...
void OMR::Z::Instruction::assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    //
    if (numTrgtPairs == 0 && _targetReg && _targetRegSize == 1 && _targetMem == NULL && !self()->getOpCode().usesTarget() && //ii: !getOpCode().isRegCopy() &&
    ...
    if (_targetReg) {
        for (i = 0; i < _targetRegSize; ++i) {
            ...
            if (_targetReg[i]->is64BitReg() && _targetReg[i]->getRealRegister() == NULL && _targetReg[i]->getKind() != TR_FPR && _targetReg[i]->getKind() != TR_VRF) {
...
void OMR::Z::Instruction::assignRegistersAndDependencies(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (self()->getOpCode().isBranchOp() && self()->getKind() == IsRIE && ((TR::S390RIEInstruction*)self())->getLabelSymbol()->isStartOfColdInstructionStream()) {
...
void OMR::Z::Instruction::block(TR::Register** sourceReg, int sourceRegSize, TR::Register** targetReg, int targetRegSize,
...
void OMR::Z::Instruction::unblock(TR::Register** sourceReg, int sourceRegSize, TR::Register** targetReg, int targetRegSize,
    TR::MemoryReference** sourceMem, TR::MemoryReference** targetMem)
{
    int32_t i;
...
bool OMR::Z::Instruction::checkRegForGPR0Disable(TR::InstOpCode::Mnemonic op, TR::Register* reg)
{
    ...
    if (reg->getRealRegister() == NULL && ((self()->getOpCode().isBranchOp() && self()->getOpCode().getInstructionFormat() == RR_FORMAT) || self()->getOpCodeValue() == TR::InstOpCode::EX || self()->getOpCodeValue() == TR::InstOpCode::EXRL)) {
...
void OMR::Z::Instruction::setUseDefRegisters(bool updateDependencies)
{
    ...
    if ((_sourceReg != NULL) || (_sourceMem != NULL) || (_targetMem != NULL) || (self()->implicitlyUsesGPR0() || self()->implicitlyUsesGPR1() || self()->implicitlyUsesGPR2()) || ((_targetReg != NULL) && (self()->isStore() || self()->isCompare() || self()->isTrap() || self()->getOpCode().usesTarget()))) {
        if (_useRegs == NULL) {
            _useRegs = new (self()->cg()->comp()->allocator()) RegisterArray<TR::Register*>(self()->cg()->comp());
        }
    }
    if ((self()->implicitlySetsGPR0() || self()->implicitlySetsGPR1() || self()->implicitlySetsGPR2()) || ((_targetReg != NULL) && ((!self()->isStore() && (!self()->isCompare()) && (!self()->isTrap()) && (self()->getOpCodeValue() != TR::InstOpCode::PPA)) || self()->isLoad()) || (dependencies != NULL))) {
        if (_defRegs == NULL) {
    ...
    if (_sourceReg) {
        for (i = 0; i < _sourceRegSize; ++i) {
            ...
            if (regPair != NULL) {
                ...
                } else if (self()->getOpCodeValue() != TR::InstOpCode::LM && self()->getOpCodeValue() != TR::InstOpCode::LMG && self()->getOpCodeValue() != TR::InstOpCode::LMH && self()->getOpCodeValue() != TR::InstOpCode::LMY) {
                    (*_useRegs)[indexSource++] = _sourceReg[i];
                } else {
                    (*_defRegs)[indexTarget++] = _sourceReg[i];
                }
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::LM || self()->getOpCodeValue() == TR::InstOpCode::LMG || self()->getOpCodeValue() == TR::InstOpCode::LMH || self()->getOpCodeValue() == TR::InstOpCode::LMY)
        loadOrStoreMultiple = 0; //load
    else if (self()->getOpCodeValue() == TR::InstOpCode::STM || self()->getOpCodeValue() == TR::InstOpCode::STMG || self()->getOpCodeValue() == TR::InstOpCode::STMH || self()->getOpCodeValue() == TR::InstOpCode::STMY)
