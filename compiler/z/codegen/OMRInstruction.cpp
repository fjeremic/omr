...
#define CTOR_INITIALIZER_LIST                                                  \
  _conditions(NULL), _operands(NULL), _useRegs(NULL), _defRegs(NULL),          \
      _sourceUsedInMemoryReference(NULL), _flags(0), _longDispSpillReg1(NULL), \
      _longDispSpillReg2(NULL), _binFreeRegs(0), _targetRegSize(0),            \
      _sourceRegSize(0), _sourceMemSize(0), _targetMemSize(0),                 \
      _sourceStart(-1), _targetStart(-1)

OMR::Z::Instruction::Instruction(TR::CodeGenerator *cg,
                                 TR::InstOpCode::Mnemonic op, TR::Node *node)
    : OMR::Instruction(cg, op, node), CTOR_INITIALIZER_LIST {
  TR_ASSERT_FATAL(
      TR::Compiler->target.cpu.getSupportsArch(_opcode.getMinimumALS()),
      "Processor detected does not support instruction %s\n",
...
OMR::Z::Instruction::setDependencyConditionsNoBookKeeping(
    TR::RegisterDependencyConditions *cond) {
  // If register dependency conditions exist, merge with the argument conditions
  if (_conditions) {
    TR::RegisterDependencyConditions *n = new (self()->cg()->trHeapMemory())
        TR::RegisterDependencyConditions(_conditions, cond, self()->cg());
    _conditions = 0;
    cond = n;
  }
...
TR::RegisterDependencyConditions *OMR::Z::Instruction::setDependencyConditions(
    TR::RegisterDependencyConditions *cond) {
  ...
  cond->bookKeepingRegisterUses(self(), self()->cg(), oldPreAddCursor,
                                oldPostAddCursor);
  cond->setIsUsed();
...
TR::RegisterDependencyConditions *
OMR::Z::Instruction::updateDependencyConditions(
    TR::RegisterDependencyConditions *cond) {
  ...
  cond->bookKeepingRegisterUses(self(), self()->cg(), currentPreCursor,
                                currentPostCursor);
  cond->setIsUsed();
  return _conditions = cond;
}

void OMR::Z::Instruction::initialize(TR::Instruction *precedingInstruction,
                                     bool instFlag,
                                     TR::RegisterDependencyConditions *cond,
                                     bool condFlag) {
  ...
  if (cg->getCondCodeShouldBePreserved())
    TR_ASSERT(
        (_opcode.setsOverflowFlag() || _opcode.setsZeroFlag() ||
         _opcode.setsSignFlag() || _opcode.setsCarryFlag()) == false,
        "We thought the instruction [0x%p] won't modify the CC, but it does.\n",
        self());

  self()->readCCInfo();  // Must come before setCCInfo - Instruction reads CC
                         // before setting CC.

  if (cg->comp()->getOption(TR_EnableEBBCCInfo)) {
    ((self()->isLabel() || self()->isCall()) ||
     (instFlag && precedingInstruction == NULL))
        ? self()->clearCCInfo()
        : self()->setCCInfo();
  } else {
    (!self()->isBranchOp() && (!instFlag || precedingInstruction == NULL))
        ? self()->setCCInfo()
        : self()->clearCCInfo();
  }
}
...
// Applicable to frontends which does not do binary encoding (i.e., not Java or
// OMR). Therefore, we will reuse the field for the binary buffer pointer to
// store the binary offset. This way we dont use an extra 4 byte field in
// TR::Instruction
...
void OMR::Z::Instruction::setCCInfo() {
  if (self()->getNode() == NULL) {
    // play it safe... perhaps this should be changed to an assert that the node
  ...
  if (_opcode.setsCC()) {
    ...
    if (!self()->cg()->comp()->getOption(TR_EnableEBBCCInfo)) {
      if (_opcode.setsOverflowFlag() || _opcode.setsZeroFlag() ||
          _opcode.setsSignFlag() || _opcode.setsCarryFlag()) {
        // we have detailed information on the CC setting
        self()->cg()->setHasCCInfo(true);
        self()->cg()->setHasCCOverflow(_opcode.setsOverflowFlag() &&
                                       !self()->getNode()->cannotOverflow());
        self()->cg()->setHasCCZero(_opcode.setsZeroFlag());
        self()->cg()->setHasCCSigned(_opcode.setsSignFlag() ||
                                     _opcode.setsCompareFlag());
      ...
      if (_opcode.setsOverflowFlag() || _opcode.setsZeroFlag() ||
          _opcode.setsSignFlag() || _opcode.setsCompareFlag() ||
          _opcode.setsCarryFlag())
      // track Ops that setsCompareFlag for compare ops as well, so that we may
      // remove redudent compare ops; note this will not intruduce mismatching
      // for compare immediate ops with CC from compare Ops, because compare
      // immed ops looks for setsSignedFlag or setsZeroFlags - compare ops does
      // not sets these flags
      {
        // we have detailed information on the CC setting
        self()->cg()->setHasCCInfo(true);
        self()->cg()->setHasCCOverflow(_opcode.setsOverflowFlag() &&
        ...
        // the CC setting is generic, such as Load & Test Reg, Test Under Mask
        // etc are not handled; or, there is no node associated with this
        // instruction, play it safe and record no information
        self()->cg()->setHasCCInfo(false);
      }
      self()->cg()->setCCInstruction(self());
    }  // if enableEBBCCInfo
...
bool OMR::Z::Instruction::matchesAnyRegister(TR::Register *reg,
                                             TR::Register *instReg) {
  if (!instReg) return false;
...
bool OMR::Z::Instruction::matchesAnyRegister(TR::Register *reg,
                                             TR::Register *instReg1,
                                             TR::Register *instReg2) {
  return self()->matchesAnyRegister(reg, instReg1) ||
         self()->matchesAnyRegister(reg, instReg2);
}

TR::Register *OMR::Z::Instruction::getRegForBinaryEncoding(TR::Register *reg) {
  return (reg->getRegisterPair()) ? reg->getRegisterPair()->getHighOrder()
                                  : reg;
}
  ...
  if (regPair) {
    ...
    if (firstRegister->getRealRegister() == NULL) {
      TR_ASSERT(firstRegister->getSiblingRegister(),
                "[%p] Register Pairs should be allocated using "
                "allocateConsecutivePair.\n",
                self());
    }
    ...
    // If an instruction uses a dummy register, that register should no longer
    // be considered dummy. S390RegisterDependency also calls useRegister, in
    // this case we do not want to reset the dummy status of these regs
    //
...
bool OMR::Z::Instruction::refsRegister(TR::Register *reg) {
  return self()->getDependencyConditions()
             ? self()->getDependencyConditions()->refsRegister(reg)
             : false;
}
...
bool OMR::Z::Instruction::defsAnyRegister(TR::Register *reg) {
  ...
  if (_targetReg) {
    for (int32_t i = 0; i < _targetRegSize; ++i) {
      if (_targetReg[i]->usesAnyRegister(reg)) return true;
    }
  }
  return false;
}
...
bool OMR::Z::Instruction::defsRegister(TR::Register *reg) {
  ...
  if (_targetReg) {
    for (int32_t i = 0; i < _targetRegSize; ++i) {
      if (_targetReg[i]->usesRegister(reg)) return true;
    }
  }
...
bool OMR::Z::Instruction::isDefRegister(TR::Register *reg) {
  ...
  if (self()->matchesTargetRegister(reg)) return true;

  // Test explicitly targeted register by the instruction
  if (_defRegs) {
    for (int32_t i = 0; i < _defRegs->size(); ++i) {
      if ((*_defRegs)[i]->usesRegister(reg)) return true;
    }
  }
...
bool OMR::Z::Instruction::usesOnlyRegister(TR::Register *reg) {
  ...
  for (i = 0; i < _sourceRegSize; ++i) {
    if (_sourceReg[i]->usesRegister(reg)) return true;
  }

  for (i = 0; i < _sourceMemSize; ++i) {
    if (_sourceMem[i]->usesRegister(reg)) return true;
  }

  for (i = 0; i < _targetMemSize; ++i) {
    if (_targetMem[i]->usesRegister(reg)) return true;
  }
  ...
  if (outOfLineEXInstr && outOfLineEXInstr->usesOnlyRegister(reg)) return true;
...
bool OMR::Z::Instruction::usesRegister(TR::Register *reg) {
  ...
  if (self()->usesOnlyRegister(reg) || self()->defsRegister(reg)) return true;

  // The only thing that is not covered by defsRegister is target registers
  // found in EX We might have an out of line EX instruction.  If so, we need to
  // check whether either the EX instruction OR the instruction it refers to
...
static bool isInternalControlFlowOneEntryOneExit(TR::Instruction *regionEnd,
                                                 TR::Compilation *comp) {
  ...
  for (curr = regionEnd; !done; curr = curr->getPrev()) {
    if (curr->isLabel()) {
      TR::LabelSymbol *labelSym =
          toS390LabelInstruction(curr)->getLabelSymbol();
  ...
  for (curr = regionEnd; curr != regionStart; curr = curr->getPrev()) {
    if (curr->isBranchOp()) {
      TR::S390LabeledInstruction *labeledInstr =
          (TR::S390LabeledInstruction *)curr;
      ...
      if (!found) return false;  // Found a branch to an outside label
    }
  }
...
TR::Instruction *OMR::Z::Instruction::getOutOfLineEXInstr() {
  if (!self()->isOutOfLineEX()) return NULL;
  switch (self()->getOpCodeValue()) {
    case TR::InstOpCode::EX: {
      TR::MemoryReference *tempMR =
          ((TR::S390RXInstruction *)self())->getMemoryReference();
      TR::S390ConstantInstructionSnippet *cis =
          (TR::S390ConstantInstructionSnippet *)
              tempMR->getConstantDataSnippet();
      TR_ASSERT(cis != NULL,
                "Out of line EX instruction doesn't have a "
                "constantInstructionSnippet\n");
      return cis->getInstruction();
    }
    case TR::InstOpCode::EXRL: {
      TR::S390ConstantInstructionSnippet *cis =
          (TR::S390ConstantInstructionSnippet *)((TR::S390RILInstruction *)
                                                     self())
              ->getTargetSnippet();
      TR_ASSERT(cis != NULL,
                "Out of line EXRL instruction doesn't have a "
                "constantInstructionSnippet\n");
      return cis->getInstruction();
    }
    default:
...
void OMR::Z::Instruction::setupThrowsImplicitNullPointerException(
    TR::Node *n, TR::MemoryReference *mr) {
  ...
  if (self()->cg()->getSupportsImplicitNullChecks()) {
    ...
    // 2. The memory reference of this instruction can cause a null pointer
    // exception
    // 3. The null check reference node must be a child of this node
    // 4. This memory reference uses the same register as the null check
    // reference
    // 5. This is the first instruction in the evaluation of this null check
    // node to have met all the conditions

    // Test conditions 1, 2, and 5
    if (n != NULL & mr != NULL && mr->getCausesImplicitNullPointerException() &&
        self()
            ->cg()
            ->getCurrentEvaluationTreeTop()
            ->getNode()
            ->getOpCode()
            .isNullCheck() &&
      ...
      if (comp->useCompressedPointers() &&
        ...
        while (firstChild->getOpCodeValue() != loadOp &&
               firstChild->getOpCodeValue() != rdbarOp)
          firstChild = firstChild->getFirstChild();
        nullCheckReference = firstChild->getFirstChild();
      } else
        nullCheckReference = self()
                                 ->cg()
                                 ->getCurrentEvaluationTreeTop()
                                 ->getNode()
                                 ->getNullCheckReference();

      // Test conditions 3 and 4
      if ((self()->getNode()->getOpCode().hasSymbolReference() &&
           self()->getNode()->getSymbolReference() ==
               comp->getSymRefTab()->findVftSymbolRef()) ||
          (n->hasChild(nullCheckReference) &&
           mr->usesRegister(nullCheckReference->getRegister()))) {
        traceMsg(
            comp,
...
TR::S390ImmInstruction *OMR::Z::Instruction::getS390ImmInstruction() {
  return NULL;
}
#endif
...
bool OMR::Z::Instruction::dependencyRefsRegister(TR::Register *reg) {
  return false;
}
...
void OMR::Z::Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned) {
  ...
  if (self()->getOpCodeValue() != TR::InstOpCode::ASSOCREGS) {
    ...
    // if (kindToBeAssigned == TR_GPR)
    {
      ...
      // Step 1 : First traverse the existing associations and remove them so
      // that they don't interfere with the new ones
      for (int32_t i = first; i <= last; ++i) {
        TR::Register *virtReg = machine->getVirtualAssociatedWithReal(
      ...
      // Step 2 : loop through and set up the new associations (both on the
      // machine and by associating the virtual registers with their real
      // dependencies)
      TR_S390RegisterDependencyGroup *depGroup =
          self()->getDependencyConditions()->getPostConditions();
      for (int32_t j = 0; j < last; ++j) {
        TR::Register *virtReg =
            depGroup->getRegisterDependency(j)->getRegister();
        machine->setVirtualAssociatedWithReal((TR::RealRegister::RegNum)(j + 1),
                                              virtReg);
      }
  ...
  // If we are register assigning an EX or EXRL instruction with the
  // isOutOfLineEX flag set we must register assign the instruction object
  // hanging off of the memory reference's constantInstructionSnippet
  TR::Instruction *outOfLineEXInstr = self()->getOutOfLineEXInstr();
  if (outOfLineEXInstr) {
    TR::Instruction *savePrev = outOfLineEXInstr->getPrev();

    outOfLineEXInstr->setPrev(
        self()->getPrev());  // Temporarily set Prev() instruction of snippet to
                             // Prev() of EX just in case we insert LR_move on
                             // assignRegisters()
    ...
    // Java and other languages probably need the value field to be set to the
    // binary encoding of the instruction This can be done any time after
    // register assignment If the value is set here, the snippet can be handled
    // in an identical fashion to a normal constantdataSnippet
  }
  ...
  if (debugObj && comp->getOption(TR_TraceRegisterState)) {
    ...
    uint32_t numRegs =
        TR::RealRegister::LastAssignableGPR - TR::RealRegister::FirstGPR + 1;

    // print freeRegs as a binary number
    char *strFreeRegs =
        (char *)self()->cg()->trMemory()->allocateHeapMemory(numRegs + 1);
    ...
    char *comment = (char *)self()->cg()->trMemory()->allocateHeapMemory(
        strlen(commentTemplate) + strlen(strFreeRegs) - 1);
    if (comment != NULL) {
  ...
  // live registers.
  if (self()->getOpCodeValue() == TR::InstOpCode::TBEGIN ||
      self()->getOpCodeValue() == TR::InstOpCode::TBEGINC) {
    uint8_t linkageBasedSaveMask = 0;

    for (int32_t i = TR::RealRegister::GPR0; i != TR::RealRegister::GPR15 + 1;
         i++) {
      if (0 != self()->cg()->getS390Linkage()->getPreserved(
                   (TR::RealRegister::RegNum)i)) {
        // linkageBasedSaveMask is 8 bit mask where each bit represents
        // consecutive even/odd regpair to save.
    ...
    uint8_t grsm = (self()->cg()->machine()->genBitVectOfLiveGPRPairs() |
                    linkageBasedSaveMask);
    ...
    uint16_t originalGRSM =
        ((TR::S390SILInstruction *)self())->getSourceImmediate();
    ((TR::S390SILInstruction *)self())
...
void OMR::Z::Instruction::recordOperand(void *operand, int8_t &operandCount) {
  ...
  if (_operands == NULL) {
    ...
    for (int i = 0; i < n; i++)
      ((uintptrj_t **)p)[i] = ((uintptrj_t **)_operands)[i];
...
/*
 * Tests if instruction contains given register. More comprehensive containment
 * search than matchAnyRegister.
 */
bool OMR::Z::Instruction::containsRegister(TR::Register *reg) {
  ...
  int i;
  for (i = 0; i < _sourceRegSize; i++)
    if (_sourceReg[i]->usesAnyRegister(reg)) return true;
  ...
  if (self()->isOutOfLineEX()) {
    if (self()->getOpCodeValue() == TR::InstOpCode::EX) {
      ...
      TR_ASSERT(cis != NULL,
                "Out of line EX instruction doesn't have a "
                "constantInstructionSnippet\n");
      if (cis->getInstruction()->containsRegister(reg)) return true;
...
uint32_t OMR::Z::Instruction::useTargetRegister(TR::Register *reg) {
  ...
  if (self()->cg()->hasCCInfo() && (self() != ccInst) &&
...
bool OMR::Z::Instruction::hasLongDisplacementSupport() {
  // Includes all instructions with long displacement support - RXE, RX, RXY,
  // RS, RSY, SI, SIY
  return self()->getOpCode().hasLongDispSupport();
}

uint32_t OMR::Z::Instruction::useSourceMemoryReference(
...
uint32_t OMR::Z::Instruction::useTargetMemoryReference(
    TR::MemoryReference *memRef, TR::MemoryReference *sourceMemRef) {
  TR::Compilation *comp = self()->cg()->comp();
  TR_ASSERT(!memRef->getMemRefUsedBefore(), "Memory Reference Used Before");
  memRef->setMemRefUsedBefore();
...
//  Groups regpairs at the front of the array and returns the number of pair
//  elements
int OMR::Z::Instruction::gatherRegPairsAtFrontOfArray(TR::Register **regArr,
                                                      int32_t regArrSize) {
  int32_t pairCnt = 0;
...
TR::Register *OMR::Z::Instruction::assignRegisterNoDependencies(
    TR::Register *reg) {
  TR::Compilation *comp = self()->cg()->comp();
  ...
  if (reg->getRealRegister() != NULL) {
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::BCR &&
        toRealRegister(reg)->getRegisterNumber() == TR::RealRegister::GPR0 &&
        (((TR::S390RegInstruction *)self())->getBranchCondition() ==
  ...
  if ((reg->getRegisterPair() &&
    ...
    TR::RealRegister *realRegHigh =
        reg->getRegisterPair()->getHighOrder()->getRealRegister();
    TR::RealRegister *realRegLow =
        reg->getRegisterPair()->getLowOrder()->getRealRegister();
    TR::Register *virtRegHigh = realRegHigh->getAssignedRegister();
    TR::Register *virtRegLow = realRegLow->getAssignedRegister();

    if (realRegHigh->getState() != TR::RealRegister::Locked && BOOKKEEPING &&
        virtRegHigh && virtRegHigh->decFutureUseCount() == 0) {
    ...
    if (realRegLow->getState() != TR::RealRegister::Locked && BOOKKEEPING &&
        virtRegLow && virtRegLow->decFutureUseCount() == 0) {
...
//
bool OMR::Z::Instruction::anySpilledRegisters(TR::Register **listReg,
                                              int32_t listRegSize) {
  int32_t i;
...
bool OMR::Z::Instruction::anySpilledRegisters(TR::MemoryReference **listMem,
                                              int32_t listMemSize) {
  int32_t i;
...
void OMR::Z::Instruction::assignOrderedRegisters(
  ...
  // register pairs on the first pass and single registers on the second pass)
  // but care must be taken to avoid calling assignRegisterNoDependencies on the
  // same index twice.
  //
  // For example, a GPR:AR register pair seems to be replaced with one real
  // register. If using a two pass method that only checks for 'if
  // reg[i]->getRegisterPair()' on the second pass, the following bug can occur:
  //
  // 1st pass: GPR:AR assigned to S390 REAL
  // 2nd pass: S390 REAL assigned to S390 REAL (instead of being skipped because
  // it was assigned on the first pass)
  //
  // Unneccesary futureUseCount dec'ing would occur on the second pass if reg[i]
  // wasn't skipped.
  //
  // In the *assigned arrays, register pairs are marked with a 2 and single
  // registers are marked with a 1.
  //
  ...
  if ((self()->getOpCodeValue() == TR::InstOpCode::XGR ||
       self()->getOpCodeValue() == TR::InstOpCode::XR) &&
  ...
  //
  if (numTrgtPairs == 0 && _targetReg && _targetRegSize == 1 &&
      _targetMem == NULL &&
    ...
    blockTarget = false;
    self()->block(_sourceReg, _sourceRegSize, NULL, 0, self()->sourceMemBase(),
                  self()->targetMemBase());
  } else {
    blockTarget = true;
    self()->block(_sourceReg, _sourceRegSize, _targetReg, _targetRegSize,
                  self()->sourceMemBase(), self()->targetMemBase());
  }

  // Do regPair assignment first. Start with source pairs, then do target pairs
  for (i = 0; i < _targetRegSize; ++i) {
    if (!_targetReg[i]->getRegisterPair()) continue;
  ...
  for (i = 0; i < _sourceRegSize; ++i) {
    if (!_sourceReg[i]->getRegisterPair()) continue;
  ...
  if (_targetReg) {
    for (i = 0; i < _targetRegSize; ++i) {
      if (tgtAssigned[i] != 0) continue;

      registerOperandNum =
          (_targetReg < _sourceReg) ? i + 1 : _sourceRegSize + i + 1;

      if (_targetReg[i]->is64BitReg() &&
          _targetReg[i]->getRealRegister() == NULL &&
          _targetReg[i]->getKind() != TR_FPR &&
          _targetReg[i]->getKind() != TR_VRF) {
  ...
  // "The solution is to unblock all _targetRegs before PreCond assignment and
  // reblock them right after (required for srcReg assignment)." "We only
  // unblock those target registers that are not alive anymore. Block them again
  // after pre conds are done."
  if (_targetReg && blockTarget) {
    for (i = 0; i < _targetRegSize; ++i) {
      // All register pairs will have been assigned already, so skip those.
      if (tgtAssigned[i] == 2) continue;
  ...
  self()->unblock(_sourceReg, _sourceRegSize, _targetReg, _targetRegSize,
                  _sourceMem, _targetMem);
}

void OMR::Z::Instruction::assignRegistersAndDependencies(
    TR_RegisterKinds kindToBeAssigned) {
  //
  ...
  if (self()->getOpCode().isBranchOp() && self()->getKind() == IsRIE &&
    ...
    TR_S390OutOfLineCodeSection *oi =
        self()->cg()->findS390OutOfLineCodeSectionFromLabel(
  ...
  if (self()->getDependencyConditions()) {
    self()->block(_sourceReg, _sourceRegSize, _targetReg, _targetRegSize,
                  _sourceMem, _targetMem);
    self()->getDependencyConditions()->assignPostConditionRegisters(
        self(), kindToBeAssigned, self()->cg());

    // FPR and VRF register files overlap. Some of the FPRs are non-volatile but
    // if they hold vector data that part of the data would not be preserved.
    // This code looks at the data stored in non-volatile FPRs and if it is
    // assigned a vector virtual then it needs to be killed.
...
int32_t OMR::Z::Instruction::renameRegister(TR::Register *from,
                                            TR::Register *to) {
  int32_t j, n, i;
...
void OMR::Z::Instruction::block(TR::Register **sourceReg, int sourceRegSize,
                                TR::Register **targetReg, int targetRegSize,
                                TR::MemoryReference **sourceMem,
                                TR::MemoryReference **targetMem) {
  int32_t i;
...
void OMR::Z::Instruction::unblock(TR::Register **sourceReg, int sourceRegSize,
                                  TR::Register **targetReg, int targetRegSize,
                                  TR::MemoryReference **sourceMem,
                                  TR::MemoryReference **targetMem) {
  int32_t i;
...
bool OMR::Z::Instruction::checkRegForGPR0Disable(TR::InstOpCode::Mnemonic op,
                                                 TR::Register *reg) {
  ...
  if (reg->getRealRegister() == NULL &&
      ((self()->getOpCode().isBranchOp() &&
        self()->getOpCode().getInstructionFormat() == RR_FORMAT) ||
       self()->getOpCodeValue() == TR::InstOpCode::EX ||
       self()->getOpCodeValue() == TR::InstOpCode::EXRL)) {
...
// all registers from both _sourceMem and _targetMem are recorded in _useRegs
// array.
...
void OMR::Z::Instruction::setUseDefRegisters(bool updateDependencies) {
  ...
  TR::RegisterDependencyConditions *dependencies =
      self()->getDependencyConditions();
  if (self()->cg()->getCodeGeneratorPhase() ==
      TR::CodeGenPhase::PeepholePhase) {
    // In Peephole Phase, UseDefRegisters need to reset. The check on non
    // Peephole Phase needs to be avoided here.
    if ((_useRegs != NULL && _useRegs->size() != 0) ||
        (_defRegs != NULL && _defRegs->size() != 0)) {
    } else
      _sourceUsedInMemoryReference = new (self()->cg()->comp()->allocator())
          RegisterBitVector(self()->cg()->comp());
    // allocate arrays
  } else {
    TR_ASSERT(((_useRegs == NULL || _useRegs->size() == 0) ||
               (_defRegs == NULL || _defRegs->size() == 0)),
  ...
  // The check for isStore() || isCompare() || isTrap() should not hurt but
  // should unnecessary as each instruction will put used register operand 1 in
  // source array
  if ((_sourceReg != NULL) || (_sourceMem != NULL) || (_targetMem != NULL) ||
      (self()->implicitlyUsesGPR0() || self()->implicitlyUsesGPR1() ||
       self()->implicitlyUsesGPR2()) ||
      ((_targetReg != NULL) &&
       (self()->isStore() || self()->isCompare() || self()->isTrap() ||
        self()->getOpCode().usesTarget()))) {
    if (_useRegs == NULL) {
      _useRegs = new (self()->cg()->comp()->allocator())
          RegisterArray<TR::Register *>(self()->cg()->comp());
    }
  }
  if ((self()->implicitlySetsGPR0() || self()->implicitlySetsGPR1() ||
       self()->implicitlySetsGPR2()) ||
      ((_targetReg != NULL) &&
           ((!self()->isStore() && (!self()->isCompare()) &&
             (!self()->isTrap()) &&
             (self()->getOpCodeValue() != TR::InstOpCode::PPA)) ||
            self()->isLoad()) ||
       (dependencies != NULL))) {
    if (_defRegs == NULL) {
      _defRegs = new (self()->cg()->comp()->allocator())
          RegisterArray<TR::Register *>(self()->cg()->comp());
    }
  }

  int32_t loadOrStoreMultiple =
      -1;  // -1 unknown, 0 loadmultiple and 1 storemultiple
  ...
  if (_targetReg) {
    for (i = 0; i < _targetRegSize; ++i) {
      ...
      if (regPair != NULL) {
        ...
        if ((!self()->isStore() && !self()->getOpCode().setsCompareFlag()) ||
            self()->isLoad()) {
  ...
  if (self()->getOpCodeValue() == TR::InstOpCode::LM ||
      self()->getOpCodeValue() == TR::InstOpCode::LMG ||
      self()->getOpCodeValue() == TR::InstOpCode::LMH ||
      self()->getOpCodeValue() == TR::InstOpCode::LMY)
    loadOrStoreMultiple = 0;  // load
  else if (self()->getOpCodeValue() == TR::InstOpCode::STM ||
           self()->getOpCodeValue() == TR::InstOpCode::STMG ||
           self()->getOpCodeValue() == TR::InstOpCode::STMH ||
  ...
  if (loadOrStoreMultiple >= 0) {
    ...
    if (lastReg->getRealRegister() && firstReg->getRealRegister()) {
      ...
      if (numRegs > 0) {
        ...
        {
          TR::RealRegister *reg = self()->cg()->machine()->getRealRegister(
              i + TR::RealRegister::GPR0);
  ...
  if (updateDependencies) {
    // dont look at the dependencies for anything other than
    // TR::InstOpCode::DEPEND to avoid double entries due to register pair
    // dependencies
    TR::Register *tempRegister;
    if ((dependencies != NULL) &&
        ((self()->getOpCode().getOpCodeValue() == TR::InstOpCode::DEPEND))) {
      TR_S390RegisterDependencyGroup *preConditions =
          dependencies->getPreConditions();
      TR_S390RegisterDependencyGroup *postConditions =
      ...
      if (postConditions != NULL) {
        for (i = 0; i < dependencies->getNumPostConditions(); i++) {
          tempRegister =
              postConditions->getRegisterDependency(i)->getRegister();
          if (tempRegister && tempRegister->getRegisterPair() == NULL)
            (*_defRegs)[indexTarget++] = tempRegister;
        }
      }
...
bool OMR::Z::Instruction::sourceRegUsedInMemoryReference(uint32_t i) {
...
bool OMR::Z::Instruction::getTwoLocalLocalAllocFreeReg(
    TR::RealRegister **reg1, TR::RealRegister **reg2) {
  int32_t i;
  int32_t cnt = 1;                                 // skip GPR0
  int32_t first = TR::RealRegister::FirstGPR + 1;  // skip GPR0
  int32_t last = TR::RealRegister::LastAssignableGPR;
  ...
  *reg2 = NULL;
  TR_ASSERT(
      0, "OMR::Z::Instruction::getTwoLocalLocalAllocFreeReg -- untested code.");
...
TR::RealRegister *OMR::Z::Instruction::assignBestSpillRegister() {
  // Possible improvement here would be to pick a register using the SPILL
  // policy
  return _longDispSpillReg1 =
             self()->cg()->machine()->findRegNotUsedInInstruction(self());
}

// Populate the local-local reg alloc fields
//  Select the best spill register
TR::RealRegister *OMR::Z::Instruction::assignBestSpillRegister2() {
  return _longDispSpillReg2 =
             self()->cg()->machine()->findRegNotUsedInInstruction(self());
}
...
uint32_t OMR::Z::Instruction::assignFreeRegBitVector() {
  return _binFreeRegs =
             self()->cg()->machine()->constructFreeRegBitVector(self());
}

bool OMR::Z::Instruction::isCall() {
  if (self()->isLoad() || self()->isStore())
    if (self()->getMemoryReference() &&
        self()->getMemoryReference()->getUnresolvedSnippet() != NULL)
      return true;
...
void OMR::Z::Instruction::setMaskField(uint32_t *instruction, int8_t mask,
                                       int8_t nibbleIndex) {
...
TR::Register *OMR::Z::Instruction::tgtRegArrElem(int32_t i) {
  return (self()->targetRegBase() != NULL) ? (self()->targetRegBase())[i]
                                           : NULL;
}

TR::Register *OMR::Z::Instruction::srcRegArrElem(int32_t i) {
  return (self()->sourceRegBase() != NULL) ? (self()->sourceRegBase())[i]
                                           : NULL;
}
...
int32_t OMR::Z::Instruction::getMachineOpCode() {
  return self()->getOpCodeValue();
}

bool OMR::Z::Instruction::is4ByteLoad() {
  return self()->getOpCodeValue() == TR::InstOpCode::L;
}

bool OMR::Z::Instruction::isRet() {
  return self()->getOpCodeValue() == TR::InstOpCode::RET;
}

int8_t OMR::Z::Instruction::lastOperandIndex(void) {
