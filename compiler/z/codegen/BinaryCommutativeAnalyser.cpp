...
void TR_S390BinaryCommutativeAnalyser::remapInputs(
    TR::Node *firstChild, TR::Register *firstRegister, TR::Node *secondChild,
    TR::Register *secondRegister, bool nonClobberingDestination) {
  if (!cg()->useClobberEvaluate()) {
    ...
    if (firstChild == secondChild && firstChild->getReferenceCount() == 2 &&
        firstRegister && cg()->isRegisterClobberable(firstRegister, 1)) {
      ...
      //    =>d2f (in f1)
      //
      // then it is not legal to clobber f1 because the commoned i2d requires
      // the unclobbered f1 value isRegisterClobberable(firstRegister,1) returns
      // false in this latter case as firstRegister is attached to more than one
      // unique node
      setClob1();
      setClob2();
    }
...
TR::Register *TR_S390BinaryCommutativeAnalyser::allocateAddSubRegister(
    TR::Node *node, TR::Register *src1Reg) {
...
void TR_S390BinaryCommutativeAnalyser::genericAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic regToRegOpCode,
  ...
  //     (2)        iaload #177[000000008012C5E0]
  //     Shadow[java/lang/ThreadGroup.childrenGroups (1)          ==>aload at
  //     [000000008012DBE8] (in &GPR_0058) (1)    l2i (in GPR_0067) (2) ==>i2l
  //     at [0000000080117A70]
  //  We need to be sure that the operation is non-clobbering to safely omit the
  //  l2i
  //
  ...
  setInputs(firstChild, firstRegister, secondChild, secondRegister,
            nonClobberingDestination, false, comp);
  ...
  if ((root->getSize() == 1 || root->getSize() == 2) &&
  ...
  if (getOpReg1Reg2()) {
    if (targetLabel != NULL) {
      finalInstr = generateS390CompareAndBranchInstruction(
          cg(), regToRegOpCode, root, firstRegister, secondRegister,
          getReversedOperands() ? rBranchOpCond : fBranchOpCond, targetLabel,
          false, true);
      ...
      if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z14)) {
        ...
        if (root->getOpCodeValue() == TR::lmul && firstRegister != NULL &&
            firstRegister->is64BitReg() && secondRegister != NULL &&
            secondRegister->is64BitReg()) {
          z14OpCode = TR::InstOpCode::MSGRKC;
        } else if (root->getOpCodeValue() == TR::imul &&
                   firstRegister != NULL && secondRegister != NULL) {
        ...
        if (z14OpCode != TR::InstOpCode::BAD) {
          ...
          nodeReg =
              isCanClobberFirstReg ? firstRegister : cg()->allocateRegister();
          generateRRFInstruction(cg(), z14OpCode, root, nodeReg, firstRegister,
                                 secondRegister, 0, 0);
        }
      }
      ...
      if (nodeReg == NULL) {
        ...
        if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) &&
            cg()->isActiveCompareCC(regToRegOpCode, firstRegister,
                                    secondRegister) &&
            performTransformation(
                comp,
                "O^O BinaryCommunicativeAnalyser case 3 RR Compare [%s\t %s, "
                "%s]: reuse CC from ccInst [%p].",
                ccInst->getOpCode().getMnemonicName(),
                debugObj->getName(firstRegister),
                debugObj->getName(secondRegister), ccInst)) {
          nodeReg = firstRegister;  // CCInfo already exists; don't generate
                                    // compare op any more;
        } else if (opcTmp.setsCompareFlag() &&
                   comp->getOption(TR_EnableEBBCCInfo) &&
                   cg()->isActiveCompareCC(regToRegOpCode, secondRegister,
                                           firstRegister) &&
                   performTransformation(
                       comp,
                       "O^O BinaryCommunicativeAnalyser case 4 RR Compare "
                       "[%s\t %s, %s]: reuse CC from ccInst [%p].",
                       ccInst->getOpCode().getMnemonicName(),
                       debugObj->getName(firstRegister),
                       debugObj->getName(secondRegister), ccInst)) {
          nodeReg = secondRegister;
          notReversedOperands();  // CCInfo already exists; don't generate
                                  // compare op any more;
        } else {
          generateRRInstruction(cg(), regToRegOpCode, root, firstRegister,
                                secondRegister);
          nodeReg = firstRegister;  // CCInfo already exists; don't generate
                                    // compare op any more;
        }
    ...
    if (targetLabel != NULL) {
      finalInstr = generateS390CompareAndBranchInstruction(
          cg(), regToRegOpCode, root, secondRegister, firstRegister,
          (!getReversedOperands()) ? rBranchOpCond : fBranchOpCond, targetLabel,
          false, true);
      nodeReg = secondRegister;
      notReversedOperands();
    } else {
      // for Compare GPR1, GPR2 or compare FPR1, FPR2, check if valid CCInfo
      // exists; with same compare ops; if reg is switched, swap them..
      TR::Instruction *ccInst = cg()->ccInstruction();
      TR::InstOpCode opcTmp = TR::InstOpCode(regToRegOpCode);
      if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) &&
          cg()->isActiveCompareCC(regToRegOpCode, firstRegister,
          ...
          performTransformation(
              comp,
              "O^O BinaryCommunicativeAnalyser case 5 RR Compare [%s\t %s, "
              "%s]: reuse CC from ccInst [%p].",
              ccInst->getOpCode().getMnemonicName(),
              debugObj->getName(firstRegister),
              debugObj->getName(secondRegister), ccInst)) {
        nodeReg = firstRegister;  // CCInfo already exists; don't generate
                 ...
                 performTransformation(
                     comp,
                     "O^O BinaryCommunicativeAnalyser case 6 RR Compare [%s\t "
                     "%s, %s]: reuse CC from ccInst [%p].",
                     ccInst->getOpCode().getMnemonicName(),
                     debugObj->getName(firstRegister),
                     debugObj->getName(secondRegister), ccInst)) {
        nodeReg = secondRegister;
        notReversedOperands();  // CCInfo already exists; don't generate compare
                                // op any more;
      } else {
        generateRRInstruction(cg(), regToRegOpCode, root, secondRegister,
                              firstRegister);
        nodeReg = secondRegister;
        notReversedOperands();
      }
    ...
    switch (firstRegister->getKind()) {
      ...
      default: {
        TR_ASSERT_FATAL(false,
                        "Generic binary commutative analyser for register kind "
                        "(%d) is unimplemented",
    ...
    cursor =
        generateRRInstruction(cg(), copyOpCode, root, nodeReg, firstRegister);
    if (debugObj) {
    ...
    if (targetLabel != NULL)
      finalInstr = generateS390CompareAndBranchInstruction(
          cg(), regToRegOpCode, root, nodeReg, secondRegister,
          getReversedOperands() ? rBranchOpCond : fBranchOpCond, targetLabel,
          false, true);
    else
      generateRRInstruction(cg(), regToRegOpCode, root, nodeReg,
                            secondRegister);
  } else if (getCopyReg2()) {
    ...
    cursor =
        generateRRInstruction(cg(), copyOpCode, root, nodeReg, secondRegister);
    if (debugObj) {
      debugObj->addInstructionComment(toS390RRInstruction(cursor),
                                      CLOBBER_EVAL);
    }
    ...
    if (targetLabel != NULL)
      finalInstr = generateS390CompareAndBranchInstruction(
          cg(), regToRegOpCode, root, nodeReg, firstRegister,
          (!getReversedOperands()) ? rBranchOpCond : fBranchOpCond, targetLabel,
          false, true);
    else
    ...
    if (getOpReg3Mem2()) {
      switch (firstRegister->getKind()) {
        ...
        default: {
          TR_ASSERT_FATAL(false,
                          "Generic binary commutative analyser for register "
                          "kind (%d) is unimplemented",
                          firstRegister->getKind());
          break;
        }
      }
    ...
    TR::MemoryReference *tempMR =
        generateS390MemoryReference(secondChild, cg(), true);
    // floating-point arithmatics don't have RXY format instructions, so no long
    // displacement
    ...
    auto instructionFormat =
        TR::InstOpCode(memToRegOpCode).getInstructionFormat();
  ...
  {
    TR_ASSERT(!getInvalid(),
              "TR_S390CommutativeBinaryAnalyser::invalid case\n");
    ...
    if (getOpReg3Mem1()) {
      ...
      cursor = generateRRInstruction(cg(), copyOpCode, root, nodeReg,
                                     secondRegister);
    }

    TR::Node *loadNode =
        isLoadNodeNested ? firstChild->getFirstChild() : firstChild;
    TR::MemoryReference *tempMR =
        generateS390MemoryReference(loadNode, cg(), true);
  ...
  if (!finalInstr && targetLabel != NULL) {
    generateS390BranchInstruction(
        ...
        getReversedOperands() ? rBranchOpCond : fBranchOpCond, root,
        targetLabel);
  }
 ...
 */
bool TR_S390BinaryCommutativeAnalyser::conversionIsRemoved(TR::Node *root,
                                                           TR::Node *&child) {
  if (((root->getDataType() == TR::Int64 &&
        child->getOpCodeValue() == TR::a2l && TR::Compiler->target.is64Bit()) ||
       (root->getDataType() == TR::Int32 &&
        child->getOpCodeValue() == TR::a2i &&
 ...
 */
void TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic regToRegOpCode,
    TR::InstOpCode::Mnemonic memToRegOpCode,
  ...
  TR::Node *secondChild;
  TR::Instruction *cursor = NULL;
  // the following flag would be required if analyser in future generates
  // alternative add instructions which do not produce a carry. The flag would
  // be used to prevent such optimizations when the carry is needed.
  /* bool setsOrReadsCC = NEED_CC(node) || (node->getOpCodeValue() ==
   * TR::luaddc) || (node->getOpCodeValue() == TR::iuaddc); */
  ...
  if (secondRegister == NULL) noDec2 = conversionIsRemoved(root, secondChild);
  ...
  if ((root->getOpCodeValue() == TR::iuaddc ||
       root->getOpCodeValue() == TR::luaddc) &&
  ...
  if (getOpReg1Reg2()) {
    ...
    TR::Register *tempReg =
        root->setRegister(allocateAddSubRegister(root, firstRegister));
    bool done = false;

    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      if (regToRegOpCode == TR::InstOpCode::AR) {
        cursor = generateRRRInstruction(cg(), TR::InstOpCode::ARK, root,
                                        tempReg, secondRegister, firstRegister);
        done = true;
      } else if (regToRegOpCode == TR::InstOpCode::ALR) {
        cursor = generateRRRInstruction(cg(), TR::InstOpCode::ALRK, root,
                                        tempReg, secondRegister, firstRegister);
        done = true;
      } else if (regToRegOpCode == TR::InstOpCode::AGR) {
        cursor = generateRRRInstruction(cg(), TR::InstOpCode::AGRK, root,
                                        tempReg, secondRegister, firstRegister);
        done = true;
      } else if (regToRegOpCode == TR::InstOpCode::ALGR) {
        cursor = generateRRRInstruction(cg(), TR::InstOpCode::ALGRK, root,
                                        tempReg, secondRegister, firstRegister);
        done = true;
      }
    }
    ...
    if (!done) {
      auto mnemonic = firstRegister->is64BitReg() ? TR::InstOpCode::LGR
                                                  : TR::InstOpCode::LR;

      generateRRInstruction(cg(), mnemonic, root, tempReg, firstRegister);
      generateRRInstruction(cg(), regToRegOpCode, root, tempReg,
                            secondRegister);
    }
  } else if (getOpReg3Mem2() || getOpReg1Mem2()) {
    ...
    if (getOpReg3Mem2()) {
      ...
      cursor =
          generateRRInstruction(cg(), copyOpCode, root, tempReg, firstRegister);
    }

    TR::MemoryReference *tempMR = generateS390MemoryReference(
        is16BitMemory2Operand ? secondChild->getFirstChild() : secondChild,
        cg());
    ...
    if (getOpReg3Mem1()) {
      ...
      cursor = generateRRInstruction(cg(), copyOpCode, root, tempReg,
                                     secondRegister);
    }
...
bool TR_S390BinaryCommutativeAnalyser::getCopyRegs() {
  if (!cg()->useClobberEvaluate()) {
    return (clobEvalActionMap[getInputs()] & (CopyReg1 | CopyReg2)) ? true
                                                                    : false;
  }
...
const uint16_t
    TR_S390BinaryCommutativeAnalyser::clobEvalActionMap[NUM_ACTIONS] = {
        // Reg1 Mem1 Clob1 Reg2 Mem2 Clob2
