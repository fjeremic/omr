...
void
TR_S390BinaryCommutativeAnalyser::remapInputs(TR::Node* firstChild,
...
TR::Register*
TR_S390BinaryCommutativeAnalyser::allocateAddSubRegister(TR::Node* node,
                                                         TR::Register* src1Reg)
{
...
void
TR_S390BinaryCommutativeAnalyser::genericAnalyser(
  TR::Node* root,
  ...
  if (getOpReg1Reg2()) {
    if (targetLabel != NULL) {
      ...
      if (nodeReg == NULL) {
        ...
        if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) &&
            cg()->isActiveCompareCC(
              regToRegOpCode, firstRegister, secondRegister) &&
          ...
          nodeReg = firstRegister; // CCInfo already exists; don't generate
                   ...
                   cg()->isActiveCompareCC(
                     regToRegOpCode, secondRegister, firstRegister) &&
                   performTransformation(
                     comp,
                     "O^O BinaryCommunicativeAnalyser case 4 RR Compare [%s\t "
    ...
    if (targetLabel != NULL) {
      ...
      if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) &&
          ...
          performTransformation(
            comp,
            "O^O BinaryCommunicativeAnalyser case 5 RR Compare [%s\t %s, %s]: "
            "reuse CC from ccInst [%p].",
            ccInst->getOpCode().getMnemonicName(),
            debugObj->getName(firstRegister),
            debugObj->getName(secondRegister),
            ccInst)) {
 ...
 */
bool
TR_S390BinaryCommutativeAnalyser::conversionIsRemoved(TR::Node* root,
                                                      TR::Node*& child)
{
 ...
 */
void
TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(
  TR::Node* root,
  TR::InstOpCode::Mnemonic regToRegOpCode,
  TR::InstOpCode::Mnemonic memToRegOpCode,
  TR::InstOpCode::Mnemonic copyOpCode)
{
  ...
  setInputs(
    firstChild, firstRegister, secondChild, secondRegister, false, false, comp);
  ...
  if (getOpReg1Reg2()) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      if (regToRegOpCode == TR::InstOpCode::AR) {
        cursor = generateRRRInstruction(cg(),
                                        TR::InstOpCode::ARK,
                                        root,
                                        tempReg,
                                        secondRegister,
    ...
    if (!done) {
      auto mnemonic =
        firstRegister->is64BitReg() ? TR::InstOpCode::LGR : TR::InstOpCode::LR;

      generateRRInstruction(cg(), mnemonic, root, tempReg, firstRegister);
      generateRRInstruction(
        cg(), regToRegOpCode, root, tempReg, secondRegister);
    }
  } else if (getOpReg3Mem2() || getOpReg1Mem2()) {
    ...
    TR::MemoryReference* tempMR = generateS390MemoryReference(
      is16BitMemory2Operand ? secondChild->getFirstChild() : secondChild, cg());
    ...
    if (getOpReg3Mem1()) {
      ...
      cursor =
        generateRRInstruction(cg(), copyOpCode, root, tempReg, secondRegister);
    }
