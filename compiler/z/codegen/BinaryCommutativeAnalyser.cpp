  ...
  TR::Node* secondChild, TR::Register* secondRegister, bool nonClobberingDestination)
{
  if (!cg()->useClobberEvaluate()) {
    ...
    if (firstChild == secondChild && firstChild->getReferenceCount() == 2 && firstRegister
      ...
      // isRegisterClobberable(firstRegister,1) returns false in this latter case as firstRegister is attached to more
      // than one unique node
      setClob1();
      setClob2();
    }
  }
  ...
  TR::InstOpCode::S390BranchCondition rBranchOpCond)
{
  ...
  if (getOpReg1Reg2()) {
    if (targetLabel != NULL) {
      finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, firstRegister, secondRegister,
      ...
      if (nodeReg == NULL) {
        ...
        if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
          ...
          && performTransformation(comp,
            ...
            ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister), debugObj->getName(secondRegister),
            ccInst)) {
    ...
    if (targetLabel != NULL) {
      finalInstr = generateS390CompareAndBranchInstruction(cg(), regToRegOpCode, root, secondRegister, firstRegister,
        (!getReversedOperands()) ? rBranchOpCond : fBranchOpCond, targetLabel, false, true);
      nodeReg = secondRegister;
      notReversedOperands();
    } else {
    ...
    default: {
      TR_ASSERT_FATAL(
        false, "Generic binary commutative analyser for register kind (%d) is unimplemented", firstRegister->getKind());
      break;
    }
    }
...
{
  if (((root->getDataType() == TR::Int64 && child->getOpCodeValue() == TR::a2l && TR::Compiler->target.is64Bit())
        || (root->getDataType() == TR::Int32 && child->getOpCodeValue() == TR::a2i && TR::Compiler->target.is32Bit()))
  ...
  TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
  ...
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z14) && secondChild->getOpCodeValue() == TR::s2l
    && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi && secondChild->isSingleRefUnevaluated()
  ...
  if (getOpReg1Reg2()) {
    ...
    TR::MemoryReference* tempMR
      = generateS390MemoryReference(is16BitMemory2Operand ? secondChild->getFirstChild() : secondChild, cg());
