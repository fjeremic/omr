    ...
    bool nonClobberingDestination)
{
    if (!cg()->useClobberEvaluate()) {
        ...
        if (firstChild == secondChild && firstChild->getReferenceCount() == 2 && firstRegister && cg()->isRegisterClobberable(firstRegister, 1)) {
...
void TR_S390BinaryCommutativeAnalyser::genericAnalyser(TR::Node* root, TR::InstOpCode::Mnemonic regToRegOpCode, TR::InstOpCode::Mnemonic memToRegOpCode,
    ...
    TR::LabelSymbol* targetLabel, TR::InstOpCode::S390BranchCondition fBranchOpCond, TR::InstOpCode::S390BranchCondition rBranchOpCond)
{
    ...
    if (root->getOpCode().isBooleanCompare() || nonClobberingDestination) {
        if (firstChild->getOpCodeValue() == TR::bu2i && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && (firstChild->getFirstChild()->getOpCodeValue() == TR::buload || firstChild->getFirstChild()->getOpCodeValue() == TR::buloadi)) {
        ...
        if (secondChild->getOpCodeValue() == TR::bu2i && secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL && (secondChild->getFirstChild()->getOpCodeValue() == TR::buload || secondChild->getFirstChild()->getOpCodeValue() == TR::buloadi)) {
    ...
    if (TR::Compiler->target.is64Bit()) {
        if (firstChild->getOpCodeValue() == TR::l2i && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && nonClobberingDestination) {
            initFirstChild = firstChild;
            firstChild = firstChild->getFirstChild();
        ...
        if (secondChild->getOpCodeValue() == TR::l2i && secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL && nonClobberingDestination) {
            initSecondChild = secondChild;
            secondChild = secondChild->getFirstChild();
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z14)) {
        ...
        if (root->getOpCodeValue() == TR::lmul) {
            if (firstChild->getOpCodeValue() == TR::s2l && firstChild->getFirstChild()->getOpCodeValue() == TR::sloadi && firstChild->isSingleRefUnevaluated() && firstChild->getFirstChild()->isSingleRefUnevaluated()) {
            ...
            } else if (firstChild->getOpCodeValue() == TR::lloadi && firstChild->isSingleRefUnevaluated()) {
            ...
            if (firstChild->getOpCodeValue() == TR::iloadi && firstChild->isSingleRefUnevaluated()) {
    ...
    if (getOpReg1Reg2()) {
        if (targetLabel != NULL) {
            ...
            if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z14)) {
                ...
                if (root->getOpCodeValue() == TR::lmul && firstRegister != NULL && firstRegister->is64BitReg() && secondRegister != NULL && secondRegister->is64BitReg()) {
            ...
            if (nodeReg == NULL) {
                ...
                if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, firstRegister, secondRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 3 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
                    nodeReg = firstRegister; // CCInfo already exists; don't generate compare op any more;
                } else if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, secondRegister, firstRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 4 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
        ...
        if (targetLabel != NULL) {
            ...
            TR::Instruction* ccInst = cg()->ccInstruction();
            TR::InstOpCode opcTmp = TR::InstOpCode(regToRegOpCode);
            if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, firstRegister, secondRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 5 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
                nodeReg = firstRegister; // CCInfo already exists; don't generate compare op any more;
            } else if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, secondRegister, firstRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 6 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", ccInst->getOpCode().getMnemonicName(), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
                nodeReg = secondRegister;
                notReversedOperands(); // CCInfo already exists; don't generate compare op any more;
            } else {
...
{
    if (((root->getDataType() == TR::Int64 && child->getOpCodeValue() == TR::a2l && TR::Compiler->target.is64Bit()) || (root->getDataType() == TR::Int32 && child->getOpCodeValue() == TR::a2i && TR::Compiler->target.is32Bit())) && child->getFirstChild()->getOpCodeValue() == TR::aloadi && child->getFirstChild()->getRegister() == NULL && child->getFirstChild()->getReferenceCount() == 1 && child->getReferenceCount() == 1) {
 ...
 */
void TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(TR::Node* root, TR::InstOpCode::Mnemonic regToRegOpCode, TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    TR::Node* firstChild;
    TR::Node* secondChild;
    TR::Instruction* cursor = NULL;
    ...
    */
    if (secondChild->getOpCodeValue() == TR::s2i && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi && secondChild->isSingleRefUnevaluated() && secondChild->getFirstChild()->isSingleRefUnevaluated()) {
        setMem2();
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z14) && secondChild->getOpCodeValue() == TR::s2l && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi && secondChild->isSingleRefUnevaluated() && secondChild->getFirstChild()->isSingleRefUnevaluated()) {
        setMem2();
    ...
    if ((root->getOpCodeValue() == TR::iuaddc || root->getOpCodeValue() == TR::luaddc) && TR_S390ComputeCC::setCarryBorrow(root->getChild(2), false, cg())) {
