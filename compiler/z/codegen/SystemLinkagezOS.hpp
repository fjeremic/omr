...
private:
  /** \brief
   *
   *  Represents the XPLINK 31-bit call descriptor relocation which is a
   * (positive) signed offset in doublewords from the doubleword at or preceding
   * the return point (NOP) to the XPLINK call descriptor for this signature.
   *
   *  [1]
  ...
  public:
    /** \brief
     *     Initializes the XPLINKCallDescriptorRelocation relocation using a \c
     * NULL base target pointer
     *
     *  \param nop
     *     The 4-byte NOP descriptor instruction whose binary encoding location
     * (+2) will be used for the relocation
     *
...
public:
  /** \brief
   *     Size (in bytes) of the XPLINK stack frame bias as defined by the system
   * linkage.
   */
...
public:
  ...
  TR::Instruction *genCallNOPAndDescriptor(TR::Instruction *cursor,
                                           TR::Node *node, TR::Node *callNode,
                                           TR_XPLinkCallTypes callType);

  /** \brief
   *     Gets the label symbol representing the start of the Entry Point Marker
   * in the prologue.
   *
   *  \return
  ...
  /** \brief
   *     Gets the label symbol representing the stack pointer update instruction
   * following it.
   *
   *  \return
...
private:
  virtual TR::Instruction *addImmediateToRealRegister(
      TR::RealRegister *targetReg, int32_t immediate, TR::RealRegister *tempReg,
      TR::Node *node, TR::Instruction *cursor, bool *checkTempNeeded = NULL);
