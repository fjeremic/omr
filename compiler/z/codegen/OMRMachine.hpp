...
#define TR_PACKED_TO_UNICODE_SOURCE_SIZE                                       \
  16 ///< The source byte size in memory for a packed to unicode conversion
     ///< (i.e. with UNPKU)

#define TR_PACKED_TO_DECIMAL_FLOAT_SIZE                                        \
  8 ///< The source byte size in memory for a pd2df conversion (i.e. with CDSTR
    ///< -- pd2df and pd2dd use same instruction)
#define TR_PACKED_TO_DECIMAL_FLOAT_PRECISION 15

#define TR_PACKED_TO_DECIMAL_DOUBLE_SIZE                                       \
  8 ///< The source byte size in memory for a pd2dd conversion (i.e. with CDSTR)
...
#define TR_PACKED_TO_DECIMAL_FLOAT_SIZE_ARCH11                                 \
  9 ///< The source byte size in memory for a pd2df conversion (i.e. with CDPT
    ///< -- pd2df and pd2dd use same instruction)
...
#define TR_DECIMAL_FLOAT_TO_ZONED_MAX_PRECISION                                \
  16 ///< CZDT - same as DecimalDouble
...
class OMR_EXTENSIBLE Machine : public OMR::Machine {
  ...
  uint32_t _globalRegisterNumberToRealRegisterMap[NUM_S390_GPR + NUM_S390_FPR +
                                                  NUM_S390_VRF];
  ...
  uint32_t _globalRegisterNumberToRealRegisterMapSnapShot
...
public:
  ...
  createDepCondForLiveGPRs(TR::list<TR::Register *> *spilledRegisterList);
  ...
  TR::RealRegister *findBestSwapRegister(TR::Register *reg1,
                                         TR::Register *reg2);
  TR::Instruction *registerCopy(TR::CodeGenerator *cg, TR_RegisterKinds rk,
                                TR::RealRegister *targetReg,
                                TR::RealRegister *sourceReg,
                                TR::Instruction *precedingInstruction);
  TR::Instruction *registerExchange(TR::CodeGenerator *cg, TR_RegisterKinds rk,
                                    TR::RealRegister *targetReg,
                                    TR::RealRegister *sourceReg,
                                    TR::RealRegister *middleReg,
                                    TR::Instruction *precedingInstruction);

  bool isLegalEvenOddPair(TR::RealRegister *evenReg, TR::RealRegister *oddReg,
                          uint64_t availRegMask = 0x0000ffff);
  bool isLegalEvenRegister(TR::RealRegister *reg, bool allowBlocked,
                           uint64_t availRegMask = 0x0000ffff,
                           bool allowLocked = false);
  bool isLegalOddRegister(TR::RealRegister *reg, bool allowBlocked,
                          uint64_t availRegMask = 0x0000ffff,
                          bool allowLocked = false);

  TR::RealRegister *
  findBestLegalEvenRegister(uint64_t availRegMask = 0x0000ffff);
  TR::RealRegister *
  findBestLegalOddRegister(uint64_t availRegMask = 0x0000ffff);
  bool isLegalFPPair(TR::RealRegister *highReg, TR::RealRegister *lowReg,
                     uint64_t availRegMask = 0x0000ffff);
  bool isLegalFirstOfFPRegister(TR::RealRegister *reg, bool allowBlocked,
                                uint64_t availRegMask = 0x0000ffff,
                                bool allowLocked = false);
  bool isLegalSecondOfFPRegister(TR::RealRegister *reg, bool allowBlocked,
                                 uint64_t availRegMask = 0x0000ffff,
                                 bool allowLocked = false);
  TR::RealRegister *
  findBestLegalSiblingFPRegister(bool isFirst,
                                 uint64_t availRegMask = 0x0000ffff);

  bool findBestFreeRegisterPair(TR::RealRegister **firstReg,
                                TR::RealRegister **lastReg, TR_RegisterKinds rk,
                                TR::Instruction *currInst,
                                uint64_t availRegMask = 0x0000ffff);
  void freeBestRegisterPair(TR::RealRegister **firstReg,
                            TR::RealRegister **lastReg, TR_RegisterKinds rk,
                            TR::Instruction *currInst,
                            uint64_t availRegMask = 0x0000ffff);
  void freeBestFPRegisterPair(TR::RealRegister **firstReg,
  ...
  TR::RealRegister *
  findRegNotUsedInInstruction(TR::Instruction *currentInstruction);
  ...
  TR::RealRegister *
  findBestRegisterForShuffle(TR::Instruction *currentInstruction,
  ...
  TR::Instruction *
  coerceRegisterAssignment(TR::Instruction *currentInstruction,
  ...
  setLastGlobalGPRRegisterNumber(TR_GlobalRegisterNumber reg);
  ...
  TR_GlobalRegisterNumber
  setFirstGlobalGPRRegisterNumber(TR_GlobalRegisterNumber reg);

  TR_GlobalRegisterNumber getLast8BitGlobalGPRRegisterNumber() {
    return _last8BitGlobalGPRRegisterNumber;
  }
  TR_GlobalRegisterNumber
  setLast8BitGlobalGPRRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setFirstGlobalFPRRegisterNumber(TR_GlobalRegisterNumber reg);
  ...
  TR_GlobalRegisterNumber
  setLastGlobalFPRRegisterNumber(TR_GlobalRegisterNumber reg);
  ...
  TR_GlobalRegisterNumber
  setFirstOverlappedGlobalFPRRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setLastOverlappedGlobalFPRRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setFirstGlobalVRFRegisterNumber(TR_GlobalRegisterNumber reg);
  ...
  TR_GlobalRegisterNumber
  setLastGlobalVRFRegisterNumber(TR_GlobalRegisterNumber reg);
  ...
  TR_GlobalRegisterNumber
  setFirstOverlappedGlobalVRFRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setLastOverlappedGlobalVRFRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setGlobalEnvironmentRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setGlobalCAARegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setGlobalParentDSARegisterNumber(TR_GlobalRegisterNumber reg) {
    return _globalParentDSARegisterNumber = reg;
  }

  TR_GlobalRegisterNumber
  setGlobalReturnAddressRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setGlobalEntryPointRegisterNumber(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setLastGlobalCCRRegisterNumber(TR_GlobalRegisterNumber reg);
  ...
  TR_GlobalRegisterNumber
  setLastVolatileNonLinkGPR(TR_GlobalRegisterNumber reg) {
  ...
  TR_GlobalRegisterNumber
  setLastVolatileNonLinkFPR(TR_GlobalRegisterNumber reg) {
