...
#define TR_PACKED_TO_DECIMAL_FLOAT_SIZE \
  8 ///< The source byte size in memory for a pd2df conversion (i.e. with CDSTR -- pd2df and pd2dd use same instruction)
...
#define TR_PACKED_TO_DECIMAL_LONG_DOUBLE_SIZE \
  16 ///< The source byte size in memory for a pd2de conversion (i.e. with CXSTR)
#define TR_PACKED_TO_DECIMAL_LONG_DOUBLE_PRECISION 31

#define TR_PACKED_TO_DECIMAL_FLOAT_SIZE_ARCH11 \
  9 ///< The source byte size in memory for a pd2df conversion (i.e. with CDPT -- pd2df and pd2dd use same instruction)
#define TR_PACKED_TO_DECIMAL_FLOAT_PRECISION_ARCH11 \
  16 ///< NOTE: Although max size is 9 bytes only 16 and not 17 digits can be converted by CDPT to DFP -- df2pd and
     ///< dd2pd use same instruction
...
#define TR_PACKED_TO_DECIMAL_DOUBLE_PRECISION_ARCH11 \
  16 ///< NOTE: Although max size is 9 bytes only 16 and not 17 digits can be converted by CDPT to DFP
...
#define TR_PACKED_TO_DECIMAL_LONG_DOUBLE_PRECISION_ARCH11 \
  34 ///< NOTE: Although max size is 18 bytes only 34 and not 35 digits can be converted by CDPT to DFP

#define TR_DECIMAL_FLOAT_TO_PACKED_SIZE \
  8 ///< The destination byte size in memory for a df2pd conversion (i.e. with CSDTR -- df2pd and dd2pd use same
    ///< instruction)
#define TR_DECIMAL_FLOAT_TO_PACKED_PRECISION 15

#define TR_DECIMAL_DOUBLE_TO_PACKED_SIZE \
  8 ///< The destination byte size in memory for a dd2pd conversion (i.e. with CSDTR)
#define TR_DECIMAL_DOUBLE_TO_PACKED_PRECISION 15

#define TR_DECIMAL_LONG_DOUBLE_TO_PACKED_SIZE \
  16 ///< The destination byte size in memory for a de2pd conversion (i.e. with CSXTR)
#define TR_DECIMAL_LONG_DOUBLE_TO_PACKED_PRECISION 31

#define TR_DECIMAL_FLOAT_TO_PACKED_SIZE_ARCH11 \
  9 ///< The destination byte size in memory for a df2pd conversion (i.e. with CPDT -- df2pd and dd2pd use same
    ///< instruction)
#define TR_DECIMAL_FLOAT_TO_PACKED_PRECISION_ARCH11 \
  16 ///< NOTE: Although max size is 9 bytes only 16 and not 17 digits can be converted by CPDT -- df2pd and dd2pd use
     ///< same instruction

#define TR_DECIMAL_DOUBLE_TO_PACKED_SIZE_ARCH11 \
  9 ///< The destination byte size in memory for a dd2pd conversion (i.e. with CPDT)
#define TR_DECIMAL_DOUBLE_TO_PACKED_PRECISION_ARCH11 \
  16 ///< NOTE: Although max size is 9 bytes only 16 and not 17 digits can be converted by CPDT

#define TR_DECIMAL_LONG_DOUBLE_TO_PACKED_SIZE_ARCH11 \
  18 ///< The destination byte size in memory for a de2pd conversion (i.e. with CPXT)
#define TR_DECIMAL_LONG_DOUBLE_TO_PACKED_PRECISION_ARCH11 \
  34 ///< NOTE: Although max size is 18 bytes only 34 and not 35 digits can be converted by CPXT
...
public:
  ...
  uint8_t getGPRSize();
  uint8_t getFPRSize() const
  {
    return 8;
  }
  uint8_t getVRFSize() const
  {
    return 16;
  }
   ...
   */
  int32_t getAGIDelay() const
  {
    return 4;
  }

  /** GENERAL INTERFACE for OBTAINING ANY TYPE of REG */
  TR::Register* assignBestRegister(
    TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask = 0xffffffff);
  ...
  TR::Register* assignBestRegisterSingle(
    TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask = 0xffffffff);
