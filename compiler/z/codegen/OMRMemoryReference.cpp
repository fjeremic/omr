 ...
 */
void
OMR::Z::MemoryReference::setupCausesImplicitNullPointerException(
  TR::CodeGenerator* cg)
{
 ...
 */
void
OMR::Z::MemoryReference::addToTemporaryNegativeOffset(TR::Node* node,
                                                      int32_t offset,
                                                      TR::CodeGenerator* cg)
{
  ...
  if (offset < 0) {
                                               ...
                                               // temporary negative offset
    {
      ...
      self()->enforceSSFormatLimits(
        node, cg, NULL); // call SSFormatLimits to also take this chance to fold
                         // in an index register if needed
    }
 ...
 */
void
OMR::Z::MemoryReference::setupCheckForLongDispFlag(TR::CodeGenerator* cg)
{
 ...
 */
bool
OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(TR::CodeGenerator* cg,
                                                       TR::Node* addressChild)
{
  ...
  if (addressChild->getRegister() != NULL) {
    ...
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
               ...
               "has register %s\n",
               addressChild->getOpCode().getName(),
               addressChild,
  ...
  if (addressChild->getSecondChild()->getRegister() != NULL) {
    ...
    if (cg->traceBCDCodeGen()) {
      traceMsg(
        comp,
        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) const "
        "child %s (%p) has register %s, cannot fold.\n",
        addressChild->getOpCode().getName(),
        addressChild,
  ...
  while (eventualNonConversion->getOpCode().isConversion()) {
    ...
    allConvNodesAreUnneeded =
      allConvNodesAreUnneeded && eventualNonConversion->isUnneededConversion();
  ...
  bool startRemoving)
{
  ...
  for (size_t i = 0; i < cur->getNumChildren(); i++) {
    recursivelyDecrementIncrementedNodesIfUnderRegister(cg,
                                                        cur->getChild(i),
                                                        _incNodesList,
                                                        nodesToNotRecurse,
                                                        vc,
  ...
  , _incrementedNodesList(cg->comp()->trMemory())
{
  ...
  self()->tryForceFolding(rootLoadOrStore,
                          cg,
                          storageReference,
                          symRef,
                          symbol,
  ...
  if (rootLoadOrStore->getOpCode().isIndirect() ||
    ...
    if ((symbol && symbol->isStatic()) && symRef &&
      ...
      if (symRef->isUnresolved()) {
        self()->createUnresolvedDataSnippetForiaload(
    ...
    if (symRef && symRef->isUnresolved()) {
      self()->createUnresolvedDataSnippet(
        rootLoadOrStore, cg, symRef, tempReg, isStore);
    }
  } else {
    // symbol is static
    if (symbol && symbol->isStatic()) {
      ...
      if (!rootLoadOrStore->getOpCode().isLoad() &&
        ...
        TR_ASSERT(rootLoadOrStore->getDataType() == TR::Address ||
                  ...
                  "and not dt %s\n",
                  rootLoadOrStore->getOpCode().getName(),
                  rootLoadOrStore,
...
void
OMR::Z::MemoryReference::initSnippetPointers(TR::Snippet* s,
                                             TR::CodeGenerator* cg)
{
...
void
OMR::Z::MemoryReference::setOriginalSymbolReference(TR::SymbolReference* ref,
                                                    TR::CodeGenerator* cg)
{
...
void
OMR::Z::MemoryReference::bookKeepingRegisterUses(TR::Instruction* instr,
                                                 TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
 ...
 */
void
noteAllNodesWithRefCountNotOne(TR_Array<TR::Node*>& nodeArray,
                               TR::Node* n,
                               TR::Compilation* comp)
{
  ...
  for (uint32_t i = 0; i < nodeArray.size(); i++) {
    ...
    if ((nodeArray[i]->getReferenceCount() == 1) &&
      ...
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(
          comp,
          " ArtificiallyInflateReferenceCountWhenNecessary: bug potential with "
          "node %p (reg %p %s%s): ref count after %s is 1.\n",
          nodeArray[i],
      ...
      if (incrementedNodesList) {
        ...
        if (listNode == nodeArray[i]) {
          ...
          if (comp->getOption(TR_TraceCG)) {
            traceMsg(
              comp,
              " ArtificiallyInflateReferenceCountWhenNecessary: skip inflating "
              "node %p (reg %p %s%s) because it exists in "
...
void
OMR::Z::MemoryReference::populateAddTree(TR::Node* subTree,
                                         TR::CodeGenerator* cg)
{
  ...
  ArtificiallyInflateReferenceCountWhenNecessary(
    self(),
    "OMR::Z::MemoryReference::populateAddTree",
    nodesBefore,
    cg,
    &_incrementedNodesList);
}
...
void
OMR::Z::MemoryReference::populateShiftLeftTree(TR::Node* subTree,
                                               TR::CodeGenerator* cg)
{
  if (_indexRegister != NULL) {
    if (_baseRegister != NULL) {
...
void
OMR::Z::MemoryReference::populateAloadTree(TR::Node* subTree,
                                           TR::CodeGenerator* cg,
                                           bool privateArea)
{
...
void
OMR::Z::MemoryReference::populateLoadAddrTree(TR::Node* subTree,
                                              TR::CodeGenerator* cg)
{
...
bool
OMR::Z::MemoryReference::tryBaseIndexDispl(TR::CodeGenerator* cg,
                                           TR::Node* loadStore,
                                           TR::Node* topAdd)
{
  ...
  if (debug)
    traceMsg(comp,
             "&&& TBID load=%llx addr1=%llx addr2=%llx\n",
             loadStore,
             topAdd,
             addressChild);
  ...
  if (debug) {
    if (base)
      traceMsg(
        comp, "&&& TBID base %llx count %d\n", base, base->getReferenceCount());
    if (index)
      traceMsg(comp,
               "&&& TBID index %llx count %d\n",
               index,
  ...
  ArtificiallyInflateReferenceCountWhenNecessary(
    self(),
    "OMR::Z::MemoryReference::tryBaseIndexDispl",
    nodesBefore,
    cg,
    &_incrementedNodesList);
  return true;
...
void
OMR::Z::MemoryReference::populateThroughEvaluation(TR::Node* rootLoadOrStore,
                                                   TR::CodeGenerator* cg)
{
  _baseNode = NULL;
  _indexRegister = NULL;
...
bool
OMR::Z::MemoryReference::ZeroBasePtr_EvaluateSubtree(
  TR::Node* subTree,
  TR::CodeGenerator* cg,
...
void
OMR::Z::MemoryReference::populateMemoryReference(TR::Node* subTree,
                                                 TR::CodeGenerator* cg)
{
  TR::StackMemoryRegion stackMemoryRegion(*cg->trMemory());
  ...
  ArtificiallyInflateReferenceCountWhenNecessary(
    self(),
    "OMR::Z::MemoryReference::populateMemoryReference",
    nodesBefore,
    cg,
    &_incrementedNodesList);
}
...
void
OMR::Z::MemoryReference::consolidateRegisters(TR::Node* node,
                                              TR::CodeGenerator* cg)
{
  ...
  generateRXInstruction(
    cg, TR::InstOpCode::LA, node, tempTargetRegister, interimMemoryReference);
...
TR::Register*
OMR::Z::MemoryReference::swapBaseRegister(TR::Register* br,
                                          TR::CodeGenerator* cg)
{
...
TR::Instruction*
OMR::Z::MemoryReference::handleLargeOffset(
  TR::Node* node,
  ...
  if (_offset > MINLONGDISP && _offset < MAXLONGDISP) {
    ...
    if (preced)
      preced = generateRXInstruction(
        cg, op, node, tempTargetRegister, interimMemoryReference, preced);
    else
      generateRXInstruction(
        cg, op, node, tempTargetRegister, interimMemoryReference);
  } else {
    op = TR::InstOpCode::LA;
 ...
 */
bool
OMR::Z::MemoryReference::shouldLabelForRAS(TR::SymbolReference* symRef,
                                           TR::CodeGenerator* cg)
{
  return true;
}
...
bool
OMR::Z::MemoryReference::alignmentBumpMayRequire4KFixup(TR::Node* node,
                                                        TR::CodeGenerator* cg)
{
  ...
  if (_offset < MAXDISP && // skip if going to fixup naturally anyway
      self()->getFixedSizeForAlignment() >
        0 && // i.e. do not check temps -- these will have an offset of 0 anyway
 ...
 */
TR::Instruction*
OMR::Z::MemoryReference::enforceSSFormatLimits(TR::Node* node,
                                               TR::CodeGenerator* cg,
                                               TR::Instruction* preced)
{
  ...
  preced = self()->enforce4KDisplacementLimit(
    node, cg, preced, false, forceDueToAlignmentBump);
  return preced;
}

TR::Instruction*
OMR::Z::MemoryReference::enforceRSLFormatLimits(TR::Node* node,
                                                TR::CodeGenerator* cg,
                                                TR::Instruction* preced)
{
...
TR::Instruction*
OMR::Z::MemoryReference::enforceVRXFormatLimits(TR::Node* node,
                                                TR::CodeGenerator* cg,
                                                TR::Instruction* preced)
{
  bool forceDueToAlignmentBump =
 ...
 */
TR::Instruction*
OMR::Z::MemoryReference::enforce4KDisplacementLimit(TR::Node* node,
                                                    TR::CodeGenerator* cg,
                                                    TR::Instruction* preced,
                                                    bool forcePLXFixup,
                                                    bool forceFixup)
{
  ...
  TR_ASSERT(
    !forcePLXFixup,
    "This logic is only used for markAndAdjustForLongDisplacementIfNeeded. You "
    "probably want the other flag.");
  ...
  if ((_offset < 0 || _offset >= MAXDISP || forcePLXFixup || forceFixup) &&
    ...
    preced = self()->handleLargeOffset(
      node, interimMemoryReference, tempTargetRegister, cg, preced);
 ...
 */
TR::Instruction*
OMR::Z::MemoryReference::enforceDisplacementLimit(TR::Node* node,
                                                  TR::CodeGenerator* cg,
                                                  TR::Instruction* preced)
{
  if (self()->ignoreNegativeOffset())
 ...
 */
void
OMR::Z::MemoryReference::eliminateNegativeDisplacement(TR::Node* node,
                                                       TR::CodeGenerator* cg)
{
  if (self()->ignoreNegativeOffset())
    return;
  ...
  if (_offset < 0 && !cg->isDispInRange(_offset)) {
    ...
    self()->handleLargeOffset(
      node, interimMemoryReference, tempTargetRegister, cg, NULL);
 ...
 */
TR::Instruction*
OMR::Z::MemoryReference::separateIndexRegister(TR::Node* node,
                                               TR::CodeGenerator* cg,
                                               ...
                                               bool forceDueToAlignmentBump)
{
  ...
  if (_indexRegister != NULL) {
    ...
    if (!self()->ignoreNegativeOffset() && enforce4KDisplacementLimit &&
      ...
      if (preced)
        preced = generateRXInstruction(cg,
                                       TR::InstOpCode::LA,
                                       node,
                                       tempTargetRegister,
...
void
OMR::Z::MemoryReference::propagateAlignmentInfo(
 ...
 */
void
OMR::Z::MemoryReference::assignRegisters(TR::Instruction* currentInstruction,
                                         TR::CodeGenerator* cg)
{
  ...
  if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
    ...
    if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
      assignedBaseRegister = static_cast<TR::RealRegister*>(
        machine->assignBestRegisterSingle(_baseRegister,
                                          currentInstruction,
                                          NOBOOKKEEPING,
  ...
  if (_indexRegister != NULL && _indexRegister->getRealRegister() == NULL) {
    ...
    assignedIndexRegister = static_cast<TR::RealRegister*>(
      machine->assignBestRegisterSingle(_indexRegister,
                                        currentInstruction,
                                        BOOKKEEPING,
                                        ~TR::RealRegister::GPR0Mask));
...
bool
OMR::Z::MemoryReference::needsAdjustDisp(TR::Instruction* instr,
...
int32_t
OMR::Z::MemoryReference::estimateBinaryLength(int32_t currentEstimate,
                                              TR::CodeGenerator* cg,
 ...
 */
void
OMR::Z::MemoryReference::setFixedSizeForAlignment(int32_t size)
{
...
int32_t
OMR::Z::MemoryReference::getRightAlignmentBump(TR::Instruction* instr,
                                               TR::CodeGenerator* cg)
{
...
int32_t
OMR::Z::MemoryReference::getLeftAlignmentBump(TR::Instruction* instr,
                                              TR::CodeGenerator* cg)
{
  ...
  TR_ASSERT(
    self()->getTotalSizeForAlignment() > 0,
...
int32_t
OMR::Z::MemoryReference::getSizeIncreaseBump(TR::Instruction* instr,
                                             TR::CodeGenerator* cg)
{
...
int32_t
OMR::Z::MemoryReference::calcDisplacement(uint8_t* cursor,
                                          TR::Instruction* instr,
                                          TR::CodeGenerator* cg)
{
...
generateImmToRegister(TR::CodeGenerator* cg,
                      TR::Node* node,
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      cursor =
        generateRSInstruction(cg,
                              TR::InstOpCode::SLL, // SLL Rscrtch,Rscrtch,12
                              node,
                              targetRegister,
                              12,
                              cursor);
 ...
 */
int32_t
OMR::Z::MemoryReference::generateBinaryEncoding(uint8_t* cursor,
                                                TR::CodeGenerator* cg,
                                                TR::Instruction* instr)
{
  ...
  TR::RealRegister* base =
    (self()->getBaseRegister() ? toRealRegister(self()->getBaseRegister()) : 0);
  TR::RealRegister* index =
  ...
  if (displacement < MINDISP || displacement >= MAXDISP) {
    ...
    if ((displacement < MAXLONGDISP && displacement > MINLONGDISP) &&
      ...
      if (longDisplacementMnemonic != TR::InstOpCode::BAD) {
        TR::DebugCounter::incStaticDebugCounter(
          ...
          TR::DebugCounter::debugCounterName(
            comp,
            "z/memref/long-displacement-upgrade/(%s)",
            comp->signature()));
      }
    } else {
      ...
      if (spillNeeded) {
        currInstr = generateRXInstruction(
          ...
          generateS390MemoryReference(
            LongDispSlotBaseReg, offsetToLongDispSlot, cg),
          prevInstr);
      ...
      currInstr = generateImmToRegister(
        cg, node, scratchReg, (intptr_t)(displacement), prevInstr);
      currInstr->setEstimatedBinaryLength(
        currInstr->getOpCode().getInstructionLength());
      ...
      if (instructionFormat == TR::Instruction::IsRS ||
        ...
        TR_ASSERT_FATAL(base != NULL,
                        ...
                        "on %s [%p] instruction",
                        instr->getOpCode().getMnemonicName(),
                        instr);
        ...
        currInstr = generateRXInstruction(
          ...
          node,
          scratchReg,
          generateS390MemoryReference(base, scratchReg, 0, cg),
          prevInstr);
        currInstr->setEstimatedBinaryLength(
          currInstr->getOpCode().getInstructionLength());
        cg->setBinaryBufferCursor(cursor = currInstr->generateBinaryEncoding());
      ...
      } else if (index != NULL && base != NULL) // Neither is NULL, pick one
      {
        ...
        currInstr = generateRXInstruction(
          ...
          node,
          scratchReg,
          generateS390MemoryReference(index, scratchReg, 0, cg),
          prevInstr);
        currInstr->setEstimatedBinaryLength(
          currInstr->getOpCode().getInstructionLength());
        cg->setBinaryBufferCursor(cursor = currInstr->generateBinaryEncoding());
      ...
      TR::DebugCounter::incStaticDebugCounter(
        comp,
        TR::DebugCounter::debugCounterName(
          comp, "z/memref/huge-displacement-upgrade/(%s)", comp->signature()));

      if (comp->getOption(TR_TraceCG))
        traceMsg(comp,
                 "[%p] Long Disp Inst using %s as scratch reg\n",
                 instr,
  ...
  if (instructionFormat == TR::Instruction::IsRXY ||
    ...
    *reinterpret_cast<uint32_t*>(cursor + 4) |=
    ...
    TR_ASSERT_FATAL(displacement >= MINDISP && displacement < MAXDISP,
                    "Displacement (%d) is out of range",
                    displacement);
...
TR::MemoryReference*
generateS390MemoryReference(int32_t iValue,
                            TR::DataType type,
                            TR::CodeGenerator* cg,
                            TR::Register* treg,
                            TR::Node* node)
...
TR::MemoryReference*
generateS390MemoryReference(int64_t iValue,
                            TR::DataType type,
                            TR::CodeGenerator* cg,
                            TR::Register* treg,
                            TR::Node* node)
...
TR::MemoryReference*
generateS390MemoryReference(float fValue,
                            TR::DataType type,
                            TR::CodeGenerator* cg,
                            TR::Node* node)
{
...
TR::MemoryReference*
generateS390MemoryReference(double dValue,
                            TR::DataType type,
                            TR::CodeGenerator* cg,
                            TR::Node* node)
{
...
TR::MemoryReference*
generateS390MemoryReference(TR::Register* br,
                            int32_t disp,
                            TR::CodeGenerator* cg,
                            const char* name)
{
...
TR::MemoryReference*
generateS390MemoryReference(TR::Node* node,
                            TR::CodeGenerator* cg,
                            bool canUseRX)
{
...
findBestSymRefForArrayCopy(TR::CodeGenerator* cg,
                            ...
                            bool forSS)
{
  ...
  TR::MemoryReference* mr =
    generateS390MemoryReference(cg, node->getSymbolReference(), baseNode, disp);
  TR::SymbolReference* symRefForAliasing =
...
TR::MemoryReference*
generateS390ConstantAreaMemoryReference(TR::Register* br,
                                        int32_t disp,
                                        TR::Node* node,
                                        TR::CodeGenerator* cg,
                                        bool forSS)
...
TR::MemoryReference*
generateS390MemoryReference(TR::MemoryReference& mr,
                            int32_t n,
                            TR::CodeGenerator* cg)
{
...
TR::MemoryReference*
reuseS390MemoryReference(TR::MemoryReference* baseMR,
                         int32_t offset,
                         TR::Node* node,
                         TR::CodeGenerator* cg,
...
TR::MemoryReference*
generateS390MemoryReference(TR::Snippet* tas,
                            TR::CodeGenerator* cg,
                            TR::Register* base,
                            TR::Node* node)
{
...
static TR::MemoryReference*
generateS390AlignedMemoryReference(TR::MemoryReference& baseMR,
                                   TR::Node* node,
                                   int32_t offset,
                                   TR::CodeGenerator* cg,
                                   bool enforceSSLimits)
...
TR::MemoryReference*
generateS390RightAlignedMemoryReference(TR::MemoryReference& baseMR,
                                        TR::Node* node,
                                        int32_t offset,
                                        TR::CodeGenerator* cg,
                                        bool enforceSSLimits)
...
TR::MemoryReference*
generateS390LeftAlignedMemoryReference(TR::MemoryReference& baseMR,
                                       TR::Node* node,
                                       int32_t offset,
                                       TR::CodeGenerator* cg,
