...
void OMR::Z::MemoryReference::addToTemporaryNegativeOffset(TR::Node* node, int32_t offset, TR::CodeGenerator* cg)
{
  ...
  if (offset < 0) {
    if (cg->traceBCDCodeGen())
      traceMsg(comp,
        "\taddToTemporaryNegativeOffset : %s (%p) new offset %d, existing mr offset %d (existing mr hasTempNegOffset = "
        "%s)\n",
        node ? node->getOpCode().getName() : "NULL", node, offset, self()->getOffset(),
        self()->hasTemporaryNegativeOffset() ? "yes" : "no");
    if (self()->getOffset() < 0
      && !self()->hasTemporaryNegativeOffset()) // if there is a 'real' negative offset then deal with this first before
                                                // adding in the temporary negative offset
    {
      if (cg->traceBCDCodeGen())
        traceMsg(comp,
...
bool OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(TR::CodeGenerator* cg, TR::Node* addressChild)
{
  ...
  if (!addressChild->getOpCode().isAdd()) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) is not an add type - most likely cannot fold\n",
  ...
  if (!addressChild->getSecondChild()->getOpCode().isLoadConst()) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) second child is not a load const - cannot fold\n",
        addressChild->getOpCode().getName(), addressChild);
    }
    return false;
  }
  ...
  if (haveToEvalConvIntoRegister) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) first child has conv. with ref count > 1 and no "
        "register, must evaluate - setForceFolding()\n",
        addressChild->getOpCode().getName(), addressChild);
    }
  ...
  if (!allConvNodesAreUnneeded) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) first child has conv. which are needed, folding "
        "after evaluation.\n",
        addressChild->getOpCode().getName(), addressChild);
    }
    cg->evaluate(addressChild->getFirstChild());
  ...
  bool foundRegister
    = eventualNonConversion->getRegister() || (foundConvNodeWithRegister && foundConvNodeWithRegister->getRegister());
  ...
  if (foundRegister) {
    ...
    if (cg->traceBCDCodeGen()) {
      ...
      traceMsg(comp, "\t\taddressChild %s (%p) node %s (%p) is evaluatedReg+const (%s + %lld) so %s=true\n",
        ...
        cg->getDebug()->getName(dispNode->getRegister()), addressChild->getSecondChild()->get64bitIntegralValue(),
  ...
  if (!eventualNonConversionIsSuitableForFolding) {
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
        " inside setForceFoldingIfAdvantageous, addressChild %s (%p) eventual non conversion %s (%p) is not suitable "
        "for folding.\n",
  ...
  if (!eventualNonConversion->getOpCode().isIndirect()) {
    ...
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
        " inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register and refCount==1 and is a "
        "aload+const so setForceFolding=true\n",
  ...
  } else // if (eventualNonConversion->getReferenceCount() == 1)
  {
    ...
    if (cg->traceBCDCodeGen()) {
      traceMsg(comp,
        " inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register and refCount==1 and is "
        "an iaload+const so eval(%s - %p) and setForceFolding=true\n",
        eventualNonConversion->getOpCode().getName(), eventualNonConversion,
      ...
      if (startRemoving) {
        if (cg->traceBCDCodeGen()) {
          traceMsg(comp, "\t decReferenceCount on incremented node %p (%d) (%d->%d)\n", cur, cur->getGlobalIndex(),
            cur->getReferenceCount(), cur->getReferenceCount() - 1);
        }
  ...
  , _incrementedNodesList(cg->comp()->trMemory())
{
  ...
  if (rootLoadOrStore->getOpCode().isIndirect() || self()->isExposedConstantAddressing()) {
    ...
    if ((symbol && symbol->isStatic()) && rootLoadOrStore->getFirstChild()->getOpCode().hasSymbolReference()
      ...
      && rootLoadOrStore->getFirstChild()->getSymbolReference()->isUnresolved() && symRef->isUnresolved() && symRef
      && !symRef->isFromLiteralPool() && rootLoadOrStore->getFirstChild()->getSymbolReference()->isFromLiteralPool()) {
      if (alloc) {
    ...
    if (symbol && symbol->isStatic()) {
      ...
      if (symRef && symRef->isUnresolved()) {
        ...
        genLoadAddressConstant(
          cg, rootLoadOrStore, (uintptrj_t)symRef->getSymbol()->getStaticSymbol()->getStaticAddress(), _baseRegister);
        cg->stopUsingRegister(tempReg);
      }
    } else if (!symRef) {
...
const bool OMR::Z::MemoryReference::isLongDisplacementRequired()
{
  ...
  return (
    (displacement > MINLONGDISP && displacement < MINDISP) || (displacement >= MAXDISP && displacement < MAXLONGDISP));
}
...
void OMR::Z::MemoryReference::bookKeepingRegisterUses(TR::Instruction* instr, TR::CodeGenerator* cg)
{
  if (_baseRegister != NULL) {
    ...
    if (TR::Compiler->target.is64Bit()
      && (instr->getOpCode().isLoad() || instr->getOpCode().isStore() || instr->getOpCodeValue() == TR::InstOpCode::LA
        || instr->getOpCodeValue() == TR::InstOpCode::LARL || instr->getOpCodeValue() == TR::InstOpCode::LAY
        || instr->getOpCodeValue() == TR::InstOpCode::LAE || instr->getOpCodeValue() == TR::InstOpCode::LAEY)) {
  ...
  for (uint32_t i = 0; i < nodeArray.size(); i++) {
    ...
    if ((nodeArray[i]->getReferenceCount() == 1) && (nodeArray[i]->getRegister() != NULL)) {
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp,
          " ArtificiallyInflateReferenceCountWhenNecessary: bug potential with node %p (reg %p %s%s): ref count after "
          "%s is 1.\n",
      ...
      if (incrementedNodesList) {
        ...
        if (listNode == nodeArray[i]) {
          // Skip processing the current node because the reference count was already artificially inflated somewhere
          // else.
          if (comp->getOption(TR_TraceCG)) {
            traceMsg(comp,
              " ArtificiallyInflateReferenceCountWhenNecessary: skip inflating node %p (reg %p %s%s) because it exists "
              "in incrementedNodesList.\n",
              nodeArray[i], nodeArray[i]->getRegister(),
              nodeArray[i]->getRegister() ? (nodeArray[i]->getRegister()->isPlaceholderReg() ? "D_" : "") : "?",
              comp->getDebug()->getName(nodeArray[i]->getRegister()), functionString);
          }
...
void OMR::Z::MemoryReference::populateAddTree(TR::Node* subTree, TR::CodeGenerator* cg)
{
  ...
  if ((integerChild->getOpCodeValue() == TR::isub || integerChild->getOpCodeValue() == TR::lsub)) {
     ...
     * instruction is zero. When we fold the lsub the lRegLoad becomes the _baseRegister and the lconst becomes offset.
    ...
    if (subTree->getOpCode().isArrayRef()
      || (subTree->getOpCodeValue() == TR::iadd || subTree->getOpCodeValue() == TR::ladd
        || ((subTree->getOpCodeValue() == TR::isub || subTree->getOpCodeValue() == TR::lsub)
          && subTree->getChild(1)->getOpCode().isLoadConst() && subTree->getChild(1)->getConst<int64_t>() <= 0))) {
...
bool OMR::Z::MemoryReference::alignmentBumpMayRequire4KFixup(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (_offset < MAXDISP && // skip if going to fixup naturally anyway
    ...
    int32_t maxAlignmentBump = self()->getFixedSizeForAlignment()
      - 1; // alignment is getFixedSizeForAlignment() - (length or leftMostByte) and each of these must be at least 1
  ...
  TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced)
{
  ...
  preced
    = self()->separateIndexRegister(node, cg, true, preced, forceDueToAlignmentBump); // enforce4KDisplacementLimit=true
  ...
  bool enforce4KDisplacementLimit, TR::Instruction* preced, bool forceDueToAlignmentBump)
{
  ...
  if (_indexRegister != NULL) {
    ...
    if (!self()->ignoreNegativeOffset() && enforce4KDisplacementLimit
      ...
      if (preced)
        preced
          = generateRXInstruction(cg, TR::InstOpCode::LA, node, tempTargetRegister, interimMemoryReference, preced);
      else
        generateRXInstruction(cg, TR::InstOpCode::LA, node, tempTargetRegister, interimMemoryReference);
    }
...
void OMR::Z::MemoryReference::assignRegisters(TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
  ...
  if (_indexRegister != NULL && _indexRegister->getRealRegister() == NULL) {
    ...
    assignedIndexRegister = static_cast<TR::RealRegister*>(
      machine->assignBestRegisterSingle(_indexRegister, currentInstruction, BOOKKEEPING, ~TR::RealRegister::GPR0Mask));
  ...
  TR::Instruction* instr, OMR::Z::MemoryReference* mRef, TR::CodeGenerator* cg)
{
  ...
  bool isValidOp = (opCode == TR::InstOpCode::A || opCode == TR::InstOpCode::AL || opCode == TR::InstOpCode::C
    ...
    || opCode == TR::InstOpCode::LE || opCode == TR::InstOpCode::MEEB || opCode == TR::InstOpCode::MAEB
    || opCode == TR::InstOpCode::SEB || opCode == TR::InstOpCode::MEE || opCode == TR::InstOpCode::MAE
...
int32_t OMR::Z::MemoryReference::getRightAlignmentBump(TR::Instruction* instr, TR::CodeGenerator* cg)
{
  ...
  // a negative bump means that the allocated symbol size is not big enough to contain the encoded operation and this is
  // surely wrong.
...
int32_t OMR::Z::MemoryReference::getSizeIncreaseBump(TR::Instruction* instr, TR::CodeGenerator* cg)
{
  ...
  TR_ASSERT(
    self()->getSymbolReference()->getSymbol()->getSize() > 0, "symbol size not initialized in getSizeIncreaseBump()\n");
...
int32_t OMR::Z::MemoryReference::calcDisplacement(uint8_t* cursor, TR::Instruction* instr, TR::CodeGenerator* cg)
{
  ...
  if (self()->rightAlignMemRef()) {
    ...
    if (cg->traceBCDCodeGen())
      traceMsg(comp,
        "instr %p : right aligning memRef with symRef #%d (%s, isTemp %s) _offset %d, totalSize %d : bump = ", instr,
  ...
  if (self()->leftAlignMemRef()) {
    ...
    if (cg->traceBCDCodeGen())
      traceMsg(comp,
        "instr %p : left aligning memRef with symRef #%d (%s, isTemp %s), _offset %d : bump = totalSize - leftMostByte "
        "= %d - %d = ",
  ...
  if (snippet != NULL) {
    ...
    if (symbol->isRegisterMappedSymbol()) {
      ...
      if (TR::Compiler->target.is64Bit() && TR::MemoryReference::needsAdjustDisp(instr, this, cg)
  ...
  TR::CodeGenerator* cg, TR::Node* node, TR::Register* targetRegister, intptr_t value, TR::Instruction* cursor)
{
  ...
  if (value >= (int32_t)0xFFFF8000 && value <= (int32_t)0x00007FFF) {
    ...
    low = (value & 0xfff);
    cursor
      = generateRIInstruction(cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), node, // LHI Rscrtch, (value>>12)&0xFFFF
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      cursor
        = generateRSInstruction(cg, TR::InstOpCode::SLL, node, targetRegister, 12, cursor); // SLL Rscrtch,Rscrtch,12
    }
    ...
    if ((displacement < MAXLONGDISP && displacement > MINLONGDISP)
      ...
      if (longDisplacementMnemonic != TR::InstOpCode::BAD) {
        TR::DebugCounter::incStaticDebugCounter(
          comp, TR::DebugCounter::debugCounterName(comp, "z/memref/long-displacement-upgrade/(%s)", comp->signature()));
      }
    } else {
      ...
      TR_ASSERT(
        scratchReg != NULL, "OMR::Z::MemoryReference::generateBinaryEncoding -- A scratch reg should always be found.");
      ...
      if (instructionFormat == TR::Instruction::IsRS || instructionFormat == TR::Instruction::IsRSY
        ...
        TR_ASSERT_FATAL(base != NULL, "Expected non-NULL base register for long displacement on %s [%p] instruction",
        ...
        currInstr = generateRXInstruction(
          cg, TR::InstOpCode::LA, node, scratchReg, generateS390MemoryReference(base, scratchReg, 0, cg), prevInstr);
        currInstr->setEstimatedBinaryLength(currInstr->getOpCode().getInstructionLength());
        cg->setBinaryBufferCursor(cursor = currInstr->generateBinaryEncoding());
        prevInstr = currInstr;
      ...
      } else if (index != NULL && base != NULL) // Neither is NULL, pick one
      {
        ...
        currInstr = generateRXInstruction(
          cg, TR::InstOpCode::LA, node, scratchReg, generateS390MemoryReference(index, scratchReg, 0, cg), prevInstr);
        currInstr->setEstimatedBinaryLength(currInstr->getOpCode().getInstructionLength());
        cg->setBinaryBufferCursor(cursor = currInstr->generateBinaryEncoding());
        prevInstr = currInstr;
      ...
      {
        TR_ASSERT_FATAL(instructionFormat == TR::Instruction::IsRX || instructionFormat == TR::Instruction::IsVRX
      ...
      {
        TR_ASSERT_FATAL(instructionFormat == TR::Instruction::IsRX || instructionFormat == TR::Instruction::IsRXY
      ...
      TR::DebugCounter::incStaticDebugCounter(
        comp, TR::DebugCounter::debugCounterName(comp, "z/memref/huge-displacement-upgrade/(%s)", comp->signature()));
...
TR::MemoryReference* generateS390MemoryReference(TR::Node* node, TR::CodeGenerator* cg, bool canUseRX)
{
