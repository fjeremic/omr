...
{
    if (cg->getSupportsImplicitNullChecks() && cg->getCurrentEvaluationTreeTop()->getNode()->getOpCode().isNullCheck()) {
...
bool OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(TR::CodeGenerator* cg, TR::Node* addressChild)
{
    ...
    while (eventualNonConversion->getOpCode().isConversion()) {
        ...
        haveToEvalConvIntoRegister = haveToEvalConvIntoRegister || ((eventualNonConversion->getReferenceCount() != 1) && (eventualNonConversion->getRegister() == NULL));
    ...
    bool foundRegister = eventualNonConversion->getRegister() || (foundConvNodeWithRegister && foundConvNodeWithRegister->getRegister());
    ...
    bool eventualNonConversionIsSuitableForFolding = (eventualNonConversion->getOpCodeValue() == TR::loadaddr || eventualNonConversion->getOpCode().isLoad() || eventualNonConversion->getRegister() || eventualNonConversion->getOpCode().isLoadReg());
    ...
    , _incrementedNodesList(cg->comp()->trMemory())
{
    ...
    if (rootLoadOrStore->getOpCode().isIndirect() || self()->isExposedConstantAddressing()) {
        ...
        if (subTree->getOpCodeValue() == TR::aconst) {
            ...
            bool tryBID = (addressChild->getOpCodeValue() == TR::aiadd || addressChild->getOpCodeValue() == TR::aladd) && _baseRegister == NULL && _indexRegister == NULL && self()->tryBaseIndexDispl(cg, rootLoadOrStore, addressChild);
            if (!tryBID) {
        ...
        if ((symbol && symbol->isStatic()) && rootLoadOrStore->getFirstChild()->getOpCode().hasSymbolReference() && rootLoadOrStore->getFirstChild()->getSymbolReference()->getSymbol()->isStatic() && rootLoadOrStore->getFirstChild()->getSymbolReference()->isUnresolved() && symRef->isUnresolved() && symRef && !symRef->isFromLiteralPool() && rootLoadOrStore->getFirstChild()->getSymbolReference()->isFromLiteralPool()) {
            if (alloc) {
    ...
    , _incrementedNodesList(cg->comp()->trMemory())
{
    bool done = false;
    if (addressChild->getRegister() == NULL) {
        if (addressChild->getReferenceCount() == 1 && addressChild->getOpCode().isAdd() && addressChild->getSecondChild()->getOpCode().isLoadConst()) {
...
void OMR::Z::MemoryReference::initSnippetPointers(TR::Snippet* s, TR::CodeGenerator* cg)
{
    ...
    } else if (s->getKind() == TR::Snippet::IsWritableData || s->getKind() == TR::Snippet::IsConstantInstruction || s->getKind() == TR::Snippet::IsConstantData) {
...
{
    if ((_baseRegister != NULL) && ((_baseRegister->getRealRegister() == NULL) || (_baseRegister->getRealRegister()->getState() != TR::RealRegister::Locked))) {
        cg->stopUsingRegister(_baseRegister);
    }
...
void OMR::Z::MemoryReference::bookKeepingRegisterUses(TR::Instruction* instr, TR::CodeGenerator* cg)
{
    if (_baseRegister != NULL) {
        ...
        if (TR::Compiler->target.is64Bit() && (instr->getOpCode().isLoad() || instr->getOpCode().isStore() || instr->getOpCodeValue() == TR::InstOpCode::LA || instr->getOpCodeValue() == TR::InstOpCode::LARL || instr->getOpCodeValue() == TR::InstOpCode::LAY || instr->getOpCodeValue() == TR::InstOpCode::LAE || instr->getOpCodeValue() == TR::InstOpCode::LAEY)) {
...
void OMR::Z::MemoryReference::populateAddTree(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (integerChild->getOpCode().isLoadConst() && (!comp->useCompressedPointers() || (integerChild->getOpCodeValue() != TR::lconst) || (integerChild->getLongInt() != TR::Compiler->vm.heapBaseAddress()))) {
        self()->populateMemoryReference(addressChild, cg);
        if ((subTree->getOpCodeValue() != TR::iadd) && (subTree->getOpCodeValue() != TR::aiadd) && (subTree->getOpCodeValue() != TR::iuadd) && (subTree->getOpCodeValue() != TR::aiuadd) && (subTree->getOpCodeValue() != TR::isub)) {
    ...
    } else if (_baseRegister == NULL && _indexRegister == NULL && integerChild->getReferenceCount() == 1 && (integerChild->getOpCodeValue() == TR::isub || integerChild->getOpCodeValue() == TR::lsub)) {
        //
...
void OMR::Z::MemoryReference::populateShiftLeftTree(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (((subTree->getOpCodeValue() == TR::ishl) && (subTree->getSecondChild()->getOpCodeValue() == TR::iconst) && (subTree->getSecondChild()->getInt() == 1)) || ((subTree->getOpCodeValue() == TR::lshl) && (subTree->getSecondChild()->getOpCodeValue() == TR::iconst) && (subTree->getSecondChild()->getInt() == 1))) {
...
void OMR::Z::MemoryReference::populateMemoryReference(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (self()->doEvaluate(subTree, cg)) {
        ...
        if (subTree->getOpCode().isArrayRef() || (subTree->getOpCodeValue() == TR::iadd || subTree->getOpCodeValue() == TR::ladd || ((subTree->getOpCodeValue() == TR::isub || subTree->getOpCodeValue() == TR::lsub) && subTree->getChild(1)->getOpCode().isLoadConst() && subTree->getChild(1)->getConst<int64_t>() <= 0))) {
            self()->populateAddTree(subTree, cg);
        } else if ((subTree->getOpCodeValue() == TR::ishl || subTree->getOpCodeValue() == TR::lshl) && subTree->getSecondChild()->getOpCode().isLoadConst()) {
...
void OMR::Z::MemoryReference::consolidateRegisters(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((_baseRegister && (_baseRegister->containsCollectedReference() || _baseRegister->containsInternalPointer())) || (_indexRegister && (_indexRegister->containsCollectedReference() || _indexRegister->containsInternalPointer()))) {
 ...
 */
bool OMR::Z::MemoryReference::shouldLabelForRAS(TR::SymbolReference* symRef, TR::CodeGenerator* cg)
{
    return true;
}
...
OMR::Z::MemoryReference::enforce4KDisplacementLimit(TR::Node* node, TR::CodeGenerator* cg, TR::Instruction* preced, bool forcePLXFixup, bool forceFixup)
{
    ...
    if ((_offset < 0 || _offset >= MAXDISP || forcePLXFixup || forceFixup) && !self()->isAdjustedForLongDisplacement()) {
...
OMR::Z::MemoryReference::separateIndexRegister(TR::Node* node, TR::CodeGenerator* cg, bool enforce4KDisplacementLimit, TR::Instruction* preced, bool forceDueToAlignmentBump)
{
    ...
    if (_indexRegister != NULL) {
        ...
        if (!self()->ignoreNegativeOffset() && enforce4KDisplacementLimit && (_offset < 0 || _offset >= MAXDISP || forceDueToAlignmentBump)) {
            preced = self()->handleLargeOffset(node, interimMemoryReference, tempTargetRegister, cg, preced);
            _offset = 0;
        } else {
            if (preced)
...
bool OMR::Z::MemoryReference::needsAdjustDisp(TR::Instruction* instr, OMR::Z::MemoryReference* mRef, TR::CodeGenerator* cg)
{
    ...
    bool isValidOp = (opCode == TR::InstOpCode::A || opCode == TR::InstOpCode::AL || opCode == TR::InstOpCode::C || opCode == TR::InstOpCode::CL || opCode == TR::InstOpCode::L || opCode == TR::InstOpCode::M || opCode == TR::InstOpCode::MS || opCode == TR::InstOpCode::N || opCode == TR::InstOpCode::O || opCode == TR::InstOpCode::S || opCode == TR::InstOpCode::ST || opCode == TR::InstOpCode::X || opCode == TR::InstOpCode::AEB || opCode == TR::InstOpCode::CEB || opCode == TR::InstOpCode::DEB || opCode == TR::InstOpCode::AE || opCode == TR::InstOpCode::CE || opCode == TR::InstOpCode::DE || opCode == TR::InstOpCode::LE || opCode == TR::InstOpCode::MEEB || opCode == TR::InstOpCode::MAEB || opCode == TR::InstOpCode::SEB || opCode == TR::InstOpCode::MEE || opCode == TR::InstOpCode::MAE || opCode == TR::InstOpCode::SE || opCode == TR::InstOpCode::STE);
    bool isValidSym = comp->getSymRefTab()->isNonHelper(symRef->getReferenceNumber(), TR::SymbolReferenceTable::contiguousArraySizeSymbol) || comp->getSymRefTab()->isNonHelper(symRef->getReferenceNumber(), TR::SymbolReferenceTable::discontiguousArraySizeSymbol) || sym == comp->getSymRefTab()->findGenericIntShadowSymbol();
...
{
    if (self()->getOriginalSymbolReference() && self()->getOriginalSymbolReference()->getSymbol() && self()->getOriginalSymbolReference()->getSymbol()->isVariableSizeSymbol()) {
        TR_ASSERT(false, "variable sized symbols do not use _fixedSizeForAlignment a\n");
        _fixedSizeForAlignment = 0;
    } else if (self()->getSymbolReference() && self()->getSymbolReference()->getSymbol() && self()->getSymbolReference()->getSymbol()->isVariableSizeSymbol()) {
...
OMR::Z::MemoryReference::generateBinaryEncoding(uint8_t* cursor, TR::CodeGenerator* cg, TR::Instruction* instr)
{
    ...
    if (displacement < MINDISP || displacement >= MAXDISP) {
        ...
        if ((displacement < MAXLONGDISP && displacement > MINLONGDISP)
            ...
            if (instructionFormat == TR::Instruction::IsRS || instructionFormat == TR::Instruction::IsRSY || instructionFormat == TR::Instruction::IsRSL || instructionFormat == TR::Instruction::IsSI || instructionFormat == TR::Instruction::IsSIY || instructionFormat == TR::Instruction::IsSIL || instructionFormat == TR::Instruction::IsSS1 || instructionFormat == TR::Instruction::IsSS2 || instructionFormat == TR::Instruction::IsSS4 || instructionFormat == TR::Instruction::IsS || instructionFormat == TR::Instruction::IsRIS || instructionFormat == TR::Instruction::IsRRS || instructionFormat == TR::Instruction::IsSSE || instructionFormat == TR::Instruction::IsSSF ||
                // Vector instruction formats that have 12-bit displacements and no index registers
                instructionFormat == TR::Instruction::IsVSI || instructionFormat == TR::Instruction::IsVRV || instructionFormat == TR::Instruction::IsVRSa || instructionFormat == TR::Instruction::IsVRSb || instructionFormat == TR::Instruction::IsVRSc || instructionFormat == TR::Instruction::IsVRSd) {
            ...
            {
                TR_ASSERT_FATAL(instructionFormat == TR::Instruction::IsRX || instructionFormat == TR::Instruction::IsVRX || instructionFormat == TR::Instruction::IsRXY || instructionFormat == TR::Instruction::IsRXYb || instructionFormat == TR::Instruction::IsRXE || instructionFormat == TR::Instruction::IsRXF,
            ...
            {
                TR_ASSERT_FATAL(instructionFormat == TR::Instruction::IsRX || instructionFormat == TR::Instruction::IsRXY || instructionFormat == TR::Instruction::IsRXYb || instructionFormat == TR::Instruction::IsRXE || instructionFormat == TR::Instruction::IsRXF,
                    "Unexpected instruction type for long displacement on %s [%p] instruction", instr->getOpCode().getMnemonicName(), instr);
    ...
    if (instructionFormat == TR::Instruction::IsRXY || instructionFormat == TR::Instruction::IsRXYb || instructionFormat == TR::Instruction::IsRSY || instructionFormat == TR::Instruction::IsSIY) {
    ...
    if (!largeDisp && self()->getCheckForLongDispSlot() && displacement == offsetToLongDispSlot && ((base == LongDispSlotBaseReg && index == NULL) || (index == LongDispSlotBaseReg && base == NULL))) {
...
bool OMR::Z::MemoryReference::doEvaluate(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    // to the disp
    if ((subTree->getOpCode().isArrayRef() || (subTree->getOpCodeValue() == TR::ladd || subTree->getOpCodeValue() == TR::iadd)) && subTree->getSecondChild()->getOpCode().isLoadConst() && !cg->isDispInRange(subTree->getSecondChild()->get64bitIntegralValue()))
        return true;
...
static TR::SymbolReference* findBestSymRefForArrayCopy(TR::CodeGenerator* cg, TR::Node* arrayCopyNode, TR::Node* srcNode)
{
    ...
    if (srcNode->getOpCodeValue() == TR::aiadd || srcNode->getOpCodeValue() == TR::aiuadd || srcNode->getOpCodeValue() == TR::aladd || srcNode->getOpCodeValue() == TR::aluadd || srcNode->getOpCodeValue() == TR::asub) {
