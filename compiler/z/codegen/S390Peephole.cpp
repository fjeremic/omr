...
bool TR_S390PreRAPeephole::attemptLoadStoreReduction(TR::InstOpCode::Mnemonic storeOpCode, uint16_t size)
{
    if (_cursor->getNext()->getOpCodeValue() == storeOpCode) {
        ...
        if (loadInst->getMemoryReference()->getSymbolReference() && loadInst->getMemoryReference()->getSymbolReference()->isUnresolved() || storeInst->getMemoryReference()->getSymbolReference() && storeInst->getMemoryReference()->getSymbolReference()->isUnresolved()) {
            return false;
        }

        if (!loadInst->getMemoryReference()->getBaseRegister() && !loadInst->getMemoryReference()->getIndexRegister() || !storeInst->getMemoryReference()->getBaseRegister() && !storeInst->getMemoryReference()->getIndexRegister()) {
            return false;
        }
...
{
    while (inst && (inst->getKind() == TR::Instruction::IsPseudo || inst->getKind() == TR::Instruction::IsNotExtended || inst->getKind() == TR::Instruction::IsLabel)) {
...
{
    while (inst && (inst->getKind() == TR::Instruction::IsPseudo || inst->getKind() == TR::Instruction::IsNotExtended) && !inst->isRet()) {
        inst = forward ? inst->getNext() : inst->getPrev();
    }
...
bool TR_S390PostRAPeephole::AGIReduction()
{
    ...
    if (!(curOp == TR::InstOpCode::LR || curOp == TR::InstOpCode::LTR || curOp == TR::InstOpCode::LGR || curOp == TR::InstOpCode::LTGR)) {
        return false;
    }
    ...
    if (toRealRegister(lgrSourceReg) == gpr0 || (lgrTargetReg == lgrSourceReg) || toRealRegister(lgrTargetReg) == _cg->getStackPointerRealRegister(NULL))
        return performed;
    ...
    while ((current != NULL) && !current->matchesTargetRegister(lgrTargetReg) && !isBarrierToPeepHoleLookback(current) && windowSize < MaxWindowSize) {
        // do not look across Transactional Regions, the Register save mask is optimistic and does not allow renaming
        if (current->getOpCodeValue() == TR::InstOpCode::TBEGIN || current->getOpCodeValue() == TR::InstOpCode::TBEGINC || current->getOpCodeValue() == TR::InstOpCode::TEND || current->getOpCodeValue() == TR::InstOpCode::TABORT) {
            return false;
        }
    ...
    if (attemptLA) {
        ...
        windowSize = 0;
        while ((current != NULL) && !isBarrierToPeepHoleLookback(current) && !current->matchesTargetRegister(lgrSourceReg) && windowSize < MaxLAWindowSize) {
...
bool TR_S390PostRAPeephole::LAReduction()
{
    ...
    if (s390Instr->getKind() == TR::Instruction::IsRX) {
        ...
        if (mr && mr->getOffset() == 0 && laBaseReg && laTargetReg && laBaseReg == laTargetReg && laIndexReg == NULL && (symRef == NULL || symRef->getOffset() == 0) && (symRef == NULL || symRef->getSymbol() == NULL)) {
            // remove LA Rx,0(Rx)
            if (comp()->getOption(TR_TraceCG)) {
                printInfo("\n");
            }
            ...
            if (cast_na->isImm() == cast_nb->isImm()) {
                if (cast_na->isImm()) {
                    if (cast_na->getSourceImmediate() == cast_nb->getSourceImmediate()) {
                        if (cast_na->matchesTargetRegister(cast_nb->getRegisterOperand(1)) && cast_nb->matchesTargetRegister(cast_na->getRegisterOperand(1))) {
...
findActiveCCInst(TR::Instruction* curr, TR::InstOpCode::Mnemonic op, TR::Register* ccReg)
{
    ...
    while (next = next->getNext()) {
        if (next->getOpCodeValue() == op)
            return next;
        if (next->usesRegister(ccReg) || next->isLabel() || next->getOpCode().setsCC() || next->isCall() || next->getOpCode().setsCompareFlag())
            break;
    }
    return NULL;
}
...
bool TR_S390PostRAPeephole::seekRegInFutureMemRef(int32_t maxWindowSize, TR::Register* targetReg)
{
    ...
    while ((current != NULL) && !current->matchesTargetRegister(targetReg) && !isBarrierToPeepHoleLookback(current) && windowSize < maxWindowSize) {
        // does instruction load or store? otherwise just ignore and move to next instruction
        if (current->isLoad() || current->isStore()) {
            TR::MemoryReference* mr = current->getMemoryReference();
...
bool TR_S390PostRAPeephole::LRReduction()
{
    ...
    if (lgrTargetReg == lgrSourceReg && (lgrOpCode.getOpCodeValue() == TR::InstOpCode::LR || lgrOpCode.getOpCodeValue() == TR::InstOpCode::LGR || lgrOpCode.getOpCodeValue() == TR::InstOpCode::LDR || lgrOpCode.getOpCodeValue() == TR::InstOpCode::CPYA)) {
    ...
    // convert it to a CHI
    if (lgrTargetReg == lgrSourceReg && (lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTR || lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTGR)) {
        ...
        TR::Instruction* prev = _cursor->getPrev();
        if ((prev->getOpCodeValue() == TR::InstOpCode::LR && lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTR) || (prev->getOpCodeValue() == TR::InstOpCode::LGR && lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTGR)) {
    ...
    while ((current != NULL) && !isBarrierToPeepHoleLookback(current) && !(current->isBranchOp() && current->getKind() == TR::Instruction::IsRIL && ((TR::S390RILInstruction*)current)->getTargetSnippet()) && windowSize < maxWindowSize) {
        ...
        if ((curOpCode.getOpCodeValue() == TR::InstOpCode::CHI || curOpCode.getOpCodeValue() == TR::InstOpCode::CGHI) && ((curOpCode.is32bit() && lgrOpCode.is32bit()) || (curOpCode.is64bit() && lgrOpCode.is64bit()))) {
        ...
        if (curOpCode.getOpCodeValue() == lgrOpCode.getOpCodeValue() && current->getKind() == TR::Instruction::IsRR) {
            ...
            if (((curSourceReg == lgrTargetReg && curTargetReg == lgrSourceReg) || (curSourceReg == lgrSourceReg && curTargetReg == lgrTargetReg))) {
...
bool TR_S390PostRAPeephole::trueCompEliminationForCompare()
{
    ...
    while (nextInst && !nextInst->isLabel() && !nextInst->getOpCode().setsCC() && !nextInst->isCall() && !nextInst->getOpCode().setsCompareFlag()) {
...
bool TR_S390PostRAPeephole::trueCompEliminationForCompareAndBranch()
{
    ...
    if (backwardBranch) {
        if ((prev && btar) && (prev->usesRegister(compReg) || btar->usesRegister(compReg)) && (!prev->usesRegister(trueReg) && !btar->usesRegister(trueReg))) {
            if (comp()->getOption(TR_TraceCG)) {
                printInfo("\n");
            }
        ...
        if ((prev && next) && (prev->usesRegister(compReg) || next->usesRegister(compReg)) && (!prev->usesRegister(trueReg) && !next->usesRegister(trueReg))) {
            if (comp()->getOption(TR_TraceCG)) {
                printInfo("\n");
            }
...
bool TR_S390PostRAPeephole::DAARemoveOutlinedLabelNop(bool hasPadding)
{
    ...
    if (_cursor->throwsImplicitException()) {
        ...
        if ((nextInst->getOpCodeValue() == TR::InstOpCode::BRC || nextInst->getOpCodeValue() == TR::InstOpCode::BRCL) && (nextInst2->getOpCodeValue() == TR::InstOpCode::BRC || nextInst2->getOpCodeValue() == TR::InstOpCode::BRCL)) {
...
bool TR_S390PostRAPeephole::inlineEXtarget()
{
    ...
    while (inst && count < maxCount) {
        ...
        if (inst->isLabel()) {
            ...
            if (inst && inst->getOpCode().getOpCodeValue() == TR::InstOpCode::BRC && ((TR::S390BranchInstruction*)inst)->getBranchCondition() == TR::InstOpCode::COND_BRC) {
    ...
    if (!comp()->getOption(TR_DisableForcedEXInlining) && performTransformation(comp(), "O^O S390 PEEPHOLE: Inserting unconditional branch to hide EX target instr=[%p]\n", _cursor)) {
...
bool TR_S390PostRAPeephole::attemptZ7distinctOperants()
{
    ...
    while ((current != NULL) && !current->isLabel() && !current->isCall() && !(current->isBranchOp() && !(current->isExceptBranchOp())) && windowSize < maxWindowSize) {
        // do not look across Transactional Regions, the Register save mask is optimistic and does not allow renaming
        if (current->getOpCodeValue() == TR::InstOpCode::TBEGIN || current->getOpCodeValue() == TR::InstOpCode::TBEGINC || current->getOpCodeValue() == TR::InstOpCode::TEND || current->getOpCodeValue() == TR::InstOpCode::TABORT) {
            return false;
        }
        ...
        if (current->usesRegister(lgrTargetReg)) {
            if (current->defsRegister(lgrTargetReg)) {
                ...
                if (curOpCode != TR::InstOpCode::AHI && curOpCode != TR::InstOpCode::AGHI && curOpCode != TR::InstOpCode::SLL && curOpCode != TR::InstOpCode::SLA && curOpCode != TR::InstOpCode::SRA && curOpCode != TR::InstOpCode::SRLK) {
...
void TR_S390PostRAPeephole::markBlockThatModifiesRegister(TR::Instruction* cursor,
...
void TR_S390PostRAPeephole::perform()
{
    ...
    while (_cursor != NULL) {
        ...
        if (_cursor->getOpCodeValue() != TR::InstOpCode::FENCE && _cursor->getOpCodeValue() != TR::InstOpCode::ASSOCREGS && _cursor->getOpCodeValue() != TR::InstOpCode::DEPEND) {
        ...
        if (_cursor->getOpCode().setsCompareFlag() && _cursor->getOpCodeValue() != TR::InstOpCode::CHLR && _cursor->getOpCodeValue() != TR::InstOpCode::CLHLR) {
        ...
        case TR::InstOpCode::NILF:
            if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILF) {
                ...
                if ((na->getKind() == TR::Instruction::IsRIL) && (nb->getKind() == TR::Instruction::IsRIL)) {
                    ...
                    bool instructionsMatch = (cast_na->getTargetPtr() == cast_nb->getTargetPtr()) && (cast_na->getTargetSnippet() == cast_nb->getTargetSnippet()) && (cast_na->getTargetSymbol() == cast_nb->getTargetSymbol()) && (cast_na->getTargetLabel() == cast_nb->getTargetLabel()) && (cast_na->getMask() == cast_nb->getMask()) && (cast_na->getSymbolReference() == cast_nb->getSymbolReference());
                    ...
                    if (instructionsMatch) {
                        if (cast_na->matchesTargetRegister(cast_nb->getRegisterOperand(1)) && cast_nb->matchesTargetRegister(cast_na->getRegisterOperand(1))) {
                            ...
                            //
                            else if (((cast_na->getSourceImmediate() & cast_nb->getSourceImmediate()) == cast_na->getSourceImmediate()) && ((cast_nb->getSourceImmediate() & cast_na->getSourceImmediate()) != cast_nb->getSourceImmediate())) {
        ...
        case TR::InstOpCode::SLL: {
            ...
            if (_cursor->getNext()->getOpCodeValue() == _cursor->getOpCodeValue()) {
                ...
                if ((na->getKind() == TR::Instruction::IsRS) && (nb->getKind() == TR::Instruction::IsRS)) {
                    ...
                    bool instrMatch = (cast_na->getFirstRegister() == cast_nb->getFirstRegister()) && (!cast_na->hasMaskImmediate()) && (!cast_nb->hasMaskImmediate()) && (cast_na->getMemoryReference() == NULL) && (cast_nb->getMemoryReference() == NULL);

                    if (instrMatch) {
                        if (((cast_na->getSourceImmediate() + cast_nb->getSourceImmediate()) < 64) && performTransformation(comp(), "O^O S390 PEEPHOLE: merging SRL/SLL pair %p %p\n", _cursor, _cursor->getNext())) {
...
bool TR_S390PostRAPeephole::forwardBranchTarget()
{
    ...
    if (tmp->getOpCodeValue() == TR::InstOpCode::BRC) {
        ...
        if (firstBranch->getBranchCondition() == TR::InstOpCode::COND_BRC && performTransformation(comp(), "\nO^O S390 PEEPHOLE: forwarding branch target in %p\n", _cursor)) {
