...
void TR_S390PreRAPeephole::perform() {
...
bool TR_S390PreRAPeephole::attemptLoadStoreReduction(
    TR::InstOpCode::Mnemonic storeOpCode, uint16_t size) {
  if (_cursor->getNext()->getOpCodeValue() == storeOpCode) {
    TR::S390RXInstruction *loadInst =
        static_cast<TR::S390RXInstruction *>(_cursor);
    TR::S390RXInstruction *storeInst =
        static_cast<TR::S390RXInstruction *>(_cursor->getNext());

    // Cannot change to MVC if symbol references are unresolved, MVC doesn't
    // follow same format as load and store so patching won't work
    if (loadInst->getMemoryReference()->getSymbolReference() &&
            loadInst->getMemoryReference()
                ->getSymbolReference()
                ->isUnresolved() ||
        storeInst->getMemoryReference()->getSymbolReference() &&
            storeInst->getMemoryReference()
                ->getSymbolReference()
                ->isUnresolved()) {
      return false;
    }
    ...
    if (!loadInst->getMemoryReference()->getBaseRegister() &&
            !loadInst->getMemoryReference()->getIndexRegister() ||
        !storeInst->getMemoryReference()->getBaseRegister() &&
            !storeInst->getMemoryReference()->getIndexRegister()) {
      return false;
    }
    ...
    // Register must only be used in the load-store sequence, possibly used in
    // the memory reference of the load however, not in the store since the the
    // register would have to be loaded first.
    ncount_t uses = 2;
    uses += reg == loadInst->getMemoryReference()->getBaseRegister();

    // Bail out if register could have future uses, or if either instruction has
    // dependencies. Since 2 instructions are replaced with one before RA, we
    // can't handle things like merging dependencies.
    if (reg->getTotalUseCount() != uses ||
        loadInst->getDependencyConditions() ||
    ...
    if (performTransformation(
            comp(),
            "O^O S390 PEEPHOLE: Transforming load-store sequence at %p to MVC.",
            storeInst)) {
      TR::DebugCounter::incStaticDebugCounter(_cg->comp(),
                                              "z/peephole/load-store");
      ...
      TR::Instruction *newInst = generateSS1Instruction(
          _cg, TR::InstOpCode::MVC, comp()->getStartTree()->getNode(), size - 1,
          storeInst->getMemoryReference(), loadInst->getMemoryReference(),
          _cursor->getPrev());
...
TR::Instruction *realInstructionWithLabels(TR::Instruction *inst,
                                           bool forward) {
  while (inst && (inst->getKind() == TR::Instruction::IsPseudo ||
                  inst->getKind() == TR::Instruction::IsNotExtended)) {
    inst = forward ? inst->getNext() : inst->getPrev();
...
TR::Instruction *realInstructionWithLabelsAndRET(TR::Instruction *inst,
                                                 bool forward) {
  while (inst &&
         (inst->getKind() == TR::Instruction::IsPseudo ||
          inst->getKind() == TR::Instruction::IsNotExtended) &&
         !inst->isRet()) {
    inst = forward ? inst->getNext() : inst->getPrev();
  }
...
bool TR_S390PostRAPeephole::isBarrierToPeepHoleLookback(
...
bool TR_S390PostRAPeephole::AGIReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(), "O^O Random Codegen  - Disable AGIReduction on 0x%p.\n",
            _cursor))
      return false;
  }
  ...
  if (disableRenaming != NULL) return false;

  TR::Register *lgrTargetReg =
      ((TR::S390RRInstruction *)_cursor)->getRegisterOperand(1);
  TR::Register *lgrSourceReg =
      ((TR::S390RRInstruction *)_cursor)->getRegisterOperand(2);

  TR::RealRegister *gpr0 =
      _cg->machine()->getRealRegister(TR::RealRegister::GPR0);
  ...
  while ((current != NULL) && !current->matchesTargetRegister(lgrTargetReg) &&
         !isBarrierToPeepHoleLookback(current) && windowSize < MaxWindowSize) {
    // do not look across Transactional Regions, the Register save mask is
    // optimistic and does not allow renaming
    ...
    // does instruction load or store? otherwise just ignore and move to next
    // instruction
      ...
      while (mr) {
        if (mr && mr->getBaseRegister() == lgrTargetReg) {
          ...
          if (!reachedLabel && !reachedBranch && !sourceRegInvalid) {
            ...
            if (performTransformation(
                    comp(), "O^O S390 PEEPHOLE: AGI Reduction on %p.\n",
                    current)) {
              if (comp()->getOption(TR_TraceCG)) {
    ...
    // If Current instruction invalidates the source register, subsequent
    // instructions' memref cannot be renamed.
    sourceRegInvalid =
        sourceRegInvalid || current->matchesTargetRegister(lgrSourceReg);
  ...
  attemptLA &= _cursor->getOpCodeValue() == TR::InstOpCode::LGR &&
  ...
  if (attemptLA) {
    ...
    // if we reached the end of the loop and didn't find a conflict, switch the
    // instruction to LA
    if (windowSize == MaxLAWindowSize &&
      ...
      if (performTransformation(
        ...
        TR::MemoryReference *mr =
            generateS390MemoryReference(lgrSourceReg, 0, _cg);
        TR::Instruction *oldCursor = _cursor;
        _cursor = generateRXInstruction(_cg, TR::InstOpCode::LA,
                                        comp()->getStartTree()->getNode(),
                                        lgrTargetReg, mr, _cursor->getPrev());
...
///////////////////////////////////////////////////////////////////////////////
bool TR_S390PostRAPeephole::ICMReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(), "O^O Random Codegen  - Disable ICMReduction on 0x%p.\n",
            _cursor))
      return false;
  }
  ...
  if (next->getOpCodeValue() != TR::InstOpCode::LTR &&
      next->getOpCodeValue() != TR::InstOpCode::CHI)
    return false;
  ...
  if (next->getOpCodeValue() == TR::InstOpCode::LTR) {
    // We must check for sourceRegister on the LTR instruction, in case there
    // was a dead load
    //        L      GPRX, MEM       <-- this is a dead load, for whatever
    //        reason (e.g. GRA) LTR    GPRX, GPRY
    // it is wrong to transform the above sequence into
    //        ICM    GPRX, MEM
    if (next->getRegisterOperand(1) !=
        ((TR::S390RRInstruction *)next)->getRegisterOperand(2))
      return false;
  ...
  if (isICMOpportunity &&
    ...
    TR::MemoryReference *memcp =
        generateS390MemoryReference(*load->getMemoryReference(), 0, _cg);
    ...
    TR::S390RSInstruction *icm = new (_cg->trHeapMemory())
        TR::S390RSInstruction(TR::InstOpCode::ICM, load->getNode(),
                              load->getRegisterOperand(1), 0xF, memcp, prev,
                              _cg);
    _cursor = icm;

    // Check if the load has an implicit NULLCHK.  If so, we need to ensure a
    // GCmap is copied.
    if (load->throwsImplicitNullPointerException()) {
      ...
      if (debugObj)
        debugObj->addInstructionComment(
            icm, "Throws Implicit Null Pointer Exception");
    }
...
bool TR_S390PostRAPeephole::LAReduction() {
  ...
  if (s390Instr->getKind() == TR::Instruction::IsRX) {
    ...
    if (mr && mr->getOffset() == 0 && laBaseReg && laTargetReg &&
        laBaseReg == laTargetReg && laIndexReg == NULL &&
      ...
      if (performTransformation(
              comp(),
              "O^O S390 PEEPHOLE: Removing redundant LA (0x%p) with matching "
              "target and base registers (%s)\n",
              _cursor, comp()->getDebug()->getName(laTargetReg))) {
        if (comp()->getOption(TR_TraceCG)) printInstr(comp(), _cursor);
...
bool TR_S390PostRAPeephole::duplicateNILHReduction() {
      ...
      if (cast_na->isImm() == cast_nb->isImm()) {
        if (cast_na->isImm()) {
          if (cast_na->getSourceImmediate() == cast_nb->getSourceImmediate()) {
            if (cast_na->matchesTargetRegister(
                    cast_nb->getRegisterOperand(1)) &&
                cast_nb->matchesTargetRegister(
                    cast_na->getRegisterOperand(1))) {
              if (performTransformation(comp(),
                                        "O^O S390 PEEPHOLE: deleting duplicate "
                                        "NILH from pair %p %p*\n",
                                        _cursor, _cursor->getNext())) {
...
bool TR_S390PostRAPeephole::clearsHighBitOfAddressInReg(
    TR::Instruction *inst, TR::Register *targetReg) {
  if (inst->defsRegister(targetReg)) {
    if (inst->getOpCodeValue() == TR::InstOpCode::LA) {
      if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(),
                 "LA inst 0x%x clears high bit on targetReg 0x%x (%s)\n", inst,
                 targetReg, targetReg->getRegisterName(comp()));
      return true;
    }
    ...
    if (inst->getOpCodeValue() == TR::InstOpCode::LAY) {
      if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(),
                 "LAY inst 0x%x clears high bit on targetReg 0x%x (%s)\n", inst,
                 targetReg, targetReg->getRegisterName(comp()));
      return true;
    }
    ...
    if (inst->getOpCodeValue() == TR::InstOpCode::NILH) {
      ...
      if (NILH_RI->isImm() && NILH_RI->getSourceImmediate() == 0x7FFF) {
        if (comp()->getOption(TR_TraceCG))
          traceMsg(comp(),
                   "NILH inst 0x%x clears high bit on targetReg 0x%x (%s)\n",
...
bool TR_S390PostRAPeephole::unnecessaryNILHReduction() { return false; }

bool TR_S390PostRAPeephole::NILHReduction() {
  bool transformed = false;

  transformed |= duplicateNILHReduction();
  TR_ASSERT(_cursor->getOpCodeValue() == TR::InstOpCode::NILH,
            "Expecting duplicateNILHReduction to leave a NILH as current "
            "instruction\n");
...
static TR::Instruction *findActiveCCInst(TR::Instruction *curr,
                                         TR::InstOpCode::Mnemonic op,
                                         TR::Register *ccReg) {
  TR::Instruction *next = curr;
  while (next = next->getNext()) {
    if (next->getOpCodeValue() == op) return next;
    if (next->usesRegister(ccReg) || next->isLabel() ||
        next->getOpCode().setsCC() || next->isCall() ||
...
bool TR_S390PostRAPeephole::branchReduction() { return false; }

/**
 * Peek ahead in instruction stream to see if we find register being used
 * in a memref
 */
bool TR_S390PostRAPeephole::seekRegInFutureMemRef(int32_t maxWindowSize,
                                                  TR::Register *targetReg) {
  TR::Instruction *current = _cursor->getNext();
  int32_t windowSize = 0;

  while ((current != NULL) && !current->matchesTargetRegister(targetReg) &&
         !isBarrierToPeepHoleLookback(current) && windowSize < maxWindowSize) {
    // does instruction load or store? otherwise just ignore and move to next
    // instruction
    if (current->isLoad() || current->isStore()) {
      ...
      if (mr && (mr->getBaseRegister() == targetReg ||
                 mr->getIndexRegister() == targetReg)) {
        return true;
      }
    }
    current = current->getNext();
 ...
 * in SignalHandler.c
 */
bool TR_S390PostRAPeephole::removeMergedNullCHK() {
  if (TR::Compiler->target.isZOS()) {
    // CLT cannot do the job in zOS because in zOS it is legal to read low
    // memory address (like 0x000000, literally NULL), and CLT will read the low
    // memory address legally (in this case NULL) to compare it with the other
    // operand. The result will not make sense since we should trap for NULLCHK
    // first.
    return false;
  }
  ...
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(),
            "O^O Random Codegen  - Disable removeMergedNullCHK on 0x%p.\n",
            _cursor))
      return false;
  }
  ...
  static char *disableRemoveMergedNullCHK =
      feGetEnv("TR_DisableRemoveMergedNullCHK");
  ...
  while ((current != NULL) && !isBarrierToPeepHoleLookback(current) &&
    ...
    if (current->getOpCodeValue() == TR::InstOpCode::CLT) {
      ...
      if (!cgitSource || !cltSource || !cltSource2) return false;
      if (toRealRegister(cltSource2)->getRegisterNumber() ==
          toRealRegister(cgitSource)->getRegisterNumber()) {
        if (performTransformation(
 ...
 * to
 *      CHI GPRx, 0
 * This is an AGI reduction as LTR defines GPRx once again, while CHI simply
 ...
 *       LTR GPRx, GPRy
 */
bool TR_S390PostRAPeephole::LRReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(), "O^O Random Codegen  - Disable LRReduction on 0x%p.\n",
            _cursor))
      return false;
  }
  ...
  // The _defRegs in the instruction records virtual def reg till now that needs
  // to be reset to real reg.
  ...
  if (lgrTargetReg == lgrSourceReg &&
    ...
    if (performTransformation(
            comp(),
            "O^O S390 PEEPHOLE: Removing redundant LR/LGR/LDR/CPYA at %p\n",
            _cursor)) {
  ...
  if (lgrTargetReg == lgrSourceReg &&
    ...
    if (isAGI &&
        performTransformation(comp(),
                              "\nO^O S390 PEEPHOLE: Transforming load and test "
                              "to compare halfword immediate at %p\n",
                              _cursor)) {
      // replace LTGR with CGHI, LTR with CHI
      TR::Instruction *oldCursor = _cursor;
      _cursor = generateRIInstruction(
          _cg, lgrOpCode.is64bit() ? TR::InstOpCode::CGHI : TR::InstOpCode::CHI,
          comp()->getStartTree()->getNode(), lgrTargetReg, 0,
          _cursor->getPrev());
    ...
    TR::Instruction *prev = _cursor->getPrev();
    if ((prev->getOpCodeValue() == TR::InstOpCode::LR &&
         lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTR) ||
        (prev->getOpCodeValue() == TR::InstOpCode::LGR &&
         lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTGR)) {
      TR::Register *prevTargetReg =
          ((TR::S390RRInstruction *)prev)->getRegisterOperand(1);
      TR::Register *prevSourceReg =
          ((TR::S390RRInstruction *)prev)->getRegisterOperand(2);
      if ((lgrTargetReg == prevTargetReg || lgrTargetReg == prevSourceReg) &&
          performTransformation(
              comp(),
              "\nO^O S390 PEEPHOLE: Transforming load register into load and "
              "test register and removing current at %p\n",
              _cursor)) {
        TR::Instruction *newInst = generateRRInstruction(
            _cg,
            lgrOpCode.is64bit() ? TR::InstOpCode::LTGR : TR::InstOpCode::LTR,
            prev->getNode(), prevTargetReg, prevSourceReg, prev->getPrev());
    ...
    TR::Instruction *next = _cursor->getNext();
    // try to remove redundant LTR, LTGR when we can reuse the condition code of
    // an arithmetic logical operation, ie. Add/Subtract Logical this is also
    // done by isActiveLogicalCC, and the end of
    // generateS390CompareAndBranchOpsHelper when the virtual registers match but
    // those cannot handle the case when the virtual registers are not the same
    // but we do have the same restricted register which is why we are handling
    // it here when all the register assignments are done, and the redundant LR's
    // from the clobber evaluate of the add/sub logical are cleaned up as well.
    ...
    // SLR @01, @04
    // BRC (0x10) Label
    // checks that the prev instruction is an add/sub logical operation that
    // sets the same target register as the LTR/LTGR insn, and that we branch
    // immediately after
    if (prev->getOpCode().setsCC() && prev->getOpCode().setsCarryFlag() &&
        prev->getRegisterOperand(1) == lgrTargetReg &&
        next->getOpCodeValue() == TR::InstOpCode::BRC) {
      TR::InstOpCode::S390BranchCondition branchCond =
          ((TR::S390BranchInstruction *)next)->getBranchCondition();

      if ((branchCond == TR::InstOpCode::COND_BE ||
           branchCond == TR::InstOpCode::COND_BNE) &&
          performTransformation(comp(),
                                "\nO^O S390 PEEPHOLE: Removing redundant Load "
                                "and Test instruction at %p, because CC can be "
                                "reused from logical instruction %p\n",
                                _cursor, prev)) {
        _cg->deleteInst(_cursor);
        if (branchCond == TR::InstOpCode::COND_BE)
          ((TR::S390BranchInstruction *)next)
              ->setBranchCondition(TR::InstOpCode::COND_MASK10);
        else if (branchCond == TR::InstOpCode::COND_BNE)
          ((TR::S390BranchInstruction *)next)
              ->setBranchCondition(TR::InstOpCode::COND_MASK5);
        performed = true;
        return performed;
      }
    }
  ...
  while ((current != NULL) && !isBarrierToPeepHoleLookback(current) &&
         !(current->isBranchOp() &&
           current->getKind() == TR::Instruction::IsRIL &&
    ...
    if ((curOpCode.getOpCodeValue() == TR::InstOpCode::CHI ||
         curOpCode.getOpCodeValue() == TR::InstOpCode::CGHI) &&
        ((curOpCode.is32bit() && lgrOpCode.is32bit()) ||
         (curOpCode.is64bit() && lgrOpCode.is64bit()))) {
      TR::Register *curTargetReg =
          ((TR::S390RIInstruction *)current)->getRegisterOperand(1);
      ...
      if (curTargetReg == lgrTargetReg && (srcImm == 0) && !(setCC || useCC)) {
        ...
        if (performTransformation(
                comp(), "O^O S390 PEEPHOLE: Transforming LR/CHI to LTR at %p\n",
                _cursor)) {
          if (comp()->getOption(TR_TraceCG)) {
          ...
          TR::Instruction *oldCursor = _cursor;
          _cursor = generateRRInstruction(
              _cg,
              lgrOpCode.is64bit() ? TR::InstOpCode::LTGR : TR::InstOpCode::LTR,
              comp()->getStartTree()->getNode(), lgrTargetReg, lgrSourceReg,
              _cursor->getPrev());
    ...
    if (curOpCode.getOpCodeValue() == lgrOpCode.getOpCodeValue() &&
      ...
      TR::Register *curSourceReg =
          ((TR::S390RRInstruction *)current)->getRegisterOperand(2);
      TR::Register *curTargetReg =
          ((TR::S390RRInstruction *)current)->getRegisterOperand(1);
      ...
      if (((curSourceReg == lgrTargetReg && curTargetReg == lgrSourceReg) ||
        ...
        if ((!lgrSetCC || !(setCC || useCC))) {
          ...
          if (performTransformation(
                  comp(),
                  "O^O S390 PEEPHOLE: Duplicate LR/CPYA removal at %p\n",
                  current)) {
            if (comp()->getOption(TR_TraceCG)) {
              printInfo("\nDuplicate LR/CPYA:");
              printInstr(comp(), current);
    ...
    // If instruction overwrites either of the original source and target
    // registers, we cannot remove any duplicates, as register contents may have
    // changed.
 ...
 *    LLGC  Rs, Mem
 */
bool TR_S390PostRAPeephole::LLCReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(), "O^O Random Codegen  - Disable LLCReduction on 0x%p.\n",
            _cursor))
      return false;
  }
  ...
  if (curOpCode == TR::InstOpCode::LGFR || curOpCode == TR::InstOpCode::LLGTR) {
    TR::Register *llcTgtReg =
        ((TR::S390RRInstruction *)_cursor)->getRegisterOperand(1);

    TR::Register *curSrcReg =
        ((TR::S390RRInstruction *)current)->getRegisterOperand(2);
    TR::Register *curTgtReg =
        ((TR::S390RRInstruction *)current)->getRegisterOperand(1);
    ...
    if (llcTgtReg == curSrcReg && llcTgtReg == curTgtReg) {
      ...
      if (performTransformation(
              comp(), "O^O S390 PEEPHOLE: Transforming LLC/%s to LLGC at %p.\n",
              (curOpCode == TR::InstOpCode::LGFR) ? "LGFR" : "LLGTR",
              current)) {
        if (comp()->getOption(TR_TraceCG)) {
        ...
        TR::MemoryReference *memRef =
            ((TR::S390RXInstruction *)oldCursor)->getMemoryReference();
        memRef->resetMemRefUsedBefore();
        _cursor = generateRXInstruction(_cg, TR::InstOpCode::LLGC,
                                        comp()->getStartTree()->getNode(),
                                        llcTgtReg, memRef, _cursor->getPrev());
        _cg->replaceInst(oldCursor, _cursor);
        performed = true;
      }
    }
 ...
 *    LGFR  Rt, Rs
 */
bool TR_S390PostRAPeephole::LGFRReduction() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(), "O^O Random Codegen  - Disable LGFRReduction on 0x%p.\n",
            _cursor))
      return false;
  }
  ...
  if (curOpCode == TR::InstOpCode::LGFR) {
    ...
    if (curSourceReg == lgrTargetReg && curTargetReg == lgrTargetReg) {
      ...
      if (performTransformation(
              comp(), "O^O S390 PEEPHOLE: Redundant LGR at %p.\n", current)) {
        if (comp()->getOption(TR_TraceCG)) {
          printInfo("\n\tRedundent LGR/LGFR:");
          printInstr(comp(), current);
          char tmp[50];
 ...
 *    ...    ...
 */
bool TR_S390PostRAPeephole::ConditionalBranchReduction(
    TR::InstOpCode::Mnemonic branchOPReplacement) {
  bool disabled = comp()->getOption(TR_DisableZ13) ||
                  comp()->getOption(TR_DisableZ13LoadImmediateOnCond);
  ...
  TR::S390RIEInstruction *branchInst =
      static_cast<TR::S390RIEInstruction *>(_cursor);

  TR::Instruction *currInst = _cursor;
  TR::Instruction *nextInst = _cursor->getNext();

  TR::Instruction *label =
      branchInst->getBranchDestinationLabel()->getInstruction();

  // Check that the instructions within the fall-through block can be
  // conditionalized
  while (currInst = realInstructionWithLabelsAndRET(nextInst, true)) {
    ...
    if (currInst->getOpCodeValue() != TR::InstOpCode::LHI &&
        currInst->getOpCodeValue() != TR::InstOpCode::LGHI)
      return false;
  ...
  TR::InstOpCode::S390BranchCondition cond = getBranchConditionForMask(
      0xF -
      (getMaskForBranchCondition(branchInst->getBranchCondition()) & 0xF));

  if (performTransformation(comp(),
                            "O^O S390 PEEPHOLE: Conditionalizing fall-through "
                            "block following %p.\n",
                            currInst)) {
    // Conditionalize the fall-though block
    while (currInst = realInstructionWithLabelsAndRET(nextInst, true)) {
      if (currInst == label) break;

      // Because of the previous checks, LHI or LGHI instruction is guaranteed
      // to be here
      TR::S390RIInstruction *RIInst =
          static_cast<TR::S390RIInstruction *>(currInst);

      // Conditionalize from "Load Immediate" to "Load Immediate on Condition"
      _cg->replaceInst(
          RIInst,
          _cursor = generateRIEInstruction(
              _cg,
              RIInst->getOpCode().getOpCodeValue() == TR::InstOpCode::LHI
                  ? TR::InstOpCode::LOCHI
                  : TR::InstOpCode::LOCGHI,
              RIInst->getNode(), RIInst->getRegisterOperand(1),
              RIInst->getSourceImmediate(), cond, RIInst->getPrev()));

      currInst = _cursor;
      nextInst = _cursor->getNext();
    }

    // Conditionalize the branch instruction from "Compare and Branch" to
    // "Compare"
    _cg->replaceInst(
        branchInst,
        generateRRInstruction(_cg, branchOPReplacement, branchInst->getNode(),
                              branchInst->getRegisterOperand(1),
                              branchInst->getRegisterOperand(2),
                              branchInst->getPrev()));
 ...
 */
bool TR_S390PostRAPeephole::CompareAndBranchReduction() {
  if (!TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) return false;
  ...
  if (_cursor->getOpCodeValue() == TR::InstOpCode::CLR &&
    ...
    if (labelSymbol->getSnippet()) {
      ...
      for (auto branchTakenInstIndex = 0;
           branchTakenInstIndex < 5 && NULL != branchInstruction;
    ...
    for (auto fallThroughInstIndex = 0;
         fallThroughInstIndex < 5 && NULL != fallThroughInstruction;
    ...
    if (fallThroughPerformReduction && branchTakenPerformReduction &&
        performTransformation(
            comp(),
            "O^O S390 PEEPHOLE: Transforming CLR [%p] and BRC [%p] to CLRJ\n",
      ...
      TR::Instruction *clrjInstruction = generateRIEInstruction(
          _cg, TR::InstOpCode::CLRJ, clrInstruction->getNode(),
          clrInstruction->getRegisterOperand(1),
          clrInstruction->getRegisterOperand(2), labelSymbol,
          static_cast<TR::S390BranchInstruction *>(brcInstruction)
              ->getBranchCondition(),
 ...
 *    LZOpCode  Rx, Mem
 */
bool TR_S390PostRAPeephole::LoadAndMaskReduction(
    TR::InstOpCode::Mnemonic LZOpCode) {
  bool disabled = comp()->getOption(TR_DisableZ13) ||
                  comp()->getOption(TR_DisableZ13LoadAndMask);
  ...
  if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILL) {
    TR::S390RXInstruction *loadInst =
        static_cast<TR::S390RXInstruction *>(_cursor);
    TR::S390RIInstruction *nillInst =
        static_cast<TR::S390RIInstruction *>(_cursor->getNext());
    ...
    if (loadTargetReg != nillTargetReg) return false;

    if (performTransformation(
            comp(),
            "O^O S390 PEEPHOLE: Transforming load-and-mask sequence at %p.\n",
            nillInst)) {
      ...
      _cg->replaceInst(
          loadInst,
          _cursor = generateRXInstruction(
              _cg, LZOpCode, comp()->getStartTree()->getNode(), loadTargetReg,
              loadInst->getMemoryReference(), _cursor->getPrev()));
...
bool swapOperands(TR::Register *trueReg, TR::Register *compReg,
                  TR::Instruction *curr) {
  TR::InstOpCode::S390BranchCondition branchCond;
  uint8_t mask;
  switch (curr->getKind()) {
    case TR::Instruction::IsRR:
      ...
      ((TR::S390RRInstruction *)curr)
          ->setBranchCondition(getReverseBranchCondition(branchCond));
...
void insertLoad(TR::Compilation *comp, TR::CodeGenerator *cg,
                TR::Instruction *i, TR::Register *r) {
  switch (r->getKind()) {
    case TR_FPR:
      new (comp->trHeapMemory())
          TR::S390RRInstruction(TR::InstOpCode::LDR, i->getNode(), r, r, i, cg);
      break;
    default:
      new (comp->trHeapMemory())
          TR::S390RRInstruction(TR::InstOpCode::LR, i->getNode(), r, r, i, cg);
      break;
  }
}
 ...
 */
bool TR_S390PostRAPeephole::trueCompEliminationForCompare() {
  // z10 specific
  ...
  if (branchInst && prev && prev->usesRegister(compReg) &&
      !prev->usesRegister(trueReg)) {
    if (comp()->getOption(TR_TraceCG)) {
      printInfo("\n");
    }
    if (performTransformation(comp(),
                              "O^O S390 PEEPHOLE: true complement elimination "
                              "for compare case 1 at %p.\n",
                              curr)) {
      swapOperands(trueReg, compReg, curr);
      if (next && next->usesRegister(trueReg))
        insertLoad(comp(), _cg, next->getPrev(), compReg);
      TR::InstOpCode::S390BranchCondition branchCond =
          ((TR::S390BranchInstruction *)branchInst)->getBranchCondition();
      ((TR::S390BranchInstruction *)branchInst)
          ->setBranchCondition(getReverseBranchCondition(branchCond));
      return true;
    } else
      return false;
 ...
 * in same grouping, causing pipeline flush + late load = perf hit.
 */
bool TR_S390PostRAPeephole::trueCompEliminationForCompareAndBranch() {
  // z10 specific
  if (!TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10) ||
      TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
    return false;
  ...
  switch (curr->getKind()) {
    case TR::Instruction::IsRIE:
      compReg = ((TR::S390RIEInstruction *)curr)->getRegisterOperand(2);
      trueReg = ((TR::S390RIEInstruction *)curr)->getRegisterOperand(1);
      btar = ((TR::S390RIEInstruction *)curr)
  ...
  if (backwardBranch) {
    if ((prev && btar) &&
      ...
      if (performTransformation(
              comp(),
              "O^O S390 PEEPHOLE: true complement elimination for compare and "
              "branch case 1 at %p.\n",
              curr)) {
        swapOperands(trueReg, compReg, curr);
        if (next && next->usesRegister(trueReg))
    ...
    if ((prev && next) &&
      ...
      if (!isWCodeCmpEqSwap &&
          performTransformation(
              ...
              curr)) {
        swapOperands(trueReg, compReg, curr);
        if (btar && btar->usesRegister(trueReg))
          insertLoad(comp(), _cg, btar->getPrev(), compReg);
        return true;
      } else
        return false;
    }
  ...
  if (prev && prev->usesRegister(compReg) && !prev->usesRegister(trueReg)) {
    ...
    if (!isWCodeCmpEqSwap &&
        performTransformation(comp(),
                              "O^O S390 PEEPHOLE: true complement elimination "
                              "for compare and branch case 3 at %p.\n",
                              curr)) {
      swapOperands(trueReg, compReg, curr);
      if (btar && btar->usesRegister(trueReg))
        insertLoad(comp(), _cg, btar->getPrev(), compReg);
  ...
  if (prev && prev->usesRegister(compReg)) {
    ...
    if (performTransformation(comp(),
                              "O^O S390 PEEPHOLE: true complement elimination "
                              "for compare and branch case 4 at %p.\n",
                              curr)) {
      insertLoad(comp(), _cg, prev, trueReg);
      loadInserted = true;
    }
  ...
  if (btar && btar->usesRegister(compReg)) {
    ...
    if (performTransformation(comp(),
                              "O^O S390 PEEPHOLE: true complement elimination "
                              "for compare and branch case 5 at %p.\n",
                              curr)) {
  ...
  if (next && next->usesRegister(compReg)) {
    ...
    if (performTransformation(comp(),
                              "O^O S390 PEEPHOLE: true complement elimination "
                              "for compare and branch case 6 at %p.\n",
                              curr)) {
      insertLoad(comp(), _cg, next->getPrev(), trueReg);
      loadInserted = true;
    }
...
bool TR_S390PostRAPeephole::trueCompEliminationForLoadComp() {
  if (!TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10) ||
      TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
    return false;
  }
  ...
  if (prev && prev->defsRegister(srcReg)) {
    ...
    // used in the next instruction, if so, inject a load after the current
    // insruction
  ...
  if (prev2 && prev2->defsRegister(srcReg)) {
    // src register is defined 2 instructions ago, insert a load before the
    // current instruction if the true value is used before or after
    if ((next && next->usesRegister(srcReg)) ||
        (prev && prev->usesRegister(srcReg))) {
      if (comp()->getOption(TR_TraceCG)) {
        printInfo("\n");
      }
      if (performTransformation(comp(),
                                "O^O S390 PEEPHOLE: true complement "
                                "elimination for load complement at %p.\n",
                                curr)) {
  ...
  if (prev3 && prev3->defsRegister(srcReg)) {
    // src registers is defined 3 instructions ago, insert a load before the
    // current instruction if the true value is use before
...
/**
 * Catch pattern where a DAA instruction is followed by 2 BRCL NOPs; The second
 * one to an outlined label, e.g.
 *
 * CVBG    GPR6, 144(GPR5)
 ...
 *
 * The second BRCL is generate to create the proper RA snapshot in case we
 * branch to the OOL path. Since Peephole optimizations come after RA, we can
 * safely remove this instruction.
 *
 * Note that for 4-byte instructions such as CVB there is an extra 2-byte NOP
 * used as padding following the 4-byte instruction.
 */
bool TR_S390PostRAPeephole::DAARemoveOutlinedLabelNop(bool hasPadding) {
  ...
  if (_cursor->throwsImplicitException()) {
    ...
    if (hasPadding) {
      if (nextInst->getOpCodeValue() == TR::InstOpCode::NOP)
        ...
        TR_ASSERT(0,
                  "S390 PEEPHOLE: DAA instruction padding was not found "
    ...
    if ((nextInst->getOpCodeValue() == TR::InstOpCode::BRC ||
         nextInst->getOpCodeValue() == TR::InstOpCode::BRCL) &&
        (nextInst2->getOpCodeValue() == TR::InstOpCode::BRC ||
         nextInst2->getOpCodeValue() == TR::InstOpCode::BRCL)) {
      TR::S390RegInstruction *OOLbranchInst =
          static_cast<TR::S390RegInstruction *>(nextInst2);

      if (OOLbranchInst->getBranchCondition() == TR::InstOpCode::COND_MASK0)
        if (performTransformation(comp(),
                                  "O^O S390 PEEPHOLE: Eliminating redundant "
                                  "DAA NOP BRC/BRCL to OOL path %p.\n",
                                  OOLbranchInst)) {
...
/**
 * Catch pattern where a DAA instruction memory reference generates spill code
 * for long displacement, e.g.
 *
 * STG     GPR1, 148(GPR5)
 * LGHI    GPR1, 74048
 ...
 * BRCL    NOP(0x0),         Label L0058, labelTargetAddr=0x0000000029B8C896
 *
 * Because the SignalHandler looks for the RestoreGPR7 Snippet address by a
 * hardcoded offset it will instead encounter the memory reference spill code in
 * the place where the RestoreGPR7 Snippet BRCL NOP should have been. This will
 * result in a wild branch.
 *
 * Since this is such a rare occurrence, the workaround is to remove the DAA
 * intrinsic instructions and replace them by an unconditional branch to the OOL
 * path.
 *
 * Note that for 4-byte instructions such as CVB there is an extra 2-byte NOP
 * used as padding following the 4-byte instruction.
 */
bool TR_S390PostRAPeephole::DAAHandleMemoryReferenceSpill(bool hasPadding) {
  // Make sure the current instruction is a DAA intrinsic instruction
  if (_cursor->throwsImplicitException()) {
    TR::Instruction *currInst = _cursor;
    TR::Instruction *nextInst = _cursor->getNext();
    ...
    if (nextInst->getOpCodeValue() != TR::InstOpCode::BRC &&
      ...
      if (performTransformation(
              comp(),
              "O^O S390 PEEPHOLE: Eliminating DAA intrinsic due to a memory "
              "reference spill near instruction %p.\n",
              currInst)) {
        currInst = _cursor;
        nextInst = _cursor->getNext();
        ...
        while (currInst != NULL) {
          if (currInst->getOpCodeValue() == TR::InstOpCode::BRC ||
              currInst->getOpCodeValue() == TR::InstOpCode::BRCL) {
            // Convert the instruction to a Branch Instruction to access the
            // Label Symbol
            OOLbranchInst = static_cast<TR::S390BranchInstruction *>(currInst);

            // We have found the correct BRC/BRCL
            if (OOLbranchInst->getLabelSymbol()
        ...
        TR_ASSERT(
            OOLbranchInst->getBranchCondition() == TR::InstOpCode::COND_MASK0,
            "S390 PEEPHOLE: DAA BRC/BRCL to the OOL path is NOT a NOP branch "
            "%p.\n",
            OOLbranchInst);
 ...
 *         false otherwise
 */
bool TR_S390PostRAPeephole::revertTo32BitShift() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
            comp(),
            "O^O Random Codegen  - Disable revertTo32BitShift on 0x%p.\n",
            _cursor))
      return false;
  }
  ...
  if ((oldOpCode == TR::InstOpCode::SLLG ||
       oldOpCode == TR::InstOpCode::SLAG) &&
      instr->getNode()->getOpCodeValue() != TR::ishl) {
    return false;
  }

  TR::Register *sourceReg =
      (instr->getSecondRegister())
          ? (instr->getSecondRegister()->getRealRegister())
          : NULL;
  TR::Register *targetReg =
      (instr->getRegisterOperand(1))
          ? (instr->getRegisterOperand(1)->getRealRegister())
          : NULL;
  ...
  if (performTransformation(comp(),
                            "O^O S390 PEEPHOLE: Reverting int shift at %p from "
                            "SLLG/SLAG/S[LR][LA]K to SLL/SLA/SRL/SRA.\n",
                            instr)) {
    reverted = true;
    ...
    if (instr->getSourceImmediate()) {
      newInstr = new (_cg->trHeapMemory()) TR::S390RSInstruction(
          newOpCode, instr->getNode(), instr->getRegisterOperand(1),
          instr->getSourceImmediate(), instr->getPrev(), _cg);
    } else if (instr->getMemoryReference()) {
      TR::MemoryReference *memRef = instr->getMemoryReference();
      memRef->resetMemRefUsedBefore();
      newInstr = new (_cg->trHeapMemory()) TR::S390RSInstruction(
          newOpCode, instr->getNode(), instr->getRegisterOperand(1), memRef,
          instr->getPrev(), _cg);
    } else {
    ...
    if (comp()->getOption(TR_TraceCG))
      traceMsg(comp(),
               "\nTransforming int shift from SLLG/SLAG/S[LR][LA]K (%p) to "
               "SLL/SLA/SRL/SRA %p.\n",
               instr, newInstr);
  }
  return reverted;
 ...
 * runtime performance.
 */
bool TR_S390PostRAPeephole::inlineEXtargetHelper(TR::Instruction *inst,
                                                 TR::Instruction *instrB4EX) {
  if (performTransformation(
          comp(),
          "O^O S390 PEEPHOLE: Converting LARL;EX into EXRL instr=[%p]\n",
          _cursor)) {
    // fetch the dispatched SS instr
    TR::S390ConstantDataSnippet *cnstDataSnip =
        _cursor->getMemoryReference()->getConstantDataSnippet();
    TR_ASSERT(
        (cnstDataSnip->getKind() == TR::Snippet::IsConstantInstruction),
        "The constant data snippet kind should be ConstantInstruction!\n");
    TR::Instruction *cnstDataInstr =
        ((TR::S390ConstantInstructionSnippet *)cnstDataSnip)->getInstruction();

    // EX Dispatch would have created a load of the literal pool address, a
    // register which is then killed immediately after its use in the EX
    // instruction. Since we are transforming the EX to an EXRL, this load is no
    // longer needed. We search backwards for this load of the literal pool and
    // remove it.
    ...
    while (iterator && count < 3) {
      ...
      if (iterator->getOpCodeValue() == TR::InstOpCode::LARL) {
        TR::S390RILInstruction *LARLInst =
            reinterpret_cast<TR::S390RILInstruction *>(iterator);

        // Make sure the register we load the literal pool address in is the
        // same register referenced by the EX instruction
        if (LARLInst->isLiteralPoolAddress() &&
            LARLInst->getRegisterOperand(1) ==
                _cursor->getMemoryReference()->getBaseRegister()) {
        ...
        TR::Register *baseReg =
            _cursor->getMemoryReference()->getBaseRegister();
        if (iterator && iterator->usesRegister(baseReg)) break;
      }
      ++count;
    }
    ...
    TR::Instruction *labelInstr = generateS390LabelInstruction(
        _cg, TR::InstOpCode::LABEL, _cursor->getNode(), ssInstrLabel, inst);
    ...
    // generate EXRL
    TR::S390RILInstruction *newEXRLInst = new (_cg->trHeapMemory())
        TR::S390RILInstruction(TR::InstOpCode::EXRL, _cursor->getNode(),
                               _cursor->getRegisterOperand(1), ssInstrLabel,
                               _cursor->getPrev(), _cg);
...
bool TR_S390PostRAPeephole::inlineEXtarget() {
  ...
  while (inst && count < maxCount) {
    ...
    if (inst->isLabel()) {
      ...
      if (inst && inst->getOpCode().getOpCodeValue() == TR::InstOpCode::BRC &&
          ((TR::S390BranchInstruction *)inst)->getBranchCondition() ==
  ...
  if (!comp()->getOption(TR_DisableForcedEXInlining) &&
      performTransformation(comp(),
                            "O^O S390 PEEPHOLE: Inserting unconditional branch "
                            "to hide EX target instr=[%p]\n",
                            _cursor)) {
    // Find previous instruction, skipping LARL so we can find it to delete it
    // later.
    TR::Instruction *prev =
        realInstructionWithLabels(_cursor->getPrev(), false);
    ...
    TR::Instruction *branchInstr = generateS390BranchInstruction(
        _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, _cursor->getNode(),
        targetLabel, prev);
    TR::Instruction *labelInstr = generateS390LabelInstruction(
        _cg, TR::InstOpCode::LABEL, _cursor->getNode(), targetLabel,
        branchInstr);
 ...
 * AHIK    GPR6,GPR0, -1
 */
bool TR_S390PostRAPeephole::attemptZ7distinctOperants() {
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(comp(),
                              "O^O Random Codegen  - Disable "
                              "attemptZ7distinctOperants on 0x%p.\n",
                              _cursor))
      return false;
  }
  ...
  if (instr->getOpCodeValue() != TR::InstOpCode::LR &&
      instr->getOpCodeValue() != TR::InstOpCode::LGR) {
    return false;
  }
  ...
  while ((current != NULL) && !current->isLabel() && !current->isCall() &&
    ...
    if (current->usesRegister(lgrTargetReg)) {
      if (current->defsRegister(lgrTargetReg)) {
        ...
        if (curOpCode != TR::InstOpCode::AHI &&
            curOpCode != TR::InstOpCode::AGHI &&
            curOpCode != TR::InstOpCode::SLL &&
            curOpCode != TR::InstOpCode::SLA &&
            curOpCode != TR::InstOpCode::SRA &&
            curOpCode != TR::InstOpCode::SRLK) {
        ...
        if ((current->getOpCode().is32bit() &&
             instr->getOpCodeValue() == TR::InstOpCode::LGR) ||
            (current->getOpCode().is64bit() &&
             instr->getOpCodeValue() == TR::InstOpCode::LR)) {
          // Make sure we abort if the register copy and the subsequent
          // operation
        ...
        switch (curOpCode) {
          case TR::InstOpCode::AR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::ARK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::AGR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::AGRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::ALR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::ALRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::ALGR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::ALGRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::AHI: {
            int16_t imm =
                ((TR::S390RIInstruction *)current)->getSourceImmediate();
            newInstr = generateRIEInstruction(_cg, TR::InstOpCode::AHIK, node,
                                              lgrTargetReg, lgrSourceReg, imm,
                                              prevInstr);
            break;
          }
          case TR::InstOpCode::AGHI: {
            int16_t imm =
                ((TR::S390RIInstruction *)current)->getSourceImmediate();
            newInstr = generateRIEInstruction(_cg, TR::InstOpCode::AGHIK, node,
                                              lgrTargetReg, lgrSourceReg, imm,
                                              prevInstr);
            break;
          }
          case TR::InstOpCode::NR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::NRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::NGR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::NGRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::XR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::XRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::XGR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::XGRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::OR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::ORK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::OGR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::OGRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::SLA: {
            int16_t imm =
                ((TR::S390RSInstruction *)current)->getSourceImmediate();
            TR::MemoryReference *mf =
                ((TR::S390RSInstruction *)current)->getMemoryReference();
            if (mf != NULL) {
              mf->resetMemRefUsedBefore();
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SLAK, node,
                                               lgrTargetReg, lgrSourceReg, mf,
                                               prevInstr);
            } else
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SLAK, node,
                                               lgrTargetReg, lgrSourceReg, imm,
          ...
          case TR::InstOpCode::SLL: {
            ...
            if (mf != NULL) {
              mf->resetMemRefUsedBefore();
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SLLK, node,
                                               lgrTargetReg, lgrSourceReg, mf,
                                               prevInstr);
            } else
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SLLK, node,
          ...
          case TR::InstOpCode::SRA: {
            ...
            if (mf != NULL) {
              mf->resetMemRefUsedBefore();
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SRAK, node,
                                               lgrTargetReg, lgrSourceReg, mf,
                                               prevInstr);
            } else
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SRAK, node,
          ...
          case TR::InstOpCode::SRL: {
            ...
            if (mf != NULL) {
              mf->resetMemRefUsedBefore();
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SRLK, node,
                                               lgrTargetReg, lgrSourceReg, mf,
                                               prevInstr);
            } else
              newInstr = generateRSInstruction(_cg, TR::InstOpCode::SRLK, node,
          ...
          case TR::InstOpCode::SR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::SRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::SGR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::SGRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::SLR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::SLRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
          case TR::InstOpCode::SLGR: {
            newInstr = generateRRRInstruction(_cg, TR::InstOpCode::SLGRK, node,
                                              lgrTargetReg, lgrSourceReg,
                                              srcReg, prevInstr);
            break;
          }
...
void TR_S390PostRAPeephole::markBlockThatModifiesRegister(
    TR::Instruction *cursor, TR::Register *targetReg) {
      ...
      if (cursor->getOpCodeValue() == TR::InstOpCode::getLoadMultipleOpCode()) {
        ...
        if (numRegs > 0) {
          for (uint8_t i = highReg->getRegisterNumber() + 1; i++;
               i <= numRegs) {
            _cg->getS390Linkage()->getRealRegister(REGNUM(i))->setModified(
                true);
          }
        }
      }
...
void TR_S390PostRAPeephole::reloadLiteralPoolRegisterForCatchBlock() {
  // When dynamic lit pool reg is disabled, we lock R6 as dedicated lit pool
  // reg. This causes a failure when we come back to a catch block because the
  // register context will not be preserved. Hence, we can not assume that R6
  // will still contain the lit pool register and hence need to reload it.

  bool isZ10 = TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10);

  // we only need to reload literal pool for Java on older z architecture on zos
  // when on demand literal pool is off
  if (TR::Compiler->target.isZOS() && !isZ10 &&
      !_cg->isLiteralPoolOnDemandOn()) {
    ...
    if ((_cg->getLinkage())->setupLiteralPoolRegister(firstSnippet) > 0) {
      // the imm. operand will be patched when the actual address of the literal
      // pool is known at binary encoding phase
      TR::S390RILInstruction *inst =
          (TR::S390RILInstruction *)generateRILInstruction(
              _cg, TR::InstOpCode::LARL, _cursor->getNode(),
              _cg->getLitPoolRealRegister(), reinterpret_cast<void *>(0xBABE),
              _cursor);
...
bool TR_S390PostRAPeephole::DeadStoreToSpillReduction() { return false; }

bool TR_S390PostRAPeephole::tryMoveImmediate() { return false; }

/** \details
 *     This transformation may not always be possible because the LHI
 * instruction does not modify the condition code while the XR instruction does.
 * We must be pessimistic in our algorithm and carry out the transformation if
 * and only if there exists an instruction B that sets the condition code
 * between the LHI instruction A and some instruction C that reads the condition
 * code.
 *
 *     That is, we are trying to find instruction that comes after the LHI in
 * the execution order that will clobber the condition code before any
 * instruction that consumes a condition code.
 */
bool TR_S390PostRAPeephole::ReduceLHIToXR() {
  TR::S390RIInstruction *lhiInstruction =
      static_cast<TR::S390RIInstruction *>(_cursor);
  ...
  if (lhiInstruction->getSourceImmediate() == 0) {
    ...
    while (nextInstruction != NULL && !nextInstruction->getOpCode().readsCC()) {
      if (nextInstruction->getOpCode().setsCC() ||
          nextInstruction->getNode()->getOpCodeValue() == TR::BBEnd) {
        TR::DebugCounter::incStaticDebugCounter(_cg->comp(),
                                                "z/peephole/LHI/XR");

        TR::Instruction *xrInstruction = generateRRInstruction(
            _cg, TR::InstOpCode::XR, lhiInstruction->getNode(),
            lhiInstruction->getRegisterOperand(1),
            lhiInstruction->getRegisterOperand(1));
...
void TR_S390PostRAPeephole::perform() {
  ...
  while (_cursor != NULL) {
    if (_cursor->getNode() != NULL &&
        _cursor->getNode()->getOpCodeValue() == TR::BBStart) {
    ...
    if (_cursor->getOpCodeValue() != TR::InstOpCode::FENCE &&
      ...
      TR::RegisterDependencyConditions *deps =
          _cursor->getDependencyConditions();
    ...
    // this code is used to handle all compare instruction which sets the
    // compare flag we can eventually extend this to include other instruction
    // which sets the condition code and and uses a complemented register
    if (_cursor->getOpCode().setsCompareFlag() &&
      ...
      if (comp()->getOption(TR_TraceCG)) printInst();
    }

    if (_cursor->isBranchOp()) forwardBranchTarget();
    ...
    switch (_cursor->getOpCodeValue()) {
      ...
      case TR::InstOpCode::ST: {
        if (DeadStoreToSpillReduction()) break;
        if (tryMoveImmediate()) break;
        if (comp()->getOption(TR_TraceCG)) printInst();
        break;
      }
      ...
      case TR::InstOpCode::LHI: {
        // This optimization is disabled by default because there exist cases in
        // which we cannot determine whether this transformation is functionally
        // valid or not. The issue resides in the various runtime patching
        // sequences using the LHI instruction as a runtime patch point for an
        // offset. One concrete example can be found in the virtual dispatch
        // sequence for unresolved calls on 31-bit platforms where an LHI
        // instruction is used and is patched at runtime.
        //
        // TODO (Issue #255): To enable this optimization we need to implement
        // an API which marks instructions that will be patched at runtime and
        // prevent ourselves from modifying such instructions in any way.
        //
        // ReduceLHIToXR();
      } break;
      ...
      case TR::InstOpCode::EX: {
        ...
        if (_cursor->isOutOfLineEX() && !comp()->getCurrentBlock()->isCold() &&
            !(bool)disableEXRLDispatch &&
        ...
        if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILF) {
          ...
          if ((na->getKind() == TR::Instruction::IsRIL) &&
            ...
            bool instructionsMatch =
                ...
                (cast_na->getSymbolReference() ==
                 cast_nb->getSymbolReference());
            ...
            if (instructionsMatch) {
              if (cast_na->matchesTargetRegister(
                  ...
                  cast_nb->matchesTargetRegister(
                      cast_na->getRegisterOperand(1))) {
                if (cast_na->getSourceImmediate() ==
                    cast_nb->getSourceImmediate()) {
                  if (performTransformation(comp(),
                                            "O^O S390 PEEPHOLE: deleting "
                                            "duplicate NILF from pair %p %p*\n",
                                            _cursor, _cursor->getNext())) {
                    _cg->deleteInst(_cursor->getNext());
                  }
                }
                ...
                //
                else if (((cast_na->getSourceImmediate() &
                           cast_nb->getSourceImmediate()) ==
                          cast_na->getSourceImmediate()) &&
                         ((cast_nb->getSourceImmediate() &
                           cast_na->getSourceImmediate()) !=
                          cast_nb->getSourceImmediate())) {
                  if (performTransformation(
                          comp(),
                          "O^O S390 PEEPHOLE: deleting unnecessary NILF from "
                          "pair %p %p*\n",
                          _cursor, _cursor->getNext())) {
                    _cg->deleteInst(_cursor->getNext());
                  }
      ...
      case TR::InstOpCode::SLL: {
        ...
        if (_cursor->getNext()->getOpCodeValue() == _cursor->getOpCodeValue()) {
          TR::Instruction *na = _cursor;
          TR::Instruction *nb = _cursor->getNext();
          ...
          if ((na->getKind() == TR::Instruction::IsRS) &&
            ...
            bool instrMatch =
                ...
                (!cast_na->hasMaskImmediate()) &&
                (!cast_nb->hasMaskImmediate()) &&
                (cast_na->getMemoryReference() == NULL) &&
                (cast_nb->getMemoryReference() == NULL);

            if (instrMatch) {
              if (((cast_na->getSourceImmediate() +
                    cast_nb->getSourceImmediate()) < 64) &&
                  performTransformation(
                      comp(), "O^O S390 PEEPHOLE: merging SRL/SLL pair %p %p\n",
                      _cursor, _cursor->getNext())) {
                // Delete the second instruction
                uint32_t deletedImmediate = cast_nb->getSourceImmediate();
                _cg->deleteInst(_cursor->getNext());
                cast_na->setSourceImmediate(cast_na->getSourceImmediate() +
                                            deletedImmediate);
              }
            }
          }
    ...
    if (moveInstr == true) _cursor = _cursor->getNext();
  }
...
bool TR_S390PostRAPeephole::forwardBranchTarget() {
  ...
  switch (_cursor->getOpCodeValue()) {
    case TR::InstOpCode::BRC:
      targetLabelSym = ((TR::S390BranchInstruction *)_cursor)->getLabelSymbol();
      break;
    case TR::InstOpCode::CRJ:
    case TR::InstOpCode::CGRJ:
    ...
    case TR::InstOpCode::CLGIJ:
      targetLabelSym =
          toS390RIEInstruction(_cursor)->getBranchDestinationLabel();
      break;
    default:
      return false;
  }
  if (!targetLabelSym) return false;

  auto targetLabelInsn = targetLabelSym->getInstruction();
  if (!targetLabelInsn) return false;
  auto tmp = targetLabelInsn;
  while (tmp->isLabel() || tmp->getOpCodeValue() ==
                               TR::InstOpCode::FENCE)  // skip labels and fences
    tmp = tmp->getNext();
  if (tmp->getOpCodeValue() == TR::InstOpCode::BRC) {
    ...
    if (firstBranch->getBranchCondition() == TR::InstOpCode::COND_BRC &&
      ...
      switch (_cursor->getOpCodeValue()) {
        case TR::InstOpCode::BRC:
          ((TR::S390BranchInstruction *)_cursor)
              ->setLabelSymbol(newTargetLabelSym);
          break;
        case TR::InstOpCode::CRJ:
        case TR::InstOpCode::CGRJ:
        case TR::InstOpCode::CIJ:
        ...
        case TR::InstOpCode::CLIJ:
        case TR::InstOpCode::CLGIJ:
          toS390RIEInstruction(_cursor)->setBranchDestinationLabel(
              newTargetLabelSym);
          break;
        default:
          return false;
