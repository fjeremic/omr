...
TR_S390Peephole::TR_S390Peephole(TR::Compilation *comp)
    : _fe(comp->fe()), _outFile(comp->getOutFile()),
      _cursor(comp->cg()->getFirstInstruction()), _cg(comp->cg()) {}
...
bool TR_S390PostRAPeephole::isBarrierToPeepHoleLookback(
  ...
  if (s390current->isLabel())
    return true;
  if (s390current->isCall())
    return true;
  if (s390current->isBranchOp())
    return true;
  if (s390current->getOpCodeValue() == TR::InstOpCode::DCB)
    return true;
...
bool TR_S390PostRAPeephole::ConditionalBranchReduction(
  ...
  if (performTransformation(comp(),
    ...
    // "Compare"
    _cg->replaceInst(branchInst,
                     generateRRInstruction(_cg, branchOPReplacement,
                                           branchInst->getNode(),
...
bool TR_S390PostRAPeephole::LoadAndMaskReduction(
  ...
  if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILL) {
    ...
    if (performTransformation(
      ...
      _cg->replaceInst(loadInst,
                       _cursor = generateRXInstruction(
                           _cg, LZOpCode, comp()->getStartTree()->getNode(),
                           loadTargetReg, loadInst->getMemoryReference(),
                           _cursor->getPrev()));
  ...
  if (_cursor->throwsImplicitException()) {
    ...
    if (nextInst->getOpCodeValue() != TR::InstOpCode::BRC &&
      ...
      if (performTransformation(
        ...
        TR_ASSERT(OOLbranchInst->getBranchCondition() ==
                      TR::InstOpCode::COND_MASK0,
                  "S390 PEEPHOLE: DAA BRC/BRCL to the OOL path is NOT a NOP "
                  "branch %p.\n",
                  OOLbranchInst);
      ...
      if (current->defsRegister(lgrTargetReg)) {
        ...
        case TR::InstOpCode::AR: {
          newInstr = generateRRRInstruction(_cg, TR::InstOpCode::ARK, node,
                                            lgrTargetReg, lgrSourceReg, srcReg,
                                            prevInstr);
          break;
        }
...
void TR_S390PostRAPeephole::perform() {
  ...
  while (_cursor != NULL) {
    ...
    case TR::InstOpCode::LHI: {
      ...
      //
      // TODO (Issue #255): To enable this optimization we need to implement an
      // API which marks instructions that will be patched at runtime and
          ...
          if (instructionsMatch) {
            if (cast_na->matchesTargetRegister(
              ...
              //
                ...
                if (performTransformation(comp(),
                                          "O^O S390 PEEPHOLE: deleting "
                                          "unnecessary NILF from pair %p %p*\n",
                                          _cursor, _cursor->getNext())) {
                  _cg->deleteInst(_cursor->getNext());
                }
              }
...
bool TR_S390PostRAPeephole::forwardBranchTarget() {
  ...
  case TR::InstOpCode::CLGIJ:
    targetLabelSym = toS390RIEInstruction(_cursor)->getBranchDestinationLabel();
    break;
  default:
    return false;
  }
