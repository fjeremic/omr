...
bool
TR_S390PreRAPeephole::attemptLoadStoreReduction(
  ...
  uint16_t size)
{
  if (_cursor->getNext()->getOpCodeValue() == storeOpCode) {
    ...
    if (performTransformation(
      ...
      TR::Instruction* newInst =
        generateSS1Instruction(_cg,
                               TR::InstOpCode::MVC,
...
TR::Instruction*
realInstruction(TR::Instruction* inst, bool forward)
{
...
TR::Instruction*
realInstructionWithLabels(TR::Instruction* inst, bool forward)
{
  while (inst && (inst->getKind() == TR::Instruction::IsPseudo ||
                  inst->getKind() == TR::Instruction::IsNotExtended)) {
    inst = forward ? inst->getNext() : inst->getPrev();
...
TR::Instruction*
realInstructionWithLabelsAndRET(TR::Instruction* inst, bool forward)
{
  while (inst &&
  ...
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
          comp(),
          "O^O Random Codegen  - Disable AGIReduction on 0x%p.\n",
          _cursor))
      return false;
  }
  ...
  if (attemptLA) {
    ...
    if (windowSize == MaxLAWindowSize &&
      ...
      if (performTransformation(
        ...
        _cursor = generateRXInstruction(_cg,
                                        TR::InstOpCode::LA,
                                        comp()->getStartTree()->getNode(),
                                        lgrTargetReg,
                                        mr,
...
TR_S390PostRAPeephole::ICMReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
          comp(),
          "O^O Random Codegen  - Disable ICMReduction on 0x%p.\n",
          _cursor))
      return false;
  }
  ...
  if (isICMOpportunity &&
    ...
    TR::S390RSInstruction* icm = new (_cg->trHeapMemory())
      TR::S390RSInstruction(TR::InstOpCode::ICM,
                            load->getNode(),
                            load->getRegisterOperand(1),
                            0xF,
                            memcp,
                            prev,
                            _cg);
  ...
  if (s390Instr->getKind() == TR::Instruction::IsRX) {
    ...
    if (mr && mr->getOffset() == 0 && laBaseReg && laTargetReg &&
      ...
      if (performTransformation(
            ...
            _cursor,
            comp()->getDebug()->getName(laTargetReg))) {
        if (comp()->getOption(TR_TraceCG))
          printInstr(comp(), _cursor);
...
bool
TR_S390PostRAPeephole::clearsHighBitOfAddressInReg(TR::Instruction* inst,
    ...
    if (inst->getOpCodeValue() == TR::InstOpCode::LA) {
      if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(),
                 ...
                 inst,
                 targetReg,
                 targetReg->getRegisterName(comp()));
      return true;
    }
...
findActiveCCInst(TR::Instruction* curr,
 ...
 */
bool
TR_S390PostRAPeephole::seekRegInFutureMemRef(int32_t maxWindowSize,
                                             TR::Register* targetReg)
{
  TR::Instruction* current = _cursor->getNext();
  int32_t windowSize = 0;
...
TR_S390PostRAPeephole::LRReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
          comp(),
          "O^O Random Codegen  - Disable LRReduction on 0x%p.\n",
          _cursor))
      return false;
  }
  ...
  if (lgrTargetReg == lgrSourceReg &&
    ...
    if (isAGI &&
      ...
      TR::Instruction* oldCursor = _cursor;
      _cursor = generateRIInstruction(_cg,
                                      lgrOpCode.is64bit() ? TR::InstOpCode::CGHI
                                                          : TR::InstOpCode::CHI,
                                      comp()->getStartTree()->getNode(),
                                      lgrTargetReg,
                                      0,
    ...
    if (prev->getOpCode().setsCC() && prev->getOpCode().setsCarryFlag() &&
      ...
      if ((branchCond == TR::InstOpCode::COND_BE ||
          ...
          performTransformation(
            comp(),
            "\nO^O S390 PEEPHOLE: Removing redundant Load and Test instruction "
            "at %p, because CC can be reused from logical instruction %p\n",
            _cursor,
            prev)) {
        _cg->deleteInst(_cursor);
        if (branchCond == TR::InstOpCode::COND_BE)
  ...
  while ((current != NULL) && !isBarrierToPeepHoleLookback(current) &&
    ...
    if ((curOpCode.getOpCodeValue() == TR::InstOpCode::CHI ||
      ...
      if (curTargetReg == lgrTargetReg && (srcImm == 0) && !(setCC || useCC)) {
        ...
        if (performTransformation(
              comp(),
              "O^O S390 PEEPHOLE: Transforming LR/CHI to LTR at %p\n",
              _cursor)) {
          if (comp()->getOption(TR_TraceCG)) {
...
TR_S390PostRAPeephole::LLCReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
          comp(),
          "O^O Random Codegen  - Disable LLCReduction on 0x%p.\n",
          _cursor))
      return false;
  }
  ...
  if (curOpCode == TR::InstOpCode::LGFR || curOpCode == TR::InstOpCode::LLGTR) {
    ...
    if (llcTgtReg == curSrcReg && llcTgtReg == curTgtReg) {
      ...
      if (performTransformation(
            comp(),
            "O^O S390 PEEPHOLE: Transforming LLC/%s to LLGC at %p.\n",
...
TR_S390PostRAPeephole::LGFRReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
          comp(),
          "O^O Random Codegen  - Disable LGFRReduction on 0x%p.\n",
          _cursor))
      return false;
  }
 ...
 */
bool
TR_S390PostRAPeephole::ConditionalBranchReduction(
  TR::InstOpCode::Mnemonic branchOPReplacement)
{
  ...
  TR::InstOpCode::S390BranchCondition cond = getBranchConditionForMask(
    0xF - (getMaskForBranchCondition(branchInst->getBranchCondition()) & 0xF));
  ...
  if (performTransformation(comp(),
    ...
    while (currInst = realInstructionWithLabelsAndRET(nextInst, true)) {
      ...
      _cg->replaceInst(
        RIInst,
        _cursor = generateRIEInstruction(_cg,
                                         RIInst->getOpCode().getOpCodeValue() ==
                                             TR::InstOpCode::LHI
                                           ? TR::InstOpCode::LOCHI
                                           : TR::InstOpCode::LOCGHI,
                                         RIInst->getNode(),
                                         RIInst->getRegisterOperand(1),
                                         RIInst->getSourceImmediate(),
                                         cond,
                                         RIInst->getPrev()));
...
bool
swapOperands(TR::Register* trueReg,
             TR::Register* compReg,
             TR::Instruction* curr)
{
  TR::InstOpCode::S390BranchCondition branchCond;
...
void
insertLoad(TR::Compilation* comp,
           TR::CodeGenerator* cg,
           TR::Instruction* i,
           TR::Register* r)
{
...
bool
hasDefineToRegister(TR::Instruction* curr, TR::Register* reg)
{
...
TR_S390PostRAPeephole::revertTo32BitShift()
{
  ...
  TR::Register* sourceReg = (instr->getSecondRegister())
  ...
  if (performTransformation(comp(),
    ...
    if (instr->getSourceImmediate()) {
      newInstr = new (_cg->trHeapMemory())
        TR::S390RSInstruction(newOpCode,
                              instr->getNode(),
                              instr->getRegisterOperand(1),
 ...
 */
bool
TR_S390PostRAPeephole::inlineEXtargetHelper(TR::Instruction* inst,
                                            TR::Instruction* instrB4EX)
{
  if (performTransformation(
    ...
    TR::S390RILInstruction* newEXRLInst = new (_cg->trHeapMemory())
      TR::S390RILInstruction(TR::InstOpCode::EXRL,
                             _cursor->getNode(),
                             _cursor->getRegisterOperand(1),
                             ssInstrLabel,
                             _cursor->getPrev(),
                             _cg);
  ...
  if (!comp()->getOption(TR_DisableForcedEXInlining) &&
    ...
    TR::Instruction* branchInstr =
      generateS390BranchInstruction(_cg,
                                    ...
                                    prev);
    TR::Instruction* labelInstr = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _cursor->getNode(), targetLabel, branchInstr);
...
TR_S390PostRAPeephole::attemptZ7distinctOperants()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean() &&
        performTransformation(
          comp(),
          "O^O Random Codegen  - Disable attemptZ7distinctOperants on 0x%p.\n",
          _cursor))
      return false;
  }
      ...
      if (current->defsRegister(lgrTargetReg)) {
        ...
        switch (curOpCode) {
          case TR::InstOpCode::AR: {
            newInstr = generateRRRInstruction(_cg,
                                              TR::InstOpCode::ARK,
                                              node,
                                              lgrTargetReg,
                                              lgrSourceReg,
          ...
          case TR::InstOpCode::AHI: {
            ...
            newInstr = generateRIEInstruction(_cg,
                                              TR::InstOpCode::AHIK,
                                              node,
                                              lgrTargetReg,
                                              lgrSourceReg,
          ...
          case TR::InstOpCode::SLA: {
            ...
            if (mf != NULL) {
              ...
              newInstr = generateRSInstruction(_cg,
                                               TR::InstOpCode::SLAK,
                                               node,
                                               lgrTargetReg,
                                               lgrSourceReg,
 ...
 */
bool
TR_S390PostRAPeephole::ReduceLHIToXR()
{
          ...
          if ((na->getKind() == TR::Instruction::IsRS) &&
            ...
            if (instrMatch) {
              if (((cast_na->getSourceImmediate() +
                  ...
                  performTransformation(
                    comp(),
                    "O^O S390 PEEPHOLE: merging SRL/SLL pair %p %p\n",
                    _cursor,
                    _cursor->getNext())) {
...
bool
TR_S390PostRAPeephole::forwardBranchTarget()
{
