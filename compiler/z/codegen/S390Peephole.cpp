...
bool TR_S390PreRAPeephole::attemptLoadStoreReduction(TR::InstOpCode::Mnemonic storeOpCode, uint16_t size)
{
  if (_cursor->getNext()->getOpCodeValue() == storeOpCode) {
    ...
    if (loadInst->getMemoryReference()->getSymbolReference()
        && loadInst->getMemoryReference()->getSymbolReference()->isUnresolved()
      || storeInst->getMemoryReference()->getSymbolReference()
        && storeInst->getMemoryReference()->getSymbolReference()->isUnresolved()) {
      return false;
    }
    ...
    if (performTransformation(comp(), "O^O S390 PEEPHOLE: Transforming load-store sequence at %p to MVC.", storeInst)) {
      ...
      TR::Instruction* newInst = generateSS1Instruction(_cg, TR::InstOpCode::MVC, comp()->getStartTree()->getNode(),
        size - 1, storeInst->getMemoryReference(), loadInst->getMemoryReference(), _cursor->getPrev());
...
{
  while (inst
    && (inst->getKind() == TR::Instruction::IsPseudo || inst->getKind() == TR::Instruction::IsNotExtended
...
{
  while (inst && (inst->getKind() == TR::Instruction::IsPseudo || inst->getKind() == TR::Instruction::IsNotExtended)) {
    inst = forward ? inst->getNext() : inst->getPrev();
  }
...
bool TR_S390PostRAPeephole::isBarrierToPeepHoleLookback(TR::Instruction* current)
{
  if (NULL == current)
    return true;
...
bool TR_S390PostRAPeephole::AGIReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable AGIReduction on 0x%p.\n", _cursor))
      return false;
  }
  ...
  if (!(curOp == TR::InstOpCode::LR || curOp == TR::InstOpCode::LTR || curOp == TR::InstOpCode::LGR
        || curOp == TR::InstOpCode::LTGR)) {
    return false;
  }
  ...
  while ((current != NULL) && !current->matchesTargetRegister(lgrTargetReg) && !isBarrierToPeepHoleLookback(current)
    ...
    if (current->getOpCodeValue() == TR::InstOpCode::TBEGIN || current->getOpCodeValue() == TR::InstOpCode::TBEGINC
      || current->getOpCodeValue() == TR::InstOpCode::TEND || current->getOpCodeValue() == TR::InstOpCode::TABORT) {
      return false;
    }
  ...
  if (attemptLA) {
    ...
    windowSize = 0;
    while ((current != NULL) && !isBarrierToPeepHoleLookback(current) && !current->matchesTargetRegister(lgrSourceReg)
    ...
    if (windowSize == MaxLAWindowSize
      ...
      if (performTransformation(comp(), "O^O S390 PEEPHOLE: AGI Reduction on %p.\n", current)) {
        ...
        _cursor = generateRXInstruction(
          _cg, TR::InstOpCode::LA, comp()->getStartTree()->getNode(), lgrTargetReg, mr, _cursor->getPrev());
...
bool TR_S390PostRAPeephole::ICMReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable ICMReduction on 0x%p.\n", _cursor))
      return false;
  }
  ...
  if (isICMOpportunity
    ...
    TR::S390RSInstruction* icm = new (_cg->trHeapMemory())
      TR::S390RSInstruction(TR::InstOpCode::ICM, load->getNode(), load->getRegisterOperand(1), 0xF, memcp, prev, _cg);
    _cursor = icm;
  ...
  if (s390Instr->getKind() == TR::Instruction::IsRX) {
    ...
    if (mr && mr->getOffset() == 0 && laBaseReg && laTargetReg && laBaseReg == laTargetReg && laIndexReg == NULL
      ...
      if (performTransformation(comp(),
            "O^O S390 PEEPHOLE: Removing redundant LA (0x%p) with matching target and base registers (%s)\n", _cursor,
...
bool TR_S390PostRAPeephole::duplicateNILHReduction()
{
  if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILH) {
    ...
    if ((na->getKind() == TR::Instruction::IsRI) && (nb->getKind() == TR::Instruction::IsRI)) {
...
bool TR_S390PostRAPeephole::clearsHighBitOfAddressInReg(TR::Instruction* inst, TR::Register* targetReg)
{
...
bool TR_S390PostRAPeephole::NILHReduction()
{
  ...
  TR_ASSERT(_cursor->getOpCodeValue() == TR::InstOpCode::NILH,
    "Expecting duplicateNILHReduction to leave a NILH as current instruction\n");
  transformed |= unnecessaryNILHReduction();

  return transformed;
}

static TR::Instruction* findActiveCCInst(TR::Instruction* curr, TR::InstOpCode::Mnemonic op, TR::Register* ccReg)
{
  TR::Instruction* next = curr;
  while (next = next->getNext()) {
    if (next->getOpCodeValue() == op)
      return next;
    if (next->usesRegister(ccReg) || next->isLabel() || next->getOpCode().setsCC() || next->isCall()
 ...
 * in a memref
 */
bool TR_S390PostRAPeephole::seekRegInFutureMemRef(int32_t maxWindowSize, TR::Register* targetReg)
{
  TR::Instruction* current = _cursor->getNext();
  int32_t windowSize = 0;
...
bool TR_S390PostRAPeephole::removeMergedNullCHK()
{
  if (TR::Compiler->target.isZOS()) {
    ...
    // NULL), and CLT will read the low memory address legally (in this case NULL) to compare it with the other operand.
  ...
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable removeMergedNullCHK on 0x%p.\n", _cursor))
      return false;
  }
...
bool TR_S390PostRAPeephole::LRReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable LRReduction on 0x%p.\n", _cursor))
      return false;
  }
  ...
  if (lgrTargetReg == lgrSourceReg
    && (lgrOpCode.getOpCodeValue() == TR::InstOpCode::LR || lgrOpCode.getOpCodeValue() == TR::InstOpCode::LGR
      || lgrOpCode.getOpCodeValue() == TR::InstOpCode::LDR || lgrOpCode.getOpCodeValue() == TR::InstOpCode::CPYA)) {
  ...
  if (lgrTargetReg == lgrSourceReg
    ...
    if (isAGI
      ...
      TR::Instruction* oldCursor = _cursor;
      _cursor = generateRIInstruction(_cg, lgrOpCode.is64bit() ? TR::InstOpCode::CGHI : TR::InstOpCode::CHI,
        comp()->getStartTree()->getNode(), lgrTargetReg, 0, _cursor->getPrev());
    ...
    if ((prev->getOpCodeValue() == TR::InstOpCode::LR && lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTR)
      ...
      if ((lgrTargetReg == prevTargetReg || lgrTargetReg == prevSourceReg)
        ...
        TR::Instruction* newInst
          = generateRRInstruction(_cg, lgrOpCode.is64bit() ? TR::InstOpCode::LTGR : TR::InstOpCode::LTR,
    ...
    TR::Instruction* next = _cursor->getNext();
    // try to remove redundant LTR, LTGR when we can reuse the condition code of an arithmetic logical operation, ie.
    // Add/Subtract Logical this is also done by isActiveLogicalCC, and the end of generateS390CompareAndBranchOpsHelper
    // when the virtual registers match but those cannot handle the case when the virtual registers are not the same but
    // we do have the same restricted register which is why we are handling it here when all the register assignments are
    // done, and the redundant LR's from the clobber evaluate of the add/sub logical are cleaned up as well.
    ...
    // SLR @01, @04
    // BRC (0x10) Label
    // checks that the prev instruction is an add/sub logical operation that sets the same target register as the
    // LTR/LTGR insn, and that we branch immediately after
    if (prev->getOpCode().setsCC() && prev->getOpCode().setsCarryFlag() && prev->getRegisterOperand(1) == lgrTargetReg
      ...
      if ((branchCond == TR::InstOpCode::COND_BE || branchCond == TR::InstOpCode::COND_BNE)
        && performTransformation(comp(),
          "\nO^O S390 PEEPHOLE: Removing redundant Load and Test instruction at %p, because CC can be reused from "
          "logical instruction %p\n",
          _cursor, prev)) {
        _cg->deleteInst(_cursor);
    ...
    if ((curOpCode.getOpCodeValue() == TR::InstOpCode::CHI || curOpCode.getOpCodeValue() == TR::InstOpCode::CGHI)
      ...
      if (curTargetReg == lgrTargetReg && (srcImm == 0) && !(setCC || useCC)) {
        ...
        if (performTransformation(comp(), "O^O S390 PEEPHOLE: Transforming LR/CHI to LTR at %p\n", _cursor)) {
          ...
          TR::Instruction* oldCursor = _cursor;
          _cursor = generateRRInstruction(_cg, lgrOpCode.is64bit() ? TR::InstOpCode::LTGR : TR::InstOpCode::LTR,
            comp()->getStartTree()->getNode(), lgrTargetReg, lgrSourceReg, _cursor->getPrev());
...
bool TR_S390PostRAPeephole::LLCReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable LLCReduction on 0x%p.\n", _cursor))
      return false;
  }
  ...
  if (curOpCode == TR::InstOpCode::LGFR || curOpCode == TR::InstOpCode::LLGTR) {
    ...
    if (llcTgtReg == curSrcReg && llcTgtReg == curTgtReg) {
      ...
      if (performTransformation(comp(), "O^O S390 PEEPHOLE: Transforming LLC/%s to LLGC at %p.\n",
        ...
        _cursor = generateRXInstruction(
          _cg, TR::InstOpCode::LLGC, comp()->getStartTree()->getNode(), llcTgtReg, memRef, _cursor->getPrev());
        _cg->replaceInst(oldCursor, _cursor);
        performed = true;
      }
    }
...
bool TR_S390PostRAPeephole::LGFRReduction()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable LGFRReduction on 0x%p.\n", _cursor))
      return false;
  }
 ...
 *    ...    ...
 */
bool TR_S390PostRAPeephole::ConditionalBranchReduction(TR::InstOpCode::Mnemonic branchOPReplacement)
{
  ...
  if (performTransformation(
        comp(), "O^O S390 PEEPHOLE: Conditionalizing fall-through block following %p.\n", currInst)) {
    ...
    while (currInst = realInstructionWithLabelsAndRET(nextInst, true)) {
      ...
      _cg->replaceInst(RIInst,
        _cursor = generateRIEInstruction(_cg,
          RIInst->getOpCode().getOpCodeValue() == TR::InstOpCode::LHI ? TR::InstOpCode::LOCHI : TR::InstOpCode::LOCGHI,
          RIInst->getNode(), RIInst->getRegisterOperand(1), RIInst->getSourceImmediate(), cond, RIInst->getPrev()));
    ...
    _cg->replaceInst(branchInst,
      generateRRInstruction(_cg, branchOPReplacement, branchInst->getNode(), branchInst->getRegisterOperand(1),
        branchInst->getRegisterOperand(2), branchInst->getPrev()));
...
bool TR_S390PostRAPeephole::CompareAndBranchReduction()
{
  ...
  if (_cursor->getOpCodeValue() == TR::InstOpCode::CLR && _cursor->getNext()->getOpCodeValue() == TR::InstOpCode::BRC) {
    ...
    if (fallThroughPerformReduction && branchTakenPerformReduction
      && performTransformation(
        comp(), "O^O S390 PEEPHOLE: Transforming CLR [%p] and BRC [%p] to CLRJ\n", clrInstruction, brcInstruction)) {
      ...
      TR::Instruction* clrjInstruction = generateRIEInstruction(_cg, TR::InstOpCode::CLRJ, clrInstruction->getNode(),
        clrInstruction->getRegisterOperand(1), clrInstruction->getRegisterOperand(2), labelSymbol,
        static_cast<TR::S390BranchInstruction*>(brcInstruction)->getBranchCondition(), clrInstruction->getPrev());
 ...
 *    LZOpCode  Rx, Mem
 */
bool TR_S390PostRAPeephole::LoadAndMaskReduction(TR::InstOpCode::Mnemonic LZOpCode)
{
  ...
  if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILL) {
    ...
    if (performTransformation(comp(), "O^O S390 PEEPHOLE: Transforming load-and-mask sequence at %p.\n", nillInst)) {
      ...
      _cg->replaceInst(loadInst,
        _cursor = generateRXInstruction(_cg, LZOpCode, comp()->getStartTree()->getNode(), loadTargetReg,
...
bool TR_S390PostRAPeephole::trueCompEliminationForCompare()
{
  ...
  while (nextInst && !nextInst->isLabel() && !nextInst->getOpCode().setsCC() && !nextInst->isCall()
    && !nextInst->getOpCode().setsCompareFlag()) {
  ...
  if (branchInst && prev && prev->usesRegister(compReg) && !prev->usesRegister(trueReg)) {
    ...
    if (performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare case 1 at %p.\n", curr)) {
  ...
  if (next && next->usesRegister(compReg) && !next->usesRegister(trueReg)) {
    ...
    if (performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare case 2 at %p.\n", curr)) {
  ...
  if (prev && prev->usesRegister(compReg)) {
    ...
    if (performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare case 3 at %p.\n", curr)) {
  ...
  if (next && next->usesRegister(compReg)) {
    ...
    if (performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare case 4 at %p.\n", curr)) {
...
bool TR_S390PostRAPeephole::trueCompEliminationForCompareAndBranch()
{
  ...
  if (backwardBranch) {
    if ((prev && btar) && (prev->usesRegister(compReg) || btar->usesRegister(compReg))
      && (!prev->usesRegister(trueReg) && !btar->usesRegister(trueReg))) {
      if (comp()->getOption(TR_TraceCG)) {
        printInfo("\n");
      }
      if (performTransformation(
            comp(), "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 1 at %p.\n", curr)) {
        swapOperands(trueReg, compReg, curr);
        if (next && next->usesRegister(trueReg))
          insertLoad(comp(), _cg, next->getPrev(), compReg);
        return true;
    ...
    if ((prev && next) && (prev->usesRegister(compReg) || next->usesRegister(compReg))
      ...
      if (!isWCodeCmpEqSwap
        && performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 2 at %p.\n", curr)) {
        swapOperands(trueReg, compReg, curr);
  ...
  if (prev && prev->usesRegister(compReg) && !prev->usesRegister(trueReg)) {
    ...
    if (!isWCodeCmpEqSwap
      && performTransformation(
        comp(), "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 3 at %p.\n", curr)) {
      swapOperands(trueReg, compReg, curr);
      if (btar && btar->usesRegister(trueReg))
        insertLoad(comp(), _cg, btar->getPrev(), compReg);
      if (next && next->usesRegister(trueReg))
  ...
  if (prev && prev->usesRegister(compReg)) {
    ...
    if (performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 4 at %p.\n", curr)) {
      insertLoad(comp(), _cg, prev, trueReg);
      loadInserted = true;
    }
  }
  if (btar && btar->usesRegister(compReg)) {
    ...
    if (performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 5 at %p.\n", curr)) {
  ...
  if (next && next->usesRegister(compReg)) {
    ...
    if (performTransformation(
          comp(), "O^O S390 PEEPHOLE: true complement elimination for compare and branch case 6 at %p.\n", curr)) {
      insertLoad(comp(), _cg, next->getPrev(), trueReg);
      loadInserted = true;
    }
  }
...
bool TR_S390PostRAPeephole::trueCompEliminationForLoadComp()
{
  ...
  if (prev2 && prev2->defsRegister(srcReg)) {
    ...
    if ((next && next->usesRegister(srcReg)) || (prev && prev->usesRegister(srcReg))) {
      ...
      if (performTransformation(
            comp(), "O^O S390 PEEPHOLE: true complement elimination for load complement at %p.\n", curr)) {
  ...
  if (_cursor->throwsImplicitException()) {
    ...
    if ((nextInst->getOpCodeValue() == TR::InstOpCode::BRC || nextInst->getOpCodeValue() == TR::InstOpCode::BRCL)
      ...
      if (OOLbranchInst->getBranchCondition() == TR::InstOpCode::COND_MASK0)
        if (performTransformation(
              comp(), "O^O S390 PEEPHOLE: Eliminating redundant DAA NOP BRC/BRCL to OOL path %p.\n", OOLbranchInst)) {
...
bool TR_S390PostRAPeephole::DAAHandleMemoryReferenceSpill(bool hasPadding)
{
  ...
  if (_cursor->throwsImplicitException()) {
    ...
    if (nextInst->getOpCodeValue() != TR::InstOpCode::BRC && nextInst->getOpCodeValue() != TR::InstOpCode::BRCL) {
      if (performTransformation(comp(),
            "O^O S390 PEEPHOLE: Eliminating DAA intrinsic due to a memory reference spill near instruction %p.\n",
            currInst)) {
        currInst = _cursor;
        nextInst = _cursor->getNext();
        ...
        TR_ASSERT(OOLbranchInst->getBranchCondition() == TR::InstOpCode::COND_MASK0,
          "S390 PEEPHOLE: DAA BRC/BRCL to the OOL path is NOT a NOP branch %p.\n", OOLbranchInst);
...
bool TR_S390PostRAPeephole::revertTo32BitShift()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable revertTo32BitShift on 0x%p.\n", _cursor))
      return false;
  }
  ...
  if (performTransformation(comp(),
    ...
    if (instr->getSourceImmediate()) {
      newInstr = new (_cg->trHeapMemory()) TR::S390RSInstruction(
        newOpCode, instr->getNode(), instr->getRegisterOperand(1), instr->getSourceImmediate(), instr->getPrev(), _cg);
    } else if (instr->getMemoryReference()) {
      TR::MemoryReference* memRef = instr->getMemoryReference();
      memRef->resetMemRefUsedBefore();
      newInstr = new (_cg->trHeapMemory())
        TR::S390RSInstruction(newOpCode, instr->getNode(), instr->getRegisterOperand(1), memRef, instr->getPrev(), _cg);
    } else {
 ...
 * runtime performance.
 */
bool TR_S390PostRAPeephole::inlineEXtargetHelper(TR::Instruction* inst, TR::Instruction* instrB4EX)
{
  if (performTransformation(comp(), "O^O S390 PEEPHOLE: Converting LARL;EX into EXRL instr=[%p]\n", _cursor)) {
    ...
    TR_ASSERT((cnstDataSnip->getKind() == TR::Snippet::IsConstantInstruction),
    ...
    while (iterator && count < 3) {
      ...
      if (iterator->getOpCodeValue() == TR::InstOpCode::LARL) {
        ...
        // Make sure the register we load the literal pool address in is the same register referenced by the EX
        // instruction
        if (LARLInst->isLiteralPoolAddress()
          && LARLInst->getRegisterOperand(1) == _cursor->getMemoryReference()->getBaseRegister()) {
    ...
    TR::Instruction* labelInstr
      = generateS390LabelInstruction(_cg, TR::InstOpCode::LABEL, _cursor->getNode(), ssInstrLabel, inst);
    ...
    // generate EXRL
    TR::S390RILInstruction* newEXRLInst = new (_cg->trHeapMemory()) TR::S390RILInstruction(
      TR::InstOpCode::EXRL, _cursor->getNode(), _cursor->getRegisterOperand(1), ssInstrLabel, _cursor->getPrev(), _cg);
...
bool TR_S390PostRAPeephole::inlineEXtarget()
{
  ...
  if (!comp()->getOption(TR_DisableForcedEXInlining)
    && performTransformation(
      comp(), "O^O S390 PEEPHOLE: Inserting unconditional branch to hide EX target instr=[%p]\n", _cursor)) {
    ...
    TR::Instruction* branchInstr = generateS390BranchInstruction(
      _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, _cursor->getNode(), targetLabel, prev);
    TR::Instruction* labelInstr
      = generateS390LabelInstruction(_cg, TR::InstOpCode::LABEL, _cursor->getNode(), targetLabel, branchInstr);
...
bool TR_S390PostRAPeephole::attemptZ7distinctOperants()
{
  if (comp()->getOption(TR_Randomize)) {
    if (_cg->randomizer.randomBoolean()
      && performTransformation(comp(), "O^O Random Codegen  - Disable attemptZ7distinctOperants on 0x%p.\n", _cursor))
      return false;
  }
  ...
  while ((current != NULL) && !current->isLabel() && !current->isCall()
    ...
    if (current->usesRegister(lgrTargetReg)) {
      if (current->defsRegister(lgrTargetReg)) {
        ...
        if (curOpCode != TR::InstOpCode::AHI && curOpCode != TR::InstOpCode::AGHI && curOpCode != TR::InstOpCode::SLL
          && curOpCode != TR::InstOpCode::SLA && curOpCode != TR::InstOpCode::SRA
        ...
        case TR::InstOpCode::AR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::ARK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::AGR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::AGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::ALR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::ALRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::ALGR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::ALGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::AHI: {
          int16_t imm = ((TR::S390RIInstruction*)current)->getSourceImmediate();
          newInstr
            = generateRIEInstruction(_cg, TR::InstOpCode::AHIK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
          break;
        }
        case TR::InstOpCode::AGHI: {
          int16_t imm = ((TR::S390RIInstruction*)current)->getSourceImmediate();
          newInstr
            = generateRIEInstruction(_cg, TR::InstOpCode::AGHIK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
          break;
        }
        case TR::InstOpCode::NR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::NRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::NGR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::NGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::XR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::XRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::XGR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::XGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::OR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::ORK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::OGR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::OGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::SLA: {
          ...
          if (mf != NULL) {
            ...
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SLAK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
          } else
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SLAK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
          break;
        }
        case TR::InstOpCode::SLL: {
          ...
          if (mf != NULL) {
            mf->resetMemRefUsedBefore();
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SLLK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
          } else
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SLLK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
          break;
        }
        ...
        case TR::InstOpCode::SRA: {
          ...
          if (mf != NULL) {
            mf->resetMemRefUsedBefore();
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SRAK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
          } else
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SRAK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
          break;
        }
        case TR::InstOpCode::SRL: {
          ...
          if (mf != NULL) {
            mf->resetMemRefUsedBefore();
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SRLK, node, lgrTargetReg, lgrSourceReg, mf, prevInstr);
          } else
            newInstr
              = generateRSInstruction(_cg, TR::InstOpCode::SRLK, node, lgrTargetReg, lgrSourceReg, imm, prevInstr);
          break;
        }
        case TR::InstOpCode::SR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::SRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::SGR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::SGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::SLR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::SLRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        case TR::InstOpCode::SLGR: {
          newInstr
            = generateRRRInstruction(_cg, TR::InstOpCode::SLGRK, node, lgrTargetReg, lgrSourceReg, srcReg, prevInstr);
          break;
        }
        default:
          return false;
...
void TR_S390PostRAPeephole::markBlockThatModifiesRegister(TR::Instruction* cursor, TR::Register* targetReg)
{
...
void TR_S390PostRAPeephole::reloadLiteralPoolRegisterForCatchBlock()
{
  ...
  if (TR::Compiler->target.isZOS() && !isZ10 && !_cg->isLiteralPoolOnDemandOn()) {
    ...
    if ((_cg->getLinkage())->setupLiteralPoolRegister(firstSnippet) > 0) {
      ...
      TR::S390RILInstruction* inst = (TR::S390RILInstruction*)generateRILInstruction(_cg, TR::InstOpCode::LARL,
        _cursor->getNode(), _cg->getLitPoolRealRegister(), reinterpret_cast<void*>(0xBABE), _cursor);
  ...
  if (lhiInstruction->getSourceImmediate() == 0) {
    ...
    while (nextInstruction != NULL && !nextInstruction->getOpCode().readsCC()) {
      if (nextInstruction->getOpCode().setsCC() || nextInstruction->getNode()->getOpCodeValue() == TR::BBEnd) {
        ...
        TR::Instruction* xrInstruction = generateRRInstruction(_cg, TR::InstOpCode::XR, lhiInstruction->getNode(),
...
void TR_S390PostRAPeephole::perform()
{
  ...
  while (_cursor != NULL) {
    ...
    if (_cursor->getOpCodeValue() != TR::InstOpCode::FENCE && _cursor->getOpCodeValue() != TR::InstOpCode::ASSOCREGS
    ...
    if (_cursor->getOpCode().setsCompareFlag() && _cursor->getOpCodeValue() != TR::InstOpCode::CHLR
    ...
    case TR::InstOpCode::LHI: {
      ...
      // runtime.
      //
      // TODO (Issue #255): To enable this optimization we need to implement an API which marks instructions that will
      // be patched at runtime and prevent ourselves from modifying such instructions in any way.
      //
      // ReduceLHIToXR();
    } break;
    ...
    case TR::InstOpCode::NILF:
      if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILF) {
        ...
        if ((na->getKind() == TR::Instruction::IsRIL) && (nb->getKind() == TR::Instruction::IsRIL)) {
          ...
          bool instructionsMatch = (cast_na->getTargetPtr() == cast_nb->getTargetPtr())
            && (cast_na->getTargetSnippet() == cast_nb->getTargetSnippet())
            && (cast_na->getTargetSymbol() == cast_nb->getTargetSymbol())
            && (cast_na->getTargetLabel() == cast_nb->getTargetLabel()) && (cast_na->getMask() == cast_nb->getMask())
          ...
          if (instructionsMatch) {
            if (cast_na->matchesTargetRegister(cast_nb->getRegisterOperand(1))
              ...
              //
              else if (((cast_na->getSourceImmediate() & cast_nb->getSourceImmediate())
    ...
    case TR::InstOpCode::SLL: {
      ...
      if (_cursor->getNext()->getOpCodeValue() == _cursor->getOpCodeValue()) {
        ...
        if ((na->getKind() == TR::Instruction::IsRS) && (nb->getKind() == TR::Instruction::IsRS)) {
          ...
          bool instrMatch = (cast_na->getFirstRegister() == cast_nb->getFirstRegister())
...
bool TR_S390PostRAPeephole::forwardBranchTarget()
{
  ...
  if (tmp->getOpCodeValue() == TR::InstOpCode::BRC) {
    ...
    if (firstBranch->getBranchCondition() == TR::InstOpCode::COND_BRC
      ...
      switch (_cursor->getOpCodeValue()) {
      case TR::InstOpCode::BRC:
        ((TR::S390BranchInstruction*)_cursor)->setLabelSymbol(newTargetLabelSym);
        break;
      case TR::InstOpCode::CRJ:
      case TR::InstOpCode::CGRJ:
      case TR::InstOpCode::CIJ:
      ...
      case TR::InstOpCode::CLIJ:
      case TR::InstOpCode::CLGIJ:
        toS390RIEInstruction(_cursor)->setBranchDestinationLabel(newTargetLabelSym);
        break;
      default:
        return false;
      }
