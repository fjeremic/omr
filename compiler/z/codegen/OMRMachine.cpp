...
///////////////////////////////////////////////////////////////////////////////
TR::Instruction *OMR::Z::Machine::registerCopy(
    TR::CodeGenerator *cg, TR_RegisterKinds rk, TR::RealRegister *targetReg,
    TR::RealRegister *sourceReg, TR::Instruction *precedingInstruction) {
  // TODO: In reality we should be validating that the the source is NULL and
  // target is non-NULL however we cannot currently do this because the
  // registerCopy and registerExchange APIs are not updating the register states
  // as registers are shuffled around.
  //
  // A typical example of when this assert would fire is when attempting a
  // register exchange (which uses this API) via a spare reigster. We will
  // generate 3 register shuffles but the register state is only updated later,
  // thus during the second shuffle the spare register will the the target in
  // the above parameter and as such we would assert since the spare register
  // presumably did not have any virtual register associated with it.
  TR_ASSERT_FATAL(sourceReg->getAssignedRegister() != NULL ||
                      targetReg->getAssignedRegister() != NULL,
                  "Attempting register copy with source (%s) and target (%s) "
                  "real registers not corresponding to any virtual register",
  ...
  switch (rk) {
    case TR_GPR: {
      // TODO: Once above is fixed we can always rely on
      // targetReg->getAssignedRegister()
      auto nonNullAssignedReg = targetReg->getAssignedRegister() != NULL
                                    ? targetReg->getAssignedRegister()
                                    : sourceReg->getAssignedRegister();

      auto mnemonic = nonNullAssignedReg->is64BitReg() ? TR::InstOpCode::LGR
                                                       : TR::InstOpCode::LR;

      cursor = generateRRInstruction(cg, mnemonic, node, targetReg, sourceReg,
                                     precedingInstruction);
      break;
    }
    case TR_FPR:
      cursor = generateRRInstruction(cg, TR::InstOpCode::LDR, node, targetReg,
                                     sourceReg, precedingInstruction);
      break;
    case TR_VRF:
      cursor = generateVRRaInstruction(cg, TR::InstOpCode::VLR, node, targetReg,
                                       sourceReg, precedingInstruction);
      break;
  }
 ...
 */
TR::Instruction *OMR::Z::Machine::registerExchange(
    TR::CodeGenerator *cg, TR_RegisterKinds rk, TR::RealRegister *targetReg,
    TR::RealRegister *sourceReg, TR::RealRegister *middleReg,
    TR::Instruction *precedingInstruction) {
  TR_ASSERT_FATAL(sourceReg->getAssignedRegister() != NULL,
                  "Attempting register exchange with source real register (%s) "
                  "not corresponding to any virtual register",
                  getRegisterName(sourceReg, cg));
  TR_ASSERT_FATAL(targetReg->getAssignedRegister() != NULL,
                  "Attempting register exchange with target real register (%s) "
                  "not corresponding to any virtual register",
                  getRegisterName(targetReg, cg));
  ...
  if (rk == TR_FPR) {
    if (middleReg != NULL) {
      ...
      currentInstruction = machine->registerCopy(cg, rk, sourceReg, middleReg,
                                                 precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = machine->registerCopy(cg, rk, targetReg, sourceReg,
                                                 precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = machine->registerCopy(cg, rk, middleReg, targetReg,
                                                 precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
    } else {
      ...
      TR::MemoryReference *tempMR = generateS390MemoryReference(
          currentNode, location->getSymbolReference(), cg);
      location->getSymbolReference()->getSymbol()->setSpillTempLoaded();

      currentInstruction =
          generateRXInstruction(cg, TR::InstOpCode::STD, currentNode, targetReg,
                                tempMR, currentInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction =
          generateRRInstruction(cg, TR::InstOpCode::LDR, currentNode, targetReg,
                                sourceReg, currentInstruction);
      cg->traceRAInstruction(currentInstruction);
      TR::MemoryReference *tempMR2 =
          generateS390MemoryReference(*tempMR, 0, cg);
      currentInstruction =
          generateRXInstruction(cg, TR::InstOpCode::LD, currentNode, sourceReg,
                                tempMR2, currentInstruction);
      cg->traceRAInstruction(currentInstruction);

      cg->freeSpill(location, 8, 0);
    }

    cg->generateDebugCounter("RegisterAllocator/Exchange/FPR", 1,
  ...
  // exchange vector registers
    ...
    if (middleReg != NULL) {
      ...
      currentInstruction =
          generateVRXInstruction(cg, TR::InstOpCode::VST, currentNode,
                                 targetReg, tempMR, 0, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction =
          generateVRRaInstruction(cg, TR::InstOpCode::VLR, currentNode,
                                  targetReg, sourceReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      TR::MemoryReference *tempMR2 =
          generateS390MemoryReference(*tempMR, 0, cg);
      currentInstruction =
          generateVRXInstruction(cg, TR::InstOpCode::VL, currentNode, sourceReg,
                                 tempMR2, 0, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);

      cg->freeSpill(location, 16, 0);
    }

    cg->generateDebugCounter("RegisterAllocator/Exchange/VRF", 1,
                             TR::DebugCounter::Free);
  } else {
    TR_ASSERT_FATAL(targetReg->getAssignedRegister()->is64BitReg() ==
                        sourceReg->getAssignedRegister()->is64BitReg(),
                    "Attempting register exchange with one 64-bit register "
                    "(%s) and one 32-bit register (%s)",
                    getRegisterName(sourceReg, cg),
                    getRegisterName(targetReg, cg));
    ...
    //
    if (middleReg == NULL) {
      // Nasty OSC... should happen very rarely.  Only in cases where deps reach
      // critical mass in reg table and last dep resolution requires a regexch
      // from 'blocked' state.
      //
      TR::Instruction *currentInstruction = precedingInstruction;
      TR_BackingStore *location;
      ...
      currentInstruction = generateRXInstruction(
          cg, opLoad, currentNode, sourceReg, tempMR, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction =
          generateRRInstruction(cg, opLoadReg, currentNode, targetReg,
                                sourceReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      TR::MemoryReference *tempMR2 =
          generateS390MemoryReference(*tempMR, 0, cg);
      currentInstruction = generateRXInstruction(
          cg, opStore, currentNode, targetReg, tempMR2, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      ...
      currentInstruction =
          generateRRInstruction(cg, opLoadReg, currentNode, sourceReg,
                                middleReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      if (debugObj) {
        debugObj->addInstructionComment(toS390RRInstruction(currentInstruction),
                                        REG_EXCHANGE);
      }
      ...
      currentInstruction =
          generateRRInstruction(cg, opLoadReg, currentNode, middleReg,
                                targetReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      if (debugObj) {
        debugObj->addInstructionComment(toS390RRInstruction(currentInstruction),
                                        REG_EXCHANGE);
    ...
    cg->generateDebugCounter(precedingInstruction,
                             "RegisterAllocator/Exchange/GPR", 1,
                             TR::DebugCounter::Free);
  }

  if (middleReg) {
    cg->traceRegisterAssignment("registerExchange: %R and %R via %R", targetReg,
                                sourceReg, middleReg);
  } else {
    cg->traceRegisterAssignment("registerExchange: %R and %R via memory",
                                targetReg, sourceReg);
  }
...
static bool boundNext(TR::Instruction *currentInstruction, int32_t realNum,
                      TR::Register *virtReg) {
...
uint8_t OMR::Z::Machine::getGPRSize() {
...
/**
 *  This method implements a policy for finding the best swap register to be
 * used when performing register exchange. It looks for blocked regs that are
 * assigned dummy virt regs or for free regs. It is possible for this method to
 * return NULL.
 */
TR::RealRegister *OMR::Z::Machine::findBestSwapRegister(TR::Register *reg1,
                                                        TR::Register *reg2) {
  ...
  for (int32_t i = first; i <= last; i++) {
    if (_registerFile[i]->getState() == TR::RealRegister::Blocked &&
        _registerFile[i]->getAssignedRegister() != NULL &&
        _registerFile[i]->getAssignedRegister()->isPlaceholderReg() &&
        _registerFile[i] != realReg1 && _registerFile[i] != realReg2) {
...
uint8_t OMR::Z::Machine::genBitVectOfLiveGPRPairs() {
  ...
  for (int32_t i = TR::RealRegister::FirstGPR, j = 8;
       i <= TR::RealRegister::LastAssignableGPR; i += 2, --j) {
...
bool OMR::Z::Machine::isLegalEvenOddPair(TR::RealRegister *evenReg,
                                         TR::RealRegister *oddReg,
                                         uint64_t availRegMask) {
  if (evenReg == NULL || oddReg == NULL) {
    return false;
  }

  else if (toRealRegister(evenReg)->getRegisterNumber() + 1 ==
           toRealRegister(oddReg)->getRegisterNumber()) {
...
bool OMR::Z::Machine::isLegalEvenRegister(TR::RealRegister *reg,
                                          bool allowBlocked,
                                          uint64_t availRegMask,
                                          bool allowLocked) {
  ...
  // Not a legal Even reg if 1) not even or 2) reg+1 is locked/blocked with
  // non-sibling 3) using GPR0 when we shouldn't
  if (regNum % 2 != 0 ||
      ...
      _registerFile[toRealRegister(reg)->getRegisterNumber() + 1]->getState() ==
          ...
          allowLocked == false ||
      (_registerFile[toRealRegister(reg)->getRegisterNumber() + 1]
               ->getState() == TR::RealRegister::Blocked &&
       allowBlocked == false)) {
    return false;
  } else {
    return true;
...
bool OMR::Z::Machine::isLegalFPPair(TR::RealRegister *firstReg,
                                    TR::RealRegister *secondReg,
                                    uint64_t availRegMask) {
  if (firstReg == NULL || secondReg == NULL) {
    return false;
  } else if (toRealRegister(firstReg)->getRegisterNumber() + 2 ==
             toRealRegister(secondReg)->getRegisterNumber()) {
    return self()->isLegalFirstOfFPRegister(firstReg, ALLOWBLOCKED,
                                            availRegMask) &&
           self()->isLegalSecondOfFPRegister(secondReg, ALLOWBLOCKED,
                                             availRegMask);
  } else {
    return false;
  }
...
bool OMR::Z::Machine::isLegalFirstOfFPRegister(TR::RealRegister *reg,
                                               bool allowBlocked,
                                               uint64_t availRegMask,
                                               bool allowLocked) {
  // Is the register assigned
  ...
  if ((regNum % 4 != 0 && regNum % 4 != 1) ||
      ...
      _registerFile[toRealRegister(reg)->getRegisterNumber() + 2]->getState() ==
              TR::RealRegister::Locked &&
          allowLocked == false ||
      (_registerFile[toRealRegister(reg)->getRegisterNumber() + 2]
               ->getState() == TR::RealRegister::Blocked &&
       allowBlocked == false)) {
    return false;
  } else {
...
bool OMR::Z::Machine::isLegalSecondOfFPRegister(TR::RealRegister *reg,
                                                bool allowBlocked,
                                                uint64_t availRegMask,
                                                bool allowLocked) {
  // Is the register assigned
  ...
  // Not a legal Even reg if 1) not even or 2) reg+1 is locked/blocked with
  // non-sibling 3) using GPR0 when we shouldn't 1) must be one of FPR2, FPR6,
  // FPR10, FPR14, FPR3, FPR7, FPR11, FPR15 2) its sibling FP Reg is NOT
  // locked/blocked
 ...
 */
TR::RealRegister *OMR::Z::Machine::findBestLegalOddRegister(
    uint64_t availRegMask) {
  ...
  for (int32_t i = TR::RealRegister::FirstGPR + 1;
       i <= TR::RealRegister::LastAssignableGPR; i += 2) {
...
bool OMR::Z::Machine::isLegalOddRegister(TR::RealRegister *reg,
                                         bool allowBlocked,
                                         uint64_t availRegMask,
                                         bool allowLocked) {
  // Is the register assigned
 ...
 */
TR::RealRegister *OMR::Z::Machine::findBestLegalEvenRegister(
    uint64_t availRegMask) {
  uint32_t bestWeightSoFar = 0xFFFFFFFF;
  TR::RealRegister *freeRegister = NULL;
  ...
  if (freeRegister == NULL) {
    if (lastEvenReg)
      self()->cg()->traceRegisterAssignment("BEST LAST LEGAL EVEN: %R",
                                            lastEvenReg);
    return lastEvenReg;
  } else {
 ...
 */
TR::RealRegister *OMR::Z::Machine::findBestLegalSiblingFPRegister(
    bool isFirst, uint64_t availRegMask) {
  uint32_t bestWeightSoFar = 0xFFFFFFFF;
  TR::RealRegister *freeRegister = NULL;
  ...
  for (int32_t i = 0; i < NUM_S390_FPR_PAIRS; i++) {
    ...
    // self()->cg()->traceRegWeight(lastSiblingFPReg,
    // lastSiblingFPReg->getWeight());
  ...
  if (freeRegister == NULL) {
    self()->cg()->traceRegisterAssignment("BEST LEGAL sibling FP Reg: %R",
                                          lastSiblingFPReg);
 ...
 *  @return the real reg that gets assigned. For a pair, it returns a new
 * placeholder for the pair.  The real regs are double linked to associated virt
 * regs.
 */
TR::Register *OMR::Z::Machine::assignBestRegister(TR::Register *targetRegister,
  ...
  if (rp) {
    returnRegister = self()->assignBestRegisterPair(
        targetRegister, currInst, doBookKeeping, availRegMask);
  } else {
    returnRegister = self()->assignBestRegisterSingle(
        targetRegister, currInst, doBookKeeping, availRegMask);
  }
...
TR::RealRegister *OMR::Z::Machine::findBestRegisterForShuffle(
    TR::Instruction *currentInstruction, TR::Register *currentAssignedRegister,
    uint64_t availRegMask) {
  TR_ASSERT(currentAssignedRegister,
            "Register shuffling at instruction %P, but the register is not "
            "currently assigned?\n",
            currentInstruction);
  ...
  // we need to make sure we do not use same register that is assigned to this
  // instruction for shuffling Usually, other real registers assigned to the
  // current instruction are block, except in situation where we have exact one,
  // non-pair target register. So temporary block it if it is not blocked
  // already
  ...
  if (targetRegister && targetRegister->getRealRegister() &&
      targetRegister->getRealRegister()->getState() !=
          TR::RealRegister::Blocked) {
    targetRegister->getRealRegister()->block();
    blockingRegister = true;
  }

  // find a free register
  if ((newRegister = self()->findBestFreeRegister(
           currentInstruction, kindOfRegister, currentAssignedRegister,
           availRegMask)) == NULL) {
    newRegister = self()->freeBestRegister(
        currentInstruction, currentAssignedRegister, kindOfRegister);
  }
  // unblock if we blocked target reg of the instruction
  if (blockingRegister) targetRegister->getRealRegister()->unblock();

  self()->cg()->traceRegisterAssignment(
      "%R is assigned to invalid register in this instruction. Shuffling %R => "
      "%R...",
      currentAssignedRegister, currentAssignedRegister->getAssignedRegister(),
      newRegister);
  return newRegister;
}

/**
  * try to shuffle the virt reg to another real reg or spill that virtual
  register
  @return if the virtual reg was shuffled, the real reg that it was shuffled to,
  otherwise NULL
*/
TR::RealRegister *OMR::Z::Machine::shuffleOrSpillRegister(
  ...
  TR::RealRegister *bestRegister = self()->findBestFreeRegister(
      currInst, toFreeReg->getKind(), toFreeReg, availRegMask);
  if (bestRegister == NULL)
    self()->spillRegister(currInst, toFreeReg);
  else {
    TR::Instruction *cursor =
        self()->registerCopy(self()->cg(), toFreeReg->getKind(),
                             assignedRegister, bestRegister, currInst);
...
TR::Register *OMR::Z::Machine::assignBestRegisterSingle(
    TR::Register *targetRegister, TR::Instruction *currInst, bool doBookKeeping,
    uint64_t availRegMask) {
  TR_RegisterKinds kindOfRegister = targetRegister->getKind();
  TR::RealRegister *assignedRegister =
      targetRegister->getAssignedRealRegister();
  ...
  if (kindOfRegister != TR_FPR && kindOfRegister != TR_VRF &&
      assignedRegister != NULL) {
    // In case we need a register shuffle, if the register we are assigning is a
    // source register of the instruction, the shuffling instruction should
    // happen before this instruction
    ...
    if ((assignedRegister->getRealRegisterMask() & availRegMask) == 0) {
      // Oh no.. targetRegister is assigned something it shouldn't be assigned
      // to. Do some shuffling find a new register to shuffle to
      TR::RealRegister *newAssignedRegister =
          self()->findBestRegisterForShuffle(currInst, targetRegister,
                                             availRegMask);
      TR::Instruction *cursor = self()->registerCopy(
          self()->cg(), kindOfRegister, toRealRegister(assignedRegister),
          newAssignedRegister, appendInst);
      ...
      assignedRegister = newAssignedRegister;
             ...
             (assignedRegister->getRealRegisterMask() & availRegMask) == 0) {
    ...
    TR::RealRegister *newAssignedRegister = self()->findBestRegisterForShuffle(
        currInst, targetRegister, availRegMask);
    assignedRegister->unblock();
    TR::Instruction *cursor =
        self()->registerCopy(self()->cg(), kindOfRegister, assignedRegister,
  ...
  //   "assignBestRegisterSingle: Trying to assign a real reg to a real reg in
  //   inst\n");

  // Have we already assigned a real register
  if (assignedRegister == NULL) {
    // These values are only equal upon the first assignment.  Hence, if they
    // arn't the same, and there is no assigned reg, we must have spilled the
    // reg, so invert the spill state to get the reg back
    if (targetRegister->getTotalUseCount() !=
        targetRegister->getFutureUseCount()) {
      ...
      if ((assignedRegister = self()->findBestFreeRegister(
               currInst, kindOfRegister, targetRegister, availRegMask)) ==
          NULL) {
        assignedRegister =
            self()->freeBestRegister(currInst, targetRegister, kindOfRegister);
      }
    }
    ...
    //   targetRegister                      <-
    //   assignedRegister->_assignedRegister()
  ...
  if (doBookKeeping &&
    ...
    TR_ASSERT(targetRegister->getFutureUseCount() >= 0,
              "\nRegister assignment: register [%s] futureUseCount should not "
              "be negative (for node [%s], ref count=%d) !\n",
    ...
    // OOL: if a register is first defined (became live) in the hot path, no
    // matter how many futureUseCount left (in the code path) the register is
    // considered as dead now in the hot path, so GC map contains the correct
    ...
    if (self()->cg()->isOutOfLineHotPath() &&
      ...
      // this is to prevent problems with instructions such as XR
      // virtRegx,virtRegx We assign target registers first, but in this case we
      // do not want to kill the virtual register because the source register
      // (which has the same virtual reg) is not yet assigned
      if (currS390Inst->getRegisterOperand(2) ==
              currS390Inst->getRegisterOperand(1) &&
...
/**
 * Assign the best even/odd pair of consecutive registers to the regPair object
 * passed in. If both virt regs are not assigned, assign a consecutive free pair
 * (curr we do not handle the case where a free cons pair is not available). If
 * either reg is already assigned, coerce assignment of respective even/odd
 * register.
 *
 ...
 *            0      32   63
 */
TR::Register *OMR::Z::Machine::assignBestRegisterPair(TR::Register *regPair,
                                                      TR::Instruction *currInst,
                                                      bool doBookKeeping,
                                                      uint64_t availRegMask) {
  TR_ASSERT(regPair->getRegisterPair() != NULL,
            "OMR::Z::Machine::assignBestRegisterPair: Attempting to assign a "
            "real pair to a non-pair virtual\n");
  ...
  self()->cg()->traceRegisterAssignment(
      "attempt to assign components of register pair ( %R %R )", firstReg,
      lastReg);
  ...
  if (freeRegisterHigh == NULL || freeRegisterLow == NULL) {
    if (self()->cg()->insideInternalControlFlow()) {
      TR_ASSERT(0,
                "ASSERTION assignBestRegisterPair inside Internal Control Flow "
                "for inst %p.\n"
                "Ensure all registers within ICF have a dependency anchored at "
                "the end-ICF label\n",
                currInst);
    }
  }
  ...
  // We need a new placeholder for the pair that will stay with the instruction,
  // leaving the input pair free to change under further allocation.
  TR::RegisterPair *assignedRegPair =
      new (self()->cg()->trHeapMemory(), TR_MemoryBase::RegisterPair)
          TR::RegisterPair(lastReg, firstReg);
  if (regPair->getKind() == TR_FPR) assignedRegPair->setKind(TR_FPR);

  // In case we need to unspill both sibling regs, we do so concurrently to
  // avoid runspill of siblings triping over each other.
  if (freeRegisterLow == NULL &&
      (lastReg->getTotalUseCount() != lastReg->getFutureUseCount()) &&
      freeRegisterHigh == NULL &&
    ...
    if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow,
                                          regPairKind, currInst,
                                          availRegMask)) {
      self()->freeBestRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow,
                                   regPairKind, currInst, availRegMask);
    }
    freeRegisterLow = self()->reverseSpillState(
        currInst, lastReg, toRealRegister(tfreeRegisterLow));
    freeRegisterHigh = self()->reverseSpillState(
        currInst, firstReg, toRealRegister(tfreeRegisterHigh));
  }

  // TotalUse & FutureUse are only equal upon the first assignment.  Hence, if
  // they arn't the same, and there is no assigned reg, we must have spilled the
  // reg, so invert the spill state to get the reg back
  if (freeRegisterLow == NULL &&
      (lastReg->getTotalUseCount() != lastReg->getFutureUseCount())) {
    if (freeRegisterHigh) {
  ...
  if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF) &&
       self()->isLegalEvenOddPair(freeRegisterHigh, freeRegisterLow,
                                  availRegMask)) ||
      ((regPair->getKind() == TR_FPR) &&
       self()->isLegalFPPair(freeRegisterHigh, freeRegisterLow,
                             availRegMask))) {
    // already correctly assigned. Do nothing.
  } else if (freeRegisterLow == NULL && freeRegisterHigh == NULL) {
    // Assign a real register pair for
    //
    if (!self()->findBestFreeRegisterPair(&freeRegisterHigh, &freeRegisterLow,
                                          regPairKind, currInst,
                                          availRegMask)) {
      self()->freeBestRegisterPair(&freeRegisterHigh, &freeRegisterLow,
                                   regPairKind, currInst, availRegMask);
    }
    ...
    freeRegisterLow->setState(TR::RealRegister::Assigned);
             ...
             regPair->getKind()) {  // GPR pair case, real reg assigned but not
                                    // a legal even-odd pair
    if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED,
                                     availRegMask)) {
      // Let's make sure the ODD register is legal
      //
      if (!self()->isLegalOddRegister(freeRegisterLow, DISALLOWBLOCKED,
                                      availRegMask)) {
        ...
        firstReg->unblock();
        lastReg->unblock();
        TR::RealRegister *newOddReg =
            self()->findBestLegalOddRegister(availRegMask);
        TR_ASSERT(
            newOddReg,
            "OMR::Z::Machine::assignBestRegisterPair: newOddReg is NULL!\n");
        self()->coerceRegisterAssignment(currInst, lastReg,
                                         newOddReg->getRegisterNumber());
        freeRegisterLow = newOddReg;
      }
      if (lastReg) {
        lastReg->block();  // Make sure we don't move this anymore when coercing
                           // firstReg
      }
      ...
      self()->coerceRegisterAssignment(
          ...
          (TR::RealRegister::RegNum)(
              toRealRegister(freeRegisterLow)->getRegisterNumber() - 1));
      freeRegisterHigh = self()->getRealRegister((TR::RealRegister::RegNum)(
          toRealRegister(freeRegisterLow)->getRegisterNumber() - 1));
    } else if (!self()->isLegalOddRegister(freeRegisterLow, DISALLOWBLOCKED,
                                           availRegMask)) {
      // No need to check that the EVEN register is legal,
      // because otherwise we would have gone into the other branch.

      if (firstReg) {
        firstReg->block();  // Make sure we don't move this anymore when
                            // coercing lastReg
      }
      ...
      self()->coerceRegisterAssignment(
          currInst, lastReg,
          (TR::RealRegister::RegNum)(
              toRealRegister(freeRegisterHigh)->getRegisterNumber() + 1));
      freeRegisterLow = self()->getRealRegister((TR::RealRegister::RegNum)(
          toRealRegister(freeRegisterHigh)->getRegisterNumber() + 1));
    } else if (!self()->isLegalEvenOddPair(freeRegisterHigh, freeRegisterLow,
                                           availRegMask)) {
      ...
      // if one of them is a legal register, then we can just force the
      ...
      if (self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED,
        ...
        freeRegisterLow = self()->getRealRegister((TR::RealRegister::RegNum)(
            toRealRegister(freeRegisterHigh)->getRegisterNumber() + 1));
                                            ...
                                            availRegMask)) {
        if (lastReg) {
          lastReg->block();  // Make sure we don't move this anymore when
                             // coercing firstReg
        }
        ...
        // Assign a real register pair for
        //
        if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh,
                                              &tfreeRegisterLow, regPairKind,
                                              currInst, availRegMask)) {
          self()->freeBestRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow,
                                       regPairKind, currInst, availRegMask);
        }
  ...
  else if (TR_FPR == regPair->getKind()) {  // FP pair case, real reg assigned
                                            // but not a legal fp reg pair
    if (!self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED,
                                          availRegMask)) {
      // make sure the low FP register is legal
      //
      if (!self()->isLegalSecondOfFPRegister(freeRegisterLow, DISALLOWBLOCKED,
                                             availRegMask)) {
        TR::RealRegister *newLowFPReg =
            self()->findBestLegalSiblingFPRegister(false, availRegMask);
        self()->coerceRegisterAssignment(currInst, lastReg,
                                         newLowFPReg->getRegisterNumber());
      ...
      self()->coerceRegisterAssignment(
          currInst, firstReg,
          (TR::RealRegister::RegNum)(
              toRealRegister(freeRegisterLow->getSiblingRegister())
                  ->getRegisterNumber()));
      freeRegisterHigh = toRealRegister(freeRegisterLow->getSiblingRegister());
    } else if (!self()->isLegalSecondOfFPRegister(
                   freeRegisterLow, DISALLOWBLOCKED, availRegMask)) {
      // make sure the low FP register is legal
      //
      if (!self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED,
                                            availRegMask)) {
        TR::RealRegister *newHighFPReg =
            self()->findBestLegalSiblingFPRegister(true, availRegMask);
        self()->coerceRegisterAssignment(currInst, firstReg,
                                         newHighFPReg->getRegisterNumber());
      ...
      self()->coerceRegisterAssignment(
          currInst, lastReg,
          (TR::RealRegister::RegNum)(
              toRealRegister(freeRegisterHigh->getSiblingRegister())
                  ->getRegisterNumber()));
      freeRegisterLow = toRealRegister(freeRegisterHigh->getSiblingRegister());
    } else if (!self()->isLegalFPPair(freeRegisterHigh, freeRegisterLow,
                                      availRegMask)) {
      // We could probably does something a little more  fancy to avoid
      // moving both regs in cases where one has a legal sibling that
      // is free.
      ...
      if (!self()->findBestFreeRegisterPair(
              &tfreeRegisterHigh, &tfreeRegisterLow, regPair->getKind(),
              currInst, availRegMask)) {
        self()->freeBestRegisterPair(&tfreeRegisterHigh, &tfreeRegisterLow,
                                     regPair->getKind(), currInst,
                                     availRegMask);
      }
  ...
  // OOL: if a register is first defined (became live) in the hot path, no
  // matter how many futureUseCount left (in the code path) the register is
  // considered as dead now in the hot path, so GC map contains the correct list
  // of live registers
  if (doBookKeeping) {
    firstReg->setIsLive();
    if (((firstReg->decFutureUseCount() == 0) ||
         (self()->cg()->isOutOfLineHotPath() &&
          firstReg->getStartOfRange() == currInst)) &&
    ...
    if (((lastReg->decFutureUseCount() == 0) ||
         (self()->cg()->isOutOfLineHotPath() &&
          lastReg->getStartOfRange() == currInst)) &&
 ...
 */
bool OMR::Z::Machine::findBestFreeRegisterPair(TR::RealRegister **firstRegister,
                                               TR::RealRegister **lastRegister,
                                               TR_RegisterKinds rk,
                                               TR::Instruction *currInst,
                                               uint64_t availRegMask) {
  ...
  if (rk != TR_FPR && rk != TR_VRF) {
    ...
    for (int32_t i = TR::RealRegister::FirstGPR;
      ...
      if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free ||
          ...
          (_registerFile[i + 1]->getState() == TR::RealRegister::Free ||
           _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched) &&
          (_registerFile[i + 0]->getWeight() +
           _registerFile[i + 1]->getWeight()) < bestWeightSoFar) {
        freeRegisterHigh = _registerFile[i];
        freeRegisterLow = _registerFile[i + 1];

        bestWeightSoFar =
            freeRegisterHigh->getWeight() + freeRegisterLow->getWeight();
      }
    }
  } else {
    // TODO: What about VRFs?
    // TODO: We look for _registerFile[i + 0] and _registerFile[i + 2] below, is
    // this correct? Look at all FP reg pairs
    for (int32_t k = 0; k < NUM_S390_FPR_PAIRS; k++) {
      ...
      if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free ||
          ...
          (_registerFile[i + 0]->getWeight() +
           _registerFile[i + 2]->getWeight()) < bestWeightSoFar) {
        freeRegisterHigh = _registerFile[i];
  ...
  if (freeRegisterHigh != NULL &&
      freeRegisterHigh->getState() == TR::RealRegister::Unlatched) {
    freeRegisterHigh->setAssignedRegister(NULL);
    freeRegisterHigh->setState(TR::RealRegister::Free);
  }
  ...
  // If unlatched, set it free
  if (freeRegisterLow != NULL &&
      freeRegisterLow->getState() == TR::RealRegister::Unlatched) {
    freeRegisterLow->setAssignedRegister(NULL);
    freeRegisterLow->setState(TR::RealRegister::Free);
  }
  ...
  if (freeRegisterLow != NULL && freeRegisterHigh != NULL) {
    ...
    *lastRegister = freeRegisterLow;
    ...
    self()->cg()->traceRegisterAssignment("BEST FREE PAIR: (%R, %R)",
                                          freeRegisterHigh, freeRegisterLow);
    return true;
  } else {
    return false;
  }
...
void OMR::Z::Machine::freeBestFPRegisterPair(TR::RealRegister **firstReg,
                                             TR::RealRegister **lastReg,
                                             TR::Instruction *currInst,
                                             uint64_t availRegMask) {
  ...
  for (int32_t i = 0; i < NUM_S390_FPR_PAIRS; i++) {
    ...
    } else if ((_registerFile[secondReg]->getState() ==
                    TR::RealRegister::Free ||
                _registerFile[secondReg]->getState() ==
                    TR::RealRegister::Unlatched) &&
               _registerFile[firstReg]->getState() ==
  ...
  TR::Register *bestVirtCandidateHigh =
      bestCandidateHigh->getAssignedRegister();
  ...
  if (bestVirtCandidateLow != NULL) {
    ...
    TR::MemoryReference *tempMRLow = generateS390MemoryReference(
        currentNode, locationLow->getSymbolReference(), self()->cg());
    locationLow->getSymbolReference()->getSymbol()->setSpillTempLoaded();
    bestVirtCandidateLow->setBackingStorage(locationLow);
    cursor =
        generateRXInstruction(self()->cg(), TR::InstOpCode::LD, currentNode,
                              bestCandidateLow, tempMRLow, currInst);
    ...
    if (!comp->getOption(TR_DisableOOL)) {
      if (!self()->cg()->isOutOfLineColdPath()) {
        // the spilledRegisterList contains all registers that are spilled
        // before entering the OOL cold path, post dependencies will be
        // generated using this list
        self()->cg()->getSpilledRegisterList()->push_front(
            bestVirtCandidateLow);

        // OOL cold path: depth = 3, hot path: depth =2,  main line: depth = 1
        // if the spill is outside of the OOL cold/hot path, we need to protect
        // the spill slot if we reverse spill this register inside the OOL
        // cold/hot path
        ...
        if (debugObj)
          self()->cg()->traceRegisterAssignment(
              "OOL: adding reg pair low %s to the spilledRegisterList, "
              "maxSpillDepth = %d\n",
              debugObj->getName(bestVirtCandidateLow),
              locationLow->getMaxSpillDepth());
      } else {
        // do not overwrite mainline and hot path spill depth
        // if this spill is inside OOL cold path, we do not need to protecting
        // the spill slot because the post condition at OOL entry does not
        // expect this register to be spilled
        if (locationLow->getMaxSpillDepth() != 1 &&
  ...
  if (bestVirtCandidateHigh != NULL) {
    ...
    TR::MemoryReference *tempMRHigh = generateS390MemoryReference(
        currentNode, locationHigh->getSymbolReference(), self()->cg());
    locationHigh->getSymbolReference()->getSymbol()->setSpillTempLoaded();
    bestVirtCandidateHigh->setBackingStorage(locationHigh);
    cursor =
        generateRXInstruction(self()->cg(), TR::InstOpCode::LD, currentNode,
                              bestCandidateHigh, tempMRHigh, currInst);
    if (!cursor->assignFreeRegBitVector()) {
      cursor->assignBestSpillRegister();
    }
    ...
    if (!comp->getOption(TR_DisableOOL)) {
      if (!self()->cg()->isOutOfLineColdPath()) {
        ...
        if (debugObj)
          self()->cg()->traceRegisterAssignment(
              "OOL: adding reg pair high %s to the spilledRegisterList, "
              "maxSpillDepth = %d\n",
              debugObj->getName(bestVirtCandidateHigh),
              locationHigh->getMaxSpillDepth());
      } else {
        // do not overwrite mainline and hot path spill depth
        // if this spill is inside OOL cold path, we do not need to protecting
        // the spill slot because the post condition at OOL entry does not
...
void OMR::Z::Machine::freeBestRegisterPair(TR::RealRegister **firstReg,
                                           TR::RealRegister **lastReg,
                                           TR_RegisterKinds rk,
                                           TR::Instruction *currInst,
                                           uint64_t availRegMask) {
  ...
  if (bestCandidateHigh == NULL && bestCandidateLow == NULL) {
    if (self()->cg()->getDebug() != NULL) {
      self()->cg()->getDebug()->printGPRegisterStatus(comp->getOutFile(),
                                                      machine);
    }

    TR_ASSERT_FATAL(
        0, "Ran out of register pairs to use as a pair on instruction [%p]",
        currInst);
  }
  ...
  if (bestVirtCandidateLow != NULL) {
    ...
    if (locationLow == NULL) {
      if (bestVirtCandidateLow->containsInternalPointer()) {
        locationLow = self()->cg()->allocateInternalPointerSpill(
            bestVirtCandidateLow->getPinningArrayPointer());
      } else {
        locationLow =
            bestVirtCandidateLow->is64BitReg()
                ? self()->cg()->allocateSpill(
                      8, bestVirtCandidateLow->containsCollectedReference(),
                      NULL, true)
                : self()->cg()->allocateSpill(
                      4, bestVirtCandidateLow->containsCollectedReference(),
                      NULL, true);
      }
    ...
    if (bestVirtCandidateLow->is64BitReg()) {
      cursor =
          generateRXInstruction(self()->cg(), TR::InstOpCode::LG, currentNode,
                                bestCandidateLow, tempMRLow, currInst);
    } else {
      cursor =
          generateRXInstruction(self()->cg(), TR::InstOpCode::L, currentNode,
                                bestCandidateLow, tempMRLow, currInst);
    }
  ...
  if (bestVirtCandidateHigh != NULL) {
    ...
    if (locationHigh == NULL) {
      if (bestVirtCandidateHigh->containsInternalPointer()) {
        locationHigh = self()->cg()->allocateInternalPointerSpill(
            bestVirtCandidateHigh->getPinningArrayPointer());
      } else {
        locationHigh =
            bestVirtCandidateHigh->is64BitReg()
                ? self()->cg()->allocateSpill(
                      8, bestVirtCandidateHigh->containsCollectedReference(),
                      NULL, true)
                : self()->cg()->allocateSpill(
                      4, bestVirtCandidateHigh->containsCollectedReference(),
                      NULL, true);
      }
...
TR::RealRegister *OMR::Z::Machine::findBestFreeRegister(
    TR::Instruction *currentInstruction, TR_RegisterKinds rk,
  ...
  bool useGPR0 =
      (virtualReg == NULL) ? false : (virtualReg->isUsedInMemRef() == false);
  ...
  if (comp->getOption(TR_Randomize)) {
    ...
    if (TR::RealRegister::isFPR((TR::RealRegister::RegNum)preference)) {
      randomPreference = self()->cg()->randomizer.randomInt(
          TR::RealRegister::FirstFPR, TR::RealRegister::LastFPR);
    } else if (TR::RealRegister::isVRF((TR::RealRegister::RegNum)preference)) {
      randomPreference = self()->cg()->randomizer.randomInt(
          TR::RealRegister::FirstVRF, TR::RealRegister::LastVRF);
    } else if (TR::RealRegister::isGPR((TR::RealRegister::RegNum)preference)) {
      if (useGPR0) {
        randomPreference = self()->cg()->randomizer.randomInt(
            TR::RealRegister::GPR0, TR::RealRegister::LastGPR);
      } else {
        randomPreference = self()->cg()->randomizer.randomInt(
            TR::RealRegister::GPR1, TR::RealRegister::LastGPR);
      }
    }
    if (preference != randomPreference &&
        performTransformation(
            comp,
            "O^O Random Codegen - Randomizing Preference from: %d to: %d\n",
            preference, randomPreference)) {
      preference = randomPreference;
    }
  }

  uint64_t prefRegMask =
      TR::RealRegister::isRealReg((TR::RealRegister::RegNum)preference)
  ...
  if (liveRegOn && virtualReg != NULL) {
    ...
    if (comp->getOption(TR_Randomize)) {
      ...
      if (performTransformation(comp,
                                "O^O Random Codegen - Randomizing Interference "
                                "for %s: Original=%x Random=%x\n",
                                self()->cg()->getDebug()->getName(virtualReg),
  ...
  if (virtualReg->getKind() == TR_VRF &&
      self()->cg()->getSupportsVectorRegisters() &&
      currentInstruction->isCall()) {
    for (int32_t i = TR::RealRegister::FPR0Mask;
         i <= TR::RealRegister::FPR15Mask; ++i) {
      availRegMask &= ~i;
    }
  }
  ...
  /****************************************************************************************************************/
  // Register Associations are best effort. If you really need to map a virtual
  // to a real, use register pre/post dependency conditions.

  if (self()->cg()->enableRegisterPairAssociation() &&
      preference == TR::RealRegister::LegalEvenOfPair) {
    // Check to see if there is a sibling already assigned
    if ((virtualReg->getSiblingRegister()) &&
        (realSibling =
             virtualReg->getSiblingRegister()->getAssignedRegister()) &&
        self()->isLegalOddRegister((TR::RealRegister *)realSibling,
                                   ALLOWBLOCKED, availRegMask)) {
      uint64_t sibRegMask =
          toRealRegister(toRealRegister(realSibling)->getSiblingRegister())
              ->getRealRegisterMask();
      // If the sibling is assigned we choose the even register next to it
      if (availRegMask & sibRegMask) {
        bestRegister =
            _registerFile[toRealRegister(realSibling)->getRegisterNumber() - 1];
      }
    } else {
      if (bestRegister = self()->findBestLegalEvenRegister(availRegMask))
      // Set the pair's sibling to a high weight so that assignment to this real
      // reg is unlikely
      {
        _registerFile[toRealRegister(bestRegister)->getRegisterNumber() + 1]
            ->setWeight(S390_REGISTER_PAIR_SIBLING);
      }
    }
  ...
  } else if (self()->cg()->enableRegisterPairAssociation() &&
             preference == TR::RealRegister::LegalOddOfPair) {
    // Check to see if there is a sibling already assigned
    if ((virtualReg->getSiblingRegister()) &&
        (realSibling =
             virtualReg->getSiblingRegister()->getAssignedRegister()) &&
        self()->isLegalEvenRegister((TR::RealRegister *)realSibling,
                                    ALLOWBLOCKED, availRegMask)) {
      uint64_t sibRegMask =
          toRealRegister(toRealRegister(realSibling)->getSiblingRegister())
              ->getRealRegisterMask();
    ...
    if (bestRegister != NULL &&
      ...
      return bestRegister;
             ...
             preference == TR::RealRegister::LegalFirstOfFPPair) {
    // Check to see if there is a sibling already assigned
    if ((virtualReg->getSiblingRegister()) &&
        (realSibling =
             virtualReg->getSiblingRegister()->getAssignedRegister()) &&
        self()->isLegalSecondOfFPRegister((TR::RealRegister *)realSibling,
                                          ALLOWBLOCKED, availRegMask)) {
      uint64_t sibRegMask =
          toRealRegister(toRealRegister(realSibling)->getSiblingRegister())
              ->getRealRegisterMask();
      ...
      if (availRegMask & sibRegMask) {
        bestRegister =
            _registerFile[toRealRegister(realSibling)->getRegisterNumber() - 2];
      }
    } else {
      if (bestRegister =
              self()->findBestLegalSiblingFPRegister(true, availRegMask))
      ...
      // reg is unlikely
      {
        _registerFile[toRealRegister(bestRegister)->getRegisterNumber() + 2]
            ->setWeight(S390_REGISTER_PAIR_SIBLING);
      }
    }
    ...
    if (bestRegister != NULL &&
      ...
      return bestRegister;
             ...
             preference == TR::RealRegister::LegalSecondOfFPPair) {
    // Check to see if there is a sibling already assigned
    if ((virtualReg->getSiblingRegister()) &&
        (realSibling =
             virtualReg->getSiblingRegister()->getAssignedRegister()) &&
        self()->isLegalFirstOfFPRegister((TR::RealRegister *)realSibling,
                                         ALLOWBLOCKED, availRegMask)) {
      uint64_t sibRegMask =
          toRealRegister(toRealRegister(realSibling)->getSiblingRegister())
              ->getRealRegisterMask();
    ...
    // If preference register is also marked as an interference at some point,
    // check to see if the interference still applies.
    if (liveRegOn && preference != 0 &&
    ...
    if ((prefRegMask & availRegMask) && _registerFile[preference] != NULL &&
        (_registerFile[preference]->getState() == TR::RealRegister::Free ||
         _registerFile[preference]->getState() ==
             TR::RealRegister::Unlatched)) {
      ...
      if (bestRegister != NULL)
        self()->cg()->traceRegisterAssignment(
            "BEST FREE REG by pref for %R is %R", virtualReg, bestRegister);
      else
        self()->cg()->traceRegisterAssignment(
            "BEST FREE REG by pref for %R is NULL", virtualReg);
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    // self()->cg()->traceRegWeight(_registerFile[i],
    ...
    if ((_registerFile[i]->getState() == TR::RealRegister::Free ||
      ...
      if (comp->getOption(TR_Randomize)) {
        ...
        if (performTransformation(
                comp,
                "O^O Random Codegen - Randomizing Weight for %s, Original "
                "bestWeightSoFar: %x randomized to: %x\n",
                self()->cg()->getDebug()->getName(_registerFile[i]),
  ...
  if (freeRegister != NULL &&
      freeRegister->getState() == TR::RealRegister::Unlatched) {
    freeRegister->setAssignedRegister(NULL);
    freeRegister->setState(TR::RealRegister::Free);
  }

  if (freeRegister != NULL)
    self()->cg()->traceRegisterAssignment("BEST FREE REG for %R is %R",
                                          virtualReg, freeRegister);
  else
    self()->cg()->traceRegisterAssignment(
        "BEST FREE REG for %R is NULL (could not find one)", virtualReg);
...
/**
 * Construct a bit vector of all regs that are volatile or that have already
 * been assigned and are also FREE.
 */
uint64_t OMR::Z::Machine::constructFreeRegBitVector(
 ...
 */
TR::RealRegister *OMR::Z::Machine::findRegNotUsedInInstruction(
    TR::Instruction *currentInstruction) {
  ...
  for (int32_t i = first; i <= last, spill == NULL; i++) {
    TR::RealRegister *realReg =
        self()->getRealRegister((TR::RealRegister::RegNum)i);
  ...
  TR_ASSERT(spill != NULL,
            "OMR::Z::Machine::findRegNotUsedInInstruction -- A spill reg "
 ...
 */
void OMR::Z::Machine::spillAllVolatileHighRegisters(
    TR::Instruction *currentInstruction) {
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    bool volatileReg =
        true;  // All high regs are volatile
...
TR::RealRegister *OMR::Z::Machine::freeBestRegister(
    TR::Instruction *currentInstruction, TR::Register *virtReg,
  ...
  TR::Machine *machine = self()->cg()->machine();
  bool useGPR0 =
      (virtReg == NULL) ? false : (virtReg->isUsedInMemRef() == false);
  ...
  if (self()->cg()->getLiveRegisters(rk) != NULL && virtReg != NULL) {
    ...
    // interference might not be acurate beacuse not all vRegs clobber high
    // words todo: merge with boundnext
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    // TODO: This assert was added as it existed in a path which was guarded by
    // is64BitReg. I'm fairly certain this assert should never fire because
    // otherwise we would be trying to free the best register when a free
    // register already exists, meaning that the caller is to blame for not
    // checking if a free register was available. An alternative would be to
    // just return the free register right away? In either event we'll leave
    // this assert here for a little while and we can remove it once it has had
    // time to bake.
    TR_ASSERT_FATAL(realReg->getState() != TR::RealRegister::Free,
                    "Attempting to free best register for virtual register "
                    "(%s) when a free register (%s) already exists",
  ...
  if (numCandidates == 0) {
    if (!allowNullReturn) {
      ...
      TR_ASSERT_FATAL(
          false, "Ran out of register candidates to free on instruction [%p]",
          currentInstruction);
    }
  ...
  while (numCandidates > 1 && cursor != NULL &&
    ...
    for (int32_t i = 0; i < numCandidates; i++) {
      if (cursor->refsRegister(candidates[i])) {
        ...
        i--;  // on continue repeat test for candidate[i] as candidate[i] is now
              // new.
      }
    }
    cursor = cursor->getPrev();
 ...
 *  A GPRX, ....      // use GPRX
 */
void OMR::Z::Machine::spillRegister(TR::Instruction *currentInstruction,
                                    TR::Register *virtReg) {
  TR::Compilation *comp = self()->cg()->comp();
    ...
    case TR_GPR:
      if (!comp->getOption(TR_DisableOOL) &&
        ...
        location =
            virtReg->is64BitReg()
                ? self()->cg()->allocateSpill(
                      8, virtReg->containsCollectedReference(), NULL, true)
                : self()->cg()->allocateSpill(
                      4, virtReg->containsCollectedReference(), NULL, true);

        if (debugObj)
          self()->cg()->traceRegisterAssignment(
              "\nSpilling %s to (%p)\n", debugObj->getName(virtReg), location);
      } else {
        location = self()->cg()->allocateInternalPointerSpill(
    ...
    case TR_FPR:
      if (!comp->getOption(TR_DisableOOL) &&
        ...
        location = self()->cg()->allocateSpill(
            8, false, NULL, true);  // TODO: Use 4 for single-precision values
        if (debugObj)
          self()->cg()->traceRegisterAssignment("\nSpilling FPR %s to (%p)\n",
                                                debugObj->getName(virtReg),
                                                location);
      ...
      break;
    case TR_VRF:
      // Spill of size 16 has never been done before. The call hierarchy seems
      // to support it but this should be watched closely.
  ...
  TR::MemoryReference *tempMR = generateS390MemoryReference(
      currentNode, location->getSymbolReference(), self()->cg());
  location->getSymbolReference()->getSymbol()->setSpillTempLoaded();
  virtReg->setBackingStorage(location);

  if (opCode == TR::InstOpCode::VL)
    cursor = generateVRXInstruction(self()->cg(), opCode, currentNode, best,
                                    tempMR, 0, currentInstruction);
  else
    cursor = generateRXInstruction(self()->cg(), opCode, currentNode, best,
                                   tempMR, currentInstruction);
  ...
  if (!comp->getOption(TR_DisableOOL) && location != NULL) {
    if (!self()->cg()->isOutOfLineColdPath()) {
      // the spilledRegisterList contains all registers that are spilled before
      // entering the OOL cold path, post dependencies will be generated using
      // this list
      ...
      if (!self()->cg()->isOutOfLineHotPath()) {  // main line
        ...
        // hot path
        // do not overwrite main line spill depth
        if (location->getMaxSpillDepth() != 1) location->setMaxSpillDepth(2);
      }
      if (debugObj)
        self()->cg()->traceRegisterAssignment(
            "OOL: adding %s to the spilledRegisterList, maxSpillDepth = %d\n",
            debugObj->getName(virtReg), location->getMaxSpillDepth());
    } else {
      // do not overwrite mainline and hot path spill depth
      // if this spill is inside OOL cold path, we do not need to protecting the
      // spill slot because the post condition at OOL entry does not expect this
...
TR::RealRegister *OMR::Z::Machine::reverseSpillState(
    TR::Instruction *currentInstruction, TR::Register *spilledRegister,
    TR::RealRegister *targetRegister) {
  TR_ASSERT_FATAL(
      spilledRegister->getAssignedRegister() == NULL,
      "Attempting to fill an already assigned virtual register (%s)",
      getRegisterName(spilledRegister, self()->cg()));
  ...
  self()->cg()->traceRegisterAssignment("REVERSE SPILL STATE FOR %R",
                                        spilledRegister);
  ...
  if (targetRegister == NULL) {
    ...
    targetRegister = self()->findBestFreeRegister(
        currentInstruction, spilledRegister->getKind(), spilledRegister);
    if (targetRegister == NULL) {
      targetRegister = self()->freeBestRegister(
          currentInstruction, spilledRegister, spilledRegister->getKind());
    }
  }

  if (self()->cg()->isOutOfLineColdPath()) {
    // the future and total use count might not always reflect register spill
    // state for example a new register assignment in the hot path would cause
  ...
  if (location == NULL) {
    if (rk == TR_GPR) {
      location = spilledRegister->is64BitReg()
                     ? self()->cg()->allocateSpill(
                           8, spilledRegister->containsCollectedReference(),
                           NULL, true)
                     : self()->cg()->allocateSpill(
                           4, spilledRegister->containsCollectedReference(),
                           NULL, true);
    } else if (rk == TR_VRF) {
      location = self()->cg()->allocateSpill(16, false, NULL, true);
    } else {
    ...
    if (comp->getOption(TR_DisableOOL)) {
      ...
      if (self()->cg()->isOutOfLineColdPath()) {
        ...
        if (currentInstruction->isLabel()) {
          if (toS390LabelInstruction(currentInstruction)
                  ->getLabelSymbol()
                  ->isStartOfColdInstructionStream()) {
        ...
        // OOL: only free the spill slot if the register was spilled in the same
        // or less dominant path ex: spilled in cold path, reverse spill in hot
        // path or main line we have to spill this register again when we reach
        // OOL entry point due to post conditions. We want to guarantee that the
        // same spill slot will be protected and reused. maxSpillDepth: 3:cold
        // path, 2:hot path, 1:main line
        if (location->getMaxSpillDepth() == 0 ||
            location->getMaxSpillDepth() == 3 || isOOLentryReverseSpill) {
          ...
          if (debugObj)
            self()->cg()->traceRegisterAssignment(
                "\nOOL: reverse spill %s in less dominant path (%d / 3), "
                "protect spill slot (%p)\n",
                debugObj->getName(spilledRegister),
                location->getMaxSpillDepth(), location);
        }
      } else if (self()->cg()->isOutOfLineHotPath()) {
        // the spilledRegisterList contains all registers that are spilled
        // before entering the OOL path (in backwards RA). Post dependencies
        // will be generated using this list. Any registers reverse spilled
        // before entering OOL should be removed from the spilled list
        if (debugObj)
          self()->cg()->traceRegisterAssignment(
        ...
        if (location->getMaxSpillDepth() == 2) {
          ...
          if (debugObj)
            self()->cg()->traceRegisterAssignment(
                "\nOOL: reverse spilling %s in less dominant path (%d / 2), "
                "protect spill slot (%p)\n",
                debugObj->getName(spilledRegister),
                location->getMaxSpillDepth(), location);
          location->setMaxSpillDepth(0);
  ...
  if (opCode == TR::InstOpCode::VST)
    cursor =
        generateVRXInstruction(self()->cg(), opCode, currentNode,
                               targetRegister, tempMR, 0, currentInstruction);
  else
    cursor = generateRXInstruction(self()->cg(), opCode, currentNode,
                                   targetRegister, tempMR, currentInstruction);
 ...
 */
bool OMR::Z::Machine::isAssignable(TR::Register *virtReg,
                                   TR::RealRegister *realReg) {
  if (virtReg->isUsedInMemRef() &&
      realReg == _registerFile[TR::RealRegister::GPR0]) {
    return false;
  } else {
    // TODO: This is needlessly restrictive. The registerExchange API cannot
    // handle register exchanges with one 32-bit and one 64-bit register and the
    // code below effectively guards against calling the registerExchange API in
    // such situations. This can definitely be relaxed and the registerExchange
    // API taught how to handle those cases. If you look at the places this API
    // (isAssignable) is used you will see we effectively handle it there
    // already. That code needs to be cleaned up and consolidated into the
    // registerExchange API.
    if (virtReg->getKind() != TR_FPR && virtReg->getKind() != TR_VRF) {
      if (realReg->getAssignedRegister() != NULL) {
        return virtReg->is64BitReg() ==
               realReg->getAssignedRegister()->is64BitReg();
      }
    }
...
TR::Instruction *OMR::Z::Machine::coerceRegisterAssignment(
  ...
  TR::RealRegister *currentAssignedRegister =
      (realReg == NULL) ? NULL : toRealRegister(realReg);
  ...
  self()->cg()->traceRegisterAssignment("COERCE %R into %R", virtualRegister,
                                        targetRegister);

  if (rk != TR_FPR && rk != TR_VRF) {
    if (virtualRegister->is64BitReg()) {
      self()->cg()->traceRegisterAssignment(" coerceRA: %R needs 64 bit reg ",
                                            virtualRegister);
    } else {
      self()->cg()->traceRegisterAssignment(" coerceRA: %R needs 32 bit reg ",
                                            virtualRegister);
    }
  }
  ...
  // the target reg is free
    ...
    if (virtualRegister->isPlaceholderReg())
      targetRegister
          ->setIsAssignedMoreThanOnce();  // Register is killed invalidate it
                                          // for moving spill out of loop
    ...
    if (currentAssignedRegister == NULL) {
      ...
      // get the value of virtual register back from spill state if not first
      // use
      if (virtualRegister->getTotalUseCount() !=
          virtualRegister->getFutureUseCount()) {
        self()->cg()->setRegisterAssignmentFlag(TR_RegisterReloaded);
        self()->reverseSpillState(currentInstruction, virtualRegister,
                                  targetRegister);
      } else {
        if (!comp->getOption(TR_DisableOOL) &&
            self()->cg()->isOutOfLineColdPath()) {
          self()->cg()->getFirstTimeLiveOOLRegisterList()->push_front(
              virtualRegister);
        }
      }
    } else {
      ...
      cursor = self()->registerCopy(self()->cg(), rk, currentAssignedRegister,
  ...
  // the target reg is blocked so we need to guarantee it stays in a reg
    ...
    self()->cg()->traceRegisterAssignment(
        "target %R is blocked, assigned to %R", targetRegister,
        currentTargetVirtual);

    spareReg = self()->findBestFreeRegister(currentInstruction, rk,
                                            currentTargetVirtual);
    ...
    if (spareReg == NULL) {
      ...
      spareReg =
          self()->freeBestRegister(currentInstruction, currentTargetVirtual,
    ...
    // find a free register if the virtual register hasn't been assigned to any
    // real register or it is a FPR for later use virtual register is currently
    // assigned to a different register,
    if (currentAssignedRegister != NULL) {
      if (!self()->isAssignable(currentTargetVirtual,
                                currentAssignedRegister)) {
        {
          TR_ASSERT(spareReg != NULL,
                    "coerce reg - blocked, sparereg cannot be NULL.");
          self()->cg()->traceRegAssigned(currentTargetVirtual, spareReg);

          cursor = self()->registerCopy(self()->cg(), rk, targetRegister,
                                        spareReg, currentInstruction);
          cursor =
              self()->registerCopy(self()->cg(), rk, currentAssignedRegister,
                                   targetRegister, currentInstruction);
        ...
        self()->cg()->traceRegAssigned(currentTargetVirtual,
                                       currentAssignedRegister);

        cursor = self()->registerExchange(self()->cg(), rk, targetRegister,
                                          currentAssignedRegister, spareReg,
                                          currentInstruction);
      ...
      cursor = self()->registerCopy(self()->cg(), rk, targetRegister, spareReg,
                                    currentInstruction);
  ...
  // the target reg is assigned
    ...
    currentTargetVirtual = targetRegister->getAssignedRegister();
    self()->cg()->traceRegisterAssignment(
        "target %R is assigned, assigned to %R", targetRegister,
        currentTargetVirtual);
    ...
    // In case we do need to spill, we let the spill policy defined in
    // freeBestReg choose the best spill candidate.
    if (currentAssignedRegister != NULL) {
      ...
      if (!self()->isAssignable(currentTargetVirtual,
        ...
        if (spareReg == NULL) {
          ...
          virtualRegister->block();
          if (virtualRegister->is64BitReg()) {
            // TODO: Can we allow a null return here? Why are the two paths
            // different?
            spareReg = self()->freeBestRegister(
                currentInstruction, currentTargetVirtual,
          ...
          // For some reason (blocked/locked regs etc), we couldn't find a spare
          // reg so spill the virtual in the target and use it for coercion
          if (spareReg == NULL) {
        ...
        // Spill policy decided the best reg to spill was not the targetReg, so
        // move target to the spareReg, and move the source reg to the target.
        if (targetRegister->getRegisterNumber() !=
        ...
        // A dummy reg is better than a free reg, so clobber spareReg if
        // succesfull.
        ...
        if (reg = self()->findBestSwapRegister(virtualRegister,
                                               currentTargetVirtual)) {
          spareReg = reg;
        }
      ...
      if (spareReg == NULL && virtualRegister->isPlaceholderReg()) {
        ...
        if (spareReg == NULL) {
          ...
          if (virtualRegister->is64BitReg()) {
            // TODO: Can we allow a null return here? Why are the two paths
            // different? There is a similar case above.
            spareReg = self()->freeBestRegister(
                currentInstruction, currentTargetVirtual,
                currentTargetVirtual->getKind());
          } else {
        ...
        //  If we chose to spill a reg that wasn't the target, we use the new
        //  space to free up the target.
        if (targetRegister->getRegisterNumber() !=
                spareReg->getRegisterNumber() &&
            !doNotRegCopy) {
          ...
          currentTargetVirtual->setAssignedRegister(spareReg);
          self()->cg()->recordRegisterAssignment(spareReg,
                                                 currentTargetVirtual);
        }
  ...
  if (targetRegister->getState() != TR::RealRegister::Locked) {
    ...
    traceMsg(comp,
             "             This assignment is equivalent to using a hard coded "
             "real register.\n");
...
void OMR::Z::Machine::initializeRegisterFile() {
  ...
  _registerFile[TR::RealRegister::GPR0] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR0, TR::RealRegister::GPR0Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR1] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR1, TR::RealRegister::GPR1Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR2] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR2, TR::RealRegister::GPR2Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR3] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR3, TR::RealRegister::GPR3Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR4] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR4, TR::RealRegister::GPR4Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR5] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR5, TR::RealRegister::GPR5Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR6] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR6, TR::RealRegister::GPR6Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR7] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR7, TR::RealRegister::GPR7Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR8] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR8, TR::RealRegister::GPR8Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR9] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR9, TR::RealRegister::GPR9Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR10] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR10, TR::RealRegister::GPR10Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR11] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR11, TR::RealRegister::GPR11Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR12] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR12, TR::RealRegister::GPR12Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR13] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR13, TR::RealRegister::GPR13Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR14] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR14, TR::RealRegister::GPR14Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR15] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::GPR15, TR::RealRegister::GPR15Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::GPR0]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR1]);
  _registerFile[TR::RealRegister::GPR1]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR0]);
  _registerFile[TR::RealRegister::GPR2]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR3]);
  _registerFile[TR::RealRegister::GPR3]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR2]);
  _registerFile[TR::RealRegister::GPR4]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR5]);
  _registerFile[TR::RealRegister::GPR5]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR4]);
  _registerFile[TR::RealRegister::GPR6]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR7]);
  _registerFile[TR::RealRegister::GPR7]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR6]);
  _registerFile[TR::RealRegister::GPR8]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR9]);
  _registerFile[TR::RealRegister::GPR9]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR8]);
  _registerFile[TR::RealRegister::GPR10]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR11]);
  _registerFile[TR::RealRegister::GPR11]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR10]);
  _registerFile[TR::RealRegister::GPR12]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR13]);
  _registerFile[TR::RealRegister::GPR13]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR12]);
  _registerFile[TR::RealRegister::GPR14]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR15]);
  _registerFile[TR::RealRegister::GPR15]->setSiblingRegister(
      _registerFile[TR::RealRegister::GPR14]);

  // Initialize FPRs

  _registerFile[TR::RealRegister::FPR0] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR0, TR::RealRegister::FPR0Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR1] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR1, TR::RealRegister::FPR1Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR2] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR2, TR::RealRegister::FPR2Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR3] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR3, TR::RealRegister::FPR3Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR4] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR4, TR::RealRegister::FPR4Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR5] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR5, TR::RealRegister::FPR5Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR6] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR6, TR::RealRegister::FPR6Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR7] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR7, TR::RealRegister::FPR7Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR8] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR8, TR::RealRegister::FPR8Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR9] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR9, TR::RealRegister::FPR9Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR10] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR10, TR::RealRegister::FPR10Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR11] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR11, TR::RealRegister::FPR11Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR12] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR12, TR::RealRegister::FPR12Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR13] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR13, TR::RealRegister::FPR13Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR14] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR14, TR::RealRegister::FPR14Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::FPR15] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free,
                       TR::RealRegister::FPR15, TR::RealRegister::FPR15Mask,
                       self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF10] =
      _registerFile[TR::RealRegister::FPR10];
  _registerFile[TR::RealRegister::VRF11] =
      _registerFile[TR::RealRegister::FPR11];
  _registerFile[TR::RealRegister::VRF12] =
      _registerFile[TR::RealRegister::FPR12];
  _registerFile[TR::RealRegister::VRF13] =
      _registerFile[TR::RealRegister::FPR13];
  _registerFile[TR::RealRegister::VRF14] =
      _registerFile[TR::RealRegister::FPR14];
  _registerFile[TR::RealRegister::VRF15] =
      _registerFile[TR::RealRegister::FPR15];

  _registerFile[TR::RealRegister::VRF16] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF16, TR::RealRegister::VRF16Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF17] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF17, TR::RealRegister::VRF17Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF18] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF18, TR::RealRegister::VRF18Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF19] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF19, TR::RealRegister::VRF19Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF20] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF20, TR::RealRegister::VRF20Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF21] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF21, TR::RealRegister::VRF21Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF22] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF22, TR::RealRegister::VRF22Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF23] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF23, TR::RealRegister::VRF23Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF24] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF24, TR::RealRegister::VRF24Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF25] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF25, TR::RealRegister::VRF25Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF26] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF26, TR::RealRegister::VRF26Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF27] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF27, TR::RealRegister::VRF27Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF28] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF28, TR::RealRegister::VRF28Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF29] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF29, TR::RealRegister::VRF29Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF30] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF30, TR::RealRegister::VRF30Mask,
                       self()->cg());

  _registerFile[TR::RealRegister::VRF31] = new (self()->cg()->trHeapMemory())
      TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free,
                       TR::RealRegister::VRF31, TR::RealRegister::VRF31Mask,
                       self()->cg());

  // set siblings for Floating Point register pairs used by long doubles
  _registerFile[TR::RealRegister::FPR0]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR2]);
  _registerFile[TR::RealRegister::FPR1]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR3]);
  _registerFile[TR::RealRegister::FPR2]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR0]);
  _registerFile[TR::RealRegister::FPR3]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR1]);
  _registerFile[TR::RealRegister::FPR4]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR6]);
  _registerFile[TR::RealRegister::FPR5]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR7]);
  _registerFile[TR::RealRegister::FPR6]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR4]);
  _registerFile[TR::RealRegister::FPR7]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR5]);
  _registerFile[TR::RealRegister::FPR8]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR10]);
  _registerFile[TR::RealRegister::FPR9]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR11]);
  _registerFile[TR::RealRegister::FPR10]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR8]);
  _registerFile[TR::RealRegister::FPR11]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR9]);
  _registerFile[TR::RealRegister::FPR12]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR14]);
  _registerFile[TR::RealRegister::FPR13]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR15]);
  _registerFile[TR::RealRegister::FPR14]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR12]);
  _registerFile[TR::RealRegister::FPR15]->setSiblingRegister(
      _registerFile[TR::RealRegister::FPR13]);
}

// GRA register table ///////////////////////////

int32_t OMR::Z::Machine::addGlobalReg(TR::RealRegister::RegNum reg,
                                      int32_t tableIndex) {
  if (reg == TR::RealRegister::NoReg) return tableIndex;
...
int32_t OMR::Z::Machine::getGlobalReg(TR::RealRegister::RegNum reg) {
  for (int32_t i = 0; i <= self()->getLastGlobalVRFRegisterNumber(); i++) {
    if (_globalRegisterNumberToRealRegisterMap[i] == reg) return i;
  }
  return -1;
}

int32_t OMR::Z::Machine::addGlobalRegLater(TR::RealRegister::RegNum reg,
                                           int32_t tableIndex) {
  if (reg == TR::RealRegister::NoReg) return tableIndex;
  if (self()->getRealRegister(reg)->getState() == TR::RealRegister::Locked)
    return tableIndex;
  for (int32_t i = 0; i < tableIndex; i++)
    if (_globalRegisterNumberToRealRegisterMap[i] == reg) {
      // It's already in the table.  Delete it from where it is, and then we'll
      // put it at the end.
      tableIndex--;
      for (; i < tableIndex; i++)
        _globalRegisterNumberToRealRegisterMap[i] =
            _globalRegisterNumberToRealRegisterMap[i + 1];
      break;
    }
  _globalRegisterNumberToRealRegisterMap[tableIndex++] = reg;
  return tableIndex;
...
void OMR::Z::Machine::initializeFPRegPairTable() {
  ...
  _S390FirstOfFPRegisterPairs[3] =
      TR::RealRegister::FPR12;  // pair FPR12 & FPR14
  ...
  _S390FirstOfFPRegisterPairs[7] =
      TR::RealRegister::FPR13;  // pair FPR13 & FPR15
  ...
  _S390SecondOfFPRegisterPairs[2] =
      TR::RealRegister::FPR10;  // pair FPR8 & FPR10
  _S390SecondOfFPRegisterPairs[3] =
      TR::RealRegister::FPR14;  // pair FPR12 & FPR14
  ...
  _S390SecondOfFPRegisterPairs[6] =
      TR::RealRegister::FPR11;  // pair FPR9 & FPR11
  _S390SecondOfFPRegisterPairs[7] =
      TR::RealRegister::FPR15;  // pair FPR13 & FPR15
}
...
uint32_t *OMR::Z::Machine::initializeGlobalRegisterTable() {
  ...
  if ((self()->cg()->isLiteralPoolOnDemandOn() &&
       !linkage->isZLinuxLinkageType()) ||
      (self()->cg()->isLiteralPoolOnDemandOn() &&
       !linkage->getPreserved(linkage->getLitPoolRegister())))
  ...
  if (linkage->isZLinuxLinkageType())  // ordering seems to make crashes on zos.
  {
    for (int32_t i = TR::RealRegister::LastAssignableGPR;
         i >= TR::RealRegister::FirstGPR; --i) {
  ...
  if (TR::Compiler->target.isZOS()) {
    p = self()->addGlobalRegLater(
        self()->cg()->getS390Linkage()->getStackPointerRegister(), p);
  }
  ...
  // p = addGlobalRegLater(linkage->getLitPoolRegister(), p); // zOS private
  // linkage might want this here?
  ...
  for (int32_t i = 0; i < p; i++) {
    if (_globalRegisterNumberToRealRegisterMap[i] ==
        linkage->getCAAPointerRegister())
      self()->setGlobalCAARegisterNumber(i);
    if (_globalRegisterNumberToRealRegisterMap[i] ==
        linkage->getENVPointerRegister())
      self()->setGlobalEnvironmentRegisterNumber(i);
    if (_globalRegisterNumberToRealRegisterMap[i] ==
        linkage->getParentDSAPointerRegister())
      self()->setGlobalParentDSARegisterNumber(i);
    if (_globalRegisterNumberToRealRegisterMap[i] ==
        linkage->getEntryPointRegister())
      self()->setGlobalEntryPointRegisterNumber(i);
    if (_globalRegisterNumberToRealRegisterMap[i] ==
        linkage->getReturnAddressRegister())
...
/**
 * Initialize global vectors in a common location since all configurations
 * (zOS/zLinux 31/64) have the same linkage conventions Each register is
 * explicitly set to stay consistent with a majority of the global register
 * number setup procedures across z-supported platforms and configurations
 *
...
uint32_t OMR::Z::Machine::initGlobalVectorRegisterMap(uint32_t vectorOffset) {
  if (!self()->cg()->getSupportsVectorRegisters() &&
      !self()->cg()->comp()->getOption(TR_DisableVectorRegGRA)) {
  ...
  // Since Vector/FPR overlapping GRA is not yet complete, enable entire vector
  // reg map use only if auto-SIMD or SIMD library are enabled. Eventually, this
  // will be the default case.
  //
  // This flag prevents the low reg file (VRF0-15 from being part of GRA). This
  // ensures no overlap.

  static const char *hideLowerHalf =
      feGetEnv("TR_hideOverlappingVecRegsFromGRA");
  const bool useEntireRegFile =
      ((hideLowerHalf == NULL) && self()->cg()->getSupportsVectorRegisters());
  ...
  // For other machines with peculiar overlapping btwn FPR/VRF, highly recommend
  // overriding initGlobalVectorRegisterMap
  //         .--------.
  ...
  // Colouring will make sure to map global virtual FPRs only to the first 16
  // and vector global virtuals can map to all 32 registers.
  ...
  if (useEntireRegFile) {
    ...
    firstOverlappingVecOffset =
        static_cast<TR_GlobalRegisterNumber>(vectorOffset);
    int32_t numOverlappingVRF = lof - fof + 1;
    // match FPR global reg numbering
    for (int i = 0; i < numOverlappingVRF; i++) {
      _globalRegisterNumberToRealRegisterMap[vectorOffset++] =
          _globalRegisterNumberToRealRegisterMap[dictIdx++];
    }
    lastOverlappingVecOffset =
        static_cast<TR_GlobalRegisterNumber>(vectorOffset - 1);
  }

  // Add non-overlapping VRFs
#define addToGRAMap(regNum)                                \
  _globalRegisterNumberToRealRegisterMap[vectorOffset++] = \
      TR::RealRegister::regNum
  ...
  // We need to ensure fpr's grn's are part of vrf grn's since available reg bit
  // vector in GRA depends on it. This greatly simplifies GRA. By setting first
  // VRF reg num to first FPR, we'll essentially see 16 + 32 grn's for vectors
  // BUT gra takes care to ensure we do things right i.e if any of the 16 fpr
  // grn's are assigned, remove the corresponding reg in the 32.
  self()->setFirstGlobalVRFRegisterNumber(
      self()->getFirstGlobalFPRRegisterNumber());            // equiv to FPR0
  self()->setLastGlobalVRFRegisterNumber(vectorOffset - 1);  // equiv to VRF31

  self()->setFirstOverlappedGlobalVRFRegisterNumber(
      firstOverlappingVecOffset);  // equiv to VRF0
  self()->setLastOverlappedGlobalVRFRegisterNumber(
      lastOverlappingVecOffset);  // equiv to VRF16

  // Similar FPR and VRF will have same 'lastOverlapping' grn.
  self()->setLastGlobalFPRRegisterNumber(
      self()->getLastOverlappedGlobalVRFRegisterNumber());
...
void OMR::Z::Machine::releaseLiteralPoolRegister() {
  ...
  if (comp->getOption(TR_DisableRegisterPressureSimulation)) {
    _globalRegisterNumberToRealRegisterMap[GLOBAL_REG_FOR_LITPOOL] =
        TR::RealRegister::GPR6;
  }
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalGPRRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  return _lastGlobalGPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalGPRRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  return _firstGlobalGPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalFPRRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  self()->setFirstOverlappedGlobalFPRRegisterNumber(reg);
  return _firstGlobalFPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalFPRRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  self()->setLastOverlappedGlobalFPRRegisterNumber(reg);
  return _lastGlobalFPRRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalVRFRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  return _firstGlobalVRFRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalVRFRegisterNumber(
    TR_GlobalRegisterNumber reg) {
  return _lastGlobalVRFRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalCCRRegisterNumber(
    TR_GlobalRegisterNumber reg) {
...
void OMR::Z::Machine::setRegisterWeightsFromAssociations() {
  ...
  for (int32_t i = first; i <= last; ++i) {
    ...
    if (linkage->getPreserved((TR::RealRegister::RegNum)i) &&
      ...
      if (i >= TR::RealRegister::FirstOverlapVRF &&
          i <= TR::RealRegister::LastOverlapVRF)
...
void OMR::Z::Machine::createRegisterAssociationDirective(
    TR::Instruction *cursor) {
  TR::Compilation *comp = self()->cg()->comp();
  int32_t last = TR::RealRegister::LastAssignableVRF;
  TR::RegisterDependencyConditions *associations = new (
      self()->cg()->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
      TR::RegisterDependencyConditions(0, last, self()->cg());
  ...
  TR::Instruction *cursor1 =
      new (self()->cg()->trHeapMemory(), TR_MemoryBase::S390Instruction)
          TR::Instruction(cursor, TR::InstOpCode::ASSOCREGS, associations,
                          self()->cg());
...
TR::Register *OMR::Z::Machine::setVirtualAssociatedWithReal(
    TR::RealRegister::RegNum regNum, TR::Register *virtReg) {
  if ((regNum == TR::RealRegister::EvenOddPair) ||
    ...
    return NULL;
  } else if (regNum >= TR::RealRegister::NumRegisters ||
             _registerFile[regNum]->getRealRegisterMask() == 0) {
...
void OMR::Z::Machine::takeRegisterStateSnapShot() {
  for (int32_t i = TR::RealRegister::FirstGPR;
       i < TR::RealRegister::NumRegisters; i++) {
    ...
    _registerAssociationsSnapShot[i] =
        self()->getVirtualAssociatedWithReal((TR::RealRegister::RegNum)(i));
    _assignedRegisterSnapShot[i] = _registerFile[i]->getAssignedRegister();
    _registerAssignedSnapShot[i] =
        _registerFile[i]->getHasBeenAssignedInMethod();
...
void OMR::Z::Machine::restoreRegisterStateFromSnapShot() {
  for (int32_t i = TR::RealRegister::FirstGPR;
    ...
    if (_registerAssociationsSnapShot[i])
      self()->setVirtualAssociatedWithReal((TR::RealRegister::RegNum)(i),
                                           _registerAssociationsSnapShot[i]);
    if (_registerFile[i]->getState() == TR::RealRegister::Free) {
      if (_registerFile[i]->getAssignedRegister() != NULL) {
        if (_registerFile[i]->getAssignedRegister()->getAssignedRegister() ==
            _registerFile[i]) {
          // clear the Virt -> Real reg assignment if we restored the Real reg
          // state to FREE
      ...
      if (_registerFile[i]->getAssignedRegister() != NULL &&
          _registerFile[i]->getAssignedRegister() !=
              _assignedRegisterSnapShot[i]) {
        // If the virtual register associated with the _registerFile[i] is not
        // assigned to the current real register it must have been updated by
        // prior _registerFile.... Do NOT Clear.
        //   Ex:
        //     RegFile starts as:
        ...
        //  When we handled _registerFile[12], we would have updated the
        //  assignment of GPR_3545 (currently to GPR15) to GPR12. When we
        //  subsequently handle _registerFile[15], we cannot blindly reset
        //  GPR_3545's assigned register to NULL, as that will incorrectly break
        //  the assignment to GPR12.
        if (_registerFile[i]->getAssignedRegister()->getAssignedRegister() ==
            _registerFile[i]) {
          // clear the Virt -> Real reg assignment for any newly assigned
          // virtual register (due to spills) in the hot path
          _registerFile[i]->getAssignedRegister()->setAssignedRegister(NULL);
        }
      }
    }
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned) {
      // self()->cg()->traceRegisterAssignment("\nOOL: restoring %R : %R",
      // _registerFile[i], _registerFile[i]->getAssignedRegister());
      _registerFile[i]->getAssignedRegister()->setAssignedRegister(
          _registerFile[i]);
    }

    // If a register is only created and used in multiple OOL hot paths (created
    // in one OOL hot path the dead in a later OOL hot path) and has never been
    // used in any OOL cold path or main line, then we need to make sure to not
    // include this register in the snapshot once the register is dead,
    // otherwise we will end up having a virtual register with futureUseCount=0
    // assigned to a real reg dangling in the register state table. (Bug exposed
    // by PPC Tarok)
    if (_registerFile[i]->getState() == TR::RealRegister::Assigned &&
...
TR::RegisterDependencyConditions *OMR::Z::Machine::createDepCondForLiveGPRs(
    TR::list<TR::Register *> *spilledRegisterList) {
  int32_t i, c = 0;
  // Calculate number of register dependencies required. This step is not really
  // necessary, but it is space conscious
  //
  TR::Compilation *comp = self()->cg()->comp();
  for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastVRF;
       i = ((i == TR::RealRegister::LastAssignableGPR)
    ...
    if (realReg->getState() == TR::RealRegister::Assigned) c++;
  }
  ...
  if (c) {
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastVRF;
      ...
      if (realReg->getState() == TR::RealRegister::Assigned) {
        ...
        //{
        // TR_ASSERTC(comp, !spilledRegisterList ||
        // !spilledRegisterList->find(virtReg),
        // "a register should not be in both an assigned state and in the
        // spilled list\n");
  ...
  if (spilledRegisterList) {
    for (auto li = spilledRegisterList->begin();
         li != spilledRegisterList->end(); ++li) {
