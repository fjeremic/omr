...
OMR::Z::Machine::Machine(TR::CodeGenerator *cg)
    : OMR::Machine(cg), _lastGlobalGPRRegisterNumber(-1),
      _last8BitGlobalGPRRegisterNumber(-1), _lastGlobalFPRRegisterNumber(-1),
      _lastGlobalCCRRegisterNumber(-1), _lastVolatileNonLinkGPR(-1),
      _lastLinkageGPR(-1), _lastVolatileNonLinkFPR(-1), _lastLinkageFPR(-1),
      _globalEnvironmentRegisterNumber(-1), _globalCAARegisterNumber(-1),
 ...
 */
TR::RealRegister *
OMR::Z::Machine::findBestLegalSiblingFPRegister(bool isFirst,
                                                uint64_t availRegMask) {
  uint32_t bestWeightSoFar = 0xFFFFFFFF;
  TR::RealRegister *freeRegister = NULL;
...
TR::Register *OMR::Z::Machine::assignBestRegisterPair(TR::Register *regPair,
  ...
  if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF) &&
  ...
  } else if (TR_FPR != regPair->getKind()) { // GPR pair case, real reg assigned
                                             // but not a legal even-odd pair
    if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED,
      ...
      if (firstReg) {
        firstReg->block(); // Make sure we don't move this anymore when coercing
                           // lastReg
      }
...
TR::RealRegister *OMR::Z::Machine::findRegNotUsedInInstruction(
  ...
  TR_ASSERT(spill != NULL, "OMR::Z::Machine::findRegNotUsedInInstruction -- A "
                           "spill reg should always be found.");
...
TR::RealRegister *
OMR::Z::Machine::freeBestRegister(TR::Instruction *currentInstruction,
                                  TR::Register *virtReg, TR_RegisterKinds rk,
...
void OMR::Z::Machine::spillRegister(TR::Instruction *currentInstruction,
  ...
  case TR_FPR:
    ...
    break;
  case TR_VRF:
    // Spill of size 16 has never been done before. The call hierarchy seems to
    // support it but this should be watched closely.
...
TR::Instruction *OMR::Z::Machine::coerceRegisterAssignment(
  ...
  // the target reg is free
    ...
    if (virtualRegister->isPlaceholderReg())
      targetRegister
          ->setIsAssignedMoreThanOnce(); // Register is killed invalidate it for
                                         // moving spill out of loop
  ...
  if (targetRegister->getState() != TR::RealRegister::Locked) {
    ...
    traceMsg(comp, "             This assignment is equivalent to using a hard "
                   "coded real register.\n");
...
TR::Register *
OMR::Z::Machine::setVirtualAssociatedWithReal(TR::RealRegister::RegNum regNum,
                                              TR::Register *virtReg) {
