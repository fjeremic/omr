    ...
    TR::Instruction* precedingInstruction)
{
    ...
    case TR_GPR: {
        ...
        auto nonNullAssignedReg = targetReg->getAssignedRegister() != NULL ? targetReg->getAssignedRegister() : sourceReg->getAssignedRegister();
    ...
    TR::Instruction* precedingInstruction)
{
    ...
    // exchange vector registers
        ...
        if (middleReg == NULL) {
            ...
            currentInstruction = generateRRInstruction(cg, opLoadReg, currentNode, sourceReg, middleReg, precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
            if (debugObj) {
            ...
            currentInstruction = generateRRInstruction(cg, opLoadReg, currentNode, middleReg, targetReg, precedingInstruction);
            cg->traceRAInstruction(currentInstruction);
            if (debugObj) {
                debugObj->addInstructionComment(toS390RRInstruction(currentInstruction), REG_EXCHANGE);
            }
...
OMR::Z::Machine::findBestSwapRegister(TR::Register* reg1, TR::Register* reg2)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        if (
            _registerFile[i]->getState() == TR::RealRegister::Blocked && _registerFile[i]->getAssignedRegister() != NULL && _registerFile[i]->getAssignedRegister()->isPlaceholderReg() && _registerFile[i] != realReg1 && _registerFile[i] != realReg2) {
...
OMR::Z::Machine::genBitVectOfLiveGPRPairs()
{
    ...
    for (int32_t i = TR::RealRegister::FirstGPR, j = 8; i <= TR::RealRegister::LastAssignableGPR; i += 2, --j) {
        if (_registerFile[i + 0]->getState() != TR::RealRegister::Free || _registerFile[i + 1]->getState() != TR::RealRegister::Free) {
...
bool OMR::Z::Machine::isLegalEvenRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    if (regNum % 2 != 0 || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() + 1]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() + 1]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
bool OMR::Z::Machine::isLegalFirstOfFPRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
        return false;
    ...
    if ((regNum % 4 != 0 && regNum % 4 != 1) || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() + 2]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() + 2]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
bool OMR::Z::Machine::isLegalSecondOfFPRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
        return false;
    ...
    // 2) its sibling FP Reg is NOT locked/blocked
    if ((regNum % 4 != 2 && regNum % 4 != 3) || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() - 2]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() - 2]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
OMR::Z::Machine::findBestLegalOddRegister(uint64_t availRegMask)
{
    ...
    for (int32_t i = TR::RealRegister::FirstGPR + 1; i <= TR::RealRegister::LastAssignableGPR; i += 2) {
        ...
        if (_registerFile[i - 1]->getState() == TR::RealRegister::Locked || _registerFile[i - 0]->getState() == TR::RealRegister::Locked || _registerFile[i - 1]->getState() == TR::RealRegister::Blocked || _registerFile[i - 0]->getState() == TR::RealRegister::Blocked || (_registerFile[i - 0]->getRealRegisterMask() & availRegMask) == 0 || (_registerFile[i - 1]->getRealRegisterMask() & availRegMask) == 0) {
            continue;
        }

        lastOddReg = _registerFile[i];

        if ((_registerFile[i - 0]->getState() == TR::RealRegister::Free || _registerFile[i - 0]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i - 1]->getState() == TR::RealRegister::Free || _registerFile[i - 1]->getState() == TR::RealRegister::Unlatched) && _registerFile[i - 0]->getWeight() < bestWeightSoFar) {
            // Always prioritize a free pair
            foundFreePair = true;
            freeRegister = _registerFile[i];
            bestWeightSoFar = _registerFile[i]->getWeight();
        } else if (foundFreePair == false && (_registerFile[i]->getState() == TR::RealRegister::Free || _registerFile[i]->getState() == TR::RealRegister::Unlatched) && _registerFile[i]->getWeight() < bestWeightSoFar) {
            freeRegister = _registerFile[i];
...
bool OMR::Z::Machine::isLegalOddRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    // Is the register assigned
    if (reg == NULL) {
        return false;
    ...
    if (regNum % 2 != 1 || (toRealRegister(reg)->getRealRegisterMask() & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
OMR::Z::Machine::findBestLegalEvenRegister(uint64_t availRegMask)
{
    ...
    for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i += 2) {
        // Don't consider registers that can't be assigned.
        if (_registerFile[i + 0]->getState() == TR::RealRegister::Locked || _registerFile[i + 1]->getState() == TR::RealRegister::Locked || _registerFile[i + 0]->getState() == TR::RealRegister::Blocked || _registerFile[i + 1]->getState() == TR::RealRegister::Blocked || (_registerFile[i + 0]->getRealRegisterMask() & availRegMask) == 0 || (_registerFile[i + 1]->getRealRegisterMask() & availRegMask) == 0) {
            continue;
        }
...
OMR::Z::Machine::findBestLegalSiblingFPRegister(bool isFirst, uint64_t availRegMask)
{
    ...
    for (int32_t i = 0; i < NUM_S390_FPR_PAIRS; i++) {
        ...
        } else if (foundFreePair == false && (lastSiblingFPReg->getState() == TR::RealRegister::Free || lastSiblingFPReg->getState() == TR::RealRegister::Unlatched)
            && lastSiblingFPReg->getWeight() < bestWeightSoFar) {
            freeRegister = lastSiblingFPReg;
            bestWeightSoFar = freeRegister->getWeight();
        }
    ...
    uint64_t availRegMask)
{
    ...
    if (targetRegister && targetRegister->getRealRegister() && targetRegister->getRealRegister()->getState() != TR::RealRegister::Blocked) {
    ...
    uint64_t availRegMask)
{
    ...
    if (doBookKeeping && (assignedRegister->getState() != TR::RealRegister::Locked)) {
        ...
        if (self()->cg()->isOutOfLineHotPath() && targetRegister->getStartOfRange() == currInst) {
            ...
            if (currS390Inst->getRegisterOperand(2) == currS390Inst->getRegisterOperand(1) && currS390Inst->getRegisterOperand(2) == targetRegister) {
                killOOLReg = false;
            }
        }
    ...
    uint64_t availRegMask)
{
    ...
    if (freeRegisterLow == NULL && (lastReg->getTotalUseCount() != lastReg->getFutureUseCount()) && freeRegisterHigh == NULL && (firstReg->getTotalUseCount() != firstReg->getFutureUseCount())) {
    ...
    if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF) && self()->isLegalEvenOddPair(freeRegisterHigh, freeRegisterLow, availRegMask))
    ...
    if (doBookKeeping) {
        firstReg->setIsLive();
        if (((firstReg->decFutureUseCount() == 0) || (self()->cg()->isOutOfLineHotPath() && firstReg->getStartOfRange() == currInst)) && (freeRegisterHigh->getState() != TR::RealRegister::Locked)) {
        ...
        lastReg->setIsLive();
        if (((lastReg->decFutureUseCount() == 0) || (self()->cg()->isOutOfLineHotPath() && lastReg->getStartOfRange() == currInst)) && (freeRegisterLow->getState() != TR::RealRegister::Locked)) {
            lastReg->resetIsLive();
    ...
    TR_RegisterKinds rk, TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    if (rk != TR_FPR && rk != TR_VRF) {
        ...
        for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i += 2) {
            // Don't consider registers that can't be assigned.
            if (_registerFile[i + 0]->getState() == TR::RealRegister::Locked || (_registerFile[i + 0]->getRealRegisterMask() & availRegMask) == 0 || _registerFile[i + 1]->getState() == TR::RealRegister::Locked || (_registerFile[i + 1]->getRealRegisterMask() & availRegMask) == 0) {
                continue;
            }

            // See if this pair is available, and better than the prev
            if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free || _registerFile[i + 0]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 1]->getState() == TR::RealRegister::Free || _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 0]->getWeight() + _registerFile[i + 1]->getWeight()) < bestWeightSoFar) {
        ...
        for (int32_t k = 0; k < NUM_S390_FPR_PAIRS; k++) {
            ...
            // Don't consider registers that can't be assigned
            if (_registerFile[i + 0]->getState() == TR::RealRegister::Locked || (_registerFile[i + 0]->getRealRegisterMask() & availRegMask) == 0 || _registerFile[i + 2]->getState() == TR::RealRegister::Locked || (_registerFile[i + 2]->getRealRegisterMask() & availRegMask) == 0) {
                continue;
            }

            // See if this pair is available and better than the previous
            if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free || _registerFile[i + 0]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 2]->getState() == TR::RealRegister::Free || _registerFile[i + 2]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 0]->getWeight() + _registerFile[i + 2]->getWeight()) < bestWeightSoFar) {
                freeRegisterHigh = _registerFile[i];
...
void OMR::Z::Machine::freeBestFPRegisterPair(TR::RealRegister** firstReg, TR::RealRegister** lastReg, TR::Instruction* currInst,
    uint64_t availRegMask)
{
    ...
    for (int32_t i = 0; i < NUM_S390_FPR_PAIRS; i++) {
        ...
        TR::RealRegister::RegNum secondReg = _S390SecondOfFPRegisterPairs[i];
        // Don't consider registers that can't be assigned.
        if ((_registerFile[firstReg]->getState() == TR::RealRegister::Locked) || (_registerFile[secondReg]->getState() == TR::RealRegister::Locked) || (_registerFile[firstReg]->getState() == TR::RealRegister::Blocked) || (_registerFile[secondReg]->getState() == TR::RealRegister::Blocked) || (_registerFile[firstReg]->getRealRegisterMask() & availRegMask) == 0 || (_registerFile[secondReg]->getRealRegisterMask() & availRegMask) == 0) {
            continue;
        }

        // Priority to assigned/unassigned pairs.
        if ((_registerFile[firstReg]->getState() == TR::RealRegister::Free || _registerFile[firstReg]->getState() == TR::RealRegister::Unlatched) && _registerFile[secondReg]->getState() == TR::RealRegister::Assigned) {
            bestCandidateHigh = _registerFile[firstReg];
            bestCandidateLow = _registerFile[secondReg];
        } else if ((_registerFile[secondReg]->getState() == TR::RealRegister::Free || _registerFile[secondReg]->getState() == TR::RealRegister::Unlatched) && _registerFile[firstReg]->getState() == TR::RealRegister::Assigned) {
            bestCandidateHigh = _registerFile[firstReg];
            bestCandidateLow = _registerFile[secondReg];
        }
        // Both assigned
...
void OMR::Z::Machine::freeBestRegisterPair(TR::RealRegister** firstReg, TR::RealRegister** lastReg, TR_RegisterKinds rk, TR::Instruction* currInst,
    uint64_t availRegMask)
{
    ...
    for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i += 2) {
        ...
        // Priority to assigned/unassigned pairs.
        if ((_registerFile[i + 0]->getState() == TR::RealRegister::Free || _registerFile[i + 0]->getState() == TR::RealRegister::Unlatched) && _registerFile[i + 1]->getState() == TR::RealRegister::Assigned) {
            bestCandidateHigh = _registerFile[i];
            bestCandidateLow = _registerFile[i + 1];
        } else if ((_registerFile[i + 1]->getState() == TR::RealRegister::Free || _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched) && _registerFile[i + 0]->getState() == TR::RealRegister::Assigned) {
            bestCandidateHigh = _registerFile[i];
            bestCandidateLow = _registerFile[i + 1];
        }
        // Both assigned
    ...
    if (bestVirtCandidateLow != NULL) {
        ...
        if (locationLow == NULL) {
            if (bestVirtCandidateLow->containsInternalPointer()) {
                ...
                locationLow = bestVirtCandidateLow->is64BitReg() ? self()->cg()->allocateSpill(8, bestVirtCandidateLow->containsCollectedReference(), NULL, true) : self()->cg()->allocateSpill(4, bestVirtCandidateLow->containsCollectedReference(), NULL, true);
            }
    ...
    if (bestVirtCandidateHigh != NULL) {
        ...
        if (locationHigh == NULL) {
            if (bestVirtCandidateHigh->containsInternalPointer()) {
                ...
                locationHigh = bestVirtCandidateHigh->is64BitReg() ? self()->cg()->allocateSpill(8, bestVirtCandidateHigh->containsCollectedReference(), NULL, true) : self()->cg()->allocateSpill(4, bestVirtCandidateHigh->containsCollectedReference(), NULL, true);
            }
    ...
    uint64_t availRegMask)
{
    ...
    if (self()->cg()->enableRegisterPairAssociation() && preference == TR::RealRegister::LegalEvenOfPair) {
        ...
        if ((virtualReg->getSiblingRegister()) && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister()) && self()->isLegalOddRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
       ...
       *    5. None of the above
       */
        if (bestRegister != NULL && (bestRegister->getState() == TR::RealRegister::Free || bestRegister->getState() == TR::RealRegister::Unlatched)) {
        ...
        // Check to see if there is a sibling already assigned
        if ((virtualReg->getSiblingRegister()) && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister()) && self()->isLegalEvenRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
            uint64_t sibRegMask = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())->getRealRegisterMask();
        ...
        // Check to see if there is a sibling already assigned
        if ((virtualReg->getSiblingRegister()) && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister()) && self()->isLegalSecondOfFPRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
            uint64_t sibRegMask = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())->getRealRegisterMask();
            // If the sibling is assigned we choose the even register next to it
            if (availRegMask & sibRegMask) {
        ...
        // Check to see if there is a sibling already assigned
        if ((virtualReg->getSiblingRegister()) && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister()) && self()->isLegalFirstOfFPRegister((TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
            uint64_t sibRegMask = toRealRegister(toRealRegister(realSibling)->getSiblingRegister())->getRealRegisterMask();
            // If the sibling is assigned we choose the even register next to it
            if (availRegMask & sibRegMask) {
                bestRegister = _registerFile[toRealRegister(realSibling)->getRegisterNumber() + 2];
        ...
        if ((prefRegMask & availRegMask) && _registerFile[preference] != NULL && (_registerFile[preference]->getState() == TR::RealRegister::Free || _registerFile[preference]->getState() == TR::RealRegister::Unlatched)) {
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if ((_registerFile[i]->getState() == TR::RealRegister::Free || (_registerFile[i]->getState() == TR::RealRegister::Unlatched)) && (freeRegister == NULL || (iOld && !iNew) || ((iOld || !iNew) && _registerFile[i]->getWeight() < bestWeightSoFar))) {
            iOld = iNew;
...
OMR::Z::Machine::constructFreeRegBitVector(TR::Instruction* currentInstruction)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (realReg->getState() == TR::RealRegister::Free && !currentInstruction->usesRegister(realReg) && (realReg->getHasBeenAssignedInMethod() || !linkage->getPreserved((TR::RealRegister::RegNum)(cnt + 1)))) {
            vector |= (0x1 << cnt);
        }
        cnt++;
    }
...
void OMR::Z::Machine::spillAllVolatileHighRegisters(TR::Instruction* currentInstruction)
{
    ...
    for (int32_t i = first; i <= last; i++) {
        ...
        if (volatileReg && realReg->getState() == TR::RealRegister::Assigned && (virtReg = realReg->getAssignedRegister()) && virtReg->is64BitReg()) {
...
void OMR::Z::Machine::spillRegister(TR::Instruction* currentInstruction, TR::Register* virtReg)
{
    ...
    switch (rk) {
    case TR_GPR:
        if (!comp->getOption(TR_DisableOOL) && (self()->cg()->isOutOfLineColdPath() || self()->cg()->isOutOfLineHotPath()) && virtReg->getBackingStorage()) {
            ...
            location = virtReg->is64BitReg() ? self()->cg()->allocateSpill(8, virtReg->containsCollectedReference(), NULL, true) : self()->cg()->allocateSpill(4, virtReg->containsCollectedReference(), NULL, true);
    ...
    TR::RealRegister* targetRegister)
{
    ...
    if (location == NULL) {
        if (rk == TR_GPR) {
            location = spilledRegister->is64BitReg() ? self()->cg()->allocateSpill(8, spilledRegister->containsCollectedReference(), NULL, true) : self()->cg()->allocateSpill(4, spilledRegister->containsCollectedReference(), NULL, true);
        } else if (rk == TR_VRF) {
            location = self()->cg()->allocateSpill(16, false, NULL, true);
        } else {
            location = self()->cg()->allocateSpill(8, false, NULL, true); // TODO: Use 4 for single-precision values
    ...
    // the target reg is assigned
        ...
        if (currentAssignedRegister != NULL) {
            ...
            if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister) || (rk != TR_FPR && rk != TR_VRF)) {
                ...
                if (targetRegister->getAssignedRegister() == currentTargetVirtual && currentTargetVirtual->getAssignedRegister() == NULL)
    ...
    if (linkage->isZLinuxLinkageType()) // ordering seems to make crashes on zos.
    {
        for (int32_t i = TR::RealRegister::LastAssignableGPR; i >= TR::RealRegister::FirstGPR; --i) {
            ...
            if (linkage->getPreserved(regNum)) {
                if (regNum != linkage->getStaticBaseRegister() && regNum != linkage->getPrivateStaticBaseRegister() && regNum != linkage->getStackPointerRegister()) {
        ...
        for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i++) {
            ...
            if (linkage->getPreserved(regNum)) {
                if (regNum != linkage->getLitPoolRegister() && regNum != linkage->getStaticBaseRegister() && regNum != linkage->getPrivateStaticBaseRegister() && regNum != linkage->getStackPointerRegister()) {
                    p = self()->addGlobalReg(regNum, p);
                }
            }
        }
...
{
    if ((regNum == TR::RealRegister::EvenOddPair) || (regNum == TR::RealRegister::LegalEvenOfPair) || (regNum == TR::RealRegister::LegalOddOfPair)) {
...
void OMR::Z::Machine::restoreRegisterStateFromSnapShot()
{
    for (int32_t i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters; i++) {
        ...
        if (_registerFile[i]->getState() == TR::RealRegister::Free) {
            ...
            if (_registerFile[i]->getAssignedRegister() != NULL && _registerFile[i]->getAssignedRegister() != _assignedRegisterSnapShot[i]) {
        ...
        if (_registerFile[i]->getState() == TR::RealRegister::Assigned && _registerFile[i]->getAssignedRegister()->getFutureUseCount() == 0) {
            _registerFile[i]->getAssignedRegister()->setAssignedRegister(NULL);
...
TR::RegisterDependencyConditions* OMR::Z::Machine::createDepCondForLiveGPRs(TR::list<TR::Register*>* spilledRegisterList)
{
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastVRF; i = ((i == TR::RealRegister::LastAssignableGPR) ? TR::RealRegister::FirstVRF : i + 1)) {
        ...
        TR_ASSERT(realReg->getState() == TR::RealRegister::Assigned || realReg->getState() == TR::RealRegister::Free || realReg->getState() == TR::RealRegister::Locked,
