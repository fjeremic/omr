  ...
  switch (rk) {
    case TR_GPR: {
      ...
      cursor = generateRRInstruction(
        cg, mnemonic, node, targetReg, sourceReg, precedingInstruction);
      break;
    }
    case TR_FPR:
      ...
      break;
    case TR_VRF:
      cursor = generateVRRaInstruction(cg,
                                       TR::InstOpCode::VLR,
                                       node,
                                       targetReg,
                                       sourceReg,
                                  ...
                                  TR::Instruction* precedingInstruction)
{
  ...
  if (rk == TR_FPR) {
    if (middleReg != NULL) {
      ...
      currentInstruction = machine->registerCopy(
        cg, rk, sourceReg, middleReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = machine->registerCopy(
        cg, rk, targetReg, sourceReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = machine->registerCopy(
        cg, rk, middleReg, targetReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
    } else {
      ...
      currentInstruction = generateRXInstruction(cg,
                                                 TR::InstOpCode::STD,
                                                 currentNode,
                                                 targetReg,
                                                 tempMR,
                                                 currentInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = generateRRInstruction(cg,
                                                 TR::InstOpCode::LDR,
                                                 currentNode,
                                                 targetReg,
                                                 sourceReg,
    ...
    cg->generateDebugCounter(
      "RegisterAllocator/Exchange/FPR", 1, TR::DebugCounter::Free);
  }
  // exchange vector registers
    ...
    if (middleReg != NULL) {
      ...
      currentInstruction = generateVRXInstruction(cg,
                                                  TR::InstOpCode::VST,
                                                  currentNode,
                                                  targetReg,
                                                  tempMR,
                                                  ...
                                                  precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = generateVRRaInstruction(cg,
                                                   TR::InstOpCode::VLR,
                                                   currentNode,
                                                   targetReg,
                                                   sourceReg,
    ...
    cg->generateDebugCounter(
      "RegisterAllocator/Exchange/VRF", 1, TR::DebugCounter::Free);
  } else {
    ...
    if (middleReg == NULL) {
      ...
      cg->traceRAInstruction(currentInstruction);
      currentInstruction = generateRRInstruction(
        cg, opLoadReg, currentNode, targetReg, sourceReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      TR::MemoryReference* tempMR2 =
        generateS390MemoryReference(*tempMR, 0, cg);
      currentInstruction = generateRXInstruction(
      ...
      currentInstruction = generateRRInstruction(
        cg, opLoadReg, currentNode, sourceReg, middleReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      if (debugObj) {
      ...
      currentInstruction = generateRRInstruction(
        cg, opLoadReg, currentNode, middleReg, targetReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      if (debugObj) {
        debugObj->addInstructionComment(toS390RRInstruction(currentInstruction),
                                        REG_EXCHANGE);
    ...
    cg->generateDebugCounter(precedingInstruction,
                             "RegisterAllocator/Exchange/GPR",
                             1,
  ...
  if (middleReg) {
    cg->traceRegisterAssignment(
      "registerExchange: %R and %R via %R", targetReg, sourceReg, middleReg);
  } else {
    cg->traceRegisterAssignment(
      "registerExchange: %R and %R via memory", targetReg, sourceReg);
  }
...
static bool
boundNext(TR::Instruction* currentInstruction,
          int32_t realNum,
          TR::Register* virtReg)
{
...
bool
OMR::Z::Machine::isLegalEvenOddPair(TR::RealRegister* evenReg,
...
bool
OMR::Z::Machine::isLegalEvenRegister(TR::RealRegister* reg,
                                     bool allowBlocked,
                                     uint64_t availRegMask,
                                     bool allowLocked)
{
...
bool
OMR::Z::Machine::isLegalFPPair(TR::RealRegister* firstReg,
                               ...
                               uint64_t availRegMask)
{
  if (firstReg == NULL || secondReg == NULL) {
    ...
    return self()->isLegalFirstOfFPRegister(
             firstReg, ALLOWBLOCKED, availRegMask) &&
           self()->isLegalSecondOfFPRegister(
             secondReg, ALLOWBLOCKED, availRegMask);
  } else {
    return false;
  }
}
...
bool
OMR::Z::Machine::isLegalFirstOfFPRegister(TR::RealRegister* reg,
                                          bool allowBlocked,
                                          uint64_t availRegMask,
                                          bool allowLocked)
{
...
bool
OMR::Z::Machine::isLegalSecondOfFPRegister(TR::RealRegister* reg,
                                           bool allowBlocked,
                                           uint64_t availRegMask,
                                           bool allowLocked)
{
...
bool
OMR::Z::Machine::isLegalOddRegister(TR::RealRegister* reg,
                                    bool allowBlocked,
                                    uint64_t availRegMask,
                                    bool allowLocked)
{
...
TR::RealRegister*
OMR::Z::Machine::findBestRegisterForShuffle(
  ...
  uint64_t availRegMask)
{
  ...
  if ((newRegister = self()->findBestFreeRegister(currentInstruction,
                                                  kindOfRegister,
  ...
  self()->cg()->traceRegisterAssignment(
    ...
    "%R...",
    currentAssignedRegister,
    currentAssignedRegister->getAssignedRegister(),
    newRegister);
  return newRegister;
}
                                        ...
                                        uint64_t availRegMask)
{
  ...
  if (bestRegister == NULL)
    ...
    TR::Instruction* cursor = self()->registerCopy(self()->cg(),
                                                   toFreeReg->getKind(),
                                                   assignedRegister,
                                                   bestRegister,
                                                   currInst);
  ...
  if (assignedRegister == NULL) {
    if (self()->cg()->insideInternalControlFlow()) {
      TR_ASSERT_FATAL(false,
                      "Attempting to assign a register (%s) inside ICF",
  ...
  if (kindOfRegister != TR_FPR && kindOfRegister != TR_VRF &&
    ...
    if ((assignedRegister->getRealRegisterMask() & availRegMask) == 0) {
      ...
      TR::RealRegister* newAssignedRegister =
        self()->findBestRegisterForShuffle(
          currInst, targetRegister, availRegMask);
      TR::Instruction* cursor =
        self()->registerCopy(self()->cg(),
                             kindOfRegister,
                             toRealRegister(assignedRegister),
                             newAssignedRegister,
                             appendInst);
  ...
  if (assignedRegister == NULL) {
    ...
    if (targetRegister->getTotalUseCount() !=
      ...
      if ((assignedRegister = self()->findBestFreeRegister(
             currInst, kindOfRegister, targetRegister, availRegMask)) == NULL) {
        assignedRegister =
                                        ...
                                        uint64_t availRegMask)
{
  ...
  self()->cg()->traceRegisterAssignment(
    "attempt to assign components of register pair ( %R %R )",
    firstReg,
    lastReg);
  ...
  if (freeRegisterLow == NULL &&
    ...
    if (!self()->findBestFreeRegisterPair(&tfreeRegisterHigh,
                                          ...
                                          currInst,
                                          availRegMask)) {
      self()->freeBestRegisterPair(&tfreeRegisterHigh,
                                   &tfreeRegisterLow,
                                   regPairKind,
                                   currInst,
                                   availRegMask);
  ...
  if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF) &&
       self()->isLegalEvenOddPair(
         freeRegisterHigh, freeRegisterLow, availRegMask)) ||
      ((regPair->getKind() == TR_FPR) && self()->isLegalFPPair(freeRegisterHigh,
                                                               freeRegisterLow,
                                                               availRegMask))) {
    ...
    // Assign a real register pair for
    //
    if (!self()->findBestFreeRegisterPair(&freeRegisterHigh,
                                          &freeRegisterLow,
                                          regPairKind,
                                          currInst,
                                          availRegMask)) {
      self()->freeBestRegisterPair(&freeRegisterHigh,
                                   &freeRegisterLow,
                                   regPairKind,
                                   currInst,
                                   availRegMask);
    ...
    if (!self()->isLegalEvenRegister(
          freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
      // Let's make sure the ODD register is legal
      //
      if (!self()->isLegalOddRegister(
        ...
        self()->coerceRegisterAssignment(
          currInst, lastReg, newOddReg->getRegisterNumber());
      ...
      freeRegisterHigh = self()->getRealRegister((TR::RealRegister::RegNum)(
        toRealRegister(freeRegisterLow)->getRegisterNumber() - 1));
    } else if (!self()->isLegalOddRegister(
                 freeRegisterLow, DISALLOWBLOCKED, availRegMask)) {
      ...
      freeRegisterLow = self()->getRealRegister((TR::RealRegister::RegNum)(
        toRealRegister(freeRegisterHigh)->getRegisterNumber() + 1));
    } else if (!self()->isLegalEvenOddPair(
                 freeRegisterHigh, freeRegisterLow, availRegMask)) {
    ...
    if (!self()->isLegalFirstOfFPRegister(
      ...
      if (!self()->isLegalFirstOfFPRegister(
        ...
        self()->coerceRegisterAssignment(
          currInst, firstReg, newHighFPReg->getRegisterNumber());
 ...
 */
bool
OMR::Z::Machine::findBestFreeRegisterPair(TR::RealRegister** firstRegister,
                                          ...
                                          uint64_t availRegMask)
{
  ...
  if (freeRegisterLow != NULL && freeRegisterHigh != NULL) {
    ...
    *lastRegister = freeRegisterLow;
    ...
    self()->cg()->traceRegisterAssignment(
      "BEST FREE PAIR: (%R, %R)", freeRegisterHigh, freeRegisterLow);
    return true;
  } else {
    return false;
  }
...
void
OMR::Z::Machine::freeBestFPRegisterPair(TR::RealRegister** firstReg,
                                        ...
                                        uint64_t availRegMask)
{
  ...
  if (bestVirtCandidateLow != NULL) {
    ...
    cursor = generateRXInstruction(self()->cg(),
                                   TR::InstOpCode::LD,
                                   currentNode,
                                   bestCandidateLow,
                                   tempMRLow,
...
void
OMR::Z::Machine::freeBestRegisterPair(TR::RealRegister** firstReg,
                                      TR::RealRegister** lastReg,
                                      TR_RegisterKinds rk,
                                      TR::Instruction* currInst,
                                      uint64_t availRegMask)
{
  ...
  if (bestCandidateHigh == NULL && bestCandidateLow == NULL) {
    ...
    TR_ASSERT_FATAL(
      0,
      "Ran out of register pairs to use as a pair on instruction [%p]",
      currInst);
  }
      ...
      if (bestVirtCandidateLow->containsInternalPointer()) {
        ...
        locationLow = bestVirtCandidateLow->is64BitReg()
                        ? self()->cg()->allocateSpill(
                            8,
                            bestVirtCandidateLow->containsCollectedReference(),
                            NULL,
                            true)
                        : self()->cg()->allocateSpill(
                            4,
      ...
      if (bestVirtCandidateHigh->containsInternalPointer()) {
        ...
        locationHigh =
          bestVirtCandidateHigh->is64BitReg()
            ? self()->cg()->allocateSpill(
                8,
                bestVirtCandidateHigh->containsCollectedReference(),
                NULL,
                true)
            : self()->cg()->allocateSpill(
                4,
                                      ...
                                      uint64_t availRegMask)
{
  ...
  if (self()->cg()->enableRegisterPairAssociation() &&
    ...
    if ((virtualReg->getSiblingRegister()) &&
        ...
        self()->isLegalOddRegister(
          (TR::RealRegister*)realSibling, ALLOWBLOCKED, availRegMask)) {
      uint64_t sibRegMask =
  ...
  if (freeRegister != NULL)
    self()->cg()->traceRegisterAssignment(
      "BEST FREE REG for %R is %R", virtualReg, freeRegister);
  else
    self()->cg()->traceRegisterAssignment(
 ...
 */
TR::RealRegister*
OMR::Z::Machine::findRegNotUsedInInstruction(
  TR::Instruction* currentInstruction)
{
 ...
 */
void
OMR::Z::Machine::spillAllVolatileHighRegisters(
  TR::Instruction* currentInstruction)
{
                                  ...
                                  bool allowNullReturn)
{
  ...
  if (numCandidates == 0) {
    if (!allowNullReturn) {
      ...
      TR_ASSERT_FATAL(
        false,
        "Ran out of register candidates to free on instruction [%p]",
        currentInstruction);
    }
 ...
 */
void
OMR::Z::Machine::spillRegister(TR::Instruction* currentInstruction,
                               TR::Register* virtReg)
{
  TR::Compilation* comp = self()->cg()->comp();
  ...
  if (opCode == TR::InstOpCode::VL)
    cursor = generateVRXInstruction(
      self()->cg(), opCode, currentNode, best, tempMR, 0, currentInstruction);
  else
    cursor = generateRXInstruction(
      self()->cg(), opCode, currentNode, best, tempMR, currentInstruction);
                                   ...
                                   TR::RealRegister* targetRegister)
{
  ...
  if (location == NULL) {
    if (rk == TR_GPR) {
      location =
        spilledRegister->is64BitReg()
          ? self()->cg()->allocateSpill(
              8, spilledRegister->containsCollectedReference(), NULL, true)
          : self()->cg()->allocateSpill(
              4, spilledRegister->containsCollectedReference(), NULL, true);
    } else if (rk == TR_VRF) {
      location = self()->cg()->allocateSpill(16, false, NULL, true);
    } else {
      location = self()->cg()->allocateSpill(
    ...
    if (comp->getOption(TR_DisableOOL)) {
      ...
      if (self()->cg()->isOutOfLineColdPath()) {
        ...
        if (location->getMaxSpillDepth() == 0 ||
          ...
          if (debugObj)
            self()->cg()->traceRegisterAssignment(
              "\nOOL: reverse spill %s in less dominant path (%d / 3), protect "
              "spill slot (%p)\n",
              debugObj->getName(spilledRegister),
              location->getMaxSpillDepth(),
              location);
  ...
  if (opCode == TR::InstOpCode::VST)
    cursor = generateVRXInstruction(self()->cg(),
                                    opCode,
                                    currentNode,
                                    targetRegister,
                                    tempMR,
 ...
 */
TR::Instruction*
OMR::Z::Machine::coerceRegisterAssignment(
  TR::Instruction* currentInstruction,
  ...
  TR::RealRegister::RegNum registerNumber)
{
  ...
  self()->cg()->traceRegisterAssignment(
    "COERCE %R into %R", virtualRegister, targetRegister);
    ...
    if (currentAssignedRegister == NULL) {
      ...
      if (virtualRegister->getTotalUseCount() !=
        ...
        self()->reverseSpillState(
          currentInstruction, virtualRegister, targetRegister);
      } else {
        if (!comp->getOption(TR_DisableOOL) &&
            self()->cg()->isOutOfLineColdPath()) {
      ...
      cursor = self()->registerCopy(self()->cg(),
                                    rk,
                                    currentAssignedRegister,
                                    targetRegister,
                                    currentInstruction);
  ...
  // the target reg is blocked so we need to guarantee it stays in a reg
    ...
    self()->cg()->traceRegisterAssignment(
      "target %R is blocked, assigned to %R",
      targetRegister,
      currentTargetVirtual);

    spareReg = self()->findBestFreeRegister(
      currentInstruction, rk, currentTargetVirtual);
    ...
    if (spareReg == NULL) {
      ...
      spareReg = self()->freeBestRegister(currentInstruction,
                                          currentTargetVirtual,
      ...
      if (!self()->isAssignable(currentTargetVirtual,
                                currentAssignedRegister)) {
        {
          ...
          cursor = self()->registerCopy(
            self()->cg(), rk, targetRegister, spareReg, currentInstruction);
          cursor = self()->registerCopy(self()->cg(),
                                        rk,
                                        currentAssignedRegister,
                                        targetRegister,
        ...
        cursor = self()->registerExchange(self()->cg(),
                                          rk,
                                          targetRegister,
                                          currentAssignedRegister,
                                          spareReg,
  ...
  // the target reg is assigned
    ...
    currentTargetVirtual = targetRegister->getAssignedRegister();
    self()->cg()->traceRegisterAssignment(
      "target %R is assigned, assigned to %R",
      targetRegister,
      currentTargetVirtual);
  ...
  if (targetRegister->getState() != TR::RealRegister::Locked) {
    ...
    traceMsg(comp,
             "    WARNING: Assigning a Locked register %s to %s\n",
             getRegisterName(targetRegister, self()->cg()),
...
OMR::Z::Machine::initializeRegisterFile()
{
  ...
  _registerFile[TR::RealRegister::GPR0] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR0,
                     TR::RealRegister::GPR0Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR1] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR1,
                     TR::RealRegister::GPR1Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR2] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR2,
                     TR::RealRegister::GPR2Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR3] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR3,
                     TR::RealRegister::GPR3Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR4] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR4,
                     TR::RealRegister::GPR4Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR5] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR5,
                     TR::RealRegister::GPR5Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR7] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR7,
                     TR::RealRegister::GPR7Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR8] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR8,
                     TR::RealRegister::GPR8Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR13] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR13,
                     TR::RealRegister::GPR13Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR14] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR14,
                     TR::RealRegister::GPR14Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::GPR15] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::GPR15,
                     TR::RealRegister::GPR15Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR0] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR0,
                     TR::RealRegister::FPR0Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR1] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR1,
                     TR::RealRegister::FPR1Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR2] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR2,
                     TR::RealRegister::FPR2Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR3] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR3,
                     TR::RealRegister::FPR3Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR4] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR4,
                     TR::RealRegister::FPR4Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR5] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR5,
                     TR::RealRegister::FPR5Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR6] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR6,
                     TR::RealRegister::FPR6Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR7] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR7,
                     TR::RealRegister::FPR7Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR8] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR8,
                     TR::RealRegister::FPR8Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR9] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR9,
                     TR::RealRegister::FPR9Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR10] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR10,
                     TR::RealRegister::FPR10Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR11] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR11,
                     TR::RealRegister::FPR11Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR12] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR12,
                     TR::RealRegister::FPR12Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR13] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR13,
                     TR::RealRegister::FPR13Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR14] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR14,
                     TR::RealRegister::FPR14Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::FPR15] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::FPR15,
                     TR::RealRegister::FPR15Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF16] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF16,
                     TR::RealRegister::VRF16Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF17] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF17,
                     TR::RealRegister::VRF17Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF18] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF18,
                     TR::RealRegister::VRF18Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF19] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF19,
                     TR::RealRegister::VRF19Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF20] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF20,
                     TR::RealRegister::VRF20Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF21] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF21,
                     TR::RealRegister::VRF21Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF22] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF22,
                     TR::RealRegister::VRF22Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF23] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF23,
                     TR::RealRegister::VRF23Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF24] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF24,
                     TR::RealRegister::VRF24Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF25] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF25,
                     TR::RealRegister::VRF25Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF26] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF26,
                     TR::RealRegister::VRF26Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF27] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF27,
                     TR::RealRegister::VRF27Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF28] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF28,
                     TR::RealRegister::VRF28Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF29] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF29,
                     TR::RealRegister::VRF29Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF30] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF30,
                     TR::RealRegister::VRF30Mask,
                     self()->cg());
  ...
  _registerFile[TR::RealRegister::VRF31] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF,
                     0,
                     TR::RealRegister::Free,
                     TR::RealRegister::VRF31,
                     TR::RealRegister::VRF31Mask,
                     self()->cg());
...
int32_t
OMR::Z::Machine::addGlobalReg(TR::RealRegister::RegNum reg, int32_t tableIndex)
{
...
int32_t
OMR::Z::Machine::getGlobalReg(TR::RealRegister::RegNum reg)
{
...
int32_t
OMR::Z::Machine::addGlobalRegLater(TR::RealRegister::RegNum reg,
                                   int32_t tableIndex)
{
  if (reg == TR::RealRegister::NoReg)
    return tableIndex;
...
#define addToGRAMap(regNum)                                                    \
  ...
  if (traceVectorGRN) {
    printf("Java func: %s func: %s\n",
           self()->cg()->comp()->getCurrentMethod()->nameChars(),
           __FUNCTION__);
...
OMR::Z::Machine::createRegisterAssociationDirective(TR::Instruction* cursor)
{
  ...
  TR::Instruction* cursor1 =
    new (self()->cg()->trHeapMemory(), TR_MemoryBase::S390Instruction)
      TR::Instruction(
        cursor, TR::InstOpCode::ASSOCREGS, associations, self()->cg());
...
{
  for (int32_t i = TR::RealRegister::FirstGPR;
       i < TR::RealRegister::NumRegisters;
       i++) {
...
OMR::Z::Machine::createDepCondForLiveGPRs(
