  ...
  TR::RealRegister* sourceReg, TR::Instruction* precedingInstruction)
{
  ...
  case TR_GPR: {
    ...
    auto nonNullAssignedReg
      = targetReg->getAssignedRegister() != NULL ? targetReg->getAssignedRegister() : sourceReg->getAssignedRegister();
  ...
  TR::Instruction* precedingInstruction)
{
  ...
  // exchange vector registers
    ...
    if (middleReg != NULL) {
      ...
      currentInstruction
        = generateVRXInstruction(cg, TR::InstOpCode::VST, currentNode, targetReg, tempMR, 0, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      currentInstruction
        = generateVRRaInstruction(cg, TR::InstOpCode::VLR, currentNode, targetReg, sourceReg, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
      TR::MemoryReference* tempMR2 = generateS390MemoryReference(*tempMR, 0, cg);
      currentInstruction
        = generateVRXInstruction(cg, TR::InstOpCode::VL, currentNode, sourceReg, tempMR2, 0, precedingInstruction);
      cg->traceRAInstruction(currentInstruction);
    ...
    TR_ASSERT_FATAL(targetReg->getAssignedRegister()->is64BitReg() == sourceReg->getAssignedRegister()->is64BitReg(),
      "Attempting register exchange with one 64-bit register (%s) and one 32-bit register (%s)",
      getRegisterName(sourceReg, cg), getRegisterName(targetReg, cg));
  ...
  TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
  ...
  if (assignedRegister == NULL) {
    if (self()->cg()->insideInternalControlFlow()) {
      TR_ASSERT_FATAL(
        false, "Attempting to assign a register (%s) inside ICF", getRegisterName(targetRegister, self()->cg()));
    }
  }
  if (kindOfRegister != TR_FPR && kindOfRegister != TR_VRF && assignedRegister != NULL) {
    ...
    if ((assignedRegister->getRealRegisterMask() & availRegMask) == 0) {
      ...
      TR::Instruction* cursor = self()->registerCopy(
        self()->cg(), kindOfRegister, toRealRegister(assignedRegister), newAssignedRegister, appendInst);
  ...
  if (assignedRegister == NULL) {
    ...
    if (targetRegister->getTotalUseCount() != targetRegister->getFutureUseCount()) {
      ...
      if ((assignedRegister = self()->findBestFreeRegister(currInst, kindOfRegister, targetRegister, availRegMask))
        == NULL) {
  ...
  if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF)
    ...
    if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
      ...
      self()->coerceRegisterAssignment(
        currInst, firstReg, (TR::RealRegister::RegNum)(toRealRegister(freeRegisterLow)->getRegisterNumber() - 1));
      freeRegisterHigh
        = self()->getRealRegister((TR::RealRegister::RegNum)(toRealRegister(freeRegisterLow)->getRegisterNumber() - 1));
      ...
      self()->coerceRegisterAssignment(
        currInst, lastReg, (TR::RealRegister::RegNum)(toRealRegister(freeRegisterHigh)->getRegisterNumber() + 1));
  ...
  TR::RealRegister** firstReg, TR::RealRegister** lastReg, TR::Instruction* currInst, uint64_t availRegMask)
{
  ...
  if (bestVirtCandidateLow != NULL) {
    ...
    cursor
      = generateRXInstruction(self()->cg(), TR::InstOpCode::LD, currentNode, bestCandidateLow, tempMRLow, currInst);
  ...
  if (bestVirtCandidateHigh != NULL) {
    ...
    cursor
      = generateRXInstruction(self()->cg(), TR::InstOpCode::LD, currentNode, bestCandidateHigh, tempMRHigh, currInst);
    if (!cursor->assignFreeRegBitVector()) {
      cursor->assignBestSpillRegister();
    }
  ...
  TR::Instruction* currInst, uint64_t availRegMask)
{
  ...
  if (bestVirtCandidateLow != NULL) {
    ...
    if (bestVirtCandidateLow->is64BitReg()) {
      cursor
        = generateRXInstruction(self()->cg(), TR::InstOpCode::LG, currentNode, bestCandidateLow, tempMRLow, currInst);
    } else {
      cursor
        = generateRXInstruction(self()->cg(), TR::InstOpCode::L, currentNode, bestCandidateLow, tempMRLow, currInst);
    }
  ...
  if (bestVirtCandidateHigh != NULL) {
    ...
    if (bestVirtCandidateHigh->is64BitReg()) {
      cursor
        = generateRXInstruction(self()->cg(), TR::InstOpCode::LG, currentNode, bestCandidateHigh, tempMRHigh, currInst);
    } else {
      cursor
        = generateRXInstruction(self()->cg(), TR::InstOpCode::L, currentNode, bestCandidateHigh, tempMRHigh, currInst);
    }
  ...
  TR::Instruction* currentInstruction, TR_RegisterKinds rk, TR::Register* virtualReg, uint64_t availRegMask)
{
  ...
  if (self()->cg()->enableRegisterPairAssociation() && preference == TR::RealRegister::LegalEvenOfPair) {
    ...
    if ((virtualReg->getSiblingRegister()) && (realSibling = virtualReg->getSiblingRegister()->getAssignedRegister())
    ...
    if ((_registerFile[i]->getState() == TR::RealRegister::Free
      ...
      if (comp->getOption(TR_Randomize)) {
        ...
        if (performTransformation(comp,
              "O^O Random Codegen - Randomizing Weight for %s, Original bestWeightSoFar: %x randomized to: %x\n",
...
void OMR::Z::Machine::spillAllVolatileHighRegisters(TR::Instruction* currentInstruction)
{
  ...
  for (int32_t i = first; i <= last; i++) {
    ...
    if (volatileReg && realReg->getState() == TR::RealRegister::Assigned && (virtReg = realReg->getAssignedRegister())
      && virtReg->is64BitReg()) {
...
void OMR::Z::Machine::spillRegister(TR::Instruction* currentInstruction, TR::Register* virtReg)
{
  ...
  switch (rk) {
  case TR_GPR:
    if (!comp->getOption(TR_DisableOOL) && (self()->cg()->isOutOfLineColdPath() || self()->cg()->isOutOfLineHotPath())
  ...
  case TR_FPR:
    ...
    break;
  case TR_VRF:
    // Spill of size 16 has never been done before. The call hierarchy seems to support it but this should be watched
    // closely.
  ...
  TR::Instruction* currentInstruction, TR::Register* virtualRegister, TR::RealRegister::RegNum registerNumber)
{
  ...
  // the target reg is blocked so we need to guarantee it stays in a reg
    ...
    if (spareReg == NULL) {
      ...
      spareReg
        = self()->freeBestRegister(currentInstruction, currentTargetVirtual, currentTargetVirtual->getKind(), true);
      ...
      if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister) || (rk != TR_FPR && rk != TR_VRF)) {
        ...
        if (spareReg == NULL) {
          ...
          if (virtualRegister->is64BitReg()) {
            ...
            spareReg
              = self()->freeBestRegister(currentInstruction, currentTargetVirtual, currentTargetVirtual->getKind());
          } else {
            spareReg = self()->freeBestRegister(
              currentInstruction, currentTargetVirtual, currentTargetVirtual->getKind(), true);
          }

          // For some reason (blocked/locked regs etc), we couldn't find a spare reg so spill the virtual in the target
          // and use it for coercion
          if (spareReg == NULL) {
    ...
    for (int32_t i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i++) {
      ...
      if (linkage->getPreserved(regNum)) {
        if (regNum != linkage->getLitPoolRegister() && regNum != linkage->getStaticBaseRegister()
          && regNum != linkage->getPrivateStaticBaseRegister() && regNum != linkage->getStackPointerRegister()) {
          p = self()->addGlobalReg(regNum, p);
        }
      }
    }
...
uint32_t OMR::Z::Machine::initGlobalVectorRegisterMap(uint32_t vectorOffset)
{
  ...
  // Colouring will make sure to map global virtual FPRs only to the first 16 and vector global virtuals can map to all
  // 32 registers.
...
void OMR::Z::Machine::createRegisterAssociationDirective(TR::Instruction* cursor)
{
  ...
  TR::Instruction* cursor1 = new (self()->cg()->trHeapMemory(), TR_MemoryBase::S390Instruction)
    TR::Instruction(cursor, TR::InstOpCode::ASSOCREGS, associations, self()->cg());
  ...
  for (int32_t i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters; i++) {
    ...
    if (_registerFile[i]->getState() == TR::RealRegister::Free) {
      ...
      if (_registerFile[i]->getAssignedRegister() != NULL
        ...
        //  When we handled _registerFile[12], we would have updated the assignment of GPR_3545 (currently to GPR15) to
        //  GPR12. When we subsequently handle _registerFile[15], we cannot blindly reset GPR_3545's assigned register
        //  to NULL, as that will incorrectly break the assignment to GPR12.
        if (_registerFile[i]->getAssignedRegister()->getAssignedRegister() == _registerFile[i]) {
          // clear the Virt -> Real reg assignment for any newly assigned virtual register (due to spills) in the hot
          // path
          _registerFile[i]->getAssignedRegister()->setAssignedRegister(NULL);
        }
      }
