...
TR::S390zOSSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
  ...
  cursor = generateS390LabelInstruction(
    cg(), TR::InstOpCode::LABEL, node, generateLabelSymbol(cg()), cursor);
  ...
  if (TR::Compiler->target.is64Bit()) {
    cursor = generateRXInstruction(
      ...
      InstOpCode::BC,
      node,
      getRealRegister(TR::RealRegister::GPR15),
...
void
TR::S390zOSSystemLinkage::createPrologue(TR::Instruction* cursor)
{
  ...
  size_t localSize =
    ...
    ~(8 - 1);
  setStackFrameSize(
    (((getOffsetToFirstParm() + argSize + localSize) + (32 - 1)) & ~(32 - 1)) -
    XPLINK_STACK_FRAME_BIAS);

  int32_t stackFrameSize = getStackFrameSize();

  TR_ASSERT_FATAL((stackFrameSize & 31) == 0,
                  "Misaligned stack frame size (%d) detected",
                  stackFrameSize);
  ...
  cursor = generateS390LabelInstruction(
    cg, InstOpCode::LABEL, node, _entryPointMarkerLabel, cursor);

  // "C.E.E.1."
  cursor = generateDataConstantInstruction(
    cg, TR::InstOpCode::DC, node, 0x00C300C5, cursor);
  cursor = generateDataConstantInstruction(
    cg, TR::InstOpCode::DC, node, 0x00C500F1, cursor);
  cursor = generateDataConstantInstruction(
    cg, TR::InstOpCode::DC, node, 0x00000000, cursor);
  ...
  // alloca()
  cursor = generateDataConstantInstruction(
    cg, TR::InstOpCode::DC, node, stackFrameSize, cursor);
 ...
 */
TR::Register*
TR::S390zOSSystemLinkage::callNativeFunction(
  TR::Node* callNode,
  ...
  bool isJNIGCPoint)
{
  ...
  /*****************************/
  generateInstructionsForCall(callNode,
                              deps,
                              targetAddress,
                              methodAddressReg,
                              javaLitOffsetReg,
  ...
  switch (callNode->getOpCodeValue()) {
    ...
    case TR::lcall: {
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateRSInstruction(
          codeGen, TR::InstOpCode::SLLG, callNode, highReg, highReg, 32);
        cursor = generateRRInstruction(
          codeGen, TR::InstOpCode::LR, callNode, highReg, lowReg);
    ...
    default:
      ...
      returnRegister = retReg;
      TR_ASSERT(
        0, "Unknown direct call Opcode %d.", callNode->getOpCodeValue());
  }

  if (returnRegister != retReg) {
    generateRRInstruction(codeGen,
...
TR::S390zOSSystemLinkage::getRegisterSaveOffset(TR::RealRegister::RegNum srcReg)
{
  ...
  if ((srcReg >= TR::RealRegister::GPR4) &&
    ...
    int32_t offset =
      2048 + (srcReg - TR::RealRegister::GPR4) * cg()->machine()->getGPRSize();
    return offset;
  } else {
    TR_ASSERT(false,
...
void
TR::S390zOSSystemLinkage::generateInstructionsForCall(
  TR::Node* callNode,
  TR::RegisterDependencyConditions* deps,
  intptrj_t targetAddress,
  TR::Register* methodAddressReg,
  ...
  bool isJNIGCPoint)
{
  ...
  if (callNode->getOpCode().isIndirect()) {
    ...
    TR::Instruction* callInstr = new (cg()->trHeapMemory())
      TR::S390RILInstruction(TR::InstOpCode::BRASL,
                             callNode,
                             systemReturnAddressRegister,
                             callSymbol,
                             callSymRef,
  ...
  generateS390LabelInstruction(
    codeGen, InstOpCode::LABEL, callNode, returnFromJNICallLabel);
  ...
  TR::RegisterDependencyConditions* postDeps =
    new (trHeapMemory()) TR::RegisterDependencyConditions(
      NULL, deps->getPostConditions(), 0, deps->getAddCursorForPost(), cg());
  generateS390LabelInstruction(
    codeGen, InstOpCode::LABEL, callNode, afterNOP, postDeps);
}
TR::LabelSymbol*
                                                  ...
                                                  TR_XPLinkCallTypes callType)
{
  if (TR::Compiler->target.is32Bit()) {
    ...
    cursor = generateDataConstantInstruction(
      cg(), TR::InstOpCode::DC, node, nopDescriptor, cursor);
...
TR::Instruction*
TR::S390zOSSystemLinkage::addImmediateToRealRegister(
  ...
  if (smallPositiveValue) {
    if (!checkTempNeeded)
      cursor = generateRXInstruction(
        ...
        node,
        targetReg,
        generateS390MemoryReference(targetReg, value, cg()),
        cursor);
  } else if (largeValue) {
    ...
    if (checkTempNeeded)
      *checkTempNeeded = true;
      ...
      cursor = generateS390ImmToRegister(
        cg(), node, tempReg, (intptr_t)(value), cursor);
                                             ...
                                             TR::Instruction* cursor)
{
  ...
  if (firstSaved >= 0) {
    ...
    if (firstSaved == lastSaved) {
      ...
      cursor =
        restorePreservedRegs(REGNUM(firstSaved + TR::RealRegister::FirstGPR),
  ...
  if (!(GPRSaveMask &
        (1 << GPREGINDEX(
           getNormalStackPointerRegister())))) { // GPR4 not restored by
                                                 // previous and needs restoring
    cursor = addImmediateToRealRegister(
      spReg, stackFrameSize, NULL, currentNode, cursor);
                                             ...
                                             TR::Instruction* cursor)
{
  ...
  for (int32_t i = TR::Linkage::getFirstMaskedBit(FPRSaveMask);
       i <= TR::Linkage::getLastMaskedBit(FPRSaveMask);
       ++i) {
    if (FPRSaveMask & (1 << (i))) {
      ...
      cursor = generateRXInstruction(
        ...
        TR::InstOpCode::LD,
        node,
        getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)),
        fillMemRef,
        cursor);
                                              ...
                                              TR::Instruction* cursor)
{
  ...
  addImmediateToRealRegister(
    NULL, stackFrameSize, NULL, NULL, NULL, &needAddTempReg);
  ...
  if (comp()->getOption(TR_TraceCG)) {
    traceMsg(
      comp(), "GPRSaveMask: Register context %x\n", GPRSaveMask & 0xffff);
  }
  ...
  switch (frameType) {
    case TR_XPLinkSmallFrame:
      ...
      cursor = generateRSInstruction(
        cg(),
        TR::InstOpCode::getStoreMultipleOpCode(),
        node,
        getRealRegister(REGNUM(firstSaved + TR::RealRegister::FirstGPR)),
        getRealRegister(REGNUM(lastSaved + TR::RealRegister::FirstGPR)),
        rsa,
        cursor);

      _stackPointerUpdateLabel = generateLabelSymbol(cg());
      cursor = generateS390LabelInstruction(
        cg(), InstOpCode::LABEL, node, _stackPointerUpdateLabel, cursor);

      // AHI R4,-stackFrameSize
      cursor = addImmediateToRealRegister(
        spReg, (stackFrameSize) * -1, NULL, node, cursor);
      break;
    ...
    case TR_XPLinkIntermediateFrame:
      if (needAddTempReg) {
        ...
        cursor = addImmediateToRealRegister(spReg,
                                            (stackFrameSize) * -1,
                                            gpr3Real,
                                            node,
                                            cursor); // R4 <- R4 - stack size
      ...
      if (needAddTempReg) {                  // ST R0,2048(R4)
        ...
        rsa = generateS390MemoryReference(spReg, rsaOffset, cg());
        cursor = generateRXInstruction(
          cg(), TR::InstOpCode::getStoreOpCode(), node, gpr0Real, rsa, cursor);
      }
      break;
    ...
    case TR_XPLinkStackCheckFrame:
      ...
      if (needAddTempReg) // LR R0,R4
        cursor = generateRRInstruction(cg(),
                                       ...
                                       cursor);
      cursor = addImmediateToRealRegister(
        spReg, (stackFrameSize) * -1, gpr3Real, node, cursor); // AHI ...
      ...
      if (TR::Compiler->target.is64Bit()) {
        ...
        cursor = generateRXInstruction(
          cg(), InstOpCode::LLGT, node, gpr3Real, laaRef, cursor);
      ...
      cursor = generateRRInstruction(
        cg(), InstOpCode::BASR, node, gpr3Real, gpr3Real, cursor);
      cursor =
      ...
      // STM rx,ry,disp(R4)
      cursor = generateS390LabelInstruction(
        cg(), InstOpCode::LABEL, node, stmLabel, cursor);
...
void
TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::apply(
  TR::CodeGenerator* cg)
{
