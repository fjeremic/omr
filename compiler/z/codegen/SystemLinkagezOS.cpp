...
void TR::S390zOSSystemLinkage::createPrologue(TR::Instruction* cursor)
{
  ...
  setStackFrameSize(
    (((getOffsetToFirstParm() + argSize + localSize) + (32 - 1)) & ~(32 - 1)) - XPLINK_STACK_FRAME_BIAS);
  ...
  cg->addRelocation(
    new (cg->trHeapMemory()) InstructionLabelRelative32BitRelocation(cursor, -8, _ppa1Snippet->getSnippetLabel(), 1));
  ...
  TR::LabelSymbol* returnFromJNICallLabel, TR::S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
  ...
  /*****************************/
  generateInstructionsForCall(callNode, deps, targetAddress, methodAddressReg, javaLitOffsetReg, returnFromJNICallLabel,
  ...
  case TR::lcall: {
    if (TR::Compiler->target.is64Bit()) {
      ...
      cursor = generateRRInstruction(codeGen, TR::InstOpCode::LR, callNode, highReg, lowReg);
...
{
  return TR::Compiler->target.is64Bit() ? TR::RealRegister::NoReg : TR::RealRegister::GPR12;
}
  ...
  TR::LabelSymbol* returnFromJNICallLabel, TR::S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
  ...
  if (callNode->getOpCode().isIndirect()) {
    generateRRInstruction(
      cg(), InstOpCode::BASR, callNode, systemReturnAddressRegister, systemEntryPointRegister, preDeps);
    ...
    TR::Instruction* callInstr = new (cg()->trHeapMemory()) TR::S390RILInstruction(
      TR::InstOpCode::BRASL, callNode, systemReturnAddressRegister, callSymbol, callSymRef, cg());
  ...
  TR::Instruction* cursor, TR::Node* node, TR::Node* callNode, TR_XPLinkCallTypes callType)
{
  if (TR::Compiler->target.is32Bit()) {
    ...
    TR::S390ConstantDataSnippet* callDescriptor
      = new (self()->trHeapMemory()) TR::XPLINKCallDescriptorSnippet(cg(), this, callDescriptorValue);
    ...
    cg()->addRelocation(
      new (cg()->trHeapMemory()) XPLINKCallDescriptorRelocation(cursor, callDescriptor->getSnippetLabel()));
  } else {
  ...
  TR::RealRegister* tempReg, TR::Node* node, TR::Instruction* cursor, bool* checkTempNeeded)
{
  ...
  if (smallPositiveValue) {
    ...
    if (!checkTempNeeded)
      cursor = generateRXInstruction(
        cg(), TR::InstOpCode::LAY, node, targetReg, generateS390MemoryReference(targetReg, value, cg()), cursor);
  }
...
TR::Instruction* TR::S390zOSSystemLinkage::fillGPRsInEpilogue(TR::Node* node, TR::Instruction* cursor)
{
  ...
  if (!(GPRSaveMask
        & (1 << GPREGINDEX(getNormalStackPointerRegister())))) { // GPR4 not restored by previous and needs restoring
...
TR::Instruction* TR::S390zOSSystemLinkage::fillFPRsInEpilogue(TR::Node* node, TR::Instruction* cursor)
{
  ...
  for (int32_t i = TR::Linkage::getFirstMaskedBit(FPRSaveMask); i <= TR::Linkage::getLastMaskedBit(FPRSaveMask); ++i) {
    if (FPRSaveMask & (1 << (i))) {
      ...
      cursor = generateRXInstruction(
        cg(), TR::InstOpCode::LD, node, getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), fillMemRef, cursor);
...
TR::Instruction* TR::S390zOSSystemLinkage::spillGPRsInPrologue(TR::Node* node, TR::Instruction* cursor)
{
  ...
  if ((frameType == TR_XPLinkStackCheckFrame)
    || ((frameType == TR_XPLinkIntermediateFrame)
      && needAddTempReg)) { // GPR4 is saved but indirectly via GPR0 in stack check or medium case - so don't save GPR4
                            // via STM
  ...
  case TR_XPLinkIntermediateFrame:
    if (needAddTempReg) {
      ...
      if (saveTempReg || needAddTempReg) {
        cursor
          = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real, spReg, cursor); // LR R0,R4
      }
      if (saveTempReg) {
      ...
      cursor
        = addImmediateToRealRegister(spReg, (stackFrameSize) * -1, gpr3Real, node, cursor); // R4 <- R4 - stack size
  ...
  case TR_XPLinkStackCheckFrame:
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      cursor
        = generateRXInstruction(cg(), TR::InstOpCode::getCmpOpCode(), node, spReg, bosRef, cursor); // C R4,bos(tempreg)
    } else {
      bosOffset = 868; // 868=offset in CAA to BOS
      bosRef = generateS390MemoryReference(caaReal, bosOffset, cg());
      cursor
        = generateRXInstruction(cg(), TR::InstOpCode::getCmpOpCode(), node, spReg, bosRef, cursor); // C R4,bos(R12)
    }
    ...
    if (!needAddTempReg) {
      cursor
        = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real, gpr3Real, cursor); // LR R0,R3
    }
...
TR::Instruction* TR::S390zOSSystemLinkage::spillFPRsInPrologue(TR::Node* node, TR::Instruction* cursor)
{
  ...
  for (int32_t i = TR::Linkage::getFirstMaskedBit(FPRSaveMask); i <= TR::Linkage::getLastMaskedBit(FPRSaveMask); ++i) {
    if (FPRSaveMask & (1 << (i))) {
      ...
      cursor = generateRXInstruction(
        cg(), TR::InstOpCode::STD, node, getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), spillMemRef, cursor);
