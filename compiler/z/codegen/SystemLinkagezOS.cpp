...
TR::S390zOSSystemLinkage::S390zOSSystemLinkage(TR::CodeGenerator *cg)
    : TR::SystemLinkage(cg, TR_SystemXPLink), _entryPointMarkerLabel(NULL),
      _stackPointerUpdateLabel(NULL), _ppa1Snippet(NULL), _ppa2Snippet(NULL) {
...
void TR::S390zOSSystemLinkage::generateInstructionsForCall(
  ...
  TR::RegisterDependencyConditions *preDeps = new (trHeapMemory())
      TR::RegisterDependencyConditions(deps->getPreConditions(), NULL,
...
TR::Instruction *
TR::S390zOSSystemLinkage::fillGPRsInEpilogue(TR::Node *node,
...
TR::Instruction *
TR::S390zOSSystemLinkage::fillFPRsInEpilogue(TR::Node *node,
                                             TR::Instruction *cursor) {
...
TR::Instruction *
TR::S390zOSSystemLinkage::spillGPRsInPrologue(TR::Node *node,
                                              TR::Instruction *cursor) {
  ...
  case TR_XPLinkIntermediateFrame:
    if (needAddTempReg) {
      ...
      saveTempReg = true; // PERFORMANCE NOTE: TODO: set to false if GPR3 is not
                          // used as a parameter
  ...
  case TR_XPLinkStackCheckFrame:
    ...
    if (needAddTempReg) // LR R0,R4
      cursor = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(),
                                     node, gpr0Real, spReg, cursor);
    cursor = addImmediateToRealRegister(spReg, (stackFrameSize) * -1, gpr3Real,
                                        node, cursor); // AHI ...
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      cursor = generateRXInstruction(cg(), TR::InstOpCode::getCmpOpCode(), node,
                                     spReg, bosRef, cursor); // C R4,bos(R12)
    }
    cursor = generateS390BranchInstruction(cg(), InstOpCode::BRC,
                                           InstOpCode::COND_BNM, node, stmLabel,
                                           cursor); // BNM stmLabel
    ...
    if (!needAddTempReg) {
      cursor =
          generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node,
                                gpr0Real, gpr3Real, cursor); // LR R0,R3
    }
...
TR::Instruction *
TR::S390zOSSystemLinkage::spillFPRsInPrologue(TR::Node *node,
                                              TR::Instruction *cursor) {
  TR::RealRegister *spReg = getNormalStackPointerRealRegister();
  int32_t offset = getFPRSaveAreaEndOffset();
  int16_t FPRSaveMask = getFPRSaveMask();
...
void TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::apply(
  ...
  *reinterpret_cast<int16_t *>(p) =
      static_cast<int16_t>(offsetToCallDescriptor);
}
