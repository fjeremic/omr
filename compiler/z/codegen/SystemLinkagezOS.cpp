...
void TR::S390zOSSystemLinkage::createEpilogue(TR::Instruction* cursor) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    cursor = generateRXInstruction(
        cg(), InstOpCode::BC, node, getRealRegister(TR::RealRegister::GPR15),
        generateS390MemoryReference(getReturnAddressRealRegister(), 2, cg()),
        cursor);
  } else {
    cursor = generateRXInstruction(
        cg(), InstOpCode::BC, node, getRealRegister(TR::RealRegister::GPR15),
        generateS390MemoryReference(getReturnAddressRealRegister(), 4, cg()),
        cursor);
  }
}
...
void TR::S390zOSSystemLinkage::createPrologue(TR::Instruction* cursor) {
  ...
  // Calculate size of locals determined in prior call to `mapStack`. We make
  // the size a multiple of the strictest alignment symbol so that backwards
  // mapping of auto symbols will follow the alignment.
  //
  // TODO: We should be using OMR::align here once mapStack is fixed so we don't
  // pass negative offsets
  size_t localSize =
      ((-1 * static_cast<int32_t>(bodySymbol->getLocalMappingCursor())) +
       (8 - 1)) &
      ~(8 - 1);
  setStackFrameSize(
      (((getOffsetToFirstParm() + argSize + localSize) + (32 - 1)) &
       ~(32 - 1)) -
      XPLINK_STACK_FRAME_BIAS);

  int32_t stackFrameSize = getStackFrameSize();

  TR_ASSERT_FATAL((stackFrameSize & 31) == 0,
                  "Misaligned stack frame size (%d) detected", stackFrameSize);
  ...
  cursor = generateS390LabelInstruction(cg, InstOpCode::LABEL, node,
                                        _entryPointMarkerLabel, cursor);

  // "C.E.E.1."
  cursor = generateDataConstantInstruction(cg, TR::InstOpCode::DC, node,
                                           0x00C300C5, cursor);
  cursor = generateDataConstantInstruction(cg, TR::InstOpCode::DC, node,
                                           0x00C500F1, cursor);
  cursor = generateDataConstantInstruction(cg, TR::InstOpCode::DC, node,
  ...
  cg->addRelocation(new (cg->trHeapMemory())
                        InstructionLabelRelative32BitRelocation(
                            cursor, -8, _ppa1Snippet->getSnippetLabel(), 1));

  // DSA size is the frame size aligned to 32-bytes which means it's least
  // significant 5 bits are zero and are used to represent the flags which are
  // always 0 for OMR as we do not support leaf frames or direct calls to
  // alloca()
  cursor = generateDataConstantInstruction(cg, TR::InstOpCode::DC, node,
                                           stackFrameSize, cursor);
...
int32_t TR::S390zOSSystemLinkage::getOutgoingParameterBlockSize() {
  ...
  if (argAreaSize) {
    ...
    argAreaSize =
        (argAreaSize < minimalArgAreaSize) ? minimalArgAreaSize : argAreaSize;
  }
  return argAreaSize;
}
 ...
 * return value will be return value from system routine copied to private
 * linkage return reg
 */
TR::Register* TR::S390zOSSystemLinkage::callNativeFunction(
  ...
  TR::Register *Real_highReg = NULL, *Real_lowReg = NULL, *Img_highReg = NULL,
               *Img_lowReg = NULL, *Real = NULL, *Imaginary = NULL;
  switch (callNode->getOpCodeValue()) {
    case TR::acall:
    case TR::acalli:
      retReg = deps->searchPostConditionRegister(getIntegerReturnRegister());
      returnRegister =
          deps->searchPostConditionRegister(getIntegerReturnRegister());
      if (TR::Compiler->target.is64Bit() && returnRegister &&
          !returnRegister->is64BitReg()) {
    ...
    case TR::lcall: {
      if (TR::Compiler->target.is64Bit()) {
        ...
        highReg =
            deps->searchPostConditionRegister(getLongHighReturnRegister());

        generateRSInstruction(codeGen, TR::InstOpCode::SLLG, callNode, highReg,
                              highReg, 32);
        cursor = generateRRInstruction(codeGen, TR::InstOpCode::LR, callNode,
                                       highReg, lowReg);
    ...
    case TR::decalli:
      highReg =
          deps->searchPostConditionRegister(getLongDoubleReturnRegister0());
      lowReg =
          deps->searchPostConditionRegister(getLongDoubleReturnRegister2());
    ...
    default:
      retReg = NULL;
      returnRegister = retReg;
      TR_ASSERT(0, "Unknown direct call Opcode %d.",
                callNode->getOpCodeValue());
  }
...
TR::RealRegister::RegNum TR::S390zOSSystemLinkage::getENVPointerRegister() {
  return TR::RealRegister::GPR5;
}

TR::RealRegister::RegNum TR::S390zOSSystemLinkage::getCAAPointerRegister() {
  return TR::Compiler->target.is64Bit() ? TR::RealRegister::NoReg
                                        : TR::RealRegister::GPR12;
}

int32_t TR::S390zOSSystemLinkage::getRegisterSaveOffset(
    TR::RealRegister::RegNum srcReg) {
  int32_t offset;
  if ((srcReg >= TR::RealRegister::GPR4) &&
      (srcReg <= TR::RealRegister::GPR15)) {
    int32_t offset = 2048 + (srcReg - TR::RealRegister::GPR4) *
                                cg()->machine()->getGPRSize();
    return offset;
  } else {
    TR_ASSERT(false,
              "ERROR: TR::S390zOSSystemLinkage::getRegisterSaveOffset called "
              "for volatile reg: %d\n",
              srcReg);
    return -1;
  }
...
void TR::S390zOSSystemLinkage::generateInstructionsForCall(
    TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg,
    TR::Register* javaLitOffsetReg, TR::LabelSymbol* returnFromJNICallLabel,
    TR::S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint) {
  ...
  //                      BASR R7,R6
  //          where:
  //             a) regfp is a register containing pointer to function
  //             descriptor
  //                (most likely placed in R6)
  //             b) disp is offset into function descriptor - 16/0 (31/64 bit
  //             respectively)
  //   case 3) (direct) Call to (static or global) function defined in the
  //   compilation unit:
  //                      BRASL R7,func
  //
  ...
  //                      BASR R7,R6
  //         where:
  //             a) disp is an offset in the environment (aka ADA) containing
  //             the
  //                function descriptor body (i.e. not pointer to function
  //                descriptor)
  TR_XPLinkCallTypes callType;

  // Find GPR6 (xplink) or GPR15 (os linkage) in post conditions
  TR::Register* systemEntryPointRegister =
      deps->searchPostConditionRegister(getEntryPointRegister());
  ...
  TR::RegisterDependencyConditions* preDeps =
      new (trHeapMemory()) TR::RegisterDependencyConditions(
          deps->getPreConditions(), NULL, deps->getAddCursorForPre(), 0, cg());

  if (callNode->getOpCode().isIndirect()) {
    generateRRInstruction(cg(), InstOpCode::BASR, callNode,
                          systemReturnAddressRegister, systemEntryPointRegister,
                          preDeps);
    ...
    TR::Instruction* callInstr = new (cg()->trHeapMemory())
        TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode,
                               systemReturnAddressRegister, callSymbol,
                               callSymRef, cg());
    callInstr->setDependencyConditions(preDeps);
    callType = TR_XPLinkCallType_BRASL7;
  }

  generateS390LabelInstruction(codeGen, InstOpCode::LABEL, callNode,
  ...
  TR::RegisterDependencyConditions* postDeps = new (trHeapMemory())
      TR::RegisterDependencyConditions(NULL, deps->getPostConditions(), 0,
                                       deps->getAddCursorForPost(), cg());
  generateS390LabelInstruction(codeGen, InstOpCode::LABEL, callNode, afterNOP,
                               postDeps);
}
TR::LabelSymbol* TR::S390zOSSystemLinkage::getEntryPointMarkerLabel() const {
  return _entryPointMarkerLabel;
}

TR::LabelSymbol* TR::S390zOSSystemLinkage::getStackPointerUpdateLabel() const {
  return _stackPointerUpdateLabel;
}

TR::PPA1Snippet* TR::S390zOSSystemLinkage::getPPA1Snippet() const {
  return _ppa1Snippet;
}

TR::PPA2Snippet* TR::S390zOSSystemLinkage::getPPA2Snippet() const {
  return _ppa2Snippet;
}

TR::Instruction* TR::S390zOSSystemLinkage::genCallNOPAndDescriptor(
    TR::Instruction* cursor, TR::Node* node, TR::Node* callNode,
    TR_XPLinkCallTypes callType) {
  if (TR::Compiler->target.is32Bit()) {
    uint32_t callDescriptorValue =
        TR::XPLINKCallDescriptorSnippet::generateCallDescriptorValue(this,
                                                                     callNode);

    TR::S390ConstantDataSnippet* callDescriptor = new (self()->trHeapMemory())
        TR::XPLINKCallDescriptorSnippet(cg(), this, callDescriptorValue);
    cg()->addDataConstantSnippet(callDescriptor);

    uint32_t nopDescriptor =
        0x47000000 | (static_cast<uint32_t>(callType) << 16);
    cursor = generateDataConstantInstruction(cg(), TR::InstOpCode::DC, node,
                                             nopDescriptor, cursor);

    cg()->addRelocation(new (cg()->trHeapMemory())
                            XPLINKCallDescriptorRelocation(
                                cursor, callDescriptor->getSnippetLabel()));
  } else {
    // TODO: Once we support generic TR::InstOpCode::DC of any size we need to
    // modify this line to use it, similarly to what we do above for the 31-bit
    // case.
    uint16_t nopDescriptor = 0x1800 | static_cast<uint16_t>(callType);
    cursor = new (cg()->trHeapMemory()) TR::S390Imm2Instruction(
        TR::InstOpCode::DC2, node, nopDescriptor, cursor, cg());
  }

  return cursor;
}

TR::Instruction* TR::S390zOSSystemLinkage::addImmediateToRealRegister(
    TR::RealRegister* targetReg, int32_t value, TR::RealRegister* tempReg,
    TR::Node* node, TR::Instruction* cursor, bool* checkTempNeeded) {
  ...
  if (smallPositiveValue) {
    ...
    if (checkTempNeeded)
      *checkTempNeeded = true;
      ...
      cursor = generateS390ImmToRegister(cg(), node, tempReg, (intptr_t)(value),
                                         cursor);
      cursor = generateRRInstruction(cg(), TR::InstOpCode::getAddRegOpCode(),
                                     node, targetReg, tempReg, cursor);
    }
  } else {
    if (!checkTempNeeded)
      cursor = generateRXInstruction(
          cg(), TR::InstOpCode::LAY, node, targetReg,
          generateS390MemoryReference(targetReg, value, cg()), cursor);
  }
...
TR::Instruction* TR::S390zOSSystemLinkage::fillGPRsInEpilogue(
    TR::Node* node, TR::Instruction* cursor) {
  int16_t GPRSaveMask;
  ...
  TR::RealRegister* spReg =
      getNormalStackPointerRealRegister();  // normal sp reg used in prol/epil
  stackFrameSize = getStackFrameSize();

  GPRSaveMask = getGPRSaveMask();  // restore mask is subset of save mask
  GPRSaveMask &=
      ~(1 << GPREGINDEX(
            getEntryPointRegister()));  // entry point register not restored
  ...
  if (firstSaved >= 0) {
    offsetToFirstSavedReg = (firstSaved - firstPossibleSaved) *
                            gprSize;  // relative to start of save area
    ...
    if (firstSaved == lastSaved) {
      cursor = generateRXInstruction(
          cg(), TR::InstOpCode::getLoadOpCode(), currentNode,
          getRealRegister(REGNUM(firstSaved + TR::RealRegister::FirstGPR)), rsa,
          cursor);
    } else if (firstSaved < lastSaved) {
      int8_t numNeededToRestore = -1;

      cursor = restorePreservedRegs(
          REGNUM(firstSaved + TR::RealRegister::FirstGPR),
          REGNUM(lastSaved + TR::RealRegister::FirstGPR), blockNumber, cursor,
          currentNode, spReg, rsa, getNormalStackPointerRegister());
    }
  }

  if (!(GPRSaveMask &
        (1 << GPREGINDEX(
             getNormalStackPointerRegister())))) {  // GPR4 not restored by
                                                    // previous and needs
                                                    // restoring
    cursor = addImmediateToRealRegister(spReg, stackFrameSize, NULL,
                                        currentNode, cursor);
    TR_ASSERT(cursor != NULL,
              "xplink retore code - should not need temp register");
  }

  return cursor;
}

TR::Instruction* TR::S390zOSSystemLinkage::fillFPRsInEpilogue(
    TR::Node* node, TR::Instruction* cursor) {
  ...
  for (int32_t i = TR::Linkage::getFirstMaskedBit(FPRSaveMask);
       i <= TR::Linkage::getLastMaskedBit(FPRSaveMask); ++i) {
    if (FPRSaveMask & (1 << (i))) {
      TR::MemoryReference* fillMemRef =
          generateS390MemoryReference(spReg, offset, cg());

      cursor = generateRXInstruction(
          cg(), TR::InstOpCode::LD, node,
          getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), fillMemRef,
          cursor);
...
TR::Instruction* TR::S390zOSSystemLinkage::spillGPRsInPrologue(
    TR::Node* node, TR::Instruction* cursor) {
  ...
  int8_t gprSize = cg()->machine()->getGPRSize();
  int32_t stackFrameSize, offsetToRegSaveArea, offsetToFirstSavedReg,
      gpr3ParmOffset;
  int32_t firstSaved, lastSaved, firstPossibleSaved;

  // As defined by XPLINK specification
  int32_t intermediateThreshold =
      TR::Compiler->target.is64Bit() ? 1024 * 1024 : 4096;
  ...
  //   2) XPLink noleaf:  routines mandates the save of the entry point register
  //   (r6) & return register (r7)
  ...
  addImmediateToRealRegister(NULL, stackFrameSize, NULL, NULL, NULL,
                             &needAddTempReg);
  ...
  if ((frameType == TR_XPLinkStackCheckFrame) ||
      ((frameType == TR_XPLinkIntermediateFrame) &&
       needAddTempReg)) {  // GPR4 is saved but indirectly via GPR0 in stack
                           // check or medium case - so don't save GPR4 via STM
    firstSaved = TR::Linkage::getFirstMaskedBit(
        GPRSaveMask & ~(1 << GPREGINDEX(getNormalStackPointerRegister())));
  } else
    firstSaved = TR::Linkage::getFirstMaskedBit(GPRSaveMask);
  ...
  firstPossibleSaved =
      GPREGINDEX(TR::RealRegister::GPR4);  // GPR4 is first reg in save area
  lastSaved = TR::Linkage::getLastMaskedBit(GPRSaveMask);
  offsetToFirstSavedReg = (firstSaved - firstPossibleSaved) *
                          gprSize;  // relative to start of save area

  // variables for stack extension code - xplink spec mandates these particular
  // registers
  ...
  TR::RealRegister* caaReal =
      getRealRegister(getCAAPointerRegister());  // 31 bit oly
  ...
  switch (frameType) {
    case TR_XPLinkSmallFrame:
      // STM rx,ry,offset(R4)
      rsaOffset =
          (offsetToRegSaveArea - stackFrameSize) + offsetToFirstSavedReg;
      rsa = generateS390MemoryReference(spReg, rsaOffset, cg());
      cursor = generateRSInstruction(
          cg(), TR::InstOpCode::getStoreMultipleOpCode(), node,
          getRealRegister(REGNUM(firstSaved + TR::RealRegister::FirstGPR)),
          getRealRegister(REGNUM(lastSaved + TR::RealRegister::FirstGPR)), rsa,
          cursor);

      _stackPointerUpdateLabel = generateLabelSymbol(cg());
      cursor = generateS390LabelInstruction(cg(), InstOpCode::LABEL, node,
                                            _stackPointerUpdateLabel, cursor);

      // AHI R4,-stackFrameSize
      cursor = addImmediateToRealRegister(spReg, (stackFrameSize) * -1, NULL,
                                          node, cursor);
      break;
    ...
    case TR_XPLinkIntermediateFrame:
      if (needAddTempReg) {
        ...
        saveTempReg = true;  // PERFORMANCE NOTE: TODO: set to false if GPR3 is
                             // not used as a parameter

        if (saveTempReg || needAddTempReg) {
          cursor =
              generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(),
                                    node, gpr0Real, spReg, cursor);  // LR R0,R4
        }
        if (saveTempReg) {
          gpr3ParmRef = generateS390MemoryReference(
              spReg, gpr3ParmOffset, cg());  // used for temporary register
          cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(),
                                         node, gpr3Real, gpr3ParmRef,
                                         cursor);  // ST R3,parmRef(R4)
        }
        ...
        cursor =
            addImmediateToRealRegister(spReg, (stackFrameSize) * -1, gpr3Real,
                                       node, cursor);  // R4 <- R4 - stack size

        if (saveTempReg) {
          cursor =
              generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(),
                                    node, gpr3Real, gpr0Real, cursor);
          gpr3ParmRef =
              generateS390MemoryReference(gpr3Real, gpr3ParmOffset, cg());
          cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(),
                                         node, gpr3Real, gpr3ParmRef, cursor);
        }
    ...
    case TR_XPLinkStackCheckFrame:
      ...
      saveTempReg =
          needAddTempReg;  // TODO: and with check that R3 is used as parm
      ...
      if (needAddTempReg)  // LR R0,R4
        cursor = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(),
                                       node, gpr0Real, spReg, cursor);
      cursor = addImmediateToRealRegister(spReg, (stackFrameSize) * -1,
                                          gpr3Real, node, cursor);  // AHI ...
      ...
      if (TR::Compiler->target.is64Bit()) {
        ...
        cursor = generateRXInstruction(cg(), InstOpCode::LLGT, node, gpr3Real,
                                       laaRef, cursor);
        bosRef = generateS390MemoryReference(gpr3Real, bosOffset, cg());
        cursor =
            generateRXInstruction(cg(), TR::InstOpCode::getCmpOpCode(), node,
                                  spReg, bosRef, cursor);  // C R4,bos(tempreg)
      } else {
        bosOffset = 868;  // 868=offset in CAA to BOS
        bosRef = generateS390MemoryReference(caaReal, bosOffset, cg());
        cursor =
            generateRXInstruction(cg(), TR::InstOpCode::getCmpOpCode(), node,
                                  spReg, bosRef, cursor);  // C R4,bos(R12)
      }
      cursor = generateS390BranchInstruction(cg(), InstOpCode::BRC,
                                             InstOpCode::COND_BNM, node,
                                             stmLabel, cursor);  // BNM stmLabel
      ...
      if (TR::Compiler->target.is64Bit()) {
        ...
        extenderRef =
            generateS390MemoryReference(gpr3Real, extenderOffset, cg());
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(),
                                       node, gpr3Real, extenderRef,
                                       cursor);  //  L R3,extender(R12)
      } else {
        extenderOffset = 872;  // offset of extender routine in CAA
        extenderRef =
            generateS390MemoryReference(caaReal, extenderOffset, cg());
        cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(),
                                       node, gpr3Real, extenderRef,
                                       cursor);  //  L R3,extender(R12)
      }
      cursor = generateRRInstruction(cg(), InstOpCode::BASR, node, gpr3Real,
                                     gpr3Real, cursor);
      cursor =
          genCallNOPAndDescriptor(cursor, node, NULL, TR_XPLinkCallType_BASR33);
...
TR::Instruction* TR::S390zOSSystemLinkage::spillFPRsInPrologue(
    TR::Node* node, TR::Instruction* cursor) {
  TR::RealRegister* spReg = getNormalStackPointerRealRegister();
  int32_t offset = getFPRSaveAreaEndOffset();
  int16_t FPRSaveMask = getFPRSaveMask();
  ...
  for (int32_t i = TR::Linkage::getFirstMaskedBit(FPRSaveMask);
       i <= TR::Linkage::getLastMaskedBit(FPRSaveMask); ++i) {
    if (FPRSaveMask & (1 << (i))) {
      TR::MemoryReference* spillMemRef =
          generateS390MemoryReference(spReg, offset, cg());

      cursor = generateRXInstruction(
          cg(), TR::InstOpCode::STD, node,
          getRealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), spillMemRef,
          cursor);
...
TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::
    XPLINKCallDescriptorRelocation(TR::Instruction* nop,
                                   TR::LabelSymbol* callDescriptor)
    : TR::LabelRelocation(NULL, callDescriptor), _nop(nop) {}
...
void TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::apply(
    TR::CodeGenerator* cg) {
  int64_t offsetToCallDescriptor =
      static_cast<int64_t>(getLabel()->getCodeLocation() -
                           (_nop->getBinaryEncoding() - 7)) /
      8;

  if (offsetToCallDescriptor < std::numeric_limits<int16_t>::min() ||
      offsetToCallDescriptor > std::numeric_limits<int16_t>::max()) {
    cg->comp()->failCompilation<TR::ExcessiveComplexity>(
        "Unable to encode offset to XPLINK call descriptor");
  }
