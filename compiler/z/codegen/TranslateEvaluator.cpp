 ...
 * We can avoid the CC compare by directly branching on the CC that is still in
 * effect right after trt. It is necessary to map mask and compare value into a
 * semantically equivalent branching condition for direct use.
 */
TR::InstOpCode::S390BranchCondition getConditionCodeFromCCCompare(
    uint8_t maskOnIf, uint8_t compVal) {
  ...
  switch (maskOnIf) {
    ...
    case 0x0A:
      ...
      break;  // ifXcmpge
    default:
      TR_ASSERT(0, "maskOnIf (%d) is not supported\n", maskOnIf);
      break;
  }

  static TR::InstOpCode::S390BranchCondition convertTable[6][3] = {
      // 0            1            2
      {TR::InstOpCode::COND_MASK8, TR::InstOpCode::COND_MASK4,
       TR::InstOpCode::COND_MASK2},  // ifXcmpeq
      {TR::InstOpCode::COND_MASK6, TR::InstOpCode::COND_MASK10,
       TR::InstOpCode::COND_MASK12},  // ifXcmpne
      {TR::InstOpCode::COND_MASK0, TR::InstOpCode::COND_MASK8,
       TR::InstOpCode::COND_MASK12},  // ifXcmplt
      {TR::InstOpCode::COND_MASK8, TR::InstOpCode::COND_MASK12,
       TR::InstOpCode::COND_MASK14},  // ifXcmple
      {TR::InstOpCode::COND_MASK6, TR::InstOpCode::COND_MASK2,
       TR::InstOpCode::COND_MASK0},  // ifXcmpgt
      {TR::InstOpCode::COND_MASK14, TR::InstOpCode::COND_MASK6,
       TR::InstOpCode::COND_MASK2}  // ifXcmpge
  };

  return convertTable[maskOnIfIndex][compVal];
}

TR::Register *inlineTrtEvaluator(TR::Node *node, TR::CodeGenerator *cg,
                                 TR::InstOpCode::Mnemonic opCode,
                                 bool isFoldedIf, bool isFoldedLookup,
                                 TR::Node *parent) {
  TR::Compilation *comp = cg->comp();
  bool storeRegs = node->getNumChildren() == 5;
  bool isSimple = node->getOpCodeValue() == TR::trtSimple;
  bool packR2 =
      node->getOpCodeValue() == TR::trt && !storeRegs && !isFoldedLookup;
  int32_t defaultCCMask = 0;

  if (cg->traceBCDCodeGen())
    traceMsg(comp,
             "\tinlineTrtEvaluator %s (%p) -- numChildren %d, storeR1AndR2=%d, "
             "isSimple=%d, packR2=%d\n",
             node->getOpCode().getName(), node, node->getNumChildren(),
             storeRegs, isSimple, packR2);
  ...
  TR::MemoryReference *sourceMemRef =
      sourceReg ? generateS390MemoryReference(sourceReg, 0, cg)
                : generateS390MemoryReference(cg, node, sourceNode, 0, true);
  TR::MemoryReference *tableMemRef =
      tableReg ? generateS390MemoryReference(tableReg, 0, cg)
               : generateS390MemoryReference(cg, node, tableNode, 0, true);
  ...
  regDeps->addPostCondition(r1Reg, TR::RealRegister::GPR1,
                            DefinesDependentRegister);
  regDeps->addPostCondition(r2Reg, TR::RealRegister::GPR2,
                            DefinesDependentRegister);
  ...
  if (lengthNode->getOpCode().isLoadConst()) {
    ...
    TR::Instruction *cursor = generateSS1WithImplicitGPRsInstruction(
        cg, opCode, node, length, sourceMemRef, tableMemRef, regDeps, NULL,
        NULL, r1Reg, r2Reg);
  } else {
    TR::Register *lengthReg = cg->evaluate(lengthNode);

    TR::Instruction *cursor = generateSS1WithImplicitGPRsInstruction(
        cg, opCode, node, 0, sourceMemRef, tableMemRef, regDeps, NULL, NULL,
        r1Reg, r2Reg);
  ...
  if ((opCode == TR::InstOpCode::TRTR) && (TR::Compiler->target.is32Bit())) {
    TR::MemoryReference *r1BitClearRef =
        generateS390MemoryReference(r1Reg, 0, cg);
    TR::Instruction *cursor = generateRXInstruction(cg, TR::InstOpCode::LA,
                                                    node, r1Reg, r1BitClearRef);
    cursor->setDependencyConditions(regDeps);
  }
  ...
  if (packR2) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)) {
      generateRIEInstruction(cg, TR::InstOpCode::RISBGN, node, conditionCodeReg,
                             r2Reg, 48, 55, 8);
    } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
      generateRIEInstruction(cg, TR::InstOpCode::RISBG, node, conditionCodeReg,
                             r2Reg, 48, 55, 8);
    } else {
      generateRSInstruction(cg, TR::InstOpCode::SLL, node, r2Reg, 8);
      generateRRInstruction(cg, TR::InstOpCode::OR, node, conditionCodeReg,
                            r2Reg);
    }
  ...
  {
    TR_ASSERT(parent->getOpCodeValue() == TR::trtLookup,
              "The node %s (%p) is not trtLookup\n",
    ...
    for (int32_t ii = 1; ii < numChildren; ii++) {
      ...
      TR::Node *child =
          processDefault ? parent->getChild(1) : parent->getChild(ii + 1);
      ...
      if (child->getNumChildren() > 0) {
        ...
        if (needDefaultBranch)
          cursor = generateS390BranchInstruction(
              cg, TR::InstOpCode::BRC, brCond, parent,
              caseDest->getNode()->getLabel(),
    ...
    if (assignDepsToLastBranch && cursor->getDependencyConditions() == NULL)
      cursor->setDependencyConditions(
          generateRegisterDependencyConditions(cg, defaultRegDepChild, 0));
  ...
  if (sourceReg) cg->decReferenceCount(sourceNode);
  if (tableReg) cg->decReferenceCount(tableNode);
  ...
  if (packR2) return node->getRegister();
  ...
  return OMR::Z::TreeEvaluator::getConditionCodeOrFoldIf(node, cg, isFoldedIf,
                                                         parent);
}
...
TR::Register *inlineTrEvaluator(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (lengthNode->getOpCode().isLoadConst()) {
    ...
    generateSS1Instruction(cg, TR::InstOpCode::TR, node, length, sourceMem,
                           tableMem);
    ...
    TR::Instruction *cursor = generateSS1Instruction(
        cg, TR::InstOpCode::TR, node, 0,
