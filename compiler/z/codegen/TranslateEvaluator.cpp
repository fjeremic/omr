...
TR::InstOpCode::S390BranchCondition getConditionCodeFromCCCompare(uint8_t maskOnIf, uint8_t compVal)
{
  ...
  TR_ASSERT(compVal == 0 || compVal == 1 || compVal == 2, "compVal (%d) should be in range of 0/1/2\n", compVal);
...
TR::Register* inlineTrtEvaluator(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic opCode,
  bool isFoldedIf, bool isFoldedLookup, TR::Node* parent)
{
  ...
  if (cg->traceBCDCodeGen())
    traceMsg(comp, "\tinlineTrtEvaluator %s (%p) -- numChildren %d, storeR1AndR2=%d, isSimple=%d, packR2=%d\n",
      node->getOpCode().getName(), node, node->getNumChildren(), storeRegs, isSimple, packR2);
  ...
  TR::MemoryReference* sourceMemRef = sourceReg ? generateS390MemoryReference(sourceReg, 0, cg)
                                                : generateS390MemoryReference(cg, node, sourceNode, 0, true);
  TR::MemoryReference* tableMemRef = tableReg ? generateS390MemoryReference(tableReg, 0, cg)
  ...
  if (lengthNode->getOpCode().isLoadConst()) {
    ...
    TR::Instruction* cursor = generateSS1WithImplicitGPRsInstruction(
      cg, opCode, node, length, sourceMemRef, tableMemRef, regDeps, NULL, NULL, r1Reg, r2Reg);
  } else {
    TR::Register* lengthReg = cg->evaluate(lengthNode);

    TR::Instruction* cursor = generateSS1WithImplicitGPRsInstruction(
      cg, opCode, node, 0, sourceMemRef, tableMemRef, regDeps, NULL, NULL, r1Reg, r2Reg);
    ...
    for (int32_t ii = 1; ii < numChildren; ii++) {
      ...
      if (child->getNumChildren() > 0) {
        ...
        if (needDefaultBranch)
          cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, brCond, parent,
            caseDest->getNode()->getLabel(), generateRegisterDependencyConditions(cg, child->getFirstChild(), 0));
      } else {
        if (needDefaultBranch)
          cursor
            = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, brCond, parent, caseDest->getNode()->getLabel());
      }
