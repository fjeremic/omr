                   ...
                   TR::Node* parent)
{
  ...
  regDeps->addPostCondition(
    r1Reg, TR::RealRegister::GPR1, DefinesDependentRegister);
  regDeps->addPostCondition(
    r2Reg, TR::RealRegister::GPR2, DefinesDependentRegister);
  ...
  if (lengthNode->getOpCode().isLoadConst()) {
    ...
    TR::Instruction* cursor =
      generateSS1WithImplicitGPRsInstruction(cg,
                                             opCode,
                                             node,
                                             length,
                                             sourceMemRef,
  ...
  if ((opCode == TR::InstOpCode::TRTR) && (TR::Compiler->target.is32Bit())) {
    ...
    TR::Instruction* cursor =
      generateRXInstruction(cg, TR::InstOpCode::LA, node, r1Reg, r1BitClearRef);
    cursor->setDependencyConditions(regDeps);
  }
  ...
  if (packR2) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)) {
      generateRIEInstruction(
        cg, TR::InstOpCode::RISBGN, node, conditionCodeReg, r2Reg, 48, 55, 8);
    } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
      generateRIEInstruction(
        cg, TR::InstOpCode::RISBG, node, conditionCodeReg, r2Reg, 48, 55, 8);
    } else {
      generateRSInstruction(cg, TR::InstOpCode::SLL, node, r2Reg, 8);
      generateRRInstruction(
        cg, TR::InstOpCode::OR, node, conditionCodeReg, r2Reg);
    }
  ...
  return OMR::Z::TreeEvaluator::getConditionCodeOrFoldIf(
    node, cg, isFoldedIf, parent);
}
...
inlineTrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (lengthNode->getOpCode().isLoadConst()) {
    ...
    generateSS1Instruction(
      cg, TR::InstOpCode::TR, node, length, sourceMem, tableMem);
