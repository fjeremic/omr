...
/**
 * 64bit version lconstHelper: load long integer constant (64-bit signed 2's
 * complement)
 */
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iconstEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::aconstEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lconstEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bconstEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *tempReg = node->setRegister(cg->allocateRegister());
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sconstEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *tempReg = node->setRegister(cg->allocateRegister());
  int32_t value =
      node->isUnsigned() ? node->getConst<uint16_t>() : node->getShortInt();
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::cconstEvaluator(TR::Node *node,
                                                     TR::CodeGenerator *cg) {
  TR::Register *tempReg = node->setRegister(cg->allocateRegister());
  generateLoad32BitConstant(cg, node, node->getConst<uint16_t>(), tempReg,
                            true);
  return tempReg;
}
 ...
 * labsEvaluator -
 */
TR::Register *OMR::Z::TreeEvaluator::labsEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *targetRegister = cg->allocateRegister();
  ;
  TR::Register *sourceRegister = cg->evaluate(firstChild);
  generateRRInstruction(cg, TR::InstOpCode::LPGR, node, targetRegister,
                        sourceRegister);
  node->setRegister(targetRegister);
 ...
 * iabsEvaluator -
 */
TR::Register *OMR::Z::TreeEvaluator::iabsEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Compilation *comp = cg->comp();
...
#ifdef J9_PROJECT_SPECIFIC
  else if (node->getOpCodeValue() == TR::dfabs ||
           node->getOpCodeValue() == TR::ddabs ||
           node->getOpCodeValue() == TR::deabs)
...
#ifdef J9_PROJECT_SPECIFIC
  if (node->getOpCodeValue() == TR::deabs) {
    generateRRInstruction(cg, opCode, node, targetRegister->getHighOrder(),
                          sourceRegister->getHighOrder());

    // If we aren't clobbering, also need to move the low half; otherwise, a
    // load positive here is setting the highest bit in the high-order register,
    // so the low half can remain unchanged
    if (!cg->canClobberNodesRegister(firstChild))
      generateRRInstruction(cg, TR::InstOpCode::LDR, node,
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::l2aEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::Z::TreeEvaluator::dsqrtEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (firstChild->isSingleRefUnevaluated() &&
    ...
    generateRXEInstruction(cg, TR::InstOpCode::SQDB, node, targetRegister,
                           generateS390MemoryReference(firstChild, cg), 0);
  } else {
    TR::Register *opRegister = cg->evaluate(firstChild);
    generateRRInstruction(cg, TR::InstOpCode::SQDBR, node, targetRegister,
                          opRegister);
    cg->decReferenceCount(firstChild);
  }
  node->setRegister(targetRegister);
...
TR::Register *OMR::Z::TreeEvaluator::fsqrtEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  ...
  generateRRInstruction(cg, TR::InstOpCode::SQEBR, node, targetRegister,
                        opRegister);
...
TR::Register *OMR::Z::TreeEvaluator::dnintEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *targetRegister = NULL;
  TR::Register *opRegister = cg->evaluate(firstChild);
  ...
  generateRRInstruction(cg, TR::InstOpCode::FIDBR, node, targetRegister,
                        opRegister);
...
TR::Register *OMR::Z::TreeEvaluator::fnintEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *targetRegister = NULL;
  TR::Register *opRegister = cg->evaluate(firstChild);
  ...
  generateRRInstruction(cg, TR::InstOpCode::FIEBR, node, targetRegister,
                        opRegister);
...
TR::Register *OMR::Z::TreeEvaluator::getpmEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Register *targetRegister = cg->allocateRegister();
  generateRRInstruction(cg, TR::InstOpCode::IPM, node, targetRegister,
                        targetRegister);
...
TR::Register *OMR::Z::TreeEvaluator::setpmEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
