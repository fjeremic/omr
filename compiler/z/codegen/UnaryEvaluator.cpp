 ...
 */
TR::Register* OMR::Z::TreeEvaluator::cconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* tempReg = node->setRegister(cg->allocateRegister());
...
TR::Register* OMR::Z::TreeEvaluator::iabsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
#ifdef J9_PROJECT_SPECIFIC
  else if (node->getOpCodeValue() == TR::dfabs || node->getOpCodeValue() == TR::ddabs
  ...
  if (node->getOpCodeValue() == TR::deabs) {
    ...
    if (node->getOpCodeValue() == TR::fabs
#ifdef J9_PROJECT_SPECIFIC
      || node->getOpCodeValue() == TR::dabs || node->getOpCodeValue() == TR::dfabs
  ...
  if (node->getOpCodeValue() == TR::deabs) {
    ...
    if (!cg->canClobberNodesRegister(firstChild))
      generateRRInstruction(
        cg, TR::InstOpCode::LDR, node, targetRegister->getLowOrder(), sourceRegister->getLowOrder());
  } else
#endif
  {
...
TR::Register* OMR::Z::TreeEvaluator::dsqrtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (firstChild->isSingleRefUnevaluated() && firstChild->getOpCodeValue() == TR::dloadi) {
    generateRXEInstruction(
      cg, TR::InstOpCode::SQDB, node, targetRegister, generateS390MemoryReference(firstChild, cg), 0);
  } else {
...
TR::Register* OMR::Z::TreeEvaluator::dnintEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* targetRegister = NULL;
  TR::Register* opRegister = cg->evaluate(firstChild);
...
TR::Register* OMR::Z::TreeEvaluator::fnintEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* targetRegister = NULL;
  TR::Register* opRegister = cg->evaluate(firstChild);
...
TR::Register* OMR::Z::TreeEvaluator::getpmEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* targetRegister = cg->allocateRegister();
...
TR::Register* OMR::Z::TreeEvaluator::setpmEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
