...
// Multi Code Cache Routines for checking whether an entry point is within reach
// of a BASRL.
...
public:
  ...
  struct TR_BranchPreloadCallData {
    ...
    TR_BranchPreloadCallData(TR::Instruction *i, TR::LabelSymbol *l,
                             TR::Block *b, TR::SymbolReference *s, int32_t f)
        : _callInstr(i), _callLabel(l), _callBlock(b), _callSymRef(s),
          _frequency(f) {}
  };
  ...
  TR::list<TR::Register *> *getFirstTimeLiveOOLRegisterList() {
    return _firstTimeLiveOOLRegisterList;
  }
  TR::list<TR::Register *> *
  setFirstTimeLiveOOLRegisterList(TR::list<TR::Register *> *r) {
    return _firstTimeLiveOOLRegisterList = r;
  }

  TR::list<TR_BranchPreloadCallData *> *_callsForPreloadList;

  TR::list<TR_BranchPreloadCallData *> *getCallsForPreloadList() {
    return _callsForPreloadList;
  }
  void createBranchPreloadCallData(TR::LabelSymbol *callLabel,
  ...
  /**
   * \brief Tells the optimzers and codegen whether a load constant node should
   * be rematerialized.
   *
   * \details Large constants should be materialized (constant node should be
   * commoned up) because loading them as immediate values is expensive.
   *
   * A constant is large if it's outside of the range specified by the largest
   * negative const and smallest positive const. And the ranges are hardware
   * platform dependent.
   */
  ...
  generateScratchRegisterManager(int32_t capacity = 8);
  ...
  void
  addPICsListForInterfaceSnippet(TR::S390ConstantDataSnippet *ifcSnippet,
                                 TR::list<TR_OpaqueClassBlock *> *PICSlist);
  TR::list<TR_OpaqueClassBlock *> *
  getPICsListForInterfaceSnippet(TR::S390ConstantDataSnippet *ifcSnippet);
  ...
  void recordRegisterAssignment(TR::Register *assignedReg,
  ...
  void
  StopUsingEscapedMemRefsRegisters(int32_t topOfMemRefStackBeforeEvaluation);
  ...
  TR::RegisterIterator *getVRFRegisterIterator() {
    return _vrfRegisterIterator;
  }
  TR::RegisterIterator *setVRFRegisterIterator(TR::RegisterIterator *iter) {
    return _vrfRegisterIterator = iter;
  }

  bool supportsLengthMinusOneForMemoryOpts() { return true; }

  bool codegenSupportsLoadlessBNDCheck() {
    return TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12);
  }
  TR::Register *evaluateLengthMinusOneForMemoryOps(TR::Node *, bool,
                                                   bool &lenMinusOne);

  virtual TR_GlobalRegisterNumber getGlobalRegisterNumber(uint32_t realRegNum);

  TR::RegisterDependencyConditions *
  createDepsForRRMemoryInstructions(TR::Node *node, TR::RegisterPair *sourceReg,
  ...
  virtual TR::RegisterPair *
  allocateConsecutiveRegisterPair(TR::Register *lowRegister,
                                  TR::Register *highRegister);
  virtual TR::RegisterPair *allocateFPRegisterPair();
  virtual TR::RegisterPair *allocateFPRegisterPair(TR::Register *lowRegister,
                                                   TR::Register *highRegister);

  void processUnusedNodeDuringEvaluation(TR::Node *node);

  void apply12BitLabelRelativeRelocation(int32_t *cursor, TR::LabelSymbol *,
  ...
  void setUnavailableRegisters(TR::Block *b,
                               TR_BitVector &unavailableRegisters);

  void resetGlobalRegister(TR::RealRegister::RegNum reg,
                           TR_BitVector &globalRegisters);
  void setGlobalRegister(TR::RealRegister::RegNum reg,
                         TR_BitVector &globalRegisters);
  ...
  void genMemCpy(TR::MemoryReference *targetMR, TR::Node *targetNode,
                 TR::MemoryReference *sourceMR, TR::Node *sourceNode,
                 int64_t copySize);
  void genMemClear(TR::MemoryReference *targetMR, TR::Node *targetNode,
                   int64_t clearSize);

  void
  genCopyFromLiteralPool(TR::Node *node, int32_t bytesToCopy,
                         TR::MemoryReference *targetMR, size_t litPoolOffset,
                         TR::InstOpCode::Mnemonic op = TR::InstOpCode::MVC);
  ...
  TR::Instruction *genLoadAddressToRegister(TR::Register *reg,
  ...
  TR::Register *gprClobberEvaluate(TR::Node *node, bool force_copy = false,
                                   bool ignoreRefCount = false);
  TR::Register *fprClobberEvaluate(TR::Node *node);
  TR_OpaquePseudoRegister *ssrClobberEvaluate(TR::Node *node,
  ...
  void setCurrentBCDCHKHandlerLabel(TR::LabelSymbol *l) {
    _currentBCDCHKHandlerLabel = l;
  }
  TR::LabelSymbol *getCurrentBCDCHKHandlerLabel() {
    return _currentBCDCHKHandlerLabel;
  }

  TR::RegisterDependencyConditions *_currentBCDRegDeps;
  void setCurrentCheckNodeRegDeps(TR::RegisterDependencyConditions *daaDeps) {
    _currentBCDRegDeps = daaDeps;
  }
  TR::RegisterDependencyConditions *getCurrentCheckNodeRegDeps() {
    return _currentBCDRegDeps;
  }

  // TODO : Do we need these? And if so there has to be a better way of tracking
  // this this than what we're doing here.
  /** Active counter used to track control flow basic blocks generated at
   * instruction selection. */
  int32_t _nextAvailableBlockIndex;
  /** The index of the current basic block (used and updated during instruction
   * selection). */
  int32_t _currentBlockIndex;

  void setNextAvailableBlockIndex(int32_t blockIndex) {
    _nextAvailableBlockIndex = blockIndex;
  }
  ...
  bool foundConflictingNode(TR::Node *node,
                            TR::list<TR::Node *> *conflictingAddressNodes);
  void
  collectConflictingAddressNodes(TR::Node *parent, TR::Node *node,
                                 TR::list<TR::Node *> *conflictingAddressNodes);

  template <class TR_AliasSetInterface>
  bool loadAndStoreMayOverlap(TR::Node *store, size_t storeSize, TR::Node *load,
                              size_t loadSize,
  ...
  bool checkSimpleLoadStore(TR::Node *loadNode, TR::Node *storeNode,
                            TR::Block *block);
  bool checkBitWiseChild(TR::Node *node);

  TR_StorageDestructiveOverlapInfo
  getStorageDestructiveOverlapInfo(TR::Node *src, size_t srcLength,
                                   TR::Node *dst, size_t dstLength);
  ...
  bool supportsBranchPreload() {
    return _cgFlags.testAny(S390CG_enableBranchPreload);
  }
  ...
  bool supportsBranchPreloadForCalls() {
    return _cgFlags.testAny(S390CG_enableBranchPreloadForCalls);
  }
  void setEnableBranchPreloadForCalls() {
    _cgFlags.set(S390CG_enableBranchPreloadForCalls);
  }
  void setDisableBranchPreloadForCalls() {
    _cgFlags.reset(S390CG_enableBranchPreloadForCalls);
  }

  bool isOutOfLineHotPath() {
    return _cgFlags.testAny(S390CG_isOutOfLineHotPath);
  }
  void setIsOutOfLineHotPath(bool val) {
    _cgFlags.set(S390CG_isOutOfLineHotPath, val);
  }

  bool getExitPointsInMethod() { return _cgFlags.testAny(S390CG_doesExit); }
  void setExitPointsInMethod(bool val) {
    return _cgFlags.set(S390CG_doesExit, val);
  }

  bool isPrefetchNextStackCacheLine() {
    return _cgFlags.testAny(S390CG_prefetchNextStackCacheLine);
  }
  void setPrefetchNextStackCacheLine(bool val) {
    return _cgFlags.set(S390CG_prefetchNextStackCacheLine, val);
  }

  // Heap object prefetching
  bool enableTLHPrefetching() {
    return _cgFlags.testAny(S390CG_enableTLHPrefetching);
  }
  ...
  TR_BitVector *getGlobalGPRsPreservedAcrossCalls() {
    return &_globalGPRsPreservedAcrossCalls;
  }
  TR_BitVector *getGlobalFPRsPreservedAcrossCalls() {
    return &_globalFPRsPreservedAcrossCalls;
  }
  ...
  bool getSupportsImplicitNullChecks() {
    return _cgFlags.testAny(S390CG_implicitNullChecks);
  }
  void setSupportsImplicitNullChecks(bool b) {
    _cgFlags.set(S390CG_implicitNullChecks, b);
  }

  bool getConditionalMovesEvaluationMode() {
    return _cgFlags.testAny(S390CG_conditionalMovesEvaluation);
  }
  void setConditionalMovesEvaluationMode(bool b) {
    _cgFlags.set(S390CG_conditionalMovesEvaluation, b);
  }

  bool getEnableRIOverPrivateLinkage() {
    return _cgFlags.testAny(S390CG_enableRIOverPrivateLinkage);
  }
  void setEnableRIOverPrivateLinkage(bool b) {
    _cgFlags.set(S390CG_enableRIOverPrivateLinkage, b);
  }

  bool getCondCodeShouldBePreserved() {
    return _cgFlags.testAny(S390CG_condCodeShouldBePreserved);
  }
  void setCondCodeShouldBePreserved(bool b) {
    _cgFlags.set(S390CG_condCodeShouldBePreserved, b);
  }
  ...
  void clearTotalSpills() {
    _totalColdSpills = 0;
    _totalHotSpills = 0;
  }
  void clearTotalRegisterXfers() {
    _totalColdRegisterXfers = 0;
    _totalHotRegisterXfers = 0;
  }
  void clearTotalRegisterMoves() {
    _totalColdRegisterMoves = 0;
    _totalHotRegisterMoves = 0;
  }

  // current RA block is only valid during register allocation pass and is only
  // used for debug
  ...
  findS390OutOfLineCodeSectionFromLabel(TR::LabelSymbol *label);
   ...
   *
   *  \param cursor
   *     The instruction following which the padding instructions will be
   * inserted.
   *
   *  \param size
   *     The size in number of bytes of how much padding to insert. This value
   * can be one of 0, 2, 4, or 6.
   *
   *  \param prependCursor
   *     Determines whether the padding instructions will be inserted before or
   * after \param cursor.
   *
   *  \return
   *     The last padding instruction generated, or \param cursor if \param size
   * is zero.
   */
  TR::Instruction *insertPad(TR::Node *node, TR::Instruction *cursor,
                             uint32_t size, bool prependCursor);
  ...
  struct constantHashInfo {
    static CS2::HashValue
    Hash(const TR_S390ConstantDataSnippetKey &key,
         const CS2::HashValue hv = CS2::CS2_FNV_OFFSETBASIS) {
      return CS2::Hash_FNV((const unsigned char *)key.c, key.size, hv);
    }
    static bool Equal(const TR_S390ConstantDataSnippetKey &key1,
                      const TR_S390ConstantDataSnippetKey &key2) {
  ...
  typedef CS2::HashTable<TR_S390ConstantDataSnippetKey,
                         TR::S390ConstantDataSnippet *, TR::Allocator,
                         constantHashInfo>
  ...
  bool hasDataSnippets() {
    return (_constantList.empty() && _writableList.empty() &&
            _snippetDataList.empty() && _constantHash.IsEmpty())
               ? false
               : true;
  }
  TR::list<TR::S390ConstantDataSnippet *> &getConstantInstructionSnippets() {
    return _constantList;
  }
  TR::list<TR::S390ConstantDataSnippet *> &getConstantDataStringSnippets() {
    return _constantList;
  }
  TR_ConstHashCursor getConstantDataSnippets() { return _constantHashCur; }
  TR::S390ConstantDataSnippet *getConstantDataSnippet(CS2::HashIndex hi) {
    return _constantHash.DataAt(hi);
  }

  TR::S390ConstantDataSnippet *create64BitLiteralPoolSnippet(TR::DataType dt,
                                                             int64_t value);
  TR::S390ConstantDataSnippet *createLiteralPoolSnippet(TR::Node *node);
  TR::S390ConstantInstructionSnippet *
  createConstantInstruction(TR::CodeGenerator *cg, TR::Node *node,
                            TR::Instruction *instr);
  TR::S390ConstantDataSnippet *findOrCreateConstant(TR::Node *, void *c,
                                                    uint16_t size);
  ...
  TR::S390ConstantDataSnippet *findOrCreate8ByteConstant(TR::Node *, int64_t c,
                                                         bool isWarm = 0);
  TR::S390ConstantDataSnippet *Create4ByteConstant(TR::Node *, int32_t c,
                                                   bool writable);
  TR::S390ConstantDataSnippet *Create8ByteConstant(TR::Node *, int64_t c,
                                                   bool writable);
  TR::S390ConstantDataSnippet *CreateConstant(TR::Node *, void *c,
                                              uint16_t size, bool writable);
  TR::S390ConstantDataSnippet *getFirstConstantData();

  // Writable Data List functions
  bool hasWritableDataSnippets() {
    return _writableList.empty() ? false : true;
  }
  TR::S390WritableDataSnippet *CreateWritableConstant(TR::Node *);

  // OutOfLineCodeSection List functions
  TR::list<TR_S390OutOfLineCodeSection *> &getS390OutOfLineCodeSectionList() {
    return _outOfLineCodeSectionList;
  }
  TR_S390OutOfLineCodeSection *
  ...
  TR_Array<TR::Register *> &getTransientLongRegisters() {
    return _transientLongRegisters;
  }
  ...
  // list of branch unconditional relative instructions used for patching entry
  // point need to be updated with correct offset to sampling preprologue
  ...
  void setOnDemandLiteralPoolRun(bool val) {
    _cgFlags.set(S390CG_literalPoolOnDemandOnRun, val);
  }
  bool isLiteralPoolOnDemandOn() {
    return _cgFlags.testAny(S390CG_literalPoolOnDemandOnRun);
  }
  virtual void setGlobalStaticBaseRegisterOn(bool val) {
    _cgFlags.set(S390CG_globalStaticBaseRegisterOn, val);
  }
  virtual bool isGlobalStaticBaseRegisterOn() {
    return _cgFlags.testAny(S390CG_globalStaticBaseRegisterOn);
  }
  ...
  void setCanExceptByTrap(bool val) {
    _cgFlags.set(S390CG_canExceptByTrap, val);
  }
  virtual bool canExceptByTrap() {
    return _cgFlags.testAny(S390CG_canExceptByTrap);
  }
  ...
  // The reusable temp slot is an 8-byte scalar temp that gets mapped to the
  // stack the first time allocateReusableTempSlot() is called.  This temp is
  // meant to be used for short periods of time.
  TR::SymbolReference *allocateReusableTempSlot();
  void freeReusableTempSlot();

  bool isActiveCompareCC(TR::InstOpCode::Mnemonic opcd, TR::Register *tReg,
                         TR::Register *sReg);
  bool isActiveArithmeticCC(TR::Register *tstReg);
  bool isActiveLogicalCC(TR::Node *ccNode, TR::Register *tstReg);

  bool mulDecompositionCostIsJustified(int32_t numOfOperations,
                                       char bitPosition[], char operationType[],
                                       int64_t value);
  ...
  /** Checks if the immediate value can fit in 32-bit immediate field that is
   * unsigned. */
   ...
   */
  bool signExtendedHighOrderBits(TR::Node *node, TR::Register *targetRegister,
   ...
   */
  bool clearHighOrderBits(TR::Node *node, TR::Register *targetRegister,
                          uint32_t numberOfBits);
  ...
  /**
   * array translate on TRex and below requires that the translate table be
   * page-aligned for TRTO/TRTT and double-word aligned for TROT/TROO array
   * translate takes awhile to initialize - for very short translations, the
   * original loop of instructions should be driven instead of the translate
   * instruction LL: For Golden Eagle, TRTO/TRTT is also double-word aligned.
   */
...
private:
  // TODO: These should move into the base class. There also seems to be a
  // little overlap between these and _firstInstruction and _appendInstruction.
  // We should likely expose a getLastInstruction API as well. The former API
  // should return _methodBegin and latter _methodEnd always. The append
  // instruction will be the instruction preceeding _methodEnd.
  ...
  CS2::HashTable<TR::Register *, TR::RealRegister::RegNum, TR::Allocator>
      _previouslyAssignedTo;
