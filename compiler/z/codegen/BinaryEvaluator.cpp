             ...
             TR::RegisterDependencyConditions* dep = 0)
{
  ...
  generateS390BranchInstruction(
    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, cFlowRegionEnd);

  // Increment MS int due to overflow in LS int
  generateRIInstruction(
    cg, TR::InstOpCode::AHI, node, targetRegisterPair->getHighOrder(), -1);

  // Not equal, straight through
  generateS390LabelInstruction(
    cg, TR::InstOpCode::LABEL, node, cFlowRegionEnd, localDeps);
              ...
              TR::RegisterDependencyConditions* dep = 0)
{
  ...
  // 0, i.e.
  generateRIInstruction(
    cg, TR::InstOpCode::AGHI, node, targetRegisterPair->getHighOrder(), -1);
          ...
          TR::RegisterDependencyConditions* deps)
{
  ...
  if (performTransformation(
        comp,
        "O^O Use AL/ALC to perform long add.\n")) { // wrong carry bit to be set
    if (l_value != 0) {
      ...
      generateS390ImmOp(
        cg, TR::InstOpCode::AL, node, tempReg, lowOrder, l_value, deps);
      generateS390ImmOp(
        cg, TR::InstOpCode::ALC, node, tempReg, highOrder, h_value, deps);
      ...
      generateS390ImmOp(
        cg, TR::InstOpCode::A, node, highOrder, highOrder, h_value);
    }
  } else {
    // Add high value
    generateS390ImmOp(
    ...
    if (l_value != 0) {
      ...
      generateS390ImmOp(
        cg, TR::InstOpCode::AL, node, tempReg, lowOrder, l_value);
...
laddHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (hasCompressedPointers && ((secondChild->getOpCodeValue() != TR::lconst) ||
    ...
    if (skipAdd) {
      ...
      generateS390LabelInstruction(
        cg, TR::InstOpCode::LABEL, node, skipAdd, conditions);
    }
    ...
    generateS390ImmOp(
      cg, TR::InstOpCode::AG, node, targetRegister, targetRegister, long_value);
    node->setRegister(targetRegister);
    ...
    temp.integerAddAnalyser(
      node, TR::InstOpCode::AGR, TR::InstOpCode::AG, TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
...
findPreviousAGIDef(TR::Instruction* prev,
                   ...
                   TR::Compilation* comp)
{
  ...
  if (trace)
    traceMsg(
      comp, "in find at %p, looking for %p for %d\n", prev, useA, searchLimit);
...
generic32BitAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (!findPreviousAGIDef(
          cg->getAppendInstruction(), delay, childTargetReg, comp)) {
      useLA = false;
    }
    ...
    if (useAHIK) {
      generateRIEInstruction(
        cg, TR::InstOpCode::AHIK, node, targetRegister, childTargetReg, value);
    } else if (useLA &&
      ...
      generateRXInstruction(
        cg, TR::InstOpCode::LA, node, targetRegister, memRef);
    } else {
      if (!canClobberReg) {
        generateRRInstruction(cg,
      ...
      generateS390ImmOp(
        cg, TR::InstOpCode::A, node, targetRegister, targetRegister, value);
    }
...
generic32BitSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196) &&
      ...
      generateRIEInstruction(
        cg, TR::InstOpCode::AHIK, node, targetRegister, sourceRegister, -value);
    } else {
      targetRegister = cg->gprClobberEvaluate(firstChild);
      generateS390ImmOp(
        cg, TR::InstOpCode::A, node, targetRegister, targetRegister, -value);
 ...
 */
inline TR::Register*
lDivRemGenericEvaluator64(TR::Node* node,
                          TR::CodeGenerator* cg,
                          bool isDivision)
{
  ...
  if (firstChild == secondChild) {
    ...
    generateRIInstruction(
      cg, TR::InstOpCode::LGHI, node, returnRegister, retValue);
    ...
    if (denominator == -1) {
      ...
      if (!isDivision) // remainder
      {
        ...
        generateRRInstruction(
          cg, TR::InstOpCode::LCGR, node, firstRegister, firstRegister);
      ...
      if (isDivision) {
        ...
        if (!isUnsigned && !firstChild->isNonNegative()) {
          ...
          generateS390ImmOp(cg,
                            ...
                            deps);
          generateS390LabelInstruction(
            cg, TR::InstOpCode::LABEL, node, skipSet, deps);
      ...
      } else // remainder
      {
        ...
        if (!isUnsigned && !firstChild->isNonNegative()) {
          ...
          generateRSInstruction(
            cg, TR::InstOpCode::SRAG, node, tempRegister2, firstRegister, 63);
          generateRRInstruction(
            cg, TR::InstOpCode::LGR, node, tempRegister1, firstRegister);
          ...
          generateRRInstruction(
            cg, TR::InstOpCode::AGR, node, firstRegister, tempRegister2);
          if (denominator != static_cast<int32_t>(absValueOfDenominator))
            generateRILInstruction(
              ...
              firstRegister,
              static_cast<int32_t>(
                (CONSTANT64(0xFFFFFFFFFFFFFFFF) - absValueOfDenominator + 1) >>
                32));
          generateRILInstruction(
            cg,
            TR::InstOpCode::NILF,
            ...
            static_cast<int32_t>(CONSTANT64(0xFFFFFFFFFFFFFFFF) -
                                 absValueOfDenominator + 1));
          generateRRInstruction(
            cg, TR::InstOpCode::SGR, node, tempRegister1, firstRegister);
          generateRRInstruction(
            cg, TR::InstOpCode::LGR, node, firstRegister, tempRegister1);
          cg->stopUsingRegister(tempRegister1);
          cg->stopUsingRegister(tempRegister2);
        } else {
          generateShiftAndKeepSelected64Bit(node,
                                            cg,
                                            firstRegister,
                                            firstRegister,
                                            64 - shiftAmnt,
        ...
        generateS390LabelInstruction(
          cg, TR::InstOpCode::LABEL, node, done, deps);
      }
  ...
  if (!doConditionalRemainder && !node->isSimpleDivCheck() && !isUnsigned &&
    ...
    generateS390ImmOp(cg,
                      ...
                      (int64_t)CONSTANT64(0x8000000000000000));
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, doDiv);
    generateS390ImmOp(
      cg, TR::InstOpCode::CG, node, sourceRegister, sourceRegister, -1);
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, doDiv);
    ...
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, skipDiv);
  ...
  if (doConditionalRemainder) {
    ...
    if (firstChild->isNonNegative()) {
      ...
      generateRRInstruction(
        cg, TR::InstOpCode::LPGR, node, absDividendReg, remRegister);
    }
    ...
    if (value <= TR::getMaxSigned<TR::Int32>()) {
      ...
      generateRRInstruction(
        cg, TR::InstOpCode::CLGR, node, absDividendReg, sourceRegister);
    }
    TR::LabelSymbol* cFlowRegionStart = generateLabelSymbol(cg);
    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
  ...
  if (isUnsigned) {
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::DLGR, node, targetRegisterPair, sourceRegister);
  } else {
    if (doConditionalRemainder)
      generateRRInstruction(cg,
                            TR::InstOpCode::LGR,
                            ...
                            remRegister); // in doConditionalRemainder case this
                                          // is the slow path so do move here
    generateRRInstruction(
      cg, TR::InstOpCode::DSGR, node, targetRegisterPair, sourceRegister);
  }

  // Label to skip the division
  if (skipDiv != NULL) {
    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, skipDiv, dependencies);
  } else {
    generateS390PseudoInstruction(
      cg, TR::InstOpCode::DEPEND, node, dependencies);
  }
 ...
 */
TR::Register*
iDivRemGenericEvaluator(TR::Node* node,
                        TR::CodeGenerator* cg,
                        bool isDivision,
                        TR::MemoryReference* divchkDivisorMR)
{
  ...
  if (firstChild == secondChild) {
    ...
    generateRIInstruction(
      cg, TR::InstOpCode::LHI, node, returnRegister, retValue);
  ...
  if (divchkDivisorMR && !needCheck) {
    ...
    if (divchkDivisorMR) {
      ...
      generateRXInstruction(cg,
                            dtype.isInt64() ? TR::InstOpCode::LG
                                            : TR::InstOpCode::L,
                            secondChild,
                            sourceRegister,
                            tempMR);
  ...
  if (needCheck) {
    ...
    cursor = generateRRInstruction(
      cg, TR::InstOpCode::LR, node, quoRegister, remRegister);
  ...
  if (node->getOpCode().isUnsigned())
    generateRSInstruction(
      cg, TR::InstOpCode::SRDL, node, targetRegisterPair, 32);
  else
    generateRSInstruction(
      cg, TR::InstOpCode::SRDA, node, targetRegisterPair, 32);

  // Do division
  if (sourceMR) {
    if (node->getOpCode().isUnsigned())
      generateRXInstruction(
        cg, TR::InstOpCode::DL, node, targetRegisterPair, sourceMR);
    else
      generateRXInstruction(
        cg, TR::InstOpCode::D, node, targetRegisterPair, sourceMR);
  } else {
    if (node->getOpCode().isUnsigned())
      generateRRInstruction(
        cg, TR::InstOpCode::DLR, node, targetRegisterPair, sourceRegister);
    else
      generateRRInstruction(
        cg, TR::InstOpCode::DR, node, targetRegisterPair, sourceRegister);
  }
...
inline TR::Register*
genericLongShiftSingle(TR::Node* node,
                       TR::CodeGenerator* cg,
                ...
                TR::InstOpCode::Mnemonic altShiftOp)
{
  ...
  if (shiftOp == TR::InstOpCode::SRL || shiftOp == TR::InstOpCode::SRA) {
    canUseAltShiftOp =
      altShiftOp == TR::InstOpCode::SRLK || altShiftOp == TR::InstOpCode::SRAK;
  }
  ...
  if (node->getOpCodeValue() == TR::bushr) {
    if (node->getFirstChild()->getOpCodeValue() == TR::bconst &&
      ...
      generateLoad32BitConstant(cg,
                                node->getFirstChild(),
                                node->getFirstChild()->getByte() & 0xFF,
                                srcReg,
                                true);
    } else {
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    if (shiftAmount != 0) {
      if (trgReg != srcReg && canUseAltShiftOp) {
        generateRSInstruction(
          cg, altShiftOp, node, trgReg, srcReg, shiftAmount);
      } else {
  ...
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
        ...
        if (rShftAmnt + lShftAmnt == 64 && lshlSourceNode == lushrSourceNode) {
          ...
          TR::InstOpCode::Mnemonic opCode =
            TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)
              ...
              : TR::InstOpCode::RISBG;
          generateRIEInstruction(
            cg, opCode, node, targetReg, sourceReg, 0, 63, lShftAmnt);
    ...
    if (node->getOpCodeValue() == TR::lor && andChild &&
      ...
      if (mask1 && mask1 == (~mask2) && shiftBy <= 63 &&
        ...
        if (mask1 == mask2 &&
          ...
          TR::Register* targetReg = cg->allocateRegister();
          generateRRInstruction(
            cg, TR::InstOpCode::LGR, node, targetReg, toReg);
          generateRIEInstruction(cg,
                                 TR::InstOpCode::RISBG,
                                 node,
                                 targetReg,
    ...
    if (node->getOpCodeValue() == TR::lor ||
      ...
      if (shiftChild &&
          ...
          performTransformation(
            cg->comp(), "O^O Combine or/shift into rotate node [%p]\n", node)) {
        ...
        generateRRInstruction(cg, TR::InstOpCode::LGR, node, targetReg, toReg);
        generateRIEInstruction(
          cg, opcode, node, targetReg, fromReg, firstBit, lastBit, shiftBy);
        cg->decReferenceCount(shiftChild->getFirstChild());
        cg->decReferenceCount(shiftChild->getSecondChild());
        return targetReg;
      }
      ...
      if (popCnt == (64 - lZeros - tZeros)) {
        ...
        if (msBit > lsBit) {
          if (node->getType().isInt64()) {
            generateRRInstruction(
              cg, TR::InstOpCode::XGR, node, targetReg, targetReg);
          } else {
            generateRRInstruction(
              cg, TR::InstOpCode::XR, node, targetReg, targetReg);
          }
...
TR::Register*
OMR::Z::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(
  TR::Node* node,
  TR::CodeGenerator* cg,
  int32_t shiftAmount,
  bool isSignedShift)
  ...
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    ...
    if (node->getOpCodeValue() == TR::land &&
      ...
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(
          comp,
          "[%p] land => rotated-and-insert: tZeros %d, lZeros %d, popCnt %d\n",
          node,
          tZeros,
          lZeros,
          popCnt);
        traceMsg(comp,
                 "          => rotated-and-insert: lOnes %d, tOnes %d\n",
                 lOnes,
                 tOnes);
      }
...
lsubHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (hasCompressedPointers && ((secondChild->getOpCodeValue() != TR::lconst) ||
    ...
    secondRegister = cg->evaluate(secondChild);
    generateRRInstruction(
      cg, TR::InstOpCode::SGR, node, targetRegister, secondRegister);
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::LCGR, secondChild, targetRegister, targetRegister);
    generateS390ImmOp(
      cg, TR::InstOpCode::AG, node, targetRegister, targetRegister, long_value);
 ...
 */
TR::Register*
OMR::Z::TreeEvaluator::dualMulHelper64(TR::Node* node,
                                       TR::Node* lmulNode,
                                       TR::Node* lumulhNode,
                                       TR::CodeGenerator* cg)
{
  ...
  cursor = generateRRInstruction(
    cg, TR::InstOpCode::MLGR, node, trgtRegPair, secondRegister);
...
lmulHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChildIsConstant) {
    ...
    if (create_LA) {
      ...
      generateRXInstruction(
        cg, TR::InstOpCode::LA, node, targetRegister, interimMemoryReference);
    } else {
      generateS390ImmOp(
        cg, TR::InstOpCode::MSG, node, sourceRegister, targetRegister, value);
    }
    ...
    temp.genericAnalyser(
      node, TR::InstOpCode::MSGR, TR::InstOpCode::MSG, TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
...
OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    generateRRInstruction(
      cg, TR::InstOpCode::LCGR, node, sourceRegister, sourceRegister);
    ...
    // Branch to done
    generateS390BranchInstruction(
      cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, doneMulh);
    ...
    // Label for done
    generateS390LabelInstruction(
      cg, TR::InstOpCode::LABEL, node, doneMulh, dependencies);
    ...
    TR_ASSERT(
      0,
      "TR::TreeEvaluator::lmulhEvaluator -- RegReg not implemented for lmulh.");
  }
...
OMR::Z::TreeEvaluator::mulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getInt() == 0) {
    cursor = generateRRInstruction(
      cg, TR::InstOpCode::XR, node, targetRegister, targetRegister);
  } else {
    TR::Register* secondRegister = cg->evaluate(secondChild);
    if (isUnsigned) {
      cursor = generateRREInstruction(
        cg, TR::InstOpCode::MLR, node, targetRegisterPair, secondRegister);
    } else {
      cursor = generateRRInstruction(
        cg, TR::InstOpCode::MR, node, targetRegisterPair, secondRegister);
    }
  }
...
OMR::Z::TreeEvaluator::imulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (create_LA) {
      ...
      generateS390ImmOp(
        cg, TR::InstOpCode::MS, node, sourceRegister, targetRegister, value);
    }
    ...
    if (targetRegister->containsCollectedReference()) {
      ...
      generateRRInstruction(
        cg, TR::InstOpCode::LR, node, tmpReg, targetRegister);
    ...
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(
      node, TR::InstOpCode::MSR, TR::InstOpCode::MS, TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (value == -1 && !node->getOpCode().isUnsigned()) {
      ...
      if (!firstChild->isNonNegative() && !node->getOpCode().isUnsigned()) {
        ...
        // load and test
        generateRRInstruction(
          cg, TR::InstOpCode::LTR, node, targetRegister, targetRegister);
        ...
        cFlowRegionStart->setStartInternalControlFlow();
        generateS390BranchInstruction(
          cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, node, doShift);
        ...
        generateS390LabelInstruction(
          cg, TR::InstOpCode::LABEL, node, doShift, dependencies);
      ...
      if (node->getOpCode().isUnsigned()) {
        generateRSInstruction(
          cg, TR::InstOpCode::SRL, node, targetRegister, shftAmnt);
      } else {
        generateRSInstruction(
          cg, TR::InstOpCode::SRA, node, targetRegister, shftAmnt);
      }
    } else {
      ...
      if (node->getOpCode().isUnsigned())
        generateRRInstruction(
          cg, TR::InstOpCode::DLR, node, targetRegisterPair, tmpReg);
      else
        generateRRInstruction(
          cg, TR::InstOpCode::DR, node, targetRegisterPair, tmpReg);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (!isUnsigned && (value == -1)) {
      ...
      if (!isUnsigned && !firstChild->isNonNegative()) {
        ...
        TR::Register* tempRegister2 = cg->allocateRegister();
        generateRRInstruction(
          cg, TR::InstOpCode::LR, node, tempRegister2, targetRegister);
        generateRRInstruction(
          cg, TR::InstOpCode::LR, node, tempRegister1, targetRegister);
        generateRSInstruction(cg, TR::InstOpCode::SRA, node, tempRegister2, 31);
        generateRILInstruction(
          cg, TR::InstOpCode::NILF, node, tempRegister2, absValue - 1);
        generateRRInstruction(
          cg, TR::InstOpCode::AR, node, tempRegister1, tempRegister2);
        generateRILInstruction(cg,
                               TR::InstOpCode::NILF,
                               node,
                               tempRegister1,
        ...
        generateRRInstruction(
          cg, TR::InstOpCode::SR, node, targetRegister, tempRegister1);
        cg->stopUsingRegister(tempRegister1);
        cg->stopUsingRegister(tempRegister2);
      } else {
...
OMR::Z::TreeEvaluator::inegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (firstChild->getOpCodeValue() == TR::iabs &&
    ...
    if (TR::Compiler->target.is64Bit() && targetRegister->alreadySignExtended())
      generateRRInstruction(
        cg, TR::InstOpCode::LNGR, node, targetRegister, sourceRegister);
    else
      generateRRInstruction(
        cg, TR::InstOpCode::LNR, node, targetRegister, sourceRegister);
    ...
    if (TR::Compiler->target.is64Bit() && targetRegister->alreadySignExtended())
      generateRRInstruction(
        cg, TR::InstOpCode::LCGR, node, targetRegister, sourceRegister);
    else
      generateRRInstruction(
        cg, TR::InstOpCode::LCR, node, targetRegister, sourceRegister);
  }
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (rotateAmount == 0) {
      ...
      generateRSInstruction(cg,
                            nodeIs64Bit ? TR::InstOpCode::RLLG
                                        : TR::InstOpCode::RLL,
                            node,
                            targetRegister,
                            sourceRegister,
...
OMR::Z::TreeEvaluator::iandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateS390ImmOp(
      cg, TR::InstOpCode::N, node, targetRegister, targetRegister, value);

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(
      node, TR::InstOpCode::NR, TR::InstOpCode::N, TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
...
OMR::Z::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(
      node, TR::InstOpCode::NGR, TR::InstOpCode::NG, TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
...
OMR::Z::TreeEvaluator::iorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateS390ImmOp(
      cg, TR::InstOpCode::O, node, targetRegister, targetRegister, value);

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(
      node, TR::InstOpCode::OR, TR::InstOpCode::O, TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
...
OMR::Z::TreeEvaluator::lorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(
      node, TR::InstOpCode::OGR, TR::InstOpCode::OG, TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
...
OMR::Z::TreeEvaluator::ixorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateS390ImmOp(
      cg, TR::InstOpCode::X, node, targetRegister, targetRegister, value);

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(
      node, TR::InstOpCode::XR, TR::InstOpCode::X, TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
...
OMR::Z::TreeEvaluator::lxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(
      node, TR::InstOpCode::XGR, TR::InstOpCode::XG, TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
