 ...
 */
TR::RegisterPair *lnegFor32Bit(TR::Node *node, TR::CodeGenerator *cg,
                               TR::RegisterPair *targetRegisterPair,
                               TR::RegisterDependencyConditions *dep = 0) {
  TR::LabelSymbol *cFlowRegionStart = generateLabelSymbol(cg);
  TR::LabelSymbol *cFlowRegionEnd = generateLabelSymbol(cg);
  TR::RegisterDependencyConditions *localDeps = NULL;

  // If deps are passed in, we assume that there is a wider scoped set of deps
  // that are handled by the caller.   If the deps are null, we must handle deps
  // for internal control flow ourselves here.
  //
  if (!dep) {
    localDeps =
        new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 1, cg);
    localDeps->addPostCondition(targetRegisterPair->getHighOrder(),
                                TR::RealRegister::AssignAny);
  } else {
    dep->addPostConditionIfNotAlreadyInserted(
        targetRegisterPair->getHighOrder(), TR::RealRegister::AssignAny);
  }

  // Do complements on reg pair
  generateRRInstruction(cg, TR::InstOpCode::LCR, node,
  ...
  generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                TR::InstOpCode::COND_BE, node, cFlowRegionEnd);

  // Increment MS int due to overflow in LS int
  generateRIInstruction(cg, TR::InstOpCode::AHI, node,
                        targetRegisterPair->getHighOrder(), -1);
 ...
 */
TR::RegisterPair *lnegFor128Bit(TR::Node *node, TR::CodeGenerator *cg,
                                TR::RegisterPair *targetRegisterPair,
                                TR::RegisterDependencyConditions *dep = 0) {
  TR::LabelSymbol *cFlowRegionStart = generateLabelSymbol(cg);
  TR::LabelSymbol *cFlowRegionEnd = generateLabelSymbol(cg);
  ...
  // Do complements on reg pair
  generateRRInstruction(cg, TR::InstOpCode::LCGR, node,
                        targetRegisterPair->getHighOrder(),
                        targetRegisterPair->getHighOrder());
  generateRRInstruction(cg, TR::InstOpCode::LCGR, node,
                        targetRegisterPair->getLowOrder(),
  ...
  // Check to see if we need to propagate an overflow bit from low word long to
  // high word long.
  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                               cFlowRegionStart);
  cFlowRegionStart->setStartInternalControlFlow();
  ...
  // Subtract 1 from high word that was added in first LCGR if low word is not
  // 0, i.e.
  generateRIInstruction(cg, TR::InstOpCode::AGHI, node,
                        targetRegisterPair->getHighOrder(), -1);
 ...
 */
inline TR::RegisterPair *laddConst(TR::Node *node, TR::CodeGenerator *cg,
                                   TR::RegisterPair *targetRegisterPair,
                                   int64_t value,
  ...
  if (performTransformation(
          comp, "O^O Use AL/ALC to perform long add.\n")) {  // wrong carry bit
                                                             // to be set
    if (l_value != 0) {
      ...
      generateS390ImmOp(cg, TR::InstOpCode::AL, node, tempReg, lowOrder,
                        l_value, deps);
      generateS390ImmOp(cg, TR::InstOpCode::ALC, node, tempReg, highOrder,
                        h_value, deps);

      if (deps) deps->addPostCondition(tempReg, TR::RealRegister::AssignAny);
      cg->stopUsingRegister(tempReg);
    } else {
      generateS390ImmOp(cg, TR::InstOpCode::A, node, highOrder, highOrder,
                        h_value);
    }
  } else {
    // Add high value
    ...
    if (l_value != 0) {
      ...
      // Check for overflow in LS(h_value) int. If overflow, increment
      // MS(l_value) int.
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                   cFlowRegionStart);
      cFlowRegionStart->setStartInternalControlFlow();
      generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                    TR::InstOpCode::COND_MASK12, node,
                                    cFlowRegionEnd);
...
static bool genNullTestForCompressedPointers(TR::Node *node,
  ...
  if (TR::Compiler->target.is64Bit() &&
    ...
    if (n->getOpCodeValue() == TR::ladd) {
      ...
      if (n->getFirstChild()->isNonZero()) isNonZero = true;
      if (n->getFirstChild()->getOpCodeValue() == TR::a2l ||
          n->getFirstChild()->getOpCode().isShift()) {
        if (n->getFirstChild()->getFirstChild()->isNonNull()) isNonZero = true;
      }
    }
    ...
    if (!isNonZero) {
      ...
      if (addOrSubNode->getFirstChild()->getOpCode().isShift() &&
          addOrSubNode->getFirstChild()->getRegister()) {
        TR::Register *r = addOrSubNode->getFirstChild()->getRegister();
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                     cFlowRegionStart);
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::getCmpOpCode(), node, r, 0,
            TR::InstOpCode::COND_BE, skipAdd);
      } else {
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                     cFlowRegionStart);
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::getCmpOpCode(), node, targetRegister, 0,
            TR::InstOpCode::COND_BE, skipAdd);
      }
...
TR::Register *laddHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc)) {
    TR_ASSERT(!isCompressionSequence,
              "CC computation not supported with compression sequence.\n");
    ...
    // we need the carry from integerAddAnalyser for the CC sequence, thus we
    // use logical add instead of add we assume that the analyser will use the
    // opcodes provided and does not generate alternative instructions which do
    // not produce a carry.
  ...
  bool hasCompressedPointers =
      genNullTestForCompressedPointers(node, cg, targetRegister, skipAdd);

  if (hasCompressedPointers && ((secondChild->getOpCodeValue() != TR::lconst) ||
    ...
    if (!targetRegister) targetRegister = cg->allocateRegister();
    ...
    if (skipAdd) {
      ...
      if (addDepForCompressedValue)
        conditions->addPostCondition(firstChild->getRegister(),
                                     TR::RealRegister::AssignAny);
    ...
    generateS390ImmOp(cg, TR::InstOpCode::AG, node, targetRegister,
                      targetRegister, long_value);
    node->setRegister(targetRegister);
    ...
    temp.integerAddAnalyser(node, TR::InstOpCode::AGR, TR::InstOpCode::AG,
                            TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  ...
  if (bumpedRefCount) firstChild->decReferenceCount();
...
/**
 * Search backwards looking for any preceeding definition of the specified
 * registers. Look backwards iat at most searchLimit instructions.
 * @return Defining instruction if found, NULL if none found
 */
static TR::Instruction *findPreviousAGIDef(TR::Instruction *prev,
...
TR::Register *generic32BitAddEvaluator(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    switch (secondChild->getDataType()) {
      ...
      default:
        TR_ASSERT_FATAL(false,
                        "Unexpected data type (%s) in generic32BitAddEvaluator",
    ...
    if (!findPreviousAGIDef(cg->getAppendInstruction(), delay, childTargetReg,
                            comp)) {
      useLA = false;
    }
    ...
    if (useAHIK) {
      generateRIEInstruction(cg, TR::InstOpCode::AHIK, node, targetRegister,
                             childTargetReg, value);
    } else if (useLA &&
               performTransformation(
                   comp, "O^O Use LA/LAY instead of AHI for %p.\n", node)) {
      TR::MemoryReference *memRef =
          generateS390MemoryReference(childTargetReg, value, cg);
      ...
      generateS390ImmOp(cg, TR::InstOpCode::A, node, targetRegister,
    ...
    temp.integerAddAnalyser(
        node, TR::InstOpCode::AR,
        type == TR::Int16 ? TR::InstOpCode::AH : TR::InstOpCode::A,
        TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
 ...
 * This generic subtrac evaluator is invoked from different evaluator functions
 * and handles all subtract types shorter than an int (i.e. byte-bsub,
 * char-csub, short-ssub, and int-isub)
 *
 * It does not handle long type.
 */
inline TR::Register *generic32BitSubEvaluator(TR::Node *node,
                                              TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    switch (secondChild->getDataType()) {
      ...
      default:
        TR_ASSERT_FATAL(false,
                        "Unexpected data type (%s) in generic32BitSubEvaluator",
                        secondChild->getDataType().toString());
        break;
    }
  ...
  } else  // The second child of the sub node isn't a loadConstant type.
  {
    ...
    TR::DataType type = node->getDataType();
    temp.intBinaryAnalyser(
        node, TR::InstOpCode::SR,
        type == TR::Int16 ? TR::InstOpCode::SH : TR::InstOpCode::S);
    targetRegister = node->getRegister();
  }
 ...
 */
inline TR::Register *lDivRemGenericEvaluator64(TR::Node *node,
                                               TR::CodeGenerator *cg,
                                               bool isDivision) {
  ...
  if (firstChild == secondChild) {
    ...
    generateRIInstruction(cg, TR::InstOpCode::LGHI, node, returnRegister,
                          retValue);
  ...
  if (isUnsigned && isConstDenominator) {
    ...
    handleConstDenominator =
        (denominator == 1) ||
        // use denoninator > 0 test to bypass the code
        (denominator > 0 &&
         TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(denominator) &&
         isDivision);
  }
  ...
  if (handleConstDenominator) {
    ...
    if (denominator == -1) {
      ...
      if (!isDivision)  // remainder
      {
        ...
        // special case, need to check if dividend is 0x8000000000000000 - but
        // need to check it in 2 steps
        generateS390ImmOp(cg, TR::InstOpCode::CG, node, firstRegister,
                          firstRegister,
        ...
        generateRRInstruction(cg, TR::InstOpCode::LCGR, node, firstRegister,
                              firstRegister);
    ...
    // case A/(+-)2^x - division is done using shifts
    else if ((shiftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(
                  denominator)) > 0) {
      TR::Register *firstRegister = NULL;

      int64_t absValueOfDenominator =
          denominator > 0 ? denominator : -denominator;

      if (isDivision) {
        firstRegister = cg->gprClobberEvaluate(firstChild);

        // This adjustment only applies to signed division as we are effectively
        // trying to skirt the sign bit during the shift operation below
        if (!isUnsigned && !firstChild->isNonNegative()) {
          ...
          cFlowRegionStart->setStartInternalControlFlow();
          generateS390CompareAndBranchInstruction(
              cg, TR::InstOpCode::CG, node, firstRegister, 0,
          ...
          TR::RegisterDependencyConditions *deps = new (cg->trHeapMemory())
              TR::RegisterDependencyConditions(0, 2, cg);
          deps->addPostCondition(firstRegister, TR::RealRegister::AssignAny);
          generateS390ImmOp(cg, TR::InstOpCode::AG, node, firstRegister,
                            firstRegister, absValueOfDenominator - 1, deps);
          generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, skipSet,
                                       deps);
        ...
        // divide
        if (isUnsigned)
          generateRSInstruction(cg, TR::InstOpCode::SRLG, node, firstRegister,
                                firstRegister, shiftAmnt);
        else
          generateRSInstruction(cg, TR::InstOpCode::SRAG, node, firstRegister,
                                firstRegister, shiftAmnt);
      ...
      } else  // remainder
      {
        ...
        if (!isUnsigned && !firstChild->isNonNegative()) {
          ...
          generateRSInstruction(cg, TR::InstOpCode::SRAG, node, tempRegister2,
                                firstRegister, 63);
          generateRRInstruction(cg, TR::InstOpCode::LGR, node, tempRegister1,
                                firstRegister);
          generateRILInstruction(
              cg, TR::InstOpCode::NIHF, node, tempRegister2,
              static_cast<int32_t>((absValueOfDenominator - 1) >> 32));
          generateRILInstruction(
              cg, TR::InstOpCode::NILF, node, tempRegister2,
              static_cast<int32_t>(absValueOfDenominator - 1));
          generateRRInstruction(cg, TR::InstOpCode::AGR, node, firstRegister,
                                tempRegister2);
          if (denominator != static_cast<int32_t>(absValueOfDenominator))
            generateRILInstruction(
                cg, TR::InstOpCode::NIHF, node, firstRegister,
                static_cast<int32_t>((CONSTANT64(0xFFFFFFFFFFFFFFFF) -
                                      absValueOfDenominator + 1) >>
                                     32));
          generateRILInstruction(
              cg, TR::InstOpCode::NILF, node, firstRegister,
              static_cast<int32_t>(CONSTANT64(0xFFFFFFFFFFFFFFFF) -
                                   absValueOfDenominator + 1));
          generateRRInstruction(cg, TR::InstOpCode::SGR, node, tempRegister1,
                                firstRegister);
          generateRRInstruction(cg, TR::InstOpCode::LGR, node, firstRegister,
                                tempRegister1);
          cg->stopUsingRegister(tempRegister1);
          cg->stopUsingRegister(tempRegister2);
        } else {
          generateShiftAndKeepSelected64Bit(node, cg, firstRegister,
                                            firstRegister, 64 - shiftAmnt, 63,
                                            0, true, false);
        }
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, done,
                                     deps);
      }
  ...
  TR::RegisterPair *targetRegisterPair =
      cg->allocateConsecutiveRegisterPair(quoRegister, remRegister);
  TR::RegisterDependencyConditions *dependencies = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(0, numPostConditions, cg);
  dependencies->addPostCondition(targetRegisterPair,
                                 TR::RealRegister::EvenOddPair);
  dependencies->addPostCondition(remRegister,
  ...
  if (!doConditionalRemainder && !node->isSimpleDivCheck() && !isUnsigned &&
    ...
    //  We have to check for a special case where we do
    //  0x8000000000000000/0xffffffffffffffff JAVA requires we return
    //  0x8000000000000000 rem 0x00000000
    generateS390ImmOp(cg, TR::InstOpCode::CG, node, quoRegister, quoRegister,
                      (int64_t)CONSTANT64(0x8000000000000000));
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BNE, node, doDiv);
    generateS390ImmOp(cg, TR::InstOpCode::CG, node, sourceRegister,
                      sourceRegister, -1);
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BNE, node, doDiv);
    ...
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BRC, node, skipDiv);
  ...
  if (doConditionalRemainder) {
    ...
    if (skipDiv == NULL) skipDiv = generateLabelSymbol(cg);
    ...
    if (firstChild->isNonNegative()) {
      ...
      // LGPR is needed so negative numbers do not always take the slow path
      // after the logical compare (but functionally it is not needed)
      generateRRInstruction(cg, TR::InstOpCode::LPGR, node, absDividendReg,
                            remRegister);
    }
    ...
    if (value <= TR::getMaxSigned<TR::Int32>()) {
      generateRILInstruction(cg, TR::InstOpCode::CLGFI, node, absDividendReg,
                             static_cast<int32_t>(value));
    } else {
      generateRRInstruction(cg, TR::InstOpCode::CLGR, node, absDividendReg,
                            sourceRegister);
    }
    TR::LabelSymbol *cFlowRegionStart = generateLabelSymbol(cg);
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
  ...
  if (isUnsigned) {
    if (doConditionalRemainder)
      generateRRInstruction(
          cg, TR::InstOpCode::LGR, node, quoRegister,
          remRegister);  // in doConditionalRemainder case this is the slow path
                         // so do move here
    generateRIInstruction(cg, TR::InstOpCode::LGHI, node, remRegister, 0);
    generateRRInstruction(cg, TR::InstOpCode::DLGR, node, targetRegisterPair,
                          sourceRegister);
  } else {
    if (doConditionalRemainder)
      generateRRInstruction(
          ...
          remRegister);  // in doConditionalRemainder case this is the slow path
                         // so do move here
    generateRRInstruction(cg, TR::InstOpCode::DSGR, node, targetRegisterPair,
                          sourceRegister);
  }

  // Label to skip the division
  if (skipDiv != NULL) {
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, skipDiv,
                                 dependencies);
  } else {
    generateS390PseudoInstruction(cg, TR::InstOpCode::DEPEND, node,
                                  dependencies);
  }
 ...
 *               = false we return the remainder register.
 */
TR::Register *iDivRemGenericEvaluator(TR::Node *node, TR::CodeGenerator *cg,
                                      bool isDivision,
  ...
  if (firstChild == secondChild) {
    ...
    generateRIInstruction(cg, TR::InstOpCode::LHI, node, returnRegister,
                          retValue);
  ...
  // TODO(#3685): Remove this Java-ism from OMR and push it into OpenJ9
  bool needCheck =
      (!node->isSimpleDivCheck() && !node->getOpCode().isUnsigned() &&
       !firstChild->isNonNegative() && !secondChild->isNonNegative());
  ...
  if (divchkDivisorMR && !needCheck) {
  ...
  } else if (secondChild->getRegister() == NULL &&
    ...
    if (divchkDivisorMR) {
      TR::MemoryReference *tempMR =
          generateS390MemoryReference(*divchkDivisorMR, 0, cg);
      TR::DataType dtype = secondChild->getType();
      sourceRegister = cg->allocateRegister();
      generateRXInstruction(
          cg, dtype.isInt64() ? TR::InstOpCode::LG : TR::InstOpCode::L,
          secondChild, sourceRegister, tempMR);
  ...
  TR::RegisterDependencyConditions *dependencies = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(0, sourceMR ? 6 : 4, cg);
  dependencies->addPostCondition(targetRegisterPair,
                                 TR::RealRegister::EvenOddPair);
  dependencies->addPostCondition(remRegister,
                                 TR::RealRegister::LegalEvenOfPair);
  ...
  if (sourceRegister)
    dependencies->addPostConditionIfNotAlreadyInserted(
        sourceRegister, TR::RealRegister::AssignAny);
  if (sourceMR && sourceMR->getBaseRegister() &&
      !sourceMR->getBaseRegister()->getRealRegister())
    dependencies->addPostConditionIfNotAlreadyInserted(
        sourceMR->getBaseRegister(), TR::RealRegister::AssignAny);
  if (sourceMR && sourceMR->getIndexRegister() &&
      !sourceMR->getIndexRegister()->getRealRegister())
    dependencies->addPostConditionIfNotAlreadyInserted(
        sourceMR->getIndexRegister(), TR::RealRegister::AssignAny);
  ...
  if (needCheck) {
    ...
    generateS390ImmOp(cg, TR::InstOpCode::C, node, remRegister, remRegister,
                      (int32_t)0x80000000);
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                 cFlowRegionStart);
    cFlowRegionStart->setStartInternalControlFlow();
    ...
    cursor = generateRRInstruction(cg, TR::InstOpCode::LR, node, quoRegister,
                                   remRegister);
    ...
    // TODO: Can we allow setting the condition code here by moving the load
    // before the compare?
  ...
  if (node->getOpCode().isUnsigned())
    generateRSInstruction(cg, TR::InstOpCode::SRDL, node, targetRegisterPair,
                          32);
  else
    generateRSInstruction(cg, TR::InstOpCode::SRDA, node, targetRegisterPair,
                          32);

  // Do division
  if (sourceMR) {
    if (node->getOpCode().isUnsigned())
      generateRXInstruction(cg, TR::InstOpCode::DL, node, targetRegisterPair,
                            sourceMR);
    else
      generateRXInstruction(cg, TR::InstOpCode::D, node, targetRegisterPair,
                            sourceMR);
  } else {
    if (node->getOpCode().isUnsigned())
      generateRRInstruction(cg, TR::InstOpCode::DLR, node, targetRegisterPair,
                            sourceRegister);
    else
      generateRRInstruction(cg, TR::InstOpCode::DR, node, targetRegisterPair,
                            sourceRegister);
  }
...
inline TR::Register *genericLongShiftSingle(TR::Node *node,
                                            TR::CodeGenerator *cg,
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
      ...
      if (node->getOpCodeValue() == TR::lshl) {
        if (firstChild->getOpCodeValue() == TR::i2l &&
          ...
          generateRIEInstruction(cg, mnemonic, node, trgReg, srcReg,
                                 (int8_t)(32 - value),
          ...
          if (trgReg =
                  TR::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(
                      firstChild, cg, value,
      ...
      } else if (node->getOpCodeValue() == TR::lshr ||
                 node->getOpCodeValue() == TR::lushr) {
    ...
    if ((shiftOp == TR::InstOpCode::SRLG || shiftOp == TR::InstOpCode::SLLG) &&
        ...
        secondChild->getOpCode().isAnd() &&
        secondChild->getOpCode().isInteger() &&
    ...
    tempMR->stopUsingMemRefRegister(cg);
    if (skippedAnd) cg->decReferenceCount(secondChild->getFirstChild());
  }
...
inline TR::Register *genericIntShift(TR::Node *node, TR::CodeGenerator *cg,
                                     TR::InstOpCode::Mnemonic shiftOp,
  ...
  // If we are on z/Architecture, then we can use SLLG even in 31-bit mode for
  // doing a shift. Only generate SLLG over SLL if it saves an instruction - an
  // LR to copy a register - because we need a non-destructive op. Same goes for
  // SLA/SLAG. Right shift may be dangerous as upper half of register may have
  // garbage hense don't use SRLG, SRAG.

  if (shiftOp == TR::InstOpCode::SRL || shiftOp == TR::InstOpCode::SRA) {
    canUseAltShiftOp = altShiftOp == TR::InstOpCode::SRLK ||
                       altShiftOp == TR::InstOpCode::SRAK;
  }
  ...
  if (node->getOpCodeValue() == TR::bushr) {
    if (node->getFirstChild()->getOpCodeValue() == TR::bconst &&
      ...
      generateLoad32BitConstant(cg, node->getFirstChild(),
                                node->getFirstChild()->getByte() & 0xFF, srcReg,
                                true);
    } else {
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    if (shiftAmount != 0) {
      if (trgReg != srcReg && canUseAltShiftOp) {
        generateRSInstruction(cg, altShiftOp, node, trgReg, srcReg,
                              shiftAmount);
      } else {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iaddEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::laddEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
...
TR::Register *genericRotateLeft(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    if (node->getOpCodeValue() == TR::lor) {
      ...
      if (lushr && lushr->getOpCodeValue() == TR::lushr &&
          lushr->getReferenceCount() == 1 && lushr->getRegister() == NULL &&
          lshl->getOpCodeValue() == TR::lshl &&
          lshl->getReferenceCount() == 1 && lshl->getRegister() == NULL) {
        ...
        if (rShftAmnt + lShftAmnt == 64 && lshlSourceNode == lushrSourceNode) {
          ...
          TR::InstOpCode::Mnemonic opCode =
              TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)
                  ? TR::InstOpCode::RISBGN
                  : TR::InstOpCode::RISBG;
          generateRIEInstruction(cg, opCode, node, targetReg, sourceReg, 0, 63,
                                 lShftAmnt);
    ...
    if (node->getOpCodeValue() == TR::lor && andChild &&
        andChild->getRegister() == NULL && andChild->getReferenceCount() == 1) {
      TR::Node *data = NULL;
      uint64_t shiftBy = 0;
      uint64_t mask1 = 0;
      uint64_t mask2 = 0;
      ...
      if (mask1 && mask1 == (~mask2) && shiftBy <= 63 &&
          performTransformation(
              cg->comp(), "O^O Insert bit using RISBG node [%p]\n", node)) {
        ...
        generateRIEInstruction(cg, TR::InstOpCode::RISBG, node, targetReg,
                               fromReg, bitPos, bitPos, shiftBy);
        ...
        if (mask1 == mask2 &&
            performTransformation(
                cg->comp(),
                "O^O Insert contiguous bits using RISBG node [%p]\n", node)) {
          TR::Register *toReg = cg->evaluate(andChild->getFirstChild());
          TR::Register *fromReg = cg->evaluate(shiftChild->getFirstChild());
          TR::Register *targetReg = cg->allocateRegister();
          generateRRInstruction(cg, TR::InstOpCode::LGR, node, targetReg,
                                toReg);
          generateRIEInstruction(cg, TR::InstOpCode::RISBG, node, targetReg,
                                 fromReg, 0, bitPos, shiftBy);
    ...
    if (node->getOpCodeValue() == TR::lor ||
      ...
      if (shiftChild &&
          ...
          performTransformation(cg->comp(),
                                "O^O Combine or/shift into rotate node [%p]\n",
                                node)) {
        ...
        if (node->getOpCodeValue() == TR::lxor) opcode = TR::InstOpCode::RXSBG;
        generateRRInstruction(cg, TR::InstOpCode::LGR, node, targetReg, toReg);
        generateRIEInstruction(cg, opcode, node, targetReg, fromReg, firstBit,
                               lastBit, shiftBy);
        cg->decReferenceCount(shiftChild->getFirstChild());
        cg->decReferenceCount(shiftChild->getSecondChild());
        return targetReg;
 ...
 * Rotate and insert
 */
TR::Register *genericRotateAndInsertHelper(TR::Node *node,
                                           TR::CodeGenerator *cg) {
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
    ...
    if (firstChild->getOpCode().isConversion()) {
      ...
      if (!firstChild->getFirstChild()
               ->getType()
               .isIntegral())  // only skip integer to integer conversions
      {
        return NULL;
      }
    ...
    if (node->getOpCode().isAnd() &&
      ...
      switch (secondChild->getDataType()) {
        case TR::Address:
          TR_ASSERT(TR::Compiler->target.is32Bit(),
                    "genericRotateAndInsertHelper: unexpected data type");
        case TR::Int64:
          value = (uint64_t)secondChild->getLongInt();
          break;
        case TR::Int32:
          value = ((uint64_t)secondChild->getInt()) &
                  0x00000000FFFFFFFFL;  // avoid sign extension
          break;
        case TR::Int16:
      ...
      int useOrder = 0;  // For l2i conversions in 31-bit, 1 = lowOrder, 2 =
                         // highOrder, 0 = neither
      ...
      if (shiftMsBit >
          msBit)  // Conditions for when we don't need any sign extension
      {
        if (firstChild->getFirstChild()->isNonNegative() ||
            firstChild->chkUnsigned() ||
            firstChild->getOpCodeValue() == TR::lushr)
          msBit = shiftMsBit;
        else {
          traceMsg(comp,
                   "Cannot use RISBG, number could be negative, no sign "
                   "extension available for RISBG\n");
          return NULL;
        }
      }
      if (lsBit > shiftLsBit) lsBit = shiftLsBit;

      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp,
                 "[%p] and/sh[r,l] => rotated-and-insert: tZeros %d, lZeros "
                 "%d, popCnt %d\n",
                 node, tZeros, lZeros, popCnt);
        traceMsg(comp,
                 "\t               => rotated-and-insert: shiftMsBit %d, "
                 "shiftLsBit %d \n",
                 shiftMsBit, shiftLsBit);
        traceMsg(
            comp,
            "\t               => rotated-and-insert: msBit %d, lsBit %d \n",
            msBit, lsBit);
      }
      ...
      if (popCnt == (64 - lZeros - tZeros)) {
        ...
        if (msBit > lsBit) {
          if (node->getType().isInt64()) {
            generateRRInstruction(cg, TR::InstOpCode::XGR, node, targetReg,
                                  targetReg);
          } else {
            generateRRInstruction(cg, TR::InstOpCode::XR, node, targetReg,
                                  targetReg);
          }
        } else if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196) &&
                   !node->getType().isInt64()) {
          generateRIEInstruction(cg, TR::InstOpCode::RISBLG, node, targetReg,
                                 sourceReg, msBit, 0x80 + lsBit, shiftAmnt);
        } else {
          auto mnemonic =
              TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)
                  ? TR::InstOpCode::RISBGN
          ...
          generateRIEInstruction(cg, mnemonic, node, targetReg, sourceReg,
                                 msBit, 0x80 + lsBit, shiftAmnt);
        }
...
TR::Register *OMR::Z::TreeEvaluator::tryToReplaceShiftLandWithRotateInstruction(
    TR::Node *node, TR::CodeGenerator *cg, int32_t shiftAmount,
    bool isSignedShift) {
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    TR::Node *firstChild = node->getFirstChild();
    TR::Node *secondChild = node->getSecondChild();
    ...
    // Given the right cases, transform land into RISBG as it may be better than
    // discrete pair of ands
    if (node->getOpCodeValue() == TR::land &&
        secondChild->getOpCode().isLoadConst()) {
      uint64_t longConstValue = secondChild->getLongInt();

      // RISBG instructions perform unsigned rotation, so if we are selecting
      // the sign bit via the logical AND we cannot perform a signed shift
      // because we must preserve the sign bit. In general we cannot handle this
      // case because we are not certain our input will be non-negative, hence
      // we must conservatively disallow the optimization in such a case.
      if (longConstValue >= 0x8000000000000000LL && shiftAmount != 0 &&
          isSignedShift) {
        return NULL;
      }
      // The Shift amount operand in RISBG only holds 6 bits so the optimization
      // will not work on shift amounts >63 or < 63.
      if (shiftAmount > 63 || shiftAmount < -63) {
        return NULL;
      }
      // If the node entered is a common node, then there is no guarantee that
      // the child of the common node will still be alive. And when we try to
      // access the value in that node, we might receive values from a node that
      // might've been killed already. Thus, the optimization will not work.
      ...
      // because a popCnt=0 means there are no overlapping bits between the two
      // AND operands. We cannot generate a RISBG for this case as RISBG cannot
      // be used to zero out the contents of the entire register. The starting
      // and ending positions are inclusive, so even if start=end, we will
      // preserve at least one bit.
      if (popCnt == 0) return NULL;

      // the minimum value for the bit positions is 0, and lsBit can equal msBit
      // iff popCnt = 1 if popCnt == 0 then we cannot generate a RISBG
      // instruction because RISBG preserves at least one bit position
      int32_t msBit = 0;
      int32_t lsBit = 0;

      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp,
                 "[%p] land => rotated-and-insert: tZeros %d, lZeros %d, "
                 "popCnt %d\n",
                 node, tZeros, lZeros, popCnt);
      ...
      // We now try to detect if the lconst's bit pattern is such that we can
      // use a single RISBG instead of multiple AND instructions

      // The first block will catch all cases where the lConstValue is a series
      // of contiguous ones surrounded by zeros ex: 0001111000, 011110, 111111,
      // 001111, 11100 etc...
      //
      // The 'else if' block will consider all cases where we have a series of
      // contiguous zeros surrounded by ones ex: 1110000111, 1000001
      if (popCnt == (64 - lZeros - tZeros)) {
        msBit = lZeros;
        lsBit = 63 - tZeros;
        // The below checks make sure we are not better off using one of Z
        // architecture's 16-bit AND immediate instructions (NILF,NILH,NILL,
        // NIHF,NIHH, NIHL). Details for each check below:
        //
        // "firstChild->getReferenceCount() > 1"
        //    If the firstChild's refCount>1, we will need to clobber evaluate
        //    this case because we cannot destroy the value. In such a case
        //    we're better off with RISBG because RISBG won't clobber anything
        //    and hence can do the operation faster.
        //
        // "|| lZeros > 31 || tZeros > 31"
        //    If there are more than 31 contiguous zeros, then we must use RISBG
        //    because no NI** instruction do zero out that many bits in a single
        //    instruction.
        //
        // "|| (lZeros > 0 && tZeros > 0"
        //    NI** instructions need to explicitly specify which bit positions
        //    to zero out. If our constant at minimum looks like 0111...110
        //    (there are zeros in the top half and bottom half of the register),
        //    then it's better to use RISBG. This is because there are no NI**
        //    instructions allowing us to specify bits in the top half and
        //    bottom half of the register to zero out.

        if (firstChild->getReferenceCount() > 1 || lZeros > 31 || tZeros > 31 ||
            (lZeros > 0 && tZeros > 0)) {
          // If the shift amount is NOT zero, then we must account for the
          // shifted range [1] If the resultant shifted msb in a right shift is
          // shifted "too far" to the right, then we are basically zeroing the
          // register. This optimization will not be used. [2] Same case if the
          // resultant lsb of a left shift is shifted out of range. Eg[1]:
          // 000...0011 >> 3. msb = 62, lsb = 63. If we right shift, then msb
          // and lsb will both be > 63 (out of range) Eg[2]: 11100...00 << 10.
          // msb = 0, lsb = 2. If we left shift, then msb and lsb will both be <
          // 0 (out of range)
          if ((shiftAmount < 0 && msBit - shiftAmount > 63) ||
              (shiftAmount > 0 && lsBit - shiftAmount < 0)) {
            return NULL;
          }
        ...
        //
        // "(lOnes < 32 && tOnes < 32)"
        //    This case implies there are zeros at the high/low boundary of the
        //    register. Example value: 11111000011111. The NI** instructions can
        //    only operate on the top half or bottom half of a register at a
        //    time. So such a case will require two NI** instructions. Hence we
        //    are better off using a single RISBG
        //
        // "shiftAmount == 0"
        //    For cases where we have zeros surrounded by one, if the shift
        //    value is not zero then the optimization will not work.

        if ((firstChild->getReferenceCount() > 1 ||
             (lOnes < 32 && tOnes < 32)) &&
            shiftAmount == 0) {
          // Similar to above, if the shift amount is NOT zero, then we must
          // account for the shifted range
          if ((shiftAmount < 0 && msBit - shiftAmount > 63) ||
              (shiftAmount > 0 && lsBit - shiftAmount < 0)) {
            return NULL;
          }
      ...
      if (doTransformation &&
          performTransformation(
              comp, "O^O Use RISBG instead of 2 ANDs for %p.\n", node)) {
        TR::Register *targetReg = cg->allocateRegister();
        TR::Register *sourceReg = cg->evaluate(firstChild);

        // if possible then use the instruction that doesn't set the CC as it's
        // faster
        TR::InstOpCode::Mnemonic opCode =
            TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)
                ? TR::InstOpCode::RISBGN
        ...
        // If the shift amount is zero, this instruction sets the rotation
        // factor to 0 and sets the zero bit(0x80). So it's effectively zeroing
        // out every bit except the inclusive range of lsBit to msBit. The bits
        // in that range are preserved as is.
        //
        // If the shift amount is NOT zero, then we must account for the shifted
        // range [1] If the shifted lsb of a left shift is still in range, but
        // the msb is out of range, then we can still "capture" the shifted bits
        // by upperbounding the msb. [2] Similar case if the msb of a right
        // shift is still in range but the lsb is out of range.
        //
        // Eg[1]: 00...0111 >> 2. msb = 61, lsb = 63. If we right shift, we get
        // 000...0001, so we can still preserve the last bit. Eg[2]: 1110...00
        // << 2. msb = 0, lsb = 2. If we left left, we can still preserve the
        // first bit.
        ...
        generateRIEInstruction(cg, opCode, node, targetReg, sourceReg,
                               rangeStart, 0x80 + rangeEnd, shiftAmount);
        cg->decReferenceCount(firstChild);
        cg->decReferenceCount(secondChild);
        return targetReg;
      }
...
TR::Register *lsubHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb)) {
    ...
    // and we assume that the analyser does not generate alternative
    // instructions which do not produce a borrow.
  ...
  if (hasCompressedPointers && ((secondChild->getOpCodeValue() != TR::lconst) ||
    ...
    secondRegister = cg->evaluate(secondChild);
    generateRRInstruction(cg, TR::InstOpCode::SGR, node, targetRegister,
                          secondRegister);
    ...
    if (!targetRegister) targetRegister = cg->gprClobberEvaluate(firstChild);
    ...
    if (!targetRegister) targetRegister = cg->gprClobberEvaluate(secondChild);

    generateRRInstruction(cg, TR::InstOpCode::LCGR, secondChild, targetRegister,
                          targetRegister);
    generateS390ImmOp(cg, TR::InstOpCode::AG, node, targetRegister,
                      targetRegister, long_value);
  ...
  if (bumpedRefCount) firstChild->getFirstChild()->decReferenceCount();
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::dualMulHelper64(TR::Node *node,
                                                     TR::Node *lmulNode,
                                                     TR::Node *lumulhNode,
                                                     TR::CodeGenerator *cg) {
  ...
  //
  // MLGR R1, R2 does this, where targetHigh = R1, targetLow = R1+1, firstChild
  // = R1+1, secondChild = R2
  //
  ...
  TR::RegisterPair *trgtRegPair = cg->allocateConsecutiveRegisterPair(
  ...
  dependencies->addPostCondition(trgtRegPair->getHighOrder(),
                                 TR::RealRegister::LegalEvenOfPair);
  dependencies->addPostCondition(trgtRegPair->getLowOrder(),
                                 TR::RealRegister::LegalOddOfPair);

  // do multiplication
  // TODO: sometimes reg-mem may be more efficient?
  cursor = generateRRInstruction(cg, TR::InstOpCode::MLGR, node, trgtRegPair,
                                 secondRegister);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::dualMulEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
...
TR::Register *lmulHelper64(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (!secondChildIsConstant && firstChild->isHighWordZero() &&
      !secondChild->isHighWordZero()) {
    // swap children pointers since optimizer puts highWordZero children as
    // firstChild if the second one is non-constant and non-highWordZero
  ...
  if (secondChildIsConstant) {
    ...
    if (create_LA) {
      ...
      generateS390ImmOp(cg, TR::InstOpCode::MSG, node, sourceRegister,
                        targetRegister, value);
    }
    ...
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::MSGR, TR::InstOpCode::MSG,
                         TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::baddEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return generic32BitAddEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::saddEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return generic32BitAddEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::caddEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return generic32BitAddEvaluator(node, cg);
}
 ...
 * (child1 - child2)
 */
TR::Register *OMR::Z::TreeEvaluator::isubEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 * (child1 - child2)
 */
TR::Register *OMR::Z::TreeEvaluator::lsubEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return lsubHelper64(node, cg);
}
 ...
 * (child1 - child2)
 */
TR::Register *OMR::Z::TreeEvaluator::bsubEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return generic32BitSubEvaluator(node, cg);
}
 ...
 * (child1 - child2)
 */
TR::Register *OMR::Z::TreeEvaluator::ssubEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return generic32BitSubEvaluator(node, cg);
}
 ...
 * (child1 - child2)
 */
TR::Register *OMR::Z::TreeEvaluator::csubEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return generic32BitSubEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  bool needsUnsignedHighMulOnly =
      (node->getOpCodeValue() == TR::lumulh) && !node->isDualCyclic();
  ...
  TR::RegisterPair *targetRegisterPair =
      cg->allocateConsecutiveRegisterPair(sourceRegister, targetRegister);

  dependencies->addPostCondition(targetRegisterPair,
                                 TR::RealRegister::EvenOddPair);
  dependencies->addPostCondition(targetRegister,
                                 TR::RealRegister::LegalEvenOfPair);
  dependencies->addPostCondition(sourceRegister,
                                 TR::RealRegister::LegalOddOfPair);
  ...
  if (secondChild->getOpCodeValue() == TR::lconst) {
    ...
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node,
                                 cFlowRegionStart);
    cFlowRegionStart->setStartInternalControlFlow();
    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node,
                                            sourceRegister, 0,
                                            TR::InstOpCode::COND_BNL, posMulh);

    // Negative first child, do complements on register
    generateRRInstruction(cg, TR::InstOpCode::LCGR, node, sourceRegister,
                          sourceRegister);

    // MLG
    generateS390ImmOp(cg, TR::InstOpCode::MLG, node, targetRegisterPair,
                      targetRegisterPair, absValue, dependencies);

    // Undo earlier complement
    targetRegisterPair =
        lnegFor128Bit(node, cg, targetRegisterPair, dependencies);

    // Branch to done
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BRC, node, doneMulh);
    ...
    // Label for done
    generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, doneMulh,
                                 dependencies);
    ...
    TR_ASSERT(0,
              "TR::TreeEvaluator::lmulhEvaluator -- RegReg not implemented for "
              "lmulh.");
  }
 ...
 *
 */
TR::Register *OMR::Z::TreeEvaluator::mulhEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  ...
  TR::RegisterPair *targetRegisterPair =
      cg->allocateConsecutiveRegisterPair(firstRegister, targetRegister);
  dependencies->addPostCondition(targetRegisterPair,
                                 TR::RealRegister::EvenOddPair);
  dependencies->addPostCondition(targetRegister,
                                 TR::RealRegister::LegalEvenOfPair);
  dependencies->addPostCondition(firstRegister,
                                 TR::RealRegister::LegalOddOfPair);

  if (secondChild->getOpCode().isLoadConst() && secondChild->getInt() == 0) {
    cursor = generateRRInstruction(cg, TR::InstOpCode::XR, node, targetRegister,
                                   targetRegister);
  } else {
    TR::Register *secondRegister = cg->evaluate(secondChild);
    if (isUnsigned) {
      cursor = generateRREInstruction(cg, TR::InstOpCode::MLR, node,
                                      targetRegisterPair, secondRegister);
    } else {
      cursor = generateRRInstruction(cg, TR::InstOpCode::MR, node,
                                     targetRegisterPair, secondRegister);
    }
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::imulEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    bool create_LA = false;
    if ((firstChild->getRegister() != NULL) && (value == 2) &&
        (node->isNonNegative() && TR::Compiler->target.is64Bit())) {
      create_LA = true;
    }
    ...
    if (create_LA) {
      ...
      generateS390ImmOp(cg, TR::InstOpCode::MS, node, sourceRegister,
                        targetRegister, value);
    }

    // This hack is to compensate bad IL tree from escape analysis to compute
    // hashcode
    //  imul
    //     loadAddr
    //     iconst
    // When refCount is 1, We can't use srcReg as targetReg, because the
    // targetReg must not be collected reference reg
    if (targetRegister->containsCollectedReference()) {
      TR::Register *tmpReg = cg->allocateRegister();
      generateRRInstruction(cg, TR::InstOpCode::LR, node, tmpReg,
                            targetRegister);
    ...
    TR::MemoryReference *tmpMR =
        generateS390MemoryReference(halfwordNode->getFirstChild(), cg);
    ...
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::MSR, TR::InstOpCode::MS,
                         TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lmulEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  bool needsUnsignedHighMulOnly =
      (node->getOpCodeValue() == TR::lumulh) && !node->isDualCyclic();
  if (node->isDualCyclic() || needsUnsignedHighMulOnly) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bmulEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::smulEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return NULL;
}
 ...
 * (child1 / child2)
 */
TR::Register *OMR::Z::TreeEvaluator::idivEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *targetRegister = NULL;
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (value == -1 && !node->getOpCode().isUnsigned()) {
      ...
      generateS390ImmOp(cg, TR::InstOpCode::C, node, targetRegister,
                        targetRegister, (int32_t)0x80000000);
      ...
      generateRRInstruction(cg, TR::InstOpCode::LCR, node, targetRegister,
                            targetRegister);
      ...
      skipDiv->setEndInternalControlFlow();
                    ...
                    TR::TreeEvaluator::checkNonNegativePowerOfTwo(value)) > 0) {
      // Strength reduction
      TR::LabelSymbol *cFlowRegionStart = generateLabelSymbol(cg);
      ...
      if (!firstChild->isNonNegative() && !node->getOpCode().isUnsigned()) {
        ...
        // load and test
        generateRRInstruction(cg, TR::InstOpCode::LTR, node, targetRegister,
                              targetRegister);
        ...
        cFlowRegionStart->setStartInternalControlFlow();
        generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                      TR::InstOpCode::COND_BNL, node, doShift);

        // add (divisor-1) before the shift if negative value
        generateS390ImmOp(cg, TR::InstOpCode::A, node, targetRegister,
                          targetRegister, value - 1, dependencies);

        // Label to do the shift
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, doShift,
                                     dependencies);
      ...
      if (node->getOpCode().isUnsigned()) {
        generateRSInstruction(cg, TR::InstOpCode::SRL, node, targetRegister,
                              shftAmnt);
      } else {
        generateRSInstruction(cg, TR::InstOpCode::SRA, node, targetRegister,
                              shftAmnt);
      }
    } else {
      ...
      targetRegisterPair =
          cg->allocateConsecutiveRegisterPair(quoRegister, targetRegister);
      ...
      if (node->getOpCode().isUnsigned())
        generateRSInstruction(cg, TR::InstOpCode::SRDL, node,
                              targetRegisterPair, 32);
      else
        generateRSInstruction(cg, TR::InstOpCode::SRDA, node,
                              targetRegisterPair, 32);
      ...
      if (useTmp) cg->stopUsingRegister(tmpReg);
    }
 ...
 * (child1 / child2)
 */
TR::Register *OMR::Z::TreeEvaluator::ldivEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 * (child1 / child2)
 */
TR::Register *OMR::Z::TreeEvaluator::bdivEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return NULL;
}
 ...
 * (child1 / child2)
 */
TR::Register *OMR::Z::TreeEvaluator::sdivEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return NULL;
}
 ...
 * (child1 % child2)
 */
TR::Register *OMR::Z::TreeEvaluator::iremEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *targetRegister = NULL;
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (!isUnsigned && (value == -1)) {
      ...
      generateRRInstruction(cg, TR::InstOpCode::XR, node, targetRegister,
                            targetRegister);
      ...
      if (!isUnsigned && !firstChild->isNonNegative()) {
        ...
        TR::Register *tempRegister1 = cg->allocateRegister();
        TR::Register *tempRegister2 = cg->allocateRegister();
        generateRRInstruction(cg, TR::InstOpCode::LR, node, tempRegister2,
                              targetRegister);
        generateRRInstruction(cg, TR::InstOpCode::LR, node, tempRegister1,
                              targetRegister);
        ...
        generateRILInstruction(cg, TR::InstOpCode::NILF, node, tempRegister2,
                               absValue - 1);
        generateRRInstruction(cg, TR::InstOpCode::AR, node, tempRegister1,
                              tempRegister2);
        generateRILInstruction(cg, TR::InstOpCode::NILF, node, tempRegister1,
                               0xFFFFFFFF - absValue + 1);
        generateRRInstruction(cg, TR::InstOpCode::SR, node, targetRegister,
                              tempRegister1);
        cg->stopUsingRegister(tempRegister1);
        cg->stopUsingRegister(tempRegister2);
      } else {
        generateShiftAndKeepSelected31Bit(
            node, cg, targetRegister, targetRegister, 0x3f & (32 - shftAmnt),
            31, 0, true, false);
      }
 ...
 * (child1 % child2)
 */
TR::Register *OMR::Z::TreeEvaluator::lremEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 * (child1 % child2)
 */
TR::Register *OMR::Z::TreeEvaluator::bremEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return NULL;
}
 ...
 * (child1 % child2)
 */
TR::Register *OMR::Z::TreeEvaluator::sremEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return NULL;
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::inegEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *sourceRegister;
  TR::Register *targetRegister = cg->allocateRegister();

  // ineg over iabs is an integer form of pdSetSign 0xd (negative); handle it in
  // one operation
  if (firstChild->getOpCodeValue() == TR::iabs &&
      firstChild->getReferenceCount() == 1 &&
      firstChild->getRegister() == NULL) {
    ...
    if (TR::Compiler->target.is64Bit() && targetRegister->alreadySignExtended())
      generateRRInstruction(cg, TR::InstOpCode::LNGR, node, targetRegister,
                            sourceRegister);
    else
      generateRRInstruction(cg, TR::InstOpCode::LNR, node, targetRegister,
                            sourceRegister);
             ...
             performTransformation(
                 cg->comp(),
                 "O^O Replace ineg/imul by const with imul by -const.\n")) {
    ...
    // Do complement
    if (TR::Compiler->target.is64Bit() && targetRegister->alreadySignExtended())
      generateRRInstruction(cg, TR::InstOpCode::LCGR, node, targetRegister,
                            sourceRegister);
    else
      generateRRInstruction(cg, TR::InstOpCode::LCR, node, targetRegister,
                            sourceRegister);
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lnegEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *targetRegister = NULL;
  TR::Register *sourceRegister;
  ...
  if (firstChild->getOpCodeValue() == TR::labs &&
      firstChild->getReferenceCount() == 1 &&
      firstChild->getRegister() == NULL) {
    // Load Negative
    sourceRegister = firstChild->getFirstChild()->getRegister();
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bnegEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *sourceRegister = cg->evaluate(firstChild);
  TR::Register *targetRegister = cg->allocateRegister();
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::snegEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *sourceRegister = cg->evaluate(firstChild);
  TR::Register *targetRegister = cg->allocateRegister();
  ...
  // This code path can only be generated by the optimizer, but currently it is
  // not.
 ...
 * (child1 << child2)
 */
TR::Register *OMR::Z::TreeEvaluator::ishlEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 * (child1 << child2)
 */
TR::Register *OMR::Z::TreeEvaluator::lshlEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 * (child1 << child2)
 */
TR::Register *OMR::Z::TreeEvaluator::bshlEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  auto altShiftOp = TR::InstOpCode::SLLG;
 ...
 * (child1 << child2)
 */
TR::Register *OMR::Z::TreeEvaluator::sshlEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  auto altShiftOp = TR::InstOpCode::SLLG;
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::ishrEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::lshrEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::bshrEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::InstOpCode::Mnemonic altShiftOp = TR::InstOpCode::SRAG;
  return genericIntShift(node, cg, TR::InstOpCode::SRA, altShiftOp);
}
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::sshrEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::InstOpCode::Mnemonic altShiftOp = TR::InstOpCode::SRAG;
  return genericIntShift(node, cg, TR::InstOpCode::SRA, altShiftOp);
}
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::iushrEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::lushrEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::bushrEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  auto altShiftOp = TR::InstOpCode::SRLG;
 ...
 * (child1 >> child2)
 */
TR::Register *OMR::Z::TreeEvaluator::sushrEvaluator(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  auto altShiftOp = TR::InstOpCode::SRLG;
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::integerRolEvaluator(
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (rotateAmount == 0) {
      ...
      sourceRegister = cg->evaluate(firstChild);
      targetRegister =
          nodeIs64Bit ? cg->allocateRegister() : cg->allocateRegister();
      generateRSInstruction(
          cg, nodeIs64Bit ? TR::InstOpCode::RLLG : TR::InstOpCode::RLL, node,
          targetRegister, sourceRegister, rotateAmount);
    }
  } else {
    sourceRegister = cg->evaluate(firstChild);
    targetRegister =
    ...
    generateRSInstruction(
        cg, nodeIs64Bit ? TR::InstOpCode::RLLG : TR::InstOpCode::RLL, node,
        targetRegister, sourceRegister, memRef);
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iandEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateS390ImmOp(cg, TR::InstOpCode::N, node, targetRegister,
                      targetRegister, value);

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::NR, TR::InstOpCode::N,
                         TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::landEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  // LL: if second child is long constant & bitwiseOpNeedsLiteralFromPool
  // returns false, then we don't need to put the constant in literal pool, so
  // we'll evaluate first child and use AND Immediate instructions.
  if (secondChild->getOpCodeValue() == TR::lconst) {
    targetRegister = cg->gprClobberEvaluate(firstChild);

    generateS390ImmOp(cg, TR::InstOpCode::getAndOpCode(), node, targetRegister,
                      targetRegister, secondChild->getLongInt());

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::NGR, TR::InstOpCode::NG,
                         TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bandEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sandEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::iandEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::candEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::iandEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::iorEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateS390ImmOp(cg, TR::InstOpCode::O, node, targetRegister,
                      targetRegister, value);

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::OR, TR::InstOpCode::O,
                         TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lorEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  // LL: if second child is constant & bitwiseOpNeedsLiteralFromPool function
  // call return false, then we don't need to put the constant in literal pool,
  // so we'll evaluate first child and use OR Immediate instructions.
  if (secondChild->getOpCodeValue() == TR::lconst) {
    targetRegister = cg->gprClobberEvaluate(firstChild);

    generateS390ImmOp(cg, TR::InstOpCode::getOrOpCode(), node, targetRegister,
                      targetRegister, secondChild->getLongInt());

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::OGR, TR::InstOpCode::OG,
                         TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::borEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sorEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::iorEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::corEvaluator(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::iorEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::ixorEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateS390ImmOp(cg, TR::InstOpCode::X, node, targetRegister,
                      targetRegister, value);

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::XR, TR::InstOpCode::X,
                         TR::InstOpCode::LR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::lxorEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR::Register *targetRegister = NULL;
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  // LL: if second child is constant & bitwiseOpNeedsLiteralFromPool function
  // call return false, then we don't need to put the constant in literal pool,
  // so we'll evaluate first child and use XOR Immediate instructions.
  if (secondChild->getOpCodeValue() == TR::lconst) {
    targetRegister = cg->gprClobberEvaluate(firstChild);

    generateS390ImmOp(cg, TR::InstOpCode::getXOROpCode(), node, targetRegister,
                      targetRegister, secondChild->getLongInt());

    node->setRegister(targetRegister);
  } else {
    TR_S390BinaryCommutativeAnalyser temp(cg);
    temp.genericAnalyser(node, TR::InstOpCode::XGR, TR::InstOpCode::XG,
                         TR::InstOpCode::LGR);
    targetRegister = node->getRegister();
  }
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::bxorEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::sxorEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ixorEvaluator(node, cg);
}
 ...
 */
TR::Register *OMR::Z::TreeEvaluator::cxorEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::ixorEvaluator(node, cg);
}

TR::Register *OMR::Z::TreeEvaluator::dexpEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
...
TR::Register *OMR::Z::TreeEvaluator::fexpEvaluator(TR::Node *node,
                                                   TR::CodeGenerator *cg) {
  TR_ASSERT(0, "This evaluator is not functionally correct. Do Not use.");
