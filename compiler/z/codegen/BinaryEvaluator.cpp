...
TR::Register* laddHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  bool isCompressionSequence
    = TR::Compiler->target.is64Bit() && comp->useCompressedPointers() && node->containsCompressionSequence();
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::luaddc)) {
    ...
    TR_ASSERT(
      node->getOpCodeValue() == TR::ladd || node->getOpCodeValue() == TR::luadd || node->getOpCodeValue() == TR::luaddc,
 ...
 * than an int (i.e. byte-bsub, char-csub, short-ssub, and int-isub)
 *
...
inline TR::Register* generic32BitSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196) && firstChild->getRegister()
      && !cg->canClobberNodesRegister(firstChild) && ((-value) >= MIN_IMMEDIATE_VAL && (-value) <= MAX_IMMEDIATE_VAL)) {
...
inline TR::Register* lDivRemGenericEvaluator64(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
  ...
  if (handleConstDenominator) {
    ...
    if (denominator == -1) {
      ...
      if (!isDivision) // remainder
      {
        ...
        generateS390ImmOp(
          cg, TR::InstOpCode::CG, node, firstRegister, firstRegister, (int64_t)CONSTANT64(0x8000000000000000));
      ...
      } else // remainder
      {
        ...
        if (!isUnsigned && !firstChild->isNonNegative()) {
          ...
          generateRILInstruction(
            cg, TR::InstOpCode::NIHF, node, tempRegister2, static_cast<int32_t>((absValueOfDenominator - 1) >> 32));
          generateRILInstruction(
  ...
  if (!doConditionalRemainder && !node->isSimpleDivCheck() && !isUnsigned && !firstChild->isNonNegative()
    ...
    && !(secondChild->getOpCodeValue() == TR::lconst && secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
  ...
  if (doConditionalRemainder) {
    ...
    if (firstChild->isNonNegative()) {
      ...
      // LGPR is needed so negative numbers do not always take the slow path after the logical compare (but functionally
      // it is not needed)
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
      ...
      if (node->getOpCodeValue() == TR::lshl) {
        if (firstChild->getOpCodeValue() == TR::i2l && firstChild->isSingleRefUnevaluated()
          && (firstChild->isNonNegative() || firstChild->getFirstChild()->isNonNegative())) {
          srcReg = cg->evaluate(firstChild->getFirstChild());
          trgReg = cg->allocateRegister();
          auto mnemonic
            = TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) ? TR::InstOpCode::RISBGN : TR::InstOpCode::RISBG;

          generateRIEInstruction(
            cg, mnemonic, node, trgReg, srcReg, (int8_t)(32 - value), (int8_t)((63 - value) | 0x80), (int8_t)value);
    ...
    if ((shiftOp == TR::InstOpCode::SRLG || shiftOp == TR::InstOpCode::SLLG) && secondChild->isSingleRefUnevaluated()
...
TR::Register* genericRotateLeft(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    ...
    if (node->getOpCodeValue() == TR::lor || node->getOpCodeValue() == TR::lxor) {
      ...
      if (shiftChild && shiftChild->getSecondChild()->getOpCode().isLoadConst() && shiftChild->isSingleRefUnevaluated()
    ...
    if (node->getOpCode().isAnd() && (firstChild->getOpCode().isLeftShift() || firstChild->getOpCode().isRightShift())
      ...
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(comp, "[%p] and/sh[r,l] => rotated-and-insert: tZeros %d, lZeros %d, popCnt %d\n", node, tZeros,
          lZeros, popCnt);
        traceMsg(
          comp, "\t               => rotated-and-insert: shiftMsBit %d, shiftLsBit %d \n", shiftMsBit, shiftLsBit);
  ...
  TR::Node* node, TR::CodeGenerator* cg, int32_t shiftAmount, bool isSignedShift)
{
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
    ...
    if (node->getOpCodeValue() == TR::land && secondChild->getOpCode().isLoadConst()) {
      ...
      // perform a signed shift because we must preserve the sign bit. In general we cannot handle this case because we
      // are not certain our input will be non-negative, hence we must conservatively disallow the optimization in such
      // a case.
      ...
      if (comp->getOption(TR_TraceCG)) {
        traceMsg(
          comp, "[%p] land => rotated-and-insert: tZeros %d, lZeros %d, popCnt %d\n", node, tZeros, lZeros, popCnt);
      ...
      if (popCnt == (64 - lZeros - tZeros)) {
        ...
        if (firstChild->getReferenceCount() > 1 || lZeros > 31 || tZeros > 31 || (lZeros > 0 && tZeros > 0)) {
          ...
          // [1] If the resultant shifted msb in a right shift is shifted "too far" to the right, then we are basically
          // zeroing the register. This optimization will not be used. [2] Same case if the resultant lsb of a left
          // shift is shifted out of range. Eg[1]: 000...0011 >> 3. msb = 62, lsb = 63. If we right shift, then msb and
          // lsb will both be > 63 (out of range) Eg[2]: 11100...00 << 10. msb = 0, lsb = 2. If we left shift, then msb
          // and lsb will both be < 0 (out of range)
      ...
      if (doTransformation && performTransformation(comp, "O^O Use RISBG instead of 2 ANDs for %p.\n", node)) {
        ...
        // "capture" the shifted bits by upperbounding the msb. [2] Similar case if the msb of a right shift is still in
        // range but the lsb is out of range.
        //
        // Eg[1]: 00...0111 >> 2. msb = 61, lsb = 63. If we right shift, we get 000...0001, so we can still preserve the
        // last bit. Eg[2]: 1110...00 << 2. msb = 0, lsb = 2. If we left left, we can still preserve the first bit.
...
TR::Register* lsubHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (NEED_CC(node) || (node->getOpCodeValue() == TR::lusubb)) {
    TR_ASSERT(!isCompressionSequence, "CC computation not supported with compression sequence.\n");
    TR_ASSERT(
      node->getOpCodeValue() == TR::lsub || node->getOpCodeValue() == TR::lusub || node->getOpCodeValue() == TR::lusubb,
      "CC computation not supported for this node %p\n", node);
 ...
 */
TR::Register* OMR::Z::TreeEvaluator::caddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return generic32BitAddEvaluator(node, cg);
}
 ...
 * (child1 - child2)
 */
TR::Register* OMR::Z::TreeEvaluator::csubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return generic32BitSubEvaluator(node, cg);
}
...
TR::Register* OMR::Z::TreeEvaluator::imulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (firstChild->getOpCodeValue() == TR::s2i && firstChild->getFirstChild()->getOpCodeValue() == TR::sloadi
    && firstChild->isSingleRefUnevaluated() && firstChild->getFirstChild()->isSingleRefUnevaluated()) {
    isMultHalf = true;
    halfwordNode = firstChild;
    regNode = secondChild;
  } else if (secondChild->getOpCodeValue() == TR::s2i && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi
    && secondChild->isSingleRefUnevaluated() && secondChild->getFirstChild()->isSingleRefUnevaluated()) {
    isMultHalf = true;
    ...
    if (!isUnsigned && (value == -1)) {
      ...
      if (!isUnsigned && !firstChild->isNonNegative()) {
        ...
        generateShiftAndKeepSelected31Bit(
          node, cg, targetRegister, targetRegister, 0x3f & (32 - shftAmnt), 31, 0, true, false);
      }
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, done, deps);
    } else {
      // Division with no exception check
...
TR::Register* OMR::Z::TreeEvaluator::dexpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::Z::TreeEvaluator::fexpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(0, "This evaluator is not functionally correct. Do Not use.");
