...
genNullTestForCompressedPointers(TR::Node* node, TR::CodeGenerator* cg, TR::Register* targetRegister, TR::LabelSymbol*& skipAdd)
{
    ...
    if (TR::Compiler->target.is64Bit() && comp->useCompressedPointers() && node->containsCompressionSequence() /* &&
         !node->isNonZero() */
    ) {
...
laddHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool isCompressionSequence = TR::Compiler->target.is64Bit() && comp->useCompressedPointers() && node->containsCompressionSequence();
    ...
    if (isCompressionSequence && curTreeNode->getOpCode().isNullCheck() && (curTreeNode->getNullCheckReference()->getOpCodeValue() == TR::l2a) && (curTreeNode->getNullCheckReference()->getFirstChild() == node)) {
        bumpedRefCount = true;
    ...
    if (hasCompressedPointers && ((secondChild->getOpCodeValue() != TR::lconst) || (secondChild->getRegister() != NULL))) {
        secondRegister = cg->evaluate(secondChild);
        ...
        if (firstChild->getReferenceCount() > 1 && firstChild->getRegister() && targetRegister != firstChild->getRegister())
...
{
    return (op.isStore() || op.isLoad() || op.isBranchOp() || op.getOpCodeValue() == TR::InstOpCode::CHI || op.getOpCodeValue() == TR::InstOpCode::CLR || op.getOpCodeValue() == TR::InstOpCode::CLGR || op.getOpCodeValue() == TR::InstOpCode::CHI || op.getOpCodeValue() == TR::InstOpCode::CGHI);
}
...
generic32BitSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196) && firstChild->getRegister() && !cg->canClobberNodesRegister(firstChild) && ((-value) >= MIN_IMMEDIATE_VAL && (-value) <= MAX_IMMEDIATE_VAL)) {
...
lDivRemGenericEvaluator64(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
    ...
    if (!doConditionalRemainder && !node->isSimpleDivCheck() && !isUnsigned && !firstChild->isNonNegative() && !secondChild->isNonNegative() && !(secondChild->getOpCodeValue() == TR::lconst && secondChild->getLongInt() != CONSTANT64(0xFFFFFFFFFFFFFFFF))) {
...
iDivRemGenericEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool isDivision, TR::MemoryReference* divchkDivisorMR)
{
    ...
    // Note:  D and DR require that R1=0 before executing the division
    // TODO(#3685): Remove this Java-ism from OMR and push it into OpenJ9
    bool needCheck = (!node->isSimpleDivCheck() && !node->getOpCode().isUnsigned() && !firstChild->isNonNegative() && !secondChild->isNonNegative());
    ...
    } else if (secondChild->getRegister() == NULL && secondChild->getReferenceCount() == 1 && secondChild->getOpCode().isMemoryReference() && !needCheck) {
...
genericLongShiftSingle(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if ((shiftOp == TR::InstOpCode::SRLG || shiftOp == TR::InstOpCode::SLLG) && secondChild->isSingleRefUnevaluated() && secondChild->getOpCode().isAnd() && secondChild->getOpCode().isInteger() && secondChild->getSecondChild()->getOpCode().isLoadConst() && secondChild->getSecondChild()->getConst<int64_t>() == 63) {
...
genericIntShift(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp, TR::InstOpCode::Mnemonic altShiftOp)
{
    ...
    if (node->getOpCodeValue() == TR::bushr) {
        if (node->getFirstChild()->getOpCodeValue() == TR::bconst && node->getReferenceCount() == 1 && node->getFirstChild()->getRegister() == NULL) {
    ...
    if (!cg->canClobberNodesRegister(firstChild) || srcReg->containsCollectedReference() || ((refCnt == 1) && canUseAltShiftOp && (shiftAmount != 0))) {
        trgReg = cg->allocateRegister();
...
genericRotateLeft(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
        if (node->getOpCodeValue() == TR::lor) {
            ...
            if (lushr && lushr->getOpCodeValue() == TR::lushr && lushr->getReferenceCount() == 1 && lushr->getRegister() == NULL && lshl->getOpCodeValue() == TR::lshl && lshl->getReferenceCount() == 1 && lshl->getRegister() == NULL) {
        ...
        if (node->getOpCodeValue() == TR::lor && andChild && andChild->getRegister() == NULL && andChild->getReferenceCount() == 1) {
            TR::Node* data = NULL;
            uint64_t shiftBy = 0;
            uint64_t mask1 = 0;
            uint64_t mask2 = 0;
            ...
            if (shiftChild->getOpCodeValue() == TR::lshl && shiftChild->getRegister() == NULL && shiftChild->getReferenceCount() == 1 && shiftChild->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCodeValue() == TR::lor || node->getOpCodeValue() == TR::lxor) {
            ...
            if (shiftChild && shiftChild->getSecondChild()->getOpCode().isLoadConst() && shiftChild->isSingleRefUnevaluated() && performTransformation(cg->comp(), "O^O Combine or/shift into rotate node [%p]\n", node)) {
...
genericRotateAndInsertHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
        ...
        //
        if (node->getOpCode().isAnd() && (firstChild->getOpCode().isLeftShift() || firstChild->getOpCode().isRightShift()) && firstChild->getRegister() == NULL && firstChild->getReferenceCount() <= 1 && secondChild->getOpCode().isLoadConst() && firstChild->getSecondChild()->getOpCode().isLoadConst()) {
            uint64_t value = 0;
...
lsubHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    */
    ...
    bool bumpedRefCount = false;
    if (isCompressionSequence && curTreeNode->getOpCode().isNullCheck() && (firstChild->getOpCodeValue() == TR::a2l) && (firstChild->getFirstChild() == curTreeNode->getNullCheckReference())) {
        bumpedRefCount = true;
    ...
    if (hasCompressedPointers && ((secondChild->getOpCodeValue() != TR::lconst) || (secondChild->getRegister() != NULL))) {
       ...
       */
        ...
        cg->decReferenceCount(firstChild);
        cg->decReferenceCount(secondChild);
    } else if (firstChild->getOpCodeValue() == TR::lconst && firstChild->getRegister() == NULL && secondChild->getRegister() != NULL) {
...
lmulHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChildIsConstant) {
        ...
        if (firstChild->getRegister() != NULL && value == 2 && TR::Compiler->target.is64Bit()) // 3 way AGEN LA is cracked on zG
        {
            create_LA = true;
        }
...
OMR::Z::TreeEvaluator::imulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getOpCodeValue() == TR::s2i && firstChild->getFirstChild()->getOpCodeValue() == TR::sloadi && firstChild->isSingleRefUnevaluated() && firstChild->getFirstChild()->isSingleRefUnevaluated()) {
        isMultHalf = true;
        halfwordNode = firstChild;
        regNode = secondChild;
    } else if (secondChild->getOpCodeValue() == TR::s2i && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi && secondChild->isSingleRefUnevaluated() && secondChild->getFirstChild()->isSingleRefUnevaluated()) {
        isMultHalf = true;
...
OMR::Z::TreeEvaluator::iremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        } else if ((!isUnsigned || isNonNegativePowerOf2(value)) && ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(value)) > 0)) {
            TR::LabelSymbol* done = generateLabelSymbol(cg);
            TR::RegisterDependencyConditions* deps = NULL;
...
OMR::Z::TreeEvaluator::inegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else if (firstChild->getOpCodeValue() == TR::imul && firstChild->getSecondChild()->getOpCode().isLoadConst() && firstChild->getSecondChild()->getInt() != 0x80000000 && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && performTransformation(cg->comp(), "O^O Replace ineg/imul by const with imul by -const.\n")) {
