...
void
TR_S390OutOfLineCodeSection::generateS390OutOfLineCodeSectionDispatch()
{
  ...
  if (_targetReg)
    temp = generateRRInstruction(_cg,
                                 _targetRegMovOpcode,
                                 _callNode,
                                 _targetReg,
                                 _callNode->getRegister());

  _cg->decReferenceCount(_callNode);

  temp = generateS390BranchInstruction(_cg,
                                       TR::InstOpCode::BRC,
 ...
 */
void
TR_S390OutOfLineCodeSection::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
  TR::Compilation* comp = _cg->comp();
  ...
  for (auto li = firstTimeLiveOOLRegisterList->begin();
       li != firstTimeLiveOOLRegisterList->end();
       ++li) {
