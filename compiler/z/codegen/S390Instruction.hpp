...
class S390BranchInstruction : public TR::S390LabeledInstruction {
 ...
 public:
  ...
  virtual TR::Snippet *getSnippetForGC() {
    if (getLabelSymbol() != NULL) return getLabelSymbol()->getSnippet();
    return NULL;
  }
  ...
  TR::InstOpCode::S390BranchCondition setBranchCondition(
      TR::InstOpCode::S390BranchCondition branchCondition) {
 ...
 public:
  S390LabelInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390LabeledInstruction(op, n, sym, cg),
        ...
        _handle(0),
        _flags(0) {
    if (op == TR::InstOpCode::LABEL) sym->setInstruction(this);
 ...
 public:
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
    ...
    if (!getOpCode().setsOperand1())
      ...
      useTargetRegister(treg);
    if (sym->isLabel()) _targetLabel = sym->castToLabelSymbol();
  }
...
class S390RRSInstruction : public TR::S390RRInstruction {
 ...
 public:
  ...
  virtual TR::InstOpCode::S390BranchCondition setBranchCondition(
      TR::InstOpCode::S390BranchCondition branchCondition) {
    return _branchCondition = branchCondition;
  }
  uint8_t getMask() {
 ...
 public:
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cg),
        ...
        _constantField(-1),
        _memref(mf) {
    if (use) useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
      (mf->getUnresolvedSnippet())->setDataReferenceInstruction(this);
  }
...
class S390SILInstruction : public TR::S390MemInstruction {
 ...
 public:
  S390SILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : S390MemInstruction(op, n, mf, cg, false), _sourceImmediate(imm) {
    if (op != TR::InstOpCode::TBEGINC) useSourceMemoryReference(mf);
  }
...
class S390SS1WithImplicitGPRsInstruction : public TR::S390SS1Instruction {
 public:
  S390SS1WithImplicitGPRsInstruction(
    ...
    for (i = 0; i < nr; i++) _operands[i] = _operands[i + nm];
    for (i = 0; i < nm; i++) _operands[i + nr] = vp[i];
  }
...
class S390SS5WithImplicitGPRsInstruction : public TR::S390SS4Instruction {
 public:
  S390SS5WithImplicitGPRsInstruction(
    ...
    for (i = 0; i < nm; i++) _operands[i + explicitSourceRegSize + nr] = vp[i];
  }
...
class S390VStorageInstruction : public S390VInstruction {
 protected:
  ...
  S390VStorageInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
      ...
      : S390VInstruction(cg, op, n, targetReg, precedingInstruction),
        _maskField(mask) {
    if (sourceReg) useSourceRegister(sourceReg);
    useSourceMemoryReference(mr);
    setupThrowsImplicitNullPointerException(n, mr);
