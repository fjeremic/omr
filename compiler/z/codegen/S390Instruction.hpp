...
public:
  S390LabeledInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
    ...
    if (!cg->comp()->getOption(TR_EnableEBBCCInfo))
      clearCCInfo();
    else if (isLabel() || isCall()) // otherwise clearCCInfo for real Label
                                    // instruction or call only
      clearCCInfo();
  }
  ...
  S390LabeledInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                         ...
                         TR::Instruction *precedingInstruction,
                         TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, precedingInstruction, cg), _symbol(sym),
        _snippet(NULL) {
    if (!cg->comp()->getOption(TR_EnableEBBCCInfo))
      clearCCInfo();
    else if (isLabel() || isCall()) // otherwise clearCCInfo for real Label
  ...
  S390LabeledInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                         TR::Snippet *s, TR::Instruction *precedingInstruction,
                         TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _symbol(NULL),
        _snippet(s) {
    if (!cg->comp()->getOption(TR_EnableEBBCCInfo))
      clearCCInfo();
    else if (isLabel() || isCall()) // otherwise clearCCInfo for real Label
  ...
  S390LabeledInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                         ...
                         TR::Instruction *precedingInstruction,
                         TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, precedingInstruction, cg), _symbol(NULL),
        _snippet(s) {
    if (!cg->comp()->getOption(TR_EnableEBBCCInfo))
      clearCCInfo();
    else if (isLabel() || isCall()) // otherwise clearCCInfo for real Label
  ...
  virtual bool isNopCandidate(); // Check to determine whether we should NOP to
                                 // align instruction.
};
...
public:
  ...
  S390BranchInstruction(TR::InstOpCode::Mnemonic op,
                        TR::InstOpCode::S390BranchCondition branchCondition,
                        TR::Node *n, TR::Snippet *s, TR::CodeGenerator *cg)
  ...
  TR::InstOpCode::S390BranchCondition getBranchCondition() {
    return _branchCondition;
  }
  TR::InstOpCode::S390BranchCondition
  setBranchCondition(TR::InstOpCode::S390BranchCondition branchCondition) {
    return _branchCondition = branchCondition;
  }
  uint8_t getMask() {
    return getMaskForBranchCondition(getBranchCondition()) << 4;
  }
};
...
public:
  S390VirtualGuardNOPInstruction(TR::Node *node, TR_VirtualGuardSite *site,
                                 TR::RegisterDependencyConditions *cond,
                                 TR::LabelSymbol *label, TR::CodeGenerator *cg)
      : S390BranchInstruction(TR::InstOpCode::BRC, TR::InstOpCode::COND_VGNOP,
                              node, label, cond, cg),
        _site(site) {}
  ...
  S390VirtualGuardNOPInstruction(TR::Node *node, TR_VirtualGuardSite *site,
                                 ...
                                 TR::CodeGenerator *cg)
      : S390BranchInstruction(TR::InstOpCode::BRC, TR::InstOpCode::COND_VGNOP,
                              node, label, cond, precedingInstruction, cg),
        _site(site) {}
...
public:
  S390BranchOnCountInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                               TR::Register *targetReg, TR::LabelSymbol *sym,
                               TR::CodeGenerator *cg)
      : S390LabeledInstruction(op, n, sym, cg) {
...
public:
  S390BranchOnIndexInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
...
public:
  S390LabelInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                       TR::LabelSymbol *sym, TR::CodeGenerator *cg)
      : S390LabeledInstruction(op, n, sym, cg), _alignment(0), _handle(0),
        _flags(0) {
  ...
  S390LabelInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                       ...
                       TR::CodeGenerator *cg)
      : S390LabeledInstruction(op, n, sym, precedingInstruction, cg),
        _alignment(0), _handle(0), _flags(0) {
    if (op == TR::InstOpCode::LABEL)
      sym->setInstruction(this);
    cg->getNextAvailableBlockIndex();
  }
  ...
  S390LabelInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                       ...
                       TR::RegisterDependencyConditions *cond,
                       TR::CodeGenerator *cg)
      : S390LabeledInstruction(op, n, sym, cond, cg), _alignment(0), _handle(0),
        _flags(0) {
    if (op == TR::InstOpCode::LABEL)
      sym->setInstruction(this);
    cg->getNextAvailableBlockIndex();
  ...
  S390LabelInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, TR::Snippet *s,
                       TR::CodeGenerator *cg)
      : S390LabeledInstruction(op, n, s, cg), _alignment(0), _handle(0),
        _flags(0) {}

  S390LabelInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, TR::Snippet *s,
                       TR::RegisterDependencyConditions *cond,
                       TR::CodeGenerator *cg)
      : S390LabeledInstruction(op, n, s, cond, cg), _alignment(0), _handle(0),
        _flags(0) {}
  ...
  bool isSkipForLabelTargetNOPs() {
    return _flags.testAny(skipForLabelTargetNOPs);
  }

  void setEstimateDoneForLabelTargetNOPs() {
    _flags.set(estimateDoneForLabelTargetNOPs);
  }
  bool wasEstimateDoneForLabelTargetNOPs() {
    return _flags.testAny(estimateDoneForLabelTargetNOPs);
  }
...
public:
  S390PseudoInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                        TR::Node *fenceNode, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _fenceNode(fenceNode), _callDescValue(0),
        _padbytes(0), _callDescLabel(NULL), _shouldBeginNewLine(false) {}

  S390PseudoInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                        TR::Node *fenceNode,
                        TR::RegisterDependencyConditions *cond,
                        TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _fenceNode(fenceNode),
        _callDescValue(0), _padbytes(0), _callDescLabel(NULL),
  ...
  S390PseudoInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cond, precedingInstruction, cg),
        _fenceNode(fenceNode), _callDescValue(0), _padbytes(0),
        _callDescLabel(NULL), _shouldBeginNewLine(false) {}
  ...
  //   TR::Register  *getTargetRegister() { return (_targetRegSize!=0) ?
  //   (targetRegBase())[0] : NULL;}
  ...
  TR::LabelSymbol *setCallDescLabel(TR::LabelSymbol *ls) {
    return _callDescLabel = ls;
  }
...
/**
 * Extends from S390PseudoInstruction to exploit the property of register
 * assignment ignoring these types or treating them specially
 */
...
public:
  S390DebugCounterBumpInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                                  TR::Snippet *counterSnip,
                                  TR::CodeGenerator *cg, int32_t delta)
      : S390PseudoInstruction(op, n, NULL, cg), _counterSnippet(counterSnip),
        _assignableReg(NULL), _delta(delta) {}
  ...
  S390DebugCounterBumpInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390PseudoInstruction(op, n, NULL, precedingInstruction, cg),
        _counterSnippet(counterSnip), _assignableReg(NULL), _delta(delta) {}

  /**
   * A real register must only be assigned for DCB to use if it is guaranteed to
   * be free at the cursor position where DCB is inserted
  ...
  /**
   * @return The snippet that holds the debugCounter's counter address in
   * persistent memory
   */
...
public:
  S390AnnotationInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                            int16_t regionNum, int32_t statementNum,
                            int32_t flags, bool printNumber, char *annotation,
                            TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _annotation(annotation), _flags(flags),
  ...
  S390AnnotationInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, precedingInstruction, cg),
        _annotation(annotation), _flags(flags),
        _shouldPrintNumber(printNumber) {
    setRegionNumber(regionNum);
  }
  ...
  S390AnnotationInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                            ...
                            TR::RegisterDependencyConditions *cond,
                            TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _annotation(annotation),
        _flags(flags), _shouldPrintNumber(printNumber) {
    setRegionNumber(regionNum);
  }
...
public:
  S390ImmInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t imm,
                     TR::CodeGenerator *cg)
...
public:
  S390Imm2Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint16_t imm,
                      TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _sourceImmediate(imm) {
...
public:
  S390ImmSnippetInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                            uint32_t imm, TR::UnresolvedDataSnippet *us,
                            TR::Snippet *s, TR::CodeGenerator *cg)
  ...
  setUnresolvedSnippet(TR::UnresolvedDataSnippet *us) {
...
public:
  S390ImmSymInstruction(TR::InstOpCode::Mnemonic op, TR::Node *node,
...
public:
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *reg, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _branchCondition(TR::InstOpCode::COND_NOP),
        _firstConstant(-1) {
    ...
    // ShouldUseRegPairForTarget returns true for those instructions that
    // require consequtive even-odd register pairs. CanUseRegPairForTarget
    // returns true for all the above instructions + STM/LTM (which can
    // potentially take a register pair range).

    // So, if we find a register pair passed in, we check to make sure the
    // instruction CAN use a register pair. If we do not find a register pair,
    // we check to make sure the instruction SHOULD NOT use a register pair.
  ...
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _branchCondition(TR::InstOpCode::COND_NOP),
        _firstConstant(-1), _targetPairFlag(false) {}

  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _branchCondition(TR::InstOpCode::COND_NOP), _firstConstant(-1),
        _targetPairFlag(false) {}

  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     int8_t firstConstant, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _branchCondition(TR::InstOpCode::COND_NOP),
        _firstConstant(firstConstant), _targetPairFlag(false) {}
  ...
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *reg, TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
  ...
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *reg, TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _branchCondition(TR::InstOpCode::COND_NOP), _firstConstant(-1) {
    checkRegForGPR0Disable(op, reg);
  ...
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Register *reg, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _branchCondition(brCond),
        _firstConstant(0) {
    checkRegForGPR0Disable(op, reg);
    _targetPairFlag = reg->getRegisterPair() ? true : false;
    if (!getOpCode().setsOperand1())
  ...
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _branchCondition(brCond), _firstConstant(0) {
    checkRegForGPR0Disable(op, reg);
    _targetPairFlag = reg->getRegisterPair() ? true : false;
    if (!getOpCode().setsOperand1())
      useSourceRegister(reg);
  ...
  S390RegInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Register *reg, TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _branchCondition(brCond),
        _firstConstant(0) {
    checkRegForGPR0Disable(op, reg);
    _targetPairFlag = reg->getRegisterPair() ? true : false;
    if (!getOpCode().setsOperand1())
  ...
  //   TR::Register  *getTargetRegister()               { return (_targetRegSize
  //   != 0) ? tgtRegArrElem(0) : NULL; } TR::Register
  //   *setTargetRegister(TR::Register *r) {assume0(_targetRegSize != 0); return
  //   (targetRegBase())[0] = r;}

  /**
   * Given that instruction expresses a register range (eg. LM or STM) the first
   * and last register of the range is fetched from first operand as a register
   * pair or from the first two operands
   */
  TR::Register *getFirstRegister() {
    return _targetPairFlag ? getRegisterOperand(1)->getHighOrder()
                           : getRegisterOperand(1);
  }
  TR::Register *getLastRegister() {
    return _targetPairFlag ? getRegisterOperand(1)->getLowOrder() : NULL;
  }
  ...
  bool matchesTargetRegister(TR::Register *reg) {
    ...
    if (isTargetPair()) {
      ...
      if (reg->getKind() != TR_FPR && reg->getKind() != TR_VRF &&
          getFirstRegister()->getRealRegister()) {
...
public:
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
  ...
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, int8_t secondConstant,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
  ...
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    int8_t firstConstant, int8_t secondConstant,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
  ...
  S390RRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _secondConstant(-1) {}
  ...
  //   TR::Register  *getSourceRegister()                {return
  //   (_sourceRegSize!=0) ? (sourceRegBase())[0] : NULL;  } TR::Register
  //   *setSourceRegister(TR::Register *sr) { (sourceRegBase())[0] = sr; return
  //   sr; }
...
public:
   ...
   */
  S390TranslateInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                           TR::Register *treg, TR::Register *sreg,
                           TR::Register *tableReg, TR::Register *termCharReg,
                           TR::RegisterDependencyConditions *cond,
                           TR::CodeGenerator *cg)
  ...
  //   TR::Register *setTableRegister(TR::Register* tableReg)        {return
  //   (sourceRegBase())[tr_tblidx] =  tableReg;}

  TR::Register *getTermCharRegister() { return getRegisterOperand(tr_termidx); }
  // TR::Register *setTermCharRegister(TR::Register* termCharReg)  {return
  // (sourceRegBase())[tr_termidx] = termCharReg;}

  // TR::Register *getSourceRegister()                {return
  // (sourceRegBase())[tr_srcidx]; } TR::Register
  // *setSourceRegister(TR::Register *sr) {return (sourceRegBase())[tr_srcidx] =
  // sr;}

  //   TR::Register *getTargetRegister()                { return
  //   (targetRegBase())[tr_targidx];} TR::Register
  //   *setTargetRegister(TR::Register *sr) { return
  //   (targetRegBase())[tr_targidx]=sr;}
...
public:
  S390RRFInstruction(bool encodeAsRRD, TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Register *sreg,
                     TR::Register *sreg2, TR::CodeGenerator *cg)
      : S390RRInstruction(op, n, treg, sreg, cg), _encodeAsRRD(encodeAsRRD),
        _isSourceReg2Present(true), _isMask3Present(false),
  ...
  S390RRFInstruction(bool encodeAsRRD, TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Register *sreg,
                     TR::Register *sreg2, TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : S390RRInstruction(op, n, treg, sreg, precedingInstruction, cg),
        _encodeAsRRD(encodeAsRRD), _isSourceReg2Present(true),
        _isMask3Present(false), _isMask4Present(false) {
    if (!getOpCode().setsOperand3())
      useSourceRegister(sreg2);
    else
      useTargetRegister(sreg2);
  ...
  S390RRFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Register *sreg,
                     TR::Register *sreg2, TR::CodeGenerator *cg)
  ...
  S390RRFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Register *sreg, uint8_t mask,
                     bool isMask3, TR::CodeGenerator *cg)
      : S390RRInstruction(op, n, treg, sreg, cg), _encodeAsRRD(false),
        _isSourceReg2Present(false), _isMask3Present(isMask3),
  ...
  S390RRFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     bool isMask3, TR::CodeGenerator *cg)
      : S390RRInstruction(op, n, cg), _encodeAsRRD(false),
        _isMask3Present(isMask3), _isMask4Present(!isMask3),
  ...
  S390RRFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Register *sreg, uint8_t mask,
                     bool isMask3, TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
  ...
  S390RRFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RRInstruction(op, n, treg, sreg, cg), _encodeAsRRD(false),
        _isSourceReg2Present(true), _isMask3Present(false),
        _isMask4Present(true), _mask4(mask) {
    if (!getOpCode().setsOperand3())
      useSourceRegister(sreg2);
    else
      useTargetRegister(sreg2);
  ...
  S390RRFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : S390RRInstruction(op, n, treg, sreg, precedingInstruction, cg),
        _encodeAsRRD(false), _isSourceReg2Present(true), _isMask3Present(false),
        _isMask4Present(false) {
    if (!getOpCode().setsOperand3())
      useSourceRegister(sreg2);
    else
  ...
  S390RRFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Register *sreg, uint8_t mask3,
                     uint8_t mask4, TR::CodeGenerator *cg)
      : S390RRInstruction(op, n, treg, sreg, cg), _mask3(mask3), _mask4(mask4),
        _encodeAsRRD(false), _isSourceReg2Present(false), _isMask3Present(true),
  ...
  // TR::Register *getSourceRegister2()                {return
  // (_sourceRegSize==2) ? (sourceRegBase())[1] : NULL; } TR::Register
  // *setSourceRegister2(TR::Register *sr) {return (sourceRegBase())[1]=sr;}
  ...
  uint8_t getMask() {
    TR_ASSERT(0,
              "RRF: getMask() is obsolete, use getMask3(..) or getMask4(..)\n");
    return 0;
  }
  uint8_t setMask(uint8_t mask) {
    TR_ASSERT(0,
              "RRF: setMask() is obsolete, use setMask3(..) or setMask4(..)\n");
    return 0;
  }
...
#define rrr_srcidx 1
public:
  S390RRRInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Register *sreg,
                     TR::Register *sreg2, TR::CodeGenerator *cg)
  ...
  // TR::Register *getSourceRegister2()                {return
  // (sourceRegBase())[rrr_srcidx];} TR::Register
  // *setSourceRegister2(TR::Register *sr) {return
  // (sourceRegBase())[rrr_srcidx]=sr;}
...
public:
  ...
  S390RIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, precedingInstruction, cg), _isImm(false),
        _sourceImmediate(0){};

  S390RIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _isImm(false),
        _sourceImmediate(0){};
  ...
  S390RIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _isImm(false), _sourceImmediate(0){};

  S390RIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, char *data, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _namedDataField(data),
        _isImm(false){};
  ...
  S390RIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _namedDataField(data), _isImm(false){};

  S390RIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, int32_t imm, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _sourceImmediate(imm),
        _isImm(true){};
  ...
  S390RIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _sourceImmediate(imm), _isImm(true){};
...
class S390RILInstruction : public TR::Instruction {
  ...
  /** _flagsRIL */
  enum { isLiteralPoolAddressFlag = 0x01, isImmediateOffsetInBytesFlag = 0x02 };

  /** Use to store immediate value where the immediate is not used as a relative
   * offset in address calculation.*/
  uint32_t _sourceImmediate;

public:
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, uint32_t imm, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(NULL), _targetSnippet(NULL),
        _targetSymbol(NULL), _flagsRIL(0), _mask(0xffffffff),
        _targetLabel(NULL), _symbolReference(NULL), _sourceImmediate(imm) {
    TR_ASSERT_FATAL(getOpCode().isExtendedImmediate(),
                    "Incorrect S390RILInstruction constructor used.\n"
                    "%s expects an address as the immediate but this "
                    "constructor is for integer immediates.",
                    getOpCode().getMnemonicName());
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, void *targetPtr, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(targetPtr), _targetSnippet(NULL),
        _targetSymbol(NULL), _flagsRIL(0), _mask(0xffffffff),
        _targetLabel(NULL), _symbolReference(NULL), _sourceImmediate(0) {
    TR_ASSERT_FATAL(!getOpCode().isExtendedImmediate(),
                    "Incorrect S390RILInstruction constructor used.\n"
                    "%s expects an integer as the immediate but this "
                    "constructor is for address immediates.",
                    getOpCode().getMnemonicName());
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
      useSourceRegister(treg);
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, void *targetPtr,
                     TR::SymbolReference *sr, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(targetPtr), _targetSnippet(NULL),
        _targetSymbol(NULL), _flagsRIL(0), _mask(0xffffffff),
        _targetLabel(NULL), _symbolReference(sr), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
      useSourceRegister(treg);
    else
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(targetPtr),
        _targetSnippet(NULL), _targetSymbol(NULL), _flagsRIL(0),
        _mask(0xffffffff), _targetLabel(NULL), _symbolReference(sr),
        _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
      useSourceRegister(treg);
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(NULL),
        _targetSnippet(NULL), _targetSymbol(NULL), _flagsRIL(0),
        _mask(0xffffffff), _targetLabel(NULL), _symbolReference(NULL),
        _sourceImmediate(imm) {
    TR_ASSERT_FATAL(getOpCode().isExtendedImmediate(),
                    "Incorrect S390RILInstruction constructor used.\n"
                    "%s expects an adddress as the immediate but this "
                    "constructor is for integer immediates.",
                    getOpCode().getMnemonicName());
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Snippet *ts, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(NULL), _targetSnippet(ts),
        _targetSymbol(NULL), _flagsRIL(0), _mask(0xffffffff),
        _targetLabel(NULL), _symbolReference(NULL), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
                     void *targetPtr, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(targetPtr), _targetSnippet(NULL),
        _targetSymbol(NULL), _flagsRIL(0), _mask(mask), _targetLabel(NULL),
        _symbolReference(NULL), _sourceImmediate(0) {}

  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
                     TR::Snippet *ts, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(NULL), _targetSnippet(ts),
        _targetSymbol(NULL), _flagsRIL(0), _mask(mask), _targetLabel(NULL),
        _symbolReference(NULL), _sourceImmediate(0) {}

  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Snippet *ts,
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(NULL),
        _targetSnippet(ts), _targetSymbol(NULL), _flagsRIL(0),
        _mask(0xffffffff), _targetLabel(NULL), _symbolReference(NULL),
        _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
                     void *targetPtr, TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(targetPtr),
        _targetSnippet(NULL), _targetSymbol(NULL), _flagsRIL(0), _mask(mask),
        _targetLabel(NULL), _symbolReference(NULL), _sourceImmediate(0) {}
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _targetPtr(NULL), _targetSnippet(ts),
        _targetSymbol(NULL), _flagsRIL(0), _mask(0xffffffff),
        _targetLabel(NULL), _symbolReference(NULL), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
                     TR::Snippet *ts, TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _targetPtr(NULL), _targetSnippet(ts),
        _targetSymbol(NULL), _flagsRIL(0), _mask(mask), _targetLabel(NULL),
        _symbolReference(NULL), _sourceImmediate(0) {}
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _targetPtr(targetPtr),
        _targetSnippet(NULL), _targetSymbol(NULL), _flagsRIL(0),
        _mask(0xffffffff), _targetLabel(NULL), _symbolReference(NULL),
        _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Symbol *sym,
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _targetPtr(NULL),
        _targetSnippet(NULL), _targetSymbol(sym), _flagsRIL(0),
        _mask(0xffffffff), _targetLabel(NULL), _symbolReference(NULL),
        _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::Symbol *sym, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(NULL), _targetSnippet(NULL),
        _targetSymbol(sym), _flagsRIL(0), _mask(0xffffffff), _targetLabel(NULL),
        _symbolReference(NULL), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    useTargetRegister(treg);
    if (sym->isLabel())
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cg), _targetPtr(NULL), _targetSnippet(NULL),
        _targetSymbol(sym), _flagsRIL(0), _mask(0xffffffff), _targetLabel(NULL),
        _symbolReference(sr), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
      useSourceRegister(treg);
    else
  ...
  /** For PFDRL */
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
                     TR::Symbol *sym, TR::SymbolReference *sr,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(NULL), _targetSnippet(NULL),
        _targetSymbol(sym), _flagsRIL(0), _mask(mask), _targetLabel(NULL),
        _symbolReference(sr), _sourceImmediate(0) {
    if (sym->isLabel())
      _targetLabel = sym->castToLabelSymbol();
  }
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t mask,
                     ...
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(NULL),
        _targetSnippet(NULL), _targetSymbol(sym), _flagsRIL(0), _mask(mask),
        _targetLabel(NULL), _symbolReference(sr), _sourceImmediate(0) {
    if (sym->isLabel())
      _targetLabel = sym->castToLabelSymbol();
  }
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::LabelSymbol *label,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _targetPtr(NULL), _targetSnippet(NULL),
        _targetSymbol(NULL), _flagsRIL(0), _mask(0xffffffff),
        _targetLabel(label), _symbolReference(NULL), _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
      useSourceRegister(treg);
    else
  ...
  S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, precedingInstruction, cg), _targetPtr(NULL),
        _targetSnippet(NULL), _targetSymbol(NULL), _flagsRIL(0),
        _mask(0xffffffff), _targetLabel(label), _symbolReference(NULL),
        _sourceImmediate(0) {
    checkRegForGPR0Disable(op, treg);
    if (!getOpCode().setsOperand1())
      useSourceRegister(treg);
  ...
  bool isLiteralPoolAddress() {
    return _flagsRIL.testAny(isLiteralPoolAddressFlag);
  }
  void setIsLiteralPoolAddress() { _flagsRIL.set(isLiteralPoolAddressFlag); }

  bool isImmediateOffsetInBytes() {
    return _flagsRIL.testAny(isImmediateOffsetInBytesFlag);
  }
  void setIsImmediateOffsetInBytes() {
    _flagsRIL.set(isImmediateOffsetInBytesFlag);
  }

  uintptrj_t getTargetPtr() { return reinterpret_cast<uintptrj_t>(_targetPtr); }
  uintptrj_t setTargetPtr(uintptrj_t tp) {
    TR_ASSERT(!isImmediateOffsetInBytes(),
              "Immediate Offset already set on RIL instruction.");
    _targetPtr = reinterpret_cast<void *>(tp);
    return tp;
  }
  uintptrj_t getImmediateOffsetInBytes() {
    TR_ASSERT(isImmediateOffsetInBytes(),
              "Immediate Offset not set for RIL Instruction.");
  ...
  //   TR::Register  *getTargetRegister()               { return
  //   (_targetRegSize!=0) ? (targetRegBase())[0] : NULL;} TR::Register
  //   *setTargetRegister(TR::Register *r) { assume0(_targetRegSize!=0);
  //   (targetRegBase())[0] = r; return r; }
  bool matchesTargetRegister(TR::Register *reg) {
    TR::RealRegister *realReg = NULL;
    ...
    // if we are matching real regs
    if (reg->getKind() != TR_FPR && reg->getKind() != TR_VRF &&
        getRegisterOperand(1) && getRegisterOperand(1)->getRealRegister()) {
...
protected:
  ...
  /// Set to true if a mask value was specified during the construction of this
  /// object, indicating an RS instruction of RS-b format where the third
  /// operand is a mask
...
public:
   ...
   * e.g. SLL   R1,12(0) - shifting R1 with constant value 12 and the value is <
   * 4K in this case, base register doesn't need to be set
   */
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, uint32_t imm, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _sourceImmediate(imm),
        _maskImmediate(0), _idx(-1), _hasMaskImmediate(false),
        _hasSourceImmediate(true) {
    // 1 Register is specified - If it is not a register pair, make sure
    // instruction doesn't take a range (i.e. STM).
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _sourceImmediate(imm), _maskImmediate(0), _idx(-1),
        _hasMaskImmediate(false), _hasSourceImmediate(true) {
    // 1 Register is specified - If it is not a register pair, make sure
    // instruction doesn't take a range (i.e. STM).
    TR_ASSERT(treg->getRegisterPair() || !getOpCode().usesRegRangeForTarget(),
              "OpCode [%s] requires a register range.\n",
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, _conditions, cg), _sourceImmediate(imm),
        _maskImmediate(0), _idx(-1), _hasMaskImmediate(false),
        _hasSourceImmediate(true)
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, TR::MemoryReference *mf,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _sourceImmediate(0),
        _maskImmediate(0), _idx(-1), _hasMaskImmediate(false),
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _sourceImmediate(0), _maskImmediate(0), _idx(-1),
        _hasMaskImmediate(false), _hasSourceImmediate(false) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
      (mf->getUnresolvedSnippet())->setDataReferenceInstruction(this);
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, uint32_t mask, TR::MemoryReference *mf,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, precedingInstruction, cg),
        _sourceImmediate(0), _maskImmediate(mask), _idx(-1),
        _hasMaskImmediate(true), _hasSourceImmediate(false) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
      (mf->getUnresolvedSnippet())->setDataReferenceInstruction(this);
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    ...
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _sourceImmediate(0),
        _maskImmediate(mask), _idx(-1), _hasMaskImmediate(true),
        _hasSourceImmediate(false) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : S390RegInstruction(op, n, freg, cg), _sourceImmediate(0),
        _maskImmediate(0), _idx(-1), _hasMaskImmediate(false),
        _hasSourceImmediate(false) {
    if (getOpCode().setsOperand2())
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::RegisterPair *regp, TR::MemoryReference *mf,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, regp, cg), _sourceImmediate(0),
        _maskImmediate(0), _idx(-1), _hasMaskImmediate(false),
        _hasSourceImmediate(false) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
  ...
  /** Used specifically for source/target consecutive even-odd pairs
   * (CLCLE,MVCLE)*/
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::RegisterPair *regp, TR::RegisterPair *regp2,
                    TR::MemoryReference *mf, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, regp, cg), _sourceImmediate(0),
        _maskImmediate(0), _idx(-1), _hasMaskImmediate(false),
        _hasSourceImmediate(false)
  ...
  S390RSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::Register *treg, TR::Register *sreg, uint32_t imm,
                    TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, treg, cg), _sourceImmediate(imm),
        _maskImmediate(0), _idx(-1), _hasMaskImmediate(false),
        _hasSourceImmediate(true) {
  ...
  /// Determines whether a mask value was specified during the construction of
  /// this object, indicating an RS instruction of RS-b format where the third
  /// operand is a mask
  ...
  TR::Register *getFirstRegister() {
    return isTargetPair() ? S390RegInstruction::getFirstRegister()
                          : getRegisterOperand(1);
  }
  TR::Register *getLastRegister() {
    return isTargetPair() ? S390RegInstruction::getLastRegister()
                          : getRegisterOperand(2);
  }

  TR::Register *getSecondRegister() { return getRegisterOperand(2); }

  virtual TR::MemoryReference *getMemoryReference() {
    return (_sourceMemSize != 0) ? (sourceMemBase())[0] : NULL;
  }
...
public:
  S390RSYInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, uint32_t mask, TR::MemoryReference *mf,
                     TR::CodeGenerator *cg)
  ...
  S390RSYInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::RegisterPair *treg, TR::RegisterPair *sreg,
                     TR::MemoryReference *mf, TR::CodeGenerator *cg)
...
public:
  ...
  S390RRSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
  ...
  S390RRSInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : S390RRInstruction(op, n, targetRegister, sourceRegister,
  ...
  virtual TR::InstOpCode::S390BranchCondition getBranchCondition() {
    return _branchCondition;
  }
  virtual TR::InstOpCode::S390BranchCondition
  setBranchCondition(TR::InstOpCode::S390BranchCondition branchCondition) {
    return _branchCondition = branchCondition;
  }
  uint8_t getMask() {
  ...
  virtual TR::MemoryReference *getBranchDestinationLabel() {
...
public:
  S390RREInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::CodeGenerator *cg)
...
public:
  S390IEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t im1,
                    uint8_t im2, TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _immediate1(im1), _immediate2(im2) {}

  S390IEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t im1,
                    uint8_t im2, TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _immediate1(im1),
        _immediate2(im2) {}
...
private:
  /** This member will determine which form  of RIE you have based on how we get
   * constructed */
  RIEForm _instructionFormat;

public:
  /** Construct a Reg-Reg form RIE with no preceding instruction */
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *targetRegister, TR::Register *sourceRegister,
                     TR::LabelSymbol *branchDestination,
                     TR::InstOpCode::S390BranchCondition branchCondition,
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, cg),
        _instructionFormat(RIE_RR), _branchDestination(branchDestination),
        _branchCondition(branchCondition) {
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *targetRegister, int8_t sourceImmediate,
                     TR::LabelSymbol *branchDestination,
                     TR::InstOpCode::S390BranchCondition branchCondition,
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, cg),
        _instructionFormat(RIE_RI8), _branchDestination(branchDestination),
        _branchCondition(branchCondition), _sourceImmediate8(sourceImmediate) {
    // note that _targetRegister is registered for use via the
    // S390RegInstruction constructor call
  }
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *targetRegister, TR::Register *sourceRegister,
                     int8_t sourceImmediateOne, int8_t sourceImmediateTwo,
                     int8_t sourceImmediate, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, cg),
        _instructionFormat(RIE_IMM), _branchDestination(0),
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *targetRegister, int16_t sourceImmediate,
                     TR::InstOpCode::S390BranchCondition branchCondition,
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, cg),
        _instructionFormat(RIE_RI16A), _branchDestination(0),
        _branchCondition(branchCondition), _sourceImmediate16(sourceImmediate) {
    // Note that _targetRegister is registered for use via the
    // S390RegInstruction constructor call
    if (op == TR::InstOpCode::LOCGHI || op == TR::InstOpCode::LOCHI ||
        op == TR::InstOpCode::LOCHHI)
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
    ...
    if (isTrap())
      TR_ASSERT((sourceImmediate >> 8) != 0x00B9,
                "ASSERTION FAILURE: should not generate RIE trap instruction "
                "with imm field = B9XX!\n");
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, precedingInstruction, cg),
        _instructionFormat(RIE_RRI16), _branchDestination(0),
        _branchCondition(TR::InstOpCode::COND_NOPR), _sourceImmediate8(0),
  ...
  S390RIEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *targetRegister, TR::Register *sourceRegister,
                     int16_t sourceImmediate, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, targetRegister, cg),
        _instructionFormat(RIE_RRI16), _branchDestination(0),
        _branchCondition(TR::InstOpCode::COND_NOPR), _sourceImmediate8(0),
        _sourceImmediate16(sourceImmediate) {
  ...
  // virtual TR::Register * getSourceRegister() { return (_sourceRegSize!=0) ?
  // (sourceRegBase())[0] : NULL; }
  //   virtual TR::Register * getTargetRegister() { return
  //   S390RegInstruction::getTargetRegister(); }

  // set register informtion
  //   virtual void setSourceRegister(TR::Register * reg) {
  //   assume0(_sourceRegSize==1); (sourceRegBase())[0] = reg; }
  ...
  /** Get branch destination information */
  virtual TR::LabelSymbol *getBranchDestinationLabel() {
    return _branchDestination;
  }
...
public:
  S390SMIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     TR::LabelSymbol *inst, TR::MemoryReference *mf3,
                     TR::CodeGenerator *cg)
  ...
  S390SMIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _mask(mask),
  ...
  S390SMIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     ...
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _mask(mask),
        _branchInstruction(inst) {
    useSourceMemoryReference(mf3);
  }
  ...
  S390SMIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, precedingInstruction, cg), _mask(mask),
        _branchInstruction(inst) {
    useSourceMemoryReference(mf3);
  }
...
public:
  S390MIIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     TR::LabelSymbol *inst2, TR::SymbolReference *inst3,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg), _mask(mask), _callSymRef(inst3),
  ...
  S390MIIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     ...
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, precedingInstruction, cg), _mask(mask),
        _callSymRef(inst3), _branchInstruction(inst2) {}
  S390MIIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     TR::LabelSymbol *inst2, TR::SymbolReference *inst3,
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cond, cg), _mask(mask), _callSymRef(inst3),
        _branchInstruction(inst2) {}
  S390MIIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                     TR::LabelSymbol *inst2, TR::SymbolReference *inst3,
                     TR::RegisterDependencyConditions *cond,
...
public:
  ...
  S390RISInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *targetRegister, int8_t sourceImmediate,
                     TR::MemoryReference *branchDestination,
                     TR::InstOpCode::S390BranchCondition branchCondition,
                     TR::CodeGenerator *cg)
  ...
  S390RISInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg)
      : S390RIInstruction(op, n, targetRegister, sourceImmediate,
                          precedingInstruction, cg),
        _branchDestination(branchDestination),
        _branchCondition(branchCondition) {
    setupThrowsImplicitNullPointerException(n, branchDestination);
...
public:
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::MemoryReference *mf, TR::CodeGenerator *cg,
                     bool use = true)
      : TR::Instruction(op, n, cg), _memAccessMode(-1), _constantField(-1),
        _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
  ...
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
      ...
      : TR::Instruction(op, n, cond, cg), _memAccessMode(-1),
        _constantField(-1), _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
  ...
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     int8_t memAccessMode, TR::MemoryReference *mf,
                     TR::CodeGenerator *cg, bool use = true)
      : TR::Instruction(op, n, cg), _memAccessMode(memAccessMode),
        _constantField(-1), _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
  ...
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     int8_t constantField, int8_t memAccessMode,
                     TR::MemoryReference *mf, TR::CodeGenerator *cg,
                     bool use = true)
      : TR::Instruction(op, n, cg), _memAccessMode(memAccessMode),
        _constantField(constantField), _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
  ...
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::Instruction *precedingInstruction,
                     TR::CodeGenerator *cg, bool use = true)
      : TR::Instruction(op, n, precedingInstruction, cg), _memAccessMode(-1),
        _constantField(-1), _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
  ...
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg, bool use = true)
      : TR::Instruction(op, n, cond, precedingInstruction, cg),
        _memAccessMode(-1), _constantField(-1), _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
  ...
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg, bool use = true)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _memAccessMode(memAccessMode), _constantField(-1), _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
  ...
  S390MemInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     ...
                     TR::CodeGenerator *cg, bool use = true)
      : TR::Instruction(op, n, precedingInstruction, cg),
        _memAccessMode(memAccessMode), _constantField(constantField),
        _memref(mf) {
    if (use)
      useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
...
public:
  S390SIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::MemoryReference *mf, uint8_t imm, TR::CodeGenerator *cg)
      : S390MemInstruction(op, n, mf, cg, false), _sourceImmediate(imm) {
    useSourceMemoryReference(
        mf); // need to call this *after* S390SIInstruction constructor
  }

  S390SIInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                    TR::MemoryReference *mf, uint8_t imm,
                    TR::Instruction *precedingInstruction,
                    TR::CodeGenerator *cg)
...
public:
  S390SIYInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::MemoryReference *mf, uint8_t imm,
                     TR::CodeGenerator *cg)
...
public:
  S390SILInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::MemoryReference *mf, uint16_t imm,
                     TR::CodeGenerator *cg)
      : S390MemInstruction(op, n, mf, cg, false), _sourceImmediate(imm) {
...
public:
  S390SInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                   TR::MemoryReference *mf, TR::CodeGenerator *cg)
...
public:
  S390RSLInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint16_t len,
                     TR::MemoryReference *mf1, TR::CodeGenerator *cg)
 ...
 *
 *
 * RSLb is actually very different in encoding and use then RSL so creating a
 * new class
 */
...
public:
  S390RSLbInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                      TR::Register *reg, uint16_t length,
                      TR::MemoryReference *mf, uint8_t mask,
                      TR::CodeGenerator *cg)
  ...
  S390RSLbInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                      TR::Register *reg, int16_t length,
                      TR::MemoryReference *mf, int8_t mask,
                      TR::Instruction *preced, TR::CodeGenerator *cg)
      : S390RegInstruction(op, n, reg, preced, cg), _length(length),
        _mask(mask) {
    useSourceMemoryReference(mf);
    setupThrowsImplicitNullPointerException(n, mf);
    if (mf->getUnresolvedSnippet() != NULL)
...
/**
 * Common base class for instructions with two memory references (except SSF as
 * this is also like an RX format instruction)
 */
...
public:
  ...
  virtual TR::MemoryReference *getMemoryReference2() {
    return (_targetMemSize != 0) ? (targetMemBase())[0] : NULL;
  }
...
public:
  S390SS1Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint16_t len,
                     TR::MemoryReference *mf1, TR::MemoryReference *mf2,
                     TR::CodeGenerator *cg)
  ...
  S390SS1Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint16_t len,
      ...
      : S390MemMemInstruction(op, n, mf1, mf2, precedingInstruction, cg),
        _symbol(NULL), _len(len) {}

  S390SS1Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint16_t len,
                     TR::MemoryReference *mf1, TR::MemoryReference *mf2,
                     TR::RegisterDependencyConditions *cond,
                     TR::CodeGenerator *cg)
      : S390MemMemInstruction(op, n, mf1, mf2, cond, cg), _symbol(NULL),
        _len(len) {}
...
public:
  S390SS1WithImplicitGPRsInstruction(
  ...
  S390SS1WithImplicitGPRsInstruction(
      ...
      TR::MemoryReference *mf1, TR::MemoryReference *mf2,
      TR::RegisterDependencyConditions *cond,
      TR::Instruction *precedingInstruction, TR::Register *implicitRegSrc0,
      TR::Register *implicitRegSrc1, TR::Register *implicitRegTrg0,
      TR::Register *implicitRegTrg1, TR::CodeGenerator *cg)
      : S390SS1Instruction(op, n, len, mf1, mf2, cond, precedingInstruction,
                           cg) {
    // Make sure memory references appear after registers in operands array
    int32_t i;
    int32_t nm = _sourceMemSize + _targetMemSize;
...
public:
  S390SS2Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint16_t len1,
                     TR::MemoryReference *mf1, uint16_t len2,
                     TR::MemoryReference *mf2, TR::CodeGenerator *cg)
      : S390SS1Instruction(op, n, len1, mf1, mf2, cg), _len2(len2),
        _shiftAmount(0) {}
  ...
  S390SS2Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint16_t len1,
      ...
      : S390SS1Instruction(op, n, len1, mf1, mf2, precedingInstruction, cg),
        _len2(len2), _shiftAmount(0) {}

  S390SS2Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint32_t len1,
                     TR::MemoryReference *mf1, int32_t shiftAmount,
                     uint32_t roundAmount, TR::CodeGenerator *cg)
      : S390SS1Instruction(op, n, len1, mf1, NULL, cg),
        _shiftAmount(shiftAmount), _len2(roundAmount) {}
 ...
 *
 *
 * Also used for an SS5 where B1(D1) is called B2(D2) and B2(D2) is called
 * B4(D4)
 */
...
public:
  S390SS4Instruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *lengthReg, TR::MemoryReference *mf1,
                     TR::MemoryReference *mf2, TR::Register *sourceKeyReg,
                     TR::CodeGenerator *cg)
      : S390SS1Instruction(op, n, 0, mf1, mf2, cg), _ss4_lenidx(-1),
        _ss4_keyidx(-1) {
    // Make sure memory references appear after registers in operands array
    int32_t i;
    int32_t nm = _sourceMemSize + _targetMemSize;
  ...
  TR::Register *getLengthReg() {
    return (_ss4_lenidx != -1) ? (sourceRegBase())[_ss4_lenidx] : NULL;
  }
  void setLengthReg(TR::Register *lengthReg) {
    (sourceRegBase())[_ss4_lenidx] = lengthReg;
  }

  TR::Register *getSourceKeyReg() {
    return (_ss4_keyidx != -1) ? (sourceRegBase())[_ss4_keyidx] : NULL;
  }
  void setSourceKeyReg(TR::Register *sourceKeyReg) {
    (sourceRegBase())[_ss4_keyidx] = sourceKeyReg;
  }
...
public:
  S390SS5WithImplicitGPRsInstruction(
      TR::InstOpCode::Mnemonic op, TR::Node *n, TR::Register *op1,
      TR::MemoryReference *mf2, TR::Register *op3, TR::MemoryReference *mf4,
    ...
    if (implicitRegSrc1 != NULL)
      useSourceRegister(implicitRegSrc1);
    // Because op1 and op3 are source registers let all source registers be
    // first. Infrastructure can handle target after source but as long as they
    // are contiguous
    if (implicitRegTrg0 != NULL)
      useTargetRegister(implicitRegTrg0);
    if (implicitRegTrg1 != NULL)
      useTargetRegister(implicitRegTrg1);
    // The first 2 (explictSourceRegSize) register operands are in place
    // followed by the up to two memory operands Move all the added implicit
    // registers adjacents to the first two register operands
    int32_t nr = _targetRegSize + _sourceRegSize - explicitSourceRegSize;
    for (i = 0; i < nr; i++)
      _operands[i + explicitSourceRegSize] =
          _operands[i + explicitSourceRegSize + nm];
    for (i = 0; i < nm; i++)
  ...
  S390SS5WithImplicitGPRsInstruction(
      ...
      TR::Register *implicitRegSrc1, TR::Register *implicitRegTrg0,
      TR::Register *implicitRegTrg1, TR::CodeGenerator *cg)
      : S390SS4Instruction(op, n, op1, mf2, mf4, op3, precedingInstruction,
                           cg) {
    // Make sure memory references appear after registers in operands array
    int32_t i;
    int32_t nm = _sourceMemSize + _targetMemSize;
...
public:
  S390RXEInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *treg, TR::MemoryReference *mf, uint8_t m3,
                     TR::CodeGenerator *cg)
...
public:
  S390RXYbInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t mask,
                      TR::MemoryReference *mf, TR::CodeGenerator *cg)
...
class S390SSFInstruction : public TR::S390RXInstruction {
  // TR::MemoryReference *_memoryReference2;     // second memory reference
  // operand

public:
  S390SSFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::Register *reg, TR::MemoryReference *mf1,
                     TR::MemoryReference *mf2, TR::CodeGenerator *cg)
  ...
  S390SSFInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                     TR::RegisterPair *regp, TR::MemoryReference *mf1,
                     TR::MemoryReference *mf2, TR::CodeGenerator *cg)
  ...
  virtual TR::MemoryReference *getMemoryReference2() {
    return (_sourceMemSize == 2) ? (sourceMemBase())[1] : NULL;
  }
...
protected:
  S390VInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                   TR::Node *n, TR::Register *reg1)
...
private:
  ...
  // each can be 8, 12, or 16 bit long, not necessarily consecutive in each
  // instruction combined length can be up to 20 bits. Different sub-types use
  // the following two pieces differently
   ...
   */
protected:
  S390VRIInstruction(TR::CodeGenerator *cg = NULL,
                     TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                     TR::Node *n = NULL, TR::Register *targetReg = NULL,
                     uint16_t constantImm16 = 0, uint8_t constantImm8 = 0,
                     uint8_t m3 = 0, /*  4 bits (28 - 31 bit) */
                     uint8_t m4 = 0, /*  4 bits (28 - 31 bit) */
                     uint8_t m5 = 0) /*  4 bits (32 - 35 bit) */
      : S390VInstruction(cg, op, n, targetReg), _constantImm16(constantImm16),
...
#define CAT8TO16(val1, val2)                                                   \
  ((uint16_t)((((uint16_t)val1) << 8) | ((uint16_t)val2)))
...
public:
  S390VRIaInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
...
public:
  S390VRIbInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      uint8_t constantImm2 = 0, /*  8 bits */
                      uint8_t constantImm3 = 0, /*  8 bits */
                      uint8_t mask4 = 0)        /*  4 bits */
      : S390VRIInstruction(cg, op, n, targetReg,
                           CAT8TO16(constantImm2, constantImm3), 0, 0, mask4,
                           0) {}
...
public:
  S390VRIcInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
...
public:
  S390VRIdInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
      ...
      : S390VRIInstruction(cg, op, n, targetReg, CAT8TO16(0, constantImm4), 0,
                           0, 0, mask5) {
    if (getOpCode().setsOperand2())
...
public:
  S390VRIeInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      ...
                      uint8_t mask5 = 0,         /*  4 bits */
                      uint8_t mask4 = 0)         /*  4 bits */
      : S390VRIInstruction(cg, op, n, targetReg, (constantImm3 << 4), 0, 0,
                           mask4, mask5) {
    // Error Checking
    TR_ASSERT((constantImm3 & 0xf000) == 0,
              "Incorrect length in immediate 3 value");
...
public:
  S390VRIfInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      TR::Register *sourceReg3 = NULL,
...
public:
  S390VRIgInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      uint8_t constantImm3 = 0, /*  8 bits */
                      uint8_t constantImm4 = 0, /*  8 bits */
                      uint8_t mask5 = 0)        /*  4 bits */
      : S390VRIInstruction(cg, op, n, targetReg, CAT8TO16(0, constantImm3),
                           constantImm4, 0, 0, mask5) {
    if (getOpCode().setsOperand2())
      useTargetRegister(sourceReg2);
    else
      useSourceRegister(sourceReg2);
...
public:
  S390VRIhInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      uint16_t constantImm2 = 0, /*  16 bits */
                      uint8_t constantImm3 = 0)  /*  4 bits */
      : S390VRIInstruction(cg, op, n, targetReg, constantImm2, constantImm3, 0,
                           0, 0) {
    // Error check constantImm3 is no longer than 4bit
    TR_ASSERT((constantImm3 & 0xf0) == 0,
              "Incorrect length in immediate 3 value");
  }
...
public:
  S390VRIiInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      uint8_t constantImm3 = 0, /*  8 bits */
...
public:
  ...
  /* We want these to be called only by helper constructors */
protected:
  S390VRRInstruction(TR::CodeGenerator *cg = NULL,
                     TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                     TR::Node *n = NULL, TR::Register *targetReg = NULL,
                     TR::Register *sourceReg2 = NULL,
                     uint8_t m3 = 0, /* Mask3 */
  ...
  S390VRRInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
...
public:
  S390VRRaInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      uint8_t mask5 = 0, /* 4 bits */
                      uint8_t mask4 = 0, /* 4 bits */
                      uint8_t mask3 = 0) /* 4 bits */
      : S390VRRInstruction(cg, op, n, targetReg, sourceReg2, mask3, mask4,
                           mask5, 0) {}

  S390VRRaInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                      TR::Node *n, TR::Register *targetReg,
...
public:
  S390VRRbInstruction(TR::CodeGenerator *cg = NULL,
                      ...
                      uint8_t mask5 = 0, /* 4 bits */
                      uint8_t mask4 = 0) /* 4 bits */
      : S390VRRInstruction(cg, op, n, targetReg, sourceReg2, 0, mask4, mask5,
                           0) {
    if (getOpCode().setsOperand3())
      useTargetRegister(sourceReg3);
    else
...
public:
  S390VRRcInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      TR::Register *sourceReg3 = NULL,
...
public:
  S390VRRdInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      TR::Register *sourceReg3 = NULL,
                      ...
                      uint8_t mask6 = 0, /* 4 bits */
                      uint8_t mask5 = 0) /* 4 bits */
      : S390VRRInstruction(cg, op, n, targetReg, sourceReg2, 0, 0, mask5,
                           mask6) {
    if (getOpCode().setsOperand3())
      useTargetRegister(sourceReg3);
    else
...
public:
  S390VRReInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
                      TR::Register *sourceReg2 = NULL,
                      TR::Register *sourceReg3 = NULL,
...
public:
  S390VRRfInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *targetReg = NULL,
...
public:
  S390VRRgInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
...
public:
  S390VRRhInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL, TR::Register *v1Reg = NULL,
                      TR::Register *v2Reg = NULL, uint8_t mask3 = 0)
 ...
 *   0        8    12   16           24    28       36   40    47
 *
 * z15 and above have an optional M4 if the vector-packed-decimal-enhancement
 * facility is installed.
 */
class S390VRRiInstruction : public S390VRRInstruction {
public:
  S390VRRiInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
...
protected:
  ...
  S390VStorageInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                          TR::Node *n, TR::Register *targetReg, /* VRF or GPR */
  ...
  /**
   * This S390VStorageInstruction contructor does not take target/source
   * registers nor mask bits. It is useful for vector storage formats that uses
   * registers and immediates only: VSI.
   */
  S390VStorageInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                          TR::Node *n)
      : S390VInstruction(cg, op, n), _maskField(0) {}
...
public:
  ...
  /* We want these to be called only by helper constructors */
protected:
  S390VRSInstruction(TR::CodeGenerator *cg = NULL,
                     TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                     TR::Node *n = NULL,
...
public:
  S390VRSaInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL,
                      TR::Register *targetReg = NULL, /* VRF */
  ...
  S390VRSaInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
...
public:
  S390VRSbInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL,
                      TR::Register *targetReg = NULL, /* VRF */
  ...
  S390VRSbInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                      TR::Node *n, TR::Register *targetReg,   /* VRF */
...
public:
  S390VRScInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                      TR::Node *n = NULL,
  ...
  S390VRScInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                      TR::Node *n, TR::Register *targetReg,   /* GPR */
                      TR::Register *sourceReg,                /* VRF */
                      TR::MemoryReference *mr, uint8_t mask4, /*  4 bits */
                      TR::Instruction *preced)
      : S390VStorageInstruction(cg, op, n, targetReg, sourceReg, mr, mask4,
...
public:
  S390VRSdInstruction(TR::CodeGenerator *cg = NULL,
                      TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
  ...
  S390VRSdInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                      TR::Node *n, TR::Register *r3Reg, /* GPR */
...
private:
  void initVRSd(TR::Register *r3Reg, TR::Register *v1Reg,
                TR::MemoryReference *mr) {
    if (getOpCode().isStore()) {
      useTargetRegister(r3Reg);
      useSourceRegister(v1Reg);
      // In the current design, memRef is always the source for non-Mem-Mem
      // instructions
      useSourceMemoryReference(mr);
    } else {
...
public:
  S390VRVInstruction(TR::CodeGenerator *cg = NULL,
                     TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
                     TR::Node *n = NULL,
                     TR::Register *sourceReg = NULL, /* VRF */
                     TR::MemoryReference *mr = NULL,
  ...
  S390VRVInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                     TR::Node *n, TR::Register *sourceReg,   /* VRF */
...
public:
  S390VRXInstruction(TR::CodeGenerator *cg = NULL,
                     TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
  ...
  S390VRXInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                     TR::Node *n, TR::Register *reg,         /* GPR */
                     TR::MemoryReference *mr, uint8_t mask3, /*  4 bits */
                     TR::Instruction *precedingInstruction)
      : S390VStorageInstruction(cg, op, n, reg, NULL, mr, mask3,
...
public:
  S390VSIInstruction(TR::CodeGenerator *cg = NULL,
      ...
      : S390VStorageInstruction(cg, op, n), _constantImm3(constantImm3) {
    initVSI(n, v1Reg, memRef);
  }

  S390VSIInstruction(TR::CodeGenerator *cg, TR::InstOpCode::Mnemonic op,
                     TR::Node *n, TR::Register *v1Reg,
                     TR::MemoryReference *memRef, uint8_t constantImm3,
                     TR::Instruction *preced)
...
private:
  ...
  void initVSI(TR::Node *n, TR::Register *v1Reg, TR::MemoryReference *memRef) {
    ...
    // memrefs are always named source memory reference regardless of what they
    // actually are.
...
public:
  S390IInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n, uint8_t im,
                   TR::CodeGenerator *cg)
...
public:
  S390EInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                   TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg) {
    setBinaryLength(2);
    setEstimatedBinaryLength(2);
  ...
  /**
   * Following constructor is for PFPO instruction, where FPR0/FPR2 and GPR1 are
   * target registers; FPR4/FPR6 and GPR0 are source registers; they are implied
   * by the opcode, purpose of adding these is to notify RA the reg use/def
   * dependencies
   */
  S390EInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                   TR::CodeGenerator *cg, TR::Register *tgt, TR::Register *tgt2,
...
public:
  S390OpCodeOnlyInstruction(TR::InstOpCode::Mnemonic op, TR::Node *n,
                            TR::CodeGenerator *cg)
      : TR::Instruction(op, n, cg) {}
...
#if defined(DEBUG) || defined(PROD_WITH_ASSUMES)
  TR_ASSERT(i->getS390ImmInstruction() != NULL,
            "trying to downcast to an S390ImmInstruction");
#endif
