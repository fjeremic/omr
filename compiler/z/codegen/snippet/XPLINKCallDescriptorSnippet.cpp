...
uint32_t TR::XPLINKCallDescriptorSnippet::generateCallDescriptorValue(
    TR::S390zOSSystemLinkage *linkage, TR::Node *callNode) {
  uint32_t result = 0;
  ...
  if (TR::Compiler->target.is32Bit()) {
    ...
    default:
      TR_ASSERT_FATAL(false, "Unknown datatype (%s) for call node (%p)",
                      dataType.toString(), callNode);
      break;
    }
    ...
    if (callSymbol->isJNI() && callNode->isPreparedForDirectJNI()) {
      ...
      // JNI Calls include a JNIEnv* pointer that is not included in list of
      // children nodes. For FastJNI, certain calls do not require us to pass
      // the JNIEnv.
    ...
    for (int32_t i = firstArgumentChild; (i <= to) && !done; i++, parmCount++) {
      ...
      TR::SymbolReference *parmSymRef = child->getOpCode().hasSymbolReference()
      ...
      //
      // PERFORMANCE TODO: it is desirable to use the defined "parameter count"
      // of the function symbol to help determine if we have an unprototyped
      // argument of a call (site) to a vararg function.  Currently we
      // overcompensate for outgoing float parms to vararg functions and always
      // shadow in FPR and and stack/gprs as with an unprotoyped call - see
      // pushArg(). Precise information can help remove such compensation.
      // Changes to fix this would involve: this function, pushArg() and
      // buildArgs().
      ...
      if (numFPRsNeeded != 0) {
        ...
        uint32_t wordsToPreviousParm =
            (parmAreaOffset - lastFloatParmAreaOffset) / gprSize;
        if (wordsToPreviousParm >
            0xF) {     // to big for descriptor. Will pass in stack
          done = true; // done
        }
...
TR::XPLINKCallDescriptorSnippet::XPLINKCallDescriptorSnippet(
    TR::CodeGenerator *cg, TR::S390zOSSystemLinkage *linkage,
    uint32_t callDescriptorValue)
    : TR::S390ConstantDataSnippet(cg, NULL, NULL, SIZE), _linkage(linkage),
...
uint8_t *TR::XPLINKCallDescriptorSnippet::emitSnippetBody() {
  uint8_t *cursor = cg()->getBinaryBufferCursor();

  // TODO: We should not have to do this here. This should be done by the
  // caller.
  getSnippetLabel()->setCodeLocation(cursor);

  TR_ASSERT_FATAL(
      (reinterpret_cast<uintptr_t>(cursor) % 8) == 0,
      "XPLINKCallDescriptorSnippet is not aligned on a doubleword bounary");

  // Signed offset, in bytes, to Entry Point Marker (if it exists)
  if (_linkage->getEntryPointMarkerLabel() != NULL) {
    *reinterpret_cast<int32_t *>(cursor) = _linkage->getEntryPointMarkerLabel()
                                               ->getInstruction()
                                               ->getBinaryEncoding() -
                                           cursor;
