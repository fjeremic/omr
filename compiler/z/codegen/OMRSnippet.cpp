...
OMR::Z::Snippet::Snippet(TR::CodeGenerator *cg, TR::Node *node,
                         TR::LabelSymbol *label, bool isGCSafePoint)
    : OMR::Snippet(cg, node, label, isGCSafePoint), _codeBaseOffset(-1),
      _pad_bytes(0), _snippetDestAddr(0), _zflags(0) {
  self()->setNeedLitPoolBasePtr();
}

OMR::Z::Snippet::Snippet(TR::CodeGenerator *cg, TR::Node *node,
                         TR::LabelSymbol *label)
    : OMR::Snippet(cg, node, label), _codeBaseOffset(-1), _pad_bytes(0),
      _snippetDestAddr(0), _zflags(0) {
  self()->setNeedLitPoolBasePtr();
}
 ...
 */
uint8_t *OMR::Z::Snippet::generatePICBinary(TR::CodeGenerator *cg,
                                            uint8_t *cursor,
  ...
  if (self()->getKind() == TR::Snippet::IsUnresolvedCall) {
    ...
    *(int16_t *)cursor = 0xC0E0;
    ...
    intptrj_t destAddr = (intptrj_t)(cursor + self()->getPICBinaryLength(cg) +
                                     self()->getPadBytes() - 2);
    ...
    *(int16_t *)cursor = 0xC0E5;
    ...
    //     necessary.
    intptrj_t destAddr = (intptrj_t)(
        glueRef->getSymbol()->castToMethodSymbol()->getMethodAddress());
...
#endif
    {
      if (cg->directCallRequiresTrampoline(destAddr, instructionStartAddress)) {
        ...
        destAddr = TR::CodeCacheManager::instance()->findHelperTrampoline(
            glueRef->getReferenceNumber(), (void *)cursor);
    ...
    *(int32_t *)cursor = (int32_t)((destAddr - instructionStartAddress) / 2);
    ...
    cg->addProjectSpecializedRelocation(cursor, (uint8_t *)glueRef, NULL,
                                        TR_HelperAddress, __FILE__, __LINE__,
                                        self()->getNode());
    cursor += sizeof(int32_t);
  }
  return cursor;
 ...
 */
uint32_t OMR::Z::Snippet::getPICBinaryLength(TR::CodeGenerator *cg) {
 ...
 * @todo okay to assume that we can destroy previous contents of r13? is
 * register volatile?
 */
uint8_t *OMR::Z::Snippet::generateLoadVMThreadInstruction(TR::CodeGenerator *cg,
                                                          uint8_t *cursor) {
  return cursor;
}
...
/**
 * Helper method to insert Runtime Instrumentation Hooks RION or RIOFF in
 * snippet
 *
 * @param cg               Code Generator Pointer
 * @param cursor           Current binary encoding cursor
 * @param op               Runtime Instrumentation opcode: TR::InstOpCode::RION
 * or TR::InstOpCode::RIOFF
 * @param isPrivateLinkage Whether the snippet is involved in a private JIT
 * linkage (i.e. Call Helper to JITCODE)
 * @return                 Updated binary encoding cursor after RI hook
 * generation.
 */
uint8_t *OMR::Z::Snippet::generateRuntimeInstrumentationOnOffInstruction(
    TR::CodeGenerator *cg, uint8_t *cursor, TR::InstOpCode::Mnemonic op,
    bool isPrivateLinkage) {
  return cursor;
}

/**
 * Helper method to query the length of Runtime Instrumentation Hooks RION or
 * RIOFF in snippet
 *
 * @param cg               Code Generator Pointer
 * @param isPrivateLinkage Whether the snippet is involved in a private JIT
 * linkage (i.e. Call Helper to JITCODE)
 * @return                 The length of RION or RIOFF encoding if generated.
 * Zero otherwise.
 */
uint32_t OMR::Z::Snippet::getRuntimeInstrumentationOnOffInstructionLength(
    TR::CodeGenerator *cg, bool isPrivateLinkage) {
  return 0;
}
