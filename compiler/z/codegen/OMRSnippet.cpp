 ...
 */
uint8_t* OMR::Z::Snippet::generatePICBinary(TR::CodeGenerator* cg, uint8_t* cursor, TR::SymbolReference* glueRef)
{
  ...
  if (self()->getKind() == TR::Snippet::IsUnresolvedCall) {
    ...
    *(int32_t*)cursor = (int32_t)((destAddr - instructionStartAddress) / 2);
    ...
    cg->addProjectSpecializedRelocation(
      cursor, (uint8_t*)glueRef, NULL, TR_HelperAddress, __FILE__, __LINE__, self()->getNode());
    cursor += sizeof(int32_t);
  }
  return cursor;
}
 ...
 */
uint8_t* OMR::Z::Snippet::generateLoadVMThreadInstruction(TR::CodeGenerator* cg, uint8_t* cursor)
{
  return cursor;
}

uint32_t OMR::Z::Snippet::getLoadVMThreadInstructionLength(TR::CodeGenerator* cg)
{
  return 0;
}
