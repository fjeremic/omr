...
void TR_S390CompareAnalyser::integerCompareAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic regRegOpCode,
...
TR_S390CompareAnalyser::longOrderedCompareAndBranchAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic branchOp,
    TR::InstOpCode::S390BranchCondition brCmpLowTrueCond,
    TR::InstOpCode::S390BranchCondition brCmpHighTrueCond,
    TR::InstOpCode::S390BranchCondition brCmpHighFalseCond,
    TR::LabelSymbol *trueTarget, TR::LabelSymbol *falseTarget,
  ...
  // parent. Unsigned compare is used for address comparisons, and as such will
  // almost never be compared to constants, including NULL.  Avoid complications
  // with below optimization and just skip it.
  if (firstChild->isHighWordZero() && !isUnsignedCmp &&
      (brCmpLowTrueCond == TR::InstOpCode::COND_BE ||
       brCmpLowTrueCond == TR::InstOpCode::COND_BNE)) {
    firstHighZero = true;
  }
  ...
  setInputs(firstChild, firstRegister, secondChild, secondRegister, true, false,
            _cg->comp());
    ...
    if (getEvalChild1()) {
      if (firstChild->getReferenceCount() == 1 &&
        ...
        if (!firstIU2L) {
          ...
          generateRXInstruction(_cg, TR::InstOpCode::L, root, delayedFirst,
                                highFirstMR);
        }
      } else {
    ...
    if (getEvalChild2()) {
      if (secondChild->getReferenceCount() == 1 &&
        ...
        if (!secondIU2L) {
          ...
          generateRXInstruction(_cg, TR::InstOpCode::L, root, delayedSecond,
                                highSecondMR);
        }
      } else {
  ...
  if (getCmpReg1Reg2()) {
    ...
    if (hasGlobalDeps) {
      ...
      deps = generateRegisterDependencyConditions(_cg, thirdChild,
                                                  numAdditionalRegDeps);
    } else {
      deps = new (_cg->trHeapMemory())
          TR::RegisterDependencyConditions(0, numAdditionalRegDeps, _cg);
    }
    ...
    if (firstHighZero == false) {
      ...
      if (hasGlobalDeps) {
        deps->addPostConditionIfNotAlreadyInserted(
            firstRegister->getHighOrder(), TR::RealRegister::AssignAny);
        deps->addPostConditionIfNotAlreadyInserted(firstRegister->getLowOrder(),
      ...
      deps->addPostConditionIfNotAlreadyInserted(firstLow,
                                                 TR::RealRegister::AssignAny);
      TR_ASSERT(firstLow->getRegisterPair() == NULL,
                "TR_S390CompareAnalyser::longOrderedCompareAndBranchAnalyser: "
                "Using reg pair as single reg\n");
    }
    ...
    if (secondHighZero == false) {
      ...
      if (hasGlobalDeps) {
        deps->addPostConditionIfNotAlreadyInserted(
            secondRegister->getHighOrder(), TR::RealRegister::AssignAny);
        deps->addPostConditionIfNotAlreadyInserted(
            secondRegister->getLowOrder(), TR::RealRegister::AssignAny);
      } else {
      ...
      deps->addPostConditionIfNotAlreadyInserted(secondLow,
                                                 TR::RealRegister::AssignAny);
    ...
    if (firstHighZero) {
      ...
      {
        generateRRInstruction(_cg, TR::InstOpCode::LTR, root,
      ...
      if (isUnsignedCmp) {
        generateRRInstruction(_cg, TR::InstOpCode::CLR, root,
                              firstRegister->getHighOrder(),
                              secondRegister->getHighOrder());
      } else {
        generateRRInstruction(_cg, TR::InstOpCode::CR, root,
                              firstRegister->getHighOrder(),
                              secondRegister->getHighOrder());
      }
    }
    ...
    if (firstHighZero == false || secondHighZero == false) {
      ...
      generateS390LabelInstruction(_cg, TR::InstOpCode::LABEL, root,
                                   cflowRegionStart);

      // See if we can prove results as false
      if (brCmpHighFalseCond != TR::InstOpCode::COND_NOP) {
        generateS390BranchInstruction(_cg, branchOp, brCmpHighFalseCond, root,
                                      falseTarget);
      }
      // See if we can prove the result as true
      if (brCmpHighTrueCond != TR::InstOpCode::COND_NOP) {
        generateS390BranchInstruction(_cg, branchOp, brCmpHighTrueCond, root,
                                      trueTarget);
      }
    }
  ...
  ///// Reg + Mem  /////////////////////////////////////////
    ...
    // If 2nd child is a TR::iu2l, then the memory contents is only an int and
    // should be loaded into lowMR. The highword is just zero.
    if (secondIU2L) {
      highMR = NULL;
    ...
    if (delayedFirst) {
      firstLow = NULL;
      deps->addPostConditionIfNotAlreadyInserted(delayedFirst,
                                                 TR::RealRegister::AssignAny);
    ...
    if (secondIU2L) {
      ...
      if (temp != NULL) {
        if (isUnsignedCmp) {
          generateRXInstruction(_cg, TR::InstOpCode::CL, firstChild, temp,
                                highMR);
        } else {
          generateRXInstruction(_cg, TR::InstOpCode::C, firstChild, temp,
                                highMR);
        }
      } else {
    ...
    if (delayedFirst) {
      generateRXInstruction(_cg, TR::InstOpCode::L, firstChild, delayedFirst,
                            lowFirstMR);
  ...
  {
    TR_ASSERT(0, "TR_S390CompareAnalyser::longOrderedCompareAndBranchAnalyser: "
                 "Unchecked code");

    // If 1st child is a TR::iu2l, then the memory contents is only an int and
    // should be loaded into lowMR. The highword is just zero.
    if (firstIU2L) {
      highMR = NULL;
    ...
    if (delayedSecond) {
      deps->addPostConditionIfNotAlreadyInserted(delayedSecond,
                                                 TR::RealRegister::AssignAny);
      secondLow = NULL;
    } else if (secondHighZero == false) {
      deps->addPostConditionIfNotAlreadyInserted(secondRegister->getHighOrder(),
                                                 TR::RealRegister::AssignAny);
      secondLow = secondRegister->getLowOrder();
    } else {
      secondLow = secondRegister;
    ...
    if (delayedSecond) {
      generateRXInstruction(_cg, TR::InstOpCode::L, firstChild, delayedSecond,
                            lowSecondMR);
  ...
  if (brCmpLowTrueCond != TR::InstOpCode::COND_NOP) {
    generateS390BranchInstruction(_cg, branchOp, brCmpLowTrueCond, root,
                                  trueTarget);
  }
  ...
  if (root->getFirstChild() !=
      firstChild) // May have skipped original first child earlier (i.e. if 1st
                  // child is iu2l)
...
void TR_S390CompareAnalyser::longEqualityCompareAndBranchAnalyser(
    TR::Node *root, TR::LabelSymbol *firstBranchLabel,
    TR::LabelSymbol *secondBranchLabel,
    TR::InstOpCode::Mnemonic secondBranchOp) {
  TR_ASSERT(0, "TR_S390CompareAnalyser::longEqualityCompareAndBranchAnalyser: "
               "Not implemented");
  return;
}

TR::Register *TR_S390CompareAnalyser::longEqualityBooleanAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic setOpCode,
    TR::InstOpCode::Mnemonic combineOpCode) {
  TR_ASSERT(
      0,
      "TR_S390CompareAnalyser::longEqualityBooleanAnalyser: Not implemented");
  return NULL;
}

TR::Register *TR_S390CompareAnalyser::longOrderedBooleanAnalyser(
    TR::Node *root, TR::InstOpCode::Mnemonic highSetOpCode,
    TR::InstOpCode::Mnemonic lowSetOpCode) {
  TR_ASSERT(
      0, "TR_S390CompareAnalyser::longOrderedBooleanAnalyser: Not implemented");
  return NULL;
}
