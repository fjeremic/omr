...
void
TR_S390CompareAnalyser::integerCompareAnalyser(
  TR::Node* root,
  ...
  bool& internalControlFlowStarted)
{
      ...
      if (firstChild->getReferenceCount() == 1 &&
        ...
        if (!firstIU2L) {
          ...
          generateRXInstruction(
            _cg, TR::InstOpCode::L, root, delayedFirst, highFirstMR);
        }
      } else {
    ...
    if (getEvalChild2()) {
      if (secondChild->getReferenceCount() == 1 &&
        ...
        if (!secondIU2L) {
          ...
          generateRXInstruction(
            _cg, TR::InstOpCode::L, root, delayedSecond, highSecondMR);
        }
      } else {
  ...
  if (getCmpReg1Reg2()) {
    ...
    if (hasGlobalDeps) {
      ...
      deps = generateRegisterDependencyConditions(
        _cg, thirdChild, numAdditionalRegDeps);
    } else {
  ...
  ///// Reg + Mem  /////////////////////////////////////////
    ...
    if (secondIU2L) {
      ...
      if (temp != NULL) {
        if (isUnsignedCmp) {
          generateRXInstruction(
            _cg, TR::InstOpCode::CL, firstChild, temp, highMR);
        } else {
          generateRXInstruction(
            _cg, TR::InstOpCode::C, firstChild, temp, highMR);
        }
      } else {
    ...
    if (delayedFirst) {
      generateRXInstruction(
        _cg, TR::InstOpCode::L, firstChild, delayedFirst, lowFirstMR);
  ...
  else // Must be CmpMem1Reg2
  {
    ...
    if (firstHighZero == false || secondHighZero == false) {
      ...
      if (brCmpHighFalseCond != TR::InstOpCode::COND_NOP) {
        generateS390BranchInstruction(
          _cg, branchOp, brCmpHighTrueCond, root, falseTarget);
      }
      // See if we can prove the result as true
      if (brCmpHighTrueCond != TR::InstOpCode::COND_NOP) {
        generateS390BranchInstruction(
          _cg, branchOp, brCmpHighFalseCond, root, trueTarget);
      }
    }

    if (delayedSecond) {
      generateRXInstruction(
        _cg, TR::InstOpCode::L, firstChild, delayedSecond, lowSecondMR);
  ...
  if (brCmpLowTrueCond != TR::InstOpCode::COND_NOP) {
    generateS390BranchInstruction(
      _cg, branchOp, brCmpLowTrueCond, root, trueTarget);
  }
...
void
TR_S390CompareAnalyser::longEqualityCompareAndBranchAnalyser(
  TR::Node* root,
...
TR::Register*
TR_S390CompareAnalyser::longEqualityBooleanAnalyser(
  TR::Node* root,
  TR::InstOpCode::Mnemonic setOpCode,
  TR::InstOpCode::Mnemonic combineOpCode)
{
  TR_ASSERT(
    0, "TR_S390CompareAnalyser::longEqualityBooleanAnalyser: Not implemented");
  return NULL;
}

TR::Register*
TR_S390CompareAnalyser::longOrderedBooleanAnalyser(
  TR::Node* root,
