...
class OMR_EXTENSIBLE Snippet : public OMR::Snippet {
  ...
  /** For trace files: Snippet Target Address (addr of helper
   * method/trampoline). */
  intptrj_t _snippetDestAddr;

 public:
  Snippet(TR::CodeGenerator *cg, TR::Node *node, TR::LabelSymbol *lab,
          bool requiresGCMap);
  ...
  uint8_t *generatePICBinary(TR::CodeGenerator *, uint8_t *,
                             TR::SymbolReference *);
  uint32_t getPICBinaryLength(TR::CodeGenerator *);

  /** Helper method to reload VM Thread into GPR13 */
  uint8_t *generateLoadVMThreadInstruction(TR::CodeGenerator *cg,
                                           uint8_t *cursor);
  ...
  uint32_t getRuntimeInstrumentationOnOffInstructionLength(
      TR::CodeGenerator *cg, bool isPrivateLinkage = false);
 ...
 private:
  enum  // _zflags
  { NeedLitPoolBasePtr =
        0x0001,  ///< For determining the proper load instructions for accessing
                 ///< the literal pool
