...
public:
  ...
  int32_t setCodeBaseOffset(int32_t offset)
  {
    return _codeBaseOffset = offset;
  }
  int32_t getCodeBaseOffset()
  {
    return _codeBaseOffset;
  }

  int8_t setPadBytes(int8_t numOfBytes)
  {
    return _pad_bytes = numOfBytes;
  }
  int8_t getPadBytes()
  {
    return _pad_bytes;
  ...
  intptrj_t setSnippetDestAddr(intptrj_t addr)
  {
    return _snippetDestAddr = addr;
  }
  intptrj_t getSnippetDestAddr()
  {
    return _snippetDestAddr;
  }
  ...
  bool getNeedLitPoolBasePtr()
  {
    return _zflags.testAny(NeedLitPoolBasePtr);
  }
  void setNeedLitPoolBasePtr()
  {
    _zflags.set(NeedLitPoolBasePtr);
  }
  void resetNeedLitPoolBasePtr()
  {
    _zflags.reset(NeedLitPoolBasePtr);
  }

  // For trace files: whether snippet used MCC trampoline to reach target
  void setUsedTrampoline(bool used)
  {
    return _zflags.set(UsedTrampoline, used);
  }
  bool usedTrampoline()
  {
    return _zflags.testAny(UsedTrampoline);
  }

  // Whether snippet was used or not.
  bool getIsUsed()
  {
    return _zflags.testAny(IsUsed);
  }
  void setIsUsed(bool isUsed)
  {
    return _zflags.set(IsUsed, isUsed);
  }
