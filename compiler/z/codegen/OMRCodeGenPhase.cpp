...
void OMR::Z::CodeGenPhase::performMarkLoadAsZeroOrSignExtensionPhase(
  ...
  if (TR::Compiler->target.cpu.isZ() && cg->getOptimizationPhaseIsComplete()) {
    ...
    TR::OptimizationManager *manager =
        comp->getOptimizer()->getOptimization(OMR::loadExtensions);
    TR_ASSERT(manager, "Load extensions optimization should be initialized.");
    TR_LoadExtensions *loadExtensions =
        (TR_LoadExtensions *)manager->factory()(manager);
    loadExtensions->perform();
    delete loadExtensions;
  }
}

void OMR::Z::CodeGenPhase::performSetBranchOnCountFlagPhase(
    TR::CodeGenerator *cg, TR::CodeGenPhase *) {
...
void OMR::Z::CodeGenPhase::performPreRAPeepholePhase(TR::CodeGenerator *cg,
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(
        comp->getOutFile(), "Pre Register Assignment Peephole Instructions",
        false);
}

void OMR::Z::CodeGenPhase::performPeepholePhase(TR::CodeGenerator *cg,
                                                TR::CodeGenPhase *phase) {
  TR::Compilation *comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(
        comp->getOutFile(), "Post Register Assignment Peephole Instructions",
        false);
}

int OMR::Z::CodeGenPhase::getNumPhases() {
  return static_cast<int>(TR::CodeGenPhase::LastOMRZPhase);
}

const char *OMR::Z::CodeGenPhase::getName() {
