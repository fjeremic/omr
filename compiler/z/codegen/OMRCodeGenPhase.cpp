...
void
OMR::Z::CodeGenPhase::performMarkLoadAsZeroOrSignExtensionPhase(
  TR::CodeGenerator* cg,
...
void
OMR::Z::CodeGenPhase::performSetBranchOnCountFlagPhase(TR::CodeGenerator* cg,
                                                       TR::CodeGenPhase*)
{
...
void
OMR::Z::CodeGenPhase::performPreRAPeepholePhase(TR::CodeGenerator* cg,
                                                TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(
      comp->getOutFile(),
      "Pre Register Assignment Peephole Instructions",
      false);
}

void
OMR::Z::CodeGenPhase::performPeepholePhase(TR::CodeGenerator* cg,
                                           TR::CodeGenPhase* phase)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (comp->getOption(TR_TraceCG))
    comp->getDebug()->dumpMethodInstrs(
      comp->getOutFile(),
      "Post Register Assignment Peephole Instructions",
      false);
}
