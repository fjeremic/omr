...
uint8_t* TR::S390HelperCallSnippet::emitSnippetBody()
{
...
#endif
  {
    if (cg()->directCallRequiresTrampoline(destAddr, branchInstructionStartAddress)) {
      destAddr
        = TR::CodeCacheManager::instance()->findHelperTrampoline(helperSymRef->getReferenceNumber(), (void*)cursor);
  ...
  TR_ASSERT_FATAL(
    TR::Compiler->target.cpu.isTargetWithinBranchRelativeRILRange(destAddr, branchInstructionStartAddress),
