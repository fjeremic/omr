...
uint8_t *TR::S390HelperCallSnippet::emitSnippetBody() {
  ...
  if (jitInduceOSR) {
    ...
    cursor = TR::S390CallSnippet::S390flushArgumentsToStack(
        cursor, callNode, getSizeOfArguments(), cg());
  }
  ...
  cursor = generateRuntimeInstrumentationOnOffInstruction(
      cg(), cursor, TR::InstOpCode::RIOFF);
  ...
  } else // Otherwise:
  {
    ...
    intptrj_t returnAddr = (intptrj_t)getReStartLabel()
                               ->getCodeLocation(); // LARL   R14, <Return Addr>
  ...
  intptrj_t destAddr = (intptrj_t)(
      helperSymRef->getSymbol()->castToMethodSymbol()->getMethodAddress());
...
#endif
  {
    if (cg()->directCallRequiresTrampoline(destAddr,
                                           branchInstructionStartAddress)) {
      ...
      // We clobber rEP if we take a trampoline. Update our register map if
      // necessary.
  ...
  TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinBranchRelativeRILRange(
                      destAddr, branchInstructionStartAddress),
                  "Helper Call is not reachable.");
  this->setSnippetDestAddr(destAddr);

  *(int32_t *)cursor =
      (int32_t)((destAddr - branchInstructionStartAddress) / 2);
  AOTcgDiag1(cg()->comp(), "add TR_HelperAddress cursor=%x\n", cursor);
  cg()->addProjectSpecializedRelocation(cursor, (uint8_t *)helperSymRef, NULL,
...
uint32_t TR::S390HelperCallSnippet::getLength(int32_t) {
  ...
  if (helperSymRef->isOSRInductionHelper()) {
    length +=
        TR::S390CallSnippet::instructionCountForArguments(getNode(), cg());
  }
...
void TR_Debug::print(TR::FILE *pOutFile, TR::S390HelperCallSnippet *snippet) {
  ...
  printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                    "Helper Call Snippet", getName(helperSymRef));
