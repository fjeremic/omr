...
MemToMemConstLenMacroOp::generateLoop()
{
    ...
    bool inRange = len >= MVCL_THRESHOLD_LOW && len < MVCL_THRESHOLD_HIGH && (len % 4096) >= (uint64_t)1 && (len % 4096) <= (uint64_t)4089;
...
MemCpyAtomicMacroOp::MemCpyAtomicMacroOp(TR::Node* rootNode, TR::Node* dstNode, TR::Node* srcNode, TR::CodeGenerator* cg, TR::DataType destType, TR::Register* lenReg, TR::Node* lenNode, bool isForward, bool unroll, int32_t constLength)
