...
bool
useLockedLitPoolBaseRegister(TR::CodeGenerator* cg)
{
...
TR::Register*
allocateRegForLiteralPoolBase(TR::CodeGenerator* cg)
{
...
MemToMemVarLenMacroOp::generateLoop()
{
  ...
  if (!useEXForRemainder()) {
    ...
    if (_cg->needRelocationsForHelpers()) {
      generateRegLitRefInstruction(_cg,
                                   TR::InstOpCode::getLoadOpCode(),
                                   _rootNode,
                                   _raReg,
                                   (uintptrj_t)getHelperSymRef(),
                                   TR_HelperAddress,
                                   NULL,
                                   NULL,
                                   NULL);
  ...
  if (needs64BitOpCode) {
    generateRSInstruction(
      _cg, TR::InstOpCode::SRAG, _rootNode, _itersReg, _regLen, 8);
  } else {
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      generateRSInstruction(
        _cg, TR::InstOpCode::SRAK, _rootNode, _itersReg, _regLen, 8);
    } else {
      generateRRInstruction(
        _cg, TR::InstOpCode::LR, _rootNode, _itersReg, _regLen);
      generateRSInstruction(_cg, TR::InstOpCode::SRA, _rootNode, _itersReg, 8);
    }
  }

  generateS390BranchInstruction(
    _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, _rootNode, bottomOfLoop);

  generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _rootNode, topOfLoop);

  generateInstruction(0, 256);
  generateRXInstruction(_cg,
                        TR::InstOpCode::LA,
                        _srcNode,
                        _srcReg,
  ...
  generateS390BranchInstruction(
    _cg, TR::InstOpCode::BRCT, _rootNode, _itersReg, topOfLoop);
  ...
  if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10) &&
    ...
    if (useEXForRemainder()) {
      ...
      generateS390LabelInstruction(_cg,
                                   TR::InstOpCode::LABEL,
                                   _rootNode,
...
MemToMemMacroOp::genSrcLoadAddress(int32_t offset, TR::Instruction* cursor)
{
  ...
  cursor = _cg->genLoadAddressToRegister(
    _srcRegTemp,
    reuseS390MemoryReference(_srcMR, offset, _srcNode, _cg, false),
    _srcNode,
    cursor);
...
MemToMemMacroOp::genDstLoadAddress(int32_t offset, TR::Instruction* cursor)
{
  ...
  cursor = _cg->genLoadAddressToRegister(
    _dstRegTemp,
    reuseS390MemoryReference(_dstMR, offset, _dstNode, _cg, false),
    _dstNode,
    cursor);
...
MemToMemConstLenMacroOp::generateLoop()
{
  ...
  if (largeCopies != 0 && largeCopies < _maxCopies) {
    ...
    if (_srcReg == NULL || _dstReg == NULL || _srcReg != _dstReg) {
      ...
      if (largeCopies * 256 + dstMROffset >= 4096) {
        ...
        if (_dstReg == NULL)
          ...
          cursor = generateRXInstruction(_cg,
                                         TR::InstOpCode::LA,
                                         _dstNode,
                                         _dstReg,
                                         new (_cg->trHeapMemory())
                                           TR::MemoryReference(_dstReg, 0, _cg),
                                         cursor);
      }
    }
    ...
    while (largeCopies > 0) {
      cursor = generateInstruction(
        _offset + (copies - largeCopies) * 256, 256, cursor);
      --largeCopies;
    }
    len = len - copies * 256;
    _length = (int64_t)len;
  ...
  if (inRange && (aliasingPattern ||
    ...
    TR::RegisterDependencyConditions* dependencies =
      _cg->createDepsForRRMemoryInstructions(
        _rootNode, sourcePairRegister, targetPairRegister);
  ...
  if (TR::Compiler->target.is64Bit())
    cursor = genLoadLongConstant(
      _cg, _rootNode, largeCopies, _itersReg, cursor, NULL, NULL);
  else
    cursor = generateLoad32BitConstant(
      _cg, _rootNode, largeCopies, _itersReg, true, cursor, NULL, NULL);
  ...
  cursor = generateS390BranchInstruction(
    _cg, TR::InstOpCode::BRCT, _rootNode, _itersReg, topOfLoop, cursor);

  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _rootNode, bottomOfLoop, cursor);
  ...
  if (len >= (uint64_t)1) {
    if (_useByteVal)
      cursor = generateSIInstruction(
        _cg, TR::InstOpCode::MVI, _rootNode, _dstMR, _byteVal, cursor);
    else
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STC, _rootNode, _initReg, _dstMR, cursor);
...
MemInitVarLenMacroOp::getHelper()
{
  return (intptrj_t)_cg->symRefTab()
    ->findOrCreateRuntimeHelper(
      TR_S390arraySetGeneralHelper, false, false, false)
    ->getMethodAddress();
}
...
{
  return (intptrj_t)_cg->symRefTab()
    ->findOrCreateRuntimeHelper(TR_S390arraySetZeroHelper, false, false, false)
    ->getMethodAddress();
}
  ...
  if (noLoop()) {
    ...
    if (useEXForRemainder()) {
      dependencies->addPostCondition(
        _dstReg, TR::RealRegister::AssignAny, RefsAndDefsDependentRegister);
      if (_initReg)
        dependencies->addPostCondition(_initReg, TR::RealRegister::AssignAny);
      dependencies->addPostCondition(_itersReg, TR::RealRegister::AssignAny);
    } else {
      dependencies->addPostCondition(
        _dstReg, TR::RealRegister::GPR1, RefsAndDefsDependentRegister);
      if (_initReg)
    ...
    if (noLoop()) {
      ...
      if (useEXForRemainder()) {
        dependencies->addPostCondition(
          _dstReg, TR::RealRegister::AssignAny, RefsAndDefsDependentRegister);
        dependencies->addPostCondition(
          _srcReg, TR::RealRegister::AssignAny, RefsAndDefsDependentRegister);
        dependencies->addPostCondition(_itersReg, TR::RealRegister::AssignAny);
      } else {
        dependencies->addPostCondition(
          _dstReg, TR::RealRegister::GPR1, RefsAndDefsDependentRegister);
        dependencies->addPostCondition(
          _srcReg, TR::RealRegister::GPR2, RefsAndDefsDependentRegister);
        dependencies->addPostCondition(_itersReg, TR::RealRegister::GPR0);
      }
    }
    if (_inNestedICF) {
  ...
  if (useEXForRemainder()) {
    ...
    if (TR::Compiler->target.is64Bit())
      generateShiftAndKeepSelected64Bit(
        _rootNode, _cg, _regLen, _regLen, 52, 59, 4, true, false);
    else
      generateShiftAndKeepSelected31Bit(
        _rootNode, _cg, _regLen, _regLen, 20, 27, 4, true, false);

    TR::MemoryReference* targetMR =
      new (_cg->trHeapMemory()) TR::MemoryReference(_raReg, _regLen, 0, _cg);
    generateRXInstruction(
      _cg, TR::InstOpCode::BAS, _rootNode, _raReg, targetMR);
    _cursor = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _rootNode, remainderDoneLabel);
  }
  ...
  if (checkLengthAfterLoop()) {
    ...
    if (TR::Compiler->target.is64Bit()) {
      generateRILInstruction(
        _cg, TR::InstOpCode::NILF, _rootNode, _regLen, 0xFF);
    ...
    if (TR::Compiler->target.is64Bit())
      generateS390CompareAndBranchInstruction(_cg,
                                              TR::InstOpCode::CG,
                                              _rootNode,
                                              _regLen,
                                              (int32_t)0,
...
MemClearVarLenMacroOp::generateRemainder()
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    cursor = generateS390ImmOp(
      _cg, TR::InstOpCode::NG, _rootNode, _regLen, _regLen, (int64_t)0xFF);
    generateS390CompareAndBranchInstruction(_cg,
                                            TR::InstOpCode::CG,
                                            _rootNode,
                                            _regLen,
    ...
    cursor = generateS390ImmOp(
      _cg, TR::InstOpCode::N, _rootNode, _regLen, _regLen, (int32_t)0xFF);
    generateS390CompareAndBranchInstruction(_cg,
                                            TR::InstOpCode::C,
                                            _rootNode,
                                            _regLen,
...
TR::Instruction*
MemInitConstLenMacroOp::generateInstruction(int32_t offset,
                                            int64_t length,
                                            TR::Instruction* cursor1)
{
  ...
  if (length == 1 && !_useByteVal) {
    ...
    cursor = generateSS1Instruction(
      _cg, TR::InstOpCode::MVC, _rootNode, length - 1, _dstMR, _srcMR, cursor);
  }
...
TR::Instruction*
MemClearConstLenMacroOp::generateInstruction(int32_t offset,
                                             int64_t length,
                                             TR::Instruction* cursor1)
{
  ...
  if (srcMR == NULL) {
    ...
    srcMR = reuseS390MemoryReference(
      _srcMR, offset, _rootNode, _cg, true); // enforceSSLimits=true
  }
  ...
  if (_dstNode == _srcNode) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10) && length <= 8 &&
      ...
      switch (length) {
        case 8:
          cursor = generateSILInstruction(
            _cg, TR::InstOpCode::MVGHI, _rootNode, srcMR, 0, cursor);
          return cursor;
        case 4:
          cursor = generateSILInstruction(
            _cg, TR::InstOpCode::MVHI, _rootNode, srcMR, 0, cursor);
          return cursor;
        case 2:
          cursor = generateSILInstruction(
            _cg, TR::InstOpCode::MVHHI, _rootNode, srcMR, 0, cursor);
          return cursor;
        case 1:
          cursor = generateSIInstruction(
            _cg, TR::InstOpCode::MVI, _rootNode, srcMR, 0, cursor);
          return cursor;
      }
    } else if (length == 1) {
      cursor = generateSIInstruction(
  ...
  cursor = generateSS1Instruction(
    _cg, TR::InstOpCode::XC, _rootNode, length - 1, dstMR, srcMR, cursor);

  return cursor;
}

TR::Instruction*
MemToMemConstLenMacroOp::generateInstruction(int32_t offset,
                                             int64_t length,
                                             TR::Instruction* cursor1)
{
  TR::Compilation* comp = _cg->comp();
  ...
  if (srcMR == NULL) {
    ...
    srcMR = reuseS390MemoryReference(
      srcMR, offset, _rootNode, _cg, true); // enforceSSLimits=true
  }
  ...
  if (dstMR == NULL) {
    ...
    dstMR = reuseS390MemoryReference(
      dstMR, offset, _rootNode, _cg, true); // enforceSSLimits=true
  }

  cursor = _cg->getAppendInstruction();

  cursor = generateSS1Instruction(
    _cg, _opcode, _rootNode, length - 1, dstMR, srcMR, cursor);

  return cursor;
}

TR::Instruction*
MemCmpConstLenMacroOp::generateInstruction(int32_t offset,
                                           int64_t length,
                                           TR::Instruction* cursor1)
{
  TR::Compilation* comp = _cg->comp();
  ...
  cursor = _cg->getAppendInstruction();
  cursor = generateSS1Instruction(
    _cg, TR::InstOpCode::CLC, _rootNode, length - 1, dstMR, srcMR, cursor);

  if (!inRemainder()) {
    cursor = generateS390BranchInstruction(_cg,
                                           TR::InstOpCode::BRC,
...
BitOpMemVarLenMacroOp::generateInstruction(int32_t offset, int64_t length)
{
  ...
  cursor = _cg->getAppendInstruction();
  cursor = generateSS1Instruction(
    _cg, _opcode, _rootNode, length - 1, _dstMR, _srcMR, cursor);
...
MemCmpVarLenMacroOp::generateInstruction(int32_t offset, int64_t length)
{
  ...
  cursor = _cg->getAppendInstruction();
  cursor = generateSS1Instruction(
    _cg, TR::InstOpCode::CLC, _rootNode, length - 1, _dstMR, _srcMR, cursor);
...
generateCmpResult(TR::CodeGenerator* cg,
                  TR::Node* rootNode,
                  TR::Register* dstReg,
                  TR::Register* srcReg,
                  ...
                  TR::LabelSymbol* doneLabel)
{
  ...
  TR::Instruction* cursor;
  cursor = generateS390LabelInstruction(
    cg, TR::InstOpCode::LABEL, rootNode, doneLabel);

  return cursor;
}

TR::Instruction*
MemCmpVarLenMacroOp::generate(TR::Register* dstReg,
                              TR::Register* srcReg,
                              TR::Register* tmpReg,
                              int32_t offset,
                              TR::Instruction* cursor)
...
TR::Instruction*
MemCmpConstLenMacroOp::generate(TR::Register* dstReg,
                                TR::Register* srcReg,
                                TR::Register* tmpReg,
                                int32_t offset,
                                TR::Instruction* cursor)
...
static TR::Instruction*
generateCmpSignResult(TR::CodeGenerator* cg,
                      TR::Node* rootNode,
                      TR::Register* dstReg,
                      TR::Register* srcReg,
                      TR::Register* resultReg,
                      ...
                      TR::LabelSymbol* doneLabel)
{
  ...
  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, rootNode, falseLabel);
  generateRRInstruction(
    cg, TR::InstOpCode::IPM, rootNode, resultReg, resultReg);
  if (TR::Compiler->target.is64Bit()) {
    generateRSInstruction(
      cg, TR::InstOpCode::SLLG, rootNode, resultReg, resultReg, 34);
    generateRSInstruction(
      cg, TR::InstOpCode::SRAG, rootNode, resultReg, resultReg, 64 - 2);
  } else {
    generateRSInstruction(
      cg, TR::InstOpCode::SLL, rootNode, resultReg, 34 - 32);
    generateRSInstruction(
      cg, TR::InstOpCode::SRA, rootNode, resultReg, (64 - 2) - 32);
  }
  //
...
#else
  generateS390BranchInstruction(
    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, rootNode, falseLabel);

  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, rootNode, trueLabel);
  generateRRInstruction(
    cg, TR::InstOpCode::getXORRegOpCode(), rootNode, resultReg, resultReg);
  generateS390BranchInstruction(
    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, rootNode, doneLabel);

  generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, rootNode, falseLabel);
  generateS390BranchInstruction(
    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, rootNode, gtLabel);

  generateRIInstruction(
    cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), rootNode, resultReg, -1);
  generateS390BranchInstruction(
    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, rootNode, doneLabel);
...
TR::Instruction*
MemCmpVarLenSignMacroOp::generate(TR::Register* dstReg,
                                  TR::Register* srcReg,
                                  TR::Register* tmpReg,
                                  int32_t offset,
                                  TR::Instruction* cursor)
{
  ...
  _cursor = generateCmpSignResult(_cg,
                                  _rootNode,
                                  _dstReg,
                                  _srcReg,
                                  _resultReg,
...
TR::Instruction*
MemCmpConstLenSignMacroOp::generate(TR::Register* dstReg,
                                    TR::Register* srcReg,
                                    TR::Register* tmpReg,
                                    int32_t offset,
                                    TR::Instruction* cursor)
...
MemToMemTypedVarLenMacroOp::generateLoop()
{
  ...
  generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _dstNode, cFlowRegionStart);
  cFlowRegionStart->setStartInternalControlFlow();
  generateS390CompareAndBranchInstruction(_cg,
  ...
  if (_isForward) {
    ...
    generateRRInstruction(
      _cg, TR::InstOpCode::getLoadRegOpCode(), _dstNode, _endReg, _startReg);
    generateRIInstruction(_cg,
    ...
    TR::LabelSymbol* topOfLoop = generateLabelSymbol(_cg);
    generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _dstNode, topOfLoop);
    ...
    if (_srcReg != _startReg)
      generateRXInstruction(_cg,
                            ...
                            _srcReg,
                            new (_cg->trHeapMemory())
                              TR::MemoryReference(_srcReg, strideSize(), _cg));
    ...
    generateRRInstruction(
      _cg, TR::InstOpCode::getAddRegOpCode(), _dstNode, _startReg, _strideReg);

    generateRRInstruction(
      _cg, TR::InstOpCode::getLoadRegOpCode(), _dstNode, _endReg, _startReg);

    generateRRInstruction(
      _cg, TR::InstOpCode::getAddRegWidenOpCode(), _dstNode, _endReg, _lenReg);
  ...
  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _dstNode, doneLoop);
...
MemInitVarLenTypedMacroOp::generateInstruction()
{
  ...
  switch (_destType) {
    case TR::Int16:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STH, _dstNode, _initReg, newMR);
      break;
    case TR::Int32:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::ST, _dstNode, _initReg, newMR);
      break;
    case TR::Int64:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STG, _dstNode, _initReg, newMR);
      break;
    case TR::Float:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STE, _dstNode, _initReg, newMR);
      break;
    case TR::Double:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STD, _dstNode, _initReg, newMR);
      break;
    default:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STG, _dstNode, _initReg, newMR);
...
MemCpyVarLenTypedMacroOp::generateInstruction()
{
  ...
  switch (_destType) {
    case TR::Int8:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::IC, _srcNode, _workReg, srcMR);
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STC, _dstNode, _workReg, dstMR);
      break;
    case TR::Int16:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::LH, _srcNode, _workReg, srcMR);
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STH, _dstNode, _workReg, dstMR);
      break;
    case TR::Int32:
    case TR::Float:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::L, _srcNode, _workReg, srcMR);
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::ST, _dstNode, _workReg, dstMR);
      break;
    case TR::Int64:
    case TR::Double:
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::LG, _srcNode, _workReg, srcMR);
      cursor = generateRXInstruction(
        _cg, TR::InstOpCode::STG, _dstNode, _workReg, dstMR);
      break;
    case TR::Address:
      if (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) {
        if (_needsGuardedLoad) {
          cursor = generateRXInstruction(
            _cg, TR::InstOpCode::LGG, _srcNode, _workReg, srcMR);
        } else {
          cursor = generateRXInstruction(
            _cg, TR::InstOpCode::LG, _srcNode, _workReg, srcMR);
        }
        ...
        if (_needsGuardedLoad) {
          ...
          cursor = generateRXInstruction(
            _cg, TR::InstOpCode::LLGFSG, _srcNode, _workReg, srcMR);
          if (shiftAmount != 0) {
            cursor = generateRSInstruction(_cg,
                                           TR::InstOpCode::SRLG,
                                           _srcNode,
                                           _workReg,
                                           _workReg,
...
TR::Instruction*
MemCpyAtomicMacroOp::generateSTXLoop(int32_t strideSize,
                                     ...
                                     bool unroll)
{
  ...
  cursor = generateRRInstruction(
    _cg, TR::InstOpCode::getLoadRegOpCode(), _dstNode, _startReg, _endReg);
  ...
  if (!_isForward) {
    ...
    // size
    cursor = generateRIInstruction(_cg,
                                   TR::InstOpCode::getAddHalfWordImmOpCode(),
                                   _dstNode,
                                   _srcReg,
                                   -1 * strideSize);
    ...
    cursor = generateRIInstruction(_cg,
                                   ...
                                   _startReg,
                                   -1 * strideSize);
    cursor = generateRRInstruction(_cg,
    ...
    cursor = generateRRInstruction(
      _cg, TR::InstOpCode::getAddRegOpCode(), _dstNode, _startReg, _lenReg);
    cursor = generateRIInstruction(_cg,
                                   TR::InstOpCode::getLoadHalfWordImmOpCode(),
                                   _dstNode,
                                   _strideReg,
  ...
  if (_unrollFactor >= 3)
    cursor =
      generateInstruction(loadOp, storeOp, _workReg3, (int32_t)strideSize * 2);
  if (_unrollFactor >= 4)
    cursor =
      generateInstruction(loadOp, storeOp, _workReg4, (int32_t)strideSize * 3);
  if (_unrollFactor >= 5)
    cursor =
      generateInstruction(loadOp, storeOp, _workReg5, (int32_t)strideSize * 4);
  if (_unrollFactor >= 6)
    cursor =
      generateInstruction(loadOp, storeOp, _workReg6, (int32_t)strideSize * 5);
  if (_unrollFactor >= 7)
    cursor =
      generateInstruction(loadOp, storeOp, _workReg7, (int32_t)strideSize * 6);
  if (_unrollFactor >= 8)
    cursor =
      generateInstruction(loadOp, storeOp, _workReg8, (int32_t)strideSize * 7);
  ...
  if (_srcReg != _startReg) {
    if (!_isForward) {
      cursor = generateRXInstruction(
        ...
        _srcReg,
        new (_cg->trHeapMemory())
          TR::MemoryReference(_srcReg, -1 * _unrollFactor * strideSize, _cg));
    } else {
      cursor = generateRXInstruction(
        _cg,
        TR::InstOpCode::LA,
  ...
  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _dstNode, endOfLoop);
  ...
  cursor = generateRIInstruction(
    _cg, TR::InstOpCode::CHI, _dstNode, _strideReg, _unrollFactor * strideSize);
...
TR::Instruction*
MemCpyAtomicMacroOp::generateSTXLoopLabel(TR::LabelSymbol* oolStartLabel,
                                          ...
                                          TR::InstOpCode::Mnemonic storeOp)
{
  ...
  // Label to OOL
  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _srcNode, oolStartLabel);
  ...
  TR::InstOpCode::Mnemonic storeOp2)
{
  ...
  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _rootNode, oolStartLabel);
  ...
  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _rootNode, skipRoutineLabel);

  cursor = generateS390BranchInstruction(
    _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, _srcNode, doneCopyLabel);
...
MemCpyAtomicMacroOp::generateLoop()
{
  ...
  if (!_isForward) {
    cursor = generateRRInstruction(
      _cg, TR::InstOpCode::getAddRegOpCode(), _dstNode, _srcReg, _lenReg);
    cursor = generateRRInstruction(
      _cg, TR::InstOpCode::getAddRegOpCode(), _dstNode, _startReg, _lenReg);
  }
  ...
  if (_destType == TR::NoType) {
    ...
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
      ...
      cursor = generateRRRInstruction(
        _cg, mnemonic, _srcNode, _alignedReg, _srcReg, _startReg);
    } else {
      cursor = generateRRInstruction(_cg,
    ...
    cursor = generateRILInstruction(
      _cg, TR::InstOpCode::NILF, _srcNode, _alignedReg, 0x7);
    ...
    cursor = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _rootNode, fourByteLoop);
    cursor = generateRIInstruction(_cg,
                                   TR::InstOpCode::NILL,
                                   _srcNode,
                                   _alignedReg,
                                   0x3); // 0x3 == 11, last 2 bits
    ...
    cursor = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _rootNode, twoByteLoop);
    cursor = generateRIInstruction(_cg,
                                   TR::InstOpCode::NILL,
                                   _srcNode,
                                   _alignedReg,
    ...
    cursor = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _rootNode, oneByteLoop);
    ...
    if (_destType == TR::Int16) {
      ...
      cursor =
        generateS390CompareAndBranchInstruction(_cg,
                                                ...
                                                oolStartLabel2); // 0x2 = 0x010
      cursor = generateOneSTXthenSTYLoopLabel(oolStartLabel2,
                                              preDoneCopyLabel2,
                                              2,
                                              TR::InstOpCode::LH,
                                              TR::InstOpCode::STH,
                                              ...
                                              TR::InstOpCode::ST);
      cursor = generateS390LabelInstruction(
        _cg, TR::InstOpCode::LABEL, _dstNode, preDoneCopyLabel2);
      cursor = generateS390BranchInstruction(_cg,
                                             TR::InstOpCode::BRC,
                                             TR::InstOpCode::COND_BE,
                                             _srcNode,
      ...
      cursor =
        generateS390CompareAndBranchInstruction(_cg,
                                                ...
                                                0x6,
                                                TR::InstOpCode::COND_BE,
                                                oolStartLabel3); // 0x6 = 0x110
      cursor = generateOneSTXthenSTYLoopLabel(oolStartLabel3,
                                              preDoneCopyLabel3,
                                              2,
                                              TR::InstOpCode::LH,
                                              TR::InstOpCode::STH,
                                              ...
                                              TR::InstOpCode::ST);
      cursor = generateS390LabelInstruction(
        _cg, TR::InstOpCode::LABEL, _dstNode, preDoneCopyLabel3);
      cursor = generateS390BranchInstruction(_cg,
                                             TR::InstOpCode::BRC,
                                             TR::InstOpCode::COND_BE,
                                             _srcNode,
    ...
    cursor =
      generateS390CompareAndBranchInstruction(_cg,
                                              ...
                                              0x0,
                                              TR::InstOpCode::COND_BE,
                                              oolStartLabel4); // 0x0 = 0x000

    cursor = generateSTXLoopLabel(oolStartLabel4,
                                  preDoneCopyLabel4,
                                  8,
                                  TR::InstOpCode::LG,
                                  TR::InstOpCode::STG);
    cursor = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _dstNode, preDoneCopyLabel4);
    cursor = generateS390BranchInstruction(_cg,
                                           TR::InstOpCode::BRC,
                                           TR::InstOpCode::COND_BE,
                                           _srcNode,
    ...
    if (_destType == TR::Int16) {
      cursor =
        generateS390CompareAndBranchInstruction(_cg,
                                                ...
                                                0x4,
                                                TR::InstOpCode::COND_BE,
                                                oolStartLabel5); // 0x4 = 0x100
      cursor = generateSTXLoopLabel(oolStartLabel5,
                                    preDoneCopyLabel5,
                                    4,
                                    TR::InstOpCode::L,
                                    TR::InstOpCode::ST);
      cursor = generateS390LabelInstruction(
        _cg, TR::InstOpCode::LABEL, _dstNode, preDoneCopyLabel5);
      cursor = generateS390BranchInstruction(_cg,
                                             TR::InstOpCode::BRC,
                                             TR::InstOpCode::COND_BE,
                                             _srcNode,
    ...
    cursor =
      generateS390CompareAndBranchInstruction(_cg,
                                              ...
                                              0x8,
                                              TR::InstOpCode::COND_BE,
                                              oolStartLabel1); // 0x8 = 0x1000

    cursor = generateOneSTXthenSTYLoopLabel(oolStartLabel1,
                                            preDoneCopyLabel1,
                                            4,
                                            TR::InstOpCode::L,
                                            TR::InstOpCode::ST,
                                            ...
                                            TR::InstOpCode::STG);
    cursor = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _dstNode, preDoneCopyLabel1);
    cursor = generateS390BranchInstruction(_cg,
                                           TR::InstOpCode::BRC,
                                           TR::InstOpCode::COND_BE,
                                           _srcNode,
    ...
    cursor = generateS390BranchInstruction(_cg,
                                           ...
                                           _srcNode,
                                           oolStartLabel6);
    cursor = generateSTXLoopLabel(oolStartLabel6,
                                  preDoneCopyLabel6,
                                  strideSize(),
                                  unalignedLoadOp,
                                  unalignedStoreOp);
    cursor = generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _dstNode, preDoneCopyLabel6);
    cursor = generateS390BranchInstruction(_cg,
                                           TR::InstOpCode::BRC,
                                           TR::InstOpCode::COND_BE,
                                           _srcNode,
  ...
  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _dstNode, remainderLabel);
  ...
  if (generateRemainder && _isForward) {
    switch (_destType) {
      ...
      case TR::Double:
        ...
        break;
      case TR::Address:
        cursor = generateSTXLoop(
          strideSize(), unalignedLoadOp, unalignedStoreOp, false);
        break;
      default:
  ...
  cursor = generateS390LabelInstruction(
    _cg, TR::InstOpCode::LABEL, _dstNode, doneArrayCopyLabel);
...
void
MemCpyAtomicMacroOp::allocWorkReg()
{
  TR::Compilation* comp = _cg->comp();
  if (_trace) {
...
void
MemCpyAtomicMacroOp::createLoopDependencies(TR::Instruction* cursor)
{
  TR::Compilation* comp = _cg->comp();
  if (_trace)
