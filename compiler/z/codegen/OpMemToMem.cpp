...
TR::Instruction* MemToMemVarLenMacroOp::generateLoop()
{
  ...
  if (_srcReg != _dstReg) {
    generateRXInstruction(
      _cg, TR::InstOpCode::LA, _dstNode, _dstReg, new (_cg->trHeapMemory()) TR::MemoryReference(_dstReg, 256, _cg));
  }
...
TR::Instruction* MemToMemConstLenMacroOp::generateLoop()
{
  ...
  if (largeCopies != 0 && largeCopies < _maxCopies) {
    ...
    if (largeCopies * 256 + _offset + srcMROffset >= 4096) {
      // this early LA isn't needed for correctness as the later generated SS instructions will enforce the limits but
      // doing this upfront instead saves possibly multiple fixup LAs later on
      if (_srcReg == NULL)
  ...
  // detected) In order to use MVCL, we have also to make sure that the use is safe. i.e., src and dst are NOT aliased.
...
intptrj_t MemInitVarLenMacroOp::getHelper()
{
  return (intptrj_t)_cg->symRefTab()
    ->findOrCreateRuntimeHelper(TR_S390arraySetGeneralHelper, false, false, false)
    ->getMethodAddress();
}
  ...
  if (checkLengthAfterLoop()) {
    ...
    if (TR::Compiler->target.is64Bit())
      generateS390CompareAndBranchInstruction(
        _cg, TR::InstOpCode::CG, _rootNode, _regLen, (int32_t)0, TR::InstOpCode::COND_BNH, _doneLabel, false, false);
    else
      generateS390CompareAndBranchInstruction(
        _cg, TR::InstOpCode::C, _rootNode, _regLen, (int32_t)0, TR::InstOpCode::COND_BNH, _doneLabel, false, false);

    if (_firstByteInitialized)
      generateRIInstruction(
        _cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::AGHI : TR::InstOpCode::AHI, _rootNode, _regLen, -1);
  ...
  TR::Register* dstReg, TR::Register* srcReg, TR::Register* tmpReg, int32_t offset, TR::Instruction* cursor)
{
  ...
  if (_startControlFlow != _cursor) {
    ...
    generateS390LabelInstruction(
      _cg, TR::InstOpCode::LABEL, _rootNode, cFlowRegionStart, dependencies, _startControlFlow->getPrev());
...
TR::Instruction* MemCpyAtomicMacroOp::generateSTXLoopLabel(TR::LabelSymbol* oolStartLabel,
  TR::LabelSymbol* doneCopyLabel, int32_t strideSize, TR::InstOpCode::Mnemonic loadOp, TR::InstOpCode::Mnemonic storeOp)
{
  TR::Compilation* comp = _cg->comp();
  if (_trace)
  ...
  TR::InstOpCode::Mnemonic storeOp2)
{
  ...
  if (false && !_isForward) {
    ...
    cursor = generateRRInstruction(_cg, TR::InstOpCode::getSubstractRegOpCode(), _dstNode, _startReg, _lenReg);
    cursor
      = generateRIInstruction(_cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), _dstNode, _strideReg, -1 * strideSize1);
  } else {
    // Set _startReg to the last element to be copied (i.e. start + length - stride)
    cursor
      = generateRIInstruction(_cg, TR::InstOpCode::getAddHalfWordImmOpCode(), _dstNode, _startReg, -1 * strideSize1);
    cursor = generateRRInstruction(_cg, TR::InstOpCode::getAddRegOpCode(), _dstNode, _startReg, _lenReg);
...
TR::Instruction* MemCpyAtomicMacroOp::generateLoop()
{
  ...
  if (_destType == TR::NoType) {
    ...
    if (_destType == TR::Int16) {
      ...
      cursor = generateS390CompareAndBranchInstruction(
        _cg, TR::InstOpCode::C, _srcNode, _alignedReg, 0x2, TR::InstOpCode::COND_BE, oolStartLabel2); // 0x2 = 0x010
      ...
      cursor
        = generateS390BranchInstruction(_cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, _srcNode, remainderLabel);

      cursor = generateS390CompareAndBranchInstruction(
        _cg, TR::InstOpCode::C, _srcNode, _alignedReg, 0x6, TR::InstOpCode::COND_BE, oolStartLabel3); // 0x6 = 0x110
    ...
    if (_destType == TR::Int16) {
      cursor = generateS390CompareAndBranchInstruction(
        _cg, TR::InstOpCode::C, _srcNode, _alignedReg, 0x4, TR::InstOpCode::COND_BE, oolStartLabel5); // 0x4 = 0x100
    ...
    cursor = generateS390CompareAndBranchInstruction(
      _cg, TR::InstOpCode::C, _srcNode, _alignedReg, 0x8, TR::InstOpCode::COND_BE, oolStartLabel1); // 0x8 = 0x1000

    cursor = generateOneSTXthenSTYLoopLabel(oolStartLabel1, preDoneCopyLabel1, 4, TR::InstOpCode::L, TR::InstOpCode::ST,
      8, TR::InstOpCode::LG, TR::InstOpCode::STG);
    ...
    cursor
      = generateS390BranchInstruction(_cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, _srcNode, oolStartLabel6);
