...
TR::S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  switch (reloType) {
    ...
    case TR_ClassObject: {
      AOTcgDiag3(comp,
                 "add relocation (%d) cursor=%x symbolReference=%x\n",
                 reloType,
                 cursor,
                 getSymbolReference());
    ...
    case TR_GlobalValue:
      AOTcgDiag2(comp,
                 "add TR_GlobalValue (countForRecompile) cursor=%x\n",
                 reloType,
                 cursor);
      cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(
    ...
    case TR_DebugCounter: {
      ...
      AOTcgDiag3(comp,
                 "add relocation (%d) cursor=%x counter=%x\n",
                 reloType,
                 cursor,
                 counter);
    ...
    *(intptrj_t*)udsPatchLocation = (intptrj_t)cursor;
        ...
        cg()->wantToPatchClassPointer(NULL, cursor)) // unresolved
    {
      cg()->jitAddPicToPatchOnClassRedefinition(
        (void*)*(uintptrj_t*)cursor, (void*)cursor, true);
    }
  }
#endif
}
...
TR::S390ConstantDataSnippet::emitSnippetBody()
{
  ...
  AOTcgDiag1(
    comp, "TR::S390ConstantDataSnippet::emitSnippetBody cursor=%x\n", cursor);
  ...
  AOTcgDiag6(comp,
             "this=%p cursor=%p node=%p value=%p length=%x reloType=%x\n",
             this,
             cursor,
             getNode(),
             *((int*)cursor),
...
TR::S390JNICallDataSnippet::emitSnippetBody()
{
  ...
  */
  ...
  AOTcgDiag1(
    comp, "TR::S390JNICallDataSnippet::emitSnippetBody cursor=%x\n", cursor);
  ...
  *(intptrj_t*)cursor = (intptrj_t)_targetAddress;
  ...
  TR_PatchJNICallSite::make(cg()->fe(),
                            cg()->trPersistentMemory(),
                            (uintptrj_t)method,
                            cursor,
...
TR::S390JNICallDataSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
  ...
  */
  ...
  debug->printSnippetLabel(
    pOutFile, getSnippetLabel(), bufferPos, "JNI Call Data Snippet");
  ...
  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(
    pOutFile, "DC   \t%p \t\t# JNICallOutFrameFlags", *((intptrj_t*)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(
    pOutFile, "DC   \t%p \t\t# returnFromJNICall", *((intptrj_t*)bufferPos));
  bufferPos += sizeof(intptrj_t);
  ...
  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(
    pOutFile, "DC   \t%p \t\t# jitStackFrameFlags", *((intptrj_t*)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile,
            "DC   \t%p \t\t# constReleaseVMAccessMask",
            *((intptrj_t*)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile,
            "DC   \t%p \t\t# constReleaseVMAccessOutOfLineMask",
            *((intptrj_t*)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(
    pOutFile, "DC   \t%p \t\t# targetAddress", *((intptrj_t*)bufferPos));
  bufferPos += sizeof(intptrj_t);
}
...
TR_Debug::print(TR::FILE* pOutFile, TR::S390ConstantDataSnippet* snippet)
{
  ...
  if (snippet->getKind() == TR::Snippet::IsWritableData) {
    printSnippetLabel(
      pOutFile, snippet->getSnippetLabel(), bufferPos, "Writable Data Snippet");
    ...
    printSnippetLabel(
      pOutFile, snippet->getSnippetLabel(), bufferPos, "Constant Data Snippet");
  }
