...
void TR::S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t* cursor)
{
  ...
  case TR_ClassObject: {
    ...
    if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
      ...
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)getDataAs8Bytes(),
  ...
  case TR_ArrayCopyHelper:
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
                                  (uint8_t*)getSymbolReference(), (TR_ExternalRelocationTargetKind)reloType, cg()),
      __FILE__, __LINE__, getNode());
    break;
  ...
  case TR_HelperAddress:
    ...
    if (TR::Compiler->target.is64Bit()) {
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                    cursor, (uint8_t*)*((uint64_t*)cursor), TR_AbsoluteHelperAddress, cg()),
        __FILE__, __LINE__, getNode());
    } else {
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                    cursor, (uint8_t*)(intptr_t) * ((uint32_t*)cursor), TR_AbsoluteHelperAddress, cg()),
        __FILE__, __LINE__, getNode());
    }
    break;
  ...
  case TR_BodyInfoAddress:
    ...
    if (TR::Compiler->target.is64Bit()) {
      cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor,
                                    (uint8_t*)*((uint64_t*)cursor), (TR_ExternalRelocationTargetKind)reloType, cg()),
        __FILE__, __LINE__, getNode());
    } else {
      cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor, (uint8_t*)(intptr_t) * ((uint32_t*)cursor),
                                      (TR_ExternalRelocationTargetKind)reloType, cg()),
        __FILE__, __LINE__, getNode());
    }
    break;
  ...
  case TR_GlobalValue:
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                  TR::ExternalRelocation(cursor, (uint8_t*)TR_CountForRecompile, TR_GlobalValue, cg()),
      __FILE__, __LINE__, getNode());
    break;
  ...
  case TR_ClassPointer:
    ...
    if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
      ...
      TR::Relocation* relo;
      // for optimizations where we are trying to relocate either profiled j9class or getfrom signature we can't use
      // node to get the target address so we need to pass it to relocation in targetaddress2 for now two instances where
      // use this relotype in such way are: profile checkcast and arraystore check object check optimiztaions
      ...
      relo = new (cg()->trHeapMemory()) TR::ExternalRelocation(
        cursor, (uint8_t*)getNode(), targetAdress2, (TR_ExternalRelocationTargetKind)reloType, cg());
  ...
  case TR_DebugCounter: {
    ...
    if (counter == NULL) {
      cg()->comp()->failCompilation<TR::CompilationException>(
        "Could not generate relocation for debug counter in OMR::X86::MemoryReference::addMetaDataForCodeAddress\n");
    }
  ...
  if (comp->getOption(TR_EnableHCR)) {
    if (std::find(comp->getSnippetsToBePatchedOnClassRedefinition()->begin(),
          comp->getSnippetsToBePatchedOnClassRedefinition()->end(), this)
    ...
    if (std::find(comp->getSnippetsToBePatchedOnClassUnload()->begin(),
          comp->getSnippetsToBePatchedOnClassUnload()->end(), this)
    ...
    if (std::find(comp->getMethodSnippetsToBePatchedOnClassUnload()->begin(),
          comp->getMethodSnippetsToBePatchedOnClassUnload()->end(), this)
      != comp->getMethodSnippetsToBePatchedOnClassUnload()->end()) {
      void* classPointer = (void*)cg()
                             ->fe()
                             ->createResolvedMethod(cg()->trMemory(), (TR_OpaqueMethodBlock*)(*(uintptrj_t*)cursor),
...
uint8_t* TR::S390ConstantDataSnippet::emitSnippetBody()
{
  ...
  AOTcgDiag6(comp, "this=%p cursor=%p node=%p value=%p length=%x reloType=%x\n", this, cursor, getNode(),
    *((int*)cursor), _length, reloType);

  switch (reloType) {
  case TR_ClassAddress:
  case TR_ClassObject: {
    uintptrj_t romClassPtr
      = TR::Compiler->cls.persistentClassPointerFromClassPointer(comp, (TR_OpaqueClassBlock*)(*((uintptrj_t*)cursor)));
...
uint8_t* TR::S390JNICallDataSnippet::emitSnippetBody()
{
  ...
  *(intptrj_t*)cursor = (intptrj_t)_ramMethod;
  ...
  if (cg()->needClassAndMethodPointerRelocations()) {
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                  TR::ExternalRelocation(cursor, (uint8_t*)callNode->getSymbolReference(),
  ...
  *(intptrj_t*)cursor = (intptrj_t)_targetAddress;
  ...
  TR_PatchJNICallSite::make(
    cg()->fe(), cg()->trPersistentMemory(), (uintptrj_t)method, cursor, comp->getMetadataAssumptionList());
