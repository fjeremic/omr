  ...
  switch (reloType) {
    ...
    case TR_ClassObject: {
      ...
      if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
        ...
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory())
                TR::ExternalRelocation(cursor, (uint8_t *)getDataAs8Bytes(),
    ...
    case TR_ClassPointer:
      ...
      if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
        ...
        // j9class or getfrom signature we can't use node to get the target
        // address so we need to pass it to relocation in targetaddress2 for now
...
uint8_t *TR::S390ConstantInstructionSnippet::emitSnippetBody() {
  ...
  for (int i = 0; i < _length; ++i) _value[i] = cursor[i];
