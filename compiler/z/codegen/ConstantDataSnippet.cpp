...
TR::S390ConstantDataSnippet::S390ConstantDataSnippet(TR::CodeGenerator *cg,
                                                     TR::Node *n, void *c,
                                                     uint16_t size)
...
void TR::S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t *cursor) {
  ...
  case TR_ClassObject: {
    ...
    TR::SymbolReference *reloSymRef = (reloType == TR_ClassAddress)
    ...
    if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
      ...
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
              cursor, (uint8_t *)getDataAs8Bytes(),
  ...
  case TR_HelperAddress:
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
              cursor, (uint8_t *)(intptr_t) * ((uint32_t *)cursor),
              TR_AbsoluteHelperAddress, cg()),
          __FILE__, __LINE__, getNode());
    }
    break;
  ...
  case TR_BodyInfoAddress:
    ...
    if (TR::Compiler->target.is64Bit()) {
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
              cursor, (uint8_t *)*((uint64_t *)cursor),
              (TR_ExternalRelocationTargetKind)reloType, cg()),
          __FILE__, __LINE__, getNode());
    } else {
      cg()->addExternalRelocation(
  ...
  case TR_ClassPointer:
    ...
    if (cg()->comp()->getOption(TR_UseSymbolValidationManager)) {
      ...
      cg()->addExternalRelocation(
          new (cg()->trHeapMemory()) TR::ExternalRelocation(
              cursor, (uint8_t *)getDataAs8Bytes(), (uint8_t *)symbolKind,
              TR_SymbolFromManager, cg()),
          __FILE__, __LINE__, getNode());
    } else {
      TR::Relocation *relo;
      // for optimizations where we are trying to relocate either profiled
      // j9class or getfrom signature we can't use node to get the target address
      // so we need to pass it to relocation in targetaddress2 for now
      // two instances where use this relotype in such way are: profile
      // checkcast and arraystore check object check optimiztaions
      ...
      relo = new (cg()->trHeapMemory()) TR::ExternalRelocation(
          cursor, (uint8_t *)getNode(), targetAdress2,
          (TR_ExternalRelocationTargetKind)reloType, cg());
  ...
  case TR_DebugCounter: {
    TR::DebugCounterBase *counter = cg()->comp()->getCounterFromStaticAddress(
        getNode()->getSymbolReference());
    if (counter == NULL) {
  ...
  if (comp->getOption(TR_EnableHCR)) {
    if (std::find(comp->getSnippetsToBePatchedOnClassRedefinition()->begin(),
                  comp->getSnippetsToBePatchedOnClassRedefinition()->end(),
                  this) !=
        comp->getSnippetsToBePatchedOnClassRedefinition()->end()) {
      cg()->jitAddPicToPatchOnClassRedefinition(
          ((void *)(*(uintptrj_t *)cursor)), (void *)(uintptrj_t *)cursor);
    }

    if (std::find(comp->getSnippetsToBePatchedOnClassUnload()->begin(),
                  comp->getSnippetsToBePatchedOnClassUnload()->end(),
                  this) != comp->getSnippetsToBePatchedOnClassUnload()->end())
      cg()->jitAddPicToPatchOnClassUnload(((void *)(*(uintptrj_t *)cursor)),
                                          (void *)(uintptrj_t *)cursor);

    if (std::find(comp->getMethodSnippetsToBePatchedOnClassUnload()->begin(),
                  comp->getMethodSnippetsToBePatchedOnClassUnload()->end(),
                  this) !=
        comp->getMethodSnippetsToBePatchedOnClassUnload()->end()) {
      void *classPointer =
          (void *)cg()
              ...
              ->createResolvedMethod(
                  cg()->trMemory(),
                  (TR_OpaqueMethodBlock *)(*(uintptrj_t *)cursor),
                  comp->getCurrentMethod())
              ->classOfMethod();
      cg()->jitAddPicToPatchOnClassUnload(classPointer,
                                          (void *)(uintptrj_t *)cursor);
    }
  } else {
    if (std::find(comp->getSnippetsToBePatchedOnClassUnload()->begin(),
  ...
  if (getUnresolvedDataSnippet() != NULL) {
    uint8_t *udsPatchLocation =
        getUnresolvedDataSnippet()->getLiteralPoolPatchAddress();
    TR_ASSERT(
        udsPatchLocation != NULL,
        "Literal Pool Reference has NULL Unresolved Data Snippet patch site!");
    *(intptrj_t *)udsPatchLocation = (intptrj_t)cursor;
    getUnresolvedDataSnippet()->setLiteralPoolSlot(cursor);
    if (getUnresolvedDataSnippet()->getDataSymbol()->isClassObject() &&
        cg()->wantToPatchClassPointer(NULL, cursor)) // unresolved
    {
      cg()->jitAddPicToPatchOnClassRedefinition((void *)*(uintptrj_t *)cursor,
                                                (void *)cursor, true);
    }
  }
#endif
...
uint8_t *TR::S390ConstantDataSnippet::emitSnippetBody() {
  ...
  case TR_ClassAddress:
  case TR_ClassObject: {
    uintptrj_t romClassPtr =
        TR::Compiler->cls.persistentClassPointerFromClassPointer(
            comp, (TR_OpaqueClassBlock *)(*((uintptrj_t *)cursor)));
    memcpy(cursor, &romClassPtr, _length);
  } break;

  case TR_AbsoluteMethodAddress:
    TR_ASSERT(getNode()->getSymbol() &&
                  getNode()->getSymbol()->getStaticSymbol() &&
                  getNode()->getSymbol()->getStaticSymbol()->isStartPC(),
              "Expecting start PC");
    *(uintptr_t *)cursor = (uintptr_t)getNode()
                               ->getSymbol()
                               ->getStaticSymbol()
                               ->getStaticAddress();
    break;
...
uint32_t TR::S390ConstantDataSnippet::getLength(int32_t estimatedSnippetStart) {
  return _length;
}
...
TR::S390EyeCatcherDataSnippet::S390EyeCatcherDataSnippet(TR::CodeGenerator *cg,
                                                         TR::Node *n)
    : TR::S390ConstantDataSnippet(cg, n, NULL, 0) {
  // Cold Eyecatcher is used for padding of endPC so that Return Address for
  // exception snippets will never equal the endPC.
  /*
...
TR::S390WritableDataSnippet::S390WritableDataSnippet(TR::CodeGenerator *cg,
                                                     TR::Node *n, void *c,
                                                     uint16_t size)
...
uint32_t TR::S390JNICallDataSnippet::getSavedPCOffset() {
  return 3 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getTagBitsOffset() {
  return 4 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getPCOffset() {
  return 5 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR::S390JNICallDataSnippet::getLiteralsOffset() {
...
uint32_t TR::S390JNICallDataSnippet::getTargetAddressOffset() {
...
uint32_t TR::S390JNICallDataSnippet::getLength(int32_t estimatedSnippetStart) {
  return 12 *
         TR::Compiler->om
             .sizeofReferenceAddress(); /*one ptr more for possible padding */
}
...
TR::S390JNICallDataSnippet::S390JNICallDataSnippet(TR::CodeGenerator *cg,
    ...
    : TR::S390ConstantDataSnippet(cg, node, generateLabelSymbol(cg), 0),
      _baseRegister(0), _ramMethod(0), _JNICallOutFrameFlags(0),
      _returnFromJNICallLabel(0), _savedPC(0), _tagBits(0), _pc(0),
      _literals(0), _jitStackFrameFlags(0), _constReleaseVMAccessMask(0),
      _constReleaseVMAccessOutOfLineMask(0), _targetAddress(0) {
  return;
}
...
uint8_t *TR::S390JNICallDataSnippet::emitSnippetBody() {
  ...
  */
  ...
  int32_t padBytes =
      ((intptrj_t)cursor + alignSize - 1) / alignSize * alignSize -
      (intptrj_t)cursor;
  cursor += padBytes;
  ...
  *(intptrj_t *)cursor = (intptrj_t)_ramMethod;
  ...
  if (cg()->needClassAndMethodPointerRelocations()) {
    cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(
            cursor, (uint8_t *)callNode->getSymbolReference(),
            callNode ? (uint8_t *)(intptr_t)callNode->getInlinedSiteIndex()
                     : (uint8_t *)-1,
            (TR_ExternalRelocationTargetKind)reloType, cg()),
  ...
  *(intptrj_t *)cursor = (intptrj_t)_targetAddress;
  TR_OpaqueMethodBlock *method = getNode()
                                     ->getSymbol()
                                     ->castToResolvedMethodSymbol()
                                     ->getResolvedMethod()
                                     ->getPersistentIdentifier();
  TR_PatchJNICallSite::make(cg()->fe(), cg()->trPersistentMemory(),
                            (uintptrj_t)method, cursor,
...
void TR::S390JNICallDataSnippet::print(TR::FILE *pOutFile, TR_Debug *debug) {
  /*
         ramMethod
         JNICallOutFrameFlags
         returnFromJNICall
  ...
  */
  ...
  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# JNICallOutFrameFlags",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# returnFromJNICall",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);
  ...
  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# jitStackFrameFlags",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# constReleaseVMAccessMask",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# constReleaseVMAccessOutOfLineMask",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);

  debug->printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
  trfprintf(pOutFile, "DC   \t%p \t\t# targetAddress",
            *((intptrj_t *)bufferPos));
  bufferPos += sizeof(intptrj_t);
}
...
void TR_Debug::print(TR::FILE *pOutFile, TR::S390ConstantDataSnippet *snippet) {
  ...
  if (snippet->getKind() == TR::Snippet::IsWritableData) {
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                      "Writable Data Snippet");
  } else if (snippet->getKind() == TR::Snippet::IsEyeCatcherData) {
    // Cold Eyecatcher is used for padding of endPC so that Return Address for
    // exception snippets will never equal the endPC.
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                      "EyeCatcher Data Snippet");
    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(pOutFile, "Eye Catcher = JITM\n");
    return;
  } else if (snippet->getKind() == TR::Snippet::IsConstantInstruction) {
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                      "Constant Instruction Snippet");
    print(pOutFile,
          ((TR::S390ConstantInstructionSnippet *)snippet)->getInstruction());
    return;
  } else if (snippet->getKind() == TR::Snippet::IsInterfaceCallData) {
#ifdef J9_PROJECT_SPECIFIC
    print(pOutFile,
          reinterpret_cast<TR::J9S390InterfaceCallDataSnippet *>(snippet));
#endif
    return;
  } else {
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos,
                      "Constant Data Snippet");
  }
