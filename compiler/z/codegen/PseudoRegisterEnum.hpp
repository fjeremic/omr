...
EvenOddPair = LastVRF + 1, // Assign an even/odd pair to the reg pair
  LegalEvenOfPair =
    LastVRF +
    2, // Assign an even reg that is followed by an unlocked odd register
  LegalOddOfPair =
    LastVRF +
    3, // Assign an odd reg that is preceded by an unlocked even register
  ...
  MayDefine =
    LastVRF + 9, // This instruction's result should be modelled as live before
                 // as this instruction only 'may defines' the register
