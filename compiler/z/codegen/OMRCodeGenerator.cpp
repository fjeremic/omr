...
OMR::Z::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(), _extentOfLitPool(-1),
      ...
      _interfaceSnippetToPICsListHashTab(NULL), _currentCheckNode(NULL),
      ...
      _firstTimeLiveOOLRegisterList(NULL), _methodBegin(NULL), _methodEnd(NULL),
      _afterRA(false) {
...
TR::RealRegister *
OMR::Z::CodeGenerator::getStackPointerRealRegister(TR::Symbol *symbol) {
  TR::Linkage *linkage = self()->getS390Linkage();
  ...
  if (size != 0) {
    ...
    case 2: {
      cursor = new (self()->trHeapMemory())
          TR::S390NOPInstruction(TR::InstOpCode::NOP, 2, node, cursor, self());
    } break;

    case 4: {
      cursor = new (self()->trHeapMemory())
          TR::S390NOPInstruction(TR::InstOpCode::NOP, 4, node, cursor, self());
    } break;
...
void OMR::Z::CodeGenerator::beginInstructionSelection() {
  ...
  if (self()->comp()->getJittedMethodSymbol()->getLinkageConvention() ==
    ...
    _returnTypeInfoInstruction = new (self()->trHeapMemory())
        TR::S390ImmInstruction(TR::InstOpCode::DC, startNode, 0, NULL, cursor,
                               self());
  }
  ...
  if (true) //! isInLoop || !bppMaxPath)
  {
    ...
    while (first != cursor) {
      ...
      if (real != NULL && op.getOpCodeValue() == TR::InstOpCode::DEPEND &&
        ...
        if (cursor->getNext() &&
            first !=
                cursor->getNext()) // op.getOpCodeValue() ==
              ...
              if (instructionCursor->getNode()
                ...
                if ((instructionCursor->getNode()
                  ...
                  if (instructionCursor->getNode()
                    ...
                    TR_ASSERT(1, "ASSERTION - Branch found outside Internal "
 ...
 */
TR::RegisterPair *
OMR::Z::CodeGenerator::allocateFPRegisterPair(TR::Register *lowRegister,
...
OMR::Z::CodeGenerator::findOrCreateConstant(TR::Node *node, void *c,
                                            uint16_t size) {
  CS2::HashIndex hi;
...
TR::S390ConstantDataSnippet *
OMR::Z::CodeGenerator::CreateConstant(TR::Node *node, void *c, uint16_t size,
                                      bool writable) {
...
OMR::Z::CodeGenerator::createLiteralPoolSnippet(TR::Node *node) {
  ...
  case TR::Address:
    ...
    if (node->isClassUnloadingConst()) {
      TR_ASSERT(
          0,
          "Shouldn't come here, isClassUnloadingConst is done in aloadHelper");
      if (TR::Compiler->target.is64Bit()) {
      ...
      if (node->isMethodPointerConstant())
        self()->comp()->getMethodSnippetsToBePatchedOnClassUnload()->push_front(
            targetSnippet);
      else
...
TR::S390ConstantDataSnippet *
OMR::Z::CodeGenerator::Create4ByteConstant(TR::Node *node, int32_t c,
                                           bool writable) {
  return self()->CreateConstant(node, &c, 4, writable);
}

TR::S390ConstantDataSnippet *
OMR::Z::CodeGenerator::Create8ByteConstant(TR::Node *node, int64_t c,
                                           bool writable) {
...
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(
  ...
  default:
    TR_ASSERT(0, "No appropriate instruction to use for sign extend this "
                 "number of bits!\n");
    break;
  }
...
bool OMR::Z::CodeGenerator::clearHighOrderBits(TR::Node *node,
  ...
  default:
    TR_ASSERT(0, "No appropriate instruction to use for clearing this number "
                 "of bits!\n");
    break;
  }
...
uint32_t getRegMaskFromRange(TR::Instruction *inst) {
  ...
  TR_ASSERT(lowRegNum >= 0 && lowRegNum <= 15 && highRegNum >= 0 &&
                highRegNum <= 15,
...
bool OMR::Z::CodeGenerator::reliesOnAParticularSignEncoding(TR::Node *node) {
  TR::ILOpCode op = node->getOpCode();
  if (op.isBCDStore() &&
      !node->chkCleanSignInPDStoreEvaluator()) // most common case of needing so
                                               // put up-front -- the value
                                               // escapes
    return true;
...
bool OMR::Z::CodeGenerator::loadAndStoreMayOverlap(TR::Node *store,
                                                   ...
                                                   TR::Node *load,
                                                   size_t loadSize) {
  TR_ASSERT(store->getOpCode().hasSymbolReference() &&
                store->getSymbolReference(),
  ...
  if (loadNode->getOpCode().isIndirect() &&
    ...
    return false;
             ...
             loadNode->getReferenceCount() ==
                 1) { // The const may be loaded into reg and shared. So dont do
                      // it.
    return true;
