    ...
    , _afterRA(false)
{
    ...
    self()->setSupportsTestCharComparisonControl(); // TRXX instructions on Danu have mask to disable test char
                                                    // comparison.
    ...
    if (self()->comp()->getJittedMethodSymbol()->getLinkageConvention() == TR_Private) {
        ...
        if (self()->comp()->getJittedMethodSymbol()->isJNI()
            ...
            cursor = new (self()->trHeapMemory()) TR::S390ImmInstruction(
                TR::InstOpCode::DC, startNode, UPPER_4_BYTES(jniMethodTargetAddress), cursor, self());

            if (TR::Compiler->target.is64Bit()) {
                cursor = new (self()->trHeapMemory()) TR::S390ImmInstruction(
                    TR::InstOpCode::DC, startNode, LOWER_4_BYTES(jniMethodTargetAddress), cursor, self());
            }
        }

        _returnTypeInfoInstruction = new (self()->trHeapMemory())
            TR::S390ImmInstruction(TR::InstOpCode::DC, startNode, 0, NULL, cursor, self());
    }
                    ...
                    if (instructionCursor->getNode() != NULL) {
                        if (instructionCursor->getNode()->getBranchDestination() != NULL) {
                            if (instructionCursor->getNode()->getBranchDestination()->getNode() != NULL) {
                                if ((instructionCursor->getNode()->getBranchDestination()->getNode()->getBlock()
                                        != NULL)
                                    && (instructionCursor->getNode()->getBlock() != NULL)) {
                                    // if branching to the same basic block then the branch should be located inside ICF
                                    // otherwise it's potentially not a case where the branch has to be inside Internal
                                    // Control Flow
                                    if (instructionCursor->getNode()
                                        ...
                                        TR_ASSERT(1,
                                            "ASSERTION - Branch found outside Internal Control Flow that should be "
                                            "located inside Internal Control Flow\n");
                                    }
                                }
                            }
                        }
...
void OMR::Z::CodeGenerator::printStats(int32_t instCount)
{
    ...
    fprintf(stderr,
        ...
        self()->comp()->getHotnessName(self()->comp()->getMethodHotness()), (double)(totHot / instCount),
        _totalHotSpills, _totalHotRegisterXfers, _totalHotRegisterMoves, _totalColdSpills, _totalColdRegisterXfers,
        ...
        if (data.cursorInstruction->getOpCodeValue() == TR::InstOpCode::RET) {
            ...
            if (skipOneReturn == false) {
                ...
                if (self()->comp()->getOption(TR_EnableLabelTargetNOPs)) {
                    for (TR::Instruction* inst = temp->getNext(); inst != originalNextInstruction;
                         inst = inst->getNext()) {
        ...
        while (oiIterator != self()->getS390OutOfLineCodeSectionList().end()) {
            ...
            if (needsETE && block && !block->getExceptionSuccessors().empty()) {
                uint32_t startOffset
                    = (*oiIterator)->getFirstInstruction()->getBinaryEncoding() - self()->getCodeStart();
                uint32_t endOffset
...
bool OMR::Z::CodeGenerator::usesImplicit64BitGPRs(TR::Node* node)
{
    ...
    return node->getOpCodeValue() == TR::pd2df || node->getOpCodeValue() == TR::pd2dd
        || node->getOpCodeValue() == TR::pd2de || node->getOpCodeValue() == TR::df2pd
        || node->getOpCodeValue() == TR::dd2pd || node->getOpCodeValue() == TR::de2pd;
#else
    return false;
#endif
}
