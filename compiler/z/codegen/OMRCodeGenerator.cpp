    ...
    if (parent->getOpCodeValue() == TR::ifacmpne || parent->getOpCodeValue() == TR::ificmpeq
      ...
      if (!parent->isNopableInlineGuard() || !self()->getSupportsVirtualGuardNOPing()) {
        ...
        if (!parent->isHCRGuard() && !parent->isOSRGuard() && !self()->comp()->performVirtualGuardNOPing()
          && self()->comp()->isVirtualGuardNOPingRequired(virtualGuard) && virtualGuard->canBeRemoved()) {
          node->setUnneededIALoad(false);
        }
      }
  ...
  TR::Node* node, int32_t childNumberOfNode, TR::Node* parent, TR::TreeTop* tt)
{
  ...
  // Z
  if (TR::Compiler->target.cpu.isZ() && (node->getOpCode().isLoadVar() || node->getOpCode().isStore())
  ...
  , _afterRA(false)
{
  ...
  if (enableBranchPreload
    || (!disableBranchPreload && comp->isOptServer() && TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)))
  ...
  if (self()->supportsBranchPreloadForCalls()) {
    _callsForPreloadList = new (self()->trHeapMemory())
  ...
  self()
    ->setSupportsTestCharComparisonControl(); // TRXX instructions on Danu have mask to disable test char comparison.
  ...
  if (comp->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRARegisterStates)) {
    self()->setGPRegisterIterator(new (self()->trHeapMemory())
        TR::RegisterIterator(self()->machine(), TR::RealRegister::FirstGPR, TR::RealRegister::LastAssignableGPR));
    self()->setFPRegisterIterator(new (self()->trHeapMemory())
        TR::RegisterIterator(self()->machine(), TR::RealRegister::FirstFPR, TR::RealRegister::LastFPR));
    self()->setVRFRegisterIterator(new (self()->trHeapMemory())
        TR::RegisterIterator(self()->machine(), TR::RealRegister::FirstVRF, TR::RealRegister::LastVRF));
  }
  ...
  if (!_globalRegisterTable) {
    ...
    if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
      ...
      for (grn = 0; grn < self()->getNumberOfGlobalRegisters(); grn++) {
        ...
        TR_ASSERT(reg != -1,
          "Register pressure simulator doesn't support gaps in the global register table; reg %d must be removed", grn);
...
void OMR::Z::CodeGenerator::beginInstructionSelection()
{
  ...
  if (self()->comp()->getJittedMethodSymbol()->getLinkageConvention() == TR_Private) {
    ...
    if (self()->comp()->getJittedMethodSymbol()->isJNI() && !self()->comp()->getOption(TR_MimicInterpreterFrameShape)) {
      TR::ResolvedMethodSymbol* methodSymbol = self()->comp()->getJittedMethodSymbol();
      intptrj_t jniMethodTargetAddress
        = (intptrj_t)methodSymbol->getResolvedMethod()->startAddressForJNIMethod(self()->comp());

      cursor = new (self()->trHeapMemory())
        TR::S390ImmInstruction(TR::InstOpCode::DC, startNode, UPPER_4_BYTES(jniMethodTargetAddress), cursor, self());

      if (TR::Compiler->target.is64Bit()) {
        cursor = new (self()->trHeapMemory())
          TR::S390ImmInstruction(TR::InstOpCode::DC, startNode, LOWER_4_BYTES(jniMethodTargetAddress), cursor, self());
      }
    }

    _returnTypeInfoInstruction
      = new (self()->trHeapMemory()) TR::S390ImmInstruction(TR::InstOpCode::DC, startNode, 0, NULL, cursor, self());
  }
...
{
  TR_BranchPreloadCallData* data = new (self()->trHeapMemory()) TR_BranchPreloadCallData(
    instr, callLabel, self()->comp()->getCurrentBlock(), callSymRef, self()->comp()->getCurrentBlock()->getFrequency());
...
void OMR::Z::CodeGenerator::insertInstructionPrefetches()
{
  if (self()->supportsBranchPreload()) {
    if (_hottestReturn._frequency > 6) {
      ...
      cursor = generateRXInstruction(self(), TR::InstOpCode::getExtendedLoadOpCode(), node, tempReg, tempMR, cursor);
  ...
  if (self()->supportsBranchPreloadForCalls()) {
    ...
    for (auto iterator = self()->getCallsForPreloadList()->begin(); iterator != self()->getCallsForPreloadList()->end();
         ++iterator)
    ...
    for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
      ...
      while (first != cursor) {
        ...
        if (real != NULL && op.getOpCodeValue() == TR::InstOpCode::DEPEND && real->getOpCode().isLabel()) {
          ...
          if (cursor->getNext()
            && first
              != cursor) // op.getOpCodeValue() == TR::InstOpCode::BPRP || op.getOpCodeValue() == TR::InstOpCode::BPP)
          {
            op = cursor->getNext()->getOpCode();
            if (canReachWithBPRP
              && op.getOpCodeValue() == TR::InstOpCode::BPRP) // || op.getOpCodeValue() == TR::InstOpCode::BPP)
            {
      ...
      if (bppCall && (!canReachWithBPRP || bppOnly)) {
        ...
        cursor = new (self()->trHeapMemory()) TR::S390RILInstruction(
          TR::InstOpCode::LARL, node, tempReg, (data->_callSymRef)->getSymbol(), data->_callSymRef, cursor, self());
    ...
    if (self()->comp()->getOption(TR_Randomize)) {
      if (randomizer.randomBoolean(300)
        && performTransformation(self()->comp(),
          "O^O Random Codegen - Added 6000 dummy slots to  Literal Pool frame to test  Literal Pool displacement.\n"))
...
void OMR::Z::CodeGenerator::doRegisterAssignment(TR_RegisterKinds kindsToAssign)
{
  ...
  if (!self()->comp()->getOption(TR_DisableOOL)) {
    TR::list<TR::Register*>* firstTimeLiveOOLRegisterList = new (self()->trHeapMemory())
    ...
    if (!self()->isOutOfLineColdPath()) {
      TR::list<TR::Register*>* spilledRegisterList = new (self()->trHeapMemory())
  ...
  while (instructionCursor) {
    ...
    if (instructionCursor->getOpCodeValue() == TR::InstOpCode::FENCE && treeNode->getOpCodeValue() == TR::BBEnd) {
      ...
      for (int8_t i = TR::RealRegister::GPR0; i != TR::RealRegister::GPR15 + 1; i++) {
        if (self()->machine()->realRegister(static_cast<TR::RealRegister::RegNum>(i))->getState()
          ...
          if (instructionCursor->getNode() != NULL) {
            if (instructionCursor->getNode()->getBranchDestination() != NULL) {
              if (instructionCursor->getNode()->getBranchDestination()->getNode() != NULL) {
                if ((instructionCursor->getNode()->getBranchDestination()->getNode()->getBlock() != NULL)
                  ...
                  if (instructionCursor->getNode()->getBranchDestination()->getNode()->getBlock()->getNumber()
                    == instructionCursor->getNode()->getBlock()->getNumber()) {
                    TR_ASSERT(1,
                      "ASSERTION - Branch found outside Internal Control Flow that should be located inside Internal "
                      "Control Flow\n");
                  }
                }
              }
...
void OMR::Z::CodeGenerator::printStats(int32_t instCount)
{
  ...
  fprintf(stderr,
    "REGISTER MOTION Compiled as [%10.10s] Factor [%4.4f] Contains hot blocks with "
    "[spills:%4.4d,xfers:%4.4d,moves:%4.4d] cold blocks with [spills:%4.4d,xfers:%4.4d,moves:%4.4d] for %s\n",
    self()->comp()->getHotnessName(self()->comp()->getMethodHotness()), (double)(totHot / instCount), _totalHotSpills,
...
void OMR::Z::CodeGenerator::StopUsingEscapedMemRefsRegisters(int32_t topOfMemRefStackBeforeEvaluation)
{
  while (_stackOfMemoryReferencesCreatedDuringEvaluation.topIndex() > topOfMemRefStackBeforeEvaluation) {
    ...
    // 2) AddFoldedMemRefToStack is only defined for OMR::Z::CodeGenerator, and requires TR::MemoryReference instead of
    // TR_MemoryReference.
    //
...
{
  return self()->getSupportsVirtualGuardNOPing() && self()->comp()->performVirtualGuardNOPing()
...
bool OMR::Z::CodeGenerator::anyNonConstantSnippets()
{
  ...
  for (auto iterator = _snippetList.begin(); iterator != _snippetList.end(); ++iterator) {
    if ((*iterator)->getKind() != TR::Snippet::IsConstantData || (*iterator)->getKind() != TR::Snippet::IsWritableData
...
void OMR::Z::CodeGenerator::doBinaryEncoding()
{
  ...
  if (self()->comp()->getJittedMethodSymbol()->isJNI() && !self()->comp()->getOption(TR_FullSpeedDebug)) {
    data.preProcInstruction = TR::Compiler->target.is64Bit() ? data.cursorInstruction->getNext()->getNext()->getNext()
  ...
  while (instr) {
    ...
    if (reg) {
      if (reg->getRegisterPair()) {
        ...
        if (instr->getOpCodeValue() == TR::InstOpCode::LM || instr->getOpCodeValue() == TR::InstOpCode::LMG) {
  ...
  if (node) {
    ...
    if (node->getOpaquePseudoRegister()) {
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
          "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg %s - handle extra ref to addr child "
          "(ref is node based %s - %s %p)\n",
    ...
    if (node->safeToDoRecursiveDecrement()) {
      ...
      // i.e. a direct load that has no future refs or a indirect load with no future refs that itself has a child that
      // is safeToRecDec
      self()->recursivelyDecReferenceCount(node);
    } else {
      self()->evaluate(node);
      ...
      if (node->getOpaquePseudoRegister() && !alreadyProcessedUnusedStorageRef) {
        ...
        if (self()->traceBCDCodeGen())
          traceMsg(self()->comp(),
            ...
            ref && ref->isNodeBased() ? "yes" : "no", ref->isNodeBased() ? ref->getNode()->getOpCode().getName() : "",
...
{
  uint8_t numDeps
    = 8 + extraDeps; // TODO: only 6 deps are created below so why is there is always an implicit 2 extra deps created?
  ...
  int8_t linkageRegisterIndex, TR::DataType type)
{
...
#ifdef J9_PROJECT_SPECIFIC
    || type == TR::DecimalFloat || type == TR::DecimalDouble || type == TR::DecimalLongDouble
#endif
  );
  ...
  if (isFloat) {
    ...
    result
      = self()->getGlobalRegisterNumber(self()->getS390Linkage()->getIntegerArgumentRegister(linkageRegisterIndex) - 1);
  }
...
bool OMR::Z::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate* rc, TR::Node* branchNode)
{
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
...
#ifdef J9_PROJECT_SPECIFIC
      || dt == TR::DecimalFloat || dt == TR::DecimalDouble || dt == TR::DecimalLongDouble
#endif
    );
  ...
  if (node->getOpCode().isSwitch()) {
    ...
    if (node->getSecondChild()->getOpCode().isLoadConst()) {
      ...
      if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) && value >= MIN_IMMEDIATE_BYTE_VAL
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(TR::Node* node)
{
  return 8;
}
  ...
  TR::Node* node, TR::MemoryReference* sourceMR) // sourceMR can be NULL
{
  ...
  if (isBCD) {
    ...
    TR_ASSERT(
      bcdSrcRegister, "bcdSrcRegister should be non-NULL for bcd node %s (%p)\n", node->getOpCode().getName(), node);
  }
  ...
  if (needsClobberDueToRefCount || needsClobberDueToRegCount || needsClobberDueToReuse) {
    ...
    if (self()->traceBCDCodeGen())
      traceMsg(self()->comp(),
        "ssrClobberEvaluate: node %s (%p) with srcReg %s (srcRegSize %d, srcReg->validSymSize %d, srcReg->liveSymSize "
        "%d) and srcStorageReference #%d with %s (symSize %d)\n",
        node->getOpCode().getName(), node, self()->getDebug()->getName(srcRegister), srcRegister->getSize(),
        srcRegister->getValidSymbolSize(), srcRegister->getLiveSymbolSize(), srcStorageReference->getReferenceNumber(),
    ...
    if (srcStorageReference->isReadOnlyTemporary() && srcRegister->getRightAlignedIgnoredBytes() > 0) {
      // The MVC must copy over the entire source field including any ignored (but still valid) bytes (but dead bytes
      // are not included as these have been invalidated/clobbered). The subsequent uses of this copied field (via
      // copyStorageReference) will adjust based on the owning registers' ignoredBytes/deadBytes setting
    ...
    if (self()->traceBCDCodeGen() && alreadyClearedDigits) {
      ...
      traceMsg(self()->comp(),
        ...
        "digitsToClear = %d - %d = %d)\n",
        alreadyClearedDigits, self()->getDebug()->getName(srcRegister), copyStorageReference->getReferenceNumber(),
        digitsInSrc, digitsToClear, alreadyClearedDigits);
    }
    ...
    if (srcStorageReference->getNodesToUpdateOnClobber()) {
      ...
      //
      //       AP    t1,t2 (pdadd)
      //       ZAP   t1,t1 mark t1 as readOnly, add pdcleanA (w/regA) to list (pdcleanA), increment t1 refCount by 2
      //       (pdaddA nodeRefCount) nop         mark t1 as readOnly (redundant), add pdcleanB (w/regB) to list
      //       (pdcleanB), increment t1 refCount by 3 (pdcleanA nodeRefCount) UNPK  t3,t1 non-clobber operation, t1 is
      //       still alive and readOnly (pd2zd)
      //    ...
      //    pd2zd
      //       =>pdcleanA
      //
      ...
      //       =>pdcleanB regB
      //
      //       MVC   t4,t1  clobber eval for pdadd parent, regB has other use so updated above, go through list and
      //       update regA with t4 (on clobber eval for pdaddB) AP    t1,tx
      //    ...
      ...
      for (TR::Node* listNode = listIt.getFirst(); listNode; listNode = listIt.getNext()) {
        ...
        if (listReg != srcRegister) {
          if (listReg->getStorageReference() != srcStorageReference) {
            ...
            // skipCopyOnStore/changeCommonedChildAddress in pdstoreEvaluator so skip this update (the reference counts
            // of the address subtree will be wrong if the refCount is update here. See addsub2 first pdstore on
            // line_no=335 with disableVIP)
            //
            // ipdstore p=29 symA (store then does a ZAP and updates child VTS_2 to symA)
            //    aiuadd
            //       aload
            //       iconst
            //    pdModPrecA   p=29 (passThrough so skip ssrClobber evaluate and reuse VTS_2 as a read only temp) 1 ref
            //    remaining to pdModPrecA
            ...
            // ipdstore p=30
            //    addr
            //    pdModPrecB   p=30 (non-passThrough, needs NI, so needs clobberEval with copy ref VTS_3 and goes to
            //    update pdModPrecA symA -> VTS_3)
            //       =>pdaddOverflowMessage p=31 (VTS_2)
            //
            // prevent the symA -> VTS_3 update as symA != the orig VTS_2
            // if the update is done then the last pdModPrecA ref is now using VTS_3 and the final decReferenceCount of
            // the symA aload is never done (live reg alive bug)
            //
            if (self()->traceBCDCodeGen())
              traceMsg(self()->comp(),
                "\ty^y : skip update on reg %s node %s (%p), storageRef has already changed -- was #%d (%s), now #%d "
                "(%s)\n",
            ...
            if (self()->traceBCDCodeGen())
              traceMsg(self()->comp(),
                "\ty^y : update reg %s with ref #%d (%s) on node %s (%p) refCount %d from _nodesToUpdateOnClobber with "
            ...
            if (listNode->getReferenceCount() > 0)
              listReg->setStorageReference(
                copyStorageReference, listNode); // TODO: safe to transfer alreadyClearedDigits in this case too?
          }
        }
      }
  ...
  int32_t* cursor, TR::LabelSymbol* label, int8_t addressDifferenceDivisor, bool isInstrOffset)
{
  if (isInstrOffset) {
    ...
    *(int16_t*)cursor
      = (int16_t)(((intptrj_t)label->getCodeLocation() - (((intptrj_t)cursor) - addressDifferenceDivisor))
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::apply32BitLabelTableRelocation(int32_t* cursor, TR::LabelSymbol* label)
{
...
TR::S390ConstantInstructionSnippet* OMR::Z::CodeGenerator::createConstantInstruction(
  TR::CodeGenerator* cg, TR::Node* node, TR::Instruction* instr)
{
  TR::S390ConstantInstructionSnippet* cis
    = new (cg->trHeapMemory()) TR::S390ConstantInstructionSnippet(cg, node, instr);
...
void OMR::Z::CodeGenerator::emitDataSnippets()
{
      ...
      if (HANDLE_CONSTANT_SNIPPET((*iterator), size)) {
        if (first) {
          ...
          self()->setBinaryBufferCursor(
            (uint8_t*)(((uintptrj_t)(self()->getBinaryBufferCursor() + size - 1) / size) * size));
        }
  ...
  TR_ASSERT(
    1 << self()->constantDataSnippetExponent() >= maxSize, "Failed to emit 1 or more snippets of size %d\n", maxSize);
...
TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate2ByteConstant(TR::Node* node, int16_t c, bool isWarm)
{
  return self()->findOrCreateConstant(node, &c, 2);
}

TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate4ByteConstant(TR::Node* node, int32_t c, bool isWarm)
{
  return self()->findOrCreateConstant(node, &c, 4);
}

TR::S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate8ByteConstant(TR::Node* node, int64_t c, bool isWarm)
{
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::dumpPreGPRegisterAssignment(TR::Instruction* instructionCursor)
{
  ...
  if (self()->hasCCInfo() && self()->hasCCCompare()) {
    ...
    if (opcd == ccInst->getOpCodeValue() && tReg == ccTgtReg && sReg == ccSrcReg
      && performTransformation(self()->comp(),
        ...
        ccInst->getOpCode().getMnemonicName(), self()->getDebug()->getName(tReg), self()->getDebug()->getName(sReg),
        ccInst)) {
      return true;
    }
  }
...
void OMR::Z::CodeGenerator::removeUnavailableRegisters(
  TR_RegisterCandidate* rc, TR::Block** blocks, TR_BitVector& availableRegisters)
{
  ...
  while (loe.hasMoreElements()) {
    ...
    if (lastTreeTopNode->getNumChildren() > 0 && lastTreeTopNode->getFirstChild()->getOpCode().isFunctionCall()
      && lastTreeTopNode->getFirstChild()
           ->getOpCode()
           .isJumpWithMultipleTargets()) // ensure calls that have multiple targets remove all volatile registers from
                                         // available registers
    {
...
void OMR::Z::CodeGenerator::genCopyFromLiteralPool(
  TR::Node* node, int32_t bytesToCopy, TR::MemoryReference* targetMR, size_t litPoolOffset, TR::InstOpCode::Mnemonic op)
{
...
bool OMR::Z::CodeGenerator::usesImplicit64BitGPRs(TR::Node* node)
{
  ...
  return node->getOpCodeValue() == TR::pd2df || node->getOpCodeValue() == TR::pd2dd
    || node->getOpCodeValue() == TR::pd2de || node->getOpCodeValue() == TR::df2pd || node->getOpCodeValue() == TR::dd2pd
...
bool OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node* node)
{
  ...
  if (node->getOpCode().isPackedRightShift()) {
    ...
    // However since this instruction will perform the 'shift' by moving the sign code to the left there are right
    // aligned dead bytes and implied left aligned zero bytes introduced. These dead and implied bytes cannot be within
    // a user variable as the actual full value must be materialized therefore require a temporary to be used for these
    // even right shifts.
    //
...
bool OMR::Z::CodeGenerator::endAccumulatorSearchOnOperation(TR::Node* node)
{
  ...
  if (self()->comp()->getOption(TR_PrivatizeOverlaps)) {
    ...
    if (node->getRegister() == NULL && endHint) {
      ...
      // common) cases where a node will initialize to a new storageReference (endHintOnOperation check) and one of the
      // conditions below is true 1) a conversion between types that use storageRefs (BCD and Aggr) and types that don't
      // (e.g. int,float) -- i2pd,i2o etc. It is always safe to end the search
      ...
      // endAccumulatorSearchOnOperation will return true to allow the pd2zd to UNPK (accumulate) right to the store
      // location "a" as the pd2zd will check and deal with any "a" and "b" overlap if any exists.
      //
...
#ifdef J9_PROJECT_SPECIFIC
  else if (node->getOpCode().isPackedLeftShift() && isOdd(node->getDecimalAdjust())) // odd left shifts need a new temp
  {
...
bool compareNodes(TR::Node* node1, TR::Node* node2, TR::CodeGenerator* cg)
{
  ...
  if (!(node1->getOpCode().isArithmetic() || node1->getOpCode().isConversion()
...
void handleLoadWithRegRanges(TR::Instruction* inst, TR::CodeGenerator* cg)
{
  ...
  TR::Register* lowReg
    = isVector ? toS390VRSInstruction(inst)->getFirstRegister() : toS390RSInstruction(inst)->getFirstRegister();
  TR::Register* highReg
    = isVector ? toS390VRSInstruction(inst)->getSecondRegister() : toS390RSInstruction(inst)->getSecondRegister();
  ...
  for (uint32_t i = lowRegNum; i != highRegNum; i = ((i == (isVector ? numVRFs - 1 : 15)) ? 0 : i + 1)) {
    ...
    // avoid needlessly spilling them. (ex. LM(GPR00F,GPR14P,PSALCCAV->LCCAEMS0); ! PSALCCAV needs a reg and only needed
    // on LM)
...
bool OMR::Z::CodeGenerator::opCodeIsNoOpOnThisPlatform(TR::ILOpCode& opCode)
{
  ...
  if ((op == TR::a2l || op == TR::l2a) && TR::Compiler->target.is64Bit() && !self()->comp()->useCompressedPointers())
    return true;
    ...
    if (node->getRegister() == NULL && node->getOpCode().canHaveStorageReferenceHint()) {
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\t\t\tpossiblyConflicting=true %s (%p) : BCDOrAggrType_Case, reg=%s, refCount %d\n",
          node->getOpCode().getName(), node,
          node->getRegister() ? self()->getDebug()->getName(node->getRegister()) : "NULL", node->getReferenceCount());
      possiblyConflicting = true;
    }
  }
#endif
...
bool OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node* node1, TR::Node* node2)
{
  ...
  if (node1->getSize() != node2->getSize()) {
    if (self()->comp()->getOption(TR_TraceVIP))
      traceMsg(self()->comp(),
        "\t\tloadOrStoreAddressesMatch = false (sizes differ) : node1 %s (%p) size = %d and node2 %s (%p) size = %d\n",
...
bool OMR::Z::CodeGenerator::reliesOnAParticularSignEncoding(TR::Node* node)
{
  ...
  // left shifts do not rely on a clean sign so the only thing to check is if a clean sign property has been propagated
  // thru it (if not then can return false)
...
// Z
bool OMR::Z::CodeGenerator::checkSimpleLoadStore(TR::Node* loadNode, TR::Node* storeNode, TR::Block* block)
{
  // The pattern below leads to use index reg in addressing, LAY for example, that costs extra for instructions like
  // MVC.
  //   aiadd
  //     load-addr
  //     laod-non-const
  ...
  if (loadNode->getOpCode().isIndirect() && loadNode->getFirstChild()->getNumChildren() > 1
    && loadNode->getFirstChild()->getOpCode().isAdd()
    && !loadNode->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
    return false;
  } else if (storeNode->getOpCode().isIndirect() && storeNode->getFirstChild()->getNumChildren() > 1
    ...
    && ((storeNode->getOpCode().hasSymbolReference() && storeNode->getSymbol()->isStatic()
          && !storeNode->getOpCode().isIndirect())
      || (loadNode->getOpCode().hasSymbolReference() && loadNode->getSymbol()->isStatic()
        && !loadNode->getOpCode().isIndirect()))) {

    return false;
  } else if (loadNode->getOpCode().isLoadConst()
    && loadNode->getReferenceCount() == 1) { // The const may be loaded into reg and shared. So dont do it.
    return true;
  } else if (loadNode->getOpCode().isLoadVar() && loadNode->getReferenceCount() == 1
...
void OMR::Z::CodeGenerator::setBranchOnCountFlag(TR::Node* node, vcount_t visitCount)
{
  ...
  // compare, and in the GLRegDeps under the compare. futureUseCount should've been initialized to be referenceCount - 1
  // (which will have to be an iRegStore in order for us to see the add node under GLRegDeps here). The compare node
...
bool OMR::Z::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress, intptrj_t sourceAddress)
{
  return !TR::Compiler->target.cpu.isTargetWithinBranchRelativeRILRange(targetAddress, sourceAddress)
