    ...
    if (node->isUnneededIALoad()) {
        if (parent->getOpCodeValue() == TR::ifacmpne || parent->getOpCodeValue() == TR::ificmpeq || parent->getOpCodeValue() == TR::ificmpne || parent->getOpCodeValue() == TR::ifacmpeq) {
            if (!parent->isNopableInlineGuard() || !self()->getSupportsVirtualGuardNOPing()) {
                ...
                if (!parent->isHCRGuard() && !parent->isOSRGuard() && !self()->comp()->performVirtualGuardNOPing() && self()->comp()->isVirtualGuardNOPingRequired(virtualGuard) && virtualGuard->canBeRemoved()) {
                    node->setUnneededIALoad(false);
                }
            }
    ...
    TR::TreeTop* tt)
{
    ...
    // Z
    if (TR::Compiler->target.cpu.isZ() && (node->getOpCode().isLoadVar() || node->getOpCode().isStore()) && node->getOpCode().isIndirect()) {
        TR::Node* add1 = NULL;
        TR::Node* add2 = NULL;
        TR::Node* sub = NULL;
        TR::Node* const1 = NULL;
...
bool OMR::Z::CodeGenerator::mulDecompositionCostIsJustified(int32_t numOfOperations, char bitPosition[], char operationType[], int64_t value)
{
...
{
    if ((node->getOpCodeValue() == TR::sadd || node->getOpCodeValue() == TR::ssub) && (node->getFirstChild()->getOpCodeValue() != TR::i2s && node->getSecondChild()->getOpCodeValue() != TR::i2s))
        return true;

    if ((node->getOpCodeValue() == TR::badd || node->getOpCodeValue() == TR::bsub) && (node->getFirstChild()->getOpCodeValue() != TR::i2b && node->getSecondChild()->getOpCodeValue() != TR::i2b))
        return true;
...
void OMR::Z::CodeGenerator::createBranchPreloadCallData(TR::LabelSymbol* callLabel, TR::SymbolReference* callSymRef, TR::Instruction* instr)
{
...
void OMR::Z::CodeGenerator::insertInstructionPrefetchesForCalls(TR_BranchPreloadCallData* data)
{
    ...
    if (bppMethod && self()->getDebug() && (strcmp(self()->getDebug()->getMethodName(data->_callSymRef), "java/lang/String.equals(Ljava/lang/Object;)Z") && strcmp(self()->getDebug()->getMethodName(data->_callSymRef), "java/util/HashMap.hash(Ljava/lang/Object;)I") && strcmp(self()->getDebug()->getMethodName(data->_callSymRef), "java/util/HashMap.addEntry(ILjava/lang/Object;Ljava/lang/Object;I)V")))
        return;
    ...
    */
    ...
    if (offset2 >= MIN_24_RELOCATION_VAL && offset2 <= MAX_24_RELOCATION_VAL && offset1 >= MIN_24_RELOCATION_VAL && offset1 <= MAX_24_RELOCATION_VAL) {
...
{
    return self()->getSupportsVirtualGuardNOPing() && self()->comp()->performVirtualGuardNOPing() && !self()->comp()->compileRelocatableCode();
}
...
void OMR::Z::CodeGenerator::addPICsListForInterfaceSnippet(TR::S390ConstantDataSnippet* ifcSnippet, TR::list<TR_OpaqueClassBlock*>* PICSlist)
{
...
bool OMR::Z::CodeGenerator::anyNonConstantSnippets()
{
    ...
    for (auto iterator = _snippetList.begin(); iterator != _snippetList.end(); ++iterator) {
        if ((*iterator)->getKind() != TR::Snippet::IsConstantData || (*iterator)->getKind() != TR::Snippet::IsWritableData || (*iterator)->getKind() != TR::Snippet::IsEyeCatcherData) {
            return true;
        }
    }
    return false;
...
{
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z196)) {
        return (!self()->comp()->getOption(TR_DisableUTF16BEEncoder) || (self()->getSupportsVectorRegisters() && !self()->comp()->getOption(TR_DisableSIMDUTF16BEEncoder)));
    }
...
void OMR::Z::CodeGenerator::doBinaryEncoding()
{
    ...
    if (self()->comp()->getJittedMethodSymbol()->isJNI() && !self()->comp()->getOption(TR_FullSpeedDebug)) {
        data.preProcInstruction = TR::Compiler->target.is64Bit() ? data.cursorInstruction->getNext()->getNext()->getNext() : data.cursorInstruction->getNext()->getNext();
    } else {
...
void OMR::Z::CodeGenerator::processUnusedNodeDuringEvaluation(TR::Node* node)
{
    ...
    if (node) {
        ...
        } else if (node->getReferenceCount() == 1 && node->getOpCode().isLoadVar() && (node->getNumChildren() == 0 || (node->getNumChildren() == 1 && node->getFirstChild()->safeToDoRecursiveDecrement()))) {
...
bool OMR::Z::CodeGenerator::useMVCLForMemcpyWithPad(TR::Node* node, TR_MemCpyPadTypes type)
{
    ...
    if (maxLen != 0 && maxLen < MVCL_THRESHOLD) // 0 is an unknown maxLength so have to be conservative and not use MVCL
    {
        ...
        if (targetLenNode->get64bitIntegralValue() < MVCL_THRESHOLD && sourceLenNode->get64bitIntegralValue() < MVCL_THRESHOLD) {
            useMVCL = true;
        }
    }
    return useMVCL;
...
void OMR::Z::CodeGenerator::apply12BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol* label, bool isCheckDisp)
{
    ...
    if (s != NULL && (s->getKind() == TR::Snippet::IsConstantData || s->getKind() == TR::Snippet::IsWritableData || s->getKind() == TR::Snippet::IsConstantInstruction)) {
 ...
 */
void OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol* label, int8_t addressDifferenceDivisor, bool isInstrOffset)
{
    if (isInstrOffset) {
        uint16_t* label_cursor = (uint16_t*)label->getCodeLocation();
        if ((*(uint16_t*)label_cursor) == bos(0x1800))
...
OMR::Z::CodeGenerator::findOrCreateConstant(TR::Node* node, void* c, uint16_t size)
{
    ...
    if (self()->profiledPointersRequireRelocation() && node && (node->getOpCodeValue() == TR::aconst && (node->isClassPointerConstant() || node->isMethodPointerConstant()) || node->getOpCodeValue() == TR::loadaddr && node->getSymbol()->isClassObject() || isLiteralPoolAddress))
        key.node = node;
    else
        key.node = NULL;
 ...
 */
void OMR::Z::CodeGenerator::dumpPostGPRegisterAssignment(TR::Instruction* instructionCursor, TR::Instruction* origNextInstruction)
{
    if (self()->comp()->getOutFile() == NULL) {
        return;
    }
...
OMR::Z::CodeGenerator::evaluateLengthMinusOneForMemoryOps(TR::Node* node, bool clobberEvaluate, bool& lenMinusOne)
{
    ...
    if ((node->getReferenceCount() == 1) && self()->supportsLengthMinusOneForMemoryOpts() && ((node->getOpCodeValue() == TR::iadd && node->getSecondChild()->getOpCodeValue() == TR::iconst && node->getSecondChild()->getInt() == 1) || (node->getOpCodeValue() == TR::isub && node->getSecondChild()->getOpCodeValue() == TR::iconst && node->getSecondChild()->getInt() == -1)) && (node->getRegister() == NULL)) {
        if (clobberEvaluate)
...
bool OMR::Z::CodeGenerator::isActiveCompareCC(TR::InstOpCode::Mnemonic opcd, TR::Register* tReg, TR::Register* sReg)
{
    if (self()->hasCCInfo() && self()->hasCCCompare()) {
        ...
        if (opcd == ccInst->getOpCodeValue() && tReg == ccTgtReg && sReg == ccSrcReg && performTransformation(self()->comp(), "O^O isActiveCompareCC: RR Compare Op [%s\t %s, %s] can reuse CC from ccInstr [%p]\n", ccInst->getOpCode().getMnemonicName(), self()->getDebug()->getName(tReg), self()->getDebug()->getName(sReg), ccInst)) {
            return true;
        }
    }
    return false;
...
{
    if (self()->hasCCInfo() && self()->hasCCSigned() && !self()->hasCCOverflow() && !self()->hasCCCarry() && performTransformation(self()->comp(), "O^O Arithmetic CC: CCInfo(%d) CCSigned(%d) CCOverflow(%d) CCCarry(%d)\n", self()->hasCCInfo(), self()->hasCCSigned(), self()->hasCCOverflow(), self()->hasCCCarry())) {
...
bool OMR::Z::CodeGenerator::isActiveLogicalCC(TR::Node* ccNode, TR::Register* tstReg)
{
    ...
    if (self()->hasCCInfo() && self()->hasCCZero() && !self()->hasCCOverflow() && (TR::ILOpCode::isEqualCmp(op) || TR::ILOpCode::isNotEqualCmp(op)) && performTransformation(self()->comp(), "O^O Logical CC Info: CCInfo(%d) CCZero(%d) CCOverflow(%d)\n", self()->hasCCInfo(), self()->hasCCZero(), self()->hasCCOverflow())) {
        TR::Instruction* inst = self()->ccInstruction();
        TR::Register* tgtReg = inst->tgtRegArrElem(0);
 ...
 */
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(TR::Node* node, TR::Register* targetRegister, uint32_t numberOfBits)
{
 ...
 * This function sign extended the specified number of high order bits in the register.
 */
bool OMR::Z::CodeGenerator::signExtendedHighOrderBits(TR::Node* node, TR::Register* targetRegister, TR::Register* srcRegister, uint32_t numberOfBits)
{
    switch (numberOfBits) {
    case 16:
 ...
 * This function zero filled the specified number of high order bits in the register.
 */
bool OMR::Z::CodeGenerator::clearHighOrderBits(TR::Node* node, TR::Register* targetRegister, TR::Register* srcRegister, uint32_t numberOfBits)
{
    switch (numberOfBits) {
    case 8:
...
void OMR::Z::CodeGenerator::setUnavailableRegistersUsage(TR_Array<TR_BitVector>& liveOnEntryUsage, TR_Array<TR_BitVector>& liveOnExitUsage)
{
...
void OMR::Z::CodeGenerator::removeUnavailableRegisters(TR_RegisterCandidate* rc, TR::Block** blocks, TR_BitVector& availableRegisters)
{
...
void OMR::Z::CodeGenerator::genMemCpy(TR::MemoryReference* targetMR, TR::Node* targetNode, TR::MemoryReference* sourceMR, TR::Node* sourceNode, int64_t copySize)
{
 ...
 */
void OMR::Z::CodeGenerator::genCopyFromLiteralPool(TR::Node* node, int32_t bytesToCopy, TR::MemoryReference* targetMR, size_t litPoolOffset, TR::InstOpCode::Mnemonic op)
{
...
bool OMR::Z::CodeGenerator::usesImplicit64BitGPRs(TR::Node* node)
{
    ...
    return node->getOpCodeValue() == TR::pd2df || node->getOpCodeValue() == TR::pd2dd || node->getOpCodeValue() == TR::pd2de || node->getOpCodeValue() == TR::df2pd || node->getOpCodeValue() == TR::dd2pd || node->getOpCodeValue() == TR::de2pd;
#else
    return false;
#endif
}
...
bool OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node* node)
{
    ...
    if (node->getOpCode().isPackedRightShift()) {
        ...
        return true;
    } else if (node->getOpCode().isConversion() && node->getType().isAnyZoned() && node->getFirstChild()->getType().isZonedSeparateSign()) {
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\tnodeRequiresATemporary = true for zonedSepToZoned %s (%p)\n", node->getOpCode().getName(), node);
        return true;
    } else
#endif
        if (TR::MemoryReference::typeNeedsAlignment(node) && node->getType().isAggregate() && node->getOpCode().isRightShift()) {
        if (self()->traceBCDCodeGen())
...
bool OMR::Z::CodeGenerator::endAccumulatorSearchOnOperation(TR::Node* node)
{
    ...
    if (self()->comp()->getOption(TR_PrivatizeOverlaps)) {
        ...
        if (node->getRegister() == NULL && endHint) {
            ...
            if (node->getOpCode().isConversion() && (self()->isStorageReferenceType(node) && !self()->isStorageReferenceType(node->getFirstChild())) || // i2pd
...
bool OMR::Z::CodeGenerator::endHintOnOperation(TR::Node* node)
{
    ...
    if (node->getOpCode().isConversion() && node->getOpCode().isBinaryCodedDecimalOp() && !TR::ILOpCode::isZonedToZonedConversion(node->getOpCodeValue()) && node->getType().isBCD()) {
        return true;
    } else
#endif
...
void handleLoadWithRegRanges(TR::Instruction* inst, TR::CodeGenerator* cg)
{
    ...
    if (!(opCodeToUse == TR::InstOpCode::LM || opCodeToUse == TR::InstOpCode::LMG || opCodeToUse == TR::InstOpCode::LMH || opCodeToUse == TR::InstOpCode::LMY || opCodeToUse == TR::InstOpCode::VLM))
        return;
...
bool OMR::Z::CodeGenerator::foundConflictingNode(TR::Node* node, TR::list<TR::Node*>* conflictingAddressNodes)
{
    ...
    if (!conflictingAddressNodes->empty() && self()->possiblyConflictingNode(node) && (std::find(conflictingAddressNodes->begin(), conflictingAddressNodes->end(), node) != conflictingAddressNodes->end())) {
...
// Z
void OMR::Z::CodeGenerator::collectConflictingAddressNodes(TR::Node* parent, TR::Node* node, TR::list<TR::Node*>* conflictingAddressNodes)
{
    if (self()->traceBCDCodeGen())
...
bool OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node* node1, TR::Node* node2)
{
    ...
    {
        if (node1->getOpCode().isIndirect() && node2->getOpCode().isIndirect() && node1->getSymbolReference()->getOffset() == node2->getSymbolReference()->getOffset()) {
            TR::Node* addr1 = node1->getFirstChild();
            TR::Node* addr2 = node2->getFirstChild();

            foundMatch = self()->addressesMatch(addr1, addr2);
        } else if (!node1->getOpCode().isIndirect() && !node2->getOpCode().isIndirect() && node1->getSymbolReference() == node2->getSymbolReference()) {
            foundMatch = true;
        }
    }
    if (self()->comp()->getOption(TR_TraceVIP))
...
bool OMR::Z::CodeGenerator::checkIfcmpxx(TR::Node* node)
{
    ...
    bool firstChildOK = (firstChildNode->getOpCode().isLoadVar() || firstChildNode->getOpCode().isLoadConst()) && firstChildNode->getType().isIntegral() && firstChildNode->getReferenceCount() == 1;

    bool secondChildOK = firstChildOK && (secondChildNode->getOpCode().isLoadVar() || secondChildNode->getOpCode().isLoadConst()) && secondChildNode->getType().isIntegral() && secondChildNode->getReferenceCount() == 1;
...
bool OMR::Z::CodeGenerator::checkSimpleLoadStore(TR::Node* loadNode, TR::Node* storeNode, TR::Block* block)
{
    ...
    if (loadNode->getOpCode().isIndirect() && loadNode->getFirstChild()->getNumChildren() > 1 && loadNode->getFirstChild()->getOpCode().isAdd() && !loadNode->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        return false;
    } else if (storeNode->getOpCode().isIndirect() && storeNode->getFirstChild()->getNumChildren() > 1 && storeNode->getFirstChild()->getOpCode().isAdd() && !storeNode->getFirstChild()->getSecondChild()->getOpCode().isLoadConst()) {
        return false;
    } else if (loadNode->getSize() != storeNode->getSize()) {
        return false;
    } else if (!self()->getAccessStaticsIndirectly() && ((storeNode->getOpCode().hasSymbolReference() && storeNode->getSymbol()->isStatic() && !storeNode->getOpCode().isIndirect()) || (loadNode->getOpCode().hasSymbolReference() && loadNode->getSymbol()->isStatic() && !loadNode->getOpCode().isIndirect()))) {

        return false;
    } else if (loadNode->getOpCode().isLoadConst() && loadNode->getReferenceCount() == 1) { //The const may be loaded into reg and shared. So dont do it.
        return true;
    } else if (loadNode->getOpCode().isLoadVar() && loadNode->getReferenceCount() == 1 && loadNode->getSymbolReference()) {
...
void OMR::Z::CodeGenerator::setBranchOnCountFlag(TR::Node* node, vcount_t visitCount)
{
    ...
    for (int32_t i = 0; i < regDeps->getNumChildren(); i++) {
        ...
        if (regLoad->getGlobalRegisterNumber() == passThrough->getGlobalRegisterNumber() && passThrough->getNumChildren() >= 1 && passThrough->getFirstChild() == addNode) {
            found = true;
            break;
        }
    }
...
{
    return (TR::Compiler->om.generateCompressedObjectHeaders() && node->getOpCode().hasSymbolReference() && (node->getSymbol()->isClassObject() || node->getSymbolReference() == self()->getSymRefTab()->findVftSymbolRef()));
}
...
{
    return !TR::Compiler->target.cpu.isTargetWithinBranchRelativeRILRange(targetAddress, sourceAddress) || self()->comp()->getOption(TR_StressTrampolines);
}
