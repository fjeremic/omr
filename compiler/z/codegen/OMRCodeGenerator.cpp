...
bool
compareNodes(TR::Node* node1, TR::Node* node2, TR::CodeGenerator* cg);
bool
isMatchingStoreRestore(TR::Instruction* cursorLoad,
                       TR::Instruction* cursorStore,
                       TR::CodeGenerator* cg);
void
handleLoadWithRegRanges(TR::Instruction* inst, TR::CodeGenerator* cg);
...
void
OMR::Z::CodeGenerator::lowerTreesWalk(TR::Node* parent,
                                      TR::TreeTop* treeTop,
                                      vcount_t visitCount)
{
...
void
OMR::Z::CodeGenerator::checkIsUnneededIALoad(TR::Node* parent,
                                             TR::Node* node,
                                             TR::TreeTop* tt)
{
...
void
OMR::Z::CodeGenerator::lowerTreeIfNeeded(TR::Node* node,
  ...
  if (comp->getOptions()->getRegisterAssignmentTraceOption(
    ...
    self()->setGPRegisterIterator(
      new (self()->trHeapMemory())
        TR::RegisterIterator(self()->machine(),
                             TR::RealRegister::FirstGPR,
                             TR::RealRegister::LastAssignableGPR));
    self()->setFPRegisterIterator(new (
      self()->trHeapMemory()) TR::RegisterIterator(self()->machine(),
                                                   TR::RealRegister::FirstFPR,
                                                   TR::RealRegister::LastFPR));
    self()->setVRFRegisterIterator(new (
      self()->trHeapMemory()) TR::RegisterIterator(self()->machine(),
                                                   TR::RealRegister::FirstVRF,
    ...
    if (enableVectorGRA) {
      ...
      if (traceVecGRN) {
        printf("Java func: %s func: %s\n",
               self()->comp()->getCurrentMethod()->nameChars(),
               __FUNCTION__);
        printf("ff  %d\t",
               self()->machine()->getFirstGlobalFPRRegisterNumber());
...
bool
OMR::Z::CodeGenerator::mulDecompositionCostIsJustified(int32_t numOfOperations,
                                                       char bitPosition[],
                                                       char operationType[],
                                                       int64_t value)
{
  bool trace =
    self()->comp()->getOptions()->getTraceSimplifier(TR_TraceMulDecomposition);
 ...
 */
bool
OMR::Z::CodeGenerator::considerAggregateSizeForGRA(int32_t size)
{
  switch (size) {
    case 1:
    case 2:
...
TR::Instruction*
OMR::Z::CodeGenerator::generateNop(TR::Node* n,
...
TR::Instruction*
OMR::Z::CodeGenerator::insertPad(TR::Node* node,
  ...
  if (self()->comp()->getJittedMethodSymbol()->getLinkageConvention() ==
    ...
    if (self()->comp()->getJittedMethodSymbol()->isJNI() &&
      ...
      intptrj_t jniMethodTargetAddress =
        (intptrj_t)methodSymbol->getResolvedMethod()->startAddressForJNIMethod(
          self()->comp());

      cursor = new (self()->trHeapMemory())
        TR::S390ImmInstruction(TR::InstOpCode::DC,
                               startNode,
                               UPPER_4_BYTES(jniMethodTargetAddress),
                               cursor,
                               self());
      ...
      if (TR::Compiler->target.is64Bit()) {
        cursor = new (self()->trHeapMemory())
          TR::S390ImmInstruction(TR::InstOpCode::DC,
                                 startNode,
                                 LOWER_4_BYTES(jniMethodTargetAddress),
                                 cursor,
                                 self());
      }
    }
...
void
OMR::Z::CodeGenerator::createBranchPreloadCallData(
  TR::LabelSymbol* callLabel,
  TR::SymbolReference* callSymRef,
  TR::Instruction* instr)
{
  TR_BranchPreloadCallData* data = new (self()->trHeapMemory())
    TR_BranchPreloadCallData(instr,
                             callLabel,
                             self()->comp()->getCurrentBlock(),
                             callSymRef,
...
OMR::Z::CodeGenerator::insertInstructionPrefetches()
{
  if (self()->supportsBranchPreload()) {
    if (_hottestReturn._frequency > 6) {
      ...
      cursor = generateRXInstruction(self(),
                                     TR::InstOpCode::getExtendedLoadOpCode(),
                                     node,
                                     tempReg,
                                     tempMR,
                                     cursor);
  ...
  if (self()->supportsBranchPreloadForCalls()) {
    ...
    for (auto iterator = self()->getCallsForPreloadList()->begin();
         iterator != self()->getCallsForPreloadList()->end();
         ++iterator)
...
void
OMR::Z::CodeGenerator::insertInstructionPrefetchesForCalls(
  ...
  if (bppMaxPath) {
    ...
    for (auto e = block->getPredecessors().begin();
      ...
      if (bppCall && (!canReachWithBPRP || bppOnly)) {
        ...
        cursor = new (self()->trHeapMemory())
          TR::S390RILInstruction(TR::InstOpCode::LARL,
                                 node,
                                 tempReg,
        ...
        tempMR = generateS390MemoryReference(tempReg, 0, self());
        cursor =
          generateS390BranchPredictionPreloadInstruction(self(),
                                                         TR::InstOpCode::BPP,
                                                         node,
                                                         data->_callLabel,
                                                         (int8_t)0xD,
...
skipInternalControlFlow(TR::Instruction* insertInstr)
{
 ...
 */
bool
OMR::Z::CodeGenerator::shouldValueBeInACommonedNode(int64_t value)
{
...
void
OMR::Z::CodeGenerator::recordRegisterAssignment(TR::Register* assignedReg,
...
OMR::Z::CodeGenerator::doRegisterAssignment(TR_RegisterKinds kindsToAssign)
{
              ...
              if (instructionCursor->getNode()
                ...
                if ((instructionCursor->getNode()
                  ...
                  if (instructionCursor->getNode()
                    ...
                    TR_ASSERT(
                      1,
                      "ASSERTION - Branch found outside Internal Control Flow "
                      "that should be located inside Internal Control Flow\n");
                  }
                }
              }
            }
  ...
  if (!self()->isOutOfLineColdPath()) {
    for (uint32_t i = TR::RealRegister::FirstGPR;
         i <= TR::RealRegister::LastGPR;
         ++i) {
      TR_ASSERT(
        self()->machine()->getRealRegister(i)->getState() !=
          TR::RealRegister::Assigned,
        "ASSERTION: GPR registers are still assigned at end of first RA pass! "
        "%s regNum=%s\n",
        self()->comp()->signature(),
...
OMR::Z::CodeGenerator::RemoveMemRefFromStack(TR::MemoryReference* mr)
{
  if (_stackOfMemoryReferencesCreatedDuringEvaluation.contains(mr)) {
    for (size_t i = 0;
         i < _stackOfMemoryReferencesCreatedDuringEvaluation.size();
         i++) {
...
void
OMR::Z::CodeGenerator::StopUsingEscapedMemRefsRegisters(
...
bool
OMR::Z::CodeGenerator::supportsNonHelper(
...
void
OMR::Z::CodeGenerator::addPICsListForInterfaceSnippet(
  ...
  TR::list<TR_OpaqueClassBlock*>* PICSlist)
{
  ...
  _interfaceSnippetToPICsListHashTab->add(
    (void*)ifcSnippet, hashIndex, (void*)PICSlist);
}
...
OMR::Z::CodeGenerator::anyLitPoolSnippets()
{
  ...
  for (auto constantiterator = _constantList.begin();
       constantiterator != _constantList.end();
       ++constantiterator) {
  ...
  for (auto writeableiterator = _writableList.begin();
       writeableiterator != _writableList.end();
       ++writeableiterator) {
...
OMR::Z::CodeGenerator::doBinaryEncoding()
{
  ...
  _methodBegin = new (self()->trHeapMemory())
    TR::S390LabelInstruction(TR::InstOpCode::LABEL,
                             self()->comp()->getStartTree()->getNode(),
                             generateLabelSymbol(self()),
                             static_cast<TR::Instruction*>(NULL),
                             self());

  _methodEnd =
    generateS390LabelInstruction(self(),
                                 TR::InstOpCode::LABEL,
                                 self()->comp()->findLastTree()->getNode(),
                                 generateLabelSymbol(self()));
            ...
            if (s390Inst->getKind() == TR::Instruction::IsLabel) {
              if (self()->comp()->getOption(TR_TraceLabelTargetNOPs))
                traceMsg(
                  ...
                  s390Inst,
                  s390Inst->getOpCode().getMnemonicName());
  ...
  if (self()->comp()->getOption(TR_TraceCG))
    self()->comp()->getDebug()->dumpMethodInstrs(
      ...
      "Post Prologue/epilogue Instructions",
      false);
  ...
  uint8_t* temp =
    self()->allocateCodeMemory(self()->getEstimatedCodeLength(), 0, &coldCode);
  ...
  while (data.cursorInstruction) {
    ...
    if (isPrivateLinkage && data.cursorInstruction == data.jitTojitStart) {
      ...
      uint32_t magicWord = (argSize << 16) | static_cast<uint32_t>(
                                               self()->comp()->getReturnInfo());
  ...
  if (node) {
    ...
    if (node->getOpaquePseudoRegister()) {
      ...
      if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
                 "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload "
                 "%s (%p) reg %s - handle extra ref to addr child (ref is node "
                 "based %s - %s %p)\n",
    ...
    if (node->safeToDoRecursiveDecrement()) {
      ...
      if (node->getOpaquePseudoRegister() &&
        ...
        if (self()->traceBCDCodeGen())
          traceMsg(self()->comp(),
                   "\tprocessUnusedNodeDuringEvaluation : bcd/aggr "
                   "const/ixload %s (%p) reg %s - handle extra ref to addr "
...
OMR::Z::CodeGenerator::getLinkageGlobalRegisterNumber(
...
inline bool
isBetween(int64_t value, int32_t low, int32_t high)
{
  return low <= value && value < high;
}

static int64_t
getValueIfIntegralConstant(TR::Node* node, TR::Compilation* comp)
{
...
static bool
canProbablyFoldSecondChildIfConstant(TR::Node* node)
{
...
void
OMR::Z::CodeGenerator::simulateNodeEvaluation(
  TR::Node* node,
  ...
  TR_RegisterPressureSummary* summary)
{
  ...
  if (isBetween(child2Value, -1 << 15, 1 << 15)) {
    ...
    self()->simulateSkippedTreeEvaluation(
      node->getSecondChild(), state, summary, 'i');
...
bool
OMR::Z::CodeGenerator::allowGlobalRegisterAcrossBranch(TR_RegisterCandidate* rc,
                                                       TR::Node* branchNode)
{
...
void
OMR::Z::CodeGenerator::setRealRegisterAssociation(
  TR::Register* reg,
...
bool
OMR::Z::CodeGenerator::isGlobalRegisterAvailable(TR_GlobalRegisterNumber i,
                                                 TR::DataType dt)
{
 ...
 */
TR::Register*
OMR::Z::CodeGenerator::gprClobberEvaluate(TR::Node* node,
                                          bool force_copy,
                                          bool ignoreRefCount)
{
  TR::Instruction* cursor = NULL;
  ...
  if (!self()->canClobberNodesRegister(node, 1, &data, ignoreRefCount) ||
    ...
    if (node->getOpCode().isFloat()) {
      ...
      cursor = generateRRInstruction(
        self(), TR::InstOpCode::LER, node, targetRegister, srcRegister);
      if (debugObj) {
      ...
      TR::Register* targetRegister = self()->allocateSinglePrecisionRegister();
      cursor = generateRRInstruction(
        self(), TR::InstOpCode::LDR, node, targetRegister, srcRegister);
      if (debugObj) {
        debugObj->addInstructionComment(toS390RRInstruction(cursor),
                                        CLOBBER_EVAL);
      }
      ...
      cursor = generateRRInstruction(
        self(), TR::InstOpCode::LXR, node, targetRegister, srcRegister);
      if (debugObj) {
        debugObj->addInstructionComment(toS390RRInstruction(cursor),
                                        CLOBBER_EVAL);
      }
      ...
      generateVRRaInstruction(
        self(), TR::InstOpCode::VLR, node, targetRegister, srcRegister);
      return targetRegister;
    } else {
      TR::Register* targetRegister =
        self()->allocateClobberableRegister(srcRegister);
      ...
      cursor = generateRRInstruction(
        self(), loadRegOpCode, node, targetRegister, srcRegister);
...
OMR::Z::CodeGenerator::ssrClobberEvaluate(
  TR::Node* node,
  TR::MemoryReference* sourceMR) // sourceMR can be NULL
{
  ...
  if (node->getReferenceCount() > 1) {
    ...
    TR_ASSERT(
      false, "needsClobberDueToReuse should not be true for node %p\n", node);
  } else {
    if (self()->traceBCDCodeGen()) {
      traceMsg(
        self()->comp(),
  ...
  if (needsClobberDueToRefCount || needsClobberDueToRegCount ||
    ...
    TR_StorageReference* copyStorageReference =
      TR_StorageReference::createTemporaryBasedStorageReference(byteLength,
                                                                self()->comp());
    ...
    if (isBCD)
      ...
      copyMR = generateS390MemRefFromStorageRef(
        node, copyStorageReference, cg); // enforceSSLimits=true
    ...
    int32_t digitsInSrc =
      isBCD
        ? TR::DataType::getBCDPrecisionFromSize(node->getDataType(), byteLength)
        : 0;
    ...
    if (self()->traceBCDCodeGen() && alreadyClearedDigits) {
      ...
      traceMsg(
        self()->comp(),
        "\t\t* setting surviving leftAlignedZeroDigits %d on srcRegister %s "
        "with new copy ref #%d (digitsInSrc - digitsToClear = %d - %d = %d)\n",
        alreadyClearedDigits,
        self()->getDebug()->getName(srcRegister),
        copyStorageReference->getReferenceNumber(),
        digitsInSrc,
        digitsToClear,
        ...
        if (listReg != srcRegister) {
          if (listReg->getStorageReference() != srcStorageReference) {
            ...
            if (self()->traceBCDCodeGen())
              traceMsg(
                self()->comp(),
                "\ty^y : update reg %s with ref #%d (%s) on node %s (%p) "
                "refCount %d from _nodesToUpdateOnClobber with copy ref #%d "
                "(%s) (skip if refCount == 0)\n",
                self()->getDebug()->getName(listReg),
                listReg->getStorageReference()->getReferenceNumber(),
                self()->getDebug()->getName(
                  listReg->getStorageReference()->getSymbol()),
...
bool
OMR::Z::CodeGenerator::useMVCLForMemcpyWithPad(TR::Node* node,
...
bool
OMR::Z::CodeGenerator::isValidCompareConst(int64_t compareConst)
{
...
bool
OMR::Z::CodeGenerator::isIfFoldable(TR::Node* node, int64_t compareConst)
{
...
////////////////////////////////////////////////////////////////////////////////
void
OMR::Z::CodeGenerator::apply12BitLabelRelativeRelocation(int32_t* cursor,
 ...
 */
void
OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(
  int32_t* cursor,
  TR::LabelSymbol* label,
...
void
OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(int32_t* cursor,
                                                         TR::LabelSymbol* label)
{
...
////////////////////////////////////////////////////////////////////////////////
void
OMR::Z::CodeGenerator::apply32BitLabelRelativeRelocation(int32_t* cursor,
                                                         TR::LabelSymbol* label)
{
...
////////////////////////////////////////////////////////////////////////////////
void
OMR::Z::CodeGenerator::apply32BitLabelTableRelocation(int32_t* cursor,
                                                      TR::LabelSymbol* label)
{
...
OMR::Z::CodeGenerator::findOrCreateConstant(TR::Node* node,
                                            void* c,
                                            uint16_t size)
{
  CS2::HashIndex hi;
...
TR::S390ConstantDataSnippet*
OMR::Z::CodeGenerator::CreateConstant(TR::Node* node,
                                      void* c,
                                      uint16_t size,
                                      bool writable)
{
...
void
OMR::Z::CodeGenerator::addDataConstantSnippet(
...
int32_t
OMR::Z::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
  int32_t estimatedSnippetStart)
{
  ...
  for (auto snippetDataIterator = _snippetDataList.begin();
       snippetDataIterator != _snippetDataList.end();
       ++snippetDataIterator) {
    (*snippetDataIterator)
      ->getSnippetLabel()
      ->setEstimatedCodeLocation(estimatedSnippetStart);
...
TR::S390ConstantDataSnippet*
OMR::Z::CodeGenerator::findOrCreate2ByteConstant(TR::Node* node,
                                                 int16_t c,
                                                 bool isWarm)
{
...
TR::S390ConstantDataSnippet*
OMR::Z::CodeGenerator::findOrCreate4ByteConstant(TR::Node* node,
                                                 int32_t c,
                                                 bool isWarm)
{
...
TR::S390ConstantDataSnippet*
OMR::Z::CodeGenerator::findOrCreate8ByteConstant(TR::Node* node,
                                                 int64_t c,
                                                 bool isWarm)
{
...
TR::S390ConstantDataSnippet*
OMR::Z::CodeGenerator::Create4ByteConstant(TR::Node* node,
                                           int32_t c,
                                           bool writable)
{
...
TR::S390ConstantDataSnippet*
OMR::Z::CodeGenerator::Create8ByteConstant(TR::Node* node,
                                           int64_t c,
                                           bool writable)
{
...
{
  if (TR::Compiler->target.is64Bit()) {
    return (TR::S390WritableDataSnippet*)self()->Create8ByteConstant(
      node, 0, true);
  } else {
    return (TR::S390WritableDataSnippet*)self()->Create4ByteConstant(
      node, 0, true);
  }
}
...
////////////////////////////////////////////////////////////////////////////////
void
OMR::Z::CodeGenerator::dumpPreGPRegisterAssignment(
  TR::Instruction* instructionCursor)
{
  ...
  if (instructionCursor->totalReferencedGPRegisters() > 0) {
    ...
    trfprintf(comp()->getOutFile(),
              "\n<< Pre-GPR assignment for instruction: %p",
              instructionCursor);
 ...
 */
void
OMR::Z::CodeGenerator::dumpPostGPRegisterAssignment(
  TR::Instruction* instructionCursor,
  TR::Instruction* origNextInstruction)
{
  if (self()->comp()->getOutFile() == NULL) {
...
TR::Register*
OMR::Z::CodeGenerator::evaluateLengthMinusOneForMemoryOps(TR::Node* node,
...
bool
OMR::Z::CodeGenerator::bitwiseOpNeedsLiteralFromPool(TR::Node* parent,
                                                     TR::Node* child)
{
 ...
 */
bool
OMR::Z::CodeGenerator::isActiveCompareCC(TR::InstOpCode::Mnemonic opcd,
                                         TR::Register* tReg,
                                         TR::Register* sReg)
{
 ...
 */
bool
OMR::Z::CodeGenerator::isActiveArithmeticCC(TR::Register* tstReg)
{
...
bool
OMR::Z::CodeGenerator::isActiveLogicalCC(TR::Node* ccNode, TR::Register* tstReg)
{
 ...
 */
bool
OMR::Z::CodeGenerator::signExtendedHighOrderBits(TR::Node* node,
                                                 TR::Register* targetRegister,
                                                 uint32_t numberOfBits)
{
  return self()->signExtendedHighOrderBits(
    node, targetRegister, targetRegister, numberOfBits);
}
                                                 ...
                                                 uint32_t numberOfBits)
{
  switch (numberOfBits) {
    case 16:
      generateRRInstruction(
        self(), TR::InstOpCode::LHR, node, targetRegister, targetRegister);
      break;
    ...
    case 24:
      ...
      // target register
      generateRRInstruction(
        self(), TR::InstOpCode::LBR, node, targetRegister, targetRegister);
      break;
    ...
    case 48:
      ...
      // target register
      generateRRInstruction(
        self(), TR::InstOpCode::LGHR, node, targetRegister, srcRegister);
      break;
    ...
    case 56:
      ...
      // target register
      generateRRInstruction(
        self(), TR::InstOpCode::LGBR, node, targetRegister, srcRegister);
      break;
 ...
 */
bool
OMR::Z::CodeGenerator::clearHighOrderBits(TR::Node* node,
                                          TR::Register* targetRegister,
                                          uint32_t numberOfBits)
{
  return self()->clearHighOrderBits(
    node, targetRegister, targetRegister, numberOfBits);
}
  ...
  switch (numberOfBits) {
    case 8:
      ...
      generateRIInstruction(
        self(), TR::InstOpCode::NILH, node, targetRegister, (int16_t)0x00FF);
      break;

    case 16:
      // Can use AND immediate to clear high 16 bits
      generateRIInstruction(
        self(), TR::InstOpCode::NILH, node, targetRegister, (int16_t)0x0000);
      break;

    case 24:
      // Can use AND immediate to clear high 24 bits
      generateRILInstruction(
        self(), TR::InstOpCode::NILF, node, targetRegister, 0x000000FF);
      break;
    ...
    case 48:
      ...
      // register.
      generateRRInstruction(
        self(), TR::InstOpCode::LLGHR, node, targetRegister, srcRegister);
      break;
    ...
    case 56:
      ...
      // register.
      generateRRInstruction(
        self(), TR::InstOpCode::LLGCR, node, targetRegister, srcRegister);
      break;
...
bool
OMR::Z::CodeGenerator::isILOpCodeSupported(TR::ILOpCodes o)
{
...
bool
OMR::Z::CodeGenerator::needs64bitPrecision(TR::Node* node)
{
  if (TR::Compiler->target.is64Bit())
    return true;
  else
...
void
OMR::Z::CodeGenerator::setUnavailableRegistersUsage(
...
void
OMR::Z::CodeGenerator::resetGlobalRegister(TR::RealRegister::RegNum reg,
...
void
OMR::Z::CodeGenerator::setGlobalRegister(TR::RealRegister::RegNum reg,
                                         TR_BitVector& globalRegisters)
{
  int32_t globalRegNum = self()->machine()->getGlobalReg(reg);
  if (globalRegNum != -1)
...
void
OMR::Z::CodeGenerator::setRegister(TR::RealRegister::RegNum reg,
...
void
OMR::Z::CodeGenerator::setUnavailableRegisters(
  TR::Block* b,
    ...
    if (lastTreeTopNode->getNumChildren() > 0 &&
        ...
        lastTreeTopNode->getFirstChild()
          ...
          .isJumpWithMultipleTargets()) // ensure calls that have multiple
                                        // targets remove all volatile registers
...
void
OMR::Z::CodeGenerator::genMemCpy(TR::MemoryReference* targetMR,
                                 TR::Node* targetNode,
...
void
OMR::Z::CodeGenerator::genMemClear(TR::MemoryReference* targetMR,
                                   TR::Node* targetNode,
                                   int64_t clearSize)
{
 ...
 */
void
OMR::Z::CodeGenerator::genCopyFromLiteralPool(TR::Node* node,
                                              int32_t bytesToCopy,
...
TR::Instruction*
OMR::Z::CodeGenerator::genLoadAddressToRegister(TR::Register* reg,
                                                ...
                                                TR::Instruction* preced)
{
  ...
  if (offset >= TR_MIN_RX_DISP && offset <= TR_MAX_RX_DISP) {
    preced =
      generateRXInstruction(cg, TR::InstOpCode::LA, node, reg, origMR, preced);
  } else if (offset > MINLONGDISP && offset < MAXLONGDISP) {
    preced =
      generateRXInstruction(cg, TR::InstOpCode::LAY, node, reg, origMR, preced);
  } else {
    TR::MemoryReference* tempMR = generateS390MemoryReference(*origMR, 0, cg);
    tempMR->setOffset(0);
    preced =
      generateRXInstruction(cg, TR::InstOpCode::LA, node, reg, tempMR, preced);
    preced = generateS390ImmOp(cg,
                               TR::InstOpCode::getAddOpCode(),
                               node,
                               reg,
                               reg,
                               (int64_t)offset,
...
bool
OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
...
bool
OMR::Z::CodeGenerator::isStorageReferenceType(TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
  if (node->getType().isBCD())
    return true;
...
bool
OMR::Z::CodeGenerator::endAccumulatorSearchOnOperation(TR::Node* node)
{
    ...
    if (node->getRegister() == NULL && endHint) {
      ...
      if (node->getOpCode().isConversion() &&
            (self()->isStorageReferenceType(node) &&
             !self()->isStorageReferenceType(node->getFirstChild())) || // i2pd
          (!self()->isStorageReferenceType(node) &&
...
bool
OMR::Z::CodeGenerator::endHintOnOperation(TR::Node* node)
{
#ifdef J9_PROJECT_SPECIFIC
...
bool
OMR::Z::CodeGenerator::nodeMayCauseException(TR::Node* node)
{
...
void
OMR::Z::CodeGenerator::setUsesZeroBasePtr(bool v)
{
...
bool
OMR::Z::CodeGenerator::IsInMemoryType(TR::DataType type)
{
#ifdef J9_PROJECT_SPECIFIC
  return type.isBCD();
#else
...
uint32_t
getRegMaskFromRange(TR::Instruction* inst);
 ...
 */
TR::Instruction*
OMR::Z::CodeGenerator::generateDebugCounterBump(
  TR::Instruction* cursor,
...
TR::Instruction*
OMR::Z::CodeGenerator::generateDebugCounterBump(TR::Instruction* cursor,
                                                TR::DebugCounterBase* counter,
                                                int32_t delta,
 ...
 */
bool
OMR::Z::CodeGenerator::isDispInRange(int64_t disp)
{
  return (MINLONGDISP <= disp) && (disp <= MAXLONGDISP);
}

bool
OMR::Z::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode,
                                                    TR::DataType dt)
{
...
bool
OMR::Z::CodeGenerator::directLoadAddressMatch(TR::Node* load1,
                                              TR::Node* load2,
                                              bool trace)
{
...
OMR::Z::CodeGenerator::isOutOf32BitPositiveRange(int64_t value, bool trace)
{
  if (value < 0 || value > INT_MAX) {
    if (trace)
      traceMsg(
        self()->comp(),
        "\tisOutOf32BitPositiveRange = true : value %lld < 0 (or > INT_MAX)\n",
        value);
    return true;
  }
  return false;
...
int32_t
OMR::Z::CodeGenerator::getMaskSize(int32_t leftMostNibble, int32_t nibbleCount)
{
...
// Z
bool
OMR::Z::CodeGenerator::foundConflictingNode(
  TR::Node* node,
...
// Z
void
OMR::Z::CodeGenerator::collectConflictingAddressNodes(
  TR::Node* parent,
  TR::Node* node,
  TR::list<TR::Node*>* conflictingAddressNodes)
{
                                                 ...
                                                 TR::Node* node2)
{
  ...
  if (self()->comp()->getOption(TR_TraceVIP))
    traceMsg(
      self()->comp(),
      "\t\tloadOrStoreAddressesMatch = %s : node1 %s (%p) and node2 %s (%p)\n",
      foundMatch ? "true" : "false",
      node1->getOpCode().getName(),
      node1,
      node2->getOpCode().getName(),
      node2);
...
#ifdef J9_PROJECT_SPECIFIC
bool
OMR::Z::CodeGenerator::reliesOnAParticularSignEncoding(TR::Node* node)
{
  TR::ILOpCode op = node->getOpCode();
  if (op.isBCDStore() &&
...
bool
OMR::Z::CodeGenerator::loadAndStoreMayOverlap(TR::Node* store,
                                              size_t storeSize,
                                              TR::Node* load,
                                              size_t loadSize)
{
  ...
  return self()->loadAndStoreMayOverlap(
    store, storeSize, load, loadSize, storeAliases);
}
...
// Z
bool
OMR::Z::CodeGenerator::checkSimpleLoadStore(TR::Node* loadNode,
                                            TR::Node* storeNode,
                                            TR::Block* block)
{
  ...
  if (loadNode->getOpCode().isIndirect() &&
    ...
    if (storeNode->getSize() == 1 && loadNode->getSize() == 1)
      return true;
    else if (self()->loadAndStoreMayOverlap(
               storeNode, storeNode->getSize(), loadNode, loadNode->getSize()))
      return false;
    else
      return true;
  } else {
...
void
setIntegralAddSubFutureUseCount(TR::Node* node, vcount_t visitCount)
{
  if (node == NULL)
    return;
...
OMR::Z::CodeGenerator::setBranchOnCountFlag(TR::Node* node, vcount_t visitCount)
{
  ...
  if (!performTransformation(
        self()->comp(), "%s Found branch on count opportunity\n", OPT_DETAILS))
    return;
  dumpOptDetails(
    self()->comp(),
...
bool
OMR::Z::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress,
