...
OMR::Z::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(),
      ...
      _snippetDataList(getTypedAllocator<TR::S390ConstantDataSnippet*>(
          self()->comp()->allocator())),
      _outOfLineCodeSectionList(getTypedAllocator<TR_S390OutOfLineCodeSection*>(
          self()->comp()->allocator())),
...
bool OMR::Z::CodeGenerator::getSupportsBitPermute() {
  return true;
}
...
bool OMR::Z::CodeGenerator::internalPointerSupportImplemented() {
  return true;
}
...
bool OMR::Z::CodeGenerator::enableRegisterAssociations() {
  return true;
}

bool OMR::Z::CodeGenerator::enableRegisterPairAssociation() {
  return true;
}
...
void OMR::Z::CodeGenerator::apply16BitLabelRelativeRelocation(
    int32_t* cursor,
    TR::LabelSymbol* label,
    int8_t addressDifferenceDivisor,
    bool isInstrOffset) {
  if (isInstrOffset) {
    uint16_t* label_cursor = (uint16_t*)label->getCodeLocation();
    if ((*(uint16_t*)label_cursor) == bos(0x1800))
...
bool OMR::Z::CodeGenerator::excludeInvariantsFromGRAEnabled() {
  return true;
}
