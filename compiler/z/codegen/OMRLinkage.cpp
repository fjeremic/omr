...
extern bool
storeHelperImmediateInstruction(TR::Node *valueChild, TR::CodeGenerator *cg,
                                bool isReversed, TR::InstOpCode::Mnemonic op,
...
OMR::Z::Linkage::Linkage(TR::CodeGenerator *codeGen)
    : OMR::Linkage(codeGen), _explicitLinkageType(TR_S390LinkageDefault),
      _linkageType(TR_None), _stackSizeCheckNeeded(true),
      ...
      _raContextRestoreNeeded(true), _firstSaved(TR::RealRegister::NoReg),
      _lastSaved(TR::RealRegister::NoReg), _lastPrologueInstr(NULL),
...
OMR::Z::Linkage::Linkage(TR::CodeGenerator *codeGen,
    ...
    : OMR::Linkage(codeGen), _explicitLinkageType(elc), _linkageType(lc),
      _stackSizeCheckNeeded(true), _raContextSaveNeeded(true),
      _integerReturnRegister(TR::RealRegister::NoReg),
      _floatReturnRegister(TR::RealRegister::NoReg),
      _doubleReturnRegister(TR::RealRegister::NoReg),
      _longLowReturnRegister(TR::RealRegister::NoReg),
        ...
        case TR::Aggregate: // Should only happen on zLinux
        {
          if (genBinary) {
            cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                storeOpCode, (uint8_t *)cursor, self()->getRealRegister(regNum),
                offset, self()->cg());
          } else {
        ...
        case TR::DecimalFloat:
          ...
          if (genBinary) {
            ...
            TR::MemoryReference *mr = generateS390MemoryReference(
                stackPtr, offset, self()->cg(), param_name);
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::STE, firstNode,
        ...
        case TR::VectorDouble:
          if (genBinary) {
            ...
            TR::MemoryReference *mr = generateS390MemoryReference(
                stackPtr, offset, self()->cg(), param_name);
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::VST, firstNode,
                self()->getRealRegister(regNum), mr, (TR::Instruction *)cursor);
            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
            if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
              self()->removeOSCOnSavedArgument((TR::Instruction *)cursor,
    ...
    for (i1 = 0; i1 < busyIndex; i1++) {
      ...
      if (!(target < 0) && (freeScratchable.isSet(target) ||
        ...
        case 6: // 64bit reg on 31bit
          ...
          cursor =
              generateRRInstruction(self()->cg(), TR::InstOpCode::LR, firstNode,
                                    self()->getRealRegister(REGNUM(target)),
                                    self()->getRealRegister(REGNUM(source)),
                                    (TR::Instruction *)cursor);
          freeScratchable.set(source);
...
void OMR::Z::Linkage::setParameterLinkageRegisterIndex(
  ...
  while ((paramCursor != NULL) &&
    ...
    case TR::DecimalLongDouble:
      ...
      if ((numFloatArgs & 0x1) !=
          0) // if there are odd number of fp args in before a long double arg,
             // need to skip one
        numFloatArgs++;
      if (numFloatArgs < self()->getNumFloatArgumentRegisters()) {
        index = numFloatArgs;
...
TR::InstOpCode::Mnemonic OMR::Z::Linkage::getOpCodeForLinkage(TR::Node *child,
  ...
  case TR::Int64:
    if (TR::Compiler->target.is32Bit())
      ...
      return (isStore) ? TR::InstOpCode::STG
  ...
  case TR::Int32:
    if (TR::Compiler->target.is32Bit())
      ...
      return (isStore)
                 ? (isWiden) ? TR::InstOpCode::STG : TR::InstOpCode::ST
                 : (isRegReg)
                       ? (isWiden) ? TR::InstOpCode::LGFR : TR::InstOpCode::LGR
                       : (isWiden) ? TR::InstOpCode::LGF : TR::InstOpCode::L;
 ...
 */
int32_t
OMR::Z::Linkage::buildArgs(TR::Node *callNode,
                           TR::RegisterDependencyConditions *dependencies,
                           bool isFastJNI, int64_t killMask,
                           TR::Register *&vftReg, bool PassReceiver) {
  ...
  for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to);
    ...
    case TR::Address:
      if (isFastJNI && child->getDataType() == TR::Address) {
        argRegister = self()->pushJNIReferenceArg(callNode, child,
    ...
    case TR::Int64:
      if (TR::Compiler->target.is32Bit()) {
        argRegister =
            self()->pushLongArg32(callNode, child, numIntegerArgs, numFloatArgs,
                                  &stackOffset, dependencies);
        numIntegerArgs += 2;
      } else {
        argRegister = self()->pushArg(callNode, child, numIntegerArgs,
    ...
    case TR::Aggregate:
      if (self()->isAggregatesPassedInParmRegs() ||
        ...
        argRegister =
            self()->pushAggregateArg(callNode, child, numIntegerArgs,
                                     numFloatArgs, &stackOffset, dependencies);
  ...
  case TR::DecimalDouble:
    ...
    self()->cg()->setRealRegisterAssociation(resultReg,
                                             self()->getFloatReturnRegister());
    dependencies->addPostCondition(resultReg, self()->getFloatReturnRegister(),
