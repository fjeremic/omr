...
void OMR::Z::Linkage::markPreservedRegsInDep(TR::RegisterDependencyConditions* deps)
{
  for (int32_t curReg = TR::RealRegister::FirstGPR; curReg <= TR::RealRegister::LastFPR; curReg++) {
    if (self()->getPreserved(REGNUM(curReg)) && deps->searchPostConditionRegister(REGNUM(curReg))) {
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
    ...
    if (lri >= 0) {
      ...
      if (type.isFloatingPoint()) // float/double/long double
      {
        ...
        if (dtype == TR::DecimalLongDouble) {
          ...
          TR_ASSERT(
            (lri < (self()->getNumFloatArgumentRegisters() - 1)), "long double must be passed in FP reg pair in full");
      ...
      // isReferencedParameterAtIlGenTime) was replaced with isParmHasToBeOnStack, which would make sense. However, this
      // broke some java modes (OSR,HCR).  Likely, those modes should rely on 'unconditionalSave' to ensure we always
      // store out to the stack for whenever we need all variables on the stack. This investigation will be a future
      // todo .
      ...
      if ((ai < 0 && paramCursor->isReferencedParameter()) || self()->hasToBeOnStack(paramCursor)
        ...
        case TR::Aggregate: // Should only happen on zLinux
        {
          if (genBinary) {
            ...
            if (storeOpCode == TR::InstOpCode::STCM)
              ...
              cursor = generateRXInstruction(
                self()->cg(), storeOpCode, firstNode, self()->getRealRegister(regNum), mr, (TR::Instruction*)cursor);
          ...
          if (secondStore && fullLong && TR::Compiler->target.is32Bit()) {
            if (genBinary) {
              cursor = (void*)TR::S390CallSnippet::storeArgumentItem(
                TR::InstOpCode::ST, (uint8_t*)cursor, self()->getRealRegister(regNum), offset, self()->cg());
            } else {
        ...
        case TR::DecimalFloat:
          ...
          if (genBinary) {
            cursor = (void*)TR::S390CallSnippet::storeArgumentItem(
              TR::InstOpCode::STE, (uint8_t*)cursor, self()->getRealRegister(regNum), offset, self()->cg());
          } else {
            TR::MemoryReference* mr = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
        ...
        case TR::DecimalDouble:
          ...
          if (genBinary) {
            cursor = (void*)TR::S390CallSnippet::storeArgumentItem(
              TR::InstOpCode::STD, (uint8_t*)cursor, self()->getRealRegister(regNum), offset, self()->cg());
          } else {
            TR::MemoryReference* mr = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
        ...
        case TR::DecimalLongDouble:
          if (genBinary) {
            ...
            cursor = (void*)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::STD, (uint8_t*)cursor,
              self()->getRealRegister(REGNUM(regNum + 2)), offset + 8, self()->cg());
          } else {
            TR::MemoryReference* mr = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
            ...
            cursor = (void*)generateRXInstruction(self()->cg(), TR::InstOpCode::STD, firstNode,
              self()->getRealRegister(regNum), mr, (TR::Instruction*)cursor);
            cursor = (void*)generateRXInstruction(self()->cg(), TR::InstOpCode::STD, firstNode,
              self()->getRealRegister(REGNUM(regNum + 2)), hiMR, (TR::Instruction*)cursor);
            ...
            if (!InPreProlog && !globalAllocatedRegisters.isSet(REGNUM(regNum + 2)))
              self()->removeOSCOnSavedArgument(
                (TR::Instruction*)cursor, self()->getRealRegister(REGNUM(regNum + 2)), offset + 8);
          }
          break;
#endif
        case TR::VectorInt8:
        ...
        case TR::VectorDouble:
          if (genBinary) {
            cursor = (void*)TR::S390CallSnippet::storeArgumentItem(
              TR::InstOpCode::VST, (uint8_t*)cursor, self()->getRealRegister(regNum), offset, self()->cg());
          } else {
            TR::MemoryReference* mr = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
          ...
          if (freeScratchable.isSet(ai)) {
            if (dtype == TR::Float || dtype == TR::Double
              ...
              cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LDR, firstNode,
                self()->getRealRegister(REGNUM(ai)), self()->getRealRegister(regNum), (TR::Instruction*)cursor);
              ...
              cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LXR, firstNode,
                self()->cg()->allocateFPRegisterPair(
                  self()->getRealRegister(REGNUM(ai_l)), self()->getRealRegister(REGNUM(ai))),
                self()->cg()->allocateFPRegisterPair(
                  self()->getRealRegister(REGNUM(regNum + 2)), self()->getRealRegister(REGNUM(regNum))),
              ...
              if (dtype == TR::Int64 && TR::Compiler->target.is32Bit()) {
                cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::SLLG, firstNode,
                  self()->getRealRegister(REGNUM(ai)), self()->getRealRegister(regNum), 32, (TR::Instruction*)cursor);
                ...
                if (fullLong) {
                  cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LR, firstNode,
                    self()->getRealRegister(REGNUM(ai)), self()->getRealRegister(REGNUM(regNum + 1)),
                    (TR::Instruction*)cursor);
          ...
          if (!fullLong) // if long int lower word pass in stack ...
        {
          if (freeScratchable.isSet(ai_l)) {
            cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::getLoadOpCode(), firstNode,
              self()->getRealRegister(REGNUM(ai_l)), generateS390MemoryReference(stackPtr, offset + 4, self()->cg()),
              (TR::Instruction*)cursor);
    ...
    //
      ...
      case TR::Aggregate: // Should only happen on zLinux
        if (freeScratchable.isSet(ai)) {
          ...
          if (loadOpCode == TR::InstOpCode::ICM)
            ...
            cursor = generateRXInstruction(
              self()->cg(), loadOpCode, firstNode, self()->getRealRegister(REGNUM(ai)), mr, (TR::Instruction*)cursor);
      ...
      case TR::DecimalFloat:
        ...
        if (freeScratchable.isSet(ai)) {
          cursor
            = generateRXInstruction(self()->cg(), TR::InstOpCode::LE, firstNode, self()->getRealRegister(REGNUM(ai)),
      ...
      case TR::DecimalDouble:
        ...
        if (freeScratchable.isSet(ai)) {
          cursor
            = generateRXInstruction(self()->cg(), TR::InstOpCode::LD, firstNode, self()->getRealRegister(REGNUM(ai)),
              generateS390MemoryReference(stackPtr, offset, self()->cg()), (TR::Instruction*)cursor);
          ((TR::Instruction*)cursor)->setBinLocalFreeRegs(binLocalRegs);
      ...
      case TR::DecimalLongDouble:
        ...
        if (freeScratchable.isSet(ai)) {
          ...
          ((TR::Instruction*)cursor)->setBinLocalFreeRegs(binLocalRegs);
          cursor
            = generateRXInstruction(self()->cg(), TR::InstOpCode::LD, firstNode, self()->getRealRegister(REGNUM(ai_l)),
  ...
  while (freeMore && numMoves > 0) {
    ...
    for (i1 = 0; i1 < busyIndex; i1++) {
      ...
      if (!(target < 0)
        ...
        {
          cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
            self()->getRealRegister(REGNUM(target)), self()->getRealRegister(REGNUM(source)), (TR::Instruction*)cursor);
        ...
        case 1: // Int or Addr  Reg from Stack Slot
        {
          ...
          if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::ICM)
            ...
            cursor = generateRXInstruction(self()->cg(), (TR::InstOpCode::Mnemonic)busyMoves[3][i1], firstNode,
              self()->getRealRegister(REGNUM(target)), mr, (TR::Instruction*)cursor);
        ...
        case 3: // Floats
          cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::LE, firstNode,
            self()->getRealRegister(REGNUM(target)), generateS390MemoryReference(stackPtr, source, self()->cg()),
            (TR::Instruction*)cursor);
          ((TR::Instruction*)cursor)->setBinLocalFreeRegs(binLocalRegs);
          break;
        case 4: // Doubles
        ...
        case 7: // 64bit reg on 31bit
          ...
          target = busyMoves[1][i1];
          cursor
            = generateRXInstruction(self()->cg(), TR::InstOpCode::L, firstNode, self()->getRealRegister(REGNUM(target)),
...
void OMR::Z::Linkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol* method)
{
  ...
  TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
  ...
  while ((paramCursor != NULL)
    && (numIntArgs < self()->getNumIntegerArgumentRegisters() || numFloatArgs < self()->getNumFloatArgumentRegisters()
    ...
    if (self()->isFastLinkLinkageType()) {
      if ((numFloatArgs == 1) || (numIntArgs >= self()->getNumIntegerArgumentRegisters())) {
        // force fastlink ABI condition of only one float parameter for fastlink parameter and it must be within first
        // slots
...
TR::InstOpCode::Mnemonic OMR::Z::Linkage::getOpCodeForLinkage(TR::Node* child, bool isStore, bool isRegReg)
{
  ...
  case TR::Address:
    return (isStore) ? TR::InstOpCode::getStoreOpCode()
                     : (isRegReg) ? TR::InstOpCode::getLoadRegOpCode() : TR::InstOpCode::getLoadOpCode();
    break;
  case TR::Int8:
  case TR::Int16:
  case TR::Int32:
  ...
  if (isStoreArg) {
    ...
    if (argRegister != NULL) {
      ...
      if (child->getDataType() == TR::DecimalLongDouble)
        self()->storeLongDoubleArgumentOnStack(
          callNode, child->getDataType(), storeOp, argRegister, stackOffsetPtr, stackRegister);
      else
#endif
  ...
  int64_t killMask, TR::Register*& vftReg, bool PassReceiver)
{
  ...
  for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to); i += step) {
    ...
    case TR::Address:
      if (isFastJNI && child->getDataType() == TR::Address) {
        argRegister
          = self()->pushJNIReferenceArg(callNode, child, numIntegerArgs, numFloatArgs, &stackOffset, dependencies);
        numIntegerArgs++;
        break;
      }
    case TR::Int8:
    ...
    case TR::DecimalDouble:
      ...
      break;
    case TR::DecimalLongDouble:
      if (numFloatArgs % 2 == 1) { // need to skip fp arg 'hole' for long double arg
        numFloatArgs++;
    ...
    case TR::Aggregate:
      if (self()->isAggregatesPassedInParmRegs() || self()->isAggregatesPassedOnParmStack()) {
        argRegister
          = self()->pushAggregateArg(callNode, child, numIntegerArgs, numFloatArgs, &stackOffset, dependencies);
  ...
  case TR::Int32:
    resultReg
      = (resType.isAddress()) ? self()->cg()->allocateCollectedReferenceRegister() : self()->cg()->allocateRegister();
...
int64_t OMR::Z::Linkage::killAndAssignRegister(int64_t killMask, TR::RegisterDependencyConditions* deps,
  TR::Register** virtualRegPtr, TR::RealRegister::RegNum regNum, TR::CodeGenerator* codeGen, bool isAllocate,
  bool isDummy)
{
...
int64_t OMR::Z::Linkage::killAndAssignRegister(int64_t killMask, TR::RegisterDependencyConditions* deps,
  TR::Register** virtualRegPtr, TR::RealRegister* realReg, TR::CodeGenerator* codeGen, bool isAllocate, bool isDummy)
{
  return self()->killAndAssignRegister(
    killMask, deps, virtualRegPtr, realReg->getRegisterNumber(), codeGen, isAllocate, isDummy);
}

void OMR::Z::Linkage::generateDispatchReturnLable(TR::Node* callNode, TR::CodeGenerator* codeGen,
  ...
  TR::Node*& GlobalRegDeps, bool& hasGlRegDeps, TR::Register** methodAddressReg, TR::Register*& JavaLitOffsetReg)
{
  ...
  if (dispatchType == TR_SystemDispatch) {
    if (!TR::Compiler->target.isZOS())
      killMask
        = self()->killAndAssignRegister(killMask, deps, methodAddressReg, TR::RealRegister::GPR14, self()->cg(), true);
  }
}
  ...
  TR::RegisterDependencyConditions*& deps, bool isFastJNI, bool isPassReceiver, bool isJNIGCPoint)
{
  ...
  // todo: can be cleaned up better (i.e, get rid of some j9 only params) once other related linkage methods are cleaned
  // up
  self()->setupRegisterDepForLinkage(callNode, dispatchType, deps, killMask, systemLinkage, GlobalRegDeps, hasGlRegDeps,
  ...
  self()->performCallNativeFunctionForLinkage(
    callNode, dispatchType, javaReturnRegister, systemLinkage, deps, javaLitOffsetReg, methodAddressReg, isJNIGCPoint);
  ...
  TR::RealRegister* spReg, TR::MemoryReference* rsa, TR::RealRegister::RegNum spRealReg)
{
  ...
  if (break64 || newNumUsedRegs == 1) {
    ...
    for (uint8_t i = 0; i < newNumUsedRegs; i++) {
      cursor = generateRXInstruction(
        self()->cg(), TR::InstOpCode::getLoadOpCode(), nextNode, self()->getRealRegister(REGNUM(curReg)), rsa, cursor);
  ...
  else if ((TR::Compiler->target.is64Bit() && !break64) || ((newFirstUsedReg + 1) % 2 == 0)) {
    cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
      self()->getRealRegister(REGNUM(newFirstUsedReg)), self()->getRealRegister(REGNUM(newLastUsedReg)), rsa, cursor);
