...
extern TR::Instruction *generateS390ImmToRegister(TR::CodeGenerator *cg,
                                                  TR::Node *node,
                                                  TR::Register *targetRegister,
                                                  intptr_t value,
                                                  TR::Instruction *cursor);
extern bool storeHelperImmediateInstruction(
    TR::Node *valueChild, TR::CodeGenerator *cg, bool isReversed,
...
OMR::Z::Linkage::Linkage(TR::CodeGenerator *codeGen)
    : OMR::Linkage(codeGen),
      _explicitLinkageType(TR_S390LinkageDefault),
      _linkageType(TR_None),
      _stackSizeCheckNeeded(true),
 ...
 */
OMR::Z::Linkage::Linkage(TR::CodeGenerator *codeGen,
                         TR_S390LinkageConventions elc,
                         TR_LinkageConventions lc)
    : OMR::Linkage(codeGen),
      _explicitLinkageType(elc),
      _linkageType(lc),
      _stackSizeCheckNeeded(true),
      _raContextSaveNeeded(true),
      _integerReturnRegister(TR::RealRegister::NoReg),
...
void OMR::Z::Linkage::markPreservedRegsInDep(
    TR::RegisterDependencyConditions *deps) {
  for (int32_t curReg = TR::RealRegister::FirstGPR;
       curReg <= TR::RealRegister::LastFPR; curReg++) {
  ...
  // and the RAReg
  if (deps && deps->searchPostConditionRegister(
                  self()->cg()->getReturnAddressRegister())) {
    self()
        ->getRealRegister(self()->getReturnAddressRegister())
        ->setModified(true);
  }
}

void OMR::Z::Linkage::markPreservedRegsInBlock(int32_t blockNum) {
  // kill RAReg
  self()
      ->getRealRegister(self()->getReturnAddressRegister())
      ->setModified(true);
...
TR::Instruction *OMR::Z::Linkage::loadUpArguments(TR::Instruction *cursor) {
  return NULL;
}

void OMR::Z::Linkage::removeOSCOnSavedArgument(TR::Instruction *instr,
                                               TR::Register *sReg,
                                               int32_t stackOffset) {
  bool done = false;
  ...
  while ((current != NULL) && !current->isLabel() && !current->isCall() &&
         !current->isBranchOp() && windowSize < maxWindowSize && !done) {
    if (self()->comp()->getOption(TR_TraceCG))
      traceMsg(self()->comp(), "%p inspecting\n", current);

    if (current->isLoad()) {
      TR::Register *tReg =
          ((TR::S390RXInstruction *)current)->getRegisterOperand(1);
      ...
      //
      if (sym && sym->isRegisterMappedSymbol() &&
          sym->getKind() == TR::Symbol::IsParameter) {
      ...
      if (disp == stackOffset) {
        ...
        if (opCode == TR::InstOpCode::L) {
          newInst = generateRRInstruction(self()->cg(), TR::InstOpCode::LR,
                                          current->getNode(), tReg, sReg, prev);
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     "\nProlog peeking changing TR::InstOpCode::L %s,%d(SP) to "
                     "TR::InstOpCode::LR %s,%s : [%p] => [%p]\n",
                     tReg->getRegisterName(self()->comp()), stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()), current, newInst);
          }

          mustReplace =
              false;  // If sReg = treg, just delete the redundent load
        } else if (opCode == TR::InstOpCode::LG) {
          newInst = generateRRInstruction(self()->cg(), TR::InstOpCode::LGR,
                                          current->getNode(), tReg, sReg, prev);
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     "\nProlog peeking changing TR::InstOpCode::LG %s,%d(SP) "
                     "to TR::InstOpCode::LGR %s,%s : [%p] => [%p]\n",
                     tReg->getRegisterName(self()->comp()), stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()), current, newInst);
          }
          ...
          newInst = generateRRInstruction(self()->cg(), TR::InstOpCode::LER,
                                          current->getNode(), tReg, sReg, prev);
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     "\nProlog peeking changing TR::InstOpCode::LE %s,%d(SP) "
                     "to TR::InstOpCode::LER %s,%s : [%p] => [%p]\n",
                     tReg->getRegisterName(self()->comp()), stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()), current, newInst);
          }
          ...
          newInst = generateRRInstruction(self()->cg(), TR::InstOpCode::LDR,
                                          current->getNode(), tReg, sReg, prev);
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     "\nProlog peeking changing TR::InstOpCode::LD %s,%d(SP) "
                     "to TR::InstOpCode::LDR %s,%s : [%p] => [%p]\n",
                     tReg->getRegisterName(self()->comp()), stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()), current, newInst);
          }
          ...
          newInst = generateRRInstruction(self()->cg(), TR::InstOpCode::LTR,
                                          current->getNode(), tReg, sReg, prev);
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     "\nProlog peeking changing TR::InstOpCode::LT %s,%d(SP) "
                     "to TR::InstOpCode::LTR %s,%s : [%p] => [%p]\n",
                     tReg->getRegisterName(self()->comp()), stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()), current, newInst);
          }
        } else if (opCode == TR::InstOpCode::LTG) {
          newInst = generateRRInstruction(self()->cg(), TR::InstOpCode::LTGR,
                                          current->getNode(), tReg, sReg, prev);
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     "\nProlog peeking changing TR::InstOpCode::LTG %s,%d(SP) "
                     "to TR::InstOpCode::LTGR %s,%s : [%p] => [%p]\n",
                     tReg->getRegisterName(self()->comp()), stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()), current, newInst);
          }
        } else if (opCode == TR::InstOpCode::LGF) {
          newInst = generateRRInstruction(self()->cg(), TR::InstOpCode::LGFR,
                                          current->getNode(), tReg, sReg, prev);
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     "\nProlog peeking changing TR::InstOpCode::LGF %s,%d(SP) "
                     "to TR::InstOpCode::LGFR %s,%s : [%p] => [%p]\n",
                     tReg->getRegisterName(self()->comp()), stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()), current, newInst);
          }
        ...
        //
        if (newInst && performTransformation(
                           self()->comp(),
                           "O^O : Prolog peeking removing [%p]\n", current)) {
 ...
 * the prolog code.
 */
void *OMR::Z::Linkage::saveArguments(void *cursor, bool genBinary,
                                     bool InPreProlog, int32_t frameOffset,
                                     List<TR::ParameterSymbol> *parameterList) {
  TR::RealRegister *stackPtr = self()->getNormalStackPointerRealRegister();
  static const bool enableVectorLinkage =
  ...
  TR::ResolvedMethodSymbol *bodySymbol =
      self()->comp()->getJittedMethodSymbol();

  ListIterator<TR::ParameterSymbol> paramIterator(
      (parameterList != NULL) ? parameterList
                              : &(bodySymbol->getParameterList()));
  ...
  int32_t busyIndex = 0, lastFreeIntArgIndex = 0, lastFreeFloatArgIndex = 0,
          lastFreeVectorArgIndex = 0, i1;

  uint32_t binLocalRegs =
      0x1 << 14;  // Binary pattern representing reg14 as free for local alloc

  int8_t gprSize = self()->machine()->getGPRSize();

  bool unconditionalSave = false;

  // If we use preexistence or FSD or HCR, then we could be reverting back to
  // the interpreter by creating prePrologue snippets.  In such cases, we need
  ...
  unconditionalSave |=
      (InPreProlog &&
       self()->comp()->cg()->mustGenerateSwitchToInterpreterPrePrologue());
  ...
  if (enableVectorLinkage) {
    ...
    globalAllocatedRegisters.init(TR::RealRegister::LastVRF + 1,
                                  self()->trMemory());
  } else {
    freeScratchable.init(TR::RealRegister::LastFPR + 1, self()->trMemory());
    globalAllocatedRegisters.init(TR::RealRegister::LastFPR + 1,
                                  self()->trMemory());
  }
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    int32_t ai_l =
        paramCursor->getAssignedLowGlobalRegisterIndex();  //  low reg of a pair
  ...
  if (self()->isSkipGPRsForFloatParms()) {  // set GPRs skipped for FPRs to free
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
      ...
      if (type.isFloatingPoint()) {
        ...
        numIntArgs +=
            (type.isInt64() && TR::Compiler->target.is32Bit()) ? 2 : 1;
      }
    }
  }
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    int32_t ai =
        paramCursor->getAssignedGlobalRegisterIndex();  // global reg number
    ...
    // Treat GPR6 special on zLinux.  It is a linkage register but also
    // preserved. If it has no global reg number, locate it on local save area
    // instead of register save area
    if (TR::Compiler->target.isLinux() &&
        (lri > 0 && ai < 0 &&
         (self()->getPreserved(REGNUM(lri + 3)) || dtype.isVector())))
      paramCursor->setParmHasToBeOnStack();

    if (self()->comp()->getOption(TR_TraceCG)) {
      traceMsg(self()->comp(),
               "save argument: %s, lri: %d, ai: %d, offset: %d, isReferenced: "
               "%d, hasToBeOnStack: %d\n",
               param_name, lri, ai, offset,
               paramCursor->isReferencedParameter(),
               self()->hasToBeOnStack(paramCursor));
    }
    ...
    if (((self()->isSmallIntParmsAlignedRight() &&
          paramCursor->getType().isIntegral()) ||
         (self()->isPadFloatParms() &&
          paramCursor->getType().isFloatingPoint())) &&
    ...
    if (lri >= 0) {
      ...
      //
      bool fullLong = (type.isInt64() &&
                       lri < self()->getNumIntegerArgumentRegisters() - 1);
      ...
      if (type.isFloatingPoint())  // float/double/long double
      {
        ...
        if (dtype == TR::DecimalLongDouble) {
          TR_ASSERT(((lri % 2) == 0),
                    "LongDouble must be passed in legal FP reg pair!");
          TR_ASSERT((lri < (self()->getNumFloatArgumentRegisters() - 1)),
                    "long double must be passed in FP reg pair in full");
          lastFreeFloatArgIndex++;
        }
#endif
      } else if (TR::Compiler->target.isLinux() && dtype == TR::Aggregate) {
        TR_ASSERT(
            paramCursor->getSize() <= 8,
        ...
        if (TR::Compiler->target.is32Bit() && paramCursor->getSize() > 4) {
          fullLong = true;  // On 31 bit this larger aggregate will be treated
                            // like a 64 bit long
          lastFreeIntArgIndex++;
        }
      ...
      if (!paramCursor->isReferencedParameter() &&
          !paramCursor->isParmHasToBeOnStack() &&
      ...
      //
      // For static zLinux, if you are a leaf routine, you cannot store anything
      // on to the stack. This query here is actually not ideal.  originally
      // isReferencedParameter (which actually means
      // isReferencedParameterAtIlGenTime) was replaced with
      // isParmHasToBeOnStack, which would make sense. However, this broke some
      // java modes (OSR,HCR).  Likely, those modes should rely on
      // 'unconditionalSave' to ensure we always store out to the stack for
      // whenever we need all variables on the stack. This investigation will be
      // a future todo .

      if ((ai < 0 && paramCursor->isReferencedParameter()) ||
        ...
        switch (dtype) {
          ...
          case TR::Aggregate:  // Should only happen on zLinux
          {
            if (genBinary) {
              cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                  storeOpCode, (uint8_t *)cursor,
                  self()->getRealRegister(regNum), offset, self()->cg());
            } else {
              TR::MemoryReference *mr = generateS390MemoryReference(
                  stackPtr, offset, self()->cg(), param_name);

              if (storeOpCode == TR::InstOpCode::STCM)
                cursor = generateRSInstruction(
                    self()->cg(), TR::InstOpCode::STCM, firstNode,
                    self()->getRealRegister(regNum), opcodeMask, mr,
                    (TR::Instruction *)cursor);

              else
                cursor =
                    generateRXInstruction(self()->cg(), storeOpCode, firstNode,
                                          self()->getRealRegister(regNum), mr,
                                          (TR::Instruction *)cursor);
              ...
              if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                self()->removeOSCOnSavedArgument(
                    (TR::Instruction *)cursor, self()->getRealRegister(regNum),
                    offset);
            }
          }

            if (secondStore && fullLong && TR::Compiler->target.is32Bit()) {
              if (genBinary) {
                cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                    TR::InstOpCode::ST, (uint8_t *)cursor,
                    self()->getRealRegister(regNum), offset, self()->cg());
              } else {
                TR::MemoryReference *mr = generateS390MemoryReference(
                    stackPtr, offset + 4, self()->cg(), param_name);
                cursor = generateRXInstruction(
                    self()->cg(), TR::InstOpCode::ST, firstNode,
                    self()->getRealRegister(REGNUM(regNum + 1)), mr,
                    (TR::Instruction *)cursor);
                ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                  self()->removeOSCOnSavedArgument(
          ...
          case TR::DecimalFloat:
            ...
            if (genBinary) {
              cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                  TR::InstOpCode::STE, (uint8_t *)cursor,
                  self()->getRealRegister(regNum), offset, self()->cg());
            } else {
              TR::MemoryReference *mr = generateS390MemoryReference(
                  stackPtr, offset, self()->cg(), param_name);
              cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::STE,
                                             firstNode,
                                             self()->getRealRegister(regNum),
          ...
          case TR::DecimalDouble:
            ...
            if (genBinary) {
              cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                  TR::InstOpCode::STD, (uint8_t *)cursor,
                  self()->getRealRegister(regNum), offset, self()->cg());
            } else {
              TR::MemoryReference *mr = generateS390MemoryReference(
                  stackPtr, offset, self()->cg(), param_name);
              cursor = (void *)generateRXInstruction(
                  self()->cg(), TR::InstOpCode::STD, firstNode,
                  self()->getRealRegister(regNum), mr,
                  (TR::Instruction *)cursor);
              ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
              if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
          ...
          case TR::DecimalLongDouble:
            if (genBinary) {
              ...
              cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                  TR::InstOpCode::STD, (uint8_t *)cursor,
                  self()->getRealRegister(REGNUM(regNum + 2)), offset + 8,
                  self()->cg());
            } else {
              TR::MemoryReference *mr = generateS390MemoryReference(
                  stackPtr, offset, self()->cg(), param_name);
              TR::MemoryReference *hiMR =
                  generateS390MemoryReference(*mr, 8, self()->cg());
              ...
              cursor = (void *)generateRXInstruction(
                  self()->cg(), TR::InstOpCode::STD, firstNode,
                  self()->getRealRegister(REGNUM(regNum + 2)), hiMR,
                  (TR::Instruction *)cursor);
              ...
              if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                self()->removeOSCOnSavedArgument(
                    ...
                    offset);
              if (!InPreProlog &&
                  !globalAllocatedRegisters.isSet(REGNUM(regNum + 2)))
                self()->removeOSCOnSavedArgument(
                    (TR::Instruction *)cursor,
                    self()->getRealRegister(REGNUM(regNum + 2)), offset + 8);
            }
          ...
          case TR::VectorDouble:
            if (genBinary) {
              cursor = (void *)TR::S390CallSnippet::storeArgumentItem(
                  TR::InstOpCode::VST, (uint8_t *)cursor,
                  self()->getRealRegister(regNum), offset, self()->cg());
            } else {
              TR::MemoryReference *mr = generateS390MemoryReference(
                  stackPtr, offset, self()->cg(), param_name);
              cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::VST,
                                             firstNode,
                                             self()->getRealRegister(regNum),
                                             mr, (TR::Instruction *)cursor);
              ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
        ...
        if (ai < 0) {
          freeScratchable.set(regNum);
#ifdef J9_PROJECT_SPECIFIC
          if (dtype == TR::DecimalLongDouble) freeScratchable.set(regNum + 2);
#endif
        }
      }

      // Global register is allocated to this argument, try and move arg to
      // global
      //
      if (ai >= 0 && !InPreProlog) {
        // Argument regNum is not the same as global reg so add in some register
        // moves to fix up Or we have long reg on 31bir, so we need to build the
        // 64bit register from the two arguments (reg + reg) or (reg + mem) also
        // need to take care of longdouble/complex types which takes 2 or more
        // slots
        if (regNum != ai ||
            (dtype == TR::Int64 && TR::Compiler->target.is32Bit())) {
          ...
          if (freeScratchable.isSet(ai)) {
            if (dtype == TR::Float || dtype == TR::Double
              ...
              cursor = generateRRInstruction(
                  self()->cg(), TR::InstOpCode::LDR, firstNode,
              ...
              int32_t ai_l =
                  paramCursor
                      ->getAssignedLowGlobalRegisterIndex();  //  low reg of a
                                                              //  pair
              TR_ASSERT((ai_l == (ai + 2)),
                        "global RA incorrect for long double params");
              cursor = generateRRInstruction(
                  self()->cg(), TR::InstOpCode::LXR, firstNode,
                  self()->cg()->allocateFPRegisterPair(
                      self()->getRealRegister(REGNUM(ai_l)),
                      self()->getRealRegister(REGNUM(ai))),
                  self()->cg()->allocateFPRegisterPair(
                      self()->getRealRegister(REGNUM(regNum + 2)),
                      self()->getRealRegister(REGNUM(regNum))),
                  (TR::Instruction *)cursor);
              ...
              if (dtype == TR::Int64 && TR::Compiler->target.is32Bit()) {
                cursor = generateRSInstruction(
                    self()->cg(), TR::InstOpCode::SLLG, firstNode,
                    self()->getRealRegister(REGNUM(ai)),
                    self()->getRealRegister(regNum), 32,
                    (TR::Instruction *)cursor);
                ...
                if (fullLong) {
                  cursor = generateRRInstruction(
                      self()->cg(), TR::InstOpCode::LR, firstNode,
                      self()->getRealRegister(REGNUM(ai)),
                      self()->getRealRegister(REGNUM(regNum + 1)),
                      (TR::Instruction *)cursor);

                  freeScratchable.set(regNum + 1);
                } else {
                  cursor = generateRXInstruction(
                      self()->cg(), TR::InstOpCode::L, firstNode,
                      self()->getRealRegister(REGNUM(ai)),
                      generateS390MemoryReference(stackPtr, offset + 4,
                                                  self()->cg()),
                      (TR::Instruction *)cursor);
                }

                // Don't do anything for the low word later on as it was handled
                // here
                //
                skipLong = true;
              } else {
                cursor = generateRRInstruction(
                    self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
                    self()->getRealRegister(REGNUM(ai)),
                    self()->getRealRegister(regNum), (TR::Instruction *)cursor);
      ...
      int32_t ai_l = 0;
      if (!skipLong) ai_l = paramCursor->getAssignedLowGlobalRegisterIndex();
      if (ai_l >= 0 && !InPreProlog && !skipLong) {
        ...
        //  - complex:  can pass real part in FP reg (or FP reg pair for complex
        //  long double) and imag part on stack
        //      - complex float / double
        //      - complex long double
#ifdef J9_PROJECT_SPECIFIC
        if (dtype ==
            TR::DecimalLongDouble)  // if long int lower word pass in stack ...
        {
          TR_ASSERT(ai_l == (ai + 2),
                    "global RA incorrect for long double param");
            ...
            if (!fullLong)  // if long int lower word pass in stack ...
        {
          if (freeScratchable.isSet(ai_l)) {
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::getLoadOpCode(), firstNode,
                self()->getRealRegister(REGNUM(ai_l)),
    ...
    // lri<0: arg was passed in memory, but ai >= 0 means arg was assigned a
    // global reg.
    //
      ...
      switch (dtype) {
        ...
        case TR::Aggregate:  // Should only happen on zLinux
          if (freeScratchable.isSet(ai)) {
            TR::MemoryReference *mr =
                generateS390MemoryReference(stackPtr, offset, self()->cg());

            if (loadOpCode == TR::InstOpCode::ICM)
              cursor = generateRSInstruction(
                  self()->cg(), TR::InstOpCode::ICM, firstNode,
                  self()->getRealRegister(REGNUM(ai)), opcodeMask, mr,
                  (TR::Instruction *)cursor);
            else
              cursor =
                  generateRXInstruction(self()->cg(), loadOpCode, firstNode,
                                        self()->getRealRegister(REGNUM(ai)), mr,
                                        (TR::Instruction *)cursor);
        ...
        case TR::DecimalFloat:
          ...
          if (freeScratchable.isSet(ai)) {
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::LE, firstNode,
                self()->getRealRegister(REGNUM(ai)),
                generateS390MemoryReference(stackPtr, offset, self()->cg()),
                (TR::Instruction *)cursor);
            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
        ...
        case TR::DecimalDouble:
          ...
          if (freeScratchable.isSet(ai)) {
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::LD, firstNode,
                self()->getRealRegister(REGNUM(ai)),
                generateS390MemoryReference(stackPtr, offset, self()->cg()),
                (TR::Instruction *)cursor);
            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
        ...
        case TR::DecimalLongDouble:
          ...
          if (freeScratchable.isSet(ai)) {
            ...
            cursor = generateRXInstruction(
                self()->cg(), TR::InstOpCode::LD, firstNode,
                self()->getRealRegister(REGNUM(ai_l)),
                generateS390MemoryReference(stackPtr, offset + 8, self()->cg()),
                (TR::Instruction *)cursor);
            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
  ...
  for (i1 = lastFreeIntArgIndex; i1 < self()->getNumIntegerArgumentRegisters();
       i1++)
    freeScratchable.set(self()->getIntegerArgumentRegister(i1));

  for (i1 = lastFreeFloatArgIndex; i1 < self()->getNumFloatArgumentRegisters();
       i1++)
    freeScratchable.set(self()->getFloatArgumentRegister(i1));

  if (enableVectorLinkage) {
    for (i1 = lastFreeVectorArgIndex;
         i1 < self()->getNumVectorArgumentRegisters(); i1++)
  ...
  while (freeMore && numMoves > 0) {
    ...
    for (i1 = 0; i1 < busyIndex; i1++) {
      ...
      if (!(target < 0) && (freeScratchable.isSet(target) ||
                            (source == target && busyMoves[2][i1] == 6) ||
        ...
        switch (busyMoves[2][i1]) {
          ...
          {
            TR::MemoryReference *mr =
                generateS390MemoryReference(stackPtr, source, self()->cg());

            if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] ==
                TR::InstOpCode::ICM)
              cursor = generateRSInstruction(
                  self()->cg(), TR::InstOpCode::ICM, firstNode,
            ...
            else if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] ==
                     TR::InstOpCode::LFH)
              cursor = generateRXInstruction(
                  self()->cg(), TR::InstOpCode::LFH, firstNode,
                  self()->getRealRegister(REGNUM(target)), mr,
                  (TR::Instruction *)cursor);

            else
              cursor = generateRXInstruction(
                  self()->cg(), (TR::InstOpCode::Mnemonic)busyMoves[3][i1],
                  firstNode, self()->getRealRegister(REGNUM(target)), mr,
                  (TR::Instruction *)cursor);
          ...
          case 6:  // 64bit reg on 31bit
            cursor = generateRSInstruction(
                self()->cg(), TR::InstOpCode::SLLG, firstNode,
                self()->getRealRegister(REGNUM(target)),
                self()->getRealRegister(REGNUM(source)), 32,
                (TR::Instruction *)cursor);
          ...
          case 9:  // LongDouble
            ...
            cursor = generateRXInstruction(
                ...
                generateS390MemoryReference(stackPtr, source + 8, self()->cg()),
                (TR::Instruction *)cursor);
            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
          ...
          case 10:  // LongDouble
            cursor = generateRRInstruction(
                ...
                self()->cg()->allocateFPRegisterPair(
                    ...
                    self()->getRealRegister(REGNUM(target))),
                self()->cg()->allocateFPRegisterPair(
                    self()->getRealRegister(REGNUM(source + 2)),
                    self()->getRealRegister(REGNUM(source))),
                (TR::Instruction *)cursor);
            freeScratchable.set(source);
  ...
  TR_ASSERT(numMoves <= 0,
            "Circular argument register dependency can and should be avoided.");

  return cursor;
}

void OMR::Z::Linkage::setParameterLinkageRegisterIndex(
  ...
  while ((paramCursor != NULL) &&
    ...
    switch (dt) {
      ...
      case TR::DecimalLongDouble:
        ...
        if ((numFloatArgs & 0x1) !=
            0)  // if there are odd number of fp args in before a long double
                // arg, need to skip one
          numFloatArgs++;
        if (numFloatArgs < self()->getNumFloatArgumentRegisters()) {
          index = numFloatArgs;
        }
    ...
    if (self()->isFastLinkLinkageType()) {
      if ((numFloatArgs == 1) ||
          (numIntArgs >= self()->getNumIntegerArgumentRegisters())) {
        // force fastlink ABI condition of only one float parameter for fastlink
        // parameter and it must be within first slots
        numFloatArgs =
            self()->getNumFloatArgumentRegisters();  // no more float args
                                                     // possible now
      }
    }
  }
...
TR::InstOpCode::Mnemonic OMR::Z::Linkage::getOpCodeForLinkage(TR::Node *child,
                                                              bool isStore,
                                                              bool isRegReg) {
  ...
  switch (child->getDataType()) {
    case TR::Int64:
      if (TR::Compiler->target.is32Bit())
        return (isStore) ? TR::InstOpCode::STM
                         : (isRegReg) ? TR::InstOpCode::LR : TR::InstOpCode::LM;
      else
        return (isStore)
                   ? TR::InstOpCode::STG
                   : (isRegReg) ? TR::InstOpCode::LGR : TR::InstOpCode::LG;
      break;
    case TR::Address:
    ...
    case TR::Int32:
      if (TR::Compiler->target.is32Bit())
        return (isStore) ? TR::InstOpCode::ST
                         : (isRegReg) ? TR::InstOpCode::LR : TR::InstOpCode::L;
      else
        return (isStore) ? (isWiden) ? TR::InstOpCode::STG : TR::InstOpCode::ST
                         : (isRegReg) ? (isWiden) ? TR::InstOpCode::LGFR
                                                  : TR::InstOpCode::LGR
    ...
    case TR::DecimalFloat:
#endif
      return (isStore) ? TR::InstOpCode::STE
                       : (isRegReg) ? TR::InstOpCode::LER : TR::InstOpCode::LE;
    case TR::Double:
#ifdef J9_PROJECT_SPECIFIC
    case TR::DecimalDouble:
#endif
      return (isStore) ? TR::InstOpCode::STD
                       : (isRegReg) ? TR::InstOpCode::LDR : TR::InstOpCode::LD;
#ifdef J9_PROJECT_SPECIFIC
    case TR::DecimalLongDouble:
      return (isStore) ? TR::InstOpCode::STD
                       : (isRegReg) ? TR::InstOpCode::LXR : TR::InstOpCode::LD;
#endif
    case TR::VectorInt8:
    case TR::VectorInt16:
    case TR::VectorInt32:
    case TR::VectorInt64:
    case TR::VectorDouble:
      return (isStore) ? TR::InstOpCode::VST
                       : (isRegReg) ? TR::InstOpCode::VLR : TR::InstOpCode::VL;
  }
  return TR::InstOpCode::NOP;
}

TR::InstOpCode::Mnemonic OMR::Z::Linkage::getRegCopyOpCodeForLinkage(
    TR::Node *child) {
  return self()->getOpCodeForLinkage(child, false, true);
}

TR::InstOpCode::Mnemonic OMR::Z::Linkage::getStoreOpCodeForLinkage(
    TR::Node *child) {
  return self()->getOpCodeForLinkage(child, true, false);
}

TR::InstOpCode::Mnemonic OMR::Z::Linkage::getLoadOpCodeForLinkage(
    TR::Node *child) {
  return self()->getOpCodeForLinkage(child, false, false);
}

TR::Register *OMR::Z::Linkage::copyArgRegister(TR::Node *callNode,
                                               TR::Node *child,
                                               TR::Register *argRegister) {
  TR::Register *tempRegister = NULL;
  TR::InstOpCode::Mnemonic copyOpCode =
      self()->getRegCopyOpCodeForLinkage(child);
  ...
  if (TR::Compiler->target.is32Bit() && child->getDataType() == TR::Int64 &&
    ...
    cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::SRLG, callNode,
                                   tempRegH, argRegister, 32);
    if (debugObj) {
      debugObj->addInstructionComment(toS390RSInstruction(cursor), REG_PARAM);
    }

    cursor = generateRRInstruction(self()->cg(), copyOpCode, callNode, tempRegL,
                                   argRegister);
    if (debugObj) {
    ...
    argRegister =
        self()->cg()->allocateConsecutiveRegisterPair(tempRegL, tempRegH);
    ...
    if (argRegister->containsCollectedReference()) {
      ...
      TR_ASSERT(argRegister->getRegisterPair() == NULL,
                "No longer should have regpair arg here");
    ...
    cursor = generateRRInstruction(self()->cg(), copyOpCode, callNode,
                                   tempRegister, argRegister);

    if ((argRegister->getKind() == TR_FPR) &&
        (argRegister->getRegisterPair() != NULL))
...
TR::Register *OMR::Z::Linkage::pushLongArg32(
    TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
  ...
  switch (callNode->getSymbol()
              ->castToMethodSymbol()
              ->getMandatoryRecognizedMethod()) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (isStorePair || isStoreOnlyLow) {
    ...
    TR::InstOpCode::Mnemonic storeOp =
        isStorePair ? TR::InstOpCode::STM : TR::InstOpCode::ST;
    TR::Register *storeReg = isStorePair ? argRegister : argRegisterLow;
    self()->storeArgumentOnStack(callNode, storeOp, storeReg, stackOffsetPtr,
                                 stackRegister);
  ...
  } else if (self()->isXPLinkLinkageType() ||
             self()->isFastLinkLinkageType())  // call specific
...
TR::Register *OMR::Z::Linkage::getStackRegisterForOutgoingArguments(
  ...
  TR::Register *stackRegister = dependencies->searchPreConditionRegister(
      self()->getNormalStackPointerRegister());
  if (stackRegister == NULL ||
      (!self()->cg()->supportsJITFreeSystemStackPointer())) {
...
TR::Register *OMR::Z::Linkage::pushArg(
    TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
    int32_t numFloatArgs, int32_t *stackOffsetPtr,
    TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister) {
  ...
  if (TR::Compiler->target.is32Bit() && argType.isInt64())
    return self()->pushLongArg32(callNode, child, numIntegerArgs, numFloatArgs,
                                 stackOffsetPtr, dependencies, argRegister);

  bool isStoreArg = self()->isAllParmsOnStack();
  bool isFloatShadowedToIntRegs = false;

  TR::RealRegister::RegNum argRegNum = TR::RealRegister::NoReg,
                           argRegNum2 = TR::RealRegister::NoReg;
  switch (callNode->getSymbol()
              ->castToMethodSymbol()
              ->getMandatoryRecognizedMethod()) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  if ((argRegNum == TR::RealRegister::NoReg)
    ...
    if (argType.isLongDouble()) {
      TR::Register *argRegisterHigh =
          argRegister->getRegisterPair()->getHighOrder();
      TR::Register *argRegisterLow =
          argRegister->getRegisterPair()->getLowOrder();
      dependencies->addPreCondition(argRegisterHigh, argRegNum);
  ...
  if (isStoreArg) {
    ...
    if (argRegister == NULL && child->getOpCode().isLoadConst() &&
      ...
      TR::Register *stackRegister =
          self()->getStackRegisterForOutgoingArguments(
              callNode,
              dependencies);  // delay (possibly) creating this till needed
      if (storeHelperImmediateInstruction(
              child, self()->cg(), false,
              (storeOp == TR::InstOpCode::ST) ? InstOpCode::MVHI
                                              : InstOpCode::MVGHI,
              callNode,
              generateS390MemoryReference(stackRegister, *stackOffsetPtr,
                                          self()->cg()))) {
        argRegister = self()->cg()->evaluate(child);
    ...
    if (argRegister != NULL) {
      ...
      if (child->getDataType() == TR::DecimalLongDouble)
        self()->storeLongDoubleArgumentOnStack(callNode, child->getDataType(),
                                               storeOp, argRegister,
                                               stackOffsetPtr, stackRegister);
      else
#endif
        self()->storeArgumentOnStack(callNode, storeOp, argRegister,
                                     stackOffsetPtr, stackRegister);
    }
  ...
  if (isFloatShadowedToIntRegs) {
    TR::Register *stackRegister = self()->getStackRegisterForOutgoingArguments(
        callNode, dependencies);  // delay (possibly) creating this till needed
    self()->loadIntArgumentsFromStack(callNode, dependencies, argDataType,
                                      (*stackOffsetPtr) - argSize, argSize,
...
TR::Register *OMR::Z::Linkage::pushJNIReferenceArg(
    TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
    int32_t numFloatArgs, int32_t *stackOffsetPtr,
    TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister) {
  ...
  if (child->getOpCodeValue() == TR::loadaddr) {
    ...
    if (sym) {
      if (!sym->isAddressOfClassObject()) isCheckForNull = true;
    } else {
  ...
  if (isNull) {
    ...
    generateRRInstruction(self()->cg(), TR::InstOpCode::getXORRegOpCode(),
                          child, pushRegister, pushRegister);
    ...
    generateRXInstruction(
        self()->cg(), TR::InstOpCode::getLoadOpCode(), child, whatReg,
        generateS390MemoryReference(addrReg, (int32_t)0, self()->cg()));

    self()->cg()->decReferenceCount(child);
    if (!self()->cg()->canClobberNodesRegister(child, 0)) {
      pushRegister = self()->cg()->allocateRegister();
      generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(),
                            child, pushRegister, addrReg);
    } else {
      pushRegister = addrReg;
    }
    generateRIInstruction(self()->cg(),
                          TR::InstOpCode::getCmpHalfWordImmOpCode(), child,
                          whatReg, 0);

    generateS390LabelInstruction(self()->cg(), TR::InstOpCode::LABEL, child,
                                 cFlowRegionStart);
    cFlowRegionStart->setStartInternalControlFlow();
    generateS390BranchInstruction(self()->cg(), TR::InstOpCode::BRC,
                                  TR::InstOpCode::COND_BNE, child,
                                  nonNullLabel);
    generateRRInstruction(self()->cg(), TR::InstOpCode::getXORRegOpCode(),
                          child, pushRegister, pushRegister);

    TR::RegisterDependencyConditions *conditions = new (self()->trHeapMemory())
        TR::RegisterDependencyConditions(0, 3, self()->cg());
    ...
    generateS390LabelInstruction(self()->cg(), TR::InstOpCode::LABEL, child,
                                 nonNullLabel, conditions);
    nonNullLabel->setEndInternalControlFlow();

    self()->cg()->stopUsingRegister(whatReg);
  }

  return self()->pushArg(callNode, child, numIntegerArgs, numFloatArgs,
                         stackOffsetPtr, dependencies, pushRegister);
}

TR::Register *OMR::Z::Linkage::pushVectorArg(
    TR::Node *callNode, TR::Node *child, int32_t numVectorArgs, int32_t pindex,
    int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies,
    TR::Register *argRegister) {
  TR::DataType argType = child->getType();
...

  {
    ...
    if (argRegNum == TR::RealRegister::NoReg) isStoreArg = true;
  }
  ...
  if (isStoreArg) {
    ...
    TR::Register *stackRegister = self()->getStackRegisterForOutgoingArguments(
        callNode, dependencies);  // delay (possibly) creating this till needed
    self()->storeArgumentOnStack(callNode, storeOp, argRegister, stackOffsetPtr,
                                 stackRegister);
...
TR::Register *OMR::Z::Linkage::pushAggregateArg(
    TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
    int32_t numFloatArgs, int32_t *stackOffsetPtr,
    TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister) {
#ifdef J9_PROJECT_SPECIFIC
 ...
 * canonical positions
 */
void OMR::Z::Linkage::loadIntArgumentsFromStack(
    TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::DataType argType, int32_t stackOffset, int32_t argSize,
    int32_t numIntegerArgs, TR::Register *stackRegister) {
  ...
  if (argRegNum != TR::RealRegister::NoReg) {
    ...
    for (int32_t i = 0; i < regsNeeded; i++) {
      ...
      dependencies->addPreCondition(argRegister, argRegNum);
      TR::MemoryReference *argMemRef =
          generateS390MemoryReference(stackRegister, stackOffset, self()->cg());
      TR::InstOpCode::Mnemonic loadOp = TR::InstOpCode::getLoadOpCode();
#ifdef J9_PROJECT_SPECIFIC
      if (argType == TR::DecimalFloat &&
          TR::Compiler->target.is64Bit())  // special case for DecFloat on 64bit
        loadOp = TR::InstOpCode::LLGF;
#endif

      generateRXInstruction(self()->cg(), loadOp, callNode, argRegister,
                            argMemRef);

      stackOffset += gprSize;
      numIntegerArgs++;
      argRegNum = self()->getIntegerArgumentRegister(numIntegerArgs);
      if (argRegNum == TR::RealRegister::NoReg) break;
    }
  }
}

/**
 * Do not kill special regs (java stack ptr, system stack ptr, and method
 * metadata reg)
 */
void OMR::Z::Linkage::doNotKillSpecialRegsForBuildArgs(TR::Linkage *linkage,
                                                       bool isFastJNI,
                                                       int64_t &killMask) {
  int32_t i;
  for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastFPR; i++) {
    if (linkage->getPreserved(REGNUM(i))) killMask &= ~(0x1L << REGINDEX(i));
  }
}
 ...
 */
int32_t OMR::Z::Linkage::buildArgs(
    TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    bool isFastJNI, int64_t killMask, TR::Register *&vftReg,
    bool PassReceiver) {
  TR::SystemLinkage *systemLinkage =
      (TR::SystemLinkage *)self()->cg()->getLinkage(TR_System);
  ...
  // For the generated classObject argument, we didn't use them in the dispatch
  // sequence. Simply evaluating them would be enough. Care must be taken when
  // we begin to use them, in order not to spill in the dispatch sequence.
  ...
  if ((callNode->getNumChildren() >= 1) &&
      (callNode->getChild(numChildren)->getOpCodeValue() == TR::GlRegDeps))
    numChildren--;

  // setup helper routine arguments in reverse order
  bool rightToLeft = self()->isParmsInReverseOrder() &&
                     // we want the arguments for induceOSR to be passed from
                     // left to right as in any other non-helper call
  ...
  for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to);
       i += step) {
  ...
  if (!self()->isFirstParmAtFixedOffset()) {
    ...
    if (isXPLinkToPureOSLinkageCall) {
      ...
      // The OS linkage argument list is embedded in the XPLink outbound
      // argument area at offset 8 instead of 0 (to bypass collision with long
      // displacement slot)
      ...
      TR::RealRegister *stackPtr = self()->getNormalStackPointerRealRegister();
      generateRXInstruction(
          self()->cg(), TR::InstOpCode::LA, callNode, r1Reg,
  ...
  stackOffset = self()->storeExtraEnvRegForBuildArgs(
      callNode, self(), dependencies, isFastJNI, stackOffset, gprSize,
      numIntegerArgs);
  ...
  for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to);
    ...
    switch (child->getDataType()) {
      case TR::Address:
        if (isFastJNI && child->getDataType() == TR::Address) {
          argRegister = self()->pushJNIReferenceArg(
              callNode, child, numIntegerArgs, numFloatArgs, &stackOffset,
              dependencies);
          numIntegerArgs++;
          break;
        }
      ...
      case TR::Int16:
      case TR::Int32:
        argRegister = self()->pushArg(callNode, child, numIntegerArgs,
                                      numFloatArgs, &stackOffset, dependencies);
        numIntegerArgs++;
        break;
      case TR::Int64:
        if (TR::Compiler->target.is32Bit()) {
          argRegister =
              self()->pushLongArg32(callNode, child, numIntegerArgs,
                                    numFloatArgs, &stackOffset, dependencies);
          numIntegerArgs += 2;
        } else {
          argRegister =
              self()->pushArg(callNode, child, numIntegerArgs, numFloatArgs,
      ...
      case TR::DecimalLongDouble:
        if (numFloatArgs % 2 ==
            1) {  // need to skip fp arg 'hole' for long double arg
          numFloatArgs++;
          if (self()->isSkipGPRsForFloatParms() &&
              numIntegerArgs < self()->getNumIntegerArgumentRegisters())
            numIntegerArgs++;
        }
      ...
      case TR::Aggregate:
        if (self()->isAggregatesPassedInParmRegs() ||
            self()->isAggregatesPassedOnParmStack()) {
          argRegister = self()->pushAggregateArg(callNode, child,
                                                 numIntegerArgs, numFloatArgs,
                                                 &stackOffset, dependencies);
      ...
      case TR::VectorInt64:
      case TR::VectorDouble:
        argRegister = self()->pushVectorArg(callNode, child, numVectorArgs, i,
                                            &stackOffset, dependencies);
        numVectorArgs++;
        break;
    }
  ...
  switch (resDataType) {
    ...
    case TR::DecimalDouble:
      ...
      self()->cg()->setRealRegisterAssociation(
          resultReg, self()->getFloatReturnRegister());
      dependencies->addPostCondition(resultReg,
                                     self()->getFloatReturnRegister(),
    ...
    case TR::DecimalLongDouble: {
      ...
      self()->cg()->setRealRegisterAssociation(
          highReg, self()->getLongDoubleReturnRegister0());
      dependencies->addPostCondition(highReg,
                                     self()->getLongDoubleReturnRegister0(),
                                     DefinesDependentRegister);
      self()->cg()->setRealRegisterAssociation(
          lowReg, self()->getLongDoubleReturnRegister2());
      dependencies->addPostCondition(lowReg,
                                     self()->getLongDoubleReturnRegister2(),
                                     DefinesDependentRegister);
    ...
    case TR::Int64:
      if (TR::Compiler->target.is32Bit()) {
        // In this case, private and system linkage use same regs for return
        // value
        TR::Register *resultRegLow = self()->cg()->allocateRegister();
        TR::Register *resultRegHigh = self()->cg()->allocateRegister();

        self()->cg()->setRealRegisterAssociation(
            resultRegLow, self()->getLongLowReturnRegister());
        dependencies->addPostCondition(resultRegLow,
                                       self()->getLongLowReturnRegister(),
                                       DefinesDependentRegister);
        killMask &= (~(0x1L << REGINDEX(self()->getLongLowReturnRegister())));

        self()->cg()->setRealRegisterAssociation(
            resultRegHigh, self()->getLongHighReturnRegister());
        dependencies->addPostCondition(resultRegHigh,
                                       self()->getLongHighReturnRegister(),
                                       DefinesDependentRegister);
    ...
    case TR::Int16:
    case TR::Int32:
      resultReg = (resType.isAddress())
                      ? self()->cg()->allocateCollectedReferenceRegister()
                      : self()->cg()->allocateRegister();
      self()->cg()->setRealRegisterAssociation(
          resultReg, self()->getIntegerReturnRegister());
      dependencies->addPostCondition(resultReg,
                                     self()->getIntegerReturnRegister(),
                                     DefinesDependentRegister);
      killMask &= (~(0x1L << REGINDEX(self()->getIntegerReturnRegister())));
      // add extra return register dependency
      killMask = self()->addFECustomizedReturnRegDependency(
          killMask, self(), resType, dependencies);
      break;
    case TR::VectorInt8:
    case TR::VectorInt16:
    case TR::VectorInt32:
    ...
    case TR::VectorDouble:
      ...
      self()->cg()->setRealRegisterAssociation(
          resultReg, self()->getVectorReturnRegister());
      dependencies->addPostCondition(resultReg,
                                     self()->getVectorReturnRegister(),
                                     DefinesDependentRegister);
      killMask &= (~(0x1L << REGINDEX(self()->getVectorReturnRegister())));
      break;
  }

  // In zLinux, GPR6 may be used for param passing, in this case, we need to
  // kill it so kill GPR6 if used in preDeps and is not in postDeps
  if (TR::Compiler->target.isLinux()) {
    TR::Register *gpr6Reg =
        dependencies->searchPreConditionRegister(TR::RealRegister::GPR6);
    if (gpr6Reg) {
      gpr6Reg =
          dependencies->searchPostConditionRegister(TR::RealRegister::GPR6);
    }
  }
  ...
  for (i = TR::RealRegister::FirstGPR; i <= last; i++) {
    if ((killMask & (0x1L << REGINDEX(i)))) {
      dummyReg = NULL;
      self()->killAndAssignRegister(killMask, dependencies, &dummyReg,
                                    REGNUM(i), self()->cg(), true, true);
    }
  }
...
void OMR::Z::Linkage::replaceCallWithJumpInstruction(
    TR::Instruction *callInstruction) {
  TR::InstOpCode::Mnemonic opCode = callInstruction->getOpCodeValue();
  TR_ASSERT(
      opCode == TR::InstOpCode::BASSM || opCode == TR::InstOpCode::BASR ||
          opCode == TR::InstOpCode::BALR || opCode == TR::InstOpCode::BAS ||
          opCode == TR::InstOpCode::BAL || opCode == TR::InstOpCode::BRAS ||
          opCode == TR::InstOpCode::BRASL,
      "Wrong opcode type on instruction!\n");

  TR::Node *node = (callInstruction)->getNode();
  TR::SymbolReference *callSymRef =
      (callInstruction)->getNode()->getSymbolReference();
  TR::Symbol *callSymbol =
      (callInstruction)->getNode()->getSymbolReference()->getSymbol();

  TR::Instruction *replacementInst = 0;

  replacementInst = new (self()->trHeapMemory())
      TR::S390RILInstruction(TR::InstOpCode::BRCL, node, (uint32_t)0xf,
                             callSymbol, callSymRef, self()->cg());
...
TR::Instruction *OMR::Z::Linkage::storeArgumentOnStack(
    TR::Node *callNode, TR::InstOpCode::Mnemonic opCode, TR::Register *argReg,
    int32_t *stackOffsetPtr, TR::Register *stackRegister) {
  TR::Instruction *cursor;
  int32_t padding =
      (self()->isPadFloatParms() && opCode == TR::InstOpCode::STE) ? 4 : 0;
  ...
  if (opCode == TR::InstOpCode::STM) {
    cursor = generateRSInstruction(self()->cg(), opCode, callNode, argReg,
                                   argMemRef);
  } else if (opCode == TR::InstOpCode::VST) {
    cursor = generateVRXInstruction(self()->cg(), opCode, callNode, argReg,
                                    argMemRef);
  } else {
    cursor = generateRXInstruction(self()->cg(), opCode, callNode, argReg,
                                   argMemRef);
  }
 ...
 */
TR::Instruction *OMR::Z::Linkage::storeLongDoubleArgumentOnStack(
    TR::Node *callNode, TR::DataType argType, TR::InstOpCode::Mnemonic opCode,
    TR::Register *argReg, int32_t *stackOffsetPtr,
    TR::Register *stackRegister) {
  TR::Instruction *cursor;
  ...
  TR::MemoryReference *argMemRef =
      generateS390MemoryReference(stackRegister, storeOffset, self()->cg());
  TR::MemoryReference *argMemRef2 =
      generateS390MemoryReference(*argMemRef, size, self()->cg());
  generateRXInstruction(self()->cg(), opCode, callNode, argReg->getHighOrder(),
                        argMemRef);
  argMemRef->stopUsingMemRefRegister(self()->cg());
  cursor = generateRXInstruction(self()->cg(), opCode, callNode,
                                 argReg->getLowOrder(), argMemRef2);
...
int64_t OMR::Z::Linkage::killAndAssignRegister(
    int64_t killMask, TR::RegisterDependencyConditions *deps,
  ...
  if (depVirReg) {
    ...
    *virtualRegPtr = depVirReg;
    ...
    if (isAllocate) codeGen->stopUsingRegister(*virtualRegPtr);

    if (isDummy) {
      (*virtualRegPtr)->setPlaceholderReg();
      if (TR::Compiler->target.is64Bit() &&
          regNum <= TR::RealRegister::LastGPR) {
...
int64_t OMR::Z::Linkage::killAndAssignRegister(
    ...
    TR::CodeGenerator *codeGen, bool isAllocate, bool isDummy) {
  return self()->killAndAssignRegister(killMask, deps, virtualRegPtr,
                                       realReg->getRegisterNumber(), codeGen,
                                       isAllocate, isDummy);
}

void OMR::Z::Linkage::generateDispatchReturnLable(
    TR::Node *callNode, TR::CodeGenerator *codeGen,
    TR::RegisterDependencyConditions *&deps, TR::Register *javaReturnRegister,
    bool hasGlRegDeps, TR::Node *GlobalRegDeps) {
  ...
  TR::RegisterDependencyConditions *postDeps = new (self()->trHeapMemory())
      TR::RegisterDependencyConditions(NULL, deps->getPostConditions(), 0,
                                       deps->getAddCursorForPost(),
                                       self()->cg());

  generateS390LabelInstruction(codeGen, TR::InstOpCode::LABEL, callNode,
                               endOfDirectToJNILabel, postDeps);

#ifdef J9_PROJECT_SPECIFIC
  if (codeGen->getSupportsRuntimeInstrumentation())
    TR::TreeEvaluator::generateRuntimeInstrumentationOnOffSequence(
        codeGen, TR::InstOpCode::RION, callNode);
#endif
  ...
  if (javaReturnRegister) {
    deps->stopUsingDepRegs(self()->cg(),
                           javaReturnRegister->getRegisterPair() == NULL
                               ? javaReturnRegister
                               : javaReturnRegister->getHighOrder(),
...
void OMR::Z::Linkage::setupRegisterDepForLinkage(
    TR::Node *callNode, TR_DispatchType dispatchType,
    TR::RegisterDependencyConditions *&deps, int64_t &killMask,
    TR::SystemLinkage *systemLinkage, TR::Node *&GlobalRegDeps,
    bool &hasGlRegDeps, TR::Register **methodAddressReg,
  ...
  hasGlRegDeps =
      (callNode->getNumChildren() >= 1) &&
      (callNode->getChild(callNode->getNumChildren() - 1)->getOpCodeValue() ==
       TR::GlRegDeps);
  ...
  if (!TR::Compiler->target.isZOS()) {
    ...
    killMask = self()->killAndAssignRegister(
        killMask, deps, &parm3VirtualRegister, TR::RealRegister::GPR4,
        self()->cg(), true);
  }

  if (dispatchType == TR_SystemDispatch) {
    if (!TR::Compiler->target.isZOS())
      killMask = self()->killAndAssignRegister(killMask, deps, methodAddressReg,
                                               TR::RealRegister::GPR14,
                                               self()->cg(), true);
  }
}

void OMR::Z::Linkage::setupBuildArgForLinkage(
    TR::Node *callNode, TR_DispatchType dispatchType,
    TR::RegisterDependencyConditions *deps, bool isFastJNI, bool isPassReceiver,
    int64_t &killMask, TR::Node *GlobalRegDeps, bool hasGlRegDeps,
    TR::SystemLinkage *systemLinkage) {
  TR::RegisterDependencyConditions *glRegDeps;

  TR::Register *systemReturnAddressVirtualRegister = NULL;
  TR::RealRegister::RegNum systemReturnAddressRegister =
      systemLinkage->getReturnAddressRegister();

  killMask = self()->killAndAssignRegister(
      killMask, deps, &systemReturnAddressVirtualRegister,
      systemReturnAddressRegister, self()->cg(), true);

  // Evaluate and load arguments to native function
  TR::Register *vftReg = NULL;
  int32_t argSize = systemLinkage->buildArgs(callNode, deps, isFastJNI,
                                             killMask, vftReg, isPassReceiver);

  if (hasGlRegDeps) {
    self()->cg()->evaluate(GlobalRegDeps);
    glRegDeps =
        generateRegisterDependencyConditions(self()->cg(), GlobalRegDeps, 32);
    ...
    for (int i = 0; i < glRegDeps->getAddCursorForPre(); i++) {
      ...
      deps->addPreConditionIfNotAlreadyInserted(regDep->getRegister(),
                                                regDep->getRealRegister());
    }

    // Add postconditions
    for (int i = 0; i < glRegDeps->getAddCursorForPost(); i++) {
      regDep = glRegDeps->getPostConditions()->getRegisterDependency(i);
      deps->addPostConditionIfNotAlreadyInserted(regDep->getRegister(),
                                                 regDep->getRealRegister());
    }
  }
}

void OMR::Z::Linkage::performCallNativeFunctionForLinkage(
    TR::Node *callNode, TR_DispatchType dispatchType,
    TR::Register *&javaReturnRegister, TR::SystemLinkage *systemLinkage,
    TR::RegisterDependencyConditions *&deps, TR::Register *javaLitOffsetReg,
    TR::Register *methodAddressReg, bool isJNIGCPoint) {
  ...
  if (dispatchType == TR_SystemDispatch) {
    ...
    TR::MethodSymbol *methodSymbol =
        methodSymRef->getSymbol()->castToMethodSymbol();
    targetAddress = (intptrj_t)methodSymbol->getMethodAddress();
  }

  javaReturnRegister = systemLinkage->callNativeFunction(
      callNode, deps, targetAddress, methodAddressReg, javaLitOffsetReg,
      returnFromJNICallLabel, jniCallDataSnippet, isJNIGCPoint);
}

TR::Register *OMR::Z::Linkage::buildNativeDispatch(
    TR::Node *callNode, TR_DispatchType dispatchType,
    TR::RegisterDependencyConditions *&deps, bool isFastJNI,
    bool isPassReceiver, bool isJNIGCPoint) {
  if (self()->comp()->getOption(TR_TraceCG))
    traceMsg(self()->comp(), "\nbuildNativeDispatch\n");
  ...
  /*********************************/
  // omr: extracted out code that deals with register dependencies into it's own
  // method in order to sink j9 specific customizations out with virutal
  // dispatch. todo: can be cleaned up better (i.e, get rid of some j9 only
  // params) once other related linkage methods are cleaned up
  self()->setupRegisterDepForLinkage(callNode, dispatchType, deps, killMask,
                                     systemLinkage, GlobalRegDeps, hasGlRegDeps,
                                     &methodAddressReg, javaLitOffsetReg);

  /*********************************/
  /* Build Arguments               */
  /*********************************/
  self()->setupBuildArgForLinkage(callNode, dispatchType, deps, isFastJNI,
                                  isPassReceiver, killMask, GlobalRegDeps,
                                  hasGlRegDeps, systemLinkage);

  // omr todo: this should be cleaned up with
  // TR::S390PrivateLinkage::setupBuildArgForLinkage and JNI Dispatch
  // if (dispatchType == TR_JNIDispatch)  return NULL;

  /*********************************/
  /*  Call Native Function         */
  /*********************************/
  self()->performCallNativeFunctionForLinkage(
      callNode, dispatchType, javaReturnRegister, systemLinkage, deps,
      javaLitOffsetReg, methodAddressReg, isJNIGCPoint);

  /*********************************/
  /* Generate Return Labels etc.   */
  /*********************************/
  self()->generateDispatchReturnLable(callNode, self()->cg(), deps,
                                      javaReturnRegister, hasGlRegDeps,
                                      GlobalRegDeps);
 ...
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getFirstSavedRegister(int32_t fromreg,
                                                                int32_t toreg) {
 ...
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getLastSavedRegister(int32_t fromreg,
                                                               int32_t toreg) {
 ...
 * Get last used register between fromreg to toreg
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getFirstRestoredRegister(
    int32_t fromreg, int32_t toreg) {
 ...
 * Get last used register between fromreg to toreg
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getLastRestoredRegister(
    int32_t fromreg, int32_t toreg) {
...
TR::Instruction *OMR::Z::Linkage::restorePreservedRegs(
    TR::RealRegister::RegNum firstUsedReg, TR::RealRegister::RegNum lastUsedReg,
    int32_t blockNumber, TR::Instruction *cursor, TR::Node *nextNode,
    TR::RealRegister *spReg, TR::MemoryReference *rsa,
    TR::RealRegister::RegNum spRealReg) {
  bool break64 = TR::Compiler->target.is64Bit() &&
                 self()->comp()->getOption(TR_Enable39064Epilogue);
  ...
  if (break64 || newNumUsedRegs == 1) {
    ...
    for (uint8_t i = 0; i < newNumUsedRegs; i++) {
      cursor = generateRXInstruction(
          self()->cg(), TR::InstOpCode::getLoadOpCode(), nextNode,
  ...
  // 64-bit non-optimized CASE AND 32-bit shorter LMG case
    ...
    cursor = generateRSInstruction(
        self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
  ...
  } else  // 32-bit (try to use bumps)
  {
    ...
    rsa = generateS390MemoryReference(
        spReg, rsa->getOffset() + self()->machine()->getGPRSize(),
        self()->cg());
    cursor = generateRSInstruction(
        self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
...
int32_t OMR::Z::Linkage::getFirstMaskedBit(int16_t mask, int32_t from,
                                           int32_t to) {
...
int32_t OMR::Z::Linkage::getLastMaskedBit(int16_t mask, int32_t from,
                                          int32_t to) {
...
int32_t OMR::Z::Linkage::getFirstMaskedBit(int16_t mask) {
  return TR::Linkage::getFirstMaskedBit(mask, 0, 15);
}

int32_t OMR::Z::Linkage::getLastMaskedBit(int16_t mask) {
  return TR::Linkage::getLastMaskedBit(mask, 0, 15);
}

bool OMR::Z::Linkage::isOSLinkageType() {
  return (self()->getExplicitLinkageType() & TR_SystemOS_MASK) != 0;
}

bool OMR::Z::Linkage::isXPLinkLinkageType() {
  return self()->getExplicitLinkageType() == TR_SystemXPLink;
}

bool OMR::Z::Linkage::isFastLinkLinkageType() {
  return self()->getExplicitLinkageType() == TR_SystemFastLink;
}

bool OMR::Z::Linkage::isZLinuxLinkageType() {
...
TR::Register *OMR::Z::Linkage::buildNativeDispatch(
  ...
  return self()->buildNativeDispatch(callNode, dispatchType, deps, false, true,
                                     true);
}

int32_t OMR::Z::Linkage::getOutgoingParameterBlockSize() {
...
void OMR::Z::Linkage::setIntegerArgumentRegister(uint32_t index,
...
TR::RealRegister::RegNum OMR::Z::Linkage::getLongHighArgumentRegister(
    uint32_t index) {
  if ((index < _numIntegerArgumentRegisters - 1) ||
      ((index == _numIntegerArgumentRegisters - 1) &&
       self()->isSplitLongParm()))
...
void OMR::Z::Linkage::setFloatArgumentRegister(uint32_t index,
                                               TR::RealRegister::RegNum r) {
...
void OMR::Z::Linkage::setVectorArgumentRegister(uint32_t index,
                                                TR::RealRegister::RegNum r) {
  _vectorArgRegisters[index] = r;
  self()->setRegisterFlag(r, VectorArgument);
}

TR::RealRegister::RegNum OMR::Z::Linkage::setIntegerReturnRegister(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, IntegerReturn);
  return _integerReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongLowReturnRegister(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, IntegerReturn);
  return _longLowReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongHighReturnRegister(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, IntegerReturn);
  return _longHighReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongReturnRegister(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, IntegerReturn);
  return _longReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setFloatReturnRegister(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, FloatReturn);
  return _floatReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setDoubleReturnRegister(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, FloatReturn);
  return _doubleReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister0(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, FloatReturn);
  return _longDoubleReturnRegister0 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister2(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, FloatReturn);
  return _longDoubleReturnRegister2 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister4(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, FloatReturn);
  return _longDoubleReturnRegister4 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister6(
    TR::RealRegister::RegNum r) {
  self()->setRegisterFlag(r, FloatReturn);
  return _longDoubleReturnRegister6 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setVectorReturnRegister(
    TR::RealRegister::RegNum r) {
...
TR::RealRegister::RegNum OMR::Z::Linkage::getNormalStackPointerRegister() {
  return self()->getStackPointerRegister();
}

TR::RealRegister *OMR::Z::Linkage::getNormalStackPointerRealRegister() {
  return self()->getStackPointerRealRegister();
}

TR::RealRegister *OMR::Z::Linkage::getEntryPointRealRegister() {
  return self()->getRealRegister(_entryPointRegister);
}

TR::RealRegister *OMR::Z::Linkage::getLitPoolRealRegister() {
  return self()->getRealRegister(_litPoolRegister);
}

TR::RealRegister *OMR::Z::Linkage::getStaticBaseRealRegister() {
...
TR::RealRegister *OMR::Z::Linkage::getRealRegister(
