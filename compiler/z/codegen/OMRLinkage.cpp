...
OMR::Z::Linkage::markPreservedRegsInDep(TR::RegisterDependencyConditions* deps)
{
  for (int32_t curReg = TR::RealRegister::FirstGPR;
       curReg <= TR::RealRegister::LastFPR;
       curReg++) {
...
void
OMR::Z::Linkage::removeOSCOnSavedArgument(TR::Instruction* instr,
                                          TR::Register* sReg,
                                          int32_t stackOffset)
{
  bool done = false;
  ...
  if (self()->comp()->getOption(TR_TraceCG))
    traceMsg(
      self()->comp(), "%p SP OFFSET %d, SREG %p\n", instr, stackOffset, sReg);
    ...
    if (current->isLoad()) {
      ...
      if (disp == stackOffset) {
        ...
        if (opCode == TR::InstOpCode::L) {
          newInst = generateRRInstruction(self()->cg(),
                                          TR::InstOpCode::LR,
                                          current->getNode(),
                                          tReg,
                                          sReg,
          ...
          if (self()->comp()->getOption(TR_TraceCG)) {
            traceMsg(self()->comp(),
                     ...
                     stackOffset,
                     tReg->getRegisterName(self()->comp()),
                     sReg->getRegisterName(self()->comp()),
                     current,
                     newInst);
          }
 ...
 */
void*
OMR::Z::Linkage::saveArguments(void* cursor,
                               bool genBinary,
                               bool InPreProlog,
                               int32_t frameOffset,
                               List<TR::ParameterSymbol>* parameterList)
{
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    if (lri >= 0) {
      ...
      //
      bool fullLong =
        (type.isInt64() && lri < self()->getNumIntegerArgumentRegisters() - 1);
          ...
          case TR::Aggregate: // Should only happen on zLinux
          {
            if (genBinary) {
              ...
              if (storeOpCode == TR::InstOpCode::STCM)
                cursor = generateRSInstruction(self()->cg(),
                                               TR::InstOpCode::STCM,
                                               firstNode,
                                               self()->getRealRegister(regNum),
                                               opcodeMask,
          ...
          case TR::DecimalDouble:
            ...
            if (genBinary) {
              ...
              TR::MemoryReference* mr = generateS390MemoryReference(
                stackPtr, offset, self()->cg(), param_name);
              cursor =
                (void*)generateRXInstruction(self()->cg(),
                                             TR::InstOpCode::STD,
                                             firstNode,
                                             self()->getRealRegister(regNum),
                                             mr,
        ...
        if (regNum != ai ||
          ...
          if (freeScratchable.isSet(ai)) {
            if (dtype == TR::Float || dtype == TR::Double
              ...
              int32_t ai_l =
                paramCursor
                  ->getAssignedLowGlobalRegisterIndex(); //  low reg of a pair
              TR_ASSERT((ai_l == (ai + 2)),
              ...
              if (dtype == TR::Int64 && TR::Compiler->target.is32Bit()) {
                ...
                if (fullLong) {
                  ...
                  cursor =
                    generateRXInstruction(self()->cg(),
                                          TR::InstOpCode::L,
                                          firstNode,
                                          self()->getRealRegister(REGNUM(ai)),
                                          generateS390MemoryReference(
                                            stackPtr, offset + 4, self()->cg()),
                                          (TR::Instruction*)cursor);
                }
  ...
  if (enableVectorLinkage) {
    for (i1 = lastFreeVectorArgIndex;
         i1 < self()->getNumVectorArgumentRegisters();
         i1++)
        ...
        switch (busyMoves[2][i1]) {
          ...
          case 1: // Int or Addr  Reg from Stack Slot
          {
            ...
            if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] ==
              ...
              cursor = generateRXInstruction(
                self()->cg(),
                (TR::InstOpCode::Mnemonic)busyMoves[3][i1],
                firstNode,
                self()->getRealRegister(REGNUM(target)),
                mr,
                (TR::Instruction*)cursor);
...
void
OMR::Z::Linkage::setParameterLinkageRegisterIndex(
  ...
  while ((paramCursor != NULL) &&
    ...
    if (self()->isFastLinkLinkageType()) {
      if ((numFloatArgs == 1) ||
        ...
        numFloatArgs =
          self()
            ->getNumFloatArgumentRegisters(); // no more float args possible now
      }
    }
  }
}
...
OMR::Z::Linkage::getOpCodeForLinkage(TR::Node* child,
                                     bool isStore,
                                     bool isRegReg)
{
...
TR::Register*
OMR::Z::Linkage::copyArgRegister(TR::Node* callNode,
                                 TR::Node* child,
                                 TR::Register* argRegister)
{
  ...
  if (TR::Compiler->target.is32Bit() && child->getDataType() == TR::Int64 &&
    ...
    cursor = generateRSInstruction(
      self()->cg(), TR::InstOpCode::SRLG, callNode, tempRegH, argRegister, 32);
    if (debugObj) {
      debugObj->addInstructionComment(toS390RSInstruction(cursor), REG_PARAM);
    }

    cursor = generateRRInstruction(
      self()->cg(), copyOpCode, callNode, tempRegL, argRegister);
    if (debugObj) {
    ...
    cursor = generateRRInstruction(
      self()->cg(), copyOpCode, callNode, tempRegister, argRegister);
...
TR::Register*
OMR::Z::Linkage::pushLongArg32(TR::Node* callNode,
                               TR::Node* child,
                               ...
                               TR::Register* argRegister)
{
  ...
  if (isStorePair || isStoreOnlyLow) {
    ...
    self()->storeArgumentOnStack(
      callNode, storeOp, storeReg, stackOffsetPtr, stackRegister);
 ...
 */
TR::Register*
OMR::Z::Linkage::getStackRegisterForOutgoingArguments(
  TR::Node* n,
  TR::RegisterDependencyConditions* dependencies)
{
...
TR::Register*
OMR::Z::Linkage::pushArg(TR::Node* callNode,
                         TR::Node* child,
                         int32_t numIntegerArgs,
                         int32_t numFloatArgs,
                         int32_t* stackOffsetPtr,
                         ...
                         TR::Register* argRegister)
{
  ...
  if (TR::Compiler->target.is32Bit() && argType.isInt64())
    return self()->pushLongArg32(callNode,
                                 child,
                                 numIntegerArgs,
                                 numFloatArgs,
                                 stackOffsetPtr,
  ...
  if (isStoreArg) {
    ...
    if (argRegister == NULL && child->getOpCode().isLoadConst() &&
      ...
      if (storeHelperImmediateInstruction(
            ...
            callNode,
            generateS390MemoryReference(
              stackRegister, *stackOffsetPtr, self()->cg()))) {
        argRegister = self()->cg()->evaluate(child);
    ...
    if (argRegister != NULL) {
      ...
      if (child->getDataType() == TR::DecimalLongDouble)
        self()->storeLongDoubleArgumentOnStack(callNode,
                                               child->getDataType(),
                                               storeOp,
                                               argRegister,
                                               stackOffsetPtr,
...
#endif
        self()->storeArgumentOnStack(
          callNode, storeOp, argRegister, stackOffsetPtr, stackRegister);
    }
  ...
  if (isFloatShadowedToIntRegs) {
    TR::Register* stackRegister = self()->getStackRegisterForOutgoingArguments(
      callNode, dependencies); // delay (possibly) creating this till needed
    self()->loadIntArgumentsFromStack(callNode,
                                      dependencies,
                                      argDataType,
                                      (*stackOffsetPtr) - argSize,
                                      argSize,
...
TR::Register*
OMR::Z::Linkage::pushJNIReferenceArg(
  TR::Node* callNode,
  TR::Node* child,
  int32_t numIntegerArgs,
  int32_t numFloatArgs,
  ...
  TR::Register* argRegister)
{
  ...
  if (isNull) {
    ...
    generateRIInstruction(self()->cg(),
                          TR::InstOpCode::getCmpHalfWordImmOpCode(),
                          child,
                          whatReg,
                          0);

    generateS390LabelInstruction(
      self()->cg(), TR::InstOpCode::LABEL, child, cFlowRegionStart);
    cFlowRegionStart->setStartInternalControlFlow();
    generateS390BranchInstruction(self()->cg(),
                                  TR::InstOpCode::BRC,
    ...
    generateS390LabelInstruction(
      self()->cg(), TR::InstOpCode::LABEL, child, nonNullLabel, conditions);
    nonNullLabel->setEndInternalControlFlow();

    self()->cg()->stopUsingRegister(whatReg);
  }

  return self()->pushArg(callNode,
                         child,
                         numIntegerArgs,
                         numFloatArgs,
                         stackOffsetPtr,
...
TR::Register*
OMR::Z::Linkage::pushVectorArg(TR::Node* callNode,
                               TR::Node* child,
...
TR::Register*
OMR::Z::Linkage::pushAggregateArg(
  TR::Node* callNode,
  TR::Node* child,
  int32_t numIntegerArgs,
  int32_t numFloatArgs,
 ...
 */
void
OMR::Z::Linkage::loadIntArgumentsFromStack(
  TR::Node* callNode,
  TR::RegisterDependencyConditions* dependencies,
  TR::DataType argType,
  int32_t stackOffset,
  ...
  TR::Register* stackRegister)
{
  ...
  if (argRegNum != TR::RealRegister::NoReg) {
    ...
    for (int32_t i = 0; i < regsNeeded; i++) {
      ...
      generateRXInstruction(
        self()->cg(), loadOp, callNode, argRegister, argMemRef);
 ...
 */
void
OMR::Z::Linkage::doNotKillSpecialRegsForBuildArgs(TR::Linkage* linkage,
                                                  bool isFastJNI,
                                                  int64_t& killMask)
{
  int32_t i;
                           ...
                           bool PassReceiver)
{
  ...
  // store env register
  stackOffset = self()->storeExtraEnvRegForBuildArgs(callNode,
                                                     self(),
                                                     dependencies,
                                                     isFastJNI,
                                                     stackOffset,
    ...
    switch (child->getDataType()) {
      case TR::Address:
        if (isFastJNI && child->getDataType() == TR::Address) {
          argRegister = self()->pushJNIReferenceArg(callNode,
                                                    child,
                                                    numIntegerArgs,
                                                    numFloatArgs,
                                                    &stackOffset,
      ...
      case TR::Int16:
      case TR::Int32:
        argRegister = self()->pushArg(callNode,
                                      child,
                                      numIntegerArgs,
                                      numFloatArgs,
                                      &stackOffset,
      ...
      case TR::Int64:
        if (TR::Compiler->target.is32Bit()) {
          argRegister = self()->pushLongArg32(callNode,
                                              child,
                                              numIntegerArgs,
                                              numFloatArgs,
                                              &stackOffset,
      ...
      case TR::Aggregate:
        if (self()->isAggregatesPassedInParmRegs() ||
          ...
          argRegister = self()->pushAggregateArg(callNode,
                                                 child,
                                                 numIntegerArgs,
                                                 numFloatArgs,
                                                 &stackOffset,
      ...
      case TR::VectorInt64:
      case TR::VectorDouble:
        argRegister = self()->pushVectorArg(
          callNode, child, numVectorArgs, i, &stackOffset, dependencies);
        numVectorArgs++;
        break;
    }
  ...
  switch (resDataType) {
    ...
    case TR::DecimalDouble:
      ...
      dependencies->addPostCondition(
        resultReg, self()->getFloatReturnRegister(), DefinesDependentRegister);
    ...
    case TR::VectorDouble:
      ...
      dependencies->addPostCondition(
        resultReg, self()->getVectorReturnRegister(), DefinesDependentRegister);
  ...
  for (i = TR::RealRegister::FirstGPR; i <= last; i++) {
    if ((killMask & (0x1L << REGINDEX(i)))) {
      dummyReg = NULL;
      self()->killAndAssignRegister(
        killMask, dependencies, &dummyReg, REGNUM(i), self()->cg(), true, true);
    }
  }
  ...
  if (enableVectorLinkage) {
    for (int32_t i = TR::RealRegister::FirstVRF; i <= TR::RealRegister::LastVRF;
      ...
      if (!self()->getPreserved(REGNUM(i))) {
        dummyReg = NULL;
        self()->killAndAssignRegister(killMask,
                                      dependencies,
                                      &dummyReg,
                                      REGNUM(i),
                                      self()->cg(),
...
void
OMR::Z::Linkage::replaceCallWithJumpInstruction(
  TR::Instruction* callInstruction)
{
  TR::InstOpCode::Mnemonic opCode = callInstruction->getOpCodeValue();
  TR_ASSERT(opCode == TR::InstOpCode::BASSM || opCode == TR::InstOpCode::BASR ||
  ...
  replacementInst =
    new (self()->trHeapMemory()) TR::S390RILInstruction(TR::InstOpCode::BRCL,
                                                        node,
                                                        (uint32_t)0xf,
                                                        callSymbol,
                                                        callSymRef,
  ...
  if (self()->comp()->getOption(TR_TraceCG))
    traceMsg(self()->comp(),
             "Replacing instruction %p to a jump %p !\n",
             callInstruction,
             replacementInst);
...
TR::Instruction*
OMR::Z::Linkage::storeArgumentOnStack(TR::Node* callNode,
                                      ...
                                      TR::Register* stackRegister)
{
  ...
  if (opCode == TR::InstOpCode::STM) {
    cursor =
      generateRSInstruction(self()->cg(), opCode, callNode, argReg, argMemRef);
  } else if (opCode == TR::InstOpCode::VST) {
    cursor =
      generateVRXInstruction(self()->cg(), opCode, callNode, argReg, argMemRef);
  } else {
    cursor =
      generateRXInstruction(self()->cg(), opCode, callNode, argReg, argMemRef);
  }
 ...
 */
TR::Instruction*
OMR::Z::Linkage::storeLongDoubleArgumentOnStack(TR::Node* callNode,
                                                TR::DataType argType,
                                                TR::InstOpCode::Mnemonic opCode,
                                                TR::Register* argReg,
                                                int32_t* stackOffsetPtr,
                                                TR::Register* stackRegister)
{
  ...
  generateRXInstruction(
    self()->cg(), opCode, callNode, argReg->getHighOrder(), argMemRef);
  argMemRef->stopUsingMemRefRegister(self()->cg());
  cursor = generateRXInstruction(
    self()->cg(), opCode, callNode, argReg->getLowOrder(), argMemRef2);
...
int64_t
OMR::Z::Linkage::killAndAssignRegister(int64_t killMask,
...
OMR::Z::Linkage::killAndAssignRegister(int64_t killMask,
                                       ...
                                       bool isDummy)
{
  return self()->killAndAssignRegister(killMask,
                                       deps,
                                       virtualRegPtr,
                                       realReg->getRegisterNumber(),
                                       codeGen,
                                       isAllocate,
                                       isDummy);
}
...
void
OMR::Z::Linkage::generateDispatchReturnLable(
  TR::Node* callNode,
  TR::CodeGenerator* codeGen,
  TR::RegisterDependencyConditions*& deps,
  ...
  TR::Node* GlobalRegDeps)
{
  ...
  TR::RegisterDependencyConditions* postDeps = new (self()->trHeapMemory())
    TR::RegisterDependencyConditions(NULL,
  ...
  generateS390LabelInstruction(
    codeGen, TR::InstOpCode::LABEL, callNode, endOfDirectToJNILabel, postDeps);
...
void
OMR::Z::Linkage::setupRegisterDepForLinkage(
  TR::Node* callNode,
  ...
  TR::Register*& JavaLitOffsetReg)
{
  ...
  if (!TR::Compiler->target.isZOS()) {
    ...
    killMask = self()->killAndAssignRegister(killMask,
                                             deps,
                                             &parm3VirtualRegister,
...
void
OMR::Z::Linkage::setupBuildArgForLinkage(TR::Node* callNode,
                                         TR_DispatchType dispatchType,
                                         TR::RegisterDependencyConditions* deps,
                                         bool isFastJNI,
                                         bool isPassReceiver,
                                         ...
                                         TR::SystemLinkage* systemLinkage)
{
  ...
  killMask = self()->killAndAssignRegister(killMask,
                                           deps,
                                           &systemReturnAddressVirtualRegister,
                                           systemReturnAddressRegister,
                                           self()->cg(),
                                           true);

  // Evaluate and load arguments to native function
  TR::Register* vftReg = NULL;
  int32_t argSize = systemLinkage->buildArgs(
    callNode, deps, isFastJNI, killMask, vftReg, isPassReceiver);
...
void
OMR::Z::Linkage::performCallNativeFunctionForLinkage(
  TR::Node* callNode,
  TR_DispatchType dispatchType,
  TR::Register*& javaReturnRegister,
  TR::SystemLinkage* systemLinkage,
  ...
  bool isJNIGCPoint)
{
  ...
  javaReturnRegister = systemLinkage->callNativeFunction(callNode,
                                                         deps,
                                                         targetAddress,
                                                         methodAddressReg,
                                                         javaLitOffsetReg,
                                     ...
                                     bool isJNIGCPoint)
{
  ...
  self()->setupRegisterDepForLinkage(callNode,
                                     dispatchType,
                                     deps,
                                     killMask,
                                     systemLinkage,
  ...
  /* Build Arguments               */
  /*********************************/
  self()->setupBuildArgForLinkage(callNode,
                                  dispatchType,
                                  deps,
                                  isFastJNI,
                                  isPassReceiver,
  ...
  /*********************************/
  self()->performCallNativeFunctionForLinkage(callNode,
                                              dispatchType,
                                              javaReturnRegister,
                                              systemLinkage,
                                              deps,
  ...
  /*********************************/
  self()->generateDispatchReturnLable(callNode,
                                      self()->cg(),
                                      deps,
                                      javaReturnRegister,
                                      hasGlRegDeps,
...
TR::Register*
OMR::Z::Linkage::buildSystemLinkageDispatch(TR::Node* callNode)
{
...
bool
OMR::Z::Linkage::needsAlignment(TR::DataType dt, TR::CodeGenerator* cg)
{
                                      ...
                                      TR::RealRegister::RegNum spRealReg)
{
  ...
  if (break64 || newNumUsedRegs == 1) {
    ...
    for (uint8_t i = 0; i < newNumUsedRegs; i++) {
      cursor = generateRXInstruction(self()->cg(),
                                     TR::InstOpCode::getLoadOpCode(),
                                     nextNode,
                                     self()->getRealRegister(REGNUM(curReg)),
                                     rsa,
                                     cursor);
  ...
  // 64-bit non-optimized CASE AND 32-bit shorter LMG case
    ...
    cursor =
      generateRSInstruction(self()->cg(),
                            TR::InstOpCode::getLoadMultipleOpCode(),
                            nextNode,
                            self()->getRealRegister(REGNUM(newFirstUsedReg)),
                            self()->getRealRegister(REGNUM(newLastUsedReg)),
                            rsa,
                            cursor);
  } else // 32-bit (try to use bumps)
  {
    ...
    rsa = generateS390MemoryReference(
      spReg, rsa->getOffset() + self()->machine()->getGPRSize(), self()->cg());
    cursor = generateRSInstruction(
      self()->cg(),
      TR::InstOpCode::getLoadMultipleOpCode(),
      nextNode,
...
int32_t
OMR::Z::Linkage::getFirstMaskedBit(int16_t mask, int32_t from, int32_t to)
{
...
int32_t
OMR::Z::Linkage::getLastMaskedBit(int16_t mask, int32_t from, int32_t to)
{
                                     ...
                                     TR_DispatchType dispatchType)
{
  ...
  return self()->buildNativeDispatch(
    callNode, dispatchType, deps, false, true, true);
}
...
void
OMR::Z::Linkage::setIntegerArgumentRegister(uint32_t index,
...
void
OMR::Z::Linkage::setFloatArgumentRegister(uint32_t index,
                                          TR::RealRegister::RegNum r)
{
...
void
OMR::Z::Linkage::setVectorArgumentRegister(uint32_t index,
                                           TR::RealRegister::RegNum r)
{
