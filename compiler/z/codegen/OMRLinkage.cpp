...
void OMR::Z::Linkage::removeOSCOnSavedArgument(TR::Instruction* instr, TR::Register* sReg, int32_t stackOffset)
{
    ...
    while ((current != NULL) && !current->isLabel() && !current->isCall() && !current->isBranchOp() && windowSize < maxWindowSize && !done) {
        if (self()->comp()->getOption(TR_TraceCG))
 ...
 * the prolog code.
 */
void* OMR::Z::Linkage::saveArguments(void* cursor, bool genBinary, bool InPreProlog, int32_t frameOffset, List<TR::ParameterSymbol>* parameterList)
{
    ...
    for (i1 = TR::RealRegister::FirstGPR; i1 <= lastReg; i1++) {
        ...
        if ((gpr6Test || !self()->getPreserved(REGNUM(i1))) && !self()->getIntegerArgument(REGNUM(i1)) && !self()->getFloatArgument(REGNUM(i1))) {
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (TR::Compiler->target.isLinux() && (lri > 0 && ai < 0 && (self()->getPreserved(REGNUM(lri + 3)) || dtype.isVector())))
        ...
        if (((self()->isSmallIntParmsAlignedRight() && paramCursor->getType().isIntegral()) || (self()->isPadFloatParms() && paramCursor->getType().isFloatingPoint())) && (gprSize > paramCursor->getSize())) {
        ...
        if (lri >= 0) {
            ...
            // XPLINK(STOREARGS)
            if (!paramCursor->isReferencedParameter() && !paramCursor->isParmHasToBeOnStack() && (!unconditionalSave || (unconditionalSave && dtype != TR::Address))) {
    ...
    while (freeMore && numMoves > 0) {
        ...
        for (i1 = 0; i1 < busyIndex; i1++) {
            ...
            if (!(target < 0) && (freeScratchable.isSet(target) || (source == target && busyMoves[2][i1] == 6) || (source == target && busyMoves[2][i1] == 7))) {
...
void OMR::Z::Linkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
    ...
    int32_t paramNum = -1;
    while ((paramCursor != NULL) && (numIntArgs < self()->getNumIntegerArgumentRegisters() || numFloatArgs < self()->getNumFloatArgumentRegisters() || numVectorArgs < self()->getNumVectorArgumentRegisters())) {
        int32_t index = -1;
        paramNum++;
...
OMR::Z::Linkage::pushArg(TR::Node* callNode, TR::Node* child, int32_t numIntegerArgs, int32_t numFloatArgs, int32_t* stackOffsetPtr, TR::RegisterDependencyConditions* dependencies, TR::Register* argRegister)
{
    ...
    if (isStoreArg) {
        ...
        if (argRegister == NULL && child->getOpCode().isLoadConst() && (storeOp == TR::InstOpCode::ST || storeOp == TR::InstOpCode::STG)) {
            TR::Register* stackRegister = self()->getStackRegisterForOutgoingArguments(callNode, dependencies); // delay (possibly) creating this till needed
 ...
 * canonical positions
 */
void OMR::Z::Linkage::loadIntArgumentsFromStack(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, TR::DataType argType, int32_t stackOffset, int32_t argSize, int32_t numIntegerArgs, TR::Register* stackRegister)
{
    ...
    TR::Register* javaReturnRegister, bool hasGlRegDeps, TR::Node* GlobalRegDeps)
{
    ...
    if (javaReturnRegister) {
        deps->stopUsingDepRegs(self()->cg(), javaReturnRegister->getRegisterPair() == NULL ? javaReturnRegister : javaReturnRegister->getHighOrder(), javaReturnRegister->getLowOrder());
    } else {
    ...
    TR::Node*& GlobalRegDeps, bool& hasGlRegDeps, TR::Register** methodAddressReg, TR::Register*& JavaLitOffsetReg)
{
    ...
    hasGlRegDeps = (callNode->getNumChildren() >= 1) && (callNode->getChild(callNode->getNumChildren() - 1)->getOpCodeValue() == TR::GlRegDeps);
...
void OMR::Z::Linkage::setupBuildArgForLinkage(TR::Node* callNode, TR_DispatchType dispatchType, TR::RegisterDependencyConditions* deps, bool isFastJNI,
...
void OMR::Z::Linkage::performCallNativeFunctionForLinkage(TR::Node* callNode, TR_DispatchType dispatchType, TR::Register*& javaReturnRegister, TR::SystemLinkage* systemLinkage,
...
{
    if ((index < _numIntegerArgumentRegisters - 1) || ((index == _numIntegerArgumentRegisters - 1) && self()->isSplitLongParm()))
