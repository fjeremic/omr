...
class InstOpCode : public OMR::InstOpCodeConnector {
 public:
  InstOpCode() : OMR::InstOpCodeConnector() {}

  InstOpCode(Mnemonic m) : OMR::InstOpCodeConnector(m) {}

  const char* getMnemonicName() const {
    return TR::InstOpCode::metadata[_mnemonic].name;
  }
  /* OMR TODO: Delegating method; need to be removed and replace with
   * getMnemonic */
...
inline TR::InstOpCode::S390BranchCondition getBranchConditionForMask(
    uint32_t branchMask) {
  switch (branchMask) {
    case 0:
...
inline uint8_t getMaskForBranchCondition(
    TR::InstOpCode::S390BranchCondition branchCond) {
  // TR_ASSERT(branchCond <= lastBranchCondition , "ERROR: unknown branch
  // condition\n");
  switch (branchCond) {
...
/**
 * Gives the reverse branch condition when the order of the 2 operands for a
 * compare operation is reversed.  For example "a > b" become "b < a", "a >= b"
 * becomes "b <= a" basically, this corresponds to flipping the 2 middle bits of
 * the branch condition mask and this method gives the corresponding branch
 * conditions to accomplish that.
 */
inline TR::InstOpCode::S390BranchCondition getReverseBranchCondition(
