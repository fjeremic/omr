...
class SymbolReferenceTable {
 public:
  ...
  enum CommonNonhelperSymbol {
    ...
    usesAllMethodSymbol,   // A dummy method whose use only alias set includes
                           // all
    ...
    gcrPatchPointSymbol,      // address where we need to patch GCR guard
                              // instruction
  ...
  void getAllSymRefs(BitVector &allSymRefs) {
    for (int32_t symRefNumber = getIndexOfFirstSymRef();
      ...
      if (symRef) allSymRefs[symRefNumber] = true;
    }
  }
  ...
  TR::SymbolReference *findOrCreateCodeGenInlinedHelper(
  ...
  TR::SymbolReference *findOrCreateInduceOSRSymbolRef(
      TR_RuntimeHelper induceOSRHelper);

  TR::ParameterSymbol *createParameterSymbol(
  ...
  TR::SymbolReference *findOrCreateAutoSymbol(
      TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t slot, TR::DataType,
  ...
  TR::SymbolReference *createTemporary(
      TR::ResolvedMethodSymbol *owningMethodSymbol, TR::DataType,
  ...
  TR::SymbolReference *findOrCreatePendingPushTemporary(
      TR::ResolvedMethodSymbol *owningMethodSymbol, int32_t stackDepth,
      TR::DataType, size_t size = 0);
  ...
  TR::SymbolReference *methodSymRefFromName(
      TR::ResolvedMethodSymbol *owningMethodSymbol, char *className,
      char *methodName, char *signature, TR::MethodSymbol::Kinds kind,
      int32_t cpIndex = -1);
  ...
  TR::SymbolReference *createKnownStaticDataSymbolRef(
      void *counterAddress, TR::DataType type,
      TR::KnownObjectTable::Index knownObjectIndex);
  ...
  TR::SymbolReference *findOrCreateAThrowSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  TR::SymbolReference *findOrCreateCheckCastSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  TR::SymbolReference *findOrCreateDivCheckSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  TR::SymbolReference *findOrCreateOverflowCheckSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  TR::SymbolReference *findOrCreateInstanceOfSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  ...
  // base recompilation
  TR::SymbolReference *findOrCreateRecompilationCounterSymbolRef(
      void *counterAddress);
  ...
  // FE, CG, optimizer
  TR::SymbolReference *findOrCreateNullCheckSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  ...
  TR::SymbolReference *findOrCreateNewArrayNoZeroInitSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  TR::SymbolReference *findOrCreateNewObjectSymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  ...
  TR::SymbolReference *findOrCreateArrayShadowSymbolRef(
      TR::DataType, TR::Node *baseAddress = 0);
  ...
  // fe, cg
  TR::SymbolReference *findOrCreateNewArraySymbolRef(
      TR::ResolvedMethodSymbol *owningMethodSymbol);
  ...
  TR::SymbolReference *findOrCreateVtableEntrySymbolRef(
  ...
  TR::SymbolReference *createGenericIntShadowSymbolReference(
      intptrj_t offset, bool allocateUseDefBitVector = false);
  TR::SymbolReference *findOrCreateGenericIntArrayShadowSymbolReference(
      intptrj_t offset);
  TR::SymbolReference *findOrCreateGenericIntNonArrayShadowSymbolReference(
      intptrj_t offset);
 ...
 protected:
   ...
   *
   */
  TR::SymbolReference *findSymRefWithKnownObject(
   ...
   */
  TR::SymbolReference *findTempSymRefWithKnownObject(
      TR::KnownObjectTable::Index knownObjectIndex);
  TR::SymbolReference *findOrCreateCPSymbol(
      TR::ResolvedMethodSymbol *, int32_t, TR::DataType, bool, void *,
      TR::KnownObjectTable::Index knownObjectIndex =
          TR::KnownObjectTable::UNKNOWN);
