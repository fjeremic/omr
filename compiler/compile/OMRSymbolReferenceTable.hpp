...
public:
  ...
  enum CommonNonhelperSymbol {
     ...
     * protection of OSR points. When the assumption becomes wrong, the execution of jitted code with the assumption has
     * to be transition to the VM interpreter before running the invalid code.
     *
     *  \code
  ...
  //
  int32_t getNumSymRefs()
  {
    return baseArray.size();
  }
  void setNumSymRefs(int32_t n)
  {
    baseArray.setSize(n);
  }
  ...
  //
  uint32_t getNumPredefinedSymbols()
  {
    return _numPredefinedSymbols;
  }

  int32_t getIndexOfFirstSymRef()
  {
    return 0;
  }
  ...
  int32_t getNumUnresolvedSymbols()
  {
    return _numUnresolvedSymbols;
  }
  int32_t getNonhelperIndex(CommonNonhelperSymbol s);
  int32_t getNumHelperSymbols()
  {
    return _numHelperSymbols;
  }
  ...
  TR::SymbolReference* getSymRef(int32_t i)
  {
    return baseArray.element(i);
  }
  ...
  TR::SymbolReference* findOrCreateRuntimeHelper(
    TR_RuntimeHelper index, bool canGCandReturn, bool canGCandExcept, bool preservesAllRegisters);
  ...
  TR::SymbolReference* findOrCreateAutoSymbol(TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t slot, TR::DataType,
    bool isReference = true, bool isInternalPointer = false, bool reuseAuto = true, bool isAdjunct = false,
    size_t size = 0);
  ...
  TR::Symbol* findGenericIntShadowSymbol()
  {
  ...
  {
    return _numInternalPointers;
  }

  TR::SymbolReference* methodSymRefFromName(TR::ResolvedMethodSymbol* owningMethodSymbol, char* className,
    char* methodName, char* signature, TR::MethodSymbol::Kinds kind, int32_t cpIndex = -1);
  ...
  void clearAvailableAutos()
  {
    _availableAutos.init();
  }
...
protected:
   ...
   * reference. Take a static field with known object index for example, \p symbol is the original static field symbol.
   *
