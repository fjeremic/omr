...
OMR::AliasBuilder::AliasBuilder(TR::SymbolReferenceTable *symRefTab,
                                size_t sizeHint, TR::Compilation *c)
    : _addressShadowSymRefs(sizeHint, c->trMemory(), heapAlloc, growable),
      ...
      _genericIntArrayShadowSymRefs(sizeHint, c->trMemory(), heapAlloc,
                                    growable),
      _genericIntNonArrayShadowSymRefs(sizeHint, c->trMemory(), heapAlloc,
                                       growable),
      _nonIntPrimitiveShadowSymRefs(sizeHint, c->trMemory(), heapAlloc,
                                    growable),
      _addressStaticSymRefs(sizeHint, c->trMemory(), heapAlloc, growable),
      _intStaticSymRefs(sizeHint, c->trMemory(), heapAlloc, growable),
      _nonIntPrimitiveStaticSymRefs(sizeHint, c->trMemory(), heapAlloc,
                                    growable),
      ...
      _refinedNonIntPrimitiveArrayShadows(1, c->trMemory(), heapAlloc,
                                          growable),
...
TR::AliasBuilder *OMR::AliasBuilder::self() {
  return static_cast<TR::AliasBuilder *>(this);
}

TR::SymbolReference *OMR::AliasBuilder::getSymRefForAliasing(
    TR::Node *node, TR::Node *addrChild) {
...
void OMR::AliasBuilder::updateSubSets(TR::SymbolReference *ref) {
  ...
  if (sym && sym->isMethod()) methodSymRefs().set(refNum);
}
...
void OMR::AliasBuilder::setVeryRefinedCallAliasSets(TR::ResolvedMethodSymbol *m,
                                                    TR_BitVector *bc) {
  _callAliases.add(new (trHeapMemory()) CallAliases(bc, m));
}

TR_BitVector *OMR::AliasBuilder::getVeryRefinedCallAliasSets(
    TR::ResolvedMethodSymbol *methodSymbol) {
  for (CallAliases *callAliases = _callAliases.getFirst(); callAliases;
       callAliases = callAliases->getNext())
...
void OMR::AliasBuilder::createAliasInfo() {
  ...
  defaultMethodDefAliases().init(symRefTab()->getNumSymRefs(),
                                 comp()->trMemory(), heapAlloc, growable);
  ...
  defaultMethodUseAliases().init(symRefTab()->getNumSymRefs(),
                                 comp()->trMemory(), heapAlloc, growable);
  ...
  if (symRefTab()->element(TR::SymbolReferenceTable::contiguousArraySizeSymbol))
    defaultMethodUseAliases().set(
        symRefTab()
            ->element(TR::SymbolReferenceTable::contiguousArraySizeSymbol)
            ->getReferenceNumber());
  if (symRefTab()->element(
          TR::SymbolReferenceTable::discontiguousArraySizeSymbol))
    defaultMethodUseAliases().set(
        symRefTab()
            ->element(TR::SymbolReferenceTable::discontiguousArraySizeSymbol)
            ->getReferenceNumber());
  if (symRefTab()->element(TR::SymbolReferenceTable::vftSymbol))
    defaultMethodUseAliases().set(
        symRefTab()
            ->element(TR::SymbolReferenceTable::vftSymbol)
            ->getReferenceNumber());

  methodsThatMayThrow().init(symRefTab()->getNumSymRefs(), comp()->trMemory(),
                             heapAlloc, growable);
...
void OMR::AliasBuilder::setCatchLocalUseSymRefs() {
  ...
  for (TR::CFGNode *node = comp()->getFlowGraph()->getFirstNode(); node;
       node = node->getNext()) {
    if (!node->getExceptionPredecessors().empty()) {
      bool isOSRCatch = false;
      if (node->asBlock()->isOSRCatchBlock()) isOSRCatch = true;
...
void OMR::AliasBuilder::gatherLocalUseInfo(
    TR::Block *block, TR_BitVector &storeVector,
    TR_ScratchList<TR_Pair<TR::Block, TR_BitVector> > *seenBlockInfos,
    vcount_t visitCount, bool isOSRCatch) {
  ...
  for (TR::CFGEdge *edge = edges.getFirst(); edge; edge = edges.getNext()) {
    ...
    if (((edge->getTo()->getPredecessors().size() == 1) && edge->getTo()->getExceptionPredecessors().empty()) /* ||
         ...
         {
      predBitVector = new (comp()->trStackMemory()) TR_BitVector(
          symRefTab()->getNumSymRefs(), comp()->trMemory(), stackAlloc);
      *predBitVector = storeVector;
    }

    TR_Pair<TR::Block, TR_BitVector> *pair = new (trStackMemory())
        TR_Pair<TR::Block, TR_BitVector>(toBlock(edge->getTo()), predBitVector);
    seenBlockInfos->add(pair);
  }
}

void OMR::AliasBuilder::gatherLocalUseInfo(TR::Block *catchBlock,
                                           bool isOSRCatch) {
  vcount_t visitCount = comp()->getVisitCount();
  TR_ScratchList<TR_Pair<TR::Block, TR_BitVector> > seenBlockInfos(trMemory());

  TR_Pair<TR::Block, TR_BitVector> *p =
      new (trStackMemory()) TR_Pair<TR::Block, TR_BitVector>(catchBlock, NULL);
  ...
  while (!seenBlockInfos.isEmpty()) {
    ...
    if (predStoreVector)
      gatherLocalUseInfo(block, *predStoreVector, &seenBlockInfos, visitCount,
                         isOSRCatch);
    else {
      TR_BitVector storeVector(symRefTab()->getNumSymRefs(), comp()->trMemory(),
                               stackAlloc);
      gatherLocalUseInfo(block, storeVector, &seenBlockInfos, visitCount,
                         isOSRCatch);
    }
  }
}

void OMR::AliasBuilder::gatherLocalUseInfo(TR::Node *node,
                                           TR_BitVector &storeVector,
                                           vcount_t visitCount,
                                           bool isOSRCatch) {
  ...
  if (symRef && symRef->getSymbol()->isAutoOrParm()) {
    ...
    if (node->getOpCode().isStoreDirect())
      ...
      if (!isOSRCatch) notOsrCatchLocalUseSymRefs().set(refNumber);
    }
  }
}

bool OMR::AliasBuilder::hasUseonlyAliasesOnlyDueToOSRCatchBlocks(
    TR::SymbolReference *symRef) {
...
void OMR::AliasBuilder::addNonIntPrimitiveArrayShadows(TR_BitVector *aliases) {
  if (supportArrayRefinement) *aliases |= refinedNonIntPrimitiveArrayShadows();
...
void OMR::AliasBuilder::addAddressArrayShadows(TR_BitVector *aliases) {
  if (supportArrayRefinement) *aliases |= refinedAddressArrayShadows();

  aliases->set(symRefTab()->getArrayShadowIndex(TR::Address));
}

void OMR::AliasBuilder::addIntArrayShadows(TR_BitVector *aliases) {
  if (supportArrayRefinement) *aliases |= refinedIntArrayShadows();
