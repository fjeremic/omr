...
OMR::AliasBuilder::methodAliases(TR::SymbolReference* symRef)
{
  if (comp()->getOption(TR_TraceAliases))
    traceMsg(comp(),
             "For method sym %d default aliases\n",
...
void
OMR::AliasBuilder::setVeryRefinedCallAliasSets(TR::ResolvedMethodSymbol* m,
                                               TR_BitVector* bc)
{
...
TR_BitVector*
OMR::AliasBuilder::getVeryRefinedCallAliasSets(
...
void
OMR::AliasBuilder::gatherLocalUseInfo(
  TR::Block* block,
...
OMR::AliasBuilder::gatherLocalUseInfo(TR::Block* catchBlock, bool isOSRCatch)
{
  ...
  while (!seenBlockInfos.isEmpty()) {
    ...
    if (predStoreVector)
      gatherLocalUseInfo(
        block, *predStoreVector, &seenBlockInfos, visitCount, isOSRCatch);
    else {
      TR_BitVector storeVector(
        symRefTab()->getNumSymRefs(), comp()->trMemory(), stackAlloc);
      gatherLocalUseInfo(
        block, storeVector, &seenBlockInfos, visitCount, isOSRCatch);
    }
  }
}

void
OMR::AliasBuilder::gatherLocalUseInfo(TR::Node* node,
                                      TR_BitVector& storeVector,
                                      vcount_t visitCount,
                                      bool isOSRCatch)
{
...
bool
OMR::AliasBuilder::hasUseonlyAliasesOnlyDueToOSRCatchBlocks(
  TR::SymbolReference* symRef)
{
