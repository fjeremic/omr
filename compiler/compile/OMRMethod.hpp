...
class mcount_t {
 ...
 public:
  ...
  mcount_t operator+(int32_t increment) const {
    return mcount_t(_value + increment);
  }

  uint32_t value() const { return _value; }
};

#define JITTED_METHOD_INDEX \
  (mcount_t::valueOf(       \
      (uint32_t)0))  // Index of the top-level method being compiled
#define MAX_CALLER_INDEX \
  (mcount_t::valueOf(    \
      (uint32_t)INT_MAX))  // Could be UINT_MAX, in theory, but let's avoid
                           // corner cases until that day comes when we need 3
                           // billion caller indexes
...
class TR_MethodParameterIterator {
 public:
  ...
  getOpaqueClass() = 0;        //  if getDataType() == TR::Aggregate
...
class Method {
 public:
  ...
  Method(Type t = J9) : _typeOfMethod(t) {
  ...
  virtual TR_MethodParameterIterator *getParameterIterator(
      TR::Compilation &, TR_ResolvedMethod * = NULL);

  // ---------------------------------------------------------------------------

  // Use this for optional logic that takes advantage of known information about
  // a particular method
  //
  ...
  TR::RecognizedMethod getMandatoryRecognizedMethod() {
    return _mandatoryRecognizedMethod;
  }

  void setRecognizedMethod(TR::RecognizedMethod rm) { _recognizedMethod = rm; }
  void setMandatoryRecognizedMethod(TR::RecognizedMethod rm) {
    _mandatoryRecognizedMethod = rm;
  }
