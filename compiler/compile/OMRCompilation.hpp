...
class OMR_EXTENSIBLE Compilation {
 ...
 public:
  ...
  TR::SymbolReference *getSymbolReferenceByReferenceNumber(
   ...
   */
  TR::DebugCounterBase *getCounterFromStaticAddress(
  ...
  TR_DevirtualizedCallInfo *findOrCreateDevirtualizedCall(
      TR::Node *, TR_OpaqueClassBlock *);
  ...
  TR::ResolvedMethodSymbol *createJittedMethodSymbol(
      TR_ResolvedMethod *resolvedMethod);
  ...
  TR_YesNoMaybe isCpuExpensiveCompilation(
      int64_t threshold);  // threshold in ns
 ...
 private:
  ...
  TR_RandomGenerator *_primaryRandom;  // Used to spawn other RandomGenerators
                                       // to keep nondeterminism contained
 ...
 private:
  ...
  ncount_t _lastValidNodeCount;  // set to _nodeCount when
                                 // _accurateNumberOfNodes is set. Used to tell
                                 // how many nodes have been created since
 ...
 private:
  ...
  const int32_t _compThreadID;  // The ID of the supporting compilation thread;
                                // 0 for compilation an application thread
