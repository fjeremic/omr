 ...
 * \brief Header for OMR::Compilation, root of the Compilation extensible class
 * hierarchy.
 */
...
#if defined(DEBUG)
// For a production build the body of of TR::Compilation::diagnostic is empty,
// so the call will be inlined and become a NOP.  The problem is that unless the
// compiler is very smart, it will continue to add the string (the first
// argument) to the dll.  The use of these macros should ensure that the string
// is not added to the dll.
//
#if defined(_MSC_VER)
...
#define performTransformation(comp, msg, ...)                                  \
  ((comp)->getOptimizer() ? (comp)->getOptimizer()->incOptMessageIndex() > 0   \
                          : true)
...
#define TRACE_OPT_DETAILS(comp)                                                \
  (comp)->getOptions()->getAnyOption(TR_TraceOptDetails |                      \
                                     TR_CountOptTransformations)

#define traceMsgVarArgs(comp, msg, arg)                                        \
  ((comp)->getDebug() ? (comp)->getDebug()->vtrace(msg, arg) : (void)0)

#if defined(_MSC_VER)
#define traceMsg(comp, msg, ...)                                               \
  ((comp)->getDebug() ? (comp)->getDebug()->trace(msg, __VA_ARGS__) : (void)0)
#define dumpOptDetails(comp, msg, ...)                                         \
  (TRACE_OPT_DETAILS(comp) ? (comp)->getDebug()->performTransformationImpl(    \
                                 false, msg, __VA_ARGS__)                      \
                           : true)
#define performTransformation(comp, msg, ...)                                  \
  (TRACE_OPT_DETAILS(comp)                                                     \
       ? (comp)->getDebug()->performTransformationImpl(true, msg, __VA_ARGS__) \
       : ((comp)->getOptimizer()                                               \
              ? (comp)->getOptimizer()->incOptMessageIndex() > 0               \
              : true))
#else // XLC or GCC
#define traceMsg(comp, msg, ...)                                               \
  ((comp)->getDebug() ? (comp)->getDebug()->trace(msg, ##__VA_ARGS__) : (void)0)
...
#define performTransformation(comp, msg, ...)                                  \
  (TRACE_OPT_DETAILS(comp)                                                     \
       ? (comp)->getDebug()->performTransformationImpl(true, msg,              \
                                                       ##__VA_ARGS__)          \
       : ((comp)->getOptimizer()                                               \
              ? (comp)->getOptimizer()->incOptMessageIndex() > 0               \
              : true))
...
#define performNodeTransformation0(comp, a)                                    \
  (comp->getOption(TR_TraceNodeFlags) ? performTransformation(comp, a) : true)
#define performNodeTransformation1(comp, a, b)                                 \
  (comp->getOption(TR_TraceNodeFlags) ? performTransformation(comp, a, b)      \
                                      : true)
#define performNodeTransformation2(comp, a, b, c)                              \
  (comp->getOption(TR_TraceNodeFlags) ? performTransformation(comp, a, b, c)   \
                                      : true)
#define performNodeTransformation3(comp, a, b, c, d)                           \
  (comp->getOption(TR_TraceNodeFlags)                                          \
       ? performTransformation(comp, a, b, c, d)                               \
       : true)
...
public:
  ...
  Compilation(int32_t compThreadId, OMR_VMThread *omrVMThread, TR_FrontEnd *,
              TR_ResolvedMethod *, TR::IlGenRequest &, TR::Options &,
              TR::Region &heapMemoryRegion, TR_Memory *,
              TR_OptimizationPlan *optimizationPlan);

  TR::SymbolReference *
  getSymbolReferenceByReferenceNumber(int32_t referenceNumber);
  ...
  TR_PersistentMemory *trPersistentMemory() {
    return _trMemory->trPersistentMemory();
  }
  ...
  TR::Allocator allocator(const char *name = NULL) {
    return TR::Allocator(_allocator);
  }
  ...
  bool suppressAllocationInlining() {
    return _options->getOption(TR_DisableAllocationInlining) ||
           _options->getOption(TR_OptimizeForSpace);
  }
  ...
  void setCurrentSymRefTab(TR::SymbolReferenceTable *t) {
    _currentSymRefTab = t;
  }
  ...
  /**
   * @brief Answers whether a call to the provided resolved method would
   * represent a recursive method call from the method currently being compiled.
   *
   * @param[in] targetResolvedMethod : the method symbol to check
   *
   * @return true if calling the targetResolvedMethod is a recursive call from
   * the method being compiled; false otherwise.
   */
  bool isRecursiveMethodTarget(TR_ResolvedMethod *targetResolvedMethod);

  /**
   * @brief Answers whether a call to the provided target symbol would represent
   * a recursive method call from the method currently being compiled.
   *
   * @param[in] targetSymbol : the method symbol to check
   *
   * @return true if calling the targetSymbol is a recursive call from the
  ...
  TR::list<TR::Instruction *> *getStaticHCRPICSites() {
    return &_staticHCRPICSites;
  }
  TR::list<TR::Instruction *> *getStaticMethodPICSites() {
    return &_staticMethodPICSites;
  }

  bool isPICSite(TR::Instruction *instruction);

  TR::list<TR::Snippet *> *getSnippetsToBePatchedOnClassUnload() {
    return &_snippetsToBePatchedOnClassUnload;
  }
  ...
  TR_RegisterCandidates *getGlobalRegisterCandidates() {
    return _globalRegisterCandidates;
  }
  void setGlobalRegisterCandidates(TR_RegisterCandidates *t) {
    _globalRegisterCandidates = t;
  }
  ...
  // P codegen
  TR::list<TR_PrefetchInfo *> &getExtraPrefetchInfo() {
    return _extraPrefetchInfo;
  }
  ...
  TR::list<TR_Pair<TR_ByteCodeInfo, TR::Node> *> &getCheckcastNullChkInfo() {
    return _checkcastNullChkInfo;
  }
  ...
  // J9
  bool getLoopWasVersionedWrtAsyncChecks() {
    return _loopVersionedWrtAsyncChecks;
  }
  void setLoopWasVersionedWrtAsyncChecks(bool v) {
    _loopVersionedWrtAsyncChecks = v;
  }
  ...
  bool incInlineDepth(TR::ResolvedMethodSymbol *, TR_ByteCodeInfo &,
                      int32_t cpIndex, TR::SymbolReference *callSymRef,
                      bool directCall, TR_PrexArgInfo *argInfo = 0);
  bool incInlineDepth(TR_OpaqueMethodBlock *, TR::ResolvedMethodSymbol *,
                      TR_ByteCodeInfo &, TR::SymbolReference *callSymRef,
                      bool directCall, TR_PrexArgInfo *argInfo = 0);
  ...
  public:
    TR_InlinedCallSiteInfo(TR_OpaqueMethodBlock *methodInfo,
                           ...
                           TR::SymbolReference *callSymRef, bool directCall)
        : _resolvedMethod(resolvedMethod), _callSymRef(callSymRef),
          _directCall(directCall), _osrCallSiteRematTable(0),
    ...
    TR_ResolvedMethod *resolvedMethod() {
      return _resolvedMethod->getResolvedMethod();
    }
    ...
    void setOSRCallSiteRematTable(int32_t *array) {
      _osrCallSiteRematTable = array;
    }
    bool directCall() { return _directCall; }
    bool cannotAttemptOSRDuring() { return _cannotAttemptOSRDuring; }
    void setCannotAttemptOSRDuring(bool cannotOSR) {
      _cannotAttemptOSRDuring = cannotOSR;
    }
  };
  ...
  void getOSRCallSiteRemat(uint32_t callSiteIndex, uint32_t slot,
                           TR::SymbolReference *&ppSymRef,
                           TR::SymbolReference *&loadSymRef);
  void setOSRCallSiteRemat(uint32_t callSiteIndex,
                           TR::SymbolReference *ppSymRef,
                           TR::SymbolReference *loadSymRef);
  ...
  void incNumLivePendingPushSlots(int32_t n) {
    _numLivePendingPushSlots = _numLivePendingPushSlots + n;
  }
  int32_t getNumLivePendingPushSlots() { return _numLivePendingPushSlots; }

  void incNumLoopNestingLevels(int32_t n) {
    _numNestingLevels = _numNestingLevels + n;
  }
  ...
  void setPeekingSymRefTab(TR::SymbolReferenceTable *symRefTab) {
    _peekingSymRefTab = symRefTab;
  }
  int32_t getMaxPeekedBytecodeSize() const {
    return TR::Options::getMaxPeekedBytecodeSize() >>
           (_optimizationPlan->getReduceMaxPeekedBytecode());
  }
  ...
  void dumpMethodTrees(char *title1, const char *title2,
                       TR::ResolvedMethodSymbol * = 0);
  void dumpFlowGraph(TR::CFG * = 0);

  bool getAddressEnumerationOption(TR_CompilationOptions o) {
    return _options->getAddressEnumerationOption(o);
  }

  TR::ILValidator *getILValidator() { return _ilValidator; }
  void setILValidator(TR::ILValidator *ilValidator) {
    _ilValidator = ilValidator;
  }
  ...
  /*
   * \brief
   *    Check to make sure the rdbars are anchored and anchor a rdbar if it's
   * found unanchored
   *
   * \note
   *    Ideally all optimizations should anchor a rdbar when it's created.
   *    This API is for fixing the unanchored rdbars in case some optimizations
   * forget to anchor the node by mistake. Optimizations should still try to
   * generate correct rdbar trees instead of relying on this API.
   */
  void verifyAndFixRdbarAnchors();

  void setIlVerifier(TR::IlVerifier *ilVerifier) { _ilVerifier = ilVerifier; }

  typedef std::pair<const void *const, TR::DebugCounterBase *>
      DebugCounterEntry;
  typedef TR::typed_allocator<DebugCounterEntry, TR::Allocator>
      DebugCounterMapAllocator;
  typedef std::map<const void *, TR::DebugCounterBase *,
                   std::less<const void *>, DebugCounterMapAllocator>
      DebugCounterMap;

  /**
   * @brief mapStaticAddressToCounter
   * @param symRef the symref containing the static address of the memory used
   * to store the count value of the debug counter
   * @param counter pointer to the debug counter
   *
   * Maps the staticAddress of the symRef to the debug counter. The map has to
   * be between the static address and the debug counter, instead of the symref
   * of the static address and the debug counter, because sometimes,
   * transformations can create a new symref that will not exist in this map;
   * however because the static address is constant, it is guaranteed to exist
   * in the map.
   */
  void mapStaticAddressToCounter(TR::SymbolReference *symRef,
   ...
   */
  TR::DebugCounterBase *
  getCounterFromStaticAddress(TR::SymbolReference *symRef);

#ifdef DEBUG
  void dumpMethodGraph(int index, TR::ResolvedMethodSymbol * = 0);
#endif

  int32_t getVerboseOptTransformationCount() {
    return _verboseOptTransformationCount;
  }
  void incVerboseOptTransformationCount(int32_t delta = 1) {
    _verboseOptTransformationCount += delta;
  }
  ...
  findOrCreateDevirtualizedCall(TR::Node *, TR_OpaqueClassBlock *);
  ...
  TR::Node *createDummyOrSideEffectGuard(TR::Compilation *, int16_t, TR::Node *,
                                         TR::TreeTop *);
  TR::Node *createSideEffectGuard(TR::Compilation *, TR::Node *, TR::TreeTop *);
  TR::Node *createAOTInliningGuard(TR::Compilation *, int16_t, TR::Node *,
                                   TR::TreeTop *, TR_VirtualGuardKind);
  TR::Node *createAOTGuard(TR::Compilation *, int16_t, TR::Node *,
                           TR::TreeTop *, TR_VirtualGuardKind);
  TR::Node *createDummyGuard(TR::Compilation *, int16_t, TR::Node *,
                             TR::TreeTop *);

  TR_LinkHead<TR_ClassLoadCheck> *getClassesThatShouldNotBeLoaded() {
    return &_classesThatShouldNotBeLoaded;
  }
  TR_LinkHead<TR_ClassExtendCheck> *getClassesThatShouldNotBeNewlyExtended() {
    return &_classesThatShouldNotBeNewlyExtended;
  }

  void setHasMethodOverrideAssumptions(bool v = true) {
    _flags.set(HasMethodOverrideAssumptions);
  }
  bool hasMethodOverrideAssumptions() {
    return _flags.testAny(HasMethodOverrideAssumptions);
  }

  void setHasClassExtendAssumptions(bool v = true) {
    _flags.set(HasClassExtendAssumptions);
  }
  bool hasClassExtendAssumptions() {
    return _flags.testAny(HasClassExtendAssumptions);
  }
  ...
  void setNextOptLevel(TR_Hotness nextOptLevel) {
    _nextOptLevel = nextOptLevel;
  }
  TR_Hotness getMethodHotness();
  static const char *getHotnessName(TR_Hotness t);
  const char *getHotnessName();

  template <typename Exception> void failCompilation(const char *format, ...) {
    char buffer[512];
  ...
  void setOptimizationPlan(TR_OptimizationPlan *optimizationPlan) {
  ...
  bool areSlotsSharedByRefAndNonRef() {
    return _flags.testAny(SlotsSharedByRefAndNonRef);
  }
  void setSlotsSharedByRefAndNonRef(bool b) {
    _flags.set(SlotsSharedByRefAndNonRef, b);
  }
  ...
  /*
   * Return true if the supplied method in the inlining table is known to not
   * run for very long and so not require asyncchecks or other yields
   */
  bool isShortRunningMethod(int32_t callerIndex);

  /*
   * isPotentialOSRPoint - used to check if a given treetop could be a point
   * where we would make an OSR transition from the JIT'd code to the
   * interpreter. Normal usage requires you to pass in a TreeTop and the
   * transition node will be distilled from that. Supplying a node directly is a
   * very special operation and is primarily intended for ILGen before blocks
   * are created - you risk incorrect answers if you fail to supply the TreeTop.
   *
  ...
  void setHasClassUnloadAssumptions(bool v = true) {
    _flags.set(HasClassUnloadAssumptions);
  }
  bool hasClassUnloadAssumptions() {
    return _flags.testAny(HasClassUnloadAssumptions);
  }

  void setHasClassRedefinitionAssumptions(bool v = true) {
    _flags.set(HasClassRedefinitionAssumptions);
  }
  bool hasClassRedefinitionAssumptions() {
    return _flags.testAny(HasClassRedefinitionAssumptions);
  }

  void setHasClassPreInitializeAssumptions(bool v = true) {
    _flags.set(HasClassPreInitializeAssumptions);
  }
  bool hasClassPreInitializeAssumptions() {
    return _flags.testAny(HasClassPreInitializeAssumptions);
  }

  void setUsesBlockFrequencyInGRA() { _flags.set(UsesBlockFrequencyInGRA); }
  bool getUsesBlockFrequencyInGRA() {
    return _flags.testAny(UsesBlockFrequencyInGRA);
  }
  ...
  bool getHasMethodHandleInvoke() {
    return _flags.testAny(HasMethodHandleInvoke);
  }
  ...
  createJittedMethodSymbol(TR_ResolvedMethod *resolvedMethod);
  ...
  // surely J9 specific
  void *getRelocatableMethodCodeStart() const {
    return _relocatableMethodCodeStart;
  }
  void setRelocatableMethodCodeStart(void *p) {
    _relocatableMethodCodeStart = p;
  }

  bool getFailCHTableCommit() const { return _failCHtableCommitFlag; }
  void setFailCHTableCommit(bool v) { _failCHtableCommitFlag = v; }

  TR_RandomGenerator &adhocRandom() {
    return *_adhocRandom;
  } // Not recommended if you care about reproducibility
  TR_RandomGenerator &primaryRandom() { return *_primaryRandom; }
  int32_t convertNonDeterministicInput(int32_t i, int32_t max,
                                       TR_RandomGenerator *randomGen = 0,
                                       int32_t min = 0,
                                       bool emitVerbose = true);

  int64_t getCpuTimeSpentInCompilation(); // coarse value (~0.5 sec
                                          // granularity). Result in ns
...
public:
  ...
  OMR::RuntimeAssumption **getMetadataAssumptionList() {
    return &_metadataAssumptionList;
  }
  void setMetadataAssumptionList(OMR::RuntimeAssumption *a) {
    _metadataAssumptionList = a;
  }
#endif
...
private:
  ...
  int16_t restoreInlineDepthUntil(int32_t stopIndex,
...
private:
  ...
  TR_RandomGenerator *_primaryRandom; // Used to spawn other RandomGenerators to
                                      // keep nondeterminism contained
  TR_RandomGenerator *_adhocRandom; // Used by callers who can't be bothered to
                                    // maintain their own TR_RandomGenerator
...
private:
  ...
  ncount_t
      _nodeCount; // _nodeCount is a global count of number of created nodes
  ncount_t _accurateNodeCount;  // this number is the current number of nodes in
                                // the trees. it can go stale
  ncount_t _lastValidNodeCount; // set to _nodeCount when _accurateNumberOfNodes
                                // is set. Used to tell how many nodes have been
                                // created since
  ...
  ToCommentMap
      _toCommentMap; // maps list of strings, etc. objects to list of strings

  /**
   * @brief _debugCounterMap
   *
   * A map between the static address of the memory used to store the count
   * value of the debug counter and the debug counter
   */
...
protected:
  ...
  OMR::RuntimeAssumption
      *_metadataAssumptionList; // A special OMR::RuntimeAssumption to play the
                                // role of a sentinel for a linked list
#endif

private:
  void *_relocatableMethodCodeStart;
  const int32_t _compThreadID; // The ID of the supporting compilation thread; 0
                               // for compilation an application thread
  ...
  // Length of the last node OpCode that was printed. This should get reset to 0
  // at the beginning of a line where it is needed, e.g. for printing
  // optimization trees it gets set to 0 in
  // TR_Debug::printBasicPreNodeInfoAndIndent() which preceeds node opCode
  // printouts. Any methods that print parts of node opCodes should always
  // increment this as there may be information following opCodes that need to
  // be aligned as in the case of trees or in future development.
  int32_t _nodeOpCodeLength;

  // Size of the previous symbol refernce table- used to keep track of symbols
  ...
  ListHeadAndTail<int32_t>
      _gpuKernelLineNumberList; // TODO: fix to get real line numbers
  int32_t _gpuPtxCount;

  BitVectorPool _bitVectorPool; // MUST be declared after _trMemory

  typedef TR::typed_allocator<
      std::pair<TR_OpaqueClassBlock *const, const TR::TypeLayout *>,
      TR::Region &>
      LayoutAllocator;
  typedef std::less<TR_OpaqueClassBlock *> LayoutComparator;
  typedef std::map<TR_OpaqueClassBlock *, const TR::TypeLayout *,
                   LayoutComparator, LayoutAllocator>
      TypeLayoutMap;
  TypeLayoutMap _typeLayoutMap;

  /*
   * This must be last
   * NOTE: TLS for Compilation needs to be set before any object that may use it
   * is initialized.
   */
