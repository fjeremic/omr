...
public:
  ...
  virtual TR::DataType
  parmType(uint32_t parmNumber); // returns the type of the parmNumber'th
                                 // parameter (0-based)
  ...
  virtual char *
  classNameChars(); // returns the utf8 of the class that this method is in.
  virtual char *nameChars();      // returns the utf8 of the method name
  virtual char *signatureChars(); // returns the utf8 of the signature

  virtual bool
  isConstructor(); // returns true if this method is object constructor.
  ...
  virtual bool isCold(TR::Compilation *, bool,
                      TR::ResolvedMethodSymbol *sym = NULL);
  ...
  getClassFromConstantPool(TR::Compilation *, uint32_t cpIndex,
                           bool returnClassForAot = false);
  virtual bool
  canAlwaysShareSymbolDespiteOwningMethod(TR_ResolvedMethod *other) {
    return false;
  }
  virtual char *getClassNameFromConstantPool(uint32_t cpIndex,
                                             uint32_t &length);
  ...
  virtual bool fieldsAreSame(int32_t, TR_ResolvedMethod *, int32_t,
                             bool &sigSame);
  virtual bool staticsAreSame(int32_t, TR_ResolvedMethod *, int32_t,
                              bool &sigSame);
  ...
  virtual bool fieldAttributes(TR::Compilation *, int32_t cpIndex,
                               uint32_t *fieldOffset, TR::DataType *type,
                               bool *volatileP, bool *isFinal, bool *isPrivate,
                               bool isStore, bool *unresolvedInCP = 0,
                               bool needAOTValidation = true);
  virtual bool staticAttributes(TR::Compilation *, int32_t cpIndex, void **,
                                TR::DataType *type, bool *volatileP,
                                bool *isFinal, bool *isPrivate, bool isStore,
  ...
  TR_OpaqueClassBlock *
  getClassFromFieldOrStatic(TR::Compilation *comp, int32_t cpIndex,
                            bool returnClassForAOT = false) {
    return getClassFromConstantPool(comp, classCPIndexOfFieldOrStatic(cpIndex),
                                    returnClassForAOT);
  }
  virtual TR_OpaqueClassBlock *
  getDeclaringClassFromFieldOrStatic(TR::Compilation *comp, int32_t cpIndex);

  // These functions actually return a complete description of the field/static
  virtual char *fieldName(int32_t cpIndex, TR_Memory *,
                          TR_AllocationKind kind = heapAlloc);
  virtual char *staticName(int32_t cpIndex, TR_Memory *,
                           TR_AllocationKind kind = heapAlloc);
  virtual char *localName(uint32_t slotNumber, uint32_t bcIndex, TR_Memory *);
  virtual char *fieldName(int32_t cpIndex, int32_t &len, TR_Memory *,
                          TR_AllocationKind kind = heapAlloc);
  virtual char *staticName(int32_t cpIndex, int32_t &len, TR_Memory *,
                           TR_AllocationKind kind = heapAlloc);
  virtual char *localName(uint32_t slotNumber, uint32_t bcIndex, int32_t &len,
                          TR_Memory *);
  ...
  virtual TR_OpaqueClassBlock *classOfStatic(int32_t cpIndex,
                                             bool returnClassForAOT = false);
  ...
  virtual TR_OpaqueClassBlock *
  getResolvedInterfaceMethod(int32_t cpIndex, uintptrj_t *pITableIndex);

  virtual TR_ResolvedMethod *getResolvedStaticMethod(TR::Compilation *,
                                                     int32_t cpIndex,
                                                     bool *unresolvedInCP = 0);
  virtual TR_ResolvedMethod *getResolvedSpecialMethod(TR::Compilation *,
                                                      int32_t cpIndex,
                                                      bool *unresolvedInCP = 0);
  virtual TR_ResolvedMethod *getResolvedDynamicMethod(TR::Compilation *,
                                                      int32_t cpIndex,
                                                      bool *unresolvedInCP = 0);
  virtual TR_ResolvedMethod *getResolvedHandleMethod(TR::Compilation *,
                                                     int32_t cpIndex,
                                                     bool *unresolvedInCP = 0);
  virtual TR_ResolvedMethod *
  getResolvedHandleMethodWithSignature(TR::Compilation *, int32_t cpIndex,
                                       char *signature);
  virtual TR_ResolvedMethod *
  getResolvedVirtualMethod(TR::Compilation *, int32_t cpIndex,
                           bool ignoreReResolve = true,
                           bool *unresolvedInCP = 0);

  virtual uint32_t
  getResolvedInterfaceMethodOffset(TR_OpaqueClassBlock *classObject,
                                   int32_t cpIndex);
  virtual TR_ResolvedMethod *
  getResolvedInterfaceMethod(TR::Compilation *,
                             TR_OpaqueClassBlock *classObject, int32_t cpIndex);
  virtual TR_ResolvedMethod *
  getResolvedVirtualMethod(TR::Compilation *, TR_OpaqueClassBlock *classObject,
  ...
  virtual int32_t exceptionData(int32_t exceptionNumber, int32_t *startIndex,
                                int32_t *endIndex, int32_t *catchType);
  ...
  // genMethodILForPeeking and _genMethodILForPeeking is an implementation of a
  // C++ pattern. Namely, we would like to have the same default arg for every
  // override of genMethodILForPeeking. Unfortunately, Consistently setting the
  // default arg for every override is error-prone, so what we do instead is to
  // create a non-virtual genMethodILForPeeking with a default arg that calls a
  // virtual counterpart. This approach allows us to mantain the def arg
  // invariant across all the overrides.
  TR::SymbolReferenceTable *
  genMethodILForPeeking(TR::ResolvedMethodSymbol *methodSymbol,
                        TR::Compilation *comp, bool resetVisitCount = false,
                        TR_PrexArgInfo *argInfo = NULL);

  /** \brief
   *     Generate IL for a method for peeking even under method redefinition
   * mode. To be called by optimizations that have protections against method
   * redefinition, e.g. inlining with TR_HCRGuard.
   *
   *  \param methodSymbol
   ...
   *
   *  \return
   *     The SymbolReferenceTable for the method to peek if peeking succeeds; 0
   * otherwise.
   */
  TR::SymbolReferenceTable *genMethodILForPeekingEvenUnderMethodRedefinition(
  ...
  virtual TR::SymbolReferenceTable *
  _genMethodILForPeeking(TR::ResolvedMethodSymbol *methodSymbol,
                         TR::Compilation *, bool resetVisitCount,
  ...
  /**
   * @brief Create TR::ParameterSymbols from the signature of a method, and add
   * them to the ParameterList on the ResolvedMethodSymbol.
   *
   * @param[in] methodSym : the ResolvedMethodSymbol to create the parameter
   * list for
   */
  virtual void makeParameterList(TR::ResolvedMethodSymbol *);

  virtual TR::ResolvedMethodSymbol *
  findOrCreateJittedMethodSymbol(TR::Compilation *comp);
};
