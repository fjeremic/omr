...
enum TR_Hotness
    ...
    : int8_t // use only 8 bits to save space; The ifdef is needed because xlC
             // BE compiler does not accept the syntax
#endif
{ noOpt,
  deadCold = noOpt,
  cold,
  ...
  unknownHotness,
  numHotnessLevels
}; // NOTE: if adding new hotnessLevels, corresponding names must be added to
   // *pHotnessNames[numHotnessLevels] table in OMRCompilation.cpp

// If adding new entries in TR_CallingContext, also add corresponding names in
// callingContextNames[] in J9Compilation.cpp
...
public:
  TR_DevirtualizedCallInfo(TR::Node *callNode, TR_OpaqueClassBlock *thisType)
      : _callNode(callNode), _thisType(thisType) {}
...
public:
  ...
  TR_PrefetchInfo(TR::TreeTop *defTree, TR::TreeTop *useTree,
                  TR::Node *addrNode, TR::Node *useNode, int32_t offset,
                  bool increasing)
      : _defTree(defTree), _useTree(useTree), _addrNode(addrNode),
        _useNode(useNode), _offset(offset), _increasing(increasing) {}
