...
{ noOpt,
  ...
  unknownHotness,
  numHotnessLevels }; // NOTE: if adding new hotnessLevels, corresponding names must be added to
...
public:
  ...
  TR_PrefetchInfo(
    TR::TreeTop* defTree, TR::TreeTop* useTree, TR::Node* addrNode, TR::Node* useNode, int32_t offset, bool increasing)
    : _defTree(defTree)
    , _useTree(useTree)
    , _addrNode(addrNode)
    , _useNode(useNode)
