...
TR::Node*
TR_VirtualGuard::createVftGuard(TR_VirtualGuardKind kind,
                                TR::Compilation* comp,
                                int16_t calleeIndex,
                                TR::Node* callNode,
                                ...
                                TR_OpaqueClassBlock* thisClass)
{
  ...
  TR::Node* vft =
    TR::Node::createWithSymRef(TR::aloadi,
                               1,
                               1,
...
TR::Node*
TR_VirtualGuard::createMethodGuard(TR_VirtualGuardKind kind,
                                   TR::Compilation* comp,
                                   int16_t calleeIndex,
                                   TR::Node* callNode,
                                   TR::TreeTop* destination,
                                   ...
                                   TR_OpaqueClassBlock* thisClass)
{
  return createMethodGuardWithReceiver(kind,
                                       comp,
                                       calleeIndex,
                                       callNode,
                                       destination,
  ...
  TR::ResolvedMethodSymbol* calleeSymbol)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    flagBit = TR::Node::create(
      callNode, TR::iconst, 0, comp->fej9()->offsetOfMethodIsBreakpointedBit());
 ...
 */
TR::Node*
TR_VirtualGuard::createBreakpointGuard(TR::Compilation* comp,
                                       int16_t calleeIndex,
                                       TR::Node* callNode,
                                       TR::TreeTop* destination,
                                       TR::ResolvedMethodSymbol* calleeSymbol)
{
  TR::Node* guard = createBreakpointGuardNode(
    comp, calleeIndex, callNode, destination, calleeSymbol);
  TR_VirtualGuard* vg = new (comp->trHeapMemory())
    TR_VirtualGuard(TR_FSDTest,
                    TR_BreakpointGuard,
                    comp,
                    callNode,
                    guard,
  ...
  bool forInline)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    TR::Node* flagBit = TR::Node::create(
      callNode, TR::iconst, 0, comp->fe()->offsetOfIsOverriddenBit());
  ...
  TR_VirtualGuard* vg = (TR_VirtualGuard*)(new (
    comp->trHeapMemory()) TR_VirtualGuard(TR_NonoverriddenTest,
                                          kind,
                                          comp,
                                          callNode,
                                          guard,
...
TR::Node*
TR_VirtualGuard::createMutableCallSiteTargetGuard(
  TR::Compilation* comp,
  int16_t calleeIndex,
  TR::Node* node,
  TR::TreeTop* destination,
...
TR::Node*
TR_VirtualGuard::createDummyOrSideEffectGuard(TR::Compilation* comp,
                                              TR::Node* callNode,
                                              TR::TreeTop* destination)
{
  TR::SymbolReferenceTable* symRefTab = comp->getSymRefTab();
...
TR::Node*
TR_VirtualGuard::createSideEffectGuard(TR::Compilation* comp,
                                       TR::Node* callNode,
                                       TR::TreeTop* destination)
{
  ...
  TR_VirtualGuard* vguard = new (comp->trHeapMemory())
    TR_VirtualGuard(TR_NonoverriddenTest,
                    TR_SideEffectGuard,
                    comp,
                    callNode,
                    guard,
...
TR::Node*
TR_VirtualGuard::createAOTInliningGuard(TR::Compilation* comp,
                                        int16_t calleeIndex,
                                        TR::Node* callNode,
                                        TR::TreeTop* destination,
                                        TR_VirtualGuardKind kind)
...
TR::Node*
TR_VirtualGuard::createAOTGuard(TR::Compilation* comp,
                                int16_t siteIndex,
                                TR::Node* callNode,
                                TR::TreeTop* destination,
                                TR_VirtualGuardKind kind)
...
TR::Node*
TR_VirtualGuard::createDummyGuard(TR::Compilation* comp,
                                  int16_t calleeIndex,
                                  TR::Node* callNode,
                                  TR::TreeTop* destination)
{
...
TR_VirtualGuard*
TR_VirtualGuard::createGuardedDevirtualizationGuard(TR_VirtualGuardKind kind,
                                                    TR::Compilation* comp,
                                                    TR::Node* callNode)
{
...
TR_VirtualGuard*
TR_VirtualGuard::createArrayStoreCheckGuard(TR::Compilation* comp,
                                            TR::Node* ascNode,
...
TR_VirtualGuard::createOSRGuard(TR::Compilation* comp, TR::TreeTop* destination)
{
  ...
  TR::SymbolReference* addressSymRef =
    symRefTab->createKnownStaticDataSymbolRef(0, TR::Int32);
  addressSymRef->setSideEffectInfo();
  TR::Node* load = destination
                     ? TR::Node::createWithSymRef(
                         destination->getNode(), TR::iload, 0, addressSymRef)
...
TR::Node*
TR_VirtualGuard::createHCRGuard(TR::Compilation* comp,
                                int16_t calleeIndex,
                                TR::Node* node,
                                TR::TreeTop* destination,
...
void
TR_VirtualGuard::addInnerAssumption(TR::Compilation* comp,
                                    int32_t ordinal,
...
void
TR_VirtualGuard::setGuardKind(TR::Node* node,
                              TR_VirtualGuardKind kind,
                              TR::Compilation* comp)
{
  switch (kind) {
