  ...
  TR::Node* callNode, TR::TreeTop* destination, TR_OpaqueClassBlock* thisClass)
{
  ...
  TR::Node* vft
    = TR::Node::createWithSymRef(TR::aloadi, 1, 1, callNode->getSecondChild(), symRefTab->findOrCreateVftSymbolRef());
  ...
  TR::Node* callNode, TR::TreeTop* destination, TR::ResolvedMethodSymbol* calleeSymbol, TR_OpaqueClassBlock* thisClass)
{
  return createMethodGuardWithReceiver(
    kind, comp, calleeIndex, callNode, destination, calleeSymbol, thisClass, callNode->getSecondChild());
}
  ...
  TR::TreeTop* destination, TR::ResolvedMethodSymbol* calleeSymbol)
{
  ...
  TR::Node* aconstNode
    = TR::Node::aconst(callNode, (uintptrj_t)calleeSymbol->getResolvedMethod()->getPersistentIdentifier());
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    guard = TR::Node::createif(TR::iflcmpne, TR::Node::create(TR::land, 2, constantPool, flagBit), zero, destination);
  } else {
    ...
    guard = TR::Node::createif(TR::ificmpne, TR::Node::create(TR::iand, 2, constantPool, flagBit), zero, destination);
  }
  ...
  TR_OpaqueClassBlock* thisClass, TR::Node* receiverNode)
{
  ...
  if (realThisClass && !TR::Compiler->cls.isInterfaceClass(comp, realThisClass)
    && callSymRef->getSymbol()->castToMethodSymbol()->isInterface())
    offset
      = callSymRef->getOwningMethod(comp)->getResolvedInterfaceMethodOffset(realThisClass, callSymRef->getCPIndex());
  else
  ...
  bool forInline)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    guard = TR::Node::createif(
      TR::iflcmpne, TR::Node::create(TR::land, 2, load, flagBit), TR::Node::create(callNode, TR::lconst), destination);
  } else {
    ...
    guard = TR::Node::createif(
      TR::ificmpne, TR::Node::create(TR::iand, 2, load, flagBit), TR::Node::create(callNode, TR::iconst), destination);
  }
  ...
  TR::TreeTop* destination, uintptrj_t* mcsObject, TR::KnownObjectTable::Index mcsEpoch)
{
  ...
  TR_VirtualGuard* vguard = new (comp->trHeapMemory())
    TR_VirtualGuard(TR_DummyTest, TR_MutableCallSiteTargetGuard, comp, node, guard, comp->getCurrentInlinedSiteIndex());
  ...
  TR::Compilation* comp, int16_t calleeIndex, TR::Node* callNode, TR::TreeTop* destination, TR_VirtualGuardKind kind)
{
  ...
  setGuardKind(guard, kind, comp);
  TR_VirtualGuard* vguard = new (comp->trHeapMemory())
    TR_VirtualGuard(TR_NonoverriddenTest, kind, comp, callNode, guard, calleeIndex, comp->getCurrentInlinedSiteIndex());
  vguard->dontGenerateChildrenCode();
...
TR::Node* TR_VirtualGuard::createOSRGuard(TR::Compilation* comp, TR::TreeTop* destination)
{
  ...
  TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(TR_DummyTest, TR_OSRGuard, comp, NULL, guard,
    destination ? destination->getNode()->getByteCodeInfo().getCallerIndex() : -1, comp->getCurrentInlinedSiteIndex(),
    NULL);
  vguard->dontGenerateChildrenCode();
  return guard;
}
  ...
  TR::TreeTop* destination, TR::ResolvedMethodSymbol* symbol, TR_OpaqueClassBlock* thisClass)
{
  ...
  TR_VirtualGuard* vguard = new (comp->trHeapMemory()) TR_VirtualGuard(
    TR_NonoverriddenTest, TR_HCRGuard, comp, node, guard, calleeIndex, comp->getCurrentInlinedSiteIndex(), thisClass);
  vguard->dontGenerateChildrenCode();
...
void TR_VirtualGuard::setGuardKind(TR::Node* node, TR_VirtualGuardKind kind, TR::Compilation* comp)
{
  switch (kind) {
  ...
  default:
    TR_ASSERT(
      kind == TR_NonoverriddenGuard, "Expected TR_NonoverriddenGuard(%d); found %d", (int)TR_NonoverriddenGuard, kind);
