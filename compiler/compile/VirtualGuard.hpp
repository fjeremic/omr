...
enum TR_VirtualGuardTestType {
  ...
  TR_FSDTest /**< used in debugging mode to test if a breakpoint is set for the
                inlined callee */
};
...
public:
  ...
  TR_VirtualGuard(TR_VirtualGuardTestType test, TR_VirtualGuardKind kind,
                  TR::Compilation *comp, TR::Node *callNode,
                  TR::Node *guardNode, int16_t calleeIndex,
                  int32_t currentSiteIndex, TR_OpaqueClassBlock *thisClass = 0);
  ...
  static TR::Node *createVftGuard(TR_VirtualGuardKind, TR::Compilation *,
                                  int16_t calleeIndex, TR::Node *,
                                  TR::TreeTop *, TR_OpaqueClassBlock *);

  static TR::Node *createMethodGuard(TR_VirtualGuardKind, TR::Compilation *,
                                     int16_t calleeIndex, TR::Node *,
                                     TR::TreeTop *, TR::ResolvedMethodSymbol *,
                                     TR_OpaqueClassBlock *thisClass);

  static TR::Node *createMethodGuardWithReceiver(
      TR_VirtualGuardKind, TR::Compilation *, int16_t calleeIndex, TR::Node *,
      TR::TreeTop *, TR::ResolvedMethodSymbol *, TR_OpaqueClassBlock *thisClass,
      TR::Node *receiverNode);

  static TR::Node *
  createNonoverriddenGuard(TR_VirtualGuardKind, TR::Compilation *,
                           int16_t calleeIndex, TR::Node *, TR::TreeTop *,
                           TR::ResolvedMethodSymbol *, bool forInling = true);

  static TR::Node *
  createMutableCallSiteTargetGuard(TR::Compilation *comp, int16_t calleeIndex,
                                   TR::Node *node, TR::TreeTop *destination,
                                   uintptrj_t *mcsObject,
                                   TR::KnownObjectTable::Index mcsEpoch);

  static TR::Node *createDummyOrSideEffectGuard(TR::Compilation *, TR::Node *,
                                                TR::TreeTop *);

  static TR::Node *createSideEffectGuard(TR::Compilation *, TR::Node *,
                                         TR::TreeTop *);

  static TR::Node *createAOTInliningGuard(TR::Compilation *, int16_t,
                                          TR::Node *, TR::TreeTop *,
                                          TR_VirtualGuardKind);

  static TR::Node *createAOTGuard(TR::Compilation *, int16_t, TR::Node *,
                                  TR::TreeTop *, TR_VirtualGuardKind);

  static TR::Node *createDummyGuard(TR::Compilation *, int16_t, TR::Node *,
                                    TR::TreeTop *);

  static TR_VirtualGuard *
  createGuardedDevirtualizationGuard(TR_VirtualGuardKind, TR::Compilation *,
                                     TR::Node *callNode);

  static TR_VirtualGuard *
  createArrayStoreCheckGuard(TR::Compilation *, TR::Node *node,
                             TR_OpaqueClassBlock *clazz);

  static TR::Node *createOSRGuard(TR::Compilation *, TR::TreeTop *destination);

  static TR::Node *createHCRGuard(TR::Compilation *comp, int16_t calleeIndex,
                                  TR::Node *node, TR::TreeTop *destination,
                                  TR::ResolvedMethodSymbol *symbol,
                                  TR_OpaqueClassBlock *thisClass);

  static TR::Node *
  createBreakpointGuard(TR::Compilation *comp, int16_t calleeIndex,
                        TR::Node *callNode, TR::TreeTop *destination,
                        TR::ResolvedMethodSymbol *calleeSymbol);
  static TR::Node *
  createBreakpointGuardNode(TR::Compilation *comp, int16_t calleeIndex,
                            TR::Node *callNode, TR::TreeTop *destination,
                            TR::ResolvedMethodSymbol *calleeSymbol);

  static void setGuardKind(TR::Node *guard, TR_VirtualGuardKind kind,
                           TR::Compilation *comp);
  ...
  uintptrj_t *mutableCallSiteObject() {
    TR_ASSERT(
        _kind == TR_MutableCallSiteTargetGuard,
  ...
  bool canBeRemoved(bool ignoreMerged = false) {
    return ignoreMerged ? !_cannotBeRemoved
                        : !(_cannotBeRemoved || _mergedWithHCRGuard ||
                            _mergedWithOSRGuard);
  }
  ...
  void addInnerAssumption(TR::Compilation *comp, int32_t ordinal,
  ...
  void setMergedWithHCRGuard(bool merged = true) {
    _mergedWithHCRGuard = merged;
  }
  bool mergedWithHCRGuard() { return _mergedWithHCRGuard; }

  void setMergedWithOSRGuard(bool merged = true) {
    _mergedWithOSRGuard = merged;
  }
  bool mergedWithOSRGuard() { return _mergedWithOSRGuard; }

  int32_t getCurrentInlinedSiteIndex() { return _currentInlinedSiteIndex; }
  void setCurrentInlinedSiteIndex(int32_t index) {
    _currentInlinedSiteIndex = index;
  }
