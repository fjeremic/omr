...
typedef TR::typed_allocator<std::pair<const int32_t, TR_BitVector *>,
                            TR::Region &>
    DefiningMapAllocator;
typedef std::less<int32_t> DefiningMapComparator;
typedef std::map<int32_t, TR_BitVector *, DefiningMapComparator,
                 DefiningMapAllocator>
    DefiningMap;
...
public:
  ...
  void addSlotInfo(int32_t slot, int32_t symRefNum, int32_t symRefOrder,
                   int32_t symSize, bool takesTwoSlots);
  class TR_SlotInfo {
  public:
    TR_SlotInfo(int32_t _slot, int32_t _symRefNum, int32_t _symRefOrder,
                int32_t _symSize, bool _takesTwoSlots)
        : slot(_slot), symRefNum(_symRefNum), symRefOrder(_symRefOrder),
          symSize(_symSize), takesTwoSlots(_takesTwoSlots){};
    int32_t slot;
    int32_t symRefNum;
    int32_t symRefOrder;
    int32_t symSize;
  ...
  friend TR::Compilation &operator<<(TR::Compilation &out,
                                     const TR_OSRSlotSharingInfo *);
 ...
 *    instruction PC offset and a set of symbols that share slot(s), which
 * symbol is live at that offset. This is needed so that we can popluate the OSR
 * buffer.
 *
 * 2. Write the information in (1) into the meta data.
 *
 * 3. Check that size of OSR frame, OSR scratch buffer, and OSR stack frame do
 ...
 * --------------
 *
 * There is one OSR code block for the root method and one for each of the
 * inlined methods.  It's a basic block that initially (i.e., during ILGen)
 * contains a call to `prepareForOSR` which has all the autos and pending push
 * temps of its corresponding method as its children. Eventually, i.e., in
 * `CodeGenPrep.cpp`, the call will be replaced by
 *
 * 1. A number of treetops that store all symbols that do not share slots to the
 *    OSR buffer, and a number of treetops that store all symbols that share
 * slots to the OSR scratch buffer.
 *
 * 2. A call to a C helper called `_prepareForOSR` (defined in
 *    runtime/CRuntimeImpl.cpp). This helper uses meta data information to find
 * out, given a instruction PC offset and a set of symbols that share slot(s),
 * the symbol that is live at that offset. The symbol is read from the OSR
 * scratch buffer and written to the OSR buffer.
 *
 * During inlining, we link OSR code blocks in the sense that, each OSR code
 * block will have a goto to the OSR code block of its caller. The OSR code
 * block of the root method will have a jump to the VM.
 *
 * `symRefOrder`
 * -------------
 *
 * Each inlined method (including the root method), has two arrays:
 * one for its autos and one for its pending push temps. Each element of the
 * array is a sequence of symrefs. If a symref doesn't share its slot with
 * another symref, the sequence contains only one element, otherwise, the
 * sequence contains the list of all symrefs that share the same slot(s).
 *
...
public:
  ...
  const TR_Array<TR_OSRMethodData *> &getOSRMethodDataArray() const {
    return osrMethodDataArray;
  }
  TR_OSRMethodData *findOSRMethodData(int32_t inlinedSiteIndex,
                                      TR::ResolvedMethodSymbol *methodSymbol);
  TR_OSRMethodData *
  findOrCreateOSRMethodData(int32_t inlinedSiteIndex,
                            TR::ResolvedMethodSymbol *methodSymbol);
  TR_OSRMethodData *findCallerOSRMethodData(TR_OSRMethodData *callee);
  void addSlotSharingInfo(const TR_ByteCodeInfo &T, int32_t slot,
                          int32_t symRefNum, int32_t symRefOrder,
                          int32_t symSize, bool takesTwoSlots);
  ...
  void setSeenClassPreventingInducedOSR() {
    _classPreventingInducedOSRSeen = true;
  }
  bool seenClassPreventingInducedOSR() {
    return _classPreventingInducedOSRSeen;
  }

  int32_t getNumOfSymsThatShareSlot() { return numOfSymsThatShareSlot; }
  void updateNumOfSymsThatShareSlot(int32_t value) {
    numOfSymsThatShareSlot += value;
  }
  ...
  void buildFinalMap(int32_t callerIndex, DefiningMap *finalMap,
  ...
  /**
   * This class is basically a mapping from a given instruction PC offset to
   * information that that enables to copy values of symbols that share slot(s)
   * from OSR scratch buffer to OSR buffer. This information is deduced from
   * live symbols at that instruction pc. More formally, it represents a pair
   * (i, s) where i is an instruction PC and s is an array of
   * `TR_ScratchBufferInfo`.
   */
  class TR_Instruction2SharedSlotMapEntry {
  public:
    TR_Instruction2SharedSlotMapEntry(
        int32_t _instructionPC,
        const TR_ScratchBufferInfos &_scratchBufferInfos)
        : instructionPC(_instructionPC),
          scratchBufferInfos(_scratchBufferInfos) {}
    TR_Instruction2SharedSlotMapEntry(
        const TR_Instruction2SharedSlotMapEntry &entry)
        : instructionPC(entry.instructionPC),
          scratchBufferInfos(entry.scratchBufferInfos) {}
    TR_Instruction2SharedSlotMapEntry &
    operator=(const TR_Instruction2SharedSlotMapEntry &entry) {
      instructionPC = entry.instructionPC;
      scratchBufferInfos = entry.scratchBufferInfos;
      return *this;
    }
  ...
  void addInstruction2SharedSlotMapEntry(int32_t instructionPC,
                                         const TR_ScratchBufferInfos &infos);
  void compressInstruction2SharedSlotMap();

  friend TR::Compilation &operator<<(TR::Compilation &,
                                     const TR_OSRCompilationData &);

private:
  typedef TR::deque<TR_Instruction2SharedSlotMapEntry, TR::Region &>
      TR_Instruction2SharedSlotMap;
  ...
  void
  buildSymRefOrderMapAux(TR_Array<List<TR::SymbolReference>> *symListArray);
  TR_SymRefOrderMap symRefOrderMap;
  TR_Instruction2SharedSlotMap instruction2SharedSlotMap;
  // an array of TR_OSRMethodData, one element for each inlined method and one
  // for the root method
...
public:
  ...
  TR_OSRMethodData(int32_t _inlinedSiteIndex,
  ...
  int32_t getNumOfSymsThatShareSlot() { return numOfSymsThatShareSlot; }
  void addSlotSharingInfo(int32_t byteCodeIndex, int32_t slot,
                          int32_t symRefNum, int32_t symRefOrder,
                          int32_t symSize, bool takesTwoSlots);
  ...
  int32_t slotIndex2OSRBufferIndex(int32_t slotIndex, int symSize,
                                   bool takesTwoSlots) const;
  void addScratchBufferOffset(int32_t slotIndex, int32_t symRefOrder,
                              int32_t scratchBufferOffset);
  ...
  void addPendingPushLivenessInfo(int32_t byteCodeIndex,
  ...
  void buildDefiningMap(TR::Block *block, DefiningMap *blockDefiningMap,
                        DefiningMap *prepareForOSRCallMap = NULL);
  ...
  void collectSubTreeSymRefs(TR::Node *node, TR_BitVector *subTreeSymRefs,
...
private:
  ...
  typedef CS2::HashTable<int32_t, TR_BitVector *, TR::Allocator>
      TR_BCLiveRangeInfoHashTable;
  typedef CS2::HashTable<int32_t, TR_OSRSlotSharingInfo *, TR::Allocator>
      TR_BCInfoHashTable;
  typedef CS2::HashTable<int32_t, TR_Array<int32_t> *, TR::Allocator>
      TR_ArgInfoHashTable;
  typedef CS2::HashTable<int32_t, TR_Array<int32_t>, TR::Allocator>
      TR_Slot2ScratchBufferOffset;

  /// method symbol corresponding to this method
  TR::ResolvedMethodSymbol
      *methodSymbol; // NOTE: This field must appear first, because its comp()
                     // is used to initialize other fields

  /// a mapping from shared slot indices to an array of offsets in the scratch
  /// buffer. For each shared slot, there is a set of two or more syms that
  /// share that slot. The elements of the array above is the same as this set.
  /// The elements of the array are indexed by their symRefOrder.
  ...
  /// This is an empty catch block right before the osr code block, i.e., it
  /// falls through to the osr code block. All incoming exception edges (from
  /// normal blocks) should go to the osr catch block and all incoming normal
  /// edges (from callees code blocks) should go to the osr code block.
...
public:
  ...
  TR_OSRPoint(TR_ByteCodeInfo &bcInfo, TR_OSRMethodData *methodData,
              TR_Memory *m);
...
TR::Compilation &operator<<(TR::Compilation &out,
                            const TR_OSRCompilationData &osrData);
