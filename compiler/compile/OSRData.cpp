    ...
    int32_t slot, int32_t symRefNum, int32_t symRefOrder, int32_t symSize, bool takesTwoSlots)
{
    ...
    getOSRMethodDataArray()[bcinfo.getCallerIndex() + 1]->addSlotSharingInfo(bcinfo.getByteCodeIndex(), slot, symRefNum, symRefOrder, symSize, takesTwoSlots);
}
...
TR_OSRCompilationData::findOSRMethodData(int32_t inlinedSiteIndex, TR::ResolvedMethodSymbol* methodSymbol)
{
    ...
    if (osrMethodData && osrMethodData->getInlinedSiteIndex() == inlinedSiteIndex && osrMethodData->getMethodSymbol() == methodSymbol)
 ...
 */
void TR_OSRCompilationData::buildFinalMap(int32_t callerIndex,
 ...
 */
void TR_OSRMethodData::buildDefiningMapForOSRCodeBlockAndPrepareForOSRCall(TR::Block* block, DefiningMap* osrCodeBlockMap, DefiningMap* prepareForOSRCallMap)
{
...
void TR_OSRMethodData::buildDefiningMap(TR::Block* block, DefiningMap* blockMap, DefiningMap* prepareForOSRCallMap)
{
    for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextRealTreeTop()) {
        ...
        } else if (node->getFirstChild() && node->getFirstChild()->getOpCode().isCall() && node->getFirstChild()->getSymbolReference()->getReferenceNumber() == TR_prepareForOSR) {
...
void TR_OSRMethodData::addSlotSharingInfo(int32_t byteCodeIndex, int32_t slot, int32_t symRefNum,
...
TR_OSRMethodData::slotIndex2OSRBufferIndex(int32_t slotIndex, int symSize, bool takesTwoSlots) const
{
    ...
    if (slotIndex < 0)
        ...
        return headerSize + (getMethodSymbol()->getNumPPSlots() + getMethodSymbol()->getResolvedMethod()->numberOfTemps() + numOfSyncObjects + getMethodSymbol()->getNumParameterSlots() - slotIndex - 1 + (takesTwoSlots ? -1 : 0)) * TR::Compiler->om.sizeofReferenceAddress();
}
...
{
    int32_t numOfSyncObjects = (getMethodSymbol()->getSyncObjectTemp() != NULL) ? 1 : 0;
    return getMethodSymbol()->getNumPPSlots() + getMethodSymbol()->getResolvedMethod()->numberOfTemps() + numOfSyncObjects + getMethodSymbol()->getNumParameterSlots();
}
...
void TR_OSRSlotSharingInfo::addSlotInfo(int32_t slot, int32_t symRefNum, int32_t symRefOrder, int32_t symSize, bool takesTwoSlots)
{
    ...
    for (int i = 0; i < slotInfos.size(); i++) {
        ...
        if (info.symRefNum != -1) {
            if ((info.slot == slot) && (info.symRefNum != symRefNum) && (symRefTab->getSymRef(info.symRefNum)->getSymbol()->getDataType() == symRefTab->getSymRef(symRefNum)->getSymbol()->getDataType())) {
...
{
    return inlinedSiteIndex == info.inlinedSiteIndex && osrBufferOffset == info.osrBufferOffset && scratchBufferOffset == info.scratchBufferOffset && symSize == info.symSize;
}
