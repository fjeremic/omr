...
void
TR_OSRCompilationData::addInstruction(int32_t instructionPC,
                                      TR_ByteCodeInfo bcInfo)
{
  bool trace = comp->getOption(TR_TraceOSR);
  if (trace)
    traceMsg(comp,
             "instructionPC %x callerIndex %d bcidx %d ",
             instructionPC,
...
TR_OSRMethodData*
TR_OSRCompilationData::findOSRMethodData(int32_t inlinedSiteIndex,
...
TR_OSRMethodData*
TR_OSRCompilationData::findOrCreateOSRMethodData(
  ...
  *((int32_t*)buffer) = numberOfMappings;
  ...
  for (int i = 0; i < numberOfMappings; i++) {
    ...
    if (osrMethodData == NULL || osrMethodData->getOSRCodeBlock() == NULL)
      ...
      *((int32_t*)buffer) =
        osrMethodData->getOSRCatchBlock()->getInstructionBoundaries()._startPC;
    buffer += sizeof(int32_t);
  }
  uint32_t numberOfBytesWritten = buffer - initialBuffer;
  TR_ASSERT(numberOfBytesWritten == sectionSize,
...
///
void
TR_OSRCompilationData::checkOSRLimits()
{
 ...
 */
uint32_t
TR_OSRCompilationData::getOSRStackFrameSize(uint32_t methodIndex)
{
...
static void
printMap(DefiningMap* map, TR::Compilation* comp)
{
...
void
TR_OSRCompilationData::buildDefiningMap()
{
  ...
  for (intptrj_t i = 0; i < methodDataArray.size(); ++i) {
    ...
    if (!osrCatchBlockRemoved && !osrCodeBlockRemoved)
      buildFinalMap(i - 1,
                    osrMethodData->getDefiningMap(),
                    definingMapAtOSRCatchBlocks[i],
                    definingMapAtOSRCodeBlocks,
  ...
  if (comp->getOption(TR_TraceOSR)) {
    for (intptrj_t i = 0; i < methodDataArray.size(); ++i) {
      ...
      if (osrMethodData->getOSRCatchBlock()) {
        traceMsg(comp,
                 "final map for OSRCatchBlock(block_%d): \n",
 ...
 */
static void
updateDefiningSymRefs(TR_BitVector* subTreeSymRefs,
 ...
 */
static void
mergeDefiningMaps(DefiningMap* firstMap,
 ...
 */
void
TR_OSRCompilationData::buildFinalMap(
  int32_t callerIndex,
  ...
  DefiningMaps& definingMapAtPrepareForOSRCalls)
{
  ...
  for (auto entry = prepareForOSRCallMap->begin();
       entry != prepareForOSRCallMap->end();
       ++entry) {
    ...
    if (comp->getOption(TR_TraceOSR)) {
      traceMsg(comp,
               "adding symRef #%d and its defining symbols to finalMap\n",
               symRefNum);
      result->print(comp);
      traceMsg(comp, "\n");
    }
  ...
  buildFinalMap(nextCallerIndex,
                finalMap,
                workingCatchBlockMap,
                definingMapAtOSRCodeBlocks,
                definingMapAtPrepareForOSRCalls);
}
 ...
 */
void
TR_OSRMethodData::buildDefiningMapForOSRCodeBlockAndPrepareForOSRCall(
  TR::Block* block,
 ...
 */
void
TR_OSRMethodData::buildDefiningMapForBlock(TR::Block* block,
 ...
 */
void
TR_OSRMethodData::buildDefiningMap(TR::Block* block,
                                   DefiningMap* blockMap,
                                   DefiningMap* prepareForOSRCallMap)
{
  for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit();
    ...
    if (symRef) {
      if (symRef->getSymbol()->isAutoOrParm()) {
        ...
        collectSubTreeSymRefs(
          node->getFirstChild(), &subTreeSymRefs, checklist);

        if (comp()->getOption(TR_TraceOSR)) {
          traceMsg(comp(),
                   "buildDefiningMap: node n%dn: defining symbol #%d: ",
                   node->getGlobalIndex(),
      ...
      for (int32_t child = firstSymChildIndex;
           child + 2 < callNode->getNumChildren();
           child += 3) {
        ...
        if (comp()->getOption(TR_TraceOSR)) {
          traceMsg(
            comp(),
            "collect subTreeSymRefs of loadNode n%dn for original symRef #%d\n",
            loadNode->getGlobalIndex(),
            symRefNumber);
          subTreeSymRefs.print(comp());
          traceMsg(comp(), "\n");
        }
 ...
 */
void
TR_OSRMethodData::collectSubTreeSymRefs(TR::Node* node,
...
void
TR_OSRCompilationData::buildSymRefOrderMap()
{
...
void
TR_OSRCompilationData::buildSymRefOrderMapAux(
...
int32_t
TR_OSRCompilationData::getSymRefOrder(int32_t symRefNumber)
{
...
TR_OSRMethodData::createOSRBlocks(TR::Node* n)
{
  ...
  osrCatchBlock->setHandlerInfoWithOutBCInfo(
    ...
    comp()->getInlineDepth(),
    -1,
    getMethodSymbol()->getResolvedMethod(),
    comp());
...
void
TR_OSRMethodData::addLiveRangeInfo(int32_t byteCodeIndex,
 ...
 */
void
TR_OSRMethodData::addArgInfo(int32_t byteCodeIndex,
                             int32_t argIndex,
                             int32_t argSymRef)
{
  CS2::HashIndex hashIndex;
 ...
 */
void
TR_OSRMethodData::addPendingPushLivenessInfo(int32_t byteCodeIndex,
...
void
TR_OSRMethodData::addSlotSharingInfo(int32_t byteCodeIndex,
                                     int32_t slot,
                                     int32_t symRefNum,
                                     int32_t symRefOrder,
                                     int32_t symSize,
...
void
TR_OSRMethodData::addScratchBufferOffset(int32_t slotIndex,
                                         int32_t symRefOrder,
...
TR_OSRMethodData::addInstruction(int32_t instructionPC, int32_t byteCodeIndex)
{
  ...
  if (bcInfoHashTab.Locate(byteCodeIndex, hashIndex)) {
    ...
    for (int i = 0; i < slotInfos.size(); i++) {
      ...
      if (slotInfos[i].symRefOrder == -1) {
        TR_ASSERT(slotInfos[i].symRefNum == -1,
                  ...
                  "actually %d\n",
                  i,
                  slotInfos[i].symRefOrder,
                  slotInfos[i].symRefNum);
        scratchBufferOffset = -1;
      } else
        scratchBufferOffset =
          slot2ScratchBufferOffset.DataAt(hashIndex)[slotInfos[i].symRefOrder];
      int32_t osrBufferOffset = slotIndex2OSRBufferIndex(
        slotInfos[i].slot, slotInfos[i].symSize, slotInfos[i].takesTwoSlots);
      info.add(
        TR_OSRCompilationData::TR_ScratchBufferInfo(getInlinedSiteIndex(),
                                                    osrBufferOffset,
                                                    scratchBufferOffset,
                                                    slotInfos[i].symSize));
      if (trace)
    ...
    if (trace)
      traceMsg(comp(),
               "  rejected: byteCodeIndex %d is not an OSR point\n",
               byteCodeIndex);
  }
}

TR::Compilation*
TR_OSRMethodData::comp() const
{
...
int32_t
TR_OSRMethodData::slotIndex2OSRBufferIndex(int32_t slotIndex,
                                           int symSize,
...
TR::Compilation&
operator<<(TR::Compilation& out, const TR_OSRMethodData& osrMethodData)
{
  bool first = true;
...
void
TR_OSRSlotSharingInfo::addSlotInfo(int32_t slot,
                                   int32_t symRefNum,
                                   int32_t symRefOrder,
                                   int32_t symSize,
                                   bool takesTwoSlots)
...
bool
TR_OSRCompilationData::TR_ScratchBufferInfo::operator==(
