...
void TR_OSRCompilationData::addInstruction(int32_t instructionPC, TR_ByteCodeInfo bcInfo)
{
  ...
  while (1) {
    ...
    TR_ASSERT(getNumOfSymsThatShareSlot() >= 0, "number of symbols that share slots must be non-negative; actually %d",
  ...
  if (!TR::Compiler->vm.ensureOSRBufferSize(comp, maxFrameSize, getMaxScratchBufferSize(), maxStackFrameSize)) {
    ...
    if (comp->getOptions()->isAnyVerboseOptionSet(TR_VerboseOSR, TR_VerboseOSRDetails))
      TR_VerboseLog::writeLineLocked(TR_Vlog_OSR,
        "OSR COMPILE ABORT in %s: frame size %d, scratch size %d, stack size %d were not accommodated by the VM\n",
...
uint32_t TR_OSRCompilationData::getOSRStackFrameSize(uint32_t methodIndex)
{
  ...
  if (methodIndex < getOSRMethodDataArray().size()) {
    ...
    if (osrMethodData != NULL)
      return (1 + osrMethodData->getMethodSymbol()->getNumParameterSlots()) * TR::Compiler->om.sizeofReferenceAddress();
    else
      return 0;
  } else
    return 0;
  ...
  for (intptrj_t i = 0; i < methodDataArray.size(); ++i) {
    ...
    if (osrCatchBlock && osrCatchBlock->getExceptionPredecessors().size() > 0) {
      definingMapAtOSRCatchBlocks[i]
        = new DefiningMap(DefiningMapComparator(), DefiningMapAllocator(comp->trMemory()->currentStackRegion()));
    ...
    if (!osrCatchBlockRemoved && !osrCodeBlockRemoved)
      buildFinalMap(i - 1, osrMethodData->getDefiningMap(), definingMapAtOSRCatchBlocks[i], definingMapAtOSRCodeBlocks,
...
void TR_OSRMethodData::buildDefiningMap(TR::Block* block, DefiningMap* blockMap, DefiningMap* prepareForOSRCallMap)
{
  for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextRealTreeTop()) {
    ...
    } else if (node->getFirstChild() && node->getFirstChild()->getOpCode().isCall()
...
void TR_OSRMethodData::createOSRBlocks(TR::Node* n)
{
  ...
  osrCatchBlock->setHandlerInfoWithOutBCInfo(
    TR::Block::CanCatchOSR, comp()->getInlineDepth(), -1, getMethodSymbol()->getResolvedMethod(), comp());
  ...
  if (comp()->getOption(TR_TraceOSR)) {
    traceMsg(comp(), "Created OSR code block_%d(%p) and OSR catch block_%d(%p) for %s %s\n", osrCodeBlock->getNumber(),
      osrCodeBlock, osrCatchBlock->getNumber(), osrCatchBlock,
...
void TR_OSRMethodData::addInstruction(int32_t instructionPC, int32_t byteCodeIndex)
{
  ...
  if (bcInfoHashTab.Locate(byteCodeIndex, hashIndex)) {
    ...
    for (int i = 0; i < slotInfos.size(); i++) {
      ...
      if (trace)
        traceMsg(comp(), "    %3d: %3d %3d %3d %3d\n", i, getInlinedSiteIndex(), osrBufferOffset, scratchBufferOffset,
          slotInfos[i].symSize);
    }
...
int32_t TR_OSRMethodData::slotIndex2OSRBufferIndex(int32_t slotIndex, int symSize, bool takesTwoSlots) const
{
  ...
  // Note that in 64 bit mode, a long or a double value will be stored in the low-memory slot, and an int will be store
  // in the low-memory double-word
  if (slotIndex < 0)
    return headerSize
    ...
    return headerSize
      + (getMethodSymbol()->getNumPPSlots() + getMethodSymbol()->getResolvedMethod()->numberOfTemps() + numOfSyncObjects
          + getMethodSymbol()->getNumParameterSlots() - slotIndex - 1 + (takesTwoSlots ? -1 : 0))
      * TR::Compiler->om.sizeofReferenceAddress();
}
...
{
  int32_t numOfSyncObjects = (getMethodSymbol()->getSyncObjectTemp() != NULL) ? 1 : 0;
  return getMethodSymbol()->getNumPPSlots() + getMethodSymbol()->getResolvedMethod()->numberOfTemps() + numOfSyncObjects
...
DefiningMap* TR_OSRMethodData::getDefiningMap()
{
  if (_symRefDefiningMap == NULL)
    _symRefDefiningMap
      = new DefiningMap(DefiningMapComparator(), DefiningMapAllocator(comp()->trMemory()->heapMemoryRegion()));
  ...
  for (int i = 0; i < slotInfos.size(); i++) {
    ...
    if (info.symRefNum != -1) {
      if ((info.slot == slot) && (info.symRefNum != symRefNum)
        ...
        // Two different variables sharing the same slot, can be live at a certain node (when there is a loop). So
        // commenting the assertion out for now. TR_ASSERTC(0, comp, "ERROR: For slot %d symref #%d conflicts with
        // symref#%d\n",
    ...
    if ((info.slot == slot) && (info.symRefNum == symRefNum)) {
      TR_ASSERT(info.symRefOrder == symRefOrder && info.symSize == symSize,
        "symref order (%d,%d) and symsize (%d,%d) must match", info.symRefOrder, symRefOrder, info.symSize, symSize);
      found = true;
    }
    ...
    if (info.symRefNum != symRefNum && ((slot < 0 && info.slot < 0) || (slot >= 0 && info.slot >= 0))) {
      ...
      if ((startSlot1 <= endSlot2) && (startSlot2 <= endSlot1)) {
        if (trace)
          traceMsg(
            comp, "addSlotInfo: symbols #%d and #%d overlap zeroing out slot %d\n", symRefNum, info.symRefNum, slot);
