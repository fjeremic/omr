...
TR::SymbolReference* OMR::SymbolReferenceTable::findClassAndDepthFlagsSymbolRef()
{
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateOSRReturnAddressSymbolRef()
{
  if (!element(osrReturnAddressSymbol)) {
    ...
    // We can't let the load/store of the exception symbol swing down
    aliasBuilder.addressStaticSymRefs().set(
      getNonhelperIndex(osrReturnAddressSymbol)); // add the symRef to the statics list to get correct aliasing info
  }
...
TR::SymbolReference* OMR::SymbolReferenceTable::methodSymRefFromName(TR::ResolvedMethodSymbol* owningMethodSymbol,
  char* className, char* methodName, char* methodSignature, TR::MethodSymbol::Kinds kind, int32_t cpIndex)
{
  ...
  // Note: we use cpIndex=-1 here so we don't end up getting back the original symref (which will not have the signature
  // we want)
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateSymRefWithKnownObject(
  TR::SymbolReference* original, uintptrj_t* referenceLocation, bool isArrayWithConstantElements)
{
  ...
  TR::Symbol* symbol, TR::KnownObjectTable::Index knownObjectIndex)
{
  ...
  while (bvi.hasMoreElements()) {
    ...
    if (symbol && symRef->getSymbol() == symbol)
      return symRef;
    if (!symbol
      && symRef->getSymbol()->isAuto()) // For temp, symbol can be NULL. Find any temp with the given knownObjectIndex.
      return symRef;
  }
  return NULL;
}
  ...
  TR::SymbolReference* originalSymRef, TR::KnownObjectTable::Index knownObjectIndex)
{
  ...
  // If the known object symref is created for an immutable array shadow, it should be aliased to other array shadows as
  // well Put the symRef in arrayElementSymRefs so that its alias set can be built correctly
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateGenericIntArrayShadowSymbolReference(intptrj_t offset)
{
  TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(
    self(), findOrCreateGenericIntShadowSymbol(), comp()->getMethodSymbol()->getResolvedMethodIndex(), -1);
  symRef->setOffset(offset);
...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateGenericIntNonArrayShadowSymbolReference(intptrj_t offset)
{
  TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(
    self(), findOrCreateGenericIntShadowSymbol(), comp()->getMethodSymbol()->getResolvedMethodIndex(), -1);
  symRef->setOffset(offset);
  ...
  TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, void* classObject, bool cpIndexOfStatic)
{
  ...
  // the shared symref uses the cpindex for the explicit reference and has the 'addressIsCPIndexOfStatic' set to false.
  //
  TR::SymbolReference* symRef
  ...
  size_t size, TR::KnownObjectTable::Index knownObjectIndex)
{
  ...
  if (TR::Symbol::convertTypeToNumberOfSlots(type) == 2) {
    ...
    for (; symRef2; symRef2 = i2.getNext()) {
      if (comp()->getOption(TR_MimicInterpreterFrameShape) && (symRef2->getSymbol()->getDataType() == TR::Address)) {
...
TR::SymbolReference* OMR::SymbolReferenceTable::createSymbolReference(TR::Symbol* sym, intptrj_t offset)
{
...
{
  if (ref >= _numHelperSymbols && ref < _numHelperSymbols + getLastCommonNonhelperSymbol()
  ...
  TR::SymbolReference* improved, TR::SymbolReference* original)
{
  ...
  auto insertResult
    = _originalUnimprovedSymRefs.insert(std::make_pair(improved->getReferenceNumber(), original->getReferenceNumber()));
