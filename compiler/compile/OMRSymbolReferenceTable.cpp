...
{
    int32_t index = symRef->getReferenceNumber();
    return aliasBuilder.refinedNonIntPrimitiveArrayShadows().isSet(index) || aliasBuilder.refinedIntArrayShadows().isSet(index) || aliasBuilder.refinedAddressArrayShadows().isSet(index);
}
...
{
    TR::SymbolReference* symRef;
    TR_SymRefIterator i(type == TR::Address ? aliasBuilder.addressStaticSymRefs() : (type == TR::Int32 ? aliasBuilder.intStaticSymRefs() : aliasBuilder.nonIntPrimitiveStaticSymRefs()), self());
    while ((symRef = i.getNext())) {
        if (symRef->getSymbol()->getDataType() == type && symRef->getCPIndex() != -1 && TR::Compiler->cls.jitStaticsAreSame(comp(), owningMethod, cpIndex, symRef->getOwningMethod(comp()), symRef->getCPIndex()))
            return symRef;
    }
    return 0;
}
...
OMR::SymbolReferenceTable::findOrCreateAutoSymbolImpl(TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t slot, TR::DataType type, bool isReference, bool isInternalPointer, bool reuseAuto, bool isAdjunct, size_t size, TR::KnownObjectTable::Index knownObjectIndex)
{
    ...
    List<TR::SymbolReference>& list = slot < 0 ? owningMethodSymbol->getPendingPushSymRefs(-slot - 1) : owningMethodSymbol->getAutoSymRefs(slot);
    bool slotSharedbyRefAndNonRef = false;
    ListIterator<TR::SymbolReference> i(&list);
    TR::SymbolReference* symRef = i.getFirst();
    for (; symRef; symRef = i.getNext())
        if ((type == symRef->getSymbol()->getDataType() && (symRef->isAdjunct() == isAdjunct) && (size == 0 || size == symRef->getSymbol()->getSize()))
            // standard match
        ...
        // dual symref match, when original type is an address, and needs to be coerced
        {
            ...
            return symRef;
        } else if (comp()->getOption(TR_MimicInterpreterFrameShape) && (type == TR::Address || symRef->getSymbol()->getDataType() == TR::Address)) {
    ...
    if (TR::Symbol::convertTypeToNumberOfSlots(type) == 2) {
        List<TR::SymbolReference>& list2 = slot < 0 ? owningMethodSymbol->getPendingPushSymRefs(-(slot - 1) - 1) : owningMethodSymbol->getAutoSymRefs(slot + 1);
    ...
    if ((type == TR::Address) && (slot != -1) && (slot != 0)) {
        ...
        TR::SymbolReference* symRef2 = i2.getFirst();
        for (; symRef2; symRef2 = i2.getNext()) {
            if (comp()->getOption(TR_MimicInterpreterFrameShape) && (TR::Symbol::convertTypeToNumberOfSlots(symRef2->getSymbol()->getDataType()) == 2)) {
                if (symRef2->getSymbol()->getParmSymbol() || comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
    ...
    if (!symRef) {
        ...
        if (isInternalPointer) {
            sym = size ? TR::AutomaticSymbol::createInternalPointer(trHeapMemory(), type, size, comp()->fe()) : TR::AutomaticSymbol::createInternalPointer(trHeapMemory(), type);
            ...
            sym = size ? TR::AutomaticSymbol::create(trHeapMemory(), type, size) : TR::AutomaticSymbol::create(trHeapMemory(), type);
        }
    ...
    List<TR::SymbolReference>& list3 = slot < 0 ? owningMethodSymbol->getPendingPushSymRefs(-slot - 1) : owningMethodSymbol->getAutoSymRefs(slot);
    list3.add(symRef);
    return symRef;
}
    ...
    TR::DataType type, bool behavesLikeTemp, bool isAdjunct)
{
    ...
    if (!(comp()->cg()->getSupportsCompactedLocals() && comp()->getMethodHotness() >= hot)) {
        ...
        for (prev = 0, a = autos.getFirst(); a; prev = autos.getCurrentElement(), a = autos.getNext())
            if (type == a->getSymbol()->getDataType() && !notSharing && !a->getSymbol()->holdsMonitoredObject() && !a->hasKnownObjectIndex() && (a->isAdjunct() == isAdjunct) && (comp()->cg()->getSupportsJavaFloatSemantics() || (type != TR::Float && type != TR::Double) || (a->isTemporary(comp()) && behavesLikeTemp == !a->getSymbol()->behaveLikeNonTemp()))) {
