...
static TR::CodeGenerator*
allocateCodeGenerator(TR::Compilation* comp)
{
...
OMR::Compilation::Compilation(int32_t id,
  ...
  , _debugCounterMap(std::less<const void*>(),
    ...
    if (((self()->getMethodHotness() < warm) ||
        ...
        !options.getOption(
          TR_FullSpeedDebug)) // Off for two reasons : 1) not sure if we can
                              // afford the increase in compile time due to the
                              // extra OSR control flow at cold and 2) not sure
                              // at this stage in 727 whether OSR can work with
                              // AOT (will try to find out soon) but disabling
                              // till I do find out
...
OMR::Compilation::supportsInduceOSR()
{
  ...
  if (!self()->canAffordOSRControlFlow()) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(
        self(),
        "canAffordOSRControlFlow is false - OSR induction is not supported\n");
    return false;
  }
  ...
  if (self()->getOption(TR_MimicInterpreterFrameShape) &&
    ...
    if (self()->getOption(TR_TraceOSR))
      traceMsg(
        self(),
        "MimicInterpreterFrameShape is set - OSR induction is not supported\n");
    return false;
  }

  if (self()
        ->isDLT() /* && getJittedMethodSymbol()->sharesStackSlots(self()) */) {
    if (self()->getOption(TR_TraceOSR))
...
bool
OMR::Compilation::isShortRunningMethod(int32_t callerIndex)
{
  return false;
}

bool
OMR::Compilation::isPotentialOSRPoint(TR::Node* node,
...
bool
OMR::Compilation::isPotentialOSRPointWithSupport(TR::TreeTop* tt)
{
  TR::Node* osrNode;
  ...
  if (potentialOSRPoint && self()->getOSRMode() == TR::voluntaryOSR) {
    ...
    if (potentialOSRPoint) {
      ...
      potentialOSRPoint =
        method->supportsInduceOSR(bci, tt->getEnclosingBlock(), self(), false);
    }
  }
...
OMR::Compilation::compile()
{
  ...
  if (self()->getOptions()->getDelayCompile()) {
    ...
    fprintf(
      stderr,
      "\nDelayCompile: Starting a delay of length %d for method %s at time %d.",
      limit,
      self()->signature(),
      starttime);
    fflush(stderr);
  ...
  if (self()->getOption(TR_DebugBeforeCompile)) {
    ...
    self()->getDebug()->setupDebugger(
      (void*)&stopBeforeCompile, (void*)&stopBeforeCompile, true);
  ...
  {
    TR::RegionProfiler rpIlgen(
      self()->trMemory()->heapMemoryRegion(), *self(), "comp/ilgen");
    if (printCodegenTime)
      genILTime.startTiming(self());
    _ilGenSuccess = _methodSymbol->genIL(
      self()->fe(), self(), self()->getSymRefTab(), _ilGenRequest);
    if (printCodegenTime)
...

  {
    ...
    TR::LexicalMemProfiler mp(
      "compile", self()->signature(), self()->phaseMemProfiler());
    ...
    if (_ilGenSuccess) {
      ...
      {
        TR::RegionProfiler rpOpt(
          self()->trMemory()->heapMemoryRegion(), *self(), "comp/opt");
      ...
      {
        TR::RegionProfiler rpCodegen(
          self()->trMemory()->heapMemoryRegion(), *self(), "comp/codegen");
      ...
      if (self()->getOptions()->getVerboseOption(TR_VerboseInlining)) {
        ...
        TR_VerboseLog::writeLine(TR_Vlog_INL,
                                 "%d methods inlined into %x %s @ %p",
                                 self()->getNumInlinedCallSites(),
                                 jittedBodyHash,
                                 self()->signature(),
        ...
        for (int32_t i = 0; i < self()->getNumInlinedCallSites(); i++) {
          ...
          TR_VerboseLog::writeLine(
            ...
            strHash(calleeBuf),
            TR::Compiler->mtd.bytecodeSize(site._methodInfo),
            calleeBuf);
        }
...
#else
    self()->getDebug()->setupDebugger(
      (void*)jitTojitStart, self()->cg()->getCodeEnd(), false);
#endif /* defined(AIXPPC) */
  }
#elif defined(LINUX) || defined(J9ZOS390) || defined(OMR_OS_WINDOWS)
  if (self()->getOption(TR_DebugOnEntry)) {
    self()->getDebug()->setupDebugger(
      self()->cg()->getCodeStart(), self()->cg()->getCodeEnd(), false);
  }
...
TR_YesNoMaybe
OMR::Compilation::isCpuExpensiveCompilation(int64_t threshold)
{
...
bool
OMR::Compilation::incInlineDepth(TR::ResolvedMethodSymbol* method,
                                 ...
                                 TR_PrexArgInfo* argInfo)
{
  if (self()->compileRelocatableCode()) {
    ...
    return self()->incInlineDepth((TR_OpaqueMethodBlock*)aotMethodInfo,
                                  method,
                                  bcInfo,
                                  callSymRef,
                                  directCall,
    ...
    return self()->incInlineDepth(
      method->getResolvedMethod()->getPersistentIdentifier(),
      method,
      bcInfo,
      callSymRef,
      directCall,
...
bool
OMR::Compilation::foundOnTheStack(TR_ResolvedMethod* method,
                                  int32_t occurrences)
{
...
bool
OMR::Compilation::incInlineDepth(TR_OpaqueMethodBlock* methodInfo,
                                 TR::ResolvedMethodSymbol* method,
                                 TR_ByteCodeInfo& bcInfo,
                                 TR::SymbolReference* callSymRef,
                                 bool directCall,
                                 TR_PrexArgInfo* argInfo)
{
  ...
  uint32_t callSiteIndex = _inlinedCallSites.add(
    TR_InlinedCallSiteInfo(methodInfo, bcInfo, method, callSymRef, directCall));
...
void
OMR::Compilation::decInlineDepth(bool removeInlinedCallSitesEntry)
{
...
int16_t
OMR::Compilation::restoreInlineDepth(TR_ByteCodeInfo& existingInfo)
{
...
int16_t
OMR::Compilation::adjustInlineDepth(TR_ByteCodeInfo& bcInfo)
{
  if (bcInfo.getCallerIndex() == -1)
    return 0;
  else
...
int32_t
OMR::Compilation::convertNonDeterministicInput(
  int32_t i,
  int32_t max,
...
OMR::Compilation::findVirtualGuardInfo(TR::Node* guardNode)
{
  ...
  if (guardKind != TR_NoGuard) {
    for (auto current = _virtualGuards.begin(); current != _virtualGuards.end();
      ...
      if ((*current)->getKind() == guardKind &&
        ...
        if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
          traceMsg(
            self(), "found guard %p, guardkind = %d\n", guard, guardKind);
        break;
      }
    }
  } else {
  ...
  TR_ASSERT(guard,
            "Can't find the virtual guard @ bci %d:%d, node %p \n",
            guardNode->getByteCodeInfo().getCallerIndex(),
            guardNode->getByteCodeInfo().getByteCodeIndex(),
            guardNode);
...
TR::Node*
OMR::Compilation::createSideEffectGuard(TR::Compilation* comp,
                                        TR::Node* node,
...
TR::Node*
OMR::Compilation::createAOTInliningGuard(TR::Compilation* comp,
                                         int16_t calleeIndex,
                                         TR::Node* node,
                                         TR::TreeTop* destination,
                                         TR_VirtualGuardKind kind)
{
  return TR_VirtualGuard::createAOTInliningGuard(
    comp, calleeIndex, node, destination, kind);
}

TR::Node*
OMR::Compilation::createAOTGuard(TR::Compilation* comp,
                                 int16_t calleeIndex,
                                 TR::Node* node,
                                 TR::TreeTop* destination,
                                 TR_VirtualGuardKind kind)
{
  return TR_VirtualGuard::createAOTGuard(
    comp, calleeIndex, node, destination, kind);
}

TR::Node*
OMR::Compilation::createDummyGuard(TR::Compilation* comp,
                                   int16_t calleeIndex,
                                   TR::Node* node,
                                   TR::TreeTop* destination)
{
  return TR_VirtualGuard::createDummyGuard(
    comp, calleeIndex, node, destination);
}
...
int32_t
OMR::Compilation::findPrefetchInfo(TR::Node* node)
{
  for (auto pair = self()->getNodesThatShouldPrefetchOffset().begin();
       pair != self()->getNodesThatShouldPrefetchOffset().end();
       ++pair) {
    if ((*pair)->getKey() == node) {
...
OMR::Compilation::findExtraPrefetchInfo(TR::Node* node, bool use)
{
  for (auto it = self()->getExtraPrefetchInfo().begin();
       it != self()->getExtraPrefetchInfo().end();
       ++it) {
    if (use) {
      if ((*it)->_useNode == node)
        return *it;
...
OMR::Compilation::getMethodFromNode(TR::Node* node)
{
  TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
  ...
  if (bcInfo.getCallerIndex() >= 0 && self()->getNumInlinedCallSites() > 0)
    method =
      self()->compileRelocatableCode()
        ? ((TR_ResolvedMethod*)node->getAOTMethod())->getPersistentIdentifier()
...
int32_t
OMR::Compilation::getLineNumber(TR::Node* node)
{
  TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
...
int32_t
OMR::Compilation::getLineNumberInCurrentMethod(TR::Node* node)
{
...
void
OMR::Compilation::resetVisitCounts(vcount_t count)
{
...
void
OMR::Compilation::resetVisitCounts(vcount_t count, TR::TreeTop* start)
{
  TR::TreeTop* tt;
...
void
OMR::Compilation::reportFailure(const char* reason)
{
...
void
OMR::Compilation::AddCopyPropagationRematerializationCandidate(
...
void
OMR::Compilation::RemoveCopyPropagationRematerializationCandidate(
  TR::SymbolReference* sr)
{
...
bool
OMR::Compilation::IsCopyPropagationRematerializationCandidate(
  TR::SymbolReference* sr)
{
  return (
    _copyPropagationRematerializationCandidates[sr->getReferenceNumber()] == 1);
...
bool
OMR::Compilation::isVirtualGuardNOPingRequired(TR_VirtualGuard* virtualGuard)
{
  if (self()->isProfilingCompilation()) {
    if (!virtualGuard) {
      for (auto current = _virtualGuards.begin();
           current != _virtualGuards.end();
           ++current)
...
void
OMR::Compilation::setUsesPreexistence(bool v)
{
  if (v)
...
//
void
OMR::Compilation::dumpMethodTrees(char* title,
...
void
OMR::Compilation::dumpMethodTrees(char* title1,
                                  const char* title2,
                                  TR::ResolvedMethodSymbol* methodSymbol)
{
...
void
OMR::Compilation::dumpFlowGraph(TR::CFG* cfg)
{
  if (cfg == 0)
...
void
OMR::Compilation::mapStaticAddressToCounter(TR::SymbolReference* symRef,
                                            TR::DebugCounterBase* counter)
{
  const void* staticAddress =
    (const void*)symRef->getSymbol()->castToStaticSymbol()->getStaticAddress();
...
void
OMR::Compilation::validateIL(TR::ILValidationContext ilValidationContext)
{
  TR_ASSERT_FATAL(
    _ilValidator != NULL,
    "Attempting to validate the IL without the ILValidator being initialized");
  _ilValidator->validate(TR::omrValidationStrategies[ilValidationContext]);
}

void
OMR::Compilation::verifyTrees(TR::ResolvedMethodSymbol* methodSymbol)
{
...
void
OMR::Compilation::verifyBlocks(TR::ResolvedMethodSymbol* methodSymbol)
{
  if (self()->getDebug() && !self()->getOption(TR_DisableVerification) &&
      !self()->isPeekingMethod()) {
    if (!methodSymbol)
...
void
OMR::Compilation::verifyCFG(TR::ResolvedMethodSymbol* methodSymbol)
{
  if (self()->getDebug() && !self()->getOption(TR_DisableVerification) &&
      !self()->isPeekingMethod()) {
    if (!methodSymbol)
  ...
  for (TR::PreorderNodeIterator iter(self()->getStartTree(), self());
    ...
    if (node->getOpCodeValue() == TR::treetop ||
      ...
      if (!anchoredRdbarNodes.contains(node)) {
        TR_ASSERT(0,
                  "node (n%dn) %p is rdbar but not anchored\n",
                  node->getGlobalIndex(),
                  node);
...
#ifdef DEBUG
void
OMR::Compilation::dumpMethodGraph(int index,
                                  TR::ResolvedMethodSymbol* methodSymbol)
{
  if (self()->getOutFile() == NULL)
    return;
...
void
OMR::Compilation::registerResolvedMethodSymbolReference(
 ...
 */
void
OMR::Compilation::getOSRCallSiteRemat(uint32_t callSiteIndex,
                                      uint32_t slot,
                                      ...
                                      TR::SymbolReference*& loadSymRef)
{
  ...
  loadSymRef =
    table[slot * 2 + 1] == 0 ? NULL : symRefTab->getSymRef(table[slot * 2 + 1]);
}
 ...
 */
void
OMR::Compilation::setOSRCallSiteRemat(uint32_t callSiteIndex,
                                      TR::SymbolReference* ppSymRef,
                                      TR::SymbolReference* loadSymRef)
{
  int32_t* table = _inlinedCallSites[callSiteIndex].osrCallSiteRematTable();
...
void
OMR::Compilation::diagnosticImpl(const char* s, ...)
{
#if defined(DEBUG)
  va_list ap;
  va_start(ap, s);
...
void
OMR::Compilation::diagnosticImplVA(const char* s, va_list ap)
{
#if defined(DEBUG)
  if (self()->getOutFile() != NULL) {
    va_list copy;
...
void
addDebug(const char* debugString)
{
...
BitVectorPool::get()
{
  ...
  TR_BitVector* newBitVector = new (_comp->trHeapMemory())
    TR_BitVector(_comp->getNodeCount() /*an estimate*/,
                 _comp->trMemory(),
                 heapAlloc,
                 growable);
  return newBitVector;
...
void
BitVectorPool::release(TR_BitVector* v)
{
  v->empty();
  _pool.push(v);
}
...
namespace TR {
Compilation&
operator<<(Compilation& comp, const char* str)
{
  traceMsg(&comp, "%s", str);
  return comp;
}
...
Compilation&
operator<<(Compilation& comp, const int n)
{
  traceMsg(&comp, "%d", n);
  return comp;
}
...
Compilation&
operator<<(Compilation& comp, const ::TR_ByteCodeInfo& bcInfo)
{
  char tmp[20];
...
void
OMR::Compilation::restoreCompilationPhase(
...
bool
OMR::Compilation::incompleteOptimizerSupportForReadWriteBarriers()
{
  return false;
}

bool
OMR::Compilation::isRecursiveMethodTarget(
...
bool
OMR::Compilation::isRecursiveMethodTarget(TR::Symbol* targetSymbol)
{
...
const TR::TypeLayout*
OMR::Compilation::typeLayout(TR_OpaqueClassBlock* clazz)
{
