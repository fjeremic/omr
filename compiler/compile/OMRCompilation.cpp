  ...
  , _tlsManager(*self())
{
  ...
  if (self()->isDLT()
    || (((self()->getMethodHotness() < warm) || self()->compileRelocatableCode() || self()->isProfilingCompilation())
  ...
  if (options.getOption(TR_EnableOSR)) {
    ...
    if (((self()->getMethodHotness() < warm) || self()->compileRelocatableCode() || self()->isProfilingCompilation())
      && !enableOSRAtAllOptLevels
      && !options.getOption(
        TR_FullSpeedDebug)) // Off for two reasons : 1) not sure if we can afford the increase in compile time due to
                            // the extra OSR control flow at cold and 2) not sure at this stage in 727 whether OSR can
                            // work with AOT (will try to find out soon) but disabling till I do find out
...
bool OMR::Compilation::supportsInduceOSR()
{
  ...
  if (_osrCompilationData && _osrCompilationData->seenClassPreventingInducedOSR()) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(),
        "Cannot guarantee OSR transfer of control to the interpreter will work for calls preventing induced OSR (e.g. "
        "Quad) because of differences in JIT vs interpreter representations\n");
    return false;
  }
...
int32_t OMR::Compilation::compile()
{
  ...
  if (self()->getOptions()->getDelayCompile()) {
    ...
    fprintf(stderr, "\nDelayCompile: Starting a delay of length %d for method %s at time %d.", limit,
      self()->signature(), starttime);
    fflush(stderr);
    uint64_t temp = 0;
    while (1) {
  ...
  if (self()->getOutFile() != NULL
    && (self()->getOption(TR_TraceAll) || debug("traceStartCompile") || self()->getOptions()->getAnyTraceCGOption()
...

  {
    ...
    if (_ilGenSuccess) {
      ...
      if (self()->getOptions()->getVerboseOption(TR_VerboseInlining)) {
        ...
        TR_VerboseLog::writeLine(TR_Vlog_INL, "%d methods inlined into %x %s @ %p", self()->getNumInlinedCallSites(),
          jittedBodyHash, self()->signature(), self()->cg()->getCodeStart());
        for (int32_t i = 0; i < self()->getNumInlinedCallSites(); i++) {
          ...
          TR_VerboseLog::writeLine(TR_Vlog_INL, "#%d: %x #%d inlined %x@%d -> %x bcsz=%d %s", i, jittedBodyHash,
            callerIndex, strHash(callerSig), site._byteCodeInfo.getByteCodeIndex(), strHash(calleeBuf),
              ...
              if (method) {
                ...
                if (node->getSymbol()->getResolvedMethodSymbol())
                  calleeSize = TR::Compiler->mtd.bytecodeSize(
                    node->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod()->getPersistentIdentifier());
                TR_VerboseLog::writeLine(TR_Vlog_INL, "%x %s %x@%d -> %x bcsz=%d %s", jittedBodyHash,
                  curBlock->isCold() ? "coldCalled" : "called", strHash(callerSig), bcInfo.getByteCodeIndex(),
  ...
  TR::SymbolReference* callSymRef, bool directCall, TR_PrexArgInfo* argInfo)
{
  if (self()->compileRelocatableCode()) {
    TR_AOTMethodInfo* aotMethodInfo
      = (TR_AOTMethodInfo*)self()->trMemory()->allocateHeapMemory(sizeof(TR_AOTMethodInfo));
    aotMethodInfo->resolvedMethod = method->getResolvedMethod();
    aotMethodInfo->cpIndex = cpIndex;
    return self()->incInlineDepth(
      (TR_OpaqueMethodBlock*)aotMethodInfo, method, bcInfo, callSymRef, directCall, argInfo);
  } else {
    return self()->incInlineDepth(
      method->getResolvedMethod()->getPersistentIdentifier(), method, bcInfo, callSymRef, directCall, argInfo);
  }
}
  ...
  TR_ByteCodeInfo& bcInfo, TR::SymbolReference* callSymRef, bool directCall, TR_PrexArgInfo* argInfo)
{
  ...
  uint32_t callSiteIndex
    = _inlinedCallSites.add(TR_InlinedCallSiteInfo(methodInfo, bcInfo, method, callSymRef, directCall));
...
{
  TR_ASSERT(guardNode->isTheVirtualGuardForAGuardedInlinedCall() || guardNode->isOSRGuard() || guardNode->isHCRGuard()
      || guardNode->isProfiledGuard() || guardNode->isMethodEnterExitGuard() || guardNode->isBreakpointGuard(),
    "@node %p\n", guardNode);
  ...
  if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
    traceMsg(self(), "Looking for a guard for node %p with kind %d bcindex %d calleeindex %d\n", guardNode, guardKind,
      guardNode->getByteCodeInfo().getByteCodeIndex(), guardNode->getByteCodeInfo().getCallerIndex());
  ...
  if (guardKind != TR_NoGuard) {
    ...
    for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
      ...
      if ((*current)->getByteCodeIndex() == guardNode->getByteCodeInfo().getByteCodeIndex()
        && (*current)->getCalleeIndex() == guardNode->getByteCodeInfo().getCallerIndex()
        && (*current)->getKind() != TR_SideEffectGuard && (*current)->getKind() != TR_HCRGuard
        && (*current)->getKind() != TR_OSRGuard && (*current)->getKind() != TR_MethodEnterExitGuard
        && (*current)->getKind() != TR_MutableCallSiteTargetGuard && (*current)->getKind() != TR_BreakpointGuard) {
        guard = *current;
        if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
...
void OMR::Compilation::removeVirtualGuard(TR_VirtualGuard* guard)
{
  for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
    if (((*current)->getCalleeIndex() == guard->getCalleeIndex())
      && ((*current)->getByteCodeIndex() == guard->getByteCodeIndex()) && ((*current)->getKind() == guard->getKind())) {
      if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
...
TR_OpaqueMethodBlock* OMR::Compilation::getMethodFromNode(TR::Node* node)
{
  ...
  if (bcInfo.getCallerIndex() >= 0 && self()->getNumInlinedCallSites() > 0)
    method = self()->compileRelocatableCode() ? ((TR_ResolvedMethod*)node->getAOTMethod())->getPersistentIdentifier()
...
void OMR::Compilation::dumpMethodTrees(char* title, TR::ResolvedMethodSymbol* methodSymbol)
{
  ...
  if (self()->isOutermostMethod()
    && self()
         ->getKnownObjectTable()) // This is pretty verbose.  Let's just dump it when we're dumping the whole method.
...
{
  return (self()->getFlowGraph()->getStructure()->getNumberOfLoops() >= self()->getOptions()->getLargeNumberOfLoops());
}
...
void OMR::Compilation::diagnosticImplVA(const char* s, va_list ap)
{
  ...
  if (self()->getOutFile() != NULL) {
    ...
    char buffer[256];
    TR::IO::vfprintf(
      self()->getOutFile(), self()->getDebug()->getDiagnosticFormat(s, buffer, sizeof(buffer) / sizeof(char)), copy);
    trfflush(self()->getOutFile());
    va_end(copy);
  }
#endif
...
char* debug(const char* option)
{
  ...
  if (head) {
    for (DebugValue* val = head;; val = val->next) {
      if (!strncmp(option, val->value, optionLen) && (val->value[optionLen] == 0 || val->value[optionLen] == '=')) {
        head = val;
