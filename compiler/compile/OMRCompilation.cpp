...
// this ratio defines how full the alias memory region is allowed to become
// before it is recreated after an optimization finishes
...
TR::SymbolReference *OMR::Compilation::getSymbolReferenceByReferenceNumber(
...
const char *OMR::Compilation::getHotnessName(TR_Hotness h) {
  if (h < minHotness || h >= numHotnessLevels) return "unknownHotness";
...
OMR::Compilation::Compilation(int32_t id, OMR_VMThread *omrVMThread,
                              TR_FrontEnd *fe, TR_ResolvedMethod *compilee,
                              TR::IlGenRequest &ilGenRequest,
                              TR::Options &options,
                              TR::Region &heapMemoryRegion, TR_Memory *m,
    ...
    : _signature(compilee->signature(m)),
      ...
      _currentOptSubIndex(
          0),  // The transformation index within the current opt
      ...
      _devirtualizedCalls(
          getTypedAllocator<TR_DevirtualizedCallInfo *>(self()->allocator())),
      _inlinedCalls(0),
      _inlinedFramesAdded(0),
      _virtualGuards(getTypedAllocator<TR_VirtualGuard *>(self()->allocator())),
      _staticPICSites(
          getTypedAllocator<TR::Instruction *>(self()->allocator())),
      _staticHCRPICSites(
          getTypedAllocator<TR::Instruction *>(self()->allocator())),
      _staticMethodPICSites(
          getTypedAllocator<TR::Instruction *>(self()->allocator())),
      _snippetsToBePatchedOnClassUnload(
          getTypedAllocator<TR::Snippet *>(self()->allocator())),
      _methodSnippetsToBePatchedOnClassUnload(
          getTypedAllocator<TR::Snippet *>(self()->allocator())),
      _snippetsToBePatchedOnClassRedefinition(
          getTypedAllocator<TR::Snippet *>(self()->allocator())),
      _genILSyms(
          getTypedAllocator<TR::ResolvedMethodSymbol *>(self()->allocator())),
      _noEarlyInline(true),
      ...
      _checkcastNullChkInfo(
          getTypedAllocator<TR_Pair<TR_ByteCodeInfo, TR::Node> *>(
              self()->allocator())),
      _nodesThatShouldPrefetchOffset(
          getTypedAllocator<TR_Pair<TR::Node, uint32_t> *>(
              self()->allocator())),
      _extraPrefetchInfo(
          getTypedAllocator<TR_PrefetchInfo *>(self()->allocator())),
      _debugCounterMap(
          std::less<const void *>(),
          getTypedAllocator<DebugCounterEntry>(self()->allocator())),
      _currentBlock(NULL),
      _verboseOptTransformationCount(0),
      _relocatableMethodCodeStart(NULL),
      _compThreadID(id),
      _failCHtableCommitFlag(false),
      _phaseTimer("Compilation", self()->allocator("phaseTimer"),
                  self()->getOption(TR_Timing)),
      _phaseMemProfiler("Compilation", self()->allocator("phaseMemProfiler"),
                        self()->getOption(TR_LexicalMemProfiler)),
      _compilationNodes(NULL),
      _copyPropagationRematerializationCandidates(
          self()->allocator("CP rematerialization")),
      _nodeOpCodeLength(0),
      ...
      _tlsManager(*self()) {
  // Avoid expensive initialization and uneeded option checking if we are doing
  // AOT Loads
  ...
  _metadataAssumptionList = NULL;
#endif
  _symRefTab = new (_trMemory->trHeapMemory())
      TR::SymbolReferenceTable(_method->maxBytecodeIndex(), self());
  _compilationNodes =
      new (_trMemory->trHeapMemory()) TR::NodePool(self(), self()->allocator());

#ifdef J9_PROJECT_SPECIFIC
  // The following must stay before the first assumption gets created which
  // could happen while the PersistentMethodInfo is allocated during codegen
  // creation Access to this list must be performed with assumptionTableMutex in
  // hand
  //
  if (!options.getOption(TR_DisableFastAssumptionReclamation))
    _metadataAssumptionList =
        new (m->trPersistentMemory()) TR::SentinelRuntimeAssumption();
#endif

  // Random fields must be set before allocating codegen
  _primaryRandom =
      new (m->trHeapMemory()) TR_RandomGenerator(options.getRandomSeed());
  _adhocRandom =
      new (m->trHeapMemory()) TR_RandomGenerator(options.getRandomSeed());
  ...
  if (optimizationPlan) {
    if (optimizationPlan->isGPUCompilation()) _flags.set(IsGPUCompilation);
...
#ifdef J9_PROJECT_SPECIFIC
                  || (self()->getPersistentInfo()->getNumLoadedClasses() >=
                      TR::Options::_bigAppThreshold)
#endif
                 );
  _isServerInlining = !options.getOption(TR_NoOptServer);

  // TR_DisableInternalPointers must be set before allocateCodeGenerator(self())
  // is called because CodeGenerator's _disableInternalPointers member is set in
  // its constructor and this is one of options that is checked for
  if (_isOptServer) {
    if (self()->getMethodHotness() <= warm) {
      if (!TR::Compiler->target.cpu
               .isPower())  // Temporarily exclude PPC due to perf regression
        self()->setOption(TR_DisableInternalPointers);
    }
  }

  //_methodSymbol must be done after symRefTab, but before codegen
  // _methodSymbol must be initialized here because creating a jitted method
  // symbol
  //   actually inspects TR::comp()->_methodSymbol (to compare against the new
  //   object)
  _methodSymbol = NULL;
  {
    _methodSymbol = TR::ResolvedMethodSymbol::createJittedMethodSymbol(
        self()->trHeapMemory(), compilee, self());
  }

  // initPersistentCPUField and createOpCode must be done after method symbol
  // creation
  ...
  _globalRegisterCandidates =
      new (self()->trHeapMemory()) TR_RegisterCandidates(self());
  ...
  if (firstTime) {
    ...
    TR_ASSERT(TR::DataType::getMaxPackedDecimalSize() ==
                  TR::DataType::packedDecimalPrecisionToByteLength(
                      TR_MAX_DECIMAL_PRECISION),
              "TR::DataType::getMaxPackedDecimalSize() (%d) does not agree "
              "with precToSize(TR_MAX_DECIMAL_PRECISION) (%d)\n",
  ...
  static const char *enableOSRAtAllOptLevels =
      feGetEnv("TR_EnableOSRAtAllOptLevels");

  // If the outermost method is native, OSR is not supported
  if (_methodSymbol->isNative()) self()->setOption(TR_DisableOSR);
  ...
  if (!self()->getOption(TR_DisableNextGenHCR) &&
      !self()->getOption(TR_DisableOSR) &&
      !self()->getOption(TR_FullSpeedDebug) &&
      self()->getOption(TR_EnableHCR)) {
    self()->setOption(TR_EnableOSR);  // OSR must be enabled for NextGenHCR
  }

  if (self()->isDLT() ||
      (((self()->getMethodHotness() < warm) ||
        self()->compileRelocatableCode() || self()->isProfilingCompilation()) &&
  ...
  if (options.getOption(TR_EnableOSR)) {
    ...
    _osrCompilationData =
        new (self()->trHeapMemory()) TR_OSRCompilationData(self());
    ...
    if (((self()->getMethodHotness() < warm) ||
        ...
        !options.getOption(
            TR_FullSpeedDebug))  // Off for two reasons : 1) not sure if we can
                                 // afford the increase in compile time due to
                                 // the extra OSR control flow at cold and 2)
                                 // not sure at this stage in 727 whether OSR
                                 // can work with AOT (will try to find out
                                 // soon) but disabling till I do find out
...
bool OMR::Compilation::ilGenTrace() {
  if (self()->isOutermostMethod() || self()->getOption(TR_DebugInliner) ||
      self()->trace(OMR::inlining))
    return true;
  return false;
}
...
bool OMR::Compilation::allocateAtThisOptLevel() {
  return self()->getOptLevel() > warm;
}

TR::PersistentInfo *OMR::Compilation::getPersistentInfo() {
  return self()->trPersistentMemory()->getPersistentInfo();
}

int32_t OMR::Compilation::getMaxAliasIndex() {
  return self()->getSymRefCount();
}

bool OMR::Compilation::isPeekingMethod() {
  return self()->getCurrentSymRefTab() != 0;
}

TR_Hotness OMR::Compilation::getMethodHotness() {
...
const char *OMR::Compilation::getHotnessName() {
  return TR::Compilation::getHotnessName(self()->getMethodHotness());
}

TR::ResolvedMethodSymbol *OMR::Compilation::createJittedMethodSymbol(
    TR_ResolvedMethod *resolvedMethod) {
  return TR::ResolvedMethodSymbol::createJittedMethodSymbol(
      self()->trHeapMemory(), resolvedMethod, self());
}
...
bool OMR::Compilation::supportsInduceOSR() {
  if (_osrInfrastructureRemoved) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(),
               "OSR induction cannot be performed after OSR infrastructure has "
               "been removed\n");
    return false;
  }
  ...
  if (!self()->canAffordOSRControlFlow()) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(),
               "canAffordOSRControlFlow is false - OSR induction is not "
               "supported\n");
    return false;
  }
  ...
  if (self()->getOption(TR_MimicInterpreterFrameShape) &&
      !self()->getOption(
          TR_FullSpeedDebug) /* && areSlotsSharedByRefAndNonRef() */) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(),
               "MimicInterpreterFrameShape is set - OSR induction is not "
               "supported\n");
    return false;
  }
  ...
  if (_osrCompilationData &&
      _osrCompilationData->seenClassPreventingInducedOSR()) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(),
               "Cannot guarantee OSR transfer of control to the interpreter "
               "will work for calls preventing induced OSR (e.g. Quad) because "
               "of differences in JIT vs interpreter representations\n");
    return false;
  }
...
bool OMR::Compilation::penalizePredsOfOSRCatchBlocksInGRA() {
  if (!self()->getOption(TR_EnableOSR)) return false;

  if (self()->getOption(TR_FullSpeedDebug)) return false;
...
bool OMR::Compilation::isPotentialOSRPoint(TR::Node *node,
  ...
  if (self()->isOSRTransitionTarget(TR::postExecutionOSR)) {
    ...
    if (_osrInfrastructureRemoved && !ignoreInfra)
      ...
      if (disableAsyncCheckOSR == NULL)
        potentialOSRPoint = !self()->isShortRunningMethod(
      ...
      if (node->isPotentialOSRPointHelperCall()) {
        potentialOSRPoint = true;
                 ...
                 self()->getSymRefTab()->getNonhelperIndex(
                     self()->getSymRefTab()->getLastCommonNonhelperSymbol())) {
  ...
  if (osrPointNode && potentialOSRPoint) (*osrPointNode) = node;
...
bool OMR::Compilation::isPotentialOSRPointWithSupport(TR::TreeTop *tt) {
  ...
  if (potentialOSRPoint && self()->getOSRMode() == TR::voluntaryOSR) {
    if (self()->isOSRTransitionTarget(TR::postExecutionOSR) &&
        tt->getNode() != osrNode) {
      // The OSR point applies where the node is anchored, rather than where it
      // may be commoned. Therefore, it is necessary to check if the node is
      // anchored under a prior treetop.
      if (osrNode->getReferenceCount() > 1) {
        TR::TreeTop *cursor = tt->getPrevTreeTop();
        while (cursor) {
          if ((cursor->getNode()->getOpCode().isCheck() ||
               cursor->getNode()->getOpCodeValue() == TR::treetop) &&
    ...
    if (potentialOSRPoint) {
      ...
      potentialOSRPoint = method->supportsInduceOSR(
          bci, tt->getEnclosingBlock(), self(), false);
    }
  }
 ...
 *
 * In involuntary OSR, the JITed code does not control when an OSR transition
 * occurs. It can be initiated externally at any potential OSR point.
 *
 * In voluntary OSR, the JITed code does control when an OSR transition occurs,
 * allowing it to limit the OSR points with transitions.
 */
TR::OSRMode OMR::Compilation::getOSRMode() {
  if (self()->getOption(TR_FullSpeedDebug)) return TR::involuntaryOSR;
...
int32_t OMR::Compilation::getOSRInductionOffset(TR::Node *node) {
  ...
  if (!self()->isOSRTransitionTarget(TR::postExecutionOSR)) return 0;
  ...
  if (osrNode->getOpCode().isCall()) return 3;
  ...
  if (osrNode->getOpCodeValue() == TR::asynccheck) return 0;
...
bool OMR::Compilation::requiresAnalysisOSRPoint(TR::Node *node) {
  ...
  if (!self()->isOSRTransitionTarget(TR::postExecutionOSR)) return false;

  TR::Node *osrNode;
  if (!self()->isPotentialOSRPoint(node, &osrNode)) {
    TR_ASSERT(0,
              "requiresAnalysisOSRPoint should only be called on OSR points\n");
    return false;
  }

  // Calls require an analysis and transition point as liveness may change
  // across them
  if (osrNode->getOpCode().isCall()) return true;

  switch (osrNode->getOpCodeValue()) {
    // Monents only require a trailing OSR point as they will perform OSR when
    // executing the monitor and there is no change in liveness due to the
    // monent
    case TR::monent:
...
/**
 * To reduce OSR overhead, OSRLiveRangeAnalysis supports solving liveness
 * analysis during IlGen, as an approximation of this information is commonly
 * known at this stage.
 *
 * If this function returns true, OSRLiveRangeAnalysis expects pending push
 * liveness information to be stashed in OSRData using
 * addPendingPushLivenessInfo prior to its execution in IlGenOpts. Returning
 * true without the correct information stashed will result in reduced
 * performance and failed OSR transitions.
 *
 * If this function returns false, OSRLiveRangeAnalysis will perform liveness
 * analysis on pending push symbols.
 */
 ...
 * on block and value frequencies. isProfilingCompilation() should return true
 * for such a compilation and getProfilingMode() can distinguish between the
 * profiling implementations.
 */
bool OMR::Compilation::isProfilingCompilation() {
  return _recompilationInfo ? _recompilationInfo->isProfilingCompilation()
                            : false;
}

ProfilingMode OMR::Compilation::getProfilingMode() {
  if (!self()->isProfilingCompilation()) return DisabledProfiling;

  if (self()->getOption(TR_EnableJProfiling) ||
      !self()->getOption(TR_DisableJProfilingInProfilingCompilations))
    return JProfiling;
...
static int32_t strHash(const char *str) {
  ...
  int32_t result = 0;
  for (const unsigned char *s = reinterpret_cast<const unsigned char *>(str);
       *s; s++)
    result = result * 33 + *s;
  return result;
}
...
int32_t OMR::Compilation::compile() {
  if (!self()->getOption(TR_DisableSupportForCpuSpentInCompilation))
    _cpuTimeAtStartOfCompilation =
        TR::Compiler->vm.cpuTimeSpentInCompilationThread(self());

  bool printCodegenTime = self()->getOption(TR_CummTiming);

  if (self()->isOptServer()) self()->setOption(TR_DisablePartialInlining);
  ...
  if (self()->getOptions()->getDelayCompile()) {
    ...
    fprintf(stderr,
            "\nDelayCompile: Starting a delay of length %d for method %s at "
            "time %d.",
  ...
  if (self()->getOption(TR_DebugBeforeCompile)) {
    ...
    self()->getDebug()->setupDebugger((void *)*((long *)&(stopBeforeCompile)),
                                      (void *)*((long *)&(stopBeforeCompile)),
                                      true);
#else
    self()->getDebug()->setupDebugger((void *)&stopBeforeCompile,
                                      (void *)&stopBeforeCompile, true);
  ...
  if (self()->getOutFile() != NULL &&
      (self()->getOption(TR_TraceAll) || debug("traceStartCompile") ||
       self()->getOptions()->getAnyTraceCGOption() ||
       self()->getOption(TR_Timing))) {
    ...
    if (self()->getOption(TR_Randomize) || randomExercisePeriodStr != NULL)
      traceMsg(self(), "Random seed is %d%s\n", _options->getRandomSeed(),
               self()->getOption(TR_RandomSeedSignatureHash)
                   ? " hashed with signature"
                   : "");
    if (randomExercisePeriodStr != NULL) {
      auto period = atoi(randomExercisePeriodStr);
      if (period > 0) TR_RandomGenerator::exercise(period, self());
    }
  }

  if (printCodegenTime) compTime.startTiming(self());
  if (_recompilationInfo) _recompilationInfo->startOfCompilation();

  // Create the compile time profiler
  TR::CompileTimeProfiler perf(self(), "compileTimePerf");

  {
    TR::RegionProfiler rpIlgen(self()->trMemory()->heapMemoryRegion(), *self(),
                               "comp/ilgen");
    if (printCodegenTime) genILTime.startTiming(self());
    _ilGenSuccess = _methodSymbol->genIL(self()->fe(), self(),
                                         self()->getSymRefTab(), _ilGenRequest);
    if (printCodegenTime) genILTime.stopTiming(self());
  }

  // Force a crash during compilation if the crashDuringCompile option is set
  TR_ASSERT_FATAL(!self()->getOption(TR_CrashDuringCompilation),
                  "crashDuringCompile option is set");

  {
    LexicalTimer t("compile", self()->signature(), self()->phaseTimer());
    TR::LexicalMemProfiler mp("compile", self()->signature(),
                              self()->phaseMemProfiler());

    if (_ilGenSuccess) {
      _methodSymbol->detectInternalCycles(_methodSymbol->getFlowGraph(),
                                          self());
      ...
      if (_methodSymbol->catchBlocksHaveRealPredecessors(
              _methodSymbol->getFlowGraph(), self())) {
        self()->failCompilation<TR::CompilationException>(
            "Catch blocks have real predecessors");
      }

      if ((debug("dumpInitialTrees") || self()->getOption(TR_TraceTrees)) &&
          self()->getOutFile() != NULL) {
      ...
      {
        TR::RegionProfiler rpOpt(self()->trMemory()->heapMemoryRegion(),
                                 *self(), "comp/opt");
      ...
      if (self()->useCompressedPointers()) {
        if (self()->verifyCompressedRefsAnchors(true))
          ...
          dumpOptDetails(self(),
                         "failed while verifying compressedRefs anchors\n");
      }
#endif
      ...
      if (abortafterilgen) {
        self()->failCompilation<TR::CompilationException>(
            "Aborting after IL Gen due to TR_TOSS_IL");
      }

      if (_recompilationInfo) _recompilationInfo->beforeCodeGen();

      {
        TR::RegionProfiler rpCodegen(self()->trMemory()->heapMemoryRegion(),
                                     *self(), "comp/codegen");
      ...
      if (_recompilationInfo) _recompilationInfo->endOfCompilation();
      ...
      if (self()->getOptions()->getVerboseOption(TR_VerboseInlining)) {
        ...
        TR_VerboseLog::writeLine(
            TR_Vlog_INL, "%d methods inlined into %x %s @ %p",
            self()->getNumInlinedCallSites(), jittedBodyHash,
            self()->signature(), self()->cg()->getCodeStart());
        for (int32_t i = 0; i < self()->getNumInlinedCallSites(); i++) {
          ...
          if (callerIndex != -1) {
            int32_t callerLen = self()->fej9()->printTruncatedSignature(
                callerBuf, sizeof(callerBuf) - 1,
          ...
          TR_VerboseLog::writeLine(
              TR_Vlog_INL, "#%d: %x #%d inlined %x@%d -> %x bcsz=%d %s", i,
              jittedBodyHash, callerIndex, strHash(callerSig),
              site._byteCodeInfo.getByteCodeIndex(), strHash(calleeBuf),
              TR::Compiler->mtd.bytecodeSize(site._methodInfo), calleeBuf);
        }

        TR::Block *curBlock = NULL;
        for (TR::TreeTop *tt = self()->getStartTree(); tt;
          ...
          if (tt->getNode()->getOpCodeValue() == TR::BBStart) {
            ...
            if (node->getOpCode().isCall()) {
              ...
              if (method) {
                ...
                const char *calleeSig =
                    method->signature(self()->trMemory(), stackAlloc);
                uint32_t calleeSize = UINT_MAX;
                if (node->getSymbol()->getResolvedMethodSymbol())
                  calleeSize = TR::Compiler->mtd.bytecodeSize(
                      node->getSymbol()
                          ->getResolvedMethodSymbol()
                          ->getResolvedMethod()
                          ->getPersistentIdentifier());
                TR_VerboseLog::writeLine(
                    TR_Vlog_INL, "%x %s %x@%d -> %x bcsz=%d %s", jittedBodyHash,
  ...
  if (self()->getOption(TR_TraceCG)) {
    ...
    if (chTable) self()->getDebug()->dump(self()->getOutFile(), chTable);
  }
...
#else
    self()->getDebug()->setupDebugger((void *)jitTojitStart,
                                      self()->cg()->getCodeEnd(), false);
#endif /* defined(AIXPPC) */
  }
#elif defined(LINUX) || defined(J9ZOS390) || defined(OMR_OS_WINDOWS)
  if (self()->getOption(TR_DebugOnEntry)) {
    self()->getDebug()->setupDebugger(self()->cg()->getCodeStart(),
                                      self()->cg()->getCodeEnd(), false);
  }
...
int64_t OMR::Compilation::getCpuTimeSpentInCompilation() {
  if (_cpuTimeAtStartOfCompilation >=
      0)  // negative values means no support for compCPU
  {
    int64_t cpuTimeSpentInCompThread =
        TR::Compiler->vm.cpuTimeSpentInCompilationThread(self());
    return cpuTimeSpentInCompThread >= 0
               ? (cpuTimeSpentInCompThread - _cpuTimeAtStartOfCompilation)
               : -1;
  }
  return -1;
}
...
void OMR::Compilation::performOptimizations() {
  _optimizer = TR::Optimizer::createOptimizer(
      self(), self()->getJittedMethodSymbol(), false);

  if (_optimizer) _optimizer->optimize();
}

bool OMR::Compilation::incInlineDepth(TR::ResolvedMethodSymbol *method,
                                      TR_ByteCodeInfo &bcInfo, int32_t cpIndex,
                                      TR::SymbolReference *callSymRef,
                                      bool directCall,
                                      TR_PrexArgInfo *argInfo) {
  if (self()->compileRelocatableCode()) {
    TR_AOTMethodInfo *aotMethodInfo =
        (TR_AOTMethodInfo *)self()->trMemory()->allocateHeapMemory(
            sizeof(TR_AOTMethodInfo));
    aotMethodInfo->resolvedMethod = method->getResolvedMethod();
    aotMethodInfo->cpIndex = cpIndex;
    return self()->incInlineDepth((TR_OpaqueMethodBlock *)aotMethodInfo, method,
                                  bcInfo, callSymRef, directCall, argInfo);
  } else {
    return self()->incInlineDepth(
        method->getResolvedMethod()->getPersistentIdentifier(), method, bcInfo,
        callSymRef, directCall, argInfo);
  }
}
...
bool OMR::Compilation::foundOnTheStack(TR_ResolvedMethod *method,
                                       int32_t occurrences) {
  ...
  for (int32_t i = topIndex; i >= 0; --i) {
    ...
    if (matches && ++counter == occurrences) return true;
  }

  return false;
}

bool OMR::Compilation::incInlineDepth(TR_OpaqueMethodBlock *methodInfo,
                                      TR::ResolvedMethodSymbol *method,
  ...
  // This restriction is due to a limited number of bits allocated to
  // callerIndex in TR_ByteCodeInfo For example, in Java
  // TR_ByteCodeInfo::maxCallerIndex is set to 4095 (12 bits and one used for
  // signness)
  ...
  // in jsr292 "smart" inliner can run multiple times. we need to reconstruct a
  // callstack for all the callsites with the caller indices != -1 to make sure
  // each catch block can be uniquely identified by a (depthIndex, handlerIndex)
  // pair. otherwise JVM might print out an incorrect stacktrace if an exception
  // is thrown
  int16_t inlinedFramesAdded = self()->adjustInlineDepth(bcInfo);

  if (inlinedFramesAdded != 0) {
    TR_ASSERT(_inlinedFramesAdded == 0 &&
                  self()->getInlineDepth() == inlinedFramesAdded,
              "We can only perform the above inline stack correction on an "
              "empty stack");
    _inlinedFramesAdded = inlinedFramesAdded;
  }

  uint32_t callSiteIndex = _inlinedCallSites.add(TR_InlinedCallSiteInfo(
      methodInfo, bcInfo, method, callSymRef, directCall));
  ...
  if (inlinedCallStackSize >= TR_ByteCodeInfo::maxCallerIndex) {
    ...
    self()->failCompilation<TR::ExcessiveComplexity>(
        "max number of inlined calls exceeded");
  }
...
void OMR::Compilation::decInlineDepth(bool removeInlinedCallSitesEntry) {
  if (removeInlinedCallSitesEntry) {
    ...
    if (self()->getOption(TR_EnableOSR)) {
      // OSR's method data array needs to reflect the changes made to inline
      // table
      self()->getOSRCompilationData()->setOSRMethodDataArraySize(
          self()->getNumInlinedCallSites() + 1);
    }
  }
...
int16_t OMR::Compilation::matchingCallStackPrefixLength(
    TR_ByteCodeInfo &bcInfo) {
  if (bcInfo.getCallerIndex() == -1) return 0;

  int16_t callerIndex = bcInfo.getCallerIndex();
  TR_ByteCodeInfo &callerBCInfo =
      self()->getInlinedCallSite(callerIndex)._byteCodeInfo;
  int16_t callerPrefixLength =
      self()->matchingCallStackPrefixLength(callerBCInfo);

  int16_t result = callerPrefixLength;
  if (callerPrefixLength < self()->getInlineDepth()) {
    int32_t nextCallOnStack =
        self()->getInlinedCallStack().element(callerPrefixLength);
    if (nextCallOnStack == callerIndex) result++;
  }

  TR_ASSERT(
      result <= self()->getInlineDepth(),
      "matchingCallStackPrefixLength can be no greater than getInlineDepth");
  return result;
}

int16_t OMR::Compilation::restoreInlineDepthUntil(
    int32_t stopIndex, TR_ByteCodeInfo &currentInfo) {
  ...
  int16_t framesAdded = self()->restoreInlineDepthUntil(
      stopIndex, self()->getInlinedCallSite(callerIndex)._byteCodeInfo);
  _inlinedCallStack.push(callerIndex);
  _inlinedCallArgInfoStack.push(
      NULL);  // We have no argInfo for previously-inlined frames, though
              // there's not much we could do with it anyway

  return framesAdded + 1;
}

int16_t OMR::Compilation::restoreInlineDepth(TR_ByteCodeInfo &existingInfo) {
  TR_ASSERT((int32_t)existingInfo.getCallerIndex() <=
                (int32_t)self()->getNumInlinedCallSites(),
            "restoreInlineDepth requires valid caller index");
  int16_t numStackEntriesToKeep =
      self()->matchingCallStackPrefixLength(existingInfo);
  ...
  inlineFrameChange += self()->restoreInlineDepthUntil(
      self()->getCurrentInlinedSiteIndex(), existingInfo);
...
void OMR::Compilation::resetInlineDepth() {
  while (self()->getInlineDepth() > 0) self()->decInlineDepth();

  _inlinedFramesAdded = 0;
}

int32_t OMR::Compilation::convertNonDeterministicInput(
    int32_t i, int32_t max, TR_RandomGenerator *randomGenerator, int32_t min,
    bool emitVerbose) {
  int32_t answer = i;
  TR_PseudoRandomNumbersListElement *pseudoRandomList =
      self()->getPersistentInfo()->getPseudoRandomNumbersList();
  if (pseudoRandomList && self()->getOption(TR_VerbosePseudoRandom)) {
    ...
    if (self()->getOption(TR_Randomize)) {
      if (!randomGenerator) randomGenerator = _adhocRandom;
...
TR_DevirtualizedCallInfo *OMR::Compilation::getFirstDevirtualizedCall() {
  if (!_devirtualizedCalls.empty()) return _devirtualizedCalls.front();
  return NULL;
}

TR_DevirtualizedCallInfo *OMR::Compilation::createDevirtualizedCall(
    TR::Node *callNode, TR_OpaqueClassBlock *thisType) {
  TR_DevirtualizedCallInfo *dc =
      (TR_DevirtualizedCallInfo *)self()->trMemory()->allocateHeapMemory(
          sizeof(TR_DevirtualizedCallInfo));
...
TR_DevirtualizedCallInfo *OMR::Compilation::findDevirtualizedCall(
    TR::Node *callNode) {
  auto currDc = _devirtualizedCalls.begin();  // getFirstDevirtualizedCall();
  while (currDc != _devirtualizedCalls.end()) {
    if ((*currDc)->_callNode == callNode) return (*currDc);
    ++currDc;
  }
  return NULL;
}
...
TR_DevirtualizedCallInfo *OMR::Compilation::findOrCreateDevirtualizedCall(
  ...
  if (!dc ||
      self()->fe()->isInstanceOf(thisType, dc->_thisType, false) == TR_yes) {
    if (dc)
      dc->_thisType = thisType;
    else {
      TR_DevirtualizedCallInfo *newDc =
          self()->createDevirtualizedCall(callNode, thisType);
      dc = newDc;
    }
  }
  return dc;
...
TR_VirtualGuard *OMR::Compilation::findVirtualGuardInfo(TR::Node *guardNode) {
  TR_ASSERT(guardNode->isTheVirtualGuardForAGuardedInlinedCall() ||
                guardNode->isOSRGuard() || guardNode->isHCRGuard() ||
  ...
  if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
    traceMsg(self(),
             "Looking for a guard for node %p with kind %d bcindex %d "
             "calleeindex %d\n",
             guardNode, guardKind,
  ...
  if (guardKind != TR_NoGuard) {
    for (auto current = _virtualGuards.begin(); current != _virtualGuards.end();
         ++current) {
      // traceMsg(self(), "Looking at guard %p with kind %d bcindex %d
      // calleeindex %d\n", current, current->getKind(),
      // current->getByteCodeIndex(), current->getCalleeIndex());
      if ((*current)->getKind() == guardKind &&
          (*current)->getByteCodeIndex() ==
              guardNode->getByteCodeInfo().getByteCodeIndex() &&
          (*current)->getCalleeIndex() ==
              guardNode->getByteCodeInfo().getCallerIndex()) {
        guard = *current;
        if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
    ...
    for (auto current = _virtualGuards.begin(); current != _virtualGuards.end();
         ++current) {
      // traceMsg(self(), "Looking at guard %p kind %d bcindex %d calleeindex
      // %d\n", current, current->getKind(), current->getByteCodeIndex(),
      // current->getCalleeIndex());
      if ((*current)->getByteCodeIndex() ==
              guardNode->getByteCodeInfo().getByteCodeIndex() &&
          (*current)->getCalleeIndex() ==
              guardNode->getByteCodeInfo().getCallerIndex() &&
  ...
  TR_ASSERT(guard, "Can't find the virtual guard @ bci %d:%d, node %p \n",
            guardNode->getByteCodeInfo().getCallerIndex(),
            guardNode->getByteCodeInfo().getByteCodeIndex(), guardNode);
...
TR::Node *OMR::Compilation::createSideEffectGuard(TR::Compilation *comp,
                                                  TR::Node *node,
                                                  TR::TreeTop *destination) {
  return TR_VirtualGuard::createSideEffectGuard(comp, node, destination);
}

TR::Node *OMR::Compilation::createAOTInliningGuard(TR::Compilation *comp,
                                                   int16_t calleeIndex,
                                                   TR::Node *node,
                                                   TR::TreeTop *destination,
                                                   TR_VirtualGuardKind kind) {
  return TR_VirtualGuard::createAOTInliningGuard(comp, calleeIndex, node,
                                                 destination, kind);
}

TR::Node *OMR::Compilation::createAOTGuard(TR::Compilation *comp,
                                           int16_t calleeIndex, TR::Node *node,
                                           TR::TreeTop *destination,
                                           TR_VirtualGuardKind kind) {
  return TR_VirtualGuard::createAOTGuard(comp, calleeIndex, node, destination,
                                         kind);
}

TR::Node *OMR::Compilation::createDummyGuard(TR::Compilation *comp,
                                             int16_t calleeIndex,
                                             TR::Node *node,
                                             TR::TreeTop *destination) {
  return TR_VirtualGuard::createDummyGuard(comp, calleeIndex, node,
                                           destination);
}
...
// Are spine checks for array element accesses required in this compilation
// unit.
//
bool OMR::Compilation::requiresSpineChecks() {
...
TR_PrefetchInfo *OMR::Compilation::findExtraPrefetchInfo(TR::Node *node,
                                                         bool use) {
  for (auto it = self()->getExtraPrefetchInfo().begin();
       it != self()->getExtraPrefetchInfo().end(); ++it) {
    if (use) {
      if ((*it)->_useNode == node) return *it;
    } else {
      if ((*it)->_addrNode == node) return *it;
    }
  }
...
TR_OpaqueMethodBlock *OMR::Compilation::getMethodFromNode(TR::Node *node) {
  ...
  if (bcInfo.getCallerIndex() >= 0 && self()->getNumInlinedCallSites() > 0)
    method =
        self()->compileRelocatableCode()
            ? ((TR_ResolvedMethod *)node->getAOTMethod())
                  ->getPersistentIdentifier()
            : self()->getInlinedCallSite(bcInfo.getCallerIndex())._methodInfo;
  else
...
int32_t OMR::Compilation::getLineNumberInCurrentMethod(TR::Node *node) {
  ...
  if (callerIndex == -1) return self()->getLineNumber(node);
...
void OMR::Compilation::resetVisitCounts(vcount_t count) {
  if (self()->getMethodSymbol() == self()->getJittedMethodSymbol()) {
    ...
    for (auto current = _genILSyms.begin(); current != _genILSyms.end();
         ++current) {
      if ((*current) && (*current)->getFlowGraph() &&
          (*current) != self()->getMethodSymbol())
...
void OMR::Compilation::AddCopyPropagationRematerializationCandidate(
    TR::SymbolReference *sr) {
  _copyPropagationRematerializationCandidates[sr->getReferenceNumber()] = 1;
}

void OMR::Compilation::RemoveCopyPropagationRematerializationCandidate(
    TR::SymbolReference *sr) {
  _copyPropagationRematerializationCandidates[sr->getReferenceNumber()] = 0;
}

bool OMR::Compilation::IsCopyPropagationRematerializationCandidate(
    TR::SymbolReference *sr) {
  return (
      _copyPropagationRematerializationCandidates[sr->getReferenceNumber()] ==
      1);
}
...
TR::Block *OMR::Compilation::getStartBlock() {
  ...
  TR_ASSERT(startNode->getOpCodeValue() == TR::BBStart,
            "start node is not a BBStart");
...
bool OMR::Compilation::performVirtualGuardNOPing() {
  ...
  if (self()->getMethodHotness() < minLevel) return false;
  return true;
}

bool OMR::Compilation::isVirtualGuardNOPingRequired(
    TR_VirtualGuard *virtualGuard) {
  if (self()->isProfilingCompilation()) {
    if (!virtualGuard) {
      for (auto current = _virtualGuards.begin();
           current != _virtualGuards.end(); ++current)
        if (self()->isVirtualGuardNOPingRequired(*current)) return true;
      return false;
               ...
               ((virtualGuard->getKind() == TR_InterfaceGuard) &&
                (virtualGuard->getTestType() == TR_MethodTest)))
      return true;
    else
      return false;
  } else
...
void OMR::Compilation::setUsesPreexistence(bool v) {
  if (v)
    TR_ASSERT(self()->ilGenRequest().details().supportsInvalidation(),
              "Can't use preexistence on ilgen request that does not support "
              "invalidation");
  _usesPreexistence = v;
}
...
//
void OMR::Compilation::dumpMethodTrees(char *title,
  ...
  if (self()->isOutermostMethod() &&
      self()
          ->getKnownObjectTable())  // This is pretty verbose.  Let's just dump
                                    // it when we're dumping the whole method.
    self()->getKnownObjectTable()->dumpTo(self()->getOutFile(), self());

  trfflush(self()->getOutFile());
}

void OMR::Compilation::dumpMethodTrees(char *title1, const char *title2,
                                       TR::ResolvedMethodSymbol *methodSymbol) {
  TR::StackMemoryRegion stackMemoryRegion(*self()->trMemory());
  char *title = (char *)self()->trMemory()->allocateStackMemory(
      20 + strlen(title1) + strlen(title2));
...
void OMR::Compilation::dumpFlowGraph(TR::CFG *cfg) {
  ...
  if (debug("dumpCFG") || self()->getOption(TR_TraceTrees) ||
      self()->getOption(TR_TraceCG) || self()->getOption(TR_TraceUseDefs)) {
    if (cfg)
...
void OMR::Compilation::mapStaticAddressToCounter(
    TR::SymbolReference *symRef, TR::DebugCounterBase *counter) {
  const void *staticAddress = (const void *)symRef->getSymbol()
                                  ->castToStaticSymbol()
                                  ->getStaticAddress();
  self()->getDebugCounterMap().insert(std::make_pair(staticAddress, counter));
}

TR::DebugCounterBase *OMR::Compilation::getCounterFromStaticAddress(
    TR::SymbolReference *symRef) {
  const void *staticAddress = (const void *)symRef->getSymbol()
                                  ->castToStaticSymbol()
                                  ->getStaticAddress();
  TR::Compilation::DebugCounterMap::iterator entry =
      self()->getDebugCounterMap().find(staticAddress);
...
void OMR::Compilation::validateIL(TR::ILValidationContext ilValidationContext) {
  TR_ASSERT_FATAL(_ilValidator != NULL,
                  "Attempting to validate the IL without the ILValidator being "
                  "initialized");
...
void OMR::Compilation::verifyTrees(TR::ResolvedMethodSymbol *methodSymbol) {
  if (self()->getDebug() && !self()->getOption(TR_DisableVerification) &&
      !self()->isPeekingMethod()) {
    if (!methodSymbol) methodSymbol = _methodSymbol;
...
void OMR::Compilation::verifyAndFixRdbarAnchors() {
  ...
  for (TR::PreorderNodeIterator iter(self()->getStartTree(), self());
       iter.currentTree() != NULL; ++iter) {
    ...
    if (node->getOpCodeValue() == TR::treetop ||
      ...
      if (!anchoredRdbarNodes.contains(node)) {
        ...
        iter.currentTree()->insertBefore(
            TR::TreeTop::create(self(), newttNode));
        traceMsg(self(),
                 "node (n%dn) %p is an unanchored readbar, anchor it now under "
                 "treetop node (n%dn) %p\n",
                 node->getGlobalIndex(), node, newttNode->getGlobalIndex(),
                 newttNode);
      }
    }
  }
...
#ifdef DEBUG
void OMR::Compilation::dumpMethodGraph(int index,
                                       TR::ResolvedMethodSymbol *methodSymbol) {
  if (self()->getOutFile() == NULL) return;
...
void OMR::Compilation::shutdown(TR_FrontEnd *fe) {
  ...
  bool printCummStats =
      ((fe != 0) && TR::Options::getCmdLineOptions() &&
       TR::Options::getCmdLineOptions()->getOption(TR_CummTiming));
  if (printCummStats) {
  ...
  if (TR::Options::getCmdLineOptions() &&
    ...
    fprintf(stderr,
            "Statistics regarding time between 2 consective compilation yield "
            "points\n");
...
#ifdef OPT_TIMING
  fprintf(stderr,
          "Statistics for time taken by individual options. Time in ms\n");
...
void OMR::Compilation::registerResolvedMethodSymbolReference(
    TR::SymbolReference *symRef) {
  _resolvedMethodSymbolReferences[((TR::ResolvedMethodSymbol *)
                                       symRef->getSymbol())
                                      ->getResolvedMethodIndex()
                                      .value()] = symRef;
}
...
void OMR::Compilation::setReturnInfo(TR_ReturnInfo i) {
  ...
  //
  if (_method->isConstructor() && TR::Compiler->cls.hasFinalFieldsInClass(
                                      self(), _method->containingClass())) {
...
TR::ResolvedMethodSymbol *OMR::Compilation::getOwningMethodSymbol(
...
TR::ResolvedMethodSymbol *OMR::Compilation::getOwningMethodSymbol(
  ...
  for (int32_t i = _methodSymbols.size() - 1; i >= 0; --i)
    if (_methodSymbols[i]->getResolvedMethod()->getPersistentIdentifier() ==
        method)
      return _methodSymbols[i];
  return 0;
}
...
vcount_t OMR::Compilation::setVisitCount(vcount_t vc) {
...
vcount_t OMR::Compilation::incVisitCount() {
  if (_visitCount == MAX_VCOUNT - 1) {
    ...
    self()->failCompilation<TR::CompilationException>(
        "_visitCount equals MAX_VCOUNT-1");
  }
  return ++_visitCount;
}

vcount_t OMR::Compilation::incOrResetVisitCount() {
  if (_visitCount > HIGH_VISIT_COUNT) self()->resetVisitCounts(0);
  return self()->incVisitCount();
}

uint32_t OMR::Compilation::getNumInlinedCallSites() {
  return _inlinedCallSites.size();
}

TR_InlinedCallSite &OMR::Compilation::getInlinedCallSite(uint32_t index) {
  return _inlinedCallSites[index].site();
}

TR::ResolvedMethodSymbol *OMR::Compilation::getInlinedResolvedMethodSymbol(
    uint32_t index) {
...
bool OMR::Compilation::isInlinedDirectCall(uint32_t index) {
  return _inlinedCallSites[index].directCall();
}

/*
 * Get the number of pending push slots for a caller, as this will be the size
 * of the OSRCallSiteRemat table. In the event that the table has not been
 * initialized, it will return 0.
 */
uint32_t OMR::Compilation::getOSRCallSiteRematSize(uint32_t callSiteIndex) {
  if (!_inlinedCallSites[callSiteIndex].osrCallSiteRematTable()) return 0;

  int32_t callerIndex =
      self()->getInlinedCallSite(callSiteIndex)._byteCodeInfo.getCallerIndex();
  return callerIndex < 0 ? self()->getMethodSymbol()->getNumPPSlots()
                         : self()
                               ->getInlinedResolvedMethodSymbol(callerIndex)
                               ->getNumPPSlots();
}

/*
 * Get the pending push symbol reference and the corresponding load, to later
 * remat the pending push within OSR code blocks inside the callee. To get a
 * mapping, the call site index for the callee and the caller's pending push
 * slot should be provided.
 */
void OMR::Compilation::getOSRCallSiteRemat(uint32_t callSiteIndex,
                                           uint32_t slot,
                                           TR::SymbolReference *&ppSymRef,
  ...
  TR_ASSERT(slot < callerNumPPSlots,
            "can only perform call site remat for the caller's pending pushes");
#endif

  TR::SymbolReferenceTable *symRefTab = self()->getSymRefTab();
  ppSymRef =
      table[slot * 2] == 0 ? NULL : symRefTab->getSymRef(table[slot * 2]);
  loadSymRef = table[slot * 2 + 1] == 0
                   ? NULL
                   : symRefTab->getSymRef(table[slot * 2 + 1]);
}

/*
 * Set the pending push symbol reference and the corresponding load, to later
 * remat the pending push within OSR code blocks inside the callee. To correctly
 * add the entry, the callee's call site index and the caller's pending push
 * with its matching load should be provided.
 *
 * It is necessary to provide the pending push symbol reference, rather than
 * just its slot, as the slots may be shared. The information necessary to find
 * the symref from the slot is stored, as the call site should be an OSR point,
 * however the pending push symref is expected to be already known, so it is
 * cheaper to use directly.
 */
void OMR::Compilation::setOSRCallSiteRemat(uint32_t callSiteIndex,
                                           TR::SymbolReference *ppSymRef,
                                           TR::SymbolReference *loadSymRef) {
  int32_t *table = _inlinedCallSites[callSiteIndex].osrCallSiteRematTable();
  ...
  if (!table) {
    int32_t callerIndex = self()
                              ->getInlinedCallSite(callSiteIndex)
                              ._byteCodeInfo.getCallerIndex();
    uint32_t callerNumPPSlots =
        callerIndex < 0 ? self()->getMethodSymbol()->getNumPPSlots()
                        : self()
                              ->getInlinedResolvedMethodSymbol(callerIndex)
                              ->getNumPPSlots();
    table = (int32_t *)self()->trMemory()->allocateHeapMemory(
        callerNumPPSlots * 2 * sizeof(int32_t));
  ...
  uint32_t callerNumPPSlots = self()->getOSRCallSiteRematSize(callSiteIndex);
  TR_ASSERT(ppSymRef->getSymbol()->isPendingPush(),
            "can only perform call site remat on pending pushes");
  TR_ASSERT(slot >= 0 && slot < callerNumPPSlots,
            "can only perform call site remat for the caller's pending pushes");
#endif
...
/*
 * Check if it is not possible to perform an OSR transition during this call
 * site. A true result means transitioning within this method is definitely not
 * possible, however, a false result does not ensure OSR is possible.
 */
 ...
 */
void OMR::Compilation::setCannotAttemptOSRDuring(uint32_t index,
                                                 bool cannotOSR) {
  _inlinedCallSites[index].setCannotAttemptOSRDuring(cannotOSR);
}

TR_InlinedCallSite *OMR::Compilation::getCurrentInlinedCallSite() {
  return _inlinedCallStack.isEmpty()
             ? 0
             : &_inlinedCallSites[_inlinedCallStack.top()].site();
}
...
TR_PrexArgInfo *OMR::Compilation::getCurrentInlinedCallArgInfo() {
  return _inlinedCallArgInfoStack.isEmpty() ? 0
                                            : _inlinedCallArgInfoStack.top();
}

TR_Stack<TR_PrexArgInfo *> &OMR::Compilation::getInlinedCallArgInfoStack() {
  return _inlinedCallArgInfoStack;
}

TR_Stack<TR_PeekingArgInfo *> *OMR::Compilation::getPeekingArgInfo() {
...
void OMR::Compilation::addPeekingArgInfo(TR_PeekingArgInfo *info) {
...
TR::ResolvedMethodSymbol *OMR::Compilation::getMethodSymbol() {
  static const bool disableReturnCalleeInIlgen =
      feGetEnv("TR_DisableReturnCalleeInIlgen") ? true : false;
  if (self()->getCurrentIlGenerator() && !disableReturnCalleeInIlgen)
    return self()->getCurrentIlGenerator()->methodSymbol();
  if (self()->getOptimizer()) return self()->getOptimizer()->getMethodSymbol();
...
TR::CFG *OMR::Compilation::getFlowGraph() {
  return self()->getMethodSymbol()->getFlowGraph();
}

TR::TreeTop *OMR::Compilation::getStartTree() {
  return self()->getMethodSymbol()->getFirstTreeTop();
}

TR::TreeTop *OMR::Compilation::findLastTree() {
  return self()->getMethodSymbol()->getLastTreeTop();
}

bool OMR::Compilation::mayHaveLoops() {
  return self()->getMethodSymbol()->mayHaveLoops();
}

bool OMR::Compilation::hasNews() {
  return self()->getMethodSymbol()->hasNews();
}

TR::HCRMode OMR::Compilation::getHCRMode() {
  if (!self()->getOption(TR_EnableHCR)) return TR::none;
  if (self()->isDLT() || self()->isProfilingCompilation() ||
      self()->getOptLevel() <= cold)
    return TR::traditional;
  return self()->getOption(TR_EnableOSR) &&
                 !self()->getOption(TR_DisableNextGenHCR)
             ? TR::osr
             : TR::traditional;
}

uint32_t OMR::Compilation::getSymRefCount() {
  return self()->getSymRefTab()->getNumSymRefs();
}

void OMR::Compilation::setStartTree(TR::TreeTop *tt) {
  _methodSymbol->setFirstTreeTop(tt);
}

bool OMR::Compilation::isPICSite(TR::Instruction *instruction) {
  return (std::find(self()->getStaticPICSites()->begin(),
                    self()->getStaticPICSites()->end(),
                    instruction) != self()->getStaticPICSites()->end()) ||
         (std::find(self()->getStaticMethodPICSites()->begin(),
                    self()->getStaticMethodPICSites()->end(),
                    instruction) != self()->getStaticMethodPICSites()->end()) ||
         (std::find(self()->getStaticHCRPICSites()->begin(),
                    self()->getStaticHCRPICSites()->end(),
                    instruction) != self()->getStaticHCRPICSites()->end());
}
...
TR_Debug *OMR::Compilation::findOrCreateDebug() {
  if (!_debug) _debug = self()->fe()->createDebug(self());
  return _debug;
}
...
void OMR::Compilation::diagnosticImplVA(const char *s, va_list ap) {
  ...
  if (self()->getOutFile() != NULL) {
    ...
    TR::IO::vfprintf(self()->getOutFile(),
                     self()->getDebug()->getDiagnosticFormat(
                         s, buffer, sizeof(buffer) / sizeof(char)),
                     copy);
    trfflush(self()->getOutFile());
    va_end(copy);
  }
...
void addDebug(const char *debugString) {
  ...
  //
  char *t, *savedPointer;
  for (t = STRTOK(newString, delimiters, &savedPointer); t;
       t = STRTOK(0, " ", &savedPointer)) {
...
char *debug(const char *option) {
  ...
  if (!TR_DEBUGValue) {
    ...
    if (!TR_DEBUGValue) TR_DEBUGValue = "";
  ...
  if (head) {
    for (DebugValue *val = head;; val = val->next) {
      if (!strncmp(option, val->value, optionLen) &&
        ...
        head = val;
        if (val->value[optionLen]) ++optionLen;
        return val->value + optionLen;
      }
      if (val->next == head) break;
    }
  }
...
BitVectorPool::BitVectorPool(TR::Compilation *c)
    : _comp(c), _pool(c->trMemory()){};

TR_BitVector *BitVectorPool::get() {
  if (_pool.size() > 0) return _pool.pop();

  TR_BitVector *newBitVector = new (_comp->trHeapMemory())
      TR_BitVector(_comp->getNodeCount() /*an estimate*/, _comp->trMemory(),
                   heapAlloc, growable);
  return newBitVector;
}
...
void OMR::Compilation::restoreCompilationPhase(
    TR::Compilation::CompilationPhase phase) {
...
OMR::Compilation::CompilationPhaseScope::CompilationPhaseScope(
    TR::Compilation *comp)
    : _comp(comp), _savedPhase(comp->saveCompilationPhase()) {}
...
void OMR::Compilation::invalidateAliasRegion() {
  if (self()->_aliasRegion.bytesAllocated() >
      (ALIAS_REGION_LOAD_FACTOR)*TR::Region::initialSize()) {
...
bool OMR::Compilation::isRecursiveMethodTarget(
    TR_ResolvedMethod *targetResolvedMethod) {
  return targetResolvedMethod &&
...
bool OMR::Compilation::isRecursiveMethodTarget(TR::Symbol *targetSymbol) {
  ...
  if (targetSymbol) {
    TR::MethodSymbol *methodSymbol =
        targetSymbol->isMethod() ? targetSymbol->castToMethodSymbol() : NULL;
    TR::ResolvedMethodSymbol *resolvedSymbol =
        methodSymbol ? methodSymbol->getResolvedMethodSymbol() : NULL;
    TR_ResolvedMethod *resolvedMethod =
        resolvedSymbol ? resolvedSymbol->getResolvedMethod() : NULL;
...
const TR::TypeLayout *OMR::Compilation::typeLayout(TR_OpaqueClassBlock *clazz) {
  ...
  if (it != _typeLayoutMap.end()) {
    ...
    const TR::TypeLayout *layout =
        TR::Compiler->cls.enumerateFields(region, clazz, self());
