...
OMR::Compilation::Compilation(int32_t id, OMR_VMThread *omrVMThread,
    ...
    : _signature(compilee->signature(m)), _options(&options),
      _heapMemoryRegion(heapMemoryRegion), _trMemory(m), _fe(fe),
      _ilGenRequest(ilGenRequest), _currentOptIndex(0), _lastBegunOptIndex(0),
      _lastPerformedOptIndex(0),
      _currentOptSubIndex(0), // The transformation index within the current opt
      _lastPerformedOptSubIndex(0), _debug(0), _knownObjectTable(NULL),
      _omrVMThread(omrVMThread), _allocator(TRCS2MemoryAllocator(m)),
      _method(compilee),
      _arenaAllocator(TR::Allocator(self()->allocator("Arena"))),
      _aliasRegion(heapMemoryRegion), _allocatorName(NULL), _ilGenerator(0),
      _ilValidator(NULL), _optimizer(0), _currentSymRefTab(NULL),
      _recompilationInfo(0), _optimizationPlan(optimizationPlan),
      _primaryRandom(NULL), _adhocRandom(NULL), _methodSymbols(m, 10),
      _resolvedMethodSymbolReferences(m), _inlinedCallSites(m),
      _inlinedCallStack(m), _inlinedCallArgInfoStack(m),
      _devirtualizedCalls(
          getTypedAllocator<TR_DevirtualizedCallInfo *>(self()->allocator())),
      _inlinedCalls(0), _inlinedFramesAdded(0),
      ...
      _noEarlyInline(true), _returnInfo(TR_VoidReturn), _visitCount(0),
      _nodeCount(0), _accurateNodeCount(0), _lastValidNodeCount(0),
      _maxInlineDepth(0), _numLivePendingPushSlots(0), _numNestingLevels(0),
      _usesPreexistence(options.getOption(TR_ForceUsePreexistence)),
      _loopVersionedWrtAsyncChecks(false), _commitedCallSiteInfo(false),
      _containsBigDecimalLoad(false), _osrStateIsReliable(true),
      _canAffordOSRControlFlow(true), _osrInfrastructureRemoved(false),
      _toNumberMap(self()->allocator("toNumberMap")),
      _toStringMap(self()->allocator("toStringMap")),
      _toCommentMap(self()->allocator("toCommentMap")),
      _nextOptLevel(unknownHotness), _errorCode(COMPILATION_SUCCEEDED),
      _peekingArgInfo(m), _peekingSymRefTab(NULL),
      ...
      _currentBlock(NULL), _verboseOptTransformationCount(0),
      _relocatableMethodCodeStart(NULL), _compThreadID(id),
      ...
      _compilationNodes(NULL), _copyPropagationRematerializationCandidates(
                                   self()->allocator("CP rematerialization")),
      _nodeOpCodeLength(0), _prevSymRefTabSize(0),
      _scratchSpaceLimit(TR::Options::_scratchSpaceLimit),
      _cpuTimeAtStartOfCompilation(-1), _ilVerifier(NULL), _gpuPtxList(m),
      _gpuKernelLineNumberList(m), _gpuPtxCount(0), _bitVectorPool(self()),
    ...
    if (((self()->getMethodHotness() < warm) ||
        ...
        !options.getOption(
            TR_FullSpeedDebug)) // Off for two reasons : 1) not sure if we can
                                ...
                                // the extra OSR control flow at cold and 2) not
                                // sure at this stage in 727 whether OSR can
                                // work with AOT (will try to find out soon) but
                                // disabling till I do find out
...
OMR::Compilation::createJittedMethodSymbol(TR_ResolvedMethod *resolvedMethod) {
...
bool OMR::Compilation::supportsInduceOSR() {
  if (_osrInfrastructureRemoved) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(), "OSR induction cannot be performed after OSR "
  ...
  if (!self()->canAffordOSRControlFlow()) {
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(), "canAffordOSRControlFlow is false - OSR induction is "
                       "not supported\n");
    return false;
  }
  ...
  if (self()->getOption(TR_MimicInterpreterFrameShape) &&
    ...
    if (self()->getOption(TR_TraceOSR))
      traceMsg(self(), "MimicInterpreterFrameShape is set - OSR induction is "
                       "not supported\n");
    return false;
  }
...
bool OMR::Compilation::requiresAnalysisOSRPoint(TR::Node *node) {
  ...
  // executing the monitor and there is no change in liveness due to the monent
  case TR::monent:
...
int16_t
OMR::Compilation::matchingCallStackPrefixLength(TR_ByteCodeInfo &bcInfo) {
...
int16_t
OMR::Compilation::restoreInlineDepthUntil(int32_t stopIndex,
                                          TR_ByteCodeInfo &currentInfo) {
  ...
  _inlinedCallArgInfoStack.push(
      NULL); // We have no argInfo for previously-inlined frames, though there's
             // not much we could do with it anyway
...
TR_DevirtualizedCallInfo *
OMR::Compilation::createDevirtualizedCall(TR::Node *callNode,
...
TR_DevirtualizedCallInfo *
OMR::Compilation::findDevirtualizedCall(TR::Node *callNode) {
...
TR_DevirtualizedCallInfo *
OMR::Compilation::findOrCreateDevirtualizedCall(TR::Node *callNode,
                                                TR_OpaqueClassBlock *thisType) {
...
void OMR::Compilation::dumpMethodTrees(char *title,
  ...
  if (self()->isOutermostMethod() &&
      self()->getKnownObjectTable()) // This is pretty verbose.  Let's just dump
...
void OMR::Compilation::validateIL(TR::ILValidationContext ilValidationContext) {
  TR_ASSERT_FATAL(_ilValidator != NULL, "Attempting to validate the IL without "
                                        "the ILValidator being initialized");
...
void OMR::Compilation::shutdown(TR_FrontEnd *fe) {
  ...
  if (TR::Options::getCmdLineOptions() &&
    ...
    fprintf(stderr, "Statistics regarding time between 2 consective "
                    "compilation yield points\n");
