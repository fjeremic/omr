...
OMR::Compilation::Compilation(int32_t id,
    ...
    : _signature(compilee->signature(m)),
      ...
      _nodesThatShouldPrefetchOffset(
          getTypedAllocator<TR_Pair<TR::Node, uint32_t>*>(self()->allocator())),
      _extraPrefetchInfo(
      ...
      _phaseTimer("Compilation",
                  self()->allocator("phaseTimer"),
                  self()->getOption(TR_Timing)),
      _phaseMemProfiler("Compilation",
                        self()->allocator("phaseMemProfiler"),
...
void OMR::Compilation::freeKnownObjectTable() {
  _knownObjectTable = NULL;
}

int32_t OMR::Compilation::maxInternalPointers() {
  return 0;
}
...
int32_t OMR::Compilation::getOptLevel() {
 ...
 */
bool OMR::Compilation::pendingPushLivenessDuringIlgen() {
  return false;
}
...
bool OMR::Compilation::isJProfilingCompilation() {
  return false;
}
...
static int32_t strHash(const char* str) {
  ...
  int32_t result = 0;
  for (const unsigned char* s = reinterpret_cast<const unsigned char*>(str); *s;
       s++)
    result = result * 33 + *s;
  return result;
}
...
int32_t OMR::Compilation::convertNonDeterministicInput(
    int32_t i,
    int32_t max,
    TR_RandomGenerator* randomGenerator,
    int32_t min,
    bool emitVerbose) {
  int32_t answer = i;
...
void OMR::Compilation::dumpMethodTrees(char* title1,
                                       const char* title2,
                                       TR::ResolvedMethodSymbol* methodSymbol) {
...
vcount_t OMR::Compilation::getVisitCount() {
  return _visitCount;
}
...
void OMR::Compilation::removePeekingArgInfo() {
  _peekingArgInfo.pop();
}
