    ...
    , _tlsManager(*self())
{
    ...
    _isOptServer = (!options.getOption(TR_NoOptServer)) && (options.getOption(TR_Server)
#ifdef J9_PROJECT_SPECIFIC
...
bool OMR::Compilation::isPotentialOSRPoint(TR::Node* node, TR::Node** osrPointNode, bool ignoreInfra)
{
    ...
    if (self()->isOSRTransitionTarget(TR::postExecutionOSR)) {
        ...
        if (_osrInfrastructureRemoved && !ignoreInfra)
            ...
            } else if (callSymRef->getReferenceNumber() >= self()->getSymRefTab()->getNonhelperIndex(self()->getSymRefTab()->getLastCommonNonhelperSymbol())) {
...
bool OMR::Compilation::isPotentialOSRPointWithSupport(TR::TreeTop* tt)
{
    ...
    if (potentialOSRPoint && self()->getOSRMode() == TR::voluntaryOSR) {
        if (self()->isOSRTransitionTarget(TR::postExecutionOSR) && tt->getNode() != osrNode) {
            ...
            if (osrNode->getReferenceCount() > 1) {
                ...
                while (cursor) {
                    ...
                    if (cursor->getNode()->getOpCodeValue() == TR::BBStart && !cursor->getNode()->getBlock()->isExtensionOfPreviousBlock())
                        break;
        ...
        if (potentialOSRPoint) {
            ...
            TR::ResolvedMethodSymbol* method = bci.getCallerIndex() == -1 ? self()->getMethodSymbol() : self()->getInlinedResolvedMethodSymbol(bci.getCallerIndex());
...
OMR::Compilation::findVirtualGuardInfo(TR::Node* guardNode)
{
    ...
    if (guardKind != TR_NoGuard) {
        for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
            ...
            if ((*current)->getKind() == guardKind && (*current)->getByteCodeIndex() == guardNode->getByteCodeInfo().getByteCodeIndex() && (*current)->getCalleeIndex() == guardNode->getByteCodeInfo().getCallerIndex()) {
                guard = *current;
                if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
        ...
        for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
            ...
            if ((*current)->getByteCodeIndex() == guardNode->getByteCodeInfo().getByteCodeIndex() && (*current)->getCalleeIndex() == guardNode->getByteCodeInfo().getCallerIndex() && (*current)->getKind() != TR_SideEffectGuard && (*current)->getKind() != TR_HCRGuard && (*current)->getKind() != TR_OSRGuard && (*current)->getKind() != TR_MethodEnterExitGuard && (*current)->getKind() != TR_MutableCallSiteTargetGuard && (*current)->getKind() != TR_BreakpointGuard) {
                guard = *current;
                if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
...
{
    for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
        if (((*current)->getCalleeIndex() == guard->getCalleeIndex()) && ((*current)->getByteCodeIndex() == guard->getByteCodeIndex()) && ((*current)->getKind() == guard->getKind())) {
            if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
...
{
    if (!self()->getOptions()->realTimeGC() && !TR::Compiler->om.canGenerateArraylets() && self()->cg()->canTransformUnsafeCopyToArrayCopy())
        return true;
...
{
    if (!self()->getOptions()->realTimeGC() && !TR::Compiler->om.canGenerateArraylets() && self()->cg()->canTransformUnsafeSetMemory())
        return true;
...
{
    if (!self()->getRecompilationInfo() || !self()->cg()->getSupportsVirtualGuardNOPing() || self()->getOption(TR_DisableVirtualGuardNOPing) || self()->getOption(TR_DisableCHOpts))
        return false;
        ...
        if (!virtualGuard) {
            for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current)
                if (self()->isVirtualGuardNOPingRequired(*current))
                    return true;
            return false;
        } else if ((virtualGuard->getKind() == TR_SideEffectGuard) || (virtualGuard->getKind() == TR_DummyGuard) || (virtualGuard->getKind() == TR_OSRGuard) || (virtualGuard->getKind() == TR_HCRGuard) || (virtualGuard->getKind() == TR_MutableCallSiteTargetGuard) || ((virtualGuard->getKind() == TR_InterfaceGuard) && (virtualGuard->getTestType() == TR_MethodTest)))
            return true;
        else
            return false;
    } else
...
void OMR::Compilation::verifyAndFixRdbarAnchors()
{
    ...
    for (TR::PreorderNodeIterator iter(self()->getStartTree(), self()); iter.currentTree() != NULL; ++iter) {
        ...
        if (node->getOpCodeValue() == TR::treetop || node->getOpCode().isResolveOrNullCheck() || node->getOpCode().isAnchor()) {
...
OMR::Compilation::getOSRCallSiteRematSize(uint32_t callSiteIndex)
{
    ...
    return callerIndex < 0 ? self()->getMethodSymbol()->getNumPPSlots() : self()->getInlinedResolvedMethodSymbol(callerIndex)->getNumPPSlots();
}
 ...
 */
void OMR::Compilation::getOSRCallSiteRemat(uint32_t callSiteIndex, uint32_t slot, TR::SymbolReference*& ppSymRef, TR::SymbolReference*& loadSymRef)
{
 ...
 */
void OMR::Compilation::setOSRCallSiteRemat(uint32_t callSiteIndex, TR::SymbolReference* ppSymRef, TR::SymbolReference* loadSymRef)
{
    int32_t* table = _inlinedCallSites[callSiteIndex].osrCallSiteRematTable();
    ...
    if (!table) {
        int32_t callerIndex = self()->getInlinedCallSite(callSiteIndex)._byteCodeInfo.getCallerIndex();
        uint32_t callerNumPPSlots = callerIndex < 0 ? self()->getMethodSymbol()->getNumPPSlots() : self()->getInlinedResolvedMethodSymbol(callerIndex)->getNumPPSlots();
