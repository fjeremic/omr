...
class AliasBuilder {
 public:
  ...
  AliasBuilder(TR::SymbolReferenceTable *symRefTab, size_t sizeHintInElements,
               TR::Compilation *comp);
  ...
  TR::SymbolReference *getSymRefForAliasing(TR::Node *node,
                                            TR::Node *addrChild);
  ...
  void gatherLocalUseInfo(TR::Block *, TR_BitVector &,
                          TR_ScratchList<TR_Pair<TR::Block, TR_BitVector> > *,
                          vcount_t, bool);
 ...
 protected:
  ...
  struct CallAliases : TR_Link<CallAliases> {
    CallAliases(TR_BitVector *bv, TR::ResolvedMethodSymbol *m)
        : _callAliases(bv), _methodSymbol(m) {}
