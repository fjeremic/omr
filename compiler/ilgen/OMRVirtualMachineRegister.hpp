 ...
 *
 * The simulated register value is simply stored in a single local variable in
 * the native stack frame, which gives the compiler visibility to all changes to
 * the register (and enables dataflow optimization / simplification). Because
 * there is just a single local variable, the CopyState() and MergeInto()
 * functions do not need to do anything (the value can accessible from that
 * variable at all program locations). The Commit() and Reload() functions
 * simply move the value back and forth between the local variable and the
 * address of the actual virtual machine state variable.
 *
 * VirtualMachineRegister provides two additional functions:
 * Load() will load the *simulated* value of the register for use in the builder
 * "b" Store() will store the provided "value" into the *simulated* register by
...
class VirtualMachineRegister : public TR::VirtualMachineState {
 protected:
  /**
   * @brief constructor can be used by subclasses to initialize just the const
   * pointer
   * @param localName the name of the local variable where the simulated value
   * is to be stored
   */
  VirtualMachineRegister(const char *const localName)
      : TR::VirtualMachineState(), _localName(localName) {}

 public:
  /**
   * @brief public constructor used to create a virtual machine state variable
   * @param b a builder object where the first Reload operations will be
   * inserted
   * @param localName the name of the local variable where the simulated value
   * is to be stored
   ...
   * @param b the builder where the operation will be placed to store the
   * virtual machine value
   */
  virtual void Commit(TR::IlBuilder *b) {
    b->StoreAt(_addressOfRegister, b->Load(_localName));
  }

  /**
   * @brief transfer the current virtual machine register value to the simulated
   * local variable
   * @param b the builder where the operation will be placed to load the virtual
   * machine value
   */
  virtual void Reload(TR::IlBuilder *b) {
    b->Store(_localName, b->LoadAt((TR::IlType *)_pointerToRegisterType,
                                   _addressOfRegister));
  }
   ...
   * @param b the builder where the operation will be placed to load the
   * simulated value
  ...
  /**
   * @brief used in the compiled method to store to the (simulated) register's
   * value
   * @param b the builder where the operation will be placed to store to the
   * simulated value
   */
  ...
  /**
   * @brief used in the compiled method to add to the (simulated) register's
   * value
   * @param b the builder where the operation will be placed to add to the
   * simulated value
   * @param amount the TR::IlValue that should be added to the simulated value,
   * after multiplication by _adjustByStep Adjust() is really a convenience
   * function for the common operation of adding a value to the register. More
   * complicated operations (e.g. multiplying the value) can be built using
   * Load() and Store() if needed.
   */
  virtual void Adjust(TR::IlBuilder *b, TR::IlValue *amount) {
    TR::IlValue *off =
        b->Mul(b->ConvertTo(_integerTypeForAdjustments, amount),
   ...
   * @param b the builder where the operation will be placed to add to the
   * simulated value
   * @param amount the constant value that should be added to the simulated
   * value, after multiplication by _adjustByStep Adjust() is really a
   * convenience function for the common operation of adding a value to the
   * register. More complicated operations (e.g. multiplying the value) can be
   * built using Load() and Store() if needed.
   */
  virtual void Adjust(TR::IlBuilder *b, size_t amount) {
    adjust(b,
           b->ConstInteger(_integerTypeForAdjustments, amount * _adjustByStep));
  }
