...
OMR::IlBuilder::IlBuilder(TR::IlBuilder *source)
    : TR::IlInjector(source), _client(0), _clientCallbackBuildIL(0),
      _methodBuilder(source->_methodBuilder), _sequence(0),
      _sequenceAppender(0), _entryBlock(0), _exitBlock(0), _count(-1),
      _partOfSequence(false), _connectedTrees(false), _comesBack(true) {}
...
TR::IlValue *OMR::IlBuilder::Copy(TR::IlValue *value) {
  ...
  TraceIL("IlBuilder[ %p ]::Copy value (%d) dataType (%d) to newVal (%d) at "
          "cpIndex (%d)\n",
...
bool OMR::IlBuilder::connectTrees() {
  ...
  if (_methodBuilder != this || _methodBuilder->callerMethodBuilder() != NULL) {
    ...
    TraceIL("[ %p ] Connecting last tree %p [ node %p ] to exit block entry %p "
            "[ node %p ]\n",
...
TR::IlValue *OMR::IlBuilder::CreateLocalArray(int32_t numElements,
  ...
  TraceIL("IlBuilder[ %p ]::CreateLocalArray array allocated %d bytes, address "
          "in %d\n",
...
TR::IlValue *OMR::IlBuilder::CreateLocalStruct(TR::IlType *structType) {
  ...
  TraceIL("IlBuilder[ %p ]::CreateLocalStruct struct allocated %d bytes, "
          "address in %d\n",
...
void OMR::IlBuilder::Return(TR::IlValue *value) {
  ...
  if (returnBuilder != NULL) {
    ...
    TraceIL("IlBuilder[ %p ]::Return %d back to caller's returnBuilder [ %p ] "
            "via return symbol called %s\n",
...
TR::IlValue *OMR::IlBuilder::Sub(TR::IlValue *left, TR::IlValue *right) {
  ...
  if (left->getDataType() == TR::Address) {
    ...
    right = Sub(TR::Compiler->target.is32Bit() ? ConstInt32(0) : ConstInt64(0),
                right);
    returnValue = binaryOpFromNodes(TR::Compiler->target.is32Bit() ? TR::aiadd
                                                                   : TR::aladd,
                                    loadValue(left), loadValue(right));
  } else {
...
OMR::IlBuilder::MakeCondition(TR::IlBuilder *conditionBuilder,
                              TR::IlValue *conditionValue) {
...
void OMR::IlBuilder::ForLoop(bool countsUp, const char *indVar,
  ...
  *loopCode = createBuilderIfNeeded(*loopCode);
  ...
  TraceIL("IlBuilder[ %p ]::ForLoop ind %s initial %d end %d increment %d "
          "loopCode %p countsUp %d\n",
