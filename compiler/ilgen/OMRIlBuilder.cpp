...
TR::IlValue* OMR::IlBuilder::Copy(TR::IlValue* value)
{
  ...
  TraceIL("IlBuilder[ %p ]::Copy value (%d) dataType (%d) to newVal (%d) at cpIndex (%d)\n", this, value->getID(), dt,
    newVal->getID(), newSymRef->getCPIndex());
...
TR::IlValue* OMR::IlBuilder::Negate(TR::IlValue* v)
{
  ...
  TR_ASSERT_FATAL(
    negateOp != TR::BadILOp, "Builder [ %p ] cannot negate value %d of type %s", this, v->getID(), dataType.toString());
...
TR::IlValue* OMR::IlBuilder::ConvertBitsTo(TR::IlType* t, TR::IlValue* v)
{
  ...
  if (typeTo == typeFrom) {
    TraceIL(
      "IlBuilder[ %p ]::%d is ConvertBitsTo (already has type %s) %d\n", this, v->getID(), t->getName(), v->getID());
    return v;
  }
  ...
  TR_ASSERT_FATAL(convertOpcode != TR::BadILOp && TR::DataType::getSize(typeTo) == TR::DataType::getSize(typeFrom),
    "Builder [ %p ] requested bit conversion for value %d from type of size %d (%s) to type of size %d (%s) (consider "
...
void OMR::IlBuilder::Return()
{
  ...
  if (returnBuilder != NULL) {
    TR_ASSERT_FATAL(
      _methodBuilder->returnSymbol() == NULL, "Return() from inlined call did not expect a pre-existing returnSymbol");
...
TR::IlValue* OMR::IlBuilder::Sub(TR::IlValue* left, TR::IlValue* right)
{
  ...
  if (left->getDataType() == TR::Address) {
    ...
    returnValue
      = binaryOpFromNodes(TR::Compiler->target.is32Bit() ? TR::aiadd : TR::aladd, loadValue(left), loadValue(right));
  } else {
...
void OMR::IlBuilder::setHandlerInfo(uint32_t catchType)
{
  ...
  catchBlock->setHandlerInfoWithOutBCInfo(
    catchType, comp()->getInlineDepth(), -1, _methodSymbol->getResolvedMethod(), comp());
}

TR::Node* OMR::IlBuilder::genOverflowCHKTreeTop(TR::Node* operationNode, TR::ILOpCodes overflow)
{
  TR::Node* overflowChkNode
    = TR::Node::createWithRoomForOneMore(overflow, 3, symRefTab()->findOrCreateOverflowCheckSymbolRef(_methodSymbol),
      operationNode, operationNode->getFirstChild(), operationNode->getSecondChild());
...
TR::ILOpCodes OMR::IlBuilder::getOpCode(TR::IlValue* leftValue, TR::IlValue* rightValue)
{
  ...
  if (leftValue->getDataType() == TR::Address) {
    TR_ASSERT_FATAL((TR::Compiler->target.is64Bit() && rightValue->getDataType() == TR::Int64)
      ...
      "the right child type must be either TR::Int32 (on 32-bit ISA) or TR::Int64 (on 64-bit ISA) when the left child "
      "of Add is TR::Address\n");
...
TR::IlValue* OMR::IlBuilder::AddWithUnsignedOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
  ...
  TraceIL(
    "IlBuilder[ %p ]::%d is AddWithUnsignedOverflow %d + %d\n", this, addValue->getID(), left->getID(), right->getID());
  return addValue;
}
...
TR::IlValue* OMR::IlBuilder::UnsignedLessThan(TR::IlValue* left, TR::IlValue* right)
{
  ...
  TraceIL(
    "IlBuilder[ %p ]::%d is UnsignedLessThan %d < %d?\n", this, returnValue->getID(), left->getID(), right->getID());
  return returnValue;
}
...
TR::IlValue* OMR::IlBuilder::UnsignedGreaterThan(TR::IlValue* left, TR::IlValue* right)
{
  ...
  TraceIL(
    "IlBuilder[ %p ]::%d is UnsignedGreaterThan %d > %d?\n", this, returnValue->getID(), left->getID(), right->getID());
  return returnValue;
}
...
TR::IlValue* OMR::IlBuilder::GreaterOrEqualTo(TR::IlValue* left, TR::IlValue* right)
{
  ...
  TraceIL(
    "IlBuilder[ %p ]::%d is GreaterOrEqualTo %d >= %d?\n", this, returnValue->getID(), left->getID(), right->getID());
  return returnValue;
}
 ...
 *              |-->persistentFailurie
 */
void OMR::IlBuilder::Transaction(
  TR::IlBuilder** persistentFailureBuilder, TR::IlBuilder** transientFailureBuilder, TR::IlBuilder** transactionBuilder)
{
...
void OMR::IlBuilder::Switch(const char* selectionVar, TR::IlBuilder** defaultBuilder, uint32_t numCases, JBCase** cases)
{
...
void OMR::IlBuilder::Switch(const char* selectionVar, TR::IlBuilder** defaultBuilder, uint32_t numCases, ...)
{
  va_list args;
...
void OMR::IlBuilder::TableSwitch(
  const char* selectionVar, TR::IlBuilder** defaultBuilder, bool generateBoundsCheck, uint32_t numCases, JBCase** cases)
{
  TR::IlValue* selectorValue = Load(selectionVar);
...
void OMR::IlBuilder::TableSwitch(
  const char* selectionVar, TR::IlBuilder** defaultBuilder, bool generateBoundsCheck, uint32_t numCases, ...)
{
  va_list args;
  va_start(args, numCases);
  JBCase** cases = createCaseArray(numCases, args);
