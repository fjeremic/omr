...
void OMR::IlBuilder::initSequence() {
  _sequence = new (_comp->trMemory()->trHeapMemory())
      List<SequenceEntry>(_comp->trMemory());
  _sequenceAppender = new (_comp->trMemory()->trHeapMemory())
      ListAppender<SequenceEntry>(_sequence);
}
...
bool OMR::IlBuilder::injectIL() {
  ...
  rc = connectTrees();
  if (TraceEnabled) comp()->dumpMethodTrees("after connectTrees");
...
void OMR::IlBuilder::print(const char *title, bool recurse) {
  ...
  for (SequenceEntry *entry = iter.getFirst(); !iter.atEnd();
       entry = iter.getNext()) {
    if (entry->_isBlock) {
...
TR::SymbolReference *OMR::IlBuilder::lookupSymbol(const char *name) {
  TR_ASSERT_FATAL(
      _methodBuilder,
      "cannot look up symbols in an IlBuilder that has no MethodBuilder");
  return _methodBuilder->lookupSymbol(name);
}

void OMR::IlBuilder::defineSymbol(const char *name,
                                  TR::SymbolReference *symRef) {
  TR_ASSERT_FATAL(
      _methodBuilder,
      "cannot define symbols in an IlBuilder that has no MethodBuilder");
...
void OMR::IlBuilder::defineValue(const char *name, TR::IlType *type) {
  ...
  TR::SymbolReference *newSymRef =
      symRefTab()->createTemporary(methodSymbol(), dt);
...
TR::IlValue *OMR::IlBuilder::newValue(TR::DataType dt, TR::Node *n) {
  ...
  TR::IlValue *value = new (_comp->trHeapMemory())
      TR::IlValue(n, tt, _currentBlock, _methodBuilder);
  return value;
}

TR::IlValue *OMR::IlBuilder::newValue(TR::IlType *dt, TR::Node *n) {
...
TR::IlValue *OMR::IlBuilder::Copy(TR::IlValue *value) {
  ...
  TR::SymbolReference *newSymRef =
      symRefTab()->createTemporary(_methodSymbol, dt);
  ...
  TR::IlValue *newVal =
      newValue(newSymRef->getSymbol()->getDataType(), loadTemp(newSymRef));

  TraceIL(
      "IlBuilder[ %p ]::Copy value (%d) dataType (%d) to newVal (%d) at "
      "cpIndex (%d)\n",
...
uint32_t OMR::IlBuilder::countBlocks() {
  ...
  _count = 0;  // prevent recursive counting; will be updated to real value
               // before returning
...
void OMR::IlBuilder::pullInBuilderTrees(TR::IlBuilder *builder,
...
bool OMR::IlBuilder::connectTrees() {
  ...
  if (_connectedTrees) return true;
  ...
  for (SequenceEntry *entry = iter.getFirst(); !iter.atEnd();
    ...
    if (lastTree) {
      TraceIL(
          "[ %p ] Connecting tree %p [ node %p ] to new tree %p [ node %p ]\n",
  ...
  if (_methodBuilder != this || _methodBuilder->callerMethodBuilder() != NULL) {
    ...
    TraceIL(
        "[ %p ] Connecting last tree %p [ node %p ] to exit block entry %p [ "
        "node %p ]\n",
        this, lastTree, lastTree->getNode(), _exitBlock->getEntry(),
        _exitBlock->getEntry()->getNode());
    lastTree->join(_exitBlock->getEntry());

    // also add exit block to blocks array and add an edge from last block to
    // EXIT if the builder comes back (i.e. doesn't return or branch off
    // somewhere)
    if (comesBack()) cfg()->addEdge(blocks[currentBlock - 1], getExit());

    blocks[currentBlock++] = _exitBlock;
    TraceIL("[ %p ] exit block %d is %p (%p -> %p)\n", this,
            _exitBlock->getNumber(), _exitBlock->getEntry(),
            _exitBlock->getExit());
...
TR::Node *OMR::IlBuilder::zero(TR::IlType *dt) {
  return zero(dt->getPrimitiveType());
}

TR::Node *OMR::IlBuilder::zeroNodeForValue(TR::IlValue *v) {
...
TR::IlBuilder *OMR::IlBuilder::OrphanBuilder() {
  TR::IlBuilder *orphan =
      new (comp()->trHeapMemory()) TR::IlBuilder(_methodBuilder, _types);
  orphan->initialize(_details, _methodSymbol, _fe, _symRefTab);
  orphan->setupForBuildIL();
  return orphan;
}

TR::BytecodeBuilder *OMR::IlBuilder::OrphanBytecodeBuilder(int32_t bcIndex,
                                                           char *name) {
  TR::BytecodeBuilder *orphan = new (comp()->trHeapMemory())
      TR::BytecodeBuilder(_methodBuilder, bcIndex, name);
  orphan->initialize(_details, _methodSymbol, _fe, _symRefTab);
  orphan->setupForBuildIL();
  return orphan;
}
...
TR::IlBuilder *OMR::IlBuilder::createBuilderIfNeeded(TR::IlBuilder *builder) {
  if (builder == NULL) builder = OrphanBuilder();
  return builder;
}

OMR::IlBuilder::SequenceEntry *OMR::IlBuilder::blockEntry(TR::Block *block) {
  return new (_comp->trMemory()->trHeapMemory())
      IlBuilder::SequenceEntry(block);
}

OMR::IlBuilder::SequenceEntry *OMR::IlBuilder::builderEntry(
    TR::IlBuilder *builder) {
  return new (_comp->trMemory()->trHeapMemory())
      IlBuilder::SequenceEntry(builder);
}
...
void OMR::IlBuilder::appendBlock(TR::Block *newBlock, bool addEdge) {
  ...
  if (_currentBlock && addEdge) {
    // if current block does not fall through to new block, use
    // appendNoFallThroughBlock() rather than appendBlock()
...
void OMR::IlBuilder::AppendBuilder(TR::IlBuilder *builder) {
  TR_ASSERT_FATAL(builder->_partOfSequence == false,
                  "builder cannot be in two places");
  ...
  // when trees are connected, exit block will swing down to become last trees
  // in builder, so it can fall through to this block we're about to create need
  // to add edge explicitly because of this exit block sleight of hand
  appendNoFallThroughBlock();
  cfg()->addEdge(builder->getExit(), _currentBlock);
}

TR::Node *OMR::IlBuilder::loadValue(TR::IlValue *v) {
...
void OMR::IlBuilder::indirectStoreNode(TR::Node *addr, TR::Node *v) {
  ...
  TR::SymbolReference *storeSymRef =
      symRefTab()->findOrCreateArrayShadowSymbolRef(dt, addr);
...
TR::IlValue *OMR::IlBuilder::indirectLoadNode(TR::IlType *dt, TR::Node *addr,
                                              bool isVectorLoad) {
  TR_ASSERT_FATAL(dt->isPointer(),
                  "indirectLoadNode must apply to pointer type");
  ...
  if (isVectorLoad) symRefType = symRefType.scalarToVector();

  TR::SymbolReference *storeSymRef =
      symRefTab()->findOrCreateArrayShadowSymbolRef(symRefType, addr);
  ...
  TR::Node *loadNode =
      TR::Node::createWithSymRef(loadOp, 1, 1, addr, storeSymRef);
 ...
 * @param varName the name of the local variable to be stored into. If the name
 * has not been used before, this local variable will take the same data type as
 * the value being written to it.
 * @param value IlValue that should be written to the local variable, which
...
void OMR::IlBuilder::Store(const char *varName, TR::IlValue *value) {
  if (!_methodBuilder->symbolDefined(varName))
    _methodBuilder->defineValue(varName,
                                _types->PrimitiveType(value->getDataType()));
  TR::SymbolReference *symRef = lookupSymbol(varName);

  TraceIL("IlBuilder[ %p ]::Store %s %d (%d) gets %d\n", this, varName,
          symRef->getCPIndex(), symRef->getReferenceNumber(), value->getID());
 ...
 * @param varName the name of the local variable to be vector stored into. if
 * the name has not been used before, this local variable will take the same
 * data type as the value being written to it. The width of this data will be
 * determined by the vector data type of IlValue.
 * @param value IlValue with the vector data that should be written to the local
 * variable, and should have the same data type
 */
...
void OMR::IlBuilder::StoreAt(TR::IlValue *address, TR::IlValue *value) {
  TR_ASSERT_FATAL(address->getDataType() == TR::Address,
                  "StoreAt needs an address operand");

  TraceIL("IlBuilder[ %p ]::StoreAt address %d gets %d\n", this,
          address->getID(), value->getID());
  indirectStoreNode(loadValue(address), loadValue(value));
}

/**
 * @brief Store a vector IlValue through a pointer
 * @param address the pointer address through which the vector value will be
 * written. The width of the store will be determined by the vector data type of
 * IlValue.
 * @param value IlValue with the vector data that should be written  at
 * "address"
 */
void OMR::IlBuilder::VectorStoreAt(TR::IlValue *address, TR::IlValue *value) {
  TR_ASSERT_FATAL(address->getDataType() == TR::Address,
                  "VectorStoreAt needs an address operand");
...
TR::IlValue *OMR::IlBuilder::CreateLocalArray(int32_t numElements,
                                              TR::IlType *elementType) {
  uint32_t size = numElements * elementType->getSize();
  TR::SymbolReference *localArraySymRef = symRefTab()->createLocalPrimArray(
      size, methodSymbol(), 8 /*FIXME: JVM-specific - byte*/);
  char *name = (char *)_comp->trMemory()->allocateHeapMemory(
      (2 + 10 + 1) * sizeof(char));  // 2 ("_T") + max 10 digits + trailing zero
  ...
  TR::Node *arrayAddress =
      TR::Node::createWithSymRef(TR::loadaddr, 0, localArraySymRef);
  TR::IlValue *arrayAddressValue = newValue(TR::Address, arrayAddress);

  TraceIL(
      "IlBuilder[ %p ]::CreateLocalArray array allocated %d bytes, address in "
      "%d\n",
...
TR::IlValue *OMR::IlBuilder::CreateLocalStruct(TR::IlType *structType) {
  // similar to CreateLocalArray except writing a method in StructType to get
  // the struct size
  uint32_t size = structType->getSize();
  TR::SymbolReference *localStructSymRef = symRefTab()->createLocalPrimArray(
      size, methodSymbol(), 8 /*FIXME: JVM-specific - byte*/);
  char *name = (char *)_comp->trMemory()->allocateHeapMemory(
      (2 + 10 + 1) * sizeof(char));  // 2 ("_T") + max 10 digits + trailing zero
  ...
  TR::Node *structAddress =
      TR::Node::createWithSymRef(TR::loadaddr, 0, localStructSymRef);
  TR::IlValue *structAddressValue = newValue(TR::Address, structAddress);

  TraceIL(
      "IlBuilder[ %p ]::CreateLocalStruct struct allocated %d bytes, address "
      "in %d\n",
      this, size, structAddressValue->getID());
  return structAddressValue;
}

void OMR::IlBuilder::StoreIndirect(const char *type, const char *field,
                                   TR::IlValue *object, TR::IlValue *value) {
  ...
  TraceIL("IlBuilder[ %p ]::StoreIndirect %s.%s (%d) into (%d)\n", this, type,
          field, value->getID(), object->getID());
  TR::ILOpCodes storeOp = comp()->il.opCodeForIndirectStore(fieldType);
  genTreeTop(TR::Node::createWithSymRef(storeOp, 2, loadValue(object),
                                        loadValue(value), 0, symRef));
}
...
TR::IlValue *OMR::IlBuilder::Load(const char *name) {
  ...
  TR::IlValue *returnValue =
      newValue(symRef->getSymbol()->getDataType(), valueNode);
  TraceIL("IlBuilder[ %p ]::Load %s into %d from symref %d\n", this, name,
          returnValue->getID(), symRef->getReferenceNumber());
  return returnValue;
}
...
TR::IlValue *OMR::IlBuilder::VectorLoad(const char *name) {
  ...
  TR_ASSERT_FATAL(returnType.isVector(),
                  "VectorLoad must load symbol with a vector type");

  TR::Node *loadNode = TR::Node::createWithSymRef(
      0, TR::comp()->il.opCodeForDirectLoad(returnType), 0, nameSymRef);
  TR::IlValue *returnValue = newValue(returnType, loadNode);
  TraceIL("IlBuilder[ %p ]::%d is VectorLoad %s (%d)\n", this,
          returnValue->getID(), name, nameSymRef->getCPIndex());

  return returnValue;
}

TR::IlValue *OMR::IlBuilder::LoadIndirect(const char *type, const char *field,
                                          TR::IlValue *object) {
  TR::IlReference *fieldRef = _types->FieldReference(type, field);
  TR::SymbolReference *symRef = fieldRef->symRef();
  TR::DataType fieldType = symRef->getSymbol()->getDataType();
  TR::IlValue *returnValue = newValue(
      fieldType,
      TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(fieldType), 1,
                                 loadValue(object), 0, symRef));
  TraceIL("IlBuilder[ %p ]::%d is LoadIndirect %s.%s from (%d)\n", this,
          returnValue->getID(), type, field, object->getID());
  return returnValue;
}

TR::IlValue *OMR::IlBuilder::LoadAt(TR::IlType *dt, TR::IlValue *address) {
  TR_ASSERT_FATAL(address->getDataType() == TR::Address,
                  "LoadAt needs an address operand");
  TR::IlValue *returnValue = indirectLoadNode(dt, loadValue(address));
  TraceIL("IlBuilder[ %p ]::%d is LoadAt type %d address %d\n", this,
          returnValue->getID(), dt->getPrimitiveType(), address->getID());
  return returnValue;
}

TR::IlValue *OMR::IlBuilder::VectorLoadAt(TR::IlType *dt,
                                          TR::IlValue *address) {
  TR_ASSERT_FATAL(address->getDataType() == TR::Address,
                  "LoadAt needs an address operand");
...
TR::IlValue *OMR::IlBuilder::IndexAt(TR::IlType *dt, TR::IlValue *base,
                                     TR::IlValue *index) {
  TR::IlType *elemType = dt->baseType();
  TR_ASSERT_FATAL(base->getDataType() == TR::Address,
                  "IndexAt must be called with a pointer base");
  TR_ASSERT_FATAL(elemType != NULL,
                  "IndexAt should be called with pointer type");
  TR_ASSERT_FATAL(elemType->getPrimitiveType() != TR::NoType,
  ...
  TraceIL("IlBuilder[ %p ]::%d is IndexAt(%s) base %d index %d\n", this,
          address->getID(), dt->getName(), base->getID(), index->getID());
 ...
 */
TR::IlValue *OMR::IlBuilder::StructFieldInstanceAddress(const char *structName,
                                                        const char *fieldName,
                                                        TR::IlValue *obj) {
  auto offset = typeDictionary()->OffsetOf(structName, fieldName);
  auto ptype = typeDictionary()->PointerTo(
      typeDictionary()->GetFieldType(structName, fieldName));
 ...
 */
TR::IlValue *OMR::IlBuilder::UnionFieldInstanceAddress(const char *unionName,
                                                       const char *fieldName,
                                                       TR::IlValue *obj) {
  auto ptype = typeDictionary()->PointerTo(
      typeDictionary()->UnionFieldType(unionName, fieldName));
...
TR::IlValue *OMR::IlBuilder::ConstDouble(double value) {
  ...
  TraceIL("IlBuilder[ %p ]::%d is ConstDouble %lf\n", this,
          returnValue->getID(), value);
  return returnValue;
}

TR::IlValue *OMR::IlBuilder::ConstString(const char *const value) {
  TR::IlValue *returnValue =
      newValue(Address, TR::Node::aconst((uintptrj_t)value));
...
TR::IlValue *OMR::IlBuilder::UnsignedConvertTo(TR::IlType *t, TR::IlValue *v) {
  ...
  if (typeFrom == typeTo) {
    TraceIL(
        "IlBuilder[ %p ]::%d is UnsignedConvertTo (already has type %s) %d\n",
...
TR::IlValue *OMR::IlBuilder::Negate(TR::IlValue *v) {
  ...
  TR_ASSERT_FATAL(negateOp != TR::BadILOp,
                  "Builder [ %p ] cannot negate value %d of type %s", this,
                  v->getID(), dataType.toString());

  TR::Node *result = TR::Node::create(negateOp, 1, loadValue(v));
  TR::IlValue *negatedValue = newValue(dataType, result);
  TraceIL("IlBuilder[ %p ]::%d is Negated %d\n", this, negatedValue->getID(),
          v->getID());
  return negatedValue;
}

TR::IlValue *OMR::IlBuilder::convertTo(TR::DataType typeTo, TR::IlValue *v,
                                       bool needUnsigned) {
  TR::DataType typeFrom = v->getDataType();

  TR::ILOpCodes convertOp =
      ILOpCode::getProperConversion(typeFrom, typeTo, needUnsigned);
  TR_ASSERT_FATAL(
      convertOp != TR::BadILOp,
      "Builder [ %p ] unknown conversion requested for value %d %s to %s", this,
...
TR::IlValue *OMR::IlBuilder::ConvertBitsTo(TR::IlType *t, TR::IlValue *v) {
  ...
  TR::ILOpCodes convertOpcode =
      TR::DataType::getDataTypeBitConversion(typeFrom, typeTo);
  TR_ASSERT_FATAL(
      convertOpcode != TR::BadILOp &&
          TR::DataType::getSize(typeTo) == TR::DataType::getSize(typeFrom),
      "Builder [ %p ] requested bit conversion for value %d from type of size "
      "%d (%s) to type of size %d (%s) (consider using ConvertTo() to for "
      "narrowing/widening)",
      this, v->getID(), TR::DataType::getSize(typeFrom), typeFrom.toString(),
      TR::DataType::getSize(typeTo), typeTo.toString());
  TR_ASSERT_FATAL(convertOpcode != TR::BadILOp,
                  "Builder [ %p ] unknown bit conversion requested for value "
                  "%d (%s) to type %s",
                  this, v->getID(), typeFrom.toString(), t->getName());
...
void OMR::IlBuilder::doVectorConversions(TR::Node **leftPtr,
                                         TR::Node **rightPtr) {
...
TR::Node *OMR::IlBuilder::binaryOpNodeFromNodes(TR::ILOpCodes op,
                                                TR::Node *leftNode,
                                                TR::Node *rightNode) {
  ...
  TR_ASSERT_FATAL(leftType == rightType || isAddressBump || isRevAddressBump,
                  "binaryOp requires both left and right operands to have same "
                  "type or one is address and other is Int32/64");
...
TR::IlValue *OMR::IlBuilder::binaryOpFromNodes(TR::ILOpCodes op,
                                               TR::Node *leftNode,
                                               TR::Node *rightNode) {
...
TR::IlValue *OMR::IlBuilder::binaryOpFromOpMap(OpCodeMapper mapOp,
                                               TR::IlValue *left,
                                               TR::IlValue *right) {
...
TR::IlValue *OMR::IlBuilder::binaryOpFromOpCode(TR::ILOpCodes op,
                                                TR::IlValue *left,
                                                TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
...
TR::IlValue *OMR::IlBuilder::compareOp(TR_ComparisonTypes ct, bool needUnsigned,
                                       TR::IlValue *left, TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::ILOpCodes op =
      TR::ILOpCode::compareOpCode(leftNode->getDataType(), ct, needUnsigned);
...
TR::IlValue *OMR::IlBuilder::NotEqualTo(TR::IlValue *left, TR::IlValue *right) {
  ...
  TraceIL("IlBuilder[ %p ]::%d is NotEqualTo %d != %d?\n", this,
          returnValue->getID(), left->getID(), right->getID());
  return returnValue;
}
...
void OMR::IlBuilder::Goto(TR::IlBuilder *dest) {
  TR_ASSERT_FATAL(
      dest != NULL,
      "This goto implementation requires a non-NULL builder object");
...
void OMR::IlBuilder::Return() {
  ...
  if (returnBuilder != NULL) {
    TR_ASSERT_FATAL(_methodBuilder->returnSymbol() == NULL,
                    "Return() from inlined call did not expect a pre-existing "
                    "returnSymbol");
  ...
  if (returnBuilder != NULL) {
    ...
    if (returnSymbol == NULL) {
      ...
      returnSymbol = (char *)_comp->trMemory()->allocateHeapMemory(
          (3 + 10 + 1) *
          sizeof(char));  // 3 ("_RV") + max 10 digits + trailing zero
      ...
      // also proactively define this symbol in the caller, so that it can
      // access it after the inlined body!
      _methodBuilder->callerMethodBuilder()->defineSymbol(returnSymbol,
                                                          newSymRef);
    }

    TraceIL(
        "IlBuilder[ %p ]::Return %d back to caller's returnBuilder [ %p ] via "
        "return symbol called %s\n",
    ...
    TR::Node *returnNode = TR::Node::create(
        TR::ILOpCode::returnOpCode(value->getDataType()), 1, loadValue(value));
    genTreeTop(returnNode);
    cfg()->addEdge(_currentBlock, cfg()->getEnd());
    setDoesNotComeBack();
  }
  ...
  if (left->getDataType() == TR::Address) {
    if (TR::Compiler->target.is64Bit() && right->getDataType() == TR::Int32) {
      right = unaryOp(TR::i2l, right);
               ...
               right->getDataType() == TR::Int64) {
      right = unaryOp(TR::l2i, right);
    }
    right = Sub(TR::Compiler->target.is32Bit() ? ConstInt32(0) : ConstInt64(0),
                right);
    returnValue = binaryOpFromNodes(
        TR::Compiler->target.is32Bit() ? TR::aiadd : TR::aladd, loadValue(left),
        loadValue(right));
  } else {
 ...
 */
void OMR::IlBuilder::appendExceptionHandler(TR::Block *blockThrowsException,
                                            TR::IlBuilder **handler,
                                            uint32_t catchType) {
  // split block after overflowCHK, and add an goto to
  // blockAfterExceptionHandler
  appendBlock();
...
void OMR::IlBuilder::setHandlerInfo(uint32_t catchType) {
  ...
  catchBlock->setHandlerInfoWithOutBCInfo(
      catchType, comp()->getInlineDepth(), -1,
      _methodSymbol->getResolvedMethod(), comp());
}

TR::Node *OMR::IlBuilder::genOverflowCHKTreeTop(TR::Node *operationNode,
                                                TR::ILOpCodes overflow) {
  TR::Node *overflowChkNode = TR::Node::createWithRoomForOneMore(
      overflow, 3,
      symRefTab()->findOrCreateOverflowCheckSymbolRef(_methodSymbol),
      operationNode, operationNode->getFirstChild(),
...
TR::IlValue *OMR::IlBuilder::genOperationWithOverflowCHK(
    TR::ILOpCodes op, TR::Node *leftNode, TR::Node *rightNode,
  ...
  TR::IlValue *resultValue =
      newValue(operationNode->getDataType(), operationNode);
  genTreeTop(
      TR::Node::createStore(resultValue->getSymbolReference(), operationNode));

  appendExceptionHandler(blockWithOverflowCHK, handler,
...
TR::ILOpCodes OMR::IlBuilder::getOpCode(TR::IlValue *leftValue,
                                        TR::IlValue *rightValue) {
  TR::ILOpCodes op;
  if (leftValue->getDataType() == TR::Address) {
    TR_ASSERT_FATAL((TR::Compiler->target.is64Bit() &&
                     rightValue->getDataType() == TR::Int64) ||
                        (TR::Compiler->target.is32Bit() &&
                         rightValue->getDataType() == TR::Int32),
                    "the right child type must be either TR::Int32 (on 32-bit "
                    "ISA) or TR::Int64 (on 64-bit ISA) when the left child of "
                    "Add is TR::Address\n");
...
TR::IlValue *OMR::IlBuilder::AddWithOverflow(TR::IlBuilder **handler,
                                             TR::IlValue *left,
                                             TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::ILOpCodes opcode = getOpCode(left, right);
  TR::IlValue *addValue = genOperationWithOverflowCHK(
      opcode, leftNode, rightNode, handler, TR::OverflowCHK);
  TraceIL("IlBuilder[ %p ]::%d is AddWithOverflow %d + %d\n", this,
          addValue->getID(), left->getID(), right->getID());
  return addValue;
}

TR::IlValue *OMR::IlBuilder::AddWithUnsignedOverflow(TR::IlBuilder **handler,
                                                     TR::IlValue *left,
                                                     TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::ILOpCodes opcode = getOpCode(left, right);
  TR::IlValue *addValue = genOperationWithOverflowCHK(
      opcode, leftNode, rightNode, handler, TR::UnsignedOverflowCHK);
  TraceIL("IlBuilder[ %p ]::%d is AddWithUnsignedOverflow %d + %d\n", this,
          addValue->getID(), left->getID(), right->getID());
  return addValue;
}

TR::IlValue *OMR::IlBuilder::SubWithOverflow(TR::IlBuilder **handler,
                                             TR::IlValue *left,
                                             TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::IlValue *subValue = genOperationWithOverflowCHK(
      TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode,
      rightNode, handler, TR::OverflowCHK);
  TraceIL("IlBuilder[ %p ]::%d is SubWithOverflow %d + %d\n", this,
          subValue->getID(), left->getID(), right->getID());
  return subValue;
}

TR::IlValue *OMR::IlBuilder::SubWithUnsignedOverflow(TR::IlBuilder **handler,
                                                     TR::IlValue *left,
                                                     TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::IlValue *unsignedSubValue = genOperationWithOverflowCHK(
      TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode,
      rightNode, handler, TR::UnsignedOverflowCHK);
  TraceIL("IlBuilder[ %p ]::%d is UnsignedSubWithOverflow %d + %d\n", this,
          unsignedSubValue->getID(), left->getID(), right->getID());
  return unsignedSubValue;
}

TR::IlValue *OMR::IlBuilder::MulWithOverflow(TR::IlBuilder **handler,
                                             TR::IlValue *left,
                                             TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::IlValue *mulValue = genOperationWithOverflowCHK(
      TR::ILOpCode::multiplyOpCode(leftNode->getDataType()), leftNode,
      rightNode, handler, TR::OverflowCHK);
  TraceIL("IlBuilder[ %p ]::%d is MulWithOverflow %d + %d\n", this,
          mulValue->getID(), left->getID(), right->getID());
  return mulValue;
}

TR::IlValue *OMR::IlBuilder::Mul(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::multiplyOpCode, left, right);
...
TR::IlValue *OMR::IlBuilder::Div(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::divideOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is Div %d / %d\n", this, returnValue->getID(),
          left->getID(), right->getID());
  return returnValue;
}

TR::IlValue *OMR::IlBuilder::UnsignedDiv(TR::IlValue *left,
                                         TR::IlValue *right) {
  ...
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::unsignedDivideOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is UnsignedDiv %d / %d\n", this,
          returnValue->getID(), left->getID(), right->getID());

  if (returnValue->getDataType() != returnType)
    returnValue =
        UnsignedConvertTo(_types->PrimitiveType(returnType), returnValue);
...
TR::IlValue *OMR::IlBuilder::Rem(TR::IlValue *left, TR::IlValue *right) {
  ...
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::remainderOpCode, left, right);
...
TR::IlValue *OMR::IlBuilder::UnsignedRem(TR::IlValue *left,
                                         TR::IlValue *right) {
  TR::DataType returnType = left->getDataType();
  TR::IlValue *returnValue;
  ...
  if (returnType.isInt64()) {
    ...
    TR::IlValue *truncLeft = binaryOpFromOpCode(
        TR::ILOpCodes::lmul,
        binaryOpFromOpCode(TR::ILOpCodes::ludiv, left, right), right);
...
TR::IlValue *OMR::IlBuilder::And(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::andOpCode, left, right);
...
TR::IlValue *OMR::IlBuilder::Or(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::orOpCode, left, right);
...
TR::IlValue *OMR::IlBuilder::Xor(TR::IlValue *left, TR::IlValue *right) {
  TR::IlValue *returnValue =
      binaryOpFromOpMap(TR::ILOpCode::xorOpCode, left, right);
  TraceIL("IlBuilder[ %p ]::%d is Xor %d ^ %d\n", this, returnValue->getID(),
          left->getID(), right->getID());
  return returnValue;
}

TR::Node *OMR::IlBuilder::shiftOpNodeFromNodes(TR::ILOpCodes op,
                                               TR::Node *leftNode,
                                               TR::Node *rightNode) {
  TR::DataType leftType = leftNode->getDataType();
  TR::DataType rightType = rightNode->getDataType();
  TR_ASSERT_FATAL(leftType.isIntegral() && rightType.isInt32(),
                  "shift operation first operand must be an integer, and shift "
                  "amount must be 32-bit integer");

  return TR::Node::create(op, 2, leftNode, rightNode);
}

TR::IlValue *OMR::IlBuilder::shiftOpFromNodes(TR::ILOpCodes op,
                                              TR::Node *leftNode,
                                              TR::Node *rightNode) {
...
TR::IlValue *OMR::IlBuilder::shiftOpFromOpMap(OpCodeMapper mapOp,
                                              TR::IlValue *left,
                                              TR::IlValue *right) {
  TR::Node *leftNode = loadValue(left);
  TR::DataType leftType = leftNode->getDataType();
  TR_ASSERT_FATAL(leftType.isIntegral(),
                  "left operand of shift must be integer type");

  TR::Node *rightNode = loadValue(right);
  if (!rightNode->getDataType().isInt32()) right = ConvertTo(Int32, right);
...
TR::IlValue *OMR::IlBuilder::ShiftL(TR::IlValue *v, TR::IlValue *amount) {
  TR::IlValue *returnValue =
      shiftOpFromOpMap(TR::ILOpCode::shiftLeftOpCode, v, amount);
  TraceIL("IlBuilder[ %p ]::%d is shr %d << %d\n", this, returnValue->getID(),
          v->getID(), amount->getID());
  return returnValue;
}

TR::IlValue *OMR::IlBuilder::ShiftR(TR::IlValue *v, TR::IlValue *amount) {
  TR::IlValue *returnValue =
      shiftOpFromOpMap(TR::ILOpCode::shiftRightOpCode, v, amount);
  TraceIL("IlBuilder[ %p ]::%d is shr %d >> %d\n", this, returnValue->getID(),
          v->getID(), amount->getID());
  return returnValue;
}

TR::IlValue *OMR::IlBuilder::UnsignedShiftR(TR::IlValue *v,
                                            TR::IlValue *amount) {
  TR::IlValue *returnValue =
      shiftOpFromOpMap(TR::ILOpCode::unsignedShiftRightOpCode, v, amount);
  TraceIL("IlBuilder[ %p ]::%d is unsigned shr %d >> %d\n", this,
          returnValue->getID(), v->getID(), amount->getID());
  return returnValue;
}

/*
 * @brief IfAnd service for constructing short circuit AND conditional nests
 * (like the && operator)
 * @param allTrueBuilder builder containing operations to execute if all
 * conditional tests evaluate to true (automatically allocated if pointed-to
 * pointer is null)
 * @param anyFalseBuilder builder containing operations to execute if any
 * conditional test is false (automatically allocated if pointed-to pointer is
 * null)
 ...
 * TR::IlBuilder *conditions[] = {MakeCondition(cond1Builder, cond1),
 * MakeCondition(cond2Builder, cond2)}; IfAnd(&inRange, &outOfRange, 2,
 * conditions);
 */
void OMR::IlBuilder::IfAnd(TR::IlBuilder **allTrueBuilder,
                           TR::IlBuilder **anyFalseBuilder, int32_t numTerms,
                           JBCondition **terms) {
 ...
 * null)
 * @param numTerms the number of conditional terms
 * @param ... for each term, provide a TR::IlBuilder::JBCondition object for the
 * condition
 *
 * Example:
 * TR::IlBuilder *cond1Builder = OrphanBuilder();
 ...
 *                      cond2Builder->   Load("upper"));
 * TR::IlBuilder *inRange = NULL, *outOfRange = NULL;
 * IfAnd(&inRange, &outOfRange, 2, MakeCondition(cond1Builder, cond1),
 * MakeCondition(cond2Builder, cond2));
 */
void OMR::IlBuilder::IfAnd(TR::IlBuilder **allTrueBuilder,
                           TR::IlBuilder **anyFalseBuilder, int32_t numTerms,
                           ...) {
  JBCondition **terms = (JBCondition **)_comp->trMemory()->allocateHeapMemory(
      numTerms * sizeof(JBCondition *));
...
/*
 * @brief IfOr service for constructing short circuit OR conditional nests (like
 * the || operator)
 * @param anyTrueBuilder builder containing operations to execute if any
 * conditional test evaluates to true (automatically allocated if pointed-to
 * pointer is null)
 * @param allFalseBuilder builder containing operations to execute if all
 * conditional tests are false (automatically allocated if pointed-to pointer is
 * null)
 * @param numTerms the number of conditional terms
 * @param terms array of JBCondition instances that evaluate a condition
 *
 ...
 * TR::IlBuilder *inRange = NULL, *outOfRange = NULL;
 * TR::IlBuilder *conditions[] = {MakeCondition(cond1Builder, cond1),
 * MakeCondition(cond2Builder, cond2)}; IfOr(&outOfRange, &inRange, 2,
 * conditions);
 */
void OMR::IlBuilder::IfOr(TR::IlBuilder **anyTrueBuilder,
                          TR::IlBuilder **allFalseBuilder, int32_t numTerms,
                          JBCondition **terms) {
  TR::IlBuilder *mergePoint = OrphanBuilder();
  ...
  *allFalseBuilder = createBuilderIfNeeded(*allFalseBuilder);
  ...
  // reverse condition on last term so that it can fall through to
  // anyTrueBuilder
 ...
 *                      cond2Builder->   Load("upper"));
 * TR::IlBuilder *inRange = NULL, *outOfRange = NULL;
 * IfOr(&outOfRange, &inRange, 2, MakeCondition(cond1Builder, cond1),
 * MakeCondition(cond2Builder, cond2));
 */
void OMR::IlBuilder::IfOr(TR::IlBuilder **anyTrueBuilder,
                          TR::IlBuilder **allFalseBuilder, int32_t numTerms,
...
TR::IlBuilder::JBCondition *OMR::IlBuilder::MakeCondition(
    TR::IlBuilder *conditionBuilder, TR::IlValue *conditionValue) {
  TR_ASSERT_FATAL(conditionBuilder != NULL,
                  "MakeCondition needs to have non-null conditionBuilder");
  TR_ASSERT_FATAL(conditionValue != NULL,
                  "MakeCondition needs to have non-null conditionValue");
  return new (_comp->trHeapMemory())
      JBCondition(conditionBuilder, conditionValue);
}
...
void OMR::IlBuilder::integerizeAddresses(TR::IlValue **leftPtr,
                                         TR::IlValue **rightPtr) {
  TR::IlValue *left = *leftPtr;
  if (left->getDataType() == TR::Address) *leftPtr = ConvertTo(Word, left);

  TR::IlValue *right = *rightPtr;
  if (right->getDataType() == TR::Address) *rightPtr = ConvertTo(Word, right);
}
...
TR::IlValue *OMR::IlBuilder::UnsignedLessThan(TR::IlValue *left,
                                              TR::IlValue *right) {
  integerizeAddresses(&left, &right);
...
TR::IlValue *OMR::IlBuilder::LessOrEqualTo(TR::IlValue *left,
                                           TR::IlValue *right) {
  integerizeAddresses(&left, &right);
...
TR::IlValue *OMR::IlBuilder::UnsignedLessOrEqualTo(TR::IlValue *left,
                                                   TR::IlValue *right) {
  integerizeAddresses(&left, &right);
...
TR::IlValue *OMR::IlBuilder::GreaterThan(TR::IlValue *left,
                                         TR::IlValue *right) {
  integerizeAddresses(&left, &right);
...
TR::IlValue *OMR::IlBuilder::UnsignedGreaterThan(TR::IlValue *left,
                                                 TR::IlValue *right) {
  integerizeAddresses(&left, &right);
...
TR::IlValue *OMR::IlBuilder::GreaterOrEqualTo(TR::IlValue *left,
                                              TR::IlValue *right) {
  integerizeAddresses(&left, &right);
...
TR::IlValue *OMR::IlBuilder::UnsignedGreaterOrEqualTo(TR::IlValue *left,
                                                      TR::IlValue *right) {
  integerizeAddresses(&left, &right);
...
TR::IlValue **OMR::IlBuilder::processCallArgs(TR::Compilation *comp,
                                              int numArgs, va_list args) {
  TR::IlValue **argValues =
      (TR::IlValue **)comp->trMemory()->allocateHeapMemory(
          numArgs * sizeof(TR::IlValue *));
 ...
 */
TR::IlValue *OMR::IlBuilder::ComputedCall(const char *functionName,
                                          int32_t numArgs, ...) {
  ...
  TR::ResolvedMethod *resolvedMethod =
      _methodBuilder->lookupFunction(functionName);
  ...
  TR::SymbolReference *methodSymRef =
      symRefTab()->findOrCreateComputedStaticMethodSymbol(JITTED_METHOD_INDEX,
                                                          -1, resolvedMethod);
...
TR::IlValue *OMR::IlBuilder::Call(TR::MethodBuilder *calleeMB, int32_t numArgs,
  ...
  if (vmState() != NULL) calleeMB->setVMState(vmState());
  ...
  // there shouldn't be any fall-through, but if there is it should go to the
  // return block and we need to put it somewhere anyway
...
TR::IlValue *OMR::IlBuilder::Call(const char *functionName, int32_t numArgs,
  ...
  TR::SymbolReference *methodSymRef =
      symRefTab()->findOrCreateStaticMethodSymbol(JITTED_METHOD_INDEX, -1,
                                                  resolvedMethod);
...
TR::IlValue *OMR::IlBuilder::genCall(TR::SymbolReference *methodSymRef,
                                     int32_t numArgs, TR::IlValue **argValues,
  ...
  TR::Node *callNode = TR::Node::createWithSymRef(
      isDirectCall ? TR::ILOpCode::getDirectCall(returnType)
...
/** \brief
 *     The service is used to atomically increase memory location \p baseAddress
 * by amount of \p value.
 *
 *  \param value
 ...
 */
TR::IlValue *OMR::IlBuilder::AtomicAdd(TR::IlValue *baseAddress,
                                       TR::IlValue *value) {
  TR_ASSERT_FATAL(baseAddress->getDataType() == TR::Address,
                  "baseAddress must be TR::Address");

  // Determine the implementation type and returnType by detecting "value"'s
  // type
  TR::DataType returnType = value->getDataType();
  TR_ASSERT_FATAL(returnType == TR::Int32 || (returnType == TR::Int64 &&
                                              TR::Compiler->target.is64Bit()),
                  "AtomicAdd currently only supports Int32/64 values");
  TraceIL("IlBuilder[ %p ]::AtomicAdd(%d, %d)\n", this, baseAddress->getID(),
          value->getID());

  TR::SymbolReference *methodSymRef =
      symRefTab()->findOrCreateCodeGenInlinedHelper(
          TR::SymbolReferenceTable::atomicAddSymbol);
  TR::Node *callNode;
  callNode = TR::Node::createWithSymRef(TR::ILOpCode::getDirectCall(returnType),
                                        2, methodSymRef);
...
/**
 * \brief
 *  The service is for generating a treetop for transaction begin when the user
 * needs to use transactional memory. At the end of transaction path, service
 * will automatically generate transaction end instruction.
 *
 * \verbatim
 *
 *    +---------------------------------+
 ...
 *    +-----------------------------------------+
 * +-------------------------------+    +-------------------------+
 *    |<block_$transaction>                     |   |<block_$persistentFailure>
 * |    |<block_$transientFailure>|
 *    |====================                     |   |===========================
 * |    |=========================| |     add (For illustration, we take add  |
 * |AtomicAdd (For illustration, we|    |   ...                   | |     ... as
 * an example. User could       |   |...       take atomicAdd as an |    |   ...
 * | |     ... replace it with other non-atomic|   |...       example. User
 * could  |    |   ...                   | |     ... operations.) |   |...
 * replace it with other|    |   ...                   | |     ... |   |...
 * atomic operations.)  |    |   ...                   | |     TEND |   |... |
 * |   ...                   | |goto --> block_$merge                    |
 * |goto->block_$merge             |    |goto->block_$merge       |
 *    +----------------------------------------+
 * +-------------------------------+    +-------------------------+ | | | | | |
 *                      |------------------------------------------+---------------------------------+
 *                      |
 ...
 *     tstart
 *       |
 *       ----persistentFailure // This is a permanent failure, try atomic way to
 * do it instead
 *       |
 ...
 *
 * \note
 *      If user's platform doesn't support TM, go to persistentFailure path
 * directly/ In this case, current IlBuilder walks around
 * transientFailureBuilder and transactionBuilder and goes to
 ...
 *              |-->persistentFailurie
 */
void OMR::IlBuilder::Transaction(TR::IlBuilder **persistentFailureBuilder,
                                 TR::IlBuilder **transientFailureBuilder,
                                 TR::IlBuilder **transactionBuilder) {
  // This assertion is to rule out platforms which don't have tstart evaluator
  // yet.
  TR_ASSERT_FATAL(
      comp()->cg()->hasTMEvaluator(),
      "this platform doesn't support tstart or tfinish evaluator yet");

  TraceIL("IlBuilder[ %p ]::transactionBegin %p, %p, %p, %p)\n", this,
          *persistentFailureBuilder, *transientFailureBuilder,
          *transactionBuilder);
  ...
  *transactionBuilder = createBuilderIfNeeded(*transactionBuilder);
  ...
  TR::Node *persistentFailureNode = TR::Node::create(
      TR::branch, 0, (*persistentFailureBuilder)->getEntry()->getEntry());
  TR::Node *transientFailureNode = TR::Node::create(
      TR::branch, 0, (*transientFailureBuilder)->getEntry()->getEntry());
  TR::Node *transactionNode = TR::Node::create(
      TR::branch, 0, (*transactionBuilder)->getEntry()->getEntry());

  TR::Node *tStartNode =
      TR::Node::create(TR::tstart, 3, persistentFailureNode,
                       transientFailureNode, transactionNode);
  tStartNode->setSymbolReference(
      comp()->getSymRefTab()->findOrCreateTransactionEntrySymbolRef(
          comp()->getMethodSymbol()));

  genTreeTop(tStartNode);

  // connecting the block having tstart with persistentFailure's and
  ...
  tEndNode->setSymbolReference(
      comp()->getSymRefTab()->findOrCreateTransactionExitSymbolRef(
          comp()->getMethodSymbol()));
  genTreeTop(tEndNode);
...
void OMR::IlBuilder::TransactionAbort() {
  ...
  tAbortNode->setSymbolReference(
      comp()->getSymRefTab()->findOrCreateTransactionAbortSymbolRef(
          comp()->getMethodSymbol()));
  genTreeTop(tAbortNode);
}

void OMR::IlBuilder::IfCmpNotEqualZero(TR::IlBuilder **target,
...
void OMR::IlBuilder::IfCmpNotEqualZero(TR::IlBuilder *target,
                                       TR::IlValue *condition) {
  TR_ASSERT_FATAL(target != NULL,
                  "This IfCmpNotEqualZero requires a non-NULL builder object");
  TraceIL("IlBuilder[ %p ]::IfCmpNotEqualZero %d? -> [ %p ] B%d\n", this,
          condition->getID(), target, target->getEntry()->getNumber());
  ifCmpNotEqualZero(condition, target->getEntry());
}

void OMR::IlBuilder::IfCmpNotEqual(TR::IlBuilder **target, TR::IlValue *left,
                                   TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpNotEqual(TR::IlBuilder *target, TR::IlValue *left,
                                   TR::IlValue *right) {
  TR_ASSERT_FATAL(target != NULL,
                  "This IfCmpNotEqual requires a non-NULL builder object");
  TraceIL("IlBuilder[ %p ]::IfCmpNotEqual %d == %d? -> [ %p ] B%d\n", this,
          left->getID(), right->getID(), target,
          target->getEntry()->getNumber());
  ifCmpCondition(TR_cmpNE, false, left, right, target->getEntry());
}

void OMR::IlBuilder::IfCmpEqualZero(TR::IlBuilder **target,
                                    TR::IlValue *condition) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpEqualZero(TR::IlBuilder *target,
                                    TR::IlValue *condition) {
  TR_ASSERT_FATAL(target != NULL,
                  "This IfCmpEqualZero requires a non-NULL builder object");
  TraceIL("IlBuilder[ %p ]::IfCmpEqualZero %d == 0? -> [ %p ] B%d\n", this,
          condition->getID(), target, target->getEntry()->getNumber());
  ifCmpEqualZero(condition, target->getEntry());
}

void OMR::IlBuilder::IfCmpEqual(TR::IlBuilder **target, TR::IlValue *left,
                                TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpEqual(TR::IlBuilder *target, TR::IlValue *left,
                                TR::IlValue *right) {
  TR_ASSERT_FATAL(target != NULL,
                  "This IfCmpEqual requires a non-NULL builder object");
...
void OMR::IlBuilder::IfCmpLessThan(TR::IlBuilder **target, TR::IlValue *left,
                                   TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpLessThan(TR::IlBuilder *target, TR::IlValue *left,
                                   TR::IlValue *right) {
  TR_ASSERT_FATAL(target != NULL,
                  "This IfCmpLessThan requires a non-NULL builder object");
...
void OMR::IlBuilder::IfCmpUnsignedLessThan(TR::IlBuilder **target,
                                           TR::IlValue *left,
                                           TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedLessThan(TR::IlBuilder *target,
  ...
  TR_ASSERT_FATAL(
      target != NULL,
      "This IfCmpUnsignedLessThan requires a non-NULL builder object");
...
void OMR::IlBuilder::IfCmpLessOrEqual(TR::IlBuilder **target, TR::IlValue *left,
                                      TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpLessOrEqual(TR::IlBuilder *target, TR::IlValue *left,
                                      TR::IlValue *right) {
  TR_ASSERT_FATAL(target != NULL,
                  "This IfCmpLessOrEqual requires a non-NULL builder object");
...
void OMR::IlBuilder::IfCmpUnsignedLessOrEqual(TR::IlBuilder **target,
                                              TR::IlValue *left,
                                              TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedLessOrEqual(TR::IlBuilder *target,
  ...
  TR_ASSERT_FATAL(
      target != NULL,
      "This IfCmpUnsignedLessOrEqual requires a non-NULL builder object");
...
void OMR::IlBuilder::IfCmpGreaterThan(TR::IlBuilder **target, TR::IlValue *left,
                                      TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedGreaterThan(TR::IlBuilder **target,
                                              TR::IlValue *left,
                                              TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpGreaterOrEqual(TR::IlBuilder **target,
                                         TR::IlValue *left,
                                         TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedGreaterOrEqual(TR::IlBuilder **target,
                                                 TR::IlValue *left,
                                                 TR::IlValue *right) {
  *target = createBuilderIfNeeded(*target);
...
void OMR::IlBuilder::IfCmpUnsignedGreaterOrEqual(TR::IlBuilder *target,
                                                 ...
                                                 TR::IlValue *right) {
  TraceIL(
      "IlBuilder[ %p ]::IfCmpUnsignedGreaterOrEqual %d >= %d? -> [ %p ] B%d\n",
      this, left->getID(), right->getID(), target,
      target->getEntry()->getNumber());
  ifCmpCondition(TR_cmpGE, true, left, right, target->getEntry());
}

void OMR::IlBuilder::ifCmpCondition(TR_ComparisonTypes ct, bool isUnsignedCmp,
                                    TR::IlValue *left, TR::IlValue *right,
                                    TR::Block *target) {
  integerizeAddresses(&left, &right);
  TR::Node *leftNode = loadValue(left);
  TR::Node *rightNode = loadValue(right);
  TR::ILOpCode cmpOpCode(
      TR::ILOpCode::compareOpCode(leftNode->getDataType(), ct, isUnsignedCmp));
  ifjump(cmpOpCode.convertCmpToIfCmp(), leftNode, rightNode, target);
  appendBlock();
}

void OMR::IlBuilder::ifCmpNotEqualZero(TR::IlValue *condition,
                                       TR::Block *target) {
 ...
 *    2) only a then path
 *    3) only an else path
 * #2 and #3 are similar: the existing path is appended and the proper condition
 * causes branch around to the merge point #1: then path exists "out of line",
 * else path falls through from If and then jumps to merge, followed by then
 * path which falls through to the merge point
 */
void OMR::IlBuilder::IfThenElse(TR::IlBuilder **thenPath,
                                TR::IlBuilder **elsePath,
                                TR::IlValue *condition) {
  TR_ASSERT_FATAL(thenPath != NULL || elsePath != NULL,
                  "IfThenElse needs at least one conditional path");
  ...
  if (thenEntry) TraceIL(" then B%d", thenEntry->getNumber());
  if (elseEntry) TraceIL(" else B%d", elseEntry->getNumber());
  ...
  } else  // case #1
  {
    ...
    if (!(*thenPath)->_partOfSequence) AppendBuilder(*thenPath);
...
void OMR::IlBuilder::Switch(const char *selectionVar,
                            TR::IlBuilder **defaultBuilder, uint32_t numCases,
                            JBCase **cases) {
  TR::IlValue *selectorValue = Load(selectionVar);
  TR_ASSERT_FATAL(selectorValue->getDataType() == TR::Int32,
...
void OMR::IlBuilder::TableSwitch(const char *selectionVar,
                                 TR::IlBuilder **defaultBuilder,
                                 bool generateBoundsCheck, uint32_t numCases,
                                 JBCase **cases) {
  TR::IlValue *selectorValue = Load(selectionVar);
  TR_ASSERT_FATAL(selectorValue->getDataType() == TR::Int32,
                  "TableSwitch only supports selector having type Int32");
  ...
  TR_ASSERT_FATAL(numCases == casesCovered,
                  "TableSwitch only supports dense case sets");
  if (low != 0) selectorValue = Sub(selectorValue, ConstInt32(low));

  *defaultBuilder = createBuilderIfNeeded(*defaultBuilder);

  TR::Node *defaultNode =
      TR::Node::createCase(0, (*defaultBuilder)->getEntry()->getEntry());
  TR::Node *tableNode = TR::Node::create(TR::table, numCases + 2,
                                         loadValue(selectorValue), defaultNode);
  if (!generateBoundsCheck) tableNode->setIsSafeToSkipTableBoundCheck(true);
...
void OMR::IlBuilder::TableSwitch(const char *selectionVar,
  ...
  TableSwitch(selectionVar, defaultBuilder, generateBoundsCheck, numCases,
              cases);
}

TR::IlBuilder::JBCase *OMR::IlBuilder::MakeCase(int32_t caseValue,
                                                TR::IlBuilder **caseBuilder,
                                                int32_t caseFallsThrough) {
  TR_ASSERT_FATAL(caseBuilder != NULL,
                  "MakeCase, needs to have non-null caseBuilder");
  *caseBuilder = createBuilderIfNeeded(*caseBuilder);
  auto *c = new (_comp->trHeapMemory())
      JBCase(caseValue, *caseBuilder, caseFallsThrough);
  return c;
}

TR::IlValue *OMR::IlBuilder::Select(TR::IlValue *condition,
                                    TR::IlValue *trueValue,
                                    TR::IlValue *falseValue) {
  ...
  if (opCode == TR::BadILOp) {
    ...
    TR::Node *resultNode =
        createWithoutSymRef(opCode, 3, conditionNode, ifTrueNode, ifFalseNode);
    result = newValue(dt, resultNode);
  }
  return result;
}

TR::IlBuilder::JBCase **OMR::IlBuilder::createCaseArray(uint32_t numCases,
                                                        va_list args) {
  JBCase **cases = (JBCase **)_comp->trMemory()->allocateHeapMemory(
      numCases * sizeof(JBCase *));
...
void OMR::IlBuilder::generateSwitchCases(TR::Node *switchNode,
                                         TR::Node *defaultNode,
                                         TR::IlBuilder **defaultBuilder,
                                         uint32_t numCases, JBCase **cases) {
  // get the lookup tree into this builder, even though we haven't completely
  // filled it in yet
  ...
  // each case handler is a sequence of two builder objects: first the one
  // passed in via `cases`,
  //   and second a builder that branches to the breakBuilder (unless this case
  //   falls through)
  for (int32_t c = 0; c < numCases; c++) {
    ...
    if (!cases[c]->_fallsThrough) {
      ...
      // handle "break" with a separate builder so user can add whatever they
      // want into caseBuilders[c]
...
void OMR::IlBuilder::ForLoop(bool countsUp, const char *indVar,
                             TR::IlBuilder **loopCode,
                             TR::IlBuilder **breakBuilder,
                             TR::IlBuilder **continueBuilder,
                             TR::IlValue *initial, TR::IlValue *end,
  ...
  *loopCode = createBuilderIfNeeded(*loopCode);
  ...
  TraceIL(
      "IlBuilder[ %p ]::ForLoop ind %s initial %d end %d increment %d loopCode "
      "%p countsUp %d\n",
      this, indVar, initial->getID(), end->getID(), increment->getID(),
      *loopCode, countsUp);
  ...
  loopCondition =
      countsUp ? LessThan(Load(indVar), end) : GreaterThan(Load(indVar), end);
  ...
  if (breakBuilder) {
    TR_ASSERT_FATAL(
        *breakBuilder == NULL,
        "ForLoop returns breakBuilder, cannot provide breakBuilder as input");
  ...
  if (continueBuilder) {
    TR_ASSERT_FATAL(*continueBuilder == NULL,
                    "ForLoop returns continueBuilder, cannot provide "
                    "continueBuilder as input");
    *continueBuilder = loopContinue;
  }

  if (countsUp) {
    loopContinue->Store(
        indVar, loopContinue->Add(loopContinue->Load(indVar), increment));
    loopContinue->IfCmpLessThan(&loopBody, loopContinue->Load(indVar), end);
  } else {
    loopContinue->Store(
        indVar, loopContinue->Sub(loopContinue->Load(indVar), increment));
    loopContinue->IfCmpGreaterThan(&loopBody, loopContinue->Load(indVar), end);
  }

  // make sure any subsequent operations go into their own block *after* the
  // loop
  appendBlock();
}

void OMR::IlBuilder::DoWhileLoop(const char *whileCondition,
                                 TR::IlBuilder **body,
                                 TR::IlBuilder **breakBuilder,
                                 TR::IlBuilder **continueBuilder) {
  methodSymbol()->setMayHaveLoops(true);
  ...
  *body = createBuilderIfNeeded(*body);
  TraceIL("IlBuilder[ %p ]::DoWhileLoop do body B%d while %s\n", this,
          (*body)->getEntry()->getNumber(), whileCondition);
  ...
  if (continueBuilder) {
    TR_ASSERT_FATAL(*continueBuilder == NULL,
                    "DoWhileLoop returns continueBuilder, cannot provide "
                    "continueBuilder as input");
  ...
  if (breakBuilder) {
    TR_ASSERT_FATAL(*breakBuilder == NULL,
                    "DoWhileLoop returns breakBuilder, cannot provide "
                    "breakBuilder as input");
    *breakBuilder = OrphanBuilder();
    AppendBuilder(*breakBuilder);
  }
...
void OMR::IlBuilder::WhileDoLoop(const char *whileCondition,
                                 TR::IlBuilder **body,
                                 TR::IlBuilder **breakBuilder,
                                 TR::IlBuilder **continueBuilder) {
  methodSymbol()->setMayHaveLoops(true);
  ...
  if (breakBuilder) {
    TR_ASSERT_FATAL(*breakBuilder == NULL,
                    "WhileDoLoop returns breakBuilder, cannot provide "
                    "breakBuilder as input");
    *breakBuilder = done;
  }
  ...
  if (continueBuilder) {
    TR_ASSERT_FATAL(*continueBuilder == NULL,
                    "WhileDoLoop returns continueBuilder, cannot provide "
                    "continueBuilder as input");
    *continueBuilder = loopContinue;
  }
  ...
  *body = createBuilderIfNeeded(*body);
  ...
  Goto(&loopContinue);
  setComesBack();  // this goto is on one particular flow path, doesn't mean
                   // every path does a goto
