...
class IlValue {
 public:
   ...
   */
  IlValue(TR::Node *node, TR::TreeTop *treeTop, TR::Block *block,
          TR::MethodBuilder *methodBuilder);

  /**
   * @brief return a TR::Node that computes this value, either directly if in
   * same block or by loading a local variable if not
   * @param block TR::Block for the load point so it can be compared to the
   * TR::Block where value is computed
   */
  TR::Node *load(TR::Block *block);

  /**
   * @brief ensures that the provided value replaces the current value, but only
   * affects subsequent loads
   * @param value the new value that should be returned whenever the current
   * value is loaded
   * @param block TR::Block for the replacement point, so that either the node
   * pointer is changed or what's stored in the symbol reference
   */
  ...
  /**
   * @brief returns the TR::SymbolReference holding the value, or NULL if
   * willBeUsedInAnotherBlock() has not yet been called Caller should ensure the
   * current block is different than the block that computes the value; if the
   * current block is the same then it is better to use the value as a node
   * pointer via GetNodePointerIfLegal().
   */
  ...
  /**
   * @brief returns a unique identifier for this IlValue object within its
   * MethodBuilder Note that IlValues from different MethodBuilders can have the
   * same ID.
   */
 ...
 protected:
  /**
   * @brief ensures this value is accessible via an auto symbol, but only
   * generates store if hasn't already been stored
   */
  void storeToAuto();
  ...
  /**
   * @brief identifying number for values guaranteed to be unique per
   * MethodBuilder
   */
  int32_t _id;
  ...
  /**
   * @brief TR::SymbolReference for temp holding value if it needs to be used
   * outside basic block that computes it
   */
