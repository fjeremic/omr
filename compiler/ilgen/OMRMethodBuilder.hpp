...
public:
  ...
  MethodBuilder(TR::TypeDictionary *types,
                TR::VirtualMachineState *vmState = NULL);
  ...
  void AppendBuilder(TR::IlBuilder *b) {
    this->OMR::IlBuilder::AppendBuilder(b);
  }
  ...
  void DefineFunction(const char *const name, const char *const fileName,
                      const char *const lineNumber, void *entryPoint,
                      TR::IlType *returnType, int32_t numParms, ...);
  void DefineFunction(const char *const name, const char *const fileName,
                      const char *const lineNumber, void *entryPoint,
                      TR::IlType *returnType, int32_t numParms,
                      TR::IlType **parmTypes);
  ...
  /**
   * @brief will be called if a Call is issued to a function that has not yet
   * been defined, provides a mechanism for MethodBuilder subclasses to provide
   * method lookup on demand rather than all up front via the constructor.
   * @returns true if the function was found and DefineFunction has been called
   * for it, otherwise false
   */
   ...
   * @param builder the bytecode builder object to add, usually for bytecode
   * index 0 A side effect of this call is that the builder is added to the
   * worklist so that all other bytecodes can be processed by asking for
   * GetNextBytecodeFromWorklist()
   */
  void AppendBytecodeBuilder(TR::BytecodeBuilder *builder);

  /**
   * @brief add a bytecode builder to the worklist
   * @param bcBuilder is the bytecode builder whose bytecode index will be added
   * to the worklist
   */
   ...
   * It is important to use the worklist because it guarantees no bytecode will
   * be processed before at least one predecessor bytecode has been processed,
   * which means there should be a non-NULL VirtualMachineState object on the
   * associated BytecodeBuilder object.
   */
  ...
  /**
   * @brief associate a particular IlBuilder object as the return landing pad
   * for this (inlined) MethodBuilder
  ...
  /**
   * @brief get the return landing pad IlBuilder for this (inlined)
   * MethodBuilder
  ...
  /**
   * @brief set the name of the symbol to use to store the return value from
   * this (inined) MethodBuilder
  ...
  /**
   * @brief get the return symbol name to store any return value from this
   * (inlined) MethodBuilder
   * @returns the return symbol name
   */
  const char *returnSymbol() { return _returnSymbolName; }

  /*
   * @brief If this is an inlined MethodBuilder, return the MethodBuilder that
   * directly inlined it
   * @returns the directly inlining MethodBuilder or NULL if no MethodBuilder
   * inlined this one
   */
  ...
  /**
   * @brief Store callback function to be called on client when RequestFunction
   * is called
   */
...
protected:
  ...
  /*
   * @brief adjusts a local variable name so that it will be unique to the
   * current inlined site to prevent inlining-induced name aliasing
...
private:
  ...
  typedef TR::typed_allocator<
      std::pair<const char *const, TR::SymbolReference *>, TR::Region &>
      SymbolMapAllocator;
  typedef std::map<const char *, TR::SymbolReference *, StrComparator,
                   SymbolMapAllocator>
      SymbolMap;
  ...
  typedef TR::typed_allocator<std::pair<const char *const, int32_t>,
                              TR::Region &>
      ParameterMapAllocator;
  typedef std::map<const char *, int32_t, StrComparator, ParameterMapAllocator>
      ParameterMap;
  ParameterMap _parameterSlot;

  typedef TR::typed_allocator<std::pair<const char *const, TR::IlType *>,
                              TR::Region &>
      SymbolTypeMapAllocator;
  typedef std::map<const char *, TR::IlType *, StrComparator,
                   SymbolTypeMapAllocator>
      SymbolTypeMap;
  SymbolTypeMap _symbolTypes;

  typedef TR::typed_allocator<std::pair<int32_t const, const char *>,
                              TR::Region &>
      SlotToSymNameMapAllocator;
  typedef std::map<int32_t, const char *, std::less<int32_t>,
                   SlotToSymNameMapAllocator>
      SlotToSymNameMap;
  SlotToSymNameMap _symbolNameFromSlot;

  typedef TR::typed_allocator<const char *, TR::Region &> StringSetAllocator;
  typedef std::set<const char *, StrComparator, StringSetAllocator>
      ArrayIdentifierSet;

  // This set acts as an identifier for symbols which correspond to arrays
  ArrayIdentifierSet _symbolIsArray;

  typedef TR::typed_allocator<std::pair<const char *const, void *>,
                              TR::Region &>
      MemoryLocationMapAllocator;
  typedef std::map<const char *, void *, StrComparator,
                   MemoryLocationMapAllocator>
      MemoryLocationMap;
  MemoryLocationMap _memoryLocations;

  typedef TR::typed_allocator<
      std::pair<const char *const, TR::ResolvedMethod *>, TR::Region &>
      FunctionMapAllocator;
