...
OMR::VirtualMachineRegister::VirtualMachineRegister(
    TR::IlBuilder *b, const char *const localName,
    TR::IlType *pointerToRegisterType, uint32_t adjustByStep,
  ...
  TR_ASSERT_FATAL(pointerToRegisterType->isPointer(),
                  "VirtualMachineRegister for %s requires "
                  "pointerToRegisterType (%s) to be a pointer",
  ...
  if (_integerTypeForAdjustments->isPointer()) {
    ...
    TR_ASSERT_FATAL(adjustByStep == baseType->getSize(),
                    "VirtualMachineRegister for %s adjustByStep (%u) != size "
                    "represented by pointerToRegisterType (%u)",
                    localName, _adjustByStep, baseType->getSize());
    _isAdjustable = true;
  } else {
    TR_ASSERT_FATAL(adjustByStep == 0,
                    "VirtualMachineRegister for %s is representing a primitive "
                    "type but adjustByStep (%u) != 0",
                    localName, _adjustByStep);
    _isAdjustable = false;
  }
  Reload(b);
}

void OMR::VirtualMachineRegister::adjust(TR::IlBuilder *b,
                                         TR::IlValue *rawAmount) {
  TR_ASSERT_FATAL(_isAdjustable, "VirtualMachineRegister can not be adjusted "
                                 "as it is simulating a primitive type");
...
TR::VirtualMachineState *OMR::VirtualMachineRegister::MakeCopy() {
  return new (TR::comp()->trMemory()->trHeapMemory())
      TR::VirtualMachineRegister(_localName, _pointerToRegisterType,
                                 _adjustByStep, _addressOfRegister);
}
