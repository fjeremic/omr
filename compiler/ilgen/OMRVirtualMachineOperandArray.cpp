...
OMR::VirtualMachineOperandArray::VirtualMachineOperandArray(
  TR::MethodBuilder* mb, int32_t numOfElements, TR::IlType* elementType, TR::VirtualMachineRegister* arrayBaseRegister)
...
void OMR::VirtualMachineOperandArray::MergeInto(TR::VirtualMachineState* o, TR::IlBuilder* b)
{
  ...
  for (int32_t i = 0; i < _numberOfElements; i++) {
    if (NULL == other->_values[i]) {
      TR_ASSERT_FATAL(
        _values[i] == NULL, "if an element in the dest array at index is NULL it has to be NULL in the src array");
      ...
      TraceIL("VirtualMachineOperandArray[ %p ]::MergeInto builder %p index %d storeOver %p(%d) with %p(%d)\n", this, b,
        i, other->_values[i], other->_values[i]->getID(), _values[i], _values[i]->getID());
...
void OMR::VirtualMachineOperandArray::Move(TR::IlBuilder* b, int32_t dstIndex, int32_t srcIndex)
{
  ...
  TraceIL("VirtualMachineOperandArray[ %p ]::Move builder %p move srcIndex %d %p(%d) to dstIndex %d %p(%d)\n", this, b,
    srcIndex, _values[srcIndex], _values[srcIndex]->getID(), dstIndex, _values[dstIndex], _values[dstIndex]->getID());
}
