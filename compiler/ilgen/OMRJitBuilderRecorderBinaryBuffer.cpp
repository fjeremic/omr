...
void OMR::JitBuilderRecorderBinaryBuffer::Close() {
...
void OMR::JitBuilderRecorderBinaryBuffer::String(const char *const string) {
  ...
  TR_ASSERT_FATAL(len < max,
                  "JBIL: binary String format limited to %d characters", max);

  Number((int16_t)len);
  for (int16_t i = 0; i < len; i++) _buf.push_back((uint8_t)string[i]);
}

void OMR::JitBuilderRecorderBinaryBuffer::Number(int8_t num) {
...
void OMR::JitBuilderRecorderBinaryBuffer::Statement(const char *s) {
  ID(lookupID(s));
}

void OMR::JitBuilderRecorderBinaryBuffer::Type(const TR::IlType *type) {
  ID(lookupID(type));
}

void OMR::JitBuilderRecorderBinaryBuffer::Value(const TR::IlValue *v) {
  ID(lookupID(v));
}

void OMR::JitBuilderRecorderBinaryBuffer::Builder(const TR::IlBuilder *b) {
  ID(lookupID(b));
}
