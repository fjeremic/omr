...
OMR::IlInjector::IlInjector(TR::TypeDictionary *types)
    : TR_IlGenerator(), _types(types), _comp(NULL), _fe(NULL), _symRefTab(NULL),
      _details(NULL), _methodSymbol(NULL), _currentBlock(0),
      _currentBlockNumber(-1), _numBlocks(0), _blocks(0),
      _blocksAllocatedUpFront(false) {
  initPrimitiveTypes();
}

OMR::IlInjector::IlInjector(TR::IlInjector *source)
    : TR_IlGenerator(), _types(source->_types), _comp(source->_comp),
      _fe(source->_fe), _symRefTab(source->_symRefTab),
      _details(source->_details), _methodSymbol(source->_methodSymbol),
      _currentBlock(0), _currentBlockNumber(-1), _numBlocks(0), _blocks(0),
      _blocksAllocatedUpFront(false) {
  initPrimitiveTypes();
}
...
void OMR::IlInjector::initialize(TR::IlGeneratorMethodDetails *details,
...
void OMR::IlInjector::allocateBlocks(int32_t num) {
  _numBlocks = num;
  _blocks = (TR::Block **)_comp->trMemory()->allocateHeapMemory(
      num * sizeof(TR::Block *));
  _blocksAllocatedUpFront = true;
}

TR::Block *OMR::IlInjector::newBlock() {
...
TR::Node *OMR::IlInjector::parameter(int32_t slot, TR::IlType *dt) {
  return TR::Node::createLoad(symRefTab()->findOrCreateAutoSymbol(
      _methodSymbol, slot, dt->getPrimitiveType(), true, false, true));
}

TR::Node *OMR::IlInjector::iconst(int32_t value) {
  return TR::Node::iconst(value);
}

TR::Node *OMR::IlInjector::lconst(int64_t value) {
  return TR::Node::lconst(value);
}

TR::Node *OMR::IlInjector::bconst(int8_t value) {
  return TR::Node::bconst(value);
}

TR::Node *OMR::IlInjector::sconst(int16_t value) {
  return TR::Node::sconst(value);
}

TR::Node *OMR::IlInjector::aconst(uintptrj_t value) {
...
TR::Node *OMR::IlInjector::staticAddress(void *address) {
  return TR::Node::aconst((uintptrj_t)address);
}

void OMR::IlInjector::storeToTemp(TR::SymbolReference *tempSymRef,
                                  TR::Node *value) {
  genTreeTop(TR::Node::createStore(tempSymRef, value));
}

TR::Node *OMR::IlInjector::loadTemp(TR::SymbolReference *tempSymRef) {
  return TR::Node::createLoad(tempSymRef);
}

TR::Node *OMR::IlInjector::i2l(TR::Node *n) {
  return TR::Node::create(TR::i2l, 1, n);
}

TR::Node *OMR::IlInjector::iu2l(TR::Node *n) {
  return TR::Node::create(TR::iu2l, 1, n);
}

void OMR::IlInjector::ifjump(TR::ILOpCodes op, TR::Node *first,
                             TR::Node *second, TR::Block *targetBlock) {
  TR::Node *ifNode =
      TR::Node::createif(op, first, second, targetBlock->getEntry());
  genTreeTop(ifNode);
...
void OMR::IlInjector::ifjump(TR::ILOpCodes op, TR::Node *first,
                             TR::Node *second, int32_t targetBlockNumber) {
...
TR::Node *OMR::IlInjector::shiftLeftBy(TR::Node *value, int32_t shift) {
  ...
  if (value->getDataType() == TR::Int32)
    ...
    TR_ASSERT(value->getDataType() == TR::Int64,
              "expecting Int32 or Int64 for shiftLeftBy value expression");
...
TR::Node *OMR::IlInjector::multiplyBy(TR::Node *value, int64_t factor) {
  ...
  if (value->getDataType() == TR::Int32)
    ...
    TR_ASSERT(value->getDataType() == TR::Int64,
              "expecting Int32 or Int64 for multiplyBy value expression");
    result = TR::Node::create(TR::lmul, 2, value, TR::Node::lconst(factor));
  }
  return result;
}

TR::Node *OMR::IlInjector::arrayLoad(TR::Node *base, TR::Node *index,
                                     TR::IlType *dt) {
  TR::Node *scaledIndex;
  TR::Node *element;
  if (index->getDataType() == TR::Int32) {
    scaledIndex = createWithoutSymRef(
        TR::imul, 2, index,
        iconst((int64_t)TR::DataType::getSize(dt->getPrimitiveType())));
    element = TR::Node::create(TR::aiadd, 2, base, scaledIndex);
  } else {
    TR_ASSERT(index->getDataType() == TR::Int64,
              "expecting Int32 or Int64 for arrayLoad index expression");
    scaledIndex = createWithoutSymRef(
        TR::lmul, 2, index,
        lconst((int64_t)TR::DataType::getSize(dt->getPrimitiveType())));
    element = TR::Node::create(TR::aladd, 2, base, scaledIndex);
  }
  TR::SymbolReference *symRef = symRefTab()->findOrCreateArrayShadowSymbolRef(
      dt->getPrimitiveType(), base);
  TR::Node *load = TR::Node::createWithSymRef(
      TR::ILOpCode::indirectLoadOpCode(dt->getPrimitiveType()), 1, element, 0,
      symRef);
  return load;
}
...
void OMR::IlInjector::returnValue(TR::Node *value) {
  TR::Node *returnNode = TR::Node::create(
      TR::ILOpCode::returnOpCode(value->getDataType()), 1, value);
...
void OMR::IlInjector::validateTargetBlock() {
  TR_ASSERT(_currentBlock != NULL, "Current Block is NULL! Probably need to "
                                   "call generateToBlock() beforehand");
  TR_ASSERT(_currentBlockNumber >= 0,
            "Current Block Number is invalid! Probably need to call "
            "generateToBlock() beforehand");
}
 ...
 * */
TR::Node *OMR::IlInjector::createWithoutSymRef(TR::ILOpCodes opCode,
                                               uint32_t numArgs, ...) {
  TR_ASSERT(numArgs > 0,
