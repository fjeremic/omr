...
void OMR::IlValue::storeToAuto()
{
  if (_symRefThatCanBeUsedInOtherBlocks == NULL) {
    ...
    TR::SymbolReference* symRef
      = comp->getSymRefTab()->createTemporary(_methodBuilder->methodSymbol(), _nodeThatComputesValue->getDataType());
...
void OMR::IlValue::storeOver(TR::IlValue* value, TR::Block* block)
{
  if (value->_blockThatComputesValue == block && _blockThatComputesValue == block) {
    ...
    // more commonly, if any value is in another block or this use will be in a different block, first ensure this value
    // is stored to an auto symbol NOTE this may mean that nodes will be stored to more than one auto, but that's ok
    storeToAuto();
    ...
    // if, however, value was computed in a different block, make sure it's stored to an auto and then load it from that
    // auto
