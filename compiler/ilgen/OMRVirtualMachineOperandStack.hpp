...
/**
 * @brief simulates an operand stack used by many bytecode based virtual
 * machines In such virtual machines, the operand stack holds the intermediate
 * expression values computed by the bytecodes. The compiler simulates this
 * operand stack as well, but what is pushed to and popped from the simulated
 * operand stack are expression nodes that represent the value computed by the
 * bytecodes. As each bytecode pops expression nodes off the operand stack, it
 * combines them to form more complicated expressions which are then pushed back
 * onto the operand stack for consumption by other bytecodes.
 *
 ...
 * unlike VirtualMachineRegister, the simulated operand stack is *not*
 * maintained by the method code as part of the method's stack frame. This
 * approach requires modelling the state of the operand stack at all program
 * points, which means there cannot only be one VirtualMachineOperandStack
 * object.
 *
 ...
 *   value onto the virtual machine's operand stack (more details at
 * definition). Reload() is left empty; assumption is that each BytecodeBuilder
 * handler will update the state of the operand stack appropriately on return
 * from the interpreter. MakeCopy() copies the state of the operand stack
...
public:
  /**
   * @brief public constructor, must be instantiated inside a compilation
   * because uses heap memory
   * @param mb TR::MethodBuilder object of the method currently being compiled
   * @param sizeHint initial size used to allocate the stack; will grow larger
   * if needed
   * @param elementType TR::IlType representing the underlying type of the
   * virtual machine's operand stack entries
   * @param stackTop previously allocated and initialized VirtualMachineRegister
   * representing the top of stack
   * @param growsUp to configure virtual machine stack growth direction, set to
   * true if virtual machine stack grows towards larger addresses, false
   * otherwise
   * @param stackInitialOffset to configure virtual machine stack stack offset
   * set to the difference in elements between initial stack pointer and actual
   * bottom of stack Some stacks Push by incrementing stack pointer then
   * storing, some by storing and then incrementing stack pointer. In the first
   * case, stackInitialOffset should be -1 because the stack pointer initially
   * points one element below the bottom of the stack. In the second case,
   * stackInitialOffset should be 0, because the stack pointer initially points
   * at the bottom of the stack. Other values are possible but would be
   * considered highly unusual.
   * Default behaviour for compatibility constructor will be optional arguments,
   * growsUp is true, and stackInitialOffset is -1.
   */

  VirtualMachineOperandStack(TR::MethodBuilder *mb, int32_t sizeHint,
   ...
   * @param other the operand stack whose values should be used to initialize
   * this object
   */
   ...
   * @param b the builder where the operations will be placed to recreate the
   ...
   * @param b the builder where the operations will be placed to recreate the
   * simulated operand stack Users can call Drop beforehand with the appropriate
   * positive or negative count to ensure the simulated stack accounts for new
   * or dropped virtual machine stack elements.
   */
  ...
  /**
   * @brief emit operands to store current operand stack values into same
   * variables as used in another operand stack
   * @param other operand stack for the builder object control is merging into
   * @param b builder object where the operations will be added to make the
   * current operand stack the same as the other
   */
   ...
   * @param stack the new stack base address.  It is assumed that the address is
   * already adjusted to _stackOffset
   */
  virtual void UpdateStack(TR::IlBuilder *b, TR::IlValue *stack);

  /**
   * @brief Push an expression onto the simulated operand stack
   * @param b builder object to use for any operations used to implement the
   * push (e.g. update the top of stack)
   ...
   * @param b builder object to use for any operations used to implement the pop
   * (e.g. to update the top of stack)
   ...
   * @param b builder object to use for any operations used to implement the
   * drop (e.g. to update the top of stack)
   ...
   * @param b builder object to use for any operations used to duplicate the
   * expression (e.g. to update the top of stack)
   */
