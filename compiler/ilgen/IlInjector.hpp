...
public:
  ...
  TR::IlGeneratorMethodDetails* details() const
  {
    return _details;
  }
  TR::FrontEnd* fe() const
  {
    return _fe;
  }
  ...
  TR::Block* block(int32_t num)
  {
    return _blocks[num];
  }
  void gotoBlock(TR::Block* block);
  void branchToBlock(int32_t num)
  {
    gotoBlock(_blocks[num]);
  }
...
private:
  ...
  TR::Block* block(int32_t num) const
  {
    return _blocks[num];
  }
