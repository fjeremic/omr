...
/**
 * @brief simulates an operand array used by many bytecode based virtual
 * machines In such virtual machines, the operand array holds the intermediate
 * expression values computed by the bytecodes. The compiler simulates this
 * operand array as well, but what is modified from the simulated operand array
 * are expression nodes that represent the value computed by the bytecodes.
 *
 ...
 *   value onto the virtual machine's operand array (more details at
 * definition). Reload() read the virtual machine array back into the simulated
 * operand array MakeCopy() copies the state of the operand array MergeInto() is
 * slightly subtle. Operations may have been already created below the merge
 * point, and those operations will have assumed the expressions are stored in
 * the TR::IlValue's for the state being merged *to*. So the purpose of
 * MergeInto() is to store the values of the current state into the same
 * variables as in the "other" state. UpdateArray() update OperandArray_base so
 * Reload/Commit will use the right one if the array moves in memory
 *
 */
...
class VirtualMachineOperandArray : public TR::VirtualMachineState {
 public:
   ...
   * virtual machine's operand array entries
   * @param arrayBase previously allocated and initialized
   * VirtualMachineRegister representing the base of the array
   */
  VirtualMachineOperandArray(TR::MethodBuilder *mb, int32_t numOfElements,
                             TR::IlType *elementType,
                             TR::VirtualMachineRegister *arrayBase);

  /**
   * @brief constructor used to copy the array from another state
   * @param other the operand array whose values should be used to initialize
   * this object
   */
   ...
   * @param b the builder where the operations will be placed to recreate the
   * simulated operand array stack accounts for new or dropped virtual machine
   * stack elements.
   */
  ...
  /**
   * @brief emit operands to store current operand array values into same
   * variables as used in another operand array
   ...
   * current operand array the same as the other
   */
  ...
  /**
   * @brief Returns the expression at the given index of the simulated operand
   * array
  ...
  /**
   * @brief Set the expression into the simulated operand array at the given
   * index
  ...
  /**
   * @brief Move the expression from one index to another index in the simulated
   * operand array
