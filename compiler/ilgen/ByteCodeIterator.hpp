...
template <typename ByteCode, typename ResolvedMethod>
class TR_ByteCodeIterator {
public:
  TR_ByteCodeIterator(TR::ResolvedMethodSymbol *resolvedMethodSym,
                      ResolvedMethod *m, TR::Compilation *comp)
      : _methodSymbol(resolvedMethodSym), _method(m), _compilation(comp),
        _trMemory(comp->trMemory()), _maxByteCodeIndex(m->maxBytecodeIndex()) {}

  struct TryCatchInfo : TR_Link<TryCatchInfo> {
    void *operator new(size_t s, void *p) { return p; }

    TryCatchInfo(uint16_t s, uint16_t e, uint16_t h, uint32_t c)
        : _startIndex(s), _endIndex(e), _handlerIndex(h), _catchType(c),
          _firstBlock(0), _lastBlock(0), _catchBlock(0) {}
