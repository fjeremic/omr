...
OMR::JitBuilderRecorder::TypeID OMR::JitBuilderRecorder::getNewID() {
  ...
  //  to avoid any synchronization issues in how decoders/encoders count IDs,
  //  use a statement to signal change
  // check if we're close to 8-bit / 16-bit boundary
  // need to leave space for one ID because these statements will not have been
  // defined yet
...
OMR::JitBuilderRecorder::TypeID OMR::JitBuilderRecorder::myID() {
  return lookupID(_mb);
}
...
OMR::JitBuilderRecorder::TypeID OMR::JitBuilderRecorder::lookupID(
    const void *ptr) {
  TypeMapID::iterator it = _idMap.find(ptr);
  if (it == _idMap.end())
    TR_ASSERT_FATAL(
        0, "Attempt to lookup a builder object that has not yet been created");
...
void OMR::JitBuilderRecorder::BeginStatement(const char *s) {
  BeginStatement(_mb, s);
}

void OMR::JitBuilderRecorder::BeginStatement(const TR::MethodBuilder *b,
                                             const char *s) {
