...
OMR::ThunkBuilder::ThunkBuilder(TR::TypeDictionary *types, const char *name,
                                TR::IlType *returnType,
                                uint32_t numCalleeParams,
                                TR::IlType **calleeParamTypes)
    : TR::MethodBuilder(types), _numCalleeParams(numCalleeParams),
...
bool OMR::ThunkBuilder::buildIL() {
  ...
  TR::IlValue **args = (TR::IlValue **)comp()->trMemory()->allocateHeapMemory(
  ...
  for (uint32_t p = 0; p < _numCalleeParams; p++) {
    uint32_t a = p + 1;
    args[a] =
        ConvertTo(_calleeParamTypes[p],
                  LoadAt(pWord, IndexAt(pWord, Load("args"), ConstInt32(p))));
  }
