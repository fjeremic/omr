...
{
  _definingLine[0] = '\0';
  initialize(callerMB->_details,
             callerMB->_methodSymbol,
             callerMB->_fe,
             callerMB->_symRefTab);
}
...
OMR::MethodBuilder::countBlocks()
{
  ...
  while (!_countBlocksWorklist->isEmpty()) {
    ...
    for (TR::BytecodeBuilder* builder = iter.getFirst(); !iter.atEnd();
      ...
      if (builder->_count < 0) {
        TraceIL(
          "[ %p ] Adding unreachable BytecodeBuilder %p to counting worklist\n",
          this,
          builder);
...
OMR::MethodBuilder::connectTrees()
{
  ...
  TraceIL(
    "[ %p ] TR::MethodBuilder::connectTrees total blocks %d\n", this, _count);
...
{
  TR_ASSERT_FATAL(
    _symbols.find(name) == _symbols.end(), "Symbol '%s' already defined", name);
...
OMR::MethodBuilder::lookupSymbol(const char* name)
{
  ...
  TR_ASSERT_FATAL(
    symTypesIterator != _symbolTypes.end(), "Symbol '%s' doesn't exist", name);
...
void
OMR::MethodBuilder::DefineMemory(const char* name,
                                 TR::IlType* dt,
                                 void* location)
{
...
void
OMR::MethodBuilder::DefineArrayParameter(const char* name,
                                   ...
                                   ...)
{
  ...
  DefineFunction(
    name, fileName, lineNumber, entryPoint, returnType, numParms, parmTypes);
}
                                   ...
                                   TR::IlType** parmTypes)
{
  ...
  TR::ResolvedMethod* method =
    new (trMemory()->heapMemoryRegion()) TR::ResolvedMethod((char*)fileName,
                                                            (char*)lineNumber,
                                                            (char*)name,
                                                            numParms,
                                                            copiedParmTypes,
...
OMR::MethodBuilder::getSymbolName(int32_t slot)
{
  ...
  TR_ASSERT_FATAL(
    it != _symbolNameFromSlot.end(), "No symbol found in slot %d", slot);
...
OMR::MethodBuilder::getParameterTypes()
{
  ...
  for (int32_t p = 0; p < _numParameters; p++) {
    ...
    TR_ASSERT_FATAL(symTypesIterator != _symbolTypes.end(),
                    "No matching symbol type for parameter '%s'",
                    name);
...
{
  TraceIL(
    "[ %p ] TR::MethodBuilder::addToBlockCountingWorklist %p\n", this, builder);
  _countBlocksWorklist->add(builder);
}
