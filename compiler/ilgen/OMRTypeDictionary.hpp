...
public:
   ...
   */
  void DefineField(const char *structName, const char *fieldName,
                   TR::IlType *type, size_t offset);
  ...
  void UnionField(const char *unionName, const char *fieldName,
                  TR::IlType *type);
  ...
  TR::IlType *PointerTo(TR::DataType baseType) {
    return PointerTo(_primitiveType[baseType]);
  }
  ...
  /*
   * @brief advise that compilation is complete so compilation-specific objects
   * like symbol references can be cleared from caches
   */
...
protected:
  ...
  typedef TR::typed_allocator<std::pair<const char *const, OMR::StructType *>,
                              TR::Region &>
      StructMapAllocator;
  typedef std::map<const char *, OMR::StructType *, StrComparator,
                   StructMapAllocator>
      StructMap;
  StructMap _structsByName;

  typedef TR::typed_allocator<std::pair<const char *const, OMR::UnionType *>,
                              TR::Region &>
      UnionMapAllocator;
  typedef std::map<const char *, OMR::UnionType *, StrComparator,
                   UnionMapAllocator>
      UnionMap;
