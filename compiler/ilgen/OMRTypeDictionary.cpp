...
public:
  ...
  {
    return TR::DataType::getSize(_type);
  }
...
public:
  ...
  void cacheSymRef(TR::SymbolReference* symRef)
  {
    _symRef = symRef;
  }
  ...
  TR::DataType getPrimitiveType()
  {
    return _type->getPrimitiveType();
  }
...
public:
  ...
  {
    TR_ASSERT_FATAL(
      _size <= finalSize, "Final size %d of struct %s is less than its current size %d\n", finalSize, _name, _size);
    _size = finalSize;
    _closed = true;
  };
  void Close()
...
public:
  ...
  virtual size_t getSize()
  {
    return TR::DataType::getSize(TR::Address);
  }
...
void OMR::StructType::AddField(const char* name, TR::IlType* typeInfo, size_t offset)
{
  ...
  TR_ASSERT_FATAL(_size <= offset,
    "Offset of new struct field %s::%s is %d, which is less than the current size of the struct %d\n", _name, name,
    offset, _size);
