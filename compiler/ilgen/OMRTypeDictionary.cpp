...
OMR::UnionType::getFieldSymRef(const char* fieldName)
{
  OMR::FieldInfo* info = findField(fieldName);
  TR_ASSERT_FATAL(
    info, "Struct %s has no field with name %s\n", getName(), fieldName);
  ...
  if (NULL == symRef) {
    ...
    symRef = new (comp->trHeapMemory()) TR::SymbolReference(
      symRefTab, symbol, comp->getMethodSymbol()->getResolvedMethodIndex(), -1);
    symRef->setOffset(0);
...
OMR::TypeDictionary::MemoryManager::MemoryManager()
  ...
  , _memoryRegion(new (TR::Compiler->persistentAllocator())
...
void
OMR::TypeDictionary::DefineField(const char* structName,
...
void
OMR::TypeDictionary::UnionField(const char* unionName,
                                const char* fieldName,
                                TR::IlType* type)
{
...
TR::IlType*
OMR::TypeDictionary::UnionFieldType(const char* unionName,
                                    const char* fieldName)
{
...
OMR::TypeDictionary::getStruct(const char* structName)
{
  ...
  TR_ASSERT_FATAL(
    it != _structsByName.end(), "No struct named '%s'", structName);
