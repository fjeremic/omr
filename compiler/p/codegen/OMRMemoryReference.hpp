...
class OMR_EXTENSIBLE MemoryReference : public OMR::MemoryReference {
 ...
 public:
  ...
  MemoryReference(TR::Register *br, TR::Register *ir, uint8_t len,
                  TR::CodeGenerator *cg);

  MemoryReference(TR::Register *br, int32_t disp, uint8_t len,
                  TR::CodeGenerator *cg);

  virtual TR::RegisterDependencyConditions *getConditions() {
    return _conditions;
  }
  ...
  void setBaseModifiable() {
    _flag |= TR_PPCMemoryReferenceControl_Base_Modifiable;
  }
  void clearBaseModifiable() {
    _flag &= ~TR_PPCMemoryReferenceControl_Base_Modifiable;
  }
  ...
  void setIndexModifiable() {
    _flag |= TR_PPCMemoryReferenceControl_Index_Modifiable;
  }
  void clearIndexModifiable() {
    _flag &= ~TR_PPCMemoryReferenceControl_Index_Modifiable;
  }
  ...
  void setOffsetRequiresWordAlignment(TR::Node *node, TR::CodeGenerator *cg,
  ...
  void clearUingStaticTOC() {
    _flag &= ~TR_PPCMemoryReferenceControl_Static_TOC;
  }
  ...
  void setUsingDelayedIndexedForm() {
    _flag |= TR_PPCMemoryReferenceControl_Late_Xform;
  }
  void clearUingDelayedIndexedForm() {
    _flag &= ~TR_PPCMemoryReferenceControl_Late_Xform;
  }
  ...
  MemoryReference(TR::Node *rootLoadOrStore, uint32_t len,
                  TR::CodeGenerator *cg);

  MemoryReference(TR::Node *node, TR::SymbolReference *symRef, uint32_t len,
                  TR::CodeGenerator *cg);

  MemoryReference(TR::Node *node, TR::MemoryReference &mr, int32_t n,
                  uint32_t len, TR::CodeGenerator *cg);

  void accessStaticItem(TR::Node *node, TR::SymbolReference *ref, bool isStore,
                        TR::CodeGenerator *cg);

  void addToOffset(TR::Node *node, intptrj_t amount, TR::CodeGenerator *cg);

  void forceIndexedForm(TR::Node *node, TR::CodeGenerator *cg,
  ...
  void consolidateRegisters(TR::Register *reg, TR::Node *subTree, bool m,
                            TR::CodeGenerator *cg);
  ...
  uint8_t *generateBinaryEncoding(TR::Instruction *ci, uint8_t *modRM,
                                  TR::CodeGenerator *cg);
};
}  // namespace Power
}  // namespace OMR
