...
TR::Register* OMR::Power::TreeEvaluator::aconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isClass && cg->wantToPatchClassPointer((TR_OpaqueClassBlock*)address, node) || isProfiledPointerConstant && cg->profiledPointersRequireRelocation()) {
...
TR::Register* OMR::Power::TreeEvaluator::lconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((highValue >= LOWER_IMMED && highValue <= UPPER_IMMED) || difference < LOWER_IMMED || difference > UPPER_IMMED) {
...
TR::Register* OMR::Power::TreeEvaluator::b2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* trgReg;
    if (node->isUnneededConversion() || (child->getOpCodeValue() != TR::bload && child->getOpCodeValue() != TR::bloadi && child->getOpCodeValue() != TR::buload && child->getOpCodeValue() != TR::buloadi && child->getOpCodeValue() != TR::s2b && child->getOpCodeValue() != TR::f2b && child->getOpCodeValue() != TR::d2b && child->getOpCodeValue() != TR::i2b && child->getOpCodeValue() != TR::l2b && child->getOpCodeValue() != TR::iRegLoad && child->getOpCodeValue() != TR::iuRegLoad)) {
...
TR::Register* OMR::Power::TreeEvaluator::iu2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* child = node->getFirstChild();
    if (TR::Compiler->target.is64Bit()) {
        if (node->getOpCodeValue() == TR::iu2l && child && child->getReferenceCount() == 1 && !child->getRegister() && (child->getOpCodeValue() == TR::iloadi || child->getOpCodeValue() == TR::iload)) {
        ...
        if (!cg->useClobberEvaluate() && // Sharing registers between nodes only works when not using simple clobber evaluation
            node->getOpCodeValue() == TR::iu2l && child && child->getRegister() && (child->getOpCodeValue() == TR::iloadi || child->getOpCodeValue() == TR::iload)) {
...
TR::Register* OMR::Power::TreeEvaluator::i2sEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.cpu.id() != TR_PPCp6 && // avoid algebraic loads on P6
        child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference() && child->getRegister() == NULL) {
        TR::MemoryReference* tempMR = new (cg->trHeapMemory()) TR::MemoryReference(child, 2, cg);
        tempMR->addToOffset(node, TR::Compiler->target.cpu.isBigEndian() ? 2 : 0, cg);
        generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node, trgReg, tempMR);
        generateTrg1Src1Instruction(cg, TR::InstOpCode::extsh, node, trgReg, trgReg);
