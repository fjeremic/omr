...
TR::Register* OMR::Power::TreeEvaluator::b2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->isUnneededConversion()
    || (child->getOpCodeValue() != TR::bload && child->getOpCodeValue() != TR::bloadi
      && child->getOpCodeValue() != TR::buload && child->getOpCodeValue() != TR::buloadi
      && child->getOpCodeValue() != TR::s2b && child->getOpCodeValue() != TR::f2b && child->getOpCodeValue() != TR::d2b
      && child->getOpCodeValue() != TR::i2b && child->getOpCodeValue() != TR::l2b
      && child->getOpCodeValue() != TR::iRegLoad && child->getOpCodeValue() != TR::iuRegLoad)) {
...
TR::Register* OMR::Power::TreeEvaluator::s2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR_ASSERT(node->getOpCodeValue() == TR::s2i, "s2iEvaluator called for a %s node", node->getOpCode().getName());
...
TR::Register* OMR::Power::TreeEvaluator::l2cEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (child->getReferenceCount() == 1 && child->getOpCode().isMemoryReference() && (temp == NULL)) {
    ...
    if (TR::Compiler->target.is64Bit())
      ...
      generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwinm, node, trgReg, cg->evaluate(child)->getLowOrder(), 0, 0xffff);
    cg->decReferenceCount(child);
  }
  return node->setRegister(trgReg);
}
