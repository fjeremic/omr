...
TR::Register *OMR::Power::TreeEvaluator::iconstEvaluator(
...
TR::Register *OMR::Power::TreeEvaluator::aconstEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  bool isPicSite = node->isClassPointerConstant() &&
                   cg->fe()->isUnloadAssumptionRequired(
                       (TR_OpaqueClassBlock *)node->getAddress(), method);
  if (!isPicSite)
                ...
                cg->fe()->isUnloadAssumptionRequired(
                    cg->fe()
                        ->createResolvedMethod(
                            cg->trMemory(),
                            (TR_OpaqueMethodBlock *)node->getAddress(), method)
                        ->classOfMethod(),
                    method);

  bool isProfiledPointerConstant =
      node->isClassPointerConstant() || node->isMethodPointerConstant();

  // use data snippet only on class pointers when HCR is enabled
  intptrj_t address =
      TR::Compiler->target.is64Bit() ? node->getLongInt() : node->getInt();
  if (isClass &&
          cg->wantToPatchClassPointer((TR_OpaqueClassBlock *)address, node) ||
      isProfiledPointerConstant && cg->profiledPointersRequireRelocation()) {
    TR::Register *trgReg = cg->allocateRegister();
    loadAddressConstantInSnippet(cg, node, address, trgReg, NULL,
                                 TR::InstOpCode::Op_load, isPicSite, NULL);
...
TR::Register *OMR::Power::TreeEvaluator::lconstEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if ((highValue >= LOWER_IMMED && highValue <= UPPER_IMMED) ||
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, highReg,
                                   lowReg, difference);
  }
...
TR::Register *OMR::Power::TreeEvaluator::inegEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *trgReg;
...
TR::Register *OMR::Power::TreeEvaluator::lnegEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit()) {
...
TR::Register *OMR::Power::TreeEvaluator::iabsEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateRegister();
  ...
  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, trgReg,
                                 sourceRegister, 31);
  generateTrg1Src2Instruction(cg, TR::InstOpCode::XOR, node, tmpReg,
                              sourceRegister, trgReg);
  generateTrg1Src2Instruction(cg, TR::InstOpCode::subfc, node, trgReg, trgReg,
                              tmpReg);
...
TR::Register *OMR::Power::TreeEvaluator::labsEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit()) {
    TR::Register *trgReg = cg->allocateRegister();
    TR::Register *tmpReg = cg->allocateRegister();
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, tempReg,
                                   sourceRegister->getHighOrder(), 31);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::XOR, node, highReg,
                                sourceRegister->getHighOrder(), tempReg);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::XOR, node, lowReg,
                                sourceRegister->getLowOrder(), tempReg);
...
TR::Register *OMR::Power::TreeEvaluator::fabsEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *trgReg;
  trgReg = cg->allocateSinglePrecisionRegister();
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *sourceRegister = cg->evaluate(firstChild);
  generateTrg1Src1Instruction(cg, TR::InstOpCode::fabs, node, trgReg,
                              sourceRegister);
  cg->decReferenceCount(firstChild);
  return node->setRegister(trgReg);
}

TR::Register *OMR::Power::TreeEvaluator::dabsEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *trgReg;
...
TR::Register *OMR::Power::TreeEvaluator::b2iEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::b2aEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::s2iEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg = 0;

  TR_ASSERT(node->getOpCodeValue() == TR::s2i,
            "s2iEvaluator called for a %s node", node->getOpCode().getName());

  if (node->getOpCodeValue() == TR::s2i)  // s2i sign extend
  {
    trgReg = child->getRegister();
    if (!trgReg && child->getOpCode().isMemoryReference() &&
        child->getReferenceCount() == 1) {
      trgReg = cg->allocateRegister();
      TR::MemoryReference *tempMR =
          new (cg->trHeapMemory()) TR::MemoryReference(child, 2, cg);
#ifdef J9_ProjectSpecific
      if (node->getFirstChild()->getOpCodeValue() == TR::irsload) {
        tempMR->forceIndexedForm(node->getFirstChild(), cg);
        generateTrg1MemInstruction(cg, TR::InstOpCode::lhbrx, node, trgReg,
                                   tempMR);
        generateTrg1Src1Instruction(cg, TR::InstOpCode::extsh, node, trgReg,
                                    trgReg);
      } else
#endif
        generateTrg1MemInstruction(cg, TR::InstOpCode::lha, node, trgReg,
                                   tempMR);
...
TR::Register *OMR::Power::TreeEvaluator::b2lEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  if (TR::Compiler->target.is64Bit()) {
    TR::Register *trgReg = cg->allocateRegister();
  ...
  } else  // 32 bit target
  {
    ...
    TR::Register *highReg = cg->allocateRegister();
    TR::RegisterPair *trgReg = cg->allocateRegisterPair(lowReg, highReg);
    generateTrg1Src1Instruction(cg, TR::InstOpCode::extsb, node, lowReg,
                                cg->evaluate(child));
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, highReg,
                                   trgReg->getLowOrder(), 31);
    node->setRegister(trgReg);
    cg->decReferenceCount(child);
    return trgReg;
  }
...
// also handles i2l
TR::Register *OMR::Power::TreeEvaluator::s2lEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  if (TR::Compiler->target.is64Bit()) {
  ...
  } else  // 32 bit target
  {
    ...
    TR::Register *trgReg = cg->allocateRegisterPair(
        cg->gprClobberEvaluate(child), cg->allocateRegister());
...
// also handles su2l
TR::Register *OMR::Power::TreeEvaluator::iu2lEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  if (TR::Compiler->target.is64Bit()) {
    if (node->getOpCodeValue() == TR::iu2l && child &&
        child->getReferenceCount() == 1 && !child->getRegister() &&
    ...
    if (!cg->useClobberEvaluate() &&  // Sharing registers between nodes only
                                      // works when not using simple clobber
                                      // evaluation
    ...
    TR::Register *trgReg = cg->allocateRegister();
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicl, node, trgReg,
                                    cg->evaluate(child), 0,
...
TR::Register *OMR::Power::TreeEvaluator::su2aEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::su2iEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *sourceRegister = NULL;
  TR::Register *trgReg = cg->allocateRegister();
...
TR::Register *OMR::Power::TreeEvaluator::s2aEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
// also handles s2b
TR::Register *OMR::Power::TreeEvaluator::i2bEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
...
TR::Register *OMR::Power::TreeEvaluator::i2cEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *sourceRegister = NULL;
  TR::Register *trgReg = cg->allocateRegister();
  ...
  if (child->getReferenceCount() == 1 &&
    ...
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(child, 2, cg);
    tempMR->addToOffset(node, TR::Compiler->target.cpu.isBigEndian() ? 2 : 0,
                        cg);
    generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node, trgReg, tempMR);
    tempMR->decNodeReferenceCounts(cg);
  } else {
...
TR::Register *OMR::Power::TreeEvaluator::i2sEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg = cg->allocateRegister();

  if (TR::Compiler->target.cpu.id() !=
          TR_PPCp6 &&  // avoid algebraic loads on P6
      child->getReferenceCount() == 1 &&
      child->getOpCode().isMemoryReference() && child->getRegister() == NULL) {
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(child, 2, cg);
...
TR::Register *OMR::Power::TreeEvaluator::i2aEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
    return TR::TreeEvaluator::s2lEvaluator(node, cg);
  else
...
TR::Register *OMR::Power::TreeEvaluator::iu2aEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
    return TR::TreeEvaluator::iu2lEvaluator(node, cg);
  else
...
TR::Register *OMR::Power::TreeEvaluator::l2bEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg;
  ...
  if (child->getReferenceCount() == 1 &&
    ...
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(child, 1, cg);
    trgReg = cg->allocateRegister();
    tempMR->addToOffset(node, TR::Compiler->target.cpu.isBigEndian() ? 7 : 0,
                        cg);
...
// is this used?
TR::Register *OMR::Power::TreeEvaluator::l2buEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg = cg->allocateRegister();
...
TR::Register *OMR::Power::TreeEvaluator::l2cEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *temp;
  TR::Register *trgReg = cg->allocateRegister();
  ...
  if (child->getReferenceCount() == 1 &&
    ...
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(child, 2, cg);
    tempMR->addToOffset(node, TR::Compiler->target.cpu.isBigEndian() ? 6 : 0,
                        cg);
    generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node, trgReg, tempMR);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    if (TR::Compiler->target.is64Bit())
      ...
      generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, trgReg,
                                      cg->evaluate(child)->getLowOrder(), 0,
                                      0xffff);
    cg->decReferenceCount(child);
  }
  return node->setRegister(trgReg);
...
TR::Register *OMR::Power::TreeEvaluator::l2sEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg = cg->allocateRegister();
...
TR::Register *OMR::Power::TreeEvaluator::l2iEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg;
  TR::Register *temp;
  ...
  if (child->getReferenceCount() == 1 &&
    ...
    tempMR->addToOffset(node, TR::Compiler->target.cpu.isBigEndian() ? 4 : 0,
                        cg);
...
TR::Register *OMR::Power::TreeEvaluator::l2aEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (comp->useCompressedPointers()) {
      ...
      //
      /// TR_ASSERT(hasCompPtrs, "no compression sequence found under l2a node
      /// [%p]\n", node);
...
TR::Register *OMR::Power::TreeEvaluator::su2lEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *sourceRegister = NULL;
  if (TR::Compiler->target.is64Bit()) {
  ...
  } else  // 32 bit target
  {
    ...
    if (child->getReferenceCount() == 1 &&
      ...
      TR::MemoryReference *tempMR =
          new (cg->trHeapMemory()) TR::MemoryReference(child, 2, cg);
      generateTrg1MemInstruction(cg, TR::InstOpCode::lhz, node,
                                 trgReg->getLowOrder(), tempMR);
      tempMR->decNodeReferenceCounts(cg);
    } else {
    ...
    if (decSharedNode) cg->decReferenceCount(child);

    return trgReg;
  }
}

TR::Register *OMR::Power::TreeEvaluator::bu2iEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg;
...
TR::Register *OMR::Power::TreeEvaluator::bu2lEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg;
  ...
  } else  // 32 bit target
  {
    ...
    if (child->getOpCode().isMemoryReference()) {
      ...
      generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, lowReg,
                                      cg->evaluate(child), 0, 0xff);
    }
...
TR::Register *OMR::Power::TreeEvaluator::bu2aEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::a2iEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
    return TR::TreeEvaluator::l2iEvaluator(node, cg);
  else
...
TR::Register *OMR::Power::TreeEvaluator::a2lEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
    return TR::TreeEvaluator::passThroughEvaluator(node, cg);
  else
...
TR::Register *OMR::Power::TreeEvaluator::a2bEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::a2sEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::libmFuncEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::strcmpEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::strcFuncEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::dfloorEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  ...
  if ((node->getOpCodeValue() == TR::dfloor) ||
      (node->getOpCodeValue() == TR::dceil)) {
    trgReg = cg->allocateRegister(TR_FPR);
  ...
  generateTrg1Src3Instruction(cg, TR::InstOpCode::fsel, node, trgReg, tmp3Reg,
                              trgReg, tmp2Reg);

  // if x is a NaN or out of range  (i.e. fabs(x)>2**52), then floor(x) = x,
  // otherwise the value computed above

  generateTrg1Src1Instruction(cg, TR::InstOpCode::fabs, node, tmp4Reg,
                              srcReg);  // tmp4  = fabs(x)

  generateTrg1Src2Instruction(cg, TR::InstOpCode::fsub, node, tmp5Reg, tmp5Reg,
                              tmp4Reg);  // tmp5  = 2**52 - fabs(x)

  generateTrg1Src3Instruction(cg, TR::InstOpCode::fsel, node, trgReg, tmp5Reg,
                              trgReg, srcReg);
