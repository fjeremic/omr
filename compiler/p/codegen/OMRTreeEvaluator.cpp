...
TR::Instruction* loadAddressConstantInSnippet(TR::CodeGenerator* cg, TR::Node* node, intptrj_t address,
...
TR::Instruction* loadAddressConstant(TR::CodeGenerator* cg, bool isRelocatable, TR::Node* node, intptrj_t value,
...
// loadAddressConstant could be merged with loadConstant 64-bit
TR::Instruction* loadAddressConstant(TR::CodeGenerator* cg, TR::Node* node, intptrj_t value, TR::Register* trgReg,
...
TR::Instruction* loadActualConstant(
  TR::CodeGenerator* cg, TR::Node* node, intptrj_t value, TR::Register* trgReg, TR::Instruction* cursor, bool isPicSite)
{
  ...
  TR::Instruction* cursor, bool isPicSite, bool useTOC)
{
  ...
  bool canUseTOC = (!TR::isJ9() || !isPicSite) && !comp->getOption(TR_DisableTOCForConsts);
  if (canUseTOC && useTOC)
  ...
  if (offset != PTOC_FULL_INDEX) {
    ...
    if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
      ...
      cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, trgReg,
        new (cg->trHeapMemory()) TR::MemoryReference(trgReg, LO_VALUE(offset), 8, cg), cursor);
    } else {
      cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, trgReg,
        new (cg->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), offset, 8, cg), cursor);
    }
  } else {
    // Could not use TOC.
...
TR::Instruction* fixedSeqMemAccess(TR::CodeGenerator* cg, TR::Node* node, intptrj_t addr, TR::Instruction** nibbles,
  ...
  TR::Instruction* cursor, TR::Register* tempReg)
{
  ...
  if (TR::Compiler->target.is32Bit()) {
    ...
    if (tempReg == NULL) {
      ...
      nibbles[2] = cursor
        = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::oris, node, baseReg, baseReg, hiAddr & 0x0000FFFF, cursor);
    } else {
      nibbles[0] = cursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, tempReg, hiAddr >> 32, cursor);
      nibbles[2] = cursor
        = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, baseReg, hiAddr & 0x0000FFFF, cursor);
      nibbles[1] = cursor = generateTrg1Src1ImmInstruction(
  ...
  if (opCode == TR::InstOpCode::lxvdsx) {
    ...
    if (TR::Compiler->target.is64Bit() && tempReg) {
      nibbles[idx] = cursor
        = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, tempReg, ((int32_t)loAddr) << 16 >> 16, cursor);
    ...
    TR::MemoryReference* memRef
      = new (cg->trHeapMemory()) TR::MemoryReference(baseReg, ((int32_t)loAddr) << 16 >> 16, opSize, cg);
    if (op.isLoad())
...
TR::Register* OMR::Power::TreeEvaluator::iloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (reverseLoad) {
    ...
    // For compr refs caseL if monitored loads is supported and we are loading an address, use monitored loads
    // instruction
  ...
  if (needSync) {
    TR::TreeEvaluator::postSyncConditions(node, cg, tempReg, tempMR,
      TR::Compiler->target.cpu.id() >= TR_PPCp7 ? TR::InstOpCode::lwsync : TR::InstOpCode::isync);
  }
...
TR::Register* OMR::Power::TreeEvaluator::aloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  bool checkFeedingVGDNOP = !disableLoadForVGDNOP && node->getOpCodeValue() == TR::aloadi && node->getSymbolReference()
    && node->getReferenceCount() == 2 && !node->getSymbolReference()->isUnresolved()
  ...
  if (OMR_UNLIKELY(checkFeedingVGDNOP)) {
    ...
    if (!nodeIsNeeded(topNode, node) && (comp->getSymRefTab()->isNonHelper(index, TR::SymbolReferenceTable::vftSymbol))
...
TR::Register* OMR::Power::TreeEvaluator::lloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (needSync) {
      TR::TreeEvaluator::postSyncConditions(node, cg, trgReg, tempMR,
        TR::Compiler->target.cpu.id() >= TR_PPCp7 ? TR::InstOpCode::lwsync : TR::InstOpCode::isync);
    }
    ...
    if (reverseLoad) // 32-bit reverse load. Handles sync
    {
      ...
      if (needSync) {
        TR::MemoryReference* tempMRStore1
          = new (cg->trHeapMemory()) TR::MemoryReference(node, location->getSymbolReference(), 8, cg);
        generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node, tempMRStore1, doubleReg);
        generateInstruction(
          cg, TR::Compiler->target.cpu.id() >= TR_PPCp7 ? TR::InstOpCode::lwsync : TR::InstOpCode::isync, node);
      ...
      if (!node->getSymbolReference()->isUnresolved() && cg->is64BitProcessor()) // resolved as volatile
      {
        ...
        if (TR::Compiler->target.isSMP()) {
          ...
          TR::MemoryReference* tempMRLoad1
            = new (cg->trHeapMemory()) TR::MemoryReference(node, *tempMRStore1, 0, 4, cg);
          TR::MemoryReference* tempMRLoad2
            = new (cg->trHeapMemory()) TR::MemoryReference(node, *tempMRStore1, 4, 4, cg);
          generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node, tempMRStore1, doubleReg);
          generateInstruction(
            cg, TR::Compiler->target.cpu.id() >= TR_PPCp7 ? TR::InstOpCode::lwsync : TR::InstOpCode::isync, node);
        ...
        if (tempMR->getIndexRegister() != NULL)
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::add, node, highReg, tempMR->getBaseRegister(), tempMR->getIndexRegister());
        else
        ...
        generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
          (uintptrj_t)vrlRef->getSymbol()->castToMethodSymbol()->getMethodAddress(), dependencies, vrlRef);
...
TR::Register* OMR::Power::TreeEvaluator::istoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->getOpCode().isIndirect()) {
    ...
    if (comp->useCompressedPointers() && (node->getSymbolReference()->getSymbol()->getDataType() == TR::Address)) {
      ...
      if ((translatedNode->getOpCode().isSub()) || valueChild->isNull() || TR::Compiler->vm.heapBaseAddress() == 0)
  ...
  if (needSync && caller && !comp->compileRelocatableCode()) {
    TR_ResolvedMethod* m
      = comp->fe()->createResolvedMethod(cg->trMemory(), caller, node->getSymbolReference()->getOwningMethod(comp));
...
TR::Register* OMR::Power::TreeEvaluator::astoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  //
  bool isClass
    = (node->getSymbol()->isClassObject() || (node->getSymbolReference() == comp->getSymRefTab()->findVftSymbolRef()));
  TR::InstOpCode::Mnemonic storeOp = TR::InstOpCode::stw;
  if (TR::Compiler->target.is64Bit() && !(isClass && TR::Compiler->om.generateCompressedObjectHeaders()))
    ...
    if (needSync) {
      ...
      if (!node->getSymbolReference()->isUnresolved() && cg->is64BitProcessor()) // resolved as volatile
      {
        ...
        if (TR::Compiler->target.isSMP()) {
          ...
          location = cg->allocateSpill(8, false, NULL);
          TR::MemoryReference* tempMRStore1
            = new (cg->trHeapMemory()) TR::MemoryReference(node, location->getSymbolReference(), 4, cg);
          ...
          if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
            ...
            generateMvFprGprInstructions(
              cg, node, gpr2fprHost32, false, doubleReg, valueReg->getHighOrder(), valueReg->getLowOrder(), tmp1);
            cg->stopUsingRegister(tmp1);
          } else
            generateMvFprGprInstructions(
              cg, node, gpr2fprHost32, false, doubleReg, valueReg->getHighOrder(), valueReg->getLowOrder());
          generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node, tempMR, doubleReg);
        }
        cg->stopUsingRegister(doubleReg);
      } else {
        ...
        if (tempMR->getIndexRegister() != NULL)
          generateTrg1Src2Instruction(
            cg, TR::InstOpCode::add, node, addrReg, tempMR->getBaseRegister(), tempMR->getIndexRegister());
        else
        ...
        generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
          (uintptrj_t)vwlRef->getSymbol()->castToMethodSymbol()->getMethodAddress(), dependencies, vwlRef);
...
TR::Register* OMR::Power::TreeEvaluator::viremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::addi2, node, srcV1IdxReg, new (cg->trHeapMemory()) TR::MemoryReference(node, srcV1, 16, cg));
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::addi2, node, srcV2IdxReg, new (cg->trHeapMemory()) TR::MemoryReference(node, srcV2, 16, cg));

  // store the src regs to memory
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stxvw4x, node, new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcV1IdxReg, 16, cg), lhsReg);
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stxvw4x, node, new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcV2IdxReg, 16, cg), rhsReg);
  ...
  for (i = 0; i < 4; i++) {
    ...
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwa, node, srcA1Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcV1IdxReg, i * 4, 4, cg));
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwa, node, srcA2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcV2IdxReg, i * 4, 4, cg));
    ...
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(srcV1IdxReg, i * 4, 4, cg), trgAReg);
  ...
  TR::Register* resReg = cg->allocateRegister(TR_VRF);
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lxvw4x, node, resReg, new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcV1IdxReg, 16, cg));
...
TR::Register* OMR::Power::TreeEvaluator::vigetelemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::addi2, node, resReg, new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 16, cg));
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stxvw4x, node, new (cg->trHeapMemory()) TR::MemoryReference(NULL, resReg, 16, cg), addrReg);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwa, node, resReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * 4, 4, cg));
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lwa, node, resReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, offsetReg, 4, cg));
...
TR::Register* OMR::Power::TreeEvaluator::getvelemDirectMoveHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (elementCount == 4) {
       ...
       */
      bool skipSplat = (firstChild->getDataType() == TR::VectorInt32 && 1 == elem)
    ...
    if (firstChild->getDataType() == TR::VectorInt32) {
       ...
       * invalid. The element indicated by indexReg needs to be move to element 1 in intermediateResReg. In most cases,
       * this is done by splatting the indicated value into every element of intermediateResReg. If the indicated value
       * is already in element 1 of srcVectorReg, the vector is just copied to intermediateResReg. Finally, a direct
       * move instruction is used to move the data we want from element 1 of the vector into a GPR.
       */
      ...
      /*
       * Conditional statements are used to determine if the indexReg has the value 0, 1, 2 or 3. Other values are
       * invalid. The element indicated by indexReg needs to be move to element 0 in intermediateResReg. In most cases,
       * this is done by splatting the indicated value into every element of intermediateResReg. If the indicated value
       * is already in element 0 of srcVectorReg, the vector is just copied to intermediateResReg. Finally, a direct
       * move instruction is used to move the data we want from element 0 of the vector into an FPR.
       */
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, indexReg, 1);
      generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, jumpLabel23, condReg);
...
TR::Register* OMR::Power::TreeEvaluator::getvelemMemoryMoveHelper(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::addi2, node, resReg, new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 16, cg));
  generateMemSrc1Instruction(
    cg, vecStoreOpCode, node, new (cg->trHeapMemory()) TR::MemoryReference(NULL, resReg, 16, cg), vectorReg);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (firstChild->getDataType() == TR::VectorFloat || firstChild->getDataType() == TR::VectorDouble) {
      generateTrg1MemInstruction(cg, loadOpCode, node, fResReg,
        new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * (16 / elementCount), 16 / elementCount, cg));
      ...
      if (!TR::Compiler->target.cpu.isLittleEndian()) {
        ...
        lowResReg = resReg;
        generateTrg1MemInstruction(
          cg, loadOpCode, node, highResReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * 8, 4, cg));
        generateTrg1MemInstruction(
          cg, loadOpCode, node, lowResReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * 8 + 4, 4, cg));
      } else {
        highResReg = resReg;
        lowResReg = cg->allocateRegister();
        generateTrg1MemInstruction(
          cg, loadOpCode, node, lowResReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * 8, 4, cg));
        generateTrg1MemInstruction(
          cg, loadOpCode, node, highResReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, elem * 8 + 4, 4, cg));
      }
      resReg = cg->allocateRegisterPair(lowResReg, highResReg);
    } else {
  ...
  if (firstChild->getDataType() == TR::VectorFloat || firstChild->getDataType() == TR::VectorDouble) {
    generateTrg1MemInstruction(cg, loadOpCode, node, fResReg,
      new (cg->trHeapMemory()) TR::MemoryReference(resReg, offsetReg, 16 / elementCount, cg));
    cg->stopUsingRegister(resReg);
  } else if (TR::Compiler->target.is32Bit() && firstChild->getDataType() == TR::VectorInt64) {
    if (!TR::Compiler->target.cpu.isLittleEndian()) {
      highResReg = cg->allocateRegister();
      lowResReg = resReg;
      generateTrg1MemInstruction(
        cg, loadOpCode, node, highResReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, offsetReg, 4, cg));
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, offsetReg, offsetReg, 4);
      generateTrg1MemInstruction(
        cg, loadOpCode, node, lowResReg, new (cg->trHeapMemory()) TR::MemoryReference(resReg, offsetReg, 4, cg));
    } else {
      highResReg = resReg;
      lowResReg = cg->allocateRegister();
      generateTrg1MemInstruction(
...
TR::Register* OMR::Power::TreeEvaluator::visetelemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::SymbolReference* localTemp = cg->allocateLocalTemp(TR::VectorInt32);
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::addi2, node, addrReg, new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 16, cg));
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stxvw4x, node, new (cg->trHeapMemory()) TR::MemoryReference(NULL, addrReg, 16, cg), vectorReg);
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(addrReg, elem * 4, 4, cg), valueReg);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::mulli, node, offsetReg, idxReg, 4);
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(addrReg, offsetReg, 4, cg), valueReg);
    cg->stopUsingRegister(offsetReg);
  }

  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lxvw4x, node, resReg, new (cg->trHeapMemory()) TR::MemoryReference(NULL, addrReg, 16, cg));
...
TR::Register* OMR::Power::TreeEvaluator::vdmergeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  node->setRegister(resReg);
  generateTrg1Src2ImmInstruction(
    cg, TR::InstOpCode::xxpermdi, node, resReg, lhsReg, rhsReg, node->getOpCodeValue() == TR::vdmergeh ? 0x00 : 0x3);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
  return resReg;
}
...
TR::Register* OMR::Power::TreeEvaluator::vdlogEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::SymbolReference* helper
    = cg->comp()->getSymRefTab()->findOrCreateRuntimeHelper(TR_PPCVectorLogDouble, false, false, true);
...
bool OMR::Power::TreeEvaluator::inlineVectorCompareBranch(
  TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
  ...
  if (!node->getSecondChild()->getOpCode().isLoadConst()
    || (node->getSecondChild()->getInt() != 0 && node->getSecondChild()->getInt() != 1))
    return false;
  ...
  if (node->getNumChildren() == 3) {
    ...
    generateDepConditionalBranchInstruction(
      cg, branchOp, vecCmpNode, dstLabel, cndReg, generateRegisterDependencyConditions(cg, thirdChild, 0));
...
TR::Register* OMR::Power::TreeEvaluator::inlineVectorCompareAllOrAnyOp(
  TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic vcmpOp, TR::InstOpCode::Mnemonic branchOp)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
...
TR::Register* OMR::Power::TreeEvaluator::vicmpallHelper(
  TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, int nBit)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register *lhsReg = NULL, *rhsReg = NULL;
  ...
  TR::Node* node, int64_t byteLen, TR::Register* src, TR::Register* dst, TR::CodeGenerator* cg)
{
  ...
  bool supportsLEArrayCopyInline = (processor >= TR_PPCp8) && !disableLEArrayCopyInline
    && TR::Compiler->target.cpu.isLittleEndian() && TR::Compiler->target.cpu.hasFPU() && TR::Compiler->target.is64Bit();
  ...
  if (groups != 0) {
    ...
    if (supportsLEArrayCopyInline) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfd, node, fpRegs[3], new (cg->trHeapMemory()) TR::MemoryReference(src, 0, memRefSize, cg));
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfd, node, fpRegs[2],
        new (cg->trHeapMemory()) TR::MemoryReference(src, memRefSize, memRefSize, cg));
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfd, node, fpRegs[1],
        new (cg->trHeapMemory()) TR::MemoryReference(src, 2 * memRefSize, memRefSize, cg));
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfd, node, fpRegs[0],
        new (cg->trHeapMemory()) TR::MemoryReference(src, 3 * memRefSize, memRefSize, cg));
    } else {
      generateTrg1MemInstruction(
        cg, load, node, regs[3], new (cg->trHeapMemory()) TR::MemoryReference(src, 0, memRefSize, cg));
      generateTrg1MemInstruction(
        cg, load, node, regs[2], new (cg->trHeapMemory()) TR::MemoryReference(src, memRefSize, memRefSize, cg));
      generateTrg1MemInstruction(
        cg, load, node, regs[1], new (cg->trHeapMemory()) TR::MemoryReference(src, 2 * memRefSize, memRefSize, cg));
      generateTrg1MemInstruction(
        cg, load, node, regs[0], new (cg->trHeapMemory()) TR::MemoryReference(src, 3 * memRefSize, memRefSize, cg));
    }
    if (groups != 1)
    ...
    if (supportsLEArrayCopyInline) {
      generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
        new (cg->trHeapMemory()) TR::MemoryReference(dst, 0, memRefSize, cg), fpRegs[3]);
      generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
        new (cg->trHeapMemory()) TR::MemoryReference(dst, memRefSize, memRefSize, cg), fpRegs[2]);
      generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
        new (cg->trHeapMemory()) TR::MemoryReference(dst, 2 * memRefSize, memRefSize, cg), fpRegs[1]);
      generateMemSrc1Instruction(cg, TR::InstOpCode::stfd, node,
        new (cg->trHeapMemory()) TR::MemoryReference(dst, 3 * memRefSize, memRefSize, cg), fpRegs[0]);
    } else {
      generateMemSrc1Instruction(
        cg, store, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, 0, memRefSize, cg), regs[3]);
      generateMemSrc1Instruction(
        cg, store, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, memRefSize, memRefSize, cg), regs[2]);
      generateMemSrc1Instruction(
        cg, store, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, 2 * memRefSize, memRefSize, cg), regs[1]);
      generateMemSrc1Instruction(
        cg, store, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, 3 * memRefSize, memRefSize, cg), regs[0]);
    }
    if (groups != 1) {
  ...
  for (; residual >= memRefSize; residual -= memRefSize, ix += memRefSize) {
    if (supportsLEArrayCopyInline) {
      ...
      if (fpRegIx > 3 || fpRegs[fpRegIx] == NULL)
        fpRegIx = 0;
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfd, node, oneReg, new (cg->trHeapMemory()) TR::MemoryReference(src, ix, memRefSize, cg));
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stfd, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, memRefSize, cg), oneReg);
    } else {
      TR::Register* oneReg = regs[regIx++];
      if (regIx > 3 || regs[regIx] == NULL)
        regIx = 0;
      generateTrg1MemInstruction(
        cg, load, node, oneReg, new (cg->trHeapMemory()) TR::MemoryReference(src, ix, memRefSize, cg));
      generateMemSrc1Instruction(
        cg, store, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, memRefSize, cg), oneReg);
    }
  }
    ...
    if (residual & 4) {
      if (supportsLEArrayCopyInline) {
        ...
        if (fpRegIx > 3 || fpRegs[fpRegIx] == NULL)
          fpRegIx = 0;
        generateTrg1MemInstruction(
          cg, TR::InstOpCode::lfs, node, oneReg, new (cg->trHeapMemory()) TR::MemoryReference(src, ix, 4, cg));
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stfs, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, 4, cg), oneReg);
      } else {
        TR::Register* oneReg = regs[regIx++];
        if (regIx > 3 || regs[regIx] == NULL)
          regIx = 0;
        generateTrg1MemInstruction(
          cg, TR::InstOpCode::lwz, node, oneReg, new (cg->trHeapMemory()) TR::MemoryReference(src, ix, 4, cg));
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, 4, cg), oneReg);
      }
      ix += 4;
    }
    if (residual & 2) {
      ...
      if (regIx > 3 || regs[regIx] == NULL)
        regIx = 0;
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lhz, node, oneReg, new (cg->trHeapMemory()) TR::MemoryReference(src, ix, 2, cg));
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::sth, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, 2, cg), oneReg);
      ix += 2;
    }
    if (residual & 1) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lbz, node, regs[regIx], new (cg->trHeapMemory()) TR::MemoryReference(src, ix, 1, cg));
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stb, node, new (cg->trHeapMemory()) TR::MemoryReference(dst, ix, 1, cg), regs[regIx]);
    }
  }
...
static void inlineVSXArrayCopy(TR::Node* node, TR::Register* srcAddrReg, TR::Register* dstAddrReg,
  TR::Register* lengthReg, TR::Register* cndRegister, TR::Register* tmp1Reg, TR::Register* tmp2Reg,
  TR::Register* tmp3Reg, TR::Register* tmp4Reg, TR::Register* fp1Reg, TR::Register* fp2Reg, TR::Register* vec0Reg,
  TR::Register* vec1Reg, bool supportsLEArrayCopy, TR::RegisterDependencyConditions* conditions, TR::CodeGenerator* cg)
{
  ...
  generateMemInstruction(
    cg, TR::InstOpCode::dcbt, node, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 16, cg));
  generateMemInstruction(
    cg, TR::InstOpCode::dcbtst, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 16, cg));
  ...
  if (!node->isForwardArrayCopy()) {
    ...
    generateTrg1Src2Instruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmpl8 : TR::InstOpCode::cmpl4,
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lbz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 1, cg));
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stb, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 1, cg), tmp2Reg);
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lhz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 2, cg));
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::sth, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 2, cg), tmp2Reg);
  ...
  if (supportsLEArrayCopy) {
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lfs, node, fp1Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 4, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stfs, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), fp1Reg);
  } else {
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 4, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), tmp2Reg);
  }
  ...
  if (supportsLEArrayCopy) {
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lfd, node, fp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 8, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stfd, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 8, cg), fp2Reg);
  } else if (TR::Compiler->target.is64Bit()) {
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::ld, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 8, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::std, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 8, cg), tmp2Reg);
  } else {
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwz, node, tmp1Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 0, 4, cg));
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, 4, 4, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), tmp1Reg);
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 4, 4, cg), tmp2Reg);
  }
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lxvw4x, node, vec0Reg, new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcAddrReg, 16, cg));
  ...
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stxvw4x, node, new (cg->trHeapMemory()) TR::MemoryReference(NULL, dstAddrReg, 16, cg), vec0Reg);
  generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
    new (cg->trHeapMemory()) TR::MemoryReference(tmp2Reg, dstAddrReg, 16, cg), vec1Reg);
  ...
  generateConditionalBranchInstruction(cg, TR::InstOpCode::blt, node, quadWordAligned_vsx_residue8, cndRegister);
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lxvw4x, node, vec1Reg, new (cg->trHeapMemory()) TR::MemoryReference(NULL, srcAddrReg, 16, cg));
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stxvw4x, node, new (cg->trHeapMemory()) TR::MemoryReference(NULL, dstAddrReg, 16, cg), vec1Reg);
  ...
  if (!node->isForwardArrayCopy()) {
    ...
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, reverseQuadWordAligned_vsx_residue8, cndRegister);

    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 7);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, reverseQuadWordAlignment_vsx_test8, cndRegister);

    // check 2 aligned
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 1);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, reverseQuadWordAlignment_vsx_test2, cndRegister);
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lbz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -1, 1, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stb, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -1, 1, cg), tmp2Reg);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -1);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -1);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -1);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 2);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, reverseQuadWordAlignment_vsx_test4, cndRegister);
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lhz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -2, 2, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::sth, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -2, 2, cg), tmp2Reg);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -2);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -2);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -2);
    ...
    if (supportsLEArrayCopy) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfs, node, fp1Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -4, 4, cg));
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stfs, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -4, 4, cg), fp1Reg);
    } else {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -4, 4, cg));
      generateMemSrc1Instruction(
    ...
    if (supportsLEArrayCopy) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfd, node, fp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -8, 8, cg));
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stfd, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -8, 8, cg), fp2Reg);
    } else if (TR::Compiler->target.is64Bit()) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::ld, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -8, 8, cg));
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::std, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -8, 8, cg), tmp2Reg);
    } else {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwz, node, tmp1Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -4, 4, cg));
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -8, 4, cg));
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -4, 4, cg), tmp1Reg);
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -8, 4, cg), tmp2Reg);
    }
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -8);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -8);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -8);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndRegister, tmp4Reg, 0);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, reverseQuadWordAligned_vsx_residue32, cndRegister);
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tmp4Reg);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec0Reg,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp2Reg, srcAddrReg, 16, cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec1Reg,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp1Reg, srcAddrReg, 16, cg));
    generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp2Reg, dstAddrReg, 16, cg), vec0Reg);
    generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp1Reg, dstAddrReg, 16, cg), vec1Reg);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -32);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -32);
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec0Reg,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp2Reg, srcAddrReg, 16, cg));
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 32);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, reverseQuadWordAligned_vsx_residue16, cndRegister);
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec0Reg,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp2Reg, srcAddrReg, 16, cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, vec1Reg,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp1Reg, srcAddrReg, 16, cg));
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 8);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, reverseQuadWordAligned_vsx_residue4, cndRegister);
    if (supportsLEArrayCopy) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfd, node, fp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -8, 8, cg));
      generateMemSrc1Instruction(
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 4);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, reverseQuadWordAligned_vsx_residue2, cndRegister);
    if (supportsLEArrayCopy) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfs, node, fp1Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -4, 4, cg));
      generateMemSrc1Instruction(
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 2);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, reverseQuadWordAligned_vsx_residue1, cndRegister);
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lhz, node, tmp2Reg, new (cg->trHeapMemory()) TR::MemoryReference(srcAddrReg, -2, 2, cg));
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::sth, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, -2, 2, cg), tmp2Reg);
...
{
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::Op_load, node, src1, new (cg->trHeapMemory()) TR::MemoryReference(src1Addr, 0, loadSize, cg));
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::Op_load, node, src2, new (cg->trHeapMemory()) TR::MemoryReference(src2Addr, 0, loadSize, cg));
  if (loadSize != byteLen) {
...
static TR::Register* inlineArrayCmp(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src1ImmInstruction(cg, (byteLen == 8) ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node, condReg2,
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::srawi, node, tempReg, byteLenRemainingRegister, (byteLen == 8) ? 3 : 2);
  ...
  if (byteLen == 4) {
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwzu, node, src1Reg, new (cg->trHeapMemory()) TR::MemoryReference(src1AddrReg, 4, 4, cg));
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwzu, node, src2Reg, new (cg->trHeapMemory()) TR::MemoryReference(src2AddrReg, 4, 4, cg));
  } else {
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::ldu, node, src1Reg, new (cg->trHeapMemory()) TR::MemoryReference(src1AddrReg, 8, 8, cg));
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::ldu, node, src2Reg, new (cg->trHeapMemory()) TR::MemoryReference(src2AddrReg, 8, 8, cg));
  }
  ...
  generateTrg1Src1ImmInstruction(
    cg, (byteLen == 8) ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node, condReg2, byteLenRemainingRegister, 0);
  generateTrg1Src2Instruction(
  ...
  generateTrg1Src2Instruction(cg, (byteLen == 8) ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4, node, condReg2,
  ...
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lbzu, node, src1Reg, new (cg->trHeapMemory()) TR::MemoryReference(src1AddrReg, 1, 1, cg));
  generateTrg1MemInstruction(
    cg, TR::InstOpCode::lbzu, node, src2Reg, new (cg->trHeapMemory()) TR::MemoryReference(src2AddrReg, 1, 1, cg));
  ...
  TR::RegisterDependencyConditions* dependencies
    = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(numRegs, numRegs, cg->trMemory());
...
{
  TR::SymbolReference* helperSym = cg->symRefTab()->findOrCreateRuntimeHelper(helperIndex, false, false, false);

  return generateDepImmSymInstruction(
    cg, TR::InstOpCode::bl, node, (uintptrj_t)helperSym->getMethodAddress(), conditions, helperSym);
}
...
TR::Register* OMR::Power::TreeEvaluator::setmemoryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateLabelInstruction(cg, TR::InstOpCode::label, node, loopStartLabel);
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::std, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 8, cg), valueReg);
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::std, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 8, 8, cg), valueReg);
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::std, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 16, 8, cg), valueReg);
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::std, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 24, 8, cg), valueReg);
  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 32);
  ...
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 4, cg), valueReg);
  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 4);
  ...
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::sth, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 2, cg), valueReg);
  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 2);
  ...
  generateMemSrc1Instruction(
    cg, TR::InstOpCode::stb, node, new (cg->trHeapMemory()) TR::MemoryReference(dstAddrReg, 0, 1, cg), valueReg);
...
TR::Register* OMR::Power::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  static bool disableVSXArrayCopyInlining
    = (feGetEnv("TR_enableVSXArrayCopyInlining") == NULL); // Disabling due to a performance regression

  bool supportsLEArrayCopy
    = !disableLEArrayCopyHelper && TR::Compiler->target.cpu.isLittleEndian() && TR::Compiler->target.cpu.hasFPU();
  ...
  if (!disableVSXArrayCopyInlining && supportsVSX) {
    inlineVSXArrayCopy(node, srcAddrReg, dstAddrReg, lengthReg, cndRegister, tmp1Reg, tmp2Reg, tmp3Reg, tmp4Reg, fp1Reg,
      fp2Reg, vec0Reg, vec1Reg, supportsLEArrayCopy, conditions, cg);
  } else {
    TR_RuntimeHelper helper;
...
static TR::Register* inlineLongHighestOneBit(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::srw, node, targetRegister->getHighOrder(), targetRegister->getHighOrder(), tempRegister);
    generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
  ...
  TR::Node* node, bool isAddOp, bool isLong, bool isGetThenUpdate, TR::CodeGenerator* cg)
{
  ...
  if (deltaChild->getOpCode().isLoadConst() && !deltaChild->getRegister()
    && (deltaChild->getDataType() == TR::Int32
      || (deltaChild->getDataType() == TR::Int64 && deltaChild->getLongInt() <= UPPER_IMMED
    ...
    } else if (deltaLong & 0xFFFF == 0 && ((deltaLong >> 32) == -1 || (deltaLong >> 32) == 0)) {
  ...
  if (isAddOp) {
    ...
    if (isDeltaImmediateShifted)
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addis, node, newValueReg, oldValueReg, ((delta & 0xFFFF0000) >> 16));
  ...
  generateMemSrc1Instruction(cg, isLong ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r, node,
    new (cg->trHeapMemory()) TR::MemoryReference(0, valueAddrReg, len, cg), newValueReg);
  ...
  conditions
    = new (cg->trHeapMemory()) TR::RegisterDependencyConditions((uint16_t)numDeps, (uint16_t)numDeps, cg->trMemory());
        ...
        if (mref->hasDelayedOffset()) {
          ...
          if (offset >= LOWER_IMMED && offset <= UPPER_IMMED) {
            ...
            src2ForStatic = generateTrg1Src1ImmInstruction(
              cg, TR::InstOpCode::addi2, node, resultReg, mref->getBaseRegister(), offset);
            if (mref->getStaticRelocation() != NULL)
              mref->getStaticRelocation()->setSource2Instruction(src2ForStatic);
          } else {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, resultReg, mref->getBaseRegister(),
...
TR::Register* OMR::Power::TreeEvaluator::passThroughEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  bool skipCopy = (node->getOpCodeValue() == TR::a2i && node->getReferenceCount() == 1 && currentTop->getOpCode().isIf()
...
TR::Register* OMR::Power::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if ((!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) && node->getNumChildren() > 0) {
    ...
    lastInst = generateDepLabelInstruction(
      cg, TR::InstOpCode::label, node, generateLabelSymbol(cg), generateRegisterDependencyConditions(cg, child, 0));
    cg->decReferenceCount(child);
  }
  ...
  if ((!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock())) {
    ...
    assoc
      = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, TR::RealRegister::NumRegisters, cg->trMemory());
...
TR::Register* OMR::Power::TreeEvaluator::cmpsetEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::MemoryReference* ldMemRef = new (cg->trHeapMemory()) TR::MemoryReference(0, ptrReg, size, cg);
  TR::MemoryReference* stMemRef = new (cg->trHeapMemory()) TR::MemoryReference(0, ptrReg, size, cg);
  ...
  TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
  ...
  int32_t callerSaveTOCOffset
    = (TR::Compiler->target.cpu.isBigEndian() ? 5 : 3) * TR::Compiler->om.sizeofReferenceAddress();

  cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
    new (cg->trHeapMemory())
      TR::MemoryReference(grSysStackReg, callerSaveTOCOffset, TR::Compiler->om.sizeofReferenceAddress(), cg),
    grTOCReg, cursor);
...
void OMR::Power::TreeEvaluator::restoreTOCRegister(
  TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
  // Here we restore the JIT TOC after returning from a call out. We're restoring from the caller TOC slot as defined by
  // the ABI.
  TR::Register* grTOCReg = cg->machine()->getRealRegister(TR::RealRegister::gr2);
  TR::Register* grSysStackReg = cg->machine()->getRealRegister(TR::RealRegister::gr1);
  TR::Compilation* comp = cg->comp();
...
TR::Register* OMR::Power::TreeEvaluator::ibyteswapEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!firstChild->getRegister() && firstChild->getOpCode().isMemoryReference()
