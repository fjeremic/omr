...
{
    bool result = (checkNode->getOpCode().isCall() || (checkNode != node && (checkNode->getOpCodeValue() == TR::aloadi || checkNode->getOpCodeValue() == TR::aload)));
    TR::Node* child = NULL;
...
TR::Register* OMR::Power::TreeEvaluator::aloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool checkFeedingVGDNOP = !disableLoadForVGDNOP && node->getOpCodeValue() == TR::aloadi && node->getSymbolReference() && node->getReferenceCount() == 2 && !node->getSymbolReference()->isUnresolved() && node->getSymbolReference()->getSymbol()->getKind() == TR::Symbol::IsShadow;
    ...
    if (OMR_UNLIKELY(checkFeedingVGDNOP)) {
        ...
        if (!nodeIsNeeded(topNode, node) && (comp->getSymRefTab()->isNonHelper(index, TR::SymbolReferenceTable::vftSymbol)) && topNode->getOpCodeValue() != TR::treetop && // side effects free
            ...
            while (tt && (tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
                ...
                if (cg->getSupportsVirtualGuardNOPing() && (checkNode->isNopableInlineGuard() || checkNode->isHCRGuard() || checkNode->isOSRGuard()) && (checkNode->getOpCodeValue() == TR::ificmpne || checkNode->getOpCodeValue() == TR::iflcmpne || checkNode->getOpCodeValue() == TR::ifacmpne) && checkNode->getFirstChild() == node) {
                    // Try get the virtual guard
                    TR_VirtualGuard* virtualGuard = comp->findVirtualGuardInfo(checkNode);
                    if (!((comp->performVirtualGuardNOPing() || node->isHCRGuard() || node->isOSRGuard()) && comp->isVirtualGuardNOPingRequired(virtualGuard)) && virtualGuard->canBeRemoved()) {
                        break;
                    } else {
...
TR::Register* OMR::Power::TreeEvaluator::astoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    //
    bool isClass = (node->getSymbol()->isClassObject() || (node->getSymbolReference() == comp->getSymRefTab()->findVftSymbolRef()));
...
TR::Register* OMR::Power::TreeEvaluator::bstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((valueChild->getOpCodeValue() == TR::i2b || valueChild->getOpCodeValue() == TR::s2b) && valueChild->getReferenceCount() == 1 && valueChild->getRegister() == NULL) {
...
TR::Register* OMR::Power::TreeEvaluator::sstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((valueChild->getOpCodeValue() == TR::i2s) && valueChild->getReferenceCount() == 1 && valueChild->getRegister() == NULL) {
        valueChild = valueChild->getFirstChild();
    }
    TR::Register* valueReg = cg->evaluate(valueChild);
    bool needSync = (node->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP());
...
bool OMR::Power::TreeEvaluator::inlineVectorCompareBranch(TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    static char* disable = feGetEnv("TR_DisableVectorCompareBranch");
    if (disable)
        return false;

    if (!node->getSecondChild()->getOpCode().isLoadConst() || (node->getSecondChild()->getInt() != 0 && node->getSecondChild()->getInt() != 1))
        return false;
...
TR::Register* OMR::Power::TreeEvaluator::arraytranslateEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (sourceByte) {
        ...
        if (cg->getSupportsArrayTranslateTRTO255() && node->getChild(3)->getOpCodeValue() == TR::iconst && node->getChild(3)->getInt() == 0x0ff00ff00)
...
TR::Register* OMR::Power::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((simpleCopy || !arrayStoreCheckIsNeeded) && (node->isForwardArrayCopy() || alwaysInlineArrayCopy(cg)) && lengthNode->getOpCode().isLoadConst()) {
...
TR::Register* OMR::Power::TreeEvaluator::PrefetchEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::MemoryReference* memRef = indexReg ? new (cg->trHeapMemory()) TR::MemoryReference(baseReg, indexReg, sizeChild->getInt(), cg) : new (cg->trHeapMemory()) TR::MemoryReference(NULL, baseReg, sizeChild->getInt(), cg);
...
TR::Register* OMR::Power::TreeEvaluator::gprRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (OMR_LIKELY(node->getOpCodeValue() == TR::aRegLoad)) {
        if (node->getRegLoadStoreSymbolReference()->getSymbol()->isNotCollected() || node->getRegLoadStoreSymbolReference()->getSymbol()->isInternalPointer()) {
...
TR::Register* OMR::Power::TreeEvaluator::passThroughEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool skipCopy = (node->getOpCodeValue() == TR::a2i && node->getReferenceCount() == 1 && currentTop->getOpCode().isIf() && (currentTop->getFirstChild() == node || currentTop->getSecondChild() == node));
...
TR::Register* OMR::Power::TreeEvaluator::cmpsetEvaluator(
    ...
    TR::CodeGenerator* cg)
{
    TR_ASSERT(node->getOpCodeValue() == TR::lcmpset || node->getOpCodeValue() == TR::icmpset, "only icmpset and lcmpset nodes are supported");
...
TR::Register* OMR::Power::TreeEvaluator::ibyteswapEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!firstChild->getRegister() && firstChild->getOpCode().isMemoryReference() && firstChild->getReferenceCount() == 1) {
