...
static inline bool
alwaysInlineArrayCopy(TR::CodeGenerator* cg)
{
  return false;
}

TR::Instruction*
loadAddressConstantInSnippet(TR::CodeGenerator* cg,
                             TR::Node* node,
                             intptrj_t address,
                             TR::Register* trgReg,
                             TR::Register* tempReg,
                             ...
                             TR::Instruction* cursor)
{
  ...
  TR::Instruction* c = fixedSeqMemAccess(
    cg, node, 0, q, trgReg, trgReg, opCode, sizeof(intptrj_t), cursor, tempReg);
  cg->findOrCreateAddressConstant(
    &address, TR::Address, q[0], q[1], q[2], q[3], node, isUnloadablePicSite);
  if (isTmpRegLocal)
...
TR::Instruction*
loadAddressConstant(TR::CodeGenerator* cg,
                    bool isRelocatable,
                    TR::Node* node,
                    intptrj_t value,
                    TR::Register* trgReg,
...
{
  if (isRelocatable)
    return cg->loadAddressConstantFixed(
      node, value, trgReg, cursor, NULL, typeAddress);
...
TR::Instruction*
loadActualConstant(TR::CodeGenerator* cg,
                   TR::Node* node,
                   intptrj_t value,
                   TR::Register* trgReg,
                   TR::Instruction* cursor,
...
TR::Instruction*
loadConstant(TR::CodeGenerator* cg,
             TR::Node* node,
             int32_t value,
             TR::Register* trgReg,
             TR::Instruction* cursor,
             bool isPicSite)
{
  ...
  if ((LOWER_IMMED <= localVal.getValue()) &&
    ...
    cursor = generateTrg1ImmInstruction(
      cg, TR::InstOpCode::li, node, trgReg, localVal.getValue(), cursor);
  } else {
    ulit = localVal.getHighBits();
    llit = localVal.getLowBits();
    cursor = generateTrg1ImmInstruction(
      cg, TR::InstOpCode::lis, node, trgReg, ulit, cursor);
    if (llit != 0 || isPicSite) {
      if (isPicSite)
        cursor = generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::ori, node, trgReg, trgReg, value, cursor);
      else
        cursor = generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::ori, node, trgReg, trgReg, llit, cursor);
    }
  }
             ...
             bool useTOC)
{
  ...
  if (canUseTOC && useTOC)
    offset = TR_PPCTableOfConstants::lookUp(
      (int8_t*)&value, sizeof(int64_t), true, 0, cg);
  ...
  if (offset != PTOC_FULL_INDEX) {
    ...
    if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
      cursor = generateTrg1Src1ImmInstruction(cg,
                                              TR::InstOpCode::addis,
                                              node,
                                              trgReg,
                                              cg->getTOCBaseRegister(),
                                              cg->hiValue(offset),
                                              cursor);
    ...
    if ((hhval == -1 && (hlval & 0x8000) != 0) ||
      ...
      cursor = generateTrg1ImmInstruction(
        cg, TR::InstOpCode::li, node, trgReg, hlval, cursor);
    } else {
      // lis trgReg, upper 16-bits
      cursor = generateTrg1ImmInstruction(
        cg, TR::InstOpCode::lis, node, trgReg, hhval, cursor);
      // ori trgReg, trgReg, next 16-bits
      if (hlval != 0)
        cursor = generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::ori, node, trgReg, trgReg, hlval, cursor);
    }

    // Shift upper 32 bits into place.
    // shiftli trgReg, trgReg, 32
    cursor = generateTrg1Src1Imm2Instruction(cg,
                                             TR::InstOpCode::rldicr,
                                             node,
                                             trgReg,
                                             trgReg,
                                             32,
    ...
    if (lhval != 0)
      cursor = generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::oris, node, trgReg, trgReg, lhval, cursor);
    ...
    if (llval != 0 || isPicSite) {
      if (isPicSite)
        ...
        cursor = generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::ori, node, trgReg, trgReg, llval, cursor);
    }
...
TR::Instruction*
fixedSeqMemAccess(TR::CodeGenerator* cg,
                  TR::Node* node,
                  intptrj_t addr,
                  ...
                  TR::Register* tempReg)
{
  ...
  if (TR::Compiler->target.is32Bit()) {
    ...
    if (tempReg == NULL) {
      ...
      cursor = generateTrg1Src1Imm2Instruction(cg,
                                               ...
                                               CONSTANT64(0xFFFFFFFF00000000),
                                               cursor);
      nibbles[2] = cursor = generateTrg1Src1ImmInstruction(cg,
                                                           TR::InstOpCode::oris,
                                                           node,
                                                           baseReg,
                                                           baseReg,
  ...
  if (opCode == TR::InstOpCode::lxvdsx) {
    ...
    if (TR::Compiler->target.is64Bit() && tempReg) {
      nibbles[idx] = cursor =
        generateTrg1ImmInstruction(cg,
                                   TR::InstOpCode::li,
                                   node,
                                   tempReg,
                                   ((int32_t)loAddr) << 16 >> 16,
                                   cursor);
      ...
      nibbles[idx] = cursor =
        generateTrg1Src1ImmInstruction(cg,
                                       ...
                                       cursor);
      memRef =
        new (cg->trHeapMemory()) TR::MemoryReference(NULL, baseReg, opSize, cg);
    }
    cursor =
    ...
    if (op.isLoad())
      ...
      nibbles[idx] = cursor =
        generateMemSrc1Instruction(cg, opCode, node, memRef, srcOrTrg, cursor);
  }
...
TR::Register*
OMR::Power::TreeEvaluator::iloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (reverseLoad) {
    ...
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwbrx, node, tempReg, tempMR);
  } else {
    ...
    if (node->getOpCodeValue() == TR::iloadi && cg->getSupportsLM() &&
      ...
      tempMR->forceIndexedForm(node, cg);
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwzmx, node, tempReg, tempMR);
    } else {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwz, node, tempReg, tempMR);
    }
  }
...
static bool
nodeIsNeeded(TR::Node* checkNode, TR::Node* node)
{
  bool result =
...
TR::Register*
OMR::Power::TreeEvaluator::aloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (TR::Compiler->target.is64Bit()) {
    if (TR::Compiler->om.generateCompressedObjectHeaders() &&
      ...
      if (node->getOpCodeValue() == TR::aloadi && cg->getSupportsLM() &&
        ...
        tempMR->forceIndexedForm(node, cg);
        generateTrg1MemInstruction(
          cg, TR::InstOpCode::ldmx, node, tempReg, tempMR);
      } else {
        generateTrg1MemInstruction(
          cg, TR::InstOpCode::ld, node, tempReg, tempMR);
      }
    }
  } else {
...
TR::Register*
OMR::Power::TreeEvaluator::lloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (reverseLoad) // 64-bit only
    {
      tempMR->forceIndexedForm(node, cg);
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::ldbrx, node, trgReg, tempMR);
    } else
  ...
  } else // 32 bit target
  {
    ...
    if (reverseLoad) // 32-bit reverse load. Handles sync
    {
      ...
      if (needSync) {
        ...
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stfd, node, tempMRStore1, doubleReg);
      ...
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwbrx, node, lowReg, tempMRLoad1);
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwbrx, node, highReg, tempMRLoad2);
    ...
    if (needSync) {
          ...
          cg->is64BitProcessor()) // resolved as volatile
      {
        TR::Register* doubleReg = cg->allocateRegister(TR_FPR);
        generateTrg1MemInstruction(
          cg, TR::InstOpCode::lfd, node, doubleReg, tempMR);
        ...
        if (TR::Compiler->target.isSMP()) {
          ...
          generateInstruction(cg,
                              ...
                              node);
          generateTrg1MemInstruction(
            cg, TR::InstOpCode::lwz, node, highReg, tempMRLoad1);
          generateTrg1MemInstruction(
            cg, TR::InstOpCode::lwz, node, lowReg, tempMRLoad2);
          cg->freeSpill(location, 8, 0);
        } else {
          if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
            TR::Register* tmp1 = cg->allocateRegister(TR_FPR);
            generateMvFprGprInstructions(
              cg, node, fpr2gprHost32, false, highReg, lowReg, doubleReg, tmp1);
            cg->stopUsingRegister(tmp1);
          } else
            generateMvFprGprInstructions(
              cg, node, fpr2gprHost32, false, highReg, lowReg, doubleReg);
        }
        cg->stopUsingRegister(doubleReg);
      } else {
        ...
        if (tempMR->getIndexRegister() != NULL)
          ...
          generateTrg1MemInstruction(
            cg, TR::InstOpCode::addi2, node, highReg, tempMR);

        TR::RegisterDependencyConditions* dependencies =
          new (cg->trHeapMemory())
            TR::RegisterDependencyConditions(6, 6, cg->trMemory());
        TR::addDependency(
          dependencies, lowReg, TR::RealRegister::gr4, TR_GPR, cg);
        TR::addDependency(
          dependencies, highReg, TR::RealRegister::gr3, TR_GPR, cg);
        TR::addDependency(
          dependencies, NULL, TR::RealRegister::gr11, TR_GPR, cg);

        if (node->getSymbolReference()->isUnresolved()) {
          if (tempMR->getBaseRegister() != NULL) {
            TR::addDependency(dependencies,
                              tempMR->getBaseRegister(),
        ...
        generateDepImmSymInstruction(cg,
                                     TR::InstOpCode::bl,
                                     node,
      ...
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwz, node, highReg, highMR);
...
TR::Register*
OMR::Power::TreeEvaluator::commonByteLoadEvaluator(TR::Node* node,
                                                   bool signExtend,
                                                   TR::CodeGenerator* cg)
{
  TR::Register* trgReg = cg->allocateRegister();
  ...
  if (signExtend)
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::extsb, node, trgReg, trgReg);
...
// also handles ibload
TR::Register*
OMR::Power::TreeEvaluator::bloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
// also handles isload
TR::Register*
OMR::Power::TreeEvaluator::sloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
#ifdef J9_PROJECT_SPECIFIC
  ...
  if (reverseLoad) {
    tempMR->forceIndexedForm(node, cg);
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lhbrx, node, tempReg, tempMR);
  } else
...
// also handles icload
TR::Register*
OMR::Power::TreeEvaluator::cloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* tempReg = node->setRegister(cg->allocateRegister());
  TR::MemoryReference* tempMR;
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (reverseStore) {
    tempMR->forceIndexedForm(node, cg);
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::stwbrx, node, tempMR, srcReg);
  } else
  ...
  if (needSync) {
    ...
    TR::TreeEvaluator::postSyncConditions(
      node, cg, srcReg, tempMR, TR::InstOpCode::sync, lazyVolatile);
  }
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (needSync) {
    ...
    // PicBuilder overwriting the instruction that normally would have been sync
    TR::TreeEvaluator::postSyncConditions(
      node, cg, valueChild, tempMR, TR::InstOpCode::sync, lazyVolatile);
  }
  tempMR->decNodeReferenceCounts(cg);
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (reverseStore) {
      tempMR->forceIndexedForm(node, cg);
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stdbrx, node, tempMR, valueReg);
    } else
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::std, node, tempMR, valueReg);
    if (needSync) {
      // ordered and lazySet operations will not generate a post-write sync
      // and will not mark unresolved snippets with in sync sequence to avoid
      ...
      // sync
      TR::TreeEvaluator::postSyncConditions(
        node, cg, valueReg, tempMR, TR::InstOpCode::sync);
    }
    tempMR->decNodeReferenceCounts(cg);
  } else // 32 bit target
  {
    ...
    if (needSync) {
          ...
          cg->is64BitProcessor()) // resolved as volatile
      {
        ...
        if (TR::Compiler->target.isSMP()) {
          ...
          if (reverseStore) {
            ...
            if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
              TR::Register* tmp1 = cg->allocateRegister(TR_FPR);
              generateMvFprGprInstructions(cg,
                                           node,
                                           fpr2gprHost32,
                                           false,
                                           highReg,
            ...
            generateMemSrc1Instruction(
              cg, TR::InstOpCode::stwbrx, node, tempMRStore1, lowReg);
            generateMemSrc1Instruction(
              cg, TR::InstOpCode::stwbrx, node, tempMRStore2, highReg);
            cg->stopUsingRegister(highReg);
            cg->stopUsingRegister(lowReg);
          } else
            generateMemSrc1Instruction(
              cg, TR::InstOpCode::stfd, node, tempMR, doubleReg);
          if (!lazyVolatile)
        ...
        if (tempMR->getIndexRegister() != NULL)
          ...
          generateTrg1MemInstruction(
            cg, TR::InstOpCode::addi2, node, addrReg, tempMR);
        ...
        TR::addDependency(dependencies,
                          ...
                          cg);
        TR::addDependency(
          dependencies, addrReg, TR::RealRegister::gr3, TR_GPR, cg);
        TR::addDependency(
          dependencies, NULL, TR::RealRegister::gr11, TR_GPR, cg);
        ...
        dependencies->stopUsingDepRegs(
          cg, valueReg->getLowOrder(), valueReg->getHighOrder());
        cg->machine()->setLinkRegisterKilled(true);
      }
      tempMR->decNodeReferenceCounts(cg);
    } else {
      ...
      if (reverseStore) {
        ...
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stwbrx, node, highMR, valueReg->getLowOrder());
      } else
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stw, node, highMR, valueReg->getHighOrder());
      ...
      if (reverseStore) {
        ...
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stwbrx, node, lowMR, valueReg->getHighOrder());
      } else
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stw, node, lowMR, valueReg->getLowOrder());
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (reverseStore) {
    tempMR->forceIndexedForm(node, cg);
    generateMemSrc1Instruction(
      cg, TR::InstOpCode::sthbrx, node, tempMR, valueReg);
  } else
...
TR::Register*
OMR::Power::TreeEvaluator::vloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::InstOpCode::Mnemonic opcode;
  ...
  if (srcMemRef->hasDelayedOffset()) {
    ...
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::addi2, node, tmpReg, srcMemRef);
...
TR::Register*
OMR::Power::TreeEvaluator::inlineVectorUnaryOp(TR::Node* node,
                                               TR::CodeGenerator* cg,
...
TR::Register*
OMR::Power::TreeEvaluator::inlineVectorBinaryOp(TR::Node* node,
                                                TR::CodeGenerator* cg,
                                                TR::InstOpCode::Mnemonic op)
{
  TR::Node* firstChild = node->getFirstChild();
...
TR::Register*
OMR::Power::TreeEvaluator::inlineVectorTernaryOp(TR::Node* node,
                                                 TR::CodeGenerator* cg,
                                                 TR::InstOpCode::Mnemonic op)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  if (op != TR::InstOpCode::xvmaddadp && op != TR::InstOpCode::xvmsubadp &&
    ...
    generateTrg1Src3Instruction(
      cg, op, node, resReg, firstReg, secondReg, thirdReg);
  } else {
    if (cg->canClobberNodesRegister(thirdChild)) {
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::xxlor, node, resReg, thirdReg, thirdReg);
    }
...
TR::Register*
OMR::Power::TreeEvaluator::viminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::vminsw);
}

TR::Register*
OMR::Power::TreeEvaluator::vimaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::vmaxsw);
}

TR::Register*
OMR::Power::TreeEvaluator::vandEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::Power::TreeEvaluator::vorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::InstOpCode::Mnemonic opCode = TR::InstOpCode::bad;
...
TR::Register*
OMR::Power::TreeEvaluator::vxorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::InstOpCode::Mnemonic opCode = TR::InstOpCode::bad;
...
TR::Register*
OMR::Power::TreeEvaluator::vnotEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* srcReg = NULL;
  ...
  node->setRegister(resReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vnor, node, resReg, srcReg, srcReg);
  cg->decReferenceCount(firstChild);
  return resReg;
}

TR::Register*
OMR::Power::TreeEvaluator::viremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register *lhsReg = NULL, *rhsReg = NULL;
  ...
  for (i = 0; i < 4; i++) {
    ...
    generateTrg1MemInstruction(
      ...
      node,
      srcA2Reg,
      new (cg->trHeapMemory()) TR::MemoryReference(srcV2IdxReg, i * 4, 4, cg));
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::modsw, node, trgAReg, srcA1Reg, srcA2Reg);
    } else {
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::divw, node, trgAReg, srcA1Reg, srcA2Reg);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mullw, node, trgAReg, trgAReg, srcA2Reg);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::subf, node, trgAReg, trgAReg, srcA1Reg);
    }
    generateMemSrc1Instruction(cg,
                               TR::InstOpCode::stw,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(srcV1IdxReg, i * 4, 4, cg),
                               trgAReg);
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateTrg1MemInstruction(cg,
                               ...
                               resReg,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(resReg, elem * 4, 4, cg));
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::mulli, node, offsetReg, idxReg, 4);
  generateTrg1MemInstruction(cg,
                             TR::InstOpCode::lwa,
                             node,
                             resReg,
                                                    ...
                                                    TR::CodeGenerator* cg)
{
  ...
  switch (firstChild->getDataType()) {
    ...
    case TR::VectorInt16:
      TR_ASSERT(false,
                "unsupported vector type %s in getvelemEvaluator.\n",
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    TR_ASSERT(elem >= 0 && elem < elementCount,
              "Element can only be 0 to %u\n",
              elementCount - 1);
    ...
    } else // elementCount == 2
    {
      ...
      if (readElemOne) {
        generateTrg1Src2ImmInstruction(cg,
                                       TR::InstOpCode::xxsldwi,
                                       node,
                                       intermediateResReg,
                                       srcVectorReg,
                                       srcVectorReg,
    ...
    if (firstChild->getDataType() == TR::VectorInt32) {
       ...
       */
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::cmpi4, node, condReg, indexReg, 1);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bgt, node, jumpLabel23, condReg);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bne, node, jumpLabel0, condReg);
      generateTrg1Src2Instruction(cg,
                                  TR::InstOpCode::xxlor,
                                  node,
                                  intermediateResReg,
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::cmpi4, node, condReg, indexReg, 3);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, jumpLabel3, condReg);
      generateTrg1Src1ImmInstruction(cg,
                                     TR::InstOpCode::xxspltw,
                                     node,
                                     intermediateResReg,
      ...
      generateDepLabelInstruction(
        cg, TR::InstOpCode::label, node, jumpLabelDone, deps);
      generateMvFprGprInstructions(
        cg, node, fpr2gprLow, false, resReg, intermediateResReg);
      ...
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bgt, node, jumpLabel23, condReg);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, jumpLabel1, condReg);
      generateTrg1Src2Instruction(cg,
                                  TR::InstOpCode::xxlor,
                                  node,
                                  intermediateResReg,
      ...
      generateDepLabelInstruction(
        cg, TR::InstOpCode::label, node, jumpLabelDone, deps);
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::xscvspdp, node, resReg, intermediateResReg);
    } else {
      /*
       * Conditional statements are used to determine if the indexReg has the
      ...
      if (TR::Compiler->target.is32Bit() &&
        ...
        generateMvFprGprInstructions(
          cg, node, fpr2gprHost64, false, resReg, intermediateResReg);
      } else {
        generateTrg1Src2Instruction(cg,
                                    TR::InstOpCode::xxlor,
                                    node,
                                                    ...
                                                    TR::CodeGenerator* cg)
{
  ...
  TR::Register* offsetReg = cg->allocateRegister();
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::mulli, node, offsetReg, idxReg, 16 / elementCount);
  ...
  if (firstChild->getDataType() == TR::VectorFloat ||
    ...
    if (!TR::Compiler->target.cpu.isLittleEndian()) {
      ...
      generateTrg1MemInstruction(
        ...
        node,
        highResReg,
        new (cg->trHeapMemory()) TR::MemoryReference(resReg, offsetReg, 4, cg));
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, offsetReg, offsetReg, 4);
      generateTrg1MemInstruction(
        cg,
        loadOpCode,
        node,
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    generateMemSrc1Instruction(cg,
                               ...
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, elem * 4, 4, cg),
                               valueReg);
    ...
    generateMemSrc1Instruction(cg,
                               ...
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, offsetReg, 4, cg),
                               valueReg);
    cg->stopUsingRegister(offsetReg);
  }
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (node->getOpCodeValue() == TR::vimergeh) {
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::xxmrghw, node, resReg, lhsReg, rhsReg);
  } else if (node->getOpCodeValue() == TR::vimergel) {
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::xxmrglw, node, resReg, lhsReg, rhsReg);
  } else {
                                           ...
                                           TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorTernaryOp(
    node, cg, TR::InstOpCode::xvmaddadp);
}
...
{
  return TR::TreeEvaluator::inlineVectorTernaryOp(
    node, cg, TR::InstOpCode::xvnmsubadp);
}
...
{
  return TR::TreeEvaluator::inlineVectorTernaryOp(
    node, cg, TR::InstOpCode::xvmsubadp);
}

TR::Register*
OMR::Power::TreeEvaluator::vdselEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorTernaryOp(
    node, cg, TR::InstOpCode::xxsel);
}
...
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::xvcmpgtdp);
}
...
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::xvcmpgedp);
}
...
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::xvcmpeqdp);
}
...
TR::Register*
OMR::Power::TreeEvaluator::vdcmpanyHelper(TR::Node* node,
                                          TR::CodeGenerator* cg,
                                          TR::InstOpCode::Mnemonic op)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  generateTrg1Src1Imm2Instruction(
    cg, TR::InstOpCode::rlwinm, node, resReg, resReg, 27, 0x1);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::subfic, node, resReg, resReg, 1);
...
TR::Register*
OMR::Power::TreeEvaluator::vdcmpallHelper(TR::Node* node,
                                          TR::CodeGenerator* cg,
                                          TR::InstOpCode::Mnemonic op)
{
  TR::Node* firstChild = node->getFirstChild();
  ...
  generateTrg1ImmInstruction(cg, TR::InstOpCode::mfocrf, node, resReg, 0x2);
  generateTrg1Src1Imm2Instruction(
    cg, TR::InstOpCode::rlwinm, node, resReg, resReg, 25, 0x1);
...
OMR::Power::TreeEvaluator::vdcmpalleqEvaluator(TR::Node* node,
                                               TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::vdcmpallHelper(
    node, cg, TR::InstOpCode::xvcmpeqdp_r);
}
...
{
  return TR::TreeEvaluator::vdcmpallHelper(
    node, cg, TR::InstOpCode::xvcmpgedp_r);
}
...
{
  return TR::TreeEvaluator::vdcmpallHelper(
    node, cg, TR::InstOpCode::xvcmpgtdp_r);
}
...
OMR::Power::TreeEvaluator::vdcmpanyeqEvaluator(TR::Node* node,
                                               TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::vdcmpanyHelper(
    node, cg, TR::InstOpCode::xvcmpeqdp_r);
}
...
TR::Register*
OMR::Power::TreeEvaluator::vaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  switch (node->getDataType()) {
    case TR::VectorInt32:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::vadduwm);
    case TR::VectorInt64:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::vaddudm);
    case TR::VectorFloat:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::xvaddsp);
    case TR::VectorDouble:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::xvadddp);
    default:
      TR_ASSERT(
...
TR::Register*
OMR::Power::TreeEvaluator::vsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  switch (node->getDataType()) {
    case TR::VectorInt32:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::vsubuwm);
    case TR::VectorInt64:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::vsubudm);
    case TR::VectorFloat:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::xvsubsp);
    case TR::VectorDouble:
      return TR::TreeEvaluator::inlineVectorBinaryOp(
        node, cg, TR::InstOpCode::xvsubdp);
    default:
      TR_ASSERT(
        false, "unrecognized vector type %s\n", node->getDataType().toString());
      return NULL;
...
TR::Register*
OMR::Power::TreeEvaluator::vnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  switch (node->getDataType()) {
    case TR::VectorInt32:
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  resReg = cg->allocateRegister(TR_VRF);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::xxlxor, node, resReg, srcReg, srcReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vsubuwm, node, resReg, resReg, srcReg);
  node->setRegister(resReg);
...
OMR::Power::TreeEvaluator::vnegFloatHelper(TR::Node* node,
                                           TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorUnaryOp(
    node, cg, TR::InstOpCode::xvnegsp);
}
...
{
  return TR::TreeEvaluator::inlineVectorUnaryOp(
    node, cg, TR::InstOpCode::xvnegdp);
}

TR::Register*
OMR::Power::TreeEvaluator::vmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  switch (node->getDataType()) {
    case TR::VectorInt32:
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vmulouh, node, tempC, lhsReg, rhsReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vrlw, node, resReg, rhsReg, tempA);
  generateTrg1Src3Instruction(
    cg, TR::InstOpCode::vmsumuhm, node, tempB, lhsReg, resReg, tempB);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vslw, node, tempA, tempB, tempA);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vadduhm, node, resReg, tempA, tempC);
...
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::xvmuldp);
}

TR::Register*
OMR::Power::TreeEvaluator::vdivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  switch (node->getDataType()) {
    case TR::VectorInt32:
...
TR::Register*
OMR::Power::TreeEvaluator::vdminEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::xvmindp);
}

TR::Register*
OMR::Power::TreeEvaluator::vdmaxEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::xvmaxdp);
}
...
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::vcmpequw);
}
...
{
  return TR::TreeEvaluator::inlineVectorBinaryOp(
    node, cg, TR::InstOpCode::vcmpgtsw);
}
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  node->setRegister(resReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vcmpgtsw, node, resReg, lhsReg, rhsReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vcmpequw, node, tempReg, lhsReg, rhsReg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::vor, node, resReg, tempReg, resReg);
  cg->stopUsingRegister(tempReg);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
...
{
  return TR::TreeEvaluator::inlineVectorTernaryOp(
    node, cg, TR::InstOpCode::vsel);
}

TR::Register*
OMR::Power::TreeEvaluator::vpermEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorTernaryOp(
    node, cg, TR::InstOpCode::vperm);
}
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  node->setRegister(resReg);
  generateTrg1Src2ImmInstruction(cg,
                                 TR::InstOpCode::xxpermdi,
                                 node,
                                 resReg,
                                 lhsReg,
                                 rhsReg,
                                 node->getOpCodeValue() == TR::vdmergeh ? 0x00
                                                                        : 0x3);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
  return resReg;
...
{
  return TR::TreeEvaluator::inlineVectorUnaryOp(
    node, cg, TR::InstOpCode::xvsqrtdp);
}

TR::Register*
OMR::Power::TreeEvaluator::vdlogEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::Power::TreeEvaluator::vl2vdEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::inlineVectorUnaryOp(
    node, cg, TR::InstOpCode::xvcvsxddp);
}
...
bool
OMR::Power::TreeEvaluator::inlineVectorCompareBranch(TR::Node* node,
                                                     TR::CodeGenerator* cg,
                                                     bool isHint,
                                                     bool likeliness)
{
  ...
  TR::Register* rhsReg = cg->evaluate(secondChild);
  generateTrg1Src2Instruction(
    cg, vcmpOp, vecCmpNode, vecTmpReg, lhsReg, rhsReg); // vecTmpReg is not used
  ...
  if (node->getNumChildren() == 3) {
    ...
    generateDepLabelInstruction(cg,
                                TR::InstOpCode::label,
                                vecCmpNode,
    ...
    generateDepConditionalBranchInstruction(
      cg, branchOp, vecCmpNode, dstLabel, cndReg, conditions);
  }
...
TR::Register*
OMR::Power::TreeEvaluator::inlineVectorCompareAllOrAnyOp(
  TR::Node* node,
  TR::CodeGenerator* cg,
  ...
  TR::InstOpCode::Mnemonic branchOp)
{
  ...
  TR::Register* rhsReg = cg->evaluate(secondChild);
  generateTrg1Src2Instruction(
    cg, vcmpOp, node, vecTmpReg, lhsReg, rhsReg); // vecTmpReg is not used
  ...
  generateDepLabelInstruction(
    cg, TR::InstOpCode::label, node, doneLabel, conditions);
  conditions->stopUsingDepRegs(cg);
...
TR::Register*
OMR::Power::TreeEvaluator::vicmpallHelper(TR::Node* node,
                                          TR::CodeGenerator* cg,
                                          TR::InstOpCode::Mnemonic op,
                                          int nBit)
{
  ...
  generateTrg1ImmInstruction(cg, TR::InstOpCode::mfocrf, node, resReg, 0x2);
  generateTrg1Src1Imm2Instruction(
    cg, TR::InstOpCode::rlwinm, node, resReg, resReg, nBit, 0x1);
...
TR::Register*
OMR::Power::TreeEvaluator::vicmpanyHelper(TR::Node* node,
                                          TR::CodeGenerator* cg,
                                          TR::InstOpCode::Mnemonic op,
                                          int nBit)
{
...
OMR::Power::TreeEvaluator::vicmpalleqEvaluator(TR::Node* node,
                                               TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::vicmpallHelper(
    node, cg, TR::InstOpCode::vcmpeuwr, 25);
}
...
{
  return TR::TreeEvaluator::vicmpallHelper(
    node, cg, TR::InstOpCode::vcmpgswr, 25);
}
TR::Register*
                                               ...
                                               TR::CodeGenerator* cg)
{
  node->swapChildren();
  return TR::TreeEvaluator::vicmpallHelper(
    node, cg, TR::InstOpCode::vcmpgswr, 27);
}
TR::Register*
...
OMR::Power::TreeEvaluator::vicmpanyeqEvaluator(TR::Node* node,
                                               TR::CodeGenerator* cg)
{
  return TR::TreeEvaluator::vicmpanyHelper(
    node, cg, TR::InstOpCode::vcmpeuwr, 27);
}
                ...
                TR::CodeGenerator* cg)
{
  ...
  if (supportsLEArrayCopyInline) {
    ...
    TR::addDependency(
      conditions, fpRegs[0], TR::RealRegister::NoReg, TR_FPR, cg);
    TR::addDependency(
      conditions, fpRegs[1], TR::RealRegister::NoReg, TR_FPR, cg);
    TR::addDependency(
      conditions, fpRegs[2], TR::RealRegister::NoReg, TR_FPR, cg);
    TR::addDependency(
      conditions, fpRegs[3], TR::RealRegister::NoReg, TR_FPR, cg);
  }
  ...
  if (groups != 0) {
    ...
    if (groups != 1) {
      if (groups <= UPPER_IMMED)
        generateTrg1ImmInstruction(
          cg, TR::InstOpCode::li, node, regs[0], groups);
      else
    ...
    if (supportsLEArrayCopyInline) {
      generateTrg1MemInstruction(cg,
                                 ...
                                 fpRegs[3],
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(src, 0, memRefSize, cg));
      generateTrg1MemInstruction(cg,
                                 TR::InstOpCode::lfd,
                                 node,
                                 fpRegs[2],
    ...
    if (groups != 1)
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, src, src, 4 * memRefSize);
    if (supportsLEArrayCopyInline) {
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stfd,
                                 node,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(dst, 0, memRefSize, cg),
                                 fpRegs[3]);
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stfd,
                                 node,
    ...
    if (groups != 1) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi2, node, dst, dst, 4 * memRefSize);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bdnz, node, loopStart, cndReg);
    } else {
      ix = 4 * memRefSize;
    }
  }
  ...
  for (; residual >= memRefSize; residual -= memRefSize, ix += memRefSize) {
    if (supportsLEArrayCopyInline) {
      ...
      generateTrg1MemInstruction(
        ...
        node,
        oneReg,
        new (cg->trHeapMemory()) TR::MemoryReference(src, ix, memRefSize, cg));
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stfd,
                                 node,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(dst, ix, memRefSize, cg),
                                 oneReg);
    } else {
  ...
  if (residual != 0) {
    ...
    if (residual & 2) {
      ...
      generateMemSrc1Instruction(cg,
                                 ...
                                 node,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(dst, ix, 2, cg),
                                 oneReg);
      ix += 2;
    }
    if (residual & 1) {
...
static void
inlineVSXArrayCopy(TR::Node* node,
                   ...
                   TR::CodeGenerator* cg)
{
  ...
  generateMemInstruction(cg,
                         TR::InstOpCode::dcbt,
                         node,
                         new (cg->trHeapMemory())
                           TR::MemoryReference(srcAddrReg, 0, 16, cg));
  generateMemInstruction(cg,
                         TR::InstOpCode::dcbtst,
                         node,
                         new (cg->trHeapMemory())
                           TR::MemoryReference(dstAddrReg, 0, 16, cg));
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 0);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::ble, node, doneLabel, cndRegister);

  if (!node->isForwardArrayCopy()) {
    reverseQuadWordAlignment_vsx = generateLabelSymbol(cg);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::subf, node, tmp4Reg, srcAddrReg, dstAddrReg);
    generateTrg1Src2Instruction(cg,
                                ...
                                lengthReg);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, reverseQuadWordAlignment_vsx, cndRegister);
  }

  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 16);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::blt, node, quadWordAligned_vsx_residue8, cndRegister);

  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 7);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, 1, node, quadWordAlignment_vsx_test8, cndRegister);

  // check 2 aligned
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 1);
  generateConditionalBranchInstruction(
  ...
  generateMemSrc1Instruction(cg,
                             ...
                             tmp2Reg);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, 1);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 1);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -1);

  // check 4 aligned
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAlignment_vsx_test2);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 2);
  generateConditionalBranchInstruction(
  ...
  generateMemSrc1Instruction(cg,
                             ...
                             tmp2Reg);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, 2);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 2);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -2);

  // check 8 aligned
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAlignment_vsx_test4);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 4);
  generateConditionalBranchInstruction(
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, 4);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 4);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -4);

  // check 16 aligned
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAlignment_vsx_test8);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 8);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, quadWordAligned_vsx, cndRegister);
  if (supportsLEArrayCopy) {
    generateTrg1MemInstruction(cg,
                               TR::InstOpCode::lfd,
                               node,
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, 8);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 8);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -8);

  // 16 aligned
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAligned_vsx);
  generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, tmp2Reg, 16);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::srawi, node, tmp4Reg, lengthReg, 6);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, lengthReg, lengthReg, 63);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, tmp4Reg, 0);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, quadWordAligned_vsx_residue32, cndRegister);
  ...
  generateTrg1MemInstruction(
    ...
    node,
    vec1Reg,
    new (cg->trHeapMemory()) TR::MemoryReference(tmp2Reg, srcAddrReg, 16, cg));
  generateMemSrc1Instruction(cg,
                             TR::InstOpCode::stxvw4x,
                             node,
                             new (cg->trHeapMemory())
  ...
  generateMemSrc1Instruction(cg,
                             ...
                             node,
                             new (cg->trHeapMemory())
                               TR::MemoryReference(tmp2Reg, dstAddrReg, 16, cg),
                             vec1Reg);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, 32);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 32);
  generateTrg1MemInstruction(cg,
                             TR::InstOpCode::lxvw4x,
                             node,
                             vec0Reg,
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 32);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bdnz, node, quadWordLoop_vsx, cndRegister);

  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 0);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, doneLabel, cndRegister);

  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAligned_vsx_residue32);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 32);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::blt, node, quadWordAligned_vsx_residue16, cndRegister);
  generateTrg1MemInstruction(cg,
                             TR::InstOpCode::lxvw4x,
                             node,
                             vec0Reg,
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 32);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -32);

  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAligned_vsx_residue16);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 16);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::blt, node, quadWordAligned_vsx_residue8, cndRegister);
  ...
  generateMemSrc1Instruction(cg,
                             ...
                             vec1Reg);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, 16);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 16);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -16);

  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAligned_vsx_residue8);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 8);
  generateConditionalBranchInstruction(
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAligned_vsx_residue4);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 4);
  generateConditionalBranchInstruction(
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAligned_vsx_residue2);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 2);
  generateConditionalBranchInstruction(
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, quadWordAligned_vsx_residue1);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 1);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::blt, node, doneLabel, cndRegister);
  generateTrg1MemInstruction(cg,
                             TR::InstOpCode::lbz,
                             node,
                             tmp2Reg,
  ...
  if (!node->isForwardArrayCopy()) {
    ...
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAlignment_vsx);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::add, node, srcAddrReg, srcAddrReg, lengthReg);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::add, node, dstAddrReg, dstAddrReg, lengthReg);

    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 16);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::blt,
                                         node,
    ...
    // check 4 aligned
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAlignment_vsx_test2);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 2);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::beq,
    ...
    // check 8 aligned
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAlignment_vsx_test4);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 4);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::beq,
    ...
    // check 16 aligned
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAlignment_vsx_test8);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::andi_r, node, tmp4Reg, srcAddrReg, 8);
    generateConditionalBranchInstruction(
    ...
    if (supportsLEArrayCopy) {
      generateTrg1MemInstruction(cg,
                                 ...
                                 fp2Reg,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(srcAddrReg, -8, 8, cg));
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stfd,
                                 node,
                                 new (cg->trHeapMemory())
      ...
      generateTrg1MemInstruction(cg,
                                 ...
                                 tmp2Reg,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(srcAddrReg, -8, 4, cg));
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stw,
                                 node,
                                 new (cg->trHeapMemory())
    ...
    // 16 aligned
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAligned_vsx);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, tmp2Reg, -16);
    ...
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tmp4Reg);
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordLoop_vsx);
    generateTrg1MemInstruction(cg,
                               TR::InstOpCode::lxvw4x,
                               node,
                               vec0Reg,
    ...
    generateMemSrc1Instruction(
      ...
      TR::InstOpCode::stxvw4x,
      node,
      new (cg->trHeapMemory()) TR::MemoryReference(tmp1Reg, dstAddrReg, 16, cg),
      vec1Reg);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi, node, srcAddrReg, srcAddrReg, -32);
    generateTrg1Src1ImmInstruction(
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, -32);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bdnz, node, reverseQuadWordLoop_vsx, cndRegister);
    ...
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAligned_vsx_residue32);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 32);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::blt,
    ...
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAligned_vsx_residue16);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 16);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::blt,
    ...
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAligned_vsx_residue8);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 8);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::blt,
    ...
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAligned_vsx_residue4);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 4);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::blt,
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi, node, lengthReg, lengthReg, -4);
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAligned_vsx_residue2);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 2);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::blt,
    ...
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, reverseQuadWordAligned_vsx_residue1);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::Op_cmpli, node, cndRegister, lengthReg, 1);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, doneLabel, cndRegister);
                                                   ...
                                                   TR::CodeGenerator* cg)
{
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, labelArrayTranslateStart);

  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpi, node, condReg, inputLenReg, 0);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bgt, node, labelNonZeroLengthInput, condReg);

  { // Zero length input, return value in outputLenReg.
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, inputReg, 0);
    generateLabelInstruction(
      cg, TR::InstOpCode::b, node, labelArrayTranslateDone);
  }

  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, labelNonZeroLengthInput);
  ...
  { // Array Translate helper call, greater-than-zero length input.
    ...
    generateDepImmSymInstruction(
      cg, TR::InstOpCode::bl, node, addr, deps, helperSym);
  }

  generateDepLabelInstruction(
    cg, TR::InstOpCode::label, node, labelArrayTranslateDone, deps);
           ...
           TR::CodeGenerator* cg)
{
  ...
  if (valueSize == 1) {
    ...
    if (constFill) {
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::ori, node, tmp1Reg, valReg, 0);
      generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwimi, node, tmp1Reg, tmp1Reg, 8, 0xff00);
    }
  } else {
  ...
  if (valueSize == 1) {
    ...
    if (constFill) {
      ...
      generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwimi, node, tmp1Reg, tmp1Reg, 8, 0xff00);
      generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwimi, node, tmp1Reg, tmp1Reg, 16, 0xffff0000);
    }
  } else if (valueSize == 2) {
    tmp1Reg = cg->allocateRegister();
    retVal = true;
  ...
  if (valueSize == 1) {
    ...
    if (constFill) {
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::ori, node, tmp1Reg, valReg, 0);
      generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rldimi, node, tmp1Reg, tmp1Reg, 32, 0xffff0000);
    }
  } else {
    TR_ASSERT(valueSize == length, "Wrong data size.");
    retVal = false;
...
static inline void
loadArrayCmpSources(TR::Node* node,
                    ...
                    TR::CodeGenerator* cg)
{
  generateTrg1MemInstruction(cg,
                             ...
                             src1,
                             new (cg->trHeapMemory())
                               TR::MemoryReference(src1Addr, 0, loadSize, cg));
  generateTrg1MemInstruction(cg,
                             TR::InstOpCode::Op_load,
                             node,
                             src2,
                             new (cg->trHeapMemory())
                               TR::MemoryReference(src2Addr, 0, loadSize, cg));
  if (loadSize != byteLen) {
    TR_ASSERT(loadSize == 4 || loadSize == 8, "invalid load size\n");
    if (loadSize == 4) {
      generateShiftRightLogicalImmediate(
        cg, node, src1, src1, (loadSize - byteLen) * 8);
      generateShiftRightLogicalImmediate(
        cg, node, src2, src2, (loadSize - byteLen) * 8);
    } else {
      generateShiftRightLogicalImmediateLong(
        cg, node, src1, src1, (loadSize - byteLen) * 8);
      generateShiftRightLogicalImmediateLong(
        cg, node, src2, src2, (loadSize - byteLen) * 8);
    }
...
static TR::Register*
inlineArrayCmp(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::mr, node, byteLenRemainingRegister, byteLenRegister);
  ...
  generateTrg1Src1ImmInstruction(cg,
                                 ...
                                 byteLen);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::blt, node, mid2Label, condReg2);

  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi2, node, src1AddrReg, src1AddrReg, -1 * byteLen);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi2, node, src2AddrReg, src2AddrReg, -1 * byteLen);
  ...
  generateTrg1Src2Instruction(cg,
                              ...
                              src2Reg);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bne, node, residueStartLabel, condReg);

  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bdnz, node, loopStartLabel, condReg);

  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi2, node, src1AddrReg, src1AddrReg, byteLen);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi2, node, src2AddrReg, src2AddrReg, byteLen);

  generateLabelInstruction(cg, TR::InstOpCode::label, node, residueStartLabel);

  generateTrg1Instruction(
    cg, TR::InstOpCode::mfctr, node, byteLenRemainingRegister);
  ...
  generateTrg1Src2Instruction(cg,
                              ...
                              byteLenRemainingRegister,
                              tempReg);
  generateShiftLeftImmediate(cg,
                             node,
                             byteLenRemainingRegister,
                             byteLenRemainingRegister,
                             (byteLen == 8) ? 3 : 2);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bne, node, midLabel, condReg2);
  ...
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, resultLabel, condReg2);

  generateSrc1Instruction(
    cg, TR::InstOpCode::mtctr, node, byteLenRemainingRegister);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi2, node, src1AddrReg, src1AddrReg, -1);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi2, node, src2AddrReg, src2AddrReg, -1);

  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, residueLoopStartLabel);
  ...
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::cmp4, node, condReg, src1Reg, src2Reg);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bne, node, cntrLabel, condReg);

  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bdnz, node, residueLoopStartLabel, condReg);
  ...
  if (node->isArrayCmpLen())
    ...
    generateTrg1Src1ImmInstruction(cg,
                                   ...
                                   0);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bne, node, result2Label, condReg2);
    ...
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, residueEndLabel, condReg);
  ...
  TR::addDependency(
    dependencies, src1AddrReg, TR::RealRegister::NoReg, TR_GPR, cg);
  TR::addDependency(
    dependencies, src2AddrReg, TR::RealRegister::NoReg, TR_GPR, cg);
  TR::addDependency(
    dependencies, byteLenRegister, TR::RealRegister::NoReg, TR_GPR, cg);
  TR::addDependency(dependencies,
                    byteLenRemainingRegister,
                    TR::RealRegister::NoReg,
                    TR_GPR,
  ...
  TR::addDependency(
    dependencies, condReg2, TR::RealRegister::NoReg, TR_CCR, cg);

  generateDepLabelInstruction(
    cg, TR::InstOpCode::label, node, residueEndLabel, dependencies);
...
bool
OMR::Power::TreeEvaluator::stopUsingCopyReg(TR::Node* node,
                                            TR::Register*& reg,
                                            TR::CodeGenerator* cg)
{
  if (node != NULL) {
  ...
  TR::CodeGenerator* cg)
{
  ...
  return generateDepImmSymInstruction(cg,
                                      TR::InstOpCode::bl,
                                      node,
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (!cg->canClobberNodesRegister(lengthNode)) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::mr, lengthNode, lenCopyReg, lengthReg);
  ...
  if (!cg->canClobberNodesRegister(valueNode)) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::mr, valueNode, valCopyReg, valueReg);
    valueReg = valCopyReg;
  ...
  TR::addDependency(
    conditions, dstAddrReg, TR::RealRegister::NoReg, TR_GPR, cg);
  ...
  generateTrg1Src1Imm2Instruction(
    cg, TR::InstOpCode::rlwimi, node, valueReg, valueReg, 8, 0xff00);
  generateTrg1Src1Imm2Instruction(
    cg, TR::InstOpCode::rlwimi, node, valueReg, valueReg, 16, 0xffff0000);
  generateTrg1Src1Imm2Instruction(cg,
                                  TR::InstOpCode::rldimi,
                                  node,
                                  valueReg,
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::Op_cmpli, node, cndReg, lengthReg, 32);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::blt, node, residualLabel, cndReg);

  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::srawi, node, tempReg, lengthReg, 5);
  generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
  generateLabelInstruction(cg, TR::InstOpCode::label, node, loopStartLabel);
  generateMemSrc1Instruction(cg,
                             TR::InstOpCode::std,
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi, node, dstAddrReg, dstAddrReg, 32);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::bdnz, node, loopStartLabel, cndReg);

  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, residualLabel); // check 16 aligned
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tempReg, lengthReg, 16);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, label8aligned, cndReg);
  generateMemSrc1Instruction(cg,
                             TR::InstOpCode::std,
                             node,
                             new (cg->trHeapMemory())
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, label8aligned); // check 8 aligned
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tempReg, lengthReg, 8);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, label4aligned, cndReg);
  generateMemSrc1Instruction(cg,
                             TR::InstOpCode::std,
                             node,
                             new (cg->trHeapMemory())
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, label4aligned); // check 4 aligned
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tempReg, lengthReg, 4);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, label2aligned, cndReg);
  generateMemSrc1Instruction(cg,
                             TR::InstOpCode::stw,
                             node,
                             new (cg->trHeapMemory())
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, label2aligned); // check 2 aligned
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tempReg, lengthReg, 2);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, label1aligned, cndReg);
  generateMemSrc1Instruction(cg,
                             TR::InstOpCode::sth,
                             node,
                             new (cg->trHeapMemory())
  ...
  generateLabelInstruction(
    cg, TR::InstOpCode::label, node, label1aligned); // check 1 aligned
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::andi_r, node, tempReg, lengthReg, 1);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, doneLabel, cndReg);
  generateMemSrc1Instruction(cg,
                             TR::InstOpCode::stb,
                             node,
                             new (cg->trHeapMemory())
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if ((simpleCopy || !arrayStoreCheckIsNeeded) &&
    ...
    if (len >= 0 && (!simpleCopy || len < MAX_PPC_ARRAYCOPY_INLINE)) {
      ...
      if (!simpleCopy) {
#ifdef J9_PROJECT_SPECIFIC
        TR::TreeEvaluator::genWrtbarForArrayCopy(
          node, srcObjReg, dstObjReg, cg);
#endif
...
static TR::Register*
inlineFixedTrg1Src1(TR::Node* node,
                    TR::InstOpCode::Mnemonic op,
                    TR::CodeGenerator* cg)
{
...
static TR::Register*
inlineLongNumberOfLeadingZeros(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(node->getNumChildren() == 1,
  ...
  generateTrg1Src1Instruction(cg,
                              TR::InstOpCode::cntlzw,
                              node,
                              targetRegister,
                              srcRegister->getHighOrder());
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::cntlzw, node, tempRegister, srcRegister->getLowOrder());
  generateTrg1Src1Imm2Instruction(
    cg, TR::InstOpCode::rlwinm, node, maskRegister, targetRegister, 27, 0x1);
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::neg, node, maskRegister, maskRegister);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::AND, node, tempRegister, tempRegister, maskRegister);
  generateTrg1Src2Instruction(cg,
                              TR::InstOpCode::add,
                              node,
                              targetRegister,
                            ...
                            TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::addi2, node, targetRegister, srcRegister, -1);
  generateTrg1Src2Instruction(cg,
                              TR::InstOpCode::andc,
                              node,
                              targetRegister,
...
static TR::Register*
inlineLongBitCount(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(node->getNumChildren() == 1,
...
static TR::Register*
inlineLongNumberOfTrailingZeros(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(node->getNumChildren() == 1,
  ...
  generateTrg1Src2Instruction(cg,
                              ...
                              srcRegister->getHighOrder());
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::cntlzw, node, targetRegister, targetRegister);
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::cntlzw, node, tempRegister, tempRegister);
  generateTrg1Src1Imm2Instruction(
    cg, TR::InstOpCode::rlwinm, node, maskRegister, targetRegister, 27, 0x1);
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::neg, node, maskRegister, maskRegister);
  ...
  generateTrg1Src2Instruction(cg,
                              ...
                              tempRegister);
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::subfic, node, targetRegister, targetRegister, 64);
...
static TR::Register*
inlineIntegerHighestOneBit(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(node->getNumChildren() == 1,
  ...
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::cntlzw, node, tempRegister, srcRegister);
  generateTrg1ImmInstruction(
    cg, TR::InstOpCode::lis, node, targetRegister, 0x8000);
  generateTrg1Src2Instruction(cg,
                              TR::InstOpCode::srw,
                              node,
                              targetRegister,
...
static TR::Register*
inlineLongHighestOneBit(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_ASSERT(node->getNumChildren() == 1,
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::cntlzd, node, tempRegister, srcRegister);
    generateTrg1ImmInstruction(
      cg, TR::InstOpCode::lis, node, targetRegister, 0x8000);
    generateTrg1Src1Imm2Instruction(cg,
                                    TR::InstOpCode::rldicr,
    ...
    TR::RegisterPair* targetRegister =
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::cmpi4, node, condReg, srcRegister->getHighOrder(), 0);
    generateTrg1Src1Instruction(cg,
                                TR::InstOpCode::cntlzw,
                                node,
                                tempRegister,
                                srcRegister->getHighOrder());
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, jumpLabel, condReg);
    generateTrg1ImmInstruction(
      cg, TR::InstOpCode::lis, node, targetRegister->getHighOrder(), 0x8000);
    generateTrg1ImmInstruction(
      cg, TR::InstOpCode::li, node, targetRegister->getLowOrder(), 0);
    generateTrg1Src2Instruction(cg,
                                TR::InstOpCode::srw,
                                node,
    ...
    generateTrg1Src1Instruction(cg,
                                ...
                                srcRegister->getLowOrder());
    generateTrg1ImmInstruction(
      cg, TR::InstOpCode::lis, node, targetRegister->getLowOrder(), 0x8000);
    generateTrg1ImmInstruction(
      cg, TR::InstOpCode::li, node, targetRegister->getHighOrder(), 0);
    generateTrg1Src2Instruction(cg,
                                TR::InstOpCode::srw,
                                node,
...
static TR::Register*
inlineSimpleAtomicUpdate(TR::Node* node,
                         bool isAddOp,
                         bool isLong,
                         ...
                         TR::CodeGenerator* cg)
{
  ...
  generateTrg1MemInstruction(
    cg,
    isLong ? TR::InstOpCode::ldarx : TR::InstOpCode::lwarx,
    node,
    oldValueReg,
  ...
  if (isAddOp) {
    ...
    if (isDeltaImmediateShifted)
      ...
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, newValueReg, oldValueReg, delta);
    else
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::add, node, newValueReg, oldValueReg, deltaReg);
  } else {
  ...
  generateMemSrc1Instruction(
    cg,
    isLong ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r,
    node,
    new (cg->trHeapMemory()) TR::MemoryReference(0, valueAddrReg, len, cg),
    newValueReg);
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCgp) {
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::bne,
                                         PPCOpProp_BranchUnlikely,
                                         node,
                                         loopLabel,
                                         cndReg);
  } else {
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bne, node, loopLabel, cndReg);
  }
  ...
  TR::addDependency(
    conditions, valueAddrReg, TR::RealRegister::NoReg, TR_GPR, cg);
  TR::addDependency(
    conditions, oldValueReg, TR::RealRegister::NoReg, TR_GPR, cg);
  conditions->getPreConditions()->getRegisterDependency(1)->setExcludeGPR0();
  conditions->getPostConditions()->getRegisterDependency(1)->setExcludeGPR0();
  TR::addDependency(conditions, cndReg, TR::RealRegister::cr0, TR_CCR, cg);
  numDeps = numDeps - 3;
  ...
  if (newValueReg) {
    TR::addDependency(
      conditions, newValueReg, TR::RealRegister::NoReg, TR_GPR, cg);
    numDeps--;
  }

  if (deltaReg) {
    TR::addDependency(
      conditions, deltaReg, TR::RealRegister::NoReg, TR_GPR, cg);
    numDeps--;
  }
...
bool
OMR::Power::CodeGenerator::inlineDirectCall(TR::Node* node,
                                            TR::Register*& resultReg)
{
...
TR::Register*
OMR::Power::TreeEvaluator::performCall(TR::Node* node,
                                       bool isIndirect,
                                       TR::CodeGenerator* cg)
{
  TR::SymbolReference* symRef = node->getSymbolReference();
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  if (mref->getUnresolvedSnippet() != NULL) {
    ...
    if (mref->useIndexedForm()) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::add, node, resultReg, mref);
    } else {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::addi2, node, resultReg, mref);
    }
  } else {
    if (mref->useIndexedForm()) {
      ...
      if (mref->hasDelayedOffset() || offset != 0 || comp->getOption(TR_AOT)) {
        ...
        if (resultReg == cg->getMethodMetaDataRegister()) {
          ...
          generateTrg1Src1Instruction(
            cg, TR::InstOpCode::mr, node, resultReg, mref->getBaseRegister());
        }
      }
    }
  }
                                                ...
                                                TR::CodeGenerator* cg)
{
  ...
  if (node->getOpCodeValue() != TR::lRegLoad &&
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::extsw, node, globalReg, globalReg);
                                                ...
                                                TR::CodeGenerator* cg)
{
  ...
  if ((child->getReferenceCount() > 1 &&
    ...
    TR::InstOpCode::Mnemonic move_opcode =
      (srcReg->getKind() == TR_GPR) ? TR::InstOpCode::mr : TR::InstOpCode::fmr;
    ...
    if (srcReg->getRegisterPair()) {
      ...
      generateTrg1Src1Instruction(
        cg, move_opcode, node, copyReg, srcReg->getHighOrder());
      generateTrg1Src1Instruction(
        cg, move_opcode, node, copyRegLow, srcReg->getLowOrder());
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (node->getLabel() != NULL) {
    ...
    if (deps == NULL)
      ...
      node->getLabel()->setInstruction(
        labelInst = generateDepLabelInstruction(
          cg, TR::InstOpCode::label, node, label, deps));
  }
...
TR::Register*
OMR::Power::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Block* block = node->getBlock();
  TR::Compilation* comp = cg->comp();
...
TR::Register*
OMR::Power::TreeEvaluator::NOPEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return NULL;
}
...
#else
  TR_ASSERT(
    false, "Unimplemented evaluator for opcode %d\n", node->getOpCodeValue());
#endif
  return NULL;
}

void
OMR::Power::TreeEvaluator::postSyncConditions(TR::Node* node,
                                              TR::CodeGenerator* cg,
                                              TR::Register* dataReg,
                                              TR::MemoryReference* memRef,
                                              ...
                                              bool lazyVolatile)
{
  ...
  if (symRef->isUnresolved() && memRef->getUnresolvedSnippet() != NULL) {
    ...
    if (baseReg != NULL) {
      TR::addDependency(
        conditions, baseReg, TR::RealRegister::NoReg, TR_GPR, cg);
    ...
    TR::addDependency(
      conditions, dataReg, TR::RealRegister::NoReg, dataReg->getKind(), cg);
    if (memRef->getIndexRegister() != NULL)
      TR::addDependency(conditions,
    ...
    generateDepLabelInstruction(
      cg, TR::InstOpCode::label, node, generateLabelSymbol(cg), conditions);
    cg->stopUsingRegister(tempReg);
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  //
  generateDepLabelInstruction(
    cg, TR::InstOpCode::label, node, startLabel, deps);
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
    ...
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bne, node, endLabel, condReg);
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
    ...
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bne, node, endLabel, cr0);
...
bool
TR_PPCComputeCC::setCarryBorrow(TR::Node* flagNode,
                                bool invertValue,
                                TR::Register** flagReg,
                                TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::Power::TreeEvaluator::longBitCount(TR::Node* node, TR::CodeGenerator* cg)
{
  if (TR::Compiler->target.is64Bit())
...
void
OMR::Power::TreeEvaluator::preserveTOCRegister(
  TR::Node* node,
  TR::CodeGenerator* cg,
  TR::RegisterDependencyConditions* dependencies)
{
  ...
  cursor = generateMemSrc1Instruction(
    ...
    node,
    new (cg->trHeapMemory())
      TR::MemoryReference(grSysStackReg,
                          callerSaveTOCOffset,
                          TR::Compiler->om.sizeofReferenceAddress(),
                          cg),
    grTOCReg,
...
void
OMR::Power::TreeEvaluator::restoreTOCRegister(
  TR::Node* node,
  TR::CodeGenerator* cg,
  TR::RegisterDependencyConditions* dependencies)
{
...
TR::Register*
OMR::Power::TreeEvaluator::retrieveTOCRegister(
  TR::Node* node,
  TR::CodeGenerator* cg,
  TR::RegisterDependencyConditions* dependencies)
{
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (!firstChild->getRegister() &&
    ...
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lwbrx, node, tgtRegister, tempMR);
    tempMR->decNodeReferenceCounts(cg);
  } else {
    TR::Register* srcRegister = cg->evaluate(firstChild);
    ...
    generateTrg1Src1Imm2Instruction(cg,
                                    ...
                                    0x0000ff0000);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
    generateTrg1Src1Imm2Instruction(cg,
                                    TR::InstOpCode::rlwinm,
                                    node,
                                    tmp1Register,
