...
struct PPCLinkageProperties {
  ...
  TR::RealRegister::RegNum
      _computedCallTargetRegister; // for icallVMprJavaSendPatchupVirtual
  TR::RealRegister::RegNum
  ...
  uint32_t
  getCallerAllocatesBackingStore(TR::RealRegister::RegNum regNum) const {
  ...
  uint32_t getNumberOfDependencyGPRegisters() const {
...
public:
  ...
  virtual TR::MemoryReference *
  getOutgoingArgumentMemRef(int32_t argSize, TR::Register *argReg,
                            TR::InstOpCode::Mnemonic opCode,
                            TR::PPCMemoryArgument &memArg, uint32_t len);
  virtual TR::Instruction *saveArguments(TR::Instruction *cursor,
                                         bool fsd = false,
                                         bool saveOnly = false);
  virtual TR::Instruction *saveArguments(TR::Instruction *cursor, bool fsd,
                                         bool saveOnly,
