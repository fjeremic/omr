...
static void switchDispatch(TR::Node *node, bool fromTableEval,
                           TR::CodeGenerator *cg);
static bool isGlDepsUnBalanced(TR::Node *node, TR::CodeGenerator *cg);
static void lookupScheme1(TR::Node *node, bool unbalanced, bool fromTableEval,
                          TR::CodeGenerator *cg);
static void lookupScheme2(TR::Node *node, bool unbalanced, bool fromTableEval,
                          TR::CodeGenerator *cg);
static void lookupScheme3(TR::Node *node, bool unbalanced,
                          TR::CodeGenerator *cg);
static void lookupScheme4(TR::Node *node, TR::CodeGenerator *cg);

TR::Register *OMR::Power::TreeEvaluator::ifacmpltEvaluator(
...
TR::Register *OMR::Power::TreeEvaluator::ifacmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::ifacmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::ifacmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::acmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::acmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::acmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::acmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::compareIntsForOrder(
    TR::InstOpCode::Mnemonic branchOp, TR::LabelSymbol *dstLabel,
    TR::Node *node, TR::CodeGenerator *cg, bool isSigned, bool isHint,
    bool likeliness) {
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (isSigned) {
      ...
      if (value >= LOWER_IMMED && value <= UPPER_IMMED) {
        ...
        generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, condReg,
                                    src1Reg, src2Reg);
      }
    } else {
      ...
      bool newReg = false;
      if (node->getOpCodeValue() == TR::ifbucmplt ||
          node->getOpCodeValue() == TR::ifbucmple ||
          node->getOpCodeValue() == TR::ifbucmpgt ||
          node->getOpCodeValue() == TR::ifbucmpge) {
        tReg = cg->allocateRegister();
        newReg = true;
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, tReg,
                                        src1Reg, 0, 0xff);
        value &= (uint64_t)0xff;
      } else if (node->getOpCodeValue() == TR::ifsucmplt ||
                 node->getOpCodeValue() == TR::ifsucmple ||
                 node->getOpCodeValue() == TR::ifsucmpgt ||
                 node->getOpCodeValue() == TR::ifsucmpge) {
        tReg = cg->allocateRegister();
        newReg = true;
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, tReg,
                                        src1Reg, 0, 0xffff);
      ...
      if (value <= (uint64_t)0xFFFF)
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node,
                                       condReg, tReg, value);
      else {
        ...
        if (node->getOpCodeValue() == TR::ifbucmplt ||
          ...
          sNewReg = true;
          generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node,
                                          secondReg, secondReg, 0, 0xffff);
        } else
          secondReg = cg->evaluate(secondChild);

        generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, condReg,
                                    tReg, secondReg);

        if (sNewReg) cg->stopUsingRegister(secondReg);
      }

      if (newReg) cg->stopUsingRegister(tReg);
    }
  } else {
    TR::Register *tReg = NULL;
    bool newReg = false;
    ...
    if (node->getOpCodeValue() == TR::ifbucmplt ||
      ...
      generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, src2Reg,
                                      src2Reg, 0, 0xff);
    } else if (node->getOpCodeValue() == TR::ifsucmplt ||
               node->getOpCodeValue() == TR::ifsucmple ||
               node->getOpCodeValue() == TR::ifsucmpgt ||
    ...
    if (isSigned)
      generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, condReg, tReg,
                                  src2Reg);
    else
      generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, condReg,
                                  tReg, src2Reg);
  ...
  if (node->getOpCode().isIf() && node->getNumChildren() == 3) {
    ...
    if (isHint && TR::Compiler->target.cpu.id() >= TR_PPCgp)
      generateDepConditionalBranchInstruction(
          cg, branchOp, likeliness, node, dstLabel, condReg,
          generateRegisterDependencyConditions(cg, thirdChild, 0));
    else
      generateDepConditionalBranchInstruction(
          cg, branchOp, node, dstLabel, condReg,
          generateRegisterDependencyConditions(cg, thirdChild, 0));
    cg->decReferenceCount(thirdChild);
  } else {
    if (isHint && TR::Compiler->target.cpu.id() >= TR_PPCgp)
      generateConditionalBranchInstruction(cg, branchOp, likeliness, node,
                                           dstLabel, condReg);
    else
      generateConditionalBranchInstruction(cg, branchOp, node, dstLabel,
                                           condReg);
  }
...
TR::Register *OMR::Power::TreeEvaluator::compareIntsForOrder(
    TR::InstOpCode::Mnemonic branchOp, TR::Node *node, TR::CodeGenerator *cg,
    bool isSigned) {
  return TR::TreeEvaluator::compareIntsForOrder(
      branchOp, node->getBranchDestination()->getNode()->getLabel(), node, cg,
      isSigned, false, false);
}

static void fixDepsForLongCompare(TR::RegisterDependencyConditions *deps,
                                  TR::Register *src1High, TR::Register *src1Low,
                                  TR::Register *src2High,
                                  TR::Register *src2Low) {
  if (deps == NULL) return;
...
static TR::Register *compareLongsForOrderWithAnalyser(
    TR::InstOpCode::Mnemonic branchOp,
    TR::InstOpCode::Mnemonic reversedBranchOp, TR::Register *condReg,
    TR::LabelSymbol *destinationLabel, TR::RegisterDependencyConditions *deps,
    TR::Node *node, TR::CodeGenerator *cg, bool isHint, bool likeliness) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Node *thirdChild;
  ...
  if (firstHighZero && secondHighZero) {
    ...
    if (deps) {
      if (isHint && TR::Compiler->target.cpu.id() >= TR_PPCgp)
        ...
        generateDepConditionalBranchInstruction(
            cg, branchOp, node, destinationLabel, condReg, deps);
    } else {
      if (isHint && TR::Compiler->target.cpu.id() >= TR_PPCgp)
        generateConditionalBranchInstruction(cg, branchOp, likeliness, node,
                                             destinationLabel, condReg);
      else
        generateConditionalBranchInstruction(cg, branchOp, node,
                                             destinationLabel, condReg);
    }
  } else {
    if (deps == NULL)
    ...
    TR::PPCControlFlowInstruction *cfop =
        (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
            cg, TR::InstOpCode::iflong, node, deps);
...
TR::Register *OMR::Power::TreeEvaluator::compareLongsForOrder(
    TR::InstOpCode::Mnemonic branchOp,
    TR::InstOpCode::Mnemonic reversedBranchOp, TR::LabelSymbol *dstLabel,
    TR::Node *node, TR::CodeGenerator *cg, bool isSigned, bool isHint,
    bool likeliness) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *src1Reg;
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      if (isSigned)
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli8, node,
                                       condReg, src1Reg, value);
    } else {
      if (isSigned)
        generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp8, node, condReg,
                                    src1Reg, cg->evaluate(secondChild));
      else
        generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl8, node, condReg,
                                    src1Reg, cg->evaluate(secondChild));
    }
    if (node->getOpCode().isIf() && node->getNumChildren() == 3) {
      TR::Node *thirdChild = node->getChild(2);
      TR_ASSERT(thirdChild->getOpCodeValue() == TR::GlRegDeps,
                "The third child of a compare is assumed to be a "
                "TR::GlRegDeps, but wasn't");
      cg->evaluate(thirdChild);
      if (isHint && TR::Compiler->target.cpu.id() >= TR_PPCgp)
        generateDepConditionalBranchInstruction(
            cg, branchOp, likeliness, node, dstLabel, condReg,
  ...
  } else  // 32 bit target
  {
    ...
    if ((firstChild->isHighWordZero() || secondChild->isHighWordZero()) &&
        !(secondChild->getOpCode().isLoadConst() &&
          secondChild->getRegister() == NULL)) {
      return compareLongsForOrderWithAnalyser(branchOp, reversedBranchOp,
                                              condReg, dstLabel, deps, node, cg,
                                              isHint, likeliness);
    }
    ...
    bool useImmed = secondChild->getOpCode().isLoadConst() &&
                    secondChild->getRegister() == NULL && value >= 0 &&
                    value <= 0xffff;

    if (!useImmed) src2Reg = cg->evaluate(secondChild);
    ...
    fixDepsForLongCompare(deps, src1Reg->getHighOrder(), src1Reg->getLowOrder(),
                          useImmed ? NULL : src2Reg->getHighOrder(),
                          useImmed ? NULL : src2Reg->getLowOrder());
    TR::PPCControlFlowInstruction *cfop =
        (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
            cg, TR::InstOpCode::iflong, node, deps);
    cfop->addTargetRegister(condReg);
...
TR::Register *OMR::Power::TreeEvaluator::compareLongsForOrder(
    TR::InstOpCode::Mnemonic branchOp,
    TR::InstOpCode::Mnemonic reversedBranchOp, TR::Node *node,
    TR::CodeGenerator *cg, bool isSigned) {
  return TR::TreeEvaluator::compareLongsForOrder(
      branchOp, reversedBranchOp,
      node->getBranchDestination()->getNode()->getLabel(), node, cg, isSigned,
      false, false);
}

TR::Register *compareLongAndSetOrderedBoolean(
    TR::InstOpCode::Mnemonic compareOp, TR::InstOpCode::Mnemonic branchOp,
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  TR::Node *secondChild = node->getSecondChild();
  ...
  TR::PPCControlFlowInstruction *cfop =
      (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
          cg, TR::InstOpCode::setblong, node);
  cfop->addTargetRegister(condReg);
...
TR::Register *OMR::Power::TreeEvaluator::gotoEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ireturnEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR::RegisterDependencyConditions *dependencies = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(1, 0, cg->trMemory());
...
TR::Register *OMR::Power::TreeEvaluator::lreturnEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *returnRegister = cg->evaluate(node->getFirstChild());
  const TR::PPCLinkageProperties &linkageProperties = cg->getProperties();
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    dependencies = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(2, 0, cg->trMemory());
...
TR::Register *OMR::Power::TreeEvaluator::returnEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  generateAdminInstruction(cg, TR::InstOpCode::ret, node);
...
static TR::InstOpCode::Mnemonic cmp2branch(TR::ILOpCodes op,
                                           TR::CodeGenerator *cg) {
  switch (op) {
    case TR::icmpeq:
    case TR::acmpeq:
...
static TR::InstOpCode::Mnemonic cmp2cmp(TR::ILOpCodes op,
                                        TR::CodeGenerator *cg) {
  switch (op) {
    case TR::icmpeq:
    case TR::icmpne:
    ...
    case TR::lcmpgt:
    case TR::lcmple:
      return TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmp8
                                            : TR::InstOpCode::cmp4;
    case TR::lucmplt:
    case TR::lucmpge:
    case TR::lucmpgt:
    ...
    case TR::acmpgt:
    case TR::acmple:
      return TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmpl8
                                            : TR::InstOpCode::cmpl4;
    default:
      TR_ASSERT(false, "assertion failure");
  }
...
static TR::InstOpCode::Mnemonic cmp2cmpi(TR::ILOpCodes op,
                                         TR::CodeGenerator *cg) {
  switch (op) {
    case TR::icmpeq:
    case TR::icmpne:
    ...
    case TR::lcmpgt:
    case TR::lcmple:
      return TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmpi8
                                            : TR::InstOpCode::cmpi4;
    case TR::lucmplt:
    case TR::lucmpge:
    case TR::lucmpgt:
    ...
    case TR::acmpgt:
    case TR::acmple:
      return TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmpli8
                                            : TR::InstOpCode::cmpli4;
    default:
      TR_ASSERT(false, "assertion failure");
  }
  return TR::InstOpCode::bad;
...
TR::Register *OMR::Power::TreeEvaluator::iternaryEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::DataType type = node->getType();
  bool two_reg =
      (TR::Compiler->target.is32Bit()) && type.getDataType() == TR::Int64;
  TR::Register *resultReg =
      two_reg ? cg->allocateRegisterPair(cg->allocateRegister(),
                                         cg->allocateRegister())
  ...
  TR::InstOpCode::Mnemonic move_opcode = (type.isIntegral() || type.isAddress())
                                             ? TR::InstOpCode::mr
                                             : TR::InstOpCode::fmr;
  TR::Node *firstChild = node->getFirstChild();
  ...
  if (firstChild->getOpCode().isBooleanCompare() &&
      ...
      !(firstChild->getFirstChild()->getType().isInt64() &&
    ...
    TR::InstOpCode::Mnemonic branch_opcode =
        cmp2branch(firstChild->getOpCodeValue(), cg);
    ...
    if (compare_type.isIntegral() || compare_type.isAddress()) {
      ...
      // We do not have an immediate within range at
      // firstChild->getSecondChild(), so evaluate it.
      ...
      if (useImmediateCompare) {
        generateTrg1Src1ImmInstruction(
            cg, cmp2cmpi(firstChild->getOpCodeValue(), cg), node, ccr, cmp1Reg,
            value);
      } else {
        generateTrg1Src2Instruction(cg,
                                    cmp2cmp(firstChild->getOpCodeValue(), cg),
                                    node, ccr, cmp1Reg, cmp2Reg);
      }

      if (two_reg) {
        generateTrg1Src1Instruction(cg, move_opcode, node,
                                    resultReg->getHighOrder(),
                                    trueReg->getHighOrder());
        generateTrg1Src1Instruction(cg, move_opcode, node,
      ...
      generateConditionalBranchInstruction(cg, branch_opcode, node, doneLabel,
                                           ccr);
      if (two_reg) {
        generateTrg1Src1Instruction(cg, move_opcode, node,
                                    resultReg->getHighOrder(),
                                    falseReg->getHighOrder());
        generateTrg1Src1Instruction(cg, move_opcode, node,
                                    resultReg->getLowOrder(),
                                    falseReg->getLowOrder());
      } else {
        generateTrg1Src1Instruction(cg, move_opcode, node, resultReg, falseReg);
      }
    } else if (compare_type.isFloatingPoint()) {
      cmp2Reg = cg->evaluate(firstChild->getSecondChild());
      generateTrg1Src2Instruction(cg, TR::InstOpCode::fcmpu, node, ccr, cmp1Reg,
                                  cmp2Reg);
      ...
      generateConditionalBranchInstruction(cg, branch_opcode, node, doneLabel,
                                           ccr);
      TR::InstOpCode::Mnemonic branch_opcode2 =
          cmp2bun(firstChild->getOpCodeValue());
      if (branch_opcode2 != TR::InstOpCode::bad)
        generateConditionalBranchInstruction(cg, branch_opcode2, node,
                                             doneLabel, ccr);
    ...
    TR::RegisterDependencyConditions *dep = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(0, two_reg ? 8 : 5, cg->trMemory());
    if (two_reg) {
    ...
    TR::PPCControlFlowInstruction *i =
        (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
            cg, TR::InstOpCode::iternary, node, NULL, 0, two_reg,
            useRegPairForCond);
    ...
    if (useRegPairForCond) i->addSourceRegister(condReg->getHighOrder());
...
TR::Register *OMR::Power::TreeEvaluator::compareIntsForEquality(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::InstOpCode::Mnemonic branchOp = node->getOpCode().isCompareTrueIfEqual()
                                          ? TR::InstOpCode::beq
                                          : TR::InstOpCode::bne;
...
TR::Register *OMR::Power::TreeEvaluator::compareIntsForEquality(
    TR::InstOpCode::Mnemonic branchOp, TR::LabelSymbol *dstLabel,
    TR::Node *node, TR::CodeGenerator *cg, bool isHint, bool likeliness) {
  if (virtualGuardHelper(node, cg)) return NULL;
  TR::Compilation *comp = cg->comp();
  ...
  if (firstChild->getOpCodeValue() == TR::b2i &&
      ...
      // That's why we check that both b2i nodes have loads (lbz) as children to
      // guarantee high bits are clear
      firstChild->getFirstChild()->getOpCode().isLoad() &&
      firstChild->getFirstChild()->getOpCodeValue() ==
          secondChild->getFirstChild()->getOpCodeValue()) {
    /*
     ...
     *     load
     *
     * Does the compare against the children of the b2i nodes, since eq/ne is
     * not affected by sign extension Skips evaluating b2i if it is unused (i.e.
     * refcount == 1, which is usually the case)
     */
    ...
    // 3) had we evaluated the child, the grandchild's refcount would have been
    // decremented, so we have to decrement it instead If a child is in a
    // register, regardless of whether or not we evaluated it, we don't have to
  ...
  int64_t value = secondChild->getOpCode().isLoadConst()
  ...
  if (isSigned) {
    if (secondChild->getOpCode().isLoadConst() &&
        secondChild->getRegister() == NULL && value >= LOWER_IMMED &&
        value <= UPPER_IMMED) {
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg,
                                     src1Reg, value);
    } else {
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, condReg,
                                     tReg, value);
...
TR::Register *OMR::Power::TreeEvaluator::ificmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ificmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild(),
...
TR::Register *OMR::Power::TreeEvaluator::compareLongsForEquality(
  ...
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  static bool disableCompareToOpt =
      (feGetEnv("TR_disableCompareToOpt") != NULL);
  ...
  if (!disableCompareToOpt && TR::Compiler->target.is32Bit() &&
      firstChild->getOpCodeValue() == TR::lshr &&
      secondChild->getOpCodeValue() == TR::lshr &&
      firstChild->getReferenceCount() == 1 &&
    ...
    if (firstShiftAmountNode->getOpCode().isLoadConst() &&
        secondShiftAmountNode->getOpCode().isLoadConst() &&
      ...
      if (firstShiftValue > 32 && secondShiftValue > 32) {
        ...
        TR::Register *secondShiftSourceReg =
            cg->evaluate(secondShiftSourceNode);
        ...
        return TR::TreeEvaluator::compareIntsForEquality(
            branchOp, dstLabel, node, cg, isHint, likeliness);
      }
    }
  }
...
TR::Register *OMR::Power::TreeEvaluator::iflcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForEquality(node, cg);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::blt,
                                          TR::InstOpCode::bgt, node, cg, true);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::bge,
                                          TR::InstOpCode::ble, node, cg, true);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::bgt,
                                          TR::InstOpCode::blt, node, cg, true);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::ble,
                                          TR::InstOpCode::bge, node, cg, true);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::blt,
                                          TR::InstOpCode::bgt, node, cg, false);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::bge,
                                          TR::InstOpCode::ble, node, cg, false);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::bgt,
                                          TR::InstOpCode::blt, node, cg, false);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::iflucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::TreeEvaluator::compareLongsForOrder(TR::InstOpCode::ble,
                                          TR::InstOpCode::bge, node, cg, false);
  return NULL;
}

TR::Register *OMR::Power::TreeEvaluator::ifacmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit()) {
...
TR::Register *OMR::Power::TreeEvaluator::ifacmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit()) {
...
TR::Register *OMR::Power::TreeEvaluator::icmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateRegister();
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      TR::Register *src2Reg = cg->evaluate(secondChild);
      generateTrg1Src2Instruction(cg, TR::InstOpCode::subf, node, temp1Reg,
                                  src2Reg, src1Reg);
      killTemp1 = true;
    }
  }
  generateTrg1Src1Instruction(cg, TR::InstOpCode::cntlzw, node, temp2Reg,
                              temp1Reg);
  generateShiftRightLogicalImmediate(cg, node, trgReg, temp2Reg, 5);

  if (killTemp1) cg->stopUsingRegister(temp1Reg);
...
TR::Register *OMR::Power::TreeEvaluator::icmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateRegister();
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::xori, node, trgReg,
                                   temp2Reg, 1);
  } else {
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addic, node, temp2Reg,
                                   temp1Reg, -1);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subfe, node, trgReg,
                                temp2Reg, temp1Reg);
  }
...
void OMR::Power::TreeEvaluator::genBranchSequence(
    TR::Node *node, TR::Register *src1Reg, TR::Register *src2Reg,
    TR::Register *trgReg, TR::InstOpCode::Mnemonic branchOpCode,
    TR::InstOpCode::Mnemonic cmpOpCode, TR::CodeGenerator *cg) {
  TR::Register *condReg = cg->allocateRegister(TR_CCR);
  TR::PPCControlFlowInstruction *cfop =
      (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
          cg, TR::InstOpCode::setbool, node);
  cfop->addTargetRegister(condReg);
  cfop->addTargetRegister(trgReg);
...
TR::Register *OMR::Power::TreeEvaluator::icmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      ...
      killTemp1 = true;
      if (value > 0) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, temp1Reg,
                                    temp2Reg, src1Reg);
      } else  // if (value < 0)
      {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, temp1Reg,
                                    temp2Reg, src1Reg);
      }
      cg->stopUsingRegister(temp2Reg);
    }
    ...
    TR::Register *src2Reg = cg->evaluate(secondChild);
    TR::TreeEvaluator::genBranchSequence(node, src1Reg, src2Reg, trgReg,
                                         TR::InstOpCode::blt,
                                         TR::InstOpCode::cmp4, cg);
  }
  node->setRegister(trgReg);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
...
TR::Register *OMR::Power::TreeEvaluator::icmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == -1) {
      ...
      TR::Register *temp2Reg =
          addConstantToInteger(node, src1Reg, -(value + 1), cg);
      temp1Reg = cg->allocateRegister();
      killTemp1 = true;
      if (value >= 0) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, temp1Reg,
...
TR::Register *OMR::Power::TreeEvaluator::icmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    int64_t value = secondChild->get64bitIntegralValue();
    if (value == 0) {
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi, node, temp1Reg,
                                     src1Reg, 31);
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addic, node, trgReg,
                                     temp1Reg, 1);
    } else {
      TR::Register *temp2Reg = addConstantToInteger(node, src1Reg, -value, cg);
      if (value > 0) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::nor, node, temp1Reg,
                                    temp2Reg, src1Reg);
      } else  // if (value < 0)
      {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::nand, node, temp1Reg,
                                    temp2Reg, src1Reg);
      }
      cg->stopUsingRegister(temp2Reg);
      generateShiftRightLogicalImmediate(cg, node, trgReg, temp1Reg, 31);
...
TR::Register *OMR::Power::TreeEvaluator::icmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *OMR::Power::TreeEvaluator::iucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *OMR::Power::TreeEvaluator::iucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *OMR::Power::TreeEvaluator::iucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *OMR::Power::TreeEvaluator::iucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
...
TR::Register *OMR::Power::TreeEvaluator::lcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit()) {
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmp4,
                                           TR::InstOpCode::beq, node, cg);
  }
  ...
  if (secondChild->getOpCode().isLoadConst()) value = secondChild->getLongInt();
  ...
  generateTrg1Src1Instruction(cg, TR::InstOpCode::cntlzd, node, temp2Reg,
                              temp1Reg);
...
TR::Register *OMR::Power::TreeEvaluator::lcmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit()) {
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmp4,
                                           TR::InstOpCode::bne, node, cg);
  }
  ...
  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addic, node, temp2Reg,
                                 temp1Reg, -1);
  generateTrg1Src2Instruction(cg, TR::InstOpCode::subfe, node, trgReg, temp2Reg,
                              temp1Reg);
...
TR::Register *OMR::Power::TreeEvaluator::lcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmp4,
                                           TR::InstOpCode::blt, node, cg);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      ...
      TR::Register *temp2Reg =
          addConstantToLong(node, src1Reg, -value, NULL, cg);
      temp1Reg = cg->allocateRegister();
      killTemp1 = true;
      if (value > 0) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, temp1Reg,
...
TR::Register *OMR::Power::TreeEvaluator::lcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmp4,
                                           TR::InstOpCode::ble, node, cg);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == -1) {
      ...
      TR::Register *temp2Reg =
          addConstantToLong(node, src1Reg, -(value + 1), NULL, cg);
      temp1Reg = cg->allocateRegister();
      killTemp1 = true;
      if (value >= 0) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, temp1Reg,
...
TR::Register *OMR::Power::TreeEvaluator::lcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmp4,
                                           TR::InstOpCode::bge, node, cg);
  ...
  if (secondChild->getOpCode().isLoadConst() &&
    ...
    if (value == 0) {
      ...
      TR::Register *temp2Reg = cg->allocateRegister();
      generateTrg1Src1Instruction(cg, TR::InstOpCode::neg, node, temp1Reg,
                                  src1Reg);
      generateTrg1Src2Instruction(cg, TR::InstOpCode::andc, node, temp2Reg,
                                  temp1Reg, src1Reg);
...
TR::Register *OMR::Power::TreeEvaluator::lcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmp4,
                                           TR::InstOpCode::bgt, node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::lucmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmpl4,
                                           TR::InstOpCode::blt, node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::lucmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmpl4,
                                           TR::InstOpCode::ble, node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::lucmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmpl4,
                                           TR::InstOpCode::bge, node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::lucmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is32Bit())
    return compareLongAndSetOrderedBoolean(TR::InstOpCode::cmpl4,
                                           TR::InstOpCode::bgt, node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::lcmpEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateRegister();
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
  ...
  if (TR::Compiler->target.is64Bit()) {
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi8, node, condReg,
                                       src1Reg, 0);
        generateTrg1Src1Instruction(cg, TR::InstOpCode::setb, node, trgReg,
                                    condReg);
        cg->stopUsingRegister(condReg);
      } else {
        temp1Reg = cg->allocateRegister();

        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::sradi, node, trgReg,
                                       src1Reg, 63);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addic, node,
                                       temp1Reg, src1Reg, -1);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::adde, node, trgReg,
                                    trgReg, trgReg);
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
        ...
        generateTrg1Src2Instruction(cg, TR::InstOpCode::subfc, node, temp3Reg,
                                    src1Reg, src2Reg);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::subfe, node, trgReg,
                                    temp2Reg, temp1Reg);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::subfc, node, temp2Reg,
                                    temp3Reg, trgReg);
        generateTrg1Src1Instruction(cg, TR::InstOpCode::subfze, node, trgReg,
                                    trgReg);
    ...
    if (secondChild->getOpCode().isLoadConst() &&
      ...
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::subfic, node, temp2Reg,
                                     src1Reg->getLowOrder(), 0);
      generateTrg1Src1Instruction(cg, TR::InstOpCode::subfze, node, temp3Reg,
                                  src1Reg->getHighOrder());
      generateShiftRightLogicalImmediate(cg, node, temp2Reg, temp3Reg, 31);
      generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, trgReg,
                                  temp1Reg, temp2Reg);
...
TR::Register *OMR::Power::TreeEvaluator::acmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
TR::Register *OMR::Power::TreeEvaluator::acmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  if (TR::Compiler->target.is64Bit())
...
static bool isGlDepsUnBalanced(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  numDeps = (child->getNumChildren() > 0)
                ? child->getFirstChild()->getNumChildren()
                : 0;
  for (ii = 2; ii < total; ii++) {
    child = node->getChild(ii);
    if ((child->getNumChildren() == 0 && numDeps != 0) ||
        (child->getNumChildren() > 0 &&
         numDeps != child->getFirstChild()->getNumChildren()))
      break;
  }
  if (ii != total) return (true);

  if (numDeps == 0) return (false);
  ...
  for (ii = 2; ii < total; ii++) {
    ...
    if (!(avec == bvec)) return (true);
    if (ii != total - 1) bvec.empty();
  }
  return (false);
}
...
static void switchDispatch(TR::Node *node, bool fromTableEval,
  ...
  if (!unbalanced) {
    for (ii = 2; ii < total; ii++) {
      if (node->getChild(ii)->getNumChildren() > 0) {
        ...
        if (glDepNode != NULL) cg->evaluate(glDepNode);
      }
    }
  }
  ...
  if (total <= 12) {
    for (ii = 2;
         ii < total && node->getChild(ii)->getCaseConstant() >= LOWER_IMMED &&
  ...
  if (total <= 9) {
    ...
    for (ii = 3; ii < total; ii++) {
      ...
      preInt += diff;
      if (diff < 0 || diff > UPPER_IMMED) break;
    }
    if (ii >= total) {
...
static void lookupScheme1(TR::Node *node, bool unbalanced, bool fromTableEval,
  ...
  if (two_reg) {
    ...
    TR::addDependency(conditions, selector->getHighOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(conditions, selector->getLowOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
  } else
    TR::addDependency(conditions, selector, TR::RealRegister::NoReg, TR_GPR,
                      cg);

  TR::addDependency(conditions, cndRegister, TR::RealRegister::NoReg, TR_CCR,
                    cg);

  acond = conditions;
  if (secondChild->getNumChildren() > 0 && !unbalanced) {
    cg->evaluate(secondChild->getFirstChild());
    acond = acond->clone(cg, generateRegisterDependencyConditions(
                                 cg, secondChild->getFirstChild(), 0));
  }
  ...
  for (int i = 2; i < total; i++) {
    ...
    if (isInt64) {
      ...
      if (TR::Compiler->target.is64Bit()) {
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi8, node,
                                       cndRegister, selector, value);
      } else {
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node,
                                       cndRegister, selector->getHighOrder(),
                                       0);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node,
                                             toDefaultLabel, cndRegister);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node,
                                       cndRegister, selector->getHighOrder(),
                                       value);
      }  // 64bit target?
      ...
      generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::cmpi4, node, cndRegister, selector,
    ...
    if (unbalanced) {
      ...
      if (child->getNumChildren() > 0) {
        ...
        bcond = bcond->clone(cg, generateRegisterDependencyConditions(
                                     cg, child->getFirstChild(), 0));
      }
      generateDepConditionalBranchInstruction(
          cg, TR::InstOpCode::beq, node,
  ...
  generateDepLabelInstruction(
      cg, TR::InstOpCode::b, node,
      secondChild->getBranchDestination()->getNode()->getLabel(), acond);
...
static void lookupScheme2(TR::Node *node, bool unbalanced, bool fromTableEval,
  ...
  TR::LabelSymbol *toDefaultLabel = NULL;
  if (two_reg) {
    valRegister = cg->allocateRegisterPair(cg->allocateRegister(),
                                           cg->allocateRegister());
    toDefaultLabel = generateLabelSymbol(cg);
  } else {
  ...
  if (two_reg) {
    TR::addDependency(conditions, valRegister->getHighOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(conditions, valRegister->getLowOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(conditions, selector->getHighOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(conditions, selector->getLowOrder(),
    ...
    TR::addDependency(conditions, valRegister, TR::RealRegister::NoReg, TR_GPR,
                      cg);
    TR::addDependency(conditions, selector, TR::RealRegister::NoReg, TR_GPR,
                      cg);
  }
  ...
  if (isInt64) {
    if (two_reg) {
      loadConstant(cg, node, (int32_t)(preInt >> 32),
                   valRegister->getHighOrder());
  ...
  for (int i = 2; i < total; i++) {
    ...
    if (isInt64) {
      if (TR::Compiler->target.is64Bit()) {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp8, node, cndRegister,
                                    selector, valRegister);
      } else {
        generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, cndRegister,
    ...
    if (i < total - 1) {
      int32_t diff = fromTableEval
                         ? 1
                         : (node->getChild(i + 1)->getCaseConstant() - preInt);
      preInt += diff;
      if (!isInt64)
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node,
                                       valRegister, valRegister, (int32_t)diff);
      else
...
static void lookupScheme3(TR::Node *node, bool unbalanced,
  ...
  if (isInt64) {
    dataTableSize *= 2;
    dataTable64 = (int64_t *)cg->allocateCodeMemory(
        dataTableSize, cg->getCurrentEvaluationBlock()->isCold());
  } else
    dataTable = (int32_t *)cg->allocateCodeMemory(
        dataTableSize, cg->getCurrentEvaluationBlock()->isCold());

  intptrj_t address =
      isInt64 ? ((intptrj_t)dataTable64) : ((intptrj_t)dataTable);
  TR::Register *selector = cg->evaluate(node->getFirstChild());
  TR::Register *cndRegister = cg->allocateRegister(TR_CCR);
  ...
  TR::LabelSymbol *toDefaultLabel = NULL;
  if (two_reg) {
    dataRegister = cg->allocateRegisterPair(cg->allocateRegister(),
                                            cg->allocateRegister());
    toDefaultLabel = generateLabelSymbol(cg);
  } else
  ...
  TR::addDependency(conditions, addrRegister, TR::RealRegister::NoReg, TR_GPR,
                    cg);
  if (isInt64 && TR::Compiler->target.is64Bit()) {
    TR::addDependency(conditions, dataRegister->getHighOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(conditions, dataRegister->getLowOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(conditions, selector->getHighOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
    TR::addDependency(conditions, selector->getLowOrder(),
    ...
    TR::addDependency(conditions, dataRegister, TR::RealRegister::NoReg, TR_GPR,
                      cg);
    TR::addDependency(conditions, selector, TR::RealRegister::NoReg, TR_GPR,
                      cg);
  }
    ...
    if (offset != PTOC_FULL_INDEX) {
      ...
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node,
                                       addrRegister, cg->getTOCBaseRegister(),
                                       cg->hiValue(offset));
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::Op_load, node, addrRegister,
            new (cg->trHeapMemory())
                TR::MemoryReference(addrRegister, LO_VALUE(offset), 8, cg));
      } else {
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::Op_load, node, addrRegister,
            new (cg->trHeapMemory())
      ...
      if (cg->needRelocationsForLookupEvaluationData()) {
        ...
        loadAddressConstant(cg, false, node, cg->hiValue(address) << 16,
                            addrRegister);
        nextAddress = LO_VALUE((int32_t)address);
        if (isInt64) {
          if (nextAddress >= 32760) {
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::ldu, node, dataRegister,
                new (cg->trHeapMemory())
                    TR::MemoryReference(addrRegister, nextAddress, 8, cg));
            nextAddress = 8;
          } else {
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::ld, node, dataRegister,
                new (cg->trHeapMemory())
                    TR::MemoryReference(addrRegister, nextAddress, 8, cg));
            nextAddress += 8;
          }
          ...
          if (nextAddress >= 32764) {
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::lwzu, node, dataRegister,
                new (cg->trHeapMemory())
                    TR::MemoryReference(addrRegister, nextAddress, 4, cg));
            nextAddress = 4;
          } else {
            generateTrg1MemInstruction(
                cg, TR::InstOpCode::lwz, node, dataRegister,
                new (cg->trHeapMemory())
                    TR::MemoryReference(addrRegister, nextAddress, 4, cg));
            nextAddress += 4;
          }
  ...
  {
    rel1 =
        generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, addrRegister,
                                   cg->hiValue(address) & 0x0000ffff);

    if (isInt64) {
      nextAddress = LO_VALUE(address);
      if (nextAddress == 32760) {
        rel2 = generateTrg1MemInstruction(
            cg, TR::InstOpCode::lwz, node, dataRegister->getHighOrder(),
            new (cg->trHeapMemory())
                TR::MemoryReference(addrRegister, nextAddress, 4, cg));
        nextAddress += 4;
        rel2 = generateTrg1MemInstruction(
            cg, TR::InstOpCode::lwzu, node, dataRegister->getLowOrder(),
            new (cg->trHeapMemory())
                TR::MemoryReference(addrRegister, nextAddress, 4, cg));
        nextAddress = 4;
      } else {
        ...
        nextAddress += 4;
        rel2 = generateTrg1MemInstruction(
            cg, TR::InstOpCode::lwz, node, dataRegister->getLowOrder(),
            new (cg->trHeapMemory())
                TR::MemoryReference(addrRegister, nextAddress, 4, cg));
        nextAddress += 4;
      }
      ...
      if (cg->comp()->compileRelocatableCode()) {
        rel2 = generateTrg1MemInstruction(
            cg, TR::InstOpCode::lwzu, node, dataRegister,
            new (cg->trHeapMemory())
                TR::MemoryReference(addrRegister, LO_VALUE(address), 4, cg));
      } else {
        nextAddress = LO_VALUE(address);
        if (nextAddress >= 32764) {
          rel2 = generateTrg1MemInstruction(
    ...
    cg->getAheadOfTimeCompile()->getRelocationList().push_front(
        new (cg->trHeapMemory())
            TR::PPCPairedRelocation(rel1, rel2, (uint8_t *)recordInfo,
...
static void lookupScheme4(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR::InstOpCode::Mnemonic cmp_opcode =
      isUnsigned ? TR::InstOpCode::cmpl4 : TR::InstOpCode::cmp4;

  cg->machine()->setLinkRegisterKilled(true);
  TR::addDependency(conditions, addrRegister, TR::RealRegister::NoReg, TR_GPR,
                    cg);
  TR::addDependency(conditions, pivotRegister, TR::RealRegister::NoReg, TR_GPR,
                    cg);
  if (two_reg) {
    TR::addDependency(conditions, dataRegister->getHighOrder(),
                      TR::RealRegister::NoReg, TR_GPR, cg);
  ...
  TR::addDependency(conditions, lowRegister, TR::RealRegister::NoReg, TR_GPR,
                    cg);
  TR::addDependency(conditions, highRegister, TR::RealRegister::NoReg, TR_GPR,
                    cg);
  TR::addDependency(conditions, cndRegister, TR::RealRegister::NoReg, TR_CCR,
                    cg);
  conditions->getPreConditions()->getRegisterDependency(0)->setExcludeGPR0();
  ...
  if (secondChild->getNumChildren() > 0) {
    cg->evaluate(secondChild->getFirstChild());
    conditions =
        conditions->clone(cg, generateRegisterDependencyConditions(
                                  cg, secondChild->getFirstChild(), 0));
  }
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (offset != PTOC_FULL_INDEX) {
      ...
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node,
                                       pivotRegister, cg->getTOCBaseRegister(),
                                       cg->hiValue(offset));
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::Op_load, node, addrRegister,
            new (cg->trHeapMemory())
                TR::MemoryReference(pivotRegister, LO_VALUE(offset), 8, cg));
      } else {
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::Op_load, node, addrRegister,
            new (cg->trHeapMemory())
      ...
      loadAddressConstant(cg, cg->needRelocationsForLookupEvaluationData(),
                          node, address, addrRegister);
    }
  } else {
    rel1 = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node,
                                      pivotRegister, cg->hiValue(address));
    rel2 = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node,
                                          addrRegister, pivotRegister,
                                          LO_VALUE(address));
  ...
  generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, pivotRegister,
                              highRegister, lowRegister);
  generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node,
                                  pivotRegister, pivotRegister, 31, 0xfffffffc);
  if (isInt64) {
    if (two_reg) {
      generateTrg1MemInstruction(
          cg, TR::InstOpCode::lwzx, node, dataRegister->getHighOrder(),
          new (cg->trHeapMemory())
              TR::MemoryReference(addrRegister, pivotRegister, 4, cg));
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node,
                                     pivotRegister, pivotRegister, 4);
      generateTrg1MemInstruction(
          cg, TR::InstOpCode::lwzx, node, dataRegister->getLowOrder(),
          new (cg->trHeapMemory())
              TR::MemoryReference(addrRegister, pivotRegister, 4, cg));
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node,
                                     pivotRegister, pivotRegister, -4);
      ...
      generateTrg1MemInstruction(cg, TR::InstOpCode::ldx, node, dataRegister,
                                 new (cg->trHeapMemory()) TR::MemoryReference(
                                     addrRegister, pivotRegister, 8, cg));
    }
  } else {
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwzx, node, dataRegister,
                               new (cg->trHeapMemory()) TR::MemoryReference(
                                   addrRegister, pivotRegister, 4, cg));
  }

  if (isInt64) {
    if (two_reg) {
      generateTrg1Src2Instruction(cg, cmp_opcode, node, cndRegister,
                                  dataRegister->getHighOrder(),
                                  selector->getHighOrder());
      generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node,
                                           searchLabel, cndRegister);
      generateTrg1Src2Instruction(cg, cmp_opcode, node, cndRegister,
                                  dataRegister->getLowOrder(),
                                  selector->getLowOrder());
    } else {
      ...
      generateTrg1Src2Instruction(cg, cmp_opcode, node, cndRegister,
                                  dataRegister, selector);
    }
  } else {
    generateTrg1Src2Instruction(cg, cmp_opcode, node, cndRegister, dataRegister,
                                selector);
  }
  ...
  static bool disableNewLookupScheme4 =
      (feGetEnv("TR_DisableNewLookupScheme4") != NULL);
  if (disableNewLookupScheme4) {
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node,
                                   pivotRegister, pivotRegister, 20);
  } else {
    // New code
    cg->fixedLoadLabelAddressIntoReg(node, dataRegister, linkLabel);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, linkLabel);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node,
                                   pivotRegister, pivotRegister, 16);
  }

  generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, dataRegister,
                              pivotRegister, dataRegister);
  ...
  generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node,
                                       biggerLabel, cndRegister);
  generateTrg1Src2Instruction(cg, cmp_opcode, node, cndRegister, pivotRegister,
                              highRegister);
  generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node,
      node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
      cndRegister);
  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, lowRegister,
                                 pivotRegister, 4);
  ...
  generateConditionalBranchInstruction(
      ...
      node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
      cndRegister);
  generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, highRegister,
                                 pivotRegister, -4);

  generateDepLabelInstruction(cg, TR::InstOpCode::b, node, backLabel,
                              conditions);
...
TR::Register *OMR::Power::TreeEvaluator::lookupEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::tableEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  ...
  if (!node->isSafeToSkipTableBoundCheck()) {
    if (numCases > UPPER_IMMED) {
      ...
      generateTrg1Src2Instruction(cg, TR::InstOpCode::cmpl4, node, ccReg, sReg,
                                  tReg);
    } else
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, node, ccReg,
                                     sReg, numCases);

    generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bge, node,
...

  {
    if (TR::Compiler->target.is64Bit()) {
      ...
      if (tReg == NULL) {
        tReg = cg->allocateRegister();
        TR::addDependency(conditions, tReg, TR::RealRegister::NoReg, TR_GPR,
                          cg);
      }
      ...
      if (offset != PTOC_FULL_INDEX) {
        ...
        if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
          generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, reg1,
                                         cg->getTOCBaseRegister(),
                                         cg->hiValue(offset));
          generateTrg1MemInstruction(
              cg, TR::InstOpCode::Op_load, node, reg1,
              new (cg->trHeapMemory())
                  TR::MemoryReference(reg1, LO_VALUE(offset), 8, cg));
        } else {
          generateTrg1MemInstruction(
              cg, TR::InstOpCode::Op_load, node, reg1,
              new (cg->trHeapMemory())
      ...
      generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, reg1, reg1,
                                  tReg);
    } else {
  ...
  if (tReg != NULL) cg->stopUsingRegister(tReg);
...
TR::Instruction *OMR::Power::TreeEvaluator::generateNullTestInstructions(
  ...
  if (cg->getHasResumableTrapHandler()) {
    if (TR::Compiler->target.is64Bit())
      gcPoint = generateSrc1Instruction(cg, TR::InstOpCode::tdeqi, node, trgReg,
                                        NULLVALUE);
    else
      gcPoint = generateSrc1Instruction(cg, TR::InstOpCode::tweqi, node, trgReg,
                                        NULLVALUE);
    cg->setCanExceptByTrap();
  } else {
    TR::SymbolReference *symRef = node->getSymbolReference();
    if (nullPtrSymRefRequired) {
      symRef = comp->getSymRefTab()->findOrCreateNullCheckSymbolRef(
          comp->getMethodSymbol());
    }

    TR::LabelSymbol *snippetLabel =
        cg->lookUpSnippet(TR::Snippet::IsHelperCall, symRef);
    TR::RegisterDependencyConditions *conditions = new (cg->trHeapMemory())
    ...
    if (snippetLabel == NULL) {
      ...
      cg->addSnippet(new (cg->trHeapMemory()) TR::PPCHelperCallSnippet(
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
      ...
      gcPoint = generateDepConditionalBranchInstruction(
          cg, TR::InstOpCode::beql, PPCOpProp_BranchUnlikely, node,
          snippetLabel, condReg, conditions);
    else
      gcPoint = generateDepConditionalBranchInstruction(
          cg, TR::InstOpCode::beql, node, snippetLabel, condReg, conditions);
...
TR::Register *OMR::Power::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(
  ...
  if (comp->useCompressedPointers() &&
    ...
    TR::ILOpCodes rdbarOp =
        cg->comp()->il.opCodeForIndirectReadBarrier(TR::Int32);
  ...
  /*
   * If the first child is a load with a ref count of 1, just decrement the
   * reference count on the child. If the first child does not have a register,
   * it means it was never evaluated. As a result, the grandchild (the variable
   * reference) needs to be decremented as well.
   *
   * In other cases, evaluate the child node.
   *
   * Under compressedpointers, the first child will have a refCount of at least
   * 2 (the other under an anchor node).
   */
...
TR::Register *OMR::Power::TreeEvaluator::NULLCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return TR::TreeEvaluator::evaluateNULLCHKWithPossibleResolve(node, false, cg);
}

TR::Register *OMR::Power::TreeEvaluator::ZEROCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  TR_PPCOutOfLineCodeSection *outlinedHelperCall =
      new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(
          node, TR::call, NULL, slowPathLabel, restartLabel, cg);
  ...
  if (valueToCheck->getOpCode().isBooleanCompare() &&
      ...
      performTransformation(cg->comp(),
                            "O^O CODEGEN Optimizing ZEROCHK+%s %s\n",
    ...
    if (valueToCheck->getOpCode().isCompareForOrder()) {
      if (valueToCheck->getChild(0)->getOpCode().is8Byte() ||
        ...
        TR::InstOpCode::Mnemonic branchOp = cmp2branch(
            valueToCheck->getOpCode().getOpCodeForReverseBranch(), cg);
        TR::InstOpCode::Mnemonic reverseBranchOp =
            cmp2branch(valueToCheck->getOpCodeValue(), cg);
        TR::TreeEvaluator::compareLongsForOrder(
            branchOp, reverseBranchOp, slowPathLabel, valueToCheck, cg,
            valueToCheck->getOpCode().isUnsignedCompare(), true,
            PPCOpProp_BranchUnlikely);
      } else {
        // switch branches since we want to go to OOL on node evaluating to 0
        // which corresponds to an ifcmp fall-through
        TR::TreeEvaluator::compareIntsForOrder(
            cmp2branch(valueToCheck->getOpCode().getOpCodeForReverseBranch(),
                       cg),
            slowPathLabel, valueToCheck, cg,
            valueToCheck->getOpCode().isUnsignedCompare(), true,
            PPCOpProp_BranchUnlikely);
      }
    } else {
      TR_ASSERT(
          valueToCheck->getOpCode().isCompareForEquality(),
          "Compare opcode must either be compare for order or for equality");
      if (valueToCheck->getChild(0)->getOpCode().is8Byte() ||
          valueToCheck->getChild(1)->getOpCode().is8Byte()) {
        // switch branches since we want to go to OOL on node evaluating to 0
        // which corresponds to an ifcmp fall-through
        TR::TreeEvaluator::compareLongsForEquality(
            cmp2branch(valueToCheck->getOpCode().getOpCodeForReverseBranch(),
                       cg),
            slowPathLabel, valueToCheck, cg, true, PPCOpProp_BranchUnlikely);
      } else {
        TR::TreeEvaluator::compareIntsForEquality(
            cmp2branch(valueToCheck->getOpCode().getOpCodeForReverseBranch(),
                       cg),
            slowPathLabel, valueToCheck, cg, true, PPCOpProp_BranchUnlikely);
      }
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)  // Use PPC AS branch hint.
      generateConditionalBranchInstruction(cg, TR::InstOpCode::beq,
                                           PPCOpProp_BranchUnlikely, node,
                                           slowPathLabel, condReg);
    else
...
TR::Register *OMR::Power::TreeEvaluator::resolveAndNULLCHKEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
static bool virtualGuardHelper(TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (cg->comp()->compileRelocatableCode()) {
    ...
    switch (virtualGuard->getKind()) {
      ...
      default:
        TR_ASSERT(0,
                  "got AOT guard in node but virtual guard not one of known "
                  "guards supported for AOT. Guard: %d",
...
static TR::Register *generateMaxMin(TR::Node *node, TR::CodeGenerator *cg,
                                    bool max) {
  TR::Node *child = node->getFirstChild();
  TR::DataType data_type = child->getDataType();
  TR::DataType type = child->getType();
  TR::InstOpCode::Mnemonic move_op =
      type.isIntegral() ? TR::InstOpCode::mr : TR::InstOpCode::fmr;
  ...
  if (cg->canClobberNodesRegister(child)) {
    ...
    if (two_reg) {
      generateTrg1Src1Instruction(cg, move_op, node, trgReg->getLowOrder(),
                                  reg->getLowOrder());
      generateTrg1Src1Instruction(cg, move_op, node, trgReg->getHighOrder(),
                                  reg->getHighOrder());
    } else {
  ...
  for (int i = 1; i < n; i++) {
    ...
    if (two_reg) {
      ...
      generateConditionalBranchInstruction(
          cg, max ? TR::InstOpCode::bge : TR::InstOpCode::ble, node, label,
          condReg);
      generateTrg1Src1Instruction(cg, move_op, node, trgReg, reg);
...
TR::Register *OMR::Power::TreeEvaluator::maxEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return generateMaxMin(node, cg, true);
}

TR::Register *OMR::Power::TreeEvaluator::minEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  return generateMaxMin(node, cg, false);
}

TR::Register *OMR::Power::TreeEvaluator::igotoEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
...
//******************************************************************************
// evaluator for ificmno, ificmnno, ificmpo, ificmpno, iflcmno, iflcmnno,
// iflcmpo, iflcmpno
TR::Register *OMR::Power::TreeEvaluator::ifxcmpoEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if ((opCode == TR::ificmno) || (opCode == TR::ificmnno) ||
      (opCode == TR::iflcmno) || (opCode == TR::iflcmnno)) {
    generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, res, rs1,
                                rs2);  // compare negative
    generateTrg1Src2Instruction(cg, TR::InstOpCode::eqv, node, tmp, rs1, rs2);
  } else {
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subf, node, res, rs2,
                                rs1);  // compare
  ...
  generateTrg1Src1ImmInstruction(
      cg, nodeIs64Bit ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node, cr,
      res, 0);

  TR::LabelSymbol *dstLabel =
      node->getBranchDestination()->getNode()->getLabel();
  TR::InstOpCode::Mnemonic branchOp =
      reverseBranch ? TR::InstOpCode::bge : TR::InstOpCode::blt;

  if (node->getNumChildren() == 3) {
    TR::Node *thirdChild = node->getChild(2);
    cg->evaluate(thirdChild);
    generateDepConditionalBranchInstruction(
        cg, branchOp, node, dstLabel, cr,
        generateRegisterDependencyConditions(cg, thirdChild, 0));
    cg->decReferenceCount(thirdChild);
  } else {
