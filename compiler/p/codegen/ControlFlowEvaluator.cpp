    ...
    TR::RegisterDependencyConditions* deps, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    ...
    if (firstChild->isHighWordZero()) {
        ...
        if (firstChild->getReferenceCount() == 1 && src1Reg == NULL) {
            if (firstOp == TR::iu2l || firstOp == TR::su2l || (firstOp == TR::lushr && firstChild->getSecondChild()->getOpCode().isLoadConst() && (firstChild->getSecondChild()->getInt() & LONG_SHIFT_MASK) == 32)) {
    ...
    if (secondChild->isHighWordZero()) {
        ...
        if (secondChild->getReferenceCount() == 1 && src2Reg == NULL) {
            if (secondOp == TR::iu2l || secondOp == TR::su2l || (secondOp == TR::lushr && secondChild->getSecondChild()->getOpCode().isLoadConst() && (secondChild->getSecondChild()->getInt() & LONG_SHIFT_MASK) == 32)) {
    ...
    TR::Node* node, TR::CodeGenerator* cg, bool isSigned, bool isHint, bool likeliness)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        src1Reg = cg->evaluate(firstChild);
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && ((!isSigned && ((uint64_t)value <= 0xffff)) || (isSigned && (value >= LOWER_IMMED && value <= UPPER_IMMED)))) {
            if (isSigned)
    ...
    } else // 32 bit target
    {
        ...
        if ((firstChild->isHighWordZero() || secondChild->isHighWordZero()) && !(secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL)) {
            return compareLongsForOrderWithAnalyser(branchOp, reversedBranchOp, condReg, dstLabel, deps, node, cg, isHint, likeliness);
        }

        TR::Register* src2Reg;
        src1Reg = cg->evaluate(firstChild);
        bool useImmed = secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && value >= 0 && value <= 0xffff;
...
TR::Register* OMR::Power::TreeEvaluator::iternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* resultReg = two_reg ? cg->allocateRegisterPair(cg->allocateRegister(), cg->allocateRegister()) : (type.getDataType() == TR::Float ? cg->allocateSinglePrecisionRegister() : (type.getDataType() == TR::Double ? cg->allocateRegister(TR_FPR) : cg->allocateRegister(TR_GPR)));
    ...
    if (firstChild->getOpCode().isBooleanCompare() && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && !(firstChild->getFirstChild()->getType().isInt64() && TR::Compiler->target.is32Bit())) {
...
TR::Register* OMR::Power::TreeEvaluator::compareIntsForEquality(TR::InstOpCode::Mnemonic branchOp, TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
...
#ifdef J9_PROJECT_SPECIFIC
    if (cg->profiledPointersRequireRelocation() && secondChild->getOpCodeValue() == TR::aconst && (secondChild->isClassPointerConstant() || secondChild->isMethodPointerConstant())) {
    ...
    if (firstChild->getOpCodeValue() == TR::b2i && firstChild->getOpCodeValue() == secondChild->getOpCodeValue() &&
        ...
        firstChild->getFirstChild()->getOpCode().isLoad() && firstChild->getFirstChild()->getOpCodeValue() == secondChild->getFirstChild()->getOpCodeValue()) {
        /*
       ...
       */
        ...
        TR::InstOpCode::Mnemonic opCode = (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifscmpeq || node->getOpCodeValue() == TR::ifbcmpeq)
            ? TR::InstOpCode::beq
            : TR::InstOpCode::bne;
    ...
    if ((firstChild->getOpCodeValue() == TR:: instanceof) && !(comp->getOption(TR_OptimizeForSpace) || comp->getOption(TR_DisableInlineIfInstanceOf)) && (firstChild->getRegister() == NULL) && (node->getReferenceCount() <= 1) && secondChild->getOpCode().isLoadConst() && (value == 0 || value == 1)) {
#ifdef J9_PROJECT_SPECIFIC
    ...
    if (isSigned) {
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && value >= LOWER_IMMED && value <= UPPER_IMMED) {
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpi4, node, condReg, src1Reg, value);
        } else {
            TR::Register* src2Reg = cg->evaluate(secondChild);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::cmp4, node, condReg, src1Reg, src2Reg);
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && secondChild->get64bitIntegralValue() >= 0 && secondChild->get64bitIntegralValue() <= 0xFFFF) {
...
TR::Register* OMR::Power::TreeEvaluator::compareLongsForEquality(TR::InstOpCode::Mnemonic branchOp, TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    ...
    if (!disableCompareToOpt && TR::Compiler->target.is32Bit() && firstChild->getOpCodeValue() == TR::lshr && secondChild->getOpCodeValue() == TR::lshr && firstChild->getReferenceCount() == 1 && secondChild->getReferenceCount() == 1) {
        ...
        if (firstShiftAmountNode->getOpCode().isLoadConst() && secondShiftAmountNode->getOpCode().isLoadConst() && !firstShiftResultReg && !secondShiftResultReg) {
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && ((!isSigned && ((uint64_t)value <= 0xffff)) || (isSigned && (value >= LOWER_IMMED && value <= UPPER_IMMED))) && (secondChild->getOpCodeValue() != TR::aconst || !cg->constantAddressesCanChangeSize(secondChild)))
...
TR::Register* OMR::Power::TreeEvaluator::iucmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && secondChild->get64bitIntegralValue() == 0) {
...
static bool isGlDepsUnBalanced(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    for (ii = 2; ii < total; ii++) {
        ...
        if ((child->getNumChildren() == 0 && numDeps != 0) || (child->getNumChildren() > 0 && numDeps != child->getFirstChild()->getNumChildren()))
            break;
    }
    if (ii != total)
        return (true);
...
TR::Register* OMR::Power::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (valueToCheck->getOpCode().isBooleanCompare() && valueToCheck->getChild(0)->getOpCode().isIntegerOrAddress() && valueToCheck->getChild(1)->getOpCode().isIntegerOrAddress() && performTransformation(cg->comp(), "O^O CODEGEN Optimizing ZEROCHK+%s %s\n", valueToCheck->getOpCode().getName(), valueToCheck->getName(cg->getDebug()))) {
