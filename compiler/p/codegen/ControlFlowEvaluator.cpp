  ...
  if (node->getOpCode().isIf() && node->getNumChildren() == 3) {
    ...
    if (isHint && TR::Compiler->target.cpu.id() >= TR_PPCgp)
      generateDepConditionalBranchInstruction(
        cg, branchOp, likeliness, node, dstLabel, condReg, generateRegisterDependencyConditions(cg, thirdChild, 0));
    else
      generateDepConditionalBranchInstruction(
  ...
  bool isSigned, bool isHint, bool likeliness)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
      && ((!isSigned && ((uint64_t)value <= 0xffff)) || (isSigned && (value >= LOWER_IMMED && value <= UPPER_IMMED)))) {
      if (isSigned)
  ...
  } else // 32 bit target
  {
    ...
    src1Reg = cg->evaluate(firstChild);
    bool useImmed
      = secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && value >= 0 && value <= 0xffff;
  ...
  TR::InstOpCode::Mnemonic reversedBranchOp, TR::Node* node, TR::CodeGenerator* cg, bool isSigned)
{
  return TR::TreeEvaluator::compareLongsForOrder(
    branchOp, reversedBranchOp, node->getBranchDestination()->getNode()->getLabel(), node, cg, isSigned, false, false);
}

TR::Register* compareLongAndSetOrderedBoolean(
  TR::InstOpCode::Mnemonic compareOp, TR::InstOpCode::Mnemonic branchOp, TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
  TR::Node* secondChild = node->getSecondChild();
  ...
  TR::Register* src2Reg = cg->evaluate(secondChild);
  TR::PPCControlFlowInstruction* cfop
    = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::setblong, node);
  cfop->addTargetRegister(condReg);
...
TR::Register* OMR::Power::TreeEvaluator::gotoEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (node->getNumChildren() > 0) {
    ...
    cg->evaluate(child);
    generateDepLabelInstruction(cg, TR::InstOpCode::b, node, temp, generateRegisterDependencyConditions(cg, child, 0));
...
TR::Register* OMR::Power::TreeEvaluator::iternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::Register* resultReg = two_reg
    ...
    : (type.getDataType() == TR::Float
        ...
        : (type.getDataType() == TR::Double ? cg->allocateRegister(TR_FPR) : cg->allocateRegister(TR_GPR)));
  ...
  TR::InstOpCode::Mnemonic move_opcode
    = (type.isIntegral() || type.isAddress()) ? TR::InstOpCode::mr : TR::InstOpCode::fmr;
  TR::Node* firstChild = node->getFirstChild();

  if (firstChild->getOpCode().isBooleanCompare() && firstChild->getRegister() == NULL
    ...
    if (compare_type.isIntegral() || compare_type.isAddress()) {
      ...
      if (useImmediateCompare) {
        generateTrg1Src1ImmInstruction(cg, cmp2cmpi(firstChild->getOpCodeValue(), cg), node, ccr, cmp1Reg, value);
      } else {
        generateTrg1Src2Instruction(cg, cmp2cmp(firstChild->getOpCodeValue(), cg), node, ccr, cmp1Reg, cmp2Reg);
      }
    ...
    TR::RegisterDependencyConditions* dep
      = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, two_reg ? 8 : 5, cg->trMemory());
    if (two_reg) {
    ...
    TR::PPCControlFlowInstruction* i = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(
      cg, TR::InstOpCode::iternary, node, NULL, 0, two_reg, useRegPairForCond);
  ...
  TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
  ...
  if (firstChild->getOpCodeValue() == TR::b2i && firstChild->getOpCodeValue() == secondChild->getOpCodeValue() &&
    ...
    TR::InstOpCode::Mnemonic opCode = (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifscmpeq
    ...
    // 3) had we evaluated the child, the grandchild's refcount would have been decremented, so we have to decrement it
    // instead If a child is in a register, regardless of whether or not we evaluated it, we don't have to decrement the
    // grandchild
  ...
  if ((firstChild->getOpCodeValue() == TR:: instanceof)
    && !(comp->getOption(TR_OptimizeForSpace) || comp->getOption(TR_DisableInlineIfInstanceOf))
    && (firstChild->getRegister() == NULL) && (node->getReferenceCount() <= 1) && secondChild->getOpCode().isLoadConst()
    && (value == 0 || value == 1)) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
  ...
  if (!disableCompareToOpt && TR::Compiler->target.is32Bit() && firstChild->getOpCodeValue() == TR::lshr
...
void OMR::Power::TreeEvaluator::genBranchSequence(TR::Node* node, TR::Register* src1Reg, TR::Register* src2Reg,
  TR::Register* trgReg, TR::InstOpCode::Mnemonic branchOpCode, TR::InstOpCode::Mnemonic cmpOpCode,
  TR::CodeGenerator* cg)
{
  TR::Register* condReg = cg->allocateRegister(TR_CCR);
  TR::PPCControlFlowInstruction* cfop
    = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::setbool, node);
  cfop->addTargetRegister(condReg);
  cfop->addTargetRegister(trgReg);
...
TR::Register* OMR::Power::TreeEvaluator::iucmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
    ...
    TR::Register* src2Reg = cg->evaluate(secondChild);
    TR::TreeEvaluator::genBranchSequence(
      node, src1Reg, src2Reg, trgReg, TR::InstOpCode::blt, TR::InstOpCode::cmpl4, cg);
  }
  node->setRegister(trgReg);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
...
TR::Register* OMR::Power::TreeEvaluator::iucmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
    ...
    TR::Register* src2Reg = cg->evaluate(secondChild);
    TR::TreeEvaluator::genBranchSequence(
      node, src1Reg, src2Reg, trgReg, TR::InstOpCode::bge, TR::InstOpCode::cmpl4, cg);
  }
  node->setRegister(trgReg);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
...
TR::Register* OMR::Power::TreeEvaluator::lcmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0) {
      ...
      TR::PPCControlFlowInstruction* cfop
        = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::lcmp, node);
...
static void lookupScheme1(TR::Node* node, bool unbalanced, bool fromTableEval, TR::CodeGenerator* cg)
{
  ...
  for (int i = 2; i < total; i++) {
    ...
    if (unbalanced) {
      ...
      generateDepConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, child->getBranchDestination()->getNode()->getLabel(), cndRegister, bcond);
    } else {
    ...
    if (isInt64) {
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::cmp4, node, cndRegister, selector->getHighOrder(), valRegister->getHighOrder());
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, toDefaultLabel, cndRegister);
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::cmp4, node, cndRegister, selector->getLowOrder(), valRegister->getLowOrder());
      }
    } else {
    ...
    if (offset != PTOC_FULL_INDEX) {
      ...
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
          new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, LO_VALUE(offset), 8, cg));
      } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
          new (cg->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), offset, 8, cg));
      }
        ...
        if (isInt64) {
          if (nextAddress >= 32760) {
            generateTrg1MemInstruction(cg, TR::InstOpCode::ldu, node, dataRegister,
              new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, nextAddress, 8, cg));
            nextAddress = 8;
          } else {
            generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, dataRegister,
              new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, nextAddress, 8, cg));
          ...
          if (nextAddress >= 32764) {
            generateTrg1MemInstruction(cg, TR::InstOpCode::lwzu, node, dataRegister,
              new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, nextAddress, 4, cg));
            nextAddress = 4;
          } else {
            generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, dataRegister,
              new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, nextAddress, 4, cg));
    ...
    if (isInt64) {
      ...
      if (cg->comp()->compileRelocatableCode()) {
        rel2 = generateTrg1MemInstruction(cg, TR::InstOpCode::lwzu, node, dataRegister,
          new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, LO_VALUE(address), 4, cg));
      } else {
        nextAddress = LO_VALUE(address);
        if (nextAddress >= 32764) {
          rel2 = generateTrg1MemInstruction(cg, TR::InstOpCode::lwzu, node, dataRegister,
  ...
  for (int32_t ii = 2; ii < total; ii++) {
    ...
    if (isInt64) {
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateTrg1Src2Instruction(
          cg, TR::InstOpCode::cmp4, node, cndRegister, selector->getHighOrder(), dataRegister->getHighOrder());
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, toDefaultLabel, cndRegister);
        generateTrg1Src2Instruction(
...
static void lookupScheme4(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (offset != PTOC_FULL_INDEX) {
      ...
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
          new (cg->trHeapMemory()) TR::MemoryReference(pivotRegister, LO_VALUE(offset), 8, cg));
      } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, addrRegister,
          new (cg->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), offset, 8, cg));
      }
    ...
    rel2
      = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, addrRegister, pivotRegister, LO_VALUE(address));
  ...
  if (isInt64) {
    if (two_reg) {
      generateTrg1MemInstruction(cg, TR::InstOpCode::lwzx, node, dataRegister->getHighOrder(),
        new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, pivotRegister, 4, cg));
      ...
      generateTrg1MemInstruction(cg, TR::InstOpCode::ldx, node, dataRegister,
        new (cg->trHeapMemory()) TR::MemoryReference(addrRegister, pivotRegister, 8, cg));
    }
  } else {
  ...
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, node->getSecondChild()->getBranchDestination()->getNode()->getLabel(), cndRegister);
...

  {
    if (TR::Compiler->target.is64Bit()) {
      ...
      if (offset != PTOC_FULL_INDEX) {
        ...
        if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
          generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addis, node, reg1, cg->getTOCBaseRegister(), cg->hiValue(offset));
          generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, reg1,
            new (cg->trHeapMemory()) TR::MemoryReference(reg1, LO_VALUE(offset), 8, cg));
        } else {
          generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, reg1,
            new (cg->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), offset, 8, cg));
        }
    ...
    generateDepLabelInstruction(
      cg, TR::InstOpCode::b, node, node->getChild(i + 2)->getBranchDestination()->getNode()->getLabel(), conditions);
  }
  ...
  TR::CodeGenerator* cg, TR::Register* trgReg, TR::Node* node, bool nullPtrSymRefRequired)
{
  ...
  if (cg->getHasResumableTrapHandler()) {
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
      ...
      gcPoint
        = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::beql, node, snippetLabel, condReg, conditions);
  ...
  TR::Node* node, bool needsResolve, TR::CodeGenerator* cg)
{
  ...
  if (opCode.isLoad() && firstChild->getReferenceCount() == 1 && !firstChild->getSymbolReference()->isUnresolved()) {
    ...
    if (comp->useCompressedPointers()) {
      ...
      if (firstChild->getOpCode().isStoreIndirect() && firstChild->getReferenceCount() > 1) {
...
TR::Register* OMR::Power::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (valueToCheck->getOpCode().isBooleanCompare() && valueToCheck->getChild(0)->getOpCode().isIntegerOrAddress()
    ...
    if (valueToCheck->getOpCode().isCompareForOrder()) {
      if (valueToCheck->getChild(0)->getOpCode().is8Byte() || valueToCheck->getChild(1)->getOpCode().is8Byte()) {
        ...
        TR::TreeEvaluator::compareIntsForOrder(cmp2branch(valueToCheck->getOpCode().getOpCodeForReverseBranch(), cg),
          slowPathLabel, valueToCheck, cg, valueToCheck->getOpCode().isUnsignedCompare(), true,
      ...
      if (valueToCheck->getChild(0)->getOpCode().is8Byte() || valueToCheck->getChild(1)->getOpCode().is8Byte()) {
        ...
        TR::TreeEvaluator::compareLongsForEquality(
          cmp2branch(valueToCheck->getOpCode().getOpCodeForReverseBranch(), cg), slowPathLabel, valueToCheck, cg, true,
          PPCOpProp_BranchUnlikely);
      } else {
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp) // Use PPC AS branch hint.
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, PPCOpProp_BranchUnlikely, node, slowPathLabel, condReg);
    else
