...
class TR_PPCRegisterDependencyGroup {
 ...
 public:
  ...
  bool containsVirtualRegister(TR::Register *r, uint32_t numberOfRegisters) {
    for (int i = 0; i < numberOfRegisters; i++) {
      if (_dependencies[i].getRegister() == r) return true;
    }
    return (false);
  }
  ...
  void stopUsingDepRegs(uint32_t numberOfRegisters, int numRetReg,
    ...
    for (uint32_t i = 0; i < numberOfRegisters; i++) {
      ...
      for (int j = 0; j < numRetReg; j++)
        if (depReg == retReg[j]) found = true;
      if (!found) cg->stopUsingRegister(depReg);
    }
  }
...
class TR_PPCScratchRegisterDependencyConditions {
 public:
  ...
  static TR::RegisterDependencyConditions *createDependencyConditions(
