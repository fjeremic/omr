  ...
  void setExcludeGPR0(TR::Register* r, uint32_t numberOfRegisters)
  {
    for (uint32_t i = 0; i < numberOfRegisters; ++i) {
      if (_dependencies[i].getRegister() == r) {
        ...
        // Even if the virtual reg r is in the dependencies multiple times, it's sufficient to exclude gr0 on the first
        // of such dependencies because !gr0 NoReg dependencies are handled before NoReg dependencies, so we'll assign
        // it to !gr0 first and then any remaining dependencies become no-ops
        break;
      }
    }
  }
...
public:
  ...
  TR_PPCRegisterDependencyGroup* getPreConditions()
  {
  ...
  TR_PPCRegisterDependencyGroup* getPostConditions()
  {
...
public:
  ...
  uint32_t getNumberOfGPRDependencies()
  {
    return _numGPRDeps;
  }
  uint32_t getNumberOfCCRDependencies()
  {
    return _numCCRDeps;
  }
  uint32_t getNumberOfDependencies()
  {
    return _numGPRDeps + _numCCRDeps;
  }
    ...
    uint8_t flag = UsesDependentRegister)
  {
    ...
    TR_ASSERT(
      _numGPRDeps < TR::RealRegister::LastAssignableGPR - TR::RealRegister::FirstGPR + 1, "Too many GPR dependencies");
    TR_ASSERT(
      _numCCRDeps < TR::RealRegister::LastAssignableCCR - TR::RealRegister::FirstCCR + 1, "Too many CCR dependencies");
