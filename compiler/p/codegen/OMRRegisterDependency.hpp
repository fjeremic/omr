...
namespace OMR {
typedef OMR::Power::RegisterDependencyConditions
...
public:
  ...
  // Use TR_PPCRegisterDependencyGroup::create to allocate an object of this
  // type
  //
  ...
  static TR_PPCRegisterDependencyGroup *create(int32_t numDependencies,
                                               TR_Memory *m) {
  ...
  TR::Register *searchForRegister(TR::RealRegister::RegNum rr,
  ...
  void stopUsingDepRegs(uint32_t numberOfRegisters, int numRetReg,
                        TR::Register **retReg, TR::CodeGenerator *cg) {
    for (uint32_t i = 0; i < numberOfRegisters; i++) {
  ...
  void setExcludeGPR0(TR::Register *r, uint32_t numberOfRegisters) {
    for (uint32_t i = 0; i < numberOfRegisters; ++i) {
      if (_dependencies[i].getRegister() == r) {
        ...
        // Even if the virtual reg r is in the dependencies multiple times, it's
        // sufficient to exclude gr0 on the first of such dependencies because
        // !gr0 NoReg dependencies are handled before NoReg dependencies, so
        // we'll assign it to !gr0 first and then any remaining dependencies
        // become no-ops
        break;
      }
    }
...
public:
  ...
  RegisterDependencyConditions(uint16_t numPreConds, uint16_t numPostConds,
      ...
      : _preConditions(TR_PPCRegisterDependencyGroup::create(numPreConds, m)),
        ...
        _numPostConditions(numPostConds), _addCursorForPost(0) {}

  RegisterDependencyConditions(TR::CodeGenerator *cg, TR::Node *node,
                               uint32_t extranum,
                               TR::Instruction **cursorPtr = NULL);
  TR::RegisterDependencyConditions *
  clone(TR::CodeGenerator *cg, TR::RegisterDependencyConditions *added = NULL);
  TR::RegisterDependencyConditions *
  cloneAndFix(TR::CodeGenerator *cg,
              TR::RegisterDependencyConditions *added = NULL);
  ...
  void addPreCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
    ...
    TR_ASSERT(_addCursorForPre < _numPreConditions,
              " Pre Condition array bounds overflow");
  ...
  void addPostCondition(TR::Register *vr, TR::RealRegister::RegNum rr,
                        uint8_t flag = UsesDependentRegister) {
    TR_ASSERT(_addCursorForPost < _numPostConditions,
              " Post Condition array bounds overflow");
  ...
  TR::Register *searchPreConditionRegister(TR::RealRegister::RegNum rr) {
    return (_preConditions == NULL
                ? NULL
                : _preConditions->searchForRegister(rr, _addCursorForPre));
  }

  TR::Register *searchPostConditionRegister(TR::RealRegister::RegNum rr) {
    return (_postConditions == NULL
                ? NULL
                : _postConditions->searchForRegister(rr, _addCursorForPost));
  }

  bool preConditionContainsVirtual(TR::Register *r) {
    return (_preConditions == NULL
                ? false
                : _preConditions->containsVirtualRegister(r, _addCursorForPre));
  }

  bool postConditionContainsVirtual(TR::Register *r) {
    return (_postConditions == NULL ? false
                                    : _postConditions->containsVirtualRegister(
                                          r, _addCursorForPost));
  }
  ...
  void stopUsingDepRegs(TR::CodeGenerator *cg, int numRetReg,
                        TR::Register **retReg);
...
// This class allocates space for the maximum number of GPR and CCR dependencies
// that an evaluator can use and therefore frees you from trying to determine
// ahead of time how many dependencies you need. It can create a
// TR::RegisterDependencyConditions object with exactly the number of
// dependencies currently defined. Therefore, objects of this class should be
// stack allocated so that they'll automatically be freed when no longer needed.
//
...
public:
  ...
  TR_PPCScratchRegisterDependencyConditions()
      : _numGPRDeps(0), _excludeGPR0(0), _numCCRDeps(0) {}
  ...
  void addDependency(TR::CodeGenerator *cg, TR::Register *vr,
                     TR::RealRegister::RegNum rr, bool excludeGPR0 = false,
                     uint8_t flag = UsesDependentRegister) {
    TR_ASSERT(sizeof(_gprDeps) / sizeof(TR::RegisterDependency) <=
                  sizeof(_excludeGPR0) * 8,
              "Too many GPR dependencies, use a bit vector if more are needed");
    TR_ASSERT(_numGPRDeps < TR::RealRegister::LastAssignableGPR -
                                TR::RealRegister::FirstGPR + 1,
              "Too many GPR dependencies");
    TR_ASSERT(_numCCRDeps < TR::RealRegister::LastAssignableCCR -
                                TR::RealRegister::FirstCCR + 1,
              "Too many CCR dependencies");
    bool isGPR = rr >= TR::RealRegister::FirstGPR &&
                 rr <= TR::RealRegister::LastAssignableGPR;
    TR_ASSERT(!excludeGPR0 || isGPR,
              "Exclude gr0 doesn't make sense for CCR dependency");
    TR_ASSERT(isGPR || (rr >= TR::RealRegister::FirstCCR &&
                        rr <= TR::RealRegister::LastAssignableCCR),
    ...
    if (isGPR) {
      ...
      _excludeGPR0 = excludeGPR0 ? _excludeGPR0 | 1 << _numGPRDeps
                                 : _excludeGPR0 & ~(1 << _numGPRDeps);
      ++_numGPRDeps;
    } else {
  ...
  static TR::RegisterDependencyConditions *
  createDependencyConditions(TR::CodeGenerator *cg,
                             TR_PPCScratchRegisterDependencyConditions *pre,
                             TR_PPCScratchRegisterDependencyConditions *post);

private:
  uint32_t _numGPRDeps;
  TR::RegisterDependency _gprDeps[TR::RealRegister::LastAssignableGPR -
                                  TR::RealRegister::FirstGPR + 1];
  uint32_t _excludeGPR0;
  uint32_t _numCCRDeps;
  TR::RegisterDependency _ccrDeps[TR::RealRegister::LastAssignableCCR -
                                  TR::RealRegister::FirstCCR + 1];
};
