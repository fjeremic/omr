...
OMR::Power::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(), _stackPtrRegister(NULL), _constantData(NULL),
      _blockCallInfo(NULL), _transientLongRegisters(self()->trMemory()),
...
TR_RuntimeHelper
directToInterpreterHelper(TR::ResolvedMethodSymbol *methodSymbol,
                          TR::CodeGenerator *cg) {
#ifdef J9_PROJECT_SPECIFIC
...
bool OMR::Power::CodeGenerator::mulDecompositionCostIsJustified(
  ...
  case TR_PPCgpul:
  case TR_PPCgp:
  case TR_PPCgr: // 2 FXU out-of-order back-to-back 2 cycles. Mul is only 4 to 6
                 // cycles
...
TR::Instruction *
OMR::Power::CodeGenerator::generateGroupEndingNop(TR::Node *node,
                                                  TR::Instruction *preced) {
...
TR::Instruction *
OMR::Power::CodeGenerator::generateProbeNop(TR::Node *node,
                                            TR::Instruction *preced) {
    ...
    if (memRef != NULL) {
      ...
      case OMR::Instruction::IsSrc1: {
        ...
        if (inst->getSource1Register() == mrTargetReg) {
          if (performTransformation(
                  comp, "O^O PPC PEEPHOLE: Rewrite Src1 source register %p.\n",
                  current)) {
      ...
      case OMR::Instruction::IsTrg1Src2Imm: {
        ...
        if (inst->getSource1Register() == mrTargetReg) {
          if (performTransformation(comp,
                                    "O^O PPC PEEPHOLE: Rewrite Trg1Src2Imm 1st "
                                    "source register %p.\n",
                                    current)) {
            inst->setSource1Register(mrSourceReg);
            extendWindow = true;
        ...
        if (inst->getSource2Register() == mrTargetReg) {
          if (performTransformation(comp,
                                    "O^O PPC PEEPHOLE: Rewrite Trg1Src2Imm 2nd "
                                    "source register %p.\n",
                                    current)) {
            inst->setSource2Register(mrSourceReg);
            extendWindow = true;
...
static void recordFormPeephole(TR::CodeGenerator *cg,
  ...
  if (disableRecordFormPeephole != NULL)
    return;
...
static void syncPeephole(TR::CodeGenerator *cg,
  ...
  while (visited < window) {
    ...
    case TR::InstOpCode::isync: {
      if (nextOp == TR::InstOpCode::sync || nextOp == TR::InstOpCode::lwsync ||
...
int32_t OMR::Power::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(
    TR::Node *node) {
  return 32; // pickRegister will ensure that the number of free registers isn't
             // exceeded
}
    ...
    case TR_ClassAddress: {
      ...
      TR_RelocationRecordInformation *recordInfo =
          (TR_RelocationRecordInformation *)comp->trMemory()->allocateMemory(
              sizeof(TR_RelocationRecordInformation), heapAlloc);
      recordInfo->data1 =
          (uintptr_t)symRef->getSymbol()->getStaticSymbol()->getStaticAddress();
