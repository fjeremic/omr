...
void OMR::Power::CodeGenerator::itemTracking(
    int32_t offset,
...
{
    if (instr != NULL && !instr->willBePatched() && (!instr->isLabel() || (instr->getNode()->getOpCodeValue() == TR::BBStart && instr->getNode()->getBlock()->isExtensionOfPreviousBlock())) &&
...
static void mrPeepholes(TR::CodeGenerator* cg, TR::Instruction* mrInstruction)
{
    ...
    while (isWAWOrmrPeepholeCandidateInstr(current) && !isEBBTerminatingBranch(current) && windowSize < maxWindowSize) {
        ...
        if (current->getGCMap() && current->getGCMap()->getRegisterMap() & cg->registerBitMask(toRealRegister(mrTargetReg)->getRegisterNumber()))
            stackMaps.push_front(current->getGCMap());

        if (current->isBranchOp())
            inEBB = true;

        if (current->getOpCodeValue() == TR::InstOpCode::mr && current->getSourceRegister(0) == mrTargetReg && current->getTargetRegister(0) == mrSourceReg) {
        ...
        if (memRef != NULL) {
            ...
            if (memRef->isUsingDelayedIndexedForm() && (memRef->getBaseRegister() == mrSourceReg || memRef->getIndexRegister() == mrSourceReg || memRef->getBaseRegister() == mrTargetReg || memRef->getIndexRegister() == mrTargetReg))
                return;
            ...
            if (current->getKind() == OMR::Instruction::IsMemSrc1 && ((TR::PPCMemSrc1Instruction*)current)->getSourceRegister() == mrTargetReg) {
            ...
            case OMR::Instruction::IsTrg1Src1Imm: {
                ...
                if (inst->getSource1Register() == mrTargetReg) {
                    if (toRealRegister(mrSourceReg)->getRegisterNumber() == TR::RealRegister::gr0 && (current->getOpCodeValue() == TR::InstOpCode::addi || current->getOpCodeValue() == TR::InstOpCode::addi2 || current->getOpCodeValue() == TR::InstOpCode::addis))
                        all_mr_source_uses_rewritten = false;
            ...
            case OMR::Instruction::IsTrg1Src2: {
                if (current->getOpCodeValue() == TR::InstOpCode::xvmaddadp || current->getOpCodeValue() == TR::InstOpCode::xvmsubadp || current->getOpCodeValue() == TR::InstOpCode::xvnmsubadp)
                    return;
        ...
        if (current->defsRegister(toRealRegister(mrTargetReg))) {
            ...
            if (!inEBB && all_mr_source_uses_rewritten && performTransformation(comp, "O^O PPC PEEPHOLE: Remove mr %p when target redefined at %p.\n", mrInstruction, current)) {
...
static void recordFormPeephole(TR::CodeGenerator* cg, TR::Instruction* cmpiInstruction)
{
    ...
    if (cmpiInstruction->getSourceImmediate() != 0 || toRealRegister(cmpiTargetReg)->getRegisterNumber() != TR::RealRegister::cr0)
        return;

    TR::Instruction* current = cmpiInstruction->getPrev();
    while (current != NULL && !current->isLabel() && (!isEBBTerminatingBranch(current) || current->getKind() == OMR::Instruction::IsControlFlow) && current->getOpCodeValue() != TR::InstOpCode::wrtbar) {
        ...
        // if target reg is same as source of compare
        {
            ...
            if (current->getOpCode().hasRecordForm()) {
                ...
                if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.cpu.id() == TR_PPCgr) {
                    ...
                    if (opCode == TR::InstOpCode::addc || opCode == TR::InstOpCode::subfc || opCode == TR::InstOpCode::divw || opCode == TR::InstOpCode::divd)
                        return;
            ...
            if (current->defsRegister(toRealRegister(cmpiSourceReg)) || current->defsRegister(toRealRegister(cmpiTargetReg)) || current->getOpCode().isRecordForm())
                return;
        }
        current = current->getPrev();
    }
...
static void lhsPeephole(TR::CodeGenerator* cg, TR::Instruction* storeInstruction)
{
    ...
    if (!loadOp.isLoad() || storeMemRef->getLength() != loadMemRef->getLength() || !(storeOp.gprOp() == loadOp.gprOp() || storeOp.fprOp() == loadOp.fprOp()) || storeOp.isUpdate() || loadOp.isUpdate() || storeMemRef->getUnresolvedSnippet() || loadMemRef->getUnresolvedSnippet()) {
        return;
    }
    ...
    if (storeMemRef->getBaseRegister() != loadMemRef->getBaseRegister() || storeMemRef->getOffset(*comp) != loadMemRef->getOffset(*comp)) {
        return;
    }
...
static void trapPeephole(TR::CodeGenerator* cg, TR::Instruction* instructionCursor)
{
    ...
    if (disableTrapPeephole != NULL || instructionCursor->getSourceRegister(1) != NULL || instructionCursor->getOffset() != 0) {
        return;
    }
...
static void wawPeephole(TR::CodeGenerator* cg, TR::Instruction* instr)
{
    ...
    if (currTargetReg && currTargetReg->getKind() != TR_CCR && isWAWOrmrPeepholeCandidateInstr(instr) && !instr->isBranchOp() && !instr->isCall() && !instr->getOpCode().isRecordForm() && !instr->getOpCode().setsCarryFlag()) {
        TR::Instruction* current = instr->getNext();

        while (isWAWOrmrPeepholeCandidateInstr(current) && !current->isBranchOp() && !current->isCall() && window < maxWindowSize) {
            ...
            case OMR::Instruction::IsMemSrc1:
                if (currTargetReg == current->getSourceRegister(0) || currTargetReg == current->getSourceRegister(1) || currTargetReg == current->getSourceRegister(2))
                    return;
                break;
            default:
                return;
...
void OMR::Power::CodeGenerator::doBinaryEncoding()
{
    ...
    if (!self()->comp()->getOption(TR_DisableOOL)) {
        ...
        while (oiIterator != self()->getPPCOutOfLineCodeSectionList().end()) {
            ...
            bool needsETE = (*oiIterator)->getFirstInstruction()->getNode()->getOpCode().hasSymbolReference() && (*oiIterator)->getFirstInstruction()->getNode()->getSymbolReference() && (*oiIterator)->getFirstInstruction()->getNode()->getSymbolReference()->canCauseGC();
...
static int32_t identifyFarConditionalBranches(int32_t estimate, TR::CodeGenerator* cg)
{
    ...
    for (int32_t i = candidateBranches.lastIndex(); i >= 0; i--) {
        ...
        if (targetLocation >= myLocation) {
            for (j = i + 1; j < candidateBranches.size() && targetLocation > candidateBranches[j]->getEstimatedBinaryLocation();
                 j++)
                ;
...
inline static bool callInTree(TR::TreeTop* treeTop)
{
    ...
    if (l1OpCode == TR::monent || l1OpCode == TR::monexit || l1OpCode == TR::checkcast || l1OpCode == TR:: instanceof || l1OpCode == TR::ArrayStoreCHK || l1OpCode == TR::MethodEnterHook || l1OpCode == TR::MethodExitHook || l1OpCode == TR::New || l1OpCode == TR::newarray || l1OpCode == TR::anewarray || l1OpCode == TR::multianewarray || l1OpCode == TR::MergeNew)
        return (true);

    return (node->getNumChildren() != 0 && node->getFirstChild()->getOpCode().isCall() && node->getFirstChild()->getOpCodeValue() != TR::arraycopy);
}
...
void OMR::Power::CodeGenerator::simulateNodeEvaluation(TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if (immediateChild && immediateChild->getOpCode().isLoadConst() && (immediateChild->getType().isIntegral() && !immediateChild->getType().isInt64() || immediateChild->getType().isAddress())) {
    ...
    if (gprCandidate) {
        ...
        while (livec.hasMoreElements()) {
            ...
            for (prev = candidates->getFirst(); prev; prev = prev->getNext()) {
                bool gprCandidate = true;
                if ((prev->getSymbol()->getDataType() == TR::Float) || (prev->getSymbol()->getDataType() == TR::Double) || sym->getDataType().isVector())
                    gprCandidate = false;
...
int32_t OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
    if (node->getOpCode().isSwitch()) {
        ...
        if (node->getOpCodeValue() == TR::table) {
            ...
            if (total <= 9) {
                for (ii = 3; ii < total && (node->getChild(ii)->getCaseConstant() - node->getChild(ii - 1)->getCaseConstant()) <= UPPER_IMMED && (node->getChild(ii)->getCaseConstant() - node->getChild(ii - 1)->getCaseConstant()) > 0; /* in case of an overflow */
                     ii++)
                    ;
...
bool OMR::Power::CodeGenerator::isRotateAndMask(TR::Node* node)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL &&
        ...
        (secondOp == TR::iconst && contiguousBits(secondChild->getInt()) && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && ((firstOp == TR::imul && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && firstChild->getSecondChild()->getInt() > 0 && isNonNegativePowerOf2(firstChild->getSecondChild()->getInt())) || ((firstOp == TR::ishr || firstOp == TR::iushr) && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && firstChild->getSecondChild()->getInt() > 0 && (firstOp == TR::iushr || leadingZeroes(secondChild->getInt()) >= firstChild->getSecondChild()->getInt())))))
        return true;
    else
        return false;
}
...
bool OMR::Power::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode, TR::DataType dt)
{
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp8 && (opcode.getOpCodeValue() == TR::vadd || opcode.getOpCodeValue() == TR::vsub) && dt == TR::Int64)
        return true;
    ...
    TR::Instruction* secondInstruction)
{
    ...
    self()->getAheadOfTimeCompile()->getRelocationList().push_front(new (self()->trHeapMemory()) TR::PPCPairedRelocation(firstInstruction, secondInstruction, (uint8_t*)recordInfo,
        TR_AbsoluteMethodAddressOrderedPair, node));

    self()->addRelocation(new (self()->trHeapMemory()) TR::PPCPairedLabelAbsoluteRelocation(firstInstruction, secondInstruction, NULL, NULL, label));
}
...
{
    return !TR::Compiler->target.cpu.isTargetWithinIFormBranchRange(targetAddress, sourceAddress) || self()->comp()->getOption(TR_StressTrampolines);
}
