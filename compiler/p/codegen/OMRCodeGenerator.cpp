...
static int32_t identifyFarConditionalBranches(int32_t estimate,
                                              TR::CodeGenerator *cg);
...
OMR::Power::CodeGenerator::CodeGenerator()
    : OMR::CodeGenerator(),
      ...
      _outOfLineCodeSectionList(getTypedAllocator<TR_PPCOutOfLineCodeSection *>(
          self()->comp()->allocator())) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    self()->setTrackStatics(
        new (self()->trHeapMemory())
            TR_Array<TR::SymbolReference *>(self()->trMemory()));
    self()->setTrackItems(
        new (self()->trHeapMemory())
            TR_Array<TR_PPCLoadLabelItem *>(self()->trMemory()));
  } else {
    self()->setTrackStatics(NULL);
    self()->setTrackItems(NULL);
  }

  self()->setStackPointerRegister(self()->machine()->getRealRegister(
      _linkageProperties->getNormalStackPointerRegister()));
  self()->setMethodMetaDataRegister(self()->machine()->getRealRegister(
      _linkageProperties->getMethodMetaDataRegister()));
  self()->setTOCBaseRegister(self()->machine()->getRealRegister(
      _linkageProperties->getTOCBaseRegister()));
  self()->getLinkage()->initPPCRealRegisterLinkage();
  self()->getLinkage()->setParameterLinkageRegisterIndex(
  ...
  self()->setGlobalRegisterTable(
      _linkageProperties->getRegisterAllocationOrder());
  _numGPR = _linkageProperties->getNumAllocatableIntegerRegisters();
  _firstGPR = 0;
  _lastGPR = _numGPR - 1;
  _firstParmGPR =
      _linkageProperties->getFirstAllocatableIntegerArgumentRegister();
  _lastVolatileGPR =
      _linkageProperties->getLastAllocatableIntegerVolatileRegister();
  _numFPR = _linkageProperties->getNumAllocatableFloatRegisters();
  _firstFPR = _numGPR;
  _lastFPR = _firstFPR + _numFPR - 1;
  _firstParmFPR =
      _linkageProperties->getFirstAllocatableFloatArgumentRegister();
  _lastVolatileFPR =
      _linkageProperties->getLastAllocatableFloatVolatileRegister();
  ...
  if (TR::Compiler->target.is32Bit()) self()->setUsesRegisterPairsForLongs();
  ...
  if (accessStaticsIndirectly) self()->setAccessStaticsIndirectly(true);
  ...
  if (!debug("noLiveRegisters")) {
    ...
    self()->setLiveRegisters(
        new (self()->trHeapMemory()) TR_LiveRegisters(self()->comp()), TR_GPR);
    self()->setLiveRegisters(
        new (self()->trHeapMemory()) TR_LiveRegisters(self()->comp()), TR_FPR);
    self()->setLiveRegisters(
        new (self()->trHeapMemory()) TR_LiveRegisters(self()->comp()), TR_CCR);
    self()->setLiveRegisters(
        new (self()->trHeapMemory()) TR_LiveRegisters(self()->comp()), TR_VRF);
    self()->setLiveRegisters(new (self()->trHeapMemory())
                                 TR_LiveRegisters(self()->comp()),
                             TR_VSX_SCALAR);
    self()->setLiveRegisters(new (self()->trHeapMemory())
                                 TR_LiveRegisters(self()->comp()),
                             TR_VSX_VECTOR);
  ...
  if (TR::Compiler->target.cpu.getPPCSupportsVSX()) {
    ...
    static bool disablePPCTROTNoBreak =
        (feGetEnv("TR_disablePPCTROTNoBreak") != NULL);

    if (!disablePPCTRTO) self()->setSupportsArrayTranslateTRTO();
#ifndef __LITTLE_ENDIAN__
    else if (!disablePPCTRTO255)
      setSupportsArrayTranslateTRTO255();
#endif

    if (!disablePPCTROT) self()->setSupportsArrayTranslateTROT();

    if (!disablePPCTROTNoBreak) self()->setSupportsArrayTranslateTROTNoBreak();
  }
  ...
  if (TR::Compiler->target.is64Bit()) self()->setSupportsLoweringConstLDiv();
  self()->setSupportsLoweringConstLDivPower2();

  static bool disableDCAS = (feGetEnv("TR_DisablePPCDCAS") != NULL);

  if (!disableDCAS && TR::Compiler->target.is64Bit() &&
  ...
  // TODO: either improves the query to be size-based or gets the offset into a
  // register
  if (TR::Compiler->target.is64Bit()) self()->setSupportsAlignedAccessOnly();

  // TODO: distinguishing among OOO and non-OOO implementations
  if (TR::Compiler->target.isSMP()) self()->setEnforceStoreOrder();

  if (TR::Compiler->vm.hasResumableTrapHandler(self()->comp()))
    self()->setHasResumableTrapHandler();

  /*
   * TODO: TM is currently not compatible with read barriers. If read barriers
   * are required, TM is disabled until the issue is fixed.
   */
  if (TR::Compiler->target.cpu.getPPCSupportsTM() &&
      !self()->comp()->getOption(TR_DisableTM) &&
      TR::Compiler->om.readBarrierType() == gc_modron_readbar_none)
    self()->setSupportsTM();

  // enable LM if hardware supports instructions and running the reduced-pause
  // GC policy
  if (TR::Compiler->target.cpu.getPPCSupportsLM()) self()->setSupportsLM();

  if (TR::Compiler->target.cpu.getPPCSupportsVMX() &&
  ...
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    for (TR_GlobalRegisterNumber grn = 0;
         grn < self()->getNumberOfGlobalRegisters(); grn++) {
      ...
      if (self()->getProperties().getIntegerArgument(reg) ||
          self()->getProperties().getFloatArgument(reg))
  ...
  for (i = 0; i < linkageProperties.getNumIntArgRegs(); i++)
    _gprLinkageGlobalRegisterNumbers[i] =
        globalRegNumbers[linkageProperties.getIntegerArgumentRegister(i)];
  for (i = 0; i < linkageProperties.getNumFloatArgRegs(); i++)
    _fprLinkageGlobalRegisterNumbers[i] =
        globalRegNumbers[linkageProperties.getFloatArgumentRegister(i)];

  if (self()->comp()->getOptions()->getRegisterAssignmentTraceOption(
          TR_TraceRARegisterStates)) {
    self()->setGPRegisterIterator(
        new (self()->trHeapMemory())
            TR::RegisterIterator(self()->machine(), TR::RealRegister::FirstGPR,
                                 TR::RealRegister::LastGPR));
  ...
  TR_ResolvedMethod *method =
      self()->comp()->getJittedMethodSymbol()->getResolvedMethod();
  TR_ReturnInfo returnInfo =
      self()->getLinkage()->getReturnInfoFromReturnType(method->returnType());
  self()->comp()->setReturnInfo(returnInfo);
}

uintptrj_t *OMR::Power::CodeGenerator::getTOCBase() {
  TR_PPCTableOfConstants *pTOC = toPPCTableOfConstants(
      self()->comp()->getPersistentInfo()->getPersistentTOC());
...
OMR::Power::CodeGenerator::generateScratchRegisterManager(int32_t capacity) {
  return new (self()->trHeapMemory())
      TR_PPCScratchRegisterManager(capacity, self());
}
...
TR_RuntimeHelper directToInterpreterHelper(
    TR::ResolvedMethodSymbol *methodSymbol, TR::CodeGenerator *cg) {
#ifdef J9_PROJECT_SPECIFIC
...
OMR::Power::CodeGenerator::generateSwitchToInterpreterPrePrologue(
  ...
  TR::SymbolReference *revertToInterpreterSymRef =
      self()->symRefTab()->findOrCreateRuntimeHelper(
          TR_PPCrevertToInterpreterGlue, false, false, false);
  uintptrj_t ramMethod =
      (uintptrj_t)methodSymbol->getResolvedMethod()->resolvedMethodAddress();
  TR::SymbolReference *helperSymRef =
      self()->symRefTab()->findOrCreateRuntimeHelper(
          directToInterpreterHelper(methodSymbol, self()), false, false, false);
  ...
  cursor = new (self()->trHeapMemory())
      TR::PPCTrg1Instruction(TR::InstOpCode::mflr, node, gr0, cursor, self());
  cursor = self()->getLinkage()->flushArguments(cursor);
  cursor = generateDepImmSymInstruction(
      self(), TR::InstOpCode::bl, node,
      (uintptrj_t)revertToInterpreterSymRef->getMethodAddress(),
      new (self()->trHeapMemory())
          TR::RegisterDependencyConditions(0, 0, self()->trMemory()),
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    cursor = generateImmInstruction(self(), TR::InstOpCode::dd, node,
                                    (int32_t)ramMethod, TR_RamMethod, cursor);
  }
  ...
  if (TR::Compiler->target.is64Bit()) {
    int32_t highBits = (int32_t)(helperAddr >> 32),
            lowBits = (int32_t)helperAddr;
    cursor = generateImmInstruction(
        self(), TR::InstOpCode::dd, node,
        TR::Compiler->target.cpu.isBigEndian() ? highBits : lowBits,
    ...
    cursor = generateImmInstruction(
        self(), TR::InstOpCode::dd, node, (int32_t)helperAddr,
        TR_AbsoluteHelperAddress, helperSymRef, cursor);
  }
...
void OMR::Power::CodeGenerator::beginInstructionSelection() {
  ...
  if ((self()->comp()->getJittedMethodSymbol()->getLinkageConvention() ==
    ...
    _returnTypeInfoInstruction = new (self()->trHeapMemory())
        TR::PPCImmInstruction(TR::InstOpCode::dd, firstNode, 0, NULL, self());
  }
...
void OMR::Power::CodeGenerator::staticTracking(TR::SymbolReference *symRef) {
  if (TR::Compiler->target.is64Bit()) {
    ...
    for (idx = 0; idx < symRefArray->size(); idx++)
      if (symRefArray->element(idx) == symRef) return;
    symRefArray->add(symRef);
  }
}

void OMR::Power::CodeGenerator::itemTracking(int32_t offset,
                                             TR::LabelSymbol *label) {
  if (TR::Compiler->target.is64Bit()) {
    self()->getTrackItems()->add(new (self()->trHeapMemory())
                                     TR_PPCLoadLabelItem(offset, label));
  }
}

bool OMR::Power::CodeGenerator::mulDecompositionCostIsJustified(
    int numOfOperations, char bitPosition[], char operationType[],
    int64_t value) {
  if (numOfOperations <= 0) return false;

  // Notice: the total simple operations is (2*numOfOperations-1). The following
  // checking is a heuristic
  ...
  switch (TR::Compiler->target.cpu.id()) {
    ...
    case TR_PPCgpul:
    case TR_PPCgp:
    case TR_PPCgr:  // 2 FXU out-of-order back-to-back 2 cycles. Mul is only 4
                    // to 6 cycles
...
void OMR::Power::CodeGenerator::doRegisterAssignment(
  ...
  if (self()->getDebug()) self()->getDebug()->startTracingRegisterAssignment();
  ...
  while (instructionCursor) {
    ...
    if (instructionCursor->isLabel()) {
      TR::PPCLabelInstruction *li =
          (TR::PPCLabelInstruction *)instructionCursor;
...
TR::Instruction *OMR::Power::CodeGenerator::generateNop(TR::Node *n,
...
TR::Instruction *OMR::Power::CodeGenerator::generateGroupEndingNop(
...
TR::Instruction *OMR::Power::CodeGenerator::generateProbeNop(
    TR::Node *node, TR::Instruction *preced) {
...
static bool isWAWOrmrPeepholeCandidateInstr(TR::Instruction *instr) {
  if (instr != NULL && !instr->willBePatched() &&
      ...
      // FIXME: properly -- defs/refs of instructions with Mem are not quite
      // right.
...
static void mrPeepholes(TR::CodeGenerator *cg, TR::Instruction *mrInstruction) {
  ...
  TR::list<TR_GCStackMap *> stackMaps(
      getTypedAllocator<TR_GCStackMap *>(comp->allocator()));
  ...
  if (mrTargetReg == mrSourceReg) {
    if (performTransformation(
            comp, "O^O PPC PEEPHOLE: Remove redundant mr %p.\n", mrInstruction))
  ...
  while (isWAWOrmrPeepholeCandidateInstr(current) &&
         !isEBBTerminatingBranch(current) && windowSize < maxWindowSize) {
    // Keep track of any GC points we cross in order to fix them if we remove a
    // register move Note: Don't care about GC points in snippets, since we
    // won't remove a reg move if inEBB, i.e. if we've encountered a branch (to
    // a snippet or elsewhere)
    if (current->getGCMap() &&
        current->getGCMap()->getRegisterMap() &
            cg->registerBitMask(
                toRealRegister(mrTargetReg)->getRegisterNumber()))
      stackMaps.push_front(current->getGCMap());

    if (current->isBranchOp()) inEBB = true;

    if (current->getOpCodeValue() == TR::InstOpCode::mr &&
        current->getSourceRegister(0) == mrTargetReg &&
        current->getTargetRegister(0) == mrSourceReg) {
      if (performTransformation(
              comp, "O^O PPC PEEPHOLE: Remove mr copyback %p.\n", current)) {
        current->remove();
        extendWindow = true;
      } else
        return;
    ...
    if (memRef != NULL) {
      ...
      // We didn't try to test if base or index will be modified in
      // delayedIndexForm
      if (memRef->isUsingDelayedIndexedForm() &&
        ...
        return;
      if (current->isUpdate() && (memRef->getBaseRegister() == mrSourceReg ||
                                  memRef->getBaseRegister() == mrTargetReg))
        return;
      if (memRef->getBaseRegister() == mrTargetReg) {
        if (toRealRegister(mrSourceReg)->getRegisterNumber() ==
            TR::RealRegister::gr0)
          all_mr_source_uses_rewritten = false;
        else if (performTransformation(
                     comp, "O^O PPC PEEPHOLE: Rewrite base register %p.\n",
                     current)) {
      ...
      if (memRef->getIndexRegister() == mrTargetReg) {
        if (performTransformation(
                comp, "O^O PPC PEEPHOLE: Rewrite index register %p.\n",
                current)) {
      ...
      if (current->getKind() == OMR::Instruction::IsMemSrc1 &&
          ((TR::PPCMemSrc1Instruction *)current)->getSourceRegister() ==
              mrTargetReg) {
        if (performTransformation(
                comp, "O^O PPC PEEPHOLE: Rewrite store source register %p.\n",
                current)) {
          ((TR::PPCMemSrc1Instruction *)current)
              ->setSourceRegister(mrSourceReg);
          extendWindow = true;
        } else
          return;
      }
      ...
      switch (current->getKind()) {
        case OMR::Instruction::IsSrc1: {
          ...
          if (inst->getSource1Register() == mrTargetReg) {
            if (performTransformation(
                    comp,
                    "O^O PPC PEEPHOLE: Rewrite Src1 source register %p.\n",
                    current)) {
        ...
        case OMR::Instruction::IsTrg1Src1: {
          TR::PPCTrg1Src1Instruction *inst =
              (TR::PPCTrg1Src1Instruction *)current;
          if (inst->getSource1Register() == mrTargetReg) {
            if (performTransformation(
                    comp,
                    "O^O PPC PEEPHOLE: Rewrite Trg1Src1 source register %p.\n",
                    current)) {
              inst->setSource1Register(mrSourceReg);
              extendWindow = true;
            } else
        ...
        case OMR::Instruction::IsTrg1Src1Imm: {
          TR::PPCTrg1Src1ImmInstruction *inst =
              (TR::PPCTrg1Src1ImmInstruction *)current;
          if (inst->getSource1Register() == mrTargetReg) {
            if (toRealRegister(mrSourceReg)->getRegisterNumber() ==
                    TR::RealRegister::gr0 &&
              ...
              all_mr_source_uses_rewritten = false;
                                           ...
                                           "O^O PPC PEEPHOLE: Rewrite "
                                           "Trg1Src1Imm source register %p.\n",
                                           current)) {
              inst->setSource1Register(mrSourceReg);
              extendWindow = true;
            } else
        ...
        case OMR::Instruction::IsTrg1Src1Imm2: {
          TR::PPCTrg1Src1Imm2Instruction *inst =
              (TR::PPCTrg1Src1Imm2Instruction *)current;
          if (inst->getSource1Register() == mrTargetReg) {
            if (performTransformation(comp,
                                      "O^O PPC PEEPHOLE: Rewrite Trg1Src1Imm2 "
                                      "source register %p.\n",
                                      current)) {
              inst->setSource1Register(mrSourceReg);
              extendWindow = true;
        ...
        case OMR::Instruction::IsSrc2: {
          ...
          if (inst->getSource1Register() == mrTargetReg) {
            if (performTransformation(
                    comp,
                    "O^O PPC PEEPHOLE: Rewrite Src2 1st source register %p.\n",
                    current)) {
              inst->setSource1Register(mrSourceReg);
              extendWindow = true;
            } else
          ...
          if (inst->getSource2Register() == mrTargetReg) {
            if (performTransformation(
                    comp,
                    "O^O PPC PEEPHOLE: Rewrite Src2 2nd source register %p.\n",
                    current)) {
        ...
        case OMR::Instruction::IsTrg1Src2: {
          ...
          TR::PPCTrg1Src2Instruction *inst =
              (TR::PPCTrg1Src2Instruction *)current;
          if (inst->getSource1Register() == mrTargetReg) {
            if (performTransformation(comp,
                                      "O^O PPC PEEPHOLE: Rewrite Trg1Src2 1st "
                                      "source register %p.\n",
                                      current)) {
              inst->setSource1Register(mrSourceReg);
              extendWindow = true;
          ...
          if (inst->getSource2Register() == mrTargetReg) {
            if (performTransformation(comp,
                                      "O^O PPC PEEPHOLE: Rewrite Trg1Src2 2nd "
                                      "source register %p.\n",
                                      current)) {
              inst->setSource2Register(mrSourceReg);
              extendWindow = true;
        ...
        case OMR::Instruction::IsTrg1Src3: {
          TR::PPCTrg1Src3Instruction *inst =
              (TR::PPCTrg1Src3Instruction *)current;
          if (inst->getSource1Register() == mrTargetReg) {
            if (performTransformation(comp,
                                      "O^O PPC PEEPHOLE: Rewrite Trg1Src3 1st "
                                      "source register %p.\n",
                                      current)) {
              inst->setSource1Register(mrSourceReg);
              extendWindow = true;
          ...
          if (inst->getSource2Register() == mrTargetReg) {
            if (performTransformation(comp,
                                      "O^O PPC PEEPHOLE: Rewrite Trg1Src3 2nd "
                                      "source register %p.\n",
                                      current)) {
              inst->setSource2Register(mrSourceReg);
              extendWindow = true;
          ...
          if (inst->getSource3Register() == mrTargetReg) {
            if (performTransformation(comp,
                                      "O^O PPC PEEPHOLE: Rewrite Trg1Src3 3nd "
                                      "source register %p.\n",
                                      current)) {
        ...
        case OMR::Instruction::IsTrg1Src2Imm: {
          TR::PPCTrg1Src2ImmInstruction *inst =
              (TR::PPCTrg1Src2ImmInstruction *)current;
          if (inst->getSource1Register() == mrTargetReg) {
            if (performTransformation(comp,
                                      "O^O PPC PEEPHOLE: Rewrite Trg1Src2Imm "
                                      "1st source register %p.\n",
                                      current)) {
              inst->setSource1Register(mrSourceReg);
              extendWindow = true;
            } else
    ...
    if (current->defsRegister(toRealRegister(mrTargetReg))) {
      ...
      if (!inEBB && all_mr_source_uses_rewritten &&
        ...
        if (!stackMaps.empty()) {
          for (auto stackMapIter = stackMaps.begin();
               stackMapIter != stackMaps.end(); ++stackMapIter) {
            if (cg->getDebug())
              if (comp->getOption(TR_TraceCG))
                traceMsg(comp,
                         "Adjusting register map %p; removing %s, adding %s "
                         "due to removal of mr %p\n",
                         *stackMapIter, cg->getDebug()->getName(mrTargetReg),
                         cg->getDebug()->getName(mrSourceReg), mrInstruction);
            (*stackMapIter)
                ->resetRegistersBits(cg->registerBitMask(
                    toRealRegister(mrTargetReg)->getRegisterNumber()));
            (*stackMapIter)
                ->setRegisterBits(cg->registerBitMask(
                    toRealRegister(mrSourceReg)->getRegisterNumber()));
          }
        }
    ...
    if (current->defsRegister(toRealRegister(mrSourceReg))) return;
...
static void recordFormPeephole(TR::CodeGenerator *cg,
                               TR::Instruction *cmpiInstruction) {
  TR::Compilation *comp = cg->comp();
  static char *disableRecordFormPeephole =
      feGetEnv("TR_DisableRecordFormPeephole");
  ...
  if (cmpiInstruction->getSourceImmediate() != 0 ||
      toRealRegister(cmpiTargetReg)->getRegisterNumber() !=
          TR::RealRegister::cr0)
    return;
  ...
  while (current != NULL && !current->isLabel() &&
    ...
    // if target reg is same as source of compare
    {
      if (current->getOpCode().isRecordForm()) {
        if (performTransformation(
                comp,
                "O^O PPC PEEPHOLE: Remove redundant compare immediate %p.\n",
                cmpiInstruction))
      ...
      if (current->getOpCode().hasRecordForm()) {
        ...
        if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
          ...
          if (opCode == TR::InstOpCode::addc ||
              opCode == TR::InstOpCode::subfc ||
          ...
          if (opCode == TR::InstOpCode::rlwinm) {
            ...
            if (inst->getSourceImmediate() == 0 &&
              ...
              if (performTransformation(
                      comp,
                      "O^O PPC PEEPHOLE: Change %p to andi_r, remove compare "
                      "immediate %p.\n",
                      current, cmpiInstruction)) {
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::andi_r, inst->getNode(),
                    inst->getPrimaryTargetRegister(),
                    inst->getSourceRegister(0), cmpiTargetReg, inst->getMask(),
                    current);
                current->remove();
                cmpiInstruction->remove();
              }
              ...
              if (performTransformation(
                      comp,
                      "O^O PPC PEEPHOLE: Change %p to andis_r, remove compare "
                      "immediate %p.\n",
                      current, cmpiInstruction)) {
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::andis_r, inst->getNode(),
                    inst->getPrimaryTargetRegister(),
                    inst->getSourceRegister(0), cmpiTargetReg,
        ...
        if (performTransformation(comp,
                                  "O^O PPC PEEPHOLE: Change %p to record form, "
                                  "remove compare immediate %p.\n",
                                  current, cmpiInstruction)) {
...
static void lhsPeephole(TR::CodeGenerator *cg,
                        TR::Instruction *storeInstruction) {
  ...
  TR::MemoryReference *storeMemRef =
      ((TR::PPCMemSrc1Instruction *)storeInstruction)->getMemoryReference();
  TR::MemoryReference *loadMemRef =
      ((TR::PPCTrg1MemInstruction *)loadInstruction)->getMemoryReference();
  ...
  // TODO: Relax the first constraint a bit and handle cases were unrelated
  // instructions appear between the store and the load
  if (!loadOp.isLoad() || storeMemRef->getLength() != loadMemRef->getLength() ||
      !(storeOp.gprOp() == loadOp.gprOp() ||
        storeOp.fprOp() == loadOp.fprOp()) ||
      storeOp.isUpdate() || loadOp.isUpdate() ||
  ...
  // The store and load mem refs have to agree on whether or not they need
  // delayed offsets
  if (storeMemRef->hasDelayedOffset() != loadMemRef->hasDelayedOffset()) {
    // Memory references that use delayed offsets will not have the correct
    // offset at this point, so we can't compare their offsets to the offsets
    // used by regular references, however we can still compare two mem refs
    // that both use delayed offsets.
    return;
  }
  ...
  TR::Register *srcReg =
      ((TR::PPCMemSrc1Instruction *)storeInstruction)->getSourceRegister();
  TR::Register *trgReg =
      ((TR::PPCTrg1MemInstruction *)loadInstruction)->getTargetRegister();
    ...
    if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
      if (performTransformation(
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, loadInstruction->getNode(), trgReg,
            srcReg, 0, 0xffffffff, storeInstruction);
        loadInstruction->remove();
      }
    } else if (performTransformation(comp,
                                     "O^O PPC PEEPHOLE: Remove redundant "
                                     "load " POINTER_PRINTF_FORMAT
  ...
  if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
    if (performTransformation(
            ...
            " after store " POINTER_PRINTF_FORMAT " with rlwinm.\n",
            loadInstruction, storeInstruction)) {
      generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm,
                                      loadInstruction->getNode(), trgReg,
                                      srcReg, 0, 0xffffffff, storeInstruction);
      loadInstruction->remove();
    }
  } else if (performTransformation(comp,
                                   "O^O PPC PEEPHOLE: Replace redundant "
                                   "load " POINTER_PRINTF_FORMAT
                                   " after store " POINTER_PRINTF_FORMAT
                                   " with mr.\n",
                                   loadInstruction, storeInstruction)) {
    generateTrg1Src1Instruction(cg, TR::InstOpCode::mr,
                                loadInstruction->getNode(), trgReg, srcReg,
                                storeInstruction);
    loadInstruction->remove();
  }
}

static void syncPeephole(TR::CodeGenerator *cg,
                         TR::Instruction *instructionCursor, int window) {
  TR::Compilation *comp = cg->comp();
  TR::Instruction *first = instructionCursor;
  TR::InstOpCode::Mnemonic nextOp =
      instructionCursor->getNext()->getOpCodeValue();
  ...
  while (visited < window) {
    ...
    if (instructionCursor->getNext()->isSyncSideEffectFree()) {
      for (; visited < window &&
             instructionCursor->getNext()->isSyncSideEffectFree();
           visited++) {
    ...
    switch (first->getOpCodeValue()) {
      case TR::InstOpCode::isync: {
        if (nextOp == TR::InstOpCode::sync ||
            nextOp == TR::InstOpCode::lwsync || nextOp == TR::InstOpCode::isync)
          removeFirst = true;
        break;
      }
      case TR::InstOpCode::lwsync: {
        if (nextOp == TR::InstOpCode::sync)
          removeFirst = true;
        else if (nextOp == TR::InstOpCode::lwsync ||
                 nextOp == TR::InstOpCode::isync)
          removeNext = true;
        break;
      }
    ...
    if (removeFirst) {
      ...
      if (performTransformation(comp,
        ...
        cg->generateNop(instructionCursor->getNext()->getNode(),
                        instructionCursor, TR_NOPStandard);
...
static void trapPeephole(TR::CodeGenerator *cg,
  ...
  if (disableTrapPeephole != NULL ||
      instructionCursor->getSourceRegister(1) != NULL ||
  ...
  for (int i = 0; i < WINDOW && next != NULL; i++) {
    ...
    if (NULL != next->getMemoryReference() &&
      ...
      return;
    if (NULL != next->getGCMap()) return;

    for (int j = 0; next->getTargetRegister(j) != NULL; j++) {
      if (next->getTargetRegister(j)->getRealRegister() == RAr) return;
    }
    if (next->isLoad() && next->getSourceRegister(0) != NULL &&
        next->getSourceRegister(0)->getRealRegister() == RAr) {
      if (next->getSourceRegister(1) != NULL) return;

      // Page 0 which is readable on AIX is 4096 bytes long, and the largest
      // possible load is 16 bytes.
      if (next->getOffset() >= (4096 - 16) || next->getOffset() < 0) return;

      TR::Instruction *trap = instructionCursor;
      TR::Instruction *load = next;

      if (trap->getPrev() != NULL && load->getNext() != NULL) {
        for (TR::Instruction *prev = load->getPrev(); prev != trap->getPrev();
             prev = prev->getPrev()) {
...
static void wawPeephole(TR::CodeGenerator *cg, TR::Instruction *instr) {
  ...
  if (disableWAWPeephole) return;

  // Get and check if there is a target register
  TR::Register *currTargetReg = instr->getTargetRegister(0);

  if (currTargetReg && currTargetReg->getKind() != TR_CCR &&
      isWAWOrmrPeepholeCandidateInstr(instr) && !instr->isBranchOp() &&
      !instr->isCall() && !instr->getOpCode().isRecordForm() &&
      !instr->getOpCode().setsCarryFlag()) {
    TR::Instruction *current = instr->getNext();

    while (isWAWOrmrPeepholeCandidateInstr(current) && !current->isBranchOp() &&
           !current->isCall() && window < maxWindowSize) {
      if (current->getOpCode().usesTarget()) return;
      switch (current->getKind()) {
        case OMR::Instruction::IsSrc1:
        case OMR::Instruction::IsTrg1Src1:
        case OMR::Instruction::IsTrg1Src1Imm:
        case OMR::Instruction::IsTrg1Src1Imm2:
          if (currTargetReg == current->getSourceRegister(0)) return;
          break;
        case OMR::Instruction::IsSrc2:
        case OMR::Instruction::IsTrg1Src2:
        case OMR::Instruction::IsTrg1Src2Imm:
      ...
      if (current->getTargetRegister(0) == currTargetReg) {
        ...
        if (isConstData) {
          if (performTransformation(comp,
                                    "O^O PPC PEEPHOLE: Remove dead instrcution "
                                    "group from WAW %p -> %p.\n",
                                    instr, current)) {
...
bool OMR::Power::CodeGenerator::checkAndFetchRequestor(TR::Instruction *instr,
                                                       TR::Instruction **q) {
  if (_constantData) {
...
void OMR::Power::CodeGenerator::doPeephole() {
  ...
  if (disablePeephole) return;

  int syncPeepholeWindow = self()->comp()->isOptServer() ? 12 : 6;

  if (self()->comp()->getOptLevel() == noOpt) return;
...
bool OMR::Power::CodeGenerator::supportsAESInstructions() {
  if (TR::Compiler->target.cpu.getPPCSupportsAES() &&
...
TR::Linkage *OMR::Power::CodeGenerator::createLinkage(
    TR_LinkageConventions lc) {
...
void OMR::Power::CodeGenerator::expandInstructions() {
  for (TR::Instruction *instr = self()->getFirstInstruction(); instr;
       instr = instr->getNext()) {
...
void OMR::Power::CodeGenerator::doBinaryEncoding() {
  ...
  while (data.cursorInstruction) {
    if (data.cursorInstruction->getOpCodeValue() == TR::InstOpCode::ret) {
      if (skipOneReturn == false) {
        ...
        if (temp->getNext() != data.cursorInstruction) skipOneReturn = true;
  ...
  while (data.cursorInstruction) {
    ...
    if (data.cursorInstruction == data.preProcInstruction) {
      self()->setPrePrologueSize(self()->getBinaryBufferCursor() -
                                 self()->getBinaryBufferStart() -
    ...
    if (isPrivateLinkage && data.cursorInstruction == data.jitTojitStart) {
      uint32_t magicWord =
          ((self()->getBinaryBufferCursor() - self()->getCodeStart()) << 16) |
          static_cast<uint32_t>(self()->comp()->getReturnInfo());
      ...
      *(uint32_t *)(data.preProcInstruction->getBinaryEncoding()) = magicWord;
      ...
      toPPCImmInstruction(data.preProcInstruction)
          ->setSourceImmediate(
              *(uint32_t *)(data.preProcInstruction->getBinaryEncoding()));
    }

    self()->getLinkage()->performPostBinaryEncoding();
  }

  // We late-processing TOC entries here: obviously cannot deal with snippet
  // labels. If needed, we should move this step to common code around
  // relocation processing.
  if (TR::Compiler->target.is64Bit()) {
    int32_t idx;
    for (idx = 0; idx < self()->getTrackItems()->size(); idx++)
      TR_PPCTableOfConstants::setTOCSlot(
          self()->getTrackItems()->element(idx)->getTOCOffset(),
          (uintptrj_t)self()
              ->getTrackItems()
              ->element(idx)
              ->getLabel()
...
TR::Register *OMR::Power::CodeGenerator::gprClobberEvaluate(TR::Node *node) {
  ...
  if (!self()->canClobberNodesRegister(node)) {
    if (TR::Compiler->target.is32Bit() && node->getType().isInt64()) {
      ...
      generateTrg1Src1Instruction(self(), TR::InstOpCode::mr, node, lowReg,
                                  resultReg->getLowOrder());

      generateTrg1Src1Instruction(self(), TR::InstOpCode::mr, node, highReg,
                                  resultReg->getHighOrder());
      return longReg;
    } else {
      TR::Register *targetReg =
          resultReg->containsCollectedReference()
      ...
      generateTrg1Src1Instruction(self(),
                                  resultReg->getKind() == TR_GPR
                                      ? TR::InstOpCode::mr
                                      : TR::InstOpCode::fmr,
                                  node, targetReg, resultReg);
      return targetReg;
    }
  } else {
    return resultReg;
...
static int32_t identifyFarConditionalBranches(int32_t estimate,
                                              TR::CodeGenerator *cg) {
  TR_Array<TR::PPCConditionalBranchInstruction *> candidateBranches(
      cg->trMemory(), 256);
  TR::Instruction *cursorInstruction = cg->getFirstInstruction();

  while (cursorInstruction) {
    TR::PPCConditionalBranchInstruction *branch =
        cursorInstruction->getPPCConditionalBranchInstruction();
    if (branch != NULL) {
      if (abs(branch->getEstimatedBinaryLocation() -
              branch->getLabelSymbol()->getEstimatedCodeLocation()) > 16384) {
  ...
  for (int32_t i = candidateBranches.lastIndex(); i >= 0; i--) {
    ...
    int32_t targetLocation =
        candidateBranches[i]->getLabelSymbol()->getEstimatedCodeLocation();
    int32_t j;

    if (targetLocation >= myLocation) {
      for (j = i + 1;
           j < candidateBranches.size() &&
      ...
      // We could improve this by only adding 4 when the target is a Interface
      // Call Snippet
      if (TR::Compiler->target.is64Bit()) targetLocation += 4;
...
void OMR::Power::CodeGenerator::buildRegisterMapForInstruction(
    TR_GCStackMap *map) {
  ...
  //
  for (int i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR;
       i++) {
      ...
      if (virtReg) {
        if (virtReg->containsInternalPointer()) {
          ...
          internalPtrMap->addInternalPointerPair(
              virtReg->getPinningArrayPointer(), 31 - (i - 1));
...
int32_t OMR::Power::CodeGenerator::findOrCreateFloatConstant(
    void *v, TR::DataType t, TR::Instruction *n0, TR::Instruction *n1,
    TR::Instruction *n2, TR::Instruction *n3) {
  if (_constantData == NULL)
    _constantData =
        new (self()->trHeapMemory()) TR::ConstantDataSnippet(self());
  return (_constantData->addConstantRequest(v, t, n0, n1, n2, n3, NULL, false));
}

int32_t OMR::Power::CodeGenerator::findOrCreateAddressConstant(
    void *v, TR::DataType t, TR::Instruction *n0, TR::Instruction *n1,
    TR::Instruction *n2, TR::Instruction *n3, TR::Node *node,
    bool isUnloadablePicSite) {
  if (_constantData == NULL)
    _constantData =
        new (self()->trHeapMemory()) TR::ConstantDataSnippet(self());
  return (_constantData->addConstantRequest(v, t, n0, n1, n2, n3, node,
                                            isUnloadablePicSite));
}

bool OMR::Power::CodeGenerator::isSnippetMatched(TR::Snippet *snippet,
                                                 int32_t snippetKind,
                                                 TR::SymbolReference *symRef) {
  if ((int32_t)(snippet->getKind()) != snippetKind) return false;

  switch (snippetKind) {
    case TR::Snippet::IsHelperCall: {
      TR::PPCHelperCallSnippet *helperSnippet =
          (TR::PPCHelperCallSnippet *)snippet;
      if (helperSnippet->getRestartLabel() == NULL &&
          helperSnippet->getDestination() == symRef)
        return true;
      return false;
    }
...
int32_t OMR::Power::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
...
inline static bool callInTree(TR::TreeTop *treeTop) {
  ...
  if (l1OpCode == TR::treetop || l1OpCode == TR::ificmpeq ||
      l1OpCode == TR::ificmpne)
    l1OpCode = node->getFirstChild()->getOpCodeValue();

  if (l1OpCode == TR::monent || l1OpCode == TR::monexit ||
          l1OpCode == TR::checkcast || l1OpCode == TR::
          instanceof
      || l1OpCode == TR::ArrayStoreCHK || l1OpCode == TR::MethodEnterHook ||
          l1OpCode == TR::MethodExitHook || l1OpCode == TR::New ||
          l1OpCode == TR::newarray || l1OpCode == TR::anewarray ||
          l1OpCode == TR::multianewarray || l1OpCode == TR::MergeNew)
    return (true);
...
TR_BitVector *OMR::Power::CodeGenerator::computeCallInfoBitVector() {
  ...
  TR_BitVector bvec, *ebvec = new (self()->trHeapMemory())
                         TR_BitVector(bcount, self()->trMemory());
  ...
  for (pTree = self()->comp()->getStartTree(); pTree != NULL;
    ...
    TR_ASSERT(bnum < bcount,
              "Block index must be less than the total number of blocks.\n");
  ...
  for (pTree = self()->comp()->getStartTree(); pTree != NULL;
    ...
    while (!bvec.isSet(btemp)) {
      ...
      if (eblock == NULL || !eblock->isExtensionOfPreviousBlock()) break;
      btemp = eblock->getNumber();
    }
    if (bvec.isSet(btemp)) ebvec->set(bnum);
  }
...
void OMR::Power::CodeGenerator::simulateNodeEvaluation(
  ...
  TR::Node *immediateChild =
      (node->getNumChildren() == 2 && !node->getOpCode().isCall() &&
       !node->getOpCode().isLoad())
          ? node->getSecondChild()
          : NULL;
  if (immediateChild && immediateChild->getOpCode().isLoadConst() &&
      (immediateChild->getType().isIntegral() &&
           !immediateChild->getType().isInt64() ||
...
TR_GlobalRegisterNumber OMR::Power::CodeGenerator::pickRegister(
    TR_RegisterCandidate *regCan, TR::Block **barr, TR_BitVector &availRegs,
    TR_GlobalRegisterNumber &highRegisterNumber,
    TR_LinkHead<TR_RegisterCandidate> *candidates) {
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation))
    return OMR::CodeGenerator::pickRegister(regCan, barr, availRegs,
                                            highRegisterNumber, candidates);
  ...
  bool hasCallInPath = false, isParm = sym->isParm(), isAddressType = false,
       isFloatType = false;
  bool isVector = false;
  ...
  if (gprCandidate) {
    ...
    if (!_assignedGlobalRegisters)
      _assignedGlobalRegisters = new (self()->trStackMemory())
          TR_BitVector(self()->comp()->getSymRefCount(), self()->trMemory(),
                       stackAlloc, growable);
    ...
    while (livec.hasMoreElements()) {
      ...
      for (prev = candidates->getFirst(); prev; prev = prev->getNext()) {
        ...
        if (gprCandidate && prev->getBlocksLiveOnEntry().get(liveBlockNum)) {
          ...
          if (prev->getDataType() == TR::Int64) numAssignedGlobalRegs++;
          _assignedGlobalRegisters->set(
              prev->getSymbolReference()->getReferenceNumber());
        }
      }
      maxRegisterPressure = self()->estimateRegisterPressure(
          block, visitCount, maxStaticFrequency, maxFrequency, vmThreadUsed,
          numAssignedGlobalRegs, _assignedGlobalRegisters,
          regCan->getSymbolReference(), assigningEDX);
      if (maxRegisterPressure >= _numGPR) break;
    }

    // Determine if we can spare any extra registers for this candidate without
    // spilling in any hot (critical) blocks
    if (maxRegisterPressure < _numGPR)
      numExtraRegs = _numGPR - maxRegisterPressure;

    if (numExtraRegs <= 0) return -1;
  }
  ...
  if (hasCallInPath) {
    i1 = lastVolIndex + 1;
    while (i1 <= lastIndex && !availRegs.isSet(i1)) i1++;
    if (i1 <= lastIndex) return (i1);
    if (isParm) {
      i1 = sym->getParmSymbol()->getLinkageRegisterIndex();
      if (i1 >= 0) {
        if (isVector) TR_ASSERT(false, "assertion failure");  // TODO
        i1 = (isFloatType ? _firstParmFPR : _firstParmGPR) - i1 - longLow;
        if (availRegs.isSet(i1)) return (i1);
      }
    } else {
      i1 = firstIndex;
      while (i1 <= lastVolIndex && !availRegs.isSet(i1)) i1++;
...
bool OMR::Power::CodeGenerator::allowGlobalRegisterAcrossBranch(
...
int32_t OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
    TR::Node *node) {
  if (node->getOpCode().isSwitch()) {
    ...
    if (node->getOpCodeValue() == TR::table) {
      // Have to be conservative here since we don't know whether it is
      // balanced.
      ...
      if (total <= 12) {
        for (ii = 2; ii < total &&
                     node->getChild(ii)->getCaseConstant() >= LOWER_IMMED &&
                     node->getChild(ii)->getCaseConstant() <= UPPER_IMMED;
             ii++)
          ;
        if (ii == total) return _numGPR - 1;
      }
      ...
      if (total <= 9) {
        for (ii = 3;
             ...
             (node->getChild(ii)->getCaseConstant() -
              node->getChild(ii - 1)->getCaseConstant()) <= UPPER_IMMED &&
             (node->getChild(ii)->getCaseConstant() -
              node->getChild(ii - 1)->getCaseConstant()) >
                 0; /* in case of an overflow */
             ii++)
          ;

        if (ii == total) return _numGPR - 2;
      }

      if (total <= 8) return _numGPR - 3;
      return _numGPR -
             6;  // Be conservative when we don't know whether it is balanced.
    }
  } else if (node->getOpCode().isIf() &&
             node->getFirstChild()->getType().isInt64()) {
    return (TR::Compiler->target.is64Bit()
                ? _numGPR
                : _numGPR - 4);  // 4 GPRs are needed for long-compare arguments
                                 // themselves.
  }

  return _numGPR;  // pickRegister will ensure that the number of free registers
                   // isn't exceeded
}

int32_t OMR::Power::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(
    TR::Node *node) {
  return 32;  // pickRegister will ensure that the number of free registers
              // isn't exceeded
}

TR_GlobalRegisterNumber
OMR::Power::CodeGenerator::getLinkageGlobalRegisterNumber(
    int8_t linkageRegisterIndex, TR::DataType type) {
  TR_GlobalRegisterNumber result;
  if (type == TR::Float || type == TR::Double) {
    if (linkageRegisterIndex >=
        self()->getProperties()._numFloatArgumentRegisters)
      result = -1;
    else
    ...
    if (linkageRegisterIndex >=
        self()->getProperties()._numIntegerArgumentRegisters)
      result = -1;
    else
      result = _gprLinkageGlobalRegisterNumbers[linkageRegisterIndex];
  }
  diagnostic("getLinkageGlobalRegisterNumber(%d, %s) = %d\n",
             linkageRegisterIndex, self()->comp()->getDebug()->getName(type),
             result);
  return result;
}

void OMR::Power::CodeGenerator::apply16BitLabelRelativeRelocation(
    int32_t *cursor, TR::LabelSymbol *label) {
  TR_ASSERT(label->getCodeLocation(),
            "Attempt to relocate to a NULL label address!");

  *cursor |=
      ((uintptrj_t)label->getCodeLocation() - (uintptrj_t)cursor) & 0x0000fffc;
}

void OMR::Power::CodeGenerator::apply24BitLabelRelativeRelocation(
    int32_t *cursor, TR::LabelSymbol *label) {
  TR_ASSERT(label->getCodeLocation(),
            "Attempt to relocate to a NULL label address!");

  *cursor = ((*cursor) & 0xfc000003) |
            (((uintptrj_t)label->getCodeLocation() - (uintptrj_t)cursor) &
             0x03fffffc);
}

void OMR::Power::CodeGenerator::apply16BitLoadLabelRelativeRelocation(
    TR::Instruction *liInstruction, TR::LabelSymbol *startLabel,
    TR::LabelSymbol *endLabel, int32_t deltaToStartLabel) {
  // Patch up the mainline code that stores an offset value into a register
  // using: li   addrReg, Value & 0x0000ffff

  TR::Instruction *liPPCInstruction = liInstruction;
  TR_ASSERT(liPPCInstruction->getOpCodeValue() == TR::InstOpCode::li,
            "wrong load instruction used for "
            "apply32BitLoadLabelRelativeRelocation\n");

  int32_t distance = int32_t(
      (endLabel->getCodeLocation() - startLabel->getCodeLocation()) / 4);
  TR_ASSERT(
      !((endLabel->getCodeLocation() - startLabel->getCodeLocation()) % 4),
      "misaligned code");
  TR_ASSERT(!(distance & 0xffff0000),
            "relative label does not fit in 16-bit li instruction");

  int32_t *patchAddr = (int32_t *)(liPPCInstruction->getBinaryEncoding());

  TR_ASSERT((*(int32_t *)patchAddr & 0x0000ffff) == 0,
            "offset should be 0 to start\n");

  // This type of relocation is used in some projects so an exception handler
  // can get the return address off of the stack and then use this return
  // address and the offset loaded here to find an exit label in the generated
  // code. If the register deps are not done properly then the register assigner
  // may insert instructions after the call but before the startLabel. If this
  // bad register motion occurs then the loaded offset would be wrong (when used
  // in this way) and if an exception occurs we may end up with a wild branch.
  // This would be a very tough bug to find and fix so it is worth some ugliness
  // in the function interface.
  *patchAddr |= distance & 0x0000ffff;
}

void OMR::Power::CodeGenerator::apply64BitLoadLabelRelativeRelocation(
    TR::Instruction *lastInstruction, TR::LabelSymbol *label) {
  // Patch up the mainline code that materializes a base register and does a
  // load with update from this base register This is the instruction sequence
  // generated by fixedSeqMemAccess hiAddr = HI_VALUE(addrValue) patchAddr-4 lis
  // tempReg, hiAddr>>32 patchAddr-3   lis    baseReg, hiAddr & 0x0000FFFF
  // patchAddr-2   ori    tempReg, tempReg, (hiAddr>>16) & 0x0000ffff
  // patchAddr-1   rldimi baseReg, tempReg, 32, 0xffffFFFF00000000  (not
  // patched) patchAddr     ldu    trgReg, [baseReg+addrValue]  <--
  // lastInstruction

  TR::Instruction *lastPPCInstruction = lastInstruction;
  TR_ASSERT((lastPPCInstruction->getOpCodeValue() == TR::InstOpCode::ldu ||
             lastPPCInstruction->getOpCodeValue() == TR::InstOpCode::ld) &&
                lastPPCInstruction->getPrev()->getPrev()->getOpCodeValue() ==
                    TR::InstOpCode::ori &&
                lastPPCInstruction->getPrev()
                        ->getPrev()
                        ->getPrev()
...
static int32_t getAssociationIndex(TR::Register *reg,
  ...
  TR_ASSERT(associationIndex >= 1 && associationIndex <= maxIndex,
            "Invalid real register to be associated.\n");
  return associationIndex;
}

void OMR::Power::CodeGenerator::setRealRegisterAssociation(
    TR::Register *reg, TR::RealRegister::RegNum realNum) {
  if (!reg->isLive() || realNum == TR::RealRegister::NoReg) return;
  TR::RealRegister *realReg = self()->machine()->getRealRegister(realNum);
  self()->getLiveRegisters(reg->getKind())->setAssociation(reg, realReg);
}

void OMR::Power::CodeGenerator::addRealRegisterInterference(
    TR::Register *reg, TR::RealRegister::RegNum realNum) {
  if (!reg->isLive() || realNum == TR::RealRegister::NoReg) return;
  TR::RealRegister *realReg = self()->machine()->getRealRegister(realNum);
...
TR_BackingStore *OMR::Power::CodeGenerator::allocateStackSlot() {
  ...
  if (conversionBuffer == NULL) {
    conversionBuffer =
        new (self()->trHeapMemory()) List<TR_BackingStore>(self()->trMemory());
    ...
    conversionBufferIt = new (self()->trHeapMemory())
        ListIterator<TR_BackingStore>(conversionBuffer);
  }
  ...
  if (tempElement->getData() != NULL) {
    ...
    TR::AutomaticSymbol *spillSymbol =
        TR::AutomaticSymbol::create(self()->trHeapMemory(), TR::Int64, 8);
    spillSymbol->setSpillTempAuto();
    self()->comp()->getMethodSymbol()->addAutomatic(spillSymbol);
    return tempElement->setData(new (self()->trHeapMemory()) TR_BackingStore(
        self()->comp()->getSymRefTab(), spillSymbol, 0));
  }
}
...
/**
 * \brief
 * Determine if value fits in the immediate field (if any) of instructions that
 * machine provides.
 *
 * \details
 *
...
bool OMR::Power::CodeGenerator::isRotateAndMask(TR::Node *node) {
  if (!node->getOpCode().isAnd()) return false;
  ...
  if (secondChild->getOpCode().isLoadConst() &&
      ...
      (secondOp == TR::iconst && contiguousBits(secondChild->getInt()) &&
...
TR_PPCOutOfLineCodeSection *
OMR::Power::CodeGenerator::findOutLinedInstructionsFromLabel(
    TR::LabelSymbol *label) {
  auto oiIterator = self()->getPPCOutOfLineCodeSectionList().begin();
  while (oiIterator != self()->getPPCOutOfLineCodeSectionList().end()) {
...
TR::Snippet *OMR::Power::CodeGenerator::findSnippetInstructionsFromLabel(
    TR::LabelSymbol *label) {
  auto snippetIterator = self()->getSnippetList().begin();
  while (snippetIterator != self()->getSnippetList().end()) {
    if ((*snippetIterator)->getSnippetLabel() == label) return *snippetIterator;
    ++snippetIterator;
  }
...
TR::Instruction *OMR::Power::CodeGenerator::generateDebugCounterBump(
  ...
  if (delta > UPPER_IMMED || delta < LOWER_IMMED) {
    ...
    if (cond) {
      TR::addDependency(cond, deltaReg, TR::RealRegister::NoReg, TR_GPR,
                        self());
    }
  ...
  cursor = loadAddressConstant(self(), self()->comp()->compileRelocatableCode(),
                               node, addr, addrReg, cursor);
  cursor = generateTrg1MemInstruction(
      self(), TR::InstOpCode::lwz, node, counterReg,
      new (self()->trHeapMemory()) TR::MemoryReference(addrReg, 0, 4, self()),
      cursor);
  cursor =
      generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addi, node,
                                     counterReg, counterReg, delta, cursor);
  cursor = generateMemSrc1Instruction(
      self(), TR::InstOpCode::stw, node,
      new (self()->trHeapMemory()) TR::MemoryReference(addrReg, 0, 4, self()),
      counterReg, cursor);
  if (cond) {
    ...
    cond->getPreConditions()
        ->getRegisterDependency(preCondCursor)
        ->setExcludeGPR0();
    cond->getPostConditions()
        ->getRegisterDependency(postCondCursor)
        ->setExcludeGPR0();
    TR::addDependency(cond, counterReg, TR::RealRegister::NoReg, TR_GPR,
                      self());
  }
...
TR::Instruction *OMR::Power::CodeGenerator::generateDebugCounterBump(
  ...
  cursor = generateTrg1MemInstruction(
      ...
      cursor);
  cursor =
      generateTrg1Src2Instruction(self(), TR::InstOpCode::add, node, counterReg,
                                  counterReg, deltaReg, cursor);
  cursor = generateMemSrc1Instruction(
      self(), TR::InstOpCode::stw, node,
      new (self()->trHeapMemory()) TR::MemoryReference(addrReg, 0, 4, self()),
      counterReg, cursor);
...
bool OMR::Power::CodeGenerator::isGlobalRegisterAvailable(
    TR_GlobalRegisterNumber i, TR::DataType dt) {
  return self()
             ->machine()
             ->getRealRegister(
                 (TR::RealRegister::RegNum)self()->getGlobalRegister(i))
...
int32_t OMR::Power::CodeGenerator::getPreferredLoopUnrollFactor() {
  return self()->comp()->getMethodHotness() > warm
             ? (self()->comp()->isOptServer() ? 12 : 8)
             : 4;
}

bool OMR::Power::CodeGenerator::needsNormalizationBeforeShifts() {
  return true;
}
...
bool OMR::Power::CodeGenerator::getSupportsOpCodeForAutoSIMD(
    TR::ILOpCode opcode, TR::DataType dt) {
  // alignment issues
  if (TR::Compiler->target.cpu.id() < TR_PPCp8 && dt != TR::Double &&
      dt != TR::Int64)
    return false;

  if (TR::Compiler->target.cpu.id() >= TR_PPCp8 &&
      (opcode.getOpCodeValue() == TR::vadd ||
       opcode.getOpCodeValue() == TR::vsub) &&
      dt == TR::Int64)
    return true;
...
void OMR::Power::CodeGenerator::addMetaDataForLoadAddressConstantFixed(
    TR::Node *node, TR::Instruction *firstInstruction, TR::Register *tempReg,
    int16_t typeAddress, intptrj_t value) {
  if (value == 0x0) return;

  if (typeAddress == -1) typeAddress = TR_FixedSequenceAddress2;
    ...
    case TR_DataAddress: {
      ...
      relo = new (self()->trHeapMemory())
          TR::BeforeBinaryEncodingExternalRelocation(
              firstInstruction, (uint8_t *)recordInfo, TR_DataAddress, self());
      break;
    }

    case TR_DebugCounter: {
      TR::DebugCounterBase *counter =
          comp->getCounterFromStaticAddress(node->getSymbolReference());
      if (counter == NULL)
        comp->failCompilation<TR::CompilationException>(
            "Could not generate relocation for debug counter in "
            "OMR::Power::CodeGenerator::"
            "addMetaDataForLoadAddressConstantFixed\n");

      TR::DebugCounter::generateRelocation(comp, firstInstruction, node,
                                           counter, seqKind);
      return;
    }
  }
    ...
    switch (typeAddress) {
      case TR_ClassAddress: {
        ...
        TR_RelocationRecordInformation *recordInfo =
            (TR_RelocationRecordInformation *)comp->trMemory()->allocateMemory(
                sizeof(TR_RelocationRecordInformation), heapAlloc);
        recordInfo->data1 = (uintptr_t)symRef->getSymbol()
                                ->getStaticSymbol()
                                ->getStaticAddress();
        ...
        relo = new (self()->trHeapMemory())
            TR::BeforeBinaryEncodingExternalRelocation(
                firstInstruction, (uint8_t *)recordInfo,
                TR_DiscontiguousSymbolFromManager, self());
        break;
      }
      ...
      case TR_RamMethodSequence: {
        TR_RelocationRecordInformation *recordInfo =
            (TR_RelocationRecordInformation *)comp->trMemory()->allocateMemory(
                sizeof(TR_RelocationRecordInformation), heapAlloc);
        recordInfo->data1 = (uintptr_t)self()
                                ->comp()
                                ->getJittedMethodSymbol()
                                ->getResolvedMethod()
                                ->resolvedMethodAddress();
  ...
  if (!relo) {
    relo =
        new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
            firstInstruction, (uint8_t *)value, (uint8_t *)seqKind,
...
TR::Instruction *OMR::Power::CodeGenerator::loadAddressConstantFixed(
    TR::Node *node, intptrj_t value, TR::Register *trgReg,
    TR::Instruction *cursor, TR::Register *tempReg, int16_t typeAddress,
    bool doAOTRelocation) {
  TR::Compilation *comp = self()->comp();
  bool isAOT = comp->compileRelocatableCode();

  if (TR::Compiler->target.is32Bit()) {
    return self()->loadIntConstantFixed(node, (int32_t)value, trgReg, cursor,
                                        typeAddress);
  }
  ...
  if (tempReg == NULL) {
    ...
    cursor = firstInstruction =
        generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node, tempReg,
                                   isAOT ? 0 : value >> 48, cursor);

    // lis trgReg, bits[32-47]
    cursor = generateTrg1ImmInstruction(
        self(), TR::InstOpCode::lis, node, trgReg,
        isAOT ? 0 : (value >> 16) & 0x0000ffff, cursor);
    // ori tempReg, tempReg, bits[16-31]
    cursor = generateTrg1Src1ImmInstruction(
        self(), TR::InstOpCode::ori, node, tempReg, tempReg,
        isAOT ? 0 : (value >> 32) & 0x0000ffff, cursor);
    ...
    cursor = generateTrg1Src1Imm2Instruction(
        self(), TR::InstOpCode::rldimi, node, trgReg, tempReg, 32,
        CONSTANT64(0xFFFFFFFF00000000), cursor);
  }

  if (doAOTRelocation) {
    self()->addMetaDataForLoadAddressConstantFixed(node, firstInstruction,
                                                   tempReg, typeAddress, value);
  }

  if (temp == NULL) self()->setAppendInstruction(cursor);

  return (cursor);
}

void OMR::Power::CodeGenerator::addMetaDataForLoadIntConstantFixed(
    TR::Node *node, TR::Instruction *firstInstruction,
  ...
  if (typeAddress == TR_DataAddress) {
    ...
    recordInfo->data1 = (uintptr_t)node->getSymbolReference();
    recordInfo->data2 =
        node ? (uintptr_t)node->getInlinedSiteIndex() : (uintptr_t)-1;
    recordInfo->data3 = orderedPairSequence2;
    self()->addExternalRelocation(
        new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
    ...
    TR::DebugCounter::generateRelocation(comp, firstInstruction,
                                         secondInstruction, node, counter,
    ...
    self()->addExternalRelocation(
        new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
            (uint8_t *)firstInstruction, (uint8_t *)secondInstruction,
            (uint8_t *)recordInfo, (TR_ExternalRelocationTargetKind)typeAddress,
            self()),
        __FILE__, __LINE__, node);
  }
}
...
TR::Instruction *OMR::Power::CodeGenerator::loadIntConstantFixed(
    TR::Node *node, int32_t value, TR::Register *trgReg,
  ...
  cursor = firstInstruction = generateTrg1ImmInstruction(
      self(), TR::InstOpCode::lis, node, trgReg, value >> 16, cursor);
  cursor = secondInstruction =
      generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::ori, node, trgReg,
                                     trgReg, value & 0x0000ffff, cursor);

  self()->addMetaDataForLoadIntConstantFixed(
      node, firstInstruction, secondInstruction, typeAddress, value);
...
void OMR::Power::CodeGenerator::addMetaDataFor32BitFixedLoadLabelAddressIntoReg(
    TR::Node *node, TR::LabelSymbol *label, TR::Instruction *firstInstruction,
  ...
  self()->addRelocation(
      new (self()->trHeapMemory()) TR::PPCPairedLabelAbsoluteRelocation(
...
void OMR::Power::CodeGenerator::addMetaDataFor64BitFixedLoadLabelAddressIntoReg(
    TR::Node *node, TR::LabelSymbol *label, TR::Register *tempReg,
    TR::Instruction **q) {
  if (!self()->comp()->compileRelocatableCode()) {
    self()->addRelocation(new (self()->trHeapMemory())
                              TR::PPCPairedLabelAbsoluteRelocation(
                                  q[0], q[1], q[2], q[3], label));
  }
...
TR::Instruction *OMR::Power::CodeGenerator::fixedLoadLabelAddressIntoReg(
    TR::Node *node, TR::Register *trgReg, TR::LabelSymbol *label,
    TR::Instruction *cursor, TR::Register *tempReg, bool useADDISFor32Bit) {
  TR::Compilation *comp = self()->comp();
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (offset != PTOC_FULL_INDEX) {
      ...
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addis, node,
                                       trgReg, self()->getTOCBaseRegister(),
                                       self()->hiValue(offset));
        generateTrg1MemInstruction(
            self(), TR::InstOpCode::Op_load, node, trgReg,
            new (self()->trHeapMemory())
                TR::MemoryReference(trgReg, LO_VALUE(offset), 8, self()));
      } else {
        generateTrg1MemInstruction(
            self(), TR::InstOpCode::Op_load, node, trgReg,
            new (self()->trHeapMemory()) TR::MemoryReference(
                self()->getTOCBaseRegister(), offset, 8, self()));
      }
    } else {
      TR::Instruction *q[4];

      fixedSeqMemAccess(self(), node, 0, q, trgReg, trgReg,
                        TR::InstOpCode::addi2, 4, NULL, tempReg);

      self()->addMetaDataFor64BitFixedLoadLabelAddressIntoReg(node, label,
                                                              tempReg, q);
    }
  } else {
    ...
    if (!useADDISFor32Bit) {
      instr1 = generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node,
                                          trgReg, 0);
    } else {
      instr1 = generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addis,
                                              node, trgReg, trgReg, 0);
    }
    instr2 = generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addi2, node,
                                            trgReg, trgReg, 0);

    self()->addMetaDataFor32BitFixedLoadLabelAddressIntoReg(node, label, instr1,
                                                            instr2);
  }
...
void TR_PPCScratchRegisterManager::addScratchRegistersToDependencyList(
  ...
  while (msr) {
    ...
    if (excludeGPR0 && msr->_reg->getKind() == TR_GPR) {
      TR_ASSERT(deps->getPostConditions()
                        ->getRegisterDependency(deps->getAddCursorForPost() - 1)
                        ->getRegister() == msr->_reg,
                "Virtual register for last post condition dependency is not "
                "the expected one");
      deps->getPostConditions()
          ->getRegisterDependency(deps->getAddCursorForPost() - 1)
          ->setExcludeGPR0();
    }
...
TR::SymbolReference &OMR::Power::CodeGenerator::getArrayCopySymbolReference() {
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCarrayCopy_dp, false,
                                                  false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCarrayCopy, false, false,
                                                  false);
}
...
OMR::Power::CodeGenerator::getWordArrayCopySymbolReference() {
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCwordArrayCopy_dp, false,
                                                  false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCwordArrayCopy, false,
                                                  false, false);
}
...
OMR::Power::CodeGenerator::getForwardArrayCopySymbolReference() {
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    ...
    return *_symRefTab->findOrCreateRuntimeHelper(TR_PPCforwardArrayCopy, false,
                                                  false, false);
}
...
OMR::Power::CodeGenerator::getForwardHalfWordArrayCopySymbolReference() {
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(
        TR_PPCforwardHalfWordArrayCopy_dp, false, false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(
        TR_PPCforwardHalfWordArrayCopy, false, false, false);
}
...
bool OMR::Power::CodeGenerator::is64BitProcessor() {
  /*
   * If the target is 64 bit, the CPU must also be 64 bit (even if we don't
   * specifically know which Power CPU it is) so this can just return true. If
   * the target is not 64 bit, we need to check the CPU properties to try and
   * find out if the CPU is 64 bit or not.
   */
  if (TR::Compiler->target.is64Bit()) {
    return true;
  } else {
...
bool OMR::Power::CodeGenerator::getSupportsIbyteswap() { return true; }

bool OMR::Power::CodeGenerator::supportsNonHelper(
...
uint32_t OMR::Power::CodeGenerator::getJitMethodEntryAlignmentBoundary() {
  return 128;
}

bool OMR::Power::CodeGenerator::directCallRequiresTrampoline(
    intptrj_t targetAddress, intptrj_t sourceAddress) {
  return !TR::Compiler->target.cpu.isTargetWithinIFormBranchRange(
             targetAddress, sourceAddress) ||
         self()->comp()->getOption(TR_StressTrampolines);
}

// Multiply a register by a constant
void mulConstant(TR::Node *node, TR::Register *trgReg, TR::Register *sourceReg,
                 int32_t value, TR::CodeGenerator *cg) {
  if (value == 0) {
    loadConstant(cg, node, 0, trgReg);
  } else if (value == 1) {
    generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, trgReg,
                                sourceReg);
  } else if (value == -1) {
    generateTrg1Src1Instruction(cg, TR::InstOpCode::neg, node, trgReg,
                                sourceReg);
  } else if (isNonNegativePowerOf2(value) ||
             value == TR::getMinSigned<TR::Int32>()) {
    generateShiftLeftImmediate(cg, node, trgReg, sourceReg,
                               trailingZeroes(value));
  } else if (isNonPositivePowerOf2(value)) {
    TR::Register *tempReg = cg->allocateRegister();
    generateShiftLeftImmediate(cg, node, tempReg, sourceReg,
                               trailingZeroes(value));
    generateTrg1Src1Instruction(cg, TR::InstOpCode::neg, node, trgReg, tempReg);
    cg->stopUsingRegister(tempReg);
  } else if (isNonNegativePowerOf2(value - 1) ||
             (value - 1) == TR::getMinSigned<TR::Int32>()) {
    TR::Register *tempReg = cg->allocateRegister();
    generateShiftLeftImmediate(cg, node, tempReg, sourceReg,
                               trailingZeroes(value - 1));
    generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, trgReg, tempReg,
                                sourceReg);
    cg->stopUsingRegister(tempReg);
  } else if (isNonNegativePowerOf2(value + 1)) {
    TR::Register *tempReg = cg->allocateRegister();
    generateShiftLeftImmediate(cg, node, tempReg, sourceReg,
                               trailingZeroes(value + 1));
    generateTrg1Src2Instruction(cg, TR::InstOpCode::subf, node, trgReg,
                                sourceReg, tempReg);
    cg->stopUsingRegister(tempReg);
  } else if (value >= LOWER_IMMED && value <= UPPER_IMMED) {
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::mulli, node, trgReg,
                                   sourceReg, value);
  } else  // constant won't fit
  {
    ...
    if (leadingZeroes(abs(value)) >= 24)
      ...
      generateTrg1Src2Instruction(cg, TR::InstOpCode::mullw, node, trgReg,
                                  sourceReg, tempReg);
    else
...
// Multiply a register by a constant
void mulConstant(TR::Node *node, TR::Register *trgReg, TR::Register *sourceReg,
                 int64_t value, TR::CodeGenerator *cg) {
  if (value == 0) {
    loadConstant(cg, node, 0, trgReg);
  } else if (value == 1) {
    generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, trgReg,
    ...
    generateTrg1Src1Instruction(cg, TR::InstOpCode::neg, node, trgReg,
             ...
             value == TR::getMinSigned<TR::Int64>()) {
    generateShiftLeftImmediateLong(cg, node, trgReg, sourceReg,
                                   trailingZeroes(value));
  } else if (isNonPositivePowerOf2(value)) {
    TR::Register *tempReg = cg->allocateRegister();
    generateShiftLeftImmediateLong(cg, node, tempReg, sourceReg,
                                   trailingZeroes(value));
    generateTrg1Src1Instruction(cg, TR::InstOpCode::neg, node, trgReg, tempReg);
    cg->stopUsingRegister(tempReg);
  } else if (isNonNegativePowerOf2(value - 1) ||
             (value - 1) == TR::getMinSigned<TR::Int64>()) {
    TR::Register *tempReg = cg->allocateRegister();
    generateShiftLeftImmediateLong(cg, node, tempReg, sourceReg,
                                   trailingZeroes(value - 1));
    generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, trgReg, tempReg,
  ...
  } else  // constant won't fit
  {
    ...
    if (leadingZeroes(abs_value) >= 56)
      // the constant is fairly small, so put it in RB
      generateTrg1Src2Instruction(cg, TR::InstOpCode::mulld, node, trgReg,
                                  sourceReg, tempReg);
    else
      // the constant is fairly big, so put it in RA
      generateTrg1Src2Instruction(cg, TR::InstOpCode::mulld, node, trgReg,
...
TR::Register *addConstantToLong(TR::Node *node, TR::Register *srcReg,
                                ...
                                TR::CodeGenerator *cg) {
  if (!trgReg) trgReg = cg->allocateRegister();

  if ((int16_t)value == value) {
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, trgReg,
                                   srcReg, value);
  }
  // NOTE: the following only works if the second add's immediate is not sign
  // extended
  else if (((int32_t)value == value) && ((value & 0x8000) == 0)) {
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node, trgReg,
                                   srcReg, value >> 16);
    if (value & 0xffff)
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, trgReg,
                                     trgReg, value);
    ...
    TR::Register *tempReg = cg->allocateRegister();
    loadConstant(cg, node, value, tempReg);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::add, node, trgReg, srcReg,
                                tempReg);
    cg->stopUsingRegister(tempReg);
  }
...
TR::Register *addConstantToLong(TR::Node *node, TR::Register *srcHighReg,
                                TR::Register *srclowReg, int32_t highValue,
  ...
  if (lowValue >= 0 && lowValue <= UPPER_IMMED) {
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addic, node, lowReg,
                                   srclowReg, lowValue);
  ...
  } else  // constant won't fit
  {
    ...
    generateTrg1Src2Instruction(cg, TR::InstOpCode::addc, node, lowReg,
                                srclowReg, lowValueReg);
    cg->stopUsingRegister(lowValueReg);
  }
  if (highValue == 0) {
    generateTrg1Src1Instruction(cg, TR::InstOpCode::addze, node, highReg,
                                srcHighReg);
  } else if (highValue == -1) {
    generateTrg1Src1Instruction(cg, TR::InstOpCode::addme, node, highReg,
                                srcHighReg);
  } else {
    TR::Register *highValueReg = cg->allocateRegister();
    loadConstant(cg, node, highValue, highValueReg);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::adde, node, highReg,
                                srcHighReg, highValueReg);
    cg->stopUsingRegister(highValueReg);
  }
  return trgReg;
}

TR::Register *addConstantToInteger(TR::Node *node, TR::Register *trgReg,
                                   TR::Register *srcReg, int32_t value,
                                   TR::CodeGenerator *cg) {
  intParts localVal(value);

  if (localVal.getValue() >= LOWER_IMMED &&
      localVal.getValue() <= UPPER_IMMED) {
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi2, node, trgReg,
                                   srcReg, localVal.getValue());
  } else {
...
TR::Register *addConstantToInteger(TR::Node *node, TR::Register *srcReg,
                                   int32_t value, TR::CodeGenerator *cg) {
