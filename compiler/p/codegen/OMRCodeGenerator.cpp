  ...
  , _outOfLineCodeSectionList(getTypedAllocator<TR_PPCOutOfLineCodeSection*>(self()->comp()->allocator()))
{
  ...
  self()->setStackPointerRegister(
    self()->machine()->getRealRegister(_linkageProperties->getNormalStackPointerRegister()));
  self()->setMethodMetaDataRegister(
    self()->machine()->getRealRegister(_linkageProperties->getMethodMetaDataRegister()));
  ...
  // disabled for now
  //
  if (self()->comp()->getOption(TR_AggressiveOpts) && !self()->comp()->getOption(TR_DisableArraySetOpts)) {
  ...
  if (!disableDCAS && TR::Compiler->target.is64Bit() && TR::Options::useCompressedPointers()) {
  ...
  if (self()->comp()->getOptions()->getRegisterAssignmentTraceOption(TR_TraceRARegisterStates)) {
    self()->setGPRegisterIterator(new (self()->trHeapMemory())
        TR::RegisterIterator(self()->machine(), TR::RealRegister::FirstGPR, TR::RealRegister::LastGPR));
  ...
  TR::Instruction* cursor, TR::Node* node)
{
  ...
  TR::SymbolReference* revertToInterpreterSymRef
    = self()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCrevertToInterpreterGlue, false, false, false);
  uintptrj_t ramMethod = (uintptrj_t)methodSymbol->getResolvedMethod()->resolvedMethodAddress();
  TR::SymbolReference* helperSymRef = self()->symRefTab()->findOrCreateRuntimeHelper(
  ...
  cursor = generateDepImmSymInstruction(self(), TR::InstOpCode::bl, node,
    ...
    new (self()->trHeapMemory()) TR::RegisterDependencyConditions(0, 0, self()->trMemory()), revertToInterpreterSymRef,
    NULL, cursor);
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    cursor = generateImmInstruction(
      self(), TR::InstOpCode::dd, node, TR::Compiler->target.cpu.isBigEndian() ? lowBits : highBits, cursor);
  } else {
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    cursor = generateImmInstruction(self(), TR::InstOpCode::dd, node,
      TR::Compiler->target.cpu.isBigEndian() ? highBits : lowBits, TR_AbsoluteHelperAddress, helperSymRef, cursor);
    cursor = generateImmInstruction(
      self(), TR::InstOpCode::dd, node, TR::Compiler->target.cpu.isBigEndian() ? lowBits : highBits, cursor);
  } else {
    cursor = generateImmInstruction(
  ...
  while (isWAWOrmrPeepholeCandidateInstr(current) && !isEBBTerminatingBranch(current) && windowSize < maxWindowSize) {
    ...
    if (memRef != NULL) {
      ...
      if (memRef->isUsingDelayedIndexedForm()
        ...
        return;
      if (current->isUpdate() && (memRef->getBaseRegister() == mrSourceReg || memRef->getBaseRegister() == mrTargetReg))
        return;
      ...
      case OMR::Instruction::IsTrg1Src1Imm: {
        ...
        if (inst->getSource1Register() == mrTargetReg) {
          if (toRealRegister(mrSourceReg)->getRegisterNumber() == TR::RealRegister::gr0
            && (current->getOpCodeValue() == TR::InstOpCode::addi || current->getOpCodeValue() == TR::InstOpCode::addi2
              || current->getOpCodeValue() == TR::InstOpCode::addis))
            all_mr_source_uses_rewritten = false;
          else if (performTransformation(
                     comp, "O^O PPC PEEPHOLE: Rewrite Trg1Src1Imm source register %p.\n", current)) {
            inst->setSource1Register(mrSourceReg);
            extendWindow = true;
          } else
            return;
    ...
    if (current->defsRegister(toRealRegister(mrTargetReg))) {
      ...
      if (!inEBB && all_mr_source_uses_rewritten
        && performTransformation(
          comp, "O^O PPC PEEPHOLE: Remove mr %p when target redefined at %p.\n", mrInstruction, current)) {
            ...
            if (cg->getDebug())
              if (comp->getOption(TR_TraceCG))
                traceMsg(comp, "Adjusting register map %p; removing %s, adding %s due to removal of mr %p\n",
                  *stackMapIter, cg->getDebug()->getName(mrTargetReg), cg->getDebug()->getName(mrSourceReg),
                  mrInstruction);
  ...
  while (current != NULL && !current->isLabel()
    ...
    // if target reg is same as source of compare
    {
      ...
      if (current->getOpCode().hasRecordForm()) {
        ...
        if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.cpu.id() == TR_PPCgr) {
          TR::InstOpCode::Mnemonic opCode = current->getOpCodeValue();
          // addc_r, subfc_r, divw_r and divd_r are microcoded
          if (opCode == TR::InstOpCode::addc || opCode == TR::InstOpCode::subfc || opCode == TR::InstOpCode::divw
            || opCode == TR::InstOpCode::divd)
            return;
          ...
          if (opCode == TR::InstOpCode::rlwinm) {
            ...
            if (inst->getSourceImmediate() == 0 && (inst->getMask() & 0xffff0000) == 0) {
              if (performTransformation(comp, "O^O PPC PEEPHOLE: Change %p to andi_r, remove compare immediate %p.\n",
                ...
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, inst->getNode(),
                  inst->getPrimaryTargetRegister(), inst->getSourceRegister(0), cmpiTargetReg, inst->getMask(),
                  current);
                current->remove();
                cmpiInstruction->remove();
              }
              ...
              if (performTransformation(comp, "O^O PPC PEEPHOLE: Change %p to andis_r, remove compare immediate %p.\n",
                    current, cmpiInstruction)) {
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, inst->getNode(),
      ...
      if (current->defsRegister(toRealRegister(cmpiSourceReg)) || current->defsRegister(toRealRegister(cmpiTargetReg))
  ...
  if (srcReg == trgReg) {
    ...
    if (loadInstruction->getOpCodeValue() == TR::InstOpCode::lwz) {
      if (performTransformation(comp,
            "O^O PPC PEEPHOLE: Replace lwz " POINTER_PRINTF_FORMAT " with rlwinm after store " POINTER_PRINTF_FORMAT
            ".\n",
            loadInstruction, storeInstruction)) {
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rlwinm, loadInstruction->getNode(), trgReg, srcReg, 0, 0xffffffff, storeInstruction);
        loadInstruction->remove();
      }
    } else if (performTransformation(comp,
                 "O^O PPC PEEPHOLE: Remove redundant load " POINTER_PRINTF_FORMAT " after store " POINTER_PRINTF_FORMAT
                 ".\n",
                 loadInstruction, storeInstruction)) {
      loadInstruction->remove();
    }
...
static void wawPeephole(TR::CodeGenerator* cg, TR::Instruction* instr)
{
  ...
  if (currTargetReg && currTargetReg->getKind() != TR_CCR && isWAWOrmrPeepholeCandidateInstr(instr)
    && !instr->isBranchOp() && !instr->isCall() && !instr->getOpCode().isRecordForm()
      ...
      if (current->getTargetRegister(0) == currTargetReg) {
        ...
        if (isConstData) {
          if (performTransformation(
                comp, "O^O PPC PEEPHOLE: Remove dead instrcution group from WAW %p -> %p.\n", instr, current)) {
          ...
          if (performTransformation(
                comp, "O^O PPC PEEPHOLE: Remove dead instrcution from WAW %p -> %p.\n", instr, current)) {
            instr->remove();
          }
        }
        return;
...
TR::CodeGenerator::TR_RegisterPressureSummary* OMR::Power::CodeGenerator::calculateRegisterPressure()
{
  return NULL;
}
...
void OMR::Power::CodeGenerator::doBinaryEncoding()
{
  ...
  while (data.cursorInstruction) {
    ...
    if (data.cursorInstruction == data.preProcInstruction) {
      self()->setPrePrologueSize(
        self()->getBinaryBufferCursor() - self()->getBinaryBufferStart() - self()->getJitMethodEntryPaddingSize());
    ...
    if (isPrivateLinkage && data.cursorInstruction == data.jitTojitStart) {
      uint32_t magicWord = ((self()->getBinaryBufferCursor() - self()->getCodeStart()) << 16)
      ...
      *(uint32_t*)(data.preProcInstruction->getBinaryEncoding()) = magicWord;
      ...
      toPPCImmInstruction(data.preProcInstruction)
        ->setSourceImmediate(*(uint32_t*)(data.preProcInstruction->getBinaryEncoding()));
    }
  ...
  if (!self()->canClobberNodesRegister(node)) {
    if (TR::Compiler->target.is32Bit() && node->getType().isInt64()) {
      ...
      generateTrg1Src1Instruction(
        self(), resultReg->getKind() == TR_GPR ? TR::InstOpCode::mr : TR::InstOpCode::fmr, node, targetReg, resultReg);
      return targetReg;
    }
  } else {
    return resultReg;
...
static int32_t identifyFarConditionalBranches(int32_t estimate, TR::CodeGenerator* cg)
{
  ...
  for (int32_t i = candidateBranches.lastIndex(); i >= 0; i--) {
    ...
    if (targetLocation >= myLocation) {
      for (j = i + 1;
           j < candidateBranches.size() && targetLocation > candidateBranches[j]->getEstimatedBinaryLocation(); j++)
        ;
...
int32_t OMR::Power::CodeGenerator::findOrCreateAddressConstant(void* v, TR::DataType t, TR::Instruction* n0,
...
{
  TR::Node* immediateChild = (node->getNumChildren() == 2 && !node->getOpCode().isCall() && !node->getOpCode().isLoad())
    ? node->getSecondChild()
    : NULL;
...
TR_GlobalRegisterNumber OMR::Power::CodeGenerator::pickRegister(TR_RegisterCandidate* regCan, TR::Block** barr,
  TR_BitVector& availRegs, TR_GlobalRegisterNumber& highRegisterNumber, TR_LinkHead<TR_RegisterCandidate>* candidates)
{
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation))
  ...
  if (gprCandidate) {
    ...
    while (livec.hasMoreElements()) {
      ...
      maxRegisterPressure = self()->estimateRegisterPressure(block, visitCount, maxStaticFrequency, maxFrequency,
        vmThreadUsed, numAssignedGlobalRegs, _assignedGlobalRegisters, regCan->getSymbolReference(), assigningEDX);
...
int32_t OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
  if (node->getOpCode().isSwitch()) {
    ...
    return (TR::Compiler->target.is64Bit() ? _numGPR
  ...
  TR::Instruction* liInstruction, TR::LabelSymbol* startLabel, TR::LabelSymbol* endLabel, int32_t deltaToStartLabel)
{
  ...
  // This type of relocation is used in some projects so an exception handler can get the return address off of the
  // stack and then use this return address and the offset loaded here to find an exit label in the generated code. If
  // the register deps are not done properly then the register assigner may insert instructions after the call but
  // before the startLabel. If this bad register motion occurs then the loaded offset would be wrong (when used in this
  // way) and if an exception occurs we may end up with a wild branch. This would be a very tough bug to find and fix so
  // it is worth some ugliness in the function interface.
    ...
    (secondOp == TR::iconst && contiguousBits(secondChild->getInt()) && firstChild->getReferenceCount() == 1
      ...
      && ((firstOp == TR::imul && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst
        ...
        || ((firstOp == TR::ishr || firstOp == TR::iushr)
          && firstChild->getSecondChild()->getOpCodeValue() == TR::iconst && firstChild->getSecondChild()->getInt() > 0
          && (firstOp == TR::iushr
...
bool OMR::Power::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode, TR::DataType dt)
{
  ...
  // alignment issues
  if (TR::Compiler->target.cpu.id() < TR_PPCp8 && dt != TR::Double && dt != TR::Int64)
    return false;

  if (TR::Compiler->target.cpu.id() >= TR_PPCp8
    && (opcode.getOpCodeValue() == TR::vadd || opcode.getOpCodeValue() == TR::vsub) && dt == TR::Int64)
    return true;
...
{
  return TR::Compiler->target.cpu.getPPCSupportsVSX() && !self()->comp()->getOption(TR_DisableSIMDUTF16LEEncoder);
}

bool OMR::Power::CodeGenerator::getSupportsEncodeUtf16BigWithSurrogateTest()
{
  return TR::Compiler->target.cpu.getPPCSupportsVSX() && !self()->comp()->getOption(TR_DisableSIMDUTF16BEEncoder);
}
...
TR::Instruction* OMR::Power::CodeGenerator::loadAddressConstantFixed(TR::Node* node, intptrj_t value,
  TR::Register* trgReg, TR::Instruction* cursor, TR::Register* tempReg, int16_t typeAddress, bool doAOTRelocation)
{
  ...
  if (tempReg == NULL) {
    ...
    cursor = firstInstruction
      = generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node, trgReg, isAOT ? 0 : (value >> 48), cursor);

    // ori trgReg, trgReg, next 16-bits
    cursor = generateTrg1Src1ImmInstruction(
      self(), TR::InstOpCode::ori, node, trgReg, trgReg, isAOT ? 0 : ((value >> 32) & 0x0000ffff), cursor);
    ...
    cursor = generateTrg1Src1ImmInstruction(
      self(), TR::InstOpCode::oris, node, trgReg, trgReg, isAOT ? 0 : ((value >> 16) & 0x0000ffff), cursor);
    // ori trgReg, trgReg, last 16-bits
    cursor = generateTrg1Src1ImmInstruction(
      self(), TR::InstOpCode::ori, node, trgReg, trgReg, isAOT ? 0 : (value & 0x0000ffff), cursor);
  } else {
    // lis tempReg, bits[0-15]
    cursor = firstInstruction
      = generateTrg1ImmInstruction(self(), TR::InstOpCode::lis, node, tempReg, isAOT ? 0 : value >> 48, cursor);

    // lis trgReg, bits[32-47]
    cursor = generateTrg1ImmInstruction(
      self(), TR::InstOpCode::lis, node, trgReg, isAOT ? 0 : (value >> 16) & 0x0000ffff, cursor);
    // ori tempReg, tempReg, bits[16-31]
    cursor = generateTrg1Src1ImmInstruction(
      self(), TR::InstOpCode::ori, node, tempReg, tempReg, isAOT ? 0 : (value >> 32) & 0x0000ffff, cursor);
  ...
  if (typeAddress == TR_DataAddress) {
    ...
    self()->addExternalRelocation(
      new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation((uint8_t*)firstInstruction,
        (uint8_t*)secondInstruction, (uint8_t*)recordInfo, (TR_ExternalRelocationTargetKind)TR_DataAddress, self()),
    ...
    self()->addExternalRelocation(
      new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation((uint8_t*)firstInstruction,
        (uint8_t*)secondInstruction, (uint8_t*)recordInfo, (TR_ExternalRelocationTargetKind)typeAddress, self()),
      __FILE__, __LINE__, node);
  }
}
  ...
  TR::Node* node, TR::LabelSymbol* label, TR::Register* tempReg, TR::Instruction** q)
{
  ...
  self()->addExternalRelocation(
    new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
      q[0], (uint8_t*)(label), (uint8_t*)(tempReg ? fixedSequence4 : fixedSequence2), TR_FixedSequenceAddress, self()),
    __FILE__, __LINE__, node);
}
    ...
    if (offset != PTOC_FULL_INDEX) {
      ...
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        generateTrg1Src1ImmInstruction(
          self(), TR::InstOpCode::addis, node, trgReg, self()->getTOCBaseRegister(), self()->hiValue(offset));
        generateTrg1MemInstruction(self(), TR::InstOpCode::Op_load, node, trgReg,
          new (self()->trHeapMemory()) TR::MemoryReference(trgReg, LO_VALUE(offset), 8, self()));
      } else {
        generateTrg1MemInstruction(self(), TR::InstOpCode::Op_load, node, trgReg,
          new (self()->trHeapMemory()) TR::MemoryReference(self()->getTOCBaseRegister(), offset, 8, self()));
      }
    } else {
      TR::Instruction* q[4];
  ...
  TR::RegisterDependencyConditions* deps, bool excludeGPR0)
{
  ...
  while (msr) {
    ...
    if (excludeGPR0 && msr->_reg->getKind() == TR_GPR) {
      TR_ASSERT(
        deps->getPostConditions()->getRegisterDependency(deps->getAddCursorForPost() - 1)->getRegister() == msr->_reg,
...
{
  /*
   * If the target is 64 bit, the CPU must also be 64 bit (even if we don't specifically know which Power CPU it is) so
   * this can just return true. If the target is not 64 bit, we need to check the CPU properties to try and find out if
   * the CPU is 64 bit or not.
   */
  if (TR::Compiler->target.is64Bit()) {
    return true;
  } else {
...
bool OMR::Power::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress, intptrj_t sourceAddress)
{
  return !TR::Compiler->target.cpu.isTargetWithinIFormBranchRange(targetAddress, sourceAddress)
