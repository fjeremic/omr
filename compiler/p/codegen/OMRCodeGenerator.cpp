...
static int32_t
identifyFarConditionalBranches(int32_t estimate, TR::CodeGenerator* cg);
      ...
      self()->comp()->allocator()))
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    self()->setTrackItems(new (self()->trHeapMemory())
  ...
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
    ...
    for (TR_GlobalRegisterNumber grn = 0;
         grn < self()->getNumberOfGlobalRegisters();
         grn++) {
  ...
  if (self()->comp()->getOptions()->getRegisterAssignmentTraceOption(
    ...
    self()->setGPRegisterIterator(new (
  ...
  TR::Node* node)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    cursor = generateImmInstruction(
      ...
      TR::InstOpCode::dd,
      node,
      TR::Compiler->target.cpu.isBigEndian() ? lowBits : highBits,
      cursor);
  } else {
    cursor = generateImmInstruction(self(),
                                    TR::InstOpCode::dd,
                                    node,
                                    (int32_t)ramMethod,
                                    TR_RamMethod,
...
void
OMR::Power::CodeGenerator::itemTracking(int32_t offset, TR::LabelSymbol* label)
{
  if (TR::Compiler->target.is64Bit()) {
...
bool
OMR::Power::CodeGenerator::mulDecompositionCostIsJustified(int numOfOperations,
                                                           char bitPosition[],
                                                           char operationType[],
                                                           int64_t value)
{
  ...
  switch (TR::Compiler->target.cpu.id()) {
    ...
    case TR_PPCgpul:
    case TR_PPCgp:
    case TR_PPCgr: // 2 FXU out-of-order back-to-back 2 cycles. Mul is only 4 to
                   // 6 cycles
...
void
OMR::Power::CodeGenerator::doRegisterAssignment(TR_RegisterKinds kindsToAssign)
{
...
TR::Instruction*
OMR::Power::CodeGenerator::generateNop(TR::Node* n,
...
static void
mrPeepholes(TR::CodeGenerator* cg, TR::Instruction* mrInstruction)
{
  ...
  while (isWAWOrmrPeepholeCandidateInstr(current) &&
    ...
    if (current->getGCMap() &&
        current->getGCMap()->getRegisterMap() &
          cg->registerBitMask(toRealRegister(mrTargetReg)->getRegisterNumber()))
    ...
    if (memRef != NULL) {
      ...
      if (memRef->getBaseRegister() == mrTargetReg) {
        if (toRealRegister(mrSourceReg)->getRegisterNumber() ==
          ...
          all_mr_source_uses_rewritten = false;
                   ...
                   comp,
                   "O^O PPC PEEPHOLE: Rewrite base register %p.\n",
                   current)) {
      ...
      if (memRef->getIndexRegister() == mrTargetReg) {
        if (performTransformation(
              comp,
              "O^O PPC PEEPHOLE: Rewrite index register %p.\n",
              current)) {
      ...
      if (current->getKind() == OMR::Instruction::IsMemSrc1 &&
        ...
        if (performTransformation(
              comp,
              "O^O PPC PEEPHOLE: Rewrite store source register %p.\n",
              current)) {
...
static void
recordFormPeephole(TR::CodeGenerator* cg, TR::Instruction* cmpiInstruction)
{
  TR::Compilation* comp = cg->comp();
...
static void
lhsPeephole(TR::CodeGenerator* cg, TR::Instruction* storeInstruction)
{
...
static void
syncPeephole(TR::CodeGenerator* cg,
...
static void
trapPeephole(TR::CodeGenerator* cg, TR::Instruction* instructionCursor)
{
...
static void
wawPeephole(TR::CodeGenerator* cg, TR::Instruction* instr)
{
  TR::Compilation* comp = cg->comp();
...
bool
OMR::Power::CodeGenerator::checkAndFetchRequestor(TR::Instruction* instr,
                                                  TR::Instruction** q)
{
  if (_constantData) {
...
void
OMR::Power::CodeGenerator::deleteInst(TR::Instruction* old)
{
...
void
OMR::Power::CodeGenerator::generateBinaryEncodingPrologue(
...
TR::Register*
OMR::Power::CodeGenerator::gprClobberEvaluate(TR::Node* node)
{
  ...
  if (!self()->canClobberNodesRegister(node)) {
    if (TR::Compiler->target.is32Bit() && node->getType().isInt64()) {
      ...
      generateTrg1Src1Instruction(
        self(), TR::InstOpCode::mr, node, lowReg, resultReg->getLowOrder());

      generateTrg1Src1Instruction(
        self(), TR::InstOpCode::mr, node, highReg, resultReg->getHighOrder());
      return longReg;
    } else {
...
void
OMR::Power::CodeGenerator::buildRegisterMapForInstruction(TR_GCStackMap* map)
{
...
int32_t
OMR::Power::CodeGenerator::findOrCreateFloatConstant(void* v,
                                                     TR::DataType t,
                                                     TR::Instruction* n0,
                                                     TR::Instruction* n1,
                                                     TR::Instruction* n2,
...
int32_t
OMR::Power::CodeGenerator::findOrCreateAddressConstant(void* v,
                                                       TR::DataType t,
                                                       TR::Instruction* n0,
                                                       TR::Instruction* n1,
                                                       TR::Instruction* n2,
                                                       ...
                                                       bool isUnloadablePicSite)
{
  if (_constantData == NULL)
    _constantData =
      new (self()->trHeapMemory()) TR::ConstantDataSnippet(self());
  return (_constantData->addConstantRequest(
    v, t, n0, n1, n2, n3, node, isUnloadablePicSite));
}

bool
OMR::Power::CodeGenerator::isSnippetMatched(TR::Snippet* snippet,
                                            int32_t snippetKind,
...
int32_t
OMR::Power::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(
...
void
OMR::Power::CodeGenerator::simulateNodeEvaluation(
  TR::Node* node,
...
OMR::Power::CodeGenerator::pickRegister(
...
{
  if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation))
    return OMR::CodeGenerator::pickRegister(
      regCan, barr, availRegs, highRegisterNumber, candidates);
  ...
  if (gprCandidate) {
    ...
    if (!_assignedGlobalRegisters)
      _assignedGlobalRegisters = new (self()->trStackMemory())
        TR_BitVector(self()->comp()->getSymRefCount(),
                     self()->trMemory(),
                     stackAlloc,
                     growable);
    ...
    while (livec.hasMoreElements()) {
      ...
      maxRegisterPressure =
        self()->estimateRegisterPressure(block,
                                         visitCount,
                                         maxStaticFrequency,
                                         maxFrequency,
                                         vmThreadUsed,
...
bool
OMR::Power::CodeGenerator::allowGlobalRegisterAcrossBranch(
...
int32_t
OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(
  TR::Node* node)
{
  if (node->getOpCode().isSwitch()) {
    ...
    return (
      TR::Compiler->target.is64Bit()
        ? _numGPR
        : _numGPR -
            4); // 4 GPRs are needed for long-compare arguments themselves.
  }
...
int32_t
OMR::Power::CodeGenerator::getMaximumNumberOfFPRsAllowedAcrossEdge(
  TR::Node* node)
{
  ...
  TR::DataType type)
{
  ...
  diagnostic("getLinkageGlobalRegisterNumber(%d, %s) = %d\n",
             ...
             self()->comp()->getDebug()->getName(type),
             result);
  return result;
}

void
OMR::Power::CodeGenerator::apply16BitLabelRelativeRelocation(
  int32_t* cursor,
...
void
OMR::Power::CodeGenerator::apply24BitLabelRelativeRelocation(
  int32_t* cursor,
  TR::LabelSymbol* label)
{
  TR_ASSERT(label->getCodeLocation(),
  ...
  *cursor =
    ((*cursor) & 0xfc000003) |
    (((uintptrj_t)label->getCodeLocation() - (uintptrj_t)cursor) & 0x03fffffc);
}

void
OMR::Power::CodeGenerator::apply16BitLoadLabelRelativeRelocation(
  ...
  int32_t deltaToStartLabel)
{
  ...
  TR_ASSERT(
    liPPCInstruction->getOpCodeValue() == TR::InstOpCode::li,
    "wrong load instruction used for apply32BitLoadLabelRelativeRelocation\n");

  int32_t distance =
    int32_t((endLabel->getCodeLocation() - startLabel->getCodeLocation()) / 4);
  TR_ASSERT(
...
void
OMR::Power::CodeGenerator::apply64BitLoadLabelRelativeRelocation(
  ...
  TR::LabelSymbol* label)
{
  ...
  TR_ASSERT(
    (lastPPCInstruction->getOpCodeValue() == TR::InstOpCode::ldu ||
     lastPPCInstruction->getOpCodeValue() == TR::InstOpCode::ld) &&
      lastPPCInstruction->getPrev()->getPrev()->getOpCodeValue() ==
        TR::InstOpCode::ori &&
      lastPPCInstruction->getPrev()->getPrev()->getPrev()->getOpCodeValue() ==
        TR::InstOpCode::lis &&
...
static int32_t
getAssociationIndex(TR::Register* reg, TR::RealRegister::RegNum realNum)
{
...
void
OMR::Power::CodeGenerator::setRealRegisterAssociation(
  TR::Register* reg,
...
void
OMR::Power::CodeGenerator::addRealRegisterInterference(
  TR::Register* reg,
  TR::RealRegister::RegNum realNum)
{
  if (!reg->isLive() || realNum == TR::RealRegister::NoReg)
...
#if DEBUG
void
OMR::Power::CodeGenerator::dumpDataSnippets(TR::FILE* outFile)
{
  if (outFile == NULL)
    return;
 ...
 */
bool
OMR::Power::CodeGenerator::shouldValueBeInACommonedNode(int64_t value)
{
...
bool
OMR::Power::CodeGenerator::isRotateAndMask(TR::Node* node)
{
      ...
      (secondOp == TR::iconst && contiguousBits(secondChild->getInt()) &&
       ...
       ((firstOp == TR::imul &&
        ...
        ((firstOp == TR::ishr || firstOp == TR::iushr) &&
         firstChild->getSecondChild()->getOpCodeValue() == TR::iconst &&
         firstChild->getSecondChild()->getInt() > 0 &&
         (firstOp == TR::iushr || leadingZeroes(secondChild->getInt()) >=
...
TR::Snippet*
OMR::Power::CodeGenerator::findSnippetInstructionsFromLabel(
  TR::LabelSymbol* label)
{
...
bool
OMR::Power::CodeGenerator::isILOpCodeSupported(TR::ILOpCodes o)
{
...
TR::Instruction*
OMR::Power::CodeGenerator::generateDebugCounterBump(
  TR::Instruction* cursor,
  ...
  TR::RegisterDependencyConditions* cond)
{
  ...
  if (delta > UPPER_IMMED || delta < LOWER_IMMED) {
    ...
    if (cond) {
      TR::addDependency(
        cond, deltaReg, TR::RealRegister::NoReg, TR_GPR, self());
    }
  ...
  cursor = loadAddressConstant(self(),
  ...
  cursor = generateTrg1Src1ImmInstruction(
    self(), TR::InstOpCode::addi, node, counterReg, counterReg, delta, cursor);
  ...
  if (cond) {
    ...
    cond->getPostConditions()
      ...
      ->setExcludeGPR0();
    TR::addDependency(
      cond, counterReg, TR::RealRegister::NoReg, TR_GPR, self());
  }
  ...
  TR::RegisterDependencyConditions* cond)
{
  ...
  cursor = generateTrg1MemInstruction(
    ...
    new (self()->trHeapMemory()) TR::MemoryReference(addrReg, 0, 4, self()),
    cursor);
  cursor = generateTrg1Src2Instruction(self(),
                                       TR::InstOpCode::add,
                                       node,
                                       counterReg,
                                       counterReg,
...
bool
OMR::Power::CodeGenerator::isGlobalRegisterAvailable(TR_GlobalRegisterNumber i,
                                                     TR::DataType dt)
{
  return self()
           ->machine()
...
bool
OMR::Power::CodeGenerator::getSupportsOpCodeForAutoSIMD(TR::ILOpCode opcode,
                                                        TR::DataType dt)
{
  ...
  switch (typeAddress) {
    ...
    case TR_DebugCounter: {
      ...
      TR::DebugCounter::generateRelocation(
        comp, firstInstruction, node, counter, seqKind);
      return;
    }
  }
...
TR::Instruction*
OMR::Power::CodeGenerator::loadAddressConstantFixed(TR::Node* node,
                                                    intptrj_t value,
                                                    TR::Register* trgReg,
                                                    TR::Instruction* cursor,
                                                    TR::Register* tempReg,
                                                    ...
                                                    bool doAOTRelocation)
{
  ...
  if (TR::Compiler->target.is32Bit()) {
    return self()->loadIntConstantFixed(
      node, (int32_t)value, trgReg, cursor, typeAddress);
  }
  ...
  if (tempReg == NULL) {
    ...
    cursor = firstInstruction =
      generateTrg1ImmInstruction(self(),
                                 TR::InstOpCode::lis,
                                 node,
                                 trgReg,
                                 isAOT ? 0 : (value >> 48),
    ...
    cursor =
      generateTrg1Src1ImmInstruction(self(),
                                     TR::InstOpCode::ori,
                                     node,
                                     trgReg,
                                     trgReg,
                                     isAOT ? 0 : ((value >> 32) & 0x0000ffff),
                                     cursor);
    // shiftli trgReg, trgReg, 32
    cursor = generateTrg1Src1Imm2Instruction(self(),
    ...
    cursor =
      generateTrg1Src1ImmInstruction(self(),
                                     ...
                                     trgReg,
                                     trgReg,
                                     isAOT ? 0 : ((value >> 16) & 0x0000ffff),
                                     cursor);
    // ori trgReg, trgReg, last 16-bits
    cursor = generateTrg1Src1ImmInstruction(self(),
                                            TR::InstOpCode::ori,
                                            node,
                                            trgReg,
                                            trgReg,
                                            isAOT ? 0 : (value & 0x0000ffff),
                                            cursor);
  } else {
    ...
    cursor = generateTrg1ImmInstruction(self(),
                                        TR::InstOpCode::lis,
                                        node,
                                        trgReg,
                                        isAOT ? 0 : (value >> 16) & 0x0000ffff,
  ...
  if (doAOTRelocation) {
    self()->addMetaDataForLoadAddressConstantFixed(
      node, firstInstruction, tempReg, typeAddress, value);
  }
  ...
  if (typeAddress == TR_DataAddress) {
    ...
    self()->addExternalRelocation(
      new (self()->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
        ...
        (uint8_t*)recordInfo,
        (TR_ExternalRelocationTargetKind)TR_DataAddress,
        self()),
      __FILE__,
      __LINE__,
      node);
...
//
TR::Instruction*
OMR::Power::CodeGenerator::loadIntConstantFixed(TR::Node* node,
                                                int32_t value,
                                                TR::Register* trgReg,
                                                TR::Instruction* cursor,
                                                int16_t typeAddress)
  ...
  TR::Instruction* secondInstruction)
{
  ...
  self()->getAheadOfTimeCompile()->getRelocationList().push_front(
    new (self()->trHeapMemory())
      TR::PPCPairedRelocation(firstInstruction,
                              secondInstruction,
                              (uint8_t*)recordInfo,
  ...
  TR::Instruction** q)
{
  ...
  if (!self()->comp()->compileRelocatableCode()) {
    self()->addRelocation(
      new (self()->trHeapMemory())
        TR::PPCPairedLabelAbsoluteRelocation(q[0], q[1], q[2], q[3], label));
  }
...
TR::Instruction*
OMR::Power::CodeGenerator::fixedLoadLabelAddressIntoReg(TR::Node* node,
                                                        TR::Register* trgReg,
                                                        TR::LabelSymbol* label,
                                                        TR::Instruction* cursor,
                                                        TR::Register* tempReg,
                                                        bool useADDISFor32Bit)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    if (offset != PTOC_FULL_INDEX) {
      ...
      self()->addMetaDataFor64BitFixedLoadLabelAddressIntoReg(
        node, label, tempReg, q);
    }
  } else {
    TR::Instruction *instr1, *instr2;

    if (!useADDISFor32Bit) {
      instr1 = generateTrg1ImmInstruction(
        self(), TR::InstOpCode::lis, node, trgReg, 0);
    } else {
      instr1 = generateTrg1Src1ImmInstruction(
        self(), TR::InstOpCode::addis, node, trgReg, trgReg, 0);
    }
    instr2 = generateTrg1Src1ImmInstruction(
      self(), TR::InstOpCode::addi2, node, trgReg, trgReg, 0);

    self()->addMetaDataFor32BitFixedLoadLabelAddressIntoReg(
      node, label, instr1, instr2);
  }
...
void
TR_PPCScratchRegisterManager::addScratchRegistersToDependencyList(
...
OMR::Power::CodeGenerator::getArrayCopySymbolReference()
{
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCarrayCopy_dp, false, false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCarrayCopy, false, false, false);
}
...
OMR::Power::CodeGenerator::getWordArrayCopySymbolReference()
{
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCwordArrayCopy_dp, false, false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCwordArrayCopy, false, false, false);
}
...
OMR::Power::CodeGenerator::getHalfWordArrayCopySymbolReference()
{
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPChalfWordArrayCopy_dp, false, false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPChalfWordArrayCopy, false, false, false);
}
...
OMR::Power::CodeGenerator::getForwardArrayCopySymbolReference()
{
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCforwardArrayCopy_dp, false, false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCforwardArrayCopy, false, false, false);
}
...
OMR::Power::CodeGenerator::getForwardWordArrayCopySymbolReference()
{
  if (TR::Compiler->target.cpu.id() >= TR_PPCp6)
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCforwardWordArrayCopy_dp, false, false, false);
  else
    return *_symRefTab->findOrCreateRuntimeHelper(
      TR_PPCforwardWordArrayCopy, false, false, false);
}
...
bool
OMR::Power::CodeGenerator::supportsNonHelper(
...
bool
OMR::Power::CodeGenerator::directCallRequiresTrampoline(intptrj_t targetAddress,
            ...
            TR::CodeGenerator* cg)
{
  if (value == 0) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::mr, node, trgReg, sourceReg);
  } else if (value == -1) {
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::neg, node, trgReg, sourceReg);
  } else if (isNonNegativePowerOf2(value) ||
             value == TR::getMinSigned<TR::Int32>()) {
    generateShiftLeftImmediate(
      cg, node, trgReg, sourceReg, trailingZeroes(value));
  } else if (isNonPositivePowerOf2(value)) {
    TR::Register* tempReg = cg->allocateRegister();
    generateShiftLeftImmediate(
      cg, node, tempReg, sourceReg, trailingZeroes(value));
    ...
    TR::Register* tempReg = cg->allocateRegister();
    generateShiftLeftImmediate(
      cg, node, tempReg, sourceReg, trailingZeroes(value - 1));
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::add, node, trgReg, tempReg, sourceReg);
    cg->stopUsingRegister(tempReg);
  } else if (isNonNegativePowerOf2(value + 1)) {
    TR::Register* tempReg = cg->allocateRegister();
    generateShiftLeftImmediate(
      cg, node, tempReg, sourceReg, trailingZeroes(value + 1));
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::subf, node, trgReg, sourceReg, tempReg);
    cg->stopUsingRegister(tempReg);
  } else if (value >= LOWER_IMMED && value <= UPPER_IMMED) {
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::mulli, node, trgReg, sourceReg, value);
  } else // constant won't fit
  {
    ...
    if (leadingZeroes(abs(value)) >= 24)
      ...
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mullw, node, trgReg, sourceReg, tempReg);
    else
      // the constant is fairly big, so put it in RA
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mullw, node, trgReg, tempReg, sourceReg);
    cg->stopUsingRegister(tempReg);
  }
}
            ...
            TR::CodeGenerator* cg)
{
  ...
  } else // constant won't fit
  {
    ...
    if (leadingZeroes(abs_value) >= 56)
      // the constant is fairly small, so put it in RB
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mulld, node, trgReg, sourceReg, tempReg);
    else
      // the constant is fairly big, so put it in RA
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::mulld, node, trgReg, tempReg, sourceReg);
    cg->stopUsingRegister(tempReg);
  }
}

TR::Register*
addConstantToLong(TR::Node* node,
                  TR::Register* srcReg,
                  int64_t value,
                  TR::Register* trgReg,
                  TR::CodeGenerator* cg)
{
  ...
  if ((int16_t)value == value) {
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi2, node, trgReg, srcReg, value);
  }
  // NOTE: the following only works if the second add's immediate is not sign
  // extended
  else if (((int32_t)value == value) && ((value & 0x8000) == 0)) {
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addis, node, trgReg, srcReg, value >> 16);
    if (value & 0xffff)
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi2, node, trgReg, trgReg, value);
  } else {
    TR::Register* tempReg = cg->allocateRegister();
    loadConstant(cg, node, value, tempReg);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::add, node, trgReg, srcReg, tempReg);
    cg->stopUsingRegister(tempReg);
  }
                  ...
                  TR::CodeGenerator* cg)
{
  ...
  if (lowValue >= 0 && lowValue <= UPPER_IMMED) {
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addic, node, lowReg, srclowReg, lowValue);
  ...
  } else // constant won't fit
  {
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::addc, node, lowReg, srclowReg, lowValueReg);
    cg->stopUsingRegister(lowValueReg);
  }
  if (highValue == 0) {
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::addze, node, highReg, srcHighReg);
  } else if (highValue == -1) {
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::addme, node, highReg, srcHighReg);
  } else {
    TR::Register* highValueReg = cg->allocateRegister();
    loadConstant(cg, node, highValue, highValueReg);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::adde, node, highReg, srcHighReg, highValueReg);
...
TR::Register*
addConstantToInteger(TR::Node* node,
                     TR::Register* trgReg,
                     TR::Register* srcReg,
                     int32_t value,
                     TR::CodeGenerator* cg)
{
  ...
  if (localVal.getValue() >= LOWER_IMMED &&
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi2, node, trgReg, srcReg, localVal.getValue());
  } else {
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addis, node, trgReg, srcReg, upperLit);
    if (lowerLit != 0) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi2, node, trgReg, trgReg, lowerLit);
    }
  }
  return trgReg;
}
