...
uint32_t mappedOffsetToFirstLocal(TR::CodeGenerator* cg, const TR::PPCLinkageProperties& linkage)
{
  ...
  uint32_t min_arg_area = machine->getLinkRegisterKilled() ? (TR::Compiler->target.is64Bit() ? 8 * 8 : 8 * 4) : 0;
...
void TR::PPCSystemLinkage::mapParameters(TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
  ...
  // XXX: This hack fixes ppc64le by saving params in the current stack frame, rather than the caller's parameter save
  // area, which may not exist
  //      This code needs to be refactored to accomodate ABIs that don't guarantee a parameter save area
  // XXX: Also, for some reason all these system linkages are marked passing parms right-to-left when they should be
  // left-to-right
...
void TR::PPCSystemLinkage::createPrologue(TR::Instruction* cursor, List<TR::ParameterSymbol>& parmList)
{
  ...
  if (machine->getLinkRegisterKilled())
    cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
      new (trHeapMemory()) TR::MemoryReference(
        sp, 2 * TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
      gr0, cursor);
  ...
  if (!bodySymbol->isEHAware()) {
    while (
      savedFirst <= TR::RealRegister::LastFPR && !machine->getRealRegister(savedFirst)->getHasBeenAssignedInMethod())
  ...
  for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
    ...
    cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::stfd, firstNode,
      new (trHeapMemory()) TR::MemoryReference(sp, argSize, 8, cg()), machine->getRealRegister(regIndex), cursor);
  }
  ...
  if (savedFirst <= TR::RealRegister::LastGPR) {
    if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit()
      || (!comp()->getOption(TR_OptimizeForSpace) && TR::RealRegister::LastGPR - savedFirst <= 3))
      for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
           regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex - 1)) {
        argSize = argSize - TR::Compiler->om.sizeofReferenceAddress();
        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
          new (trHeapMemory()) TR::MemoryReference(sp, argSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
      ...
      cursor = generateMemSrc1Instruction(cg(),
        ...
        new (trHeapMemory()) TR::MemoryReference(sp, argSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
  ...
  if (bodySymbol->isEHAware() || machine->getRealRegister(TR::RealRegister::cr2)->getHasBeenAssignedInMethod()
    ...
    cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
      new (trHeapMemory()) TR::MemoryReference(
        sp, TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
      gr0, cursor);
  }
  ...
  if (size > properties.getOffsetToFirstParm() - argSize) {
    if (size > (-LOWER_IMMED)) {
      ...
      cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stux, firstNode,
        new (trHeapMemory()) TR::MemoryReference(sp, gr11, TR::Compiler->om.sizeofReferenceAddress(), cg()), sp,
        cursor);
    } else {
      cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stu, firstNode,
        new (trHeapMemory()) TR::MemoryReference(sp, -size, TR::Compiler->om.sizeofReferenceAddress(), cg()), sp,
        cursor);
    }
  } else // leaf routine
  {
...
void TR::PPCSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
  ...
  for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
    ...
    cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::lfd, currentNode, machine->getRealRegister(regIndex),
      new (trHeapMemory()) TR::MemoryReference(sp, saveSize, 8, cg()), cursor);
  }
  ...
  if (savedFirst <= TR::RealRegister::LastGPR) {
    if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit()
      ...
      for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
        ...
        cursor
          = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode, machine->getRealRegister(regIndex),
            new (trHeapMemory()) TR::MemoryReference(sp, saveSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
            cursor);
      }
    else {
      ...
      cursor = generateTrg1MemInstruction(cg(),
        ...
        new (trHeapMemory()) TR::MemoryReference(sp, saveSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
        cursor);
    }
  }
...

{
  ...
  bool aix_style_linkage
    = (TR::Compiler->target.isAIX() || (TR::Compiler->target.is64Bit() && TR::Compiler->target.isLinux()));
  ...
  for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
    ...
    case TR::Int64:
        ...
        if (!cg()->canClobberNodesRegister(child, 0)) {
          if (TR::Compiler->target.is64Bit()) {
            ...
            generateTrg1Src1Instruction(
              cg(), TR::InstOpCode::mr, callNode, tempRegister, argRegister->getRegisterPair()->getHighOrder());
      ...
      {
        if (TR::Compiler->target.is64Bit()) {
          mref = getOutgoingArgumentMemRef(argSize, argRegister, TR::InstOpCode::std, pushToMemory[argIndex++],
            TR::Compiler->om.sizeofReferenceAddress());
        } else {
          if (!aix_style_linkage)
            argSize = (argSize + 4) & (~7);
          mref = getOutgoingArgumentMemRef(
            argSize, argRegister->getRegisterPair()->getHighOrder(), TR::InstOpCode::stw, pushToMemory[argIndex++], 4);
          mref = getOutgoingArgumentMemRef(argSize + 4, argRegister->getRegisterPair()->getLowOrder(),
            TR::InstOpCode::stw, pushToMemory[argIndex++], 4);
          numIntegerArgs++;
          if (!aix_style_linkage)
          ...
          if (numIntegerArgs < properties.getNumIntArgRegs()) {
            ...
            if (TR::Compiler->target.is32Bit()) {
              if ((numIntegerArgs + 1) < properties.getNumIntArgRegs())
                TR::addDependency(
                  dependencies, NULL, properties.getIntegerArgumentRegister(numIntegerArgs + 1), TR_GPR, cg());
              else {
                mref = getOutgoingArgumentMemRef(argSize, argReg, TR::InstOpCode::stfd, pushToMemory[argIndex++], 8);
              }
            }
    ...
    if (TR::Compiler->target.cpu.isBigEndian()) {
      ...
      generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, geReg,
        new (cg()->trHeapMemory()) TR::MemoryReference(cg()->getTOCBaseRegister(),
          (refNum - 1) * TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg()));
    }
  }

  generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, callNode, (uintptrj_t)callSymbol->getMethodAddress(),
  ...
  TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, uint32_t sizeOfArguments)
{
  ...
  TR_ASSERT(
    (grTOCReg != NULL), "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on Library TOC register gr2.");
  ...
  if (TR::Compiler->target.cpu.isBigEndian()) {
    ...
    generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr0,
      new (trHeapMemory())
        TR::MemoryReference(callNode->getChild(0)->getRegister(), 0, TR::Compiler->om.sizeofReferenceAddress(), cg()));
  ...
  if (TR::Compiler->target.cpu.isBigEndian())
    generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, grTOCReg,
      new (trHeapMemory()) TR::MemoryReference(callNode->getChild(0)->getRegister(),
        TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg()));
