...
void TR::PPCSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
  ...
  if (align > 1) stackIndex = (stackIndex + align - 1) & (~(align - 1));
...
int32_t TR::PPCSystemLinkage::buildArgs(
...

{
  ...
  for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
    ...
    switch (child->getDataType()) {
      ...
      case TR::Int32:
      case TR::Address:
        if (numIntegerArgs >= properties.getNumIntArgRegs()) memArgs++;
        numIntegerArgs++;
        break;
      case TR::Int64:
        if (TR::Compiler->target.is64Bit()) {
          ...
          if (aix_style_linkage) {
            ...
            if (numIntegerArgs & 1) numIntegerArgs++;
            if (numIntegerArgs >= properties.getNumIntArgRegs()) memArgs += 2;
          }
          numIntegerArgs += 2;
        }
        break;
      case TR::Float:
        if (aix_style_linkage) {
          ...
          if (numFloatArgs >= properties.getNumFloatArgRegs()) memArgs++;
        }
        numFloatArgs++;
        break;
      case TR::Double:
        if (aix_style_linkage) {
          if (TR::Compiler->target.is64Bit()) {
            ...
            if (numIntegerArgs >= properties.getNumIntArgRegs() - 1) memArgs++;
            numIntegerArgs += 2;
          }
        } else {
          if (numFloatArgs >= properties.getNumFloatArgRegs()) memArgs++;
      ...
      case TR::Int64:
        ...
        } else  // numIntegerArgs >= properties.getNumIntArgRegs()
        {
          if (TR::Compiler->target.is64Bit()) {
            ...
            if (!aix_style_linkage) argSize = (argSize + 4) & (~7);
            mref = getOutgoingArgumentMemRef(
            ...
            mref = getOutgoingArgumentMemRef(
                ...
                TR::InstOpCode::stw, pushToMemory[argIndex++], 4);
            numIntegerArgs++;
            if (!aix_style_linkage) argSize += 8;
          }
        }
        numIntegerArgs++;
        if (aix_style_linkage) argSize += 8;
        ...
        for (int r = 0; r < ((childType == TR::Float) ? 1 : 2); r++) {
          ...
          if (numFloatArgs < properties.getNumFloatArgRegs()) {
            ...
            if ((numFloatArgs == 0 && resType.isFloatingPoint())) {
              ...
              TR::addDependency(
                  dependencies, argReg,
                  properties.getFloatArgumentRegister(numFloatArgs), TR_FPR,
                  cg());
      ...
      case TR::VectorDouble:
        ...
        if (numVectorArgs < properties.getNumVectorArgRegs()) {
          ...
          if (numVectorArgs == 0) {
            ...
            TR::addDependency(
                dependencies, argReg,
                properties.getVectorArgumentRegister(numVectorArgs), TR_VRF,
                cg());
        } else {
          TR_ASSERT(false, "not handling in memory vector arguments yet\n");
        }
        numVectorArgs++;
        if (aix_style_linkage) argSize += 16;
  ...
  if (liveVMX || liveVSXScalar || liveVSXVector) {
    for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::LastFPR +
      ...
      if (numVectorArgs > 0) {
        ...
        if (i == TR::RealRegister::vsr34) continue;
      }
...
void TR::PPCSystemLinkage::buildVirtualDispatch(
  ...
  TR_ASSERT((gr0 != NULL),
            "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on "
            "scratch register gr0.");
  TR_ASSERT((grTOCReg != NULL),
            "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on "
            "Library TOC register gr2.");
  ...
  if (TR::Compiler->target.cpu.isBigEndian()) {
    // load target from FD
    generateTrg1MemInstruction(
        cg(), TR::InstOpCode::Op_load, callNode, gr0,
        new (trHeapMemory()) TR::MemoryReference(
