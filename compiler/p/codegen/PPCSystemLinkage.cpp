...
uint32_t mappedOffsetToFirstLocal(TR::CodeGenerator *cg,
  ...
  uint32_t offset = linkage.getOffsetToFirstLocal() +
                    (cg->getLargestOutgoingArgSize() > min_arg_area
                         ? cg->getLargestOutgoingArgSize()
                         : min_arg_area);
...
TR::PPCSystemLinkage::PPCSystemLinkage(TR::CodeGenerator *cg)
  ...
  _properties._properties =
      IntegersInRegisters | FloatsInRegisters | RightToLeft;
  _properties._registerFlags[TR::RealRegister::NoReg] = 0;
  _properties._registerFlags[TR::RealRegister::gr0] = 0;
  _properties._registerFlags[TR::RealRegister::gr1] =
      Preserved | PPC_Reserved; // system sp
  _properties._registerFlags[TR::RealRegister::gr2] =
      Preserved | PPC_Reserved; // TOC/system reserved
  _properties._registerFlags[TR::RealRegister::gr3] =
      IntegerReturn | IntegerArgument;
  _properties._registerFlags[TR::RealRegister::gr4] =
      IntegerReturn | IntegerArgument;
  ...
  _properties._registerFlags[TR::RealRegister::fp1] =
      FloatArgument | FloatReturn;
  _properties._registerFlags[TR::RealRegister::fp2] =
      FloatArgument | FloatReturn;
  _properties._registerFlags[TR::RealRegister::fp3] =
      FloatArgument | FloatReturn;
  _properties._registerFlags[TR::RealRegister::fp4] =
      FloatArgument | FloatReturn;
  ...
  // Note: FPRs 14-31 are preserved, however if you use them as vector registers
  // you must assume the additional 64 bits are volatile because the callee will
  // only preserve the FPR portion and the additional 64 bits will be undefined
  // after the callee returns.
  if (TR::Compiler->target.is64Bit()) {
    bool isBE = TR::Compiler->target.cpu.isBigEndian();

    // Volatile GPR (0,2-12) + FPR (0-13) + CCR (0-1,5-7) + VR (0-19) + FPR
    // (14-31) if used as vector
    ...
    if (TR::Compiler->target.isAIX()) {
      ...
      // Volatile GPR (0,3-12) + FPR (0-13) + CCR (0-1,5-7) + VR (0-19) + FPR
      // (14-31) if used as vector
...
const TR::PPCLinkageProperties &TR::PPCSystemLinkage::getProperties() {
  return _properties;
}
...
void TR::PPCSystemLinkage::initPPCRealRegisterLinkage() {
  ...
  for (icount = TR::RealRegister::LastGPR; icount >= TR::RealRegister::gr14;
       icount--)
    machine->getRealRegister((TR::RealRegister::RegNum)icount)
        ->setWeight(0xf000 - icount);

  for (icount = TR::RealRegister::FirstFPR; icount <= TR::RealRegister::fp13;
       icount++)
    machine->getRealRegister((TR::RealRegister::RegNum)icount)
        ->setWeight(icount);

  for (icount = TR::RealRegister::LastFPR; icount >= TR::RealRegister::fp14;
       icount--)
    machine->getRealRegister((TR::RealRegister::RegNum)icount)
        ->setWeight(0xf000 - icount);

  for (icount = TR::RealRegister::FirstVRF; icount <= TR::RealRegister::LastVRF;
       icount++)
    machine->getRealRegister((TR::RealRegister::RegNum)icount)
        ->setWeight(icount);

  for (icount = TR::RealRegister::FirstCCR; icount <= TR::RealRegister::LastCCR;
       icount++)
...
uint32_t TR::PPCSystemLinkage::getRightToLeft() {
...
void TR::PPCSystemLinkage::mapParameters(TR::ResolvedMethodSymbol *method,
  ...
  // XXX: This hack fixes ppc64le by saving params in the current stack frame,
  // rather than the caller's parameter save area, which may not exist
  //      This code needs to be refactored to accomodate ABIs that don't
  //      guarantee a parameter save area
  // XXX: Also, for some reason all these system linkages are marked passing
  // parms right-to-left when they should be left-to-right
  ...
  if (linkage.getRightToLeft()) {
    while (parmCursor != NULL) {
      if (saveParmsInLocalArea)
        ...
        parmCursor->setParameterOffset(offset_from_top + offsetToFirstParm +
                                       stackIndex);
      offset_from_top +=
          (parmCursor->getSize() + slot_size - 1) & (~(slot_size - 1));
    ...
    while (parmCursor != NULL) {
      parmCursor->setParameterOffset(sizeOfParameterArea - offset_from_top -
...
void TR::PPCSystemLinkage::mapStack(TR::ResolvedMethodSymbol *method) {
  ...
  if (!method->isEHAware()) {
    while (regIndex <= TR::RealRegister::LastGPR &&
           !machine->getRealRegister(regIndex)->getHasBeenAssignedInMethod())
      regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex + 1);
  }

  stackIndex += (TR::RealRegister::LastGPR - regIndex + 1) *
  ...
  if (!method->isEHAware()) {
    while (regIndex <= TR::RealRegister::LastFPR &&
           !machine->getRealRegister(regIndex)->getHasBeenAssignedInMethod())
      regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex + 1);
  }
...
void TR::PPCSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                              uint32_t &stackIndex) {
  size_t align = 1;
...
void TR::PPCSystemLinkage::createPrologue(TR::Instruction *cursor,
  ...
  cg()->setStackPointerRegister(
      machine->getRealRegister(properties.getNormalStackPointerRegister()));
  ...
  cursor = loadConstant(cg(), firstNode, (int64_t)(cg()->getTOCBase()), gr2,
                        cursor, false, false);
#endif

  if (machine->getLinkRegisterKilled()) {
    cursor = generateTrg1Instruction(cg(), TR::InstOpCode::mflr, firstNode, gr0,
                                     cursor);
  }

  if (machine->getLinkRegisterKilled())
    cursor = generateMemSrc1Instruction(
        cg(), TR::InstOpCode::Op_st, firstNode,
        new (trHeapMemory()) TR::MemoryReference(
            sp, 2 * TR::Compiler->om.sizeofReferenceAddress(),
  ...
  // Force standard saveArguments behaviour by setting fsd=false and
  // saveOnly=false
  ...
  for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
    ...
    argSize = argSize - 8;
    cursor = generateMemSrc1Instruction(
        cg(), TR::InstOpCode::stfd, firstNode,
        new (trHeapMemory()) TR::MemoryReference(sp, argSize, 8, cg()),
    ...
    if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
      ...
      for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
        ...
        cursor = generateMemSrc1Instruction(
            cg(), TR::InstOpCode::Op_st, firstNode,
            new (trHeapMemory()) TR::MemoryReference(
                sp, argSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
            machine->getRealRegister(regIndex), cursor);
      }
    else {
      ...
      cursor = generateMemSrc1Instruction(
          cg(),
          (savedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::stw
                                                    : TR::InstOpCode::stmw,
          firstNode,
          new (trHeapMemory()) TR::MemoryReference(
              sp, argSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1),
              cg()),
  ...
  if (bodySymbol->isEHAware() ||
      machine->getRealRegister(TR::RealRegister::cr2)
          ->getHasBeenAssignedInMethod() ||
      machine->getRealRegister(TR::RealRegister::cr3)
          ->getHasBeenAssignedInMethod() ||
      machine->getRealRegister(TR::RealRegister::cr4)
          ->getHasBeenAssignedInMethod()) {
    cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::mfcr, firstNode,
                                        gr0, 0xff, cursor);
    cursor = generateMemSrc1Instruction(
        cg(), TR::InstOpCode::Op_st, firstNode,
        new (trHeapMemory()) TR::MemoryReference(
            sp, TR::Compiler->om.sizeofReferenceAddress(),
            TR::Compiler->om.sizeofReferenceAddress(), cg()),
        gr0, cursor);
  }
  ...
  if (size > properties.getOffsetToFirstParm() - argSize) {
    if (size > (-LOWER_IMMED)) {
      ...
      cursor = generateMemSrc1Instruction(
          cg(), TR::InstOpCode::Op_stux, firstNode,
          new (trHeapMemory()) TR::MemoryReference(
              sp, gr11, TR::Compiler->om.sizeofReferenceAddress(), cg()),
          sp, cursor);
    } else {
      cursor = generateMemSrc1Instruction(
          cg(), TR::InstOpCode::Op_stu, firstNode,
          new (trHeapMemory()) TR::MemoryReference(
              sp, -size, TR::Compiler->om.sizeofReferenceAddress(), cg()),
          sp, cursor);
    }
  } else // leaf routine
  {
...
void TR::PPCSystemLinkage::createEpilogue(TR::Instruction *cursor) {
  ...
  for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
    ...
    cursor = generateTrg1MemInstruction(
        cg(), TR::InstOpCode::lfd, currentNode,
        machine->getRealRegister(regIndex),
        new (trHeapMemory()) TR::MemoryReference(sp, saveSize, 8, cg()),
        cursor);
  }
  ...
  if (savedFirst <= TR::RealRegister::LastGPR) {
    if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
      ...
      for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
        ...
        cursor = generateTrg1MemInstruction(
            cg(), TR::InstOpCode::Op_load, currentNode,
            machine->getRealRegister(regIndex),
            new (trHeapMemory()) TR::MemoryReference(
                sp, saveSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
            cursor);
      }
    else {
      ...
      cursor = generateTrg1MemInstruction(
          cg(),
          (savedFirst == TR::RealRegister::LastGPR) ? TR::InstOpCode::lwz
                                                    : TR::InstOpCode::lmw,
          currentNode, machine->getRealRegister(savedFirst),
          new (trHeapMemory()) TR::MemoryReference(
              sp, saveSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1),
              cg()),
          cursor);
    }
  }
  ...
  if (size > properties.getOffsetToFirstParm() - saveSize) {
    if (size > UPPER_IMMED) {
      ...
      cursor = generateTrg1Src2Instruction(cg(), TR::InstOpCode::add,
                                           currentNode, sp, sp2, gr11, cursor);
    } else {
      cursor = generateTrg1Src1ImmInstruction(
          cg(), TR::InstOpCode::addi2, currentNode, sp, sp2, size, cursor);
    }
  }

  if (machine->getLinkRegisterKilled()) {
    cursor = generateTrg1MemInstruction(
        cg(), TR::InstOpCode::Op_load, currentNode, gr0,
        new (trHeapMemory()) TR::MemoryReference(
            sp, 2 * TR::Compiler->om.sizeofReferenceAddress(),
            TR::Compiler->om.sizeofReferenceAddress(), cg()),
        cursor);
    cursor = generateSrc1Instruction(cg(), TR::InstOpCode::mtlr, currentNode,
                                     gr0, 0, cursor);
  }
  ...
  if (bodySymbol->isEHAware() ||
    ...
    cursor = generateTrg1MemInstruction(
        ...
        new (trHeapMemory()) TR::MemoryReference(
            ...
            TR::Compiler->om.sizeofReferenceAddress(), cg()),
        cursor);
    cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::mtcrf,
                                        currentNode, gr0, 0xff, cursor);
  }
}

int32_t
TR::PPCSystemLinkage::buildArgs(TR::Node *callNode,
...

{
  ...
  TR_Array<TR::Register *> &tempLongRegisters =
      cg()->getTransientLongRegisters();
  TR::Symbol *callSymbol = callNode->getSymbolReference()->getSymbol();

  uint32_t firstArgumentChild = callNode->getFirstArgumentIndex();
  bool aix_style_linkage =
      (TR::Compiler->target.isAIX() ||
       (TR::Compiler->target.is64Bit() && TR::Compiler->target.isLinux()));

  /* Step 1 - figure out how many arguments are going to be spilled to memory
   * i.e. not in registers */
  for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
    ...
    case TR::Aggregate: {
      ...
      size = (size + sizeof(uintptr_t) - 1) &
             (~(sizeof(uintptr_t) - 1)); // round up the size
      ...
      if (numIntegerArgs >= properties.getNumIntArgRegs())
        ...
        memArgs +=
            (properties.getNumIntArgRegs() - numIntegerArgs) > slots
                ? 0
  ...
  // From here, down, any new stack allocations will expire / die when the
  // function returns
  ...
  for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
    ...
    case TR::Address: // have to do something for GC maps here
      ...
      if (numIntegerArgs < properties.getNumIntArgRegs()) {
        ...
        if (checkSplit && !cg()->canClobberNodesRegister(child, 0)) {
          ...
          generateTrg1Src1Instruction(cg(), TR::InstOpCode::mr, callNode,
        ...
        if (numIntegerArgs == 0 &&
        ...
        } else if (TR::Compiler->target.is32Bit() && numIntegerArgs == 1 &&
                   resType.isInt64()) {
          ...
          TR::addDependency(
              dependencies, argRegister,
              properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR,
              cg());
        }
      } else // numIntegerArgs >= properties.getNumIntArgRegs()
      {
        mref = getOutgoingArgumentMemRef(
            argSize, argRegister, TR::InstOpCode::Op_st,
            pushToMemory[argIndex++],
            TR::Compiler->om.sizeofReferenceAddress());
        if (!aix_style_linkage)
    ...
    case TR::Int64:
      ...
      if (numIntegerArgs < properties.getNumIntArgRegs()) {
        if (!cg()->canClobberNodesRegister(child, 0)) {
          if (TR::Compiler->target.is64Bit()) {
            ...
            generateTrg1Src1Instruction(
                cg(), TR::InstOpCode::mr, callNode, tempRegister,
                argRegister->getRegisterPair()->getHighOrder());
            argRegister = cg()->allocateRegisterPair(
                argRegister->getRegisterPair()->getLowOrder(), tempRegister);
        ...
        if (numIntegerArgs == 0 &&
          ...
          if (TR::Compiler->target.is64Bit())
            ...
            TR::addDependency(
                dependencies, argRegister->getRegisterPair()->getHighOrder(),
                properties.getIntegerArgumentRegister(numIntegerArgs), TR_GPR,
                cg());
        }
        if (TR::Compiler->target.is32Bit()) {
          if (numIntegerArgs < properties.getNumIntArgRegs() - 1) {
            if (!cg()->canClobberNodesRegister(child, 0)) {
              TR::Register *over_lowReg =
                  argRegister->getRegisterPair()->getLowOrder();
              tempRegister = cg()->allocateRegister();
              generateTrg1Src1Instruction(cg(), TR::InstOpCode::mr, callNode,
                                          tempRegister, over_lowReg);
      ...
      } else // numIntegerArgs >= properties.getNumIntArgRegs()
      {
        if (TR::Compiler->target.is64Bit()) {
          mref = getOutgoingArgumentMemRef(
              argSize, argRegister, TR::InstOpCode::std,
              pushToMemory[argIndex++],
              TR::Compiler->om.sizeofReferenceAddress());
        } else {
          if (!aix_style_linkage)
            argSize = (argSize + 4) & (~7);
          mref = getOutgoingArgumentMemRef(
              argSize, argRegister->getRegisterPair()->getHighOrder(),
              TR::InstOpCode::stw, pushToMemory[argIndex++], 4);
          mref = getOutgoingArgumentMemRef(
              argSize + 4, argRegister->getRegisterPair()->getLowOrder(),
              TR::InstOpCode::stw, pushToMemory[argIndex++], 4);
    ...
    case TR::Float:
      ...
      for (int r = 0; r < ((childType == TR::Float) ? 1 : 2); r++) {
        ...
        if (childType == TR::Float)
          ...
          argReg = (r == 0) ? argRegister->getHighOrder()
                            : argRegister->getLowOrder();

        if (numFloatArgs < properties.getNumFloatArgRegs()) {
          if (!cg()->canClobberNodesRegister(child, 0)) {
            tempRegister = cg()->allocateRegister(TR_FPR);
            generateTrg1Src1Instruction(cg(), TR::InstOpCode::fmr, callNode,
                                        tempRegister, argReg);
            argReg = tempRegister;
          }
          if ((numFloatArgs == 0 && resType.isFloatingPoint())) {
            ...
            dependencies->addPreCondition(argReg, (r == 0)
                                                      ? TR::RealRegister::fp1
                                                      : TR::RealRegister::fp2);
            dependencies->addPostCondition(resultReg,
                                           (r == 0) ? TR::RealRegister::fp1
                                                    : TR::RealRegister::fp2);
          } else
            TR::addDependency(dependencies, argReg,
                              properties.getFloatArgumentRegister(numFloatArgs),
                              TR_FPR, cg());
        } else if (!aix_style_linkage)
        // numFloatArgs >= properties.getNumFloatArgRegs()
        {
          mref =
              getOutgoingArgumentMemRef(argSize, argReg, TR::InstOpCode::stfs,
        ...
        if (aix_style_linkage) {
          ...
          } else // numIntegerArgs >= properties.getNumIntArgRegs()
          {
            if (TR::Compiler->target.is64Bit() &&
              ...
              mref = getOutgoingArgumentMemRef(argSize + 4, argReg,
                                               TR::InstOpCode::stfs,
                                               pushToMemory[argIndex++], 4);
            } else {
              mref = getOutgoingArgumentMemRef(argSize, argReg,
                                               TR::InstOpCode::stfs,
                                               pushToMemory[argIndex++], 4);
            }
          }
    ...
    case TR::Double:
      ...
      for (int r = 0; r < ((childType == TR::Double) ? 1 : 2); r++) {
        ...
        // numFloatArgs >= properties.getNumFloatArgRegs()
        {
          argSize = (argSize + 4) & (~7);
          mref =
              getOutgoingArgumentMemRef(argSize, argReg, TR::InstOpCode::stfd,
    ...
    case TR::VectorDouble:
      ...
      if (numVectorArgs < properties.getNumVectorArgRegs()) {
        if (!cg()->canClobberNodesRegister(child, 0)) {
          ...
          generateTrg1Src1Instruction(cg(), TR::InstOpCode::xxlor, callNode,
                                      tempRegister, argReg);
          argReg = tempRegister;
        }
        if (numVectorArgs == 0) {
          ...
          TR::addDependency(dependencies, argReg,
                            properties.getVectorArgumentRegister(numVectorArgs),
                            TR_VRF, cg());
      } else {
  ...
  TR::TreeEvaluator::buildArgsProcessFEDependencies(callNode, cg(),
                                                    dependencies);
#endif

  int32_t floatRegsUsed = (numFloatArgs > properties.getNumFloatArgRegs())
                              ? properties.getNumFloatArgRegs()
                              : numFloatArgs;

  bool isHelper = false;
  if (callNode->getSymbolReference()->getReferenceNumber() ==
      TR_PPCVectorLogDouble) {
    isHelper = true;
  }

  if (liveVMX || liveVSXScalar || liveVSXVector) {
    for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::LastFPR +
                                        1);
         i <= TR::RealRegister::LastVSR; i++) {
      if (numVectorArgs > 0) {
        TR_ASSERT(numVectorArgs == 1,
                  "Handling only one vector argument for now\n");
  ...
  for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 +
                                      floatRegsUsed + 1);
       i <= TR::RealRegister::LastFPR; i++) {
    if (!properties.getPreserved((TR::RealRegister::RegNum)i) ||
        liveVSXVector) {
      TR::addDependency(dependencies, NULL, (TR::RealRegister::RegNum)i, TR_FPR,
                        cg());
    }
  }
  ...
  if (memArgs > 0) {
    for (argIndex = 0; argIndex < memArgs; argIndex++) {
      ...
      generateMemSrc1Instruction(cg(), pushToMemory[argIndex].opCode, callNode,
                                 pushToMemory[argIndex].argMemory, aReg);
...
void TR::PPCSystemLinkage::buildDirectCall(
    TR::Node *callNode, TR::SymbolReference *callSymRef,
    TR::RegisterDependencyConditions *dependencies,
    const TR::PPCLinkageProperties &pp, int32_t argSize) {
  ...
  if (aix_style_linkage) {
    ...
    // This is wrong with regard to where system TOC is restored from, but
    // happens to be right for JIT for the time being.
    if (TR::Compiler->target.cpu.isBigEndian()) {
#if !defined(JITTEST)
      TR::TreeEvaluator::restoreTOCRegister(callNode, cg(), dependencies);
#endif
    } else {
      // For Little Endian, load target's Global Entry Point into r12, TOC will
      // be restored at branch target.
      TR::Register *geReg =
          dependencies->searchPreConditionRegister(TR::RealRegister::gr12);
      generateTrg1MemInstruction(
          cg(), TR::InstOpCode::Op_load, callNode, geReg,
  ...
  generateDepImmSymInstruction(
      cg(), TR::InstOpCode::bl, callNode,
      (uintptrj_t)callSymbol->getMethodAddress(), dependencies,
      callSymRef ? callSymRef : callNode->getSymbolReference());

  // Bug fix: JIT doesn't need to restore caller's system TOC since there is no
  // infrastructure to generate
  //         ABI-conforming module of dynamic code.  Plus, linux32 has no system
  //         TOC.
#if defined(JITTEST)
  TR::TreeEvaluator::restoreTOCRegister(callNode, cg(), dependencies);
#endif
...
TR::Register *TR::PPCSystemLinkage::buildDirectDispatch(TR::Node *callNode) {
  ...
  TR::RegisterDependencyConditions *dependencies = new (trHeapMemory())
      TR::RegisterDependencyConditions(pp.getNumberOfDependencyGPRegisters(),
  ...
  case TR::lcall: {
    if (TR::Compiler->target.is64Bit())
      returnRegister =
          dependencies->searchPostConditionRegister(pp.getLongReturnRegister());
    else {
  ...
  case TR::dcall:
    returnRegister =
        dependencies->searchPostConditionRegister(pp.getFloatReturnRegister());
    break;
  case TR::vcall:
    returnRegister =
        dependencies->searchPostConditionRegister(pp.getVectorReturnRegister());
    break;
  case TR::call:
    returnRegister = NULL;
    break;
  ...
  dependencies->stopUsingDepRegs(
      cg(), lowReg == NULL ? returnRegister : highReg, lowReg);
...
void TR::PPCSystemLinkage::buildVirtualDispatch(
  ...
  bool aix_style_linkage =
      (TR::Compiler->target.isAIX() ||
       (TR::Compiler->target.is64Bit() && TR::Compiler->target.isLinux()));
  TR_ASSERT(callNode->getSymbolReference()
                ->getSymbol()
                ->castToMethodSymbol()
                ->isComputed(),
  ...
  if (!aix_style_linkage) {
    TR_ASSERT(false,
              "Don't know how to dispatch to 32-bit Linux SystemLinkage\n");
    return;
  }
...
#if defined(DEBUG)
  // Ensure we set dependencies on the following registers when building the
  // arguments to the call. Result register
  TR::Register *gr3 =
      dependencies->searchPreConditionRegister(TR::RealRegister::gr3);
  TR_ASSERT((gr3 != NULL), "TR::PPCSystemLinkage::buildVirtualDispatch no "
                           "dependence set on return register gr3.");
#endif
  ...
  // Scratch register.
  TR::Register *gr0 =
      dependencies->searchPreConditionRegister(TR::RealRegister::gr0);
  // Library TOC Register
  TR::Register *grTOCReg =
      TR::TreeEvaluator::retrieveTOCRegister(callNode, cg(), dependencies);
  // Native Stack Pointer (C Stack)
  TR::RealRegister *grSysStackReg = cg()->machine()->getRealRegister(
      properties.getNormalStackPointerRegister());

  TR_ASSERT((gr0 != NULL), "TR::PPCSystemLinkage::buildVirtualDispatch no "
                           "dependence set on scratch register gr0.");
  TR_ASSERT((grTOCReg != NULL), "TR::PPCSystemLinkage::buildVirtualDispatch no "
                                "dependence set on Library TOC register gr2.");
  TR_ASSERT((grSysStackReg != NULL),
            "TR::PPCSystemLinkage::buildVirtualDispatch no dependence set on "
            "Native C Stack register gr1.");
  ...
  if (TR::Compiler->target.cpu.isBigEndian()) {
    ...
    generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr0,
                               new (trHeapMemory()) TR::MemoryReference(
                                   callNode->getChild(0)->getRegister(), 0,
                                   TR::Compiler->om.sizeofReferenceAddress(),
                                   cg()));
    ...
    TR::Register *gr12 =
        dependencies->searchPreConditionRegister(TR::RealRegister::gr12);
    generateTrg1Src1Instruction(cg(), TR::InstOpCode::mr, callNode, gr12,
                                callNode->getChild(0)->getRegister());
    targetRegister = gr12;
  }

  generateSrc1Instruction(cg(), TR::InstOpCode::mtctr, callNode, targetRegister,
                          0);

  if (TR::Compiler->target.cpu.isBigEndian())
    generateTrg1MemInstruction(
        cg(), TR::InstOpCode::Op_load, callNode, grTOCReg,
        new (trHeapMemory()) TR::MemoryReference(
            callNode->getChild(0)->getRegister(),
            TR::Compiler->om.sizeofReferenceAddress(),
            TR::Compiler->om.sizeofReferenceAddress(), cg()));
