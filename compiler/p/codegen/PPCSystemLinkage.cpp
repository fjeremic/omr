...
uint32_t
mappedOffsetToFirstLocal(TR::CodeGenerator* cg,
...
void
TR::PPCSystemLinkage::mapParameters(TR::ResolvedMethodSymbol* method,
...
void
TR::PPCSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p,
                                         uint32_t& stackIndex)
{
  size_t align = 1;
                                     ...
                                     List<TR::ParameterSymbol>& parmList)
{
  ...
  cursor = loadConstant(
    cg(), firstNode, (int64_t)(cg()->getTOCBase()), gr2, cursor, false, false);
#endif

  if (machine->getLinkRegisterKilled()) {
    cursor = generateTrg1Instruction(
      cg(), TR::InstOpCode::mflr, firstNode, gr0, cursor);
  }
  ...
  if (savedFirst <= TR::RealRegister::LastGPR) {
    if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
      ...
      cursor = generateMemSrc1Instruction(
        ...
        firstNode,
        new (trHeapMemory()) TR::MemoryReference(
          sp, argSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
  ...
  if (bodySymbol->isEHAware() ||
    ...
    cursor = generateTrg1ImmInstruction(
      cg(), TR::InstOpCode::mfcr, firstNode, gr0, 0xff, cursor);
    cursor = generateMemSrc1Instruction(
      cg(),
      TR::InstOpCode::Op_st,
      firstNode,
...
TR::PPCSystemLinkage::createEpilogue(TR::Instruction* cursor)
{
  ...
  if (savedFirst <= TR::RealRegister::LastGPR) {
    if (TR::Compiler->target.cpu.id() == TR_PPCgp ||
      ...
      cursor = generateTrg1MemInstruction(
        ...
        machine->getRealRegister(savedFirst),
        new (trHeapMemory()) TR::MemoryReference(
          sp, saveSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
        cursor);
    }
  }
  ...
  if (size > properties.getOffsetToFirstParm() - saveSize) {
    if (size > UPPER_IMMED) {
      ...
      cursor = generateTrg1Src2Instruction(
        cg(), TR::InstOpCode::add, currentNode, sp, sp2, gr11, cursor);
    } else {
    ...
    cursor = generateTrg1MemInstruction(
      ...
      new (trHeapMemory())
        TR::MemoryReference(sp,
                            ...
                            cg()),
      cursor);
    cursor = generateSrc1Instruction(
      cg(), TR::InstOpCode::mtlr, currentNode, gr0, 0, cursor);
  }
  ...
  if (bodySymbol->isEHAware() ||
    ...
    cursor = generateTrg1MemInstruction(
      ...
      cursor);
    cursor = generateTrg1ImmInstruction(
      cg(), TR::InstOpCode::mtcrf, currentNode, gr0, 0xff, cursor);
  }
}
...

{
        ...
        if (numIntegerArgs < properties.getNumIntArgRegs()) {
          ...
          if (checkSplit && !cg()->canClobberNodesRegister(child, 0)) {
            ...
            generateTrg1Src1Instruction(
              cg(), TR::InstOpCode::mr, callNode, tempRegister, argRegister);
        ...
        for (int r = 0; r < ((childType == TR::Float) ? 1 : 2); r++) {
          ...
          if (numFloatArgs < properties.getNumFloatArgRegs()) {
            if (!cg()->canClobberNodesRegister(child, 0)) {
              ...
              generateTrg1Src1Instruction(
                cg(), TR::InstOpCode::fmr, callNode, tempRegister, argReg);
              argReg = tempRegister;
            }
          ...
          {
            mref = getOutgoingArgumentMemRef(argSize,
                                             argReg,
                                             TR::InstOpCode::stfs,
                                             pushToMemory[argIndex++],
                                             4);
          ...
          if (aix_style_linkage) {
            ...
            } else // numIntegerArgs >= properties.getNumIntArgRegs()
            {
              if (TR::Compiler->target.is64Bit() &&
                ...
                mref = getOutgoingArgumentMemRef(argSize + 4,
                                                 argReg,
                                                 TR::InstOpCode::stfs,
                                                 pushToMemory[argIndex++],
                                                 4);
      ...
      case TR::VectorDouble:
        ...
        if (numVectorArgs < properties.getNumVectorArgRegs()) {
          if (!cg()->canClobberNodesRegister(child, 0)) {
            ...
            generateTrg1Src1Instruction(
              cg(), TR::InstOpCode::xxlor, callNode, tempRegister, argReg);
            argReg = tempRegister;
          }
          if (numVectorArgs == 0) {
  ...
  TR::TreeEvaluator::buildArgsProcessFEDependencies(
    callNode, cg(), dependencies);
#endif
  ...
  if (liveVMX || liveVSXScalar || liveVSXVector) {
    for (i =
           (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::LastFPR + 1);
         i <= TR::RealRegister::LastVSR;
         i++) {
      if (numVectorArgs > 0) {
      ...
      if (!properties.getPreserved((TR::RealRegister::RegNum)i) || !isHelper) {
        TR::addDependency(
          dependencies, NULL, (TR::RealRegister::RegNum)i, TR_VSX_SCALAR, cg());
      }
    }
  }
  ...
  for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fp0 +
    ...
    if (!properties.getPreserved((TR::RealRegister::RegNum)i) ||
        liveVSXVector) {
      TR::addDependency(
        dependencies, NULL, (TR::RealRegister::RegNum)i, TR_FPR, cg());
    }
  }
  ...
  if (memArgs > 0) {
    for (argIndex = 0; argIndex < memArgs; argIndex++) {
      ...
      generateMemSrc1Instruction(cg(),
                                 pushToMemory[argIndex].opCode,
                                 callNode,
                                 pushToMemory[argIndex].argMemory,
                                 aReg);
...
void
TR::PPCSystemLinkage::buildDirectCall(
  TR::Node* callNode,
...
TR::Register*
TR::PPCSystemLinkage::buildDirectDispatch(TR::Node* callNode)
{
...
void
TR::PPCSystemLinkage::buildVirtualDispatch(
  TR::Node* callNode,
  TR::RegisterDependencyConditions* dependencies,
  uint32_t sizeOfArguments)
{
  ...
  generateSrc1Instruction(
    cg(), TR::InstOpCode::mtctr, callNode, targetRegister, 0);
...
TR::Register*
TR::PPCSystemLinkage::buildIndirectDispatch(TR::Node* callNode)
{
  const TR::PPCLinkageProperties& pp = getProperties();
  TR::RegisterDependencyConditions* dependencies = new (trHeapMemory())
    TR::RegisterDependencyConditions(pp.getNumberOfDependencyGPRegisters(),
...
void
TR::PPCSystemLinkage::setParameterLinkageRegisterIndex(
...
intptrj_t
TR::PPCSystemLinkage::entryPointFromCompiledMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}

intptrj_t
TR::PPCSystemLinkage::entryPointFromInterpretedMethod()
{
  return reinterpret_cast<intptrj_t>(cg()->getCodeStart());
}
