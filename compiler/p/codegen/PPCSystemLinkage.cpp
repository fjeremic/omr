    ...
    const TR::PPCLinkageProperties& linkage)
{
    ...
    uint32_t min_arg_area = machine->getLinkRegisterKilled() ? (TR::Compiler->target.is64Bit() ? 8 * 8 : 8 * 4)
                                                             : 0;

    uint32_t offset = linkage.getOffsetToFirstLocal() + (cg->getLargestOutgoingArgSize() > min_arg_area ? cg->getLargestOutgoingArgSize() : min_arg_area);
...
void TR::PPCSystemLinkage::mapParameters(
    ...
    List<TR::ParameterSymbol>& parmList)
{
    ...
    if (linkage.getRightToLeft()) {
        ...
        while (parmCursor != NULL) {
            parmCursor->setParameterOffset(sizeOfParameterArea - offset_from_top - parmCursor->getSize() + stackIndex + offsetToFirstParm);
            offset_from_top += (parmCursor->getSize() + slot_size - 1) & (~(slot_size - 1));
            parmCursor = parameterIterator.getNext();
        }
    }
...
void TR::PPCSystemLinkage::mapSingleAutomatic(
...
void TR::PPCSystemLinkage::createPrologue(
    ...
    List<TR::ParameterSymbol>& parmList)
{
    ...
    if (savedFirst <= TR::RealRegister::LastGPR) {
        if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit() || (!comp()->getOption(TR_OptimizeForSpace) && TR::RealRegister::LastGPR - savedFirst <= 3))
    ...
    if (bodySymbol->isEHAware() || machine->getRealRegister(TR::RealRegister::cr2)->getHasBeenAssignedInMethod() || machine->getRealRegister(TR::RealRegister::cr3)->getHasBeenAssignedInMethod() || machine->getRealRegister(TR::RealRegister::cr4)->getHasBeenAssignedInMethod()) {
...
void TR::PPCSystemLinkage::setParameterLinkageRegisterIndex(TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
    ...
    while ((paramCursor != NULL) && ((numIntArgs < properties.getNumIntArgRegs()) || (numFloatArgs < properties.getNumFloatArgRegs()))) {
        int32_t index = -1;
