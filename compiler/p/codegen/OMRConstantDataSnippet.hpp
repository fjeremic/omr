...
public:
  ...
  PPCConstant(TR::CodeGenerator *cg, T v, TR::Node *n = NULL, bool ps = false)
      : _instructionPairs(cg->trMemory()), _value(v), _node(n),
  ...
  void addValueRequest(TR::Instruction *n0, TR::Instruction *n1,
...
public:
  ...
  ConstantDataSnippet(TR::CodeGenerator *cg)
      : _cg(cg), _doubleConstants(cg->trMemory()),
  ...
  int32_t addConstantRequest(void *v, TR::DataType type,
                             TR::Instruction *nibble0, TR::Instruction *nibble1,
                             TR::Instruction *nibble2, TR::Instruction *nibble3,
                             TR::Node *node, bool isUnloadablePicSite);

  void emitFloatingPointConstant(TR_Array<TR::Instruction *> &requestors,
                                 uint8_t *codeCursor, int32_t count);

  virtual void emitAddressConstant(PPCConstant<intptrj_t> *acursor,
                                   TR_Array<TR::Instruction *> &requestors,
                                   uint8_t *codeCursor, int32_t count);

  bool getRequestorsFromNibble(TR::Instruction *nibble, TR::Instruction **q,
                               bool remove);
