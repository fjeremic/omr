...
void TR::PPCLabelInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
  ...
  if (((isBranchOp() && !usesCountRegister()) || isVirtualGuardNOPInstruction())
    ...
    TR_ASSERT(
      oi, "Could not find PPCOutOfLineCodeSection stream from label.  instr=%p, label=%p\n", this, getLabelSymbol());
...
void TR::PPCTrg1Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned, bool excludeGPR0)
{
  ...
  if (excludeGPR0 && (assignedRegister != NULL)
    ...
    if ((alternativeRegister = machine->findBestFreeRegister(this, kindOfRegister, excludeGPR0, false, virtualRegister))
      == NULL) {
...
void TR::PPCTrg1Src1Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
  ...
  if (getOpCodeValue() == TR::InstOpCode::addi || getOpCodeValue() == TR::InstOpCode::addis
  ...
  if (excludeGPR0 && (assignedRegister != NULL)
    ...
    if ((alternativeRegister = machine->findBestFreeRegister(this, kindOfRegister, excludeGPR0, false, sourceVirtual))
      == NULL) {
      cg()->setRegisterAssignmentFlag(TR_RegisterSpilled);
  ...
  , _memoryReference(mf)
{
  ...
  TR_ASSERT(_hint == PPCOpProp_NoHint
      || ((op == TR::InstOpCode::lwarx || op == TR::InstOpCode::ldarx) && (_hint == PPCOpProp_LoadReserveAtomicUpdate)
...
void TR::PPCControlFlowInstruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
  ...
  case TR::InstOpCode::iflong:
    ...
    if (getOpCode2Value() == TR::InstOpCode::bne)
      cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(cg(), TR::InstOpCode::bne, currentNode,
    ...
    if (getOpCode2Value() == TR::InstOpCode::beq) {
      cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                 cg(), getOpCode2Value(), currentNode, getLabelSymbol(), getTargetRegister(0), cursor));
  ...
  case TR::InstOpCode::setblong:
    ...
    cg()->traceRAInstruction(cursor = generateTrg1Src2Instruction(cg(), getOpCode2Value(), currentNode,
                               getTargetRegister(0), getSourceRegister(0), getSourceRegister(2), cursor));
    cg()->traceRAInstruction(
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), 1, cursor));
    cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                               cg(), TR::InstOpCode::bne, currentNode, label1, getTargetRegister(0), cursor));
    cg()->traceRAInstruction(cursor = generateTrg1Src2Instruction(cg(), TR::InstOpCode::cmpl4, currentNode,
                               getTargetRegister(0), getSourceRegister(1), getSourceRegister(3), cursor));
    cg()->traceRAInstruction(
      cursor = generateLabelInstruction(cg(), TR::InstOpCode::label, currentNode, label1, cursor));
    cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                               cg(), getOpCode3Value(), currentNode, label2, getTargetRegister(0), cursor));
    cg()->traceRAInstruction(
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), 0, cursor));
    cg()->traceRAInstruction(
      cursor = generateLabelInstruction(cg(), TR::InstOpCode::label, currentNode, label2, cursor));
    break;
  case TR::InstOpCode::setbool:
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9 && getCmpOpValue() == TR::InstOpCode::fcmpu
      && (getOpCode2Value() == TR::InstOpCode::blt || getOpCode2Value() == TR::InstOpCode::bgt)) {

      /* Used with: dcmpgt, dcmplt, fcmpgt and fcmplt*/
      /* Determine relationship between the two inputs, translate correct return value into GT bit of the Target
      Condition Register, clear the LT field of the target condition register and finally set the result. */

      int64_t imm
        = (TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RT | TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RA
      ...
      cg()->traceRAInstruction(cursor = generateTrg1Src2Instruction(cg(), TR::InstOpCode::fcmpu, currentNode,
                                 getTargetRegister(0), getSourceRegister(0), getSourceRegister(1), cursor));

      if (getOpCode2Value() != TR::InstOpCode::bgt)
        cg()->traceRAInstruction(
          cursor = generateTrg1Src2ImmInstruction(cg(), TR::InstOpCode::cror, currentNode, getTargetRegister(0),
            getTargetRegister(0), getTargetRegister(0), getSourceImmediate(2), cursor));

      cg()->traceRAInstruction(cursor = generateTrg1Src2ImmInstruction(cg(), TR::InstOpCode::crxor, currentNode,
                                 getTargetRegister(0), getTargetRegister(0), getTargetRegister(0), imm, cursor));
      cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), TR::InstOpCode::setb, currentNode,
                                 getTargetRegister(1), getTargetRegister(0), cursor));

    } else {
      cg()->traceRAInstruction(cursor = generateTrg1Src2Instruction(cg(), getCmpOpValue(), currentNode,
                                 getTargetRegister(0), getSourceRegister(0), getSourceRegister(1), cursor));
      cg()->traceRAInstruction(
        cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), 1, cursor));
      cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                 cg(), getOpCode2Value(), currentNode, label2, getTargetRegister(0), cursor));
      cg()->traceRAInstruction(
        cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), 0, cursor));
      cg()->traceRAInstruction(
        cursor = generateLabelInstruction(cg(), TR::InstOpCode::label, currentNode, label2, cursor));
  ...
  case TR::InstOpCode::lcmp:
    ...
    cg()->traceRAInstruction(
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), 1, cursor));
    cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                               cg(), TR::InstOpCode::bgt, currentNode, label2, getTargetRegister(0), cursor));
    cg()->traceRAInstruction(
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), -1, cursor));
    cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                               cg(), TR::InstOpCode::blt, currentNode, label2, getTargetRegister(0), cursor));
    cg()->traceRAInstruction(cursor = generateTrg1Src2Instruction(cg(), TR::InstOpCode::cmpl4, currentNode,
                               getTargetRegister(0), getSourceRegister(1), getSourceRegister(3), cursor));
    cg()->traceRAInstruction(
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(1), 1, cursor));
  ...
  case TR::InstOpCode::d2i:
    ...
    cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), TR::InstOpCode::fctiwz, currentNode,
                               getTargetRegister(1), getSourceRegister(0), cursor));
    cg()->traceRAInstruction(
      cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(2), 0, cursor));
    ...
    if (TR::Compiler->target.cpu.id() < TR_PPCp8) {
      ...
      cg()->traceRAInstruction(cursor
        = generateMemSrc1Instruction(cg(), TR::InstOpCode::stfd, currentNode, tempMR, getTargetRegister(1), cursor));
    }
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp8)
      ...
      cg()->traceRAInstruction(
        cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::lwz, currentNode, getTargetRegister(2),
          new (cg()->trHeapMemory()) TR::MemoryReference(currentNode, *tempMR, 4, 4, cg()), cursor));
    cg()->traceRAInstruction(
      cursor = generateLabelInstruction(cg(), TR::InstOpCode::label, currentNode, label2, cursor));
  ...
  case TR::InstOpCode::d2l:
    ...
    if (TR::Compiler->target.is64Bit())
      ...
      cg()->traceRAInstruction(
        cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(2), 0, cursor));
      cg()->traceRAInstruction(
        cursor = generateTrg1ImmInstruction(cg(), TR::InstOpCode::li, currentNode, getTargetRegister(3), 0, cursor));
    }
    ...
    if (TR::Compiler->target.is64Bit()) {
      if (TR::Compiler->target.cpu.id() >= TR_PPCp8 && TR::Compiler->target.is64Bit())
        ...
        cg()->traceRAInstruction(cursor
          = generateTrg1MemInstruction(cg(), TR::InstOpCode::ld, currentNode, getTargetRegister(2), tempMR, cursor));
    } else {
      cg()->traceRAInstruction(
        cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::lwz, currentNode, getTargetRegister(2),
          new (cg()->trHeapMemory()) TR::MemoryReference(currentNode, *tempMR, 0, 4, cg()), cursor));
      cg()->traceRAInstruction(
  ...
  case TR::InstOpCode::iternary: {
    cg()->traceRAInstruction(cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::cmpi4, currentNode,
                               getTargetRegister(0), getSourceRegister(0), 0, cursor));

    if (useRegPairForResult()) {
      cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), getOpCode2Value(), currentNode,
                                 getTargetRegister(1), getSourceRegister(1), cursor));
      cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), getOpCode2Value(), currentNode,
                                 getTargetRegister(2), getSourceRegister(2), cursor));
    } else {
      cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), getOpCode2Value(), currentNode,
                                 getTargetRegister(1), getSourceRegister(1), cursor));
    }
    ...
    if (useRegPairForResult()) {
      if (useRegPairForCond()) {
        cg()->traceRAInstruction(cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::cmpi4, currentNode,
                                   getTargetRegister(0), getSourceRegister(5), 0, cursor));
        cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                   cg(), TR::InstOpCode::bne, currentNode, label2, getTargetRegister(0), cursor));
      }
    } else {
      if (useRegPairForCond()) {
        cg()->traceRAInstruction(cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::cmpi4, currentNode,
                                   getTargetRegister(0), getSourceRegister(3), 0, cursor));
        cg()->traceRAInstruction(cursor = generateConditionalBranchInstruction(
                                   cg(), TR::InstOpCode::bne, currentNode, label2, getTargetRegister(0), cursor));
      }
    }
    ...
    if (useRegPairForResult()) {
      cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), getOpCode2Value(), currentNode,
                                 getTargetRegister(1), getSourceRegister(3), cursor));
      cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), getOpCode2Value(), currentNode,
                                 getTargetRegister(2), getSourceRegister(4), cursor));
    } else {
      cg()->traceRAInstruction(cursor = generateTrg1Src1Instruction(cg(), getOpCode2Value(), currentNode,
                                 getTargetRegister(1), getSourceRegister(2), cursor));
    }
    cg()->traceRAInstruction(
      cursor = generateLabelInstruction(cg(), TR::InstOpCode::label, currentNode, label2, cursor));
  } break;
