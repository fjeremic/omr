...
void TR::PPCTrg1Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned, bool excludeGPR0)
{
    ...
    if (excludeGPR0 && (assignedRegister != NULL) && (toRealRegister(assignedRegister) == machine->getRealRegister(TR::RealRegister::gr0))) {
...
{
    if (reg == getTargetRegister() || reg == getSource1Register() || reg == getSource2Register() || reg == getSource3Register()) {
        return true;
    }
    return false;
}
...
void TR::PPCMemSrc1Instruction::assignRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    bool excludeGPR0 = mref->isUsingStaticTOC() && (mref->getUnresolvedSnippet() == NULL) && (mref->getTOCOffset() > UPPER_IMMED || mref->getTOCOffset() < LOWER_IMMED);
    ...
    , _memoryReference(mf)
{
    ...
    TR_ASSERT(_hint == PPCOpProp_NoHint || ((op == TR::InstOpCode::lwarx || op == TR::InstOpCode::ldarx) && (_hint == PPCOpProp_LoadReserveAtomicUpdate) || (_hint == PPCOpProp_LoadReserveExclusiveAccess)), "Hint set for the wrong instruction");
