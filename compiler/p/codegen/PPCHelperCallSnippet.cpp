...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCHelperCallSnippet *snippet) {
  ...
  trfprintf(pOutFile, "%s \t" POINTER_PRINTF_FORMAT "\t\t; %s %s",
            restartLabel ? "bl" : "b", (intptrj_t)cursor + distance,
  ...
  if (restartLabel) {
    ...
    distance = *((int32_t *)cursor) & 0x03fffffc;
    distance = (distance << 6) >> 6; // sign extend
    trfprintf(pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t\t; Restart",
...
uint8_t *TR_Debug::print(TR::FILE *pOutFile,
  ...
  printPrefix(pOutFile, NULL, cursor, 4);
  trfprintf(pOutFile, "li \t%s, %d", getName(lengthReg),
