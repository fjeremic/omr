...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCHelperCallSnippet* snippet)
{
  ...
  trfprintf(pOutFile, "%s \t" POINTER_PRINTF_FORMAT "\t\t; %s %s", restartLabel ? "bl" : "b",
    (intptrj_t)cursor + distance, getName(snippet->getDestination()), info);
...
uint8_t* TR::PPCHelperCallSnippet::genHelperCall(uint8_t* buffer)
{
  ...
  if (cg()->directCallRequiresTrampoline(helperAddress, (intptrj_t)buffer)) {
    helperAddress
      = TR::CodeCacheManager::instance()->findHelperTrampoline(getDestination()->getReferenceNumber(), (void*)buffer);
