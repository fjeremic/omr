...
uint8_t*
TR::PPCHelperCallSnippet::emitSnippetBody()
{
...
TR_Debug::print(TR::FILE* pOutFile, TR::PPCHelperCallSnippet* snippet)
{
  ...
  if (snippet->getKind() == TR::Snippet::IsArrayCopyCall) {
    ...
    printSnippetLabel(
      pOutFile, snippet->getSnippetLabel(), cursor, "Helper Call Snippet");
  }
  ...
  if (restartLabel) {
    ...
    distance = (distance << 6) >> 6; // sign extend
    trfprintf(pOutFile,
              "b \t" POINTER_PRINTF_FORMAT "\t\t; Restart",
              (intptrj_t)cursor + distance);
  }
}

uint32_t
TR::PPCHelperCallSnippet::getLength(int32_t estimatedSnippetStart)
{
  return getHelperCallLength();
}

uint8_t*
TR::PPCHelperCallSnippet::genHelperCall(uint8_t* buffer)
{
  intptrj_t helperAddress = (intptrj_t)getDestination()
                              ->getSymbol()
                              ->castToMethodSymbol()
                              ->getMethodAddress();
  ...
  if (cg()->directCallRequiresTrampoline(helperAddress, (intptrj_t)buffer)) {
    ...
    TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinIFormBranchRange(
                      helperAddress, (intptrj_t)buffer),
...
uint32_t
TR::PPCHelperCallSnippet::getHelperCallLength()
{
  return ((_restartLabel == NULL) ? 4 : 8);
}

uint8_t*
TR::PPCArrayCopyCallSnippet::emitSnippetBody()
{
  ...
  int64_t byteLen =
    (lengthNode->getType().isInt32() ? lengthNode->getInt()
                ...
                uint8_t* cursor)
{
  printSnippetLabel(pOutFile,
                    ...
                    cursor,
                    "ArrayCopy Helper Call Snippet");

  TR::RealRegister* lengthReg =
    _cg->machine()->getRealRegister(snippet->getLengthRegNum());

  printPrefix(pOutFile, NULL, cursor, 4);
  trfprintf(pOutFile,
            "li \t%s, %d",
            getName(lengthReg),
            *((int32_t*)cursor) & 0x0000ffff);
  cursor += 4;
...
uint32_t
TR::PPCArrayCopyCallSnippet::getLength(int32_t estimatedSnippetStart)
{
