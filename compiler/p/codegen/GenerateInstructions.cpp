    ...
    TR::Register* treg, TR::Register* s1reg, TR::Register* s2reg, TR::Instruction* preced)
{
    ...
    if (!disableFlipCompare && TR::Compiler->target.cpu.id() == TR_PPCp6 && TR::InstOpCode(op).isCompare() && n->getOpCode().isBranch() && n->getOpCode().isBooleanCompare()) {
        treg->resetFlippedCCR();
        if (s1reg->containsInternalPointer() || (!TR::InstOpCode(op).isFloat() && registerRecentlyWritten(s1reg, preced ? preced : cg->getAppendInstruction()))) {
...
void generateZeroExtendInstruction(TR::Node* node,
    ...
    TR::CodeGenerator* cg)
{
    TR_ASSERT((TR::Compiler->target.is64Bit() && bitsInTarget > 0 && bitsInTarget < 64) || (TR::Compiler->target.is32Bit() && bitsInTarget > 0 && bitsInTarget < 32),
