  ...
  TR::Register* reg0, TR::Register* reg1, TR::Register* reg2, TR::Register* reg3, TR::Instruction* cursor)
{
  ...
  bool checkp8DirectMove
    = TR::Compiler->target.cpu.id() >= TR_PPCp8 && !disableDirectMove && TR::Compiler->target.cpu.getPPCSupportsVSX();
...
TR::Instruction* generateImm2Instruction(
  TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n, uint32_t imm, uint32_t imm2, TR::Instruction* preced)
{
  if (preced)
...
TR::Instruction* generateDepConditionalBranchInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op,
  TR::Node* n, TR::LabelSymbol* sym, TR::Register* cr, TR::RegisterDependencyConditions* cond, TR::Instruction* preced)
{
  if (cr->isFlippedCCR())
    op = flipBranch(cg, op);
  ...
  TR::Register* treg, TR::Register* s1reg, TR::Register* s2reg, TR::Instruction* preced)
{
  ...
  if (!disableFlipCompare && TR::Compiler->target.cpu.id() == TR_PPCp6 && TR::InstOpCode(op).isCompare()
    && n->getOpCode().isBranch() && n->getOpCode().isBooleanCompare()) {
    treg->resetFlippedCCR();
    if (s1reg->containsInternalPointer()
      || (!TR::InstOpCode(op).isFloat()
        && registerRecentlyWritten(s1reg, preced ? preced : cg->getAppendInstruction()))) {
  ...
  TR::Register* srcReg, int32_t shiftAmount, TR::Instruction* preced)
{
  ...
  if (preced)
    return new (cg->trHeapMemory())
      TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rlwinm, n, trgReg, srcReg, shiftAmount % 32, temp, preced, cg);
  return new (cg->trHeapMemory())
  ...
  TR::Register* srcReg, int32_t shiftAmount, TR::Instruction* preced)
{
  ...
  if (preced)
    return new (cg->trHeapMemory())
      TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rldicr, n, trgReg, srcReg, shiftAmount % 64, mask, preced, cg);
  return new (cg->trHeapMemory())
...
TR::Instruction* generateDepImmSymInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
  uintptrj_t imm, TR::RegisterDependencyConditions* cond, TR::SymbolReference* sr, TR::Snippet* s,
  TR::Instruction* preced)
{
  if (preced)
...
TR::InstOpCode::Mnemonic flipBranch(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op)
{
  ...
  default:
    TR_ASSERT(
      0, "Cannot use this branch opcode to branch off the comparison of an internal pointer : %d\n", (int32_t)op);
  ...
  TR::Node* node, TR::Register* trgReg, TR::Register* srcReg, int32_t bitsInTarget, TR::CodeGenerator* cg)
{
  ...
  generateTrg1Src1Imm2Instruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::rldicl : TR::InstOpCode::rlwinm,
