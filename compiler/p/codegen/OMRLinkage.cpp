...
void
OMR::Power::Linkage::mapStack(TR::ResolvedMethodSymbol* method)
{}

void
OMR::Power::Linkage::mapSingleAutomatic(TR::AutomaticSymbol* p,
                                        uint32_t& stackIndex)
{}
...
void
OMR::Power::Linkage::setParameterLinkageRegisterIndex(
  TR::ResolvedMethodSymbol* method)
{}

bool
OMR::Power::Linkage::hasToBeOnStack(TR::ParameterSymbol* parm)
{
  return (false);
}

TR::MemoryReference*
OMR::Power::Linkage::getOutgoingArgumentMemRef(int32_t argSize,
                                               TR::Register* argReg,
                                               ...
                                               uint32_t length)
{
  ...
  TR::MemoryReference* result =
    new (self()->trHeapMemory()) TR::MemoryReference(
      ...
      argSize + properties.getOffsetToFirstParm(),
      length,
      self()->cg());
...
// saveOnly=false
TR::Instruction*
OMR::Power::Linkage::saveArguments(TR::Instruction* cursor,
                                   bool fsd,
                                   bool saveOnly)
{
      ...
      if (ai >= 0 && (!fsd || !saveOnly)) {
        ...
        if (TR::Compiler->target.is32Bit() && type.isInt64()) {
          ...
          } else if (regNum + 1 != aiLow) // Low part needs to be moved
          {
            if (freeScratchable.isSet(aiLow)) {
              cursor =
                generateTrg1Src1Instruction(self()->cg(),
                                            TR::InstOpCode::mr,
                                            firstNode,
                                            REAL_REGISTER(REGNUM(aiLow)),
                                            REAL_REGISTER(REGNUM(regNum + 1)),
...
TR::Instruction*
OMR::Power::Linkage::loadUpArguments(TR::Instruction* cursor)
{
...
TR::Instruction*
OMR::Power::Linkage::flushArguments(TR::Instruction* cursor)
{
  TR::Machine* machine = self()->machine();
  TR::RealRegister* stackPtr = self()->cg()->getStackPointerRegister();
  TR::ResolvedMethodSymbol* bodySymbol =
...
TR::Register*
OMR::Power::Linkage::pushIntegerWordArg(TR::Node* child)
{
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    ...
    loadConstant(self()->cg(),
                 child,
                 (int32_t)child->get64bitIntegralValue(),
                 pushRegister);
...
TR::Register*
OMR::Power::Linkage::pushAddressArg(TR::Node* child)
{
  TR::Register* pushRegister = NULL;
  TR_ASSERT(child->getDataType() == TR::Address, "assumption violated");
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    ...
    if (isClass && self()->cg()->wantToPatchClassPointer(
      ...
      loadAddressConstantInSnippet(self()->cg(),
                                   child,
                                   child->getAddress(),
                                   pushRegister,
                                   NULL,
...
TR::Register*
OMR::Power::Linkage::pushThis(TR::Node* child)
{
...
TR::Register*
OMR::Power::Linkage::pushLongArg(TR::Node* child)
{
  TR::Register* pushRegister = NULL;
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    if (TR::Compiler->target.is64Bit()) {
...
TR::Register*
OMR::Power::Linkage::pushFloatArg(TR::Node* child)
{
...
TR::Register*
OMR::Power::Linkage::pushDoubleArg(TR::Node* child)
{
  TR::Register* pushRegister = self()->cg()->evaluate(child);
  self()->cg()->decReferenceCount(child);
  return (pushRegister);
