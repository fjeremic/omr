...
void OMR::Power::Linkage::mapSingleAutomatic(TR::AutomaticSymbol *p,
                                             uint32_t &stackIndex) {}

void OMR::Power::Linkage::initPPCRealRegisterLinkage() {}

void OMR::Power::Linkage::setParameterLinkageRegisterIndex(
...
TR::MemoryReference *OMR::Power::Linkage::getOutgoingArgumentMemRef(
    int32_t argSize, TR::Register *argReg, TR::InstOpCode::Mnemonic opCode,
    TR::PPCMemoryArgument &memArg, uint32_t length) {
...
TR::Instruction *OMR::Power::Linkage::saveArguments(TR::Instruction *cursor,
                                                    bool fsd, bool saveOnly) {
  return self()->saveArguments(
      cursor, fsd, saveOnly,
      self()->comp()->getJittedMethodSymbol()->getParameterList());
}

TR::Instruction *OMR::Power::Linkage::saveArguments(
    TR::Instruction *cursor, bool fsd, bool saveOnly,
...
#define REGNUM(ri) ((TR::RealRegister::RegNum)(ri))
  ...
  // no additional conditions were added with the intention of keeping the code
  // easier to read and not full of if conditions
  ...
  // second, go through all parameters and reset registers that are actually
  // used
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    if (lri >= 0) {
      ...
      bool twoRegs = (TR::Compiler->target.is32Bit() && type.isInt64() &&
                      lri < properties.getNumIntArgRegs() - 1);
      ...
      if (paramCursor->isReferencedParameter()) {
        ...
        if (twoRegs) freeScratchable.reset(regNum + 1);
      }
    }
  }
  ...
  for (paramCursor = paramIterator.getFirst(); paramCursor != NULL;
    ...
    if (lri >= 0) {
      ...
      // Do not save arguments to the stack if in Full Speed Debug and saveOnly
      // is not set. If not in Full Speed Debug, the arguments will be saved.
      if (((ai < 0 || self()->hasToBeOnStack(paramCursor)) && !fsd) ||
          (fsd && saveOnly)) {
        ...
        cursor = generateMemSrc1Instruction(
            self()->cg(), op, firstNode,
            new (self()->trHeapMemory())
                TR::MemoryReference(stackPtr, offset, length, self()->cg()),
            REAL_REGISTER(regNum), cursor);

        if (twoRegs) {
          cursor = generateMemSrc1Instruction(
              self()->cg(), TR::InstOpCode::stw, firstNode,
              new (self()->trHeapMemory())
                  TR::MemoryReference(stackPtr, offset + 4, 4, self()->cg()),
              REAL_REGISTER(REGNUM(regNum + 1)), cursor);
          if (ai < 0) freeScratchable.set(regNum + 1);
        }

        if (ai < 0) freeScratchable.set(regNum);
      }
      ...
      if (ai >= 0 && (!fsd || !saveOnly)) {
        if (regNum != ai)  // Equal assignment: do nothing
        {
          if (freeScratchable.isSet(ai)) {
            cursor = generateTrg1Src1Instruction(
                self()->cg(),
                (type.isFloatingPoint()) ? TR::InstOpCode::fmr
                                         : TR::InstOpCode::mr,
                firstNode, REAL_REGISTER(REGNUM(ai)), REAL_REGISTER(regNum),
                cursor);
            freeScratchable.reset(ai);
            freeScratchable.set(regNum);
          } else  // The status of target global register is unclear (i.e. it is
                  // a arg reg)
          {
        ...
        if (TR::Compiler->target.is32Bit() && type.isInt64()) {
          ...
          if (!twoRegs)  // Low part needs to come from memory
          {
            ...
            if (freeScratchable.isSet(aiLow)) {
              cursor = generateTrg1MemInstruction(
                  self()->cg(), TR::InstOpCode::lwz, firstNode,
                  REAL_REGISTER(REGNUM(aiLow)),
                  new (self()->trHeapMemory())
                      TR::MemoryReference(stackPtr, offset, 4, self()->cg()),
                  cursor);
          ...
          } else if (regNum + 1 != aiLow)  // Low part needs to be moved
          {
            if (freeScratchable.isSet(aiLow)) {
              cursor = generateTrg1Src1Instruction(
                  self()->cg(), TR::InstOpCode::mr, firstNode,
                  REAL_REGISTER(REGNUM(aiLow)),
                  REAL_REGISTER(REGNUM(regNum + 1)), cursor);
              freeScratchable.reset(aiLow);
    ...
    // Don't process if in Full Speed Debug and saveOnly is set
             ...
             (!fsd || !saveOnly))  // lri<0: arg needs to come from memory
    {
      switch (dtype) {
        case TR::Int8:
        case TR::Int16:
        ...
        case TR::Address:
          if (freeScratchable.isSet(ai)) {
            cursor = generateTrg1MemInstruction(
                self()->cg(), TR::InstOpCode::Op_load, firstNode,
                REAL_REGISTER(REGNUM(ai)),
                new (self()->trHeapMemory()) TR::MemoryReference(
                    stackPtr, offset, TR::Compiler->om.sizeofReferenceAddress(),
                    self()->cg()),
                cursor);
            freeScratchable.reset(ai);
          } else {
          ...
          if (TR::Compiler->target.is64Bit()) {
            if (freeScratchable.isSet(ai)) {
              cursor = generateTrg1MemInstruction(
                  ...
                  REAL_REGISTER(REGNUM(ai)),
                  new (self()->trHeapMemory())
                      TR::MemoryReference(stackPtr, offset, 8, self()->cg()),
                  cursor);
              freeScratchable.reset(ai);
            } else {
              busyMoves[0][busyIndex] = offset;
        ...
        case TR::Float:
          if (freeScratchable.isSet(ai)) {
            cursor = generateTrg1MemInstruction(
                self()->cg(), TR::InstOpCode::lfs, firstNode,
                REAL_REGISTER(REGNUM(ai)),
                new (self()->trHeapMemory())
                    TR::MemoryReference(stackPtr, offset, 4, self()->cg()),
                cursor);
        ...
        case TR::Double:
          if (freeScratchable.isSet(ai)) {
            cursor = generateTrg1MemInstruction(
                self()->cg(), TR::InstOpCode::lfd, firstNode,
                REAL_REGISTER(REGNUM(ai)),
                new (self()->trHeapMemory())
                    TR::MemoryReference(stackPtr, offset, 8, self()->cg()),
                cursor);
        ...
        if (!(target < 0) && freeScratchable.isSet(target)) {
          ...
          switch (busyMoves[2][i1]) {
            case 0:
              cursor = generateTrg1Src1Instruction(
                  self()->cg(),
                  (source <= TR::RealRegister::LastGPR) ? TR::InstOpCode::mr
                                                        : TR::InstOpCode::fmr,
                  firstNode, REAL_REGISTER(REGNUM(target)),
                  REAL_REGISTER(REGNUM(source)), cursor);
          ...
          if (busyMoves[2][i1] != 0) {
            cursor = generateTrg1MemInstruction(
                self()->cg(), op, firstNode, REAL_REGISTER(REGNUM(target)),
                new (self()->trHeapMemory())
                    TR::MemoryReference(stackPtr, source, length, self()->cg()),
                cursor);
          }
    ...
    switch (paramCursor->getDataType()) {
      ...
      case TR::Int32:
        if (hasToLoadFromStack && numIntArgs < properties.getNumIntArgRegs()) {
          ...
          cursor = generateTrg1MemInstruction(
              self()->cg(), TR::InstOpCode::lwz, firstNode, argRegister,
              new (self()->trHeapMemory())
                  TR::MemoryReference(stackPtr, offset, 4, self()->cg()),
              cursor);
        }
      ...
      case TR::Address:
        if (numIntArgs < properties.getNumIntArgRegs()) {
          argRegister = machine->getRealRegister(
              properties.getIntegerArgumentRegister(numIntArgs));
          cursor = generateTrg1MemInstruction(
              self()->cg(), TR::InstOpCode::Op_load, firstNode, argRegister,
              new (self()->trHeapMemory()) TR::MemoryReference(
                  stackPtr, offset, TR::Compiler->om.sizeofReferenceAddress(),
                  self()->cg()),
              cursor);
      ...
      case TR::Int64:
        if (hasToLoadFromStack && numIntArgs < properties.getNumIntArgRegs()) {
          ...
          if (TR::Compiler->target.is64Bit())
            cursor = generateTrg1MemInstruction(
                self()->cg(), TR::InstOpCode::ld, firstNode, argRegister,
                new (self()->trHeapMemory())
                    TR::MemoryReference(stackPtr, offset, 8, self()->cg()),
                cursor);
          else {
      ...
      case TR::Double:
        if (hasToLoadFromStack &&
          ...
          cursor = generateTrg1MemInstruction(
              self()->cg(), op, firstNode, argRegister,
              new (self()->trHeapMemory())
                  TR::MemoryReference(stackPtr, offset, length, self()->cg()),
              cursor);
        }
...
TR::Instruction *OMR::Power::Linkage::flushArguments(TR::Instruction *cursor) {
  ...
  while ((paramCursor != NULL) &&
    ...
    // If parm is referenced or required to be on stack (i.e. FSD), we have to
    // flush.
    bool hasToStoreToStack = paramCursor->isReferencedParameter() ||
                             paramCursor->isParmHasToBeOnStack();
      ...
      case TR::Address:
        if (numIntArgs < properties.getNumIntArgRegs()) {
          argRegister = machine->getRealRegister(
              properties.getIntegerArgumentRegister(numIntArgs));
          cursor = generateMemSrc1Instruction(
              self()->cg(), TR::InstOpCode::Op_st, firstNode,
              new (self()->trHeapMemory()) TR::MemoryReference(
                  stackPtr, offset, TR::Compiler->om.sizeofReferenceAddress(),
                  self()->cg()),
              argRegister, cursor);
...
TR::Register *OMR::Power::Linkage::pushIntegerWordArg(TR::Node *child) {
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    ...
    loadConstant(self()->cg(), child, (int32_t)child->get64bitIntegralValue(),
                 pushRegister);
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    ...
    if (isClass && self()->cg()->wantToPatchClassPointer(
      ...
      loadAddressConstantInSnippet(self()->cg(), child, child->getAddress(),
                                   pushRegister, NULL, TR::InstOpCode::Op_load,
                                   NULL, NULL);
    else {
      if (child->isMethodPointerConstant())
        loadAddressConstant(self()->cg(),
                            self()->cg()->comp()->compileRelocatableCode(),
                            child, child->getAddress(), pushRegister, NULL,
                            false, TR_RamMethodSequence);
      else
        loadAddressConstant(self()->cg(),
                            self()->cg()->comp()->compileRelocatableCode(),
                            child, child->getAddress(), pushRegister);
    }
    child->setRegister(pushRegister);
  } else {
    pushRegister = self()->cg()->evaluate(child);
...
TR::Register *OMR::Power::Linkage::pushLongArg(TR::Node *child) {
  ...
  if (child->getRegister() == NULL && child->getOpCode().isLoadConst()) {
    if (TR::Compiler->target.is64Bit()) {
      ...
      pushRegister =
          self()->cg()->allocateRegisterPair(lowRegister, highRegister);
...
TR_ReturnInfo OMR::Power::Linkage::getReturnInfoFromReturnType(
