...
static void ifFloatEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic branchOp1,
                             TR::InstOpCode::Mnemonic branchOp2,
                             TR::CodeGenerator *cg);
static TR::Register *compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::Mnemonic branchOp1, TR::InstOpCode::Mnemonic branchOp2,
    int64_t imm, TR::Node *node, TR::CodeGenerator *cg);
static TR::Register *singlePrecisionEvaluator(TR::Node *node,
                                              TR::InstOpCode::Mnemonic OpCode,
                                              TR::CodeGenerator *cg);
static TR::Register *doublePrecisionEvaluator(TR::Node *node,
                                              TR::InstOpCode::Mnemonic OpCode,
                                              TR::CodeGenerator *cg);

TR::Register *OMR::Power::TreeEvaluator::ibits2fEvaluator(
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(child, 4, cg);
#ifdef J9_PROJECT_SPECIFIC
    if (node->getFirstChild()->getOpCodeValue() == TR::iriload) {
      tempMR->forceIndexedForm(child, cg);
      generateTrg1MemInstruction(cg, TR::InstOpCode::lwbrx, node, target,
                                 tempMR);
    } else
#endif
    ...
    generateMvFprGprInstructions(cg, node, gprSp2fpr,
                                 TR::Compiler->target.is64Bit(), target,
                                 cg->evaluate(child));
...
TR::Register *OMR::Power::TreeEvaluator::fbits2iEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfs, node, floatReg,
    ...
    floatReg = TR::Compiler->target.cpu.id() >= TR_PPCp8
                   ? cg->gprClobberEvaluate(child)
                   : cg->evaluate(child);
    generateMvFprGprInstructions(
        cg, node, fpr2gprSp, TR::Compiler->target.is64Bit(), target, floatReg);
  ...
  if (node->normalizeNanValues()) {
    ...
    TR::RegisterDependencyConditions *deps = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(2, 2, cg->trMemory());
    ...
    generateTrg1Src2Instruction(cg, TR::InstOpCode::fcmpu, node, condReg,
                                floatReg, floatReg);
    generateLabelInstruction(cg, TR::InstOpCode::label, node,
                             nanNormalizeStartLabel);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq,
                                         PPCOpProp_BranchLikely, node,
    ...
    generateDepLabelInstruction(cg, TR::InstOpCode::label, node,
                                nanNormalizeEndLabel, deps);

    cg->stopUsingRegister(condReg);
  }

  if (!childEval) cg->stopUsingRegister(floatReg);

  node->setRegister(target);
  return target;
}

TR::Register *OMR::Power::TreeEvaluator::lbits2dEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
      child->getOpCode().isLoadVar()) {
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(child, 8, cg);
#ifdef J9_PROJECT_SPECIFIC
    if (child->getOpCodeValue() == TR::irlload &&
        TR::Compiler->target.is64Bit())  // 64-bit only
    {
      TR::Register *tmpReg = cg->allocateRegister();
      tempMR->forceIndexedForm(child, cg);
      generateTrg1MemInstruction(cg, TR::InstOpCode::ldbrx, node, tmpReg,
                                 tempMR);
      generateMvFprGprInstructions(cg, node, gpr2fprHost64, true, target,
                                   tmpReg);
      cg->stopUsingRegister(tmpReg);
    } else if (child->getOpCodeValue() == TR::irlload &&
               TR::Compiler->target.is32Bit())  // 32-bit
    {
      TR::Register *highReg = cg->allocateRegister();
      TR::Register *lowReg = cg->allocateRegister();
      TR::MemoryReference *tempMRLoad1 = new (cg->trHeapMemory())
          TR::MemoryReference(child, *tempMR, 0, 4, cg);
      TR::MemoryReference *tempMRLoad2 = new (cg->trHeapMemory())
          TR::MemoryReference(child, *tempMR, 4, 4, cg);
      ...
      generateTrg1MemInstruction(cg, TR::InstOpCode::lwbrx, node, highReg,
                                 tempMRLoad2);
      generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, target,
                                   highReg, lowReg);
...
TR::Register *OMR::Power::TreeEvaluator::dbits2lEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      TR::MemoryReference *tempMR2 =
          new (cg->trHeapMemory()) TR::MemoryReference(node, *tempMR, 0, 8, cg);
    ...
    if (TR::Compiler->target.is64Bit()) {
      lReg = cg->allocateRegister();
      generateMvFprGprInstructions(cg, node, fpr2gprHost64, true, lReg,
                                   doubleReg);
    } else {
      highReg = cg->allocateRegister();
      lowReg = cg->allocateRegister();
      if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
        TR::Register *tmp1 = cg->allocateRegister(TR_FPR);
        generateMvFprGprInstructions(cg, node, fpr2gprHost32, false, highReg,
                                     lowReg, doubleReg, tmp1);
        cg->stopUsingRegister(tmp1);
      } else
        generateMvFprGprInstructions(cg, node, fpr2gprHost32, false, highReg,
                                     lowReg, doubleReg);
  ...
  if (node->normalizeNanValues()) {
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, highReg,
                                 0x7ff8);
  ...
  if (!childEval) cg->stopUsingRegister(doubleReg);
...
static TR::Register *fconstHandler(TR::Node *node, TR::CodeGenerator *cg,
                                   float value) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    offset = cg->findOrCreateFloatConstant(&value, TR::Float, NULL, NULL, NULL,
                                           NULL);
    if (offset != PTOC_FULL_INDEX) {
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addis, node,
                                       srcRegister, cg->getTOCBaseRegister(),
                                       HI_VALUE(offset));
        generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, trgRegister,
                                   new (cg->trHeapMemory()) TR::MemoryReference(
                                       srcRegister, LO_VALUE(offset), 4, cg));
        cg->stopUsingRegister(srcRegister);
      } else {
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::lfs, node, trgRegister,
            new (cg->trHeapMemory())
                TR::MemoryReference(cg->getTOCBaseRegister(), offset, 4, cg));
      }
    }
  }
  ...
  if (TR::Compiler->target.is32Bit() || offset == PTOC_FULL_INDEX) {
    srcRegister = cg->allocateRegister();
    if (TR::Compiler->target.is64Bit()) tempReg = cg->allocateRegister();
    fixedSeqMemAccess(cg, node, 0, q, trgRegister, srcRegister,
                      TR::InstOpCode::lfs, 4, NULL, tempReg);
    cg->findOrCreateFloatConstant(&value, TR::Float, q[0], q[1], q[2], q[3]);
    cg->stopUsingRegister(srcRegister);
    if (TR::Compiler->target.is64Bit()) cg->stopUsingRegister(tempReg);
  }
  node->setRegister(trgRegister);
  return trgRegister;
}

TR::Register *OMR::Power::TreeEvaluator::fconstEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return fconstHandler(node, cg, node->getFloat());
}

TR::Register *OMR::Power::TreeEvaluator::dconstEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (TR::Compiler->target.is64Bit()) {
    offset = cg->findOrCreateFloatConstant(&value, TR::Double, NULL, NULL, NULL,
                                           NULL);
    if (offset != PTOC_FULL_INDEX) {
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        srcRegister = cg->allocateRegister();
        ...
        TR::MemoryReference *memRef = new (cg->trHeapMemory())
            TR::MemoryReference(srcRegister, LO_VALUE(offset), 8, cg);
        if (splats) memRef->forceIndexedForm(node, cg);
  ...
  if (TR::Compiler->target.is32Bit() || offset == PTOC_FULL_INDEX) {
    ...
    if (TR::Compiler->target.is64Bit()) tempReg = cg->allocateRegister(TR_GPR);
    fixedSeqMemAccess(cg, node, 0, q, trgRegister, srcRegister, opcode, 8, NULL,
                      tempReg);
    cg->stopUsingRegister(srcRegister);
    if (TR::Compiler->target.is64Bit()) cg->stopUsingRegister(tempReg);
...
TR::Register *OMR::Power::TreeEvaluator::floadEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Register *tempReg =
      node->setRegister(cg->allocateSinglePrecisionRegister());
  TR::MemoryReference *tempMR;
  bool needSync = node->getSymbolReference()->getSymbol()->isSyncVolatile() &&
                  TR::Compiler->target.isSMP();

  // If the reference is volatile or potentially volatile, the layout needs to
  // be fixed for patching if it turns out to be not a volatile atfer all.
...
TR::Register *OMR::Power::TreeEvaluator::dloadHelper(
    TR::Node *node, TR::CodeGenerator *cg, TR::Register *tempReg,
    TR::InstOpCode::Mnemonic opcode) {
  TR::Compilation *comp = cg->comp();
  TR::MemoryReference *tempMR;
  bool needSync = node->getSymbolReference()->getSymbol()->isSyncVolatile() &&
  ...
  if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
    ...
    TR::SymbolReference *vrlRef =
        comp->getSymRefTab()->findOrCreateVolatileReadDoubleSymbolRef(
            comp->getMethodSymbol());

    if (node->getSymbolReference()->isUnresolved()) {
#ifdef J9_PROJECT_SPECIFIC
      // NOTE NOTE -- If the reference is volatile or potentially volatile, the
      // layout needs to be fixed for patching if it turns out to be not a
      // volatile after all.
      tempMR = new (cg->trHeapMemory()) TR::MemoryReference(node, 8, cg);

      if (opcode == TR::InstOpCode::lxvdsx) tempMR->forceIndexedForm(node, cg);
    ...
    // If the reference is volatile or potentially volatile, the layout needs to
    // be fixed for patching if it turns out to be not a volatile after all.
    tempMR = new (cg->trHeapMemory()) TR::MemoryReference(node, 8, cg);
...
TR::Register *OMR::Power::TreeEvaluator::dloadEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::vsplatsEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  ...
  if (node->getDataType() == TR::VectorInt8) {
    ...
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(node, temp, 1, cg);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvd2x, node, trgReg,
                               tempMR);
#if defined(__LITTLE_ENDIAN__)
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::vspltb, node, trgReg,
                                   trgReg, 7);
#else
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::vspltb, node, trgReg,
                                   trgReg, 0);
    ...
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(node, temp, 2, cg);
    TR::Register *srcReg = cg->evaluate(child);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvd2x, node, trgReg,
                               tempMR);
#if defined(__LITTLE_ENDIAN__)
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::vsplth, node, trgReg,
                                   trgReg, 3);
#else
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::vsplth, node, trgReg,
                                   trgReg, 0);
    ...
    if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8 &&
      ...
      generateMvFprGprInstructions(cg, node, gprLow2fpr, false, resReg,
                                   tempReg);
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::xxspltw, node, resReg,
                                     resReg, 0x1);
    } else {
      TR::SymbolReference *localTemp = cg->allocateLocalTemp(TR::Int32);
      generateMemSrc1Instruction(
          cg, TR::InstOpCode::stw, node,
          new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 4, cg),
          tempReg);

      TR::MemoryReference *tempMR =
          new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 4, cg);
      tempMR->forceIndexedForm(node, cg);
      generateTrg1MemInstruction(cg, TR::InstOpCode::lxsdx, node, resReg,
                                 tempMR);
      tempMR->decNodeReferenceCounts(cg);
#if defined(__LITTLE_ENDIAN__)
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::xxspltw, node, resReg,
    ...
    if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8 &&
        TR::Compiler->target.cpu.getPPCSupportsVSX()) {
      if (TR::Compiler->target.is64Bit()) {
        generateMvFprGprInstructions(cg, node, gpr2fprHost64, false, trgReg,
                                     srcReg);
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxpermdi, node,
                                       trgReg, trgReg, trgReg, 0x0);
      } else {
        TR::Register *tempVectorReg = cg->allocateRegister(TR_VSX_VECTOR);
        generateMvFprGprInstructions(cg, node, gpr2fprHost32, false, trgReg,
                                     srcReg->getHighOrder(),
      ...
      TR::MemoryReference *tempMR =
          new (cg->trHeapMemory()) TR::MemoryReference(node, temp, 8, cg);

      if (TR::Compiler->target.is64Bit()) {
        generateMemSrc1Instruction(cg, TR::InstOpCode::std, node, tempMR,
                                   srcReg);
      } else {
        TR::MemoryReference *tempMR2 = new (cg->trHeapMemory())
            TR::MemoryReference(node, *tempMR, 4, 4, cg);
        generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, tempMR,
                                   srcReg->getHighOrder());
        generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, tempMR2,
                                   srcReg->getLowOrder());
      }
      ...
      tempMR =
          new (cg->trHeapMemory()) TR::MemoryReference(NULL, tmpReg, 16, cg);
      cg->stopUsingRegister(tmpReg);
      generateTrg1MemInstruction(cg, TR::InstOpCode::lxvdsx, node, trgReg,
                                 tempMR);
    }
    ...
    generateTrg1Src1Instruction(cg, TR::InstOpCode::xscvdpsp, node, trgReg,
                                srcReg);
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::xxspltw, node, trgReg,
                                   trgReg, 0);
  ...
  if (child->getOpCode().isLoadConst()) {
    ...
    cg->decReferenceCount(child);
    return resReg;
  } else if (child->getRegister() == NULL && child->getOpCode().isLoadVar()) {
    ...
    TR::Register *resReg =
        node->setRegister(cg->allocateRegister(TR_VSX_VECTOR));
    return TR::TreeEvaluator::dloadHelper(child, cg, resReg,
                                          TR::InstOpCode::lxvdsx);
  } else {
    TR::Register *srcReg = cg->evaluate(child);
    TR::Register *resReg =
        node->setRegister(cg->allocateRegister(TR_VSX_VECTOR));
    generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxpermdi, node, resReg,
                                   srcReg, srcReg, 0x0);
...
TR::Register *OMR::Power::TreeEvaluator::vdgetelemEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (elem == 1)
      generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxsldwi, node, resReg,
                                     srcReg, srcReg, 0x2);
    else
  ...
  generateTrg1Src2Instruction(cg, TR::InstOpCode::xxlor, node, resReg,
                              vectorReg, vectorReg);
  generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, jumpLabel,
                                       condReg);
  generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxsldwi, node, resReg,
                                 vectorReg, vectorReg, 0x2);
...
TR::Register *OMR::Power::TreeEvaluator::vdsetelemEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  if (thirdChild->getOpCode().isLoadConst()) {
    ...
    if (secondChild->getRegister() == NULL &&
      ...
      TR::TreeEvaluator::dloadHelper(secondChild, cg, resReg,
                                     TR::InstOpCode::lxsdx);
    } else {
    ...
    if (elem == 0)
      generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxpermdi, node, resReg,
                                     resReg, vectorReg, 0x1);
    else
      generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxpermdi, node, resReg,
                                     vectorReg, resReg, 0x0);
  ...
  generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxpermdi, node, resReg,
                                 valueReg, vectorReg, 0x1);
  generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel,
                                       condReg);
  generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxpermdi, node, resReg,
                                 vectorReg, valueReg, 0x0);

  TR::RegisterDependencyConditions *dep = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(0, 5, cg->trMemory());
  dep->addPostCondition(vectorReg, TR::RealRegister::NoReg);
  dep->addPostCondition(idxReg, TR::RealRegister::NoReg);
...
TR::Register *OMR::Power::TreeEvaluator::fstoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::dstoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  bool indirect = node->getOpCode().isIndirect();
  int childIndex = indirect ? 1 : 0;
  TR::Node *child = node->getChild(childIndex);
  ...
  if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
    TR::Register *addrReg = cg->allocateRegister();
    TR::SymbolReference *vrlRef =
        comp->getSymRefTab()->findOrCreateVolatileWriteDoubleSymbolRef(
            comp->getMethodSymbol());
    TR::RegisterDependencyConditions *dependencies = new (cg->trHeapMemory())
        TR::RegisterDependencyConditions(5, 5, cg->trMemory());
    if (isUnresolved) {
    ...
    TR::addDependency(dependencies, valueReg, TR::RealRegister::fp0, TR_FPR,
                      cg);
    TR::addDependency(dependencies, addrReg, TR::RealRegister::gr3, TR_GPR, cg);
    TR::addDependency(dependencies, NULL, TR::RealRegister::gr11, TR_GPR, cg);
    if (isUnresolved) {
...
TR::Register *OMR::Power::TreeEvaluator::freturnEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
static bool isFPStrictMul(TR::Node *node, TR::Compilation *comp) {
  if (!node->getOpCode().isMul() || !node->isFPStrictCompliant() ||
      node->getRegister())
    return false;

  if (node->getReferenceCount() < 2 && node->getRegister() == NULL) return true;

  node->setIsFPStrictCompliant(
      false);  // need to set this otherwise children get incorrectly bumped
  return false;
}

/** Generate a fused multiply add from the tree (A * B) + C, where addNode is
 * the + node and mulNode the * subtree.
 */
static TR::Register *generateFusedMultiplyAdd(TR::Node *addNode,
                                              TR::InstOpCode::Mnemonic OpCode,
                                              TR::CodeGenerator *cg) {
  ...
  TR_ASSERT(mulNode->getReferenceCount() < 2,
            "Mul node 0x%p reference count %d >= 2\n", mulNode,
            mulNode->getReferenceCount());
  TR_ASSERT(mulNode->getOpCode().isMul(), "Unexpected op!=mul %p\n", mulNode);
  TR_ASSERT(mulNode->isFPStrictCompliant(),
            "mul node %p is not fpStrict Compliant\n", mulNode);
  ...
  generateTrg1Src3Instruction(cg, OpCode, addNode, trgRegister, source1Register,
                              source2Register, source3Register);
...
TR::Register *OMR::Power::TreeEvaluator::faddEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
...
TR::Register *OMR::Power::TreeEvaluator::daddEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Register *result = NULL;
  if ((isFPStrictMul(node->getFirstChild(), comp) ||
...
TR::Register *OMR::Power::TreeEvaluator::dsubEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Register *result = NULL;
  if (isFPStrictMul(node->getFirstChild(), comp) &&
...
TR::Register *OMR::Power::TreeEvaluator::fsubEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Register *result = NULL;
  if (isFPStrictMul(node->getFirstChild(), comp) &&
...
TR::Register *OMR::Power::TreeEvaluator::fmulEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return singlePrecisionEvaluator(node, TR::InstOpCode::fmuls, cg);
}

TR::Register *OMR::Power::TreeEvaluator::dmulEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return doublePrecisionEvaluator(node, TR::InstOpCode::fmul, cg);
}

TR::Register *OMR::Power::TreeEvaluator::fdivEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return singlePrecisionEvaluator(node, TR::InstOpCode::fdivs, cg);
}

TR::Register *OMR::Power::TreeEvaluator::ddivEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  return doublePrecisionEvaluator(node, TR::InstOpCode::fdiv, cg);
}

TR::Register *OMR::Power::TreeEvaluator::fremEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  ...
  TR::Register *copyReg;
  TR::RegisterDependencyConditions *dependencies = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(16, 16, cg->trMemory());

  if (!cg->canClobberNodesRegister(child1)) {
    copyReg = cg->allocateSinglePrecisionRegister();
    generateTrg1Src1Instruction(cg, TR::InstOpCode::fmr, node, copyReg,
                                source1Reg);
    source1Reg = copyReg;
  }
  ...
  TR::addDependency(dependencies, source1Reg, TR::RealRegister::fp0, TR_FPR,
                    cg);
  TR::addDependency(dependencies, source2Reg, TR::RealRegister::fp1, TR_FPR,
                    cg);
...
TR::Register *OMR::Power::TreeEvaluator::dremEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *child1 = node->getFirstChild();
  TR::Node *child2 = node->getSecondChild();
  TR::Register *source1Reg = cg->evaluate(child1);
  ...
  if (source1Reg->isSinglePrecision()) source1Reg->setIsSinglePrecision(false);
 ...
 */
TR::Register *OMR::Power::TreeEvaluator::fnegEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  bool noFMA = true;
  TR::Register *trgRegister;
  TR::Compilation *comp = cg->comp();
  ...
  if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() &&
      isAddOrSub) {
    TR::Register *result = NULL;
    ...
    if ((isFPStrictMul(firstChild->getFirstChild(), comp) ||
      ...
      firstChild
          ->unsetRegister();  // unset as the first child isn't the result node
    }
  }
  if (noFMA) {
    TR::Register *sourceRegister = NULL;
    trgRegister = node->getOpCode().isFloat()
                      ? cg->allocateSinglePrecisionRegister()
                      : cg->allocateRegister(TR_FPR);
    sourceRegister = cg->evaluate(firstChild);
    generateTrg1Src1Instruction(cg, TR::InstOpCode::fneg, node, trgRegister,
                                sourceRegister);
  }
  cg->decReferenceCount(firstChild);
  node->setRegister(trgRegister);
...
TR::Register *OMR::Power::TreeEvaluator::int2dbl(TR::Node *node,
                                                 TR::Register *srcReg,
                                                 bool canClobber,
                                                 TR::CodeGenerator *cg) {
  if (node->getOpCodeValue() == TR::b2f || node->getOpCodeValue() == TR::b2d) {
    TR::Register *tmpReg = cg->allocateRegister();
    generateTrg1Src1Instruction(cg, TR::InstOpCode::extsb, node, tmpReg,
                                srcReg);
    srcReg = tmpReg;
    cg->stopUsingRegister(tmpReg);
  } else if (node->getOpCodeValue() == TR::bu2f ||
             node->getOpCodeValue() == TR::bu2d) {
    TR::Register *tmpReg = cg->allocateRegister();
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, tmpReg,
                                    srcReg, 0, 0xff);
    srcReg = tmpReg;
    cg->stopUsingRegister(tmpReg);
  }
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
                                 TR::Compiler->target.is64Bit())) {
    if (TR::Compiler->target.is64Bit()) {
      if (node->getOpCodeValue() == TR::iu2f ||
        ...
        generateMvFprGprInstructions(cg, node, gpr2fprHost64, false, trgReg,
                                     tempReg);
        cg->stopUsingRegister(tempReg);
      } else if (node->getOpCodeValue() == TR::i2f ||
                 node->getOpCodeValue() == TR::i2d) {
        tempReg = cg->allocateRegister();
        generateTrg1Src1Instruction(cg, TR::InstOpCode::extsw, node, tempReg,
                                    srcReg);
        generateMvFprGprInstructions(cg, node, gpr2fprHost64, false, trgReg,
                                     tempReg);
        cg->stopUsingRegister(tempReg);
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp6 &&
          ...
          node->getOpCodeValue() != TR::iu2d)
        generateMvFprGprInstructions(cg, node, gprLow2fpr, false, trgReg,
                                     srcReg);
      else {
        tempReg = cg->allocateRegister();
    ...
    if ((TR::Compiler->target.cpu.id() >= TR_PPCp7) &&
        (node->getOpCodeValue() == TR::i2f ||
         node->getOpCodeValue() == TR::iu2f)) {
      // Generate the code to produce the float result here, setting the
      // register flag is done afterwards
      if (node->getOpCodeValue() == TR::i2f)
        generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfids, node, trgReg,
                                    trgReg);
      else
        generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfidus, node, trgReg,
                                    trgReg);
    } else {
      generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfid, node, trgReg,
                                  trgReg);
      if (node->getOpCodeValue() == TR::i2f ||
          node->getOpCodeValue() == TR::iu2f)
        generateTrg1Src1Instruction(cg, TR::InstOpCode::frsp, node, trgReg,
    ...
    if (!canClobber) {
      tempReg = cg->allocateRegister();
      generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, tempReg,
                                  srcReg);
      srcReg = tempReg;
    }
    ...
    TR::TreeEvaluator::generateHelperBranchAndLinkInstruction(
        TR_PPCinteger2Double, node, dependencies, cg);
...
// also handles iu2f
TR::Register *OMR::Power::TreeEvaluator::i2fEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *tempReg;
  TR::Register *trgReg;
  ...
  if (((TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
        (node->getOpCodeValue() == TR::iu2f &&
         (child->getOpCodeValue() == TR::iuload ||
          child->getOpCodeValue() == TR::iuloadi))) ||
       (TR::Compiler->target.cpu.id() >= TR_PPCp6 &&
        (node->getOpCodeValue() == TR::i2f &&
         (child->getOpCodeValue() == TR::iload ||
          child->getOpCodeValue() == TR::iloadi)))) &&
      child->getReferenceCount() == 1 && child->getRegister() == NULL &&
      !(child->getSymbolReference()->getSymbol()->isSyncVolatile() &&
        TR::Compiler->target.isSMP())) {
    TR::MemoryReference *tempMR =
        new (cg->trHeapMemory()) TR::MemoryReference(child, 4, cg);
    tempMR->forceIndexedForm(node, cg);
    ...
    if (node->getOpCodeValue() == TR::i2f) {
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfiwax, node, tempReg,
                                 tempMR);
      if (TR::Compiler->target.cpu.id() >= TR_PPCp7) {
        generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfids, node, trgReg,
                                    tempReg);
      } else {
        generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfid, node, tempReg,
                                    tempReg);
        generateTrg1Src1Instruction(cg, TR::InstOpCode::frsp, node, trgReg,
                                    tempReg);
      }
    } else {
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfiwzx, node, tempReg,
                                 tempMR);  // TR::iu2f, must be P7
      generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfidus, node, trgReg,
                                  tempReg);
    }
    tempMR->decNodeReferenceCounts(cg);
    ...
    trgReg = TR::TreeEvaluator::int2dbl(node, cg->evaluate(child),
                                        cg->canClobberNodesRegister(child), cg);
...
TR::Register *OMR::Power::TreeEvaluator::i2dEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *trgReg;
  ...
  if (((TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
    ...
    if (node->getOpCodeValue() == TR::i2d) {
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfiwax, node, trgReg,
                                 tempMR);
      generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfid, node, trgReg,
                                  trgReg);
    } else {
      generateTrg1MemInstruction(cg, TR::InstOpCode::lfiwzx, node, trgReg,
                                 tempMR);  // iu2d
      generateTrg1Src1Instruction(cg, TR::InstOpCode::fcfidu, node, trgReg,
                                  trgReg);
    }
    tempMR->decNodeReferenceCounts(cg);
  } else {
...
TR::Register *OMR::Power::TreeEvaluator::long2dbl(TR::Node *node,
                                                  TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(child);
  TR::Register *trgReg = cg->allocateRegister(TR_FPR);
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
    ...
    if (!cg->canClobberNodesRegister(child)) {
      ...
      generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, srcHigh,
                                  srcReg->getHighOrder());
      generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, node, srcLow,
                                  srcReg->getLowOrder());
    } else {
    ...
    TR::TreeEvaluator::generateHelperBranchAndLinkInstruction(
        TR_PPClong2Double, node, dependencies, cg);
...
TR::Register *OMR::Power::TreeEvaluator::long2float(TR::Node *node,
                                                    TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(child);
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
      (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
                                  TR::Compiler->target.is64Bit()))) {
    if (TR::Compiler->target.is64Bit())
      generateMvFprGprInstructions(cg, node, gpr2fprHost64, false, trgReg,
                                   srcReg);
    ...
    TR::TreeEvaluator::generateHelperBranchAndLinkInstruction(
        TR_PPClong2Float, node, dependencies, cg);
...
TR::Register *OMR::Power::TreeEvaluator::l2fEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Register *trgReg;
  TR::Node *child = node->getFirstChild();
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
      ...
      (child->getOpCodeValue() == TR::lload ||
       child->getOpCodeValue() == TR::lloadi) &&
      child->getReferenceCount() == 1 && child->getRegister() == NULL &&
      !(child->getSymbolReference()->getSymbol()->isSyncVolatile() &&
        TR::Compiler->target.isSMP())) {
    TR::MemoryReference *tempMR =
...
TR::Register *OMR::Power::TreeEvaluator::l2dEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Register *trgReg;
  TR::Node *child = node->getFirstChild();
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
...
TR::Register *OMR::Power::TreeEvaluator::f2dEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
...
//
TR::Register *OMR::Power::TreeEvaluator::d2iEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  ...
  TR::Register *tempReg = (opcode == TR::f2b || opcode == TR::f2s ||
                           opcode == TR::f2c || opcode == TR::f2i)
                              ? cg->allocateSinglePrecisionRegister()
                              : cg->allocateRegister(TR_FPR);
  ...
  if (true) {
    ...
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node,
                                         doneLabel, condReg);
    generateTrg1Src1Instruction(cg, TR::InstOpCode::fctiwz, node, tempReg,
                                sourceReg);
  ...
  if (opcode == TR::f2s || opcode == TR::d2s)
    generateTrg1Src1Instruction(cg, TR::InstOpCode::extsh, node, trgReg,
                                trgReg);
...
TR::Register *OMR::Power::TreeEvaluator::d2iuEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Compilation *comp = cg->comp();
  TR::Node *child = node->getFirstChild();
  TR::Register *sourceReg = cg->evaluate(child);
...
// also handles f2l
TR::Register *OMR::Power::TreeEvaluator::d2lEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
  TR::Register *sourceReg = cg->evaluate(child);
  TR::Register *trgReg;
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
                                 TR::Compiler->target.is64Bit())) {
    TR::Register *condReg = cg->allocateRegister(TR_CCR);
    TR::Register *tempReg = (node->getOpCodeValue() == TR::f2l)
                                ? cg->allocateSinglePrecisionRegister()
                                : cg->allocateRegister(TR_FPR);
    TR::PPCControlFlowInstruction *cfop =
        (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
            cg, TR::InstOpCode::d2l, node);
    cfop->addTargetRegister(condReg);
    cfop->addTargetRegister(tempReg);
    if (TR::Compiler->target.is64Bit())
      cfop->addTargetRegister(trgReg);
    ...
    TR::addDependency(dependencies, sourceReg, TR::RealRegister::fp0, TR_FPR,
                      cg);
    TR::addDependency(dependencies, trgReg->getHighOrder(),
                      TR::RealRegister::gr3, TR_GPR, cg);
    TR::addDependency(dependencies, trgReg->getLowOrder(),
    ...
    TR::TreeEvaluator::generateHelperBranchAndLinkInstruction(
        TR_PPCdouble2Long, node, dependencies, cg);

    dependencies->stopUsingDepRegs(cg, trgReg->getHighOrder(),
                                   trgReg->getLowOrder());
    cg->machine()->setLinkRegisterKilled(true);
  }
...
TR::Register *OMR::Power::TreeEvaluator::d2fEvaluator(TR::Node *node,
                                                      TR::CodeGenerator *cg) {
  TR::Register *trgReg = cg->allocateSinglePrecisionRegister();
  TR::Node *child = node->getFirstChild();
...
TR::Register *OMR::Power::TreeEvaluator::ifdcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
// also handles TR::iffcmpeq
TR::Register *OMR::Power::TreeEvaluator::ifdcmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
// also handles TR::iffcmpeq
TR::Register *OMR::Power::TreeEvaluator::ifdcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
// also handles TR::iffcmpeq
TR::Register *OMR::Power::TreeEvaluator::ifdcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ifdcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ifdcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ifdcmpequEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ifdcmpltuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ifdcmpgeuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // TR::InstOpCode::bge is really not less than which is equivalent to greater
  // than equal to or unordered
  ifFloatEvaluator(node, TR::InstOpCode::bge, TR::InstOpCode::bad, cg);
  return NULL;
}

// also handles TR::iffcmpgtu
TR::Register *OMR::Power::TreeEvaluator::ifdcmpgtuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::ifdcmpleuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // TR::InstOpCode::ble is really not greater than which is equivalent to less
  // than equal to or unordered
...
TR::Register *OMR::Power::TreeEvaluator::dcmpeqEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::beq,
                                          TR::InstOpCode::bad, 0, node, cg);
}

// also handles TR::fcmpne
TR::Register *OMR::Power::TreeEvaluator::dcmpneEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::bne,
                                          TR::InstOpCode::bad, 0, node, cg);
}

// also handles TR::fcmplt
TR::Register *OMR::Power::TreeEvaluator::dcmpltEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  int64_t imm = 0;
  if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
    imm = (TR::RealRegister::CRCC_GT << TR::RealRegister::pos_RT |
           TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RA |
           TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RB);
  }
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::blt,
                                          TR::InstOpCode::bad, imm, node, cg);
}

// also handles TR::fcmpge
TR::Register *OMR::Power::TreeEvaluator::dcmpgeEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::bgt,
                                          TR::InstOpCode::beq, 0, node, cg);
}

// also handles TR::fcmpgt
TR::Register *OMR::Power::TreeEvaluator::dcmpgtEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  int64_t imm = 0;
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::bgt,
                                          TR::InstOpCode::bad, imm, node, cg);
...
TR::Register *OMR::Power::TreeEvaluator::dcmpleEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::blt,
                                          TR::InstOpCode::beq, 0, node, cg);
}
...
TR::Register *OMR::Power::TreeEvaluator::dcmpequEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::beq,
                                          TR::InstOpCode::bun, 0, node, cg);
}

// also handles TR::fcmpltu
TR::Register *OMR::Power::TreeEvaluator::dcmpltuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::blt,
                                          TR::InstOpCode::bun, 0, node, cg);
}
...
TR::Register *OMR::Power::TreeEvaluator::dcmpgeuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::bge,
                                          TR::InstOpCode::bad, 0, node, cg);
}

// also handles TR::fcmpgtu
TR::Register *OMR::Power::TreeEvaluator::dcmpgtuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::bgt,
                                          TR::InstOpCode::bun, 0, node, cg);
}
...
TR::Register *OMR::Power::TreeEvaluator::dcmpleuEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  // TR::InstOpCode::ble is really not greater than which is equivalent to less
  // than equal to or unordered
  return compareFloatAndSetOrderedBoolean(TR::InstOpCode::ble,
                                          TR::InstOpCode::bad, 0, node, cg);
}

// also handles TR::fcmpl
TR::Register *OMR::Power::TreeEvaluator::dcmplEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
...
TR::Register *OMR::Power::TreeEvaluator::dcmpgEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  TR::Register *src1Reg = cg->evaluate(firstChild);
...
static TR::Register *singlePrecisionEvaluator(TR::Node *node,
  ...
  TR::Register *trgRegister = cg->allocateSinglePrecisionRegister();
  generateTrg1Src2Instruction(cg, OpCode, node, trgRegister, source1Register,
                              source2Register);
  node->setRegister(trgRegister);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
...
static void ifFloatEvaluator(TR::Node *node, TR::InstOpCode::Mnemonic branchOp1,
  ...
  generateTrg1Src2Instruction(cg, TR::InstOpCode::fcmpu, node,
                              conditionRegister, src1Register, src2Register);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
  ...
  if (branchOp2 == TR::InstOpCode::bad && deps != NULL) {
    ...
    generateConditionalBranchInstruction(cg, branchOp1, node, label,
                                         conditionRegister);
  }
...
static TR::Register *compareFloatAndSetOrderedBoolean(
  ...
  if (true) {
    TR::PPCControlFlowInstruction *cfop;
    if (branchOp2 == TR::InstOpCode::bad) {
      cfop = (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
          cg, TR::InstOpCode::setbool, node);
    } else {
      cfop = (TR::PPCControlFlowInstruction *)generateControlFlowInstruction(
          cg, TR::InstOpCode::setbflt, node);
    ...
    cfop->addSourceRegister(src2Reg);
    if (TR::Compiler->target.cpu.id() >= TR_PPCp9 &&
        branchOp2 == TR::InstOpCode::bad && imm != 0) {
    ...
    generateTrg1Src2Instruction(cg, TR::InstOpCode::fcmpu, node, condReg,
                                src1Reg, src2Reg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 1);
    generateConditionalBranchInstruction(cg, branchOp1, node, doneLabel,
                                         condReg);
    if (branchOp2 != TR::InstOpCode::bad)
      generateConditionalBranchInstruction(cg, branchOp2, node, doneLabel,
                                           condReg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, doneLabel);
  }
...
TR::Register *OMR::Power::TreeEvaluator::fRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
...
TR::Register *OMR::Power::TreeEvaluator::dRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *globalReg = node->getRegister();
...
TR::Register *OMR::Power::TreeEvaluator::vRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *globalReg = node->getRegister();
...
TR::Register *OMR::Power::TreeEvaluator::fRegStoreEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *child = node->getFirstChild();
...
TR::Register *OMR::Power::TreeEvaluator::iexpEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return 0;
}

TR::Register *OMR::Power::TreeEvaluator::lexpEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return 0;
}
...
// also handles fexp
TR::Register *OMR::Power::TreeEvaluator::dexpEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR_UNIMPLEMENTED();
  return 0;
}
...
// also handles fxfrs
TR::Register *OMR::Power::TreeEvaluator::dxfrsEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Node *secondChild = node->getSecondChild();
  ...
  TR::Register *condReg = cg->allocateRegister(TR_CCR);
  generateTrg1Src2Instruction(cg, TR::InstOpCode::fcmpu, node, condReg, tmpReg,
                              reg2);
  cg->stopUsingRegister(tmpReg);
...
// also handles fint
TR::Register *OMR::Power::TreeEvaluator::dintEvaluator(TR::Node *node,
                                                       TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(firstChild);
...
// also handles fnint
TR::Register *OMR::Power::TreeEvaluator::dnintEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(firstChild);
  ...
  if (!cg->canClobberNodesRegister(firstChild)) {
    trgReg = dnint ? cg->allocateRegister(TR_FPR)
                   : cg->allocateSinglePrecisionRegister();
  }
  ...
  TR::Register *tmp2Reg = dnint ? cg->allocateRegister(TR_FPR)
                                : cg->allocateSinglePrecisionRegister();

  generateTrg1Src1Instruction(cg, TR::InstOpCode::fabs, node, tmp2Reg, srcReg);

  TR::Register *condReg = cg->allocateRegister(TR_CCR);
  generateTrg1Src2Instruction(cg, TR::InstOpCode::fcmpu, node, condReg, tmp1Reg,
                              tmp2Reg);

  TR::LabelSymbol *moveLabel = generateLabelSymbol(cg);
  generateConditionalBranchInstruction(cg, TR::InstOpCode::ble, node, moveLabel,
                                       condReg);
  cg->stopUsingRegister(condReg);
  ...
  generateTrg1Src2Instruction(cg, TR::InstOpCode::fadd, node, tmp1Reg, srcReg,
                              tmp1Reg);
  generateTrg1Src2Instruction(cg, TR::InstOpCode::fadd, node, tmp2Reg, srcReg,
                              tmp2Reg);
  generateTrg1Src3Instruction(cg, TR::InstOpCode::fsel, node, trgReg, srcReg,
                              tmp1Reg, tmp2Reg);
  cg->stopUsingRegister(tmp1Reg);
  cg->stopUsingRegister(tmp2Reg);
...
TR::Register *OMR::Power::TreeEvaluator::fsqrtEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(firstChild);
...
TR::Register *OMR::Power::TreeEvaluator::dsqrtEvaluator(TR::Node *node,
                                                        TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(firstChild);
...
TR::Register *OMR::Power::TreeEvaluator::getstackEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  const TR::PPCLinkageProperties &properties = cg->getProperties();

  TR::Register *spReg = cg->machine()->getRealRegister(
      properties.getNormalStackPointerRegister());
  TR::Register *trgReg = cg->allocateRegister();
...
TR::Register *OMR::Power::TreeEvaluator::deallocaEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Node *firstChild = node->getFirstChild();
  TR::Register *srcReg = cg->evaluate(firstChild);
  const TR::PPCLinkageProperties &properties = cg->getProperties();
...
TR::Register *OMR::Power::TreeEvaluator::xfRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *globalReg = node->getRegister();
...
TR::Register *OMR::Power::TreeEvaluator::xdRegLoadEvaluator(
    TR::Node *node, TR::CodeGenerator *cg) {
  TR::Register *globalReg = node->getRegister();

  if (globalReg == NULL) {
    globalReg = cg->allocateFloatingPointRegisterPair(
        cg->allocateRegister(TR_FPR), cg->allocateRegister(TR_FPR));
    node->setRegister(globalReg);
  }
  return (globalReg);
}
