...
static TR::Register* compareFloatAndSetOrderedBoolean(TR::InstOpCode::Mnemonic branchOp1,
  TR::InstOpCode::Mnemonic branchOp2, int64_t imm, TR::Node* node, TR::CodeGenerator* cg);
...
TR::Register* OMR::Power::TreeEvaluator::fbits2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 && child->getOpCode().isLoadVar()) {
    ...
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lfs, node, floatReg, new (cg->trHeapMemory()) TR::MemoryReference(node, *tempMR, 0, 4, cg));
    ...
    if (TR::Compiler->target.is64Bit())
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
        ...
        generateMvFprGprInstructions(
          cg, node, gpr2fprHost32, false, target, longReg->getHighOrder(), longReg->getLowOrder(), tmp1);
    ...
    if (offset != PTOC_FULL_INDEX) {
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        ...
        generateTrg1Src1ImmInstruction(
          cg, TR::InstOpCode::addis, node, srcRegister, cg->getTOCBaseRegister(), HI_VALUE(offset));
        generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, trgRegister,
          new (cg->trHeapMemory()) TR::MemoryReference(srcRegister, LO_VALUE(offset), 4, cg));
        cg->stopUsingRegister(srcRegister);
      } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::lfs, node, trgRegister,
          new (cg->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), offset, 4, cg));
      }
    }
  }
    ...
    if (offset != PTOC_FULL_INDEX) {
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        ...
        TR::MemoryReference* memRef
          = new (cg->trHeapMemory()) TR::MemoryReference(srcRegister, LO_VALUE(offset), 8, cg);
        if (splats)
...
TR::Register* OMR::Power::TreeEvaluator::dloadHelper(
  TR::Node* node, TR::CodeGenerator* cg, TR::Register* tempReg, TR::InstOpCode::Mnemonic opcode)
{
  TR::Compilation* comp = cg->comp();
  TR::MemoryReference* tempMR;
  bool needSync = node->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP();
  if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
    ...
    TR::SymbolReference* vrlRef
      = comp->getSymRefTab()->findOrCreateVolatileReadDoubleSymbolRef(comp->getMethodSymbol());
  ...
  if (node->getDataType() == TR::VectorInt8) {
    ...
    if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8
      ...
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stw, node, new (cg->trHeapMemory()) TR::MemoryReference(node, localTemp, 4, cg), tempReg);
    ...
    if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8
      ...
      if (TR::Compiler->target.is64Bit()) {
        ...
        generateMvFprGprInstructions(
          cg, node, gpr2fprHost32, false, trgReg, srcReg->getHighOrder(), srcReg->getLowOrder(), tempVectorReg);
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::xxpermdi, node, trgReg, trgReg, trgReg, 0x0);
...
TR::Register* OMR::Power::TreeEvaluator::vdsetelemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (thirdChild->getOpCode().isLoadConst()) {
    ...
    if (secondChild->getRegister() == NULL && secondChild->getOpCode().isLoadVar()) {
...
TR::Register* OMR::Power::TreeEvaluator::dstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
    TR::Register* addrReg = cg->allocateRegister();
    TR::SymbolReference* vrlRef
      = comp->getSymRefTab()->findOrCreateVolatileWriteDoubleSymbolRef(comp->getMethodSymbol());
    TR::RegisterDependencyConditions* dependencies
      = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(5, 5, cg->trMemory());
    if (isUnresolved) {
#ifdef J9_PROJECT_SPECIFIC
  ...
  TR::Node* node, TR::Register* srcReg, bool canClobber, TR::CodeGenerator* cg)
{
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && TR::Compiler->target.is64Bit())) {
    if (TR::Compiler->target.is64Bit()) {
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp6 && node->getOpCodeValue() != TR::iu2f
...
TR::Register* OMR::Power::TreeEvaluator::i2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (((TR::Compiler->target.cpu.id() >= TR_PPCp7
         && (node->getOpCodeValue() == TR::iu2f
           && (child->getOpCodeValue() == TR::iuload || child->getOpCodeValue() == TR::iuloadi)))
...
TR::Register* OMR::Power::TreeEvaluator::i2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (((TR::Compiler->target.cpu.id() >= TR_PPCp7
         && (node->getOpCodeValue() == TR::iu2d
           && (child->getOpCodeValue() == TR::iuload || child->getOpCodeValue() == TR::iuloadi)))
        || (TR::Compiler->target.cpu.id() >= TR_PPCp6 && node->getOpCodeValue() == TR::i2d
          && (child->getOpCodeValue() == TR::iload || child->getOpCodeValue() == TR::iloadi)))
    && child->getReferenceCount() == 1 && child->getRegister() == NULL
    && !(child->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
...
TR::Register* OMR::Power::TreeEvaluator::l2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg;
  TR::Node* child = node->getFirstChild();
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 && node->getOpCodeValue() == TR::l2f
...
TR::Register* OMR::Power::TreeEvaluator::l2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg;
  TR::Node* child = node->getFirstChild();
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 && node->getOpCodeValue() == TR::l2d
    && (child->getOpCodeValue() == TR::lload || child->getOpCodeValue() == TR::lloadi)
    && child->getReferenceCount() == 1 && child->getRegister() == NULL
    && !(child->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
    TR::MemoryReference* tempMR = new (cg->trHeapMemory()) TR::MemoryReference(child, 4, cg);
...
TR::Register* OMR::Power::TreeEvaluator::d2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && TR::Compiler->target.is64Bit())) {
    TR::Register* condReg = cg->allocateRegister(TR_CCR);
    TR::Register* tempReg
      = (node->getOpCodeValue() == TR::f2l) ? cg->allocateSinglePrecisionRegister() : cg->allocateRegister(TR_FPR);
    TR::PPCControlFlowInstruction* cfop
...
TR::Register* OMR::Power::TreeEvaluator::dcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.cpu.id() >= TR_PPCp9) {
    imm = (TR::RealRegister::CRCC_GT << TR::RealRegister::pos_RT | TR::RealRegister::CRCC_LT << TR::RealRegister::pos_RA
...
TR::Register* OMR::Power::TreeEvaluator::dcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (true) {
    TR::PPCControlFlowInstruction* cfop
      = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::flcmpl, node);
    cfop->addTargetRegister(condReg);
    cfop->addTargetRegister(trgReg);
...
TR::Register* OMR::Power::TreeEvaluator::dcmpgEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (true) {
    TR::PPCControlFlowInstruction* cfop
      = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::flcmpg, node);
    cfop->addTargetRegister(condReg);
    cfop->addTargetRegister(trgReg);
    cfop->addSourceRegister(src1Reg);
    cfop->addSourceRegister(src2Reg);
  ...
  TR::InstOpCode::Mnemonic branchOp2, int64_t imm, TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (true) {
    TR::PPCControlFlowInstruction* cfop;
    if (branchOp2 == TR::InstOpCode::bad) {
      cfop = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::setbool, node);
    } else {
      cfop = (TR::PPCControlFlowInstruction*)generateControlFlowInstruction(cg, TR::InstOpCode::setbflt, node);
...
TR::Register* OMR::Power::TreeEvaluator::vRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (globalReg == NULL) {
    if (node->getOpCode().getOpCodeValue() == TR::vbRegLoad || node->getOpCode().getOpCodeValue() == TR::vsRegLoad
      || node->getOpCode().getOpCodeValue() == TR::viRegLoad || node->getOpCode().getOpCodeValue() == TR::vlRegLoad)
      globalReg = cg->allocateRegister(TR_VRF);
    else if (node->getOpCode().getOpCodeValue() == TR::vfRegLoad || node->getOpCode().getOpCodeValue() == TR::vdRegLoad)
...
TR::Register* OMR::Power::TreeEvaluator::dxfrsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::Register* trgReg
    = (node->getOpCodeValue() == TR::dxfrs) ? cg->allocateRegister(TR_FPR) : cg->allocateSinglePrecisionRegister();
...
TR::Register* OMR::Power::TreeEvaluator::dintEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  TR::Register* trgReg
    = (node->getOpCodeValue() == TR::dint) ? cg->allocateRegister(TR_FPR) : cg->allocateSinglePrecisionRegister();
