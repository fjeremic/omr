...
TR::Register* OMR::Power::TreeEvaluator::faddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((isFPStrictMul(node->getFirstChild(), comp) || isFPStrictMul(node->getSecondChild(), comp)) && performTransformation(comp, "O^O Changing [%p] to fmadd\n", node))
...
TR::Register* OMR::Power::TreeEvaluator::daddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR::Register* result = NULL;
    if ((isFPStrictMul(node->getFirstChild(), comp) || isFPStrictMul(node->getSecondChild(), comp)) && performTransformation(comp, "O^O Changing [%p] to dmadd\n", node))
...
TR::Register* OMR::Power::TreeEvaluator::dsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isFPStrictMul(node->getFirstChild(), comp) && performTransformation(comp, "O^O Changing [%p] to fmsub\n", node))
        ...
        if (isFPStrictMul(node->getSecondChild(), comp) && performTransformation(comp, "O^O Changing [%p] to fnmsub\n", node)) {
...
TR::Register* OMR::Power::TreeEvaluator::fnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() && isAddOrSub) {
        ...
        if ((isFPStrictMul(firstChild->getFirstChild(), comp) || (isAdd && isFPStrictMul(firstChild->getSecondChild(), comp))) && performTransformation(comp, "O^O Changing [%p] to fnmadd/sub\n", node)) {
...
TR::Register* OMR::Power::TreeEvaluator::int2dbl(TR::Node* node, TR::Register* srcReg, bool canClobber, TR::CodeGenerator* cg)
{
    ...
    if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && TR::Compiler->target.is64Bit())) {
        ...
        if ((TR::Compiler->target.cpu.id() >= TR_PPCp7) && (node->getOpCodeValue() == TR::i2f || node->getOpCodeValue() == TR::iu2f)) {
...
TR::Register* OMR::Power::TreeEvaluator::i2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (((TR::Compiler->target.cpu.id() >= TR_PPCp7 && (node->getOpCodeValue() == TR::iu2f && (child->getOpCodeValue() == TR::iuload || child->getOpCodeValue() == TR::iuloadi))) || (TR::Compiler->target.cpu.id() >= TR_PPCp6 && (node->getOpCodeValue() == TR::i2f && (child->getOpCodeValue() == TR::iload || child->getOpCodeValue() == TR::iloadi)))) && child->getReferenceCount() == 1 && child->getRegister() == NULL && !(child->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
        TR::MemoryReference* tempMR = new (cg->trHeapMemory()) TR::MemoryReference(child, 4, cg);
        tempMR->forceIndexedForm(node, cg);
...
TR::Register* OMR::Power::TreeEvaluator::i2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (((TR::Compiler->target.cpu.id() >= TR_PPCp7 && (node->getOpCodeValue() == TR::iu2d && (child->getOpCodeValue() == TR::iuload || child->getOpCodeValue() == TR::iuloadi))) || (TR::Compiler->target.cpu.id() >= TR_PPCp6 && node->getOpCodeValue() == TR::i2d && (child->getOpCodeValue() == TR::iload || child->getOpCodeValue() == TR::iloadi))) && child->getReferenceCount() == 1 && child->getRegister() == NULL && !(child->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
...
TR::Register* OMR::Power::TreeEvaluator::long2float(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCp7 && (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() && TR::Compiler->target.is64Bit()))) {
        if (TR::Compiler->target.is64Bit())
            generateMvFprGprInstructions(cg, node, gpr2fprHost64, false, trgReg, srcReg);
        else {
            if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
...
TR::Register* OMR::Power::TreeEvaluator::l2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* trgReg;
    TR::Node* child = node->getFirstChild();
    if (TR::Compiler->target.cpu.id() >= TR_PPCp7 && node->getOpCodeValue() == TR::l2f && (child->getOpCodeValue() == TR::lload || child->getOpCodeValue() == TR::lloadi) && child->getReferenceCount() == 1 && child->getRegister() == NULL && !(child->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
        TR::MemoryReference* tempMR = new (cg->trHeapMemory()) TR::MemoryReference(child, 4, cg);
        tempMR->forceIndexedForm(node, cg);
...
TR::Register* OMR::Power::TreeEvaluator::l2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Node* child = node->getFirstChild();
    if (
        TR::Compiler->target.cpu.id() >= TR_PPCp7 && node->getOpCodeValue() == TR::l2d && (child->getOpCodeValue() == TR::lload || child->getOpCodeValue() == TR::lloadi) && child->getReferenceCount() == 1 && child->getRegister() == NULL && !(child->getSymbolReference()->getSymbol()->isSyncVolatile() && TR::Compiler->target.isSMP())) {
        TR::MemoryReference* tempMR = new (cg->trHeapMemory()) TR::MemoryReference(child, 4, cg);
        tempMR->forceIndexedForm(node, cg);
...
TR::Register* OMR::Power::TreeEvaluator::vRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (globalReg == NULL) {
        if (node->getOpCode().getOpCodeValue() == TR::vbRegLoad || node->getOpCode().getOpCodeValue() == TR::vsRegLoad || node->getOpCode().getOpCodeValue() == TR::viRegLoad || node->getOpCode().getOpCodeValue() == TR::vlRegLoad)
...
TR::Register* OMR::Power::TreeEvaluator::dxfrsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* trgReg = (node->getOpCodeValue() == TR::dxfrs) ? cg->allocateRegister(TR_FPR)
                                                                 : cg->allocateSinglePrecisionRegister();
...
TR::Register* OMR::Power::TreeEvaluator::dintEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* trgReg = (node->getOpCodeValue() == TR::dint) ? cg->allocateRegister(TR_FPR)
                                                                : cg->allocateSinglePrecisionRegister();
