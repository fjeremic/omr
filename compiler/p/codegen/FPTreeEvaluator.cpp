...
static void
ifFloatEvaluator(TR::Node* node,
                 TR::InstOpCode::Mnemonic branchOp1,
                 TR::InstOpCode::Mnemonic branchOp2,
                 TR::CodeGenerator* cg);
static TR::Register*
compareFloatAndSetOrderedBoolean(TR::InstOpCode::Mnemonic branchOp1,
                                 TR::InstOpCode::Mnemonic branchOp2,
                                 int64_t imm,
                                 TR::Node* node,
                                 TR::CodeGenerator* cg);
static TR::Register*
singlePrecisionEvaluator(TR::Node* node,
                         TR::InstOpCode::Mnemonic OpCode,
                         TR::CodeGenerator* cg);
static TR::Register*
doublePrecisionEvaluator(TR::Node* node,
                         TR::InstOpCode::Mnemonic OpCode,
                         TR::CodeGenerator* cg);
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::iriload) {
      ...
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwbrx, node, target, tempMR);
    } else
#endif
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    generateTrg1MemInstruction(cg,
                               ...
                               floatReg,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(node, *tempMR, 0, 4, cg));
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
        ...
        TR::Compiler->target.is64Bit()) // 64-bit only
    {
      ...
      tempMR->forceIndexedForm(child, cg);
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::ldbrx, node, tmpReg, tempMR);
      generateMvFprGprInstructions(
        cg, node, gpr2fprHost64, true, target, tmpReg);
               ...
               TR::Compiler->target.is32Bit()) // 32-bit
    {
      ...
      TR::MemoryReference* tempMRLoad1 =
        new (cg->trHeapMemory()) TR::MemoryReference(child, *tempMR, 0, 4, cg);
      TR::MemoryReference* tempMRLoad2 =
        new (cg->trHeapMemory()) TR::MemoryReference(child, *tempMR, 4, 4, cg);
      ...
      generateMvFprGprInstructions(
        cg, node, gpr2fprHost32, false, target, highReg, lowReg);
    ...
    if (TR::Compiler->target.is64Bit())
      generateMvFprGprInstructions(
        cg, node, gpr2fprHost64, true, target, cg->evaluate(child));
    else {
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (child->getRegister() == NULL && child->getReferenceCount() == 1 &&
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lwz, node, highReg, highMem);
      lowMem =
    ...
    if (TR::Compiler->target.is64Bit()) {
      lReg = cg->allocateRegister();
      generateMvFprGprInstructions(
        cg, node, fpr2gprHost64, true, lReg, doubleReg);
    } else {
      highReg = cg->allocateRegister();
      lowReg = cg->allocateRegister();
      if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
  ...
  if (node->normalizeNanValues()) {
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::fcmpu, node, condReg, doubleReg, doubleReg);
    generateLabelInstruction(
      cg, TR::InstOpCode::label, node, nanNormalizeStartLabel);
    generateConditionalBranchInstruction(cg,
                                         TR::InstOpCode::beq,
    ...
    if (TR::Compiler->target.is64Bit()) {
      ...
      generateTrg1ImmInstruction(
        cg, TR::InstOpCode::lis, node, highReg, 0x7ff8);
...
static TR::Register*
fconstHandler(TR::Node* node, TR::CodeGenerator* cg, float value)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    offset =
      cg->findOrCreateFloatConstant(&value, TR::Float, NULL, NULL, NULL, NULL);
    if (offset != PTOC_FULL_INDEX) {
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        ...
        generateTrg1MemInstruction(cg,
                                   ...
                                   trgRegister,
                                   new (cg->trHeapMemory()) TR::MemoryReference(
                                     cg->getTOCBaseRegister(), offset, 4, cg));
      }
    }
  }
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is64Bit()) {
    offset =
      cg->findOrCreateFloatConstant(&value, TR::Double, NULL, NULL, NULL, NULL);
    if (offset != PTOC_FULL_INDEX) {
      if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        srcRegister = cg->allocateRegister();
        generateTrg1Src1ImmInstruction(cg,
  ...
  if (TR::Compiler->target.is32Bit() || offset == PTOC_FULL_INDEX) {
    ...
    fixedSeqMemAccess(
      cg, node, 0, q, trgRegister, srcRegister, opcode, 8, NULL, tempReg);
    cg->stopUsingRegister(srcRegister);
    if (TR::Compiler->target.is64Bit())
      cg->stopUsingRegister(tempReg);
...
TR::Register*
OMR::Power::TreeEvaluator::floadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* tempReg =
  ...
  if (needSync) {
    TR::TreeEvaluator::postSyncConditions(
      node, cg, tempReg, tempMR, TR::InstOpCode::isync);
  }
  tempMR->decNodeReferenceCounts(cg);
...
TR::Register*
OMR::Power::TreeEvaluator::dloadHelper(TR::Node* node,
                                       TR::CodeGenerator* cg,
                                       TR::Register* tempReg,
                                       TR::InstOpCode::Mnemonic opcode)
{
  ...
  if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
    ...
    if (opcode == TR::InstOpCode::lxvdsx || opcode == TR::InstOpCode::lxsdx) {
      if (tempMR->hasDelayedOffset()) {
        TR::Register* tmpReg = cg->allocateRegister();
        generateTrg1MemInstruction(
          cg, TR::InstOpCode::addi2, node, tmpReg, tempMR);
        tempMR->decNodeReferenceCounts(cg);
...
TR::Register*
OMR::Power::TreeEvaluator::dloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  if (node->getDataType() == TR::VectorInt8) {
    ...
    generateTrg1MemInstruction(
      cg, TR::InstOpCode::lxvd2x, node, trgReg, tempMR);
#if defined(__LITTLE_ENDIAN__)
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::vspltb, node, trgReg, trgReg, 7);
#else
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::vspltb, node, trgReg, trgReg, 0);
#endif
...
#if defined(__LITTLE_ENDIAN__)
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::vsplth, node, trgReg, trgReg, 3);
#else
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::vsplth, node, trgReg, trgReg, 0);
#endif
    node->setRegister(trgReg);
    cg->decReferenceCount(child);
    return trgReg;
    ...
    if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8 &&
      ...
      generateMvFprGprInstructions(
        cg, node, gprLow2fpr, false, resReg, tempReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::xxspltw, node, resReg, resReg, 0x1);
    } else {
      ...
      generateMemSrc1Instruction(cg,
                                 ...
                                 node,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(node, localTemp, 4, cg),
                                 tempReg);
      ...
      tempMR->forceIndexedForm(node, cg);
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lxsdx, node, resReg, tempMR);
      tempMR->decNodeReferenceCounts(cg);
#if defined(__LITTLE_ENDIAN__)
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::xxspltw, node, resReg, resReg, 1);
#else
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::xxspltw, node, resReg, resReg, 0);
#endif
    }
    ...
    if (!disableDirectMove && TR::Compiler->target.cpu.id() >= TR_PPCp8 &&
      ...
      if (TR::Compiler->target.is64Bit()) {
        generateMvFprGprInstructions(
          cg, node, gpr2fprHost64, false, trgReg, srcReg);
        generateTrg1Src2ImmInstruction(
          cg, TR::InstOpCode::xxpermdi, node, trgReg, trgReg, trgReg, 0x0);
      } else {
      ...
      if (TR::Compiler->target.is64Bit()) {
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::std, node, tempMR, srcReg);
      } else {
        TR::MemoryReference* tempMR2 =
          new (cg->trHeapMemory()) TR::MemoryReference(node, *tempMR, 4, 4, cg);
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stw, node, tempMR, srcReg->getHighOrder());
        generateMemSrc1Instruction(
          cg, TR::InstOpCode::stw, node, tempMR2, srcReg->getLowOrder());
      }
      ...
      cg->stopUsingRegister(tmpReg);
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lxvdsx, node, trgReg, tempMR);
    }
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::xscvdpsp, node, trgReg, srcReg);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::xxspltw, node, trgReg, trgReg, 0);
  ...
  if (child->getOpCode().isLoadConst()) {
    ...
    return TR::TreeEvaluator::dloadHelper(
      child, cg, resReg, TR::InstOpCode::lxvdsx);
  } else {
    TR::Register* srcReg = cg->evaluate(child);
    TR::Register* resReg =
      node->setRegister(cg->allocateRegister(TR_VSX_VECTOR));
    generateTrg1Src2ImmInstruction(
      cg, TR::InstOpCode::xxpermdi, node, resReg, srcReg, srcReg, 0x0);
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (secondChild->getOpCode().isLoadConst()) {
    ...
    if (elem == 1)
      generateTrg1Src2ImmInstruction(
        cg, TR::InstOpCode::xxsldwi, node, resReg, srcReg, srcReg, 0x2);
    else
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::xxlor, node, resReg, srcReg, srcReg);
  ...
  generateTrg1Src1ImmInstruction(
    cg, TR::InstOpCode::cmpi4, node, condReg, idxReg, 0);

  // copy vector register to result register
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::xxlor, node, resReg, vectorReg, vectorReg);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, jumpLabel, condReg);
  generateTrg1Src2ImmInstruction(
    cg, TR::InstOpCode::xxsldwi, node, resReg, vectorReg, vectorReg, 0x2);
                                              ...
                                              TR::CodeGenerator* cg)
{
  ...
  if (thirdChild->getOpCode().isLoadConst()) {
    ...
    if (secondChild->getRegister() == NULL &&
      ...
      TR::TreeEvaluator::dloadHelper(
        secondChild, cg, resReg, TR::InstOpCode::lxsdx);
    } else {
      TR::Register* fprReg = cg->evaluate(secondChild);
      generateTrg1Src2Instruction(
        cg, TR::InstOpCode::xxlor, node, resReg, fprReg, fprReg);
    ...
    if (elem == 0)
      generateTrg1Src2ImmInstruction(
        cg, TR::InstOpCode::xxpermdi, node, resReg, resReg, vectorReg, 0x1);
    else
      generateTrg1Src2ImmInstruction(
        cg, TR::InstOpCode::xxpermdi, node, resReg, vectorReg, resReg, 0x0);
  ...
  generateTrg1Src2ImmInstruction(
    cg, TR::InstOpCode::xxpermdi, node, resReg, valueReg, vectorReg, 0x1);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::beq, node, doneLabel, condReg);
  generateTrg1Src2ImmInstruction(
    cg, TR::InstOpCode::xxpermdi, node, resReg, vectorReg, valueReg, 0x0);
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  if (TR::Compiler->target.is32Bit() && needSync && !cg->is64BitProcessor()) {
    ...
    if (isUnresolved) {
      ...
      tempMR = new (cg->trHeapMemory()) TR::MemoryReference(node, 8, cg);
      generateMemSrc1Instruction(
        cg, TR::InstOpCode::stfd, node, tempMR, valueReg);
      tempMR->getUnresolvedSnippet()->setIsSpecialDouble();
      tempMR->getUnresolvedSnippet()->setInSyncSequence();
#endif
    } else {
    ...
    TR::addDependency(
      dependencies, valueReg, TR::RealRegister::fp0, TR_FPR, cg);
    TR::addDependency(dependencies, addrReg, TR::RealRegister::gr3, TR_GPR, cg);
    TR::addDependency(dependencies, NULL, TR::RealRegister::gr11, TR_GPR, cg);
    if (isUnresolved) {
      if (tempMR->getBaseRegister() != NULL) {
...
//
static bool
isFPStrictMul(TR::Node* node, TR::Compilation* comp)
{
 ...
 */
static TR::Register*
generateFusedMultiplyAdd(TR::Node* addNode,
                         TR::InstOpCode::Mnemonic OpCode,
                         TR::CodeGenerator* cg)
{
...
TR::Register*
OMR::Power::TreeEvaluator::faddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
...
TR::Register*
OMR::Power::TreeEvaluator::daddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Register* result = NULL;
  if ((isFPStrictMul(node->getFirstChild(), comp) ||
...
TR::Register*
OMR::Power::TreeEvaluator::dsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Register* result = NULL;
  if (isFPStrictMul(node->getFirstChild(), comp) &&
...
TR::Register*
OMR::Power::TreeEvaluator::fsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Register* result = NULL;
  if (isFPStrictMul(node->getFirstChild(), comp) &&
...
TR::Register*
OMR::Power::TreeEvaluator::fmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return singlePrecisionEvaluator(node, TR::InstOpCode::fmuls, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::dmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return doublePrecisionEvaluator(node, TR::InstOpCode::fmul, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::fdivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return singlePrecisionEvaluator(node, TR::InstOpCode::fdivs, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::ddivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  return doublePrecisionEvaluator(node, TR::InstOpCode::fdiv, cg);
}

TR::Register*
OMR::Power::TreeEvaluator::fremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  ...
  if (!cg->canClobberNodesRegister(child1)) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::fmr, node, copyReg, source1Reg);
    source1Reg = copyReg;
  }
  if (!cg->canClobberNodesRegister(child2)) {
    copyReg = cg->allocateSinglePrecisionRegister();
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::fmr, node, copyReg, source2Reg);
    source2Reg = copyReg;
  }

  TR::addDependency(
    dependencies, source1Reg, TR::RealRegister::fp0, TR_FPR, cg);
  TR::addDependency(
    dependencies, source2Reg, TR::RealRegister::fp1, TR_FPR, cg);
  ...
  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::frsp, node, trgReg, source1Reg);
...
TR::Register*
OMR::Power::TreeEvaluator::dremEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child1 = node->getFirstChild();
  TR::Node* child2 = node->getSecondChild();
  TR::Register* source1Reg = cg->evaluate(child1);
 ...
 */
TR::Register*
OMR::Power::TreeEvaluator::fnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  bool noFMA = true;
  TR::Register* trgRegister;
  TR::Compilation* comp = cg->comp();
  ...
  if (firstChild->getReferenceCount() < 2 && !firstChild->getRegister() &&
    ...
    if ((isFPStrictMul(firstChild->getFirstChild(), comp) ||
        ...
        performTransformation(
          comp, "O^O Changing [%p] to fnmadd/sub\n", node)) {
      TR::InstOpCode::Mnemonic opCode =
  ...
  if (noFMA) {
    ...
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::fneg, node, trgRegister, sourceRegister);
  }
  cg->decReferenceCount(firstChild);
  node->setRegister(trgRegister);
  return trgRegister;
...
TR::Register*
OMR::Power::TreeEvaluator::int2dbl(TR::Node* node,
                                   TR::Register* srcReg,
                                   bool canClobber,
                                   TR::CodeGenerator* cg)
{
  if (node->getOpCodeValue() == TR::b2f || node->getOpCodeValue() == TR::b2d) {
    TR::Register* tmpReg = cg->allocateRegister();
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::extsb, node, tmpReg, srcReg);
    srcReg = tmpReg;
    cg->stopUsingRegister(tmpReg);
    ...
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, tmpReg, srcReg, 0, 0xff);
    srcReg = tmpReg;
    cg->stopUsingRegister(tmpReg);
  }
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
    ...
    if (TR::Compiler->target.is64Bit()) {
      if (node->getOpCodeValue() == TR::iu2f ||
        ...
        generateMvFprGprInstructions(
          cg, node, gpr2fprHost64, false, trgReg, tempReg);
        cg->stopUsingRegister(tempReg);
      } else if (node->getOpCodeValue() == TR::i2f ||
                 node->getOpCodeValue() == TR::i2d) {
        tempReg = cg->allocateRegister();
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::extsw, node, tempReg, srcReg);
        generateMvFprGprInstructions(
          cg, node, gpr2fprHost64, false, trgReg, tempReg);
        cg->stopUsingRegister(tempReg);
      } else
      ...
      if (TR::Compiler->target.cpu.id() >= TR_PPCp6 &&
          ...
          node->getOpCodeValue() != TR::iu2d)
        generateMvFprGprInstructions(
          cg, node, gprLow2fpr, false, trgReg, srcReg);
      else {
        tempReg = cg->allocateRegister();
        ...
        if (node->getOpCodeValue() == TR::iu2f ||
          ...
          generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::srawi, node, tempReg, srcReg, 31);

        if (TR::Compiler->target.cpu.id() >= TR_PPCp8) {
          TR::Register* tmp1 = cg->allocateRegister(TR_FPR);
          generateMvFprGprInstructions(
            cg, node, gpr2fprHost32, false, trgReg, tempReg, srcReg, tmp1);
          cg->stopUsingRegister(tmp1);
        } else
          generateMvFprGprInstructions(
            cg, node, gpr2fprHost32, false, trgReg, tempReg, srcReg);
        cg->stopUsingRegister(tempReg);
      }
    }
    if ((TR::Compiler->target.cpu.id() >= TR_PPCp7) &&
      ...
      if (node->getOpCodeValue() == TR::i2f)
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::fcfids, node, trgReg, trgReg);
      else
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::fcfidus, node, trgReg, trgReg);
    } else {
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::fcfid, node, trgReg, trgReg);
      if (node->getOpCodeValue() == TR::i2f ||
          node->getOpCodeValue() == TR::iu2f)
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::frsp, node, trgReg, trgReg);
    }
  } else {
    TR::RegisterDependencyConditions* dependencies = new (cg->trHeapMemory())
    ...
    if (!canClobber) {
      tempReg = cg->allocateRegister();
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::mr, node, tempReg, srcReg);
      srcReg = tempReg;
    }
...
// also handles iu2f
TR::Register*
OMR::Power::TreeEvaluator::i2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* tempReg;
  TR::Register* trgReg;
  ...
  if (((TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
    ...
    if (node->getOpCodeValue() == TR::i2f) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfiwax, node, tempReg, tempMR);
      if (TR::Compiler->target.cpu.id() >= TR_PPCp7) {
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::fcfids, node, trgReg, tempReg);
      } else {
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::fcfid, node, tempReg, tempReg);
        generateTrg1Src1Instruction(
          cg, TR::InstOpCode::frsp, node, trgReg, tempReg);
      }
    } else {
      generateTrg1MemInstruction(cg,
                                 TR::InstOpCode::lfiwzx,
                                 node,
                                 tempReg,
                                 tempMR); // TR::iu2f, must be P7
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::fcfidus, node, trgReg, tempReg);
    }
    tempMR->decNodeReferenceCounts(cg);
    cg->stopUsingRegister(tempReg);
  } else {
    trgReg = TR::TreeEvaluator::int2dbl(
      node, cg->evaluate(child), cg->canClobberNodesRegister(child), cg);
...
TR::Register*
OMR::Power::TreeEvaluator::i2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* trgReg;
  ...
  if (((TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
    ...
    if (node->getOpCodeValue() == TR::i2d) {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfiwax, node, trgReg, tempMR);
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::fcfid, node, trgReg, trgReg);
    } else {
      generateTrg1MemInstruction(
        cg, TR::InstOpCode::lfiwzx, node, trgReg, tempMR); // iu2d
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::fcfidu, node, trgReg, trgReg);
    }
    tempMR->decNodeReferenceCounts(cg);
  } else {
    trgReg = TR::TreeEvaluator::int2dbl(
...
TR::Register*
OMR::Power::TreeEvaluator::long2dbl(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* srcReg = cg->evaluate(child);
  TR::Register* trgReg = cg->allocateRegister(TR_FPR);
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
    ...
    if (!cg->canClobberNodesRegister(child)) {
      ...
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::mr, node, srcHigh, srcReg->getHighOrder());
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::mr, node, srcLow, srcReg->getLowOrder());
    } else {
...
TR::Register*
OMR::Power::TreeEvaluator::long2float(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* srcReg = cg->evaluate(child);
...
TR::Register*
OMR::Power::TreeEvaluator::l2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg;
  TR::Node* child = node->getFirstChild();
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
...
TR::Register*
OMR::Power::TreeEvaluator::l2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg;
  TR::Node* child = node->getFirstChild();
  if (TR::Compiler->target.cpu.id() >= TR_PPCp7 &&
...
TR::Register*
OMR::Power::TreeEvaluator::f2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
...
//
TR::Register*
OMR::Power::TreeEvaluator::d2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  ...
  if (true) {
    ...
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::fcmpu, node, condReg, sourceReg, sourceReg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bne, node, doneLabel, condReg);
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::fctiwz, node, tempReg, sourceReg);
  ...
  if (opcode == TR::f2s || opcode == TR::d2s)
    generateTrg1Src1Instruction(
      cg, TR::InstOpCode::extsh, node, trgReg, trgReg);
  if (opcode == TR::f2c || opcode == TR::d2c)
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, trgReg, trgReg, 0, 0xffff);
...
TR::Register*
OMR::Power::TreeEvaluator::d2iuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Compilation* comp = cg->comp();
  TR::Node* child = node->getFirstChild();
  TR::Register* sourceReg = cg->evaluate(child);
  ...
  if (opcode == TR::f2bu || opcode == TR::d2bu)
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, trgReg, trgReg, 0, 0xff);
  if (opcode == TR::f2c || opcode == TR::d2c)
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, trgReg, trgReg, 0, 0xffff);
...
// also handles f2l
TR::Register*
OMR::Power::TreeEvaluator::d2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* child = node->getFirstChild();
  TR::Register* sourceReg = cg->evaluate(child);
  TR::Register* trgReg;
  ...
  if (cg->is64BitProcessor() || (cg->comp()->compileRelocatableCode() &&
    ...
    if (!cg->canClobberNodesRegister(child)) {
      if (node->getOpCodeValue() == TR::f2l)
        ...
        copyReg = cg->allocateRegister(TR_FPR);
      generateTrg1Src1Instruction(
        cg, TR::InstOpCode::fmr, node, copyReg, sourceReg);
      sourceReg = copyReg;
    }
    ...
    TR::addDependency(
      dependencies, sourceReg, TR::RealRegister::fp0, TR_FPR, cg);
    TR::addDependency(
      dependencies, trgReg->getHighOrder(), TR::RealRegister::gr3, TR_GPR, cg);
    TR::addDependency(
      dependencies, trgReg->getLowOrder(), TR::RealRegister::gr4, TR_GPR, cg);
    TR::addDependency(dependencies, NULL, TR::RealRegister::gr0, TR_GPR, cg);
    TR::addDependency(dependencies, NULL, TR::RealRegister::gr5, TR_GPR, cg);
    TR::addDependency(dependencies, NULL, TR::RealRegister::gr6, TR_GPR, cg);
    TR::addDependency(dependencies, NULL, TR::RealRegister::gr11, TR_GPR, cg);
    ...
    dependencies->stopUsingDepRegs(
      cg, trgReg->getHighOrder(), trgReg->getLowOrder());
    cg->machine()->setLinkRegisterKilled(true);
  }
...
TR::Register*
OMR::Power::TreeEvaluator::d2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Register* trgReg = cg->allocateSinglePrecisionRegister();
  TR::Node* child = node->getFirstChild();

  generateTrg1Src1Instruction(
    cg, TR::InstOpCode::frsp, node, trgReg, cg->evaluate(child));
  cg->decReferenceCount(child);
  node->setRegister(trgReg);
  return trgReg;
}
...
OMR::Power::TreeEvaluator::dcmpeqEvaluator(TR::Node* node,
                                           TR::CodeGenerator* cg)
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::beq, TR::InstOpCode::bad, 0, node, cg);
}
...
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::bne, TR::InstOpCode::bad, 0, node, cg);
}
                                           ...
                                           TR::CodeGenerator* cg)
{
  ...
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::blt, TR::InstOpCode::bad, imm, node, cg);
}
...
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::bgt, TR::InstOpCode::beq, 0, node, cg);
}
                                           ...
                                           TR::CodeGenerator* cg)
{
  int64_t imm = 0;
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::bgt, TR::InstOpCode::bad, imm, node, cg);
}
...
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::blt, TR::InstOpCode::beq, 0, node, cg);
}
...
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::beq, TR::InstOpCode::bun, 0, node, cg);
}
...
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::blt, TR::InstOpCode::bun, 0, node, cg);
}
...
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::bge, TR::InstOpCode::bad, 0, node, cg);
}
...
{
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::bgt, TR::InstOpCode::bun, 0, node, cg);
}
                                            ...
                                            TR::CodeGenerator* cg)
{
  ...
  // than equal to or unordered
  return compareFloatAndSetOrderedBoolean(
    TR::InstOpCode::ble, TR::InstOpCode::bad, 0, node, cg);
}

// also handles TR::fcmpl
TR::Register*
OMR::Power::TreeEvaluator::dcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  if (true) {
    ...
    TR::LabelSymbol* doneLabel = generateLabelSymbol(cg);
    generateTrg1Src2Instruction(
      cg, TR::InstOpCode::fcmpu, node, condReg, src1Reg, src2Reg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 1);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bgt, node, doneLabel, condReg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, doneLabel, condReg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, -1);
...
TR::Register*
OMR::Power::TreeEvaluator::dcmpgEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  TR::Register* src1Reg = cg->evaluate(firstChild);
  ...
  if (true) {
    ...
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, -1);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::blt, node, doneLabel, condReg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, doneLabel, condReg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 1);
                         ...
                         TR::CodeGenerator* cg)
{
  ...
  TR::Register* trgRegister = cg->allocateSinglePrecisionRegister();
  generateTrg1Src2Instruction(
    cg, OpCode, node, trgRegister, source1Register, source2Register);
  node->setRegister(trgRegister);
  cg->decReferenceCount(firstChild);
  cg->decReferenceCount(secondChild);
  return trgRegister;
                 ...
                 TR::CodeGenerator* cg)
{
  ...
  if (branchOp2 == TR::InstOpCode::bad && deps != NULL) {
    generateDepConditionalBranchInstruction(
      cg, branchOp1, node, label, conditionRegister, deps);

  } else {
    generateConditionalBranchInstruction(
      cg, branchOp1, node, label, conditionRegister);
  }

  if (branchOp2 != TR::InstOpCode::bad) {
    if (deps == NULL) {
      generateConditionalBranchInstruction(
        cg, branchOp2, node, label, conditionRegister);
    } else {
      generateDepConditionalBranchInstruction(
        cg, branchOp2, node, label, conditionRegister, deps);
    }
  }
                                 ...
                                 TR::CodeGenerator* cg)
{
  ...
  if (true) {
    ...
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 1);
    generateConditionalBranchInstruction(
      cg, branchOp1, node, doneLabel, condReg);
    if (branchOp2 != TR::InstOpCode::bad)
      generateConditionalBranchInstruction(
        cg, branchOp2, node, doneLabel, condReg);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, trgReg, 0);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, doneLabel);
  }
  node->setRegister(trgReg);
...
TR::Register*
OMR::Power::TreeEvaluator::iexpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_UNIMPLEMENTED();
  return 0;
}
...
TR::Register*
OMR::Power::TreeEvaluator::lexpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_UNIMPLEMENTED();
  return 0;
}
...
// also handles fexp
TR::Register*
OMR::Power::TreeEvaluator::dexpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR_UNIMPLEMENTED();
  return 0;
}
...
// also handles fxfrs
TR::Register*
OMR::Power::TreeEvaluator::dxfrsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Node* secondChild = node->getSecondChild();
  ...
  TR::Register* condReg = cg->allocateRegister(TR_CCR);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::fcmpu, node, condReg, tmpReg, reg2);
  cg->stopUsingRegister(tmpReg);
  ...
  TR::LabelSymbol* doneLabel = generateLabelSymbol(cg);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::ble, node, doneLabel, condReg);
  cg->stopUsingRegister(condReg);
...
// also handles fint
TR::Register*
OMR::Power::TreeEvaluator::dintEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* srcReg = cg->evaluate(firstChild);
...
// also handles fnint
TR::Register*
OMR::Power::TreeEvaluator::dnintEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* srcReg = cg->evaluate(firstChild);
  ...
  TR::Register* condReg = cg->allocateRegister(TR_CCR);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::fcmpu, node, condReg, tmp1Reg, tmp2Reg);

  TR::LabelSymbol* moveLabel = generateLabelSymbol(cg);
  generateConditionalBranchInstruction(
    cg, TR::InstOpCode::ble, node, moveLabel, condReg);
  cg->stopUsingRegister(condReg);
  ...
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::fadd, node, tmp1Reg, srcReg, tmp1Reg);
  generateTrg1Src2Instruction(
    cg, TR::InstOpCode::fadd, node, tmp2Reg, srcReg, tmp2Reg);
  generateTrg1Src3Instruction(
    cg, TR::InstOpCode::fsel, node, trgReg, srcReg, tmp1Reg, tmp2Reg);
  cg->stopUsingRegister(tmp1Reg);
  cg->stopUsingRegister(tmp2Reg);
...
TR::Register*
OMR::Power::TreeEvaluator::fsqrtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* srcReg = cg->evaluate(firstChild);
...
TR::Register*
OMR::Power::TreeEvaluator::dsqrtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
  TR::Node* firstChild = node->getFirstChild();
  TR::Register* srcReg = cg->evaluate(firstChild);
                                             ...
                                             TR::CodeGenerator* cg)
{
  ...
  TR::Register* spReg =
    cg->machine()->getRealRegister(properties.getNormalStackPointerRegister());
  TR::Register* trgReg = cg->allocateRegister();
