...
TR::Register *OMR::Power::TreeEvaluator::arraysetEvaluator(
  ...
  if (constFill) {
    ...
    switch (fillNodeSize) {
      ...
      case 4: {
        uint64_t word =
            (uint64_t)(fillNode->getOpCodeValue() == TR::fconst
      ...
      case 8: {
        if (fillNode->getDataType() == TR::Double) {
          ...
          if (!fp1Reg) fp1Reg = cg->evaluate(fillNode);
          fillReg = cg->allocateRegister();
  ...
  } else  // variable fill value
  {
    switch (fillNodeSize) {
      ...
      case 8: {
        ...
        if (!fillReg) fillReg = cg->allocateRegister();
      } break;
      default:
        TR_ASSERT(false, "unrecognised address precision\n");
    }
