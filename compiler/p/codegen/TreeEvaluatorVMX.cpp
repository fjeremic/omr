...
OMR::Power::TreeEvaluator::arraysetEvaluator(TR::Node *node,
  ...
  if (constFill) {
    ...
    case 1: {
      ...
      if (dofastPath && TR::Compiler->target.is64Bit()) {
        generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, fillReg,
                                   halfword);
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node,
                                        fillReg, fillReg, 16, 0xffff0000);
        generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldimi, node,
                                        fillReg, fillReg, 32,
                                        0xffffffff00000000ULL);
      } else {
        generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, fillReg,
                                   halfword);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, fillReg,
                                       fillReg, halfword);
      }
    } break;
    case 2: {
    ...
    if (fillNode->getDataType() != TR::Double && dofastPath) {
      ...
      if (TR::Compiler->target.is32Bit()) {
        fixedSeqMemAccess(cg, node, 0, q, fp1Reg, tempReg, TR::InstOpCode::lfd,
                          8, NULL, tempReg);
        cg->findOrCreateFloatConstant(&doubleword, TR::Double, q[0], q[1], q[2],
                                      q[3]);
      }
    }
  } else // variable fill value
  {
    ...
    case 1: {
      ...
      generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, fillReg,
                                      fillReg, 8, 0xff00);
      generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, fillReg,
                                      fillReg, 16, 0xffff0000);
    } break;
    case 2: {
      fillReg = cg->gprClobberEvaluate(fillNode);
      generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwimi, node, fillReg,
    ...
    case 8: {
      if (fillNode->getDataType() == TR::Double) {
        ...
        generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, tempMRStore1,
                                   valueReg->getHighOrder());
        generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, tempMRStore2,
                                   valueReg->getLowOrder());
        generateInstruction(cg, TR::InstOpCode::lwsync, node);
        generateTrg1MemInstruction(
            cg, TR::InstOpCode::lfd, node, fp1Reg,
  ...
  if (dofastPath) {
    ...
    if (1 == fillNodeSize) {
      ...
      generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
                                 ...
                                 fillReg);
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, addrReg,
                                     addrReg, 1);
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, lenReg,
                                     lenReg, -1);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi_r, node, tempReg,
                                   lenReg, 6);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, label2,
                                         condReg);
    ...
    generateMemSrc1Instruction(cg, dblStoreOp, node,
                               new (cg->trHeapMemory())
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, addrReg,
                                   addrReg, 64);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node, label3,
                                         condReg);
    generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, lenReg,
                                    lenReg, 0, 63);                    // Mask
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi_r, node, tempReg,
                                   lenReg, 3);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, label5,
                                         condReg);
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, addrReg,
                                   addrReg, 8);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node, label7,
                                         condReg);
  ...
  } else // regular word store path
  {
    ...
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, label3); // L3
    generateMemSrc1Instruction(cg, wdStoreOp, node,
                               new (cg->trHeapMemory())
                                   TR::MemoryReference(addrReg, 0, 4, cg),
                               fillReg);
    generateMemSrc1Instruction(cg, wdStoreOp, node,
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::srawi_r, node, tempReg,
                                   lenReg, 2);
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, label4,
                                         condReg);
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
...
TR::Register *OMR::Power::TreeEvaluator::arraytranslateAndTestEvaluator(
