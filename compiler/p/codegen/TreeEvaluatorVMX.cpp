...
TR::Register*
OMR::Power::TreeEvaluator::arraysetEvaluator(TR::Node* node,
                                             TR::CodeGenerator* cg)
{
  ...
  bool dofastPath =
    ((constLength && constFill && (length >= 64)) || (8 == fillNodeSize));
  ...
  if (constFill) {
    ...
    switch (fillNodeSize) {
      case 1: {
        ...
        if (dofastPath) {
          doubleword =
            (halfword << 48) | (halfword << 32) | (halfword << 16) | halfword;
        }
        fillReg = cg->allocateRegister();
        if (dofastPath && TR::Compiler->target.is64Bit()) {
          generateTrg1ImmInstruction(
            cg, TR::InstOpCode::li, node, fillReg, halfword);
          generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwimi, node, fillReg, fillReg, 16, 0xffff0000);
          ...
          generateTrg1ImmInstruction(
            cg, TR::InstOpCode::lis, node, fillReg, halfword);
          generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::ori, node, fillReg, fillReg, halfword);
        }
      } break;
      case 2: {
      ...
      case 4: {
        uint64_t word = (uint64_t)(fillNode->getOpCodeValue() == TR::fconst
                                     ? fillNode->getFloatBits() & 0xffffffff
    ...
    if (fillNode->getDataType() != TR::Double && dofastPath) {
      ...
      if (TR::Compiler->target.is32Bit()) {
        fixedSeqMemAccess(cg,
                          ...
                          NULL,
                          tempReg);
        cg->findOrCreateFloatConstant(
          &doubleword, TR::Double, q[0], q[1], q[2], q[3]);
      }
    }
  } else // variable fill value
  {
    switch (fillNodeSize) {
      case 1: {
        ...
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rlwimi, node, fillReg, fillReg, 8, 0xff00);
        generateTrg1Src1Imm2Instruction(
          cg, TR::InstOpCode::rlwimi, node, fillReg, fillReg, 16, 0xffff0000);
      } break;
      case 2: {
  ...
  if (dofastPath) {
    ...
    if (1 == fillNodeSize) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, tempReg, addrReg, 1); // 2 aligned??
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, label0, condReg);
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stb,
                                 node,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(addrReg, 0, 1, cg),
                                 fillReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, addrReg, addrReg, 1);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, lenReg, lenReg, -1);
    ...
    if (1 == fillNodeSize || 2 == fillNodeSize) {
      generateTrg1Src1ImmInstruction(cg,
                                     ...
                                     2);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::blt, node, label1, condReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, tempReg, addrReg, 2); // 4 aligned??
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, label1, condReg);
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::sth,
                                 node,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(addrReg, 0, 2, cg),
                                 fillReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, addrReg, addrReg, 2);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, lenReg, lenReg, -2);
    ...
    if (1 == fillNodeSize || 2 == fillNodeSize || 4 == fillNodeSize) {
      generateTrg1Src1ImmInstruction(cg,
                                     ...
                                     4);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::blt, node, label6, condReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, tempReg, addrReg, 4); // 8 aligned??
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, label6, condReg);
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stw,
                                 node,
                                 new (cg->trHeapMemory())
                                   TR::MemoryReference(addrReg, 0, 4, cg),
                                 fillReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, addrReg, addrReg, 4);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, node, lenReg, lenReg, -4);
    ...
    if (TR::Compiler->target.is64Bit() &&
        fillNode->getDataType() != TR::Double) {
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::srawi_r, node, tempReg, lenReg, 6);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, label2, condReg);
    ...
    generateMemSrc1Instruction(cg,
                               ...
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 0, 8, cg),
                               dblFillReg);
    generateMemSrc1Instruction(cg,
                               dblStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 8, 8, cg),
                               dblFillReg);
    generateMemSrc1Instruction(cg,
                               dblStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 16, 8, cg),
                               dblFillReg);
    generateMemSrc1Instruction(cg,
                               dblStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 24, 8, cg),
                               dblFillReg);
    generateMemSrc1Instruction(cg,
                               dblStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 32, 8, cg),
                               dblFillReg);
    generateMemSrc1Instruction(cg,
                               dblStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 40, 8, cg),
                               dblFillReg);
    generateMemSrc1Instruction(cg,
                               dblStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 48, 8, cg),
                               dblFillReg);
    generateMemSrc1Instruction(cg,
                               dblStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 56, 8, cg),
                               dblFillReg);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi, node, addrReg, addrReg, 64);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bdnz, node, label3, condReg);
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, lenReg, lenReg, 0, 63);        // Mask
    generateLabelInstruction(cg, TR::InstOpCode::label, node, label2); // L2

    // residual double words
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::srawi_r, node, tempReg, lenReg, 3);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, label5, condReg);
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
    ...
    generateMemSrc1Instruction(cg,
                               ...
                               dblFillReg);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi, node, addrReg, addrReg, 8);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bdnz, node, label7, condReg);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, label5); // L5

    // residual words
    if (1 == fillNodeSize || 2 == fillNodeSize || 4 == fillNodeSize) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, tempReg, lenReg, 4);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, label4, condReg);
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::stw,
                                 node,
                                 new (cg->trHeapMemory())
    ...
    if (1 == fillNodeSize || 2 == fillNodeSize) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, tempReg, lenReg, 2);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, label9, condReg);
      generateMemSrc1Instruction(cg,
                                 TR::InstOpCode::sth,
                                 node,
                                 new (cg->trHeapMemory())
    ...
    if (1 == fillNodeSize) {
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, tempReg, lenReg, 1);
      generateConditionalBranchInstruction(
    ...
    generateDepLabelInstruction(
      cg, TR::InstOpCode::label, node, donelabel, deps); // LdoneLabel
  ...
  } else // regular word store path
  {
    ...
    if (1 == fillNodeSize || 2 == fillNodeSize) {
      generateTrg1Src1ImmInstruction(cg,
                                     ...
                                     2);
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::blt, node, label5, condReg);
      generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, tempReg, addrReg, 2); // 4 aligned??
      generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, node, label5, condReg);
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::srawi_r, node, tempReg, lenReg, 5);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, label2, condReg);
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
    generateLabelInstruction(cg, TR::InstOpCode::label, node, label3); // L3
    ...
    generateMemSrc1Instruction(cg,
                               ...
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 4, 4, cg),
                               fillReg);
    generateMemSrc1Instruction(cg,
                               wdStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 8, 4, cg),
                               fillReg);
    generateMemSrc1Instruction(cg,
                               wdStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 12, 4, cg),
                               fillReg);
    generateMemSrc1Instruction(cg,
                               wdStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 16, 4, cg),
                               fillReg);
    generateMemSrc1Instruction(cg,
                               wdStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 20, 4, cg),
                               fillReg);
    generateMemSrc1Instruction(cg,
                               wdStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 24, 4, cg),
                               fillReg);
    generateMemSrc1Instruction(cg,
                               wdStoreOp,
                               node,
                               new (cg->trHeapMemory())
                                 TR::MemoryReference(addrReg, 28, 4, cg),
                               fillReg);
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi, node, addrReg, addrReg, 32);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bdnz, node, label3, condReg);
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, lenReg, lenReg, 0, 31);        // Mask
    generateLabelInstruction(cg, TR::InstOpCode::label, node, label2); // L2

    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::srawi_r, node, tempReg, lenReg, 2);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::beq, node, label4, condReg);
    generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tempReg);
    ...
    generateTrg1Src1ImmInstruction(
      cg, TR::InstOpCode::addi, node, addrReg, addrReg, 4);
    generateConditionalBranchInstruction(
      cg, TR::InstOpCode::bdnz, node, label10, condReg);
    generateTrg1Src1Imm2Instruction(
      cg, TR::InstOpCode::rlwinm, node, lenReg, lenReg, 0, 3);         // Mask
    generateLabelInstruction(cg, TR::InstOpCode::label, node, label4); // L4
...
TR::Register*
OMR::Power::TreeEvaluator::arraytranslateAndTestEvaluator(TR::Node* node,
                                                          TR::CodeGenerator* cg)
{
  return NULL;
}
