...
void OMR::ConstantDataSnippet::emitFloatingPointConstant(
  TR_Array<TR::Instruction*>& requestors, uint8_t* codeCursor, int32_t count)
{
  ...
  for (i = 0; i < size; i += count) {
    ...
    if (count == 4) {
      ...
      *(int32_t*)iloc2 |= LO_VALUE(addr) & 0x0000ffff;
      ...
      cg()->addExternalRelocation(new (_cg->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
                                    iloc1, iloc2, (uint8_t*)recordInfo, TR_AbsoluteMethodAddressOrderedPair, cg()),
        __FILE__, __LINE__, requestors[i]->getNode());
    }
  }
}
...
void OMR::ConstantDataSnippet::emitAddressConstant(
  PPCConstant<intptrj_t>* acursor, TR_Array<TR::Instruction*>& requestors, uint8_t* codeCursor, int32_t count)
{
  uint8_t *iloc1, *iloc2, *iloc3, *iloc4;
  intptrj_t addr;
  int32_t i;
  ...
  if (cg()->profiledPointersRequireRelocation()) {
    ...
    if (node != NULL && node->getOpCodeValue() == TR::aconst) {
      if (comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        TR::Relocation* relo = new (cg()->trHeapMemory())
          TR::ExternalRelocation(codeCursor, (uint8_t*)node->getAddress(), (uint8_t*)type, TR_SymbolFromManager, cg());
    ...
    if (acursor->isUnloadablePicSite()) {
      ...
      cg()->jitAddPicToPatchOnClassUnload(
        (void*)acursor->getConstantValue(), (void*)(codeCursor + ((TR::Compiler->target.is64Bit()) ? 4 : 0)));
    }
  }
