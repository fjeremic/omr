...
int32_t OMR::ConstantDataSnippet::addConstantRequest(
  ...
  case TR::Address: {
    ...
    while (acursor != NULL) {
      ...
      // if pointers require relocation, then not all pointers may be relocated
      // for the same reason
      //   so be conservative and do not combine them (e.g. HCR versus profiled
      //   inlined site enablement)
    ...
    if (acursor && acursor->isUnloadablePicSite() != isUnloadablePicSite) {
      TR_ASSERT(0, "Existing address constant does not have a matching "
                   "unloadable state.\n");
      acursor = NULL; // If asserts are turned off then we should just create a
                      // duplicate constant
    }
    if (acursor == NULL) {
  ...
  default:
    TR_ASSERT(
        0, "Only float and address constants are supported. Data type is %s.\n",
        type.toString());
  }
