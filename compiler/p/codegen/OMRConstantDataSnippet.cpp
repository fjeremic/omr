...
int32_t
OMR::ConstantDataSnippet::addConstantRequest(void* v,
                                             TR::DataType type,
...
bool
OMR::ConstantDataSnippet::getRequestorsFromNibble(TR::Instruction* nibble,
                                                  TR::Instruction** q,
                                                  bool remove)
{
  ListIterator<PPCConstant<double>> diterator(&_doubleConstants);
  ...
  for (i = 0; i < size; i += count) {
    ...
    if (count == 4) {
      ...
      if (!cg()->comp()->compileRelocatableCode()) {
        ...
        *(int32_t*)iloc1 |= (addr >> 32) & 0x0000ffff;
        ...
        cg()->addExternalRelocation(
          new (cg()->trHeapMemory())
            TR::BeforeBinaryEncodingExternalRelocation(requestors[i],
                                                       (uint8_t*)(addr),
      ...
      *(int32_t*)iloc2 |= LO_VALUE(addr) & 0x0000ffff;
      ...
      cg()->addExternalRelocation(new (_cg->trHeapMemory())
    ...
    if (node != NULL && node->getOpCodeValue() == TR::aconst) {
      if (comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        TR::Relocation* relo = new (cg()->trHeapMemory())
          TR::ExternalRelocation(codeCursor,
...
uint8_t*
OMR::ConstantDataSnippet::emitSnippetBody()
{
...
uint32_t
OMR::ConstantDataSnippet::getLength()
{
...
#if DEBUG
void
OMR::ConstantDataSnippet::print(TR::FILE* outFile)
{
  if (outFile == NULL)
    return;
  ...
  while (dcursor != NULL) {
    if (TR::Compiler->target.is32Bit() || dcursor->getRequestors().size() > 0) {
      trfprintf(outFile,
                ...
                *(int32_t*)codeCursor,
                *(int32_t*)(codeCursor + 4),
                dcursor->getConstantValue());
      codeCursor += 8;
    }
    dcursor = diterator.getNext();
  }
