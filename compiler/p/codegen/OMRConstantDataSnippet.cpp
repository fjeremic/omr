...
int32_t OMR::ConstantDataSnippet::addConstantRequest(
    void *v, TR::DataType type, TR::Instruction *nibble0,
    TR::Instruction *nibble1, TR::Instruction *nibble2,
    TR::Instruction *nibble3, TR::Node *node, bool isUnloadablePicSite) {
  ...
  switch (type) {
    case TR::Float: {
      ...
      while (fcursor != NULL) {
        ...
        if (fin.ivalue == fex.ivalue) break;
    ...
    case TR::Double: {
      ...
      while (dcursor != NULL) {
        ...
        if (din.lvalue == dex.lvalue) break;
        dcursor = diterator.getNext();
      }
      if (dcursor == NULL) {
        dcursor =
            new (_cg->trHeapMemory()) PPCConstant<double>(_cg, din.dvalue);
    ...
    case TR::Address: {
      ...
      while (acursor != NULL) {
        ...
        // if pointers require relocation, then not all pointers may be
        // relocated for the same reason
        //   so be conservative and do not combine them (e.g. HCR versus
        //   profiled inlined site enablement)
        if (ain == aex && (!cg()->profiledPointersRequireRelocation() ||
                           acursor->getNode() == node))
          break;
        acursor = aiterator.getNext();
      }
      if (acursor && acursor->isUnloadablePicSite() != isUnloadablePicSite) {
        TR_ASSERT(0,
                  "Existing address constant does not have a matching "
                  "unloadable state.\n");
        acursor = NULL;  // If asserts are turned off then we should just create
                         // a duplicate constant
      }
      if (acursor == NULL) {
        acursor = new (_cg->trHeapMemory())
            PPCConstant<intptrj_t>(_cg, ain, node, isUnloadablePicSite);
    ...
    default:
      TR_ASSERT(
          0,
          "Only float and address constants are supported. Data type is %s.\n",
          type.toString());
  }
...
bool OMR::ConstantDataSnippet::getRequestorsFromNibble(TR::Instruction *nibble,
                                                       TR::Instruction **q,
                                                       bool remove) {
  ListIterator<PPCConstant<double> > diterator(&_doubleConstants);
  PPCConstant<double> *dcursor = diterator.getFirst();
    ...
    if (requestors.size() > 0) {
      for (int32_t i = 0; i < requestors.size(); i += count) {
        ...
        } else  // count == 4
        {
          int32_t j;
          if (requestors[i] == nibble || requestors[i + 1] == nibble ||
              requestors[i + 2] == nibble || requestors[i + 3] == nibble) {
            for (j = 0; j < count; j++) q[j] = requestors[i + j];
            if (remove) {
              for (j = count - 1; j >= 0; j--) requestors.remove(i + j);
            }
            return true;
          }
        }
...
void OMR::ConstantDataSnippet::emitFloatingPointConstant(
    TR_Array<TR::Instruction *> &requestors, uint8_t *codeCursor,
    int32_t count) {
  ...
  for (i = 0; i < size; i += count) {
    ...
    if (count == 4) {
        ...
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory())
                TR::BeforeBinaryEncodingExternalRelocation(
                    requestors[i], (uint8_t *)(addr), (uint8_t *)fixedSequence4,
      ...
      *(int32_t *)iloc2 |= LO_VALUE(addr) & 0x0000ffff;
      ...
      cg()->addExternalRelocation(
          new (_cg->trHeapMemory()) TR::ExternalOrderedPair32BitRelocation(
              iloc1, iloc2, (uint8_t *)recordInfo,
...
void OMR::ConstantDataSnippet::emitAddressConstant(
    PPCConstant<intptrj_t> *acursor, TR_Array<TR::Instruction *> &requestors,
    uint8_t *codeCursor, int32_t count) {
  uint8_t *iloc1, *iloc2, *iloc3, *iloc4;
  intptrj_t addr;
  int32_t i;
  int32_t size = requestors.size();
  ...
  if (cg()->profiledPointersRequireRelocation()) {
    ...
    if (node != NULL && node->getOpCodeValue() == TR::aconst) {
      if (comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        TR::Relocation *relo = new (cg()->trHeapMemory())
            TR::ExternalRelocation(codeCursor, (uint8_t *)node->getAddress(),
        ...
        if (node->isClassPointerConstant())
          ...
          kind = (node->getInlinedSiteIndex() == -1) ? TR_RamMethod
                                                     : TR_MethodPointer;

        if (kind != TR_NoRelocation) {
          TR::Relocation *relo = new (cg()->trHeapMemory())
              TR::ExternalRelocation(codeCursor, (uint8_t *)node, kind, cg());
          cg()->addExternalRelocation(relo, __FILE__, __LINE__, node);
        }
      }
    }
    ...
    cg()->jitAddPicToPatchOnClassRedefinition(
        (void *)acursor->getConstantValue(), (void *)codeCursor);

    if (acursor->isUnloadablePicSite()) {
      // Register an unload assumption on the lower 32bit of the class constant.
      // The patching code thinks it's low bit tagging an instruction not a
      // class pointer!!
  ...
  for (i = 0; i < size; i += count) {
    ...
    iloc2 = requestors[i + 1]->getBinaryEncoding();
    addr = (intptrj_t)codeCursor;
    // if it's the start PC, don't use the constant, but grab it from the
    // symbol:
...
uint8_t *OMR::ConstantDataSnippet::emitSnippetBody() {
  ...
  // Emit order is double, address, float. In this order we can ensure the 8
  // bytes alignment of double and address.
...
uint32_t OMR::ConstantDataSnippet::getLength() {
  return _doubleConstants.getSize() * 8 + _floatConstants.getSize() * 4 +
         _addressConstants.getSize() * sizeof(intptrj_t) + 4;
}
...
void OMR::ConstantDataSnippet::print(TR::FILE *outFile) {
  ...
  while (dcursor != NULL) {
    if (TR::Compiler->target.is32Bit() || dcursor->getRequestors().size() > 0) {
      trfprintf(outFile, "\n%08x %08x %08x\t\t; %16f Double",
                codeCursor - codeStart, *(int32_t *)codeCursor,
