...
TR_PPCScratchRegisterDependencyConditions::createDependencyConditions(
  ...
  bool liveVSXVectorReg =
      (!lrVector || (lrVector->getNumberOfLiveRegisters() > 0));
  TR_LiveRegisters *lrScalar = cg->getLiveRegisters(TR_VSX_SCALAR);
  bool liveVSXScalarReg =
      (!lrScalar || (lrScalar->getNumberOfLiveRegisters() > 0));
  if (liveVSXVectorReg) {
    preCount += 64;
    postCount += 64;
  ...
  TR::RegisterDependencyConditions *dependencies = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(preCount, postCount, cg->trMemory());
  for (int i = 0; i < (pre ? pre->_numGPRDeps : 0); ++i) {
    dependencies->addPreCondition(pre->_gprDeps[i].getRegister(),
                                  pre->_gprDeps[i].getRealRegister(),
                                  pre->_gprDeps[i].getFlags());
    if (pre->_excludeGPR0 & (1 << i))
      dependencies->getPreConditions()
          ->getRegisterDependency(i)
          ->setExcludeGPR0();
  }
  for (int i = 0; i < (post ? post->_numGPRDeps : 0); ++i) {
    dependencies->addPostCondition(post->_gprDeps[i].getRegister(),
                                   post->_gprDeps[i].getRealRegister(),
  ...
  for (int i = 0; i < (pre ? pre->_numCCRDeps : 0); ++i) {
    dependencies->addPreCondition(pre->_ccrDeps[i].getRegister(),
                                  pre->_ccrDeps[i].getRealRegister(),
                                  pre->_ccrDeps[i].getFlags());
  }
  for (int i = 0; i < (post ? post->_numCCRDeps : 0); ++i) {
    dependencies->addPostCondition(post->_ccrDeps[i].getRegister(),
                                   post->_ccrDeps[i].getRealRegister(),
                                   post->_ccrDeps[i].getFlags());
  }

  const TR::PPCLinkageProperties &properties =
      cg->getLinkage()->getProperties();
  if (liveVSXVectorReg) {
    for (int32_t i = TR::RealRegister::FirstVSR; i <= TR::RealRegister::LastVSR;
         i++) {
    ...
    if (liveVSXScalarReg) {
      for (int32_t i = TR::RealRegister::vsr32; i <= TR::RealRegister::LastVSR;
           i++) {
        if (!properties.getPreserved((TR::RealRegister::RegNum)i)) {
          TR::Register *vreg = cg->allocateRegister(TR_FPR);
          vreg->setPlaceholderReg();
...
OMR::Power::RegisterDependencyConditions::RegisterDependencyConditions(
  ...
  _postConditions =
      new (totalNum, cg->trMemory()) TR_PPCRegisterDependencyGroup;
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    TR_GlobalRegisterNumber validHighRegNum =
        TR::TreeEvaluator::getHighGlobalRegisterNumberIfAny(child, cg);

    if (validHighRegNum != -1) {
      highRegNum =
          (TR::RealRegister::RegNum)cg->getGlobalRegister(validHighRegNum);
    ...
    TR_ASSERT(!regList.find(reg) && (!highReg || !regList.find(highReg)),
              "Should not happen\n");
  ...
  for (i = 0; i < node->getNumChildren(); i++) {
    ...
    if (regList.find(reg) || (highReg && regList.find(highReg))) {
      ...
      if (regList.find(reg)) {
        ...
        if (reg->getPinningArrayPointer()) containsInternalPointer = true;

        copyReg =
            (reg->containsCollectedReference() && !containsInternalPointer)
        ...
        if (opCode == TR::InstOpCode::vor || opCode == TR::InstOpCode::xxlor)
          iCursor = generateTrg1Src2Instruction(cg, opCode, node, copyReg, reg,
                                                reg, iCursor);
        else
          iCursor = generateTrg1Src1Instruction(cg, opCode, node, copyReg, reg,
                                                iCursor);
      ...
      if (highReg && regList.find(highReg)) {
        bool containsInternalPointer = false;
        if (highReg->getPinningArrayPointer()) containsInternalPointer = true;
        ...
        if (opCode == TR::InstOpCode::vor || opCode == TR::InstOpCode::xxlor)
          iCursor = generateTrg1Src2Instruction(cg, opCode, node, highCopyReg,
                                                highReg, highReg, iCursor);
        else
          iCursor = generateTrg1Src1Instruction(cg, opCode, node, highCopyReg,
                                                highReg, iCursor);
  ...
  if (iCursor != NULL && cursorPtr != NULL) *cursorPtr = iCursor;
}

void OMR::Power::RegisterDependencyConditions::unionNoRegPostCondition(
...
bool OMR::Power::RegisterDependencyConditions::defsRealRegister(
    TR::Register *r) {
  for (int i = 0; i < _addCursorForPre; i++) {
    if (_preConditions->getRegisterDependency(i)
                ->getRegister()
                ->getAssignedRegister() == r &&
        _preConditions->getRegisterDependency(i)->getDefsRegister()) {
      return true;
    }
  }
  for (int j = 0; j < _addCursorForPost; j++) {
    if (_postConditions->getRegisterDependency(j)
                ->getRegister()
                ->getAssignedRegister() == r &&
        _postConditions->getRegisterDependency(j)->getDefsRegister()) {
      return true;
...
void OMR::Power::RegisterDependencyConditions::bookKeepingRegisterUses(
  ...
  if (instr->getOpCodeValue() == TR::InstOpCode::assocreg) return;

  // Don't track associations or emit assocregs in outlined code
  // Register assigner can save/restore associations across outlined sections
  // properly, however no such mechanism exists for instruction selection so we
  // don't want these associations to clobber the associations that were set in
  // main line code, which are more important
  // TODO: Fix this by saving/restoring the associations in
  // swapInstructionListsWithCompilation()
  bool isOOL = cg->getIsInOOLSection();

  TR::Machine *machine = cg->machine();
  assoc = !isOOL ? new (cg->trHeapMemory()) TR::RegisterDependencyConditions(
                       0, _addCursorForPre, cg->trMemory())
                 : 0;
  ...
  if (numAssoc > 0) {
    ...
    generateDepInstruction(cg, TR::InstOpCode::assocreg, instr->getNode(),
                           assoc, instr->getPrev());
  }
...
OMR::Power::RegisterDependencyConditions::clone(
  ...
  result = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(
      getNumPreConditions() + addPre, getNumPostConditions() + addPost,
      cg->trMemory());
  ...
  for (idx = 0; idx < postNum; idx++) {
    ...
    if (!((TR::RealRegister::NoReg == singlePair->getRealRegister()) &&
          (added->postConditionContainsVirtual(singlePair->getRegister()))))
      result->addPostCondition(singlePair->getRegister(),
                               singlePair->getRealRegister(),
                               singlePair->getFlags());
#if defined(DEBUG)
    if (singlePair->getRegister()->getKind() == TR_GPR) postGPR++;
#endif
  }
  ...
  for (idx = 0; idx < preNum; idx++) {
    ...
    // eliminate duplicate virtual->NoReg dependencies in 'this' conditions
    if (!((TR::RealRegister::NoReg == singlePair->getRealRegister()) &&
          (added->preConditionContainsVirtual(singlePair->getRegister()))))
      result->addPreCondition(singlePair->getRegister(),
                              singlePair->getRealRegister(),
                              singlePair->getFlags());
#if defined(DEBUG)
    if (singlePair->getRegister()->getKind() == TR_GPR) preGPR++;
#endif
  }
  ...
  TR_ASSERT(preGPR <= max_gpr && postGPR <= max_gpr,
            "Over the limit of available global regsiters.");
#endif
  return result;
}

TR::RegisterDependencyConditions *
OMR::Power::RegisterDependencyConditions::cloneAndFix(
    TR::CodeGenerator *cg, TR::RegisterDependencyConditions *added) {
  TR::RegisterDependencyConditions *result;
  TR::RegisterDependency *singlePair;
  ...
  postNum = this->getAddCursorForPost();
  result = new (cg->trHeapMemory())
      TR::RegisterDependencyConditions(0, postNum + addPost, cg->trMemory());
...
static TR::RegisterDependency *findDependencyChainHead(
    TR::RegisterDependency *dep, OMR::RegisterDependencyMap &map) {
  TR::RegisterDependency *cursor =
      map.getDependencyWithSourceAssigned(dep->getRealRegister());

  // Already at head of chain
  if (!cursor) return dep;

  // Follow the chain until we get to a dep who's target isn't assigned to
  // another dep or until we're back at the first dep
  while (cursor != dep) {
    TR::RegisterDependency *nextDep =
        map.getDependencyWithSourceAssigned(cursor->getRealRegister());
    if (!nextDep) break;
    cursor = nextDep;
  }
...
static void assignFreeRegisters(TR::Instruction *currentInstruction,
  ...
  // Assign a chain of dependencies where the head of the chain depends on a
  // free reg
  while (dep) {
    TR_ASSERT(machine->getRealRegister(dep->getRealRegister())->getState() ==
                  TR::RealRegister::Free,
              "Expecting free target register");
    TR::RealRegister *assignedReg =
        dep->getRegister()->getAssignedRealRegister()
            ? toRealRegister(dep->getRegister()->getAssignedRealRegister())
            : NULL;
    machine->coerceRegisterAssignment(currentInstruction, dep->getRegister(),
                                      dep->getRealRegister());
    dep->getRegister()->block();
    dep = assignedReg
              ? map.getDependencyWithTarget(assignedReg->getRegisterNumber())
              : NULL;
  }
}
...
static void assignContendedRegisters(TR::Instruction *currentInstruction,
                                     TR::RegisterDependency *dep,
                                     OMR::RegisterDependencyMap &map,
                                     bool depsBlocked, TR::CodeGenerator *cg) {
  TR::Machine *machine = cg->machine();
  ...
  // Chain of length 2, handled here instead of below to get 3*xor exchange on
  // GPRs
  ...
  // If it's blocked or assigned we allocate a spare and assign the target's
  // virtual to it If it's NoReg, the spare reg will be used as the first dep's
  // actual target
  TR::RealRegister *spareReg = machine->findBestFreeRegister(
      currentInstruction, virtReg->getKind(),
      targetRegNum == TR::RealRegister::NoReg ? dep->getExcludeGPR0() : false,
      false,
  ...
  if (!spareReg) {
    // If the regs in this dep group are not blocked we need to make sure we
    // don't spill a reg that's in the middle of the chain
    if (!depsBlocked) {
      if (targetRegNum == TR::RealRegister::NoReg)
        spareReg = machine->freeBestRegister(
            currentInstruction,
            map.getDependencyWithTarget(assignedReg->getRegisterNumber())
                ->getRegister(),
            assignedReg, false);
      else
        spareReg = machine->freeBestRegister(currentInstruction, virtReg,
                                             targetReg, false);
    } else {
      if (targetRegNum == TR::RealRegister::NoReg)
        spareReg = machine->freeBestRegister(currentInstruction, virtReg, NULL,
                                             dep->getExcludeGPR0());
      else
        spareReg = machine->freeBestRegister(
            currentInstruction, targetReg->getAssignedRegister(), NULL, false);
    }
  }

  if (targetRegNum != TR::RealRegister::NoReg && spareReg != targetReg) {
    machine->coerceRegisterAssignment(currentInstruction,
                                      targetReg->getAssignedRegister(),
                                      spareReg->getRegisterNumber());
  }

  TR_ASSERT(targetRegNum == TR::RealRegister::NoReg ||
                targetReg->getState() == TR::RealRegister::Free,
            "Expecting free target register");
  ...
  while (dep) {
    ...
    TR_ASSERT(targetReg->getState() == TR::RealRegister::Free ||
                  targetReg == spareReg,
              "Expecting free target register or target to have been filled to "
              "free spare register");
...
void TR_PPCRegisterDependencyGroup::assignRegisters(
    ...
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (dependentRegNum == TR::RealRegister::SpilledReg) {
        TR_ASSERT(
            ...
            "Should have a backing store if dependentRegNum == SpilledReg");
        if (virtReg->getAssignedRealRegister()) {
          ...
          if (comp->getOption(TR_TraceCG))
            traceMsg(comp,
                     "\nOOL: Found register spilled in main line and "
                     "re-assigned inside OOL");
          ...
          TR::MemoryReference *tempMR =
              new (cg->trHeapMemory()) TR::MemoryReference(
                  currentNode,
                  (TR::SymbolReference *)virtReg->getBackingStorage()
                      ->getSymbolReference(),
                  sizeof(uintptr_t), cg);
          ...
          switch (rk) {
            case TR_GPR:
              ...
              break;
            case TR_FPR:
              opCode = virtReg->isSinglePrecision() ? TR::InstOpCode::lfs
                                                    : TR::InstOpCode::lfd;
              break;
            default:
          ...
          TR::Instruction *inst = generateTrg1MemInstruction(
              cg, opCode, currentNode, assignedReg, tempMR, currentInstruction);
  ...
  // If spilled CCRs are present in the dependencies and we are not blocking
  // GPRs (because we may not have any spare ones) we will assign the CCRs
  // first. This is because in order to assign a spilled virtual to a physical
  // CCR we need to reverse spill it to a GPR first and then assign it to the
  // CCR, so we do that first to avoid having all the GPRs become assigned and
  // blocked. Note: It is safe to assign CCRs before GPRs in any circumstance,
  // but the extra checks are cheap and let us avoid an extra loop through the
  // dependencies for what should be a relatively infrequent situation anyway.
  if (haveSpilledCCRs && !haveSpareGPRs) {
    for (i = 0; i < numberOfRegisters; i++) {
      virtReg = _dependencies[i].getRegister();
      if (virtReg->getKind() != TR_CCR) continue;
      dependentRegNum = _dependencies[i].getRealRegister();
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (dependentRegNum != TR::RealRegister::NoReg &&
      ...
      assignContendedRegisters(currentInstruction, &_dependencies[i], map,
                               depsBlocked, cg);
    }
  }
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg &&
        _dependencies[i].getExcludeGPR0()) {
      TR::RealRegister *realOne;

      virtReg = _dependencies[i].getRegister();
      realOne = virtReg->getAssignedRealRegister();
      if (realOne != NULL && toRealRegister(realOne)->getRegisterNumber() ==
                                 TR::RealRegister::gr0) {
        if ((assignedRegister = machine->findBestFreeRegister(
                 currentInstruction, virtReg->getKind(), true, false,
                 virtReg)) == NULL) {
          assignedRegister = machine->freeBestRegister(currentInstruction,
                                                       virtReg, NULL, true);
        }
        machine->coerceRegisterAssignment(
            currentInstruction, virtReg, assignedRegister->getRegisterNumber());
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    // dependentRegister->getAssignedRegister() is NULL if the reg has already
    // been spilled due to a spilledReg dep
    if (comp->getOption(TR_DisableOOL) ||
        (!(cg->isOutOfLineColdPath()) && !(cg->isOutOfLineHotPath()))) {
      TR_ASSERT(dependentRegister->getAssignedRegister(),
                "Assigned register can not be NULL");
    }
    if (dependentRegister->getAssignedRegister()) {
      TR::RealRegister *assignedRegister =
          dependentRegister->getAssignedRegister()->getRealRegister();

      if (getRegisterDependency(i)->getRealRegister() ==
          TR::RealRegister::NoReg)
        getRegisterDependency(i)->setRealRegister(
...
TR::Register *OMR::Power::RegisterDependencyConditions::getTargetRegister(
    uint32_t index, TR::CodeGenerator *cg) {
  if (index >= _addCursorForPost) return NULL;

  return _postConditions->getRegisterDependency(index)->getRegister();
}

TR::Register *OMR::Power::RegisterDependencyConditions::getSourceRegister(
    uint32_t index) {
  if (index >= _addCursorForPre) return NULL;
