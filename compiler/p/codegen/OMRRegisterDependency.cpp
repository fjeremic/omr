...
void OMR::Power::RegisterDependencyConditions::bookKeepingRegisterUses(TR::Instruction* instr, TR::CodeGenerator* cg)
{
  ...
  // Register assigner can save/restore associations across outlined sections properly, however no such mechanism exists
  // for instruction selection so we don't want these associations to clobber the associations that were set in main
  // line code, which are more important
  ...
  OMR::RegisterDependencyMap& map, bool depsBlocked, TR::CodeGenerator* cg)
{
  ...
  if (!spareReg) {
    // If the regs in this dep group are not blocked we need to make sure we don't spill a reg that's in the middle of
    // the chain
    if (!depsBlocked) {
  ...
  TR_RegisterKinds kindToBeAssigned, uint32_t numberOfRegisters, TR::CodeGenerator* cg)
{
  ...
  if (haveSpilledCCRs && !haveSpareGPRs) {
    for (i = 0; i < numberOfRegisters; i++) {
      ...
      if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::SpilledReg
