...
{
    for (int i = 0; i < _addCursorForPre; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister()->getAssignedRegister() == r && _preConditions->getRegisterDependency(i)->getDefsRegister()) {
            return true;
        }
    }
    for (int j = 0; j < _addCursorForPost; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister()->getAssignedRegister() == r && _postConditions->getRegisterDependency(j)->getDefsRegister()) {
            return true;
        }
    }
    return false;
    ...
    TR::CodeGenerator* cg)
{
    ...
    while (dep) {
        ...
        TR::RealRegister* assignedReg = dep->getRegister()->getAssignedRealRegister() ? toRealRegister(dep->getRegister()->getAssignedRealRegister()) : NULL;
    ...
    TR::CodeGenerator* cg)
{
    ...
    TR::RealRegister* assignedReg = virtReg->getAssignedRealRegister() ? toRealRegister(virtReg->getAssignedRealRegister()) : NULL;
    ...
    TR_ASSERT(targetRegNum == TR::RealRegister::NoReg || targetReg->getState() == TR::RealRegister::Free, "Expecting free target register");
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (haveSpilledCCRs && !haveSpareGPRs) {
        for (i = 0; i < numberOfRegisters; i++) {
            ...
            if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::SpilledReg && dependentRealReg->getState() == TR::RealRegister::Free) {
    ...
    for (i = 0; i < numberOfRegisters; i++) {
        ...
        dependentRegNum = _dependencies[i].getRealRegister();
        dependentRealReg = machine->getRealRegister(dependentRegNum);
        if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::SpilledReg && dependentRealReg != assignedRegister) {
