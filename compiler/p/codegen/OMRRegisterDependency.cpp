    ...
    if (regList.find(reg) || (highReg && regList.find(highReg))) {
      ...
      if (regList.find(reg)) {
        ...
        if (opCode == TR::InstOpCode::vor || opCode == TR::InstOpCode::xxlor)
          iCursor = generateTrg1Src2Instruction(
            cg, opCode, node, copyReg, reg, reg, iCursor);
        else
          iCursor = generateTrg1Src1Instruction(
            cg, opCode, node, copyReg, reg, iCursor);
      ...
      if (highReg && regList.find(highReg)) {
        ...
        if (opCode == TR::InstOpCode::vor || opCode == TR::InstOpCode::xxlor)
          iCursor = generateTrg1Src2Instruction(
            cg, opCode, node, highCopyReg, highReg, highReg, iCursor);
        else
          iCursor = generateTrg1Src1Instruction(
...
void
OMR::Power::RegisterDependencyConditions::unionNoRegPostCondition(
  TR::Register* reg,
  TR::CodeGenerator* cg)
{
...
bool
OMR::Power::RegisterDependencyConditions::refsRegister(TR::Register* r)
{
...
bool
OMR::Power::RegisterDependencyConditions::defsRegister(TR::Register* r)
{
  for (int i = 0; i < _addCursorForPre; i++) {
    if (_preConditions->getRegisterDependency(i)->getRegister() == r &&
...
bool
OMR::Power::RegisterDependencyConditions::defsRealRegister(TR::Register* r)
{
  for (int i = 0; i < _addCursorForPre; i++) {
...
bool
OMR::Power::RegisterDependencyConditions::usesRegister(TR::Register* r)
{
  for (int i = 0; i < _addCursorForPre; i++) {
    if (_preConditions->getRegisterDependency(i)->getRegister() == r &&
...
void
OMR::Power::RegisterDependencyConditions::bookKeepingRegisterUses(
  ...
  TR::CodeGenerator* cg)
{
  ...
  if (numAssoc > 0) {
    ...
    generateDepInstruction(
      cg, TR::InstOpCode::assocreg, instr->getNode(), assoc, instr->getPrev());
  }
  ...
  TR::RegisterDependencyConditions* added)
{
  ...
  result = new (cg->trHeapMemory())
    TR::RegisterDependencyConditions(getNumPreConditions() + addPre,
                                     getNumPostConditions() + addPost,
                                     cg->trMemory());
...
static void
assignFreeRegisters(TR::Instruction* currentInstruction,
                    ...
                    TR::CodeGenerator* cg)
{
  ...
  while (dep) {
    ...
    machine->coerceRegisterAssignment(
      currentInstruction, dep->getRegister(), dep->getRealRegister());
...
static void
assignContendedRegisters(TR::Instruction* currentInstruction,
                         TR::RegisterDependency* dep,
                         OMR::RegisterDependencyMap& map,
                         bool depsBlocked,
                         TR::CodeGenerator* cg)
{
  ...
  if (!assignedReg ||
      !map.getDependencyWithTarget(assignedReg->getRegisterNumber())) {
    machine->coerceRegisterAssignment(
      currentInstruction, virtReg, targetRegNum);
    virtReg->block();
    return;
  }
  ...
  TR::RealRegister* spareReg = machine->findBestFreeRegister(
    ...
    false,
    targetRegNum == TR::RealRegister::NoReg ? virtReg
  ...
  if (!spareReg) {
    ...
    if (!depsBlocked) {
      if (targetRegNum == TR::RealRegister::NoReg)
        ...
        spareReg = machine->freeBestRegister(
          currentInstruction, virtReg, targetReg, false);
    } else {
      if (targetRegNum == TR::RealRegister::NoReg)
        spareReg = machine->freeBestRegister(
          currentInstruction, virtReg, NULL, dep->getExcludeGPR0());
      else
        spareReg = machine->freeBestRegister(
...
void
TR_PPCRegisterDependencyGroup::assignRegisters(
  ...
  for (i = 0; i < numberOfRegisters; i++) {
    ...
    if (dependentRegNum != TR::RealRegister::NoReg &&
      ...
      assignContendedRegisters(
        currentInstruction, &_dependencies[i], map, depsBlocked, cg);
    }
  }
    ...
    if (_dependencies[i].getRealRegister() == TR::RealRegister::NoReg &&
      ...
      if (realOne != NULL && toRealRegister(realOne)->getRegisterNumber() ==
        ...
        if ((assignedRegister = machine->findBestFreeRegister(
               currentInstruction, virtReg->getKind(), true, false, virtReg)) ==
            NULL) {
          assignedRegister =
            machine->freeBestRegister(currentInstruction, virtReg, NULL, true);
        }
        machine->coerceRegisterAssignment(
