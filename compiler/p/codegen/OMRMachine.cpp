  ...
  bool excludeGPR0, bool considerUnlatched, TR::Register* virtualReg)
{
  ...
  bool avoidGPR0
    = (virtualReg != NULL ? virtualReg->containsCollectedReference() || virtualReg->containsInternalPointer() : false);
...

{
  ...
  case TR_CCR:
    if (false /*!cg()->comp()->getOption(TR_DisableOOL) && (cg()->isOutOfLineColdPath() || cg()->isOutOfLineHotPath()) &&
      ...
      if (candidates[0]->getBackingStorage()) {
        ...
        location = new (self()->cg()->trHeapMemory())
          TR_PPCCRBackingStore(comp, self()->cg()->allocateSpill(4, false, NULL)); // TODO: Could this be 1 byte?
      }
    }
    break;
  case TR_VSX_VECTOR:
  ...
  tmemref = new (self()->cg()->trHeapMemory()) TR::MemoryReference(
    currentNode, location->getSymbolReference(), TR::Compiler->om.sizeofReferenceAddress(), self()->cg());
  ...
  if (rk == TR_CCR) {
    ...
    TR::Instruction* ccrInstr = generateTrg1ImmInstruction(self()->cg(), TR::InstOpCode::mtcrf, currentNode, crtemp,
  ...
  TR::Register* spilledRegister, TR::RealRegister* targetRegister, bool excludeGPR0)
{
  ...
  case TR_FPR:
    ...
    spillInstr
      = generateMemSrc1Instruction(self()->cg(), opCode, currentNode, tmemref, targetRegister, currentInstruction);
    break;
  case TR_VSX_SCALAR:
    tempIndexRegister = self()->findBestFreeRegister(currentInstruction, TR_GPR);
    if (tempIndexRegister == NULL)
  ...
  if (rk == TR_CCR) {
    ...
    if (sindex != tindex) {
      ...
      self()->cg()->traceRAInstruction(generateTrg1Src1Imm2Instruction(
        self()->cg(), TR::InstOpCode::rlwinm, currentNode, crtemp, crtemp, sindex, 0xFFFFFFFF, currentInstruction));
    }
    self()->cg()->traceRAInstruction(
      generateTrg1ImmInstruction(self()->cg(), TR::InstOpCode::mfcr, currentNode, crtemp, 0xFF, currentInstruction));
  }
  ...
  TR::Instruction* currentInstruction, TR::Register* virtualRegister, TR::RealRegister::RegNum registerNumber)
{
  ...
  if (targetRegister->getState() == TR::RealRegister::Free
    ...
    bool needTemp = !(
      (ctv_rk == TR_GPR) || (ctv_rk == TR_VRF) || (TR::Compiler->target.cpu.getPPCSupportsVSX() && ctv_rk != TR_CCR));
    ...
    if (targetRegister->getState() == TR::RealRegister::Blocked) {
      ...
      if (currentAssignedRegister != NULL && currentAssignedUseful) {
        ...
        registerExchange(currentInstruction, ctv_rk, targetRegister, currentAssignedRegister, spareReg, self()->cg());
...
void OMR::Power::Machine::initializeRegisterFile()
{
  ...
  _registerFile[TR::RealRegister::gr0] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr0, TR::RealRegister::gr0Mask, self()->cg());

  _registerFile[TR::RealRegister::gr1] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr1, TR::RealRegister::gr1Mask, self()->cg());

  _registerFile[TR::RealRegister::gr2] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr2, TR::RealRegister::gr2Mask, self()->cg());

  _registerFile[TR::RealRegister::gr3] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr3, TR::RealRegister::gr3Mask, self()->cg());

  _registerFile[TR::RealRegister::gr4] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr4, TR::RealRegister::gr4Mask, self()->cg());

  _registerFile[TR::RealRegister::gr5] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr5, TR::RealRegister::gr5Mask, self()->cg());

  _registerFile[TR::RealRegister::gr6] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr6, TR::RealRegister::gr6Mask, self()->cg());

  _registerFile[TR::RealRegister::gr7] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr7, TR::RealRegister::gr7Mask, self()->cg());

  _registerFile[TR::RealRegister::gr8] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr8, TR::RealRegister::gr8Mask, self()->cg());

  _registerFile[TR::RealRegister::gr9] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::gr9, TR::RealRegister::gr9Mask, self()->cg());
  ...
  _registerFile[TR::RealRegister::fp0] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp0, TR::RealRegister::fp0Mask, self()->cg());

  _registerFile[TR::RealRegister::fp1] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp1, TR::RealRegister::fp1Mask, self()->cg());

  _registerFile[TR::RealRegister::fp2] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp2, TR::RealRegister::fp2Mask, self()->cg());

  _registerFile[TR::RealRegister::fp3] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp3, TR::RealRegister::fp3Mask, self()->cg());

  _registerFile[TR::RealRegister::fp4] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp4, TR::RealRegister::fp4Mask, self()->cg());

  _registerFile[TR::RealRegister::fp5] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp5, TR::RealRegister::fp5Mask, self()->cg());

  _registerFile[TR::RealRegister::fp6] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp6, TR::RealRegister::fp6Mask, self()->cg());

  _registerFile[TR::RealRegister::fp7] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp7, TR::RealRegister::fp7Mask, self()->cg());

  _registerFile[TR::RealRegister::fp8] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp8, TR::RealRegister::fp8Mask, self()->cg());

  _registerFile[TR::RealRegister::fp9] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::fp9, TR::RealRegister::fp9Mask, self()->cg());
  ...
  _registerFile[TR::RealRegister::cr0] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr0, TR::RealRegister::cr0Mask, self()->cg());

  _registerFile[TR::RealRegister::cr1] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr1, TR::RealRegister::cr1Mask, self()->cg());

  _registerFile[TR::RealRegister::cr2] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr2, TR::RealRegister::cr2Mask, self()->cg());

  _registerFile[TR::RealRegister::cr3] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr3, TR::RealRegister::cr3Mask, self()->cg());

  _registerFile[TR::RealRegister::cr4] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr4, TR::RealRegister::cr4Mask, self()->cg());

  _registerFile[TR::RealRegister::cr5] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr5, TR::RealRegister::cr5Mask, self()->cg());

  _registerFile[TR::RealRegister::cr6] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr6, TR::RealRegister::cr6Mask, self()->cg());

  _registerFile[TR::RealRegister::cr7] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_CCR, 0, TR::RealRegister::Free, TR::RealRegister::cr7, TR::RealRegister::cr7Mask, self()->cg());
  ...
  _registerFile[TR::RealRegister::vr0] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr0, TR::RealRegister::vr0Mask, self()->cg());

  _registerFile[TR::RealRegister::vr1] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr1, TR::RealRegister::vr1Mask, self()->cg());

  _registerFile[TR::RealRegister::vr2] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr2, TR::RealRegister::vr2Mask, self()->cg());

  _registerFile[TR::RealRegister::vr3] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr3, TR::RealRegister::vr3Mask, self()->cg());

  _registerFile[TR::RealRegister::vr4] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr4, TR::RealRegister::vr4Mask, self()->cg());

  _registerFile[TR::RealRegister::vr5] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr5, TR::RealRegister::vr5Mask, self()->cg());

  _registerFile[TR::RealRegister::vr6] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr6, TR::RealRegister::vr6Mask, self()->cg());

  _registerFile[TR::RealRegister::vr7] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr7, TR::RealRegister::vr7Mask, self()->cg());

  _registerFile[TR::RealRegister::vr8] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr8, TR::RealRegister::vr8Mask, self()->cg());

  _registerFile[TR::RealRegister::vr9] = new (self()->cg()->trHeapMemory())
    TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::vr9, TR::RealRegister::vr9Mask, self()->cg());
...
TR::RealRegister** OMR::Power::Machine::cloneRegisterFileByType(TR::RealRegister** registerFileClone,
  TR::RealRegister** registerFile, int32_t start, int32_t end, TR_RegisterKinds kind, TR_AllocationKind allocKind)
{
  TR_LiveRegisters* liveRegs = self()->cg()->getLiveRegisters(kind);
  if (liveRegs && liveRegs->getNumberOfLiveRegisters() > 0) {
    for (int32_t i = start; i <= end; i++) {
      registerFileClone[i]
        = (TR::RealRegister*)self()->cg()->trMemory()->allocateMemory(sizeof(TR::RealRegister), allocKind);
...
TR::RealRegister** OMR::Power::Machine::cloneRegisterFile(TR::RealRegister** registerFile, TR_AllocationKind allocKind)
{
  int32_t arraySize = sizeof(TR::RealRegister*) * TR::RealRegister::NumRegisters;
  TR::RealRegister** registerFileClone
    = (TR::RealRegister**)self()->cg()->trMemory()->allocateMemory(arraySize, allocKind);
  registerFileClone = self()->cloneRegisterFileByType(
    registerFileClone, registerFile, TR::RealRegister::FirstGPR, TR::RealRegister::LastGPR, TR_GPR, allocKind);
  registerFileClone = self()->cloneRegisterFileByType(
    registerFileClone, registerFile, TR::RealRegister::FirstFPR, TR::RealRegister::LastFPR, TR_FPR, allocKind);
  registerFileClone = self()->cloneRegisterFileByType(
    registerFileClone, registerFile, TR::RealRegister::FirstCCR, TR::RealRegister::LastCCR, TR_CCR, allocKind);
  registerFileClone = self()->cloneRegisterFileByType(
    registerFileClone, registerFile, TR::RealRegister::FirstVRF, TR::RealRegister::LastVRF, TR_VRF, allocKind);
  ...
  TR::RealRegister* sourceReg, TR::CodeGenerator* cg)
{
  ...
  case TR_GPR:
    instr
      = generateTrg1Src1Instruction(cg, TR::InstOpCode::mr, currentNode, targetReg, sourceReg, precedingInstruction);
    break;
  case TR_FPR:
    if (useVSXLogical)
      ...
      instr
        = generateTrg1Src1Instruction(cg, TR::InstOpCode::fmr, currentNode, targetReg, sourceReg, precedingInstruction);
    break;
  case TR_CCR:
    instr
      = generateTrg1Src1Instruction(cg, TR::InstOpCode::mcrf, currentNode, targetReg, sourceReg, precedingInstruction);
    break;
  case TR_VSX_SCALAR:
  case TR_VSX_VECTOR:
    instr = generateTrg1Src2Instruction(
  ...
  TR::RealRegister* sourceReg, TR::RealRegister* middleReg, TR::CodeGenerator* cg)
{
  ...
  if (middleReg != NULL) {
    ...
    cg->traceRAInstruction(
      generateTrg1Src2Instruction(cg, opCode, currentNode, targetReg, targetReg, sourceReg, precedingInstruction));
    cg->traceRAInstruction(
      generateTrg1Src2Instruction(cg, opCode, currentNode, sourceReg, targetReg, sourceReg, precedingInstruction));
    cg->traceRAInstruction(
      generateTrg1Src2Instruction(cg, opCode, currentNode, targetReg, targetReg, sourceReg, precedingInstruction));
  }
}
  ...
  if (virtualRegister->getFutureUseCount() == 0 && location != NULL && self()->cg()->isOutOfLineColdPath()) {
    ...
    if (trace)
      traceMsg(comp, "\nFreeing backing storage " POINTER_PRINTF_FORMAT " of size %u from dead virtual %s\n", location,
        size, cg->getDebug()->getName(virtualRegister));
