    ...
    TR::Register* virtualReg)
{
    ...
    bool avoidGPR0 = (virtualReg != NULL ? virtualReg->containsCollectedReference() || virtualReg->containsInternalPointer() : false);
    ...
    if (preference != 0 && (!excludeGPR0 || preference != TR::RealRegister::gr0) && (_registerFile[preference]->getState() == TR::RealRegister::Free || (considerUnlatched && _registerFile[preference]->getState() == TR::RealRegister::Unlatched))) {
    ...
    if (rk == TR_FPR || rk == TR_VSX_SCALAR || rk == TR_VSX_VECTOR || rk == TR_VRF) {
        ...
        if (rk != TR_VRF) {
            ...
            do {
                ...
                if ((_registerFile[_registerAllocationFPR[i]]->getState() == TR::RealRegister::Free || (considerUnlatched && _registerFile[_registerAllocationFPR[i]]->getState() == TR::RealRegister::Unlatched))
                    && (freeRegister == NULL || (iOld && !iNew) || ((iOld || !iNew) && !rollingAllocator && // Rolling: weight is not considered once used
        ...
        for (int i = first; i <= last; i++) {
            ...
            if ((_registerFile[i]->getState() == TR::RealRegister::Free || (considerUnlatched && _registerFile[i]->getState() == TR::RealRegister::Unlatched))
    ...
    TR::RealRegister::RegNum registerNumber)
{
    ...
    if (targetRegister->getState() == TR::RealRegister::Free
        ...
        bool currentAssignedUseful = (vr_rk == ctv_rk) || !(ctv_rk == TR_FPR || ctv_rk == TR_VRF) || (currentAssignedRegister != NULL && currentAssignedRegister->getKind() == ctv_rk);
...
void OMR::Power::Machine::decFutureUseCountAndUnlatch(TR::Register* virtualRegister)
{
    ...
    if (virtualRegister->getFutureUseCount() == 0 || (self()->cg()->isOutOfLineHotPath() && virtualRegister->getFutureUseCount() == virtualRegister->getOutOfLineUseCount())) {
