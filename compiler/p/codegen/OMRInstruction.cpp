...
OMR::Power::Instruction::Instruction(TR::CodeGenerator *cg,
...
void OMR::Power::Instruction::remove() {
...
TR::Register *OMR::Power::Instruction::getTargetRegister(uint32_t i) {
  return (_conditions == NULL)
             ? NULL
             : _conditions->getTargetRegister(i, self()->cg());
}
...
bool OMR::Power::Instruction::isCall() {
  return _opcode.isCall() ||
         (self()->getMemoryReference() &&
          self()->getMemoryReference()->getUnresolvedSnippet());
}
...
TR::Register *OMR::Power::Instruction::getMemoryDataRegister() {
  return (NULL);
}

bool OMR::Power::Instruction::refsRegister(TR::Register *reg) {
  return OMR::Power::Instruction::getDependencyConditions()
             ? OMR::Power::Instruction::getDependencyConditions()->refsRegister(
                   reg)
             : false;
}
...
bool OMR::Power::Instruction::defsRegister(TR::Register *reg) {
  return OMR::Power::Instruction::getDependencyConditions()
             ? OMR::Power::Instruction::getDependencyConditions()->defsRegister(
                   reg)
             : false;
}
...
bool OMR::Power::Instruction::defsRealRegister(TR::Register *reg) {
  return OMR::Power::Instruction::getDependencyConditions()
             ? OMR::Power::Instruction::getDependencyConditions()
                   ->defsRealRegister(reg)
             : false;
}

bool OMR::Power::Instruction::usesRegister(TR::Register *reg) {
  return OMR::Power::Instruction::getDependencyConditions()
             ? OMR::Power::Instruction::getDependencyConditions()->usesRegister(
                   reg)
             : false;
}
...
TR::PPCDepImmInstruction *OMR::Power::Instruction::getPPCDepImmInstruction() {
  return NULL;
}
...
TR::PPCImmInstruction *OMR::Power::Instruction::getPPCImmInstruction() {
  return NULL;
}
#endif

void OMR::Power::Instruction::assignRegisters(
    TR_RegisterKinds kindToBeAssigned) {
  if (OMR::Power::Instruction::getDependencyConditions()) {
    OMR::Power::Instruction::getDependencyConditions()
        ->assignPostConditionRegisters(self(), kindToBeAssigned, self()->cg());
    OMR::Power::Instruction::getDependencyConditions()
        ->assignPreConditionRegisters(self()->getPrev(), kindToBeAssigned,
                                      self()->cg());
  }
}

bool OMR::Power::Instruction::isBeginBlock() {
  return self()->getPrev()->getOpCodeValue() == TR::InstOpCode::label;
}

bool OMR::Power::Instruction::setsCountRegister() {
  if (_opcode.getOpCodeValue() == TR::InstOpCode::beql) return true;
  return self()->isCall() | _opcode.setsCountRegister();
}

bool OMR::Power::Instruction::is4ByteLoad() {
  return (self()->getOpCodeValue() == TR::InstOpCode::lwz);
}

int32_t OMR::Power::Instruction::getMachineOpCode() {
  return self()->getOpCodeValue();
}

uint8_t OMR::Power::Instruction::getBinaryLengthLowerBound() {
  return self()->getEstimatedBinaryLength();
}

int32_t TR::PPCDepImmSymInstruction::estimateBinaryLength(
...
uint8_t *TR::PPCDepImmSymInstruction::generateBinaryEncoding() {
  ...
  if (getOpCodeValue() == TR::InstOpCode::bl ||
      getOpCodeValue() == TR::InstOpCode::b) {
    if (cg()->comp()->isRecursiveMethodTarget(
      ...
      if (cg()->directCallRequiresTrampoline(targetAddress,
                                             (intptrj_t)cursor)) {
        ...
        if (refNum < TR_PPCnumRuntimeHelpers) {
          ...
          targetAddress = cg()->fe()->methodTrampolineLookup(
              cg()->comp(), getSymbolReference(), (void *)cursor);
        }
      }

      TR_ASSERT_FATAL(TR::Compiler->target.cpu.isTargetWithinIFormBranchRange(
    ...
    // Place holder only: non-TR::InstOpCode::b[l] usage of this instruction
