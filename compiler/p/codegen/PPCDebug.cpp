...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCAdminInstruction* instr)
{
  ...
  if (instr->getOpCodeValue() == TR::InstOpCode::fence && instr->getFenceNode() != NULL) {
    trfprintf(
      pOutFile, "\t%s[", (instr->getFenceNode()->getRelocationType() == TR_AbsoluteAddress) ? "Absolute" : "Relative");
...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCDepImmSymInstruction* instr)
{
  ...
  if (name)
    trfprintf(pOutFile, "%s \t" POINTER_PRINTF_FORMAT "\t\t; Direct Call \"%s\"", getOpCodeName(&instr->getOpCode()),
      (intptrj_t)cursor + distance, name);
  else
    trfprintf(
      pOutFile, "%s \t" POINTER_PRINTF_FORMAT, getOpCodeName(&instr->getOpCode()), (intptrj_t)cursor + distance);
...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCTrg1Src1ImmInstruction* instr)
{
  ...
  if (op == TR::InstOpCode::subfic || op == TR::InstOpCode::addi || op == TR::InstOpCode::addi2
    || op == TR::InstOpCode::addic || op == TR::InstOpCode::addic_r || op == TR::InstOpCode::addis
