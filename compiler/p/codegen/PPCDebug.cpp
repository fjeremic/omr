...
TR_Debug::print(TR::FILE* pOutFile, TR::PPCAdminInstruction* instr)
{
  ...
  if (instr->getOpCodeValue() == TR::InstOpCode::fence &&
    ...
    for (i = 0; i < instr->getFenceNode()->getNumRelocations(); i++) {
      trfprintf(
        pOutFile, " %08x", instr->getFenceNode()->getRelocationDestination(i));
    }
    trfprintf(pOutFile, " ]");
  }
...
void
TR_Debug::printPPCOOLSequences(TR::FILE* pOutFile)
{
