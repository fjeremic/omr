...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::PPCAlignmentNopInstruction *instr) {
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s\t; Align to %u bytes",
            getOpCodeName(&instr->getOpCode()), instr->getAlignment());
  trfflush(pOutFile);
}
    ...
    if (snippet) {
      ...
      switch (snippet->getKind()) {
        ...
        case TR::Snippet::IsUnresolvedCall:
          callSym =
              ((TR::PPCCallSnippet *)snippet)->getNode()->getSymbolReference();
          break;
      ...
      if (callSym) trfprintf(pOutFile, "\t; Call \"%s\"", getName(callSym));
    }
  }
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::PPCConditionalBranchInstruction *instr) {
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s \t", getOpCodeName(&instr->getOpCode()));
  ...
  if (instr->haveHint())
    trfprintf(pOutFile, " # with prediction hints: %s",
              instr->getLikeliness() ? "Likely" : "Unlikely");
  trfflush(_comp->getOutFile());
}

void TR_Debug::print(TR::FILE *pOutFile,
                     TR::PPCDepConditionalBranchInstruction *instr) {
  print(pOutFile, (TR::PPCConditionalBranchInstruction *)instr);
  if (instr->getDependencyConditions())
    print(pOutFile, instr->getDependencyConditions());
  trfflush(_comp->getOutFile());
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCAdminInstruction *instr) {
  ...
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s ", getOpCodeName(&instr->getOpCode()));
  if (instr->isDebugFence()) trfprintf(pOutFile, "DBG");
  if (instr->getOpCodeValue() == TR::InstOpCode::fence &&
      instr->getFenceNode() != NULL) {
    trfprintf(pOutFile, "\t%s[",
              (instr->getFenceNode()->getRelocationType() == TR_AbsoluteAddress)
                  ? "Absolute"
                  : "Relative");
    for (i = 0; i < instr->getFenceNode()->getNumRelocations(); i++) {
      trfprintf(pOutFile, " %08x",
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCSrc1Instruction *instr) {
  ...
  if (!(instr->getOpCodeValue() == TR::InstOpCode::mtlr ||
        instr->getOpCodeValue() == TR::InstOpCode::mtctr))
    trfprintf(pOutFile, ", " POINTER_PRINTF_FORMAT,
              (intptrj_t)(int32_t)instr->getSourceImmediate());

  trfflush(_comp->getOutFile());
}

void TR_Debug::print(TR::FILE *pOutFile, TR::PPCDepImmInstruction *instr) {
  printPrefix(pOutFile, instr);
...
bool TR_Debug::isBranchToTrampoline(TR::SymbolReference *symRef,
  ...
  if (_cg->directCallRequiresTrampoline(methodAddress, (intptrj_t)cursor)) {
    methodAddress = TR::CodeCacheManager::instance()->findHelperTrampoline(
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCDepImmSymInstruction *instr) {
  ...
  if (cursor) {
    if (label) {
    ...
    } else if (_cg->directCallRequiresTrampoline(targetAddress,
                                                 (intptrj_t)cursor)) {
      ...
      if (refNum < TR_PPCnumRuntimeHelpers) {
        ...
        targetAddress = _comp->fe()->methodTrampolineLookup(
            _comp, instr->getSymbolReference(), (void *)cursor);
      }
    }
  }
  ...
  if (name)
    trfprintf(
        ...
        getOpCodeName(&instr->getOpCode()), (intptrj_t)cursor + distance, name);
  else
    trfprintf(pOutFile, "%s \t" POINTER_PRINTF_FORMAT,
              getOpCodeName(&instr->getOpCode()), (intptrj_t)cursor + distance);
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCTrg1Src1ImmInstruction *instr) {
  ...
  if (op == TR::InstOpCode::subfic || op == TR::InstOpCode::addi ||
      op == TR::InstOpCode::addi2 || op == TR::InstOpCode::addic ||
      op == TR::InstOpCode::addic_r || op == TR::InstOpCode::addis ||
      op == TR::InstOpCode::mulli)
...
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::PPCTrg1Src1Imm2Instruction *instr) {
  uint64_t lmask;
  printPrefix(pOutFile, instr);
  trfprintf(pOutFile, "%s \t", getOpCodeName(&instr->getOpCode()));
  print(pOutFile, instr->getTargetRegister(), TR_WordReg);
  ...
  if (TR::Compiler->target.is64Bit())
    trfprintf(pOutFile, ", " POINTER_PRINTF_FORMAT ", " POINTER_PRINTF_FORMAT,
              instr->getSourceImmediate(), lmask);
  else
    trfprintf(pOutFile, ", " POINTER_PRINTF_FORMAT ", 0x%x",
              instr->getSourceImmediate(), (uint32_t)lmask);
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCMemSrc1Instruction *instr) {
  ...
  if (instr->getSourceRegister()->getRealRegister())
    toRealRegister(instr->getSourceRegister())
        ->setUseVSR(instr->getOpCode().isVSX());
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCTrg1MemInstruction *instr) {
  ...
  // Use the VSR name instead of the FPR/VR
  if (instr->getTargetRegister()->getRealRegister())
    toRealRegister(instr->getTargetRegister())
        ->setUseVSR(instr->getOpCode().isVSX());
  ...
  print(pOutFile, instr->getMemoryReference(),
        strncmp("addi", getOpCodeName(&instr->getOpCode()), 4) != 0);
  TR::Symbol *symbol =
...
void TR_Debug::print(TR::FILE *pOutFile, TR::PPCControlFlowInstruction *instr) {
  ...
  for (i = 0; i < numTargets; i++) {
    ...
    if (i != numTargets + numSources - 1) trfprintf(pOutFile, ", ");
  }
  for (i = 0; i < numSources; i++) {
    ...
    if (i != numSources - 1) trfprintf(pOutFile, ", ");
  }
...
#ifdef J9_PROJECT_SPECIFIC
void TR_Debug::print(TR::FILE *pOutFile,
                     TR::PPCVirtualGuardNOPInstruction *instr) {
  printPrefix(pOutFile, instr);
...
void TR_Debug::printPPCGCRegisterMap(TR::FILE *pOutFile,
...
static const char *getRegisterName(TR::RealRegister::RegNum num,
                                   bool isVSR = false) {
  switch (num) {
