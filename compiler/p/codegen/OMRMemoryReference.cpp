...
OMR::Power::MemoryReference::MemoryReference(TR::Register* br, int32_t disp, uint8_t len, TR::CodeGenerator* cg)
  : _baseRegister(br)
  , _baseNode(NULL)
  , _indexRegister(NULL)
  , _indexNode(NULL)
  ...
  TR::Node* node, TR::MemoryReference& mr, int32_t displacement, uint32_t len, TR::CodeGenerator* cg)
{
  ...
  if (mr.getUnresolvedSnippet() != NULL) {
    _unresolvedSnippet = new (cg->trHeapMemory())
      TR::UnresolvedDataSnippet(cg, node, _symbolReference, mr.getUnresolvedSnippet()->isUnresolvedStore(), false);
  ...
  if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
    ...
    if (subTree->getOpCode().isArrayRef()
      ...
      if (integerChild->getOpCode().isLoadConst()) {
        ...
        if (TR::Compiler->target.is64Bit()) {
          intptrj_t amount
            = (integerChild->getOpCodeValue() == TR::iconst) ? integerChild->getInt() : integerChild->getLongInt();
...
void OMR::Power::MemoryReference::assignRegisters(TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
  ...
  if (_baseRegister != NULL) {
    ...
    if ((assignedBaseRegister != NULL)
      ...
      if ((alternativeRegister = machine->findBestFreeRegister(currentInstruction, TR_GPR, true, false, _baseRegister))
        == NULL) {
  ...
  TR::Instruction* currentInstruction, uint8_t* cursor, TR::CodeGenerator* cg)
{
  ...
  base = (self()->getBaseRegister() == NULL) ? cg->machine()->getRealRegister(TR::RealRegister::gr0)
  ...
  if (TR::Compiler->target.is64Bit() && self()->isTOCAccess()) {
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
      ...
      cg->addRelocation(
        new (cg->trHeapMemory()) TR::LabelRelative24BitRelocation(cursor, getUnresolvedSnippet()->getSnippetLabel()));
        ...
        *wcursor = preserve | lower;
        ...
        if (comp->compileRelocatableCode() && symbol->isStatic() && symbol->isClassObject()) {
          ...
          if (comp->getOption(TR_UseSymbolValidationManager)) {
            recordInfo->data1
              = (uintptr_t)self()->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress();
            recordInfo->data2 = (uintptr_t)TR::SymbolType::typeClass;
            recordInfo->data3 = fixedSequence1;
            cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(
                                        cursor, (uint8_t*)recordInfo, TR_DiscontiguousSymbolFromManager, cg),
            ...
            cg->addExternalRelocation(new (cg->trHeapMemory())
                                        TR::ExternalRelocation(cursor, (uint8_t*)recordInfo, TR_ClassAddress, cg),
              __FILE__, __LINE__, node);
          }
  ...
  TR::Node* node, TR::SymbolReference* ref, bool isStore, TR::CodeGenerator* cg)
{
  ...
  if (isPicSite && !cg->comp()->compileRelocatableCode()
    ...
    loadAddressConstantInSnippet(cg, node ? node : cg->getCurrentEvaluationTreeTop()->getNode(), address, reg, NULL,
      isStore ? TR::InstOpCode::Op_st : TR::InstOpCode::Op_load, false, NULL);
    return;
  }
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    TR::MemoryReference* tocRef
      = new (cg->trHeapMemory()) TR::MemoryReference(cg->getTOCBaseRegister(), 0, sizeof(uintptrj_t), cg);
    ...
    if (ref->isUnresolved() || comp->compileRelocatableCode()) {
      ...
      if (symbol->isDebugCounter()) {
        ...
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(
          cg, comp->compileRelocatableCode(), nodeForSymbol, TR_CountForRecompile, reg, NULL, false, TR_GlobalValue);
        return;
      } else if (symbol->isRecompilationCounter()) {
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(
          cg, comp->compileRelocatableCode(), nodeForSymbol, 0, reg, NULL, false, TR_BodyInfoAddressLoad);
        return;
      } else if (symbol->isCompiledMethod()) {
        TR::Register* reg = _baseRegister = cg->allocateRegister();
        loadAddressConstant(
          cg, comp->compileRelocatableCode(), nodeForSymbol, 0, reg, NULL, false, TR_RamMethodSequence);
        return;
      } else if (symbol->isStartPC()) {
        // use inSnippet, as the relocation mechanism is already set up there
        TR::Register* reg = _baseRegister = cg->allocateRegister();
    ...
    if (comp->compileRelocatableCode()) {
      ...
      staticRelocation
        = new (cg->trHeapMemory()) TR::PPCPairedRelocation(rel1, NULL, (uint8_t*)recordInfo, TR_ClassAddress, node);
