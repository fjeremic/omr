...
OMR::Power::MemoryReference::MemoryReference(TR::Node *rootLoadOrStore,
  ...
  if (rootLoadOrStore->getOpCode().isIndirect()) {
    ...
    if (!ref->isUnresolved() && isLocalObject) {
      ...
      if (ref->isUnresolved()) {
        ...
        // not correct.
        if (isLocalObject) cg->evaluate(base);
  ...
  if (self()->getUnresolvedSnippet() != NULL) self()->adjustForResolution(cg);
  // TODO: aliasing sets?
}
...
void OMR::Power::MemoryReference::addToOffset(TR::Node *node, intptrj_t amount,
  ...
  if (displacement < LOWER_IMMED || displacement > UPPER_IMMED) {
    ...
    if (!node) node = cg->getAppendInstruction()->getNode();
...
void OMR::Power::MemoryReference::forceIndexedForm(TR::Node *node,
  ...
  if (self()->useIndexedForm()) return;  // already X-form
  ...
  if (!cursor) cursor = cg->getAppendInstruction();
  if (!node) node = cursor->getNode();
...
void OMR::Power::MemoryReference::decNodeReferenceCounts(
  ...
  if (_modBase != NULL) cg->stopUsingRegister(_modBase);
}
  ...
  if (_baseRegister != NULL) {
    ...
    if ((assignedBaseRegister != NULL) &&
      ...
      if ((alternativeRegister = machine->findBestFreeRegister(
               currentInstruction, TR_GPR, true, false, _baseRegister)) ==
          NULL) {
...
void OMR::Power::MemoryReference::mapOpCode(
  ...
  if (index == NULL && !self()->isUsingDelayedIndexedForm()) return;
...
uint32_t OMR::Power::MemoryReference::estimateBinaryLength(
    TR::CodeGenerator &codeGen) {
  TR::Compilation *comp = codeGen.comp();
  if (self()->isTOCAccess()) {
    int32_t tocOffset = self()->getTOCOffset();
    if (tocOffset == PTOC_FULL_INDEX) return (5 * PPC_INSTRUCTION_LENGTH);
...
void OMR::Power::MemoryReference::accessStaticItem(TR::Node *node,
  ...
  if (useUnresSnippetToAvoidRelo) {
    ...
    if (reloIsRequired) useUnresSnippetToAvoidRelo = false;
  }
  ...
  if (TR::Compiler->target.is64Bit()) {
    ...
    generateTrg1MemInstruction(cg, loadOp, node == NULL ? topNode : node,
                               addrReg, tocRef);
    if (snippet != NULL) cg->stopUsingRegister(tocRef->getModBase());
