...
static bool isLoadConstAndShift(TR::Node* subTree, TR::CodeGenerator* cg)
{
    if (TR::Compiler->target.is64Bit()) {
        if (subTree->getOpCodeValue() == TR::lshl) {
            ...
            //    lconst
            //    iconst
            if (subTree->getFirstChild()->getOpCode().isLoadConst() && subTree->getSecondChild()->getOpCode().isLoadConst())
                return true;
            ...
            //       iconst
            //    iconst
            else if (subTree->getFirstChild()->getOpCodeValue() == TR::i2l && subTree->getFirstChild()->getFirstChild()->getOpCode().isLoadConst() && subTree->getSecondChild()->getOpCode().isLoadConst())
                return true;
            else
                return false;
        } else
    ...
    {
        if (subTree->getOpCodeValue() == TR::ishl && subTree->getFirstChild()->getOpCode().isLoadConst() && subTree->getSecondChild()->getOpCode().isLoadConst())
            return true;
        else
            return false;
    }
...
void OMR::Power::MemoryReference::populateMemoryReference(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
        ...
        if (subTree->getOpCode().isArrayRef() || (subTree->getOpCodeValue() == TR::iadd || subTree->getOpCodeValue() == TR::ladd)) {
            ...
            if (integerChild->getOpCode().isLoadConst()) {
                ...
                if (TR::Compiler->target.is64Bit()) {
                    intptrj_t amount = (integerChild->getOpCodeValue() == TR::iconst) ? integerChild->getInt() : integerChild->getLongInt();
                    self()->addToOffset(integerChild, amount, cg);
                } else
                    self()->addToOffset(subTree, integerChild->getInt(), cg);
                cg->decReferenceCount(integerChild);
            } else if (integerChild->getEvaluationPriority(cg) > addressChild->getEvaluationPriority(cg) && !(subTree->getOpCode().isArrayRef() && TR::Compiler->target.cpu.id() == TR_PPCp6)) {
            ...
            if (TR::Compiler->target.is64Bit()) { // 64-bit
                intptrj_t amount = (subTree->getSecondChild()->getOpCodeValue() == TR::iconst) ? subTree->getSecondChild()->getInt() : subTree->getSecondChild()->getLongInt();
                // lshl
                //    lconst
                //    iconst(lconst)
        ...
        } else if (subTree->getOpCodeValue() == TR::aconst || subTree->getOpCodeValue() == TR::iconst || // subTree->getOpCode().isLoadConst ?
            subTree->getOpCodeValue() == TR::lconst) {
            intptrj_t amount = (subTree->getOpCodeValue() == TR::iconst) ? subTree->getInt() : subTree->getLongInt();
...
void OMR::Power::MemoryReference::consolidateRegisters(TR::Register* srcReg, TR::Node* srcTree, bool srcModifiable, TR::CodeGenerator* cg)
{
    ...
    if (self()->getUnresolvedSnippet() != NULL) {
        ...
        if (_indexRegister != NULL) {
            ...
            else if ((srcReg && (srcReg->containsCollectedReference() || srcReg->containsInternalPointer())) || (_indexRegister && (_indexRegister->containsCollectedReference() || _indexRegister->containsInternalPointer()))) {
        ...
        if (_indexRegister != NULL) {
            ...
            else if ((_baseRegister && (_baseRegister->containsCollectedReference() || _baseRegister->containsInternalPointer())) || (_indexRegister && (_indexRegister->containsCollectedReference() || _indexRegister->containsInternalPointer()))) {
                if (srcTree != NULL && srcTree->isInternalPointer() && srcTree->getPinningArrayPointer()) {
                    tempTargetRegister = cg->allocateRegister();
                    tempTargetRegister->setContainsInternalPointer();
                    tempTargetRegister->setPinningArrayPointer(srcTree->getPinningArrayPointer());
            ...
            else if ((srcReg && (srcReg->containsCollectedReference() || srcReg->containsInternalPointer())) || (_baseRegister && (_baseRegister->containsCollectedReference() || _baseRegister->containsInternalPointer()))) {
                if (srcTree != NULL && srcTree->isInternalPointer() && srcTree->getPinningArrayPointer()) {
                    tempTargetRegister = cg->allocateRegister();
                    tempTargetRegister->setContainsInternalPointer();
                    tempTargetRegister->setPinningArrayPointer(srcTree->getPinningArrayPointer());
...
void OMR::Power::MemoryReference::assignRegisters(TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    ...
    if (_baseRegister != NULL) {
        ...
        if ((assignedBaseRegister != NULL) && (toRealRegister(assignedBaseRegister)->getRegisterNumber() == TR::RealRegister::gr0)) {
    ...
    if (TR::Compiler->target.is64Bit() && self()->isTOCAccess()) {
        ...
        if (self()->getUnresolvedSnippet() != NULL) {
            ...
            *wcursor = 0x48000000; // b SnippetLabel;
            ...
            uint64_t addr = symbol->isStatic() ? (uint64_t)symbol->getStaticSymbol()->getStaticAddress() : (uint64_t)symbol->getMethodSymbol()->getMethodAddress();
            if (displacement != PTOC_FULL_INDEX) {
    ...
    if (index != NULL) {
        ...
        if (self()->isUsingDelayedIndexedForm()) {
            ...
            *wcursor = preserve;
            // just use 0 as RA, base as RB
            base->setRegisterFieldRB(wcursor);
        } else if (displacement < LOWER_IMMED || displacement > UPPER_IMMED || (self()->offsetRequireWordAlignment() && (displacement & 3 != 0))) {
            if (self()->isBaseModifiable()) {
                *wcursor = 0x3c000000 | upper;
                base->setRegisterFieldRT(wcursor);
                base->setRegisterFieldRA(wcursor);
...
static TR::RealRegister::RegNum choose_rX(TR::Instruction* currentInstruction, TR::RealRegister* base)
{
    ...
    int32_t exclude = ((TSnum == TR::RealRegister::gr10) ? 1 : 0) | ((TSnum == TR::RealRegister::gr9) ? 2 : 0) | ((basenum == TR::RealRegister::gr10) ? 1 : 0) | ((basenum == TR::RealRegister::gr9) ? 2 : 0);
    switch (exclude) {
    case 0:
    case 2:
...
void OMR::Power::MemoryReference::accessStaticItem(TR::Node* node, TR::SymbolReference* ref, bool isStore, TR::CodeGenerator* cg)
{
    ...
    if (useUnresSnippetToAvoidRelo) {
        ...
        bool reloIsRequired = symbol->isStatic()
            && symbol->isClassObject()
            && ref->getCPIndex() == -1;
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if ((ref->isUnresolved() || useUnresSnippetToAvoidRelo) && (topNode->getOpCodeValue() == TR::ResolveCHK || tocIndex == PTOC_FULL_INDEX)) {
        ...
        addr = symbol->isStatic() ? (intptrj_t)symbol->getStaticSymbol()->getStaticAddress() : (intptrj_t)symbol->getMethodSymbol()->getMethodAddress();
...
void OMR::Power::MemoryReference::setSymbol(
    TR::Symbol* symbol,
    TR::CodeGenerator* cg)
{
    ...
    if (_baseRegister != NULL && _indexRegister != NULL && (self()->hasDelayedOffset() || self()->getOffset(*cg->comp()) != 0)) {
        self()->consolidateRegisters(NULL, NULL, false, cg);
    }
}
