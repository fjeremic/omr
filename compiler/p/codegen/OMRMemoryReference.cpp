    ...
    if (subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL) {
        ...
        if (subTree->getOpCode().isArrayRef()
            ...
            if (integerChild->getOpCode().isLoadConst()) {
                ...
                if (TR::Compiler->target.is64Bit()) {
                    intptrj_t amount = (integerChild->getOpCodeValue() == TR::iconst) ? integerChild->getInt()
    ...
    if (TR::Compiler->target.is64Bit() && self()->isTOCAccess()) {
        ...
        if (self()->getUnresolvedSnippet() != NULL) {
            ...
            cg->addRelocation(new (cg->trHeapMemory())
                    TR::LabelRelative24BitRelocation(cursor, getUnresolvedSnippet()->getSnippetLabel()));
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if (ref->isUnresolved() || comp->compileRelocatableCode()) {
            ...
            if (symbol->isDebugCounter()) {
                ...
                TR::Register* reg = _baseRegister = cg->allocateRegister();
                loadAddressConstant(cg, comp->compileRelocatableCode(), nodeForSymbol, TR_CountForRecompile, reg, NULL,
                    false, TR_GlobalValue);
                return;
            } else if (symbol->isRecompilationCounter()) {
                TR::Register* reg = _baseRegister = cg->allocateRegister();
                loadAddressConstant(
                ...
                TR::Register* reg = _baseRegister = cg->allocateRegister();
                loadAddressConstant(
                    cg, comp->compileRelocatableCode(), nodeForSymbol, 1, reg, NULL, false, TR_DataAddress);
                return;
